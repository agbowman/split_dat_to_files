CREATE PROGRAM dm_ce_event_action_rows:dba
 ...
 SET reply->table_name = "CE_EVENT_ACTION"
 ...
 DECLARE action_type_order = f8 WITH constant(uar_get_code_by("MEANING",21,"ORDER"))
 IF (action_type_order <= 0.0)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"ACTION_TYPE_ORDER",
   "Unable to find 'ORDER' code value in code set 21.")
 ...
 ELSEIF (v_unit=1
  AND v_value < 7)
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,value,
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(cea.ce_event_action_id)
   FROM ce_event_action cea
   WHERE cea.ce_event_action_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(cea.ce_event_action_id)
  FROM ce_event_action cea
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    cea.rowid
    FROM ce_event_action cea
    WHERE cea.ce_event_action_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists("cea.updt_dt_tm < cnvtdatetime(target_dt_tm)","cea"))
     AND cea.action_type_cd=action_type_order
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = cea.rowid
    WITH nocounter, maxqual(cea,value(v_rows_left))
   ;end select
