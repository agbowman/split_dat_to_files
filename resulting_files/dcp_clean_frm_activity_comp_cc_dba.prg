CREATE PROGRAM dcp_clean_frm_activity_comp_cc:dba
 ...
 SET ms_child_c_info_domain_nm = "DELETING DUPS ON DCP_FORMS_ACTIVITY_COMP TABLE"
 SET ms_child_c_max_name = "MAX DCP_FORMS_ACTIVITY_ID EVALUATED"
 CALL sbr_get_min_max(mf_min_id,mf_max_id)
 CALL echo("Processing...")
 CALL echo("Deleting duplicate rows on dcp_forms_activity_comp...")
 CALL echo(concat(trim(cnvtstring(mf_min_id,20,2),3)," thru ",trim(cnvtstring(mf_max_id,20,2),3)))
 CALL parser(
  " rdb asis(^   delete from dcp_forms_activity_comp dfac                                   ^)")
 IF ((((readme_data->readme_id=4510)) OR (mf_readme_num=4)) )
  CALL parser(
   "asis(^       where dfac.dcp_forms_activity_id in                                       ^)")
  CALL parser(
   "asis(^        ( select distinct dfac3.dcp_forms_activity_id                            ^)")
  CALL parser(
   "asis(^            from dcp_forms_activity_comp dfac3                                   ^)")
  CALL parser(concat("asis(^    where dfac3.dcp_forms_activity_comp_id >= ",trim(cnvtstring(mf_min_id,
      20,2),3),"   ^)"))
  CALL parser(concat("asis(^      and dfac3.dcp_forms_activity_comp_id <= ",trim(cnvtstring(mf_max_id,
      20,2),3)," ) ^)"))
 ELSE
  CALL parser(concat("asis(^ where dfac.dcp_forms_activity_id >= ",trim(cnvtstring(mf_min_id,20,2),3),
    "            ^)"))
  CALL parser(concat("asis(^   and dfac.dcp_forms_activity_id <= ",trim(cnvtstring(mf_max_id,20,2),3),
    "            ^)"))
 ENDIF
 CALL parser(
  "asis(^          and dfac.dcp_forms_activity_comp_id not in                             ^)")
 CALL parser(
  "asis(^             ( select min(dfac2.dcp_forms_activity_comp_id)                      ^)")
 CALL parser(
  "asis(^                 from dcp_forms_activity_comp dfac2                              ^)")
 CALL parser(
  "asis(^                where dfac2.dcp_forms_activity_id =                              ^)")
 CALL parser(
  "asis(^                      dfac.dcp_forms_activity_id                                 ^)")
 CALL parser(
  "asis(^                  and dfac2.parent_entity_id+0 > 0                               ^)")
 CALL parser(
  "asis(^                group by dfac2.dcp_forms_activity_id,                            ^)")
 CALL parser(
  "asis(^                         dfac2.parent_entity_id,                                 ^)")
 CALL parser(
  "asis(^                         dfac2.component_cd )                                    ^)go")
