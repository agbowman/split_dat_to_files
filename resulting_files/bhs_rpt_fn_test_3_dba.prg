CREATE PROGRAM bhs_rpt_fn_test_3:dba
 PROMPT
 ...
 SELECT INTO "nl"
  ...
  DETAIL
   pat->pat_name = p.name_full_formatted, pat->pat_encnt = ed.encntr_id, pat->pat_p_id = ed.person_id,
 ...
 SELECT INTO "nl"
  FROM allergy a,
   nomenclature n
  PLAN (a
   WHERE (a.encntr_id=pat->pat_encnt)
 ...
 SELECT INTO "nl:"
  ...
  porparicaine_dose_r, pt_on_b_w_cerv_collar, c_sp_his_powergrid, c_sp_phy_exam_pg, c_sp_clin_cleared,
  ...
  DETAIL
   ...
   ELSEIF (ce.event_cd=acetaminophen_dose_route)
    ac_cnt = (pat->acetaminophen_dose_route_cnt+ 1), pat->acetaminophen_dose_route_cnt = ac_cnt, stat
     = alterlist(pat->acetaminophen_dose_route,ac_cnt),
 ...
 SELECT INTO "nl:"
  ...
  HEAD ce.event_cd
   ...
   ELSEIF (ce.event_cd=temperature_route
    ...
    pat->tempature_route_l_result_val = build2(trim(ce.result_val),"     (",format(ce.event_end_dt_tm,
      "MM/DD/YY HH:MM;;;D")," ) ")
   ELSEIF (ce.event_cd=pulse_rate
   ...
   ELSEIF (ce.event_cd=pmh)
    pat->pmh_val = build2(trim(ce.result_val),"     (",format(ce.event_end_dt_tm,"MM/DD/YY HH:MM;;;D"
      )," ) ")
 ...
 SELECT INTO value( $OUTDEV)
  ...
  PLAN (d)
  HEAD REPORT
   y_pos = 18, x_pos = 20, y = (y_pos+ 10),
   a_cnt = 0, no_data = null, xd = 20,
   b = "{B}", eb = "{ENDB}", printpsheader = 0,
   col 0, "{PS/792 0 }", row + 1,
   row + 1,
   CALL print(calcpos(225,(y_pos+ 0))), "{F/5}{CPI/10}{U}",
   "FirstNet Triage / Assessment Report", "{endu}", y_pos = (y_pos+ 10),
   row + 1,
   CALL print(calcpos(x_pos,y_pos)), "{F/4}{CPI/14}",
   b, "Patient Name: ", eb,
   pat->pat_name, x_pos = (x_pos+ 400), row + 1,
   CALL print(calcpos(x_pos,y_pos)), b, "Date & Time: ",
   eb, curdate, " ",
   curtime, y_pos = (y_pos+ 10), x_pos = 20,
   row + 1,
   CALL print(calcpos(x_pos,y_pos)), b,
   "DOB: ", eb, pat->pat_dob,
   "( Age ", pat->pat_age, " )",
   x_pos = (x_pos+ 150), row + 1,
   CALL print(calcpos(x_pos,y_pos)),
   b, "ACCT # : ", eb,
   pat->pat_fn, x_pos = (x_pos+ 150), row + 1,
   CALL print(calcpos(x_pos,y_pos)), b, "MRN :",
   eb, pat->pat_mrn, y_pos = (y_pos+ 10),
   row + 1,
   CALL print(calcpos(x_pos,y_pos))
  DETAIL
   x_pos = 20, y_pos = (y_pos+ 10),
   CALL print(calcpos(x_pos,y_pos)),
   b, "Allergy : ", eb,
   y_pos = (y_pos+ 10)
   ...
   ELSE
    ...
    FOR (ai = 1 TO pat->initial_treatment_cnt)
      ...
      CALL print(build2(pat->initial_treatment[ai].result_val," (",pat->initial_treatment[ai].
       dta_dt_tm,")")),
      row + 1
    ENDFOR
   ENDIF
   ...
   ELSE
    ...
    FOR (ci = 1 TO pat->acetaminophen_dose_route_cnt)
      ...
      CALL print(build2(pat->acetaminophen_dose_route[ci].result_val," (",pat->
       acetaminophen_dose_route[ci].dta_dt_tm,")"))
    ENDFOR
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos(x_pos,y_pos)),
    b, "Ambulatory On Scene: ", eb,
    pat->ambulatroy_o_scen_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos(x_pos,y_pos)),
    b, "Pt. on Backboard W/ Cervical Collar:", eb,
    pat->pt_on_b_w_cerv_collar_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos(x_pos,y_pos)),
    b, "C-Spine History Powergrid: ", eb,
    pat->c_sp_his_powergrid_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos((x_pos+ xd),y_pos)),
    "Extremity Weakness: ", pat->extremity_weakness_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos((x_pos+ xd),y_pos)),
    "Parasthesia Or Numbness:", pat->parasthesia_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos((x_pos+ xd),y_pos)),
    "Hx. Loss of Consciousness:", pat->hx_loss_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos((x_pos+ xd),y_pos)),
    "Pt.Has Distracting Injury: ", pat->pt_has_distracting_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos(x_pos,y_pos)),
    b, "C-Spine Physical Exam powergrid: ", eb,
    pat->c_sp_phy_exam_pg_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos((x_pos+ xd),y_pos)),
    "Mental Status Change:", pat->mental_status_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos((x_pos+ xd),y_pos)),
    "Neurological Deficit:", pat->neurological_deficit_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos((x_pos+ xd),y_pos)),
    "Distracting Painful Injury:", pat->distracting_pain_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos((x_pos+ xd),y_pos)),
    "Tenderness On Neck Palpation:", pat->tenderness_on_neck_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos((x_pos+ xd),y_pos)),
    "Palpable Deformity:", pat->palpable_defor_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos((x_pos+ xd),y_pos)),
    "Pain/Tenderness on Palpation:", pat->pain_tenderness_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos(x_pos,y_pos)),
    b, "C-spine Clinically Cleared: ", eb,
    pat->c_sp_clin_cleared_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos(x_pos,y_pos)),
    b, "C-spine Cleared by: ", eb,
    pat->c_sp_cleared_by_val
   ENDIF
   ...
   ELSE
    y_pos = (y_pos+ 10), row + 1,
    CALL print(calcpos(x_pos,y_pos)),
    b, "Patient Cleared for Triage: ", eb,
    pat->pat_clear_f_triage_val
   ENDIF
  FOOT REPORT
