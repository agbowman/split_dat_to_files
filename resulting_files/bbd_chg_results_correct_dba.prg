CREATE PROGRAM bbd_chg_results_correct:dba
 ...
 END ;Subroutine
 SET log_program_name = "bbd_chg_results_correct"
 RECORD reply(
   1 event_dt_tm = dq8
   1 orders[*]
     2 order_id = f8
 ...
 DECLARE gm_u_person_aborh_r0793_f8(icol_name=vc,ival=f8,iqual=i4,null_ind=i2,wq_ind=i2) = i2
 DECLARE gm_u_person_aborh_r0793_i2(icol_name=vc,ival=i2,iqual=i4,null_ind=i2,wq_ind=i2) = i2
 DECLARE gm_u_person_aborh_r0793_dq8(icol_name=vc,ival=f8,iqual=i4,null_ind=i2,wq_ind=i2) = i2
 DECLARE gm_u_person_aborh_r0793_i4(icol_name=vc,ival=i4,iqual=i4,null_ind=i2,wq_ind=i2) = i2
 SUBROUTINE gm_u_person_aborh_r0793_f8(icol_name,ival,iqual,null_ind,wq_ind)
   DECLARE stat = i2 WITH protect, noconstant(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE gm_u_person_aborh_r0793_i2(icol_name,ival,iqual,null_ind,wq_ind)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_person_aborh_r0793_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_person_aborh_r0793_req->qual,iqual)
    IF (stat=0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE gm_u_person_aborh_r0793_i4(icol_name,ival,iqual,null_ind,wq_ind)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_person_aborh_r0793_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_person_aborh_r0793_req->qual,iqual)
    IF (stat=0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE gm_u_person_aborh_r0793_dq8(icol_name,ival,iqual,null_ind,wq_ind)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_person_aborh_r0793_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_person_aborh_r0793_req->qual,iqual)
    IF (stat=0)
 ...
 SUBROUTINE gm_i_donor_antibo7811_dq8(icol_name,ival,iqual,null_ind)
   ...
   RETURN(1)
 END ;Subroutine
 EXECUTE gm_donor_antibo7811_def "U"
 DECLARE gm_u_donor_antibo7811_f8(icol_name=vc,ival=f8,iqual=i4,null_ind=i2,wq_ind=i2) = i2
 DECLARE gm_u_donor_antibo7811_i4(icol_name=vc,ival=i4,iqual=i4,null_ind=i2,wq_ind=i2) = i2
 DECLARE gm_u_donor_antibo7811_i2(icol_name=vc,ival=i2,iqual=i4,null_ind=i2,wq_ind=i2) = i2
 DECLARE gm_u_donor_antibo7811_dq8(icol_name=vc,ival=f8,iqual=i4,null_ind=i2,wq_ind=i2) = i2
 SUBROUTINE gm_u_donor_antibo7811_f8(icol_name,ival,iqual,null_ind,wq_ind)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_donor_antibo7811_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_donor_antibo7811_req->qual,iqual)
    IF (stat=0)
     CALL echo("can not expand request structure")
     RETURN(0)
    ENDIF
   ENDIF
   CASE (cnvtlower(icol_name))
    OF "donor_antibody_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->donor_antibody_idf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].donor_antibody_id = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->donor_antibody_idw = 1
     ENDIF
    OF "encntr_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->encntr_idf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].encntr_id = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->encntr_idw = 1
     ENDIF
    OF "antibody_cd":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->antibody_cdf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].antibody_cd = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->antibody_cdw = 1
     ENDIF
    OF "result_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->result_idf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].result_id = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->result_idw = 1
     ENDIF
    OF "contributor_system_cd":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->contributor_system_cdf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].contributor_system_cd = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->contributor_system_cdw = 1
     ENDIF
    OF "active_status_cd":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->active_status_cdf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].active_status_cd = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->active_status_cdw = 1
     ENDIF
    OF "active_status_prsnl_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->active_status_prsnl_idf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].active_status_prsnl_id = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->active_status_prsnl_idw = 1
     ENDIF
    OF "person_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->person_idf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].person_id = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->person_idw = 1
     ENDIF
    ELSE
     CALL echo("invalid column name passed")
     RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE gm_u_donor_antibo7811_i2(icol_name,ival,iqual,null_ind,wq_ind)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_donor_antibo7811_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_donor_antibo7811_req->qual,iqual)
    IF (stat=0)
   ...
   CASE (cnvtlower(icol_name))
    OF "active_ind":
     IF (null_ind=1)
      ...
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->active_indf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].active_ind = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->active_indw = 1
     ENDIF
    ELSE
     CALL echo("invalid column name passed")
     RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE gm_u_donor_antibo7811_i4(icol_name,ival,iqual,null_ind,wq_ind)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_donor_antibo7811_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_donor_antibo7811_req->qual,iqual)
    IF (stat=0)
   ...
   CASE (cnvtlower(icol_name))
    OF "bb_result_nbr":
     IF (null_ind=1)
      ...
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->bb_result_nbrf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].bb_result_nbr = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->bb_result_nbrw = 1
     ENDIF
    OF "updt_cnt":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->updt_cntf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].updt_cnt = ival
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->updt_cntw = 1
     ENDIF
    ELSE
     CALL echo("invalid column name passed")
     RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE gm_u_donor_antibo7811_dq8(icol_name,ival,iqual,null_ind,wq_ind)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_donor_antibo7811_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_donor_antibo7811_req->qual,iqual)
    IF (stat=0)
   ...
   CASE (cnvtlower(icol_name))
    OF "active_status_dt_tm":
     IF (null_ind=1)
      ...
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->active_status_dt_tmf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].active_status_dt_tm = cnvtdatetime(ival)
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->active_status_dt_tmw = 1
     ENDIF
    OF "updt_dt_tm":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_donor_antibo7811_req->updt_dt_tmf = 1
     SET gm_u_donor_antibo7811_req->qual[iqual].updt_dt_tm = cnvtdatetime(ival)
     IF (wq_ind=1)
      SET gm_u_donor_antibo7811_req->updt_dt_tmw = 1
     ENDIF
    ELSE
     CALL echo("invalid column name passed")
     RETURN(0)
 ...
 SUBROUTINE add_donor_aborh_result(sub_specimen_id,sub_container_id,sub_drawn_dt_tm,
  ...
  sub_donor_aborh_id)
   ...
   ENDIF
 END ;Subroutine
 SET gm_u_person_aborh_r0793_req->allow_partial_ind = 0
 SET gm_u_person_aborh_r0793_req->force_updt_ind = 0
 SET gm_u_person_aborh_r0793_req->person_aborh_rs_idw = 1
 SET gm_u_person_aborh_r0793_req->active_indf = 1
 SET gm_u_person_aborh_r0793_req->active_status_cdf = 1
 DECLARE chg_donor_aborh_result(sub_person_id=f8,sub_encntr_id=f8,sub_result_id=f8,sub_result_cd=f8,
  sub_active_ind=i2,
  sub_active_status_cd=f8,sub_updt_cnt=i4) = i2
 SUBROUTINE chg_donor_aborh_result(sub_person_id,sub_encntr_id,sub_result_id,sub_result_cd,
  sub_active_ind,sub_active_status_cd,sub_updt_cnt)
   SET stat = alterlist(gm_u_person_aborh_r0793_req->qual,1)
   IF (stat=0)
    CALL echo("can not size request structure")
    RETURN(0)
   ENDIF
   SELECT INTO "nl:"
    ...
    DETAIL
     gm_u_person_aborh_r0793_req->qual[1].person_aborh_rs_id = pa.person_aborh_rs_id,
     gm_u_person_aborh_r0793_req->qual[1].updt_cnt = pa.updt_cnt
    WITH nocounter
   ;end select
   IF (curqual=0)
    RETURN(2)
   ENDIF
   SET gm_u_person_aborh_r0793_req->qual[1].active_ind = sub_active_ind
   SET gm_u_person_aborh_r0793_req->qual[1].active_status_cd = sub_active_status_cd
   EXECUTE gm_u_person_aborh_r0793  WITH replace(request,gm_u_person_aborh_r0793_req), replace(reply,
    gm_u_person_aborh_r0793_rep)
   IF ((gm_u_person_aborh_r0793_rep->status_data.status="F"))
    CALL echo("Update person_aborh_result table failed.")
    RETURN(0)
   ELSE
    CALL echo("Update person_aborh_result table success.")
   ENDIF
   RETURN(1)
 END ;Subroutine
 DECLARE chg_donor_aborh_result_2(sub_person_id=f8,sub_encntr_id=f8,sub_result_id=f8,sub_active_ind=
  i2,sub_active_status_cd=f8) = i2
 SUBROUTINE chg_donor_aborh_result_2(sub_person_id,sub_encntr_id,sub_result_id,sub_active_ind,
 ...
 SUBROUTINE add_donor_antibody(sub_person_id,sub_encntr_id,sub_antibody_cd,sub_result_id,
  ...
  sub_active_status_prsnl_id,sub_contributor_system_cd)
   ...
   RETURN(1)
 END ;Subroutine
 SET gm_u_donor_antibo7811_req->allow_partial_ind = 0
 SET gm_u_donor_antibo7811_req->force_updt_ind = 0
 SET gm_u_donor_antibo7811_req->donor_antibody_idw = 1
 SET gm_u_donor_antibo7811_req->active_indf = 1
 SET gm_u_donor_antibo7811_req->active_status_cdf = 1
 DECLARE chg_donor_antibody(sub_encntr_id=f8,sub_antibody_cd=f8,sub_result_id=f8,sub_bb_result_id=f8,
  sub_active_ind=i2,
  sub_active_status_cd=f8,sub_updt_cnt=i4) = i2
 SUBROUTINE chg_donor_antibody(sub_encntr_id,sub_antibody_cd,sub_result_id,sub_bb_result_nbr,
  sub_active_ind,sub_active_status_cd,sub_updt_cnt)
   SET stat = alterlist(gm_u_donor_antibo7811_req->qual,1)
   SELECT INTO "nl:"
    FROM donor_antibody da
    WHERE da.encntr_id=sub_encntr_id
     AND da.antibody_cd=sub_antibody_cd
     AND da.result_id=sub_result_id
     AND da.bb_result_nbr=sub_bb_result_nbr
     AND da.active_ind=1
     AND da.updt_cnt=sub_updt_cnt
    DETAIL
     gm_u_donor_antibo7811_req->qual[1].donor_antibody_id = da.donor_antibody_id,
     gm_u_donor_antibo7811_req->qual[1].updt_cnt = da.updt_cnt
    WITH nocounter
   ;end select
   IF (curqual=0)
    RETURN(2)
   ENDIF
   SET gm_u_donor_antibo7811_req->qual[1].active_ind = sub_active_ind
   SET gm_u_donor_antibo7811_req->qual[1].active_status_cd = sub_active_status_cd
   EXECUTE gm_u_donor_antibo7811  WITH replace(request,gm_u_donor_antibo7811_req), replace(reply,
    gm_u_donor_antibo7811_rep)
   IF ((gm_u_donor_antibo7811_rep->status_data.status="F"))
    CALL echo("Update donor_antibody table failed.")
    RETURN(0)
   ELSEIF ((gm_u_donor_antibo7811_rep->status_data.status="S"))
    CALL echo("Update donor_antibody table success.")
   ENDIF
   RETURN(1)
 END ;Subroutine
 DECLARE chg_donor_antibody_2(sub_encntr_id=f8,sub_result_id=f8,sub_bb_result_id=f8,sub_active_ind=i2,
  sub_active_status_cd=f8) = i2
 SUBROUTINE chg_donor_antibody_2(sub_encntr_id,sub_result_id,sub_bb_result_nbr,sub_active_ind,
  sub_active_status_cd)
   SET stat = alterlist(gm_u_donor_antibo7811_req->qual,1)
   SELECT INTO "nl:"
    FROM donor_antibody da
 ...
 SET gm_u_donor_antige7812_req->active_indf = 1
 SET gm_u_donor_antige7812_req->active_status_cdf = 1
 DECLARE chg_donor_antigen(sub_encntr_id=f8,sub_antigen_cd=f8,sub_result_id=f8,sub_bb_result_nbr=f8,
  sub_active_ind=i2,
  sub_active_status_cd=f8,sub_updt_cnt=i4) = i2
 SUBROUTINE chg_donor_antigen(sub_encntr_id,sub_antigen_cd,sub_result_id,sub_bb_result_nbr,
  sub_active_ind,sub_active_status_cd,sub_updt_cnt)
   SET stat = alterlist(gm_u_donor_antige7812_req->qual,1)
   SELECT INTO "nl:"
    FROM donor_antigen da
    WHERE da.encntr_id=sub_encntr_id
     AND ((da.antigen_cd+ 0)=sub_antigen_cd)
     AND da.result_id=sub_result_id
     AND da.bb_result_nbr=sub_bb_result_nbr
     AND da.active_ind=1
     AND da.updt_cnt=sub_updt_cnt
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE chg_donor_antigen_2(sub_encntr_id=f8,sub_result_id=f8,sub_bb_result_nbr=f8,sub_active_ind=i2,
  sub_active_status_cd=f8) = i2
 SUBROUTINE chg_donor_antigen_2(sub_encntr_id,sub_result_id,sub_bb_result_nbr,sub_active_ind,
  sub_active_status_cd)
   SET stat = alterlist(gm_u_donor_antige7812_req->qual,1)
   SELECT INTO "nl:"
    FROM donor_antigen da
 ...
 ENDIF
 SET select_ok_ind = 0
 SET bbidnbr_result_processing_mean = "BB ID NBR"
 ...
 DECLARE antigen_exist_ind = i2 WITH protect, noconstant(0)
 DECLARE special_testing_code_set = i4 WITH protect, constant(1612)
 DECLARE gsub_donor_antigen_inact_status = c2 WITH protect, noconstant(fillstring(2,"  "))
...
#script
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=1901
    ...
    AND cv.active_ind=1)
  DETAIL
   result_status_verified_cd = cv.code_value, result_status_verified_disp = cv.display
  WITH nocounter
 ;end select
 ...
 SET cv_cnt = 1
 SET stat = uar_get_meaning_by_codeset(1901,"OLDCORRECTED",cv_cnt,result_status_old_corr_cd)
 ...
 IF (((result_type_text_cd=0.0
  AND (request->result_text_ind=1)) OR (((result_type_alpha_cd=0.0
  AND (request->result_alpha_ind=1)) OR (((result_type_interp_cd=0.0
  AND (request->result_interp_ind=1)) OR (((result_type_numeric_cd=0.0
  AND (request->result_numeric_ind=1)) OR (((result_type_date_cd=0.0
  AND (request->result_date_ind=1)) OR (((result_type_freetext_cd=0.0
  AND (request->result_freetext_ind=1)) OR (((result_type_calc_cd=0.0
  AND (request->result_calc_ind=1)) OR (((result_status_verified_cd=0.0) OR (((
 result_status_old_verf_cd=0.0) OR (((result_status_corrected_cd=0.0) OR (((result_status_old_corr_cd
 =0.0) OR (((drawn_cd=0.0) OR (((tested_cd=0.0) OR (((quarantined_cd=0.0) OR (((verified_cd=0) OR (((
 available_cd=0) OR (disposed_cd=0.0
  AND (request->disposed_ind=1))) )) )) )) )) )) )) )) )) )) )) )) )) )) )) )) )
  CALL fail_script("SELECT","F","Code Value",
   IF (result_type_text_cd=0.0
    AND (request->result_text_ind=1)) "Unable to retrieve code_value for text meaning"
   ELSEIF (result_type_alpha_cd=0.0
    AND (request->result_alpha_ind=1)) "Unable to retrieve code_value for alpha meaning"
   ELSEIF (result_type_interp_cd=0.0
    AND (request->result_interp_ind=1)) "Unable to retrieve code_value for interp meaning"
   ELSEIF (result_type_numeric_cd=0.0
    AND (request->result_numeric_ind=1)) "Unable to retrieve code_value for numerice meaning"
   ELSEIF (result_type_date_cd=0.0
    AND (request->result_date_ind=1)) "Unable to retrieve code_value for date meaning"
   ELSEIF (result_type_freetext_cd=0.0
    AND (request->result_freetext_ind=1)) "Unable to retrieve code_value for freetext meaning"
   ELSEIF (result_type_calc_cd=0.0
    AND (request->result_calc_ind=1)) "Unable to retrieve code_value for calculated meaning"
   ELSEIF (result_status_verified_cd=0.0) "Unable to retrieve code_value for verified meaning"
   ELSEIF (result_status_old_verf_cd=0.0) "Unable to retrieve code_value for old verified meaning"
   ELSEIF (result_status_corrected_cd=0.0) "Unable to retrieve code_value for corrected meaning"
   ELSEIF (result_status_old_corr_cd=0.0) "Unable to retrieve code_value for old corrected meaning"
   ELSEIF (quarantined_cd=0.0) "Unable to retrieve code_value for quarantined meaning"
   ELSEIF (disposed_cd=0.0
    AND (request->disposed_ind=1)) "Unable to retrieve code_value for disposed meaning"
   ELSEIF (drawn_cd=0.0) "Unable to retrieve code_value for drawn meaning"
   ELSEIF (tested_cd=0.0) "Unable to retrieve code_value for tested meaning"
   ELSEIF (verified_cd=0.0) "Unable to retrieve code_value for verified event meaning"
   ELSEIF (available_cd=0.0) "Unable to retrieve code_value for available meaning"
   ELSE "Unable to retrieve code_value"
   ENDIF
   )
 ENDIF
   ...
   FOR (aidx = 1 TO nbr_of_assays)
     ...
     FOR (midx = 1 TO nbr_of_family)
       ...
       spd_dir))
        ...
        ;end update
       ENDIF
       IF ((((request->orders[oidx].assays[aidx].inact_product_drawn_yn="Y")
        AND (request->orders[oidx].assays[aidx].next_row_ind=1)) OR ((modproductlist->product[midx].
       drawn_event_id > 0.0))) )
        ...
        ENDIF
        IF (curqual=0)
         CALL fail_script("SELECT","F","Product Event","Unable to lock product event for drawn state"
          )
        ELSEIF ((cur_updt_cnt != request->orders[oidx].assays[aidx].drawn_state_updt_cnt))
         CALL fail_script("SELECT","F","Product Event",
       ...
       IF ((request->orders[oidx].assays[aidx].add_product_quar_yn="Y")
        ...
        IF (gsub_product_event_status="FS")
         CALL fail_script("INSERT","F","Product Event",
          "Unable to insert quarantine product event due to product event id")
        ELSEIF (gsub_product_event_status="FA")
         CALL fail_script("INSERT","F","Product Event","Unable to insert quarantine product event")
        ENDIF
        INSERT  FROM quarantine qu
         SET qu.product_event_id = sub_product_event_id, qu.product_id = modproductlist->product[midx
          ].product_id, qu.quar_reason_cd = request->orders[oidx].assays[aidx].quarantine_reason_cd,
          qu.quar_reason_cd = 0, qu.orig_quar_qty = 0, qu.cur_quar_qty = 0,
          qu.orig_quar_intl_units = 0, qu.cur_quar_intl_units = 0, qu.updt_cnt = 0,
          qu.updt_dt_tm = cnvtdatetime(curdate,curtime3), qu.updt_id = reqinfo->updt_id, qu.updt_task
           = reqinfo->updt_task,
          qu.updt_applctx = reqinfo->updt_applctx, qu.active_ind = 1, qu.active_status_cd = reqdata->
          active_status_cd,
          qu.active_status_dt_tm = cnvtdatetime(curdate,curtime3), qu.active_status_prsnl_id =
          reqinfo->updt_id
         WITH nocounter
        ;end insert
       ENDIF
       IF ((request->orders[oidx].assays[aidx].add_product_disposed_yn="Y")
        ...
        ELSEIF (gsub_product_event_status="FA")
         CALL fail_script("INSERT","F","Product Event","Unable to insert disposed product event")
        ENDIF
        ...
        ;end insert
       ENDIF
       IF ((request->orders[oidx].assays[aidx].add_product_drawn_yn="Y")
        AND (request->orders[oidx].assays[aidx].next_row_ind=1))
        SET product_event_id = 0.0
        SET sub_product_event_id = 0.0
        SET re_event_type_cd = drawn_cd
        SET gsub_product_event_status = "  "
        CALL add_product_event(modproductlist->product[midx].product_id,0,0,request->orders[oidx].
         order_id,0,
         drawn_cd,cnvtdatetime(request->event_dt_tm),request->event_personnel_id,0,0,
         0,0,1,reqdata->active_status_cd,cnvtdatetime(current->system_dt_tm),
         request->event_personnel_id)
        SET sub_product_event_id = product_event_id
        IF (gsub_product_event_status="FS")
         CALL fail_script("INSERT","F","Product Event",
          "Unable to insert drawn product event due to product event id")
        ELSEIF (gsub_product_event_status="FA")
         CALL fail_script("INSERT","F","Product Event","Unable to insert drawn product event")
        ENDIF
       ENDIF
       IF ((request->orders[oidx].assays[aidx].add_product_tested_yn="Y")
        ...
        ELSEIF (gsub_product_event_status="FA")
         CALL fail_script("INSERT","F","Product Event","Unable to insert tested product event")
        ENDIF
        ...
        IF (verified_event_id > 0)
         ...
         IF (tested_event_id > 0)
          ...
          CALL add_product_event(modproductlist->product[midx].product_id,0,0,0,0,
           ...
           0,0,1,reqdata->active_status_cd,cnvtdatetime(current->system_dt_tm),
           request->event_personnel_id)
          SET sub_product_id = product_event_id
          IF (gsub_product_event_status="FS")
           SET failed = "T"
           SET status_count = (status_count+ 1)
           IF (status_count > 1)
        ...
        ENDIF
       ENDIF
       IF ((request->orders[oidx].assays[aidx].inact_product_tested_yn="Y")
        AND (request->orders[oidx].assays[aidx].next_row_ind=1))
        IF ((modproductlist->product[midx].product_id=mod_product_id))
         SELECT INTO "nl:"
          p.product_event_id
          FROM product_event p
          WHERE (p.product_event_id=request->orders[oidx].assays[aidx].tested_prod_event_id)
          DETAIL
           cur_updt_cnt = p.updt_cnt
          WITH nocounter, forupdate(p)
         ;end select
        ELSEIF ((modproductlist->product[midx].tested_event_id > 0.0))
         SELECT INTO "nl:"
          p.product_event_id
          FROM product_event p
          WHERE (p.product_event_id=modproductlist->product[midx].tested_event_id)
          DETAIL
           cur_updt_cnt = p.updt_cnt
          WITH nocounter, forupdate(p)
         ;end select
        ...
        IF (curqual=0)
         CALL fail_script("SELECT","F","Product Event",
          "Unable to lock product event for tested state")
        ELSEIF ((cur_updt_cnt != request->orders[oidx].assays[aidx].tested_state_updt_cnt))
         CALL fail_script("SELECT","F","Product Event",
          "Update conflict on product event for tested state")
        ELSE
         IF ((modproductlist->product[midx].product_id=mod_product_id))
          UPDATE  FROM product_event pe
           SET pe.bb_result_id = 0, pe.active_ind = 0, pe.active_status_cd = reqdata->
            ...
            .updt_id = reqinfo->updt_id,
            pe.updt_task = reqinfo->updt_task, pe.updt_applctx = reqinfo->updt_applctx
           WHERE (pe.product_event_id=request->orders[oidx].assays[aidx].tested_prod_event_id)
            AND (pe.updt_cnt=request->orders[oidx].assays[aidx].tested_state_updt_cnt)
           WITH nocounter
          ;end update
         ELSEIF ((modproductlist->product[midx].tested_event_id > 0.0))
          UPDATE  FROM product_event pe
           SET pe.bb_result_id = 0, pe.active_ind = 0, pe.active_status_cd = reqdata->
            ...
            .updt_id = reqinfo->updt_id,
            pe.updt_task = reqinfo->updt_task, pe.updt_applctx = reqinfo->updt_applctx
           WHERE (pe.product_event_id=modproductlist->product[midx].tested_event_id)
            AND (pe.updt_cnt=modproductlist->product[midx].tested_updt_cnt)
           WITH nocounter
          ;end update
         ENDIF
         IF (curqual=0)
          CALL fail_script("CHANGE","F","Product Event",
           "Unable to update product event for tested state")
         ENDIF
        ENDIF
       ENDIF
       IF ((request->orders[oidx].assays[aidx].inact_product_quar_yn="Y")
        AND (request->orders[oidx].assays[aidx].next_row_ind=1))
        IF ((modproductlist->product[midx].product_id=mod_product_id))
         SELECT INTO "nl:"
          p.product_event_id
          FROM product_event p
          WHERE (p.product_event_id=request->orders[oidx].assays[aidx].quar_prod_event_id)
          DETAIL
           cur_updt_cnt = p.updt_cnt
          WITH nocounter, forupdate(p)
         ;end select
        ELSEIF ((modproductlist->product[midx].quarantined_event_id > 0.0))
         SELECT INTO "nl:"
          p.product_event_id
          FROM product_event p
          WHERE (p.product_event_id=modproductlist->product[midx].quarantined_event_id)
          DETAIL
           cur_updt_cnt = p.updt_cnt
          WITH nocounter, forupdate(p)
         ;end select
        ...
        IF (curqual=0)
         CALL fail_script("SELECT","F","Product Event",
          "Unable to lock product event for quarantine state")
        ELSEIF ((cur_updt_cnt != request->orders[oidx].assays[aidx].quar_state_updt_cnt))
         CALL fail_script("SELECT","F","Product Event",
          "Update conflict on product event for quarantine state")
        ELSE
         IF ((modproductlist->product[midx].product_id=mod_product_id))
          UPDATE  FROM product_event pe
           SET pe.bb_result_id = 0, pe.active_ind = 0, pe.active_status_cd = reqdata->
            ...
            .updt_id = reqinfo->updt_id,
            pe.updt_task = reqinfo->updt_task, pe.updt_applctx = reqinfo->updt_applctx
           WHERE (pe.product_event_id=request->orders[oidx].assays[aidx].quar_prod_event_id)
            AND (pe.updt_cnt=request->orders[oidx].assays[aidx].quar_state_updt_cnt)
           WITH nocounter
          ;end update
         ELSEIF ((modproductlist->product[midx].quarantined_event_id > 0.0))
          UPDATE  FROM product_event pe
           SET pe.bb_result_id = 0, pe.active_ind = 0, pe.active_status_cd = reqdata->
            ...
            .updt_id = reqinfo->updt_id,
            pe.updt_task = reqinfo->updt_task, pe.updt_applctx = reqinfo->updt_applctx
           WHERE (pe.product_event_id=modproductlist->product[midx].quarantined_event_id)
            AND (pe.updt_cnt=modproductlist->product[midx].quarantined_updt_cnt)
           WITH nocounter
          ;end update
         ENDIF
         IF (curqual=0)
          CALL fail_script("CHANGE","F","Product Event",
           "Unable to update product event for quar state")
         ENDIF
         IF ((modproductlist->product[midx].product_id=mod_product_id))
          SELECT INTO "nl:"
           q.product_event_id
           FROM quarantine q
           WHERE (q.product_event_id=request->orders[oidx].assays[aidx].quar_prod_event_id)
           WITH nocounter, forupdate(q)
          ;end select
         ELSEIF ((modproductlist->product[midx].quarantined_event_id > 0.0))
          SELECT INTO "nl:"
           q.product_event_id
           FROM product_event q
           WHERE (q.product_event_id=modproductlist->product[midx].quarantined_event_id)
           WITH nocounter, forupdate(q)
          ;end select
         ENDIF
         IF (curqual=0)
          CALL fail_script("SELECT","F","Quarantine","Unable to lock quarantine table")
         ELSE
          IF ((modproductlist->product[midx].product_id=mod_product_id))
           UPDATE  FROM quarantine q
            SET q.active_ind = 0, q.active_status_cd = reqdata->active_status_cd, q.updt_cnt = (q
             .updt_cnt+ 1),
             q.updt_dt_tm = cnvtdatetime(curdate,curtime3), q.updt_id = reqinfo->updt_id, q.updt_task
              = reqinfo->updt_task,
          ...
          ENDIF
          IF (curqual=0)
           CALL fail_script("CHANGE","F","Quarantine","Unable to update quarantine")
          ENDIF
         ENDIF
        ENDIF
       ENDIF
       IF ((request->orders[oidx].assays[aidx].product_aborh_verify_yn="Y")
        ...
        ELSE
         IF (gsub_blood_product_status="FL")
          CALL fail_script("SELECT","F","BLOOD_PRODUCT","Unable to lock blood_product")
         ELSEIF (gsub_blood_product_status="FU")
          CALL fail_script("UPDATE","F","BLOOD_PRODUCT","Unable to update blood product")
         ENDIF
         GO TO exit_script
        ENDIF
       ENDIF
       IF ((request->orders[oidx].assays[aidx].aborh_verify_yn="Y")
        ...
        ELSE
         IF (gsub_donor_aborh_status="FS")
          CALL fail_script("INSERT","F","Donor ABORH","Unable to insert donor_aborh due to aborh id")
         ELSEIF (gsub_donor_aborh_status="FA")
          CALL fail_script("INSERT","F","Donor ABORH","Unable to insert donor aborh")
         ENDIF
         IF (gsub_donor_aborh_inact_status="FL")
          CALL fail_script("LOCK","F","Donor ABORH","Unable to lock donor_aborh for update")
         ELSEIF (gsub_donor_aborh_inact_status="FU")
          CALL fail_script("CHANGE","F","Donor ABORH","Unable to inactivate donor aborh")
         ENDIF
         IF (gsub_inactive_aborh_rsl_status="FL")
          CALL fail_script("LOCK","F","Person ABORH Result",
           "Unable to lock person_aborh_result for update")
         ELSEIF (gsub_inactive_aborh_rsl_status="FU")
          CALL fail_script("CHANGE","F","DONOR ABORH Result",
           "Unable to inactivate donor aborh result")
         ENDIF
         IF (gsub_aborh_result_status="FP")
          CALL fail_script("select","F","DONOR ABORH",
           "Unable to find active person record on donor_aborh")
         ELSEIF (gsub_aborh_result_status="FS")
          CALL fail_script("INSERT","F","ABORH Result",
           "Unable to insert aborh result due to next sequence number")
         ELSEIF (gsub_aborh_result_status="FA")
          CALL fail_script("INSERT","F","ABORH Result","Unable to insert aborh result")
         ENDIF
         GO TO exit_script
        ENDIF
       ENDIF
        ...
        IF ((request->orders[oidx].assays[aidx].upd_product_rh_phenotype_yn="Y"))
         IF ((request->orders[oidx].assays[aidx].product_rh_phenotype_id > 0))
          ...
          IF (gsub_rh_phenotype_status="FL")
           CALL fail_script("SELECT forupdate","F","product rh phenotype",concat(
          ...
          ELSEIF (gsub_rh_phenotype_status="FU")
           CALL fail_script("UPDATE","F","product rh phenotype",concat(
          ...
          FOR (rh_a = 1 TO rh_a_cnt)
            CALL chg_special_testing_by_key(request->orders[oidx].assays[aidx].
             product_rh_antigenlist[rh_a].table_id,request->orders[oidx].assays[aidx].
             product_rh_antigenlist[rh_a].updt_cnt,0,reqdata->inactive_status_cd)
            IF (gsub_rh_phenotype_status="FL")
             CALL fail_script("SELECT forupdate","F","special testing",concat(
               ...
               cnvtstring(request->orders[oidx].order_id)))
            ELSEIF (gsub_rh_phenotype_status="FU")
             CALL fail_script("UPDATE","F","special testing",concat(
            ...
            ELSEIF (gsub_rh_phenotype_status != "OK")
             CALL fail_script("UPDATE","F","special testing",concat(
               "Update into special_testing failed.  Invalid file status (",trim(
                gsub_rh_phenotype_status),") for special_testing_id = ",cnvtstring(request->orders[
                oidx].assays[aidx].product_rh_antigenlist[rh_a].table_id),", updt_cnt = ",
               cnvtstring(request->orders[oidx].assays[aidx].product_rh_antigenlist[rh_a].updt_cnt),
               ", order_id = ",cnvtstring(request->orders[oidx].order_id)))
            ENDIF
            CALL chg_special_testing_result(request->orders[oidx].assays[aidx].
             product_rh_antigenlist[rh_a].table_id,request->orders[oidx].assays[aidx].product_id,
             reply->orders[oidx].assays[aidx].result_id,0.0,0,
             reqdata->inactive_status_cd,request->orders[oidx].assays[aidx].product_rh_antigenlist[
             rh_a].spcl_tst_rsl_updt_cnt,1,0.0,0)
            IF (gsub_inactive_spc_tst_rsl_status != "OK")
             IF (gsub_inactive_spcl_tst_status="FL")
              CALL fail_script("LOCK","F","Special Testing",
               "Unable to lock special_testing_result for phenotype update")
             ELSEIF (gsub_inactive_spcl_tst_status="FU")
              CALL fail_script("CHANGE","F","Special Testing","Unable to inactivate special testing")
             ELSE
              CALL fail_script("CHANGE","F","Special testing",concat(
         ...
         IF (gsub_bbd_rh_phenotype_status != "OK")
          CALL fail_script("SELECT","F","bb_rh_phenotype",concat(
         ...
         ;end select
         IF (rh_a_cnt != rh_cnt)
          CALL fail_script("SELECT","F","code_value_extension",
         ...
         IF (opposite_found_ind=1)
          ...
          CALL fail_script("INSERT","Z","Special Testing",
         ...
         IF (gsub_rh_phenotype_status="FZ")
          CALL fail_script("SELECT","F","bb_rh_phenotype",concat(
            "No rows exist on bb_rh_phenotype for resulted nomenclature_id for order_id = ",
         ...
         ELSEIF (gsub_rh_phenotype_status="FF")
          CALL fail_script("SELECT","F","bb rh phenotype",concat(
         ...
         ELSEIF (gsub_rh_phenotype_status="FS")
          CALL fail_script("INSERT","F","product rh phenotype",concat(
         ...
         IF (gsub_rh_phenotype_status != "OK")
          CALL fail_script("SELECT","F","product rh phenotype",concat(
            "rh_phenotype select failed for nomenclature_id = ",cnvtstring(request->orders[oidx].
             assays[aidx].nomenclature_id),", for order_id = ",cnvtstring(request->orders[oidx].
             order_id),".  No results added/updated."))
         ENDIF
         ...
         FOR (rh_a = 1 TO rh_a_cnt)
           SET gsub_special_testing_status = "  "
           SET new_special_testing_id = 0.0
           CALL add_special_testing(request->orders[oidx].assays[aidx].product_id,rh_a_rec->
            antigenlist[rh_a].antigen_cd,1,new_product_rh_phenotype_id,1,
            reqdata->active_status_cd,cnvtdatetime(curdate,curtime3),reqinfo->updt_id,"N")
           IF (gsub_special_testing_status="FS")
            CALL fail_script("INSERT","F","special testing",concat(
           ...
           ELSEIF (gsub_special_testing_status != "OK")
            CALL fail_script("INSERT","F","special testing",concat(
              "Could not insert special_testing row due to invalid insert status (",trim(
               ...
               order_id)))
           ENDIF
           CALL add_special_testing_result(new_special_testing_id,request->orders[oidx].assays[aidx].
           ...
           IF (gsub_spc_tst_result_status != "OK")
            IF (gsub_spc_tst_result_status="FA")
             CALL fail_script("INSERT","F","special testing result",concat(
               "Insert special_testing_result row failed for order_id = ",cnvtstring(request->orders[
                oidx].order_id),", product_id = ",cnvtstring(request->orders[oidx].assays[aidx].
                product_id)))
            ELSE
             CALL fail_script("INSERT","F","special testing result",concat(
               "Insert special_testing_result row failed for order_id = ",cnvtstring(request->orders[
       ...
       IF ((request->orders[oidx].assays[aidx].donor_rh_phenotype_verify_yn="Y")
         ...
         FOR (rh_a = 1 TO rh_a_cnt)
           ...
           SELECT INTO "nl:"
            FROM special_testing_result str,
             ...
             donor_antigen da
            PLAN (str
             WHERE (str.special_testing_id=request->orders[oidx].assays[aidx].product_rh_antigenlist[
             rh_a].table_id))
             JOIN (st
             ...
             WHERE (da.person_id=request->orders[oidx].person_id)
              ...
              AND ((da.antigen_cd+ 0)=st.special_testing_cd))
            DETAIL
           ...
           IF (antigen_exist_ind=1)
            ...
            IF (gsub_donor_rh_phenotype_status="FL")
             CALL fail_script("SELECT forupdate","F","donor_antigen",concat(
            ...
            ELSEIF (gsub_donor_rh_phenotype_status="FU")
             CALL fail_script("UPDATE","F","donor_antigen",concat(
               "Update into donor_antigen failed for donor_antigen_id = ",cnvtstring(request->orders[
                oidx].assays[aidx].product_rh_antigenlist[rh_a].table_id),", updt_cnt = ",cnvtstring(
                request->orders[oidx].assays[aidx].product_rh_antigenlist[rh_a].updt_cnt),
               ", order_id = ",
               cnvtstring(request->orders[oidx].order_id)))
            ELSEIF (gsub_donor_rh_phenotype_status != "OK")
             CALL fail_script("UPDATE","F","donor_antigen",concat(
               "Update into donor_antigen failed.  Invalid status (",trim(gsub_rh_phenotype_status),
               ") for donor_antigen_id = ",cnvtstring(request->orders[oidx].assays[aidx].
                product_rh_antigenlist[rh_a].table_id),", updt_cnt = ",
               cnvtstring(request->orders[oidx].assays[aidx].product_rh_antigenlist[rh_a].updt_cnt),
               ", order_id = ",cnvtstring(request->orders[oidx].order_id)))
            ENDIF
           ENDIF
        ...
        ;end select
        IF (rh_a_cnt != rh_cnt)
         CALL fail_script("SELECT","F","Code Value Extension",concat(
           "Could not load opposites or posting to donor indicator.",", nomenclature_id = ",
           cnvtstring(request->orders[oidx].assays[aidx].nomenclature_id),", rh_cnt = ",cnvtstring(
            rh_cnt),
           ", rh_a_cnt = ",cnvtstring(rh_a_cnt),
           ".  No opposites inactivated.  No results added/updated."))
        ENDIF
        SET rh_a_cnt = size(rh_a_rec->antigenlist,5)
        FOR (rh_a = 1 TO rh_a_cnt)
          IF ((rh_a_rec->antigenlist[rh_a].post_to_donor_ind=1))
           ...
           SELECT INTO "nl:"
            ...
            PLAN (d
             ...
             WHERE da.encntr_id=e.encntr_id
              AND ((da.antigen_cd+ 0)=rh_a_rec->antigenlist[d.seq].opposite_cd)
              AND da.active_ind=1)
            DETAIL
             opposite_found_ind = 1
            WITH nocounter
           ...
           IF (opposite_found_ind=1)
            ...
            perform_result_key
            SET reply->status_data.status = "Z"
            CALL fail_script("INSERT","Z","Donor Antigen",
           ...
           IF (gsub_donor_antigen_status="FS")
            CALL fail_script("INSERT","F","donor_antigen",concat(
       ...
       IF ((request->orders[oidx].assays[aidx].special_testing_verify_yn="Y")
        ...
        IF ((request->review_queue_ind=0))
         CALL chg_special_testing_result(orig_special_testing_id,request->orders[oidx].assays[aidx].
          product_id,reply->orders[oidx].assays[aidx].result_id,reply->orders[oidx].assays[aidx].
          bb_result_id,0,
         ...
         IF (gsub_inactive_spc_tst_rsl_status != "OK")
          IF (gsub_inactive_spc_tst_rsl_status="FL")
           CALL fail_script("LOCK","F","Special Testing Result",concat(
             "Unable to lock special_testing_result for product update. special_testing_id = ",trim(
              cnvtstring(orig_special_testing_id)),"  product_id = ",trim(cnvtstring(request->orders[
               oidx].assays[aidx].product_id)),"  result_id = ",
             trim(cnvtstring(reply->orders[oidx].assays[aidx].result_id)),"  updt_cnt = ",trim(
              cnvtstring(request->orders[oidx].assays[aidx].spcl_tst_rsl_updt_cnt))))
          ELSEIF (gsub_inactive_spc_tst_rsl_status="FU")
           CALL fail_script("CHANGE","F","Special Testing",concat(
             "Unable to inactivate special testing. special_testing_id = ",trim(cnvtstring(
               orig_special_testing_id)),"  product_id = ",trim(cnvtstring(request->orders[oidx].
               assays[aidx].product_id)),"  result_id = ",
             trim(cnvtstring(reply->orders[oidx].assays[aidx].result_id)),"  updt_cnt = ",trim(
              cnvtstring(request->orders[oidx].assays[aidx].spcl_tst_rsl_updt_cnt))))
          ENDIF
          GO TO exit_script
        ...
        ELSEIF ((request->review_queue_ind=1))
         CALL chg_special_testing_result_2(request->orders[oidx].assays[aidx].product_id,reply->
          orders[oidx].assays[aidx].result_id,reply->orders[oidx].assays[aidx].bb_result_id,0,reqdata
          ->active_status_cd)
         IF (gsub_inactive_spc_tst_rsl_status != "OK")
          IF (gsub_inactive_spc_tst_rsl_status="FL")
           CALL fail_script("LOCK","F","Special Testing Result",concat(
        ...
        IF ((request->orders[oidx].assays[aidx].upd_prod_spcl_tst_yn="Y"))
         IF ((request->review_queue_ind=0))
          CALL chg_special_testing(request->orders[oidx].assays[aidx].product_id,request->orders[oidx
           ].assays[aidx].orig_result_code_set_cd,0,reqdata->active_status_cd,request->orders[oidx].
           assays[aidx].special_testing_updt_cnt,
           1)
          IF (gsub_inactive_spcl_tst_status != "OK")
           IF (gsub_inactive_spcl_tst_status="FL")
            CALL fail_script("LOCK","F","Special Testing",concat(
              "Unable to lock special_testing for update. product_id = ",trim(cnvtstring(request->
                orders[oidx].assays[aidx].product_id)),"  orig_result_code_set_cd = ",trim(cnvtstring
               (request->orders[oidx].assays[aidx].orig_result_code_set_cd)),"  updt_cnt = ",
              trim(cnvtstring(request->orders[oidx].assays[aidx].special_testing_updt_cnt))))
           ELSEIF (gsub_inactive_spcl_tst_status="FU")
            CALL fail_script("CHANGE","F","Special Testing",concat(
              "Unable to inactivate special testing. product_id = ",trim(cnvtstring(request->orders[
                oidx].assays[aidx].product_id)),"  orig_result_code_set_cd = ",trim(cnvtstring(
                request->orders[oidx].assays[aidx].orig_result_code_set_cd)),"  updt_cnt = ",
              trim(cnvtstring(request->orders[oidx].assays[aidx].special_testing_updt_cnt))))
           ENDIF
           GO TO exit_script
         ...
         ENDIF
        ENDIF
        CALL add_special_testing(request->orders[oidx].assays[aidx].product_id,request->orders[oidx].
         assays[aidx].bb_result_code_set_cd,1,0.0,1,
         reqdata->active_status_cd,cnvtdatetime(current->system_dt_tm),request->event_personnel_id,
         "Y")
        IF (gsub_special_testing_status != "OK")
         IF (gsub_special_testing_status="FS")
          CALL fail_script("INSERT","F","Special Testing",
           "Unable to insert special testing due to special testing id")
         ELSEIF (gsub_special_testing_status="FA")
          CALL fail_script("INSERT","F","Special_testing","Unable to insert special testing")
         ELSEIF (gsub_special_testing_status="FL")
          CALL fail_script("LOCK","F","Special Testing","Unable to lock special testing for update")
         ELSEIF (gsub_special_testing_status="FU")
          CALL fail_script("CHANGE","F","Special Testing",
        ...
        IF (gsub_spc_tst_result_status != "OK")
         IF (gsub_spc_tst_result_status="FA")
          CALL fail_script("INSERT","F","Special Testing Result",
       ...
       IF ((request->orders[oidx].assays[aidx].antigen_verify_yn="Y")
        ...
        AND (modproductlist->product[midx].product_id=mod_product_id))
        SET gsub_donor_antigen_status = "  "
        SET gsub_donor_antigen_inact_status = "  "
        SET stat = chg_donor_antigen(request->orders[oidx].encntr_id,request->orders[oidx].assays[
         aidx].orig_result_code_set_cd,reply->orders[oidx].assays[aidx].result_id,bb_result_seq,0,
         reqdata->inactive_status_cd,request->orders[oidx].assays[aidx].person_antigen_updt_cnt)
        IF (stat=1)
         SET gsub_donor_antigen_inact_status = "OK"
        ELSEIF (stat=2)
         SET gsub_donor_antigen_inact_status = "FL"
        ELSE
         SET gsub_donor_antigen_inact_status = "FU"
        ENDIF
        IF (gsub_donor_antigen_inact_status != "OK")
         IF (gsub_inactive_prs_antigen_status="FL")
          CALL fail_script("LOCK","F","Donor Antigen","Unable to lock donor_antigen for update")
         ELSEIF (gsub_donor_antigen_inact_status="FU")
          CALL fail_script("CHANGE","F","Donor Antigen","Unable to inactivate donor antigen")
         ENDIF
        ENDIF
        ...
        ;end select
        IF (curqual=0)
         CALL fail_script("SELECT","Z","code_value_extension 2",
         ...
         FOR (cntr = 1 TO nbr_of_excepts)
           ...
           result_status_corr_in_review_cd))
            CALL add_bbd_exception(sub_product_event_id,request->orders[oidx].assays[aidx].
             ...
             .override_reason_cd,re_event_type_cd,result_seq,
             ...
             to_rh_cd,
             request->orders[oidx].person_id)
            IF (exception_status="F")
             CALL fail_script("INSERT","F","BB EXCEPTION","Unable to insert exception")
             GO TO exit_script
            ELSEIF (exception_status="FU")
             CALL fail_script("UAR","F","BB EXCEPTION","Unable to get exception_type_cd")
            ENDIF
           ELSE
            CALL add_bb_inactive_exception(0.0,0.0,0.0,cnvtdatetime(""),sub_product_event_id,
             request->orders[oidx].assays[aidx].exceptlist[cntr].exception_type_mean,request->orders[
             oidx].assays[aidx].exceptlist[cntr].override_reason_cd,re_event_type_cd,result_seq,
             perf_result_seq,
             ...
             cntr].to_abo_cd,request->orders[oidx].assays[aidx].exceptlist[cntr].to_rh_cd)
            IF (exception_status="F")
             CALL fail_script("INSERT","F","BB EXCEPTION","Unable to insert exception")
             GO TO exit_script
            ELSEIF (exception_status="FU")
       ...
       ELSEIF ((request->review_queue_ind=1))
        ...
        IF (nbr_of_excepts > 0)
         FOR (cntr = 1 TO nbr_of_excepts)
           SET exception_status = "I"
           SET bb_exception_id = 0.0
           CALL activate_bb_exception(request->orders[oidx].assays[aidx].exceptlist[cntr].
            exception_id,request->orders[oidx].assays[aidx].exceptlist[cntr].updt_cnt)
           IF (exception_status="FL")
            CALL fail_script("Lock","F","BB EXCEPTION","Unable to lock exception")
           ELSEIF (exception_status="F")
            CALL fail_script("UPDATE","F","BB EXCEPTION","Unable to activate exception")
            GO TO exit_script
           ENDIF
         ENDFOR
        ENDIF
...
#person_donor_start
 FOR (didx = 1 TO nbr_of_donors)
  IF ((request->donors[didx].updt_person_donor_ind=1))
   CALL update_person_donor(request->donors[didx].person_id,1,request->donors[didx].
    ...
    cnvtdatetime(request->donors[didx].elig_defer_dt_tm),request->donors[didx].person_donor_updt_cnt,
    request->donors[didx].elig_for_reinstate_ind,request->donors[didx].updt_elig_for_reinstate_ind_yn,
    request->donors[didx].reinstated_ind,
   ...
   IF (gsub_person_donor_status="FL")
    CALL fail_script("UPDATE","F","Person Donor","Unable to update person_donor due to lock_ind")
    GO TO exit_script
   ELSEIF (gsub_person_donor_status="UF")
    CALL fail_script("UPDATE","F","Person Donor","Unable to update person_donor")
    GO TO exit_script
   ENDIF
  ENDIF
  IF ((request->donors[didx].updt_bbd_eligibility_ind=1))
   CALL update_donor_eligibility(request->donors[didx].eligibility_id,0,request->donors[didx].
    ...
    updt_applctx)
   IF (gsub_person_donor_status="FL")
    CALL fail_script("SELECT","F","Donor Eligibility",
     "Unable to update bbd_donor_eligibility due to locking for update")
    GO TO exit_script
   ELSEIF (gsub_person_donor_status="UF")
    CALL fail_script("UPDATE","F","BBD Donor Eligibility","Unable to add bbd_donor_eligibility")
    GO TO exit_script
   ENDIF
  ENDIF
 ENDFOR
 ...
 FOR (ridx = 1 TO nbr_of_deferral_reas)
  ...
  IF (gsub_bbd_deferral_reason_status="FS")
   CALL fail_script("SELECT","F","PathNet Sequence",
    "Unable to update bbd_donor_eligibility due to sequence generation")
   GO TO exit_script
  ELSEIF (gsub_bbd_deferral_reason_status="FA")
   CALL fail_script("INSERT","F","BBD Deferral Reason","Unable to add bbd_deferral_reason")
   GO TO exit_script
  ENDIF
 ENDFOR
 ...
 SUBROUTINE process_updated_result(none1)
   ...
   IF (read_perform_result(result_seq,perf_result_seq)=0)
    CALL fail_script("SELECT","F","Perform Result","Unable to retrieve perform result")
    RETURN(0)
   ENDIF
   ...
   IF (insert_perform_result(result_seq,parent_perf_result_id)=0)
    CALL fail_script("INSERT","F","Perform Result","Unable to insert perform result")
    RETURN(0)
   ENDIF
   ...
   IF (insert_result_event(result_seq,perf_result_seq)=0)
    CALL fail_script("INSERT","F","Result Event","Unable to insert result event")
    RETURN(0)
   ENDIF
   RETURN(1)
   ...
   SUBROUTINE update_result(arg_result_id)
     ...
     ;end select
     IF (curqual=0)
      CALL fail_script("CHANGE","F","Result",concat("Unable to lock result for result_id = ",trim(
         cnvtstring(result_seq))))
      SET return_value = 0
     ELSEIF ((cur_updt_cnt != request->orders[oidx].assays[aidx].result_updt_cnt))
      CALL fail_script("CHANGE","F","Result",concat("Update conflict on result for result_id = ",
        cnvtstring(result_seq)))
      SET return_value = 0
     ELSE
      UPDATE  FROM result r
       SET r.call_back_ind = request->orders[oidx].assays[aidx].call_back_ind, r.result_status_cd =
        ...
        r.security_level_cd = request->orders[oidx].assays[aidx].security_level_cd, r.repeat_number
         = request->orders[oidx].assays[aidx].repeat_number, r.updt_dt_tm = cnvtdatetime(curdate,
         curtime3),
      ...
      ;end update
      IF (curqual=0)
       CALL fail_script("CHANGE","F","Result","Unable to update result")
       SET return_value = 0
      ELSE
       SET return_value = 1
      ENDIF
   ...
   SUBROUTINE insert_perform_result(arg_result_id,arg_parent_perf_rslt_id)
     ...
     INSERT  FROM perform_result pr
      SET pr.perform_result_id = perf_result_seq, pr.result_id = arg_result_id, pr
       ...
       ENDIF
       ,
       pr.reference_range_factor_id = request->orders[oidx].assays[aidx].reference_range_factor_id,
       pr.normal_cd = request->orders[oidx].assays[aidx].normal_cd, pr.critical_cd = request->orders[
       oidx].assays[aidx].critical_cd,
       pr.review_cd = request->orders[oidx].assays[aidx].review_cd, pr.delta_cd = request->orders[
       oidx].assays[aidx].delta_cd, pr.notify_cd = request->orders[oidx].assays[aidx].notify_cd,
       pr.units_cd = request->orders[oidx].assays[aidx].units_cd, pr.normal_low = request->orders[
   ...
   SUBROUTINE update_perform_result(arg_result_id,arg_perf_result_id,arg_result_status_cd)
     ...
     ;end select
     IF (curqual=0)
      CALL fail_script("CHANGE","F","Perform Result","Unable to lock perform result")
      SET return_value = 0
     ELSEIF ((cur_updt_cnt != request->orders[oidx].assays[aidx].perform_result_updt_cnt))
      CALL fail_script("CHANGE","F","Perform Result","Update conflict on perform result")
      SET return_value = 0
     ELSE
      ...
      ;end update
      IF (curqual=0)
       CALL fail_script("CHANGE","F","Perform Result","Unable to update perform result")
       SET return_value = 0
      ELSE
       SET return_value = 1
      ENDIF
   ...
   SUBROUTINE insert_result_event(arg_result_id,arg_perf_result_id)
    INSERT  FROM result_event re
     SET re.result_id = arg_result_id, re.perform_result_id = arg_perf_result_id, re.event_sequence
      ...
      .called_back_ind = request->orders[oidx].assays[aidx].call_back_ind, re.event_type_cd = request
      ->orders[oidx].assays[aidx].result_status_cd,
   ...
   SUBROUTINE read_long_data_seq(none2)
     ...
     RETURN(curqual)
   END ;Subroutine
   DECLARE update_donor_aborh(none3) = i4
   SUBROUTINE update_donor_aborh(none3)
       ...
       ELSE
        ...
        IF ((request->orders[oidx].assays[aidx].orig_rh_cd=0))
         IF ((request->orders[oidx].assays[aidx].orig_abo_cd=0))
          SET stat = add_donor_aborh(request->orders[oidx].person_id,request->orders[oidx].assays[
           aidx].orig_abo_cd,request->orders[oidx].assays[aidx].new_rh_cd,1,reqdata->active_status_cd,
          ...
          SET write_aborh_result = "Y"
         ELSE
          SET stat = chg_donor_aborh(request->orders[oidx].person_id,request->orders[oidx].assays[
           aidx].orig_abo_cd,request->orders[oidx].assays[aidx].orig_rh_cd,0,reqdata->
           inactive_status_cd,
      ...
      ELSEIF (abo_test_only="Y")
       ...
       ELSE
        IF ((request->orders[oidx].assays[aidx].orig_abo_cd=0))
         IF ((request->orders[oidx].assays[aidx].orig_rh_cd=0))
          SET stat = add_donor_aborh(request->orders[oidx].person_id,request->orders[oidx].assays[
           aidx].new_abo_cd,request->orders[oidx].assays[aidx].orig_rh_cd,1,reqdata->active_status_cd,
           cnvtdatetime(current->system_dt_tm),request->event_personnel_id,cnvtdatetime(request->
            event_dt_tm),reqdata->contributor_system_cd,donor_aborh_id)
          IF (stat=1)
           SET gsub_donor_aborh_status = "OK"
       ...
       ELSE
        ...
        orig_abo_cd))
         ...
         ELSE
          ...
          ENDIF
          SET stat = add_donor_aborh(request->orders[oidx].person_id,request->orders[oidx].assays[
           aidx].new_abo_cd,request->orders[oidx].assays[aidx].new_rh_cd,1,reqdata->active_status_cd,
           cnvtdatetime(current->system_dt_tm),request->event_personnel_id,cnvtdatetime(request->
            event_dt_tm),reqdata->contributor_system_cd,donor_aborh_id)
          IF (stat=1)
           SET gsub_donor_aborh_status = "OK"
     ...
     IF (write_aborh_result="Y")
      SET write_aborh_result = "N"
      SET stat = chg_donor_aborh_result(request->orders[oidx].person_id,request->orders[oidx].
       encntr_id,reply->orders[oidx].assays[aidx].result_id,request->orders[oidx].assays[aidx].
       orig_result_code_set_cd,0,
       reqdata->inactive_status_cd,request->orders[oidx].assays[aidx].aborh_result_updt_cnt)
      IF (stat=1)
      ...
      ENDIF
      SET gsub_aborh_result_status = "  "
      SET stat = add_donor_aborh_result(request->orders[oidx].assays[aidx].specimen_id,request->
       orders[oidx].assays[aidx].container_id,request->orders[oidx].assays[aidx].drawn_dt_tm,0.0,
       request->orders[oidx].person_id,
       request->orders[oidx].encntr_id,reply->orders[oidx].assays[aidx].result_id,request->orders[
       oidx].assays[aidx].bb_result_code_set_cd,1,reqdata->active_status_cd,
     ...
     RETURN(1)
   END ;Subroutine
   DECLARE update_product_aborh(none4) = i4
   SUBROUTINE update_product_aborh(none4)
     ...
     ENDIF
     SET gsub_abo_testing_status = "  "
     SET sub_product_event_id = conf_product_event_id
     IF (write_result="Y")
      SET write_result = "N"
      ...
      IF (gsub_abo_testing_status != "OK")
       IF (gsub_abo_testing_status="FA")
        CALL fail_script("INSERT","F","ABO_TESTING","Unable to insert row in ABO_TESTING")
       ELSEIF (gsub_abo_testing_status="FS")
        CALL fail_script("INSERT","F","Ab Testing",
         "Unable to insert aborh result due to next sequence number")
       ENDIF
       RETURN(0)
      ENDIF
     ...
     RETURN(1)
   END ;Subroutine
   DECLARE inactivate_abo_testing(none6) = i4
   SUBROUTINE inactivate_abo_testing(none6)
     SET return_value = 0
     SET cur_updt_cnt = 0
     SELECT INTO "nl:"
      at.product_id
      ...
      WHERE at.product_id=sub_product_id
       ...
       AND ((at.active_ind+ 0)=1)
      DETAIL
       cur_updt_cnt = at.updt_cnt
      WITH nocounter, forupdate(at)
     ;end select
     IF (curqual=0)
      CALL fail_script("CHANGE","F","Abo Testing","Unable to lock abo testing")
      SET return_value = 0
     ELSEIF ((cur_updt_cnt != request->orders[oidx].assays[aidx].abo_testing_upd_cnt))
      CALL fail_script("CHANGE","F","Abo Testing","Update conflict on abo testing")
      SET return_value = 0
     ELSE
      UPDATE  FROM abo_testing at
       SET at.active_ind = 0, at.active_status_cd = reqdata->active_status_cd, at.updt_cnt = (at
        .updt_cnt+ 1),
        at.updt_dt_tm = cnvtdatetime(curdate,curtime3), at.updt_id = reqinfo->updt_id, at.updt_task
         = reqinfo->updt_task,
      ...
      ;end update
      IF (curqual=0)
       CALL fail_script("CHANGE","F","Abo Testing","Unable to update abo testing")
       SET return_value = 0
      ELSE
       SET return_value = 1
      ENDIF
     ...
     ELSE
      ...
      INSERT  FROM product_event pe
       SET pe.product_event_id = sub_product_event_id, pe.product_id = sub_product_id, pe.person_id
        ...
        pe.related_product_event_id = sub_related_product_event_id, pe.active_ind = sub_active_ind,
   ...
   SUBROUTINE add_bbd_exception(prod_event_id,exception_type_mean,sub_override_reason_cd,
    sub_event_type_cd,sub_result_id,sub_perform_result_id,sub_from_abo_cd,sub_from_rh_cd,
    sub_to_abo_cd,sub_to_rh_cd,sub_person_id)
     SET exception_status = "I"
   ...
   SUBROUTINE add_reqs_exception(sub_special_testing_cd,sub_requirement_cd)
     ...
     INSERT  FROM bb_reqs_exception b
      SET b.reqs_exception_id = new_pathnet_seq, b.exception_id = bb_exception_id, b
   ...
   SUBROUTINE add_autodir_reqs_exception(sub_product_id)
     ...
     INSERT  FROM bb_autodir_exception b
      SET b.bb_autodir_exc_id = new_pathnet_seq, b.bb_exception_id = sub_bb_exception_id, b
   ...
   ELSE
    INSERT  FROM person_antigen p
     SET p.person_antigen_id = unique_id, p.person_id = sub_person_id, p.antigen_cd = sub_antigen_cd,
      ...
      sub_active_status_prsnl_id, p.updt_dt_tm = cnvtdatetime(curdate,curtime3),
 ...
 SUBROUTINE chg_special_testing(sub_product_id,sub_special_testing_cd,sub_active_ind,
  sub_active_status_cd,sub_updt_cnt,sub_lock_forupdate_ind)
   ...
   IF (((sub_lock_forupdate_ind=0) OR (sub_lock_forupdate_ind=1
    ...
    ENDIF
   ENDIF
   DECLARE chg_special_testing_2(sub_special_testing_id=f8,sub_active_ind=i2,sub_active_status_cd=f8)
    = null
   SUBROUTINE chg_special_testing_2(sub_special_testing_id,sub_active_ind,sub_active_status_cd)
     SET gsub_inactive_spcl_tst_status = "  "
     SELECT INTO "nl:"
      s.special_testing_id
      FROM special_testing s
 ...
 SUBROUTINE chg_special_testing_result(sub_special_testing_id,sub_product_id,sub_result_id,
  ...
  sub_special_testing_cd,sub_spcl_tst_updt_cnt)
   ...
   IF (((sub_lock_forupdate_ind=0) OR (sub_lock_forupdate_ind=1
    ...
    ENDIF
   ENDIF
   DECLARE chg_special_testing_result_2(sub_product_id=f8,sub_result_id=f8,sub_bb_result_id=f8,
    sub_active_ind=i2,sub_active_status_cd=f8) = null
   SUBROUTINE chg_special_testing_result_2(sub_product_id,sub_result_id,sub_bb_result_id,
    sub_active_ind,sub_active_status_cd)
     SET gsub_inactive_spc_tst_rsl_status = "  "
     SELECT INTO "nl:"
      s.special_testing_id
      FROM special_testing_result s
 ...
 SUBROUTINE add_bbd_deferral_reason(sub_eligibility_id,sub_contact_id,sub_person_id,sub_reason_cd,
  ...
  sub_active_status_prsnl_id)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE fail_script(sub_operation_name,sub_operation_status,sub_target_object_name,
  sub_target_object_value)
   SET failed = "T"
   SET status_count = (status_count+ 1)
   IF (status_count > 1)
    SET stat = alterlist(reply->status_data.subeventstatus,status_count)
   ENDIF
   SET reply->status_data.subeventstatus[status_count].operationname = sub_operation_name
   SET reply->status_data.subeventstatus[status_count].operationstatus = sub_operation_status
   SET reply->status_data.subeventstatus[status_count].targetobjectname = sub_target_object_name
   SET reply->status_data.subeventstatus[status_count].targetobjectvalue = sub_target_object_value
   GO TO exit_script
 END ;Subroutine
...
#exit_script
 ...
 FREE RECORD gm_u_donor_antibo7811_req
 FREE RECORD gm_u_donor_antibo7811_rep
