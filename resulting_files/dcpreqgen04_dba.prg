CREATE PROGRAM dcpreqgen04:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   format(e.reg_dt_tm,"@SHORTDATETIMENOSEC"), orders->dischg_dt = format(e.disch_dt_tm,"@SHORTDATE"),
 ...
 ENDFOR
 SET max_length = 90
 IF (textlen(allergy->line) > max_length)
  SET pt->line_cnt = 0
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   HEAD PAGE
    "{lpi/8}{cpi/12}{font/12}", "  ", row + 1,
    "{pos/1/60}{cpi/10}{b}",
    CALL center(orders->facility,1,190), "{endb}",
    row + 1, "{cpi/14}{pos/60/105}", "Patient: ",
    "{cpi/10}{b}", orders->name, "{endb}",
    "{cpi/14}{pos/300/105}", "Age: ", "{cpi/10}{b}",
    orders->age, "{endb}", row + 1,
    "{cpi/14}{pos/380/105}", "Gender: ", "{cpi/10}{b}",
    orders->sex, "{endb}", "{cpi/14}{pos/470/105}",
    "Room/Bed: ", "{cpi/10}{b}", orders->room,
    "  ", orders->bed, row + 1,
    "{cpi/14}{pos/60/122}", "MR#: ", "{cpi/12}{B}",
    orders->mrn, "{endb}", "{cpi/14}{pos/300/122}",
    "Acct #: ", "{cpi/12}{b}", orders->fnbr,
    row + 1, "{cpi/14}{pos/60/139}", "Dx: ",
    "{cpi/12}"
    ...
    ENDIF
    "{cpi/12}{pos/300/139}", "Attending Physician: ", orders->attending,
    row + 1, "{cpi/14}{pos/60/156}", "Admit Date/Time: ",
    "{cpi/12}", orders->admit_dt, row + 1,
    "{cpi/14}{pos/60/173}", "Height: ", "{cpi/12}{b}",
    orders->height, "{endb}", "{cpi/14}{pos/155/173}",
    "Weight: ", "{cpi/12}{b}", orders->weight,
    row + 1, "{cpi/14}{pos/300/173}", "Isolation Type: ",
    "{cpi/12}{b}", orders->isolation, row + 1
    IF (saved_pos > 0)
     "{f/4}", "{cpi/12}{pos/60/190}{b}", "Order Action: ",
     "{cpi/10}", orders->qual[saved_pos].action, "{endb}",
     "{cpi/14}{pos/300/190}", "Ordering Physician: ", "{cpi/12}{b}",
     orders->qual[saved_pos].order_dr, row + 1, "{cpi/14}{pos/60/207}{b}",
     "Order Entered By: ", "{cpi/12}", orders->qual[saved_pos].enter_by,
     "{endb}", row + 1, "{cpi/14}{pos/300/207}",
     "Date/Time Ordered: ", "{cpi/12}{b}", orders->qual[saved_pos].signed_dt,
     row + 1, "{pos/55/228}{cpi/9}{COLOR/20/157}", row + 1,
     "{pos/55/233}{COLOR/20/157}", row + 1, "{pos/55/239}{COLOR/20/157}",
     row + 1, "{CPI/11}{pos/60/233}{b}", "Procedure/Order: ",
     orders->qual[saved_pos].mnemonic, row + 1, "{font/8}{cpi/12}{pos/50/738}{b}",
     "Order Id#: ",
     CALL print(trim(cnvtstring(cnvtint(orders->qual[saved_pos].order_id)))), "{endb}",
     saved_pos = 0
    ENDIF
   DETAIL
    FOR (vv = 1 TO value(ord_cnt))
      IF ((orders->qual[vv].display_ind=1))
       ...
       IF (first_page="N")
        BREAK
       ENDIF
       first_page = "N", "{f/4}", "{CPI/12}{pos/60/190}{b}",
       "Order Action: ", "{cpi/10}", orders->qual[vv].action,
       "{endb}", "{cpi/14}{pos/300/190}", "Ordering Physician: ",
       "{cpi/12}{b}", orders->qual[vv].order_dr, row + 1,
       "{cpi/14}{pos/60/207}{b}", "Order Entered By: ", "{cpi/12}",
       orders->qual[vv].enter_by, "{endb}", row + 1,
       "{cpi/14}{pos/300/207}", "Date/Time Ordered: ", "{cpi/12}{b}",
       orders->qual[vv].signed_dt, row + 1, "{cpi/9}{pos/55/228}{COLOR/20/157}",
       row + 1, "{pos/55/233}{COLOR/20/157}", row + 1,
       "{pos/55/239}{COLOR/20/157}", row + 1, "{cpi/11}{pos/60/233}{b}",
       "Procedure/Order: ", orders->qual[vv].mnemonic, row + 1,
       "{font/8}{cpi/12}{pos/50/738}{b}", "Order Id#: ",
       CALL print(trim(cnvtstring(cnvtint(orders->qual[vv].order_id)))),
       "{endb}", xcol = 60, ycol = 255
       FOR (fsub = 1 TO 31)
         FOR (ww = 1 TO orders->qual[vv].d_cnt)
          IF ((((orders->qual[vv].d_qual[ww].group_seq=fsub)) OR (fsub=31))
           ...
           IF (textlen(trim(orders->qual[vv].d_qual[ww].value,3)) > 0)
            ...
            "  ", row + 1, xcol = 212
            IF (textlen(orders->qual[vv].d_qual[ww].label_text) > 34)
             ycol = (ycol+ 12)
            ENDIF
          ...
          ,
          IF (ycol > 680
           AND (x < orders->qual[vv].d_cnt))
           saved_pos = vv, BREAK, xcol = 60,
           ycol = 255
          ENDIF
         ENDFOR
       ENDFOR
       xcol = 60, ycol = (ycol+ 15)
       IF (ycol > 544)
        saved_pos = vv, BREAK, ycol = 255
       ENDIF
       "{cpi/13}",
       CALL print(calcpos(xcol,ycol)), "Blood Bank ID: ",
       row + 1, ycol = (ycol+ 15), "{cpi/13}",
       CALL print(calcpos(xcol,ycol)), "Collected By: ", row + 1,
       ycol = (ycol+ 15), "{cpi/13}",
       CALL print(calcpos(xcol,ycol)),
       "Collection Date/Time: ", row + 1, ycol = (ycol+ 15)
       IF ((orders->qual[vv].comment_ind=1)
        AND (orders->qual[vv].com_ln_cnt > 0))
        xcol = 60, ycol = (ycol+ 15), "{cpi/13}",
        CALL print(calcpos(xcol,ycol)), "Comments: ", row + 1,
        ycol = (ycol+ 12)
        ...
        FOR (com_cnt = 1 TO ocnt)
          ...
          orders->qual[vv].com_ln_qual[com_cnt].com_line, "{endb}", row + 1,
          ycol = (ycol+ 12)
          IF (ycol > 680
           AND com_cnt < ocnt)
           saved_pos = vv, BREAK, xcol = 60,
           ycol = 255, "{cpi/13}",
           CALL print(calcpos(xcol,ycol)),
           "Comments (Cont.): ", row + 1, ycol = (ycol+ 12),
           xcol = 75
          ENDIF
        ENDFOR
        IF ((orders->qual[vv].com_ln_cnt > 7))
         "{CPI/13}",
         CALL print(calcpos(xcol,ycol)), "**** Please check chart for further comments ****",
         ycol = (ycol+ 12)
        ENDIF
       ENDIF
      ENDIF
