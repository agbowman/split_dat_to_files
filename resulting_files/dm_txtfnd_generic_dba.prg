CREATE PROGRAM dm_txtfnd_generic:dba
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 IF (check_logfile("dm_tgtfnd_generic",".log","DM_TXTFND_GENERIC LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_generic
 ENDIF
 SET dm_err->eproc = "Starting DM_TXTFND_GENERIC..."
 ...
 IF ((script_rec->dm_text_find_log_id <= 0.0))
  ...
  GO TO exit_generic
 ENDIF
 DECLARE dtg_ltr_id = f8 WITH protect, noconstant(0.0)
 DECLARE dtg_desc = vc WITH protect, noconstant("")
 DECLARE dtg_query = vc WITH protect, noconstant("")
 SET dm_err->eproc = "Querying DM_TEXT_FIND_DETAIL table."
 SELECT INTO "NL:"
  ...
  WHERE dm_text_find_detail_id=dtla_detail_id
   ...
   AND dm_text_find_detail_id > 0.0
  DETAIL
   dtg_ltr_id = d.detail_text_id, dtg_desc = d.detail_description, script_rec->dm_text_find_detail_id
    = d.dm_text_find_detail_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  GO TO exit_generic
 ENDIF
 IF (((curqual=0) OR (dtg_ltr_id=0.0)) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No DETAIL_TEXT_ID found in DM_TEXT_FIND_DETAIL table.  Unable to proceed."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
  GO TO exit_generic
 ENDIF
 SET dm_err->eproc = "Gathering LONG data"
 SELECT INTO "NL:"
  bloblen = textlen(l.long_text)
  FROM long_text_reference l
  WHERE l.long_text_id=dtg_ltr_id
  HEAD REPORT
  ...
  DETAIL
   ...
   WHILE (retlen > 0)
     ...
     IF (freers=0)
      dtg_query = notrim(concat(trim(dtg_query),notrim(outbuf)))
     ELSE
      dtg_query = notrim(concat(notrim(dtg_query),notrim(outbuf)))
     ENDIF
     freers = 1
   ENDWHILE
 ...
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying LONG_TEXT_REFERENCE:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_generic
 ENDIF
 IF (dtg_query > " ")
  SET dm_err->eproc = "Executing LONG query"
  CALL parser(dtg_query,1)
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Executing LONG query:",dm_err
     ->emsg),0.0,0.0)
   GO TO exit_generic
  ENDIF
  IF ((dtg->cnt > 0))
 ...
 ENDIF
#exit_generic
 SET dm_err->eproc = "Ending DM_TXTFND_GENERIC"
 CALL final_disp_msg("dm_txtfnd_generic")
END GO
