CREATE PROGRAM bhs_sys_inpt_mlh:dba
 PROMPT
 ...
 FREE DEFINE rtl
 DEFINE rtl "ccluserdir:2010_mlh_mrn.txt"
 SELECT INTO "nl:"
  FROM rtlt r
  WHERE r.line > " "
  HEAD REPORT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE (pa.alias=temp->qual[d.seq].mrn)
    ...
    AND ((pa.alias_pool_cd+ 0)=674614.00))
   JOIN (e
   WHERE e.person_id=pa.person_id
    AND e.disch_dt_tm BETWEEN cnvtdatetime(cnvtdate(080109),0) AND cnvtdatetime(cnvtdate(09312010),0)
