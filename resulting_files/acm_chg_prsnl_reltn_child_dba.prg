CREATE PROGRAM acm_chg_prsnl_reltn_child:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->prsnl_reltn_child_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   prsnl_reltn_child p
  SET p.prsnl_reltn_child_id = acm_request->prsnl_reltn_child_qual[xref->chg[d.seq].idx].
   prsnl_reltn_child_id, p.prsnl_reltn_id =
   IF ((acm_request->prsnl_reltn_child_qual[xref->chg[d.seq].idx].prsnl_reltn_id > 0)) acm_request->
    prsnl_reltn_child_qual[xref->chg[d.seq].idx].prsnl_reltn_id
   ELSE reply->prsnl_reltn_qual[acm_request->prsnl_reltn_child_qual[xref->chg[d.seq].idx].
    prsnl_reltn_idx].prsnl_reltn_id
   ENDIF
   , p.parent_entity_id = acm_request->prsnl_reltn_child_qual[xref->chg[d.seq].idx].parent_entity_id,
   p.parent_entity_name = acm_request->prsnl_reltn_child_qual[xref->chg[d.seq].idx].
   parent_entity_name, p.display_seq = acm_request->prsnl_reltn_child_qual[xref->chg[d.seq].idx].
   display_seq, p.beg_effective_dt_tm = cnvtdatetime(acm_request->prsnl_reltn_child_qual[xref->chg[d
    .seq].idx].beg_effective_dt_tm),
   p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_reltn_child_qual[xref->chg[d.seq].idx].
    end_effective_dt_tm), p.updt_cnt = (p.updt_cnt+ 1), p.updt_dt_tm = cnvtdatetime(curdate,curtime3),
   p.updt_id = reqinfo->updt_id, p.updt_applctx = reqinfo->updt_applctx, p.updt_task = reqinfo->
   updt_task
  PLAN (d)
   JOIN (p
   WHERE (p.prsnl_reltn_child_id=acm_request->prsnl_reltn_child_qual[xref->chg[d.seq].idx].
   prsnl_reltn_child_id))
  WITH nocounter, status(reply->prsnl_reltn_child_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->prsnl_reltn_child_qual[xref->chg[index].idx].status != 1))
    SET failed = update_error
    SET table_name = "PRSNL_RELTN_CHILD"
    GO TO exit_script
   ENDIF
 ENDFOR
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM prsnl_reltn_child p
    WHERE expand(t1,f_val,t_val,p.prsnl_reltn_child_id,acm_request->prsnl_reltn_child_qual[xref->chg[
     t1].idx].prsnl_reltn_child_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.prsnl_reltn_child_id,acm_request->prsnl_reltn_child_qual[xref->
      chg[t1].idx].prsnl_reltn_child_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->prsnl_reltn_child_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->prsnl_reltn_child_qual[idx].status = - (1), reply->prsnl_reltn_child_qual[idx].
      prsnl_reltn_child_id = acm_request->prsnl_reltn_child_qual[idx].prsnl_reltn_child_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->prsnl_reltn_child_qual[idx].chg_str
     IF (findstring("PRSNL_RELTN_ID,",chg_str)=0)
      acm_request->prsnl_reltn_child_qual[idx].prsnl_reltn_id = p.prsnl_reltn_id
     ENDIF
     IF (findstring("PARENT_ENTITY_ID,",chg_str)=0)
      acm_request->prsnl_reltn_child_qual[idx].parent_entity_id = p.parent_entity_id
     ENDIF
     IF (findstring("PARENT_ENTITY_NAME,",chg_str)=0)
      acm_request->prsnl_reltn_child_qual[idx].parent_entity_name = p.parent_entity_name
     ENDIF
     IF (findstring("DISPLAY_SEQ,",chg_str)=0)
      acm_request->prsnl_reltn_child_qual[idx].display_seq = p.display_seq
     ENDIF
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_reltn_child_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_reltn_child_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_reltn_child_qual[xref->chg[index].idx].status=0))
