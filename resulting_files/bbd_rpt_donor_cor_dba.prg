CREATE PROGRAM bbd_rpt_donor_cor:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 DECLARE deligcd = f8 WITH protect, noconstant(0.0)
 DECLARE dunlockcd = f8 WITH protect, noconstant(0.0)
 DECLARE dresultcd = f8 WITH protect, noconstant(0.0)
 DECLARE bnullrpt = i2 WITH protect, noconstant(0)
 DECLARE correction_cs = i4 WITH protect, constant(14115)
 DECLARE script_name = c17 WITH protect, constant("bbd_rpt_donor_cor")
 ...
 IF (trim(request->batch_selection) > " ")
  ...
  SET bnullrpt = evaluate(sret,"Y",1,0)
 ...
 ELSE
  SET sub_get_location_name = "<<INFORMATION NOT FOUND>>"
 ENDIF
 SET dunlockcd = uar_get_code_by("MEANING",correction_cs,"DNRUNLOCK")
 SET dresultcd = uar_get_code_by("MEANING",correction_cs,"DNRRESULTS")
 SET deligcd = uar_get_code_by("MEANING",correction_cs,"DNRELIG")
 ...
 IF ((request->correction_type_cd IN (dunlockcd, 0.0)))
  EXECUTE cpm_create_file_name_logical "bbd_unlock_donor", "txt", "x"
  ...
  EXECUTE bbd_rpt_donor_unlock  WITH replace("captions","cap1")
 ENDIF
 IF ((request->correction_type_cd IN (dresultcd, 0.0)))
  EXECUTE cpm_create_file_name_logical "bbd_donation_corr", "txt", "x"
  SET nrptcnt = (nrptcnt+ 1)
  SET result_disp = uar_get_code_display(dresultcd)
  SET stat = alterlist(reply->rpt_list,nrptcnt)
  SET reply->rpt_list[nrptcnt].rpt_filename = cpm_cfn_info->file_name_path
  EXECUTE bbd_rpt_donation_corr  WITH replace("captions","cap2")
 ENDIF
 IF ((request->correction_type_cd IN (deligcd, 0.0)))
  EXECUTE cpm_create_file_name_logical "bbd_elig_corr", "txt", "x"
  SET nrptcnt = (nrptcnt+ 1)
  SET elig_disp = uar_get_code_display(deligcd)
  SET stat = alterlist(reply->rpt_list,nrptcnt)
  SET reply->rpt_list[nrptcnt].rpt_filename = cpm_cfn_info->file_name_path
  EXECUTE bbd_rpt_elig_corr  WITH replace("captions","cap3")
 ENDIF
 IF (bopscall=1)
  IF (checkqueue(request->output_dist)=1)
   FOR (n = 1 TO nrptcnt)
     SET spool value(reply->rpt_list[n].rpt_filename) value(request->output_dist)
   ENDFOR
  ELSE
   SET reply->status_data.status = "F"
   SET reply->status_data.subeventstatus[1].operationname = script_name
   ...
   SET reply->status_data.subeventstatus[1].targetobjectname = script_name
   GO TO exit_script
  ENDIF
 ENDIF
