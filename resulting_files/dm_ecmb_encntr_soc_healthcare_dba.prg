CREATE PROGRAM dm_ecmb_encntr_soc_healthcare:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "ENCNTR_SOCIAL_HEALTHCARE"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_ECMB_ENCNTR_SOC_HEALTHCARE"
 ...
 SELECT INTO "nl:"
  frm.*
  FROM encntr_social_healthcare frm
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count_from].from_id = frm.encntr_social_healthcare_id, rreclist->
 ...
 SELECT INTO "nl:"
  tu.*
  FROM encntr_social_healthcare tu
  ...
  DETAIL
   ...
   ENDIF
   rreclist->to_rec[v_cust_count_to].to_id = tu.encntr_social_healthcare_id, rreclist->to_rec[
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_to_fk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =
  i4)
   UPDATE  FROM encntr_social_healthcare frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.active_status_dt_tm =
     ...
     cnvtdatetime(trans_dt_tm),
     frm.encntr_id = s_df_to_fk_id
    WHERE frm.encntr_social_healthcare_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "ENCNTR_SOCIAL_HEALTHCARE"
   ...
   IF (add_hist(s_df_pk_id)=0)
    ...
    SET request->error_message = substring(1,132,build(
      "Could not insert history for encntr_social_healthcare with pk=",s_df_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM encntr_social_healthcare frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.encntr_social_healthcare_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
 ...
 SUBROUTINE (add_hist(s_at_to_parent_pk_id=f8) =i4)
   ...
   SET dm_cmb_cust_cols2->add_col_val[1].col_name = "ENCNTR_SOCIAL_HEALTH_HIST_ID"
   ...
   SET dm_cmb_cust_cols2->where_col_val[1].col_name = "ENCNTR_SOCIAL_HEALTHCARE_ID"
   SET dm_cmb_cust_cols2->where_col_val[1].col_value = build(s_at_to_parent_pk_id)
   IF (size(dm_cmb_cust_cols2->col,5)=0)
    SET dm_cmb_cust_cols2->tbl_name = "ENCNTR_SOCIAL_HEALTH_HIST"
    SET dm_cmb_cust_cols2->sub_select_from_tbl = "ENCNTR_SOCIAL_HEALTHCARE"
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0)
    ...
    SET dcipht_request->transaction_reason_txt = "ENCOUNTER_CMB_ENCNTR_SOCIAL_HEALTHCARE"
