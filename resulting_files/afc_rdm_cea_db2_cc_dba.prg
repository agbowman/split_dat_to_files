CREATE PROGRAM afc_rdm_cea_db2_cc:dba
 ...
 SET ms_child_c_info_domain_nm = "AFC_RDM_CEA_DB2"
 SET ms_child_c_max_name = "MAX CHARGE EVENT ID EVALUATED"
 CALL sbr_get_min_max(mf_min_id,mf_max_id)
 CALL echo("Processing...")
 CALL echo(concat(cnvtstring(mf_min_id)," thru ",cnvtstring(mf_max_id)))
 UPDATE  FROM charge_event_act cea
  SET cea.item_ext_price = cea.cea_misc2_id, cea.item_price = cea.cea_misc4_id, cea.item_copay = cea
   .cea_misc5_id,
   cea.item_reimbursement = cea.cea_misc6_id, cea.discount_amount = cea.cea_misc7_id
  WHERE cea.charge_event_act_id >= mf_min_id
   AND cea.charge_event_act_id <= mf_max_id
  WITH nocounter
 ;end update
