CREATE PROGRAM dab_pt_transfer_a2tst:dba
 PROMPT
 ...
 SELECT INTO "NL:"
  ...
  DETAIL
   IF (trim(srp.display_key) IN (value("NURSINGPROGRESSNOTEBHS*"))
    ...
    cnt = (cnt+ 1), info->powernote[cnt].title = trim(srp.display,3), info->powernote[cnt].sortseq =
    1,
    info->powernote[cnt].event_id = ce.event_id, nursingprogressnotebhs_cnt = (
    nursingprogressnotebhs_cnt+ 1)
   ELSEIF (trim(srp.display_key) IN (value("WOUNDCAREPROGRESSNOTE*"))
    AND woundcareprogressnote_cnt < 3)
    cnt = (cnt+ 1), info->powernote[cnt].title = trim(srp.display,3), info->powernote[cnt].sortseq =
    2,
    info->powernote[cnt].event_id = ce.event_id, woundcareprogressnote_cnt = (
    woundcareprogressnote_cnt+ 1)
   ENDIF
  FOOT REPORT
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   xcol = 500, row + 1,
   CALL print(calcpos(xcol,ycol)),
   pt_transfer->religion, ycol = (ycol+ 12), row + 1,
   xcol = 30, row + 1,
   CALL print(calcpos(xcol,ycol)),
   ...
   xcol = 310, row + 1,
   CALL print(calcpos(xcol,ycol)),
   pt_transfer->pcp, ycol = (ycol+ 12), row + 1,
   xcol = 30, row + 1,
   CALL print(calcpos(xcol,ycol)),
   "{b}Admit Reason:{endb}"
   ...
   IF (found=0)
    ycol = (ycol+ 12), row + 1, xcol = 30,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}No Discharge/Transfer Note found on visit.{endb}", ycol = (ycol+ 12),
    row + 1
   ENDIF
   found = 0
   IF (test_ind=1)
   ...
   ENDIF
   xcol = 30, ycol = (ycol+ 12), row + 1,
   CALL print(calcpos(xcol,ycol)), "{b}{REPEAT/110/_/}{endb}", ycol = (ycol+ 12),
   row + 1
   IF (ycol > 725)
    BREAK
   ENDIF
   ...
   IF (ycol > 725)
    BREAK
   ENDIF
   BREAK, ycol = (ycol+ 12), row + 1,
   xcol = 30, row + 1,
   CALL print(calcpos(xcol,ycol)),
   "{f/0}{cpi/14}{b}{u}Current Medication List{endu}{endb}", ycol = (ycol+ 12), row + 1
   IF (ycol > 725)
    BREAK
   ENDIF
   ...
   IF (size(continuous_orders_disp->continuous_orders,5) > 0)
    FOR (z1 = 1 TO size(continuous_orders_disp->continuous_orders,5))
      IF ((continuous_orders_disp->continuous_orders[z1].action_type="Ordered"))
       ...
       meds_found = 1
       IF (print_title=0)
        xcol = 30, ycol = (ycol+ 12), title = build(
         "{b}{u}IV Infusions before Discharge:{endb}{endu}"),
   ...
   IF (size(prn_orders_disp->prn_orders,5) > 0)
    FOR (z1 = 1 TO size(prn_orders_disp->prn_orders,5))
      IF ((prn_orders_disp->prn_orders[z1].action_type="Ordered"))
       ...
       ENDIF
       IF (print_title=0)
        xcol = 30, ycol = (ycol+ 12), title = build("{b}{u}PRN Meds before Discharge:{endb}{endu}"),
   ...
   IF (size(wound_band->wound_section,5) > 0
    ...
    FOR (wndord_prt = 1 TO size(wounds->wound_ord,5))
      xcol = 30, ycol = (ycol+ 12), row + 1,
      CALL print(calcpos(xcol,ycol)), wounds->wound_ord[wndord_prt].name, ycol = (ycol+ 12),
      row + 1
    ENDFOR
    FOR (wndband_prt = 1 TO size(wound_band->wound_section,5))
      xcol = 30, ycol = (ycol+ 12), row + 1,
      CALL print(calcpos(xcol,ycol)), "{b}{u}", wound_band->band_name,
      "{endu}{endb}", ycol = (ycol+ 12), row + 1
      FOR (wndres_prt = 1 TO size(wound_band->wound_section[wndband_prt].result,5))
       IF (wndres_prt=1)
        IF (ycol > 713)
         BREAK, xcol = 30, ycol = (ycol+ 12),
         row + 1,
         CALL print(calcpos(xcol,ycol)), "{b}",
         wound_band->band_name, "{endb}", ycol = (ycol+ 12),
         row + 1, xcol = 30, ycol = (ycol+ 12),
         row + 1,
         CALL print(calcpos(xcol,ycol)), "{b}",
       ...
       IF (ycol > 725)
        ...
        row + 1,
        CALL print(calcpos(xcol,ycol)), "{b}",
        wound_band->wound_section[wndband_prt].section_name, "{endb}", ycol = (ycol+ 12),
        row + 1
       ELSE
        xcol = 30, ycol = (ycol+ 10), row + 1
   ...
   ELSE
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{b}{u}No Wound Results{endu}{endb}", ycol = (ycol+ 12), row + 1
   ENDIF
   xcol = 30, ycol = (ycol+ 12), row + 1,
   CALL print(calcpos(xcol,ycol)), "{b}{REPEAT/110/_/}{endb}", ycol = (ycol+ 12),
   ...
   IF ((pt_transfer->special_labs_cnt > 0))
    FOR (x = 1 TO size(pt_transfer->special_labs,5))
      ...
      pt_transfer->special_labs[x].header2, "{endb}{endu}", ycol = (ycol+ 12),
      row + 1
   ...
   IF (size(info->powernote,5) > 0)
    xcol = 30, ycol = (ycol+ 12), row + 1,
    CALL print(calcpos(xcol,ycol)), "{b}{u}Last Three Progress Notes:{endu}{endb}", ycol = (ycol+ 12),
    row + 1
   ...
   ELSE
    xcol = 30, ycol = (ycol+ 12), row + 1,
    CALL print(calcpos(xcol,ycol)), "{b}{u}No Progress Notes Found:{endu}{endb}", ycol = (ycol+ 12),
    row + 1
   ENDIF
 ...
 SELECT INTO value(printer_disp)
  ...
  DETAIL
   ...
   ELSE
    null, xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)), "{b}No Ancillary Documenation.{endb}", ycol = (ycol+ 12),
    row + 1
   ENDIF
