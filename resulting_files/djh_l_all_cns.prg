CREATE PROGRAM djh_l_all_cns
 ...
 SELECT INTO  $OUTDEV
  ...
  p.updt_dt_tm, p.updt_id, p1.name_full_formatted
  FROM prsnl p,
   prsnl p1
  PLAN (p
   WHERE p.active_ind=1
    AND p.position_cd > 0
    AND ((p.username="CN*") OR (p.username="CR*")) )
   JOIN (p1
  ...
  HEAD REPORT
   ...
   , row + 1,
   "{F/1}{CPI/10}",
   CALL print(calcpos(152,(y_pos+ 11))), "List Active Contractors/Consulants",
