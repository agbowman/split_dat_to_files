CREATE PROGRAM bhs_rpt_joint_comm_test:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   m_lname_to_exclude_rec->n_cnt = (m_lname_to_exclude_rec->n_cnt+ 1), stat = alterlist(
    m_lname_to_exclude_rec->person,m_lname_to_exclude_rec->n_cnt), m_lname_to_exclude_rec->person[
   m_lname_to_exclude_rec->n_cnt].f_prsnl_id = p.person_id
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM code_value cv
  ...
  DETAIL
   m_position_cd_rec->n_pos_cd_cnt = (m_position_cd_rec->n_pos_cd_cnt+ 1), stat = alterlist(
    m_position_cd_rec->position,m_position_cd_rec->n_pos_cd_cnt), m_position_cd_rec->position[
   m_position_cd_rec->n_pos_cd_cnt].s_display = trim(cv.display,3),
 ...
 ELSEIF (ms_report_name="BWH")
  SELECT INTO "nl:"
   ...
   HEAD REPORT
    mn_org_cnt = 0
   DETAIL
    mn_org_cnt = (mn_org_cnt+ 1), stat = alterlist(m_organization_rec->organization,mn_org_cnt),
 ...
 IF (ms_report_name != "BMCRDR")
  SELECT INTO value( $OUTDEV)
   ...
   PLAN (c
    ...
    JOIN (d
    WHERE outerjoin(d.encntr_id)=c.encntr_id
 ...
 ELSE
  SELECT INTO value( $OUTDEV)
   ...
   PLAN (c
    ...
    JOIN (b
    WHERE outerjoin(d.action_prsnl_id)=b.person_id)
