CREATE PROGRAM dm_mic_qc_result_sched_rows:dba
 ...
 SET reply->table_name = "MIC_QC_RESULT_SCHEDULE"
 SET reply->rows_between_commit = 50
 DECLARE lmonthstokeep = i4 WITH protect, noconstant(- (1))
 DECLARE dpurgedttm = dq8 WITH protect, noconstant(0.0)
 DECLARE ltokenidx = i4 WITH protect, noconstant(0)
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 FOR (ltokenidx = 1 TO size(request->tokens,5))
   IF ((request->tokens[ltokenidx].token_str="MONTHSTOKEEP"))
    SET lmonthstokeep = ceil(cnvtreal(request->tokens[ltokenidx].value))
   ENDIF
 ENDFOR
 IF (lmonthstokeep < 24)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"TOKENOUTOFORANGE",
   "You must keep at least 24 month's worth of data.  You entered %1 month(s) or did not enter any value.",
   "i",lmonthstokeep)
  GO TO exit_script
 ENDIF
 SET dpurgedttm = cnvtlookbehind(build(lmonthstokeep,"M"),cnvtdatetime(curdate,curtime3))
 SELECT INTO "nl:"
  mqrs.rowid
  FROM mic_qc_result_schedule mqrs
  WHERE mqrs.complete_ind=1
   AND ((mqrs.result_schedule_id+ 0) > 0)
   AND ((((mqrs.result_sched_dt_tm+ 0) < cnvtdatetime(dpurgedttm))) OR (((mqrs.setup_sched_dt_tm+ 0)
   < cnvtdatetime(dpurgedttm))))
  HEAD REPORT
   row_cnt = 0
  DETAIL
   row_cnt = (row_cnt+ 1)
   ...
   ENDIF
   reply->rows[row_cnt].row_id = mqrs.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,row_cnt)
  WITH nocounter, maxqual(mqrs,value(request->max_rows))
 ;end select
