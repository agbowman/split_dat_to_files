CREATE PROGRAM bhs_athn_get_endorsements_v2
 FREE RECORD result
 RECORD result(
   1 endorsements[*]
     2 person_id = f8
 ...
 FREE RECORD req967146
 RECORD req967146(
   1 assign_prsnl_group_list[*]
     2 assign_prsnl_group_id = f8
   1 task_type_list[*]
     2 task_type = i2
   1 beg_dt_tm = dq8
   1 end_dt_tm = dq8
 ...
 DECLARE callgetendorsementsforpool(null) = i4
 ...
 IF (( $5 > 0.0))
  SET stat = callgetendorsementsforpool(null)
  IF (stat=fail)
   GO TO exit_script
  ENDIF
 ELSE
...
#exit_script
 ...
 SUBROUTINE callgetendorsements(null)
   ...
   IF ((rep967140->status_data.status="S"))
    ...
    FOR (idx = 1 TO size(rep967140->endorsement_list,5))
      SET discrete_ind = evaluate(rep967140->endorsement_list[idx].event_class_cd,c_hlatyping_cd,0,
       ...
       0,c_helix_cd,0,c_resultdoc_cd,0,
       1)
      IF (discrete_ind=1)
   ...
   RETURN(fail)
 END ;Subroutine
 SUBROUTINE callgetendorsementsforpool(null)
   DECLARE applicationid = i4 WITH protect, constant(600005)
   DECLARE taskid = i4 WITH protect, constant(967100)
   DECLARE requestid = i4 WITH protect, constant(967146)
   SET stat = alterlist(req967146->assign_prsnl_group_list,1)
   SET req967146->assign_prsnl_group_list[1].assign_prsnl_group_id =  $5
   SET stat = alterlist(req967146->task_type_list,2)
   SET req967146->task_type_list[1].task_type = 1
   SET req967146->task_type_list[2].task_type = 3
   SET req967146->beg_dt_tm = cnvtdatetime( $3)
   SET req967146->end_dt_tm = cnvtdatetime( $4)
   SET req967146->get_onhold_endorsements_ind = 1
   SET req967146->load_name_ind = 1
   CALL echorecord(req967146)
   CALL echo(build("TDBEXECUTE FOR ",requestid))
   SET stat = tdbexecute(applicationid,taskid,requestid,"REC",req967146,
    "REC",rep967146,1)
   IF (stat > 0)
    SET errcode = error(errmsg,1)
    CALL echo(build("TDBEXECUTE ",requestid," - ERROR CODE: ",errcode," - ERROR MESSAGE: ",
    ...
    RETURN(fail)
   ENDIF
   CALL echorecord(rep967146)
   IF ((rep967146->status_data.status="S"))
    SET stat = alterlist(result->endorsements,size(rep967146->endorsement_list,5))
    FOR (idx = 1 TO size(rep967146->endorsement_list,5))
      SET discrete_ind = evaluate(rep967146->endorsement_list[idx].event_class_cd,c_hlatyping_cd,0,
       c_rad_cd,0,
       c_ap_cd,0,c_mbo_cd,0,c_procedure_cd,
       0,c_helix_cd,0,c_resultdoc_cd,0,
       1)
      IF (discrete_ind=1)
       SET pos = locateval(locidx,1,ecnt,rep967146->endorsement_list[idx].task_activity_cd,result->
        endorsements[locidx].task_activity_cd,
        rep967146->endorsement_list[idx].person_id,result->endorsements[locidx].person_id,1,result->
        endorsements[locidx].discrete_ind)
      ELSE
       SET pos = locateval(locidx,1,ecnt,rep967146->endorsement_list[idx].task_activity_cd,result->
        endorsements[locidx].task_activity_cd,
        rep967146->endorsement_list[idx].person_id,result->endorsements[locidx].person_id,rep967146->
        endorsement_list[idx].event_id,result->endorsements[locidx].non_discrete_event_id)
      ENDIF
      IF (pos=0)
       SET ecnt = (ecnt+ 1)
       SET pos = ecnt
       SET result->endorsements[pos].task_activity_cd = rep967146->endorsement_list[idx].
       task_activity_cd
       SET result->endorsements[pos].task_activity_meaning = rep967146->endorsement_list[idx].
       task_activity_meaning
       SET result->endorsements[pos].person_id = rep967146->endorsement_list[idx].person_id
       SET result->endorsements[pos].person_name = rep967146->endorsement_list[idx].person_name
       IF ((result->endorsements[pos].task_activity_meaning="SIGN RESULT"))
        SET result->endorsements[pos].type = c_type_to_sign
       ELSEIF ((result->endorsements[pos].task_activity_meaning="REVIEW RESUL"))
        SET result->endorsements[pos].type = c_type_to_review
       ...
       ENDIF
       SET result->endorsements[pos].discrete_ind = discrete_ind
       SET result->endorsements[pos].non_discrete_event_id = evaluate(discrete_ind,1,0,rep967146->
        endorsement_list[idx].event_id)
      ENDIF
      SET tcnt = (size(result->endorsements[pos].tasks,5)+ 1)
      SET stat = alterlist(result->endorsements[pos].tasks,tcnt)
      SET result->endorsements[pos].tasks[tcnt].task_id = rep967146->endorsement_list[idx].task_id
      SET result->endorsements[pos].tasks[tcnt].task_status_cd = rep967146->endorsement_list[idx].
      task_status_cd
      SET result->endorsements[pos].tasks[tcnt].task_status_disp = rep967146->endorsement_list[idx].
      task_status_display
      SET result->endorsements[pos].tasks[tcnt].task_status_meaning = rep967146->endorsement_list[idx
      ].task_status_meaning
      SET result->endorsements[pos].tasks[tcnt].task_dt_tm = rep967146->endorsement_list[idx].
      task_dt_tm
      SET result->endorsements[pos].tasks[tcnt].task_create_dt_tm = rep967146->endorsement_list[idx].
      task_create_dt_tm
      SET result->endorsements[pos].tasks[tcnt].reference_task_id = rep967146->endorsement_list[idx].
      reference_task_id
      SET result->endorsements[pos].tasks[tcnt].event_id = rep967146->endorsement_list[idx].event_id
      SET result->endorsements[pos].tasks[tcnt].event_tag = rep967146->endorsement_list[idx].
      event_tag
      SET result->endorsements[pos].tasks[tcnt].event_class_cd = rep967146->endorsement_list[idx].
      event_class_cd
      SET result->endorsements[pos].tasks[tcnt].event_class_meaning = rep967146->endorsement_list[idx
      ].event_class_meaning
      SET result->endorsements[pos].tasks[tcnt].event_end_dt_tm = rep967146->endorsement_list[idx].
      event_end_dt_tm
      SET result->endorsements[pos].tasks[tcnt].performed_prsnl_id = rep967146->endorsement_list[idx]
      .performed_prsnl_id
      SET result->endorsements[pos].tasks[tcnt].performed_prsnl_name = rep967146->endorsement_list[
      idx].performed_prsnl_name
      SET result->endorsements[pos].tasks[tcnt].result_status_cd = rep967146->endorsement_list[idx].
      result_status_cd
      SET result->endorsements[pos].tasks[tcnt].result_status_disp = rep967146->endorsement_list[idx]
      .result_status_display
      SET result->endorsements[pos].tasks[tcnt].event_set_cd = rep967146->endorsement_list[idx].
      event_set_cd
      SET result->endorsements[pos].tasks[tcnt].event_set_display = rep967146->endorsement_list[idx].
      event_set_display
      SET result->endorsements[pos].tasks[tcnt].parent_event_id = rep967146->endorsement_list[idx].
      parent_event_id
      SET result->endorsements[pos].tasks[tcnt].normalcy_cd = rep967146->endorsement_list[idx].
      normalcy_cd
      SET result->endorsements[pos].tasks[tcnt].comment = rep967146->endorsement_list[idx].comment
      SET result->endorsements[pos].tasks[tcnt].msg_sender_id = rep967146->endorsement_list[idx].
      msg_sender_id
      SET result->endorsements[pos].tasks[tcnt].msg_sender_name = rep967146->endorsement_list[idx].
      msg_sender_name
      SET result->endorsements[pos].tasks[tcnt].updt_dt_tm = rep967146->endorsement_list[idx].
      updt_dt_tm
      SET result->endorsements[pos].tasks[tcnt].encntr_id = rep967146->endorsement_list[idx].
      encntr_id
      SET result->endorsements[pos].tasks[tcnt].msg_subject = rep967146->endorsement_list[idx].
      msg_subject
    ENDFOR
    SET stat = alterlist(result->endorsements,ecnt)
    FOR (idx = 1 TO ecnt)
    ...
    ENDFOR
    RETURN(success)
   ELSEIF ((rep967146->status_data.status="Z"))
    RETURN(success)
   ENDIF
   RETURN(fail)
 END ;Subroutine
