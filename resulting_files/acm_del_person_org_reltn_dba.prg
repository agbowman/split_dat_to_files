CREATE PROGRAM acm_del_person_org_reltn:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.person_org_reltn_id=acm_request->person_org_reltn_qual[xref->del[d.seq].idx].
   person_org_reltn_id))
  WITH nocounter, status(reply->person_org_reltn_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->person_org_reltn_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,p.person_org_reltn_id,acm_request->person_org_reltn_qual[xref->del[t1
     ].idx].person_org_reltn_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.person_org_reltn_id,acm_request->person_org_reltn_qual[xref->
      del[t1].idx].person_org_reltn_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->person_org_reltn_qual[xref->del[index].idx].status=0))
