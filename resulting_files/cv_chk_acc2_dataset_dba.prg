CREATE PROGRAM cv_chk_acc2_dataset:dba
 ...
 SET failure = "F"
 SET v_count1 = 0
 SET xref_expect_cnt = 163
 SET response_expect_cnt = 549
 SET dataset_expect_cnt = 1
 SET xref_flag = 0
 SET response_flag = 0
 SET dataset_flag = 0
 ...
 SET readme_type = "CV DATASET CSV FILE:"
 SELECT INTO "nl:"
  table_count = count(*)
  FROM cv_dataset cd
  ...
  WITH nocounter
 ;end select
 IF (dataset_actual_cnt != dataset_expect_cnt)
  SET failure = "T"
  SET dataset_flag = 1
 ENDIF
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 IF (xref_actual_cnt != xref_expect_cnt)
  SET failure = "T"
  SET xref_flag = 1
 ENDIF
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 IF (response_actual_cnt != response_expect_cnt)
  SET failure = "T"
  SET response_flag = 1
 ENDIF
 IF (failure="T")
  DECLARE error_msg = vc
  SET error_msg = " Incorrect ACC-V2 Dataset file readme counts found in the following tables:"
  IF (dataset_flag=1)
 ...
 ELSE
  ...
  CALL echo("CVNet Dataset CSV File has been successfully loaded!")
 ENDIF
