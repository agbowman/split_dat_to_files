CREATE PROGRAM dm2_aload_adoc_daf
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_adoc_daf",".log","dm2_aload_adoc_daf")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_adoc_daf"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dadaf_cnt = i4 WITH protect, noconstant(0)
 DECLARE dadaf_str = vc WITH protect, noconstant("")
 DECLARE dadaf_req_size = i4 WITH protect, noconstant(0)
 SET dadaf_req_size = size(requestin->list_0,5)
 IF (dac_pop_coldic_rec("DM_ALPHA_FEATURES")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  SET dm_err->eproc = "Determining maintenance type for DM_ALPHA_FEATURES."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_alpha_features
  WHERE alpha_feature_nbr=cnvtint(requestin->list_0[1].alpha_feature_nbr)
   AND owner=currdbuser
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(dadaf_req_size) )"),0)
  ...
  CALL dm2_push_cmd("plan d where d.seq > 0 ",0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_adoc_daf completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_adoc_daf")
END GO
