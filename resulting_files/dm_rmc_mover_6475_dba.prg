CREATE PROGRAM dm_rmc_mover_6475:dba
 ...
 SUBROUTINE drmm_breakup_string(sbr_rs_reply)
   ...
   RETURN("S")
 END ;Subroutine
 DECLARE cust_temp_id = f8
 DECLARE cust_disp_val = vc
 DECLARE cust_from_value = vc
 DECLARE cust_column_cnt = i4
 DECLARE cust_tab_name = vc
 DECLARE cust_temp_table_str = vc
 DECLARE cust_item = vc
 DECLARE cust_item_col = vc
 DECLARE cust_rpt_missing = vc
 DECLARE cust_err_val = i4
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_column_cnt = get_col_pos(cust_tab_name,"EVALUATOR_TXT")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_column_cnt].translated = 0
 SET cust_temp_id = cnvtreal(get_value(cust_tab_name,"PFT_TASK_CRITERIA_ID","FROM"))
 SET cust_temp_table_str = dm2_get_rdds_tname("TASK_CRITERIA")
 SELECT DISTINCT INTO "nl:"
  t.pft_criteria_name
  FROM (parser(cust_temp_table_str) t)
  WHERE t.pft_task_criteria_id=cust_temp_id
  DETAIL
   cust_disp_val = t.pft_criteria_name
  WITH nocounter
 ;end select
 IF (curqual > 1)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV24"
  CALL disp_msg("More than one row qualified when trying to retrieve Display Value",dm_err->logfile,0
   )
  GO TO exit_6475
 ENDIF
 SET cust_from_value = get_value(cust_tab_name,"EVALUATOR_TXT","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_6475
 ENDIF
 CASE (cust_disp_val)
  OF "Days Since Claims Transmission":
  OF "Days from Discharge":
  OF "Days in Self Pay Balance":
  OF "Days since claim generation":
  OF "Days since payment applied":
  OF "Dollar Amount of Self Pay":
  OF "Encounter Balance":
  OF "Financial Encounter balance":
  OF "Guarantor Last Name":
  OF "Patient Last Name":
  OF "Dollar amount of claim":
   CALL put_value(cust_tab_name,"EVALUATOR_TXT",cust_from_value)
  OF "Dunning Level":
  OF "Encounter Type to Exclude":
  OF "Financial Class":
   SET cust_item = "CODE_VALUE"
   SET cust_item_col = "CODE_VALUE"
  OF "Primary Insurance Organization":
  OF "Insurance Organization":
   SET cust_item = "ORGANIZATION"
   SET cust_item_col = "ORGANIZATION_ID"
  OF "Primary Health Plan":
   SET cust_item = "HEALTH_PLAN"
   SET cust_item_col = "HEALTH_PLAN_ID"
  ELSE
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV24"
   CALL disp_msg(concat("ERROR: the display value of ",cust_disp_val,
     " is not supported at this time."),dm_err->logfile,0)
   GO TO exit_6475
 ENDCASE
 IF (cust_item > " ")
  IF (cnvtreal(cust_from_value) > 0)
   SET new_value = select_merge_translate(cust_from_value,cust_item)
   IF (isnumeric(new_value))
    CALL put_value(cust_tab_name,"EVALUATOR_TXT",new_value)
   ELSE
    SET cust_rpt_missing = report_missing(cust_item,cust_item_col,cnvtreal(cust_from_value))
 ...
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_6475
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"PFT_CA_CRITERIA_VALUE_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1)
   ...
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"PFT_CA_CRITERIA_VALUE_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"PFT_CA_CRITERIA_VALUE_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,
       "  PFT_CA_CRITERIA_VALUE_ID")
     ELSE
      GO TO exit_6475
     ENDIF
    ELSE
   ...
   SET from_cd = get_value(cust_tab_name,"PFT_CA_CRITERIA_VALUE_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6475
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(from_cd)
   ENDIF
   SET new_cd = get_value(cust_tab_name,"PFT_CA_CRITERIA_VALUE_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6475
   ELSE
 ...
 ENDIF
 IF (idcd_check=1)
  SET idcd_check = is_translated(cust_tab_name,"EVALUATOR_TXT")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg =
    "This row wasn't translated because of the EVALUATOR_TXT column"
