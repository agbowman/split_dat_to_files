CREATE PROGRAM bhs_ma_ancillary_pend2:dba
 PROMPT
 ...
 SELECT INTO  $1
  ...
  FOOT REPORT
   ...
   ELSE
    row + 2
   ENDIF
   CALL center("*****  END OF REPORT  *****",1,178), row 52, col 1,
   line, row + 1, col 1,
   "Page: ", curpage"###", col 143,
   "Report Date: ", curdate, col 166,
   curtime
  WITH nocounter, nullreport, maxrow = 55,
   maxcol = 180, landscape
 ;end select
 IF (ops_ind != "N")
