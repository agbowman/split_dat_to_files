CREATE PROGRAM bhs_ma_extract_rad_results:dba
 PROMPT
 ...
 DECLARE bwh_fin_pool_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",263,"BWHACCTNBR"))
 DECLARE bnh_fin_pool_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",263,"BNHACCTNBR"))
 ...
 SELECT INTO concat("export_dir:",filename)
  ...
  HEAD ce.order_id
   ...
   ELSEIF (fin_pool=bwh_fin_pool_cd)
    temp->qual[cnt].facility_alias = "BWH"
   ELSEIF (fin_pool=bnh_fin_pool_cd)
    temp->qual[cnt].facility_alias = "BNH"
   ENDIF
   ...
   text = concat(trim(mrn_alias),"|",trim(p.name_last_key),"|",trim(p.name_first_key),
    "|",trim(format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1),"YYYYMMDDHHMMSS;;D")),
    "|",trim(sex),"|",
