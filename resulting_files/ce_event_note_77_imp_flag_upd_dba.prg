CREATE PROGRAM ce_event_note_77_imp_flag_upd:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 UPDATE  FROM ce_event_note t,
  ...
  SET t.ce_event_note_id = evaluate2(
   ...
   t.valid_until_dt_tm = evaluate2(
    ...
    ENDIF
    ), t.importance_flag = request->lst[d.seq].importance_flag, t.updt_dt_tm = cnvtdatetimeutc(
    request->lst[d.seq].updt_dt_tm),
  ...
  PLAN (d)
   JOIN (t
   WHERE (t.ce_event_note_id=request->lst[d.seq].ce_event_note_id))
