CREATE PROGRAM bhs_rpt_surg_downtime_mar:dba
 FREE RECORD work
 RECORD work(
   1 beg_dt_tm = dq8
   1 end_dt_tm = dq8
 ...
 SET work->unit_str = trim(ms_surg_loc_ops,3)
 ...
 ELSE
  SET test_ind = 0
 ENDIF
 DECLARE ms_varlocation = vc WITH noconstant("  "), protect
 DECLARE ms_varoutput = vc WITH noconstant(" "), protect
 IF (ms_surg_loc_ops != " ")
  SET ms_varoutput = ms_ops_printer
  SET ms_varlocation = ms_surg_loc_ops
  CALL echo(build("varlocation =",ms_varlocation))
  CALL echo(build("ms_varoutput =",ms_varoutput))
 ENDIF
 SET unit_cd = 0
 DECLARE unit_cd_disp_key = vc WITH protect, noconstant("")
 IF ( NOT (validate(requestin)))
  FREE RECORD requestin
  RECORD requestin(
    1 list_0[*]
      2 room_id = vc
      2 facility_account_no = vc
      2 medical_record_no = vc
      2 case_no = vc
      2 diagnosis = vc
      2 provider_short_name = vc
      2 case_comments = vc
      2 procedure_desc = vc
      2 abbr_codes_pat_type = vc
      2 start_date = vc
      2 start_time = vc
      2 description = vc
      2 abbr_codes_anes_type = vc
      2 create_date = vc
      2 create_time = vc
      2 stop_time = vc
      2 mrap_id = vc
      2 main_procedure_id = vc
      2 facility_id = vc
  )
 ENDIF
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   JOIN (ea
   WHERE ea.alias=format(cnvtstring(requestin->list_0[d1.seq].facility_account_no),
    "####################")
  ...
  HEAD d1.seq
   ...
   encntrs[work->e_cnt].room = trim(requestin->list_0[d1.seq].room_id,3), work->encntrs[work->e_cnt].
   allergy_text = "NKA",
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 CALL echo("Gather Medications Start")
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 CALL echo("Gather Medications end")
 SELECT INTO "nl:"
 ...
 WITH protect
 DECLARE site_legends(ncalc=i2) = f8 WITH protect
 DECLARE site_legendsabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE mar_section_head(ncalc=i2) = f8 WITH protect
 DECLARE mar_section_headabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE report_divider(ncalc=i2) = f8 WITH protect
 DECLARE report_dividerabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE med_description(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE med_descriptionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8 WITH
 protect
 DECLARE med_details(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE med_detailsabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8 WITH
 protect
 DECLARE med_instructions(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE med_instructionsabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8
 WITH protect
 DECLARE med_last_dose(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE med_last_doseabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8 WITH
 protect
 DECLARE med_next_dose(ncalc=i2) = f8 WITH protect
 DECLARE med_next_doseabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE med_assoc_dtas(ncalc=i2) = f8 WITH protect
 DECLARE med_assoc_dtasabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE med_addtl_notes(ncalc=i2) = f8 WITH protect
 DECLARE med_addtl_notesabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE refused_medications(ncalc=i2) = f8 WITH protect
 DECLARE refused_medicationsabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE refused_med_notes(ncalc=i2) = f8 WITH protect
 DECLARE refused_med_notesabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE prn_angio_head(ncalc=i2) = f8 WITH protect
 DECLARE prn_angio_headabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE prn_angio_notes(ncalc=i2) = f8 WITH protect
 DECLARE prn_angio_notesabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 ...
 DECLARE med_blank_details_sch(ncalc=i2) = f8 WITH protect
 DECLARE med_blank_details_schabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE med_blank_details_prn(ncalc=i2) = f8 WITH protect
 DECLARE med_blank_details_prnabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE med_blank_details_iv(ncalc=i2) = f8 WITH protect
 DECLARE med_blank_details_ivabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 ...
 SUBROUTINE patient_allergiesabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE site_legends(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = site_legendsabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE site_legendsabs(ncalc,offsetx,offsety)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE mar_section_head(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = mar_section_headabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE mar_section_headabs(ncalc,offsetx,offsety)
 ...
 SUBROUTINE report_dividerabs(ncalc,offsetx,offsety)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE med_description(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = med_descriptionabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE med_descriptionabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   IF (_remmed_desc > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remmed_desc,((size(__med_desc) -
     ...
     SET _remmed_desc = (_remmed_desc+ rptsd->m_drawlength)
    ELSE
     SET _remmed_desc = 0
    ENDIF
    SET growsum = (growsum+ _remmed_desc)
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_y = (offsety+ 0.000)
   SET rptsd->m_x = (offsetx+ 0.500)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE med_details(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = med_detailsabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE med_detailsabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   ...
   IF (_remmed_order_details > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remmed_order_details,((size(
     ...
     SET _remmed_order_details = (_remmed_order_details+ rptsd->m_drawlength)
    ELSE
     SET _remmed_order_details = 0
    ENDIF
    SET growsum = (growsum+ _remmed_order_details)
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE med_instructions(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = med_instructionsabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE med_instructionsabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   ...
   IF (_remmed_order_instructions > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remmed_order_instructions,((size(
     ...
     SET _remmed_order_instructions = (_remmed_order_instructions+ rptsd->m_drawlength)
    ELSE
     SET _remmed_order_instructions = 0
    ENDIF
    SET growsum = (growsum+ _remmed_order_instructions)
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE med_last_dose(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = med_last_doseabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE med_last_doseabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   ...
   IF (_remlast_dose > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remlast_dose,((size(__last_dose) -
     ...
     SET _remlast_dose = (_remlast_dose+ rptsd->m_drawlength)
    ELSE
     SET _remlast_dose = 0
    ENDIF
    SET growsum = (growsum+ _remlast_dose)
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_y = (offsety+ 0.000)
   SET rptsd->m_x = (offsetx+ 0.500)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE med_next_dose(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = med_next_doseabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE med_next_doseabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE med_assoc_dtas(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = med_assoc_dtasabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE med_assoc_dtasabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE med_addtl_notes(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = med_addtl_notesabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE med_addtl_notesabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE refused_medications(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = refused_medicationsabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE refused_medicationsabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.720000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 20
    SET rptsd->m_borders = rpt_sdnoborders
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE refused_med_notes(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = refused_med_notesabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE refused_med_notesabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE prn_angio_head(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = prn_angio_headabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE prn_angio_headabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 20
    SET rptsd->m_borders = rpt_sdnoborders
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE prn_angio_notes(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = prn_angio_notesabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE prn_angio_notesabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
 ...
 SUBROUTINE page_footabs(ncalc,offsetx,offsety)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE med_blank_details_sch(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = med_blank_details_schabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE med_blank_details_schabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE med_blank_details_prn(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = med_blank_details_prnabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE med_blank_details_prnabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE med_blank_details_iv(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = med_blank_details_ivabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE med_blank_details_ivabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.190000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_RPT_SURG_DOWNTIME_MAR"
 ...
 SUBROUTINE sub_foot_report(zero4)
   ...
   IF (test_ind=0)
    ...
    SET spool value(work->encntrs[e].filename) value(ms_varoutput) WITH notify, deleted
 ...
 ENDFOR
 IF (test_ind=1)
  SET d0 = finalizereport(value(ms_varoutput))
 ENDIF
#exit_script
 CALL echorecord(work)
 IF (test_ind=1)
