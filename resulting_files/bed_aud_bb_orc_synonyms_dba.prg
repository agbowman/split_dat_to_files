CREATE PROGRAM bed_aud_bb_orc_synonyms:dba
 ...
 DECLARE bb = f8 WITH public, noconstant(0.0)
 SELECT INTO "nl:"
  FROM code_value cv
  PLAN (cv
 ...
 SELECT INTO "nl:"
  ...
  PLAN (oc
   ...
   JOIN (cv5
   WHERE cv5.code_value=outerjoin(sd.bb_processing_cd)
    AND cv5.active_ind=outerjoin(1))
  ORDER BY cnvtupper(oc.description), cv3.cdf_meaning DESC
  HEAD oc.description
   ...
   temp->oqual[ocnt].clin_category = cv4.display, temp->oqual[ocnt].proc_type = cv5.display
 ...
 SET reply->collist[7].header_text = "Hide"
 SET reply->collist[7].data_type = 1
 SET reply->collist[7].hide_ind = 0
 SET reply->collist[8].header_text = "Bill Only"
 SET reply->collist[8].data_type = 1
 SET reply->collist[8].hide_ind = 0
 SET reply->collist[9].header_text = "Department Only"
 SET reply->collist[9].data_type = 1
 SET reply->collist[9].hide_ind = 0
 SET reply->collist[10].header_text = "Careset"
 SET reply->collist[10].data_type = 1
 SET reply->collist[10].hide_ind = 0
 SET reply->collist[11].header_text = "Order Entry Format"
 ...
 SET reply->collist[13].header_text = "Procedure Type"
 ...
 FOR (z = 1 TO temp->ocnt)
   FOR (w = 1 TO temp->oqual[z].syncnt)
     ...
     SET reply->rowlist[row_nbr].celllist[7].string_value = temp->oqual[z].synqual[w].hide
     SET reply->rowlist[row_nbr].celllist[8].string_value = temp->oqual[z].bill_only
     SET reply->rowlist[row_nbr].celllist[9].string_value = temp->oqual[z].dept_only
     SET reply->rowlist[row_nbr].celllist[10].string_value = temp->oqual[z].careset
     SET reply->rowlist[row_nbr].celllist[11].string_value = temp->oqual[z].synqual[w].format
     SET reply->rowlist[row_nbr].celllist[12].string_value = temp->oqual[z].clin_category
     SET reply->rowlist[row_nbr].celllist[13].string_value = temp->oqual[z].proc_type
     SET reply->rowlist[row_nbr].celllist[14].double_value = temp->oqual[z].catalog_cd
   ENDFOR
 ENDFOR
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("bb_orc_synonyms_audit.csv")
 ENDIF
