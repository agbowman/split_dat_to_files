CREATE PROGRAM dm2_mrg_load_dm_info:dba
 ...
 DECLARE dmldi_env_id = f8
 DECLARE dmldi_curqual = i4
 DECLARE dmldi_int = i4
 DECLARE dmldi_seq = f8
 DECLARE dml_upper = f8
 DECLARE dml_lower = f8
 DECLARE num = i4
 ...
 SET dim_filename = "cer_install:dm2_mrg_dm_info.csv"
 SET readme_data->status = "F"
 SET readme_data->message = "Starting Readme..."
 IF (check_logfile("dm2_mrg_dminfo_",".log","dm2_mrg_dminfo LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_mrg_dm_info..."
 CALL disp_msg("",dm_err->logfile,0)
 IF (findfile(dim_filename)=0)
  SET readme_data->status = "F"
  SET readme_data->message = "Cannot find dm2_mrg_dm_info.csv in CER_INSTALL."
  SET dm_err->emsg = "Cannot find dm2_mrg_dm_info.csv in CER_INSTALL."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF (merge_check_cnt > 0)
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (dc
    WHERE (dc.info_name=requestin->list_0[d.seq].info_name)
     AND (dc.info_domain=requestin->list_0[d.seq].info_domain))
   DETAIL
    rs_exist->qual[d.seq].is_there = 1
   WITH nocounter
  ;end select
  SET dm_err->eproc = "Updating dm_info..."
  CALL disp_msg("",dm_err->logfile,0)
  SET readme_data->status = "F"
  SET readme_data->message = "Updating dm_info.."
  UPDATE  FROM dm_info dc,
    (dummyt d  WITH seq = merge_check_cnt)
   SET dc.info_char = requestin->list_0[d.seq].info_char, dc.info_number = cnvtreal(requestin->
     list_0[d.seq].info_number)
   PLAN (d
  ...
  INSERT  FROM dm_info dc,
   ...
   SET dc.info_char = requestin->list_0[d.seq].info_char, dc.info_name = requestin->list_0[d.seq].
    info_name, dc.info_domain = requestin->list_0[d.seq].info_domain,
    dc.info_number = cnvtreal(requestin->list_0[d.seq].info_number)
   PLAN (d
 ...
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading dm_info with info in dm2_mrg_dm_info.csv."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = "Error loading dm_info with info in dm2_mrg_dm_info.csv."
  GO TO exit_script
 ELSE
  SET dm_err->eproc = "Finished update of dm_info"
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE (dc.info_name=requestin->list_0[d.seq].info_name)
    AND (dc.info_domain=requestin->list_0[d.seq].info_domain)
    AND (dc.info_char=requestin->list_0[d.seq].info_char)
    AND dc.info_number=cnvtreal(requestin->list_0[d.seq].info_number))
  DETAIL
   dim_cnt = (dim_cnt+ 1)
  WITH nocounter
 ;end select
 IF (after_cnt=dim_cnt)
  SET readme_data->status = "S"
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = "Check failed. Rows in request != rows in local dm_info."
  GO TO exit_script
 ENDIF
 IF (checkprg("DM_RMC_AUTO_VERIFY_SETUP") > 0)
  SELECT INTO "nl:"
   ...
   WHERE di.info_domain="DATA MANAGEMENT"
    AND di.info_name="DM_ENV_ID"
   DETAIL
    dmldi_env_id = di.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->emsg) > 0)
   SET readme_data->status = "F"
   SET readme_data->script = "Failed to get current environment id from dm_info."
   SET dm_err->emsg = "Readme Failed.  Could not get current environment id."
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dml_lower = locateval(num,1,merge_check_cnt,"Lower Boundary",requestin->list_0[num].info_name)
  SET dml_upper = locateval(num,1,merge_check_cnt,"Upper Boundary",requestin->list_0[num].info_name)
  SET stat = alterlist(auto_ver_request->qual,1)
  SET stat = alterlist(auto_ver_request->qual[1].detail_qual,2)
  SET auto_ver_request->qual[1].rdds_event = "Code Version Requirements"
  SET auto_ver_request->qual[1].cur_environment_id = dmldi_env_id
  SET auto_ver_request->qual[1].paired_environment_id = 0
  SET auto_ver_request->qual[1].event_reason = "Readme 3604"
  SET auto_ver_request->qual[1].detail_qual[1].event_detail1_txt = "Lower Boundary"
  SET auto_ver_request->qual[1].detail_qual[1].event_value = cnvtint(requestin->list_0[dml_lower].
   info_number)
  SET auto_ver_request->qual[1].detail_qual[2].event_detail1_txt = "Upper Boundary"
  SET auto_ver_request->qual[1].detail_qual[2].event_value = cnvtint(requestin->list_0[dml_upper].
   info_number)
  ...
  IF ((auto_ver_reply->status="F"))
   ...
   SET readme_data->status = "F"
   SET readme_data->message =
   "Readme Failed.  Could not insert into rdds event log and event detail tables."
   SET dm_err->emsg = "Readme Failed.  Could not insert into rdds event log and event detail tables."
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ENDIF
 ENDIF
