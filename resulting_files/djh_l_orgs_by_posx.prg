CREATE PROGRAM djh_l_orgs_by_posx
 PROMPT
  "Position Code",
 ...
 SELECT DISTINCT INTO  $OUTDEV
  ...
  HEAD PAGE
   ...
   y_val = ((792 - y_pos) - 61), "{PS/newpath 2 setlinewidth   38 ", y_val,
   " moveto  523 ", y_val, " lineto stroke 38 ",
   y_val, " moveto/}", row + 1,
   ...
   pr_position_disp1,
   CALL print(calcpos(222,(y_pos+ 34))), "UserName",
   CALL print(calcpos(269,(y_pos+ 26))), "phys",
   CALL print(calcpos(270,(y_pos+ 35))),
   "flg",
   CALL print(calcpos(293,(y_pos+ 33))), "|------ Standard Orgs -------|",
   row + 1, "{F/0}{CPI/14}",
  ...
  FOOT  p.person_id
   ...
   name_full_formatted1 = substring(1,35,pr.name_full_formatted), username1 = substring(1,9,pr
    .username),
   CALL print(calcpos(38,(y_pos+ 12))),
   name_full_formatted1,
   CALL print(calcpos(223,(y_pos+ 12))), username1,
   row + 1,
   CALL print(calcpos(275,(y_pos+ 12))), physflg,
   row + 1,
   CALL print(calcpos(295,(y_pos+ 12))), bhs,
   row + 1,
   CALL print(calcpos(313,(y_pos+ 12))), bmc,
   row + 1,
   CALL print(calcpos(330,(y_pos+ 12))), bmcip,
   row + 1,
   CALL print(calcpos(359,(y_pos+ 12))), cctr,
   row + 1,
   CALL print(calcpos(380,(y_pos+ 12))), fmc,
   row + 1,
   CALL print(calcpos(399,(y_pos+ 12))), fmcip,
   row + 1,
   CALL print(calcpos(428,(y_pos+ 12))), mlh,
   row + 1, "{F/0}{CPI/14}",
   CALL print(calcpos(454,(y_pos+ 11))),
   pr.active_status_cd, cntothorgs, lncntr = (lncntr+ 1)
  FOOT PAGE
