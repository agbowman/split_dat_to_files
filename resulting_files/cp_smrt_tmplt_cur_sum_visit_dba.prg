CREATE PROGRAM cp_smrt_tmplt_cur_sum_visit:dba
 ...
 DECLARE getpathwaysameenctrtraildata(null) = null
 DECLARE getpathwaydiffenctrtraildata(null) = null
 ...
 DECLARE option_1_header = vc WITH protect
 DECLARE option_1_body = vc WITH protect
 DECLARE reply_text = vc WITH protect
 DECLARE same_enc_body = vc WITH protect
 DECLARE diff_enc_body = vc WITH protect
 ...
 SUBROUTINE main(null)
   ...
   CALL getpathwaysameenctrtraildata(null)
   CALL getpathwaydiffenctrtraildata(null)
   SET header_text = "{\rtf1\ansi\"
   SET option_1_header = header_text
   SET same_enc_body = replace(same_enc_body,header_text,"",1)
   SET same_enc_body = replace(same_enc_body,"}","",2)
   SET diff_enc_body = replace(diff_enc_body,header_text,"",1)
   SET diff_enc_body = replace(diff_enc_body,"}","",2)
   SET option_1_body = concat(same_enc_body," ",c_return," ",diff_enc_body)
   ...
   CALL log_message(build("Exit main(), Time (in secs) : ",((curtime3 - begin_curtime3)/ 100.0)),
    log_level_debug)
 END ;Subroutine
 SUBROUTINE getpathwaysameenctrtraildata(null)
   CALL log_message(" In getPathwaySameEnctrTrailData() ",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   ...
   SET stat = alterlist(same_enc_request->person,1)
   SET same_enc_request->person[1].person_id = person_id
   SET stat = alterlist(same_enc_request->visit,1)
   SET same_enc_request->visit[1].encntr_id = encounter_id
   EXECUTE cp_smrt_tmplt_current_visit  WITH replace("REQUEST","SAME_ENC_REQUEST"), replace("REPLY",
    "SAME_ENC_REPLY")
   SET same_enc_body = same_enc_reply->text
   CALL log_message(build(" Exit getPathwaySameEnctrTrailData(), Time (in secs) : ",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getpathwaydiffenctrtraildata(null)
   CALL log_message(" In getPathwayDiffEnctrTrailData() ",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE dcnt = i4 WITH noconstant(0), protect
   DECLARE vcparserstring = vc WITH noconstant(""), private
   DECLARE date_string = vc WITH noconstant(""), protect
   RECORD diff_enc_request(
     1 person[*]
       2 person_id = f8
     1 visit[*]
       2 encntr_id = f8
   ...
   SET stat = alterlist(diff_enc_request->person,1)
   SET diff_enc_request->person[1].person_id = person_id
   SET stat = alterlist(diff_enc_request->visit,1)
   SET diff_enc_request->visit[1].encntr_id = encounter_id
   EXECUTE cp_smrt_tmplt_summary_view  WITH replace("REQUEST","DIFF_ENC_REQUEST"), replace("REPLY",
    "DIFF_ENC_REPLY")
   SET diff_enc_body = diff_enc_reply->text
   CALL log_message(build(" Exit getPathwayDiffEnctrTrailData(), Time (in secs) : ",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SET log_program_name = "cp_smrt_tmplt_cur_sum_visit"
