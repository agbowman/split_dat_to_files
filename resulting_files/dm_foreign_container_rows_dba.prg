CREATE PROGRAM dm_foreign_container_rows:dba
 ...
 SET reply->table_name = "FOREIGN_CONTAINER"
 ...
 FOR (v_tok_ndx = 1 TO size(request->tokens,5))
   IF ((request->tokens[v_tok_ndx].token_str="DAYSTOKEEP"))
    SET v_days_to_keep = ceil(cnvtreal(request->tokens[v_tok_ndx].value))
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 1)
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(fc.foreign_container_id)
   FROM foreign_container fc
   WHERE fc.foreign_container_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(fc.foreign_container_id)
  FROM foreign_container fc
  DETAIL
  ...
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    ...
    WHERE parser(sbr_getrowidnotexists(
      "fc.created_dt_tm < cnvtdatetime(curdate - v_days_to_keep, curtime3)","fc"))
     AND fc.foreign_container_id != 0
    DETAIL
     v_rows = (v_rows+ 1)
     IF (size(reply->rows,5) < v_rows)
      stat = alterlist(reply->rows,(v_rows+ 49))
     ENDIF
     reply->rows[v_rows].row_id = fc.rowid
    FOOT REPORT
     stat = alterlist(reply->rows,v_rows)
    WITH nocounter, maxqual(fc,value(request->max_rows))
   ;end select
