CREATE PROGRAM cls_physstats_test2:dba
 PROMPT
 ...
 SELECT INTO value(output_dest)
  ...
  HEAD REPORT
   ...
   IF (( $PROMPT6="1"))
    ...
    ENDIF
   ELSE
    output_string = concat(',"Month End","DocName","Specialty","Facility_1","Facility_2","MD Ord"',
     ',"Prot Ord","PhVb Ord","SID Ord","Writ Ord","Other Ord","Total Ord"',',"CPOE Ord"'), col 1,
    output_string,
    row + 1
   ENDIF
  ...
  DETAIL
   ...
   ELSE
    ...
    "FMC INPT PSYCH",
    "BRAT RETRE"))
     loc_facility_group = "FMC "
    ELSEIF (loc_facility_display="MLH")
     loc_facility_group = "MLH "
    ...
    cpoe_count = (phys->list[d.seq].mdcount+ phys->list[d.seq].wrcount), output_string = build(',"',
     end_date_disp,'","',phys->list[d.seq].person_name,'","',
