CREATE PROGRAM dm2_readme_estimator:dba
 ...
 SUBROUTINE drr_chk_active_runners(dcar_dblink,dcar_count_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_rdm_est",".log","dm2_readme_estimator")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting DM2_README_ESTIMATOR"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dre_build_report(null) = i2
 DECLARE dre_exit(de_message=vc) = i2
 DECLARE dre_ocd = i4 WITH protect, noconstant( $1)
 DECLARE dre_env_id_hold = f8 WITH protect, noconstant(0.0)
 DECLARE dre_est_file = vc WITH protect, noconstant("")
 IF ((dm2_install_schema->logfile_prefix != "NONE"))
  SET dre_est_file = build(dm2_install_schema->logfile_prefix,"rdmest.rpt")
 ELSE
  IF (get_unique_file("dm2_rdm_est_",".rpt")=0)
   GO TO exit_program
  ELSE
   SET dre_est_file = concat(dm_err->unique_fname)
  ENDIF
 ENDIF
 IF ((dm2_rr_misc->env_id=0.0))
  IF (dm2_get_env_data(1,dre_env_id_hold)=0)
   GO TO exit_program
  ENDIF
  SET dm2_rr_misc->env_id = dre_env_id_hold
 ENDIF
 IF (dre_build_report(null)=0)
  GO TO exit_program
 ENDIF
 GO TO exit_program
 SUBROUTINE dre_exit(de_message)
   SET dm_err->eproc = "Displaying exit message"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(dre_est_file)
    FROM dummyt d
    DETAIL
     "Readme Estimate Report", row + 1, "================",
     row + 1, de_message, row + 1
    WITH nocounter
   ;end select
   IF ((dir_ui_misc->dm_process_event_id > 0))
    IF (drr_assign_file_to_installs("RPT:README_ESTIMATES",dre_est_file,dir_ui_misc->
     dm_process_event_id)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (dir_silmode_requested_ind=1)
    CALL dir_add_silmode_entry("Readme Estimate Report",dre_est_file)
    SET dm_err->eproc = concat("Silent Mode:  Readme Estimate Report was bypassed.  Report exits",
     " in CCLUSERDIR under file name: ",dre_est_file)
    CALL disp_msg("",dm_err->logfile,0)
   ELSE
    IF (dm2_disp_file(dre_est_file,"Readme Estimate Report")=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dre_build_report(null)
   DECLARE dbr_idx = i4 WITH protect, noconstant(0)
   DECLARE dbr_rows = f8 WITH protect, noconstant(0.0)
   DECLARE dbr_seconds = f8 WITH protect, noconstant(0.0)
   DECLARE dbr_minutes = f8 WITH protect, noconstant(0.0)
   DECLARE dbr_driver = f8 WITH protect, noconstant(0.0)
   DECLARE dbr_format = vc WITH protect, noconstant("DD.HH:MM:SS;3;z")
   DECLARE dbr_title_text = vc WITH protect, noconstant(" ")
   DECLARE dbr_spcchk_flag = i2 WITH protect, noconstant(0)
   DECLARE drtr_idx = i4 WITH public, noconstant(0)
   ...
   DECLARE drtr_exists = i2 WITH public, noconstant(0)
   SET dm2_rr_misc->package_number = dre_ocd
   ...
   IF (dre_ocd)
    ...
    IF ((drr_readmes_to_run->readme_cnt > 0)
     ...
     AND currdb="ORACLE")
     IF (drr_load_space_chk_readmes("ALL",dbr_spcchk_flag)=0)
      RETURN(0)
     ENDIF
    ENDIF
   ...
   ELSE
    ...
    ENDIF
   ENDIF
   IF ( NOT (drr_readmes_to_run->readme_cnt))
    IF (dre_exit("No readmes found.")=0)
     RETURN(0)
    ELSE
     RETURN(1)
    ENDIF
   ENDIF
   FOR (drtr_idx = 1 TO drr_readmes_to_run->timer_readme_cnt)
     SET drtr_timing_script = value(drr_readmes_to_run->timer_readme[drtr_idx].script)
     IF (checkprg(drtr_timing_script) > 0)
      SET drtr_exists_ind = 1
     ELSE
      CALL echo(build("Timing script not found: ",drtr_timing_script))
      SET drtr_exists_ind = 0
     ENDIF
     IF (drtr_exists_ind=1
      AND (drr_readmes_to_run->timer_readme[drtr_idx].skip=0))
      EXECUTE value(drtr_timing_script)
     ENDIF
   ENDFOR
   EXECUTE dm2_dbimport "cer_install:dac_rdm_runtime_stats.csv", "dm2_readme_estimator_load", 1000
   IF ((dm_err->err_ind != 0))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   FOR (dbr_idx = 1 TO drr_readmes_to_run->readme_cnt)
    CASE (drr_readmes_to_run->readme[dbr_idx].execution)
     OF dm2_rr_pre_schema_up:
      SET drr_readmes_to_run->readme[dbr_idx].category = "01"
      SET drr_readmes_to_run->readme[dbr_idx].execution_order = "01"
     OF dm2_rr_post_schema_up:
      SET drr_readmes_to_run->readme[dbr_idx].category = "02"
      SET drr_readmes_to_run->readme[dbr_idx].execution_order = "02"
     OF dm2_rr_post_schema_up2:
      SET drr_readmes_to_run->readme[dbr_idx].category = "10"
      SET drr_readmes_to_run->readme[dbr_idx].execution_order = "03"
     OF dm2_rr_pre_cycle:
      SET drr_readmes_to_run->readme[dbr_idx].category = "09"
      SET drr_readmes_to_run->readme[dbr_idx].execution_order = "04"
     OF dm2_rr_pre_schema_down:
      SET drr_readmes_to_run->readme[dbr_idx].category = "03"
      SET drr_readmes_to_run->readme[dbr_idx].execution_order = "05"
     OF dm2_rr_post_schema_down:
      SET drr_readmes_to_run->readme[dbr_idx].category = "04"
      SET drr_readmes_to_run->readme[dbr_idx].execution_order = "06"
     OF dm2_rr_uptime:
      SET drr_readmes_to_run->readme[dbr_idx].category = "05"
      SET drr_readmes_to_run->readme[dbr_idx].execution_order = "07"
     ELSE
      SET drr_readmes_to_run->readme[dbr_idx].category = "99"
      SET drr_readmes_to_run->readme[dbr_idx].execution_order = "99"
    ENDCASE
    IF ((drr_readmes_to_run->readme[dbr_idx].estimated_time > 0))
     SET dbr_minutes = drr_readmes_to_run->readme[dbr_idx].estimated_time
     SET drr_readmes_to_run->readme[dbr_idx].execution_time = (dbr_minutes/ (24.0 * 60.0))
    ELSE
     SET dbr_seconds = drr_readmes_to_run->readme[dbr_idx].execution_time
     IF (size(trim(drr_readmes_to_run->readme[dbr_idx].driver_table,3)))
      IF (dir_row_count(drr_readmes_to_run->readme[dbr_idx].driver_table,dbr_driver)=0)
       RETURN(0)
      ENDIF
      IF (dbr_driver)
       ...
       SELECT INTO "nl:"
        ...
        WHERE l.project_type=dm2_rr_readme
         AND l.project_name=trim(cnvtstring(drr_readmes_to_run->readme[dbr_idx].readme_id),3)
         AND (l.project_instance=drr_readmes_to_run->readme[dbr_idx].instance)
         AND l.status=dm2_rr_done
         AND l.start_dt_tm IS NOT null
         AND l.end_dt_tm IS NOT null
        DETAIL
         IF (l.driver_count)
          dbr_rows = (dbr_rows+ l.driver_count), dbr_seconds = (dbr_seconds+ datetimediff(l.end_dt_tm,
           l.start_dt_tm,5))
         ENDIF
        WITH nocounter
       ;end select
       ...
       ENDIF
       IF (dbr_rows)
        SET dbr_seconds = ((dbr_seconds/ dbr_rows) * dbr_driver)
       ELSE
        SET dbr_seconds = (drr_readmes_to_run->readme[dbr_idx].execution_time * dbr_driver)
       ENDIF
      ENDIF
     ENDIF
     SET drr_readmes_to_run->readme[dbr_idx].execution_time = (dbr_seconds/ ((24.0 * 60.0) * 60.0))
    ENDIF
   ENDFOR
   IF ((dm_err->debug_flag >= 2))
    CALL echorecord(drr_readmes_to_run)
   ENDIF
   SET dm_err->eproc = concat("Write report to ccluserdir:",dre_est_file)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(dre_est_file)
    execution_order = drr_readmes_to_run->readme[d.seq].execution_order
    FROM (dummyt d  WITH seq = value(drr_readmes_to_run->readme_cnt))
    PLAN (d
     WHERE (drr_readmes_to_run->readme[d.seq].skip != 1)
      AND (drr_readmes_to_run->readme[d.seq].execution_order != "99"))
    ORDER BY execution_order, drr_readmes_to_run->readme[d.seq].ocd, drr_readmes_to_run->readme[d.seq
     ].description
    HEAD REPORT
     IF (dre_ocd=0)
      dbr_title_text = "Readme Execution Time Estimation Report for Mass Move."
     ELSEIF (dre_ocd < 0)
      dbr_title_text = concat("Readme Execution Time Estimation Report for Install Plan ",build((
        dre_ocd * - (1))),".")
     ELSE
      dbr_title_text = concat("Readme Execution Time Estimation Report for Single Package ",build(
        dre_ocd),".")
     ENDIF
     CALL print(dbr_title_text), row + 1,
     CALL print("======================================="),
     row + 2,
     CALL print("Report may be found in ccluserdir:"), col 34,
     CALL print(dre_est_file), row + 1,
     CALL print("======================================="),
     row + 2
     IF (dbr_spcchk_flag=1)
      dbr_title_text =
      "IMPORTANT: Readmes found with tablespace check scripts and Oracle resumable functionality.",
      CALL print(dbr_title_text), row + 2,
      dbr_title_text = concat("Readmes marked with an asterisk will perform tablespace checks that",
       " cannot be evaluated during PREVIEW mode."), col 5, dbr_title_text,
      row + 1, dbr_title_text = concat(
       "The tablespace checks will be executed during each package installation mode (i.e. ",
       " BATCHUP, BATCHPOST etc) until the"), col 5,
      dbr_title_text, row + 1, dbr_title_text = concat(
       "readmes marked below complete successfully.  If additional tablespace is required to ",
       "complete the readme successfully,"),
      col 5, dbr_title_text, row + 1,
      dbr_title_text = concat(
       "the installation process will pause and allow the user to satisfy the tablespace needs.  ",
       "This pause can occur even when"), col 5, dbr_title_text,
      row + 1, dbr_title_text = "the SILENT installation mode is used.", col 5,
      dbr_title_text, row + 2, dbr_title_text = concat(
       "Readmes marked with an asterisk will also utilize the Oracle resumable functionality and ",
       "can therefore be placed in a "),
      col 5, dbr_title_text, row + 1,
      dbr_title_text = concat(
       "'suspended' state if tablespace issues are encountered.  To monitor the package install ",
       "process for suspended readmes"), col 5, dbr_title_text,
      row + 1, dbr_title_text = concat(
       "execute DM2_DOMAIN_MAINT -> Monitoring -> Resumable Sessions.  ",
       "Refer to the Installation Tools Overview Guide for"), col 5,
      dbr_title_text, row + 1, dbr_title_text = "more details.",
      col 5, dbr_title_text, row + 2
     ENDIF
     report_total = 0.0, rpt_estimate_total = 0.0, rpt_actual_total = 0.0,
     actual_secs = 0, actual_time = 0.0, dre_readme_found = 0
    HEAD execution_order
     CALL print(concat("Category: ",drr_readmes_to_run->readme[d.seq].execution)), row + 2, col 2,
    ...
    DETAIL
     dre_readme_found = 1
     IF ((drr_readmes_to_run->readme[d.seq].spchk_readme_cnt > 0))
      col 2, "*"
     ENDIF
     CALL center(trim(cnvtstring(drr_readmes_to_run->readme[d.seq].readme_id),3),2,11), col 14,
     CALL print(drr_readmes_to_run->readme[d.seq].description),
     col 74
     IF (drr_readmes_to_run->readme[d.seq].execution_time)
      CALL print(trim(format(drr_readmes_to_run->readme[d.seq].execution_time,dbr_format),3))
     ELSE
      CALL print("* Unknown *")
     ENDIF
     IF (dre_ocd=0)
      col 90
      IF ((drr_readmes_to_run->readme[d.seq].status IN ("SUCCESS", "FAILED")))
       actual_secs = datetimediff(drr_readmes_to_run->readme[d.seq].end_dt_tm,drr_readmes_to_run->
        readme[d.seq].start_dt_tm,5), actual_time = (actual_secs/ ((24.0 * 60.0) * 60.0)),
       CALL print(trim(format(actual_time,dbr_format),3))
      ELSEIF ((drr_readmes_to_run->readme[d.seq].status IN ("NOT RUN", "RUNNING")))
       actual_time = 0.0,
       CALL print("              ")
      ENDIF
      col 105,
      CALL print(trim(drr_readmes_to_run->readme[d.seq].status,3))
     ENDIF
     row + 1
     IF (dre_ocd)
      category_total = (category_total+ drr_readmes_to_run->readme[d.seq].execution_time)
     ELSE
      cat_estimate_total = (cat_estimate_total+ drr_readmes_to_run->readme[d.seq].execution_time),
      cat_actual_total = (cat_actual_total+ actual_time)
     ENDIF
    FOOT  execution_order
     row + 1, col 74,
     CALL print("----------")
     IF (dre_ocd=0)
      col 90,
     ...
     row + 1, col 67
     IF (dre_ocd)
      CALL print(concat("Total: ",trim(format(category_total,dbr_format),3))), report_total = (
      report_total+ category_total)
     ELSE
      CALL print(concat("Total: ",trim(format(cat_estimate_total,dbr_format),3))), col 90,
      CALL print(trim(format(cat_actual_total,dbr_format),3)),
    ...
    FOOT REPORT
     IF (dre_readme_found)
      ...
      row + 1, col 61
      IF (dre_ocd)
       CALL print(concat("Grand Total: ",trim(format(report_total,dbr_format),3)))
      ELSE
       CALL print(concat("Grand Total: ",trim(format(rpt_estimate_total,dbr_format),3))), col 90,
       CALL print(trim(format(rpt_actual_total,dbr_format),3))
      ENDIF
     ELSE
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending DM2_README_ESTIMATOR"
 ENDIF
 CALL final_disp_msg("dm2_readme_estimator")
END GO
