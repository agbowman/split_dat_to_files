CREATE PROGRAM dm_refchg_task_queue_rows:dba
 ...
 SET reply->table_name = "DM_REFCHG_TASK_QUEUE"
 ...
 IF ((request->tokens[1].token_str="DAYSTOKEEP"))
  SET v_days_to_keep = ceil(cnvtreal(request->tokens[1].value))
 ENDIF
 ...
 ENDIF
 SELECT INTO "nl:"
  d.dm_refchg_task_queue_id
  FROM dm_refchg_task_queue d
  WHERE parser(sbr_getrowidnotexists("d.dm_refchg_task_queue_id+0 > 0","d"))
   AND d.task_status="FINISHED"
   AND d.end_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
   AND  NOT ( EXISTS (
  (SELECT
   1
   FROM dm_refchg_task_queue d2
   WHERE d2.task_status != "FINISHED"
    AND d2.process_name=d.process_name)))
  DETAIL
   v_rows = (v_rows+ 1)
   IF (mod(v_rows,100)=1)
