CREATE PROGRAM dm_dcp_shift_assignment_rows:dba
 ...
 SET reply->table_name = "DCP_SHIFT_ASSIGNMENT"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(dsa.assignment_id)
   FROM dcp_shift_assignment dsa
   WHERE dsa.assignment_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    dsa.rowid
    FROM dcp_shift_assignment dsa
    WHERE parser(sbr_getrowidnotexists("dsa.assignment_id between curMinID and curMaxID","dsa"))
     AND dsa.purge_ind=1
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,100)=1)
     ...
     ENDIF
     reply->rows[rows].row_id = dsa.rowid
    WITH nocounter, maxqual(dsa,value(rowsleft))
   ;end select
