CREATE PROGRAM bh_rc_rl_dep_ann_inc:dba
 CALL echo("***** bh_rc_rl_dep_ann_inc.prg - 677918 *****")
 ...
 SUBROUTINE filltemprec(null)
   FOR (lchgcnt = 1 TO size(request->integer_params,5))
     CASE (cnvtupper(trim(request->integer_params[lchgcnt].key_txt,3)))
      OF "DEP_ANNUAL":
       SET tmprec->dep_month = (request->integer_params[lchgcnt].integer_value/ 12)
     ENDCASE
   ENDFOR
 END ;Subroutine
 DECLARE createreply(null) = null
 SUBROUTINE createreply(null)
  CALL addtoreplyintegers("DEP_MONTH",tmprec->dep_month)
