CREATE PROGRAM ct_mp_get_filter:dba
 PROMPT
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "ct_mp_get_filter"
 ...
 RECORD reply(
   1 codes[*]
     ...
     2 description = vc
     2 meaning = vc
   1 quick_prescreen_status_ind = i2
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 DECLARE getfilter(null) = null WITH protect
 SET reply->status_data.status = "F"
 SET reply->quick_prescreen_status_ind = checkdic("CT_PROT_PRESCREEN_JOB_INFO.CURR_EVAL_PAT_CNT","A",
  0)
 IF (( $CODESET=0))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_filter","No Code Value","reply")
  GO TO exit_script
 ENDIF
 CALL getfilter(null)
 SET reply->status_data.status = "S"
 SUBROUTINE getfilter(null)
   CALL log_message("In GetFilter()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE filtercodeset = f8 WITH noconstant( $CODESET)
   SELECT
    FROM code_value cv
    PLAN (cv
     WHERE cv.code_set=filtercodeset
      AND cv.active_ind=1)
    ...
    DETAIL
     codecount += 1
     IF (codecount > size(reply->codes,5))
      stat = alterlist(reply->codes,(codecount+ 9))
     ENDIF
     reply->codes[codecount].code = cv.code_value, reply->codes[codecount].sequence = cv
     .collation_seq, reply->codes[codecount].meaning = cv.cdf_meaning,
     reply->codes[codecount].display = cv.display, reply->codes[codecount].description = cv
     .description, reply->codes[codecount].code_set = cv.code_set
    FOOT REPORT
     stat = alterlist(reply->codes,codecount)
    WITH nocounter
   ;end select
   CALL log_message(build("Exit GetFilter(), Elapsed time in seconds:",datetimediff(cnvtdatetime(
