CREATE PROGRAM bhs_rpt_careset_extract:dba
 PROMPT
 ...
 SELECT INTO "NL:"
  ...
  DETAIL
   IF (((( $RUNTYPE IN ("ALL"))) OR (( $RUNTYPE IN ("UPDATE"))
    AND ((cc.updt_dt_tm >= cnvtdatetime((curdate - lookbackdays),0)) OR (((os.updt_dt_tm >=
   cnvtdatetime((curdate - lookbackdays),0)) OR (((ocs.updt_dt_tm >= cnvtdatetime((curdate -
    lookbackdays),0)) OR (((ocs1.updt_dt_tm >= cnvtdatetime((curdate - lookbackdays),0)) OR (((lt
   .updt_dt_tm >= cnvtdatetime((curdate - lookbackdays),0)) OR (lt2.updt_dt_tm >= cnvtdatetime((
    curdate - lookbackdays),0))) )) )) )) )) )) )
    addrec = 1
   ENDIF
  FOOT  oc.catalog_cd
   IF (addrec=1)
 ...
 SELECT INTO "NL:"
  ...
  PLAN (oc
   ...
   WHERE ofr.synonym_id=ocs.synonym_id
    ...
    AND ofr.updt_dt_tm >= cnvtdatetime((curdate - lookbackdays),0))
  ORDER BY oc.catalog_cd
  HEAD oc.catalog_cd
   idx = 0, indx = 0, idx = locateval(indx,1,count1,ocs.catalog_cd,careset->qual[indx].catalog_cd)
   IF (idx <= 0)
    count1 = (count1+ 1), stat = alterlist(careset->qual,count1), careset->qual[count1].catalog_cd =
    oc.catalog_cd
   ENDIF
  WITH nocounter
 ;end select
 ...
 IF (textlen(trim( $EMAIL,3)) <= 0)
  SELECT INTO value(var_output)
   ...
   ELSEIF (cc.comp_type_cd=orderable) format(greatest(ocs.updt_dt_tm,os.updt_dt_tm,lt2.updt_dt_tm),
     ";;q")
   ELSE "error"
   ENDIF
   FROM order_catalog oc,
   ...
   PLAN (d)
    ...
    WHERE (oc.catalog_cd=careset->qual[d.seq].catalog_cd)
     AND oc.catalog_cd > 0
     AND (careset->qual[d.seq].locations != "MOCK"))
    JOIN (cc
    WHERE cc.catalog_cd=oc.catalog_cd)
    JOIN (ocs
    WHERE ocs.synonym_id=outerjoin(cc.comp_id)
 ...
 IF (cnvtupper( $EMAIL)="OPSJOB")
  ...
  SET ms_file_name_in = concat(var_output,".csv")
  SET ms_file_name_out = "careset.txt"
  ...
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
    msg1 = "careset.txt was SFTPd to -", msg2 = concat("   ",ms_server_name," - ",ms_back_dir), col 0,
    "{PS/792 0 translate 90 rotate/}", y_pos = 1, row + 1,
    "{F/1}{CPI/9}",
    CALL print(calcpos(36,(y_pos+ 0))), msg1,
    row + 2, msg2
 ...
 IF (( $SYNRPT="1"))
  ...
  IF (cnvtupper( $EMAIL)="OPSJOB")
   ...
   SET ms_file_name_out = "synonym.txt"
   SET ms_user_name = "transport"
   SET ms_server_name = "zxslorshdbpr03"
   SET ms_local_dir = "$CCLUSERDIR"
   SET ms_back_dir = "/u01/home/extracts/knowmgmt"
   ...
   SELECT INTO  $OUTDEV
    FROM dummyt
    HEAD REPORT
     msg1 = "synonym.txt was SFTPd to -", msg2 = concat("   ",ms_server_name," - ",ms_back_dir), col
     0,
     "{PS/792 0 translate 90 rotate/}", y_pos = 1, row + 1,
     "{F/1}{CPI/9}",
     CALL print(calcpos(36,(y_pos+ 0))), msg1,
...
#no_data
 ...
 GO TO exit_program
#exit_program
 SET last_mod = "001 03/26/2018 BR048640  added code to not show error when report qualifies no data"
END GO
