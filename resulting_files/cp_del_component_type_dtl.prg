CREATE PROGRAM cp_del_component_type_dtl
 PROMPT
  "Output to File/Printer/MINE:" = "MINE",
  "List of Component Group ID:" = 0.0
  WITH outdev, cgids
 RECORD reply(
   1 componentdetails
     2 component_id = f8
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE checkcomponentgroupsize(null) = i4 WITH protect
 DECLARE lstat = i4 WITH protect, noconstant(0)
 DECLARE dcomponentgroupid = f8 WITH protect, constant( $CGIDS)
 ...
 SET reply->status_data.status = "F"
 CALL checkcomponentgroupsize(null)
 IF (dcomponentgroupid > 0.0)
 ...
 ENDIF
 SET reply->status_data.status = "S"
 SUBROUTINE checkcomponentgroupsize(null)
   CALL log_message("In CheckComponentGroupSize()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE lqual = i4 WITH protect, noconstant(0)
   CALL echo(build("dComponentGroupId : ",dcomponentgroupid))
   SELECT INTO "nl:"
    FROM cp_component_detail ccd
    WHERE ccd.cp_component_detail_id=dcomponentgroupid
    WITH nocounter
   ;end select
   SET lqual = curqual
   CALL echo(build("curqual : ",curqual))
   IF (curqual=1)
    CALL log_message(build("Deleting CP_CONCEPT_GROUP_ID[",dcomponentgroupid,"]..."),log_level_debug)
    DELETE  FROM cp_component_detail ccd
     WHERE ccd.cp_component_detail_id=dcomponentgroupid
     WITH nocounter
    ;end delete
    COMMIT
    SET reply->componentdetails.component_id = dcomponentgroupid
    SET reply->status_data.status = "S"
    SET reply->status_data.subeventstatus.operationname = "PERFORM"
    SET reply->status_data.subeventstatus.operationstatus = "S"
    SET reply->status_data.subeventstatus.targetobjectname = "DELETE"
    SET reply->status_data.subeventstatus.targetobjectvalue = build2(dcomponentgroupid,"is deleted")
   ELSE
    CALL log_message(build("Delete failed because detail ID was not found."),log_level_debug)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus.operationname = "PERFORM"
    SET reply->status_data.subeventstatus.operationstatus = "F"
    SET reply->status_data.subeventstatus.targetobjectname = "DELETE"
    SET reply->status_data.subeventstatus.targetobjectvalue = build2(dcomponentgroupid,
     "was not deleted")
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit CheckComponentGroupSize(), Elapsed time in seconds:",((curtime3 -
