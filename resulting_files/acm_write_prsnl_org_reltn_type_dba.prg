CREATE PROGRAM acm_write_prsnl_org_reltn_type:dba
 ...
 SET reply->prsnl_org_reltn_type_qual_cnt = size(acm_request->prsnl_org_reltn_type_qual,5)
 DECLARE index = i4 WITH protect, noconstant(0)
 DECLARE lbound = i4 WITH protect, noconstant(1)
 DECLARE ubound = i4 WITH protect, noconstant(reply->prsnl_org_reltn_type_qual_cnt)
 SET stat = alterlist(reply->prsnl_org_reltn_type_qual,reply->prsnl_org_reltn_type_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->prsnl_org_reltn_type_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->prsnl_org_reltn_type_qual[index].prsnl_org_reltn_type_id = acm_request->
     prsnl_org_reltn_type_qual[index].prsnl_org_reltn_type_id
     SET reply->prsnl_org_reltn_type_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_prsnl_org_reltn_type
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->act_cnt > 0))
  EXECUTE acm_act_prsnl_org_reltn_type
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_prsnl_org_reltn_type
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_prsnl_org_reltn_type
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
