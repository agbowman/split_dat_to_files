CREATE PROGRAM dts_rdm_cleanup_note_dt_tm_p:dba
 ...
 SET ms_info_domain_nm = "CE_EVENT_NOTE CLEANUP PROCESS"
 SET ms_range_prefix = "CE_EVENT_NOTE_ID RANGE"
 SET mn_num_children = 3
 SELECT INTO "nl:"
  min_val = min(c.ce_event_note_id)
  FROM ce_event_note c
  WHERE c.ce_event_note_id > 0
  DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  max_val = max(c.ce_event_note_id)
  FROM ce_event_note c
  DETAIL
