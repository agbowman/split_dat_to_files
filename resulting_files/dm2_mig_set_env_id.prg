CREATE PROGRAM dm2_mig_set_env_id
 ...
 ENDIF
 DECLARE dmsei_src_envid = f8 WITH protect, noconstant(0.0)
 DECLARE dmsei_src_envname = vc WITH protect, noconstant(" ")
 DECLARE dmsei_tgt_envid = f8 WITH protect, noconstant(0.0)
 DECLARE dmsei_tgt_envname = vc WITH protect, noconstant(" ")
 DECLARE dmsei_env_checks(dec_type=vc,dec_env_id_out=f8(ref),dec_env_name_out=vc(ref)) = i2
 IF (check_logfile("dm2_mig_set_env_id",".log","dm2_mig_set_env_id LOG FILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "BEGIN dm2_mig_set_env_id"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dmr_prompt_retrieve_mig_data(0,1,1,0)=0)
  GO TO exit_program
 ENDIF
 IF (dmsei_env_checks("SOURCE",dmsei_src_envid,dmsei_src_envname)=0)
  GO TO exit_program
 ENDIF
 IF (dmsei_env_checks("TARGET",dmsei_tgt_envid,dmsei_tgt_envname)=0)
  GO TO exit_program
 ENDIF
 IF (dmsei_tgt_envid != dmsei_src_envid)
  SET dm_err->eproc = concat("Set Database Name for Source to ",dmr_mig_data->tgt_db_name,
   " for Source Env_Id",build(dmsei_src_envid))
  CALL disp_msg("",dm_err->logfile,0)
  UPDATE  FROM dm_environment de
   SET de.database_name = dmr_mig_data->tgt_db_name, de.v500_connect_string = concat("v500/",
     dmr_mig_data->tgt_v500_pwd,"@",dmr_mig_data->tgt_v500_cnct_str)
   WHERE de.environment_id=dmsei_src_envid
   WITH nocounter
  ;end update
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
   ...
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = concat("Set ENV_ID for Target to Source values. Source ID:",build(
    dmsei_src_envid)," Source Name:",dmsei_src_envname)
  CALL disp_msg("",dm_err->logfile,0)
  UPDATE  FROM dm_info d
   SET d.info_number = dmsei_src_envid, d.info_char = dmr_mig_data->tgt_db_name
   WHERE d.info_domain="DATA MANAGEMENT"
    AND d.info_name="DM_ENV_ID"
   WITH nocounter
  ;end update
  ...
  ENDIF
  UPDATE  FROM dm_info d
   SET d.info_char = dmsei_src_envname
   WHERE d.info_domain="DATA MANAGEMENT"
  ...
  IF (checkprg("DM_RMC_AUTO_VERIFY_SETUP") > 0)
   ...
   SET auto_ver_request->qual[1].rdds_event = "MIGRATION ENV ID UPDATED"
   SET auto_ver_request->qual[1].cur_environment_id = dmsei_tgt_envid
   SET auto_ver_request->qual[1].paired_environment_id = dmsei_src_envid
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE dmsei_env_checks(dec_type,dec_env_id_out,dec_env_name_out)
   DECLARE dec_env_name = vc WITH protect, noconstant("")
   DECLARE dec_env_id = f8 WITH protect, noconstant(0.0)
   DECLARE dec_env_db = vc WITH protect, noconstant("")
   DECLARE dec_nulldb_ind = i2 WITH protect, noconstant(0)
   DECLARE dec_dbname = vc WITH protect, noconstant("")
   IF (dmr_prompt_connect_data(dec_type,"V500","CO")=0)
    RETURN(0)
   ENDIF
   IF (dec_type="SOURCE")
    SET dec_dbname = cnvtupper(dmr_mig_data->src_db_name)
   ELSE
    SET dec_dbname = cnvtupper(dmr_mig_data->tgt_db_name)
   ENDIF
   SET dm_err->eproc = "Obtain environment information from DM_INFO"
   SELECT INTO "nl:"
    charisnull = nullind(d.info_char)
    FROM dm_info d
    WHERE d.info_domain="DATA MANAGEMENT"
     AND d.info_name IN ("DM_ENV_ID", "DM_ENV_NAME")
    DETAIL
     IF (d.info_name="DM_ENV_NAME")
      dec_env_name = cnvtupper(d.info_char)
     ELSE
      dec_env_id = d.info_number
      IF (charisnull=1)
       dec_nulldb_ind = 1
      ELSE
       dec_env_db = cnvtupper(d.info_char)
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF (dec_type="SOURCE")
    IF (dec_env_db != dec_dbname
     AND dec_nulldb_ind=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat(dec_type," Environment information incorrectly configured.",
      " DB Name: ",cnvtupper(dmr_mig_data->src_db_name)," and Environment DB Name: ",
      dec_env_db," do not match.")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->eproc = "Verify that environment information exists in DM_ENVIRONMENT"
   SELECT INTO "nl:"
    FROM dm_environment d
    WHERE d.environment_id=dec_env_id
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (curqual=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat(dec_type," Environment information incorrectly configured.",
     "Environment_id:",build(dec_env_id)," does not exist in DM_ENVIRONMENT")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dec_env_name_out = dec_env_name
   SET dec_env_id_out = dec_env_id
   RETURN(1)
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "DM2_MIG_SET_ENV_ID completes successfully"
 CALL final_disp_msg("dm2_mig_set_env_id")
END GO
