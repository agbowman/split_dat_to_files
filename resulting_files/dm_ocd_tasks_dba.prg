CREATE PROGRAM dm_ocd_tasks:dba
 ...
 SUBROUTINE drr_chk_active_runners(dcar_dblink,dcar_count_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE task_report_name = vc WITH protect, constant("Tasks Installed Report")
 IF (dir_silmode_requested_ind=0)
  SET message = window
 ENDIF
 ...
 END ;Subroutine
#1000_initialize
 DECLARE dot_envid = f8 WITH protect, noconstant(0.0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="DM_ENV_ID"
  DETAIL
   dot_envid = di.info_number
  WITH nocounter
 ;end select
 IF (((curqual=0) OR (dot_envid=0)) )
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Verify current environment information."
  SET dm_err->emsg = "FATAL ERROR: current environment doesn't exist!"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  IF (dir_silmode_requested_ind=0)
   CALL text(20,1,"FATAL ERROR: current environment doesn't exist!")
  ENDIF
  GO TO 9999_exit_program
 ENDIF
 SET ot_task = "TASK"
 ...
 SET ot_data->low_date = cnvtdatetime("01-JAN-1800")
 SET ot_data->high_date = cnvtdatetime("31-DEC-2100")
 SET ot_data->high_ocd = 999999
 IF (validate(ocd_number,0))
  SET ot_data->low_ocd = ocd_number
  SET ot_data->high_ocd = ocd_number
  SET ot_data->called = 1
  SET ot_data->mode = ocd_tasks_mode
  SET ot_data->filter_style = "O"
  IF (dir_silmode_requested_ind=0)
   SET message = nowindow
  ENDIF
 ELSE
...
#2000_process
 ...
 CALL clear(1,1)
 CALL box(1,1,3,132)
 CALL text(2,3,"O C D   T A S K   L O G   M A N A G E R")
 CALL text(5,3,"Preview tasks on OCD, Report task logs, Cleanup task logs, or Quit (P/R/C/Q)?")
 CALL accept(5,81,"P;CU","R"
  WHERE curaccept IN ("P", "R", "C", "Q"))
 CASE (curaccept)
  OF ot_report:
   SET ot_data->mode = ot_report
  OF ot_clear:
   SET ot_data->mode = ot_clear
  OF ot_preview:
   SET ot_data->mode = ot_preview
   SET ot_data->filter_style = "O"
   GO TO 2050_filter_prompt
  ELSE
   GO TO 9999_exit_program
 ENDCASE
 CALL text(7,3,"All log rows, filter by Date, filter by Mass Move, or filter by OCD (A/D/M/O)?")
 CALL accept(7,82,"P;CU","A"
  WHERE curaccept IN ("A", "D", "O", "M"))
 SET ot_data->filter_style = curaccept
#2050_filter_prompt
 SET ot_data->low_date = cnvtdatetime("01-JAN-1800")
 SET ot_data->high_date = cnvtdatetime("31-DEC-2100")
 SET ot_data->high_ocd = 999999
 CASE (ot_data->filter_style)
  OF "D":
   CALL text(9,3,concat("Enter date range:             to ",format(cnvtdatetime(curdate,curtime3),
      "DD-MMM-YYYY;;D")))
   SET ot_data->low_date = 0.0
   SET ot_data->high_date = 0.0
   WHILE ( NOT (ot_data->low_date))
    CALL accept(9,21,"NNDCCCDNNNN;CU",format(cnvtdatetime((curdate - 30),curtime3),"DD-MMM-YYYY;;D"))
    SET ot_data->low_date = cnvtdatetime(concat(curaccept," 00:00:00.00"))
   ENDWHILE
   WHILE ((ot_data->high_date < ot_data->low_date))
    CALL accept(9,36,"NNDCCCDNNNN;CU",format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY;;D"))
    SET ot_data->high_date = cnvtdatetime(concat(curaccept," 23:59:59.00"))
   ENDWHILE
  OF "O":
   CALL text(9,3,"Enter an OCD number:")
   CALL accept(9,24,"9(6)")
   SET ot_data->low_ocd = curaccept
   SET ot_data->high_ocd = curaccept
 ENDCASE
#2100_perform
 DECLARE dot_txt_file = vc WITH protect
 IF ((ot_data->mode=ot_report))
  CASE (ot_data->filter_style)
   OF "D":
    SET dot_txt_file = build("ocd",format(ot_data->low_date,"mmddyy;;D"),"_task.txt")
   OF "O":
    SET dot_txt_file = build("ocd",ot_data->low_ocd,"_task.txt")
   OF "M":
    SET dot_txt_file = "ocd_mass_task.txt"
   OF "A":
    SET dot_txt_file = "ocd_all_task.txt"
  ENDCASE
 ENDIF
 IF ((ot_data->mode=ot_preview))
  SELECT INTO "nl:"
   z = count(*)
   FROM dm_ocd_task d
   WHERE d.alpha_feature_nbr BETWEEN ot_data->low_ocd AND ot_data->high_ocd
    AND  NOT ( EXISTS (
   (SELECT
    t.task_number
    FROM application_task t
    WHERE t.task_number=d.task_number)))
   DETAIL
    ot_data->rows = z
   WITH nocounter
  ;end select
 ELSEIF ((ot_data->filter_style="M"))
  SELECT INTO "nl:"
   z = count(*)
   FROM ocd_install_log l,
    dm_alpha_features_env d
   WHERE d.environment_id=dot_envid
    AND d.curr_migration_ind=1
    AND l.component_type=ot_task
    AND l.ocd=d.alpha_feature_nbr
   DETAIL
    ot_data->rows = z
   WITH nocounter
  ;end select
 ELSE
  SELECT INTO "nl:"
   z = count(*)
   FROM ocd_install_log l
   WHERE l.component_type=ot_task
    AND l.install_dt_tm BETWEEN cnvtdatetime(ot_data->low_date) AND cnvtdatetime(ot_data->high_date)
    AND l.ocd BETWEEN ot_data->low_ocd AND ot_data->high_ocd
   DETAIL
    ot_data->rows = z
   WITH nocounter
  ;end select
 ...
 IF ( NOT (ot_data->rows))
  IF (ot_data->called)
   IF ((ot_data->mode=ot_preview))
    CALL echo("No new tasks found on this OCD.  OCD task report will be skipped.")
    GO TO 2999_process_exit
   ELSE
    IF ((ot_data->mode=ot_report))
     SELECT INTO value(dot_txt_file)
      d.seq
      FROM (dummyt d  WITH seq = 1)
      DETAIL
       row + 1, "No new tasks were installed by this OCD.  OCD task report will be skipped."
      WITH nocounter
     ;end select
    ENDIF
    CALL echo("No new tasks were installed by this OCD.  OCD task report will be skipped.")
    GO TO 2999_process_exit
   ENDIF
  ELSE
   IF ((ot_data->mode=ot_preview))
    CALL message("No new tasks found on this OCD.  Press <RETURN> to continue...")
    GO TO 2999_process_exit
   ELSE
    IF ((ot_data->mode=ot_report))
     SELECT INTO value(dot_txt_file)
      ...
      FROM (dummyt d  WITH seq = 1)
      DETAIL
       row + 1, "No new tasks were found by this OCD.  OCD task report will be skipped."
      WITH nocounter
     ;end select
    ENDIF
    CALL message("No task logs found.  Press <RETURN> to continue...")
    GO TO 2900_continue
   ENDIF
  ENDIF
 ENDIF
 IF ((ot_data->mode=ot_preview))
  SELECT
   d.alpha_feature_nbr, d.task_number
   FROM dm_ocd_task d
   WHERE d.alpha_feature_nbr BETWEEN ot_data->low_ocd AND ot_data->high_ocd
    AND  NOT ( EXISTS (
   (SELECT
    ...
    FROM application_task t
    WHERE t.task_number=d.task_number)))
   ORDER BY d.alpha_feature_nbr, d.task_number
   HEAD PAGE
    CALL print("OCD Task Preview"), row + 2,
    CALL print("Task                                                                     OCD"),
    row + 1,
    CALL print("----                                                                     ---"), row
     + 1
   DETAIL
    col 0,
    CALL print(concat(trim(cnvtstring(d.task_number),3)," - ",substring(1,60,d.description))), col 73,
    CALL print(trim(cnvtstring(d.alpha_feature_nbr),3)), row + 1
   WITH nocounter, maxcol = 132
  ;end select
 ENDIF
 IF ((ot_data->mode=ot_report))
  IF ((ot_data->filter_style="M"))
   SELECT INTO value(dot_txt_file)
    l.ocd
    FROM ocd_install_log l,
     dm_alpha_features_env d,
    ...
    PLAN (d
     ...
     WHERE l.component_type=ot_task
      AND l.ocd=d.alpha_feature_nbr)
     JOIN (a
     WHERE a.task_number=cnvtint(l.end_state))
    ORDER BY a.task_number
    HEAD PAGE
     CALL print(task_report_name), row + 2,
     CALL print(
    ...
    DETAIL
     col 0,
     CALL print(concat(trim(cnvtstring(a.task_number),3)," - ",substring(1,60,a.description))), col
     73,
     CALL print(format(l.install_dt_tm,"DD-MMM-YYYY HH:mm;;D")), col 93,
     CALL print(trim(cnvtstring(l.ocd),3)),
     row + 1
    WITH nocounter, maxcol = 132
   ;end select
   SELECT INTO value(dot_txt_file)
    ...
    PLAN (l
     ...
     WHERE d.environment_id=dot_envid
      ...
      AND l.ocd=d.alpha_feature_nbr)
     JOIN (at
     WHERE at.task_number=cnvtint(l.end_state))
     JOIN (a
     WHERE a.application_number=at.application_number)
    ORDER BY at.task_number, at.application_number
    HEAD REPORT
     row + 1,
     CALL print("The above task are related to the following applications:"), row + 2
    HEAD PAGE
     row + 1,
     CALL print("Task           Application"), row + 1,
     CALL print("----           -----------"), row + 1
    DETAIL
     col 0,
     CALL print(trim(cnvtstring(at.task_number),3)), col 15,
     CALL print(concat(trim(cnvtstring(a.application_number),3)," ",substring(1,60,a.description))),
     row + 1
    FOOT REPORT
     row + 1, "*** end of report ***"
    WITH nocounter, append, maxcol = 132,
     formfeed = none
   ;end select
  ELSE
   SELECT INTO value(dot_txt_file)
    ...
    PLAN (l
     WHERE l.component_type=ot_task
      ...
      AND l.ocd BETWEEN ot_data->low_ocd AND ot_data->high_ocd)
     JOIN (t
     WHERE t.task_number=cnvtint(l.end_state))
    ORDER BY l.install_dt_tm DESC, l.end_state, l.ocd
    HEAD PAGE
     CALL print(task_report_name), row + 2,
     CALL print(
     "Task                                                                     Installed           Package"
    ...
    DETAIL
     col 0,
     CALL print(concat(trim(cnvtstring(t.task_number),3)," - ",substring(1,60,t.description))), col
     73,
     CALL print(format(l.install_dt_tm,"DD-MMM-YYYY HH:mm;;D")), col 93,
     CALL print(trim(cnvtstring(l.ocd),3)),
     row + 1
   ...
   SELECT INTO value(dot_txt_file)
    ...
    PLAN (l
     ...
     JOIN (a
     WHERE a.application_number=at.application_number)
    ORDER BY l.install_dt_tm DESC, l.end_state
    HEAD REPORT
     row + 1,
     CALL print("The above task are related to the following applications:"), row + 2
    HEAD PAGE
  ...
  ENDIF
  IF ((dir_ui_misc->dm_process_event_id > 0))
   IF (drr_assign_file_to_installs("RPT:TASKS INSTALLED",dot_txt_file,dir_ui_misc->
    dm_process_event_id)=0)
    GO TO 9999_exit_program
   ENDIF
  ENDIF
  IF (dir_silmode_requested_ind=1)
   CALL dir_add_silmode_entry(task_report_name,dot_txt_file)
   SET dm_err->eproc = concat("Silent Mode: ",task_report_name," was bypassed.  Report exits",
    " in CCLUSERDIR under file name: ",dot_txt_file)
   CALL disp_msg("",dm_err->logfile,0)
  ELSE
   IF (dm2_disp_file(dot_txt_file,task_report_name)=0)
    SET x = x
   ENDIF
  ENDIF
 ENDIF
 IF ((ot_data->mode=ot_clear))
  SET ot_data->text = concat(trim(cnvtstring(ot_data->rows),3),
   " task logs will be removed.  Continue (Y/N)?")
  CALL text(24,2,ot_data->text)
  CALL accept(24,(size(ot_data->text)+ 3),"P;CU","Y"
  ...
  IF (curaccept="Y")
   ...
   ;end delete
   COMMIT
   CALL message(concat(trim(cnvtstring(curqual),3),
     " task logs removed.  Press <RETURN> to continue..."))
  ENDIF
 ENDIF
#2900_continue
 IF ( NOT (ot_data->called))
  GO TO 2000_process
 ENDIF
#2999_process_exit
#9999_exit_program
