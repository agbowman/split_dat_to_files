CREATE PROGRAM aps_get_ft_and_oc_alerts:dba
 ...
 IF (cnt > 0)
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (cr
    WHERE (temp->qual[d.seq].case_id=cr.case_id)
     AND cr.case_id != 0.0
     AND  NOT (cr.status_cd IN (verified_cd, cancelled_cd, corrected_cd, signinproc_cd,
    csigninproc_cd)))
   WITH nocounter
  ;end select
  IF (curqual > 0)
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (ft
    WHERE (temp->qual[d.seq].case_id=ft.case_id)
