CREATE PROGRAM dm_pm_patient_event_rows:dba
 CALL echo("*****dm_pm_patient_event_rows.prg - 788890*****")
 ...
 END ;Subroutine
 FREE RECORD retain_events
 RECORD retain_events(
   1 codevalues[*]
     2 patienteventcodevalue = f8
 )
 DECLARE errmsg = vc WITH noconstant(""), protect
 DECLARE errcd = i4 WITH noconstant(0), protect
 DECLARE days_to_keep = i4 WITH noconstant(- (1)), protect
 DECLARE tok_ndx = i4 WITH noconstant(0), protect
 DECLARE batchsize = f8 WITH noconstant(50000.0), protect
 DECLARE maxid = f8 WITH noconstant(0.0), protect
 DECLARE curminid = f8 WITH noconstant(1.0), protect
 DECLARE curmaxid = f8 WITH noconstant(0.0), protect
 DECLARE rowsleft = i4 WITH noconstant(request->max_rows), protect
 DECLARE rows = i4 WITH noconstant(0), protect
 DECLARE retainonpurgecnt = i4 WITH noconstant(0.0), protect
 DECLARE eventindex = i4 WITH noconstant(0), protect
 SET reply->table_name = "PATIENT_EVENT"
 ...
 IF (days_to_keep < 180)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must keep at least 180 day's worth of data. You entered %1 days or did not enter any value.",
   "i",days_to_keep)
  SET reply->status_data.status = "F"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(pe.patient_event_id)
   FROM patient_event pe
   WHERE pe.patient_event_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(pe.patient_event_id)
  FROM patient_event pe
  DETAIL
 ...
 SELECT INTO "nl:"
  ...
  WHERE cve.code_set=4002773
   AND cve.field_value="1"
  DETAIL
   retainonpurgecnt += 1, stat = alterlist(retain_events->codevalues,retainonpurgecnt), retain_events
   ->codevalues[retainonpurgecnt].patienteventcodevalue = cve.code_value
  WITH nocounter
 ;end select
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    pe.rowid
    FROM patient_event pe
    WHERE pe.patient_event_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("pe.updt_dt_tm < cnvtdatetime((curdate - days_to_keep), 0)",
      "pe"))
     AND  NOT (expand(eventindex,1,size(retain_events->codevalues,5),pe.event_type_cd,retain_events->
     codevalues[eventindex].patienteventcodevalue))
     AND  NOT ( EXISTS (
    (SELECT
     peh.patient_event_id
     FROM patient_event_hist peh
     WHERE peh.patient_event_id=pe.patient_event_id)))
    DETAIL
     rows += 1
     IF (mod(rows,500)=1)
      stat = alterlist(reply->rows,(rows+ 499))
     ENDIF
     reply->rows[rows].row_id = pe.rowid
    WITH nocounter, maxqual(pe,value(rowsleft))
   ;end select
