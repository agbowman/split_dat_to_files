CREATE PROGRAM ccps_cs_bill_item_audit:dba
 PROMPT
 ...
 DECLARE zip_file_name = vc WITH constant("bill_item_audit.zip")
 ...
 ELSE
  ...
  ;end select
  IF (((ispromptempty(2)) OR (cursys2="VMS")) )
   SELECT INTO  $OUTDEV
    FROM (dummyt d  WITH seq = 1)
    PLAN (d)
    HEAD REPORT
     "{CPI/9}{FONT/4}", row 0, col 0,
     CALL print(build2("A bill item audit was created.  A csv file is in the ccluserdir directory.")),
     row + 1, row 3,
     col 0,
     CALL print(build2("The csv file is named ",file_name,".")), row + 1,
     row 6, col 0,
     CALL print(build2("Please retrieve these files, then delete them from the ccluserdir directory."
      ))
    WITH nocounter, nullreport, maxcol = 300,
     dio = postscript
   ;end select
  ELSEIF (cursys2 IN ("AIX", "HPX", "LNX"))
   IF (findfile(full_file_path))
    SET email_stat = email_file(email_address,email_from,email_subject,full_file_path,
     zip_full_file_path,
     file_removal)
    CALL echo(build2("***  File ",full_file_path," emailed to ",email_address," with status ",
   ...
   IF (email_stat=1)
    SELECT INTO  $OUTDEV
     ...
     HEAD REPORT
      "{CPI/9}{FONT/4}", row 0, col 0,
      CALL print(build2("A bill item audit was created, zipped, and email to:  ",email_address,".")),
      row + 1, row 3,
      col 0,
      CALL print(build2("The Subject Line of the email is:  ",email_subject)), row + 1,
      row 6, col 0,
      CALL print(build2(
       "The email recipient should check their inbox and junk mail folders for this email.")),
      row + 1, row 9, col 0,
      CALL print(build2("The file should contain ",bill_item_audit->bia_cnt," records."))
     WITH nocounter, nullreport, maxcol = 300,
      dio = postscript
    ;end select
   ELSE
    SELECT INTO  $OUTDEV
     ...
     HEAD REPORT
      ...
      row 9, col 0,
      CALL print(build2(
       "Please retrieve these files, then delete them from the ccluserdir directory."))
     WITH nocounter, nullreport, maxcol = 300,
      dio = postscript
    ;end select
   ENDIF
