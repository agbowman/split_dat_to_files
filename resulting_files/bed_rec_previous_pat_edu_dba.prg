CREATE PROGRAM bed_rec_previous_pat_edu:dba
 ...
 CALL echo(build("hide",hide_previous_ok))
 IF (hide_previous_ok="Y")
  SET hpref = uar_prefcreateinstance(18)
      ...
      FOR (z = 0 TO (valcnt - 1))
       ...
       IF (cnvtint(trim(xvalue))=1)
        ...
        SELECT INTO "nl:"
         ...
         DETAIL
          valid_ind = 1,
          CALL echo(build(cv.display)),
          CALL echo(build(tr.description))
         WITH nocounter
        ;end select
        IF (valid_ind=1)
         SET hide_previous_ok = "N"
