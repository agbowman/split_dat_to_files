CREATE PROGRAM dm_ecmb_rc_timeline:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_RC_TIMELINE"
 ...
 SUBROUTINE (add_to(s_at_from_pk_id=f8,s_at_to_fk_id=f8) =i4)
   ...
   IF ((dm_err->err_ind=1))
    SET failed = insert_error
    SET request->error_message = "Error in executing dm_cmb_ins_cust_row"
    RETURN(0)
   ENDIF
   SET icombinedet += 1
