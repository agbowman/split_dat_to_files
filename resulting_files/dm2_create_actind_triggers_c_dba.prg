CREATE PROGRAM dm2_create_actind_triggers_c:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE act_ind_trig_cmd = vc
 DECLARE act_ind_trig_text = vc
 DECLARE act_ind_trig_name = vc
 DECLARE c_drop_ind = i4
 DECLARE reg_drop_ind = i4
 DECLARE daic_blocking_str = vc WITH protect, noconstant("")
 DECLARE daic_loop_var = i2 WITH protect, noconstant(0)
 DECLARE daic_in_parse_ind = i2 WITH protect, noconstant(1)
 DECLARE daic_time_in_sec = i2 WITH protect, noconstant(0)
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("WORKING ON TRIGGERS FOR TABLE ",tlist->qual[dai_cnt].tname)
 CALL disp_msg(" ",dm_err->logfile,0)
 SET reg_drop_ind = 0
 SET c_drop_ind = 0
 IF ((tlist->qual[dai_cnt].c_exists="Y")
  AND (tlist->qual[dai_cnt].reg_exists="Y"))
  SET act_ind_trig_name = tlist->qual[dai_cnt].reg_name
  IF (currdb="DB2UDB")
   SET reg_drop_ind = 1
  ENDIF
  SET c_drop_ind = 1
 ELSEIF ((tlist->qual[dai_cnt].c_exists="Y"))
  SET act_ind_trig_name = tlist->qual[dai_cnt].c_name
  IF (currdb="DB2UDB")
   SET c_drop_ind = 1
  ENDIF
 ELSEIF ((tlist->qual[dai_cnt].reg_exists="Y"))
  SET act_ind_trig_name = tlist->qual[dai_cnt].reg_name
  IF (currdb="DB2UDB")
   SET reg_drop_ind = 1
  ENDIF
 ...
 ENDIF
 IF (reg_drop_ind=1)
  SET dm_err->eproc = concat("Dropping trigger ",tlist->qual[dai_cnt].reg_name)
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  IF (dm2_push_cmd(concat("RDB drop trigger ",tlist->qual[dai_cnt].reg_name," go"),1)=0)
   ROLLBACK
 ...
 ENDIF
 IF (c_drop_ind=1)
  SET dm_err->eproc = concat("Dropping trigger ",tlist->qual[dai_cnt].c_name)
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  IF (dm2_push_cmd(concat("RDB drop trigger ",tlist->qual[dai_cnt].c_name," go"),1)=0)
   ROLLBACK
   CALL disp_msg(" ",dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = concat("Creating trigger ",act_ind_trig_name," for table ",tlist->qual[dai_cnt].
  tname)
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (currdb="DB2UDB")
  SET act_ind_trig_text = concat("create trigger ",act_ind_trig_name,
   " no cascade before update of active_ind on ",tlist->qual[dai_cnt].tname,
   " referencing new as nt old as ot ",
   " for each row mode DB2SQL "," when (ot.active_ind <> nt.active_ind) ","begin atomic ",
   " set nt.active_status_prsnl_id = nt.updt_id; ",
   " if (nt.active_status_dt_tm is null or nt.active_status_dt_tm = ot.active_status_dt_tm) then ",
   "    set nt.active_status_dt_tm = current timestamp; "," end if; ","end ")
 ELSE
  SET act_ind_trig_text = concat(
   "create or replace procedure dm_get_date(new_date IN date,old_date IN date,current_date OUT date)",
   " as begin","   if (new_date is null or new_date=old_date) then",
   "      select sysdate into current_date from dual;","   else",
   "      current_date:=new_date;","   end if;","  end;")
  SET act_ind_trig_cmd = concat("rdb asis(^",act_ind_trig_text,"^) end go")
  IF (dm2_push_cmd(act_ind_trig_cmd,1)=0)
   ROLLBACK
   CALL disp_msg(" ",dm_err->logfile,1)
   GO TO exit_script
  ELSE
   COMMIT
  ENDIF
  IF (dai_retry_ceiling > 0)
   WHILE (daic_in_parse_ind=1
    AND daic_loop_var < dai_retry_ceiling)
     IF (dir_check_in_parse(currdbuser,tlist->qual[dai_cnt].tname,daic_in_parse_ind,daic_blocking_str
      )=0)
      RETURN(0)
     ENDIF
     SET daic_loop_var = (daic_loop_var+ 1)
     IF (daic_in_parse_ind=1)
      SET dm_err->eproc = concat("Pausing 10 seconds due to hard parse event detected on ",trim(
        currdbuser),".",tlist->qual[dai_cnt].tname,". Attempt ",
       trim(cnvtstring(daic_loop_var)),"/",trim(cnvtstring(dai_retry_ceiling)),".")
      CALL disp_msg(" ",dm_err->logfile,0)
      CALL pause(10)
      IF (daic_loop_var=dai_retry_ceiling)
       SET daic_time_in_sec = (dai_retry_ceiling * 10)
       SET dm_err->err_ind = 1
       SET dm_err->eproc = concat("Validating ",trim(tlist->qual[dai_cnt].tname),
        " table is NOT part of a ","long parse event prior to creation of active indicator trigger.")
       SET dm_err->emsg = concat("Parse event found against ",trim(tlist->qual[dai_cnt].tname),
        " table, cannot continue with trigger creation. Wait time exceeded (",trim(cnvtstring(
          daic_time_in_sec))," seconds). ",
        daic_blocking_str)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
       GO TO exit_script
      ENDIF
   ...
   ENDWHILE
  ENDIF
  SET act_ind_trig_text = concat("create or replace trigger ",act_ind_trig_name,
   " before update of active_ind on ",tlist->qual[dai_cnt].tname," for each row ",
   " when (old.active_ind <> new.active_ind) "," begin ",
   "  :new.active_status_prsnl_id := :NEW.updt_id; ",
   "  dm_get_date(:new.active_status_dt_tm, :old.active_status_dt_tm, :new.active_status_dt_tm);",
   " end; ")
 ENDIF
 SET act_ind_trig_cmd = concat("rdb asis(^",act_ind_trig_text,"^) end go")
 IF (dm2_push_cmd(act_ind_trig_cmd,1)=0)
