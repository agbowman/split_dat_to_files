CREATE PROGRAM dm_rmc_cutover_check:dba
 ...
 CALL check_logfile("DM_RMC_CUTOVER_CHECK",".log","DM_RMC_CUTOVER_CHECK")
 SET dm_err->eproc = "Beginning DM_RMC_CUTOVER_CHECK"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET logfile_name = dm_err->logfile
 IF ((validate(request->source_env_id,- (1))=- (1)))
  SET v_source_env_id =  $1
 ELSE
  SET v_source_env_id = request->source_env_id
 ENDIF
 DECLARE v_where_string = vc
 DECLARE v_r_tab_name = vc
 IF (validate(request->r_table_name,"ABC")="ABC")
  SET v_r_tab_name = cnvtupper(trim( $2,3))
 ELSE
  SET v_r_tab_name = cnvtupper(trim(request->r_table_name,3))
 ENDIF
 IF (v_r_tab_name > " ")
  SET v_where_string = build("ut.table_name = '",v_r_tab_name,"'")
 ELSE
  SET v_where_string = "1 = 1"
 ENDIF
 IF (validate(reply->status_data.status,"Q")="Q")
  RECORD reply(
    1 row_chg_ind = i2
    1 status_data
      2 status = c1
 ...
 FREE RECORD check_request
 RECORD check_request(
   1 stmt[*]
     2 str = vc
 )
 DECLARE v_pc_stmt_cnt = i4
 DECLARE v_pk_col_str = vc
 DECLARE v_pk_match_str = vc
 DECLARE v_tab_name = vc
 SET dm_err->eproc = "Gathering $R tables"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM user_tables ut
  WHERE ut.table_name="*$R"
   AND parser(v_where_string)
  HEAD REPORT
   t_cnt = 0
  DETAIL
   t_cnt = (t_cnt+ 1)
   IF (mod(t_cnt,25)=1)
    stat = alterlist(pre_tab->tab,(t_cnt+ 24))
   ENDIF
   pre_tab->tab[t_cnt].tab_$r = ut.table_name
  FOOT REPORT
   stat = alterlist(pre_tab->tab,t_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FOR (t_ndx = 1 TO size(pre_tab->tab,5))
   SET v_pk_col_str = " "
   SET v_pk_match_str = " "
   SET v_tab_name = " "
   SET v_tab_name = get_reg_tab_name(pre_tab->tab[t_ndx].tab_$r,"")
   SET dm_err->eproc = concat("Gathering PK info for ",v_tab_name)
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "NL:"
    ucc.column_name, utc.data_type
    FROM user_constraints uc,
     user_cons_columns ucc,
     user_tab_columns utc
    WHERE uc.table_name=v_tab_name
     AND uc.constraint_type="P"
     AND uc.constraint_name=ucc.constraint_name
     AND uc.table_name=ucc.table_name
     AND utc.table_name=v_tab_name
     AND ((utc.column_name=ucc.column_name) UNION (
    (SELECT INTO "NL:"
     utc2.data_type, utc2.column_name
     FROM user_tab_columns utc2
     WHERE ((v_tab_name="DCP_FORMS_REF"
      AND utc2.column_name="DCP_FORMS_REF_ID") OR (v_tab_name="DCP_SECTION_REF"
      AND utc2.column_name="DCP_SECTION_REF_ID")) )))
    HEAD REPORT
     where_cnt = 0
    DETAIL
     v_pk_col_str = concat(v_pk_col_str,", r1.",ucc.column_name), v_pk_match_str = concat(
      v_pk_match_str," and r2.",trim(ucc.column_name,3)," = r1.",trim(ucc.column_name,3)), where_cnt
      = (where_cnt+ 1),
     stat = alterlist(pk_stmt->stmt,where_cnt)
     IF (where_cnt=1)
      pk_stmt->stmt[where_cnt].str = concat('" ',trim(ucc.column_name,3),' = ",')
     ELSE
      pk_stmt->stmt[where_cnt].str = concat('," and ',trim(ucc.column_name,3),' = ",')
     ENDIF
     IF (utc.data_type IN ("VARCHAR", "VARCHAR2", "CHAR", "CLOB", "BLOB"))
      pk_stmt->stmt[where_cnt].str = concat(pk_stmt->stmt[where_cnt].str,'build(^"^,r1.',trim(ucc
        .column_name,3),',^"^)')
     ELSEIF (utc.data_type IN ("NUMBER", "FLOAT"))
      pk_stmt->stmt[where_cnt].str = concat(pk_stmt->stmt[where_cnt].str,"build(r1.",trim(ucc
        .column_name,3),")")
     ELSEIF (utc.data_type IN ("DATE"))
      pk_stmt->stmt[where_cnt].str = concat(pk_stmt->stmt[where_cnt].str,
       ^"cnvtdatetime('",concat(format(r1.^,trim(ucc.column_name,3)), where_cnt = (where_cnt+ 1),
      stat = alterlist(pk_stmt->stmt,where_cnt),
      pk_stmt->stmt[where_cnt].str = concat(^, "DD-MMM-YYYY"),' ',format(r1.^,trim(ucc.column_name,3),
       ^, "HH:MM:SS")),"')"^)
     ENDIF
    FOOT REPORT
     v_pk_col_str = substring(3,10000,v_pk_col_str), v_pk_match_str = substring(6,10000,
      v_pk_match_str)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->eproc = concat("Gathering PK/UI info for ",v_tab_name)
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     dcd.column_name
     FROM dm_columns_doc_local dcd,
      user_tab_columns utc
     WHERE dcd.table_name=v_tab_name
     ...
     HEAD REPORT
      where_cnt = 0
     DETAIL
      v_pk_col_str = concat(v_pk_col_str,", r1.",dcd.column_name)
      IF (utc.nullable="Y")
       v_pk_match_str = concat(v_pk_match_str," and (r2.",trim(dcd.column_name,3)," = r1.",trim(dcd
         .column_name,3),
        " or (r2.",trim(dcd.column_name,3)," is null and r1.",trim(dcd.column_name,3)," is null))")
      ELSE
       v_pk_match_str = concat(v_pk_match_str," and r2.",trim(dcd.column_name,3)," = r1.",trim(dcd
         .column_name,3))
      ENDIF
      where_cnt = (where_cnt+ 1), stat = alterlist(pk_stmt->stmt,where_cnt)
      IF (where_cnt=1)
       pk_stmt->stmt[where_cnt].str = concat('" ',trim(dcd.column_name,3),' ",')
      ELSE
       pk_stmt->stmt[where_cnt].str = concat('," and ',trim(dcd.column_name,3),' ",')
      ENDIF
      IF (utc.data_type IN ("VARCHAR", "VARCHAR2", "CHAR", "CLOB", "BLOB"))
       IF (utc.nullable="N")
        pk_stmt->stmt[where_cnt].str = concat(pk_stmt->stmt[where_cnt].str,'build(^= "^,r1.',trim(dcd
          .column_name,3),',^"^)')
       ELSE
        pk_stmt->stmt[where_cnt].str = concat(pk_stmt->stmt[where_cnt].str,"evaluate(nullind(r1.",
         trim(dcd.column_name,3)), where_cnt = (where_cnt+ 1), stat = alterlist(pk_stmt->stmt,
         where_cnt),
        pk_stmt->stmt[where_cnt].str = concat('),1," IS NULL ", build(^ = "^,r1.',trim(dcd
          .column_name,3),',^"^))')
       ENDIF
      ELSEIF (utc.data_type IN ("NUMBER", "FLOAT"))
       IF (utc.nullable="N")
       ...
       ELSE
        pk_stmt->stmt[where_cnt].str = concat(pk_stmt->stmt[where_cnt].str,"evaluate(nullind(r1.",
         trim(dcd.column_name,3)), where_cnt = (where_cnt+ 1), stat = alterlist(pk_stmt->stmt,
         where_cnt),
        pk_stmt->stmt[where_cnt].str = concat('),1," IS NULL",build(" = ", r1.',trim(dcd.column_name,
          3),"))")
       ENDIF
      ELSEIF (utc.data_type IN ("DATE"))
       IF (utc.nullable="N")
        pk_stmt->stmt[where_cnt].str = concat(pk_stmt->stmt[where_cnt].str,
         ^ "= cnvtdatetime('",concat(format(r1.^,trim(dcd.column_name,3)), where_cnt = (where_cnt+ 1),
        stat = alterlist(pk_stmt->stmt,where_cnt),
        pk_stmt->stmt[where_cnt].str = concat(^, "DD-MMM-YYYY"),' ',format(r1.^,trim(dcd.column_name,
          3),^, "HH:MM:SS")),"')"^)
       ELSE
        pk_stmt->stmt[where_cnt].str = concat(pk_stmt->stmt[where_cnt].str,"evaluate(nullind(r1.",
         trim(dcd.column_name,3)), where_cnt = (where_cnt+ 1), stat = alterlist(pk_stmt->stmt,
         where_cnt),
        pk_stmt->stmt[where_cnt].str = concat(
         ^),1," IS NULL ", build(" = cnvtdatetime('",concat(format(r1.^,trim(dcd.column_name,3)),
        where_cnt = (where_cnt+ 1), stat = alterlist(pk_stmt->stmt,where_cnt),
        pk_stmt->stmt[where_cnt].str = concat(^, "DD-MMM-YYYY"),' ',format(r1.^,trim(dcd.column_name,
          3),^, "HH:MM:SS")),"')"))^)
       ENDIF
      ENDIF
     FOOT REPORT
      v_pk_col_str = substring(2,10000,v_pk_col_str), v_pk_match_str = substring(6,10000,
       v_pk_match_str)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
   ENDIF
   SET dm_err->eproc = concat("Checking for duplicates in ",pre_tab->tab[t_ndx].tab_$r)
   CALL disp_msg(" ",dm_err->logfile,0)
   SET stat = alterlist(pk_val->val,0)
   CALL add_stmt('select into "nl:" ',0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt(v_pk_col_str,0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt(concat(" from ",pre_tab->tab[t_ndx].tab_$r),0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt(" r1 where  rdds_delete_ind = 0 ",0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt(" and rdds_source_env_id = v_source_env_id ",0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt(" and rdds_status_flag < 9000 ",0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt(concat(" group by ",v_pk_col_str),0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt(" having count(*) > 1 ",0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt(" head report pk_cnt = 0 ",0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt(" detail pk_cnt = pk_cnt + 1 ",0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt("   stat = alterlist(pk_val->val,pk_cnt) ",0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   CALL add_stmt("   pk_val->val[pk_cnt].str = concat(",0,0,0,v_pc_stmt_cnt,
    pc_stmts)
   FOR (pws_ndx = 1 TO size(pk_stmt->stmt,5))
     CALL add_stmt(pk_stmt->stmt[pws_ndx].str,0,0,0,v_pc_stmt_cnt,
      pc_stmts)
   ENDFOR
   CALL add_stmt(") with nocounter ",0,1,0,v_pc_stmt_cnt,
    pc_stmts)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SET pc_stmts->batch_ind = 0
   SET pc_stmts->source_env_id = v_source_env_id
   SET pc_stmts->move_long_ind = 0
   SET pc_stmts->table_name = v_tab_name
   EXECUTE dm_rmc_run_stmt  WITH replace("REQUEST","PC_STMTS"), replace("RS_STMT","DRCC_RS_STMT"),
   replace("RS_CONTEXTS","DRCC_RS_CONTEXTS"),
   replace("CONTEXTS_TO_CUTOVER","DRCC_CONTEXTS_TO_CUTOVER")
   FOR (pkv_ndx = 1 TO size(pk_val->val,5))
     SET dm_err->eproc = concat("Cleaning up duplicates for ",pre_tab->tab[t_ndx].tab_$r)
     CALL disp_msg(" ",dm_err->logfile,0)
     SET v_pc_stmt_cnt = 0
     SET stat = alterlist(check_request->stmt,10)
     SET check_request->stmt[1].str = concat(" update into ",pre_tab->tab[t_ndx].tab_$r," r1 ")
     SET check_request->stmt[2].str = " set r1.rdds_status_flag = 9001 "
     SET check_request->stmt[3].str = concat(" where ",pk_val->val[pkv_ndx].str)
     SET check_request->stmt[4].str = " and r1.rowid !=  "
     SET check_request->stmt[5].str = concat(" (select max(r2.rowid) from ",pre_tab->tab[t_ndx].
      tab_$r," r2 ")
     SET check_request->stmt[6].str = concat(" where ",v_pk_match_str)
     SET check_request->stmt[7].str = " and r2.rdds_delete_ind = r1.rdds_delete_ind "
     SET check_request->stmt[8].str = " and r2.rdds_source_env_id = r1.rdds_source_env_id "
     SET check_request->stmt[9].str =
     " and r2.rdds_status_flag < 9000 and r1.rdds_status_flag < 9000) "
     SET check_request->stmt[10].str = " with nocounter go"
     EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","CHECK_REQUEST")
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ROLLBACK
      GO TO exit_program
     ELSEIF (curqual > 0)
      SET reply->row_chg_ind = 1
     ENDIF
     COMMIT
   ENDFOR
   SET v_pc_stmt_cnt = 0
