CREATE PROGRAM bed_del_dmart_reports_by_cat:dba
 ...
 SUBROUTINE delreportfilters(null)
   CALL bedlogmessage("delReportFilters","Entering ...")
   RECORD delfilterrequest(
     1 views[*]
       2 br_datamart_category_id = f8
   )
   DECLARE req_counter = i2 WITH protect, noconstant(0)
   SET stat = alterlist(delfilterrequest->views,req_size)
   FOR (req_counter = 1 TO value(req_size))
     SET delfilterrequest->views[req_counter].br_datamart_category_id = request->views[req_counter].
     br_datamart_category_id
   ENDFOR
   EXECUTE bed_del_dmart_filters_by_cat  WITH replace("REQUEST",delfilterrequest)
   CALL bedlogmessage("delReportFilters","Exiting ...")
 END ;Subroutine
 SUBROUTINE delreportlayouts(null)
   CALL bedlogmessage("delReportLayouts","Entering ...")
   DELETE  FROM br_datam_report_layout b,
    ...
    PLAN (d)
     ...
     (SELECT
      b1.br_datamart_report_id
      FROM br_datamart_report b1
      WHERE (b1.br_datamart_category_id=request->views[d.seq].br_datamart_category_id))))
    WITH nocounter
   ;end delete
   CALL bederrorcheck("Error deleting from br_datam_report_layout table")
   CALL bedlogmessage("delReportLayouts","Exiting ...")
 END ;Subroutine
 SUBROUTINE delreporttext(null)
   CALL bedlogmessage("delReportText","Entering ...")
   ...
   CALL bedlogmessage("delReportText","Exiting ...")
 END ;Subroutine
 SUBROUTINE delreportdefaults(null)
   CALL bedlogmessage("delReportDefaults","Entering ...")
   ...
   CALL bedlogmessage("delReportDefaults","Exiting ...")
 END ;Subroutine
 SUBROUTINE delreports(null)
   ...
   DELETE  FROM br_datamart_report b,
    ...
    WITH nocounter
   ;end delete
   CALL bederrorcheck("Error deleting from br_datamart_report table")
