CREATE PROGRAM dm2_setup_table_monitoring:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dstm_ndx = i4 WITH protect, noconstant(0)
 DECLARE dstm_attempt = i4 WITH protect, noconstant(0)
 DECLARE dstm_error_ind = i2 WITH protect, noconstant(0)
 DECLARE dstm_user = vc WITH protect, noconstant(" ")
 DECLARE dstm_err_reset_str = vc WITH protect, noconstant(" ")
 IF (check_logfile("DM2_SETTBLMON",".log","dm2_setup_table_monitoring LOG FILE")=0)
  GO TO exit_script
 ENDIF
 FREE RECORD dstm_cmd
 RECORD dstm_cmd(
   1 cmd_cnt = i4
   1 qual[*]
     2 cmd_str = vc
 ...
 SET dm_err->eproc = "Starting Dm2_Setup_Table_Monitoring"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dstm_user = cnvtupper( $1)
 IF (dstm_user="")
  SET stat = error(dstm_err_reset_str,1)
  SET dstm_user = currdbuser
  SET dm_err->emsg = concat("User entered is blank. Default to :",dstm_user)
  SET dm_err->user_action = "Above errors are ignorable"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 SET dm_err->eproc = "Check if user is valid."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_users d
  WHERE d.username=dstm_user
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->error_ind = 1
  SET dm_err->emsg = concat("User entered is INVALID. Does not exist in DBA_USERS:",dstm_user)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF ((dm2_db_options->table_monitoring != "Y"))
  SET dm_err->eproc = "Table monitoring not in use"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Load tables into structure"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dba_tables u
  WHERE u.owner=dstm_user
   AND u.monitoring="NO"
   AND u.temporary="N"
   AND u.tablespace_name IS NOT null
  DETAIL
   dstm_cmd->cmd_cnt = (dstm_cmd->cmd_cnt+ 1)
   IF (mod(dstm_cmd->cmd_cnt,100)=1)
    stat = alterlist(dstm_cmd->qual,(dstm_cmd->cmd_cnt+ 99))
   ENDIF
   dstm_cmd->qual[dstm_cmd->cmd_cnt].tname = u.table_name, dstm_cmd->qual[dstm_cmd->cmd_cnt].cmd_str
    = concat('RDB ASIS("ALTER TABLE ',dstm_user,".",trim(u.table_name),' MONITORING ") go')
  FOOT REPORT
   stat = alterlist(dstm_cmd->qual,dstm_cmd->cmd_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dstm_cmd)
 ENDIF
 IF ((dstm_cmd->cmd_cnt=0))
  SET dm_err->eproc = "Monitoring currently enabled for all tables"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 FOR (dstm_ndx = 1 TO dstm_cmd->cmd_cnt)
   SET dstm_attempt = 1
   SET dm_err->eproc = concat("Enable table monitoring for ",dstm_cmd->qual[dstm_ndx].tname)
   CALL disp_msg(" ",dm_err->logfile,0)
   WHILE ((dstm_attempt != - (1))
    AND dstm_attempt <= cnvtint(dm2_db_options->table_monitoring_maxretry))
     IF (dm2_push_cmd(dstm_cmd->qual[dstm_ndx].cmd_str,1)=0)
      IF (((findstring("ORA-00054",dm_err->emsg) > 0) OR (findstring("ORA-30006",dm_err->emsg) > 0))
      )
       SET dstm_attempt = (dstm_attempt+ 1)
       CALL pause(5)
      ELSE
       SET dstm_attempt = - (1)
       SET dstm_error_ind = 1
      ENDIF
      SET dm_err->err_ind = 0
     ...
     ENDIF
   ENDWHILE
   IF (dstm_attempt > cnvtint(dm2_db_options->table_monitoring_maxretry))
    SET dstm_error_ind = 1
   ENDIF
 ENDFOR
 GO TO exit_script
#exit_script
 SET dm_err->eproc = "Dm2_setup_table_monitoring COMPLETED."
 CALL final_disp_msg("DM2_SETTBLMON")
END GO
