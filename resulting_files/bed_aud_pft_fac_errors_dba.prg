CREATE PROGRAM bed_aud_pft_fac_errors:dba
 ...
 SET reply->collist[5].header_text = "No Billing Entity Association"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   no_tier_ind = 1,
   temp->org[ocnt].no_billing_entity_ind = 1, temp->org[ocnt].client_ind = 0
  WITH nocounter
 ;end select
 IF (ocnt=0)
  GO TO exit_script
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   temp->org[d.seq].client_ind = 1
  WITH nocounter
 ;end select
 FOR (oidx = 1 TO ocnt)
   IF ((temp->org[oidx].client_ind=0))
    SET temp->org[oidx].no_tier_ind = 0
    SET temp->org[oidx].no_billing_entity_ind = 0
   ENDIF
 ENDFOR
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = ocnt),
   bill_org_payor bop
  PLAN (d
   WHERE (temp->org[d.seq].client_ind=1))
   JOIN (bop
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   WHERE (temp->org[d.seq].client_ind=1))
   JOIN (bor
   WHERE (bor.organization_id=temp->org[d.seq].id)
    AND bor.active_ind=1
    AND bor.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime)
    AND bor.end_effective_dt_tm >= cnvtdatetime(curdate,curtime))
   JOIN (be
   WHERE be.billing_entity_id=bor.billing_entity_id
    AND be.active_ind=1
    AND be.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime)
    AND be.end_effective_dt_tm >= cnvtdatetime(curdate,curtime))
  DETAIL
   temp->org[d.seq].no_billing_entity_ind = 0
  WITH nocounter
 ;end select
 SET missing_bus_addr_tot = 0
 SET no_tier_tot = 0
 SET no_billing_entity_tot = 0
 SET rrow = 0
 FOR (x = 1 TO ocnt)
   IF ((((temp->org[x].missing_bus_addr_ind=1)) OR ((((temp->org[x].no_tier_ind=1)) OR ((temp->org[x]
   .no_billing_entity_ind=1))) )) )
    SET rrow = (rrow+ 1)
    ...
    ENDIF
    IF ((temp->org[x].no_billing_entity_ind=1))
     SET reply->rowlist[rrow].celllist[5].string_value = "X"
     SET no_billing_entity_tot = (no_billing_entity_tot+ 1)
    ELSE
 ...
 IF (missing_bus_addr_tot=0
  AND no_tier_tot=0
  AND no_billing_entity_tot=0)
  SET reply->run_status_flag = 1
 ELSE
  SET reply->run_status_flag = 3
 ENDIF
 SET stat = alterlist(reply->statlist,3)
 SET reply->statlist[1].statistic_meaning = "PFTFACNOBUSADDR"
 ...
 ENDIF
 SET reply->statlist[2].statistic_meaning = "PFTFACNOTIER"
 ...
 ENDIF
 SET reply->statlist[3].statistic_meaning = "PFTFACNOBILLENTITY"
 SET reply->statlist[3].total_items = ocnt
 SET reply->statlist[3].qualifying_items = no_billing_entity_tot
 IF (no_billing_entity_tot=0)
...
#exit_script
 ...
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("pft_fac_errors_audit.csv")
 ENDIF
