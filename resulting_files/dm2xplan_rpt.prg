CREATE PROGRAM dm2xplan_rpt
 ...
 SUBROUTINE dcr_get_instance_info(dgii_instance_name,dgii_instance_nbr)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2xplanrpt",".log","dm2xplanrpt")=0)
  GO TO exit_program
 ENDIF
 ...
 DECLARE dcpar_entered_detail_ind = i2 WITH protect, noconstant(0)
 DECLARE dcp_xplan(p1=i2,p2=vc,p3=i4,p4=vc,p5=i2) = null WITH sql = "DM2CBO.XPLAN"
 DECLARE dcp_xplan_match(p1=vc,p2=i2,p3=vc,p4=vc,p5=i2,
  p6=vc) = null WITH sql = "DM2CBO.XPLAN_MATCH"
 IF (dcpar_search="S")
 ...
 CASE (dcpar_sort_order)
  OF "0":
   SET dcpar_sortby = "sql_id"
  OF "1":
   SET dcpar_sortby = "executions"
  OF "2":
   SET dcpar_sortby = "elapsed_time"
  OF "3":
   SET dcpar_sortby = "buffer_gets"
  OF "4":
   SET dcpar_sortby = "disk_reads"
  OF "5":
   SET dcpar_sortby = "cpu_time"
  OF "2R":
   SET dcpar_sortby = "ceil(elapsed_time/decode(executions,0,1,executions))"
  OF "3R":
   SET dcpar_sortby = "ceil(buffer_gets/decode(executions,0,1,executions))"
  OF "4R":
   SET dcpar_sortby = "ceil(disk_reads/decode(executions,0,1,executions))"
  OF "5R":
   SET dcpar_sortby = "ceil(cpu_time/decode(executions,0,1,executions))"
 ENDCASE
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2xplan"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT
  IF (dcpar_search="S")INTO value(dcpar_fname)
   FROM (
    (
    (SELECT
     column_value
     FROM (dcp_xplan(null,dcpar_sqlid,evaluate(dcpar_child,"ALL",null,cnvtint(dcpar_child)),
       dcpar_mode,1))
     WITH sqltype("C300")))
    p)
  ELSE INTO value(dcpar_fname)
   FROM (
    (
    (SELECT
     column_value
     FROM (dcp_xplan_match(dcpar_search_text,1,dcpar_mode,dcpar_sortby,cnvtint(dcpar_qnum),
       dcpar_child))
     WITH sqltype("C300")))
    p)
  ENDIF
  HEAD REPORT
   ...
   row + 1
  DETAIL
   CALL print(p.column_value), row + 1, dcpar_entered_detail_ind = 1
  FOOT REPORT
   IF (dcpar_entered_detail_ind=0)
    CALL print("SQL execution information not found for search criteria.")
   ENDIF
  WITH nocounter, format = variable, nullreport,
   maxcol = 500, formfeed = none, maxrow = 1
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "dm2xplanrpt completed"
 CALL final_disp_msg("dm2xplanrpt")
END GO
