CREATE PROGRAM dm2_chk_col_doc:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ccd_logfile_prefix = vc WITH protect, constant("dm2_chk_col_doc")
 DECLARE ccd_pkg_num = i4 WITH protect
 DECLARE ccd_table_size = i4 WITH protect, noconstant(0)
 DECLARE ccd_dyn_where = vc WITH protect, noconstant(" ")
 DECLARE ccd_tbl_idx = i4 WITH protect, noconstant(0)
 DECLARE ccd_col_idx = i4 WITH protect, noconstant(0)
 IF (check_logfile(ccd_logfile_prefix,".log","DM2_CHK_COL_DOC LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning maintenance check on DM_COLUMNS_DOC...."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET ccd_pkg_num =  $1
 ...
 IF (validate(col_doc_maint->qual[1].maint_type,"@")="@")
  ...
  GO TO exit_program
 ENDIF
 SET ccd_table_size = size(col_doc->qual,5)
 SET stat = alterlist(col_doc_maint->qual,ccd_table_size)
 IF ((validate(col_doc->qual[1].absolute_date_ind,- (1))=- (1)))
  SET ccd_dyn_where = "at.table_name = td.table_name"
 ELSE
  IF (cnvtupper(currdb) IN ("DB2UDB", "SQLSRV"))
   SET ccd_dyn_where = "at.table_name = td.suffixed_table_name"
  ELSE
   SET ccd_dyn_where = "at.table_name = td.full_table_name"
  ENDIF
 ENDIF
 IF (dm2_set_autocommit(1)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining maintenance type on DM_COLUMNS_DOC."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET col_doc_maint->num_of_inserts = 0
 SET col_doc_maint->num_of_updates = 0
 SET col_doc_maint->num_of_none = 0
 FOR (ccd_tbl_idx = 1 TO ccd_table_size)
  SET col_doc_maint->qual[ccd_tbl_idx].maint_type = "I"
  SET col_doc_maint->num_of_inserts = (col_doc_maint->num_of_inserts+ 1)
 ENDFOR
 SELECT
  IF (ccd_pkg_num > 0)
   FROM dm_columns_doc cd,
    dm_afd_tables at,
    dm_tables_doc td
   WHERE at.alpha_feature_nbr=ccd_pkg_num
    AND at.owner=currdbuser
    AND parser(ccd_dyn_where)
    AND td.table_name=cd.table_name
  ELSEIF ((dm2_install_schema->process_option="INHOUSE")
   AND currdb="SQLSRV")
   FROM dm_columns_doc cd,
    dm_tables_doc td
   WHERE td.full_table_name=validate(inhouse_reply->table_name,"@")
    AND td.table_name=cd.table_name
  ELSE
   FROM dm_columns_doc cd
  ENDIF
  ...
  HEAD cd.table_name
   ccd_tbl_idx = 0, ccd_tbl_idx = locateval(ccd_tbl_idx,1,ccd_table_size,cd.table_name,col_doc->qual[
    ccd_tbl_idx].table_name)
  DETAIL
   IF (ccd_tbl_idx > 0)
    ccd_col_idx = 0, ccd_col_idx = locateval(ccd_col_idx,ccd_tbl_idx,ccd_table_size,cd.column_name,
     col_doc->qual[ccd_col_idx].column_name)
    IF (ccd_col_idx > 0)
     IF ((col_doc->qual[ccd_col_idx].table_name=col_doc->qual[ccd_tbl_idx].table_name))
      IF ((col_doc->qual[ccd_col_idx].updt_cnt >= cd.updt_cnt))
       IF ((dm2_install_schema->process_option="INHOUSE")
        AND currdb="SQLSRV"
        AND (col_doc->qual[ccd_col_idx].updt_cnt=cd.updt_cnt))
        col_doc_maint->qual[ccd_col_idx].maint_type = "N", col_doc_maint->num_of_none = (
        col_doc_maint->num_of_none+ 1), col_doc_maint->num_of_inserts = (col_doc_maint->
        num_of_inserts - 1)
       ELSE
        col_doc_maint->qual[ccd_col_idx].maint_type = "U", col_doc_maint->num_of_updates = (
        col_doc_maint->num_of_updates+ 1), col_doc_maint->num_of_inserts = (col_doc_maint->
        num_of_inserts - 1)
       ENDIF
      ELSE
       col_doc_maint->qual[ccd_col_idx].maint_type = "N", col_doc_maint->num_of_none = (col_doc_maint
       ->num_of_none+ 1), col_doc_maint->num_of_inserts = (col_doc_maint->num_of_inserts - 1)
      ENDIF
     ENDIF
    ENDIF
   ENDIF
 ...
 ELSEIF (validate(inhouse_reply->table_name,"@")="@"
  ...
  AND curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No Table/Column Informatin Found"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Maintenance to DM_COLUMNS_DOC includes: ",cnvtstring(col_doc_maint->
   num_of_inserts,9,0)," Inserts  ",cnvtstring(col_doc_maint->num_of_updates,9,0)," Updates  ",
  cnvtstring(col_doc_maint->num_of_none,9,0)," w/ no changes.")
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_set_autocommit(0)=0)
  GO TO exit_program
 ENDIF
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Maintenance check on DM_COLUMNS_DOC Complete"
 CALL final_disp_msg(ccd_logfile_prefix)
END GO
