CREATE PROGRAM bed_rec_kdmo_common_ind:dba
 ...
 SELECT INTO "NL:"
  ...
  WHERE cv.code_set=6016
   AND cv.cdf_meaning IN ("HXORDERS", "PRESCRIBE")
   AND cv.active_ind=1
  DETAIL
   IF (cv.cdf_meaning="HXORDERS")
 ...
 IF (((build_powerchart_pass=0
  AND disp_powerchart_pass=0) OR (((build_surginet_pass=0
  AND disp_surginet_pass=0) OR (build_firstnet_pass=0
