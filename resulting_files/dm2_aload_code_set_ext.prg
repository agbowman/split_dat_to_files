CREATE PROGRAM dm2_aload_code_set_ext
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_cse",".log","dm2_aload_code_set_ext")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_code_set_ext"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dacse_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dacse_file = vc WITH protect, noconstant( $2)
 DECLARE dacse_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dacse_batch = i4 WITH protect, noconstant(0)
 DECLARE dacse_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE dacse_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dacse_pkg,dacse_file_loc_ret,dacse_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dacse_batch=0)
  SET dacse_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_AFD_CODE_SET_EXTENSION for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND dacse_rows_returned=10000)
   DELETE  FROM dm_afd_code_set_extension d
    WHERE d.alpha_feature_nbr=dacse_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET dacse_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",dacse_rows_returned))
   ENDIF
 ENDWHILE
 IF (dacse_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dacse_file_loc_ret,dacse_file), "dm2_aload_cs_cse", dacse_batch
  CALL echo(concat(dacse_file_loc_ret,dacse_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_code_set_Ext completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_code_set_Ext")
END GO
