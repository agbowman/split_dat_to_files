CREATE PROGRAM dm_pcmb_pca_person_measure_out:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "PCA_PERSON_MEASURE_OUTCOME"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_PCA_PERSON_MEASURE_OUT"
 ...
 SELECT INTO "nl:"
  frm.*
  FROM pca_person_measure_outcome frm
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = frm.pca_person_measure_outcome_id, rreclist->from_rec[
   ...
   pca_quality_measure_id = frm.pca_quality_measure_id
 ...
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   tu.*
   FROM pca_person_measure_outcome tu
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count2].to_id = tu.pca_person_measure_outcome_id, rreclist->to_rec[
    ...
    pca_quality_measure_id = tu.pca_quality_measure_id
  ...
  IF (v_cust_count2 > 0)
   FOR (v_cust_loopcount1 = 1 TO v_cust_count1)
     FOR (v_cust_loopcount2 = 1 TO v_cust_count2)
       IF ((rreclist->from_rec[v_cust_loopcount1].pca_quality_measure_id=rreclist->to_rec[
       v_cust_loopcount2].pca_quality_measure_id))
        IF (del_from(rreclist->from_rec[v_cust_loopcount1].from_id,rreclist->to_rec[v_cust_loopcount2
         ].person_id,rreclist->from_rec[v_cust_loopcount1].active_ind,rreclist->from_rec[
         v_cust_loopcount1].active_status_cd)=0)
         GO TO exit_sub
        ENDIF
        SET bfound = 1
       ENDIF
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_to_fk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =
  i2)
   UPDATE  FROM pca_person_measure_outcome frm
    ...
    WHERE frm.pca_person_measure_outcome_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "PCA_PERSON_MEASURE_OUTCOME"
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i2)
   UPDATE  FROM pca_person_measure_outcome frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ->updt_applctx,
     frm.updt_task = reqinfo->updt_task, frm.updt_dt_tm = cnvtdatetime(sysdate), frm
     .measure_process_flag = 4,
     frm.person_id = s_uf_to_fk_id
    WHERE frm.pca_person_measure_outcome_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
