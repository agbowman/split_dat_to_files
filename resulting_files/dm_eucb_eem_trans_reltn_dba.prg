CREATE PROGRAM dm_eucb_eem_trans_reltn:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_EUCB_EEM_TRANS_RELTN"
 ...
 SUBROUTINE cust_ucb_upt(dummy)
   UPDATE  FROM eem_trans_reltn
    ...
    WHERE (parent_entity_id=request->xxx_uncombine[ucb_cnt].from_xxx_id)
     ...
     AND parent_entity_name="ENCOUNTER"
    WITH nocounter
   ;end update
   IF (curqual=0)
    SET error_log_message = "The EEM_TRANS_RELTN update was not successful"
    CALL sch_log_message("EEM_TRANS_RELTN_UNCOMBINE","dm_eucb_eem_trans_reltn",error_log_message,2)
   ENDIF
