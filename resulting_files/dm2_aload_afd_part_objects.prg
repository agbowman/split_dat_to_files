CREATE PROGRAM dm2_aload_afd_part_objects
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_afd_part_objects",".log","dm2_aload_afd_part_objects")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_afd_part_objects"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daapo_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daapo_file = vc WITH protect, noconstant( $2)
 DECLARE daapo_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daapo_batch = i4 WITH protect, noconstant(0)
 DECLARE daapo_rows_returned = i4 WITH protect, noconstant(1)
 DECLARE daapo_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daapo_pkg,daapo_file_loc_ret,daapo_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daapo_batch=0)
  SET daapo_batch = 5000
 ENDIF
 IF (daapo_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daapo_file_loc_ret,daapo_file), "dm2_aload_sch_part_objects",
  daapo_batch
  CALL echo(concat(daapo_file_loc_ret,daapo_file))
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Remove extraneous DM_AFD_PART_OBJECTS rows"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dm_afd_part_objects dapo
   WHERE dapo.alpha_feature_nbr=daapo_pkg
    AND dapo.owner=currdbuser
    AND  NOT ( EXISTS (
   (SELECT
    "x"
    FROM dm_afd_tables dat
    WHERE dat.alpha_feature_nbr=dapo.alpha_feature_nbr
     AND dat.owner=dapo.owner
     AND dat.table_name=dapo.table_name
     AND dat.updt_dt_tm=dapo.updt_dt_tm)))
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_afd_part_objects completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_afd_part_objects")
END GO
