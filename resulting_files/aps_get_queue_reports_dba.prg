CREATE PROGRAM aps_get_queue_reports:dba
 ...
 SET nbr_of_ranges = size(request->qual,5)
 SET x = 0
 SET cnt = 0
 SET nbr_of_failed_ranges = 0
...
#start_of_script
 ...
 IF (csigninproc_cd=0.0)
  ...
  GO TO exit_script
 ENDIF
 FOR (x = 1 TO nbr_of_ranges)
  SELECT INTO "nl:"
   pc.case_id, cr.case_id, cr.report_id,
   rt.responsible_pathologist_id, sd.short_description
   FROM pathology_case pc,
    ...
    prsnl p2
   PLAN (pc
    WHERE (request->qual[x].prefix_cd=pc.prefix_id)
     AND (request->qual[x].begin_case_year <= pc.case_year)
     AND (request->qual[x].begin_case_number <= pc.case_number)
     AND (request->qual[x].end_case_year >= pc.case_year)
     AND (request->qual[x].end_case_number >= pc.case_number)
   ...
   ORDER BY pc.accession_nbr, cr.report_id
   HEAD REPORT
   ...
   HEAD cr.report_id
    ...
    IF (isresourceviewable(service_resource_cd)=true)
     ...
     reply->qual[cnt].case_number = pc.case_number, reply->qual[cnt].patient_name = trim(p
      .name_full_formatted), reply->qual[cnt].report_id = cr.report_id,
     reply->qual[cnt].report_sequence = cr.report_sequence, reply->qual[cnt].
     responsible_pathologist_name = trim(p1.name_full_formatted), reply->qual[cnt].
     responsible_resident_name = trim(p2.name_full_formatted),
     reply->qual[cnt].short_description = sd.short_description
    ENDIF
   WITH nocounter
  ;end select
  IF (curqual=0)
   SET nbr_of_failed_ranges += 1
  ENDIF
 ENDFOR
 IF (nbr_of_failed_ranges=nbr_of_ranges)
  CALL subevent_add("SELECT","Z","TABLE","QUEUE REPORTS")
