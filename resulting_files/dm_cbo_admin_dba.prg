CREATE PROGRAM dm_cbo_admin:dba
 ...
 DECLARE dca_threshold = i4 WITH private, constant(3000)
 DECLARE dca_alter_trigger(dat_trigger_name=vc,dat_trigger_status=vc) = null
 DECLARE dca_load_cbo_threshold(threshold=i4) = i2
 DECLARE dca_reports_screen(null) = i2
 DECLARE drs_current_sql(null) = i2
 DECLARE drs_tuned_sql(null) = i2
 DECLARE drs_tuned_status(null) = i2
 ...
 IF (check_logfile("dm_cbo_admin",".log","DM_CBO_ADMIN LOGFILE")=0)
  GO TO exit_script
 ENDIF
 ...
 WHILE (true)
   ...
   CALL text(6,10,"3. Reports")
   CALL text(7,10,"4. Maintenance")
   ...
   CASE (curaccept)
    ...
    OF 1:
     CASE (dca_switch_cbo_prompt(null))
      OF "Y":
       ...
       SET dm_err->err_ind = 0
       IF (dca_load_cbo_threshold(dca_threshold)=0)
        GO TO exit_script
       ENDIF
       IF (dca_alter_trigger("DM2_NOANALYZE","ENABLE")=0)
        SET dm_err->err_ind = 0
       ENDIF
    ...
    OF 2:
     ...
     ENDCASE
    OF 3:
     IF (dca_reports_screen(null)=0)
      GO TO exit_script
     ENDIF
    OF 4:
     IF (dca_maintenance_screen(null)=0)
      GO TO exit_script
     ENDIF
   ENDCASE
 ENDWHILE
 SUBROUTINE dca_load_cbo_threshold(threshold)
   SET dm_err->eproc = "Checking if DM CBO IMPLEMENTER->GETS PER EXECUTE MAX row exists in dm_info."
   SELECT INTO "nl:"
    FROM dm_info dm
    WHERE dm.info_domain="DM CBO IMPLEMENTER"
     AND dm.info_name="GETS PER EXECUTE MAX"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->eproc = "Inserting DM CBO IMPLEMENTER->GETS PER EXECUTE MAX row into dm_info."
    INSERT  FROM dm_info dm
     SET dm.info_domain = "DM CBO IMPLEMENTER", dm.info_name = "GETS PER EXECUTE MAX", dm.info_number
       = value(threshold)
     WITH nocounter
    ;end insert
    IF (check_error(dm_err->eproc)=1)
 ...
 SUBROUTINE dca_reports_screen(null)
  WHILE (true)
    ...
    CALL text(7,10,"4. View Table Statistics")
    CALL text(8,10,"5. SQL Plan Report")
    ...
    CASE (curaccept)
     OF 0:
      RETURN(1)
     OF 1:
      IF (drs_current_sql(null)=0)
       RETURN(0)
      ENDIF
     OF 2:
      IF (drs_tuned_sql(null)=0)
       RETURN(0)
      ENDIF
     OF 3:
      IF (drs_tuned_status(null)=0)
       RETURN(0)
      ENDIF
     OF 4:
      EXECUTE dm2_dbstats_table_report
      IF ((dm_err->err_ind=1))
       GO TO exit_script
      ENDIF
      SET dm_err->err_ind = 0
     OF 5:
      IF (drs_sql_plan(null)=0)
       RETURN(0)
      ENDIF
    ENDCASE
  ENDWHILE
  RETURN(1)
 END ;Subroutine
 SUBROUTINE drs_current_sql(null)
   SET dm_err->eproc = "Selecting currently running SQL from v$sqlarea/v$session"
   SELECT DISTINCT INTO mine
    inst = substring(1,4,cnvtstring(vs.inst_id)), machine = substring(1,7,vs.machine), osuser =
    substring(1,10,vs.osuser),
    vs.process, elapsed = substring(1,8,cnvtstring(vs.last_call_et))";R", hash_value = substring(1,11,
     cnvtstring(va.hash_value))";R",
    va.sql_text
    FROM gv$sql va,
     gv$session vs
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drs_tuned_sql(null)
   DECLARE dts_best_optimimzer_mode = vc WITH protect, noconstant("")
   ...
   CALL text(4,2,"Tuning Status:")
   CALL text(6,2,"Best Optimizer Mode:")
   CALL text(23,2,"Press <Shift><F5> for help list of filter options.")
   SET help = fix("*          ,TUNED      ,IN PROCESS ,UNTUNABLE ")
   CALL accept(4,17,"P(11);CU","*"
    WHERE trim(curaccept) IN ("TUNED", "IN PROCESS", "UNTUNABLE", "*"))
   SET dts_tuning_status = trim(curaccept)
   SET help =
   SELECT DISTINCT INTO "nl:"
    dsp.best_optimizer_mode
    FROM dm_sql_performance dsp
    WHERE ((dsp.tuning_status=patstring(dts_tuning_status)) UNION (
    (SELECT
     "*"
     FROM dual)))
    WITH nocounter
   ;end select
   CALL accept(6,23,"P(40);CU","*")
   SET dts_best_optimimzer_mode = trim(curaccept)
   SET help = off
   SET dm_err->eproc = "Selecting SQL data from dm_sql_performance/dm_sql_perf_text"
   SELECT INTO mine
    FROM dm_sql_performance dsp,
     dm_sql_perf_text dspt
    WHERE dsp.dm_sql_id=dspt.dm_sql_id
     AND dsp.tuning_status=patstring(dts_tuning_status)
     AND dsp.best_optimizer_mode=patstring(dts_best_optimimzer_mode)
    ...
    ORDER BY dsp.last_tuning_dt_tm DESC, dsp.hash_value, dspt.sql_text_seq
    HEAD REPORT
     found = 0,
     CALL print("CBO Reports            SQL Report"), row + 1,
     CALL print(fillstring(131,"-"))
    HEAD dsp.hash_value
     found = 1, row + 2, col 0,
     "Hash Value:", col 30, dsp.hash_value"################;L",
     row + 1, col 0, "Cost Ratio:",
     col 30, dsp.initial_buffer_gets_exec"#############.##;L", row + 1,
     col 0, "Rule Ratio:", col 30,
     dsp.final_buffer_gets_exec"#############.##;L", row + 1, col 0,
     "Tuning Status:", col 30,
     CALL print(trim(dsp.tuning_status,3))
     IF (trim(dsp.tuning_status)="UNTUNABLE")
      row + 1, col 0, "Untunable Reason:",
      col 30,
      CALL print(trim(dsp.comments))
     ELSEIF (trim(dsp.comments) != "")
      row + 1, col 0, "Tuning Reasoning:",
      col 30,
      CALL print(trim(dsp.comments))
     ENDIF
     row + 1, col 0, "Best Optimizer Mode:",
     col 30,
     CALL print(trim(dsp.best_optimizer_mode,3)), row + 1,
     col 0, "Tuned Date/Time:", col 30,
     dsp.last_tuning_dt_tm"MM/DD/YY HH:MM;;Q"
     IF (trim(dsp.outline_name) != "")
      row + 1, col 0, "Outline Name:",
      col 30,
      CALL print(trim(dsp.outline_name,3))
     ENDIF
     row + 1, col 0, "SQL:"
    DETAIL
     col 30,
     CALL print(trim(dspt.sql_text,3)), row + 1
    FOOT REPORT
     IF (found=0)
      row + 1, col 0, "No Data Returned"
     ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drs_tuned_status(null)
   SET dm_err->eproc = "Selecting tuned status data from dm_sql_performance"
   SELECT INTO mine
    FROM dm_sql_performance dsp
    WHERE  NOT (dsp.dm_sql_id IN (
    (SELECT
     di.info_number
     FROM dm_info di
     WHERE di.info_domain="DM_CBO_IMPLEMENTER"
    ...
    (SELECT
     ...
     WHERE di.info_domain="DM_CBO_IMPLEMENTER"
      AND di.info_name="RESET_ALL")))
    ORDER BY dsp.tuning_status, dsp.best_optimizer_mode
    HEAD REPORT
     found = 0,
     CALL print("CBO Reports            Tuned Status by Best Optimizer Mode Summary"), row + 1,
     CALL print(fillstring(131,"-")), row + 2, col 0,
     "Best_Optimizer_Mode", col 25, "Tuning Status",
     col 45, "Count", row + 1,
     col 0, "-------------------", col 25,
     "----------------", col 45, "------"
    HEAD dsp.best_optimizer_mode
     found = 1
    FOOT  dsp.best_optimizer_mode
     row + 1, col 0, dsp.best_optimizer_mode";L",
     col 25, dsp.tuning_status";L", col 45,
     count(dsp.hash_value)";L"
    FOOT REPORT
     IF (found=0)
      row + 1, col 0, "No Data Returned"
     ENDIF
...
#exit_script
 ...
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 SET dm_err->eproc = "Ending DM_CBO_ADMIN."
 CALL final_disp_msg("dm_cbo_admin")
END GO
