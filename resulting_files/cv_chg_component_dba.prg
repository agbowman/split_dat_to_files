CREATE PROGRAM cv_chg_component:dba
 ...
 IF (validate(chg_component_request,0))
 ...
 IF (called_by_script_server=true)
  ...
  SET stat = alterlist(chg_component_request->qual,nbr1)
  SET chg_component_request->call_echo_ind = request->call_echo_ind
  FOR (i = 1 TO nbr1)
    SET chg_component_request->qual[i].updt_cnt = request->qual[i].updt_cnt
    SET chg_component_request->qual[i].component_type_flag = request->qual[i].component_type_flag
    SET chg_component_request->qual[i].component_id = request->qual[i].component_id
    SET chg_component_request->qual[i].long_text_id = request->qual[i].long_text_id
    SET chg_component_request->qual[i].algorithm_id = request->qual[i].algorithm_id
    SET chg_component_request->qual[i].mnemonic = request->qual[i].mnemonic
    SET chg_component_request->qual[i].parent_component_id = request->qual[i].parent_component_id
    SET chg_component_request->qual[i].source_name = request->qual[i].source_name
    SET chg_component_request->qual[i].source_id = request->qual[i].source_id
    SET chg_component_request->qual[i].modifier = request->qual[i].modifier
    SET chg_component_request->qual[i].allow_partial_ind = request->qual[i].allow_partial_ind
    SET chg_component_request->qual[i].version_ind = request->qual[i].version_ind
    SET chg_component_request->qual[i].force_updt_ind = request->qual[i].force_updt_ind
  ENDFOR
  RECORD reply(
    1 qual_cnt = i4
    1 qual[*]
 ...
 SET chg_component_reply->qual_cnt = size(chg_component_request->qual,5)
 SET stat = alterlist(chg_component_reply->qual,chg_component_reply->qual_cnt)
 IF ((chg_component_reply->qual_cnt=0))
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  t.updt_cnt, d.seq
  FROM cv_component t,
   (dummyt d  WITH seq = value(chg_component_reply->qual_cnt))
  PLAN (d)
   JOIN (t
   WHERE (t.component_id=chg_component_request->qual[d.seq].component_id))
  HEAD REPORT
   i_version = 0
  DETAIL
   IF ((((t.updt_cnt=chg_component_request->qual[d.seq].updt_cnt)) OR ((chg_component_request->qual[d
   .seq].force_updt_ind=1))) )
    chg_component_reply->qual[d.seq].status = 1
   ELSE
    chg_component_reply->qual[d.seq].status = update_cnt_error
   ENDIF
  WITH nocounter, forupdate(t)
 ;end select
 SET nbr_correct = 0
 FOR (i = 1 TO chg_component_reply->qual_cnt)
   IF ((chg_component_reply->qual[i].status=1))
    SET nbr_correct = (nbr_correct+ 1)
   ELSE
    IF ((chg_component_request->qual[i].allow_partial_ind != 1))
 ...
 IF (nbr_correct=0)
  GO TO exit_script
 ENDIF
 UPDATE  FROM cv_component t,
   (dummyt d  WITH seq = value(chg_component_reply->qual_cnt))
  SET t.updt_id = reqinfo->updt_id, t.updt_dt_tm = cnvtdatetime(curdate,curtime3), t.updt_task =
   reqinfo->updt_task,
   t.updt_applctx = reqinfo->updt_applctx, t.updt_cnt = (t.updt_cnt+ 1), t.component_type_flag =
   chg_component_request->qual[d.seq].component_type_flag,
   t.long_text_id = chg_component_request->qual[d.seq].long_text_id, t.algorithm_id =
   chg_component_request->qual[d.seq].algorithm_id, t.mnemonic =
   IF ((chg_component_request->qual[d.seq].mnemonic > " ")) trim(chg_component_request->qual[d.seq].
     mnemonic)
   ELSE null
   ENDIF
   ,
   t.mnemonic_key =
   IF ((chg_component_request->qual[d.seq].mnemonic > " ")) trim(cnvtupper(chg_component_request->
      qual[d.seq].mnemonic))
   ELSE null
   ENDIF
   , t.parent_component_id = chg_component_request->qual[d.seq].parent_component_id, t.source_name =
   trim(chg_component_request->qual[d.seq].source_name),
   t.source_id = chg_component_request->qual[d.seq].source_id, t.modifier = chg_component_request->
   qual[d.seq].modifier
  PLAN (d
   WHERE (chg_component_reply->qual[d.seq].status=1))
   JOIN (t
   WHERE (t.component_id=chg_component_request->qual[d.seq].component_id))
  WITH nocounter, status(chg_component_reply->qual[d.seq].status)
 ;end update
 SET nbr_correct = 0
 FOR (i = 1 TO chg_component_reply->qual_cnt)
   IF ((chg_component_reply->qual[i].status=1))
    SET nbr_correct = (nbr_correct+ 1)
   ELSE
    IF ((chg_component_reply->qual[i].status=0))
     SET chg_component_reply->qual[i].status = update_error
     IF ((chg_component_request->qual[i].allow_partial_ind != 1))
...
#exit_script
 IF (called_by_script_server=true)
  SET reply->qual_cnt = chg_component_reply->qual_cnt
  SET stat = alterlist(reply->qual,reply->qual_cnt)
  FOR (i = 1 TO reply->qual_cnt)
    SET reply->qual[i].status = chg_component_reply->qual[i].status
  ENDFOR
 ENDIF
#check_failed
 IF (failed=false)
  CASE (nbr_correct)
   ...
   OF chg_component_reply->qual_cnt:
  ...
  ENDCASE
 ELSE
  CALL echorecord(chg_component_request)
  CALL echorecord(chg_component_reply)
  SET reqinfo->commit_ind = false
  IF (failed != true
   AND failed != false)
