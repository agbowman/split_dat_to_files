CREATE PROGRAM dm_pcmb_person_plan_profile:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "PERSON_PLAN_PROFILE"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_PERSON_PLAN_PROFILE"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = frm.person_plan_profile_id, rreclist->from_rec[
   v_cust_count1].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count1].active_status_cd =
   frm.active_status_cd,
   rreclist->from_rec[v_cust_count1].profile_type_cd = frm.profile_type_cd
  WITH forupdatewait(frm)
 ;end select
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   tu.*
   FROM person_plan_profile tu
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count2].to_id = tu.person_plan_profile_id, rreclist->to_rec[v_cust_count2
    ].active_ind = tu.active_ind, rreclist->to_rec[v_cust_count2].active_status_cd = tu
    .active_status_cd,
    rreclist->to_rec[v_cust_count2].profile_type_cd = tu.profile_type_cd
   WITH forupdatewait(tu)
  ;end select
  FOR (v_cust_loopcount1 = 1 TO v_cust_count1)
   IF ((rreclist->from_rec[v_cust_loopcount1].active_ind=1))
    SET move_profile_ind = 1
    FOR (v_cust_loopcount2 = 1 TO v_cust_count2)
      IF ((rreclist->from_rec[v_cust_loopcount1].profile_type_cd=rreclist->to_rec[v_cust_loopcount2].
      profile_type_cd)
 ...
 SUBROUTINE del_from(s_df_pk_id,s_df_to_fk_id,s_df_prev_act_ind,s_df_prev_act_status)
   UPDATE  FROM person_plan_profile frm
    ...
    WHERE frm.person_plan_profile_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "PERSON_PLAN_PROFILE"
 ...
 SUBROUTINE del_to(s_df_pk_id,s_df_prev_act_ind,s_df_prev_act_status)
   UPDATE  FROM person_plan_profile tu
    SET tu.active_ind = false, tu.active_status_cd = combinedaway, tu.active_status_dt_tm =
     ...
     cnvtdatetime(sysdate)
    WHERE tu.person_plan_profile_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
 ...
 SUBROUTINE upt_from(s_uf_pk_id,s_uf_to_fk_id)
   UPDATE  FROM person_plan_profile frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.person_plan_profile_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
 ...
 SUBROUTINE add_hist(s_at_to_parent_pk_id)
   ...
   SET dm_cmb_cust_cols2->add_col_val[1].col_name = "PERSON_PLAN_PROFILE_HIST_ID"
   ...
   SET dm_cmb_cust_cols2->where_col_val[1].col_name = "PERSON_PLAN_PROFILE_ID"
   SET dm_cmb_cust_cols2->where_col_val[1].col_value = build(s_at_to_parent_pk_id)
   IF (size(dm_cmb_cust_cols2->col,5)=0)
    SET dm_cmb_cust_cols2->tbl_name = "PERSON_PLAN_PROFILE_HIST"
    SET dm_cmb_cust_cols2->sub_select_from_tbl = "PERSON_PLAN_PROFILE"
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0.0)
    ...
    SET dcipht_request->transaction_reason_txt = "PERSON_CMB_PERSON_PLAN_PROFILE"
