CREATE PROGRAM afc_rca_get_modify_chrg_model:dba
 ...
 IF ( NOT (validate(getfeaturetoggledetail)))
  ...
  END ;Subroutine
 ENDIF
 CALL beginservice("CHARGSRV-14215.013")
 IF ( NOT (validate(reply->status_data)))
  RECORD reply(
    1 chargeid = f8
    1 serviceitemid = f8
    1 serviceitemdescription = vc
    1 servicedatetime = dq8
    1 orderingphysicianid = f8
    1 orderingphysicianname = vc
    1 renderingphysicianid = f8
    1 renderingphysicianname = vc
    1 performinglocationcd = f8
    1 quantity = i4
    1 ceaqty = i4
    1 isquantitytaskind = i2
    1 amount = f8
    1 itemprice = f8
    1 iscreditind = i2
    1 hasoffsetind = i2
    1 isadjustchargetaskind = i2
    1 isupdtmiscind = i2
    1 ismodifycptschedule = i2
    1 ismodifycdmschedule = i2
    1 ismodifyicddiagschedule = i2
    1 ismodifyicdprocschedule = i2
    1 ismodifyhcpcsschedule = i2
    1 ismodifyrevenueschedule = i2
    1 ismodifymodifierschedule = i2
    1 ismodifyndcschedule = i2
    1 researchaccountid = f8
    1 abnstatuscd = f8
    1 itemintervalid = f8
    1 intervaltemplatecd = f8
    1 flexfieldsmodel[*]
      2 categorydescription = vc
      2 chargemodifiermodel[*]
        3 chargemodifierid = f8
        3 chargeeventmodid = f8
        3 modifierdescription = vc
        3 modifiercd = f8
        3 requiredind = i2
        3 fielddatatype = vc
        3 fieldvalueoptionlist[*]
          4 optionvalue = vc
          4 optioncd = f8
        3 fieldvaluedatetime = dq8
        3 fieldvaluechar = vc
        3 fieldvaluenumber = f8
        3 fieldvalueid = f8
      2 categorycd = f8
    1 billcodesmodel[*]
      2 billcodetype = vc
      ...
      2 billcodes[*]
        ...
        3 cm1nbr = f8
        3 code1cd = f8
        3 sourcevocabularydisplay = vc
        3 chargemodsourcecode = f8
    1 cdmschedfield1id = f8
    1 cptschedfield1id = f8
    1 ndcschedfield1id = f8
    1 icddiagschedfield1id = f8
    1 icdprocschedfield1id = f8
    1 hcpcsschedfield1id = f8
    1 revenueschedfield1id = f8
    1 modifierschedfield1id = f8
    1 billitemquantity = f8
    1 status_data
      2 status = c1
      2 subeventstatus[1]
        3 operationname = c25
 ...
 DECLARE getbillcodescheduleforuser(dummyvar=i2) = null
 ...
 DECLARE rca_task_upt_misc_number = i4 WITH protect, constant(130129)
 DECLARE rca_task_upt_adjustment_number = i4 WITH protect, constant(130127)
 DECLARE rca_task_upt_adjust_qty_greater = i4 WITH protect, constant(130128)
 DECLARE rca_task_modify_cpt_schedule = i4 WITH protect, constant(130131)
 DECLARE rca_task_modify_cdm_schedule = i4 WITH protect, constant(130135)
 ...
 DECLARE rca_task_modify_hcpcs_schedule = i4 WITH protect, constant(130132)
 DECLARE rca_task_modify_revenue_schedule = i4 WITH protect, constant(130133)
 DECLARE rca_task_modify_modifier_schedule = i4 WITH protect, constant(130138)
 DECLARE rca_task_modify_ndc_schedule = i4 WITH protect, constant(130137)
 DECLARE cscv_task_upt_misc_number = i4 WITH protect, constant(951069)
 DECLARE cscv_task_upt_adjustment_number = i4 WITH protect, constant(951064)
 DECLARE cscv_task_upt_adjust_qty_greater = i4 WITH protect, constant(951077)
 DECLARE cscv_task_upt_cpt4_number = i4 WITH protect, constant(951065)
 DECLARE cscv_task_upt_cdm_number = i4 WITH protect, constant(951066)
 DECLARE cscv_task_upt_icd9_diag_number = i4 WITH protect, constant(951067)
 DECLARE cscv_task_upt_icd9_proc_number = i4 WITH protect, constant(951068)
 DECLARE cscv_task_upt_hcpcs_number = i4 WITH protect, constant(951071)
 DECLARE cscv_task_upt_revenue_number = i4 WITH protect, constant(951072)
 DECLARE cscv_task_upt_modifier_number = i4 WITH protect, constant(951073)
 DECLARE cscv_task_modify_ndc = i4 WITH protect, constant(951108)
 ...
 ENDIF
 IF ( NOT (validate(cs14002_ndc)))
  DECLARE cs14002_ndc = f8 WITH protect, constant(getcodevalue(14002,"NDC",0))
 ENDIF
 ...
 ENDIF
 IF ( NOT (getchargedata(request->chargeid,reply)))
  CALL exitservicefailure("Failed to find charge",true)
 ENDIF
 CALL getintervalchargedata(reply)
 CALL gettaskaccess(0)
 IF ((reply->serviceitemid > 0.0))
  CALL getflexfielddata(reply->serviceitemid,reply->chargeid,reply)
 ENDIF
 CALL getbillcodedata(1)
 CALL exitservicesuccess("")
#exit_script
 IF (validate(debug) > 0)
  CALL echorecord(reply)
 ENDIF
 SUBROUTINE (getchargedata(pchargeid=f8,prchargeinfo=vc(ref)) =i2)
   ...
   SELECT INTO "nl:"
    FROM charge c,
     ...
     pft_charge pc
    PLAN (c
     WHERE c.charge_item_id=pchargeid)
     JOIN (pc
    ...
    HEAD pc.pft_charge_id
     prchargeinfo->chargeid = c.charge_item_id, prchargeinfo->serviceitemid = c.bill_item_id,
     prchargeinfo->serviceitemdescription = c.charge_description,
     prchargeinfo->servicedatetime = c.service_dt_tm, prchargeinfo->orderingphysicianid = c
     .ord_phys_id, prchargeinfo->orderingphysicianname = p.name_full_formatted,
     prchargeinfo->renderingphysicianid = c.verify_phys_id, prchargeinfo->renderingphysicianname = p1
     .name_full_formatted, prchargeinfo->performinglocationcd = c.perf_loc_cd,
     prchargeinfo->quantity = pc.billing_quantity, prchargeinfo->billitemquantity = round(c
      .item_quantity,3), prchargeinfo->amount = pc.billing_amount,
     prchargeinfo->itemprice = c.item_price, prchargeinfo->researchaccountid = c.research_acct_id,
     prchargeinfo->abnstatuscd = c.abn_status_cd,
     prchargeinfo->itemintervalid = c.item_interval_id
     IF (c.offset_charge_item_id > 0)
      prchargeinfo->iscreditind = true
     ENDIF
     IF (pc.billing_quantity=0
      AND abs(pc.billing_amount) < 0.009)
      prchargeinfo->hasoffsetind = true
     ENDIF
    WITH nocounter
   ;end select
   IF (curqual=0)
    CALL logmessage("getChargeData","Failed to find charge",log_error)
   ELSE
    SET statusind = true
   ENDIF
   ...
   RETURN(statusind)
 END ;Subroutine
 SUBROUTINE (getintervalchargedata(prchargeinfo=vc(ref)) =i2)
   DECLARE billitemid = f8 WITH noconstant(0.0)
   DECLARE priceschedid = f8 WITH noconstant(0.0)
   DECLARE servicedatetime = dq8
   SELECT INTO "n1:"
    FROM charge c
    WHERE c.charge_item_id > 0.0
     AND (c.charge_item_id=request->chargeid)
     AND ((c.active_ind+ 0)=true)
    DETAIL
     billitemid = c.bill_item_id, priceschedid = c.price_sched_id, servicedatetime = c.service_dt_tm
    WITH nocounter
   ;end select
   IF (curqual=0)
    CALL logmessage("getIntervalChargeData","Current charge doesn't carry inteerval charge",log_error
     )
   ENDIF
   SELECT INTO "nl:"
    ...
    PLAN (psi
     WHERE psi.bill_item_id=billitemid
      AND psi.price_sched_id=priceschedid
      AND psi.beg_effective_dt_tm <= cnvtdatetime(servicedatetime)
      AND psi.end_effective_dt_tm > cnvtdatetime(servicedatetime)
    ...
    ORDER BY it.beg_value, iit.item_interval_id
    HEAD it.beg_value
     prchargeinfo->intervaltemplatecd = it.interval_template_cd
    WITH nocounter
   ;end select
   SELECT INTO "n1:"
    FROM charge c,
    ...
    WHERE c.charge_item_id > 0.0
     AND (c.charge_item_id=request->chargeid)
     AND ((c.active_ind+ 0)=true)
     AND cea.charge_event_id=c.charge_event_id
     AND ((cea.active_ind+ 0)=true)
     AND cea.quantity != 0
    DETAIL
     prchargeinfo->ceaqty = cea.quantity
    WITH nocounter
   ;end select
 END ;Subroutine
 SUBROUTINE (getflexfielddata(pserviceitemid=f8,pchargeid=f8,prchargeinfo=vc(ref)) =i2)
   ...
   SET afcgetfieldsrequest->serviceitems[1].chargeid = request->chargeid
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     IF (isscsaenabled)
      IF ((afcgetfieldsreply->serviceitems[1].categories[d1.seq].categoryname != i18n_qty_calculation
       ...
       IF ((afcgetfieldsreply->serviceitems[1].categories[d1.seq].categoryname !=
       uar_get_code_display(cs4002353_other))
        AND (((afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].fieldcode=
       cs4002352_service_connected)) OR ((afcgetfieldsreply->serviceitems[1].categories[d1.seq].
       fields[d2.seq].fieldcode=cs4002352_special_authority))) )
        CALL logmessage("getFlexFieldData",build(
         "When the feature toggle is ON SC/SA is not supported for Category : ",afcgetfieldsreply->
         serviceitems[1].categories[d1.seq].categoryname),log_debug)
       ELSE
        IF (icount=1)
         categorycnt += 1, stat = alterlist(prchargeinfo->flexfieldsmodel,categorycnt), prchargeinfo
         ->flexfieldsmodel[categorycnt].categorydescription = afcgetfieldsreply->serviceitems[1].
         categories[d1.seq].categoryname,
         prchargeinfo->flexfieldsmodel[categorycnt].categorycd = afcgetfieldsreply->serviceitems[1].
         categories[d1.seq].categorycd
        ENDIF
        flexfieldcnt += 1, stat = alterlist(prchargeinfo->flexfieldsmodel[categorycnt].
         chargemodifiermodel,flexfieldcnt), prchargeinfo->flexfieldsmodel[categorycnt].
        chargemodifiermodel[flexfieldcnt].modifiercd = afcgetfieldsreply->serviceitems[1].categories[
        d1.seq].fields[d2.seq].fieldcode,
        prchargeinfo->flexfieldsmodel[categorycnt].chargemodifiermodel[flexfieldcnt].requiredind =
        afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].fieldrequiredind,
        prchargeinfo->flexfieldsmodel[categorycnt].chargemodifiermodel[flexfieldcnt].
        modifierdescription = afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].
        fielddisplay, prchargeinfo->flexfieldsmodel[categorycnt].chargemodifiermodel[flexfieldcnt].
        fielddatatype = afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].
        fieldtype
        IF ((afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].fieldtype=
        "INDICATOR"))
         prchargeinfo->flexfieldsmodel[categorycnt].chargemodifiermodel[flexfieldcnt].
         fieldvaluenumber = - (1)
        ENDIF
        IF (size(afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].optionlist,5)
         > 0)
         stat = movereclist(afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].
          optionlist,prchargeinfo->flexfieldsmodel[categorycnt].chargemodifiermodel[flexfieldcnt].
          fieldvalueoptionlist,1,0,size(afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[
           d2.seq].optionlist,5),
          true)
        ENDIF
        icount += 1
     ...
     ELSE
      IF ((afcgetfieldsreply->serviceitems[1].categories[d1.seq].categoryname != i18n_qty_calculation
       ...
       modifierdescription = afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].
       fielddisplay, prchargeinfo->flexfieldsmodel[categorycnt].chargemodifiermodel[flexfieldcnt].
       fielddatatype = afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].fieldtype
       IF ((afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].fieldtype=
       "INDICATOR"))
        prchargeinfo->flexfieldsmodel[categorycnt].chargemodifiermodel[flexfieldcnt].fieldvaluenumber
         = - (1)
       ENDIF
       IF (size(afcgetfieldsreply->serviceitems[1].categories[d1.seq].fields[d2.seq].optionlist,5) >
       0)
    ...
    WITH nocounter
   ;end select
   CALL echorecord(prchargeinfo)
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = value(size(prchargeinfo->flexfieldsmodel,5))),
     (dummyt d2  WITH seq = 1),
     charge_mod cm,
     charge c,
     charge_event_mod cem
    PLAN (d1
     WHERE maxrec(d2,size(prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel,5)))
     JOIN (d2)
     JOIN (cm
     WHERE cm.charge_item_id=pchargeid
      AND cm.active_ind=true
      AND cm.charge_mod_type_cd=cs13019_flex_cd
      AND (cm.field1_id=prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].modifiercd)
     )
     JOIN (c
     ...
     WHERE cem.charge_event_id=c.charge_event_id
      ...
      AND (cem.field1_id=prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].modifiercd
     ))
    DETAIL
     prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].chargemodifierid = cm
     .charge_mod_id, prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].
     chargeeventmodid = cem.charge_event_mod_id
     IF ((prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fielddatatype="DATE"))
      prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fieldvaluedatetime = cm
      .activity_dt_tm
     ELSEIF ((prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fielddatatype=
     "STRING"))
      prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fieldvaluechar = cm.field1
     ELSEIF ((prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fielddatatype IN (
     "NUMBER", "CURRENCY")))
      prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fieldvaluenumber = cm.cm1_nbr
     ELSEIF ((prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fielddatatype=
     "PROVLOOKUP"))
      prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fieldvalueid = cm.field3_id
     ELSEIF ((prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fielddatatype="CODE")
     )
      prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fieldvaluenumber = cm
      .code1_cd
     ELSEIF ((prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fielddatatype=
     "INDICATOR"))
      prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fieldvaluenumber = cm
   ...
   SELECT INTO "nl:"
    ...
    PLAN (d1
     ...
     JOIN (d2)
     JOIN (p
     WHERE (p.person_id=prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].
     fieldvalueid)
      AND (prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fielddatatype=
     "PROVLOOKUP"))
    DETAIL
     prchargeinfo->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].fieldvaluechar = p
     .name_full_formatted
    WITH nocounter
   ;end select
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (getbillcodedata(dummyvar=i2) =i2)
   CALL logmessage("getBillCodeData","Entering...",log_debug)
   DECLARE billcodemodelcount = i4
   DECLARE billcodecount = i4
   DECLARE billcodevalues = vc
   DECLARE sourcevocabularydisplay = vc
   CALL getbillcodeschedidforuser(0)
   SELECT
    IF (size(billcodeschedules->billcodeschedule,5) > 0)
     FROM (dummyt d1  WITH seq = value(size(billcodeschedules->billcodeschedule,5))),
      charge_mod cm,
      nomenclature n
     PLAN (d1)
      JOIN (cm
      WHERE (cm.charge_item_id=request->chargeid)
       ...
       AND (cm.field1_id=billcodeschedules->billcodeschedule[d1.seq].bcschedid))
      JOIN (n
    ...
    ORDER BY billcodemeaning, cm.field2_id
    HEAD billcodemeaning
     billcodemodelcount += 1, stat = alterlist(reply->billcodesmodel,billcodemodelcount), reply->
     billcodesmodel[billcodemodelcount].billcodetype = billcodemeaning,
     billcodecount = 0, billcodevalues = "", sourcevocabularydisplay = ""
    DETAIL
     billcodecount += 1, stat = alterlist(reply->billcodesmodel[billcodemodelcount].billcodes,
      billcodecount), reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].
     chargemodifierid = cm.charge_mod_id,
     CALL populatefields1to5(null), reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount
     ].field6 = cm.field6, reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field7
      = cm.field7,
     reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field1id = cm.field1_id,
     reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field2id = cm.field2_id,
     reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field3id = cm.field3_id,
     reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field4id = cm.field4_id,
     reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field5id = cm.field5_id,
     reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].nomenid = cm.nomen_id,
     reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].cm1nbr = cm.cm1_nbr, reply->
     billcodesmodel[billcodemodelcount].billcodes[billcodecount].code1cd = cm.code1_cd
     IF (checkdic("CHARGE_MOD.CHARGE_MOD_SOURCE_CD","A",0)=2)
      reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].chargemodsourcecode = cm
     ...
     ENDIF
     reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].sourcevocabularydisplay =
     sourcevocabularydisplay
    ...
    FOOT  billcodemeaning
     ...
     ENDIF
     reply->billcodesmodel[billcodemodelcount].billcodevalues = billcodevalues
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = value(size(reply->billcodesmodel,5))),
     (dummyt d2  WITH seq = 1),
     charge c,
     charge_event_mod cem
    PLAN (d1
     WHERE maxrec(d2,size(reply->billcodesmodel[d1.seq].billcodes,5)))
     JOIN (d2)
     JOIN (c
     ...
     WHERE cem.charge_event_id=c.charge_event_id
      ...
      AND (cem.field1_id=reply->billcodesmodel[d1.seq].billcodes[d2.seq].field1id)
      AND (cem.nomen_id=reply->billcodesmodel[d1.seq].billcodes[d2.seq].nomenid)
      AND (cem.field6=reply->billcodesmodel[d1.seq].billcodes[d2.seq].field6)
      AND (cem.field3_id=reply->billcodesmodel[d1.seq].billcodes[d2.seq].field3id)
      AND cem.active_ind=true)
    DETAIL
     reply->billcodesmodel[d1.seq].billcodes[d2.seq].chargeeventmodid = cem.charge_event_mod_id
    WITH nocounter
   ;end select
 ...
 SUBROUTINE (populatefields1to5(dummy=i2) =null)
   IF (validate(reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field1))
    SET reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field1 = cm.field1
    SET reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field2 = cm.field2
    SET reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field3 = cm.field3
    SET reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field4 = cm.field4
    SET reply->billcodesmodel[billcodemodelcount].billcodes[billcodecount].field5 = cm.field5
   ENDIF
 END ;Subroutine
 IF (validate(gettaskaccess,char(128))=char(128))
  SUBROUTINE (gettaskaccess(dummyvar=i2) =i2)
    DECLARE btaskenable = i2 WITH protect, noconstant(0)
    SET btaskenable = isvalidtask(rca_task_upt_adjustment_number)
    IF (btaskenable)
     SET reply->isadjustchargetaskind = btaskenable
    ELSE
     SET reply->isadjustchargetaskind = isvalidtask(cscv_task_upt_adjustment_number)
    ENDIF
    SET btaskenable = isvalidtask(rca_task_upt_adjust_qty_greater)
    IF (btaskenable)
     SET reply->isquantitytaskind = btaskenable
    ELSE
     SET reply->isquantitytaskind = isvalidtask(cscv_task_upt_adjust_qty_greater)
    ENDIF
    SET btaskenable = isvalidtask(rca_task_upt_misc_number)
    IF (btaskenable)
     SET reply->isupdtmiscind = btaskenable
    ELSE
     SET reply->isupdtmiscind = isvalidtask(cscv_task_upt_misc_number)
    ENDIF
    SET btaskenable = isvalidtask(rca_task_modify_cpt_schedule)
    IF (btaskenable)
     SET reply->ismodifycptschedule = btaskenable
    ELSE
     SET reply->ismodifycptschedule = isvalidtask(cscv_task_upt_cpt4_number)
    ENDIF
    SET btaskenable = isvalidtask(rca_task_modify_cdm_schedule)
    IF (btaskenable)
     SET reply->ismodifycdmschedule = btaskenable
    ELSE
     SET reply->ismodifycdmschedule = isvalidtask(cscv_task_upt_cdm_number)
    ENDIF
    SET btaskenable = isvalidtask(rca_task_modify_icd_diag_schedule)
    IF (btaskenable)
     SET reply->ismodifyicddiagschedule = btaskenable
    ELSE
     SET reply->ismodifyicddiagschedule = isvalidtask(cscv_task_upt_icd9_diag_number)
    ENDIF
    SET btaskenable = isvalidtask(rca_task_modify_icd_proc_schedule)
    IF (btaskenable)
     SET reply->ismodifyicdprocschedule = btaskenable
    ELSE
     SET reply->ismodifyicdprocschedule = isvalidtask(cscv_task_upt_icd9_proc_number)
    ENDIF
    SET btaskenable = isvalidtask(rca_task_modify_hcpcs_schedule)
    IF (btaskenable)
     SET reply->ismodifyhcpcsschedule = btaskenable
    ELSE
     SET reply->ismodifyhcpcsschedule = isvalidtask(cscv_task_upt_hcpcs_number)
    ENDIF
    SET btaskenable = isvalidtask(rca_task_modify_revenue_schedule)
    IF (btaskenable)
     SET reply->ismodifyrevenueschedule = btaskenable
    ELSE
     SET reply->ismodifyrevenueschedule = isvalidtask(cscv_task_upt_revenue_number)
    ENDIF
    SET btaskenable = isvalidtask(rca_task_modify_modifier_schedule)
    IF (btaskenable)
     SET reply->ismodifymodifierschedule = btaskenable
    ELSE
     SET reply->ismodifymodifierschedule = isvalidtask(cscv_task_upt_modifier_number)
    ENDIF
    SET btaskenable = isvalidtask(rca_task_modify_ndc_schedule)
    IF (btaskenable)
     SET reply->ismodifyndcschedule = btaskenable
    ELSE
     SET reply->ismodifyndcschedule = isvalidtask(cscv_task_modify_ndc)
    ENDIF
  END ;Subroutine
 ENDIF
 ...
 SUBROUTINE (getbillcodeschedidforuser(dummyvar=i2) =null)
   DECLARE cntbcsched = i4
   DECLARE securitypref = i2
   ...
   SELECT
    ...
    ELSE
     ...
     WHERE cv.code_set=14002
      ...
      AND cv.cdf_meaning IN ("CDM_SCHED", "CPT4", "NDC", "ICD9", "PROCCODE",
     "HCPCS", "REVENUE", "MODIFIER")
    ENDIF
    INTO "nl:"
    ...
    HEAD cv.code_value
     ...
     IF (cv.cdf_meaning="CDM_SCHED"
      AND (reply->cdmschedfield1id=0.0))
      reply->cdmschedfield1id = cv.code_value
     ELSEIF (cv.cdf_meaning="CPT4"
      AND (reply->cptschedfield1id=0.0))
      reply->cptschedfield1id = cv.code_value
     ...
     ELSEIF (cv.cdf_meaning="HCPCS"
      AND (reply->hcpcsschedfield1id=0.0))
      reply->hcpcsschedfield1id = cv.code_value
     ELSEIF (cv.cdf_meaning="REVENUE"
      AND (reply->revenueschedfield1id=0.0))
      reply->revenueschedfield1id = cv.code_value
     ELSEIF (cv.cdf_meaning="MODIFIER"
      AND (reply->modifierschedfield1id=0.0))
      reply->modifierschedfield1id = cv.code_value
     ENDIF
     reply->ndcschedfield1id = cs14002_ndc
    FOOT REPORT
     IF (securitypref)
      cntbcsched += 1, stat = alterlist(billcodeschedules->billcodeschedule,cntbcsched),
      billcodeschedules->billcodeschedule[cntbcsched].bcschedid = cs14002_ndc
     ENDIF
    WITH nocounter
   ;end select
 END ;Subroutine
