CREATE PROGRAM cv_da_add_cv_order_category:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_add_cv_order_category_vrsn = vc WITH private, constant("20080311")
 ...
 WHILE (i <= size(request->objarray,5))
   SET did = 0.0
   IF (validate(request->objarray[i].cv_order_category_id,- (0.00001)) <= 0.0)
    SET did = id_list->item[i].id
    SET request->objarray[i].cv_order_category_id = did
   ELSE
    SET did = request->objarray[i].cv_order_category_id
   ENDIF
   IF (validate(reply->insert_ids)=1)
    SET reply->insert_ids[i].id = did
   ENDIF
   INSERT  FROM cv_order_category c
    SET c.cv_order_category_id = did, c.category_name =
     IF (validate(request->objarray[i].category_name,char(128)) != char(128)) validate(request->
       objarray[i].category_name,char(128))
     ELSE ""
     ENDIF
     , c.category_limit =
     IF ((validate(request->objarray[i].category_limit,- (1)) != - (1))) validate(request->objarray[i
       ].category_limit,- (1))
     ELSE 0
     ENDIF
     ,
