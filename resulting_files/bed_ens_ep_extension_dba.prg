CREATE PROGRAM bed_ens_ep_extension:dba
 ...
 DECLARE extensionexists(eligproviderid=f8) = i2
 DECLARE modifyextension(eligproviderindex=i4) = i2
 DECLARE insertextension(eligproviderindex=i4) = i2
 DECLARE createnewid(dummyvar=i2) = f8
 FOR (x = 1 TO size(request->elig_providers,5))
   IF (extensionexists(request->elig_providers[x].elig_prov_id))
    CALL modifyextension(x)
   ELSE
    CALL insertextension(x)
   ENDIF
 ENDFOR
 SUBROUTINE extensionexists(eligproviderid)
   CALL bedlogmessage("extensionExists","Entering...")
   SELECT INTO "nl:"
    FROM br_elig_prov_extension bepe
    WHERE bepe.br_eligible_provider_id=eligproviderid
 ...
 SUBROUTINE createnewid(dummyvar)
   ...
   RETURN(new_id)
 END ;Subroutine
 SUBROUTINE modifyextension(eligproviderindex)
   CALL bedlogmessage("modifyExtension","Entering...")
   SELECT INTO "nl:"
    FROM br_elig_prov_extension bepe
    WHERE (bepe.br_eligible_provider_id=request->elig_providers[eligproviderindex].elig_prov_id)
     AND bepe.active_ind=1
     AND bepe.end_effective_dt_tm > cnvtdatetime(curdate,curtime3)
    DETAIL
     ...
     copyextension->program_type_txt = bepe.program_type_txt, copyextension->medicaid_stage_cd = bepe
     .medicaid_stage_cd, copyextension->medicare_year = bepe.medicare_year,
   ...
   INSERT  FROM br_elig_prov_extension bepe
    SET bepe.br_elig_prov_extension_id = modifynewid, bepe.orig_br_elig_prov_extension_id =
     copyextension->orig_br_elig_prov_extension_id, bepe.br_eligible_provider_id = copyextension->
     br_eligible_provider_id,
     bepe.program_type_txt = copyextension->program_type_txt, bepe.medicaid_stage_cd = copyextension
     ->medicaid_stage_cd, bepe.medicare_year = copyextension->medicare_year,
   ...
   UPDATE  FROM br_elig_prov_extension bepe
    SET bepe.program_type_txt = request->elig_providers[eligproviderindex].program_type, bepe
     .medicaid_stage_cd = request->elig_providers[eligproviderindex].medicaid, bepe.medicare_year =
     request->elig_providers[eligproviderindex].medicare,
     bepe.beg_effective_dt_tm = cnvtdatetime(curdate,curtime3), bepe.updt_dt_tm = cnvtdatetime(
      curdate,curtime3), bepe.updt_id = reqinfo->updt_id,
     bepe.updt_task = reqinfo->updt_task, bepe.updt_applctx = reqinfo->updt_applctx, bepe.updt_cnt =
     (bepe.updt_cnt+ 1)
 ...
 END ;Subroutine
 SUBROUTINE insertextension(eligproviderindex)
   ...
   INSERT  FROM br_elig_prov_extension bepe
    SET bepe.br_elig_prov_extension_id = insertnewid, bepe.orig_br_elig_prov_extension_id =
     insertnewid, bepe.br_eligible_provider_id = request->elig_providers[eligproviderindex].
     elig_prov_id,
     bepe.program_type_txt = request->elig_providers[eligproviderindex].program_type, bepe
     .medicaid_stage_cd = request->elig_providers[eligproviderindex].medicaid, bepe.medicare_year =
     request->elig_providers[eligproviderindex].medicare,
     bepe.beg_effective_dt_tm = cnvtdatetime(curdate,curtime3), bepe.end_effective_dt_tm =
     cnvtdatetime("31-DEC-2100"), bepe.active_ind = 1,
     bepe.updt_dt_tm = cnvtdatetime(curdate,curtime3), bepe.updt_id = reqinfo->updt_id, bepe
     .updt_task = reqinfo->updt_task,
     bepe.updt_applctx = reqinfo->updt_applctx, bepe.updt_cnt = 0
    WITH nocounter
