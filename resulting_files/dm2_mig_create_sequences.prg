CREATE PROGRAM dm2_mig_create_sequences
 ...
 SUBROUTINE dctx_restore_prev_contexts(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmcs_prompt = i2 WITH protect, noconstant(0)
 DECLARE dmcs_op_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmcs_ndx = i4 WITH protect, noconstant(0)
 DECLARE dmcs_op_str = vc WITH protect, noconstant("")
 DECLARE dmcs_increase_value = f8 WITH protect, noconstant(1000.00)
 DECLARE dmcs_remove_reconnect_target(null) = i2
 DECLARE dmcs_max_value_str = vc WITH protect, noconstant("")
 DECLARE dmcs_src_clinical_seq_nextval = f8 WITH protect, noconstant(0.0)
 DECLARE dmcs_tgt_clinical_seq_nextval = f8 WITH protect, noconstant(0.0)
 IF ((dm2_install_schema->curprog="NONE"))
  SET dmcs_prompt = 1
 ENDIF
 IF (check_logfile("dm2_mig_create_seq",".log","dm2_mig_create_sequences")=0)
  GO TO exit_program
 ENDIF
 SET dmcs_max_value_str = "99999999999999.0"
 FREE RECORD dmcs_seqs
 RECORD dmcs_seqs(
   1 cnt = i4
   1 qual[*]
     2 seq_name = vc
 ...
 SET dmcs_seqs->cnt = 0
 SET dm_err->eproc = "Starting dm2_mig_create_sequences"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ENDIF
 IF (dmcs_prompt=1)
  SET dm_err->eproc = "Prompt user for Verification."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  IF (dmr_issue_summary_screen(1,1,
   "Continuing this process will drop and create sequences on the TARGET database.")=0)
   RETURN(0)
  ENDIF
 ENDIF
 SET dm_err->eproc = "While connected to source, get the clinical_seq_nextval."
 SELECT INTO "nl:"
  nextseq = seq(dm_clinical_seq,nextval)
  FROM dual
  DETAIL
   dmcs_src_clinical_seq_nextval = nextseq
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "While connected to source, store off all sequences. Build sequence ddl."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dba_sequences us
  WHERE us.sequence_owner IN (
  (SELECT
   di.info_name
   FROM dm_info di
   WHERE ((di.info_domain="DM2_MIG_DT_USER"
    AND di.info_char="1") OR (di.info_domain="DM2_MIG_USER")) ))
  ORDER BY us.sequence_name
  DETAIL
   dmcs_seqs->cnt = (dmcs_seqs->cnt+ 1)
   IF (mod(dmcs_seqs->cnt,100)=1)
    stat = alterlist(dmcs_seqs->qual,(dmcs_seqs->cnt+ 99))
   ENDIF
   dmcs_seqs->qual[dmcs_seqs->cnt].seq_name = us.sequence_name, dmcs_seqs->qual[dmcs_seqs->cnt].owner
    = us.sequence_owner, dmcs_seqs->qual[dmcs_seqs->cnt].start_value = us.last_number,
   dmcs_seqs->qual[dmcs_seqs->cnt].max_value = us.max_value
   IF (us.cycle_flag="N")
    dmcs_seqs->qual[dmcs_seqs->cnt].eval_increase = 1
   ENDIF
   dmcs_seqs->qual[dmcs_seqs->cnt].create_str_beg = concat("RDB CREATE SEQUENCE ",trim(us
     .sequence_owner),".",trim(us.sequence_name)," INCREMENT BY ",
    build(us.increment_by)," start with ")
   IF (us.cycle_flag="N"
    AND us.max_value > cnvtreal(dmcs_max_value_str))
    dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
     create_str_end," NOMAXVALUE ")
   ELSE
    dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
     create_str_end," MAXVALUE ",build(us.max_value))
   ENDIF
   IF (us.min_value=null)
    dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
     create_str_end," NOMINVALUE ")
   ELSE
    dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
     create_str_end," MINVALUE ",build(us.min_value))
   ENDIF
   IF (us.cycle_flag="N")
    dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
     create_str_end," NOCYCLE ")
   ELSE
    dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
     create_str_end," CYCLE ")
   ENDIF
   IF (us.cache_size=0)
    dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
     create_str_end," NOCACHE ")
   ELSE
    dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
     create_str_end," CACHE ",build(us.cache_size))
   ENDIF
   IF (us.order_flag="N")
    dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
     create_str_end," NOORDER ")
   ELSE
    dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
     create_str_end," ORDER ")
   ENDIF
   dmcs_seqs->qual[dmcs_seqs->cnt].create_str_end = concat(dmcs_seqs->qual[dmcs_seqs->cnt].
    create_str_end," go ")
  FOOT REPORT
   stat = alterlist(dmcs_seqs->qual,dmcs_seqs->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmcs_seqs)
  CALL echorecord(dm2_install_schema)
 ENDIF
 IF (dmr_prompt_connect_data("TARGET","V500","CO")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Get clinical_seq_nextval from target"
 SELECT INTO "nl:"
  nextseq = seq(dm_clinical_seq,nextval)
  FROM dual
  DETAIL
   dmcs_tgt_clinical_seq_nextval = nextseq
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Get all sequences from target"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dba_sequences u
  WHERE u.sequence_owner IN (
  (SELECT
   di.info_name
   FROM dm_info@ref_data_link di
   WHERE ((di.info_domain="DM2_MIG_DT_USER"
    AND di.info_char="1") OR (di.info_domain="DM2_MIG_USER")) ))
  ORDER BY u.sequence_name
  DETAIL
   dmcs_ndx = locateval(dmcs_ndx,1,dmcs_seqs->cnt,u.sequence_name,dmcs_seqs->qual[dmcs_ndx].seq_name,
    u.sequence_owner,dmcs_seqs->qual[dmcs_ndx].owner)
   IF (dmcs_ndx > 0)
    dmcs_seqs->qual[dmcs_ndx].drop_ind = 1
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmcs_seqs)
 ENDIF
 SET dm_err->eproc = "Check for start value increase"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_MIG_CREATE_SEQUENCES"
   AND d.info_name="INCREASE_VALUE"
  DETAIL
   dmcs_increase_value = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (mod(dmcs_increase_value,2)=1)
  SET dmcs_increase_value = (dmcs_increase_value+ 1)
  SET dm_err->eproc = build("DM_INFO INCREASE_VALUE adjusted to even. New value is (",
   dmcs_increase_value,")")
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 ...
 ENDIF
 IF ((dmcs_seqs->cnt > 0))
  FOR (dmcs_op_cnt = 1 TO value(dmcs_seqs->cnt))
    IF ((dmcs_seqs->qual[dmcs_op_cnt].drop_ind=1))
     SET dm_err->eproc = concat("Drop and recreate sequence:",dmcs_seqs->qual[dmcs_op_cnt].owner,".",
      dmcs_seqs->qual[dmcs_op_cnt].seq_name)
    ELSE
     SET dm_err->eproc = concat("Create sequence:",dmcs_seqs->qual[dmcs_op_cnt].owner,".",dmcs_seqs->
      qual[dmcs_op_cnt].seq_name)
    ENDIF
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    IF ((dmcs_seqs->qual[dmcs_op_cnt].drop_ind=1))
     IF (dm2_push_cmd(concat("rdb drop sequence ",dmcs_seqs->qual[dmcs_op_cnt].owner,".",dmcs_seqs->
       qual[dmcs_op_cnt].seq_name," go"),1)=0)
      GO TO exit_program
     ENDIF
    ENDIF
    IF (dmcs_tgt_clinical_seq_nextval > dmcs_src_clinical_seq_nextval
     AND (dmcs_seqs->qual[dmcs_op_cnt].seq_name="DM_CLINICAL_SEQ"))
     SET dmcs_seqs->qual[dmcs_op_cnt].start_value = dmcs_tgt_clinical_seq_nextval
    ENDIF
    IF ((dmcs_seqs->qual[dmcs_op_cnt].eval_increase=1)
     AND ((dmcs_seqs->qual[dmcs_op_cnt].max_value - dmcs_seqs->qual[dmcs_op_cnt].start_value) >
    dmcs_increase_value))
     SET dmcs_op_str = concat(dmcs_seqs->qual[dmcs_op_cnt].create_str_beg," ",build((dmcs_seqs->qual[
       dmcs_op_cnt].start_value+ dmcs_increase_value))," ",dmcs_seqs->qual[dmcs_op_cnt].
      create_str_end)
    ELSE
     SET dmcs_op_str = concat(dmcs_seqs->qual[dmcs_op_cnt].create_str_beg," ",build(dmcs_seqs->qual[
       dmcs_op_cnt].start_value)," ",dmcs_seqs->qual[dmcs_op_cnt].create_str_end)
    ENDIF
    IF (dm2_push_cmd(dmcs_op_str,1)=0)
     GO TO exit_program
    ENDIF
  ENDFOR
 ENDIF
 SUBROUTINE dmcs_remove_reconnect_target(null)
   ...
   IF (((currdbhandle=" ") OR ((dm2_install_schema->p_word="NONE"))) )
    SET message = nowindow
    SET dm_err->err_ind = 1
 ...
 END ;Subroutine
#exit_program
 CALL dmcs_remove_reconnect_target(null)
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_create_sequences completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_create_seq")
END GO
