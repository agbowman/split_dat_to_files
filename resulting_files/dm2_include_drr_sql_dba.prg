CREATE PROGRAM dm2_include_drr_sql:dba
 ...
 SET readme_data->message = "Readme failed: Starting script dm2_include_drr_sql.prg..."
 DECLARE inhouse_ind = i2 WITH protect, noconstant(0)
 DECLARE errmsg = vc WITH protect, noconstant("")
 DECLARE context_creation_query = vc WITH protect, noconstant("")
 DECLARE dcms_ora_version = i4 WITH protect, noconstant(0)
 ...
 IF ((((dm2_rdbms_version->level1 < 11)) OR ((dm2_rdbms_version->level1=11)
  ...
  SET readme_data->message = "Auto Success: Oracle version is less than 11.2"
  GO TO exit_script
 ENDIF
 SET context_creation_query = concat("CREATE OR REPLACE CONTEXT DRR_CORE_CONTEXT USING DRR_CORE")
 CALL echo(context_creation_query)
 CALL parser(concat("rdb asis(^ ",context_creation_query,"^) go"))
 EXECUTE dm_readme_include_sql "cer_install:drr_core_header.sql"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql_chk "drr_core", "package"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql "cer_install:drr_core_body.sql"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql_chk "drr_core", "package body"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
