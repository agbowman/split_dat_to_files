CREATE PROGRAM dm2_mv_drop:dba
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmd_mv_name = vc WITH protect, noconstant(" ")
 DECLARE dmd_mv_cur_instance = i2 WITH protect, noconstant(0)
 DECLARE dmd_mv_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE dmd_syn_exist_ind = i2 WITH protect, noconstant(0)
 DECLARE dmd_syn_ref_name = vc WITH protect, noconstant(" ")
 DECLARE dmd_cmd = vc WITH protect, noconstant(" ")
 DECLARE dmd_mv_status = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm2_mv_drop",".log","dm2_mv_drop LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dmd_mv_name = trim(cnvtupper( $1))
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg = "Parameter usage: dm2_mv_drop '<Materialzied View Name>'"
  GO TO exit_program
 ENDIF
 CALL dm2_get_rdbms_version(null)
 IF ((dm2_rdbms_version->level1 < 9))
  SET dm_err->eproc = "Materialized View drops not needed on Oracle 8.  Exiting..."
  CALL disp_msg(" ",dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(dmd_mv_name)
 ENDIF
 SET dm_err->eproc = concat("Retrieving Current Instance of MVIEW installed for ",dmd_mv_name)
 CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MVIEW_INSTANCE"
   AND di.info_name=cnvtupper(dmd_mv_name)
  DETAIL
   dmd_mv_cur_instance = di.info_number, dmd_mv_status = cnvtupper(di.info_char)
  WITH nocounter
 ;end select
 IF (dmd_mv_cur_instance=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Current Instance not found for Materialized View ",dmd_mv_name,
   ". Drop not processed.")
  SET dm_err->user_action = "Resolve missing Instance data and restart the process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Retrieving Admin Metadata for MVIEW ",dmd_mv_name)
 CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
 IF (dmvr_get_mv_metadata(dmd_mv_name,dmd_mv_cur_instance)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(mv)
 ...
 IF (trim(mv->name)="")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Metadata not found for Materialized View ",dmd_mv_name,
   ". Drop not processed.")
  SET dm_err->user_action = "Resolve missing metadata and restart the process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 ...
 IF (dmvr_create_mv_syn(mv->syn_name,mv->cview_name)=0)
  GO TO exit_program
 ENDIF
 IF (dmvr_get_mv(mv->name,dmd_mv_exists_ind)=0)
  GO TO exit_program
 ENDIF
 IF (dmd_mv_exists_ind=1)
  SET dm_err->eproc = concat("Dropping Materialized View ",mv->name)
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dmd_cmd = concat("RDB DROP MATERIALIZED VIEW ",dmd_mv_name," GO ")
  IF ( NOT (dm2_push_cmd(dmd_cmd,1)))
   GO TO exit_program
  ENDIF
 ELSE
  SET dm_err->eproc = concat("Materialized View ",mv->name," does not exist.  Drop bypassed.")
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (dmd_mv_status != "DISABLED")
  IF (dmvr_updt_mv_inst("U",dmd_mv_name,0,"CVIEW")=0)
   GO TO exit_program
  ENDIF
 ENDIF
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "DM2_MV_DROP completed"
 CALL final_disp_msg("dm2_mv_drop")
END GO
