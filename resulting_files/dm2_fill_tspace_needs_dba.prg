CREATE PROGRAM dm2_fill_tspace_needs:dba
 SET width = 132
 ...
 SUBROUTINE db2_push_dcl_w_connect(sbr_dwc_dbase,sbr_dwc_user,sbr_dwc_user_pwd,sbr_dwc_str,
  sbr_dwc_commit_ind)
   ...
   RETURN(push_rtrn)
 END ;Subroutine
 DECLARE dftn_logfile_prefix = vc WITH protect, constant("dm2filltspaceneeds")
 DECLARE dftn_tspaces_to_process_cnt = i4 WITH protect, noconstant(0)
 DECLARE dftn_tmpsize_table_name = vc WITH protect, noconstant(" ")
 DECLARE rtspace_cnt = i4 WITH protect, noconstant(0)
 DECLARE dftn_set_tmpsize_table_name() = null
 DECLARE dftn_import_tmpsize_table() = null
 DECLARE dftn_display_main_window() = null
 DECLARE dftn_check_tmpsize_table() = null
 DECLARE dftn_apply_tsprec_to_tgtsch() = null
 DECLARE dftn_build_rtspace_rs() = i2
 DECLARE dftn_match_rtspace_to_tablespace() = i2
 DECLARE dftn_tablespaces_to_install() = i2
 IF (check_logfile(dftn_logfile_prefix,".log","DM2_FILL_TSPACE_NEEDS LOGFILE")=false)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting DM2_FILL_TSPACE_NEEDS..."
 CALL disp_msg("",dm_err->logfile,0)
 IF (currdb="DB2UDB")
  IF (dftn_set_tmpsize_table_name(1) != 1)
   GO TO exit_program
  ENDIF
  IF (dftn_import_tmpsize_table(1) != 1)
   GO TO exit_program
  ENDIF
  EXECUTE oragen3 value(dftn_tmpsize_table_name)
  IF ((dm_err->err_ind=1))
   RETURN(0)
  ENDIF
  IF (dftn_check_tmpsize_table(1) != 1)
   GO TO exit_program
  ENDIF
  WHILE (dftn_tspaces_to_process_cnt > 0)
   CALL dftn_display_main_window(null)
   CASE (cnvtupper(curaccept))
    ...
    OF "Q":
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("** User requested to QUIT the installation process",
      " from dm2_fill_tspace_needs. ")
     SET dm_err->eproc = concat("DM2_FILL_TSPACE_NEEDS")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
   ENDCASE
  ENDWHILE
 ELSEIF (currdb="SQLSRV")
  CALL dftn_build_rtspace_rs(null)
  SET dm2_install_schema->default_fg_name = "DEFAULT_FG"
  IF ((dm2_install_schema->process_option IN ("ADMIN CREATE", "NONPROD CREATE")))
   SET rtspace_cnt = size(rtspace->qual,5)
   SET rtspace_cnt = (rtspace_cnt+ 1)
   SET stat = alterlist(rtspace->qual,rtspace_cnt)
   SET rtspace->qual[rtspace_cnt].tspace_name = dm2_install_schema->default_fg_name
   SET rtspace->qual[rtspace_cnt].new_ind = 1
  ENDIF
  IF ((dm2_install_schema->process_option="NONPROD CREATE"))
   SET rtspace_cnt = size(rtspace->qual,5)
   SET rtspace_cnt = (rtspace_cnt+ 1)
   SET stat = alterlist(rtspace->qual,rtspace_cnt)
   SET rtspace->qual[rtspace_cnt].tspace_name = "D_A_SMALL"
   SET rtspace->qual[rtspace_cnt].new_ind = 1
   SET rtspace_cnt = (rtspace_cnt+ 1)
   SET stat = alterlist(rtspace->qual,rtspace_cnt)
   SET rtspace->qual[rtspace_cnt].tspace_name = "I_A_SMALL"
   SET rtspace->qual[rtspace_cnt].new_ind = 1
  ENDIF
  IF (dftn_match_rtspace_to_tablespace(null)=0)
   GO TO exit_program
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echorecord(rtspace)
  ENDIF
  IF (dftn_tablespaces_to_install(null)=1)
   EXECUTE dm2_tspace_menu
   IF ((dm_err->err_ind=1))
    GO TO exit_program
   ENDIF
  ELSE
   SET dm_err->eproc = concat("** No Tablespaces to Install. **")
   CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = concat("** All Tablespaces successfully installed.  Exiting menu... ")
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 GO TO exit_program
 SUBROUTINE dftn_display_main_window(null)
   DECLARE ddmw_step_cnt = i2 WITH private, noconstant(0)
   DECLARE ddmw_winline_cnt = i2 WITH private, noconstant(0)
   DECLARE ddmw_dbval = vc WITH private, noconstant(" ")
   CASE (currdbuser)
    OF "V500":
     SET ddmw_dbval = "v500"
    OF "CDBA":
   ...
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL text(2,3,"******************   F I L L   T A B L E S P A C E   N E E D S  ******************"
    )
   CALL text(4,3,build("The installation process found (",dftn_tspaces_to_process_cnt,
     ") tablespace(s) that need(s) to be created or extended."))
   CALL text(6,3,concat(
     "You must perform the tablespace create/extend process in the target domain itself ",
     "so that the physical disk drives for that domain "))
   CALL text(7,3,"can be accessed.")
   CALL text(9,3,concat(
     "To create and/or extend tablespaces, please launch a new session in the target ",
     "domain and perform the following steps: "))
   SET ddmw_winline_cnt = 9
   IF (cursys="AIX")
    SET ddmw_step_cnt = (ddmw_step_cnt+ 1)
    SET ddmw_winline_cnt = (ddmw_winline_cnt+ 1)
    CALL text(ddmw_winline_cnt,5,concat(trim(cnvtstring(ddmw_step_cnt)),
      ") Login to AIX as the root user so that raw devices can be created."))
    IF (currdb="DB2UDB")
     SET ddmw_step_cnt = (ddmw_step_cnt+ 1)
     SET ddmw_winline_cnt = (ddmw_winline_cnt+ 1)
     CALL text(ddmw_winline_cnt,5,concat(trim(cnvtstring(ddmw_step_cnt)),") Perform the ",
       "unix command to set your environment ( for example, newgrp - ________ )."))
     SET ddmw_step_cnt = (ddmw_step_cnt+ 1)
     SET ddmw_winline_cnt = (ddmw_winline_cnt+ 1)
     CALL text(ddmw_winline_cnt,5,concat(trim(cnvtstring(ddmw_step_cnt)),
       ") At OS prompt:    . /db2/home/",trim(ddmw_dbval),"/sqllib/db2profile ",
       "   (don't forget to include the dot)"))
     SET ddmw_step_cnt = (ddmw_step_cnt+ 1)
     SET ddmw_winline_cnt = (ddmw_winline_cnt+ 1)
     CALL text(ddmw_winline_cnt,5,concat(trim(cnvtstring(ddmw_step_cnt)),
       ") At OS prompt:    echo $INSTHOME  (a return value of ",trim(ddmw_dbval),
       " will confirm that step ",trim(cnvtstring((ddmw_step_cnt - 1))),
       " was successful"))
    ENDIF
   ENDIF
   SET ddmw_step_cnt = (ddmw_step_cnt+ 1)
   SET ddmw_winline_cnt = (ddmw_winline_cnt+ 1)
   CALL text(ddmw_winline_cnt,5,concat(trim(cnvtstring(ddmw_step_cnt)),") Login to CCL "))
   SET ddmw_winline_cnt = (ddmw_winline_cnt+ 1)
   SET ddmw_step_cnt = (ddmw_step_cnt+ 1)
   CALL text(ddmw_winline_cnt,5,concat(trim(cnvtstring(ddmw_step_cnt)),
     ") At CCL prompt:   DM2_TSPACE_MENU GO "))
   SET ddmw_winline_cnt = (ddmw_winline_cnt+ 2)
   CALL text(ddmw_winline_cnt,3,concat(
     "After you have successfully created/extended your tablespaces via ",
     "DM2_TSPACE_MENU, return back to this session to continue or quit"))
   SET ddmw_winline_cnt = (ddmw_winline_cnt+ 1)
   CALL text(ddmw_winline_cnt,3,"the installation process. ")
   CALL text(21,3,"Enter 'C' (without the quotes) to CONTINUE the installation process. ")
   CALL text(22,3,"Enter 'Q' (without the quotes) to QUIT the installation process. ")
   CALL text(24,3,"Continue (C) / Quit (Q) : ")
   CALL accept(24,50,"A;CU","C"
    WHERE curaccept IN ("C", "Q", "c", "q"))
   CALL clear(1,1)
   SET message = nowindow
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dftn_set_tmpsize_table_name(null)
   CASE (currdbuser)
    OF "CDBA":
     SET dftn_tmpsize_table_name = "DM2A01312003_TMPSIZE"
    OF "V500":
     SET dftn_tmpsize_table_name = "DM2C02012003_TMPSIZE"
    ELSE
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("DM2_FILL_TSPACE_NEEDS.PRG - ** Invalid user (",currdbuser,
      " to run this process.  Must be V500 or CDBA. ")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
   ENDCASE
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("** dftn_tmpsize_table_name=",dftn_tmpsize_table_name))
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dftn_import_tmpsize_table(null)
   DECLARE dsttn_table_exists = i2 WITH protect, noconstant(0)
   DECLARE dsttn_import_cmd = vc WITH protect, noconstant(" ")
   SELECT INTO "nl:"
    FROM dm2_user_tables d
    WHERE d.table_name=dftn_tmpsize_table_name
    DETAIL
     dsttn_table_exists = 1
    WITH nocounter
   ;end select
   IF (check_error(concat("DM2_FILL_TSPACE_NEEDS.PRG - Subroutine dftn_import_tmpsize_table: ",
     " select from dm2_user_tables view. ")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    RETURN(0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("** dsttn_table_exists=",dsttn_table_exists))
   ENDIF
   IF (dsttn_table_exists != 1)
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("Importing the temp sizing table...",dftn_tmpsize_table_name))
    ENDIF
    SET dsttn_import_cmd = concat(" import from $cer_install/",cnvtlower(dftn_tmpsize_table_name),
     ".ixf of ixf"," commitcount 100 create into ",dftn_tmpsize_table_name)
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("Import command =",dsttn_import_cmd))
    ENDIF
    IF (db2_push_dcl_w_connect(currdblink,currdbuser,currdbuser,dsttn_import_cmd,1)=0)
     ROLLBACK
     RETURN(0)
    ELSE
     COMMIT
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dftn_check_tmpsize_table(null)
   SET dftn_tspaces_to_process_cnt = 0
   SELECT INTO "nl:"
    FROM (value(dftn_tmpsize_table_name))
    WHERE install_status != "SUCCESS"
    DETAIL
     dftn_tspaces_to_process_cnt = (dftn_tspaces_to_process_cnt+ 1)
    WITH nocounter
   ;end select
   IF (check_error(concat(
     "DM2_FILL_TSPACE_NEEDS.PRG - Error occurred in Subroutine dftn_check_tmpsize_table: ",
     " Select From ",dftn_tmpsize_table_name)) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    RETURN(0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("** dftn_tspaces_to_process_cnt=",dftn_tspaces_to_process_cnt))
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dftn_apply_tsprec_to_tgtsch(null)
   DECLARE dattt_found = i2 WITH protect, noconstant(0)
   DECLARE dattt_cnt = i4 WITH protect, noconstant(0)
   CALL echo(" ** IN dftn_apply_tsprec_to_tgtsch **")
   SELECT INTO "nl:"
    dtd.table_name
    FROM dm_tables_doc dtd,
     dm_ts_precedence dtp
    WHERE dtd.table_name=dtd.suffixed_table_name
     AND dtd.table_name=dtp.table_name
     AND dtp.precedence=1
    DETAIL
     dattt_found = 0, dattt_cnt = 0
     WHILE (dattt_found=0
      AND (dattt_cnt < tgtsch->tbl_cnt))
      dattt_cnt = (dattt_cnt+ 1),
      IF ((tgtsch->tbl[dattt_cnt].tbl_name=dtp.table_name))
       dattt_found = 1, tgtsch->tbl[dattt_cnt].tspace_name = dtp.data_tablespace, tgtsch->tbl[
       dattt_cnt].ind_tspace = dtp.index_tablespace
      ENDIF
     ENDWHILE
    WITH nocounter
   ;end select
   IF (check_error(concat(
     "DM2_FILL_TSPACE_NEEDS.PRG - Error occurred in Subroutine : dftn_apply_tsprec_to_tgtsch",
     " Select From dm_ts_precedence/dm_tables_doc")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dftn_build_rtspace_rs(null)
   DECLARE tblx = i4 WITH protect, noconstant(0)
   DECLARE indx = i4 WITH protect, noconstant(0)
   DECLARE rtspacex = i4 WITH protect, noconstant(0)
   FOR (tblx = 1 TO tgtsch->tbl_cnt)
    IF ((dm_err->debug_flag > 0))
     CALL echo(build("TgtSch table name:",tgtsch->tbl[tblx].tbl_name))
    ENDIF
    IF ((((tgtsch->tbl[tblx].new_ind=1)) OR ((tgtsch->tbl[tblx].diff_ind=1))) )
     CALL echo(build("New Ind:",tgtsch->tbl[tblx].new_ind,":Diff Ind:",tgtsch->tbl[tblx].diff_ind))
     SET indx = 0
     SET rtspacex = 0
     SET rtspace_cnt = size(rtspace->qual,5)
     SET rtspacex = locateval(indx,1,rtspace_cnt,trim(tgtsch->tbl[tblx].tspace_name),rtspace->qual[
      indx].tspace_name)
     IF (rtspacex=0)
      SET rtspace_cnt = (rtspace_cnt+ 1)
      SET stat = alterlist(rtspace->qual,rtspace_cnt)
      SET rtspace->qual[rtspace_cnt].tspace_name = tgtsch->tbl[tblx].tspace_name
      SET rtspace->qual[rtspace_cnt].new_ind = 1
      IF ((dm_err->debug_flag > 0))
       CALL echo(build("TBL Tablespace:",tgtsch->tbl[tblx].tspace_name,
         ":not found on rTspace rs, adding to row",rtspace_cnt))
      ENDIF
     ELSE
      IF ((dm_err->debug_flag > 0))
       CALL echo(build("TBL Tablespace:",tgtsch->tbl[tblx].tspace_name,
         ":found on rTspace rs, Location:",rtspacex))
      ENDIF
     ENDIF
     SET indx = 0
     SET rtspacex = 0
     SET rtspace_cnt = size(rtspace->qual,5)
     SET rtspacex = locateval(indx,1,rtspace_cnt,trim(tgtsch->tbl[tblx].ind_tspace),rtspace->qual[
      indx].tspace_name)
     IF (rtspacex=0)
      SET rtspace_cnt = (rtspace_cnt+ 1)
      SET stat = alterlist(rtspace->qual,rtspace_cnt)
      SET rtspace->qual[rtspace_cnt].tspace_name = tgtsch->tbl[tblx].ind_tspace
      SET rtspace->qual[rtspace_cnt].new_ind = 1
      IF ((dm_err->debug_flag > 0))
       CALL echo(build("IDX Tablespace:",tgtsch->tbl[tblx].ind_tspace,
         ":not found on rTspace rs, adding to row:",rtspace_cnt))
      ENDIF
     ELSE
      IF ((dm_err->debug_flag > 0))
   ...
   ENDFOR
 END ;Subroutine
 SUBROUTINE dftn_match_rtspace_to_tablespace(null)
   SELECT INTO "nl:"
    tablespace_name
    FROM dm2_dba_tablespaces ts
    DETAIL
     indx = 0, rtspacex = 0, rtspace_cnt = size(rtspace->qual,5),
     rtspacex = locateval(indx,1,rtspace_cnt,trim(ts.tablespace_name),rtspace->qual[indx].tspace_name
      )
     IF (rtspacex > 0)
      rtspace->qual[rtspacex].new_ind = 0
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error("dftn_match_rTspace_to_tablespace")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dftn_tablespaces_to_install(null)
   DECLARE x = i4 WITH private, noconstant(0)
   FOR (x = 1 TO size(rtspace->qual,5))
     IF ((rtspace->qual[x].new_ind=1))
      RETURN(1)
     ENDIF
   ENDFOR
   RETURN(0)
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "...Ending DM2_FILL_TSPACE_NEEDS"
 CALL final_disp_msg(dftn_logfile_prefix)
END GO
