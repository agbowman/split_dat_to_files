CREATE PROGRAM dcp_hc_zscore_calc:dba
 ...
 DECLARE calculatehczscore(null) = null
 ...
 DECLARE hc_zscore = f8 WITH noconstant(0.0)
 ...
 SUBROUTINE getstatsdata(null)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE calculatehczscore(null)
  DECLARE pi = f8 WITH noconstant(3.14159265)
  IF (hc_box_cox_power != 0)
   SET hc_zscore = ((((hc/ hc_median)** hc_box_cox_power) - 1)/ (hc_box_cox_power * hc_coeff_of_var))
  ELSE
   SET hc_zscore = (log((hc/ hc_median))/ hc_coeff_of_var)
  ENDIF
 END ;Subroutine
#exit_script
 SET log_misc1 = concat(trim(cnvtstringchk(hc_zscore,10,6)),"|",trim(cnvtstringchk(hc_chart_source_cd
    )),"|",hc_chart_source_disp)
 IF (failed=1)
  SET retval = 0
  SET log_message = error_msg
 ELSE
  SET retval = 100
  SET log_message = "HC Z-score calculation finished."
 ENDIF
END GO
