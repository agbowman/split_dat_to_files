CREATE PROGRAM ae_attend_rpt:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Start of reporting period:" = "SYSDATE",
  "End of reporting period:" = "SYSDATE",
  "Trust:" = "",
  "Facility" = "",
  "AE department" = ""
...
#ukr_common_rpt_exit
 ...
 IF (validate(ukr_4001902_sub_ind) != 1)
  ...
  DECLARE getoption1fieldvalue(cdf_meaning=vc) = vc
  DECLARE getreportingconfigcv(cdf_meaning=vc) = f8
  SUBROUTINE getoption1fieldvalue(cdf_meaning)
  ...
  END ;Subroutine
  SUBROUTINE getreportingconfigcv(cdf_meaning)
 ...
 IF (textlen(trim(loc_parser,3)) > 0)
  ...
  DECLARE encntr_type_where = vc WITH protect, noconstant(build2("e.encntr_type_cd = ae_cd and e.",
    loc_parser))
 ...
 IF (( $STARTDATE="CURDATE"))
  SET start_date = format(cnvtdatetime((curdate - 1),0730),";;Q")
 ELSE
  SET start_date =  $STARTDATE
 ENDIF
 IF (( $ENDDATE="CURDATE"))
  SET end_date = format(cnvtdatetime(curdate,0730),";;Q")
 ELSE
 ...
 SELECT DISTINCT INTO "nl:"
  FROM encounter e,
   tracking_item ti
  PLAN (e
   WHERE e.arrive_dt_tm >= cnvtdatetime(start_date)
    AND e.arrive_dt_tm < cnvtdatetime(end_date)
    AND expand(idx,1,trusts->trust_cnt,e.organization_id,trusts->trust_list[idx].trust_id)
    AND parser(encntr_type_where)
    AND e.active_ind=1
    AND e.arrive_dt_tm >= cnvtdatetime(start_date)
    AND e.arrive_dt_tm < cnvtdatetime(end_date))
   JOIN (ti
  ...
  HEAD REPORT
   ae_cnt = 0, stat = alterlist(ae_attend->qual,50)
  HEAD e.encntr_id
   ae_cnt = (ae_cnt+ 1)
   IF (ae_cnt > size(ae_attend->qual,5))
    stat = alterlist(ae_attend->qual,(ae_cnt+ 49))
   ENDIF
   ae_attend->qual[ae_cnt].encntr_id = e.encntr_id, ae_attend->qual[ae_cnt].tracking_id = ti
   .tracking_id
  FOOT  e.encntr_id
   null
  FOOT REPORT
   ae_attend->enc_cnt = ae_cnt, stat = alterlist(ae_attend->qual,ae_cnt)
  WITH nocounter, expand = 1
 ;end select
 SELECT DISTINCT INTO "nl:"
  FROM encntr_info ei
  PLAN (ei
   WHERE expand(idx,1,ae_attend->enc_cnt,ei.encntr_id,ae_attend->qual[idx].encntr_id)
    AND ei.info_sub_type_cd=stream_cd
    AND ei.active_ind=1)
  DETAIL
   loc1 = locateval(idx,1,ae_attend->enc_cnt,ei.encntr_id,ae_attend->qual[idx].encntr_id), ae_attend
   ->qual[loc1].pm_stream = uar_get_code_display(ei.value_cd)
  WITH nocounter, expand = 1
 ;end select
 SELECT INTO "nl:"
  FROM clinical_event ce
  PLAN (ce
   WHERE expand(idx,1,ae_attend->enc_cnt,ce.encntr_id,ae_attend->qual[idx].encntr_id)
  ...
  HEAD ce.encntr_id
   loc1 = locateval(idx,1,ae_attend->enc_cnt,ce.encntr_id,ae_attend->qual[idx].encntr_id), ae_attend
   ->qual[loc1].pf_stream = trim(ce.result_val,3)
  WITH nocounter, expand = 1
 ;end select
 SELECT INTO "nl:"
  FROM tracking_checkin tc,
   code_value cv
  PLAN (tc
   WHERE expand(idx,1,ae_attend->enc_cnt,tc.tracking_id,ae_attend->qual[idx].tracking_id)
    AND tc.active_ind=1)
   JOIN (cv
  ...
  DETAIL
   loc2 = locateval(idx,1,ae_attend->enc_cnt,tc.tracking_id,ae_attend->qual[idx].tracking_id)
   IF (tc.checkout_dt_tm=cnvtdatetime("31-DEC-2100 00:00"))
    ae_attend->qual[loc2].point_dt_tm = cnvtdatetime(curdate,curtime3)
   ELSE
    ae_attend->qual[loc2].point_dt_tm = tc.checkout_dt_tm
   ENDIF
  WITH nocounter, expand = 1
 ;end select
 SELECT INTO "nl:"
  FROM (dummyt d1  WITH seq = value(ae_attend->enc_cnt))
  DETAIL
   IF (textlen(trim(ae_attend->qual[d1.seq].pm_stream,3)) > 0
    AND textlen(trim(ae_attend->qual[d1.seq].pf_stream,3))=0)
    ae_attend->qual[d1.seq].stream = ae_attend->qual[d1.seq].pm_stream
   ELSEIF (textlen(trim(ae_attend->qual[d1.seq].pf_stream,3)) > 0
    AND textlen(trim(ae_attend->qual[d1.seq].pm_stream,3))=0)
    ae_attend->qual[d1.seq].stream = ae_attend->qual[d1.seq].pf_stream
   ENDIF
   IF (textlen(trim(ae_attend->qual[d1.seq].pm_stream,3)) > 0
    AND textlen(trim(ae_attend->qual[d1.seq].pf_stream,3)) > 0)
    CASE (stream_priority)
     OF "C":
      IF ((ae_attend->qual[d1.seq].point_dt_tm <= stream_opt_dt))
       ae_attend->qual[d1.seq].stream = ae_attend->qual[d1.seq].pf_stream
      ELSEIF ((ae_attend->qual[d1.seq].point_dt_tm > stream_opt_dt))
       ae_attend->qual[d1.seq].stream = ae_attend->qual[d1.seq].pm_stream
      ENDIF
     OF "P":
      IF ((ae_attend->qual[d1.seq].point_dt_tm <= stream_opt_dt))
 ...
 ;end select
 SELECT INTO  $OUTDEV
  stream = trim(ae_attend->qual[d1.seq].stream,3)
  FROM (dummyt d1  WITH seq = value(ae_attend->enc_cnt))
  ORDER BY stream
  HEAD REPORT
   encntr_tot = 0, head_line = concat("{CPI/10}{B}",trim(trust_name)," Daily A+E Attendances"),
   date_line = concat("{CPI/10}{B}From ",start_date," to ",end_date),
   col 0,
   CALL center(head_line,1,80), row + 1,
   col 0,
   CALL center(date_line,1,80)
   IF (textlen(trim(fac_disp,3)) > 0)
    row + 1, col 0, fac_line = concat("{CPI/10}{B}",trim(fac_disp,3)),
    CALL center(fac_line,1,80)
   ENDIF
   IF (textlen(trim(aeloc_disp,3)) > 0)
    row + 1, col 0, fac_line = concat("{CPI/10}{B}",trim(aeloc_disp,3)),
    CALL center(fac_line,1,80)
   ENDIF
   "{ENDB}{CPI/12}", row + 2, col 10,
   "{B}Stream", col 35, "Attendances",
   row + 2
  HEAD stream
   IF (textlen(trim(stream,3)) > 0)
    stream_tot = 0, col 10, stream
   ENDIF
  DETAIL
   stream_tot = (stream_tot+ 1)
  FOOT  stream
   IF (textlen(trim(stream,3)) > 0)
    col 32, stream_tot, row + 1,
    encntr_tot = (encntr_tot+ stream_tot)
   ENDIF
  FOOT REPORT
   row + 1, col 10, "{B}TOTAL:",
   col 40, encntr_tot"{ENDB}"
  WITH nocounter, format, separator = " ",
   dio = postscript, maxcol = 5000
 ;end select
 IF (curqual=0)
  CALL validationfailuremsg(value( $OUTDEV),"No data found for any qualifying A&E Streams.",
   log_level_info)
  GO TO exit_script
 ENDIF
#exit_program
