CREATE PROGRAM dm2_noupdt_seq_match_c1:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dfsm_c1_cnt = i4
 DECLARE v_my_current = f8
 DECLARE v_source_current = f8
 DECLARE v_target_current = f8
 DECLARE fm_seq_cnt = i4
 DECLARE fm_tbl_cnt = i4
 DECLARE fms_c1_err_cd = i2
 DECLARE dfsm_debug_cnt = i4
 DECLARE v_errmsg = vc
 DECLARE dfsm_c1_tbl_name = vc
 DECLARE dfsm_c1_col_name = vc
 DECLARE dfsm_c1_source_link = vc
 DECLARE dfsm_c1_chk_col_cnt = i4
 DECLARE col_loop_cnt = i4
 DECLARE v_low_val = f8
 DECLARE v_rtn_val = f8
 DECLARE dfsm_c2_match = i2
 DECLARE dm2_noupdt_flag = i2
 DECLARE dm2_sm_get_cols(dsgc_tbl_name=vc) = i2
 DECLARE sbr_dbl_seq_chk(sbr_max_val=f8,sbr_tbl_name=vc,sbr_col_name=vc,sbr_seq_cnt=i4,sbr_envir=vc)
  = i2
 FREE RECORD fsm_sql
 RECORD fsm_sql(
   1 sql_lst[*]
 ...
 SET fm_seq_cnt =  $1
 SET fm_tbl_cnt =  $2
 SET dfsm_c1_tbl_name = dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].tbl_lst[fm_tbl_cnt].tbl_name
 SET dfsm_c1_col_name = dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].tbl_lst[fm_tbl_cnt].tbl_col
 SET dfsm_c1_source_link = concat(dfsm_c1_tbl_name,"@",dfsm_seq_lst_rec->db_link)
 SET fms_c1_err_cd = 0
 SET v_my_current = - (1)
 SET v_source_current = - (1)
 SET v_target_current = - (1)
 IF (validate(rdds_noupdt_sequence_match,- (1)) > 0)
  SET dm2_noupdt_flag = 1
 ELSE
  SET dm2_noupdt_flag = 0
 ENDIF
 SET dm_err->eproc = "Selecting max value from target."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  max_value_found = max(parser(concat("vt.",dfsm_c1_col_name)))
  FROM (parser(dfsm_c1_tbl_name) vt)
  DETAIL
   v_target_current = max_value_found
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_program
 ENDIF
 IF ((v_target_current > dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].tgt_last_num))
  IF (sbr_dbl_seq_chk(v_target_current,dfsm_c1_tbl_name,dfsm_c1_col_name,fm_seq_cnt,"TARGET") < 1)
   IF ((dm_err->err_ind=1))
    GO TO end_program
   ENDIF
   INSERT  FROM dm_info d
    SET d.info_domain = "RDDS SKIP SEQMATCH", d.info_name = dfsm_c1_tbl_name, d.updt_cnt = 0,
     d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_task = 15301
    WITH nocounter
   ;end insert
   IF (check_error("Insert into SKIP row into dm_info for Target") != 0)
    ROLLBACK
   ENDIF
   GO TO end_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Selecting max value from source."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  max_value_found = max(parser(concat("vs.",dfsm_c1_col_name)))
  FROM (parser(dfsm_c1_source_link) vs)
  DETAIL
   v_source_current = max_value_found
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_program
 ENDIF
 IF ((v_source_current > dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].src_last_num))
  IF (sbr_dbl_seq_chk(v_source_current,dfsm_c1_tbl_name,dfsm_c1_col_name,fm_seq_cnt,"SOURCE") < 1)
   IF ((dm_err->err_ind=1))
    GO TO end_program
   ENDIF
   INSERT  FROM dm_info d
    ...
    WITH nocounter
   ;end insert
   IF (check_error("Insert into SKIP row into dm_info for Source") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ROLLBACK
   ENDIF
   GO TO end_program
  ENDIF
 ENDIF
 IF (v_source_current <= v_target_current)
  SET v_my_current = v_source_current
 ELSE
  SET v_my_current = v_target_current
 ENDIF
 IF ((dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].tgt_src_max_val > 0)
  AND (v_my_current > dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].tgt_src_max_val))
  SET v_my_current = dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].tgt_src_max_val
 ENDIF
 SET dm_err->eproc = concat("Target value: ",trim(cnvtstring(v_target_current)),"  Source value: ",
  trim(cnvtstring(v_source_current)),"  Lowest value between T&S: ",
  trim(cnvtstring(v_my_current)),"  Current Record value: ",trim(cnvtstring(dfsm_seq_lst_rec->
    seq_lst[fm_seq_cnt].info_number)))
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].info_number >= v_my_current))
  SET dm_err->eproc = concat("Current record value is greater than current from T & S: ",trim(
    cnvtstring(dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].info_number)))
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO end_program
 ENDIF
 IF (validate(drdm_mover_ind,- (1))=1)
  SET dm_err->eproc = "CALLED FROM DATA MOVER"
 ELSE
  SET dm_err->eproc = "CALLED STAND ALONE"
 ENDIF
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_sm_get_cols(dfsm_c1_tbl_name)=0)
  GO TO end_program
 ENDIF
 SET v_low_val = 0
 SET dfsm_c1_cnt = 0
 SET v_rtn_val = - (1)
 SET found_match = 0
 SET dfsm_c1_chk_col_cnt = 0
 SET stat = alterlist(fsm_sql->sql_lst,10)
 SET dfsm_c1_chk_col_cnt = 1
 SET fsm_sql->sql_lst[dfsm_c1_chk_col_cnt].sql_stmt = concat("SELECT into ",'"',"nl:",'"',
  " temp = MAX (t1.",
  dfsm_c1_col_name,")  ")
 SET dfsm_c1_chk_col_cnt = 2
 SET fsm_sql->sql_lst[dfsm_c1_chk_col_cnt].sql_stmt = concat(" FROM ",dfsm_c1_tbl_name," t1 ,",
  dfsm_c1_source_link," t2 ")
 SET dfsm_c1_chk_col_cnt = 3
 SET fsm_sql->sql_lst[dfsm_c1_chk_col_cnt].sql_stmt = "hold for dynamic where clause"
 FOR (col_loop_cnt = 1 TO dfsm_col_rec->col_cnt)
   IF ((dfsm_col_rec->col_qual[col_loop_cnt].data_type != "LONG")
    AND (dfsm_col_rec->col_qual[col_loop_cnt].data_type != "LONG RAW")
    AND (dfsm_col_rec->col_qual[col_loop_cnt].data_type != "BLOB")
    AND (dfsm_col_rec->col_qual[col_loop_cnt].data_type != "CLOB")
    AND (dfsm_col_rec->col_qual[col_loop_cnt].data_type != dfsm_c1_col_name))
    SET dfsm_c1_chk_col_cnt = (dfsm_c1_chk_col_cnt+ 1)
    IF (mod(dfsm_c1_chk_col_cnt,10)=1
     AND dfsm_c1_chk_col_cnt != 1)
     SET stat = alterlist(fsm_sql->sql_lst,(dfsm_c1_chk_col_cnt+ 9))
    ENDIF
    IF ((dfsm_col_rec->col_qual[col_loop_cnt].nullable="N"))
     SET fsm_sql->sql_lst[dfsm_c1_chk_col_cnt].sql_stmt = concat(" AND t1.",dfsm_col_rec->col_qual[
      col_loop_cnt].column_name," = t2.",dfsm_col_rec->col_qual[col_loop_cnt].column_name)
    ELSE
     SET fsm_sql->sql_lst[dfsm_c1_chk_col_cnt].sql_stmt = concat(" AND (t1.",dfsm_col_rec->col_qual[
      col_loop_cnt].column_name," = t2.",dfsm_col_rec->col_qual[col_loop_cnt].column_name)
     SET dfsm_c1_chk_col_cnt = (dfsm_c1_chk_col_cnt+ 1)
     IF (mod(dfsm_c1_chk_col_cnt,10)=1
      ...
      SET stat = alterlist(fsm_sql->sql_lst,(dfsm_c1_chk_col_cnt+ 9))
     ENDIF
     SET fsm_sql->sql_lst[dfsm_c1_chk_col_cnt].sql_stmt = concat(" OR (t1.",dfsm_col_rec->col_qual[
      col_loop_cnt].column_name," = NULL AND t2.",dfsm_col_rec->col_qual[col_loop_cnt].column_name,
      " = NULL)) ")
    ENDIF
   ENDIF
 ENDFOR
 ...
 IF (mod(dfsm_c1_chk_col_cnt,10)=1
  ...
  SET stat = alterlist(fsm_sql->sql_lst,(dfsm_c1_chk_col_cnt+ 9))
 ENDIF
 SET fsm_sql->sql_lst[dfsm_c1_chk_col_cnt].sql_stmt = concat(
  " DETAIL v_rtn_val = temp with nocounter go")
 SET stat = alterlist(fsm_sql->sql_lst,dfsm_c1_chk_col_cnt)
 IF ((dm_err->debug_flag > 0))
  FOR (dfsm_debug_cnt = 1 TO dfsm_c1_chk_col_cnt)
   SET dm_err->eproc = concat(trim(cnvtstring(dfsm_debug_cnt))," ",fsm_sql->sql_lst[dfsm_debug_cnt].
    sql_stmt)
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDFOR
 ENDIF
 WHILE (v_my_current > 0
  AND dfsm_c2_match=0)
   SET dfsm_c1_cnt = (dfsm_c1_cnt+ 1)
   SET dm_err->eproc = concat("Processing batch ",trim(cnvtstring(dfsm_c1_cnt))," for table ",
    dfsm_c1_tbl_name,"...")
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (v_my_current > v_batchsize)
    SET v_low_val = (v_my_current - v_batchsize)
   ELSE
    SET v_low_val = 0
   ENDIF
   IF ((v_low_val < dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].info_number))
    SET v_low_val = dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].info_number
   ENDIF
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = concat("MEM USAGE BEFORE CALL TO C2: ",cnvtstring(curmem))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   EXECUTE dm2_find_sequence_match_c2
   IF (fms_c1_err_cd=1)
    SET dm_err->emsg = concat("Error caught from C2 during parser transaction...",dfsm_c1_tbl_name)
    SET dm_err->err_ind = 1
    SET dm_err->eproc = v_errmsg
    SET dm_err->user_action = "Please check logfile for details."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO end_program
   ENDIF
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = concat("MEM USAGE AFTER CALL TO C2: ",cnvtstring(curmem))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SET v_my_current = v_low_val
   IF ((dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].info_number >= v_my_current))
    SET v_my_current = 0
   ENDIF
   IF (v_rtn_val > 0)
    SET dfsm_c2_match = 1
   ENDIF
   SET dm_err->eproc = concat("My new v_my_current value: ",trim(cnvtstring(v_my_current)),
    ". My return value: ",trim(cnvtstring(v_rtn_val)),". My current record value: ",
    trim(cnvtstring(dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].info_number)),". My dfsm_c2_match value: ",
    trim(cnvtstring(dfsm_c2_match)))
   CALL disp_msg(" ",dm_err->logfile,0)
 ENDWHILE
 IF ((dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].info_number > v_rtn_val))
  SET dm_err->eproc = concat("No higher match found on table: ",dfsm_c1_tbl_name,". Max = ",trim(
    cnvtstring(dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].info_number)))
  CALL disp_msg(" ",dm_err->logfile,0)
 ELSE
  SET dfsm_seq_lst_rec->seq_lst[fm_seq_cnt].info_number = v_rtn_val
  SET dm_err->eproc = concat("Return match value found and set as new match max: ",trim(cnvtstring(
     v_rtn_val))," for ",dfsm_c1_tbl_name)
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
#end_program
 FREE SET dfsm_c1_tbl_name
 FREE SET dfsm_c1_col_name
 FREE SET dfsm_c1_source_link
 FREE RECORD fsm_sql
 FREE RECORD dfsm_col_rec
 SUBROUTINE dm2_sm_get_cols(dsgc_tbl_name)
   DECLARE dsgc_rtn_status = i2 WITH protect, noconstant(1)
   DECLARE dsgc_source_link = vc WITH protect
   DECLARE idx = i4 WITH protect
   DECLARE idx2 = i4 WITH protect
   SET dsgc_source_link = concat("user_tab_cols@",dfsm_seq_lst_rec->db_link)
   SELECT
    IF (dm2_noupdt_flag=1)INTO "nl:"
     uttgt.column_name, uttgt.data_type, uttgt.nullable
     FROM user_tab_cols uttgt,
      (parser(dsgc_source_link) utsrc)
     WHERE uttgt.table_name=dsgc_tbl_name
      AND uttgt.hidden_column="NO"
      AND uttgt.virtual_column="NO"
      AND  NOT ( EXISTS (
     (SELECT
      "x"
      FROM dm_info di
      WHERE di.info_domain="RDDS IGNORE COL LIST:*"
       AND sqlpassthru(" uttgt.column_name like di.info_name and uttgt.table_name like di.info_char")
     )))
      AND uttgt.column_name=utsrc.column_name
      AND utsrc.table_name=dsgc_tbl_name
      AND utsrc.column_name != "UPDT*"
      AND utsrc.hidden_column="NO"
      AND utsrc.virtual_column="NO"
    ELSE INTO "nl:"
     uttgt.column_name, uttgt.data_type, uttgt.nullable
     FROM user_tab_cols uttgt,
      (parser(dsgc_source_link) utsrc)
     ...
     (SELECT
      ...
      FROM dm_info di
      WHERE di.info_domain="RDDS IGNORE COL LIST:*"
       AND sqlpassthru(" di.info_name like uttgt.column_name and uttgt.table_name like di.info_char")
     )))
      AND uttgt.column_name=utsrc.column_name
      AND utsrc.table_name=dsgc_tbl_name
      AND utsrc.hidden_column="NO"
    ...
    ENDIF
    HEAD REPORT
     stat = alterlist(dfsm_col_rec->col_qual,10), dfsm_col_rec->col_cnt = 0
    DETAIL
     dfsm_col_rec->col_cnt = (dfsm_col_rec->col_cnt+ 1)
     IF (mod(dfsm_col_rec->col_cnt,10)=1
      AND (dfsm_col_rec->col_cnt != 1))
      stat = alterlist(dfsm_col_rec->col_qual,(dfsm_col_rec->col_cnt+ 9))
     ENDIF
     dfsm_col_rec->col_qual[dfsm_col_rec->col_cnt].column_name = uttgt.column_name, dfsm_col_rec->
     col_qual[dfsm_col_rec->col_cnt].data_type = uttgt.data_type, dfsm_col_rec->col_qual[dfsm_col_rec
     ->col_cnt].nullable = "Y"
    FOOT REPORT
     stat = alterlist(dfsm_col_rec->col_qual,dfsm_col_rec->col_cnt)
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    FROM dm2_user_notnull_cols dunc
    WHERE dunc.table_name=dsgc_tbl_name
     AND expand(idx,1,size(dfsm_col_rec->col_qual,5),dunc.column_name,dfsm_col_rec->col_qual[idx].
     column_name)
    DETAIL
     idx2 = 0, idx2 = locateval(idx,1,size(dfsm_col_rec->col_qual,5),dunc.column_name,dfsm_col_rec->
      col_qual[idx].column_name)
     IF (idx2 > 0)
      dfsm_col_rec->col_qual[idx2].nullable = "N"
     ENDIF
    WITH nocounter
   ;end select
   SET dm_err->eproc = concat("Retrieving and loading column data for table ",dsgc_tbl_name)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dsgc_rtn_status = 0
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dfsm_col_rec)
   ENDIF
   RETURN(dsgc_rtn_status)
 END ;Subroutine
 SUBROUTINE sbr_dbl_seq_chk(sbr_max_val,sbr_tbl_name,sbr_col_name,sbr_seq_cnt,sbr_envir)
   DECLARE dsc_rtn_status = i2 WITH protect, noconstant(1)
   DECLARE dsc_seq_tbl = vc WITH protect
   DECLARE dsc_seq_num = f8 WITH protect
   DECLARE dsc_seq_last_num = vc WITH protect, noconstant("")
   DECLARE dsc_env_id = vc WITH protect, noconstant("")
   IF (sbr_envir="SOURCE")
    SET dsc_seq_tbl = concat("dm2_user_sequences@",dfsm_seq_lst_rec->db_link)
    SET dsc_seq_last_num = trim(cnvtstring(dfsm_seq_lst_rec->seq_lst[sbr_seq_cnt].src_last_num))
    SET dsc_env_id = trim(cnvtstring(dfsm_seq_lst_rec->source))
   ELSE
    SET dsc_seq_tbl = "dm2_user_sequences"
    SET dsc_seq_last_num = trim(cnvtstring(dfsm_seq_lst_rec->seq_lst[sbr_seq_cnt].tgt_last_num))
    SET dsc_env_id = trim(cnvtstring(dfsm_seq_lst_rec->target))
   ENDIF
   SELECT INTO "nl:"
    dus.last_number
    FROM (parser(dsc_seq_tbl) dus)
    WHERE (dus.sequence_name=dfsm_seq_lst_rec->seq_lst[sbr_seq_cnt].seq_name)
     AND dus.last_number >= sbr_max_val
    DETAIL
     dsc_seq_num = dus.last_number
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dsc_rtn_status = - (1)
   ELSEIF (curqual=0)
    SET dm_err->eproc = concat("Max val: ",trim(cnvtstring(sbr_max_val))," for ",sbr_tbl_name,".",
     sbr_col_name," > sequence val: ",dsc_seq_last_num)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dsc_rtn_status = 0
   ELSE
    IF (sbr_envir="SOURCE")
     SET dfsm_seq_lst_rec->seq_lst[sbr_seq_cnt].src_last_num = dsc_seq_num
    ELSE
     SET dfsm_seq_lst_rec->seq_lst[sbr_seq_cnt].tgt_last_num = dsc_seq_num
    ENDIF
    SET dsc_rtn_status = 1
   ENDIF
   RETURN(dsc_rtn_status)
 END ;Subroutine
END GO
