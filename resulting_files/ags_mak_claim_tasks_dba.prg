CREATE PROGRAM ags_mak_claim_tasks:dba
 CALL echo("***")
 CALL echo("***   BEG AGS_MAK_CLAIM_TASKS")
 CALL echo("***")
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_MAK_CLAIM_TASKS"
 CALL echo("***")
 CALL echo("***   Find CLAIM Batch Id Values")
 CALL echo("***")
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 SELECT INTO "nl:"
  little_id = min(o.ags_claim_data_id), big_id = max(o.ags_claim_data_id)
 ...
 IF (ierrcode > 0)
  ...
  SET table_name = "GET CLAIM BATCH_ID VALUES"
  SET ilog_status = 1
  SET log->qual_knt = (log->qual_knt+ 1)
  SET stat = alterlist(log->qual,log->qual_knt)
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("GET CLAIM BATCH_ID VALUES :: Select Error :: ",trim(
    serrmsg))
  SET serrmsg = log->qual[log->qual_knt].smsg
  GO TO exit_script
 ENDIF
 CALL echo("***")
 CALL echo(build("***   AGS_CLAIM_DATA min_batch_id :",min_batch_id))
 CALL echo(build("***   AGS_CLAIM_DATA max_batch_id :",max_batch_id))
 CALL echo("***")
 CALL echo("***")
 CALL echo("***   Create CLAIM Tasks")
 CALL echo("***")
 WHILE (min_batch_id <= max_batch_id
  AND min_batch_id >= 1)
   ...
   SET item->qual[item->qual_knt].task_type = "CLAIM"
   ...
   SET item->qual[item->qual_knt].batch_program = "ags_claim_load"
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   SELECT INTO "nl:"
 ...
 IF ((item->qual_knt < 1))
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "No CLAIM tasks found"
  GO TO exit_script
 ENDIF
 CALL echo("***")
 ...
 COMMIT
 CALL echo("***")
 CALL echo("***   Update CLAIM Data to WAITING")
 CALL echo("***")
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 ...
 IF (ierrcode > 0)
  ...
  SET table_name = "AGS_CLAIM_DATA"
  SET ilog_status = 1
  SET log->qual_knt = (log->qual_knt+ 1)
  SET stat = alterlist(log->qual,log->qual_knt)
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE AGS_CLAIM_DATA ITEMS :: Update Error :: ",trim(
    serrmsg))
  SET serrmsg = log->qual[log->qual_knt].smsg
  GO TO exit_script
 ENDIF
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_MAK_CLAIM_TASKS"
 CALL echo("***")
 CALL echo("***   END AGS_MAK_CLAIM_TASKS")
 CALL echo("***")
