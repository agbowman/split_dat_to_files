CREATE PROGRAM dm2_mv_drop_bytable:dba
 ...
 SUBROUTINE dmvr_drop_mlog(ddm_bt_name)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmdb_base_table = vc WITH protect, noconstant(" ")
 DECLARE dmdb_mlog_exists = i2 WITH protect, noconstant(0)
 DECLARE dmdb_iter = i2 WITH protect, noconstant(0)
 DECLARE dmdb_mv_name = vc WITH protect, noconstant(" ")
 DECLARE dmdb_mlog_table = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm2_mv_dropbytab",".log","dm2_mv_drop_bytable LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dmdb_base_table = trim(cnvtupper( $1))
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg = "Parameter usage: dm2_mv_drop_bytable '<table_name>'"
 ...
 IF ((dm2_rdbms_version->level1 < 9))
  SET dm_err->eproc = "Materialized Views drops not needed on Oracle 8.  Exiting..."
  CALL disp_msg(" ",dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (dmvr_get_mvs_for_bt(dmdb_base_table)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(btmv)
 ENDIF
 FOR (dmdb_iter = 1 TO btmv->mv_cnt)
   SET dmdb_mv_name = btmv->mv[dmdb_iter].name
   EXECUTE dm2_mv_drop value(dmdb_mv_name)
   IF ((dm_err->err_ind=1))
    GO TO exit_program
   ENDIF
 ENDFOR
 FOR (dmdb_iter = 1 TO btmv->mvl_cnt)
  SET dmdb_mlog_table = btmv->mvl[dmdb_iter].table_name
  IF (dmvr_drop_mlog(dmdb_mlog_table)=0)
   GO TO exit_program
  ENDIF
 ENDFOR
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "DM2_MV_DROP_BYTABLE completed"
 CALL final_disp_msg("dm2_mv_dropbytab")
END GO
