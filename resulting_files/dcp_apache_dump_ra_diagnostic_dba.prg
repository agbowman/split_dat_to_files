CREATE PROGRAM dcp_apache_dump_ra_diagnostic:dba
...
#3000_load_dump_path
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dump_path = di.info_char, update_count = di.updt_cnt
  WITH nocounter
 ;end select
 CALL echo(build("date last dumped = ",last_dt_tm_dumped))
 IF (((cursys="VMS") OR (cursys="AXP")) )
  SET tmp_ra_dump_file_name = build(dump_path,"TMP_APACHE_RA_DUMP_DIAG")
  SET tmp_rad_dump_file_name = build(dump_path,"TMP_APACHE_RAD_DUMP_DIAG")
  SET tmp_rao_dump_file_name = build(dump_path,"TMP_APACHE_RAO_DUMP_DIAG")
  SET tmp_rat_dump_file_name = build(dump_path,"TMP_APACHE_RAT_DUMP_DIAG")
  SET tmp_rae_dump_file_name = build(dump_path,"TMP_APACHE_RAE_DUMP_DIAG")
  SET tmp_full_ra_dump_file_name = build(dump_path,"TMP_APACHE_RA_DUMP_DIAG.DAT")
  SET tmp_full_rad_dump_file_name = build(dump_path,"TMP_APACHE_RAD_DUMP_DIAG.DAT")
  SET tmp_full_rao_dump_file_name = build(dump_path,"TMP_APACHE_RAO_DUMP_DIAG.DAT")
  SET tmp_full_rat_dump_file_name = build(dump_path,"TMP_APACHE_RAT_DUMP_DIAG.DAT")
  SET tmp_full_rae_dump_file_name = build(dump_path,"TMP_APACHE_RAE_DUMP_DIAG.DAT")
  SET full_ra_dump_file_name = build(dump_path,"APACHE_RA_DUMP_DIAG.DAT")
  SET full_rad_dump_file_name = build(dump_path,"APACHE_RAD_DUMP_DIAG.DAT")
  SET full_rao_dump_file_name = build(dump_path,"APACHE_RAO_DUMP_DIAG.DAT")
  SET full_rat_dump_file_name = build(dump_path,"APACHE_RAT_DUMP_DIAG.DAT")
  SET full_rae_dump_file_name = build(dump_path,"APACHE_RAE_DUMP_DIAG.DAT")
 ELSE
  SET tmp_ra_dump_file_name = build(dump_path,"/tmp_apache_ra_dump_diag")
  SET tmp_rad_dump_file_name = build(dump_path,"/tmp_apache_rad_dump_diag")
  SET tmp_rao_dump_file_name = build(dump_path,"/tmp_apache_rao_dump_diag")
...
#4000_build_pat_list
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = (cnt+ 1), stat = alterlist(pat_list->pat_data,cnt), pat_list->pat_data[cnt].active_ind = ra
   .active_ind,
   pat_list->pat_data[cnt].risk_adjustment_id = ra.risk_adjustment_id, pat_list->pat_data[cnt].
   encntr_id = ra.encntr_id, pat_list->pat_data[cnt].active_status_dt_tm = ra.active_status_dt_tm,
   pat_list->pat_data[cnt].active_status_prsnl_id = ra.active_status_prsnl_id, pat_list->pat_data[cnt
   ].adm_doc_id = ra.adm_doc_id, pat_list->pat_data[cnt].admit_age = ra.admit_age,
   pat_list->pat_data[cnt].admit_diagnosis = ra.admit_diagnosis, pat_list->pat_data[cnt].
   el_loc_nurse_unit_cd = uar_get_code_display(ra.admit_icu_cd), pat_list->pat_data[cnt].admit_source
    = ra.admit_source,
   pat_list->pat_data[cnt].admitsource_flag = ra.admitsource_flag, pat_list->pat_data[cnt].aids_ind
    = ra.aids_ind, pat_list->pat_data[cnt].ami_location = ra.ami_location,
   pat_list->pat_data[cnt].bed_count = ra.bed_count, pat_list->pat_data[cnt].body_system = ra
   .body_system, pat_list->pat_data[cnt].cc_during_stay_ind = ra.cc_during_stay_ind,
   pat_list->pat_data[cnt].chronic_health_none_ind = ra.chronic_health_none_ind, pat_list->pat_data[
   cnt].chronic_health_unavail_ind = ra.chronic_health_unavail_ind, pat_list->pat_data[cnt].
   cirrhosis_ind = ra.cirrhosis_ind,
   pat_list->pat_data[cnt].copd_flag = ra.copd_flag, pat_list->pat_data[cnt].copd_ind = ra.copd_ind,
   pat_list->pat_data[cnt].diabetes_ind = ra.diabetes_ind,
   pat_list->pat_data[cnt].dialysis_ind = ra.dialysis_ind, pat_list->pat_data[cnt].diedinicu_ind = ra
   .diedinicu_ind
   ...
   pat_list->pat_data[cnt].pat_name_last = p.name_last_key
  FOOT REPORT
   pat_list->cnt = cnt
  WITH nocounter
 ;end select
 IF ((pat_list->cnt > 0))
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (ea
    WHERE (ea.encntr_id=pat_list->pat_data[d.seq].encntr_id)
     AND ea.encntr_alias_type_cd=fin_cd
     AND ea.active_ind=1)
   ORDER BY d.seq
   DETAIL
    pat_list->pat_data[d.seq].fin_nbr = ea.alias
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = pat_list->cnt),
 ...
 SELECT INTO value(tmp_ra_dump_file_name)
  ...
  HEAD REPORT
   ...
   out_row = build(out_row,
    ...
    out_row,"PAT_NAME_FIRST|PAT_NAME_LAST|DOC_NAME_FIRST|DOC_NAME_LAST"), out_row
  DETAIL
   ...
   out_row = build(out_row,pat_list->pat_data[d.seq].pat_name_first,"|"), out_row = build(out_row,
    ...
    doc_name_first,"|"),
   out_row = build(out_row,pat_list->pat_data[d.seq].doc_name_last), out_row
   ...
   out_row = build(out_row,trim(format(rat.risk_adj_tiss_id,"#################"),3),"|",rat
    .active_status_cd,"|"), out_row = build(out_row,format(rat.active_status_dt_tm,
     "DD-MMM-YYYY HH:MM:SS;;D"),"|"), out_row = build(out_row,trim(format(rat.active_status_prsnl_id,
      "#################"),3),"|"),
   out_row = build(out_row,format(rat.tiss_beg_dt_tm,"DD-MMM-YYYY HH:MM:SS;;D"),"|",rat.tiss_cd,"|"),
   out_row = build(out_row,format(rat.tiss_end_dt_tm,"DD-MMM-YYYY HH:MM:SS;;D"),"|",rat.updt_applctx,
    "|",
    rat.updt_cnt,"|"), out_row = build(out_row,format(rat.updt_dt_tm,"DD-MMM-YYYY HH:MM:SS;;D"),"|"),
   out_row = build(out_row,trim(format(rat.updt_id,"#################"),3)), out_row = build(out_row,
    "|"), out_row = build(out_row,rat.updt_task),
   out_row
  WITH maxcol = 6000, formfeed = none, format = variable,
   maxrow = 1
 ;end select
