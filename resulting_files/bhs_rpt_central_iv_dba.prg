CREATE PROGRAM bhs_rpt_central_iv:dba
 PROMPT
 ...
 ELSE
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="BHS_RPT_CENTRAL_IV*"
    AND di.info_char=cnvtupper(ms_facility)
   DETAIL
 ...
 SELECT INTO value(concat("bhscust:",ms_email_filename))
  ...
  HEAD REPORT
   ...
   ms_line
  DETAIL
   pos = 0, ms_line = concat('"',trim(m_ivs->ivs[d.seq].s_patient_name),'",',trim(m_ivs->ivs[d.seq].
     s_loc_unit),";"), ms_line = concat(ms_line,trim(m_ivs->ivs[d.seq].s_loc_room),";",trim(m_ivs->
     ivs[d.seq].s_loc_bed),","),
   ms_line = concat(ms_line,trim(m_ivs->ivs[d.seq].s_mrn_nbr),",",trim(m_ivs->ivs[d.seq].s_fin_nbr),
    ","), ms_line = concat(ms_line,'"',trim(m_ivs->ivs[d.seq].s_catheter_type),'","',trim(m_ivs->ivs[
     d.seq].s_start_dt_tm),
...
#exit_script
 ...
 FREE RECORD m_ivs
 FREE RECORD m_refs
 SET last_mod = "* 005 06-06-17 SH013356 416542599		added an email recipient for the facility noted"
 CALL echo(last_mod)
END GO
