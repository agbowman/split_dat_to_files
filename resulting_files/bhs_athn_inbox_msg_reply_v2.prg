CREATE PROGRAM bhs_athn_inbox_msg_reply_v2
 FREE RECORD result
 RECORD result(
   1 addendum = vc
   1 subject = vc
 ...
 ELSEIF (( $3 <= 0.0))
  ...
  GO TO exit_script
 ENDIF
 SET result->pool_ind = evaluate( $27,0.0,0,1)
 ...
 SET result->task_uid = concat("urn:cerner:mid:object.task:",trim(cnvtlower(curdomain),3),":taskId=",
  ...
  evaluate(result->pool_ind,1,trim(replace(cnvtstring( $27),".00","",0),3),trim(replace(cnvtstring(
 ...
 IF (size(result->to_pool,5) > 0)
  ...
  ;end select
 ENDIF
 IF (((size(result->to_prsnl,5) > 0) OR (size(result->to_pool,5))) )
...
#exit_script
 ...
 SUBROUTINE calladdnotification(null)
   ...
   SET req967503->message_list[1].msg_sender_pool_id =  $27
 ...
 SUBROUTINE calladdreminder(null)
   ...
   SET req967731->action_pool_id =  $27
