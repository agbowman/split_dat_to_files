CREATE PROGRAM bhs_eks_updt_covid_charge:dba
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   knt = 0
  DETAIL
   eksopsrequest->expert_trigger = "BHS_EKS_UPDT_COVID_CHARGE", knt += 1, stat = alterlist(
    eksopsrequest->qual,knt),
   eksopsrequest->qual[knt].person_id = e.person_id, eksopsrequest->qual[knt].encntr_id = e.encntr_id,
   eksopsrequest->qual[knt].order_id = 0.00
  WITH nocounter
 ;end select
