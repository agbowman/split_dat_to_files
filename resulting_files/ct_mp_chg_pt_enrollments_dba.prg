CREATE PROGRAM ct_mp_chg_pt_enrollments:dba
 PROMPT
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "CT_MP_CHG_PT_ENROLLMENTS"
 ...
 SET updatecon = false
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_chg_pt_enrollments","Invalid JSON Request",
   "reply")
  GO TO exit_script
 ENDIF
 FREE RECORD request
 ...
 FOR (i = 1 TO numofupdts)
   ...
   IF (schema_exists=2)
    SELECT INTO "nl:"
     ...
     DETAIL
      ...
      r->end_effective_dt_tm = p_pr_r.end_effective_dt_tm, r->nomenclature_id = p_pr_r
      ...
      r->off_tx_reason_cd = p_pr_r.reason_off_tx_cd, r->off_tx_reason_desc = p_pr_r
      .reason_off_tx_desc, r->off_tx_removal_org_id = p_pr_r.off_tx_removal_organization_id,
      r->off_tx_removal_person_id = p_pr_r.off_tx_removal_person_id, r->episode_id = p_pr_r
      .episode_id, r->on_tx_organization_id = p_pr_r.on_tx_organization_id,
      r->on_tx_assign_prsnl_id = p_pr_r.on_tx_assign_prsnl_id, r->on_tx_comment = p_pr_r
      .on_tx_comment, r->status_enum = p_pr_r.status_enum
   ...
   IF (curqual=1)
    ...
    SET list_count += 1
    SET stat = alterlist(audit_event->list,list_count)
    SET audit_event->list[list_count].participant_name = concat(registration_id," ",lst_updt_dt_tm,
     " (UPDT_DT_TM)")
    SET audit_event->list[list_count].person_id = r->person_id
    SET stat = alterlist(audit_event->list[list_count].events,0)
    SET event_ind = 0
    ...
    IF (doupdate=true)
     ...
     IF ((request->es[i].removalorgid != r->removal_organization_id))
      ...
      IF ((r->removal_organization_id != 0))
       SET event_ind += 1
       SET stat = alterlist(audit_event->list[list_count].events,event_ind)
       SET audit_event->list[list_count].events[event_ind].event_name = "OS_Removal_Institute"
       SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
      ENDIF
     ENDIF
     IF ((request->es[i].removalperid != r->removal_person_id))
      ...
      IF ((r->removal_person_id != 0))
       SET event_ind += 1
       SET stat = alterlist(audit_event->list[list_count].events,event_ind)
       SET audit_event->list[list_count].events[event_ind].event_name = "OS_Removal_person"
       SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
      ENDIF
     ENDIF
     IF ((request->es[i].dateonstudy != "0"))
      IF (datetimecmp(cnvtdatetime(request->es[i].dateonstudy),r->on_study_dt_tm) != 0)
       CALL echo("on study date changed")
       SET updatereg = true
       SET event_ind += 1
       SET stat = alterlist(audit_event->list[list_count].events,event_ind)
       SET audit_event->list[list_count].events[event_ind].event_name = "On_Study_Modify"
       SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
       SELECT INTO "nl:"
     ...
     ENDIF
     IF (datetimecmp(cnvtdatetime(request->es[i].dateoffstudy),r->off_study_dt_tm) != 0)
      CALL echo("off study date change")
      SET event_ind += 1
      SET stat = alterlist(audit_event->list[list_count].events,event_ind)
      IF ((r->off_study_dt_tm != cnvtdatetime("31-DEC-2100 00:00:00.00")))
       IF (datetimecmp(cnvtdatetime(request->es[i].dateoffstudy),cnvtdatetime(
         "31-DEC-2100 00:00:00.00")) != 0)
        SET audit_event->list[list_count].events[event_ind].event_name = "Off_Study_Modify"
        SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
       ELSE
        SET audit_event->list[list_count].events[event_ind].event_name = "Off_Study_Delete"
        SET audit_event->list[list_count].events[event_ind].event_type = "Delete"
       ENDIF
      ELSE
       SET audit_event->list[list_count].events[event_ind].event_name = "Off_Study_Add"
       SET audit_event->list[list_count].events[event_ind].event_type = "Add"
      ENDIF
     ...
     IF (schema_exists=2)
      IF ((((request->es[i].dateontherapy != "0")
       AND datetimecmp(cnvtdatetime(request->es[i].dateontherapy),r->tx_start_dt_tm) != 0) OR ((((
      request->es[i].ontxorgid != r->on_tx_organization_id)) OR ((((request->es[i].ontxperid != r->
      on_tx_assign_prsnl_id)) OR ((request->es[i].ontxcomment != r->on_tx_comment))) )) )) )
       SET updatereg = true
      ENDIF
      IF (datetimecmp(cnvtdatetime(request->es[i].dateontherapy),r->tx_start_dt_tm) != 0)
       SET event_ind += 1
       SET stat = alterlist(audit_event->list[list_count].events,event_ind)
       IF ((r->tx_start_dt_tm != cnvtdatetime("31-DEC-2100 00:00:00.00")))
        IF (datetimecmp(cnvtdatetime(request->es[i].dateontherapy),cnvtdatetime(
          "31-DEC-2100 00:00:00.00")) != 0)
         SET audit_event->list[list_count].events[event_ind].event_name = "On_Treatment_Date_Modify"
         SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
        ELSE
         SET audit_event->list[list_count].events[event_ind].event_name = "On_Treatment_Delete"
         SET audit_event->list[list_count].events[event_ind].event_type = "Delete"
        ENDIF
       ELSE
        SET audit_event->list[list_count].events[event_ind].event_name = "On_Treatment_Add"
        SET audit_event->list[list_count].events[event_ind].event_type = "Add"
       ENDIF
      ENDIF
      IF ((request->es[i].ontxperid != r->on_tx_assign_prsnl_id)
       AND (request->es[i].ontxperid != 0)
       AND (r->on_tx_assign_prsnl_id != 0))
       SET event_ind += 1
       SET stat = alterlist(audit_event->list[list_count].events,event_ind)
       SET audit_event->list[list_count].events[event_ind].event_name = "On_Treatment_Person_Modify"
       SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
      ENDIF
      IF ((request->es[i].ontxorgid != r->on_tx_organization_id)
       AND (request->es[i].ontxorgid != 0)
       AND (r->on_tx_organization_id != 0))
       SET event_ind += 1
       SET stat = alterlist(audit_event->list[list_count].events,event_ind)
       SET audit_event->list[list_count].events[event_ind].event_name =
       "On_Treatment_Institute_Modify"
       SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
      ENDIF
     ENDIF
     IF (datetimecmp(cnvtdatetime(request->es[i].dateofftherapy),r->tx_completion_dt_tm) != 0)
      SET event_ind += 1
      SET stat = alterlist(audit_event->list[list_count].events,event_ind)
      IF ((r->tx_completion_dt_tm != cnvtdatetime("31-DEC-2100 00:00:00.00")))
       IF (datetimecmp(cnvtdatetime(request->es[i].dateofftherapy),cnvtdatetime(
         "31-DEC-2100 00:00:00.00")) != 0)
        SET audit_event->list[list_count].events[event_ind].event_name = "Off_Treatment_Modify"
        SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
       ELSE
        SET audit_event->list[list_count].events[event_ind].event_name = "Off_Treatment_Delete"
        SET audit_event->list[list_count].events[event_ind].event_type = "Delete"
       ENDIF
      ELSE
       SET audit_event->list[list_count].events[event_ind].event_name = "Off_Treatment_Add"
       SET audit_event->list[list_count].events[event_ind].event_type = "Add"
      ENDIF
      SET updatereg = true
     ENDIF
     IF ((request->es[i].datefirstpdfail != "0"))
      IF ((cnvtdatetime(request->es[i].datefirstpdfail) != r->first_pd_failure_dt_tm))
       SET updatereg = true
      ENDIF
     ELSE
     ...
     IF ((request->es[i].enrollingorgid != r->enrolling_organization_id))
      ...
      SET event_ind += 1
      SET stat = alterlist(audit_event->list[list_count].events,event_ind)
      SET audit_event->list[list_count].events[event_ind].event_name = "Enrolling_Institute"
      SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
     ELSE
     ...
     IF ((request->es[i].datefirstpd != "0"))
      IF ((cnvtdatetime(request->es[i].datefirstpd) != r->first_pd_dt_tm))
       SET updatereg = true
      ENDIF
     ELSE
     ...
     IF ((request->es[i].datefirstcr != "0"))
      IF ((cnvtdatetime(request->es[i].datefirstcr) != r->first_pd_dt_tm))
       SET updatereg = true
      ENDIF
     ELSE
     ...
     ELSEIF (((textlen(request->es[i].protaccessionnbr) > 0) OR ((request->es[i].protaccessionnbr=" "
     ))) )
      IF ((request->es[i].protaccessionnbr != r->prot_accession_nbr))
       ...
       ELSE
        ...
        SET event_ind += 1
        SET stat = alterlist(audit_event->list[list_count].events,event_ind)
        SET audit_event->list[list_count].events[event_ind].event_name = "Enrollment_ID_Modify"
        SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
       ENDIF
      ELSE
     ...
     IF ((request->es[i].removaltxorgid != r->off_tx_removal_org_id))
      ...
      IF ((r->off_tx_removal_org_id != 0))
       SET event_ind += 1
       SET stat = alterlist(audit_event->list[list_count].events,event_ind)
       SET audit_event->list[list_count].events[event_ind].event_name = "OT_Removal_Institute"
       SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
      ENDIF
     ENDIF
     IF ((request->es[i].removaltxperid != r->off_tx_removal_person_id))
      ...
      IF ((r->off_tx_removal_person_id != 0))
       SET event_ind += 1
       SET stat = alterlist(audit_event->list[list_count].events,event_ind)
       SET audit_event->list[list_count].events[event_ind].event_name = "OT_Removal_person"
       SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
      ENDIF
     ENDIF
     IF ((request->es[i].removalreasoncd != r->removal_reason_cd))
      ...
      SET event_ind += 1
      SET stat = alterlist(audit_event->list[list_count].events,event_ind)
      SET audit_event->list[list_count].events[event_ind].event_name = "OS_Removal_Reason"
      SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
     ENDIF
     ...
     IF ((request->es[i].offtxremovalreasoncd != r->off_tx_reason_cd))
      ...
      SET event_ind += 1
      SET stat = alterlist(audit_event->list[list_count].events,event_ind)
      SET audit_event->list[list_count].events[event_ind].event_name = "OT_Removal_Reason"
      SET audit_event->list[list_count].events[event_ind].event_type = "Modify"
     ENDIF
     ...
     IF (schema_exists=2)
      INSERT  FROM pt_prot_reg p_pr_r
       SET p_pr_r.person_id = r->person_id, p_pr_r.nomenclature_id = request->es[i].nomenclatureid,
        ...
        p_pr_r.tx_start_dt_tm = cnvtdatetime(request->es[i].dateontherapy), p_pr_r
        .tx_completion_dt_tm = cnvtdatetime(request->es[i].dateofftherapy), p_pr_r
        .first_pd_failure_dt_tm = cnvtdatetime(request->es[i].datefirstpdfail),
     ...
     ELSE
      INSERT  FROM pt_prot_reg p_pr_r
       SET p_pr_r.person_id = r->person_id, p_pr_r.nomenclature_id = request->es[i].nomenclatureid,
        ...
        .off_tx_removal_organization_id = request->es[i].removaltxorgid, p_pr_r
        .off_tx_removal_person_id = request->es[i].removaltxperid,
        p_pr_r.episode_id = r->episode_id
       WITH nocounter
      ;end insert
     ENDIF
     IF (curqual=0)
 ...
 IF ((reply->status_data.status="S"))
  CALL echorecord(audit_event,"RECORDRESULTSAUDIT",1)
  FOR (x = 1 TO list_count)
    FOR (y = 1 TO event_ind)
      CASE (audit_event->list[x].events[y].event_type)
       OF "Add":
        EXECUTE cclaudit audit_mode, audit_event->list[x].events[y].event_name, audit_event->list[x].
        events[y].event_type,
        "Person", "Patient", "Patient",
        "Origination", audit_event->list[x].person_id, ""
       OF "Modify":
        EXECUTE cclaudit audit_mode, audit_event->list[x].events[y].event_name, audit_event->list[x].
        events[y].event_type,
        "Person", "Patient", "Patient",
        "Amendment", audit_event->list[x].person_id, audit_event->list[x].participant_name
       OF "Delete":
        EXECUTE cclaudit audit_mode, audit_event->list[x].events[y].event_name, audit_event->list[x].
        events[y].event_type,
        "Person", "Patient", "Patient",
        "Destruction", audit_event->list[x].person_id, audit_event->list[x].participant_name
      ENDCASE
    ENDFOR
  ENDFOR
 ENDIF
