CREATE PROGRAM bhs_rpt_amb_downtime:dba
 PROMPT
  ...
  facility, unit
 IF (datetimediff(cnvtdatetime(cnvtdate( $END_DT),0),cnvtdatetime(cnvtdate( $BEG_DT),0)) > 1)
  SET var_output = "MINE"
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
    d0 = date_outofrange(rpt_render), d0 = finalizereport(var_output)
   WITH nocounter
  ;end select
  GO TO exit_prg
 ...
 IF (validate(request->batch_selection))
  ...
  SET var_output = concat(trim(tempname),"_")
  ...
  CALL echo(build("time1=",time1))
  CALL echo(build("month1=",month1))
  SET var_output = build(var_output,month1,"_",day1,".ps")
  CALL echo(var_output)
 ELSE
 ...
 SELECT INTO "nl:"
  ...
  FROM sch_appt sa,
   ...
   sch_appt sa1
  PLAN (sa
   WHERE sa.beg_dt_tm BETWEEN cnvtdatetime( $BEG_DT) AND (cnvtdatetime( $END_DT)+ 0)
 ...
 SELECT INTO "nl"
  ...
  HEAD p.problem_id
   IF (((n.source_string > " ") OR (p.problem_ftdesc > " "))
    ...
    ENDIF
    dlrec->seq[d1.seq].problem[prob_cnt].status = uar_get_code_display(p.life_cycle_status_cd), dlrec
    ->seq[d1.seq].problem[prob_cnt].beg_effective_dt_tm = substring(1,14,format(p.beg_effective_dt_tm,
      "@SHORTDATE;;Q")), dlrec->seq[d1.seq].problem[prob_cnt].full_text = build(dlrec->seq[d1.seq].
     problem[prob_cnt].status,": ",dlrec->seq[d1.seq].problem[prob_cnt].text)
   ENDIF
 ...
 SELECT INTO "nl:"
  ...
  HEAD od.detail_sequence
   ...
   ELSEIF (od.oe_field_meaning="DOSEUNIT")
    CALL echo(build2("9 med select cnt:",trim(build2(cnt),3))), dlrec->seq[d1.seq].meds[cnt].doseunit
     = trim(od.oe_field_display_value), dlrec->seq[d1.seq].meds[cnt].doseunit_cnt = 1
  ...
  FOOT  o.order_id
   ...
   CALL echo(build2("dlrec->seq[d1.seq]->meds[cnt].dose_cnt:",trim(build2(dlrec->seq[d1.seq].meds[cnt
      ].dose_cnt),3)))
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  HEAD c.event_cd
   CASE (c.event_cd)
    ...
    OF mf_bmi_cd:
     dlrec->seq[d1.seq].measurements[cnt].s_bmi = trim(c.result_val),dlrec->seq[d1.seq].measurements[
     cnt].s_bmi_dt_tm = substring(1,14,format(c.event_end_dt_tm,"@SHORTDATE"))
   ENDCASE
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_RPT_AMB_DOWNTIME"
