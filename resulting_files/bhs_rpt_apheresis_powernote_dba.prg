CREATE PROGRAM bhs_rpt_apheresis_powernote:dba
 PROMPT
  "Begin Date" = "SYSDATE",
  "Stop Date" = "SYSDATE"
 ...
 DECLARE ms_file_name = vc WITH protect, constant(build(logical("bhscust"),
   "/ftp/bhs_rpt_apheresis_powernote/apheresis_medicine_service_data_",format(curdate,"YYYYMMDD;;D"),
   ".csv"))
 FREE RECORD m_rec
 RECORD m_rec(
   1 pt[*]
 ...
 IF (validate(request->batch_selection))
  SET start_date = datetimefind(cnvtlookbehind("1M",cnvtdatetime(curdate,0)),"M","B","B")
  SET end_date = datetimefind(cnvtlookbehind("1M",cnvtdatetime(curdate,0)),"M","E","E")
 ELSE
 ...
 SELECT INTO "nl:"
  ...
  PLAN (sst
   WHERE sst.story_completion_status_cd=mf_form_status_cd
    AND sst.updt_dt_tm BETWEEN cnvtdatetime(start_date) AND cnvtdatetime(end_date)
  ...
  HEAD REPORT
   cnt = 0
  HEAD p.person_id
   cnt1 = 0, cnt += 1, stat = alterlist(m_rec->pt,cnt),
  ...
  DETAIL
   cnt1 += 1, stat = alterlist(m_rec->pt[cnt].story,cnt1), m_rec->pt[cnt].story[cnt1].f_scd_story_id
    = sst.scd_story_id
  FOOT  p.person_id
   cnt1 = 0
  FOOT REPORT
