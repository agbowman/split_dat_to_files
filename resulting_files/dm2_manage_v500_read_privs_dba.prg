CREATE PROGRAM dm2_manage_v500_read_privs:dba
 ...
 ENDIF
 DECLARE dmvrp_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmvrp_priv_cnt = i4 WITH protect, noconstant(0)
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: Starting script dm2_manage_v500_read_privs"
 IF (check_logfile("dm2_manage_v500rp",".log","dm2_manage_v500_read_privs LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_manage_v500_read_privs"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Check for the V500_READ user in dba_users."
 SELECT INTO "nl:"
  FROM dba_users du
  WHERE du.username="V500_READ"
  WITH nocounter
 ;end select
 ...
 IF (curqual=0)
  ...
  SET readme_data->message = "dm2_manage_v500_read_privs auto-successed due to no V500_READ user."
  SET dm_err->eproc = "dm2_manage_v500_read_privs completed successfully."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSE
  SET dm_err->eproc = "Checking for the SHARED_LIST_GTTD in ALL_OBJECTS"
  SELECT INTO "nl:"
   FROM all_objects ao
   WHERE ao.object_type="TABLE"
    AND ao.object_name="SHARED_LIST_GTTD"
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) > 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  ...
  ENDIF
  IF (curqual > 0)
   SET dmvrp_cnt = (dmvrp_cnt+ 1)
   IF (dm2_push_cmd("rdb grant select,insert,update,delete on SHARED_LIST_GTTD to V500_READ go",1)=0)
    GO TO exit_script
   ENDIF
  ENDIF
  SET dm_err->eproc = "Checking for the SHARED_TXT_GTTD in ALL_OBJECTS"
  SELECT INTO "nl:"
   FROM all_objects ao
   WHERE ao.object_type="TABLE"
    AND ao.object_name="SHARED_TXT_GTTD"
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) > 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  ...
  IF (curqual > 0)
   SET dmvrp_cnt = (dmvrp_cnt+ 1)
   IF (dm2_push_cmd("rdb grant select,insert,update,delete on SHARED_TXT_GTTD to V500_READ go",1)=0)
    GO TO exit_script
   ENDIF
  ENDIF
  SET dm_err->eproc = "Checking for the SHARED_VALUE_GTTD in ALL_OBJECTS"
  SELECT INTO "nl:"
   FROM all_objects ao
   WHERE ao.object_type="TABLE"
    AND ao.object_name="SHARED_VALUE_GTTD"
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) > 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  ...
  IF (curqual > 0)
   SET dmvrp_cnt = (dmvrp_cnt+ 1)
   IF (dm2_push_cmd("rdb grant select,insert,update,delete on SHARED_VALUE_GTTD to V500_READ go",1)=0
   )
    GO TO exit_script
   ENDIF
  ENDIF
  SET dm_err->eproc = "Checking for the PLAN_TABLE in ALL_OBJECTS"
  SELECT INTO "nl:"
   FROM all_objects ao
   WHERE ao.object_type="TABLE"
    AND ao.object_name="PLAN_TABLE"
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) > 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  ...
  IF (curqual > 0)
   SET dmvrp_cnt = (dmvrp_cnt+ 1)
   IF (dm2_push_cmd("rdb grant select,insert,update,delete on PLAN_TABLE to V500_READ go",1)=0)
    GO TO exit_script
   ENDIF
  ENDIF
 ENDIF
 SET dm_err->eproc = "Verify ORACLE privileges for V500_READ."
 SELECT INTO "nl:"
  FROM dba_tab_privs dtp
  WHERE dtp.grantee="V500_READ"
   AND dtp.table_name IN ("SHARED_LIST_GTTD", "SHARED_VALUE_GTTD", "SHARED_TXT_GTTD", "PLAN_TABLE")
  DETAIL
   dmvrp_priv_cnt = (dmvrp_priv_cnt+ 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF ((dmvrp_priv_cnt < (dmvrp_cnt * 4)))
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "Few privileges in DBA_SYS_PRIVS were found for the V500_READ user.  more privileges expected."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
#exit_script
 IF ((dm_err->err_ind=1))
  SET readme_data->status = "F"
  SET readme_data->message = "dm2_manage_v500_read_privs failed..."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET readme_data->status = "S"
  SET readme_data->message = "dm2_manage_v500_read_privs ran successfully."
  SET dm_err->eproc = "dm2_manage_v500_read_privs completed successfully."
 ENDIF
 EXECUTE dm_readme_status
 CALL echorecord(readme_data)
 SET dm_err->eproc = "Ending dm2_manage_v500_read_privs"
 CALL final_disp_msg("dm2_manage_v500rp")
END GO
