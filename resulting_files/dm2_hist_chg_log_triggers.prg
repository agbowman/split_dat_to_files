CREATE PROGRAM dm2_hist_chg_log_triggers
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 CALL check_logfile("hist_chglog_trigger",".log","DM2_HIST_CHG_LOG_TRIGGERS LOGFILE")
 SET dm_err->eproc = "Beginning DM2_HIST_CHG_LOG_TRIGGERS"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE current_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dhclt_cnt = i4 WITH protect, noconstant(0)
 DECLARE trig_count = vc
 DECLARE tbl_count = vc
 DECLARE target = vc
 DECLARE dhcl_last_md_refresh = vc WITH protect, noconstant(" ")
 FREE RECORD tr_hist
 RECORD tr_hist(
   1 hist_cnt = i4
   1 env_big = i4
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   current_env_id = a.info_number
  WITH nocounter
 ;end select
 IF (check_error("Get current environment id") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Querying for last time local meta-data was refreshed."
 SELECT INTO "nl:"
  el.cur_environment_id, el.rdds_event_key, dhcl_max = max(el.event_dt_tm)
  FROM dm_rdds_event_log el
  WHERE el.cur_environment_id=current_env_id
   AND el.rdds_event_key="LOCALMETADATAREFRESH"
  GROUP BY el.cur_environment_id, el.rdds_event_key
  DETAIL
   dhcl_last_md_refresh = format(dhcl_max,"dd-mmm-yy HH:MM:SS;;q")
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dhcl_last_md_refresh <= " ")
  SET dhcl_last_md_refresh = "None"
 ENDIF
 SELECT INTO "nl:"
  FROM dm_rdds_event_log el
  WHERE el.cur_environment_id=current_env_id
   AND el.rdds_event_key IN ("ADDENVIRONMENTTRIGGERS", "SUBSETOFTRIGGERSADDED",
  "DROPENVIRONMENTTRIGGERS", "SUBSETOFTRIGGERSDROPPED")
  ORDER BY el.event_dt_tm DESC
  HEAD REPORT
   dhclt_cnt = 0
  DETAIL
   dhclt_cnt = (dhclt_cnt+ 1)
   IF (mod(dhclt_cnt,100)=1)
    stat = alterlist(tr_hist->list,(dhclt_cnt+ 99))
   ENDIF
   tr_hist->list[dhclt_cnt].log_type = el.rdds_event_key, tr_hist->list[dhclt_cnt].log_dt_tm = el
   .event_dt_tm, tr_hist->list[dhclt_cnt].log_id = el.dm_rdds_event_log_id,
   tr_hist->list[dhclt_cnt].target_env = el.paired_environment_id
  FOOT REPORT
   stat = alterlist(tr_hist->list,dhclt_cnt), tr_hist->hist_cnt = dhclt_cnt
  WITH nocounter
 ;end select
 IF (check_error("Get trigger activity") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((tr_hist->hist_cnt > 0))
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(tr_hist->hist_cnt)),
    dm_environment de
   PLAN (d
    WHERE (tr_hist->list[d.seq].target_env_name <= " "))
    JOIN (de
    WHERE (de.environment_id=tr_hist->list[d.seq].target_env))
   DETAIL
    tr_hist->list[d.seq].target_env_name = de.environment_name
   WITH nocounter
  ;end select
  IF (check_error("Get target environment name") != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SELECT INTO "nl:"
   y = sum(de.event_detail_value)
   FROM (dummyt d  WITH seq = value(tr_hist->hist_cnt)),
    dm_rdds_event_detail de
   PLAN (d
    WHERE (tr_hist->list[d.seq].trig_cnt=0))
    JOIN (de
    WHERE (de.dm_rdds_event_log_id=tr_hist->list[d.seq].log_id))
   DETAIL
    tr_hist->list[d.seq].trig_cnt = y
   WITH nocounter
  ;end select
  IF (check_error("Get trigger count field") != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SELECT INTO "nl:"
   y = count(de.event_detail1_txt)
   FROM (dummyt d  WITH seq = value(tr_hist->hist_cnt)),
    dm_rdds_event_detail de
   PLAN (d
    WHERE (tr_hist->list[d.seq].tbl_cnt=0))
    JOIN (de
    WHERE (de.dm_rdds_event_log_id=tr_hist->list[d.seq].log_id))
   DETAIL
    tr_hist->list[d.seq].tbl_cnt = y
   WITH nocounter
  ;end select
  IF (check_error("Get table count field") != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 IF ((tr_hist->hist_cnt=0))
  SET tr_hist->hist_cnt = 1
 ENDIF
 SELECT INTO mine
  FROM (dummyt d  WITH seq = tr_hist->hist_cnt)
  HEAD REPORT
   col 40, "******  Trigger History Report  ******", row + 2,
   col 1, "Last Local Meta-Data Refresh:", col 31,
   dhcl_last_md_refresh, row + 1
  HEAD PAGE
   row + 1, col 1, "DATE",
   col 27, "ACTION", col 36,
   "DETAIL", row + 1, col 1,
   "----------------------------------------------------------------------------------------------------------------------",
   row + 1
  DETAIL
   IF ((((tr_hist->list[d.seq].log_type="DROPENVIRONMENTTRIGGERS")) OR ((tr_hist->list[d.seq].
   log_type="SUBSETOFTRIGGERSDROPPED"))) )
    trig_count = cnvtstring(tr_hist->list[d.seq].trig_cnt), tbl_count = cnvtstring(tr_hist->list[d
     .seq].tbl_cnt), target = cnvtstring(tr_hist->list[d.seq].target_env),
    col 1, tr_hist->list[d.seq].log_dt_tm";;Q", col 27,
    "DROP", col 36, trig_count,
    col + 1, "triggers were dropped from", col + 1,
    tbl_count, col + 1, "tables",
    col + 1, "for target environment", col + 1,
    target, col + 1, "-",
    col + 1, tr_hist->list[d.seq].target_env_name, col + 1,
    row + 1
   ELSEIF ((((tr_hist->list[d.seq].log_type="ADDENVIRONMENTTRIGGERS")) OR ((tr_hist->list[d.seq].
   log_type="SUBSETOFTRIGGERSADDED"))) )
    trig_count = cnvtstring(tr_hist->list[d.seq].trig_cnt), tbl_count = cnvtstring(tr_hist->list[d
     .seq].tbl_cnt), target = cnvtstring(tr_hist->list[d.seq].target_env),
    col 1, tr_hist->list[d.seq].log_dt_tm";;Q", col 27,
    "ADD", col 36, trig_count
    IF ((tr_hist->list[d.seq].target_env=0))
     col + 1, "triggers were recompiled on", col + 1,
     tbl_count, col + 1, "tables"
    ELSE
     col + 1, "triggers were created on", col + 1,
     tbl_count, col + 1, "tables",
     col + 1, "for target environment", col + 1,
     target, col + 1, "-",
     col + 1, tr_hist->list[d.seq].target_env_name, col + 1
 ...
 ;end select
 IF (check_error("Error from executing dm2_hist_chg_log_triggers")=1)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
#exit_program
 SET dm_err->eproc = "...Ending DM2_HIST_CHG_LOG_TRIGGERS"
 CALL final_disp_msg("dm2_hist_chg_log_triggers")
END GO
