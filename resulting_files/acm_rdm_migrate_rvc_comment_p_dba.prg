CREATE PROGRAM acm_rdm_migrate_rvc_comment_p:dba
 ...
 IF (error(ms_errmsg,0) != 0)
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_dm_info_row(mf_max_timeline_id)
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ) IN (2, 0))
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_RDM_ACM_MIGRATE_COMMENT",concat(
   "create global temporary table TEMP_RDM_ACM_MIGRATE_COMMENT(encntr_id number, ",
   "rvc_comment_hist_id number, activity_created_dt_tm date, activity_created_prsnl_id number, ",
   "comment_clob clob, parent_entity_id number, parent_entity_name varchar2(32), ",
   "pm_hist_tracking_id number, applied_to_txt varchar2(255), applied_to_cd number, ",
   "updt_id number, updt_dt_tm date, updt_task number, active_status_dt_tm date, ",
   "active_status_prsnl_id number, active_status_cd number) on commit preserve rows"),
  "Creating temporary table")
 CALL sbr_add_sql_stmt("INDEX","XIETEMPACMRDMPROCESS",
  "create unique index XIETEMPACMRDMPROCESS ON TEMP_RDM_ACM_MIGRATE_COMMENT(rvc_comment_hist_id)",
  "Creating unique index on temporary table")
 CALL sbr_add_sql_stmt("INDEX","XIE2TEMPACMRDMPROCESS",
  "create index XIE2TEMPACMRDMPROCESS ON TEMP_RDM_ACM_MIGRATE_COMMENT(parent_entity_id)",
  "Creating non-unique index on temporary table")
 IF (sbr_run_sql(null)=0)
  GO TO exit_program
 ENDIF
 SET readme_data->status = "S"
 SET readme_data->message = "Readme completed successfully"
 SUBROUTINE (sbr_add_dm_info_row(rc_timeline_id=f8) =null)
   SELECT INTO "nl:"
    FROM dm_info di
    PLAN (di
   ...
   IF (error(ms_errmsg,0) != 0)
    SET readme_data->status = "F"
    SET readme_data->message = concat("Error selecting from dm_info: ",ms_errmsg)
    GO TO exit_program
   ENDIF
   IF (curqual=0)
    INSERT  FROM dm_info di
     SET di.info_name = ms_dm_info_name, di.info_domain = ms_dm_info_domain, di.info_long_id =
      rc_timeline_id,
      di.info_date = cnvtdatetime(sysdate), di.updt_applctx = reqinfo->updt_applctx, di.updt_id =
      reqinfo->updt_id,
      di.updt_task = reqinfo->updt_task, di.updt_dt_tm = cnvtdatetime(sysdate), di.updt_cnt = 0
     WITH nocounter
    ;end insert
   ENDIF
   IF (error(ms_errmsg,0) != 0)
    SET readme_data->status = "F"
    SET readme_data->message = concat("Error inserting the max ID on dm_info: ",ms_errmsg)
    GO TO exit_program
   ENDIF
 END ;Subroutine
#exit_program
