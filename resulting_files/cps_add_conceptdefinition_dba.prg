CREATE PROGRAM cps_add_conceptdefinition:dba
 ...
 SELECT INTO "nl:"
  ...
  WHERE ct.concept_identifier=concept_identifier
   AND ct.concept_source_cd=concept_source_cd
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET failed = select_error
  SET dhtable_name = "CONCEPT"
  SET serrmsg_error = "invalid data entered or problem in reading concept table"
  GO TO error_check
 ELSE
  ...
  IF (ncode=0)
   ...
   SET serrmsg_error = "problem in generating sequence number for concept_definition_id"
   GO TO error_check
  ENDIF
  INSERT  FROM concept_definition con
   SET con.concept_identifier = trim(concept_identifier,3), con.concept_source_cd = concept_source_cd,
    con.concept_definition_id = ncode,
    con.definition = trim(definition,3), con.source_vocabulary_cd = source_vocabulary_cd, con
    .updt_cnt = 0,
    ...
    con.end_effective_dt_tm = cnvtdatetime("31-DEC-2100")
   WITH nocounter
  ;end insert
  IF (curqual=0)
   SET failed = insert_error
   SET dhtable_name = "CONCEPT_DEFINITION"
   SET serrmsg_error = "problem in inserting data into conept_definition table"
   GO TO error_check
  ENDIF
  ...
  SET reply->conceptdef[k].concept_definition_id = ncode
 ENDIF
#error_check
 IF (failed=false)
