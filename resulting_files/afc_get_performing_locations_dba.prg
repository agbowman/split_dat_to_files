CREATE PROGRAM afc_get_performing_locations:dba
 IF ("Z"=validate(afc_get_performing_locations_vrsn,"Z"))
  DECLARE afc_get_performing_locations_vrsn = vc WITH noconstant("CHARGSRV-15928.008"), public
 ENDIF
 RECORD reply(
   1 qual[*]
     2 location_cd = f8
 ...
 DECLARE ncount1 = i4
 DECLARE dnurseunitcd = f8
 DECLARE dambulatorycd = f8
 DECLARE code_set = i4
 DECLARE cdf_meaning = c12
 DECLARE cnt = i4
 DECLARE norgsecurity = i2
 DECLARE curlogicaldomainid = f8 WITH protect, noconstant(0.0)
 SET code_set = 222
 SET cdf_meaning = "AMBULATORY"
 SET cnt = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cnt,dambulatorycd)
 SET code_set = 222
 SET cdf_meaning = "NURSEUNIT"
 SET cnt = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cnt,dnurseunitcd)
  ...
  SELECT INTO "nl:"
   ...
   PLAN (por
    ...
    WHERE l.organization_id=por.organization_id
     AND l.location_type_cd IN (dnurseunitcd, dambulatorycd)
     AND l.active_ind=1)
   DETAIL
    ncount1 += 1
    IF (mod(ncount1,100)=1)
     stat = alterlist(tmp_reply->qual,(ncount1+ 99))
    ENDIF
    tmp_reply->qual[ncount1].location_cd = l.location_cd, tmp_reply->qual[ncount1].display =
    cnvtupper(uar_get_code_display(l.location_cd))
   FOOT REPORT
    stat = alterlist(tmp_reply->qual,ncount1)
   WITH nocounter
  ;end select
 ELSE
  SELECT INTO "nl:"
   FROM location l,
    organization o
   PLAN (l
    WHERE l.location_type_cd IN (dnurseunitcd, dambulatorycd)
     AND l.active_ind=1)
    JOIN (o
  ...
  ;end select
 ENDIF
 IF (size(tmp_reply->qual,5) > 0)
  SET ncount1 = 0
  SELECT INTO "nl:"
   FROM (dummyt d1  WITH seq = value(size(tmp_reply->qual,5)))
   ORDER BY tmp_reply->qual[d1.seq].display
   DETAIL
    ncount1 += 1, stat = alterlist(reply->qual,ncount1), reply->qual[ncount1].location_cd = tmp_reply
    ->qual[d1.seq].location_cd
   WITH nocounter
  ;end select
 ENDIF
 FREE SET tmp_reply
