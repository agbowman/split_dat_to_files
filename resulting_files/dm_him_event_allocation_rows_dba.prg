CREATE PROGRAM dm_him_event_allocation_rows:dba
 ...
 IF (v_days_to_keep < 60)
  ...
  GO TO exit_script
 ENDIF
 SET v_sign_type_cd = uar_get_code_by("MEANING",21,"SIGN")
 IF (v_sign_type_cd=0.0)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"SIGN",
   "Unable to find code value for CDF meaning 'SIGN' in codeset 21.")
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(hea.him_event_allocation_id)
   FROM him_event_allocation hea
   WHERE hea.him_event_allocation_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(hea.him_event_allocation_id)
  FROM him_event_allocation hea
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    hea.rowid
    FROM him_event_allocation hea
    WHERE parser(sbr_getrowidnotexists("hea.him_event_allocation_id between curMinID and curMaxID",
      "hea"))
     AND hea.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND ((hea.completed_dt_tm+ 0) != cnvtdatetime("31-DEC-2100"))
     AND hea.action_type_cd != v_sign_type_cd
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = hea.rowid
    WITH nocounter, maxqual(hea,value(rowsleft))
   ;end select
