CREATE PROGRAM cp_publish_pathway:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "CP Pathway ID" = 0.0,
  "Change Text" = "",
  "Version Name" = "",
  "User Changes Documentation URL" = ""
  WITH outdev, dinput_pathway_id, sinput_change_text,
  sinput_version_name, sinput_change_doc_url
 ...
 RECORD temp_unpub_nodes(
   1 cnt = i4
   1 qual[*]
     2 cp_node_id = f8
     2 version_node_id = f8
 )
 DECLARE getunpublishednodes(null) = null WITH protect
 DECLARE publishnodes(null) = null WITH protect
 ...
 CALL getunpublishednodes(null)
 CALL publishnodes(null)
 SET mpagereply->status_data.status = "S"
 SUBROUTINE getunpublishednodes(null)
   CALL log_message("Begin GetUnPublishedNodes()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SELECT INTO "NL:"
    FROM cp_node cn
    WHERE (cn.cp_pathway_id= $DINPUT_PATHWAY_ID)
     AND cn.publish_ind=0
    ...
    DETAIL
     nodecnt += 1
     IF (nodecnt > size(temp_unpub_nodes->qual,5))
      stat = alterlist(temp_unpub_nodes->qual,(nodecnt+ 5))
     ENDIF
     temp_unpub_nodes->qual[nodecnt].cp_node_id = cn.cp_node_id, temp_unpub_nodes->qual[nodecnt].
     version_node_id = cn.version_node_id
    FOOT REPORT
     temp_unpub_nodes->cnt = nodecnt, stat = alterlist(temp_unpub_nodes->qual,nodecnt)
    WITH nocounter
   ;end select
   IF (validate(debug_ind,0))
    CALL echorecord(temp_unpub_nodes)
   ENDIF
   CALL error_and_zero_check_rec(curqual,"GetUnPublishedNodes","",1,1,
    mpagereply)
   CALL log_message(build("Exit GetUnPublishedNodes(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE publishnodes(null)
   CALL log_message("Begin PublishNodes()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE px = i4 WITH noconstant(0), protect
   FOR (px = 1 TO temp_unpub_nodes->cnt)
     UPDATE  FROM cp_node cn
      SET cn.end_effective_dt_tm = cnvtdatetime(sysdate), cn.active_ind = 0, cn.publish_ind = 0
      WHERE (cn.cp_node_id=temp_unpub_nodes->qual[px].version_node_id)
       AND cn.publish_ind=1
       AND (cn.cp_node_id != temp_unpub_nodes->qual[px].cp_node_id)
      WITH nocounter
     ;end update
     CALL error_and_zero_check_rec(curqual,"PublishNodes","Inactive Current Node Version",1,0,
      mpagereply)
     UPDATE  FROM cp_node cn
      SET cn.publish_ind = 1
      WHERE (cn.cp_node_id=temp_unpub_nodes->qual[px].cp_node_id)
      WITH nocounter
     ;end update
     CALL error_and_zero_check_rec(curqual,"PublishNodes","Active new Node Version",1,1,
      mpagereply)
   ENDFOR
   SET stat = alterlist(mpagereply->published_nodes,temp_unpub_nodes->cnt)
   FOR (px = 1 TO temp_unpub_nodes->cnt)
     SET mpagereply->published_nodes[px].cp_node_id = temp_unpub_nodes->qual[px].cp_node_id
   ENDFOR
   CALL log_message(build("Exit PublishNodes(), Elapsed time in seconds:",((curtime3 - begin_curtime3
