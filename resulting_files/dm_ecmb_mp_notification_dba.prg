CREATE PROGRAM dm_ecmb_mp_notification:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_MP_NOTIFICATION"
 ...
 IF (v_cust_count1 > 0)
  FOR (v_cust_loopcount1 = 1 TO v_cust_count1)
    IF (upt_from(rreclist->from_rec[v_cust_loopcount1].from_id,request->xxx_combine[icombine].
     to_xxx_id,rreclist->from_rec[v_cust_loopcount1].prsnl_id,rreclist->from_rec[v_cust_loopcount1].
     notification_type_flag)=0)
     GO TO exit_sub
    ENDIF
  ENDFOR
 ENDIF
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8,prsnl_id=f8,notif_flag=i2) =i2)
   ...
   SELECT INTO "nl:"
    ...
    WHERE mn.encntr_id=s_uf_to_fk_id
     AND mn.prsnl_id=prsnl_id
   ...
   ELSE
    UPDATE  FROM mp_notification frm
     SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_task = reqinfo->
      updt_task,
      frm.updt_dt_tm = cnvtdatetime(sysdate), frm.encntr_id = s_uf_to_fk_id
     WHERE frm.mp_notification_id=s_uf_pk_id
     WITH nocounter
    ;end update
   ENDIF
