CREATE PROGRAM dm2_ads_validate_configs
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE davc_logfile_prefix = vc WITH protect, constant("dm2_ads_val_cfg")
 DECLARE davc_cur_latest_tbl_dt_tm = dq8 WITH protect, noconstant(0.0)
 DECLARE davc_last_latest_tbl_dt_tm = dq8 WITH protect, noconstant(0.0)
 DECLARE davc_cnt = i4 WITH protect, noconstant(0)
 DECLARE davc_cnt2 = i4 WITH protect, noconstant(0)
 DECLARE davc_cur_extract_id = f8 WITH protect, noconstant(0.0)
 DECLARE davc_cur_table_name = vc WITH protect, noconstant("")
 DECLARE davc_ndx = i4 WITH protect, noconstant(0)
 DECLARE davc_pos = i4 WITH protect, noconstant(0)
 FREE RECORD davc_new_tables
 RECORD davc_new_tables(
   1 table_cnt = i4
   1 table_qual[*]
     2 table_name = vc
     2 extract_id = f8
 ...
 SET width = 132
 SET message = nowindow
 IF (check_logfile(davc_logfile_prefix,".log","dm2_ads_validate_configs")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_ads_validate_configs"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Obtaining most recent creation date for any table"
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 SELECT INTO "nl:"
  max_date = max(uo.created)
  FROM user_objects uo
  WHERE uo.object_type="TABLE"
  DETAIL
   davc_cur_latest_tbl_dt_tm = cnvtdatetime(max_date)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Obtaining last recorded creation date for any table from dm_info"
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_ADS_VALIDATE_CONFIGS"
   AND di.info_name="LAST_TBL_CREATED_DT_TM"
  DETAIL
   davc_last_latest_tbl_dt_tm = cnvtdatetime(di.info_date)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inseting last recorded creation date for any table into dm_info"
  CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2_ADS_VALIDATE_CONFIGS", di.info_name = "LAST_TBL_CREATED_DT_TM", di
    .info_date = cnvtdatetime(davc_cur_latest_tbl_dt_tm)
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
  ...
  COMMIT
 ENDIF
 IF (cnvtdatetime(davc_cur_latest_tbl_dt_tm) > cnvtdatetime(davc_last_latest_tbl_dt_tm))
  IF (dar_get_extract_list(1,"*",- (1),0)=0)
   GO TO exit_program
  ENDIF
  IF (dar_scr_dads_list(0)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Obtaining list of new tables"
  CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM user_objects uo,
   ...
   PLAN (d
    ...
    JOIN (uo
    WHERE uo.object_type="TABLE"
     AND (dads_list->extract_qual[d.seq].table_name=uo.object_name)
     AND uo.created > cnvtdatetime(davc_last_latest_tbl_dt_tm))
   ORDER BY uo.object_name
   HEAD REPORT
    davc_new_tables->table_cnt = 0, stat = alterlist(davc_new_tables->table_qual,0)
   DETAIL
    davc_pos = locateval(davc_ndx,1,davc_new_tables->table_cnt,dads_list->extract_qual[d.seq].
     table_name,davc_new_tables->table_qual[davc_ndx].table_name)
    IF (davc_pos > 0)
     IF ((davc_new_tables->table_qual[davc_pos].table_extract_instance_nbr < dads_list->extract_qual[
     d.seq].table_extract_instance_nbr))
      davc_new_tables->table_qual[davc_pos].table_extract_instance_nbr = dads_list->extract_qual[d
      .seq].table_extract_instance_nbr, davc_new_tables->table_qual[davc_pos].extract_id = dads_list
      ->extract_qual[d.seq].extract_id, davc_new_tables->table_qual[davc_pos].table_extract_nbr =
      dads_list->extract_qual[d.seq].table_extract_nbr,
      davc_new_tables->table_qual[davc_pos].apply_where_ind = dads_list->extract_qual[d.seq].
      apply_where_ind
     ENDIF
    ELSE
     davc_new_tables->table_cnt = (davc_new_tables->table_cnt+ 1), stat = alterlist(davc_new_tables->
      table_qual,davc_new_tables->table_cnt), davc_new_tables->table_qual[davc_new_tables->table_cnt]
     .extract_id = dads_list->extract_qual[d.seq].extract_id,
     davc_new_tables->table_qual[davc_new_tables->table_cnt].table_name = dads_list->extract_qual[d
     .seq].table_name, davc_new_tables->table_qual[davc_new_tables->table_cnt].
     table_extract_instance_nbr = dads_list->extract_qual[d.seq].table_extract_instance_nbr,
     davc_new_tables->table_qual[davc_new_tables->table_cnt].table_extract_nbr = dads_list->
     extract_qual[d.seq].table_extract_nbr,
     davc_new_tables->table_qual[davc_new_tables->table_cnt].apply_where_ind = dads_list->
     extract_qual[d.seq].apply_where_ind, davc_new_tables->table_qual[davc_new_tables->table_cnt].
     config_cnt = 0
    ENDIF
   WITH nocounter
  ;end select
  ...
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echorecord(davc_new_tables)
  ENDIF
  FOR (davc_cnt = 1 TO davc_new_tables->table_cnt)
    SET dm_err->eproc = concat("Check existance of config data for new table: ",davc_new_tables->
     table_qual[davc_cnt].table_name)
    CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm_ads_config_extract dace,
      dm_ads_config dac,
      dummyt d
     PLAN (dac
      WHERE dac.sample_method != dpl_custom)
      JOIN (d)
      JOIN (dace
      WHERE dace.dm_ads_config_id=dac.dm_ads_config_id
       AND (dace.dm_ads_extract_id=davc_new_tables->table_qual[davc_cnt].extract_id))
     HEAD REPORT
      davc_cnt2 = 0, stat = alterlist(davc_new_tables->table_qual[davc_cnt].config_qual,davc_cnt2)
     DETAIL
      davc_cnt2 = (davc_cnt2+ 1), stat = alterlist(davc_new_tables->table_qual[davc_cnt].config_qual,
       davc_cnt2), davc_new_tables->table_qual[davc_cnt].config_qual[davc_cnt2].config_id = dac
      .dm_ads_config_id
     FOOT REPORT
      davc_new_tables->table_qual[davc_cnt].config_cnt = davc_cnt2
    ...
    IF (check_error(dm_err->eproc)=1)
     ...
     GO TO exit_program
    ENDIF
    IF ((davc_new_tables->table_qual[davc_cnt].config_cnt > 0))
     SET dm_err->eproc = "Inserting rows for new tables into dm_ads_config_extract"
     CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
     INSERT  FROM dm_ads_config_extract dace,
       (dummyt d  WITH seq = davc_new_tables->table_qual[davc_cnt].config_cnt)
      SET dace.dm_ads_config_extract_id = cnvtreal(seq(dm_clinical_seq,nextval)), dace
       .dm_ads_config_id = davc_new_tables->table_qual[davc_cnt].config_qual[d.seq].config_id, dace
       .dm_ads_extract_id = davc_new_tables->table_qual[davc_cnt].extract_id,
       dace.status_txt = dpl_incomplete, dace.message_txt = dpl_new_table, dace.dm_process_queue_id
        = 0.0,
       dace.updt_dt_tm = cnvtdatetime(curdate,curtime3), dace.updt_id = 0, dace.updt_task = 0,
       dace.updt_applctx = 0, dace.updt_cnt = 0
      PLAN (d)
       JOIN (dace)
      WITH nocounter
     ;end insert
     ...
     ENDIF
     COMMIT
     FOR (davc_cnt2 = 1 TO davc_new_tables->table_qual[davc_cnt].config_cnt)
       IF (dar_sync_config_status(davc_new_tables->table_qual[davc_cnt].config_qual[davc_cnt2].
        config_id)=0)
        GO TO exit_program
       ENDIF
     ENDFOR
    ENDIF
  ENDFOR
  SET dm_err->eproc = "Updating last recorded creation date for any table into dm_info"
  CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
  UPDATE  FROM dm_info di
   SET di.info_date = cnvtdatetime(davc_cur_latest_tbl_dt_tm)
   WHERE di.info_domain="DM2_ADS_VALIDATE_CONFIGS"
    AND di.info_name="LAST_TBL_CREATED_DT_TM"
   WITH nocounter
  ;end update
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Ending dm2_ads_validate_configs"
 CALL final_disp_msg(davc_logfile_prefix)
END GO
