CREATE PROGRAM dm_transaction_eligibilit_rows:dba
 ...
 SET reply->table_name = "TRANSACTION_ELIGIBILITY"
 ...
 FOR (t_idx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 730)
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must keep at least 730 days worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
  GO TO exit_script
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    minval = min(te.transaction_eligibility_id)
    FROM transaction_eligibility te
    WHERE te.transaction_eligibility_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   maxval = max(te.transaction_eligibility_id)
   FROM transaction_eligibility te
   DETAIL
  ...
  WHILE (v_cur_min_id <= v_max_id
   AND v_rows_left > 0)
    SELECT INTO "nl:"
     te.rowid
     FROM transaction_eligibility te
     WHERE parser(sbr_getrowidnotexists("te.sent_dt_tm < cnvtdatetime(v_days_back)","te"))
      AND te.transaction_eligibility_id BETWEEN v_cur_min_id AND v_cur_max_id
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,100)=1)
       stat = alterlist(reply->rows,(rows+ 99))
      ENDIF
      reply->rows[rows].row_id = te.rowid
     WITH nocounter, maxqual(te,value(v_rows_left))
    ;end select
