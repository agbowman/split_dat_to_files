CREATE PROGRAM cv_da_add_cv_order_category_r:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_add_cv_order_category_r_vrsn = vc WITH private, constant("20080311")
 ...
 WHILE (i <= size(request->objarray,5))
   SET did = 0.0
   IF (validate(request->objarray[i].cv_order_category_r_id,- (0.00001)) <= 0.0)
    SET did = id_list->item[i].id
    SET request->objarray[i].cv_order_category_r_id = did
   ELSE
    SET did = request->objarray[i].cv_order_category_r_id
   ENDIF
   IF (validate(reply->insert_ids)=1)
    SET reply->insert_ids[i].id = did
   ENDIF
   INSERT  FROM cv_order_category_r c
    SET c.cv_order_category_r_id = did, c.cv_order_category_id =
