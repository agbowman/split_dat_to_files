CREATE PROGRAM cp_upd_concept
 PROMPT
  "Output to File/Printer/MINE:" = "MINE",
  "List of Concept Code:" = 0.0,
  "Set Active Indicator:" = 0
  WITH outdev, concept_cd, act_ind
 RECORD cp_updt_reply(
   1 code_value = f8
   1 display = vc
   1 cdf_meaning = c12
   1 description = vc
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE checkconceptcode(null) = i4 WITH protect
 DECLARE lstat = i4 WITH protect, noconstant(0)
 DECLARE dconcept_cd = f8 WITH protect, constant( $CONCEPT_CD)
 DECLARE nactive_ind = i2 WITH protect, constant( $ACT_IND)
 DECLARE dinactive_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",48,"INACTIVE"))
 ...
 SET cp_updt_reply->status_data.status = "F"
 SET lstat = checkconceptcode(null)
 IF (validate(debug_ind,0)=1)
  CALL echorecord(cp_updt_reply)
 ENDIF
 IF (dconcept_cd > 0
  AND lstat > 0)
  UPDATE  FROM code_value cv
   SET cv.active_ind = nactive_ind, cv.inactive_dt_tm = cnvtdatetime(sysdate), cv.active_type_cd =
    dinactive_cd,
    cv.updt_applctx = reqinfo->updt_applctx, cv.updt_cnt = (cv.updt_cnt+ 1), cv.updt_dt_tm =
    cnvtdatetime(sysdate),
    cv.updt_id = reqinfo->updt_id, cv.active_status_prsnl_id = reqinfo->updt_id, cv.updt_task =
    reqinfo->updt_task
   WHERE cv.code_value=dconcept_cd
   WITH nocounter
  ;end update
 ENDIF
 SET lstat = checkconceptcode(null)
 SET cp_updt_reply->status_data.status = "S"
 GO TO exit_script
 SUBROUTINE checkconceptcode(null)
   CALL log_message("In CheckConceptCode()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SELECT INTO "nl:"
    FROM code_value cv
    WHERE cv.code_value=dconcept_cd
    DETAIL
     cp_updt_reply->code_value = cv.code_value, cp_updt_reply->display = cv.display, cp_updt_reply->
     cdf_meaning = cv.cdf_meaning,
     cp_updt_reply->description = cv.description, cp_updt_reply->definition = cv.definition,
     cp_updt_reply->active_ind = cv.active_ind
    WITH nocounter
   ;end select
   CALL log_message(build("Exit CheckConceptCode(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(curqual)
 END ;Subroutine
#exit_script
 IF (( $OUTDEV != "NOFORMS"))
  CALL putjsonrecordtofile(cp_updt_reply)
 ENDIF
