CREATE PROGRAM bbt_rpt_trans_req:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->transfusion_requirements = uar_i18ngetmessage(i18nhandle,"transfusion_requirements",
  "TRANSFUSION REQUIREMENTS")
 SET captions->display_name = uar_i18ngetmessage(i18nhandle,"display_name","DISPLAY NAME")
 SET captions->special_instructions = uar_i18ngetmessage(i18nhandle,"special_instructions",
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_trans_req", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  cv.display, cv.description, cv.active_ind
  FROM code_value cv
  WHERE cv.code_set=1611
  HEAD REPORT
   select_ok_ind = 0
  HEAD PAGE
   ...
   col 48, captions->transfusion_requirements, row + 2,
   line = fillstring(122,"-"), line, row + 1,
   col 4, captions->display_name, col 30,
   captions->special_instructions, col 64, captions->active,
   row + 1, line, row + 2
  DETAIL
   col 2, cv.display, col 24,
   cv.description
   IF (cv.active_ind=1)
    col 68, captions->yes
