CREATE PROGRAM ct_get_pt_prescreen_list:dba
 RECORD reply(
   1 prescreenlist[*]
 ...
 IF (bfacfound=1)
   ...
   SELECT
    ...
    HEAD temptable.pt_prot_prescreen_id
     ...
     IF (((person_cnt >= start_rownum
      ...
      prescreenlist[cnt].referral_dt_tm = temptable.referral_dt_tm, reply->prescreenlist[cnt].
      referral_person_id = temptable.referral_person_id,
      reply->prescreenlist[cnt].comment_text = temptable.comment_text, reply->prescreenlist[cnt].
      reason_text = temptable.reason_text, reply->prescreenlist[cnt].open_amendment_id = temptable
      .open_amendment_id,
      reply->prescreenlist[cnt].added_via_flag = temptable.added_via_flag
     ENDIF
    WITH nocounter
   ;end select
  ...
  ELSE
   ...
   ELSE
    SET where_person = build("ps.person_id = ",request->person_id)
   ENDIF
   SET status_list_cnt = size(request->statuslist,5)
   SELECT INTO "NL:"
    ps.pt_prot_prescreen_id, p.name_first, p.name_last,
    p.name_full_formatted, p.person_id, pm.primary_mnemonic
    FROM prot_master pm,
     person p,
     pt_prot_prescreen ps,
     prot_amendment pra,
    ...
    PLAN (ps
     WHERE ps.prot_master_id > 0
      AND ps.screening_status_cd != syscancelcd
      AND parser(where_status)
      AND parser(where_person)
      AND parser(interest_where_string))
     JOIN (pm
     WHERE pm.prot_master_id=ps.prot_master_id
     ...
     JOIN (pra
     WHERE (pra.prot_master_id= Outerjoin(pm.prot_master_id))
      AND (pra.amendment_status_cd= Outerjoin(opencd))
      AND pm.end_effective_dt_tm > cnvtdatetime(sysdate))
     JOIN (cfg
     WHERE (cfg.protocol_type_cd= Outerjoin(pra.participation_type_cd))
      ...
      AND parser(cfg_where))
     JOIN (pr
     WHERE pr.prot_amendment_id=pra.prot_amendment_id
      AND pr.prot_role_type_cd=institution_cd
      AND parser(userorgstr)
      AND pr.end_effective_dt_tm >= cnvtdatetime(sysdate))
     JOIN (cd
     WHERE (cd.prot_amendment_id= Outerjoin(pra.prot_amendment_id))
    ...
    HEAD ps.pt_prot_prescreen_id
     cnt += 1
     IF (mod(cnt,50)=1)
      new = (cnt+ 50), stat = alterlist(reply->prescreenlist,new)
     ENDIF
     reply->prescreenlist[cnt].pt_prot_prescreen_id = ps.pt_prot_prescreen_id, reply->prescreenlist[
     cnt].prot_master_id = ps.prot_master_id, reply->prescreenlist[cnt].person_id = p.person_id,
     reply->prescreenlist[cnt].last_name = p.name_last, reply->prescreenlist[cnt].first_name = p
     .name_first, reply->prescreenlist[cnt].full_name = p.name_full_formatted,
     reply->prescreenlist[cnt].birth_dt_tm = p.birth_dt_tm, reply->prescreenlist[cnt].sex_cd = p
     .sex_cd, reply->prescreenlist[cnt].race_cd = p.race_cd,
     reply->prescreenlist[cnt].prot_alias = pm.primary_mnemonic, reply->prescreenlist[cnt].
     screening_dt_tm = ps.screened_dt_tm, reply->prescreenlist[cnt].screener_person_id = ps
     .screener_person_id,
     reply->prescreenlist[cnt].screening_status_cd = ps.screening_status_cd, reply->prescreenlist[cnt
     ].referral_dt_tm = ps.referred_dt_tm, reply->prescreenlist[cnt].referral_person_id = ps
     .referred_person_id,
     reply->prescreenlist[cnt].comment_text = ps.comment_text, reply->prescreenlist[cnt].reason_text
      = ps.reason_text
     IF (ps.added_via_flag=0)
      reply->latest_prescreen_dt_tm = ps.screened_dt_tm, reply->latest_prescreen_person_id = ps
      .screener_person_id
     ENDIF
     reply->prescreenlist[cnt].open_amendment_id = pra.prot_amendment_id, reply->prescreenlist[cnt].
     added_via_flag = ps.added_via_flag
    DETAIL
   ...
   IF (curqual=0
    ...
    GO TO exit_script
   ENDIF
   SET reply->total_patients_cnt = cnt
  ENDIF
 ELSE
  IF (cur_list_cnt > 0)
   FOR (i = (cur_list_cnt+ 1) TO new_list_cnt)
  ...
  ELSE
   ...
   SELECT INTO "NL:"
    ...
    HEAD ps.pt_prot_prescreen_id
     ...
     .referred_person_id,
     reply->prescreenlist[cnt].comment_text = ps.comment_text, reply->prescreenlist[cnt].reason_text
      = ps.reason_text, reply->prescreenlist[cnt].added_via_flag = ps.added_via_flag
     IF (ps.added_via_flag=0)
      reply->latest_prescreen_dt_tm = ps.screened_dt_tm, reply->latest_prescreen_person_id = ps
      .screener_person_id
     ENDIF
     IF (((pra.amendment_status_cd=opencd) OR (pra.amendment_status_cd=conceptcd)) )
      reply->prescreenlist[cnt].open_amendment_id = pra.prot_amendment_id
     ENDIF
    DETAIL
     IF (cdv.display_ind=1)
      reply->prescreenlist[cnt].displayable_docs_ind = 1
