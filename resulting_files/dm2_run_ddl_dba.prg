CREATE PROGRAM dm2_run_ddl:dba
 ...
 END ;Subroutine
 DECLARE drd_blocking_str = vc WITH protect, noconstant(" ")
 DECLARE drd_resource_busy_max_retry = i4 WITH protect, noconstant(0)
 DECLARE drd_evnt_ind = i2 WITH protect, noconstant(0)
 DECLARE drd_idx = i4 WITH protect, noconstant(0)
 DECLARE drd_in_parse_ind = i2 WITH protect, noconstant(0)
 DECLARE drd_ddl_failed_ind = i2 WITH protect, noconstant(0)
 DECLARE drd_cmd_rscbsy_error = i2 WITH protect, noconstant(0)
 DECLARE drd_cmd_liblock_error = i2 WITH protect, noconstant(0)
 DECLARE drd_retry_attempt = i4 WITH protect, noconstant(0)
 DECLARE drd_parse_retry_cnt = i4 WITH protect, noconstant(0)
 DECLARE drd_rsrc_retry_cnt = i4 WITH protect, noconstant(0)
 DECLARE drd_lib_lock_retry_cnt = i4 WITH protect, noconstant(0)
 DECLARE drd_parse_chk_ovr_ind = i2 WITH protect, noconstant(0)
 DECLARE drd_rsbsy_chk_ovr_ind = i2 WITH protect, noconstant(0)
 DECLARE drd_liblock_chk_ovr_ind = i2 WITH protect, noconstant(0)
 DECLARE drd_ddl_success_ind = i2 WITH protect, noconstant(0)
 DECLARE drd_status_message = vc WITH protect, noconstant(" ")
 DECLARE drd_ver_ind = i2 WITH protect, noconstant(0)
 DECLARE drd_half_maxretry = i4 WITH protect, noconstant(0)
 DECLARE drd_begin_dt_tm = f8 WITH public, noconstant(0.0)
 DECLARE drd_end_dt_tm = f8 WITH public, noconstant(0.0)
 DECLARE drd_notify_ind = i2 WITH protect, noconstant(0)
 DECLARE drd_available_retry = i4 WITH protect, noconstant(0)
 DECLARE drd_duration = f8 WITH protect, noconstant(0.0)
 DECLARE drd_unhandled_err_ind = i2 WITH protect, noconstant(0)
 DECLARE drd_liblock_event_ind = i2 WITH protect, noconstant(0)
 CALL dm2_prg_maint("BEGIN")
 SET dm_err->eproc = "Starting dm2_run_ddl"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Retrieving override info if exists")
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_RUN_DDL_OPTION"
   AND d.info_name IN ("BYPASS RSBSY CHECK", "BYPASS PARSE CHECK", "BYPASS LIBLOCK CHECK",
  "USE LIBLOCK EVENT")
   AND d.info_number=1
  DETAIL
   IF (d.info_name="BYPASS PARSE CHECK")
    drd_parse_chk_ovr_ind = d.info_number
   ENDIF
   IF (d.info_name="BYPASS RSBSY CHECK")
    drd_rsbsy_chk_ovr_ind = d.info_number
   ENDIF
   IF (d.info_name="BYPASS LIBLOCK CHECK")
    drd_liblock_chk_ovr_ind = d.info_number
   ENDIF
   IF (d.info_name="USE LIBLOCK EVENT")
    drd_liblock_event_ind = d.info_number
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
 ...
 ENDIF
 IF (curqual > 0)
  SET dm_err->eproc = concat("Override Detected: P",build(drd_parse_chk_ovr_ind),"|R",build(
    drd_rsbsy_chk_ovr_ind),"|L",
   build(drd_liblock_chk_ovr_ind),"|E",build(drd_liblock_event_ind))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (drd_liblock_event_ind=1)
  IF ((((dm2_rdbms_version->level1 > 11)) OR ((((dm2_rdbms_version->level1=11)
   AND (dm2_rdbms_version->level2 > 2)) OR ((((dm2_rdbms_version->level1=11)
   AND (dm2_rdbms_version->level2=2)
   AND (dm2_rdbms_version->level3 > 0)) OR ((dm2_rdbms_version->level1=11)
   ...
   SET drd_ver_ind = 1
   IF (dm2_push_cmd("rdb alter session set events '24439608 trace name context forever, level 1' go",
    1)=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ENDIF
  ...
  ENDIF
 ENDIF
 SET drd_resource_busy_max_retry = cnvtint(dm2_db_options->resource_busy_maxretry)
 SET drd_half_maxretry = ceil((drd_resource_busy_max_retry/ 2))
 SET drd_begin_dt_tm = cnvtdatetime(curdate,curtime3)
 FOR (drd_idx = 1 TO ddr_ddl_req->ddl_cnt)
   SET drd_retry_attempt = 0
   SET drd_parse_retry_cnt = 0
   SET drd_rsrc_retry_cnt = 0
   SET drd_lib_lock_retry_cnt = 0
   SET drd_ddl_success_ind = 0
   SET drd_ddl_failed_ind = 0
   SET drd_unhandled_err_ind = 0
   WHILE (drd_retry_attempt < drd_resource_busy_max_retry)
     SET drd_in_parse_ind = 0
     SET drd_cmd_rscbsy_error = 0
     SET drd_cmd_liblock_error = 0
     SET drd_notify_ind = 0
     IF (drd_retry_attempt=0)
      SET drd_begin_dt_tm = cnvtdatetime(curdate,curtime3)
     ELSE
      SET drd_end_dt_tm = cnvtdatetime(curdate,curtime3)
      SET drd_duration = ((drd_end_dt_tm - drd_begin_dt_tm)/ 10000000)
     ENDIF
     IF ((ddr_ddl_req->ddls[drd_idx].ddl_bypass_parse_chk=0)
      AND (dm2_rdbms_version->level1 >= 11)
      AND drd_parse_chk_ovr_ind=0)
      IF (ddf_check_in_parse(ddr_ddl_req->ddls[drd_idx].ddl_table_owner,ddr_ddl_req->ddls[drd_idx].
       ddl_table_name,drd_in_parse_ind,drd_blocking_str)=0)
       GO TO exit_program
      ENDIF
     ENDIF
     IF (drd_in_parse_ind=0)
      SET dm_err->eproc = "Executing DDL"
      CALL dm2_push_cmd(ddr_ddl_req->ddls[drd_idx].ddl_cmd,1)
      IF ((dm_err->err_ind > 0))
       IF ((ddr_ddl_req->ddls[drd_idx].ddl_bypass_rsbsy_chk=0)
        AND drd_rsbsy_chk_ovr_ind=0
        AND ((findstring("ORA-00054",dm_err->emsg) > 0) OR (((findstring("ORA-14450",dm_err->emsg) >
       0) OR (((findstring("ORA-14452",dm_err->emsg) > 0) OR (findstring("ORA-14411",dm_err->emsg) >
       0)) )) )) )
        SET drd_cmd_rscbsy_error = 1
        SET drd_rsrc_retry_cnt = (drd_rsrc_retry_cnt+ 1)
       ELSEIF ((ddr_ddl_req->ddls[drd_idx].ddl_bypass_liblock_chk=0)
        AND drd_liblock_chk_ovr_ind=0
        AND findstring("ORA-04022",dm_err->emsg) > 0)
        SET drd_lib_lock_retry_cnt = (drd_lib_lock_retry_cnt+ 1)
        SET drd_cmd_liblock_error = 1
       ELSE
        SET drd_ddl_failed_ind = 1
        SET drd_unhandled_err_ind = 1
       ENDIF
      ELSE
       SET drd_ddl_success_ind = 1
       SET drd_retry_attempt = drd_resource_busy_max_retry
      ENDIF
     ELSE
      SET dm_err->eproc = drd_blocking_str
      CALL disp_msg(" ",dm_err->logfile,0)
      SET drd_parse_retry_cnt = (drd_parse_retry_cnt+ 1)
      IF ((ddr_ddl_req->ddl_notify_ind=1))
       SET dm_err->emsg = drd_blocking_str
      ENDIF
     ENDIF
     IF (drd_ddl_success_ind=0)
      IF (drd_ddl_failed_ind=0)
       SET drd_retry_attempt = ((drd_parse_retry_cnt+ drd_rsrc_retry_cnt)+ drd_lib_lock_retry_cnt)
      ELSE
       SET drd_retry_attempt = drd_resource_busy_max_retry
      ENDIF
      IF (drd_retry_attempt=drd_resource_busy_max_retry)
       IF (drd_ddl_failed_ind=1)
        SET drd_status_message = dm_err->emsg
       ELSE
        SET drd_ddl_failed_ind = 1
        SET drd_status_message = concat("DDL failed after ",build(drd_retry_attempt)," attempts: P",
         build(drd_parse_retry_cnt),"|R",
         build(drd_rsrc_retry_cnt),"|L",build(drd_lib_lock_retry_cnt))
       ENDIF
      ELSE
       SET dm_err->eproc = concat("Retrying DDL. Total Retried: (",build(drd_retry_attempt),
        "). Retry Reasons: P",build(drd_parse_retry_cnt),"|R",
        build(drd_rsrc_retry_cnt),"|L",build(drd_lib_lock_retry_cnt))
       CALL disp_msg(" ",dm_err->logfile,0)
       CALL pause(3)
      ENDIF
      IF (((drd_retry_attempt=drd_half_maxretry) OR (drd_retry_attempt=drd_resource_busy_max_retry))
      )
       SET drd_notify_ind = 1
      ELSEIF (drd_retry_attempt > drd_half_maxretry)
       IF (mod(drd_retry_attempt,ceil((drd_resource_busy_max_retry/ 10)))=0)
        SET drd_notify_ind = 1
       ENDIF
      ENDIF
      IF ((ddr_ddl_req->ddl_notify_ind=1)
       AND drd_notify_ind=1
       AND drd_unhandled_err_ind=0)
       SET drd_available_retry = (drd_resource_busy_max_retry - drd_retry_attempt)
       SET dnotify->process = ddr_ddl_req->ddl_process
       SET dnotify->install_status = dpl_executing
       SET dnotify->mode = "DDL EXECUTION"
       IF (drd_retry_attempt=drd_resource_busy_max_retry)
        SET dnotify->event = concat("Schema retry ceiling reached for DDL operation")
        SET dnotify->msgtype = dpl_failure
       ELSE
        SET dnotify->event = concat("Schema retry event encountered DDL operation")
        SET dnotify->msgtype = dpl_warning
       ENDIF
       CALL dn_add_body_text(concat("DDL execution has been attempted ",trim(cnvtstring(
           drd_retry_attempt))," times over the last ",trim(cnvtstring(ceil((drd_duration/ 60)))),
         " minutes."),0)
       IF (drd_retry_attempt=drd_half_maxretry)
        CALL dn_add_body_text(concat("There are ",trim(cnvtstring(drd_half_maxretry)),
          " attempts remaining.","If condition not resolved, DDL will be failed in ",trim(cnvtstring(
            ceil((drd_duration/ 60)))),
          " minutes."),0)
       ELSEIF (drd_retry_attempt != drd_resource_busy_max_retry)
        CALL dn_add_body_text(concat("There are ",trim(cnvtstring(drd_available_retry)),
          " attempts remaining.","If condition not resolved, DDL will be failed in ",trim(cnvtstring(
            ceil((drd_available_retry * ((drd_duration/ drd_retry_attempt)/ 60))))),
          " minutes."),0)
       ENDIF
       IF (drd_retry_attempt=drd_resource_busy_max_retry)
       ...
       CALL dn_add_body_text(" ",0)
       CALL dn_add_body_text(dm_err->emsg,0)
       CALL dn_add_body_text(drd_status_message,0)
       CALL dn_add_body_text(" ",0)
       CALL dn_add_body_text("The operation being executed is: ",0)
       CALL dn_add_body_text(ddr_ddl_req->ddls[drd_idx].ddl_cmd,0)
       CALL dn_add_body_text(" ",0)
     ...
     SET dm_err->emsg = " "
   ENDWHILE
   IF (drd_ddl_success_ind=1)
    SET ddr_ddl_req->ddls[drd_idx].ddl_status = "S"
    SET ddr_ddl_req->ddls[drd_idx].ddl_status_message = " "
   ELSE
    SET ddr_ddl_req->ddls[drd_idx].ddl_status = "F"
    SET ddr_ddl_req->ddl_fail_cnt = (ddr_ddl_req->ddl_fail_cnt+ 1)
    SET ddr_ddl_req->ddls[drd_idx].ddl_status_message = drd_status_message
    SET dm_err->eproc = concat(drd_status_message)
    CALL disp_msg(" ",dm_err->logfile,0)
    IF ((ddr_ddl_req->ddl_ignore_failures=0))
     GO TO exit_program
    ENDIF
   ENDIF
 ENDFOR
#exit_program
 ...
 IF ((ddr_ddl_req->ddl_fail_cnt > 0))
  CALL echo(build(ddr_ddl_req->ddl_fail_cnt," DDL(s) failed"))
 ENDIF
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "dm2_run_ddl completed"
 ENDIF
