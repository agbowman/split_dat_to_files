CREATE PROGRAM dm_bb_ptc_temp_person_rows:dba
 ...
 SET reply->table_name = "BB_PTC_TEMP_PERSON"
 ...
 IF (ldaystokeep < 30)
  ...
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"INVALIDDAYS",
   "You must enter 30 or greater for the days to keep token.  You entered %1 or did not enter any value.",
   "i",ldaystokeep)
  GO TO exit_script
 ENDIF
 SET dpurgedttm = cnvtlookbehind(build(ldaystokeep,"D"),cnvtdatetime(sysdate))
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(p.bb_ptc_temp_person_id)
   FROM bb_ptc_temp_person p
   WHERE p.bb_ptc_temp_person_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(p.bb_ptc_temp_person_id)
  FROM bb_ptc_temp_person p
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    p.rowid
    FROM bb_ptc_temp_person p
    WHERE parser(sbr_getrowidnotexists("p.updt_dt_tm < cnvtdatetime(dPurgeDtTm)","p"))
     AND p.bb_ptc_temp_person_id != 0
    DETAIL
     v_rows += 1
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = p.rowid
    WITH nocounter, maxqual(p,value(v_rows_left))
   ;end select
