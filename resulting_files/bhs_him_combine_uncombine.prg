CREATE PROGRAM bhs_him_combine_uncombine
 PROMPT
 ...
 SELECT INTO "NL:"
  ...
  PLAN (pc
   ...
   WHERE pc.updt_id=pr.person_id
    AND pr.username=patstring( $USERNAME))
   JOIN (ea
 ...
 SELECT INTO "NL:"
  ...
  HEAD ec.encntr_combine_id
   ...
   format(ec.updt_dt_tm,";;Q"), cmb->combines[c_cnt].from_person_id = p.person_id,
 ...
 SELECT INTO value(var_output)
  ...
  HEAD REPORT
   ...
   col + 0,
   CALL print(build("TO_PERSON_ALIASES",char(9)))
  DETAIL
