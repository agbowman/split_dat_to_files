CREATE PROGRAM dm_rx_claim_auth_audit_rows:dba
 ...
 SET reply->table_name = "RX_CLAIM_AUTH_AUDIT"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minimumvalue = min(rcaa.rx_claim_auth_audit_id)
   FROM rx_claim_auth_audit rcaa
   WHERE rcaa.rx_claim_auth_audit_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maximumvalue = max(rcaa.rx_claim_auth_audit_id)
  FROM rx_claim_auth_audit rcaa
  DETAIL
 ...
 WHILE (batchlowerboundid <= maximumid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    rcaa.rx_claim_auth_audit_id
    FROM rx_claim_auth_audit rcaa
    WHERE rcaa.rx_claim_auth_audit_id > 0
     AND parser(sbr_getrowidnotexists(
      "rcaa.rx_claim_auth_audit_id between batchLowerBoundId and batchUpperBoundId","rcaa"))
     AND rcaa.updt_dt_tm <= cnvtdatetime((curdate - daystokeep),235959)
    DETAIL
     recordcount = (recordcount+ 1)
     IF (mod(recordcount,100)=1)
      stat = alterlist(reply->rows,(recordcount+ 99))
     ENDIF
     reply->rows[recordcount].row_id = rcaa.rowid
    WITH nocounter, maxqual(rcaa,value(rowsleft))
   ;end select
