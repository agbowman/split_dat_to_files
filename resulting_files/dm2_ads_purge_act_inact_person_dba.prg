CREATE PROGRAM dm2_ads_purge_act_inact_person:dba
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (validate(reply->status_msg,"NOT DEFINED")="NOT DEFINED")
  RECORD reply(
    1 status = c1
    1 status_msg = vc
  )
 ENDIF
 DECLARE dapaip_person_table = vc WITH protect, noconstant("PERSON")
 DECLARE dapaip_config_id = f8 WITH protect, noconstant(0.0)
 DECLARE dapaip_sec_name = c80 WITH protect, noconstant("")
 DECLARE dapaip_user_id = f8 WITH protect, noconstant(0.0)
 DECLARE dapaip_auth_cd = f8 WITH protect, noconstant(0.0)
 DECLARE dapaip_active_cd = f8 WITH protect, noconstant(0.0)
 DECLARE dapaip_purged_cd = f8 WITH protect, noconstant(0.0)
 DECLARE dapaip_active_ind = i2 WITH protect, noconstant(0)
 DECLARE dapaip_inhouse_ind = i2 WITH protect, noconstant(0)
 SET dapaip_config_id = cnvtreal( $1)
 SET dapaip_active_ind = cnvtint( $2)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dapaip_inhouse_ind = 1
  WITH nocounter
 ;end select
 IF ((xxcclseclogin->loggedin=0))
  SET reply->status = "F"
  SET reply->status_msg = "User must be logged in to CCLSECLOGIN"
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  ...
  DETAIL
   dapaip_active_cd = c.code_value
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET reply->status = "F"
  SET reply->status_msg = "Error while querying for the ACTIVE code value"
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.code_set=48
   AND c.cdf_meaning="PURGED"
  DETAIL
   dapaip_purged_cd = c.code_value
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET reply->status = "F"
  SET reply->status_msg = "Error while querying for the PURGED code value"
  GO TO exit_program
 ENDIF
 IF (dapaip_purged_cd=0)
  SELECT INTO "nl:"
   FROM code_value c
   WHERE c.code_set=8
    AND c.cdf_meaning="AUTH"
   DETAIL
    dapaip_auth_cd = c.code_value
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET reply->status = "F"
   SET reply->status_msg = "Error while querying for the AUTH code value"
   GO TO exit_program
  ENDIF
  SELECT INTO "nl:"
  ...
  IF (check_error(dm_err->eproc)=1)
   SET reply->status = "F"
   SET reply->status_msg = "Error while getting new code_value off of the sequence"
   GO TO exit_program
  ENDIF
  INSERT  FROM code_value
   SET code_value = dapaip_purged_cd, code_set = 48, cdf_meaning = "PURGED",
    display = "Purged", display_key = cnvtupper(cnvtalphanum("Purged")), description =
    "Row has been purged or is ready to be purged",
    definition = "Row has been purged or is ready to be purged", active_ind = 1, active_type_cd =
    dapaip_active_cd,
    data_status_cd = dapaip_auth_cd, begin_effective_dt_tm = cnvtdatetime(curdate,curtime3),
  ...
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
   SET reply->status = "F"
   SET reply->status_msg = "Error while inserting the PURGED code_value into the CODE_VALUE table"
   GO TO exit_program
  ELSE
   COMMIT
  ENDIF
 ENDIF
 IF (dapaip_inhouse_ind=0)
  SET stat = uar_secgetusername(dapaip_sec_name,size(dapaip_sec_name))
  SET dapaip_sec_name = trim(dapaip_sec_name)
  SELECT INTO "nl:"
   FROM prsnl p
   WHERE p.username=dapaip_sec_name
    AND p.active_ind=1
    AND p.data_status_cd=dapaip_auth_cd
   DETAIL
    dapaip_user_id = p.person_id
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET reply->status = "F"
   SET reply->status_msg = "Error while selecting for user_id on the PRSNL table"
   GO TO exit_program
  ENDIF
  IF (curqual=0)
   SET reply->status = "F"
   SET reply->status_msg = "User not found in PRSNL table"
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm2scramble->mode_ind = 1
 SET dm2scramble->in_text = dapaip_person_table
 IF (ds_scramble(null)=0)
  SET reply->status = "F"
  SET reply->status_msg = dm_err->emsg
  GO TO exit_program
 ENDIF
 SET dapaip_person_table = trim(dm2scramble->out_text,3)
 IF (dapaip_active_ind=0)
  UPDATE  FROM person p
   SET p.active_ind = dapaip_active_ind, p.updt_dt_tm = cnvtdatetime(curdate,curtime3), p.updt_id =
    dapaip_user_id,
    p.active_status_cd = dapaip_purged_cd, p.updt_cnt = (p.updt_cnt+ 1)
   WHERE p.active_ind=1
    AND p.person_id IN (
   (SELECT
    d.driver_key_id
    FROM dm_ads_config_driver d
    WHERE d.dm_ads_config_id=dapaip_config_id
     AND (d.dm_ads_extract_id=
    (SELECT
     d2.dm_ads_extract_id
     FROM dm_ads_extract d2
     WHERE d2.table_name=dapaip_person_table
      AND d2.driver_table_ind=1))))
  ;end update
 ELSE
  UPDATE  FROM person p
   SET p.active_ind = dapaip_active_ind, p.updt_dt_tm = cnvtdatetime(curdate,curtime3), p.updt_id =
    dapaip_user_id,
    p.active_status_cd = dapaip_active_cd, p.updt_cnt = (p.updt_cnt+ 1)
   WHERE p.active_ind=0
    AND p.active_status_cd=dapaip_purged_cd
    AND p.person_id IN (
   (SELECT
    d.driver_key_id
    FROM dm_ads_config_driver d
 ...
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
  SET reply->status = "F"
  SET reply->status_msg = "Error while activating/inactivating person_ids on the PERSON table"
  GO TO exit_program
 ENDIF
 SET reply->status = "S"
 COMMIT
