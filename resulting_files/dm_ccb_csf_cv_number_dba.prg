CREATE PROGRAM dm_ccb_csf_cv_number:dba
 ...
 ENDIF
 DECLARE csf_cvn_status = c1 WITH protect, noconstant("S")
 DECLARE csf_cvn_errcode = i4 WITH protect, noconstant(0)
 DECLARE csf_cvn_errmsg = vc WITH protect, noconstant(" ")
 DECLARE csf_cvn_filter = vc WITH protect, noconstant(" ")
 IF (size(request->filterlist,5)=0)
  SET csf_cvn_errmsg = "No code value number search criteria submitted."
  SET csf_cvn_status = "F"
  GO TO exit_program
 ENDIF
 IF (cnvtreal(request->filterlist[1].column_value)=0.0)
  SET csf_cvn_errmsg = "Must submit a non-zero code value number."
  SET csf_cvn_status = "F"
  GO TO exit_program
 ENDIF
 SET csf_cvn_filter = filterstring_builder(csf_cvn_filter,"cv",request)
 SELECT INTO "nl:"
  cvs.code_set, cvs.definition, cvs.description,
  cvs.display, cnt = count(DISTINCT cv.code_value)
  FROM code_value cv,
   code_value_set cvs
  PLAN (cv
   WHERE parser(csf_cvn_filter))
   JOIN (cvs
  ...
  WITH nocounter
 ;end select
 SET csf_cvn_errcode = error(csf_cvn_errmsg,1)
 IF (csf_cvn_errcode != 0)
  SET csf_cvn_errmsg = concat("Error searching for code value number: ",csf_cvn_errmsg)
  SET csf_cvn_status = "F"
  GO TO exit_program
 ELSEIF (size(reply->code_set_list,5)=0)
  SET csf_cvn_errmsg = build2("No values found for code value number: ",request->filterlist[1].
   column_value)
  SET csf_cvn_status = "F"
  GO TO exit_program
 ENDIF
#exit_program
 SET reply->status_data.status = csf_cvn_status
 SET reply->message = csf_cvn_errmsg
END GO
