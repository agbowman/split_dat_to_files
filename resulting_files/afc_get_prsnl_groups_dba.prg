CREATE PROGRAM afc_get_prsnl_groups:dba
 ...
 IF (validate(copytracemessages,char(128))=char(128))
  ...
  END ;Subroutine
 ENDIF
 CALL beginservice("376573.001")
 RECORD request(
   1 prsnlgroupclasscd = f8
   1 prsnlgrouptypecd = f8
   1 physicianind = i2
 ) WITH protect
 ...
 SELECT INTO "nl:"
  FROM prsnl_group pg
  PLAN (pg
   WHERE pg.prsnl_group_class_cd=evaluate(request->prsnlgroupclasscd,0.0,pg.prsnl_group_class_cd,
    request->prsnlgroupclasscd)
    AND pg.prsnl_group_type_cd=evaluate(request->prsnlgrouptypecd,0.0,pg.prsnl_group_type_cd,request
    ->prsnlgrouptypecd)
    AND pg.active_ind=1
   ...
   (SELECT
    ...
    WHERE pgr.prsnl_group_id=pg.prsnl_group_id
     ...
     AND p.logical_domain_id=logicaldomainid
     AND p.physician_ind=evaluate(request->physicianind,0,p.physician_ind,1))))
  HEAD REPORT
   groupcount = 0
  DETAIL
   groupcount += 1, stat = alterlist(reply->prsnlgroups,groupcount), reply->prsnlgroups[groupcount].
   prsnlgroupid = pg.prsnl_group_id,
   reply->prsnlgroups[groupcount].prsnlgroupname = pg.prsnl_group_name
  WITH nocounter
 ;end select
 IF (size(reply->prsnlgroups,5) <= 0)
  CALL exitservicenodata("There were no personnel groups found with the given parameters",
   go_to_exit_script)
 ENDIF
 CALL exitservicesuccess("Successfully retrieved personnel group(s)")
#exit_script
