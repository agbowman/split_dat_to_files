CREATE PROGRAM dm2_mig_custccl_imp
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmcci_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_imp")
 DECLARE dmcci_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmcci_custom_ind = i2 WITH protect, noconstant(0)
 DECLARE dmcci_master_id = f8 WITH protect, noconstant(0.0)
 SET width = 132
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_imp"
 CALL disp_msg("",dm_err->logfile,0)
 IF ((dm_err->debug_flag > 0))
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dmcci_master_id = dena.dma_env_node_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FOR (dmcci_cnt = 1 TO size(requestin->list_0,5))
   SET dm_err->eproc = build("Processing CSV Row <",dmcci_cnt,"> of <",size(requestin->list_0,5),">."
    )
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dma_env_node_dprotect d
    WHERE d.dma_env_node_id=dmcci_master_id
     AND (d.object_name=requestin->list_0[dmcci_cnt].script_name)
     AND d.group_nbr=cnvtint(requestin->list_0[dmcci_cnt].group)
    DETAIL
     dmcci_custom_ind = d.merge_ind
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF (curqual > 0)
    IF ((evaluate(dmcci_custom_ind,1,"Y","N") != requestin->list_0[dmcci_cnt].custom_ind))
     SET dm_err->eproc = concat("Updating dma_env_node_dprotect row for script: ",requestin->list_0[
      dmcci_cnt].script_name)
     UPDATE  FROM dma_env_node_dprotect d
      SET d.merge_ind = evaluate(requestin->list_0[dmcci_cnt].custom_ind,"Y",1,"N",0)
      WHERE d.dma_env_node_id=dmcci_master_id
       AND (d.object_name=requestin->list_0[dmcci_cnt].script_name)
       AND d.group_nbr=cnvtint(requestin->list_0[dmcci_cnt].group)
      WITH nocounter
