CREATE PROGRAM afc_get_billing_entities:dba
 ...
 IF (validate(copytracemessages,char(128))=char(128))
  ...
  END ;Subroutine
 ENDIF
 IF ( NOT (validate(reply->billingentities)))
  RECORD reply(
    1 billingentities[*]
 ...
 ENDIF
 CALL beginservice("374527.001")
 IF ( NOT (getbillingentities(1)))
  CALL exitservicenodata("No Billing Entities found",true)
 ENDIF
 CALL exitservicesuccess("Successfully fetched the billing entities..")
 SUBROUTINE (getbillingentities(dummyvar=i2) =i2)
   ...
   SELECT INTO "nl:"
    ...
    HEAD be.billing_entity_id
     IF (be.billing_entity_id != 0.0)
      bcnt += 1
      IF (mod(bcnt,10)=1)
       stat = alterlist(reply->billingentities,(bcnt+ 9))
      ENDIF
      reply->billingentities[bcnt].billing_entity_id = be.billing_entity_id, reply->billingentities[
      bcnt].billing_entity_name = be.be_name
     ENDIF
    FOOT REPORT
     stat = alterlist(reply->billingentities,bcnt)
    WITH nocounter
   ;end select
   IF (curqual=0)
    CALL logmessage("getBillingEntities","Billing Entities not found for the personal...",log_debug)
    RETURN(false)
   ENDIF
