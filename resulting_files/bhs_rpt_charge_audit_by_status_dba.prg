CREATE PROGRAM bhs_rpt_charge_audit_by_status:dba
 PROMPT
 ...
 SELECT
  IF (any_status_ind="C")
   PLAN (c
    ...
    177, 777, 997)
     AND (c.process_flg= $F_PROCESS_FLAG)
     AND c.active_ind=1)
  ELSE
   PLAN (c
    WHERE c.process_flg IN (3, 2, 4, 5, 8,
  ...
  DETAIL
   ...
   chgs->qual[cnt].charge_desc = concat("1=",c.charge_description)
   IF (c.process_flg=3)
 ...
 IF (( $F_PROCESS_FLAG=1))
  SELECT
   ...
   HEAD c.charge_item_id
    ...
    chgs->qual[cnt].item_quantity = c.item_quantity, chgs->qual[cnt].updt_id = c.updt_id, chgs->qual[
    cnt].tier_group_cd = c.tier_group_cd,
    chgs->qual[cnt].charge_desc = concat("2=",c.charge_description), chgs->qual[cnt].include_rsn = 2,
    chgs->qual[cnt].charge_status = c.process_flg,
 ...
 SELECT
  ...
  HEAD c.charge_item_id
   ...
   IF (report_charge > 0)
    ...
    chgs->qual[cnt].encntr_id = c.encntr_id, chgs->qual[cnt].person_id = c.person_id, chgs->qual[cnt]
    .charge_item_id = c.charge_item_id,
    chgs->qual[cnt].service_dt_tm = ic.updt_dt_tm, chgs->qual[cnt].activity_dt_tm = ic.posted_dt_tm,
    chgs->qual[cnt].activity_type_cd = c.activity_type_cd,
    chgs->qual[cnt].updt_id = c.updt_id, chgs->qual[cnt].item_quantity = c.item_quantity, chgs->qual[
    cnt].tier_group_cd = c.tier_group_cd,
    chgs->qual[cnt].charge_desc = concat("IC-",c.charge_description), chgs->qual[cnt].include_rsn =
    report_charge, chgs->qual[cnt].charge_status = c.process_flg
   ENDIF
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_id=chgs->qual[d.seq].encntr_id))
  DETAIL
   chgs->qual[d.seq].nurse_unit = uar_get_code_display(e.loc_nurse_unit_cd)
  WITH counter
 ;end select
 ...
 SELECT INTO value(output_dest)
  ...
  HEAD REPORT
   IF (( $OUTTYPE="1"))
    ...
    col 152, "Nurse Unit", row + 1
   ELSEIF (( $OUTTYPE="2"))
    outstring = concat('CDM,"Description","Quan","Service Dt","Activity","Status",',
     '"Reason","Activity Type","FAC","Account Number","Update Personnel",','"Nurse Unit"'), col 1,
    outstring,
    row + 1
   ENDIF
  ...
  DETAIL
   ...
   IF (( $OUTTYPE="1"))
    ...
    col 152, chgs->qual[d.seq].nurse_unit, row + 1
   ELSEIF (( $OUTTYPE="2"))
    outstring = trim(build(chgs->qual[d.seq].cdm_num,',"',chgs->qual[d.seq].charge_desc,'",',quan_out,
      ...
      .seq].fin_nbr,'","',
      chgs->qual[d.seq].updt_name,'","',chgs->qual[d.seq].nurse_unit,'"')), col 1, outstring,
    row + 1
   ENDIF
 ...
 IF (email_ind=1)
  ...
  IF (( $OUTTYPE="1"))
   SET filename_out = trim(concat(output_dest,".txt"))
  ELSEIF (( $OUTTYPE="2"))
   SET filename_out = trim(concat(output_dest,".csv"))
  ENDIF
  IF (( $1="chargeusers@"))
   SET email_address = "CISARD@baystatehealth.org"
  ELSE
   SET email_address =  $1
  ENDIF
