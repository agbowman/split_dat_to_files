CREATE PROGRAM cp_save_structured_section:dba
 PROMPT
  ...
  prsnl_id, author_id, workflow_id,
  ppr_cd, concept, section_activity_json
 ...
 DECLARE srv_request = vc WITH protect, constant("SaveStructureDocSection")
 DECLARE conceptckistr = vc WITH protect, constant( $CONCEPT)
 DECLARE cur_workflow_id = f8 WITH protect, constant(cnvtreal( $WORKFLOW_ID))
 DECLARE cur_wkf_comp_id = f8 WITH noconstant(0)
 IF ( NOT (validate(_memory_reply_string)))
  DECLARE _memory_reply_string = vc WITH noconstant(""), protect
 ENDIF
 DECLARE main(null) = null
 DECLARE savesectionactivity(null) = null
 DECLARE unescapecontentstring(vc) = vc
 DECLARE deletewkfcomponent(null) = null
 IF ( NOT (validate(reply_data)))
  RECORD reply_data(
    1 reply_json = gvc
    1 components[*]
      2 component_id = f8
 ...
 FREE RECORD del_wkf_reply
 RECORD del_wkf_reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 SUBROUTINE savesectionactivity(null)
   CALL log_message("In saveSectionActivity()",log_level_debug)
   DECLARE srv_request = vc WITH protect, constant("SaveStructureDocSection")
   DECLARE start_tm = dq8 WITH noconstant(curtime3), protect
   DECLARE topfreetext = gvc WITH private, noconstant("")
   DECLARE bottomfreetext = gvc WITH private, noconstant("")
   DECLARE activityjsonblob = vc WITH private, noconstant("")
   IF (validate(request->blob_in,"") != "")
    SET activityjsonblob = trim(encodeinternationalcharacters(request->blob_in),3)
   ELSEIF (size(trim( $SECTION_ACTIVITY_JSON,3)) > 0)
    SET activityjsonblob = trim(encodeinternationalcharacters( $SECTION_ACTIVITY_JSON),3)
   ELSE
    CALL populate_subeventstatus_rec("PopulateRequest","F","MISSING_ACTIVITY_JSON",
     "No activity JSON provided to script.",reply_data)
    GO TO exit_script
   ENDIF
   SET stat = cnvtjsontorec(activityjsonblob)
   CALL initializesrvrequest(reply_data,969527)
   SET nsrvstat = uar_srvsetdouble(hreq,"person_id",cnvtreal( $PERSON_ID))
   SET nsrvstat = uar_srvsetdouble(hreq,"encntr_id",cnvtreal( $ENCNTR_ID))
   SET nsrvstat = uar_srvsetdouble(hreq,"author_id",cnvtreal( $AUTHOR_ID))
   SET nsrvstat = uar_srvsetdouble(hreq,"prsnl_id",cnvtreal( $PRSNL_ID))
   SET nsrvstat = uar_srvsetdouble(hreq,"workflow_id",cnvtreal( $WORKFLOW_ID))
   SET nsrvstat = uar_srvsetdouble(hreq,"ppr_cd",cnvtreal( $PPR_CD))
   SET nsrvstat = uar_srvsetstring(hreq,"concept_cki",nullterm( $CONCEPT))
   SET nsrvstat = uar_srvsetstring(hreq,"entry_mode_mean",nullterm("WKFDOCCOMP"))
   SET nsrvstat = uar_srvsetdouble(hreq,"parent_entity_id",cnvtreal(activity->parent_entity_id))
   SET nsrvstat = uar_srvsetlong(hreq,"parent_entity_version",cnvtint(activity->parent_entity_version
     ))
   SET nsrvstat = uar_srvsetstring(hreq,"parent_entity_name",nullterm(activity->parent_entity_name))
   SET nsrvstat = uar_srvsetshort(hreq,"return_json_ind",1)
   SET topfreetext = unescapecontentstring(activity->top_freetext)
   SET bottomfreetext = unescapecontentstring(activity->bottom_freetext)
   IF (size(nullterm(topfreetext)) > 0)
    DECLARE htopfreetext = i4 WITH private, noconstant(0)
    SET htopfreetext = uar_srvadditem(hreq,"section_component")
    SET nsrvstat = uar_srvsetasis(htopfreetext,"narrative_text",nullterm(topfreetext),size(nullterm(
       topfreetext)))
   ENDIF
   DECLARE hsectioncomponent = i4 WITH private, noconstant(0)
   SET hsectioncomponent = uar_srvadditem(hreq,"section_component")
   CALL addsectionactivity(0,hsectioncomponent)
   IF (size(nullterm(bottomfreetext)) > 0)
    DECLARE hbottomfreetext = i4 WITH private, noconstant(0)
    SET hbottomfreetext = uar_srvadditem(hreq,"section_component")
    SET nsrvstat = uar_srvsetasis(hbottomfreetext,"narrative_text",nullterm(bottomfreetext),size(
      nullterm(bottomfreetext)))
   ENDIF
   SET nsrvstat = uar_srvexecute(hmsg,hreq,hrep)
   SET hrep = validatesrvreply(nsrvstat,reply_data,0)
   IF (hrep != 0)
    SET reply_data->reply_json = getasisstring(hrep,"reply_json")
    SET reply_data->status_data.status = "S"
   ENDIF
   CALL exit_srvrequest(happ,htask,hstep)
   CALL log_message(build("Exit saveSectionActivity(), Elapsed time:",((curtime3 - start_tm)/ 100.0)),
    log_level_debug)
 END ;Subroutine
 SUBROUTINE unescapecontentstring(contentstring)
   DECLARE cleanstring = vc WITH protect, noconstant("")
   SET cleanstring = replace(contentstring,'\"','"',0)
   SET cleanstring = replace(cleanstring,"\b",char(8),0)
   SET cleanstring = replace(cleanstring,"\t",char(9),0)
   SET cleanstring = replace(cleanstring,"\r",char(10),0)
   SET cleanstring = replace(cleanstring,"\v",char(11),0)
   SET cleanstring = replace(cleanstring,"\f",char(12),0)
   SET cleanstring = replace(cleanstring,"\n",char(13),0)
   SET cleanstring = replace(cleanstring,"\'",char(39),0)
   SET cleanstring = replace(cleanstring,"\\",char(92),0)
   RETURN(cleanstring)
 END ;Subroutine
 SUBROUTINE deletewkfcomponent(null)
  SELECT INTO "nl:"
   FROM wkf_component wc
   PLAN (wc
    WHERE wc.wkf_workflow_id=cur_workflow_id
     AND wc.component_concept=conceptckistr)
   DETAIL
    cur_wkf_comp_id = wc.wkf_component_id
   WITH nocounter
  ;end select
  IF (cur_wkf_comp_id > 0)
   EXECUTE cp_del_wkf_component "NOFORMS", value(cur_wkf_comp_id) WITH replace("REPLY",
    "DEL_WKF_REPLY")
  ENDIF
 END ;Subroutine
 SUBROUTINE main(null)
   CALL deletewkfcomponent(null)
   CALL savesectionactivity(null)
   IF ((reply_data->status_data.status="S"))
    EXECUTE mp_get_workflow "NOFORMS",  $PERSON_ID,  $ENCNTR_ID,
     $PRSNL_ID
    SET stat = cnvtjsontorec(_memory_reply_string)
    SET stat = movereclist(record_data->components,reply_data->components,1,0,size(record_data->
      components,5),
     1)
    CALL deletewkfcomponent(null)
   ENDIF
...
#exit_script
 ...
 CALL log_message(build("Elapsed time in seconds:",((curtime3 - script_start_tm)/ 100.0)),
  log_level_debug)
 CALL putjsonrecordtofile(reply_data)
END GO
