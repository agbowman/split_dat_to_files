CREATE PROGRAM aps_rdm_dt_format_imprt:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_rdm_dt_format_imprt"
 ...
 CALL logmsg("Preparing to read CSV file for data.",((e_dispmsg+ e_rdmstatus)+ e_logtofile))
 FOR (x = 1 TO list_cnt)
   IF ((((cdf_mean != requestin->list_0[x].parent_cdf_mean)) OR (code_set != cnvtint(requestin->
    ...
    SET request->parent_qual[p_cnt].parent_code_value = 0.0
   ...
   child_cdf_mean
   SET request->parent_qual[p_cnt].child_qual[c_cnt].child_code_value = 0.0
 ENDFOR
 ...
 FOR (x = 1 TO p_cnt)
   ...
   ENDIF
 ENDFOR
 CALL logmsg("Preparing to read code_value/code_value_group tables.",((e_dispmsg+ e_rdmstatus)+
  e_logtofile))
 SELECT INTO "nl:"
  cv1.code_value
  FROM code_value cv1,
  ...
  DETAIL
   request->parent_qual[d1.seq].child_qual[d2.seq].child_code_value = cv2.code_value, insert_cnt = (
   insert_cnt+ 1)
  WITH nocounter, outerjoin = d3, dontexist
 ;end select
 CALL logmsg("Preparing to insert CSV file data into code_value_group table.",((e_dispmsg+
 ...
 INSERT  FROM code_value_group cvg,
  ...
  PLAN (d1
   WHERE (request->parent_qual[d1.seq].parent_code_value > 0))
   JOIN (d2
   WHERE d2.seq <= size(request->parent_qual[d1.seq].child_qual,5))
   JOIN (d3
  ...
  WITH nocounter
 ;end insert
 IF (curqual != insert_cnt)
  SET failed = "T"
 ENDIF
#exit_script
 IF (failed="T")
  ROLLBACK
  CALL logmsg("Failed to populate date/time format information.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
  CALL logstatus("Execution failed - see log message for more details.","F")
 ELSE
  CALL logmsg("Successfully populated date/time format information.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
