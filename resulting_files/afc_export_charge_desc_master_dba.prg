CREATE PROGRAM afc_export_charge_desc_master:dba
 DECLARE afc_export_charge_desc_master = vc WITH constant("CHARGSRV-14225.001")
 ...
 IF (validate(copytracemessages,char(128))=char(128))
  ...
  END ;Subroutine
 ENDIF
 CALL beginservice(afc_export_charge_desc_master)
 IF ( NOT (validate(cdminfo)))
  RECORD cdminfo(
    1 cdmlist[*]
 ...
 ENDIF
 IF ( NOT (validate(fileinfo)))
  RECORD fileinfo(
    1 output[*]
      2 filename = vc
  ) WITH protect
 ENDIF
 DECLARE filename = vc WITH protect, noconstant( $1)
 DECLARE default_max_rows_per_file = i4 WITH protect, constant(100000)
 IF (validate(max_rows_per_file,0.0)=0.0)
  DECLARE max_rows_per_file = i4 WITH protect, constant(default_max_rows_per_file)
 ELSEIF (max_rows_per_file=0)
  DECLARE max_rows_per_file = i4 WITH protect, constant(default_max_rows_per_file)
 ENDIF
 DECLARE column_separator = c1 WITH protect, constant(",")
 DECLARE report_header = vc WITH protect, constant(build2(
   "CHARGE_DESC_MASTER_ID,CDM_CODE,CDM_DESCRIPTION,SERVICE_TYPE"))
 ...
 DECLARE service_type_flag_tech = i2 WITH protect, constant(1)
 DECLARE service_type_flag_prof = i2 WITH protect, constant(2)
 DECLARE curlogicaldomainid = f8 WITH protect, noconstant(0.0)
 DECLARE totalrowcount = i4 WITH protect, noconstant(0)
 DECLARE beginrange = i4 WITH protect, noconstant(0)
 DECLARE endrange = i4 WITH protect, noconstant(0)
 DECLARE tempfilename = vc WITH protect, noconstant("")
 ...
 ENDIF
 SET totalrowcount = 0
 SET stat = alterlist(cdminfo->cdmlist,totalrowcount)
 SELECT INTO "nl:"
  FROM charge_desc_master cdm
  WHERE cdm.logical_domain_id=curlogicaldomainid
   AND cdm.active_ind=1
  ORDER BY cdm.cdm_code_txt
  DETAIL
   totalrowcount += 1, stat = alterlist(cdminfo->cdmlist,totalrowcount), cdminfo->cdmlist[
   totalrowcount].charge_desc_master_id = cdm.charge_desc_master_id,
   cdminfo->cdmlist[totalrowcount].cdm_code_txt = cdm.cdm_code_txt, cdminfo->cdmlist[totalrowcount].
   description = cdm.description, cdminfo->cdmlist[totalrowcount].service_type_flag = cdm
   .service_type_flag
  WITH nocounter
 ;end select
 IF (validate(debug,- (1)) > 0)
  CALL echo(build("totalRowCount=",totalrowcount," rows"))
 ENDIF
 IF (totalrowcount <= max_rows_per_file)
  SET stat = alterlist(fileinfo->output,1)
  SET fileinfo->output[1].filename = filename
  SET beginrange = (endrange+ 1)
  SET endrange = totalrowcount
  CALL writetofile(filename,beginrange,endrange)
 ELSE
  DECLARE filecount = i4 WITH protect, noconstant(0)
  DECLARE totalfilecount = i4 WITH protect, noconstant((totalrowcount/ max_rows_per_file))
  IF (mod(totalrowcount,max_rows_per_file) > 0)
   SET totalfilecount += 1
  ENDIF
  FOR (filecount = 1 TO totalfilecount)
    SET beginrange = (endrange+ 1)
    IF (((endrange+ max_rows_per_file) > totalrowcount))
     SET endrange = totalrowcount
    ELSE
     SET endrange += max_rows_per_file
    ENDIF
    SET stat = alterlist(fileinfo->output,filecount)
    SET tempfilename = build(filename,"_",filecount)
    SET fileinfo->output[filecount].filename = tempfilename
    CALL writetofile(tempfilename,beginrange,endrange)
  ENDFOR
 ENDIF
 SUBROUTINE (writetofile(prfilename=vc(ref),beginrange=i4,endrange=i4) =i4)
   IF (validate(debug,- (1)) > 0)
    CALL echo(build2("writeToFile: ",prfilename," between ",value(beginrange)," and ",
      value(endrange)))
   ENDIF
   DECLARE rowcount = i4 WITH protect, noconstant(0)
   DECLARE line = vc WITH protect, noconstant("")
   DECLARE cdmid = vc WITH protect, noconstant("")
   DECLARE cdmcode = vc WITH protect, noconstant("")
   DECLARE description = vc WITH protect, noconstant("")
   DECLARE servicetype = vc WITH protect, noconstant("")
   SELECT INTO value(prfilename)
    FROM (dummyt d1  WITH seq = value(size(cdminfo->cdmlist,5)))
    WHERE d1.seq BETWEEN beginrange AND endrange
    HEAD REPORT
     report_header
    DETAIL
     cdmid = build('"',cnvtstring(cdminfo->cdmlist[d1.seq].charge_desc_master_id),'"'), cdmcode =
     build('"',cdminfo->cdmlist[d1.seq].cdm_code_txt,'"'), description = build('"',cdminfo->cdmlist[
      d1.seq].description,'"')
     CASE (cdminfo->cdmlist[d1.seq].service_type_flag)
      OF service_type_flag_tech:
       servicetype = build('"',service_type_tech,'"')
      OF service_type_flag_prof:
       servicetype = build('"',service_type_prof,'"')
      ELSE
       servicetype = '""'
     ENDCASE
     line = concat(trim(cdmid),",",trim(cdmcode),",",trim(description),
      ",",trim(servicetype)), row + 1, line,
     rowcount += 1
    WITH nocounter, noformfeed, format = variable
   ;end select
   IF (validate(debug,- (1)) > 0)
    CALL echo(build2("written ",rowcount," rows to file ",prfilename))
   ENDIF
   RETURN(rowcount)
 END ;Subroutine
#exit_script
