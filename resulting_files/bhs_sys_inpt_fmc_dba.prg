CREATE PROGRAM bhs_sys_inpt_fmc:dba
 PROMPT
 ...
 FREE DEFINE rtl
 DEFINE rtl "ccluserdir:2010_fmc_mrn.txt"
 SELECT INTO "nl:"
  FROM rtlt r
  WHERE r.line > " "
  HEAD REPORT
  ...
  DETAIL
   ...
   CALL echo(trim(r.line,3))
  WITH nocounter
 ;end select
 CALL echo(build("1_qualified:",size(temp->qual,5)))
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE (pa.alias=temp->qual[d.seq].mrn)
    ...
    AND ((pa.alias_pool_cd+ 0)=674559.00))
   JOIN (e
