CREATE PROGRAM dcpreqbb01:dba
 ...
 SELECT INTO "nl:"
  ...
  epr.encntr_id, pl.person_id, p1.person_id,
  p2.prsnl_alias, pa.person_id, o.org_name
  FROM person p,
   encounter e,
   person_alias pa,
   encntr_alias ea,
  ...
  PLAN (p
   ...
   JOIN (d5)
   JOIN (p1
   WHERE epr.prsnl_person_id=p1.person_id)
   JOIN (d6)
   JOIN (p2
  ...
  HEAD REPORT
   orders->organization = build(format(o.org_name,"##############################;c;c"),
    " Requisition   "), orders->facility = trim(uar_get_code_description(e.loc_facility_cd)), orders
   ->dob = format(p.birth_dt_tm,"MM/DD/YYYY;;D"),
   orders->name = trim(p.name_full_formatted), orders->sex = trim(uar_get_code_display(p.sex_cd)),
   orders->admit_diagnosis = trim(e.reason_for_visit),
   orders->race = trim(uar_get_code_display(p.race_cd)), orders->admit_dt_tm = e.reg_dt_tm, orders->
   nurse_unit = trim(uar_get_code_display(e.loc_nurse_unit_cd)),
  ...
  WITH outerjoin = d1, dontcare = pa, outerjoin = d2,
   ...
   outerjoin = d5, outerjoin = d6, nocounter
 ;end select
  ...
  SELECT INTO "nl"
   ...
   PLAN (c
    WHERE (c.person_id=request->person_id)
     ...
     AND c.event_cd IN (710750, 710741, 62616)
   ...
   DETAIL
    IF (c.event_cd IN (62610, 62611, 62589, 62612))
     IF (all_last_ind=0)
      ...
      all_ind = 1
     ENDIF
    ELSEIF (c.event_cd IN (710750, 710741, 62616))
     IF (prec_last_ind=0)
 ...
 SELECT INTO "nl:"
  ...
  p.position_cd, p2.person_id, p1.person_id,
  p1.position_cd, ph.phone_id, o.current_start_dt_tm
  FROM orders o,
   order_action oa,
   prsnl p,
   prsnl p1,
  ...
  PLAN (d1)
   ...
   JOIN (p1
   WHERE oa.order_provider_id=p1.person_id)
   JOIN (d3)
   JOIN (p2
   WHERE p1.person_id=p2.person_id
    AND p2.prsnl_alias_type_cd=docnbr_cd
   ...
   WHERE ph.parent_entity_id=oa.order_provider_id
    ...
    AND ph.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
    AND ph.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3))
  ORDER BY oa.action_type_cd, o.current_start_dt_tm
  HEAD REPORT
   orders->order_location = trim(uar_get_code_display(oa.order_locn_cd))
  HEAD o.order_id
   provider = cnvtstring(cnvtint(p2.alias)), signed = format(o.orig_order_dt_tm,"MM/DD/YYYY HH:MM;;d"
   ...
   order_provider_name = trim(p1.name_full_formatted), ord_pos = uar_get_code_display(p1.position_cd)
   IF (ph.phone_format_cd > 0)
   ...
   ENDIF
   IF (oa.action_type_cd IN (order_code, activate_code)
    AND oa.order_status_cd != future_code
    AND (orders->qual[d1.seq].template_order_flag != 2)
    AND o.cs_flag != 2
    AND o.cs_flag != 8
    AND oa.order_app_nbr != 225037)
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(request->printer_name)
   ...
   HEAD PAGE
    line9 = fillstring(80,"_"), reg_dio, "  ",
    row + 1, "{CPI/10}{POS/35/70}{b}", line9,
    row + 1, "{CPI/12}{POS/35/82}", "Patient Name:  ",
    "{CPI/11}{POS/110/82}{b}",
    CALL print(trim(orders->name,3)), row + 1,
    "{CPI/12}{POS/390/82}", "{b}", "Fin Number: ",
    orders->fnbr, row + 1, "{bcr/250}{lpi/24}{CPI/8}{font/28/7}{POS/390/94}",
    CALL print(build("*",orders->fnbr,"*")), row + 1, reg_dio,
    " ", row + 1, "{CPI/12}{POS/35/97}",
    "Address: ", row + 1, "{CPI/12}{POS/110/97}",
    orders->street, row + 1, "{CPI/12}{POS/110/112}",
    orders->city_state_zip, row + 1, "{CPI/12}{POS/35/132}",
    "Date of Birth: ", "{CPI/12}{POS/110/132}", orders->dob,
    row + 1, "{CPI/12}{POS/260/132}", "Age: ",
    orders->age, row + 1, "{CPI/12}{POS/390/132}",
    "Sex: ", orders->sex, row + 1,
    "{CPI/12}{POS/35/152}", "Pt. Location:  ", "{CPI/12}{POS/110/152}",
    orders->location, row + 1, "{CPI/12}{POS/260/152}",
    "Ord Loc:  ", orders->order_location, row + 1,
    "{CPI/12}{POS/35/172}", "EMR#:  ", "{CPI/12}{POS/110/172}",
    orders->mrn, row + 1, "{CPI/12}{POS/260/172}",
    "Visit Number:  ",
    CALL print(cnvtint(request->order_qual[1].encntr_id)), row + 1,
    "{CPI/12}{POS/390/152}", "SS#:  ", orders->ssn,
    row + 1, "{CPI/12}{POS/35/192}", "Admit Dr:  ",
    "{CPI/12}{POS/460/192}", "Ht:  ",
    CALL print(trim(pt_height)),
    " ", ht_date, row + 1,
    "{CPI/12}{POS/460/202}", "Wt:  ",
    CALL print(trim(pt_weight)),
    " ", wt_date, row + 1,
    "{CPI/12}{POS/35/212}", "Reason for Visit:  ", "{CPI/12}{POS/120/212}",
    orders->admit_diagnosis, row + 1, "{CPI/12}{POS/35/232}",
    "Order: ", row + 1
   DETAIL
    FOR (vv = 1 TO value(num_of_orders))
      IF ((orders->qual[vv].display_ind=1))
       ...
       ENDIF
       reg_dio, "  ", row + 1,
       "{CPI/12}{POS/110/192}", orders->admitting_dr_num, " ",
       orders->admitting_dr, row + 1, "{CPI/12}{POS/260/192}",
       "Ord Dr:  ",
       CALL print(trim(provider)), " ",
       orders->qual[vv].order_provider_name, row + 1, "{CPI/12}{POS/120/232}",
       "{b}", orders->qual[vv].order_mnemonic, "{endb}",
       row + 1, "{CPI/13}{POS/340/740}", "Entered by: ",
       orders->qual[vv].action_prsnl_name, row + 1, xcol = 120,
       ycol = 247
       FOR (fsub = 1 TO 31)
         FOR (ww = 1 TO orders->qual[vv].detail_cnt)
           IF ((((orders->qual[vv].detail_qual[ww].group_seq=fsub)) OR (fsub=31
            ...
            orders->qual[vv].detail_qual[ww].print_ind = 1, "{CPI/12}",
            CALL print(calcpos(xcol,ycol)),
            CALL print(orders->qual[vv].detail_qual[ww].label_text), ":  ", xcol = 280
            IF ((orders->qual[vv].detail_qual[ww].field_id=357054))
             "{CPI/12}{b}",
             CALL print(calcpos(xcol,ycol)),
             CALL print(orders->qual[vv].detail_qual[ww].display_value),
             "{endb}", row + 1
            ELSE
             "{CPI/12}",
             CALL print(calcpos(xcol,ycol)),
             ...
             row + 1
            ENDIF
            xcol = 120, ycol = (ycol+ 15)
            IF (ycol > 620)
             xcol = 120, ycol = 247, BREAK
             IF (trim(uar_get_code_meaning(orders->qual[vv].action_type_cd))="ORDER")
              "{CPI/9}{POS/180/60}{f/5}{b}", orders->facility, "{endb}",
              row + 1
             ELSE
             ...
             "{b}", orders->qual[vv].order_mnemonic, "{endb}",
             row + 1, "{CPI/13}{POS/340/740}", "Entered by: ",
             orders->qual[vv].action_prsnl_name, row + 1
            ENDIF
           ENDIF
         ENDFOR
       ENDFOR
   ...
   FOOT PAGE
    ...
    "{CPI/13}{POS/135/680}", orders->fnbr, row + 1,
    "{CPI/13}{POS/185/680}", "Pt Loc: ", orders->location,
    row + 1, "{CPI/13}{POS/135/690}", orders->street,
    row + 1, "{CPI/13}{POS/135/700}", orders->city_state_zip,
    row + 1, "{CPI/13}{POS/135/710}", "DOB: ",
    orders->dob, row + 1, "{CPI/13}{POS/315/670}{b}",
    orders->name, row + 1, "{CPI/13}{POS/315/680}",
    orders->fnbr, row + 1, "{CPI/13}{POS/365/680}",
    "Pt Loc: ", orders->location, row + 1,
    "{CPI/13}{POS/315/690}", orders->street, row + 1,
    "{CPI/13}{POS/315/700}", orders->city_state_zip, row + 1,
    "{CPI/13}{POS/315/710}", "DOB: ", orders->dob,
    row + 1, "{CPI/13}{POS/35/740}", "Printed Dt/Tm: ",
    curdate"MM/DD/YYYY;;D", " ", curtime"HH:MM;;M",
    row + 1, "{CPI/13}{POS/185/740}", "Ordered Dt/Tm: ",
    signed, row + 1, "{CPI/13}{POS/520/740}",
    "Page: ", curpage"###", row + 1,
