CREATE PROGRAM bed_get_resource_children:dba
 ...
 DECLARE group_type_code = f8 WITH protect, constant(uar_get_code_by("MEANING",223,trim(cnvtupper(
     request->cdf_meaning))))
 ...
 SELECT
  IF ((request->get_all_flag=0)
   ...
   PLAN (r1
    WHERE (r1.parent_service_resource_cd=request->service_resource_cd)
     AND r1.resource_group_type_cd=group_type_code
     AND (r1.root_service_resource_cd=request->root_service_resource_cd)
     AND r1.active_ind=1
     AND r1.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
     AND r1.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)
     AND r1.active_status_cd != deleted_code)
    JOIN (sr
    WHERE r1.child_service_resource_cd=sr.service_resource_cd)
    JOIN (c
    ...
    WHERE d.seq=1)
    JOIN (r2
    WHERE r1.child_service_resource_cd=r2.parent_service_resource_cd
     AND (r2.root_service_resource_cd=request->root_service_resource_cd)
     AND r2.active_ind=1
     AND r2.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
     AND r2.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)
     AND r2.active_status_cd != deleted_code)
    JOIN (sr2
    WHERE r2.child_service_resource_cd=sr2.service_resource_cd)
  ...
  ENDIF
  INTO "nl:"
  r1.child_service_resource_cd, c.code_value
  FROM resource_group r1,
   ...
   organization o
  ORDER BY r1.child_service_resource_cd
  HEAD REPORT
   resource_cnt = 0, stat = alterlist(reply->resources,1)
  HEAD r1.child_service_resource_cd
   resource_cnt = (resource_cnt+ 1)
   IF (mod(resource_cnt,10)=2)
    stat = alterlist(reply->resources,(resource_cnt+ 9))
   ENDIF
   reply->resources[resource_cnt].child_service_resource_cd = r1.child_service_resource_cd, reply->
   resources[resource_cnt].active_ind = c.active_ind
   IF (c.active_ind=1
    AND c.begin_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
    AND c.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3))
   ...
   ENDIF
   reply->resources[resource_cnt].service_resource_type_mean = c.cdf_meaning
  DETAIL
   IF ((request->get_all_flag=0)
    AND r2.parent_service_resource_cd > 0
    AND r2.active_ind=1
    AND r2.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
    AND r2.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3))
   ...
   ELSEIF ((request->get_all_flag=1)
    ...
    reply->resources[resource_cnt].child_ind = 1
   ENDIF
  WITH outerjoin = d, nocounter, orahint("index(r1 xpkresource_group) index(r2 xpkresource_group)")
 ;end select
