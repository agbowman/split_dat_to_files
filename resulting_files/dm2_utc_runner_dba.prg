CREATE PROGRAM dm2_utc_runner:dba
 SET width = 132
 DECLARE dur_prompt_run_background(null) = i2
 DECLARE dur_get_conversion_status(null) = i2
 DECLARE dur_connect_str = vc WITH protect, noconstant(" ")
 DECLARE dur_dbase_name = vc WITH protect, noconstant("NOT_SET")
 ...
 SUBROUTINE dum_cust_incl_abort_gen(dciag_tgt_dbname,dciag_src_orcl_ver,dciag_tgt_orcl_ver)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_utc_runner",".log","dm2_utc_runner")=false)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_utc_runner"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF (trim(currdbhandle,3) > " ")
  IF (cnvtupper(currdbuser) IN ("V500", "CDBA"))
   IF (dm2_get_dbase_name(dur_dbase_name)=0)
    RETURN(0)
   ENDIF
   SET dm2_install_schema->dbase_name = dur_dbase_name
   SET dm2_install_schema->u_name = cnvtupper(currdbuser)
   IF (cnvtupper(dm2_install_schema->v500_p_word) != "NONE")
 ...
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No database connection or invalid user.")
 ...
 IF (val_user_privs(1)=0)
  GO TO end_program
 ENDIF
 IF (dur_get_conversion_status(null)=0)
  GO TO end_program
 ENDIF
 IF (dur_prompt_run_background(null)=0)
  GO TO end_program
 ENDIF
 GO TO end_program
 SUBROUTINE dur_prompt_run_background(null)
   DECLARE dprb_runner_script = vc WITH protect, noconstant("NOT SET")
   DECLARE dprb_logfile_name = vc WITH protect, noconstant("NOT SET")
   DECLARE dprb_prompt_str = vc WITH protect, noconstant("NOT SET")
   DECLARE dprb_prompt_str2 = vc WITH protect, noconstant("NOT SET")
   DECLARE dprb_command_str = vc WITH protect, noconstant("NOT SET")
   DECLARE dprb_dcl_err_ind = i2 WITH protect, noconstant(0)
   DECLARE dprb_file_ext = vc WITH protect, noconstant("NOT SET")
   DECLARE dprb_cerexe = vc WITH protect, constant(logical("cer_exe"))
   DECLARE dprb_debug_flag = vc WITH protect, noconstant(" ")
   DECLARE dprb_rdbdebug_flag = i2 WITH protect, noconstant(0)
   DECLARE dprb_rdbbind_flag = i2 WITH protect, noconstant(0)
   SET dprb_rdbdebug_flag = trace("RDBDEBUG")
   SET dprb_rdbbind_flag = trace("RDBBIND")
   SET dprb_debug_flag = cnvtstring(dm_err->debug_flag)
   IF (cursys="AIX")
    SET dprb_file_ext = ".ksh"
   ELSEIF (cursys="AXP")
    SET dprb_file_ext = ".com"
   ENDIF
   IF (get_unique_file("dm2_utc_runner_",dprb_file_ext)=0)
    GO TO end_program
   ENDIF
   SET dprb_runner_script = dm_err->unique_fname
   SET dprb_logfile_name = concat(substring(1,(size(trim(dprb_runner_script)) - 4),dprb_runner_script
     ),".out")
   IF (cursys="AIX")
    SET dprb_prompt_str = concat("A shell script ($CCLUSERDIR/",dprb_runner_script,
     ") was started in the background to run schema changes in parallel. ")
    SET dprb_prompt_str2 = concat(
     "This process will write all output to a separate log file ($CCLUSERDIR/",dprb_logfile_name,")."
     )
    SELECT INTO value(dprb_runner_script)
     d.seq
     FROM (dummyt d  WITH seq = 1)
     DETAIL
      col 0, "#!/usr/bin/ksh", row + 1,
      col 0, "#", row + 1,
      col 0, "# UTC Parallel Runner", row + 1,
      col 0, "#", row + 1,
      col 0, ". $cer_mgr/",
      CALL print(trim(cnvtlower(logical("environment")))),
      "_environment.ksh", row + 1, col 0,
      "ccl <<!", row + 1, col 0,
      "free define oraclesystem go", row + 1
      IF (trim(dm2_install_schema->connect_str,3) > "")
       col 0, "define oraclesystem 'v500/", dm2_install_schema->p_word,
       "@", dm2_install_schema->connect_str, "' go"
      ELSE
       col 0, "define oraclesystem 'v500/", dm2_install_schema->p_word,
       "' go"
      ENDIF
      row + 1, col 0, "dm2_set_context 'FIRE_EA_TRG', 'NO' go",
      row + 1, col 0, "dm2_set_context 'FIRE_REFCHG_TRG', 'NO' go",
      row + 1, col 0, "set dm2_debug_flag = ",
      dprb_debug_flag, " go", row + 1
      IF (dprb_rdbdebug_flag=1)
       col 0, "set trace rdbdebug go", row + 1
      ENDIF
      IF (dprb_rdbbind_flag=1)
       col 0, "set trace rdbbind go", row + 1
      ENDIF
      row + 1, col 0, "set dm2_utc_process_option = '",
      dm2_install_schema->process_option, "' go", row + 1,
      col 0, "execute dm2_utc_background_runner go", row + 1,
      col 0, "free define oraclesystem go", row + 1,
      col 0, "exit", row + 1,
      col 0, "!", row + 1,
      col 0, "sleep 30"
     WITH nocounter, format = variable, formfeed = none,
    ...
    IF (check_error(dm_err->eproc)=1)
     ...
     RETURN(0)
    ENDIF
    IF (dm2_push_dcl(concat("chmod ugo+x ",dprb_runner_script))=0)
     RETURN(0)
    ENDIF
    SET dprb_command_str = concat("nohup $CCLUSERDIR/",dprb_runner_script," > $CCLUSERDIR/",
     dprb_logfile_name," 2>&1 ",
     dprb_logfile_name," &")
    CALL dcl(dprb_command_str,size(dprb_command_str),dprb_dcl_err_ind)
    IF (dprb_dcl_err_ind=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat(
      "An error was encountered. See the following file for more information: ",dprb_logfile_name)
     SET dm_err->eproc = "Submitting job to background."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
   ELSEIF (cursys="AXP")
    SET dprb_prompt_str = concat("A command procedure (CCLUSERDIR:",dprb_runner_script,
     ") was submitted to batch to run schema changes in parallel. ")
    SET dprb_prompt_str2 = concat(
     "This process will write all output to a separate log file (CCLUSERDIR:",dprb_logfile_name,"). "
     )
    SELECT INTO value(dprb_runner_script)
    ...
    IF (check_error(dm_err->eproc)=1)
     ...
     RETURN(0)
    ENDIF
    IF (dm2_push_dcl(concat("set file/prot=(s:RWED, o:RWED, g:RWED, w:RWED) ",dprb_runner_script))=0)
     RETURN(0)
    ENDIF
    IF (dir_setup_batch_queue(dir_batch_queue)=0)
     RETURN(0)
    ENDIF
    SET dprb_command_str = concat("submit /queue=",dir_batch_queue," /log=CCLUSERDIR:",
     dprb_logfile_name," CCLUSERDIR:",
     dprb_runner_script)
    IF (dm2_push_dcl(dprb_command_str)=0)
     RETURN(0)
    ENDIF
    SET dprb_command_str = concat("$show queue /all_jobs ",dir_batch_queue)
    SET dm_err->disp_dcl_err_ind = 0
    IF (dm2_push_dcl(dprb_command_str)=0)
     IF ((dm_err->err_ind=1))
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (parse_errfile(dm_err->errfile)=0)
     RETURN(0)
    ENDIF
    IF (findstring(cnvtupper(trim(replace(dprb_runner_script,".com","",0))),cnvtupper(dm_err->errtext
      ))=0)
     SET dm_err->disp_msg_emsg = "Unable to locate process for this job."
     SET dm_err->emsg = dm_err->disp_msg_emsg
     SET dm_err->eproc = concat("Validating ",trim(dprb_runner_script)," was successfully executed.")
     SET dm_err->err_ind = 1
     CALL disp_msg("dm_err->disp_msg_emsg",dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ...
   CALL clear(1,1)
   CALL box(1,1,13,132)
   CALL text(4,3,dprb_prompt_str)
   CALL text(6,3,dprb_prompt_str2)
   CALL text(8,3,
    "Note:  You can execute this command multiple times to initiate additional parallel UTC runners."
    )
   CALL text(9,3,"       Cerner recommends that you run no more than 4 parallel UTC runners at once."
    )
   CALL text(11,3,"Press enter to continue.")
   CALL accept(11,28,"P;CHU","X")
   SET message = nowindow
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dur_get_conversion_status(null)
   SET dm_err->eproc = "Checking for UTC Conversion status."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    di.info_name
    FROM dm2_admin_dm_info di
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ELSEIF (curqual=0)
    SET dm_err->eproc = "UTC Conversion not started.  Exiting."
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(0)
   ELSEIF ( NOT ((dum_utc_data->status IN ("UPTIME", "DOWNTIME"))))
    SET dm_err->eproc = "UTC Conversion not UPTIME or DOWNTIME mode.  Exiting."
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(0)
   ENDIF
   IF (dm2_get_appl_status(dum_utc_data->appl_id)="I")
    SET dm_err->eproc = "The main background process is not active.  Exiting."
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(0)
 ...
 IF (cnvtupper(dm2_install_schema->p_word) != "NONE")
  IF ((dm2_install_schema->connect_str > " "))
   SET dur_connect_str = cnvtlower(build(dm2_install_schema->u_name,"/",dm2_install_schema->p_word,
     "@",dm2_install_schema->connect_str))
  ELSE
   SET dur_connect_str = cnvtlower(build(dm2_install_schema->u_name,"/",dm2_install_schema->p_word))
  ENDIF
  CALL parser(concat("define oraclesystem '",trim(dur_connect_str),"' go"),1)
 ELSE
  CALL echo("*")
  CALL echo("**************************************************************************")
  ...
  CALL echo("*")
 ENDIF
 SET dm_err->eproc = "Ending dm2_utc_runner"
 CALL final_disp_msg("dm2_utc_runner")
END GO
