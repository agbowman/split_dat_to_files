CREATE PROGRAM dm_pucb_person_plan_auth_r:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "PERSON_PLAN_AUTH_R"
  SET dcem_request->qual[1].op_type = "UNCOMBINE"
  SET dcem_request->qual[1].script_name = "dm_pucb_person_plan_auth_r"
 ...
 SUBROUTINE cust_ucb_add(dummy)
   ...
   SELECT INTO "nl:"
    p.seq
    FROM person_plan_auth_r p
    WHERE (p.person_plan_auth_r_id=rchildren->qual1[det_cnt].entity_id)
    DETAIL
     authorization_id = p.authorization_id
   ;end select
   FREE SET cust_add_buff
