CREATE PROGRAM dm_cv_miss_xlat_cleanup:dba
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 SET dm_err->eproc = "Starting dm_cv_miss_xlat_cleanup"
 IF (check_logfile("DM_CV_XLAT_CLEANUP",".log","DM_CV_MISS_XLAT_CLEANUP")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DECLARE source_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE mock_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE merge_link = vc WITH protect, noconstant("")
 DECLARE target_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE src_dcl_tab_name = vc WITH protect, noconstant("")
 DECLARE src_dmt_tab_name = vc WITH protect, noconstant("")
 DECLARE upd_idx = i4 WITH protect, noconstant(0)
 DECLARE del_idx = i4 WITH protect, noconstant(0)
 DECLARE cv_idx = i4 WITH protect, noconstant(0)
 DECLARE cleanup_batch_size = i4 WITH protect, noconstant(0)
 DECLARE expand_batch_size = i4 WITH protect, constant(200)
 ...
 DECLARE commit_cnt = i4 WITH protect, noconstant(0)
 FREE RECORD cv_missing_xlats
 RECORD cv_missing_xlats(
   1 cv_cnt = i4
   1 qual[*]
     2 code_value = f8
     2 log_id = f8
 ...
 IF (reflect(parameter(1,0)) != "I*"
  ...
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Expected syntax: dm_cv_miss_xlat_cleanup <Source Environment_ID>")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (( $1 < 1))
 ...
 ENDIF
 SET source_env_id =  $1
 SET dm_err->eproc = "Checking if code_value $R table exists"
 SELECT INTO "nl:"
  FROM user_tables
  WHERE table_name="CODE_VALUE0619$R"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "$R table does not exist exiting program"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Checking if code_value $R table exists"
 SELECT INTO "nl:"
  FROM dtable dt
  WHERE dt.table_name="CODE_VALUE0619$R"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  EXECUTE oragen3 "CODE_VALUE0619$R"
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Retrieve target and mock environment ids"
 SELECT INTO "NL:"
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="DM_ENV_ID"
  DETAIL
   target_env_id = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET mock_env_id = target_env_id
 ENDIF
 SET dm_err->eproc = "Getting merge_link from dm_env_reltn"
 SELECT INTO "NL:"
  FROM dm_env_reltn der
  WHERE der.child_env_id=target_env_id
   AND der.relationship_type="REFERENCE MERGE"
   AND der.parent_env_id=source_env_id
  DETAIL
   merge_link = der.post_link_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("A relationship with a source_env_id of :",cnvtstring(source_env_id),
   " was not found.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting cleanup batch size from dm_info"
 SELECT INTO "NL:"
  FROM dm_info di
  WHERE di.info_domain="RDDS CLEANUP BATCH SIZE"
   AND di.info_name="DM_CV_MISS_XLAT_CLEANUP"
  DETAIL
   cleanup_batch_size = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET cleanup_batch_size = 10000
 ENDIF
 SET commit_cnt = (cleanup_batch_size/ expand_batch_size)
 SET src_dcl_tab_name = concat("DM_CHG_LOG",merge_link)
 SET src_dmt_tab_name = concat("DM_MERGE_TRANSLATE",merge_link)
 SET dm_err->eproc = "Get $R rows to reset"
 SELECT INTO "nl:"
  FROM code_value0619$r cv
  WHERE  NOT ( EXISTS (
  (SELECT
   "x"
   FROM (parser(src_dmt_tab_name) sdmt)
   WHERE cv.code_value=sdmt.from_value
    AND sdmt.table_name="CODE_VALUE"
    AND sdmt.env_source_id=mock_env_id
    AND sdmt.env_target_id=source_env_id)))
   AND  NOT ( EXISTS (
  (SELECT
   "x"
   FROM dm_merge_translate dmt
   WHERE cv.code_value=dmt.to_value
    AND dmt.table_name="CODE_VALUE"
    AND dmt.env_source_id=source_env_id
    AND dmt.env_target_id=mock_env_id)))
   AND cv.rdds_status_flag < 9000
   AND cv.rdds_source_env_id=source_env_id
  HEAD REPORT
   cv_missing_xlats->cv_cnt = 0
  DETAIL
   cv_missing_xlats->cv_cnt = (cv_missing_xlats->cv_cnt+ 1)
   IF (mod(cv_missing_xlats->cv_cnt,10)=1)
    stat = alterlist(cv_missing_xlats->qual,(cv_missing_xlats->cv_cnt+ 9))
   ENDIF
   cv_missing_xlats->qual[cv_missing_xlats->cv_cnt].code_value = cv.code_value, cv_missing_xlats->
   qual[cv_missing_xlats->cv_cnt].log_id = cv.rdds_log_id
  FOOT REPORT
   stat = alterlist(cv_missing_xlats->qual,cv_missing_xlats->cv_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((cv_missing_xlats->cv_cnt > 0))
  SET cur_list_size = cv_missing_xlats->cv_cnt
  SET loop_cnt = ceil((cnvtreal(cur_list_size)/ expand_batch_size))
  SET new_list_size = (loop_cnt * expand_batch_size)
  SET stat = alterlist(cv_missing_xlats->qual,new_list_size)
  SET nstart = 1
  FOR (cv_idx = (cur_list_size+ 1) TO new_list_size)
   SET cv_missing_xlats->qual[cv_idx].code_value = cv_missing_xlats->qual[cur_list_size].code_value
   SET cv_missing_xlats->qual[cv_idx].log_id = cv_missing_xlats->qual[cur_list_size].log_id
  ENDFOR
  FOR (upd_idx = 1 TO loop_cnt)
    SET dm_err->eproc = "Reset dm_chg_log rows referencing Code_value missing xlats"
    UPDATE  FROM (parser(src_dcl_tab_name) dcl)
     SET dcl.log_type = "REFCHG"
     WHERE expand(cv_idx,nstart,(nstart+ (expand_batch_size - 1)),dcl.log_id,cv_missing_xlats->qual[
      cv_idx].log_id)
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
     ...
     GO TO exit_program
    ENDIF
    IF (mod(upd_idx,commit_cnt)=0)
     COMMIT
    ENDIF
    SET nstart = (nstart+ expand_batch_size)
  ENDFOR
  IF (mod(loop_cnt,commit_cnt) != 0)
   COMMIT
  ENDIF
  SET nstart = 1
  FOR (del_idx = 1 TO loop_cnt)
    SET dm_err->eproc = "Deleting code_value rows that are missing xlats"
    DELETE  FROM code_value0619$r cv
     WHERE expand(cv_idx,nstart,(nstart+ (expand_batch_size - 1)),cv.code_value,cv_missing_xlats->
      qual[cv_idx].code_value)
     WITH nocounter
    ;end delete
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "End of dm_cv_miss_xlat_cleanup"
 CALL final_disp_msg("DM_CV_XLAT_CLEANUP")
#exit_program
 FREE RECORD cv_missing_xlats
END GO
