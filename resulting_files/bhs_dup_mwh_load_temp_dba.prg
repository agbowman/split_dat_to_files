CREATE PROGRAM bhs_dup_mwh_load_temp:dba
 FREE DEFINE rtl3
 ...
 UPDATE  FROM cust_dup_mrn j,
   (dummyt d  WITH seq = value(l_cnt))
  SET j.bwh_mrn = temp_rec->list[d.seq].facility_id
  WHERE j.cn=format(temp_rec->list[d.seq].corp_mrn,"#######;rp0")
   AND textlen(temp_rec->list[d.seq].inactive_date)=0
  WITH nocounter
 ;end update
 ROLLBACK
#exit_program
