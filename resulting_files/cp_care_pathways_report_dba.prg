CREATE PROGRAM cp_care_pathways_report:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Care Pathway Name: " = "",
  "Days Back: " = 30
  WITH outdev, pathway_name, days_back
 ...
 FREE RECORD pathway_record
 RECORD pathway_record(
   1 questions_answers[*]
     2 pathway_name = vc
     2 node_name = vc
     2 question_name = vc
 ...
 DECLARE nodecnt = i2 WITH protect, noconstant(0)
 DECLARE actioncnt = i2 WITH protect, noconstant(0)
 DECLARE pwname = vc WITH protect
 DECLARE activestatus = f8 WITH protect, constant(uar_get_code_by("MEANING",4003198,"ACTIVE"))
 IF (( $PATHWAY_NAME > ""))
  SET pwname =  $PATHWAY_NAME
 ELSE
  SET pwname = "*"
 ENDIF
 SELECT INTO "nl:"
  FROM cp_pathway_action_detail cpad,
   ...
   prsnl pl
  PLAN (cp
   WHERE cp.pathway_name=patstring(pwname)
    AND cp.pathway_status_cd=activestatus)
   JOIN (cn
   WHERE cn.cp_pathway_id=cp.cp_pathway_id
    AND cn.active_ind=1)
   JOIN (cpa
   WHERE cpa.cp_node_id=cn.cp_node_id
    AND cpa.action_dt_tm >= cnvtdatetime((curdate - value( $DAYS_BACK)),0000))
   JOIN (cpad
   WHERE cpad.cp_pathway_action_id=cpa.cp_pathway_action_id)
   JOIN (lt
   WHERE lt.long_text_id=cpad.action_detail_entity_id)
   JOIN (pl
   WHERE pl.person_id=cpa.prsnl_id)
  ORDER BY cp.pathway_name, cn.node_name
  DETAIL
   actioncnt = (actioncnt+ 1), stat = alterlist(pathway_info->actions,actioncnt), pathway_info->
   actions[actioncnt].pathway_name = cp.pathway_name,
   pathway_info->actions[actioncnt].node_name = cn.node_name, pathway_info->actions[actioncnt].
   username = pl.username, pathway_info->actions[actioncnt].details = lt.long_text,
   pathway_info->actions[actioncnt].action_dt_tm = format(cpa.action_dt_tm,
    "DD-MMM-YYYY HH:MM:SS:CC;3;Q")
  WITH nocounter
 ;end select
 CALL echorecord(pathway_info)
 SET j = 1
 FOR (i = 1 TO actioncnt)
  SET jrec = cnvtjsontorec(pathway_info->actions[i].details)
  FOR (k = 1 TO size(action_log->actions,5))
    SET stat = alterlist(pathway_record->questions_answers,j)
    SET pathway_record->questions_answers[j].pathway_name = pathway_info->actions[i].pathway_name
    SET pathway_record->questions_answers[j].node_name = pathway_info->actions[i].node_name
    SET pathway_record->questions_answers[j].username = pathway_info->actions[i].username
    SET pathway_record->questions_answers[j].question_name = action_log->actions[k].group_display
    SET pathway_record->questions_answers[j].unique_id = action_log->actions[k].term_ocid
    SET pathway_record->questions_answers[j].action_dt_tm = pathway_info->actions[i].action_dt_tm
    IF ((action_log->actions[k].term_type IN ("INPUT_ALPHA", "INPUT_NUMERIC")))
     SET pathway_record->questions_answers[j].answer_name = action_log->actions[k].term_state
    ELSEIF ((action_log->actions[k].group_type IN ("Single", "Multiple"))
     AND (action_log->actions[k].term_state="true"))
     SET pathway_record->questions_answers[j].answer_name = action_log->actions[k].term_display
    ELSEIF ((action_log->actions[k].group_type="YesNo")
     AND (action_log->actions[k].term_state="true"))
     SET pathway_record->questions_answers[j].answer_name = "Yes"
    ELSEIF ((action_log->actions[k].group_type="YesNo")
     AND (action_log->actions[k].term_state="false"))
     SET pathway_record->questions_answers[j].answer_name = "No"
    ENDIF
    SET pathway_record->questions_answers[j].answer_name = replace(pathway_record->questions_answers[
     j].answer_name,"'"," ")
    SET pathway_record->questions_answers[j].answer_name = replace(pathway_record->questions_answers[
     j].answer_name,","," ")
    SET pathway_record->questions_answers[j].answer_name = replace(pathway_record->questions_answers[
  ...
  ENDFOR
 ENDFOR
 CALL echorecord(pathway_record)
 IF (( $OUTDEV != "NOFORMS"))
  CALL putjsonrecordtofile(pathway_record)
 ENDIF
 DECLARE at_string = vc
 DECLARE header_string = vc
 DECLARE curdatetime = dq8 WITH constant(cnvtdatetime(curdate,curtime3))
 DECLARE filestring = vc WITH constant(concat("ccluserdir:pathway_report",cnvtstring(curdatetime),
   ".csv"))
 CALL echo(filestring)
 SELECT INTO patstring(filestring)
  FROM (dummyt d  WITH seq = size(pathway_record->questions_answers,5))
  PLAN (d)
  HEAD REPORT
   header_string = "Care Pathway,Treatment Line,Question,Answer,Username,Action Date,Unique ID", col
   0, header_string,
   row + 1
  DETAIL
   at_string = build('"',pathway_record->questions_answers[d.seq].pathway_name,'","',pathway_record->
    questions_answers[d.seq].node_name,'","',
    pathway_record->questions_answers[d.seq].question_name,'","',pathway_record->questions_answers[d
    .seq].answer_name,'","',pathway_record->questions_answers[d.seq].username,
    '","',pathway_record->questions_answers[d.seq].action_dt_tm,'","',pathway_record->
    questions_answers[d.seq].unique_id,'"'), col 0, at_string,
   row + 1
  WITH nocounter, format = pcformat, maxrow = 5000,
   maxcol = 2500
 ;end select
END GO
