CREATE PROGRAM dm_get_ocd_constraints:dba
 FREE SET reply
 RECORD reply(
   1 qual[*]
     2 table_name = vc
 ...
 FOR (x = 1 TO tbl_cnt)
   ...
   IF (ocd_flag=1)
    SELECT DISTINCT INTO "nl:"
     ...
     DETAIL
      reply->qual[reply->count].cons_knt = (reply->qual[reply->count].cons_knt+ 1), stat = alterlist(
       reply->qual[reply->count].cons,reply->qual[reply->count].cons_knt), reply->qual[reply->count].
      cons[reply->qual[reply->count].cons_knt].constraint_name = d.constraint_name,
      reply->qual[reply->count].cons[reply->qual[reply->count].cons_knt].cons_status = "N"
     WITH nocounter
    ;end select
   ELSE
    SELECT DISTINCT INTO "nl:"
    ...
    IF (ocd_flag=1)
     SELECT DISTINCT INTO "nl:"
      ...
      DETAIL
       reply->qual[reply->count].cons_knt = (reply->qual[reply->count].cons_knt+ 1), stat = alterlist
       (reply->qual[reply->count].cons,reply->qual[reply->count].cons_knt), reply->qual[reply->count]
       .cons[reply->qual[reply->count].cons_knt].constraint_name = a.constraint_name,
       reply->qual[reply->count].cons[reply->qual[reply->count].cons_knt].cons_status = "Y"
      WITH nocounter
     ;end select
    ELSE
     SELECT DISTINCT INTO "nl:"
