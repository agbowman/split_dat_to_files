CREATE PROGRAM bbd_rdm_enable_app_activation:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "BBD_RDM_ENABLE_APP_ACTIVATION"
 CALL logscriptstart(sreadme_name)
 SET readme_data->status = "F"
 DECLARE info_domain = vc WITH protect, constant("PATHNET BBD")
 DECLARE info_name = vc WITH protect, constant("ENABLE BBD APPS")
 DECLARE info_char = vc WITH protect, constant("Y")
 DECLARE nstat = i2 WITH protect, noconstant(0)
 DECLARE findenablerow(null) = i2
 SET nstat = findenablerow(null)
 IF (nstat=1)
  CALL logstatus(build(sreadme_name,"--Row already exists."),"S")
  GO TO exit_script
 ENDIF
 CALL initdminfoinsert(0)
 IF (insertdminfo(0.0,info_domain,info_name,info_char,0.0)=0)
  ROLLBACK
  CALL logstatus(build(sreadme_name,"--Failed inserting row on DM_INFO."),"F")
  GO TO exit_script
 ENDIF
 ...
 ELSE
  ROLLBACK
  CALL logstatus(build(sreadme_name,"--Failed! DM_INFO row not inserted."),"F")
 ENDIF
 SUBROUTINE findenablerow(null)
  SELECT INTO "nl:"
   FROM dm_info d
   PLAN (d
    WHERE d.info_domain=info_domain
     AND d.info_name=info_name
     AND d.info_char=info_char)
   WITH nocounter
  ;end select
  IF (curqual != 0)
   RETURN(1)
