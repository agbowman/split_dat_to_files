CREATE PROGRAM dm_get_target_schema:dba
 SET tgtdb->tbl_cnt = 0
 SET tcnt = 0
 SET tc_cnt = 0
 SET icnt = 0
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    IF (sd_null_ind=0
     ...
     tgtdb->tbl[tcnt].schema_date = cnvtdatetime(a2.schema_date), tgtdb->tbl[tcnt].alpha_feature_nbr
      = a2.alpha_feature_nbr, tgtdb->tbl[tcnt].tspace_name = a2.tablespace_name
    ENDIF
 ...
 IF ((tgtdb->tbl_cnt > 0))
  SELECT
   ...
   HEAD c.index_name
    ...
    tgtdb->tbl[d.seq].ind[icnt].ind_col_cnt = 0
   DETAIL
