CREATE PROGRAM cp_get_suggested_pathways:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Person ID" = 0.0,
  "Encntr ID" = 0.0,
  "Pathway Type Code" = 0.0
  WITH outdev, input_person_id, input_encntr_id,
  pathway_type_cd
 ...
 DECLARE person_id = dq8 WITH constant(cnvtreal( $INPUT_PERSON_ID))
 DECLARE encntr_id = dq8 WITH constant(cnvtreal( $INPUT_ENCNTR_ID))
 DECLARE active_pathway_cd = f8 WITH constant(uar_get_code_by("MEANING",4003198,"ACTIVE")), protect
 DECLARE user_logical_domain_id = f8 WITH constant(getuserlogicaldomain(reqinfo->updt_id)), protect
 DECLARE main(null) = null
 IF ( NOT (validate(ptnomenids)))
  RECORD ptnomenids(
    1 cnt = i4
    1 qual[*]
      2 nomenclature_id = f8
      2 nomenclature_display = vc
      2 problem_ind = i2
  )
 ENDIF
 IF ( NOT (validate(cpm)))
 ...
 SUBROUTINE getptnomenids(null)
   ...
   DECLARE active_problem_cd = f8 WITH constant(uar_get_code_by("MEANING",12030,"ACTIVE")), protect
   DECLARE cnt = i4 WITH noconstant(0), protect
   DECLARE idx = i4 WITH noconstant(0), protect
   CALL log_message("Begin getPtNomenIds()",log_level_debug)
   SELECT INTO "nl:"
    FROM diagnosis d,
     nomenclature n
    PLAN (d
     WHERE d.person_id=person_id
      AND d.encntr_id=encntr_id
     ...
     WHERE ((n.nomenclature_id=d.originating_nomenclature_id
      AND d.originating_nomenclature_id > 0) OR (n.nomenclature_id=d.nomenclature_id
      AND d.nomenclature_id > 0)) )
    DETAIL
     cnt += 1
     IF (cnt > size(ptnomenids->qual,5))
      stat = alterlist(ptnomenids->qual,(cnt+ 10))
     ENDIF
     ptnomenids->qual[cnt].nomenclature_id = d.nomenclature_id
     IF (trim(d.diag_ftdesc) != "")
      ptnomenids->qual[cnt].nomenclature_display = d.diag_ftdesc
     ELSEIF (trim(d.diagnosis_display) != "")
      ptnomenids->qual[cnt].nomenclature_display = d.diagnosis_display
     ELSEIF (((d.originating_nomenclature_id > 0
      AND d.originating_nomenclature_id=n.nomenclature_id) OR (d.nomenclature_id > 0
      AND d.nomenclature_id=n.nomenclature_id)) )
      ptnomenids->qual[cnt].nomenclature_display = n.source_string
     ENDIF
    FOOT REPORT
     ptnomenids->cnt = cnt, stat = alterlist(ptnomenids->qual,cnt)
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    FROM problem pr,
    ...
    PLAN (pr
     WHERE pr.person_id=person_id
      AND pr.problem_id > 0
      AND pr.nomenclature_id > 0
      AND pr.life_cycle_status_cd=active_problem_cd
      AND pr.beg_effective_dt_tm <= cnvtdatetime(current_date_time)
      AND pr.end_effective_dt_tm >= cnvtdatetime("31-DEC-2100"))
     JOIN (n
     WHERE ((n.nomenclature_id=pr.originating_nomenclature_id
      AND pr.originating_nomenclature_id > 0) OR (n.nomenclature_id=pr.nomenclature_id
      AND pr.nomenclature_id > 0)) )
    DETAIL
     IF (locateval(idx,1,ptnomenids->cnt,pr.nomenclature_id,ptnomenids->qual[idx].nomenclature_id)=0)
      cnt += 1
      IF (cnt > size(ptnomenids->qual,5))
       stat = alterlist(ptnomenids->qual,(cnt+ 10))
      ENDIF
      ptnomenids->qual[cnt].nomenclature_id = pr.nomenclature_id, ptnomenids->qual[cnt].problem_ind
       = 1
      IF (trim(pr.problem_ftdesc) != "")
       ptnomenids->qual[cnt].nomenclature_display = pr.problem_ftdesc
      ELSEIF (trim(pr.annotated_display) != "")
       ptnomenids->qual[cnt].nomenclature_display = pr.annotated_display
      ELSEIF (((pr.originating_nomenclature_id > 0
       AND pr.originating_nomenclature_id=n.nomenclature_id) OR (pr.nomenclature_id > 0
       AND pr.nomenclature_id=n.nomenclature_id)) )
       ptnomenids->qual[cnt].nomenclature_display = n.source_string
      ENDIF
     ENDIF
    FOOT REPORT
    ...
    WITH nocounter
   ;end select
   CALL log_message(build("Exit getPtNomenIds(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getisasuggestedpathways(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   ...
   DECLARE nomen_cnt = i4 WITH noconstant(0), protect
   DECLARE nomen_pos = i4 WITH noconstant(0), protect
   DECLARE idx = i4 WITH noconstant(0), protect
   CALL log_message("Begin getIsASuggestedPathways()",log_level_debug)
   SELECT
    IF (( $PATHWAY_TYPE_CD > 0.0))
     FROM cp_pathway cp,
      cp_concept_group ccg,
      cp_concept_group_dtl ccgd
     PLAN (cp
      WHERE cp.pathway_status_cd=active_pathway_cd
       AND (cp.pathway_type_cd= $PATHWAY_TYPE_CD)
      ...
      WHERE ccgd.concept_entity_name="NOMENCLATURE"
       ...
       AND expand(idx,1,ptnomenids->cnt,ccgd.concept_entity_id,ptnomenids->qual[idx].nomenclature_id)
      )
    ...
    HEAD cp.cp_pathway_id
     nomen_cnt = 0, pathway_cnt += 1
     IF (pathway_cnt > size(reply->suggested_pathways,5))
      stat = alterlist(reply->suggested_pathways,(pathway_cnt+ 10))
     ENDIF
     reply->suggested_pathways[pathway_cnt].pathway_id = cp.cp_pathway_id, reply->suggested_pathways[
     pathway_cnt].pathway_type_cd = cp.pathway_type_cd, reply->suggested_pathways[pathway_cnt].
     pathway_name = cp.pathway_name,
     reply->suggested_pathways[pathway_cnt].concept_cd = cp.cp_concept_cd
    HEAD ccgd.concept_entity_id
     nomen_pos = locateval(idx,1,ptnomenids->cnt,ccgd.concept_entity_id,ptnomenids->qual[idx].
      nomenclature_id)
     IF (nomen_pos > 0)
      nomen_cnt += 1
      IF (nomen_cnt > size(reply->suggested_pathways[pathway_cnt].qualifying_nomenclature,5))
       stat = alterlist(reply->suggested_pathways[pathway_cnt].qualifying_nomenclature,(nomen_cnt+ 10
        ))
      ENDIF
      stat = movereclist(ptnomenids->qual,reply->suggested_pathways[pathway_cnt].
       qualifying_nomenclature,nomen_pos,0,1,
       true)
     ENDIF
    FOOT  cp.cp_pathway_id
     reply->suggested_pathways[pathway_cnt].qualifying_nomenclature_cnt = nomen_cnt, stat = alterlist
     (reply->suggested_pathways[pathway_cnt].qualifying_nomenclature,nomen_cnt)
    FOOT REPORT
     reply->suggested_pathway_cnt = pathway_cnt, stat = alterlist(reply->suggested_pathways,
      pathway_cnt)
    WITH nocounter
   ;end select
   CALL log_message(build("Exit getIsASuggestedPathways(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE main(null)
   SET reply->person_id = person_id
   SET reply->encounter_id = encntr_id
   CALL getptnomenids(null)
   IF ((ptnomenids->cnt > 0))
    CALL getisasuggestedpathways(null)
    SET reply->status_data.status = "S"
   ELSE
    CALL log_message("Could not find any nomenclature IDs for the person.",log_level_debug)
