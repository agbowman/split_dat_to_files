CREATE PROGRAM dm2_ref_data_mover_3203:dba
 ...
 ELSEIF (inactivate_ind=0
  ...
  CALL version_exception(cust_tab_name,"",cnvtreal(rs_3203->from_values.dcp_section_instance_id))
 ELSE
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[inst_num].translated = 1
 ENDIF
 IF (((active_ind=1) OR (inactivate_ind=1)) )
  ...
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[ref_num].translated=0))
   ...
   IF ((ref_val=- (3)))
    IF (inactivate_ind=1)
     CALL echo("DM2_REF_DATA_MOVER_3203:The inactive section doesn't exist in target")
   ...
   ELSEIF ((ref_val=- (2)))
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "Multiple values returned with UI"
    SET stat = alterlist(dm2_ref_data_reply->qual,0)
    GO TO exit_3203
   ELSE
    CALL put_value(cust_tab_name,"DCP_SECTION_REF_ID",cnvtstring(ref_val))
  ...
  IF (idcd_check=1)
   ...
   SET from_id = cnvtreal(get_value(cust_tab_name,"DCP_SECTION_REF_ID","FROM"))
   IF (cust_beg_eff_ind != 1)
    UPDATE  FROM dcp_section_ref d
     SET d.active_ind = 0, d.end_effective_dt_tm = cnvtdatetime(cur_dt_tm)
     WHERE (d.dcp_section_ref_id=rs_3203->to_values.dcp_section_ref_id)
      AND d.active_ind=1
     WITH nocounter
    ;end update
   ENDIF
