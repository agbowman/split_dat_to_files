CREATE PROGRAM cv_update_procedure:dba
 IF (validate(stat)=0)
 ...
 FOR (proc_idx = 1 TO proc_cnt)
  ...
  ENDFOR
 ENDFOR
 EXECUTE cv_save_procedure  WITH replace("REQUEST",request), replace("REPLY",reply)
 IF ((reply->status_data.status != "S"))
  CALL cv_log_stat(cv_warning,"EXECUTE",reply->status_data.status,"CV_SAVE_PROCEDURE","")
  GO TO exit_script
 ENDIF
...
#exit_script
 ...
 IF ((reqdata->loglevel >= cv_debug))
  CALL echorecord(reply)
 ENDIF
 CALL cv_log_msg_post("000 31/03/23 AS043139")
END GO
