CREATE PROGRAM dm2_aa_add_srs_trigger:dba
 ...
 IF (check_logfile(dact_logfile_name,".log","AA_ADD_SRS_TRIGGER LOG ...") != 1)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "BEGINNING AA_ADD_SRS_TRIGGER.PRG ..."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (currdb="ORACLE")
  SET tcnt = 1
 ...
 ELSEIF (currdb="DB2UDB")
  SET tcnt = 3
  SET stat = alterlist(tlist->qual,tcnt)
  SET tlist->qual[1].tbl_name = "PRSNL_SERVICE_7215"
  SET tlist->qual[1].full_tbl_name = "PRSNL_SERVICE_RESOURCE_RELTN"
  SET tlist->qual[1].trig_name = "TRG_SRS_INS"
  SET tlist->qual[1].trig_type = "INSERT"
  SET tlist->qual[2].tbl_name = "PRSNL_SERVICE_7215"
  SET tlist->qual[2].full_tbl_name = "PRSNL_SERVICE_RESOURCE_RELTN"
  SET tlist->qual[2].trig_name = "TRG_SRS_UPDT"
  SET tlist->qual[2].trig_type = "UPDATE"
  SET tlist->qual[3].tbl_name = "PRSNL_SERVICE_7215"
  SET tlist->qual[3].full_tbl_name = "PRSNL_SERVICE_RESOURCE_RELTN"
  SET tlist->qual[3].trig_name = "TRG_SRS_DEL"
 ...
 FOR (cnt = 1 TO tcnt)
   ...
   ELSEIF (currdb="DB2UDB")
    IF (dact_validate_trigger(cnt) != 1)
     GO TO exit_program
    ENDIF
    SET str->str = concat("create trigger ",value(dact_trigger_name))
    SET str->str = concat(str->str," after ",tlist->qual[cnt].trig_type," on ",value(
      dact_trigger_tbl_name))
    CASE (tlist->qual[cnt].trig_type)
     OF "INSERT":
      SET str->str = concat(str->str," referencing new as new ")
     OF "UPDATE":
      SET str->str = concat(str->str," referencing new as new old as old ")
     OF "DELETE":
      SET str->str = concat(str->str," referencing old as old ")
    ENDCASE
    SET str->str = concat(str->str," for each row mode db2sql ")
    SET str->str = concat(str->str," begin atomic ")
    SET str->str = concat(str->str," declare rcount integer; ")
    SET str->str = concat(str->str," declare v_str varchar(70); ")
    SET str->str = concat(str->str," 	update dm_info ")
    SET str->str = concat(str->str," 	set info_char = '",value(tlist->qual[cnt].full_tbl_name),"',")
    SET str->str = concat(str->str," 	    info_date = current timestamp, ")
    SET str->str = concat(str->str,"		updt_dt_tm = current timestamp, ")
    SET str->str = concat(str->str,"		updt_cnt = updt_cnt + 1 ")
    SET str->str = concat(str->str,"	where info_domain = 'SRS' ")
    SET str->str = concat(str->str,"      and info_name = 'SRS UPDATE'; ")
    SET str->str = concat(str->str,"	get diagnostics rcount = ROW_COUNT; ")
    SET str->str = concat(str->str,"  if (rcount = 0) then ")
    SET str->str = concat(str->str,"	  insert into dm_info ")
    SET str->str = concat(str->str,"		(info_domain, info_name, info_date,",
     "       info_char, updt_dt_tm, updt_cnt) ")
    SET str->str = concat(str->str,"	  values ")
    SET str->str = concat(str->str,"	 ('SRS', 'SRS UPDATE',current timestamp,","'",value(tlist->qual[
      cnt].full_tbl_name),"',",
     "current timestamp, 0 ); ")
    SET str->str = concat(str->str,"	   get diagnostics rcount = ROW_COUNT; ")
    SET str->str = concat(str->str,"	   if(rcount != 1) then ")
    SET str->str = concat(str->str,"	     set v_str= 'Bad diagnostic row_count value ('",
     "||rtrim(char(rcount))||') for insert command'; ")
    SET str->str = concat(str->str,"	     signal sqlstate '70999' set message_text = v_str; ")
    SET str->str = concat(str->str,"	   end if; ")
    SET str->str = concat(str->str,"  elseif (rcount != 1) then ")
    SET str->str = concat(str->str,"	  set v_str = 'Bad diagnostic row_count value ('||",
     "rtrim(char(rcount))||') for update command'; ")
    SET str->str = concat(str->str,"	  signal sqlstate '70999' set message_text = v_str; ")
    SET str->str = concat(str->str,"	end if;")
    SET str->str = concat(str->str," end")
    SET str->str = concat("RDB ASIS (^ ",str->str," ^) GO")
    IF (dm2_push_cmd(str->str,1) != 1)
     ROLLBACK
     GO TO exit_program
    ELSE
     COMMIT
 ...
 ENDIF
 SUBROUTINE (dact_validate_trigger(dvt_cnt=i2) =i4)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     dact_table_count += 1, dact_table_name = a.table_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   CASE (dact_trigger_count)
    OF 1:
     IF (currdb="DB2UDB")
      SET str->str = concat("RDB DROP TRIGGER ",dact_trigger_name," GO")
    ...
    ELSE
     ...
     SET str->str = concat("RDB DROP TRIGGER ",trim(tlist->qual[dvt_cnt].trig_name),"$C GO")
     IF (dm2_push_cmd(str->str,1) != 1)
      ROLLBACK
      RETURN(0)
     ENDIF
     IF (currdb="DB2UDB")
      SET str->str = concat("RDB DROP TRIGGER ",trim(tlist->qual[dvt_cnt].trig_name)," GO")
      IF (dm2_push_cmd(str->str,1) != 1)
       ROLLBACK
       RETURN(0)
      ENDIF
