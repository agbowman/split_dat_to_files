CREATE PROGRAM dcp_copy_io_results_child_med:dba
 ...
 DECLARE auth = f8 WITH public, noconstant(uar_get_code_by("MEANING",8,"AUTH"))
 DECLARE root = f8 WITH public, noconstant(uar_get_code_by("MEANING",24,"ROOT"))
 DECLARE active = f8 WITH public, noconstant(uar_get_code_by("MEANING",48,"ACTIVE"))
 DECLARE io = f8 WITH public, noconstant(uar_get_code_by("MEANING",53,"IO"))
 DECLARE med = f8 WITH public, noconstant(uar_get_code_by("MEANING",53,"MED"))
 DECLARE ivparent = f8 WITH public, noconstant(uar_get_code_by("MEANING",72,"IVPARENT"))
 DECLARE powerchart73 = f8 WITH public, noconstant(uar_get_code_by("MEANING",73,"POWERCHART"))
 DECLARE powerchart89 = f8 WITH public, noconstant(uar_get_code_by("MEANING",89,"POWERCHART"))
 DECLARE medadmin = f8 WITH public, noconstant(uar_get_code_by("MEANING",255431,"MEDADMIN"))
 DECLARE confirmed = f8 WITH public, noconstant(uar_get_code_by("MEANING",4000160,"CONFIRMED"))
 DECLARE ml = f8 WITH public, noconstant(uar_get_code_by_cki("CKI.CODEVALUE!3780"))
 DECLARE dcpgeneric = f8 WITH public, noconstant(0.0)
 DECLARE medintake = f8 WITH public, noconstant(0.0)
 IF (((auth < 0.0) OR (((root < 0.0) OR (((active < 0.0) OR (((io < 0.0) OR (((med < 0.0) OR (((
 ivparent < 0.0) OR (((powerchart73 < 0.0) OR (((powerchart89 < 0.0) OR (((medadmin < 0.0) OR (((
 confirmed < 0.0) OR (ml < 0.0)) )) )) )) )) )) )) )) )) )) )
 ...
 IF ((powerchart73 > - (1.0)))
  SELECT INTO "nl:"
   ...
   WHERE cva.alias IN ("DCPGENERIC", "MEDINTAKE")
    AND cva.code_set=72.0
    AND cva.contributor_source_cd=powerchart73
   DETAIL
    IF (cva.alias="DCPGENERIC")
     dcpgeneric = cva.code_value
    ELSEIF (cva.alias="MEDINTAKE")
     medintake = cva.code_value
    ENDIF
   WITH nocounter
  ;end select
  IF (((dcpgeneric=0.0) OR (((medintake=0.0) OR (error(string_struct->ms_err_msg,0) != 0)) )) )
   CALL echo(concat("FAILURE TO RETRIEVE INFO FROM CODE_VALUE_ALIAS:",string_struct->ms_err_msg))
   GO TO exit_program
  ENDIF
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="COPY EXISTING IO RESULTS TO IO2G COMPATIBLE - MED"
 ...
 IF (curqual=0)
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="COPY EXISTING IO RESULTS TO IO2G COMPATIBLE PARENT - MED"
  ...
  IF (curqual=1)
   ...
   INSERT  FROM dm_info di
    SET di.info_domain = "COPY EXISTING IO RESULTS TO IO2G COMPATIBLE - MED", di.info_name = concat(
      "MAX EVENT_ID EVALUATED BY ",string_struct->ms_parent_rowid), di.info_number = mf_min_epr_id,
  ...
  ELSE
   IF (error(string_struct->ms_err_msg,0) != 0)
    ...
    GO TO exit_program
   ENDIF
   CALL echo("DONE PROCESSING MEDS.")
   SET mn_success = 1
 ...
 WHILE (mf_cur_min_id <= mf_max_epr_id)
   ...
   ENDIF
   EXECUTE dcp_copy_io_results_med value(mf_cur_min_id), value(mf_cur_max_id)
