CREATE PROGRAM cc_get_demog
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CC_GET_DEMOG"
 ...
 SUBROUTINE (validatesrvreply(nsrvstat=i4,recorddata=vc(ref),zeroforceexit=i2) =i4 WITH protect)
   ...
   ENDIF
 END ;Subroutine
 DECLARE request_number = i4 WITH protect, constant(3208634)
 DECLARE srv_request = vc WITH protect, constant("QueryDemographics")
 DECLARE hpatients = i4 WITH noconstant(0), protect
 SET reply_data->status_data.status = "F"
 CALL getparametervalueswithzeros(2,patients_rec)
 CALL getparametervalueswithzeros(3,encntrs_rec)
 ...
 SUBROUTINE (getdata(hreply=i4(ref)) =null WITH protect)
   ...
   SET putrequest->source_dir =  $OUTDEV
   SET putrequest->isblob = "1"
   SET pos = uar_srvgetasissize(hreply,"patient_demog_json")
   SET putrequest->document = substring(1,pos,uar_srvgetasisptr(hreply,"patient_demog_json"))
   SET putrequest->document_size = size(putrequest->document)
   CALL exit_servicerequest(happ,htask,hstep)
   ...
   EXECUTE eks_put_source  WITH replace(request,putrequest), replace(reply,putreply)
 END ;Subroutine
 SUBROUTINE (getparametervalueswithzeros(index=i4(val),value_rec=vc(ref)) =null WITH protect)
   DECLARE par = vc WITH noconstant(""), protect
