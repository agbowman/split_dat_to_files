CREATE PROGRAM dm2_mig_monitoring:dba
 ...
 SUBROUTINE dm2_get_dbase_name(dgdn_name_out)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD dmm_file_contents
 RECORD dmm_file_contents(
   1 cnt = i4
   1 file_name = vc
   1 qual[*]
     2 line = vc
 )
 FREE RECORD dmm_ignore_err
 RECORD dmm_ignore_err(
   1 ccnt = i4
   1 cap_qual[*]
     2 error_nbr = vc
     2 error_str = vc
 ...
 SET dmm_ignore_err->ccnt = 0
 SET dmm_ignore_err->dcnt = 0
 FREE RECORD dmm_ignore_warn
 RECORD dmm_ignore_warn(
   1 ccnt = i4
   1 cap_qual[*]
     2 warn_nbr = vc
     2 warn_str = vc
 ...
 SET dmm_ignore_warn->ccnt = 0
 SET dmm_ignore_warn->dcnt = 0
 RECORD dmm_table_list(
   1 cnt = i4
   1 missing_triggers = i2
   1 disabled_triggers = i2
   1 qual[*]
     2 owner = vc
     2 table_name = vc
     2 upd_trig_ind = i2
     2 del_trig_ind = i2
     2 upd_trig_disabled = i2
     2 del_trig_disabled = i2
 )
 SET dmm_table_list->cnt = 0
 FREE RECORD dmm_di_found
 RECORD dmm_di_found(
   1 cnt = i4
   1 qual[*]
     2 owner_table = vc
 )
 FREE RECORD dmm_latency
 RECORD dmm_latency(
   1 cnt = i4
   1 qual[*]
     2 local_database = vc
     2 remote_database = vc
     2 incoming_path = vc
     2 incoming_lag = f8
 )
 DECLARE dmm_gg_warning_str = vc WITH protect, noconstant("*GGS WARNING*")
 DECLARE dmm_gg_error_str = vc WITH protect, noconstant("*GGS ERROR*")
 DECLARE dmm_gg_warning_lit = vc WITH protect, noconstant("GGS WARNING")
 DECLARE dmm_gg_error_lit = vc WITH protect, noconstant("GGS ERROR")
 DECLARE dmm_gg_ddl_excl_code = vc WITH protect, noconstant("OGG-00488")
 DECLARE dmm_gg_ddl_ts_str = vc WITH protect, noconstant("EXCLUDE ALL OBJTYPE 'TABLESPACE'")
 DECLARE dmm_gg_ddl_dbl_str = vc WITH protect, noconstant("EXCLUDE ALL OBJTYPE 'DATABASE LINK'")
 DECLARE dmm_gg_ddl_ignore_str = vc WITH protect, noconstant("*OGG-00492*")
 DECLARE dmm_gg_ddl_err_code1 = vc WITH protect, noconstant("24344")
 DECLARE dmm_gg_ddl_ignore_ind = i2 WITH protect, noconstant(0)
 DECLARE dmm_gg_ddl_excl_ind = i2 WITH protect, noconstant(0)
 DECLARE dmm_gg_warning_ind = i2 WITH protect, noconstant(0)
 DECLARE dmm_gg_error_ind = i2 WITH protect, noconstant(0)
 DECLARE dmm_report_all = i2 WITH protect, noconstant(0)
 DECLARE dmm_report_delivery = i2 WITH protect, noconstant(0)
 DECLARE dmm_report_capture = i2 WITH protect, noconstant(0)
 DECLARE dmm_source_connect_string = vc WITH protect, noconstant("")
 DECLARE dmm_job_mode = vc WITH protect, noconstant("")
 DECLARE dmm_file_location = vc WITH protect, noconstant("")
 DECLARE dmm_log_file = vc WITH protect, noconstant("")
 DECLARE dmm_monitor_time = i4 WITH protect, noconstant(0)
 DECLARE dmm_heartbeat_time = i4 WITH protect, noconstant(0)
 DECLARE dmm_heartbeat_timestamp = dq8 WITH protect, noconstant(0.0)
 DECLARE dmm_start_timestamp = dq8 WITH protect, noconstant(cnvtdatetime(curdate,curtime3))
 DECLARE dmm_database_name = vc WITH protect, noconstant("")
 DECLARE dmm_capture_timestamp = vc WITH protect, noconstant("")
 DECLARE dmm_capture_ts_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE dmm_delivery_timestamp = vc WITH protect, noconstant("")
 DECLARE dmm_tmp_delivery_timestamp = vc WITH protect, noconstant("")
 DECLARE dmm_tmp_capture_timestamp = vc WITH protect, noconstant("")
 DECLARE dmm_delivery_ts_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE dmm_push_dcl = vc WITH protect, noconstant("")
 DECLARE dmm_push_dcl_ret = i4 WITH protect, noconstant(0)
 DECLARE dmm_temp_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dmm_nomatch_found_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dmm_str = vc WITH protect, noconstant("")
 DECLARE dmm_archive_job_running = i2 WITH protect, noconstant(0)
 DECLARE dmm_iter = i4 WITH protect, noconstant(0)
 DECLARE dmm_ignore = i2 WITH protect, noconstant(0)
 DECLARE dmm_start_pos = i4 WITH protect, noconstant(0)
 DECLARE dmm_error_str = vc WITH protect, noconstant("")
 DECLARE dmm_warn_str = vc WITH protect, noconstant("")
 DECLARE dmm_temp_info_char = vc WITH protect, noconstant("")
 DECLARE dmm_ast_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmm_search1 = i4 WITH protect, noconstant(0)
 DECLARE dmm_search2 = i4 WITH protect, noconstant(0)
 DECLARE dmm_long_run_trxn_hr = i4 WITH protect, noconstant(12)
 DECLARE dmm_pk_trig_needed = i2 WITH protect, noconstant(0)
 DECLARE dmm_gg_version = i2 WITH protect, noconstant(0)
 DECLARE dmm_latency_cnt = i2 WITH protect, noconstant(0)
 DECLARE dmm_write_to_monitor_file(single_line=vc) = i2
 DECLARE dmm_check_hb_tbls(null) = i2
 IF (check_logfile("dm2_mig_monitoring",".log","dm2_mig_monitoring LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_monitoring"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dmm_source_connect_string = trim(cnvtupper( $1),3)
 SET dmm_job_mode = trim(cnvtupper( $2),3)
 SET dmm_file_location = trim( $3,3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg =
  "Parameter usage: dm2_mig_monitoring '<src_connect_str>', '<job_mode>', '<file_location>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dmm_source_connect_string="")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Source connect string must be supplied."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 CASE (dmm_job_mode)
  OF "ALL":
   SET dmm_report_all = 1
  OF "CAPTURE":
   SET dmm_report_capture = 1
  OF "DELIVERY":
   SET dmm_report_delivery = 1
  ELSE
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Valid values for <job_mode> are 'ALL', 'CAPTURE', 'DELIVERY'."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
 ENDCASE
 IF (dmm_file_location="")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "File location must be supplied."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Obtain Goldengate Version"
 SELECT INTO "nl:"
  FROM dm2_admin_dm_info di
  WHERE di.info_domain="DM2MIG_DELDATA"
   AND di.info_name="GOLDENGATE_VERSION"
  DETAIL
   IF (di.info_number >= 11)
    dmm_gg_warning_str = "*WARNING OGG-*", dmm_gg_error_str = "*ERROR   OGG-*", dmm_gg_warning_lit =
    "WARNING OGG-",
    dmm_gg_error_lit = "ERROR   OGG-"
   ENDIF
   dmm_gg_version = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (dmm_gg_version=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Goldengate version not found."
  SET dm_err->user_action = "Check cronmain and ensure working properly."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(build("dmm_report_all:",dmm_report_all))
  CALL echo(build("dmm_report_capture:",dmm_report_capture))
  CALL echo(build("dmm_report_delivery:",dmm_report_delivery))
  CALL echo(build("dmm_gg_warning_str:",dmm_gg_warning_str))
  CALL echo(build("dmm_gg_error_str:",dmm_gg_error_str))
 ENDIF
 SET dm_err->eproc = "Obtain DM_INFO indicator for triggers"
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_PK_TRIGGERS"
   AND di.info_name="ON"
  DETAIL
   dmm_pk_trig_needed = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Querying for ignorable errors from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_GGS_*_ERROR"
  HEAD REPORT
   dmm_ignore_err->ccnt = 0, stat = alterlist(dmm_ignore_err->cap_qual,0), dmm_ignore_err->dcnt = 0,
   stat = alterlist(dmm_ignore_err->del_qual,0)
  DETAIL
   dmm_temp_info_char = di.info_char, dmm_ast_cnt = 0
   FOR (dmm_iter = 1 TO size(dmm_temp_info_char))
     IF (substring(dmm_iter,1,dmm_temp_info_char)=char(42))
      dmm_ast_cnt = (dmm_ast_cnt+ 1)
      IF (((dmm_iter=1) OR (dmm_iter=size(dmm_temp_info_char))) )
       dm_err->err_ind = 1, dm_err->emsg = concat("Invalid error message string (",dmm_temp_info_char,
        ").  Do not start/end with an asterisk."), dmm_iter = size(dmm_temp_info_char),
       CALL cancel(1)
      ELSE
       IF (dmm_ast_cnt > 1)
        dm_err->err_ind = 1, dm_err->emsg = concat("Invalid error message string (",
         dmm_temp_info_char,").  Only one asterisk allowed."), dmm_iter = size(dmm_temp_info_char),
        CALL cancel(1)
       ENDIF
      ENDIF
     ENDIF
   ENDFOR
   IF (di.info_domain="DM2_MIG_GGS_CAPTURE_ERROR")
    dmm_ignore_err->ccnt = (dmm_ignore_err->ccnt+ 1), stat = alterlist(dmm_ignore_err->cap_qual,
     dmm_ignore_err->ccnt), dmm_ignore_err->cap_qual[dmm_ignore_err->ccnt].error_nbr = di.info_name,
    dmm_ignore_err->cap_qual[dmm_ignore_err->ccnt].error_str = dmm_temp_info_char, dmm_ignore_err->
    cap_qual[dmm_ignore_err->ccnt].multi_str = dmm_ast_cnt
   ELSEIF (di.info_domain="DM2_MIG_GGS_DELIVERY_ERROR")
    dmm_ignore_err->dcnt = (dmm_ignore_err->dcnt+ 1), stat = alterlist(dmm_ignore_err->del_qual,
     dmm_ignore_err->dcnt), dmm_ignore_err->del_qual[dmm_ignore_err->dcnt].error_nbr = di.info_name,
    dmm_ignore_err->del_qual[dmm_ignore_err->dcnt].error_str = dmm_temp_info_char, dmm_ignore_err->
    del_qual[dmm_ignore_err->dcnt].multi_str = dmm_ast_cnt
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dmm_ignore_err->ccnt = (dmm_ignore_err->ccnt+ 1)
 SET stat = alterlist(dmm_ignore_err->cap_qual,dmm_ignore_err->ccnt)
 SET dmm_ignore_err->cap_qual[dmm_ignore_err->ccnt].error_nbr = "515"
 SET dmm_ignore_err->cap_qual[dmm_ignore_err->ccnt].error_str = "opening redo log*.RDO"
 SET dmm_ignore_err->cap_qual[dmm_ignore_err->ccnt].multi_str = 1
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(dmm_ignore_err)
 ENDIF
 SET dm_err->eproc = "Querying for ignorable warnings from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_GGS_*_WARNING"
  HEAD REPORT
   dmm_ignore_warn->ccnt = 0, stat = alterlist(dmm_ignore_warn->cap_qual,0), dmm_ignore_warn->dcnt =
   0,
   stat = alterlist(dmm_ignore_warn->del_qual,0)
  DETAIL
   dmm_temp_info_char = di.info_char, dmm_ast_cnt = 0
   FOR (dmm_iter = 1 TO size(dmm_temp_info_char))
     IF (substring(dmm_iter,1,dmm_temp_info_char)=char(42))
      dmm_ast_cnt = (dmm_ast_cnt+ 1)
      IF (((dmm_iter=1) OR (dmm_iter=size(dmm_temp_info_char))) )
       dm_err->err_ind = 1, dm_err->emsg = concat("Invalid warning message string (",
        dmm_temp_info_char,").  Do not start/end with an asterisk."), dmm_iter = size(
        dmm_temp_info_char),
       CALL cancel(1)
      ELSE
       IF (dmm_ast_cnt > 1)
     ...
     ENDIF
   ENDFOR
   IF (di.info_domain="DM2_MIG_GGS_CAPTURE_WARNING")
    dmm_ignore_warn->ccnt = (dmm_ignore_warn->ccnt+ 1), stat = alterlist(dmm_ignore_warn->cap_qual,
     dmm_ignore_warn->ccnt), dmm_ignore_warn->cap_qual[dmm_ignore_warn->ccnt].warn_nbr = di.info_name,
    dmm_ignore_warn->cap_qual[dmm_ignore_warn->ccnt].warn_str = dmm_temp_info_char, dmm_ignore_warn->
    cap_qual[dmm_ignore_warn->ccnt].multi_str = dmm_ast_cnt
   ELSEIF (di.info_domain="DM2_MIG_GGS_DELIVERY_WARNING")
    dmm_ignore_warn->dcnt = (dmm_ignore_warn->dcnt+ 1), stat = alterlist(dmm_ignore_warn->del_qual,
     dmm_ignore_warn->dcnt), dmm_ignore_warn->del_qual[dmm_ignore_warn->dcnt].warn_nbr = di.info_name,
    dmm_ignore_warn->del_qual[dmm_ignore_warn->dcnt].warn_str = dmm_temp_info_char, dmm_ignore_warn->
    del_qual[dmm_ignore_warn->dcnt].multi_str = dmm_ast_cnt
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dmm_ignore_warn->ccnt = (dmm_ignore_warn->ccnt+ 1)
 SET stat = alterlist(dmm_ignore_warn->cap_qual,dmm_ignore_warn->ccnt)
 SET dmm_ignore_warn->cap_qual[dmm_ignore_warn->ccnt].warn_nbr = "250"
 SET dmm_ignore_warn->cap_qual[dmm_ignore_warn->ccnt].multi_str = 0
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(dmm_ignore_warn)
 ENDIF
 IF (dmr_fill_email_list(null)=0)
  GO TO exit_script
 ENDIF
 IF ((dmr_emails->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Verifying email addresses."
  SET dm_err->emsg = concat(
   "There must be at least 1 email address set up to be notified in case of errors.  ",
   "Please set up email addresses using dm2_mig_menu.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dmr_get_unique_file_name(dmm_file_location,"dm2_monitor_msgs_",".dat")=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dmm_log_file = dm_err->unique_fname
 SET dm_err->eproc = "Determining existing submit time from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name=value(evaluate(dmm_report_delivery,1,"MONITOR_DELIVERY_TIME",
    "MONITOR_CAPTURE_TIME"))
  DETAIL
   dmm_monitor_time = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Row does not exist."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining existing heartbeat time from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name=value(evaluate(dmm_report_delivery,1,"MONITOR_DELIVERY_HEARTBEAT",
    "MONITOR_CAPTURE_HEARTBEAT"))
  DETAIL
   dmm_heartbeat_time = di.info_number, dmm_heartbeat_timestamp = di.info_date
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  ...
  GO TO exit_script
 ENDIF
 IF (dm2_get_dbase_name(dmm_database_name)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining last timestamps for logfiles."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name IN ("CAPTURE_ERROR_LOG", "DELIVERY_ERROR_LOG")
  DETAIL
   CASE (di.info_name)
    OF "CAPTURE_ERROR_LOG":
     IF (((dmm_report_all) OR (dmm_report_capture)) )
      dmm_capture_timestamp = di.info_char, dmm_capture_ts_exists_ind = 1
     ENDIF
    OF "DELIVERY_ERROR_LOG":
     IF (((dmm_report_all) OR (dmm_report_delivery)) )
      dmm_delivery_timestamp = di.info_char, dmm_delivery_ts_exists_ind = 1
     ENDIF
   ENDCASE
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF (((dmm_report_all) OR (dmm_report_capture)) )
  SET dm_err->eproc = "Querying v$logfile to determine if there are any STALE or INVALID logfiles."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM v$logfile gvlf
   WHERE gvlf.status IN ("STALE", "INVALID")
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF (curqual > 0)
   SET dmm_gg_warning_ind = 1
   IF (dmm_write_to_monitor_file(
    "WARNING: One or more redo log files are in an invalid or stale state.")=0)
    GO TO exit_script
   ENDIF
  ENDIF
  IF ((dm2_sys_misc->cur_os="AXP"))
   SET dm_err->eproc = "Determining if the archive log copy job is executing."
   CALL disp_msg("",dm_err->logfile,0)
   IF (dmr_get_queue_contents(dmr_batch_queue)=0)
    GO TO exit_script
   ENDIF
   FOR (dmm_iter = 1 TO dmr_queue->cnt)
     IF ((dmr_queue->qual[dmm_iter].jobname="DM2_MIG_COPY_ARCHIVELOGS"))
      SET dmm_archive_job_running = 1
     ENDIF
   ENDFOR
   IF (dmm_archive_job_running=0)
    SET dmm_gg_warning_ind = 1
    IF (dmm_write_to_monitor_file("WARNING: Archive Log Copy process is not currently executing.")=0)
     GO TO exit_script
    ENDIF
   ENDIF
  ENDIF
  IF (dmm_pk_trig_needed=1)
   SET dm_err->eproc = "Checking for missing or disabled migration update/delete triggers."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_tables dt,
     dba_triggers dtu,
    ...
    PLAN (dt
     ...
     (SELECT
      ...
      AND  NOT (concat(dt.owner,".",dt.table_name) IN (
     (SELECT
      info_name
      FROM dm_info
      WHERE info_domain="DM2_MIG_EXCL")))
      AND dt.temporary="N")
     JOIN (dtu
     WHERE outerjoin(dt.owner)=dtu.owner
      AND outerjoin(dt.table_name)=dtu.table_name
      AND dtu.trigger_name=outerjoin("TRG_DM2_MIG_UPD*"))
     JOIN (dtd
     WHERE outerjoin(dt.owner)=dtd.owner
      AND outerjoin(dt.table_name)=dtd.table_name
      AND dtd.trigger_name=outerjoin("TRG_DM2_MIG_DEL*"))
    ORDER BY dt.owner, dt.table_name
    HEAD REPORT
     dmm_table_list->cnt = 0, stat = alterlist(dmm_table_list->qual,0)
    HEAD dt.owner
     row + 0
    DETAIL
     IF (((dtu.trigger_name=" ") OR (((dtd.trigger_name=" ") OR (((dtu.status="DISABLED") OR (dtd
     .status="DISABLED")) )) )) )
      IF (dt.owner="V500"
       AND dt.table_name="DM_INFO")
       row + 0
      ELSE
       dmm_table_list->cnt = (dmm_table_list->cnt+ 1)
       IF (mod(dmm_table_list->cnt,10)=1)
        stat = alterlist(dmm_table_list->qual,(dmm_table_list->cnt+ 9))
       ENDIF
       dmm_table_list->qual[dmm_table_list->cnt].owner = dt.owner, dmm_table_list->qual[
       dmm_table_list->cnt].table_name = dt.table_name
       IF (dtu.trigger_name=" ")
        dmm_table_list->qual[dmm_table_list->cnt].upd_trig_ind = 0, dmm_table_list->missing_triggers
         = 1
       ELSEIF (dtu.status="DISABLED")
        dmm_table_list->disabled_triggers = 1, dmm_table_list->qual[dmm_table_list->cnt].upd_trig_ind
         = 1, dmm_table_list->qual[dmm_table_list->cnt].upd_trig_disabled = 1
       ELSE
        dmm_table_list->qual[dmm_table_list->cnt].upd_trig_ind = 1
       ENDIF
       IF (dtd.trigger_name=" ")
        dmm_table_list->qual[dmm_table_list->cnt].del_trig_ind = 0, dmm_table_list->missing_triggers
         = 1
       ELSEIF (dtd.status="DISABLED")
        dmm_table_list->disabled_triggers = 1, dmm_table_list->qual[dmm_table_list->cnt].del_trig_ind
         = 1, dmm_table_list->qual[dmm_table_list->cnt].del_trig_disabled = 1
       ELSE
        dmm_table_list->qual[dmm_table_list->cnt].del_trig_ind = 1
       ENDIF
      ENDIF
     ENDIF
    FOOT REPORT
     stat = alterlist(dmm_table_list->qual,dmm_table_list->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 1))
    CALL echorecord(dmm_table_list)
   ENDIF
   IF ((dmm_table_list->cnt > 0))
    SET dmm_gg_error_ind = 1
    FOR (dmm_iter = 1 TO dmm_table_list->cnt)
      IF ((((dmm_table_list->qual[dmm_iter].upd_trig_ind=0)) OR ((dmm_table_list->qual[dmm_iter].
      del_trig_ind=0))) )
       SET dmm_file_contents->cnt = (dmm_file_contents->cnt+ 1)
       SET stat = alterlist(dmm_file_contents->qual,dmm_file_contents->cnt)
       SET dmm_file_contents->qual[dmm_file_contents->cnt].line = concat("   ",dmm_table_list->qual[
        dmm_iter].owner,".",dmm_table_list->qual[dmm_iter].table_name)
      ENDIF
    ENDFOR
    SET dmm_file_contents->file_name = concat("CRITICAL ERRORS DETECTED: The following tables ",
     "are missing database migration triggers.  Please contact technical support for assistance.")
    IF (dmm_write_to_monitor_file("")=0)
     GO TO exit_script
    ENDIF
    FOR (dmm_iter = 1 TO dmm_table_list->cnt)
      IF ((((dmm_table_list->qual[dmm_iter].upd_trig_disabled=1)) OR ((dmm_table_list->qual[dmm_iter]
      .del_trig_disabled=1))) )
       SET dmm_file_contents->cnt = (dmm_file_contents->cnt+ 1)
       SET stat = alterlist(dmm_file_contents->qual,dmm_file_contents->cnt)
       SET dmm_file_contents->qual[dmm_file_contents->cnt].line = concat("   ",dmm_table_list->qual[
    ...
    ENDFOR
    SET dmm_file_contents->file_name = concat("CRITICAL ERRORS DETECTED: The following tables ",
     "have database migration triggers that are disabled.  Please contact technical support for assistance."
     )
    IF (dmm_write_to_monitor_file("")=0)
     GO TO exit_script
    ENDIF
   ENDIF
   SET dm_err->eproc = "Checking for primary/unique key column updates and delete."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_info di1,
     dm_info di2
    WHERE di1.info_domain="DM2_MIG_DEL_TRIG"
     AND di1.info_number=null
     AND di2.info_domain="DM2_MIG_UPD_TRIG"
     AND di2.info_number=null
     AND di1.info_name=di2.info_name
    ORDER BY di1.info_name
    HEAD REPORT
     dmm_di_found->cnt = 0, stat = alterlist(dmm_di_found->qual,0)
    DETAIL
     dmm_di_found->cnt = (dmm_di_found->cnt+ 1), stat = alterlist(dmm_di_found->qual,dmm_di_found->
      cnt), dmm_di_found->qual[dmm_di_found->cnt].owner_table = di1.info_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 1))
    CALL echorecord(dmm_di_found)
   ENDIF
   IF ((dmm_di_found->cnt > 0))
    SET dmm_gg_error_ind = 1
    FOR (dmm_iter = 1 TO dmm_di_found->cnt)
      SET dmm_file_contents->cnt = (dmm_file_contents->cnt+ 1)
      SET stat = alterlist(dmm_file_contents->qual,dmm_file_contents->cnt)
      SET dmm_file_contents->qual[dmm_file_contents->cnt].line = concat("   ",dmm_di_found->qual[
       dmm_iter].owner_table)
    ENDFOR
    SET dmm_file_contents->file_name = concat("CRITICAL ERRORS DETECTED: The following tables ",
     "have had recent primary/unique key column updates and deletes.  Please contact technical support for assistance."
     )
    IF (dmm_write_to_monitor_file("")=0)
     GO TO exit_script
    ENDIF
    SET dm_err->eproc = "Updating DM_INFO rows to denote content already reported."
    UPDATE  FROM dm_info di,
      (dummyt d  WITH seq = value(dmm_di_found->cnt))
     SET di.info_number = 1
     PLAN (d
      WHERE (dmm_di_found->cnt > 0))
      JOIN (di
      WHERE di.info_domain IN ("DM2_MIG_DEL_TRIG", "DM2_MIG_UPD_TRIG")
       AND (di.info_name=dmm_di_found->qual[d.seq].owner_table))
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
     ROLLBACK
   ...
   ENDIF
  ENDIF
  SET dm_err->eproc = "Query for LONG RUNNING TRANSACTION time override"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2_MIG_DATA"
    AND d.info_name="MONITOR_LONG_RUNNING_TRXN"
    AND d.info_number BETWEEN 1 AND 48
   DETAIL
    dmm_long_run_trxn_hr = d.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Query for long running transactions"
  SELECT INTO "nl:"
   FROM gv$session s,
    gv$transaction t
   WHERE s.saddr=t.ses_addr
    AND ((((86400 * sqlpassthru("(sysdate - to_date(t.start_time,'MM/DD/YY HH24:MI:SS'))"))/ 60)/ 60)
    > dmm_long_run_trxn_hr)
  ...
  IF (curqual > 0)
   SET dmm_gg_warning_ind = 1
   SET dmm_file_contents->file_name = "WARNING: Long running transaction detected."
   SET stat = alterlist(dmm_file_contents->qual,(dmm_file_contents->cnt+ 9))
   SET dmm_file_contents->qual[(dmm_file_contents->cnt+ 1)].line = concat(
    "From SOURCE, perform the following query to show the long running transactions.")
   SET dmm_file_contents->qual[(dmm_file_contents->cnt+ 2)].line = " "
   SET dmm_file_contents->qual[(dmm_file_contents->cnt+ 3)].line =
   "rdb select s.saddr,s.sid,s.username,s.osuser,s.status,s.program,t.start_time,"
   SET dmm_file_contents->qual[(dmm_file_contents->cnt+ 4)].line =
   "   (86400* (sysdate - to_date(t.start_time,'MM/DD/YY HH24:MI:SS'))/60) as elapsed_minutes"
   SET dmm_file_contents->qual[(dmm_file_contents->cnt+ 5)].line =
   "from gv$session s,gv$transaction t "
   SET dmm_file_contents->qual[(dmm_file_contents->cnt+ 6)].line = "where s.saddr = t.ses_addr"
   SET dmm_file_contents->qual[(dmm_file_contents->cnt+ 7)].line = concat(
    "and (86400* (sysdate - to_date(t.start_time,'MM/DD/YY HH24:MI:SS')) /60/60) > ",build(
     dmm_long_run_trxn_hr))
   SET dmm_file_contents->qual[(dmm_file_contents->cnt+ 8)].line = "order by t.start_time go"
   SET dmm_file_contents->qual[(dmm_file_contents->cnt+ 9)].line = " "
   SET dmm_file_contents->cnt = (dmm_file_contents->cnt+ 9)
   IF (dmm_write_to_monitor_file("")=0)
    GO TO exit_script
   ENDIF
  ENDIF
 ENDIF
 CALL dmr_set_gg_files(1,1)
 IF (((dmm_report_all) OR (dmm_report_capture)) )
  IF ((dm2_sys_misc->cur_os="AXP"))
   SET dmm_str = dmr_mig_data->cap_dir
  ELSE
   SET dmm_str = concat(dmr_mig_data->cap_dir,"/")
  ENDIF
  IF (dmr_check_directory(dmm_str)=0)
   SET dmm_gg_error_ind = 1
   IF (dmm_write_to_monitor_file(concat("CRITICAL ERRORS DETECTED: ",dmr_mig_data->cap_dir,
     " is not available."))=0)
    GO TO exit_script
   ENDIF
   GO TO exit_script
 ...
 IF (((dmm_report_all) OR (dmm_report_delivery)) )
  IF ((dm2_sys_misc->cur_os="AXP"))
   SET dmm_str = dmr_mig_data->del_dir
  ELSE
   SET dmm_str = concat(dmr_mig_data->del_dir,"/")
  ENDIF
  IF (dmr_check_directory(dmm_str)=0)
   SET dmm_gg_error_ind = 1
   IF (dmm_write_to_monitor_file(concat("CRITICAL ERRORS DETECTED: ",dmr_mig_data->del_dir,
     " is not available."))=0)
    GO TO exit_script
   ENDIF
   GO TO exit_script
 ...
 ENDIF
 IF (((dmm_report_all) OR (dmm_report_capture)) )
  SET dm_err->eproc = "Searching or copying capture ggserr.log to find new items."
  IF (dmm_capture_timestamp="")
   IF ((dm2_sys_misc->cur_os="AXP"))
    IF (dm2_push_dcl(concat("COPY ",dmr_mig_data->cap_dir,"ggserr.log ",dmm_file_location,
      "dm2_mig_monitoring_cap.log"))=0)
     GO TO exit_script
    ENDIF
   ELSE
    IF (dm2_push_dcl(concat("cp ",dmr_mig_data->cap_dir,"/ggserr.log ",dmm_file_location,
      "dm2_mig_monitoring_cap.log"))=0)
     GO TO exit_script
    ENDIF
   ENDIF
  ELSE
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET dmm_push_dcl = concat("pipe search ",dmr_mig_data->cap_dir,'ggserr.log "',
     dmm_capture_timestamp,'" /remaining > ',
     dmm_file_location,"dm2_mig_monitoring_cap.log")
   ELSEIF ((dm2_sys_misc->cur_os="LNX"))
    SET dmm_push_dcl = concat("tail -n +`grep -n '",dmm_capture_timestamp,"' ",dmr_mig_data->cap_dir,
     '/ggserr.log | cut -d ":" -f 1 | head -n 1` ',
     dmr_mig_data->cap_dir,"/ggserr.log > ",dmm_file_location,"dm2_mig_monitoring_cap.log")
   ELSE
    SET dmm_push_dcl = concat("tail +`grep -n '",dmm_capture_timestamp,"' ",dmr_mig_data->cap_dir,
     '/ggserr.log | cut -d ":" -f 1 | head -n 1` ',
     dmr_mig_data->cap_dir,"/ggserr.log > ",dmm_file_location,"dm2_mig_monitoring_cap.log")
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(dmm_push_dcl)
   ENDIF
   IF ((dm2_sys_misc->cur_os="AXP"))
    CALL dcl(dmm_push_dcl,size(dmm_push_dcl),dmm_push_dcl_ret)
    IF (dmm_push_dcl_ret=0)
     FREE DEFINE rtl
     FREE SET dmm_file
     SET logical dmm_file value(build(dmm_file_location,"dm2_mig_monitoring_cap.log"))
     DEFINE rtl "dmm_file"
     SET dm_err->eproc = "Reading contents of error file."
     SELECT INTO "nl:"
      FROM rtlt r
      HEAD REPORT
       dmm_nomatch_found_err_ind = 0
      DETAIL
       IF (r.line="%SEARCH-I-NOMATCHES*")
        dmm_nomatch_found_err_ind = 1
       ELSE
        dmm_file_contents->cnt = (dmm_file_contents->cnt+ 1), stat = alterlist(dmm_file_contents->
         qual,dmm_file_contents->cnt), dmm_file_contents->qual[dmm_file_contents->cnt].line = build(r
         .line)
       ENDIF
      WITH nocounter, nullreport
     ;end select
     IF (check_error(dm_err->eproc)=1)
      ...
      GO TO exit_script
     ENDIF
     IF (dmm_nomatch_found_err_ind)
      SET dmm_capture_timestamp = ""
      IF (dm2_push_dcl(concat("COPY ",dmr_mig_data->cap_dir,"ggserr.log ",dmm_file_location,
        "dm2_mig_monitoring_cap.log"))=0)
       GO TO exit_script
      ENDIF
     ELSE
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Error searching in file."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_script
     ENDIF
    ENDIF
   ELSE
    IF (dm2_push_dcl(dmm_push_dcl)=0)
     GO TO exit_script
    ENDIF
   ENDIF
  ENDIF
  ...
  FREE SET dmm_file
  SET logical dmm_file value(build(dmm_file_location,"dm2_mig_monitoring_cap.log"))
  DEFINE rtl2 "dmm_file"
  SET dm_err->eproc = "Reading Golden Gate capture file for warnings and errors."
  SELECT INTO "nl:"
   FROM rtl2t r
   HEAD REPORT
    dmm_tmp_capture_timestamp = dmm_capture_timestamp
   DETAIL
    IF (((dmm_capture_timestamp="") OR (trim(substring(1,19,r.line)) != dmm_capture_timestamp)) )
     dmm_tmp_capture_timestamp = concat(substring(1,10,r.line)," ",substring(12,8,r.line))
     IF (r.line=patstring(dmm_gg_warning_str))
      IF ((dm_err->debug_flag > 2))
       CALL echo(concat("CAPTURE GG WARNING: ",trim(r.line)))
      ENDIF
      dmm_start_pos = findstring(dmm_gg_warning_lit,r.line), dmm_warn_str = trim(substring((
        dmm_start_pos+ size(dmm_gg_warning_lit)),(size(r.line) - (dmm_start_pos+ size(
         dmm_gg_warning_lit))),r.line),3), dmm_ignore = 0
      IF ((dmm_ignore_warn->ccnt > 0))
       FOR (dmm_iter = 1 TO dmm_ignore_warn->ccnt)
         IF ((dmm_ignore_warn->cap_qual[dmm_iter].warn_nbr=substring(1,size(dmm_ignore_warn->
           cap_qual[dmm_iter].warn_nbr),dmm_warn_str)))
          IF (size(dmm_ignore_warn->cap_qual[dmm_iter].warn_str) > 0)
           IF ((dmm_ignore_warn->cap_qual[dmm_iter].multi_str=0))
            IF (findstring(dmm_ignore_warn->cap_qual[dmm_iter].warn_str,dmm_warn_str)=1)
             dmm_ignore = 1, dmm_warn_str = dmm_ignore_warn->cap_qual[dmm_iter].warn_nbr, dmm_iter =
             dmm_ignore_warn->ccnt
            ENDIF
           ELSE
            dmm_search1 = findstring(substring(1,(findstring(char(42),dmm_ignore_warn->cap_qual[
               dmm_iter].warn_str) - 1),dmm_ignore_warn->cap_qual[dmm_iter].warn_str),dmm_warn_str,1)
            IF (dmm_search1 > 0)
             dmm_search2 = findstring(substring((findstring(char(42),dmm_ignore_warn->cap_qual[
                dmm_iter].warn_str)+ 1),(size(dmm_ignore_warn->cap_qual[dmm_iter].warn_str) -
               findstring(char(42),dmm_ignore_warn->cap_qual[dmm_iter].warn_str)),dmm_ignore_warn->
               cap_qual[dmm_iter].warn_str),dmm_warn_str,dmm_search1)
             IF (dmm_search2 > 0)
              dmm_ignore = 1, dmm_warn_str = dmm_ignore_warn->cap_qual[dmm_iter].warn_nbr, dmm_iter
               = dmm_ignore_warn->ccnt
             ENDIF
      ...
      IF (dmm_ignore=1)
       IF ((dm_err->debug_flag > 1))
        CALL echo(concat("The following GoldenGate Warning [",dmm_gg_warning_lit," ",dmm_warn_str,
         "] ",
         "has been ignored:  ",trim(r.line,3)))
       ENDIF
      ELSE
       dmm_gg_warning_ind = 1, dmm_file_contents->cnt = (dmm_file_contents->cnt+ 1), stat = alterlist
       (dmm_file_contents->qual,dmm_file_contents->cnt),
       dmm_file_contents->qual[dmm_file_contents->cnt].line = trim(r.line,3)
      ENDIF
     ELSEIF (r.line=patstring(dmm_gg_error_str))
      IF ((dm_err->debug_flag > 2))
       CALL echo(concat("CAPTURE GG ERROR: ",trim(r.line)))
      ENDIF
      dmm_start_pos = findstring(dmm_gg_error_lit,r.line), dmm_error_str = trim(substring((
        dmm_start_pos+ size(dmm_gg_error_lit)),(size(r.line) - (dmm_start_pos+ size(dmm_gg_error_lit)
        )),r.line),3), dmm_ignore = 0
      IF ((dmm_ignore_err->ccnt > 0))
       FOR (dmm_iter = 1 TO dmm_ignore_err->ccnt)
         IF ((dmm_ignore_err->cap_qual[dmm_iter].error_nbr=substring(1,size(dmm_ignore_err->cap_qual[
           dmm_iter].error_nbr),dmm_error_str)))
          IF (size(dmm_ignore_err->cap_qual[dmm_iter].error_str) > 0)
           IF ((dmm_ignore_err->cap_qual[dmm_iter].multi_str=0))
            IF (findstring(dmm_ignore_err->cap_qual[dmm_iter].error_str,dmm_error_str)=1)
             dmm_ignore = 1, dmm_error_str = dmm_ignore_err->cap_qual[dmm_iter].error_nbr, dmm_iter
              = dmm_ignore_err->ccnt
            ENDIF
           ELSE
            dmm_search1 = findstring(substring(1,(findstring(char(42),dmm_ignore_err->cap_qual[
               dmm_iter].error_str) - 1),dmm_ignore_err->cap_qual[dmm_iter].error_str),dmm_error_str,
             1)
            IF (dmm_search1 > 0)
             dmm_search2 = findstring(substring((findstring(char(42),dmm_ignore_err->cap_qual[
                dmm_iter].error_str)+ 1),(size(dmm_ignore_err->cap_qual[dmm_iter].error_str) -
               findstring(char(42),dmm_ignore_err->cap_qual[dmm_iter].error_str)),dmm_ignore_err->
               cap_qual[dmm_iter].error_str),dmm_error_str,dmm_search1)
             IF (dmm_search2 > 0)
              dmm_ignore = 1, dmm_error_str = dmm_ignore_err->cap_qual[dmm_iter].error_nbr, dmm_iter
               = dmm_ignore_err->ccnt
             ENDIF
      ...
      IF (dmm_ignore=1)
       IF ((dm_err->debug_flag > 1))
        CALL echo(concat("The following GoldenGate Capture Error [",dmm_gg_error_lit," ",
         dmm_error_str,"] ",
         "has been ignored:  ",trim(r.line,3)))
       ENDIF
      ELSE
       dmm_gg_error_ind = 1, dmm_file_contents->cnt = (dmm_file_contents->cnt+ 1), stat = alterlist(
        dmm_file_contents->qual,dmm_file_contents->cnt),
       dmm_file_contents->qual[dmm_file_contents->cnt].line = trim(r.line,3)
      ENDIF
     ENDIF
    ENDIF
   FOOT REPORT
    dmm_capture_timestamp = dmm_tmp_capture_timestamp
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  SET dmm_file_contents->file_name = concat("Issues occurred in ",dmr_mig_data->cap_dir,"ggserr.log")
  IF (dmm_write_to_monitor_file("")=0)
   GO TO exit_script
  ENDIF
  IF ((dm2_sys_misc->cur_os="AXP"))
   IF (dm2_push_dcl(concat("DELETE ",dmm_file_location,"dm2_mig_monitoring_cap.log;",char(42)))=0)
    GO TO exit_script
   ENDIF
  ELSE
   IF (dm2_push_dcl(concat("rm -f ",dmm_file_location,"dm2_mig_monitoring_cap.log"))=0)
    GO TO exit_script
   ENDIF
  ENDIF
 ENDIF
 IF (((dmm_report_all) OR (dmm_report_delivery)) )
  SET dm_err->eproc = "Searching or copying delivery ggserr.log to find new items."
  IF (dmm_delivery_timestamp="")
   IF ((dm2_sys_misc->cur_os="AXP"))
    IF (dm2_push_dcl(concat("COPY ",dmr_mig_data->del_dir,"ggserr.log ",dmm_file_location,
      "dm2_mig_monitoring_del.log"))=0)
     GO TO exit_script
    ENDIF
   ELSE
    IF (dm2_push_dcl(concat("cp ",dmr_mig_data->del_dir,"/ggserr.log ",dmm_file_location,
      "dm2_mig_monitoring_del.log"))=0)
     GO TO exit_script
    ENDIF
   ENDIF
  ELSE
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET dmm_push_dcl = concat("pipe search ",dmr_mig_data->del_dir,'ggserr.log "',
     dmm_delivery_timestamp,'" /remaining > ',
     dmm_file_location,"dm2_mig_monitoring_del.log")
   ELSEIF ((dm2_sys_misc->cur_os="LNX"))
    SET dmm_push_dcl = concat("tail -n +`grep -n '",dmm_delivery_timestamp,"' ",dmr_mig_data->del_dir,
     '/ggserr.log | cut -d ":" -f 1 | head -n 1` ',
     dmr_mig_data->del_dir,"/ggserr.log > ",dmm_file_location,"dm2_mig_monitoring_del.log")
   ELSE
    SET dmm_push_dcl = concat("tail +`grep -n '",dmm_delivery_timestamp,"' ",dmr_mig_data->del_dir,
     '/ggserr.log | cut -d ":" -f 1 | head -n 1` ',
     dmr_mig_data->del_dir,"/ggserr.log > ",dmm_file_location,"dm2_mig_monitoring_del.log")
   ENDIF
   IF ((dm_err->debug_flag > 0))
   ...
   IF ((dm2_sys_misc->cur_os="AXP"))
    ...
    IF (dmm_push_dcl_ret=0)
     FREE DEFINE rtl
     FREE SET dmm_file
     SET logical dmm_file value(build(dmm_file_location,"^dm2_mig_monitoring_del.log"))
     DEFINE rtl "dmm_file"
     SET dm_err->eproc = "Reading contents of error file."
     SELECT INTO "nl:"
      FROM rtlt r
  ...
  SET logical dmm_file value(build(dmm_file_location,"dm2_mig_monitoring_del.log"))
  DEFINE rtl2 "dmm_file"
  SET dm_err->eproc = "Reading Golden Gate delivery file for warnings and errors."
  SELECT INTO "nl:"
   FROM rtl2t r
   HEAD REPORT
    dmm_tmp_delivery_timestamp = dmm_delivery_timestamp
   DETAIL
    IF (((dmm_delivery_timestamp="") OR (trim(substring(1,19,r.line)) != dmm_delivery_timestamp)) )
     dmm_tmp_delivery_timestamp = concat(substring(1,10,r.line)," ",substring(12,8,r.line))
     IF (r.line=patstring(dmm_gg_warning_str))
      dmm_start_pos = findstring(dmm_gg_warning_lit,r.line), dmm_warn_str = trim(substring((
        dmm_start_pos+ size(dmm_gg_warning_lit)),(size(r.line) - (dmm_start_pos+ size(
         dmm_gg_warning_lit))),r.line),3), dmm_ignore = 0
      IF ((dmm_ignore_warn->dcnt > 0))
       FOR (dmm_iter = 1 TO dmm_ignore_warn->dcnt)
         IF ((dmm_ignore_warn->del_qual[dmm_iter].warn_nbr=substring(1,size(dmm_ignore_warn->
           del_qual[dmm_iter].warn_nbr),dmm_warn_str)))
          IF (size(dmm_ignore_warn->del_qual[dmm_iter].warn_str) > 0)
           IF ((dmm_ignore_warn->del_qual[dmm_iter].multi_str=0))
            IF (findstring(dmm_ignore_warn->del_qual[dmm_iter].warn_str,dmm_warn_str)=1)
             dmm_ignore = 1, dmm_warn_str = dmm_ignore_warn->del_qual[dmm_iter].warn_nbr, dmm_iter =
             dmm_ignore_warn->dcnt
            ENDIF
           ELSE
            dmm_search1 = findstring(substring(1,(findstring(char(42),dmm_ignore_warn->del_qual[
               dmm_iter].warn_str) - 1),dmm_ignore_warn->del_qual[dmm_iter].warn_str),dmm_warn_str,1)
            IF (dmm_search1 > 0)
             dmm_search2 = findstring(substring((findstring(char(42),dmm_ignore_warn->del_qual[
                dmm_iter].warn_str)+ 1),(size(dmm_ignore_warn->del_qual[dmm_iter].warn_str) -
               findstring(char(42),dmm_ignore_warn->del_qual[dmm_iter].warn_str)),dmm_ignore_warn->
               del_qual[dmm_iter].warn_str),dmm_warn_str,dmm_search1)
             IF (dmm_search2 > 0)
              dmm_ignore = 1, dmm_warn_str = dmm_ignore_warn->del_qual[dmm_iter].warn_nbr, dmm_iter
               = dmm_ignore_warn->dcnt
             ENDIF
     ...
     ELSEIF (r.line=patstring(dmm_gg_error_str))
      IF ((dm_err->debug_flag > 2))
       CALL echo(concat("DELIVERY GG ERROR: ",trim(r.line)))
      ENDIF
      dmm_start_pos = findstring(dmm_gg_error_lit,r.line), dmm_error_str = trim(substring((
        dmm_start_pos+ size(dmm_gg_error_lit)),(size(r.line) - (dmm_start_pos+ size(dmm_gg_error_lit)
        )),r.line),3), dmm_ignore = 0
      IF ((dmm_ignore_err->dcnt > 0))
       FOR (dmm_iter = 1 TO dmm_ignore_err->dcnt)
         IF ((dmm_ignore_err->del_qual[dmm_iter].error_nbr=substring(1,size(dmm_ignore_err->del_qual[
           dmm_iter].error_nbr),dmm_error_str)))
          IF (size(dmm_ignore_err->del_qual[dmm_iter].error_str) > 0)
           IF ((dmm_ignore_err->del_qual[dmm_iter].multi_str=0))
            IF (findstring(dmm_ignore_err->del_qual[dmm_iter].error_str,dmm_error_str)=1)
             dmm_ignore = 1, dmm_error_str = dmm_ignore_err->del_qual[dmm_iter].error_nbr, dmm_iter
              = dmm_ignore_err->dcnt
            ENDIF
           ELSE
            dmm_search1 = findstring(substring(1,(findstring(char(42),dmm_ignore_err->del_qual[
               dmm_iter].error_str) - 1),dmm_ignore_err->del_qual[dmm_iter].error_str),dmm_error_str,
             1)
            IF (dmm_search1 > 0)
             dmm_search2 = findstring(substring((findstring(char(42),dmm_ignore_err->del_qual[
                dmm_iter].error_str)+ 1),(size(dmm_ignore_err->del_qual[dmm_iter].error_str) -
               findstring(char(42),dmm_ignore_err->del_qual[dmm_iter].error_str)),dmm_ignore_err->
               del_qual[dmm_iter].error_str),dmm_error_str,dmm_search1)
             IF (dmm_search2 > 0)
              dmm_ignore = 1, dmm_error_str = dmm_ignore_err->del_qual[dmm_iter].error_nbr, dmm_iter
               = dmm_ignore_err->dcnt
             ENDIF
      ...
      IF (dmm_ignore=1)
       IF ((dm_err->debug_flag > 1))
        CALL echo(concat("The following GoldenGate Delivery Error [",dmm_gg_error_lit," ",
         dmm_error_str,"] ",
         "has been ignored:  ",trim(r.line,3)))
       ENDIF
      ELSE
       ...
       dmm_file_contents->qual[dmm_file_contents->cnt].line = trim(r.line,3)
      ENDIF
     ELSEIF (r.line=patstring(concat("*",dmm_gg_ddl_excl_code,"*")))
      IF (((r.line=patstring(concat("*",dmm_gg_ddl_ts_str,"*"))) OR (r.line=patstring(concat("*",
        dmm_gg_ddl_dbl_str,"*")))) )
       IF ((dm_err->debug_flag > 2))
        CALL echo(concat("DELIVERY GG EXCLUDED DDL: ",trim(r.line)))
       ENDIF
       dmm_gg_ddl_excl_ind = 1, dmm_file_contents->cnt = (dmm_file_contents->cnt+ 1), stat =
       alterlist(dmm_file_contents->qual,dmm_file_contents->cnt),
       dmm_file_contents->qual[dmm_file_contents->cnt].line = trim(r.line,3)
      ENDIF
     ELSEIF (r.line=patstring(dmm_gg_ddl_ignore_str))
      IF ((dm_err->debug_flag > 2))
       CALL echo(concat("DELIVERY GG IGNORED DDL ERROR: ",trim(r.line)))
      ENDIF
      dmm_gg_ddl_ignore_ind = 1, dmm_file_contents->cnt = (dmm_file_contents->cnt+ 1), stat =
      alterlist(dmm_file_contents->qual,dmm_file_contents->cnt),
      dmm_file_contents->qual[dmm_file_contents->cnt].line = trim(r.line,3)
     ENDIF
    ENDIF
   FOOT REPORT
    dmm_delivery_timestamp = dmm_tmp_delivery_timestamp
   WITH nocounter, maxcol = 2000
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dmm_file_contents->file_name = concat("Issues occurred in ",dmr_mig_data->del_dir,"/ggserr.log"
   )
  IF (dmm_write_to_monitor_file("")=0)
   GO TO exit_script
  ENDIF
  IF ((dm2_sys_misc->cur_os="AXP"))
   IF (dm2_push_dcl(concat("DELETE ",dmm_file_location,"dm2_mig_monitoring_del.log;",char(42)))=0)
    GO TO exit_script
   ENDIF
  ELSE
   IF (dm2_push_dcl(concat("rm -f ",dmm_file_location,"dm2_mig_monitoring_del.log"))=0)
    GO TO exit_script
   ENDIF
  ENDIF
 ENDIF
 IF (((dmm_report_all) OR (dmm_report_capture)) )
  IF (dmm_capture_ts_exists_ind)
   SET dm_err->eproc = "Updating last timestamp for capture in dm_info."
   UPDATE  FROM dm_info di
    SET di.info_char = dmm_capture_timestamp
    WHERE di.info_domain="DM2_MIG_DATA"
     AND di.info_name="CAPTURE_ERROR_LOG"
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
   ...
   ENDIF
  ELSE
   SET dm_err->eproc = "Inserting initial capture timestamp into dm_info."
   INSERT  FROM dm_info di
    SET di.info_domain = "DM2_MIG_DATA", di.info_name = "CAPTURE_ERROR_LOG", di.info_char =
     dmm_capture_timestamp
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
 ...
 ENDIF
 IF (((dmm_report_all) OR (dmm_report_delivery)) )
  IF (dmm_delivery_ts_exists_ind)
   SET dm_err->eproc = "Updating last timestamp for delivery in dm_info."
   UPDATE  FROM dm_info di
    SET di.info_char = dmm_delivery_timestamp
    WHERE di.info_domain="DM2_MIG_DATA"
     AND di.info_name="DELIVERY_ERROR_LOG"
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
   ...
   ENDIF
  ELSE
   SET dm_err->eproc = "Inserting initial delivery timestamp into dm_info."
   INSERT  FROM dm_info di
    SET di.info_domain = "DM2_MIG_DATA", di.info_name = "DELIVERY_ERROR_LOG", di.info_char =
     dmm_delivery_timestamp
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
  ...
  COMMIT
 ENDIF
 IF ( NOT (dmm_gg_error_ind)
  AND  NOT (dm_err->err_ind))
  IF (dmm_start_timestamp >= dmm_heartbeat_timestamp)
   IF (dmm_check_hb_tbls(null)=0)
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = "Generating temp file for heartbeat email."
   SELECT INTO build(dmm_file_location,"dm2_mig_heartbeat.log")
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     col 0,
     CALL print(concat("Monitoring heartbeat every ",build(dmm_heartbeat_time),
      " hours.  No errors detected.  Next heartbeat at ",format(cnvtlookahead(build(
         dmm_heartbeat_time,",H"),dmm_start_timestamp),";;Q"),".")), row + 1
     IF ((dmm_latency->cnt > 0))
      col 0,
      CALL print("Current Latency : "), row + 1,
      col 5,
      CALL print("Local Database"), col 30,
      CALL print("Remote Database"), col 60,
      CALL print("Incomming Path"),
      col 120,
      CALL print("Incomming Lag"), row + 1,
      col 5,
      CALL print("--------------"), col 30,
      CALL print("---------------"), col 60,
      CALL print("--------------"),
      col 120,
      CALL print("-------------"), row + 1
      FOR (dmm_latency_cnt = 1 TO dmm_latency->cnt)
        col 5,
        CALL print(dmm_latency->qual[dmm_latency_cnt].local_database), col 30,
        CALL print(dmm_latency->qual[dmm_latency_cnt].remote_database), col 60,
        CALL print(dmm_latency->qual[dmm_latency_cnt].incoming_path),
        col 120,
        CALL print(dmm_latency->qual[dmm_latency_cnt].incoming_lag), row + 1
      ENDFOR
     ENDIF
    WITH nocounter, maxcol = 4000, format = variable
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
   IF (dmr_send_email("DATABASE MIGRATION : Heartbeat Status",dmr_emails->email_list,build(
     dmm_file_location,"dm2_mig_heartbeat.log"))=0)
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = "Updating last heartbeat timestamp in dm_info."
   UPDATE  FROM dm_info di
    SET di.info_date = cnvtlookahead(build(dmm_heartbeat_time,",H"),dmm_start_timestamp)
    WHERE di.info_domain="DM2_MIG_DATA"
     AND di.info_name=value(evaluate(dmm_report_delivery,1,"MONITOR_DELIVERY_HEARTBEAT",
      "MONITOR_CAPTURE_HEARTBEAT"))
    WITH nocounter
 ...
 ENDIF
 GO TO exit_script
 SUBROUTINE dmm_write_to_monitor_file(single_line)
   IF (trim(single_line) != "")
    SET dm_err->eproc = "Appending single line message to log file."
    SELECT INTO value(dmm_log_file)
     FROM (dummyt d  WITH seq = 1)
     DETAIL
      row + 1,
      CALL print(single_line)
     WITH nocounter, append, maxcol = 2100,
      format = variable
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ENDIF
   IF ((dmm_file_contents->cnt > 0))
    SET dm_err->eproc = "Appending message to log file."
    SELECT INTO value(dmm_log_file)
     FROM (dummyt d  WITH seq = value(dmm_file_contents->cnt))
     HEAD REPORT
      IF ((dmm_file_contents->file_name != ""))
       row + 1,
       CALL print(dmm_file_contents->file_name), row + 1,
       " ", row + 1, " "
      ENDIF
     DETAIL
      row + 1, "  ",
      CALL print(dmm_file_contents->qual[d.seq].line)
     FOOT REPORT
      dmm_file_contents->cnt = 0, stat = alterlist(dmm_file_contents->qual,0), dmm_file_contents->
      file_name = ""
     WITH nocounter, append, maxcol = 2100,
      format = variable
    ;end select
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmm_check_hb_tbls(null)
   DECLARE dcht_tgt_cnt = i2 WITH protect, noconstant(0)
   DECLARE dcht_src_cnt = i2 WITH protect, noconstant(0)
   SET dm_err->eproc = "Check if no report for HEARTBEAT exists in dm_info."
   SELECT INTO "nl:"
    FROM dm_info
    WHERE info_domain="DM2_MIG_GG_HEARTBEAT"
     AND info_name="NOMONITOR"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->eproc = "Check if MIG_TARGET database link exists."
    SELECT INTO "nl:"
     FROM dba_db_links
     WHERE db_link="MIG_TARGET.*"
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Query based on MIG_TARGET database link exists or not."
    IF (curqual > 0)
     SELECT INTO "nl:"
      dcht_cnt = count(*)
      FROM dba_tables@mig_target dt
      WHERE dt.owner="V500"
       AND dt.table_name IN ("GG_HEARTBEAT", "GG_HEARTBEAT_HISTORY")
      DETAIL
       dcht_tgt_cnt = dcht_cnt
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     ENDIF
    ENDIF
    SET dm_err->eproc = "Get the latency for replicat process."
    IF (((dcht_tgt_cnt+ dcht_src_cnt)=4))
     SELECT INTO "nl:"
      local_database = t.local_database, remote_database = t.remote_database, incoming_path = t
      .incoming_path,
      incoming_lag = t.incoming_lag
      FROM (
       (
       (SELECT
        local_database = gl.local_database, remote_database = gl.remote_database, incoming_path = gl
        .incoming_path,
        incoming_lag = gl.incoming_lag
        FROM gg_lag@mig_target gl
        WITH sqltype("VC512","VC512","VC4000","I4")))
       t)
      HEAD REPORT
       dmm_latency->cnt = 0, stat = alterlist(dmm_latency->qual,0)
      DETAIL
       dmm_latency->cnt = (dmm_latency->cnt+ 1), stat = alterlist(dmm_latency->qual,dmm_latency->cnt),
       dmm_latency->qual[dmm_latency->cnt].local_database = local_database,
       dmm_latency->qual[dmm_latency->cnt].remote_database = remote_database, dmm_latency->qual[
       dmm_latency->cnt].incoming_path = incoming_path, dmm_latency->qual[dmm_latency->cnt].
       incoming_lag = incoming_lag
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "Ending dm2_mig_monitoring."
 CALL final_disp_msg("dm2_mig_monitoring")
 SET dmm_temp_err_ind = dm_err->err_ind
 IF (((dmm_temp_err_ind) OR (dmm_gg_error_ind))
  AND size(dmr_emails->email_list) > 1)
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "Updating next heartbeat time in dm_info."
  UPDATE  FROM dm_info di
   SET di.info_date = cnvtlookahead(build(dmm_heartbeat_time,",H"),dmm_start_timestamp)
   WHERE di.info_domain="DM2_MIG_DATA"
    AND di.info_name=value(evaluate(dmm_report_delivery,1,"MONITOR_DELIVERY_HEARTBEAT",
  ...
  COMMIT
  IF (dmm_temp_err_ind
   AND trim(dmm_log_file) != "")
   FREE DEFINE rtl
   FREE SET dmm_file
   SET logical dmm_file value(dm_err->logfile)
   DEFINE rtl "dmm_file"
   SET dm_err->eproc = "Reading contents of log file."
   SELECT INTO "nl:"
    FROM rtlt r
    DETAIL
     dmm_file_contents->cnt = (dmm_file_contents->cnt+ 1)
     IF (mod(dmm_file_contents->cnt,15)=1)
      stat = alterlist(dmm_file_contents->qual,(dmm_file_contents->cnt+ 14))
     ENDIF
     dmm_file_contents->qual[dmm_file_contents->cnt].line = build(r.line)
    FOOT REPORT
     stat = alterlist(dmm_file_contents->qual,dmm_file_contents->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
   SET dmm_file_contents->file_name = concat("Errors occurred in ",dm_err->logfile)
   SET dm_err->err_ind = 0
   CALL dmm_write_to_monitor_file("")
  ENDIF
 ENDIF
 IF (trim(dmm_log_file) != ""
  AND size(dmr_emails->email_list) > 1)
  IF (dmm_temp_err_ind)
   SET dm_err->err_ind = 0
   SET dm_err->eproc = "Sending email for fatal errors detected."
   CALL disp_msg("",dm_err->logfile,0)
   CALL dmr_send_email("DATABASE MIGRATION : Monitoring Errors Detected.  Monitoring Job Stopped!",
    dmr_emails->email_list,dmm_log_file)
  ELSEIF (dmm_gg_error_ind)
   SET dm_err->err_ind = 0
   SET dm_err->eproc = "Sending email for errors detected."
   CALL disp_msg("",dm_err->logfile,0)
   CALL dmr_send_email(
    "DATABASE MIGRATION : Monitoring Errors Detected.  Monitoring Job Still Running",dmr_emails->
    email_list,dmm_log_file)
  ELSEIF ( NOT (dmm_gg_error_ind)
   AND  NOT (dmm_temp_err_ind)
   AND dmm_gg_warning_ind)
   SET dm_err->err_ind = 0
   SET dm_err->eproc = "Sending email for warnings detected."
   CALL disp_msg("",dm_err->logfile,0)
   CALL dmr_send_email(
    "DATABASE MIGRATION : Monitoring Warnings Detected.  Monitoring Job Still Running",dmr_emails->
    email_list,dmm_log_file)
  ELSEIF ( NOT (dmm_gg_error_ind)
   AND  NOT (dmm_temp_err_ind)
   AND  NOT (dmm_gg_warning_ind)
   AND dmm_gg_ddl_excl_ind)
   SET dm_err->err_ind = 0
   SET dm_err->eproc = "Sending email for DDL exclusions detected."
   CALL disp_msg("",dm_err->logfile,0)
   CALL dmr_send_email("DATABASE MIGRATION : DDL Exclusions Detected.  Monitoring Job Still Running",
    dmr_emails->email_list,dmm_log_file)
  ELSEIF ( NOT (dmm_gg_error_ind)
   AND  NOT (dmm_temp_err_ind)
   AND  NOT (dmm_gg_warning_ind)
   AND  NOT (dmm_gg_ddl_excl_ind)
   AND dmm_gg_ddl_ignore_ind)
   SET dm_err->err_ind = 0
   SET dm_err->eproc = "Sending email for Ignored DDL errors detected."
   CALL disp_msg("",dm_err->logfile,0)
   CALL dmr_send_email(
    "DATABASE MIGRATION : Ignored DDL Errors Detected.  Monitoring Job Still Running",dmr_emails->
    email_list,dmm_log_file)
  ENDIF
 ENDIF
 IF ( NOT (dmm_temp_err_ind))
  SET dm_err->eproc = "Pausing monitoring job..."
  CALL disp_msg("",dm_err->logfile,0)
  CALL pause((60 * dmm_monitor_time))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "Determining if STOP_MONITORING already exists in dm_info."
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="DM2_MIG_DATA"
    AND di.info_name=value(evaluate(dmm_report_delivery,1,"STOP_DELIVERY_MONITORING",
     "STOP_CAPTURE_MONITORING"))
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (curqual > 0)
   SET dm_err->err_ind = 0
   SET dm_err->eproc = "Deleting STOP_MONITORING row in dm_info."
   DELETE  FROM dm_info di
    WHERE di.info_domain="DM2_MIG_DATA"
     AND di.info_name=value(evaluate(dmm_report_delivery,1,"STOP_DELIVERY_MONITORING",
      "STOP_CAPTURE_MONITORING"))
  ...
  ELSE
   SET dm_err->err_ind = 0
   SET dm_err->eproc = "Re-submitting monitoring job."
   IF ((dm2_sys_misc->cur_os="AXP"))
    CALL dm2_push_dcl(concat("SUBMIT /QUEUE=",dmr_batch_queue," /NAME=dm2_mig_monitoring ",
      dmm_file_location,"dm2_mig_monitoring.com /NOLOG_FILE"))
   ELSE
    CALL dm2_push_dcl(concat("nohup ",dmm_file_location,"dm2_mig_monitoring.ksh > /dev/null 2>&1 &"))
   ENDIF
  ENDIF
 ENDIF
 SET dm_err->err_ind = 0
 IF (dmm_file_location != build(logical("CCLUSERDIR"),"/")
  AND dmm_file_location != "")
  IF ((dm2_sys_misc->cur_os="AXP"))
   CALL dm2_push_dcl(concat("COPY ",dm_err->logfile," ",dmm_file_location))
   CALL dm2_push_dcl(concat("DELETE ",dm_err->logfile,";*"))
  ELSE
   CALL dm2_push_dcl(concat("cp ",dm_err->logfile," ",dmm_file_location))
   CALL dm2_push_dcl(concat("rm ",dm_err->logfile))
  ENDIF
 ENDIF
END GO
