CREATE PROGRAM acm_add_prsnl_alias:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->prsnl_alias_qual[xref->add[index].idx].prsnl_alias_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(prsnl_seq,nextval)
     FROM dual
     DETAIL
      acm_request->prsnl_alias_qual[xref->add[index].idx].prsnl_alias_id = cnvtreal(nextseqnum),
      reply->prsnl_alias_qual[xref->add[index].idx].prsnl_alias_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->prsnl_alias_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->prsnl_alias_qual[xref->add[index].idx].prsnl_alias_id = acm_request->prsnl_alias_qual[
    xref->add[index].idx].prsnl_alias_id
   ENDIF
   SET reply->prsnl_alias_qual[xref->add[index].idx].status = 0
   IF ((acm_request->prsnl_alias_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->prsnl_alias_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->prsnl_alias_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->prsnl_alias_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   prsnl_alias p
  SET p.prsnl_alias_id = acm_request->prsnl_alias_qual[xref->add[d.seq].idx].prsnl_alias_id, p.alias
    = trim(acm_request->prsnl_alias_qual[xref->add[d.seq].idx].alias,3), p.alias_pool_cd =
   acm_request->prsnl_alias_qual[xref->add[d.seq].idx].alias_pool_cd,
   p.beg_effective_dt_tm = cnvtdatetime(acm_request->prsnl_alias_qual[xref->add[d.seq].idx].
    beg_effective_dt_tm), p.contributor_system_cd =
   IF ((acm_request->prsnl_alias_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0)) acm_request
    ->prsnl_alias_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_alias_qual[xref->add[d.seq].idx].
    end_effective_dt_tm),
   p.person_id = acm_request->prsnl_alias_qual[xref->add[d.seq].idx].person_id, p
   .prsnl_alias_sub_type_cd = acm_request->prsnl_alias_qual[xref->add[d.seq].idx].
   prsnl_alias_sub_type_cd, p.prsnl_alias_type_cd = acm_request->prsnl_alias_qual[xref->add[d.seq].
   idx].prsnl_alias_type_cd,
   p.data_status_cd =
   IF ((acm_request->prsnl_alias_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request->
    prsnl_alias_qual[xref->add[d.seq].idx].data_status_cd
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->prsnl_alias_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->prsnl_alias_qual[xref->add[index].idx].status=0))
