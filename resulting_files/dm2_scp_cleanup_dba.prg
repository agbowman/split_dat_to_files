CREATE PROGRAM dm2_scp_cleanup:dba
 ...
 SUBROUTINE ddr_val_client_mnemonic(dvcm_src_ind,dvcm_tgt_ind,dvcm_inform_only_ind,
  dvcm_invalid_data_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsc_new_start = i4 WITH protect, noconstant(0)
 DECLARE dsc_top_line = i4 WITH protect, noconstant(0)
 DECLARE dsc_bottom_line = i4 WITH protect, noconstant(0)
 DECLARE dsc_cur_line = i4 WITH protect, noconstant(0)
 DECLARE dsc_max_value = i4 WITH protect, noconstant(0)
 DECLARE dsc_max_scroll = i4 WITH protect, noconstant(0)
 DECLARE dsc_row_nbr = i4 WITH protect, noconstant(0)
 DECLARE dsc_scroll_accept = c1 WITH protect, noconstant(" ")
 DECLARE dsc_done = i2 WITH protect, noconstant(0)
 DECLARE dsc_tmp_main = vc WITH protect, noconstant(" ")
 DECLARE dsc_scp = i2 WITH protect, noconstant(0)
 DECLARE dsc_exp_prompt_done = i2 WITH protect, noconstant(0)
 DECLARE dsc_new_prompt_done = i2 WITH protect, noconstant(0)
 FREE RECORD dsc_tmp_scp
 RECORD dsc_tmp_scp(
   1 scp_cnt = i2
   1 main = vc
   1 scp[*]
 ...
 SET dsc_tmp_scp->scp_cnt = 0
 DECLARE dsc_load_scp(null) = i2
 DECLARE dsc_fill_screen(null) = null
 DECLARE dsc_export_scp(null) = i2
 SET width = 132
 IF (check_logfile("dm2_scp_cleanup",".log","dm2_scp_cleanup")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Begin dm2_scp_cleanup"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (((validate(ddr_scp_data->location,"DM2NOTSET")="DM2NOTSET") OR (((validate(ddr_scp_data->
  tmp_file,"DM2NOTSET")="DM2NOTSET") OR (validate(ddr_scp_data->new_file,"DM2NOTSET")="DM2NOTSET"))
 )) )
  SET message = window
  CALL clear(1,1)
  CALL box(1,1,10,131)
  CALL text(2,50,"SCP DATA COLLECTION MENU")
  CALL text(4,2,"Enter SCP export file location :")
  CALL accept(4,45,"P(60);C",dm2_install_schema->ccluserdir
   WHERE  NOT (curaccept=""))
  SET ddr_scp_data->location = trim(curaccept)
  IF ((dm2_sys_misc->cur_os="AXP"))
   IF (findstring("]",ddr_scp_data->location,1,1) != size(trim(ddr_scp_data->location)))
    SET ddr_scp_data->location = concat(ddr_scp_data->location,"]")
   ENDIF
  ELSE
   IF (substring(1,1,ddr_scp_data->location) != "/")
    SET ddr_scp_data->location = concat("/",ddr_scp_data->location)
   ENDIF
   IF (findstring("/",ddr_scp_data->location,1,1) != size(trim(ddr_scp_data->location)))
    SET ddr_scp_data->location = concat(ddr_scp_data->location,"/")
   ENDIF
  ENDIF
  WHILE (dsc_exp_prompt_done=0)
    CALL clear(5,2,110)
    CALL text(5,2,"Enter SCP export file name (ie. <domain_name>_save.scp):")
    CALL accept(5,60,"P(30);C"," "
     WHERE curaccept != " ")
    SET ddr_scp_data->tmp_file = trim(curaccept)
    IF (findstring(".",ddr_scp_data->tmp_file,1,0)=0)
     CALL text(6,2,"SCP export file must have an extension.")
    ELSE
     CALL clear(6,2,110)
     IF (dm2_findfile(concat(ddr_scp_data->location,ddr_scp_data->tmp_file))=0)
      CALL text(6,2,"File entered is not found.")
     ELSE
      SET dsc_exp_prompt_done = 1
     ENDIF
    ENDIF
  ENDWHILE
  WHILE (dsc_new_prompt_done=0)
    CALL clear(6,2,110)
    CALL text(6,2,"Enter new SCP file name (ie. <domain_name>_new.scp):")
    CALL accept(6,60,"P(30);C"," "
     WHERE curaccept != " ")
    SET ddr_scp_data->new_file = trim(curaccept)
    IF ((ddr_scp_data->tmp_file=ddr_scp_data->new_file))
     CALL text(7,2,"New SCP file name must be different than SCP export file name!")
    ELSE
     CALL clear(7,2,110)
     IF (findstring(".",ddr_scp_data->new_file,1,0)=0)
      CALL text(7,2,"New SCP file name must have an extension.")
     ELSE
      SET dsc_new_prompt_done = 1
     ENDIF
    ENDIF
  ENDWHILE
  CALL clear(8,2,110)
  ...
  SET message = nowindow
 ENDIF
 IF (dsc_load_scp(null)=0)
  GO TO exit_script
 ENDIF
 SET dsc_max_value = dsc_tmp_scp->scp_cnt
 SET message = window
 WHILE (dsc_done=0)
   SET dsc_scroll_accept = " "
   SET dsc_done = 0
   CALL clear(1,1)
   CALL box(1,1,22,132)
   CALL text(2,50,"SCP FILE CLEANUP")
   CALL text(4,2,concat("Below is a list of exported SCP server entries.  Use the menu functions ",
     "to mark those servers to be deleted from the export file."))
   CALL text(5,2,concat("Once all servers to be deleted have been selected, choose Complete ",
     "Deletion to update the exported file."))
   CALL text(7,2,concat(
     "WARNING:  The com_clients and ops_exec servers must be removed at this time.",
     "  Do NOT remove any other Open Engine or MDI servers"))
   CALL text(8,2,"except for the com_clients.")
   CALL text(10,2,"Marked for Deletion")
   CALL text(10,25,"Id")
   CALL text(10,35,"Description")
   IF (dsc_cur_line <= 1)
    SET dsc_top_line = 1
    SET dsc_cur_line = 1
    SET dsc_bottom_line = 0
    SET dsc_max_scroll = 10
   ENDIF
   CALL dsc_fill_screen(null)
   CALL clear(23,1,132)
   CALL video(n)
   CALL text(23,2,
    "(S)elect Server for Deletion (or Deselect to Unmark), (C)omplete Deletion,  (Q)uit: ")
   WHILE (dsc_scroll_accept=" ")
    CALL accept(23,90,"P;CUS"," "
     WHERE curaccept IN ("S", "C", "Q"))
    CASE (curscroll)
     OF 0:
      SET dsc_scroll_accept = curaccept
     OF 1:
      IF (dsc_cur_line=dsc_bottom_line)
       SET dsc_cur_line = dsc_top_line
      ELSE
       IF (dsc_cur_line >= dsc_max_value)
        SET dsc_cur_line = dsc_top_line
       ELSE
        SET dsc_cur_line = (dsc_cur_line+ 1)
       ENDIF
      ENDIF
      CALL dsc_fill_screen(null)
     OF 2:
      IF (dsc_cur_line=dsc_top_line)
       IF (dsc_bottom_line > dsc_max_value)
        SET dsc_cur_line = dsc_max_value
       ELSE
        SET dsc_cur_line = dsc_bottom_line
       ENDIF
      ELSE
       SET dsc_cur_line = (dsc_cur_line - 1)
      ENDIF
     ...
     OF 5:
      SET dsc_new_start = 0
      IF (dsc_top_line > dsc_max_scroll)
       SET dsc_top_line = (dsc_top_line - dsc_max_scroll)
       SET dsc_bottom_line = ((dsc_top_line+ dsc_max_scroll) - 1)
      ELSE
       SET dsc_top_line = 1
       IF ((dsc_max_value > (dsc_max_scroll - 1)))
        SET dsc_bottom_line = (dsc_top_line+ (dsc_max_scroll - 1))
       ELSE
        SET dsc_bottom_line = dsc_max_value
       ENDIF
      ENDIF
      SET dsc_cur_line = dsc_top_line
      CALL dsc_fill_screen(null)
     OF 6:
      SET dsc_new_start = 0
      SET dsc_new_start = (dsc_top_line+ dsc_max_scroll)
      IF (dsc_max_value >= dsc_new_start)
       SET dsc_top_line = dsc_new_start
       SET dsc_cur_line = dsc_top_line
       IF (((dsc_top_line+ (dsc_max_scroll - 1)) <= dsc_max_value))
        SET dsc_bottom_line = (dsc_top_line+ (dsc_max_scroll - 1))
       ELSE
        SET dsc_bottom_line = dsc_max_value
       ENDIF
   ...
   CASE (dsc_scroll_accept)
    ...
    OF "C":
     ...
     SET dsc_done = 1
    OF "S":
     IF ((dsc_tmp_scp->scp[dsc_cur_line].delete_ind=0))
      SET dsc_tmp_scp->scp[dsc_cur_line].delete_ind = 1
      CALL text(dsc_row_nbr,10,"X")
     ELSE
      SET dsc_tmp_scp->scp[dsc_cur_line].delete_ind = 0
      CALL text(dsc_row_nbr,10," ")
     ENDIF
   ENDCASE
 ENDWHILE
 IF ((dm_err->debug_flag > 4))
  CALL echorecord(dsc_tmp_scp)
 ENDIF
 SET dsc_tmp_main = replace(dsc_tmp_scp->main,"entry=","",1)
 IF ((dm_err->debug_flag > 4))
  CALL echo(build("dsc_tmp_main = ",dsc_tmp_main))
 ENDIF
 FOR (dsc_scp = 1 TO dsc_tmp_scp->scp_cnt)
   IF ((dsc_tmp_scp->scp[dsc_scp].delete_ind=1))
    SET dsc_tmp_main = replace(concat(" ",dsc_tmp_main," "),concat(" ",dsc_tmp_scp->scp[dsc_scp].nbr,
      " ")," ",1)
   ENDIF
 ENDFOR
 IF ((dm_err->debug_flag > 4))
  CALL echo(build("dsc_tmp_main = ",dsc_tmp_main))
 ENDIF
 SET dsc_tmp_scp->main = concat("entry=",trim(dsc_tmp_main,3))
 IF ((dm_err->debug_flag > 4))
  CALL echo(build("dsc_tmp_scp->main =",dsc_tmp_scp->main))
 ENDIF
 IF (dsc_export_scp(null)=0)
  GO TO exit_script
 ENDIF
 GO TO exit_script
 SUBROUTINE dsc_load_scp(null)
   DECLARE dls_filename = vc WITH protect, noconstant(" ")
   DECLARE dls_file = vc WITH protect, noconstant(" ")
   SET dls_file = concat(ddr_scp_data->location,ddr_scp_data->tmp_file)
   SET dm_err->eproc = concat("Load ",dls_file," into memory.")
   CALL disp_msg("",dm_err->logfile,0)
   FREE DEFINE rtl2
   FREE SET dls_filename
   SET logical dls_filename value(dls_file)
   DEFINE rtl2 "dls_filename"
   SELECT INTO "nl:"
    t.line
    FROM rtl2t t
    WHERE t.line > " "
    HEAD REPORT
     beg_pos = 1, end_pos = 0, tag_cnt = 0,
     line_cnt = 0, main_flag = 0
    DETAIL
     beg_pos = 1, end_pos = 0
     IF (findstring("[main]",t.line,1,0) > 0)
      main_flag = 1
     ELSEIF (findstring("[",t.line,1,0)=1)
      end_pos = findstring(".",t.line,1,0)
      IF (end_pos=0)
       end_pos = findstring("]",t.line,1,0)
       IF (end_pos=0)
        dm_err->err_ind = 1, dm_err->emsg = "Incorrect SCP export file layout.",
        CALL cancel(1)
       ELSE
        dsc_tmp_scp->scp_cnt = (dsc_tmp_scp->scp_cnt+ 1), stat = alterlist(dsc_tmp_scp->scp,
         dsc_tmp_scp->scp_cnt), dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].nbr = substring(2,((end_pos -
         beg_pos) - 1),t.line),
        tag_cnt = 1, dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag_cnt = tag_cnt, stat = alterlist(
         dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag,tag_cnt),
        dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag[tag_cnt].tag_name = trim(t.line), line_cnt = 0
       ENDIF
      ELSE
       IF ((substring(2,((end_pos - beg_pos) - 1),t.line) != dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].
       nbr))
        dm_err->err_ind = 1, dm_err->emsg = "Incorrect SCP export file layout.",
        CALL cancel(1)
       ELSE
        tag_cnt = (tag_cnt+ 1), dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag_cnt = tag_cnt, stat =
        alterlist(dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag,tag_cnt),
        dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag[tag_cnt].tag_name = trim(t.line), line_cnt = 0
       ENDIF
      ENDIF
     ELSE
      end_pos = size(trim(t.line))
      IF (main_flag=1)
       dsc_tmp_scp->main = trim(t.line), main_flag = 0
      ELSE
       line_cnt = (line_cnt+ 1), dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag[tag_cnt].line_cnt =
       line_cnt, stat = alterlist(dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag[tag_cnt].line,line_cnt),
       dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag[tag_cnt].line[line_cnt].text = trim(t.line),
       beg_pos = findstring("=",t.line,1,0)
       IF (beg_pos > 0)
        dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag[tag_cnt].line[line_cnt].name = substring(1,(
         beg_pos - 1),t.line), dsc_tmp_scp->scp[dsc_tmp_scp->scp_cnt].tag[tag_cnt].line[line_cnt].
        value = substring((beg_pos+ 1),(end_pos - beg_pos),t.line)
       ELSE
        dm_err->err_ind = 1, dm_err->emsg = "Incorrect SCP export file layout.",
        CALL cancel(1)
       ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsc_fill_screen(null)
   DECLARE dfs_display_line = i4 WITH protect, noconstant(0)
   DECLARE dfs_i = i4 WITH protect, noconstant(0)
   DECLARE dfs_tag = i4 WITH protect, noconstant(0)
   DECLARE dfs_desc = i4 WITH protect, noconstant(0)
   DECLARE dfs_bot_disp_line = i4 WITH protect, noconstant(0)
   SET dfs_display_line = 12
   IF (((dsc_top_line+ 10) > dsc_max_value))
    SET dsc_bottom_line = dsc_max_value
   ELSE
    SET dsc_bottom_line = (dsc_top_line+ 9)
   ENDIF
   SET dfs_bot_disp_line = (dsc_top_line+ 9)
   FOR (dfs_i = dsc_top_line TO dfs_bot_disp_line)
    IF (dfs_i <= dsc_bottom_line)
     IF (dfs_i=dsc_cur_line)
      CALL video(r)
      SET dsc_row_nbr = dfs_display_line
     ELSE
      CALL video(n)
     ENDIF
     CALL clear(dfs_display_line,02,130)
     IF ((dsc_tmp_scp->scp[dfs_i].delete_ind=1))
      CALL text(dfs_display_line,10,"X")
     ENDIF
     CALL text(dfs_display_line,25,dsc_tmp_scp->scp[dfs_i].nbr)
     SET dfs_tag = locateval(dfs_tag,1,dsc_tmp_scp->scp[dfs_i].tag_cnt,concat("[",dsc_tmp_scp->scp[
       dfs_i].nbr,"]"),dsc_tmp_scp->scp[dfs_i].tag[dfs_tag].tag_name)
     IF (dfs_tag > 0)
      SET dfs_desc = locateval(dfs_desc,1,dsc_tmp_scp->scp[dfs_i].tag[dfs_tag].line_cnt,"description",
       dsc_tmp_scp->scp[dfs_i].tag[dfs_tag].line[dfs_desc].name)
      IF (dfs_desc > 0
       AND (dsc_tmp_scp->scp[dfs_i].tag[dfs_tag].line[dfs_desc].value > " "))
       CALL text(dfs_display_line,35,dsc_tmp_scp->scp[dfs_i].tag[dfs_tag].line[dfs_desc].value)
      ELSE
       CALL text(dfs_display_line,35,"Description Not Defined.")
      ENDIF
     ELSE
      CALL text(dfs_display_line,35,"Description Not Defined.")
     ENDIF
    ELSE
     ...
     CALL clear(dfs_display_line,02,130)
    ENDIF
    SET dfs_display_line = (dfs_display_line+ 1)
   ENDFOR
   CALL video(n)
   IF (dsc_max_value > dsc_bottom_line
    AND dsc_top_line=1)
    CALL clear(24,02,100)
    CALL text(24,2,"More SCP servers available, use <PgDown> key to scroll list")
   ELSEIF (dsc_max_value > dsc_bottom_line)
    CALL clear(24,2,100)
    CALL text(24,2,"More SCP servers available, use <PgUp>/<PgDown> keys to scroll list")
   ELSE
    CALL clear(24,2,100)
    CALL text(24,2,"Bottom of list, use <PgUp> key to scroll list")
   ENDIF
 END ;Subroutine
 SUBROUTINE dsc_export_scp(null)
   DECLARE des_scp = i2 WITH protect, noconstant(0)
   DECLARE des_tag = i2 WITH protect, noconstant(0)
   DECLARE des_line = i2 WITH protect, noconstant(0)
   SET dm_err->eproc = concat("Write SCP information into file ",ddr_scp_data->new_file)
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO value(concat(ddr_scp_data->location,ddr_scp_data->new_file))
    FROM (dummyt t  WITH seq = 1)
    DETAIL
     col 0, "[main]", row + 1,
     col 0, dsc_tmp_scp->main, row + 1,
     row + 1
     FOR (des_scp = 1 TO dsc_tmp_scp->scp_cnt)
       IF ((dsc_tmp_scp->scp[des_scp].delete_ind=0))
        FOR (des_tag = 1 TO dsc_tmp_scp->scp[des_scp].tag_cnt)
          col 0, dsc_tmp_scp->scp[des_scp].tag[des_tag].tag_name, row + 1
          FOR (des_line = 1 TO dsc_tmp_scp->scp[des_scp].tag[des_tag].line_cnt)
            col 0, dsc_tmp_scp->scp[des_scp].tag[des_tag].line[des_line].text, row + 1
          ENDFOR
          row + 1
        ENDFOR
       ENDIF
    ...
    WITH nocounter, maxrow = 1, format = variable,
     maxcol = 1000, formfeed = none
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->err_ind,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "dm2_scp_cleanup executed SUCCESSFULLY"
 CALL final_disp_msg("dm2_scp_cleanup")
END GO
