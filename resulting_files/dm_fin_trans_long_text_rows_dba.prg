CREATE PROGRAM dm_fin_trans_long_text_rows:dba
 ...
 DECLARE monthstokeep = i4 WITH protect, noconstant(- (1))
 DECLARE errormsg = c132 WITH protect, noconstant("")
 DECLARE errcode = i4 WITH protect, noconstant(0)
 DECLARE tokenidx = i4 WITH protect, noconstant(0)
 ...
 SET reply->table_name = "FIN_TRANS_LONG_TEXT"
 SET reply->rows_between_commit = minval(100,request->max_rows)
 FOR (tokenidx = 1 TO size(request->tokens,5))
   IF ((request->tokens[tokenidx].token_str="MONTHSTOKEEP"))
    SET monthstokeep = ceil(cnvtreal(request->tokens[tokenidx].value))
   ENDIF
 ENDFOR
 IF (monthstokeep < 120)
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must keep at least 10 years' worth of data.  You entered %1 months or did not enter any value.",
   "i",monthstokeep)
  GO TO exit_script
 ENDIF
 SET purgedttm = cnvtlookbehind(build2(monthstokeep,",M"))
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(ftlt.fin_trans_long_text_id)
   FROM fin_trans_long_text ftlt
   WHERE ftlt.fin_trans_long_text_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(ftlt.fin_trans_long_text_id)
  FROM fin_trans_long_text ftlt
  WHERE ftlt.fin_trans_long_text_id > 0.0
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    ...
    FROM fin_trans_long_text ftlt
    PLAN (ftlt
     WHERE ftlt.fin_trans_long_text_id BETWEEN curminid AND curmaxid
      AND parser(sbr_getrowidnotexists("ftlt.updt_dt_tm <= cnvtdatetime(purgeDtTm)","ftlt"))
      AND ftlt.fin_trans_long_text_id != 0)
    DETAIL
     rowcnt += 1
     IF (mod(rowcnt,50)=1)
      stat = alterlist(reply->rows,(rowcnt+ 49))
     ENDIF
     reply->rows[rowcnt].row_id = ftlt.rowid
    WITH nocounter, maxqual(ftlt,value(rowsleft))
   ;end select
   SET errormsg = fillstring(132," ")
   SET errcode = 0
   SET errcode = error(errormsg,1)
   IF (errcode > 0)
    SET reply->err_code = errcode
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERROR",
     "Failed in row collection: %1","s",nullterm(errormsg))
    GO TO exit_script
   ENDIF
