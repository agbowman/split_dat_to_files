CREATE PROGRAM ams_upd_dup_check
 PROMPT
 ...
 FOR (i = 1 TO value(size(file_content->qual,5)))
   ...
   SET request->upd_qual[1].dup_check_seq = file_content->qual[i].dup_check_seq
   CALL echo(build("DU SEQ = ",request->upd_qual[1].dup_check_seq))
   CALL echo(build("DU SEQ = ",file_content->qual[i].dup_check_seq))
   ...
   SELECT
    ...
    DETAIL
     request->upd_qual[1].exact_hit_action_cd = cv.code_value
    WITH nocounter
   ;end select
   SET request->upd_qual[1].min_behind = cnvtint(file_content->qual[i].behind_min)
   ...
   SELECT
    ...
    DETAIL
     request->upd_qual[1].min_behind_action_cd = cv.code_value
    WITH nocounter
   ;end select
   SET request->upd_qual[1].min_ahead = cnvtint(file_content->qual[i].ahead_min)
   ...
   SELECT
    ...
    DETAIL
     request->upd_qual[1].min_ahead_action_cd = cv.code_value
    WITH nocounter
   ;end select
   SET request->upd_qual[1].active_ind = cnvtint(file_content->qual[i].active)
   SET request->upd_qual[1].outpat_flex_ind = cnvtint(file_content->qual[i].output_flex_indicator)
   SET request->upd_qual[1].outpat_min_behind = cnvtint(file_content->qual[i].output_behind_min)
   ...
   SELECT
    ...
    DETAIL
     request->upd_qual[1].outpat_min_behind_action_cd = cv.code_value
    WITH nocounter
   ;end select
   SET request->upd_qual[1].outpat_min_ahead = cnvtint(file_content->qual[i].output_ahead_min)
   ...
   SELECT
    ...
    DETAIL
     request->upd_qual[1].outpat_min_ahead_action_cd = cv.code_value
    WITH nocounter
   ;end select
   ...
   SELECT
    ...
    DETAIL
     request->upd_qual[1].outpat_exact_hit_action_cd = cv.code_value
    WITH nocounter
   ;end select
   SET request->dup_add_cnt = 0
   SET request->dup_upd_cnt = 1
