CREATE PROGRAM dm2_aload_adoc_dmoi:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE daad_commit_cnt = i4 WITH protect, noconstant(10000)
 DECLARE daad_loop_cnt = i4 WITH protect, noconstant(0)
 DECLARE daad_req_size = i4 WITH protect, noconstant(0)
 IF ((dm2_db_options->cursor_commit_cnt != "NOT_SET"))
  SET daad_commit_cnt = cnvtint(dm2_db_options->cursor_commit_cnt)
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(requestin)
  CALL echorecord(dm2_db_options)
 ENDIF
 SET daad_req_size = size(requestin->list_0,5)
 IF (dac_pop_coldic_rec("DMA_DSOI_STAGE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Load rows into  DMA_DSOI_STAGE"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
   " dcv ,(dummyt d with seq = value(daad_req_size) ) set "),0)=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_adoc_dmoi completed"
 ELSE
  DELETE  FROM dma_dsoi_stage
   WHERE 1=1
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
 ENDIF
 CALL final_disp_msg("dm2_aload_adoc_dmoi")
END GO
