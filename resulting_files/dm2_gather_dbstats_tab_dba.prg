CREATE PROGRAM dm2_gather_dbstats_tab:dba
 ...
 SUBROUTINE dmai_get_cur_mod_act(dgcma_mod_out,dmgca_act_out)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dgdt_inowner = vc WITH protect, noconstant(" ")
 DECLARE dgdt_intable = vc WITH protect, noconstant(" ")
 DECLARE dgdt_inmode = vc WITH protect, noconstant(" ")
 DECLARE dgdt_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dgdt_step_beg_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dgdt_step_end_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dgdt_export_statid = vc WITH protect, noconstant(" ")
 DECLARE dgdt_cmd = vc WITH protect, noconstant(" ")
 DECLARE dgdt_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dgdt_emsg = vc WITH protect, noconstant(" ")
 DECLARE dgdt_lock_available_ind = i2 WITH protect, noconstant(0)
 DECLARE dgdt_lock_obtained_ind = i2 WITH protect, noconstant(0)
 DECLARE dgdt_retry_ind = i2 WITH protect, noconstant(1)
 DECLARE dgdt_retry_count = i2 WITH protect, noconstant(0)
 DECLARE dgdt_object_id = f8 WITH protect, noconstant(0.0)
 DECLARE dgdt_module_name = vc WITH protect, constant("DM_STATS_PROCESS")
 DECLARE dgdt_action_name = vc WITH protect, constant("GATHER_DBSTATS_TAB")
 DECLARE dgdt_original_module = vc WITH protect, noconstant("")
 DECLARE dgdt_original_action = vc WITH protect, noconstant("")
 DECLARE dgdt_iter = i4 WITH protect, noconstant(0)
 FREE RECORD dgdt_pending_ind_stat
 RECORD dgdt_pending_ind_stat(
   1 pending_stat_cnt = i4
   1 qual[*]
     2 owner = vc
     2 index_name = vc
     2 object_id = f8
 ...
 DECLARE gather_table_stats(ownname=vc,tabname=vc,method_opt=vc,degree=i4,force=i4,
  granularity=vc) = null WITH sql = "SYS.DBMS_STATS.GATHER_TABLE_STATS", parameter
 DECLARE unlock_table_stats(ownname=vc,tabname=vc) = null WITH sql =
 "SYS.DBMS_STATS.UNLOCK_TABLE_STATS", parameter
 IF (check_logfile("dm2_gdbs_tab",".log","dm2_gather_dbstats_tab LOGFILE")=0)
  GO TO exit_script
 ENDIF
 IF (dmai_get_cur_mod_act(dgdt_original_module,dgdt_original_action)=0)
  GO TO exit_script
 ENDIF
 CALL dmai_set_mod_act(dgdt_module_name,dgdt_action_name)
 SET dm_err->eproc = "Beginning dm2_gather_dbstats_tab"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dgdt_inowner = trim(cnvtupper( $1),3)
 SET dgdt_intable = trim( $2,3)
 SET dgdt_inmode = trim(cnvtupper( $3),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg =
  "Parameter usage: dm2_gather_dbstats_tab  '<owner>','<table_name>','<STAGE|ACTUALONLY|PREVIEW>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating <mode> input parameter."
 IF ( NOT (dgdt_inmode IN ("STAGE", "ACTUALONLY", "PENDING", "PREVIEW")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Valid values for $3 <mode> are: STAGE | ACTUALONLY | PREVIEW "
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dgdt_prefs->in_object_type = "TABLE"
 SET dgdt_prefs->in_object_owner = dgdt_inowner
 SET dgdt_prefs->in_object_name = dgdt_intable
 SET dgdt_prefs->in_table_name = dgdt_intable
 SET dgdt_prefs->in_mode = dgdt_inmode
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_add_detail_text("OWNER",dgdt_prefs->in_object_owner)
 CALL dm2_process_log_add_detail_text("TABLE",dgdt_prefs->in_table_name)
 CALL dm2_process_log_add_detail_text("TABLE_OWNER.TABLE_NAME",concat(dgdt_prefs->in_object_owner,".",
   dgdt_prefs->in_table_name))
 CALL dm2_process_log_add_detail_text("MODE",dgdt_prefs->in_mode)
 CALL dm2_process_log_add_detail_text(dpl_audsid,currdbhandle)
 CALL dm2_process_log_row(dpl_statistics,dpl_table_stats_gathering,dpl_no_prev_id,1)
 SET dgdt_log_id = dm2_process_event_rs->dm_process_event_id
 IF (ddf_get_object_id(dgdt_prefs->in_object_owner,dgdt_prefs->in_table_name,dgdt_object_id)=0)
  GO TO exit_script
 ENDIF
 WHILE (dgdt_retry_ind=1)
   IF (ddf_context_lock_maint("CHECK","STAT",2,dgdt_prefs->in_object_owner,dgdt_prefs->in_table_name,
    dgdt_object_id,dgdt_lock_available_ind)=0)
    GO TO exit_script
   ENDIF
   IF (dgdt_lock_available_ind=1)
    IF (ddf_context_lock_maint("GET","STAT",1,dgdt_prefs->in_object_owner,dgdt_prefs->in_table_name,
     dgdt_object_id,dgdt_lock_obtained_ind)=0)
     GO TO exit_script
    ENDIF
    IF (dgdt_lock_obtained_ind=1)
     SET dgdt_retry_ind = 0
    ENDIF
   ENDIF
   IF (dgdt_lock_obtained_ind=0)
    IF (dgdt_retry_count > 2)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Unable to obtain gather stats lock for Table: ",dgdt_prefs->
      in_table_name,". Table stats will be re-gathered during next cycle. This error can be ignored."
      )
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ELSE
     SET dm_err->eproc = "Waiting to obtain gather stats lock"
     CALL disp_msg(" ",dm_err->logfile,0)
     CALL pause(30)
     SET dgdt_retry_count = (dgdt_retry_count+ 1)
    ENDIF
   ENDIF
 ENDWHILE
 SELECT INTO "nl:"
  d.owner, d.index_name, o.object_id
  FROM dba_indexes d,
   dba_ind_pending_stats p,
   dba_objects o
  WHERE d.table_owner=dgdt_inowner
   AND d.table_name=dgdt_intable
   AND d.index_type IN ("NORMAL", "FUNCTION-BASED NORMAL")
   AND d.table_owner=p.table_owner
   AND d.owner=p.owner
   AND d.index_name=p.index_name
   AND d.last_analyzed = null
   AND d.index_name=o.object_name
   AND d.owner=o.owner
   AND o.object_type="INDEX"
  HEAD REPORT
   dgdt_pending_ind_stat->pending_stat_cnt = 0
  DETAIL
   dgdt_pending_ind_stat->pending_stat_cnt = (dgdt_pending_ind_stat->pending_stat_cnt+ 1), stat =
   alterlist(dgdt_pending_ind_stat->qual,dgdt_pending_ind_stat->pending_stat_cnt),
   dgdt_pending_ind_stat->qual[dgdt_pending_ind_stat->pending_stat_cnt].owner = d.owner,
   dgdt_pending_ind_stat->qual[dgdt_pending_ind_stat->pending_stat_cnt].index_name = d.index_name,
   dgdt_pending_ind_stat->qual[dgdt_pending_ind_stat->pending_stat_cnt].object_id = o.object_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 FOR (dgdt_iter = 1 TO dgdt_pending_ind_stat->pending_stat_cnt)
  EXECUTE dm2_export_dbstats_pending dgdt_prefs->in_object_owner, dgdt_prefs->in_table_name, concat(
   "DM2_INSTALL_BCK",trim(cnvtstring(dgdt_pending_ind_stat->qual[dgdt_iter].object_id)))
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDFOR
 EXECUTE dm2_set_dbstats_prefs "TABLE", dgdt_prefs->in_object_owner, dgdt_prefs->in_object_name,
 dgdt_prefs->in_mode
 IF ((((dm_err->err_ind=1)) OR ((((dgdt_prefs->autosuccess_ind=1)) OR ((dgdt_prefs->in_mode="PREVIEW"
 ))) )) )
  GO TO exit_script
 ENDIF
 CALL dm2_process_log_add_detail_text("ESTIMATE_PERCENT",dgdt_prefs->est_pct)
 CALL dm2_process_log_add_detail_number("ESTIMATE PERCENT",cnvtreal(dgdt_prefs->est_pct))
 CALL dm2_process_log_add_detail_text("METHOD_OPT",dgdt_prefs->method_opt_dped)
 CALL dm2_process_log_add_detail_text("DEGREE",dgdt_prefs->degree)
 CALL dm2_process_log_add_detail_text("CASCADE",dgdt_prefs->cascade)
 CALL dm2_process_log_add_detail_text("PUBLISH",dgdt_prefs->publish)
 CALL dm2_process_log_add_detail_text("STALE_PERCENT",dgdt_prefs->stale_pct)
 CALL dm2_process_log_add_detail_text("STALE_PERCENT",dgdt_prefs->granularity)
 IF ((dgdt_prefs->block_sample != "DM2NOTSET"))
  CALL dm2_process_log_add_detail_text("BLOCK_SAMPLE",dgdt_prefs->block_sample)
 ENDIF
 IF ((dgdt_prefs->granularity != "DM2NOTSET"))
  CALL dm2_process_log_add_detail_text("GRANULARITY",dgdt_prefs->granularity)
 ENDIF
 IF ((dgdt_prefs->no_invalidate != "DM2NOTSET"))
  CALL dm2_process_log_add_detail_text("NO_INVALIDATE",dgdt_prefs->no_invalidate)
 ENDIF
 IF (dm2_process_log_dtl_row(dgdt_log_id,0)=0)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dgdt_prefs)
 ENDIF
 IF ((dgdt_prefs->in_mode IN ("STAGE", "ACTUALONLY"))
  AND (dgdt_prefs->publish="FALSE"))
  SET dgdt_step_beg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET dm_err->eproc = build("Calling DELETE_PENDING_STATISTICS for (",dgdt_prefs->in_object_owner,".",
   dgdt_prefs->in_table_name,")")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  CALL delete_pending_stats(dgdt_prefs->in_object_owner,concat('"',dgdt_prefs->in_table_name,'"'))
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dgdt_step_end_dt_tm = cnvtdatetime(curdate,curtime3)
  CALL dm2_process_log_add_detail_number("DELETE_PENDING_STATS:SECS",datetimediff(dgdt_step_end_dttm,
    dgdt_step_beg_dttm,5))
  IF (dm2_process_log_dtl_row(dgdt_log_id,0)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = concat(" Checking for locked table stats on:",dgdt_intable)
 SELECT INTO "nl:"
  FROM dba_tab_statistics d
  WHERE d.owner=dgdt_inowner
   AND d.table_name=dgdt_intable
   AND d.stattype_locked="ALL"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSEIF (curqual > 0)
  SET dm_err->eproc = concat(" Unlocking table statistics for:",dgdt_intable)
  CALL unlock_table_stats(dgdt_inowner,concat('"',dgdt_intable,'"'))
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = build("Set resumable on for table (",dgdt_prefs->in_object_owner,".",dgdt_prefs
  ->in_table_name,")")
 SET dgdt_cmd = concat('RDB ALTER SESSION ENABLE RESUMABLE TIMEOUT 28800 NAME "',dgdt_prefs->
  in_table_name,'" GO')
 IF (dm2_push_cmd(dgdt_cmd,1)=0)
  CALL disp_msg(" ",dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 EXECUTE dm2_alter_dbstats_session "TAB"
 SET dm_err->eproc = build("Gathering table statistics for table (",dgdt_prefs->in_object_owner,".",
  dgdt_prefs->in_table_name,")")
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 CALL echo(concat("METHOD_OPT:",dgdt_prefs->method_opt))
 SET dgdt_step_beg_dttm = cnvtdatetime(curdate,curtime3)
 CALL gather_table_stats(dgdt_prefs->in_object_owner,concat('"',dgdt_prefs->in_table_name,'"'),
  dgdt_prefs->method_opt,cnvtint(dgdt_prefs->degree),cnvtbool(true),
  dgdt_prefs->granularity)
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dgdt_step_end_dttm = cnvtdatetime(curdate,curtime3)
 CALL dm2_process_log_add_detail_number("GATHER_TABLE_STATS:SECS",datetimediff(dgdt_step_end_dttm,
   dgdt_step_beg_dttm,5))
 IF (dm2_process_log_dtl_row(dgdt_log_id,0)=0)
  GO TO exit_script
 ENDIF
 IF ((dgdt_prefs->in_mode IN ("STAGE", "ACTUALONLY")))
  CASE (dgdt_prefs->in_mode)
   OF "STAGE":
    SET dgdt_export_statid = "DMSTG_T"
   OF "ACTUALONLY":
    SET dgdt_export_statid = "ACTUAL"
  ENDCASE
  EXECUTE dm2_export_dbstats_pending dgdt_prefs->in_object_owner, dgdt_prefs->in_table_name,
  dgdt_export_statid
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
  EXECUTE dm2_adjust_dbstats_tab "ALL", dgdt_prefs->in_object_owner, dgdt_prefs->in_table_name,
  dgdt_export_statid
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
...
#exit_script
 ...
 SET dgdt_err_ind = dm_err->err_ind
 SET dgdt_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_table_stats_gathering,dgdt_log_id,1)
 IF (dgdt_lock_obtained_ind=1)
  CALL ddf_context_lock_maint("RELEASE","STAT",1,dgdt_prefs->in_object_owner,dgdt_prefs->
   in_table_name,
   dgdt_object_id,dgdt_lock_available_ind)
 ENDIF
 SET dm_err->err_ind = dgdt_err_ind
 SET dm_err->emsg = dgdt_emsg
 SET dm_err->eproc = "Ending dm2_gather_dbstats_tab"
 CALL dmai_set_mod_act(dgdt_original_module,dgdt_original_action)
 CALL final_disp_msg("dm2_gdbs_tab")
END GO
