CREATE PROGRAM dm_ccb_csf_cv_extension:dba
 ...
 ENDIF
 DECLARE dccce_filterstring = vc WITH protect, noconstant(" ")
 ...
 IF (daf_is_blank(request->filterlist[1].column_value))
  SET failed = "T"
  SET reply->message = "Blank search string."
  GO TO exit_script
 ENDIF
 IF ((request->code_set < 1))
  SET failed = "T"
  SET reply->message = "Must provide code set."
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  cvs.code_set, cvs.definition, cvs.description,
  cvs.display, cnt = count(DISTINCT cve.code_value)
  ...
  PLAN (cve
   WHERE (cve.code_set=request->code_set)
    AND parser(filterstring_builder(dccce_filterstring,"cve",request)))
   JOIN (cv
   ...
   JOIN (cse
   WHERE cse.field_name=cve.field_name)
  ...
  DETAIL
   ...
   reply->code_set_list[code_set_count].display = cvs.display, reply->code_set_list[code_set_count].
   cs_qual_count = cnt
  FOOT REPORT
