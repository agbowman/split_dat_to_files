CREATE PROGRAM dm2_aload_dm_flags
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_aload_df",".log","dm2_aload_dm_flags")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dm_flags"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dadf_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dadf_file = vc WITH protect, noconstant( $2)
 DECLARE dadf_passive = i2 WITH protect, noconstant( $3)
 DECLARE dadf_expected_rows = i4 WITH protect, noconstant( $4)
 DECLARE dadf_batch = i4 WITH protect, noconstant(0)
 DECLARE dadf_file_loc_ret = vc WITH protect, noconstant("")
 DECLARE dadf_fnd_ind = i2 WITH protect, noconstant(0)
 ...
 IF (dir_get_admin_db_link(0,dadf_admin_db_link,dadf_fail_ind)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Verify CDBA.DM_FLAGS@admin1 table exists."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (parser(concat("dba_tables@",dadf_admin_db_link)) dt)
  WHERE dt.table_name="DM_FLAGS"
   AND dt.owner="CDBA"
  DETAIL
   dadf_fnd_ind = 1
  WITH nocounter
 ...
 IF (dadf_fnd_ind=0
  AND dadf_passive=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "CDBA.DM_FLAGS table/ccldef does not exist. Table is not passive."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSEIF (dadf_fnd_ind=0
  AND dadf_passive=1)
  SET dm_err->eproc = "CDBA.DM_FLAGS does not exist, but is passive. Continue."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (dac_prelim(dadf_pkg,dadf_file_loc_ret,dadf_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dadf_batch=0)
  SET dadf_batch = 5000
 ENDIF
 IF (dadf_expected_rows > 0)
  EXECUTE dm2_dbimport concat(dadf_file_loc_ret,dadf_file), "dm2_aload_adoc_df", dadf_batch
  CALL echo(concat(dadf_file_loc_ret,dadf_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dm_flags completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_dm_flags")
END GO
