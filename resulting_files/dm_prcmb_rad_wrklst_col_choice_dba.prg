CREATE PROGRAM dm_prcmb_rad_wrklst_col_choice:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "RAD_WRKLST_COL_CHOICE"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PRCMB_RAD_WRKLST_COL_CHOICE"
 ...
 SELECT INTO "nl:"
  frm.*
  FROM rad_wrklst_col_choice frm
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = frm.rad_wrklst_col_choice_id, rreclist->from_rec[
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =i4)
   UPDATE  FROM rad_wrklst_col_choice frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.active_status_dt_tm =
     ...
     cnvtdatetime(sysdate)
    WHERE frm.rad_wrklst_col_choice_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "RAD_WRKLST_COL_CHOICE"
   ...
   SET request->xxx_combine_det[icombinedet].entity_pk[1].col_name = "RAD_WRKLST_COL_CHOICE_ID"
   ...
   IF (curqual=0)
    ...
    SET request->error_message = build(
     "DEL_FROM:No values found on the rad_wrklst_col_choice table for rad_wrklst_col_choice_id=",
     s_df_pk_id)
    RETURN(0)
   ENDIF
   RETURN(1)
