CREATE PROGRAM batch_cs_to_procare:dba
 DECLARE fextracttypecd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",14135,"CHARGE SVC"))
 ...
 DECLARE log_filename = vc WITH constant("PROCARE_BATCH_CHARGE")
 ...
 IF (idebugind=1)
  ...
  SET stempstr = build2(format(d_incrementalfrom,"@SHORTDATETIME")," AND ",format(d_incrementalto,
    "@SHORTDATETIME"))
  CALL omflogprint(concat("Processing Charges updated between: ",stempstr))
  CALL omflogprint(" ")
 ENDIF
 INSERT  FROM omf_extract_batch b
  ...
  b.parent_entity_id, b.active_ind, b.process_flag)(SELECT
   seq(omf_seq,nextval), fextracttypecd, "CHARGE",
   c.charge_item_id, c.active_ind, value(c_process_inserted)
   FROM charge c
   WHERE c.updt_dt_tm BETWEEN cnvtdatetime(d_incrementalfrom) AND cnvtdatetime(d_incrementalto))
  WITH nocounter
 ;end insert
 IF (idebugind=1)
  SET iitems2process = curqual
  SET stempstr = concat("Total Charges to process: ",build(iitems2process))
 ...
 WHILE (iqual > 0)
   ...
   IF (iqual > 0)
    SELECT INTO "nl:"
     ...
     HEAD REPORT
      ichargeidx = 0, stat = initrec(omf_charge_st), stat = alterlist(omf_charge_st->data,
       c_ccl_block_large)
     DETAIL
      ichargeidx += 1, omf_charge_st->data[ichargeidx].charge_item_id = cnvtreal(b.parent_entity_id)
     FOOT REPORT
      stat = alterlist(omf_charge_st->data,ichargeidx)
     WITH nocounter
    ;end select
    IF (size(omf_charge_st->data,5) > 0)
     IF (idebugind=1)
      SET stempstr = concat("   Iteration #",build(iiterationidx)," - Processing ",build(ichargeidx),
       " charges.")
      CALL omflogprint(stempstr)
     ENDIF
     CALL echo("Entering OMF_CHARGE_SVC <include file>...")
     SET c_omf_charge_svc = "OMF_CHARGE_SVC.INC 014"
     SET v_charge_ndx = size(omf_charge_st->data,5)
     ...
     SET omf_ins_interface_error_request->contributor_system_str = "CHG SERVICES"
     ...
     IF (v_utc_on_ind=1)
      ...
      SELECT INTO "nl"
       1
       FROM (dummyt d1  WITH seq = value(v_charge_ndx)),
        encounter e,
        charge c
       PLAN (d1)
        JOIN (c
        WHERE (c.charge_item_id=omf_charge_st->data[d1.seq].charge_item_id))
        JOIN (e
        WHERE c.encntr_id=e.encntr_id)
       DETAIL
        omf_charge_st->data[d1.seq].encntr_id = e.encntr_id
       WITH nocounter
      ;end select
      SELECT DISTINCT INTO "nl:"
       encntr_id = omf_charge_st->data[d1.seq].encntr_id
       FROM (dummyt d1  WITH seq = value(v_charge_ndx))
       PLAN (d1)
       ORDER BY encntr_id
       HEAD REPORT
       ...
       DETAIL
        ...
        ENDIF
        tz_request->encntrs[ndx].encntr_id = omf_charge_st->data[d1.seq].encntr_id
       FOOT REPORT
      ...
      SELECT INTO "nl:"
       ...
       FROM (dummyt d1  WITH seq = value(size(tz_reply->encntrs,5))),
        (dummyt d2  WITH seq = value(v_charge_ndx))
       PLAN (d1)
        JOIN (d2
        WHERE (tz_reply->encntrs[d1.seq].encntr_id=omf_charge_st->data[d2.seq].encntr_id))
       DETAIL
        omf_charge_st->data[d2.seq].time_zone = tz_reply->encntrs[d1.seq].time_zone, omf_charge_st->
        data[d2.seq].time_zone_indx = tz_reply->encntrs[d1.seq].time_zone_indx, omf_charge_st->data[
      ...
      SELECT INTO "nl:"
       ...
       FROM (dummyt d1  WITH seq = value(v_charge_ndx))
       PLAN (d1
        WHERE (omf_charge_st->data[d1.seq].time_zone_status=0))
       HEAD REPORT
        ndx = 0
       DETAIL
        ndx += 1
        ...
        ENDIF
        omf_ins_interface_error_request->data[ndx].error_msg = concat("Charge_Item_ID: ",trim(
          cnvtstring(omf_charge_st->data[d1.seq].charge_item_id)),
         ".  Time zone does not exist for the encounter's ","facility.")
       FOOT REPORT
      ...
      ENDIF
      SET tz->m_id = concat(trim(omf_charge_st->data[1].time_zone),char(0))
      CALL uar_datesettimezone(tz)
     ELSE
      FOR (ndx = 1 TO v_charge_ndx)
        SET omf_charge_st->data[ndx].time_zone_status = 1
      ENDFOR
     ENDIF
     IF ((reqinfo->updt_req=951097))
      CALL echo("OMF_CHARGE_SVC - REQUEST 951097")
      FOR (ndx = 1 TO size(request->charges,5))
       SET v_charge_mod_ndx = size(request->charges[ndx].mods.charge_mods,5)
       SELECT INTO "nl:"
        cv.cdf_meaning
        FROM code_value cv,
         (dummyt d1  WITH seq = value(v_charge_mod_ndx))
        PLAN (d1
         WHERE (request->charges[ndx].mods.charge_mods[d1.seq].charge_event_mod_type_cd=
         omf_prologue_cv->13019_bill_code))
         JOIN (cv
         WHERE cv.code_set=14002
          AND (cv.code_value=request->charges[ndx].mods.charge_mods[d1.seq].field1_id))
        DETAIL
         IF (cv.cdf_meaning="CPT4")
          omf_charge_st->data[ndx].nomenclature_id = request->charges[ndx].mods.charge_mods[d1.seq].
          nomen_id, omf_charge_st->data[ndx].cpt4_id = request->charges[ndx].mods.charge_mods[d1.seq]
          .field1_id, omf_charge_st->data[ndx].cpt4_desc = request->charges[ndx].mods.charge_mods[d1
          .seq].field7,
          omf_charge_st->data[ndx].cpt4_nbr = request->charges[ndx].mods.charge_mods[d1.seq].field6
         ELSEIF (cv.cdf_meaning="ICD9")
          omf_charge_st->data[ndx].icd9_id = request->charges[ndx].mods.charge_mods[d1.seq].field1_id,
          omf_charge_st->data[ndx].icd9_desc = request->charges[ndx].mods.charge_mods[d1.seq].field7,
          omf_charge_st->data[ndx].icd9_nbr = request->charges[ndx].mods.charge_mods[d1.seq].field6
         ELSEIF (cv.cdf_meaning="CDM_SCHED")
          omf_charge_st->data[ndx].bill_code_id = request->charges[ndx].mods.charge_mods[d1.seq].
          field1_id, omf_charge_st->data[ndx].cdm_desc = request->charges[ndx].mods.charge_mods[d1
          .seq].field7, omf_charge_st->data[ndx].cdm_nbr = request->charges[ndx].mods.charge_mods[d1
          .seq].field6
         ELSEIF (cv.cdf_meaning="REVENUE")
          omf_charge_st->data[ndx].rev_code_cd = request->charges[ndx].mods.charge_mods[d1.seq].
          field3_id
         ENDIF
        WITH nocounter
       ;end select
      ...
      SELECT INTO "nl:"
       ...
       FROM (dummyt d1  WITH seq = value(v_charge_ndx))
       PLAN (d1
        WHERE (request->charges[d1.seq].charge_item_id > 0))
       DETAIL
        omf_charge_st->data[d1.seq].bill_item_id = request->charges[d1.seq].bill_item_id,
        omf_charge_st->data[d1.seq].order_id = request->charges[d1.seq].order_id, omf_charge_st->
        data[d1.seq].encntr_id = request->charges[d1.seq].encntr_id,
        omf_charge_st->data[d1.seq].activity_type_cd = request->charges[d1.seq].activity_type_cd,
        omf_charge_st->data[d1.seq].admit_type_cd = request->charges[d1.seq].admit_type_cd,
        omf_charge_st->data[d1.seq].service_dt_tm = evaluate(v_utc_on_ind,1,format(cnvtdatetimeutc(
           request->charges[d1.seq].service_dt_tm,3),"dd-mmm-yyyy hh:mm:ss;;d"),format(request->
          charges[d1.seq].service_dt_tm,"dd-mmm-yyyy hh:mm:ss;;d"))
        IF (trim(omf_charge_st->data[d1.seq].service_dt_tm)="")
         omf_charge_st->data[d1.seq].service_tz = 0, omf_charge_st->data[d1.seq].service_dt_nbr = - (
         1), omf_charge_st->data[d1.seq].service_min_nbr = - (1)
        ELSE
         omf_charge_st->data[d1.seq].service_tz = omf_charge_st->data[d1.seq].time_zone_indx,
         omf_charge_st->data[d1.seq].service_dt_nbr = cnvtdate(request->charges[d1.seq].service_dt_tm
          ), omf_charge_st->data[d1.seq].service_min_nbr = (cnvtmin(request->charges[d1.seq].
          service_dt_tm,5)+ 1)
        ENDIF
        omf_charge_st->data[d1.seq].tier_group_cd = request->charges[d1.seq].tier_group_cd,
        omf_charge_st->data[d1.seq].cost_center_cd = request->charges[d1.seq].cost_center_cd,
        omf_charge_st->data[d1.seq].ord_phys_id = request->charges[d1.seq].ord_phys_id,
        omf_charge_st->data[d1.seq].perf_phys_id = request->charges[d1.seq].perf_phys_id,
        omf_charge_st->data[d1.seq].institution_cd = request->charges[d1.seq].institution_cd,
        omf_charge_st->data[d1.seq].department_cd = request->charges[d1.seq].department_cd,
        omf_charge_st->data[d1.seq].section_cd = request->charges[d1.seq].section_cd, omf_charge_st->
        data[d1.seq].subsection_cd = request->charges[d1.seq].subsection_cd, omf_charge_st->data[d1
        .seq].item_quantity = request->charges[d1.seq].item_quantity,
        omf_charge_st->data[d1.seq].item_price = request->charges[d1.seq].item_price, omf_charge_st->
        data[d1.seq].item_extended_price = request->charges[d1.seq].item_extended_price,
        omf_charge_st->data[d1.seq].payor_id = request->charges[d1.seq].payor_id,
        omf_charge_st->data[d1.seq].discount_amount = request->charges[d1.seq].discount_amount,
        omf_charge_st->data[d1.seq].charge_dt_tm = evaluate(v_utc_on_ind,1,format(cnvtdatetimeutc(
           request->charges[d1.seq].posted_dt_tm,3),"dd-mmm-yyyy hh:mm:ss;;d"),format(request->
          charges[d1.seq].posted_dt_tm,"dd-mmm-yyyy hh:mm:ss;;d"))
        IF (trim(omf_charge_st->data[d1.seq].charge_dt_tm)="")
         omf_charge_st->data[d1.seq].charge_tz = 0, omf_charge_st->data[d1.seq].charge_dt_nbr = - (1),
         omf_charge_st->data[d1.seq].charge_min_nbr = - (1)
        ELSE
         omf_charge_st->data[d1.seq].charge_tz = omf_charge_st->data[d1.seq].time_zone_indx,
         omf_charge_st->data[d1.seq].charge_dt_nbr = cnvtdate(request->charges[d1.seq].posted_dt_tm),
         omf_charge_st->data[d1.seq].charge_min_nbr = (cnvtmin(request->charges[d1.seq].posted_dt_tm,
          5)+ 1)
        ENDIF
        omf_charge_st->data[d1.seq].updt_id = reqinfo->updt_id, omf_charge_st->data[d1.seq].
        process_flg = request->charges[d1.seq].process_flg, omf_charge_st->data[d1.seq].manual_ind =
        request->charges[d1.seq].manual_ind,
        omf_charge_st->data[d1.seq].person_id = request->charges[d1.seq].person_id
        IF ((omf_charge_st->data[d1.seq].charge_dt_nbr > 0)
         AND (omf_charge_st->data[d1.seq].service_dt_nbr > 0))
         omf_charge_st->data[d1.seq].act_service_to_charge = (datetimediff(request->charges[d1.seq].
          posted_dt_tm,request->charges[d1.seq].service_dt_tm) * 1440)
        ENDIF
        omf_charge_st->data[d1.seq].charge_description = request->charges[d1.seq].charge_description,
        omf_charge_st->data[d1.seq].charge_type_cd = request->charges[d1.seq].charge_type_cd,
        omf_charge_st->data[d1.seq].transaction_ind = 1
        IF (v_utc_on_ind=1
         AND ((d1.seq+ 1) <= v_charge_ndx))
         v_time_zone = omf_charge_st->data[(d1.seq+ 1)].time_zone, tz->m_id = concat(trim(v_time_zone
     ...
     ELSE
      SELECT INTO "nl:"
       nomen_id = cm.nomen_id, field1 = cm.field1_id, field7 = cm.field7,
       field6 = cm.field6
       FROM charge_mod cm,
        code_value cv,
        (dummyt d1  WITH seq = value(size(omf_charge_st->data,5)))
       PLAN (d1
        WHERE (omf_charge_st->data[d1.seq].time_zone_status > 0))
        JOIN (cm
        WHERE (cm.charge_item_id=omf_charge_st->data[d1.seq].charge_item_id)
         AND cm.active_ind=1
         AND cm.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime)
         AND cm.end_effective_dt_tm >= cnvtdatetime(curdate,curtime)
         AND (cm.charge_mod_type_cd=omf_prologue_cv->13019_bill_code))
        JOIN (cv
        WHERE cv.code_set=14002
         AND cv.cdf_meaning="CPT4"
         AND cv.active_ind=1
         AND cm.field1_id=cv.code_value)
       DETAIL
        omf_charge_st->data[d1.seq].nomenclature_id = nomen_id, omf_charge_st->data[d1.seq].cpt4_id
         = field1, omf_charge_st->data[d1.seq].cpt4_desc = field7,
        omf_charge_st->data[d1.seq].cpt4_nbr = field6
       WITH nocounter
      ;end select
      SELECT INTO "nl:"
       field1 = cm.field1_id, field7 = cm.field7, field6 = cm.field6
       FROM charge_mod cm,
        code_value cv,
        (dummyt d1  WITH seq = value(size(omf_charge_st->data,5)))
       PLAN (d1
        ...
        WHERE cv.code_set=14002
         ...
         AND cm.field1_id=cv.code_value)
       DETAIL
        omf_charge_st->data[d1.seq].icd9_id = field1, omf_charge_st->data[d1.seq].icd9_desc = field7,
        omf_charge_st->data[d1.seq].icd9_nbr = field6
       WITH nocounter
      ;end select
      SELECT INTO "nl:"
       field1 = cm.field1_id, field7 = cm.field7, field6 = cm.field6
       ...
       PLAN (d1
        ...
        WHERE cv.code_set=14002
         ...
         AND cm.field1_id=cv.code_value)
       DETAIL
        omf_charge_st->data[d1.seq].bill_code_id = field1, omf_charge_st->data[d1.seq].cdm_desc =
        field7, omf_charge_st->data[d1.seq].cdm_nbr = field6
       WITH nocounter
      ;end select
      SELECT INTO "nl:"
       ...
       PLAN (d1
        ...
        WHERE cv.code_set=14002
         ...
         AND cm.field1_id=cv.code_value)
       DETAIL
        omf_charge_st->data[d1.seq].rev_code_cd = field3_id
       WITH nocounter
      ;end select
      SELECT INTO "nl:"
       encntr = c.encntr_id, activity = c.activity_type_cd, admit = c.admit_type_cd,
       service = c.service_dt_tm, tier_group = c.tier_group_cd, cost_center = c.cost_center_cd,
       ord_phys = c.ord_phys_id, perf_phys = c.perf_phys_id, institution = c.institution_cd,
       department = c.department_cd, section = c.section_cd, payor = c.payor_id,
       subsection = c.subsection_cd, quantity = c.item_quantity, price = c.item_price,
       ext_price = c.item_extended_price, discount = c.discount_amount, charge = c.posted_dt_tm,
       updt_id = c.updt_id, proc_flg = c.process_flg, charge_null = nullind(c.posted_dt_tm),
       service_null = nullind(c.service_dt_tm), interface = c.beg_effective_dt_tm, manual_ind = c
       .manual_ind,
       person_id = c.person_id, c_order_id = c.order_id, chg_desc = c.charge_description,
       c_type_cd = c.charge_type_cd, c_bill_item = c.bill_item_id
       FROM charge c,
        (dummyt d1  WITH seq = value(size(omf_charge_st->data,5)))
       PLAN (d1
        WHERE (omf_charge_st->data[d1.seq].time_zone_status > 0))
        ...
        WHERE (c.charge_item_id=omf_charge_st->data[d1.seq].charge_item_id)
         AND c.active_ind=1)
       DETAIL
        omf_charge_st->data[d1.seq].bill_item_id = c_bill_item, omf_charge_st->data[d1.seq].order_id
         = c_order_id, omf_charge_st->data[d1.seq].encntr_id = encntr,
        omf_charge_st->data[d1.seq].activity_type_cd = activity, omf_charge_st->data[d1.seq].
        admit_type_cd = admit, omf_charge_st->data[d1.seq].service_dt_tm = evaluate(v_utc_on_ind,1,
         format(cnvtdatetimeutc(service,3),"dd-mmm-yyyy hh:mm:ss;;d"),format(service,
          "dd-mmm-yyyy hh:mm:ss;;d"))
        IF (trim(omf_charge_st->data[d1.seq].service_dt_tm)="")
         omf_charge_st->data[d1.seq].service_tz = 0, omf_charge_st->data[d1.seq].service_dt_nbr = - (
         1), omf_charge_st->data[d1.seq].service_min_nbr = - (1)
        ELSE
         omf_charge_st->data[d1.seq].service_tz = omf_charge_st->data[d1.seq].time_zone_indx,
         omf_charge_st->data[d1.seq].service_dt_nbr = cnvtdate(service), omf_charge_st->data[d1.seq].
         service_min_nbr = (cnvtmin(service,5)+ 1)
        ENDIF
        omf_charge_st->data[d1.seq].tier_group_cd = tier_group, omf_charge_st->data[d1.seq].
        cost_center_cd = cost_center, omf_charge_st->data[d1.seq].ord_phys_id = ord_phys,
        omf_charge_st->data[d1.seq].perf_phys_id = perf_phys, omf_charge_st->data[d1.seq].
        institution_cd = institution, omf_charge_st->data[d1.seq].department_cd = department,
        omf_charge_st->data[d1.seq].section_cd = section, omf_charge_st->data[d1.seq].subsection_cd
         = subsection, omf_charge_st->data[d1.seq].item_quantity = quantity,
        omf_charge_st->data[d1.seq].item_price = price, omf_charge_st->data[d1.seq].
        item_extended_price = ext_price, omf_charge_st->data[d1.seq].payor_id = payor,
        omf_charge_st->data[d1.seq].discount_amount = discount, omf_charge_st->data[d1.seq].
        charge_dt_tm = evaluate(v_utc_on_ind,1,format(cnvtdatetimeutc(charge,3),
          "dd-mmm-yyyy hh:mm:ss;;d"),format(charge,"dd-mmm-yyyy hh:mm:ss;;d"))
        IF (trim(omf_charge_st->data[d1.seq].charge_dt_tm)="")
         omf_charge_st->data[d1.seq].charge_tz = 0, omf_charge_st->data[d1.seq].charge_dt_nbr = - (1),
         omf_charge_st->data[d1.seq].charge_min_nbr = - (1)
        ELSE
         omf_charge_st->data[d1.seq].charge_tz = omf_charge_st->data[d1.seq].time_zone_indx,
         omf_charge_st->data[d1.seq].charge_dt_nbr = cnvtdate(charge), omf_charge_st->data[d1.seq].
         charge_min_nbr = (cnvtmin(charge,5)+ 1)
        ENDIF
        omf_charge_st->data[d1.seq].updt_id = updt_id, omf_charge_st->data[d1.seq].process_flg =
        proc_flg
        IF ((omf_charge_st->data[d1.seq].process_flg=999))
         omf_charge_st->data[d1.seq].interfaced_dt_tm = evaluate(v_utc_on_ind,1,format(
           cnvtdatetimeutc(interface,3),"dd-mmm-yyyy hh:mm:ss;;d"),format(interface,
           "dd-mmm-yyyy hh:mm:ss;;d"))
         IF (trim(omf_charge_st->data[d1.seq].interfaced_dt_tm)="")
          omf_charge_st->data[d1.seq].interfaced_tz = 0, omf_charge_st->data[d1.seq].
          interfaced_dt_nbr = - (1), omf_charge_st->data[d1.seq].interfaced_min_nbr = - (1)
         ELSE
          omf_charge_st->data[d1.seq].interfaced_tz = omf_charge_st->data[d1.seq].time_zone_indx,
          omf_charge_st->data[d1.seq].interfaced_dt_nbr = cnvtdate(interface), omf_charge_st->data[d1
          .seq].interfaced_min_nbr = (cnvtmin(interface,5)+ 1)
         ENDIF
        ENDIF
        omf_charge_st->data[d1.seq].manual_ind = manual_ind, omf_charge_st->data[d1.seq].person_id =
        person_id
        IF (charge_null=0
         AND service_null=0)
         omf_charge_st->data[d1.seq].act_service_to_charge = (datetimediff(charge,service) * 1440)
        ENDIF
        omf_charge_st->data[d1.seq].charge_description = chg_desc, omf_charge_st->data[d1.seq].
        charge_type_cd = c_type_cd, omf_charge_st->data[d1.seq].transaction_ind = 1
        IF (v_utc_on_ind=1
         AND ((d1.seq+ 1) <= v_charge_ndx))
         v_time_zone = omf_charge_st->data[(d1.seq+ 1)].time_zone, tz->m_id = concat(trim(v_time_zone
           ),char(0)), stat = uar_datesettimezone(tz)
     ...
     SET v_charge_ndx = size(omf_charge_st->data,5)
     SELECT INTO "nl:"
      o_order_id = o.order_id, order_date = o.orig_order_dt_tm, order_tz = o.orig_order_tz,
      cat_cd = o.catalog_cd
      FROM orders o,
       (dummyt d1  WITH seq = value(v_charge_ndx))
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (o
       WHERE (o.order_id=omf_charge_st->data[d1.seq].order_id)
        AND o.active_ind=1)
      DETAIL
       IF (v_utc_on_ind=1)
        IF (order_tz > 0)
         tz->m_id = datetimezonebyindex(order_tz), stat = uar_datesettimezone(tz)
        ELSE
         tz->m_id = concat(trim(omf_charge_st->data[d1.seq].time_zone),char(0)), stat =
         uar_datesettimezone(tz)
        ENDIF
       ENDIF
       omf_charge_st->data[d1.seq].order_id = o_order_id, omf_charge_st->data[d1.seq].order_dt_tm =
       evaluate(v_utc_on_ind,1,format(cnvtdatetimeutc(order_date,3),"dd-mmm-yyyy hh:mm:ss;;d"),format
        (order_date,"dd-mmm-yyyy hh:mm:ss;;d"))
       IF (trim(omf_charge_st->data[d1.seq].order_dt_tm)="")
        omf_charge_st->data[d1.seq].order_tz = 0, omf_charge_st->data[d1.seq].order_dt_nbr = - (1),
        omf_charge_st->data[d1.seq].order_min_nbr = - (1)
       ELSE
        IF (order_tz > 0)
         omf_charge_st->data[d1.seq].order_tz = order_tz
        ELSE
         omf_charge_st->data[d1.seq].order_tz = omf_charge_st->data[d1.seq].time_zone_indx
        ENDIF
        omf_charge_st->data[d1.seq].order_dt_nbr = cnvtdate(order_date), omf_charge_st->data[d1.seq].
        order_min_nbr = (cnvtmin(order_date,5)+ 1)
       ENDIF
       omf_charge_st->data[d1.seq].catalog_cd = cat_cd
       IF (v_utc_on_ind=1
        AND order_tz <= 0
        AND ((d1.seq+ 1) <= v_charge_ndx))
        v_time_zone = omf_charge_st->data[(d1.seq+ 1)].time_zone, tz->m_id = concat(trim(v_time_zone),
     ...
     SELECT INTO "nl:"
      accn = ao.accession
      FROM accession_order_r ao,
       (dummyt d1  WITH seq = value(v_charge_ndx))
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (ao
       WHERE (ao.order_id=omf_charge_st->data[d1.seq].order_id))
      DETAIL
       omf_charge_st->data[d1.seq].accession = accn
      WITH nocounter
     ;end select
     SELECT INTO "nl:"
      nurse_unit = elh.loc_nurse_unit_cd, loc_fac_cd = elh.loc_facility_cd
      FROM encntr_loc_hist elh,
       (dummyt d1  WITH seq = value(v_charge_ndx))
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (elh
       WHERE (elh.encntr_id=omf_charge_st->data[d1.seq].encntr_id)
        AND cnvtdatetime(omf_charge_st->data[d1.seq].service_dt_tm) BETWEEN elh.beg_effective_dt_tm
        AND elh.end_effective_dt_tm
        AND elh.active_ind=1)
      DETAIL
       omf_charge_st->data[d1.seq].loc_nurse_unit_cd = nurse_unit, omf_charge_st->data[d1.seq].
       loc_facility_cd = loc_fac_cd
      WITH nocounter
     ;end select
     SELECT INTO "nl:"
      task_assay = bi.ext_child_reference_id
      FROM bill_item bi,
       (dummyt d1  WITH seq = value(v_charge_ndx))
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (bi
       WHERE (bi.bill_item_id=omf_charge_st->data[d1.seq].bill_item_id)
        AND (bi.ext_parent_contributor_cd=omf_prologue_cv->13016_ord_cat)
        AND (bi.ext_parent_reference_id=omf_charge_st->data[d1.seq].catalog_cd)
        AND (bi.ext_child_contributor_cd=omf_prologue_cv->13016_task_assay)
        AND bi.active_ind=1)
      DETAIL
       omf_charge_st->data[d1.seq].task_assay_cd = task_assay
      WITH nocounter
     ;end select
     SET v_endx = 0
     SET v_cndx = 0
     SET v_cost_ndx = size(omf_std_cost->data,5)
     SELECT INTO "nl:"
      1
      FROM (dummyt d1  WITH seq = value(v_charge_ndx)),
       (dummyt d2  WITH seq = value(v_cost_ndx))
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (d2
       WHERE (omf_std_cost->data[d2.seq].bill_item_id=omf_charge_st->data[d1.seq].bill_item_id))
      DETAIL
       omf_charge_st->data[d1.seq].total_std_cost = (((omf_std_cost->data[d2.seq].std_direct_var_cost
       + omf_std_cost->data[d2.seq].std_direct_fix_cost)+ omf_std_cost->data[d2.seq].
       std_indirect_var_cost)+ omf_std_cost->data[d2.seq].std_indirect_fix_cost), omf_charge_st->
       data[d1.seq].std_direct_var_cost = omf_std_cost->data[d2.seq].std_direct_var_cost,
       omf_charge_st->data[d1.seq].std_direct_fix_cost = omf_std_cost->data[d2.seq].
       std_direct_fix_cost,
       omf_charge_st->data[d1.seq].std_indirect_var_cost = omf_std_cost->data[d2.seq].
       std_indirect_var_cost, omf_charge_st->data[d1.seq].std_indirect_fix_cost = omf_std_cost->data[
       d2.seq].std_indirect_fix_cost
      WITH nocounter
     ;end select
     ...
     SELECT INTO "nl:"
      ...
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].ord_phys_id > 0)
        AND (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (d2
       WHERE (omf_charge_st->data[d1.seq].ord_phys_id=omf_groupings->cap_phys[d2.seq].phys_id)
        AND ((cnvtdatetime(omf_charge_st->data[d1.seq].service_dt_tm) >= cnvtdatetime(omf_groupings->
        cap_phys[d2.seq].beg_effective_dt_tm)
        AND size(trim(omf_groupings->cap_phys[d2.seq].end_effective_dt_tm,3))=0) OR (cnvtdatetime(
        omf_charge_st->data[d1.seq].service_dt_tm) BETWEEN cnvtdatetime(omf_groupings->cap_phys[d2
        .seq].beg_effective_dt_tm) AND cnvtdatetime(omf_groupings->cap_phys[d2.seq].
        end_effective_dt_tm))) )
      DETAIL
       omf_charge_st->data[d1.seq].order_phys_grp_cd = omf_groupings->cap_phys[d2.seq].grp_cd
      WITH nocounter
     ;end select
     SELECT INTO "nl:"
      1
      FROM (dummyt d1  WITH seq = value(v_charge_ndx)),
       (dummyt d2  WITH seq = value(v_grp_ndx))
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].perf_phys_id > 0)
        AND (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (d2
       WHERE (omf_charge_st->data[d1.seq].perf_phys_id=omf_groupings->cap_phys[d2.seq].phys_id)
        AND ((cnvtdatetime(omf_charge_st->data[d1.seq].service_dt_tm) >= cnvtdatetime(omf_groupings->
        cap_phys[d2.seq].beg_effective_dt_tm)
        AND size(trim(omf_groupings->cap_phys[d2.seq].end_effective_dt_tm,3))=0) OR (cnvtdatetime(
        omf_charge_st->data[d1.seq].service_dt_tm) BETWEEN cnvtdatetime(omf_groupings->cap_phys[d2
      ...
      WITH nocounter
     ;end select
     SET v_grp_ndx = size(omf_groupings->cost_center,5)
     SELECT INTO "nl:"
      1
      FROM (dummyt d1  WITH seq = value(v_charge_ndx)),
       (dummyt d2  WITH seq = value(v_grp_ndx))
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].cost_center_cd > 0)
        AND (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (d2
       WHERE (omf_charge_st->data[d1.seq].cost_center_cd=omf_groupings->cost_center[d2.seq].
       cost_center_cd)
        AND ((cnvtdatetime(omf_charge_st->data[d1.seq].service_dt_tm) >= cnvtdatetime(omf_groupings->
        cost_center[d2.seq].beg_effective_dt_tm)
        AND size(trim(omf_groupings->cost_center[d2.seq].end_effective_dt_tm,3))=0) OR (cnvtdatetime(
        omf_charge_st->data[d1.seq].service_dt_tm) BETWEEN cnvtdatetime(omf_groupings->cost_center[d2
        .seq].beg_effective_dt_tm) AND cnvtdatetime(omf_groupings->cost_center[d2.seq].
        end_effective_dt_tm))) )
      DETAIL
       omf_charge_st->data[d1.seq].cost_center_grp_cd = omf_groupings->cost_center[d2.seq].grp_cd
      WITH nocounter
     ;end select
     SET v_grp_ndx = size(omf_groupings->rev_cat,5)
     SELECT INTO "nl:"
      1
      FROM (dummyt d1  WITH seq = value(v_charge_ndx)),
       (dummyt d2  WITH seq = value(v_grp_ndx))
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].rev_code_cd > 0)
        AND (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (d2
       WHERE (omf_charge_st->data[d1.seq].rev_code_cd=omf_groupings->rev_cat[d2.seq].rev_code_cd)
        AND ((cnvtdatetime(omf_charge_st->data[d1.seq].service_dt_tm) >= cnvtdatetime(omf_groupings->
        rev_cat[d2.seq].beg_effective_dt_tm)
        AND size(trim(omf_groupings->rev_cat[d2.seq].end_effective_dt_tm,3))=0) OR (cnvtdatetime(
        omf_charge_st->data[d1.seq].service_dt_tm) BETWEEN cnvtdatetime(omf_groupings->rev_cat[d2.seq
        ].beg_effective_dt_tm) AND cnvtdatetime(omf_groupings->rev_cat[d2.seq].end_effective_dt_tm)
       )) )
      DETAIL
       omf_charge_st->data[d1.seq].rev_cat_cd = omf_groupings->rev_cat[d2.seq].grp_cd
      WITH nocounter
     ;end select
     SET v_grp_ndx = size(omf_groupings->rev_cat_grp,5)
     SELECT INTO "nl:"
      1
      FROM (dummyt d1  WITH seq = value(v_charge_ndx)),
       (dummyt d2  WITH seq = value(v_grp_ndx))
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].rev_cat_cd > 0)
        AND (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (d2
       WHERE (omf_charge_st->data[d1.seq].rev_cat_cd=omf_groupings->rev_cat_grp[d2.seq].rev_cat_cd)
        AND ((cnvtdatetime(omf_charge_st->data[d1.seq].service_dt_tm) >= cnvtdatetime(omf_groupings->
        rev_cat_grp[d2.seq].beg_effective_dt_tm)
        AND size(trim(omf_groupings->rev_cat_grp[d2.seq].end_effective_dt_tm,3))=0) OR (cnvtdatetime(
        omf_charge_st->data[d1.seq].service_dt_tm) BETWEEN cnvtdatetime(omf_groupings->rev_cat_grp[d2
        .seq].beg_effective_dt_tm) AND cnvtdatetime(omf_groupings->rev_cat_grp[d2.seq].
        end_effective_dt_tm))) )
      DETAIL
       omf_charge_st->data[d1.seq].rev_cat_grp_cd = omf_groupings->rev_cat_grp[d2.seq].grp_cd
      WITH nocounter
     ;end select
     SELECT INTO "nl:"
      1
      ...
      DETAIL
       FOR (x = 1 TO 4)
         CASE (x)
          OF 1:
           v_dt_nbr = omf_charge_st->data[d1.seq].service_dt_nbr,v_date = substring(1,11,
            omf_charge_st->data[d1.seq].service_dt_tm)
          OF 2:
           v_dt_nbr = omf_charge_st->data[d1.seq].charge_dt_nbr,v_date = substring(1,11,omf_charge_st
            ->data[d1.seq].charge_dt_tm)
          OF 3:
           v_dt_nbr = omf_charge_st->data[d1.seq].interfaced_dt_nbr,v_date = substring(1,11,
            omf_charge_st->data[d1.seq].interfaced_dt_tm)
          OF 4:
           v_dt_nbr = omf_charge_st->data[d1.seq].order_dt_nbr,v_date = substring(1,11,omf_charge_st
            ->data[d1.seq].order_dt_tm)
         ENDCASE
         omf_date_ndx = 1
      ...
      WITH nocounter
     ;end select
     UPDATE  FROM omf_charge_st ocs,
       (dummyt d1  WITH seq = value(v_charge_ndx))
      SET ocs.encntr_id = omf_charge_st->data[d1.seq].encntr_id, ocs.activity_type_cd = omf_charge_st
       ->data[d1.seq].activity_type_cd, ocs.admit_type_cd = omf_charge_st->data[d1.seq].admit_type_cd,
       ocs.nomenclature_id = omf_charge_st->data[d1.seq].nomenclature_id, ocs.service_dt_tm =
       evaluate(v_utc_on_ind,1,cnvtdatetimeutc(omf_charge_st->data[d1.seq].service_dt_tm,0),
        cnvtdatetime(omf_charge_st->data[d1.seq].service_dt_tm)), ocs.service_tz = omf_charge_st->
       data[d1.seq].service_tz,
       ocs.tier_group_cd = omf_charge_st->data[d1.seq].tier_group_cd, ocs.cost_center_cd =
       omf_charge_st->data[d1.seq].cost_center_cd, ocs.cost_center_grp_cd = omf_charge_st->data[d1
       .seq].cost_center_grp_cd,
       ocs.order_phys_id = omf_charge_st->data[d1.seq].ord_phys_id, ocs.order_phys_grp_cd =
       omf_charge_st->data[d1.seq].order_phys_grp_cd, ocs.perf_phys_id = omf_charge_st->data[d1.seq].
       perf_phys_id,
       ocs.perf_phys_grp_cd = omf_charge_st->data[d1.seq].perf_phys_grp_cd, ocs.institution_cd =
       omf_charge_st->data[d1.seq].institution_cd, ocs.department_cd = omf_charge_st->data[d1.seq].
       department_cd,
       ocs.section_cd = omf_charge_st->data[d1.seq].section_cd, ocs.subsection_cd = omf_charge_st->
       data[d1.seq].subsection_cd, ocs.item_quantity = omf_charge_st->data[d1.seq].item_quantity,
       ocs.item_price = omf_charge_st->data[d1.seq].item_price, ocs.item_extended_price =
       omf_charge_st->data[d1.seq].item_extended_price, ocs.discount_amount = omf_charge_st->data[d1
       .seq].discount_amount,
       ocs.service_dt_nbr = omf_charge_st->data[d1.seq].service_dt_nbr, ocs.service_min_nbr =
       omf_charge_st->data[d1.seq].service_min_nbr, ocs.charge_dt_tm = evaluate(v_utc_on_ind,1,
        cnvtdatetimeutc(omf_charge_st->data[d1.seq].charge_dt_tm,0),cnvtdatetime(omf_charge_st->data[
         d1.seq].charge_dt_tm)),
       ocs.charge_tz = omf_charge_st->data[d1.seq].charge_tz, ocs.charge_dt_nbr = omf_charge_st->
       data[d1.seq].charge_dt_nbr, ocs.charge_min_nbr = omf_charge_st->data[d1.seq].charge_min_nbr,
       ocs.process_flg = omf_charge_st->data[d1.seq].process_flg, ocs.interfaced_dt_tm = evaluate(
        v_utc_on_ind,1,cnvtdatetimeutc(omf_charge_st->data[d1.seq].interfaced_dt_tm,0),cnvtdatetime(
         omf_charge_st->data[d1.seq].interfaced_dt_tm)), ocs.interfaced_tz = omf_charge_st->data[d1
       .seq].interfaced_tz,
       ocs.interfaced_dt_nbr = omf_charge_st->data[d1.seq].interfaced_dt_nbr, ocs.interfaced_min_nbr
        = omf_charge_st->data[d1.seq].interfaced_min_nbr, ocs.manual_ind = omf_charge_st->data[d1.seq
       ].manual_ind,
       ocs.person_id = omf_charge_st->data[d1.seq].person_id, ocs.payor_id = omf_charge_st->data[d1
       .seq].payor_id, ocs.bill_code_id = omf_charge_st->data[d1.seq].bill_code_id,
       ocs.cpt4_id = omf_charge_st->data[d1.seq].cpt4_id, ocs.icd9_id = omf_charge_st->data[d1.seq].
       icd9_id, ocs.bill_code_id = omf_charge_st->data[d1.seq].bill_code_id,
       ocs.order_id = omf_charge_st->data[d1.seq].order_id, ocs.task_assay_cd = omf_charge_st->data[
       d1.seq].task_assay_cd, ocs.order_dt_tm = evaluate(v_utc_on_ind,1,cnvtdatetimeutc(omf_charge_st
         ->data[d1.seq].order_dt_tm,0),cnvtdatetime(omf_charge_st->data[d1.seq].order_dt_tm)),
       ocs.order_tz = omf_charge_st->data[d1.seq].order_tz, ocs.order_dt_nbr = omf_charge_st->data[d1
       .seq].order_dt_nbr, ocs.order_min_nbr = omf_charge_st->data[d1.seq].order_min_nbr,
       ocs.transaction_ind = omf_charge_st->data[d1.seq].transaction_ind, ocs.catalog_cd =
       omf_charge_st->data[d1.seq].catalog_cd, ocs.act_service_to_charge = omf_charge_st->data[d1.seq
       ].act_service_to_charge,
       ocs.loc_facility_cd = omf_charge_st->data[d1.seq].loc_facility_cd, ocs.loc_nurse_unit_cd =
       omf_charge_st->data[d1.seq].loc_nurse_unit_cd, ocs.bill_item_id = omf_charge_st->data[d1.seq].
       bill_item_id,
       ocs.total_std_cost = omf_charge_st->data[d1.seq].total_std_cost, ocs.std_direct_var_cost =
       omf_charge_st->data[d1.seq].std_direct_var_cost, ocs.std_direct_fix_cost = omf_charge_st->
       data[d1.seq].std_direct_fix_cost,
       ocs.std_indirect_var_cost = omf_charge_st->data[d1.seq].std_indirect_var_cost, ocs
       .std_indirect_fix_cost = omf_charge_st->data[d1.seq].std_indirect_fix_cost, ocs
       .charge_description = omf_charge_st->data[d1.seq].charge_description,
       ocs.charge_type_cd = omf_charge_st->data[d1.seq].charge_type_cd, ocs.cdm_desc = omf_charge_st
       ->data[d1.seq].cdm_desc, ocs.cdm_nbr = omf_charge_st->data[d1.seq].cdm_nbr,
       ocs.cpt4_desc = omf_charge_st->data[d1.seq].cpt4_desc, ocs.cpt4_nbr = omf_charge_st->data[d1
       .seq].cpt4_nbr, ocs.icd9_desc = omf_charge_st->data[d1.seq].icd9_desc,
       ocs.icd9_nbr = omf_charge_st->data[d1.seq].icd9_nbr, ocs.rev_code_cd = omf_charge_st->data[d1
       .seq].rev_code_cd, ocs.rev_cat_cd = omf_charge_st->data[d1.seq].rev_cat_cd,
       ocs.rev_cat_grp_cd = omf_charge_st->data[d1.seq].rev_cat_grp_cd, ocs.process_flag = 3, ocs
       .updt_dt_tm = cnvtdatetime(sysdate),
       ocs.updt_cnt = (ocs.updt_cnt+ 1), ocs.updt_id = reqinfo->updt_id, ocs.updt_task = reqinfo->
       updt_task,
       ocs.updt_applctx = reqinfo->updt_applctx
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].charge_item_id > 0.0)
        AND (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (ocs
       WHERE (ocs.charge_item_id=omf_charge_st->data[d1.seq].charge_item_id))
      WITH status(omf_charge_st->data[d1.seq].status,omf_charge_st->data[d1.seq].errnum,omf_charge_st
       ->data[d1.seq].errmsg)
     ;end update
     INSERT  FROM omf_charge_st ocs,
       (dummyt d1  WITH seq = value(v_charge_ndx))
      SET ocs.charge_item_id = omf_charge_st->data[d1.seq].charge_item_id, ocs.encntr_id =
       omf_charge_st->data[d1.seq].encntr_id, ocs.activity_type_cd = omf_charge_st->data[d1.seq].
       activity_type_cd,
       ocs.admit_type_cd = omf_charge_st->data[d1.seq].admit_type_cd, ocs.nomenclature_id =
       omf_charge_st->data[d1.seq].nomenclature_id, ocs.service_dt_tm = evaluate(v_utc_on_ind,1,
        cnvtdatetimeutc(omf_charge_st->data[d1.seq].service_dt_tm,0),cnvtdatetime(omf_charge_st->
         data[d1.seq].service_dt_tm)),
       ocs.service_tz = omf_charge_st->data[d1.seq].service_tz, ocs.tier_group_cd = omf_charge_st->
       data[d1.seq].tier_group_cd, ocs.cost_center_cd = omf_charge_st->data[d1.seq].cost_center_cd,
       ocs.cost_center_grp_cd = omf_charge_st->data[d1.seq].cost_center_grp_cd, ocs.order_phys_id =
       omf_charge_st->data[d1.seq].ord_phys_id, ocs.order_phys_grp_cd = omf_charge_st->data[d1.seq].
       order_phys_grp_cd,
       ocs.perf_phys_id = omf_charge_st->data[d1.seq].perf_phys_id, ocs.perf_phys_grp_cd =
       omf_charge_st->data[d1.seq].perf_phys_grp_cd, ocs.institution_cd = omf_charge_st->data[d1.seq]
       .institution_cd,
       ocs.department_cd = omf_charge_st->data[d1.seq].department_cd, ocs.section_cd = omf_charge_st
       ->data[d1.seq].section_cd, ocs.subsection_cd = omf_charge_st->data[d1.seq].subsection_cd,
       ocs.item_quantity = omf_charge_st->data[d1.seq].item_quantity, ocs.item_price = omf_charge_st
       ->data[d1.seq].item_price, ocs.item_extended_price = omf_charge_st->data[d1.seq].
       item_extended_price,
       ocs.discount_amount = omf_charge_st->data[d1.seq].discount_amount, ocs.service_dt_nbr =
       omf_charge_st->data[d1.seq].service_dt_nbr, ocs.service_min_nbr = omf_charge_st->data[d1.seq].
       service_min_nbr,
       ocs.charge_dt_tm = evaluate(v_utc_on_ind,1,cnvtdatetimeutc(omf_charge_st->data[d1.seq].
         charge_dt_tm,0),cnvtdatetime(omf_charge_st->data[d1.seq].charge_dt_tm)), ocs.charge_tz =
       omf_charge_st->data[d1.seq].charge_tz, ocs.charge_dt_nbr = omf_charge_st->data[d1.seq].
       charge_dt_nbr,
       ocs.charge_min_nbr = omf_charge_st->data[d1.seq].charge_min_nbr, ocs.process_flg =
       omf_charge_st->data[d1.seq].process_flg, ocs.interfaced_dt_tm = evaluate(v_utc_on_ind,1,
        cnvtdatetimeutc(omf_charge_st->data[d1.seq].interfaced_dt_tm,0),cnvtdatetime(omf_charge_st->
         data[d1.seq].interfaced_dt_tm)),
       ocs.interfaced_tz = omf_charge_st->data[d1.seq].interfaced_tz, ocs.interfaced_dt_nbr =
       omf_charge_st->data[d1.seq].interfaced_dt_nbr, ocs.interfaced_min_nbr = omf_charge_st->data[d1
       .seq].interfaced_min_nbr,
       ocs.manual_ind = omf_charge_st->data[d1.seq].manual_ind, ocs.person_id = omf_charge_st->data[
       d1.seq].person_id, ocs.payor_id = omf_charge_st->data[d1.seq].payor_id,
       ocs.bill_code_id = omf_charge_st->data[d1.seq].bill_code_id, ocs.cpt4_id = omf_charge_st->
       data[d1.seq].cpt4_id, ocs.icd9_id = omf_charge_st->data[d1.seq].icd9_id,
       ocs.bill_code_id = omf_charge_st->data[d1.seq].bill_code_id, ocs.order_id = omf_charge_st->
       data[d1.seq].order_id, ocs.task_assay_cd = omf_charge_st->data[d1.seq].task_assay_cd,
       ocs.order_dt_tm = evaluate(v_utc_on_ind,1,cnvtdatetimeutc(omf_charge_st->data[d1.seq].
         order_dt_tm,0),cnvtdatetime(omf_charge_st->data[d1.seq].order_dt_tm)), ocs.order_tz =
       omf_charge_st->data[d1.seq].order_tz, ocs.order_dt_nbr = omf_charge_st->data[d1.seq].
       order_dt_nbr,
       ocs.order_min_nbr = omf_charge_st->data[d1.seq].order_min_nbr, ocs.transaction_ind =
       omf_charge_st->data[d1.seq].transaction_ind, ocs.catalog_cd = omf_charge_st->data[d1.seq].
       catalog_cd,
       ocs.act_service_to_charge = omf_charge_st->data[d1.seq].act_service_to_charge, ocs
       .loc_facility_cd = omf_charge_st->data[d1.seq].loc_facility_cd, ocs.loc_nurse_unit_cd =
       omf_charge_st->data[d1.seq].loc_nurse_unit_cd,
       ocs.bill_item_id = omf_charge_st->data[d1.seq].bill_item_id, ocs.total_std_cost =
       omf_charge_st->data[d1.seq].total_std_cost, ocs.std_direct_var_cost = omf_charge_st->data[d1
       .seq].std_direct_var_cost,
       ocs.std_direct_fix_cost = omf_charge_st->data[d1.seq].std_direct_fix_cost, ocs
       .std_indirect_var_cost = omf_charge_st->data[d1.seq].std_indirect_var_cost, ocs
       .std_indirect_fix_cost = omf_charge_st->data[d1.seq].std_indirect_fix_cost,
       ocs.charge_description = omf_charge_st->data[d1.seq].charge_description, ocs.charge_type_cd =
       omf_charge_st->data[d1.seq].charge_type_cd, ocs.cdm_desc = omf_charge_st->data[d1.seq].
       cdm_desc,
       ocs.cdm_nbr = omf_charge_st->data[d1.seq].cdm_nbr, ocs.cpt4_desc = omf_charge_st->data[d1.seq]
       .cpt4_desc, ocs.cpt4_nbr = omf_charge_st->data[d1.seq].cpt4_nbr,
       ocs.icd9_desc = omf_charge_st->data[d1.seq].icd9_desc, ocs.icd9_nbr = omf_charge_st->data[d1
       .seq].icd9_nbr, ocs.rev_code_cd = omf_charge_st->data[d1.seq].rev_code_cd,
       ocs.rev_cat_cd = omf_charge_st->data[d1.seq].rev_cat_cd, ocs.rev_cat_grp_cd = omf_charge_st->
       data[d1.seq].rev_cat_grp_cd, ocs.process_flag = 3,
       ocs.updt_dt_tm = cnvtdatetime(sysdate), ocs.updt_cnt = 0, ocs.updt_id = reqinfo->updt_id,
       ocs.updt_task = reqinfo->updt_task, ocs.updt_applctx = reqinfo->updt_applctx
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].status=0)
        AND (omf_charge_st->data[d1.seq].charge_item_id > 0.0)
        AND (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (ocs)
      WITH nocounter
     ;end insert
     SET v_ndx = 0
     SELECT DISTINCT INTO "nl:"
      encntr = ocs.encntr_id, item_ext_price = sum(ocs.item_extended_price), std_d_fix_cst = sum(ocs
       .std_direct_fix_cost),
      std_d_var_cst = sum(ocs.std_direct_var_cost), std_id_fix_cst = sum(ocs.std_indirect_fix_cost),
      std_id_var_cst = sum(ocs.std_indirect_var_cost)
      FROM omf_charge_st ocs,
       (dummyt d1  WITH seq = value(v_charge_ndx))
      PLAN (d1
       WHERE (omf_charge_st->data[d1.seq].time_zone_status > 0))
       JOIN (ocs
       WHERE ((ocs.encntr_id+ 0)=omf_charge_st->data[d1.seq].encntr_id)
        AND (ocs.charge_item_id=omf_charge_st->data[d1.seq].charge_item_id))
      GROUP BY ocs.encntr_id
      ORDER BY encntr
      HEAD REPORT
       ndx = 0, q_item_ext_price = 0.0, q_std_d_fix_cst = 0.0,
       q_std_d_var_cst = 0.0, q_std_id_fix_cst = 0.0, q_std_id_var_cst = 0.0
      HEAD encntr
       ndx += 1, stat = alterlist(omf_encntr_st->data,ndx), omf_encntr_st->data[ndx].encntr_id =
       encntr,
       q_item_ext_price = 0.0, q_std_d_fix_cst = 0.0, q_std_d_var_cst = 0.0,
       q_std_id_fix_cst = 0.0, q_std_id_var_cst = 0.0
      DETAIL
       q_item_ext_price += item_ext_price, q_std_d_fix_cst += std_d_fix_cst, q_std_d_var_cst +=
       std_d_var_cst,
       q_std_id_fix_cst += std_id_fix_cst, q_std_id_var_cst += std_id_var_cst
      FOOT  encntr
       omf_encntr_st->data[ndx].total_accum_charges = q_item_ext_price, omf_encntr_st->data[ndx].
       total_std_direct_fix_cost = q_std_d_fix_cst, omf_encntr_st->data[ndx].
       total_std_direct_var_cost = q_std_d_var_cst,
       omf_encntr_st->data[ndx].total_std_indirect_fix_cost = q_std_id_fix_cst, omf_encntr_st->data[
       ndx].total_std_indirect_var_cost = q_std_id_var_cst, std_cost = (((q_std_d_fix_cst+
       q_std_d_var_cst)+ q_std_id_fix_cst)+ q_std_id_var_cst),
       omf_encntr_st->data[ndx].total_std_cost = std_cost
      WITH nocounter
     ;end select
     IF (value(size(omf_encntr_st->data,5)) > 0)
      SELECT DISTINCT INTO "nl:"
       tot_accum_charges = sum(oes.total_accum_charges), tot_std_direct_fix_cost = sum(oes
        .total_std_direct_fix_cost), tot_std_direct_var_cost = sum(oes.total_std_direct_var_cost),
       tot_std_indirect_fix_cost = sum(oes.total_std_indirect_fix_cost), tot_std_indirect_var_cost =
       sum(oes.total_std_indirect_var_cost), tot_std_cost = sum(oes.total_std_cost)
       ...
       GROUP BY oes.encntr_id
       ORDER BY oes.encntr_id
       DETAIL
        omf_encntr_st->data[d1.seq].total_accum_charges += tot_accum_charges, omf_encntr_st->data[d1
        .seq].total_std_direct_fix_cost += tot_std_direct_fix_cost, omf_encntr_st->data[d1.seq].
        total_std_direct_var_cost += tot_std_direct_var_cost,
        omf_encntr_st->data[d1.seq].total_std_indirect_fix_cost += tot_std_indirect_fix_cost,
        omf_encntr_st->data[d1.seq].total_std_indirect_var_cost += tot_std_indirect_var_cost,
        omf_encntr_st->data[d1.seq].total_std_cost = (((omf_encntr_st->data[d1.seq].
        total_std_direct_fix_cost+ omf_encntr_st->data[d1.seq].total_std_direct_var_cost)+
        omf_encntr_st->data[d1.seq].total_std_indirect_fix_cost)+ omf_encntr_st->data[d1.seq].
     ...
     UPDATE  FROM omf_encntr_st oes,
      ...
      SET oes.total_accum_charges = omf_encntr_st->data[d1.seq].total_accum_charges, oes
       .total_std_cost = omf_encntr_st->data[d1.seq].total_std_cost, oes.total_std_direct_var_cost =
       omf_encntr_st->data[d1.seq].total_std_direct_var_cost,
       oes.total_std_direct_fix_cost = omf_encntr_st->data[d1.seq].total_std_direct_fix_cost, oes
       .total_std_indirect_var_cost = omf_encntr_st->data[d1.seq].total_std_indirect_var_cost, oes
       .total_std_indirect_fix_cost = omf_encntr_st->data[d1.seq].total_std_indirect_fix_cost,
       oes.updt_dt_tm = cnvtdatetime(sysdate), oes.updt_cnt = (oes.updt_cnt+ 1), oes.updt_id =
       reqinfo->updt_id,
       oes.updt_task = reqinfo->updt_task, oes.updt_applctx = reqinfo->updt_applctx
      PLAN (d1
     ...
     INSERT  FROM omf_encntr_st oes,
       (dummyt d1  WITH seq = value(v_encntr_ndx))
      SET oes.encntr_id = omf_encntr_st->data[d1.seq].encntr_id, oes.total_accum_charges =
       omf_encntr_st->data[d1.seq].total_accum_charges, oes.total_std_cost = omf_encntr_st->data[d1
       .seq].total_std_cost,
       oes.total_std_direct_var_cost = omf_encntr_st->data[d1.seq].total_std_direct_var_cost, oes
       .total_std_direct_fix_cost = omf_encntr_st->data[d1.seq].total_std_direct_fix_cost, oes
       .total_std_indirect_var_cost = omf_encntr_st->data[d1.seq].total_std_indirect_var_cost,
       oes.total_std_indirect_fix_cost = omf_encntr_st->data[d1.seq].total_std_indirect_fix_cost, oes
       .updt_dt_tm = cnvtdatetime(sysdate), oes.updt_cnt = 0,
     ...
     SET stat = alterlist(omf_charge_st->data,0)
     CALL echo("...exiting OMF_CHARGE_SVC <include file>")
     IF (size(omf_ins_interface_error_request->data,5) > 0)
      IF (idebugind=1)
     ...
     SET stat = initrec(omf_charge_st)
 ...
 ;end delete
 IF (idebugind=1)
  CALL omflogprint(concat("Procare Charge batch complete with status: ",reply->status_data.status))
