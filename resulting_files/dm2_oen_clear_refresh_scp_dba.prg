CREATE PROGRAM dm2_oen_clear_refresh_scp:dba
 ...
 SUBROUTINE dm2_findfile(sbr_file_path)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE docrs_table_column_def_exists(null) = i2
 DECLARE docrs_pathname = vc WITH protect, noconstant("cer_exe/srv_drvr")
 DECLARE docrs_paramname = vc WITH protect, noconstant("cer_exe/oen_srvcom")
 DECLARE docrs_value = vc WITH protect, noconstant("")
 DECLARE docrs_file_name = vc WITH protect, noconstant("")
 DECLARE docrs_id = vc WITH protect, noconstant("")
 DECLARE docrs_name = vc WITH protect, noconstant("")
 DECLARE docrs_user = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE docrs_domain_name = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE docrs_pwd = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE docrs_cmd = vc WITH protect, noconstant("")
 DECLARE docrs_binding = vc WITH protect, noconstant("")
 DECLARE docrs_exists = i4 WITH protect, noconstant(0)
 DECLARE docrs_val = i2 WITH protect, noconstant(0)
 DECLARE docrs_exists1 = i4 WITH protect, noconstant(0)
 DECLARE docrs_curqual = i4 WITH protect, noconstant(0)
 DECLARE docrs_oen_dom_val = vc WITH protect, noconstant("")
 IF ((validate(docrs_list->eid_cnt,- (1))=- (1))
  AND (validate(docrs_list->eid_cnt,- (2))=- (2)))
  FREE RECORD docrs_list
  RECORD docrs_list(
    1 eid_cnt = i2
    1 eid[*]
      2 scp_eid = i2
  ...
  SET docrs_list->eid_cnt = 0
 ENDIF
 IF (check_logfile("dm2_docrs",".log","dm2_oen_clear_refresh_scp LOGFILE")=0)
  GO TO exit_script
 ENDIF
 IF (validate(dm2_bypass_oen_clear_refresh_scp,- (1))=1)
  SET dm_err->eproc = "User requested to bypass the OEN Interface process."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_oen_clear_refresh_scp."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET docrs_user = trim( $1)
 SET docrs_domain_name = trim( $2)
 SET docrs_pwd = trim( $3)
 IF (cnvtupper(docrs_user) IN ("", "DM2NOTSET"))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "User name validation."
  SET dm_err->emsg = concat("User ",trim(docrs_user)," is invalid. Parameter usage: ",
   "dm2_oen_clear_refresh_scp '<USER>','<DOMAIN_NAME>','<PASSWORD>' go.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (cnvtupper(docrs_domain_name) IN ("", "DM2NOTSET"))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Domain name validation."
  SET dm_err->emsg = concat("Domain ",trim(docrs_domain_name)," is invalid.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (cnvtupper(docrs_pwd) IN ("", "DM2NOTSET"))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "User password validation."
  SET dm_err->emsg = concat("User password ",trim(docrs_pwd)," is invalid.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (checkprg("SI_DOMAIN_COPY_OEN_STEPS"))
  SET dm_err->eproc =
  "Executing SI_DOMAIN_COPY_OEN_STEPS to clear and refresh OEN Interface servers."
  CALL disp_msg(" ",dm_err->logfile,0)
  EXECUTE si_domain_copy_oen_steps docrs_user, docrs_domain_name, docrs_pwd
  SET dm_err->eproc = "Verify execution of SI_DOMAIN_COPY_OEN_STEPS script successful."
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Verify SI_DOMAIN_COPY_OEN_STEPS script execution."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM oen_personality op
   WHERE op.interfaceid=997
    AND op.name="COPY_TARGET_DOMAIN"
   DETAIL
    docrs_oen_dom_val = cnvtupper(trim(op.value,3))
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  IF (docrs_oen_dom_val != cnvtupper(trim(docrs_domain_name,3)))
   SET dm_err->err_ind = 1
   SET dm_err->emsg =
   "Script failed - Copy Target Domain row not found or does not match Target domain."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Continuing dm2_oen_clear_refresh_scp process..."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dm2_sys_misc->cur_os="AXP"))
  SET dm_err->eproc = "Exiting dm2_oen_clear_refresh_scp script not supporting on VMS."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF (docrs_table_column_def_exists(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Getting the Multi node flag value from oen_personality table."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM oen_personality op
  WHERE op.name="MULTINODE_FLAG"
   AND op.interfaceid=997
  DETAIL
   docrs_value = op.value
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET docrs_curqual = curqual
 IF (docrs_value="3")
  IF (validate(drrr_responsefile_in_use,0)=1)
   IF ((drer_email_list->email_cnt > 0))
    SET drer_email_det->msgtype = "ACTIONREQ"
    SET drer_email_det->status = "NOTIFICATION"
    SET drer_email_det->status_dt_tm = cnvtdatetime(curdate,curtime3)
    SET drer_email_det->step = "MANUAL CONFIGURATION NEEDED"
    SET drer_email_det->email_level = 1
    SET drer_email_det->logfile = dm_err->logfile
    SET drer_email_det->err_ind = dm_err->err_ind
    SET drer_email_det->eproc = dm_err->eproc
    ...
    CALL drer_add_body_text(concat(
      "User Action : Further manual configuration is needed for the FSI ",
      " servers as the number of servers across the Source application nodes exceeds automation support."
      ),0)
    IF (drer_compose_email(null)=1)
     CALL drer_send_email(drer_email_det->subject,drer_email_det->file_name,drer_email_det->
      email_level)
  ...
  ELSE
   ...
   CALL clear(1,1)
   CALL box(1,1,20,131)
   CALL text(2,2," OEN_PERSONALITY table has MULTINODE_FLAG which is set to '3'. ")
   CALL text(4,2,"It will skip the necessary cleanup for the OEN_PERSONALITY table.")
   CALL text(6,2,
    "Further manual configuration is needed for the FSI servers as the number of servers across the "
    )
   CALL text(7,2,"source application nodes exceeds automation support.")
   CALL text(10,2,"Hit 'C' to continue [C]:")
   CALL accept(10,26,"A;cu"," "
    WHERE curaccept IN ("C"))
   CALL clear(1,1)
   SET message = nowindow
  ENDIF
  SET dm_err->eproc = "Notification for skipping of oen_personality cleanup."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF (docrs_curqual != 0)
  SET dm_err->eproc = "Update the Multi node flag value to 0."
  CALL disp_msg(" ",dm_err->logfile,0)
  UPDATE  FROM oen_personality
   SET value = "0"
   WHERE name="MULTINODE_FLAG"
    AND interfaceid=997
   WITH nocounter
  ;end update
  IF (check_error(dm_err->eproc)=1)
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Update the Primary Interface/Router node values to null."
 CALL disp_msg(" ",dm_err->logfile,0)
 UPDATE  FROM oen_personality
  SET value = ""
  WHERE name IN ("PRIMARY_INTERFACE_NODE", "PRIMARY_ROUTER_NODE")
   AND interfaceid=997
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Delete additional rows of name Interface node from oen_personality."
 CALL disp_msg(" ",dm_err->logfile,0)
 DELETE  FROM oen_personality op
  WHERE op.interfaceid=997
   AND op.name="INTERFACE_NODE*"
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ENDIF
 COMMIT
 IF (drr_column_and_ccldef_exists("OEN_PROCINFO","PRIMARY_NODE",docrs_exists)=0)
  GO TO exit_script
 ENDIF
 IF (docrs_exists=1)
  SET dm_err->eproc = "Update primary_node values to null on oen_procinfo table."
  CALL disp_msg(" ",dm_err->logfile,0)
  UPDATE  FROM oen_procinfo
   SET primary_node = null
   WHERE primary_node != null
   WITH nocounter
  ;end update
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  COMMIT
 ENDIF
 IF (drr_column_and_ccldef_exists("OEN_PROCINFO","CURRENT_NODE",docrs_exists1)=0)
  GO TO exit_script
 ENDIF
 IF (docrs_exists1=1)
  SET dm_err->eproc = "Update current_node values to null on oen_procinfo table."
  CALL disp_msg(" ",dm_err->logfile,0)
  UPDATE  FROM oen_procinfo
  ...
  COMMIT
 ENDIF
 SET docrs_file_name = concat(dm2_install_schema->ccluserdir,"dm2_oen_clear_scp.ksh")
 IF (dm2_findfile(docrs_file_name) > 0)
  IF (dm2_push_dcl(concat("rm ",docrs_file_name))=0)
   GO TO exit_script
  ENDIF
 ELSE
  IF ((dm_err->err_ind=1))
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = concat("Create file to delete all existing OEN SCP interface entries: ",
  docrs_file_name,".")
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO value(docrs_file_name)
  DETAIL
   CALL print("#!/usr/bin/ksh"), row + 1,
   CALL print("#"),
   row + 1,
   CALL print("# dm2_oen_clear_scp.ksh"), row + 1,
   CALL print("#"), row + 1,
   CALL print("tgt_node=`hostname`"),
   row + 1,
   CALL print(concat("pwd='",docrs_pwd,"'")), row + 1,
   CALL print("$cer_exe/scpview $tgt_node <<!"), row + 1,
   CALL print(docrs_user),
   row + 1,
   CALL print(docrs_domain_name), row + 1,
   CALL print("$pwd"), row + 1, docrs_val = 700
   WHILE (docrs_val < 1024)
     docrs_cmd = concat("del ",build(docrs_val)),
     CALL print(docrs_cmd), row + 1,
     docrs_val = (docrs_val+ 1)
   ENDWHILE
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Execute file: ",docrs_file_name,".")
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_push_dcl(concat("chmod 777 ",docrs_file_name))=0)
  GO TO exit_script
 ENDIF
 IF (dm2_push_dcl(concat(". ",docrs_file_name))=0)
  GO TO exit_script
 ELSE
  IF (parse_errfile(dm_err->errfile)=0)
   GO TO exit_script
 ...
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dm_err)
 ENDIF
 IF (findstring("User authorization failure",dm_err->errtext,1,0) > 0)
  SET dm_err->emsg = concat(
   "Failed to delete OEN SCP interface entries: Verify the user name and password are valid.")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET docrs_file_name = concat(dm2_install_schema->ccluserdir,"dm2_oen_refresh_scp.ksh")
 IF (dm2_findfile(docrs_file_name) > 0)
  IF (dm2_push_dcl(concat("rm ",docrs_file_name))=0)
   GO TO exit_script
  ENDIF
 ELSE
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = concat("Create file to create OEN SCP interface entries: ",docrs_file_name,
  " and load all the OEN scp id's to record structure.")
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO value(docrs_file_name)
  FROM oen_procinfo p
  WHERE p.scp_eid >= 700
  ...
  HEAD REPORT
   ...
   CALL print("#"),
   row + 1,
   CALL print("# dm2_oen_refresh_scp.ksh"), row + 1,
   CALL print("#"), row + 1,
   CALL print("tgt_node=`hostname`"),
   row + 1,
   CALL print(concat("pwd='",docrs_pwd,"'")), row + 1,
  ...
  DETAIL
   docrs_list->eid_cnt = (docrs_list->eid_cnt+ 1)
   IF (mod(docrs_list->eid_cnt,10)=1)
    stat = alterlist(docrs_list->eid,(docrs_list->eid_cnt+ 9))
   ENDIF
   docrs_list->eid[docrs_list->eid_cnt].scp_eid = p.scp_eid, docrs_name = build(p.proc_name),
   docrs_list->eid[docrs_list->eid_cnt].name = docrs_name,
   docrs_id = cnvtstring(p.interfaceid,4,0,r), docrs_binding = concat("Binding=OEN.ComService.",
    docrs_id), row + 1,
   col 1, "add    ", p.scp_eid"####;L",
   row + 1, col 1, "modify ",
   p.scp_eid"####;L", " -path ", docrs_pathname,
   row + 1, col 1, "modify ",
   p.scp_eid"####;L", " -restart n -inst 0", row + 1,
   col 1, "modify ", p.scp_eid"####;L",
   " -param ", '"', docrs_paramname,
   " -args ", p.interfaceid, '"',
   row + 1, col 1, "modify ",
   p.scp_eid"####;L", " -descrip ", '"OP_',
   docrs_name, '"', row + 1,
   col 1, "modify ", p.scp_eid"####;L",
   " -prop ", '"', "Paging File Limit",
   '"=800000', row + 1, col 1,
   "modify ", p.scp_eid"####;L", " -prop ",
   docrs_binding, row + 1, col 1,
   "modify ", p.scp_eid"####;L", " -prop ",
   '"', "Cqm User Name", '"=v500',
   row + 2
  FOOT REPORT
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ELSE
  SET stat = alterlist(docrs_list->eid,docrs_list->eid_cnt)
 ENDIF
 IF ((docrs_list->eid_cnt=0))
  SET dm_err->eproc = concat("No interfaces servers are available to create in SCP.")
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Execute file: ",docrs_file_name,".")
 ...
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dm_err)
 ENDIF
 SET dm_err->eproc = concat("Verify existence of all OEN interface servers created in SCP.")
 CALL disp_msg(" ",dm_err->logfile,0)
 FOR (docrs_val = 1 TO docrs_list->eid_cnt)
   SET docrs_cmd = concat("$cer_exe/lreg -getp \\node\\",trim(curnode),"\\domain\\",docrs_domain_name,
    "\\servers\\",
    build(docrs_list->eid[docrs_val].scp_eid),"\\ ServerName")
   IF (dm2_push_dcl(docrs_cmd)=0)
    GO TO exit_script
   ENDIF
   IF (parse_errfile(dm_err->errfile)=0)
    GO TO exit_script
   ENDIF
   IF (findstring(cnvtupper(docrs_list->eid[docrs_val].name),cnvtupper(dm_err->errtext),1,0)=0)
    SET dm_err->eproc = concat("Verify existence of all OEN interface servers created in SCP.")
    SET dm_err->emsg = concat("Failed to create ",build(docrs_list->eid[docrs_val].scp_eid),
     " server.")
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
 ...
 ENDFOR
 GO TO exit_script
 SUBROUTINE docrs_table_column_def_exists(null)
   ...
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_table_and_ccldef_exists("OEN_PERSONALITY",dtcd_exists)=0)
    RETURN(0)
   ENDIF
   IF (dm2_table_and_ccldef_exists("OEN_PROCINFO",dtcd_exists1)=0)
    RETURN(0)
   ENDIF
   IF (((dtcd_exists=0) OR (dtcd_exists1=0)) )
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Determining if oen_personality/oen_procinfo exists."
    SET dm_err->emsg = "One of the following two tables and/or ccl definitions does not exist."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (drr_column_and_ccldef_exists("OEN_PERSONALITY","NAME",dtcd_exists)=0)
    RETURN(0)
   ENDIF
   IF (drr_column_and_ccldef_exists("OEN_PERSONALITY","INTERFACEID",dtcd_exists1)=0)
    RETURN(0)
   ENDIF
   IF (drr_column_and_ccldef_exists("OEN_PERSONALITY","VALUE",dtcd_exists2)=0)
    RETURN(0)
   ENDIF
   IF (((dtcd_exists=0) OR (((dtcd_exists1=0) OR (dtcd_exists2=0)) )) )
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Determine columns on oen_personality and its ccl definition exist."
    SET dm_err->emsg = "One or more columns not found or ccl definition not accurate."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (drr_column_and_ccldef_exists("OEN_PROCINFO","SCP_EID",dtcd_exists)=0)
    RETURN(0)
   ENDIF
   IF (drr_column_and_ccldef_exists("OEN_PROCINFO","PROC_NAME",dtcd_exists1)=0)
    RETURN(0)
   ENDIF
   IF (drr_column_and_ccldef_exists("OEN_PROCINFO","INTERFACEID",dtcd_exists2)=0)
    RETURN(0)
   ENDIF
   IF (((dtcd_exists=0) OR (((dtcd_exists1=0) OR (dtcd_exists2=0)) )) )
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Determine columns on oen_procinfo and its ccl definition exist."
    SET dm_err->emsg = "One or more columns not found or ccl definition not accurate."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending dm2_oen_clear_refresh_scp."
 ENDIF
 CALL final_disp_msg("dm2_docrs")
END GO
