CREATE PROGRAM acm_rdm_migrate_rvc_comment_p2:dba
 ...
 SUBROUTINE (sbr_add_stats_row(mf_readme_id=i4,ms_range_name=vc,mf_min_batch=f8(ref),mf_max_batch=f8(
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (sbr_get_max_id(ms_info_domain_nm=vc) =f8)
   CALL sbr_parallel_debug_echo("Entering sbr_get_max_id()")
   CALL sbr_parallel_debug_echo(concat("Domain Name: ",ms_info_domain_nm))
   DECLARE max_id_eval = f8 WITH protect, noconstant(0.0)
   DECLARE errmsg = vc WITH protect, noconstant("")
   SELECT INTO "nl:"
    max_val = max(di.info_number)
    FROM dm_info di
    WHERE di.info_domain=ms_info_domain_nm
     AND di.info_char="SUCCESS"
    DETAIL
     max_id_eval = cnvtreal(max_val)
    WITH nocounter
   ;end select
   IF (error(errmsg,0) != 0)
    SET readme_data->status = "F"
    SET readme_data->message = concat("Error finding last ID evaluated by children: ",errmsg)
    CALL sbr_parallel_debug_echo(concat("README_DATA->MESSAGE: ",readme_data->message))
    CALL sbr_parallel_debug_echo("sbr_get_max_id() return: -1")
    RETURN(- (1))
   ENDIF
   CALL sbr_parallel_debug_echo(concat("sbr_get_max_id() return: ",cnvtstring(max_id_eval)))
   RETURN(max_id_eval)
 END ;Subroutine
 SUBROUTINE (sbr_run_parent2(mf_max_range_id=f8,ms_range_name=vc,ms_max_name=vc,ms_info_domain_nm=vc,
  ms_child_script=vc,mn_num_children=i4,full_scan_ind=i2) =i4)
   CALL sbr_parallel_debug_echo("Entering sbr_run_parent2()")
   CALL sbr_parallel_debug_echo(concat("Maximum ID:                ",cnvtstring(mf_max_range_id)))
   CALL sbr_parallel_debug_echo(concat("Range Prefix:              ",ms_range_name))
   CALL sbr_parallel_debug_echo(concat("Max Eval Row Name:         ",ms_max_name))
   CALL sbr_parallel_debug_echo(concat("Domain Name:               ",ms_info_domain_nm))
   CALL sbr_parallel_debug_echo(concat("Child Script:              ",ms_child_script))
   CALL sbr_parallel_debug_echo(concat("Number of Children:        ",build(mn_num_children)))
   CALL sbr_parallel_debug_echo(concat("Full-table Scan Indicator: ",build(full_scan_ind)))
   DECLARE child_cnt = i4 WITH protect, noconstant(0)
   DECLARE errmsg = vc WITH protect, noconstant("")
   DECLARE info_char_row = vc WITH protect, noconstant("")
   DECLARE fetch_stat = i2 WITH protect, noconstant(0)
   IF ((validate(mf_min_range_id,- (1))=- (1)))
    DECLARE mf_min_range_id = f8 WITH protect, noconstant(1.0)
   ENDIF
   CALL sbr_parallel_debug_echo(concat("Minimum ID:                ",cnvtstring(mf_min_range_id)))
   SET child_cnt = sbr_count_children(ms_info_domain_nm)
   IF (child_cnt < 0)
    CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 2")
    RETURN(2)
   ELSEIF (child_cnt < mn_num_children)
    SET readme_data->status = "F"
    SET readme_data->message =
    "The CAT2 children for this parallel set have not all run successfully; exiting..."
    CALL sbr_parallel_debug_echo(concat("README_DATA->MESSAGE: ",readme_data->message))
    CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 2")
    RETURN(2)
   ENDIF
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain=ms_info_domain_nm
     AND di.info_name=ms_range_name
    WITH nocounter
   ;end select
   IF (error(errmsg,0) > 0)
    SET readme_data->status = "F"
    SET readme_data->message = concat("Failed select to DM_INFO to check for previous readme run: ",
     errmsg)
    CALL sbr_parallel_debug_echo(concat("README_DATA->MESSAGE: ",readme_data->message))
    CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 2")
    RETURN(2)
   ENDIF
   IF (curqual > 0)
    IF ((child_cnt >= (mn_num_children+ 1)))
     SET readme_data->status = "S"
     SET readme_data->message = "This readme has previously ran; exiting..."
     CALL sbr_parallel_debug_echo(concat("README_DATA->MESSAGE: ",readme_data->message))
     CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 0")
     RETURN(0)
    ELSE
     DELETE  FROM dm_info di
      WHERE di.info_domain=ms_info_domain_nm
       AND di.info_name IN (ms_range_name, concat(trim(ms_max_name,3)," ",cnvtstring(readme_data->
        readme_id)))
      WITH nocounter
     ;end delete
     IF (error(errmsg,0) > 0)
      ROLLBACK
      SET readme_data->status = "F"
      SET readme_data->message = concat("Failed to delete rows from DM_INFO: ",errmsg)
      CALL sbr_parallel_debug_echo(concat("README_DATA->MESSAGE: ",readme_data->message))
      CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 2")
      RETURN(2)
     ELSE
      CALL sbr_parallel_debug_echo("Committing wipe of DM_INFO rows")
      COMMIT
     ENDIF
    ENDIF
   ENDIF
   IF (full_scan_ind=0)
    CALL sbr_parallel_debug_echo("Resuming from work done by last child")
    SET mf_min_range_id = sbr_get_max_id(ms_info_domain_nm)
    IF (mf_min_range_id=0)
     SET readme_data->status = "S"
     SET readme_data->message = "Success; no rows need to be updated."
     CALL sbr_parallel_debug_echo(concat("README_DATA->MESSAGE: ",readme_data->message))
     CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 0")
     RETURN(0)
    ELSE
    ...
    ENDIF
   ENDIF
   IF (((mf_min_range_id > mf_max_range_id) OR (mf_max_range_id <= 0)) )
    SET readme_data->status = "S"
    SET readme_data->message = "No new rows to process; exiting..."
    CALL sbr_parallel_debug_echo(concat("README_DATA->MESSAGE: ",readme_data->status))
    CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 0")
    RETURN(0)
   ENDIF
   SET info_char_row = concat("min:",trim(cnvtstring(mf_min_range_id),3),":max:",trim(cnvtstring(
      mf_max_range_id),3))
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain=ms_info_domain_nm
   ...
   ;end select
   IF (curqual=0)
    CALL sbr_parallel_debug_echo("Range row does not exist")
    IF (sbr_insert_dm_info(ms_info_domain_nm,ms_range_name,info_char_row)=0)
     CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 2")
     RETURN(2)
    ENDIF
   ELSE
    CALL sbr_parallel_debug_echo("Range row exists; updating...")
    IF (sbr_update_dm_info(ms_info_domain_nm,ms_range_name,info_char_row)=0)
     CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 2")
     RETURN(2)
    ENDIF
   ENDIF
   SET fetch_stat = sbr_claim_min_max_row(ms_info_domain_nm,ms_range_name,ms_max_name)
   IF (fetch_stat IN (2, 0))
    CALL sbr_parallel_debug_echo(concat("sbr_run_parent2() return: ",build(fetch_stat)))
    RETURN(fetch_stat)
   ENDIF
   IF (sbr_run_child(ms_info_domain_nm,ms_range_name,ms_max_name,ms_child_script,mf_max_range_id,
    mf_min_range_id) IN (2, 0))
    CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 0")
    RETURN(0)
   ENDIF
   CALL sbr_parallel_debug_echo("sbr_run_parent2() return: 1")
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (sbr_p2_check_for_prev_rdm(readme_id=i4,instance=i4) =i4)
   CALL sbr_parallel_debug_echo("Entering sbr_p2_check_for_prev_rdm()")
   CALL sbr_parallel_debug_echo(concat("Readme ID:       ",build(readme_id)))
   CALL sbr_parallel_debug_echo(concat("Readme Instance: ",build(instance)))
   DECLARE errmsg = vc WITH protect, noconstant("")
   SELECT INTO "nl:"
    FROM dm_ocd_log dol
    PLAN (dol
     WHERE dol.project_type="README"
      AND dol.project_name=trim(cnvtstring(readme_id),3)
      AND dol.project_instance=instance
   ...
   IF (error(errmsg,0) != 0)
    ...
    CALL sbr_parallel_debug_echo(concat("README_DATA->MESSAGE: ",readme_data->message))
    CALL sbr_parallel_debug_echo("sbr_p2_check_for_prev_rdm() return: 2")
    RETURN(2)
   ENDIF
   IF (curqual > 0)
    SET readme_data->status = "S"
    SET readme_data->message = concat("Previous readme ",trim(cnvtstring(readme_id),3),
     " has run successfully.")
    CALL sbr_parallel_debug_echo(concat("README_DATA->MESSAGE: ",readme_data->message))
    CALL sbr_parallel_debug_echo("sbr_p2_check_for_prev_rdm() return: 1")
    RETURN(1)
   ELSE
    CALL sbr_parallel_debug_echo("sbr_p2_check_for_prev_rdm() return: 0")
    RETURN(0)
   ENDIF
 END ;Subroutine
 SUBROUTINE (validrevisioncheck(rev1=i4,rev2=i4,rev3=i4,funcname=vc) =i2)
   CALL echo(concat("Validating CCL Version for ",funcname))
   DECLARE targetrev = i4
   DECLARE systemrev = i4
   SET targetrev = (rev1 * 1000)
   SET targetrev += (rev2 * 100)
   SET targetrev += rev3
   SET systemrev = (currev * 1000)
   SET systemrev += (currevminor * 100)
 ...
 DECLARE mn_full_range_scan_ind = i2 WITH protect, noconstant(0)
 DECLARE mf_financial_number_cd = f8 WITH protect, noconstant(0.0)
 DECLARE mf_person_class_comment_cd = f8 WITH protect, noconstant(0.0)
 DECLARE mf_encntr_class_comment_cd = f8 WITH protect, noconstant(0.0)
 DECLARE mf_encntr_auth_class_comment_cd = f8 WITH protect, noconstant(0.0)
 DECLARE mf_registration_solution_cd = f8 WITH protect, noconstant(0.0)
 DECLARE mf_activity_type_comment_cd = f8 WITH protect, noconstant(0.0)
 DECLARE mf_person_applied_to_cd = f8 WITH protect, noconstant(0.0)
 DECLARE mf_encounter_applied_to_cd = f8 WITH protect, noconstant(0.0)
 DECLARE mf_auth_applied_to_cd = f8 WITH protect, noconstant(0.0)
 DECLARE ms_errmsg = vc WITH protect, noconstant("")
 DECLARE ml_update_task = i4 WITH protect, constant(- (568879))
 DECLARE sbr_update_timeline_migration_complete(null) = null
 DECLARE sbr_reset_timeline_child_parent_entity(null) = null
 DECLARE ml_rdb_array_insert_check = i2 WITH protect, noconstant(0)
 SET ml_rdb_array_insert_check = validrevisioncheck(8,10,0,"RDBARRAYINSERT functionality Check")
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
  WHERE cv.cdf_meaning="FIN NBR"
   AND cv.code_set=319
   AND cv.active_ind=1
  DETAIL
   mf_financial_number_cd = cv.code_value
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat(
   "Error selecting the CDF_MEANING from CODE_VALUE table for CODE_SET 319: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
  WHERE cv.cdf_meaning IN ("PERSON", "ENCOUNTER", "ENCNTR_AUTH")
   AND cv.code_set=4003301
   AND cv.active_ind=1
  DETAIL
   CASE (cv.cdf_meaning)
    OF "PERSON":
     mf_person_class_comment_cd = cv.code_value
    OF "ENCOUNTER":
     mf_encntr_class_comment_cd = cv.code_value
    OF "ENCNTR_AUTH":
     mf_encntr_auth_class_comment_cd = cv.code_value
   ENDCASE
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat(
   "Error selecting the CDF_MEANING from CODE_VALUE table for CODE_SET 4003301: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
  WHERE cv.cdf_meaning="REGISTRATION"
   AND cv.code_set=4484006
   AND cv.active_ind=1
  DETAIL
   mf_registration_solution_cd = cv.code_value
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat(
   "Error selecting the CDF_MEANING from CODE_VALUE table for CODE_SET 4484006: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
  WHERE cv.cdf_meaning="COMMENT"
   AND cv.code_set=4490006
   AND cv.active_ind=1
  DETAIL
   mf_activity_type_comment_cd = cv.code_value
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat(
   "Error selecting the CDF_MEANING from CODE_VALUE table for CODE_SET 4490006: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
  WHERE cv.cdf_meaning IN ("PRSN", "ENCTR", "AUTH")
   AND cv.code_set=4484007
   AND cv.active_ind=1
  DETAIL
   CASE (cv.cdf_meaning)
    OF "PRSN":
     mf_person_applied_to_cd = cv.code_value
    OF "ENCTR":
     mf_encounter_applied_to_cd = cv.code_value
    OF "AUTH":
     mf_auth_applied_to_cd = cv.code_value
   ENDCASE
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat(
   "Error selecting the CDF_MEANING from CODE_VALUE table for CODE_SET 4484007: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SET ms_info_domain_nm = "ACM_RDM_MIGRATE_RVC_COMMENT_TO_TIMELINE"
 SET ms_range = "ACM_RDM_MIGRATE_RVC_COMMENT_TO_TIMELINE CAT5 RANGE"
 SET ms_max_name_p = "MAX RVC_COMMENT_HIST_ID EVALUATED"
 SET ms_child_script = "acm_rdm_migrate_rvc_comment_cc"
 SET mn_num_children = 5
 SELECT INTO "nl:"
  max_val = max(rch.rvc_comment_hist_id)
  FROM rvc_comment_hist rch
  DETAIL
   mf_max_range_id = cnvtreal(max_val)
  WITH nocounter
 ;end select
 SET mn_full_range_scan_ind = 0
 SET return_val = sbr_run_parent2(mf_max_range_id,ms_range,ms_max_name_p,ms_info_domain_nm,
  ms_child_script,
  mn_num_children,mn_full_range_scan_ind)
 IF (((return_val=2) OR ((readme_data->status="F"))) )
  GO TO exit_program
 ELSE
  CALL sbr_reset_timeline_child_parent_entity(null)
  CALL sbr_update_timeline_migration_complete(null)
  SET readme_data->status = "S"
  SET readme_data->message = "Readme completed successfully"
 ENDIF
 SUBROUTINE sbr_update_timeline_migration_complete(null)
   CALL echo("==>Updating rc_timeline_migration_level table...")
   UPDATE  FROM rc_timeline_migration_level rc
    SET rc.complete_ind = 1, rc.migration_level_nbr = 1, rc.updt_applctx = reqinfo->updt_applctx,
     rc.updt_dt_tm = cnvtdatetime(sysdate), rc.updt_cnt = (rc.updt_cnt+ 1), rc.updt_id = reqinfo->
     updt_id,
     rc.updt_task = ml_update_task
    WHERE rc.activity_type_cd=mf_activity_type_comment_cd
     AND rc.solution_cd=mf_registration_solution_cd
    WITH nocounter
   ;end update
   IF (curqual=0)
    INSERT  FROM rc_timeline_migration_level rc
     SET rc.rc_timeline_migration_level_id = seq(rc_timeline_seq,nextval), rc.complete_ind = 1, rc
      .migration_level_nbr = 1,
      rc.created_dt_tm = cnvtdatetime(sysdate), rc.activity_type_cd = mf_activity_type_comment_cd, rc
      .solution_cd = mf_registration_solution_cd,
      rc.updt_applctx = reqinfo->updt_applctx, rc.updt_dt_tm = cnvtdatetime(sysdate), rc.updt_cnt = 0,
      rc.updt_id = reqinfo->updt_id, rc.updt_task = ml_update_task
     WITH nocounter
    ;end insert
   ENDIF
   IF (error(ms_errmsg,0) != 0)
    SET readme_data->status = "F"
    SET readme_data->message = concat("Error updating the rc_timeline_migration_level table ",
     ms_errmsg)
    GO TO exit_program
   ENDIF
   RETURN(null)
 END ;Subroutine
 SUBROUTINE sbr_reset_timeline_child_parent_entity(null)
   DECLARE ms_dm_info_name = vc WITH protect, constant("T_MIN_RC_TIMELINE_ID")
   DECLARE ms_dm_info_domain = vc WITH protect, constant("ACM_RDM_MIGRATE_RVC_COMMENT_TIMELINE_ID")
   DECLARE mf_max_timeline_id = f8 WITH protect, noconstant(0.0)
   DECLARE mf_min_timeline_id = f8 WITH protect, noconstant(0.0)
   DECLARE mf_range_inc_id = f8 WITH protect, constant(250000.0)
   DECLARE mf_max_range_id = f8 WITH protect, noconstant(0.0)
   SELECT INTO "nl:"
    max_val = max(r.rc_timeline_id)
    FROM rc_timeline r
    DETAIL
     mf_max_timeline_id = cnvtreal(max_val)
    WITH nocounter
   ;end select
   IF (error(ms_errmsg,0) != 0)
    SET readme_data->status = "F"
    SET readme_data->message = concat("Error selecting the max ID on rc_timeline: ",ms_errmsg)
    GO TO exit_program
   ENDIF
   SELECT INTO "nl:"
    FROM dm_info di
    PLAN (di
     WHERE di.info_name=ms_dm_info_name
      AND di.info_domain=ms_dm_info_domain)
    DETAIL
     mf_min_timeline_id = di.info_long_id
    WITH nocounter
   ;end select
   IF (error(ms_errmsg,0) != 0)
    SET readme_data->status = "F"
    SET readme_data->message = concat("Error selecting the min ID on dm_info: ",ms_errmsg)
    GO TO exit_program
   ENDIF
   SET mf_max_range_id = (mf_min_timeline_id+ mf_range_inc_id)
   IF (mf_max_range_id > mf_max_timeline_id)
    SET mf_max_range_id = mf_max_timeline_id
   ENDIF
   WHILE (mf_min_timeline_id < mf_max_timeline_id)
     UPDATE  FROM rc_timeline r
      SET r.child_parent_entity_id = 0.0
      WHERE r.rc_timeline_id BETWEEN mf_min_timeline_id AND mf_max_range_id
       AND r.solution_cd=mf_registration_solution_cd
       AND r.activity_type_cd=mf_activity_type_comment_cd
       AND r.child_parent_entity_id > 0.0
      WITH nocounter
     ;end update
     IF (error(ms_errmsg,0) != 0)
      SET readme_data->status = "F"
      SET readme_data->message = concat("Error updating the child parent ID on rc_timeline: ",
       ms_errmsg)
      GO TO exit_program
     ENDIF
     SET mf_min_timeline_id = (mf_max_range_id+ 1)
     SET mf_max_range_id += mf_range_inc_id
     IF (mf_max_range_id > mf_max_timeline_id)
      SET mf_max_range_id = mf_max_timeline_id
     ENDIF
   ENDWHILE
   ...
   IF (error(ms_errmsg,0) != 0)
    SET readme_data->status = "F"
    SET readme_data->message = concat("Error deleting the min ID on dm_info: ",ms_errmsg)
    GO TO exit_program
   ENDIF
   RETURN(null)
 END ;Subroutine
#exit_program
 ...
 ELSE
  COMMIT
  CALL sbr_add_dropped_object("INDEX","XIE2TEMPACMRDMPROCESS","Dropping non-unique index")
  CALL sbr_add_dropped_object("INDEX","XIETEMPACMRDMPROCESS","Dropping unique index")
  CALL sbr_add_dropped_object("TABLE","TEMP_RDM_ACM_MIGRATE_COMMENT","Dropping table")
  CALL sbr_drop_sql_objects(null)
 ENDIF
 FREE RECORD parallel_drop
