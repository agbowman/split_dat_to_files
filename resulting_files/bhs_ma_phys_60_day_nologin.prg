CREATE PROGRAM bhs_ma_phys_60_day_nologin
 PROMPT
 ...
 SET date_qual = cnvtlookbehind("60,D",cnvtdatetime(curdate,curtime3))
 DECLARE ref_phys = f8
 ...
 SELECT DISTINCT INTO value(output_dest)
  ...
  PLAN (pr
   WHERE pr.active_ind=1
    ...
    AND pr.updt_dt_tm < cnvtdatetime((curdate - 60),0)
   ...
   (SELECT
    ...
    WHERE oai.person_id=pr.person_id
     AND oai.start_day > cnvtdatetime((curdate - 60),000)))))
   JOIN (oa
 ...
 IF (email_ind=1)
  ...
  SET subject_line = concat(curprog," V1.x - Baystate Health CIS PHYS Acnts Inactive 60 days")
