CREATE PROGRAM dm2_connect_to_dbase:dba
 ...
 SUBROUTINE dctx_restore_prev_contexts(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dctd_cnnct_str = vc WITH protect, noconstant(" ")
 DECLARE dctd_option = vc WITH protect, noconstant(" ")
 IF ((validate(dm2_force_connect_string,- (1))=- (1)))
  DECLARE dm2_force_connect_string = i2 WITH public, noconstant(0)
 ENDIF
 DECLARE prompt_prep_work(null) = null
 DECLARE prompt_for_user_info(null) = null
 DECLARE prompt_for_user_info_spec(null) = null
 DECLARE connect_to_dbase(null) = i2
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning connection to database..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dctd_option = cnvtupper( $1)
 IF ( NOT (dctd_option IN ("PO", "CO", "PC", "SC", "DO")))
  SET message = nowindow
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Process option for database connection is invalid.")
  SET dm_err->eproc = "Database Connection Process Option Validation"
  SET dm_err->user_action = concat("Please specify a valid database connection process option. ")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 CASE (dctd_option)
  OF "CO":
   IF ((dm_err->err_ind=0))
    SET dm_err->eproc = concat("Connecting to ",dm2_install_schema->dbase_name,
     " database for user name ",dm2_install_schema->u_name,".")
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF ((dm_err->err_ind=1))
    CALL connect_to_dbase(null)
   ELSE
    IF (connect_to_dbase(null)=0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ENDIF
   ENDIF
  OF "PC":
   CALL prompt_prep_work(null)
   SET done = 0
   WHILE (done=0)
     CALL prompt_for_user_info(null)
     IF ((dm_err->err_ind=1))
      GO TO exit_program
     ENDIF
     IF (connect_to_dbase(null)=0)
  ...
  OF "SC":
   ...
   SET done = 0
   WHILE (done=0)
     CALL prompt_for_user_info_spec(null)
     IF ((dm_err->err_ind=1))
      GO TO exit_program
     ENDIF
     IF (connect_to_dbase(null)=0)
   ...
   ENDWHILE
  OF "DO":
   SET dm_err->eproc = concat("Disconnecting from the ",dm2_install_schema->dbase_name,
    " database for user name ",dm2_install_schema->u_name,".")
   IF ((dm_err->err_ind=0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
 ...
 ENDCASE
 GO TO exit_program
 SUBROUTINE prompt_prep_work(null)
   SET message = window
   SET width = 132
   CALL clear(1,1)
   IF (dctd_option="SC")
   ...
   CALL video(n)
 END ;Subroutine
 SUBROUTINE prompt_for_user_info(null)
   ...
   IF (currdb IN ("ORACLE", "SQLSRV"))
    CALL text(9,1,"Please enter the database connect string: ")
   ELSE
    SET dm2_install_schema->connect_str = dm2_install_schema->dbase_name
   ENDIF
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE prompt_for_user_info_spec(null)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE connect_to_dbase(null)
   DECLARE ctd_cclversion = i4 WITH protect, noconstant(0)
   SET message = nowindow
   ...
   IF (currdb="ORACLE")
    IF ((dm2_install_schema->connect_str > " "))
     SET dctd_cnnct_str = build(dm2_install_schema->u_name,"/",dm2_install_schema->p_word,"@",
   ...
   ELSE
    SET dctd_cnnct_str = build(dm2_install_schema->u_name,"/",dm2_install_schema->p_word,"/",
     dm2_install_schema->connect_str)
   ENDIF
   IF ((validate(dm2_bypass_sys_connect_w_sysdba,- (1))=- (1)))
    SET ctd_cclversion = (((cnvtint(currev) * 1000000)+ (cnvtint(currevminor) * 1000))+ cnvtint(
     currevminor2))
   ENDIF
   IF (currdb="ORACLE")
    IF (ctd_cclversion >= 8012003
     AND (dm2_install_schema->u_name="SYS"))
     CALL parser(concat("define oraclesystem '",trim(dctd_cnnct_str),"' with sysdba go"),1)
    ELSE
     CALL parser(concat("define oraclesystem '",trim(dctd_cnnct_str),"' go"),1)
    ENDIF
   ELSE
    CALL parser(concat("define sqlsystem '",trim(dctd_cnnct_str),"' go"),1)
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    RETURN(0)
   ELSEIF (currdb="ORACLE")
    ...
    ENDIF
    IF ((dm_err->debug_flag > 1))
     CALL echo("After restoring context attributes in dm2_connect_to_dbase:")
     SELECT INTO noforms
