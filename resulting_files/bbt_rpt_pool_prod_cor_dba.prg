CREATE PROGRAM bbt_rpt_pool_prod_cor:dba
 RECORD pool_comp(
   1 cmp_list[*]
     2 product_id = f8
     2 product_nbr = c20
 ...
 SET captions->component_products = uar_i18ngetmessage(i18nhandle,"component_products",
  "COMPONENT PRODUCTS:")
 SET captions->in_pool = uar_i18ngetmessage(i18nhandle,"in_pool","In Pool")
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  cp.product_id, d_flag = decode(cp_p.seq,"CP_P    ",cp_rel_p.seq,"CP_REL_P","XXXXXXXX"), cp_abo_disp
   = uar_get_code_display(cp.abo_cd),
  ...
  uar_get_code_display(cp.disease_cd), cp.updt_id,
  prs.username, pr.product_nbr, pr.product_sub_nbr,
  pr_product_disp = uar_get_code_display(pr.product_cd), pr_cur_unit_meas = uar_get_code_display(pr
   .cur_unit_meas_cd), pr_donation_type_disp = uar_get_code_display(pr.donation_type_cd),
  pr_disease_disp = uar_get_code_display(pr.disease_cd), bp.supplier_prefix, bp.cur_volume,
  bp_abo_disp = uar_get_code_display(bp.cur_abo_cd), bp_rh_disp = uar_get_code_display(bp.cur_rh_cd),
  cp_p.product_id,
  cp_p.product_nbr, cp_p.product_sub_nbr, cp_p.pooled_product_id,
  cp_p_product_disp = uar_get_code_display(cp_p.product_cd), cp_p_bp_abo_disp = uar_get_code_display(
   cp_p_bp.cur_abo_cd), cp_p_bp_rh_disp = uar_get_code_display(cp_p_bp.cur_rh_cd),
  cp_p_bp.supplier_prefix, cp_rel.correction_flag, cp_rel_p.product_id,
  cp_rel_p.product_nbr, cp_rel_p.product_sub_nbr, cp_rel_p_product_disp = uar_get_code_display(
   cp_rel_p.product_cd),
  cp_rel_bp_abo_disp = uar_get_code_display(cp_rel_bp.cur_abo_cd), cp_rel_bp_rh_disp =
  uar_get_code_display(cp_rel_bp.cur_rh_cd), cp_rel_bp.supplier_prefix,
  cp_pool_nbr_sort = decode(cp_p.seq,concat(trim(cp_p_bp.supplier_prefix),trim(cp_p.product_nbr)," ",
    trim(cp_p.product_sub_nbr)),cp_rel.seq,concat(trim(cp_rel_bp.supplier_prefix),trim(cp_rel_p
     .product_nbr)," ",trim(cp_rel_p.product_sub_nbr)),"XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
  ...
  PLAN (d1)
   ...
   WHERE cp.updt_dt_tm BETWEEN cnvtdatetime(request->beg_dt_tm) AND cnvtdatetime(request->end_dt_tm)
    AND cp.correction_type_cd=chg_pool_cd
   ...
   JOIN (bp
   WHERE bp.product_cd=pr.product_cd
    AND bp.product_id=pr.product_id)
   JOIN (((d_cp
   WHERE d_cp.seq=1)
   JOIN (cp_p
   WHERE cp_p.pooled_product_id=cp.product_id)
   JOIN (cp_p_bp
   WHERE cp_p_bp.product_cd=cp_p.product_cd
    AND cp_p_bp.product_id=cp_p.product_id)
   ) ORJOIN ((d_rel
   WHERE d_rel.seq=1)
   JOIN (cp_rel
   ...
   JOIN (cp_rel_bp
   WHERE cp_rel_bp.product_cd=cp_rel_p.product_cd
    AND cp_rel_bp.product_id=cp_rel_p.product_id)
   ))
  ...
  HEAD cp.correction_id
   IF (cp.correction_id > 0.0)
    datafoundflag = true, cmp_cnt = 0, stat = alterlist(pool_comp->cmp_list,10)
    IF (row > 55)
     BREAK
    ENDIF
    IF (page_break != "Y")
    ...
    IF (trim(vol) > trim(" "))
     col 27, vol
    ENDIF
    vol = trim(cnvtstring(cp.volume,4,0,r))
    IF (trim(vol) > trim(" "))
     col 57, vol
    ENDIF
    row + 1
    IF (row > 56)
     BREAK
    ENDIF
    cur_unit_meas = trim(pr_cur_unit_meas), prev_unit_meas = trim(cp_unit_meas), col 1,
    captions->unit_of_measure, col 27, cur_unit_meas"##########",
    col 57, prev_unit_meas"##########", row + 1
    IF (row > 56)
     BREAK
    ENDIF
  ...
  FOOT  cp.correction_id
   IF (cp.correction_id > 0.0)
    ...
    col 50, captions->in_pool, col 58,
...
,
  ...
  WITH nocounter, maxrow = 63, outerjoin(d1),
   outerjoin(d_cp), outerjoin(d_rel), dontcare(cp_p),
