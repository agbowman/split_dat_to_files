CREATE PROGRAM dm_stat_export_solcap:dba
 ...
 DECLARE export_program_name = vc WITH constant( $1)
 DECLARE export_program_number = i4 WITH constant( $2)
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 SET filename = build("msa_",cnvtlower(export_request->node),"_",export_program_number,"s_",
  export_dt,"_",export_tm,".xml")
 CALL createfile(cnvtupper(export_program_name),filename)
 IF (file_flag > 0)
  INSERT  FROM dm_stat_resend_retry drr
   SET drr.dm_stat_resend_retry_id = seq(dm_clinical_seq,nextval), drr.file_name = cnvtupper(filename
    ...
    drr.ccts_resend_retry_cnt = - (1), drr.batch_size_nbr = export_program_number, drr
    .batch_index_nbr = export_program_number,
  ...
  ENDIF
 ENDIF
 SUBROUTINE createfile(programname,fname)
   SET file_flag = 0
   SET whereclause = build("d.snapshot_type = 'SOLCAP||",programname,"'")
   SELECT INTO "nl:"
    ret_cnt = count(1)
    ...
    SELECT INTO value(fname)
     ...
     HEAD REPORT
      MACRO (convert_special_chars)
       ...
       dse_temp_vc_in = replace(dse_temp_vc_in,'"',"&quot;",0), dse_temp_vc_out = replace(
        dse_temp_vc_in,"'","&apos;",0)
      ENDMACRO
