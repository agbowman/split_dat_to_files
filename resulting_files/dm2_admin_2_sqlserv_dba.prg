CREATE PROGRAM dm2_admin_2_sqlserv:dba
 DECLARE ace_str = vc WITH protect, noconstant("")
 DECLARE data_type_loop_int = i2 WITH protect, noconstant(0)
 DECLARE display_message_str = vc WITH protect, noconstant("")
 DECLARE update_counter_int = i4 WITH protect, noconstant(0)
 DECLARE counter_string = vc WITH protect, noconstant("")
 DECLARE tab_name_str = vc WITH protect, noconstant("")
 DECLARE col_name_str = vc WITH protect, noconstant("")
 DECLARE alias_name_str = vc WITH protect, noconstant("")
 DECLARE error_check_counter_int = i4 WITH protect, noconstant(0)
 DECLARE go_stmt_flag_ind = i4 WITH protect, noconstant(0)
 DECLARE update_error_check_ind = i2 WITH protect, noconstant(0)
 DECLARE date_record_int = i4 WITH protect, noconstant(0)
 DECLARE data_dflt_str = vc WITH protect, noconstant("")
 DECLARE data_type_str = vc WITH protect, noconstant("")
 DECLARE short_string = vc WITH protect, noconstant("")
 DECLARE omittance_flg = i2 WITH protect, noconstant(0)
 DECLARE exclusion_str = vc WITH protect, noconstant("")
 DECLARE size_array = i4 WITH protect, noconstant(0)
 DECLARE file_name = vc WITH protect, noconstant("")
 DECLARE dm2_nocommit_ind = i2 WITH protect, noconstant(0)
 DECLARE more_updates_ind = i2 WITH protect, noconstant(0)
 DECLARE convert_data_defaults(data_type=vc,data_default=vc,source_rdbms=vc) = vc
 RECORD daod_data_types(
   1 table_names[*]
     2 name = vc
 )
 RECORD daod_update_stmt(
   1 stmt[*]
     2 frags[10]
       3 stmt_frag = vc
 )
 RECORD daod_tables_changed(
   1 table_names[*]
     2 name = vc
     2 column = vc
     2 alias = vc
 )
 RECORD daod_exempt_stmts(
   1 stmt[*]
     2 frags[4]
       3 stmt_frag = vc
 )
 RECORD daod_substring_tables(
   1 table_names[*]
     2 name = vc
     2 column = vc
     2 alias = vc
 )
 RECORD daod_date_update(
   1 data[*]
     2 default = vc
     2 type = vc
     2 cnvt = vc
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_admin_2_sqlserv",".log","dm2_admin_2_sqlserv")=0)
  GO TO exit_program
 ENDIF
 CALL disp_msg("Starting dm2_admin_2_sqlserv",dm_err->logfile,0)
 IF (validate(dm2_nocommit_flag,- (1)) > 0)
  SET dm2_nocommit_ind = dm2_nocommit_flag
 ELSE
  SET dm2_nocommit_ind = 0
 ENDIF
 CALL echo(build("dm2_nocommit_ind:",dm2_nocommit_ind))
 IF (dm2_nocommit_ind=1)
  CALL echo("******                     IN TEST MODE                           ******")
  CALL echo("******               NOT COMMITING TRANSACTIONS.                  ******")
  CALL echo("****** WHEN SCRIPT IS FINISHED EXECUTING AND DATA HAS BEEN        ******")
  CALL echo("****** CHECKED ISSUE A ROLLBACK SO THAT DATA WILL NOT BE UPDATED. ******")
 ENDIF
 IF (currdb IN ("ORACLE", "DB2UDB"))
  SET stat = 1
 ELSE
  CALL disp_msg(concat("The current database is:",trim(currdb)),"dm2_admin_2_sqlserv",1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Storing Data Default values into a record structure"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "NL:"
  dm.data_default
  FROM dm_afd_columns dm
  WHERE dm.data_type IN ("DATE", "TIMESTAMP")
   AND ((dm.data_default="SYSDATE") OR (((dm.data_default="TO_DATE*") OR (((dm.data_default=
  '"SYSIBM"*') OR (dm.data_default="CURRENT_TIMESTAMP")) )) ))
  HEAD REPORT
   stat = alterlist(daod_date_update->data,100), date_record_int = 0
  DETAIL
   date_record_int = (date_record_int+ 1)
   IF (mod(date_record_int,100)=1
    AND date_record_int != 1)
    stat = alterlist(daod_date_update->data,(date_record_int+ 100))
   ENDIF
   daod_date_update->data[date_record_int].type = dm.data_type, daod_date_update->data[
   date_record_int].default = dm.data_default, daod_date_update->data[date_record_int].tname = dm
   .table_name,
   daod_date_update->data[date_record_int].cname = dm.column_name, daod_date_update->data[
   date_record_int].af_nbr = dm.alpha_feature_nbr
  FOOT REPORT
   stat = alterlist(daod_date_update->data,date_record_int)
  WITH nocounter
 ;end select
 SET size_array = size(daod_date_update->data,5)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Converting the Data Default values in the record structure"
 CALL disp_msg(" ",dm_err->logfile,0)
 FOR (update_counter_int = 1 TO size_array)
   SET data_dflt_str = trim(daod_date_update->data[update_counter_int].default)
   SET data_type_str = trim(daod_date_update->data[update_counter_int].type)
   SET daod_date_update->data[update_counter_int].cnvt = convert_data_defaults(data_type_str,
    data_dflt_str,"ORACLE")
 ENDFOR
 SET dm_err->eproc = "Data_default's have been changed in record structure, beginning update"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (size_array > 0)
  IF (dm2_nocommit_ind=0)
   UPDATE  FROM dm_afd_columns dm,
     (dummyt d  WITH seq = value(size_array))
    SET dm.data_default = daod_date_update->data[d.seq].cnvt
    PLAN (d)
     JOIN (dm
     WHERE (dm.table_name=daod_date_update->data[d.seq].tname)
      AND (dm.column_name=daod_date_update->data[d.seq].cname)
      AND (dm.alpha_feature_nbr=daod_date_update->data[d.seq].af_nbr))
 ...
 IF (dm2_nocommit_ind=0)
  COMMIT
 ENDIF
 SET dm_err->eproc = "All data defaults of type date updated, starting to change data types"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET stat = alterlist(daod_data_types->table_names,1)
 SET daod_data_types->table_names[1].name = "DM_AFD_COLUMNS"
 FOR (data_type_loop_int = 1 TO 1)
   CALL echo("NUMBER to BIGINT for _ID,_CD,_COMPL,_VALUE columns")
   UPDATE  FROM (value(daod_data_types->table_names[data_type_loop_int].name) t)
    SET data_type = "BIGINT"
    WHERE data_type="NUMBER"
     AND ((column_name IN ("*\_ID")) OR (((column_name IN ("*\_CD")) OR (((column_name IN ("*\_COMPL"
    )) OR (column_name IN ("*\_VALUE"))) )) ))
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (dm2_nocommit_ind=0)
    COMMIT
   ENDIF
   CALL echo("NUMBER, FLOAT, INTEGER to INT for non _ID,_CD,_COMPL,_VALUE columns")
   UPDATE  FROM (value(daod_data_types->table_names[data_type_loop_int].name) t)
    SET data_type = "INT"
    WHERE  NOT (column_name IN ("*\_ID", "*\_CD", "*\_COMPL", "*\_VALUE"))
   ...
   IF (dm2_nocommit_ind=0)
    COMMIT
   ENDIF
   CALL echo("DATE, TIMESTAMP to DATETIME")
   UPDATE  FROM (value(daod_data_types->table_names[data_type_loop_int].name) t)
   ...
   IF (dm2_nocommit_ind=0)
    COMMIT
   ENDIF
   CALL echo("LONG, CLOB to TEXT")
   UPDATE  FROM (value(daod_data_types->table_names[data_type_loop_int].name) t)
    SET data_type = "TEXT"
    WHERE data_type IN ("LONG", "CLOB")
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (dm2_nocommit_ind=0)
    COMMIT
   ENDIF
   CALL echo("LONG RAW, BLOB to IMAGE")
   UPDATE  FROM (value(daod_data_types->table_names[data_type_loop_int].name) t)
    SET data_type = "IMAGE"
    WHERE data_type IN ("LONG RAW", "BLOB")
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (dm2_nocommit_ind=0)
    COMMIT
   ENDIF
   CALL echo("ROWID to CHAR 18")
   UPDATE  FROM (value(daod_data_types->table_names[data_type_loop_int].name) t)
    SET data_type = "CHAR", data_length = 18
   ...
   IF (dm2_nocommit_ind=0)
    COMMIT
   ENDIF
   CALL echo("RAW to VARCHAR 20")
   UPDATE  FROM (value(daod_data_types->table_names[data_type_loop_int].name) t)
    SET data_type = "VARCHAR", data_length = 20
    WHERE data_type="RAW"
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
 ENDFOR
 SET dm_err->eproc = "All data types updated, starting to change constraint names"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET stat = alterlist(daod_substring_tables->table_names,4)
 SET daod_substring_tables->table_names[1].name = "DM_AFD_INDEXES"
 SET daod_substring_tables->table_names[3].name = "DM_AFD_INDEX_COLUMNS"
 SET daod_substring_tables->table_names[2].name = "DM_AFD_CONSTRAINTS"
 SET daod_substring_tables->table_names[4].name = "DM_AFD_CONS_COLUMNS"
 SET daod_substring_tables->table_names[1].column = "INDEX_NAME"
 SET daod_substring_tables->table_names[3].column = "INDEX_NAME"
 SET daod_substring_tables->table_names[2].column = "CONSTRAINT_NAME"
 SET daod_substring_tables->table_names[4].column = "CONSTRAINT_NAME"
 SET daod_substring_tables->table_names[1].alias = "DAI"
 SET daod_substring_tables->table_names[3].alias = "DAIC"
 SET daod_substring_tables->table_names[2].alias = "DAC"
 SET daod_substring_tables->table_names[4].alias = "DACC"
 FOR (update_counter_int = 1 TO 2)
   SET counter_string = trim(cnvtstring(update_counter_int))
   SET tab_name_str = trim(daod_substring_tables->table_names[update_counter_int].name)
   SET col_name_str = trim(daod_substring_tables->table_names[update_counter_int].column)
   SET alias_name_str = trim(daod_substring_tables->table_names[update_counter_int].alias)
   SET stat = alterlist(daod_exempt_stmts->stmt,4)
   SET file_name = concat("TABLE NAMES WERE NOT UPDATED FROM ",tab_name_str)
   SET daod_exempt_stmts->stmt[update_counter_int].frags[1].stmt_frag = concat("select into noforms ",
    alias_name_str,".table_name ","from ",tab_name_str,
    " ",alias_name_str)
   SET daod_exempt_stmts->stmt[update_counter_int].frags[2].stmt_frag = concat(",dm_tables_doc d",
    " where ",alias_name_str,".table_name = d.table_name ","Group by ",
    alias_name_str,".table_name, ")
   SET daod_exempt_stmts->stmt[update_counter_int].frags[3].stmt_frag = concat(alias_name_str,
    ".alpha_feature_nbr, ","substring(1,12, ",alias_name_str,".",
    col_name_str,") ")
   SET daod_exempt_stmts->stmt[update_counter_int].frags[4].stmt_frag = "having count(*) > 1 GO"
   FOR (error_check_counter_int = 1 TO 4)
     IF (error_check_counter_int=4)
      SET go_stmt_flag_ind = 1
     ELSE
      SET go_stmt_flag_ind = 0
     ENDIF
     SET short_string = daod_exempt_stmts->stmt[update_counter_int].frags[error_check_counter_int].
     stmt_frag
     SET update_error_check_ind = dm2_push_cmd(short_string,go_stmt_flag_ind)
   ENDFOR
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ...
   IF (curqual > 0)
    ...
    CALL echo("*************************************************************************")
   ENDIF
   SET stat = alterlist(daod_update_stmt->stmt,6)
   SET daod_update_stmt->stmt[update_counter_int].frags[1].stmt_frag = concat("UPDATE INTO ",
    tab_name_str," ",alias_name_str)
   SET daod_update_stmt->stmt[update_counter_int].frags[2].stmt_frag = concat(" SET  ",alias_name_str,
    ".",col_name_str," = (")
   SET daod_update_stmt->stmt[update_counter_int].frags[3].stmt_frag = concat(
    "select SUBSTRNG = concat(trim(SUBSTRING(1, 12,",alias_name_str,".",col_name_str,
    ")), d.table_suffix)")
   SET daod_update_stmt->stmt[update_counter_int].frags[4].stmt_frag = concat(
    "  from DM_TABLES_DOC d where d.table_name = ",alias_name_str,".table_name) ")
   SET daod_update_stmt->stmt[update_counter_int].frags[5].stmt_frag = concat(
    " Where not exists (Select 'x' from ",tab_name_str," DMY",
    ", dm_tables_doc dtd where dtd.table_name = ")
   SET daod_update_stmt->stmt[update_counter_int].frags[6].stmt_frag = concat(
    "       DMY.table_name and dtd.table_name = ",alias_name_str,".table_name "," group by DMY",
    ".table_name, DMY",
    ".alpha_feature_nbr, substring(1,12,DMY",".",col_name_str,") having count(*) > 1) go")
   FOR (error_check_counter_int = 1 TO 6)
     IF (error_check_counter_int=6)
      SET go_stmt_flag_ind = 1
     ELSE
      SET go_stmt_flag_ind = 0
     ENDIF
     SET short_string = daod_update_stmt->stmt[update_counter_int].frags[error_check_counter_int].
     stmt_frag
     SET update_error_check_ind = dm2_push_cmd(short_string,go_stmt_flag_ind)
     IF (update_error_check_ind=1)
      SET display_message_str = concat("Tables names converted in ",counter_string," of 5 tables.")
     ELSE
      SET display_message_str = concat("Update statement ",trim(counter_string),"of 5 failed.")
      CALL disp_msg(display_message_str,"dm2_admin_2_sqlserv",0)
      CALL disp_msg("Script is now exiting","dm2_admin_2_sqlserv",0)
      GO TO exit_program
     ENDIF
   ENDFOR
   IF (dm2_nocommit_ind=0)
   ...
   ENDIF
 ENDFOR
 FOR (update_counter_int = 3 TO 4)
   IF (update_counter_int=3)
    SET exclusion_str = "column_position"
   ELSE
    SET exclusion_str = "position"
   ENDIF
   SET counter_string = trim(cnvtstring(update_counter_int))
   SET tab_name_str = trim(daod_substring_tables->table_names[update_counter_int].name)
   SET col_name_str = trim(daod_substring_tables->table_names[update_counter_int].column)
   ...
   SET daod_exempt_stmts->stmt[update_counter_int].frags[1].stmt_frag = concat("select into noforms ",
    ...
    " ",alias_name_str)
   SET daod_exempt_stmts->stmt[update_counter_int].frags[2].stmt_frag = concat(", dm_tables_doc d",
    " where d.table_name = ",alias_name_str,".table_name and ",alias_name_str,
    ".",exclusion_str," = 1 Group by ",alias_name_str,".table_name, ")
   SET daod_exempt_stmts->stmt[update_counter_int].frags[3].stmt_frag = concat(alias_name_str,
    ".alpha_feature_nbr, ","substring(1,12, ",alias_name_str,".",
    col_name_str,") ")
   SET daod_exempt_stmts->stmt[update_counter_int].frags[4].stmt_frag = "having count(*) > 1 GO"
   ...
   SET daod_update_stmt->stmt[update_counter_int].frags[4].stmt_frag = concat(
    "from DM_TABLES_DOC d where d.table_name = ",alias_name_str,".table_name) ")
   SET daod_update_stmt->stmt[update_counter_int].frags[5].stmt_frag = concat("Where ",
    "not exists (Select 'x'"," from ",tab_name_str," DMY",
    ", dm_tables_doc dtd where dtd.table_name = ")
   SET daod_update_stmt->stmt[update_counter_int].frags[6].stmt_frag = concat(
    "DMY.table_name and dtd.table_name = ",alias_name_str,".table_name "," and DMY",".",
    exclusion_str," = 1 group by DMY",".table_name, DMY",".alpha_feature_nbr, ","substring(1,12,DMY",
    ".",col_name_str,") having count(*) > 1) go")
   FOR (error_check_counter_int = 1 TO 6)
     IF (error_check_counter_int=6)
      SET go_stmt_flag_ind = 1
     ELSE
   ...
   ENDIF
 ENDFOR
 UPDATE  FROM dm_afd_constraints dm
  SET dm.r_constraint_name =
   (SELECT DISTINCT
    dm.constraint_name
    FROM dm_afd_constraints d
    WHERE d.table_name=dm.parent_table_name
     AND d.constraint_type="P")
  WHERE constraint_type="R"
 ;end update
 CALL echo(build("Update dm_afd_constraints constraint_name:",curqual))
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 IF (currdb != "SQLSRV")
  UPDATE  FROM dm_sequences s
   SET s.max_value = 0
   WHERE s.max_value > 9000000000000000000.0
  ;end update
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
 ...
 IF (dm2_nocommit_ind=0)
  COMMIT
 ENDIF
 SET dm_err->eproc = "All column indexes/constraints updated, starting to suffix table names"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET stat = alterlist(daod_tables_changed->table_names,17)
 SET daod_tables_changed->table_names[1].name = "DM_AFD_CODE_VALUE_SET"
 SET daod_tables_changed->table_names[2].name = "DM_AFD_COLUMNS"
 SET daod_tables_changed->table_names[3].name = "DM_AFD_CONSTRAINTS"
 SET daod_tables_changed->table_names[4].name = "DM_AFD_CONSTRAINTS"
 SET daod_tables_changed->table_names[5].name = "DM_AFD_CONS_COLUMNS"
 SET daod_tables_changed->table_names[6].name = "DM_AFD_INDEXES"
 SET daod_tables_changed->table_names[7].name = "DM_AFD_INDEX_COLUMNS"
 SET daod_tables_changed->table_names[8].name = "DM_AFD_TABLES"
 SET daod_tables_changed->table_names[9].name = "DM_ENV_TABLE"
 SET daod_tables_changed->table_names[10].name = "DM_ENV_TABLE_SHIP"
 SET daod_tables_changed->table_names[11].name = "DM_FEATURE_TABLES_ENV"
 SET daod_tables_changed->table_names[12].name = "DM_FLAGS"
 SET daod_tables_changed->table_names[13].name = "DM_MRG_TBL_GRP"
 SET daod_tables_changed->table_names[14].name = "DM_PRODUCT_TABLE_RELTN"
 SET daod_tables_changed->table_names[15].name = "DM_REF_DOMAIN"
 SET daod_tables_changed->table_names[16].name = "DM_TABLE_DEPENDENCY"
 SET daod_tables_changed->table_names[17].name = "DM_VARIANCE_ROWS"
 SET daod_tables_changed->table_names[1].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[2].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[3].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[4].column = "PARENT_TABLE_NAME"
 SET daod_tables_changed->table_names[5].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[6].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[7].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[8].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[9].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[10].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[11].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[12].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[13].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[14].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[15].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[16].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[17].column = "TABLE_NAME"
 SET daod_tables_changed->table_names[1].alias = "DACVS"
 SET daod_tables_changed->table_names[2].alias = "DAC"
 SET daod_tables_changed->table_names[3].alias = "DACO"
 SET daod_tables_changed->table_names[4].alias = "DACS"
 SET daod_tables_changed->table_names[5].alias = "DACC"
 SET daod_tables_changed->table_names[6].alias = "DAI"
 SET daod_tables_changed->table_names[7].alias = "DAIC"
 SET daod_tables_changed->table_names[8].alias = "DAT"
 SET daod_tables_changed->table_names[9].alias = "DET"
 SET daod_tables_changed->table_names[10].alias = "DETS"
 SET daod_tables_changed->table_names[11].alias = "DFTE"
 SET daod_tables_changed->table_names[12].alias = "DF"
 SET daod_tables_changed->table_names[13].alias = "DMTG"
 SET daod_tables_changed->table_names[14].alias = "DPTR"
 SET daod_tables_changed->table_names[15].alias = "DRD"
 SET daod_tables_changed->table_names[16].alias = "DTD"
 SET daod_tables_changed->table_names[17].alias = "DVR"
 FOR (update_counter_int = 1 TO 17)
  SET more_updates_ind = 1
  WHILE (more_updates_ind=1)
    SET stat = alterlist(daod_update_stmt->stmt,17)
    SET counter_string = trim(cnvtstring(update_counter_int))
    SET tab_name_str = trim(daod_tables_changed->table_names[update_counter_int].name)
    SET col_name_str = trim(daod_tables_changed->table_names[update_counter_int].column)
    SET alias_name_str = trim(daod_tables_changed->table_names[update_counter_int].alias)
    SET daod_update_stmt->stmt[update_counter_int].frags[1].stmt_frag = concat("UPDATE INTO ",
     tab_name_str," ",alias_name_str)
    SET daod_update_stmt->stmt[update_counter_int].frags[2].stmt_frag = concat(" SET  ",
     alias_name_str,".",col_name_str," = (")
    SET daod_update_stmt->stmt[update_counter_int].frags[3].stmt_frag =
    "select d.suffixed_table_name "
    SET daod_update_stmt->stmt[update_counter_int].frags[4].stmt_frag = "FROM DM_TABLES_DOC d "
    SET daod_update_stmt->stmt[update_counter_int].frags[5].stmt_frag = concat(
     "Where d.Table_name = ",alias_name_str,".",col_name_str,
     " and d.suffixed_table_name is not null) ")
    SET daod_update_stmt->stmt[update_counter_int].frags[6].stmt_frag = concat("where ",
     alias_name_str,".",col_name_str," is not null ")
    SET daod_update_stmt->stmt[update_counter_int].frags[7].stmt_frag = concat(" and ",alias_name_str,
     ".",col_name_str," not in (")
    SET daod_update_stmt->stmt[update_counter_int].frags[8].stmt_frag = concat(
     " select d2.suffixed_table_name  from dm_tables_doc d2 ")
    SET daod_update_stmt->stmt[update_counter_int].frags[9].stmt_frag = concat(
     " where d2.table_name = ",alias_name_str,".",col_name_str," ) ")
    SET daod_update_stmt->stmt[update_counter_int].frags[10].stmt_frag = concat(
     " where d2.table_name = ",alias_name_str,".",col_name_str," ) ")
    SET daod_update_stmt->stmt[update_counter_int].frags[10].stmt_frag = concat(" with maxqual (",
     alias_name_str,", 5000) go")
    FOR (error_check_counter_int = 1 TO 10)
      IF (error_check_counter_int=10)
       SET go_stmt_flag_ind = 1
      ELSE
       SET go_stmt_flag_ind = 0
      ENDIF
      ...
      SET update_error_check_ind = dm2_push_cmd(short_string,go_stmt_flag_ind)
      IF (update_error_check_ind=1)
       IF (go_stmt_flag_ind=1)
        CALL echo(build("++++++++ curqual:",curqual))
        IF (curqual < 5000)
         SET more_updates_ind = 0
        ELSE
         SET display_message_str = concat("Tables names converted in ",counter_string,
          " of 17 tables.")
        ENDIF
       ENDIF
      ELSE
       SET display_message_str = concat("Update statement ",counter_string,
        " of 17 statements failed.")
       CALL disp_msg(display_message_str,"dm2_admin_2_sqlserv",0)
       CALL disp_msg("Script is now exiting","dm2_admin_2_sqlserv",0)
       GO TO exit_program
      ENDIF
 ...
 ENDFOR
 SET dm_err->eproc =
 "Table names changed to suffixed table name, freeing record structures and variables"
 CALL disp_msg(" ",dm_err->logfile,0)
 SUBROUTINE convert_data_defaults(data_type,data_default,source_rdbms)
   DECLARE data_default_scrunched = vc WITH protect, noconstant(" ")
   DECLARE data_default_return = vc WITH protect, noconstant(" ")
   IF (source_rdbms=cnvtupper(currdb))
    SET data_default_return = data_default
   ELSEIF (((data_type="DATE") OR (data_type="TIMESTAMP")) )
    SET data_default_scrunched = cnvtalphanum(data_default)
    IF (source_rdbms="ORACLE")
     IF (substring(1,6,data_default_scrunched)="TODATE")
      SET data_default_return = concat("CONVERT(DATETIME, '",substring(11,4,data_default_scrunched),
       "-",substring(7,2,data_default_scrunched),"-",
       substring(9,2,data_default_scrunched)," ",substring(15,2,data_default_scrunched),":",substring
       (17,2,data_default_scrunched),
       ":",substring(19,2,data_default_scrunched),"', 120)")
     ELSEIF (substring(1,7,data_default_scrunched)="SYSDATE")
      SET data_default_return = build("CURRENT_TIMESTAMP")
     ELSE
      SET data_default_return = data_default
     ENDIF
    ELSEIF (source_rdbms="DB2UDB")
     IF (substring(1,15,data_default_scrunched)="SYSIBMTIMESTAMP")
      SET data_default_return = concat("CONVERT(DATETIME, '",substring(16,4,data_default_scrunched),
       "-",substring(20,2,data_default_scrunched),"-",
       substring(22,2,data_default_scrunched)," ",substring(24,2,data_default_scrunched),":",
       substring(26,2,data_default_scrunched),
       ":",substring(28,2,data_default_scrunched),"', 120)")
     ELSEIF (substring(1,16,data_default_scrunched)="CURRENTTIMESTAMP")
      SET data_default_return = build("CURRENT_TIMESTAMP")
     ELSE
      SET data_default_return = data_default
     ENDIF
   ...
   ENDIF
   IF ((dm_err->debug_flag=1))
    CALL echo(build("data_type:",data_type))
    CALL echo(build("data_default:",data_default))
    CALL echo(build("source_rdbms:",source_rdbms))
    CALL echo(build("data_default_return:",data_default_return))
   ENDIF
   RETURN(data_default_return)
 END ;Subroutine
 IF (omittance_flg > 0)
  CALL echo("Scroll up to see the tables that were not updated")
 ENDIF
#exit_program
 FREE RECORD daod_data_types
 FREE RECORD daod_substring_tables
 FREE RECORD daod_update_stmt
 FREE RECORD daod_tables_changed
 FREE RECORD daod_date_update
 FREE RECORD daod_exempt_stmts
 FREE SET ace_str
 FREE SET data_type_loop_int
 FREE SET display_message_str
 FREE SET update_counter_int
 FREE SET counter_string
 FREE SET tab_name_str
 FREE SET col_name_str
 FREE SET alias_name_str
 FREE SET error_check_counter_int
 FREE SET go_stmt_flag_ind
 FREE SET update_error_check_ind
 FREE SET short_string
 FREE SET date_record_int
 FREE SET data_dflt_str
 FREE SET data_type_str
 SET dm_err->eproc = "Ending dm2_admin_2_sqlserv"
 CALL final_disp_msg(" ")
END GO
