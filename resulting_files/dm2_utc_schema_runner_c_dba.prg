CREATE PROGRAM dm2_utc_schema_runner_c:dba
 DECLARE dusrc_orig_emsg = vc WITH public, noconstant("")
 DECLARE dusrc_updlock_ind = i2 WITH protect, noconstant(0)
 DECLARE dusrc_act_duration = f8 WITH protect, noconstant(0.0)
 DECLARE dusrc_end_dt_tm = f8 WITH protect, noconstant(0.0)
 DECLARE dusrc_err_ret = i2 WITH protect, noconstant(0)
 DECLARE dusrc_log_loc_ret = vc WITH protect, noconstant("")
 DECLARE update_lock_row(null) = i2
 DECLARE update_log_table(ult_opid=f8,ult_status=vc,ult_err_msg=vc) = i2
 DECLARE dusr_execute_command(dec_operation=vc) = null
 DECLARE dusr_create_sql_error_chk(dcsec_object_type=vc,dcsec_object_name=vc) = i2
 ...
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO end_program
 ENDIF
 IF ((validate(dusr_cleanup_timestamp,- (1))=- (1)))
  ...
  GO TO end_program
 ENDIF
 IF (dusr_cmd_rscbsy_retry_cnt > 1)
  SET dusrc_updlock_ind = 1
 ELSE
  IF (update_lock_row(null)=0)
   GO TO end_program
  ENDIF
 ENDIF
 IF (dusrc_updlock_ind=1)
  CALL dusr_execute_command(dusr_command)
  IF ((dm_err->err_ind=1))
   SET dusr_cmd_successful = 0
   SET dm_err->err_ind = 0
   IF (dusr_cmd_rscbsy_error=1
    AND dusr_cmd_rscbsy_retry_cnt < cnvtint(dm2_db_options->resource_busy_maxretry))
    IF (update_log_table(dusr_op_id,"RUNNING",dusrc_orig_emsg)=0)
     GO TO end_program
    ENDIF
   ELSE
    IF (update_log_table(dusr_op_id,"ERROR",dusrc_orig_emsg)=0)
     GO TO end_program
    ENDIF
   ENDIF
  ELSE
   SET dusr_cmd_successful = 1
   IF (update_log_table(dusr_op_id,"COMPLETE"," ")=0)
    GO TO end_program
   ENDIF
  ENDIF
 ENDIF
 ...
 SUBROUTINE update_lock_row(null)
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = build("Update/Lock op_id:",dusr_execlist->exec[dusreidx].op_id)
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   IF ((dm_err->debug_flag > 1))
    CALL echo(concat("Current date and time = ",format(cnvtdatetime(curdate,curtime),
       "mm/dd/yyyy hh:mm:ss;;d")))
    CALL echo(concat("Last cleanup date and time = ",format(dusr_cleanup_timestamp,
       "mm/dd/yyyy hh:mm:ss;;d")))
    CALL echo(concat("Number of minutes elapsed = ",cnvtstring(datetimediff(cnvtdatetime(curdate,
         curtime),cnvtdatetime(dusr_cleanup_timestamp),4),4,3)))
   ENDIF
   IF (datetimediff(cnvtdatetime(curdate,curtime),cnvtdatetime(dusr_cleanup_timestamp),4) > 10)
    SET dusr_cleanup_timestamp = cnvtdatetime(curdate,curtime)
    IF (dm2_cleanup_stranded_appl(null)=0)
     RETURN(0)
    ENDIF
    IF (dusr_dm_info_exists=1)
     IF (dum_cleanup_stranded_appl_id(null)=0)
      GO TO end_program
     ENDIF
     SET dm_err->eproc = "Evaluating whether the schema runner has been marked to stop."
     SELECT INTO "nl:"
      di.info_name
      FROM dm_info di
      WHERE di.info_domain="DM2_UTC_SCHEMA_RUNNER"
    ...
    ENDIF
   ENDIF
   SET dusr_begin_dt_tm = cnvtdatetime(curdate,curtime3)
   IF (dusr_serial_op_types != "('DM2NOTSET')")
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = "RUNNING", ddol.begin_dt_tm = cnvtdatetime(dusr_begin_dt_tm), ddol
      .logfile_name = dm_err->logfile,
      ddol.appl_id = currdbhandle
     WHERE (ddol.op_id=dusr_execlist->exec[dusreidx].op_id)
      AND ddol.status = null
      AND ((parser(concat("ddol.op_type in ",dusr_serial_op_types))
      AND  NOT ( EXISTS (
     (SELECT
      "x"
      FROM dm2_ddl_ops_log x
      WHERE parser(concat("x.op_type in ",dusr_serial_op_types))
       AND x.status="RUNNING"
       AND x.run_id=ddol.run_id)))) OR (parser(concat("ddol.op_type not in ",dusr_serial_op_types))
     ))
     WITH nocounter
    ;end update
   ELSE
   ...
   IF (check_error("Updating a dm2_ddl_ops_log DDL row for execution.")=1)
    ...
    RETURN(0)
   ELSE
    SET dusr_updt_cnt = (dusr_updt_cnt+ 1)
    IF (curqual > 0)
     COMMIT
     SET dusrc_updlock_ind = 1
     SET dusr_updt_fnd_cnt = (dusr_updt_fnd_cnt+ 1)
     SET dusr_updt_constolen_cnt = 0
     SELECT INTO "nl:"
      FROM dm2_ddl_ops_log ddol1
      WHERE (ddol1.op_id=dusr_execlist->exec[dusreidx].op_id)
      DETAIL
       dusr_op_id = ddol1.op_id, dusr_op_type = ddol1.op_type, dusr_command = ddol1.operation,
       dusr_oe_command = ddol1.on_error_operation, dusr_obj_name = ddol1.obj_name
      WITH nocounter
     ;end select
     IF (check_error(build("Selecting execution information from dm2_ddl_ops_log for op_id: ",
       dusr_execlist->exec[dusreidx].op_id))=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ELSEIF (curqual != 1)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = build(
       "Error selecting execution information from dm2_ddl_ops_log for op_id: ",dusr_execlist->exec[
       dusreidx].op_id,".  Row not found.")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
    ELSE
     ROLLBACK
     SET dusrc_updlock_ind = 0
     SET dusr_updt_stolen_cnt = (dusr_updt_stolen_cnt+ 1)
     SET dusr_updt_constolen_cnt = (dusr_updt_constolen_cnt+ 1)
    ENDIF
    IF ((dm_err->debug_flag > 3))
     CALL echo(build("dusr_updt_cnt:",dusr_updt_cnt))
     CALL echo(build("dusr_updt_fnd_cnt:",dusr_updt_fnd_cnt))
     CALL echo(build("dusr_updt_stolen_cnt:",dusr_updt_stolen_cnt))
     CALL echo(build("dusr_updt_constolen_cnt:",dusr_updt_constolen_cnt))
    ENDIF
   ENDIF
   IF ((dm_err->debug_flag > 0)
    AND dusrc_updlock_ind=1)
    CALL echo(build("op id        :",dusr_op_id))
    CALL echo(build("op type      :",dusr_op_type))
    CALL echo(build("command      :",dusr_command))
    CALL echo(build("error command:",dusr_oe_command))
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE update_log_table(ult_opid,ult_status_text,ult_err_msg)
   ...
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = build("Updating DM2_DDL_OPS_LOG.STATUS to: ",ult_status_text," for OP_ID: ",
     ult_opid)
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET dusrc_end_dt_tm = cnvtdatetime(curdate,curtime3)
   SET dusrc_act_duration = ((dusrc_end_dt_tm - dusr_begin_dt_tm)/ 10000000)
   IF (ult_status_text IN ("ERROR", "RUNNING"))
    IF (dusr_cmd_rscbsy_retry_cnt > 1)
     SET ult_local_msg = build("RESOURCE BUSY: ATTEMPT (",dusr_cmd_rscbsy_retry_cnt,") OF (",
      dm2_db_options->resource_busy_maxretry,").  ERROR_MSG:",
      dusrc_orig_emsg)
    ENDIF
    SET ult_local_msg = substring(1,4000,ult_local_msg)
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = ult_status_text, ddol.end_dt_tm = cnvtdatetime(dusrc_end_dt_tm), ddol
      .act_duration = dusrc_act_duration,
    ...
    ELSE
     COMMIT
    ENDIF
   ELSEIF (ult_status_text="COMPLETE")
    IF (dusr_cmd_rscbsy_retry_cnt > 1)
     SET ult_local_msg = build("COMPLETE AFTER RESOURCE BUSY ATTEMPT (",dusr_cmd_rscbsy_retry_cnt,
      ") OF (",dm2_db_options->resource_busy_maxretry,").")
    ENDIF
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = ult_status_text, ddol.end_dt_tm = cnvtdatetime(dusrc_end_dt_tm), ddol
      .act_duration = dusrc_act_duration,
   ...
   ENDIF
   IF ((dm_err->debug_flag > 3))
    CALL echo(build("Status     :",ult_status_text))
    CALL echo(build("Op Id      :",ult_opid))
    CALL echo(build("Begin Time :",dusr_begin_dt_tm))
    CALL echo(build("End Time   :",dusrc_end_dt_tm))
    CALL echo(build("Duration   :",dusrc_act_duration))
    ...
    SELECT INTO "nl:"
     ...
     WITH nocounter
    ;end select
    IF (check_error("debug select statement in update_log_table")=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dusr_execute_command(dec_operation)
   DECLARE dusrec_pword = vc WITH protect, noconstant("")
   SET dusrec_pword = dm2_install_schema->v500_p_word
   CALL dm2_push_cmd(dec_operation,1)
   IF ((dm_err->err_ind=0)
    AND dusr_op_type="CREATE SQL*")
    SET dusr_obj_type = substring((1+ findstring("-",dusr_op_type,1)),(size(dusr_op_type) -
     findstring("-",dusr_op_type,1)),dusr_op_type)
    IF (dusr_create_sql_error_chk(dusr_obj_type,dusr_obj_name)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("dusr_execute_command->dm_err->err_ind: ",dm_err->err_ind))
   ENDIF
   IF ((dm_err->err_ind > 0))
    IF (((findstring("ORA-00054",dm_err->emsg) > 0) OR (findstring("ORA-14411",dm_err->emsg) > 0)) )
     SET dusr_cmd_rscbsy_error = 1
    ELSE
     SET dusr_cmd_rscbsy_error = 0
    ENDIF
    SET dusrc_orig_emsg = dm_err->emsg
    ROLLBACK
    IF ((dm_err->debug_flag > 0))
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dusr_create_sql_error_chk(dcsec_object_type,dcsec_object_name)
   DECLARE dcsec_err_msg = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Validating the creation of SQL object."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF (dcsec_object_type != "TRIGGER")
    SET dm_err->emsg = concat("Object type <",dcsec_object_type,"> is not currently supported.")
    SET dm_err->err_ind = 1
    RETURN(0)
   ENDIF
   SELECT INTO "nl:"
    ue.text
    FROM user_errors ue
    WHERE ue.type="TRIGGER"
     AND ue.name=dcsec_object_name
    ORDER BY ue.sequence, ue.line, ue.position
    DETAIL
     dcsec_err_msg = concat(dcsec_err_msg,"  ",ue.text)
    WITH nocounter
   ;end select
   IF (check_error("Selecting from user_errors to see if trigger was created successfully.")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ELSEIF (curqual > 0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = dcsec_err_msg
    RETURN(1)
   ENDIF
   SELECT INTO "nl:"
    uo.object_name
    ...
    WHERE uo.object_type="TRIGGER"
     AND uo.object_name=dcsec_object_name
     AND uo.status="VALID"
    WITH nocounter
   ;end select
   IF (check_error("Selecting from user_objects to see if trigger is in a valid status.")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ELSEIF (curqual=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Trigger not in a valid status."
    RETURN(1)
   ENDIF
   RETURN(1)
 END ;Subroutine
