CREATE PROGRAM dm_obs_indexes_esi_80:dba
 ...
 SET tmp->qual[1].index_name = "XPKCODE_VALUE_ALIAS"
 ...
 IF (droptotal != recordcnt)
  SET readme_data->message = build(errmsg,"- Readme FAILURE. Check dm_obs_indexes_esi_80.log")
  SET readme_data->status = "F"
 ELSE
  SET readme_data->message = build(errmsg,"- Readme SUCCESS. Check dm_obs_indexes_esi_80.log")
  SET readme_data->status = "S"
 ENDIF
 CALL echo(readme_data->message)
 SELECT INTO "dm_obs_indexes_esi_80.log"
  d.seq
  FROM dummyt d
  FOOT REPORT
   "***********************************************************************", row + 1
   IF (droptotal=recordcnt)
    "  TRACE LOG - DM_OBS_INDEXES_ESI_80: SUCCESS"
   ELSE
    "  TRACE LOG - DM_OBS_INDEXES_ESI_80: FAILURE"
   ENDIF
   row + 1, "***********************************************************************", row + 2,
