CREATE PROGRAM ct_mp_get_all_sponsor_list:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_all_sponsor_list"
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getprimarysponsor(null) = null WITH protect
 DECLARE stat = i4 WITH protect, noconstant(0)
 DECLARE protcount = i4 WITH protect, noconstant(0)
 DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
 SET reply->status_data.status = "F"
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_all_sponsor_list","Invalid JSON Request",
   reply)
  GO TO exit_script
 ENDIF
 ...
 IF (protcount <= 0)
  ...
  GO TO exit_script
 ENDIF
 CALL getprimarysponsor(null)
 FREE RECORD request
 SET reply->status_data.status = "S"
 SUBROUTINE getprimarysponsor(null)
   DECLARE sponsortypecd = f8 WITH protect, constant(uar_get_code_by("MEANING",17271,"PRIMARY"))
   DECLARE primarysponsorcount = i4 WITH protect, noconstant(0)
   DECLARE amendmentitr = i4 WITH protect, noconstant(0)
   SELECT DISTINCT INTO "nl:"
    FROM prot_grant_sponsor pgs,
     organization org
    PLAN (pgs
     WHERE expand(amendmentitr,1,protcount,pgs.prot_amendment_id,cnvtreal(request->prot_list[
    ...
    HEAD org.organization_id
     primarysponsorcount += 1
     IF (size(reply->sponsors_list,5) < primarysponsorcount)
      stat = alterlist(reply->sponsors_list,(primarysponsorcount+ 9))
     ENDIF
     reply->sponsors_list[primarysponsorcount].org_id = org.organization_id, reply->sponsors_list[
     primarysponsorcount].org_name = org.org_name
    WITH nocounter
   ;end select
   SET stat = alterlist(reply->sponsors_list,primarysponsorcount)
 END ;Subroutine
#exit_script
 ...
 CALL log_message(build("ct_mp_get_all_sponsor_list script ends here.Elapsed time in seconds:",
