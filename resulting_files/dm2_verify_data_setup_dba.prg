CREATE PROGRAM dm2_verify_data_setup:dba
 ...
 DECLARE dvds_is_mig_flag = i2 WITH protect, noconstant(0)
 IF ((dm2_install_schema->curprog="NONE"))
  SET dvds_is_mig_flag = 0
 ELSEIF ((((dm2_install_schema->curprog="dm2_mig_vdata_setup")) OR ((dm2_install_schema->curprog=
 "dm2_mig_verify_csv_setup"))) )
  SET dvds_is_mig_flag = 1
 ENDIF
 IF (check_logfile("dm2_vdata_setup",".log","dm2_vdata_setup LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Executing dm2_verify_data_setup for user(s) ",dmvd->owner)
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 IF (dvdr_get_user_connect_info(dvds_is_mig_flag)=0)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echo("************DM2_INSTALL_SCHEMA***************")
 ...
 IF ((dmvd->db_link="DM2NOTSET")
  AND (dmvd->owner="DM2NOTSET"))
  IF (dvdr_get_data_verification_info(0)=0)
   GO TO exit_script
  ENDIF
  SET message = nowindow
 ENDIF
 IF (dvdr_get_selected_user_list(dmvd->owner)=0)
  GO TO exit_script
 ELSEIF ((dvdr_user_list->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No valid users returned!"
  GO TO exit_script
 ENDIF
 IF (dvdr_save_user_list(dvds_is_mig_flag)=0)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echo("************DVDR_USER_LIST***************")
  CALL echorecord(dvdr_user_list)
 ENDIF
 SET dm2_install_schema->u_name = "V500"
 SET dm2_install_schema->dbase_name = dm2_install_schema->src_dbase_name
 SET dm2_install_schema->connect_str = dm2_install_schema->src_v500_connect_str
 ...
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 FOR (dvds_cnt = 1 TO dvdr_user_list->cnt)
  EXECUTE dm2_setup_table_monitoring dvdr_user_list->qual[dvds_cnt].user
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
 ENDFOR
 ...
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 IF (dvdr_populate_table_list(dvds_is_mig_flag)=0)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echo("************DMVD***************")
  CALL echo(dmvd->tbl_cnt)
  IF ((dm_err->debug_flag=511))
   CALL echorecord(dmvd)
  ENDIF
 ENDIF
 IF (dvdr_setup_tables_for_compare(null)=0)
  GO TO exit_script
 ENDIF
 IF (dvdr_write_exclusion_rows(null)=0)
  GO TO exit_script
 ENDIF
 IF (dvdr_write_dminfo_rows(null)=0)
  GO TO exit_script
 ENDIF
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_verify_data_setup completed succesfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 CALL final_disp_msg("dm2_verify_data_setup")
END GO
