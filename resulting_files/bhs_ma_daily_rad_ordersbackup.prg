CREATE PROGRAM bhs_ma_daily_rad_ordersbackup
 PROMPT
   ...
   PLAN (o
    ...
    (SELECT
     ...
     WHERE ((oc.catalog_cd+ 0)=o.catalog_cd)
      ...
      AND oc.activity_subtype_cd IN (computer_tomography))))
    JOIN (oc
  ...
  SELECT INTO value(out_dev)
   ...
   PLAN (o
    ...
    (SELECT
     ...
     WHERE ((oc.catalog_cd+ 0)=o.catalog_cd)
      AND oc.activity_type_cd=o.activity_type_cd
      AND oc.activity_subtype_cd IN (633747, 633750, 633752))))) )
    JOIN (oc
    WHERE oc.catalog_cd=o.catalog_cd)
    JOIN (p
    WHERE p.person_id=o.person_id)
