CREATE PROGRAM ct_mp_get_prot_activation_date:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_prot_activation_date"
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getprotactivationdate(null) = null WITH protect
 DECLARE stat = i4 WITH protect, noconstant(0)
 SET public::"reply"->status_data.status = "F"
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_activation_date",
   "Invalid JSON Request",PUBLIC::"reply")
  GO TO exit_script
 ENDIF
 SET stat = cnvtjsontorec( $JSONREQUEST)
 CALL getprotactivationdate(null)
 FREE RECORD request
 SET public::"reply"->status_data.status = "S"
 SUBROUTINE getprotactivationdate(null)
   ...
   IF (protcount > 0)
    ...
    SELECT
     ...
     DETAIL
      ...
      "reply"->prot_list[d.seq]."amendmentId" = request->prot_list[d.seq].amendmentid
      IF (pa.amendment_dt_tm < cnvtdatetime("31-DEC-2100 00:00:00.00"))
       public::"reply"->prot_list[d.seq]."prot_info"."prot_activated_date" = format(pa
