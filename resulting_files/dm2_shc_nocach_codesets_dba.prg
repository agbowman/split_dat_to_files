CREATE PROGRAM dm2_shc_nocach_codesets:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD non_cached_codesets
 RECORD non_cached_codesets(
   1 cnt = i4
   1 cs[*]
     2 code_set = vc
 )
 SET non_cached_codesets->cnt = 0
 DECLARE dsnc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dsnc_status = c1 WITH protect, noconstant("X")
 DECLARE dsnc_getscript = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsnc_prev_errfile = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsnc_dcl_str = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsnc_del_str = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsnc_extension = vc WITH protect, noconstant(".ksh")
 DECLARE dsnc_fatal_err_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_shc_cs_cache",".log","SYSTEM HEALTH CHECK CACHED CODE SET LOG FILE")=0)
  SET dsnc_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking for Code Sets not cached by the Code Cache Manager."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ENDIF
 IF (cursys="AXP")
  SET dsnc_extension = ".com"
 ENDIF
 IF (get_unique_file("dm2_get_sta_errs",dsnc_extension)=0)
  SET dsnc_status = "E"
 ENDIF
 IF (dsnc_status != "E")
  SET dsnc_getscript = dm_err->unique_fname
  SET dsnc_prev_errfile = dm_err->errfile
  SET dm_err->errfile = replace(dsnc_getscript,dsnc_extension,".dat",0)
  IF (cursys="AIX")
   SET dsnc_dcl_str = concat("$CCLUSERDIR/",dsnc_getscript)
   SET dsnc_del_str = "rm -f $CCLUSERDIR/dm2_get_sta_errs*"
  ELSE
   SET dsnc_dcl_str = concat("@ccluserdir:",dsnc_getscript)
   SET dsnc_del_str = "delete ccluserdir:dm2_get_sta_errs*.*;*"
  ENDIF
  SELECT INTO trim(dsnc_getscript)
   DETAIL
    IF (cursys="AIX")
     col + 0, "   msgview <<!"
    ELSE
     col + 0, "   $ msgview"
    ENDIF
    row + 1, col + 0, "   select cmb_0052",
    row + 1, col + 0, "   search  -event *CodesetNotInTree"
    IF (cursys="AIX")
     row + 1, col + 0, "   !"
    ENDIF
   WITH nocounter, format = variable, formfeed = none,
    maxrow = 1
  ;end select
  IF (check_error("Writing msgview extract routine.")=1)
   SET dsnc_status = "E"
  ENDIF
 ENDIF
 IF (dsnc_status != "E"
  AND cursys="AIX")
  IF (dm2_push_dcl(concat("chmod ugo+x $CCLUSERDIR/",dsnc_getscript))=0)
   SET dsnc_status = "E"
  ENDIF
 ENDIF
 IF (dsnc_status != "E")
  IF (dm2_push_dcl(dsnc_dcl_str)=0)
   SET dsnc_status = "E"
  ENDIF
 ENDIF
 IF (dsnc_status != "E")
 ...
 IF (dsnc_status != "E")
  SELECT INTO "nl:"
   ...
   FROM rtl2t r
   HEAD REPORT
    rpt_substr = fillstring(6," "), rpt_script_start_pos = 0
   DETAIL
    rpt_substr = substring(1,6,r.line)
    IF (isnumeric(rpt_substr))
     non_cached_codesets->cnt += 1, stat = alterlist(non_cached_codesets->cs,non_cached_codesets->cnt
      ), rpt_script_start_pos = (findstring("CodesetNotInTree",r.line)+ 16),
     non_cached_codesets->cs[non_cached_codesets->cnt].code_set = trim(substring(rpt_script_start_pos,
       (size(r.line) - rpt_script_start_pos),r.line),3)
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error("Parsing MSGView errors.")=1)
   SET dsnc_status = "E"
  ENDIF
 ENDIF
 IF (dsnc_status != "E")
  SET dm_err->errfile = dsnc_prev_errfile
  IF (dm2_push_dcl(dsnc_del_str)=0)
   SET dsnc_status = "E"
  ENDIF
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(non_cached_codesets)
 ENDIF
 IF ((((dm_err->err_ind=1)) OR (dsnc_status="E")) )
  SET dsnc_status = "E"
 ELSEIF ((non_cached_codesets->cnt > 0))
  SET dsnc_status = "R"
 ELSE
  SET dsnc_status = "G"
 ENDIF
 IF (dshc_add_check_to_rs("NON-CACHED CODE SETS","Non-Cached Code Sets",0,dsnc_status,
  dshc_ctf_millennium)=0)
  SET dsnc_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 IF (dsnc_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  cs = non_cached_codesets->cs[d.seq].code_set
  FROM (dummyt d  WITH seq = non_cached_codesets->cnt)
  ORDER BY cs
  HEAD cs
   ...
   ->check[dshc->chk_cnt].desc_dtl_cnt].name = "Non-Cached Code Sets",
   ...
   non_cached_codesets->cs[d.seq].code_set
  WITH nocounter
 ;end select
 SET dshc->check[dshc->chk_cnt].check_desc_text =
 "The following code sets are not being cached by the CPM Code Cache Manager."
#exit_script
 SET dm_err->eproc = "Finished checking for noncached code sets."
 CALL final_disp_msg("dm2_shc_cs_cache")
 IF (dsnc_fatal_err_ind=0)
  SET dm_err->err_ind = 0
 ENDIF
END GO
