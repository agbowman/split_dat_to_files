CREATE PROGRAM cp_add_concept_group
 ...
 DECLARE nverified = i2 WITH noconstant(0), protect
 DECLARE lstat = i4 WITH noconstant(0), protect
 DECLARE lconcept_cs = i4 WITH constant(4003132), protect
 DECLARE sconcept_disp = vc WITH constant("Concept code"), protect
 DECLARE lconcept_grp_cs = i4 WITH constant(4003133), protect
 DECLARE sconcept_grp_disp = vc WITH constant("Concept Group Code"), protect
 DECLARE lintention_cs = i4 WITH constant(4003278), protect
 DECLARE sintention_disp = vc WITH constant("Intention"), protect
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE lrecsize = i4 WITH constant(size(request->group_list,5)), private
   IF (lrecsize > 0)
    FOR (i = 1 TO lrecsize)
      CALL verifynonzerocodevalue(request->group_list[i].concept_cd,lconcept_cs,sconcept_disp)
      CALL verifynonzerocodevalue(request->group_list[i].concept_group_cd,lconcept_grp_cs,
       sconcept_grp_disp)
      CALL verifynonzerocodevalue(request->group_list[i].intention_cd,lintention_cs,sintention_disp)
    ENDFOR
   ENDIF
 ...
 SUBROUTINE insertrows(null)
   ...
   DECLARE cp_concept_group_id = f8 WITH noconstant(0), protect
   ...
   ENDIF
   SET stat = alterlist(reply->group_list,size(request->group_list,5))
   FOR (x = 1 TO size(request->group_list,5))
     SET cp_concept_group_id = 0.0
     SELECT INTO "nl:"
      nextseqnum = seq(reference_seq,nextval)
      FROM dual
      DETAIL
       cp_concept_group_id = nextseqnum
      WITH nocounter
     ;end select
     CALL echo(build2("Next CP_CONCEPT_GROUP_ID:",cp_concept_group_id))
     IF (cp_concept_group_id=0.0)
      CALL reporterror("PERFORM","INSERT",
       " Unable to generate CP_CONCEPT_GROUP_ID from REFERENCE_SEQ.")
      ROLLBACK
      GO TO exit_script
     ENDIF
     INSERT  FROM cp_concept_group ccd
      SET ccd.concept_cd = request->group_list[x].concept_cd, ccd.concept_group_cd = request->
       group_list[x].concept_group_cd, ccd.intention_cd = request->group_list[x].intention_cd,
       ccd.cp_concept_group_id = cp_concept_group_id, ccd.rule_ind = request->group_list[x].rule_ind,
       ccd.updt_cnt = 0,
       ccd.updt_dt_tm = cnvtdatetime(sysdate), ccd.updt_applctx = reqinfo->updt_applctx, ccd
       .updt_task = reqinfo->updt_task,
       ccd.updt_id = reqinfo->updt_id
      WITH nocounter
     ;end insert
     CALL error_and_zero_check(curqual,"INSERT","CP_CONCEPT_GROUP",1,1)
     SET reply->group_list[x].concept_cd = request->group_list[x].concept_cd
     SET reply->group_list[x].concept_group_cd = request->group_list[x].concept_group_cd
     SET reply->group_list[x].intention_cd = request->group_list[x].intention_cd
     SET reply->group_list[x].rule_ind = request->group_list[x].rule_ind
     SET reply->group_list[x].cp_concept_group_id = cp_concept_group_id
   ENDFOR
