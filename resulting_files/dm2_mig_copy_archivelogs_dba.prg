CREATE PROGRAM dm2_mig_copy_archivelogs:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmca_dcl_status = i2 WITH protect, noconstant(0)
 DECLARE dmca_index = i4 WITH protect, noconstant(0)
 DECLARE dmca_iter = i4 WITH protect, noconstant(0)
 DECLARE dmca_iter2 = i4 WITH protect, noconstant(0)
 DECLARE dmca_dcl_cmd = vc WITH protect, noconstant("")
 DECLARE dmca_database_name = vc WITH protect, noconstant("")
 DECLARE dmca_curtime = dq8 WITH protect, noconstant(0.0)
 DECLARE dmca_file_loc = vc WITH protect, noconstant("")
 DECLARE dmca_submit_time = i4 WITH protect, noconstant(0)
 DECLARE dmca_retain_time = i4 WITH protect, noconstant(0)
 DECLARE dmca_arccom_retain_time = i4 WITH protect, noconstant(0)
 DECLARE dmca_last_timestamp = dq8 WITH protect, noconstant(0.0)
 DECLARE dmca_arch_log_format = vc WITH protect, noconstant("")
 DECLARE dmca_full_source_connect_string = vc WITH protect, noconstant("")
 DECLARE dmca_cpyarc_file_name = vc WITH protect, noconstant("")
 DECLARE dmca_str = vc WITH protect, noconstant("")
 DECLARE dmca_file_itr = i4 WITH protect, noconstant(0)
 DECLARE dmca_use_ftp = i2 WITH protect, noconstant(0)
 DECLARE dmca_use_seq_for_cleanup = i2 WITH protect, noconstant(0)
 DECLARE dmca_ftp_user = vc WITH protect, noconstant("")
 DECLARE dmca_ftp_pass = vc WITH protect, noconstant("")
 DECLARE dmca_ftp_node = vc WITH protect, noconstant("")
 DECLARE dmca_ignore_del_errors = i2 WITH protect, noconstant(0)
 DECLARE dmca_min_keep_hours = i2 WITH protect, noconstant(0)
 DECLARE dmca_interval = vc WITH protect, noconstant("")
 DECLARE dmca_ftp_com(dfc_check_string=vc,dfc_ignore_str=vc) = i2
 DECLARE dmca_add_ftp_cmd(dafc_cmd_in=vc,dafc_reset=i2) = i2
 DECLARE dmca_nfs_dir_com(null) = i2
 FREE RECORD arch_log_destinations
 RECORD arch_log_destinations(
   1 dest_cnt = i4
   1 qual_d[*]
     2 inst_id = i4
     2 dest = vc
 ...
 FREE RECORD dmca_recovery
 RECORD dmca_recovery(
   1 rec_cnt = i4
   1 rec[*]
     2 rec_thread = i4
     2 rec_sequence = i4
 )
 FREE RECORD dmca_arcdel
 RECORD dmca_arcdel(
   1 arc_cnt = i4
   1 arc[*]
     2 arc_name = vc
 ...
 IF (check_logfile("dm2_mig_copy_arch",".log","DM2_MIG_COPY_ARCHIVELOGS LOGFILE")=0)
  GO TO exit_script
 ENDIF
 ...
 IF ((((dmr_mig_data->src_v500_pwd="DM2NOTSET")) OR ((dmr_mig_data->src_v500_cnct_str="DM2NOTSET")))
  ...
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Could not load source connection data."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dmca_full_source_connect_string = concat("V500/",dmr_mig_data->src_v500_pwd,"@",dmr_mig_data->
  src_v500_cnct_str)
 SET dm_err->eproc = "Beginning dm2_mig_copy_archivelogs"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Determining current OS."
 IF (cursys != "AXP")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "dm2_mig_copy_archivelogs can only be used from a VMS node."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dmr_fill_email_list(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining current database name from v$database."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM v$database vd
  DETAIL
   dmca_database_name = cnvtupper(vd.name)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Obtain parameter and runtime information."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND ((di.info_name IN ("ARCH_SUBMIT_TIME", "TARGET_FTP_LOGIN", "NFSARCH_CLEANOUT_USE_SINCE",
  "MISC_FILE_LOCATION", "ARCH_RETAIN_TIME",
  "ARCH_COM_FILE_RETAIN_TIME", "ARCH_LAST_TIMESTAMP", "ARCH_IGNORE_DEL_ERRORS", "ARCH_MIN_KEEP_HOURS"
  )) OR (di.info_name="OVERRIDE_ARCHIVE_LOG_DEST*"))
  HEAD REPORT
   dmca_arccom_retain_time = 168, dmca_use_seq_for_cleanup = 1, arch_log_destinations->dest_cnt = 0,
   dmca_min_keep_hours = 120, stat = alterlist(arch_log_destinations->qual_d,0)
  DETAIL
   CASE (di.info_name)
    OF "ARCH_MIN_KEEP_HOURS":
     dmca_min_keep_hours = di.info_number
    OF "ARCH_SUBMIT_TIME":
     dmca_submit_time = di.info_number
    OF "TARGET_FTP_LOGIN":
     dmca_use_seq_for_cleanup = 1,dmca_use_ftp = 1,dmca_ftp_node = substring(1,(findstring("|||",di
       .info_char,1,0) - 1),di.info_char),
     dmca_ftp_user = substring((findstring("|||",di.info_char,1,0)+ 3),(findstring("|||",di.info_char,
       1,1) - (findstring("|||",di.info_char,1,0)+ 3)),di.info_char),dmca_ftp_pass = substring((
      findstring("|||",di.info_char,1,1)+ 3),textlen(di.info_char),di.info_char)
    OF "NFSARCH_CLEANOUT_USE_SINCE":
     dmca_use_seq_for_cleanup = 0
    OF "MISC_FILE_LOCATION":
     dmca_file_loc = di.info_char
    OF "ARCH_RETAIN_TIME":
     dmca_retain_time = di.info_number
    OF "ARCH_COM_FILE_RETAIN_TIME":
     dmca_arccom_retain_time = di.info_number
    OF "ARCH_LAST_TIMESTAMP":
     dmca_last_timestamp = di.info_date
    OF "ARCH_IGNORE_DEL_ERRORS":
     dmca_ignore_del_errors = 1
   ENDCASE
   IF (di.info_name="OVERRIDE_ARCHIVE_LOG_DEST*")
    arch_log_destinations->dest_cnt = (arch_log_destinations->dest_cnt+ 1), stat = alterlist(
     arch_log_destinations->qual_d,arch_log_destinations->dest_cnt), arch_log_destinations->qual_d[
    arch_log_destinations->dest_cnt].dest = di.info_char,
    arch_log_destinations->qual_d[arch_log_destinations->dest_cnt].inst_id = cnvtint(substring((
      textlen(trim(di.info_char)) - 1),1,di.info_char))
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF (dmca_use_ftp=0)
  SET dm_err->eproc = "Check that NFSARCH directory exists."
  CALL disp_msg("",dm_err->logfile,0)
  IF (dmr_check_directory(build("NFSARCH:[V500.DB_",dmca_database_name,".ARCH]"))=0)
   SET dm_err->emsg = build("NFSARCH:[V500.DB_",dmca_database_name,
    ".ARCH] is not available or does not have write privs.")
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((arch_log_destinations->dest_cnt=0))
  SET dm_err->eproc = "Querying for archive log destinations from gv$archive_dest."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM gv$archive_dest vad
   WHERE vad.status="VALID"
    AND vad.schedule="ACTIVE"
    AND vad.binding="MANDATORY"
   HEAD REPORT
    arch_log_destinations->dest_cnt = 0, stat = alterlist(arch_log_destinations->qual_d,0)
   DETAIL
    IF (locateval(dmca_index,1,arch_log_destinations->dest_cnt,vad.destination,arch_log_destinations
     ->qual_d[dmca_index].dest)=0)
     arch_log_destinations->dest_cnt = (arch_log_destinations->dest_cnt+ 1), stat = alterlist(
      arch_log_destinations->qual_d,arch_log_destinations->dest_cnt), arch_log_destinations->qual_d[
     arch_log_destinations->dest_cnt].dest = vad.destination,
     arch_log_destinations->qual_d[arch_log_destinations->dest_cnt].inst_id = vad.inst_id
    ENDIF
   WITH nocounter, nullreport
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Archive log destinations have been overridden to user-spedified values."
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(arch_log_destinations)
 ENDIF
 IF ((arch_log_destinations->dest_cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Archive logging is not set up properly."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Querying for log_archive_format from v$parameter."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM v$parameter vp
  WHERE vp.name="log_archive_format"
  DETAIL
   dmca_arch_log_format = vp.value, dmca_arch_log_format = concat(substring(1,(findstring("%",
      dmca_arch_log_format) - 1),dmca_arch_log_format),"*.",trim(substring((findstring(".",
       dmca_arch_log_format,1,1)+ 1),size(dmca_arch_log_format),dmca_arch_log_format)))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (((curqual=0) OR (dmca_arch_log_format="")) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to get log_archive_format or log_archive_format blank."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc =
 "Processing each archive log destination - retrieving archive log files yet to be copied."
 CALL disp_msg("",dm_err->logfile,0)
 SET dmca_curtime = cnvtdatetime(curdate,curtime3)
 SET arch_log_destinations->file_cnt = 0
 SET stat = alterlist(arch_log_destinations->qual_f,0)
 FOR (dmca_iter = 1 TO arch_log_destinations->dest_cnt)
   SET dmca_dcl_cmd = concat('DIR /COLUMNS=1 /SINCE="',format(dmca_last_timestamp,
     "DD-MMM-YYYY:HH:MM:SS.CC;;Q"),'" /BEFORE="',format(dmca_curtime,"DD-MMM-YYYY:HH:MM:SS.CC;;Q"),
    '" ',
    arch_log_destinations->qual_d[dmca_iter].dest,dmca_arch_log_format,";",char(42))
   CALL echo(dmca_dcl_cmd)
   IF (dm2_push_dcl(dmca_dcl_cmd)=0)
    IF ((dm_err->errtext="%DIRECT-W-NOFILES*"))
     SET dm_err->err_ind = 0
     SET dm_err->eproc = "The above error is acceptable."
     CALL disp_msg("",dm_err->logfile,0)
    ELSE
     GO TO exit_script
    ENDIF
   ...
   FREE DEFINE rtl
   DEFINE rtl build("CCLUSERDIR:",dm_err->errfile)
   SET dm_err->eproc = "Parsing dir results to determine archive logs that need to be copied."
   SELECT INTO "nl:"
    r.line
    FROM rtlt r
    DETAIL
     dmca_index = 0
     IF (trim(r.line,3) != ""
      AND cnvtupper(trim(r.line,3)) != "DIRECTORY *"
      AND trim(r.line,3) != "%*"
      AND trim(r.line,3) != "Total of*")
      IF (locateval(dmca_index,1,arch_log_destinations->file_cnt,trim(r.line,3),arch_log_destinations
       ->qual_f[dmca_index].file_name)=0)
       arch_log_destinations->file_cnt = (arch_log_destinations->file_cnt+ 1), stat = alterlist(
        arch_log_destinations->qual_f,arch_log_destinations->file_cnt), arch_log_destinations->
       qual_f[arch_log_destinations->file_cnt].dest = arch_log_destinations->qual_d[dmca_iter].dest,
       arch_log_destinations->qual_f[arch_log_destinations->file_cnt].file_name = trim(r.line,3)
      ENDIF
     ENDIF
    WITH nocounter, nullreport
   ;end select
 ...
 ENDIF
 SET dm_err->eproc =
 "For each archive log destination and archive logs yet to be copied, submit job to copy each one."
 CALL disp_msg("",dm_err->logfile,0)
 FOR (dmca_iter = 1 TO arch_log_destinations->file_cnt)
   SET dmca_str = substring(1,(findstring(";",arch_log_destinations->qual_f[dmca_iter].file_name) - 1
    ),arch_log_destinations->qual_f[dmca_iter].file_name)
   SET dmca_cpyarc_file_name = concat("dm2_cpyarc_",trim(replace(dmca_str,"0123456789","0123456789",3
      )),".com")
   SET dm_err->eproc = concat("Writing contents to ",dmca_cpyarc_file_name)
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO value(build(dmca_file_loc,dmca_cpyarc_file_name))
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     CALL print("$!"), row + 1,
     CALL print(concat("$! Copying archive log ",arch_log_destinations->qual_f[dmca_iter].file_name)),
     row + 1,
     CALL print("!"), row + 1,
     CALL print('$CCL:=="$CER_EXE:CCLORA.EXE"'), row + 1,
     CALL print("$CCL"),
     row + 1,
     CALL print("free define oraclesystem go"), row + 1,
     CALL print(concat("define oraclesystem '",dmca_full_source_connect_string,"' go")), row + 1,
     CALL print(concat("dm2_mig_copy_archivelog ","'",arch_log_destinations->qual_f[dmca_iter].dest,
      "', ","'",
      arch_log_destinations->qual_f[dmca_iter].file_name,"' go")),
     row + 1,
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = concat("Changing file permissions on ",dmca_file_loc,dmca_cpyarc_file_name)
   CALL disp_msg("",dm_err->logfile,0)
   IF (dm2_push_dcl(concat("SET FILE/PROT =(s:RWED, o:RWED, g:RWED, w:RWED) ",dmca_file_loc,
     dmca_cpyarc_file_name))=0)
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = concat("Purging old versions of ",dmca_file_loc,dmca_cpyarc_file_name)
   CALL disp_msg("",dm_err->logfile,0)
   IF (dm2_push_dcl(concat("PURGE ",dmca_file_loc,dmca_cpyarc_file_name))=0)
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = concat("Adding file to queue: ",dmca_file_loc,dmca_cpyarc_file_name)
   CALL disp_msg("",dm_err->logfile,0)
   SET dmca_dcl_cmd = concat("SUBMIT /QUEUE=",dmr_batch_queue," /NAME=",dmca_cpyarc_file_name," ",
    dmca_file_loc,dmca_cpyarc_file_name," /NOLOG_FILE")
   CALL echo(dmca_dcl_cmd)
   IF (dm2_push_dcl(dmca_dcl_cmd)=0)
    GO TO exit_script
   ENDIF
 ENDFOR
 IF (dmca_use_seq_for_cleanup=1)
  SET dm_err->eproc = "Query for recovery sequence for GoldenGate Capture process."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   ...
   WHERE d.info_domain="DM2MIG_CAPDATA"
    AND d.info_name="RECOVERY THREAD:*"
   HEAD REPORT
    dmca_recovery->rec_cnt = 0, stat = alterlist(dmca_recovery->rec,dmca_recovery->rec_cnt)
   DETAIL
    dmca_recovery->rec_cnt = (dmca_recovery->rec_cnt+ 1), stat = alterlist(dmca_recovery->rec,
     dmca_recovery->rec_cnt), dmca_recovery->rec[dmca_recovery->rec_cnt].rec_thread = cnvtint(
     substring(textlen(trim(d.info_name)),1,d.info_name)),
    dmca_recovery->rec[dmca_recovery->rec_cnt].rec_sequence = d.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echorecord(dmca_recovery)
  ENDIF
 ENDIF
 IF (dmca_use_seq_for_cleanup=0)
  SET dm_err->eproc = "Remove files from NFSARCH that have reached their retension time."
  CALL disp_msg("",dm_err->logfile,0)
  SET dmca_dcl_cmd = concat('DELETE /BEFORE="',format(cnvtlookbehind(build(dmca_retain_time,",H")),
    "DD-MMM-YYYY:HH:MM:SS.CC;;Q"),'" NFSARCH:[V500.DB_',dmca_database_name,".ARCH]",
   dmca_arch_log_format,";",char(42))
  CALL echo(dmca_dcl_cmd)
  IF (dm2_push_dcl(dmca_dcl_cmd)=0)
   IF ((dm_err->errtext="%DELETE-W-SEARCHFAIL*"))
    SET dm_err->err_ind = 0
    SET dm_err->eproc = "The above error is acceptable."
    CALL disp_msg("",dm_err->logfile,0)
   ELSE
  ...
  ENDIF
 ELSE
  IF ((dmca_recovery->rec_cnt=0))
   SET dm_err->eproc =
   "Bypassing NFSARCH cleanup since recovery checkpoint information not available."
   CALL disp_msg("",dm_err->logfile,0)
  ELSE
   SET dm_err->eproc = "Gather sequence numbers of archived logs under the recovery point."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET dmca_interval = build(dmca_min_keep_hours,",H")
   SELECT INTO "nl:"
    FROM v$archived_log v,
     (dummyt d  WITH seq = value(dmca_recovery->rec_cnt))
    PLAN (d)
     JOIN (v
     WHERE (v.thread#=dmca_recovery->rec[d.seq].rec_thread)
      AND (v.sequence# < dmca_recovery->rec[d.seq].rec_sequence)
      AND v.completion_time < cnvtdatetimeutc(cnvtdatetime(cnvtlookbehind(dmca_interval))))
    HEAD REPORT
     stat = alterlist(dmca_arcdel->arc,0), dmca_arcdel->arc_cnt = 0
    DETAIL
     dmca_arcdel->arc_cnt = (dmca_arcdel->arc_cnt+ 1)
     IF (mod(dmca_arcdel->arc_cnt,100)=1)
      stat = alterlist(dmca_arcdel->arc,(dmca_arcdel->arc_cnt+ 99))
     ENDIF
     dmca_arcdel->arc[dmca_arcdel->arc_cnt].arc_name = substring((findstring("]",v.name,1,1)+ 1),
      textlen(v.name),v.name), dmca_arcdel->arc[dmca_arcdel->arc_cnt].del_ind = 0
    FOOT REPORT
     stat = alterlist(dmca_arcdel->arc,dmca_arcdel->arc_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF ((dm_err->debug_flag > 3))
    CALL echorecord(dmca_arcdel)
   ENDIF
   IF (dmca_use_ftp=1)
    SET dm_err->eproc = "Gather file names from target NFSARCH via FTP."
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
    CALL dmca_add_ftp_cmd("",1)
    CALL dmca_add_ftp_cmd(concat("cd /nfsarch/v500/db_",cnvtlower(dmca_database_name),"/arch/"),0)
    CALL dmca_add_ftp_cmd("dir /brief",0)
    IF ((dmr_mig_data->tgt_db_os="LNX"))
     IF (dmca_ftp_com(notrim("226 "),notrim("550 "))=0)
      GO TO exit_script
     ENDIF
    ELSE
     IF (dmca_ftp_com(notrim("226 "),notrim("550 "))=0)
    ...
    ENDIF
   ELSE
    IF (dmca_nfs_dir_com(null)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   SET dm_err->eproc = "Set which archive logs may be deleted from target /nfsarch."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(dmca_cmd->out_cnt))
    DETAIL
     dmca_index = 0
     IF (findstring(";",dmca_cmd->out[d.seq].out_line,1,1) > 0)
      dmca_str = substring(1,(findstring(";",dmca_cmd->out[d.seq].out_line,1,1) - 1),dmca_cmd->out[d
       .seq].out_line)
     ELSE
      dmca_str = dmca_cmd->out[d.seq].out_line
     ENDIF
     IF (locateval(dmca_index,1,dmca_arcdel->arc_cnt,dmca_str,dmca_arcdel->arc[dmca_index].arc_name)
      > 0)
      dmca_arcdel->arc[dmca_index].del_ind = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   IF ((dm_err->debug_flag > 3))
    CALL echorecord(dmca_arcdel)
   ENDIF
   SET dm_err->eproc = "Remove files which are no longer required by CAPTURE from nfsarch."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   IF (dmca_use_ftp=1)
    IF (locateval(dmca_index,1,dmca_arcdel->arc_cnt,1,dmca_arcdel->arc[dmca_index].del_ind) > 0)
     CALL dmca_add_ftp_cmd("",1)
     CALL dmca_add_ftp_cmd(concat("cd /nfsarch/v500/db_",cnvtlower(dmca_database_name),"/arch/"),0)
     FOR (dmca_iter = 1 TO dmca_arcdel->arc_cnt)
       IF ((dmca_arcdel->arc[dmca_iter].del_ind=1))
        CALL dmca_add_ftp_cmd(concat('delete "',dmca_arcdel->arc[dmca_iter].arc_name,'"'),0)
       ENDIF
     ENDFOR
     IF ((dmr_mig_data->tgt_db_os="LNX"))
      IF (dmca_ftp_com(notrim("250 "),"")=0)
       GO TO exit_script
      ENDIF
     ELSE
      IF (dmca_ftp_com(notrim("250 "),"")=0)
   ...
   ELSE
    FOR (dmca_iter = 1 TO dmca_arcdel->arc_cnt)
      IF ((dmca_arcdel->arc[dmca_iter].del_ind=1))
       IF (dm2_push_dcl(concat("delete NFSARCH:[V500.DB_",dmca_database_name,".ARCH]",dmca_arcdel->
         arc[dmca_iter].arc_name,";\*"))=0)
        IF (dmca_ignore_del_errors=0)
         GO TO exit_script
        ELSE
         SET dm_err->err_ind = 0
        ENDIF
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Remove com files from CCLUSERDIR that have reached their retension time."
 CALL disp_msg("",dm_err->logfile,0)
 SET dmca_dcl_cmd = concat('DELETE /BEFORE="',format(cnvtlookbehind(build(dmca_arccom_retain_time,
     ",H")),"DD-MMM-YYYY:HH:MM:SS.CC;;Q"),'" CCLUSERDIR:dm2_cpyarc_',char(42),".",
  char(42),";",char(42))
 CALL echo(dmca_dcl_cmd)
 IF (dm2_push_dcl(dmca_dcl_cmd)=0)
  IF ((dm_err->errtext="%DELETE-W-SEARCHFAIL*"))
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Updating existing last timestamp into dm_info."
 CALL disp_msg("",dm_err->logfile,0)
 UPDATE  FROM dm_info di
  SET di.info_date = cnvtdatetime(dmca_curtime)
  WHERE di.info_domain="DM2_MIG_DATA"
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Querying dm_info to see if job should not be resubmitted."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name="STOP_ARCHIVE_COPY"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual > 0)
  SET dm_err->eproc = "Deleting STOP_ARCHIVE_COPY row in dm_info."
  CALL disp_msg("",dm_err->logfile,0)
  DELETE  FROM dm_info di
   WHERE di.info_domain="DM2_MIG_DATA"
    AND di.info_name="STOP_ARCHIVE_COPY"
  ...
  COMMIT
 ELSE
  SET dm_err->eproc = "Submitting dm2_mig_copy_archivelogs to batch queue."
  CALL disp_msg("",dm_err->logfile,0)
  IF (dm2_push_dcl(concat("SUBMIT /QUEUE=",dmr_batch_queue," /NAME=dm2_mig_copy_archivelogs ",
    dmca_file_loc,"dm2_mig_copy_archivelogs.com /NOLOG_FILE",
    ' /AFTER="+0-00:',format(build(dmca_submit_time),"##;RP0"),'" /RESTART'))=0)
   GO TO exit_script
  ENDIF
 ENDIF
 GO TO exit_script
 SUBROUTINE dmca_nfs_dir_com(null)
   DECLARE dndc_outfile = vc WITH protect, noconstant("")
   DECLARE dndc_cnt = i4 WITH protect, noconstant(0)
   DECLARE dndc_os_cmd = vc WITH protect, noconstant(" ")
   DECLARE dndc_status = i2 WITH protect, noconstant(0)
   IF (get_unique_file("dm2arcout",".out")=0)
    RETURN(0)
   ELSE
    SET dndc_outfile = dm_err->unique_fname
   ENDIF
   SET dndc_os_cmd = concat("pipe dir /brief /columns =1 NFSARCH:[V500.DB_",dmca_database_name,
    ".ARCH] > ",dndc_outfile)
   SET dm_err->eproc = concat("Executing DIR cmd: ",dndc_os_cmd)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   CALL dcl(dndc_os_cmd,size(dndc_os_cmd),dndc_status)
   IF (((dndc_status=0) OR (dm2_findfile(dndc_outfile)=0)) )
    SET dm_err->emsg = concat("Error executing DIR command: ",dndc_os_cmd,". Outfile:",dndc_outfile)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   FREE DEFINE rtl2
   FREE SET dndc_out_file_loc
   SET logical dndc_out_file_loc value(dndc_outfile)
   DEFINE rtl2 "dndc_out_file_loc"
   SET dm_err->eproc = concat("Loading contents of outfile: ",dndc_outfile)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET dmca_cmd->out_cnt = 0
   SET stat = alterlist(dmca_cmd->out,0)
   SELECT INTO "nl:"
    r.line
    FROM rtl2t r
    WHERE r.line > " "
    DETAIL
     IF ((dm_err->debug_flag > 1))
      CALL echo(concat("TEXT = ",r.line))
     ENDIF
     IF (findstring(cnvtlower(concat("NFSARCH:[V500.DB_",dmca_database_name,".ARCH]")),cnvtlower(r
       .line),1,1)=0
      AND findstring(".arc",cnvtlower(r.line),1,1) > 0)
      dmca_cmd->out_cnt = (dmca_cmd->out_cnt+ 1), stat = alterlist(dmca_cmd->out,dmca_cmd->out_cnt),
      dmca_cmd->out[dmca_cmd->out_cnt].out_line = r.line
     ENDIF
    WITH nocounter
   ;end select
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dmca_cmd)
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ...
   IF ((dm_err->debug_flag=0)
    ...
    SET dm_err->eproc = "Cleanup DIR listing output file."
    IF (dm2_push_dcl(concat("delete ",dndc_outfile,";\*"))=0)
     SET dm_err->err_ind = 0
     CALL disp_msg("Error is ignorable.",dm_err->logfile,0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmca_ftp_com(dfc_check_string,dfc_ignore_str)
   DECLARE dfc_com_file = vc WITH protect, noconstant(" ")
   DECLARE dfc_cnt = i4 WITH protect, noconstant(0)
   DECLARE dfc_os_cmd = vc WITH protect, noconstant(" ")
   DECLARE dfc_status = i2 WITH protect, noconstant(0)
   DECLARE dfc_success_ind = i2 WITH protect, noconstant(0)
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dmca_cmd)
    CALL echo(concat("x",dfc_check_string,"x"))
    CALL echo(concat("x",dfc_ignore_str,"x"))
   ENDIF
   IF (get_unique_file("dm2ftpcmd",".com")=0)
    RETURN(0)
   ELSE
    SET dfc_com_file = dm_err->unique_fname
    SET dfc_outfile = replace(dfc_com_file,"dm2ftpcmd","dm2ftpout",1)
    SET dfc_outfile = replace(dfc_com_file,".com",".out",1)
   ENDIF
   SET dm_err->eproc = concat("Writing com script for FTP: ",dfc_com_file)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(dfc_com_file)
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     CALL print("FTP"), row + 1,
     CALL print(concat("connect ",dmca_ftp_node)),
     row + 1,
     CALL print(dmca_ftp_user), row + 1,
     CALL print(dmca_ftp_pass), row + 1,
     CALL print("set type image"),
     row + 1
     FOR (dfc_cnt = 1 TO dmca_cmd->cmd_cnt)
      CALL print(dmca_cmd->com[dfc_cnt].cmd_line),row + 1
     ENDFOR
     CALL print("quit"), row + 1
    WITH nocounter, maxcol = 1000, format = variable,
     maxrow = 1
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
   SET dfc_os_cmd = concat("pipe @",dfc_com_file," > ",dfc_outfile)
   SET dm_err->eproc = concat("Executing FTP cmd: ",dfc_os_cmd)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   CALL dcl(dfc_os_cmd,size(dfc_os_cmd),dfc_status)
   IF (dm2_findfile(dfc_outfile)=0)
    SET dm_err->emsg = concat("Error executing FTP script: ",dfc_com_file,". Outfile:",dfc_outfile)
    CALL echorecord(dmca_cmd)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   FREE DEFINE rtl2
   FREE SET dfc_out_file_loc
   SET logical dfc_out_file_loc value(dfc_outfile)
   DEFINE rtl2 "dfc_out_file_loc"
   SET dm_err->eproc = concat("Loading contents of outfile: ",dfc_outfile)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET dmca_cmd->out_cnt = 0
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     dmca_cmd->out_cnt = (dmca_cmd->out_cnt+ 1), stat = alterlist(dmca_cmd->out,dmca_cmd->out_cnt),
     dmca_cmd->out[dmca_cmd->out_cnt].out_line = r.line
     IF (findstring(cnvtlower(dfc_check_string),cnvtlower(r.line),1,1) > 0)
      dfc_success_ind = 1
     ENDIF
     IF (dfc_ignore_str > "")
      IF (findstring(cnvtlower(dfc_ignore_str),cnvtlower(r.line),1,1) > 0)
       dfc_success_ind = 1
      ENDIF
     ENDIF
    WITH nocounter
   ...
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dmca_cmd)
   ENDIF
   IF (dfc_success_ind=0
    AND dmca_ignore_del_errors=0)
    SET dm_err->emsg = concat('Outfile does not contain success string "',dfc_check_string,
     '". Outfile:',dfc_outfile)
    SET dm_err->err_ind = 1
    CALL echorecord(dmca_cmd)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF ((dm_err->debug_flag < 2)
    AND (dm_err->err_ind=0))
    SET dm_err->eproc = "Cleanup ftp scipt and outfile"
    IF (dm2_push_dcl(concat("delete ",dfc_com_file,";\*"))=0)
     SET dm_err->err_ind = 0
     CALL disp_msg("Error is ignorable.",dm_err->logfile,0)
    ENDIF
    IF (dm2_push_dcl(concat("delete ",dfc_outfile,";\*"))=0)
     SET dm_err->err_ind = 0
     CALL disp_msg("Error is ignorable.",dm_err->logfile,0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmca_add_ftp_cmd(dafc_cmd_in,dafc_reset)
   IF (dafc_reset=1)
    SET dmca_cmd->cmd_cnt = 0
    SET stat = alterlist(dmca_cmd->com,0)
    RETURN(1)
   ENDIF
   SET dmca_cmd->cmd_cnt = (dmca_cmd->cmd_cnt+ 1)
   SET stat = alterlist(dmca_cmd->com,dmca_cmd->cmd_cnt)
   SET dmca_cmd->com[dmca_cmd->cmd_cnt].cmd_line = dafc_cmd_in
   RETURN(1)
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "Ending dm2_mig_copy_archivelogs."
 CALL final_disp_msg("dm2_mig_copy_arch")
 IF (cursys="AXP")
  IF (size(dmr_emails->email_list) > 1
   AND (dm_err->err_ind=1))
   SET dm_err->err_ind = 0
   CALL dmr_send_email(
    "DATABASE MIGRATION : Process to copy archive logfiles failed.  Archive Log Copy Job Stopped!",
    dmr_emails->email_list,dm_err->logfile)
  ENDIF
  IF (dmca_file_loc != logical("CCLUSERDIR"))
   SET dm_err->err_ind = 0
   CALL dm2_push_dcl(concat("COPY ",dm_err->logfile," ",dmca_file_loc))
   SET dm_err->err_ind = 0
   CALL dm2_push_dcl(concat("DELETE ",dm_err->logfile,";*"))
  ENDIF
  SET dmca_dcl_cmd = concat("DELETE ",dm_err->errfile,";*")
  IF ((dm_err->debug_flag > 0))
   CALL echo(dmca_dcl_cmd)
  ENDIF
  CALL dcl(dmca_dcl_cmd,size(dmca_dcl_cmd),dmca_dcl_status)
 ENDIF
END GO
