CREATE PROGRAM dm_ecmb_cdi_pending_document:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_CDI_PENDING_DOCUMENT"
 ...
 SELECT INTO "nl:"
  ...
  WHERE (((frm.encntr_id=request->xxx_combine[icombine].from_xxx_id)) OR ((frm.parent_entity_id=
  request->xxx_combine[icombine].from_xxx_id)
   AND frm.parent_entity_name="ENCOUNTER"))
  DETAIL
   v_cust_count1 += 1
   ...
   v_cust_count1].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count1].encntr_id = frm
   .encntr_id,
 ...
 FOR (v_cust_loopcount = 1 TO v_cust_count1)
  IF ((rreclist->from_rec[v_cust_loopcount].encntr_id=request->xxx_combine[icombine].from_xxx_id))
   IF (upt_from_enctr(rreclist->from_rec[v_cust_loopcount].from_id,request->xxx_combine[icombine].
    to_xxx_id)=0)
    GO TO exit_sub
   ENDIF
  ENDIF
  ...
  from_xxx_id)
   AND (rreclist->from_rec[v_cust_loopcount].parent_entity_name="ENCOUNTER"))
  ...
  ENDIF
 ENDFOR
 SUBROUTINE (upt_from_enctr(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
