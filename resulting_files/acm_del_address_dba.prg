CREATE PROGRAM acm_del_address:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (a
   WHERE (a.address_id=acm_request->address_qual[xref->del[d.seq].idx].address_id))
  WITH nocounter, status(reply->address_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->address_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    FROM address a
    WHERE expand(t1,f_val,t_val,a.address_id,acm_request->address_qual[xref->del[t1].idx].address_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,a.address_id,acm_request->address_qual[xref->del[t1].idx].
      address_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->address_qual[xref->del[index].idx].status=0))
