CREATE PROGRAM dm_ins_dm_ref_domain_group:dba
 SET c_mod = "DM_INS_DM_REF_DOMAIN_GROUP 000"
 SET debug_ind = 0
 IF ( NOT (validate(i_debug_ind,0)=0
  AND validate(i_debug_ind,1)=1))
  SET debug_ind = i_debug_ind
 ENDIF
 SET csv_str = "refdomaingrp.csv"
 ...
 IF (qual_cnt > 0)
  FOR (mrg_cnt = 1 TO qual_cnt)
    UPDATE  FROM dm_ref_domain_group dg
     SET dg.description = cnvtupper(requestin->list_0[mrg_cnt].description)
     WHERE dg.group_name=cnvtupper(requestin->list_0[mrg_cnt].group_name)
     WITH nocounter
    ;end update
    COMMIT
    IF (curqual=0)
     INSERT  FROM dm_ref_domain_group dg
      SET dg.group_name = cnvtupper(requestin->list_0[mrg_cnt].group_name), dg.description =
       cnvtupper(requestin->list_0[mrg_cnt].description)
      WITH nocounter
     ;end insert
     COMMIT
    ENDIF
