CREATE PROGRAM bhs_ops_fax_powernotes2:dba
 PROMPT
 ...
 SET ms_xml_filename = concat("bhs_ops_fax_",cnvtlower(ms_mode),trim(format(sysdate,"mmddyyhhmmss;;d"
    )),".xml")
 IF (gl_bhs_prod_flag=0)
  SET mn_test = 1
  SET ms_ftp_path = "ciscore\NOTES_FAXING\NONPROD"
 ELSE
  SET ms_ftp_path = "ciscore\NOTES_FAXING\PROD"
 ENDIF
  ...
  IF (curqual > 0)
   ...
   IF (ms_recipients="FTP")
    IF (((gl_bhs_prod_flag=1) OR (gs_bhs_domain_name="READ")) )
     ...
     SET status = 0
     CALL dcl(ms_dclcom_str,len,status)
     SET ms_dclcom_str = concat("$cust_script/bhs_ftp_file.ksh ",ms_xml_filename,
      " transfer.baystatehealth.org 'bhs\cisftp' C!sftp01 '",'"',ms_ftp_path,
      '"',"'")
     SET len = textlen(trim(ms_dclcom_str))
     SET status = 0
    ...
    ELSE
     ...
     SET status = 0
     CALL dcl(ms_dclcom_str,len,status)
     SET ms_dclcom_str = concat("$bhscust/bhs_ftp_file.ksh ",ms_xml_filename,
      " transfer.baystatehealth.org 'bhs\cisftp' C!sftp01 '",'"',ms_ftp_path,
      '"',"'")
     SET len = textlen(trim(ms_dclcom_str))
     SET status = 0
