CREATE PROGRAM acm_chg_encntr_org_reltn:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->encntr_org_reltn_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   encntr_org_reltn e
  SET e.encntr_org_reltn_id = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].
   encntr_org_reltn_id, e.beg_effective_dt_tm = cnvtdatetime(acm_request->encntr_org_reltn_qual[xref
    ->chg[d.seq].idx].beg_effective_dt_tm), e.contributor_system_cd =
   IF ((acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   e.encntr_id =
   IF ((acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].encntr_id > 0)) acm_request->
    encntr_org_reltn_qual[xref->chg[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].encntr_idx].
    encntr_id
   ENDIF
   , e.encntr_org_alias = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].encntr_org_alias,
   e.encntr_org_nbr = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].encntr_org_nbr,
   e.encntr_org_reltn_cd = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].
   encntr_org_reltn_cd, e.encntr_org_reltn_type_cd = acm_request->encntr_org_reltn_qual[xref->chg[d
   .seq].idx].encntr_org_reltn_type_cd, e.end_effective_dt_tm = cnvtdatetime(acm_request->
    encntr_org_reltn_qual[xref->chg[d.seq].idx].end_effective_dt_tm),
   e.free_text_ind = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].free_text_ind, e
   .ft_org_name = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].ft_org_name, e
   .internal_seq = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].internal_seq,
   e.organization_id = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].organization_id, e
   .priority_seq = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].priority_seq, e
   .research_account_id = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].
   research_account_id,
   e.active_ind = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].active_ind, e
   .active_status_cd = acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].active_status_cd, e
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_org_reltn_id=acm_request->encntr_org_reltn_qual[xref->chg[d.seq].idx].
   encntr_org_reltn_id))
  WITH nocounter, status(reply->encntr_org_reltn_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->encntr_org_reltn_qual[xref->chg[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM encntr_org_reltn e
    WHERE expand(t1,f_val,t_val,e.encntr_org_reltn_id,acm_request->encntr_org_reltn_qual[xref->chg[t1
     ].idx].encntr_org_reltn_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,e.encntr_org_reltn_id,acm_request->encntr_org_reltn_qual[xref->
      chg[t1].idx].encntr_org_reltn_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((e.updt_cnt=acm_request->encntr_org_reltn_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->encntr_org_reltn_qual[idx].status = - (1), reply->encntr_org_reltn_qual[idx].
      encntr_org_reltn_id = acm_request->encntr_org_reltn_qual[idx].encntr_org_reltn_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->encntr_org_reltn_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].beg_effective_dt_tm = e.beg_effective_dt_tm
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].contributor_system_cd = e.contributor_system_cd
     ENDIF
     IF (findstring("ENCNTR_ID,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].encntr_id = e.encntr_id
     ENDIF
     IF (findstring("ENCNTR_ORG_ALIAS,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].encntr_org_alias = e.encntr_org_alias
     ENDIF
     IF (findstring("ENCNTR_ORG_NBR,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].encntr_org_nbr = e.encntr_org_nbr
     ENDIF
     IF (findstring("ENCNTR_ORG_RELTN_CD,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].encntr_org_reltn_cd = e.encntr_org_reltn_cd
     ENDIF
     IF (findstring("ENCNTR_ORG_RELTN_TYPE_CD,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].encntr_org_reltn_type_cd = e.encntr_org_reltn_type_cd
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].end_effective_dt_tm = e.end_effective_dt_tm
     ENDIF
     IF (findstring("FREE_TEXT_IND,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].free_text_ind = e.free_text_ind
     ENDIF
     IF (findstring("FT_ORG_NAME,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].ft_org_name = e.ft_org_name
     ENDIF
     IF (findstring("INTERNAL_SEQ,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].internal_seq = e.internal_seq
     ENDIF
     IF (findstring("ORGANIZATION_ID,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].organization_id = e.organization_id
     ENDIF
     IF (findstring("PRIORITY_SEQ,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].priority_seq = e.priority_seq
     ENDIF
     IF (findstring("RESEARCH_ACCOUNT_ID,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].research_account_id = e.research_account_id
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].active_ind = e.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->encntr_org_reltn_qual[idx].active_status_cd = e.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->encntr_org_reltn_qual[xref->chg[index].idx].status=0))
