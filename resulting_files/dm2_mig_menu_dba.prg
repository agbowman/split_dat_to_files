CREATE PROGRAM dm2_mig_menu:dba
 ...
 SUBROUTINE dm2_rename_login_default(sbr_rld_mode)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmm_setup_cont = i2 WITH protect, noconstant(0)
 DECLARE dmm_archive_log_copy_cont = i2 WITH protect, noconstant(0)
 DECLARE dmm_monitoring_cont = i2 WITH protect, noconstant(0)
 DECLARE dmm_reports_cont = i2 WITH protect, noconstant(0)
 DECLARE dmm_background_mon_cont = i2 WITH protect, noconstant(0)
 DECLARE dmm_file_to_display = vc WITH protect, noconstant("")
 DECLARE dmm_ln_prompt_ind = i2 WITH protect, noconstant(1)
 DECLARE dmm_lines_to_view = vc WITH protect, noconstant("NOT_SET")
 DECLARE dmm_ln_int = i4 WITH protect, noconstant(0)
 DECLARE dmm_created_date = f8 WITH protect, noconstant(0.0)
 DECLARE dmm_db_name = vc WITH protect, noconstant("")
 DECLARE dmm_vdata_rpt_cont = i2 WITH protect, noconstant(0)
 DECLARE dmm_vdata_rpt_dtl_cont = i2 WITH protect, noconstant(0)
 DECLARE dmm_msg = vc WITH protect, noconstant(" ")
 DECLARE dmm_vdata_get_cur_max = i2 WITH protect, noconstant(1)
 DECLARE dmm_email_setup(null) = i2
 DECLARE dmm_send_test_emails(null) = i2
 DECLARE dmm_display_report(report_file=vc,report_lines=vc) = i2
 DECLARE dmm_show_job_status(job_type=vc) = i2
 DECLARE dmm_utc_setup(null) = i2
 IF (check_logfile("dm2_mig_menu",".log","dm2_mig_menu LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_menu"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Determining if you are connected to a database."
 IF (size(trim(currdbhandle,3))=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "You must be connected to either SOURCE or TARGET to run dm2_mig_menu."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET width = 132
 ...
 WHILE (true)
   ...
   CALL clear(1,1)
   CALL box(1,1,24,131)
   CALL text(2,2,"Database Migration Process")
   CALL text(4,10,"Main Menu")
   CALL text(6,10,"1. Setup")
   CALL text(8,10,"2. Monitoring")
   CALL text(10,10,"3. Reporting")
   CALL text(12,11,"Your Selection (0 to Exit)?")
   CALL accept(12,39,"9;",0
    WHERE curaccept IN (0, 1, 2, 3))
   CASE (curaccept)
    OF 0:
     GO TO exit_script
    OF 1:
     SET dmm_setup_cont = 1
     WHILE (dmm_setup_cont=1)
       SET width = 132
       IF ((dm_err->debug_flag != 722))
        SET message = window
       ENDIF
       ...
       CALL box(1,1,24,131)
       CALL text(2,2,"Database Migration Process")
       CALL text(4,10,"Setup Menu")
       CALL text(6,10,"1. Gather Connection Information")
       CALL text(7,10,"2. Batch Queue Setup ")
       CALL text(8,10,"3. Archive Log Copy ")
       CALL text(9,10,"4. Setup Data Verification ")
       CALL text(10,10,"5. Goldengate Directory Setup ")
       CALL text(11,10,"6. UTC Conversion Setup ")
       CALL text(14,11,"Your Selection (0 to Exit)?")
       IF (dmm_msg > " ")
        CALL text(16,11,dmm_msg)
        SET dmm_msg = " "
       ENDIF
       CALL accept(14,39,"9;",0
        WHERE curaccept IN (0, 1, 2, 3, 4,
        5, 6))
       CASE (curaccept)
        OF 0:
         SET dmm_setup_cont = 0
        OF 1:
         IF (dmr_get_db_info(dmm_db_name,dmm_created_date)=0)
          SET message = nowindow
          GO TO exit_script
         ENDIF
         EXECUTE dm2_mig_setup_info
         IF ((dm_err->err_ind > 0))
          GO TO exit_script
         ENDIF
         IF ((dm_err->debug_flag=722))
          CALL echorecord(dmr_mig_data)
          CALL echo(dmm_db_name)
          CALL echo(dmm_created_date)
         ENDIF
         IF (dmm_db_name=cnvtupper(dmr_mig_data->src_db_name)
          AND (dmm_created_date=dmr_mig_data->src_created_date))
          IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
           GO TO exit_script
          ENDIF
         ELSEIF (dmm_db_name=cnvtupper(dmr_mig_data->tgt_db_name)
          AND (dmm_created_date=dmr_mig_data->tgt_created_date))
          IF (dmr_prompt_connect_data("TARGET","V500","CO")=0)
           GO TO exit_script
          ENDIF
         ELSE
        ...
        OF 2:
         ...
         ENDIF
         IF ((dm2_sys_misc->cur_os != "AXP"))
          SET dmm_msg = concat("Batch Queue Setup not necessary on Source having ",dm2_sys_misc->
           cur_os," operating system.")
         ELSE
          SET message = nowindow
          IF (dmr_setup_batch_queue(dmr_batch_queue)=0)
           GO TO exit_script
          ELSE
           SET dmm_msg = concat("Batch Queue Setup completed successfully.")
          ENDIF
         ENDIF
        OF 3:
         IF (dmr_get_gg_dir(null)=0)
          GO TO exit_script
         ENDIF
         IF (dmr_prompt_retrieve_mig_data(0,1,0,0)=0)
          GO TO exit_script
         ...
         IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
          ...
          ENDIF
         ENDIF
         SET dmm_archive_log_copy_cont = 1
         WHILE (dmm_archive_log_copy_cont=1)
           SET width = 132
           IF ((dm_err->debug_flag != 722))
            SET message = window
           ENDIF
           ...
           CALL box(1,1,24,131)
           CALL text(2,2,"Database Migration Process")
           CALL text(4,10,"Archive Log Copy Menu")
           CALL text(6,10,"1. Setup Email Addresses")
           CALL text(8,10,"2. Setup & Submit Job")
           CALL text(10,10,"3. Status of Job")
           CALL text(12,10,"4. Delete Job")
           CALL text(14,11,"Your Selection (0 to Exit)?")
           CALL accept(14,39,"9;",0
            WHERE curaccept IN (0, 1, 2, 3, 4))
           CASE (curaccept)
            OF 0:
             SET dmm_archive_log_copy_cont = 0
            OF 1:
             IF (dmm_email_setup(null)=0)
              GO TO exit_script
             ENDIF
            OF 2:
             EXECUTE dm2_mig_submit_archive_copy
             IF ((dm_err->err_ind=1))
              GO TO exit_script
             ENDIF
            OF 3:
             IF (dmm_show_job_status("ARCH")=0)
              GO TO exit_script
             ENDIF
            OF 4:
             IF (dmr_stop_job("ARCH",1)=0)
              GO TO exit_script
             ENDIF
           ENDCASE
         ENDWHILE
        OF 4:
         SET message = nowindow
         EXECUTE dm2_mig_verify_data_setup
         IF ((dm_err->err_ind=1))
          GO TO exit_script
         ENDIF
        OF 5:
         ...
         IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
          ...
          ENDIF
         ENDIF
         IF (dmr_mig_setup_gg_dir(null)=0)
          GO TO exit_script
         ENDIF
        OF 6:
         IF (dmr_prompt_retrieve_mig_data(0,1,0,0)=0)
    ...
    OF 2:
     IF (dmr_get_gg_dir(null)=0)
      GO TO exit_script
     ENDIF
     SET dmm_monitoring_cont = 1
     WHILE (dmm_monitoring_cont=1)
       SET width = 132
       IF ((dm_err->debug_flag != 722))
        SET message = window
       ENDIF
       ...
       CALL box(1,1,24,131)
       CALL text(2,2,"Database Migration Process")
       CALL text(4,10,"Monitoring Menu")
       CALL text(6,10,"1. Reports")
       CALL text(8,10,"2. Background Monitoring")
       CALL text(10,11,"Your Selection (0 to Exit)?")
       CALL accept(10,39,"9;",0
        WHERE curaccept IN (0, 1, 2))
       CASE (curaccept)
        OF 0:
         SET dmm_monitoring_cont = 0
        OF 1:
         SET dmm_reports_cont = 1
         WHILE (dmm_reports_cont=1)
           SET width = 132
           IF ((dm_err->debug_flag != 722))
            SET message = window
           ENDIF
           ...
           CALL box(1,1,24,131)
           CALL text(2,2,"Database Migration Process")
           CALL text(4,10,"Reports Menu")
           CALL text(6,10,concat("1. View Capture Manager Report ",evaluate(dmr_mig_data->
              report_capture,0,"**UNAVAILABLE**","")))
           CALL text(8,10,concat("2. View Capture Report ",evaluate(dmr_mig_data->report_capture,0,
              "**UNAVAILABLE**","")))
           CALL text(10,10,concat("3. View Capture Error Report ",evaluate(dmr_mig_data->
              report_capture,0,"**UNAVAILABLE**","")))
           CALL text(12,10,concat("4. View Delivery Manager Report ",evaluate(dmr_mig_data->
              report_delivery,0,"**UNAVAILABLE**","")))
           CALL text(14,10,concat("5. View Delivery Report ",evaluate(dmr_mig_data->report_delivery,0,
              "**UNAVAILABLE**","")))
           CALL text(16,10,concat("6. View Delivery Error Report ",evaluate(dmr_mig_data->
              report_delivery,0,"**UNAVAILABLE**","")))
           CALL text(18,11,"Your Selection (0 to Exit)?")
           IF ((dmr_mig_data->report_capture=1)
            AND (dmr_mig_data->report_delivery=0))
            CALL accept(18,39,"9;",0
             WHERE curaccept IN (0, 1, 2, 3))
           ELSEIF ((dmr_mig_data->report_delivery=1)
            AND (dmr_mig_data->report_capture=0))
            CALL accept(18,39,"9;",0
             WHERE curaccept IN (0, 4, 5, 6))
           ELSEIF ((dmr_mig_data->report_delivery=1)
            AND (dmr_mig_data->report_capture=1))
            CALL accept(18,39,"9;",0
             WHERE curaccept IN (0, 1, 2, 3, 4,
             5, 6))
           ELSE
            CALL text(18,11,"No reports available (0 to Exit):")
            CALL accept(18,46,"9;",0
             WHERE curaccept IN (0))
           ENDIF
           CASE (curaccept)
            OF 0:
             SET dmm_reports_cont = 0
            OF 1:
             SET dmm_file_to_display = dmr_mig_data->cap_mgr_rpt
            OF 2:
             SET dmm_file_to_display = dmr_mig_data->cap_rpt
            OF 3:
             SET dmm_file_to_display = dmr_mig_data->cap_err_rpt
            OF 4:
             SET dmm_file_to_display = dmr_mig_data->del_mgr_rpt
            OF 5:
             SET dmm_file_to_display = dmr_mig_data->del_rpt
            OF 6:
             SET dmm_file_to_display = dmr_mig_data->del_err_rpt
           ENDCASE
           IF (dmm_reports_cont=1)
            IF (dm2_findfile(dmm_file_to_display)=0)
             SET dm_err->eproc = "Checking for existence of report."
             IF (check_error(dm_err->eproc)=1)
              SET message = nowindow
              CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
              GO TO exit_script
             ENDIF
             CALL text(20,11,
              "The report was not found.  Do you want to view a different report? (Y/N)")
             CALL accept(20,86,"P;CU","Y"
              WHERE curaccept IN ("Y", "N"))
             IF (curaccept="N")
              SET dmm_reports_cont = 0
             ENDIF
            ELSE
             SET dmm_ln_prompt_ind = 1
             WHILE (dmm_ln_prompt_ind=1)
               CALL clear(20,1)
               CALL box(1,1,24,131)
               CALL text(20,11,
                "You can view the entire report, or the last 100 to 10,000 lines of the report.")
               CALL text(21,11,
                "Enter the number of lines you'd like to view, or ALL to view the entire report:")
               CALL accept(21,90,"P(5);CU","ALL")
               IF (curaccept="ALL")
                SET dmm_ln_prompt_ind = 0
                SET dmm_lines_to_view = curaccept
               ELSE
                SET dmm_ln_int = cnvtint(curaccept)
                IF (dmm_ln_int > 99
                 AND dmm_ln_int < 10001)
                 SET dmm_ln_prompt_ind = 0
                 SET dmm_lines_to_view = curaccept
                ELSE
                 CALL clear(20,1)
                 CALL box(1,1,24,131)
                 CALL text(20,11,concat("The number you entered was invalid.  ",
                   "Enter a number between 100 and 10,000.  Press ENTER to continue."))
                 CALL accept(20,115,"AA;D","OK")
                ENDIF
               ENDIF
             ENDWHILE
             IF (dmm_display_report(dmm_file_to_display,dmm_lines_to_view)=0)
              GO TO exit_script
             ENDIF
            ENDIF
           ENDIF
        ...
        OF 2:
         ...
         IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
          ...
          ENDIF
         ENDIF
         WHILE (dmm_background_mon_cont=1)
           SET width = 132
           IF ((dm_err->debug_flag != 722))
            SET message = window
           ENDIF
           ...
           CALL box(1,1,24,131)
           CALL text(2,2,"Database Migration Process")
           CALL text(4,10,"Background Monitoring Menu")
           CALL text(6,10,"1. Setup Email Addresses")
           CALL text(8,10,"2. Setup & Submit Job")
           CALL text(10,10,"3. Status of Job")
           CALL text(12,10,"4. Delete Job")
           ...
           CASE (curaccept)
            OF 0:
             SET dmm_background_mon_cont = 0
            OF 1:
             IF (dmm_email_setup(null)=0)
              GO TO exit_script
             ENDIF
            OF 2:
             EXECUTE dm2_mig_submit_monitor_job
             IF ((dm_err->err_ind=1))
              GO TO exit_script
             ENDIF
            OF 3:
             IF (dmm_show_job_status("MON")=0)
              GO TO exit_script
             ENDIF
            OF 4:
             IF (dmr_stop_job("MON",1)=0)
              GO TO exit_script
             ENDIF
           ENDCASE
         ENDWHILE
     ...
     ENDWHILE
    OF 3:
     SET dmm_vdata_rpt_cont = 1
     WHILE (dmm_vdata_rpt_cont=1)
       SET width = 132
       ...
       CALL clear(1,1)
       CALL box(1,1,24,131)
       CALL text(2,2,"Data Migration Process (Reporting)")
       CALL text(4,10,"Main Reporting Menu")
       CALL text(6,10,"1. Data Verification Reports")
       CALL text(8,11,"Your Selection (0 to Exit)?: ")
       CALL accept(8,39,"9;",0
        WHERE curaccept IN (0, 1))
       CASE (curaccept)
        OF 0:
         SET dmm_vdata_rpt_cont = 0
        OF 1:
         IF (dmr_prompt_retrieve_mig_data(0,0,1,0)=0)
          GO TO exit_script
         ENDIF
         IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
          ...
          ENDIF
         ENDIF
         SET dmm_vdata_rpt_dtl_cont = 1
         WHILE (dmm_vdata_rpt_dtl_cont=1)
           SET width = 132
           IF ((dm_err->debug_flag != 511))
            SET message = window
           ENDIF
           CALL clear(1,1)
           CALL box(1,1,24,131)
           CALL text(2,2,"Database Migration Process (Data Verification Reports)")
           CALL text(4,10,"Choose the Report Type")
           CALL text(6,10,"1. Data Verification Summary")
           CALL text(7,10,"2. Mismatch Detail Report (Unavailable)")
           CALL text(8,10,"3. Data Verification Exclusions")
           CALL text(11,10,"Your Selection (0 to Exit)? ")
           CALL accept(11,39,"9;",0
            WHERE curaccept IN (0, 1, 3))
           CASE (curaccept)
            OF 0:
             SET dmm_vdata_rpt_dtl_cont = 0
            OF 1:
             SET dmm_vdata_get_cur_max = 0
             IF ((validate(bg_get_cur_max,- (1)) != - (1))
              AND (validate(bg_get_cur_max,- (2)) != - (2)))
              CALL clear(1,1)
              CALL box(1,1,24,131)
              CALL text(2,2,"Database Migration Process (Data Verification Reports)")
              CALL text(5,2,"Would you like to gather current maximum key values?")
              CALL text(7,2,"Enter [Y]es or [N]o: ")
              CALL accept(7,24,"A;CU","Y"
               WHERE curaccept IN ("Y", "N"))
              IF (curaccept="Y")
               SET dmm_vdata_get_cur_max = 1
              ELSE
               SET dmm_vdata_get_cur_max = 0
              ENDIF
             ENDIF
             ...
             ENDIF
             IF (curqual=0)
              SET dm_err->eproc = "Insert row into dm_info to bypass current max id retrieval"
              INSERT  FROM dm_info d
               SET d.info_domain = "DM2_VDATA_INFO", d.info_name = "DM2_VDATA_GET_CUR_MAX", d
                .info_number = dmm_vdata_get_cur_max
               WITH nocounter
              ;end insert
              IF (check_error(dm_err->eproc)=1)
               ROLLBACK
              ...
              COMMIT
             ELSE
              SET dm_err->eproc = "Update row into dm_info to bypass current max id retrieval"
              UPDATE  FROM dm_info d
               SET d.info_number = dmm_vdata_get_cur_max
               WHERE d.info_domain="DM2_VDATA_INFO"
                AND d.info_name="DM2_VDATA_GET_CUR_MAX"
               WITH nocounter
              ;end update
            ...
            OF 2:
             SET message = nowindow
             EXECUTE dm2_verify_data_rpt_dtl
             IF ((dm_err->err_ind=1))
              GO TO exit_script
             ENDIF
            OF 3:
             SET message = nowindow
             EXECUTE dm2_verify_data_exclusions
             IF ((dm_err->err_ind=1))
              GO TO exit_script
             ENDIF
           ENDCASE
 ...
 ENDWHILE
 GO TO exit_script
 SUBROUTINE dmm_show_job_status(job_type)
   DECLARE dsjs_file_name = vc WITH protect, noconstant("")
   IF ((dm2_sys_misc->cur_os="AXP"))
    IF (dmr_get_queue_contents(dmr_batch_queue)=0)
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Displaying contents of the batch queue for this job to the CCL Displayer."
    SELECT INTO mine
     FROM (dummyt d  WITH seq = value(dmr_queue->cnt))
     WHERE (dmr_queue->qual[d.seq].jobname=evaluate(job_type,"MON","DM2_MIG_MONITORING","ARCH",
      "DM2_MIG_COPY_ARCHIVELOGS"))
     HEAD REPORT
      found = 0
     DETAIL
      found = 1, col 2, "Entry:    ",
      CALL print(trim(cnvtstring(dmr_queue->qual[d.seq].entry),3)), row + 1, col 2,
      "Jobname:  ", dmr_queue->qual[d.seq].jobname, row + 1,
      col 2, "Username: ", dmr_queue->qual[d.seq].username,
      row + 1, col 2, "Status:   ",
      dmr_queue->qual[d.seq].status, row + 1, col 2,
      "", row + 1, col 2,
      "", row + 1
     FOOT REPORT
      IF (found=0)
       col 2, "There are currently no matching jobs in the batch queue.", row + 1
      ENDIF
     WITH nocounter, maxrow = 1, nullreport,
      maxcol = 2000
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSE
    SET dm_err->disp_dcl_err_ind = 0
    CALL dm2_push_dcl('ps -ef | grep "dm2_mig_monitoring.ksh" | grep -v grep')
    FREE DEFINE rtl
    DEFINE rtl build("CCLUSERDIR:",dm_err->errfile)
    SET dm_err->eproc = "Displaying contents of ps command to CCL Displayer."
    SELECT INTO mine
     FROM rtlt r
     HEAD REPORT
      found = 0
     DETAIL
      ...
      IF (trim(r.line,3) != "* grep *")
       found = 1,
       CALL print(r.line), row + 1
      ENDIF
     FOOT REPORT
      IF (found=0)
       col 2, "There are currently no matching jobs running.", row + 1
      ENDIF
     WITH nocounter, maxrow = 1, nullreport,
      maxcol = 2000
    ;end select
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmm_display_report(report_file,report_lines)
   DECLARE ddr_tail_file = vc WITH protect, noconstant("NOT_SET")
   DECLARE ddr_tail_cmd = vc WITH protect, noconstant("NOT_SET")
   IF (report_lines="ALL")
    FREE DEFINE rtl2
    FREE SET dmr_file_logical
    SET logical dmr_file_logical value(report_file)
    DEFINE rtl2 "dmr_file_logical"
    SET dm_err->eproc = concat("Displaying ",report_file," to the CCL displayer.")
   ELSE
    IF ((dm2_sys_misc->cur_os="AXP"))
     SET ddr_tail_cmd = concat("type /tail=",report_lines," ",report_file)
     SET ddr_tail_file = concat(dm2_install_schema->ccluserdir,dm_err->errfile)
    ELSE
     IF (get_unique_file("dm2_rpt_tail",".dat")=0)
      RETURN(0)
     ENDIF
     SET ddr_tail_file = concat(dm2_install_schema->ccluserdir,dm_err->unique_fname)
     SET ddr_tail_cmd = concat("tail -n ",report_lines," ",report_file," > ",
      ddr_tail_file)
    ENDIF
    IF (dm2_push_dcl(ddr_tail_cmd)=0)
     RETURN(0)
    ENDIF
    FREE DEFINE rtl2
    FREE SET dmr_file_logical
    SET logical dmr_file_logical value(ddr_tail_file)
    DEFINE rtl2 "dmr_file_logical"
    SET dm_err->eproc = concat("Displaying ",ddr_tail_file," to the CCL displayer.")
   ENDIF
   SELECT INTO mine
    FROM rtl2t r
    DETAIL
     CALL print(trim(r.line)), row + 1
 ...
 END ;Subroutine
 SUBROUTINE dmm_email_setup(null)
   ...
   WHILE (true)
     ...
     FOR (des_index = 1 TO least(15,dmr_emails->cnt))
       CALL text((3+ des_index),4,concat(build(des_index),".  ",dmr_emails->qual[des_index].
         email_address))
     ENDFOR
     IF ((dmr_emails->cnt=0))
     ...
     CASE (curaccept)
      OF "A":
       ...
       CALL accept(21,24,"P(40);CH","NONE"
       ...
       IF (curaccept != "NONE"
        AND locateval(des_index,1,dmr_emails->cnt,curaccept,dmr_emails->qual[des_index].email_address
        )=0)
        SET dmr_emails->cnt = (dmr_emails->cnt+ 1)
        SET stat = alterlist(dmr_emails->qual,dmr_emails->cnt)
        SET dmr_emails->qual[dmr_emails->cnt].email_address = curaccept
        SET dmr_emails->change_ind = 1
       ENDIF
       CALL clear(21,2,129)
      OF "R":
       CALL text(21,2,"Email Address to Remove, by number:")
       CALL accept(21,38,"99;H",0
        WHERE curaccept BETWEEN 0 AND dmr_emails->cnt)
       SET des_remove_location = curaccept
       IF (des_remove_location > 0)
        FOR (des_iter = des_remove_location TO (dmr_emails->cnt - 1))
          SET dmr_emails->qual[des_iter].email_address = dmr_emails->qual[(des_iter+ 1)].
          email_address
        ENDFOR
        SET stat = alterlist(dmr_emails->qual,(dmr_emails->cnt - 1))
        SET dmr_emails->cnt = (dmr_emails->cnt - 1)
        SET dmr_emails->change_ind = 1
       ENDIF
      OF "S":
       IF ((dmr_emails->change_ind=1))
        ...
        IF ((dmr_emails->cnt > 0))
         ...
         INSERT  FROM dm_info di,
           (dummyt d  WITH seq = value(dmr_emails->cnt))
          SET di.info_domain = "DM2_MIG_EMAILS", di.info_name = dmr_emails->qual[d.seq].email_address
          PLAN (d)
           JOIN (di)
          WITH nocounter
         ;end insert
        ...
        CASE (curaccept)
         OF "Y":
          RETURN(dmm_send_test_emails(null))
         OF "N":
          RETURN(1)
        ENDCASE
       ENDIF
       RETURN(1)
      OF "T":
       IF (dmm_send_test_emails(null)=0)
        RETURN(0)
       ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmm_send_test_emails(null)
   DECLARE dste_test_file_name = vc WITH protect, constant(build(logical("CCLUSERDIR"),
     "dm2_mig_menu_test.txt"))
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   FOR (dste_iter = 1 TO dmr_emails->cnt)
     IF (dste_iter=1)
      SET dste_email_list = dmr_emails->qual[dste_iter].email_address
     ELSE
      SET dste_email_list = concat(dste_email_list,",",dmr_emails->qual[dste_iter].email_address)
     ENDIF
   ENDFOR
   RETURN(dmr_send_email("Test email",dste_email_list,dste_test_file_name))
 END ;Subroutine
 SUBROUTINE dmm_utc_setup(null)
   DECLARE dus_info_name = vc WITH protect, noconstant("NO")
   DECLARE dus_stage_tspace = vc WITH protect, noconstant("DM2NOTSET")
   DECLARE dus_mock_db_name = vc WITH protect, noconstant("DM2NOTSET")
   DECLARE dus_accept = vc WITH protect, noconstant("")
   SET width = 132
   SET message = window
   CALL clear(1,1)
   CALL box(1,1,15,131)
   CALL text(4,55,"Migration/UTC Setup")
   CALL text(6,2,"Is UTC Conversion part of this Migration process? (Y/N)?")
   CALL accept(6,70,"A;CU"," "
    WHERE curaccept IN ("Y", "N"))
   SET dus_accept = curaccept
   IF (dus_accept="Y")
    SET dus_info_name = "YES"
    CALL text(8,2,concat("Will a staging tablespace be used for indexes that will be replaced at ",
      " completion of UTC conversion (Y/N)? "))
    CALL accept(8,110,"A;CU"," "
     WHERE curaccept IN ("Y", "N"))
    IF (curaccept="Y")
     CALL text(10,2,"Enter the staging tablespace: ")
     CALL accept(10,40,"P(30);C"," "
      WHERE curaccept > "")
     SET dus_stage_tspace = curaccept
     IF (dus_stage_tspace=null)
      SET dus_stage_tspace = "DM2NOTSET"
     ENDIF
    ENDIF
    CALL text(12,2,"Enter the 'mock' database name: ")
    CALL accept(12,40,"P(9);CU"," "
     WHERE build(curaccept) > "")
    SET dus_mock_db_name = trim(cnvtupper(curaccept))
    SET message = nowindow
   ENDIF
   SET message = nowindow
   SET dm_err->eproc = "Deleting DM2_MIG_UTC_IND and DM2_MIG_UTC_IND_TSPACE rows in dm_info."
   CALL disp_msg(" ",dm_err->logfile,0)
   DELETE  FROM dm_info di
    WHERE di.info_domain IN ("DM2_MIG_UTC_IND", "DM2_MIG_UTC_IND_TSPACE", "DM2_MIG_UTC_MOCK_DB_NAME")
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc) != 0)
    ROLLBACK
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Inserting DM2_MIG_UTC_IND_TSPACE row into dm_info."
   CALL disp_msg(" ",dm_err->logfile,0)
   INSERT  FROM dm_info di
    SET di.info_domain = "DM2_MIG_UTC_IND_TSPACE", di.info_name = dus_stage_tspace, di.updt_dt_tm =
     cnvtdatetime(curdate,curtime3)
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc))
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Inserting new DM2_MIG_UTC_IND row into dm_info."
   CALL disp_msg(" ",dm_err->logfile,0)
   INSERT  FROM dm_info di
    SET di.info_domain = "DM2_MIG_UTC_IND", di.info_name = dus_info_name, di.updt_dt_tm =
     cnvtdatetime(curdate,curtime3)
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc))
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Inserting new DM2_MIG_UTC_MOCK_DB_NAME row into dm_info."
   CALL disp_msg(" ",dm_err->logfile,0)
   INSERT  FROM dm_info di
    SET di.info_domain = "DM2_MIG_UTC_MOCK_DB_NAME", di.info_name = dus_mock_db_name, di.updt_dt_tm
      = cnvtdatetime(curdate,curtime3)
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc))
...
#exit_script
 ...
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "Dm2_mig_menu completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_menu")
END GO
