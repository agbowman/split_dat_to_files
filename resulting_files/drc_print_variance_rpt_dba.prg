CREATE PROGRAM drc_print_variance_rpt:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dfr
   ...
   WHERE dp.dose_range_check_id=drc.dose_range_check_id
    ...
    AND dp.active_ind=1)
   JOIN (ddr
   WHERE ddr.drc_premise_id=dp.drc_premise_id
    AND ((ddr.min_variance_pct > 0) OR (ddr.max_variance_pct > 0)) )
   JOIN (dp2
   ...
   ELSEIF (dp2.premise_type_flag=5)
    ...
    CASE (dp2.relational_operator_flag)
     ...
     OF 6:
      ...
      ranges->groupers[grpcnt].premises[ppcnt].pma_sort_key = (ranges->groupers[grpcnt].premises[
      ppcnt].pma_sort_key+ number_of_days),ranges->groupers[grpcnt].premises[ppcnt].to_pmaday =
      number_of_days
     ELSE
      CALL echo(build("Can't recognize relational operator:",dp2.relational_operator_flag))
    ENDCASE
  ...
  *
  ...
  HEAD PAGE
   ...
   "{cpi/12}", row + 1, col 50,
   "DOSE RANGE CHECKING VARIANCE DATA REPORT", row + 1, col 1,
