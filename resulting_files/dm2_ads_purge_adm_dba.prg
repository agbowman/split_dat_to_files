CREATE PROGRAM dm2_ads_purge_adm:dba
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_ads_purge_adm",".log","dm2_ads_purge_adm")=0)
  GO TO exit_program
 ENDIF
 DECLARE dapa_continue = i2 WITH protect, noconstant(0)
 DECLARE dapa_config_status = vc WITH protect, noconstant("")
 DECLARE dapa_config_status_msg = vc WITH protect, noconstant("")
 DECLARE dapa_config_name = vc WITH protect, noconstant("")
 DECLARE dapa_config_updt_dt_tm = f8 WITH protect, noconstant(0)
 DECLARE dapa_config_id = f8 WITH protect, noconstant(0)
 DECLARE dapa_option = i2 WITH protect, noconstant(0)
 DECLARE dapa_setup_complete = i2 WITH protect, noconstant(0)
 DECLARE dm_ads_purge_main_menu = i2 WITH public, noconstant(0)
 DECLARE dapa_header(dapa_prof_stat=vc,dapa_prof_stat_msg=vc) = i2
 DECLARE dapa_reset(dapa_prof_stat=vc) = i2
 SET dm_err->eproc = "Validating user is in ADS Purge source domain"
 SELECT INTO "nl:"
  FROM dm2_ddl_ops ddo
  WHERE ddo.process_option="PURGE TARGET"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual > 0)
  ...
  CALL box(1,1,3,132)
  CALL box(1,1,24,132)
  CALL text(2,2,"ACTIVITY DATA SAMPLE (ADS) PURGE - MAIN MENU")
  CALL text(5,5,"ERROR:")
  CALL text(6,5,"User must be in ADS Purge/ADS Replicate source domain to run this program")
  CALL text(8,5,"e(X)it: ")
  CALL accept(8,13,"A;CU","X"
   WHERE curaccept IN ("X"))
  GO TO exit_program
 ENDIF
 SET dapa_continue = 1
 WHILE (dapa_continue=1)
   SET width = 132
   SET message = window
   SET dapa_config_status_msg = ""
   SET dapa_setup_complete = 0
   SET dm_err->eproc = "Retrieving ADS Purge config information"
   SELECT INTO "nl:"
    FROM dm_ads_config d
    WHERE d.sample_method=dpl_purge
    DETAIL
     dapa_config_status = d.config_status, dapa_config_name = d.config_name, dapa_config_id = d
     .dm_ads_config_id,
     dapa_config_updt_dt_tm = d.updt_dt_tm, dads_list->config_id = d.dm_ads_config_id
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual > 0)
    SET dm_err->eproc = "Determining if current ADS Purge config has driverkeys built"
    SELECT INTO "nl:"
    ...
    IF (check_error(dm_err->eproc)=1)
     ...
     GO TO exit_program
    ENDIF
    IF (dapa_config_status != "NEEDSBUILD"
     AND curqual > 0)
     SET dapa_setup_complete = 1
     SET dapa_config_status_msg = "Ready to Begin Replicate Process"
    ELSE
     SET dapa_config_status_msg = "Ready to Import List of Persons"
    ENDIF
   ELSE
    SET dapa_config_name = ""
    SET dapa_config_status_msg = "Ready to Create ADS Purge Sample"
   ENDIF
   IF (dapa_setup_complete=1
    AND dapa_config_status != "NEEDSBUILD")
    IF (((dapa_config_status="PURGE_READY") OR (dapa_config_status="PURGE_RUNNING")) )
     IF (dapa_header(dapa_config_status,"")=0)
      GO TO exit_program
     ENDIF
     CALL text(12,5,"1. Begin/Manage ADS Purge Process")
     CALL text(13,5,"2. Run ADS Purge Reports")
     CALL text(14,5,"3. Reset ADS Purge")
     CALL text(16,5,"Your Selection (0 to Exit)? ")
     CALL accept(16,34,"9",0
      WHERE curaccept IN (0, 1, 2, 3))
     SET dapa_option = curaccept
     CASE (dapa_option)
      OF 1:
       CALL clear(1,1)
       SET dm_ads_purge_main_menu = 1
       SET dm_err->eproc = "Executing DM2_ADS_PURGE_MENU"
       EXECUTE dm2_ads_purge_menu
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ROLLBACK
        GO TO exit_program
       ENDIF
      OF 2:
       SET dm_err->eproc = "Executing DM2_ADS_PURGE_REPORT"
       EXECUTE dm2_ads_purge_report
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ROLLBACK
        GO TO exit_program
       ENDIF
      OF 3:
       IF (dapa_reset(dapa_config_status)=0)
        GO TO exit_program
       ENDIF
      OF 0:
       SET dapa_continue = 0
     ENDCASE
    ELSEIF (dapa_config_status="PURGE_COMPLETE")
     IF (dapa_header(dapa_config_status,"")=0)
      GO TO exit_program
     ENDIF
     CALL text(12,5,"1. Run ADS Purge Reports")
     CALL text(13,5,"2. Start a New ADS Purge Project")
     CALL text(16,5,"Your Selection (0 to Exit)? ")
     CALL accept(16,34,"9",0
     ...
     ENDCASE
    ELSE
     IF (dapa_config_status="REPLICATE_RUNNING")
      IF (dapa_header("REPLICATE_RUNNING",dapa_config_status_msg)=0)
       GO TO exit_program
      ENDIF
     ELSE
      IF (dapa_header("REPLICATE_READY",dapa_config_status_msg)=0)
       GO TO exit_program
      ENDIF
     ENDIF
     CALL text(12,5,"1. Begin/Manage ADS Replicate Process")
     CALL text(13,5,"2. Manage ADS Sample")
     CALL text(14,5,"3. Run ADS Purge Reports")
     CALL text(15,5,"4. Reset ADS Purge")
     CALL text(17,5,"Your Selection (0 to Exit)? ")
     CALL accept(17,34,"9",0
     ...
     CASE (dapa_option)
      OF 1:
       CALL clear(1,1)
       SET message = nowindow
       SET dm_err->eproc = "Executing DM2_DOMAIN_MAINT"
       EXECUTE dm2_domain_maint
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ROLLBACK
        GO TO exit_program
       ENDIF
      OF 2:
       SET dm_err->eproc = "Executing DM2_ADS_PURGE_MANAGE"
       EXECUTE dm2_ads_purge_manage
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ROLLBACK
        GO TO exit_program
    ...
    ENDIF
   ELSE
    IF (dapa_header("PROFILE_SETUP",dapa_config_status_msg)=0)
     GO TO exit_program
    ENDIF
    CALL text(12,5,"1. Create/Manage ADS Sample")
    CALL text(13,5,"2. Run ADS Purge Reports")
    CALL text(17,5,"Your Selection (0 to Exit)? ")
    IF (dapa_config_name > "")
     CALL text(14,5,"3. Reset ADS Purge")
   ...
   ENDIF
 ENDWHILE
 SUBROUTINE dapa_header(prof_stat,prof_msg)
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL box(1,1,24,132)
   CALL text(2,2,"ACTIVITY DATA SAMPLE (ADS) PURGE - MAIN MENU")
   IF (prof_stat < "")
    SET dm_err->eproc = "Retrieving current configuration status"
    SELECT INTO "nl:"
     FROM dm_ads_config d
     WHERE (d.dm_ads_config_id=dads_list->config_id)
     DETAIL
    ...
    ENDIF
   ENDIF
   CALL text(9,18,prof_msg)
   IF (prof_stat="PROFILE_SETUP")
    CALL video(r)
    CALL text(5,3,"*")
    CALL text(5,15,"*")
    CALL text(5,4,"   SETUP   ")
   ...
   IF (prof_stat="REPLICATE_RUNNING")
    CALL text(9,18,"ADS Purge Replicate is Currently Running")
    CALL video(r)
    CALL text(5,19,"*")
    CALL text(5,31,"*")
    CALL text(5,20," REPLICATE ")
   ...
   CALL box(4,18,6,32)
   CALL video(n)
   IF (prof_stat="PURGE_READY")
    CALL text(9,18,"ADS Purge Process is Ready to Run")
    CALL video(r)
    CALL text(5,35,"*")
    CALL text(5,47,"*")
    CALL text(5,36,"   PURGE   ")
   ELSE
    CALL text(5,35,"    PURGE    ")
   ENDIF
   IF (prof_stat="PURGE_RUNNING")
    CALL text(9,18,"ADS Purge Process is Currently Running")
    CALL video(r)
    CALL text(5,35,"*")
    CALL text(5,47,"*")
    CALL text(5,36,"   PURGE   ")
   ENDIF
   IF (prof_stat="PURGE_COMPLETE")
    CALL text(9,18,"Purge Process is Complete")
    CALL video(r)
    CALL text(5,35,"*")
    CALL text(5,47,"*")
    CALL text(5,36,"   PURGE   ")
   ...
   CALL video(n)
   IF (prof_stat="RESET")
    CALL text(9,18,"Reset ADS Purge process")
   ENDIF
   CALL text(9,2,"Sample Status: ")
   CALL text(8,2,"Sample Name: ")
   CALL text(8,18,dapa_config_name)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dapa_reset(config_status)
   CALL dapa_header("RESET","")
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ROLLBACK
    GO TO exit_program
   ENDIF
   IF (config_status="PURGE_RUNNING")
    CALL text(12,5,"ADS Purge runners are currently running, you can not reset at this time.")
    CALL text(13,5,"e(X)it ")
    CALL accept(13,13,"A;CU","X"
     WHERE curaccept IN ("X"))
   ELSEIF (config_status="REPLICATE_RUNNING")
    CALL text(12,5,"ADS Replicate runners are currently running, you can not reset at this time.")
    CALL text(13,5,"e(X)it ")
    CALL accept(13,13,"A;CU","X"
     WHERE curaccept IN ("X"))
   ELSE
    IF (config_status="PURGE_COMPLETE")
     CALL text(13,8,concat("By starting a new ADS Purge project, the current sample <",
       dapa_config_name,"> will be lost."))
     CALL text(17,8,"Would you like to continue?")
    ELSE
     CALL text(12,5,concat("Resetting the current ADS Purge sample <",dapa_config_name,
       "> will result in the: "))
     CALL text(13,8,"Loss of Current ADS Purge Sample Setup")
     IF (((config_status="REPLICATE_READY") OR (config_status="PURGE_READY")) )
      CALL text(14,8,"Loss of the PERSON_IDs to Purge list/Inactivation of PERSON_IDs to Purge")
      CALL text(18,5,"Would you still like to reset?")
     ENDIF
     IF (config_status="PURGE_READY")
      CALL text(15,8,"Loss of All Replicate Work")
     ENDIF
    ENDIF
    CALL text(19,5,"(Y)es, (N)o ? ")
    CALL accept(19,19,"A;CU","N"
     WHERE curaccept IN ("Y", "N"))
   ENDIF
   IF (curaccept="Y")
    CALL text(21,5,"Really?")
    CALL text(22,5,"(Y)es, (N)o ? ")
    CALL accept(22,19,"A;CU","N"
     WHERE curaccept IN ("Y", "N"))
    IF (curaccept="Y")
     ...
     ENDIF
     SET message = nowindow
     SET dm_err->eproc = "Executing DM2_ADS_PURGE_RESET"
     EXECUTE dm2_ads_purge_reset dads_list->config_id
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ROLLBACK
      GO TO exit_program
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "LEAVING dm2_ads_purge_adm"
 CALL final_disp_msg("dm2_ads_purge_adm")
END GO
