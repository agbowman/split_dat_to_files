CREATE PROGRAM dm2_aload_request
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_request",".log","dm2_aload_request")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_request"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dar_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dar_file = vc WITH protect, noconstant( $2)
 DECLARE dar_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dar_batch = i4 WITH protect, noconstant(0)
 DECLARE dar_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE dar_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dar_pkg,dar_file_loc_ret,dar_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dar_batch=0)
  SET dar_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing dara from AFD for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND dar_rows_returned=10000)
   DELETE  FROM dm_ocd_request d
    WHERE d.alpha_feature_nbr=dar_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET dar_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",dar_rows_returned))
   ENDIF
 ENDWHILE
 IF (dar_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dar_file_loc_ret,dar_file), "dm2_aload_atr_req", dar_batch
  CALL echo(concat(dar_file_loc_ret,dar_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_request completed."
 ENDIF
 CALL final_disp_msg("dm2_aload_request")
END GO
