CREATE PROGRAM dm2_add_prsnl_acct:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 CALL check_logfile("dm2_add_prsnl_acct",".log","DM2_ADD_PRSNL_ACCT")
 SET dm_err->eproc = "Beginning dm2_add_prsnl_acct"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dapa_log_type = vc WITH protect, constant("PRSSEC")
 DECLARE dapa_prcnt = i4 WITH protect, noconstant(0)
 DECLARE con_input = i2 WITH protect, noconstant(0)
 DECLARE add_user = i4 WITH protect, noconstant(1)
 DECLARE head_screen(scr_text=vc) = null
 DECLARE disp_rpt(dapa_suc_ind=i2,dapa_rpt_head=vc) = null
 FREE RECORD prsnl_info
 RECORD prsnl_info(
   1 prsnl_size = i4
   1 env_id = f8
   1 qual[*]
 ...
 SET prsnl_info->env_id = 0
 IF ((xxcclseclogin->loggedin != 1))
  SET dm_err->eproc = "User not logged into CCLSECLOGIN. Log into CCLSECLOGIN and try again!"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  ...
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="DM_ENV_ID"
  DETAIL
   prsnl_info->env_id = di.info_number
  WITH nocounter
 ;end select
 IF ((prsnl_info->env_id=0))
  SET dm_err->eproc = "The current environment id is invalid"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM dm_chg_log d
  WHERE d.log_type=dapa_log_type
   AND (d.target_env_id=prsnl_info->env_id)
  DETAIL
   dapa_prcnt = (dapa_prcnt+ 1)
   IF (mod(dapa_prcnt,100)=1)
    stat = alterlist(prsnl_info->qual,(dapa_prcnt+ 99))
   ENDIF
   prsnl_info->qual[dapa_prcnt].log_id = d.log_id, prsnl_info->qual[dapa_prcnt].log_dt_tm = d
   .chg_dt_tm, pos = findstring("&",d.pk_where),
   pk_size = size(trim(d.pk_where))
   IF (pos > 0
    AND pk_size > 0)
    prsnl_info->qual[dapa_prcnt].username = substring(1,(pos - 1),d.pk_where), prsnl_info->qual[
    dapa_prcnt].name_full_formatted = substring((pos+ 1),pk_size,d.pk_where), prsnl_info->qual[
    dapa_prcnt].valid_ind = 1
   ELSE
    prsnl_info->qual[dapa_prcnt].name_full_formatted = substring(1,30,d.pk_where), prsnl_info->qual[
    dapa_prcnt].valid_ind = 0
   ENDIF
  FOOT REPORT
   prsnl_info->prsnl_size = dapa_prcnt, stat = alterlist(prsnl_info->qual,dapa_prcnt)
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET dm_err->eproc = "No new personel information was added to this domain"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 CALL head_screen("PRSNL REPORT")
 CALL text(7,3,"WARNING: To create personnel accounts using this program ")
 CALL text(8,3,"         you must have the ManageAccount privilege")
 CALL text(10,3,"Do you want to continue?(Y/N)")
 CALL accept(10,40,"P;CU","Y"
 ...
 SET message = nowindow
 SELECT INTO mine
  FROM (dummyt d  WITH seq = prsnl_info->prsnl_size)
  ORDER BY prsnl_info->qual[d.seq].valid_ind DESC
  HEAD REPORT
   dr_flag = 2, col 0, "NEW PERSONNEL INFORMATION",
   row + 2
  HEAD PAGE
   col 0, "FULL_NAME", col 40,
   "USERNAME", col 70, "LOG_DATE_TIME",
   row + 1
  DETAIL
   IF ((dr_flag != prsnl_info->qual[d.seq].valid_ind))
    row + 1
    IF ((prsnl_info->qual[d.seq].valid_ind=1))
     col 0, "--- This program will attempt to create accounts for the following personnel:"
    ELSE
     col 0, "--- This program will ignore the following invalid records : "
    ENDIF
    dr_flag = prsnl_info->qual[d.seq].valid_ind, row + 1
   ENDIF
   col 0, prsnl_info->qual[d.seq].name_full_formatted, col 40,
   prsnl_info->qual[d.seq].username, col 70, prsnl_info->qual[d.seq].log_dt_tm";;Q",
   row + 1
  WITH nocounter, maxcol = 132
 ;end select
 FOR (dapa_uar = 1 TO value(prsnl_info->prsnl_size))
  IF ((prsnl_info->qual[dapa_uar].valid_ind=1))
   SET add_user = uar_sec_user(nullterm(prsnl_info->qual[dapa_uar].username),nullterm(prsnl_info->
     qual[dapa_uar].username),prsnl_info->qual[dapa_uar].name_full_formatted)
   IF (add_user != 2
    AND add_user != 0)
    SET prsnl_info->qual[dapa_uar].success_ind = 0
    SET dm_err->eproc = concat("Can not create account for ",prsnl_info->qual[dapa_uar].
     name_full_formatted)
    CALL disp_msg(" ",dm_err->logfile,0)
   ELSE
    SET prsnl_info->qual[dapa_uar].success_ind = 1
    SET dm_err->eproc = concat(prsnl_info->qual[dapa_uar].name_full_formatted,
     " is processed successfully")
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
  ELSE
   SET prsnl_info->qual[dapa_uar].success_ind = 0
   SET dm_err->eproc = build(prsnl_info->qual[dapa_uar].log_id,
    ": Invalid USERNAME, NAME_FULL_FORMATTED Combination")
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  IF (check_error(concat("ERROR: Can not create the account!")) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dapa_uar = (value(prsnl_info->prsnl_size)+ 1)
  ENDIF
 ENDFOR
 DELETE  FROM dm_chg_log dcl,
   (dummyt d  WITH seq = prsnl_info->prsnl_size)
  SET dcl.seq = 1
  PLAN (d
   WHERE (prsnl_info->qual[d.seq].success_ind=1))
   JOIN (dcl
   WHERE (dcl.log_id=prsnl_info->qual[d.seq].log_id))
  WITH nocounter
 ;end delete
 COMMIT
 SELECT INTO mine
  FROM (dummyt d  WITH seq = prsnl_info->prsnl_size)
  WHERE (prsnl_info->qual[d.seq].valid_ind=1)
  ORDER BY prsnl_info->qual[d.seq].success_ind DESC
  HEAD REPORT
   dr_flag = 2, col 0, "PRSNL USER ACCOUNTS CREATION REPORT",
   row + 2
  HEAD PAGE
   col 0, "FULL_NAME", col 40,
   "USERNAME", col 70, "LOG_DATE_TIME",
   row + 1
  DETAIL
   IF ((dr_flag != prsnl_info->qual[d.seq].success_ind))
    row + 1
    IF ((prsnl_info->qual[d.seq].success_ind=1))
     col 0, "--- The Creation of PRSNL User Accounts Successful for the following users:"
    ELSE
     col 0, "--- The Creation of PRSNL User Accounts Failed for the following users: "
    ENDIF
    dr_flag = prsnl_info->qual[d.seq].success_ind, row + 1
   ENDIF
   col 0, prsnl_info->qual[d.seq].name_full_formatted, col 40,
   prsnl_info->qual[d.seq].username, col 70, prsnl_info->qual[d.seq].log_dt_tm";;Q",
   row + 1
  WITH nocounter, maxcol = 132
 ;end select
 SUBROUTINE head_screen(scr_text)
   SET message = window
   CALL clear(1,1)
   SET width = 132
   CALL box(1,1,5,80)
   CALL text(3,30,scr_text)
 END ;Subroutine
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "...Ending dm2_add_prsnl_acct"
 CALL final_disp_msg("dm2_add_prsnl_acct")
END GO
