CREATE PROGRAM cp_del_node
 ...
 DECLARE deletenoderows(null) = i2 WITH protect
 DECLARE deleteactionrows(null) = i2 WITH protect
 DECLARE deletecomponentrows(null) = i2 WITH protect
 ...
 CALL deletecomponentrows(null)
 CALL deletenoderows(null)
 ...
 SUBROUTINE verifyinputs(null)
   ...
   IF (nodelistsize <= 0)
    CALL reporterrorandexit("PERFORM","Validate","Node list must be nonzero in size.")
   ENDIF
   FOR (x = 1 TO nodelistsize)
    SELECT INTO "NL:"
     FROM cp_node cn
     WHERE (cn.cp_node_id=request->node_list[x].cp_node_id)
     WITH nocounter
    ;end select
    IF (curqual=0)
 ...
 SUBROUTINE deleteactionrows(null)
   ...
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = size(request->node_list,5)),
     cp_pathway_action cpa
    PLAN (d1
     WHERE d1.seq <= size(request->node_list,5))
     JOIN (cpa
     WHERE (cpa.cp_node_id=request->node_list[d1.seq].cp_node_id))
    HEAD REPORT
     i = 0
    DETAIL
     i += 1
     IF (i > size(node_action_list->qual,5))
      stat = alterlist(node_action_list->qual,(i+ 5))
     ENDIF
     node_action_list->qual[i].cp_node_id = cpa.cp_node_id, node_action_list->qual[i].
     cp_pathway_action_id = cpa.cp_pathway_action_id
    FOOT REPORT
     stat = alterlist(node_action_list->qual,i)
    WITH nocounter
   ;end select
   IF (size(node_action_list->qual,5) > 0)
    EXECUTE cp_del_pathway_action  WITH replace(action_del_request,node_action_list)
   ELSE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE deletenoderows(null)
   CALL log_message("In deleteNodeRows()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE x = i4 WITH noconstant(0), protect
   DECLARE cp_node_id = f8 WITH noconstant(0), protect
   DECLARE cp_br_category_cnt = i4 WITH noconstant(0), protect
   IF (checkdic("CP_COMPONENT","T",0) != 2)
    CALL reporterrorandexit("PERFORM","INSERT",
     " CP_NODE does not exist or permission is not granted to access it.")
   ENDIF
   ...
   FOR (x = 1 TO size(request->node_list,5))
     ...
     SELECT INTO "nl:"
      ...
      WHERE (cn.cp_node_id=request->node_list[x].cp_node_id)
      HEAD cn.cp_node_id
       IF (cn.br_datamart_category_id > 0)
        cp_br_category_cnt += 1, stat = alterlist(del_bed_view_req->views,cp_br_category_cnt),
        del_bed_view_req->views[cp_br_category_cnt].br_datamart_category_id = cn
        .br_datamart_category_id
       ENDIF
      WITH nocounter
     ;end select
     IF (curqual=0)
      CALL reporterror("PERFORM","RETRIEVE"," Unable to find CP_NODE_ID on CP_NODE table.")
      ROLLBACK
      GO TO exit_script
     ELSE
      SELECT INTO "nl:"
       FROM cp_node_behavior cnb
       WHERE (cnb.cp_node_id=request->node_list[x].cp_node_id)
       WITH nocounter
      ;end select
      IF (curqual > 0)
       DELETE  FROM cp_node_behavior cnb
        WHERE (cnb.cp_node_id=request->node_list[x].cp_node_id)
        WITH nocounter
       ;end delete
       CALL reporterror("PERFORM","DELETE","Unable to delete row from CP_NODE_BEHAVIOR table.")
      ENDIF
     ENDIF
     ...
     ;end delete
     IF (curqual=0)
      CALL reporterror("PERFORM","DELETE","Unable to delete row from CP_NODE table.")
      ROLLBACK
      GO TO exit_script
     ELSE
      SET reply->node_list[x].cp_node_id = request->node_list[x].cp_node_id
     ENDIF
   ENDFOR
   EXECUTE bed_del_vb_views  WITH replace(request,del_bed_view_req), replace(reply,del_bed_view_rep)
   IF ((del_bed_view_rep->status_data.status="S"))
    COMMIT
   ELSE
    SET reply->status_data.status = del_bed_view_rep->status_data.status
    SET stat = moverec(reply->status_data.subeventstatus,del_bed_view_rep->status_data.subeventstatus
     )
    ROLLBACK
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit deleteNodeRows(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE deletecomponentrows(null)
   CALL log_message("In deleteComponentRows()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE x = i4 WITH protect, noconstant(0)
   ...
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = size(request->node_list,5)),
     cp_component cc
    PLAN (d1
     WHERE d1.seq <= size(request->node_list))
     JOIN (cc
     WHERE (cc.cp_node_id=request->node_list[d1.seq].cp_node_id))
    HEAD REPORT
     i = 0
    DETAIL
     i += 1
     IF (i > size(component_list->qual,5))
      stat = alterlist(component_list->qual,(i+ 5))
     ENDIF
     component_list->qual[i].cp_node_id = cc.cp_node_id, component_list->qual[i].cp_component_id = cc
     .cp_component_id, component_list->qual[i].comp_type_cd = cc.comp_type_cd
    FOOT REPORT
     stat = alterlist(component_list->qual,i)
    WITH nocounter
   ;end select
   IF (size(component_list->qual,5) > 0)
    EXECUTE cp_del_component  WITH replace(comp_del_request,component_list)
   ELSE
    CALL echo("Unable to find CP_COMPONENT_DETAIL rows to delete.")
   ENDIF
   CALL log_message(build("Exit deleteComponentRows(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(1)
 END ;Subroutine
