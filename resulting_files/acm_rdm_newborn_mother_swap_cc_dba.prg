CREATE PROGRAM acm_rdm_newborn_mother_swap_cc:dba
 ...
 SET ms_child_c_info_domain_nm = "ACM_RDM_NEWBORN_MOTHER"
 SET ms_child_c_max_name = "MAX ENCNTR_ENCNTR_RELTN_ID EVALUATED"
 CALL sbr_get_min_max(mf_min_id,mf_max_id)
 CALL echo("Processing acm_rdm_newborn_mother_swap_cc...")
 INSERT  FROM temp_rdm_newborn_mother_swap t
  (t.encntr_encntr_reltn_id, t.mother_encntr_id, t.child_encntr_id)(SELECT DISTINCT
   eer.encntr_encntr_reltn_id, emcr.mother_encntr_id, emcr.child_encntr_id
   FROM encntr_mother_child_reltn emcr,
   ...
   WHERE emcr.mother_encntr_id=eer.related_encntr_id
    ...
    AND eer.encntr_encntr_reltn_id BETWEEN mf_min_id AND mf_max_id)
  WITH nocounter
 ;end insert
 IF (sbr_handle_cc_error(null)=0)
  GO TO exit_program
 ENDIF
 UPDATE  FROM encntr_encntr_reltn eer
  (eer.encntr_id, eer.related_encntr_id, eer.updt_dt_tm,
  eer.updt_cnt, eer.updt_id, eer.updt_task,
  eer.updt_applctx)(SELECT
   t.mother_encntr_id, t.child_encntr_id, cnvtdatetime(sysdate),
   (eer.updt_cnt+ 1), reqinfo->updt_id, reqinfo->updt_task,
   reqinfo->updt_applctx
   FROM temp_rdm_newborn_mother_swap t
   WHERE t.encntr_encntr_reltn_id=eer.encntr_encntr_reltn_id)
  WHERE (eer.encntr_encntr_reltn_id=
  (SELECT
   t2.encntr_encntr_reltn_id
   FROM temp_rdm_newborn_mother_swap t2
   WHERE t2.encntr_encntr_reltn_id > 0))
  WITH nocounter
 ;end update
 IF (sbr_handle_cc_error(null)=0)
  GO TO exit_program
 ENDIF
 CALL sbr_truncate_table("temp_rdm_newborn_mother_swap")
