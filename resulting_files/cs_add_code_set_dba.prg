CREATE PROGRAM cs_add_code_set:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 INSERT  FROM code_value_set c
  SET c.code_set = request->code_set, c.display = request->display, c.display_key = cnvtupper(
   ...
   c.description = request->description, c.definition = request->definition, c.table_name = request->
   table_name,
   c.contributor = request->contributor, c.owner_module = request->owner_module, c.cache_ind =
   request->cache_ind,
   ...
   .domain_qualifier_ind = request->domain_qualifier_ind,
   c.domain_code_set = request->domain_code_set, c.display_dup_ind = request->display_dup_ind, c
   .display_key_dup_ind = request->display_key_dup_ind,
   c.cdf_meaning_dup_ind = request->cdf_meaning_dup_ind, c.active_ind_dup_ind = request->
   active_ind_dup_ind, c.def_dup_rule_flag = 1,
