CREATE PROGRAM dm2_ins_upd_ts_prec:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE iutpp_logfile_prefix = vc WITH protect, constant("dm2_ins_upd_ts_prec")
 DECLARE iutpp_cnt = i4 WITH protect, noconstant(0)
 IF (check_logfile(iutpp_logfile_prefix,".log","DM2_INS_UPD_TS_PREC LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning maintenance on DM_TS_PRECEDENCE...."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ELSEIF (curqual=1)
  ...
  ENDIF
 ENDIF
 IF ((ts_prec_maint->num_of_inserts > 0))
  IF (dm2_set_autocommit(1)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Performing insert maintenance on DM_TS_PRECEDENCE."
  CALL disp_msg(" ",dm_err->logfile,0)
  CALL dm2_push_cmd("insert into dm_ts_precedence  dtp, ",0)
  CALL dm2_push_cmd("  (dummyt d with seq = value(size(ts_prec->qual,5))) ",0)
  CALL dm2_push_cmd(" set dtp.table_name = ts_prec->qual[d.seq]->table_name ",0)
  CALL dm2_push_cmd("  ,dtp.precedence = ts_prec->qual[d.seq]->precedence ",0)
  CALL dm2_push_cmd("  ,dtp.data_tablespace = ts_prec->qual[d.seq]->data_tablespace ",0)
  CALL dm2_push_cmd("  ,dtp.data_extent_size = ts_prec->qual[d.seq]->data_extent_size ",0)
  CALL dm2_push_cmd("  ,dtp.index_tablespace = ts_prec->qual[d.seq]->index_tablespace ",0)
  CALL dm2_push_cmd("  ,dtp.index_extent_size = ts_prec->qual[d.seq]->index_extent_size ",0)
  CALL dm2_push_cmd("  ,dtp.long_tablespace = ts_prec->qual[d.seq]->long_tablespace ",0)
  CALL dm2_push_cmd("  ,dtp.long_extent_size = ts_prec->qual[d.seq]->long_extent_size ",0)
  CALL dm2_push_cmd("  ,dtp.updt_dt_tm = cnvtdatetime(curdate,curtime3) ",0)
  CALL dm2_push_cmd("  ,dtp.updt_cnt = ts_prec->qual[d.seq]->updt_cnt ",0)
  CALL dm2_push_cmd("  ,dtp.updt_applctx = 0 ",0)
  CALL dm2_push_cmd("  ,dtp.updt_task = 0 ",0)
  CALL dm2_push_cmd("  ,dtp.updt_id = 0 ",0)
  CALL dm2_push_cmd(' plan d where ts_prec_maint->qual[d.seq]->maint_type = "I" ',0)
  CALL dm2_push_cmd(" join dtp where ts_prec->qual[d.seq]->table_name = dtp.table_name ",0)
  CALL dm2_push_cmd("            and ts_prec->qual[d.seq]->precedence = dtp.precedence ",0)
  ...
  ENDIF
 ENDIF
 IF ((ts_prec_maint->num_of_updates > 0))
  IF (currdb="DB2UDB")
  ...
  IF (dm2_set_autocommit(1)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Performing update maintenance on DM_TS_PRECEDENCE."
  CALL disp_msg(" ",dm_err->logfile,0)
  CALL dm2_push_cmd("update into dm_ts_precedence dtp, ",0)
  CALL dm2_push_cmd("  (dummyt d with seq = value(size(ts_prec->qual,5))) ",0)
  CALL dm2_push_cmd(" set dtp.data_tablespace = ts_prec->qual[d.seq]->data_tablespace ",0)
  CALL dm2_push_cmd("  ,dtp.data_extent_size = ts_prec->qual[d.seq]->data_extent_size ",0)
  CALL dm2_push_cmd("  ,dtp.index_tablespace = ts_prec->qual[d.seq]->index_tablespace ",0)
  CALL dm2_push_cmd("  ,dtp.index_extent_size = ts_prec->qual[d.seq]->index_extent_size ",0)
  CALL dm2_push_cmd("  ,dtp.long_tablespace = ts_prec->qual[d.seq]->long_tablespace ",0)
  ...
  CALL dm2_push_cmd("  ,dtp.updt_task = 0 ",0)
  CALL dm2_push_cmd("  ,dtp.updt_id = 0 ",0)
  CALL dm2_push_cmd(' plan d where ts_prec_maint->qual[d.seq]->maint_type = "U" ',0)
  CALL dm2_push_cmd(" join dtp where ts_prec->qual[d.seq]->table_name = dtp.table_name ",0)
  CALL dm2_push_cmd("            and ts_prec->qual[d.seq]->precedence = dtp.precedence ",0)
  CALL dm2_push_cmd("with nocounter go",1)
  IF ((dm_err->err_ind=0))
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Maintenance on DM_TS_PRECEDENCE Complete"
 CALL final_disp_msg(iutpp_logfile_prefix)
END GO
