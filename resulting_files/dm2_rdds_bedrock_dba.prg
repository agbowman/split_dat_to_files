CREATE PROGRAM dm2_rdds_bedrock:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drb_zip_str = vc
 DECLARE drb_env_id = f8 WITH noconstant(0.0)
 DECLARE drb_refchg_cnt = i4 WITH noconstant(1)
 DECLARE drb_old_cnt = i4 WITH noconstant(0)
 DECLARE drb_err_flag = i2 WITH noconstant(0)
 DECLARE drb_get_dir(dgd_dir=vc) = vc
 DECLARE unzip_file(uf_dir=vc) = i2
 DECLARE drb_move_zip_file(dmf_from=vc,dmf_to=vc) = i2
 DECLARE drb_inc_def(table_name=vc) = null
 DECLARE drb_del_processing_files(null) = null
 DECLARE drb_create_admin_db(null) = i2
 DECLARE drb_define_admin_db(ddd_dir=vc) = i2
 DECLARE drb_open_br_files(obff_file_name) = i2
 DECLARE drb_get_br_files(null) = i2
 DECLARE drb_close_files(null) = i2
 DECLARE drb_get_zip_file(null) = i2
 FREE RECORD drb_dir
 RECORD drb_dir(
   1 directory = vc
   1 dir_file = vc
   1 p_ccluserdir = vc
   1 to_process = vc
   1 to_process_file = vc
   1 processed = vc
   1 logical_dir = vc
   1 processing = vc
   1 processing_file = vc
   1 file_name = vc
 )
 FREE RECORD drb_br_files
 RECORD drb_br_files(
   1 br_cnt = i4
   1 list[*]
     2 br_suffix = vc
 ...
 SET dm2_rdds_rec->mode = "OS"
 IF (check_logfile("dm2_rdds_bedrock",".log","DM2_RDDS_BEDROCK FILE...") != 1)
  SET dm_err->err_ind = 1
 ...
 ELSE
  SET drb_zip_str = "*.zip"
 ENDIF
 CALL echo(build("drb_zip_str =",drb_zip_str))
 SET drb_dir->p_ccluserdir = logical("CCLUSERDIR")
 SET drb_dir->processing = drb_get_dir("rdds_processing")
 SET drb_dir->directory = drb_dir->processing
 SET drb_dir->processing_file = concat(drb_dir->processing,drb_zip_str)
 CALL echo(build("drb_dir->processing_file =",drb_dir->processing_file))
 CALL echo(drb_dir->processing)
 IF (dm2_findfile(drb_dir->processing_file)=1)
  IF (drb_get_zip_file(drb_dir->processing)=0)
   GO TO exit_program
  ENDIF
  IF (unzip_file(drb_dir->processing)=0)
   GO TO exit_program
  ENDIF
 ELSE
  SET drb_dir->to_process = drb_get_dir("rdds_to_process")
  SET drb_dir->to_process_file = concat(drb_dir->to_process,drb_zip_str)
  IF (dm2_findfile(drb_dir->to_process_file)=1)
   IF (drb_get_zip_file(drb_dir->to_process)=0)
    GO TO exit_program
   ENDIF
   IF (drb_move_zip_file(drb_dir->to_process,drb_dir->processing)=0)
    GO TO exit_program
   ENDIF
   IF (unzip_file(drb_dir->processing)=0)
    GO TO exit_program
  ...
  ELSE
   CALL echo(drb_dir->processing)
   SET dm_err->eproc = "There is no zip file to process, exit the program"
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (drb_create_admin_db(null)=0)
  GO TO exit_program
 ENDIF
 IF (drb_define_admin_db(drb_dir->directory)=0)
  GO TO exit_program
 ENDIF
 CALL drb_inc_def(concat(drb_dir->directory,"brtbldoc.def"))
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 CALL drb_inc_def(concat(drb_dir->directory,"brcoldoc.def"))
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 CALL drb_inc_def(concat(drb_dir->directory,"brutc.def"))
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm_err->err_ind = 0
 IF (drb_get_br_files(null)=0)
  GO TO exit_program
 ENDIF
 FOR (drb_lp = 1 TO drb_br_files->br_cnt)
   IF (drb_open_br_files(drb_br_files->list[drb_lp].br_suffix)=0)
    GO TO exit_program
   ENDIF
 ENDFOR
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="BEDROCK_ENV_ID"
  DETAIL
   drb_env_id = di.info_number
  WITH nocounter
 ;end select
 CALL echo(drb_env_id)
 IF (curqual=0)
  SELECT INTO "nl:"
   FROM dm_chg_log$f d
   DETAIL
    drb_env_id = d.target_env_id
   WITH nocounter, maxqual(d,1)
  ;end select
  INSERT  FROM dm_info
   SET info_domain = "DATA MANAGEMENT", info_name = "BEDROCK_ENV_ID", info_number = drb_env_id
   WITH nocounter
  ;end insert
  IF (check_error("Update bedrock environment id")=1)
   ROLLBACK
   GO TO exit_program
  ENDIF
  COMMIT
 ENDIF
 EXECUTE dm2_ref_data_mover_main drb_env_id, 0, 0
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET drb_dir->processed = drb_get_dir("rdds_processed")
 IF (drb_move_zip_file(drb_dir->processing,drb_dir->processed)=1)
  GO TO exit_program
 ENDIF
 SUBROUTINE unzip_file(uf_dir)
   DECLARE uf_command_line = vc
   SET dm_err->eproc = concat("Unzip file ",uf_dir)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (cursys="AXP")
    SET uf_dir = concat(uf_dir,"*.zip")
    IF (dm2_push_dcl(concat("unzip -o ",uf_dir," -d ",uf_dir))=0)
     RETURN(0)
    ENDIF
   ELSE
    CALL dm2_push_dcl(concat("$cer_exe/unzip -d ",uf_dir," -o ",uf_dir,drb_dir->file_name))
    SET dm_err->err_ind = 0
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_get_zip_file(gzf_dir)
   DECLARE gzf_logical_dir = vc
   DECLARE gzf_pos = i4 WITH noconstant(0)
   DECLARE gzf_length = i4 WITH noconstant(0)
   SET dm_err->eproc = "Get the oldest zip file name"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_push_dcl(concat("ls -art ",gzf_dir,drb_zip_str," | head -1 > ",gzf_dir,
     "drb_oldest_zfile.dat"))=0)
    RETURN(0)
   ENDIF
   SET gzf_dir_size = size(gzf_dir)
   SET logical gzf_logical_dir value(concat(gzf_dir,"drb_oldest_zfile.dat"))
   FREE DEFINE rtl
   DEFINE rtl "gzf_logical_dir"
   SELECT INTO "nl:"
    r.*
    FROM rtlt r
    DETAIL
     gzf_length = size(trim(r.line)), drb_dir->file_name = substring((gzf_dir_size+ 1),(gzf_length -
      gzf_pos),trim(r.line))
    WITH nocounter
   ;end select
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("The oldest zip file is ",drb_dir->file_name))
   ENDIF
   IF (dm2_push_dcl(concat("rm ",gzf_dir,"drb_oldest_zfile.dat"))=0)
    RETURN(0)
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_get_dir(dgd_sub_dir)
   DECLARE dgd_user_dir = vc
   SET dgd_user_dir = logical("CCLUSERDIR")
   IF (cursys="AXP")
    SET dgd_user_dir = replace(dgd_user_dir,"]",".",2)
    SET dgd_user_dir = concat(dgd_user_dir,dgd_sub_dir,"]")
   ELSE
    SET dgd_user_dir = concat(dgd_user_dir,"/",dgd_sub_dir,"/")
   ENDIF
   SET dm_err->eproc = concat("Check directory: ",dgd_user_dir)
   CALL disp_msg(" ",dm_err->logfile,0)
   RETURN(dgd_user_dir)
 END ;Subroutine
 SUBROUTINE drb_move_zip_file(dmf_from,dmf_to)
   DECLARE dmtp_move = vc
   SET dm_err->eproc = concat("Move file from ",dmf_from," to ",dmf_to)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (cursys="AXP")
    SET dmtp_move = concat("rename ",dmf_from,drb_dir->file_name," ",dmf_to,
     drb_dir->file_name)
   ELSE
    SET dmtp_move = concat("mv ",dmf_from,drb_dir->file_name," ",dmf_to,
     drb_dir->file_name)
   ENDIF
   CALL echo(dmtp_move)
   IF (dm2_push_dcl(dmtp_move)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_inc_def(table_name)
   SET dm_err->eproc = concat("create table definition for ",table_name)
   CALL disp_msg(" ",dm_err->logfile,0)
   DECLARE id_logic_dir = vc
   FREE RECORD rdef
   RECORD rdef(
     1 lst[*]
       2 ln = vc
   ...
   SET rdef_cnt = 0
   SET logical id_logic_dir value(table_name)
   FREE DEFINE rtl2
   DEFINE rtl2 "id_logic_dir"
   SELECT INTO "nl:"
    r.*
    FROM rtl2t r
    HEAD REPORT
     rdef_cnt = 0
    DETAIL
     rdef_cnt = (rdef_cnt+ 1), stat = alterlist(rdef->lst,rdef_cnt), rdef->lst[rdef_cnt].ln = r.line
    WITH nocounter
   ;end select
   FOR (idef = 1 TO rdef_cnt)
     IF (dm2_push_cmd(rdef->lst[idef].ln,1)=0)
      RETURN
     ENDIF
   ENDFOR
 END ;Subroutine
 SUBROUTINE drb_del_processing_files(null)
   DECLARE dpf_command = vc
   SET dm_err->eproc = "Clean up rdds_processing"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dpf_command = concat(drb_dir->processing,"br*.*")
   IF ((dm_err->err_ind=1))
    SET dm_err->err_ind = 0
    SET drb_err_flag = 1
   ENDIF
   IF (dm2_findfile(dpf_command)=1)
    IF (cursys="AXP")
     SET dpf_command = concat("del ",dpf_command,";*")
     IF (dm2_push_dcl(dpf_command)=0)
      RETURN(0)
     ENDIF
    ELSE
     SET dpf_command = concat("rm ",dpf_command)
     IF (dm2_push_dcl(dpf_command)=0)
      RETURN(0)
     ENDIF
    ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_create_admin_db(null)
   SET dm_err->eproc = "DROPPING AND RECREATING DATABASE"
   CALL disp_msg(" ",dm_err->logfile,0)
   DROP DATABASE brtbldoc WITH deps_deleted
   DROP DATABASE brcoldoc WITH deps_deleted
   DROP DATABASE brutc WITH deps_deleted
   CREATE DATABASE brtbldoc
   ORGANIZATION(sequential)
   FORMAT(fixed)
   SIZE(32000)
   CREATE DATABASE brcoldoc
   ORGANIZATION(sequential)
   FORMAT(fixed)
   SIZE(32000)
   CREATE DATABASE brutc
   ORGANIZATION(sequential)
   FORMAT(fixed)
   SIZE(32000)
   IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_define_admin_db(ddd_dir)
   SET dm_err->eproc = "Define database BRUTC, BRTBLDOC and BRCOLDOC"
   CALL disp_msg(" ",dm_err->logfile,0)
   DECLARE drb_log_dir = vc
   SET logical drb_log_dir value(ddd_dir)
   FREE DEFINE brtbldoc
   FREE DEFINE brcoldoc
   FREE DEFINE brutc
   IF (dm2_push_cmd("define BRTBLDOC is 'drb_log_dir:brtbldoc.dat' go",1)=0)
    RETURN(0)
   ENDIF
   IF (dm2_push_cmd("define BRCOLDOC is 'drb_log_dir:brcoldoc.dat' go",1)=0)
    RETURN(0)
   ENDIF
   IF (dm2_push_cmd("define BRUTC is 'drb_log_dir:brutc.dat' go",1)=0)
    RETURN(0)
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_open_br_files(obf_file_name)
   SET dm_err->eproc = concat("Open files ",obf_file_name)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_push_cmd(concat("free define ",obf_file_name," go"),1)=0)
    RETURN(0)
   ENDIF
   IF (dm2_push_cmd(concat("drop database ",obf_file_name," with deps_deleted go"),1)=0)
    RETURN(0)
   ENDIF
   IF (obf_file_name="br7773")
    IF (dm2_push_cmd(concat("create database ",obf_file_name,
      " organization (indexed) format(variable) size(1098) unique key 1 (0, 8) go"),1)=0)
     RETURN(0)
    ENDIF
   ELSEIF (obf_file_name="br8680")
    IF (dm2_push_cmd(concat("create database ",obf_file_name,
      " organization (indexed) format(variable) size(122) unique key 1 (0, 76) go"),1)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (dm2_push_cmd(concat("create database ",obf_file_name,
      " organization (sequential) format (fixed) size (32000) go"),1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (dm2_push_cmd(concat("set logical = brlog value(drb_dir->directory) go"),1)=0)
    RETURN(0)
   ENDIF
   IF (dm2_push_cmd(concat("Define ",obf_file_name,' is "brlog:',obf_file_name,'.dat" with modify go'
     ),1)=0)
    RETURN(0)
   ENDIF
   CALL drb_inc_def(concat(drb_dir->directory,obf_file_name,".def"))
   IF ((dm_err->err_ind=1))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_get_br_files(null)
   DECLARE dbf_cnt = i4 WITH noconstant(0)
   SET dm_err->eproc = "Get all br* files in rdds_prcessing"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT DISTINCT INTO "nl:"
    dtd.full_table_name
    FROM dm_tables_doc$f dtd
    DETAIL
     dbf_cnt = (dbf_cnt+ 1)
     IF (mod(dbf_cnt,100)=1)
      stat = alterlist(drb_br_files->list,(dbf_cnt+ 99))
     ENDIF
     drb_br_files->list[dbf_cnt].br_suffix = build("br",dtd.table_suffix)
    FOOT REPORT
     stat = alterlist(drb_br_files->list,dbf_cnt), drb_br_files->br_cnt = dbf_cnt
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    RETURN(0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(drb_br_files)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_close_files(null)
   DECLARE dcf_lp = i4
   SET dm_err->eproc = "Close all the $F files"
   CALL disp_msg(" ",dm_err->logfile,0)
   FOR (dcf_lp = 1 TO drb_br_files->br_cnt)
     IF (dm2_push_cmd(concat("free define ",drb_br_files->list[dcf_lp].br_suffix," go"),1)=0)
      RETURN(0)
     ENDIF
   ENDFOR
   FREE DEFINE brtbldoc
...
#exit_program
 IF (drb_err_flag=1)
  SET dm_err->err_ind = 1
 ENDIF
 SET dm_err->eproc = "dm2_rdds_bedrock Completes"
 CALL final_disp_msg("dm2_rdds_bedrock")
END GO
