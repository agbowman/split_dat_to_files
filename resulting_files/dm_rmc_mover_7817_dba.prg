CREATE PROGRAM dm_rmc_mover_7817:dba
 ...
 DECLARE cust_tname_fc = vc
 DECLARE cust_default_value = vc
 DECLARE cust_new_value = vc
 DECLARE cust_type_flag = i4
 DECLARE cust_value_pos = i4
 DECLARE cust_pe_name = vc
 DECLARE cust_pe_col = vc
 DECLARE cust_loop = i4
 DECLARE cust_rpt_missing = vc
 DECLARE cust_src_table = vc
 DECLARE cust_orig_pos = i4
 DECLARE cust_action_id = f8
 DECLARE cust_orig_id = f8
 DECLARE cust_orig_name = vc
 DECLARE cust_to_orig_id = f8
 DECLARE cust_loop = i4
 DECLARE cust_err_val = i4
 DECLARE cust_active_ind = i2
 DECLARE cust_cnt = i4
 DECLARE cust_vers_nbr = i4
 DECLARE cust_drm7817_context_name = vc
 DECLARE cust_nomv_chk = vc WITH protect, noconstant("")
 DECLARE cust_action_pos = i4
 DECLARE cust_insert_dummy(null) = f8
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_src_table = dm2_get_rdds_tname(cust_tab_name)
 SET cust_orig_id = cnvtreal(get_value(cust_tab_name,"ORIGINAL_REF_ACTION_ID","FROM"))
 SET cust_active_ind = cnvtint(get_value(cust_tab_name,"ACTIVE_IND","FROM"))
 SET cust_action_id = cnvtreal(get_value(cust_tab_name,"SA_REF_ACTION_ID","FROM"))
 SET cust_orig_pos = get_col_pos(cust_tab_name,"ORIGINAL_REF_ACTION_ID")
 SET cust_action_pos = get_col_pos(cust_tab_name,"SA_REF_ACTION_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_orig_pos].translated=0))
  SET cust_nomv_chk = select_merge_translate(cnvtstring(cust_orig_id),cust_tab_name)
  IF (cust_nomv_chk="NOMV21")
   SET insert_update_reason = "NOMV21"
   SET no_insert_update = 1
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV21"
   ...
   GO TO exit_7817
  ENDIF
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_action_pos].translated=0))
   SET cust_nomv_chk = select_merge_translate(cnvtstring(cust_action_id),cust_tab_name)
   IF (cust_nomv_chk="NOMV21")
    SET insert_update_reason = "NOMV21"
    SET no_insert_update = 1
    SET dm2_ref_data_reply->error_ind = 1
   ...
   ENDIF
  ENDIF
  IF (cust_orig_id != cust_action_id)
   IF (cust_active_ind=0)
    SELECT INTO "NL:"
     FROM (parser(cust_src_table) s)
     WHERE s.original_ref_action_id=cust_orig_id
      AND s.original_ref_action_id != s.sa_ref_action_id
      AND active_ind=1
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
    IF (curqual=1)
     CALL put_value(cust_tab_name,"ORIGINAL_REF_ACTION_ID",cnvtstring(cust_orig_id))
    ELSE
     SET cust_cnt = 0
     SELECT INTO "NL:"
      FROM sa_ref_action s
      WHERE (s.action_name_key=rs_7817->from_values.action_name_key)
       AND (s.value_type_flag=rs_7817->from_values.value_type_flag)
       AND s.sa_ref_action_id=s.original_ref_action_id
      DETAIL
       cust_cnt = (cust_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,cust_cnt),
       ui_query_eval_rec->qual[cust_cnt].root_entity_attr = s.original_ref_action_id
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ...
      GO TO exit_7817
     ENDIF
     SET cust_to_orig_id = evaluate_exec_ui_query(cust_cnt,temp_tbl_cnt,perm_col_cnt)
     IF (cust_to_orig_id > 0)
      CALL put_value(cust_tab_name,"ORIGINAL_REF_ACTION_ID",cnvtstring(cust_to_orig_id))
     ELSEIF ((cust_to_orig_id=- (2)))
      SET dm2_ref_data_reply->error_ind = 1
   ...
   ELSE
    ...
    SELECT INTO "NL:"
     ...
     DETAIL
      ...
      ->qual[cust_cnt].root_entity_attr = s.original_ref_action_id
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    SET cust_to_orig_id = evaluate_exec_ui_query(cust_cnt,temp_tbl_cnt,perm_col_cnt)
    IF ((cust_to_orig_id=- (3)))
     SELECT INTO "NL:"
      FROM (parser(cust_src_table) s)
      WHERE s.original_ref_action_id=cust_orig_id
       AND s.sa_ref_action_id=cust_orig_id
       AND (s.action_name_key != rs_7817->from_values.action_name_key)
      DETAIL
       cust_orig_name = s.action_name_key
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     ENDIF
     IF (cust_orig_name="")
      SET cust_to_orig_id = cust_insert_dummy(null)
      IF (nodelete_ind=1)
       GO TO exit_7817
      ENDIF
      CALL put_value(cust_tab_name,"ORIGINAL_REF_ACTION_ID",cnvtstring(cust_to_orig_id))
     ELSE
      ...
      SELECT INTO "NL:"
       FROM sa_ref_action s
       WHERE s.action_name_key=cust_orig_name
        AND (s.value_type_flag=rs_7817->from_values.value_type_flag)
        AND s.sa_ref_action_id=s.original_ref_action_id
       DETAIL
        cust_cnt = (cust_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,cust_cnt),
     ...
     ENDIF
    ELSEIF (cust_to_orig_id > 0)
     CALL insert_merge_translate(cust_orig_id,cust_to_orig_id,"SA_REF_ACTION")
     CALL put_value(cust_tab_name,"ORIGINAL_REF_ACTION_ID",cnvtstring(cust_to_orig_id))
    ELSEIF ((cust_to_orig_id=- (2)))
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV06"
  ...
  ELSE
   ...
   ELSE
    ...
    CALL insert_merge_translate(cust_orig_id,cust_to_orig_id,"SA_REF_ACTION")
    CALL put_value(cust_tab_name,"ORIGINAL_REF_ACTION_ID",cnvtstring(cust_to_orig_id))
    CALL put_value(cust_tab_name,"SA_REF_ACTION_ID",cnvtstring(cust_to_orig_id))
    CALL put_value(cust_tab_name,"VERSION_NBR","1")
   ENDIF
  ENDIF
 ELSE
  IF (cust_active_ind=1)
   SET cust_to_orig_id = cnvtreal(get_value(cust_tab_name,"ORIGINAL_REF_ACTION_ID","TO"))
   SELECT INTO "NL:"
    FROM sa_ref_action s
    WHERE s.original_ref_action_id=cust_to_orig_id
     AND s.active_ind=1
    DETAIL
     cust_vers_nbr = s.version_nbr
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (curqual=0)
    SET cust_vers_nbr = 1
   ENDIF
   CALL put_value(cust_tab_name,"VERSION_NBR",cnvtstring(cust_vers_nbr))
  ENDIF
 ENDIF
 EXECUTE dm_rmc_data_version
 SUBROUTINE cust_insert_dummy(null)
   DECLARE cid_new_value = f8
   DECLARE cid_ptam_result = f8
   DECLARE temp_$r_tbl = vc
   SET temp_$r_tbl = cutover_tab_name("SA_REF_ACTION","7817")
   ...
   SET cid_ptam_result = drmm_get_ptam_match_result(0.0,dm2_ref_data_doc->env_source_id,
    dm2_ref_data_doc->env_target_id,dm2_ref_data_doc->post_link_name,0,
    "FROM")
   SELECT INTO "NL:"
    y = seq(reference_seq,nextval)
   ...
   ;end select
   IF ((global_mover_rec->context_ind=0))
    SET cust_drm7817_context_name = drdm_chg->log[drdm_log_loop].context_name
   ELSE
    SET cust_drm7817_context_name = drdm_oracle_context
   ENDIF
   INSERT  FROM (parser(temp_$r_tbl))
    SET original_ref_action_id = cid_new_value, sa_ref_action_id = cid_new_value, version_nbr = 1,
     active_ind = 1, action_description = "RDDS FILL ROW", action_name_key = rs_7817->from_values.
     action_name_key,
     action_name = rs_7817->from_values.action_name, value_type_flag = rs_7817->from_values.
     value_type_flag, updt_task = 4310001,
     updt_id = 0, updt_dt_tm = cnvtdatetime(curdate,curtime3), rdds_status_flag = 0,
     rdds_dt_tm = cnvtdatetime(curdate,curtime3), rdds_log_id = drdm_chg->log[drdm_log_loop].log_id,
     rdds_context_name = cust_drm7817_context_name,
     rdds_source_env_id = dm2_ref_data_doc->env_source_id, rdds_delete_ind = 0,
     rdds_ptam_match_result = cid_ptam_result
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    IF (((findstring("ORA-00001:",dm_err->emsg) > 0) OR (((findstring("ORA-02049:",dm_err->emsg) > 0)
    ...
    ENDIF
   ELSE
    CALL insert_merge_translate(cust_orig_id,cid_new_value,"SA_REF_ACTION")
   ENDIF
   RETURN(cid_new_value)
 END ;Subroutine
#exit_7817
END GO
