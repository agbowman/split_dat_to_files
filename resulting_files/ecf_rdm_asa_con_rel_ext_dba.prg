CREATE PROGRAM ecf_rdm_asa_con_rel_ext:dba
 ...
 SUBROUTINE kia_script_inc(pkg,ccl_obj)
   ...
   CALL parser(obj_prs)
 END ;Subroutine
 DECLARE kia_concept_filename = vc WITH public, constant("asa_concept.csv")
 DECLARE kia_reltn_filename = vc WITH public, constant("asa_relation.csv")
 DECLARE kia_ext_csv_filename = vc WITH public, constant("asa_cmt_extension.csv")
 DECLARE kia_cur_ver_beg_dt = vc WITH public, noconstant("")
 SET kia_tbl_desc = "ASA CONCEPT"
 SET kia_short_desc = "ASA CONCEPT"
 SET kia_rdm_log_name = "ecf_rdm_asa_con_rel_ext.log"
 SET kia_rdm_nbr = 9982
 SET kia_rdm_vocab_mean = "ASA"
 SET readme_data->status = "F"
 ...
 EXECUTE kia_dm_dbimport "cer_install:asa_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("ecf_rdm_asa_con_rel_ext Begin> ",format(cnvtdatetime(curdate,curtime3),
 ...
 IF (kia_obj_suc_ind=0)
  ...
  DELETE  FROM cmt_concept_load_ns c
   WHERE ((c.concept_cki="ASA*") OR (c.concept_cki="SNOMED*"))
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
 ...
 IF (kia_obj_suc_ind != 1)
  ...
  DELETE  FROM cmt_concept_extension c
   WHERE c.concept_cki="ASA*"
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_ext_csv_filename,"kia_imp_cmt_concept_ext","kia_imp_asa_ext.log")
  CALL kia_chk_error(1)
 ...
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(9981,"ASA NOMEN"))
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(9982,"ASA CONCEPT"))
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(9983,"ASA XMAP"))
