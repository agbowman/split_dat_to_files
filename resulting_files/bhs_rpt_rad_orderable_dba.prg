CREATE PROGRAM bhs_rpt_rad_orderable:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (sa
   WHERE (sa.appt_location_cd= $MF_APPT_LOC)
    ...
    AND sa.state_meaning IN ("CONFIRMED", "PENDING")
   ...
   JOIN (ord
   WHERE ord.order_id=sea.order_id
    AND (ord.catalog_cd= $MF_CAT_CODE))
   JOIN (oa
   ...
   JOIN (off
   WHERE (off.oe_field_id= Outerjoin(sed.oe_field_id))
    AND (off.label_text= Outerjoin("Scheduling Referring Physician")) )
