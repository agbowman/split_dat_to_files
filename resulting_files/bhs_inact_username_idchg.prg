CREATE PROGRAM bhs_inact_username_idchg
 PROMPT
  "Output to File/Printer/MINE" = "MINE"
  WITH outdev
 UPDATE  FROM prsnl p
  SET p.username = concat(p.username,"Y",format(curdate,"YYMM;;d")), p.active_status_cd = 194, p
  ...
  WHERE p.physician_ind != 1
   ...
   AND ((p.username="Z99999999") OR (p.username="CN013355"))
  WITH maxrec = 100
 ;end update
 COMMIT
END GO
