CREATE PROGRAM bed_aud_ap_ords_proc_grps:dba
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   PLAN (oc
    ...
    WHERE apgr.task_assay_cd=outerjoin(dta.task_assay_cd)
     AND apgr.parent_entity_name=outerjoin("CODE_VALUE"))
    JOIN (cv
    WHERE cv.code_value=outerjoin(apgr.parent_entity_id)
     AND cv.code_set=outerjoin(1310)
     AND cv.active_ind=outerjoin(1))
   ORDER BY oc.primary_mnemonic, dta.mnemonic, cv.display,
    apgr.sequence, oc.catalog_cd, dta.task_assay_cd
   HEAD oc.catalog_cd
 ...
 SELECT DISTINCT INTO "NL:"
  ...
  DETAIL
   IF (cv.code_value > 0
    AND cv.active_ind=1)
    pcnt = (pcnt+ 1), stat = alterlist(temp->tqual[tcnt].assays[acnt].proc_groups,pcnt), temp->tqual[
    tcnt].assays[acnt].proc_groups[pcnt].display = cv.display
   ENDIF
  WITH nocounter
 ;end select
 ...
 SET reply->collist[5].header_text = "Processing Group"
 ...
 FOR (x = 1 TO tcnt)
   ...
   FOR (a = 1 TO acnt)
    SET pcnt = size(temp->tqual[x].assays[a].proc_groups,5)
    FOR (p = 1 TO pcnt)
      IF ((((temp->tqual[x].active_ind=0)) OR ((temp->tqual[x].assays[a].active_ind=0))) )
       ...
       ENDIF
       SET reply->rowlist[row_nbr].celllist[5].string_value = temp->tqual[x].assays[a].proc_groups[p]
       .display
       SET ord_has_issue = 1
      ENDIF
    ENDFOR
 ...
 SET reply->statlist[1].statistic_meaning = "APORDSASSAYSPROCGRPS"
...
#exit_script
 ...
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("ap_inactive_ords_with_active_proc_groups.csv")
 ENDIF
