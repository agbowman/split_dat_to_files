CREATE PROGRAM dm_him_dsm_assessment_rows:dba
 ...
 SET reply->table_name = "DSM_ASSESSMENT"
 ...
 SELECT INTO "nl:"
  dsm.rowid
  FROM dsm_assessment dsm
  WHERE parser(sbr_getrowidnotexists(
    "dsm.updt_dt_tm+0 < cnvtdatetime(curdate - v_days_to_keep, curtime3)","dsm"))
   AND ((dsm.dsm_assessment_id+ 0) > 0)
  HEAD REPORT
   row_cnt = 0
  DETAIL
   row_cnt = (row_cnt+ 1)
   ...
   ENDIF
   reply->rows[row_cnt].row_id = dsm.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,row_cnt)
  WITH nocounter, maxqual(dsm,value(request->max_rows))
 ;end select
