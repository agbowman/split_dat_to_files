CREATE PROGRAM dc_mp_get_sdoh
 ...
 IF (( $2 != null))
  ...
  IF (validate(debug_dc_mp_get_sdoh_ind,0)=1)
 ...
 IF (checkdic("LH_MP_ORGANIZER_SDOH","P",0) > 0)
  EXECUTE lh_mp_organizer_sdoh  WITH replace("REQUEST",qmreq), replace("REPLY",cacreply), replace(
   "FLEXID",_flexid),
...
#exit_script
 IF (validate(debug_dc_mp_get_sdoh_ind,0)=1)
  CALL echorecord(sdohreply)
 ENDIF
 IF (validate(_memory_reply_string)=1)
  SET _memory_reply_string = cnvtrectojson(sdohreply)
 ELSE
  CALL echojson(sdohreply, $1)
 ENDIF
END GO
