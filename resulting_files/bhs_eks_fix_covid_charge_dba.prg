CREATE PROGRAM bhs_eks_fix_covid_charge:dba
 ...
 SELECT INTO "nl:"
  FROM encounter e
  PLAN (e
   WHERE e.encntr_id IN (175599821.00))
  HEAD REPORT
   knt = 0, eksopsrequest->expert_trigger = "BHS_EKS_FIX_COVID_CHARGE"
  DETAIL
   knt += 1, stat = alterlist(eksopsrequest->qual,knt), eksopsrequest->qual[knt].person_id = e
   .person_id,
   eksopsrequest->qual[knt].encntr_id = e.encntr_id, eksopsrequest->qual[knt].order_id = 0.00
  WITH nocounter
 ;end select
 CALL echo(build2("knt: ",build(knt)))
