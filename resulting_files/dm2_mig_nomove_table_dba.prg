CREATE PROGRAM dm2_mig_nomove_table:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmnt_user = vc WITH protect, noconstant("")
 DECLARE dmnt_table = vc WITH protect, noconstant("")
 DECLARE dmnt_input = vc WITH protect, noconstant(cnvtupper( $1))
 DECLARE dmnt_user_only = i2 WITH protect, noconstant(0)
 DECLARE dmnt_dt_ind = i2 WITH protect, noconstant(0)
 DECLARE dmnt_ut_ind = i2 WITH protect, noconstant(0)
 DECLARE dmnt_excl_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_nm_table",".log","dm2_mig_nomove_table")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_nomove_table"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 IF (findstring(".",dmnt_input,1,1) > 0)
  SET dmnt_user = substring(1,(findstring(".",dmnt_input) - 1),dmnt_input)
  SET dmnt_table = substring((findstring(".",dmnt_input)+ 1),size(dmnt_input),dmnt_input)
 ELSE
  SET dmnt_user = dmnt_input
  SET dmnt_user_only = 1
 ENDIF
 SET dm_err->eproc = "Validating username supplied."
 SELECT INTO "nl:"
  a.username
  FROM all_users a
  WHERE a.username=dmnt_user
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ELSEIF (((curqual=0) OR (dmnt_user="V500"
  AND dmnt_user_only=1)) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Username ",dmnt_user," is not a valid user.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 IF (dmnt_user_only=0)
  SET dm_err->eproc = "Validating table supplied."
  SELECT INTO "nl:"
   d.table_name
   FROM dba_tables d
   WHERE d.owner=dmnt_user
    AND d.table_name=dmnt_table
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) > 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  ...
  ELSEIF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Table name ",dmnt_table," is not a valid table.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (dmnt_user_only=1)
  SET dm_err->eproc = "Check if user is DOWNTIME user"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain IN ("DM2_MIG_DT_USER", "DM2_MIG_USER")
    AND d.info_name=dmnt_user
   DETAIL
  ...
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echo(concat("MODE:",build(dmnt_user_only)))
   CALL echo(concat("UT_IND:",build(dmnt_ut_ind)))
   CALL echo(concat("DT_IND:",build(dmnt_dt_ind)))
   CALL echo(concat("EXCL_IND:",build(dmnt_excl_ind)))
  ENDIF
  IF (((dmnt_dt_ind=0
   AND dmnt_ut_ind=1) OR (dmnt_dt_ind=1
   AND dmnt_ut_ind=1)) )
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Username ",dmnt_user,
    " is not a valid user. User currently marked for UPTIME.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_script
  ELSEIF (dmnt_dt_ind=0
   AND dmnt_ut_ind=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Username ",dmnt_user,
    " is not a valid user. User not marked for UPTIME or DOWNTIME.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_script
  ENDIF
 ELSE
  SET dm_err->eproc = "Check if user.table is DOWNTIME user.table"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE ((d.info_domain IN ("DM2_MIG_USER", "DM2_MIG_DT_USER")
    AND d.info_name=dmnt_user) OR (d.info_domain="DM2_MIG_EXCL"
    AND d.info_name=dmnt_input))
   DETAIL
    IF (d.info_domain="DM2_MIG_DT_USER")
     dmnt_dt_ind = 1
    ELSEIF (d.info_domain="DM2_MIG_EXCL")
     dmnt_excl_ind = 1
    ELSEIF (d.info_domain="DM2_MIG_USER")
     dmnt_ut_ind = 1
    ENDIF
   WITH nocounter
  ;end select
  ...
  IF (dmnt_dt_ind=0
   AND dmnt_ut_ind=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Table ",dmnt_input,
    " is not a valid table. Table not marked for UPTIME or DOWNTIME.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Updating DM_INFO exclusion row."
 UPDATE  FROM dm_info di
  SET di.updt_dt_tm = cnvtdatetime(curdate,curtime3), di.info_number = evaluate(dmnt_user_only,1,di
    .info_number,40), di.info_char = "0"
  WHERE di.info_domain=evaluate(dmnt_user_only,1,"DM2_MIG_DT_USER","DM2_MIG_EXCL")
   AND di.info_name=evaluate(dmnt_user_only,1,dmnt_user,dmnt_input)
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc) > 0)
  ROLLBACK
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Insert DM_INFO exclusion row."
  INSERT  FROM dm_info di
   SET di.updt_dt_tm = cnvtdatetime(curdate,curtime3), di.info_number = evaluate(dmnt_user_only,1,0,
     40), di.info_char = "0",
    di.info_domain = evaluate(dmnt_user_only,1,"DM2_MIG_DT_USER","DM2_MIG_EXCL"), di.info_name =
    evaluate(dmnt_user_only,1,dmnt_user,dmnt_input)
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc) > 0)
   ROLLBACK
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_mig_nomove_table completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_nm_table")
END GO
