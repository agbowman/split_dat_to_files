CREATE PROGRAM acm_add_person_alias:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->person_alias_qual[xref->add[index].idx].person_alias_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(person_seq,nextval)
     FROM dual
     DETAIL
      acm_request->person_alias_qual[xref->add[index].idx].person_alias_id = cnvtreal(nextseqnum),
      reply->person_alias_qual[xref->add[index].idx].person_alias_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->person_alias_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->person_alias_qual[xref->add[index].idx].person_alias_id = acm_request->
    person_alias_qual[xref->add[index].idx].person_alias_id
   ENDIF
   SET reply->person_alias_qual[xref->add[index].idx].status = 0
   IF ((acm_request->person_alias_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->person_alias_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->person_alias_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->person_alias_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   person_alias p
  SET p.person_alias_id = acm_request->person_alias_qual[xref->add[d.seq].idx].person_alias_id, p
   .alias = cnvtupper(cnvtalphanum(acm_request->person_alias_qual[xref->add[d.seq].idx].alias)), p
   .alias_pool_cd = acm_request->person_alias_qual[xref->add[d.seq].idx].alias_pool_cd,
   p.assign_authority_sys_cd = acm_request->person_alias_qual[xref->add[d.seq].idx].
   assign_authority_sys_cd, p.beg_effective_dt_tm = cnvtdatetime(acm_request->person_alias_qual[xref
    ->add[d.seq].idx].beg_effective_dt_tm), p.check_digit = acm_request->person_alias_qual[xref->add[
   d.seq].idx].check_digit,
   p.check_digit_method_cd = acm_request->person_alias_qual[xref->add[d.seq].idx].
   check_digit_method_cd, p.contributor_system_cd =
   IF ((acm_request->person_alias_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->person_alias_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , p.end_effective_dt_tm = cnvtdatetime(acm_request->person_alias_qual[xref->add[d.seq].idx].
    end_effective_dt_tm),
   p.health_card_expiry_dt_tm =
   IF ((acm_request->person_alias_qual[xref->add[d.seq].idx].health_card_expiry_dt_tm > 0))
    cnvtdatetime(acm_request->person_alias_qual[xref->add[d.seq].idx].health_card_expiry_dt_tm)
   ELSE null
   ENDIF
   , p.health_card_issue_dt_tm =
   IF ((acm_request->person_alias_qual[xref->add[d.seq].idx].health_card_issue_dt_tm > 0))
    cnvtdatetime(acm_request->person_alias_qual[xref->add[d.seq].idx].health_card_issue_dt_tm)
   ELSE null
   ENDIF
   , p.health_card_province = acm_request->person_alias_qual[xref->add[d.seq].idx].
   health_card_province,
   p.health_card_type = acm_request->person_alias_qual[xref->add[d.seq].idx].health_card_type, p
   .health_card_ver_code = acm_request->person_alias_qual[xref->add[d.seq].idx].health_card_ver_code,
   p.person_alias_status_cd = acm_request->person_alias_qual[xref->add[d.seq].idx].
   person_alias_status_cd,
   p.person_alias_sub_type_cd = acm_request->person_alias_qual[xref->add[d.seq].idx].
   person_alias_sub_type_cd, p.person_alias_type_cd = acm_request->person_alias_qual[xref->add[d.seq]
   .idx].person_alias_type_cd, p.person_id =
   IF ((acm_request->person_alias_qual[xref->add[d.seq].idx].person_id > 0)) acm_request->
    person_alias_qual[xref->add[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->person_alias_qual[xref->add[d.seq].idx].person_idx].person_id
   ENDIF
   ,
   p.visit_seq_nbr = acm_request->person_alias_qual[xref->add[d.seq].idx].visit_seq_nbr, p
   .data_status_cd =
   IF ((acm_request->person_alias_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request->
    person_alias_qual[xref->add[d.seq].idx].data_status_cd
   ...
   IF ((((acm_request->person_alias_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((
   acm_request->person_alias_qual[xref->add[d.seq].idx].active_status_cd=reqdata->active_status_cd)
   )) ) 1
   ELSE 0
   ENDIF
   , p.active_status_cd =
   IF ((acm_request->person_alias_qual[xref->add[d.seq].idx].active_status_cd > 0.0)) acm_request->
    person_alias_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->person_alias_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->person_alias_qual[xref->add[index].idx].status=0))
 ...
 ENDFOR
 IF (acm_hist_ind=1)
  EXECUTE acm_add_person_alias_hist
