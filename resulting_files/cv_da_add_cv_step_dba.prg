CREATE PROGRAM cv_da_add_cv_step:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_add_cv_step_vrsn = vc WITH private, constant("20050914")
 ...
 WHILE (i <= size(request->objarray,5))
   ...
   IF (validate(request->objarray[i].cv_step_id,- (0.00001)) <= 0.0)
    ...
    SET request->objarray[i].cv_step_id = did
   ELSE
    SET did = request->objarray[i].cv_step_id
   ENDIF
   IF (validate(reply->insert_ids)=1)
    SET reply->insert_ids[i].id = did
   ENDIF
   INSERT  FROM cv_step c
    SET c.cv_step_id = did, c.cv_proc_id =
     ...
     ENDIF
     , c.step_status_cd =
     IF ((validate(request->objarray[i].step_status_cd,- (0.00001)) != - (0.00001))) validate(request
       ->objarray[i].step_status_cd,- (0.00001))
     ELSE 0.0
     ENDIF
     , c.sequence =
     ...
     c.updt_cnt = 0, c.preliminary_author_id =
 ...
 END ;Subroutine
#end_program
 CALL cv_log_msg_post("002 09/12/11 TK020431")
END GO
