CREATE PROGRAM dm2_mig_compare_data
 ...
 SUBROUTINE dcd_validate_prompt(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_mig_compdata",".log","dm2_mig_compare_data LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm2_compare_rec->src_data_link = "REF_DATA_LINK"
 SET dm_err->eproc = "Checking for default database link..."
 CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
 SELECT INTO "nl:"
  FROM all_db_links adl
  WHERE adl.db_link=patstring(concat(dm2_compare_rec->src_data_link,"*"))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  GO TO exit_script
 ELSEIF (curqual=0)
  SET dm_err->emsg = "REF_DATA_LINK must be set to your source database."
  SET dm_err->err_ind = 1
  GO TO exit_script
 ENDIF
 EXECUTE dm2_compare_data_filelist "cer_install:dm2_mig_compare_data.csv"
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
#exit_script
