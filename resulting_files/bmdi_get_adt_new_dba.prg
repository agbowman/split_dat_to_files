CREATE PROGRAM bmdi_get_adt_new:dba
 ...
 IF ((request->facility_ind=1)
  ...
  ELSE
   IF (cnvtupper(trim(loc_mean))="BUILDING")
    SELECT INTO "nl:"
    ...
    ;end select
   ELSE
    IF (((cnvtupper(trim(loc_mean))="NURSEUNIT") OR (cnvtupper(trim(loc_mean))="AMBULATORY")) )
     SELECT INTO "nl:"
      FROM location_group lg,
       location_group lg1
      PLAN (lg
     ...
     ;end select
    ELSE
     IF (cnvtupper(trim(loc_mean))="ROOM")
      SELECT INTO "nl:"
       FROM location_group lg,
        location_group lg1,
        location_group lg2
      ...
      ;end select
     ELSE
      IF (cnvtupper(trim(loc_mean))="BED")
       SELECT INTO "nl:"
        FROM location_group lg,
         location_group lg1,
         location_group lg2,
...
#invalid
 IF ((reply->status_data.status="F"))
  ...
  SET reply->status_data.subeventstatus[1].targetobjectname = "bmdi_get_adt_new"
