CREATE PROGRAM dab_pt_transfer_v2:dba
 PROMPT
 ...
 ELSE
  SET pt_transfer->encntr_id = 54559927
  SET printer_disp =  $1
 ENDIF
 ...
 FOR (x = 1 TO pt_transfer->blob_cnt)
  FOR (y = 1 TO pt_transfer->blob_qual[x].blobs_cnt)
    ...
    event_id
    CALL echo(build("pt_transfer->blob_qual[x].blobs_qual[y].event_id:",pt_transfer->blob_qual[x].
      blobs_qual[y].event_id))
    SET pt->line_cnt = 0
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (found=0)
    ...
    row + 1
   ENDIF
   CALL echo("Print Vitals in last 24 hours"), output_display = "", output_display1 = "",
   output_display2 = ""
   IF (ycol > 725)
    BREAK
   ENDIF
   ...
   ELSE
    ...
    ycol = (ycol+ 12), row + 1
   ENDIF
   CALL echo("Print Clinical Summary "), output_display = "", output_display1 = "",
   output_display2 = ""
   IF (ycol > 725)
    BREAK
   ENDIF
   ...
   IF (found=0)
    ...
    row + 1
   ENDIF
   CALL echo("Print Cardiovascular	"), found = 0
   IF (ycol > 725)
    BREAK
   ENDIF
   FOR (x = 1 TO pt_transfer->blob_cnt)
