CREATE PROGRAM dm_bmdi_acquired_results_rows:dba
 ...
 SET reply->table_name = "BMDI_ACQUIRED_RESULTS"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 FOR (tok_ndx = 1 TO size(request->tokens,5))
   IF ((request->tokens[tok_ndx].token_str="HOURSTOKEEP"))
    SET v_hours_to_keep = ceil(cnvtreal(request->tokens[tok_ndx].value))
   ENDIF
 ENDFOR
 IF (v_hours_to_keep < 0)
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"HOURSTOKEEP",
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    min_id = min(bar.result_id)
    FROM bmdi_acquired_results bar
    WHERE bar.result_id > 0
    DETAIL
     curminid = maxval(cnvtreal(min_id),1.0)
    WITH nocounter
   ;end select
  ELSE
  ...
  IF (v_hours_to_keep=0)
   SELECT INTO "nl:"
    max_id = max(bar.result_id)
    FROM bmdi_acquired_results bar
    DETAIL
     maxid = cnvtreal(max_id)
    WITH nocounter
   ;end select
  ELSE
   SELECT INTO "nl:"
    max_id = max(bar.result_id)
    FROM bmdi_acquired_results bar
    WHERE bar.updt_dt_tm < cnvtlookbehind(concat(trim(cnvtstring(v_hours_to_keep)),",H"))
    DETAIL
     maxid = cnvtreal(max_id)
    WITH nocounter
   ;end select
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT
     IF (v_hours_to_keep=0)
      WHERE parser(sbr_getrowidnotexists("bar.result_id between curMinID and curMaxID","bar"))
     ELSE
      WHERE parser(sbr_getrowidnotexists("bar.result_id between curMinID and curMaxID","bar"))
       AND bar.updt_dt_tm < cnvtlookbehind(concat(trim(cnvtstring(v_hours_to_keep)),",H"))
     ENDIF
     INTO "nl:"
     bar.rowid
     FROM bmdi_acquired_results bar
     DETAIL
      ...
      ENDIF
      reply->rows[rows].row_id = bar.rowid
     WITH nocounter, maxqual(bar,value(rowsleft))
    ;end select
