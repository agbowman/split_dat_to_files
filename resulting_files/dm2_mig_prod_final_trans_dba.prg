CREATE PROGRAM dm2_mig_prod_final_trans:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmpft_job_state(dmpft_job_name=vc) = i2
 IF (check_logfile("dm2_migprodft",".log","dm2_mig_prod_final_trans")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_prod_final_trans"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Gathering list of tablespaces that are in READ ONLY Mode"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM user_tablespaces ut
  WHERE ut.contents="PERMANENT"
   AND ut.status="READ ONLY"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Check for READ_ONLY_COMPLETE row."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dm2_admin_dm_info
   WHERE info_domain="DM2MIG_FINAL_TRANS_CHECKPOINT"
    AND info_name="READ_ONLY_COMPLETE"
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF (curqual > 0
   AND (validate(dm2_bypass_mig_readonly_complete_chk,- (1))=- (1)))
   SET dm_err->emsg = concat("Existence of READ_ONLY_COMPLETE Admin row is invalid.")
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 EXECUTE dm2_mig_final_trans
 IF ((dm_err->err_ind > 0))
  GO TO exit_program
 ENDIF
 IF (dmpft_job_state("DM2MIGJ_INS_DTT")=0)
  GO TO exit_program
 ENDIF
 IF (dmpft_job_state("DM2MIGJ_DEL_DTT")=0)
  GO TO exit_program
 ENDIF
 SUBROUTINE dmpft_job_state(dmpft_job_name)
   DECLARE dmpft_job_state = vc WITH protect, noconstant("")
   DECLARE ddj_job_name = vc WITH protect, noconstant("")
   SET ddj_job_name = cnvtupper(dmpft_job_name)
   SET dm_err->eproc = concat("Check for ",ddj_job_name," job")
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_scheduler_jobs dj
    WHERE dj.job_name=ddj_job_name
     AND dj.owner="V500"
    DETAIL
     dmpft_job_state = dj.state
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual > 0)
    IF (dmpft_job_state="RUNNING")
     IF (dm2_push_cmd(concat(^RDB ASIS("BEGIN DBMS_SCHEDULER.STOP_JOB('V500.^,ddj_job_name,
       ^',force=>true);END;")GO^),1)=0)
      IF (findstring("ORA-27366",dm_err->emsg) > 0
       AND findstring(ddj_job_name,dm_err->emsg) > 0)
       SET dm_err->err_ind = 0
     ...
     ENDIF
    ENDIF
    IF (dm2_push_cmd(concat(^RDB ASIS("BEGIN DBMS_SCHEDULER.DISABLE('V500.^,ddj_job_name,
      ^');END;")GO^),1)=0)
     RETURN(0)
    ENDIF
   ENDIF
...
#exit_program
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "dm2_mig_prod_final_trans completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_migprodft")
END GO
