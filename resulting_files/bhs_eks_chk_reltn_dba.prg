CREATE PROGRAM bhs_eks_chk_reltn:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (epr
   WHERE epr.encntr_id=mf_eid
    AND epr.prsnl_person_id=mf_curr_user
    AND expand(mn_num,1,mn_cnt,epr.encntr_prsnl_r_cd,pos_list->qual[mn_num].position_cd))
  WITH nocounter
 ;end select
 IF (curqual > 0)
  SET retval = 100
  SET log_message = build2("Success. Position found.")
  GO TO exit_script
 ELSE
  SET retval = 0
  SET log_message = build2("Position not found.")
 ENDIF
#exit_script
