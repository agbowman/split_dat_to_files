CREATE PROGRAM bbt_tag_comp_2d_barcode
 EXECUTE reportrtl
 ...
 SUBROUTINE (detailsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
  WITH protect)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 2.501),(offsety+ 7.875),3.251,2.875,
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 4.375),(offsety+ 3.556),3.695,1.500,
     rpt_nofill,rpt_black)
   ENDIF
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 0.376),(offsety+ 3.556),3.695,1.500,
     rpt_nofill,rpt_black)
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_ProductNumber",build2("Product Number:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 9.744)
   SET rptsd->m_x = (offsetx+ 2.626)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_ProductType",build2("Product Type:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 9.945)
   SET rptsd->m_x = (offsetx+ 2.626)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_ProductExpiration",build2("Product Expiration:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_lblHeadTag",build2("COMPONENT TRANSFUSION TAG",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_lblTagProductUnitCount",build2("Product Unit Count:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 3.112)
   SET rptsd->m_x = (offsetx+ 4.375)
   SET rptsd->m_width = 1.244
   SET rptsd->m_height = 0.202
   IF (ncalc=rpt_render
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_aztec,(offsetx+ 5.195),(offsety+ 8.875))
   ...
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 2.667)
   SET rptsd->m_x = (offsetx+ 4.375)
   SET rptsd->m_width = 3.195
   SET rptsd->m_height = 0.202
   IF (ncalc=rpt_render
    AND bcontinue=0)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(tag_per_event_unit_count,char(0)))
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    ...
    IF (tag_per_event_alias_mrn_raw != " ")
     SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_code39,(offsetx+ 0.320),(offsety+ 1.694))
     SET rptbce->m_recsize = 88
     SET rptbce->m_width = 2.25
   ...
   IF (ncalc=rpt_render
    ...
    IF (tag_per_event_alias_fin_raw != " ")
     SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_code39,(offsetx+ 0.306),(offsety+ 2.931))
     SET rptbce->m_recsize = 88
     SET rptbce->m_width = 2.25
     SET rptbce->m_height = 0.25
     SET rptbce->m_rotation = 0
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_FieldName59",build2("MRN:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_FieldName60",build2("FIN:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 2.695),(offsety+ 7.444),0.112,0.090,
     rpt_nofill,rpt_black)
   ENDIF
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 3.126),(offsety+ 7.444),0.112,0.090,
     rpt_nofill,rpt_black)
   ENDIF
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 4.841),(offsety+ 7.569),0.112,0.090,
     rpt_nofill,rpt_black)
   ENDIF
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 5.250),(offsety+ 7.569),0.112,0.090,
     rpt_nofill,rpt_black)
   ENDIF
   SET rptsd->m_flags = 12
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 0.320)
   SET rptsd->m_x = (offsetx+ 5.820)
   ...
   SET rptsd->m_x = (offsetx+ 4.375)
   SET rptsd->m_width = 1.299
   SET rptsd->m_height = 0.202
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_lblTagSerialNumber",build2("Serial Number:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 9.570)
   SET rptsd->m_x = (offsetx+ 2.626)
   SET rptsd->m_width = 1.028
   SET rptsd->m_height = 0.251
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_FieldName4",build2("Serial Number:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 9.549)
   SET rptsd->m_x = (offsetx+ 3.570)
   SET rptsd->m_width = 1.820
   SET rptsd->m_height = 0.209
   IF (ncalc=rpt_render
   ...
   IF (ncalc=rpt_render
    ...
    IF (tag_per_event_product_type_barcode != " ")
     SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_code128,(offsetx+ 4.375),(offsety+ 2.181))
     SET rptbce->m_recsize = 88
     SET rptbce->m_width = 2.09
     SET rptbce->m_height = 0.22
     SET rptbce->m_rotation = 0
     SET rptbce->m_ratio = 300
     SET rptbce->m_barwidth = 1
     SET rptbce->m_bprintinterp = 0
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BBT_TAG_COMP_2D_BARCODE"
 ...
 END ;Subroutine
 DECLARE selectcomponentquery(dummy) = null WITH protect
 SUBROUTINE selectcomponentquery(ncalc)
   ...
   SELECT INTO "nl:"
    ...
    tag_per_event_dob = tag_from_request->tag_per_event[d1.seq].dob, tag_per_event_prod_nbr =
    substring(1,30,tag_from_request->tag_per_event[d1.seq].prod_nbr),
    ...
    tag_per_event_prepared_by = substring(1,40,tag_from_request->tag_per_event[d1.seq].prepared_by),
    ...
    tag_per_event_crossmatch_interp = substring(1,30,tag_from_request->tag_per_event[d1.seq].
     crossmatch_interp), tag_per_event_crossmatch_verified_by = substring(1,30,tag_from_request->
     tag_per_event[d1.seq].crossmatch_verified_by), tag_per_event_crossmatch_dttm = tag_from_request
    ->tag_per_event[d1.seq].crossmatch_dttm,
    tag_per_event_ordering_physician_name = substring(1,40,tag_from_request->tag_per_event[d1.seq].
     ordering_physician_name), tag_per_event_accession_nbr = substring(1,30,tag_from_request->
     tag_per_event[d1.seq].accession_nbr), tag_per_event_prod_desc = substring(1,30,tag_from_request
     ->tag_per_event[d1.seq].prod_desc),
    tag_per_event_sex = tag_from_request->tag_per_event[d1.seq].sex, tag_per_event_qty_iu = substring
    (1,60,tag_from_request->tag_per_event[d1.seq].qty_iu), tag_per_event_derivative_ind =
    tag_from_request->tag_per_event[d1.seq].derivative_ind,
    ...
    address_facility_disp = substring(1,40,tag_from_request->tag_per_event[d1.seq].address[d2.seq].
     facility_disp), tag_per_event_dispense_physician_name = substring(1,30,tag_from_request->
     tag_per_event[d1.seq].dispense_physician_name), tag_from_request_bbid_preference =
    tag_from_request->bbid_preference,
    tag_per_event_alias_mrn_raw = substring(1,30,tag_from_request->tag_per_event[d1.seq].
     alias_mrn_raw), tag_per_event_alias_fin_raw = substring(1,30,tag_from_request->tag_per_event[d1
     .seq].alias_fin_raw), tag_per_event_patient_aborh_barcode = substring(1,30,tag_from_request->
     tag_per_event[d1.seq].patient_aborh_barcode),
    tag_per_event_unit_number_barcode = substring(1,30,tag_from_request->tag_per_event[d1.seq].
     unit_number_barcode), tag_per_event_product_type_barcode = substring(1,30,tag_from_request->
     tag_per_event[d1.seq].product_type_barcode), tag_per_event_product_type_barcode_nbr = substring(
     1,30,tag_from_request->tag_per_event[d1.seq].product_type_barcode_nbr)
    ...
    HEAD REPORT
     ...
     _d6 = tag_per_event_dob, _d7 = tag_per_event_prod_nbr, _d8 = tag_per_event_serial_nbr,
     _d9 = tag_per_event_prod_aborh_disp, _d10 = tag_per_event_prod_expiration, _d11 =
     tag_per_event_prod_volume,
     _d12 = tag_per_event_unit_count, _d13 = tag_per_event_prepared_dttm, _d14 =
     tag_per_event_prepared_by,
     _d15 = tag_per_event_patient_trans_req, _d16 = tag_per_event_patient_antibodies, _d17 =
     tag_per_event_product_special_testing,
     _d18 = tag_per_event_product_antigen, _d19 = tag_per_event_owninvarea_disp, _d20 =
     tag_per_event_prod_desc,
     _d21 = tag_per_event_sex, _d22 = tag_per_event_qty_iu, _d23 = tag_per_event_derivative_ind,
     _d24 = address_enc_facility_address1, _d25 = address_enc_facility_address2, _d26 =
     address_enc_facility_address3,
     _d27 = address_enc_facility_address4, _d28 = address_enc_facility_citystatezip, _d29 =
     address_enc_facility_country,
     _d30 = address_facility_disp, _d31 = tag_per_event_dispense_physician_name, _d32 =
   ...
   ;end select
 END ;Subroutine
 EXECUTE cpm_create_file_name_logical "bbt_tag_comp_2D", "dat", "x"
