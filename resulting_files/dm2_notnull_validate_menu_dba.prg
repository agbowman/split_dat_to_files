CREATE PROGRAM dm2_notnull_validate_menu:dba
 ...
 END ;Subroutine
 IF (check_logfile("dm2_nnv_menu",".log","dm2_notnull_validate_menu")=0)
  GO TO exit_program
 ENDIF
 ...
 WHILE (true)
   ...
   CASE (curaccept)
    OF 0:
     GO TO exit_program
    OF 1:
     IF (der_email_setup(dpq_notnull_validate)=0)
      GO TO exit_program
     ENDIF
     IF ((der_emails->cnt > 0))
      SET dm2_process_event_rs->status = dpl_complete
      CALL dm2_process_log_add_detail_text("EMAIL LIST",der_emails->email_list)
      IF (dm2_process_log_row(dpl_notnull_validate,dpl_auditlog,dpl_no_prev_id,1)=0)
       GO TO exit_program
      ENDIF
     ENDIF
   ENDCASE
...
#exit_program
 ...
 SET message = nowindow
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_notnull_validate_menu has completed"
 ENDIF
 CALL final_disp_msg("dm2_notnull_validate_menu")
END GO
