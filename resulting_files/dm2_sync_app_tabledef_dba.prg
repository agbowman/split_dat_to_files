CREATE PROGRAM dm2_sync_app_tabledef:dba
 ...
 SUBROUTINE dsr_chk_sd_in_process(dcsip_db_group,dcsip_owner,dcsip_sd_in_process)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsat_str = vc WITH protect, noconstant(" ")
 DECLARE dsat_domain_name = vc WITH protect, noconstant(" ")
 DECLARE dsat_info_nbr = i4 WITH protect, noconstant(0)
 DECLARE dsat_idx = i4 WITH protect, noconstant(0)
 DECLARE dsat_row_cnt = i4 WITH protect, noconstant(0)
 DECLARE dsat_err_msg = vc WITH protect, noconstant(" ")
 DECLARE dsat_err_ind = i4 WITH protect, noconstant(0)
 DECLARE dsat_sd_in_use_ind = i2 WITH protect, noconstant(0)
 DECLARE dsat_get_session_id() = c30 WITH sql = "dbms_session.unique_session_id", parameter
 DECLARE dsat_system_defs_ind = i2 WITH protect, noconstant(0)
 FREE RECORD dsat_opid_list
 RECORD dsat_opid_list(
   1 cnt = i4
   1 qual[*]
     2 op_id = i4
 )
 FREE RECORD dsat_single_list
 RECORD dsat_single_list(
   1 cnt = i4
   1 qual[*]
     2 table_name = vc
 )
 IF (check_logfile("dm2syncapp_tbldef",".log","dm2_sync_app_tabledef")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_sync_app_tabledef"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dsat_domain_name = cnvtupper(trim(logical("environment")))
 SET dm_err->eproc = "Check cronjob heartbeat row into dm_info"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  tmp_row_cnt = count(*)
  FROM dm_info d
  WHERE d.info_domain=concat("DM2_SYNC_APP_TABLEDEF_",dsat_domain_name)
   AND d.info_name=cnvtupper(curnode)
  DETAIL
   dsat_row_cnt = tmp_row_cnt, dsat_info_nbr = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (dsat_row_cnt=0)
  SET dm_err->eproc = "Insert cronjob heartbeat row into dm_info"
  CALL disp_msg(" ",dm_err->logfile,0)
  INSERT  FROM dm_info d
   SET d.info_domain = concat("DM2_SYNC_APP_TABLEDEF_",dsat_domain_name), d.info_name = cnvtupper(
     curnode), d.info_date = cnvtdatetime(curdate,curtime3),
    d.info_number = 1
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
  ...
  ELSE
   COMMIT
  ENDIF
 ELSEIF (dsat_row_cnt > 0
  AND dsat_info_nbr=0)
  SET dm_err->eproc = "Updating cronjob heartbeat row into dm_info"
  CALL disp_msg(" ",dm_err->logfile,0)
  UPDATE  FROM dm_info d
   SET d.info_number = 1, d.info_date = cnvtdatetime(curdate,curtime3)
   WHERE d.info_domain=concat("DM2_SYNC_APP_TABLEDEF_",dsat_domain_name)
    AND d.info_name=cnvtupper(curnode)
   WITH nocounter
  ;end update
 ...
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Exiting due to existing process running on the node"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Check if priority 650 rows exists in dm2_ddl_ops_log"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm2_ddl_ops_log ddol
  WHERE ddol.priority=650
   AND ddol.op_type="EXECUTE DM2_UPDATE_PKG_TABLEDEF*"
   AND ddol.obj_name=cnvtupper(curnode)
   AND ddol.status = null
   AND ddol.ready_to_run_ind=1
  DETAIL
   dsat_opid_list->cnt = (dsat_opid_list->cnt+ 1), stat = alterlist(dsat_opid_list->qual,
    dsat_opid_list->cnt), dsat_opid_list->qual[dsat_opid_list->cnt].op_id = ddol.op_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dsat_opid_list)
 ENDIF
 IF ((dsat_opid_list->cnt > 0))
  SET dm2_oragen_system_defs = 1
  EXECUTE dm2_create_system_defs
  SET dsat_system_defs_ind = 1
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
  SET dm2_oragen_system_defs = 0
 ENDIF
 FOR (dsat_idx = 1 TO dsat_opid_list->cnt)
   SET dm_err->eproc = build("Updating dm2_ddl_ops_log row to running for op_id: ",dsat_opid_list->
    qual[dsat_idx].op_id)
   CALL disp_msg(" ",dm_err->logfile,0)
   UPDATE  FROM dm2_ddl_ops_log ddol
    SET ddol.status = "RUNNING", ddol.begin_dt_tm = cnvtdatetime(curdate,curtime3), ddol.logfile_name
      = dm_err->logfile,
     ddol.appl_id = currdbhandle
    WHERE ddol.priority=650
     AND (ddol.op_id=dsat_opid_list->qual[dsat_idx].op_id)
     AND ddol.status = null
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
   ...
   ELSE
    COMMIT
   ENDIF
   SET dm_err->eproc = "Querying dm2_ddl_ops_log status RUNNING for priority 650"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE ddol.priority=650
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = "Executing dm2_update_pkg_tabledef"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_push_cmd(dsat_str,1)=0)
    GO TO exit_program
   ENDIF
   SET dsat_err_ind = dm_err->err_ind
   SET dsat_err_msg = dm_err->emsg
   SET dm_err->err_ind = 0
   SET dm_err->emsg = " "
   IF (dsat_err_ind=0)
    SET dm_err->eproc = build("Updating dm2_ddl_ops_log row to complete for op_id: ",dsat_opid_list->
     qual[dsat_idx].op_id)
    CALL disp_msg(" ",dm_err->logfile,0)
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = "COMPLETE", ddol.end_dt_tm = cnvtdatetime(curdate,curtime3), ddol.logfile_name
       = dm_err->logfile,
      ddol.appl_id = currdbhandle
     WHERE ddol.priority=650
      AND (ddol.op_id=dsat_opid_list->qual[dsat_idx].op_id)
    ...
    ENDIF
   ELSE
    SET dm_err->eproc = build("Updating dm2_ddl_ops_log row to error for op_id: ",dsat_opid_list->
     qual[dsat_idx].op_id)
    CALL disp_msg(" ",dm_err->logfile,0)
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = "ERROR", ddol.error_msg = dsat_err_msg, ddol.end_dt_tm = cnvtdatetime(curdate,
       curtime3),
      ddol.logfile_name = dm_err->logfile, ddol.appl_id = currdbhandle
     WHERE ddol.priority=650
      AND (ddol.op_id=dsat_opid_list->qual[dsat_idx].op_id)
      AND ddol.status="RUNNING"
     WITH nocounter
   ...
   SET dsat_str = " "
 ENDFOR
 IF (dsr_sd_in_use_check("DM2NOTSET",dsat_sd_in_use_ind)=0)
  GO TO exit_program
 ENDIF
 IF (dsat_sd_in_use_ind=1)
  IF (dsr_usage_prep("DM2NOTSET")=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Check if priority 650 rows exists in sd_ddl_ops"
  CALL disp_msg(" ",dm_err->logfile,0)
  SET stat = alterlist(dsat_opid_list->qual,0)
  SET dsat_opid_list->cnt = 0
  SELECT INTO "nl:"
   FROM (ceradm.sd_ddl_ops sdo)
   WHERE sdo.priority=650
    AND sdo.op_type="EXECUTE DM2_UPDATE_SD_TABLEDEF*"
    AND sdo.obj_name=cnvtupper(curnode)
    AND sdo.obj_owner=currdbuser
    AND sdo.status = null
    AND sdo.ready_to_run_ind=1
   ORDER BY sdo.tbl_name
   DETAIL
    dsat_opid_list->cnt = (dsat_opid_list->cnt+ 1), stat = alterlist(dsat_opid_list->qual,
     dsat_opid_list->cnt), dsat_opid_list->qual[dsat_opid_list->cnt].op_id = sdo.sd_ddl_ops_id
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  FOR (dsat_idx = 1 TO dsat_opid_list->cnt)
    SET dm_err->eproc = build("Updating sd_ddl_ops row to running for sd_ddl_ops_id: ",dsat_opid_list
     ->qual[dsat_idx].op_id)
    CALL disp_msg(" ",dm_err->logfile,0)
    UPDATE  FROM (ceradm.sd_ddl_ops sdo)
     SET sdo.status = "RUNNING", sdo.begin_dt_tm = cnvtdatetime(curdate,curtime3), sdo.logfile_name
       = dm_err->logfile,
      sdo.session_ident = dsat_get_session_id()
     WHERE sdo.priority=650
      AND (sdo.sd_ddl_ops_id=dsat_opid_list->qual[dsat_idx].op_id)
      AND sdo.status = null
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
    IF (curqual > 0)
     SET dm_err->eproc = "Querying sd_ddl_ops status RUNNING for priority 650"
     CALL disp_msg(" ",dm_err->logfile,0)
     SELECT INTO "nl:"
      FROM (ceradm.sd_ddl_ops sdo)
      WHERE sdo.priority=650
       AND sdo.status="RUNNING"
       AND (sdo.sd_ddl_ops_id=dsat_opid_list->qual[dsat_idx].op_id)
      DETAIL
       dsat_str = sdo.operation
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
     SET dm_err->eproc = "Executing dm2_update_sd_tabledef"
     CALL disp_msg(" ",dm_err->logfile,0)
     IF (dm2_push_cmd(dsat_str,1)=0)
      GO TO exit_program
     ENDIF
     ...
     SET dm_err->emsg = " "
     IF (dsat_err_ind=0)
      SET dm_err->eproc = build("Updating sd_ddl_ops row to complete for sd_ddl_ops_id: ",
       dsat_opid_list->qual[dsat_idx].op_id)
      CALL disp_msg(" ",dm_err->logfile,0)
      UPDATE  FROM (ceradm.sd_ddl_ops sdo)
       SET sdo.status = "COMPLETE", sdo.end_dt_tm = cnvtdatetime(curdate,curtime3), sdo.logfile_name
         = dm_err->logfile,
        sdo.session_ident = dsat_get_session_id()
       WHERE sdo.priority=650
        AND (sdo.sd_ddl_ops_id=dsat_opid_list->qual[dsat_idx].op_id)
        AND ((sdo.status="RUNNING") OR (sdo.status = null
        AND sdo.ready_to_run_ind=1))
       WITH nocounter
      ;end update
      IF (check_error(dm_err->eproc)=1)
      ...
      ENDIF
     ELSE
      SET dm_err->eproc = build("Updating sd_ddl_ops row to error for sd_ddl_ops_id: ",dsat_opid_list
       ->qual[dsat_idx].op_id)
      CALL disp_msg(" ",dm_err->logfile,0)
      UPDATE  FROM (ceradm.sd_ddl_ops sdo)
       SET sdo.status = "ERROR", sdo.error_msg = dsat_err_msg, sdo.end_dt_tm = cnvtdatetime(curdate,
         curtime3),
        sdo.logfile_name = dm_err->logfile, sdo.session_ident = dsat_get_session_id()
       WHERE sdo.priority=650
        AND (sdo.sd_ddl_ops_id=dsat_opid_list->qual[dsat_idx].op_id)
        AND sdo.status="RUNNING"
       WITH nocounter
  ...
  ENDFOR
 ENDIF
 SET dm_err->eproc = "Load single tables"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_SINGLE_TABLE_ORAGEN"
  DETAIL
   IF (substring((findstring(".",di.info_name,1,1)+ 1),(size(di.info_name) - findstring(".",di
     .info_name,1,1)),di.info_name)=cnvtupper(curnode))
    dsat_single_list->cnt = (dsat_single_list->cnt+ 1), stat = alterlist(dsat_single_list->qual,
     dsat_single_list->cnt), dsat_single_list->qual[dsat_single_list->cnt].table_name = substring(1,(
     findstring(".",di.info_name,1,1) - 1),di.info_name)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF ((dsat_single_list->cnt > 0))
  FOR (dsat_idx = 1 TO dsat_single_list->cnt)
   EXECUTE oragen3 dsat_single_list->qual[dsat_idx].table_name
   IF ((dm_err->err_ind=0))
    SET dm_err->eproc = "Remove DM2_SINGLE_TABLE_ORAGEN rows for single tables"
    CALL disp_msg(" ",dm_err->logfile,0)
    DELETE  FROM dm_info di
     WHERE di.info_domain="DM2_SINGLE_TABLE_ORAGEN"
      AND di.info_name=concat(dsat_single_list->qual[dsat_idx].table_name,".",cnvtupper(curnode))
     WITH nocounter
    ;end delete
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ELSE
    SET dm_err->eproc = "Update DM2_SINGLE_TABLE_ORAGEN rows for single tables"
    CALL disp_msg(" ",dm_err->logfile,0)
    UPDATE  FROM dm_info di
     SET di.info_char = concat("Oragen3 experienced errors on ",curnode,". Please view logfile ",
       dm_err->logfile), di.info_number = 1
     WHERE di.info_domain="DM2_SINGLE_TABLE_ORAGEN"
      AND di.info_name=concat(dsat_single_list->qual[dsat_idx].table_name,".",cnvtupper(curnode))
     WITH nocounter
    ;end update
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "Updating running status for dm2_sync_app_tabledef"
 CALL disp_msg(" ",dm_err->logfile,0)
 UPDATE  FROM dm_info d
  SET d.info_date = cnvtdatetime(curdate,curtime3), d.info_number = 0
  WHERE d.info_domain=concat("DM2_SYNC_APP_TABLEDEF_",dsat_domain_name)
   AND d.info_name=cnvtupper(curnode)
  WITH nocounter
 ;end update
 ...
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "dm2_sync_app_tabledef completed"
 ENDIF
 CALL final_disp_msg("dm2syncapp_tbldef")
END GO
