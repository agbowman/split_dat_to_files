CREATE PROGRAM ce_io_result_validation_v1:dba
 EXECUTE crmrtl
 EXECUTE srvrtl
  ...
  SELECT INTO "nl"
   ...
   PLAN (d
    ...
    WHERE (iotr.person_id=request->io_result_list[d.seq].person_id)
     AND iotr.io_total_end_dt_tm > cnvtdatetimeutc(valstartdttm)
     AND iotr.io_total_end_dt_tm >= cnvtdatetimeutc(request->io_result_list[d.seq].event_end_dt_tm)
     AND iotr.io_total_start_dt_tm <= cnvtdatetimeutc(request->io_result_list[d.seq].event_end_dt_tm)
     AND iotr.valid_until_dt_tm=cnvtdatetimeutc("31-DEC-2100")
     AND iotr.suspect_flag != 1)
   HEAD REPORT
  ...
  SELECT INTO "nl"
   ...
   PLAN (d
    ...
    WHERE (iotr.person_id=request->med_result_list[d.seq].person_id)
     AND iotr.io_total_end_dt_tm > cnvtdatetimeutc(valstartdttm)
     AND iotr.io_total_end_dt_tm >= cnvtdatetimeutc(request->med_result_list[d.seq].event_end_dt_tm)
     AND iotr.io_total_start_dt_tm <= cnvtdatetimeutc(request->med_result_list[d.seq].event_end_dt_tm
     )
     AND iotr.valid_until_dt_tm=cnvtdatetimeutc("31-DEC-2100")
     AND iotr.suspect_flag != 1)
   HEAD REPORT
 ...
 SUBROUTINE retrieve_total_definition_info(null)
   ...
   FOR (ndx = 1 TO candidatecount)
    SET pos = locateval(totaldefidx,1,totaldefcount,suspectcandidates->lst[ndx].
   ...
   ELSE
    ...
    FOR (ndx = 1 TO definitioncount)
      ...
      SET pos = locateval(totaldefidx,1,totaldefcount,uar_srvgetdouble(hitem,"definition_id"),
       totaldefinition->lst[totaldefidx].total_definition_id)
      IF (pos)
       SET totaldefinition->lst[pos].type_cd = uar_srvgetdouble(hitem,"type_cd")
       SET elementcount = uar_srvgetitemcount(hitem,"elements")
       SET stat = alterlist(totaldefinition->lst[pos].elements,elementcount)
       FOR (ndx2 = 1 TO elementcount)
         SET helement = uar_srvgetitem(hitem,"elements",(ndx2 - 1))
         SET totaldefinition->lst[pos].elements[ndx2].event_cd = uar_srvgetdouble(helement,"event_cd"
          )
         SET totaldefinition->lst[pos].elements[ndx2].route_cd = uar_srvgetdouble(helement,"route_cd"
          )
         SET totaldefinition->lst[pos].elements[ndx2].iv_event_cd = uar_srvgetdouble(helement,
          "iv_event_cd")
       ENDFOR
      ENDIF
    ENDFOR
 ...
 SUBROUTINE query_simple_medication_io_results(null)
   SELECT DISTINCT INTO "nl"
    ior.event_id, cemr.admin_route_cd, cemr.iv_event_cd
    ...
    PLAN (d
     ...
     WHERE (ceior.event_id=request->io_result_list[d.seq].event_id)
      AND ceior.valid_until_dt_tm=cnvtdatetimeutc("31-DEC_2100")
      AND ceior.io_end_dt_tm > cnvtdatetimeutc(valstartdttm))
     JOIN (ce
     WHERE ce.parent_event_id=ceior.reference_event_id
      AND ce.valid_until_dt_tm=cnvtdatetimeutc("31-DEC_2100"))
     JOIN (cemr
   ...
   SELECT DISTINCT INTO "nl"
    ...
    PLAN (d
     ...
     WHERE (ior.event_id=request->io_result_list[d.seq].event_id)
      AND ior.valid_until_dt_tm=cnvtdatetimeutc("31-DEC_2100")
   ...
   FOR (medndx = 1 TO meditemcnt)
    ...
    WHILE (candidatendx > 0
     AND candidatendx <= candidatecnt)
      ...
      IF (candidatendx)
       SET totaldefndx = locateval(totaldefndx,1,totaldefcnt,suspectcandidates->lst[candidatendx].
        io_total_definition_id,totaldefinition->lst[totaldefndx].total_definition_id)
      ENDIF
      IF (totaldefndx)
