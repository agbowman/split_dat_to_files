CREATE PROGRAM bhs_req_adt_test_aa
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 IF ((orders->spoolout_ind=1))
  ...
  IF (bmc_flag >= 0)
   ...
   SET bmcreqfile1 = build(concat("bmcdcpreq1","_",new_timedisp),".dat")
   FOR (vv = 1 TO order_cnt)
     ...
     ;end select
     CALL echo(bmcreqfile)
     SET ms_subject = concat(s_primarymnemonic," - ",orders->name)
     SET dclcom = concat("cat ",bmcreqfile,' | tr -d \\r | mailx -s "',ms_subject,'" ',
      "arunkumar.ala@bhs.org")
