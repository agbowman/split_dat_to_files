CREATE PROGRAM ecf_obs_icd10_non_billable:dba
 ...
 SUBROUTINE kia_script_inc(pkg,ccl_obj)
   ...
   CALL parser(obj_prs)
 END ;Subroutine
 DECLARE kia_end_date = dq8 WITH public, noconstant(0)
 SET kia_end_date = cnvtdatetime(curdate,curtime3)
 FREE RECORD savedate
 RECORD savedate(
   1 lst[*]
     2 ext_beg_date = dq8
     2 nomen_id = f8
 ...
 SELECT INTO "nl:"
  FROM nomenclature n,
   cmt_concept_extension i
  PLAN (i
   WHERE i.extension_type_mean="BILLABLE"
    AND i.extension_value="N"
    AND i.end_effective_dt_tm >= cnvtdatetime(kia_end_date))
   JOIN (n
   WHERE (n.source_vocabulary_cd=
   (SELECT
    cv.code_value
    FROM code_value cv
    WHERE cv.code_set=400
     AND cv.cdf_meaning IN ("ICD10-CM", "ICD10-PCS")
     AND cv.active_ind=1
     AND cv.end_effective_dt_tm > cnvtdatetime(kia_end_date)))
    AND i.concept_cki=n.concept_cki
    AND n.active_ind=1
    AND n.end_effective_dt_tm > cnvtdatetime(kia_end_date))
  DETAIL
   cntr = (cntr+ 1), stat = alterlist(savedate->lst,cntr), savedate->lst[cntr].ext_beg_date = i
   .beg_effective_dt_tm,
   savedate->lst[cntr].nomen_id = n.nomenclature_id
  WITH nocounter
 ;end select
 CALL echo(build("UPDATING "," ",cnvtstring(curqual)," NOMEN rows"))
 IF (curqual > 0)
  UPDATE  FROM nomenclature n,
    (dummyt d  WITH seq = value(cntr))
   SET n.end_effective_dt_tm = cnvtdatetime(datetimeadd(savedate->lst[d.seq].ext_beg_date,- (
      0.00001157))), n.updt_cnt = (n.updt_cnt+ 1), n.updt_dt_tm = cnvtdatetime(sysdate),
    n.updt_task = 1112
   PLAN (d
    WHERE (savedate->lst[d.seq].nomen_id > 0.0))
    JOIN (n
    WHERE (n.nomenclature_id=savedate->lst[d.seq].nomen_id))
   WITH nocounter
  ;end update
  CALL kia_chk_error(1)
  COMMIT
 ...
 SELECT INTO "nl:"
  FROM dm_info
  WHERE info_name="NONBILLABLE"
   AND info_char="OBSOLETE"
 ...
 IF (curqual > 0)
  ...
  ;end select
  UPDATE  FROM dm_info
   SET info_number = (info_number_var+ 1), updt_dt_tm = cnvtdatetime(curdate,curtime3)
   WHERE info_name="NONBILLABLE"
    AND info_char="OBSOLETE"
    AND info_domain="External Content Factory"
  ;end update
 ...
 ELSE
  INSERT  FROM dm_info
   SET info_domain = "External Content Factory", info_name = "NONBILLABLE", info_date = cnvtdatetime(
     curdate,curtime3),
    info_char = "OBSOLETE", info_number = 1.0, info_long_id = 0,
  ...
  COMMIT
 ENDIF
 CALL echo("Script Execution Completed")
#exit_script
END GO
