CREATE PROGRAM dm_store_text:dba
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 IF (check_logfile("dm_store_text",".log","DM_STORE_TEXT LOG")=0)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_STORE_TEXT..."
 SET modify maxvarlen 268435456
 DECLARE dst_size = i4 WITH protect, constant(100)
 DECLARE dst_total = i4 WITH protect, noconstant(0)
 DECLARE dst_start = i4 WITH protect, noconstant(0)
 DECLARE d_file_name = vc WITH protect, noconstant("")
 DECLARE d_pos = i4 WITH protect, noconstant(0)
 DECLARE d_idx = i4 WITH protect, noconstant(0)
 DECLARE d_cat_det_name = vc WITH protect, noconstant("")
 DECLARE d_type_flag = i4 WITH protect, noconstant(0)
 DECLARE d_logical = vc WITH protect, noconstant("")
 DECLARE d_error_ind = i4 WITH protect, noconstant(0)
 DECLARE d_error_msg = vc WITH protect, noconstant("")
 DECLARE dst_user_pos = i4 WITH protect, noconstant(0)
 DECLARE dst_pat_incl = i2 WITH protect, noconstant(0)
 IF ((validate(dtcl_csv_load_ind,- (1))=- (1)))
  DECLARE dtcl_csv_load_ind = i2 WITH protect, noconstant(0)
 ENDIF
 DECLARE dst_read_script(drs_prog=vc(ref),drs_log=vc) = i2
 FREE RECORD prog_rec
 RECORD prog_rec(
   1 cnt = i4
   1 qual[*]
     2 line_data = vc
 )
 FREE RECORD dst_user_rec
 RECORD dst_user_rec(
   1 cnt = i4
   1 qual[*]
     2 user_name = vc
 )
 FREE RECORD dst_obj_pat
 RECORD dst_obj_pat(
   1 cnt = i4
   1 qual[*]
     2 object_name = vc
 )
 IF ((script_rec->script_cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No data found in SCRIPT_REC record structure"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
  GO TO exit_script
 ENDIF
 SELECT INTO "NL:"
  FROM dm_info di
  WHERE di.info_domain="DM_TEXT_FIND_CONFIGURATIONS:USER_LIST"
  DETAIL
   dst_user_rec->cnt = (dst_user_rec->cnt+ 1), stat = alterlist(dst_user_rec->qual,dst_user_rec->cnt),
   dst_user_rec->qual[dst_user_rec->cnt].user_name = di.info_name
  WITH nocounter
 ;end select
 IF (check_error("Getting USER_NAME information") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Getting USER_NAME info:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 SELECT INTO "NL:"
  FROM dm_info di
  WHERE di.info_domain="DM_TEXT_FIND_CONFIGURATIONS:SCRIPT_PATTERN"
  DETAIL
   dst_obj_pat->cnt = (dst_obj_pat->cnt+ 1), stat = alterlist(dst_obj_pat->qual,dst_obj_pat->cnt),
   dst_obj_pat->qual[dst_obj_pat->cnt].object_name = cnvtupper(di.info_name)
  WITH nocounter
 ;end select
 IF (check_error("Getting OBJECT_NAME information") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Getting OBJECT_NAME info:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 SELECT INTO "NL:"
  FROM (dummyt d  WITH seq = script_rec->script_cnt)
  DETAIL
   script_rec->script_qual[d.seq].script_name = cnvtupper(script_rec->script_qual[d.seq].script_name)
  WITH nocounter
 ;end select
 SELECT INTO "NL:"
  FROM dprotect d
  WHERE d.object IN ("P", "E")
  DETAIL
   dst_pat_incl = 0
   FOR (d_idx = 1 TO dst_obj_pat->cnt)
     IF (d.object_name=patstring(dst_obj_pat->qual[d_idx].object_name))
      dst_pat_incl = 1, d_idx = (dst_obj_pat->cnt+ 1)
     ENDIF
   ENDFOR
   IF (dtcl_csv_load_ind=0)
    dst_user_pos = locateval(dst_user_pos,1,dst_user_rec->cnt,d.user_name,dst_user_rec->qual[
     dst_user_pos].user_name)
   ELSE
    dst_user_pos = 0
   ENDIF
   IF (((dst_user_pos=0) OR (dst_pat_incl=1)) )
    d_idx = locateval(d_pos,1,script_rec->script_cnt,d.object_name,script_rec->script_qual[d_pos].
     script_name)
    IF (d_idx > 0)
     IF ((script_rec->script_qual[d_idx].user_name > " "))
      script_rec->script_cnt = (script_rec->script_cnt+ 1), stat = alterlist(script_rec->script_qual,
       script_rec->script_cnt), script_rec->script_qual[script_rec->script_cnt].attribute_desc =
      script_rec->script_qual[d_idx].attribute_desc,
      d_idx = script_rec->script_cnt, script_rec->script_qual[d_idx].script_name = d.object_name
     ENDIF
     script_rec->script_qual[d_idx].compile_dt_tm = cnvtdatetime(d.datestamp,cnvttime2(format(d
        .timestamp,"######;rp0"),"HHMMSS")), script_rec->script_qual[d_idx].user_name = d.user_name,
     script_rec->script_qual[d_idx].object_type = d.object,
     script_rec->script_qual[d_idx].group_num = d.group, script_rec->script_qual[d_idx].path = d
     .source_name
     IF ((script_rec->script_qual[d_idx].data_source <= " "))
      script_rec->script_qual[d_idx].data_source = script_rec->script_qual[d_idx].script_name
     ENDIF
    ENDIF
   ELSE
    d_idx = locateval(d_pos,1,script_rec->script_cnt,d.object_name,script_rec->script_qual[d_pos].
     script_name)
    IF (d_idx > 0)
     script_rec->script_qual[d_idx].not_new_ind = 1
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error("Querying dprotect table") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying dprotect table:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 SELECT INTO "NL:"
  FROM dm_text_find d,
   dm_text_find_detail f
  WHERE (f.dm_text_find_detail_id=script_rec->dm_text_find_detail_id)
   AND f.dm_text_find_detail_id > 0.0
   AND d.dm_text_find_id=f.dm_text_find_id
  DETAIL
   d_cat_det_name = trim(d.find_name), d_type_flag = f.detail_type_flag
  WITH nocounter
 ;end select
 IF (check_error("Getting DM_TEXT_FIND information") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Getting DM_TEXT_FIND Info:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No data found with DM_TEXT_FIND_DETAIL_ID of ",trim(cnvtstring(
     script_rec->dm_text_find_detail_id,20,1)))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Check if script has changed since last time it was loaded."
 SET dst_total = (ceil((cnvtreal(script_rec->script_cnt)/ dst_size)) * dst_size)
 SET stat = alterlist(script_rec->script_qual,dst_total)
 SET dst_start = 1
 FOR (d_pos = (script_rec->script_cnt+ 1) TO dst_total)
   SET script_rec->script_qual[d_pos].script_name = script_rec->script_qual[script_rec->script_cnt].
   script_name
 ENDFOR
 IF (d_type_flag=4)
  SELECT INTO "NL:"
   FROM (dummyt d  WITH seq = value((1+ ((dst_total - 1)/ dst_size)))),
    dm_text_find_data t
   PLAN (d
    WHERE initarray(dst_start,evaluate(d.seq,1,1,(dst_start+ dst_size))))
    JOIN (t
    WHERE expand(d_pos,dst_start,(dst_start+ (dst_size - 1)),t.script_name,script_rec->script_qual[
     d_pos].script_name,
     t.group_num,script_rec->script_qual[d_pos].group_num,dst_size)
     AND t.status_flag != 3)
   DETAIL
    d_idx = locateval(d_pos,1,script_rec->script_cnt,t.script_name,script_rec->script_qual[d_pos].
     script_name,
     t.group_num,script_rec->script_qual[d_pos].group_num)
    IF (d_idx > 0)
     IF ((script_rec->script_qual[d_idx].not_new_ind=0))
      IF (cnvtdatetime(script_rec->script_qual[d_idx].compile_dt_tm) <= t.compile_dt_tm)
       script_rec->script_qual[d_idx].not_new_ind = 1
      ENDIF
     ENDIF
    ENDIF
 ...
 ELSE
  SELECT INTO "NL:"
   ...
   PLAN (d
    WHERE initarray(dst_start,evaluate(d.seq,1,1,(dst_start+ dst_size))))
    JOIN (t
    WHERE (((t.dm_text_find_detail_id=script_rec->dm_text_find_detail_id)) OR (t
    .dm_text_find_detail_id IN (
    (SELECT
     dm_text_find_detail_id
     FROM dm_text_find_detail
     WHERE detail_type_flag=4
      AND dm_text_find_detail_id > 0.0))))
     AND expand(d_pos,dst_start,(dst_start+ (dst_size - 1)),t.script_name,script_rec->script_qual[
     d_pos].script_name,
     t.group_num,script_rec->script_qual[d_pos].group_num,dst_size)
     AND t.status_flag != 3)
   DETAIL
    ...
    IF (d_idx > 0)
     IF ((script_rec->script_qual[d_idx].not_new_ind=0))
      IF ((t.dm_text_find_detail_id != script_rec->dm_text_find_detail_id))
       script_rec->script_qual[d_idx].not_new_ind = 0
      ELSE
       IF (cnvtdatetime(script_rec->script_qual[d_idx].compile_dt_tm) <= t.compile_dt_tm)
        script_rec->script_qual[d_idx].not_new_ind = 1
       ENDIF
  ...
  ;end select
 ENDIF
 IF (check_error("Getting DM_TEXT_FIND_DATA information") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Getting DM_TEXT_FIND_DATA Info:",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 SELECT INTO "NL:"
  FROM dm_info d
  WHERE info_domain="DM_TEXT_FIND_CONFIGURATION"
   AND info_name="LOGICAL_DIRECTORY"
  DETAIL
   d_logical = d.info_char
  WITH nocounter
 ;end select
 IF (check_error("Getting LOGICAL information") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Getting LOGICAL info:",dm_err
    ->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 IF (curqual > 0)
  CALL parser(concat("SET LOGICAL TXTFND '",d_logical,"' go"),1)
 ENDIF
 FOR (d_pos = 1 TO script_rec->script_cnt)
   IF ((script_rec->script_qual[d_pos].not_new_ind=0)
    AND (script_rec->script_qual[d_pos].compile_dt_tm != 0.0))
    IF (d_logical > " ")
     SET d_file_name = concat("'TXTFND:temp_",trim(currdbhandle),"'")
    ELSE
     SET d_file_name = concat("'CCLUSERDIR:temp_",trim(currdbhandle),"'")
    ENDIF
    IF ((script_rec->script_qual[d_pos].object_type="E"))
     CALL parser(concat("translate into ",d_file_name," EKMODULE ",script_rec->script_qual[d_pos].
       script_name," go"),1)
    ELSEIF ((script_rec->script_qual[d_pos].group_num=0))
     CALL parser(concat("translate into ",d_file_name," ",script_rec->script_qual[d_pos].script_name,
       ":DBA go"),1)
    ELSE
     CALL parser(concat("translate into ",d_file_name," ",script_rec->script_qual[d_pos].script_name,
       ":group",
       trim(cnvtstring(script_rec->script_qual[d_pos].group_num))," go"),1)
    ENDIF
    IF (check_error(concat("Translating script: ",script_rec->script_qual[d_pos].script_name)) != 0)
     SET d_file_name = concat(substring(2,(size(d_file_name) - 2),d_file_name),".ccl")
     IF (findfile(value(d_file_name)) > 0)
      SET dm_err->err_ind = 0
      CALL dst_add_log_det(script_rec->dm_text_find_log_id,"WARNING",dm_err->emsg,0.0,0.0)
      SET d_file_name = replace(d_file_name,".ccl","'",0)
      SET d_file_name = concat("'",d_file_name)
      IF (dst_read_script(prog_rec,d_file_name)=0)
       CALL dst_insert_data(d_pos,script_rec,d_cat_det_name,prog_rec,d_type_flag)
       IF (check_error(concat("Writing data to DM_TEXT_FIND_DATA: ",script_rec->script_qual[d_pos].
         script_name)) != 0)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        IF (d_error_ind=0)
         SET d_error_msg = dm_err->emsg
        ENDIF
        SET d_error_ind = 1
        SET dm_err->err_ind = 0
       ENDIF
     ...
     ELSE
      ...
      SET d_file_name = concat("'",d_file_name)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dm_err->emsg = concat("Translating ",script_rec->script_qual[d_pos].script_name,":",dm_err
       ->emsg)
      CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
      IF (d_error_ind=0)
       SET d_error_msg = dm_err->emsg
    ...
    ELSE
     ...
     ENDIF
    ENDIF
    SET d_file_name = substring(1,(size(d_file_name) - 1),d_file_name)
    SET d_file_name = concat(d_file_name,".ccl'")
    CALL parser(concat("set stat = remove(",d_file_name,") go"),1)
    IF (check_error("Dropping temp file") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Dropping temp file:",dm_err
       ->emsg),0.0,0.0)
     IF (d_error_ind=0)
      SET d_error_msg = dm_err->emsg
     ENDIF
   ...
   ENDIF
 ENDFOR
 SUBROUTINE dst_read_script(drs_prog,drs_log)
...
#exit_script
 IF (d_error_ind=1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = d_error_msg
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending DM_STORE_TEXT"
  CALL final_disp_msg("dm_store_text")
 ENDIF
END GO
