CREATE PROGRAM dm2_mig_match_srctotgt
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_mig_match_stot",".log","dm2_mig_match_srctotgt LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Declaring variables."
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 FREE RECORD matchrec
 RECORD matchrec(
   1 tbl_cnt = i4
   1 match_ind = i2
   1 tab_mismatch_cnt = i4
   1 ind_mismatch_cnt = i4
   1 tables[*]
     2 table_name = vc
     2 name_match_ind = i2
     2 index_match_ind = i2
     2 ind_cnt = i4
     2 indices[*]
       3 index_name = vc
       3 name_match_ind = i2
       3 uniqueness_match_ind = i2
 )
 DECLARE dmms_dblink = vc WITH protect, noconstant("ref_data_link")
 DECLARE dmms_owner = vc WITH protect, noconstant("")
 DECLARE dmms_tables = vc WITH protect, noconstant("")
 DECLARE dmms_remote_db_name = vc WITH protect, noconstant("x")
 DECLARE dmms_database_name = vc WITH protect, noconstant("x")
 SET dm_err->eproc = "Validating inputs"
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 SET dmms_owner = cnvtupper( $1)
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg = "Parameter usage: dm2_mig_match_schema '<owner>', '<table names to match>'"
  GO TO exit_script
 ENDIF
 SET dmms_tables = cnvtupper( $2)
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg = "Parameter usage: dm2_mig_match_schema '<owner>', '<table names to match>'"
  GO TO exit_script
 ENDIF
 ...
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dmms_remote_db_name = dmr_mig_data->src_nodes[1].instance_name
 SET dmms_database_name = dmr_mig_data->tgt_nodes[1].instance_name
 SET dm_err->eproc = "Querying for schema differences"
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 SET matchrec->match_ind = 1
 SELECT INTO "nl:"
  srct.table_name, tgtt.table_name
  FROM (value(concat("DBA_TABLES@",dmms_dblink)) srct),
   dba_tables tgtt,
   (value(concat("DBA_INDEXES@",dmms_dblink)) srci),
   dba_indexes tgti
  WHERE srct.table_name=patstring(dmms_tables)
   AND srct.owner=dmms_owner
   AND outerjoin(srct.table_name)=tgtt.table_name
   AND outerjoin(srct.owner)=tgtt.owner
   AND outerjoin(srct.table_name)=srci.table_name
   AND outerjoin(srct.owner)=srci.owner
   AND outerjoin("NORMAL")=srci.index_type
   AND outerjoin("N")=srci.generated
   AND outerjoin(srci.index_name)=tgti.index_name
   AND outerjoin(srci.owner)=tgti.owner
  ORDER BY srct.owner, srct.table_name, srci.index_name
  HEAD REPORT
   matchrec->tbl_cnt = 0, tab_tmp = 0
  HEAD srct.owner
   row + 0
  HEAD srct.table_name
   ind_tmp = 0, matchrec->tbl_cnt = (matchrec->tbl_cnt+ 1)
   IF ((matchrec->tbl_cnt > tab_tmp))
    tab_tmp = (tab_tmp+ 100), stat = alterlist(matchrec->tables,tab_tmp)
   ENDIF
   matchrec->tables[matchrec->tbl_cnt].ind_cnt = 0, matchrec->tables[matchrec->tbl_cnt].table_name =
   srct.table_name, matchrec->tables[matchrec->tbl_cnt].index_match_ind = 1
   IF (tgtt.table_name=srct.table_name)
    matchrec->tables[matchrec->tbl_cnt].name_match_ind = 1
   ELSE
    matchrec->match_ind = 0, matchrec->tab_mismatch_cnt = (matchrec->tab_mismatch_cnt+ 1)
   ENDIF
  HEAD srci.index_name
   IF ((matchrec->tables[matchrec->tbl_cnt].name_match_ind=1))
    matchrec->tables[matchrec->tbl_cnt].ind_cnt = (matchrec->tables[matchrec->tbl_cnt].ind_cnt+ 1)
    IF ((matchrec->tables[matchrec->tbl_cnt].ind_cnt > ind_tmp))
     ind_tmp = (ind_tmp+ 20), stat = alterlist(matchrec->tables[matchrec->tbl_cnt].indices,ind_tmp)
    ENDIF
    matchrec->tables[matchrec->tbl_cnt].indices[matchrec->tables[matchrec->tbl_cnt].ind_cnt].
    index_name = srci.index_name
    IF (srci.index_name != tgti.index_name)
     matchrec->tables[matchrec->tbl_cnt].index_match_ind = 0, matchrec->tables[matchrec->tbl_cnt].
     indices[matchrec->tables[matchrec->tbl_cnt].ind_cnt].name_match_ind = 0, matchrec->match_ind = 0,
     matchrec->ind_mismatch_cnt = (matchrec->ind_mismatch_cnt+ 1)
    ELSEIF (srci.uniqueness != tgti.uniqueness)
     matchrec->tables[matchrec->tbl_cnt].index_match_ind = 0, matchrec->tables[matchrec->tbl_cnt].
     indices[matchrec->tables[matchrec->tbl_cnt].ind_cnt].name_match_ind = 1, matchrec->tables[
     matchrec->tbl_cnt].indices[matchrec->tables[matchrec->tbl_cnt].ind_cnt].uniqueness_match_ind = 0,
     matchrec->match_ind = 0, matchrec->ind_mismatch_cnt = (matchrec->ind_mismatch_cnt+ 1)
    ELSE
     matchrec->tables[matchrec->tbl_cnt].indices[matchrec->tables[matchrec->tbl_cnt].ind_cnt].
     name_match_ind = 1, matchrec->tables[matchrec->tbl_cnt].indices[matchrec->tables[matchrec->
     tbl_cnt].ind_cnt].uniqueness_match_ind = 1
    ENDIF
   ENDIF
  FOOT  srct.owner
   row + 0
  FOOT  srct.table_name
   stat = alterlist(matchrec->tables[matchrec->tbl_cnt].indices,matchrec->tables[matchrec->tbl_cnt].
    ind_cnt)
  FOOT REPORT
   stat = alterlist(matchrec->tables,matchrec->tbl_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  GO TO exit_script
 ...
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 SELECT
  FROM (dummyt d  WITH seq = matchrec->tbl_cnt),
   (dummyt d2  WITH seq = 1)
  PLAN (d
   WHERE (matchrec->match_ind=0)
    AND maxrec(d2,matchrec->tables[d.seq].ind_cnt))
   JOIN (d2
   WHERE (matchrec->tables[d.seq].index_match_ind=0)
    AND (((matchrec->tables[d.seq].indices[d2.seq].name_match_ind=0)) OR ((matchrec->tables[d.seq].
   indices[d2.seq].uniqueness_match_ind=0))) )
  ORDER BY d.seq, d2.seq
  HEAD REPORT
   cnt = 1, col 0, row 0,
   x = "--------------------------------------------------------------------------------------", x,
   row + 1,
   "Schema Validation Report", row + 1,
   "--------------------------------------------------------------------------------------",
   row + 2, "Table and index objects were matched between:", row + 1,
   x = concat("  Source Database: ",dmms_remote_db_name,"@",dmms_dblink), x, row + 1,
   x = concat("  Target Database: ",dmms_database_name), x, row + 1,
   x = concat("  User Validated:  ",dmms_owner), x, row + 2
   IF ((matchrec->match_ind=1))
    x = "All schema matched."
   ELSE
    x = "Schema mismatches found."
   ENDIF
   x = concat("Validation summary: ",x), x, row + 1
   IF ((matchrec->tab_mismatch_cnt > 0))
    "The following tables existed in SOURCE but not in TARGET:", row + 1, "TABLE NAME",
    row + 1, "*************************"
    FOR (i = 1 TO matchrec->tbl_cnt)
      IF ((matchrec->tables[i].name_match_ind=0))
       x = matchrec->tables[i].table_name, row + 1, x
      ENDIF
    ENDFOR
   ENDIF
   row + 2, col 0
   IF ((matchrec->ind_mismatch_cnt > 0))
    "The following indices in SOURCE did not match to TARGET:", row + 1,
    "--------------------------------------------------------------------------------------",
    row + 1, "TABLE NAME", col 30,
    "|  INDEX NAME", col 65, "|   MISMATCH REASON ",
    row + 1, "----------", col 30,
    "   ----------", col 65, "    ---------------"
   ENDIF
  DETAIL
   IF ((matchrec->tables[d.seq].indices[d2.seq].name_match_ind=0))
    y = "INDEX NOT FOUND IN TARGET"
   ELSE
    y = "UNIQUENESS DIFFERENCE    "
   ENDIF
   row + 1, col 0, x = matchrec->tables[d.seq].table_name,
   x, col 33, z = matchrec->tables[d.seq].indices[d2.seq].index_name,
   z, col 68, y
  FOOT REPORT
   row + 1, col 0,
   "--------------------------------------------------------------------------------------",
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_match_srctotgt completed successfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 CALL final_disp_msg("dm2_mig_match_srctotgt")
END GO
