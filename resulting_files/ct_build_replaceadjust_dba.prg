CREATE PROGRAM ct_build_replaceadjust:dba
 PAINT
 IF ("Z"=validate(ct_build_replaceadjust_vrsn,"Z"))
  DECLARE ct_build_replaceadjust_vrsn = vc WITH noconstant("65970.004")
 ENDIF
 RECORD ct_detail(
   1 detail_list[*]
     2 detailtype_cd = f8
...
#the_second_prompt
 ...
 CASE (curscroll)
  OF 0:
   CASE (curaccept)
    OF "A":
     GO TO add_replaceadjust
    OF "M":
     GO TO modify_replaceadjust
    OF "D":
     GO TO delete_replaceadjust
    OF "Q":
     GO TO end_prog
   ENDCASE
  ELSE
...
#add_replaceadjust
 ...
 CALL text(5,3,"Operator_Cd:")
 CALL text(7,3,"Qualify on (0 = CPT-4, 1 = BILL ITEM):")
 CALL text(9,3,"Rule_Entity_Id:")
 ...
 SELECT
  ...
  WHERE cv.code_set=18851
   AND cv.active_ind=1
   AND cv.cdf_meaning IN ("ADD", "SUBTRACTION", "MULTIPLY")
  WITH nocounter
 ;end select
 CALL accept(5,17,"9(40);cf")
 SET v_operator_cd = cnvtreal(curaccept)
 SET help = off
 CALL accept(7,41,"x(1);c",0)
 ...
 EXECUTE ct_ens_rule_detail
 COMMIT
 GO TO top_replaceadjust
#modify_replaceadjust
 CALL clear(1,1)
 CALL box(1,1,17,70)
 CALL text(2,2,"Rule Name:")
 CALL text(2,13,ct_rule_name)
 ...
 SELECT
  ...
  WITH nocounter
 ;end select
 CALL accept(8,15,"9(40);c",ct_detail->detail_list[cur_rec].operator_name)
 SET v_operator_cd = cnvtreal(curaccept)
 SET help = off
 ...
 SET v_end_date = cnvtdatetime(curaccept)
 EXECUTE afc_ccl_msgbox "Are you sure you want to Modify?", "Replaceadjust", "YN"
 CALL clear(1,1)
 IF ((response->yes_ind=1))
  GO TO modifing_replaceadjust
 ELSEIF ((response->no_ind=1))
  GO TO top_replaceadjust
 ENDIF
#modifing_replaceadjust
 SET request->ct_rule_detail_qual = 1
 ...
 COMMIT
 GO TO top_replaceadjust
#delete_replaceadjust
 EXECUTE afc_ccl_msgbox "Are you sure you want to Delete?", "Replaceadjust", "YN"
 CALL clear(1,1)
 IF ((response->yes_ind=1))
