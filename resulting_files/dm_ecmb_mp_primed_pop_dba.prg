CREATE PROGRAM dm_ecmb_mp_primed_pop:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_MP_PRIMED_POP"
 ...
 SELECT INTO "NL:"
  ...
  DETAIL
   d_from_person_id = e.person_id
  WITH nocounter
 ;end select
 SELECT INTO "NL:"
  FROM mp_primed_view_pop mdp
  WHERE mdp.person_id=d_from_person_id
  HEAD mdp.person_id
 ...
 IF (v_cust_count1 > 0)
  FOR (v_cust_loopcount1 = 1 TO v_cust_count1)
    IF (del_from(rreclist->from_rec[v_cust_loopcount1].from_id,0,rreclist->from_rec[v_cust_loopcount1
     ].active_ind,rreclist->from_rec[v_cust_loopcount1].active_status_cd,d_from_person_id)=0)
     GO TO exit_sub
    ENDIF
  ENDFOR
 ENDIF
 SUBROUTINE del_from(s_df_pk_id,s_df_to_fk_id,s_df_prev_act_ind,s_df_prev_act_status,personid)
   IF (recombining=0)
