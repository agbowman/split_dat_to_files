CREATE PROGRAM bhs_rpt_eprescribe_utilization:dba
 PROMPT
 ...
 SELECT DISTINCT INTO value(ms_output_dest)
  o.order_id, ordered_as_mnemonic = trim(substring(1,100,o.ordered_as_mnemonic),3), order_dt_tm =
  format(oa.order_dt_tm,"mm/dd/yyyy hh:mm;;d"),
  routing_type = trim(substring(1,255,od.oe_field_display_value),3), mmdc.csa_schedule, order_status
  ...
  ordering_provider = trim(substring(1,100,pr.name_full_formatted),3), position = trim(
  ...
  ORDER BY o.order_id, o.ordered_as_mnemonic, order_dt_tm,
   routing_type, mmdc.csa_schedule, location,
   encntr_type, acc#, ordering_provider,
   position
