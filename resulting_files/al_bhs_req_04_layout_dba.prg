CREATE PROGRAM al_bhs_req_04_layout:dba
 PROMPT
  "program_name" = ""
  WITH program_name
  ...
  DETAIL
   ...
   IF (odflag=1)
    ...
    IF (od.oe_field_meaning="ICD9")
     diacnt = (diacnt+ 1), orders->qual[d1.seq].d_qual[dc].label_text = build("Diagnosis #",diacnt)
    ELSE
 ...
 IF ((orders->spoolout_ind=1))
  ...
  DECLARE transfuse_header(ncalc=i2) = f8 WITH protect
  DECLARE transfuse_headerabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE requistion_head_noencounter(ncalc=i2) = f8 WITH protect
  DECLARE requistion_head_noencounterabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE requistion_head(ncalc=i2) = f8 WITH protect
  DECLARE requistion_headabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE print_header2(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
  DECLARE print_header2abs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8 WITH
  protect
  DECLARE print_header(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
  DECLARE print_headerabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8 WITH
  protect
  DECLARE order_section(ncalc=i2) = f8 WITH protect
  DECLARE order_sectionabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE line1(ncalc=i2) = f8 WITH protect
  DECLARE line1abs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  ...
  protect
  DECLARE future_order_info(ncalc=i2) = f8 WITH protect
  DECLARE future_order_infoabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE testing_section(ncalc=i2) = f8 WITH protect
  DECLARE testing_sectionabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE print_top_foot_blank(ncalc=i2) = f8 WITH protect
  DECLARE print_top_foot_blankabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE patient_note_olo(ncalc=i2) = f8 WITH protect
  DECLARE patient_note_oloabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE patient_note_rad1(ncalc=i2) = f8 WITH protect
  DECLARE patient_note_rad1abs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE print_footer(ncalc=i2) = f8 WITH protect
  DECLARE print_footerabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE order_comments(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
  DECLARE order_commentsabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8 WITH
  protect
  DECLARE order_comments_header(ncalc=i2) = f8 WITH protect
  DECLARE order_comments_headerabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  DECLARE foot_page(ncalc=i2) = f8 WITH protect
  DECLARE foot_pageabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  ...
  END ;Subroutine
  SUBROUTINE transfuse_header(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = transfuse_headerabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE transfuse_headerabs(ncalc,offsetx,offsety)
    ...
    IF (ncalc=rpt_render)
     ...
     IF ((request->order_qual[1].encntr_id=0))
      SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Baystate Health Order Requisition",
        char(0)))
     ENDIF
     SET _rptstat = uar_rptrect(_hreport,(offsetx+ 0.250),(offsety+ 0.938),3.750,0.625,
      rpt_nofill,rpt_white)
     SET _rptstat = uar_rptrect(_hreport,(offsetx+ 4.000),(offsety+ 0.000),3.250,0.938,
      rpt_nofill,rpt_white)
     SET _rptstat = uar_rptrect(_hreport,(offsetx+ 4.000),(offsety+ 0.938),3.250,0.625,
      rpt_nofill,rpt_white)
     SET _rptstat = uar_rptrect(_hreport,(offsetx+ 0.250),(offsety+ 0.375),1.750,0.563,
      rpt_nofill,rpt_white)
     SET _rptstat = uar_rptrect(_hreport,(offsetx+ 0.250),(offsety+ 0.000),3.750,0.375,
      rpt_nofill,rpt_white)
     SET rptsd->m_flags = 4
  ...
  END ;Subroutine
  SUBROUTINE requistion_head_noencounter(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = requistion_head_noencounterabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE requistion_head_noencounterabs(ncalc,offsetx,offsety)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE requistion_head(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = requistion_headabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE requistion_headabs(ncalc,offsetx,offsety)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE print_header2(ncalc,maxheight,bcontinue)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = print_header2abs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE print_header2abs(ncalc,offsetx,offsety,maxheight,bcontinue)
    ...
    IF (_remsex > 0)
     ...
     ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remsex,((size(__sex) - _remsex)+ 1),
        __sex)))))
      SET _remsex = (_remsex+ rptsd->m_drawlength)
     ELSE
      SET _remsex = 0
     ENDIF
     SET growsum = (growsum+ _remsex)
    ENDIF
    SET rptsd->m_flags = 4
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE print_header(ncalc,maxheight,bcontinue)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = print_headerabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE print_headerabs(ncalc,offsetx,offsety,maxheight,bcontinue)
    ...
    IF ( NOT (program1 IN ("BHS_REQ_OLO")))
     ...
     IF (_remprimary_diag > 0)
      ...
      ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remprimary_diag,((size(__primary_diag)
       ...
       SET _remprimary_diag = (_remprimary_diag+ rptsd->m_drawlength)
      ELSE
       SET _remprimary_diag = 0
      ENDIF
      SET growsum = (growsum+ _remprimary_diag)
     ENDIF
    ELSE
     SET _remprimary_diag = 0
     SET _holdremprimary_diag = _remprimary_diag
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE order_section(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = order_sectionabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE order_sectionabs(ncalc,offsetx,offsety)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE line1(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = line1abs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE line1abs(ncalc,offsetx,offsety)
  ...
  SUBROUTINE order_detailsabs(ncalc,offsetx,offsety,maxheight,bcontinue)
    ...
    IF (_remdetail_value > 0)
     ...
     ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remdetail_value,((size(__detail_value)
      ...
      SET _remdetail_value = (_remdetail_value+ rptsd->m_drawlength)
     ELSE
      SET _remdetail_value = 0
     ENDIF
     SET growsum = (growsum+ _remdetail_value)
    ENDIF
    SET rptsd->m_flags = 5
    IF (bcontinue)
     SET rptsd->m_y = offsety
    ...
    IF (_remdetail_label > 0)
     ...
     ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remdetail_label,((size(__detail_label)
      ...
      SET _remdetail_label = (_remdetail_label+ rptsd->m_drawlength)
     ELSE
      SET _remdetail_label = 0
     ENDIF
     SET growsum = (growsum+ _remdetail_label)
    ENDIF
    SET rptsd->m_flags = 4
    IF (bcontinue)
     SET rptsd->m_y = offsety
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE future_order_info(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = future_order_infoabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE future_order_infoabs(ncalc,offsetx,offsety)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE testing_section(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = testing_sectionabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE testing_sectionabs(ncalc,offsetx,offsety)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE print_top_foot_blank(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = print_top_foot_blankabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE print_top_foot_blankabs(ncalc,offsetx,offsety)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE patient_note_olo(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = patient_note_oloabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE patient_note_oloabs(ncalc,offsetx,offsety)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE patient_note_rad1(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = patient_note_rad1abs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE patient_note_rad1abs(ncalc,offsetx,offsety)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE print_footer(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = print_footerabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE print_footerabs(ncalc,offsetx,offsety)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE order_comments(ncalc,maxheight,bcontinue)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = order_commentsabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE order_commentsabs(ncalc,offsetx,offsety,maxheight,bcontinue)
    ...
    IF (_remorder_comment > 0)
     ...
     ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remorder_comment,((size(__order_comment)
      ...
      SET _remorder_comment = (_remorder_comment+ rptsd->m_drawlength)
     ELSE
      SET _remorder_comment = 0
     ENDIF
     SET growsum = (growsum+ _remorder_comment)
    ENDIF
    SET rptsd->m_flags = 4
    IF (bcontinue)
     SET rptsd->m_y = offsety
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE order_comments_header(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = order_comments_headerabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE order_comments_headerabs(ncalc,offsetx,offsety)
    DECLARE sectionheight = f8 WITH noconstant(0.380000), private
    IF (ncalc=rpt_render)
     SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
     SET _rptstat = uar_rptrect(_hreport,(offsetx+ 0.250),(offsety+ 0.000),7.010,0.260,
      rpt_nofill,rpt_white)
     SET rptsd->m_flags = 4
     SET rptsd->m_borders = rpt_sdnoborders
     SET rptsd->m_padding = rpt_sdnoborders
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE foot_page(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = foot_pageabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE foot_pageabs(ncalc,offsetx,offsety)
    DECLARE sectionheight = f8 WITH noconstant(0.190000), private
  ...
  SUBROUTINE initializereport(dummy)
    ...
    SET rptreport->m_reportname = "AL_BHS_REQ_04_LAYOUT"
  ...
  IF (curqual > 0)
   ...
   FOR (prt_ord = 1 TO size(orders->qual,5))
     ...
     WHILE (becont=1)
       ...
       print_top_foot_blank(rpt_calcheight))+ patient_note_olo(rpt_calcheight))+ patient_note_oco(
        rpt_calcheight))+ patient_note_rad1(rpt_calcheight)))
       SET continued = "(continued)"
       SET d0 = print_top_foot_blank(rpt_render)
       SET d0 = patient_note_olo(rpt_render)
       SET d0 = patient_note_oco(rpt_render)
       ...
       FOR (ord_detail_cnt = 1 TO size(orders->qual[prt_ord].d_qual,5))
         IF ((((orders->qual[prt_ord].d_qual[ord_detail_cnt].group_seq=fsub)) OR (fsub=31
          ...
          IF ((orders->qual[prt_ord].d_qual[ord_detail_cnt].value > " "))
           ...
           )+ patient_note_oco(rpt_calcheight))+ patient_note_rad1(rpt_calcheight)))
     ...
     foot_page(rpt_calcheight))+ print_top_foot_blank(rpt_calcheight))+ patient_note_olo(
      rpt_calcheight))+ patient_note_oco(rpt_calcheight))+ patient_note_rad1(rpt_calcheight))) >
     page_size))
      SET _yoffset = (page_size - (((((print_footer(rpt_calcheight)+ foot_page(rpt_calcheight))+
      print_top_foot_blank(rpt_calcheight))+ patient_note_olo(rpt_calcheight))+ patient_note_oco(
       rpt_calcheight))+ patient_note_rad1(rpt_calcheight)))
