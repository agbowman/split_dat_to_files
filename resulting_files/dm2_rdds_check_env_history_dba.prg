CREATE PROGRAM dm2_rdds_check_env_history:dba
 ...
 SUBROUTINE dm2_get_program_stack(null)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_rdds_chk_en_hs",".log","dm2_rdds_check_env_history LOG...")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_rdds_check_env_history"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dir_env_maint_rs->tgt_env_id=0.0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Record structure dir_env_maint_rs is not populated"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM dm_rdds_req_install_env e
  WHERE (e.environment_id=dir_env_maint_rs->tgt_env_id)
  WITH nocounter
 ;end select
 IF (check_error("Checking dm_rdds_req_install_env for target environment id") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (curqual > 0)
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "...Ending dm2_rdds_check_env_history"
 CALL final_disp_msg("dm2_rdds_check_env_history")
END GO
