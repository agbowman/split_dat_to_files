CREATE PROGRAM dm2_xlat_backfill_child:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dbx_done_flag = i2
 DECLARE dbxc_env_ndx = i4
 DECLARE dbxc_tbl_ndx = i4
 DECLARE dbxc_max_ins_cnt = f8
 DECLARE dbxc_loop_cnt = i4
 DECLARE dbxc_beg_value = f8
 DECLARE dbxc_end_value = f8
 SET dbx_done_flag = 0
 SET dbxc_env_ndx =  $1
 SET dbxc_tbl_ndx =  $2
 SET dbc_find_tbl_ndx =  $3
 SET dbxc_max_ins_cnt = 10000
 SET dbxc_loop_cnt = 0
 WHILE (dbx_done_flag=0)
   SET dbxc_beg_value = (dbxc_loop_cnt * dbxc_max_ins_cnt)
   SET dbxc_end_value = least(((dbxc_loop_cnt * dbxc_max_ins_cnt)+ dbxc_max_ins_cnt),
    dbx_seq_match_info->parent[dbx_env_ndx].sequence[dbc_find_tbl_ndx].sequence_match_value)
   CALL parser("INSERT INTO",0)
   CALL parser("dm_refchg_invalid_xlat",0)
   CALL parser(
    "(parent_entity_name, parent_entity_id, updt_id, updt_dt_tm, updt_task, updt_applctx, updt_cnt)",
    0)
   CALL parser("(SELECT distinct",0)
   CALL parser(concat("'",dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].tbl_name,"',"),0)
   CALL parser(dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].column_name,0)
   CALL parser(",0,",0)
   CALL parser("cnvtdatetime(dbx_tbls_to_backfill->backfill_dt_tm)",0)
   CALL parser(",0,",0)
   CALL parser("0,",0)
   CALL parser("0",0)
   CALL parser("FROM",0)
   CALL parser(concat(t1_table_name," t1"),0)
   CALL parser("WHERE",0)
   CALL parser(concat(t1_column_name," <= ",cnvtstring(dbxc_end_value)),0)
   CALL parser(concat(" and ",t1_column_name," > ",cnvtstring(dbxc_beg_value)," and "),0)
   IF ((dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_ind=1))
    CALL parser(concat(dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_proc," and"),0)
   ENDIF
   CALL parser("not exists",0)
   CALL parser("(SELECT",0)
   CALL parser("'x'",0)
   CALL parser("FROM",0)
   CALL parser(concat(t2_table_name," t2"),0)
   CALL parser("WHERE",0)
   CALL parser(concat(t1_column_name," = ",t2_column_name," )"),0)
   CALL parser("and not exists",0)
   CALL parser("(SELECT",0)
   CALL parser("'x'",0)
   CALL parser("FROM",0)
   CALL parser("dm_refchg_invalid_xlat x2",0)
   CALL parser("WHERE",0)
   CALL parser(concat("x2.parent_entity_name = '",dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].tbl_name,"'"
     ),0)
   CALL parser(concat("and x2.parent_entity_id = ",t1_column_name,")"),0)
   CALL parser(") GO",1)
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    GO TO exit_program_child
   ENDIF
   SET dbxc_loop_cnt = (dbxc_loop_cnt+ 1)
   IF ((dbxc_end_value=dbx_seq_match_info->parent[dbx_env_ndx].sequence[dbc_find_tbl_ndx].
   sequence_match_value))
    SET dbx_done_flag = 1
   ENDIF
   COMMIT
 ENDWHILE
