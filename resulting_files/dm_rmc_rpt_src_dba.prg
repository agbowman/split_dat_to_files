CREATE PROGRAM dm_rmc_rpt_src:dba
 ...
 DECLARE rpa_log_type = vc WITH protect, noconstant(" ")
 ...
 DECLARE drrs_mnemonic = vc WITH protect, noconstant(logical("CLIENT_MNEMONIC"))
 DECLARE drrs_file_name = vc WITH protect, noconstant(" ")
 DECLARE drrs_subject = vc WITH protect, noconstant(" ")
 DECLARE drrs_heading_ind = i2 WITH protect, noconstant(0)
 DECLARE drrs_auto_ind = i2 WITH public, noconstant(0)
 DECLARE dm_refchg_b62tob10(p1=vc) = f8
 DECLARE drrs_query_exec(rqe_query=vc,rqe_db_link=vc,rqe_file_name=vc,rqe_heading_ind=i2(ref)) = null
 DECLARE drrs_query_del(rqd_query=vc,rqd_audit_file=vc) = null
 IF (validate(drsa_request)=0)
  FREE RECORD drsa_request
  RECORD drsa_request(
    1 table_name = vc
    1 ctxt_cnt = i4
    1 ctxt_qual[*]
 ...
 SET dm_err->eproc = "Starting dm_rmc_rpt_src..."
 IF (check_logfile("dm_rmc_rpt_src",".log","DM_RMC_RPT_SRC LOG")=0)
  SET dm_err->err_ind = 1
 ...
 IF (dgi_get_global_info(drgi_global)=1)
  GO TO exit_program
 ENDIF
 IF (validate(drrs_email)=0)
  DECLARE drrs_email = vc WITH protect, noconstant(" ")
 ENDIF
 IF (validate(drss_request)=0)
  SET rpa_tab_name = trim(cnvtupper( $1))
  SET rpa_log_type = trim(cnvtupper( $2))
  SET drsa_request->table_name = rpa_tab_name
  SET drsa_request->log_type = rpa_log_type
 ELSE
  SET rpa_tab_name = drss_request->table_name
  SET drsa_request->table_name = drss_request->table_name
  SET drsa_request->ctxt_cnt = drss_request->ctxt_cnt
  SET stat = moverec(drss_request->ctxt_qual,drsa_request->ctxt_qual)
  SET drsa_request->log_type = drss_request->log_type
 ENDIF
 EXECUTE dm_rmc_src_audit
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  ...
  WHERE d.pref_domain="RDDS AUDIT CONFIG"
   ...
   AND d.pref_name="SOURCE ONLY AUDIT"
   AND d.application_nbr=0
   AND d.person_id=0
  DETAIL
   IF (d.pref_section="AUTO-GEN AUDIT")
    drrs_auto_ind = d.pref_nbr
   ELSEIF (d.pref_nbr > 0)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   drrs_query->cnt = (drrs_query->cnt+ 1), stat = alterlist(drrs_query->qual,drrs_query->cnt),
   drrs_query->qual[drrs_query->cnt].query_id = d.parent_entity_id,
   drrs_query->qual[drrs_query->cnt].file_name = d.pref_name, drrs_query->qual[drrs_query->cnt].
 ...
 IF ((drrs_query->cnt=0))
  IF (drrs_auto_ind=1)
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   SET drrs_query->cnt = drrga_reply->query_cnt
   SET stat = alterlist(drrs_query->qual,drrs_query->cnt)
   FOR (rpa_idx = 1 TO drrs_query->cnt)
     IF (findstring("::",drrga_reply->query_qual[rpa_idx].query_name) > 0)
      SET drrs_query->qual[rpa_idx].audit_name = substring((findstring("::",drrga_reply->query_qual[
 ...
 FOR (rpa_idx = 1 TO drrs_query->cnt)
   ...
   SET drrs_heading_ind = 0
   IF ((drrs_query->qual[rpa_idx].query_id=0)
    AND (drrga_reply->query_cnt > 0))
    SET rdds_audit_query = drrga_reply->query_qual[rpa_idx].query_text
   ELSE
    SELECT INTO "nl:"
     FROM long_text_reference l
     WHERE (l.long_text_id=drrs_query->qual[rpa_idx].query_id)
     HEAD REPORT
   ...
   IF (daf_is_blank(rdds_audit_query))
    ...
    GO TO exit_program
   ENDIF
   SET drrs_file_name = drrac_get_file_name("SRC",rpa_tab_name,drrs_query->qual[rpa_idx].audit_name)
   CALL drrs_query_exec(rdds_audit_query,drgi_global->db_link,drrs_file_name,drrs_heading_ind)
   CALL drrs_query_del(rdds_audit_query,drrs_file_name)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SET rdds_audit_file = concat(rdds_audit_file,", ",drrs_file_name)
   SET drrs_subject = concat("SRC Output - ",rpa_tab_name)
   IF (daf_is_not_blank(drrs_email))
    IF (drrm_send_email(drrs_subject,drrs_email,drrs_file_name)=0)
     SET dm_err->err_ind = 1
     GO TO exit_program
    ENDIF
   ENDIF
 ...
 SET drss_reply->status = "S"
 SET drss_reply->status_msg = concat("Audit created. Filename(s): ",rdds_audit_file)
 SUBROUTINE drrs_query_exec(rqe_query,rqe_db_link,rqe_file_name,rqe_heading_ind)
   ...
   SET rqe_temp_query = replace(rqe_temp_query,"<SELECT_TAG>",concat(
     ",LOG_ID = s.l_column_value, DOMAIN = '","SOURCE","'"),0)
   SET rqe_temp_query = replace(rqe_temp_query,"<DOMAIN_FLAG>","^1^",0)
   ...
   RETURN
 END ;Subroutine
 SUBROUTINE drrs_query_del(rqd_query,rqd_audit_file)
   ...
   FOR (rqd_idx = 1 TO drqp_reply->col_cnt)
     ...
     ELSE
      SET rqd_temp_query = concat(rqd_temp_query," ' ',LOG_ID=s.l_column_value,DOMAIN='SOURCE',")
     ENDIF
   ENDFOR
...
#exit_program
 IF (check_error(dm_err->eproc)=1)
  SET drss_reply->status = "F"
  SET drss_reply->status_msg = dm_err->emsg
 ENDIF
END GO
