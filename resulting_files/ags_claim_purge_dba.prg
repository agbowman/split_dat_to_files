CREATE PROGRAM ags_claim_purge:dba
 PROMPT
  "Backout Type (J, I, P) =" = "J",
  "AGS_JOB_ID (0.0) = " = 0.0
  WITH cbackout_type, djob_id
 CALL echo("***")
 CALL echo("***   BEG :: AGS_CLAIM_PURGE")
 CALL echo("***")
 ...
 IF ( NOT (validate(log,0)))
  ...
  DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_claim_purge_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_CLAIM_PURGE"
 ...
 IF (the_type="P"
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ENDIF
    purge_rec->qual[sknt].mil_item_id = a.hea_claim_visit_id, purge_rec->qual[sknt].delete_item_id =
    a.ags_claim_data_id, purge_rec->qual[sknt].delete_person_id = a.person_id,
 ...
 ELSEIF (the_type="J"
  ...
  IF (ierrcode > 0)
   ...
   SET log->qual[log->qual_knt].smsgtype = "ERROR"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("SELECT ERROR AGS_CLAIM_DATA (delete items) >> ",trim(
     serrmsg))
   GO TO exit_script
  ENDIF
 ELSEIF (the_type="I")
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "INVALID PURGE >> CLAIM by Item Not Allowed"
  GO TO exit_script
 ELSE
  SET failed = input_error
  SET ilog_status = 1
 ...
 SELECT INTO "nl:"
  ...
  HEAD d.seq
   purge_rec->qual[d.seq].bo_item_id = a.ags_claim_data_id
  WITH nocounter
 ;end select
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("SELECT ERROR AGS_CLAIM_DATA (backout items)>> ",trim(
    serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   JOIN (a
   WHERE (a.hea_claim_visit_id=purge_rec->qual[d.seq].mil_item_id))
  HEAD REPORT
  ...
  DETAIL
   ...
   ENDIF
   sub_item_rec->qual[sknt].data_id = a.ags_claim_detail_data_id
  FOOT REPORT
 ...
 IF ((sub_item_rec->qual_knt > 0))
  EXECUTE ags_claim_detail_backout value("I"), value(0.0) WITH replace("ITEM_REC","SUB_ITEM_REC")
  IF (failed != false)
   SET ilog_status = 1
   SET log->qual_knt = (log->qual_knt+ 1)
   SET stat = alterlist(log->qual,log->qual_knt)
 ...
 ELSE
  ...
  SET log->qual[log->qual_knt].smsgtype = "INFO"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "NO ITEMS FOUND FOR AGS_CLAIM_DETAIL_BACKOUT"
 ENDIF
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 DELETE  FROM hea_claim_visit h,
  ...
  PLAN (d
   WHERE d.seq > 0
    AND (purge_rec->qual[d.seq].mil_item_id > 0))
   JOIN (h
   WHERE (h.hea_claim_visit_id=purge_rec->qual[d.seq].mil_item_id))
 ...
 UPDATE  FROM ags_claim_data a,
  ...
  PLAN (d
   ...
   JOIN (a
   WHERE (a.ags_claim_data_id=purge_rec->qual[d.seq].bo_item_id))
  WITH nocounter
 ;end update
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_CLAIM_DATA (backout items >> ",trim(
    serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 ...
 UPDATE  FROM ags_claim_data a,
  ...
  PLAN (d
   WHERE d.seq > 0
    ...
    AND (purge_rec->qual[d.seq].delete_item_id > 0))
   JOIN (a
   WHERE (a.ags_claim_data_id=purge_rec->qual[d.seq].delete_item_id))
  WITH nocounter
 ;end update
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_CLAIM_DATA (delete items) >> ",trim(
    serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("DELETE ERROR AGS_CLAIM_DATA >> ",trim(serrmsg))
  GO TO exit_script
 ENDIF
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_CLAIM_PURGE"
 IF (define_logging_sub=true)
  CALL echo("***")
