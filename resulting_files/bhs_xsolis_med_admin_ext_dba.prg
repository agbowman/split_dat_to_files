CREATE PROGRAM bhs_xsolis_med_admin_ext:dba
 ...
 SELECT INTO "nl:"
  ...
  HEAD e.encntr_id
   ...
   s_per_lname = trim(p.name_last,3), m_rec->qual[m_rec->l_cnt].s_per_dob = format(cnvtdatetimeutc(
     datetimezone(p.birth_dt_tm,p.birth_tz),1),"YYYYMMDD;;d"),
 ...
 SET frec->file_name = build(logical("bhscust"),
  "/ftp/bhs_xsolis_med_admin_ext/bhs_ma_med_admin_xsolis_",trim(format(cnvtdatetime(mf_start_dt),
    "MMDDYYYYHHmmss;;q"),3),"_",trim(format(cnvtdatetime(mf_stop_dt),"MMDDYYYYHHmmss;;q"),3),
  ".txt")
 SET frec->file_buf = "w"
 ...
 SET frec->file_name = build(logical("bhscust"),"/ftp/bhs_xsolis_med_admin_ext/v2_adminreport.",trim(
   format(cnvtdatetime(sysdate),"YYYYMMDDHHmmss;;q"),3))
 SET frec->file_buf = "w"
 SET stat = cclio("OPEN",frec)
 FOR (ml_idx1 = 1 TO m_rec->l_cnt)
   FOR (ml_idx2 = 1 TO m_rec->qual[ml_idx1].l_ocnt)
