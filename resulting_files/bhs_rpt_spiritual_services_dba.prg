CREATE PROGRAM bhs_rpt_spiritual_services:dba
 PROMPT
 ...
 IF (validate(request->batch_selection))
  ...
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="BHS_RPT_SPIRITUAL_SERVICES"
 ...
 SELECT INTO "nl:"
  ...
  HEAD ce1.event_id
   ...
   ENDIF
   m_rec->qual[ml_cnt].s_dob = trim(format(p.birth_dt_tm,"mm/dd/yyyy ;;d"),3), m_rec->qual[ml_cnt].
   s_location = build2(trim(uar_get_code_display(e.loc_facility_cd),3),"/",trim(uar_get_code_display(
      e.loc_nurse_unit_cd),3),"/",trim(uar_get_code_display(e.loc_room_cd),3)), m_rec->qual[ml_cnt].
   s_pat_name = p.name_full_formatted,
   m_rec->qual[ml_cnt].s_mrn = ea.alias, m_rec->qual[ml_cnt].s_admit_dt_tm = trim(format(e.reg_dt_tm,
     "mm/dd/yyyy ;;d"),3), m_rec->qual[ml_cnt].s_sex = uar_get_code_display(p.sex_cd),
   m_rec->qual[ml_cnt].s_length_of_stay = trim(format(evaluate(e.disch_dt_tm,null,datetimediff(
       sysdate,e.reg_dt_tm),datetimediff(e.disch_dt_tm,e.reg_dt_tm)),"###.##;R"),3)
  HEAD ce3.event_id
   CASE (ce3.event_cd)
    OF mf_spirit_pref_cd:
     m_rec->qual[ml_cnt].s_spirit_pref = ce3.result_val
    OF mf_spirit_resource_cd:
     m_rec->qual[ml_cnt].s_spirit_resource = ce3.result_val
    OF mf_spirit_start_time_cd:
     m_rec->qual[ml_cnt].s_start_dt_tm = format(cedr.result_dt_tm,"mm/dd/yyyy hh:mm;;d")
    OF mf_spirit_end_time_cd:
     m_rec->qual[ml_cnt].s_end_dt_tm = format(cedr.result_dt_tm,"mm/dd/yyyy hh:mm;;d")
   ENDCASE
  FOOT REPORT
 ...
 IF (((textlen( $S_RECIPIENTS) > 1) OR (mn_ops=1)) )
  SET frec->file_name = build(cnvtlower(curprog),"_",trim(format(mf_begin_dt_tm,"mm_dd_yy ;;d"),3),
   ...
   ".csv")
  SET ms_subject = build2("Spiritual Services Report ",trim(format(mf_begin_dt_tm,
     "mmm-dd-yyyy hh:mm ;;d"),3)," to ",trim(format(mf_end_dt_tm,"mmm-dd-yyyy hh:mm ;;d"),3))
  SET frec->file_buf = "w"
  SET stat = cclio("OPEN",frec)
  SET frec->file_buf = build('"PATIENT NAME",','"SEX",','"DOB",','"MRN #",',
   '"RELIGIOUS/SPIRITUAL PREFERENCE",',
   '"LOCATION",','"ADMIT DATE",','"START TIME",','"END TIME",','"SPIRITUAL/SACRAMENTAL RESOURCE",',
   '"LENGTH OF STAY",',char(13))
  ...
  FOR (ml_cnt = 1 TO size(m_rec->qual,5))
   SET frec->file_buf = build('"',trim(m_rec->qual[ml_cnt].s_pat_name,3),'","',trim(m_rec->qual[
    ...
    trim(m_rec->qual[ml_cnt].s_dob,3),'","',trim(m_rec->qual[ml_cnt].s_mrn,3),'","',trim(m_rec->qual[
     ml_cnt].s_spirit_pref,3),
    '","',trim(m_rec->qual[ml_cnt].s_location,3),'","',trim(m_rec->qual[ml_cnt].s_admit_dt_tm,3),
    '","',
    trim(m_rec->qual[ml_cnt].s_start_dt_tm,3),'","',trim(m_rec->qual[ml_cnt].s_end_dt_tm,3),'","',
    trim(m_rec->qual[ml_cnt].s_spirit_resource,3),
    '","',trim(m_rec->qual[ml_cnt].s_length_of_stay,3),'"',char(13))
   SET stat = cclio("WRITE",frec)
  ENDFOR
 ...
 ELSE
  SELECT INTO value( $OUTDEV)
   patient_name = substring(1,100,m_rec->qual[d.seq].s_pat_name), sex = substring(1,100,m_rec->qual[d
    .seq].s_sex), dob = substring(1,100,m_rec->qual[d.seq].s_dob),
   mrn# = substring(1,100,m_rec->qual[d.seq].s_mrn), religious_preference = substring(1,100,m_rec->
    qual[d.seq].s_spirit_pref), location = substring(1,100,m_rec->qual[d.seq].s_location),
   admit_date = substring(1,100,m_rec->qual[d.seq].s_admit_dt_tm), start_time = substring(1,100,m_rec
    ->qual[d.seq].s_start_dt_tm), end_time = substring(1,100,m_rec->qual[d.seq].s_end_dt_tm),
   spiritual_resource = substring(1,100,m_rec->qual[d.seq].s_spirit_resource), length_of_stay =
   substring(1,100,m_rec->qual[d.seq].s_length_of_stay)
