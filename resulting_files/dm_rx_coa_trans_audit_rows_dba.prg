CREATE PROGRAM dm_rx_coa_trans_audit_rows:dba
 ...
 SET reply->table_name = "RX_COA_TRANS_AUDIT"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(rc.rx_coa_trans_audit_id)
   FROM rx_coa_trans_audit rc
   WHERE rc.rx_coa_trans_audit_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(rc.rx_coa_trans_audit_id)
  FROM rx_coa_trans_audit rc
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    rc.rowid
    FROM rx_coa_trans_audit rc
    WHERE rc.rx_coa_trans_audit_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "rc.create_dt_tm <= cnvtdatetime(curdate - lDaysToKeep, curtime3)","rc"))
     AND rc.rx_coa_trans_audit_id != 0
    DETAIL
     v_rows += 1
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = rc.rowid
    WITH nocounter, maxqual(rc,value(v_rows_left))
   ;end select
