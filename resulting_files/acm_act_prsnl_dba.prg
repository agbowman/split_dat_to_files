CREATE PROGRAM acm_act_prsnl:dba
 ...
 FOR (index = 1 TO xref->act_cnt)
   SET reply->prsnl_qual[xref->act[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->act_cnt)),
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.person_id=acm_request->prsnl_qual[xref->act[d.seq].idx].person_id))
  WITH nocounter, status(reply->prsnl_qual[xref->act[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->act_cnt)
   IF ((reply->prsnl_qual[xref->act[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM prsnl p
    WHERE expand(t1,f_val,t_val,p.person_id,acm_request->prsnl_qual[xref->act[t1].idx].person_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.person_id,acm_request->prsnl_qual[xref->act[t1].idx].person_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_qual[xref->act[index].idx].status=0))
