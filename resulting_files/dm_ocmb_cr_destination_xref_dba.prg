CREATE PROGRAM dm_ocmb_cr_destination_xref:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_OCMB_CR_DESTINATION_XREF"
 ...
 SELECT INTO "nl:"
  ...
  WHERE (frm.parent_entity_id=request->xxx_combine[icombine].from_xxx_id)
   AND frm.parent_entity_name="ORGANIZATION"
 ...
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   ...
   WHERE (tu.parent_entity_id=request->xxx_combine[icombine].to_xxx_id)
    AND tu.parent_entity_name="ORGANIZATION"
    AND tu.active_ind=1
   DETAIL
