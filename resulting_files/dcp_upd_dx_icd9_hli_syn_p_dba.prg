CREATE PROGRAM dcp_upd_dx_icd9_hli_syn_p:dba
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_DX_ICD9_HLI_SYN",concat(
   "create global temporary table TEMP_DX_ICD9_HLI_SYN(diagnosis_id number,",
   "nomenclature_id number) on commit preserve rows"),"Creating temporary table")
 CALL sbr_add_sql_stmt("INDEX","XIETEMPDXICD9HLISYN",
  "create unique index XIETEMPDXICD9HLISYN ON TEMP_DX_ICD9_HLI_SYN(diagnosis_id)","Creating index")
