CREATE PROGRAM bbd_chk_import_ques_depend:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bbd_chk_import_ques_depend"
 CALL logscriptstart(sreadme_name)
 DECLARE success_ind = i2 WITH protect, noconstant(1)
 DECLARE positive_rslt_quar_cd = f8 WITH protect, noconstant(0.0)
 DECLARE positive_rslt_disp_cd = f8 WITH protect, noconstant(0.0)
 DECLARE quar_disp_product_cd = f8 WITH protect, noconstant(0.0)
 DECLARE question_code_set = i4 WITH protect, constant(1661)
 DECLARE pos_rslt_quar_cdf_meaning = c12 WITH protect, constant("POSRSLTQUAR")
 DECLARE pos_rslt_disp_cdf_meaning = c12 WITH protect, constant("POSRSLTDISP")
 DECLARE quar_disp_prod_cdf_meaning = c12 WITH protect, constant("DISPQUARPROD")
 SELECT INTO "nl:"
  c.code_value
  FROM code_value c
  PLAN (c
   WHERE c.code_set=question_code_set
    AND c.cdf_meaning IN (pos_rslt_quar_cdf_meaning, pos_rslt_disp_cdf_meaning,
   quar_disp_prod_cdf_meaning)
    AND c.active_ind=1)
  DETAIL
   IF (c.cdf_meaning=pos_rslt_quar_cdf_meaning)
    positive_rslt_quar_cd = c.code_value
   ELSEIF (c.cdf_meaning=pos_rslt_disp_cdf_meaning)
    positive_rslt_disp_cd = c.code_value
   ELSEIF (c.cdf_meaning=quar_disp_prod_cdf_meaning)
    quar_disp_product_cd = c.code_value
   ENDIF
  WITH nocounter
 ;end select
 IF (positive_rslt_quar_cd=0.0)
  SET success_ind = 0
  SET smsg = concat("positive result quarantine code lookup failed->","POSRSLTQUAR")
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 IF (positive_rslt_disp_cd=0.0)
  SET success_ind = 0
  SET smsg = concat("positive result dispose code lookup failed->","POSRSLTDISP")
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 IF (quar_disp_product_cd=0.0)
  SET success_ind = 0
  SET smsg = concat("quarantine dispose question code lookup failed->","DISPQUARPROD")
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM question_dependency q
  WHERE q.question_cd=quar_disp_product_cd
  WITH nocounter
 ;end select
 IF (curqual != 1)
  SET success_ind = 0
  SET smsg = "Can't find question for quarantine dispose product question."
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 ENDIF
 SELECT INTO "nl:"
  FROM dependency d
  WHERE d.question_cd=quar_disp_product_cd
   AND d.depend_quest_cd IN (positive_rslt_quar_cd, positive_rslt_disp_cd)
  WITH nocounter
 ;end select
 IF (curqual != 2)
  SET success_ind = 0
  SET smsg = "Can't find dependencies for quar/disp product question."
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 ENDIF
#exit_script
 IF (success_ind=0)
