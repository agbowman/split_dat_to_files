CREATE PROGRAM dm_ecmb_referral_entity_reltn:dba
 ...
 DECLARE var_referral_id = f8
 DECLARE dup_found = i4
 SET v_cust_count_from = 0
 SET v_cust_count_to = 0
 SET v_cust_loopcount_from = 0
 SET v_cust_loopcount_to = 0
 SET var_referral_id = 0.0
 SET dup_found = 0
 CALL dm_cmb_get_context(0)
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  SET stat = alterlist(dcem_request->qual,1)
  SET dcem_request->qual[1].parent_entity = "ENCOUNTER"
  SET dcem_request->qual[1].child_entity = "REFERRAL_ENTITY_RELTN"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_ECMB_REFERRAL_ENTITY_RELTN"
 ...
 SELECT INTO "nl:"
  frm.*
  FROM referral_entity_reltn frm
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count_from].from_id = frm.referral_entity_reltn_id, rreclist->from_rec[
   v_cust_count_from].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count_from].
   active_status_cd = frm.active_status_cd,
   rreclist->from_rec[v_cust_count_from].referral_id = frm.referral_id
 ...
 IF (v_cust_count_from > 0)
  SELECT INTO "nl:"
   tu.*
   FROM referral_entity_reltn tu
   WHERE (tu.parent_entity_id=request->xxx_combine[icombine].to_xxx_id)
    AND tu.parent_entity_name="ENCOUNTER"
   DETAIL
    v_cust_count_to += 1
    IF (mod(v_cust_count_to,10)=1)
     stat = alterlist(rreclist->to_rec,(v_cust_count_to+ 9))
    ENDIF
    rreclist->to_rec[v_cust_count_to].to_id = tu.referral_entity_reltn_id, rreclist->to_rec[
    v_cust_count_to].active_ind = tu.active_ind, rreclist->to_rec[v_cust_count_to].active_status_cd
     = tu.active_status_cd,
    rreclist->to_rec[v_cust_count_to].referral_id = tu.referral_id
   WITH forupdatewait(tu)
  ;end select
  FOR (v_cust_loopcount_from = 1 TO v_cust_count_from)
    SET var_referral_id = rreclist->from_rec[v_cust_loopcount_from].referral_id
    SET dup_found = 0
    FOR (v_cust_loopcount_to = 1 TO v_cust_count_to)
      IF ((rreclist->to_rec[v_cust_loopcount_to].referral_id=var_referral_id)
       AND (rreclist->to_rec[v_cust_loopcount_to].active_ind=true))
       SET dup_found = 1
      ENDIF
    ENDFOR
    IF (dup_found=1)
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM referral_entity_reltn frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     frm.updt_task = reqinfo->updt_task, frm.updt_dt_tm = cnvtdatetime(sysdate), frm.parent_entity_id
      = s_uf_to_fk_id
    WHERE frm.referral_entity_reltn_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "REFERRAL_ENTITY_RELTN"
  ...
  i4)
   UPDATE  FROM referral_entity_reltn frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.updt_cnt = (frm.updt_cnt+ 1),
     ...
     reqinfo->updt_task,
     frm.updt_dt_tm = cnvtdatetime(sysdate), frm.parent_entity_id = s_df_to_fk_id
    WHERE frm.referral_entity_reltn_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
