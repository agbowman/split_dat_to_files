CREATE PROGRAM br_add_mpage:dba
 ...
 SET readme_data->message = "Readme Failed: Starting <br_add_mpage.prg> script"
 ...
 SELECT INTO "nl:"
  FROM br_step bs
  PLAN (bs
   WHERE bs.step_mean="MPAGES")
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed checking existence of MPAGES row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_step bs
   SET bs.step_mean = "MPAGES", bs.step_disp = "MPage Setup", bs.step_type = "IMPMAINT",
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcir.br_client_id=1
   ...
   AND bcir.item_mean="MPAGES"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to verify existence of STEP/MPAGES row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_client_item_reltn bcir
   SET bcir.br_client_item_reltn_id = seq(bedrock_seq,nextval), bcir.br_client_id = 1, bcir.item_type
     = "STEP",
    bcir.item_mean = "MPAGES", bcir.item_display = "MPage Setup", bcir.step_cat_mean = "CORE",
 ...
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to check for existence of COREM/MPAGES row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  SELECT INTO "nl:"
  ...
  INSERT  FROM br_client_sol_step bcss
   SET bcss.br_client_id = 1, bcss.solution_mean = "COREM", bcss.step_mean = "MPAGES",
    bcss.sequence = (last_step_seq+ 10), bcss.updt_cnt = 0, bcss.updt_dt_tm = cnvtdatetime(sysdate),
 ...
 SET readme_data->message = "Readme Succeeded: <br_add_mpage.prg> script"
#exit_script
