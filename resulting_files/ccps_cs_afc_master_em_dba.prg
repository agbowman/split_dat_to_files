CREATE PROGRAM ccps_cs_afc_master_em:dba
 PROMPT
  ...
  "Tab To See Dates" = "",
  "Begin Activity Date" = "",
  "End Activity Date" = "SYSDATE"
  WITH outdev, rpt_type, ce_id,
  order_id, fin, tab_message_only,
  beg_date, end_date
 ...
 IF (logical_domain_error)
  GO TO exit_prg
 ENDIF
 CALL echo(build2("* * * logical_domain_id from include file ",cur_logical_domain))
 DECLARE server_diag_cd = f8 WITH constant(uar_get_code_by("MEANING",13019,"SRV DIAG")), protect
 DECLARE loaded_cea_type_cd = f8 WITH constant(uar_get_code_by("MEANING",13029,"LOADED")), protect
 DECLARE workload_only_cd = f8 WITH constant(uar_get_code_by("MEANING",13028,"WORKLOADONLY")),
 protect
 DECLARE not_at_chrg_pnt_cd = f8 WITH constant(uar_get_code_by("MEANING",18269,"NOTATCHRGPNT")),
 protect
 DECLARE no_chrg_pnt_cd = f8 WITH constant(uar_get_code_by("MEANING",18269,"NOPNTONITEM")), protect
 DECLARE no_chrg_pnt_on_tier_cd = f8 WITH constant(uar_get_code_by("MEANING",18269,"NOPNTONTIER")),
 protect
 DECLARE not_at_chrg_lvl_cd = f8 WITH constant(uar_get_code_by("MEANING",18269,"NOTATCHRGLVL")),
 protect
 DECLARE no_chrg_ind_cd = f8 WITH constant(uar_get_code_by("MEANING",18269,"NOCHARGEIND")), protect
 DECLARE no_tier_for_org_cd = f8 WITH constant(uar_get_code_by("MEANING",18269,"NOTIERFORORG")),
 protect
 DECLARE no_phleb_chrg_cd = f8 WITH constant(uar_get_code_by("MEANING",18269,"NOPHLEBCHRG")), protect
 DECLARE fin_cd = f8 WITH constant(uar_get_code_by("MEANING",319,"FIN NBR")), protect
 DECLARE fin_nbr = vc WITH constant(trim( $5,3)), protect
 DECLARE dashline = i4 WITH noconstant(0), public
 DECLARE num = i4 WITH noconstant(0), public
 DECLARE index = i4 WITH noconstant(0), public
 DECLARE pos = i4 WITH noconstant(0), public
 DECLARE temp_charge_item_id = f8 WITH noconstant(0.00), public
 DECLARE exec_date_str = vc WITH constant(concat("Execution Date/Time:  ",format(cnvtdatetime(curdate,
     curtime),"@SHORTDATETIME"))), protect
 FREE RECORD events
 RECORD events(
   1 events_cnt = i4
   1 events_detail[*]
     ...
     2 master_detail[*]
       ...
       3 service_dt_tm = vc
       3 update_dt_tm = vc
       3 srvr_diagostic_cnt = i4
       3 srvr_diagostic_detail[*]
         4 charge_event_mod_id = f8
         4 field7 = vc
         4 diagnostic_text_one = vc
         4 diagnostic_text_two_a = vc
         4 diagnostic_text_two_b = vc
         4 diagnostic_text_three_a = vc
         4 diagnostic_text_three_b = vc
         4 diagnostic_text_four_a = vc
         4 diagnostic_text_four_b = vc
 )
 IF (( $CE_ID > 0))
  SELECT INTO "nl:"
   FROM charge_event ce,
    ...
    person p
   PLAN (ce
    WHERE (ce.charge_event_id= $CE_ID))
    JOIN (ce2
    WHERE ce2.ext_m_event_id=ce.ext_m_event_id)
    JOIN (p
    WHERE p.person_id=ce2.person_id
     AND p.active_ind=1
     AND p.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
     AND p.end_effective_dt_tm > cnvtdatetime(curdate,curtime3)
     AND p.logical_domain_id=cur_logical_domain)
   ORDER BY ce.charge_event_id
   HEAD REPORT
    events_cnt = 0
   DETAIL
    events_cnt = (events_cnt+ 1), stat = alterlist(events->events_detail,events_cnt), events->
    events_detail[events_cnt].master_id = ce2.ext_m_event_id,
    events->events_detail[events_cnt].master_ref_id = ce2.ext_m_reference_id, events->events_detail[
    events_cnt].master_cont_cd = ce2.ext_m_reference_cont_cd, events->events_detail[events_cnt].
    order_id = ce2.order_id,
    events->events_detail[events_cnt].charge_event_id = ce2.charge_event_id, events->events_detail[
    events_cnt].accession = ce2.accession, events->events_detail[events_cnt].person_name = p
    .name_full_formatted,
    events->events_detail[events_cnt].person_id = ce2.person_id, events->events_detail[events_cnt].
    encounter_id = ce2.encntr_id
    IF (ce.ext_p_reference_id=0
     AND ce.ext_p_reference_cont_cd=0)
     events->events_detail[events_cnt].p_ref_id = ce2.ext_i_reference_id, events->events_detail[
     events_cnt].p_ref_cd = ce2.ext_i_reference_cont_cd
    ELSE
     events->events_detail[events_cnt].p_ref_id = ce2.ext_p_reference_id, events->events_detail[
     events_cnt].p_ref_cd = ce2.ext_p_reference_cont_cd, events->events_detail[events_cnt].c_ref_id
      = ce2.ext_i_reference_id,
     events->events_detail[events_cnt].c_ref_cd = ce2.ext_i_reference_cont_cd
    ENDIF
   FOOT REPORT
    events->events_cnt = events_cnt
   WITH nocounter
  ;end select
 ELSEIF (( $ORDER_ID > 0))
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    IF (mod(events_cnt,10)=1)
     stat = alterlist(events->events_detail,(events_cnt+ 9))
    ENDIF
    events->events_detail[events_cnt].master_id = ce.ext_m_event_id, events->events_detail[events_cnt
    ].master_ref_id = ce.ext_m_reference_id, events->events_detail[events_cnt].master_cont_cd = ce
    .ext_m_reference_cont_cd,
    events->events_detail[events_cnt].order_id = ce.order_id, events->events_detail[events_cnt].
    charge_event_id = ce.charge_event_id, events->events_detail[events_cnt].accession = ce.accession,
    events->events_detail[events_cnt].person_name = p.name_full_formatted, events->events_detail[
    events_cnt].person_id = ce.person_id, events->events_detail[events_cnt].encounter_id = ce
    .encntr_id
    IF (ce.ext_p_reference_id=0
     AND ce.ext_p_reference_cont_cd=0)
     events->events_detail[events_cnt].p_ref_id = ce.ext_i_reference_id, events->events_detail[
     events_cnt].p_ref_cd = ce.ext_i_reference_cont_cd
    ELSE
     events->events_detail[events_cnt].p_ref_id = ce.ext_p_reference_id, events->events_detail[
     events_cnt].p_ref_cd = ce.ext_p_reference_cont_cd, events->events_detail[events_cnt].c_ref_id =
     ce.ext_i_reference_id,
     events->events_detail[events_cnt].c_ref_cd = ce.ext_i_reference_cont_cd
    ENDIF
   FOOT REPORT
    stat = alterlist(events->events_detail,events_cnt), events->events_cnt = events_cnt
   WITH nocounter
  ;end select
 ELSEIF (textlen(fin_nbr) > 0)
  SELECT INTO "nl:"
   FROM encntr_alias ea,
    ...
    person p
   PLAN (ea
    WHERE ea.alias=fin_nbr
     AND ea.encntr_alias_type_cd=fin_cd)
    JOIN (ce
    WHERE ce.encntr_id=ea.encntr_id)
    JOIN (cea
    WHERE cea.charge_event_id=ce.charge_event_id
     AND ((cea.service_dt_tm+ 0) >= cnvtdatetime( $BEG_DATE))
     AND ((cea.service_dt_tm+ 0) <= cnvtdatetime( $END_DATE)))
    JOIN (p
    WHERE p.person_id=ce.person_id
     AND p.active_ind=1
     AND p.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
 ...
 DECLARE headpagemastersection0(ncalc=i2) = f8 WITH protect
 DECLARE headpagemastersection0abs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE headpagediagnosticsection0(ncalc=i2) = f8 WITH protect
 DECLARE headpagediagnosticsection0abs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE headmastersection0(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE headmastersection0abs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8
 WITH protect
 DECLARE headdiagnosticsection0(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE headdiagnosticsection0abs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =
 f8 WITH protect
 DECLARE detailmastersection(ncalc=i2) = f8 WITH protect
 DECLARE detailmastersectionabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE detaildiagnosticsection(ncalc=i2) = f8 WITH protect
 DECLARE detaildiagnosticsectionabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE detaildiagnosticsection2(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE detaildiagnosticsection2abs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref))
  = f8 WITH protect
 DECLARE detaildiagnosticsection3(ncalc=i2) = f8 WITH protect
 DECLARE detaildiagnosticsection3abs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE detaildiagnosticsection4(ncalc=i2) = f8 WITH protect
 DECLARE detaildiagnosticsection4abs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 ...
 DECLARE _bcontheadmastersection0 = i2 WITH noconstant(0), protect
 DECLARE _remfieldname7 = i4 WITH noconstant(1), protect
 DECLARE _remfieldname6 = i4 WITH noconstant(1), protect
 DECLARE _bcontheaddiagnosticsection0 = i2 WITH noconstant(0), protect
 DECLARE _remfieldname19 = i4 WITH noconstant(1), protect
 DECLARE _bcontdetaildiagnosticsection2 = i2 WITH noconstant(0), protect
 ...
 SUBROUTINE nodatasection0abs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
      "No data found for the specified prompt(s).",char(0)))
 ...
 END ;Subroutine
 SUBROUTINE headpagemastersection0(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headpagemastersection0abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headpagemastersection0abs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Charge Event Report",char(0)))
    SET rptsd->m_flags = 4
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("M_EVENT_ID",char(0)))
    SET rptsd->m_y = (offsety+ 0.604)
    SET rptsd->m_x = (offsetx+ 0.948)
    SET rptsd->m_width = 0.760
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("ORDER_ID",char(0)))
    SET rptsd->m_y = (offsety+ 0.604)
    SET rptsd->m_x = (offsetx+ 1.865)
    SET rptsd->m_width = 0.760
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("C_EVENT_ID",char(0)))
    SET rptsd->m_y = (offsety+ 0.604)
    ...
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("BILL ITEM DESCRIPTION",char(0)))
    ...
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("EVENT",char(0)))
    SET rptsd->m_y = (offsety+ 0.792)
    SET rptsd->m_x = (offsetx+ 2.500)
    ...
    SET rptsd->m_width = 1.240
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("EVENT DATE",char(0)))
    SET rptsd->m_y = (offsety+ 0.792)
    SET rptsd->m_x = (offsetx+ 5.500)
    SET rptsd->m_width = 0.750
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("CHARGE",char(0)))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.928),(offsetx+ 7.521),(offsety+
     0.928))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE headpagediagnosticsection0(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headpagediagnosticsection0abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headpagediagnosticsection0abs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Charge Creation Diagnostics",char(0))
     )
    SET rptsd->m_flags = 0
    ...
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("CEVENTACT_ID",char(0)))
    SET rptsd->m_y = (offsety+ 0.938)
    ...
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("UPDATE DATE",char(0)))
    SET rptsd->m_y = (offsety+ 0.938)
    SET rptsd->m_x = (offsetx+ 3.313)
    SET rptsd->m_width = 0.625
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("TIER",char(0)))
    SET rptsd->m_y = (offsety+ 0.938)
    SET rptsd->m_x = (offsetx+ 4.250)
    SET rptsd->m_width = 1.021
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("REASON",char(0)))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.010),(offsety+ 1.084),(offsetx+ 7.500),(offsety+
     1.084))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE headmastersection0(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headmastersection0abs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headmastersection0abs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   DECLARE __fieldname0 = vc WITH noconstant(build2(cnvtstring(events->events_detail[i].master_id),
     char(0))), protect
   DECLARE __fieldname1 = vc WITH noconstant(build2(cnvtstring(events->events_detail[i].order_id),
     char(0))), protect
   DECLARE __fieldname2 = vc WITH noconstant(build2(cnvtstring(events->events_detail[i].
      charge_event_id),char(0))), protect
   DECLARE __fieldname3 = vc WITH noconstant(build2(events->events_detail[i].accession,char(0))),
   protect
   DECLARE __fieldname4 = vc WITH noconstant(build2(
     IF (textlen(trim(events->events_detail[i].desc))=0) "**No Bill Item Found**"
     ELSE events->events_detail[i].desc
     ENDIF
     ,char(0))), protect
   DECLARE __fieldname5 = vc WITH noconstant(build2(events->events_detail[i].person_name,char(0))),
   protect
   IF (bcontinue=0)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE headdiagnosticsection0(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headdiagnosticsection0abs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headdiagnosticsection0abs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   DECLARE __fieldname3 = vc WITH noconstant(build2(events->events_detail[i].accession,char(0))),
   protect
   DECLARE __fieldname7 = vc WITH noconstant(build2(events->events_detail[i].person_name,char(0))),
   protect
   DECLARE __fieldname6 = vc WITH noconstant(build2(
     IF (textlen(trim(events->events_detail[i].desc))=0) "**No Bill Item Found**"
     ELSE events->events_detail[i].desc
     ENDIF
     ,char(0))), protect
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE detailmastersection(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = detailmastersectionabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE detailmastersectionabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.200000), private
   DECLARE __fieldname0 = vc WITH noconstant(build2(events->events_detail[i].master_detail[ii].
     cea_type,char(0))), protect
   DECLARE __fieldname1 = vc WITH noconstant(build2(events->events_detail[i].master_detail[ii].
     service_dt_tm,char(0))), protect
   DECLARE __fieldname2 = vc WITH noconstant(build2(events->events_detail[i].master_detail[ii].
     update_dt_tm,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET rptsd->m_width = 0.750
    SET rptsd->m_height = 0.198
    IF ((events->events_detail[i].master_detail[ii].charge_item_id > 0))
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE detaildiagnosticsection(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = detaildiagnosticsectionabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE detaildiagnosticsectionabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.200000), private
   DECLARE __fieldname0 = vc WITH noconstant(build2(cnvtstring(events->events_detail[i].
      master_detail[ii].charge_event_act_id),char(0))), protect
   DECLARE __fieldname1 = vc WITH noconstant(build2(events->events_detail[i].master_detail[ii].
     cea_type,char(0))), protect
   DECLARE __fieldname2 = vc WITH noconstant(build2(events->events_detail[i].master_detail[ii].
     update_dt_tm,char(0))), protect
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE detaildiagnosticsection2(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = detaildiagnosticsection2abs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE detaildiagnosticsection2abs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.380000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_fieldname19 = f8 WITH noconstant(0.0), private
   DECLARE __fieldname0 = vc WITH noconstant(build2(trim(events->events_detail[i].master_detail[ii].
      srvr_diagostic_detail[iii].diagnostic_text_two_a),char(0))), protect
   DECLARE __fieldname2 = vc WITH noconstant(build2(trim(events->events_detail[i].master_detail[ii].
      srvr_diagostic_detail[iii].diagnostic_text_two_b),char(0))), protect
   DECLARE __fieldname19 = vc WITH noconstant(build2(events->events_detail[i].master_detail[ii].
     srvr_diagostic_detail[iii].field7,char(0))), protect
   DECLARE __fieldname5 = vc WITH noconstant(build2(trim(events->events_detail[i].master_detail[ii].
      srvr_diagostic_detail[iii].diagnostic_text_one),char(0))), protect
   IF (bcontinue=0)
    SET _remfieldname19 = 1
   ENDIF
   SET rptsd->m_flags = 5
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   SET _oldfont = uar_rptsetfont(_hreport,_times80)
   SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
   SET _holdremfieldname19 = _remfieldname19
   IF (_remfieldname19 > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_remfieldname19,((size(
        __fieldname19) - _remfieldname19)+ 1),__fieldname19)))
    SET drawheight_fieldname19 = rptsd->m_height
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _remfieldname19 = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remfieldname19,((size(__fieldname19) -
       _remfieldname19)+ 1),__fieldname19)))))
     SET _remfieldname19 = (_remfieldname19+ rptsd->m_drawlength)
    ELSE
     SET _remfieldname19 = 0
    ENDIF
    SET growsum = (growsum+ _remfieldname19)
   ENDIF
   SET rptsd->m_flags = 0
   ...
   SET rptsd->m_x = (offsetx+ 3.313)
   SET rptsd->m_width = 0.865
   SET rptsd->m_height = drawheight_fieldname19
   IF (ncalc=rpt_render
    AND _holdremfieldname19 > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdremfieldname19,((size
       (__fieldname19) - _holdremfieldname19)+ 1),__fieldname19)))
   ELSE
    SET _remfieldname19 = _holdremfieldname19
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_y = (offsety+ 0.000)
   SET rptsd->m_x = (offsetx+ 4.250)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE detaildiagnosticsection3(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = detaildiagnosticsection3abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE detaildiagnosticsection3abs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.200000), private
   DECLARE __fieldname6 = vc WITH noconstant(build2(trim(events->events_detail[i].master_detail[ii].
      srvr_diagostic_detail[iii].diagnostic_text_three_a),char(0))), protect
   DECLARE __fieldname7 = vc WITH noconstant(build2(trim(events->events_detail[i].master_detail[ii].
      srvr_diagostic_detail[iii].diagnostic_text_three_b),char(0))), protect
   IF ( NOT (textlen(trim(events->events_detail[i].master_detail[ii].srvr_diagostic_detail[iii].
     diagnostic_text_three_a)) > 0))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE detaildiagnosticsection4(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = detaildiagnosticsection4abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE detaildiagnosticsection4abs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.200000), private
   DECLARE __fieldname1 = vc WITH noconstant(build2(events->events_detail[i].master_detail[ii].
     srvr_diagostic_detail[iii].diagnostic_text_four_a,char(0))), protect
   DECLARE __fieldname9 = vc WITH noconstant(build2(trim(events->events_detail[i].master_detail[ii].
      srvr_diagostic_detail[iii].diagnostic_text_four_b),char(0))), protect
   IF ( NOT (textlen(trim(events->events_detail[i].master_detail[ii].srvr_diagostic_detail[iii].
     diagnostic_text_four_a)) > 0))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CCPS_CS_AFC_MASTER_EM"
 ...
 IF ((events->events_cnt > 0))
  SET events_cnt = events->events_cnt
  SELECT INTO "nl:"
   FROM bill_item b
   PLAN (b
    WHERE expand(index,1,events->events_cnt,b.ext_parent_reference_id,events->events_detail[index].
     p_ref_id,
     b.ext_parent_contributor_cd,events->events_detail[index].p_ref_cd,b.ext_child_reference_id,
     events->events_detail[index].c_ref_id,b.ext_child_contributor_cd,
     events->events_detail[index].c_ref_cd)
     AND b.active_ind=1)
   DETAIL
    pos = locateval(num,1,events->events_cnt,b.ext_parent_reference_id,events->events_detail[num].
     p_ref_id,
     b.ext_parent_contributor_cd,events->events_detail[num].p_ref_cd,b.ext_child_reference_id,events
     ->events_detail[num].c_ref_id,b.ext_child_contributor_cd,
     events->events_detail[num].c_ref_cd)
    WHILE (pos > 0)
     IF (textlen(trim(events->events_detail[pos].desc))=0)
      events->events_detail[pos].desc = b.ext_description, events->events_detail[pos].bill_item_id =
      b.bill_item_id
     ENDIF
     ,pos = locateval(num,(pos+ 1),events->events_cnt,b.ext_parent_reference_id,events->
      events_detail[num].p_ref_id,
      b.ext_parent_contributor_cd,events->events_detail[num].p_ref_cd,b.ext_child_reference_id,events
      ->events_detail[num].c_ref_id,b.ext_child_contributor_cd,
      events->events_detail[num].c_ref_cd)
    ENDWHILE
  ...
  SELECT INTO "nl:"
   FROM bill_item b
   PLAN (b
    WHERE expand(index,1,events->events_cnt,b.ext_child_reference_id,events->events_detail[index].
     c_ref_id,
     b.ext_child_contributor_cd,events->events_detail[index].c_ref_cd)
     AND b.ext_parent_reference_id=0
     AND b.ext_parent_contributor_cd=0
     AND b.active_ind=1)
   DETAIL
    pos = locateval(num,1,events->events_cnt,b.ext_child_reference_id,events->events_detail[num].
     c_ref_id,
     b.ext_child_contributor_cd,events->events_detail[num].c_ref_cd)
    WHILE (pos > 0)
     IF (textlen(trim(events->events_detail[pos].desc))=0)
      events->events_detail[pos].desc = b.ext_description, events->events_detail[pos].bill_item_id =
      b.bill_item_id
     ENDIF
     ,pos = locateval(num,(pos+ 1),events->events_cnt,b.ext_child_reference_id,events->events_detail[
      num].c_ref_id,
      b.ext_child_contributor_cd,events->events_detail[num].c_ref_cd)
    ENDWHILE
   WITH nocounter, expand = 1
  ...
  SELECT INTO "nl:"
   ...
   PLAN (b
    WHERE expand(index,1,events->events_cnt,b.ext_parent_reference_id,events->events_detail[index].
     c_ref_id,
     b.ext_parent_contributor_cd,events->events_detail[index].c_ref_cd)
   ...
   DETAIL
    pos = locateval(num,1,events->events_cnt,b.ext_parent_reference_id,events->events_detail[num].
     c_ref_id,
     b.ext_parent_contributor_cd,events->events_detail[num].c_ref_cd)
    WHILE (pos > 0)
     IF (textlen(trim(events->events_detail[pos].desc))=0)
      events->events_detail[pos].desc = b.ext_description, events->events_detail[pos].bill_item_id =
      b.bill_item_id
  ...
  SELECT INTO "nl:"
   FROM charge_event_act cea,
    (left JOIN prsnl p ON p.person_id=cea.cea_prsnl_id),
    (left JOIN charge_event_mod cem ON cem.charge_event_id=cea.charge_event_id
     AND cem.field1_id=cea.charge_event_act_id
     AND cem.charge_event_mod_type_cd=server_diag_cd),
    (left JOIN charge c ON c.charge_event_act_id=cea.charge_event_act_id),
    (left JOIN organization o ON o.organization_id=cem.field3_id
     AND cem.field2_id IN (no_chrg_pnt_on_tier_cd, no_tier_for_org_cd)),
    (left JOIN price_sched ps ON ps.price_sched_id=cem.field3_id
     AND cem.field2_id=no_chrg_ind_cd),
    (left JOIN bill_item b ON b.bill_item_id=cem.field5_id
     AND cem.field2_id=not_at_chrg_lvl_cd)
   PLAN (cea
    WHERE expand(index,1,events->events_cnt,cea.charge_event_id,events->events_detail[index].
     charge_event_id))
    JOIN (cem)
    JOIN (c)
    JOIN (o)
    ...
    JOIN (b)
    JOIN (p)
   ORDER BY cea.charge_event_id, cea.charge_event_act_id, cem.charge_event_mod_id
   HEAD cea.charge_event_id
    master_cnt = 0, pos = locateval(num,1,events->events_cnt,cea.charge_event_id,events->
     events_detail[num].charge_event_id)
   HEAD cea.charge_event_act_id
    IF (pos > 0)
     master_cnt = (master_cnt+ 1), stat = alterlist(events->events_detail[pos].master_detail,
      master_cnt), events->events_detail[pos].master_detail[master_cnt].cea_type =
     uar_get_code_display(cea.cea_type_cd),
     events->events_detail[pos].master_detail[master_cnt].cea_type_cd = cea.cea_type_cd, events->
     events_detail[pos].master_detail[master_cnt].charge_event_act_id = cea.charge_event_act_id,
     events->events_detail[pos].master_detail[master_cnt].service_dt_tm = format(cea.service_dt_tm,
      "@SHORTDATETIME"),
     events->events_detail[pos].master_detail[master_cnt].update_dt_tm = format(cea.updt_dt_tm,
      "@SHORTDATETIME"), events->events_detail[pos].master_cnt = master_cnt
    ENDIF
    srvr_diagostic_cnt = 0
   HEAD cem.charge_event_mod_id
    IF (pos > 0)
     IF (cea.charge_type_cd=workload_only_cd)
      srvr_diagostic_cnt = (srvr_diagostic_cnt+ 1), stat = alterlist(events->events_detail[pos].
       master_detail[master_cnt].srvr_diagostic_detail,srvr_diagostic_cnt), events->events_detail[pos
      ].master_detail[master_cnt].srvr_diagostic_cnt = srvr_diagostic_cnt,
      events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].
      diagnostic_text_one = "Charge type is WORKOADONLY"
     ELSEIF (cem.field2_id > 0.00)
      srvr_diagostic_cnt = (srvr_diagostic_cnt+ 1), stat = alterlist(events->events_detail[pos].
       master_detail[master_cnt].srvr_diagostic_detail,srvr_diagostic_cnt), events->events_detail[pos
      ].master_detail[master_cnt].srvr_diagostic_cnt = srvr_diagostic_cnt,
      events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].
      diagnostic_text_one = trim(cem.field6)
      IF (cem.field2_id=not_at_chrg_pnt_cd)
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_two_a = "Charge Sched:", events->events_detail[pos].master_detail[master_cnt]
       .srvr_diagostic_detail[srvr_diagostic_cnt].diagnostic_text_two_b = trim(uar_get_code_display(
         cem.field3_id)), events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[
       srvr_diagostic_cnt].diagnostic_text_three_a = "Point on Item:",
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_three_b = trim(uar_get_code_display(cem.field4_id))
      ELSEIF (cem.field2_id=no_chrg_pnt_cd)
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_two_a = "Charge Sched:", events->events_detail[pos].master_detail[master_cnt]
       .srvr_diagostic_detail[srvr_diagostic_cnt].diagnostic_text_two_b = trim(uar_get_code_display(
         cem.field3_id))
      ELSEIF (cem.field2_id=no_chrg_pnt_on_tier_cd)
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_two_a = "Organization:", events->events_detail[pos].master_detail[master_cnt]
       .srvr_diagostic_detail[srvr_diagostic_cnt].diagnostic_text_two_b = trim(o.org_name), events->
       events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].
       diagnostic_text_three_a = "Tier Group:",
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_three_b = trim(uar_get_code_display(cem.field4_id))
      ELSEIF (cem.field2_id=not_at_chrg_lvl_cd)
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_two_a = "Charge Sched:", events->events_detail[pos].master_detail[master_cnt]
       .srvr_diagostic_detail[srvr_diagostic_cnt].diagnostic_text_two_b = trim(uar_get_code_display(
         cem.field3_id)), events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[
       srvr_diagostic_cnt].diagnostic_text_three_a = "Charge Level:",
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_three_b = trim(uar_get_code_display(cem.field4_id)), events->events_detail[
       pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].
       diagnostic_text_four_a = "Bill Item ID:", events->events_detail[pos].master_detail[master_cnt]
       .srvr_diagostic_detail[srvr_diagostic_cnt].diagnostic_text_four_b = trim(b.ext_description)
      ELSEIF (cem.field2_id=no_chrg_ind_cd)
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_two_a = "Price Sched:", events->events_detail[pos].master_detail[master_cnt].
       srvr_diagostic_detail[srvr_diagostic_cnt].diagnostic_text_two_b = trim(ps.price_sched_desc)
      ELSEIF (cem.field2_id=no_tier_for_org_cd)
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_two_a = "Organization:", events->events_detail[pos].master_detail[master_cnt]
       .srvr_diagostic_detail[srvr_diagostic_cnt].diagnostic_text_two_b = trim(o.org_name)
      ELSEIF (cem.field2_id=no_phleb_chrg_cd)
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_two_a = "Person:", events->events_detail[pos].master_detail[master_cnt].
       srvr_diagostic_detail[srvr_diagostic_cnt].diagnostic_text_two_b = trim(p.name_full_formatted),
       events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt]
       .diagnostic_text_three_a = "PhlebCharging:"
       IF (cem.field3_id=1)
        events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt
        ].diagnostic_text_three_b = "ON"
       ELSE
        events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt
        ].diagnostic_text_three_b = "OFF"
       ENDIF
      ENDIF
     ELSEIF (cem.field2_id=0.00
      AND cea.cea_type_cd=loaded_cea_type_cd)
      srvr_diagostic_cnt = (srvr_diagostic_cnt+ 1), stat = alterlist(events->events_detail[pos].
       master_detail[master_cnt].srvr_diagostic_detail,srvr_diagostic_cnt), events->events_detail[pos
      ].master_detail[master_cnt].srvr_diagostic_cnt = srvr_diagostic_cnt,
      events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].
      diagnostic_text_one = "Invalid Charge Point/No Charge Event Diagnostic Row Available"
     ENDIF
     IF (textlen(trim(cem.field7)) > 0
      AND cea.cea_type_cd != loaded_cea_type_cd)
      events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].
      charge_event_mod_id = cem.charge_event_mod_id, events->events_detail[pos].master_detail[
      master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].field7 = trim(uar_get_code_display(
        cnvtreal(cem.field7)))
     ENDIF
    ENDIF
   DETAIL
    IF (pos > 0)
     IF (c.charge_item_id > 0
      AND c.charge_item_id != temp_charge_item_id)
      srvr_diagostic_cnt = (events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_cnt+
      1), stat = alterlist(events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail,
       srvr_diagostic_cnt), events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_cnt
       = srvr_diagostic_cnt,
      events->events_detail[pos].master_detail[master_cnt].charge_item_id = c.charge_item_id, events
      ->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].
      diagnostic_text_one = "A charge was created", events->events_detail[pos].master_detail[
      master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].diagnostic_text_two_a = "Charge Item ID:",
      events->events_detail[pos].master_detail[master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].
      diagnostic_text_two_b = cnvtstring(c.charge_item_id), events->events_detail[pos].master_detail[
      master_cnt].srvr_diagostic_detail[srvr_diagostic_cnt].field7 = trim(uar_get_code_display(c
        .tier_group_cd)), temp_charge_item_id = c.charge_item_id
     ENDIF
    ENDIF
   FOOT  cem.charge_event_mod_id
    null
   ...
   WITH nocounter, expand = 1
  ;end select
  SET stat = alterlist(events->events_detail,events_cnt)
  SET events->events_cnt = events_cnt
  IF (((( $RPT_TYPE="Charge Event Report")) OR (textlen( $RPT_TYPE)=0)) )
   SELECT INTO  $OUTDEV
    primary_sort = events->events_detail[d.seq].charge_event_id, secondary_sort = events->
    events_detail[d.seq].master_detail[d1.seq].charge_event_act_id
    FROM (dummyt d  WITH seq = value(events->events_cnt)),
     (dummyt d1  WITH seq = 1)
    PLAN (d
     WHERE maxrec(d1,events->events_detail[d.seq].master_cnt))
     JOIN (d1)
    ORDER BY primary_sort, secondary_sort
    HEAD PAGE
     d0 = headpagemastersection0(rpt_render)
    HEAD primary_sort
     i = d.seq
     IF (((_yoffset+ headmastersection0(rpt_calcheight,2.0,_bcontheadmastersection0)) > 10.2))
      d0 = pagebreak(0), d0 = headpagemastersection0(rpt_render)
     ENDIF
     d0 = headmastersection0(rpt_render,2.0,_bcontheadmastersection0)
    DETAIL
     ii = d1.seq
     IF (((_yoffset+ detailmastersection(rpt_calcheight)) > 10.2))
      d0 = pagebreak(0), d0 = headpagemastersection0(rpt_render)
     ENDIF
     d0 = detailmastersection(rpt_render)
    FOOT  primary_sort
     null
    FOOT REPORT
  ...
  ELSE
   SELECT INTO  $OUTDEV
    primary_sort = events->events_detail[d.seq].charge_event_id, secondary_sort = events->
    events_detail[d.seq].master_detail[d1.seq].charge_event_act_id, tertiary_sort = events->
    events_detail[d.seq].master_detail[d1.seq].srvr_diagostic_detail[d2.seq].charge_event_mod_id
    FROM (dummyt d  WITH seq = value(events->events_cnt)),
     (dummyt d1  WITH seq = 1),
    ...
    PLAN (d
     WHERE maxrec(d1,events->events_detail[d.seq].master_cnt))
     JOIN (d1
     WHERE maxrec(d2,events->events_detail[d.seq].master_detail[d1.seq].srvr_diagostic_cnt))
     JOIN (d2)
    ORDER BY primary_sort, secondary_sort, tertiary_sort
    HEAD REPORT
     dashline = 0
    HEAD PAGE
     d0 = headpagediagnosticsection0(rpt_render)
    HEAD primary_sort
     i = d.seq
     IF (((_yoffset+ headdiagnosticsection0(rpt_calcheight,2.0,_bcontheaddiagnosticsection0)) > 10.2)
     )
      dashline = 0, d0 = pagebreak(0), d0 = headpagediagnosticsection0(rpt_render)
     ENDIF
     d0 = headdiagnosticsection0(rpt_render,2.0,_bcontheaddiagnosticsection0)
     IF (dashline=0)
      dashline = 1
     ENDIF
    HEAD secondary_sort
     ii = d1.seq, iii = d2.seq
     IF (((_yoffset+ detaildiagnosticsection(rpt_calcheight)) > 10.2))
      d0 = pagebreak(0), d0 = headpagediagnosticsection0(rpt_render)
     ENDIF
     d0 = detaildiagnosticsection(rpt_render)
    HEAD tertiary_sort
     null
    DETAIL
     iii = d2.seq
     IF (((_yoffset+ detaildiagnosticsection2(rpt_calcheight,2.0,_bcontdetaildiagnosticsection2)) >
     10.2))
      d0 = pagebreak(0), d0 = headpagediagnosticsection0(rpt_render)
     ENDIF
     d0 = detaildiagnosticsection2(rpt_render,2.0,_bcontdetaildiagnosticsection2)
     IF (((_yoffset+ detaildiagnosticsection3(rpt_calcheight)) > 10.2))
      d0 = pagebreak(0), d0 = headpagediagnosticsection0(rpt_render)
     ENDIF
     d0 = detaildiagnosticsection3(rpt_render)
     IF (((_yoffset+ detaildiagnosticsection4(rpt_calcheight)) > 10.2))
      d0 = pagebreak(0), d0 = headpagediagnosticsection0(rpt_render)
     ENDIF
     d0 = detaildiagnosticsection4(rpt_render)
    FOOT  tertiary_sort
     null
    FOOT  secondary_sort
     null
 ...
 SET last_mod = "10/30/2012 MP9098"
END GO
