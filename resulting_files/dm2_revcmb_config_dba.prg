CREATE PROGRAM dm2_revcmb_config:dba
 ...
 SUBROUTINE dm2_get_program_stack(null)
   ...
   ENDIF
 END ;Subroutine
 DECLARE sinfo_domain = vc WITH constant("DM COMBINE"), protect
 DECLARE sinfo_name = vc WITH constant("REVERSE COMBINE WITH PRSNL"), protect
 UPDATE  FROM dm_info d
  SET d.info_number = 1, d.info_char = curuser, d.updt_cnt = (d.updt_cnt+ 1),
   d.updt_dt_tm = cnvtdatetime(sysdate), d.updt_id = reqinfo->updt_id, d.updt_task = reqinfo->
   updt_task
  WHERE d.info_domain="DM COMBINE"
   AND d.info_name="REVERSE DEMOGRAPHIC COMBINE"
  WITH nocounter
 ;end update
 IF (check_error("Updating dm_info reverse demographic flag...") != 0)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSEIF (curqual=0)
  INSERT  FROM dm_info d
   SET d.info_domain = "DM COMBINE", d.info_name = "REVERSE DEMOGRAPHIC COMBINE", d.info_number = 1,
    d.info_char = curuser, d.updt_cnt = 0, d.updt_dt_tm = cnvtdatetime(sysdate),
    d.updt_id = reqinfo->updt_id, d.updt_task = reqinfo->updt_task
   WITH nocounter
  ;end insert
  IF (check_error("Inserting dm_info reverse demographic flag...") != 0)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ...
 UPDATE  FROM dm_info di
  SET di.info_number = 1, di.info_char = curuser, di.updt_cnt = (di.updt_cnt+ 1),
   di.updt_dt_tm = cnvtdatetime(sysdate), di.updt_id = reqinfo->updt_id, di.updt_task = reqinfo->
   updt_task
  WHERE di.info_domain=sinfo_domain
   AND di.info_name=sinfo_name
  WITH nocounter
 ;end update
 IF (check_error("Updating dm_info reverse combine PRSNL flag...") != 0)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSEIF (curqual=0)
  INSERT  FROM dm_info di
   SET di.info_domain = sinfo_domain, di.info_name = sinfo_name, di.info_number = 1,
    di.info_char = curuser, di.updt_cnt = 0, di.updt_dt_tm = cnvtdatetime(sysdate),
    di.updt_id = reqinfo->updt_id, di.updt_task = reqinfo->updt_task
   WITH nocounter
  ;end insert
  IF (check_error("Inserting dm_info reverse combine PRSNL flag...") != 0)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ...
 CALL echo("As of the Millennium 2018.17 release reverse combines are no longer configurable.")
 CALL echo("Reverse Demographic Combines are turned ON.")
 CALL echo("Reverse Demographic Combines with PRSNL are turned ON.")
 CALL echo(fillstring(90,"*"))
#exit_program
END GO
