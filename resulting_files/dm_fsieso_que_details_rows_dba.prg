CREATE PROGRAM dm_fsieso_que_details_rows:dba
 CALL echo("<===== DM_FSIESO_QUE_DETAILS_ROWS BEGIN =====>")
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(d.queue_id)
   FROM fsieso_que_details d
   WHERE d.queue_id > 0
   DETAIL
 ...
 SET reply->table_name = "FSIESO_QUE_DETAILS"
 SET reply->rows_between_commit = 100
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM fsieso_que_details d
    WHERE parser(sbr_getrowidnotexists("d.queue_id between curMinID and curMaxID","d"))
     AND  NOT ( EXISTS (
    (SELECT
     e.queue_id
     FROM cqm_fsieso_que e
     WHERE e.queue_id=d.queue_id)))
    DETAIL
     rows += 1
     IF (mod(rows,100)=1)
      stat = alterlist(reply->rows,(rows+ 99))
     ENDIF
     reply->rows[rows].row_id = d.rowid
    WITH nocounter, maxqual(d,value(rowsleft))
   ;end select
...
#exit_script
 CALL echo("<===== DM_FSIESO_QUE_DETAILS_ROWS END =====>")
END GO
