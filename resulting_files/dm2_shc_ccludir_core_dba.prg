CREATE PROGRAM dm2_shc_ccludir_core:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsc_err_status = c1 WITH protect, noconstant(" ")
 IF (check_logfile("DM2_SHC_CCLUDIR_CORE",".log","DM2_SHC_CCLUDIR_CORE LOG FILE")=0)
 ...
 IF (cursys="AXP")
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "System Health Check working on CCLUSERDIR core files..."
 CALL disp_msg("",dm_err->logfile,0)
 ...
 ENDIF
 SET dm_err->disp_dcl_err_ind = 0
 IF (dm2_push_dcl("ls -l $CCLUSERDIR/core")=0)
  IF ((dm_err->err_ind=1))
   SET dsc_err_status = "E"
  ELSE
   IF (findstring("does not exist",dm_err->errtext,1) > 0)
    SET dsc_err_status = "G"
   ELSE
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dsc_err_status = "E"
  ...
  ENDIF
 ELSE
  SET dsc_err_status = "R"
 ENDIF
 IF (dshc_add_check_to_rs("CCLUSERDIR CORE FILES","CCLUSERDIR Core Files",0,dsc_err_status,
  dshc_ctf_millennium)=0)
  SET dsc_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dsc_err_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 ...
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "CCLUSERDIR core files"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Core File(s) in CCLUSERDIR: "
 ...
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "if a core dump file is written to the $CCLUSERDIR directory it may be an indication ",
  "that an application server terminated abnormally.  Accessing the system error report ",
  "may help determine which application server abnormally terminated.")
#exit_script
 ...
 IF (dsc_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_CCLUDIR_CORE WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_CCLUDIR_CORE WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_CCLUDIR_CORE COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_CCLUDIR_CORE")
END GO
