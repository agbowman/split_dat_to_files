CREATE PROGRAM dcp_get_person_prsnl_tasks:dba
 RECORD internal(
   1 task_list[*]
     2 task_id = f8
 )
 ...
 SELECT
  IF (types_to_get > 0
   ...
   FROM (dummyt d  WITH seq = value(tasks_to_get)),
    ...
    (dummyt d5  WITH seq = value(encntrs_to_get)),
    task_activity ta,
   ...
   PLAN (d)
    ...
    JOIN (d3)
    JOIN (ta
    WHERE (((ta.task_id=internal->task_list[d.seq].task_id)) OR ((ta.encntr_id=request->encntr_list[
    d5.seq].encntr_id)))
     AND (ta.task_type_cd=request->type_list[d3.seq].task_type_cd)
     AND ta.active_ind=1
  ...
  ELSEIF (types_to_get > 0
   ...
   AND encntrs_to_get=0)
   FROM (dummyt d  WITH seq = value(tasks_to_get)),
    (dummyt d2  WITH seq = value(persons_to_get)),
    (dummyt d3  WITH seq = value(types_to_get)),
    task_activity ta,
    (dummyt d4  WITH seq = 1),
    task_activity_assignment taa
   PLAN (d)
    ...
    JOIN (d3)
    JOIN (ta
    WHERE (((ta.task_id=internal->task_list[d.seq].task_id)) OR ((ta.person_id=request->person_list[
    d2.seq].person_id)))
     AND (ta.task_type_cd=request->type_list[d3.seq].task_type_cd)
     AND ta.active_ind=1
     AND ta.task_status_cd != dropped
  ...
  ELSEIF (types_to_get=0
   ...
   PLAN (d)
    ...
    WHERE (((ta.task_id=internal->task_list[d.seq].task_id)) OR ((ta.person_id=request->person_list[
    d2.seq].person_id)))
     AND (ta.encntr_id=request->person_list[d5.seq].encntr_id)
     AND ta.active_ind=1
     AND ta.task_status_cd != dropped
     AND ta.task_status_cd != inerror
     AND ((ta.task_dt_tm BETWEEN cnvtdatetime(request->beg_dt_tm) AND cnvtdatetime(request->end_dt_tm
  ...
  ELSEIF (types_to_get=0
   ...
   PLAN (d)
    ...
    WHERE (((ta.task_id=internal->task_list[d.seq].task_id)) OR ((ta.person_id=request->person_list[
    d2.seq].person_id)))
     AND (ta.encntr_id=request->encntr_list[d5.seq].encntr_id)
     AND ta.active_ind=1
     AND ta.task_status_cd != dropped
     AND ta.task_status_cd != inerror)
    JOIN (d4)
