CREATE PROGRAM cp_query_pl_pathway_list:dba
 RECORD definition(
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 DECLARE loadparameters(null) = null WITH protect
 DECLARE loadpatients(null) = null WITH protect
 DECLARE pathwaywhere = vc WITH noconstant("1=1")
 DECLARE dproviderid = f8 WITH noconstant(0)
 DECLARE reltnwhere = vc WITH noconstant("1=1")
 ...
 CALL loadparameters(null)
 CALL loadpatients(null)
 SUBROUTINE loadparameters(null)
   CALL log_message("In LoadParameters()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE x = i4 WITH noconstant(0)
   DECLARE paramsize = i4 WITH noconstant(0)
   SET paramsize = size(definition->parameters[1].values,5)
   IF (paramsize > 0)
    SET pathwaywhere = "cpa.cp_pathway_id in ("
    FOR (x = 1 TO paramsize)
      SET pathwaywhere = build(pathwaywhere,cnvtstring(definition->parameters[1].values[x].value_id,
        16,3),",")
    ENDFOR
    SET pathwaywhere = replace(pathwaywhere,",",")",2)
   ENDIF
   SET paramsize = size(definition->parameters[2].values,5)
   IF (paramsize=1)
    SET dproviderid = definition->parameters[2].values[1].value_id
   ENDIF
   SET paramsize = size(definition->parameters[3].values,5)
   IF (paramsize > 0)
    SET reltnwhere = "epr.encntr_prsnl_r_cd in ("
    FOR (x = 1 TO paramsize)
      SET reltnwhere = build(reltnwhere,cnvtstring(definition->parameters[3].values[x].value_id,16,3),
       ",")
    ENDFOR
    SET reltnwhere = replace(reltnwhere,",",")",2)
   ENDIF
   CALL log_message(build("Exit LoadParameters(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE loadpatients(null)
   CALL log_message("In LoadPatients()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE pw_activity_status_active = f8 WITH constant(uar_get_code_by("MEANING",4003352,"ACTIVE"))
   SELECT INTO "NL:"
    FROM cp_pathway_activity cpa,
     encntr_prsnl_reltn epr
    PLAN (cpa
     WHERE cpa.pathway_activity_status_cd=pw_activity_status_active
      AND cpa.beg_effective_dt_tm < cnvtdatetime(sysdate)
      AND cpa.end_effective_dt_tm > cnvtdatetime(sysdate)
      AND parser(trim(pathwaywhere,3)))
     JOIN (epr
     WHERE epr.encntr_id=cpa.encntr_id
      AND parser(trim(reltnwhere,3))
      ...
      AND epr.prsnl_person_id=dproviderid)
    ORDER BY cpa.person_id, cpa.encntr_id
    HEAD REPORT
     cnt = 1
    HEAD cpa.encntr_id
     cnt += 1
     IF (cnt > size(patients->patients,5))
      stat = alterlist(patients->patients,(cnt+ 20))
     ENDIF
     patients->patients[cnt].person_id = cpa.person_id, patients->patients[cnt].encntr_id = cpa
     .encntr_id
    FOOT REPORT
     stat = alterlist(patients->patients,cnt)
    WITH nocounter
   ;end select
   CALL error_and_zero_check_rec(curqual,"CP_PATHWAY_ACTIVITY","LoadPatients",1,0,
    patients)
   CALL log_message(build("Exit LoadPatients(), Elapsed time in seconds:",((curtime3 - begin_curtime3
