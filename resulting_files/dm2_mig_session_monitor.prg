CREATE PROGRAM dm2_mig_session_monitor
 ...
 END ;Subroutine
 SET width = 132
 DECLARE dmsm_get_lines(dgl_window_mode=i2) = i2
 DECLARE dmsm_scroll(ds_rec_str=vc) = vc
 DECLARE dmsm_driver(dd_leave=i2(ref)) = i2
 DECLARE dmsm_work_menu(dwm_screen_return=vc(ref)) = i2
 DECLARE dmsm_screen_fill(null) = i2
 DECLARE dmsm_kill_session(dks_sid=f8,dks_serial=f8) = i2
 DECLARE dmsm_gather_connect_info(null) = i2
 DECLARE dmsm_leave = i2 WITH protect, noconstant(0)
 DECLARE dmsm_secondary_connect_str = vc WITH protect, noconstant("")
 FREE RECORD dmsm_inst
 RECORD dmsm_inst(
   1 inst_cnt = i4
   1 inst[*]
     2 inst_connect_str = vc
     2 inst_name = vc
     2 inst_id = f8
 )
 FREE RECORD dmsm_screen
 RECORD dmsm_screen(
   1 top_line = i4
   1 bottom_line = i4
   1 cur_line = i4
 ...
 IF (check_logfile("dm2_mig_sesmon",".log","dm2_mig_session_monitor")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_session_monitor"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 IF (dmsm_gather_connect_info(null)=0)
  GO TO exit_program
 ENDIF
 IF (dmsm_get_lines(0)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag != 722))
  SET message = window
 ENDIF
 SET dmsm_line->header_line = concat(substring(1,30,"Instance Name"),"  ",substring(1,15,
   "Instance ID"),"  ",substring(1,15,"Sid"),
  "  ",substring(1,15,"Serial#"),"  ",substring(1,20,"Schema Name"),"  ",
  substring(1,30,"OS User"),"  ",substring(1,8,"Status"),"  ",substring(1,48,"Program"),
  "  ",substring(1,50,"Machine"))
 WHILE (dmsm_leave=0)
   SET dmsm_screen->top_line = 1
   SET dmsm_screen->cur_line = 1
   SET dmsm_screen->bottom_line = 0
   SET dmsm_screen->max_scroll = 16
   SET dmsm_screen->max_col = 120
   SET dmsm_screen->cur_col = 1
   SET dmsm_screen->min_col = 1
   IF (dmsm_driver(dmsm_leave)=0)
    GO TO exit_program
   ENDIF
   IF ((dm_err->err_ind=1))
    GO TO exit_program
 ...
 ENDWHILE
 GO TO exit_program
 SUBROUTINE dmsm_gather_connect_info(null)
   DECLARE dgci_cnt = i2 WITH protect, noconstant(0)
   SET dmsm_inst->inst_cnt = dmr_mig_data->src_node_cnt
   SET stat = alterlist(dmsm_inst->inst,dmr_mig_data->src_node_cnt)
   FOR (dgci_cnt = 1 TO dmr_mig_data->src_node_cnt)
     SET dmsm_inst->inst[dgci_cnt].inst_name = dmr_mig_data->src_nodes[dgci_cnt].instance_name
     SET dmsm_inst->inst[dgci_cnt].inst_id = dmr_mig_data->src_nodes[dgci_cnt].instance_number
     IF (dgci_cnt=1)
      SET dmsm_inst->inst[dgci_cnt].inst_connect_str = dmr_mig_data->src_v500_cnct_str
     ENDIF
   ENDFOR
   FOR (dgci_cnt = 2 TO dmsm_inst->inst_cnt)
     SET dm_err->eproc = concat("Obtain connection information for instance ",dmsm_inst->inst[
      dgci_cnt].inst_name,".")
     CALL disp_msg(" ",dm_err->logfile,0)
     SET dm2_install_schema->dbase_name = dmsm_inst->inst[dgci_cnt].inst_name
     SET dm2_install_schema->u_name = "V500"
     EXECUTE dm2_connect_to_dbase "PC"
     IF ((dm_err->err_ind=1))
      RETURN(0)
     ENDIF
     SET dmsm_inst->inst[dgci_cnt].inst_connect_str = dm2_install_schema->connect_str
   ENDFOR
   IF ((dmsm_inst->inst_cnt > 1))
    IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
     RETURN(0)
    ENDIF
   ENDIF
 ...
 SUBROUTINE dmsm_driver(dd_leave)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmsm_work_menu(dwm_screen_return)
   ...
   CALL clear(1,1)
   CALL dmsm_main_screen_header(null)
   SET display_line = 5
   SET dmsm_screen->max_value = dmsm_line->line_cnt
   IF ((dmsm_screen->cur_line <= 1))
    SET dmsm_screen->top_line = 1
    SET dmsm_screen->cur_line = 1
    SET dmsm_screen->bottom_line = 0
   ENDIF
   IF (dmsm_screen_fill(null)=0)
    RETURN(0)
   ENDIF
   SET dwm_screen_return = ""
   SET dwm_scroll_accept = dmsm_scroll("dmsm_screen")
   CASE (dwm_scroll_accept)
    OF "K":
     IF ((dmsm_line->line_cnt > 0))
      CALL clear(23,1,132)
      CALL text(23,2,
       "Confirm Kill:  (C)onfirm that selected session can be killed or (Q)uit without killing :")
      CALL accept(23,92,"p;cu"," "
       WHERE curaccept IN ("C", "Q"))
      CALL clear(23,1,132)
      IF (curaccept="C")
       IF (dmsm_kill_session(dmsm_line->line[dmsm_screen->cur_line].sid,dmsm_line->line[dmsm_screen->
        cur_line].serial)=0)
        RETURN(0)
       ENDIF
       IF (dmsm_get_lines(1)=0)
        RETURN(0)
       ENDIF
      ENDIF
     ELSE
      CALL text(23,2,"No sessions available to kill. Enter (S) to refresh sessions.")
      CALL pause(2)
      CALL clear(23,1,132)
     ENDIF
    OF "E":
    ...
    OF "S":
     ...
     ENDIF
    OF "R":
     SET dmsm_screen->cur_col = least(dmsm_screen->max_col,value((dmsm_screen->cur_col+ 119)))
    OF "L":
     SET dmsm_screen->cur_col = greatest(1,value((dmsm_screen->cur_col - 119)))
   ENDCASE
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmsm_kill_session(dks_sid,dks_serial)
   DECLARE dks_ndx = i4 WITH protect, noconstant(0)
   IF ((dmsm_line->line[dmsm_screen->cur_line].inst_id != 1))
    SET dm2_install_schema->dbase_name = dmr_mig_data->src_db_name
    SET dm2_install_schema->u_name = "V500"
    SET dm2_install_schema->p_word = dmr_mig_data->src_v500_pwd
    SET dks_ndx = locateval(dks_ndx,1,dmsm_inst->inst_cnt,dmsm_line->line[dmsm_screen->cur_line].
     inst_id,dmsm_inst->inst[dks_ndx].inst_id)
    IF (dks_ndx=0)
     SET message = nowindow
     CALL echorecord(dmsm_inst)
     CALL echo(dks_ndx)
     CALL echo(dmsm_line)
     CALL echo(dmsm_screen)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Unable to connect to instance."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    SET dm2_install_schema->connect_str = dmsm_inst->inst[dks_ndx].inst_connect_str
    EXECUTE dm2_connect_to_dbase "CO"
    IF ((dm_err->err_ind=1))
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->eproc = concat("Killing session '",build(dks_sid),",",build(dks_serial),"'")
   CALL disp_msg("",dm_err->logfile,0)
   IF (dm2_push_cmd(concat("rdb alter system kill session '",trim(cnvtstring(dks_sid)),",",trim(
      cnvtstring(dks_serial)),"' immediate go"),0)=0)
    RETURN(0)
   ENDIF
   IF ((dmsm_line->line[dmsm_screen->cur_line].inst_id != 1))
    IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmsm_scroll(ds_rec_str)
   ...
   CALL line(21,1,132,horizontal)
   CALL clear(22,1,132)
   CALL text(22,2,
    "Action Items:Refresh (S)essions; (K)ill Session; Scroll (R)ight; Scroll (L)eft; (E)xit Menu;")
   WHILE (ds_scroll_accept=" ")
    CALL accept(22,97,"p;cus"," "
     WHERE curaccept IN ("K", "E", "L", "S", "R"))
    CASE (curscroll)
     OF 0:
 ...
 END ;Subroutine
 SUBROUTINE dmsm_screen_fill(null)
   ...
   SET display_line = 5
   IF (((dmsm_screen->top_line+ dmsm_screen->max_scroll) > dmsm_screen->max_value))
    SET dmsm_screen->bottom_line = dmsm_screen->max_value
   ELSE
    SET dmsm_screen->bottom_line = (dmsm_screen->top_line+ (dmsm_screen->max_scroll - 1))
   ENDIF
   CALL text(3,2,substring(dmsm_screen->cur_col,(dmsm_screen->cur_col+ 120),dmsm_line->header_line))
   FOR (display_loop = dmsm_screen->top_line TO dmsm_screen->bottom_line)
     IF ((display_loop=dmsm_screen->cur_line))
      CALL video(r)
     ELSE
      CALL video(n)
     ENDIF
     CALL clear(display_line,1,132)
     CALL text(display_line,2,substring(dmsm_screen->cur_col,(dmsm_screen->cur_col+ 120),dmsm_line->
       line[display_loop].full_line))
     SET display_line = (display_line+ 1)
   ENDFOR
   IF (display_line < 20)
    CALL video(n)
    FOR (display_blank = display_line TO 20)
      CALL clear(display_blank,1,132)
    ENDFOR
   ENDIF
   CALL video(n)
   IF ((dmsm_line->line_cnt > 0))
    CALL text(23,2,concat("Session ",trim(cnvtstring(dmsm_screen->cur_line))," of ",trim(cnvtstring(
        dmsm_screen->max_value))," Session(s)."))
   ELSE
    CALL text(23,2,"No sessions available for monitoring.")
   ENDIF
 END ;Subroutine
 SUBROUTINE dmsm_main_screen_header(null)
   CALL clear(1,1)
   CALL text(1,53,concat("*** Session Monitoring ***"))
   CALL text(3,2,dmsm_line->header_line)
   CALL line(4,1,132,horizontal)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmsm_get_lines(dgl_window_mode)
   ...
   IF (dgl_window_mode=1)
    CALL clear(23,1,132)
    CALL text(23,2,"Refreshing session information...")
   ENDIF
   SET dmsm_line->line_cnt = 0
   SET stat = alterlist(dmsm_line->line,dmsm_line->line_cnt)
   SET dm_err->eproc = "Load connected session information."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    a.machine, a.sid, a.serial#,
    a.schemaname, a.osuser, a.status,
    a.inst_id, a.program, b.instance_name
    FROM gv$session a,
     gv$instance b
    WHERE cnvtupper(a.program) != "*EXTRACT*"
     AND b.inst_id=a.inst_id
     AND a.status != "KILLED"
     AND  NOT (a.schemaname IN ("SYS", "SYSTEM"))
     AND  NOT (a.sid IN (
    (SELECT
     c.sid
     FROM v$mystat c)))
     AND  NOT (a.program IN (
    (SELECT
     concat("ORACLE (",s.server_name,")")
     FROM gv$px_process s)))
     AND ((a.client_info != "DM2_MIGRATION") OR (a.client_info = null))
    DETAIL
     dmsm_line->line_cnt = (dmsm_line->line_cnt+ 1), stat = alterlist(dmsm_line->line,dmsm_line->
      line_cnt), dmsm_line->line[dmsm_line->line_cnt].machine = a.machine,
     dmsm_line->line[dmsm_line->line_cnt].sid = a.sid, dmsm_line->line[dmsm_line->line_cnt].serial =
     a.serial#, dmsm_line->line[dmsm_line->line_cnt].schemaname = a.schemaname,
     dmsm_line->line[dmsm_line->line_cnt].osuser = a.osuser, dmsm_line->line[dmsm_line->line_cnt].
     status = a.status, dmsm_line->line[dmsm_line->line_cnt].inst_id = a.inst_id,
     dmsm_line->line[dmsm_line->line_cnt].prg = a.program, dmsm_line->line[dmsm_line->line_cnt].
     instance_name = b.instance_name, dmsm_line->line[dmsm_line->line_cnt].full_line = concat(
      substring(1,30,dmsm_line->line[dmsm_line->line_cnt].instance_name),"  ",substring(1,15,
       cnvtstring(dmsm_line->line[dmsm_line->line_cnt].inst_id)),"  ",substring(1,15,cnvtstring(
        dmsm_line->line[dmsm_line->line_cnt].sid)),
      "  ",substring(1,15,cnvtstring(dmsm_line->line[dmsm_line->line_cnt].serial)),"  ",substring(1,
       20,dmsm_line->line[dmsm_line->line_cnt].schemaname),"  ",
      substring(1,30,dmsm_line->line[dmsm_line->line_cnt].osuser),"  ",substring(1,8,dmsm_line->line[
       dmsm_line->line_cnt].status),"  ",substring(1,48,dmsm_line->line[dmsm_line->line_cnt].prg),
      "  ",substring(1,50,dmsm_line->line[dmsm_line->line_cnt].machine))
     IF ((size(dmsm_line->line[dmsm_line->line_cnt].full_line,1) > dmsm_screen->max_col))
      dmsm_screen->max_col = size(trim(dmsm_line->line[dmsm_line->line_cnt].full_line,3),1)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
...
#exit_program
 SET message = nowindow
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmsm_line)
  CALL echorecord(dmsm_screen)
 ENDIF
 FREE RECORD dmsm_screen
 FREE RECORD dmsm_line
 SET dm_err->eproc = "dm2_mig_session_monitor completed"
 CALL final_disp_msg("dm2_mig_sesmon")
END GO
