CREATE PROGRAM dm2_repl_obj_drop
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drod_rpt_ind = i2 WITH protect, noconstant(0)
 DECLARE drod_info_domain = vc WITH protect, constant("DM2_REPLICATE_CLEANUP_OBJDROP")
 DECLARE drod_obj_cnt = i4 WITH protect, noconstant(0)
 DECLARE drod_obj_exists = i2 WITH protect, noconstant(0)
 DECLARE drod_retry_ind = i4 WITH protect, noconstant(1)
 DECLARE drod_retry_cnt = i4 WITH protect, noconstant(0)
 DECLARE drod_retry_limit = i4 WITH protect, noconstant(2400)
 DECLARE drod_verify_obj_exists(dvoe_idx=i4) = i2
 IF (check_logfile("dm2_repl_obj_drop",".log","dm2_repl_obj_drop")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_repl_obj_drop"
 CALL disp_msg(" ",dm_err->logfile,0)
 FREE RECORD drod_cleanup
 RECORD drod_cleanup(
   1 obj_cnt = i4
   1 obj[*]
     2 full_name = vc
 ...
 SET drod_cleanup->obj_cnt = 0
 IF (validate(dm2_repl_obj_drop_skip,- (1))=1)
  SET dm_err->eproc = "Replicate object cleanup has been bypassed"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (validate(dm2_repl_obj_drop_rpt,- (1))=1)
  SET drod_rpt_ind = 1
 ENDIF
 SET dm_err->eproc = "Retrieve operation retry limit from dm_info"
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_REPL_OBJ_DROP"
   AND di.info_name="OPERATION_RETRY_LIMIT"
  DETAIL
   drod_retry_limit = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET drod_retry_limit = 2400
 ENDIF
 SET dm_err->eproc = "Pull in cleanup objects from dm_info"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain=drod_info_domain
   AND d.info_number=1
   AND cnvtupper(d.info_char) IN ("TABLE", "TRIGGER", "PUBLIC SYNONYM")
  ORDER BY d.info_name
  HEAD REPORT
   drod_cleanup->obj_cnt = 0
  DETAIL
   drod_cleanup->obj_cnt = (drod_cleanup->obj_cnt+ 1), stat = alterlist(drod_cleanup->obj,
    drod_cleanup->obj_cnt), drod_cleanup->obj[drod_cleanup->obj_cnt].full_name = trim(cnvtupper(d
     .info_name))
   IF (findstring(".",d.info_name,1,0) > 0)
    drod_cleanup->obj[drod_cleanup->obj_cnt].owner_name = cnvtupper(substring(1,(findstring(".",d
       .info_name,1,0) - 1),d.info_name)), drod_cleanup->obj[drod_cleanup->obj_cnt].obj_name =
    cnvtupper(substring((findstring(".",d.info_name,1,0)+ 1),(textlen(d.info_name) - findstring(".",d
       .info_name,1,0)),d.info_name))
   ELSE
    drod_cleanup->obj[drod_cleanup->obj_cnt].owner_name = "*", drod_cleanup->obj[drod_cleanup->
    obj_cnt].obj_name = trim(cnvtupper(d.info_name))
   ENDIF
   drod_cleanup->obj[drod_cleanup->obj_cnt].obj_type = d.info_char, drod_cleanup->obj[drod_cleanup->
   obj_cnt].active_ind = d.info_number, drod_cleanup->obj[drod_cleanup->obj_cnt].exists_ind = 0,
   drod_cleanup->obj[drod_cleanup->obj_cnt].command_str = "DM2_NOT_SET"
  WITH nocounter
 ;end select
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(drod_cleanup)
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "No objects requiring cleanup were reported from dm_info. Exiting Process"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Processing drod_cleanup objects"
 CALL disp_msg(" ",dm_err->logfile,0)
 FOR (drod_obj_cnt = 1 TO value(drod_cleanup->obj_cnt))
   IF ((drod_cleanup->obj[drod_obj_cnt].active_ind=1))
    IF (drod_verify_obj_exists(drod_obj_cnt)=0)
     GO TO exit_program
    ENDIF
   ENDIF
 ENDFOR
 ...
 ENDIF
 IF (drod_rpt_ind=1)
  SET dm_err->eproc = "Displaying Replicate Cleanup Report."
  SELECT INTO "mine"
   FROM (dummyt d  WITH seq = value(drod_cleanup->obj_cnt))
   WHERE (drod_cleanup->obj[d.seq].exists_ind=1)
    AND (drod_cleanup->obj[d.seq].active_ind=1)
   ORDER BY drod_cleanup->obj[d.seq].obj_type
   HEAD REPORT
    col 0, "Replicate Object Cleanup Report", row + 2,
    col 0, "| Object Type", col 25,
    "| Owner Name", col 45, "| Object Name",
    col 70, "| Drop Command", row + 1,
    col 0,
    CALL print(fillstring(125,"-")), row + 1
   DETAIL
    col 0,
    CALL print(concat("| ",drod_cleanup->obj[d.seq].obj_type)), col 25,
    CALL print(concat("| ",drod_cleanup->obj[d.seq].owner_name)), col 45,
    CALL print(concat("| ",drod_cleanup->obj[d.seq].obj_name)),
    col 70,
    CALL print(concat("| ",drod_cleanup->obj[d.seq].command_str)), row + 1
 ...
 ELSE
  FOR (drod_obj_cnt = 1 TO value(drod_cleanup->obj_cnt))
    IF ((drod_cleanup->obj[drod_obj_cnt].active_ind=1)
     AND (drod_cleanup->obj[drod_obj_cnt].exists_ind=1))
     IF ((drod_cleanup->obj[drod_obj_cnt].command_str != "DM2_NOT_SET"))
      SET dm_err->eproc = concat("Dropping object: ",drod_cleanup->obj[drod_obj_cnt].full_name)
      CALL disp_msg(" ",dm_err->logfile,0)
      SET drod_retry_ind = 1
      WHILE (drod_retry_ind=1
       AND drod_retry_cnt < drod_retry_limit)
        SET dm_err->err_ind = 0
        SET drod_retry_ind = 0
        CALL dm2_push_cmd(drod_cleanup->obj[drod_obj_cnt].command_str,1)
        IF ((dm_err->err_ind=1))
         ...
         ELSE
          SET drod_retry_ind = 1
          SET drod_retry_cnt = (drod_retry_cnt+ 1)
         ENDIF
        ENDIF
      ENDWHILE
      IF (drod_verify_obj_exists(drod_obj_cnt)=0)
       GO TO exit_program
      ENDIF
      IF ((drod_cleanup->obj[drod_obj_cnt].exists_ind=1))
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat("Unable to drop ",drod_cleanup->obj[drod_obj_cnt].full_name)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
       GO TO exit_program
      ENDIF
     ENDIF
  ...
  ENDFOR
 ENDIF
 SUBROUTINE drod_verify_obj_exists(dvoe_idx)
   DECLARE dvoe_ndx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = concat("Validate existance of object: ",drod_cleanup->obj[dvoe_idx].full_name)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   CASE (drod_cleanup->obj[dvoe_idx].obj_type)
    OF "TABLE":
     SELECT INTO "nl:"
      FROM dba_tables t
      WHERE t.owner=patstring(drod_cleanup->obj[dvoe_idx].owner_name)
       AND (t.table_name=drod_cleanup->obj[dvoe_idx].obj_name)
      HEAD REPORT
       drod_cleanup->obj[dvoe_idx].owner_name = trim(t.owner), drod_cleanup->obj[dvoe_idx].full_name
        = build(t.owner,".",t.table_name), drod_cleanup->obj[dvoe_idx].exists_ind = 1,
       drod_cleanup->obj[dvoe_idx].command_str = concat("RDB DROP TABLE ",drod_cleanup->obj[dvoe_idx]
        .full_name," go")
      DETAIL
       IF (locateval(dvoe_ndx,1,drod_cleanup->obj_cnt,t.owner,drod_cleanup->obj[dvoe_ndx].owner_name,
        t.table_name,drod_cleanup->obj[dvoe_ndx].obj_name)=0)
        drod_cleanup->obj_cnt = (drod_cleanup->obj_cnt+ 1), stat = alterlist(drod_cleanup->obj,
         drod_cleanup->obj_cnt), drod_cleanup->obj[drod_cleanup->obj_cnt].full_name = build(t.owner,
         ".",t.table_name),
        drod_cleanup->obj[drod_cleanup->obj_cnt].owner_name = trim(t.owner), drod_cleanup->obj[
        drod_cleanup->obj_cnt].obj_name = trim(t.table_name), drod_cleanup->obj[drod_cleanup->obj_cnt
        ].obj_type = drod_cleanup->obj[dvoe_idx].obj_type,
        drod_cleanup->obj[drod_cleanup->obj_cnt].active_ind = drod_cleanup->obj[dvoe_idx].active_ind,
        drod_cleanup->obj[drod_cleanup->obj_cnt].exists_ind = 1, drod_cleanup->obj[drod_cleanup->
        obj_cnt].command_str = concat("RDB DROP TABLE ",drod_cleanup->obj[drod_cleanup->obj_cnt].
         full_name," go")
       ENDIF
      WITH nocounter
     ;end select
     ...
     ENDIF
     IF (curqual=0)
      SET drod_cleanup->obj[dvoe_idx].exists_ind = 0
     ENDIF
    OF "TRIGGER":
     SELECT INTO "nl:"
      FROM dba_triggers t
      WHERE t.owner=patstring(drod_cleanup->obj[dvoe_idx].owner_name)
       AND (t.trigger_name=drod_cleanup->obj[dvoe_idx].obj_name)
      HEAD REPORT
       drod_cleanup->obj[dvoe_idx].owner_name = trim(t.owner), drod_cleanup->obj[dvoe_idx].full_name
        = build(t.owner,".",t.trigger_name), drod_cleanup->obj[dvoe_idx].exists_ind = 1,
       drod_cleanup->obj[dvoe_idx].command_str = concat("RDB DROP TRIGGER ",drod_cleanup->obj[
        dvoe_idx].full_name," go")
      DETAIL
       IF (locateval(dvoe_ndx,1,drod_cleanup->obj_cnt,t.owner,drod_cleanup->obj[dvoe_ndx].owner_name,
        t.trigger_name,drod_cleanup->obj[dvoe_ndx].obj_name)=0)
        drod_cleanup->obj_cnt = (drod_cleanup->obj_cnt+ 1), stat = alterlist(drod_cleanup->obj,
         drod_cleanup->obj_cnt), drod_cleanup->obj[drod_cleanup->obj_cnt].full_name = build(t.owner,
         ".",t.trigger_name),
        drod_cleanup->obj[drod_cleanup->obj_cnt].owner_name = trim(t.owner), drod_cleanup->obj[
        drod_cleanup->obj_cnt].obj_name = trim(t.trigger_name), drod_cleanup->obj[drod_cleanup->
        obj_cnt].obj_type = drod_cleanup->obj[dvoe_idx].obj_type,
        drod_cleanup->obj[drod_cleanup->obj_cnt].active_ind = drod_cleanup->obj[dvoe_idx].active_ind,
        drod_cleanup->obj[drod_cleanup->obj_cnt].exists_ind = 1, drod_cleanup->obj[drod_cleanup->
        obj_cnt].command_str = concat("RDB DROP TRIGGER ",drod_cleanup->obj[drod_cleanup->obj_cnt].
         full_name," go")
       ENDIF
      WITH nocounter
     ;end select
    ...
    OF "PUBLIC SYNONYM":
     SELECT INTO "nl:"
      ...
      WHERE s.owner="PUBLIC"
       AND (s.synonym_name=drod_cleanup->obj[dvoe_idx].obj_name)
      DETAIL
       drod_cleanup->obj[dvoe_idx].owner_name = trim(s.owner), drod_cleanup->obj[dvoe_idx].exists_ind
        = 1, drod_cleanup->obj[dvoe_idx].command_str = concat("RDB DROP PUBLIC SYNONYM ",drod_cleanup
        ->obj[dvoe_idx].obj_name," go")
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "dm2_repl_obj_drop completed"
 CALL final_disp_msg("dm2_repl_obj_drop")
END GO
