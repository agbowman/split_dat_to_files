CREATE PROGRAM dm2_aload_dm_alpha_features
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_daf",".log","dm2_aload_dm_alpha_features")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dm_alpha_features"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daddaf_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daddaf_file = vc WITH protect, noconstant( $2)
 DECLARE daddaf_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daddaf_batch = i4 WITH protect, noconstant(0)
 DECLARE daddaf_file_loc_ret = vc WITH protect, noconstant("")
 IF (daddaf_expected_rows=0)
  SET dm_err->emsg = "NO ROWS TO PROCESS FOR DM_ALPHA_FEATURES!"
  SET dm_err->eproc = "Evaluate rows to process for DM_ALPHA_FEATURES"
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 IF (dac_prelim(daddaf_pkg,daddaf_file_loc_ret,daddaf_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daddaf_batch=0)
  SET daddaf_batch = 5000
 ENDIF
 EXECUTE dm2_dbimport concat(daddaf_file_loc_ret,daddaf_file), "dm2_aload_adoc_daf", daddaf_batch
 CALL echo(concat(daddaf_file_loc_ret,daddaf_file))
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dm_alpha_features completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_dm_alpha_features")
END GO
