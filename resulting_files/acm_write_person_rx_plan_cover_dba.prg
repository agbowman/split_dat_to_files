CREATE PROGRAM acm_write_person_rx_plan_cover:dba
 ...
 DECLARE ubound = i4 WITH protect, noconstant(reply->p_rx_plan_coverage_qual_cnt)
 SET stat = alterlist(reply->person_rx_plan_coverage_qual,reply->p_rx_plan_coverage_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->person_rx_plan_coverage_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->person_rx_plan_coverage_qual[index].person_rx_plan_coverage_id = acm_request->
     person_rx_plan_coverage_qual[index].person_rx_plan_coverage_id
     SET reply->person_rx_plan_coverage_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_person_rx_plan_coverage
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->act_cnt > 0))
  EXECUTE acm_act_person_rx_plan_coverage
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_person_rx_plan_coverage
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_person_rx_plan_coverage
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
