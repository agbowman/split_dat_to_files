CREATE PROGRAM dm2_flexstats2:dba
 ...
 IF (check_logfile("dm2_flexstats2",".log","DM2_FLEXSTATS2 LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning DM2_FLEXSTATS2."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 SET df_est_pct = greatest(0.0,least(100.0,cnvtreal( $4)))
 SET df_method_opt = trim(cnvtupper( $5),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg =
  "Parameter usage: dm2_flexstats2 '<level>', '<owner>', '<table_name>', <est_pct>, '<method_opt>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating <level> input parameter."
 IF ( NOT (df_level IN ("SYSUSER", "TABLE")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Valid values are 'SYSUSER', 'TABLE'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF (df_est_pct=0.0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Valid values are greater than 0.0 and less than or equal to 100.0"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (df_level="SYSUSER")
  SET dm_err->eproc = concat("Calling DBMS_STATS.GATHER_SCHEMA_STATS for ",df_owner)
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (df_owner="SYS")
   IF ((dm_err->debug_flag >= 4))
    CALL echo("Current owner is 'SYS'")
   ENDIF
   CALL alter_trigger_status("DM2_NOANALYZE","ENABLE","DISABLE")
  ENDIF
  SET dclcom1 = concat(^RDB ASIS(" begin DBMS_STATS.GATHER_SCHEMA_STATS('^,trim(df_owner),
   ^'); end;") go^)
  IF (dm2_push_cmd(dclcom1,1)=0)
   GO TO exit_script
  ENDIF
 ELSE
  SET dm_err->eproc = concat("Calling DBMS_STATS.GATHER_TABLE_STATS for ",df_table_name)
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dclcom1 = concat(^RDB ASIS(" begin DBMS_STATS.GATHER_TABLE_STATS(ownname=>'^,df_owner,
   "' ,tabname=> '",df_table_name,"',",
   "estimate_percent=> ",build(df_est_pct),", method_opt=> '",df_method_opt,"' , ",
   'block_sample=> false, cascade=> true); end;") go')
  IF ((dm_err->debug_flag >= 1))
...
#exit_script
 ...
 ENDIF
 SET dm_err->eproc = "Ending dm2_flexstats2."
 CALL final_disp_msg("dm2_flexstats2")
