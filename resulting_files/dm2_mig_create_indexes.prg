CREATE PROGRAM dm2_mig_create_indexes
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmci_cnt = i4 WITH protect, noconstant(0)
 FREE RECORD dmci_ind_cmds
 RECORD dmci_ind_cmds(
   1 cnt = i4
   1 qual[*]
     2 ind_cmd = vc
     2 ind_name = vc
 ...
 IF (check_logfile("dm2_mig_crtind",".log","dm2_mig_create_indexes")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting Dm2_mig_create_indexes"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ENDIF
 SET dm_err->eproc =
 "Scan for mixed owner indexes and generate associated index creation DDL from SOURCE."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM (
   (
   (SELECT
    create_text = sqlpassthru("dbms_metadata.get_ddl('INDEX',i.index_name,i.owner)"), ind_name = i
    .index_name, ind_owner = i.owner,
    tadmci_owner = i.table_owner, tadmci_name = i.table_name
    FROM dba_indexes i
    WHERE i.table_owner IN (
    (SELECT
     info_name
     FROM dm_info
     WHERE ((info_domain="DM2_MIG_USER") OR (info_domain="DM2_MIG_DT_USER"
      AND info_char="1")) ))
     AND  NOT (concat(i.table_owner,".",i.table_name) IN (
    (SELECT
     x.info_name
     FROM dm_info x
     WHERE x.info_domain="DM2_MIG_EXCL"
      AND x.info_char="0")))
     AND i.table_owner != i.owner
    WITH sqltype("C32000","c30","c30","c30","c30")))
   a)
  DETAIL
   dmci_ind_cmds->cnt = (dmci_ind_cmds->cnt+ 1), stat = alterlist(dmci_ind_cmds->qual,dmci_ind_cmds->
    cnt), dmci_ind_cmds->qual[dmci_ind_cmds->cnt].ind_cmd = replace(a.create_text,
    "COMPUTE STATISTICS"," ",0),
   dmci_ind_cmds->qual[dmci_ind_cmds->cnt].ind_name = a.ind_name, dmci_ind_cmds->qual[dmci_ind_cmds->
   cnt].ind_owner = a.ind_owner, dmci_ind_cmds->qual[dmci_ind_cmds->cnt].tbl_owner = a.tadmci_owner,
   dmci_ind_cmds->qual[dmci_ind_cmds->cnt].tbl_name = a.tadmci_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF ((dm2_rdbms_version->level1 > 10))
  SET dm_err->eproc =
  "Scan for invisible indexes and generate associated index creation DDL from SOURCE."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM (
    (
   ...
   DETAIL
    dmci_ind_cmds->cnt = (dmci_ind_cmds->cnt+ 1), stat = alterlist(dmci_ind_cmds->qual,dmci_ind_cmds
     ...
     "COMPUTE STATISTICS"," ",0),
    dmci_ind_cmds->qual[dmci_ind_cmds->cnt].ind_name = a.ind_name, dmci_ind_cmds->qual[dmci_ind_cmds
    ->cnt].ind_owner = a.ind_owner, dmci_ind_cmds->qual[dmci_ind_cmds->cnt].tbl_owner = a
    .tadmci_owner,
    dmci_ind_cmds->qual[dmci_ind_cmds->cnt].tbl_name = a.tadmci_name
   WITH nocounter
  ;end select
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmci_ind_cmds)
 ENDIF
 IF ((dmci_ind_cmds->cnt=0))
  SET dm_err->eproc =
  "No indexes detected for indexes with owners different than the table owner or invisible."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF (dmr_prompt_connect_data("TARGET","V500","CO")=0)
 ...
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determine which indexes exist in TARGET."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_indexes di,
   (dummyt d  WITH seq = dmci_ind_cmds->cnt)
  PLAN (d
   WHERE d.seq > 0)
   JOIN (di
   WHERE (di.index_name=dmci_ind_cmds->qual[d.seq].ind_name)
    AND (di.owner=dmci_ind_cmds->qual[d.seq].ind_owner)
    AND (di.table_name=dmci_ind_cmds->qual[d.seq].tbl_name)
    AND (di.table_owner=dmci_ind_cmds->qual[d.seq].tbl_owner))
  DETAIL
   dmci_ind_cmds->qual[d.seq].exists_ind = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (locateval(dmci_cnt,1,dmci_ind_cmds->cnt,0,dmci_ind_cmds->qual[dmci_cnt].exists_ind)=0)
  SET dm_err->eproc = "All indexes exist in TARGET, no index creation required."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF ((validate(dmci_report_option,- (1)) != - (1)))
  SET dm_err->eproc = "Generate report listing indexes which will be created."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO mine
   FROM (dummyt d  WITH seq = dmci_ind_cmds->cnt)
   HEAD REPORT
   ...
   DETAIL
    IF ((dmci_ind_cmds->qual[d.seq].exists_ind=0))
     col 2,
     CALL print(concat(dmci_ind_cmds->qual[d.seq].ind_owner,".",dmci_ind_cmds->qual[d.seq].ind_name,
      " on ",dmci_ind_cmds->qual[d.seq].tbl_owner,
      ".",dmci_ind_cmds->qual[d.seq].tbl_name)), row + 1
    ENDIF
  ...
  ENDIF
 ENDIF
 FOR (dmci_cnt = 1 TO dmci_ind_cmds->cnt)
   IF ((dmci_ind_cmds->qual[dmci_cnt].exists_ind=0))
    SET dm_err->eproc = concat("Creating index ",dmci_ind_cmds->qual[dmci_cnt].ind_owner,".",
     dmci_ind_cmds->qual[dmci_cnt].ind_name," on ",
     dmci_ind_cmds->qual[dmci_cnt].tbl_owner,".",dmci_ind_cmds->qual[dmci_cnt].tbl_name)
    CALL disp_msg("",dm_err->logfile,0)
    EXECUTE dm2_exec_child_cmd "CMD", concat("rdb asis(^",dmci_ind_cmds->qual[dmci_cnt].ind_cmd,
     "^) go")
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
...
#exit_script
 ...
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_create_indexes completed"
 ENDIF
 CALL final_disp_msg("dm2_mig_crtind")
END GO
