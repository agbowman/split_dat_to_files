CREATE PROGRAM al_bhs_rpt_admin_audit_all:dba
 PROMPT
  "output:" = "MINE",
 ...
 IF (( $RUNTYPE=2))
  SET work->beg_dt_tm = cnvtdatetime((curdate - 17),060000)
  SET work->end_dt_tm = cnvtdatetime((curdate - 3),143000)
 ELSEIF (( $RUNTYPE=1))
 ...
 SELECT INTO "nl:"
  ...
  HEAD e.encntr_id
   ...
   .fin = format(trim(ea.alias,3),"############;P0"), work->encntrs[work->e_cnt].mrn = format(trim(
     ea1.alias,3),"#######;P0"),
   ...
   m_cnt = 0
  HEAD o.order_id
   m_cnt = (m_cnt+ 1), stat = alterlist(work->encntrs[work->e_cnt].meds,m_cnt), work->encntrs[work->
   e_cnt].m_cnt = m_cnt,
  ...
  HEAD cem.event_id
   eventcnt = (eventcnt+ 1), stat = alterlist(work->encntrs[work->e_cnt].meds[m_cnt].ordevent,
    eventcnt), tdose = 0.0,
   ...
   work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].admin_datetime = format(cnvtdatetime(ce
     .event_end_dt_tm),"YYYYMMDDHHMMSS;;q"), work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt
   ].bag = cem.substance_lot_number, work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].
   eventid = ce.event_id,
  ...
  FOOT  cem.event_id
   ...
   ELSE
    ...
    ENDIF
   ENDIF
   work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].diluent = cnvtstring(cem.diluent_type_cd
    ), work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].eventcd = uar_get_code_display(ce
    .event_cd), work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].dilsyn = cnvtstring(cem
    .synonym_id),
 ...
 ;end select
 IF (( $RUNTYPE > 0))
  SET var_output = "tst_adminreport.dat"
  SET sep = "|"
 ELSE
 ...
 IF (( $RUNTYPE=0))
  ...
  ;end select
 ELSE
  SET msgdttime = format(cnvtdatetime(curdate,curtime3),"YYYYMMDDHHMMSS;;q")
  SET domain =
  ...
  SELECT INTO value(var_output)
   ...
   HEAD parent_event_id
    ...
    cnt = 0
   DETAIL
    cnt = (cnt+ 1), dispenseline = build(dispenseline,char(13),"RXC","|A","|^",
  ...
  IF (gl_bhs_prod_flag=1)
   ...
   SET ipaddress = "10.26.35.47"
   SET folderdir = "/tempfiles/hold/egate/premier"
   SET username = "egprod4"
   SET password = "herewego"
  ELSE
   SET filenamein = var_output
   SET filenameout = var_output
   SET ipaddress = "10.26.35.56"
   SET folderdir = "/tempfiles/hold/egate/premier"
   SET username = "egtest"
   SET password = "egtest"
  ENDIF
