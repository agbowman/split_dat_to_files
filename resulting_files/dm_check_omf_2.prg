CREATE PROGRAM dm_check_omf_2
  ...
  *
  ...
  WHERE me.cover_period_to_fl06 BETWEEN cnvtdatetime(startdatesstring) AND cnvtdatetime(
   enddatesstring)
   AND me.admit_dt_tm_fl17 > cnvtdatetime(curdate,curtime3)
  DETAIL
 ...
 IF (curqual != 0)
  UPDATE  FROM ub92_mon_encounter_error mee,
   ...
   SET mee.error_cd = 00401, mee.status_flg = "R", mee.updt_dt_tm = cnvtdatetime(curdate,curtime3),
  ...
  IF (curqual=0)
   INSERT  FROM ub92_mon_encounter_error mee,
     (dummyt d  WITH seq = value(data->count))
    SET mee.error_cd = 00401, mee.reporting_period = cnvtdatetime(startdatesstring), mee
  ...
  *
  ...
  WHERE mpp.admit_dt_tm_fl17 > cnvtdatetime(curdate,curtime3)
  ...
  *
  ...
  WHERE md.cover_period_to_fl06 BETWEEN cnvtdatetime(startdatesstring) AND cnvtdatetime(
   enddatesstring)
   AND md.admit_dt_tm_fl17 > cnvtdatetime(curdate,curtime3)
  DETAIL
   data->count = (data->count+ 1)
   IF (mod(data->count,10)=1)
    stat = alterlist(data->list,(data->count+ 9))
