CREATE PROGRAM bhs_ma_fullcensus:dba
 PROMPT
 ...
 SELECT INTO value(prt_loc)
  ...
  r.location_cd, p_name = substring(1,30,p.name_full_formatted), p_sex_cd = uar_get_code_display(p
   .sex_cd),
  p_birthdt = format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1),"dd-mmm-yyyy ;;d"),
  fac_mrn = ea2.alias, fin_num = ea.alias,
