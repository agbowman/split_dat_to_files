CREATE PROGRAM dm_rmc_purge_adm_xlats_by_name:dba
 ...
 SUBROUTINE xta_dbl_hlv(i_gap)
   ...
   RETURN(xdh_gap)
 END ;Subroutine
 IF (check_logfile("DM_RMC_PAX_BN",".log","DM_RMC_PAX_BN LogFile")=0)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 ...
 ENDIF
 FREE RECORD pax_bn_request
 RECORD pax_bn_request(
   1 dm_rdds_chain_id = f8
 )
 SET pax_bn_request->dm_rdds_chain_id = get_chain_id(request->chain_name)
 EXECUTE dm_rmc_purge_admin_xlats  WITH replace("REQUEST","PAX_BN_REQUEST")
#end_program
 FREE RECORD pax_bn_request
END GO
