CREATE PROGRAM bbt_rpt_return:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->return_dispensed = uar_i18ngetmessage(i18nhandle,"return_dispensed",
  "RETURN DISPENSED PRODUCT REASONS")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_return", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  PLAN (cv
   WHERE cv.code_set=1615)
   JOIN (cd
   WHERE cd.code_set=outerjoin(1615)
  ...
  HEAD PAGE
   ...
   col 43, captions->return_dispensed, row + 2,
   line = fillstring(122,"-"), line, row + 1,
   col 4, captions->display_name, col 30,
   captions->description, col 75, captions->meaning,
   col 105, captions->active, row + 1,
   line, row + 2
  DETAIL
   col 2, cv.display, col 20,
   cv.description, col 75, cd.display
   IF (cv.active_ind=1)
