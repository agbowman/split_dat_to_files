CREATE PROGRAM dcp_upd_fav_icd9_hli_syn_cc:dba
 ...
 SET ms_child_c_info_domain_nm = "DCP_UPD_FAV_ALL"
 ...
 ELSE
  GO TO exit_program
 ENDIF
 INSERT  FROM temp_fav_icd9_hli_syn fav
  (nomen_cat_list_id, nomenclature_id)(SELECT
   ncl.nomen_cat_list_id, n2.nomenclature_id
   ...
   WHERE n2.cmti=ctm.cmti
    ...
    AND n1.nomenclature_id=ncl.nomenclature_id
    ...
    AND ncl.updt_dt_tm >= cnvtdatetime(dtbegin))
 ;end insert
 IF (sbr_handle_cc_error(null)=0)
  GO TO exit_program
 ENDIF
 UPDATE  FROM nomen_cat_list ncl
  SET ncl.nomenclature_id =
   (SELECT
    fav.nomenclature_id
    FROM temp_fav_icd9_hli_syn fav
    WHERE fav.nomen_cat_list_id=ncl.nomen_cat_list_id), ncl.updt_dt_tm = cnvtdatetime(sysdate), ncl
   .updt_id = reqinfo->updt_id,
   ncl.updt_applctx = reqinfo->updt_applctx, ncl.updt_cnt = (ncl.updt_cnt+ 1)
  WHERE ncl.nomen_cat_list_id IN (
  (SELECT
   fav.nomen_cat_list_id
   FROM temp_fav_icd9_hli_syn fav
   WHERE fav.nomenclature_id BETWEEN mf_min_id AND mf_max_id))
 ;end update
 IF (sbr_handle_cc_error(null)=0)
  ROLLBACK
  SET readme_data->status = "F"
