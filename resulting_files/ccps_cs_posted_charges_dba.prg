CREATE PROGRAM ccps_cs_posted_charges:dba
 PROMPT
 ...
 DECLARE detail_page_title = vc WITH constant("Posted Charge Detail")
 DECLARE foot_detail_text = vc WITH constant("Total Posted Charges with a Total Extended Price of")
 ...
 DECLARE rpt_output_paper_detail = i2 WITH constant(2), protect
 ...
 DECLARE file_name = vc WITH noconstant("cs_posted_charges.csv"), public
 ...
 CALL createldorgparsers(reqinfo->updt_id,"o.logical_domain_id",6,"c.payor_id")
 ...
 ELSE
  SET activity_type_parser = getpromptlist(7,"c.activity_type_cd")
 ENDIF
 IF (ispromptempty(8))
  SET cost_center_parser = "1=1"
 ELSE
  SET cost_center_parser = getpromptlist(8,"c.cost_center_cd")
 ENDIF
 IF (ispromptempty(9))
  SET interface_file_parser = "1=1"
 ELSE
  SET interface_file_parser = getpromptlist(9,"c.interface_file_id+0")
 ENDIF
 IF (ispromptempty(10))
 ...
 ELSE
  SET encntr_type_parser = getpromptlist(11,"e.encntr_type_cd")
 ENDIF
 FREE RECORD charges
 RECORD charges(
   1 c_cnt = i4
 ...
 SELECT INTO "nl:"
  ...
  HEAD c.charge_item_id
   ...
   service_date_time = c.service_dt_tm, charges->c_detail[c_cnt].description = c.charge_description,
   charges->c_detail[c_cnt].charge_type = uar_get_code_display(c.charge_type_cd), charges->c_detail[
   c_cnt].item_quantity = c.item_quantity, charges->c_detail[c_cnt].extended_quantity = pc
   .billing_quantity
   IF (c.charge_type_cd=credit_charge_type_cd
    AND c.item_price > 0.00)
    charges->c_detail[c_cnt].item_price = (c.item_price * - (1.00)), charges->c_detail[c_cnt].
    extended_price = (c.item_extended_price * - (1.00)), charges->c_total_extended_price = (charges->
    c_total_extended_price+ (c.item_extended_price * - (1.00)))
   ELSE
    charges->c_detail[c_cnt].item_price = c.item_price, charges->c_detail[c_cnt].extended_price = c
    .item_extended_price, charges->c_total_extended_price = (charges->c_total_extended_price+ c
    .item_extended_price)
   ENDIF
   ...
   charges->c_detail[c_cnt].organization_id = e.organization_id, charges->c_detail[c_cnt].
   organization_name = o.org_name, charges->c_detail[c_cnt].activity_type_cd = c.activity_type_cd,
   charges->c_detail[c_cnt].activity_type = uar_get_code_display(c.activity_type_cd), charges->
   c_detail[c_cnt].cost_center_cd = c.cost_center_cd, charges->c_detail[c_cnt].cost_center =
   uar_get_code_display(c.cost_center_cd),
   charges->c_detail[c_cnt].interface_file_id = c.interface_file_id, charges->c_detail[c_cnt].
   ...
   c_cnt].encntr_type_cd = e.encntr_type_cd, charges->c_detail[c_cnt].encntr_type =
   uar_get_code_display(e.encntr_type_cd),
   charges->c_detail[c_cnt].posted_date = c.posted_dt_tm
  FOOT REPORT
   stat = alterlist(charges->c_detail,c_cnt), charges->c_cnt = c_cnt
  WITH nocounter, expand = 1
 ;end select
 CALL echo(build2("* * * charges->c_cnt:    ",charges->c_cnt))
 SELECT INTO "nl:"
  FROM charge_mod cm,
   code_value cv
  PLAN (cm
  ...
  DETAIL
   ...
   IF (pos > 0)
    IF (uar_get_code_meaning(cm.field1_id)="CDM_SCHED")
     charges->c_detail[pos].cdm = cm.field6
    ELSEIF (uar_get_code_meaning(cm.field1_id)="CPT4")
     charges->c_detail[pos].cpt = cm.field6
    ELSEIF (uar_get_code_meaning(cm.field1_id)="HCPCS")
     charges->c_detail[pos].qcf = cm.cm1_nbr, charges->c_detail[pos].cpt = cm.field6
    ENDIF
   ENDIF
  WITH nocounter, expand = 1
 ;end select
 ...
 ELSE
  ...
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
    display_message = "There are no posted charges for the selected prompts.", d0 =
 ...
 ENDIF
#exit_prg
 CALL echo(build2("* * * ",display_message))
 CALL echo(build2("* * * inside exit_prg * * * "))
 IF (ld_failure_flag=1)
  ...
  SET d0 = finalizereport( $OUTDEV)
 ENDIF
 SET last_mod = "008 10/22/2012 ML011047"
END GO
