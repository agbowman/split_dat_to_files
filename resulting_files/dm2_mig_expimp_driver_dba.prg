CREATE PROGRAM dm2_mig_expimp_driver:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD dmed_user_list
 RECORD dmed_user_list(
   1 user_cnt = i4
   1 qual[*]
     2 user = vc
 )
 DECLARE dmed_user = i2 WITH protect, noconstant(0)
 DECLARE dmed_v500_exist = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_eidrv",".log","dm2_mig_expimp_driver")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM2_MIG_EXPIMP_DRIVER"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Remove Exp/Imp Driver status row."
 DELETE  FROM dm_info d
  WHERE d.info_domain="DM2_MIGRATION"
   AND d.info_name=evaluate(dmr_expimp->mode,"UPTIME","DM2_MIG_UT_EXPIMP_DRIVER_STATUS",
   "DM2_MIG_DT_EXPIMP_DRIVER_STATUS")
  WITH nocounter
 ;end delete
 COMMIT
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Verify capture setup has been completed."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_MIG_GG_CAP_SETUP"
   AND d.info_name="GG_CAPTURE_PROCESS_CREATED"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->emsg = "Capture setup has not been completed."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dmr_verify_v500_user(dmed_v500_exist)=0)
  GO TO exit_script
 ENDIF
 IF (dmed_v500_exist=1)
  SET stat = alterlist(dmr_expimp->users,0)
  SET dm_err->eproc = "Obtain user list"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE ((d.info_domain="DM2_MIG_DT_USER"
    AND d.info_char="1") OR (d.info_domain="DM2_MIG_USER"))
   DETAIL
    dmr_expimp->expimp_user_cnt = (dmr_expimp->expimp_user_cnt+ 1), stat = alterlist(dmr_expimp->
     users,dmr_expimp->expimp_user_cnt), dmr_expimp->users[dmr_expimp->expimp_user_cnt].expimp_user
     = d.info_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
 ...
 ELSE
  ...
  GO TO exit_script
 ENDIF
 FOR (dmed_user = 1 TO dmr_expimp->expimp_user_cnt)
   SET dm_err->eproc = concat("Calling DM2_MIG_EXPIMP for user ",trim(cnvtlower(dmr_expimp->users[
      dmed_user].expimp_user)))
   CALL disp_msg(" ",dm_err->logfile,0)
   EXECUTE dm2_mig_expimp dmr_expimp->users[dmed_user].expimp_user
   IF ((dm_err->err_ind=1))
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
 SET message = nowindow
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  CALL dmr_prompt_connect_data("SOURCE","V500","CO")
 ENDIF
 SET dm_err->eproc = "Remove Exp/Imp Driver status row."
 DELETE  FROM dm_info d
  WHERE d.info_domain="DM2_MIGRATION"
 ...
 IF ((dm_err->err_ind=0))
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Insert Exp/Imp Driver status row."
 INSERT  FROM dm_info d
  SET d.info_domain = "DM2_MIGRATION", d.info_name = evaluate(dmr_expimp->mode,"UPTIME",
    "DM2_MIG_UT_EXPIMP_DRIVER_STATUS","DM2_MIG_DT_EXPIMP_DRIVER_STATUS"), d.info_char = dm_err->emsg,
   d.info_number = dm_err->err_ind
  WITH nocounter
 ;end insert
 COMMIT
 IF ((dm_err->err_ind=0))
 ...
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM2_MIG_EXPIMP_DRIVER completes successfully"
 ENDIF
 CALL final_disp_msg("dm2_mig_eidrv")
END GO
