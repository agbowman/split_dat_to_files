CREATE PROGRAM dm2_setup_ref_data_mover:dba
 ...
 SUBROUTINE eos_req_add_object(oname,otype,onr,rlen,radd)
   SET eos_request->obj_cnt = (eos_request->obj_cnt+ 1)
 ...
 WHILE (dsrdm_link_con=0)
   ...
   SET dsrdm_username = " "
   SET dsrdm_pwd = " "
   CALL text(3,44,"***  SETUP REFERENCE DATA MOVER  ***")
   CALL text(4,24,concat("Environment Name: ",cur_env_name))
   CALL text(4,70,build("Environment ID: ",cur_env_id))
   WHILE (dsrdm_con=0)
     CALL text(6,3,"Please input SOURCE ENVIRONMENT ID:")
   ...
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   SET dsrdm_db_link = build("MERGE",src_env_id,cur_env_id)
   SET dsrdm_confirm = build("MERGESETUP",src_env_id,cur_env_id)
   SELECT INTO "nl:"
   ...
   IF (dsrdm_size != 0)
    ...
    ENDIF
   ENDIF
   CALL text(7,3,"Input user name: ")
   CALL accept(7,40,"P(30);CU",dsrdm_username)
   SET dsrdm_username = curaccept
   SET accept = nopatcheck
   CALL text(8,3,"Input password: ")
   CALL accept(8,40,"P(30);CEH",dsrdm_pwd)
   SET dsrdm_pwd = curaccept
   SET accept = patcheck
   IF (currdb="ORACLE")
    CALL text(9,3,"Input tnsname for this environment: ")
    CALL accept(9,40,"P(30);CU",dsrdm_tnsname)
    SET dsrdm_tnsname = curaccept
    CALL text(12,3,"Creating database link... ")
    EXECUTE dm2_make_merge_dblink dsrdm_db_link, dsrdm_tnsname, dsrdm_username,
    dsrdm_pwd, " "
   ELSEIF (currdb="DB2UDB")
    CALL text(9,3,"Input remote database name: ")
    CALL accept(9,40,"P(20);CU")
    SET dsrdm_dbase = curaccept
    CALL text(10,3,"Input remote node name: ")
    CALL accept(10,40,"P(20);CU")
    SET dsrdm_node = curaccept
    CALL text(12,3,"Creating server... ")
   ...
   IF (check_error(concat("ERROR: Create database link")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    CALL clear(11,1)
    CALL text(12,3,"Could not create database link/server for the reason")
    CALL text(13,3,dm_err->emsg)
    CALL text(15,3,"Retry or Quit (R/Q)")
    ...
    IF (currdb="ORACLE")
     UPDATE  FROM dm_env_reltn
      ...
      WHERE parent_env_id=src_env_id
       ...
       AND relationship_type="REFERENCE MERGE"
      WITH nocounter
     ;end update
     COMMIT
     CALL text(13,3,concat("Database link ",dsrdm_db_link," was created"))
    ELSE
     UPDATE  FROM dm_env_reltn
     ...
     ;end update
     COMMIT
     CALL text(13,3,concat("Server ",dsrdm_db_link," was created"))
    ENDIF
    SET dsrdm_link_con = 1
    SET link_fail_ind = 0
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Executing dm2_rdds_setup_mock_env_id"
 EXECUTE dm2_rdds_setup_mock_env_id src_env_id
 IF ((dm_err->err_ind > 0))
  GO TO exit_program
 ENDIF
 SET message = nowindow
 SET dm_err->eproc = "Executing dm2_rdds_val_reltn"
 EXECUTE dm2_rdds_val_reltn src_env_id
 IF ((dm_err->err_ind > 0))
  GO TO exit_program
 ENDIF
 SET stat = alterlist(auto_ver_request->qual,1)
 SET auto_ver_request->qual[1].rdds_event = "Creating DB Link"
 SET auto_ver_request->qual[1].cur_environment_id = cur_env_id
 SET auto_ver_request->qual[1].paired_environment_id = src_env_id
 ...
 SUBROUTINE dsdrm_count(null)
   SELECT INTO "nl:"
    t = count(*)
    FROM (value(dsrdm_src_tbl) s),
     dm_merge_translate t
    PLAN (s
     WHERE s.env_source_id=0
      AND s.env_target_id=0)
     JOIN (t
     WHERE t.from_value=s.from_value
      AND t.table_name=s.table_name
      AND t.env_source_id=0
      AND t.env_target_id=0
      AND  NOT ( EXISTS (
     (SELECT
      "X"
      FROM dm_merge_translate d
      WHERE d.from_value=t.from_value
       AND d.table_name=t.table_name
       AND d.env_source_id=src_env_id
       AND d.env_target_id=cur_env_id))))
    DETAIL
     dmt_total_row = t
    WITH nocounter
   ;end select
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "...Ending dm2_setup_ref_data_mover"
 CALL final_disp_msg("dm2_setup_ref_data_mover")
END GO
