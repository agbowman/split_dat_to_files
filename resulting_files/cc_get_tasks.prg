CREATE PROGRAM cc_get_tasks
 ...
 SUBROUTINE (validatesrvreply(nsrvstat=i4,recorddata=vc(ref),zeroforceexit=i2) =i4 WITH protect)
   ...
   ENDIF
 END ;Subroutine
 DECLARE request_number = i4 WITH protect, constant(3208635)
 DECLARE srv_request = vc WITH protect, constant("QueryTasks")
 ...
 SUBROUTINE (getdata(hreply=i4(ref)) =null WITH protect)
   ...
   ELSE
    IF (sstatus="Z")
     SET reply_data->status_data.status = "Z"
    ENDIF
   ENDIF
   CALL exit_servicerequest(happ,htask,hstep)
   EXECUTE eks_put_source  WITH replace(request,putrequest), replace(reply,putreply)
...
#exit_script
 IF ((reply_data->status_data.status != "S"))
  ...
  SET data->status = reply_data->status_data.status
