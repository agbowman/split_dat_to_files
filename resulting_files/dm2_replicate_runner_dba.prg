CREATE PROGRAM dm2_replicate_runner:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drr_dbase_name = vc WITH protect, noconstant("")
 DECLARE drr_src_dbase_name = vc WITH protect, noconstant("")
 DECLARE drr_from_replicate = i4 WITH protect, noconstant(1)
 IF (check_logfile("dm2_replicate_runner",".log","dm2_replicate_runner")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting DM2_REPLICATE_RUNNER"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((((validate(dmr_from_mig_replicate,- (1))=- (1))) OR (validate(dmr_from_mig_replicate,- (1))=0
 )) )
  SET drr_dbase_name = currdbname
  ...
  ENDIF
  SET drr_src_dbase_name = currdbname
  IF (drr_src_dbase_name=drr_dbase_name
   AND (validate(dm2_allow_same_db_name,- (1))=- (1)))
   SET dm_err->emsg = concat("Source:",drr_src_dbase_name," and Target:",drr_dbase_name,
    " databases cannot be the same.")
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   GO TO exit_program
  ELSE
   SET dm_err->eproc = concat("Source:",drr_src_dbase_name," and Target:",drr_dbase_name)
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SET dm2_install_schema->dbase_name = drr_dbase_name
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "Ending DM2_REPLICATE_RUNNER"
 CALL final_disp_msg("dm2_replicate_runner")
END GO
