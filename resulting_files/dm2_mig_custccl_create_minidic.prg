CREATE PROGRAM dm2_mig_custccl_create_minidic
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmcccm_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_createdic")
 DECLARE dmcccm_type = vc WITH protect, noconstant("")
 DECLARE dmcccm_cont = i2 WITH protect, noconstant(1)
 DECLARE dmcccm_env = vc WITH protect, noconstant("")
 DECLARE dmcccm_node = vc WITH protect, noconstant("")
 DECLARE dmcccm_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmcccm_os_type = vc WITH protect, noconstant("")
 DECLARE dmcccm_master_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmcccm_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmcccm_loc = vc WITH protect, noconstant("")
 DECLARE dmcccm_file = vc WITH protect, noconstant("")
 DECLARE dmcccm_cmd = vc WITH protect, noconstant("")
 DECLARE dmcccm_tgt_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmcccm_tgt_os = vc WITH protect, noconstant("")
 DECLARE dmcccm_tgt_env = vc WITH protect, noconstant("")
 DECLARE dmcccm_tgt_list = vc WITH protect, noconstant("")
 DECLARE dmcccm_datetime = vc WITH protect, noconstant("")
 DECLARE dmcccm_ok = i2 WITH protect, noconstant(0)
 DECLARE dmcccm_str = vc WITH protect, noconstant("")
 FREE RECORD dmcccm_cust_scripts
 RECORD dmcccm_cust_scripts(
   1 cnt = i4
   1 qual[*]
     2 script_name = vc
     2 group_nbr = i4
 ...
 DECLARE dmcccm_get_cust_scripts(dgcs_id=f8) = i2
 SET width = 132
 IF (check_logfile(dmcccm_logfile_prefix,".log","dm2_mig_custccl_create_minidic")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_create_minidic"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmcccm_env = trim(logical("environment"))
 SET dmcccm_node = trim(curnode)
 SET dmcccm_os_type = trim(cursys2)
 SET dm_err->eproc = concat("Obtaining environment id for current environment: ",dmcccm_env)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE de.environment_name=cnvtupper(dmcccm_env)
  DETAIL
   dmcccm_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating execution from MASTER"
 SELECT INTO "nl:"
  FROM dma_env_node_attr dena,
   dma_env_node dne
  WHERE dena.dma_env_node_id=dne.dma_env_node_id
   AND dena.attr_name="CCMIG_SETUPTYPE"
   AND dena.attr_value_txt="MASTER"
  DETAIL
   IF (dne.node_name=dmcccm_node
    AND dne.environment_id=dmcccm_env_id)
    dmcccm_master_id = dne.dma_env_node_id, dmcccm_cont = 1
   ELSE
    dmcccm_cont = 0
   ENDIF
  WITH nocounter
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dmcccm_env))
  CALL echo(concat("Current Environment Id: ",cnvtstring(dmcccm_env_id)))
  CALL echo(concat("Current Node: ",dmcccm_node))
  CALL echo(concat("Current OS: ",dmcccm_os_type))
  CALL echo(concat("Master node_env Id: ",cnvtstring(dmcccm_master_id)))
 ENDIF
 IF (((dmcccm_cont=0) OR (dmcccm_master_id=0)) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "This script is not being run from the MASTER"
  SET dm_err->user_action = "Re-run this script from the MASTER node"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating existence of ccldic_vms_unix_pre_merge"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 IF (checkprg("CCLDIC_VMS_UNIX_PRE_MERGE")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "The ccldic_vms_unix_pre_merge script does not exist"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dmcccm_loc = trim(logical("ccluserdir"))
 SET dmcccm_file = evaluate(cursys2,"AXP","ccldictmp2*.dat","/ccldictmp2*.dat")
 IF (dm2_findfile(build(dmcccm_loc,dmcccm_file)) > 0)
  IF ((dm2_sys_misc->cur_os="AXP"))
   SET dmcccm_cmd = concat("del ",dmcccm_file,";*")
  ELSE
   SET dmcccm_cmd = concat("rm ",dmcccm_file)
  ENDIF
  IF (dm2_push_dcl(dmcccm_cmd)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dmcccm_file = concat(dmcccm_loc,evaluate(cursys2,"AXP","dm2_mig_custccl_infile*",
   "/dm2_mig_custccl_infile*"))
 SET dmcccm_file = "dm2_mig_custccl_infile"
 IF (dm2_findfile(build(dmcccm_loc,dmcccm_file)) > 0)
  IF ((dm2_sys_misc->cur_os="AXP"))
   SET dmcccm_cmd = concat("del ",dmcccm_file,";*")
  ELSE
  ...
  ENDIF
 ENDIF
 IF (dmcccm_get_cust_scripts(dmcccm_master_id)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Generating dm2_mig_custccl_infile into $ccluserdir"
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "ccluserdir:dm2_mig_custccl_infile"
  FROM dummyt d
  DETAIL
   FOR (dmcccm_cnt = 1 TO dmcccm_cust_scripts->cnt)
     dmcccm_str = concat(dmcccm_cust_scripts->qual[dmcccm_cnt].object_type,",",format(cnvtupper(
        dmcccm_cust_scripts->qual[dmcccm_cnt].script_name),"##############################;LP ;T"),
      ",",format(dmcccm_cust_scripts->qual[dmcccm_cnt].group_nbr,"##;RP0;I")), col 0, dmcccm_str,
     row + 1
   ENDFOR
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Get <tgt_op_system> for one of the TARGET nodes from dma_env_node_attr"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  ...
  WHERE dena.attr_name="CCMIG_SETUPTYPE"
   AND dena.attr_value_txt="TARGET"
  DETAIL
   dmcccm_tgt_id = dena.dma_env_node_id
 ...
 IF (dmcccm_tgt_id=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No TARGET information found"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSE
  SELECT INTO "nl:"
   FROM dma_env_node_attr dena
   WHERE dena.attr_name IN ("CCMIG_OS_TYPE", "CCMIG_ENV_NAME")
    AND dena.dma_env_node_id=dmcccm_tgt_id
   DETAIL
    IF (dena.attr_name="CCMIG_OS_TYPE")
     dmcccm_tgt_os = dena.attr_value_txt
    ELSE
     dmcccm_tgt_env = dena.attr_value_txt
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_program
  ENDIF
  IF (((dmcccm_tgt_os="") OR ( NOT (dmcccm_tgt_os IN ("AIX", "HPX", "LNX")))) )
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "No valid TARGET OS information found"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Executing ccldic_vms_unix_pre_merge"
 CALL disp_msg("",dm_err->logfile,0)
 EXECUTE ccldic_vms_unix_pre_merge "N", dmcccm_tgt_os, "ccluserdir:dm2_mig_custccl_infile"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dmcccm_datetime = build(year(curdate),month(curdate),day(curdate),hour(curtime),minute(curtime))
 SET dmcccm_loc = trim(logical("ccluserdir"))
 SET dmcccm_file = concat("ccldictmp2",dmcccm_datetime,".dat")
 IF (dm2_findfile(build(dmcccm_loc,"ccldictmp2.dat")) > 0)
  IF ((dm2_sys_misc->cur_os="AXP"))
   SET dmcccm_cmd = concat("rename ",dmcccm_loc,"ccldictmp2.dat"," ",dmcccm_loc,
    dmcccm_file)
  ELSE
   SET dmcccm_cmd = concat("mv ",dmcccm_loc,"ccldictmp2.dat"," ",dmcccm_loc,
    dmcccm_file)
  ENDIF
  IF (dm2_push_dcl(dmcccm_cmd)=0)
   GO TO exit_script
 ...
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "The ccldictmp2.dat file was not found in CCLUSERDIR"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc =
 "Insert a row into dma_env_node_attr admin table for flat ccl binary minidic file (if row does not exist)"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dma_env_node_attr dena
  WHERE dena.dma_env_node_id=dmcccm_master_id
   AND dena.attr_name="CCMIGFILENAME"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inserting row for flat ccl binary minidic file"
  INSERT  FROM dma_env_node_attr dena
   SET dena.dma_attr_id = seq(dm_seq,nextval), dena.dma_env_node_id = dmcccm_master_id, dena
    .attr_name = "CCMIGFILENAME",
    dena.attr_value_txt = dmcccm_file
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  COMMIT
 ELSE
  SET dm_err->eproc = "Updating row for flat ccl binary minidic file"
  UPDATE  FROM dma_env_node_attr dena
   SET dena.attr_value_txt = dmcccm_file
   WHERE dena.dma_env_node_id=dmcccm_master_id
    AND dena.attr_name="CCMIGFILENAME"
   WITH nocounter
  ;end update
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Getting list of TARGET Nodes from dma_env_node"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  ...
  DETAIL
   dmcccm_tgt_list = concat(trim(cnvtupper(dne.node_name)),", ",dmcccm_tgt_list)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 CALL clear(1,1)
 CALL box(1,1,24,131)
 CALL text(1,2,"Custom CCL Migration")
 CALL text(1,70,"DATE/TIME: ")
 CALL text(1,80,build(month(curdate),"/",day(curdate),"/",year(curdate),
   " ",hour(curtime),":",minute(curtime)))
 CALL text(3,2,"A Custom CCL Script mini-ccl dictionary file has been created")
 CALL text(5,2,"File Location: CCLUSERDIR")
 CALL text(6,2,concat("File name: ",dmcccm_file))
 CALL text(8,2,concat("Target Environment:",dmcccm_tgt_env))
 CALL text(9,2,concat("Target Nodes: ",dmcccm_tgt_list))
 CALL text(11,2,
  "Perform the following steps for each Target environment /node to merge the Custom CCL scripts")
 CALL text(13,2,concat("1) Open a FTP session on ",dmcccm_node,
   " connected to $cer_temp on the target node"))
 CALL text(14,2,concat("2) FTP (in binary) the file ",dmcccm_file," to target as follows"))
 CALL text(15,10,concat("ftp> put ",dmcccm_file))
 CALL text(16,2,"3) Execute the following in CCL")
 CALL text(17,10,"ccl> dm2_mig_custccl_merge_minidic go")
 CALL text(19,2,
  "When processing on all Target nodes has completed successfully, continue from this menu")
 CALL text(20,2,"(C)ontinue, (Q)uit: ")
 CALL accept(20,22,"A;cu"," "
  WHERE curaccept IN ("C", "Q"))
 SET message = nowindow
 IF (curaccept="Q")
  SET dm_err->emsg = "User elected to quit from Custom CCL Migration Mini dic creation"
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Executing dm2_mig_custccl_chk 'MERGE'"
 CALL disp_msg("",dm_err->logfile,0)
 EXECUTE dm2_mig_custccl_chk "MERGE"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 GO TO exit_program
 SUBROUTINE dmcccm_get_cust_scripts(dgcs_id)
   SET dm_err->eproc = "Retrieving list of custom scripts for MASTER from dma_env_node_dprotect."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dma_env_node_dprotect d
    WHERE d.object_type_flag_txt="P"
     AND d.dma_env_node_id=dgcs_id
     AND d.merge_ind=1
    HEAD REPORT
     dmcccm_cust_scripts->cnt = 0, stat = alterlist(dmcccm_cust_scripts->qual,0)
    DETAIL
     dmcccm_cust_scripts->cnt = (dmcccm_cust_scripts->cnt+ 1), stat = alterlist(dmcccm_cust_scripts->
      qual,dmcccm_cust_scripts->cnt), dmcccm_cust_scripts->qual[dmcccm_cust_scripts->cnt].script_name
      = d.object_name,
     dmcccm_cust_scripts->qual[dmcccm_cust_scripts->cnt].object_type = d.object_type_flag_txt,
     dmcccm_cust_scripts->qual[dmcccm_cust_scripts->cnt].user_name = d.user_name, dmcccm_cust_scripts
     ->qual[dmcccm_cust_scripts->cnt].group_nbr = d.group_nbr
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_create_minidic"
 CALL final_disp_msg(dmcccm_logfile_prefix)
END GO
