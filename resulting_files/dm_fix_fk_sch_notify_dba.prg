CREATE PROGRAM dm_fix_fk_sch_notify:dba
 ...
 SET readme_data->message = "Readme Failed: Starting dm_fix_fk_sch_notify script."
 SELECT INTO "nl:"
  FROM dm2_user_tables ut,
   dm_tables_doc dtd
  PLAN (dtd
   WHERE dtd.table_name IN ("SCH_NOTIFY", "PERSON", "PRSNL"))
   JOIN (ut
   WHERE ut.table_name=dtd.table_name)
  DETAIL
   IF (dtd.table_name="SCH_NOTIFY")
    IF (currdb="ORACLE")
     d_name = dtd.full_table_name, tmp_fk_name = "XFK8SCH_NOTIFY*", d_fk_name = "XFK8SCH_NOTIFY"
    ELSEIF (currdb="DB2UDB")
     d_name = dtd.suffixed_table_name, tmp_fk_name = build("XFK8SCH_NOTI",dtd.table_suffix,"*"),
     d_fk_name = build("XFK8SCH_NOTI",dtd.table_suffix)
    ENDIF
 ...
 IF (curqual=0)
  SET d_str = concat("rdb alter table ",d_name," add constraint ",d_fk_name,
   " foreign key (TO_PRSNL_ID) references ",
 ...
 SELECT INTO "nl:"
  ...
  WHERE dcc.child_table=d_name
   AND dcc.child_column="TO_PRSNL_ID"
  DETAIL
