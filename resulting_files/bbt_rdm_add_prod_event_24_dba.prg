...
#start_script
 ...
 SET p_cnt = 0
 SELECT INTO "nl:"
  p.product_id, pe.product_event_id, pe2.product_event_id
  FROM product p,
   product_event pe,
   product_event pe2
  PLAN (p
   WHERE p.modified_product_id > 0.0)
   JOIN (pe
   WHERE pe.product_id=p.modified_product_id
    AND ((pe.event_type_cd+ 0)=modified_cd))
   JOIN (pe2
   WHERE pe2.product_id=outerjoin(p.product_id)
  ...
  HEAD p.product_id
   IF (pe2.product_event_id=0.0)
    ...
    ENDIF
    modification->products[p_cnt].product_id = p.product_id, modification->products[p_cnt].
    event_dt_tm = cnvtdatetime(pe.event_dt_tm), modification->products[p_cnt].event_prsnl_id = pe
    .event_prsnl_id
   ENDIF
  FOOT REPORT
 ...
 IF (p_cnt=0)
  SET failed_ind = 0
  CALL logmsg("No products found needing a modified product event.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
  GO TO exit_script
 ENDIF
 FOR (cnt = 1 TO p_cnt)
   ...
   INSERT  FROM product_event pe
    SET pe.product_event_id = sub_product_event_id, pe.product_id = modification->products[cnt].
     product_id, pe.event_dt_tm = cnvtdatetime(modification->products[cnt].event_dt_tm),
     pe.event_prsnl_id = modification->products[cnt].event_prsnl_id, pe.event_type_cd =
     modified_product_cd, pe.event_status_flag = 0,
     pe.bb_result_id = 0.0, pe.encntr_id = 0.0, pe.person_id = 0.0,
     pe.order_id = 0.0, pe.override_ind = 0, pe.override_reason_cd = 0.0,
     pe.related_product_event_id = 0.0, pe.active_ind = 0, pe.active_status_cd = inactive_status_cd,
     pe.active_status_dt_tm = cnvtdatetime(curdate,curtime3), pe.active_status_prsnl_id = 0.0, pe
     .updt_applctx = 0,
     pe.updt_task = 0, pe.updt_cnt = 0, pe.updt_dt_tm = cnvtdatetime(curdate,curtime3),
     pe.updt_id = 0.0
   ...
   IF (curqual=0)
    SET failed_ind = 1
    ROLLBACK
    SET msg = build("PRODUCT_EVENT table insert FAILED for sub_product_event_id =",
