CREATE PROGRAM dm_ecmb_cdi_doc_entity_reltn:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "CDI_DOC_ENTITY_RELTN"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "dm_ecmb_cdi_doc_entity_reltn"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = frm.cdi_doc_entity_reltn_id, rreclist->from_rec[
 ...
 FOR (v_cust_loopcount = 1 TO v_cust_count1)
   IF ((rreclist->from_rec[v_cust_loopcount].parent_entity_id > 0)
 ...
 SUBROUTINE (upt_from_parent_entity(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM cdi_doc_entity_reltn frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     frm.updt_task = reqinfo->updt_task, frm.updt_dt_tm = cnvtdatetime(sysdate), frm.parent_entity_id
      = s_uf_to_fk_id
    WHERE frm.cdi_doc_entity_reltn_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "CDI_DOC_ENTITY_RELTN"
