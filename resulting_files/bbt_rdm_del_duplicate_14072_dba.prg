...
#start_script
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=14072
    AND cv.active_ind=1)
   JOIN (be
   WHERE be.exception_type_cd=outerjoin(cv.code_value))
  ORDER BY cv.cdf_meaning, cv.code_value
  HEAD REPORT
   cdf_cnt = 0, code_cnt = 0, dup_cnt = 1,
   stat = alterlist(codeset14072->cdfs,5), stat = alterlist(deletelist->codevalues,5)
  HEAD cv.cdf_meaning
   ...
   ENDIF
   codeset14072->cdfs[cdf_cnt].cdf_meaning = cv.cdf_meaning, stat = alterlist(codeset14072->cdfs[
    cdf_cnt].codevalues,5)
  HEAD cv.code_value
   code_cnt = (code_cnt+ 1)
   IF (mod(code_cnt,5)=1
    AND code_cnt != 1)
   ...
   ENDIF
   codeset14072->cdfs[cdf_cnt].codevalues[code_cnt].code_value = cv.code_value, codeset14072->cdfs[
   cdf_cnt].codevalues[code_cnt].active_ind = cv.active_ind, codeset14072->cdfs[cdf_cnt].codevalues[
   code_cnt].delete_ind = 1,
  ...
  FOOT  cv.code_value
   row + 0
  FOOT  cv.cdf_meaning
   stat = alterlist(codeset14072->cdfs[cdf_cnt].codevalues,code_cnt), cnt = 1, foundkeeper = 0
   IF (code_cnt=1)
    codeset14072->cdfs[cdf_cnt].codevalues[code_cnt].delete_ind = 0
   ELSE
    deletelist->delete_ind = 1, cnt = code_cnt
    WHILE (cnt > 0
     AND foundkeeper=0)
     IF ((codeset14072->cdfs[cdf_cnt].codevalues[cnt].bb_excep_ind=1))
      foundkeeper = 1, codeset14072->cdfs[cdf_cnt].keeper_code_value = codeset14072->cdfs[cdf_cnt].
      codevalues[cnt].code_value, codeset14072->cdfs[cdf_cnt].codevalues[cnt].delete_ind = 0
     ENDIF
     ,cnt = (cnt - 1)
    ENDWHILE
    IF (foundkeeper=0)
    ...
    WHILE (cnt <= code_cnt)
      ...
      IF ((codeset14072->cdfs[cdf_cnt].codevalues[cnt].delete_ind=1))
       deletelist->codevalues[dup_cnt].code_value = codeset14072->cdfs[cdf_cnt].codevalues[cnt].
       code_value, deletelist->codevalues[dup_cnt].keeper_code_value = codeset14072->cdfs[cdf_cnt].
       keeper_code_value, dup_cnt = (dup_cnt+ 1)
      ENDIF
      cnt = (cnt+ 1)
    ENDWHILE
   ENDIF
  FOOT REPORT
   stat = alterlist(codeset14072->cdfs,cdf_cnt), stat = alterlist(deletelist->codevalues,dup_cnt)
  WITH nocounter
 ;end select
 IF ((deletelist->delete_ind=1))
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (be
    WHERE (be.exception_type_cd=deletelist->codevalues[d.seq].code_value)
...
#exit_script
 IF ((deletelist->delete_ind=0))
  ...
  CALL logmsg("No duplicate rows on 14072 code_set inactivated.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
 ELSEIF ((deletelist->delete_ind=99))
  ROLLBACK
  CALL logstatus("Error locking rows for update.  Please try again.","F")
  CALL logmsg("Error locking rows for update.  Please try again.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
 ELSE
  COMMIT
