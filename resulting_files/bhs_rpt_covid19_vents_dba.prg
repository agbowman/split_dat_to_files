CREATE PROGRAM bhs_rpt_covid19_vents:dba
 PROMPT
 ...
 DECLARE ms_cs6000_resp_ty_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",6000,
   "RESPIRATORYTHERAPY"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=200
    ...
    AND cv.display_key IN ("SPONTANEOUSTIDALVOLUME", "VENTILATOR", "VENTILATORINSTITUTERTDRIVENWEAN",
   "VENTILATORNEONATE", "VENTILATORPEDI",
   "IPPB", "NEGATIVEINSPIRATORYFORCE", "MINIBALVENTILATEDPATIENTS", "VENTILATORPRESSURECONTROL",
   "VENTILATORPRESSURECONTROLWITHPSV",
   "VENTILATORSIMV", "VENTILATORCPAP", "VENTILATORADULTOSCILLATOR", "VENTILATORPRESSURESUPPORT",
   "VENTILATORASSISTCONTROL",
   "VENTILATORADULTOSCILLATOR", "VENTILATORAPRV", "VENTILATORAPRV", "VENTILATORPRVC",
   "ENDTIDALCO2MONITORRT",
   "INITIATEADULTVENTILATORPROTOCOL", "INITIATEARDSALIVENTILATORPROTOCOL"))
  HEAD REPORT
   pl_cnt = 0
  HEAD cv.code_value
   pl_cnt += 1,
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   WHERE o.orig_order_dt_tm BETWEEN cnvtdatetime(ms_beg_dt_tm) AND cnvtdatetime(ms_end_dt_tm)
    AND o.catalog_type_cd=ms_cs6000_resp_ty_cd
    AND expand(ml_exp,1,size(m_rec->cd,5),o.catalog_cd,m_rec->cd[ml_exp].f_cd)
  ...
  HEAD e.encntr_id
   ...
   m_rec->enc[pl_cnt].s_age = cnvtage(p.birth_dt_tm), m_rec->enc[pl_cnt].s_los = cnvtage(e.reg_dt_tm),
   m_rec->enc[pl_cnt].s_encntr_type = trim(uar_get_code_display(e.encntr_type_cd),3),
   m_rec->enc[pl_cnt].s_encntr_class = trim(uar_get_code_display(e.encntr_class_cd),3), m_rec->enc[
   pl_cnt].s_med_service = trim(uar_get_code_display(e.med_service_cd),3), m_rec->enc[pl_cnt].
   f_facility_cd = e.loc_facility_cd,
   m_rec->enc[pl_cnt].s_facility = trim(uar_get_code_display(e.loc_facility_cd),3), m_rec->enc[pl_cnt
   ].f_nurse_unit_cd = e.loc_nurse_unit_cd, m_rec->enc[pl_cnt].s_nurse_unit = trim(
    uar_get_code_display(e.loc_nurse_unit_cd),3),
   m_rec->enc[pl_cnt].s_reg_dt_tm = trim(format(e.reg_dt_tm,"mm/dd/yyyy hh:mm;;d"),3)
