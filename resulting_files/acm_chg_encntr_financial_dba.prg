CREATE PROGRAM acm_chg_encntr_financial:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->encntr_financial_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   encntr_financial e
  SET e.encntr_financial_id = acm_request->encntr_financial_qual[xref->chg[d.seq].idx].
   encntr_financial_id, e.bill_type_cd = acm_request->encntr_financial_qual[xref->chg[d.seq].idx].
   bill_type_cd, e.contributor_system_cd =
   IF ((acm_request->encntr_financial_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->encntr_financial_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   e.person_id =
   IF ((acm_request->encntr_financial_qual[xref->chg[d.seq].idx].person_id > 0)) acm_request->
    encntr_financial_qual[xref->chg[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->encntr_financial_qual[xref->chg[d.seq].idx].person_idx].
    person_id
   ENDIF
   , e.research_account = acm_request->encntr_financial_qual[xref->chg[d.seq].idx].research_account,
   e.beg_effective_dt_tm = cnvtdatetime(acm_request->encntr_financial_qual[xref->chg[d.seq].idx].
    beg_effective_dt_tm),
   e.end_effective_dt_tm = cnvtdatetime(acm_request->encntr_financial_qual[xref->chg[d.seq].idx].
    end_effective_dt_tm), e.active_ind = acm_request->encntr_financial_qual[xref->chg[d.seq].idx].
   active_ind, e.active_status_cd = acm_request->encntr_financial_qual[xref->chg[d.seq].idx].
   active_status_cd,
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_financial_id=acm_request->encntr_financial_qual[xref->chg[d.seq].idx].
   encntr_financial_id))
  WITH nocounter, status(reply->encntr_financial_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->encntr_financial_qual[xref->chg[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM encntr_financial e
    WHERE expand(t1,f_val,t_val,e.encntr_financial_id,acm_request->encntr_financial_qual[xref->chg[t1
     ].idx].encntr_financial_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,e.encntr_financial_id,acm_request->encntr_financial_qual[xref->
      chg[t1].idx].encntr_financial_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((e.updt_cnt=acm_request->encntr_financial_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->encntr_financial_qual[idx].status = - (1), reply->encntr_financial_qual[idx].
      encntr_financial_id = acm_request->encntr_financial_qual[idx].encntr_financial_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->encntr_financial_qual[idx].chg_str
     IF (findstring("BILL_TYPE_CD,",chg_str)=0)
      acm_request->encntr_financial_qual[idx].bill_type_cd = e.bill_type_cd
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->encntr_financial_qual[idx].contributor_system_cd = e.contributor_system_cd
     ENDIF
     IF (findstring("PERSON_ID,",chg_str)=0)
      acm_request->encntr_financial_qual[idx].person_id = e.person_id
     ENDIF
     IF (findstring("RESEARCH_ACCOUNT,",chg_str)=0)
      acm_request->encntr_financial_qual[idx].research_account = e.research_account
     ENDIF
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_financial_qual[idx].beg_effective_dt_tm = e.beg_effective_dt_tm
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_financial_qual[idx].end_effective_dt_tm = e.end_effective_dt_tm
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->encntr_financial_qual[idx].active_ind = e.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->encntr_financial_qual[idx].active_status_cd = e.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->encntr_financial_qual[xref->chg[index].idx].status=0))
