CREATE PROGRAM bhs_pp_mu_export2:dba
 PROMPT
  "Provider by Pool File Name" = "MINE",
  "Start Date" = "CURDATE",
  "End Date" = "CURDATE"
 ...
 SELECT INTO  $OUTDEV
  FROM (dummyt d  WITH seq = size(m_rec2->msg,5))
  HEAD REPORT
   col 0, "--------------------------------------------------------------------------------", row + 1,
   col 0, " Complete list of providers by pool for messages sent from provider with no NPI", row + 1,
   col 0, "--------------------------------------------------------------------------------", row + 1,
   ms_temp = '"pool_id","pool","phys_name","phys_person_id","phys_NPI"', col 0, ms_temp
  DETAIL
   ms_temp = concat('"',trim(cnvtstring(m_rec2->msg[d.seq].f_pool_id)),'",','"',trim(m_rec2->msg[d
     .seq].s_pool_name),
    '",','"',trim(m_rec2->msg[d.seq].s_provider_name),'",','"',
    trim(cnvtstring(m_rec2->msg[d.seq].f_provider_person_id)),'",','"',trim(m_rec2->msg[d.seq].
     s_provider_npi),'"'), row + 1, col 0,
   ms_temp, row + 1
