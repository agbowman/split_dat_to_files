CREATE PROGRAM bhs_auto_admit_date:dba
 RECORD work(
   1 encntr_id = f8
   1 person_id = f8
   1 age_in_days = i4
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   work->stay_length = trim(cnvtstring(cnvtint(datetimediff(cnvtdatetime(curdate,curtime3),e
       .reg_dt_tm))),3), work->admit_date = format(e.reg_dt_tm,"mm/dd/yyyy;;d")
  WITH nocounter
 ;end select
 ...
 SET reply->text = concat(beg_doc,beg_bold," Admission Date: ",trim(work->admit_date,3),end_bold,
  end_doc)
#exit_script
