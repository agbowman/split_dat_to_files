CREATE PROGRAM dm_ehi_queue_rows:dba
 ...
 SET reply->table_name = "EHI_QUEUE"
 SET reply->rows_between_commit = 50
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 DECLARE v_months_to_keep = i4 WITH protect, noconstant(25)
 ...
 IF (batch_ndx=1)
  ...
  IF (v_errcode2 != 0)
   ...
   SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s2",
    "Failed while finding the minimum primary key ID: %1","s",nullterm(v_errmsg2))
   GO TO exit_script
  ENDIF
 ELSE
 ...
 IF (v_errcode2 != 0)
  ...
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s2",
   "Failed while finding the maximum primary key ID: %1","s",nullterm(v_errmsg2))
  GO TO exit_script
 ENDIF
 SET v_cur_max_id = (v_cur_min_id+ (v_batch_size - 1))
 SET v_date_to_persist = cnvtlookbehind(build(v_months_to_keep,",M"),cnvtdatetime(sysdate))
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    eq.rowid, eq.ehi_queue_id
    FROM ehi_queue eq
    WHERE eq.ehi_queue_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists("eq.extract_status_txt in ('COMPLETE','FAILED')","eq"))
     AND eq.request_dt_tm <= cnvtdatetime(v_date_to_persist)
    DETAIL
     v_rows += 1
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = eq.rowid
    WITH nocounter, maxqual(eq,value(v_rows_left))
   ;end select
   SET v_errcode2 = error(v_errmsg2,1)
   IF (v_errcode2 != 0)
    SET reply->err_code = v_errcode2
