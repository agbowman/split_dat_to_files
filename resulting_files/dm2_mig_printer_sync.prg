CREATE PROGRAM dm2_mig_printer_sync
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_mig_printer_sync",".log","dm2_mig_printer_sync")=0)
  GO TO exit_program
 ENDIF
 SET width = 132
 DECLARE dmps_scroll(ds_rec_str=vc) = vc
 DECLARE dmps_screen_header(dsh_mode=i2) = i2
 DECLARE dmps_driver(dd_leave=i2(ref)) = i2
 DECLARE dmps_work_menu(dwm_screen_return=vc(ref)) = i2
 DECLARE dmps_screen_fill(holder=i2) = i2
 DECLARE dmps_del_rel(ddr_rel_ndx=i4) = i2
 DECLARE dmps_add_rel(null) = i2
 DECLARE dmps_node_help(dnh_node_name=vc(ref)) = i2
 DECLARE dmps_chk_node(dcn_node_name=vc,dcn_node_good=i2(ref),dcn_fail_reason=vc(ref)) = i2
 DECLARE dmps_csv_create(null) = i2
 DECLARE dmps_upd_device(null) = i2
 DECLARE dmps_inform_user(diu_type=vc) = i2
 DECLARE dmps_leave = i2 WITH protect, noconstant(0)
 DECLARE dmps_rs_prefix = vc WITH protect, constant("dmps_screen")
 FREE RECORD dmps_screen
 RECORD dmps_screen(
   1 top_line = i4
   1 bottom_line = i4
   1 cur_line = i4
 ...
 FREE RECORD dmps_node_rel
 RECORD dmps_node_rel(
   1 rel_cnt = i4
   1 qual[*]
     2 src_node = vc
     2 tgt_node = vc
     2 src_node_pattern = vc
     2 tgt_node_pattern = vc
 )
 ...
 ENDIF
 WHILE (dmps_leave=0)
   SET dmps_screen->top_line = 1
   SET dmps_screen->cur_line = 1
   SET dmps_screen->bottom_line = 0
   SET dmps_screen->max_scroll = 14
   IF (dmps_driver(dmps_leave)=0)
    GO TO exit_program
   ENDIF
   IF ((dm_err->err_ind=1))
    GO TO exit_program
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE dmps_add_rel(null)
   DECLARE dar_src_node_name = vc WITH protect, noconstant("")
   DECLARE dar_node_good = i2 WITH protect, noconstant(0)
   DECLARE dar_tgt_node_name = vc WITH protect, noconstant("")
   DECLARE dar_fail_reason = vc WITH protect, noconstant("")
   CALL video(n)
   IF ((dmps_node_rel->rel_cnt >= dmps_screen->max_scroll))
    SET dmps_screen->top_line = ((dmps_node_rel->rel_cnt - mod(dmps_node_rel->rel_cnt,dmps_screen->
     max_scroll))+ 1)
    IF (dmps_screen_fill(1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   CALL clear(display_line,02,130)
   IF (dmps_node_help(dar_src_node_name)=0)
    RETURN(0)
   ENDIF
   CALL clear(24,1,130)
   CALL text(24,3,"Enter the Target Application Node Name.")
   CALL accept(display_line,66,"P(52);c"," "
    WHERE curaccept > " ")
   SET dar_tgt_node_name = cnvtlower(curaccept)
   IF (dmps_chk_node(dar_src_node_name,dar_node_good,dar_fail_reason)=0)
    RETURN(0)
   ENDIF
   IF (dar_node_good=1)
    SET dmps_node_rel->rel_cnt = (dmps_node_rel->rel_cnt+ 1)
    SET stat = alterlist(dmps_node_rel->qual,dmps_node_rel->rel_cnt)
    SET dmps_node_rel->qual[dmps_node_rel->rel_cnt].src_node = dar_src_node_name
    SET dmps_node_rel->qual[dmps_node_rel->rel_cnt].tgt_node = dar_tgt_node_name
    SET dmps_node_rel->qual[dmps_node_rel->rel_cnt].tgt_node_pattern = concat("*",dar_tgt_node_name,
     "*")
    SET dmps_node_rel->qual[dmps_node_rel->rel_cnt].src_node_pattern = concat("*",dar_src_node_name,
     "*")
    SET dmps_screen->cur_line = dmps_node_rel->rel_cnt
   ELSE
    CALL clear(24,01,130)
    CALL text(24,2,concat(dar_src_node_name," is not a valid node for SOURCE. Reason:",
      dar_fail_reason))
    CALL pause(5)
    CALL clear(24,01,130)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmps_node_help(dnh_node_name)
   DECLARE dnh_ndx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Show help list for source application node names"
   SET help = pos(12,20,10,100)
   IF ((dmps_node_rel->rel_cnt > 0))
    SET help =
    SELECT DISTINCT INTO "nl:"
     source_application_node_name______________ = cnvtlower(d.name)
     FROM device d
     WHERE (d.device_type_cd=
     (SELECT
      code_value
      FROM code_value
      WHERE code_set=3000
       AND display_key="NODE"))
      AND  EXISTS (
     (SELECT
      "x"
      FROM device d2
      WHERE (d2.device_type_cd=
      (SELECT
       code_value
       FROM code_value
       WHERE code_set=3000
        AND display_key="PRINTER"))
       AND d2.description=concat(d2.name," on ",d.name)))
      AND  NOT (expand(dnh_ndx,1,dmps_node_rel->rel_cnt,cnvtlower(d.name),dmps_node_rel->qual[dnh_ndx
      ].src_node))
     WITH nocounter
    ;end select
   ELSE
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   CALL accept(display_line,2,"P(50);cuF"," "
    WHERE curaccept > " ")
   SET dnh_node_name = cnvtlower(curaccept)
   SET help = off
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmps_chk_node(dcn_node_name,dcn_node_good,dcn_fail_reason)
   DECLARE dcn_ndx = i4 WITH protect, noconstant(0)
   IF (locateval(dcn_ndx,1,dmps_node_rel->rel_cnt,dcn_node_name,dmps_node_rel->qual[dcn_ndx].src_node
    ) > 0)
    SET dcn_node_good = 0
    SET dcn_fail_reason = "Node has been previously added to relationship."
    RETURN(1)
   ELSE
    SET dcn_node_good = 1
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmps_del_rel(ddr_rel_ndx)
   DECLARE dxp_xfer_cnt = i4 WITH protect, noconstant(0)
   DECLARE dxp_cnt = i4 WITH protect, noconstant(0)
   FREE RECORD dmps_node_rel_xfer
   RECORD dmps_node_rel_xfer(
     1 rel_cnt = i4
     1 qual[*]
       2 src_node = vc
       2 tgt_node = vc
   ...
   SET dmps_node_rel_xfer->rel_cnt = (dmps_node_rel->rel_cnt - 1)
   SET stat = alterlist(dmps_node_rel_xfer->qual,dmps_node_rel_xfer->rel_cnt)
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("ndx removed:",ddr_rel_ndx))
    CALL echo(dmps_node_rel->qual[ddr_rel_ndx].tgt_node)
    CALL echorecord(dmps_node_rel)
   ENDIF
   FOR (dxp_cnt = 1 TO dmps_node_rel->rel_cnt)
     IF (dxp_cnt != ddr_rel_ndx)
      SET dxp_xfer_cnt = (dxp_xfer_cnt+ 1)
      SET dmps_node_rel_xfer->qual[dxp_xfer_cnt].src_node = dmps_node_rel->qual[dxp_cnt].src_node
      SET dmps_node_rel_xfer->qual[dxp_xfer_cnt].tgt_node = dmps_node_rel->qual[dxp_cnt].tgt_node
      SET dmps_node_rel_xfer->qual[dxp_xfer_cnt].tgt_node_pattern = dmps_node_rel->qual[dxp_cnt].
      tgt_node_pattern
      SET dmps_node_rel_xfer->qual[dxp_xfer_cnt].src_node_pattern = dmps_node_rel->qual[dxp_cnt].
      src_node_pattern
     ENDIF
   ENDFOR
   SET stat = alterlist(dmps_node_rel->qual,0)
   SET dmps_node_rel->rel_cnt = dmps_node_rel_xfer->rel_cnt
   SET stat = alterlist(dmps_node_rel->qual,dmps_node_rel->rel_cnt)
   CALL echorecord(dmps_node_rel_xfer)
   FOR (dxp_xfer_cnt = 1 TO dmps_node_rel->rel_cnt)
     SET dmps_node_rel->qual[dxp_xfer_cnt].src_node = dmps_node_rel_xfer->qual[dxp_xfer_cnt].src_node
     SET dmps_node_rel->qual[dxp_xfer_cnt].tgt_node = dmps_node_rel_xfer->qual[dxp_xfer_cnt].tgt_node
     SET dmps_node_rel->qual[dxp_xfer_cnt].tgt_node_pattern = dmps_node_rel_xfer->qual[dxp_xfer_cnt].
     tgt_node_pattern
     SET dmps_node_rel->qual[dxp_xfer_cnt].src_node_pattern = dmps_node_rel_xfer->qual[dxp_xfer_cnt].
     src_node_pattern
   ENDFOR
   SET dmps_screen->cur_line = evaluate(dmps_screen->cur_line,1,1,(dmps_screen->cur_line - 1))
   SET dmps_screen->max_value = dmps_node_rel->rel_cnt
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmps_driver(dd_leave)
   DECLARE dd_screen_return = vc WITH protect, noconstant("")
   DECLARE dd_continue = i2 WITH protect, noconstant(0)
   WHILE (dd_continue=0)
     SET dd_screen_return = " "
     IF (dmps_work_menu(dd_screen_return)=0)
      GO TO exit_program
     ENDIF
     CASE (dd_screen_return)
      OF "QUIT":
       ...
       SET dd_continue = 1
      OF "CONTINUE":
       IF (dmps_inform_user("BACKUP")=0)
        RETURN(0)
       ENDIF
       IF (dmps_screen_header(0)=0)
        RETURN(0)
       ENDIF
       IF (dmps_screen_fill(1)=0)
        RETURN(0)
       ...
       CALL clear(24,1,132)
       CALL text(23,3,
        "Answering 'C' will update the printer configuration in TARGET with the pairings listed above."
        )
       CALL text(24,3,"Enter 'C' to Continue or 'R' to Revise:")
       CALL accept(24,43,"p;cu"," "
        WHERE curaccept IN ("C", "R"))
       IF (curaccept="C")
        IF (dmps_upd_device(null)=0)
         RETURN(0)
        ENDIF
        IF (dmps_csv_create(null)=0)
         RETURN(0)
        ENDIF
        IF (dmps_inform_user("CSV")=0)
         RETURN(0)
        ENDIF
        SET dd_leave = 1
        SET dd_continue = 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmps_screen_header(dsh_mode)
   CALL clear(1,1)
   CALL text(1,1,concat("*** PRINTER NODE RELATIONSHIP MENU ***"))
   IF (dsh_mode=1)
    CALL text(3,1,concat(
      "Please establish SOURCE and TARGET application node relationships for printers using the menu below."
      ))
   ENDIF
   CALL text(6,2,"Source Application Node Name")
   CALL text(6,54,"Target Application Node Name")
   CALL line(7,1,132,horizontal)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmps_work_menu(dwm_screen_return)
   CALL clear(1,1)
   IF (dmps_screen_header(1)=0)
    RETURN(0)
   ENDIF
   SET display_line = 8
   SET dmps_screen->max_value = dmps_node_rel->rel_cnt
   IF ((dmps_screen->cur_line <= 1))
    SET dmps_screen->top_line = 1
    SET dmps_screen->cur_line = 1
    SET dmps_screen->bottom_line = 0
    SET dmps_screen->max_scroll = 14
   ELSE
    SET dmps_screen->top_line = dmps_screen->top_line
    SET dmps_screen->cur_line = dmps_screen->cur_line
    SET dmps_screen->bottom_line = dmps_screen->bottom_line
    SET dmps_screen->max_scroll = 14
   ENDIF
   IF (dmps_screen_fill(1)=0)
    RETURN(0)
   ...
   CASE (dmps_scroll(dmps_rs_prefix))
    OF "Q":
     SET dwm_screen_return = "QUIT"
    OF "R":
     IF ((dmps_node_rel->rel_cnt > 0))
      IF (dmps_del_rel(dmps_screen->cur_line)=0)
       RETURN(0)
      ENDIF
     ENDIF
    OF "A":
     IF (dmps_add_rel(null)=0)
      RETURN(0)
     ENDIF
    OF "C":
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmps_screen_fill(holder)
   DECLARE fds_screen_cnt = i4 WITH protect, noconstant(0)
   DECLARE fds_first = i2 WITH protect, noconstant(0)
   SET display_line = 8
   SET dmps_screen->max_scroll = 14
   IF (((dmps_screen->top_line+ 14) > dmps_screen->max_value))
    SET dmps_screen->bottom_line = dmps_screen->max_value
   ELSE
    SET dmps_screen->bottom_line = (dmps_screen->top_line+ 13)
   ENDIF
   FOR (display_loop = dmps_screen->top_line TO dmps_screen->bottom_line)
     IF ((display_loop=dmps_screen->cur_line))
      CALL video(r)
     ELSE
      CALL video(n)
     ENDIF
     CALL clear(display_line,1,132)
     CALL text(display_line,2,dmps_node_rel->qual[display_loop].src_node)
     CALL text(display_line,54,dmps_node_rel->qual[display_loop].tgt_node)
   ...
   ENDIF
   CALL video(n)
   IF ((dmps_screen->bottom_line=0))
    CALL clear(24,60,72)
   ELSEIF ((dmps_screen->max_value > dmps_screen->bottom_line)
    AND (dmps_screen->top_line=1))
    CALL clear(24,60,72)
    CALL text(24,60,"More available, use <PgDown> key to scroll list")
   ELSEIF ((dmps_screen->max_value > dmps_screen->bottom_line))
    CALL clear(24,60,72)
    CALL text(24,60,"More available, use <PgUp>/<PgDown> keys to scroll list")
   ELSE
    CALL clear(24,60,72)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmps_scroll(ds_rec_str)
   ...
   CALL clear(23,1,132)
   IF (ds_disp_cont=1)
    CALL text(23,2,"(C)ontinue, (Q)uit, (R)emove, (A)dd:")
   ELSE
    CALL text(23,2,"(Q)uit, (R)emove, (A)dd:")
   ENDIF
   WHILE (ds_scroll_accept=" ")
     SET ds_valid_value = 0
     IF (ds_disp_cont=1)
      CALL accept(23,39,"p;cus",""
       WHERE curaccept IN ("C", "Q", "R", "A"))
     ELSE
      CALL accept(23,27,"p;cus",""
       WHERE curaccept IN ("Q", "R", "A"))
     ENDIF
     CASE (curscroll)
      OF 0:
 ...
 END ;Subroutine
 SUBROUTINE dmps_csv_create(null)
   DECLARE dcc_temp_line = vc WITH protect, noconstant("")
   DECLARE dcc_file = vc WITH protect, noconstant("dm2_printer_data.csv")
   SET dm_err->eproc = "Create dm2_printer_data.csv"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(dcc_file)
    FROM device d,
     printer p,
     output_dest o,
     code_value c
    PLAN (d
     ...
     (SELECT
      ...
      WHERE code_set=3000
       AND display_key="PRINTER")))
     JOIN (p
     WHERE d.device_cd=p.device_cd)
     JOIN (o
     WHERE d.device_cd=o.device_cd
      AND d.name=o.name)
     JOIN (c
     WHERE c.code_value=p.printer_type_cd)
    ORDER BY d.device_cd
    HEAD REPORT
     dcc_temp_line = build(
      '"PrinterName","PrinterDescription","RemoteQueueName","PrinterType","OSPrinterType","DeviceName","Model",',
      '"Protocol","Port","Form","Library","ProgramName","LabelPrefix","LocationCode",',
      '"Tray1","Tray2","Tray3","Tray4","Tray5","ChartingNode","ChartingLocationCode"'), col 0,
     dcc_temp_line,
     row + 1
    DETAIL
     dcc_temp_line = build('"',trim(d.name),'","',trim(d.description),'","',
      trim(d.name),'","',trim(c.display_key),'",','"","","","","","","","',
      trim(o.label_program_name),'","',trim(o.label_prefix),'",',d.location_cd,
      ',"","","","","","",""'), col 0, dcc_temp_line,
     row + 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmps_upd_device(null)
   DECLARE dud_cnt = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Convert the device.name values to lower case"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   UPDATE  FROM device
    SET name = cnvtlower(name)
    WHERE (device_type_cd=
    (SELECT
     code_value
     FROM code_value
     WHERE code_set=3000
      AND display_key IN ("PRINTQUEUE", "PRINTER")))
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Convert the output_dest.name values to lower case"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   UPDATE  FROM output_dest
    SET name = cnvtlower(name)
    WHERE (device_cd=
    (SELECT
     device_cd
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Update device.description to replace SOURCE name with TARGET name"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   FOR (dud_cnt = 1 TO dmps_node_rel->rel_cnt)
    UPDATE  FROM device v
     SET v.description = replace(cnvtlower(v.description),dmps_node_rel->qual[dud_cnt].src_node,
       dmps_node_rel->qual[dud_cnt].tgt_node)
     WHERE cnvtlower(v.description)=patstring(value(dmps_node_rel->qual[dud_cnt].src_node_pattern))
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ENDFOR
   SET dm_err->eproc = "Update device.name to replace SOURCE name with TARGET name"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   UPDATE  FROM device v,
     (dummyt d  WITH seq = dmps_node_rel->rel_cnt)
    SET v.name = replace(cnvtlower(v.name),dmps_node_rel->qual[d.seq].src_node,dmps_node_rel->qual[d
      .seq].tgt_node)
    PLAN (d
     WHERE d.seq > 0)
     JOIN (v
     WHERE (cnvtlower(v.name)=dmps_node_rel->qual[d.seq].src_node)
      AND (v.device_type_cd=
     (SELECT
      code_value
      FROM code_value
      WHERE code_set=3000
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmps_inform_user(diu_type)
   SET dm_err->eproc = "Inform user of dm2_printer_data.csv"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   CALL clear(1,1)
   CALL box(1,1,20,132)
   IF (diu_type="CSV")
    CALL text(4,3,"Utilizing Printer Import Utility")
    CALL text(6,3,
     "From this point forward follow the Printer Import Utility process.                                             "
     )
    CALL text(7,3,
     "Use csv file dm2_printer_data.csv located in CCLUSERDIR instead of the blank template                          "
     )
    CALL text(8,3,
     "provided in the Printer Import Utility process.   The dm2_printer_data.csv file created has been pre-populated "
     )
    CALL text(9,3,concat("with some of the values. Review and populate the ",
      "missing data before using it in the Printer Import Utility process."))
    CALL text(16,3,"Press <enter> to exit")
   ELSE
    CALL text(4,3,"If you would like to backup existing printer configuration, ")
    CALL text(5,3,
     "please do so now by exporting the following tables from TARGET to a backup location:")
    CALL text(8,10,"tables=DEVICE,DEVICE_XREF,PRINTER,OUTPUT_DEST")
    CALL text(16,3,"Press <enter> to continue")
   ENDIF
   CALL accept(24,1,"p;cu"," "
    WHERE curaccept IN (" "))
   RETURN(1)
 END ;Subroutine
#exit_program
 SET message = nowindow
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmps_screen)
  CALL echorecord(dmps_node_rel)
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_printer_sync has completed successfully"
 ENDIF
 CALL final_disp_msg("dm2_mig_printer_sync")
