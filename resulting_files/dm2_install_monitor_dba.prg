CREATE PROGRAM dm2_install_monitor:dba
 ...
 SUBROUTINE dipma_check_db_arch_mode(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dim_plan_id = f8 WITH protect, noconstant(0.0)
 SET dim_plan_id =  $1
 DECLARE dim_mode = vc WITH protect, noconstant("")
 SET dim_mode = cnvtupper(trim( $2))
 DECLARE dim_process_status = i2 WITH protect, noconstant(0)
 DECLARE dim_planmon_status = i2 WITH protect, noconstant(0)
 DECLARE dim_planmon_interval = i4 WITH protect, noconstant(0)
 DECLARE dim_unattended_ind = i2 WITH protect, noconstant(0)
 DECLARE dim_status = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dim_pause_sec = i4 WITH protect, noconstant(0)
 DECLARE dim_audit_type = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dim_arch_notify_threshold = f8 WITH protect, noconstant(0.0)
 DECLARE dim_arch_pause_threshold = f8 WITH protect, noconstant(0.0)
 DECLARE dim_arch_space_used = f8 WITH protect, noconstant(0.0)
 DECLARE dim_arch_dest = vc WITH protect, noconstant(" ")
 DECLARE dim_temp_num_str = vc WITH protect, noconstant(" ")
 DECLARE dim_cur_dpe_id = f8 WITH protect, noconstant(0.0)
 DECLARE dim_cur_mode_temp = vc WITH protect, noconstant(" ")
 DECLARE dim_cur_dpe_set_ind = i2 WITH protect, noconstant(0)
 DECLARE dim_cur_appl_id = vc WITH protect, noconstant(" ")
 DECLARE dim_dkill_cnt = i4 WITH protect, noconstant(0)
 DECLARE dim_rkill_cnt = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_install_monitor",".log","dm2_install_monitor LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_install_monitor"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm2_process_rs->process_name = dpl_install_monitor
 IF (drr_get_process_status("DM2_INSTALL_MONITOR",dim_plan_id,dim_process_status)=0)
  GO TO exit_script
 ENDIF
 IF (dim_process_status=1)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Checking for Active monitoring session."
  SET dm_err->emsg = concat("Monitoring is already active for the Install Plan ",trim(cnvtstring(
     dim_plan_id)))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (drr_insert_runner_row("DM2_INSTALL_MONITOR",currdbhandle," ",1,dim_plan_id)=0)
  GO TO exit_script
 ENDIF
 IF ((dm2_process_event_rs->ui_allowed_ind=1))
  IF (validate(dm2_package_os_session_logfile,"x") != "x")
   ...
   ENDIF
  ENDIF
  CALL dm2_process_log_add_detail_text(dpl_install_mode,dim_mode)
  IF (dm2_process_log_dtl_row(dir_ui_misc->dm_process_event_id,1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Update monitoring setting to Active."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 UPDATE  FROM dm_info
  SET info_number = 1
  WHERE info_domain="DM2_INSTALL_MONITOR:PLANSETTINGS"
   AND cnvtint(info_name)=dim_plan_id
   AND info_number != 1
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
 ...
 IF ((dnotify->status=0))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Checking for notification settings."
  SET dm_err->emsg = concat("Installation Monitoring was issued for Install Plan",trim(cnvtstring(
     dim_plan_id))," but could not start because Notification is turned OFF.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 WHILE (true)
   IF (drr_cleanup_dm_info_runners(null)=0)
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = "Retrieve Install Monitor settings."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    ...
    WHERE d.info_domain="DM2_INSTALL_MONITOR:PLANSETTINGS"
     AND cnvtint(d.info_name)=dim_plan_id
    HEAD REPORT
     dim_planmon_status = 1, dim_planmon_interval = 15
    DETAIL
     dim_planmon_status = d.info_number, dim_planmon_interval = d.info_long_id
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
   IF (dim_planmon_status=0)
    SET dnotify->process = "INSTALLPLANMON"
    SET dnotify->plan_id = dim_plan_id
    SET dnotify->install_status = "STOPPING"
    SET dnotify->event = "Stop Monitoring"
    SET dnotify->mode = dim_mode
    SET dnotify->msgtype = "WARNING"
    CALL dn_add_body_text(concat("Installation Monitoring was stopped for Install Plan ",trim(
       cnvtstring(dim_plan_id))," by request."),1)
    CALL dn_add_body_text(" ",0)
    CALL dn_add_body_text(
     "Please go to the following location to resubmit the Install Monitoring process.",0)
    CALL dn_add_body_text("   ccl | dm2_install_plan_menu->Installation Monitoring",0)
    SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->status = dpl_complete
    SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->install_plan_id = dim_plan_id
    CALL dm2_process_log_add_detail_text(dpl_audit_name,"EMAIL:MONITORING_STOPPED")
    CALL dm2_process_log_add_detail_text(dpl_audit_type,dpl_warning)
    IF (dn_notify(null)=0)
     GO TO exit_script
    ENDIF
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Installation Monitoring was stopped for Install Plan ",trim(cnvtstring
      (dim_plan_id))," by request.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = "Checking for Unattended installs status."
   SELECT INTO "nl:"
    FROM dm_process dp,
     dm_process_event dpe,
     dm_process_event_dtl dped1,
     dm_process_event_dtl dped2
    PLAN (dpe
     WHERE dpe.install_plan_id=dim_plan_id)
     JOIN (dp
     WHERE dp.dm_process_id=dpe.dm_process_id
      AND dp.process_name=dpl_package_install
      AND dp.action_type=dpl_execution)
     ...
     WHERE dpe.dm_process_event_id=dped1.dm_process_event_id
      AND dped1.detail_type="INSTALL_MODE"
      AND dped1.detail_text=dim_mode)
     JOIN (dped2
     WHERE dped1.dm_process_event_id=dped2.dm_process_event_id
      AND dped2.detail_type="UNATTENDED_IND")
    ORDER BY dpe.begin_dt_tm DESC
    DETAIL
     dim_status = dpe.event_status, dim_unattended_ind = dped2.detail_number
   ...
   ELSEIF (curqual=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Failed to find package execution for plan ",trim(cnvtstring(
       dim_plan_id)))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
   IF (dim_status=dpl_complete)
    SET dm_err->eproc = concat("Plan ",trim(cnvtstring(dim_plan_id))," has been completed.")
    CALL disp_msg("",dm_err->logfile,0)
    GO TO exit_script
   ELSEIF (dim_status=dpl_failed)
    SET dnotify->process = "INSTALLPLANMON"
    SET dnotify->plan_id = dim_plan_id
    SET dnotify->install_status = dpl_failed
    SET dnotify->event = "Install Plan Not Executing"
    SET dnotify->mode = dim_mode
    IF (dim_unattended_ind=1)
     SET dnotify->msgtype = dpl_actionreq
     SET dim_audit_type = dpl_actionreq
    ELSE
     SET dnotify->msgtype = dpl_warning
     SET dim_audit_type = dpl_warning
    ENDIF
    CALL dn_add_body_text(concat("Installation Monitoring detected a FAILED status for Install Plan ",
      trim(cnvtstring(dim_plan_id)),"."),1)
    CALL dn_add_body_text(" ",0)
    CALL dn_add_body_text("Please go to the following location to evaluate the error condition.",0)
    CALL dn_add_body_text("   ccl | dm2_install_plan_menu",0)
    SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->status = dpl_complete
    SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->install_plan_id = dim_plan_id
    CALL dm2_process_log_add_detail_text(dpl_audit_name,"EMAIL:INSTALL_FAILED")
    CALL dm2_process_log_add_detail_text(dpl_audit_type,dim_audit_type)
    IF (dn_notify(null)=0)
     GO TO exit_script
    ENDIF
    SET dm_err->err_ind = 1
    SET dm_err->eproc = concat("Retrieving most recent package install execution for plan ",trim(
      cnvtstring(dim_plan_id)))
    SET dm_err->emsg = concat("Installation Monitoring detected a FAILED status for Install Plan ",
     trim(cnvtstring(dim_plan_id)))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ELSEIF (dim_status=dpl_paused)
    SET dnotify->process = "INSTALLPLANMON"
    SET dnotify->plan_id = dim_plan_id
    SET dnotify->install_status = dpl_paused
    SET dnotify->event = "Install Plan is in PAUSED status"
    SET dnotify->mode = dim_mode
    IF (dim_unattended_ind=1)
     SET dnotify->msgtype = dpl_actionreq
     SET dim_audit_type = dpl_actionreq
    ELSE
     ...
     SET dim_audit_type = dpl_warning
    ENDIF
    CALL dn_add_body_text("Install Plan is in PAUSED status.",1)
    CALL dn_add_body_text(" ",0)
    CALL dn_add_body_text(concat("Please confirm that the necessary precautions have been taken "),0)
    CALL dn_add_body_text("and then proceed by executing the following:",0)
    CALL dn_add_body_text(concat("ccl | dm2_install_plan_menu go | Select Plan ",trim(cnvtstring(
        dnotify->plan_id))," | Modify Install Status | EXECUTE"),0)
    SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->status = dpl_complete
    SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->install_plan_id = dim_plan_id
    CALL dm2_process_log_add_detail_text(dpl_audit_name,"EMAIL:INSTALL_PAUSED")
    CALL dm2_process_log_add_detail_text(dpl_audit_type,dim_audit_type)
    IF (dn_notify(null)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   SET dm_err->eproc = "Check for SUSPENDED resumable session."
   SELECT INTO "nl:"
    ...
    PLAN (gvr
     ...
     JOIN (gvs
     WHERE gvr.instance_id=gvs.inst_id
      AND gvr.session_id=gvs.sid
      AND gvs.schemaname=currdbuser)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSEIF (curqual > 0)
    SET dnotify->process = "INSTALLPLANMON"
    SET dnotify->plan_id = dim_plan_id
    SET dnotify->install_status = "SUSPENDED"
    SET dnotify->event = "Oracle Suspended Operations Detected"
    SET dnotify->mode = dim_mode
    IF (dim_unattended_ind=1)
     SET dnotify->msgtype = dpl_actionreq
     SET dim_audit_type = dpl_actionreq
    ...
    CALL dn_add_body_text(concat(
      "Suspended Oracle operations were found, requiring additional tablespace for the ",
      "operation(s) to resume and the Install to proceed."),1)
    CALL dn_add_body_text(" ",0)
    CALL dn_add_body_text(
     "Please execute the following for details on which objects require additional tablespace:",0)
    CALL dn_add_body_text("   ccl | dm2_resumable_session_menu go",0)
    CALL dn_add_body_text(" ",0)
    CALL dn_add_body_text(concat(
      "After adding tablespace for suspended objects, Oracle should automatically resume ",
      "the suspended operation(s) and the Installation should automatically resume processing."),0)
    SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->status = dpl_complete
    SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->install_plan_id = dim_plan_id
    CALL dm2_process_log_add_detail_text(dpl_audit_name,"EMAIL:SUSPENDED_SESSION")
    CALL dm2_process_log_add_detail_text(dpl_audit_type,dim_audit_type)
    IF (dn_notify(null)=0)
     GO TO exit_script
    ENDIF
    ...
    SET dm_err->emsg = concat(
     "Suspended Oracle operations were found, requiring additional tablespace for the ",
     "operation(s) to resume and the Install to proceed.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = "Check for ArchiveLog monitoring"
   CALL disp_msg("",dm_err->logfile,0)
   IF (dipma_check_db_arch_mode(null)=0)
    ...
    ENDIF
   ELSE
    IF (dipma_get_arch_thresholds(dim_arch_pause_threshold,dim_arch_notify_threshold)=0)
     IF ((dm_err->err_ind=1))
      GO TO exit_script
     ENDIF
    ELSE
     IF (dipma_get_arch_dest_space(dim_arch_dest,dim_arch_space_used)=0)
      GO TO exit_script
     ENDIF
     IF (dim_arch_space_used > dim_arch_notify_threshold)
      IF (dim_arch_space_used > dim_arch_pause_threshold
       AND dim_arch_pause_threshold != 0
       AND dim_status=dpl_executing)
       SET dim_cur_mode_temp = "DM2NOTSET"
       SET dm_err->eproc = "Obtain dpe_id for Install process"
       SELECT INTO "nl:"
        FROM dm_process dp,
         dm_process_event dpe,
         dm_process_event_dtl dped
        ...
        ORDER BY dpe.begin_dt_tm DESC
        HEAD REPORT
         dim_cur_dpe_set_ind = 0
        DETAIL
         IF (dim_cur_dpe_set_ind=0)
          IF (cnvtupper(trim(dped.detail_text)) != "BATCHPREVIEW")
           dim_cur_dpe_set_ind = 1
          ENDIF
          IF (dim_cur_mode_temp != cnvtupper(trim(dped.detail_text)))
           dim_cur_dpe_id = dpe.dm_process_event_id, dim_cur_mode_temp = cnvtupper(trim(dped
             .detail_text))
          ENDIF
         ENDIF
        WITH nocounter
       ...
       IF (curqual=0)
        ...
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       ENDIF
       SET dm_err->eproc = "Obtain AUDSID for Install process"
       SELECT INTO "nl:"
        FROM dm_process_event dpe,
         dm_process_event_dtl dped
        WHERE dpe.dm_process_event_id=dim_cur_dpe_id
         AND dped.dm_process_event_id=dpe.dm_process_event_id
        DETAIL
         IF (dped.detail_type="AUDSID")
          dim_cur_appl_id = trim(dped.detail_text,3)
         ENDIF
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc)=1)
        ...
        GO TO exit_script
       ENDIF
       IF (drr_modify_install_status(dim_plan_id,dim_cur_appl_id,2,
        "DM2_INSTALL_MONITOR paused installation because ArchiveLog threshold was reached.",
        "DM2_INSTALL_MONITOR")=0)
        GO TO exit_script
       ENDIF
       SET dnotify->process = "INSTALLPLANMON"
       SET dnotify->plan_id = dim_plan_id
       SET dnotify->install_status = dpl_paused
       SET dnotify->event = "Archivelog Destination threshold exceeded"
       SET dnotify->mode = dim_mode
       SET dnotify->msgtype = dpl_actionreq
       SET dim_audit_type = dpl_actionreq
       CALL dn_add_body_text(
        "ArchiveLog Destination Space Usage has exceeded Installation Pause threshold.",1)
       CALL dn_add_body_text(" ",0)
       SET dim_temp_num_str = dipma_get_two_dec(build(dim_arch_pause_threshold))
       CALL dn_add_body_text(concat("Current Pause threshold: ",dim_temp_num_str," Gigabytes"),0)
       CALL dn_add_body_text(" ",0)
       SET dim_temp_num_str = dipma_get_two_dec(build(dim_arch_space_used))
       CALL dn_add_body_text(concat("Current Space Used: ",dim_temp_num_str," Gigabytes"),0)
       CALL dn_add_body_text(" ",0)
       CALL dn_add_body_text(concat(
         "To disable ArchiveLog monitoring or adjust ArchiveLog monitoring thresholds,",
         " navigate to the following menu location:"),0)
       CALL dn_add_body_text(" ",0)
       CALL dn_add_body_text(concat("     ccl | dm2_install_plan_menu go | select Plan id: ",trim(
          cnvtstring(dim_plan_id))," | Installation Monitoring "),0)
       CALL dn_add_body_text(" ",0)
       CALL dn_add_body_text("To resume Installation processing, execute the following: ",0)
       CALL dn_add_body_text(" ",0)
       CALL dn_add_body_text(concat("     ccl | dm2_install_plan_menu go | select Plan id: ",trim(
          cnvtstring(dim_plan_id))," | Modify Install Status | Execute "),0)
       SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
       SET dm2_process_event_rs->status = dpl_complete
       SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
       SET dm2_process_event_rs->install_plan_id = dim_plan_id
       CALL dm2_process_log_add_detail_text(dpl_audit_name,"EMAIL:ARCHLOG_PAUSED_INSTALL")
       CALL dm2_process_log_add_detail_text(dpl_audit_type,dim_audit_type)
       IF (dn_notify(null)=0)
        GO TO exit_script
       ENDIF
      ELSEIF (dim_arch_notify_threshold != 0
       AND dim_status IN (dpl_executing, dpl_paused))
       SET dnotify->process = "INSTALLPLANMON"
       SET dnotify->plan_id = dim_plan_id
       SET dnotify->install_status = dim_status
       SET dnotify->event = "Archivelog Destination Space Warning"
       SET dnotify->mode = dim_mode
       SET dnotify->msgtype = dpl_actionreq
       SET dim_audit_type = dpl_actionreq
       CALL dn_add_body_text("ArchiveLog Destination Space Usage has exceeded Warning threshold.",1)
       CALL dn_add_body_text(" ",0)
       SET dim_temp_num_str = dipma_get_two_dec(build(dim_arch_notify_threshold))
       CALL dn_add_body_text(concat("Current Warning threshold: ",dim_temp_num_str," Gigabytes"),0)
       CALL dn_add_body_text(" ",0)
       SET dim_temp_num_str = dipma_get_two_dec(build(dim_arch_space_used))
       CALL dn_add_body_text(concat("Current Space Used: ",dim_temp_num_str," Gigabytes"),0)
       CALL dn_add_body_text(" ",0)
       ...
       SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
       SET dm2_process_event_rs->install_plan_id = dim_plan_id
       CALL dm2_process_log_add_detail_text(dpl_audit_name,"EMAIL:ARCHLOG_WARNING")
       CALL dm2_process_log_add_detail_text(dpl_audit_type,dim_audit_type)
       IF (dn_notify(null)=0)
        GO TO exit_script
       ENDIF
    ...
    ENDIF
   ENDIF
   SET dm_err->eproc = "Check for killed DDL sessions."
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=
    (SELECT
     ...
     WHERE (ddo.ocd=- ((1 * dim_plan_id)))))
     AND ddol.status="ERROR"
     AND ddol.error_msg=dir_kill_clause
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ELSEIF (curqual > dim_dkill_cnt)
    SET dim_dkill_cnt = curqual
    SET dnotify->process = "INSTALLPLANMON"
    SET dnotify->plan_id = dim_plan_id
    SET dnotify->install_status = "WARNING"
    SET dnotify->event = "Killed DDL Sessions Detected"
    SET dnotify->mode = dim_mode
    SET dnotify->msgtype = dpl_warning
    SET dim_audit_type = dpl_warning
    CALL dn_add_body_text("Killed DDL sessions were detected.",1)
    CALL dn_add_body_text(" ",0)
    CALL dn_add_body_text("Please execute the following for details on schema failure:",0)
    CALL dn_add_body_text(concat("   ccl | dm2_install_plan_menu go | select Plan id: ",trim(
       cnvtstring(dim_plan_id)),
      " | Installation Reports | View Schema (DDL) Reports | View All Failed Schema Operations"),0)
    SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->status = dpl_complete
    SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->install_plan_id = dim_plan_id
    CALL dm2_process_log_add_detail_text(dpl_audit_name,"EMAIL:DDL_KILLED_WARNING")
    CALL dm2_process_log_add_detail_text(dpl_audit_type,dim_audit_type)
    IF (dn_notify(null)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   IF (dim_dkill_cnt=0)
    SET dm_err->eproc = "Check for killed Readme sessions."
    SELECT INTO "nl:"
     FROM dm_ocd_log dol
    ...
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ELSEIF (curqual > dim_rkill_cnt)
     SET dim_rkill_cnt = curqual
     SET dnotify->process = "INSTALLPLANMON"
     SET dnotify->plan_id = dim_plan_id
     SET dnotify->install_status = "WARNING"
     SET dnotify->event = "Killed Readme Sessions Detected"
     SET dnotify->mode = dim_mode
     SET dnotify->msgtype = dpl_warning
     SET dim_audit_type = dpl_warning
     CALL dn_add_body_text("Killed Readme sessions were detected.",1)
     CALL dn_add_body_text(" ",0)
     CALL dn_add_body_text("Please execute the following for details on readme failure:",0)
     CALL dn_add_body_text(concat("   ccl | dm2_install_plan_menu go | select Plan id: ",trim(
        cnvtstring(dim_plan_id)),
       " | Installation Reports | View Readme Execution Reports | View Log of Failed Readmes"),0)
     SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
     SET dm2_process_event_rs->status = dpl_complete
     SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
     SET dm2_process_event_rs->install_plan_id = dim_plan_id
     CALL dm2_process_log_add_detail_text(dpl_audit_name,"EMAIL:README_KILLED_WARNING")
     CALL dm2_process_log_add_detail_text(dpl_audit_type,dim_audit_type)
     IF (dn_notify(null)=0)
      GO TO exit_script
     ENDIF
    ENDIF
   ENDIF
   SET dim_pause_sec = (60 * dim_planmon_interval)
   CALL pause(dim_pause_sec)
 ENDWHILE
#exit_script
 CALL drr_remove_runner_row("DM2_INSTALL_MONITOR",currdbhandle)
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "DM2_INSTALL_MONITOR completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_install_monitor")
END GO
