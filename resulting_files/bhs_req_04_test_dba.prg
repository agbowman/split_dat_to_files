CREATE PROGRAM bhs_req_04_test:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SET request->person_id = 958824.00
 SET stat = alterlist(request->order_qual,1)
 SET request->order_qual[1].encntr_id = 2497006.00
 SET request->order_qual[1].order_id = 29376166.00
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (pa.person_alias_type_cd=other_alias_cd
    ...
    IF (pa.alias_pool_cd > 0)
     orders->trauma = cnvtalias(pa.alias,pa.alias_pool_cd)
    ELSE
     orders->trauma = pa.alias
    ENDIF
   ENDIF
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(request->printer_name)
   ...
   PLAN (d1)
   HEAD REPORT
    first_page = "Y", spaces = fillstring(50," "), temp_s = fillstring(1000,""),
   ...
   DETAIL
    FOR (vv = 1 TO value(order_cnt))
      ...
      FOR (fsub = 1 TO 31)
        FOR (ww = 1 TO orders->qual[vv].d_cnt)
          IF ((((orders->qual[vv].d_qual[ww].group_seq=fsub)) OR (fsub=31
           AND (orders->qual[vv].d_qual[ww].group_seq != 15))) )
           IF ((orders->qual[vv].d_qual[ww].value > " "))
            ...
            value,
            xcol = 330
            IF ((orders->qual[vv].d_qual[ww].label_text="Test(s) Requested*"))
             ...
             WHILE (limit <= maxlen)
              ...
              ELSE
               xpos = (xpos+ 1), c = substring(limit,1,tempstring),
               CALL echo(build(" c :",c)),
      ...
      IF ((orders->qual[vv].com_ln_cnt > 0))
       ...
       ENDFOR
      ENDIF
      xrow = (xrow+ 30), xcol = 470,
      CALL print(calcpos(xcol,xrow)),
      "{cpi/14}BHS_REQ_04", row + 1
    ENDFOR
