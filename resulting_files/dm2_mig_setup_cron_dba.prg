CREATE PROGRAM dm2_mig_setup_cron:dba
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmsc_cap_loc = vc WITH protect, noconstant("")
 DECLARE dmsc_del_loc = vc WITH protect, noconstant("")
 DECLARE dmsc_gg_cap_loc = vc WITH protect, noconstant("")
 DECLARE dmsc_gg_del_loc = vc WITH protect, noconstant("")
 DECLARE dmsc_inst_loc = vc WITH protect, noconstant("")
 DECLARE dmsc_env_var = vc WITH protect, noconstant("")
 DECLARE dmsc_file_copy(in_file=vc,out_file=vc) = i2
 IF (check_logfile("dm2_mig_setup_cron",".log","DM2_MIG_SETUP_CRON LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_setup_cron"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 SET dmr_mig_data->tgt_v500_pwd = dm2_install_schema->p_word
 SET dmr_mig_data->tgt_v500_cnct_str = dm2_install_schema->connect_str
 SET dmsc_inst_loc = concat(trim(logical("cer_install")),"/")
 SET dmsc_env_var = trim(logical("environment"))
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (di.info_name="GG_CAP_DIR")
    dmsc_gg_cap_loc = trim(di.info_char,3)
   ELSE
    dmsc_gg_del_loc = trim(di.info_char,3)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSEIF (((size(trim(dmsc_gg_cap_loc,3))=0) OR (size(trim(dmsc_gg_del_loc,3))=0)) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No DM_INFO rows exist for Capture/Delivery Directories path.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (substring(size(trim(dmsc_gg_cap_loc,3)),1,dmsc_gg_cap_loc)="/")
  SET dmsc_gg_cap_loc = trim(replace(dmsc_gg_cap_loc,"/","",2),3)
 ENDIF
 IF (substring(size(trim(dmsc_gg_del_loc,3)),1,dmsc_gg_del_loc)="/")
  SET dmsc_gg_del_loc = trim(replace(dmsc_gg_del_loc,"/","",2),3)
 ENDIF
 IF ((dm2_sys_misc->cur_os="AXP"))
  SET dmsc_cap_loc = "GGCAPTURE:[DIRTMP]"
  SET dmsc_del_loc = "GGDELIVERY:[DIRTMP]"
 ELSE
  SET dmsc_cap_loc = "/ggcapture/dirtmp/"
  SET dmsc_del_loc = "/ggdelivery/dirtmp/"
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_get_adata.txt",build(dmsc_cap_loc,
   "dm2_mig_get_adata.ksh"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_cronmain_capture.txt",build(dmsc_cap_loc,
   "dm2_mig_cronmain_capture.ksh"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_suplog_adder.txt",build(dmsc_cap_loc,
   "dm2_mig_suplog_adder.ksh"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_suplog_adder_delta.sql",build(dmsc_cap_loc,
   "dm2_mig_suplog_adder_delta.sql"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_lob_check.txt",build(dmsc_inst_loc,
   "dm2_mig_lob_check.ksh"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_verify_data.txt",build(dmsc_inst_loc,
   "dm2_mig_verify_data.ksh"))))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Generating ",dmsc_cap_loc,"dm2_mig_ggsci.ksh")
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO value(build(dmsc_cap_loc,"dm2_mig_ggsci.ksh"))
  FROM dummyt d
  DETAIL
   CALL print(concat(dmsc_gg_cap_loc,"/ggsci <<! > $2")), row + 1,
   CALL print("$1"),
   row + 1,
 ...
 IF (check_error(dm_err->eproc))
  ...
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_verify_rowcnts.txt",build(dmsc_del_loc,
   "dm2_mig_verify_rowcnts.ksh"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_get_adata.txt",build(dmsc_del_loc,
   "dm2_mig_get_adata.ksh"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_cronmain_delivery.txt",build(dmsc_del_loc,
   "dm2_mig_cronmain_delivery.ksh"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_get_final_trans.txt",build(dmsc_del_loc,
   "dm2_mig_get_final_trans.ksh"))))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Generating ",dmsc_del_loc,"dm2_mig_ggsci.ksh")
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO value(build(dmsc_del_loc,"dm2_mig_ggsci.ksh"))
  FROM dummyt d
  DETAIL
   CALL print(concat(dmsc_gg_del_loc,"/ggsci <<! > $2")), row + 1,
   CALL print("$1"),
   row + 1,
   CALL print("exit"), row + 1,
   CALL print("!"), row + 1
 ...
 IF (check_error(dm_err->eproc))
  ...
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_autoextend_tbs.txt",build(dmsc_del_loc,
   "dm2_mig_autoextend_tbs.ksh"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_autoextend_tbs.sql",build(dmsc_del_loc,
   "dm2_mig_autoextend_tbs.sql"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_bdm_index_set.txt",build(dmsc_del_loc,
   "dm2_mig_bdm_index_set.ksh"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_bdm_mega.sql",build(dmsc_del_loc,
   "dm2_mig_bdm_mega.sql"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_bdm_index_cnt.sql",build(dmsc_del_loc,
   "dm2_mig_bdm_index_cnt.sql"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_bdm_mega.txt",build(dmsc_del_loc,
   "dm2_mig_bdm_mega.ksh"))))
  GO TO exit_script
 ENDIF
 IF ( NOT (dmsc_file_copy("CER_INSTALL:dm2_mig_bdm_email_mega.txt",build(dmsc_del_loc,
   "dm2_mig_bdm_email_mega.ksh"))))
  GO TO exit_script
 ENDIF
 IF (dm2_push_dcl(concat("chmod 777 ",build(dmsc_inst_loc,"dm2_mig_lob_check.ksh")))=0)
  SET dm_err->eproc = "Unable to change mode for file dm2_mig_lob_check.ksh"
  GO TO exit_script
 ENDIF
 IF (dm2_push_dcl(concat("chmod 777 ",build(dmsc_inst_loc,"dm2_mig_verify_data.ksh")))=0)
  SET dm_err->eproc = "Unable to change mode for file dm2_mig_verify_data.ksh"
  GO TO exit_script
 ENDIF
 GO TO exit_script
 SUBROUTINE dmsc_file_copy(in_file,out_file)
   DECLARE dmsc_temp_line = vc WITH protect, noconstant("")
   DECLARE dmsc_str = vc WITH protect, noconstant("")
   SET dmsc_str = trim(replace(dmsc_gg_del_loc,"/","\/",0),3)
   IF (substring(1,2,dmsc_str)="\/")
    SET dmsc_str = trim(replace(dmsc_str,"\/","/",1),3)
   ENDIF
   IF (substring((size(trim(dmsc_str,3)) - 1),2,dmsc_str)="\/")
    SET dmsc_str = trim(replace(dmsc_str,"\/","",2),3)
   ENDIF
   FREE DEFINE rtl2
   SET logical file_name value(in_file)
   DEFINE rtl2 "file_name"  WITH nomodify
   SET dm_err->eproc = concat("Copying ",in_file," to ",out_file)
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO value(out_file)
    FROM rtl2t r
    DETAIL
     IF (((findstring("dm2_mig_cronmain_capture.ksh",out_file,1,1) > 0) OR (findstring(
      "dm2_mig_cronmain_delivery.ksh",out_file,1,1) > 0)) )
      dmsc_temp_line = replace(r.line,"<=GG_DEL_DIR=>",dmsc_gg_del_loc), dmsc_temp_line = replace(
       dmsc_temp_line,"<=GG_CAP_DIR=>",dmsc_gg_cap_loc), dmsc_temp_line = replace(dmsc_temp_line,
       "<=FT_GG_DEL_DIR=>",dmsc_str),
      CALL print(dmsc_temp_line), row + 1
     ELSEIF (findstring("dm2_mig_suplog_adder.ksh",out_file,1,1) > 0)
      dmsc_temp_line = replace(r.line,"<gg_cap_dir>",dmsc_gg_cap_loc),
      CALL print(dmsc_temp_line), row + 1
     ELSEIF (((findstring("dm2_mig_verify_rowcnts.ksh",out_file,1,1) > 0) OR (findstring(
      "dm2_mig_get_final_trans.ksh",out_file,1,1) > 0)) )
      dmsc_temp_line = replace(r.line,"<gg_del_dir>",dmsc_gg_del_loc)
      IF ((dmr_mig_data->src_ora_level1 > 11))
       dmsc_temp_line = replace(dmsc_temp_line,"<=PDBNAME=>",concat(dmr_mig_data->src_db_name,"."))
      ELSE
       dmsc_temp_line = replace(dmsc_temp_line,"<=PDBNAME=>","")
      ENDIF
      CALL print(dmsc_temp_line), row + 1
     ELSEIF (findstring("dm2_mig_verify_data.ksh",out_file,1,1) > 0)
      dmsc_temp_line = replace(r.line,"<=srcenv=>",dmsc_env_var),
      CALL print(dmsc_temp_line), row + 1
     ELSEIF (findstring("dm2_mig_lob_check.ksh",out_file,1,1) > 0)
      dmsc_temp_line = replace(r.line,"<=tgtpw=>",dmr_mig_data->tgt_v500_pwd), dmsc_temp_line =
      replace(dmsc_temp_line,"<=tgtconnect=>",dmr_mig_data->tgt_v500_cnct_str),
      CALL print(dmsc_temp_line),
      row + 1
     ELSE
 ...
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "Ending dm2_mig_setup_cron."
 CALL final_disp_msg("dm2_mig_setup_cron")
END GO
