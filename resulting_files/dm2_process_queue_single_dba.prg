CREATE PROGRAM dm2_process_queue_single:dba
 ...
 SUBROUTINE dpq_cleanup_stranded_procs(dcsp_proc_type,dcsp_maxcommit)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dpqrs_dpq_id = f8 WITH public, noconstant(0.0)
 DECLARE dpqrs_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dpqrs_process_type = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dpqrs_tmp_num = f8 WITH protect, noconstant(0.0)
 DECLARE dpqrs_exec_ind = i2 WITH public, noconstant(1)
 DECLARE dpqrs_iter = i4 WITH protect, noconstant(0)
 DECLARE dpqrs_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dpqrs_continue_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_dpq_single",".log","dm2_process_queue_single LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_process_queue_single"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dpqrs_dpq_id = cnvtreal( $1)
 IF (((check_error(dm_err->eproc)=1) OR ( NOT (dpqrs_dpq_id > 0.0))) )
  SET dm_err->emsg = "Parameter usage: dm2_process_queue_single <dpq_id>"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Verifying dm_process_queue_id exists and status is QUEUED"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_process_queue dpq
  WHERE dpq.dm_process_queue_id=dpqrs_dpq_id
   AND dpq.process_status="QUEUED"
  DETAIL
   dpqrs_process_type = dpq.process_type
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF (curqual=1
  AND dpqrs_process_type != "DM2NOTSET")
  SET dm2_process_event_rs->status = dpl_executing
  SET dm2_process_rs->process_name = dpl_process_queue_single
  CALL dm2_process_log_add_detail_text(dpl_proc_queue_runner_type,dpqrs_process_type)
  CALL dm2_process_log_add_detail_number(dpl_dpq_id,dpqrs_dpq_id)
  CALL dm2_process_log_add_detail_text(dpl_audsid,currdbhandle)
  IF (dm2_process_log_row(dpl_process_queue_single,dpl_execution,dpl_no_prev_id,1)=0)
   GO TO exit_script
  ENDIF
  SET dpqr_log_id = dm2_process_event_rs->dm_process_event_id
  SET dm2_process_rs->process_name = ""
  SET dpqrs_exec_ind = 1
  IF (dpq_populate_queue_rows(dpqrs_process_type)=0)
   GO TO exit_script
  ENDIF
  IF ((dpq_proc_list->proc_cnt != 1))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Unable to find a single dm_process_queue_row with the given dpq_id"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dpq_process_queue->dm_process_queue_id = dpq_proc_list->qual[1].dm_process_queue_id
  SET dpq_process_queue->operation_txt = dpq_proc_list->qual[1].operation_txt
  SET dpq_process_queue->op_method = dpq_proc_list->qual[1].op_method
  IF ( NOT (dpq_lock_queue_row(dpqrs_continue_ind)))
   GO TO exit_script
  ELSEIF (dpqrs_continue_ind)
   IF ( NOT (dpq_execute_command(dpqrs_continue_ind)))
    GO TO exit_script
   ENDIF
   IF (dpq_update_queue_row(dpqrs_continue_ind)=0)
    GO TO exit_script
   ENDIF
  ELSE
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Unable to lock dm_process_queue_row for dpq_id: ",cnvtstring(
     dpqrs_dpq_id))
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No rows qaulified with the given dm_process_queue_id"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
#exit_script
 ...
 SET dpqrs_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_process_queue_single,dpl_execution,dpqrs_log_id,1)
 SET dm2_process_rs->process_name = ""
 SET dm_err->err_ind = dpqrs_err_ind
 SET dm_err->eproc = "Ending dm2_process_queue_single"
 CALL final_disp_msg("dm2_process_queue_single")
END GO
