CREATE PROGRAM cp_br_get_nodes_config:dba
 ...
 DECLARE getnodes(null) = null WITH protect
 DECLARE getconcepts(null) = null WITH protect
 DECLARE detectnewcontentversion = null WITH protect
 DECLARE nodeidlistsize = i4 WITH noconstant(size(request->node_ids,5)), protect
 DECLARE intentioncdlistsize = i4 WITH noconstant(size(request->intention_cds,5)), protect
 ...
 CALL getnodes(null)
 IF ((request->resolve_concept_details=1))
  CALL getconcepts(null)
 ENDIF
 CALL detectnewcontentversion(null)
 ...
 SUBROUTINE verifyinputs(null)
   ...
   IF (nodeidlistsize <= 0)
    CALL reporterrorandexit("PERFORM","Validate","Node Ids must be populated")
   ENDIF
   CALL log_message(build("Exit verifyInputs(), Elapsed time in seconds:",((curtime3 - begin_curtime3
     )/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getnodes(null)
   CALL log_message("In getNodes()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE x = i4 WITH noconstant(0), protect
   DECLARE y = i4 WITH noconstant(0), protect
   DECLARE nodeexpandclause = vc WITH noconstant(""), protect
   DECLARE intentionexpandclause = vc WITH noconstant("1=1"), protect
   IF (nodeidlistsize > 0)
    SET nodeexpandclause =
    "expand(x,1,nodeIdListSize, cn.cp_node_id, request->node_ids[x].cp_node_id)"
   ENDIF
   IF (intentioncdlistsize > 0)
    SET intentionexpandclause =
    "expand(y,1,intentionCdListSize, cn.intention_cd, request->intention_cds[y].intention_cd)"
   ENDIF
   CALL echo(nodeexpandclause)
   SELECT INTO "NL:"
    FROM cp_node cn,
     (left JOIN cp_component cp ON cp.cp_node_id=cn.cp_node_id),
     (left JOIN cp_component_detail ccd ON ccd.cp_component_id=cp.cp_component_id)
    PLAN (cn
     WHERE parser(nodeexpandclause)
      AND parser(intentionexpandclause))
     JOIN (cp)
     JOIN (ccd)
    ORDER BY cn.collation_seq, cn.node_display, cn.cp_node_id,
     cp.cp_component_id, ccd.cp_component_detail_id
    HEAD REPORT
     nodecnt = 0, conceptcdcnt = 0
    HEAD cn.cp_node_id
     compcnt = 0
     IF (locateval(y,1,conceptcdcnt,cn.concept_cd,reply->concept_list[y].concept_cd)=0
      AND cn.concept_cd > 0)
      conceptcdcnt += 1
      IF (conceptcdcnt > size(reply->concept_list,5))
       stat = alterlist(reply->concept_list,(conceptcdcnt+ 5))
      ENDIF
      reply->concept_list[conceptcdcnt].concept_cd = cn.concept_cd
     ENDIF
     nodecnt += 1
     IF (nodecnt > size(reply->node_list,5))
      stat = alterlist(reply->node_list,(nodecnt+ 10))
     ENDIF
     reply->node_list[nodecnt].concept_cd = cn.concept_cd, reply->node_list[nodecnt].concept_cd_disp
      = uar_get_code_display(cn.concept_cd), reply->node_list[nodecnt].cp_node_id = cn.cp_node_id,
     reply->node_list[nodecnt].version_node_id = cn.version_node_id, reply->node_list[nodecnt].
     publish_ind = cn.publish_ind, reply->node_list[nodecnt].sequence = cn.collation_seq,
     reply->node_list[nodecnt].intention_cd = cn.intention_cd, reply->node_list[nodecnt].
     intention_cd_disp = uar_get_code_display(cn.intention_cd), reply->node_list[nodecnt].
     intention_cd_meaning = uar_get_code_meaning(cn.intention_cd),
     reply->node_list[nodecnt].node_name = cn.node_name, reply->node_list[nodecnt].category_mean = cn
     .category_mean
    HEAD cp.cp_component_id
     compdetaillistcnt = 0
     IF (cp.cp_component_id > 0)
      compcnt += 1
      IF (compcnt > size(reply->node_list[nodecnt].component_list,5))
       stat = alterlist(reply->node_list[nodecnt].component_list,(compcnt+ 10))
      ENDIF
      reply->node_list[nodecnt].component_list[compcnt].comp_type_cd = cp.comp_type_cd, reply->
      node_list[nodecnt].component_list[compcnt].comp_type_cd_disp = uar_get_code_display(cp
       .comp_type_cd), reply->node_list[nodecnt].component_list[compcnt].comp_type_cd_meaning =
      uar_get_code_meaning(cp.comp_type_cd),
      reply->node_list[nodecnt].component_list[compcnt].comp_seq_txt = cp.component_seq_txt, reply->
      node_list[nodecnt].component_list[compcnt].concept_group_cd = cp.concept_group_cd, reply->
      node_list[nodecnt].component_list[compcnt].concept_group_cd_meaning = uar_get_code_meaning(cp
       .concept_group_cd),
      reply->node_list[nodecnt].component_list[compcnt].cp_component_id = cp.cp_component_id, reply->
      node_list[nodecnt].component_list[compcnt].group = cnvtint(piece(cp.component_seq_txt,":",1,"0"
        )), reply->node_list[nodecnt].component_list[compcnt].column = cnvtint(piece(cp
        .component_seq_txt,":",2,"0")),
      reply->node_list[nodecnt].component_list[compcnt].row = cnvtint(piece(cp.component_seq_txt,":",
        3,"0"))
     ENDIF
    HEAD ccd.cp_component_detail_id
     IF (ccd.cp_component_detail_id > 0)
      compdetaillistcnt += 1
      IF (compdetaillistcnt > size(reply->node_list[nodecnt].component_list[compcnt].comp_detail_list,
       5))
       stat = alterlist(reply->node_list[nodecnt].component_list[compcnt].comp_detail_list,(
        compdetaillistcnt+ 10))
      ENDIF
      reply->node_list[nodecnt].component_list[compcnt].comp_detail_list[compdetaillistcnt].
      detail_reltn_cd = ccd.component_detail_reltn_cd, reply->node_list[nodecnt].component_list[
      compcnt].comp_detail_list[compdetaillistcnt].detail_reltn_cd_disp = uar_get_code_display(ccd
       .component_detail_reltn_cd), reply->node_list[nodecnt].component_list[compcnt].
      comp_detail_list[compdetaillistcnt].detail_reltn_cd_mean = uar_get_code_meaning(ccd
       .component_detail_reltn_cd),
      reply->node_list[nodecnt].component_list[compcnt].comp_detail_list[compdetaillistcnt].entity_id
       = ccd.component_entity_id, reply->node_list[nodecnt].component_list[compcnt].comp_detail_list[
      compdetaillistcnt].entity_name = ccd.component_entity_name, reply->node_list[nodecnt].
      component_list[compcnt].comp_detail_list[compdetaillistcnt].entity_ident = ccd.component_ident,
      reply->node_list[nodecnt].component_list[compcnt].comp_detail_list[compdetaillistcnt].
      entity_text = ccd.component_text, reply->node_list[nodecnt].component_list[compcnt].
      comp_detail_list[compdetaillistcnt].cp_component_id = ccd.cp_component_detail_id, reply->
      node_list[nodecnt].component_list[compcnt].comp_detail_list[compdetaillistcnt].version_nbr =
      ccd.version_nbr,
      reply->node_list[nodecnt].component_list[compcnt].comp_detail_list[compdetaillistcnt].
      version_text = ccd.version_text, reply->node_list[nodecnt].component_list[compcnt].
      comp_detail_list[compdetaillistcnt].default_ind = ccd.default_ind, reply->node_list[nodecnt].
      component_list[compcnt].comp_detail_list[compdetaillistcnt].source_flag = ccd.source_flag,
      reply->node_list[nodecnt].component_list[compcnt].comp_detail_list[compdetaillistcnt].
      version_flag = ccd.version_flag
     ENDIF
    FOOT  cn.cp_node_id
     stat = alterlist(reply->node_list,nodecnt), stat = alterlist(reply->concept_list,conceptcdcnt)
    FOOT  cp.cp_component_id
     IF (cp.cp_component_id > 0)
      stat = alterlist(reply->node_list[nodecnt].component_list,compcnt)
     ENDIF
    FOOT  ccd.cp_component_detail_id
     IF (ccd.cp_component_detail_id > 0)
      stat = alterlist(reply->node_list[nodecnt].component_list[compcnt].comp_detail_list,
       compdetaillistcnt)
     ENDIF
    WITH nocounter
   ;end select
   CALL error_and_zero_check_rec(curqual,"GETNODES","Get Nodes",1,1,
    reply)
   CALL log_message(build("Exit getNodes(), Elapsed time in seconds:",((curtime3 - begin_curtime3)/
     100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getconcepts(null)
   CALL log_message("In getConcepts()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE x = i4 WITH noconstant(0), protect
   DECLARE y = i4 WITH noconstant(0), protect
   DECLARE conceptlistsize = i4 WITH constant(size(reply->concept_list,5))
   IF (conceptlistsize > 0)
    SELECT INTO "NL:"
     FROM cp_concept_detail ccd
     PLAN (ccd
      WHERE expand(x,1,conceptlistsize,ccd.concept_cd,reply->concept_list[x].concept_cd))
     ORDER BY ccd.concept_cd, ccd.cp_concept_detail_id
     HEAD ccd.concept_cd
      detailcnt = 0, pos = locateval(y,1,conceptlistsize,ccd.concept_cd,reply->concept_list[y].
       concept_cd)
     HEAD ccd.cp_concept_detail_id
      detailcnt += 1
      IF (detailcnt > size(reply->concept_list[pos].concept_detail_list,5))
       stat = alterlist(reply->concept_list[pos].concept_detail_list,(detailcnt+ 10))
      ENDIF
      reply->concept_list[pos].concept_detail_list[detailcnt].entity_id = ccd.concept_entity_id,
      reply->concept_list[pos].concept_detail_list[detailcnt].entity_name = ccd.concept_entity_name,
      reply->concept_list[pos].concept_detail_list[detailcnt].detail_reltn_cd = ccd
      .concept_detail_reltn_cd,
      reply->concept_list[pos].concept_detail_list[detailcnt].detail_reltn_cd_disp =
      uar_get_code_display(ccd.concept_detail_reltn_cd)
     FOOT  ccd.cp_concept_detail_id
      stat = alterlist(reply->concept_list[pos].concept_detail_list,detailcnt)
     WITH nocounter
    ;end select
    CALL error_and_zero_check_rec(curqual,"GETCONCEPTS","Get Concepts",1,1,
     reply)
   ENDIF
   CALL log_message(build("Exit getConcepts(), Elapsed time in seconds:",((curtime3 - begin_curtime3)
     / 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE detectnewcontentversion(null)
   CALL log_message("In detectNewContentVersion()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE doccompcd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003130,"PATHWAY_DOC"))
   DECLARE gtcompcd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003130,"GUIDEDTRMNT"))
   DECLARE docdetailcd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003134,"DOCCONTENT"))
   SELECT INTO "nl:"
    FROM dd_sref_template dst1,
     dd_sref_template dst2,
     (dummyt d1  WITH seq = value(size(reply->node_list,5))),
     (dummyt d2  WITH seq = 1),
     (dummyt d3  WITH seq = 1)
    PLAN (d1
     WHERE maxrec(d2,size(reply->node_list[d1.seq].component_list,5)))
     JOIN (d2
     WHERE maxrec(d3,size(reply->node_list[d1.seq].component_list[d2.seq].comp_detail_list,5))
      AND (((reply->node_list[d1.seq].component_list[d2.seq].comp_type_cd=doccompcd)) OR ((reply->
     node_list[d1.seq].component_list[d2.seq].comp_type_cd=gtcompcd))) )
     JOIN (d3
     WHERE (reply->node_list[d1.seq].component_list[d2.seq].comp_detail_list[d3.seq].detail_reltn_cd=
     docdetailcd)
      AND (reply->node_list[d1.seq].component_list[d2.seq].comp_detail_list[d3.seq].default_ind=1))
     JOIN (dst1
     WHERE (dst1.dd_sref_tmpl_instance_ident=reply->node_list[d1.seq].component_list[d2.seq].
     comp_detail_list[d3.seq].entity_ident))
     JOIN (dst2
     WHERE dst2.cln_ident=dst1.cln_ident
      AND dst2.active_ind=1)
    HEAD d1.seq
     reply->node_list[d1.seq].latest_pub_doc_version_nbr = dst2.version_nbr, reply->node_list[d1.seq]
     .current_assoc_doc_version_nbr = dst1.version_nbr
    WITH nocounter
   ;end select
   CALL log_message(build("Exit detectNewContentVersion(), Elapsed time in seconds:",((curtime3 -
