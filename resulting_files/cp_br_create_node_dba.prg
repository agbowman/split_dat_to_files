CREATE PROGRAM cp_br_create_node:dba
 ...
 DECLARE insertrows(null) = i2 WITH protect
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE nodelistsize = i4 WITH protect, noconstant(size(request->node_list,5))
   DECLARE codevaluevalid = i2 WITH protect, noconstant(0)
   DECLARE concept_cd_codeset = i4 WITH protect, constant(4003132)
   DECLARE intention_cd_codeset = i4 WITH protect, constant(4003278)
   DECLARE treatmentline_cd_codeset = i4 WITH protect, constant(4003313)
   IF ((request->cp_pathway_id <= 0))
    SET reply->status_data.status = "F"
    ...
    SET reply->status_data.subeventstatus.targetobjectvalue = "Pathway ID must be greater than zero."
    GO TO exit_script
   ENDIF
   IF (nodelistsize <= 0)
    SET reply->status_data.status = "F"
   ...
   FOR (x = 1 TO nodelistsize)
     IF ((request->node_list[x].concept_cd > 0))
      SET codevaluevalid = checkcodevalue(request->node_list[x].concept_cd,concept_cd_codeset)
      IF (codevaluevalid != 1)
       SET reply->status_data.status = "F"
       SET reply->status_data.subeventstatus.operationname = "PERFORM"
       SET reply->status_data.subeventstatus.operationstatus = "F"
       ...
       "Concept code value does not exist or is inactive."
       GO TO exit_script
      ENDIF
     ENDIF
     IF (size(trim(request->node_list[x].node_name,3),1)=0)
      SET reply->status_data.status = "F"
      SET reply->status_data.subeventstatus.operationname = "PERFORM"
      SET reply->status_data.subeventstatus.operationstatus = "F"
      SET reply->status_data.subeventstatus.targetobjectname = "Validate"
      ...
      GO TO exit_script
     ENDIF
     IF ((request->node_list[x].intention_cd > 0))
      SET codevaluevalid = checkcodevalue(request->node_list[x].intention_cd,intention_cd_codeset)
      IF (codevaluevalid != 1)
       SET reply->status_data.status = "F"
       SET reply->status_data.subeventstatus.operationname = "PERFORM"
       SET reply->status_data.subeventstatus.operationstatus = "F"
       SET reply->status_data.subeventstatus.targetobjectname = "Validate"
       SET reply->status_data.subeventstatus.targetobjectvalue = build2("Intention CD ",trim(
         cnvtstring(request->node_list[x].intention_cd),3)," does not exist or is inactive.")
       GO TO exit_script
      ENDIF
     ELSE
      SET reply->status_data.status = "F"
      ...
      SET reply->status_data.subeventstatus.operationstatus = "F"
      SET reply->status_data.subeventstatus.targetobjectname = "Validate"
      SET reply->status_data.subeventstatus.targetobjectvalue = build2("Intention CD for list seq ",
       trim(cnvtstring(x),3)," must be non-zero.")
      GO TO exit_script
     ENDIF
     IF ((request->node_list[x].treatment_line_cd > 0))
      SET codevaluevalid = checkcodevalue(request->node_list[x].treatment_line_cd,
       treatmentline_cd_codeset)
      IF (codevaluevalid != 1)
       SET reply->status_data.status = "F"
       SET reply->status_data.subeventstatus.operationname = "PERFORM"
       SET reply->status_data.subeventstatus.operationstatus = "F"
       SET reply->status_data.subeventstatus.targetobjectname = "Validate"
       SET reply->status_data.subeventstatus.targetobjectvalue =
       "Treatment Line code value does not exist or is inactive."
       GO TO exit_script
      ENDIF
     ENDIF
   ENDFOR
 ...
 SUBROUTINE insertrows(null)
   ...
   DECLARE cp_node_id = f8 WITH noconstant(0), protect
   DECLARE version_node_id = f8 WITH noconstant(0), protect
   DECLARE node_beg_dt_tm = dq8 WITH noconstant(cnvtdatetime(sysdate))
   IF (checkdic("CP_NODE","T",0) != 2)
    ...
    GO TO exit_script
   ENDIF
   SET stat = alterlist(reply->node_list,size(request->node_list,5))
   FOR (x = 1 TO size(request->node_list,5))
     SET cp_node_id = 0.0
     SET node_beg_dt_tm = cnvtdatetime(sysdate)
     SELECT INTO "nl:"
      nextseqnum = seq(reference_seq,nextval)
      FROM dual
      DETAIL
       cp_node_id = nextseqnum
      WITH nocounter
     ;end select
     CALL echo(build2("Next CP_NODE_ID:",cp_node_id))
     IF (cp_node_id=0.0)
      SET reply->status_data.status = "F"
      SET reply->status_data.subeventstatus.operationname = "PERFORM"
      SET reply->status_data.subeventstatus.operationstatus = "F"
      ...
      GO TO exit_script
     ENDIF
     IF ((request->node_list[x].version_node_id > 0))
      SET version_node_id = request->node_list[x].version_node_id
      SELECT INTO "NL:"
       FROM cp_node cn
       WHERE cn.cp_node_id=version_node_id
       DETAIL
        node_beg_dt_tm = cn.beg_effective_dt_tm
       WITH nocounter
      ;end select
     ELSE
      SET version_node_id = cp_node_id
     ENDIF
     INSERT  FROM cp_node cn
      SET cn.active_ind = 1, cn.publish_ind = request->node_list[x].publish_ind, cn.cp_node_id =
       cp_node_id,
       cn.version_node_id = version_node_id, cn.cp_pathway_id = request->cp_pathway_id, cn.node_name
        = trim(request->node_list[x].node_name),
       cn.node_display = trim(request->node_list[x].node_name), cn.concept_cd = request->node_list[x]
       .concept_cd, cn.intention_cd = request->node_list[x].intention_cd,
       cn.treatment_line_cd = request->node_list[x].treatment_line_cd, cn.category_mean = request->
       node_list[x].category_mean, cn.collation_seq = request->node_list[x].node_seq,
       cn.create_dt_tm = cnvtdatetime(sysdate), cn.beg_effective_dt_tm = cnvtdatetime(node_beg_dt_tm),
       cn.end_effective_dt_tm = cnvtdatetime("31-DEC-2100 23:59"),
       cn.updt_cnt = 0, cn.updt_id = reqinfo->updt_id, cn.updt_applctx = reqinfo->updt_applctx,
       cn.updt_task = reqinfo->updt_task, cn.updt_dt_tm = cnvtdatetime(sysdate)
      WITH nocounter
     ;end insert
     CALL error_and_zero_check_rec(1,"INSERT","ERROR INSERTING INTO CP_NODE",1,1,
      reply)
     IF (curqual=0)
      SET reply->status_data.status = "F"
      SET reply->status_data.subeventstatus.operationname = "PERFORM"
      ...
      SET reply->status_data.subeventstatus.targetobjectname = "INSERT"
      SET reply->status_data.subeventstatus.targetobjectvalue = build2(
       "Unable to insert row into cp_node table.")
      ROLLBACK
      GO TO exit_script
     ELSE
      SET reply->node_list[x].cp_node_id = cp_node_id
      SET reply->node_list[x].concept_cd = request->node_list[x].concept_cd
      SET reply->node_list[x].node_name = request->node_list[x].node_name
      SET reply->node_list[x].intention_cd = request->node_list[x].intention_cd
      SET reply->node_list[x].treatment_line_cd = request->node_list[x].treatment_line_cd
      SET reply->node_list[x].publish_ind = request->node_list[x].publish_ind
      SET reply->node_list[x].version_node_id = version_node_id
      SET reply->node_list[x].category_mean = request->node_list[x].category_mean
     ENDIF
   ENDFOR
   SET reply->cp_pathway_id = request->cp_pathway_id
