CREATE PROGRAM djh_med_stud_90_day_nologin:dba
 PROMPT
 ...
 SELECT DISTINCT INTO value(output_dest)
  ...
  HEAD pr.name_full_formatted
   ...
   IF ( NOT (curendreport))
    row + 1
   ENDIF
  WITH format = variable, formfeed = none, maxrec = 1000
 ;end select
 IF (email_ind=1)
  SET filename_in = trim(concat(output_dest,".dat"))
  SET filename_out = concat(format(curdate,"YYYY-MM-DD;;D"),"-MedStd_90_Day_No_Activity.csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog," - ",curnode)
