CREATE PROGRAM acm_add_procedure_acti:dba
 ...
 ENDIF
 IF ( NOT (validate(add_procedure_acti_request)))
  SET failed = not_found
  SET table_name = "add_procedure_acti_request was not defined by calling script"
  GO TO exit_script
 ENDIF
 IF ( NOT (validate(add_procedure_acti_reply)))
 ...
 ENDIF
 DECLARE isupdateencntridcolexsits = i2 WITH protect, constant(checkdic(
   "PROCEDURE_ACTION.UPDATE_ENCNTR_ID","A",0))
 DECLARE contextencntrid = f8 WITH protect, constant(validate(add_procedure_acti_request->
   context_encntr_id,0.0))
 SET table_name = "PROCEDURE_ACTION"
 SET add_procedure_acti_reply->qual_cnt = size(add_procedure_acti_request->qual,5)
 SET stat = alterlist(add_procedure_acti_reply->qual,add_procedure_acti_reply->qual_cnt)
 IF ((add_procedure_acti_reply->qual_cnt=0))
  SET failed = attribute_error
  SET table_name = "No procedure action rows passed in"
  GO TO exit_script
 ENDIF
 FOR (i = 1 TO add_procedure_acti_reply->qual_cnt)
   SET generic_ind = true
   IF (generic_ind)
    IF ((add_procedure_acti_request->qual[i].procedure_action_id=0))
     SELECT INTO "nl:"
      nextseqnum = seq(reference_seq,nextval)
      FROM dual
      DETAIL
       add_procedure_acti_reply->qual[i].procedure_action_id = nextseqnum, add_procedure_acti_request
       ->qual[i].procedure_action_id = nextseqnum
      WITH nocounter
     ;end select
     IF (curqual=0)
     ...
     ENDIF
    ELSE
     SET add_procedure_acti_reply->qual[i].procedure_action_id = add_procedure_acti_request->qual[i].
     procedure_action_id
    ENDIF
   ENDIF
   IF ( NOT (generic_ind))
    SET add_procedure_acti_reply->qual[i].status = gen_nbr_error
    SET failed = gen_nbr_error
    SET table_name = "Failed to generate a sequence number"
    GO TO exit_script
   ENDIF
 ENDFOR
 IF (isupdateencntridcolexsits > 1)
  INSERT  FROM procedure_action t,
    (dummyt d  WITH seq = value(add_procedure_acti_reply->qual_cnt))
   SET t.procedure_action_id = add_procedure_acti_request->qual[d.seq].procedure_action_id, t
    .procedure_id = add_procedure_acti_request->qual[d.seq].procedure_id, t.prsnl_id =
    add_procedure_acti_request->qual[d.seq].prsnl_id,
    t.action_type_mean =
    IF ((add_procedure_acti_request->qual[d.seq].action_type_mean > " ")) trim(
      add_procedure_acti_request->qual[d.seq].action_type_mean)
    ELSE " "
    ENDIF
    , t.action_dt_tm = cnvtdatetime(add_procedure_acti_request->qual[d.seq].action_dt_tm), t.updt_cnt
     = 0,
    t.updt_dt_tm = cnvtdatetime(sysdate), t.updt_id = reqinfo->updt_id, t.updt_task = reqinfo->
    updt_task,
    t.updt_applctx = reqinfo->updt_applctx, t.update_encntr_id = contextencntrid
   PLAN (d)
    JOIN (t)
   WITH nocounter, status(add_procedure_acti_reply->qual[d.seq].status)
  ;end insert
 ELSE
  INSERT  FROM procedure_action t,
    (dummyt d  WITH seq = value(add_procedure_acti_reply->qual_cnt))
 ...
 ENDIF
 FOR (i = 1 TO add_procedure_acti_reply->qual_cnt)
   IF ((add_procedure_acti_reply->qual[i].status=0))
    SET failed = insert_error
    SET add_procedure_acti_reply->qual[i].status = insert_error
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
