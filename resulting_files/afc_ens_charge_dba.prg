CREATE PROGRAM afc_ens_charge:dba
 SET afc_ens_charge = "653343.FT.013"
 ...
 DECLARE reprocessind = i2 WITH protect, noconstant(0)
 DECLARE ordphyschangeind = i2 WITH protect, noconstant(0)
 DECLARE rendphyschangeind = i2 WITH protect, noconstant(0)
 DECLARE researchacctchangeind = i2 WITH protect, noconstant(0)
 DECLARE chargeeventid = f8 WITH protect, noconstant(0)
 DECLARE chargeeventactid = f8 WITH protect, noconstant(0)
 FOR (ind = 1 TO request->charge_qual)
   SET lockchargerequest->charge_item_id = request->charge[ind].charge_item_id
   SET lockchargerequest->updt_cnt = validate(request->charge[ind].updt_cnt,0)
   SET lockchargerequest->updt_cnt_ind = validate(request->charge[ind].updt_cnt_ind,0)
   IF (validate(debug,- (1)) > 0)
 ...
 FOR (idx0 = 1 TO request->charge_qual)
   CALL echo(concat(cnvtstring(idx0),": ",request->charge[idx0].action_type))
   SET reprocessind = false
   SET ordphyschangeind = false
   SET rendphyschangeind = false
   SET researchacctchangeind = false
   SET chargeeventid = 0.0
   SET chargeeventactid = 0.0
   IF ((request->charge[idx0].action_type="ADD"))
    SET reprocessind = true
   ELSEIF ((request->charge[idx0].action_type="UPT"))
    SELECT INTO "nl:"
     FROM charge c
     WHERE (c.charge_item_id=request->charge[idx0].charge_item_id)
      AND c.active_ind=1
     DETAIL
      chargeeventid = c.charge_event_id, chargeeventactid = c.charge_event_act_id
      IF ((request->charge[idx0].ord_phys_id != 0)
       AND (c.ord_phys_id != request->charge[idx0].ord_phys_id)
       AND  NOT (c.ord_phys_id=0
       AND (request->charge[idx0].ord_phys_id=- (1))))
       ordphyschangeind = true
      ENDIF
      IF ((request->charge[idx0].verify_phys_id != 0)
       AND (c.verify_phys_id != request->charge[idx0].verify_phys_id)
       AND  NOT (c.verify_phys_id=0
       AND (request->charge[idx0].verify_phys_id=- (1))))
       rendphyschangeind = true
      ENDIF
      IF ((request->charge[idx0].research_acct_id != 0)
       AND (c.research_acct_id != request->charge[idx0].research_acct_id)
       AND  NOT (c.research_acct_id=0
       AND (request->charge[idx0].research_acct_id=- (1))))
       researchacctchangeind = true
      ENDIF
     WITH nocounter
    ;end select
    IF (researchacctchangeind)
     IF ( NOT (modifyresearchacct(request->charge[idx0].research_acct_id,chargeeventid,
      chargeeventactid,request->charge[idx0].suspense_rsn_cd,request->charge[idx0].reason_comment)))
     ...
     ELSE
      ...
      SET stat = alterlist(reply->charge,reply->charge_qual)
      SET reply->charge[reply->charge_qual].charge_item_id = request->charge[idx0].charge_item_id
     ENDIF
     GO TO end_program
    ENDIF
   ENDIF
   IF ( NOT ((request->charge[idx0].perf_loc_cd=0.0)
    AND (request->charge[idx0].ord_phys_id=0.0)
    AND (request->charge[idx0].verify_phys_id=0.0)
    AND (request->charge[idx0].research_acct_id=0.0)
    AND (request->charge[idx0].abn_status_cd=0.0)
    AND (request->charge[idx0].service_dt_tm=0)))
    CALL getoldchargeitemdetails(request->charge[idx0].charge_item_id)
   ENDIF
   CASE (request->charge[idx0].action_type)
    OF "ADD":
     SET action_begin = idx0
     SET action_end = idx0
     EXECUTE afc_add_charge
     SET reply->charge_qual = request->charge_qual
     SET stat = alterlist(reply->charge,reply->charge_qual)
     FOR (lcount = 1 TO reply->charge_qual)
       SET reply->charge[lcount].charge_item_id = request->charge[lcount].charge_item_id
     ENDFOR
     IF (failed != false)
      GO TO check_error
     ENDIF
   ...
   IF ( NOT ((request->charge[idx0].perf_loc_cd=0.0)
    ...
    AND (request->charge[idx0].abn_status_cd=0.0)
    AND (request->charge[idx0].service_dt_tm=0)))
    CALL savechargechangelogattributes(request)
   ELSE
    IF ((((request->charge[idx0].suspense_rsn_cd > 0)) OR (trim(request->charge[idx0].reason_comment)
     != "")) )
     CALL updatechargemodrsncomment(request)
    ENDIF
   ENDIF
   IF (((reprocessind) OR (((ordphyschangeind
    AND checktierforordphys(request->charge[idx0].charge_item_id)) OR (rendphyschangeind
    AND checktierforrendphys(request->charge[idx0].charge_item_id))) )) )
    IF (validate(debug,- (1)) > 0)
    ...
    FOR (idx2 = 1 TO reprocesschargelist->charge_event_qual)
      IF ((reprocesschargelist->charge_event[idx2].charge_event_id=request->charge[idx0].
      charge_event_id))
       IF (validate(debug,- (1)) > 0)
       ...
       charge_qual].charge_item_id = request->charge[idx0].charge_item_id
    ...
    IF (chargeeventfound=false)
     ...
     charge_item_id = request->charge[idx0].charge_item_id
     IF (validate(request->charge[idx0].charge_event_id,- (1)) <= 0)
      SELECT INTO "nl:"
       FROM charge c
       WHERE (c.charge_item_id=request->charge[idx0].charge_item_id)
       DETAIL
        reprocesschargelist->charge_event[reprocesschargelist->charge_event_qual].charge_event_id = c
        .charge_event_id
       WITH nocounter
      ;end select
     ELSE
      SET reprocesschargelist->charge_event[reprocesschargelist->charge_event_qual].charge_event_id
       = request->charge[idx0].charge_event_id
     ENDIF
    ENDIF
    UPDATE  FROM charge c
     ...
     WHERE (c.charge_item_id=request->charge[idx0].charge_item_id)
    ;end update
    IF ( NOT (creditrelatedaddonsbeforereprocessing(request->charge[idx0].charge_item_id,validate(
      request->charge[idx0].reason_comment,""),validate(request->charge[idx0].suspense_rsn_cd,0.0))))
