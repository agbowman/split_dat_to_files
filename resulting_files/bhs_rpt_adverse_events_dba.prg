CREATE PROGRAM bhs_rpt_adverse_events:dba
 PROMPT
 ...
 IF (ms_outdev="OPS")
  SET mf_end_dt_tm = cnvtdatetime(curdate,070000)
 ...
 SELECT INTO "nl:"
  ...
  HEAD e.encntr_id
   ...
   cnvtdatetime(p.deceased_dt_tm), m_enc->qual[m_enc->l_cnt].s_facility = uar_get_code_display(e
    .loc_facility_cd)
   IF ((m_enc->qual[m_enc->l_cnt].f_decease_date > 0.0))
    m_enc->qual[m_enc->l_cnt].l_qual_cnt += 1
   ENDIF
  HEAD ce.event_cd
   m_enc->qual[m_enc->l_cnt].l_evcnt += 1, stat = alterlist(m_enc->qual[m_enc->l_cnt].event_qual,
    m_enc->qual[m_enc->l_cnt].l_evcnt), m_enc->qual[m_enc->l_cnt].event_qual[m_enc->qual[m_enc->l_cnt
   ].l_evcnt].f_event_cd = ce.event_cd
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM encounter e,
 ...
 SELECT INTO "nl:"
  ...
  PLAN (e
   ...
   WHERE elh.encntr_id=e.encntr_id
    ...
    f_code_value)
    AND elh.active_ind=1
    AND elh.beg_effective_dt_tm BETWEEN cnvtdatetime(mf_beg_dt_tm) AND cnvtdatetime(mf_end_dt_tm))
  ORDER BY e.encntr_id
  HEAD e.encntr_id
 ...
 IF (ms_outdev="OPS")
  ...
  SET frec->file_buf = build('"Qual Condition",','"Patient Name",','"MRN",','"Facility",',
   '"Delivering Provider",',
   '"Attending Provider",','"Date,Time of Birth",','"Gestational Age",','"Birth Weight",',
   '"1 min apgar",',
   '"5 min apgar",','"Infant Disposition",','"Type of Delivery"',char(13))
    ...
    IF ((m_enc->qual[ml_idx1].l_dyn_cnt > 0)
     ...
     FOR (ml_idx2 = 1 TO m_enc->qual[ml_idx1].l_dyn_cnt)
       SET frec->file_buf = build('"',trim(m_enc->qual[ml_idx1].s_qual_str,3),'","',m_enc->qual[
        ml_idx1].s_pat_name,'","',
        m_enc->qual[ml_idx1].s_pat_mrn,'","',m_enc->qual[ml_idx1].s_facility)
  ...
  SET ms_tmp = concat("Adverse Events Report: ",format(cnvtdatetime(sysdate),"YYYYMMDDHHMMSS;;q"))
 ...
 ELSE
  SELECT INTO  $OUTDEV
   ...
   mrn = trim(substring(1,15,m_enc->qual[d1.seq].s_pat_mrn)), facility = trim(substring(1,50,m_enc->
     qual[d1.seq].s_facility)), delivering_provider =
   ...
   birth_date = trim(substring(1,20,m_enc->qual[d1.seq].dyn_grp[d2.seq].s_birth_dt)), gestational_age
    = trim(substring(1,15,m_enc->qual[d1.seq].s_gest_age)),
   birth_weigth = trim(substring(1,10,m_enc->qual[d1.seq].dyn_grp[d2.seq].s_birth_weight)),
   1_min_apgar = trim(substring(1,10,m_enc->qual[d1.seq].dyn_grp[d2.seq].s_apgar_1min)), 5_min_apgar
    = trim(substring(1,10,m_enc->qual[d1.seq].dyn_grp[d2.seq].s_apgar_5min)),
