CREATE PROGRAM dm_create_concept_cki_gtmp:dba
 ...
 DECLARE tbl_exists = i4 WITH public, noconstant(0)
 DECLARE tbl_attr_exists = i4 WITH public, noconstant(0)
 SET readme_data->status = "F"
 SET readme_data->message = "FAILED STARTING README dm_create_CONCEPT_CKI_GTMP_tables"
 SET tbl_exists = 0
 SET tbl_attr_exists = 0
 SELECT INTO "nl:"
  FROM dba_tables d
  WHERE d.table_name="CONCEPT_CKI_GTMP"
  DETAIL
   tbl_exists = 1
  WITH nocounter
 ;end select
 IF (tbl_exists=1)
  CALL parser("rdb asis(^ ALTER TABLE CONCEPT_CKI_GTMP DROP PRIMARY KEY CASCADE ^)go")
  CALL parser("rdb asis(^ DROP TABLE CONCEPT_CKI_GTMP CASCADE CONSTRAINTS ^) go")
  SELECT INTO "nl:"
   FROM dtableattr d
   WHERE d.table_name="CONCEPT_CKI_GTMP"
   DETAIL
   ...
   WITH nocounter
  ;end select
  IF (tbl_attr_exists=1)
   DROP TABLE concept_cki_gtmp
  ENDIF
  CALL parser("rdb asis(^ CREATE GLOBAL TEMPORARY TABLE CONCEPT_CKI_GTMP ( ^)")
  CALL parser("asis(^ CONCEPT_CKI  VARCHAR2(255 BYTE) ) ^)")
  CALL parser("asis(^ ON COMMIT DELETE ROWS ^)")
  CALL parser("asis(^ NOCACHE ^) go")
  IF (error(err_msg,0) != 0)
   SET readme_data->status = "F"
   SET readme_data->message = concat("Error creating CONCEPT_CKI_GTMP (rdbms level):",err_msg)
   GO TO exit_script
  ENDIF
  CALL parser("rdb asis(^ ALTER TABLE CONCEPT_CKI_GTMP ADD ( ^)")
  CALL parser("asis(^ CONSTRAINT XPKCONCEPT_CKI_GTMP ^)")
  CALL parser("asis(^ PRIMARY KEY ^)")
  CALL parser("asis(^ (CONCEPT_CKI)) ^) go")
 ELSE
  CALL parser("rdb asis(^ CREATE GLOBAL TEMPORARY TABLE CONCEPT_CKI_GTMP ( ^)")
  CALL parser("asis(^ CONCEPT_CKI  VARCHAR2(255 BYTE) ) ^)")
  CALL parser("asis(^ ON COMMIT DELETE ROWS ^)")
  ...
  CALL parser("asis(^ (CONCEPT_CKI)) ^) go")
 ENDIF
 EXECUTE oragen3 "CONCEPT_CKI_GTMP"
 IF ((dm_err->err_ind=1))
  ...
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error executing oragen3 during creation of CONCEPT_CKI_GTMP:",
   err_msg)
  GO TO exit_script
 ENDIF
 SET readme_data->status = "S"
 IF (tbl_exists=1)
  SET readme_data->message = "Table already exists; table was dropped and re-created."
 ELSE
  SET readme_data->message = "Tables CONCEPT_CKI_GTMP successfully created"
 ENDIF
#exit_script
