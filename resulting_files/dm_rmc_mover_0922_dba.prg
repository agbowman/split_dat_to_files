CREATE PROGRAM dm_rmc_mover_0922:dba
 ...
 DECLARE src_task_num = i4
 DECLARE tar_task_num = i4
 DECLARE cust_min_task = i4
 DECLARE cust_max_task = i4
 DECLARE cust_new_task = i4
 DECLARE src_conv_desc = vc
 DECLARE cust_cnt = i4
 DECLARE evaluate_id = i4
 DECLARE drm_r_tab = vc
 ...
 SET temp_col_cnt = get_col_pos(cust_tab_name,"TASK_NUMBER")
 SET src_task_num = cnvtint(get_value(cust_tab_name,"TASK_NUMBER","FROM"))
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_0922
 ENDIF
 IF (((src_task_num >= 117000
  AND src_task_num <= 118999) OR (src_task_num >= 113100
  AND src_task_num <= 113999)) )
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
   SET cust_cnt = 0
   SET cust_new_task = 0
   IF (src_task_num >= 113100
    AND src_task_num <= 113999)
    SET cust_min_task = 113100
    SET cust_max_task = 113999
   ELSEIF (src_task_num <= 117999)
    SET cust_min_task = 117000
    SET cust_max_task = 117999
   ELSE
    SET cust_min_task = 118000
    SET cust_max_task = 118999
   ENDIF
   SET cust_nomv_chk = select_merge_translate(cnvtstring(src_task_num),cust_tab_name)
   ...
   SELECT INTO "NL:"
    ...
    WHERE (a.description=rs_0922->from_values.description)
     AND a.task_number >= cust_min_task
     AND a.task_number <= cust_max_task
    DETAIL
     cust_new_task = a.task_number, cust_cnt = (cust_cnt+ 1), stat = alterlist(ui_query_eval_rec->
      qual,cust_cnt),
     ui_query_eval_rec->qual[cust_cnt].root_entity_attr = a.task_number
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   IF (evaluate_id < 0)
    SET dm_err->eproc = "Looking for APP_TASK Sequence Lock"
    CALL disp_msg(" ",dm_err->logfile,dm_err->err_ind)
    SELECT INTO "NL:"
     FROM dm_info di
     WHERE di.info_domain="RDDS APP_TASK LOCK"
      AND di.info_name="NEW SEQUENCE LOCK"
    ...
    ELSE
     INSERT  FROM dm_info di
      SET di.info_domain = "RDDS APP_TASK LOCK", di.info_name = "NEW SEQUENCE LOCK", di.info_number
        = cnvtreal(currdbhandle)
      WITH nocounter
     ;end insert
     IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
    IF ((evaluate_id=- (3)))
     SET cust_new_task = cust_min_task
     SET drm_r_tab = cutover_tab_name("APPLICATION_TASK","0922")
     SELECT INTO "NL:"
      a.task_number
      FROM application_task a
      WHERE a.task_number >= cust_min_task
       AND ((a.task_number <= cust_max_task) UNION (
      (SELECT
       ar.task_number
       FROM (parser(drm_r_tab) ar)
       WHERE ar.task_number >= cust_min_task
        AND ((ar.task_number <= cust_max_task) UNION (
       (SELECT
        d.to_value
        FROM dm_merge_translate d
        WHERE d.table_name="APPLICATION_TASK"
         AND (d.env_source_id=dm2_ref_data_doc->env_source_id)
         AND (d.env_target_id=dm2_ref_data_doc->mock_target_id)
         AND d.to_value >= cust_min_task
         AND d.to_value <= cust_max_task))) )))
      ORDER BY 1
      DETAIL
       IF (a.task_number=cust_new_task)
        cust_new_task = (cust_new_task+ 1)
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      ...
      GO TO exit_0922
     ENDIF
     IF (cust_new_task > cust_max_task)
      SET dm2_ref_data_reply->error_ind = 1
      SET dm2_ref_data_reply->error_msg = "APPLICATION_TASK: Error locating open task number"
      GO TO exit_0922
     ELSE
      CALL put_value(cust_tab_name,"TASK_NUMBER",cnvtstring(cust_new_task))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = "APPLICATION_TASK  TASK_NUMBER"
     ENDIF
    ELSEIF ((evaluate_id=- (2)))
     ...
     SELECT INTO "NL:"
      FROM application_task a
      WHERE (a.description=rs_0922->from_values.description)
       AND (a.active_ind=rs_0922->from_values.active_ind)
       AND a.task_number >= cust_min_task
       AND a.task_number <= cust_max_task
      DETAIL
       cust_new_task = a.task_number, cust_cnt = (cust_cnt+ 1), stat = alterlist(ui_query_eval_rec->
   ...
   SET dm2_ref_data_reply->qual[1].from_value = src_task_num
   SET dm2_ref_data_reply->qual[1].to_value = cust_new_task
   CALL echo(build("P COLUMN = ",curmem))
  ENDIF
 ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  CALL put_value(cust_tab_name,"TASK_NUMBER",cnvtstring(src_task_num))
  SET stat = alterlist(dm2_ref_data_reply->qual,1)
  SET dm2_ref_data_reply->qual[1].table_name = cust_tab_name
  SET dm2_ref_data_reply->qual[1].from_value = src_task_num
  SET dm2_ref_data_reply->qual[1].to_value = src_task_num
 ENDIF
 SET idcd_check = 0
 CALL echo("")
 CALL echo("")
 ...
 IF (idcd_check=1)
  ...
  SET tar_task_num = cnvtint(get_value(cust_tab_name,"TASK_NUMBER","TO"))
  SELECT INTO "NL:"
   FROM application_task a
   WHERE a.task_number=tar_task_num
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  ...
  IF (curqual=0)
   INSERT  FROM application_task a
    SET a.description = "RDDS PLACEHOLDER ROW", a.task_number = tar_task_num, a.active_ind = 1,
     a.text = "This row is holding a task number for a row in a temporary RDDS table.", a
     .updt_applctx = 0, a.updt_cnt = 0,
     a.updt_dt_tm = cnvtdatetime(curdate,curtime3), a.updt_id = 0, a.updt_task = 4310001
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
...
#exit_0922
 ...
 FREE SET src_task_num
 FREE SET tar_task_num
 FREE SET cust_min_task
 FREE SET cust_max_task
 FREE SET cust_new_task
END GO
