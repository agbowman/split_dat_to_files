CREATE PROGRAM cern_dcpreqgen05:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   orders->dob = concat(format(datetimezone(p.birth_dt_tm,p.birth_tz),"@SHORTDATE;;Q")," ",
    datetimezonebyindex(p.birth_tz,offset,daylight,7,p.birth_dt_tm)), orders->admit_dt = concat(
    format(datetimezone(e.reg_dt_tm,tz_index),"@SHORTDATE;;Q")," ",datetimezonebyindex(tz_index,
     offset,daylight,7,e.reg_dt_tm)), orders->facility = uar_get_code_display(e.loc_facility_cd),
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   orders->qual[d1.seq].status = uar_get_code_display(o.order_status_cd), orders->qual[d1.seq].
   catalog = uar_get_code_display(o.catalog_type_cd), orders->qual[d1.seq].catalog_type_cd = o
   .catalog_type_cd,
   orders->qual[d1.seq].activity = uar_get_code_display(o.activity_type_cd), orders->qual[d1.seq].
   activity_type_cd = o.activity_type_cd, orders->qual[d1.seq].display_line = o.clinical_display_line,
   orders->qual[d1.seq].order_id = o.order_id, orders->qual[d1.seq].display_ind = 1, orders->qual[d1
   .seq].template_order_flag = o.template_order_flag,
   orders->qual[d1.seq].cs_flag = o.cs_flag, orders->qual[d1.seq].oe_format_id = o.oe_format_id
   IF (substring(245,10,o.clinical_display_line) > "  ")
    orders->qual[d1.seq].clin_line_ind = 1
   ELSE
    orders->qual[d1.seq].clin_line_ind = 0
   ENDIF
   orders->qual[d1.seq].mnemonic = o.hna_order_mnemonic, orders->qual[d1.seq].order_dt = concat(
    format(datetimezone(oa.order_dt_tm,oa.order_tz),"@SHORTDATETIME;;Q")," ",datetimezonebyindex(oa
     .order_tz,offset,daylight,7,oa.order_dt_tm)), orders->qual[d1.seq].comment_ind = o
   .order_comment_ind,
   orders->qual[d1.seq].last_action_seq = o.last_action_sequence, orders->qual[d1.seq].enter_by = pl
   .name_full_formatted, orders->qual[d1.seq].order_dr = pl2.name_full_formatted,
   orders->qual[d1.seq].type = uar_get_code_display(oa.communication_type_cd), orders->qual[d1.seq].
   action_type_cd = oa.action_type_cd, orders->qual[d1.seq].action = uar_get_code_display(oa
    .action_type_cd),
   orders->qual[d1.seq].iv_ind = o.iv_ind
   IF (o.dcp_clin_cat_cd=iv_cd)
    orders->qual[d1.seq].iv_ind = 1
   ENDIF
   IF (o.catalog_type_cd=pharmacy_cd)
    IF (mnem_disp_level="0")
     orders->qual[d1.seq].mnemonic = trim(o.hna_order_mnemonic)
    ENDIF
    IF (mnem_disp_level="1")
     IF (((o.hna_order_mnemonic=o.ordered_as_mnemonic) OR (o.ordered_as_mnemonic=" ")) )
      orders->qual[d1.seq].mnemonic = trim(o.hna_order_mnemonic)
     ELSE
      orders->qual[d1.seq].mnemonic = concat(trim(o.hna_order_mnemonic),"(",trim(o
        .ordered_as_mnemonic),")")
     ENDIF
    ENDIF
    IF (mnem_disp_level="2"
     ...
     IF (o.order_mnemonic != o.ordered_as_mnemonic
      ...
      orders->qual[d1.seq].mnemonic = concat(trim(orders->qual[d1.seq].mnemonic),"(",trim(o
        .order_mnemonic),")")
     ENDIF
    ENDIF
   ENDIF
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   PLAN (d1)
   HEAD REPORT
    reg_dio = "{LPI/8}{CPI/12}{FONT/8}", print_header = "Y", first_page = "Y",
    reg_dio, " ", line1 = fillstring(80,"_")
   HEAD PAGE
    first_page = "Y", print_header = "Y", line9 = fillstring(80,"_"),
    temp = fillstring(50," "), spaces = fillstring(20," "), pri = fillstring(20," "),
    iv = fillstring(20," "), o2 = fillstring(20," "), indications = fillstring(20," "),
    dt_tm_type = 0, start_dt_tm = fillstring(20," "), quantity = fillstring(20," "),
    transport = fillstring(20," "), wgt = fillstring(20," "), wgt_ind = fillstring(20," "),
    "{cpi/8}{pos/220/75}", "ORDER REQUISITION", row + 1,
    "{cpi/13}{pos/240/88}", "Hospital Name", row + 1,
    "{cpi/13}{pos/265/100}", "Division/Address", row + 1,
    ...
    "{cpi/12}{pos/210/130}", "Print Date/Time:  ",
    print_time, row + 1, "{cpi/12}{pos/85/150}{b}",
    line1, "{endb}", row + 1,
    "{cpi/12}{pos/85/170}", "Room: ", orders->room,
    row + 1, "{cpi/12}{pos/85/190}", "Financial #: ",
    orders->fnbr, row + 1, "{cpi/12}{pos/340/190}",
    "Medical Record #: ", orders->mrn, row + 1,
    "{cpi/12}{pos/85/210}", "Patient Name:  ",
    CALL print(trim(orders->name,3)),
    row + 1, "{cpi/12}{pos/340/210}", "DOB: ",
    orders->dob, row + 1, "{cpi/12}{pos/340/225}",
    "Age: ", orders->age, row + 1,
    "{cpi/12}{pos/340/240}", "Sex: ", orders->sex,
    row + 1, "{cpi/12}{pos/85/255}", "Allergies: "
    IF ((allergy->line_cnt > 0))
     allergy->line_qual[1].line, row + 1
    ENDIF
    IF ((allergy->line_cnt > 1))
     "{cpi/12}{pos/110/255}", allergy->line_qual[2].line, row + 1
    ENDIF
    "{cpi/12}{pos/85/280}", "Chief Complaint:  ", orders->admit_diagnosis,
    row + 1, "{cpi/12}{pos/85/300}{b}", line1,
    "{endb}", row + 1, xcol = 85,
    ycol = 300, count = 0, com_cnt = 0,
    fsub = 0
   DETAIL
    FOR (vv = 1 TO value(order_cnt))
      IF ((orders->qual[vv].display_ind=1))
       IF (first_page="N")
        count = 0, xcol = 85, ycol = 300,
        BREAK
       ENDIF
       first_page = "N", reg_dio, "  ",
       row + 1
       IF ((orders->qual[vv].special_action_ind=1)
        AND print_header="Y")
        "{cpi/10}{pos/265/124}{b}", "***", orders->qual[vv].action,
        "***", row + 1, print_header = "N"
       ENDIF
       xcol = 85, ycol = (ycol+ 20),
       CALL print(calcpos(xcol,ycol)),
       "{cpi/12}", "{b}", "Procedure:   ",
       orders->qual[vv].mnemonic, "{endb}", row + 1,
       xcol = 85, ycol = (ycol+ 20),
       CALL print(calcpos(xcol,ycol)),
       "{cpi/12}", "Scheduled Start Date/Time: ", start_dt_tm,
       row + 1, xcol = 340
       IF (quantity > "    ")
        CALL print(calcpos(xcol,ycol)), "{cpi/12}", "Quantity:  ",
        quantity, row + 1
       ENDIF
       xcol = 85, ycol = (ycol+ 20),
       CALL print(calcpos(xcol,ycol))
       IF (ycol >= 710)
        xcol = 85, ycol = 300, BREAK
        IF ((orders->qual[vv].special_action_ind=1)
         AND print_header="Y")
         "{cpi/10}{pos/265/124}{b}", "***", orders->qual[vv].action,
         "***", row + 1, print_header = "N"
       ...
       IF ((orders->qual[vv].comment_ind=1)
        ...
        IF ((orders->qual[vv].com_ln_cnt > 5))
         ocnt = 5
        ELSE
        ...
        FOR (com_cnt = 1 TO ocnt)
          IF (com_cnt=1)
           xcol = 85, ycol = (ycol+ 20),
           CALL print(calcpos(xcol,ycol)),
           "Order Comments: ", xcol = 175
          ELSE
           xcol = 110
          ENDIF
          CALL print(calcpos(xcol,ycol)), orders->qual[vv].com_ln_qual[com_cnt].com_line, row + 1,
          ycol = (ycol+ 12)
        ENDFOR
       ENDIF
       xcol = 85, ycol = (ycol+ 20),
       CALL print(calcpos(xcol,ycol)),
       "{CPI/12}", "Ordering Physician:  ", orders->qual[vv].order_dr,
       row + 2, xcol = 340,
       CALL print(calcpos(xcol,ycol)),
       "{CPI/12}", "Attending Physician:  ", orders->attending,
       row + 1, xcol = 85, ycol = (ycol+ 20),
       CALL print(calcpos(xcol,ycol)), "{cpi/12}", "Entered by: ",
       orders->qual[vv].enter_by, row + 1, xcol = 340,
       CALL print(calcpos(xcol,ycol)), "{CPI/12}", "Ordered Date/Time: ",
       orders->qual[vv].order_dt, row + 1
       IF (ycol >= 710)
        xcol = 85, ycol = 300, BREAK
        IF ((orders->qual[vv].special_action_ind=1)
         AND print_header="Y")
       ...
       FOR (fsub = 1 TO 31)
         FOR (ww = 1 TO orders->qual[vv].d_cnt)
           IF ((((orders->qual[vv].d_qual[ww].group_seq=fsub)) OR (fsub=31
            ...
            orders->qual[vv].d_qual[ww].print_ind = 1, "{cpi/12}{b}",
            CALL print(calcpos(xcol,ycol)),
            CALL print(orders->qual[vv].d_qual[ww].label_text), ":  ",
            CALL print(orders->qual[vv].d_qual[ww].value),
            row + 1, xcol = 85, ycol = (ycol+ 12)
            IF (ycol >= 710)
             xcol = 85, ycol = (ycol+ 20),
             CALL print(calcpos(xcol,ycol)),
             "{cpi/12}", "Ordering Physician:  ", orders->qual[vv].order_dr,
             row + 1, xcol = 85, ycol = (ycol+ 20),
             CALL print(calcpos(xcol,ycol)), "{cpi/12}{pos/85/460}", "Entered by: ",
             orders->qual[vv].enter_by, row + 1, xcol = 340,
             CALL print(calcpos(xcol,ycol)), "{cpi/12}", "Ordered Date/Time: ",
             orders->qual[vv].order_dt, row + 1, BREAK
             IF ((orders->qual[vv].special_action_ind=1)
              AND print_header="Y")
              "{cpi/10}{pos/265/124}{b}", "***", orders->qual[vv].action,
              "***", row + 1, print_header = "N"
             ...
             row + 1, ycol = (ycol+ 20),
             CALL print(calcpos(xcol,ycol)),
             "{cpi/12}", "{b}", "Item or Procedure:   ",
             orders->qual[vv].mnemonic, "{endb}", row + 1,
             ycol = (ycol+ 20),
             CALL print(calcpos(xcol,ycol)), "{cpi/12}",
             "Scheduled Start Date/Time: ", start_dt_tm, row + 1,
             xcol = 340
             IF (quantity > "    ")
              CALL print(calcpos(xcol,ycol)), "{cpi/12}", "Quantity:  ",
              quantity, row + 1
             ENDIF
             ycol = (ycol+ 20),
             CALL print(calcpos(xcol,ycol)), ycol = (ycol+ 20)
             IF ((orders->qual[vv].comment_ind=1)
              AND (orders->qual[vv].com_ln_cnt > 0))
              IF ((orders->qual[vv].com_ln_cnt > 5))
               ocnt = 5
              ...
              FOR (com_cnt = 1 TO ocnt)
                ...
                ELSE
                 xcol = 85
                ENDIF
                CALL print(calcpos(xcol,ycol)), orders->qual[vv].com_ln_qual[com_cnt].com_line, row
                 + 1,
                ycol = (ycol+ 12)
              ENDFOR
             ENDIF
      ...
      ENDIF
    ENDFOR
    "{font/8}{cpi/12}{pos/50/720}", "dcpreqgen05"
