CREATE PROGRAM afc_get_child_inst:dba
 DECLARE versionnbr = vc
 SET versionnbr = "003"
 CALL echo(build("AFC_GET_CHILD_INST Version: ",versionnbr))
 RECORD tempreply(
   1 qual_cnt = i4
   1 qual[*]
 ...
 DECLARE repcount = i4 WITH protect, noconstant(0)
 ...
 DECLARE occurances = i4 WITH protect, noconstant(0)
 ...
 IF ( NOT (getlogicaldomain(ld_concept_organization,curlogicaldomainid)))
  GO TO end_program
 ENDIF
 IF ((request->cdf_meaning="INSTITUTION"))
  IF ((request->active_ind=0))
   CALL getallcodevalues(request->cdf_meaning)
   IF ((csreply->status_data.status != "S"))
    SET reply->status_data.status = csreply->status_data.status
    GO TO end_program
   ENDIF
  ELSE
   ...
   SET stat = memrealloc(code_list,occurances,"f8")
   CALL uar_get_code_list_by_meaning(code_set,nullterm(cdf_meaning),start_index,occurances,
    total_remaining,
    code_list)
  ENDIF
  SELECT INTO "nl:"
   FROM service_resource sr,
    organization o
   PLAN (sr
    WHERE expand(num,1,size(code_list,5),sr.service_resource_cd,code_list[num])
     AND sr.active_ind=1)
    JOIN (o
   ...
   DETAIL
    count2 += 1
    IF (mod(count2,10)=1)
     stat = alterlist(tempreply->qual,(count2+ 9))
    ENDIF
    tempreply->qual[count2].cdf_meaning = uar_get_code_meaning(sr.service_resource_cd), tempreply->
    qual[count2].sec_display = uar_get_code_display(sr.service_resource_cd), tempreply->qual[count2].
    parent_service_resource_cd = sr.service_resource_cd
   WITH nocounter, expand = 1
  ;end select
  SET tempreply->qual_cnt = count2
  SET stat = alterlist(tempreply->qual,count2)
  FOR (count1 = 1 TO tempreply->qual_cnt)
   SET m_nressecuarstatus = uar_srsprsnlhasaccess(reqinfo->updt_id,reqinfo->position_cd,tempreply->
    qual[count1].parent_service_resource_cd)
   IF (m_nressecuarstatus=nres_sec_passed)
    SET repcount += 1
    SET stat = alterlist(reply->qual,repcount)
    SET reply->qual_cnt = repcount
    SET reply->qual[repcount].parent_service_resource_cd = tempreply->qual[count1].
    parent_service_resource_cd
    SET reply->qual[repcount].sec_display = tempreply->qual[count1].sec_display
    SET reply->qual[repcount].cdf_meaning = tempreply->qual[count1].cdf_meaning
   ENDIF
  ENDFOR
  IF (curqual > 0)
   SET reply->status_data.status = "S"
  ENDIF
 ELSEIF ((request->cdf_meaning IN ("DEPARTMENT", "SECTION", "SUBSECTION", "SURGAREA")))
  IF ((request->active_ind=0))
   CALL getallcodevalues(request->cdf_meaning)
   IF ((csreply->status_data.status != "S"))
    SET reply->status_data.status = csreply->status_data.status
  ...
  ENDIF
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(size(code_list,5))),
    resource_group r,
    service_resource sr,
    organization o
   PLAN (d)
    JOIN (r
    WHERE (r.parent_service_resource_cd=request->parent_service_resource_cd)
     AND (r.child_service_resource_cd=code_list[d.seq])
     AND r.active_ind=1)
    JOIN (sr
    WHERE sr.service_resource_cd=r.child_service_resource_cd
     AND sr.active_ind=1)
    JOIN (o
    WHERE o.organization_id=sr.organization_id
     AND o.logical_domain_id=curlogicaldomainid)
 ...
 ELSE
  IF ((request->active_ind=1))
   SET level5parser = "c.active_ind = 1"
  ELSE
   SET level5parser = "1=1"
  ENDIF
  SELECT INTO "nl:"
   ...
   PLAN (r
    ...
    WHERE c.code_set=221
     ...
     AND parser(level5parser))
    JOIN (sr
    WHERE sr.service_resource_cd=c.code_value
     AND sr.active_ind=1)
    JOIN (o
    WHERE o.organization_id=sr.organization_id
     AND o.logical_domain_id=curlogicaldomainid)
  ...
  ENDIF
 ENDIF
 SUBROUTINE (getallcodevalues(cdfmeaning=c12) =null)
   SET csrequest->code_set = 221
   EXECUTE cs_get_codes  WITH replace("REQUEST",csrequest), replace("REPLY",csreply)
   IF ((csreply->status_data.status="S"))
    SET stat = memrealloc(code_list,size(csreply->qual,5),"F8")
    SELECT INTO "nl:"
     FROM (dummyt d1  WITH seq = value(size(csreply->qual,5)))
     WHERE (csreply->qual[d1.seq].cdf_meaning=request->cdf_meaning)
     DETAIL
      listsize += 1, code_list[listsize] = csreply->qual[d1.seq].code_value
     WITH nocounter
    ;end select
    SET stat = memrealloc(code_list,listsize,"F8")
   ENDIF
 END ;Subroutine
#end_program
