CREATE PROGRAM dm2_shc_cpm_settings:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dscs_err_status = c1 WITH protect, noconstant(" ")
 DECLARE dscs_logging = vc WITH protect, noconstant(" ")
 DECLARE dscs_state = vc WITH protect, noconstant(" ")
 DECLARE dscs_timing = vc WITH protect, noconstant(" ")
 IF (check_logfile("DM2_SHC_CPM_SETTINGS",".log","DM2_SHC_CPM_SETTINGS LOG FILE")=0)
  SET dscs_err_status = "F"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "System Health Check working on CPM Settings..."
 CALL disp_msg("",dm_err->logfile,0)
 ...
 ELSE
  ...
  ENDIF
 ENDIF
 SET dscs_logging = logical("cpm_global_logging")
 IF (dscs_logging > " ")
  SET dscs_err_status = "G"
 ELSE
  SET dscs_err_status = "R"
 ENDIF
 IF (dshc_add_check_to_rs("CPM_GLOBAL_LOGGING","CPM_GLOBAL_LOGGING",0,dscs_err_status,
  dshc_ctf_millennium)=0)
  SET dscs_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dscs_err_status="R")
  ...
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "CPM_GLOBAL_LOGGING"
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Setting Value:"
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].str_value = dscs_logging
  SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
   "If CPM_GLOBAL_LOGGING is not set or is set to ON, then extra logging may be generated ",
   "into CMB_TEMP and/or CER_LOG.  <SPACE>This parameter should be reviewed periodicaly since it ",
   "is sometimes set during troubleshooting.")
 ENDIF
 SET dscs_state = logical("cpm_global_state")
 IF (dscs_state > " ")
  SET dscs_err_status = "R"
 ELSE
  SET dscs_err_status = "G"
 ENDIF
 IF (dshc_add_check_to_rs("CPM_GLOBAL_STATE","CPM_GLOBAL_STATE",0,dscs_err_status,dshc_ctf_millennium
  )=0)
  SET dscs_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dscs_err_status="R")
  ...
  SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
  SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "CPM_GLOBAL_STATE"
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Setting Defined:"
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].str_value = dscs_state
  SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
   "If CPM_GLOBAL_STATE is defined, then servers have extra processing overhead for ",
   "tracking server transaction processing.  Features in SPSMON can replace most of this ",
   "debugging functionality.  <SPACE>This parameter should be reviewed periodically since it ",
   "is sometimes set during troubleshooting.")
 ENDIF
 SET dscs_timing = logical("cpm_global_timing")
 IF (dscs_timing > " ")
  SET dscs_err_status = "R"
 ELSE
  SET dscs_err_status = "G"
 ENDIF
 IF (dshc_add_check_to_rs("CPM_GLOBAL_TIMING","CPM_GLOBAL_TIMING",0,dscs_err_status,
  dshc_ctf_millennium)=0)
  SET dscs_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dscs_err_status="G")
  GO TO exit_script
 ENDIF
 SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
 SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
 SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "CPM_GLOBAL_TIMING"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Setting Defined:"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].str_value = dscs_timing
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "If CPM_GLOBAL_TIMING is defined, then servers will post timing messages into msgview ",
  "for every transaction processed.  Features in SPSMON can replace most of this timing functionality. ",
  "<SPACE>This parameter should be reviewed periodically since it is sometimes set during troubleshooting."
  )
#exit_script
 ...
 ENDIF
 IF (dscs_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_CPM_SETTINGS WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_CPM_SETTINGS WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_CPM_SETTINGS COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_CPM_SETTINGS")
END GO
