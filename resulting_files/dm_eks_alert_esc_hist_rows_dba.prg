CREATE PROGRAM dm_eks_alert_esc_hist_rows:dba
 ...
 SET reply->table_name = "EKS_ALERT_ESC_HIST"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(hist.eks_alert_esc_hist_id)
   FROM eks_alert_esc_hist hist
   WHERE hist.eks_alert_esc_hist_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(hist.eks_alert_esc_hist_id)
  FROM eks_alert_esc_hist hist
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    hist.rowid
    FROM eks_alert_esc_hist hist
    WHERE parser(sbr_getrowidnotexists("hist.eks_alert_esc_hist_id between curMinID and curMaxID",
      "hist"))
     AND hist.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = hist.rowid
    WITH nocounter, maxqual(hist,value(rowsleft))
   ;end select
