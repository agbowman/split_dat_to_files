CREATE PROGRAM bbd_rpt_don_procedures
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 DECLARE script_name = c22 WITH protect, constant("bbd_rpt_don_procedures")
 DECLARE success = c7 WITH protect, constant("SUCCESS")
 DECLARE tempdef = c7 WITH protect, constant("TEMPDEF")
 DECLARE permdef = c7 WITH protect, constant("PERMDEF")
 DECLARE failed = c6 WITH protect, constant("FAILED")
 ...
 SET captions->rpt_title = uar_i18ngetmessage(i18nhandle,"rpt_title","SUMMARY OF DONATION PROCEDURES"
  )
 ...
 SET captions->rpt_reg = uar_i18ngetmessage(i18nhandle,"rpt_reg","Registered")
 SET captions->rpt_success = uar_i18ngetmessage(i18nhandle,"rpt_success","Successful")
 SET captions->rpt_temp = uar_i18ngetmessage(i18nhandle,"rpt_temp","Temporary")
 SET captions->rpt_perm = uar_i18ngetmessage(i18nhandle,"rpt_perm","Permanent")
 SET captions->rpt_fail = uar_i18ngetmessage(i18nhandle,"rpt_fail","Failed")
 SET captions->rpt_donations = uar_i18ngetmessage(i18nhandle,"rpt_donations","Donations")
 SET captions->rpt_deferrals = uar_i18ngetmessage(i18nhandle,"rpt_deferrals","Deferrals")
 SET captions->rpt_procedures = uar_i18ngetmessage(i18nhandle,"rpt_procedures","Procedures")
 SET captions->rpt_total = uar_i18ngetmessage(i18nhandle,"rpt_total","Total")
 SET captions->rpt_reasons = uar_i18ngetmessage(i18nhandle,"rpt_reasons","Reasons Summary")
 ...
 SET modify = predeclare
 SET reply->status_data.status = "F"
 EXECUTE cpm_create_file_name_logical "bbd_donation_procs", "txt", "x"
 SELECT
  ...
  bd_active_status_disp = uar_get_code_display(bd.active_status_cd), bdr_reason_disp =
  uar_get_code_display(bdr.reason_cd), drawn_date = cnvtdate(b.drawn_dt_tm),
  b_procedure_disp = substring(1,20,uar_get_code_display(b.procedure_cd)), e_location_disp =
  uar_get_code_display(e.loc_facility_cd)
  FROM bbd_donation_results b,
   bbd_donor_eligibility bd,
  ...
  PLAN (b
   ...
   JOIN (bd
   WHERE bd.contact_id=b.contact_id)
   JOIN (bdr
   WHERE bdr.eligibility_id=outerjoin(bd.eligibility_id))
  ORDER BY e.loc_facility_cd, drawn_date, b_procedure_disp
  HEAD REPORT
  ...
  HEAD PAGE
   ...
   row + 2
   IF (new_sum_page != 1)
    col 0, captions->rpt_don_loc, col 20,
    e_location_disp, row + 2, col 0,
    captions->rpt_don_date, col 22, captions->rpt_reg,
    col 34, captions->rpt_success, col 46,
    captions->rpt_temp, col 58, captions->rpt_perm,
    col 70, captions->rpt_fail, row + 1,
    col 0, captions->rpt_don_proc, col 22,
    captions->rpt_donors, col 34, captions->rpt_donations,
    col 46, captions->rpt_deferrals, col 58,
    captions->rpt_deferrals, col 70, captions->rpt_procedures,
    row + 1, col 0, line20,
    col 22, line10, col 34,
    line10, col 46, line10,
    col 58, line10, col 70,
    line10, row + 1
   ENDIF
  ...
  HEAD b_procedure_disp
   row + 0
  DETAIL
   IF (cnvtupper(uar_get_code_meaning(b.outcome_cd)) != success)
    num_recs = size(temp->outcomes,5), pos = locateval(index,1,num_recs,b.outcome_cd,temp->outcomes[
     index].code_value)
    IF (pos=0)
     pos = (num_recs+ 1), stat = alterlist(temp->outcomes,pos), temp->outcomes[pos].code_value = b
     .outcome_cd
     CASE (cnvtupper(uar_get_code_meaning(b.outcome_cd)))
      OF tempdef:
       temp->outcomes[pos].display = substring(1,25,concat(captions->rpt_temp," ",captions->
         rpt_deferrals))
      OF permdef:
       temp->outcomes[pos].display = substring(1,25,concat(captions->rpt_perm," ",captions->
         rpt_deferrals))
      OF failed:
       temp->outcomes[pos].display = substring(1,25,concat(captions->rpt_fail," ",captions->
         rpt_procedures))
     ENDCASE
    ENDIF
    num_recs = size(temp->outcomes[pos].reasons,5), r_pos = locateval(index,1,num_recs,bdr.reason_cd,
     temp->outcomes[pos].reasons[index].code_value)
    IF (r_pos=0)
     r_pos = (num_recs+ 1), stat = alterlist(temp->outcomes[pos].reasons,r_pos), temp->outcomes[pos].
     reasons[r_pos].code_value = bdr.reason_cd,
     temp->outcomes[pos].reasons[r_pos].display = bdr_reason_disp
    ENDIF
    temp->outcomes[pos].reasons[r_pos].count = (temp->outcomes[pos].reasons[r_pos].count+ 1)
   ENDIF
  FOOT  b_procedure_disp
   IF (row > 54)
    BREAK
   ENDIF
   r_donors = count(b.contact_id), s_dons = count(b.contact_id
    WHERE cnvtupper(uar_get_code_meaning(b.outcome_cd))=success), t_dons = count(b.contact_id
    WHERE cnvtupper(uar_get_code_meaning(b.outcome_cd))=tempdef),
   p_dons = count(b.contact_id
    WHERE cnvtupper(uar_get_code_meaning(b.outcome_cd))=permdef), f_dons = count(b.contact_id
    WHERE cnvtupper(uar_get_code_meaning(b.outcome_cd))=failed), col 0,
   b_procedure_disp";R", outstr = cnvtstring(r_donors), col 26,
   outstr
   IF (s_dons > 0)
    outstr = cnvtstring(s_dons), col 38, outstr
   ENDIF
   IF (t_dons > 0)
    outstr = cnvtstring(t_dons), col 50, outstr
   ENDIF
   IF (p_dons > 0)
    outstr = cnvtstring(p_dons), col 62, outstr
   ENDIF
   IF (f_dons > 0)
    outstr = cnvtstring(f_dons), col 74, outstr
   ENDIF
   row + 1
  FOOT  drawn_date
   row + 0
  FOOT  e.loc_facility_cd
   ...
   ENDIF
   r_donors = count(b.contact_id), s_dons = count(b.contact_id
    WHERE uar_get_code_meaning(b.outcome_cd)=success), t_dons = count(b.contact_id
    WHERE uar_get_code_meaning(b.outcome_cd)=tempdef),
   p_dons = count(b.contact_id
    WHERE uar_get_code_meaning(b.outcome_cd)=permdef), f_dons = count(b.contact_id
    WHERE uar_get_code_meaning(b.outcome_cd)=failed), col 0,
   line20, col 22, line10,
   col 34, line10, col 46,
   line10, col 58, line10,
   col 70, line10, row + 1,
   col 15, captions->rpt_total, outstr = cnvtstring(r_donors),
   col 26, outstr, outstr = cnvtstring(s_dons),
   col 38, outstr, outstr = cnvtstring(t_dons),
   col 50, outstr, outstr = cnvtstring(p_dons),
   col 62, outstr, outstr = cnvtstring(f_dons),
   col 74, outstr, row + 1
  FOOT PAGE
  ...
  FOOT REPORT
   cnt = 0
   FOR (i = 1 TO size(temp->outcomes,5))
     FOR (j = 1 TO size(temp->outcomes[i].reasons,5))
       cnt = (cnt+ temp->outcomes[i].reasons[j].count)
     ENDFOR
   ENDFOR
   IF (cnt > 0)
    ...
    FOR (i = 1 TO size(temp->outcomes,5))
      ...
      FOR (j = 1 TO size(temp->outcomes[i].reasons,5))
        IF (row > 56)
         new_outcome = 1, new_sum_page = 1, BREAK
        ENDIF
        IF (new_sum_page=1)
         col 0, captions->rpt_reasons, col 27,
         captions->rpt_total, row + 1, col 0,
         line25, col 27, line10,
         row + 1, new_sum_page = 0
        ENDIF
        IF (new_outcome=1)
         col 0, temp->outcomes[i].display, row + 1,
         new_outcome = 0
        ENDIF
        col 0, temp->outcomes[i].reasons[j].display";R", outstr = cnvtstring(temp->outcomes[i].
         reasons[j].count),
        col 30, outstr, row + 1
      ENDFOR
      row + 1
 ...
 IF (error_check != 0)
  SET reply->status_data.status = "F"
  CALL subevent_add("Select Donation Procedures","F","bbd_rpt_don_procedures.prg",errmsg)
  GO TO exit_script
 ENDIF
#set_status
 IF (curqual=0)
