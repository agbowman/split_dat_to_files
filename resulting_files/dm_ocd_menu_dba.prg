CREATE PROGRAM dm_ocd_menu:dba
 PAINT
 SET width = 132
 FREE DEFINE ocd_op
 RECORD ocd_op(
...
#menu_screen
 ...
 END ;Subroutine
 DECLARE env_id = f8
 DECLARE ocd_number = i4
 DECLARE dom_rerun = i4
 DECLARE dom_msg = vc
 DECLARE readme_num_hold = i4
 DECLARE dom_logfile_prefix = vc
 SET dom_logfile_prefix = "dm_ocd_menu"
 SET call_script = "DM_OCD_MENU"
 FREE SET mode_str
 FREE SET prev_mode
 FREE SET sdate
 SET migr_ind = 0
 IF (check_logfile(dom_logfile_prefix,".log","Mass Move Log")=0)
  GO TO end_script
 ENDIF
 ...
 IF ((dm_err->debug_flag > 1))
  SET message = nowindow
  CALL echo("After turning off triggers...")
  SELECT INTO noforms
 ...
 IF (curqual=0
  ...
  SET message = nowindow
  CALL echo("*******************************************************************************")
  CALL echo(concat("Current user, ",curuser,", does not have CCL DBA privileges required",
    " to run ocd_incl_schema.  Please contact your system administrator."))
  CALL echo("*******************************************************************************")
  GO TO end_script
 ENDIF
 FREE RECORD doc
 RECORD doc(
   ...
   1 prd_area_cnt = i4
   1 lst[*]
     2 product_area_number = i4
     2 product_area_name = vc
     2 ocd_cnt = i4
     2 olst[*]
       3 ocd_nbr = i4
   1 red_cnt = i4
   1 rlst[*]
     2 ocd_nbr = i4
     2 redundancy_ind = i2
     2 highest_ocd_nbr = i4
 )
 SET redundancy_rpt_file = "dm_ocd_redundancy_rpt.log"
 SET max_dsize = 0
 SET maxcolsize = 132
 SET header_str = fillstring(80,"-")
 SET redundancy_ind = 0
 SET message = window
 CALL clear(1,1)
 SET width = 132
 CALL box(1,1,5,132)
 CALL text(3,44,"***  DM OCD MENU  ***")
 CALL text(6,8,
  "This program is used during OCD migration.  Migration modes and reporting options are:")
 CALL text(8,8,"0) Initialize OCD Migration (INIT)")
 CALL text(9,8,"--------------------------------------------------------------------")
 CALL text(10,8,"1) CS mode (code sets)")
 CALL text(11,8,"2) PRE-UTS mode (readmes)")
 CALL text(12,8,"---- UTS (uptime schema)")
 CALL text(13,8,"3) ATR mode (ATRs)")
 CALL text(14,8,"4) POST-UTS mode (readmes)")
 CALL text(15,8,"5) PRE-DTS mode (readmes)")
 CALL text(16,8,"6) POST-DTS mode (readmes)")
 CALL text(17,8,"7) POST-INST mode (readmes)")
 CALL text(18,8,"--------------------------------------------------------------------")
 CALL text(19,8,"8) Current Migration Status Report")
 CALL text(20,8,"9) Readme Estimator Report (reports readme execution time estimates)")
 CALL text(21,8,"10) Run Concurrent Readmes")
 CALL text(23,8,">>> Please choose a number from the menu above (99 to exit): ")
 CALL accept(23,69,"9(2)")
 SET menu_mode = curaccept
 CASE (menu_mode)
  OF 0:
   SET mode_str = "INIT"
   SET prev_mode = ""
  ...
  OF 8:
   ...
   SET prev_mode = ""
  OF 9:
   EXECUTE dm_readme_estimate_log
   GO TO end_script
  OF 10:
   SET message = nowindow
   EXECUTE dm_ocd_install_runner
   GO TO end_script
  ELSE
   GO TO end_script
 ENDCASE
 CALL clear(1,1)
 CALL text(24,02,"Enter source environment name. Examples: PROD, CERT, TRAIN, etc. ")
 SET help =
 SELECT INTO "nl:"
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 SET doc->target_env_id = cur_env_id
 SET env_id = cur_env_id
 SELECT INTO "nl:"
  e.*
  FROM dm_environment e
  WHERE (e.environment_id=doc->target_env_id)
  DETAIL
   doc->target_env_name = e.environment_name
  WITH nocounter
 ;end select
 SET doc->source_ocd_cnt = 0
 IF (mode_str="INIT")
  SET message = nowindow
  SELECT INTO "nl:"
   daef.alpha_feature_nbr
   FROM dm_alpha_features_env daef
   WHERE daef.alpha_feature_nbr > 0
    AND (daef.environment_id=doc->target_env_id)
    AND daef.curr_migration_ind=1
   WITH nocounter
  ;end select
  IF (curqual > 0)
   SELECT
    ...
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     row + 1, "DM_OCD_MENU has detected that a migration to the ", doc->target_env_name,
     " environment", row + 1,
     "has previously been executed.  The following screen will contain a list of",
     row + 1,
     "the OCDs already migrated, when they were migrated, and in what mode.  After viewing the", row
      + 1,
     "list, you will be asked whether you want to clear this information or not.", row + 2,
     "To CLEAR this information (Yes):",
     row + 1,
     "      -> If you are just beginning a new migration and the information shown is from a", row +
     1,
     "         previous migration or contains any mode other than INIT, you will want to clear it.",
     row + 2, "To CLEAR this information (No):",
     row + 1, "      -> If you have already run DM_OCD_MENU in INIT mode for the current migration,",
     row + 1,
     "         and want to migrate this set of OCDs, you should choose NOT to clear the history."
    WITH nocounter
   ;end select
   SELECT
    ocd_number = e.alpha_feature_nbr, env_name = d.environment_name, e.inst_mode,
    e.start_dt_tm, e.end_dt_tm, e.status
    FROM dm_alpha_features_env e,
     dm_environment d
    WHERE (e.environment_id=doc->target_env_id)
     AND e.alpha_feature_nbr > 0
     AND e.curr_migration_ind=1
     AND e.environment_id=d.environment_id
    ORDER BY e.start_dt_tm
   ;end select
   SET message = window
   CALL clear(1,1)
   CALL text(12,02,"Do you want to clear the migration data <Y> or not <N>? Enter <Q> to quit.")
   CALL accept(12,80,"P(1);CU")
   SET dom_answer = curaccept
   IF (dom_answer="Y")
    DELETE  FROM dm_alpha_features_env daef
     WHERE daef.alpha_feature_nbr > 0
      AND (daef.environment_id=doc->target_env_id)
      AND daef.curr_migration_ind=1
      AND daef.inst_mode=mode_str
     WITH nocounter
    ;end delete
    COMMIT
    UPDATE  FROM dm_alpha_features_env daef
     SET daef.curr_migration_ind = 0
     WHERE daef.alpha_feature_nbr > 0
      AND (daef.environment_id=doc->target_env_id)
     WITH nocounter
    ;end update
   ...
   ELSEIF (dom_answer="N")
    SELECT INTO "nl:"
     ...
     WHERE daef.alpha_feature_nbr > 0
      ...
      AND daef.curr_migration_ind=1
      AND daef.inst_mode=mode_str
     ORDER BY daef.start_dt_tm
     DETAIL
      doc->source_ocd_cnt = (doc->source_ocd_cnt+ 1), stat = alterlist(doc->qual,doc->source_ocd_cnt),
      doc->qual[doc->source_ocd_cnt].ocd_nbr = daef.alpha_feature_nbr
     WITH nocounter
    ;end select
   ELSE
    GO TO end_script
  ...
  IF (dom_answer != "N")
   SELECT INTO "nl:"
    FROM dm_alpha_features_env daef
    WHERE (daef.environment_id=doc->source_env_id)
     AND daef.alpha_feature_nbr > 0
     AND  NOT ( EXISTS (
    (SELECT
     "x"
     FROM dm_alpha_features_env daef2
     WHERE daef.alpha_feature_nbr=daef2.alpha_feature_nbr
      AND (daef2.environment_id=doc->target_env_id))))
    ORDER BY daef.start_dt_tm
    DETAIL
     doc->source_ocd_cnt = (doc->source_ocd_cnt+ 1), stat = alterlist(doc->qual,doc->source_ocd_cnt),
     doc->qual[doc->source_ocd_cnt].ocd_nbr = daef.alpha_feature_nbr
   ...
   IF (curqual > 0)
    CALL clear(1,1)
    EXECUTE dm_ocd_remove_redundancy
   ENDIF
  ENDIF
  IF ((doc->source_ocd_cnt > 0))
   CALL clear(1,1)
   SELECT
    d.*
    FROM (dummyt d  WITH seq = value(doc->source_ocd_cnt))
    WHERE d.seq > 0
    HEAD REPORT
     line1 = fillstring(120," "), line2 = fillstring(120," "), row + 1,
     line1 = concat("The following OCDs will be marked for migration from the ",doc->source_env_name,
      " environment"), col 5, line1,
     row + 1, line2 = concat("to the ",doc->target_env_name," environment."), col 5,
     line2, row + 2
    DETAIL
     col + 3, doc->qual[d.seq].ocd_nbr
     IF (mod(d.seq,8)=0)
      row + 1
     ENDIF
    FOOT REPORT
     row + 1
    WITH nocounter, maxrow = 56, maxcol = 132,
     formfeed = none
   ;end select
   CALL clear(1,1)
   CALL text(1,1,concat("The following OCDs will be marked for migration from the ",doc->
     source_env_name," environment"))
   CALL text(2,1,concat("to the ",doc->target_env_name," environment."))
   CALL text(3,1,concat("The number of ocds will be migrated is ",cnvtstring(doc->source_ocd_cnt)))
   CALL text(20,1,"Please enter <G>o to proceed or <Q>uit to exit.")
   CALL accept(20,50,"A;CU","Q"
    WHERE curaccept IN ("Q", "G"))
   ...
   CALL clear(1,1)
   IF (redundancy_ind=1)
    CALL echo("Insert Redundant OCDs with Target env_id in dm_alpha_features_env table ...")
    INSERT  FROM dm_alpha_features_env defa,
      (dummyt d  WITH seq = value(doc->red_cnt))
     SET defa.seq = 1, defa.status = concat("Redundant OCD - Replaced by OCD ",cnvtstring(doc->rlst[d
        .seq].highest_ocd_nbr)), defa.start_dt_tm = cnvtdatetime(curdate,curtime3),
      defa.end_dt_tm = cnvtdatetime(curdate,curtime3), defa.environment_id = doc->target_env_id, defa
      .alpha_feature_nbr = doc->rlst[d.seq].ocd_nbr,
      defa.inst_mode = null, defa.calling_script = "DM_OCD_MENU", defa.curr_migration_ind = 0
     PLAN (d
      WHERE (doc->rlst[d.seq].redundancy_ind=1))
      JOIN (defa)
     WITH nocounter
    ;end insert
    COMMIT
   ...
   SET message = window
   IF (dom_answer != "N")
    CALL echo("Insert OCDs with Target env_id in dm_alpha_features_env table (Initialize)...")
    SET sdate = cnvtdatetime(curdate,curtime3)
    FOR (ipt = 1 TO value(doc->red_cnt))
      IF ((doc->rlst[ipt].redundancy_ind=0))
       SET sdate = datetimeadd(sdate,(10/ 86400.0))
       INSERT  FROM dm_alpha_features_env defa
        SET defa.status = "Initialize Mass Move Process", defa.start_dt_tm = cnvtdatetime(sdate),
         defa.end_dt_tm = null,
         defa.environment_id = doc->target_env_id, defa.alpha_feature_nbr = doc->rlst[ipt].ocd_nbr,
         defa.inst_mode = mode_str,
         defa.calling_script = "DM_OCD_MENU", defa.curr_migration_ind = 1
        WITH nocounter
       ;end insert
       COMMIT
       SET ocd_number = doc->rlst[ipt].ocd_nbr
       CALL dm_ocd_log_op(ocd_op->cur_op,ols_start,"Loading of OCD schema ccl file begin")
   ...
   SELECT INTO "MINE"
    FROM dual
    DETAIL
     row + 1, "The OCDs have SUCCESSFULLY been intialized for the Mass Move Process.", row + 1
    WITH nocounter
   ;end select
   GO TO menu_screen
  ENDIF
 ...
 IF (mode_str != "RPT"
  AND mode_str != "INIT")
  IF (size(trim(prev_mode,3)))
   SELECT INTO "nl:"
    a.alpha_feature_nbr
    FROM dm_alpha_features_env a
    WHERE a.alpha_feature_nbr > 0
     AND (a.environment_id=doc->target_env_id)
     AND a.curr_migration_ind=1
    ORDER BY a.start_dt_tm
    DETAIL
     doc->source_ocd_cnt = (doc->source_ocd_cnt+ 1), stat = alterlist(doc->qual,doc->source_ocd_cnt),
     doc->qual[doc->source_ocd_cnt].ocd_nbr = a.alpha_feature_nbr
    WITH nocounter
   ;end select
  ENDIF
  SET dom_repeat = 0
  ...
  SELECT INTO "nl:"
   ...
   PLAN (a
    WHERE a.alpha_feature_nbr > 0
     ...
     AND a.curr_migration_ind=1)
    JOIN (d
    WHERE a.environment_id=d.environment_id
     AND d.project_type="INSTALL LOG"
     AND d.project_name=cnvtupper(ocd_op->cur_op)
     AND d.project_instance=1
     AND a.alpha_feature_nbr=d.ocd
     AND d.status=ols_complete)
   DETAIL
    dom_comp_cnt = (dom_comp_cnt+ 1)
   WITH nocounter
  ;end select
  IF ((dom_comp_cnt=doc->source_ocd_cnt))
   SET dom_repeat = 1
  ENDIF
  IF (dom_repeat=0
   AND dom_comp_cnt > 0)
   SET dom_rerun = 0
   IF (size(trim(mode_str,3)))
    SELECT INTO "nl:"
     a.alpha_feature_nbr
     FROM dm_alpha_features_env a
     PLAN (a
     ...
     ORDER BY a.start_dt_tm
     HEAD REPORT
      stat = alterlist(doc->qual,0), doc->source_ocd_cnt = 0, dom_rerun = 1
     DETAIL
      doc->source_ocd_cnt = (doc->source_ocd_cnt+ 1), stat = alterlist(doc->qual,doc->source_ocd_cnt),
      doc->qual[doc->source_ocd_cnt].ocd_nbr = a.alpha_feature_nbr
     WITH nocounter
  ...
  ENDIF
  IF ((doc->source_ocd_cnt > 0))
   SET dom_chk_all = dm_ocd_chk_op_all(ocd_op->pre_op)
   IF (dom_chk_all=0)
    CALL dom_ocd_op_rpt(ocd_op->pre_op,0)
    SET message = window
    CALL text(1,1,concat("The previous mode ",prev_mode,
      " has not been completed.  Are you sure you want to continue?"))
    CALL text(2,70,"Yes/No")
    CALL accept(2,82,"P;CU","N"
     WHERE curaccept IN ("Y", "N"))
    IF (curaccept="Y")
     UPDATE  FROM dm_ocd_log d,
       dm_alpha_features_env a,
       (dummyt dt  WITH seq = 1)
      SET d.seq = 1, d.message = "User selected to continue."
      PLAN (d
       WHERE d.environment_id=env_id
        AND d.project_type="INSTALL LOG"
        AND d.project_name=cnvtupper(ocd_op->pre_op)
        AND d.project_instance=1
        AND ((d.status != ols_complete) OR (d.status=null)) )
       JOIN (dt)
       JOIN (a
       WHERE a.alpha_feature_nbr > 0
        AND a.environment_id=env_id
    ...
    ELSE
     GO TO end_script
    ENDIF
   ELSEIF ((dom_chk_all=- (1)))
    SELECT
     d.seq
     FROM (dummyt d  WITH seq = 1)
     DETAIL
      line1 = fillstring(120," "), line1 = concat("The previous mode:",prev_mode,
       " has not been completed."), col 0,
      line1, row + 1
     WITH nocounter
    ;end select
    GO TO end_script
   ...
   CALL clear(1,1)
   SET message = nowindow
   IF ((ocd_op->cur_op=olo_atrs))
    DECLARE dom_toolset_usage = i2
    SET dom_toolset_usage = dm2_toolset_usage(null)
    IF (dom_toolset_usage=0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     GO TO end_script
    ELSEIF (dom_toolset_usage=1)
     DECLARE dom_schema_date = vc
     DECLARE dom_sd_info_name = vc
     DECLARE dom_schema_date_not_listed = vc WITH protect, constant(
      "-- My schema date is not listed. --")
     SET dm_err->eproc = "Verifying schema upgrade success before proceeding with ATR mode."
     SET width = 132
     SET message = window
     CALL box(1,1,5,130)
     CALL text(3,3,
      "Select schema date used to create a snapshot of the source database (DD-MON-YYYY format):")
     SET help = pos(6,61,14,70)
     SET help =
     SELECT INTO "nl:"
      schema_date = concat(format(d.info_date,"DD-MMM-YYYY;;D"),"    Upgrade completed ",format(d
        .updt_dt_tm,"MMM DD, YYYY HH:MM:SS;;D"))
      FROM dm_info d
      WHERE d.info_domain="DM2 INSTALL PROCESS"
       AND d.info_name="SCHEMA DATE UPGRADE SUCCESSFUL -*"
      ORDER BY d.updt_dt_tm DESC
      DETAIL
       col 0, schema_date, row + 1
      FOOT REPORT
       col 0, dom_schema_date_not_listed
      WITH nocounter, nullreport, maxrec = 20
     ;end select
     SET validate =
     SELECT INTO "nl:"
      schema_date = concat(format(d.info_date,"DD-MMM-YYYY;;D"),"    Upgrade completed ",format(d
      ...
      WITH nocounter, nullreport
     ;end select
     CALL accept(3,94,"P(35);HCF",dom_schema_date_not_listed)
     SET dm_err->eproc = concat("The user chose <",substring(1,12,curaccept),
      "...> from the schema date list.")
     CALL disp_msg(" ",dm_err->logfile,0)
     SET dm_err->eproc = "Verifying schema upgrade success before proceeding with ATR mode."
     IF (curaccept=dom_schema_date_not_listed)
      SET message = nowindow
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Schema date not found in list."
      SET dm_err->user_action = concat("Verify that schema changes are completed successfully ",
       "for your schema date in the target database.")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      GO TO end_script
     ELSE
      SET message = nowindow
      SET dom_schema_date = substring(1,11,curaccept)
      SET dom_sd_info_name = concat("SCHEMA DATE UPGRADE SUCCESSFUL - ",dom_schema_date)
     ENDIF
     SELECT INTO "nl:"
      d.info_date
      FROM dm_info d
      WHERE d.info_domain="DM2 INSTALL PROCESS"
       AND d.info_name=dom_sd_info_name
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc) > 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     ...
     ELSEIF (curqual=0)
      SET dm_err->emsg =
      "UPTIME SCHEMA is not complete.  The selected schema date upgrade has not been completed successfully."
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      GO TO end_script
     ELSE
      SET dm_err->eproc = "UPTIME SCHEMA completion verified.  Continuing with ATR installation."
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     SET help = off
     SET validate = off
   ...
   SELECT
    ...
    HEAD REPORT
     line1 = fillstring(120," "), line2 = fillstring(120," "), row + 1,
     line1 = concat("The following OCDs will be migrated from the ",doc->source_env_name,
      " environment"), col 5, line1,
     row + 1, line2 = concat("to the ",doc->target_env_name," environment in ",mode_str," mode."),
     col 5,
     line2, row + 2
    DETAIL
     col + 3, doc->qual[d.seq].ocd_nbr
   ...
   ;end select
   CALL clear(1,1)
   CALL text(1,1,concat("The OCDs listed in the report will be migrated from the ",doc->
     source_env_name," environment"))
   CALL text(2,1,concat("to the ",doc->target_env_name," environment in ",mode_str," mode."))
   CALL text(3,1,concat("The number of ocds will be migrated is ",cnvtstring(doc->source_ocd_cnt)))
   IF (dom_repeat=1)
    CALL text(5,1,concat(
      "WARNING: The list of ocds in the report have been installed successfully to the ",doc->
      target_env_name," environment in ",mode_str," mode previously."))
    CALL text(6,1,
     "Please proceed only if you would like to re-install all the ocds in the report again.")
   ENDIF
   CALL text(20,1,"Please enter <G>o to proceed or <Q>uit to exit.")
   CALL accept(20,50,"A;CU","Q"
    WHERE curaccept IN ("Q", "G"))
   ...
   ENDIF
   SET message = nowindow
   FOR (content_i = 1 TO doc->source_ocd_cnt)
     DELETE  FROM dm_alpha_features_env defa
      WHERE defa.alpha_feature_nbr > 0
       AND (defa.environment_id=doc->target_env_id)
       AND (defa.alpha_feature_nbr=doc->qual[content_i].ocd_nbr)
     ;end delete
     INSERT  FROM dm_alpha_features_env defa
      SET defa.status = "BEGIN DM_OCD_MENU", defa.start_dt_tm = cnvtdatetime(curdate,curtime3), defa
       .end_dt_tm = null,
       defa.environment_id = doc->target_env_id, defa.alpha_feature_nbr = doc->qual[content_i].
       ocd_nbr, defa.inst_mode = mode_str,
       defa.calling_script = "DM_OCD_MENU", defa.curr_migration_ind = 1
      WITH nocounter
     ;end insert
     COMMIT
     IF (mode_str="ATR")
      SET ocd_number = doc->qual[content_i].ocd_nbr
      CALL dm_ocd_log_op(olo_uptime_schema,ols_start,"Installing uptime schema begin")
      CALL dm_ocd_log_op(olo_uptime_schema,ols_complete,"Installing uptime schema successful")
     ENDIF
     IF (mode_str="POSTDTS")
      SET ocd_number = doc->qual[content_i].ocd_nbr
      CALL dm_ocd_log_op(olo_downtime_schema,ols_start,"Installing downtime schema begin")
      CALL dm_ocd_log_op(olo_downtime_schema,ols_complete,"Installing downtime schema successful")
     ENDIF
     IF (((mode_str="CS") OR (mode_str="ATR")) )
      CALL echo(concat("Applying the ",mode_str," only portion of OCD # ",cnvtstring(doc->qual[
         content_i].ocd_nbr)))
      SET dm_err->err_ind = 0
      SET parser_buff = fillstring(100," ")
      SET parser_buff = concat("execute OCD_INCL_SCHEMA2 ",cnvtstring(doc->qual[content_i].ocd_nbr),
       ",'",mode_str,"' go")
      CALL echo(parser_buff)
      CALL parser(parser_buff,1)
      CALL echo(concat("OCD # ",cnvtstring(doc->qual[content_i].ocd_nbr)," applied."))
     ENDIF
   ENDFOR
   IF (mode_str="ATR")
    SET ocd_apps_mode = "R"
    EXECUTE dm_ocd_apps
    SET message = nowindow
    SET ocd_tasks_mode = "R"
    EXECUTE dm_ocd_tasks
    SET message = nowindow
   ENDIF
  ENDIF
  IF (((mode_str="PREUTS") OR (((mode_str="POSTUTS") OR (((mode_str="PREDTS") OR (((mode_str=
  "POSTDTS") OR (mode_str="POSTINST")) )) )) )) )
   CALL echo("Preparing to execute a batch of readmes.")
   FREE SET rb_execution
   CASE (mode_str)
    OF "PREUTS":
     SET rb_execution = "PREUP"
    OF "POSTUTS":
     SET rb_execution = "POSTUP"
    OF "PREDTS":
     SET rb_execution = "PREDOWN"
    OF "POSTDTS":
     SET rb_execution = "POSTDOWN"
    OF "POSTINST":
     SET rb_execution = "UP"
   ENDCASE
   FOR (dom_cnt = 1 TO doc->source_ocd_cnt)
    SET ocd_number = doc->qual[dom_cnt].ocd_nbr
    CALL dm_ocd_log_op(ocd_op->cur_op,ols_start,"Start dm_readme_batch.")
   ENDFOR
   ...
   IF ((dm2_rr_misc->dm2_toolset_usage="Y"))
    ...
    SELECT INTO "nl:"
     FROM dm_ocd_log d,
      (dummyt t  WITH seq = value(size(drr_readmes_to_run->readme,5)))
     PLAN (t
      WHERE (drr_readmes_to_run->readme[t.seq].skip != 1))
      JOIN (d
      WHERE d.environment_id=env_id
       AND d.project_type="README"
       AND d.project_name=cnvtstring(drr_readmes_to_run->readme[t.seq].readme_id)
       AND (d.project_instance=drr_readmes_to_run->readme[t.seq].instance)
       AND (d.ocd=drr_readmes_to_run->readme[t.seq].ocd)
       AND d.status="FAILED")
     ORDER BY d.ocd
     HEAD d.ocd
      dom_cnt = 0
      FOR (dom_cnt = 1 TO size(doc->qual,5))
        IF ((drr_readmes_to_run->readme[t.seq].ocd=doc->qual[dom_cnt].ocd_nbr))
         doc->qual[dom_cnt].rdm_status = 1
        ENDIF
      ENDFOR
     DETAIL
      dom_cnt = 0
   ...
   ELSE
    ...
    WHILE (dom_rm_done=0)
     SELECT INTO "nl:"
      FROM dm_ocd_log d,
       (dummyt t  WITH seq = value(size(rb_data->readme,5)))
      PLAN (t
       WHERE (rb_data->readme[t.seq].skip != 1))
       JOIN (d
       WHERE d.environment_id=env_id
        AND d.project_type="README"
        AND d.project_name=cnvtstring(rb_data->readme[t.seq].id)
        AND (d.project_instance=rb_data->readme[t.seq].instance)
        AND (d.ocd=rb_data->readme[t.seq].ocd)
        AND ((d.status="RUNNING") OR (d.status = null)) )
      DETAIL
       readme_num_hold = rb_data->readme[t.seq].id
      WITH nocounter
     ;end select
     IF (curqual=0)
      SET dom_rm_done = 1
     ELSE
      CALL echo(concat("Readme number ",cnvtstring(readme_num_hold),
        "is still in process, Please wait..."))
      CALL pause(60)
     ENDIF
    ENDWHILE
    SET dom_err_readme = ""
    SELECT INTO "nl:"
     ...
     HEAD d.ocd
      dom_cnt = 0
      FOR (dom_cnt = 1 TO size(doc->qual,5))
        IF ((rb_data->readme[t.seq].ocd=doc->qual[dom_cnt].ocd_nbr))
         doc->qual[dom_cnt].rdm_status = 1
        ENDIF
      ENDFOR
     DETAIL
   ...
   FOR (dom_cnt = 1 TO size(doc->qual,5))
    SET ocd_number = doc->qual[dom_cnt].ocd_nbr
    IF ((doc->qual[dom_cnt].rdm_status=1))
     CALL dm_ocd_log_op(ocd_op->cur_op,ols_error,concat("One or more readme failed! ",
       "Please type 'DM_README_OCD_LOG ",trim(cnvtstring(doc->qual[dom_cnt].ocd_nbr)),
       " go' to view the details of all readmes on this OCD."))
     UPDATE  FROM dm_alpha_features_env a
      SET a.status = concat("One or more errors occurred while running readmes.",
        " Run DM_README_OCD_LOG <ocd> for details."), a.end_dt_tm = cnvtdatetime(curdate,curtime3)
      WHERE (a.environment_id=doc->target_env_id)
       AND (a.alpha_feature_nbr=doc->qual[dom_cnt].ocd_nbr)
    ...
    ELSE
     CALL dm_ocd_log_op(ocd_op->cur_op,ols_complete,concat("Finished executing all ",trim(ocd_op->
        cur_op)," steps for ocd ",trim(cnvtstring(doc->qual[dom_cnt].ocd_nbr))))
     UPDATE  FROM dm_alpha_features_env a
      SET a.status = "Successful", a.end_dt_tm = cnvtdatetime(curdate,curtime3)
      WHERE (a.environment_id=doc->target_env_id)
       AND (a.alpha_feature_nbr=doc->qual[dom_cnt].ocd_nbr)
       AND a.end_dt_tm = null
      WITH nocounter
 ...
 ENDIF
 SET migr_ind = 0
 IF ((((doc->source_ocd_cnt=0)) OR (mode_str="RPT")) )
  SELECT
   IF (mode_str != "RPT")
    FROM dm_alpha_features_env e,
     dm_ocd_log d
    PLAN (e
     WHERE (e.environment_id=doc->target_env_id)
      ...
      AND e.curr_migration_ind=1)
     JOIN (d
     WHERE d.environment_id=e.environment_id
      AND d.project_type="INSTALL LOG"
      AND d.project_name=cnvtupper(ocd_op->cur_op)
      AND d.project_instance=1
      AND d.ocd=e.alpha_feature_nbr)
   ELSE
    FROM dm_alpha_features_env e,
     dm_ocd_log d
    PLAN (e
 ...
 ENDIF
 IF (migr_ind=1)
  CALL dom_ocd_op_rpt(ocd_op->cur_op,1)
 ELSEIF (mode_str != "RPT"
  AND mode_str != "INIT")
  SELECT
   d.seq
   FROM (dummyt d  WITH seq = 1)
   DETAIL
    row + 1, "There are no OCDs to migrate from the source environment in ", mode_str,
    " mode."
    IF (mode_str != "INIT")
     row + 1, "You may be running the migrations in the wrong order.  They must be run in the order",
     row + 1,
     "they're listed in the initial menu."
    ENDIF
   WITH nocounter
  ;end select
 ELSE
  SELECT
   ...
   FROM (dummyt d  WITH seq = 1)
   DETAIL
    row + 1, "There are no OCDs currently being migrated to the selected target environment."
   WITH nocounter
  ;end select
 ENDIF
 IF (mode_str="POSTINST")
  SET dom_toolset_usage = 0
  SET dom_toolset_usage = dm2_toolset_usage(null)
  IF (dom_toolset_usage=0)
   CALL echo("*")
   CALL echo("*")
  ...
  ENDIF
 ENDIF
 SUBROUTINE dom_ocd_op_rpt(dom_op,cur_mode_ind)
  SELECT
   IF (mode_str != "RPT")
    FROM dm_ocd_log d,
     dm_alpha_features_env a
    PLAN (a
     ...
     WHERE d.environment_id=a.environment_id
      AND d.project_type="INSTALL LOG"
      AND d.project_name=cnvtupper(dom_op)
      AND d.project_instance=1
      AND d.ocd=a.alpha_feature_nbr)
    ORDER BY d.ocd
   ELSE
    FROM dm_ocd_log d,
     dm_alpha_features_env a
    PLAN (a
     ...
     WHERE d.environment_id=a.environment_id
      ...
      AND d.project_instance=1
      AND d.ocd=a.alpha_feature_nbr)
    ORDER BY d.ocd, d.start_dt_tm
   ENDIF
   HEAD REPORT
    IF ( NOT (cur_mode_ind))
     col 0, "Can not execute ", mode_str,
     " mode until ", prev_mode, " mode is complete.",
     row + 1, row + 1
     IF (prev_mode != "INIT")
      col 0, "Below screen displays the success/failure status of all ocds you have migrated in ",
      row + 1,
      col 0, prev_mode,
      ' mode.  All of them have to be in "COMPLETE" status before continuing the run in ',
      mode_str, " mode.", row + 1,
      row + 1, col 1, "TARGET_ENV",
      col 12, "OCD_NUMBER", col 24,
      "OPERATION_NAME", col 51, "STATUS",
      col 63, "START_DT_TM", col 88,
      "END_DT_TM", col 112, "MESSAGE",
      row + 1
     ELSEIF (prev_mode="INIT")
      col 0, "Please run dm_ocd_menu in 'INIT' mode."
     ENDIF
    ELSE
     IF ((doc->source_ocd_cnt=0)
      AND mode_str != "RPT")
      col 0, "There is no ocd remaining to migrate in mode ", mode_str,
      ".See status below for details.", row + 1
     ENDIF
     IF (mode_str != "RPT")
      col 0, "Below screen displays the success/failure status of ", row + 1,
      col 0, "all ocds you have migrated in ", mode_str,
      " mode.", row + 1, col 0,
      "Any OCD not in 'COMPLETE' status will need to be re-installed", row + 1, col 0,
      "by running dm_ocd_menu in ", mode_str, " mode again.",
      row + 1, row + 1
     ENDIF
     col 1, "TARGET_ENV", col 12,
     "OCD_NUMBER", col 24, "OPERATION_NAME",
     col 51, "STATUS", col 63,
     "START_DT_TM", col 88, "END_DT_TM",
     col 112, "MESSAGE", row + 1
    ENDIF
   DETAIL
    IF (prev_mode="INIT"
     AND cur_mode_ind=0)
     row + 0
    ELSE
     col 0, d.environment_id, col 11,
     d.ocd, col 24, d.project_name,
     col 51, d.status, col 63,
     d.start_dt_tm";;q", col 88, d.end_dt_tm";;q",
     dom_msg = ""
     IF (d.project_name=cnvtupper(olo_load_ccl_file))
      dom_msg = "INITIALIZATION COMPLETE"
     ELSE
      dom_msg = d.message
     ENDIF
     col 112, dom_msg, row + 1
    ENDIF
   WITH nocounter, formfeed = none, maxcol = 300
  ;end select
  IF (curqual=0
   AND cur_mode_ind=0)
   SELECT
    ...
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     col 0, prev_mode, col + 1,
     "mode has not been started.  You must complete ", col + 0, prev_mode,
     col + 1, "mode before execute ", col + 0,
     mode_str, col + 1, "mode.",
     row + 1
    WITH nocounter
   ;end select
  ...
  ELSEIF (curqual=0
   ...
   SELECT
    ...
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     col 0, "No status has been logged for ", col + 0,
     mode_str, col + 1, "mode.  Please check for errors and rerun dm_ocd_menu in ",
     col + 0, mode_str, col + 1,
     "mode.", row + 1
    WITH nocounter
   ;end select
  ELSEIF (curqual=0
   ...
   SELECT
    ...
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     col 0, "No status has been logged for any mode in dm_ocd_menu.", row + 1,
     col + 0, "Mass Move has not been started.  Please run dm_ocd_menu in ", col + 0,
     "INIT mode.", row + 1
    WITH nocounter
   ;end select
  ENDIF
 ...
 IF ((dm_err->debug_flag > 1))
  SET message = nowindow
  CALL echo("After turning on triggers...")
  SELECT INTO noforms
   namespace, attribute, substring(1,30,value)
   FROM v$context
   WITH nocounter
  ;end select
 ENDIF
 SET dm_err->eproc = "Exiting DM_OCD_MENU."
 CALL final_disp_msg(dom_logfile_prefix)
END GO
