CREATE PROGRAM bhs_ma_pvpatlist1:dba
 ...
 DECLARE bhsgeneralsurgerymd_txt = vc WITH public, constant(concat(
   "99221  99222   99223    99231    99232    99233    99235 ",
 ...
 DECLARE bhscriticalcaremd_txt = vc WITH public, constant(concat(
   "E1=99291   E2=99292    1=99221   2=99222   3=99223   ",
   "4=99231   5=99232   6=99233   D=99238   D1=99239   C1=99251   ",
   "C2=99252   C3=99253   C4=99254   C5=99255   C6=99261   C7=99262   ",
   "C8=99263    R=Resident Assisted Service     No Charge       ",
 ...
 DECLARE bhspsychiatrymd_txt = vc WITH public, constant(concat(
   "D1 =99239  C1=99251   C2=99253   C4=99254   6=99233   ",
   "C5=99255   C6=99261   C7=99262   C8=99263   99081    90835   ",
   "90842 ..90843 ..90844  90847  99853   90862   90970   90880   ",
 ...
 DECLARE bhsanesthesiologymd_txt = vc WITH public, constant(concat(
   ...
   "C2=99252    C3=99253    C4=99254    C5=99255   C6=99261    ",
   "C7=99262    C8=99263   No Charge    Not Seen    -QI    ","Update Diagnosis:________________"))
 DECLARE dbabhs_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",88,"DBABHS"))
 DECLARE dbabhs_txt = vc WITH public, constant(concat(
   "1 = 99221   2=99222    3=99223   4=99231   5=99232   ",
   "6=99233    99235 .99236  D=99238    D1=99239    C1=99251  ",
   "C2=99252    C3=99253    C4=99254    C5=99255   C6=99261    ",
   "C7=99262    C8=99263   No Charge    Not Seen    -QI    ","Update Diagnosis:________________ "))
 ...
 IF ((request->visit_cnt > 0))
  ...
  SELECT INTO request->output_device
   ...
   HEAD REPORT
    ...
    ENDMACRO
   HEAD PAGE
    "{pos/240/80}{f/8}{cpi/10}Printed Patient List", row + 1
    IF (list_name > " ")
    ...
    ENDIF
    ycol = 140, "{pos/30/125}{f/8}{cpi/14}Location", "{pos/115/125}MR",
    "{pos/180/125}Name  ", "{pos/280/125}Acct#", "{pos/330/125}DOB",
    "{pos/370/125}Day", "{pos/410/125}Admit", "{pos/450/125}Disch",
    "{pos/490/125}Attending MD{f/8}{cpi/16}", row + 1
   DETAIL
    row + 0, xcol = 40
    IF (((ycol+ 60) > 680)
     AND x < count)
    ...
    FOR (x = start_cnt TO count)
      ...
      ELSE
       xxx = fillstring(50," ")
      ENDIF
      CALL print(calcpos(30,ycol)), "{b}", xxx,
      CALL print(calcpos(115,ycol)), "{b}", temp->qual[x].mrn,
      CALL print(calcpos(180,ycol)), "{b}", temp->qual[x].name,
      "{endb}",
      CALL print(calcpos(280,ycol)), temp->qual[x].acct,
      ...
      CALL print(calcpos(40,ycol)),
      line1, row + 1, ycol = (ycol+ 6),
      ycol = (ycol+ 48), row + 1
      IF (ycol > 680
       AND x < count)
       start_cnt = (x+ 1), BREAK
 ...
 ELSEIF ((request->person_cnt > 0))
  ...
  SELECT INTO request->output_device
   ...
   HEAD PAGE
    ...
    ELSE
     "{pos/260/92}{cpi/14}For: ", uname, row + 1
    ENDIF
    ycol = 140, "{pos/100/125}{f/8}{cpi/14}MR", "{pos/180/125}Name  ",
    "{pos/280/125}Sex/Age", "{pos/330/125}DOB{f/8}{cpi/16}", row + 1
   DETAIL
    row + 0, xcol = 40
    IF (((ycol+ 60) > 680)
     AND x < count)
    ...
    ENDIF
    FOR (x = start_cnt TO count)
      CALL print(calcpos(100,ycol)), "{b}", temp->qual[x].mrn,
      CALL print(calcpos(180,ycol)), "{b}", temp->qual[x].name,
      "{endb}",
      CALL print(calcpos(280,ycol)), temp->qual[x].gender,
      CALL print(calcpos(285,ycol)), temp->qual[x].age,
      CALL print(calcpos(330,ycol)),
      temp->qual[x].dob, ycol = (ycol+ 12), row + 1,
      CALL print(calcpos(180,ycol)), "PCP: ", temp->qual[x].pcpdoc,
      ycol = (ycol+ 12), row + 1
      CASE (reqinfo->position_cd)
       OF bhsurologymd_cd:
        cpt_text = bhsurologymd_txt
  ...
  ;end select
 ELSE
  CALL echo("We're hitting the 3rd else encntr_cn !> 0, person_cnt !>0")
  SET new_timedisp = cnvtstring(curtime3)
  SET tempfile1a = build(concat("cer_temp:ptlrpt","_",new_timedisp),".dat")
