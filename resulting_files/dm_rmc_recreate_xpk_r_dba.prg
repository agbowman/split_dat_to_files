CREATE PROGRAM dm_rmc_recreate_xpk_r:dba
 ...
 SUBROUTINE index_create(i_table_name,i_r_table_name,i_rs_list)
   ...
   RETURN("S")
 END ;Subroutine
 SET dm_err->emsg = "Readme Failed: Starting dm_rmc_recreate_xpk_r"
 SET readme_data->status = "F"
 SET readme_data->message = dm_err->emsg
 IF (check_logfile("DM_RMC_RECREATE",".log","DM_RMC_RECREATE LogFile")=0)
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
  GO TO exit_program
 ENDIF
 DECLARE xp_return = vc
 DECLARE xp_loop_r = i4
 DECLARE idx = i4
 DECLARE idx2 = i4
 DECLARE xp_src_id = f8
 FREE RECORD xp_tbl_list
 RECORD xp_tbl_list(
   1 cnt = i4
   1 qual[*]
     2 t_name = vc
 ...
 IF ((dguc_reply->dguc_err_ind=1))
  ...
  GO TO exit_program
 ENDIF
 SET xp_tbl_list->cnt = dguc_reply->rs_tbl_cnt
 SET stat = alterlist(xp_tbl_list->qual,xp_tbl_list->cnt)
 FOR (xp_loop_r = 1 TO xp_tbl_list->cnt)
  SET xp_tbl_list->qual[xp_loop_r].t_name = dguc_reply->dtd_hold[xp_loop_r].tbl_name
  SET xp_tbl_list->qual[xp_loop_r].r_name = cutover_tab_name(xp_tbl_list->qual[xp_loop_r].t_name,
   dguc_reply->dtd_hold[xp_loop_r].tbl_suffix)
 ENDFOR
 SET dm_err->eproc = "Checking which $R tables exist in the domain"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM user_tables ut
  WHERE expand(idx,1,xp_tbl_list->cnt,ut.table_name,xp_tbl_list->qual[idx].r_name)
  DETAIL
   idx2 = 0, idx2 = locateval(idx,1,xp_tbl_list->cnt,ut.table_name,xp_tbl_list->qual[idx].r_name)
   IF (idx2 != 0)
    xp_tbl_list->qual[idx2].exist_ind = 1
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 SET xp_loop_r = 0
 FOR (xp_loop_r = 1 TO xp_tbl_list->cnt)
   IF ((xp_tbl_list->qual[xp_loop_r].exist_ind=1))
    SET dm_err->eproc = "Checking which source_id is in the $R table"
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM (parser(xp_tbl_list->qual[xp_loop_r].r_name) d)
     WHERE d.rdds_status_flag=0
     DETAIL
      xp_src_id = d.rdds_source_env_id
     WITH maxqual(d,1)
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    ...
    ENDIF
    IF (curqual != 0)
     EXECUTE dm_rmc_cutover_check xp_src_id, xp_tbl_list->qual[xp_loop_r].r_name
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      SET readme_data->status = "F"
      SET readme_data->message = dm_err->emsg
     ...
     ENDIF
    ENDIF
    SET xp_return = index_create(xp_tbl_list->qual[xp_loop_r].t_name,xp_tbl_list->qual[xp_loop_r].
     r_name,dguc_reply)
    IF (xp_return="F")
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Error returned from dm_get_unique_columns, view log_file for details."
 ...
 SET readme_data->message = "SUCCESS: Created XPK_$R index on all $R tables"
#exit_program
