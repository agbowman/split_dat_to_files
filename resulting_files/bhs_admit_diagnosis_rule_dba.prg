CREATE PROGRAM bhs_admit_diagnosis_rule:dba
 PROMPT
 ...
 SELECT INTO "NL:"
  ...
  PLAN (em
   ...
   WHERE e.rec_id=em.module_audit_id
    AND e.module_name="BHS_SYN_ADMITTING_DIAG"
    AND e.conclude=2)
  DETAIL
 ...
 SELECT INTO "NL:"
  ...
  PLAN (d
   WHERE d.encntr_id=value(enconterid)
    ...
    AND d.diag_type_cd IN (admit, working))
   JOIN (n
   ...
   "ADMITDIAGNOSIS-PNEUMONIA")) OR (b.nomenclature_id=0)) )
  ORDER BY b.nomen_list_key
  HEAD REPORT
   log_misc1 = patstring(" ",50)
  HEAD b.nomen_list_key
   CALL echo(b.nomen_list_key)
   IF (b.nomen_list_key="ADMITDIAGNOSIS-PNEUMONIA")
    IF (action1=0)
     log_misc1 = cnvtstring((cnvtint(log_misc1)+ 1)), msg2 = build2(msg2,":PNEUMONIA")
    ELSE
     msg2 = build2(msg2,":AlreadyFiredForPNEUMONIA")
    ENDIF
   ELSEIF (b.nomen_list_key="ADMITDIAGNOSIS-HEARTFAILURE")
    IF (action2=0)
