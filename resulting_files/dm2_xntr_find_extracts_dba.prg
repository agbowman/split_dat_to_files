CREATE PROGRAM dm2_xntr_find_extracts:dba
 ...
 SUBROUTINE dxrc_requeue_errors(dre_job_error_id,dre_extract_error_id)
   ...
   RETURN(0)
 END ;Subroutine
 IF (check_logfile("dm2_xntr_extracts",".log","DM2_XNTR_FIND_EXTRACTS Log...") != 1)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_xntr_find_extracts."
 CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 FREE RECORD dxfe_person
 RECORD dxfe_person(
   1 cnt = i4
   1 qual[*]
     2 person_id = f8
 ...
 DECLARE dxfe_idx = i4 WITH protect, noconstant(0)
 DECLARE dxfe_new_idx = i4 WITH protect, noconstant(0)
 DECLARE dxfe_total = i4 WITH protect, noconstant(0)
 DECLARE dxfe_num = i4 WITH protect, noconstant(0)
 DECLARE dxfe_cnt = i4 WITH protect, noconstant(0)
 IF (validate(dxfe_extract)=0)
  FREE RECORD dxfe_extract
  RECORD dxfe_extract(
    1 person_id = f8
    1 job_id = f8
    1 cnt = i4
  ...
  IF (((reflect(parameter(1,0)) != "I*"
   ...
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Expected syntax: dm2_xntr_find_extract <DM_XNTR_JOB_ID>, <PERSON_ID>"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_script
  ELSE
   SET dxfe_extract->job_id =  $1
   SET dxfe_extract->person_id =  $2
  ENDIF
 ENDIF
 SET dxfe_person->cnt = 1
 SET stat = alterlist(dxfe_person->qual,dxfe_person->cnt)
 SET dxfe_person->qual[dxfe_person->cnt].person_id = dxfe_extract->person_id
 SET dxfe_person->qual[dxfe_person->cnt].cmb_dt_tm = cnvtdatetime("01-JAN-1800 00:00:00")
 SET dxfe_person->qual[dxfe_person->cnt].ucb_dt_tm = cnvtdatetime("31-DEC-2100 23:59:59")
 SET dxfe_new_idx = 1
 WHILE ((dxfe_new_idx <= dxfe_person->cnt))
   SET dxfe_total = dxfe_person->cnt
   SELECT INTO "nl:"
    FROM person_combine p
    WHERE expand(dxfe_num,dxfe_new_idx,dxfe_person->cnt,p.from_person_id,dxfe_person->qual[dxfe_num].
     person_id)
     AND p.active_ind=0
    HEAD REPORT
     dxfe_cnt = dxfe_person->cnt
    DETAIL
     IF (locateval(dxfe_num,1,dxfe_person->cnt,p.to_person_id,dxfe_person->qual[dxfe_num].person_id)=
     0)
      dxfe_cnt = (dxfe_cnt+ 1), stat = alterlist(dxfe_person->qual,dxfe_cnt), dxfe_person->qual[
      dxfe_cnt].person_id = p.to_person_id,
      dxfe_person->qual[dxfe_cnt].cmb_dt_tm = p.cmb_dt_tm, dxfe_person->qual[dxfe_cnt].ucb_dt_tm = p
      .ucb_dt_tm
     ENDIF
    FOOT REPORT
     dxfe_person->cnt = dxfe_cnt
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_script
   ENDIF
   SET dxfe_new_idx = (dxfe_total+ 1)
 ENDWHILE
 SET dm_err->eproc = "Querying for related extracts."
 SELECT INTO "NL:"
  FROM dms_media_instance di,
   dms_media_xref dx,
   dms_media_identifier d,
   dms_content_type dc,
   (dummyt x  WITH seq = dxfe_person->cnt)
  PLAN (x)
   JOIN (dx
   WHERE dx.parent_entity_name="PERSON"
    AND (dx.parent_entity_id=dxfe_person->qual[x.seq].person_id))
   JOIN (di
   WHERE di.dms_media_identifier_id=dx.dms_media_identifier_id
    AND di.created_dt_tm < cnvtdatetimeutc(dxfe_person->qual[x.seq].ucb_dt_tm,2)
    AND di.created_dt_tm > cnvtdatetimeutc(dxfe_person->qual[x.seq].cmb_dt_tm,2))
   JOIN (dc
   WHERE dc.content_type_key IN ("ORDERS_EXTRACT", "CHARGES_EXTRACT", "CLINICAL_EVENT_EXTRACT",
   "SCHEDULING_EXTRACT", "DISCERN_ALERTS_EXTRACT")
    AND dc.dms_content_type_id=di.dms_content_type_id)
   JOIN (d
   WHERE d.dms_media_identifier_id=di.dms_media_identifier_id)
  HEAD REPORT
   dxfe_cnt = 0
  DETAIL
   dxfe_cnt = (dxfe_cnt+ 1), stat = alterlist(dxfe_extract->qual,dxfe_cnt), dxfe_extract->qual[
   dxfe_cnt].extract_uuid = d.media_object_identifier,
   dxfe_extract->qual[dxfe_cnt].extract_version = di.version, dxfe_extract->qual[dxfe_cnt].
   extract_dt_tm = cnvtdatetimeutc(di.created_dt_tm,1)
  FOOT REPORT
   dxfe_extract->cnt = dxfe_cnt
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No extracts found for person_id: ",trim(cnvtstring(dxfe_extract->
     person_id,20),3))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Inserting extract rows into dm_xntr_extract."
 FOR (dxfe_idx = 1 TO dxfe_extract->cnt)
   SELECT INTO "nl:"
    y_seq = seq(dm_clinical_seq,nextval)
    FROM dual
    DETAIL
     dxfe_extract->qual[dxfe_idx].extract_id = y_seq
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_script
   ENDIF
   INSERT  FROM dm_xntr_extract d
    SET dm_xntr_extract_id = dxfe_extract->qual[dxfe_idx].extract_id, dm_xntr_job_id = dxfe_extract->
     job_id, extract_person_id = dxfe_extract->person_id,
     extract_uuid = dxfe_extract->qual[dxfe_idx].extract_uuid, extract_version = dxfe_extract->qual[
     dxfe_idx].extract_version, status = "QUEUED",
