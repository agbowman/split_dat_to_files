CREATE PROGRAM dm_env_import:dba
 ...
 DECLARE dei_connect_string = vc WITH public, noconstant(" ")
 DECLARE dei_ecode = i4 WITH public, noconstant(0)
 DECLARE dei_emsg = c132 WITH public, noconstant(" ")
 DECLARE dei_admin_connection_ind = i2 WITH public, noconstant(0)
 DECLARE dei_free_define_ind = i2 WITH public, noconstant(0)
 DECLARE osver = vc WITH protect, constant(validate(cursys2,"AIX"))
 DECLARE found_oracle_version = i2 WITH protect, noconstant(0)
 DECLARE dem_blank_line = c130 WITH protect, noconstant(fillstring(130," "))
 DECLARE dclcom = vc WITH protect, noconstant("")
 DECLARE dei_refresh_screen(null) = null
 EXECUTE FROM initialize TO initialize_end
 SET width = 132
 SET op_system = cursys
 IF (op_system IN ("AXP", "VMS"))
  SET op_system = "VMS"
 ENDIF
 EXECUTE FROM admin_prompt TO admin_prompt_end
 EXECUTE FROM main_screen TO main_screen_end
 CALL text(23,40,"(Q)uit , (S)ave and Quit , (C)ontinue ")
 CALL accept(23,90,"A;CU","C"
 ...
 ENDIF
 IF (curaccept IN ("S", "C"))
  IF (dei_new_environment_ind=1)
   EXECUTE dm_env_import_main
   IF ( NOT (des_env_switch(dm_env_import_request->target_environment_id)))
    GO TO end_script
   ENDIF
  ELSE
   UPDATE  FROM dm_environment de
    SET de.description = dm_env_import_request->target_environment_description, de.database_name =
     dm_env_import_request->target_database_name, de.db_version = dm_env_import_request->
     target_database_version,
     de.oracle_version = dm_env_import_request->target_oracle_version, de.cerner_fs_mtpt =
     dm_env_import_request->cerner_mtpt, de.ora_pri_fs_mtpt = dm_env_import_request->ora_sft_mtpt,
     de.ora_sec_fs_mtpt = dm_env_import_request->ora_link_mtpt, de.database_disk =
     dm_env_import_request->vms_database_disk, de.database_archive_disk = dm_env_import_request->
     vms_archive_disk,
     de.v500_connect_string = concat("V500/V500@",dm_env_import_request->target_database_name,"1"),
     de.v500ref_connect_string = concat("V500_REF/V500_REF@",dm_env_import_request->
      target_database_name,"1")
    WHERE (de.environment_id=dm_env_import_request->target_environment_id)
    WITH nocounter
   ;end update
  ENDIF
 ENDIF
 ...
 IF (dei_free_define_ind=1)
  SET message = nowindow
  CALL echo("*************************************************************")
  CALL echo("PLEASE EXIT OUT OF CCL BEFORE PERFORMING ANY OTHER COMMAND.")
  CALL echo("*************************************************************")
 ENDIF
 GO TO end_script
#initialize
 RECORD dm_env_import_request(
   1 master_environment_name = vc
   ...
   1 target_database_desc = vc
   1 target_oracle_version = vc
   1 target_rdbms_version = vc
   1 base_oracle_version = vc
   1 base_oracle_version_int = i4
   1 target_environment_description = vc
   1 target_extent_management = vc
   1 target_undo_ind = i2
   1 target_database_version_type = vc
   1 vms_database_disk = vc
   1 vms_archive_disk = vc
   1 cerner_mtpt = vc
   1 ora_sft_mtpt = vc
   1 ora_link_mtpt = vc
   1 envset_string = vc
 ...
 SET dm_env_import_request->target_environment_id = 0.0
 SET dm_env_import_request->target_environment_exists = 0
 ...
 SET dm_env_import_request->admin_username = "cdba"
 SET dm_env_import_request->admin_connect_string = ""
#initialize_end
#admin_prompt
 IF (currdbuser="CDBA")
  SELECT INTO "nl:"
   ...
   WHERE 1=1
   DETAIL
    dei_admin_connection_ind = 1, dei_instance_name = vdi.instance_name
   WITH nocounter
  ;end select
 ENDIF
 IF (dei_admin_connection_ind=0)
  CALL clear(1,1)
  SET width = 132
  CALL box(1,1,23,132)
  CALL clear(2,2,130)
  ...
  CALL clear(9,2,130)
  CALL clear(10,2,130)
  CALL text(2,40,"***   DM_ENV_IMPORT  (Admin Prompt)***")
  IF (currdbuser="CDBA")
   CALL text(4,8,"*** Please verify the admin connection information. ***")
  ELSE
   CALL text(4,8,"*** Please provide the admin connection information. ***")
  ENDIF
  CALL clear(6,2,130)
  CALL text(6,2,"Admin user name: ")
  CALL accept(6,40,"P(30);CS",dm_env_import_request->admin_username)
  SET dm_env_import_request->admin_username = trim(curaccept)
  IF ((((dm_env_import_request->admin_username="")) OR ((dm_env_import_request->admin_username <= " "
  ))) )
   CALL text(7,2,"*** Admin User Name required *** ")
   GO TO admin_prompt
  ENDIF
  CALL clear(7,2,130)
  CALL text(7,2,"Admin password: ")
  CALL accept(7,40,"P(30);CS","")
  SET dm_env_import_request->admin_password = trim(curaccept)
  IF ((((dm_env_import_request->admin_password="")) OR ((dm_env_import_request->admin_password <= " "
  ))) )
   CALL text(8,2,"*** Admin Password Required *** ")
   GO TO admin_prompt
  ENDIF
  CALL clear(8,2,130)
  CALL text(8,2,"Admin connect string: ")
  CALL accept(8,40,"P(30);CS",dm_env_import_request->admin_connect_string)
  SET dm_env_import_request->admin_connect_string = trim(curaccept)
  IF ((((dm_env_import_request->admin_connect_string="")) OR ((dm_env_import_request->
  admin_connect_string <= " "))) )
   CALL text(8,2,"*** Admin Password Required *** ")
   GO TO admin_prompt
  ENDIF
  SET dei_connect_string = concat(dm_env_import_request->admin_username,"/",dm_env_import_request->
   admin_password,"@",dm_env_import_request->admin_connect_string)
  CALL clear(10,2,130)
  IF (currdbuser="CDBA")
   CALL text(10,2,concat("Verifying admin connection. Connecting to admin using: ",dei_connect_string,
     "..."))
  ELSE
   CALL text(10,2,concat("Connecting to admin using: ",dei_connect_string),"...")
  ENDIF
  CALL parser("free define oraclesystem go",1)
  SET dei_free_define_ind = 1
  SET dei_ecode = error(dei_emsg,1)
  CALL parser(concat("define oraclesystem '",trim(dei_connect_string),"' go"),1)
  SET dei_ecode = error(dei_emsg,1)
  IF (dei_ecode != 0)
   CALL clear(10,2,130)
   CALL clear(11,2,130)
   CALL text(10,2,concat("*** Admin Connection UNSUCCESSFUL for ",dei_connect_string,
     " .  Please retry."))
   CALL clear(12,2,130)
   CALL text(12,5,"(P)rompt again, (Q)uit ")
   CALL accept(12,30,"A;CU","P"
    WHERE curaccept IN ("P", "Q"))
   IF (curaccept="P")
    GO TO admin_prompt
   ELSE
    GO TO end_script
...
#main_screen
 ...
 CALL clear(21,2,130)
 CALL clear(22,2,130)
 CALL text(2,40,"***   DM_ENV_IMPORT  (Database Creation Prompts) ***")
 CALL text(3,8,
  "*** Please provide information for all prompts to successfully generate the database creation scripts ***"
  )
#accept_env_name
 CALL text(5,2,"Target Environment Name: ")
 CALL text(6,2,"  * Enter a new environment name OR ")
 CALL text(7,2,"  * Press <Shift><F5> to select an existing environment name. ")
 SET help =
 SELECT
  b.environment_name, b.database_name
  FROM dm_environment b
  WHERE b.target_operating_system=op_system
   AND substring(1,5,b.environment_name) != "OLD <"
  ORDER BY b.environment_name
  WITH nocounter
 ;end select
 CALL accept(5,40,"P(20);CUS",dm_env_import_request->target_environment_name)
 IF (curscroll=2)
  GO TO accept_master_env
 ELSEIF (curscroll != 0)
 ...
 IF (curaccept="")
  GO TO accept_env_name
 ENDIF
 SET dm_env_import_request->target_environment_name = trim(cnvtupper(curaccept))
 SET help = off
 SET validate = off
 SET dei_new_environment_ind = 1
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE (cnvtupper(de.environment_name)=dm_env_import_request->target_environment_name)
  DETAIL
   dm_env_import_request->target_database_name = de.database_name, dm_env_import_request->
   target_database_version = de.db_version, dm_env_import_request->target_oracle_version = trim(de
    .oracle_version),
   dm_env_import_request->target_environment_description = trim(de.description),
   dm_env_import_request->master_environment_name = trim(de.description), dm_env_import_request->
   target_environment_id = de.environment_id,
   dm_env_import_request->vms_database_disk = de.database_disk, dm_env_import_request->
   vms_archive_disk = de.database_archive_disk
  WITH nocounter
 ;end select
 IF (curqual=1)
  IF ( NOT (des_env_switch(dm_env_import_request->target_environment_id)))
   GO TO end_script
  ENDIF
  SET dei_new_environment_ind = 0
  SET dir_env_maint_rs->tgt_env_id = dm_env_import_request->target_environment_id
  SET dir_env_maint_rs->tgt_hist_fnd = 0
  EXECUTE dm2_chk_env_history
  IF ((dm_err->err_ind=1))
   GO TO end_script
  ENDIF
  IF ((dir_env_maint_rs->tgt_hist_fnd=1))
   CALL clear(1,1)
   CALL box(1,1,23,132)
   CALL text(5,2,build("Environment history already exists for given environment id <",cnvtstring(
      dm_env_import_request->target_environment_id),"> and environment name <",dm_env_import_request
     ->target_environment_name,">.  If you continue, "))
   CALL text(6,2,concat("a new environment id will be created for the <",dm_env_import_request->
     target_environment_name,"> environment to force the creation of new history."))
   CALL text(8,2,"Press C to Continue or X to exit (C/X): ")
   CALL accept(8,42,"A;CU","X"
    WHERE curaccept IN ("C", "X"))
   IF (curaccept="X")
    GO TO end_script
   ELSE
    UPDATE  FROM dm_environment d
     SET d.environment_name = concat("OLD <",cnvtstring(dm_env_import_request->target_environment_id),
       ">"), d.description = concat("OLD_NAME:",d.environment_name)
     WHERE (d.environment_id=dm_env_import_request->target_environment_id)
     WITH nocounter
    ;end update
    SET dei_new_environment_ind = 1
    CALL dei_refresh_screen(null)
   ENDIF
  ENDIF
 ENDIF
 CALL clear(6,2,130)
...
#accept_master_env
 IF (dei_new_environment_ind=1)
  CALL text(6,2,"Master Environment to Import: ")
  SET cur_oracle_version = fillstring(15," ")
  SET found_oracle_version = 0
  IF (((osver="AIX") OR (osver="AXP")) )
   SET help =
   SELECT
    master_environment = b.environment_name
    FROM dm_environment_ship b
    WHERE target_operating_system=op_system
    ORDER BY b.environment_name
    WITH nocounter
   ;end select
  ELSE
   SET help =
   SELECT
    master_environment = replace(b.environment_name,"AIX","HPUX",0)
    FROM dm_environment_ship b
    WHERE target_operating_system=op_system
    ORDER BY b.environment_name
    WITH nocounter
   ;end select
  ENDIF
  CALL accept(6,40,"P(20);CUF",dm_env_import_request->master_environment_name)
  IF (((osver="AIX") OR (osver="AXP")) )
   SET dm_env_import_request->master_environment_name = curaccept
  ELSE
   SET dm_env_import_request->master_environment_name = replace(curaccept,"HPUX","AIX",0)
  ENDIF
  SET dm_env_import_request->target_environment_description = curaccept
  SET help = off
  SET validate = off
 ENDIF
 CALL clear(6,2,130)
 CALL text(6,2,"Target Environment Description: ")
 CALL text(7,2,"  * Modify description at this time if you wish ")
 CALL accept(6,40,"P(20);CUF",dm_env_import_request->target_environment_description)
 SET dm_env_import_request->target_environment_description = curaccept
#accept_db_name
 CALL clear(7,2,130)
 CALL clear(8,2,130)
 CALL text(8,2,"Target Database Name: ")
 CALL accept(8,40,"P(5);CS",dm_env_import_request->target_database_name)
 IF (curscroll=2)
  GO TO accept_env_name
 ELSEIF (curscroll != 0)
 ...
 IF (curaccept="")
  GO TO accept_db_name
 ENDIF
 SET dm_env_import_request->target_database_name = trim(curaccept)
#accept_db_version
 CALL text(9,2,"Target Database Version: ")
 CALL text(10,5,"* Enter database version number or <SHIFT><F5> to see a full pick list ")
 IF (((osver="AIX") OR (osver="AXP")) )
  SET help =
  SELECT INTO "nl:"
   dsd.db_version";l", dsd.description
   FROM dm_size_db_version dsd
   WHERE cnvtupper(dsd.description)=patstring(concat("*",op_system,"*"))
   ORDER BY dsd.db_version
   WITH nocounter
  ;end select
 ELSE
  SET help =
  SELECT INTO "nl:"
   dsd.db_version";l", description = replace(dsd.description,"Aix","HPUX",0)
   FROM dm_size_db_version dsd
   WHERE cnvtupper(dsd.description)=patstring(concat("*",op_system,"*"))
    AND dsd.db_version >= 1000
   ORDER BY dsd.db_version
   WITH nocounter
  ;end select
 ENDIF
 CALL accept(9,40,"9(6);S",dm_env_import_request->target_database_version)
 IF (curscroll=2)
  GO TO accept_db_name
 ELSEIF (curscroll != 0)
 ...
 IF (curaccept <= 0)
  GO TO accept_db_version
 ENDIF
 SET dm_env_import_request->target_database_version = cnvtint(curaccept)
 SET help = off
 SET validate = off
 SELECT INTO "nl:"
  FROM dm_size_db_version sdv
  WHERE (sdv.db_version=dm_env_import_request->target_database_version)
  DETAIL
   IF (((osver="AIX") OR (osver="AXP")) )
    dm_env_import_request->target_database_desc = sdv.description
   ELSE
    dm_env_import_request->target_database_desc = replace(sdv.description,"Aix","HPUX",0)
   ENDIF
  WITH nocounter
 ;end select
 IF (curqual=0)
  CALL text(24,2,
   "Target Database Version no longer exists, please do not reuse this environment name.")
  GO TO end_script
 ENDIF
 IF ((dm_env_import_request->target_database_version IN (900, 902, 910, 912, 1000)))
  SET dm_env_import_request->target_database_version_type = "PROD"
 ELSE
  SET dm_env_import_request->target_database_version_type = "NONPROD"
 ENDIF
 CALL clear(10,2,130)
 CALL text(10,2,"Target Database Desc:")
 CALL text(10,40,dm_env_import_request->target_database_desc)
#accept_oracle_version
 IF (cursys="AXP")
  CALL text(12,2,"Oracle \\rdbms key name:")
 ELSE
  CALL text(12,2,"Oracle Version:")
 ENDIF
 IF (cursys != "AIX")
 ...
 IF (found_oracle_version=1)
  CALL text(12,40,cur_oracle_version)
  SET dm_env_import_request->target_oracle_version = trim(cur_oracle_version,3)
 ELSE
  SET help = off
  CALL text(24,01,dem_blank_line)
  IF (cursys != "AIX")
   CALL text(24,02,"Enter Oracle Version.  Example oraclev817 or oraclev920")
   CALL accept(12,40,"AAAAAAAP(8);CS","oraclev")
  ELSE
   CALL text(24,02,"Enter Oracle Version.  Example 8.1.7.3 or 9.2.0.3 or 9.2.0.5")
   CALL accept(12,40,"N(15);CS")
  ENDIF
  IF (curscroll=2)
  ...
  IF (((curaccept="") OR (curaccept <= " ")) )
   GO TO accept_oracle_version
  ENDIF
  SET dm_env_import_request->target_oracle_version = trim(curaccept)
 ENDIF
 CALL text(24,01,dem_blank_line)
 CALL text(12,40,dm_env_import_request->target_oracle_version)
#get_rdbms_ver
 IF (cursys="AXP")
  SET rdbms_ver = fillstring(10," ")
  CALL text(13,2,"Oracle Version:")
  CALL text(13,40,rdbms_ver)
  SET help_list = concat("ORACLE_920,ORACLE_816")
  SET help = fix(value(help_list))
  CALL accept(13,40,"P(20);CUF"
   WHERE curaccept IN ("ORACLE_816", "ORACLE_920"))
  SET dm_env_import_request->target_rdbms_version = trim(curaccept)
  SET help = off
  SET dm_env_import_request->target_rdbms_version = trim(dm_env_import_request->target_rdbms_version,
   3)
 ENDIF
 CASE (cursys)
  OF "AXP":
   IF ((dm_env_import_request->target_rdbms_version="ORACLE_8*"))
    SET dm_env_import_request->base_oracle_version = "8"
   ELSEIF ((dm_env_import_request->target_rdbms_version="ORACLE_9*"))
    SET dm_env_import_request->base_oracle_version = "9"
   ENDIF
  ELSE
   IF ((dm_env_import_request->target_oracle_version="8*"))
    SET dm_env_import_request->base_oracle_version = "8"
   ELSEIF ((dm_env_import_request->target_oracle_version="9*"))
    SET dm_env_import_request->base_oracle_version = "9"
   ELSEIF ((dm_env_import_request->target_oracle_version="10*"))
    SET dm_env_import_request->base_oracle_version = "10"
   ELSE
    CALL text(24,2,"Oracle version is not 8, 9, or 10")
    GO TO end_script
   ENDIF
 ENDCASE
 SET dm_env_import_request->base_oracle_version_int = cnvtint(dm_env_import_request->
  base_oracle_version)
#get_rdbms_ver_end
 IF (cursys="AIX")
  SET linepos = 14
...
#get_dmt_lmt
 IF ((dm_env_import_request->base_oracle_version="8"))
  SET dm_env_import_request->target_extent_management = "DICTIONARY MANAGED"
 ELSEIF (osver="HPX")
  SET dm_env_import_request->target_extent_management = "LOCALLY MANAGED"
  SET dm_env_import_request->target_undo_ind = 1
  CALL text(linepos,2,"Database Extent Management:")
  CALL text(linepos,40,dm_env_import_request->target_extent_management)
 ELSE
  SET linepos = (linepos+ 1)
  SET dei_extmgmt = fillstring(18," ")
  CALL text(linepos,2,"Database Extent Management:")
  CALL text((linepos+ 1),2,"  * Please select how the Database system tablespace should be managed")
  CALL text(linepos,40,dei_extmgmt)
  SET em_help_list = concat("LOCALLY MANAGED,DICTIONARY MANAGED")
  SET help = fix(value(em_help_list))
  CALL accept(linepos,40,"P(20);CUF"
   WHERE curaccept IN ("LOCALLY MANAGED", "DICTIONARY MANAGED"))
  SET dm_env_import_request->target_extent_management = curaccept
  SET help = off
  CALL clear((linepos+ 1),2,130)
  CASE (dm_env_import_request->target_extent_management)
   OF "LOCALLY MANAGED":
    SET dm_env_import_request->target_undo_ind = 1
   OF "DICTIONARY MANAGED":
    SET dm_env_import_request->target_undo_ind = 0
  ENDCASE
 ENDIF
#get_dmt_lmt_end
#get_characer_set
 SET linepos = (linepos+ 1)
 CALL text(linepos,2,"Character Set:")
 CALL text(linepos,40,dm_env_import_request->character_set)
 SET cs_help_list = concat("US7ASCII,WE8MSWIN1252,WE8ISO8859P1")
 SET help = fix(value(cs_help_list))
 CALL accept(linepos,40,"P(12);CUF"
  WHERE curaccept IN ("US7ASCII", "WE8MSWIN1252", "WE8ISO8859P1"))
 SET dm_env_import_request->character_set = curaccept
 SET help = off
#get_character_set_end
#aix_cerner_mtpt
 IF (cursys="AIX")
  SET linepos = (linepos+ 1)
  CALL clear(linepos,3,130)
  CALL text(linepos,02,"Cerner Mt Pt:")
  CALL accept(linepos,40,"P(60);CS",dm_env_import_request->cerner_mtpt)
  SET dm_env_import_request->cerner_mtpt = curaccept
  IF ((((dm_env_import_request->cerner_mtpt="")) OR ((dm_env_import_request->cerner_mtpt <= " "))) )
   CALL text((linepos+ 1),02,"*** Cerner Mt Pt required *** ")
   SET linepos = (linepos - 1)
   GO TO aix_cerner_mtpt
  ENDIF
 ENDIF
 CALL clear((linepos+ 1),3,130)
#ora_sft_mtpt
 IF (cursys="AIX")
  SET linepos = (linepos+ 1)
  CALL clear(linepos,3,130)
  CALL text(linepos,02,"ORA Soft Mt Pt:")
  CALL accept(linepos,40,"P(60);CS",dm_env_import_request->ora_sft_mtpt)
  SET dm_env_import_request->ora_sft_mtpt = curaccept
  IF ((((dm_env_import_request->ora_sft_mtpt="")) OR ((dm_env_import_request->ora_sft_mtpt <= " ")))
  )
   CALL text((linepos+ 1),02,"*** ORA Soft Mt Pt required *** ")
   SET linepos = (linepos - 1)
   GO TO ora_sft_mtpt
  ENDIF
 ENDIF
...
#ora_link_mtpt
 IF (cursys="AIX")
  SET linepos = (linepos+ 1)
  CALL clear(linepos,3,130)
  CALL text(linepos,02,"ORA Link Mt Pt:")
  CALL accept(linepos,40,"P(60);CS",dm_env_import_request->ora_link_mtpt)
  SET dm_env_import_request->ora_link_mtpt = curaccept
  IF ((((dm_env_import_request->ora_link_mtpt="")) OR ((dm_env_import_request->ora_link_mtpt <= " ")
  )) )
   CALL text((linepos+ 1),02,"*** ORA Soft Mt Pt required *** ")
   SET linepos = (linepos - 1)
   GO TO ora_link_mtpt
...
#accept_database_disk
 IF (cursys="AXP")
  SET linepos = (linepos+ 1)
  CALL clear(linepos,3,130)
  CALL text(linepos,02,"Enter Database Disk :")
  CALL accept(linepos,40,"P(30);CUS",dm_env_import_request->vms_database_disk)
  SET dm_env_import_request->vms_database_disk = curaccept
  IF ((((dm_env_import_request->vms_database_disk="")) OR ((dm_env_import_request->vms_database_disk
   <= " "))) )
   SET linepos = (linepos - 1)
...
#accept_archive_disk
 IF (cursys="AXP")
  SET linepos = (linepos+ 1)
  CALL clear(linepos,3,130)
  CALL text(linepos,02,"Enter Database Archive Disk :")
  CALL accept(linepos,40,"P(30);CUS",dm_env_import_request->vms_archive_disk)
  SET dm_env_import_request->vms_archive_disk = curaccept
  IF ((((dm_env_import_request->vms_archive_disk="")) OR ((dm_env_import_request->vms_archive_disk
   <= " "))) )
   SET linepos = (linepos - 1)
   GO TO accept_archive_disk
  ENDIF
 ENDIF
#main_screen_end
 SUBROUTINE dei_refresh_screen(null)
   CALL clear(1,1)
   SET width = 132
   CALL box(1,1,23,132)
   CALL clear(2,2,130)
   ...
   CALL text(5,2,"Target Environment Name: ")
   CALL text(5,40,dm_env_import_request->target_environment_name)
 END ;Subroutine
#end_script
END GO
