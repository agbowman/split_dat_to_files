CREATE PROGRAM dm2_ref_data_mover_8796:dba
 ...
 DECLARE cust_to_value_cd = vc
 DECLARE cust_value_cdf = vc
 DECLARE cust_response_value = vc
 DECLARE cust_new_value = vc
 DECLARE cust_resp_col_pos = i4
 DECLARE cust_value_col_pos = i4
 DECLARE cust_rpt_value = vc
 DECLARE cust_loop = i4
 DECLARE cust_err_val = i4
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_value_col_pos = get_col_pos(cust_tab_name,"VALUE_TYPE_CD")
 SET cust_resp_col_pos = get_col_pos(cust_tab_name,"RESPONSE_VALUE")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_value_col_pos].translated=1))
  SET cust_to_value_cd = get_value(cust_tab_name,"VALUE_TYPE_CD","TO")
  SELECT INTO "nl:"
   FROM code_value cv
   WHERE cv.code_value=cnvtreal(cust_to_value_cd)
   DETAIL
  ...
  IF (check_error("Obtaining CDF meaning")=1)
   ...
   GO TO exit_8796
  ENDIF
  IF (cust_value_cdf="EVENT_ID")
   SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_resp_col_pos].translated = 0
   SET cust_response_value = get_value(cust_tab_name,"RESPONSE_VALUE","FROM")
   SET cust_new_value = select_merge_translate(cust_response_value,"TRACK_EVENT")
   IF (cust_new_value != "No Trans")
    CALL put_value(cust_tab_name,"RESPONSE_VALUE",cust_new_value)
   ELSE
    SET cust_rpt_value = report_missing("TRACK_EVENT","TRACK_EVENT_ID",cnvtreal(cust_response_value))
    CALL evaluate_rpt_missing(cust_rpt_value)
   ENDIF
  ENDIF
 ELSE
  ...
  GO TO exit_8796
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"SN_TRACK_RULE_RESPONSE_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"SN_TRACK_RULE_RESPONSE_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"SN_TRACK_RULE_RESPONSE_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,
       "SN_TRACK_RULE_RESPONSE_ID")
     ELSE
      GO TO exit_8796
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"SN_TRACK_RULE_RESPONSE_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_8796
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"SN_TRACK_RULE_RESPONSE_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_8796
   ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"RESPONSE_VALUE")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the RESPONSE_VALUE"
