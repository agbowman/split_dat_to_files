CREATE PROGRAM bhs_ma_clinical_summary:dba
 SET vversion = "POWERCHART"
 ...
 DECLARE nursecommunicationhcm_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "NURSECOMMUNICATIONHCM"))
 ...
 IF ((request->visit_cnt > 0))
   ...
   HEAD e.encntr_id
    IF (e.encntr_id > 0)
     ...
     dlrec->seq[cnt].name_full_formatted = trim(p.name_full_formatted), dlrec->seq[cnt].birth_dt_tm
      = cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1), dlrec->seq[cnt].account_nbr = ea
     .alias,
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   DETAIL
    IF (c.event_cd != last_event_cd)
     last_event_cd = c.event_cd, cnt += 1
     IF (mod(cnt,10)=1)
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   DETAIL
    cnt += 1,
    CALL echo(build("nomenclature_id:",n.nomenclature_id)),
    ...
    IF (n.nomenclature_id > 0)
     ...
     CALL echo(build("source1:",n.source_string))
    ELSE
     dlrec->seq[dd.seq].diagnosis[cnt].source_string = d.diag_ftdesc,
     CALL echo(build("source2:",d.diag_ftdesc))
    ENDIF
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   PLAN (dd)
    ...
    code_cd, deliveredby_cd, placeofbirth_cd)
     ...
     AND c.event_tag > " ")
    JOIN (cd
    WHERE (cd.event_id= Outerjoin(c.event_id))
  ...
  SELECT DISTINCT INTO "nl:"
   sort_order =
   IF (c.event_cd=nursecommunicationnutrition_cd) 33
   ...
   ELSEIF (c.event_cd=nursecommunicationhcm_cd) 29
   ...
   PLAN (dd)
    ...
    nursecommunicationhcm_cd, agencycontactperson_cd, portableunitrequired_cd,
    patientgoinghomeonoxygen_cd, confirmedtransferstarttimedate_cd,
    pulmonarynurseappointment_cd, earlyinterventionprograms_cd, vnahospicehomecare_cd,
    adultdayhealthcare_cd, resthomescommunityresidencesshelters_cd,
    nursinghomesskilledrehabfacilities_cd, chronichospital_cd,
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   PLAN (dd)
    ...
    JOIN (c2
    WHERE (c2.event_id= Outerjoin(c.parent_event_id)) )
  ...
  SELECT INTO request->output_device
   ...
   HEAD PAGE
    ...
    "DOB: ", dlrec->seq[i].birth_dt_tm"mm/dd/yy;;D", rowplusone2,
    CALL print(calcpos(xcol2,yrow1)), "CMRN: ", dlrec->seq[i].cmrn,
    CALL print(calcpos((xcol4 - 5),yrow1)), "Acct #: ", dlrec->seq[i].account_nbr,
    CALL print(calcpos((xcol6 - 3),yrow1)), "Admit: ", dlrec->seq[i].admit_dt,
    rowplusone2,
   ...
   DETAIL
    ...
    "** End of Clinical Summary **", row + 1, yrow1 = 650
    FOR (i = 1 TO 10)
