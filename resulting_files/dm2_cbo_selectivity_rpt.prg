CREATE PROGRAM dm2_cbo_selectivity_rpt
 ...
 SUBROUTINE dcr_get_instance_info(dgii_instance_name,dgii_instance_nbr)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2cboselrpt",".log","dm2_cbo_selectivity_rpt")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting Dm2_cbo_selectivity_rpt"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Validating input parameters."
 FREE RECORD dcsr_data
 RECORD dcsr_data(
   1 cnt = i4
   1 tab[*]
     2 tab_name = vc
     2 act_num_rows = f8
     2 act_tab_blk_below_hwm = f8
     2 act_tab_blocks = f8
     2 act_tab_unused_blocks = f8
     2 act_tab_used_blocks = f8
     2 act_tab_null_ind = i2
     2 dict_num_rows = f8
     2 dict_tab_blk_below_hwm = f8
     2 dict_tab_blocks = f8
     2 dict_tab_unused_blocks = f8
     2 dict_tab_used_blocks = f8
     2 dict_tab_null_ind = i2
     2 col_cnt = i4
     2 col[*]
       3 data_type = vc
       3 ccl_data_type = vc
 ...
 DECLARE dcsr_tab = vc WITH protect, noconstant(cnvtupper( $1))
 DECLARE dcsr_col = vc WITH protect, noconstant(cnvtupper( $2))
 DECLARE dcsr_fname = vc WITH protect, noconstant( $3)
 DECLARE dcsr_user_name = vc WITH protect, noconstant(trim(cnvtupper(currdbuser)))
 DECLARE dcsr_database_name = vc WITH protect, noconstant(trim(cnvtupper(currdbname)))
 DECLARE dcsr_sql_str = vc WITH protect, noconstant("")
 DECLARE dcsr_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsr_cnt2 = i4 WITH protect, noconstant(0)
 DECLARE dcsr_cnt3 = i4 WITH protect, noconstant(0)
 DECLARE dcsr_load_actual_col_values(dlacv_tbl_ndx=i4,dlacv_col_ndx=i4) = i2
 DECLARE dcsr_load_dict_col_values(dldcv_tbl_ndx=i4,dldcv_col_ndx=i4) = i2
 DECLARE dcsr_load_actual_tbl_info(dlati_ndx=i4) = i2
 DECLARE dcsr_load_dict_tbl_info(dlati_ndx=i4) = i2
 DECLARE dcsr_get_tab_space(dgts_owner=vc,dgts_name=vc,dgts_type=vc,dgts_tot_blks=f8,dgts_unused_blks
  =f8,
  dgts_used_blks=f8) = null WITH sql = "dm_dbms_stats.get_space_info"
 IF (check_error(dm_err->eproc)=1)
 ...
 IF ((dm2_rdbms_version->level1 < 11))
  SET dm_err->eproc = "Dm2_cbo_selectivity_rpt used only for Oracle 11 and above. Exiting."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (dcsr_fname="MINE")
  ...
  CALL text(3,2,"Table_Name:")
  CALL text(4,2,"Column_Name:")
  CALL accept(3,14,"P(30);CU"
   WHERE curaccept > " ")
  SET dcsr_tab = curaccept
  CALL accept(4,14,"P(30);CU"
   WHERE curaccept > " ")
  SET dcsr_col = curaccept
  SET message = nowindow
  CALL clear(1,1)
 ENDIF
 SET dm_err->eproc = "Verify Table and Column name entered."
 SELECT INTO "nl:"
  FROM user_tab_columns ut,
   user_tables t
  WHERE ut.table_name=patstring(dcsr_tab)
   AND ut.column_name=patstring(dcsr_col)
   AND ut.data_type IN ("NUMBER", "FLOAT", "CHAR", "VARCHAR", "VARCHAR2",
  "ROWID", "DATE")
   AND ut.table_name=t.table_name
  ORDER BY ut.table_name, ut.column_name
  HEAD ut.table_name
   dcsr_data->cnt = (dcsr_data->cnt+ 1), stat = alterlist(dcsr_data->tab,dcsr_data->cnt), dcsr_data->
   tab[dcsr_data->cnt].tab_name = ut.table_name
  DETAIL
   dcsr_data->tab[dcsr_data->cnt].col_cnt = (dcsr_data->tab[dcsr_data->cnt].col_cnt+ 1), stat =
   alterlist(dcsr_data->tab[dcsr_data->cnt].col,dcsr_data->tab[dcsr_data->cnt].col_cnt), dcsr_data->
   tab[dcsr_data->cnt].col[dcsr_data->tab[dcsr_data->cnt].col_cnt].col_name = ut.column_name,
   dcsr_data->tab[dcsr_data->cnt].col[dcsr_data->tab[dcsr_data->cnt].col_cnt].data_type = ut
   .data_type
   CASE (ut.data_type)
    OF "NUMBER":
    OF "FLOAT":
     dcsr_data->tab[dcsr_data->cnt].col[dcsr_data->tab[dcsr_data->cnt].col_cnt].ccl_data_type = "F8"
    OF "CHAR":
    OF "VARCHAR":
    OF "VARCHAR2":
    OF "ROWID":
     dcsr_data->tab[dcsr_data->cnt].col[dcsr_data->tab[dcsr_data->cnt].col_cnt].ccl_data_type = "VC"
    OF "DATE":
     dcsr_data->tab[dcsr_data->cnt].col[dcsr_data->tab[dcsr_data->cnt].col_cnt].ccl_data_type = "DQ8"
   ENDCASE
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(dcsr_data)
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = concat("Please verify that the Table and Column exists in the database. (",
   dcsr_tab,".",dcsr_col,") does not exist in ",
   trim(cnvtupper(currdbname)),".")
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 FOR (dcsr_cnt = 1 TO dcsr_data->cnt)
   IF (dcsr_load_actual_tbl_info(dcsr_cnt)=0)
    GO TO exit_program
   ENDIF
   FOR (dcsr_cnt2 = 1 TO dcsr_data->tab[dcsr_cnt].col_cnt)
     IF ((dcsr_data->tab[dcsr_cnt].act_tab_blk_below_hwm > 0))
      IF (dcsr_load_actual_col_values(dcsr_cnt,dcsr_cnt2)=0)
       GO TO exit_program
      ENDIF
     ENDIF
   ENDFOR
   IF (dcsr_load_dict_tbl_info(dcsr_cnt)=0)
    GO TO exit_program
   ENDIF
   FOR (dcsr_cnt2 = 1 TO dcsr_data->tab[dcsr_cnt].col_cnt)
     IF ((dcsr_data->tab[dcsr_cnt].dict_tab_blk_below_hwm > 0)
      AND (dcsr_data->tab[dcsr_cnt].dict_num_rows > 0))
      IF (dcsr_load_dict_col_values(dcsr_cnt,dcsr_cnt2)=0)
       GO TO exit_program
      ENDIF
     ENDIF
   ENDFOR
 ENDFOR
 SET dm_err->eproc = "Generating Selectivity Report."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO value(dcsr_fname)
  FROM (dummyt d  WITH seq = 1)
  HEAD REPORT
   CALL print("SELECTIVITY REPORT"),
   CALL center(concat("Report Date/Time: ",format(cnvtdatetime(curdate,curtime3),
     "DD-MMM-YY HH:MM:SS;;D")),1,132), row + 2,
   CALL print(concat("User Name        : ",dcsr_user_name)), row + 1,
   CALL print(concat("Database Name    : ",dcsr_database_name)),
   row + 2
   FOR (dcsr_cnt = 1 TO dcsr_data->cnt)
     CALL print(fillstring(132,"*")), row + 1,
     CALL print(dcsr_data->tab[dcsr_cnt].tab_name),
     row + 1,
     CALL print(fillstring(132,"*")), row + 2,
     CALL print("Actual Distribution"), row + 2,
     CALL print("Table blocks below HWM"),
     col 30,
     CALL print("Table Rows"), row + 1,
     CALL print("         (B)          "), col 30,
     CALL print("    (R)    "),
     row + 1,
     CALL print(fillstring(29,"-")), col 30,
     CALL print(fillstring(29,"-")), row + 1
     IF ((dcsr_data->tab[dcsr_cnt].act_tab_null_ind=1))
      CALL print("NO STAT")
     ELSE
      CALL print(trim(cnvtstring(dcsr_data->tab[dcsr_cnt].act_tab_blk_below_hwm,17,0)))
     ENDIF
     col 30,
     CALL print(trim(cnvtstring(dcsr_data->tab[dcsr_cnt].act_num_rows,17,0))), row + 2
     FOR (dcsr_cnt2 = 1 TO dcsr_data->tab[dcsr_cnt].col_cnt)
       IF (dcsr_cnt2 != 1)
        row + 2
       ENDIF
       col 32,
       CALL print("Block Selectivity"), col 52,
       CALL print("Block Count"), col 67,
       CALL print("Row Selectivity"),
       col 87,
       CALL print("Row Count"), row + 1,
       col 0,
       CALL print(dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].col_name), col 32,
       CALL print("   (pb = b/B)    "), col 52,
       CALL print("    (b)    "),
       col 67,
       CALL print("   (pr = r/R)    "), col 87,
       CALL print("  (r)   "), row + 1, col 0,
       CALL print(fillstring(31,"-")), col 32,
       CALL print(fillstring(19,"-")),
       col 52,
       CALL print(fillstring(14,"-")), col 67,
       CALL print(fillstring(19,"-")), col 87,
       CALL print(fillstring(19,"-")),
       row + 1
       IF ((dcsr_data->tab[dcsr_cnt].act_tab_null_ind=1))
        row + 1,
        CALL print("Table statistics must be gathered before selectivity data can be reported."), row
         + 1
       ELSEIF ((((dcsr_data->tab[dcsr_cnt].act_tab_blk_below_hwm=0)) OR ((dcsr_data->tab[dcsr_cnt].
       act_num_rows=0))) )
        row + 1,
        CALL print("Table must have a NUM_ROWS or BLOCKS value > 0 for selectivity to be reported."),
        row + 1
       ELSE
        FOR (dcsr_cnt3 = 1 TO dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].act_cnt)
          col 0,
          CALL print(substring(1,29,dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].act[dcsr_cnt3].col_val)),
          col 32,
          CALL print(concat(trim(cnvtstring(((dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].act[dcsr_cnt3].
             blocks_per_key/ dcsr_data->tab[dcsr_cnt].act_tab_blk_below_hwm) * 100.0),17,2)),"%")),
          col 52,
          CALL print(trim(cnvtstring(dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].act[dcsr_cnt3].
            blocks_per_key))),
          col 67,
          CALL print(concat(trim(cnvtstring(((dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].act[dcsr_cnt3].
             rows_per_key/ dcsr_data->tab[dcsr_cnt].act_num_rows) * 100.0),17,2)),"%")), col 87,
          CALL print(trim(cnvtstring(dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].act[dcsr_cnt3].
            rows_per_key))), row + 1
        ENDFOR
       ENDIF
     ...
     row + 1,
     CALL print(fillstring(132,"-")), row + 1,
     CALL print("Dictionary Statistics"), row + 2,
     CALL print("Table blocks"),
     col 30,
     CALL print("Table Rows"), row + 1,
     CALL print("    (B)     "), col 30,
     CALL print("    (R)    "),
     ...
     CALL print(fillstring(29,"-")), col 30,
     CALL print(fillstring(29,"-")), row + 1
     IF ((dcsr_data->tab[dcsr_cnt].dict_tab_null_ind=1))
      CALL print("NO STAT"), col 30,
      CALL print("NO STAT")
     ELSE
      CALL print(trim(cnvtstring(dcsr_data->tab[dcsr_cnt].dict_tab_blk_below_hwm,17,0))), col 30,
      CALL print(trim(cnvtstring(dcsr_data->tab[dcsr_cnt].dict_num_rows,17,0)))
     ENDIF
     row + 2
     FOR (dcsr_cnt2 = 1 TO dcsr_data->tab[dcsr_cnt].col_cnt)
       IF (dcsr_cnt2 != 1)
       ...
       ENDIF
       col 0,
       CALL print(concat("Histogram :",dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].histogram)), row + 1,
       col 32,
       CALL print("Row Selectivity"), col 52,
       CALL print("Row Count"), row + 1, col 0,
       CALL print(dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].col_name), col 32,
       CALL print("   (pr = r/R)    "),
       col 52,
       CALL print("  (r)   "), row + 1,
       col 0,
       CALL print(fillstring(31,"-")), col 32,
       CALL print(fillstring(19,"-")), col 52,
       CALL print(fillstring(19,"-")),
       row + 1
       IF ((dcsr_data->tab[dcsr_cnt].dict_tab_null_ind=1))
        row + 1,
        CALL print("Table statistics must be gathered before selectivity data can be reported."), row
         + 1
       ELSEIF ((((dcsr_data->tab[dcsr_cnt].dict_tab_blk_below_hwm=0)) OR ((dcsr_data->tab[dcsr_cnt].
       dict_num_rows=0))) )
        row + 1,
        CALL print("Table must have a NUM_ROWS or BLOCKS value > 0 for selectivity to be reported."),
        row + 1
       ELSEIF ((dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].histogram != "FREQUENCY"))
        CALL print("No values displayed for this type of HISTOGRAM."), row + 2
       ELSE
        FOR (dcsr_cnt3 = 1 TO dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].dict_cnt)
          col 0,
          CALL print(substring(1,29,dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].dict[dcsr_cnt3].col_val)),
          col 32,
          CALL print(concat(trim(cnvtstring(((dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].dict[dcsr_cnt3]
             .rows_per_key/ dcsr_data->tab[dcsr_cnt].dict_num_rows) * 100.0),17,2)),"%")), col 52,
          CALL print(trim(cnvtstring(dcsr_data->tab[dcsr_cnt].col[dcsr_cnt2].dict[dcsr_cnt3].
            rows_per_key))),
          row + 1
        ENDFOR
       ENDIF
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE dcsr_load_dict_col_values(dldcv_tbl_ndx,dldcv_col_ndx)
   DECLARE dldcv_cnt = i4 WITH protect, noconstant(0)
   IF (dcr_init_dict_hist_vals(null)=0)
    RETURN(0)
   ENDIF
   SET dcr_hist_vals->tbl_name = dcsr_data->tab[dldcv_tbl_ndx].tab_name
   SET dcr_hist_vals->col_name = dcsr_data->tab[dldcv_tbl_ndx].col[dldcv_col_ndx].col_name
   SET dcr_hist_vals->ccl_data_type = dcsr_data->tab[dldcv_tbl_ndx].col[dldcv_col_ndx].ccl_data_type
   IF (dcr_get_dict_hist_vals(null)=0)
    RETURN(0)
   ENDIF
   SET dcsr_data->tab[dldcv_tbl_ndx].col[dldcv_col_ndx].dict_cnt = dcr_hist_vals->hist_cnt
   SET stat = alterlist(dcsr_data->tab[dldcv_tbl_ndx].col[dldcv_col_ndx].dict,dcr_hist_vals->hist_cnt
    )
   SET dcsr_data->tab[dldcv_tbl_ndx].col[dldcv_col_ndx].histogram = dcr_hist_vals->hist_type
   FOR (dldcv_cnt = 1 TO dcr_hist_vals->hist_cnt)
    SET dcsr_data->tab[dldcv_tbl_ndx].col[dldcv_col_ndx].dict[dldcv_cnt].col_val = dcr_hist_vals->
    hist[dldcv_cnt].col_val
    SET dcsr_data->tab[dldcv_tbl_ndx].col[dldcv_col_ndx].dict[dldcv_cnt].rows_per_key = dcr_hist_vals
    ->hist[dldcv_cnt].rows_per_key
   ENDFOR
   IF ((dm_err->debug_flag > 1))
    CALL echorecord(dcsr_data)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcsr_load_actual_col_values(dlacv_tbl_ndx,dlacv_col_ndx)
   DECLARE dlacv_cnt = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = concat("Gather column values for ",dcsr_data->tab[dlacv_tbl_ndx].tab_name,".",
    dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].col_name,
    ". This operation will potentially take a significant amount of time.")
   CALL disp_msg(" ",dm_err->logfile,0)
   CALL dm2_push_cmd(concat("select into 'nl:' d.dist_blks_per_key,d.rows_per_key,d.",dcsr_data->tab[
     dlacv_tbl_ndx].col[dlacv_col_ndx].col_name,",d.total_rows from "),0)
   CALL dm2_push_cmd(" ((select ",0)
   CALL dm2_push_cmd(concat("   u.",dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].col_name,","),0)
   CALL dm2_push_cmd(
    "   dist_blks_per_key = sqlpassthru('count(distinct(dbms_rowid.rowid_block_number(rowid)))'),",0)
   CALL dm2_push_cmd("   rows_per_key = count(*),",0)
   CALL dm2_push_cmd("   total_rows = sum(count(*)) over ()",0)
   CALL dm2_push_cmd(concat("   from ",dcsr_data->tab[dlacv_tbl_ndx].tab_name," u "),0)
   CALL dm2_push_cmd(concat("   where u.",dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].col_name,
     " is not null"),0)
   CALL dm2_push_cmd(concat("   group by u.",dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].
     col_name),0)
   CALL dm2_push_cmd(concat("   order by dist_blks_per_key desc,rows_per_key desc,u.",dcsr_data->tab[
     dlacv_tbl_ndx].col[dlacv_col_ndx].col_name),0)
   CALL dm2_push_cmd(concat("   desc with sqltype ('",dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx
     ].ccl_data_type,"','F8','F8','F8')) d)"),0)
   CALL dm2_push_cmd("head report",0)
   CALL dm2_push_cmd("  dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].act_cnt = 0 ",0)
   CALL dm2_push_cmd("  dlacv_cnt = dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].act_cnt",0)
   CALL dm2_push_cmd("  stat = alterlist(dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].act,20)",0)
   CALL dm2_push_cmd("detail ",0)
   CALL dm2_push_cmd("  dlacv_cnt = dlacv_cnt + 1",0)
   CALL dm2_push_cmd("  dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].act[dlacv_cnt].col_val = ",0
    )
   CASE (dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].ccl_data_type)
    OF "DQ8":
     CALL dm2_push_cmd(concat("format(cnvtdatetime(d.",dcsr_data->tab[dlacv_tbl_ndx].col[
       dlacv_col_ndx].col_name,"),';;q')"),0)
    OF "F8":
     CALL dm2_push_cmd(concat("trim(cnvtstring(d.",dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].
       col_name,",17,2))"),0)
    ELSE
     CALL dm2_push_cmd(concat("d.",dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].col_name),0)
   ENDCASE
   CALL dm2_push_cmd("  dcsr_data->tab[dlacv_tbl_ndx].act_num_rows = d.total_rows",0)
   CALL dm2_push_cmd(
    "  dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].act[dlacv_cnt].rows_per_key = d.rows_per_key",
    0)
   CALL dm2_push_cmd(
    "  dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].act[dlacv_cnt].blocks_per_key = d.dist_blks_per_key",
    0)
   CALL dm2_push_cmd("foot report",0)
   CALL dm2_push_cmd("  dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].act_cnt = dlacv_cnt",0)
   CALL dm2_push_cmd("  stat = alterlist(dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].act,",0)
   CALL dm2_push_cmd("  dcsr_data->tab[dlacv_tbl_ndx].col[dlacv_col_ndx].act_cnt)",0)
   IF (dm2_push_cmd("with nocounter,nullreport,maxqual(d,20) go",1)=0)
    RETURN(0)
   ENDIF
   IF ((dm_err->debug_flag > 1))
    CALL echorecord(dcsr_data)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcsr_load_dict_tbl_info(dldti_ndx)
   SET dm_err->eproc = "Obtain blocks below HWM for Table from dictionary."
   SELECT INTO "nl:"
    FROM user_tables d
    WHERE (d.table_name=dcsr_data->tab[dldti_ndx].tab_name)
    HEAD REPORT
     dcsr_data->tab[dldti_ndx].dict_tab_null_ind = 1
    DETAIL
     dcsr_data->tab[dldti_ndx].dict_tab_blk_below_hwm = d.blocks, dcsr_data->tab[dldti_ndx].
     dict_num_rows = d.num_rows, dcsr_data->tab[dldti_ndx].dict_tab_null_ind = nullind(d.num_rows)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcsr_load_actual_tbl_info(dlati_ndx)
   DECLARE dlati_num1 = f8 WITH protect, noconstant(0.0)
   DECLARE dlati_num2 = f8 WITH protect, noconstant(0.0)
   DECLARE dlati_num3 = f8 WITH protect, noconstant(0.0)
   SET dm_err->eproc = "Obtain space usage information for Table."
   CALL dcsr_get_tab_space(trim(cnvtupper(currdbuser)),dcsr_data->tab[dlati_ndx].tab_name,"TABLE",
    dlati_num1,dlati_num2,
    dlati_num3)
   SET dcsr_data->tab[dlati_ndx].act_tab_blocks = dlati_num1
   SET dcsr_data->tab[dlati_ndx].act_tab_unused_blocks = dlati_num2
   SET dcsr_data->tab[dlati_ndx].act_tab_used_blocks = dlati_num3
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Obtain blocks below HWM for Table."
   SELECT INTO "nl:"
    FROM dm_user_tables_actual_stats d
    WHERE (d.table_name=dcsr_data->tab[dlati_ndx].tab_name)
    HEAD REPORT
     dcsr_data->tab[dlati_ndx].act_tab_null_ind = 1
    DETAIL
     dcsr_data->tab[dlati_ndx].act_tab_blk_below_hwm = d.blocks, dcsr_data->tab[dlati_ndx].
     act_tab_null_ind = nullind(d.num_rows)
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Dm2_cbo_selectivity_rpt completed"
 CALL final_disp_msg("dm2cboselrpt")
END GO
