CREATE PROGRAM dm2_mig_validate_all_data
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmv_num = i4 WITH protect, noconstant(0)
 DECLARE dmvad_excl_clause = vc WITH protect, noconstant("1 = 1")
 DECLARE dmvad_mode = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_mig_validate_all",".log","dm2_mig_validate_all LOGFILE")=0)
  GO TO exit_script
 ENDIF
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Check for FINAL TRANSACTION row in source DM_INFO"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  ...
  WHERE d.info_domain="DATABASE MIGRATION"
   AND d.info_char="FINAL TRANSACTION"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual > 0)
  SET dmvad_mode = "DOWNTIME"
 ELSE
  SET dmvad_mode = "UPTIME"
 ENDIF
 SET dm_err->eproc = concat("Executing in ",dmvad_mode," mode")
 CALL disp_msg("",dm_err->logfile,0)
 ...
 ELSEIF ((dmr_user_list->cnt=0))
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Creating ccluserdir:dm2_mig_validate_tables.txt"
 ...
 IF ((dm2_sch_except->tcnt > 0))
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    IF (d.seq=1)
     dmvad_excl_clause = concat("dtc.table_name NOT IN ('",dm2_sch_except->tbl[d.seq].tbl_name,"'")
    ELSE
     dmvad_excl_clause = concat(dmvad_excl_clause,",'",dm2_sch_except->tbl[d.seq].tbl_name,"'")
    ENDIF
   FOOT REPORT
    dmvad_excl_clause = concat(dmvad_excl_clause,")")
   WITH nocounter, nullreport
  ;end select
  IF (check_error(dm_err->eproc))
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF (dmvad_mode="UPTIME")
  SELECT DISTINCT INTO "ccluserdir:dm2_mig_validate_tables.txt"
   x = concat(trim(dtc.owner),".",trim(dtc.table_name))
   FROM dba_tab_columns dtc,
    dba_ind_columns dic
   WHERE dtc.table_name=dic.table_name
    AND dtc.column_name=dic.column_name
    AND trim(dtc.owner)=dic.table_owner
    AND trim(dtc.data_type) IN ("NUMBER", "FLOAT")
   ...
   (SELECT
    ...
    WHERE dic2.index_name=di.index_name
     ...
     AND trim(dic2.table_owner)=di.owner
     AND expand(dmv_num,1,dmr_user_list->cnt,trim(dic2.table_owner),dmr_user_list->qual[dmv_num].user
     )
    GROUP BY dic2.index_name, dic2.table_owner
    HAVING count(dic2.column_name)=1))
    AND parser(dmvad_excl_clause)
    AND  NOT (concat(dtc.owner,".",dtc.table_name) IN (
   (SELECT
    di.info_name
    FROM dm_info di
 ...
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Executing DM2_VALIDATE_DATA"
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 EXECUTE dm2_validate_data  $1, 1000, "ccluserdir:dm2_mig_validate_tables.txt"
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_validate_data completed successfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
