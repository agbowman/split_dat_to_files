CREATE PROGRAM ct_mp_get_pt_prescreen_list:dba
 PROMPT
 ...
 RECORD ct_get_pt_prescreen_list_reply(
   1 prescreenlist[*]
 ...
 RECORD PUBLIC::"reply"(
   1 "prescreen_list"
     2 "protocolId" = f8
     2 "manually_added_column_exists" = i2
     2 "total_cnt" = i4
     2 "prescreen_info"[*]
       ...
       3 "person_id" = f8
       3 "prot_alias" = vc
       3 "prescreening_date" = vc
       3 "prescreener_full_name" = vc
       3 "prescreening_status_cd" = f8
       3 "prescreening_status_disp" = vc
       3 "prescreening_status_mean" = vc
       3 "referral_date" = vc
       3 "referral_full_name" = vc
       3 "added_via_flag" = i2
       3 "MRNs"[*]
         4 "MRN" = vc
         4 "org_name" = vc
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_pt_prescreen_list"
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getptprescreenlist(null) = null WITH protect
 DECLARE getprescreeningstatus(null) = null WITH protect
 DECLARE prescreencount = i4 WITH protect, noconstant(0)
 DECLARE prescreenindex = i4 WITH protect, noconstant(0)
 DECLARE mrnsize = i4 WITH protect, noconstant(0)
 DECLARE mrnindex = i4 WITH protect, noconstant(0)
 SET public::"reply"->status_data.status = "F"
 IF (((( $PROTOCOLID=0)) OR (( $AMENDMENTID=0))) )
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_pt_prescreen_list",
   "Invalid/Partial Input Data",PUBLIC::"reply")
  GO TO exit_script
 ENDIF
 CALL getptprescreenlist(null)
 SET public::"reply"->status_data.status = "S"
 SUBROUTINE getptprescreenlist(null)
   CALL log_message("In GetPtPrescreenList()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   IF (( $PROTOCOLID > 0))
    SET stat = alterlist(request->protocols,1)
    SET request->protocols[1].protocolid =  $PROTOCOLID
    SET request->row_start_num =  $ROWSTART
    SET request->row_end_num =  $ROWEND
    CALL getprescreeningstatus(null)
    EXECUTE ct_mp_get_prescreen_patient  WITH replace("REPLY","CT_GET_PT_PRESCREEN_LIST_REPLY")
    IF ((ct_get_pt_prescreen_list_reply->status_data.status="S"))
     SET prescreencount = size(ct_get_pt_prescreen_list_reply->prescreenlist,5)
     IF (prescreencount > 0)
      SET stat = alterlist(public::"reply"->"prescreen_list"."prescreen_info",prescreencount)
      SET public::"reply"->"prescreen_list"."manually_added_column_exists" =
      ct_get_pt_prescreen_list_reply->manually_added_column_exists
      SET public::"reply"->"prescreen_list"."total_cnt" = ct_get_pt_prescreen_list_reply->total_cnt
      FOR (prescreenindex = 1 TO prescreencount)
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."pt_prot_prescreen_id"
         = ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].pt_prot_prescreen_id
        SET public::"reply"->"prescreen_list"."protocolId" = ct_get_pt_prescreen_list_reply->
        prescreenlist[prescreenindex].prot_master_id
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."full_name" =
        ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].full_name
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."last_name" =
        ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].last_name
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."first_name" =
        ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].first_name
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."person_id" =
        ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].person_id
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."prot_alias" =
        ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].prot_alias
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."prescreener_full_name"
         = ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].screener_full_name
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."prescreening_status_cd"
         = ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].screening_status_cd
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex].
        "prescreening_status_disp" = uar_get_code_display(ct_get_pt_prescreen_list_reply->
         prescreenlist[prescreenindex].screening_status_cd)
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex].
        "prescreening_status_mean" = uar_get_code_meaning(ct_get_pt_prescreen_list_reply->
         prescreenlist[prescreenindex].screening_status_cd)
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."referral_full_name" =
        ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].referral_full_name
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."prescreening_date" =
        getshortdate4yrtimeformat(ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].
         screening_dt_tm)
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."referral_date" =
        getshortdate4yrtimeformat(ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].
         referral_dt_tm)
        SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."added_via_flag" =
        ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].added_via_flag
        SET mrnsize = size(ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].mrns,5)
        SET stat = alterlist(public::"reply"->"prescreen_list".prescreen_info[prescreenindex]."MRNs",
         mrnsize)
        IF (mrnsize > 0)
         FOR (mrnindex = 1 TO mrnsize)
           SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex].mrns[mrnindex].
           "org_name" = ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].mrns[mrnindex].
           orgname
           SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex].mrns[mrnindex].
           "alias_pool_cd" = ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].mrns[
           mrnindex].alias_pool_cd
           IF (trim(uar_get_code_display(ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex
             ].mrns[mrnindex].alias_pool_cd)) != "")
            SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex].mrns[mrnindex].
            "alias_pool_disp" = concat("-",uar_get_code_display(ct_get_pt_prescreen_list_reply->
              prescreenlist[prescreenindex].mrns[mrnindex].alias_pool_cd))
           ENDIF
           SET public::"reply"->"prescreen_list".prescreen_info[prescreenindex].mrns[mrnindex]."MRN"
            = concat(ct_get_pt_prescreen_list_reply->prescreenlist[prescreenindex].mrns[mrnindex].mrn,
            public::"reply"->"prescreen_list".prescreen_info[prescreenindex].mrns[mrnindex].
            "alias_pool_disp")
         ENDFOR
        ENDIF
      ENDFOR
     ELSE
      SET public::"reply"->status_data.status = "Z"
      CALL populate_subeventstatus_rec("REC_SIZE","Z","ct_mp_get_pt_prescreen_list",
       "Did not find any prescreened patients for protocol.",PUBLIC::"reply")
     ENDIF
    ELSE
     CALL populate_subeventstatus_rec("EXECUTE",ct_get_pt_prescreen_list_reply->status_data.status,
      "ct_mp_get_pt_prescreen_list","Failed in CT_MP_GET_PRESCREEN_PATIENT_LIST prg",PUBLIC::"reply")
    ENDIF
   ELSE
    CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_pt_prescreen_list",
     "Did not receive any protocol ID",PUBLIC::"reply")
   ENDIF
   CALL log_message(build("Exit GetPtPrescreenList(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (getshortdate4yrtimeformat(datetime=dq8) =vc WITH protect)
   DECLARE returnvar = vc WITH protect
   IF (datetime < cnvtdatetime("31-DEC-2100 00:00:00")
    AND datetime > cnvtdatetime("00/00/0000 00:00:00"))
    SET returnvar = concat(format(datetime,"@SHORTDATE4YR")," ",cnvtupper(format(datetime,
       "@TIMENOSECONDS;;S")))
   ENDIF
   RETURN(returnvar)
 END ;Subroutine
 SUBROUTINE getprescreeningstatus(null)
   CALL log_message("In GetPrescreeningStatus()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE statuscount = i2 WITH protect, noconstant(0)
   SELECT
    FROM code_value cv
    PLAN (cv
     WHERE cv.code_set=17901
      AND cv.active_ind=1)
    HEAD REPORT
     statuscount = 0
    DETAIL
     statuscount += 1
     IF (statuscount > size(request->statuslist,5))
      stat = alterlist(request->statuslist,(statuscount+ 9))
     ENDIF
     request->statuslist[statuscount].status_cd = cv.code_value
    FOOT REPORT
     stat = alterlist(request->statuslist,statuscount)
    WITH nocounter
   ;end select
   CALL error_and_zero_check_rec(curqual,"CODE_VALUE","GetPrescreeningStatus",1,1,
    PUBLIC::"reply")
   CALL log_message(build("Exit GetPrescreeningStatus(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (g_debug_ind=1)
 ...
 FREE RECORD request
 FREE RECORD ct_get_pt_prescreen_list_reply
