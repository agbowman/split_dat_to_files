CREATE PROGRAM dm_sch_def_apply_rows:dba
 ...
 DECLARE g_rows_between_commit = i4 WITH protect, noconstant(50)
 ...
 IF (v_days_to_keep < g_minimum_keep_days)
  ...
  SET reply->status_data.status = "F"
 ELSE
  SET reply->table_name = "SCH_DEF_APPLY"
  SET reply->rows_between_commit = g_rows_between_commit
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(sda.def_apply_id)
    FROM sch_def_apply sda
    WHERE sda.def_apply_id > 0
    DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     sda.rowid
     FROM sch_def_apply sda
     WHERE sda.def_apply_id BETWEEN curminid AND curmaxid
      AND sda.beg_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
      AND parser(sbr_getrowidnotexists("sda.version_dt_tm = cnvtdatetime('31-DEC-2100 00:00:00.00')",
       "sda"))
      AND  NOT ( EXISTS (
     (SELECT
      sal.apply_list_id
      FROM sch_apply_list sal
      WHERE sal.def_apply_id=sda.def_apply_id
     ...
     DETAIL
      ...
      ENDIF
      reply->rows[rows].row_id = sda.rowid
     WITH nocounter, maxqual(sda,value(rowsleft))
    ;end select
    SET v_err_code2 = error(v_errmsg2,1)
    IF (v_err_code2 > 0)
     SET reply->err_code = v_err_code2
     SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1","Failed in row selection: %1","s",
      nullterm(v_errmsg2))
     SET reply->status_data.status = "F"
     GO TO exit_script
    ENDIF
