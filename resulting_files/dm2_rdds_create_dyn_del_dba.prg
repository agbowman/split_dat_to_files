CREATE PROGRAM dm2_rdds_create_dyn_del:dba
 ...
 SUBROUTINE dm2_findfile(sbr_file_path)
   ...
   IF ((dm2_sys_misc->cur_os="AXP"))
    ...
    ENDIF
   ELSE
    SET dff_cmd_txt = concat("test -f ",sbr_file_path,";echo $?")
 ...
 SUBROUTINE dm2_fill_seq_list(alias,col_name)
   ...
   IF (validate(dm2_install_schema->process_option," ")="CLIN COPY")
 ...
 IF (check_logfile("dm2_rdds_create",".log","DM2_RDDS_CREATE LOG")=0)
  ...
  GO TO exit_main
 ENDIF
 DECLARE dyn_f_name = vc
 DECLARE dyn_loop = i4
 DECLARE dyn_pos = i4
 DECLARE dyn_dumb_cnt = i4
 DECLARE dyn_pk_cnt = i4
 CALL get_unique_file("DM2_REF_DATA_MOVER_",".DAT")
 SET dyn_f_name = dm_err->unique_fname
 ...
 CALL parser(concat("drop program dm2_ref_data_mover_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
   suffix,":dba go"),1)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 0
 ENDIF
 SELECT INTO value(dyn_f_name)
  FROM (dummyt d  WITH seq = value(perm_col_cnt))
  HEAD REPORT
   row 1, col 1, "drop program dm2_ref_data_mover_",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, " go", row + 1,
   "create program dm2_ref_data_mover_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, row + 1,
   "declare cust_catalog_cd_s = f8", row + 1, "declare cust_catalog_cd = f8",
   row + 1, "declare comp_seq_cnt = i4", row + 1,
   "declare cust_tab_name_s = vc", row + 1, "declare cust_cs_loop = i4",
   row + 1, "declare cust_col_cnt = i4", row + 1,
   "declare cust_col_num = i4", row + 1, "declare cust_check_loop = i4",
   row + 1, "declare cust_nodelete_ind = i2", row + 1,
   "declare cust_chg_log = vc", row + 1, "declare cust_merge_loop = i4",
   row + 1, "declare cust_index_var = i4", row + 1,
   "declare cust_cur_merges = i4", row + 1, "declare cust_fail_merges = i4",
   row + 1, "declare cust_del_msg = vc", row + 1,
   "declare cust_sel_ret = vc", row + 1, "declare cust_where_clause  =vc",
   row + 1, "declare cust_parser_cnt = i4", row + 1,
   "declare cust_col_name = vc", row + 1, "declare passed_var = vc",
   row + 1, "declare drdm_dmt_tab = vc", row + 1,
   "declare cust_loop = i4", row + 1, "declare cust_from_val = f8",
   row + 1, "declare cust_to_val = f8", row + 1,
   "declare cust_from_str =vc", row + 1, "declare chg_log_smry_name =vc",
   row + 1, "declare cust_eq_cnt = i4", row + 1,
   "declare cust_sp_cnt = i4", row + 1, "declare except_tab = vc",
   row + 1, "declare except_log_type = vc", row + 1,
   "declare rpt_table = vc", row + 1, "declare rpt_column = vc",
   row + 1, "declare rpt_from = f8", row + 1,
   "declare vc_from_val = vc", row + 1, "declare orphan_ind = i2",
   row + 1, "declare err_msg = vc", row + 1,
   "declare err_ind = i2", row + 1, "declare dd_col_pos = i4",
   row + 1, "declare dd_col_loop = i4", row + 1,
   "declare dd_data_type = vc", row + 1, "declare pe_name = vc",
   row + 1, "declare pe_srch = i4", row + 1,
   "declare pe_fnd = i4", row + 1, "declare drdm_parent_col = vc",
   row + 1, "declare dd_i_domain = vc", row + 1,
   "declare dd_i_name = vc", row + 1, "declare oldver_ind = i2",
   row + 1, "declare spec_log_type = vc", row + 1,
   "declare space_loop = i4", row + 1, "declare space_size = i4",
   row + 1, "declare rlbk_orphan_ind = i2", row + 1,
   "declare rcdd_imt_check = i2", row + 1,
   ";this is used to store the sequence name and numbers so that hits on dm_info are limited",
   row + 1, "if (validate(drdm_sequence->qual[1].seq_val, -1) < 0)", row + 1,
   "   free record drdm_sequence", row + 1, "   record drdm_sequence (",
   row + 1, "   1 qual[*]", row + 1,
   "     2 seq_name = vc", row + 1, "     2 seq_val = f8)",
   row + 1, "endif ", row + 1,
   "free record cust_cs_rows", row + 1, "record cust_cs_rows (",
   row + 1, "1 qual[*]", row + 1
  DETAIL
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].pk_ind=1))
    "  2 ", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = ",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].data_type, row + 1, dyn_pk_cnt = (
    dyn_pk_cnt+ 1),
    stat = alterlist(pks->qual,(size(pks->qual,5)+ 1)), pks->qual[size(pks->qual,5)].column_name =
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name
   ENDIF
  FOOT REPORT
   "  2 trans_ind = i2)", row + 1, row + 1,
   "declare cs_Insert(null) = null", row + 1, "declare cs_merge_audit(action = vc, text= vc) = null",
   row + 1, "declare ddinsert_merge_translate(sbr_from = f8 , sbr_to = f8, sbr_table = vc) = i2", row
    + 1,
   "declare ddselect_merge_translate(sbr_f_value = vc, sbr_t_name = vc) = vc", row + 1,
   "declare dd_del_except(sbr_table_name = vc, sbr_value = f8) = null",
   row + 1, "declare cs_find_p_e_col(cs_p_e_name = vc, cs_p_e_col = i4) = vc", row + 1,
   "declare ddFILL_RS(Type = vc, Info = vc) = i4 with Public", row + 1,
   "declare dddm2_rdds_get_tbl_col_info(sbr_gtci_tname=vc) = i2 with public",
   row + 1, "declare get_rdds_tname(sbr_tname = vc) = vc", row + 1,
   "set perm_col_cnt = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt", row + 1,
   "set cust_tab_name_s = get_rdds_tname(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name)",
   row + 1
 ...
 SELECT INTO value(dyn_f_name)
  FROM (dummyt d  WITH seq = value(perm_col_cnt))
  HEAD REPORT
   'call parser(^select into "NL:"^, 0)', row + 1,
   'call parser(concat("from ",cust_tab_name_s," t",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix), 0)',
   row + 1, "call parser(drdm_chg->log[drdm_log_loop].pk_where, 0)", row + 1,
   'call parser(" head report", 0)', row + 1, 'call parser("    comp_seq_cnt = 0", 0)',
   row + 1, 'call parser("    stat = alterlist(cust_cs_rows->qual, 10)", 0)', row + 1,
   'call parser(" detail", 0)', row + 1, 'call parser("    comp_seq_cnt = comp_seq_cnt + 1", 0)',
   row + 1, 'call parser("    if (mod(comp_seq_cnt, 10) = 1 and comp_seq_cnt != 1)", 0)', row + 1,
   'call parser("       stat = alterlist(cust_cs_rows->qual, comp_seq_cnt + 9)", 0)', row + 1,
   'call parser("    endif", 0)',
   row + 1
  DETAIL
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].pk_ind=1))
    'call parser("    cust_cs_rows->qual[comp_seq_cnt].', dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
    col_qual[d.seq].column_name, " = t",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, ".", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
    col_qual[d.seq].column_name,
    '", 0)', row + 1
   ENDIF
  FOOT REPORT
   'call parser(" foot report", 0)', row + 1,
   'call parser("    stat = alterlist(cust_cs_rows->qual, comp_seq_cnt)", 0)',
   row + 1, 'call parser(" with nocounter go", 1)', row + 1,
   ";error checking", row + 1, "if (error(err_msg, 0) > 0)",
   row + 1, "   call echo(err_msg)", row + 1,
   "   go to EXIT_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, row + 1,
   "endif", row + 1, row + 1,
   "if (curqual = 0)", row + 1, '   set cust_chg_log = get_rdds_tname("DM_CHG_LOG")',
   row + 1, "   update into (parser(cust_chg_log) d)", row + 1,
   '   set d.log_type = "NO SRC",', row + 1, "       d.updt_dt_tm = cnvtdatetime(curdate, curtime3),",
   row + 1, "       d.updt_cnt = d.updt_cnt + 1", row + 1,
   "   where d.log_id = drdm_chg->log[drdm_log_loop].log_id", row + 1, "   with nocounter",
   row + 1, "   if (error(err_msg, 0) > 0)", row + 1,
   "      call echo(err_msg)", row + 1, "      go to EXIT_",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, row + 1, "   endif",
   row + 1, "   commit", row + 1,
   "   go to EXIT_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, row + 1,
   "endif", row + 1
  WITH nocounter, maxrow = 1, format = variable,
 ...
 SELECT INTO value(dyn_f_name)
  ...
  DETAIL
   ...
   row + 1
  FOOT REPORT
   "from (parser(cust_tab_name_s) c)", row + 1
  WITH nocounter, maxrow = 1, format = variable,
   formfeed = none, append
 ;end select
 SET dyn_dumb_cnt = 0
 ...
 SELECT INTO value(dyn_f_name)
  ...
  HEAD REPORT
   row 1, col 1
  DETAIL
   "rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->from_values.",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = c.", dm2_ref_data_doc->
   tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name,
   row + 1
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].idcd_ind=0))
    "rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = c.", dm2_ref_data_doc->
    tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name,
    row + 1
   ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name="UPDT_ID"))
    "rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = 0", row + 1
   ...
   ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name="UPDT_DT_TM"))
    "rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name,
    " = cnvtdatetime(curdate, curtime3)", row + 1
   ENDIF
  ...
  FOOT REPORT
   "with nocounter", row + 1, "if (error(err_msg, 0) > 0)",
   row + 1, "   call echo(err_msg)", row + 1,
   "   go to EXIT_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, row + 1,
   "endif", row + 1, "if (curqual = 0)",
   row + 1, "   go to EXIT_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
   row + 1, "endif", row + 1
  WITH append, formfeed = none, maxrow = 1,
   format = variable
 ;end select
 DECLARE dyn_tab_name = vc
 DECLARE dyn_col_name = vc
 SELECT INTO value(dyn_f_name)
  FROM (dummyt d  WITH seq = value(perm_col_cnt))
  WHERE (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].pk_ind=1)
  HEAD REPORT
   row 1, col 1, "set rs_",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->log_id = drdm_chg->log[drdm_log_loop].log_id",
   row + 1,
   "for (space_loop = 1 to perm_col_cnt)", row + 1,
   '   if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].data_type in ("VC", "C*"))',
   row + 1, '      call parser(concat("set space_size = size(RS_", ', row + 1,
   "         dm2_ref_Data_doc->tbl_qual[temp_tbl_cnt].suffix,", row + 1, '         "->from_values.",',
   row + 1, "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].column_name,",
   row + 1,
   '         ") go"), 1)', row + 1,
   "      if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].check_null = 0 and",
   row + 1, "         space_size = 0)", row + 1,
   "         set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].check_space = 1", row
    + 1, "      else",
   row + 1,
   "         set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].check_space = 0", row
    + 1,
   "      endif", row + 1, "   else",
   row + 1, "      set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[space_loop].check_space = 0",
   row + 1,
   "   endif", row + 1, "endfor",
   row + 1, 'call echo("")', row + 1,
   'call echo("")', row + 1, 'call echo(build("Post RS = ", curmem))',
   row + 1, 'call echo("")', row + 1,
   'call echo("")', row + 1, 'call echo("**************LOOKING UP TRANSLATIONS***************")',
   row + 1, 'call echo("")', row + 1,
   'call echo("")', row + 1, "set RS_",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, '->From_table_name = "', dm2_ref_data_doc->
   tbl_qual[temp_tbl_cnt].table_name,
   '"', row + 1, "set RS_",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, '->to_table_name = "', dm2_ref_data_doc->
   tbl_qual[temp_tbl_cnt].table_name,
   '"', row + 1, "set no_insert_update = 0",
   row + 1
  DETAIL
   dyn_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name, dyn_col_name =
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].idcd_ind=0))
    passed_var = concat("set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[",trim(cnvtstring(d
       .seq)),"].translated = 1"), passed_var, row + 1
   ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].exception_flg=1))
    ';transferring "from" to "to" where columns exception_flg = 1', row + 1, "set RS_",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.", dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[d.seq].column_name,
    " = ", row + 1, "   RS_",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->from_values.", dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[d.seq].column_name,
    row + 1, "set cust_cur_merges = cust_cur_merges + 1", row + 1,
    'set child_merge_audit->num[cust_cur_merges].action = "EXCEPT1"', row + 1,
    'set child_merge_audit->num[cust_cur_merges].text = "',
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name, "     ", dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[d.seq].column_name,
    '"', row + 1, "set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[",
    d.seq, "].translated = 1", row + 2
   ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].constant_value != null))
    ';transferring constant value in "to" values if "from" constant_value isnt null', row + 1,
    "set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[",
    d.seq, "].check_null = 0", row + 1,
    "set RS_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = ", dm2_ref_data_doc->
    tbl_qual[temp_tbl_cnt].col_qual[d.seq].constant_value,
    row + 1, "set cust_cur_merges= cust_cur_merges + 1", row + 1,
    'set child_merge_audit->num[cust_cur_merges].action = "CONSTANTS"', row + 1,
    'set child_merge_audit->num[cust_cur_merges].text = "',
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name, "     ", dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[d.seq].column_name,
    '"', row + 1, "set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[",
    d.seq, "].translated = 1", row + 2
   ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].exception_flg=6))
    ";assigning a new sequence value if exception_flg = 6", row + 1, 'select into "NL:" y = seq(',
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].sequence_name, ", nextval)", row + 1,
    "from dual detail RS_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = y with nocounter", row
     + 1,
    "if (error(err_msg, 0) > 0)", row + 1, "   call echo(err_msg)",
    row + 1, "   set err_ind = 0", row + 1,
    "endif", row + 1, "set cust_cur_merges= cust_cur_merges + 1",
    row + 1, 'set child_merge_audit->num[cust_cur_merges].action = "EXCEPT6"', row + 1,
    'set child_merge_audit->num[cust_cur_merges].text = "', dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
    table_name, "     ",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, '"', row + 1,
    "set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, "].translated = 1",
    row + 2
   ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].root_entity_name != null))
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].root_entity_name=dyn_tab_name)
     AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].root_entity_attr=dyn_col_name))
     ";Looking for translations of the PK", row + 1, "set vc_from_val = cnvtstring(rs_",
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->from_values.", dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[d.seq].column_name,
     ")", row + 1, 'set cust_sel_ret = ddselect_merge_translate(vc_from_val, "',
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].root_entity_name, '")', row + 1,
     'if (cust_sel_ret != "No Trans")', row + 1, "   set found_rec = 1",
     row + 1, "   set rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
     "->to_values.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name,
     " = cnvtreal(cust_sel_ret)",
     row + 1, "  set cust_cur_merges = cust_cur_merges + 1", row + 1,
     ...
     temp_tbl_cnt].col_qual[d.seq].column_name,
     '"', row + 1, "else",
     row + 1, "   ;assigning a new sequence value to a top level primary key", row + 1,
     '   select into "NL:" y = seq(', dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].
     sequence_name, "   , nextval)",
     row + 1, "   from dual detail RS_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
     "->to_values.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name,
     "    = y with nocounter",
     row + 1, "   if (error(err_msg, 0) > 0)", row + 1,
     "      call echo(err_msg)", row + 1, "       set err_ind = 0",
     row + 1, "   endif", row + 1,
     "   set cust_cur_merges= cust_cur_merges + 1", row + 1,
     '   set child_merge_audit->num[cust_cur_merges].action = "NEWSEQ"',
     row + 1, '   set child_merge_audit->num[cust_cur_merges].text = "', dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].table_name,
     "     ", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, '"',
     row + 1, "   set vc_from_val = cnvtstring(rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
     "->from_values.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, ")",
     row + 1, "   set rcdd_imt_check=ddinsert_merge_translate(cnvtreal(RS_", dm2_ref_data_doc->
     tbl_qual[temp_tbl_cnt].suffix,
     "->from_values.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, "),",
     row + 1, "   cnvtreal(RS_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
     "->to_values.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, '), "',
     dyn_tab_name, '")', row + 1,
     "  if (rcdd_imt_check = 1)", row + 1, "     set nodelete_ind = 1",
     row + 1, "     set no_insert_update = 1", row + 1,
     "     set cust_fail_merges= cust_fail_merges + 1", row + 1,
     '     set fail_merge_audit->num[cust_fail_merges].action = "FAILREASON"',
     row + 1,
     '     set fail_merge_audit->num[cust_fail_merges].text = "Preventing a 2 into 1 translation"',
     row + 1,
     "     rollback", row + 1,
     ...
     row + 1, "     endif", row + 1,
     "     commit", row + 1, "     go to EXIT_",
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, row + 1, "  endif",
     row + 1, "endif", row + 1,
     "set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, "].translated = 1",
     row + 2
    ELSE
     "if (rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->from_values.",
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, "  != 0)", row + 1,
     "set found_rec = 0", row + 1, "set vc_from_val = cnvtstring(rs_",
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->from_values.", dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[d.seq].column_name,
     ")", row + 1, 'set cust_sel_ret = ddselect_merge_translate(vc_from_val, "',
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].root_entity_name, '")', row + 1,
     ...
     " = cnvtreal(cust_sel_ret)",
     row + 1, "endif", row + 1,
     "else", row + 1, " set found_rec = 1",
     row + 1, " set rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
     "->to_values.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = 0",
     row + 1, "endif", row + 1,
     ...
     row + 2
    ENDIF
   ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].parent_entity_col != ""))
    "if (RS_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->from_values.",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = 0)", row + 1,
    "   set RS_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = 0", row + 1,
    "   set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, "].translated = 1",
    row + 1, "else", row + 1,
    "if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, '].parent_entity_col = " ")',
    row + 1, "    set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq,
    '].parent_entity_col = ""', row + 1, "endif",
    row + 1, "set dd_col_pos = locateval(dd_col_loop, 1, perm_col_cnt, ", row + 1,
    "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, "].parent_entity_col,",
    row + 1, "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[dd_col_loop].column_name)",
    row + 1,
    "      if (dd_col_pos > 0)", row + 1,
    "         set dd_data_type = value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[dd_col_pos].data_type)",
    row + 1, '         if (dd_data_type not in ("VC", "C*"))', row + 1,
    '            call echo("', dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name,
    ' has an invalid value in the PARENT_ENTITY_COL of DM_COLUMNS_DOC.")',
    row + 1, "            set cust_fail_merges= cust_fail_merges + 1", row + 1,
    '            set fail_merge_audit->num[cust_fail_merges].action = "FAILREASON"', row + 1,
    '            set fail_merge_audit->num[cust_fail_merges].text = concat("',
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, '",', row + 1,
    '" has an invalid value in the PARENT_ENTITY_COL of DM_COLUMNS_DOC.")', row + 1,
    "            rollback",
    row + 1, "            call cs_merge_audit(fail_merge_audit->num[cust_fail_merges].action, ", row
     + 1,
    "               fail_merge_audit->num[cust_fail_merges].text)", row + 1,
    "            if (drdm_error_out_ind = 1)",
    row + 1, "               rollback", row + 1,
    "            endif", row + 1, "            commit",
    row + 1, '            set drdm_parent_col = "INVALIDTABLE"', row + 1,
    "         else", row + 1, "            set pe_fnd = 0",
    row + 1,
    "            set pe_fnd = locateval(pe_srch, 1, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt, ",
    row + 1,
    "               dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq,
    "].parent_entity_col,",
    row + 1, "               dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[pe_srch].column_name)",
    row + 1,
    "            if (pe_fnd != 0)", row + 1, "               set drdm_parent_col = cnvtupper(RS_",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->from_values.", dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[d.seq].parent_entity_col,
    ")", row + 1, '               if (drdm_parent_col != "" and drdm_parent_col != " ")',
    row + 1, "                  set drdm_parent_col = cs_find_p_e_col(drdm_parent_col,", d.seq,
    ")", row + 1, "               else",
    row + 1, '                  set drdm_parent_col = "INVALIDTABLE"', row + 1,
    '                  set dd_i_domain = concat("RDDS_PE_ABBREV:", dm2_Ref_Data_doc->tbl_qual[temp_tbl_cnt].table_name)',
    row + 1, "set dd_i_name = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[",
    d.seq, "].column_name,", ' ":", drdm_parent_col)',
    row + 1, '                  select into "NL:"', row + 1,
    "                  from dm_info d", row + 1,
    "                  where d.info_domain = dd_i_domain ",
    row + 1, "                     and d.info_name = dd_i_name", row + 1,
    "                  detail", row + 1, "                     drdm_parent_col = d.info_char",
    row + 1, "                  with nocounter", row + 1,
    "               endif", row + 1, "            else",
    row + 1, '               set drdm_parent_col = "INVALIDTABLE"', row + 1,
    '               call echo("The PARENT_ENTITY_COL value in DM_COLUMNS_DOC is not valid.")', row +
    1, "               set cust_fail_merges= cust_fail_merges + 1",
    row + 1, '               set fail_merge_audit->num[cust_fail_merges].action = "FAILREASON"', row
     + 1,
    "               set fail_merge_audit->num[cust_fail_merges].text=", row + 1,
    '                  "THE PARENT_ENTITY_COL VALUE IN DM_COLUMNS_DOC IS NOT VALID"',
    row + 1, "               rollback", row + 1,
    ...
    "                  rollback", row + 1, "               endif",
    row + 1, "               commit", row + 1,
    "            endif  ", row + 1, "         endif",
    row + 1, "      else", row + 1,
    '         set drdm_parent_col = "INVALIDTABLE"', row + 1,
    '         call echo("The PARENT_ENTITY_COL value in DM_COLUMNS_DOC is not valid.")',
    row + 1, "         set cust_fail_merges= cust_fail_merges + 1", row + 1,
    '         set fail_merge_audit->num[cust_fail_merges].action = "FAILREASON"', row + 1,
    '         set fail_merge_audit->num[cust_fail_merges].text="THE PARENT_ENTITY_COL VALUE IN DM_COLUMNS_DOC IS NOT VALID"',
    row + 1, "         rollback", row + 1,
    "         call cs_merge_audit(fail_merge_audit->num[cust_fail_merges].action, ", row + 1,
    "            fail_merge_audit->num[cust_fail_merges].text)",
    row + 1, "         if (drdm_error_out_ind = 1)", row + 1,
    "            rollback", row + 1, "         endif",
    row + 1, "         commit", row + 1,
    "      endif  ", row + 1, '      if (drdm_parent_col != "INVALIDTABLE")',
    row + 1, "         set cust_sel_ret = ddselect_merge_translate(cnvtstring(RS_", dm2_ref_data_doc
    ->tbl_qual[temp_tbl_cnt].suffix,
    "->from_values.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, "),",
    row + 1, "            drdm_parent_col)", row + 1,
    '         if (cust_sel_ret != "No Trans")', row + 1, "            set RS_",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.", dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[d.seq].column_name,
    " = cnvtreal(cust_sel_ret)", row + 1,
    "            set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[",
    d.seq, "].translated = 1", row + 1,
    "         endif", row + 1, "      endif",
    row + 1, "      if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq,
    "].translated = 1)", row + 1, "         set cust_cur_merges= cust_cur_merges + 1",
    row + 1, '         set child_merge_audit->num[cust_cur_merges].action = "LOOKUPS"', row + 1,
    '         set child_merge_audit->num[cust_cur_merges].text = "', dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].table_name, "  ",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, '"', row + 1,
    "      endif", row + 1, "endif",
    row + 1
  ...
  WITH append, formfeed = none, maxrow = 1,
   format = variable
 ;end select
 FREE SET dyn_tab_name
 FREE SET dyn_col_name
 DECLARE dyn_tab_name = vc
 DECLARE dyn_col_name = vc
 SELECT INTO value(dyn_f_name)
  FROM (dummyt d  WITH seq = value(perm_col_cnt))
  WHERE (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].pk_ind=0)
  HEAD REPORT
   row 1, col 1
  DETAIL
   dyn_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name, dyn_col_name =
  ...
  FOOT REPORT
   ...
   row + 1, "   if (drdm_error_out_ind = 1)", row + 1,
   "      go to EXIT", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, row + 1,
   "   endif", row + 1, '   call echo("")',
   row + 1, '   call echo("")', row + 1,
   '   call echo("***************CHECKING ID AND CD COLUMNS******************")', row + 1,
   '   call echo("")',
   row + 1, '   call echo("")', row + 1,
   "   ;Checking id and cd columns", row + 1, "   for (cust_check_loop = 1 to perm_col_cnt)",
   row + 1,
   "      if (value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].idcd_ind) = 1)",
   row + 1,
   ...
   row + 1, '               call parser(concat("set rpt_from = RS_', dm2_ref_data_doc->tbl_qual[
   temp_tbl_cnt].suffix,
   '->from_values.",', row + 1,
   '                  dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].column_name, " go"), 1)',
   row + 1, "            endif", row + 1,
   '            if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].parent_entity_col != "")',
   row + 1, "               declare rpt_col_value = vc",
   row + 1, "               declare rpt_fnd = i4", row + 1,
   "               declare rpt_srch = i4", row + 1, "               declare rpt_parent_col = vc",
   row + 1, "               declare rpt_i_domain =vc", row + 1,
   "               declare rpt_i_name = vc", row + 1, "               declare rpt_col_pos = i4",
   row + 1, "               declare rpt_mult_cnt = i4", row + 1,
   "               declare rpt_data_type = vc", row + 1,
   '               call parser(concat("set rpt_from = RS_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,',
   row + 1, '                   "->from_values.",', row + 1,
   "                   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].column_name,",
   row + 1, '                   " go"), 1)',
   row + 1, "               set rpt_col_value =", row + 1,
   "                  value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].parent_entity_col)",
   row + 1, "                  set rpt_col_pos = locateval(rpt_srch,1,perm_col_cnt, rpt_col_value,",
   row + 1,
   "                  dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[rpt_srch].column_name)", row
    + 1,
   "               if (rpt_col_pos > 0)", row + 1, "                  set rpt_data_type = ",
   row + 1,
   "                     value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[rpt_col_pos].data_type)",
   row + 1,
   '                  if (rpt_data_type in ("VC", "C*"))', row + 1,
   "                     set rpt_fnd = 0",
   row + 1,
   "                     set rpt_fnd = locateval(rpt_srch, 1, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt,",
   row + 1,
   "                        rpt_col_value, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[rpt_srch].column_name)",
   row + 1, "                     if (rpt_fnd != 0)",
   row + 1, '                        call parser(concat("set rpt_parent_col = cnvtupper(RS_",', row
    + 1,
   "                           dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,", row + 1,
   '                           "->from_values.", rpt_col_value, ") go"), 1)',
   row + 1, '                        if (rpt_parent_col != "" and rpt_parent_col != " ")', row + 1,
   "                           set rpt_parent_col = cs_find_p_e_col(rpt_parent_col, cust_check_loop)",
   row + 1, "                        else",
   row + 1, '                           set rpt_i_domain = concat("RDDS_PE_ABBREV:",', row + 1,
   "                              dm2_Ref_Data_doc->tbl_qual[temp_tbl_cnt].table_name)", row + 1,
   "                           set rpt_i_name = concat(",
   row + 1,
   "                              dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].column_name,",
   row + 1,
   '                              ":", rpt_parent_col)', row + 1,
   '                           select into "NL:"',
   row + 1, "                           from dm_info d", row + 1,
   "                           where d.info_domain = rpt_i_domain ", row + 1,
   "                              and d.info_name = rpt_i_name",
   row + 1, "                           detail", row + 1,
   "                              rpt_parent_col = d.info_char", row + 1,
   "                           with nocounter",
   row + 1, "                        endif", row + 1,
   "                     endif  ", row + 1, "                  endif",
   row + 1, "               endif", row + 1,
   '               if (rpt_parent_col != "INVALIDTABLE" and rpt_parent_col != "")', row + 1,
   "                  set rpt_table = rpt_parent_col",
   row + 1, "                  SET rpt_fnd = LOCATEVAL(rpt_srch, 1, dguc_reply->rs_tbl_cnt, ", row +
   1,
   "                     rpt_table, dguc_reply->dtd_hold[rpt_srch].tbl_name)", row + 1,
   "                  if (rpt_fnd != 0)",
   row + 1, "                     if (dguc_reply->dtd_hold[rpt_fnd].pk_cnt >= 1)", row + 1,
   "                        set rpt_srch = 0", row + 1,
   "                        for (rpt_mult_cnt = 1 to dguc_reply->dtd_hold[rpt_fnd].pk_cnt)          ",
   row + 1,
   '                           if (dguc_reply->dtd_hold[rpt_fnd].pk_hold[rpt_mult_cnt].pk_name = "*ID" or',
   row + 1,
   '                              dguc_reply->dtd_hold[rpt_fnd].pk_hold[rpt_mult_cnt].pk_name = "*CD" or ',
   row + 1,
   '                              dguc_reply->dtd_hold[rpt_fnd].pk_hold[rpt_mult_cnt].pk_name = "CODE_VALUE")',
   row + 1,
   '                              if (dguc_reply->dtd_hold[rpt_fnd].pk_hold[rpt_mult_cnt].pk_name = "*ID" or',
   ...
   row + 1,
   "                                 set rpt_column = dguc_reply->dtd_hold[rpt_fnd].pk_hold[rpt_mult_cnt].pk_name",
   row + 1, "                                 set rpt_srch = rpt_srch + 1", row + 1,
   "                              endif", row + 1, "                           endif",
   row + 1, "                        endfor", row + 1,
   "                     endif", row + 1, "                     if (rpt_srch > 1)",
   row + 1, '                        set rpt_column = ""', row + 1,
   "                     endif", row + 1, "                  endif",
   row + 1, "               endif", row + 1,
   "            endif              ", row + 1, '            if (rpt_table != "" and rpt_from != 0)',
   row + 1, '               set except_tab = get_rdds_tname("DM_CHG_LOG_EXCEPTION")', row + 1,
   '               SELECT into "NL:"', row + 1, "               FROM (parser(except_tab) d)",
   row + 1, "               WHERE d.TARGET_ENV_ID = dm2_ref_data_doc->env_target_id", row + 1,
   "                   AND d.TABLE_NAME = rpt_table", row + 1,
   "                   AND d.COLUMN_NAME = rpt_column",
   row + 1, "                   AND d.FROM_VALUE = rpt_from", row + 1,
   "               detail", row + 1, "                  except_log_type = d.log_type",
   row + 1, "               with nocounter", row + 2,
   "               IF (curqual = 0)", row + 1, "                  rollback",
   row + 1, "                  INSERT into (parser(except_tab) d)", row + 1,
   '                  set d.log_type = "NOXLAT", ', row + 1,
   "                     d.table_name = rpt_table, ",
   row + 1, "                     d.column_name = rpt_column, ", row + 1,
   "                     d.from_value = rpt_from,", row + 1,
   "                     d.target_env_id = dm2_ref_data_doc->env_target_id",
   row + 1, "                  with nocounter", row + 1,
   "                  if (error(err_msg, 0) > 0)", row + 1, "                     call echo(err_msg)",
   row + 1, "                     set drdm_error_out_ind = 1", row + 1,
   "                     set err_ind = 0", row + 1, "                  else",
   row + 1, "                     commit", row + 1,
   "                  endif", row + 1, "               else",
   row + 1, '                  if (except_log_type = "ORPHAN")', row + 1,
   "                     if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].merge_delete_ind != 0",
   row + 1,
   "                         or dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].pk_ind != 0)",
   row + 1, "                         set orphan_ind = 1", row + 1,
   "                         set cust_check_loop = perm_col_cnt", row + 1,
   "                     else",
   row + 1, "                        set rlbk_orphan_ind = 1", row + 1,
   "                        set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].translated = 1",
   row + 1, '                        call parser(concat("set RS_',
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, '->to_VALUES.",', row + 1,
   "dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].column_name,",
   '" = 0 go"), 1)', row + 1,
   "                     endif", row + 1, "                  endif",
   row + 1, '                  if (except_log_type = "OLDVER")', row + 1,
   "                     set oldver_ind = 1", row + 1,
   "                     set cust_check_loop = perm_col_cnt",
   row + 1, "                  endif", row + 1,
   "               endif", row + 1, "            endif",
   row + 2, "            if (rlbk_orphan_ind = 0)", row + 1,
   "               set IDCD_Check = 1", row + 1, "               if (oldver_ind != 1)",
   row + 1, '               set err_msg = concat("This log_id ",', row + 1,
   '                  "was not translated because of the ",', row + 1,
   "                  dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].column_name, ",
   row + 1, '                  " column.")', row + 1,
   ...
   row + 1, "               set cust_fail_merges= cust_fail_merges + 1", row + 1,
   '               set fail_merge_audit->num[cust_fail_merges].action = "FAILREASON"', row + 1,
   "               set fail_merge_audit->num[cust_fail_merges].text = err_msg",
   row + 1, "               rollback", row + 1,
   "               set cust_cs_loop = comp_seq_cnt", row + 1,
   "               call cs_merge_audit(fail_merge_audit->num[cust_fail_merges].action, ",
   row + 1, "                  fail_merge_audit->num[cust_fail_merges].text)", row + 1,
   "               if (drdm_error_out_ind = 1)", row + 1, "                  rollback",
   row + 1, "                  set cust_check_loop = perm_col_cnt", row + 1,
   "               else", row + 1, "                  commit",
   row + 1, "               endif", row + 1,
   "               endif", row + 1, "            else",
   row + 1, "               set rlbk_orphan_ind = 0", row + 1,
   "            endif", row + 1, "         endif",
   row + 1, "      endif", row + 1,
   "   endfor ", row + 1, "   if (drdm_error_out_ind = 1)",
   row + 1, "      set cust_cs_loop = comp_seq_cnt", row + 1,
   "   endif", row + 1, row + 1,
   "   if (IDCD_check = 0)", row + 1, "       if (cust_cs_loop = 1)",
   row + 1
  WITH append, formfeed = none, maxrow = 1,
   format = variable
 ;end select
 ...
 SELECT INTO value(dyn_f_name)
  FROM (dummyt d  WITH seq = value(perm_col_cnt))
  HEAD REPORT
   "delete from ", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name, " c",
   row + 1, "where "
  DETAIL
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].merge_delete_ind=1))
    dyn_dumb_cnt = (dyn_dumb_cnt+ 1)
    IF (dyn_dumb_cnt > 1)
     " and ", row + 1
    ENDIF
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].data_type="DQ8"))
     "   c.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = evaluate(",
     "dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, "].check_null,",
     row + 1, " 1, NULL, 0, cnvtdatetime(rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
     "->to_values.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, "))",
     row + 1
    ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].data_type IN ("VC", "C*")))
     "   c.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = evaluate(",
     "dm2_ref_Data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, "].check_null,",
     row + 1, " 1, NULL, 0,evaluate(", row + 1,
     "dm2_ref_Data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, '].check_space, 1, " ", 0 ,',
     row + 1, "rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
     "->to_values.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, "))",
     row + 1
    ELSE
     "   c.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = evaluate(",
     "dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, "].check_null,",
     row + 1, " 1, NULL, 0, rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
     "->to_values.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, ")",
     row + 1
    ENDIF
   ENDIF
 ...
 SELECT INTO value(dyn_f_name)
  FROM (dummyt d  WITH seq = value(perm_col_cnt))
  HEAD REPORT
   row 1, col 1, "     delete from ",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name, " d", row + 1,
   " where "
  DETAIL
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].pk_ind=1))
    dyn_dumb_cnt = (dyn_dumb_cnt+ 1)
    IF (dyn_dumb_cnt > 1)
     " and ", row + 1
    ENDIF
    "d.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = rs_",
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.", dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[d.seq].column_name,
    row + 1
   ENDIF
  FOOT REPORT
   ...
   "   call echo(err_msg)", row + 1, "   go to EXIT_",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, row + 1, "endif",
   row + 1, row + 1, "      call cs_Insert(null)",
   ...
   '            set cust_chg_log = get_rdds_tname("DM_CHG_LOG")', row + 1,
   '            select into "nl:"',
   ...
   row + 1, "         set cust_cur_merges = 0", row + 1,
   "         set cust_cs_rows->qual[cust_cs_loop].trans_ind = 1", row + 1,
   "         if (cust_cs_loop = comp_seq_cnt)",
   row + 1, '            call echo(build("p Delete = ", curmem))', row + 1,
   "            commit ", row + 1, "         endif",
   row + 1, "      else", row + 1,
   ...
   "         call echo(cust_del_msg)", row + 1, '         call echo("")',
   row + 1, '         call echo("")', row + 1,
   "         rollback", row + 1, "         set cust_cur_merges  = 0",
   row + 1, "         set cust_cs_loop = comp_seq_cnt", row + 1,
   "      endif ", row + 1, "   else",
   row + 1, "      if (orphan_ind = 1 or oldver_ind = 1)", row + 1,
   "         set update_cnt = 0", row + 1, "         rollback",
   row + 1, "         if (oldver_ind = 1)", row + 1,
   '            set spec_log_type = "OLDVER"', row + 1, "         elseif (orphan_ind = 1)",
   row + 1, '            set spec_log_type = "ORPHAN"', row + 1,
   "         endif", row + 1, '         set cust_chg_log = get_rdds_tname("DM_CHG_LOG")',
   row + 1, '         select into "nl:"', row + 1,
   "         from (parser(cust_chg_log) dc)", row + 1,
   "         where dc.log_id = drdm_chg->log[drdm_log_loop].log_id",
   row + 1, "         detail", row + 1,
   "            update_cnt = dc.updt_cnt", row + 1, "         with nocounter",
   row + 1, "         if (update_cnt = drdm_chg->log[drdm_log_loop].updt_cnt)", row + 1,
   "            update into (parser(cust_chg_log) d)", row + 1,
   "            set d.log_type = spec_log_type,",
   row + 1, "               d.updt_dt_tm = cnvtdatetime(curdate, curtime3),", row + 1,
   "               d.updt_cnt = d.updt_cnt + 1", row + 1,
   "            where d.log_id = drdm_chg->log[drdm_log_loop].log_id",
   row + 1, "            with nocounter  ;002>", row + 1,
   "            if (error(err_msg, 0) > 0)", row + 1, "               call echo(err_msg)",
   row + 1, "               set drdm_error_out_ind = 1", row + 1,
   "               rollback", row + 1, "               go to EXIT_",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, row + 1, "            else",
   row + 1, "               commit", row + 1,
   "            endif  ;<002", row + 1, "         endif",
   row + 1, "      endif", row + 1,
   "      set cust_cur_merges = 0", row + 1, "      rollback ",
   row + 1, "      set cust_cs_loop = comp_seq_cnt", row + 1,
   "   endif", row + 1, "endfor",
   row + 1
  WITH append, formfeed = none, maxrow = 1,
 ...
 SELECT INTO value(dyn_f_name)
  ...
  HEAD REPORT
   row 1, col 1, ";*************************************************************************",
   row + 1, ";cs_Insert_row accepts no variables and returns a null                  **", row + 1,
   ";It decides whether to update the row in the target or insert a new row **", row + 1,
   ";*************************************************************************",
   row + 1, "subroutine cs_Insert(null)", row + 1,
   row + 1, ";changing updt* columns to be correct.", row + 1,
   "if (validate(rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
   "->to_values.updt_dt_tm,-1) !=-1)",
   row + 1, "set rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
   "->to_values.updt_dt_tm = cnvtdatetime(curdate,curtime3)", row + 1, "endif",
   row + 1, "if (validate(rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
   "->to_values.updt_cnt,-1) !=-1)", row + 1, "set rs_",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.updt_cnt = 0", row + 1,
   "endif", row + 1, "if (validate(rs_",
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.updt_task,-1) !=-1)", row + 1,
   "set rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->to_values.updt_task = 4310001",
   row + 1, "endif", row + 1,
   'call echo("")', row + 1, 'call echo("")',
   row + 1, 'call echo("*******************INSERTING ROW******************")', row + 1,
   'call echo("")', row + 1, 'call echo("")',
   row + 1, row + 1, ";constructing insert statement",
   row + 1, "insert into ", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,
   " c set", row + 1
  DETAIL
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].data_type IN ("DATE", "DQ8")))
    IF (d.seq=perm_col_cnt)
     "   c.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = evaluate(",
     "dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, "].check_null,",
     row + 1, " 1, NULL, 0, cnvtdatetime(rs_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
    ...
    ENDIF
   ELSE
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].data_type IN ("VC", "C*")))
     IF (d.seq=perm_col_cnt)
      "   c.", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, " = evaluate(",
      "dm2_ref_Data_doc->tbl_qual[temp_tbl_cnt].col_qual[", d.seq, "].check_null,",
      row + 1, " 1, NULL, 0,evaluate(", row + 1,
  ...
  FOOT REPORT
   "with nocounter", row + 1, "   if (error(err_msg, 0) > 0)",
   row + 1, "      call echo(err_msg)", row + 1,
   "      set cust_nodelete_ind = 1", row + 1, "      set err_ind = 0",
   row + 1, "   endif", row + 1,
   ...
   row + 1, ";*************************************************", row + 1,
   "subroutine cs_merge_audit(action, text)", row + 1, row + 1,
   "declare ma_next_seq = f8", row + 1, "declare ma_del_ind = i2",
   row + 1, 'if (drdm_log_level = 1 and action not in ("INSERT", "UPDATE", "FAILREASON"))		', row + 1,
   "   return (null)									", row + 1, "else											",
   row + 1, '   select into "NL:"', row + 1,
   "      y = seq(DM_MERGE_AUDIT_SEQ, nextval)", row + 1, "   from dual",
   row + 1, "   detail ", row + 1,
   "      ma_next_seq = y", row + 1, "   with nocounter",
   row + 1, "   if (error(err_msg, 0) > 0)", row + 1,
   "      call echo(err_msg)", row + 1, "      set err_ind = 0 ",
   row + 1, "      set drdm_error_out_ind = 1", row + 1,
   "   else", row + 1, "      update into dm_chg_log_audit dm",
   row + 1,
   "      set dm.audit_dt_tm = cnvtdatetime(curdate,curtime3),dm.updt_dt_tm = cnvtdatetime(curdate,curtime3),",
   row + 1,
   '         dm.action= action,dm.text = text, dm.table_name = "', dm2_ref_data_doc->tbl_qual[
   temp_tbl_cnt].table_name, '", dm.log_id = rs_',
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, "->log_id", row + 1,
   "      where dm.dm_chg_log_audit_id = ma_next_seq", row + 1, "      with nocounter",
   row + 1, "      if (error(err_msg, 0) > 0)", row + 1,
   "         call echo(err_msg)", row + 1, "         set err_ind = 0 ",
   row + 1, "         set drdm_error_out_ind = 1", row + 1,
   "      endif", row + 1, "     if (curqual = 0)",
   row + 1, "         insert into dm_chg_log_audit dm", row + 1,
   "         set dm.audit_dt_tm = cnvtdatetime(curdate,curtime3),dm.updt_dt_tm = cnvtdatetime(curdate,curtime3),",
   row + 1, '            dm.action= action,dm.text = text, dm.table_name = "',
   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name, '", dm.log_id = rs_', dm2_ref_data_doc->
   tbl_qual[temp_tbl_cnt].suffix,
   "->log_id,", row + 1, "         dm.dm_chg_log_audit_id = ma_next_seq",
   row + 1, "         with nocounter", row + 1,
   "         if (error(err_msg, 0) > 0)", row + 1, "            call echo(err_msg)",
   row + 1, "            set err_ind = 0", row + 1,
   ...
   ";**************************************************************************", row + 1,
   ";ddinsert_merge_translate accepts 2 f8 and 1 vc parameter and returns i2*** ",
   row + 1, ";**************************************************************************", row + 1,
   "subroutine ddinsert_merge_translate(sbr_from, sbr_to, sbr_table) ", row + 1, row + 1,
   "declare imt_seq_name = vc", row + 1, "declare imt_seq_num = f8",
   row + 1, "declare imt_seq_loop = i4", row + 1,
   "declare imt_seq_cnt = i4", row + 1, "declare imt_rs_cnt = i4",
   row + 1, "declare imt_return = i2", row + 1,
   "declare ddimt_xlat_env_tgt_id = f8", row + 1, row + 1,
   "set ddimt_xlat_env_tgt_id = dm2_ref_data_doc->mock_target_id", row + 1, "set imt_return = 0 ",
   row + 1, row + 1, "if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].skip_seqmatch_ind = 0)",
   row + 1, ";get sequence name", row + 1,
   "set imt_rs_cnt = locateval(imt_seq_loop, 1, size(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual, 5),",
   row + 1,
   "      sbr_table, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[imt_seq_loop].root_entity_name)",
   row + 1,
   "set imt_seq_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[imt_rs_cnt].sequence_name",
   row + 1,
   row + 1, ";get sequence value", row + 1,
   "set imt_seq_cnt = locateval(imt_seq_loop,1, size(drdm_sequence->qual, 5),imt_seq_name, ", row + 1,
   "drdm_sequence->qual[imt_seq_loop].seq_name)",
   row + 1, "set imt_seq_num = drdm_sequence->qual[imt_seq_loop].seq_val", row + 1,
   row + 1, ";check to see if sequence is below RS value", row + 1,
   "if (sbr_to < imt_seq_num)", row + 1, "   set imt_return = 1",
   row + 1, "endif", row + 1,
   row + 1, "endif", row + 1,
   ";check to see if to_value is already in the dm_merge_translate table", row + 1,
   'select into "NL:"',
   row + 1, "from dm_merge_translate dmt", row + 1,
   "where dmt.to_value = sbr_to", row + 1, "and dmt.table_name = sbr_table",
   row + 1, "and dmt.env_source_id = dm2_ref_data_doc->env_source_id", row + 1,
   "and dmt.env_target_id = ddimt_xlat_env_tgt_id", row + 1, "with nocounter",
   row + 1, "   if (error(err_msg, 0) > 0)", row + 1,
   "      call echo(err_msg)", row + 1, "      set nodelete_ind = 1",
   row + 1, "      set no_insert_update = 1", row + 1,
   "      set err_ind = 0", row + 1, "      return(0)",
   row + 1, "   endif", row + 1,
   row + 1, "if (curqual > 0)", row + 1,
   "   set imt_return = 1", row + 1, "endif",
   row + 1, row + 1, "if (imt_return = 0)",
   row + 1, "   insert into dm_merge_translate dm", row + 1,
   "   set dm.from_value = sbr_from,", row + 1, "   dm.to_value = sbr_to,",
   row + 1, "   dm.Table_name = sbr_table,", row + 1,
   "   dm.env_source_id = dm2_ref_data_doc->env_source_id,", row + 1,
   "   dm.env_target_id = ddimt_xlat_env_tgt_id",
   row + 1, row + 1, "   if (error(err_msg, 0) > 0)",
   row + 1, "      call echo(err_msg)", row + 1,
   "      set nodelete_ind = 1", row + 1, "      set no_insert_update = 1",
   row + 1, "      set drdm_error_out_ind = 1", row + 1,
   "      set imt_return = 1", row + 1, "      set err_ind = 0",
   row + 1, "   endif", row + 1,
   "else", row + 1, "   rollback",
   row + 1, '   set except_tab = get_rdds_tname("DM_CHG_LOG_EXCEPTION")', row + 1,
   "   update into (parser(except_tab) d)", row + 1, '   set d.log_type = "BADTRN"',
   row + 1, "   where d.table_name = sbr_table and", row + 1,
   "       d.column_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[imt_rs_cnt].column_name and",
   row + 1, "       d.from_value = sbr_from and",
   row + 1, "       d.target_env_id = dm2_ref_data_doc->env_target_id", row + 1,
   "   with nocounter", row + 1, "   if (error(err_msg, 0) > 0)",
   row + 1, "      call echo(err_msg)", row + 1,
   "      set nodelete_ind = 1", row + 1, "      set no_insert_update = 1",
   row + 1, "      set drdm_error_out_ind = 1", row + 1,
   "      set err_ind = 0", row + 1, "   endif",
   row + 1, "   if (curqual = 0)", row + 1,
   "      INSERT into (parser(except_tab) d)", row + 1, '      set d.log_type = "BADTRN", ',
   row + 1, "          d.table_name = sbr_table, ", row + 1,
   "          d.column_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[imt_rs_cnt].column_name, ",
   row + 1, "          d.from_value = sbr_from,",
   row + 1, "          d.target_env_id = dm2_ref_data_doc->env_target_id", row + 1,
   "      with nocounter", row + 1, "   endif",
   row + 1, "   if (error(err_msg, 0) > 0)", row + 1,
   "      call echo(err_msg)", row + 1, "      set nodelete_ind = 1",
   row + 1, "      set no_insert_update = 1", row + 1,
   "      set drdm_error_out_ind = 1", row + 1, "      set err_ind = 0",
   row + 1, "   else", row + 1,
   "      commit", row + 1, "   endif",
   row + 1, "endif", row + 1,
   row + 1, "call dd_del_except(sbr_table, sbr_from)", row + 1,
   row + 1, "return (imt_return)", row + 1,
   "end  ;subroutine", row + 1, row + 1,
   ";******************************************************************", row + 1,
   ";ddselect_merge_translate accepts 2 vc parameters and returns a vc***",
   row + 1, ";******************************************************************", row + 1,
   "subroutine ddselect_merge_translate(sbr_f_value, sbr_t_name)", row + 1, "declare to_val = vc",
   row + 1, "declare drdm_dmt_scr = vc", row + 1,
   "declare except_tab = vc", row + 1, "declare ddsmt_loop = i4",
   row + 1, "declare ddsmt_tbl_pos = i4", row + 1,
   "declare ddsmt_seq_name = vc", row + 1, "declare ddsmt_seq_num = f8",
   row + 1, "declare ddsmt_cur_table = i4", row + 1,
   "declare ddsmt_xlat_env_tgt_id = f8", row + 1, row + 1,
   "set ddsmt_xlat_env_tgt_id = dm2_ref_data_doc->mock_target_id", row + 1, 'set to_val = "No Trans"',
   row + 1, row + 1, 'set except_tab = get_rdds_tname("DM_CHG_LOG_EXCEPTION") ',
   row + 1, row + 1,
   "set ddsmt_tbl_pos = locateval(ddsmt_loop, 1, dm2_ref_data_doc->tbl_cnt, sbr_t_name,",
   row + 1, "                            dm2_ref_data_doc->tbl_qual[ddsmt_loop].table_name)", row + 1,
   row + 1, "if (ddsmt_tbl_pos = 0)", row + 1,
   "   set ddsmt_cur_table = temp_tbl_cnt", row + 1,
   '   set ddsmt_tbl_pos = ddfill_rs("TABLE", sbr_t_name)',
   row + 1, "   set temp_tbl_cnt = ddsmt_cur_table", row + 1,
   "endif", row + 1, row + 1,
   "if (ddsmt_tbl_pos = 0)", row + 1, "   return(to_val)",
   row + 1, "endif", row + 1,
   "if (dm2_ref_data_doc->tbl_qual[ddsmt_tbl_pos].skip_seqmatch_ind != 1)", row + 1,
   "for (ddsmt_loop = 1 to dm2_ref_data_doc->tbl_qual[ddsmt_tbl_pos].col_cnt)",
   row + 1, "   if (dm2_ref_data_doc->tbl_qual[ddsmt_tbl_pos].col_qual[ddsmt_loop].pk_ind = 1 and ",
   row + 1,
   "       dm2_ref_data_doc->tbl_qual[ddsmt_tbl_pos].col_qual[ddsmt_loop].root_entity_name = sbr_t_name)",
   row + 1,
   "      set ddsmt_seq_name = dm2_ref_data_doc->tbl_qual[ddsmt_tbl_pos].col_qual[ddsmt_loop].sequence_name",
   row + 1, "   endif", row + 1,
   "endfor", row + 1, 'if (ddsmt_seq_name = "")',
   row + 1, "   call echo(err_msg)", row + 1,
   '   call cs_merge_audit("FAILREASON", "A valid sequence was not found")', row + 1,
   "   return (to_val)",
   row + 1, "endif", row + 1,
   row + 1, "select ", row + 1,
   '   if (dm2_rdds_rec->mode = "OS")', row + 1, '      where d.info_domain = "MERGE00SEQMATCH"',
   row + 1, "         and d.info_name = ddsmt_seq_name", row + 1,
   "   else", row + 1,
   'where d.info_domain = concat("MERGE",trim(cnvtstring(dm2_ref_data_doc->env_source_id)), ',
   row + 1, '   trim(cnvtstring(ddsmt_xlat_env_tgt_id)), "SEQMATCH")  ', row + 1,
   "and d.info_name = ddsmt_seq_name", row + 1, "   endif",
   row + 1, ' into "NL:" from dm_info d', row + 1,
   "detail", row + 1, "   ddsmt_seq_num = d.info_number",
   row + 1, "with nocounter", row + 1,
   "if (error(err_msg, 0) > 0)", row + 1, "   call echo(err_msg)",
   row + 1, "   set  dm2_ref_data_reply->error_ind = 1", row + 1,
   "   set dm2_ref_data_reply->error_msg = err_msg", row + 1, "   set err_ind = 1",
   row + 1, "endif", row + 1,
   row + 1, "If (curqual = 0 or ddsmt_seq_num =-1)", row + 1,
   "   set ddsmt_cur_table = temp_tbl_cnt", row + 1,
   "   execute dm2_find_sequence_match ddsmt_seq_name, dm2_ref_data_doc->env_source_id",
   row + 1, "   set temp_tbl_cnt = ddsmt_cur_table", row + 1,
   "   if (dm_err->err_ind > 0)", row + 1, "      set drdm_error_out_ind = 1",
   row + 1, "      call echo(err_msg)", row + 1,
   "      set  dm2_ref_data_reply->error_ind = 1", row + 1,
   "      set dm2_ref_data_reply->error_msg = err_msg",
   row + 1, "      set err_ind = 1", row + 1,
   "   endif", row + 1, "   select ",
   row + 1, '   if (dm2_rdds_rec->mode = "OS")', row + 1,
   '      where d.info_domain = "MERGE00SEQMATCH"', row + 1,
   "         and d.info_name = ddsmt_seq_name",
   row + 1, "   else", row + 1,
   '   where d.info_domain = concat("MERGE",trim(cnvtstring(dm2_ref_data_doc->env_source_id)), ', row
    + 1, '      trim(cnvtstring(ddsmt_xlat_env_tgt_id)), "SEQMATCH")  ',
   row + 1, "   and d.info_name = ddsmt_seq_name", row + 1,
   "   endif", row + 1, '   into "NL:" from dm_info d',
   row + 1, "   detail", row + 1,
   "      ddsmt_seq_num = d.info_number", row + 1, "   with nocounter",
   row + 1, "   if (error(err_msg, 0) > 0)", row + 1,
   "      set drdm_error_out_ind = 1", row + 1, "      call echo(err_msg)",
   row + 1, "      set  dm2_ref_data_reply->error_ind = 1", row + 1,
   "      set dm2_ref_data_reply->error_msg = err_msg", row + 1, "      set err_ind = 1",
   row + 1, "   endif", row + 1,
   "   if (curqual = 0)", row + 1, "      set drdm_error_out_ind = 1",
   row + 1, '      call echo("A sequence match could not be found in DM_INFO")', row + 1,
   '      return ("No Trans")', row + 1, "   endif",
   row + 1, "endif", row + 1,
   row + 1, "else", row + 1,
   "   set ddsmt_seq_num = 0", row + 1, "endif",
   row + 1, "if (cnvtreal(sbr_f_value) <= ddsmt_seq_num)", row + 1,
   "   return (sbr_f_value)", row + 1, "else",
   row + 1, '   select into "NL:" ', row + 1,
   "   from dm_merge_translate dm", row + 1, "   where dm.from_value = cnvtreal(sbr_f_value) and",
   row + 1, "   dm.table_name = sbr_t_name and", row + 1,
   "   dm.env_source_id = dm2_ref_data_doc->env_source_id and", row + 1,
   "   dm.env_target_id = ddsmt_xlat_env_tgt_id",
   row + 1, "   detail", row + 1,
   "   to_val = cnvtstring(dm.to_value)", row + 1, "   with nocounter",
   row + 1, "   if (error(err_msg, 0) > 0)", row + 1,
   "      call echo(err_msg)", row + 1, "      set dm2_ref_data_reply->error_ind = 1",
   row + 1, "      set dm2_ref_data_reply->error_msg = err_msg", row + 1,
   "      set err_ind  = 0", row + 1, "   endif",
   row + 1, row + 1, "   if (curqual > 0)",
   row + 1, "      call dd_del_except(sbr_t_name, cnvtreal(sbr_f_value))", row + 1,
   "   endif", row + 1, row + 1,
   "   if (error(err_msg, 0) > 0)", row + 1, "      call Echo(err_msg)",
   row + 1, "      set dm2_ref_data_reply->error_ind = 1", row + 1,
   "      set dm2_ref_data_reply->error_msg = err_msg", row + 1, "      set err_ind  = 0",
   row + 1, "   endif", row + 1,
   row + 1, '   if (to_val = "No Trans" and sbr_t_name in ("PRSNL", "PERSON"))', row + 1,
   '      if (sbr_t_name = "PRNSL")', row + 1, '         set sbr_t_name = "PERSON"',
   row + 1, "      else", row + 1,
   '         set sbr_t_name = "PRSNL"', row + 1, "      endif",
   row + 1, row + 1, '      select into "NL:" ',
   row + 1, "      from dm_merge_translate dm", row + 1,
   "      where dm.from_value = cnvtreal(sbr_f_value) and", row + 1,
   "         dm.table_name = sbr_t_name and",
   row + 1, "         dm.env_source_id = dm2_ref_data_doc->env_source_id and", row + 1,
   "         dm.env_target_id = ddsmt_xlat_env_tgt_id", row + 1, "      detail",
   row + 1, "         to_val = cnvtstring(dm.to_value)", row + 1,
   "      with nocounter", row + 1, "      if (error(err_msg, 0) > 0)",
   row + 1, "         call echo(err_msg)", row + 1,
   "         set dm2_ref_data_reply->error_ind = 1", row + 1,
   "         set dm2_ref_data_reply->error_msg = err_msg",
   ...
   "      endif", row + 1, "   endif",
   row + 1, "endif", row + 1,
   row + 1, "return(to_val)", row + 1,
   "end ;subroutine", row + 1, row + 1,
   ";********************************************", row + 1,
   ";cs_find_p_e_col accepts 1 vc and returns vc ***",
   row + 1, ";********************************************", row + 1,
   "subroutine cs_find_p_e_col(cs_p_e_name, cs_p_e_col)", row + 1, "declare p_e_name = vc",
   row + 1, "declare r_e_name = vc", row + 1,
   "declare p_e_col = vc", row + 1, "declare tbl_loop = i4",
   row + 1, "declare kickout = i4", row + 1,
   "declare p_e_tbl_pos = i4", row + 1, "declare p_e_col_pos = i4",
   row + 1, "declare p_e_where_str = vc", row + 1,
   "declare pk_pos = i4", row + 1, "declare temp_name = vc",
   row + 1, "declare mult_cnt = i4", row + 1,
   "declare pk_num = i4", row + 1, "declare good_pk = i4",
   row + 1, "declare pk_name = vc", row + 1,
   "declare id_ind = i2", row + 1, "declare info_alias = vc",
   row + 1, "declare i_domain = vc", row + 1,
   "declare i_name = vc", row + 1, row + 1,
   'set p_e_name = "INVALIDTABLE"', row + 1, "set r_e_name = cs_p_e_name",
   row + 1, 'set info_alias = ""', row + 1,
   row + 1, "set id_ind = 0", row + 1,
   "set pk_num = 0", row + 1, 'set pk_name = ""',
   row + 1, "set good_pk = 0", row + 1,
   "while (p_e_name != r_e_name)", row + 1, "   set p_e_name = r_e_name",
   row + 1, '   set r_e_name = "INVALIDTABLE"', row + 1,
   "   set pk_pos = 0", row + 1, "   SET pk_pos = LOCATEVAL(tbl_loop, 1, dguc_reply->rs_tbl_cnt, ",
   row + 1, "      p_e_name,", row + 1,
   "      dguc_reply->dtd_hold[tbl_loop].tbl_name)", row + 1, "   if (pk_pos = 0)",
   row + 1, '      select into "NL:"', row + 1,
   "      from dtable d", row + 1, "      where d.table_name = p_e_name",
   row + 1, "      with nocounter", row + 1,
   "      if (curqual = 0)", row + 1,
   '         set i_domain = concat("RDDS_PE_ABBREV:", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name)',
   row + 1,
   "         set i_name = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cs_p_e_col].parent_entity_col,",
   row + 1,
   '             ":", p_e_name)', row + 1, '         select into "NL:"',
   row + 1, "         from dm_info d", row + 1,
   '         where d.info_domain = "RDDS_PE_ABBREVIATIONS"', row + 1,
   "            and d.info_name = p_e_name",
   row + 1, "         detail", row + 1,
   "            info_alias = d.info_char", row + 1, "         with nocounter",
   row + 1, row + 1, '         select into "NL:"',
   row + 1, "         from dm_info d", row + 1,
   "         where d.info_domain = i_domain", row + 1, "            and d.info_name = i_name",
   row + 1, "         detail", row + 1,
   "            info_alias = d.info_char", row + 1, "         with nocounter",
   row + 1, '         if (info_alias = "")', row + 1,
   '            set p_e_name = "INVALIDTABLE"', row + 1, "            set r_e_name = p_e_name",
   row + 1, '            call echo ("Parent_entity_col could not be found")', row + 1,
   "         else", row + 1, "            set p_e_name = info_alias",
   row + 1, "            SET pk_pos = LOCATEVAL(tbl_loop, 1, dguc_reply->rs_tbl_cnt, ", row + 1,
   "               p_e_name,", row + 1, "               dguc_reply->dtd_hold[tbl_loop].tbl_name)",
   row + 1, "            if (pk_pos = 0)", row + 1,
   '               set p_e_name = "INVALIDTABLE"', row + 1, "               set r_e_name = p_e_name",
   row + 1, '               call echo ("Parent_entity_col could not be found")', row + 1,
   "            endif", row + 1, "         endif",
   row + 1, "      else", row + 1,
   "         call echo(concat(", row + 1,
   '            "DM_TABLES_DOC is missing a table that is in DTABLE or the following table is activity: ", p_e_name)) ',
   row + 1, '         set p_e_name = "INVALIDTABLE"', row + 1,
   "         set r_e_name = p_e_name", row + 1, "      endif",
   row + 1, "   endif", row + 1,
   "   if (pk_pos != 0)", row + 1, "      if (dguc_reply->dtd_hold[tbl_loop].pk_cnt > 1)",
   row + 1, "         for (mult_cnt = 1 to dguc_reply->dtd_hold[tbl_loop].pk_cnt)", row + 1,
   '             if (dguc_reply->dtd_hold[tbl_loop].pk_hold[mult_cnt].pk_name = "*ID" or', row + 1,
   '                 dguc_reply->dtd_hold[tbl_loop].pk_hold[mult_cnt].pk_name = "*CD" or ',
   row + 1,
   '                 dguc_reply->dtd_hold[tbl_loop].pk_hold[mult_cnt].pk_name = "CODE_VALUE")', row
    + 1,
   '                 if (dguc_reply->dtd_hold[tbl_loop].pk_hold[mult_cnt].pk_name = "*ID")', row + 1,
   "                    set id_ind = 1",
   row + 1, "                 endif", row + 1,
   "                 set pk_num = pk_num + 1", row + 1, "                 set good_pk = mult_cnt",
   row + 1, "             endif", row + 1,
   "         endfor", row + 1, "         if (pk_num > 1)",
   row + 1, "            if (id_ind = 1)", row + 1,
   '               call echo("This Parent_Entity Table has more than a single Primary Key")', row + 1,
   '               set p_e_name = "INVALIDTABLE"',
   row + 1, "               set r_e_name = p_e_name", row + 1,
   "            else", row + 1,
   ...
   "            set pk_name = dguc_reply->dtd_hold[tbl_loop].pk_hold[good_pk].pk_name",
   row + 1, "         endif", row + 1,
   "      else", row + 1, "         set pk_name = dguc_reply->dtd_hold[tbl_loop].pk_hold[1].pk_name",
   row + 1, "      endif", row + 1,
   '      if (p_e_name != "INVALIDTABLE")', row + 1, "         set p_e_col = pk_name",
   row + 1, "         set p_e_tbl_pos = 0", row + 1,
   "         SET p_e_tbl_pos = LOCATEVAL(tbl_loop, 1, dm2_ref_Data_doc->tbl_cnt, ", row + 1,
   "             p_e_name,",
   row + 1, "             dm2_ref_data_doc->tbl_qual[tbl_loop].table_name)", row + 1,
   "         if (p_e_tbl_pos != 0)", row + 1, "            set p_e_col_pos = 0",
   row + 1,
   "            SET p_e_col_pos = LOCATEVAL(tbl_loop, 1, dm2_ref_Data_doc->tbl_qual[p_e_tbl_pos].col_cnt, ",
   row + 1,
   "               p_e_col,", row + 1,
   "               dm2_ref_data_doc->tbl_qual[p_e_tbl_pos].col_qual[tbl_loop].column_name)",
   row + 1, "            if (p_e_col_pos = 0)", row + 1,
   '               call echo("Information not found in dm_columns_doc for column")', row + 1,
   '               set p_e_name = "INVALIDTABLE"',
   row + 1, "               set r_e_name = p_e_name", row + 1,
   "            else", row + 1,
   "               set r_e_name = dm2_ref_data_doc->tbl_qual[p_e_tbl_pos].col_qual[p_e_col_pos].root_entity_name",
   row + 1, "            endif", row + 1,
   "         else", row + 1, '            set p_e_where_str = concat("d.table_name = ^",',
   row + 1, "               p_e_name,", row + 1,
   '               "^ and d.column_name = ^",', row + 1, '               p_e_col, "^")',
   row + 1, '            select into "nl:" d.root_entity_name ', row + 1,
   "            from dm_columns_doc d", row + 1, "            where parser(p_e_where_str)",
   row + 1, "            detail", row + 1,
   "               r_e_name = d.root_entity_name", row + 1, "            with nocounter",
   row + 1, "            if (curqual = 0)", row + 1,
   '               call echo("Information not found in dm_columns_doc for parent_entity_col")', row
    + 1, '               set p_e_name = "INVALIDTABLE"',
   row + 1, "               set r_e_name = p_e_name", row + 1,
   "            endif", row + 1, "         endif",
   row + 1, "         set kickout = kickout + 1", row + 1,
   "         if (kickout = 5)", row + 1,
   '            call echo("Searched through 5 Parent_entity_columns")',
   row + 1, '            set p_e_name = "INVALIDTABLE"', row + 1,
   "            set r_e_name = p_e_name", row + 1, "         endif",
   row + 1, "      endif", row + 1,
   "   endif", row + 1, "endwhile",
   row + 1, row + 1, "return (p_e_name)",
   row + 1, row + 1, "end",
   row + 1, row + 2,
   ";********************************************************************************",
   row + 1, ";dd_del_except deletes values from the dm_chg_log_exception table.", row + 1,
   ";********************************************************************************", row + 1,
   "subroutine dd_del_except(sbr_table_name, sbr_value)",
   row + 1, "declare except_tab = vc ", row + 1,
   row + 1, 'set except_tab = get_rdds_tname("DM_CHG_LOG_EXCEPTION") ', row + 1,
   row + 1, "DELETE FROM (parser(except_tab) d)", row + 1,
   "WHERE d.TARGET_ENV_ID = dm2_ref_data_doc->env_target_id", row + 1,
   "AND d.TABLE_NAME = sbr_table_name",
   row + 1, "AND d.FROM_VALUE = sbr_value", row + 1,
   "with nocounter", row + 1, row + 1,
   "if (error(err_msg, 0) > 0)", row + 1, "   call echo(err_msg)",
   row + 1, "   set nodelete_ind = 1", row + 1,
   "   set no_insert_update = 1", row + 1, "   set drdm_error_out_ind = 1",
   row + 1, "   set err_ind = 0", row + 1,
   "endif", row + 1, "end",
   row + 1, row + 1,
   ";********************************************************************************",
   row + 1, ";ddfill_rs fills out the meta-data doc structure", row + 1,
   ";********************************************************************************", row + 1,
   "subroutine DDFILL_RS(Type , Info )",
   row + 1, "declare Column_loop = i4", row + 1,
   "declare src_tab_name = vc", row + 1, "declare alg_code = f8",
   row + 1, "declare dd_IorU_only = f8", row + 1,
   "declare dd_IorU_meaning = vc", row + 1, row + 1,
   'If (Type = "TABLE")', row + 1, "   if (dddm2_rdds_get_tbl_col_info(info) = 0)",
   row + 1, "      return(0)", row + 1,
   "   endif", row + 1, row + 1,
   '   select into "nl:"', row + 1, "      di.info_char ",
   row + 1, "   from dm_info di", row + 1,
   '   where di.info_domain = "DATA MANAGEMENT"', row + 1,
   '      and di.info_name = concat("MERGE SCRIPT:", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt]->table_name)',
   row + 1, "   detail", row + 1,
   "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script = di.info_char", row + 1,
   "   with nocounter",
   row + 1, "   if (error(err_msg, 0) > 0)", row + 1,
   "      call echo(err_msg)", row + 1, "      set nodelete_ind = 1",
   row + 1, "      set no_insert_update = 1", row + 1,
   "      set drdm_error_out_ind = 1", row + 1, "      set err_ind = 0",
   row + 1, "   endif", row + 1,
   "   set alg_code = 0", row + 1, "   ; checking for seqmatch row",
   row + 1, '   select into "NL:"', row + 1,
   "   from dm_info di", row + 1, '   where di.info_domain = "RDDS SKIP SEQMATCH"',
   row + 1, "      and di.info_name = info", row + 1,
   "   with nocounter", row + 1, "   if (error(err_msg, 0) > 0)",
   row + 1, "      call echo(err_msg)", row + 1,
   "      set nodelete_ind = 1", row + 1, "      set no_insert_update = 1",
   row + 1, "      set drdm_error_out_ind = 1", row + 1,
   "      set err_ind = 0", row + 1, "   endif",
   row + 1, "   set alg_code = 0", row + 1,
   "   if (curqual = 1)", row + 1,
   "      set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].skip_seqmatch_ind = 1",
   row + 1, "   else", row + 1,
   "      set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].skip_seqmatch_ind = 0", row + 1, "   endif",
   row + 1,
   '   set alg_code = uar_get_code_by("DISPLAY", 255351, nullterm(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name))',
   row + 1,
   "   if (alg_code > 0)", row + 1,
   "      set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].version_type = uar_get_code_meaning(alg_code)",
   row + 1, '      if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].version_type = "NONE")', row + 1,
   "         set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].version_ind = 0", row + 1, "      else",
   row + 1, "         set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].version_ind = 1", row + 1,
   "      endif", row + 1, "   else",
   row + 1, "      set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].version_ind = 0", row + 1,
   "   endif", row + 1, row + 1,
   "   set dd_IorU_only = 0", row + 1,
   '   set dd_IorU_only = uar_get_code_by("DISPLAY", 4000220, nullterm(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name))',
   row + 1, "   if (dd_IorU_only > 0)", row + 1,
   "      set dd_IorU_meaning = uar_get_code_meaning(dd_IorU_only)", row + 1,
   '      if (dd_IorU_meaning = "NONE")',
   row + 1, "         set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].insert_only_ind = 0", row + 1,
   "         set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].update_only_ind = 0", row + 1,
   '      elseif (dd_IorU_meaning = "INSERT_ONLY")',
   row + 1, "         set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].insert_only_ind = 1", row + 1,
   '      elseif (dd_IorU_meaning = "UPDATE_ONLY")', row + 1,
   "         set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].update_only_ind = 1",
   row + 1, "      endif", row + 1,
   "   else", row + 1, "      set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].insert_only_ind = 0",
   row + 1, "      set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].update_only_ind = 0", row + 1,
   "   endif", row + 1, row + 1,
   '   select into "NL:"', row + 1,
   "   from (dummyt d with seq = value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)), ",
   row + 1, "      dm_info i", row + 1,
   "   plan d", row + 1,
   "   where dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].idcd_ind = 0",
   row + 1, "   join i", row + 1,
   '   where i.info_domain = concat("RDDS TRANS COLUMN:", dm2_ref_Data_doc->tbl_qual[temp_tbl_cnt].table_name)',
   row + 1,
   "     and i.info_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name",
   row + 1, "   detail", row + 1,
   "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].idcd_ind = 1", row + 1,
   "   with nocounter",
   row + 1, "   if (error(err_msg, 0) > 0)", row + 1,
   "      call echo(err_msg)", row + 1, "      set nodelete_ind = 1",
   row + 1, "      set no_insert_update = 1", row + 1,
   "      set drdm_error_out_ind = 1", row + 1, "      set err_ind = 0",
   row + 1, "   endif", row + 1,
   row + 1, "   SET tbl_loop = LOCATEVAL(tbl_loop, 1, dguc_reply->rs_tbl_cnt, ", row + 1,
   "      info, dguc_reply->dtd_hold[tbl_loop].tbl_name)", row + 1, row + 1,
   '   select into "nl:"', row + 1,
   "   from (dummyt d with seq = size(dm2_ref_Data_doc->tbl_qual[temp_tbl_cnt].col_qual, 5)),",
   row + 1, "      (dummyt d2 with seq = dguc_reply->dtd_hold[tbl_loop].pk_cnt)", row + 1,
   "   plan d", row + 1, "   join d2 ",
   row + 1, "      where dguc_reply->dtd_Hold[tbl_loop].pk_hold[d2.seq].pk_name = ", row + 1,
   "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name ", row + 1,
   "   detail",
   row + 1, "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].pk_ind = 1", row + 1,
   "   with nocounter", row + 1, row + 1,
   '   select into "NL:"', row + 1,
   "   from (dummyt d with seq = value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt))",
   row + 1, "   plan d", row + 1,
   ...
   row + 1, "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].active_ind_ind = 1", row + 1,
   "      endif", row + 1,
   ...
   row + 1,
   '         "BEG_EFF_DT_TM","CNTRCT_BEG_EFF_DT_TM","PRSNL_BEG_EFF_DT_TM"))', row + 1,
   "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].beg_col_name = ",
   row + 1, "            dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name", row
    + 1,
   "      endif", row + 1,
   "      if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name in (",
   row + 1,
   '         "END_EFFECTIVE_DT_TM","PRSNL_END_EFFECTIVE_DT_TM","END_EFFECTIVE_UTC_DT_TM","END_EFF_DT_TM","CNTRCT_EFF_DT_TM"))',
   row + 1,
   "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].end_col_name = ", row + 1,
   "            dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name",
   row + 1, "      endif", row + 1,
   '      if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].db_data_type = "*LOB")', row
    + 1,
   '         if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].db_data_type_tgt = "*LOB")',
   row + 1, '            dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].lob_process_type = "LOB_LOB"', row
    + 1,
   "         else", row + 1,
   '            dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].lob_process_type = "LOB_LONG"',
   row + 1, "         endif         ", row + 1,
   "      endif", row + 1, "   with nocounter",
   row + 1, row + 1, '   if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].beg_col_name != "" and',
   row + 1, '       dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].end_col_name != "")', row + 1,
   "      set dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind = 1", row + 1, "   endif",
   row + 1, "   set drdm_return_var = temp_tbl_cnt", row + 1,
   "endif", row + 1, "return (drdm_return_var)",
   row + 1, "end;subroutine", row + 1,
   row + 1, "subroutine dddm2_rdds_get_tbl_col_info (sbr_gtci_tname)", row + 1,
   row + 1, "declare sbr_cki_idx = i4", row + 1,
   '   Select into "NL:"', row + 1, "   from dm_tables_doc dtd",
   row + 1, "   where dtd.table_name = sbr_gtci_tname", row + 1,
   "   DETAIL", row + 1, "      temp_tbl_cnt = perm_tbl_cnt + 1",
   row + 1, "      perm_tbl_cnt = perm_tbl_cnt + 1", row + 1,
   "      STAT = ALTERLIST(dm2_ref_data_doc->tbl_qual ,value(size(dm2_ref_data_doc->tbl_qual,5) +1))",
   row + 1, "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name = dtd.table_name",
   row + 1,
   '      if (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name in ("ACCESSION","ADDRESS","PHONE","PERSON",',
   row + 1,
   '              "PERSON_NAME","PERSON_ALIAS","DCP_ENTITY_RELTN","LONG_TEXT","LONG_BLOB","ACCOUNT","AT_ACCT_RELTN"))',
   row + 1, "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].mergeable_ind = 1",
   row + 1, "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].reference_ind = 1", row + 1,
   "      else", row + 1,
   "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].mergeable_ind = dtd.mergeable_ind ",
   row + 1, "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].reference_ind = dtd.reference_ind",
   row + 1,
   "      endif", row + 1, "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix = dtd.table_suffix",
   row + 1, "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].merge_ui_query = dtd.merge_ui_query", row
    + 1,
   "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].merge_delete_ind = dtd.merge_delete_ind", row + 1,
   "      if (dtd.merge_ui_query = null)",
   row + 1, "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].merge_ui_query_ni = 1", row + 1,
   "      else", row + 1, "         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].merge_ui_query_ni = 0",
   row + 1, "      endif", row + 1,
   "   FOOT REPORT", row + 1, "      stat = alterlist(dm2_ref_data_doc->tbl_qual, temp_tbl_cnt)",
   row + 1, "      dm2_ref_data_doc->tbl_cnt= temp_tbl_cnt", row + 1,
   "   with nocounter", row + 1, "   if (error(err_msg, 0) > 0)",
   row + 1, "      call echo(err_msg)", row + 1,
   "     set nodelete_ind = 1", row + 1, "      set no_insert_update = 1",
   row + 1, "      set drdm_error_out_ind = 1", row + 1,
   "      set err_ind = 0", row + 1, "   endif",
   row + 1, "   if (curqual = 0)", row + 1,
   '      call echo("No table qualified from dm_tables_doc")', row + 1, "      return (0)",
   row + 1, "   endif", row + 1,
   '    set src_tab_name = get_rdds_tname("USER_TAB_COLUMNS")', row + 1, 'select into "NL:"',
   row + 1, "from dm_columns_doc dcd", row + 1,
   "   , user_tab_columns utc", row + 1, "   , (parser(src_tab_name) ut)",
   row + 1, "plan utc where utc.table_name = sbr_gtci_tname", row + 1,
   "join ut where utc.table_name = ut.table_name", row + 1,
   "    and utc.column_name = ut.column_name",
   row + 1, "join dcd where ut.table_name = dcd.table_name", row + 1,
   "   and ut.column_name = dcd.column_name", row + 1, "head report",
   row + 1, "   STAT = ALTERLIST(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual ,10)", row + 1,
   "   col_qual_cnt = 0", row + 1, "DETAIL",
   row + 1, "   col_qual_cnt = col_qual_cnt + 1", row + 1,
   "   IF (MOD(col_qual_cnt, 10) = 1 AND col_qual_cnt != 1)", row + 1,
   ...
   row + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].unique_ident_ind = dcd.unique_ident_ind",
   row + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].exception_flg = dcd.exception_flg",
   row + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].constant_value = dcd.constant_value",
   row + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].parent_entity_col = cnvtupper(dcd.parent_entity_col)",
   row + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].sequence_name = dcd.sequence_name",
   row + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].root_entity_name = cnvtupper(dcd.root_entity_name)",
   row + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].root_entity_attr = cnvtupper(dcd.root_entity_attr)",
   row + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].code_set = dcd.code_set", row
    + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].db_data_type = ut.data_type",
   row + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].db_data_length = ut.data_length",
   row + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].nullable = ut.nullable", row
    + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].db_data_type_tgt = utc.data_type",
   row + 1, "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].check_null = 0",
   row + 1, "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].translated = 0", row
    + 1,
   "   dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].merge_delete_ind = dcd.merge_delete_ind",
   row + 1, '   if (dcd.column_name in ("*_ID", "*_CD", "CODE_VALUE"))',
   row + 1, "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].idcd_ind = 1", row
    + 1,
   "   else", row + 1,
   "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].idcd_ind = 0",
   row + 1, "   endif", row + 1,
   '   if (ut.data_type in ("BLOB", "LONG RAW"))', row + 1,
   "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].binary_long_ind = 1",
   row + 1, "   else", row + 1,
   "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].binary_long_ind = 0", row
    + 1, "   endif",
   row + 1, "FOOT REPORT", row + 1,
   "   STAT = ALTERLIST(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual , col_qual_cnt)", row + 1,
   ...
   "       set drdm_error_out_ind = 1", row + 1, "       set err_ind = 0",
   row + 1, "    endif", row + 1,
   "    if (curqual = 0)", row + 1, '       call echo("No columns qualified from dm_columns_doc")',
   row + 1, "       return (0)", row + 1,
   "    endif", row + 1, row + 1,
   'select into "nl:" l.*', row + 1, "from dtableattr a, dtableattrl l",
   row + 1, "plan a where a.table_name = sbr_gtci_tname", row + 1,
   'join l where l.structtype = "F"', row + 1, "   and btest(l.stat,11) = 0",
   row + 1, "detail", row + 1,
   "   sbr_cdi_idx = 0", row + 1,
   "   sbr_cdi_idx = locateval(sbr_cdi_idx,1,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt,l.attr_name,",
   row + 1, "       dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[sbr_cdi_idx].column_name)", row
    + 1,
   "   if(sbr_cdi_idx > 0)", row + 1,
   "      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[sbr_cdi_idx].data_length = cnvtstring(l.len)",
   row + 1, '      If (l.type = "F")', row + 1,
   '         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[sbr_cdi_idx].data_type = "F8"', row +
   1, '      elseIf (l.type = "I")',
   row + 1,
   '         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[sbr_cdi_idx].data_type = "I4"', row +
   1,
   '      elseIf (l.type = "C")', row + 1,
   '         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[sbr_cdi_idx].data_type = "VC"',
   row + 1, '      elseIf (l.type = "Q")', row + 1,
   '         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[sbr_cdi_idx].data_type = "DQ8"', row
    + 1, "      endif",
   row + 1, "   endif", row + 1,
   "with nocounter", row + 1, "    if (error(err_msg, 0) > 0)",
   row + 1, "       call echo(err_msg)", row + 1,
   ...
   row + 1, "       set drdm_error_out_ind = 1", row + 1,
   "       set err_ind = 0", row + 1, "    endif",
   row + 1, row + 1, "    return (1)",
   row + 1, row + 1, "end  ;sub",
   row + 1, row + 1, "subroutine get_rdds_tname (sbr_tname)",
   row + 1, row + 1, "declare return_tname = vc",
   row + 1, row + 1, 'If(dm2_rdds_rec->mode = "OS") ',
   row + 1, "  ;regardless of main_process, we want to append $F", row + 1,
   '  set return_tname = concat(trim(substring(1,28,sbr_tname)),"$F")', row + 1,
   'elseif(dm2_rdds_rec->main_process="EXTRACTOR" and dm2_rdds_rec->mode = "DATABASE")',
   row + 1, "  set return_tname = sbr_tname", row + 1,
   'Elseif (dm2_rdds_rec->main_process="MOVER" and dm2_rdds_rec->mode = "DATABASE")', row + 1,
   "  set return_tname = concat(dm2_ref_data_doc->pre_link_name,sbr_tname,dm2_ref_data_doc->post_link_name)",
   row + 1, "Endif", row + 1,
   row + 1, "return(return_tname)", row + 1,
   "end ;sub", row + 1, row + 1,
   "#EXIT_", dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix, row + 1,
   "free set cust_catalog_cd_s", row + 1, "free set cust_catalog_cd",
   row + 1, "free set comp_seq_cnt", row + 1,
   "free set cust_tab_name_s", row + 1, "free set cust_cs_loop",
   row + 1, "free set cust_col_cnt", row + 1,
   "free set cust_col_num", row + 1, "free set cust_cust_index_var",
   row + 1, "free set cust_check_loop", row + 1,
   "free set cust_nodelete_ind ", row + 1, "free set cust_chg_log",
   row + 1, "free set cust_merge_loop", row + 1,
   "free set cust_cur_merges", row + 1, "free set cust_fail_merges",
   row + 1, "free set cust_del_msg", row + 1,
   "free set cust_where_clause", row + 1, "free set cust_parser_cnt",
   row + 1, "free set cust_col_name", row + 1,
   "free set passed_var", row + 1, "free set cust_loop",
   row + 1, "free set cust_from_val", row + 1,
   "free set cust_to_val", row + 1, "free set cust_from_str",
   row + 1, "end go", row + 1
  WITH append, formfeed = none, maxrow = 1,
   format = variable
 ;end select
 CALL compile(dyn_f_name)
#exit_main
END GO
