CREATE PROGRAM dm_stat_user_experience:dba
 ...
 SELECT INTO "nl:"
  ...
  pnl.physician_ind, pnl.position_cd
  ...
  DETAIL
   ...
   dsr->qual[qualcnt].qual[ds_cnt].stat_str_val = build(trim(substring(1,80,pnl.name_last)),"|",trim(
    ...
    "|",pnl.physician_ind,"|",uar_get_code_display(pnl.position_cd)), dsr->qual[qualcnt].qual[ds_cnt]
 ...
 SET qualcnt = 0
 SET ds_cnt = 1
 DECLARE fnd = i2 WITH noconstant(0)
   ...
   FROM (
    ...
    (SELECT DISTINCT
     ...
     WHERE p.problem_type_flag=0
      AND pa.updt_dt_tm BETWEEN cnvtdatetime(ds_begin_snapshot) AND cnvtdatetime(ds_end_snapshot)
 ...
 SET ds_cnt = 1
 SELECT INTO "nl:"
  a.created_prsnl_id, cnt = count(*), pnl.name_last,
  pnl.name_first, pnl.username, pnl.physician_ind,
  pnl.position_cd
  FROM allergy a,
   prsnl pnl
  WHERE a.created_dt_tm BETWEEN cnvtdatetime(ds_begin_snapshot) AND cnvtdatetime(ds_end_snapshot)
   ...
   AND a.active_ind=1
   AND a.created_prsnl_id=pnl.person_id
  GROUP BY a.created_prsnl_id, pnl.name_last, pnl.name_first,
   pnl.username, pnl.physician_ind, pnl.position_cd
  HEAD REPORT
   IF (ds_cnt=1)
    qualcnt = (qualcnt+ 1), stat = alterlist(dsr->qual,qualcnt), dsr->qual[qualcnt].stat_snap_dt_tm
     = cnvtdatetime(ds_begin_snapshot),
    dsr->qual[qualcnt].snapshot_type = "UE_ALLERGIES_HISTORIES"
   ENDIF
   stat_seq = 0
  DETAIL
   IF (mod(ds_cnt,10)=1)
  ...
  WITH nocounter
 ;end select
 CALL dsvm_error("UE_ALLERGIES_HISTORIES - UE_NBR_ALLERGIES_DOCUMENTED")
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 CALL dsvm_error("UE_ALLERGIES_HISTORIES - UE_NBR_ALLERGIES_MODIFIED")
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 CALL dsvm_error("UE_ALLERGIES_HISTORIES - UE_NBR_ALLERGIES_REMOVED")
 SELECT INTO "nl:"
  p_inner.updt_id, cnt = count(*), pnl.name_last,
  pnl.name_first, pnl.username, pnl.physician_ind,
  pnl.position_cd
  ...
  WITH nocounter
 ;end select
 CALL dsvm_error("UE_ALLERGIES_HISTORIES - UE_NBR_PAST_MED_HISTORIES_DOCUMENTED")
 IF (qualcnt > 0)
  SET stat = alterlist(dsr->qual[qualcnt].qual,(ds_cnt - 1))
 ENDIF
 EXECUTE dm_stat_snaps_load
 ...
 SET ds_cnt = 1
 SELECT INTO "nl:"
  e.dlg_prsnl_id, cnt = count(*), pnl.name_last,
  pnl.name_first, pnl.username, pnl.physician_ind,
  pnl.position_cd
  FROM eks_dlg_event e,
   prsnl pnl
  WHERE e.updt_dt_tm BETWEEN cnvtdatetime(ds_begin_snapshot) AND cnvtdatetime(ds_end_snapshot)
   AND e.dlg_name != "MUL_MED!*"
   AND e.dlg_prsnl_id=pnl.person_id
  GROUP BY e.dlg_prsnl_id, pnl.name_last, pnl.name_first,
   pnl.username, pnl.physician_ind, pnl.position_cd
  HEAD REPORT
   IF (ds_cnt=1)
    qualcnt = (qualcnt+ 1), stat = alterlist(dsr->qual,qualcnt), dsr->qual[qualcnt].stat_snap_dt_tm
     = cnvtdatetime(ds_begin_snapshot),
    dsr->qual[qualcnt].snapshot_type = "UE_ALERTS"
   ENDIF
   stat_seq = 0
  DETAIL
   IF (mod(ds_cnt,10)=1)
    stat = alterlist(dsr->qual[qualcnt].qual,(ds_cnt+ 9))
   ENDIF
   dsr->qual[qualcnt].qual[ds_cnt].stat_name = "UE_NBR_DISCERN_ALERTS_FIRED", dsr->qual[qualcnt].
   qual[ds_cnt].stat_str_val = build(trim(substring(1,80,pnl.name_last)),"|",trim(substring(1,80,pnl
      .name_first)),"|",trim(pnl.username),
    "|",pnl.physician_ind,"|",uar_get_code_display(pnl.position_cd)), dsr->qual[qualcnt].qual[ds_cnt]
   .stat_number_val = cnt,
  ...
  WITH nocounter
 ;end select
 CALL dsvm_error("UE_ALERTS - UE_NBR_DISCERN_ALERTS_FIRED")
 DECLARE overridden_cnt = f8 WITH noconstant(0.0)
 DECLARE overriddable_cnt = f8 WITH noconstant(0.0)
 SELECT INTO "nl:"
  e_out.dlg_prsnl_id, cnt1 = count(*), cnt2 = e_inner.cnt2,
  pnl.name_last, pnl.name_first, pnl.username,
  pnl.physician_ind, pnl.position_cd
  FROM eks_dlg_event e_out,
   (
   (
   (SELECT
    e.dlg_prsnl_id, cnt2 = count(*)
    FROM eks_dlg_event e
    WHERE e.updt_dt_tm BETWEEN cnvtdatetime(ds_begin_snapshot) AND cnvtdatetime(ds_end_snapshot)
     AND trim(e.dlg_name) IN (
    (SELECT DISTINCT
     e_inner.dlg_name
     FROM eks_dlg_event e_inner
     WHERE e_inner.override_reason_cd > 0
      AND e_inner.dlg_name != "MUL_MED*"
      AND e_inner.updt_dt_tm BETWEEN cnvtdatetime(ds_begin_snapshot) AND cnvtdatetime(ds_end_snapshot
      )))
    GROUP BY e.dlg_prsnl_id
    WITH sqltype("F8","F8")))
   e_inner),
   prsnl pnl
  WHERE e_out.updt_dt_tm BETWEEN cnvtdatetime(ds_begin_snapshot) AND cnvtdatetime(ds_end_snapshot)
   AND e_out.override_reason_cd > 0
   AND e_out.dlg_name != "MUL_MED*"
   AND ((e_out.dlg_prsnl_id+ 0)=e_inner.dlg_prsnl_id)
   AND e_out.dlg_prsnl_id=pnl.person_id
  GROUP BY e_out.dlg_prsnl_id, e_inner.cnt2, pnl.name_last,
   pnl.name_first, pnl.username, pnl.physician_ind,
   pnl.position_cd
  HEAD REPORT
   IF (ds_cnt=1)
  ...
  DETAIL
   IF (mod(ds_cnt,10)=1)
    stat = alterlist(dsr->qual[qualcnt].qual,(ds_cnt+ 9))
   ENDIF
   dsr->qual[qualcnt].qual[ds_cnt].stat_name = "UE_PERC_DISCERN_ALERTS_OVERRIDDEN", dsr->qual[qualcnt
   ].qual[ds_cnt].stat_str_val = build(trim(substring(1,80,pnl.name_last)),"|",trim(substring(1,80,
      pnl.name_first)),"|",trim(pnl.username),
    "|",pnl.physician_ind,"|",uar_get_code_display(pnl.position_cd))
   IF (cnt2=0)
    dsr->qual[qualcnt].qual[ds_cnt].stat_number_val = 0
   ELSE
    overridden_cnt = cnt1, overriddable_cnt = cnt2, dsr->qual[qualcnt].qual[ds_cnt].stat_number_val
     = ((overridden_cnt/ overriddable_cnt) * 100)
   ENDIF
   dsr->qual[qualcnt].qual[ds_cnt].stat_type = 1, dsr->qual[qualcnt].qual[ds_cnt].stat_seq = stat_seq,
   ds_cnt = (ds_cnt+ 1),
   stat_seq = (stat_seq+ 1)
  WITH nocounter
 ;end select
 CALL dsvm_error("UE_ALERTS - UE_PERC_DISCERN_ALERTS_OVERRIDDEN")
 IF (qualcnt > 0)
  SET stat = alterlist(dsr->qual[qualcnt].qual,(ds_cnt - 1))
 ENDIF
 EXECUTE dm_stat_snaps_load
