CREATE PROGRAM dm_eks_module_audit_rows:dba
 ...
 SET reply->table_name = "EKS_MODULE_AUDIT"
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    ema.rowid
    FROM eks_module_audit ema
    WHERE parser(sbr_getrowidnotexists("ema.rec_id between curMinID and curMaxID","ema"))
     AND ema.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = ema.rowid
    WITH nocounter, maxqual(ema,value(rowsleft))
   ;end select
