CREATE PROGRAM bed_aud_phasex_micro_orc_match:dba
  ...
  SELECT INTO "nl:"
   ...
   PLAN (b
    WHERE b.catalog_type="LAB*"
     AND cnvtupper(b.activity_type)="MICRO*")
   DETAIL
 ...
 SELECT INTO "nl:"
  ...
  PLAN (c
   WHERE c.code_set=106
    AND c.cdf_meaning="MICROBIOLOGY"
    AND c.active_ind=1)
  DETAIL
   act_cd = c.code_value
  WITH nocounter
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("x_micro_orc_match_audit.csv")
 ENDIF
