CREATE PROGRAM dm2_aload_code_value
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_cv",".log","dm2_aload_code_value")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_code_value"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dacv_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dacv_file = vc WITH protect, noconstant( $2)
 DECLARE dacv_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dacv_batch = i4 WITH protect, noconstant(0)
 DECLARE dacv_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE dacv_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dacv_pkg,dacv_file_loc_ret,dacv_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dacv_batch=0)
  SET dacv_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_AFD_CODE_VALUE for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND dacv_rows_returned=10000)
   DELETE  FROM dm_afd_code_value d
    WHERE d.alpha_feature_nbr=dacv_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET dacv_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",dacv_rows_returned))
   ENDIF
 ENDWHILE
 IF (dacv_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dacv_file_loc_ret,dacv_file), "dm2_aload_cs_cv", dacv_batch
  CALL echo(concat(dacv_file_loc_ret,dacv_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_code_value completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_code_value")
END GO
