CREATE PROGRAM bbd_rdm_upd_14220:dba
 ...
 SUBROUTINE insertreqproc(nrequest,spfmt,ntargetreq,ndeststep,sservice,nreprocess,naction,srefpfmt,
  nseq)
   ...
   ENDCASE
 END ;Subroutine
 DECLARE script_name = c17 WITH protect, constant("bbd_rdm_upd_14220")
 DECLARE counsel_mean = vc WITH protect, constant("COUNSEL")
 DECLARE donate_mean = vc WITH protect, constant("DONATE")
 DECLARE other_mean = vc WITH protect, constant("OTHER")
 DECLARE recruit_mean = vc WITH protect, constant("RECRUIT")
 DECLARE contact_type_cs = i4 WITH protect, constant(14220)
 DECLARE counsel_found_ind = i2 WITH protect, noconstant(0)
 DECLARE donate_found_ind = i2 WITH protect, noconstant(0)
 DECLARE other_found_ind = i2 WITH protect, noconstant(0)
 DECLARE recruit_found_ind = i2 WITH protect, noconstant(0)
 DECLARE list_count = i4 WITH protect, noconstant(0)
 DECLARE stat = i2 WITH protect, noconstant(0)
 DECLARE found_dup_ind = i2 WITH protect, noconstant(0)
 FREE RECORD duplicates
 RECORD duplicates(
   1 inactivate_list[*]
     2 code_value = f8
 )
 ...
 SELECT INTO "nl:"
  FROM code_value cv
  WHERE cv.code_set=contact_type_cs
   AND cv.active_ind=1
  ...
  DETAIL
   CASE (cv.cdf_meaning)
    OF counsel_mean:
     IF (counsel_found_ind=0)
      counsel_found_ind = 1
     ELSE
      found_dup_ind = 1
     ENDIF
    OF donate_mean:
     IF (donate_found_ind=0)
      donate_found_ind = 1
     ELSE
      found_dup_ind = 1
     ENDIF
    OF other_mean:
     IF (other_found_ind=0)
      other_found_ind = 1
     ELSE
      found_dup_ind = 1
     ENDIF
    OF recruit_mean:
     IF (recruit_found_ind=0)
      recruit_found_ind = 1
     ELSE
      found_dup_ind = 1
     ENDIF
   ENDCASE
   IF (found_dup_ind=1)
    list_count = (list_count+ 1), stat = alterlist(duplicates->inactivate_list,list_count),
    duplicates->inactivate_list[list_count].code_value = cv.code_value
   ENDIF
  WITH nocounter
 ;end select
 FOR (i_idx = 1 TO list_count)
   ...
   ENDIF
   IF (stat=1)
    SET stat = gm_u_code_value0619_f8("CODE_VALUE",duplicates->inactivate_list[i_idx].code_value,1,0,
     1)
   ENDIF
   IF (stat=1)
   ...
   ELSE
    ROLLBACK
    CALL logstatus(build(script_name," Execution Failed.  Unable to update code_value ",duplicates->
      inactivate_list[i_idx].code_value,"."),"F")
    GO TO script_end
   ENDIF
   FREE RECORD gm_u_code_value0619_req
   FREE RECORD gm_u_code_value0619_rep
 ENDFOR
 SET counsel_found_ind = 0
 SET donate_found_ind = 0
 SET other_found_ind = 0
 SET recruit_found_ind = 0
 SET found_dup_ind = 0
 SELECT INTO "nl:"
  FROM code_value cv
  WHERE cv.code_set=contact_type_cs
  ...
  WITH nocounter
 ;end select
 IF (counsel_found_ind=1
  AND donate_found_ind=1
  AND other_found_ind=1
  AND recruit_found_ind=1
  AND found_dup_ind=0)
  CALL logstatus(build(script_name," Code set 14220 successfully updated."),"S")
 ELSE
  CALL logstatus(build(script_name," Code set 14220 update failed."),"F")
 ENDIF
#script_end
 FREE RECORD gm_u_code_value0619_req
 FREE RECORD gm_u_code_value0619_rep
