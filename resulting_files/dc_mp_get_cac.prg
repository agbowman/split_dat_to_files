CREATE PROGRAM dc_mp_get_cac
 IF ( NOT (validate(caceval)))
  RECORD caceval(
    1 listcnt = i4
    1 colmncnt = i4
    1 list[*]
 ...
 SUBROUTINE organizesequence(rec,cond_seq,cond_id)
   ...
   CALL lhprint(build(";organizeSequence Process time: ",datetimediff(cnvtdatetime(curdate,curtime3),
      begin_dt_tm,5)))
 END ;Subroutine
 DECLARE ver_num = f8 WITH protect, constant(iqrversioncontrol("CAC",cnvtdatetime(curdate,curtime3)))
 IF (( $2 != null))
  SET jrec = cnvtjsontorec( $2)
  IF (validate(debug_dc_mp_get_cac_ind,0)=1)
 ...
 DECLARE emeasversion = i4 WITH constant(emeasversioncontrol("MU_CQM_EH_CAC",cnvtdatetime(curdate,
    curtime3))), protect
 IF ((cacreply->qmversion=1))
  GO TO exit_script
 ELSE
  SET stat = initrec(cacreply)
  IF (checkemeasureenabled(_flexid,_logicaldomainid,"MP_QM_ECAC_2")=1)
   IF (emeasversion >= 2017
    AND checkdic("LH_MP_ORGANIZER_ECAC","P",0) > 0)
    EXECUTE lh_mp_organizer_ecac  WITH replace("REQUEST",qmreq), replace("REPLY",cacreply), replace(
     "FLEXID",_flexid),
    replace("DOMAINID",_logicaldomainid)
    GO TO exit_script
   ENDIF
  ELSEIF (ver_num=5.0
   AND checkdic("LH_MP_ORGANIZER_CAC","P",0) > 0)
   EXECUTE lh_mp_organizer_cac  WITH replace("REQUEST",qmreq), replace("REPLY",cacreply), replace(
    "FLEXID",_flexid),
   replace("DOMAINID",_logicaldomainid)
   GO TO exit_script
  ELSE
...
#exit_script
 IF (validate(_memory_reply_string)=1)
  SET _memory_reply_string = cnvtrectojson(cacreply)
 ELSE
  CALL echojson(cacreply, $1)
  SELECT INTO "nl:"
   DETAIL
    row + 0
   WITH nocounter
