CREATE PROGRAM ecf_rdm_nicip_con_rel_ext:dba
 ...
 SUBROUTINE kia_script_inc(pkg,ccl_obj)
   ...
   CALL parser(obj_prs)
 END ;Subroutine
 DECLARE kia_concept_filename = vc WITH public, constant("nicip_concept.csv")
 DECLARE kia_reltn_filename = vc WITH public, constant("nicip_relation.csv")
 DECLARE kia_ext_csv_filename = vc WITH public, constant("nicip_extension.csv")
 DECLARE kia_cur_ver_beg_dt = vc WITH public, noconstant("")
 SET kia_tbl_desc = "NICIP Concept Relation Extension"
 SET kia_short_desc = "NICIP Concept Relation Extension"
 SET kia_rdm_log_name = "ecf_rdm_nicip_con_rel_ext.log"
 SET kia_rdm_nbr = 10080
 SET kia_rdm_vocab_mean = "NICIP"
 SET readme_data->status = "F"
 ...
 EXECUTE kia_dm_dbimport "cer_install:nicip_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("ecf_rdm_nicip_con_rel_ext Begin>  ",format(cnvtdatetime(curdate,curtime3),
 ...
 IF (kia_obj_suc_ind=0)
  ...
  DELETE  FROM cmt_concept_load_ns c
   WHERE ((c.concept_cki="NICIP*") OR (c.concept_cki="SNOMED*"))
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
 ...
 ENDIF
 CALL kia_chk_status_info("kia_imp_cmt_concept_ext","check")
 IF (kia_obj_suc_ind != 1)
  CALL echo("DELETING ROWS FROM CMT_CONCEPT_EXTENSION TABLE...")
  DELETE  FROM cmt_concept_extension c
   WHERE c.concept_cki="NICIP*"
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_ext_csv_filename,"kia_imp_cmt_concept_ext","kia_imp_nicip_ext.log")
  CALL kia_set_status_success("kia_imp_cmt_concept_ext")
 ENDIF
 CALL kia_set_parse_row_success(kia_tbl_desc)
 SET kia_version_chk = 0
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(10069,"NICIP NOMEN"))
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(10080,
  "NICIP Concept Relation Extension"))
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(10081,"NICIP XMAP"))
 CALL echo(build("README'S ALREADY COMPLETED:",kia_version_chk))
 IF (kia_version_chk=3)
