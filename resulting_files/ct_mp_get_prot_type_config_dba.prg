CREATE PROGRAM ct_mp_get_prot_type_config:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "protTypeCd" = 0
  WITH outdev, prottypecd
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "ct_mp_get_prot_type_config"
 ...
 IF (log_domain_exists_ind=2)
  ...
  SET log_domain_where_str = "conf.logical_domain_id = outerjoin(domain_reply->logical_domain_id)"
 ENDIF
 SELECT INTO "nl:"
  FROM code_value cv,
  ...
  PLAN (cv
   ...
   WHERE (conf.item_cd= Outerjoin(cvg.child_code_value))
    ...
    AND (conf.protocol_type_cd= Outerjoin( $PROTTYPECD))
  ...
  HEAD cv.code_value
   ...
   ENDIF
   reply->categories[cnt].category_cd = cv.code_value, reply->categories[cnt].category_mean =
   uar_get_code_meaning(cv.code_value)
  DETAIL
   item_cnt += 1
   IF (mod(item_cnt,10)=1)
    stat = alterlist(reply->categories[cnt].item_list,(item_cnt+ 9))
   ENDIF
   reply->categories[cnt].item_list[item_cnt].item_cd = cvg.child_code_value, reply->categories[cnt].
   item_list[item_cnt].item_mean = uar_get_code_meaning(cvg.child_code_value)
   IF (conf.ct_prot_type_config_id > 0.0)
    ...
    reply->categories[cnt].item_list[item_cnt].value_cd = conf.config_value_cd, reply->categories[cnt
    ].item_list[item_cnt].value_mean = uar_get_code_meaning(conf.config_value_cd)
   ENDIF
