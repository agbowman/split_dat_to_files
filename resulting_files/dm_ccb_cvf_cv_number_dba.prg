CREATE PROGRAM dm_ccb_cvf_cv_number:dba
 ...
 ENDIF
 DECLARE cvf_cvn_status = c1 WITH protect, noconstant("S")
 DECLARE cvf_cvn_errcode = i4 WITH protect, noconstant(0)
 DECLARE cvf_cvn_errmsg = vc WITH protect, noconstant(" ")
 DECLARE cvf_cvn_filter = vc WITH protect, noconstant(" ")
 IF (size(request->filterlist,5)=0)
  SET cvf_cvn_errmsg = "No code value number search criteria submitted."
  SET cvf_cvn_status = "F"
  GO TO exit_program
 ENDIF
 IF (cnvtreal(request->filterlist[1].column_value)=0.0)
  SET cvf_cvn_errmsg = "Must submit a non-zero code value number."
  SET cvf_cvn_status = "F"
  GO TO exit_program
 ENDIF
 SET cvf_cvn_filter = filterstring_builder(cvf_cvn_filter,"cv",request)
 SELECT INTO "nl:"
  cv.code_set, cv.code_value, cv.display,
  cv.cdf_meaning
  FROM code_value cv
  PLAN (cv
   WHERE parser(cvf_cvn_filter)
  ...
  WITH nocounter
 ;end select
 SET cvf_cvn_errcode = error(cvf_cvn_errmsg,1)
 IF (cvf_cvn_errcode != 0)
  SET cvf_cvn_errmsg = concat("Error searching for code value number: ",cvf_cvn_errmsg)
  SET cvf_cvn_status = "F"
  GO TO exit_program
 ELSEIF (size(reply->cd_value_list,5)=0)
  SET cvf_cvn_errmsg = build2("No values found for code value number: ",request->filterlist[1].
   column_value)
  SET cvf_cvn_status = "F"
  GO TO exit_program
 ENDIF
#exit_program
 SET reply->status_data.status = cvf_cvn_status
 SET reply->message = cvf_cvn_errmsg
END GO
