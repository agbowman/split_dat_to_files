CREATE PROGRAM dm_rmc_rpt_ptam_setup:dba
 ...
 SUBROUTINE daf_is_not_blank(dinb_str)
  ...
  RETURN(false)
 END ;Subroutine
 IF (validate(drrps_request)=0)
  FREE RECORD drrps_request
  RECORD drrps_request(
    1 table_name = vc
    1 ctxt_cnt = i4
    1 ctxt_qual[*]
      2 ctxt_name = vc
 ...
 SET stat = moverec(drrps_request,drrps_local)
 ...
 DECLARE drrps_tab_str = vc WITH protect, noconstant(" ")
 SET dm_err->eproc = "Starting dm_rmc_rpt_ptam_setup..."
 IF (check_logfile("dm_rmc_rpt_ptam",".log","DM_RMC_RPT_PTAM_SETUP LOG")=0)
  SET dm_err->err_ind = 1
 ...
 ELSEIF (daf_is_blank(drgi_global->db_link))
  ...
  GO TO exit_program
 ENDIF
 IF ((drrps_local->ctxt_cnt=1)
  AND (drrps_local->ctxt_qual[1].ctxt_name != "NULL"))
  SET rpaw_ctxt_str = " d.context_name = patstring(drrps_local->ctxt_qual[1].ctxt_name)"
 ELSEIF ((drrps_local->ctxt_cnt=1))
  SET rpaw_ctxt_str = " d.context_name is null"
 ELSEIF ((drrps_local->ctxt_cnt > 1))
  SET rpaw_ctxt_str = "( d.context_name IN("
  FOR (rpaw_pos = 1 TO drrps_local->ctxt_cnt)
    IF ((drrps_local->ctxt_qual[rpaw_pos].ctxt_name != "NULL"))
     SET rpaw_ctxt_str = concat(rpaw_ctxt_str,"'",drrps_local->ctxt_qual[rpaw_pos].ctxt_name,"',")
    ELSE
 ...
 ENDIF
 IF (daf_is_not_blank(drrps_local->table_name))
  SET drrps_tab_str = " d.table_name = drrps_local->table_name"
 ELSE
  SET drrps_tab_str = " 1 = 1 "
 ENDIF
 SET task_info->process_name = "RDDS PTAM AUDIT"
 SET task_info->environment_id = drgi_global->target_id
 IF ((drrps_local->ctxt_cnt > 0))
  SET rpaw_temp_str = concat("<VAL_SET>set drrp_request->ctxt_cnt = ",trim(cnvtstring(drrps_local->
     ctxt_cnt))," go</VAL_SET> ","<VAL_SET>set stat = alterlist(drrp_request->ctxt_qual, ",trim(
    cnvtstring(drrps_local->ctxt_cnt)),
   ") go</VAL_SET>")
  FOR (rpaw_pos = 1 TO drrps_local->ctxt_cnt)
    SET rpaw_temp_str = concat(rpaw_temp_str,"<VAL_SET>set drrp_request->ctxt_qual[",trim(cnvtstring(
       rpaw_pos)),"].ctxt_name = '",drrps_local->ctxt_qual[rpaw_pos].ctxt_name,
     "' go</VAL_SET> ")
  ENDFOR
 ENDIF
 SELECT DISTINCT INTO "nl:"
  ...
  WHERE (d.target_env_id=drgi_global->target_id)
   AND d.log_type IN ("NOMV19", "NOMV20")
   AND parser(rpaw_ctxt_str)
   AND parser(drrps_tab_str)
  DETAIL
   ...
   task_info->qual[task_info->total].task_desc = concat("Ptam audit for ",trim(d.table_name)),
   task_info->qual[task_info->total].task_name = "execute dm_rmc_rpt_ptam go", task_info->qual[
   task_info->total].task_request = concat(
    "<REC_DEF>record drrp_request (1 table_name = vc 1 audit_name = vc 1 ctxt_cnt = i4 ",
    " 1 ctxt_qual[*] 2 ctxt_name = vc) go</REC_DEF> ","<VAL_SET>set drrp_request->table_name = '",
    trim(d.table_name),"' go</VAL_SET> ",
    rpaw_temp_str),
   task_info->qual[task_info->total].task_reply = concat(
    "<REC_DEF>record drrp_reply(1 status = c1 1 status_msg = vc) go</REC_DEF> ",
    "<ERR_IND>drrp_reply->status</ERR_IND> ","<ERR_MSG>drrp_reply->status_msg</ERR_MSG> ",
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "...Ending dm_rmc_rpt_ptam_setup"
 CALL final_disp_msg("dm_rmc_rpt_ptam")
END GO
