CREATE PROGRAM bhs_req_04_layout_transfuse:dba
 PROMPT
  "program_name" = ""
  WITH program_name
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   catalog_type_cd = o.catalog_type_cd, orders->qual[ord_cnt].f_catalog_cd = o.catalog_cd,
  ...
  IF (curqual > 0)
   ...
   FOR (prt_ord = 1 TO size(orders->qual,5))
     ...
     SUBROUTINE (transfuse_headerabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
       ...
       IF ( NOT (program1="BHS_REQ_BBT"))
        RETURN(0.0)
       ENDIF
       IF (ncalc=rpt_render)
        SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
        SET _rptstat = uar_rptrect(_hreport,(offsetx+ 0.000),(offsety+ 0.938),4.000,0.625,
         rpt_nofill,rpt_white)
        SET _rptstat = uar_rptrect(_hreport,(offsetx+ 4.000),(offsety+ 0.000),3.438,0.938,
         rpt_nofill,rpt_white)
        SET _rptstat = uar_rptrect(_hreport,(offsetx+ 4.000),(offsety+ 0.938),3.438,0.625,
         rpt_nofill,rpt_white)
        SET _rptstat = uar_rptrect(_hreport,(offsetx+ 0.000),(offsety+ 0.375),1.750,0.563,
         rpt_nofill,rpt_white)
        SET _rptstat = uar_rptrect(_hreport,(offsetx+ 0.000),(offsety+ 0.000),4.000,0.375,
         rpt_nofill,rpt_white)
        SET rptsd->m_flags = 4
        ...
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Con Ordered(X) Yes[ ]  No [ ]",
          char(0)))
        ...
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
          "# of Units/Volume Ordered:_________________________",char(0)))
        SET rptsd->m_y = (offsety+ 0.188)
        SET rptsd->m_x = (offsetx+ 4.250)
        SET rptsd->m_width = 3.188
        SET rptsd->m_height = 0.146
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
          "[ ] RBC  [ ] PLT   [ ] FFT   [ ] CRY   Other:____________________",char(0)))
        ...
        SET rptsd->m_height = 0.146
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
          "O.R. Date:_____________________________",char(0)))
        SET rptsd->m_flags = 20
        SET rptsd->m_y = (offsety+ 0.563)
        SET rptsd->m_x = (offsetx+ 0.063)
        SET rptsd->m_width = 1.500
        SET rptsd->m_height = 0.146
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("[ ] T&S   [ ] ABORH   [ ] CORD",
          char(0)))
        SET rptsd->m_y = (offsety+ 0.375)
        SET rptsd->m_x = (offsetx+ 1.875)
        SET rptsd->m_width = 2.125
        ...
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
          "AntiBodies_______________________________________________________________",char(0)))
        SET rptsd->m_y = (offsety+ 0.750)
        SET rptsd->m_x = (offsetx+ 1.875)
        SET rptsd->m_width = 2.125
        SET rptsd->m_height = 0.146
        ...
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
          "PAD Billing#:__________________________",char(0)))
        ...
        SET rptsd->m_height = 0.146
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
          "Other:___________________________________________________",char(0)))
        SET rptsd->m_flags = 20
        ...
        SET _dummyfont = uar_rptsetfont(_hreport,_times8i0)
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
          "Comments:___________________________________________________________________",char(0)))
        SET rptsd->m_y = (offsety+ 1.188)
        SET rptsd->m_x = (offsetx+ 0.063)
        SET rptsd->m_width = 3.938
        SET rptsd->m_height = 0.146
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
          "and       ________________________________________________________________",char(0)))
        SET rptsd->m_flags = 20
     ...
     SUBROUTINE (requistion_headabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
       ...
       DECLARE __facilty_name = vc WITH noconstant(build2(orders->facility,char(0))), protect
       IF ( NOT (program1 != cnvtupper("BHS_REQ_BBT")))
        RETURN(0.0)
       ENDIF
       IF (ncalc=rpt_render)
        SET rptsd->m_flags = 20
     ...
     SUBROUTINE (print_headerabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
      WITH protect)
       DECLARE sectionheight = f8 WITH noconstant(2.560000), private
       ...
       DECLARE __temp_loc = vc WITH noconstant(build2(uar_get_code_display(temp_cd),char(0))),
       protect
       ...
       IF (_remprimary_diag > 0)
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_remprimary_diag,((
           size(__primary_diag) - _remprimary_diag)+ 1),__primary_diag)))
     ...
     SUBROUTINE (order_sectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
       ...
       DECLARE __mnemonic = vc WITH noconstant(build2(orders->qual[prt_ord].mnemonic,char(0))),
       protect
       DECLARE __order_status = vc WITH noconstant(build2(orders->qual[prt_ord].status,char(0))),
       protect
       DECLARE __order_id = vc WITH noconstant(build2(orders->qual[prt_ord].order_id,char(0))),
       protect
       IF (ncalc=rpt_render)
        SET rptsd->m_flags = 4
        SET rptsd->m_borders = rpt_sdnoborders
     ...
     SUBROUTINE (line1abs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
       ...
       IF (ncalc=rpt_render)
        SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
        SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.047),(offsetx+ 7.479),(
         offsety+ 0.047))
        SET _yoffset = (offsety+ sectionheight)
       ENDIF
       RETURN(sectionheight)
     ...
     SUBROUTINE (order_detailsabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
      WITH protect)
       ...
       IF (_remdetail_value > 0)
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_remdetail_value,((
           size(__detail_value) - _remdetail_value)+ 1),__detail_value)))
       ...
       IF (_remdetail_label > 0)
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_remdetail_label,((
           size(__detail_label) - _remdetail_label)+ 1),__detail_label)))
     ...
     SUBROUTINE (order_commentsabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
       ...
       IF (_remorder_comment > 0)
        SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_remorder_comment,((
           size(__order_comment) - _remorder_comment)+ 1),__order_comment)))
        ...
        ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remorder_comment,((size(
     ...
     SUBROUTINE (order_comments_headerabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
       ...
       IF (ncalc=rpt_render)
        SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
        SET _rptstat = uar_rptrect(_hreport,(offsetx+ - (0.010)),(offsety+ 1.000),7.510,0.260,
         rpt_nofill,rpt_white)
        SET rptsd->m_flags = 4
        SET rptsd->m_borders = rpt_sdnoborders
        SET rptsd->m_padding = rpt_sdnoborders
     ...
     SUBROUTINE initializereport(dummy)
       ...
       SET rptreport->m_reportname = "BHS_REQ_04_LAYOUT_TRANSFUSE"
     ...
     IF (((_yoffset+ order_comments_header(rpt_calcheight)) > page_size))
      SET _yoffset = 9.4
      SET d0 = print_footer(rpt_render)
      SET d0 = foot_page(rpt_render)
      SET d0 = pagebreak(0)
