CREATE PROGRAM ags_immun_backout:dba
 PROMPT
  "Backout Type (J, I, P) =" = "J",
  "AGS_JOB_ID (0.0) = " = 0.0
  WITH cbackout_type, djob_id
 CALL echo("***")
 CALL echo("***   BEG :: AGS_IMMUN_BACKOUT")
 CALL echo("***")
 ...
 IF ( NOT (validate(log,0)))
  ...
  DECLARE sstatus_file_name = vc WITH private, noconstant(concat("ags_immun_backout_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_IMMUN_BACKOUT"
 ...
 IF (the_type="P"
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    WHERE (a.person_id=person_rec->qual[d.seq].person_id)
     AND a.event_id > 0)
   HEAD REPORT
    sknt = 0, stat = alterlist(immun_di->qual,10)
   DETAIL
    sknt = (sknt+ 1)
    IF (mod(sknt,10)=1
     AND sknt != 1)
     stat = alterlist(immun_di->qual,(sknt+ 9))
    ENDIF
    immun_di->qual[sknt].item_id = a.event_id
   FOOT REPORT
    immun_di->qual_knt = sknt, stat = alterlist(immun_di->qual,sknt)
   WITH nocounter
  ;end select
  SET ierrcode = error(serrmsg,1)
  IF (ierrcode > 0)
   ...
   GO TO exit_script
  ENDIF
  IF ((immun_di->qual_knt > 0))
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   DELETE  FROM ce_med_result c,
     (dummyt d  WITH seq = value(immun_di->qual_knt))
    SET c.seq = 1
    PLAN (d
     WHERE d.seq > 0
      AND (immun_di->qual[d.seq].item_id > 0))
     JOIN (c
     WHERE (c.event_id=immun_di->qual[d.seq].item_id))
  ...
  IF (ierrcode > 0)
   ...
   SET log->qual[log->qual_knt].smsgtype = "ERROR"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_IMMUN_DATA >> ",trim(serrmsg))
   GO TO exit_script
  ENDIF
 ELSEIF (the_type="J"
  AND the_job_id > 0.0)
 ...
 ELSEIF (the_type="I")
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    JOIN (a
    WHERE (a.ags_immun_data_id=item_rec->qual[d.seq].data_id)
     AND a.event_id > 0)
   HEAD REPORT
    sknt = 0, stat = alterlist(immun_di->qual,10)
   DETAIL
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_IMMUN_BACKOUT"
 IF (define_logging_sub=true)
  CALL echo("***")
 ...
 ENDIF
 CALL echo("***")
 CALL echo("***   END :: AGS_IMMUN_BACKOUT")
 CALL echo("***")
