CREATE PROGRAM cv_getorganisationname:dba
 IF (validate(stat)=0)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.cdf_meaning="FACILITY"
    AND cv.code_set=220
    AND (cv.display=request->org_shortname)
    AND cv.active_ind=1)
   JOIN (l
   ...
   WHERE o.organization_id=l.organization_id
    AND o.active_ind=1)
  DETAIL
   reply->organisationname = o.org_name
  WITH nocounter
 ;end select
 IF (curqual > 0)
 ...
 ENDIF
 CALL cv_log_msg_post("MOD 001 04/10/18 JT023123")
END GO
