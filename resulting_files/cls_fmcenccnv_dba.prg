CREATE PROGRAM cls_fmcenccnv:dba
 PROMPT
 ...
 DECLARE bwhmrn_cd = f8
 SET bwhmrn_cd = 0.0
 SET bwhmrn_cd = uar_get_code_by("DISPLAYKEY",263,"BWHMRN")
 DECLARE ssn_cd = f8
 SET ssn_cd = 0.0
 ...
 SELECT INTO value( $OUTDEV)
  ...
  bmc_mrn = substring(1,10,pa2.alias), fmc_mrn = substring(1,10,pa3.alias), mlh_mrn = substring(1,10,
   pa4.alias),
  bwh_mrn = substring(1,10,pa6.alias), ssn = substring(1,12,pa5.alias), acctnum = substring(1,15,ea
   .alias),
  attdocname = substring(1,30,p1.name_full_formatted), ftaname = substring(1,30,epr.ft_prsnl_name)
  FROM encounter e,
   encntr_alias ea,
   person p,
   address a,
   ...
   person_alias pa6
  PLAN (e
   WHERE e.encntr_id > 12555032
    AND e.active_ind=1
    AND e.med_service_cd=703444
   ...
   JOIN (d8)
   JOIN (pa6
   WHERE e.person_id=pa6.person_id
    AND pa6.alias_pool_cd=bwhmrn_cd
    AND pa6.active_ind=1
    AND pa6.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
    AND pa6.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3))
  HEAD REPORT
   IF (counter1=0)
    counter1 = 1
   ENDIF
  ...
  DETAIL
   ...
   IF (loop1=counter1)
    ...
    col 626, bwh_mrn, col 639,
    ssn, col 652, acctnum,
    col 668, attdocname, col 700,
    ftaname, row + 1, loop1 = 0.0
   ENDIF
