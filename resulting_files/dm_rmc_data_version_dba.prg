CREATE PROGRAM dm_rmc_data_version:dba
 ...
 SUBROUTINE daf_is_not_blank(dinb_str)
  ...
  RETURN(false)
 END ;Subroutine
 DECLARE vers_active_ind = i2
 DECLARE vers_table_name = vc
 DECLARE vers_primary_key = vc
 DECLARE vers_val = f8
 DECLARE vers_active_num = i2
 DECLARE vers_from_id = f8
 DECLARE vers_loop = i4
 DECLARE vers_col_cnt = i4
 DECLARE vers_dt_tm_loop = i4
 DECLARE vers_cur_date = f8
 DECLARE vers_alg = vc
 DECLARE vers_translated = i2
 DECLARE vers_hist_name = vc
 DECLARE vers_rs_name = vc
 DECLARE vers_beg_dt = f8
 DECLARE vers_end_dt = f8
 DECLARE vers_col_name = vc
 DECLARE vers_rs_string = vc
 DECLARE vers_new_val = f8
 DECLARE vers_source_table = vc
 DECLARE vers_ins_sel_ind = i2
 DECLARE ver_imt_check = i2
 DECLARE cust_ins_upd_ind = vc
 DECLARE vers_query_ret = f8
 DECLARE vers_updt_id_ind = i2
 DECLARE vers_updt_task_ind = i2
 DECLARE vers_updt_dt_tm_ind = i2
 DECLARE vers_parser_cnt = i4
 DECLARE version_nbr_col = vc
 DECLARE vers_nbr_pos = i4
 DECLARE vers_r_query_ret = f8
 DECLARE vers_seq_number = i4
 DECLARE vers_oct_ret = i4
 DECLARE vers_flg_val = i4
 DECLARE vers_parent_drdm_row = i2
 DECLARE vers_ptam_match_result = f8 WITH protect, noconstant(0.0)
 DECLARE vers_ptam_match_str = vc WITH protect, noconstant(" "), notrim
 DECLARE vers_tgt_active_ind = i2 WITH protect, noconstant(0)
 DECLARE vers_hist_loc = i4 WITH protect, noconstant(0)
 DECLARE drdv_hist_idx = i4 WITH protect, noconstant(0)
 DECLARE drdv_grouper_cnt = i4 WITH protect, noconstant(0)
 DECLARE cur_pk_val = f8 WITH protect, noconstant(0.0)
 DECLARE his_pk_val = f8 WITH protect, noconstant(0.0)
 DECLARE vers_qual = i4 WITH protect, noconstant(0)
 DECLARE drdv_pk_val = f8 WITH protect, noconstant(0.0)
 DECLARE drdv_prev_pk_val = f8 WITH protect, noconstant(0.0)
 DECLARE drdv_orig_row = i4 WITH protect, noconstant(0)
 DECLARE drdv_source_val_vc = vc WITH protect, noconstant("")
 DECLARE drdv_target_val_vc = vc WITH protect, noconstant("")
 DECLARE vmu_ret_ind = i2 WITH protect, noconstant(0)
 DECLARE vers_prev_ctxt = vc WITH protect, noconstant("")
 DECLARE drdv_col_idx = i4 WITH protect, noconstant(0)
 DECLARE drdv_circ_tab_idx = i4 WITH protect, noconstant(0)
 DECLARE drdv_circ_col_idx = i4 WITH protect, noconstant(0)
 DECLARE drdv_circ_temp_cnt = i4 WITH protect, noconstant(0)
 DECLARE drdv_circ_pe_name = vc WITH protect, noconstant("")
 DECLARE drdv_circ_tab_name = vc WITH protect, noconstant("")
 DECLARE drdv_dual_ind = i2 WITH protect, noconstant(0)
 DECLARE vers_self_col_ind = i2 WITH protect, noconstant(0)
 DECLARE vers_to_xlat_ret = vc WITH protect, noconstant("")
 DECLARE vers_r_context(vrc_type=vc) = f8
 DECLARE vers_mass_update(vers_type=vc) = i2
 DECLARE fix_child_tabs(fct_type=vc) = i2
 DECLARE xlat_self_ref_col(xsrc_pk_from=f8,xsrc_pk_to=f8,xsrc_tab_pos=i4) = i2
 DECLARE vers_get_match_str(null) = vc
 IF ((global_mover_rec->ptam_ind=1))
  FREE RECORD drdv_drpb_request
  RECORD drdv_drpb_request(
    1 table_name = vc
    1 table_suffix = vc
    1 match_str_tspace_cnt = i4
    1 src_match_result = f8
  ...
  FREE RECORD drdv_drpb_reply
  RECORD drdv_drpb_reply(
    1 nomv_type = vc
    1 blocking_log_id = f8
    1 error_ind = i2
    1 reason_text = vc
  )
  SET drdv_drpb_request->table_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
  SET drdv_drpb_request->table_suffix = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix
  SET drdv_drpb_request->src_env_id = dm2_ref_data_doc->env_source_id
  SET drdv_drpb_request->trg_env_id = dm2_ref_data_doc->env_target_id
  SET drdv_drpb_request->src_match_result = drdm_chg->log[drdm_log_loop].ptam_match_result
  SET drdv_drpb_request->src_match_str = drdm_chg->log[drdm_log_loop].ptam_match_result_str
  SET drdv_drpb_request->match_str_tspace_cnt = drdm_chg->log[drdm_log_loop].ptam_str_tspace_cnt
 ENDIF
 FREE RECORD drdv_new_seq
 RECORD drdv_new_seq(
   1 cnt = i4
   1 qual[*]
     2 new_val = f8
 )
 SET vers_cur_date = cnvtdatetime(curdate,curtime3)
 SET vers_table_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET vers_alg = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].version_type
 IF ( NOT (vers_alg IN ("ALG2", "ALG5")))
  SET vers_active_ind = cnvtint(get_value(vers_table_name,"ACTIVE_IND","FROM"))
 ENDIF
 SET ui_query_rec->usage = "VERSION"
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_versions
 ENDIF
 FREE RANGE v
 CALL parser(concat("range of v is ",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name," go"),1)
 SET vers_task_ind = evaluate(validate(v.updt_task,- (999999)),- (999999),0,1)
 SET vers_dt_tm_ind = evaluate(validate(v.updt_dt_tm,- (999999.0)),- (999999.0),0,1)
 SET vers_id_ind = evaluate(validate(v.updt_id,- (999999.0)),- (999999.0),0,1)
 FREE RANGE v
 IF (vers_alg != "ALG7")
  IF (check_ui_exist(vers_table_name)=0)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
    ": Does not have any unique identifiers set.")
   GO TO exit_versions
  ENDIF
 ENDIF
 IF (vers_alg IN ("ALG7", "ALG5"))
  FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].exception_flg=12))
     SET drdv_grouper_cnt = (drdv_grouper_cnt+ 1)
    ENDIF
  ENDFOR
  IF (drdv_grouper_cnt=0)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
    ": Does not have any grouper columns set.")
   GO TO exit_versions
  ENDIF
  SET drdv_grouper_cnt = 0
 ENDIF
 SET vers_primary_key = get_primary_key(vers_table_name)
 IF (daf_is_blank(vers_primary_key)
  AND vers_alg != "ALG4")
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
   ": Could not resolve a primary key column.")
  GO TO exit_versions
 ENDIF
 SET vers_source_table = dm2_get_rdds_tname(vers_table_name)
 IF (vers_alg != "ALG4")
  SET drdv_source_val_vc = get_value(vers_table_name,vers_primary_key,"FROM")
  SET vers_prev_ctxt = select_merge_translate_rec->from_opt_ctxt
  SET select_merge_translate_rec->from_opt_ctxt = ""
  SET drdv_target_val_vc = select_merge_translate(drdv_source_val_vc,vers_table_name)
  SET select_merge_translate_rec->from_opt_ctxt = vers_prev_ctxt
  IF (drdv_target_val_vc="NOMV21")
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV21"
   SET drdm_mini_loop_status = "NOMV21"
   GO TO exit_versions
  ENDIF
 ENDIF
 FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].exception_flg=9)
    AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].version_nbr_child_ind=0))
    SET version_nbr_col = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name
    SET vers_nbr_pos = vers_loop
   ENDIF
 ENDFOR
 IF (daf_is_blank(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].beg_col_name)
  AND vers_alg IN ("ALG2", "ALG5", "ALG6"))
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg =
  "Couldn't find valid name for the BEGIN_EFFECTIVE_DT_TM column."
  GO TO exit_versions
 ENDIF
 IF (daf_is_blank(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].end_col_name)
  AND vers_alg IN ("ALG2", "ALG5", "ALG6"))
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "Couldn't find valid name for the END_EFFECTIVE_DT_TM column."
  GO TO exit_versions
 ENDIF
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind=1))
  CALL parser(concat("set vers_beg_dt = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
    "->from_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].beg_col_name," go"),1)
  CALL parser(concat("set vers_end_dt = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
    "->from_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].end_col_name," go"),1)
 ENDIF
 CALL echo("")
 CALL echo("")
 CALL echo("Performing versioning logic")
 CALL echo("")
 CALL echo("")
 IF (vers_alg != "ALG4")
  SET temp_col_cnt = get_col_pos(vers_table_name,vers_primary_key)
 ELSE
  SET temp_col_cnt = 1
 ENDIF
 CASE (vers_alg)
  ...
  OF "ALG4":
  OF "ALG3D":
   IF (vers_active_ind=0)
    IF (vers_alg != "ALG4")
     SET temp_col_cnt = get_col_pos(vers_table_name,vers_primary_key)
    ELSE
     SET temp_col_cnt = 1
    ENDIF
    SET vers_active_num = get_col_pos(vers_table_name,"ACTIVE_IND")
    SET vers_val = 0
    SET ui_query_rec->table_name = vers_source_table
    SET ui_query_rec->dom = "FROM"
    SET stat = alterlist(ui_query_rec->qual,1)
    SET ui_query_rec->qual[1].qtype = "ACTIVE"
    SET vers_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
    IF ((vers_query_ret=- (2)))
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV44"
    ELSEIF ((vers_query_ret=- (1)))
     SET vers_translated = is_translated(vers_table_name,"ALL")
    ELSEIF ((vers_query_ret=- (3)))
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind=1)
      AND vers_beg_dt <= vers_cur_date
      AND vers_end_dt >= vers_cur_date)
      SET vers_translated = is_translated(vers_table_name,"UNIQUE")
      IF (vers_translated=1)
       SET vers_translated = 0
       SET ui_query_rec->table_name = vers_table_name
       SET ui_query_rec->dom = "TO"
       SET stat = alterlist(ui_query_rec->qual,1)
       SET ui_query_rec->qual[1].qtype = "ACTIVE"
       IF (daf_is_not_blank(version_nbr_col))
        SET stat = alterlist(ui_query_rec->qual[1].other_pk_col,1)
        SET ui_query_rec->qual[1].other_pk_col[1].col_name = version_nbr_col
       ENDIF
       SET vers_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
       SET vers_seq_number = cnvtint(ui_query_eval_rec->qual[1].additional_attr)
       IF (vers_query_ret > 0)
        SET vers_translated = 0
        SET current_merges = (current_merges+ 1)
        ...
        SET vers_query_ret = vers_r_context("EFFECTIVE")
        SET vmu_ret_ind = vers_mass_update("END-EFFECTIVE")
        IF (vmu_ret_ind=1)
         GO TO exit_versions
        ENDIF
       ELSEIF ((vers_query_ret=- (2)))
        SET vers_r_query_ret = vers_r_context("ACTIVE")
        SET vmu_ret_ind = vers_mass_update("INACTIVATE")
        IF (vmu_ret_ind=1)
         GO TO exit_versions
        ENDIF
        IF (nodelete_ind=0)
         SET vers_query_ret = 1
         SET vers_translated = 0
         SET current_merges = (current_merges+ 1)
         SET child_merge_audit->num[current_merges].action = "UIEXIST"
         SET child_merge_audit->num[current_merges].text = substring(1,1000,short_string)
       ...
       IF (nodelete_ind=1)
        ...
        GO TO exit_versions
       ENDIF
       CALL parser(concat("set rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].beg_col_name," = vers_cur_date go"),1)
       IF (daf_is_not_blank(version_nbr_col))
        CALL parser(concat("set rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
          version_nbr_col," = vers_seq_number+1 go"),1)
        SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_nbr_pos].translated = 1
       ENDIF
       IF (vers_alg != "ALG4")
        IF (is_translated(vers_table_name,vers_primary_key)=0)
         SET vers_val = get_seq(vers_table_name,vers_primary_key)
         IF (vers_val > 0)
          CALL put_value(vers_table_name,vers_primary_key,build(vers_val))
          SET current_merges = (current_merges+ 1)
          SET child_merge_audit->num[current_merges].action = "NEWSEQ"
          SET child_merge_audit->num[current_merges].text = concat(vers_table_name,"    ",
           vers_primary_key)
         ELSE
          GO TO exit_versions
         ENDIF
        ENDIF
        SET vers_from_id = cnvtreal(get_value(vers_table_name,vers_primary_key,"FROM"))
        SET vers_self_col_ind = xlat_self_ref_col(vers_from_id,vers_val,temp_tbl_cnt)
        IF (vers_self_col_ind=1)
         SET dm2_ref_data_reply->error_ind = 1
         SET dm2_ref_data_reply->error_msg = dm_err->emsg
         GO TO exit_versions
        ENDIF
        SET vers_prev_ctxt = select_merge_translate_rec->from_opt_ctxt
        SET select_merge_translate_rec->from_opt_ctxt = ""
        SET cust_ins_upd_ind = select_merge_translate(build(vers_from_id),vers_table_name)
        SET select_merge_translate_rec->from_opt_ctxt = vers_prev_ctxt
        IF (cust_ins_upd_ind="NOMV21")
         SET insert_update_reason = "The primary key value has been deleted from target."
         SET no_insert_update = 1
         SET dm2_ref_data_reply->error_ind = 1
         SET dm2_ref_data_reply->error_msg = "NOMV21"
         SET drdm_mini_loop_status = "NOMV21"
         GO TO exit_versions
        ELSEIF ( NOT (isnumeric(cust_ins_upd_ind)))
         SET ver_imt_check = insert_merge_translate(vers_from_id,vers_val,vers_table_name)
         IF (ver_imt_check=1)
          GO TO exit_versions
         ENDIF
        ENDIF
       ...
       SET vers_translated = is_translated(vers_table_name,"ALL")
       IF (vers_translated=1)
        CALL insert_update_row(temp_tbl_cnt,perm_col_cnt)
       ENDIF
      ELSE
       SET vers_translated = is_translated(vers_table_name,"ALL")
       IF ((dm2_ref_data_reply->error_msg="OLDVER")
        AND vers_alg != "ALG4")
        SET vers_from_id = cnvtreal(get_value(vers_table_name,vers_primary_key,"FROM"))
        SET vers_oct_ret = orphan_child_tab(vers_table_name,"OLDVER",vers_primary_key,vers_from_id)
       ENDIF
      ENDIF
     ELSE
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind=0))
       SET vers_query_ret = - (3)
      ELSE
       SET ui_query_rec->table_name = vers_source_table
       SET ui_query_rec->dom = "FROM"
       SET stat = alterlist(ui_query_rec->qual,1)
       SET ui_query_rec->qual[1].qtype = "EFFECTIVE"
       SET vers_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
      ENDIF
      IF ((vers_query_ret=- (3)))
       SET vers_translated = is_translated(vers_table_name,"UNIQUE")
       IF (vers_translated=1)
        ...
        IF (vers_query_ret > 0)
         ...
         SET child_merge_audit->num[current_merges].text = substring(1,1000,short_string)
         SET vers_r_query_ret = vers_r_context("ACTIVE")
         CALL drmm_get_ptam_match_query(drdm_chg->log[drdm_log_loop].log_id,drdm_chg->log[
          drdm_log_loop].table_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,dm2_ref_data_doc
          ->post_link_name)
         SET vers_ptam_match_result = drmm_get_ptam_match_result(0.0,dm2_ref_data_doc->env_source_id,
          dm2_ref_data_doc->env_target_id,dm2_ref_data_doc->post_link_name,0,
          "FROM")
         IF ((global_mover_rec->ptam_ind=1))
          SET drdv_drpb_request->trg_match_str = vers_get_match_str(null)
          IF (check_error(dm_err->eproc)=1)
           SET dm2_ref_data_reply->error_msg = dm_err->emsg
           SET dm2_ref_data_reply->error_ind = 1
           GO TO exit_versions
          ENDIF
          SET drdv_drpb_request->trg_match_result = vers_ptam_match_result
          EXECUTE dm_rmc_ptam_block  WITH replace("REQUEST","DRDV_DRPB_REQUEST"), replace("REPLY",
           "DRDV_DRPB_REPLY")
          IF (check_error(dm_err->eproc)=1)
           SET dm2_ref_data_reply->error_msg = dm_err->emsg
           SET dm2_ref_data_reply->error_ind = 1
           GO TO exit_versions
          ENDIF
          IF ((drdv_drpb_reply->blocking_log_id > 0))
           SET drdm_chg->log[drdm_log_loop].blocking_log_id = drdv_drpb_reply->blocking_log_id
           SET vers_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
           SET drdm_chg->log[vers_parent_drdm_row].chg_log_reason_txt = drdv_drpb_reply->reason_text
           SET dm2_ref_data_reply->error_ind = 1
           SET drdm_mini_loop_status = drdv_drpb_reply->nomv_type
           SET drdm_no_trans_ind = 1
           SET dm2_ref_data_reply->error_msg = drdv_drpb_reply->nomv_type
           GO TO exit_versions
          ENDIF
         ENDIF
         SET vmu_ret_ind = vers_mass_update("INACTIVATE")
         ...
         ENDIF
         IF (curqual=0)
          CALL parser(concat("set rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
            "->to_values.ACTIVE_IND"," = 0 go"),1)
          IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind=1))
           CALL parser(concat("set rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
             "->to_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].end_col_name," = sysdate go"),1)
          ENDIF
          IF (vers_alg != "ALG4")
           CALL parser(concat("set rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
             "->to_values.",vers_primary_key," = ",
             trim(build(vers_query_ret))," go"),1)
          ENDIF
          SET vers_from_id = cnvtreal(get_value(vers_table_name,vers_primary_key,"FROM"))
          SET vers_self_col_ind = xlat_self_ref_col(vers_from_id,vers_query_ret,temp_tbl_cnt)
          IF (vers_self_col_ind=1)
           SET dm2_ref_data_reply->error_ind = 1
           SET dm2_ref_data_reply->error_msg = dm_err->emsg
           GO TO exit_versions
          ENDIF
          CALL insert_update_row(temp_tbl_cnt,size(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual,
            5))
         ENDIF
         SET vers_from_id = cnvtreal(get_value(vers_table_name,vers_primary_key,"FROM"))
         SET vers_prev_ctxt = select_merge_translate_rec->from_opt_ctxt
         ...
         IF (cust_ins_upd_ind="NOMV21")
          ...
          GO TO exit_versions
         ELSEIF ( NOT (isnumeric(cust_ins_upd_ind)))
          SET ver_imt_check = insert_merge_translate(vers_from_id,vers_query_ret,vers_table_name)
          IF (ver_imt_check=1)
           SET nodelete_ind = 0
           CALL insert_update_row(temp_tbl_cnt,perm_col_cnt)
           COMMIT
         ...
         ENDIF
        ELSEIF ((vers_query_ret=- (2)))
         SET vers_query_ret = vers_r_context("ACTIVE")
         CALL drmm_get_ptam_match_query(drdm_chg->log[drdm_log_loop].log_id,drdm_chg->log[
          drdm_log_loop].table_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,dm2_ref_data_doc
          ->post_link_name)
         SET vers_ptam_match_result = drmm_get_ptam_match_result(0.0,dm2_ref_data_doc->env_source_id,
         ...
         IF (curqual=0)
          ...
          IF (vers_alg != "ALG4")
           IF (is_translated(vers_table_name,vers_primary_key)=0)
            SET vers_query_ret = get_seq(vers_table_name,vers_primary_key)
            CALL parser(concat("set rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
              "->to_values.",vers_primary_key," = ",
              trim(build(vers_query_ret))," go"),1)
           ENDIF
        ...
        ELSEIF ((vers_query_ret=- (1)))
         SET dm2_ref_data_reply->error_ind = 1
         SET dm2_ref_data_reply->error_msg =
         "There was a syntax error when querying for active rows in target"
        ELSE
         SET vers_r_query_ret = vers_r_context("ACTIVE")
         IF (vers_r_query_ret > 0)
          ROLLBACK
          SET vmu_ret_ind = vers_mass_update("INACTIVATE")
          IF (((check_error(dm_err->eproc)=1) OR (vmu_ret_ind=1)) )
           ROLLBACK
           SET dm2_ref_data_reply->error_msg = dm_err->emsg
           SET dm2_ref_data_reply->error_ind = 1
           GO TO exit_versions
         ...
         ENDIF
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind=1))
          SET vers_r_query_ret = vers_r_context("EFFECTIVE")
          IF (vers_r_query_ret > 0)
           ROLLBACK
           SET vmu_ret_ind = vers_mass_update("END-EFFECTIVE")
           IF (((check_error(dm_err->eproc)=1) OR (vmu_ret_ind=1)) )
         ...
         ENDIF
         SET vers_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
         SET drdm_chg->log[vers_parent_drdm_row].chg_log_reason_txt = concat(
          "The source has no active or ",
          "effective rows that qualify with the pk_where, and target has no active or effective row to update"
          )
         SET dm2_ref_data_reply->error_ind = 1
         SET dm2_ref_data_reply->error_msg = "NOMV61"
         SET drdm_mini_loop_status = "NOMV61"
         SET drdm_no_trans_ind = 1
         GO TO exit_versions
        ENDIF
        IF (nodelete_ind=1)
      ...
      ELSEIF ((vers_query_ret=- (1)))
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg =
       "There was a syntax error when querying for effective rows in source"
      ELSE
       SET dm2_ref_data_reply->error_ind = 1
   ...
   ELSE
    SET vers_translated = 0
    IF (vers_alg != "ALG4")
     SET vers_translated = is_translated(vers_table_name,vers_primary_key)
    ENDIF
    IF (vers_translated=0)
     SET vers_translated = is_translated(vers_table_name,"UNIQUE")
     IF (vers_translated=1)
      SET vers_translated = 0
      SET ui_query_rec->table_name = vers_table_name
      ...
      IF (vers_query_ret > 0)
       SET vers_r_query_ret = vers_r_context("EFFECTIVE")
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind != 0))
        SET vmu_ret_ind = vers_mass_update("BOTH")
       ELSE
        SET vmu_ret_ind = vers_mass_update("INACTIVATE")
       ENDIF
       IF (((nodelete_ind=1) OR (vmu_ret_ind=1)) )
        SET dm2_ref_data_reply->error_ind = 1
        SET dm2_ref_data_reply->error_msg = dm_err->emsg
        GO TO exit_versions
       ENDIF
      ...
      ELSE
       ...
       ELSEIF ((vers_query_ret=- (1)))
        SET dm2_ref_data_reply->error_ind = 1
        SET dm2_ref_data_reply->error_msg =
        "There was a syntax error when querying for effective rows in target"
        GO TO exit_versions
       ENDIF
      ENDIF
     ELSE
      CALL is_translated(vers_table_name,"ALL")
      GO TO exit_versions
     ENDIF
     IF (vers_alg != "ALG4")
      SET vers_val = get_seq(vers_table_name,vers_primary_key)
     ...
     IF (vers_translated=1)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind=1))
       CALL parser(concat("set rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].beg_col_name," = vers_cur_date go"),1)
      ENDIF
      CALL put_value(vers_table_name,"ACTIVE_IND","1")
      CALL insert_update_row(temp_tbl_cnt,perm_col_cnt)
     ENDIF
    ELSE
     IF (daf_is_not_blank(version_nbr_col))
      SELECT INTO "NL:"
       parser(build("vt.",version_nbr_col))
       FROM (parser(vers_table_name) vt)
       WHERE ((parser(build("vt.",vers_primary_key," = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
         suffix,"->to_values.",
         vers_primary_key," and vt.active_ind = 1"))) UNION (
       (SELECT INTO "NL:"
        parser(build("vtr.",version_nbr_col))
        FROM (parser(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].r_table_name) vtr)
        WHERE parser(build("vtr.",vers_primary_key," = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
          suffix,"->to_values.",
          vers_primary_key," and vtr.active_ind = 1")))))
       DETAIL
        vers_flg_val = parser(build("vt.",version_nbr_col))
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       SET dm2_ref_data_reply->error_msg = dm_err->emsg
      ...
      IF (vers_flg_val=0)
       SET vers_flg_val = 1
      ENDIF
      CALL put_value(vers_table_name,version_nbr_col,cnvtstring(vers_flg_val))
     ENDIF
     SET vers_from_id = cnvtreal(get_value(vers_table_name,vers_primary_key,"FROM"))
     SET vers_val = cnvtreal(get_value(vers_table_name,vers_primary_key,"TO"))
     SET vers_self_col_ind = xlat_self_ref_col(vers_from_id,vers_val,temp_tbl_cnt)
     IF (vers_self_col_ind=1)
      SET dm2_ref_data_reply->error_ind = 1
      SET dm2_ref_data_reply->error_msg = dm_err->emsg
     ...
     IF (vers_translated=1)
      SELECT INTO "nl:"
       FROM (parser(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name) vt)
       WHERE parser(build("vt.",vers_primary_key," = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
         suffix,"->to_values.",
         vers_primary_key))
       DETAIL
        vers_tgt_active_ind = vt.active_ind
       WITH nocounter
      ;end select
      IF (vers_tgt_active_ind=0
       AND curqual=1)
       SET vers_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
       CALL parser(concat(
         "set drdm_chg->log[vers_parent_drdm_row].chg_log_reason_txt = concat('Dual Maintenance on versioned table ",
         "detected, ',vers_primary_key,' of ',trim(cnvtstring(RS_",dm2_ref_data_doc->tbl_qual[
         temp_tbl_cnt].suffix,"->from_values.",vers_primary_key,
         ",20)),' in source maps to ',trim(cnvtstring(RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
         suffix,"->to_values.",vers_primary_key,",20)),' in target which is inactive') go"),1)
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = "NOMV76"
       SET drdm_mini_loop_status = "NOMV76"
       SET drdm_no_trans_ind = 1
       GO TO exit_versions
      ENDIF
      SET vers_r_query_ret = vers_r_context("ACTIVE")
  ...
  OF "ALG2":
  OF "ALG5":
   SET vers_hist_name = "NO_HIST"
   FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].root_entity_name=
     vers_table_name)
      AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].root_entity_attr=
     vers_primary_key)
      AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name !=
     vers_primary_key))
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].exception_flg=11))
       SET vers_hist_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name
      ENDIF
     ENDIF
   ENDFOR
   IF (vers_hist_name="NO_HIST")
    FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].root_entity_name=
      vers_table_name)
       AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].root_entity_attr=
      vers_primary_key)
       AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name !=
      vers_primary_key))
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name="PREV*"))
        SET vers_hist_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name
       ENDIF
      ENDIF
    ENDFOR
   ...
   IF (vers_hist_name="NO_HIST")
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
     ": Could not resolve what the history primary_key column is named.")
    GO TO exit_versions
   ENDIF
   CALL parser(concat("set vers_beg_dt = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
     "->from_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].beg_col_name," go"),1)
   ...
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_versions
   ENDIF
   IF (vers_cur_date >= vers_beg_dt
    AND vers_cur_date <= vers_end_dt)
    CALL parser(concat("set drdv_pk_val = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
      "->from_values.",vers_primary_key," go"),1)
    CALL parser(concat("set drdv_prev_pk_val = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
      "->from_values.",vers_hist_name," go"),1)
    IF (drdv_pk_val != drdv_prev_pk_val)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV84"
     SET drdv_orig_row = find_original_log_row(drdm_chg,drdm_log_loop)
     SET drdm_chg->log[drdv_orig_row].chg_log_reason_txt = concat(
      "The row does not meet the versioning requirements to ",
      "correctly merge the data.  Information for this row will need to be rebuilt in the Target domain."
      )
     GO TO exit_versions
    ENDIF
    SET temp_col_cnt = get_col_pos(vers_table_name,vers_primary_key)
    ...
    IF (vers_translated=0)
     ...
     IF (vers_translated=1)
      ...
      IF ((vers_query_ret=- (2)))
       SET vers_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
       SET drdm_chg->log[vers_parent_drdm_row].chg_log_reason_txt =
       "There were multiple effective rows found in target, when querying on the MUI columns."
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = "NOMV06"
       GO TO exit_versions
      ELSEIF (vers_query_ret > 0)
       SET vers_r_query_ret = vers_r_context("EFFECTIVE")
       CALL put_value(vers_table_name,vers_hist_name,build(vers_query_ret))
       IF (vers_alg="ALG5"
        AND (drdm_chg->log[drdm_log_loop].md_delete_ind=1))
        CALL parser(concat("delete from ",cutover_tab_name(vers_table_name,dm2_ref_data_doc->
           tbl_qual[temp_tbl_cnt].suffix)," dc where "),0)
        CALL parser(concat("dc.rdds_source_env_id = ",build(dm2_ref_data_doc->env_source_id)),0)
        CALL parser("and dc.rdds_status_flag < 9000 ",0)
        CALL parser(" and ",0)
        SET vers_hist_loc = locateval(drdv_hist_idx,1,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
         col_cnt,vers_hist_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[drdv_hist_idx].
         column_name)
        CALL parser(concat(" dc.",vers_hist_name," in(select distinct ",vers_hist_name," from ",
          cutover_tab_name(vers_table_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix),
          " dc2 where "),0)
        SET drdv_grouper_cnt = 0
        FOR (i = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
          IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].exception_flg=12))
           SET drdv_grouper_cnt = (drdv_grouper_cnt+ 1)
           IF (drdv_grouper_cnt > 1)
            CALL parser(" and ",0)
           ENDIF
           IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].check_null=1))
            CALL parser(concat(" dc2.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].
              column_name," is NULL"),0)
           ELSE
            IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].trailing_space_cnt > 0)
             AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].data_type="VC"))
             CALL parser(concat(" dc2.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].
               column_name,"=notrim(concat(rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
               "->to_values.",
               dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].column_name,", fillstring(value(",
               trim(cnvtstring(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].
                 trailing_space_cnt)),'), " ")))'),0)
            ELSE
             CALL parser(concat(" dc2.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].
               column_name,"=rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
               dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].column_name),0)
            ENDIF
           ENDIF
          ENDIF
        ENDFOR
        ...
        IF (check_error(dm_err->eproc)=1)
         ...
         SET nodelete_ind = 1
        ENDIF
        IF (((findstring("ORA-00001:",dm_err->emsg) > 0) OR (((findstring("ORA-02049:",dm_err->emsg)
         > 0) OR (findstring("ORA-4020:",dm_err->emsg) > 0)) )) )
         SET drmm_ddl_rollback_ind = 1
         SET drdm_log_loop = redirect_to_start_row(drdm_chg,drdm_log_loop,drdm_next_row_pos,
          drdm_max_rs_size)
        ENDIF
       ENDIF
       CALL parser(concat("delete from ",cutover_tab_name(vers_table_name,dm2_ref_data_doc->tbl_qual[
          temp_tbl_cnt].suffix)," dc where "),0)
       CALL parser(concat("dc.rdds_source_env_id = ",build(dm2_ref_data_doc->env_source_id)),0)
       CALL parser("and dc.rdds_status_flag < 9000 ",0)
       SET vers_hist_loc = locateval(drdv_hist_idx,1,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt,
        vers_hist_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[drdv_hist_idx].column_name)
       CALL parser(concat(" and dc.",vers_hist_name,"=rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
         suffix,"->to_values.",
         vers_hist_name),0)
       CALL parser(" with nocounter go",1)
       IF (check_error(dm_err->eproc)=1)
       ...
       IF (((findstring("ORA-00001:",dm_err->emsg) > 0) OR (((findstring("ORA-02049:",dm_err->emsg)
        ...
        SET drdm_log_loop = redirect_to_start_row(drdm_chg,drdm_log_loop,drdm_next_row_pos,
         drdm_max_rs_size)
       ENDIF
       SET vers_ins_sel_ind = 1
       SET vers_val = vers_query_ret
       SET current_merges = (current_merges+ 1)
       SET child_merge_audit->num[current_merges].action = "UIEXIST"
       SET child_merge_audit->num[current_merges].text = substring(1,1000,short_string)
       SET vers_new_val = get_seq(vers_table_name,vers_primary_key)
       SET new_seq_ind = 0
       IF (vers_new_val > 0)
        SET current_merges = (current_merges+ 1)
        SET child_merge_audit->num[current_merges].action = "NEWSEQ"
       ...
       ELSE
        GO TO exit_versions
       ENDIF
       CALL drmm_get_ptam_match_query(drdm_chg->log[drdm_log_loop].log_id,drdm_chg->log[drdm_log_loop
        ].table_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,dm2_ref_data_doc->post_link_name
        )
       SET vers_ptam_match_result = drmm_get_ptam_match_result(0.0,dm2_ref_data_doc->env_source_id,
        dm2_ref_data_doc->env_target_id,dm2_ref_data_doc->post_link_name,0,
        "FROM")
       SET vers_ptam_match_str = vers_get_match_str(null)
       IF (check_error(dm_err->eproc)=1)
        SET dm2_ref_data_reply->error_msg = dm_err->emsg
        SET dm2_ref_data_reply->error_ind = 1
        GO TO exit_versions
       ENDIF
       IF ((drdm_chg->log[drdm_log_loop].ptam_str_tspace_cnt > 0))
        SET vers_ptam_match_str = notrim(concat(vers_ptam_match_str,notrim(fillstring(value(drdm_chg
             ->log[drdm_log_loop].ptam_str_tspace_cnt)," "))))
       ENDIF
       SET drdm_parser->statement[1].frag = concat("insert into ",cutover_tab_name(vers_table_name,
         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix)," dc (")
       SET drdm_parser_cnt = 2
       FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
         SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" dc.",dm2_ref_data_doc->tbl_qual[
          temp_tbl_cnt].col_qual[vers_loop].column_name)
         SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
         IF ((vers_loop < dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt))
          SET drdm_parser->statement[drdm_parser_cnt].frag = ", "
          SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
         ENDIF
       ENDFOR
       SET drdm_parser->statement[drdm_parser_cnt].frag = concat(
        ", dc.rdds_dt_tm, dc.rdds_log_id,dc.rdds_context_name,dc.rdds_source_env_id,",
        "dc.rdds_status_flag,dc.rdds_delete_ind,dc.rdds_ptam_match_result,dc.rdds_ptam_match_result_str"
        )
       SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
       SET drdm_parser->statement[drdm_parser_cnt].frag = ") (select "
       SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
       FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
        IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name=
        vers_primary_key))
         SET drdm_parser->statement[drdm_parser_cnt].frag = concat(trim(cnvtstring(vers_new_val,20)),
          ".0")
         SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
        ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name=
        dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].end_col_name))
         SET drdm_parser->statement[drdm_parser_cnt].frag = concat("cnvtdatetime(vers_cur_date) ")
         SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
        ELSE
         SET drdv_col_idx = locateval(drdv_col_idx,1,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
          circular_cnt,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name,
          dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[drdv_col_idx].id_col)
         IF (drdv_col_idx > 0)
          IF (daf_is_not_blank(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[drdv_col_idx].
           pe_name_col))
           CALL parser(concat("select into 'NL:' y = evaluate_pe_name('",dm2_ref_data_doc->tbl_qual[
             temp_tbl_cnt].table_name,"','",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[
             drdv_col_idx].id_col,"','",
             dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[drdv_col_idx].pe_name_col,"',l.",
             dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[drdv_col_idx].pe_name_col,") from ",
             vers_table_name,
             " l where ",vers_primary_key," = vers_query_ret ",
             " detail drdv_circ_pe_name = y with nocounter go"),1)
           IF (check_error(dm_err->eproc)=1)
            CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
            SET dm_err->err_ind = 0
            SET nodelete_ind = 1
           ENDIF
           SET drdv_circ_tab_name = "INVALIDTABLE"
           FOR (drdv_circ_col_idx = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circular_cnt)
             IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[drdv_circ_col_idx].id_col=
             dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name)
              AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[drdv_circ_col_idx].
             circ_table_name=drdv_circ_pe_name))
              SET drdv_circ_tab_name = drdv_circ_pe_name
             ENDIF
           ENDFOR
          ELSE
           SET drdv_circ_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[drdv_col_idx].
           circ_table_name
          ENDIF
          IF (drdv_circ_tab_name != "INVALIDTABLE")
           SET drdv_circ_tab_idx = locateval(drdv_circ_tab_idx,1,dm2_ref_data_doc->tbl_cnt,
            drdv_circ_tab_name,dm2_ref_data_doc->tbl_qual[drdv_circ_tab_idx].table_name)
           IF (drdv_circ_tab_idx=0)
            SET drdv_circ_temp_cnt = temp_tbl_cnt
            SET drdv_circ_tab_idx = fill_rs("TABLE",drdv_circ_tab_name)
            SET temp_tbl_cnt = drdv_circ_temp_cnt
            IF ((drdv_circ_tab_idx=- (1)))
             CALL disp_msg("Table can't be created from remote mover",dm_err->logfile,1)
             SET drdm_mini_loop_status = "REFCHG"
             SET dm_err->err_ind = 0
             SET nodelete_ind = 1
            ...
            ENDIF
           ENDIF
           SET drdv_circ_col_idx = locateval(drdv_circ_col_idx,1,dm2_ref_data_doc->tbl_qual[
            drdv_circ_tab_idx].col_cnt,dm2_ref_data_doc->tbl_qual[drdv_circ_tab_idx].root_col_name,
            dm2_ref_data_doc->tbl_qual[drdv_circ_tab_idx].col_qual[drdv_circ_col_idx].column_name)
           IF (drdv_circ_col_idx=0)
            SET dm2_ref_data_reply->error_ind = 1
            SET dm2_ref_data_reply->error_msg = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
             circ_qual[drdv_col_idx].circ_id_col_name," does not exist on table ",dm2_ref_data_doc->
             tbl_qual[drdv_circ_tab_idx].table_name)
            GO TO exit_versions
           ENDIF
           CALL parser(concat("select into 'NL:' y = seq(",dm2_ref_data_doc->tbl_qual[
             drdv_circ_tab_idx].col_qual[drdv_circ_col_idx].sequence_name,
             ",nextval) from dual d detail drdv_new_seq->cnt = drdv_new_seq->cnt + 1"),0)
           CALL parser(concat(" stat = alterlist(drdv_new_seq->qual, drdv_new_seq->cnt) ",
             " drdv_new_seq->qual[drdv_new_seq->cnt].new_val = y with nocounter go"),1)
           IF (check_error(dm_err->eproc)=1)
            SET dm2_ref_data_reply->error_msg = dm_err->emsg
            SET dm2_ref_data_reply->error_ind = 1
            GO TO exit_versions
           ENDIF
           SET drdm_parser->statement[drdm_parser_cnt].frag = trim(cnvtstring(drdv_new_seq->qual[
             drdv_new_seq->cnt].new_val,20,1))
           SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
          ELSE
           SET drdm_parser->statement[drdm_parser_cnt].frag = concat("d.",dm2_ref_data_doc->tbl_qual[
            temp_tbl_cnt].col_qual[vers_loop].column_name)
           SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
          ENDIF
         ELSE
        ...
        ENDIF
       ENDFOR
       SET drdm_parser->statement[drdm_parser_cnt].frag = concat(", cnvtdatetime(curdate,curtime3), ",
        build(drdm_chg->log[drdm_log_loop].log_id),",",build('"',drdm_chg->log[drdm_log_loop].
         context_name,'"'),", ",
        build(dm2_ref_data_doc->env_source_id),", 0, 0, vers_ptam_match_result,vers_ptam_match_str")
       SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
       SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" from ",vers_table_name,
        " d where d.",vers_primary_key," = vers_val) with notrim go")
       CALL parse_statements(drdm_parser_cnt)
       IF (((findstring("ORA-00001:",dm_err->emsg) > 0) OR (((findstring("ORA-02049:",dm_err->emsg)
        > 0) OR (findstring("ORA-4020:",dm_err->emsg) > 0)) )) )
        SET drmm_ddl_rollback_ind = 1
      ...
      ELSEIF ((vers_query_ret=- (1)))
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
        ": A query for effective rows in the target had a syntax error.")
       GO TO exit_versions
      ELSE
       SET vers_r_query_ret = vers_r_context("EFFECTIVE")
       SET vers_ins_sel_ind = 0
       SET vers_val = get_seq(vers_table_name,vers_primary_key)
       IF (vers_val > 0)
        SET current_merges = (current_merges+ 1)
        SET child_merge_audit->num[current_merges].action = "NEWSEQ"
       ...
       ELSE
        GO TO exit_versions
       ENDIF
       CALL put_value(vers_table_name,vers_hist_name,build(vers_val))
       IF (vers_alg="ALG5"
        AND (drdm_chg->log[drdm_log_loop].md_delete_ind=1))
        CALL parser(concat("delete from ",cutover_tab_name(vers_table_name,dm2_ref_data_doc->
           tbl_qual[temp_tbl_cnt].suffix)," dc where "),0)
     ...
     ENDIF
    ELSE
     CALL parser(concat("select into 'NL:' from ",vers_table_name," d where"),0)
     CALL parser(concat("d.",vers_primary_key," = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
       suffix,"->to_values.",
       vers_primary_key," with nocounter go"),1)
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET nodelete_ind = 1
      SET no_insert_update = 1
     ...
     ELSE
      ...
      SET vers_translated = is_translated(vers_table_name,"UNIQUE")
      IF (vers_translated > 0)
       SET vers_translated = is_translated(vers_table_name,vers_hist_name)
       IF (vers_translated=0)
        SET vers_translated = is_translated(vers_table_name,"ALL")
        GO TO exit_versions
       ENDIF
       ...
       IF (vers_translated=1)
        ...
        IF (vers_alg="ALG5"
         ...
         FOR (i = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
            ...
            ELSE
             IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].trailing_space_cnt > 0)
              ...
              CALL parser(concat(" dc2.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].
                column_name,"=notrim(concat(rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
                "->to_values.",
                dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].column_name,
                ", fillstring(value(",trim(cnvtstring(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
                  col_qual[i].trailing_space_cnt)),'), " ")))'),0)
             ELSE
              CALL parser(concat(" dc2.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].
                column_name,"=rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
                dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].column_name),0)
           ...
           ENDIF
         ENDFOR
         CALL parser(") with notrim, nocounter go",1)
        ENDIF
        IF (check_error(dm_err->eproc)=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         SET dm_err->err_ind = 0
        ...
        ENDIF
        SET vers_ins_sel_ind = 1
        CALL parser(concat("set vers_val = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
          "->to_values.",vers_primary_key," go"),1)
        SET current_merges = (current_merges+ 1)
        SET child_merge_audit->num[current_merges].action = "UIEXIST"
        SET child_merge_audit->num[current_merges].text = substring(1,1000,short_string)
        SET vers_new_val = get_seq(vers_table_name,vers_primary_key)
        ...
        SET drdm_parser_cnt = 2
        FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
          SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" dc.",dm2_ref_data_doc->
           tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name)
          SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
          IF ((vers_loop < dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt))
           SET drdm_parser->statement[drdm_parser_cnt].frag = ", "
           SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
        ...
        ENDFOR
        SET drdm_parser->statement[drdm_parser_cnt].frag = concat(
         ", dc.rdds_dt_tm, dc.rdds_log_id,dc.rdds_context_name,",
         "dc.rdds_source_env_id, dc.rdds_status_flag, dc.rdds_delete_ind, ",
         "dc.rdds_ptam_match_result,dc.rdds_ptam_match_result_str")
        SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
        SET drdm_parser->statement[drdm_parser_cnt].frag = ") (select "
        SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
        FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
          ...
          IF (drdv_col_idx > 0)
           IF (daf_is_not_blank(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[drdv_col_idx].
            ...
            CALL parser(concat("select into 'NL:' y = evaluate_pe_name('",dm2_ref_data_doc->tbl_qual[
              ...
              dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[drdv_col_idx].pe_name_col,") from ",
              vers_table_name,
              " l where ",vers_primary_key," = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
              "->to_values.",
              vers_primary_key," detail drdv_circ_pe_name = y with nocounter go"),1)
            IF (check_error(dm_err->eproc)=1)
             CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
             SET dm_err->err_ind = 0
             SET nodelete_ind = 1
           ...
           IF (drdv_circ_tab_name != "INVALIDTABLE")
            SET drdv_circ_tab_idx = locateval(drdv_circ_tab_idx,1,dm2_ref_data_doc->tbl_cnt,
             dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].circ_qual[drdv_col_idx].circ_table_name,
             dm2_ref_data_doc->tbl_qual[drdv_circ_tab_idx].table_name)
            IF (drdv_circ_tab_idx=0)
             SET drdv_circ_temp_cnt = temp_tbl_cnt
             SET drdv_circ_tab_idx = fill_rs("TABLE",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
              circ_qual[drdv_col_idx].circ_table_name)
             SET temp_tbl_cnt = drdv_circ_temp_cnt
             IF ((drdv_circ_tab_idx=- (1)))
              CALL disp_msg("Table can't be created from remote mover",dm_err->logfile,1)
              SET drdm_mini_loop_status = "REFCHG"
            ...
            SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
           ELSE
            SET drdm_parser->statement[drdm_parser_cnt].frag = concat("d.",dm2_ref_data_doc->
             tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name)
            SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
           ENDIF
          ELSE
        ...
        ENDFOR
        SET drdm_parser->statement[drdm_parser_cnt].frag = concat(
         ", cnvtdatetime(curdate,curtime3), ",build(drdm_chg->log[drdm_log_loop].log_id),",",build(
          '"',drdm_chg->log[drdm_log_loop].context_name,'"'),", ",
         build(dm2_ref_data_doc->env_source_id),", 0,0,vers_ptam_match_result,vers_ptam_match_str")
        SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
        SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" from ",vers_table_name,
         " d where d.",vers_primary_key," = vers_val) with notrim go")
   ...
   ELSE
    ...
    SET stat = alterlist(ui_query_rec->qual,1)
    SET ui_query_rec->qual[1].qtype = "EFFECTIVE"
    SET ui_query_rec->qual[1].additional_clause = concat(" AND <SUFFIX>.",vers_primary_key,
     " = <SUFFIX>.",vers_hist_name)
    SET vers_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
    IF (vers_query_ret > 0)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "OLDVER"
     ROLLBACK
     CALL trigger_proc_dcl(drdm_chg->log[drdm_log_loop].log_id,vers_table_name,concat("WHERE ",
       vers_primary_key,"=",trim(cnvtstring(vers_query_ret,20)),".0"),drdm_chg->log[drdm_log_loop].
      delete_ind,0.0,
      "REFCHG",drdm_chg->log[drdm_log_loop].src_cntxt_name)
    ELSEIF ((vers_query_ret=- (2)))
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV44"
     SET drdm_chg->log[drdm_log_loop].chg_log_reason_txt = concat(
      "Multiple effective rows found in source where ","current primary key = previous primary key")
    ELSEIF ((vers_query_ret=- (1)))
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
      ": A query for effective rows where current_pk = previous_pk in source had a syntax error.")
     GO TO exit_versions
    ELSE
     SET cur_pk_val = cnvtreal(get_value(vers_table_name,vers_primary_key,"FROM"))
     SET his_pk_val = cnvtreal(get_value(vers_table_name,vers_hist_name,"FROM"))
     IF (cur_pk_val=his_pk_val)
      SET vers_translated = 0
      SET vers_translated = is_translated(vers_table_name,vers_primary_key)
      IF (vers_translated=0)
       SET vers_translated = is_translated(vers_table_name,"UNIQUE")
       IF (vers_translated=1)
        ...
        IF (vers_query_ret > 0)
         ...
         CALL parser("and dc.rdds_status_flag < 9000 ",0)
         FOR (i = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
           IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].unique_ident_ind=1))
            IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].check_null=1))
             CALL parser(concat(" and dc.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].
               column_name," is NULL"),0)
            ELSE
             CALL parser(concat(" and dc.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].
               column_name,"=rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
         ...
         ENDFOR
         CALL parser(" with nocounter go",1)
         CALL put_value(vers_table_name,vers_primary_key,build(vers_query_ret))
         CALL put_value(vers_table_name,vers_hist_name,build(vers_query_ret))
         SET ver_imt_check = insert_merge_translate(cur_pk_val,vers_query_ret,vers_table_name)
         IF (ver_imt_check=1)
          GO TO exit_versions
         ENDIF
         SET vers_translated = is_translated(vers_table_name,"ALL")
        ...
        ELSEIF ((vers_query_ret=- (1)))
         SET dm2_ref_data_reply->error_ind = 1
         SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
          ": A query for effective rows in target had a syntax error.")
         GO TO exit_versions
        ELSE
         SET dm2_ref_data_reply->error_ind = 1
         SET dm2_ref_data_reply->error_msg = "OLDVER"
     ...
     ELSE
      ...
      SET ui_query_rec->dom = "FROM"
      SET stat = alterlist(ui_query_rec->qual,1)
      SET ui_query_rec->qual[1].qtype = "END_EFFECTIVE"
      SET ui_query_rec->qual[1].additional_clause = concat(" AND <SUFFIX>.",vers_primary_key,
       " = <SUFFIX>.",vers_hist_name," AND <SUFFIX>.",
       vers_primary_key," =  RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->from_values.",
       vers_hist_name)
      SET vers_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
      IF (vers_query_ret > 0)
       SET dm2_ref_data_reply->error_ind = 1
      ...
      ELSEIF ((vers_query_ret=- (2)))
       ...
       SET dm2_ref_data_reply->error_msg = "NOMV44"
       SET drdm_chg->log[drdm_log_loop].chg_log_reason_txt = concat(
        "Multiple non-effective rows found in source where",
        " current primary key = previous primary key")
      ELSEIF ((vers_query_ret=- (1)))
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
        ": A query for effective rows where current_pk = previous_pk in source had a syntax error.")
  ...
  OF "ALG6":
   ...
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_versions
   ENDIF
   IF (vers_cur_date >= vers_end_dt)
    SET ui_query_rec->table_name = vers_source_table
    SET ui_query_rec->dom = "FROM"
    SET stat = alterlist(ui_query_rec->qual,1)
    SET ui_query_rec->qual[1].qtype = "EFFECTIVE"
    SET ui_query_rec->qual[1].additional_clause = " AND <SUFFIX>.ACTIVE_IND = 1 "
    SET vers_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
    IF (((vers_query_ret > 0) OR ((vers_query_ret=- (2)))) )
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "OLDVER"
     ROLLBACK
    ELSEIF ((vers_query_ret=- (1)))
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
      ": A query for effective rows in source had a syntax error.")
     GO TO exit_versions
    ELSE
     IF (vers_active_ind=1)
      SET vers_translated = is_translated(vers_table_name,"UNIQUE")
      IF (vers_translated=1)
       SET vers_translated = 0
       SET ui_query_rec->table_name = vers_table_name
       ...
       IF (((vers_query_ret > 0) OR ((vers_query_ret=- (2)))) )
        ...
        IF ((global_mover_rec->ptam_ind=1))
         ...
         ENDIF
        ENDIF
        IF ((drdm_chg->log[drdm_log_loop].md_delete_ind=1))
         CALL parser(concat("delete from ",cutover_tab_name(vers_table_name,dm2_ref_data_doc->
            tbl_qual[temp_tbl_cnt].suffix)," dc where "),0)
         CALL parser(concat("dc.rdds_source_env_id = ",build(dm2_ref_data_doc->env_source_id)),0)
         CALL parser("and dc.rdds_status_flag < 9000 ",0)
         ...
         FOR (i = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
           IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].unique_ident_ind=1))
            ...
            ENDIF
            IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].check_null=1))
             CALL parser(concat(" dc.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].
               column_name," is NULL"),0)
            ELSE
             IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].trailing_space_cnt > 0)
              AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].data_type="VC"))
        ...
        IF (is_translated(vers_table_name,vers_primary_key)=0)
         ...
         ENDIF
        ENDIF
        CALL insert_update_row(temp_tbl_cnt,size(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual,5)
         )
       ELSEIF ((vers_query_ret=- (1)))
        SET dm2_ref_data_reply->error_ind = 1
        SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
     ...
     ELSE
      ...
      SET stat = alterlist(ui_query_rec->qual,1)
      SET ui_query_rec->qual[1].qtype = "EFFECTIVE"
      SET ui_query_rec->qual[1].additional_clause = " AND <SUFFIX>.ACTIVE_IND = 0 "
      SET vers_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
      IF (((vers_query_ret > 0) OR ((vers_query_ret=- (2)))) )
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = "OLDVER"
         ...
         IF (((vers_query_ret > 0) OR ((vers_query_ret=- (2)))) )
          ...
          IF ((drdm_chg->log[drdm_log_loop].md_delete_ind=1))
           ...
           IF (check_error(dm_err->eproc)=1)
            ...
            SET nodelete_ind = 1
           ENDIF
           IF (((findstring("ORA-00001:",dm_err->emsg) > 0) OR (((findstring("ORA-02049:",dm_err->
            emsg) > 0) OR (findstring("ORA-4020:",dm_err->emsg) > 0)) )) )
            SET drmm_ddl_rollback_ind = 1
            SET drdm_log_loop = redirect_to_start_row(drdm_chg,drdm_log_loop,drdm_next_row_pos,
             drdm_max_rs_size)
           ENDIF
     ...
     ENDIF
    ENDIF
   ELSEIF (vers_cur_date >= vers_beg_dt
    AND vers_cur_date <= vers_end_dt)
    IF (is_translated(vers_table_name,vers_primary_key)=0)
     IF (vers_active_ind=0)
      SET ui_query_rec->table_name = vers_source_table
     ...
     ELSE
      ...
      IF (vers_translated=1)
       ...
       ELSEIF ((vers_query_ret=- (3)))
        ...
        ENDIF
       ELSE
        IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].user_gen_date_ind=0))
         SET vers_query_ret = get_seq(vers_table_name,vers_primary_key)
         IF (vers_query_ret > 0)
          CALL put_value(vers_table_name,vers_primary_key,build(vers_query_ret))
          SET current_merges = (current_merges+ 1)
         ...
         ENDIF
        ELSE
         IF (((vers_table_name="BILL_ITEM_MODIFIER") OR (vers_table_name="PRICE_SCHED_ITEMS")) )
          SET select_merge_translate_rec->type = "TO"
          SET select_merge_translate_rec->to_opt_ind = 1
          SET vers_to_xlat_ret = select_merge_translate(cnvtstring(vers_query_ret,20,1),
           vers_table_name)
          SET select_merge_translate_rec->to_opt_ind = 0
          SET select_merge_translate_rec->type = "FROM"
          IF (isnumeric(vers_to_xlat_ret))
           SET vers_query_ret = get_seq(vers_table_name,vers_primary_key)
           IF (vers_query_ret > 0)
            CALL put_value(vers_table_name,vers_primary_key,build(vers_query_ret))
            SET current_merges = (current_merges+ 1)
       ...
       SET ver_imt_check = insert_merge_translate(vers_from_id,vers_query_ret,vers_table_name)
       IF (ver_imt_check=1)
        IF (drdm_mini_loop_status="NOMV60")
         SET dm2_ref_data_reply->error_ind = 1
         SET dm2_ref_data_reply->error_msg = drdm_mini_loop_status
        ENDIF
        GO TO exit_versions
       ENDIF
      ELSE
    ...
    ELSE
     SET drdv_dual_ind = 0
     CALL parser(concat('select into "NL:" from ',vers_table_name," d"),0)
     CALL parser(concat("where d.",vers_primary_key," = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt]
       .suffix,"->to_values.",
       vers_primary_key),0)
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].user_gen_date_ind=0))
      CALL parser(concat(" detail if (d.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].beg_col_name,
        " >= cnvtdatetime(curdate,curtime3) or d.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
        end_col_name," <= cnvtdatetime(curdate,curtime3))"),0)
      CALL parser(" drdv_dual_ind = 1 endif",0)
     ELSE
      CALL parser(concat(" detail if (d.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].end_col_name,
        " <= cnvtdatetime(curdate,curtime3))"),0)
      CALL parser(" drdv_dual_ind = 1 endif",0)
     ENDIF
     IF (vers_active_ind=1)
      CALL parser("if (d.active_ind != 1) drdv_dual_ind = 1 endif",0)
     ENDIF
     CALL parser(" with nocounter go",1)
     IF (check_error(dm_err->eproc)=1)
      SET dm2_ref_data_reply->error_msg = dm_err->emsg
     ...
     IF (curqual > 0
      ...
      CALL parser(concat(
        ...
        temp_tbl_cnt].suffix,"->from_values.",vers_primary_key,
        ",20)),' in source maps to ',trim(cnvtstring(RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
        suffix,"->to_values.",vers_primary_key,
        ",20)),' in target which is inactive or no longer effective') go"),1)
      SET dm2_ref_data_reply->error_ind = 1
      SET dm2_ref_data_reply->error_msg = "NOMV76"
      SET drdm_mini_loop_status = "NOMV76"
      SET drdm_no_trans_ind = 1
    ...
    IF (vers_self_col_ind=1)
     ...
     GO TO exit_versions
    ENDIF
    IF (is_translated(vers_table_name,"ALL")=1)
     SET vers_r_query_ret = vers_r_context("EFFECTIVE")
     CALL drmm_get_ptam_match_query(drdm_chg->log[drdm_log_loop].log_id,drdm_chg->log[drdm_log_loop].
      table_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,dm2_ref_data_doc->post_link_name)
     SET vers_ptam_match_result = drmm_get_ptam_match_result(0.0,dm2_ref_data_doc->env_source_id,
     ...
     IF ((drdm_chg->log[drdm_log_loop].md_delete_ind=1))
      ...
      FOR (i = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
        IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].unique_ident_ind=1))
         ...
         ENDIF
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].check_null=1))
          CALL parser(concat(" dc.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].column_name,
            " is NULL"),0)
         ELSE
          IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].trailing_space_cnt > 0)
           AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].data_type="VC"))
           CALL parser(concat(" dc.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].column_name,
             "=notrim(concat(rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
             dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].column_name,", fillstring(value(",
             trim(cnvtstring(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].trailing_space_cnt)
              ),'), " ")))'),0)
          ELSE
           CALL parser(concat(" dc.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[i].column_name,
   ...
   ELSE
    IF (is_translated(vers_table_name,vers_primary_key)=0)
     ...
     IF (vers_translated=1)
      ...
      SET stat = alterlist(ui_query_rec->qual,1)
      SET ui_query_rec->dom = "TO"
      SET ui_query_rec->qual[1].qtype = "FUTURE_EFFECTIVE"
      SET ui_query_rec->qual[1].additional_clause = " AND <SUFFIX>.ACTIVE_IND = 1 "
      SET vers_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
      IF ((vers_query_ret=- (1)))
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg =
       "There was a syntax error when querying for active future effective rows in target"
       GO TO exit_versions
      ELSEIF ((((vers_query_ret=- (3))) OR ((vers_query_ret=- (2)))) )
       SET vers_query_ret = get_seq(vers_table_name,vers_primary_key)
       IF (vers_query_ret > 0)
        CALL put_value(vers_table_name,vers_primary_key,build(vers_query_ret))
        SET current_merges = (current_merges+ 1)
  ...
  OF "ALG7":
   IF (is_translated(vers_table_name,vers_primary_key)=0)
    SET vers_translated = is_translated(vers_table_name,"EXCPTN12")
    IF (vers_translated=1)
     SET vers_query_ret = get_seq(vers_table_name,vers_primary_key)
     IF (vers_query_ret > 0)
      CALL put_value(vers_table_name,vers_primary_key,build(vers_query_ret))
   ...
   ENDIF
   IF (is_translated(vers_table_name,"ALL")=1)
    SET vers_r_query_ret = vers_r_context("GROUPERONLY")
    CALL drmm_get_ptam_match_query(drdm_chg->log[drdm_log_loop].log_id,drdm_chg->log[drdm_log_loop].
     table_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,dm2_ref_data_doc->post_link_name)
    SET vers_ptam_match_result = drmm_get_ptam_match_result(0.0,dm2_ref_data_doc->env_source_id,
     dm2_ref_data_doc->env_target_id,dm2_ref_data_doc->post_link_name,0,
   ...
   ENDIF
 ENDCASE
 SUBROUTINE vers_mass_update(vers_type)
   DECLARE vers_mult_loop = i4
   DECLARE vers_col_name = vc
   DECLARE vers_from_con = vc
   DECLARE vmu_match_str = vc WITH protect, noconstant(" "), notrim
   DECLARE vmu_ptam_result = f8 WITH protect, noconstant(0.0)
   DECLARE vmu_child_return = i2 WITH protect, noconstant(0)
   IF (daf_is_not_blank(version_nbr_col))
    SET vmu_child_return = fix_child_tabs(vers_type)
    IF (vmu_child_return=1)
     RETURN(vmu_child_return)
    ENDIF
   ENDIF
   CALL drmm_get_ptam_match_query(drdm_chg->log[drdm_log_loop].log_id,drdm_chg->log[drdm_log_loop].
    table_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,dm2_ref_data_doc->post_link_name)
   SET vmu_ptam_result = drmm_get_ptam_match_result(0.0,dm2_ref_data_doc->env_source_id,
    dm2_ref_data_doc->env_target_id,dm2_ref_data_doc->post_link_name,0,
    "FROM")
   SET vmu_match_str = vers_get_match_str(null)
   IF (check_error(dm_err->eproc)=1)
    SET dm2_ref_data_reply->error_msg = dm_err->emsg
    SET dm2_ref_data_reply->error_ind = 1
    RETURN(1)
   ENDIF
   IF ((drdm_chg->log[drdm_log_loop].ptam_str_tspace_cnt > 0))
    SET vmu_match_str = notrim(concat(vmu_match_str,notrim(fillstring(value(drdm_chg->log[
         drdm_log_loop].ptam_str_tspace_cnt)," "))))
   ENDIF
   SET drdm_parser->statement[1].frag = concat("update into ",cutover_tab_name(vers_table_name,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix)," dc set dc.active_ind = 0 ")
   SET drdm_parser->statement[2].frag = concat(",dc.rdds_status_flag = 0, dc.rdds_source_env_id = ",
    build(dm2_ref_data_doc->env_source_id),",dc.rdds_ptam_match_result = vmu_ptam_result")
   SET drdm_parser->statement[3].frag = ", dc.rdds_ptam_match_result_str = vmu_match_str"
   SET vers_parser_cnt = 3
   IF (vers_task_ind=1)
    SET vers_parser_cnt = (vers_parser_cnt+ 1)
    SET drdm_parser->statement[vers_parser_cnt].frag = " ,dc.updt_task = 4310001 "
   ENDIF
   IF (vers_dt_tm_ind=1)
    SET vers_parser_cnt = (vers_parser_cnt+ 1)
    SET drdm_parser->statement[vers_parser_cnt].frag =
    " ,dc.updt_dt_tm = cnvtdatetime(vers_cur_date) "
   ENDIF
   IF (vers_id_ind=1)
    SET vers_parser_cnt = (vers_parser_cnt+ 1)
    SET drdm_parser->statement[vers_parser_cnt].frag = " ,dc.updt_id = 0 "
   ENDIF
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind=1))
    SET vers_parser_cnt = (vers_parser_cnt+ 1)
    SET drdm_parser->statement[vers_parser_cnt].frag = concat(" ,dc.",dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].end_col_name," = cnvtdatetime(vers_cur_date) ")
   ENDIF
   SET vers_parser_cnt = (vers_parser_cnt+ 1)
   SET drdm_parser->statement[vers_parser_cnt].frag = concat(' ,dc.rdds_context_name = "',drdm_chg->
    log[drdm_log_loop].context_name,'" ')
   SET vers_parser_cnt = (vers_parser_cnt+ 1)
   SET drdm_parser->statement[vers_parser_cnt].frag = " where "
   SET vers_hold_cnt = vers_parser_cnt
   FOR (vers_mult_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_mult_loop].unique_ident_ind=1))
      IF (((vers_parser_cnt - vers_hold_cnt) >= 1))
       SET vers_parser_cnt = (vers_parser_cnt+ 1)
       SET drdm_parser->statement[vers_parser_cnt].frag = " and "
      ENDIF
      SET vers_col_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_mult_loop].
      column_name
      SET vers_from_con = concat("rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
       vers_col_name)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_mult_loop].check_null=1))
       SET vers_parser_cnt = (vers_parser_cnt+ 1)
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_mult_loop].data_type IN ("DQ8",
       "F8", "I4", "I2")))
        SET drdm_parser->statement[vers_parser_cnt].frag = concat(" dc.",vers_col_name," = NULL")
       ELSE
        SET drdm_parser->statement[vers_parser_cnt].frag = concat(" (dc.",vers_col_name," = null or ",
         vers_col_name," = ' ')")
       ENDIF
      ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_mult_loop].check_space=1))
       SET vers_parser_cnt = (vers_parser_cnt+ 1)
       SET drdm_parser->statement[vers_parser_cnt].frag = concat(" (dc.",vers_col_name," = ' ' or ",
        vers_col_name," = null)")
      ELSE
       CASE (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_mult_loop].data_type)
        OF "DQ8":
         SET vers_parser_cnt = (vers_parser_cnt+ 1)
         SET drdm_parser->statement[vers_parser_cnt].frag = concat(" dc.",vers_col_name,
          " =  cnvtdatetime(",vers_from_con,")")
        ELSE
         SET vers_parser_cnt = (vers_parser_cnt+ 1)
         SET drdm_parser->statement[vers_parser_cnt].frag = concat(" dc.",vers_col_name," = ",
          vers_from_con)
     ...
     ENDIF
   ENDFOR
   IF (vers_type="INACTIVATE")
    SET vers_parser_cnt = (vers_parser_cnt+ 1)
    SET drdm_parser->statement[vers_parser_cnt].frag = " and dc.active_ind = 1"
   ENDIF
   IF (vers_type="END-EFFECTIVE")
    SET vers_parser_cnt = (vers_parser_cnt+ 1)
    SET drdm_parser->statement[vers_parser_cnt].frag = concat(" AND dc.",dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].beg_col_name,"<=  cnvtdatetime(Vers_cur_date) AND dc.",dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].end_col_name,">= cnvtdatetime(vers_cur_date)")
   ENDIF
   SET vers_parser_cnt = (vers_parser_cnt+ 1)
   SET drdm_parser->statement[vers_parser_cnt].frag = concat(
    " AND dc.rdds_status_flag < 9000 AND dc.rdds_source_env_id = ",build(dm2_ref_data_doc->
     env_source_id))
   SET vers_parser_cnt = (vers_parser_cnt+ 1)
   SET drdm_parser->statement[vers_parser_cnt].frag = " with notrim go"
   CALL parse_statements(vers_parser_cnt)
   IF (check_error(dm_err->eproc)=1)
    SET dm2_ref_data_reply->error_msg = dm_err->emsg
    SET dm2_ref_data_reply->error_ind = 1
    RETURN(1)
   ...
   RETURN(0)
 END ;Subroutine
 SUBROUTINE vers_r_context(vrc_type)
   DECLARE vrc_query_ret = f8
   IF (vers_alg IN ("ALG2", "ALG5", "ALG6", "ALG7"))
    SET vrc_query_ret = - (3)
   ELSE
    IF ((drdm_chg->log[drdm_log_loop].par_location=0))
     SET ui_query_rec->table_name = cutover_tab_name(drdm_chg->log[drdm_log_loop].table_name,
      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix)
     SET ui_query_rec->dom = "$R"
     SET stat = alterlist(ui_query_rec->qual,1)
     SET ui_query_rec->qual[1].qtype = "ACTIVE"
     SET stat = alterlist(ui_query_rec->qual[1].other_pk_col,1)
     SET ui_query_rec->qual[1].other_pk_col[1].col_name = "RDDS_CONTEXT_NAME"
     SET vrc_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
    ENDIF
   ENDIF
   IF ((vrc_query_ret=- (3))
    AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind != 0)
    AND vrc_type="EFFECTIVE")
    SET ui_query_rec->table_name = cutover_tab_name(drdm_chg->log[drdm_log_loop].table_name,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix)
    SET ui_query_rec->dom = "$R"
    SET stat = alterlist(ui_query_rec->qual,1)
    IF (vers_alg="ALG5")
     SET ui_query_rec->qual[1].qtype = "GROUPER-EFFECTIVE"
    ELSE
     SET ui_query_rec->qual[1].qtype = "EFFECTIVE"
    ENDIF
    SET stat = alterlist(ui_query_rec->qual[1].other_pk_col,1)
   ...
   IF ((vrc_query_ret=- (3)))
    ...
    IF (vers_alg="ALG2")
     SET ui_query_rec->qual[1].qtype = "END_EFFECTIVE"
    ELSEIF (vers_alg="ALG5")
     SET ui_query_rec->qual[1].qtype = "GROUPER-END_EFFECTIVE"
    ELSEIF (vrc_type="GROUPERONLY")
     SET ui_query_rec->qual[1].qtype = "GROUPERONLY"
    ELSE
     SET ui_query_rec->qual[1].qtype = "UIONLY"
    ENDIF
    SET stat = alterlist(ui_query_rec->qual[1].other_pk_col,1)
    SET ui_query_rec->qual[1].other_pk_col[1].col_name = "RDDS_CONTEXT_NAME"
    SET vrc_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
   ENDIF
   SET drdm_chg->log[drdm_log_loop].context_name = evaluate_context_name(trim(ui_query_eval_rec->
     qual[1].additional_attr),drdm_chg->log[drdm_log_loop].context_name)
   RETURN(vrc_query_ret)
 END ;Subroutine
 SUBROUTINE fix_child_tabs(fct_type)
   DECLARE fct_loop = i4 WITH protect, noconstant(0)
   DECLARE fct_temp_pos = i4 WITH protect, noconstant(0)
   DECLARE fct_new_pos = i4 WITH protect, noconstant(0)
   DECLARE fct_parser_cnt = i4 WITH protect, noconstant(0)
   DECLARE fct_hold_cnt = i4 WITH protect, noconstant(0)
   DECLARE fct_mult_loop = i4 WITH protect, noconstant(0)
   DECLARE fct_col_name = vc WITH protect, noconstant("")
   DECLARE fct_from_con = vc WITH protect, noconstant("")
   FREE RECORD fct_children
   RECORD fct_children(
     1 cnt = i4
     1 qual[*]
       2 child_table = vc
   ...
   FREE RECORD fct_parser
   RECORD fct_parser(
     1 statement[*]
       2 frag = vc
   )
   SET stat = alterlist(fct_parser->statement,2000)
   SELECT INTO "NL:"
    FROM dm_refchg_version_r d
    WHERE (d.parent_table=drdm_chg->log[drdm_log_loop].table_name)
    DETAIL
     fct_children->cnt = (fct_children->cnt+ 1), stat = alterlist(fct_children->qual,fct_children->
      cnt), fct_children->qual[fct_children->cnt].child_table = d.child_table,
     fct_children->qual[fct_children->cnt].parent_table = d.parent_table, fct_children->qual[
     fct_children->cnt].parent_id_col = d.parent_id_col, fct_children->qual[fct_children->cnt].
     child_id_col = d.child_id_col
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm2_ref_data_reply->error_msg = dm_err->emsg
   ...
   IF (curqual=0)
    RETURN(0)
   ENDIF
   FOR (fct_loop = 1 TO fct_children->cnt)
     SET fct_new_pos = locateval(fct_temp_pos,1,dm2_ref_data_doc->tbl_cnt,fct_children->qual[fct_loop
      ].child_table,dm2_ref_data_doc->tbl_qual[fct_temp_pos].table_name)
     IF (fct_new_pos=0)
      SET fct_temp_pos = temp_tbl_cnt
      SET fct_new_pos = fill_rs("TABLE",fct_children->qual[fct_loop].child_table)
      SET temp_tbl_cnt = fct_temp_pos
      IF ((fct_new_pos=- (2)))
       SET drmm_ddl_rollback_ind = 1
       SET drdm_log_loop = redirect_to_start_row(drdm_chg,drdm_log_loop,drdm_next_row_pos,
        drdm_max_rs_size)
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg =
       "DDL was performed to create a $R table, row needs to be restarted"
       RETURN(1)
      ELSEIF (fct_new_pos <= 0)
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = dm_err->emsg
       RETURN(1)
      ENDIF
     ENDIF
     SET fct_parser->statement[1].frag = concat("update into ",dm2_ref_data_doc->tbl_qual[fct_new_pos
      ].r_table_name," d")
     SET fct_parser->statement[2].frag =
     "set d.rdds_status_flag = 9005, rdds_dt_tm = cnvtdatetime(curdate,curtime3)"
     SET fct_parser->statement[3].frag = concat("where ",fct_children->qual[fct_loop].child_id_col,
      " in (")
     SET fct_parser->statement[4].frag = concat("select ",fct_children->qual[fct_loop].parent_id_col)
     SET fct_parser->statement[5].frag = concat(" from ",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
      r_table_name," dc where ")
     SET fct_parser_cnt = 5
     SET fct_hold_cnt = fct_parser_cnt
     FOR (fct_mult_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[fct_mult_loop].unique_ident_ind=1))
        IF (((fct_parser_cnt - fct_hold_cnt) >= 1))
         SET fct_parser_cnt = (fct_parser_cnt+ 1)
         SET fct_parser->statement[fct_parser_cnt].frag = " and "
        ENDIF
        SET fct_col_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[fct_mult_loop].
        column_name
        SET fct_from_con = concat("rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
         "->to_values.",fct_col_name)
        IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[fct_mult_loop].check_null=1))
         SET fct_parser_cnt = (fct_parser_cnt+ 1)
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[fct_mult_loop].data_type IN ("DQ8",
         "F8", "I4", "I2")))
          SET fct_parser->statement[fct_parser_cnt].frag = concat(" dc.",fct_col_name," = NULL")
         ELSE
          SET fct_parser->statement[fct_parser_cnt].frag = concat(" (dc.",fct_col_name," = null or ",
           fct_col_name," = ' ')")
         ENDIF
        ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[fct_mult_loop].check_space=1))
         SET fct_parser_cnt = (fct_parser_cnt+ 1)
         SET fct_parser->statement[fct_parser_cnt].frag = concat(" (dc.",fct_col_name," = ' ' or ",
          fct_col_name," = null)")
        ELSE
         CASE (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[fct_mult_loop].data_type)
          OF "DQ8":
           SET fct_parser_cnt = (fct_parser_cnt+ 1)
           SET fct_parser->statement[fct_parser_cnt].frag = concat(" dc.",fct_col_name,
            " =  cnvtdatetime(",fct_from_con,")")
          ELSE
           SET fct_parser_cnt = (fct_parser_cnt+ 1)
           SET fct_parser->statement[fct_parser_cnt].frag = concat(" dc.",fct_col_name," = ",
            fct_from_con)
     ...
     ENDFOR
     SET fct_parser_cnt = (fct_parser_cnt+ 1)
     SET fct_parser->statement[fct_parser_cnt].frag = concat(" and not exists(select 'x' from ",
      fct_children->qual[fct_loop].parent_table," p "," where p.",fct_children->qual[fct_loop].
      parent_id_col,
      " = "," dc.",fct_children->qual[fct_loop].parent_id_col,")")
     IF (fct_type="INACTIVATE")
      SET fct_parser_cnt = (fct_parser_cnt+ 1)
      SET fct_parser->statement[fct_parser_cnt].frag = " and dc.active_ind = 1"
     ENDIF
     IF (fct_type="END-EFFECTIVE")
      SET fct_parser_cnt = (fct_parser_cnt+ 1)
      SET fct_parser->statement[fct_parser_cnt].frag = concat(" AND dc.",dm2_ref_data_doc->tbl_qual[
       temp_tbl_cnt].beg_col_name,"<=  cnvtdatetime(Vers_cur_date) AND dc.",dm2_ref_data_doc->
       tbl_qual[temp_tbl_cnt].end_col_name,">= cnvtdatetime(vers_cur_date)")
     ENDIF
     SET fct_parser_cnt = (fct_parser_cnt+ 1)
     SET fct_parser->statement[fct_parser_cnt].frag = concat(
      " AND dc.rdds_status_flag < 9000 AND dc.rdds_source_env_id = ",build(dm2_ref_data_doc->
       env_source_id))
     SET fct_parser_cnt = (fct_parser_cnt+ 1)
     SET fct_parser->statement[fct_parser_cnt].frag = concat(
      ") and d.rdds_status_flag < 9000 and d.rdds_delete_ind = 0 and d.rdds_source_env_id = ",build(
       dm2_ref_data_doc->env_source_id)," with nocounter, notrim go")
     FOR (fct_temp_pos = 1 TO fct_parser_cnt)
       SET drdm_parser->statement[fct_temp_pos].frag = fct_parser->statement[fct_temp_pos].frag
     ENDFOR
     CALL parse_statements(fct_parser_cnt)
     IF (check_error(dm_err->eproc)=1)
      SET dm2_ref_data_reply->error_msg = dm_err->emsg
      SET dm2_ref_data_reply->error_ind = 1
      RETURN(1)
     ENDIF
     IF (fct_loop=1)
      SET fct_parser->statement[1].frag = concat(
       'delete from dm_merge_translate d where d.table_name = "',fct_children->qual[fct_loop].
       parent_table,'" and ')
      SET fct_parser->statement[2].frag = concat(" d.env_source_id = ",build(dm2_ref_data_doc->
        env_source_id)," and d.env_target_id = ",build(dm2_ref_data_doc->mock_target_id))
      SET fct_parser->statement[3].frag = " and d.to_value in ("
      SET fct_parser->statement[4].frag = concat("select ",fct_children->qual[fct_loop].parent_id_col
       )
      SET fct_parser->statement[5].frag = concat(" from ",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
       r_table_name," dc where ")
      SET fct_parser->statement[fct_parser_cnt].frag = ") with nocounter, notrim go"
      FOR (fct_temp_pos = 1 TO fct_parser_cnt)
        SET drdm_parser->statement[fct_temp_pos].frag = fct_parser->statement[fct_temp_pos].frag
      ENDFOR
      CALL parse_statements(fct_parser_cnt)
   ...
   RETURN(0)
 END ;Subroutine
 SUBROUTINE xlat_self_ref_col(xsrc_pk_from,xsrc_pk_to,xsrc_tab_pos)
   DECLARE xsrc_val = f8 WITH protect, noconstant(0.0)
   DECLARE xsrc_idx = i4 WITH protect, noconstant(0)
   FOR (xsrc_idx = 1 TO dm2_ref_data_doc->tbl_qual[xsrc_tab_pos].col_cnt)
     IF ((dm2_ref_data_doc->tbl_qual[xsrc_tab_pos].table_name=dm2_ref_data_doc->tbl_qual[xsrc_tab_pos
     ].col_qual[xsrc_idx].root_entity_name)
      AND (dm2_ref_data_doc->tbl_qual[xsrc_tab_pos].col_qual[xsrc_idx].root_entity_attr=
     dm2_ref_data_doc->tbl_qual[xsrc_tab_pos].root_col_name)
      AND (dm2_ref_data_doc->tbl_qual[xsrc_tab_pos].col_qual[xsrc_idx].column_name !=
     dm2_ref_data_doc->tbl_qual[xsrc_tab_pos].root_col_name)
      AND (dm2_ref_data_doc->tbl_qual[xsrc_tab_pos].col_qual[xsrc_idx].exception_flg != 11))
      SET xsrc_val = cnvtreal(get_value(vers_table_name,dm2_ref_data_doc->tbl_qual[xsrc_tab_pos].
        col_qual[xsrc_idx].column_name,"FROM"))
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dm_err->err_ind = 0
       SET nodelete_ind = 1
       RETURN(1)
      ENDIF
      IF (xsrc_val=xsrc_pk_from)
       IF (is_translated(vers_table_name,dm2_ref_data_doc->tbl_qual[xsrc_tab_pos].col_qual[xsrc_idx].
        column_name)=0)
        CALL put_value(vers_table_name,dm2_ref_data_doc->tbl_qual[xsrc_tab_pos].col_qual[xsrc_idx].
         column_name,build(xsrc_pk_to))
        IF (check_error(dm_err->eproc)=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         SET dm_err->err_ind = 0
         SET nodelete_ind = 1
   ...
   RETURN(0)
 END ;Subroutine
 SUBROUTINE vers_get_match_str(null)
   DECLARE vgms_ret = vc WITH protect, noconstant(" ")
   SELECT INTO "NL:"
    y = sys_context("CERNER","RDDS_PTAM_MATCH_RESULT_STR",4000)
    FROM dual
    DETAIL
 ...
 END ;Subroutine
#exit_versions
 FREE SET vers_active_ind
 FREE SET vers_table_name
 FREE SET vers_primary_key
 FREE SET vers_val
 FREE SET vers_active_num
 FREE SET vers_from_id
 FREE SET vers_loop
 FREE SET vers_col_cnt
 FREE SET vers_dt_tm_loop
 FREE SET vers_cur_date
 FREE SET vers_alg
 FREE SET vers_translated
 FREE SET vers_hist_name
 FREE SET vers_rs_name
 FREE SET vers_beg_dt
 FREE SET vers_end_dt
 FREE SET vers_col_name
 FREE SET vers_rs_string
 FREE SET vers_new_val
 FREE SET vers_source_table
END GO
