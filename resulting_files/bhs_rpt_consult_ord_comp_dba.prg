CREATE PROGRAM bhs_rpt_consult_ord_comp:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Select Facility" = value(673936.00),
  "Start date" = "CURDATE",
  "End Date" = "CURDATE",
  "Select Consult Orders" = 0,
  "Select Output" = "",
  "Send file to file share" = 0,
  "Date Range" = ""
  WITH outdev, facility, s_start_date,
  s_end_date, f_consult_ood, output_type,
  ftpfiles, s_range
 ...
 IF (( $FTPFILES=1))
  ...
  ELSEIF (( $OUTPUT_TYPE != "CSV"))
   IF (( $OUTPUT_TYPE IN ("DETAIL", "DETSUM")))
    SET ms_fileprefix = "ord_det"
   ELSEIF (( $OUTPUT_TYPE IN ("SUMMARY")))
    SET ms_fileprefix = "ord_summ"
   ELSEIF (( $OUTPUT_TYPE="MDSUMMARY"))
    SET ms_fileprefix = "ord_mdsum"
   ELSEIF (( $OUTPUT_TYPE="MDSUMTOT"))
    SET ms_fileprefix = "ord_md_tot"
   ENDIF
   SET ms_var_outpmr = build(ms_files_loc,ms_name_fac,"_",ms_fileprefix,"_",
    ms_month,"_",ms_day,"_",ms_time,
    "_",ms_year,".pdf")
 ...
 IF (cnvtupper(trim( $S_RANGE,3))="DAILY")
  ...
  SET ms_end_date = format(datetimefind(cnvtlookbehind("1,D",cnvtdatetime(curdate,0000)),"D","E","E"),
   "DD-MMM-YYYY;;D")
  SET ms_end_date = format(cnvtdatetime(cnvtdate2(ms_end_date,"DD-MMM-YYYY"),2359),";;Q")
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   WHERE o.orig_order_dt_tm BETWEEN cnvtdatetime(ms_start_date) AND cnvtdatetime(ms_end_date)
    AND (o.catalog_cd= $F_CONSULT_OOD)
    AND o.person_id > 0)
   JOIN (oa
  ...
  HEAD o.order_id
   ...
   ENDIF
   CALL echo(build("mi_ord = ",mi_ord," pmr_consult->cnt_p2 = ",pmr_consult->cnt_p)), pmr_consult->
   prsnl[pmr_consult->cnt_p].pmr_order[mi_ord].fin = fin.alias, pmr_consult->prsnl[pmr_consult->cnt_p
   ].pmr_order[mi_ord].order_id = o.order_id,
   pmr_consult->prsnl[pmr_consult->cnt_p].pmr_order[mi_ord].order_status = trim(uar_get_code_display(
     o.order_status_cd),3), pmr_consult->prsnl[pmr_consult->cnt_p].pmr_order[mi_ord].order_dtttm = o
   .orig_order_dt_tm, pmr_consult->prsnl[pmr_consult->cnt_p].pmr_order[mi_ord].before_4pm = "N",
   ...
   ELSE
    pmr_consult->prsnl[pmr_consult->cnt_p].pmr_order[mi_ord].due_date2 = cnvtdatetime((cnvtdate(o
      .orig_order_dt_tm)+ 2),000000)
   ENDIF
   pmr_consult->prsnl[pmr_consult->cnt_p].pmr_order[mi_ord].order_name = trim(o.order_mnemonic,3),
   ...
   pmr_consult->cnt_p].per_on_time2 = format(round(((cnvtreal(pmr_consult->prsnl[pmr_consult->cnt_p].
      toton_time1)/ cnvtreal(pmr_consult->prsnl[pmr_consult->cnt_p].total_orders)) * 100),2),
    "###.##;p0")
 ...
 IF (( $OUTPUT_TYPE="CSV"))
  SELECT
   ...
   order_name = pmr_consult->prsnl[d1.seq].pmr_order[d2.seq].order_name, ordered_date_time = format(
    pmr_consult->prsnl[d1.seq].pmr_order[d2.seq].order_dtttm,"mm/dd/yyyy hh:mm;;d"),
   completed_date_time = format(pmr_consult->prsnl[d1.seq].pmr_order[d2.seq].completed_dt_tm,
    "mm/dd/yyyy hh:mm;;d"),
   due_date_12pm_order = format(pmr_consult->prsnl[d1.seq].pmr_order[d2.seq].due_date1,
    "mm/dd/yyyy hh:mm;;d"), discharge_date_time = format(pmr_consult->prsnl[d1.seq].pmr_order[d2.seq]
    .due_date1,"mm/dd/yyyy hh:mm;;d"), status = substring(1,30,pmr_consult->prsnl[d1.seq].pmr_order[
    d2.seq].status),
   done_on_time = pmr_consult->prsnl[d1.seq].pmr_order[d2.seq].done_on_time1
 ...
 ELSEIF (( $OUTPUT_TYPE="DETAIL"))
  EXECUTE bhs_rpt_pmr_orders_lo value(ms_var_outpmr),  $FACILITY,  $S_START_DATE,
 ...
 ELSEIF (( $OUTPUT_TYPE="DETSUM"))
  EXECUTE bhs_rpt_pmr_orders_lo value(ms_var_outpmr),  $FACILITY,  $S_START_DATE,
   ...
   $S_RANGE
 ELSEIF (( $OUTPUT_TYPE="MDSUMMARY"))
  EXECUTE bhs_rpt_pmr_ordersmd_lo value(ms_var_outpmr),  $FACILITY,  $S_START_DATE,
   $S_END_DATE,  $OUTPUT_TYPE,  $FTPFILES,
   $S_RANGE
 ELSEIF (( $OUTPUT_TYPE="MDSUMTOT"))
  EXECUTE bhs_rpt_pmr_ordersmd_lo value(ms_var_outpmr),  $FACILITY,  $S_START_DATE,
