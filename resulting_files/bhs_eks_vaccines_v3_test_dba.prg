CREATE PROGRAM bhs_eks_vaccines_v3_test:dba
 PROMPT
  ...
  prompt6
 DECLARE vaccine_parser = vc WITH noconstant(""), protect
 ...
 SET flu_vaccine_cd = uar_get_code_by("DISPLAY",200,"Influenza Virus Vaccines")
 ...
 SET pneum_vaccine_cd = uar_get_code_by("DISPLAY",200,"Pneumococcal Vaccines")
 ...
 IF (( $4="3"))
  SET vaccine_parser =
"o.catalog_cd in (flu_vaccine_cd,flu_vaccine_inact_cd,flu_vaccine_live_cd ,pneum_23vaccine_cd,pneum_13vaccine_cd,pneum_7vac\
cine_cd,pneum_vaccine_cd,h1n1_1,h1n1_2,h1n1_3,h1n1_4)\
"
 ELSEIF (( $4="2"))
  SET vaccine_parser =
  "o.catalog_cd in (pneum_23vaccine_cd,pneum_13vaccine_cd,pneum_7vaccine_cd,pneum_vaccine_cd)"
 ELSEIF (( $4="1"))
  SET vaccine_parser =
  "o.catalog_cd in (flu_vaccine_cd,flu_vaccine_inact_cd,flu_vaccine_live_cd, h1n1_1,h1n1_2,h1n1_3,h1n1_4 )"
 ENDIF
 SELECT INTO "nl:"
  FROM order_action oa,
   ...
   prsnl pr
  PLAN (o
   WHERE o.orig_order_dt_tm BETWEEN cnvtdatetime(cnvtdate( $2),0) AND cnvtdatetime(cnvtdate( $3),
    235959)
    AND parser(vaccine_parser)
    AND o.active_ind=1)
   JOIN (oa
  ...
  FOOT REPORT
   ...
   ENDIF
  WITH outerjoin = d, nullreport, orahintcbo("ddtest","MONITOR","GATHER_PLAN_STATISTICS"),
   time = 300
 ;end select
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   vaccine->list[d.seq].prsnl_name = pr.name_full_formatted, vaccine->list[d.seq].acct_num = trim(ea
    .alias)
  WITH nocounter, outerjoin = d2, expand = 1
 ;end select
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(size(vaccine->list,5))),
   order_detail od
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_EKS_VACCINES_V3_TEST"
