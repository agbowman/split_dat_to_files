CREATE PROGRAM dm2_dm_flags_local_load:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ddfll_load_local(null) = i2
 DECLARE ddfll_load_local_from_plan(dllfp_plan_id=i4) = i2
 DECLARE ddfll_local_data(dlrc_data_ind=i2(ref)) = i2
 DECLARE ddfll_gen_dm_flags_con_str(null) = vc
 DECLARE ddfll_main(null) = i2
 DECLARE ddfll_log_prefix = vc WITH protect, constant("dm_flags_local_load")
 DECLARE ddfll_admin_db_link = vc WITH protect, noconstant("")
 DECLARE ddfll_fail_ind = i2 WITH protect, noconstant(0)
 DECLARE ddfll_plan_id = i4 WITH protect, constant( $1)
 IF (check_logfile(ddfll_log_prefix,".log","Log for ") != 1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting admin link."
 IF (dir_get_admin_db_link(0,ddfll_admin_db_link,ddfll_fail_ind)=0)
  GO TO exit_program
 ENDIF
 IF (ddfll_main(null)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Dm2_dm_flags_local_load complete"
 GO TO exit_program
 SUBROUTINE ddfll_load_local(null)
   SET dm_err->eproc = "Deleting data into dm_flags_local."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   DELETE  FROM dm_flags_local
    WHERE 1=1
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Loading data into dm_flags_local from cdba.admin_flags."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   INSERT  FROM dm_flags_local
    (column_name, table_name, flag_value,
    description, definition, updt_applctx,
    updt_dt_tm, updt_cnt, updt_id,
    updt_task, owner)(SELECT
     column_name, table_name, flag_value,
     description, definition, updt_applctx,
     updt_dt_tm, updt_cnt, updt_id,
     updt_task, owner
     FROM (value(ddfll_gen_dm_flags_con_str("cdba.dm_flags"))))
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddfll_load_local_from_plan(dllfp_plan_id)
   SET dm_err->eproc = concat("Deleting data into dm_flags_local for tables on plan ",cnvtstring(
     dllfp_plan_id))
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   DELETE  FROM dm_flags_local dfl
    WHERE dfl.owner=currdbuser
     AND dfl.table_name IN (
    (SELECT
     dat.table_name
     FROM dm_afd_tables dat
     WHERE dat.owner=currdbuser
      AND (dat.alpha_feature_nbr=- ((1 * dllfp_plan_id)))))
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   SET dm_err->eproc = concat(
    "Loading data into dm_flags_local from dm_flags_all for tables on plan ",cnvtstring(dllfp_plan_id
     ))
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   INSERT  FROM dm_flags_local
    ...
    updt_task, owner)(SELECT
     ...
     updt_dt_tm, updt_cnt, updt_id,
     updt_task, owner
     FROM (value(ddfll_gen_dm_flags_con_str("cdba.dm_flags")) dfa)
     WHERE dfa.owner=currdbuser
      AND dfa.table_name IN (
     (SELECT
      dat.table_name
      FROM dm_afd_tables dat
      WHERE dat.owner=currdbuser
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddfll_local_data(dlrc_data_ind)
   SET dm_err->eproc = "Getting row count of dm_flags_local"
   SET dlrc_data_ind = 0
   SELECT INTO "nl:"
    FROM dm_flags_local d
    WITH nocounter, maxqual(d,1)
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddfll_gen_dm_flags_con_str(null)
   DECLARE dgcs_con_str = vc WITH noconstant("")
   SET dgcs_con_str = concat("cdba.dm_flags@",ddfll_admin_db_link)
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = concat("Generated connect string ",dgcs_con_str)
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   RETURN(dgcs_con_str)
 END ;Subroutine
 SUBROUTINE ddfll_main(null)
   DECLARE ddfll_local_exist_ind = i2 WITH protect, noconstant(0)
   DECLARE ddfll_local_data_ind = i2 WITH protect, noconstant(0)
   IF (dm2_table_and_ccldef_exists("DM_FLAGS_LOCAL",ddfll_local_exist_ind)=0)
    RETURN(0)
   ELSEIF (ddfll_local_exist_ind=0)
    SET dm_err->eproc = "Checking existence of dm_flags_local table"
    SET dm_err->emsg = "DM_FLAGS_LOCAL does not exist"
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (ddfll_local_data(ddfll_local_data_ind)=0)
    RETURN(0)
   ENDIF
   IF (((ddfll_local_data_ind=0) OR (ddfll_plan_id=0)) )
    IF (ddfll_load_local(null)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (ddfll_load_local_from_plan(ddfll_plan_id)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (dsl_create_dm_flags_objs(null)=0)
 ...
 END ;Subroutine
#exit_program
 CALL final_disp_msg(ddfll_log_prefix)
END GO
