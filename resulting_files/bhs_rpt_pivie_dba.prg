CREATE PROGRAM bhs_rpt_pivie:dba
 PROMPT
 ...
 DECLARE mf_cs72_insstagingadultsonly = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "PIVIEINSSTAGINGADULTSONLY"))
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   (ce.result_val) IN ("*PAINFUL*"))) )) )
    ml_idx2 = locateval(ml_idx3,1,size(m_rec->qual,5),ce.encntr_id,m_rec->qual[ml_idx3].f_encntr_id)
    IF (ml_idx2=0)
     ml_cnt += 1
     ...
     m_rec->qual[ml_cnt].s_qual_result = trim(cnvtupper(ce.result_val),3)
    ENDIF
   ENDIF
  FOOT REPORT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ce
   ...
   mf_infusatecolor_cd, mf_medication_cd, mf_dressingapplied_cd, mf_cs72_insstagingadultsonly,
   mf_skinother_cd)
    AND ce.valid_until_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00")
    AND ce.publish_flag=1)
   JOIN (p
  ...
  HEAD ce.encntr_id
   ...
   m_rec->qual[ml_idx].s_pat_name = trim(p.name_full_formatted,3), m_rec->qual[ml_idx].f_person_id =
   p.person_id,
   m_rec->qual[ml_idx].s_fin = trim(ea.alias,3)
  HEAD ce.ce_dynamic_label_id
   ml_cnt2 = (size(m_rec->qual[ml_idx].grouper,5)+ 1),
   CALL alterlist(m_rec->qual[ml_idx].grouper,ml_cnt2), ms_tmp_dt = trim(format(ce.event_end_dt_tm,
     "MMDDYYYY HH:mm;;q"),3)
  HEAD ce.event_id
   CASE (ce.event_cd)
    ...
    OF mf_cs72_insstagingadultsonly:
     m_rec->qual[ml_idx].grouper[ml_cnt2].s_pivie_ins_staging = trim(ce.result_val,3)
   ENDCASE
 ...
 SELECT INTO "nl:"
  ...
  ENDIF
  FROM (dummyt d1  WITH seq = size(m_rec->qual,5)),
   person_info pi
  PLAN (d1)
   JOIN (pi
   WHERE (pi.person_id=m_rec->qual[d1.seq].f_person_id)
    AND pi.active_ind=1
  ...
  DETAIL
   IF (textlen(trim(m_rec->qual[d1.seq].s_race,3))=0)
    m_rec->qual[d1.seq].s_race = trim(uar_get_code_display(pi.value_cd),3)
   ELSEIF (pi.value_cd > 0.0)
    m_rec->qual[d1.seq].s_race = concat(m_rec->qual[d1.seq].s_race,", ",trim(uar_get_code_display(pi
       .value_cd),3))
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (trim(bd.description,3)="ethnicity 1")
    m_rec->qual[d1.seq].s_ethnicity = trim(uar_get_code_display(bd.code_value),3)
   ELSEIF (trim(bd.description,3)="ethnicity 2")
    IF (textlen(trim(m_rec->qual[d1.seq].s_ethnicity,3))=0)
     m_rec->qual[d1.seq].s_ethnicity = trim(uar_get_code_display(bd.code_value),3)
    ELSE
     m_rec->qual[d1.seq].s_ethnicity = concat(m_rec->qual[d1.seq].s_ethnicity,", ",trim(
       uar_get_code_display(bd.code_value),3))
    ENDIF
   ELSEIF (trim(bd.description,3)="hispanic ind")
    m_rec->qual[d1.seq].s_hispanic_ind = trim(bd.display,3)
   ENDIF
  WITH nocounter
 ;end select
 IF (((mn_ops=1) OR (textlen(trim( $S_RECIPIENTS,3)) > 1)) )
  ...
  SET frec->file_buf = build('"PATIENT NAME",','"ACCOUNT NUMBER",','"FACILITY",',
   ...
   '"PIVIE SEVERITY",',
   '"PIVIE NAME OF INFUSATE",','"PIVIE INFUSATE COLOR",','"PIVIE MEDICATION",',
   '"PIVIE DRESSING APPLIED",','"PIVIE INS STAGING",',
   '"ETHNICITY",','"RACE",','"HISPANIC",',char(13))
  SET stat = cclio("WRITE",frec)
  FOR (ml_cnt = 1 TO m_rec->l_cnt)
    FOR (ml_cnt2 = 1 TO size(m_rec->qual[ml_cnt].grouper,5))
      IF (((size(trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].s_pivie_trained_rev,3)) > 0) OR (((size(
       trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].s_pivie_skin_color,3)) > 0) OR (((size(trim(m_rec->
        qual[ml_cnt].grouper[ml_cnt2].s_pivie_skin_other,3)) > 0) OR (((size(trim(m_rec->qual[ml_cnt]
        .grouper[ml_cnt2].s_pivie_blisters,3)) > 0) OR (((size(trim(m_rec->qual[ml_cnt].grouper[
        ml_cnt2].s_pivie_distal_pulse,3)) > 0) OR (((size(trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].
        s_pivie_swelling,3)) > 0) OR (((size(trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].
        s_pivie_severity,3)) > 0) OR (((size(trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].
        s_pivie_infusate_name,3)) > 0) OR (((size(trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].
        s_pivie_infusate_color,3)) > 0) OR (size(trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].
        s_pivie_dress_applied,3)) > 0)) )) )) )) )) )) )) )) )) )
       IF (((cnvtupper(trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].s_periph_dc_reason,3)) IN (
       "*DISCONT*INFILTRATE*", "*DISCONT*PHLEBITIS*", "*LEAKING*")) OR (((cnvtupper(trim(m_rec->qual[
         ml_cnt].grouper[ml_cnt2].s_periph_dc_reason,3)) IN ("*COOL*TO*TOUCH*", "*EDEMATOUS*",
       "*ERYTHEMA*")) OR (cnvtupper(trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].s_periph_dc_reason,3))
        IN ("*PAINFUL*"))) )) )
        SET frec->file_buf = build('"',trim(m_rec->qual[ml_cnt].s_pat_name,3),'","',trim(m_rec->qual[
          ml_cnt].s_fin,3),'","',
         trim(m_rec->qual[ml_cnt].s_facility,3),'","',trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].
          s_dc_loc,3),'","',trim(m_rec->qual[ml_cnt].s_reg_dt,3),
         '","',trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].s_periph_dc_reason,3),'","',trim(m_rec->
          qual[ml_cnt].grouper[ml_cnt2].s_periph_line_insert_dt,3),'","',
         trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].s_periph_line_removal_dt,3),'","',trim(m_rec->
          qual[ml_cnt].grouper[ml_cnt2].s_pivie_trained_rev,3),'","',trim(m_rec->qual[ml_cnt].
          grouper[ml_cnt2].s_pivie_skin_color,3),
         '","',trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].s_pivie_skin_other,3),'","',trim(m_rec->
          qual[ml_cnt].grouper[ml_cnt2].s_pivie_blisters,3),'","',
         trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].s_pivie_distal_pulse,3),'","',trim(m_rec->qual[
          ml_cnt].grouper[ml_cnt2].s_pivie_swelling,3),'","',trim(m_rec->qual[ml_cnt].grouper[ml_cnt2
          ].s_pivie_severity,3),
         ...
         trim(m_rec->qual[ml_cnt].grouper[ml_cnt2].s_pivie_medication,3),'","',trim(m_rec->qual[
          ml_cnt].grouper[ml_cnt2].s_pivie_dress_applied,3),'","',trim(m_rec->qual[ml_cnt].grouper[
          ml_cnt2].s_pivie_ins_staging,3),
         '","',trim(m_rec->qual[ml_cnt].s_ethnicity,3),'","',trim(m_rec->qual[ml_cnt].s_race,3),'","',
         trim(m_rec->qual[ml_cnt].s_hispanic_ind,3),'"',char(13))
        SET stat = cclio("WRITE",frec)
       ENDIF
      ENDIF
    ENDFOR
 ...
 ELSE
  SELECT INTO value( $OUTDEV)
   ...
   peripheral_iv_dc_reason = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_periph_dc_reason),
   peripheral_line_insertion_dt = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].
    s_periph_line_insert_dt), peripheral_line_discontinue_dt = substring(1,250,m_rec->qual[d.seq].
    grouper[d2.seq].s_periph_line_removal_dt), pivie_trained_reviewer_contacted = substring(1,250,
    m_rec->qual[d.seq].grouper[d2.seq].s_pivie_trained_rev),
   pivie_skin_color = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_skin_color),
   pivie_skin_other = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_skin_other),
   pivie_blisters = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_blisters),
   pivie_distal_pulse = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_distal_pulse),
   pivie_swelling = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_swelling),
   pivie_severity = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_severity),
   pivie_name_of_infusate = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_infusate_name),
   pivie_infusate_color = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_infusate_color),
   pivie_medication = substring(1,50,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_medication),
   pivie_dressing_applied = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_dress_applied),
   pivie_ins_staging = substring(1,250,m_rec->qual[d.seq].grouper[d2.seq].s_pivie_ins_staging),
   ethnicity = substring(1,250,m_rec->qual[d.seq].s_ethnicity),
   race = substring(1,250,m_rec->qual[d.seq].s_race), hispanic = substring(1,250,m_rec->qual[d.seq].
    s_hispanic_ind)
   ...
   PLAN (d
    ...
    JOIN (d2
    WHERE ((size(trim(m_rec->qual[d.seq].grouper[d2.seq].s_pivie_trained_rev,3)) > 0) OR (((size(trim
     (m_rec->qual[d.seq].grouper[d2.seq].s_pivie_skin_color,3)) > 0) OR (((size(trim(m_rec->qual[d
      .seq].grouper[d2.seq].s_pivie_skin_other,3)) > 0) OR (((size(trim(m_rec->qual[d.seq].grouper[d2
      .seq].s_pivie_blisters,3)) > 0) OR (((size(trim(m_rec->qual[d.seq].grouper[d2.seq].
      s_pivie_distal_pulse,3)) > 0) OR (((size(trim(m_rec->qual[d.seq].grouper[d2.seq].
      s_pivie_swelling,3)) > 0) OR (((size(trim(m_rec->qual[d.seq].grouper[d2.seq].s_pivie_severity,3
      )) > 0) OR (((size(trim(m_rec->qual[d.seq].grouper[d2.seq].s_pivie_infusate_name,3)) > 0) OR (
    ((size(trim(m_rec->qual[d.seq].grouper[d2.seq].s_pivie_infusate_color,3)) > 0) OR (size(trim(
      m_rec->qual[d.seq].grouper[d2.seq].s_pivie_dress_applied,3)) > 0)) )) )) )) )) )) )) )) ))
     AND ((cnvtupper(trim(m_rec->qual[d.seq].grouper[d2.seq].s_periph_dc_reason,3)) IN (
    "*DISCONT*INFILTRATE*", "*DISCONT*PHLEBITIS*", "*LEAKING*")) OR (((cnvtupper(trim(m_rec->qual[d
      .seq].grouper[d2.seq].s_periph_dc_reason,3)) IN ("*COOL*TO*TOUCH*", "*EDEMATOUS*", "*ERYTHEMA*"
    )) OR (cnvtupper(trim(m_rec->qual[d.seq].grouper[d2.seq].s_periph_dc_reason,3)) IN ("*PAINFUL*")
    )) )) )
