CREATE PROGRAM bhs_afc_purge_activity:dba
 ...
 SET cnt1 = 1
 SET stat = uar_get_meaning_by_codeset(code_set,nullterm(cdf_meaning),1,completed_cd)
 ...
 SELECT INTO "nl:"
  ...
  WHERE ce.updt_dt_tm < cnvtdatetime(from_date)
   AND  NOT ( EXISTS (
  (SELECT
   c.charge_item_id
   FROM charge c
 ...
 SELECT INTO "nl:"
  ...
  WHERE ceap.updt_dt_tm < cnvtdatetime(from_date)
  ...
  DETAIL
   ...
   .charge_event_act_id
  WITH nocounter, maxqual(ceap,50000)
 ;end select
 ...
 SELECT INTO "nl:"
  FROM charge_event_act cea
  WHERE cea.updt_dt_tm < cnvtdatetime(from_date)
   AND  NOT ( EXISTS (
  (SELECT
   ce.charge_event_id
   FROM charge_event ce
  ...
  DETAIL
   ...
   .charge_event_act_id
  WITH counter, maxqual(cea,50000)
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  WHERE cem.updt_dt_tm < cnvtdatetime(from_date)
   AND  NOT ( EXISTS (
  (SELECT
   ce.charge_event_id
   FROM charge_event ce
  ...
  DETAIL
   ...
   .charge_event_mod_id
  WITH counter, maxqual(cem,50000)
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   .charge_event_id
  WITH nocounter, maxqual(ce,50000)
 ;end select
 SET total_ce_rows_found = (total_ce_rows_found+ curqual)
