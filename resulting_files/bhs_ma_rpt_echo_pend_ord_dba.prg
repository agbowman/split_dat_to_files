CREATE PROGRAM bhs_ma_rpt_echo_pend_ord:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   m_rec->qual[m_rec->l_cnt].s_pat_dob = trim(format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p
       .birth_tz),1),"MM/DD/YYYY;;q"),3), m_rec->qual[m_rec->l_cnt].s_fin = trim(ea1.alias,3), m_rec
   ->qual[m_rec->l_cnt].s_mrn = trim(ea2.alias,3),
 ...
 IF (value( $OUTDEV)="OPS")
  ...
  DECLARE ms_email = vc WITH protect, constant(
   "angelce.lazovski@bhs.org, echopendingorders@baystatehealth.org")
