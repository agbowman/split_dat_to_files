CREATE PROGRAM bbd_rpt_registered_not_drawn:dba
 RECORD reply(
   1 report_name_list[*]
     2 report_name = vc
   1 status_data
 ...
 SET captions->rpt_title_1 = uar_i18ngetmessage(i18nhandle,"rpt_title_1",
  "D O N A T I O N S   R E G I S T E R E D   B U T   N O T")
 SET captions->rpt_time = uar_i18ngetmessage(i18nhandle,"rpt_time","Time:")
 SET captions->rpt_title_2 = uar_i18ngetmessage(i18nhandle,"rpt_title_2","D R A W N   R E P O R T")
 ...
 SET captions->rpt_id = uar_i18ngetmessage(i18nhandle,"rpt_id",
  "Report ID: BBD_RPT_REGISTERED_NOT_DRAWN")
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  DETAIL
   canceled_cd = c.code_value
 ;end select
 SET cur_username = get_username(reqinfo->updt_id)
 SELECT DISTINCT INTO "CER_TEMP:regnotdrawn.txt"
  ...
  PLAN (dc
   WHERE dc.active_ind=1
    ...
    AND ((dc.contact_status_cd=canceled_cd) OR (dc.contact_status_cd=pending_cd)) )
   JOIN (pe
  ...
  HEAD PAGE
   ...
   CALL center(captions->rpt_title_1,1,125),
   ...
   curdate"@DATECONDENSED;;d", row + 3, col 11,
   captions->date, col 29, captions->time,
   col 48, captions->procedure, col 73,
   captions->donor_name, col 97, captions->donor_number,
   row + 1, col 7, "-------------",
   col 25, "-------------", col 43,
   "--------------------", col 68, "--------------------",
   col 93, "--------------------", row + 1
 ...
 IF (report_complete_ind="Y")
  ...
  SET reply->status_data.subeventstatus[count1].targetobjectname = "bbd_rpt_registered_not_drawn"
 ...
 SET reply->report_name_list[1].report_name = "CER_TEMP:regnotdrawn.txt"
 GO TO exit_script
