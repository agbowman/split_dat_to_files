CREATE PROGRAM dm_ecmb_encntr_data_not_coll:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "ENCNTR_DATA_NOT_COLL"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_ECMB_ENCNTR_DATA_NOT_COLL"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count_from].from_id = frm.encntr_data_not_coll_id, rreclist->from_rec[
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_to_fk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =
  i4)
   UPDATE  FROM encntr_data_not_coll frm
    ...
    WHERE frm.encntr_data_not_coll_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "ENCNTR_DATA_NOT_COLL"
   ...
   IF (add_hist(s_df_pk_id)=0)
    ...
    SET request->error_message = substring(1,132,build(
      "Could not insert history for encntr_data_not_coll with pk=",s_df_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM encntr_data_not_coll frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.encntr_data_not_coll_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
 ...
 SUBROUTINE (add_hist(s_at_to_parent_pk_id=f8) =i4)
   ...
   SET dm_cmb_cust_cols2->add_col_val[1].col_name = "ENCNTR_DATA_NOT_COLL_HIST_ID"
   ...
   SET dm_cmb_cust_cols2->where_col_val[1].col_name = "ENCNTR_DATA_NOT_COLL_ID"
   SET dm_cmb_cust_cols2->where_col_val[1].col_value = build(s_at_to_parent_pk_id)
   IF (size(dm_cmb_cust_cols2->col,5)=0)
    SET dm_cmb_cust_cols2->tbl_name = "ENCNTR_DATA_NOT_COLL_HIST"
    SET dm_cmb_cust_cols2->sub_select_from_tbl = "ENCNTR_DATA_NOT_COLL"
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0)
    ...
    SET dcipht_request->transaction_reason_txt = "DM_ECMB_ENCNTR_DATA_NOT_COLL"
