CREATE PROGRAM dm_pucb_ce_io_total_result:dba
 ...
 DECLARE ucb_upd_person_id(null) = i2
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PUCB_CE_IO_TOTAL_RESULT"
 ...
 ELSE
  IF ((rchildren->qual1[det_cnt].combine_action_cd=upt))
   SET breturn = ucb_upd_person_id(null)
   IF (breturn != true)
    GO TO exit_sub
   ENDIF
  ...
  ENDIF
 ENDIF
 SUBROUTINE ucb_upd_person_id(null)
   UPDATE  FROM ce_io_total_result iotr
    SET iotr.person_id = request->xxx_uncombine[ucb_cnt].to_xxx_id, iotr.updt_dt_tm = cnvtdatetime(
   ...
   IF (error(ucberrmsg,0) != 0)
    ...
    SET request->error_message = concat(build("Error updating ce_io_total_result_id (",rchildren->
      qual1[det_cnt].entity_id,") to person_id (",request->xxx_uncombine[ucb_cnt].to_xxx_id,")"))
    RETURN(false)
   ENDIF
   RETURN(true)
 END ;Subroutine
 SUBROUTINE ucb_perform_post_processing(null)
   ...
   DECLARE nextdet = i4
   DECLARE retval = i2
   SET detcnt = size(rchildren->qual1[det_cnt],5)
 ...
 SUBROUTINE ucb_upd_mark_suspects(null)
   ...
   UPDATE  FROM ce_io_total_result iotr
    SET iotr.suspect_flag = 2, iotr.updt_dt_tm = cnvtdatetime(sysdate), iotr.updt_applctx = reqinfo->
     ...
     iotr.updt_id = reqinfo->updt_id, iotr.updt_cnt = (iotr.updt_cnt+ 1), iotr.updt_task = reqinfo->
     updt_task
    WHERE iotr.person_id IN (request->xxx_uncombine[ucb_cnt].to_xxx_id, request->xxx_uncombine[
    ucb_cnt].from_xxx_id)
