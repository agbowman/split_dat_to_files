CREATE PROGRAM dm_ccb_cvf_cvo_filter:dba
 ...
 IF ((request->code_set < 1))
  ...
  GO TO exit_program
 ENDIF
 IF ((request->filterlist[1].column_value="<sp>"))
  SET request->filterlist[1].column_value = notrim(" ")
 ENDIF
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (cvo
   WHERE (cvo.code_set=request->code_set)
    AND parser(filterstring_builder(dccd_filterstring,"cvo",request)))
   JOIN (cv
   WHERE cv.code_value=cvo.code_value)
  ORDER BY cvo.code_set
  HEAD REPORT
   cv_cnt = 0
  DETAIL
   cv_cnt += 1
   ...
   ENDIF
   reply->cd_value_list[cv_cnt].code_value = cvo.code_value, reply->cd_value_list[cv_cnt].display =
   cv.display, reply->cd_value_list[cv_cnt].cdf_meaning = cv.cdf_meaning
  FOOT REPORT
 ...
 IF (errcode != 0)
  SET errmsg = concat("Unable to read from CODE_VALUE_OUTBOUND while searching on ALIAS:",errmsg)
  SET status = "F"
  GO TO exit_program
 ENDIF
#exit_program
