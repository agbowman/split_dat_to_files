CREATE PROGRAM acm_act_person_prsnl_reltn:dba
 ...
 FOR (index = 1 TO xref->act_cnt)
   SET reply->person_prsnl_reltn_qual[xref->act[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->act_cnt)),
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.person_prsnl_reltn_id=acm_request->person_prsnl_reltn_qual[xref->act[d.seq].idx].
   person_prsnl_reltn_id))
  WITH nocounter, status(reply->person_prsnl_reltn_qual[xref->act[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->act_cnt)
   IF ((reply->person_prsnl_reltn_qual[xref->act[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,p.person_prsnl_reltn_id,acm_request->person_prsnl_reltn_qual[xref->
     act[t1].idx].person_prsnl_reltn_id,
     max_val)
    DETAIL
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->person_prsnl_reltn_qual[xref->act[index].idx].status=0))
