CREATE PROGRAM bed_get_oc_multi_match:dba
 FREE SET reply
 RECORD reply(
   1 possible_match[*]
     2 primary_name = c100
 ...
 FOR (i = 1 TO tot_nbr_client_orderables)
   IF ((client_orderable_list->orderable[i].match_ind > 0))
    SELECT INTO "NL:"
     FROM (dummyt d  WITH seq = size(client_orderable_list->orderable[i].clist)),
      order_catalog orc
     PLAN (d
     ...
     DETAIL
      ...
      IF (already_matched=0)
       ...
       possible_match[tot_poss_match].primary_name = orc.primary_mnemonic, temp_reply->
       possible_match[tot_poss_match].description = orc.description,
       temp_reply->possible_match[tot_poss_match].match_value = trim(client_orderable_list->
        orderable[i].cpt4_code), temp_reply->possible_match[tot_poss_match].match_type = 3,
       temp_reply->possible_match[tot_poss_match].concept_cki = orc.concept_cki
      ENDIF
     WITH nocounter
    ;end select
   ENDIF
 ...
 IF (tot_poss_match > 0)
  SELECT INTO "NL"
   ...
   DETAIL
    ...
    possible_match[d.seq].match_type = temp_reply->possible_match[d.seq].match_type
   WITH nocounter
  ;end select
 ENDIF
 IF (tot_poss_match > 0)
