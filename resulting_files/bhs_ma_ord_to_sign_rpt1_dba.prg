CREATE PROGRAM bhs_ma_ord_to_sign_rpt1:dba
 PROMPT
  "PRINTER " = "MINE"
 RECORD dlrec(
   1 ord_total = i4
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (seq_flag=0)
    dlrec->seq[d.seq].order_comment = replace(l.long_text,char(13)," ",0), seq_flag = 1
   ENDIF
  FOOT  oc.order_id
   col + 0
  WITH nocounter
 ...
 SELECT INTO  $1
  ...
  HEAD PAGE
   ...
   "{b/5}Order",
   CALL print(calcpos(xcol6,yrow1)), "{b/4}CMRN",
   ...
   CALL print(calcpos(xcol13,yrow1)),
   "{b/7}Reject?", yrow1 = (yrow1+ 4),
   CALL print(calcpos(xcol1,yrow1)),
   line1, rowplusone2,
   CALL print(calcpos(xcol1,yrow1)),
   "{B}ORDERS TO BE SIGNED BY: ", dlrec->seq[c].sign_doctor, "{endb}",
  ...
  DETAIL
   FOR (c = 1 TO dlrec->ord_total)
     ...
     CALL print(calcpos(xcol1,yrow1)), " ",
     dlrec->seq[c].pt_name,
     CALL print(calcpos(xcol6,yrow1)), dlrec->seq[c].cmrn,
     ...
     CALL print(calcpos(xcol12,yrow1)), dlrec->seq[c].encntr_type,
     CALL print(calcpos(xcol13,yrow1)),
     "________", rowplusone,
     CALL print(calcpos(xcol2,yrow1)),
     "Order Entered By: ", dlrec->seq[c].order_person, rowplusone,
     CALL print(calcpos(xcol2,yrow1)), "Order: ", dlrec->seq[c].order_id"########",
     CALL print(calcpos(xcol4,yrow1)), dlrec->seq[c].order_mnemonic, rowplusone,
     CALL print(calcpos(xcol2,yrow1)), "Order Detail: ", xcolvar = xcol3,
     maxlen = 110, tempstring = dlrec->seq[c].clinical_display_line, line_wrap
     ...
     IF ((dlrec->seq[c].break_flag=1))
      ...
      IF (((yrow1+ 20) > 730))
       yrow1 = 5, BREAK
      ENDIF
      CALL print(calcpos(xcol4,yrow1)), "{b}Sign all orders not refused:",
      "_____________________________________________",
      rowplusone,
      CALL print(calcpos(xcol7,yrow1)), dlrec->seq[c].sign_doctor,
      row + 1, yrow1 = 5, BREAK
     ENDIF
   ENDFOR
  FOOT PAGE
