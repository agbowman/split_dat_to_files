CREATE PROGRAM cp_add_concept_group_dtl_attr
 IF (validate(request) != 1)
  RECORD request(
    1 attr_list[*]
      2 cp_concept_group_dtl_id = f8
      2 attribute_entity_id = f8
      2 attribute_entity_name = vc
      2 attribute_entity_text = vc
      2 detail_attribute_type_cd = f8
  )
 ENDIF
 RECORD reply(
   1 attr_list[*]
     2 cp_concept_group_dtl_attr_id = f8
     2 cp_concept_group_dtl_id = f8
     2 attribute_entity_id = f8
     2 attribute_entity_name = vc
     2 attribute_entity_text = vc
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE attrlistsize = i4 WITH noconstant(size(request->attr_list,5)), protect
   DECLARE detail_attr_type_codeset = i4 WITH constant(4003333), protect
   IF (attrlistsize <= 0)
    CALL reporterrorandexit("PERFORM","Validate","Detail list must be nonzero in size.")
   ENDIF
   FOR (x = 1 TO attrlistsize)
    CALL verifynonzerocodevalue(request->attr_list[x].detail_attribute_type_cd,
     detail_attr_type_codeset,"Detail Attribute Type")
    IF ((request->attr_list[x].attribute_entity_id <= 0)
     AND (request->attr_list[x].attribute_entity_name="")
     AND (request->attr_list[x].attribute_entity_text=""))
     CALL reporterrorandexit("PERFORM","Validate","entity Fields are all empty")
    ENDIF
   ENDFOR
 ...
 SUBROUTINE insertrows(null)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE x = i4 WITH noconstant(0), protect
   DECLARE cp_concept_group_dtl_attr_id = f8 WITH noconstant(0), protect
   IF (checkdic("CP_CONCEPT_GROUP_DTL_ATTR","T",0) != 2)
    CALL reporterrorandexit("PERFORM","INSERT",
     " CP_CONCEPT_GROUP_DTL_ATTR does not exist or permission is not granted to access it.")
   ENDIF
   SET stat = alterlist(reply->attr_list,size(request->attr_list,5))
   FOR (x = 1 TO size(request->attr_list,5))
     SET cp_concept_group_dtl_attr_id = 0.0
     SELECT INTO "nl:"
      nextseqnum = seq(reference_seq,nextval)
      FROM dual
      DETAIL
       cp_concept_group_dtl_attr_id = nextseqnum
      WITH nocounter
     ;end select
     CALL echo(build2("Next CP_CONCEPT_GROUP_DTL_ATTR_ID:",cp_concept_group_dtl_attr_id))
     IF (cp_concept_group_dtl_attr_id=0.0)
      CALL reporterror("PERFORM","INSERT",
       " Unable to generate cp_concept_group_dtl_attr_id from REFERENCE_SEQ.")
      ROLLBACK
      GO TO exit_script
     ENDIF
     INSERT  FROM cp_concept_group_dtl_attr ccgda
      SET ccgda.cp_concept_group_dtl_id = request->attr_list[x].cp_concept_group_dtl_id, ccgda
       .cp_concept_group_dtl_attr_id = cp_concept_group_dtl_attr_id, ccgda.attribute_entity_id =
       request->attr_list[x].attribute_entity_id,
       ccgda.attribute_entity_name = request->attr_list[x].attribute_entity_name, ccgda
       .attribute_entity_text = request->attr_list[x].attribute_entity_text, ccgda
       .detail_attribute_type_cd = request->attr_list[x].detail_attribute_type_cd
      WITH nocounter
     ;end insert
     IF (curqual=0)
      CALL reporterror("PERFORM","INSERT",
       "Unable to insert row into cp_concept_group_dtl_attr table.")
      ROLLBACK
      GO TO exit_script
     ELSE
      SET reply->attr_list[x].cp_concept_group_dtl_id = request->attr_list[x].cp_concept_group_dtl_id
      SET reply->attr_list[x].cp_concept_group_dtl_attr_id = cp_concept_group_dtl_attr_id
      SET reply->attr_list[x].attribute_entity_id = request->attr_list[x].attribute_entity_id
      SET reply->attr_list[x].attribute_entity_name = request->attr_list[x].attribute_entity_name
      SET reply->attr_list[x].attribute_entity_text = request->attr_list[x].attribute_entity_text
      SET reply->attr_list[x].detail_attribute_type_cd = request->attr_list[x].
