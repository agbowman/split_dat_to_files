CREATE PROGRAM dm2_arc_restore_delete:dba
 ...
 SUBROUTINE update_time_window(null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE v_stmt_rest_del = vc
 CALL parser("rdb ",0)
 FOR (cnt4 = 1 TO request->temp_size)
   SET v_stmt_rest_del = arc_replace(pers_arc->statements[request->s_ndx].restore_delete[cnt4].stmt,
    pers_arc->statements[request->s_ndx].restore_delete[cnt4].link_ind,pers_arc->statements[request->
    s_ndx].restore_delete[cnt4].list_ind,pers_arc->statements[request->s_ndx].restore_delete[cnt4].
   ...
   IF ((cnt4=request->temp_size))
    CALL parser(v_stmt_rest_del,1)
   ELSE
    CALL parser(v_stmt_rest_del,0)
   ENDIF
   IF ((debug_mover->debug_level > 1))
    CALL echo(v_stmt_rest_del)
   ENDIF
 ENDFOR
