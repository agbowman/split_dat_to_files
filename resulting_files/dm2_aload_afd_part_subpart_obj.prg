CREATE PROGRAM dm2_aload_afd_part_subpart_obj
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_afd_part_subpart_obj",".log","dm2_aload_afd_part_subpart_obj")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_afd_part_subpart_obj"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daapso_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daapso_file = vc WITH protect, noconstant( $2)
 DECLARE daapso_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daapso_batch = i4 WITH protect, noconstant(0)
 DECLARE daapso_rows_returned = i4 WITH protect, noconstant(1)
 DECLARE daapso_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daapso_pkg,daapso_file_loc_ret,daapso_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daapso_batch=0)
  SET daapso_batch = 5000
 ENDIF
 IF (daapso_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daapso_file_loc_ret,daapso_file), "dm2_aload_sch_part_subpart_obj",
  daapso_batch
  CALL echo(concat(daapso_file_loc_ret,daapso_file))
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Remove extraneous DM_AFD_PART_SUBPART_OBJECTS rows"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dm_afd_part_subpart_objects dapso
   WHERE dapso.alpha_feature_nbr=daapso_pkg
    AND dapso.owner=currdbuser
    AND  NOT ( EXISTS (
   (SELECT
    "x"
    FROM dm_afd_tables dat
    WHERE dat.alpha_feature_nbr=dapso.alpha_feature_nbr
     AND dat.owner=dapso.owner
     AND dat.table_name=dapso.table_name
     AND dat.updt_dt_tm=dapso.updt_dt_tm)))
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_afd_part_subpart_obj completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_afd_part_subpart_obj")
END GO
