CREATE PROGRAM dm_pcmb_consent_ptsync_audit:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "CONSENT_PATIENTSYNC_AUDIT"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_CONSENT_PTSYNC_AUDIT"
 ...
 SELECT INTO "nl:"
  frm.*
  FROM consent_patientsync_audit frm
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = frm.consent_patientsync_audit_id, rreclist->from_rec[
   v_cust_count1].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count1].active_status_cd =
   frm.active_status_cd,
   rreclist->from_rec[v_cust_count1].organization_id = frm.organization_id, rreclist->from_rec[
   v_cust_count1].consent_type_flag = frm.consent_type_flag
  WITH forupdatewait(frm)
 ;end select
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   tu.*
   FROM consent_patientsync_audit tu
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count2].to_id = tu.consent_patientsync_audit_id, rreclist->to_rec[
    v_cust_count2].active_ind = tu.active_ind, rreclist->to_rec[v_cust_count2].active_status_cd = tu
    .active_status_cd,
    rreclist->to_rec[v_cust_count2].organization_id = tu.organization_id, rreclist->to_rec[
    v_cust_count2].consent_type_flag = tu.consent_type_flag
   WITH forupdatewait(tu)
  ;end select
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
    SET to_id_present = 0
    FOR (v_cust_loopcount2 = 1 TO v_cust_count2)
      IF ((rreclist->from_rec[v_cust_loopcount].organization_id=rreclist->to_rec[v_cust_loopcount2].
      organization_id)
       AND (rreclist->from_rec[v_cust_loopcount].consent_type_flag=rreclist->to_rec[v_cust_loopcount2
      ].consent_type_flag))
       SET to_id_present = 1
      ENDIF
    ENDFOR
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_to_fk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =
  i4)
   UPDATE  FROM consent_patientsync_audit frm
    ...
    WHERE frm.consent_patientsync_audit_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "CONSENT_PATIENTSYNC_AUDIT"
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM consent_patientsync_audit frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.consent_patientsync_audit_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
