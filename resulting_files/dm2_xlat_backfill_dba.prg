CREATE PROGRAM dm2_xlat_backfill:dba
 ...
 SUBROUTINE drmmi_backfill_mock_id(dbmi_env_id)
   ...
   RETURN(dbmi_mock_id)
 END ;Subroutine
 DECLARE dbx_source_env_id = f8
 DECLARE dbx_env_ndx = i4
 DECLARE dbx_seq_ndx = i4
 DECLARE dbx_tbl_ndx = i4
 DECLARE dbx_find_tbl_ndx = i4
 DECLARE dguc_tbl_ndx = i4
 DECLARE t1_column_name = vc
 DECLARE t2_column_name = vc
 DECLARE t1_table_name = vc
 DECLARE t2_table_name = vc
 DECLARE ut_table_name = vc
 DECLARE pl_vdb_tab = vc
 DECLARE dbx_filter_col_loop = i2
 DECLARE filter_ndx = i4
 DECLARE uo_table_name = vc
 DECLARE dxb_check_backfill_progress(s_src_env_id=f8,s_tgt_env_id=f8) = i2
 FREE RECORD dbx_env_info
 RECORD dbx_seq_match_info(
   1 child_env_id = f8
   1 parent[*]
     2 parent_env_id = f8
     2 parent_db_link = vc
     ...
     2 sequence[*]
       3 sequence_name = vc
       3 sequence_match_value = f8
 )
 FREE RECORD dbx_tbls_to_backfill
 RECORD dbx_tbls_to_backfill(
   1 backfill_dt_tm = dq8
   1 tbl[*]
     2 tbl_name = vc
     2 column_name = vc
     2 sequence_name = vc
     2 tbl_name_exst_src_flg = i2
     2 tbl_suffix = vc
     2 filter_ind = i2
     2 filter_proc = vc
     2 filter_col_cnt = i4
     2 filter_col_where = vc
     2 filter_col[*]
       3 filter_col_name = vc
 )
 IF (validate(reply->status,"1")="1")
  FREE RECORD reply
  RECORD reply(
    1 status = c1
      2 status_msg = vc
 ...
 ENDIF
 IF (validate(dguc_request->what_tables,"-10")="-10")
 ...
 ENDIF
 IF (validate(dguc_reply->dguc_err_msg,"ZXY")="ZXY")
 ...
 SET dbx_seq_ndx = 0
 SET dbx_tbl_ndx = 0
 SET dbx_source_env_id =  $1
 SET dbx_tbls_to_backfill->backfill_dt_tm = cnvtdatetime(curdate,curtime3)
 IF (check_logfile("dm2_xlat_backfill",".log","dm2_xlat_backfill LOG")=0)
  SET dm_err->err_ind = 1
 ...
 SELECT INTO "NL:"
  ...
  PLAN (di
   WHERE di.info_name="DM_ENV_ID"
    AND di.info_domain="DATA MANAGEMENT")
   JOIN (de
   WHERE di.info_number=de.environment_id)
  DETAIL
   dbx_seq_match_info->child_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dbx_seq_match_info->child_env_id = drmmi_get_mock_id(dbx_seq_match_info->child_env_id)
 IF ((dbx_seq_match_info->child_env_id < 0))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dxb_cbp_var = dxb_check_backfill_progress(dbx_source_env_id,dbx_seq_match_info->child_env_id)
 IF (dxb_cbp_var=0)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ELSEIF (dxb_cbp_var=1)
  SET reply->status = "Z"
  SET reply->status_msg = "Backfill in process. Pausing 60 seconds before checking again"
  SET dm_err->eproc = reply->status_msg
  GO TO exit_program
 ELSEIF (dxb_cbp_var=3)
  SET reply->status = "S"
  SET reply->status_msg = "Backfill was successful. Starting merge process."
  SET dm_err->eproc = reply->status_msg
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting list of source environments"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "NL:"
  der.parent_env_id, der.post_link_name
  FROM dm_env_reltn der
  WHERE (der.child_env_id=dbx_seq_match_info->child_env_id)
   AND der.parent_env_id=dbx_source_env_id
   AND der.relationship_type="REFERENCE MERGE"
  DETAIL
   dbx_env_ndx = (dbx_env_ndx+ 1), stat = alterlist(dbx_seq_match_info->parent,dbx_env_ndx),
   dbx_seq_match_info->parent[dbx_env_ndx].parent_env_id = der.parent_env_id,
   dbx_seq_match_info->parent[dbx_env_ndx].parent_db_link = der.post_link_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No rows were found in DM_ENV_RELTN for this relationship."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSE
  FOR (dbx_env_ndx = 1 TO size(dbx_seq_match_info->parent,5))
    IF ((((dbx_seq_match_info->parent[dbx_env_ndx].parent_db_link < " ")) OR ((dbx_seq_match_info->
    parent[dbx_env_ndx].parent_db_link=null))) )
     SET dm_err->eproc = "Encountered an invalild source target relationship."
     CALL disp_msg(" ",dm_err->logfile,0)
    ELSE
     SET pl_vdb_tab = concat("V$DATABASE",dbx_seq_match_info->parent[dbx_env_ndx].parent_db_link)
     SET dm_err->eproc = "Testing merge link"
     CALL disp_msg(" ",dm_err->logfile,0)
     SELECT INTO "NL:"
      vdb.name
      FROM (parser(pl_vdb_tab) vdb)
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      SET dm_err->eproc = "Invalid merge link. Exiting backfill process."
      CALL disp_msg(" ",dm_err->logfile,0)
      SET reply->status = "F"
      SET reply->status_msg = "Error encountered during backfill progress check"
      SET dbx_seq_match_info->parent[dbx_env_ndx].valid_link = 0
      GO TO exit_program
     ELSE
      SET dbx_seq_match_info->parent[dbx_env_ndx].valid_link = 1
      SET dm_err->eproc = "Finding sequence match rows"
      CALL disp_msg(" ",dm_err->logfile,0)
      SELECT INTO "NL:"
       di.info_name, di.info_number
       FROM dm_info di
       WHERE di.info_domain=concat("MERGE",trim(cnvtstring(dbx_seq_match_info->parent[dbx_env_ndx].
          parent_env_id)),trim(cnvtstring(dbx_seq_match_info->child_env_id)),"SEQMATCH")
       DETAIL
        dbx_seq_ndx = (dbx_seq_ndx+ 1), stat = alterlist(dbx_seq_match_info->parent[dbx_env_ndx].
         sequence,dbx_seq_ndx), dbx_seq_match_info->parent[dbx_env_ndx].sequence[dbx_seq_ndx].
        sequence_name = di.info_name,
        dbx_seq_match_info->parent[dbx_env_ndx].sequence[dbx_seq_ndx].sequence_match_value = di
        .info_number
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
  ENDFOR
  SET dm_err->eproc = "Gathering column information on tables that need to be backfilled"
  CALL disp_msg(" ",dm_err->logfile,0)
  ...
  SET stat = alterlist(dguc_request->req_special,9)
  ...
  IF ((dguc_reply->dguc_err_ind=1))
   ...
   GO TO exit_program
  ENDIF
  IF (value(dguc_reply->rs_tbl_cnt)=0)
   SET dm_err->eproc = concat("No tables need to be backfilled - terminating process.")
   SET dm_err->err_ind = 1
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Filtering tables through dm_get_unique_columns"
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dbx_tbl_ndx = 0
  SELECT INTO "NL:"
   drcd.column_name, drcd.sequence_name
   FROM dm_rdds_col_doc drcd,
    dm_rdds_tbl_doc drtd
   PLAN (drcd)
    JOIN (drtd
    WHERE drtd.full_table_name=drcd.table_name
     AND expand(dguc_tbl_ndx,1,dguc_reply->rs_tbl_cnt,drtd.table_name,dguc_reply->dtd_hold[
     dguc_tbl_ndx].tbl_name)
    ...
    (SELECT
     cv.display
     FROM code_value cv
     WHERE cv.code_set IN (255351, 4001912)
      AND cv.cdf_meaning IN ("ALG1", "ALG2", "NORDDSTRG")
      AND cv.active_ind=1))))
   DETAIL
    dbx_tbl_ndx = (dbx_tbl_ndx+ 1)
    IF (mod(dbx_tbl_ndx,10)=1)
     stat = alterlist(dbx_tbls_to_backfill->tbl,(dbx_tbl_ndx+ 9))
    ENDIF
    dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].tbl_name = drcd.table_name, dbx_tbls_to_backfill->tbl[
    dbx_tbl_ndx].column_name = drcd.column_name, dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].sequence_name
     = drcd.sequence_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_program
  ENDIF
  SET stat = alterlist(dbx_tbls_to_backfill->tbl,dbx_tbl_ndx)
  SET dm_err->eproc = "Getting filter information for filtered tables."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT DISTINCT INTO "NL:"
   dtd.table_name, dtd.table_suffix
   FROM dm_refchg_filter drf,
    dm_tables_doc dtd
   WHERE drf.table_name=dtd.table_name
   ...
   (SELECT
    ...
    WHERE drfp.table_name=drf.table_name
     AND drfp.active_ind=1))
    AND  EXISTS (
   (SELECT
    "x"
    FROM dm_refchg_filter_test drft
    WHERE drft.table_name=drf.table_name
     AND drft.active_ind=1))
   DETAIL
    filter_ndx = 0, filter_ndx = locateval(filter_ndx,0,size(dbx_tbls_to_backfill->tbl,5),dtd
     .table_name,dbx_tbls_to_backfill->tbl[filter_ndx].tbl_name)
    IF (filter_ndx > 0)
     dbx_tbls_to_backfill->tbl[filter_ndx].filter_ind = 1, dbx_tbls_to_backfill->tbl[filter_ndx].
     tbl_suffix = dtd.table_suffix, dbx_tbls_to_backfill->tbl[filter_ndx].filter_proc = concat(
      "REFCHG_FILTER_",dtd.table_suffix,"_*")
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Starting backfill on the DM_REFCHG_INVALID_XLAT table."
  CALL disp_msg(" ",dm_err->logfile,0)
  FOR (dbx_tbl_ndx = 1 TO size(dbx_tbls_to_backfill->tbl,5))
    FOR (dbx_env_ndx = 1 TO size(dbx_seq_match_info->parent,5))
      IF ((dbx_seq_match_info->parent[dbx_env_ndx].valid_link=1))
       SET dbx_done_flag = 0
       SET dbx_find_tbl_ndx = 0
       SET dbx_find_tbl_ndx = locateval(dbx_find_tbl_ndx,1,size(dbx_seq_match_info->parent[
         dbx_env_ndx].sequence,5),dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].sequence_name,
        dbx_seq_match_info->parent[dbx_env_ndx].sequence[dbx_find_tbl_ndx].sequence_name)
       SET t1_table_name = concat(dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].tbl_name,dbx_seq_match_info
        ->parent[dbx_env_ndx].parent_db_link)
       SET t2_table_name = dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].tbl_name
       SET t1_column_name = concat("t1.",dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].column_name)
       SET t2_column_name = concat("t2.",dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].column_name)
       SET ut_table_name = concat("USER_TABLES",dbx_seq_match_info->parent[dbx_env_ndx].
        parent_db_link)
       SET dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].tbl_name_exst_src_flg = 0
       SET uo_table_name = concat("USER_OBJECTS",dbx_seq_match_info->parent[dbx_env_ndx].
        parent_db_link)
       SELECT INTO "NL:"
        ut.table_name
        FROM (parser(ut_table_name) ut)
        WHERE (ut.table_name=dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].tbl_name)
        DETAIL
         dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].tbl_name_exst_src_flg = 1
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        GO TO exit_program
       ENDIF
       IF ((dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_ind=1))
        SELECT INTO "NL:"
         uo.object_name
         FROM (parser(uo_table_name) uo)
         WHERE uo.object_name=patstring(dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_proc)
         DETAIL
          dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_proc = uo.object_name
         WITH nocounter
        ;end select
        IF (check_error(dm_err->eproc)=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ...
        IF (curqual=0)
         SET dm_err->err_ind = 1
         SET dm_err->emsg = concat("A filter function is missing for the ",dbx_tbls_to_backfill->tbl[
          dbx_tbl_ndx].tbl_name,"table.")
         GO TO exit_program
        ENDIF
        SET dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_proc = concat(dbx_tbls_to_backfill->tbl[
         dbx_tbl_ndx].filter_proc,dbx_seq_match_info->parent[dbx_env_ndx].parent_db_link)
        CALL parser(concat("declare ",dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_proc,"() = i2 GO"
          ),1)
        SET dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_proc = concat(dbx_tbls_to_backfill->tbl[
         dbx_tbl_ndx].filter_proc,"(")
        SELECT INTO "nl:"
         dr.column_name
         FROM dm_refchg_filter_parm dr
         WHERE (dr.table_name=dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].tbl_name)
          AND dr.active_ind=1
         ORDER BY dr.parm_nbr
         HEAD REPORT
          dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_col_cnt = 0
         DETAIL
          dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_col_cnt = (dbx_tbls_to_backfill->tbl[
          dbx_tbl_ndx].filter_col_cnt+ 1), stat = alterlist(dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].
           filter_col,dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_col_cnt), dbx_tbls_to_backfill->
          tbl[dbx_tbl_ndx].filter_col[dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_col_cnt].
          filter_col_name = dr.column_name
         WITH nocounter
        ;end select
        IF (check_error(dm_err->eproc) != 0)
         SET dm_err->err_ind = 1
         SET dm_err->emsg = "Error obtaining filter"
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         GO TO exit_program
        ENDIF
        IF (curqual > 0)
         SET dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_proc = concat(dbx_tbls_to_backfill->tbl[
          dbx_tbl_ndx].filter_proc,"'ADD'")
         FOR (dbx_filter_col_loop = 1 TO dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_col_cnt)
           SET dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_proc = concat(dbx_tbls_to_backfill->tbl[
            dbx_tbl_ndx].filter_proc," ,t1.",dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_col[
            dbx_filter_col_loop].filter_col_name," ,t1.",dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].
            filter_col[dbx_filter_col_loop].filter_col_name)
         ENDFOR
         SET dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].filter_proc = concat(dbx_tbls_to_backfill->tbl[
          dbx_tbl_ndx].filter_proc,") = 1")
        ENDIF
       ENDIF
       IF ((dbx_tbls_to_backfill->tbl[dbx_tbl_ndx].tbl_name_exst_src_flg=1))
        EXECUTE dm2_xlat_backfill_child dbx_env_ndx, dbx_tbl_ndx, dbx_find_tbl_ndx
        IF ((dm_err->err_ind=1))
         SET dm_err->emsg = "Error encountered while populating DM_REFCHG_INVALID_XLAT"
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         ROLLBACK
         GO TO exit_program
        ENDIF
  ...
  FOR (dbx_env_ndx = 1 TO size(dbx_seq_match_info->parent,5))
    INSERT  FROM dm_info di
     SET di.info_domain = "INVALID SEQMATCH XLAT BACKFILL COMPLETE", di.info_name =
      dbx_seq_match_info->parent[dbx_env_ndx].parent_db_link, di.updt_dt_tm = cnvtdatetime(curdate,
       curtime3)
     WITH nocounter
    ;end insert
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
    DELETE  FROM dm_info di
     WHERE di.info_domain="INVALID XLAT BACKFILL IN PROGRESS"
      AND (di.info_name=dbx_seq_match_info->parent[dbx_env_ndx].parent_db_link)
     WITH nocounter
    ;end delete
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    SET reply->status = "S"
    SET reply->status_msg = "Backfill process completed. Starting merge process"
    COMMIT
  ENDFOR
 ENDIF
 SET dm_err->eproc = "Exiting Translation Backfill Process: BACKFILL SUCCESSFUL"
 SUBROUTINE dxb_check_backfill_progress(s_src_env_id,s_tgt_env_id)
   DECLARE s_rdb_handle = f8
   DECLARE s_pl_name = vc
   SET s_pl_name = concat("@MERGE",trim(cnvtstring(s_src_env_id)),trim(cnvtstring(s_tgt_env_id)))
   SET dm_err->eproc = "Checking to see if the backfill process is complete."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "NL:"
    FROM dm_info di
    WHERE di.info_domain="INVALID SEQMATCH XLAT BACKFILL COMPLETE"
     AND di.info_name=s_pl_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    RETURN(0)
   ...
   IF (curqual != 0)
    RETURN(3)
   ENDIF
   SET dm_err->eproc = "Checking to see if the backfill process is in progress."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "NL:"
    di.info_number
    FROM dm_info di
   ...
   ENDIF
   INSERT  FROM dm_info di
    SET di.info_domain = "INVALID XLAT BACKFILL IN PROGRESS", di.info_name = s_pl_name, di
     .info_number = cnvtint(currdbhandle),
...
#exit_program
 ...
 ENDIF
 CALL final_disp_msg("dm2_xlat_backfill")
 FREE RECORD dbx_env_info
 FREE RECORD dbx_tbls_to_backfill
END GO
