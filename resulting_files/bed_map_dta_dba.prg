CREATE PROGRAM bed_map_dta:dba
 FREE SET reply
 RECORD reply(
   1 error_msg = vc
   1 status_data
 ...
 FOR (x = 1 TO size(requestin->list_0,5))
   SET code = 0.0
   SET display_key = cnvtupper(requestin->list_0[x].hnam_mnemonic)
   SELECT INTO "nl:"
    FROM code_value cv
    PLAN (cv
   ...
   ;end select
   IF (curqual=0)
    SET error_msg = concat("Could not find code value in CS 14003 for hnam mnemonic: ",trim(requestin
      ->list_0[x].hnam_mnemonic))
    SET status = "W"
    SELECT INTO "ccluserdir:bed_map_dta.errlog"
     FROM dummyt d
     PLAN (d)
     DETAIL
    ...
    ;end select
   ELSEIF (curqual=1
    AND size(trim(requestin->list_0[x].legacy_mnemonic,3)) <= 0)
    SET error_msg = concat("No legacy mnemonic for ",trim(requestin->list_0[x].hnam_mnemonic))
    SET status = "W"
    SELECT INTO "ccluserdir:bed_map_dta.errlog"
     FROM dummyt d
     PLAN (d)
   ...
   ELSEIF (curqual=1
    ...
    INSERT  FROM code_value_alias cva
     SET cva.code_value = code, cva.code_set = 14003, cva.alias = requestin->list_0[x].
      legacy_mnemonic,
      cva.alias_type_meaning = "DTA", cva.contributor_source_cd = contributor_cd, cva.updt_cnt = 0,
...
#exit_script
 ...
 ELSE
  IF (status="W")
   ...
   CALL echo(
    "Warning -- Issue a commit but not all rows were processed -- check the error log ccluserdir:bed_map_dta.errlog"
    )
   SET reply->error_msg =
