CREATE PROGRAM dcp_get_proc_list_by_prsn_ids:dba
 DECLARE knt1 = i4 WITH public, noconstant(0,0)
 SET qual_cnt = size(request->person_qual,5)
 CALL echo("QUAL_CNT = ")
 CALL echo(qual_cnt)
 SET ierrcode = 0
 SELECT INTO "nl:"
  ...
  PLAN (dt)
   JOIN (e
   WHERE (e.person_id=request->person_qual[dt.seq].person_id))
   JOIN (p
 ...
 IF (ierrcode > 0)
  ...
  SET table_name = "PROCEDURE_PRSN_IDS"
 ENDIF
END GO
