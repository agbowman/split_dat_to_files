CREATE PROGRAM ct_sub_rpt_prescreen_by_prot:dba
 PROMPT
 ...
 DECLARE get_prescreen_result(dummy) = null WITH protect
 DECLARE pagebreak(dummy) = null WITH protect
 DECLARE finalizereport(ssendreport=vc) = null WITH protect
 DECLARE headpagesection_one(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE headpagesection_oneabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8
 WITH protect
 DECLARE headpagesection_two(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE headpagesection_twoabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8
 WITH protect
 DECLARE headpagesection_header(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE headpagesection_headerabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =
 f8 WITH protect
 ...
 WITH protect
 DECLARE footd1_seqsectionline(ncalc=i2) = f8 WITH protect
 DECLARE footd1_seqsectionlineabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 ...
 DECLARE _bcontheadpagesection_header = i2 WITH noconstant(0), protect
 ...
 SUBROUTINE get_prescreen_result(dummy)
   SELECT
    IF (1=1)
    ...
    HEAD PAGE
     ...
     dummy_val = headpagesection_two(rpt_render,((rptreport->m_pageheight - rptreport->m_marginbottom
      ) - _yoffset),_bcontheadpagesection_two), _bcontheadpagesection_header = 0, dummy_val =
     headpagesection_header(rpt_render,((rptreport->m_pageheight - rptreport->m_marginbottom) -
      _yoffset),_bcontheadpagesection_header),
     dummy_val = headpagesection_line(rpt_render)
    HEAD d1.seq
     IF (size(protlist->protqual,5) > 0)
      _bcontheadd1_seqsection = 0, bfirsttime = 1
      WHILE (((_bcontheadd1_seqsection=1) OR (bfirsttime=1)) )
        ...
        bfirsttime = 0
      ENDWHILE
      prot_count = (prot_count+ 1), match_count = 1
     ENDIF
    HEAD p.person_id
     bfound = 0
     IF (size(protlist->protqual[d1.seq].personqual,5) > 0)
      tmp_name = concat(trim(p.name_last),",",trim(p.name_first)), bfound = 1
     ENDIF
     mrn_count = 0
    DETAIL
     IF (bfound=1)
     ...
     WHILE (((_bcontfootd1_seqsection=1) OR (bfirsttime=1)) )
       ...
       IF (((_bholdcontinue=1) OR (_fdrawheight > 0)) )
        IF ((_fenddetail >= (_yoffset+ _fdrawheight)))
         _fdrawheight = (_fdrawheight+ footd1_seqsectionline(rpt_calcheight))
        ENDIF
       ENDIF
       IF (((_yoffset+ _fdrawheight) > _fenddetail))
        BREAK
       ...
       bfirsttime = 0
     ENDWHILE
     _fdrawheight = footd1_seqsectionline(rpt_calcheight)
     IF (((_yoffset+ _fdrawheight) > _fenddetail))
      BREAK
     ENDIF
     dummy_val = footd1_seqsectionline(rpt_render), match_count = 0
    FOOT PAGE
 ...
 SUBROUTINE finalizereport(ssendreport)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE headpagesection_one(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headpagesection_oneabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headpagesection_oneabs(ncalc,offsetx,offsety,maxheight,bcontinue)
 ...
 END ;Subroutine
 SUBROUTINE headpagesection_two(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headpagesection_twoabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headpagesection_twoabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE headpagesection_header(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headpagesection_headerabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headpagesection_headerabs(ncalc,offsetx,offsety,maxheight,bcontinue)
 ...
 SUBROUTINE footd1_seqsectionabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE footd1_seqsectionline(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = footd1_seqsectionlineabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE footd1_seqsectionlineabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.130000), private
   IF (ncalc=rpt_render)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s3c0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.063),(offsetx+ 8.000),(offsety+
     0.063))
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 ...
 SUBROUTINE footreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s3c0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.010),(offsety+ 0.032),(offsetx+ 8.010),(offsety+
     0.032))
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CT_SUB_RPT_PRESCREEN_BY_PROT"
