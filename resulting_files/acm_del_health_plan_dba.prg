CREATE PROGRAM acm_del_health_plan:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (h
   WHERE (h.health_plan_id=acm_request->health_plan_qual[xref->del[d.seq].idx].health_plan_id))
  WITH nocounter, status(reply->health_plan_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->health_plan_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    FROM health_plan h
    WHERE expand(t1,f_val,t_val,h.health_plan_id,acm_request->health_plan_qual[xref->del[t1].idx].
     health_plan_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,h.health_plan_id,acm_request->health_plan_qual[xref->del[t1].idx].
      health_plan_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->health_plan_qual[xref->del[index].idx].status=0))
