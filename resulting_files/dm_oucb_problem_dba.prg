CREATE PROGRAM dm_oucb_problem:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  IF (debug_ind=1)
   CALL echo("********DM_OUCB_PROBLEM - maintaining exception")
  ENDIF
  ...
  SET dcem_request->qual[1].script_name = "DM_OUCB_PROBLEM"
 ...
 ENDIF
 IF (debug_ind=1)
  CALL echo(build("********DM_OUCB_PROBLEM - FROM_XXX_ID = ",request->xxx_uncombine[ucb_cnt].
    from_xxx_id))
  CALL echo(build("********DM_OUCB_PROBLEM - TO_XXX_ID = ",request->xxx_uncombine[ucb_cnt].to_xxx_id)
   )
  CALL echo(build("********DM_OUCB_PROBLEM - data_number = ",rchildren->qual1[det_cnt].entity_pk[1].
    data_number))
  CALL echo(build("********DM_OUCB_PROBLEM - combine_action_cd = ",rchildren->qual1[det_cnt].
    combine_action_cd))
 ENDIF
