CREATE PROGRAM dm2_aload_dm_columns_doc
 ...
 ENDIF
 IF (check_logfile("dm2_aload_dcd",".log","dm2_aload_dm_columns_doc")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dm_columns_doc"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dadcd_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dadcd_file = vc WITH protect, noconstant( $2)
 DECLARE dadcd_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE dadcd_batch = i4 WITH protect, noconstant(0)
 DECLARE dadcd_file_loc_ret = vc WITH protect, noconstant("")
 DECLARE dadcd_curqual = i4 WITH noconstant(0)
 IF (dac_prelim(dadcd_pkg,dadcd_file_loc_ret,dadcd_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dadcd_batch=0)
  SET dadcd_batch = 5000
 ENDIF
 IF (dadcd_expected_rows > 0)
  EXECUTE dm2_dbimport concat(dadcd_file_loc_ret,dadcd_file), "dm2_aload_adoc_dcd", dadcd_batch
  CALL echo(concat(dadcd_file_loc_ret,dadcd_file))
 ENDIF
...
#exit_program
 ...
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dm_columns_doc completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_dm_columns_doc")
END GO
