CREATE PROGRAM br_add_healthaware:dba
 ...
 SET readme_data->message = "Readme Failed: Starting <br_add_healthaware.prg> script"
 ...
 SELECT INTO "nl:"
  FROM br_step bs
  PLAN (bs
   WHERE bs.step_mean="HEALTHAWARE")
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed checking existence of HEALTHAWARE row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_step bs
   SET bs.step_mean = "HEALTHAWARE", bs.step_disp = "HealthAware Setup", bs.step_type = "IMPMAINT",
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcir.br_client_id=1
   ...
   AND bcir.item_mean="COREH"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to check for existence of SOLUTION/COREH row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_client_item_reltn bcir
   SET bcir.br_client_item_reltn_id = seq(bedrock_seq,nextval), bcir.br_client_id = 1, bcir.item_type
     = "SOLUTION",
    bcir.item_mean = "COREH", bcir.item_display = "HealthAware", bcir.solution_seq = 270,
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcir.br_client_id=1
   ...
   AND bcir.item_mean="HEALTHAWARE"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to verify existence of STEP/HEALTHAWARE row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_client_item_reltn bcir
   SET bcir.br_client_item_reltn_id = seq(bedrock_seq,nextval), bcir.br_client_id = 1, bcir.item_type
     = "STEP",
    bcir.item_mean = "HEALTHAWARE", bcir.item_display = "HealthAware Setup", bcir.step_cat_mean =
    "CORE",
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcss.br_client_id=1
   AND bcss.solution_mean="COREH"
   AND bcss.step_mean="HEALTHAWARE"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to check for existence of COREH/HEALTHAWARE row: ",errmsg
   )
  GO TO exit_script
 ELSEIF (curqual=0)
  SELECT INTO "nl:"
   ...
   FROM br_client_sol_step bcss
   PLAN (bcss
    WHERE bcss.solution_mean="COREH")
   DETAIL
    last_step_seq = themax
   WITH nocounter
  ;end select
  INSERT  FROM br_client_sol_step bcss
   SET bcss.br_client_id = 1, bcss.solution_mean = "COREH", bcss.step_mean = "HEALTHAWARE",
 ...
 SET readme_data->message = "Readme Succeeded: <br_add_healthaware.prg> script"
#exit_script
