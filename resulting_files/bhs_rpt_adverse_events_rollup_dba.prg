CREATE PROGRAM bhs_rpt_adverse_events_rollup:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   nurs_loc->l_icu_cnt += 1, stat = alterlist(nurs_loc->icu_unit,nurs_loc->l_icu_cnt), nurs_loc->
   icu_unit[nurs_loc->l_icu_cnt].f_code_value = cv.code_value,
 ...
 SELECT INTO "nl:"
  ...
  HEAD e.encntr_id
   ...
   IF ((m_enc->qual[m_enc->l_cnt].f_decease_date > 0.0))
    m_enc->qual[m_enc->l_cnt].l_qual_cnt += 1
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  HEAD e.encntr_id
   ...
   IF (ml_idx2 > 0)
    IF ((m_enc->qual[ml_idx2].n_icu_ind=0))
     m_enc->qual[ml_idx2].n_icu_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
    ENDIF
   ENDIF
  WITH nocounter, expand = 1
 ;end select
   ...
   WHILE (ml_idx2 > 0)
    IF (n.source_identifier="26988014")
     IF ((m_enc->qual[ml_idx2].n_eclampsia_ind=0))
      m_enc->qual[ml_idx2].n_eclampsia_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ELSEIF (n.source_identifier="32441014")
     IF ((m_enc->qual[ml_idx2].n_pulmedema_ind=0))
      m_enc->qual[ml_idx2].n_pulmedema_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ELSEIF (n.source_identifier IN ("83208016", "307815016"))
     IF ((m_enc->qual[ml_idx2].n_dvtpe_ind=0))
      m_enc->qual[ml_idx2].n_dvtpe_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
  ...
  HEAD o.catalog_cd
   IF (oc.primary_mnemonic="Hysterectomy Total Abdominal")
    IF ((m_enc->qual[ml_idx2].n_hysterectomy_ind=0))
     m_enc->qual[ml_idx2].n_hysterectomy_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
    ENDIF
   ENDIF
   IF ((m_enc->qual[ml_idx2].n_surg_proc_ind=0))
    m_enc->qual[ml_idx2].n_surg_proc_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
   ENDIF
  WITH nocounter, expand = 1
 ;end select
 SELECT INTO "nl:"
   ...
   IF (ce.event_cd=mf_cs72_deliverycomplications_cd)
    IF (trim(n.source_string,3)="Uterine rupture")
     IF ((m_enc->qual[ml_idx2].n_uterine_rupture_ind=0))
      m_enc->qual[ml_idx2].n_uterine_rupture_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
    IF (trim(n.source_string,3)="Suspected Intraamniotic infection")
     IF ((m_enc->qual[ml_idx2].n_inraamniotic_infection_ind=0))
      m_enc->qual[ml_idx2].n_inraamniotic_infection_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
    IF (trim(n.source_string,3)="Cardiac arrest")
     IF ((m_enc->qual[ml_idx2].n_code_blue_ind=0))
      m_enc->qual[ml_idx2].n_code_blue_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd=mf_cs72_bloodproducttransfused_cd)
    IF (trim(n.source_string,3)="Red Blood Cells")
     ...
     IF ((m_enc->qual[ml_idx2].n_transfusion_4_unit_ind=0)
      ...
      m_enc->qual[ml_idx2].n_transfusion_4_unit_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
    ...
    IF (isnumeric(ce.result_val) > 0)
     IF (cnvtreal(ce.result_val) > 1000.0)
      IF ((m_enc->qual[ml_idx2].n_postpartum_hemorrhage_ind=0))
       m_enc->qual[ml_idx2].n_postpartum_hemorrhage_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
      ENDIF
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd=mf_cs72_groupbstreptrans_cd)
    IF (trim(cnvtupper(ce.result_val),3)="POSITIVE")
     IF ((m_enc->qual[ml_idx2].n_antibiotic_prophylaxis_ind=0)
      ...
      m_enc->qual[ml_idx2].n_antibiotic_prophylaxis_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd=mf_cs72_csectionpriority_cd)
    IF (trim(n.source_string,3)="Code White")
     IF ((m_enc->qual[ml_idx2].n_csec_code_white_ind=0))
      m_enc->qual[ml_idx2].n_csec_code_white_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
   ...
   mf_cs72_birthcomplications1_cd, mf_cs72_birthcomplications2_cd))
    IF (trim(n.source_string,3)="Shoulder Dystocia")
     IF ((m_enc->qual[ml_idx2].n_shoulder_dystocia_ind=0))
      m_enc->qual[ml_idx2].n_shoulder_dystocia_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd=mf_cs72_operativedelivery_cd)
    IF (trim(n.source_string,3)="Forceps")
     IF ((m_enc->qual[ml_idx2].n_forcep_delivery_ind=0))
      m_enc->qual[ml_idx2].n_forcep_delivery_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
    IF (trim(n.source_string,3)="Vacuum")
     IF ((m_enc->qual[ml_idx2].n_vacuum_deliver_ind=0))
      m_enc->qual[ml_idx2].n_vacuum_deliver_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd=mf_cs72_reasonforcsection_cd)
    IF (trim(n.source_string,3)="Failed TOLAC")
     IF ((m_enc->qual[ml_idx2].n_failed_tolac_ind=0))
      m_enc->qual[ml_idx2].n_failed_tolac_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd IN (mf_cs72_fetalposition1_cd, mf_cs72_fetalposition2_cd))
    ...
    "Right sacrum posterior", "Left sacrum transverse", "Right sacrum transverse"))
     IF ((m_enc->qual[ml_idx2].n_singleton_ind=0))
      m_enc->qual[ml_idx2].n_singleton_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd=mf_cs72_neonateoutcome_cd)
    IF (trim(n.source_string,3)="Neonatal death")
     IF ((m_enc->qual[ml_idx2].n_neonatal_death_ind=0))
      m_enc->qual[ml_idx2].n_neonatal_death_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd IN (mf_cs72_neonatecomplications1_cd, mf_cs72_neonatecomplications2_cd))
    IF (trim(n.source_string,3) IN ("Abrasion", "Cephalohematoma", "Deformity, suspected"))
     IF ((m_enc->qual[ml_idx2].n_traumatic_birth_ind=0))
      m_enc->qual[ml_idx2].n_traumatic_birth_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd IN (mf_cs72_transferredto1_cd, mf_cs72_transferredto2_cd))
    IF (trim(n.source_string,3)="NICU")
     IF ((m_enc->qual[ml_idx2].n_unexpected_nicu_ind=0))
      m_enc->qual[ml_idx2].n_unexpected_nicu_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
    ...
    IF (isnumeric(ce.result_val) > 0)
     IF (cnvtreal(ce.result_val) < 7.0)
      IF ((m_enc->qual[ml_idx2].n_term_apgar_ind=0))
       m_enc->qual[ml_idx2].n_term_apgar_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
      ENDIF
     ENDIF
    ENDIF
    ...
    IF (isnumeric(ce.result_val) > 0)
     IF (cnvtreal(ce.result_val) < 7.0)
      IF ((m_enc->qual[ml_idx2].n_cord_ph_ind=0))
       m_enc->qual[ml_idx2].n_cord_ph_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
      ENDIF
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd IN (mf_cs72_nicuteamcalled_cd, mf_cs72_nicuteamcalled2_cd))
    IF (trim(n.source_string,3)="Code C")
     IF ((m_enc->qual[ml_idx2].n_nicu_code_c_ind=0))
      m_enc->qual[ml_idx2].n_nicu_code_c_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
     ENDIF
    ENDIF
    ...
    IF (isnumeric(ce.result_val) > 0)
     IF (cnvtreal(ce.result_val) > 10.1)
      IF ((m_enc->qual[ml_idx2].n_magnesium_tox_ind=0))
       m_enc->qual[ml_idx2].n_magnesium_tox_ind = 1, m_enc->qual[ml_idx2].l_qual_cnt += 1
      ENDIF
     ENDIF
    ENDIF
   ELSEIF (ce.event_cd=mf_cs72_egaatdocumenteddatetime_cd)
  ...
  FOOT  ce.encntr_id
   IF ((m_enc->qual[ml_idx2].n_unexpected_nicu_ind=1))
    IF (isnumeric(m_enc->qual[ml_idx2].s_delivery_ega) > 0)
     IF (cnvtint(m_enc->qual[ml_idx2].s_delivery_ega) <= 37)
      m_enc->qual[ml_idx2].n_unexpected_nicu_ind = 0, m_enc->qual[ml_idx2].l_qual_cnt -= 1
     ENDIF
    ENDIF
   ENDIF
   IF ((m_enc->qual[ml_idx2].n_term_apgar_ind=1))
    IF (isnumeric(m_enc->qual[ml_idx2].s_delivery_ega) > 0)
     IF (cnvtint(m_enc->qual[ml_idx2].s_delivery_ega) < 37)
      m_enc->qual[ml_idx2].n_term_apgar_ind = 0, m_enc->qual[ml_idx2].l_qual_cnt -= 1
     ENDIF
    ENDIF
   ENDIF
   IF ((m_enc->qual[ml_idx2].n_antibiotic_prophylaxis_ind=1)
    AND (m_enc->qual[ml_idx2].n_vag_del != 1))
    m_enc->qual[ml_idx2].n_antibiotic_prophylaxis_ind = 0, m_enc->qual[ml_idx2].l_qual_cnt -= 1
   ENDIF
   IF ((m_enc->qual[ml_idx2].n_singleton_ind=1)
    AND (m_enc->qual[ml_idx2].n_vag_del != 1))
    m_enc->qual[ml_idx2].n_singleton_ind = 0, m_enc->qual[ml_idx2].l_qual_cnt -= 1
   ENDIF
  WITH nocounter, expand = 1
 ;end select
 SELECT INTO "nl:"
  ...
  HEAD e.encntr_id
   ...
   (ea.alias,3), m_enc->qual[d1.seq].s_facility = trim(uar_get_code_display(e.loc_facility_cd),3)
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   WHERE cd1.ce_dynamic_label_id=ce.ce_dynamic_label_id
    AND cd1.label_status_cd IN (mf_cs4002015_active_cd, mf_cs4002015_inactive_cd))
   JOIN (cdr
  ...
  HEAD cd1.ce_dynamic_label_id
   m_enc->qual[d1.seq].l_dyn_cnt += 1, stat = alterlist(m_enc->qual[d1.seq].dyn_grp,m_enc->qual[d1
    .seq].l_dyn_cnt), m_enc->qual[d1.seq].dyn_grp[m_enc->qual[d1.seq].l_dyn_cnt].f_dyn_grp_id = cd1
   .ce_dynamic_label_id,
 ...
 FREE RECORD t_rec
 RECORD t_rec(
   1 l_tot_mat_death = i4
   1 l_tot_uterine_rupture = i4
   1 l_tot_icu_admission = i4
   1 l_tot_return_to_or = i4
   1 l_tot_transfusion_of_4_units = i4
   1 l_tot_hysterectomy = i4
   1 l_tot_postpartum_hemorrhage = i4
   1 l_tot_eclampsia = i4
   1 l_tot_pulmonary_edema = i4
   1 l_tot_dvt = i4
   1 l_tot_magnesium_tox = i4
   1 l_tot_fail_to_give_antibiotic = i4
   1 l_tot_code_blue = i4
   1 l_tot_intraamniotic_infection = i4
   1 l_tot_code_white = i4
   1 l_tot_shoulder_dystocia = i4
   1 l_tot_forcep_delivery = i4
   1 l_tot_vacuum_delivery = i4
   1 l_tot_failed_tolac = i4
   1 l_tot_breech_delivery = i4
   1 l_tot_intrapartum_neonatal_death = i4
   1 l_tot_traumatic_birth_injury = i4
   1 l_tot_unexpected_nicu = i4
   1 l_tot_term_apgr_lt7_at_5min = i4
   1 l_tot_ph_lt7 = i4
   1 l_tot_nicu_code_c = i4
 ) WITH protect
 FOR (ml_idx1 = 1 TO m_enc->l_cnt)
   IF ((m_enc->qual[ml_idx1].l_dyn_cnt > 0)
    ...
    IF ((m_enc->qual[ml_idx1].n_uterine_rupture_ind=1))
     ...
     SET t_rec->l_tot_uterine_rupture += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_icu_ind=1))
     ...
     SET t_rec->l_tot_icu_admission += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_surg_proc_ind=1))
     ...
     SET t_rec->l_tot_return_to_or += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_transfusion_4_unit_ind=1))
     ...
     SET t_rec->l_tot_transfusion_of_4_units += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_hysterectomy_ind=1))
     ...
     SET t_rec->l_tot_hysterectomy += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_postpartum_hemorrhage_ind=1))
     ...
     SET t_rec->l_tot_postpartum_hemorrhage += 1
    ENDIF
    ...
    IF ((m_enc->qual[ml_idx1].n_pulmedema_ind=1))
     ...
     SET t_rec->l_tot_pulmonary_edema += 1
    ENDIF
    ...
    IF ((m_enc->qual[ml_idx1].n_magnesium_tox_ind=1))
     ...
     SET m_enc->qual[ml_idx1].s_qual_str = concat(m_enc->qual[ml_idx1].s_qual_str,
      "Magnesium Toxicity")
     SET t_rec->l_tot_magnesium_tox += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_antibiotic_prophylaxis_ind=1))
     ...
     SET t_rec->l_tot_fail_to_give_antibiotic += 1
    ENDIF
    ...
    IF ((m_enc->qual[ml_idx1].n_inraamniotic_infection_ind=1))
     ...
     SET t_rec->l_tot_intraamniotic_infection += 1
    ENDIF
    ...
    IF ((m_enc->qual[ml_idx1].n_shoulder_dystocia_ind=1))
     ...
     SET t_rec->l_tot_shoulder_dystocia += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_forcep_delivery_ind=1))
     ...
     SET t_rec->l_tot_forcep_delivery += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_vacuum_deliver_ind=1))
     ...
     SET t_rec->l_tot_vacuum_delivery += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_failed_tolac_ind=1))
     ...
     SET t_rec->l_tot_failed_tolac += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_singleton_ind=1))
     ...
     SET t_rec->l_tot_breech_delivery += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_neonatal_death_ind=1))
     ...
     SET t_rec->l_tot_intrapartum_neonatal_death += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_traumatic_birth_ind=1))
     ...
     SET t_rec->l_tot_traumatic_birth_injury += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_unexpected_nicu_ind=1))
     ...
     SET t_rec->l_tot_unexpected_nicu += 1
    ENDIF
    IF ((m_enc->qual[ml_idx1].n_term_apgar_ind=1))
     ...
     SET t_rec->l_tot_term_apgr_lt7_at_5min += 1
    ENDIF
    ...
    IF ((m_enc->qual[ml_idx1].n_nicu_code_c_ind=1))
     ...
     SET t_rec->l_tot_nicu_code_c += 1
    ENDIF
   ENDIF
 ENDFOR
 CALL echorecord(m_enc)
 IF (ms_outdev="OPS")
  SET frec->file_name = concat("bhs_rpt_adverse_events_rollup_",format(sysdate,"MMDDYYYY;;q"),".csv")
  SET frec->file_buf = "w"
  SET stat = cclio("OPEN",frec)
  SET frec->file_buf = build('"Qual Condition",','"Total"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Maternal Death:","',trim(cnvtstring(t_rec->l_tot_mat_death),3),'"',
   char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Uterine Rupture:","',trim(cnvtstring(t_rec->l_tot_uterine_rupture),3),
   '"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"ICU admission:","',trim(cnvtstring(t_rec->l_tot_icu_admission),3),'"',
   char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Return to OR:","',trim(cnvtstring(t_rec->l_tot_return_to_or),3),'"',
   char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Transfusion of >4 units:","',trim(cnvtstring(t_rec->
     l_tot_transfusion_of_4_units),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Hysterectomy:","',trim(cnvtstring(t_rec->l_tot_hysterectomy),3),'"',
   char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Postpartum Hemorrhage >1000:","',trim(cnvtstring(t_rec->
     l_tot_postpartum_hemorrhage),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Eclampsia:","',trim(cnvtstring(t_rec->l_tot_eclampsia),3),'"',char(13)
   )
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Pulmonary Edema:","',trim(cnvtstring(t_rec->l_tot_pulmonary_edema),3),
   '"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"DVT/PE:","',trim(cnvtstring(t_rec->l_tot_dvt),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Magnesium Toxicity:","',trim(cnvtstring(t_rec->l_tot_magnesium_tox),3),
   '"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Failure to give antibiotic prophylaxis:","',trim(cnvtstring(t_rec->
     l_tot_fail_to_give_antibiotic),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Code Blue:","',trim(cnvtstring(t_rec->l_tot_code_blue),3),'"',char(13)
   )
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Intraamniotic infection:","',trim(cnvtstring(t_rec->
     l_tot_intraamniotic_infection),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Code White with c-section delivery:","',trim(cnvtstring(t_rec->
     l_tot_code_white),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Shoulder Dystocia:","',trim(cnvtstring(t_rec->l_tot_shoulder_dystocia),
    3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Forcep Delivery:","',trim(cnvtstring(t_rec->l_tot_forcep_delivery),3),
   '"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Vacuum Delivery:","',trim(cnvtstring(t_rec->l_tot_vacuum_delivery),3),
   '"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Failed TOLAC:","',trim(cnvtstring(t_rec->l_tot_failed_tolac),3),'"',
   char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Breech vaginal singleton delivery:","',trim(cnvtstring(t_rec->
     l_tot_breech_delivery),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Intrapartum or neonatal death:","',trim(cnvtstring(t_rec->
     l_tot_intrapartum_neonatal_death),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Traumatic birth injury:","',trim(cnvtstring(t_rec->
     l_tot_traumatic_birth_injury),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Unexpected NICU admission >24 hours:","',trim(cnvtstring(t_rec->
     l_tot_unexpected_nicu),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"Term apgar <7 at 5 minutes:","',trim(cnvtstring(t_rec->
     l_tot_term_apgr_lt7_at_5min),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"pH <7.0:","',trim(cnvtstring(t_rec->l_tot_ph_lt7),3),'"',char(13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build('"NICU code C:","',trim(cnvtstring(t_rec->l_tot_nicu_code_c),3),'"',char
   (13))
  SET stat = cclio("WRITE",frec)
  SET frec->file_buf = build(char(13),'"Qual Condition",','"Patient Name",','"MRN",',
   '"Delivering Provider",',
   '"Attending Provider",','"Date,Time of Birth",','"Gestational Age",','"Birth Weight",',
   '"1 min apgar",',
   '"5 min apgar",','"Infant Disposition",','"Type of Delivery",','"Facility"',char(13))
  SET stat = cclio("WRITE",frec)
  FOR (ml_idx1 = 1 TO m_enc->l_cnt)
    IF ((m_enc->qual[ml_idx1].l_dyn_cnt > 0)
     AND (m_enc->qual[ml_idx1].l_qual_cnt > 0))
     FOR (ml_idx2 = 1 TO m_enc->qual[ml_idx1].l_dyn_cnt)
       ...
       SET frec->file_buf = build(frec->file_buf,'","',m_enc->qual[ml_idx1].dyn_grp[ml_idx2].
        ...
        '","',m_enc->qual[ml_idx1].dyn_grp[ml_idx2].s_del_type,'","',m_enc->qual[ml_idx1].s_facility,
        '"',
        char(13))
       SET stat = cclio("WRITE",frec)
     ENDFOR
  ...
  DECLARE ms_email = vc WITH protect, constant("obadverseevents@bhs.org")
  EXECUTE bhs_ma_email_file
  SET ms_tmp = concat("Adverse Events Rollup Report: ",format(cnvtdatetime(sysdate),
    "YYYYMMDDHHMMSS;;q"))
  CALL emailfile(value(frec->file_name),frec->file_name,ms_email,ms_tmp,1)
 ELSE
  SELECT INTO  $OUTDEV
   qualifying_condition = trim(substring(1,1000,m_enc->qual[d1.seq].s_qual_str)), patient_name = trim
   (substring(1,50,m_enc->qual[d1.seq].s_pat_name)), mrn = trim(substring(1,15,m_enc->qual[d1.seq].
     s_pat_mrn)),
   delivering_provider =
   ...
   birth_date = trim(substring(1,20,m_enc->qual[d1.seq].dyn_grp[d2.seq].s_birth_dt)),
   gestational_age = trim(substring(1,15,m_enc->qual[d1.seq].s_gest_age)), birth_weigth = trim(
    substring(1,10,m_enc->qual[d1.seq].dyn_grp[d2.seq].s_birth_weight)), 1_min_apgar = trim(substring
    (1,10,m_enc->qual[d1.seq].dyn_grp[d2.seq].s_apgar_1min)),
   5_min_apgar = trim(substring(1,10,m_enc->qual[d1.seq].dyn_grp[d2.seq].s_apgar_5min)),
   infant_disposition = trim(substring(1,50,m_enc->qual[d1.seq].dyn_grp[d2.seq].s_neonate_outcome)),
   type_of_delivery = trim(substring(1,50,m_enc->qual[d1.seq].dyn_grp[d2.seq].s_del_type)),
   facility = trim(substring(1,50,m_enc->qual[d1.seq].s_facility)), encntr_id = substring(1,20,trim(
     cnvtstring(m_enc->qual[d1.seq].f_encntr_id,20),3))
   FROM (dummyt d1  WITH seq = m_enc->l_cnt),
