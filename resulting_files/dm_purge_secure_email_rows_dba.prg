CREATE PROGRAM dm_purge_secure_email_rows:dba
 ...
 SET reply->table_name = "RRD_SECURE_EMAIL_DETAIL"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(rsed.rrd_secure_email_detail_id)
   FROM rrd_secure_email_detail rsed
   WHERE rsed.rrd_secure_email_detail_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(rsed.rrd_secure_email_detail_id)
  FROM rrd_secure_email_detail rsed
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    rsed.rowid
    FROM rrd_secure_email_detail rsed
    WHERE rsed.rrd_secure_email_detail_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "rsed.sent_dt_tm < cnvtdatetime(curdate - v_days_to_keep, curtime3)","rsed"))
    DETAIL
     v_rows += 1
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = rsed.rowid
    WITH nocounter, maxqual(rsed,value(v_rows_left))
   ;end select
