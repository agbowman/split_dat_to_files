CREATE PROGRAM dm2_aload_sch_part_objects
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_sch_part_objects",".log","dm2_aload_sch_part_objects")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_sch_part_objects"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daspo_cnt = i4 WITH protect, noconstant(0)
 DECLARE daspo_str = vc WITH protect, noconstant("")
 DECLARE daspo_req_size = i4 WITH protect, noconstant(0)
 DECLARE daspo_ndx = i4 WITH protect, noconstant(0)
 FREE RECORD daspo_maint
 RECORD daspo_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET daspo_maint->num_of_inserts = 0
 SET daspo_maint->num_of_updates = 0
 SET daspo_maint->num_of_none = 0
 SET daspo_req_size = size(requestin->list_0,5)
 SET stat = alterlist(daspo_maint->qual,daspo_req_size)
 IF (dac_pop_coldic_rec("DM_AFD_PART_OBJECTS")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining maintenance type for DM_AFD_PART_OBJECTS."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(daspo_req_size))
  DETAIL
   daspo_maint->qual[d.seq].maint_type = "I", daspo_maint->num_of_inserts = (daspo_maint->
   num_of_inserts+ 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 SELECT INTO "nl:"
  FROM dm_afd_tables dat,
   ...
   (dummyt d  WITH seq = value(daspo_req_size))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (dat
   ...
   JOIN (dapo
   WHERE dapo.owner=dat.owner
    AND dapo.alpha_feature_nbr=cnvtint(requestin->list_0[d.seq].alpha_feature_nbr)
    AND (dapo.table_name=requestin->list_0[d.seq].table_name)
    AND (dapo.object_name=requestin->list_0[d.seq].object_name)
    AND dapo.table_name=dat.table_name)
  ORDER BY dapo.table_name, dapo.object_name
  DETAIL
   IF (cnvtint(requestin->list_0[d.seq].schema_instance) < dat.schema_instance)
    daspo_maint->qual[d.seq].maint_type = "N", daspo_maint->num_of_none = (daspo_maint->num_of_none+
    1), daspo_maint->num_of_inserts = (daspo_maint->num_of_inserts - 1)
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm)=cnvtdatetime(dapo.updt_dt_tm))
    daspo_maint->qual[d.seq].maint_type = "N", daspo_maint->num_of_none = (daspo_maint->num_of_none+
    1), daspo_maint->num_of_inserts = (daspo_maint->num_of_inserts - 1)
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm) != cnvtdatetime(dapo.updt_dt_tm))
    daspo_maint->qual[d.seq].maint_type = "U", daspo_maint->num_of_updates = (daspo_maint->
    num_of_updates+ 1), daspo_maint->num_of_inserts = (daspo_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(daspo_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((daspo_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(daspo_req_size) )"),0)
  ...
  COMMIT
 ENDIF
 IF ((daspo_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where daspo_maint->qual[d.seq].maint_type = 'U' ",0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_sch_part_objects completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_sch_part_objects")
END GO
