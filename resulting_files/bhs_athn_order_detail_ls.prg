CREATE PROGRAM bhs_athn_order_detail_ls
 ...
 SELECT DISTINCT INTO  $1
  ...
  dx_string = trim(replace(replace(replace(replace(replace(order_list->qual[d1.seq].dx_string,"&",
        "&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),
  oc_stop_type_meaning = uar_get_code_meaning(oc.stop_type_cd), oc.disable_order_comment_ind,
  o_protocol_order_id = trim(replace(cnvtstring(o.protocol_order_id),".0*","",0),3), oc.ref_text_mask
  FROM (dummyt d1  WITH seq = size(order_list->qual,5)),
   orders o,
   order_detail od,
   order_action oa,
  ...
  HEAD o.order_id
   ...
   row + 1, v1085 = build("<RefTextMask>",
    IF (oc.ref_text_mask > 0) "RefTextAvailable"
  ...
  HEAD od.oe_field_id
   ...
   IF (trim(od.oe_field_meaning)="REQSTARTDTTM")
    v96 = build("<RequestedStartDateTime>",format(od.oe_field_dt_tm_value,"MM/DD/YYYY HH:MM;;D"),
