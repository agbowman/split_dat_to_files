CREATE PROGRAM bhs_rpt_ews_table_audit:dba
 PROMPT
 ...
 SELECT INTO  $OUTDEV
  ...
  PLAN (d)
   ...
   WHERE e.active_ind=1)
   JOIN (ew
   WHERE (ew.clinical_event_id=temp->qual[d.seq].clinical_event_id)
    AND (ew.event_id=temp->qual[d.seq].event_id)
    AND ew.active_ind=1
    AND ew.event_cd=e.event_cd)
   JOIN (ce
   WHERE ce.event_id=ew.event_id
