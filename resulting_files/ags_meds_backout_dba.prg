CREATE PROGRAM ags_meds_backout:dba
 PROMPT
  "Backout Type (J, I, P) =" = "J",
  "AGS_JOB_ID (0.0) = " = 0.0
  WITH cbackout_type, djob_id
 CALL echo("***")
 CALL echo("***   BEG :: AGS_MEDS_BACKOUT")
 CALL echo("***")
 ...
 IF ( NOT (validate(log,0)))
  ...
  DECLARE sstatus_file_name = vc WITH private, noconstant(concat("ags_meds_backout_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_MEDS_BACKOUT"
 ...
 IF (the_type="P"
  ...
  SELECT INTO "nl:"
   ...
   HEAD REPORT
    sknt = 0, stat = alterlist(meds_di->qual,10)
   DETAIL
    sknt = (sknt+ 1)
    IF (mod(sknt,10)=1
     AND sknt != 1)
     stat = alterlist(meds_di->qual,(sknt+ 9))
    ENDIF
    meds_di->qual[sknt].item_id = a.gs_med_claim_id
   FOOT REPORT
    meds_di->qual_knt = sknt, stat = alterlist(meds_di->qual,sknt)
   WITH nocounter
  ;end select
  SET ierrcode = error(serrmsg,1)
  IF (ierrcode > 0)
   ...
   GO TO exit_script
  ENDIF
  IF ((meds_di->qual_knt > 0))
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   DELETE  FROM gs_med_claim_alias g,
     (dummyt d  WITH seq = value(meds_di->qual_knt))
    SET g.seq = 1
    PLAN (d
     WHERE d.seq > 0
      AND (meds_di->qual[d.seq].item_id > 0))
     JOIN (g
     WHERE (g.gs_med_claim_id=meds_di->qual[d.seq].item_id))
  ...
  UPDATE  FROM ags_meds_data a,
   ...
   SET a.status = "BACK OUT", a.status_dt_tm = cnvtdatetime(curdate,curtime3), a.gs_med_claim_id =
    0.0,
  ...
  IF (ierrcode > 0)
   ...
   SET log->qual[log->qual_knt].smsgtype = "ERROR"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_MEDS_DATA >> ",trim(serrmsg))
   GO TO exit_script
  ENDIF
 ELSEIF (the_type="J"
  AND the_job_id > 0.0)
  ...
  DELETE  FROM gs_med_claim_alias g
   SET g.seq = 1
   PLAN (g
    WHERE g.gs_med_claim_id IN (
    (SELECT
     gs_med_claim_id
     FROM ags_meds_data
     WHERE ags_job_id=the_job_id
      AND gs_med_claim_id > 0))
     AND g.gs_med_claim_id > 0)
   WITH nocounter, maxcommit = 5000
  ;end delete
  SET ierrcode = error(serrmsg,1)
  IF (ierrcode > 0)
 ...
 ELSEIF (the_type="I")
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    JOIN (a
    WHERE (a.ags_meds_data_id=item_rec->qual[d.seq].data_id)
     AND a.gs_med_claim_id > 0)
   HEAD REPORT
    sknt = 0, stat = alterlist(meds_di->qual,10)
   DETAIL
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_MEDS_BACKOUT"
 IF (define_logging_sub=true)
  CALL echo("***")
 ...
 ENDIF
 CALL echo("***")
 CALL echo("***   END :: AGS_MEDS_BACKOUT")
 CALL echo("***")
