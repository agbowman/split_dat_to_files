CREATE PROGRAM bbt_rpt_dis_reas:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->dispense_reasons = uar_i18ngetmessage(i18nhandle,"dispense_reasons","DISPENSE REASONS"
  )
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_dis_reas", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  PLAN (cv
   WHERE cv.code_set=1619)
   JOIN (cd
   WHERE cd.code_set=outerjoin(1619)
  ...
  HEAD PAGE
   ...
   col 51, captions->dispense_reasons, row + 2,
