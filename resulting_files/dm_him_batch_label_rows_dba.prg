CREATE PROGRAM dm_him_batch_label_rows:dba
 ...
 SET reply->table_name = "BATCH_LABEL"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(bl.batch_label_id)
   FROM batch_label bl
   WHERE bl.batch_label_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    bl.rowid
    FROM batch_label bl
    WHERE parser(sbr_getrowidnotexists("bl.batch_label_id between curMinID and curMaxID","bl"))
     AND bl.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND bl.active_ind=0
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
     ...
     ENDIF
     reply->rows[rows].row_id = bl.rowid
    WITH nocounter, maxqual(bl,value(rowsleft))
   ;end select
