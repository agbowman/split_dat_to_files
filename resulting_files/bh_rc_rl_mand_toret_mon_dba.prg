CREATE PROGRAM bh_rc_rl_mand_toret_mon:dba
 CALL echo("***** bh_rc_rl_mand_toret_mon.prg - 677918 *****")
   ...
   FOR (lchgcnt = 1 TO size(request->integer_params,5))
     CASE (cnvtupper(trim(request->integer_params[lchgcnt].key_txt,3)))
      OF "MAND_DED_TORET_MONTHLY":
       SET tmprec->mand_ded_toret_annual = (request->integer_params[lchgcnt].integer_value * 12)
     ENDCASE
   ENDFOR
 END ;Subroutine
 DECLARE createreply(null) = null
 SUBROUTINE createreply(null)
  CALL addtoreplyintegers("MAND_DED_TORET_ANNUAL",tmprec->mand_ded_toret_annual)
  CALL echorecord(tmprec)
 END ;Subroutine
 FREE RECORD tmprec
 RECORD tmprec(
   1 mand_ded_toret_annual = i4
 )
