CREATE PROGRAM dm2_set_dbstats_prefs:dba
 ...
 SUBROUTINE ddf_context_lock_maint(dclm_mode,dclm_process,dclm_process_code,dclm_owner,
  dclm_table_name,dclm_object_id,dclm_lock_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsdp_in_object_type = vc WITH protect, noconstant(" ")
 DECLARE dsdp_in_object_owner = vc WITH protect, noconstant(" ")
 DECLARE dsdp_in_object_name = vc WITH protect, noconstant(" ")
 DECLARE dsdp_in_mode = vc WITH protect, noconstant(" ")
 DECLARE dsdp_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dsdp_colmethodopt_default = vc WITH protect, noconstant("SIZE AUTO")
 DECLARE dsdp_currprefval = vc WITH protect, noconstant(" ")
 DECLARE dsdp_cnt = i4 WITH protect, noconstant(0)
 DECLARE dsdp_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dsdp_emsg = vc WITH protect, noconstant(" ")
 DECLARE dsdp_idxcol_method_default = vc WITH protect, noconstant("SIZE 254")
 DECLARE dsdp_actualonly_tab_est_pct = vc WITH protect, noconstant("0.00001")
 DECLARE dsdp_actualonly_ind_est_pct = vc WITH protect, noconstant("0.00001")
 DECLARE set_table_prefs(ownname=vc,tabname=vc,pname=vc,pvalue=vc) = null WITH sql =
 "SYS.DBMS_STATS.SET_TABLE_PREFS", parameter
 IF (check_logfile("dm2_setdbsprefs",".log","dm2_set_dbstats_prefs LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_set_dbstats_prefs"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dsdp_in_object_type = trim(cnvtupper( $1),3)
 SET dsdp_in_object_owner = trim(cnvtupper( $2),3)
 SET dsdp_in_object_name = trim( $3,3)
 SET dsdp_inmode = trim(cnvtupper( $4),3)
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_add_detail_text("IN_OBJECT_TYPE",dsdp_in_object_type)
 CALL dm2_process_log_add_detail_text("IN_OBJECT_OWNER",dsdp_in_object_owner)
 CALL dm2_process_log_add_detail_text("IN_OBJECT_NAME",dsdp_in_object_name)
 CALL dm2_process_log_add_detail_text("OBJ_TYPE.OBJ_OWNER.OBJ_NAME",concat(dsdp_in_object_type,".",
   dsdp_in_object_owner,".",dsdp_in_object_name))
 CALL dm2_process_log_add_detail_text("INMODE",dsdp_inmode)
 ...
 SET dsdp_log_id = dm2_process_event_rs->dm_process_event_id
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = concat("Parameter usage: dm2_set_dbstats_prefs  '<object_type[TABLE|INDEX]>'",
   "'<object_owner>','<object_name>','[SET|PREVIEW]'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dgdt_prefs->in_object_type = "DM2NOTSET"
 ...
 SET dgdt_prefs->in_object_name = "DM2NOTSET"
 SET dgdt_prefs->in_table_name = "DM2NOTSET"
 SET dgdt_prefs->table_owner = "DM2NOTSET"
 SET dgdt_prefs->in_object_type = dsdp_in_object_type
 SET dgdt_prefs->in_object_owner = dsdp_in_object_owner
 SET dgdt_prefs->in_object_name = dsdp_in_object_name
 IF (dsdp_in_object_type="TABLE")
  SET dgdt_prefs->in_table_name = dsdp_in_object_name
  SET dgdt_prefs->table_owner = dsdp_in_object_owner
 ENDIF
 ...
 ENDIF
 SET dm_err->eproc =
 "Checking for object existence before setting/getting statistics gathering preferences"
 SELECT INTO "nl:"
  FROM dba_objects d
  WHERE d.object_type=dsdp_in_object_type
   AND d.owner=dsdp_in_object_owner
   AND d.object_name=dsdp_in_object_name
  DETAIL
   dgdt_prefs->in_object_id = trim(cnvtstring(d.object_id,20))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSEIF (curqual=0)
  SET dm_err->eproc = concat(dsdp_in_object_type," object (",dsdp_in_object_name,") ","for owner (",
   dsdp_in_object_owner,") ","no longer exists, auto-successing dbstats preferences.")
  SET dgdt_prefs->autosuccess_ind = 1
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 ...
 SET dgdt_prefs->di_granularity_idx = "DM2NOTSET"
 SET dgdt_prefs->di_no_invalidate_idx = "DM2NOTSET"
 SET dm_err->eproc = "Checking for DM_INFO exclusions for the object"
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2GDBS:EXCLUDE*"
  HEAD REPORT
   dgdt_prefs->di_exclusion = "DM2NOTSET"
  DETAIL
   IF (dsdp_in_object_type="TABLE")
    IF (di.info_domain="DM2GDBS:EXCLUDE_TABLE_OWNER"
     AND dsdp_in_object_owner=di.info_name)
     dgdt_prefs->di_exclusion = "TABLE_OWNER"
    ENDIF
    IF (di.info_domain="DM2GDBS:EXCLUDE_TABLE"
     AND di.info_name=concat(dsdp_in_object_owner,".",dsdp_in_object_name))
     dgdt_prefs->di_exclusion = "TABLE"
    ENDIF
   ENDIF
   IF (dsdp_in_object_type="INDEX")
    IF (di.info_domain="DM2GDBS:EXCLUDE_INDEX_OWNER"
     AND dsdp_in_object_owner=di.info_name)
     dgdt_prefs->di_exclusion = "INDEX_OWNER"
    ENDIF
    IF (dsdp_in_object_type="INDEX"
     AND di.info_domain="DM2GDBS:EXCLUDE_INDEX"
     AND di.info_name=concat(dsdp_in_object_owner,".",dsdp_in_object_name))
     dgdt_prefs->di_exclusion = "INDEX"
    ENDIF
   ENDIF
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ENDIF
 IF ((dgdt_prefs->di_exclusion != "DM2NOTSET"))
  SET dgdt_prefs->autosuccess_ind = 1
  SET dm_err->eproc = concat(dsdp_in_object_type," object (",concat(dsdp_in_object_owner,".",
    dsdp_in_object_name),") ","is a DM_INFO exclusion, auto-successing table preferences.")
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF (dsdp_in_object_type="INDEX"
  AND (dgdt_prefs->in_table_name="DM2NOTSET"))
  SET dm_err->eproc = concat("Retrieving table_owner for INDEX object (",dsdp_in_object_name,") ",
   "for owner (",dsdp_in_object_owner,
   ") during stat preference processing. ")
  SELECT INTO "nl:"
   FROM dba_indexes d
   WHERE d.owner=dsdp_in_object_owner
    AND d.index_name=dsdp_in_object_name
    AND d.status != "UNUSABLE"
   DETAIL
    dgdt_prefs->in_table_name = d.table_name, dgdt_prefs->table_owner = d.table_owner
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ELSEIF ((dgdt_prefs->in_table_name="DM2NOTSET"))
   SET dm_err->eproc = concat("A usable ",dsdp_in_object_type," object (",dsdp_in_object_name,") ",
    "for owner (",dsdp_in_object_owner,") ","was not found in DBA_INDEXES.")
   SET dgdt_prefs->autosuccess_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ...
 ENDIF
 IF (dsdp_in_object_type="TABLE")
  SET dm_err->eproc = "Checking for column level default method_opt overrides."
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE ((d.info_domain="DM2GDBS:COL_METHOD_OPT_DEFAULT"
    AND d.info_name="DEFAULT"
    AND d.info_number=1) OR (d.info_domain="DM2GDBS:IND_COL_METHOD_OPT_DEFAULT"
    AND d.info_name="DEFAULT"
    AND d.info_number=1))
   DETAIL
    CASE (d.info_domain)
     OF "DM2GDBS:COL_METHOD_OPT_DEFAULT":
      dsdp_colmethodopt_default = cnvtupper(d.info_char)
     OF "DM2GDBS:IND_COL_METHOD_OPT_DEFAULT":
      dsdp_idxcol_method_default = cnvtupper(d.info_char)
    ENDCASE
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Load columns for the table being gathered"
  SELECT INTO "nl:"
   FROM dba_tab_cols d
   WHERE d.owner=dsdp_in_object_owner
    AND d.table_name=dsdp_in_object_name
   ORDER BY d.column_name
   HEAD REPORT
    dgdt_prefs->col_cnt = 0, stat = alterlist(dgdt_prefs->cols,dgdt_prefs->col_cnt)
   DETAIL
    dgdt_prefs->col_cnt = (dgdt_prefs->col_cnt+ 1), stat = alterlist(dgdt_prefs->cols,dgdt_prefs->
     col_cnt), dgdt_prefs->cols[dgdt_prefs->col_cnt].column_name = d.column_name,
    dgdt_prefs->cols[dgdt_prefs->col_cnt].method_opt = dsdp_colmethodopt_default, dgdt_prefs->cols[
    dgdt_prefs->col_cnt].di_method_opt = "DM2NOTSET"
   WITH nocounter
  ;end select
  SET dm_err->eproc =
  "Establish size254 default inclusion criteria for indexed and nonindexed columns."
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:SIZE254-DEFAULT"
    AND d.info_name IN ("INDEXED", "NONINDEXED")
    AND d.info_number=1
   HEAD REPORT
    dgdt_prefs->method_opt_size254_ind_def =
    "(d.column_name = '*_CD' or d.column_name = '*_IND' or d.column_name = '*_FLAG')", dgdt_prefs->
    method_opt_size254_nonind_def = "(0 = 1)"
   DETAIL
    CASE (d.info_name)
     OF "INDEXED":
      dgdt_prefs->method_opt_size254_ind_def = d.info_char
     OF "NONINDEXED":
      dgdt_prefs->method_opt_size254_nonind_def = d.info_char
    ENDCASE
   WITH nocounter, nullreport
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Load method_opt size 254 defaulted index columns"
  SELECT INTO "nl:"
   FROM dba_ind_columns d
   WHERE d.table_owner=dsdp_in_object_owner
    AND d.table_name=dsdp_in_object_name
    AND  NOT (d.column_name IN ("LAST_UTC_TS", "TXN_ID_TEXT", "INST_ID"))
    AND  NOT (d.column_name="*UUID*")
    AND parser(dgdt_prefs->method_opt_size254_ind_def)
   ORDER BY d.column_name
   DETAIL
    locndx = 0, locndx = locateval(locndx,1,dgdt_prefs->col_cnt,d.column_name,dgdt_prefs->cols[locndx
     ].column_name)
    IF (locndx > 0)
     dgdt_prefs->cols[locndx].method_opt = dsdp_idxcol_method_default
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Load method_opt size 254 defaulted non-indexed columns"
  SELECT INTO "nl:"
   FROM dba_tab_columns d
   WHERE d.owner=dsdp_in_object_owner
    AND d.table_name=dsdp_in_object_name
    AND parser(dgdt_prefs->method_opt_size254_nonind_def)
   ORDER BY d.column_name
   DETAIL
    locndx = 0, locndx = locateval(locndx,1,dgdt_prefs->col_cnt,d.column_name,dgdt_prefs->cols[locndx
     ].column_name)
    IF (locndx > 0)
     dgdt_prefs->cols[locndx].method_opt = "SIZE 254"
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Load dm_info method_opt column settings"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:PREF-COL:METHOD_OPT"
    AND d.info_name=patstring(concat(dsdp_in_object_owner,".",dsdp_in_object_name,"*"))
   ORDER BY d.info_domain
   HEAD REPORT
    locndx = 0
   DETAIL
    dsdp_di_work = substring((findstring(".",d.info_name,1,1)+ 1),textlen(trim(d.info_name)),d
     .info_name), locndx = locateval(locndx,1,dgdt_prefs->col_cnt,dsdp_di_work,dgdt_prefs->cols[
     locndx].column_name)
    IF (locndx > 0)
     dgdt_prefs->cols[locndx].method_opt = trim(d.info_char), dgdt_prefs->cols[locndx].di_method_opt
      = trim(d.info_char)
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   ...
   GO TO exit_script
  ENDIF
  IF ((dgdt_prefs->di_method_opt != "DM2NOTSET"))
   SET dgdt_prefs->method_opt = dgdt_prefs->di_method_opt
  ELSE
   SET dgdt_prefs->method_opt = "FOR COLUMNS"
   FOR (dsdp_cnt = 1 TO dgdt_prefs->col_cnt)
     IF (dsdp_cnt > 1)
      SET dgdt_prefs->method_opt = concat(dgdt_prefs->method_opt,", ")
     ENDIF
     SET dgdt_prefs->method_opt = concat(dgdt_prefs->method_opt,' "',dgdt_prefs->cols[dsdp_cnt].
      column_name,'" ',dgdt_prefs->cols[dsdp_cnt].method_opt)
     IF ((dgdt_prefs->cols[dsdp_cnt].method_opt != dsdp_colmethodopt_default))
      SET dgdt_prefs->method_opt_dped = concat(dgdt_prefs->method_opt_dped,dgdt_prefs->cols[dsdp_cnt]
       .column_name," ",dgdt_prefs->cols[dsdp_cnt].method_opt,",")
      IF (textlen(dgdt_prefs->method_opt_dped) > 2000)
       SET dgdt_prefs->method_opt_dped = substring(1,1999,dgdt_prefs->method_opt_dped)
      ENDIF
     ENDIF
   ENDFOR
  ENDIF
 ENDIF
 SET dm_err->eproc = "Retrieve currently active ORACLE TABLE stats gathering preferences"
 SELECT INTO "nl:"
  FROM dba_tab_stat_prefs d
  WHERE (d.owner=dgdt_prefs->table_owner)
   AND (d.table_name=dgdt_prefs->in_table_name)
  DETAIL
   CASE (trim(cnvtupper(d.preference_name)))
    OF "ESTIMATE_PERCENT":
     dgdt_prefs->est_pct = d.preference_value
    OF "METHOD_OPT":
     dgdt_prefs->method_opt = d.preference_value
    OF "DEGREE":
     dgdt_prefs->degree = d.preference_value
    OF "CASCADE":
     dgdt_prefs->cascade = d.preference_value
    OF "PUBLISH":
     dgdt_prefs->publish = d.preference_value
    OF "STALE_PERCENT":
     dgdt_prefs->stale_pct = d.preference_value
    OF "BLOCK_SAMPLE":
     dgdt_prefs->block_sample = d.preference_value
    OF "GRANULARITY":
     dgdt_prefs->granularity = d.preference_value
    OF "NO_INVALIDATE":
     dgdt_prefs->no_invalidate = d.preference_value
   ENDCASE
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking for DM_INFO->OWNER->TABLE level dbstats gathering overrides."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2GDBS:PREF-OWN:TAB_*"
   AND (d.info_name=dgdt_prefs->table_owner)
   AND d.info_number=1
  DETAIL
   CASE (trim(cnvtupper(d.info_domain)))
    OF "DM2GDBS:PREF-OWN:TAB_ESTIMATE_PERCENT":
     dgdt_prefs->di_est_pct = d.info_char
    OF "DM2GDBS:PREF-OWN:TAB_METHOD_OPT":
     dgdt_prefs->di_method_opt = d.info_char
    OF "DM2GDBS:PREF-OWN:TAB_DEGREE":
     dgdt_prefs->di_degree = d.info_char
    OF "DM2GDBS:PREF-OWN:TAB_CASCADE":
     dgdt_prefs->di_cascade = d.info_char
    OF "DM2GDBS:PREF-OWN:TAB_PUBLISH":
     dgdt_prefs->di_publish = d.info_char
    OF "DM2GDBS:PREF-OWN:TAB_STALE_PERCENT":
     dgdt_prefs->di_stale_pct = d.info_char
    OF "DM2GDBS:PREF-OWN:TAB_BLOCK_SAMPLE":
     dgdt_prefs->di_block_sample = d.info_char
    OF "DM2GDBS:PREF-OWN:TAB_GRANULARITY":
     dgdt_prefs->di_granularity = d.info_char
    OF "DM2GDBS:PREF-OWN:TAB_NO_INVALIDATE":
     dgdt_prefs->di_no_invalidate = d.info_char
   ENDCASE
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking for DM_INFO->TABLE level dbstats gathering overrides."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2GDBS:PREF-TAB:*"
   AND d.info_name=concat(dgdt_prefs->table_owner,".",dgdt_prefs->in_table_name)
   AND d.info_number=1
  DETAIL
   CASE (trim(cnvtupper(d.info_domain)))
    OF "DM2GDBS:PREF-TAB:ESTIMATE_PERCENT":
     dgdt_prefs->di_est_pct = d.info_char
    OF "DM2GDBS:PREF-TAB:METHOD_OPT":
     dgdt_prefs->di_method_opt = d.info_char
    OF "DM2GDBS:PREF-TAB:DEGREE":
     dgdt_prefs->di_degree = d.info_char
    OF "DM2GDBS:PREF-TAB:CASCADE":
     dgdt_prefs->di_cascade = d.info_char
    OF "DM2GDBS:PREF-TAB:PUBLISH":
     dgdt_prefs->di_publish = d.info_char
    OF "DM2GDBS:PREF-TAB:STALE_PERCENT":
     dgdt_prefs->di_stale_pct = d.info_char
    OF "DM2GDBS:PREF-TAB:BLOCK_SAMPLE":
     dgdt_prefs->di_block_sample = d.info_char
    OF "DM2GDBS:PREF-TAB:GRANULARITY":
     dgdt_prefs->di_granularity = d.info_char
    OF "DM2GDBS:PREF-TAB:NO_INVALIDATE":
     dgdt_prefs->di_no_invalidate = d.info_char
   ENDCASE
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ENDIF
 IF (dsdp_inmode="ACTUALONLY")
  SET dm_err->eproc = "Retrieve default estimate percent value to use for ACTUALONLY"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:PREF-ACTUALONLY:ESTIMATE_PERCENT"
    AND d.info_number=1
   DETAIL
    CASE (trim(cnvtupper(d.info_name)))
     OF "TABLE_DEFAULT":
      dsdp_actualonly_tab_est_pct = d.info_char
     OF "INDEX_DEFAULT":
      dsdp_actualonly_ind_est_pct = d.info_char
    ENDCASE
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   ...
   GO TO exit_script
  ENDIF
  SET dgdt_prefs->method_opt = "FOR ALL COLUMNS SIZE 1"
 ENDIF
 IF (dsdp_in_object_type="TABLE")
  IF ((dgdt_prefs->est_pct="DM2NOTSET"))
   IF ((((dm2_rdbms_version->level1 > 11)) OR ((((dm2_rdbms_version->level1=11)
    AND (dm2_rdbms_version->level2 > 2)) OR ((((dm2_rdbms_version->level1=11)
    AND (dm2_rdbms_version->level2=2)
    AND (dm2_rdbms_version->level3 > 0)) OR ((dm2_rdbms_version->level1=11)
    ...
    AND (dm2_rdbms_version->level3=0)
    AND (dm2_rdbms_version->level4 >= 4))) )) )) )
    CALL echo("You are on 11.2.0.4 or higher")
    SET dgdt_prefs->est_pct = "0"
   ELSE
    SET dgdt_prefs->est_pct = "100"
   ENDIF
   IF ((dgdt_prefs->di_est_pct != "DM2NOTSET"))
    SET dgdt_prefs->est_pct = dgdt_prefs->di_est_pct
   ENDIF
   SET dm_err->eproc = "Calling DBMS_STATS.SET_TABLE_PREFS to initialize ESTIMATE_PERCENT."
   CALL set_table_prefs(dgdt_prefs->table_owner,concat('"',dgdt_prefs->in_table_name,'"'),
    "ESTIMATE_PERCENT",dgdt_prefs->est_pct)
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
  ELSE
   SET dsdp_currprefval = dgdt_prefs->est_pct
   IF ((((dm2_rdbms_version->level1 > 11)) OR ((((dm2_rdbms_version->level1=11)
    AND (dm2_rdbms_version->level2 > 2)) OR ((((dm2_rdbms_version->level1=11)
    AND (dm2_rdbms_version->level2=2)
    AND (dm2_rdbms_version->level3 > 0)) OR ((dm2_rdbms_version->level1=11)
   ...
   ENDIF
   IF (dsdp_inmode="ACTUALONLY")
    SET dgdt_prefs->est_pct = dsdp_actualonly_tab_est_pct
   ENDIF
   IF ((dgdt_prefs->di_est_pct != "DM2NOTSET"))
    SET dgdt_prefs->est_pct = dgdt_prefs->di_est_pct
   ENDIF
   IF (cnvtreal(dgdt_prefs->est_pct) != cnvtreal(dsdp_currprefval))
    SET dm_err->eproc = "Calling DBMS_STATS.SET_TABLE_PREFS to override ESTIMATE_PERCENT."
    CALL set_table_prefs(dgdt_prefs->table_owner,concat('"',dgdt_prefs->in_table_name,'"'),
     "ESTIMATE_PERCENT",dgdt_prefs->est_pct)
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
  ENDIF
  SET dsdp_currprefval = dgdt_prefs->degree
  IF ((dgdt_prefs->degree != "1"))
   SET dgdt_prefs->degree = "1"
  ENDIF
  IF ((dgdt_prefs->di_degree != "DM2NOTSET"))
   SET dgdt_prefs->degree = dgdt_prefs->di_degree
  ENDIF
  IF ((dgdt_prefs->degree != dsdp_currprefval))
   SET dm_err->eproc = "Calling DBMS_STATS.SET_TABLE_PREFS to initialize/override DEGREE."
   CALL set_table_prefs(dgdt_prefs->table_owner,concat('"',dgdt_prefs->in_table_name,'"'),"DEGREE",
    dgdt_prefs->degree)
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
  ENDIF
  SET dsdp_currprefval = dgdt_prefs->cascade
  IF ((dgdt_prefs->cascade != "FALSE"))
   SET dgdt_prefs->cascade = "FALSE"
  ENDIF
  IF ((dgdt_prefs->di_cascade != "DM2NOTSET"))
   SET dgdt_prefs->cascade = dgdt_prefs->di_cascade
  ENDIF
  IF ((dgdt_prefs->cascade != dsdp_currprefval))
   SET dm_err->eproc = "Calling DBMS_STATS.SET_TABLE_PREFS to initialize/override CASCADE."
   CALL set_table_prefs(dgdt_prefs->table_owner,concat('"',dgdt_prefs->in_table_name,'"'),"CASCADE",
    dgdt_prefs->cascade)
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
  ENDIF
  SET dsdp_currprefval = dgdt_prefs->stale_pct
  IF ((dgdt_prefs->stale_pct != "10"))
   SET dgdt_prefs->stale_pct = "10"
  ENDIF
  IF ((dgdt_prefs->di_stale_pct != "DM2NOTSET"))
   SET dgdt_prefs->stale_pct = dgdt_prefs->di_stale_pct
  ENDIF
  IF ((dgdt_prefs->stale_pct != dsdp_currprefval))
   SET dm_err->eproc =
   "Call dbms_stats.SET_TABLE_PREFS to initialize/override STALE_PERCENT to Cerner default"
   CALL set_table_prefs(dgdt_prefs->table_owner,concat('"',dgdt_prefs->in_table_name,'"'),
    "STALE_PERCENT",dgdt_prefs->stale_pct)
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
  ENDIF
  SET dsdp_currprefval = dgdt_prefs->granularity
  IF ((dgdt_prefs->granularity != "GLOBAL"))
   SET dgdt_prefs->granularity = "GLOBAL"
  ENDIF
  IF ((dgdt_prefs->di_granularity != "DM2NOTSET"))
   SET dgdt_prefs->granularity = dgdt_prefs->di_granularity
  ENDIF
  IF ((dgdt_prefs->granularity != dsdp_currprefval))
   SET dm_err->eproc =
   "Call dbms_stats.SET_TABLE_PREFS to initialize/override GRANULARITY to Cerner default"
   CALL set_table_prefs(dgdt_prefs->table_owner,concat('"',dgdt_prefs->in_table_name,'"'),
    "GRANULARITY",dgdt_prefs->granularity)
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
  ENDIF
 ENDIF
 SET dsdp_currprefval = dgdt_prefs->publish
 IF ((dgdt_prefs->publish != "FALSE"))
  SET dgdt_prefs->publish = "FALSE"
 ENDIF
 IF ((dgdt_prefs->publish != dsdp_currprefval))
  SET dm_err->eproc =
  "Call dbms_stats.SET_TABLE_PREFS to initialize/override PUBLISH to Cerner default"
  CALL set_table_prefs(dgdt_prefs->table_owner,concat('"',dgdt_prefs->in_table_name,'"'),"PUBLISH",
   dgdt_prefs->publish)
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
 ...
 ENDIF
 IF ((dgdt_prefs->di_method_opt != "DM2NOTSET")
  AND (dgdt_prefs->method_opt != dgdt_prefs->di_method_opt))
  SET dgdt_prefs->method_opt = dgdt_prefs->di_method_opt
 ENDIF
 IF (dsdp_in_object_type="INDEX")
  IF ((((dm2_rdbms_version->level1 > 11)) OR ((((dm2_rdbms_version->level1=11)
   ...
   AND (dm2_rdbms_version->level4 >= 4))) )) )) )
   CALL echo("You are on 11.2.0.4 or higher")
   SET dgdt_prefs->est_pct_idx = "0"
  ELSE
   SET dgdt_prefs->est_pct_idx = "100"
  ENDIF
  SET dgdt_prefs->degree_idx = "1"
  IF (dsdp_inmode="ACTUALONLY")
   SET dgdt_prefs->est_pct_idx = dsdp_actualonly_ind_est_pct
  ENDIF
  SET dm_err->eproc = "Checking for DM_INFO->OWNER->INDEX dbstats gathering overrides."
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:PREF-OWN:IND_*"
    AND d.info_name=dsdp_in_object_owner
    AND d.info_number=1
   DETAIL
    CASE (trim(cnvtupper(d.info_domain)))
     OF "DM2GDBS:PREF-OWN:IND_ESTIMATE_PERCENT":
      dgdt_prefs->di_est_pct_idx = d.info_char,dgdt_prefs->est_pct_idx = d.info_char
     OF "DM2GDBS:PREF-OWN:IND_DEGREE":
      dgdt_prefs->di_degree_idx = d.info_char,dgdt_prefs->degree_idx = d.info_char
     OF "DM2GDBS:PREF-OWN:IND_GRANULARITY":
      dgdt_prefs->di_granularity_idx = d.info_char,dgdt_prefs->granularity_idx = d.info_char
     OF "DM2GDBS:PREF-OWN:IND_NO_INVALIDATE":
      dgdt_prefs->di_no_invalidate_idx = d.info_char,dgdt_prefs->no_invalidate_idx = d.info_char
    ENDCASE
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Checking for DM_INFO->INDEX dbstats gathering overrides."
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:PREF-IND:*"
    AND ((d.info_name=concat(dsdp_in_object_owner,".",dgdt_prefs->in_object_name)) OR (concat(d
    .info_name,"$C")=concat(dsdp_in_object_owner,".",dgdt_prefs->in_object_name)))
    AND d.info_number=1
   DETAIL
    CASE (trim(cnvtupper(d.info_domain)))
     OF "DM2GDBS:PREF-IND:ESTIMATE_PERCENT":
      dgdt_prefs->di_est_pct_idx = d.info_char,dgdt_prefs->est_pct_idx = d.info_char
     OF "DM2GDBS:PREF-IND:DEGREE":
      dgdt_prefs->di_degree_idx = d.info_char,dgdt_prefs->degree_idx = d.info_char
     OF "DM2GDBS:PREF-IND:GRANULARITY":
      dgdt_prefs->di_granularity_idx = d.info_char,dgdt_prefs->granularity_idx = d.info_char
     OF "DM2GDBS:PREF-IND:NO_INVALIDATE":
      dgdt_prefs->di_no_invalidate_idx = d.info_char,dgdt_prefs->no_invalidate_idx = d.info_char
    ENDCASE
   WITH nocounter
  ;end select
 ...
 ENDIF
 IF ((dm_err->debug_flag=1))
  CALL echorecord(dgdt_prefs)
 ENDIF
 IF (dsdp_inmode="PREVIEW")
  SET dm_err->eproc = build("Displaying current table preferences for (",dsdp_in_object_owner,".",
   dgdt_prefs->in_table_name,")")
  SET width = 132
  SELECT INTO "MINE"
   FROM dba_tab_stat_prefs dtsp
   WHERE (dtsp.owner=dgdt_prefs->table_owner)
    AND (dtsp.table_name=dgdt_prefs->in_table_name)
   ORDER BY dtsp.preference_name
   HEAD REPORT
    "********** CERNER DATABASE STATISTICS GATHERING PREVIEW REPORT **************************", row
     + 1, "TABLE_OWNER:",
    dsdp_in_object_owner, row + 1, "TABLE_NAME: ",
    dgdt_prefs->in_table_name
    IF (dsdp_in_object_type="INDEX")
     row + 1, "INDEX_NAME: ", dsdp_in_object_name
    ENDIF
    IF (dsdp_in_object_type="INDEX")
     row + 2, "********** INDEX GATHERING PREFERENCES *******************", row + 1,
     col 0, "PREFERENCE_NAME (+ denotes DM_INFO override)", col 50,
     "PREFERENCE_VALUE", row + 1, col 0,
     "--------------------------------------------", col 50, "-------------------------------------",
     row + 1
     IF ((dgdt_prefs->di_degree_idx="DM2NOTSET"))
      col 0, " ", col 1,
      "DEGREE", col 50, dgdt_prefs->degree_idx
     ELSE
      col 0, "+", col 1,
      "DEGREE", col 50, dgdt_prefs->di_degree_idx
     ENDIF
     row + 1
     IF ((dgdt_prefs->di_est_pct_idx="DM2NOTSET"))
      col 0, " ", col 1,
      "ESTIMATE_PERCENT", col 50, dgdt_prefs->est_pct_idx
     ELSE
      col 0, "+", col 1,
      "ESTIMATE_PERCENT", col 50, dgdt_prefs->di_est_pct_idx
     ENDIF
     row + 1
     IF ((dgdt_prefs->di_granularity_idx="DM2NOTSET"))
      col 0, " ", col 1,
      "GRANULARITY", col 50,
      CALL print(evaluate(dgdt_prefs->granularity_idx,"DM2NOTSET","DEFAULT",dgdt_prefs->
       granularity_idx))
     ELSE
      col 0, "+", col 1,
      "GRANULARITY", col 50, dgdt_prefs->di_granularity_idx
     ENDIF
     row + 1
     IF ((dgdt_prefs->di_no_invalidate_idx="DM2NOTSET"))
      col 0, " ", col 1,
      "NO_INVALIDATE", col 50,
      CALL print(evaluate(dgdt_prefs->no_invalidate_idx,"DM2NOTSET","DEFAULT",dgdt_prefs->
       no_invalidate_idx))
     ELSE
      col 0, "+", col 1,
      "NO_INVALIDATE", col 50, dgdt_prefs->di_no_invalidate_idx
     ENDIF
    ENDIF
    row + 2, "********** TABLE GATHERING PREFERENCES **************************", row + 1,
    col 0, "PREFERENCE_NAME (+ denotes DM_INFO override)", col 50,
    "PREFERENCE_VALUE", row + 1, col 0,
    "--------------------------------------------", col 50, "-------------------------------------"
   DETAIL
    IF (dtsp.preference_name != "METHOD_OPT")
     row + 1
     IF (((dtsp.preference_name="ESTIMATE_PERCENT"
      AND (dgdt_prefs->di_est_pct != "DM2NOTSET")) OR (((dtsp.preference_name="DEGREE"
      AND (dgdt_prefs->di_degree != "DM2NOTSET")) OR (((dtsp.preference_name="CASCADE"
      AND (dgdt_prefs->di_cascade != "DM2NOTSET")) OR (((dtsp.preference_name="PUBLISH"
      AND (dgdt_prefs->di_publish != "DM2NOTSET")) OR (((dtsp.preference_name="STALE_PERCENT"
      AND (dgdt_prefs->di_stale_pct != "DM2NOTSET")) OR (((dtsp.preference_name="GRANULARITY"
      AND (dgdt_prefs->di_granularity != "DM2NOTSET")) OR (dtsp.preference_name="NO_INVALIDATE"
      AND (dgdt_prefs->di_no_invalidate != "DM2NOTSET"))) )) )) )) )) )) )
      col 0, "+"
     ENDIF
     col + 1, dtsp.preference_name, col 50,
     CALL print(substring(1,70,trim(dtsp.preference_value)))
    ENDIF
   FOOT REPORT
    IF (dsdp_in_object_type="TABLE")
     row + 2, "********** METHOD_OPT (+ denotes DM_INFO override) **************************"
     IF ((dgdt_prefs->di_method_opt != "DM2NOTSET"))
      row + 1, "+ ", col + 2
      IF (textlen(dgdt_prefs->di_method_opt) > 126)
       CALL print(concat(substring(1,123,dgdt_prefs->di_method_opt),"..."))
      ELSE
       CALL print(dgdt_prefs->di_method_opt)
      ENDIF
     ELSE
      row + 1, "FOR COLUMNS "
      FOR (x = 1 TO dgdt_prefs->col_cnt)
        IF ((dgdt_prefs->cols[x].di_method_opt != "DM2NOTSET"))
         col 11, "+"
        ELSE
         col 11, " "
        ENDIF
        col 12,
        CALL print(concat('"',build(dgdt_prefs->cols[x].column_name),'" ',build(dgdt_prefs->cols[x].
          method_opt),evaluate(x,dgdt_prefs->col_cnt,"",","))), row + 1
      ENDFOR
     ENDIF
    ENDIF
   WITH nocounter, nullreport
...
#exit_script
 ...
 SET dsdp_err_ind = dm_err->err_ind
 SET dsdp_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_execution,dsdp_log_id,1)
 SET dm_err->err_ind = dsdp_err_ind
 SET dm_err->emsg = dsdp_emsg
 SET dm_err->eproc = "Ending dm2_set_dbstats_prefs"
 CALL final_disp_msg("dm2_setdbsprefs")
END GO
