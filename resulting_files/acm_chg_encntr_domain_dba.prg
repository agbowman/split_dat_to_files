CREATE PROGRAM acm_chg_encntr_domain:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->encntr_domain_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   encntr_domain e
  SET e.encntr_domain_id = acm_request->encntr_domain_qual[xref->chg[d.seq].idx].encntr_domain_id, e
   .beg_effective_dt_tm = cnvtdatetime(acm_request->encntr_domain_qual[xref->chg[d.seq].idx].
    beg_effective_dt_tm), e.encntr_domain_type_cd = acm_request->encntr_domain_qual[xref->chg[d.seq].
   idx].encntr_domain_type_cd,
   e.encntr_id =
   IF ((acm_request->encntr_domain_qual[xref->chg[d.seq].idx].encntr_id > 0)) acm_request->
    encntr_domain_qual[xref->chg[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->encntr_domain_qual[xref->chg[d.seq].idx].encntr_idx].
    encntr_id
   ENDIF
   , e.end_effective_dt_tm = cnvtdatetime(acm_request->encntr_domain_qual[xref->chg[d.seq].idx].
    end_effective_dt_tm), e.loc_bed_cd = acm_request->encntr_domain_qual[xref->chg[d.seq].idx].
   loc_bed_cd,
   e.loc_building_cd = acm_request->encntr_domain_qual[xref->chg[d.seq].idx].loc_building_cd, e
   .loc_facility_cd = acm_request->encntr_domain_qual[xref->chg[d.seq].idx].loc_facility_cd, e
   .loc_nurse_unit_cd = acm_request->encntr_domain_qual[xref->chg[d.seq].idx].loc_nurse_unit_cd,
   e.loc_room_cd = acm_request->encntr_domain_qual[xref->chg[d.seq].idx].loc_room_cd, e
   .med_service_cd = acm_request->encntr_domain_qual[xref->chg[d.seq].idx].med_service_cd, e
   .person_id =
   IF ((acm_request->encntr_domain_qual[xref->chg[d.seq].idx].person_id > 0)) acm_request->
    encntr_domain_qual[xref->chg[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->encntr_domain_qual[xref->chg[d.seq].idx].person_idx].
    person_id
   ENDIF
   ,
   e.active_ind = acm_request->encntr_domain_qual[xref->chg[d.seq].idx].active_ind, e
   .active_status_cd = acm_request->encntr_domain_qual[xref->chg[d.seq].idx].active_status_cd, e
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_domain_id=acm_request->encntr_domain_qual[xref->chg[d.seq].idx].encntr_domain_id))
  WITH nocounter, status(reply->encntr_domain_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->encntr_domain_qual[xref->chg[index].idx].status != 1))
    SET failed = update_error
    SET table_name = "ENCNTR_DOMAIN"
    GO TO exit_script
   ENDIF
 ENDFOR
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,e.encntr_domain_id,acm_request->encntr_domain_qual[xref->chg[t1].idx]
     .encntr_domain_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,e.encntr_domain_id,acm_request->encntr_domain_qual[xref->chg[t1].
      idx].encntr_domain_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((e.updt_cnt=acm_request->encntr_domain_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->encntr_domain_qual[idx].status = - (1), reply->encntr_domain_qual[idx].encntr_domain_id
       = acm_request->encntr_domain_qual[idx].encntr_domain_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->encntr_domain_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].beg_effective_dt_tm = e.beg_effective_dt_tm
     ENDIF
     IF (findstring("ENCNTR_DOMAIN_TYPE_CD,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].encntr_domain_type_cd = e.encntr_domain_type_cd
     ENDIF
     IF (findstring("ENCNTR_ID,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].encntr_id = e.encntr_id
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].end_effective_dt_tm = e.end_effective_dt_tm
     ENDIF
     IF (findstring("LOC_BED_CD,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].loc_bed_cd = e.loc_bed_cd
     ENDIF
     IF (findstring("LOC_BUILDING_CD,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].loc_building_cd = e.loc_building_cd
     ENDIF
     IF (findstring("LOC_FACILITY_CD,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].loc_facility_cd = e.loc_facility_cd
     ENDIF
     IF (findstring("LOC_NURSE_UNIT_CD,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].loc_nurse_unit_cd = e.loc_nurse_unit_cd
     ENDIF
     IF (findstring("LOC_ROOM_CD,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].loc_room_cd = e.loc_room_cd
     ENDIF
     IF (findstring("MED_SERVICE_CD,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].med_service_cd = e.med_service_cd
     ENDIF
     IF (findstring("PERSON_ID,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].person_id = e.person_id
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].active_ind = e.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->encntr_domain_qual[idx].active_status_cd = e.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->encntr_domain_qual[xref->chg[index].idx].status=0))
