CREATE PROGRAM dcp_purge_pip_activity:dba
 ...
 DECLARE activitycount = i4 WITH protect, noconstant(0)
 ...
 IF (retention_days < 14)
  ...
  GO TO exit_script
 ENDIF
 SET reply->table_name = "DCP_ACTIVITY_LOG"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(dal.activity_log_id)
   FROM dcp_activity_log dal
   WHERE dal.activity_log_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(dal.activity_log_id)
  FROM dcp_activity_log dal
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM dcp_activity_log dal
    WHERE parser(sbr_getrowidnotexists("dal.activity_log_id between curMinID and curMaxID","dal"))
     AND dal.activity_dt_tm < cnvtdatetime(target_dt_tm)
    ...
    DETAIL
     ...
     ENDIF
     reply->rows[rows].row_id = dal.rowid
    WITH nocounter, maxqual(dal,value(rowsleft))
   ;end select
