CREATE PROGRAM dm2_mig_report:dba
 ...
 RECORD dmr_rpt(
   ...
   1 user[*]
     2 username = vc
     2 dtuser_ind = i2
     2 user_no_move_ind = i2
   1 tbl_cnt = i4
   1 tbl[*]
     2 owner = vc
     2 table_name = vc
     2 reason_cd = i4
     2 tbl_no_move_ind = i2
 )
 DECLARE dmr_users = vc WITH protect, noconstant("")
 DECLARE dmr_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmr_disp_header = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_report",".log","dm2_mig_report LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_report"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Determining migration user list from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE ((di.info_domain IN ("DM2_MIG_USER", "DM2_MIG_DT_USER")) OR (di.info_domain IN (
  "DM2_MIG_EXCL")
   AND di.info_number != 16))
  ORDER BY di.info_domain, di.info_name
  DETAIL
   IF (di.info_domain IN ("DM2_MIG_USER", "DM2_MIG_DT_USER"))
    dmr_rpt->user_cnt = (dmr_rpt->user_cnt+ 1), stat = alterlist(dmr_rpt->user,dmr_rpt->user_cnt),
    dmr_rpt->user[dmr_rpt->user_cnt].username = di.info_name,
    dmr_rpt->user[dmr_rpt->user_cnt].dtuser_ind = evaluate(di.info_domain,"DM2_MIG_DT_USER",1,0),
    dmr_rpt->user[dmr_rpt->user_cnt].user_no_move_ind = evaluate(di.info_char,"0",1,0)
   ELSE
    dmr_rpt->tbl_cnt = (dmr_rpt->tbl_cnt+ 1), stat = alterlist(dmr_rpt->tbl,dmr_rpt->tbl_cnt),
    dmr_rpt->tbl[dmr_rpt->tbl_cnt].owner = substring(1,(findstring(".",di.info_name) - 1),di
     .info_name),
    dmr_rpt->tbl[dmr_rpt->tbl_cnt].table_name = substring((findstring(".",di.info_name)+ 1),size(di
      .info_name),di.info_name), dmr_rpt->tbl[dmr_rpt->tbl_cnt].reason_cd = di.info_number, dmr_rpt->
    tbl[dmr_rpt->tbl_cnt].tbl_no_move_ind = evaluate(di.info_char,"0",1,0)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmr_rpt)
 ENDIF
 SET dm_err->eproc = "Determining migration user list from dba_users."
 SELECT INTO "nl:"
  FROM dba_users dt
  WHERE dt.username="C##*"
  ...
  DETAIL
   dmr_rpt->user_cnt = (dmr_rpt->user_cnt+ 1), stat = alterlist(dmr_rpt->user,dmr_rpt->user_cnt),
   dmr_rpt->user[dmr_rpt->user_cnt].username = dt.username,
   dmr_rpt->user[dmr_rpt->user_cnt].user_no_move_ind = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmr_rpt)
 ENDIF
 IF (get_unique_file("dm2migreport",".rpt")=0)
  GO TO exit_script
 ENDIF
 ...
 SELECT INTO value(cnvtlower(dm_err->unique_fname))
  ...
  HEAD REPORT
   ...
   "**************************************************************************************************************",
   row + 1,
   col 0, "Cerner Database Migration Downtime Report", row + 1,
   col 0,
   "**************************************************************************************************************",
   row + 2
  DETAIL
   dmr_disp_header = 1
   FOR (dmr_cnt = 1 TO dmr_rpt->user_cnt)
     IF ((dmr_rpt->user[dmr_cnt].dtuser_ind=0)
      AND (dmr_rpt->user[dmr_cnt].user_no_move_ind=0))
      IF (dmr_disp_header=1)
       CALL print("Database Migration Users moved in UPTIME:"), row + 1,
       CALL print("-----------------------------------------"),
       row + 1, dmr_disp_header = 0
      ENDIF
      CALL print(dmr_rpt->user[dmr_cnt].username), row + 1
     ENDIF
   ENDFOR
   row + 2, dmr_disp_header = 1
   FOR (dmr_cnt = 1 TO dmr_rpt->user_cnt)
     IF ((dmr_rpt->user[dmr_cnt].dtuser_ind=1)
      AND (dmr_rpt->user[dmr_cnt].user_no_move_ind=0))
      IF (dmr_disp_header=1)
       CALL print("Database Migration Users moved in DOWNTIME:"), row + 1,
       CALL print("-------------------------------------------"),
       row + 1, dmr_disp_header = 0
      ENDIF
      CALL print(dmr_rpt->user[dmr_cnt].username), row + 1
   ...
   ENDFOR
   row + 2, dmr_disp_header = 1
   FOR (dmr_cnt = 1 TO dmr_rpt->tbl_cnt)
     IF ((dmr_rpt->tbl[dmr_cnt].tbl_no_move_ind=0)
      AND (dmr_rpt->tbl[dmr_cnt].reason_cd < 40))
      IF (dmr_disp_header=1)
       CALL print("Individual tables moved in DOWNTIME and reason for not moving in UPTIME:"), row +
       1,
       CALL print("------------------------------------------------------------------------"),
       row + 2, col 0, "Owner",
       col 20, "Table", col 51,
       ...
       dmr_disp_header = 0
      ENDIF
      col 0, dmr_rpt->tbl[dmr_cnt].owner, col 20,
      dmr_rpt->tbl[dmr_cnt].table_name
      CASE (dmr_rpt->tbl[dmr_cnt].reason_cd)
       OF 15:
        col 51,"Table manually marked for downtime migration."
       OF 18:
        col 51,"Table contains column names which are duplicated."
       OF 19:
        col 51,"Table contains column names which have same name as oracle data type."
       OF 20:
        col 51,"Table contains invalid migration data types."
      ENDCASE
      row + 1
     ENDIF
   ENDFOR
   row + 2, dmr_disp_header = 1
   FOR (dmr_cnt = 1 TO dmr_rpt->user_cnt)
     IF ((dmr_rpt->user[dmr_cnt].user_no_move_ind=1))
      IF (dmr_disp_header=1)
       CALL print("Database Migration Users NOT being moved:"), row + 1,
       CALL print("-----------------------------------------"),
       row + 2, col 0, "User",
       col 20, "Reason", row + 1,
       col 0, "-----", col 20,
      ...
      ENDIF
      col 0,
      CALL print(dmr_rpt->user[dmr_cnt].username)
      IF (substring(1,3,dmr_rpt->user[dmr_cnt].username)="C##")
       col 20,
       CALL print("User follows naming convention of a common user and will not be moved")
      ENDIF
      row + 1
     ENDIF
   ENDFOR
   row + 2, dmr_disp_header = 1
   FOR (dmr_cnt = 1 TO dmr_rpt->tbl_cnt)
     IF ((dmr_rpt->tbl[dmr_cnt].tbl_no_move_ind=1)
      AND (dmr_rpt->tbl[dmr_cnt].reason_cd <= 40))
      IF (dmr_disp_header=1)
       CALL print("Individual tables NOT being moved and reason for not moving:"), row + 1,
       CALL print("----------------------------------------------------------------------"),
       row + 2, col 0, "Owner",
       col 20, "Table", col 51,
       "Reason", row + 1, col 0,
      ...
      CASE (dmr_rpt->tbl[dmr_cnt].reason_cd)
       OF 17:
        col 51,"Materialized view object - automatically marked not to be moved."
       OF 21:
        col 51,"External Table - automatically marked not to be moved"
       OF 22:
        col 51,"Table with space in name - automatically marked not to be moved"
       OF 23:
        col 51,"Table owned by GG - automatically marked not to be moved"
       OF 24:
        col 51,"Install Tools DDL table - automatically marked not to be moved"
       ELSE
        col 51,"Table manually marked not to be moved."
      ENDCASE
      row + 1
     ENDIF
   ENDFOR
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 IF (dm2_disp_file(dm_err->unique_fname,"Database Migration Report")=0)
  GO TO exit_script
 ENDIF
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_report completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_report")
END GO
