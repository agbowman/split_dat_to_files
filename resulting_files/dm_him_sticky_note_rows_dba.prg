CREATE PROGRAM dm_him_sticky_note_rows:dba
 ...
 DECLARE mf_permanent_cd = f8 WITH protect, noconstant(0.0)
 ...
 SET mf_permanent_cd = uar_get_code_by("MEANING",14389,"PERMANENT")
 ...
 IF (v_days_to_keep < 2190)
  ...
  GO TO exit_script
 ELSE
  IF (mf_permanent_cd=0)
   SET reply->err_code = - (1)
   SET reply->status = "F"
   SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"s3",
    "Unable to find code value by CDF meaning 'PERMANENT' in code set 14389")
   GO TO exit_script
  ELSE
   IF (batch_ndx=1)
    SELECT INTO "nl:"
     minval = min(sn.sticky_note_id)
     FROM sticky_note sn
   ...
   WHILE (v_cur_min_id <= v_max_id
    AND v_rows_left > 0)
     SELECT INTO "nl:"
      FROM sticky_note sn
      WHERE parser(sbr_getrowidnotexists("sn.updt_dt_tm < cnvtdatetime(v_days_back)","sn"))
       AND sn.sticky_note_id BETWEEN v_cur_min_id AND v_cur_max_id
       AND ((sn.sticky_note_status_cd+ 0) != mf_permanent_cd)
      DETAIL
       v_rows = (v_rows+ 1)
       IF (mod(v_rows,100)=1)
        stat = alterlist(reply->rows,(v_rows+ 99))
       ENDIF
       reply->rows[v_rows].row_id = sn.rowid
      WITH nocounter, maxqual(sn,value(v_rows_left))
     ;end select
