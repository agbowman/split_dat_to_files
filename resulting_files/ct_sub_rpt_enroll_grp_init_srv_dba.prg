CREATE PROGRAM ct_sub_rpt_enroll_grp_init_srv:dba
 PROMPT
 ...
 DECLARE _bcontheadinit_service_cdsection = i2 WITH noconstant(0), protect
 ...
 DECLARE _remoff_study_date = i4 WITH noconstant(1), protect
 DECLARE _remtotal_count = i4 WITH noconstant(1), protect
 DECLARE _bcontfootinit_service_cdsection = i2 WITH noconstant(0), protect
 ...
 DECLARE _remtotal_init_sites = i4 WITH noconstant(1), protect
 ...
 SUBROUTINE get_enroll_site(dummy)
   SELECT
    IF (( $GROUPBY=1))
     ...
     reportlist_order_by_1 = parser(reportlist->order_by_1),
     reportlist_order_by_2 = parser(reportlist->order_by_2)
     ...
     ORDER BY cnvtlower(uar_get_code_display(results->enrollments[d.seq].initiating_service_cd)),
      reportlist_order_by_1, reportlist_order_by_2
    ...
    PLAN (d)
     JOIN (d2
     WHERE (results->pis[d2.seq].prot_master_id=results->enrollments[d.seq].prot_master_id))
    ORDER BY init_service_cd, prot_id, site_id,
     person_id, pi_id
    HEAD REPORT
    ...
    HEAD init_service_cd
     ...
     off_study_total = 0, enroll_total = 0, stat = alterlist(countlist->pis,0),
     pi_cnt = 0, stat = alterlist(countlist->enroll_orgs,0), site_cnt = 0,
     stat = alterlist(countlist->protocols,0), prot_cnt = 0, tmp_init_svc = trim(uar_get_code_display
      (results->enrollments[d.seq].initiating_service_cd)),
     _bcontheadinit_service_cdsection = 0, bfirsttime = 1
     WHILE (((_bcontheadinit_service_cdsection=1) OR (bfirsttime=1)) )
       _bholdcontinue = _bcontheadinit_service_cdsection, _fdrawheight = headinit_service_cdsection(
       ...
       IF (((_bholdcontinue=1) OR (_fdrawheight > 0)) )
        IF ((_fenddetail >= (_yoffset+ _fdrawheight)))
         _fdrawheight += headinit_service_cdsection_line(rpt_calcheight)
        ENDIF
       ENDIF
       IF (((_yoffset+ _fdrawheight) > _fenddetail))
        BREAK
       ELSEIF (_bholdcontinue=1
        AND _bcontheadinit_service_cdsection=0)
        BREAK
       ENDIF
       dummy_val = headinit_service_cdsection(rpt_render,(_fenddetail - _yoffset),
        _bcontheadinit_service_cdsection), bfirsttime = 0
     ENDWHILE
     _fdrawheight = headinit_service_cdsection_line(rpt_calcheight)
     IF (((_yoffset+ _fdrawheight) > _fenddetail))
      BREAK
     ENDIF
     dummy_val = headinit_service_cdsection_line(rpt_render)
    HEAD prot_id
    ...
    FOOT  site_id
     ...
     tmp_prot = trim(results->enrollments[d.seq].primary_mnemonic), tmp_site = trim(results->
      enrollments[d.seq].enroll_org_name), on_study_total += on_study_cnt,
     off_study_total += off_study_cnt, off_tx_total += off_tx_cnt, enroll_total += total_cnt,
    ...
    FOOT  prot_id
     row + 0
    FOOT  init_service_cd
     _bcontfootinit_service_cdsection = 0, bfirsttime = 1
     WHILE (((_bcontfootinit_service_cdsection=1) OR (bfirsttime=1)) )
       _bholdcontinue = _bcontfootinit_service_cdsection, _fdrawheight = footinit_service_cdsection(
        rpt_calcheight,(_fenddetail - _yoffset),_bholdcontinue)
       IF (((_yoffset+ _fdrawheight) > _fenddetail))
        BREAK
       ELSEIF (_bholdcontinue=1
        AND _bcontfootinit_service_cdsection=0)
        BREAK
       ENDIF
       dummy_val = footinit_service_cdsection(rpt_render,(_fenddetail - _yoffset),
        _bcontfootinit_service_cdsection), bfirsttime = 0
     ENDWHILE
    FOOT PAGE
    ...
    FOOT REPORT
     tempstr = concat(label->total_init," ",trim(cnvtstring(init_svc_cnt,3))),
 ...
 SUBROUTINE (headpagesection_lineabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (headinit_service_cdsection(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headinit_service_cdsectionabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (headinit_service_cdsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(
   ref)) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.630000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_init_srv = f8 WITH noconstant(0.0), private
   DECLARE __init_srv = vc WITH noconstant(build2(tmp_init_svc,char(0))), protect
   IF (bcontinue=0)
    SET _reminit_srv = 1
   ENDIF
   SET rptsd->m_flags = 5
   ...
   IF (_reminit_srv > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_reminit_srv,((size(__init_srv) -
     ...
     SET _reminit_srv += rptsd->m_drawlength
    ELSE
     SET _reminit_srv = 0
    ENDIF
    SET growsum += _reminit_srv
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (headinit_service_cdsection_line(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headinit_service_cdsection_lineabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (headinit_service_cdsection_lineabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.060000), private
   IF (ncalc=rpt_render)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
 ...
 SUBROUTINE (footsite_idsection1abs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =
  f8 WITH protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (footinit_service_cdsection(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = footinit_service_cdsectionabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (footinit_service_cdsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(
   ref)) =f8 WITH protect)
 ...
 SUBROUTINE (footreportsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
  WITH protect)
   ...
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_total_init_sites = f8 WITH noconstant(0.0), private
   DECLARE drawheight_label_end_of_rpt = f8 WITH noconstant(0.0), private
   DECLARE __total_init_sites = vc WITH noconstant(build2(tempstr,char(0))), protect
   DECLARE __label_end_of_rpt = vc WITH noconstant(build2(label->end_of_rpt,char(0))), protect
   IF (bcontinue=0)
    SET _remtotal_init_sites = 1
   ...
   SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
   SET _holdremtotal_init_sites = _remtotal_init_sites
   IF (_remtotal_init_sites > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_remtotal_init_sites,((
       size(__total_init_sites) - _remtotal_init_sites)+ 1),__total_init_sites)))
    SET drawheight_total_init_sites = rptsd->m_height
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _remtotal_init_sites = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remtotal_init_sites,((size(
        __total_init_sites) - _remtotal_init_sites)+ 1),__total_init_sites)))))
     SET _remtotal_init_sites += rptsd->m_drawlength
    ELSE
     SET _remtotal_init_sites = 0
    ENDIF
    SET growsum += _remtotal_init_sites
   ENDIF
   SET rptsd->m_flags = 21
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   SET rptsd->m_x = (offsetx+ 0.000)
   SET rptsd->m_width = 10.500
   SET rptsd->m_height = drawheight_total_init_sites
   SET _dummyfont = uar_rptsetfont(_hreport,_courier9b0)
   IF (ncalc=rpt_render
    AND _holdremtotal_init_sites > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdremtotal_init_sites,(
       (size(__total_init_sites) - _holdremtotal_init_sites)+ 1),__total_init_sites)))
   ELSE
    SET _remtotal_init_sites = _holdremtotal_init_sites
   ENDIF
   SET rptsd->m_flags = 20
   IF (bcontinue)
    SET rptsd->m_y = offsety
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CT_SUB_RPT_ENROLL_GRP_INIT_SRV"
