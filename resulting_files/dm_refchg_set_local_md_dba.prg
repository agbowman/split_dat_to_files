CREATE PROGRAM dm_refchg_set_local_md:dba
 ...
 DECLARE v_local_cols = vc WITH protect, noconstant("")
 DECLARE v_admin_cols = vc WITH protect, noconstant("")
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: Starting dm_refchg_set_local_md script."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="DM_ENV_ID"
  DETAIL
   v_cur_env_id = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 IF (check_open_event(v_cur_env_id,0.0) > 0)
  SET v_req_size = size(requestin->list_0,5)
  FOR (v_idx = 1 TO v_req_size)
    SET v_local_cols = replace(requestin->list_0[v_idx].column_list,"X.","dcdl.",0)
    SET v_admin_cols = replace(requestin->list_0[v_idx].column_list,"X.","dcd.",0)
    SET dm_err->eproc = "Updating local meta-data from admin."
    CALL parser("update into dm_columns_doc_local dcdl",0)
    CALL parser(concat("   set (",v_local_cols,")"),0)
    CALL parser(concat("       (select ",v_admin_cols),0)
    CALL parser("          from dm_columns_doc dcd",0)
    CALL parser("         where dcd.table_name = dcdl.table_name ",0)
    CALL parser("           and dcd.column_name = dcdl.column_name)",0)
    CALL parser(" where list(dcdl.table_name, dcdl.column_name)",0)
    CALL parser("       IN(select d.table_name,",0)
    CALL parser("                 d.column_name",0)
    CALL parser("            from dm_columns_doc d",0)
    CALL parser("           where d.table_name = requestin->list_0[v_idx].table_name",0)
    CALL parser("             and d.column_name = requestin->list_0[v_idx].column_name)",0)
  ...
  SET readme_data->message = "Local Meta-data was updated successfully."
 ELSE
  SET readme_data->status = "S"
  SET readme_data->message = "There is no event open local meta-data update was not needed."
 ENDIF
#exit_script
