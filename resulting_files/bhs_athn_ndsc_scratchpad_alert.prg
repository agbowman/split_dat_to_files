CREATE PROGRAM bhs_athn_ndsc_scratchpad_alert
 FREE RECORD result
 RECORD result(
   1 ndsc_user = vc
   1 ndsc_pass = vc
 ...
 SELECT INTO "NL:"
  ...
  PLAN (oc
   WHERE (oc.catalog_cd= $9)
    AND oc.active_ind=1)
  HEAD oc.catalog_cd
   CALL echo(build("ORDERABLE: ",uar_get_code_display(oc.catalog_cd))), result->catalog_type_cd = oc
   .catalog_type_cd, result->activity_type_cd = oc.activity_type_cd,
...
#exit_script
 ...
 IF (size(trim(moutputdevice,3)) > 0
  AND (result->status_data.status="S"))
  ...
  SELECT INTO value(moutputdevice)
   ...
   HEAD REPORT
    ...
    row + 1, v1 = build("<OrderID>",cnvtint(result->order_id),"</OrderID>"), col + 1,
    v1, row + 1, v2 = build("<SessionInfo>",trim(replace(replace(replace(replace(replace(result->
           session_info_json,"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",
       0),3),"</SessionInfo>"),
    col + 1, v2, row + 1,
