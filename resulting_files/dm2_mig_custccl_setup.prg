CREATE PROGRAM dm2_mig_custccl_setup
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmccs_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_setup")
 DECLARE dmccs_type = vc WITH protect, noconstant("")
 DECLARE dmccs_cont = i2 WITH protect, noconstant(1)
 DECLARE dmccs_env = vc WITH protect, noconstant("")
 DECLARE dmccs_node = vc WITH protect, noconstant("")
 DECLARE dmccs_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccs_os_type = vc WITH protect, noconstant("")
 DECLARE dmccs_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccs_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmccs_ndx = i4 WITH protect, noconstant(0)
 DECLARE dmccs_loc = i4 WITH protect, noconstant(0)
 DECLARE dmccs_num_ins = i4 WITH protect, noconstant(0)
 DECLARE dmccs_num_upd = i4 WITH protect, noconstant(0)
 DECLARE dmccs_num_del = i4 WITH protect, noconstant(0)
 FREE RECORD dmccs_users
 RECORD dmccs_users(
   1 cnt = i4
   1 qual[*]
     2 user_name = vc
 ...
 FREE RECORD dmccs_scripts
 RECORD dmccs_scripts(
   1 cnt = i4
   1 qual[*]
     2 script_name = vc
     2 object_type = vc
     ...
     2 user_name = vc
     2 group_nbr = i4
     2 last_compile_dt_tm = dq8
     2 datestamp_txt = vc
     2 timestamp_txt = vc
     2 binary_cnt = f8
     2 app_major_version = f8
     2 app_minor_version = f8
     2 ccl_version = i4
     2 custom_ind = i2
     2 op_flag = i2
 )
 DECLARE dmccs_get_users(null) = i2
 DECLARE dmccs_get_scripts(null) = i2
 SET width = 132
 IF (check_logfile(dmccs_logfile_prefix,".log","dm2_mig_custccl_setup")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_setup"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Validating inputs for dm2_mig_custccl_setup"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmccs_type = cnvtupper( $1)
 IF (((check_error(dm_err->eproc) != 0) OR (dmccs_type != "MASTER"
  AND dmccs_type != "TARGET")) )
  SET dm_err->emsg = "Parameter usage: dm2_mig_custccl_setup '<execution type (MASTER/TARGET)>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dmccs_env = cnvtupper(trim(logical("environment")))
 SET dmccs_node = trim(curnode)
 SET dmccs_os_type = trim(cursys2)
 SET dm_err->eproc = concat("Obtaining environment ID for current environment: ",dmccs_env)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE de.environment_name=dmccs_env
  DETAIL
   dmccs_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->emsg = "No entry for current environment exists in dm_environment"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dmccs_env))
  CALL echo(concat("Current Environment Id: ",cnvtstring(dmccs_env_id)))
  CALL echo(concat("Current Node: ",dmccs_node))
  CALL echo(concat("Current Environment: ",dmccs_os_type))
 ENDIF
 IF (dmccs_type="MASTER")
  SET dm_err->eproc = "Validating execution with MASTER type"
  SELECT INTO "nl:"
   FROM dma_env_node_attr dena,
    dma_env_node dne
   WHERE dena.attr_name="CCMIG_SETUPTYPE"
    AND dena.attr_value_txt="MASTER"
    AND dena.dma_env_node_id=dne.dma_env_node_id
   DETAIL
    IF (cnvtlower(dne.node_name)=cnvtlower(dmccs_node)
     AND dne.environment_id=dmccs_env_id)
     dmccs_cont = 1
    ELSE
     dmccs_cont = 0
    ENDIF
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Validating that MASTER has already been setup"
  SELECT INTO "nl:"
   FROM dma_env_node_attr dena,
    dma_env_node dne
   WHERE dena.attr_name="CCMIG_SETUPTYPE"
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg =
   "Setup must first be performed against a MASTER environment before executing for a TARGET environment"
   SET dm_err->user_action = "Run setup on MASTER node first."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmccs_cont=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "A different MASTER has already been set in dma_env_node_attr."
  SET dm_err->user_action =
  "If you wish to use a new MASTER, refer to Custom CCL Migration Documentation."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Insert new rows into custom ccl admin tables"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Insert new row into dma_env_node admin table (if row does not exist)"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dma_env_node dne
  WHERE dne.node_name=dmccs_node
   AND dne.environment_id=dmccs_env_id
  DETAIL
   dmccs_id = dne.dma_env_node_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Insert new row into dma_env_node_id"
  INSERT  FROM dma_env_node dne
   SET dne.dma_env_node_id = seq(dm_seq,nextval), dne.node_name = dmccs_node, dne.environment_id =
    dmccs_env_id,
    dne.os_system_type = dmccs_os_type
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Retrieve dma_env_node_id just inserted"
 SELECT INTO "nl:"
  FROM dma_env_node dne
  WHERE dne.node_name=dmccs_node
   AND dne.environment_id=dmccs_env_id
 ...
 IF (dmccs_id=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Current node not setup properly"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc =
 "Insert new rows into dma_env_node_attr admin table for type (if row does not exist)"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dma_env_node_attr dena
  WHERE dena.dma_env_node_id=dmccs_id
   AND dena.attr_name="CCMIG_SETUPTYPE"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inserting row for node/env type"
  INSERT  FROM dma_env_node_attr dena
   SET dena.dma_attr_id = seq(dm_seq,nextval), dena.dma_env_node_id = dmccs_id, dena.attr_name =
    "CCMIG_SETUPTYPE",
    dena.attr_value_txt = dmccs_type
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  COMMIT
 ELSE
  SET dm_err->eproc = "Updating row for node/env type"
  UPDATE  FROM dma_env_node_attr dena
   SET dena.attr_value_txt = dmccs_type
   WHERE dena.dma_env_node_id=dmccs_id
    AND dena.attr_name="CCMIG_SETUPTYPE"
   WITH nocounter
  ;end update
 ...
 ENDIF
 SET dm_err->eproc =
 "Insert new rows into dma_env_node_attr admin table for OS type (if row does not exist)"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dma_env_node_attr dena
  WHERE dena.dma_env_node_id=dmccs_id
   AND dena.attr_name="CCMIG_OS_TYPE"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inserting row for node/env OS type"
  INSERT  FROM dma_env_node_attr dena
   SET dena.dma_attr_id = seq(dm_seq,nextval), dena.dma_env_node_id = dmccs_id, dena.attr_name =
    "CCMIG_OS_TYPE",
    dena.attr_value_txt = dmccs_os_type
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  COMMIT
 ELSE
  SET dm_err->eproc = "Updating row for node/env OS type"
  UPDATE  FROM dma_env_node_attr dena
   SET dena.attr_value_txt = dmccs_os_type
   WHERE dena.dma_env_node_id=dmccs_id
    AND dena.attr_name="CCMIG_OS_TYPE"
   WITH nocounter
  ;end update
 ...
 ENDIF
 SET dm_err->eproc =
 "Insert new rows into dma_env_node_attr admin table for Environment Name (if row does not exist)"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dma_env_node_attr dena
  WHERE dena.dma_env_node_id=dmccs_id
   AND dena.attr_name="CCMIG_ENV_NAME"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inserting row for node/env Environment Name"
  INSERT  FROM dma_env_node_attr dena
   SET dena.dma_attr_id = seq(dm_seq,nextval), dena.dma_env_node_id = dmccs_id, dena.attr_name =
    "CCMIG_ENV_NAME",
    dena.attr_value_txt = dmccs_env
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  COMMIT
 ELSE
  SET dm_err->eproc = "Updating row for node/env Environment Name"
  UPDATE  FROM dma_env_node_attr dena
   SET dena.attr_value_txt = dmccs_env
   WHERE dena.dma_env_node_id=dmccs_id
    AND dena.attr_name="CCMIG_ENV_NAME"
   WITH nocounter
  ;end update
  ...
  COMMIT
 ENDIF
 IF (dmccs_get_users(null)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmccs_users)
 ENDIF
 SET dmccs_num_ins = dmccs_users->cnt
 SET dmccs_num_upd = 0
 SET dmccs_num_del = 0
 SET dm_err->eproc = "Determining existance of users in admin table"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dma_env_node_duaf dend
  WHERE dend.dma_env_node_id=dmccs_id
  DETAIL
   dmccs_loc = locateval(dmccs_ndx,1,dmccs_users->cnt,dend.user_name,dmccs_users->qual[dmccs_ndx].
    user_name,
    dend.group_nbr,dmccs_users->qual[dmccs_ndx].group_nbr)
   IF (dmccs_loc=0)
    dmccs_users->cnt = (dmccs_users->cnt+ 1), stat = alterlist(dmccs_users->qual,dmccs_users->cnt),
    dmccs_users->qual[dmccs_users->cnt].user_name = dend.user_name,
    dmccs_users->qual[dmccs_users->cnt].group_nbr = dend.group_nbr, dmccs_users->qual[dmccs_users->
    cnt].group_name = dend.group_name, dmccs_users->qual[dmccs_users->cnt].op_flag = 3,
    dmccs_num_del = (dmccs_num_del+ 1), dmccs_num_ins = (dmccs_num_ins - 1)
   ELSE
    IF ((dmccs_users->qual[dmccs_loc].user_name=dend.user_name)
     AND (dmccs_users->qual[dmccs_loc].group_nbr=dend.group_nbr)
     AND (dmccs_users->qual[dmccs_loc].group_name=dend.group_name)
     AND (dmccs_users->qual[dmccs_loc].create_dt_tm=cnvtdatetime(dend.create_dt_tm))
     AND (dmccs_users->qual[dmccs_loc].datestamp_txt=dend.datestamp_txt)
     AND (dmccs_users->qual[dmccs_loc].timestamp_txt=dend.timestamp_txt)
     AND (dmccs_users->qual[dmccs_loc].cclcount_nbr=dend.cclcount_nbr))
     dmccs_users->qual[dmccs_loc].op_flag = 0
    ELSE
     dmccs_users->qual[dmccs_loc].op_flag = 2, dmccs_num_upd = (dmccs_num_upd+ 1), dmccs_num_ins = (
     dmccs_num_ins - 1)
    ENDIF
   ENDIF
  WITH nocounter
 ...
 ENDIF
 IF (dmccs_num_ins > 0)
  SET dm_err->eproc = "Inserting rows for new users"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  INSERT  FROM dma_env_node_duaf dend,
    (dummyt d  WITH seq = value(dmccs_users->cnt))
   SET dend.dma_duaf_id = seq(dm_seq,nextval), dend.dma_env_node_id = dmccs_id, dend.user_name =
    dmccs_users->qual[d.seq].user_name,
    dend.group_name = dmccs_users->qual[d.seq].group_name, dend.group_nbr = dmccs_users->qual[d.seq].
    group_nbr, dend.create_dt_tm = cnvtdatetime(dmccs_users->qual[d.seq].create_dt_tm),
    dend.datestamp_txt = dmccs_users->qual[d.seq].datestamp_txt, dend.timestamp_txt = dmccs_users->
    qual[d.seq].timestamp_txt, dend.cclcount_nbr = dmccs_users->qual[d.seq].cclcount_nbr
   PLAN (d
    WHERE d.seq > 0
     AND (dmccs_users->qual[d.seq].op_flag=1))
    JOIN (dend)
 ...
 ENDIF
 IF (dmccs_num_upd > 0)
  SET dm_err->eproc = "Updating rows for exisiting users"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  UPDATE  FROM dma_env_node_duaf dend,
    (dummyt d  WITH seq = value(dmccs_users->cnt))
   SET dend.group_nbr = dmccs_users->qual[d.seq].group_nbr, dend.create_dt_tm = cnvtdatetime(
     dmccs_users->qual[d.seq].create_dt_tm), dend.datestamp_txt = dmccs_users->qual[d.seq].
    datestamp_txt,
    dend.timestamp_txt = dmccs_users->qual[d.seq].timestamp_txt, dend.cclcount_nbr = dmccs_users->
    qual[d.seq].cclcount_nbr
   PLAN (d
    WHERE d.seq > 0
     AND (dmccs_users->qual[d.seq].op_flag=2))
    JOIN (dend
    WHERE dend.dma_env_node_id=dmccs_id
     AND (dend.user_name=dmccs_users->qual[d.seq].user_name)
     AND (dend.group_nbr=dmccs_users->qual[d.seq].group_nbr))
   WITH nocounter, maxcommit = 100
  ;end update
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
 ...
 ENDIF
 IF (dmccs_num_del > 0)
  SET dm_err->eproc = "Deleting rows for removed users"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dma_env_node_duaf dend,
    (dummyt d  WITH seq = value(dmccs_users->cnt))
   SET dend.seq = 1
   PLAN (d
    WHERE d.seq > 0
     AND (dmccs_users->qual[d.seq].op_flag=3))
    JOIN (dend
    WHERE dend.dma_env_node_id=dmccs_id
     AND (dend.user_name=dmccs_users->qual[d.seq].user_name)
     AND (dend.group_nbr=dmccs_users->qual[d.seq].group_nbr))
  ...
  ENDIF
 ENDIF
 IF (dmccs_type="TARGET")
  EXECUTE dm2_mig_custccl_bkup_dic
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
  EXECUTE dm2_mig_custccl_sync_duaf
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
  EXECUTE dm2_mig_custccl_dropdupccl
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmccs_get_scripts(null)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag=511))
  CALL echorecord(dmccs_scripts)
 ENDIF
 SET dmccs_num_ins = dmccs_scripts->cnt
 SET dmccs_num_upd = 0
 SET dmccs_num_del = 0
 SET dm_err->eproc = "Determining existance of scripts in admin table"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dma_env_node_dprotect dend
  WHERE dend.dma_env_node_id=dmccs_id
  DETAIL
   dmccs_loc = locateval(dmccs_ndx,1,dmccs_scripts->cnt,dend.object_name,dmccs_scripts->qual[
    dmccs_ndx].script_name,
    dend.group_nbr,dmccs_scripts->qual[dmccs_ndx].group_nbr)
   IF (dmccs_loc=0)
    dmccs_scripts->cnt = (dmccs_scripts->cnt+ 1), stat = alterlist(dmccs_scripts->qual,dmccs_scripts
     ->cnt), dmccs_scripts->qual[dmccs_scripts->cnt].script_name = dend.object_name,
    dmccs_scripts->qual[dmccs_scripts->cnt].object_type = dend.object_type_flag_txt, dmccs_scripts->
    qual[dmccs_scripts->cnt].user_name = dend.user_name, dmccs_scripts->qual[dmccs_scripts->cnt].
    group_nbr = dend.group_nbr,
    dmccs_scripts->qual[dmccs_scripts->cnt].op_flag = 3, dmccs_num_del = (dmccs_num_del+ 1),
    dmccs_num_ins = (dmccs_num_ins - 1)
   ELSE
    IF ((dmccs_scripts->qual[dmccs_loc].script_name=dend.object_name)
     AND (dmccs_scripts->qual[dmccs_loc].object_type=dend.object_type_flag_txt)
     AND (dmccs_scripts->qual[dmccs_loc].user_name=dend.user_name)
     AND (dmccs_scripts->qual[dmccs_loc].group_nbr=dend.group_nbr)
     AND (dmccs_scripts->qual[dmccs_loc].source_name=dend.source_name)
     AND (dmccs_scripts->qual[dmccs_loc].datestamp_txt=dend.datestamp_txt)
     AND (dmccs_scripts->qual[dmccs_loc].timestamp_txt=dend.timestamp_txt)
     AND (dmccs_scripts->qual[dmccs_loc].last_compile_dt_tm=cnvtdatetime(dend.last_compile_dt_tm))
     AND (dmccs_scripts->qual[dmccs_loc].binary_cnt=dend.binary_cnt)
     AND (dmccs_scripts->qual[dmccs_loc].app_major_version=dend.app_major_version_nbr)
     AND (dmccs_scripts->qual[dmccs_loc].app_minor_version=dend.app_minor_version_nbr)
     AND (dmccs_scripts->qual[dmccs_loc].ccl_version=dend.ccl_version_nbr))
     dmccs_scripts->qual[dmccs_loc].op_flag = 0
    ELSE
     dmccs_scripts->qual[dmccs_loc].op_flag = 2, dmccs_num_upd = (dmccs_num_upd+ 1), dmccs_num_ins =
     (dmccs_num_ins - 1)
    ENDIF
   ENDIF
  WITH nocounter
 ...
 ENDIF
 IF (dmccs_num_ins > 0)
  SET dm_err->eproc = "Inserting rows for new scripts"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  INSERT  FROM dma_env_node_dprotect dend,
    (dummyt d  WITH seq = value(dmccs_scripts->cnt))
   SET dend.dma_dprotect_id = seq(dm_seq,nextval), dend.dma_env_node_id = dmccs_id, dend
    .object_type_flag_txt = dmccs_scripts->qual[d.seq].object_type,
    dend.object_name = dmccs_scripts->qual[d.seq].script_name, dend.group_nbr = dmccs_scripts->qual[d
    .seq].group_nbr, dend.user_name = dmccs_scripts->qual[d.seq].user_name,
    dend.source_name = dmccs_scripts->qual[d.seq].source_name, dend.last_compile_dt_tm = cnvtdatetime
    (dmccs_scripts->qual[d.seq].last_compile_dt_tm), dend.datestamp_txt = dmccs_scripts->qual[d.seq].
    datestamp_txt,
    dend.timestamp_txt = dmccs_scripts->qual[d.seq].timestamp_txt, dend.binary_cnt = dmccs_scripts->
    qual[d.seq].binary_cnt, dend.app_major_version_nbr = dmccs_scripts->qual[d.seq].app_major_version,
    dend.app_minor_version_nbr = dmccs_scripts->qual[d.seq].app_minor_version, dend.ccl_version_nbr
     = dmccs_scripts->qual[d.seq].ccl_version, dend.custom_ind = dmccs_scripts->qual[dmccs_cnt].
    custom_ind
   PLAN (d
    WHERE d.seq > 0
     AND (dmccs_scripts->qual[d.seq].op_flag=1))
    JOIN (dend)
 ...
 ENDIF
 IF (dmccs_num_upd > 0)
  SET dm_err->eproc = "Updating rows for existing scripts"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  UPDATE  FROM dma_env_node_dprotect dend,
    (dummyt d  WITH seq = value(dmccs_scripts->cnt))
   SET dend.user_name = dmccs_scripts->qual[d.seq].user_name, dend.source_name = dmccs_scripts->qual[
    d.seq].source_name, dend.last_compile_dt_tm = cnvtdatetime(dmccs_scripts->qual[d.seq].
     last_compile_dt_tm),
    dend.datestamp_txt = dmccs_scripts->qual[d.seq].datestamp_txt, dend.timestamp_txt = dmccs_scripts
    ->qual[d.seq].timestamp_txt, dend.binary_cnt = dmccs_scripts->qual[d.seq].binary_cnt,
    dend.app_major_version_nbr = dmccs_scripts->qual[d.seq].app_major_version, dend
    .app_minor_version_nbr = dmccs_scripts->qual[d.seq].app_minor_version, dend.ccl_version_nbr =
    dmccs_scripts->qual[d.seq].ccl_version
   PLAN (d
    WHERE d.seq > 0
     AND (dmccs_scripts->qual[d.seq].op_flag=2))
    JOIN (dend
    WHERE dend.dma_env_node_id=dmccs_id
     AND (dend.object_type_flag_txt=dmccs_scripts->qual[d.seq].object_type)
     AND (dend.object_name=dmccs_scripts->qual[d.seq].script_name)
     AND (dend.group_nbr=dmccs_scripts->qual[d.seq].group_nbr))
   WITH nocounter, maxcommit = 1000
  ;end update
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
 ...
 ENDIF
 IF (dmccs_num_del > 0)
  SET dm_err->eproc = "Deleting rows for removed scripts"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dma_env_node_dprotect dend,
    (dummyt d  WITH seq = value(dmccs_scripts->cnt))
   SET dend.seq = 1
   PLAN (d
    WHERE d.seq > 0
     AND (dmccs_scripts->qual[d.seq].op_flag=3))
    JOIN (dend
    WHERE dend.dma_env_node_id=dmccs_id
     AND (dend.object_type_flag_txt=dmccs_scripts->qual[d.seq].object_type)
     AND (dend.object_name=dmccs_scripts->qual[d.seq].script_name)
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE dmccs_get_users(null)
   SET dm_err->eproc = "Retrieving list of users from duaf."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM duaf d
    HEAD REPORT
     dmccs_users->cnt = 0, stat = alterlist(dmccs_users->qual,0)
    DETAIL
     dmccs_users->cnt = (dmccs_users->cnt+ 1), stat = alterlist(dmccs_users->qual,dmccs_users->cnt),
     dmccs_users->qual[dmccs_users->cnt].user_name = d.user_name,
     dmccs_users->qual[dmccs_users->cnt].group_nbr = d.group, dmccs_users->qual[dmccs_users->cnt].
     group_name = d.group_name, dmccs_users->qual[dmccs_users->cnt].create_dt_tm = cnvtdatetime(d
      .datestamp,d.timestamp),
     dmccs_users->qual[dmccs_users->cnt].datestamp_txt = format(d.datestamp,"MM/DD/YY;;D"),
     dmccs_users->qual[dmccs_users->cnt].timestamp_txt = format(d.timestamp,"HH:MM;;M"), dmccs_users
     ->qual[dmccs_users->cnt].cclcount_nbr = d.cclcount,
     dmccs_users->qual[dmccs_users->cnt].op_flag = 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmccs_get_scripts(null)
   SET dm_err->eproc = "Retrieving list of scripts from dprotect."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dprotect d
    WHERE d.object="P"
     AND d.group < 99
    HEAD REPORT
     dmccs_scripts->cnt = 0, stat = alterlist(dmccs_scripts->qual,0)
    DETAIL
     dmccs_scripts->cnt = (dmccs_scripts->cnt+ 1), stat = alterlist(dmccs_scripts->qual,dmccs_scripts
      ->cnt), dmccs_scripts->qual[dmccs_scripts->cnt].script_name = d.object_name,
     dmccs_scripts->qual[dmccs_scripts->cnt].object_type = d.object, dmccs_scripts->qual[
     dmccs_scripts->cnt].user_name = d.user_name, dmccs_scripts->qual[dmccs_scripts->cnt].source_name
      = d.source_name,
     dmccs_scripts->qual[dmccs_scripts->cnt].group_nbr = d.group, dmccs_scripts->qual[dmccs_scripts->
     cnt].datestamp_txt = format(d.datestamp,"MM/DD/YY;;D"), dmccs_scripts->qual[dmccs_scripts->cnt].
     timestamp_txt = format(d.timestamp,"HH:MM;;M"),
     dmccs_scripts->qual[dmccs_scripts->cnt].last_compile_dt_tm = cnvtdatetime(d.datestamp,d
      .timestamp), dmccs_scripts->qual[dmccs_scripts->cnt].binary_cnt = d.binary_cnt, dmccs_scripts->
     qual[dmccs_scripts->cnt].app_major_version = d.app_major_version,
     dmccs_scripts->qual[dmccs_scripts->cnt].app_minor_version = d.app_minor_version, dmccs_scripts->
     qual[dmccs_scripts->cnt].ccl_version = d.ccl_version, dmccs_scripts->qual[dmccs_scripts->cnt].
     custom_ind = 0,
     dmccs_scripts->qual[dmccs_scripts->cnt].op_flag = 1
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_setup"
 CALL final_disp_msg(dmccs_logfile_prefix)
END GO
