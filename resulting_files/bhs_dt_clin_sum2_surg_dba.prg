CREATE PROGRAM bhs_dt_clin_sum2_surg:dba
 DECLARE varlocation = vc WITH noconstant("  "), protect
 DECLARE varoutput = vc WITH noconstant(" "), protect
 IF (surg_loc_ops != " ")
  SET varoutput = ops_printer
  SET varlocation = surg_loc_ops
  CALL echo(build("varlocation =",varlocation))
  CALL echo(build("varoutput =",varoutput))
 ENDIF
 SET unit_cd = 0
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   WHERE ea.alias=format(cnvtstring(requestin->list_0[d1.seq].facility_account_no),
    "####################")
    AND ea.active_ind=1
    AND ea.encntr_alias_type_cd=fin_type)
   JOIN (e
  ...
  DETAIL
   ...
   act_pat->qual[cnt].filename = build(trim(substring(1,14,trim(cnvtlower(cnvtalphanum(p
         .name_last_key,2)),4)),3),"_",trim(substring(1,4,trim(cnvtlower(cnvtalphanum(p
         .name_first_key,2)),4)),3),".ps"), act_pat->qual[cnt].room = trim(requestin->list_0[d1.seq].
    room_id), act_pat->qual[cnt].fin = trim(requestin->list_0[d1.seq].facility_account_no),
   act_pat->qual[cnt].medical_record_no = trim(requestin->list_0[d1.seq].medical_record_no), act_pat
   ->qual[cnt].case_no = trim(requestin->list_0[d1.seq].case_no), act_pat->qual[cnt].diagnosis = trim
   (requestin->list_0[d1.seq].diagnosis),
   act_pat->qual[cnt].provider_short_name = trim(requestin->list_0[d1.seq].provider_short_name),
   act_pat->qual[cnt].case_comments = trim(requestin->list_0[d1.seq].case_comments), act_pat->qual[
   cnt].procedure_desc = trim(requestin->list_0[d1.seq].procedure_desc),
   act_pat->qual[cnt].abbr_codes_pat_type = trim(requestin->list_0[d1.seq].abbr_codes_pat_type),
   act_pat->qual[cnt].start_date = trim(requestin->list_0[d1.seq].start_date), act_pat->qual[cnt].
   start_time = substring(1,5,requestin->list_0[d1.seq].start_time),
   act_pat->qual[cnt].unit = trim(requestin->list_0[d1.seq].description), act_pat->qual[cnt].
   abbr_codes_anes_type = trim(requestin->list_0[d1.seq].abbr_codes_anes_type), act_pat->qual[cnt].
   create_date = trim(requestin->list_0[d1.seq].create_date),
   act_pat->qual[cnt].create_time = substring(1,8,requestin->list_0[d1.seq].create_time), act_pat->
   qual[cnt].stop_time = substring(1,8,requestin->list_0[d1.seq].stop_time), act_pat->qual[cnt].
   mrap_id = trim(requestin->list_0[d1.seq].mrap_id),
   act_pat->qual[cnt].main_procedure_id = trim(requestin->list_0[d1.seq].main_procedure_id)
  WITH nocounter
 ;end select
 CALL echo("&&&&&&&&&&&&&&&&&&&&")
 ...
 SELECT INTO "nl:"
  ...
  .seq].room,
  result_age = datetimediff(cnvtdatetime(curdate,curtime3),ce.event_end_dt_tm,3), visit_reason = trim
  (act_pat->qual[d.seq].visit_reason)
  ...
  ORDER BY unit, room, e.encntr_id,
   sec.section_seq, grpr.grouper_seq, dta.task_assay_seq,
   ce.event_cd, ce.valid_from_dt_tm DESC
  HEAD REPORT
   cnt = 0, cnt2 = 0, cnt3 = 0,
   cnt4 = 0, cnt5 = 0
  HEAD e.encntr_id
   IF (((dta.max_lookback_hours=0) OR (result_age <= dta.max_lookback_hours)) )
    cnt = (cnt+ 1), stat = alterlist(pat->qual,cnt), pat->qual[cnt].pid = e.person_id,
    pat->qual[cnt].eid = e.encntr_id, pat->qual[cnt].unit_room_bed = act_pat->qual[d.seq].unit, pat->
    qual[cnt].visit_reason = visit_reason,
    pat->qual[cnt].admit_dt = reg_date, pat->qual[cnt].fin = trim(act_pat->qual[d.seq].fin), pat->
    qual[cnt].mrn = trim(act_pat->qual[d.seq].medical_record_no),
    pat->qual[cnt].abbr_codes_pat_type = trim(act_pat->qual[d.seq].abbr_codes_pat_type), pat->qual[
    cnt].abbr_codes_anes_type = trim(act_pat->qual[d.seq].abbr_codes_anes_type), pat->qual[cnt].
    main_procedure_id = trim(act_pat->qual[d.seq].main_procedure_id),
    pat->qual[cnt].start_date = trim(act_pat->qual[d.seq].start_date), pat->qual[cnt].start_time =
    trim(act_pat->qual[d.seq].start_time), pat->qual[cnt].stop_time = trim(act_pat->qual[d.seq].
     stop_time),
    pat->qual[cnt].case_no = trim(act_pat->qual[d.seq].case_no), pat->qual[cnt].create_date = trim(
     act_pat->qual[d.seq].create_date), pat->qual[cnt].create_time = trim(act_pat->qual[d.seq].
     create_time),
    pat->qual[cnt].room_id = trim(act_pat->qual[d.seq].room), pat->qual[cnt].case_comments = trim(
     act_pat->qual[d.seq].case_comments), pat->qual[cnt].provider_short_name = trim(act_pat->qual[d
     .seq].provider_short_name),
    pat->qual[cnt].diagnosis = trim(act_pat->qual[d.seq].diagnosis), pat->qual[cnt].procedure_desc =
    trim(act_pat->qual[d.seq].procedure_desc)
   ENDIF
   cnt2 = 0
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   pat->qual[d.seq].dob = p.birth_dt_tm, pat->qual[d.seq].filename = build(trim(substring(1,14,trim(
       cnvtlower(cnvtalphanum(p.name_last_key,2)),4)),3),"_",trim(substring(1,4,trim(cnvtlower(
        cnvtalphanum(p.name_first_key,2)),4)),3),".ps"), pat->qual[d.seq].birth_dt = p.birth_dt_tm
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(pat_cnt)),
 ...
 SELECT INTO "nl:"
  ...
  ELSEIF (o.catalog_cd=radiology_cd) 10
  ELSE 99
  ENDIF
  FROM (dummyt dd  WITH seq = value(pat_cnt)),
   orders o
  PLAN (dd)
   ...
   anatomicpathology_cd, bloodbank_cd, bloodbankmlh_cd, bloodbankbwh_cd, cardiactxprocedures_cd,
   ...
   IF ((pat->qual[x].filename > " "))
    SELECT INTO value(pat->qual[x].filename)
     ...
     HEAD PAGE
      ...
      row + 1, "{pos/220/45}{b}Downtime Clinical Summary Report (Ops)", row + 1,
      ...
      xcol = 500, temp1 = concat("Page :",cnvtstring(curpage)),
      CALL print(calcpos(xcol,ycol)),
      temp1, row + 1, xcol = 5,
      ycol = (ycol+ 10),
      CALL print(calcpos(xcol,ycol)), line,
      row + 1, xcol = 30, ycol = (ycol+ 10),
      CALL print(calcpos(xcol,ycol)), "OR #: ", pat->qual[x].room_id,
      row + 1, xcol = 200,
      CALL print(calcpos(xcol,ycol)),
      "Acc Nbr: ", pat->qual[x].fin, row + 1,
      xcol = 30, ycol = (ycol+ 10),
      CALL print(calcpos(xcol,ycol)),
      "Name: ", pat->qual[x].name, row + 1,
      xcol = 200,
      CALL print(calcpos(xcol,ycol)), "MR Nbr: ",
      pat->qual[x].mrn, row + 1, xcol = 350,
      CALL print(calcpos(xcol,ycol)), "Case #: ", pat->qual[x].case_no,
      row + 1, xcol = 500, dob = format(pat->qual[x].dob,"mm/dd/yy ;;q"),
      CALL print(calcpos(xcol,ycol)), "D.O.B: ", dob,
      row + 1, ycol = (ycol+ 10), xcol = 30,
      CALL print(calcpos(xcol,ycol)), "Surgeon.: ", pat->qual[x].provider_short_name,
      row + 1, xcol = 200,
      CALL print(calcpos(xcol,ycol)),
      "Anesthesia Type: ", pat->qual[x].abbr_codes_anes_type, row + 1,
      xcol = 350,
      CALL print(calcpos(xcol,ycol)), "Patient Type: ",
      pat->qual[x].abbr_codes_pat_type, row + 1, xcol = 30,
      ycol = (ycol+ 10),
      CALL print(calcpos(xcol,ycol)), "Start Date: ",
      pat->qual[x].start_date, row + 1, xcol = 200,
      CALL print(calcpos(xcol,ycol)), "Start Time: ", pat->qual[x].start_time,
      row + 1, xcol = 350,
      CALL print(calcpos(xcol,ycol)),
      "Stop Time: ", pat->qual[x].stop_time, row + 1,
      ycol = (ycol+ 10), breakflag = 0, xcol = 30
      IF (curpage=1)
       tempstring = concat("Diagnosis: ",pat->qual[x].diagnosis), b_end = ";", b_cc = 0,
       ...
       xcol = 2, ycol = (ycol+ 1)
      ENDIF
      ycol = (ycol+ 2), breakflag = 0, xcol = 30
      IF (curpage=1)
       tempstring = concat("Procedure Discription: ",pat->qual[x].procedure_desc), b_end = ";", b_cc
        = 0,
       b_s = 1, b_e = 0, b_tmp_comment = fillstring(90,""),
       a_tmp_string = fillstring(90,""), tmp_var = 0, b_cc = 1,
       b_s = 1
      ...
      ycol = (ycol+ 2), breakflag = 0, xcol = 30
      IF (curpage=1)
       tempstring = concat("Case Comments: ",pat->qual[x].case_comments), b_end = ";", b_cc = 0,
       b_s = 1, b_e = 0, b_tmp_comment = fillstring(90,""),
       a_tmp_string = fillstring(90,""), tmp_var = 0, b_cc = 1,
       b_s = 1
       WHILE (b_cc)
     ...
     DETAIL
      ...
      FOR (y = 1 TO size(pat->qual[x].sec,5))
        IF ((pat->qual[x].sec[y].sec_disp IN ("Patient Data", "Additional Patient Data")))
         IF (ycol > 740)
          ...
          "**  Continue on Next Page **", row + 1, BREAK
         ENDIF
         l = (l+ 1), temp1 = build(cnvtstring(l),"-",pat->qual[x].sec[y].sec_disp,":"),
         CALL print(calcpos(xcol,ycol)),
         "{b}", temp1, row + 1,
         ycol = (ycol+ 8)
      ...
      xcolvar = 30, wrapcol = 122, l = (l+ 1),
      title = build(l,"-","Patient Care Orders :"),
      CALL print(calcpos(xcol,ycol)), "{b}",
      title, row + 1, ycol = (ycol+ 10),
      title = build("-","{b}Diet Orders:"),
      CALL print(calcpos(xcol,ycol)), "{b}",
      title, row + 1, ycol = (ycol+ 10)
      ...
      FOR (d1 = 1 TO size(pat->qual[x].nut,5))
        ...
        IF ((pat->qual[x].nut[d1].com_ind=1))
         xcol = 40, tempstring = concat("{b}","Comment: ","{endb} ",build(pat->qual[x].nut[d1].
           comment)), line_wrap2,
         xcol = 30
        ENDIF
      ENDFOR
       ...
       FOR (z1 = 1 TO size(prn_orders_disp->prn_orders,5))
         ...
         IF (size(prn_orders_disp->prn_orders[z1].core_actions,5) > 0)
          ...
          row + 1, ycol = (ycol+ 8), tempstring = concat("Order Detail :",prn_orders_disp->
           prn_orders[z1].core_actions[1].clinical_display_line),
          wrapcol = 122, line_wrap2
          IF (comment_string > "")
    ...
    ;end select
    SET spool value(pat->qual[x].filename) value(varoutput)
