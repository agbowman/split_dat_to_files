CREATE PROGRAM cp_get_pathway_comp_reco:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "CP Component Id" = 0
  WITH outdev, cp_component_id
 DECLARE component_id = f8 WITH protect, constant( $CP_COMPONENT_ID)
 DECLARE orderoptions_comp_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003130,
   "ORDEROPTS"))
 DECLARE treatmentoptions_comp_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003130,
   "TREATMENTOPT"))
 DECLARE recomendations_comp_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003130,
   "PATHWAY_ORD"))
 DECLARE guided_treatment_comp_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003130,
   "GUIDEDTRMNT"))
 DECLARE treatment_intention_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003278,
   "TREATMENTS"))
 DECLARE regimen_catalog = vc WITH protect, constant("REGIMEN_CATALOG")
 DECLARE pathway_catalog = vc WITH protect, constant("PATHWAY_CATALOG")
 DECLARE order_cat_synonym = vc WITH protect, constant("ORDER_CATALOG_SYNONYM")
 DECLARE treamtment_catalog = vc WITH protect, constant("CP_NODE")
 DECLARE folder_catalog_mean = vc WITH protect, constant("ALT_SEL_CAT")
 IF (validate(recommendation_folders) != 1)
 ...
 ENDIF
 IF (validate(entities) != 1)
  RECORD entities(
    1 cnt = i4
    1 cp_component_detail_entity[*]
      2 entity_id = f8
  )
 ENDIF
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 DECLARE getallrecommendedorders(null) = null WITH protect
 DECLARE gettreatmentnodes(null) = null WITH protect
 DECLARE main(null) = null WITH protect
 SUBROUTINE main(null)
   SET recommendation_folders->status_data.status = "F"
   CALL getallrecommendedorders(null)
   CALL gettreatmentnodes(null)
   SET recommendation_folders->status_data.status = "S"
   IF (( $OUTDEV="NOFORMS"))
    CALL putjsonrecordtofile(recommendation_folders)
   ENDIF
 END ;Subroutine
 SUBROUTINE getallrecommendedorders(null)
   CALL log_message("In GetAllRecommendedOrders()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(curtime3), private
   DECLARE esize = i4 WITH protect, noconstant(size(entities->cp_component_detail_entity,5))
   DECLARE rfsize = i4 WITH protect, noconstant(size(recommendation_folders->orderfolders,5))
   DECLARE rrsize = i4 WITH protect, noconstant(size(recommendation_folders->regimen,5))
   DECLARE rpsize = i4 WITH protect, noconstant(size(recommendation_folders->powerplan,5))
   DECLARE rasize = i4 WITH protect, noconstant(size(recommendation_folders->allorders,5))
   DECLARE loc = i4 WITH protect, noconstant(0)
   DECLARE indx = i4 WITH protect, noconstant(0)
   DECLARE acntr = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM cp_component cc1,
     cp_component cc2,
     cp_component_detail ccd
    PLAN (cc1
     WHERE cc1.cp_component_id=component_id)
     JOIN (cc2
     WHERE cc2.cp_node_id=cc1.cp_node_id
      AND cc2.comp_type_cd IN (orderoptions_comp_cd, recomendations_comp_cd, treatmentoptions_comp_cd,
     guided_treatment_comp_cd))
     JOIN (ccd
     WHERE ccd.cp_component_id=cc2.cp_component_id)
    HEAD ccd.component_entity_id
     esize += 1, stat = alterlist(entities->cp_component_detail_entity,esize), entities->cnt = esize,
     entities->cp_component_detail_entity[esize].entity_id = ccd.component_entity_id
    WITH nocounter
   ;end select
   IF (validate(debug_ind,0)=1)
    CALL echorecord(entities)
   ENDIF
   SELECT INTO "nl:"
    FROM alt_sel_cat c,
     alt_sel_list l,
    ...
    PLAN (c
     WHERE expand(acntr,1,entities->cnt,c.alt_sel_category_id,entities->cp_component_detail_entity[
      acntr].entity_id))
     JOIN (l
     WHERE (l.alt_sel_category_id= Outerjoin(c.alt_sel_category_id))
      AND (l.alt_sel_category_id> Outerjoin(0)) )
     JOIN (c2
     WHERE (c2.alt_sel_category_id= Outerjoin(l.child_alt_sel_cat_id))
      AND (c2.alt_sel_category_id> Outerjoin(0)) )
     JOIN (ocs
     WHERE (ocs.synonym_id= Outerjoin(l.synonym_id))
     ...
     JOIN (rcs
     WHERE (rcs.regimen_cat_synonym_id= Outerjoin(l.regimen_cat_synonym_id)) )
    ORDER BY l.sequence
    HEAD c.alt_sel_category_id
     loc = 0, loc = locateval(indx,0,rfsize,c.alt_sel_category_id,recommendation_folders->
      orderfolders[indx].id)
     IF (c.alt_sel_category_id > 0.00
      AND loc=0)
      rfsize += 1, stat = alterlist(recommendation_folders->orderfolders,rfsize),
      recommendation_folders->orderfolders[rfsize].display = c.long_description,
      recommendation_folders->orderfolders[rfsize].id = c.alt_sel_category_id, recommendation_folders
      ->orderfolders[rfsize].mean = folder_catalog_mean
     ENDIF
    HEAD c2.alt_sel_category_id
     loc = 0, loc = locateval(indx,0,rfsize,c.alt_sel_category_id,recommendation_folders->
      orderfolders[indx].id)
     IF (c2.alt_sel_category_id > 0.00
      AND loc=0)
      rfsize += 1, stat = alterlist(recommendation_folders->orderfolders,rfsize),
      recommendation_folders->orderfolders[rfsize].display = c2.long_description,
      recommendation_folders->orderfolders[rfsize].id = c2.alt_sel_category_id,
      recommendation_folders->orderfolders[rfsize].mean = folder_catalog_mean
     ENDIF
    HEAD ocs.synonym_id
     row + 0
    HEAD os.order_sentence_id
     IF (ocs.synonym_id > 0.00)
      loc = 0, loc = locateval(indx,0,rasize,ocs.synonym_id,recommendation_folders->allorders[indx].
       id,
       os.order_sentence_id,recommendation_folders->allorders[indx].detail_id)
      IF (loc=0)
       rasize += 1, stat = alterlist(recommendation_folders->allorders,rasize),
       recommendation_folders->allorders[rasize].display = ocs.mnemonic,
       recommendation_folders->allorders[rasize].id = ocs.synonym_id, recommendation_folders->
       allorders[rasize].mean = order_cat_synonym, recommendation_folders->allorders[rasize].
       detail_display = os.order_sentence_display_line,
       recommendation_folders->allorders[rasize].detail_id = os.order_sentence_id
       IF (validate(recommendation_folders->allorders[rasize].cat_type_mean))
        recommendation_folders->allorders[rasize].cat_type_mean = uar_get_code_meaning(ocs
         .catalog_type_cd), recommendation_folders->allorders[rasize].ord_type_flag = ocs
        .orderable_type_flag, recommendation_folders->allorders[rasize].usage_flag = os.usage_flag
       ENDIF
      ENDIF
     ENDIF
    HEAD pc.pathway_catalog_id
     IF (pc.pathway_catalog_id > 0.00)
      loc = 0, loc = locateval(indx,0,rpsize,pcs.pathway_catalog_id,recommendation_folders->
       powerplan[indx].id)
      IF (loc=0)
       rpsize += 1, stat = alterlist(recommendation_folders->powerplan,rpsize),
       recommendation_folders->powerplan[rpsize].display = pcs.synonym_name,
       recommendation_folders->powerplan[rpsize].id = pcs.pathway_catalog_id, recommendation_folders
       ->powerplan[rpsize].mean = pathway_catalog
      ENDIF
     ENDIF
    HEAD rcs.regimen_cat_synonym_id
     IF (rcs.regimen_cat_synonym_id > 0.00)
      loc = 0, loc = locateval(indx,0,rrsize,rcs.regimen_catalog_id,recommendation_folders->regimen[
       indx].id)
      IF (loc=0)
       rrsize += 1, stat = alterlist(recommendation_folders->regimen,rrsize), recommendation_folders
       ->regimen[rrsize].display = rcs.synonym_display,
       recommendation_folders->regimen[rrsize].id = rcs.regimen_catalog_id, recommendation_folders->
       regimen[rrsize].mean = regimen_catalog
      ENDIF
     ENDIF
    FOOT REPORT
     IF (rrsize > 0)
      recommendation_folders->regimen_cnt = rrsize, stat = alterlist(recommendation_folders->regimen,
       rrsize)
     ENDIF
     IF (rpsize > 0)
      recommendation_folders->powerplan_cnt = rpsize, stat = alterlist(recommendation_folders->
       powerplan,rpsize)
     ENDIF
     IF (rasize > 0)
      recommendation_folders->allorders_cnt = rasize, stat = alterlist(recommendation_folders->
       allorders,rasize)
     ENDIF
     IF (rfsize > 0)
      recommendation_folders->orderfolders_cnt = rfsize, stat = alterlist(recommendation_folders->
       orderfolders,rfsize)
     ENDIF
    WITH nocounter, expand = 1
   ;end select
   CALL log_message(build("Exit GetAllRecommendedOrders(), Elapsed time in seconds:",((curtime3 -
     begin_date_time)/ 100)),log_level_debug)
 END ;Subroutine
 SUBROUTINE gettreatmentnodes(null)
   CALL log_message("In GetTreatmentNodes()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(curtime3), private
   DECLARE ccnt = i4 WITH protect, noconstant(0)
   DECLARE activeind = i4 WITH protect, constant(1)
   SET recommendation_folders->treatment_cnt = 0
   SELECT INTO "nl:"
    FROM cp_component cc,
     cp_node cn1,
     cp_node cn2
    PLAN (cc
     WHERE cc.cp_component_id=component_id)
     JOIN (cn1
     WHERE cn1.cp_node_id=cc.cp_node_id)
     JOIN (cn2
     WHERE cn2.cp_pathway_id=cn1.cp_pathway_id
      AND cn2.intention_cd=treatment_intention_cd
      AND cn2.active_ind=activeind)
    HEAD cn2.cp_node_id
     ccnt += 1, stat = alterlist(recommendation_folders->treatmentnodes,ccnt), recommendation_folders
     ->treatmentnodes[ccnt].display = cn2.node_display,
     recommendation_folders->treatmentnodes[ccnt].id = cn2.cp_node_id, recommendation_folders->
     treatmentnodes[ccnt].mean = treamtment_catalog
    FOOT REPORT
     IF (ccnt > 0)
      stat = alterlist(recommendation_folders->treatmentnodes,ccnt), recommendation_folders->
      treatment_cnt = ccnt
     ENDIF
    WITH nocounter
   ;end select
   CALL log_message(build("Exit GetTreatmentNodes(), Elapsed time in seconds:",((curtime3 -
     begin_date_time)/ 100)),log_level_debug)
 END ;Subroutine
 CALL main(null)
#exit_script
