CREATE PROGRAM bhs_dietary_report_new_1
 PROMPT
 ...
 ENDIF
 IF (( $ORDER_STATUS="*"))
  SET order_status = " o.order_status_cd + 0 > 0 "
 ELSE
  SET order_status = " o.order_status_cd + 0  = $ORDER_STATUS "
 ENDIF
 ...
 SELECT
  ...
  HEAD e.encntr_id
   ...
   diet->qual[cnt].has_diet_order = 0, diet->qual[cnt].has_clear_order = 0
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   WHERE (o.encntr_id=diet->qual[d.seq].encntr_id)
    ...
    AND o.active_ind=1
    AND parser(order_status)
    AND o.catalog_cd IN (clear1_cd, clear2_cd, clear3_cd, clear4_cd, fullliquid1_cd,
   fullliquid2_cd))
  ORDER BY o.encntr_id, o.order_id
  HEAD o.encntr_id
   ord_cnt = 0
