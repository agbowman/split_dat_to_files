CREATE PROGRAM ct_trial_prescreen_dx:dba
 PROMPT
 ...
 DECLARE runqueryforevaluationby(null) = null WITH protect
 ...
 DECLARE testonly = i2 WITH public, noconstant( $TESTONLYHIDDEN)
 ...
 CALL echo(concat("Execution Mode = ",cnvtstring( $EXECMODE)))
 ...
 CALL echo(concat("startDtTm = ",format(startdttm,"@MEDIUMDATETIME")))
 CALL echo(concat("endDtTm = ",format(enddttm,"@MEDIUMDATETIME")))
 ...
 CASE (agequal)
  ...
  OF between_type_cd:
   ...
   ENDIF
 ENDCASE
 SET personrace = cnvtreal( $RACE)
 IF (personrace=0.0)
  SET racequal = "1=1"
 ELSE
  SET racequal = "p.race_cd = personRace"
 ENDIF
 SET personethnicity = cnvtreal( $ETHNICITY)
 IF (personethnicity=0.0)
  SET ethnicityqual = "1=1"
 ELSE
  SET ethnicityqual = "p.ethnic_grp_cd = personEthnicity"
 ENDIF
 ...
 IF (dxcodelist != "")
  ...
  WHILE (tmp != not_found
   AND pieceidx < 100)
    SET tmp = piece(dxcodelist,",",pieceidx,not_found)
    SET pieceidx += 1
    IF (tmp != not_found)
     SET dxcodecnt += 1
     IF (dxcodecnt=1)
      SET dxcodequal = build('n.source_identifier IN ("',tmp,'"')
     ELSE
      SET dxcodequal = build(dxcodequal,',"',tmp,'"')
     ENDIF
    ENDIF
  ENDWHILE
  SET dxcodequal = build(dxcodequal,")")
 ELSE
  SET dxcodequal = "1=1"
 ENDIF
 CALL echorecord(paramlists)
 SET cnt = 0
 DECLARE colidx = i4 WITH protect, noconstant(0)
 CALL runqueryforevaluationby(null)
 ...
 ENDFOR
 GO TO endprogram
 SUBROUTINE runqueryforevaluationby(null)
   IF (( $EVALBY=0))
    SELECT
     IF (dxcodecnt > 0)DISTINCT INTO value(toutputdev)
      ...
      FROM nomenclature n,
       ...
       person p
      PLAN (e
       WHERE e.reg_dt_tm BETWEEN cnvtdatetime(startdttm) AND cnvtdatetime(enddttm)
        AND (((paramlists->eanyflag=1)) OR (expand(num,1,paramlists->etypecnt,e.encntr_type_cd,
      ...
      ORDER BY p.name_last_key, p.name_first_key, p.name_middle_key,
       p.person_id
     ELSE DISTINCT INTO value(toutputdev)
     ...
     HEAD p.person_id
      ...
      IF (bfound=1)
       ...
       IF (dxcodecnt > 0)
        eksctrequesttobechunked->qual[cnt].encntr_id = encounter
       ELSE
        eksctrequesttobechunked->qual[cnt].encntr_id = e.encntr_id
       ENDIF
       eksctrequesttobechunked->qual[cnt].race_cd = p.race_cd
      ENDIF
     FOOT REPORT
     ...
     IF (dxcodecnt > 0)DISTINCT INTO value(toutputdev)
      ...
      PLAN (e
       WHERE (((paramlists->eanyflag=1)) OR (expand(num,1,paramlists->etypecnt,e.encntr_type_cd,
        ...
        AND e.reg_dt_tm <= cnvtdatetime(enddttm)
        AND ((e.encntr_status_cd=discharged_encntr_cd
        AND e.disch_dt_tm >= cnvtdatetime(startdttm)) OR (e.encntr_status_cd=active_encntr_cd
     ...
     IF (dxcodecnt > 0)DISTINCT INTO value(toutputdev)
      ...
      PLAN (e
       ...
       WHERE sa.active_ind=1
        ...
        AND sa.beg_dt_tm BETWEEN cnvtdatetime(startdttm) AND cnvtdatetime(enddttm)
        AND sa.state_meaning IN ("SCHEDULED", "RESCHEDULED", "CHECKED IN", "CHECKED OUT", "CONFIRMED"
       ))
       JOIN (p
       WHERE p.person_id=e.person_id
        AND p.active_ind=1
