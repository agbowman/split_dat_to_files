CREATE PROGRAM bhs_ma_amb_rln_req:dba
 ...
 DECLARE 14281_dispatched = f8 WITH constant(uar_get_code_by_cki("CKI.CODEVALUE!12588")), protect
 DECLARE 14281_collected = f8 WITH protect, constant(uar_get_code_by_cki("CKI.CODEVALUE!12587"))
 ...
 ELSE
  ...
  ENDFOR
 ENDIF
 SUBROUTINE (getfuturereprintorders(in_rec=vc(ref)) =i4)
   SELECT INTO "nl:"
    FROM orders o
   ...
   PLAN (oa
    ...
    (SELECT
     ...
     WHERE oa_s.order_id=oa.order_id
      ...
      AND oa_s.dept_status_cd IN (14281_dispatched, 14281_collected))))
   HEAD REPORT
 ...
 SELECT
  ...
  HEAD o.order_id
   ...
   IF (ord_cnt > max_orders_per_req)
    ...
    req_cnt].ord_cnt = max_orders_per_req, req_cnt += 1
    IF (mod(req_cnt,5)=1)
     stat = alterlist(req_data->req_qual,(req_cnt+ 4))
    ENDIF
    col_req_cnt += 1, req_data->req_qual[req_cnt].collected_ind = coll.oe_field_value, req_data->
    req_qual[req_cnt].order_provider.id = oa.order_provider_id,
    req_data->req_qual[req_cnt].sup_physician.id = oa.supervising_provider_id, req_data->req_qual[
    req_cnt].coll_class_cd = c.coll_class_cd, req_data->req_qual[req_cnt].transfer_temp_cd = cc
    .transfer_temp_cd,
    req_data->req_qual[req_cnt].contrib_src_cd = contrib_src_cd, req_data->req_qual[req_cnt].ref_lab
 ...
 IF ((req_data->encntr_id > 0.00))
  SELECT INTO "nl:"
   ...
   HEAD epr.priority_seq
    add_type_seq_gr_zero = false, ph_type_seq_gr_zero = false, cnt += 1,
 ...
 SELECT
  ...
  HEAD n.source_identifier
   ...
   IF (pos=0)
    cnt += 1, stat = alterlist(labcorp_diagnosis->diag,(cnt+ 9)), labcorp_diagnosis->diag[cnt].
    nomenclature_id = n.nomenclature_id
    IF (cnt <= 120)
     req_data->dx_lc_list = build(req_data->dx_lc_list,",",n.source_identifier)
    ENDIF
   ENDIF
   dcnt += 1, req_data->req_qual[d1.seq].ord_qual[d2.seq].dx_list = build(req_data->req_qual[d1.seq].
    ord_qual[d2.seq].dx_list,",",n.source_identifier)
   IF (mod(dcnt,10)=1)
    ...
    IF (od.oe_field_meaning IN ("OTHER", "PROMPT")
     ...
     IF (pos > 0)
      ...
      req_qual[d1.seq].ord_qual[d2.seq].details[pos].value = concat(req_data->req_qual[d1.seq].
       ord_qual[d2.seq].details[pos].value,", ",trim(od.oe_field_display_value)), multi_cnt += 1
     ELSE
      detail_cnt += 1
      ...
      max_aoe_char))
       multi_cnt += 1, req_data->req_qual[d1.seq].ord_qual[d2.seq].details[detail_cnt].multi_select
        = true
      ENDIF
     ENDIF
    ENDIF
  ...
  FOOT  d2.seq
   ...
   FOR (index = 1 TO detail_cnt)
     IF ((req_data->req_qual[d1.seq].ord_qual[d2.seq].details[index].multi_select=false))
      cnt += 1, req_data->req_qual[d1.seq].ord_qual[d2.seq].single_cnt = cnt, stat = alterlist(
 ...
 IF ((req_data->req_cnt > 0))
  SELECT INTO "nl:"
   ...
   DETAIL
    IF (substring(1,(findstring("_BARCODE",trim(cnvtupper(cv.display),3),1,1) - 1),trim(cnvtupper(cv
     ...
     countvar += 1, format = trim(cnvtupper(cvo.alias),3), acct_id = req_data->req_qual[d1.seq].
     loc_nurse_unit_alias,
  ...
  ENDFOR
 ENDIF
 SUBROUTINE (dynamic_requisition_text(format=vc,acct_id=vc,req_id=vc,mrn=vc) =vc WITH protect)
 ...
 FOR (loopvar = 1 TO req_data->req_cnt)
  ...
  ENDIF
 ENDFOR
 SUBROUTINE (getrelatedorders(in_rec=vc(ref)) =i2)
