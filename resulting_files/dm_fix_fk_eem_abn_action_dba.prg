CREATE PROGRAM dm_fix_fk_eem_abn_action:dba
 ...
 SET readme_data->message = "Readme Failed: Starting dm_fix_fk_eem_abn_action script."
 SELECT INTO "nl:"
  FROM dm2_user_tables ut,
   dm_tables_doc dtd
  PLAN (dtd
   WHERE dtd.table_name IN ("EEM_ABN_ACTION", "PERSON", "PRSNL"))
   JOIN (ut
   WHERE ut.table_name=dtd.table_name)
  DETAIL
   IF (dtd.table_name="EEM_ABN_ACTION")
    IF (currdb="ORACLE")
     d_name = dtd.full_table_name, tmp_fk_name = "XFK3EEM_ABN_ACTION*", d_fk_name =
     "XFK3EEM_ABN_ACTION"
    ELSEIF (currdb="DB2UDB")
     d_name = dtd.suffixed_table_name, tmp_fk_name = build("XFK3EEM_ABN_",dtd.table_suffix,"*"),
     d_fk_name = build("XFK3EEM_ABN_",dtd.table_suffix)
    ENDIF
