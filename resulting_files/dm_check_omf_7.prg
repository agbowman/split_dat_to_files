CREATE PROGRAM dm_check_omf_7
 ...
 IF (curqual != 0)
  UPDATE  FROM ub92_mon_proc_phys_error mee,
   ...
   SET mee.error_cd = 01401, mee.status_flg = "W", mee.updt_dt_tm = cnvtdatetime(curdate,curtime3),
  ...
  IF (curqual=0)
   INSERT  FROM ub92_mon_proc_phys_error mee,
     (dummyt d  WITH seq = value(data->count))
    SET mee.error_cd = 01401, mee.reporting_period = cnvtdatetime(startdatesstring), mee
