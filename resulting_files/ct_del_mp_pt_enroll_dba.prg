CREATE PROGRAM ct_del_mp_pt_enroll:dba
 PROMPT
  ...
  "Partial_Del_Ind" = 0
  WITH outdev, regid, reason,
  deletion_dt_tm, pt_elig_tracking_id, partial_del_ind
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 IF (( $PARTIAL_DEL_IND=0))
  SELECT INTO "nl:"
   p_pr_r.pt_prot_reg_id
   FROM pt_prot_reg p_pr_r
   WHERE (p_pr_r.reg_id= $REGID)
    AND p_pr_r.end_effective_dt_tm > cnvtdatetime(sysdate)
   DETAIL
    pt_prot_reg_id = p_pr_r.pt_prot_reg_id, audit->person_id = p_pr_r.person_id, audit->updt_dt_tm =
    p_pr_r.updt_dt_tm
  ...
  SELECT INTO "nl:"
   ...
   WHERE (rerltn.reg_id= $REGID)
   DETAIL
    request->pt_elig_tracking_id = rerltn.pt_elig_tracking_id
  ;end select
  SET  $PT_ELIG_TRACKING_ID = request->pt_elig_tracking_id WITH nocounter, forupdate(rerltn)
  IF (( $PT_ELIG_TRACKING_ID > 0))
   UPDATE  FROM pt_reg_elig_reltn rerltn
    SET rerltn.active_ind = 0, rerltn.active_status_cd = reqdata->inactive_status_cd, rerltn
     ...
     cnvtdatetime(sysdate)
    PLAN (rerltn
     WHERE (rerltn.pt_elig_tracking_id= $PT_ELIG_TRACKING_ID))
    WITH nocounter
   ;end update
   IF (curqual=0)
    SET reply->status_data.subeventstatus[1].targetobjectvalue =
 ...
 IF (( $PT_ELIG_TRACKING_ID > 0.0))
  ...
  SELECT INTO "nl:"
   ...
   WHERE (pet.pt_elig_tracking_id= $PT_ELIG_TRACKING_ID)
  ...
  SELECT INTO "nl:"
   ...
   WHERE (per.pt_elig_tracking_id= $PT_ELIG_TRACKING_ID)
  ...
  SELECT INTO "nl:"
   ...
   WHERE (a_e.pt_elig_tracking_id= $PT_ELIG_TRACKING_ID)
  ...
  SELECT INTO "nl:"
   ...
   WHERE (ecrltn.pt_elig_tracking_id= $PT_ELIG_TRACKING_ID)
   DETAIL
    consent_count += 1
    IF (mod(consent_count,10)=1)
     stat = alterlist(consent_list->consents,(consent_count+ 9))
 ...
 INSERT  FROM ct_reason_deleted del
  SET del.reg_id = pt_prot_reg_id, del.ct_reason_del_id = seq(protocol_def_seq,nextval), del
   ...
   del.deletion_prsnl_id = reqinfo->updt_id, del.deletion_reason =  $REASON, del.pt_elig_tracking_id
    =  $PT_ELIG_TRACKING_ID,
...
#check_error
 IF (fail_flag=0)
  ...
  SET audit->prot_reg_id = build3(3,"PT_PROT_REG_ID: ",pt_prot_reg_id)
  SET participantname = concat(audit->prot_reg_id," ",lst_updt_dt_tm," (UPDT_DT_TM)")
  EXECUTE cclaudit audit_mode, "Enrollment_Delete", "Delete",
  "Person", "Patient", "Patient",
  "Destruction", audit->person_id, participantname
 ELSE
  CASE (fail_flag)
   OF insert_error:
