CREATE PROGRAM aps_chg_wp_template:dba
 ...
 DECLARE total_long_text_to_del = i4 WITH protect, noconstant(0)
 DECLARE long_text_to_chg = i4 WITH protect, noconstant(0)
 SET lend_pos = size(sac_org->organizations,5)
 SET lend_pos2 = size(request->org_del_qual,5)
 DECLARE loc_type_cd = f8 WITH protected, noconstant(0.0)
 ...
 IF (size(request->org_del_qual,5) != 0)
  SELECT INTO "nl:"
   fer.parent_entity_id, fer.filter_entity1_id
   ...
   DETAIL
    lpos = locateval(lindex_var,1,lend_pos2,fer.filter_entity1_id,request->org_del_qual[lindex_var].
     organization_id)
    IF (lpos=0)
     lloop_counter += 1
     ...
     ENDIF
     temp_org_struct->organizations[lloop_counter].organization_id = fer.filter_entity1_id
    ENDIF
   FOOT REPORT
 ...
 IF (size(temp_org_struct->organizations,5) != 0)
  ...
  ENDWHILE
  IF (lindex=0
   AND size(temp_org_struct->organizations,5) != 0)
 ...
 SET cur_updt_cnt = 0
 SET count1 = 0
 SET number_to_chg = size(request->text_qual,5)
 SET number_to_del = 0
 ...
 SET org_add_cnt = size(request->org_add_qual,5)
 SET org_del_cnt = size(request->org_del_qual,5)
 SET org_filter_cd = uar_get_code_by("MEANING",30620,"WPTEMPLATE")
 IF (number_to_chg > 1)
  SET stat = alter(temp->qual,number_to_chg)
 ENDIF
 SELECT INTO "nl:"
  t.*
  FROM wp_template t
  ...
  DETAIL
   IF ((request->template_id != t.template_id))
    reply->template_cd = t.template_id
   ENDIF
  WITH nocounter
 ;end select
 IF ((reply->template_cd > 0))
  SELECT INTO "nl:"
   FROM org_type_reltn o
   PLAN (o
    WHERE expand(lindex_var,1,lend_pos,o.organization_id,sac_org->organizations[lindex_var].
     organization_id)
     AND o.org_type_cd=loc_type_cd)
   WITH nocounter
  ;end select
  IF (curqual > 0)
   SELECT INTO "nl:"
    FROM filter_entity_reltn fer
    WHERE (fer.parent_entity_id=reply->template_cd)
    WITH nocounter
   ;end select
   IF (curqual > 0)
    SELECT INTO "nl:"
     ...
     PLAN (f
      WHERE expand(lindex_var,1,lend_pos,f.filter_entity1_id,sac_org->organizations[lindex_var].
       organization_id)
       AND (f.parent_entity_id=reply->template_cd)
 ...
 IF (curqual=0)
  ...
  SET reply->status_data.subeventstatus[1].targetobjectname = "wp_template"
  SET reply->status_data.subeventstatus[1].targetobjectvalue = build("template_id: ",request->
   template_id)
  ROLLBACK
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  tt.template_id
  FROM wp_template_text tt
  WHERE (tt.template_id=request->template_id)
  HEAD REPORT
   number_to_del = 0
  DETAIL
   number_to_del += 1
 ...
 SELECT INTO "nl:"
  lt.long_text_id
  FROM long_text lt
  WHERE (lt.parent_entity_id=request->template_id)
   AND lt.parent_entity_name="WP_TEMPLATE_TEXT"
  HEAD REPORT
   total_long_text_to_del = 0
  FOOT REPORT
   total_long_text_to_del = count(lt.long_text_id)
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  HEAD lt.long_text_id
   IF ((lt.long_text != request->text_qual[tt.sequence].text))
    temp->qual[tt.sequence].updt_cnt = (lt.updt_cnt+ 1)
   ELSE
    temp->qual[tt.sequence].updt_cnt = lt.updt_cnt
   ENDIF
  WITH nocounter
 ;end select
 FOR (ltextitemindex = 1 TO size(request->text_qual,5))
   IF ((request->text_qual[ltextitemindex].result_layout_id=0))
    SET long_text_to_chg += 1
   ENDIF
 ENDFOR
 ...
 FOR (ltextitemindex = 1 TO size(request->text_qual,5))
   ...
   ENDIF
 ENDFOR
 UPDATE  FROM wp_template t
  SET t.short_desc = cnvtupper(request->short_desc), t.template_type_cd = request->type_cd, t
   .description = request->description,
   t.activity_type_cd = request->activity_type_cd, t.person_id = request->person_id, t.active_ind =
   request->active_ind,
   t.updt_cnt = (t.updt_cnt+ 1), t.updt_dt_tm = cnvtdatetime(sysdate), t.updt_id = reqinfo->updt_id,
   t.updt_task = reqinfo->updt_task, t.updt_applctx = reqinfo->updt_applctx, t
   .result_layout_exists_ind = nresultlayoutexistsind
  WHERE (t.template_id=request->template_id)
  WITH nocounter
 ;end update
 IF (curqual=0)
 ...
 FOR (x = 1 TO number_to_chg)
   IF ((request->text_qual[x].result_layout_id > 0))
    SELECT INTO "nl:"
   ...
   ELSE
    SELECT INTO "nl:"
     ...
     FROM dual
     DETAIL
      temp->qual[x].long_text_id = seq_nbr
     WITH format, counter
    ;end select
    IF (curqual=0)
     SET stat = alter(reply->status_data.subeventstatus,1)
     SET reply->status_data.subeventstatus[1].operationstatus = "F"
     SET reply->status_data.subeventstatus[1].operationname = "LT_SEQ"
   ...
   ENDIF
 ENDFOR
 DELETE  FROM wp_template_text tt
  SET tt.template_id = request->template_id
  PLAN (tt
   WHERE (tt.template_id=request->template_id))
  WITH nocounter
 ;end delete
 IF (curqual != number_to_del)
  ...
  SET reply->status_data.subeventstatus[1].targetobjectname = "wp_template_text"
  SET reply->status_data.subeventstatus[1].targetobjectvalue = build("template_id: ",request->
   template_id)
  ROLLBACK
  GO TO exit_script
 ...
 DELETE  FROM long_text lt
  SET lt.parent_entity_id = request->template_id
  PLAN (lt
   WHERE (lt.parent_entity_id=request->template_id)
    AND lt.parent_entity_name="WP_TEMPLATE_TEXT")
  WITH nocounter
 ;end delete
 IF (curqual != total_long_text_to_del)
  SET stat = alter(reply->status_data.subeventstatus,1)
  SET reply->status_data.subeventstatus[1].operationstatus = "F"
  SET reply->status_data.subeventstatus[1].operationname = "Delete"
  SET reply->status_data.subeventstatus[1].targetobjectname = "long_text"
 ...
 INSERT  FROM long_text lt,
  ...
  SET lt.long_text_id = temp->qual[d.seq].long_text_id, lt.parent_entity_id = request->template_id,
   lt.parent_entity_name = "WP_TEMPLATE_TEXT",
   ...
   .updt_cnt = temp->qual[d.seq].updt_cnt,
   lt.updt_dt_tm = cnvtdatetime(sysdate), lt.updt_id = reqinfo->updt_id, lt.updt_task = reqinfo->
   updt_task,
   lt.updt_applctx = reqinfo->updt_applctx
  PLAN (d
   WHERE (request->text_qual[d.seq].result_layout_id=0))
   JOIN (lt
   WHERE (lt.parent_entity_id=request->template_id))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 IF (curqual != long_text_to_chg)
  SET stat = alter(reply->status_data.subeventstatus,1)
  SET reply->status_data.subeventstatus[1].operationstatus = "F"
 ...
 INSERT  FROM wp_template_text tt,
   (dummyt d  WITH seq = value(number_to_chg))
  SET tt.template_id = request->template_id, tt.sequence = request->text_qual[d.seq].sequence, tt
   .long_text_id = temp->qual[d.seq].long_text_id,
   tt.updt_cnt = temp->qual[d.seq].updt_cnt, tt.updt_dt_tm = cnvtdatetime(sysdate), tt.updt_id =
   reqinfo->updt_id,
   tt.updt_task = reqinfo->updt_task, tt.updt_applctx = reqinfo->updt_applctx, tt.pcs_rslt_layout_id
    = request->text_qual[d.seq].result_layout_id,
 ...
 ENDIF
 IF (org_add_cnt > 0)
  SET stat = alterlist(filter_entity_req->filter_entity,org_add_cnt)
  FOR (x = 1 TO org_add_cnt)
    SET stat = alterlist(filter_entity_req->filter_entity[x].values,1)
    SET filter_entity_req->filter_entity[x].filter_type_cd = org_filter_cd
    ...
    SET filter_entity_req->filter_entity[x].filter_entity1_id = request->org_add_qual[x].
    organization_id
    SET filter_entity_req->filter_entity[x].action_flag = ppr_action_add
    SET filter_entity_req->filter_entity[x].values[1].parent_entity_id = request->template_id
  ...
  IF ((filter_entity_rep->status_data.status != "S"))
   GO TO orgs_failed
  ENDIF
  SET stat = alterlist(reply->org_qual,org_add_cnt)
  FOR (x = 1 TO org_add_cnt)
   SET reply->org_qual[x].organization_id = request->org_add_qual[x].organization_id
   SET reply->org_qual[x].filter_entity_id = filter_entity_rep->filter_entity[x].values[1].
   filter_entity_reltn_id
  ENDFOR
 ENDIF
 IF (org_del_cnt > 0)
  SET stat = alterlist(filter_entity_req->filter_entity,org_del_cnt)
  FOR (x = 1 TO org_del_cnt)
    SET stat = alterlist(filter_entity_req->filter_entity[x].values,1)
    SET filter_entity_req->filter_entity[x].filter_type_cd = org_filter_cd
    SET filter_entity_req->filter_entity[x].filter_entity1_name = "ORGANIZATION"
    SET filter_entity_req->filter_entity[x].filter_entity2_name = ""
    ...
    SET filter_entity_req->filter_entity[x].filter_entity4_name = ""
    SET filter_entity_req->filter_entity[x].filter_entity5_name = ""
    SET filter_entity_req->filter_entity[x].filter_entity1_id = request->org_del_qual[x].
    organization_id
    SET filter_entity_req->filter_entity[x].action_flag = ppr_action_ina
    SET filter_entity_req->filter_entity[x].values[1].parent_entity_id = request->template_id
    SET filter_entity_req->filter_entity[x].values[1].parent_entity_name = "WP_TEMPLATE"
    SET filter_entity_req->filter_entity[x].values[1].exclusion_filter_ind = 0
    SET filter_entity_req->filter_entity[x].values[1].filter_entity_reltn_id = request->org_del_qual[
    x].filter_entity_id
  ENDFOR
  EXECUTE ppr_ens_filter_ref  WITH replace("REQUEST",filter_entity_req), replace("REPLY",
   filter_entity_rep)
