CREATE PROGRAM dcp_get_tasks_by_assign_sec:dba
 ...
 IF (assign_prsnl_count > 0)
  ...
  IF (task_status_count > 0)
   SET expandclauses = concat(trim(expandclauses),
" and expand(task_status_index, 1, task_status_count,ta.task_status_cd+0,request->task_status_list[task_status_index].statu\
s_cd)\
")
  ENDIF
  ...
  IF (encntr_count > 0
   ...
   SELECT INTO "nl:"
    ...
    PLAN (dtask)
     ...
     JOIN (douter)
     JOIN (taa1
     WHERE (taa1.task_id=internal_task_rec->task_list[dtask.seq].task_id)
      AND ((taa1.assign_prsnl_id+ 0)=reqinfo->updt_id)
