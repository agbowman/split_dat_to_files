CREATE PROGRAM dm_obs_xif2pe_status_person:dba
 ...
 IF (currdb="ORACLE")
  ...
  SET errcode = 0
  EXECUTE dm_drop_obsolete_objects "XIF2PE_STATUS_REASON", "INDEX", 1
  IF (errcode != 0)
   SET readme_data->message = build(errmsg,"- Readme Failed.")
   GO TO exit_script
  ENDIF
  EXECUTE dm_drop_obsolete_objects "XIE1BILL_REC", "INDEX", 1
  IF (errcode != 0)
   SET readme_data->message = build(errmsg,"- Readme Failed.")
   GO TO exit_script
  ENDIF
