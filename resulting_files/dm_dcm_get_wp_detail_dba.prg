CREATE PROGRAM dm_dcm_get_wp_detail:dba
 ...
 SET dm_err->eproc = "Starting DM_DCM_GET_WP_DETAIL..."
 ...
 IF ((request->work_plan_id <= 0.0))
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Getting DETAIL from LONG_TEXT"
 SELECT INTO "NL:"
  FROM long_text l,
   dm_dcm_work_plan d
  WHERE (d.dm_dcm_work_plan_id=request->work_plan_id)
   AND l.long_text_id=d.work_plan_text_id
   AND l.long_text_id > 0.0
  HEAD REPORT
   outbuf = fillstring(32767," "), offset = 0, retlen = 0
  DETAIL
   retlen = blobget(outbuf,offset,l.long_text), reply->detail_txt = " "
   WHILE (retlen > 0)
     IF ((reply->detail_txt=" "))
      reply->detail_txt = notrim(outbuf)
     ELSE
      reply->detail_txt = notrim(concat(notrim(reply->detail_txt),notrim(substring(1,retlen,outbuf)))
       )
     ENDIF
     offset += retlen, retlen = blobget(outbuf,offset,l.long_text)
   ENDWHILE
   reply->detail_txt = trim(reply->detail_txt)
