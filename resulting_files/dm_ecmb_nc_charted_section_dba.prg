CREATE PROGRAM dm_ecmb_nc_charted_section:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "NC_CHARTED_SECTION"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_ECMB_NC_CHARTED_SECTION"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = frm.nc_charted_section_id, rreclist->from_rec[
   ...
   nc_charting_section_id = frm.nc_charting_section_id
  WITH forupdatewait(frm)
 ;end select
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   tu.*
   FROM nc_charted_section tu
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count2].to_id = tu.nc_charted_section_id, rreclist->to_rec[v_cust_count2]
    ...
    nc_charting_section_id = tu.nc_charting_section_id
   WITH forupdatewait(tu)
  ;end select
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
   FOR (v_cust_loopcount2 = 1 TO v_cust_count2)
     IF ((rreclist->from_rec[v_cust_loopcount].nc_charting_section_id=rreclist->to_rec[
     v_cust_loopcount2].nc_charting_section_id)
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_to_fk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =
  i4)
   UPDATE  FROM nc_charted_section frm
    ...
    WHERE frm.nc_charted_section_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "NC_CHARTED_SECTION"
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM nc_charted_section frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.nc_charted_section_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
