CREATE PROGRAM ct_rn_vte_ops_run:dba
 RECORD status_rec(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 CALL echo(build("ct_rn_vte_ops_run:run_group_id = ",run_group_id))
 EXECUTE ct_rn_ops_prerun value(run_group_id) WITH replace("STATUS_REPLY","STATUS_REC")
 EXECUTE ct_run_vte_custom_ops value(run_group_id) WITH replace("STATUS_REPLY","STATUS_REC")
