CREATE PROGRAM dm_rmc_mover_1150:dba
 ...
 DECLARE null_check2 = i2
 DECLARE null_check3 = i2
 DECLARE null_check4 = i2
 DECLARE null_check5 = i2
 DECLARE cust_tab_name = vc
 DECLARE dmt_cnt = i4
 DECLARE from_vc = vc
 DECLARE dmt_select = i2
 DECLARE k2pos = i4
 DECLARE cust_table_name = vc
 DECLARE cust_col_name = vc
 DECLARE cust_id_col_name = vc
 DECLARE key_id1_vc = vc
 DECLARE key_id2_vc = vc
 DECLARE key_id3_vc = vc
 DECLARE key_id4_vc = vc
 DECLARE key_id5_vc = vc
 DECLARE key6_vc = vc
 DECLARE key7_vc = vc
 DECLARE key_id11_vc = vc
 DECLARE key_id12_vc = vc
 DECLARE key_id13_vc = vc
 DECLARE key_id14_vc = vc
 DECLARE key_id15_vc = vc
 DECLARE type_mean = vc
 DECLARE type_mean_trans = i2
 DECLARE type_cd = f8
 DECLARE bill_item_cd_display = vc
 DECLARE key1_pos = i4
 DECLARE key2_pos = i4
 DECLARE key3_pos = i4
 DECLARE key4_pos = i4
 DECLARE key5_pos = i4
 DECLARE key12_pos = i4
 DECLARE key14_pos = i4
 DECLARE key15_pos = i4
 DECLARE drdm_smt(ds_rdds_exception=vc(ref),ds_tab_col_name=vc,ds_tru_tab_name=vc,ds_tru_col_name=vc,
  ds_smt_id=vc,
  ds_smt_tab=vc) = vc
 DECLARE cust_sub_translate(cs_tab_name=vc,cs_tab_col=vc,cs_id_val=vc,cs_id_col_name=vc) = null
 SET stat = alterlist(rdds_exception->qual,0)
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET dmt_cnt = 0
 SET key_id1_vc = get_value(cust_tab_name,"KEY1_ID","FROM")
 SET key_id2_vc = get_value(cust_tab_name,"KEY2_ID","FROM")
 SET key_id3_vc = get_value(cust_tab_name,"KEY3_ID","FROM")
 SET key_id4_vc = get_value(cust_tab_name,"KEY4_ID","FROM")
 SET key_id5_vc = get_value(cust_tab_name,"KEY5_ID","FROM")
 SET key6_vc = get_value(cust_tab_name,"KEY6","FROM")
 SET key7_vc = get_value(cust_tab_name,"KEY7","FROM")
 SET key_id11_vc = get_value(cust_tab_name,"KEY11_ID","FROM")
 SET key_id12_vc = get_value(cust_tab_name,"KEY12_ID","FROM")
 SET key_id13_vc = get_value(cust_tab_name,"KEY13_ID","FROM")
 SET key_id14_vc = get_value(cust_tab_name,"KEY14_ID","FROM")
 SET key_id15_vc = get_value(cust_tab_name,"KEY15_ID","FROM")
 SET k2pos = get_col_pos(cust_tab_name,"KEY2_ID")
 SET key1_pos = get_col_pos(cust_tab_name,"KEY1_ID")
 SET key2_pos = get_col_pos(cust_tab_name,"KEY2_ID")
 SET key3_pos = get_col_pos(cust_tab_name,"KEY3_ID")
 SET key4_pos = get_col_pos(cust_tab_name,"KEY4_ID")
 SET key5_pos = get_col_pos(cust_tab_name,"KEY5_ID")
 SET key12_pos = get_col_pos(cust_tab_name,"KEY12_ID")
 SET key14_pos = get_col_pos(cust_tab_name,"KEY14_ID")
 SET key15_pos = get_col_pos(cust_tab_name,"KEY15_ID")
 SET type_mean_trans = is_translated(cust_tab_name,"BILL_ITEM_TYPE_CD")
 IF (type_mean_trans=1)
  SET type_cd = cnvtreal(get_value(cust_tab_name,"BILL_ITEM_TYPE_CD","TO"))
  IF ((dm2_ref_data_reply->error_ind=1))
   GO TO exit_1150
  ENDIF
  SELECT INTO "NL:"
   FROM code_value c
   WHERE c.code_value=type_cd
   DETAIL
    type_mean = c.cdf_meaning
   WITH nocounter
  ;end select
  ...
  SELECT INTO "NL:"
   ...
   WHERE c.code_value=type_cd
   DETAIL
    bill_item_cd_display = c.display
   WITH nocounter
  ;end select
  IF (check_error("Obtaining CDF display")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_1150
  ENDIF
  CASE (type_mean)
   OF "ADD ON":
    IF (cnvtreal(key_id1_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key1_pos].translated = 0
     SET cust_table_name = "BILL_ITEM"
     SET cust_col_name = "BILL_ITEM_ID"
     SET cust_id_col_name = "KEY1_ID"
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id1_vc,cust_id_col_name)
    ENDIF
    IF (cnvtreal(key_id2_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key2_pos].translated = 0
     SET cust_table_name = "CODE_VALUE"
     SET cust_col_name = "CODE_VALUE"
     SET cust_id_col_name = "KEY2_ID"
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id2_vc,cust_id_col_name)
    ENDIF
   OF "BILL CODE":
    IF (cnvtreal(key_id1_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key1_pos].translated = 0
     ...
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id1_vc,cust_id_col_name)
    ENDIF
    IF (cnvtreal(key_id3_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key3_pos].translated = 0
     SET cust_table_name = "NOMENCLATURE"
     SET cust_col_name = "NOMENCLATURE_ID"
     SET cust_id_col_name = "KEY3_ID"
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id3_vc,cust_id_col_name)
    ENDIF
    IF (cnvtreal(key_id5_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key5_pos].translated = 0
     SET cust_table_name = "CODE_VALUE"
     SET cust_col_name = "CODE_VALUE"
     SET cust_id_col_name = "KEY5_ID"
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id5_vc,cust_id_col_name)
    ENDIF
   OF "CHARGE POINT":
    IF (cnvtreal(key_id1_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key1_pos].translated = 0
    ...
    IF (cnvtreal(key_id2_vc) > 0)
     ...
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id2_vc,cust_id_col_name)
    ENDIF
    IF (cnvtreal(key_id4_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key4_pos].translated = 0
     SET cust_table_name = "CODE_VALUE"
     SET cust_col_name = "CODE_VALUE"
     SET cust_id_col_name = "KEY4_ID"
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id4_vc,cust_id_col_name)
    ENDIF
   OF "BARCODE":
    IF (cnvtreal(key_id1_vc) > 0)
     CALL put_value(cust_table_name,"KEY1_ID",key_id1_vc)
    ENDIF
   OF "WORKLOAD":
    IF (cnvtreal(key_id1_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key1_pos].translated = 0
    ...
    IF (cnvtreal(key_id3_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key3_pos].translated = 0
     SET cust_table_name = "WORKLOAD_CODE"
     SET cust_col_name = "WORKLOAD_CODE_ID"
     SET cust_id_col_name = "KEY3_ID"
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id3_vc,cust_id_col_name)
    ENDIF
    IF (cnvtreal(key_id12_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key12_pos].translated = 0
     SET cust_table_name = "CODE_VALUE"
     SET cust_col_name = "CODE_VALUE"
     SET cust_id_col_name = "KEY12_ID"
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id12_vc,cust_id_col_name)
    ENDIF
    IF (cnvtreal(key_id14_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key14_pos].translated = 0
     SET cust_table_name = "WORKLOAD_STANDARD"
     SET cust_col_name = "WORKLOAD_STANDARD_ID"
     SET cust_id_col_name = "KEY14_ID"
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id14_vc,cust_id_col_name)
    ENDIF
    IF (cnvtreal(key_id15_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key15_pos].translated = 0
     SET cust_table_name = "CODE_VALUE"
     SET cust_col_name = "CODE_VALUE"
     SET cust_id_col_name = "KEY15_ID"
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id15_vc,cust_id_col_name)
    ENDIF
   OF "NONCOVERED":
    IF (cnvtreal(key_id1_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key1_pos].translated = 0
   ...
   OF "INTERVALCODE":
    ...
    IF (cnvtreal(key_id2_vc) > 0)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[key2_pos].translated = 0
     SET cust_table_name = "ITEM_INTERVAL_TABLE"
     SET cust_col_name = "ITEM_INTERVAL_ID"
     SET cust_id_col_name = "KEY2_ID"
     CALL cust_sub_translate(cust_table_name,cust_col_name,key_id2_vc,cust_id_col_name)
    ENDIF
    IF (cnvtreal(key_id3_vc) > 0)
   ...
   ELSE
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV35"
    CALL merge_audit("FAILREASON","The BILL_ITEM_TYPE_CD is not supported",2)
  ENDCASE
  IF (check_error(dm_err->eproc) > 0)
   SET dm2_ref_data_reply->error_ind = 1
   GO TO exit_1150
 ...
 ELSE
  ...
  GO TO exit_1150
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"BILL_ITEM_MOD_ID")
 IF ((dm2_ref_data_reply->error_ind=1))
  SET idcd_check = is_translated(cust_tab_name,"ALL")
  GO TO exit_1150
 ENDIF
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1)
   ...
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"BILL_ITEM_MOD_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"BILL_ITEM_MOD_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  BILL_ITEM_MOD_ID")
     ELSE
      GO TO exit_1150
     ENDIF
    ELSE
   ...
   SET from_id = get_value(cust_tab_name,"BILL_ITEM_MOD_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1150
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(from_id)
   ENDIF
   SET dm2_ref_data_reply->qual[1].to_value = val
 ...
 ENDIF
#exit_1150
 CALL echorecord(dm2_ref_data_reply)
 CALL echo(drdm_no_trans_ind)
 SUBROUTINE cust_sub_translate(cs_tab_name,cs_tab_col,cs_id_val,cs_id_col_name)
  SET to_id = drdm_smt(rdds_exception,cs_id_col_name,cs_tab_name,cs_tab_col,cs_id_val,
   cs_tab_name)
  IF (isnumeric(to_id))
   CALL put_value(cust_tab_name,cs_id_col_name,to_id)
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1150
   ENDIF
  ELSE
   SET cust_rpt_missing = report_missing(cs_tab_name,rdds_exception->qual[size(rdds_exception->qual,5
     )].tru_col_name,cnvtreal(cs_id_val))
   CALL echo(cust_rpt_missing)
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ENDIF
 END ;Subroutine
 SUBROUTINE drdm_smt(ds_rdds_exception,ds_tab_col_name,ds_tru_tab_name,ds_tru_col_name,ds_smt_id,
  ds_smt_tab)
   DECLARE cust_temp_size = i4
   SET cust_temp_size = (size(ds_rdds_exception->qual,5)+ 1)
   SET stat = alterlist(ds_rdds_exception->qual,cust_temp_size)
   SET ds_rdds_exception->qual[cust_temp_size].tab_col_name = ds_tab_col_name
   SET ds_rdds_exception->qual[cust_temp_size].tru_tab_name = ds_tru_tab_name
   SET ds_rdds_exception->qual[cust_temp_size].tru_col_name = ds_tru_col_name
   CALL echorecord(ds_rdds_exception)
   RETURN(select_merge_translate(ds_smt_id,ds_smt_tab))
 END ;Subroutine
END GO
