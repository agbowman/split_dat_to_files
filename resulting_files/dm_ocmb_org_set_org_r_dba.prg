CREATE PROGRAM dm_ocmb_org_set_org_r:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "ORG_SET_ORG_R"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_OCMB_ORG_SET_ORG_R"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[count1].org_set_org_r_id = frm.org_set_org_r_id, rreclist->from_rec[count1].
 ...
 IF (count1 > 0)
  FOR (loopcount = 1 TO count1)
    IF (del_from(rreclist->from_rec[loopcount].org_set_org_r_id,loopcount)=0)
     GO TO end_of_program
    ENDIF
  ENDFOR
 ENDIF
 SUBROUTINE del_from(s_c_id,s_frm_cnt)
   UPDATE  FROM org_set_org_r o
    SET o.active_ind = 0, o.active_status_cd = combinedaway, o.active_status_dt_tm = cnvtdatetime(
      sysdate),
     o.active_status_prsnl_id = reqinfo->updt_id, o.updt_dt_tm = cnvtdatetime(sysdate), o.updt_id =
     reqinfo->updt_id,
     o.updt_applctx = reqinfo->updt_applctx, o.updt_task = reqinfo->updt_task, o.updt_cnt = (o
     .updt_cnt+ 1)
    WHERE o.org_set_org_r_id=s_c_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "ORG_SET_ORG_R"
   ...
   SET request->xxx_combine_det[icombinedet].entity_pk[1].col_name = "ORG_SET_ORG_R_ID"
   ...
   IF (curqual=0)
    ...
    SET request->error_message = build(
     "DEL_FROM: No values found on the ORG_SET_ORG_R table with org_set_org_r_id = ",s_c_id)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
