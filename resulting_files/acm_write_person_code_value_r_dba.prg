CREATE PROGRAM acm_write_person_code_value_r:dba
 ...
 DECLARE ubound = i4 WITH protect, noconstant(reply->person_code_value_r_qual_cnt)
 SET stat = alterlist(reply->person_code_value_r_qual,reply->person_code_value_r_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->person_code_value_r_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->person_code_value_r_qual[index].person_code_value_r_id = acm_request->
     person_code_value_r_qual[index].person_code_value_r_id
     SET reply->person_code_value_r_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_person_code_value_r
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->act_cnt > 0))
  EXECUTE acm_act_person_code_value_r
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_person_code_value_r
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_person_code_value_r
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
