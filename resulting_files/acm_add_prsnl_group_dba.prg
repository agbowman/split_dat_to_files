CREATE PROGRAM acm_add_prsnl_group:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->prsnl_group_qual[xref->add[index].idx].prsnl_group_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(prsnl_seq,nextval)
     FROM dual
     DETAIL
      acm_request->prsnl_group_qual[xref->add[index].idx].prsnl_group_id = cnvtreal(nextseqnum),
      reply->prsnl_group_qual[xref->add[index].idx].prsnl_group_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->prsnl_group_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->prsnl_group_qual[xref->add[index].idx].prsnl_group_id = acm_request->prsnl_group_qual[
    xref->add[index].idx].prsnl_group_id
   ENDIF
   SET reply->prsnl_group_qual[xref->add[index].idx].status = 0
   IF ((acm_request->prsnl_group_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->prsnl_group_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->prsnl_group_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->prsnl_group_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   prsnl_group p
  SET p.prsnl_group_id = acm_request->prsnl_group_qual[xref->add[d.seq].idx].prsnl_group_id, p
   .prsnl_group_name = acm_request->prsnl_group_qual[xref->add[d.seq].idx].prsnl_group_name, p
   .prsnl_group_name_key = trim(cnvtupper(cnvtalphanum(acm_request->prsnl_group_qual[xref->add[d.seq]
      .idx].prsnl_group_name)),3),
   p.prsnl_group_type_cd = acm_request->prsnl_group_qual[xref->add[d.seq].idx].prsnl_group_type_cd, p
   .prsnl_group_desc = acm_request->prsnl_group_qual[xref->add[d.seq].idx].prsnl_group_desc, p
   .prsnl_group_class_cd = acm_request->prsnl_group_qual[xref->add[d.seq].idx].prsnl_group_class_cd,
   p.contributor_system_cd =
   IF ((acm_request->prsnl_group_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0)) acm_request
    ->prsnl_group_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , p.beg_effective_dt_tm = cnvtdatetime(acm_request->prsnl_group_qual[xref->add[d.seq].idx].
   ...
   p.service_resource_cd = acm_request->prsnl_group_qual[xref->add[d.seq].idx].service_resource_cd, p
   .data_status_cd =
   IF ((acm_request->prsnl_group_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request->
    prsnl_group_qual[xref->add[d.seq].idx].data_status_cd
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->prsnl_group_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->prsnl_group_qual[xref->add[index].idx].status=0))
