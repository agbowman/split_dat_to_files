CREATE PROGRAM bhs_athn_med_for_sm
 IF (( $3 != 0))
 ...
 SELECT DISTINCT INTO  $1
  ...
  HEAD o.order_id
   ...
   col + 1, v941, row + 1,
   v942 = build("<ProtocolOrderId>",o_protocol_order_id,"</ProtocolOrderId>"), col + 1, v942,
   row + 1, v943 = build("<RefTextMask>",
    IF (oc.ref_text_mask > 0) "RefTextAvailable"
    ELSE "RefTextNotAvailable"
    ENDIF
    ,"</RefTextMask>"), col + 1,
   v943, row + 1, v16 = build("<StopDateTime>",format(od1.oe_field_dt_tm_value,"MM/DD/YYYY HH:MM;;D"),
    "</StopDateTime>"),
   col + 1, v16, row + 1,
   v17 = build("<StopDateTimeZone>",substring(21,3,datetimezoneformat(od1.oe_field_dt_tm_value,od1
      .oe_field_tz,"MM/dd/yyyy hh:mm:ss ZZZ",curtimezonedef)),"</StopDateTimeZone>"), col + 1, v17,
