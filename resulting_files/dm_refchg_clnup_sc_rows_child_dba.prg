CREATE PROGRAM dm_refchg_clnup_sc_rows_child:dba
 ...
 ENDIF
 IF (check_logfile("dm_refchg_clnup_sc_",".log","DM2_REFCHG_CLNUP_SC_ROWS_CHILD...") != 1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DECLARE drcsrc_child_tab = vc
 DECLARE drcsrc_r_cnt = i4 WITH noconstant(0)
 DECLARE dbp_cnt = i4 WITH noconstant(0)
 DECLARE dbp_rs_cnt = i4 WITH noconstant(0)
 DECLARE dbp_idx = i4 WITH noconstant(0)
 DECLARE drcs_suffix = vc
 DECLARE drcsrc_t_cnt = i4 WITH noconstant(0)
 DECLARE dbp_rs_cnt = i4 WITH noconstant(0)
 DECLARE tbl_loop = i4
 DECLARE nodelete_ind = i2
 DECLARE drcsrc_con_ind = i2 WITH noconstant(1)
 DECLARE drcsrc_build_parser(dbp_str=vc) = null
 DECLARE find_tab_data(ftd_child_table=vc,ftd_tab_rs=vc(ref),ftd_rs_cnt=i4(ref)) = i4
 DECLARE drcsrc_exe_parser(dep_size=i4) = i2
 DECLARE drcsrc_temp_tab = vc
 DECLARE drcsrc_env_str = vc
 DECLARE drcsrc_env_flag = i2 WITH noconstant(0)
 SET dm2_rdds_rec->main_process = "MOVER"
 SET dm2_rdds_rec->mode = "DATABASE"
 FREE RECORD drcsrc_par
 RECORD drcsrc_par(
   1 list[*]
     2 par_statement = vc
 )
 FREE RECORD drcsrc_rows
 RECORD drcsrc_rows(
   1 data[*]
     2 pk_where = vc
     2 table_name = vc
 ...
 SET dm_err->eproc = "Starting dm_refchg_clnup_sc_rows_child.."
 CALL disp_msg("",dm_err->logfile,0)
 SET reply->more_rows_ind = 0
 SET drcsrc_child_tab = cnvtupper(trim(request->table_name,3))
 SELECT INTO "nl:"
  FROM dm_env_reltn der,
   dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="DM_ENV_ID"
   AND der.parent_env_id=di.info_number
   AND der.relationship_type="REFERENCE MERGE"
  HEAD REPORT
   drcsrc_env_str = "("
  DETAIL
   IF (drcsrc_env_flag=0)
    drcsrc_env_str = build(drcsrc_env_str,der.child_env_id), drcsrc_env_flag = 1
   ELSE
    drcsrc_env_str = build(drcsrc_env_str,",",der.child_env_id)
   ENDIF
  FOOT REPORT
   drcsrc_env_str = build(drcsrc_env_str,")")
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET dm_err->eproc = "There is not a valid target environment id for current environment"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 CALL echo(drcsrc_env_str)
 SELECT INTO "nl:"
  FROM dm_refchg_tab_r d
  WHERE d.child_table=drcsrc_child_tab
   AND ((d.process_type != "MOVER ONLY") OR (d.process_type = null))
  WITH nocounter
 ;end select
 IF (curqual > 0)
  SELECT INTO "nl:"
   cname = nullval(d.context_name,"NULL")
   FROM dm_chg_log d
   WHERE parser(concat("d.target_env_id in ",drcsrc_env_str))
    AND d.log_type IN ("REFCHG", "NORDDS")
    AND d.table_name=drcsrc_child_tab
    AND d.delete_ind=0
    AND d.updt_task != 4310002
   HEAD REPORT
    stat = alterlist(drcsrc_rows->data,1000)
   DETAIL
    drcsrc_r_cnt = (drcsrc_r_cnt+ 1), drcsrc_rows->data[drcsrc_r_cnt].pk_where = d.pk_where,
    drcsrc_rows->data[drcsrc_r_cnt].table_name = d.table_name,
    drcsrc_rows->data[drcsrc_r_cnt].target_env_id = d.target_env_id, drcsrc_rows->data[drcsrc_r_cnt].
    log_id = d.log_id, drcsrc_rows->data[drcsrc_r_cnt].context_name = cname
   FOOT REPORT
    stat = alterlist(drcsrc_rows->data,drcsrc_r_cnt)
   WITH nocounter, maxqual(d,1000)
  ;end select
  IF (drcsrc_r_cnt < 1000)
   SET reply->more_rows_ind = 0
  ELSE
   SET reply->more_rows_ind = 1
  ENDIF
  FOR (lp_cnt = 1 TO drcsrc_r_cnt)
    SET drcsrc_rows->data[lp_cnt].tab_ndx = find_tab_data(drcsrc_rows->data[lp_cnt].table_name,
     drcsrc_tab,dbp_rs_cnt)
    IF ((drcsrc_rows->data[lp_cnt].tab_ndx < 0))
     GO TO exit_program
    ENDIF
    SET drcsrc_con_ind = 1
    WHILE (drcsrc_con_ind=1)
      SET drcsrc_rows->data[lp_cnt].insrc_ind = 0
      SET drcsrc_temp_tab = drcsrc_rows->data[lp_cnt].table_name
      CALL drcsrc_build_parser(concat("select into 'nl:' from ",drcsrc_rows->data[lp_cnt].table_name,
        " ",drcsrc_tab->data[drcsrc_rows->data[lp_cnt].tab_ndx].tab_suffix))
      CALL drcsrc_build_parser(drcsrc_rows->data[lp_cnt].pk_where)
      CALL drcsrc_build_parser(build("detail drcsrc_rows->data[",lp_cnt,"].insrc_ind = 1"))
      CALL drcsrc_build_parser(build("drcsrc_rows->data[",lp_cnt,"].table_id =",drcsrc_tab->data[
        drcsrc_rows->data[lp_cnt].tab_ndx].tab_suffix,".",
        drcsrc_tab->data[drcsrc_rows->data[lp_cnt].tab_ndx].parent_id_col))
      IF ((drcsrc_tab->data[drcsrc_rows->data[lp_cnt].tab_ndx].parent_tab_col > " "))
       CALL drcsrc_build_parser(build("  drcsrc_rows->data[",lp_cnt,"].table_name = ",drcsrc_tab->
         data[drcsrc_rows->data[lp_cnt].tab_ndx].tab_suffix,".",
         drcsrc_tab->data[drcsrc_rows->data[lp_cnt].tab_ndx].parent_tab_col))
      ELSE
       CALL drcsrc_build_parser(build("  drcsrc_rows->data[",lp_cnt,"].table_name = '",drcsrc_tab->
         data[drcsrc_rows->data[lp_cnt].tab_ndx].parent_table,"'"))
      ENDIF
      CALL drcsrc_build_parser("with nocounter go")
      IF (drcsrc_exe_parser(dbp_cnt)=1)
       GO TO exit_program
      ENDIF
      IF (curqual > 0)
       SELECT INTO "nl:"
        FROM dm_refchg_tab_r d
        WHERE (d.parent_table=drcsrc_rows->data[lp_cnt].table_name)
         AND d.child_table=drcsrc_temp_tab
         AND ((d.process_type != "MOVER ONLY") OR (d.process_type = null))
        WITH nocounter
       ;end select
       IF (check_error("Check the parent child relationship") > 0)
        SET drcsrc_con_ind = 0
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        GO TO exit_program
       ENDIF
       IF (curqual=0)
        SET drcsrc_con_ind = 0
        SET drcsrc_rows->data[lp_cnt].no_change_ind = 1
       ELSE
        SET drcsrc_rows->data[lp_cnt].tab_ndx = find_tab_data(drcsrc_rows->data[lp_cnt].table_name,
         drcsrc_tab,dbp_rs_cnt)
        IF ((drcsrc_rows->data[lp_cnt].tab_ndx < 0))
         GO TO exit_program
        ENDIF
        SET drcsrc_rows->data[lp_cnt].pk_where = concat("WHERE ",drcsrc_tab->data[drcsrc_rows->data[
         lp_cnt].tab_ndx].tab_suffix,".",drcsrc_tab->data[drcsrc_rows->data[lp_cnt].tab_ndx].
         pk_col_name," =",
         trim(cnvtstring(drcsrc_rows->data[lp_cnt].table_id),3),".0")
        IF (trim(drcsrc_tab->data[drcsrc_rows->data[lp_cnt].tab_ndx].parent_table,3) <= " ")
         SET dm_err->eproc = build("The parent isn't a child, the search finishes for log_id:",
          drcsrc_rows->data[lp_cnt].log_id)
         CALL disp_msg("",dm_err->logfile,0)
         SET drcsrc_con_ind = 0
        ENDIF
       ENDIF
      ELSE
       SET dm_err->eproc = build("There is no qualified data from table: ",drcsrc_temp_tab,"/log_id:",
        drcsrc_rows->data[lp_cnt].log_id)
       CALL disp_msg("",dm_err->logfile,0)
       SET drcsrc_con_ind = 0
       UPDATE  FROM dm_chg_log d
        SET d.log_type = "NO SRC", d.updt_task = 4310002, d.updt_cnt = (d.updt_cnt+ 1),
         d.updt_dt_tm = cnvtdatetime(curdate,curtime3)
        WHERE (d.log_id=drcsrc_rows->data[lp_cnt].log_id)
        WITH nocounter
       ;end update
       IF (check_error("Update dm_chg_log to NO SRC") > 0)
        SET drcsrc_con_ind = 0
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ROLLBACK
        GO TO exit_program
      ...
      ENDIF
    ENDWHILE
    IF ((drcsrc_rows->data[lp_cnt].no_change_ind=1)
     AND (drcsrc_rows->data[lp_cnt].insrc_ind=1))
     UPDATE  FROM dm_chg_log d
      SET d.updt_task = 4310002, d.updt_cnt = (d.updt_cnt+ 1), d.updt_dt_tm = cnvtdatetime(curdate,
        curtime3)
      WHERE (d.log_id=drcsrc_rows->data[lp_cnt].log_id)
      WITH nocounter
     ;end update
     IF (check_error("Update dm_chg_log to new update task") > 0)
      SET drcsrc_con_ind = 0
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ROLLBACK
      GO TO exit_program
     ENDIF
    ELSEIF ((drcsrc_rows->data[lp_cnt].no_change_ind=0)
     AND (drcsrc_rows->data[lp_cnt].insrc_ind=1))
     UPDATE  FROM dm_chg_log d
      SET d.log_type = "BADLOG", d.updt_task = 4310002, d.updt_cnt = (d.updt_cnt+ 1),
       d.updt_dt_tm = cnvtdatetime(curdate,curtime3)
      WHERE (d.log_id=drcsrc_rows->data[lp_cnt].log_id)
      WITH nocounter
     ;end update
     IF (check_error("Update dm_chg_log to BADLOG") > 0)
      SET drcsrc_con_ind = 0
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ROLLBACK
      GO TO exit_program
     ENDIF
     SET dbp_idx = 0
     IF (locateval(dbp_idx,1,size(dguc_reply->dtd_hold,5),drcsrc_rows->data[lp_cnt].table_name,
      dguc_reply->dtd_hold[dbp_idx].tbl_name)=0)
      SET dguc_request->what_tables = drcsrc_rows->data[lp_cnt].table_name
      SET dguc_request->is_mrg_ind = 0
      SET dguc_request->is_ref_ind = 1
      SET dguc_request->only_special_ind = 0
      EXECUTE dm_get_unique_columns  WITH replace("DGUC_REPLY","TEMP_DGUC")
      FOR (dh_ndx = 1 TO size(temp_dguc->dtd_hold,5))
        SET dguc_reply->rs_tbl_cnt = (dguc_reply->rs_tbl_cnt+ 1)
        SET stat = alterlist(dguc_reply->dtd_hold,dguc_reply->rs_tbl_cnt)
        SET dguc_reply->dtd_hold[dguc_reply->rs_tbl_cnt].tbl_name = temp_dguc->dtd_hold[dh_ndx].
        tbl_name
        SET dguc_reply->dtd_hold[dguc_reply->rs_tbl_cnt].tbl_suffix = temp_dguc->dtd_hold[dh_ndx].
        tbl_suffix
        SET dguc_reply->dtd_hold[dguc_reply->rs_tbl_cnt].pk_cnt = temp_dguc->dtd_hold[dh_ndx].pk_cnt
        SET stat = alterlist(dguc_reply->dtd_hold[dguc_reply->rs_tbl_cnt].pk_hold,temp_dguc->
         dtd_hold[dh_ndx].pk_cnt)
        SET dguc_reply->dtd_hold[dguc_reply->rs_tbl_cnt].pk_cnt = temp_dguc->dtd_hold[dh_ndx].pk_cnt
        FOR (pk_ndx = 1 TO temp_dguc->dtd_hold[dh_ndx].pk_cnt)
         SET dguc_reply->dtd_hold[dguc_reply->rs_tbl_cnt].pk_hold[pk_ndx].pk_datatype = temp_dguc->
         dtd_hold[dh_ndx].pk_hold[pk_ndx].pk_datatype
         SET dguc_reply->dtd_hold[dguc_reply->rs_tbl_cnt].pk_hold[pk_ndx].pk_name = temp_dguc->
         dtd_hold[dh_ndx].pk_hold[pk_ndx].pk_name
        ENDFOR
      ENDFOR
     ENDIF
     SET dm2_ref_data_doc->env_target_id = drcsrc_rows->data[lp_cnt].target_env_id
     SET dm_err->eproc = concat("Calling oracle procedure for ",drcsrc_rows->data[lp_cnt].pk_where)
     CALL disp_msg("",dm_err->logfile,0)
     IF (filter_proc_call(drcsrc_rows->data[lp_cnt].table_name,drcsrc_rows->data[lp_cnt].pk_where))
      CALL trigger_proc_call(drcsrc_rows->data[lp_cnt].table_name,drcsrc_rows->data[lp_cnt].pk_where,
       drcsrc_rows->data[lp_cnt].context_name,"",0.0)
     ENDIF
     IF ((dm_err->err_ind=1))
      ROLLBACK
      GO TO exit_program
 ...
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "There are no qualified parent child relationships from table DM_REFCHG_TAB_R"
  GO TO exit_program
 ENDIF
 SUBROUTINE drcsrc_build_parser(dbp_str)
   SET dbp_cnt = (dbp_cnt+ 1)
   IF (mod(dbp_cnt,10)=1)
    SET stat = alterlist(drcsrc_par->list,(dbp_cnt+ 9))
   ENDIF
   SET drcsrc_par->list[dbp_cnt].par_statement = dbp_str
 END ;Subroutine
 SUBROUTINE drcsrc_exe_parser(dep_size)
   FOR (dec_lp = 1 TO dep_size)
     CALL parser(drcsrc_par->list[dec_lp].par_statement,1)
   ENDFOR
   SET dbp_cnt = 0
   SET stat = alterlist(drcsrc_par->list,0)
   IF (check_error("execute parser statement") > 0)
    RETURN(1)
   ENDIF
   RETURN(0)
 END ;Subroutine
 SUBROUTINE find_tab_data(ftd_child_table,ftd_tab_rs,ftd_rs_cnt)
   SET dm_err->eproc = concat("Finding data for table ",ftd_child_table)
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE ftd_idx = i4 WITH noconstant(0)
   SET ftd_idx = locateval(ftd_idx,1,size(ftd_tab_rs->data,5),ftd_child_table,ftd_tab_rs->data[
    ftd_idx].table_name)
   IF (ftd_idx > 0)
    RETURN(ftd_idx)
   ELSE
    SET ftd_rs_cnt = (ftd_rs_cnt+ 1)
    SET stat = alterlist(ftd_tab_rs->data,ftd_rs_cnt)
    SELECT INTO "nl:"
     FROM user_constraints uc,
      user_cons_columns ucc
     PLAN (uc
      WHERE uc.table_name=ftd_child_table
       AND uc.constraint_type="P")
      JOIN (ucc
      WHERE uc.constraint_name=ucc.constraint_name)
     DETAIL
      ftd_tab_rs->data[ftd_rs_cnt].pk_col_name = ucc.column_name, ftd_tab_rs->data[ftd_rs_cnt].
      table_name = uc.table_name
     WITH nocounter
    ;end select
    IF (check_error("Get primary key columns") > 0)
     RETURN(- (1))
    ENDIF
    SELECT INTO "nl:"
     FROM dm_refchg_tab_r drtr
     WHERE drtr.child_table=ftd_child_table
      AND ((drtr.process_type != "MOVER ONLY") OR (drtr.process_type = null))
     DETAIL
      ftd_tab_rs->data[ftd_rs_cnt].parent_table = drtr.parent_table, ftd_tab_rs->data[ftd_rs_cnt].
      parent_id_col = drtr.parent_id_col, ftd_tab_rs->data[ftd_rs_cnt].parent_tab_col = drtr
      .parent_tab_col
     WITH nocounter, maxqual(drtr,1)
    ;end select
    IF (check_error("Get parent table information") > 0)
     RETURN(- (1))
    ENDIF
    SELECT INTO "nl:"
     table_suffix
     FROM dm_tables_doc d
     WHERE d.table_name=ftd_child_table
     DETAIL
      ftd_tab_rs->data[ftd_rs_cnt].tab_suffix = build("t",d.table_suffix)
     WITH nocounter
    ;end select
    IF (check_error("Get table suffix") > 0)
     RETURN(- (1))
    ENDIF
    RETURN(ftd_rs_cnt)
   ENDIF
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "...Ending dm_refchg_clnup_sc_rows_child"
 CALL final_disp_msg("dm_refchg_clnup_sc_")
END GO
