CREATE PROGRAM bhs_interpreter_service_rpt:dba
 ...
 ENDIF
 SET email_list = "tim.moriarty@bhs.org"
 ...
 SELECT INTO "nl:"
  ...
  HEAD ce.clinical_event_id
   start_date_string = concat(start_day,"-",start_month,"-",start_year,
    ...
    " ",end_hour,":",end_min), t_record->encntr_qual[idx].ce_cnt += 1
   ...
   IF (ce3_null=1)
    IF (p.active_ind=1
     AND ((p.end_effective_dt_tm > cnvtdatetime(t_record->beg_date)) OR (p.active_status_cd=
    mf_active_cd)) )
     t_record->encntr_qual[idx].ce_qual[idx2].interpreter = p.name_full_formatted
    ELSE
     t_record->encntr_qual[idx].ce_qual[idx2].interpreter = "in error"
    ENDIF
   ELSE
    t_record->encntr_qual[idx].ce_qual[idx2].interpreter = ce3.result_val
   ENDIF
   IF (ce4.result_val="Inpatient")
    t_record->encntr_qual[idx].ce_qual[idx2].inp_cnt += 1
   ELSEIF (ce4.result_val="Outpatient")
    t_record->encntr_qual[idx].ce_qual[idx2].outp_cnt += 1
   ENDIF
 ...
 SELECT INTO "language_per_unit.xls"
  ...
  HEAD d.seq
   ...
   ENDIF
  HEAD d1.seq
   sessions_cnt += 1, minutes_cnt += datetimediff(t_record->encntr_qual[d.seq].ce_qual[d1.seq].
    end_dt_tm,t_record->encntr_qual[d.seq].ce_qual[d1.seq].start_dt_tm,4)
  FOOT  unit
   t_line = concat(t_line,cnvtstring(inpt_cnt),char(9),cnvtstring(outp_cnt),char(9),
    trim(cnvtstring(patient_cnt)),char(9),trim(cnvtstring(sessions_cnt)),char(9),trim(cnvtstring(
      minutes_cnt)),
    char(9)), row + 1, col 0,
   t_line, total_patient_cnt += patient_cnt, total_sessions_cnt += sessions_cnt,
   total_minutes_cnt += minutes_cnt, total_inpt_cnt += inpt_cnt, total_outp_cnt += outp_cnt
  FOOT REPORT
 ...
 SELECT INTO "patients_per_interpreter.xls"
  ...
  HEAD interpreter
   IF (trim(interpreter,3) != "in error")
  ...
  HEAD d1.seq
   IF (trim(interpreter,3) != "in error")
    t_st_tm = t_record->encntr_qual[d.seq].ce_qual[d1.seq].start_dt_tm, t_minutes = datetimediff(
     ...
     .seq].start_dt_tm,4), sessions_cnt += 1,
    minutes_cnt += t_minutes
    IF (hour(t_st_tm) >= 0
     AND hour(t_st_tm) < 2)
     0_hour += t_minutes, total_0_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 2
     AND hour(t_st_tm) < 4)
     2_hour += t_minutes, total_2_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 4
     AND hour(t_st_tm) < 6)
     4_hour += t_minutes, total_4_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 6
     AND hour(t_st_tm) < 8)
     6_hour += t_minutes, total_6_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 8
     AND hour(t_st_tm) < 10)
     8_hour += t_minutes, total_8_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 10
     AND hour(t_st_tm) < 12)
     10_hour += t_minutes, total_10_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 12
     AND hour(t_st_tm) < 14)
     12_hour += t_minutes, total_12_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 14
     AND hour(t_st_tm) < 16)
     14_hour += t_minutes, total_14_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 16
     AND hour(t_st_tm) < 18)
     16_hour += t_minutes, total_16_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 18
     AND hour(t_st_tm) < 20)
     18_hour += t_minutes, total_18_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 20
     AND hour(t_st_tm) < 22)
     20_hour += t_minutes, total_20_hour += t_minutes
    ELSEIF (hour(t_st_tm) >= 22
     AND hour(t_st_tm) < 24)
     22_hour += t_minutes, total_22_hour += t_minutes
    ENDIF
   ENDIF
  FOOT  interpreter
   IF (trim(interpreter,3) != "in error")
 ...
 IF (findfile("language_per_unit.xls")=1
  ...
  SET dclcom = concat('echo " " | mailx -s "',subject_line,'" ','-a "language_per_unit.xls" ',
   '-a "patients_per_interpreter.xls" ',
   '-a "english_per_interpreter.xls" ',email_list)
