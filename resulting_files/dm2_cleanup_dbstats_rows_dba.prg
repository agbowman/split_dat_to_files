CREATE PROGRAM dm2_cleanup_dbstats_rows:dba
 ...
 SUBROUTINE ddf_context_lock_maint(dclm_mode,dclm_process,dclm_process_code,dclm_owner,
  dclm_table_name,dclm_object_id,dclm_lock_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dcdr_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dcdr_err_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_dcdr",".log","dm2_cleanup_dbstats_rows LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_cleanup_dbstats_rows"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm2_process_event_rs->status = dpl_executing
 SET dm2_process_rs->process_name = dpl_cleanup_stats_rows
 CALL dm2_process_log_add_detail_text(dpl_audsid,currdbhandle)
 IF (dm2_process_log_row(dpl_cleanup_stats_rows,dpl_execution,dpl_no_prev_id,1)=0)
  GO TO exit_script
 ENDIF
 SET dcdr_log_id = dm2_process_event_rs->dm_process_event_id
 SET dm2_process_rs->process_name = ""
 SET dm_err->eproc = "Checking for dm_info row to skip stats rows cleanup"
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_CLEANUP_DBSTATS_ROWS"
   AND di.info_name="SKIP CLEANUP"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual > 0)
  SET dm_err->eproc = "The dm_info row to skip stats rows cleanup was found. Exiting from script."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF (ddf_clean_dpq(dpq_statistics,"*")=0)
  GO TO exit_script
 ENDIF
 IF (ddf_clean_dpq(dpq_freq_statistics,"*")=0)
  GO TO exit_script
 ENDIF
 IF (ddf_clean_dst("GOLDAVG")=0)
  GO TO exit_script
 ENDIF
 IF (ddf_clean_dst("DMSTG*")=0)
  GO TO exit_script
 ENDIF
 IF (ddf_clean_dpe(dpl_statistics,"*")=0)
  GO TO exit_script
 ENDIF
#exit_script
 ...
 SET dcdr_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_process_queue_single,dpl_execution,dcdr_log_id,1)
 SET dm2_process_rs->process_name = ""
 SET dm_err->err_ind = dcdr_err_ind
 SET dm_err->eproc = "Ending dm2_cleanup_dbstats_rows"
 CALL final_disp_msg("dm2_dcdr")
END GO
