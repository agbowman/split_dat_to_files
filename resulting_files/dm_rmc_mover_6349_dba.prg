CREATE PROGRAM dm_rmc_mover_6349:dba
 ...
 DECLARE cust_from_cd_col = vc
 DECLARE cust_new_cd_col = vc
 DECLARE cust_prefix = vc
 DECLARE cust_cd_pos = i2
 DECLARE cust_suffix = vc
 DECLARE cust_from_cd_value = vc
 DECLARE cust_to_cd_value = vc
 DECLARE cust_pos = i2
 ...
 SET cust_col_cnt = get_col_pos(cust_tab_name,"CD_COLUMN")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_cnt].translated = 0
 SET cust_from_cd_col = get_value(cust_tab_name,"CD_COLUMN","FROM")
 IF (findstring("RADUDF",cust_from_cd_col)=1)
  SET cust_prefix = "RADUDF"
  SET cust_cd_pos = 7
 ELSEIF (findstring("UDF",cust_from_cd_col)=1)
  SET cust_prefix = "UDF"
  SET cust_cd_pos = 4
 ELSE
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV28"
  CALL merge_audit("FAILREASON",
   "RAD_OMF_MAMMO_USER_DEF: the prefix for the CD_COLUMN column value was not valid",1)
  GO TO exit_6349
 ENDIF
 SET cust_pos = findstring("TEX",cust_from_cd_col)
 IF (cust_pos=0)
  SET cust_pos = findstring("NUM",cust_from_cd_col)
  IF (cust_pos=0)
   SET cust_pos = findstring("HEA",cust_from_cd_col)
   IF (cust_pos=0)
    SET cust_pos = findstring("DAT",cust_from_cd_col)
    IF (cust_pos=0)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV28"
     CALL merge_audit("FAILREASON",
      "RAD_OMF_MAMMO_USER_DEF: the suffix for the CD_COLUMN column value was not valid",1)
     GO TO exit_6349
    ENDIF
   ENDIF
  ENDIF
 ENDIF
 SET cust_from_cd_value = substring(cust_cd_pos,(cust_pos - cust_cd_pos),cust_from_cd_col)
 SET cust_to_cd_value = select_merge_translate(cust_from_cd_value,"CODE_VALUE")
 IF ( NOT (isnumeric(cust_to_cd_value)))
  SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_from_cd_value))
  CALL evaluate_rpt_missing(cust_rpt_missing)
 ELSE
  SET cust_suffix = substring(cust_pos,((size(cust_from_cd_col,1) - cust_pos)+ 1),cust_from_cd_col)
  SET cust_new_cd_col = concat(cust_prefix,cust_to_cd_value,cust_suffix)
  CALL put_value(cust_tab_name,"CD_COLUMN",cust_new_cd_col)
 ENDIF
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"CD_COLUMN")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the CD_COLUMN"
