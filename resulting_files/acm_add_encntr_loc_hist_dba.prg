CREATE PROGRAM acm_add_encntr_loc_hist:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->encntr_loc_hist_qual[xref->add[index].idx].encntr_loc_hist_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(encounter_seq,nextval)
     FROM dual
     DETAIL
      acm_request->encntr_loc_hist_qual[xref->add[index].idx].encntr_loc_hist_id = cnvtreal(
       nextseqnum), reply->encntr_loc_hist_qual[xref->add[index].idx].encntr_loc_hist_id = cnvtreal(
       nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->encntr_loc_hist_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->encntr_loc_hist_qual[xref->add[index].idx].encntr_loc_hist_id = acm_request->
    encntr_loc_hist_qual[xref->add[index].idx].encntr_loc_hist_id
   ENDIF
   SET reply->encntr_loc_hist_qual[xref->add[index].idx].status = 0
   IF ((acm_request->encntr_loc_hist_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->encntr_loc_hist_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->encntr_loc_hist_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->encntr_loc_hist_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   encntr_loc_hist e
  SET e.encntr_loc_hist_id = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].
   encntr_loc_hist_id, e.activity_dt_tm =
   IF ((acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].activity_dt_tm > 0)) cnvtdatetime(
     acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].activity_dt_tm)
   ELSE null
   ENDIF
   , e.arrive_dt_tm =
   IF ((acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].arrive_dt_tm > 0)) cnvtdatetime(
     acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].arrive_dt_tm)
   ELSE null
   ENDIF
   ,
   e.arrive_prsnl_id = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].arrive_prsnl_id, e
   .chart_comment_ind = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].chart_comment_ind, e
   .comment_text = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].comment_text,
   e.depart_dt_tm =
   IF ((acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].depart_dt_tm > 0)) cnvtdatetime(
     acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].depart_dt_tm)
   ELSE null
   ENDIF
   , e.depart_prsnl_id = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].depart_prsnl_id, e
   .encntr_id =
   IF ((acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].encntr_id > 0)) acm_request->
    encntr_loc_hist_qual[xref->add[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].encntr_idx].
    encntr_id
   ENDIF
   ,
   e.encntr_type_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].encntr_type_cd, e
   .location_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].location_cd, e
   .location_status_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].location_status_cd,
   e.location_temp_ind = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].location_temp_ind, e
   .loc_bed_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].loc_bed_cd, e
   .loc_building_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].loc_building_cd,
   e.loc_facility_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].loc_facility_cd, e
   .loc_nurse_unit_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].loc_nurse_unit_cd, e
   .loc_room_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].loc_room_cd,
   e.med_service_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].med_service_cd, e
   .program_service_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].program_service_cd,
   e.specialty_unit_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].specialty_unit_cd,
   e.transaction_dt_tm =
   IF ((acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].transaction_dt_tm > 0)) cnvtdatetime(
     acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].transaction_dt_tm)
   ELSE null
   ENDIF
   , e.transfer_reason_cd = acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].
   ...
   e.end_effective_dt_tm = cnvtdatetime(acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].
    end_effective_dt_tm), e.data_status_cd =
   IF ((acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request->
    encntr_loc_hist_qual[xref->add[d.seq].idx].data_status_cd
   ELSE reqdata->data_status_cd
   ENDIF
   , e.data_status_dt_tm = cnvtdatetime(curdate,curtime3),
   e.data_status_prsnl_id = reqinfo->updt_id, e.active_ind =
   IF ((((acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((
   acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].active_status_cd=reqdata->active_status_cd
   ))) ) 1
   ELSE 0
   ENDIF
   , e.active_status_cd =
   IF ((acm_request->encntr_loc_hist_qual[xref->add[d.seq].idx].active_status_cd > 0.0)) acm_request
    ->encntr_loc_hist_qual[xref->add[d.seq].idx].active_status_cd
   ELSE reqdata->active_status_cd
   ENDIF
   ,
   e.active_status_prsnl_id = reqinfo->updt_id, e.active_status_dt_tm = cnvtdatetime(curdate,curtime3
    ), e.updt_cnt = 0,
   e.updt_dt_tm = cnvtdatetime(curdate,curtime3), e.updt_id = reqinfo->updt_id, e.updt_applctx =
   reqinfo->updt_applctx,
   e.updt_task = reqinfo->updt_task
  PLAN (d)
   JOIN (e)
  WITH nocounter, status(reply->encntr_loc_hist_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->encntr_loc_hist_qual[xref->add[index].idx].status=0))
