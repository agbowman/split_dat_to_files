CREATE PROGRAM bhs_gvw_gyn_onc_dx:dba
 FREE RECORD m_rec
 RECORD m_rec(
   1 l_cnt = i4
   1 qual[*]
     2 s_gyn_onc_orig_dx = vc
     2 f_gyn_onc_orig_dx_date = f8
     2 s_gyn_onc_orig_dx_date = vc
 ) WITH protect
 DECLARE mf_gynoncoriginaldiagnosis_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "GYNONCORIGINALDIAGNOSIS"))
 DECLARE mf_gynoncoriginaldiagnosisdate_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",
   72,"GYNONCORIGINALDIAGNOSISDATE"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ce
   WHERE ce.encntr_id=mf_enc_id
    AND ce.event_cd IN (mf_gynoncoriginaldiagnosis_cd, mf_gynoncoriginaldiagnosisdate_cd)
  ...
  HEAD ce.event_cd
   CASE (ce.event_cd)
    OF mf_gynoncoriginaldiagnosisdate_cd:
     m_rec->qual[ml_cnt].s_gyn_onc_orig_dx_date = format(cdr.result_dt_tm,"mm/dd/yy;;D"),m_rec->qual[
     ml_cnt].f_gyn_onc_orig_dx_date = cdr.result_dt_tm
    OF mf_gynoncoriginaldiagnosis_cd:
     m_rec->qual[ml_cnt].s_gyn_onc_orig_dx = trim(ce.result_val)
   ENDCASE
  WITH nocounter
 ;end select
 IF (ml_cnt < 1)
 ...
 IF ((reqinfo->updt_task=3202004))
  SET ms_text_temp = concat("\trowd\trgaph30\cellx4000\cellx6000\intbl",ms_wb,"{Gyn Onc Diagnosis} ",
   "\cell ",ms_wb,
   "{Gyn Onc Diagnosis Date} ","\cell ","\row ")
  SET reply->text = concat(reply->text," ",ms_text_temp)
 ENDIF
 SELECT INTO "nl:"
  gyn_onc_orig_dx_date = m_rec->qual[d.seq].f_gyn_onc_orig_dx_date
  FROM (dummyt d  WITH seq = m_rec->l_cnt)
  ORDER BY gyn_onc_orig_dx_date DESC
  DETAIL
   IF ((reqinfo->updt_task=3202004))
    ms_text_temp = concat("\trowd\trgaph30\cellx4000\cellx6000\intbl",ms_wr,"{",trim(m_rec->qual[d
      .seq].s_gyn_onc_orig_dx),"} ",
     "\cell ",ms_wr,"{",trim(m_rec->qual[d.seq].s_gyn_onc_orig_dx_date),"} ",
     "\cell ","\row "), reply->text = concat(reply->text," ",ms_text_temp)
   ELSE
    ms_text_temp = concat(ms_wb,ms_hi,"{Gyn Onc Diagnosis: } ",ms_wr,"{",
     trim(m_rec->qual[d.seq].s_gyn_onc_orig_dx),"} ",ms_reol,ms_wb,"{Gyn Onc Diagnosis Date: } ",
     ms_wr,"{",trim(m_rec->qual[d.seq].s_gyn_onc_orig_dx_date),"} ",ms_reol,
