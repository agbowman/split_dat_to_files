CREATE PROGRAM br_datamart_std_report_config:dba
 ...
 SET readme_data->message = "Readme Failed: Starting <br_datamart_std_report_config.prg> script"
 ...
 FOR (y = 1 TO cnt)
   IF (validate(requestin->list_0[y].mpage_pos,"") > " ")
    ...
    ELSEIF (cnvtupper(requestin->list_0[y].mpage_pos)="COLUMN 4")
     SET br_existsinfo->list_0[y].mpage_pos = 4
    ELSEIF (cnvtupper(requestin->list_0[y].mpage_pos)="COLUMN 5")
     SET br_existsinfo->list_0[y].mpage_pos = 5
    ELSE
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE b.report_mean=cnvtupper(requestin->list_0[d.seq].report_mean)
    AND b.br_datamart_category_id=c.br_datamart_category_id)
  DETAIL
   br_existsinfo->list_0[d.seq].existsind = 1, br_existsinfo->list_0[d.seq].categoryid = c
 ...
 UPDATE  FROM br_datamart_report b,
  ...
  PLAN (d
   ...
   JOIN (b
   WHERE b.report_mean=cnvtupper(requestin->list_0[d.seq].report_mean)
    AND (b.br_datamart_category_id=br_existsinfo->list_0[d.seq].categoryid))
  WITH nocounter
 ;end update
 IF (error(errmsg,0) > 0)
  ROLLBACK
 ...
 SET readme_data->message = "Readme Succeeded: <br_datamart_std_report_config.prg> script"
#exit_script
