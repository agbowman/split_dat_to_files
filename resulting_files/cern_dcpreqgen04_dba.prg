CREATE PROGRAM cern_dcpreqgen04:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   orders->dischg_dt = concat(format(datetimezone(e.disch_dt_tm,tz_index),"@SHORTDATETIME;;Q")," ",
    datetimezonebyindex(tz_index,offset,daylight,7,e.reg_dt_tm)), orders->facility =
   uar_get_code_description(e.loc_facility_cd), orders->nurse_unit = uar_get_code_display(e
    .loc_nurse_unit_cd),
  ...
  DETAIL
   IF (c.event_cd=height_cd)
    ...
    orders->height_dt_tm = concat(format(datetimezone(c.updt_dt_tm,c.performed_tz),
      "@SHORTDATETIME;;Q")," ",datetimezonebyindex(c.performed_tz,offset,daylight,7,c.updt_dt_tm))
  ...
  HEAD o.order_id
   ...
   orders->qual[ord_cnt].mnemonic = o.hna_order_mnemonic, orders->qual[ord_cnt].order_dt = concat(
    format(datetimezone(oa.order_dt_tm,oa.order_tz),"@SHORTDATETIME;;Q")," ",datetimezonebyindex(oa
     ...
     .orig_order_tz,offset,daylight,7,oa.orig_order_dt_tm)),
   ...
   DETAIL
    FOR (vv = 1 TO value(order_cnt))
      ...
      IF (first_page="N")
       BREAK
      ENDIF
      first_page = "N", "{f/12}{cpi/10}", row + 1,
      "{pos/140/60}HOSPITAL NAME", row + 1, "{pos/220/75}",
      orders->qual[vv].catalog, row + 1, "{f/4}",
      row + 1, "{pos/60/105}{cpi/14}Patient: ", "{cpi/10}{b}",
      orders->name, "{endb}", "{pos/300/105}{cpi/14}Age: ",
      "{cpi/10}{b}", orders->age, "{endb}",
      row + 1, "{pos/380/105}{cpi/14}Gender: ", "{cpi/10}{b}",
      orders->sex, "{endb}", "{pos/470/105}{cpi/14}Room/Bed: ",
      "{cpi/10}{b}", orders->room, "  ",
      orders->bed, row + 1, "{pos/60/122}{cpi/14}MR#: ",
      "{cpi/12}{B}", orders->mrn, "{endb}",
      "{pos/300/122}{cpi/14}Acct #: ", "{cpi/12}{b}", orders->fnbr,
      row + 1, "{pos/60/139}{cpi/14}Dx: ", "{cpi/12}",
      orders->admit_diagnosis, "{pos/300/139}{cpi/14}Attending Physician: ", "{cpi/12}",
      orders->attending, row + 1, "{pos/60/156}{cpi/14}Admit Date/Time: ",
      "{cpi/12}", orders->admit_dt, row + 1,
      "{pos/60/173}{cpi/14}Height: ", "{cpi/12}{b}", orders->height,
      "{endb}", "{pos/155/173}{cpi/14}Weight: ", "{cpi/12}{b}",
      orders->weight, row + 1, "{pos/300/173}{cpi/14}Isolation Type: ",
      "{cpi/12}{b}", orders->isolation, row + 1,
      "{pos/60/190}{cpi/12}{b}Order Action: ", "{cpi/10}", orders->qual[vv].action,
      "{endb}", "{pos/300/190}{cpi/14}Ordering Physician: ", "{cpi/12}{b}",
      orders->qual[vv].order_dr, row + 1, "{pos/60/207}{cpi/14}{b}Order Entered By: ",
      "{cpi/12}", orders->qual[vv].enter_by, "{endb}",
      row + 1, "{pos/300/207}{cpi/14}Date/Time Ordered: ", "{cpi/12}{b}",
      orders->qual[vv].signed_dt, row + 1, "{pos/55/228}{cpi/9}{COLOR/20/157}",
      row + 1, "{pos/55/233}{COLOR/20/157}", row + 1,
      "{pos/55/239}{COLOR/20/157}", row + 1, "{pos/60/233}{CPI/11}{b}Procedure/Order: ",
      orders->qual[vv].mnemonic, row + 1, "{pos/400/233}{b}Order Id#: ",
      CALL print(trim(cnvtstring(cnvtint(orders->qual[vv].order_id)))), row + 1, ycol = 255,
      xcol = 60
      FOR (fsub = 1 TO 31)
        FOR (x = 1 TO orders->qual[vv].d_cnt)
          IF ((((orders->qual[vv].d_qual[x].group_seq=fsub)) OR (fsub=31))
           ...
           orders->qual[vv].d_qual[x].print_ind = 1, xcol = 60,
           CALL print(calcpos(xcol,ycol)),
           "{cpi/14}", orders->qual[vv].d_qual[x].label_text, ": ",
           xcol = 240,
           CALL print(calcpos(xcol,ycol)), "{cpi/12}{b}",
           orders->qual[vv].d_qual[x].value, row + 1, ycol = (ycol+ 12)
          ENDIF
        ENDFOR
      ENDFOR
      xcol = 60, ycol = (ycol+ 15),
      CALL print(calcpos(xcol,ycol)),
      "{cpi/14}Blood Bank ID: {u}                                         {endu}", row + 1, ycol = (
      ycol+ 15),
      CALL print(calcpos(xcol,ycol)),
      "{cpi/14}Collected By: {u}                                            {endu}", row + 1,
      ycol = (ycol+ 15),
      CALL print(calcpos(xcol,ycol)),
      "{cpi/14}collection Date/Time: {u}                                    {endu}",
      row + 1, ycol = (ycol+ 15), xcol = 60,
      CALL print(calcpos(xcol,ycol)), "{cpi/14}{u/8}Comments: ", row + 1,
      ycol = (ycol+ 12)
      IF ((orders->qual[vv].comment_ind=1)
       ...
       IF ((orders->qual[vv].com_ln_cnt > 7))
        xcol = 75,
        CALL print(calcpos(xcol,ycol)), "{CPI/12}",
        "**** Please check chart for further comments ****", ycol = (ycol+ 12)
       ENDIF
      ENDIF
    ENDFOR
    "{font/8}{cpi/12}{pos/50/720}", "dcpreqgen04"
