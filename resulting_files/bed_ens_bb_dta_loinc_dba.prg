CREATE PROGRAM bed_ens_bb_dta_loinc:dba
 SET modify = predeclare
 FREE SET reply
 RECORD reply(
   1 success[*]
     2 bedrock_row_id = i4
     2 concept_ident_bb_dta_id = f8
 ...
 DECLARE dactivecd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",48,"ACTIVE"))
 DECLARE dinactivecd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",48,"INACTIVE"))
 ...
 FOR (x = 1 TO size(request->codes,5))
   ...
   SET ndupfoundflag = 0
   SELECT INTO "nl:"
    FROM concept_ident_bb_dta cid
    PLAN (cid
     WHERE (cid.task_assay_cd=request->codes[x].task_assay_cd)
      AND (cid.concept_type_flag=request->codes[x].concept_type_flag)
      AND cid.end_effective_dt_tm=cnvtdatetime("31 DEC 2100 00:00")
      AND (cid.concept_ident_bb_dta_id != request->codes[x].concept_ident_bb_dta_id))
    DETAIL
   ...
   ;end select
   IF (ndupfoundflag=0)
    IF ((request->codes[x].concept_ident_bb_dta_id > 0.0))
     SET nerrcode = 0
     UPDATE  FROM concept_ident_bb_dta cid
      ...
      PLAN (cid
       WHERE (cid.concept_ident_bb_dta_id=request->codes[x].concept_ident_bb_dta_id))
      WITH nocounter
     ;end update
    ...
    IF (((size(trim(request->codes[x].loinc_code)) > 0) OR ((request->codes[x].ignore_ind=1))) )
     ...
     ;end select
     SET nerrcode = error(serrmsg,1)
     INSERT  FROM concept_ident_bb_dta cid
      SET cid.active_ind = 1, cid.active_status_cd = dactivecd, cid.active_status_dt_tm =
       ...
       cid.concept_cki = sconceptcki, cid.concept_ident_bb_dta_id = dloincid, cid.concept_type_flag
        = request->codes[x].concept_type_flag,
       cid.end_effective_dt_tm = cnvtdatetime("31 DEC 2100 00:00"), cid.task_assay_cd = request->
       codes[x].task_assay_cd, cid.updt_applctx = reqinfo->updt_applctx,
       cid.updt_cnt = 0, cid.updt_dt_tm = cnvtdatetime(curdate,curtime3), cid.updt_id = reqinfo->
       updt_id,
       cid.updt_task = reqinfo->updt_task
      WITH nocounter
     ;end insert
    ...
    SET reply->success[x].concept_ident_bb_dta_id = dloincid
