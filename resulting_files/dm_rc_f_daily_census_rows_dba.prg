CREATE PROGRAM dm_rc_f_daily_census_rows:dba
 IF ( NOT (validate(script_version)))
  DECLARE script_version = vc WITH constant("601973.003"), private
 ENDIF
 ...
 DECLARE min_days_to_keep = i4 WITH protect, constant(370)
 ...
 IF (v_err_code2 != 0)
  ...
  GO TO exit_script
 ENDIF
 SET reply->table_name = "RC_F_DAILY_CENSUS"
 ...
 ELSE
  ...
  SELECT INTO "nl:"
   FROM rc_f_daily_census r
   WHERE r.activity_dt_nbr < activitydate
    AND ((r.rc_f_daily_census_id+ 0) != 0)
