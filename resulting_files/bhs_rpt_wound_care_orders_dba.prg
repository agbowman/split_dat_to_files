CREATE PROGRAM bhs_rpt_wound_care_orders:dba
 ...
 SET frec->file_name = concat("bhs_wound_care_orders_",trim(format(sysdate,"MMDDYYYY;;q")),".csv")
 SET frec->file_buf = "w"
 SELECT INTO "nl:"
  FROM order_catalog oc
  WHERE oc.primary_mnemonic IN ("Wound Care MD/AP Consult", "Wound Care RN Consult")
  HEAD REPORT
   ord_cat->l_cnt = 0
  DETAIL
 ...
 SET ms_rec = "jonathan.recchi@baystatehealth.org"
 CALL emailfile(frec->file_name,frec->file_name,ms_rec,"Consult Wound Care Orders Report",1)
#exit_script
END GO
