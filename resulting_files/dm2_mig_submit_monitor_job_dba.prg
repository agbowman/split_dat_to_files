CREATE PROGRAM dm2_mig_submit_monitor_job:dba
 ...
 SUBROUTINE dm2_rename_login_default(sbr_rld_mode)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmsmj_report_all = i2 WITH protect, noconstant(0)
 DECLARE dmsmj_report_delivery = i2 WITH protect, noconstant(0)
 DECLARE dmsmj_report_capture = i2 WITH protect, noconstant(0)
 DECLARE dmsmj_full_source_connect_string = vc WITH protect, noconstant("")
 DECLARE dmsmj_job_mode = vc WITH protect, noconstant("")
 DECLARE dmsmj_file_loc = vc WITH protect, noconstant(logical("CCLUSERDIR"))
 DECLARE dmsmj_submit_time = i4 WITH protect, noconstant(60)
 DECLARE dmsmj_heartbeat_time = i4 WITH protect, noconstant(4)
 DECLARE dmsmj_row_exists = i2 WITH protect, noconstant(0)
 DECLARE dmsmj_continue = i2 WITH protect, noconstant(1)
 DECLARE dmsmj_found_ind = i2 WITH protect, noconstant(0)
 DECLARE dmsmj_iter = i4 WITH protect, noconstant(0)
 DECLARE dmsmj_queue_fnd_ret = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_submit_mon",".log","dm2_mig_submit_monitor_job LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_submit_monitor_job"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Determining if capture setup row already exists in dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_GG_CAP_SETUP"
   AND di.info_name="GG_CAPTURE_PROCESS_CREATED"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ( NOT (curqual))
  SET dm_err->eproc = "Inserting capture setup row into dm_info."
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2_MIG_GG_CAP_SETUP", di.info_name = "GG_CAPTURE_PROCESS_CREATED"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (dmr_get_gg_dir(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining what node you are connected to."
 IF ((dmr_mig_data->report_capture=1)
  AND (dmr_mig_data->report_delivery=1))
  SET dmsmj_report_all = 1
  SET dmsmj_job_mode = "ALL"
 ELSEIF ((dmr_mig_data->report_capture=1))
  SET dmsmj_report_capture = 1
  SET dmsmj_job_mode = "CAPTURE"
 ELSEIF ((dmr_mig_data->report_delivery=1))
  SET dmsmj_report_delivery = 1
  SET dmsmj_job_mode = "DELIVERY"
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Setup work is not complete on the node."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dmsmj_full_source_connect_string = concat("V500/",dmr_mig_data->src_v500_pwd,"@",dmr_mig_data->
  src_v500_cnct_str)
 SET width = 132
 SET message = window
 CALL clear(1,1)
 ...
 IF ((dmr_emails->cnt=0))
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining existing monitor log file location from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name=value(evaluate(dmsmj_report_delivery,1,"MONITOR_DELIVERY_LOCATION",
    "MONITOR_CAPTURE_LOCATION"))
  DETAIL
   dmsmj_file_loc = di.info_char
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dmsmj_row_exists = evaluate(curqual,0,0,1)
 WHILE (dmsmj_continue=1)
   CALL text(2,2,"Please provide a location where miscellaneous log files will be stored:")
   CALL accept(2,74,"P(38);C",dmsmj_file_loc)
   SET dmsmj_file_loc = curaccept
   IF ((dm2_sys_misc->cur_os != "AXP"))
    IF (substring(size(dmsmj_file_loc),1,dmsmj_file_loc) != "/")
     SET dmsmj_file_loc = concat(dmsmj_file_loc,"/")
    ENDIF
   ENDIF
   IF (dmr_check_directory(dmsmj_file_loc)=0)
    SET dm_err->err_ind = 0
    CALL text(23,2,"The location you have provided is not valid.  Please try again.")
   ELSE
    SET dmsmj_continue = 0
    CALL clear(23,2,129)
   ENDIF
 ENDWHILE
 IF (dmsmj_row_exists=1)
  SET dm_err->eproc = "Attempting to update existing file location row in dm_info."
  UPDATE  FROM dm_info di
   SET di.info_char = dmsmj_file_loc
   WHERE di.info_domain="DM2_MIG_DATA"
    AND di.info_name=value(evaluate(dmsmj_report_delivery,1,"MONITOR_DELIVERY_LOCATION",
     "MONITOR_CAPTURE_LOCATION"))
   WITH nocounter
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Inserting file location row into dm_info."
  INSERT  FROM dm_info di
   SET di.info_char = dmsmj_file_loc, di.info_domain = "DM2_MIG_DATA", di.info_name = value(evaluate(
      dmsmj_report_delivery,1,"MONITOR_DELIVERY_LOCATION","MONITOR_CAPTURE_LOCATION"))
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name=value(evaluate(dmsmj_report_delivery,1,"MONITOR_DELIVERY_TIME",
    "MONITOR_CAPTURE_TIME"))
  DETAIL
   dmsmj_submit_time = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 SET dmsmj_row_exists = evaluate(curqual,0,0,1)
 CALL text(4,2,"How often would you like the monitoring job to be submitted (1-180 minutes)?")
 CALL accept(4,78,"999",dmsmj_submit_time
  WHERE curaccept BETWEEN 1 AND 180)
 SET dmsmj_submit_time = curaccept
 IF (dmsmj_row_exists=1)
  SET dm_err->eproc = "Attempting to update existing submit time row in dm_info."
  UPDATE  FROM dm_info di
   SET di.info_number = dmsmj_submit_time
   WHERE di.info_domain="DM2_MIG_DATA"
    AND di.info_name=value(evaluate(dmsmj_report_delivery,1,"MONITOR_DELIVERY_TIME",
     "MONITOR_CAPTURE_TIME"))
   WITH nocounter
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Inserting submit time row into dm_info."
  INSERT  FROM dm_info di
   SET di.info_number = dmsmj_submit_time, di.info_domain = "DM2_MIG_DATA", di.info_name = value(
     evaluate(dmsmj_report_delivery,1,"MONITOR_DELIVERY_TIME","MONITOR_CAPTURE_TIME"))
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name=value(evaluate(dmsmj_report_delivery,1,"MONITOR_DELIVERY_HEARTBEAT",
    "MONITOR_CAPTURE_HEARTBEAT"))
  DETAIL
   dmsmj_heartbeat_time = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SET dmsmj_row_exists = evaluate(curqual,0,0,1)
 CALL text(6,2,
  "How often would you like to receive an email that the monitoring job is running without issues (1-8 hours)?"
  )
 CALL accept(6,110,"99",dmsmj_heartbeat_time
  WHERE curaccept BETWEEN 1 AND 8)
 SET dmsmj_heartbeat_time = curaccept
 IF (dmsmj_row_exists=1)
  SET dm_err->eproc = "Attempting to update existing heartbeat time row in dm_info."
  UPDATE  FROM dm_info di
   SET di.info_number = dmsmj_heartbeat_time, di.info_date = cnvtlookahead(build(dmsmj_heartbeat_time,
      ",H"))
   WHERE di.info_domain="DM2_MIG_DATA"
    AND di.info_name=value(evaluate(dmsmj_report_delivery,1,"MONITOR_DELIVERY_HEARTBEAT",
     "MONITOR_CAPTURE_HEARTBEAT"))
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Inserting heartbeat time row into dm_info."
  INSERT  FROM dm_info di
   SET di.info_number = dmsmj_heartbeat_time, di.info_date = cnvtlookahead(build(dmsmj_heartbeat_time,
      ",H")), di.info_domain = "DM2_MIG_DATA",
    di.info_name = value(evaluate(dmsmj_report_delivery,1,"MONITOR_DELIVERY_HEARTBEAT",
      "MONITOR_CAPTURE_HEARTBEAT"))
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
 ...
 SET message = nowindow
 CALL clear(1,1)
 IF (((dmsmj_report_all) OR (dmsmj_report_capture))
  AND (dm2_sys_misc->cur_os="AXP"))
  IF (dmr_check_batch_queue(dmr_batch_queue,dmsmj_queue_fnd_ret)=0)
   GO TO exit_script
  ELSEIF (dmsmj_queue_fnd_ret=0)
   SET dm_err->err_ind = 1
   SET dm_err->eproc = concat("Check if migration batch queue, ",dmr_batch_queue,
    ", is properly setup.")
   SET dm_err->emsg = "Migration batch queue does not exist."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((dm2_sys_misc->cur_os="AXP"))
  IF (dmr_get_queue_contents(dmr_batch_queue)=0)
   GO TO exit_script
  ENDIF
  FOR (dmsmj_iter = 1 TO dmr_queue->cnt)
    IF ((dmr_queue->qual[dmsmj_iter].jobname="DM2_MIG_MONITORING"))
     SET dmsmj_found_ind = 1
    ENDIF
  ENDFOR
 ELSE
  SET dm_err->disp_dcl_err_ind = 0
  ...
  FREE DEFINE rtl
  DEFINE rtl build(dm_err->errfile)
  SET dm_err->eproc = "Determining if there were any results obtainted from the ps command."
  SELECT INTO "nl:"
   FROM rtlt r
   DETAIL
 ...
 IF (dmsmj_found_ind)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Determining if Migration Monitoring Job is already running."
  SET dm_err->emsg = "The job is already running.  Please stop the job via dm2_mig_menu."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dm2_sys_misc->cur_os="AXP"))
  SET dm_err->eproc = "Writing contents to dm2_mig_monitoring.com."
  SELECT INTO value(build(dmsmj_file_loc,"dm2_mig_monitoring.com"))
   FROM (dummyt d  WITH seq = 1)
   DETAIL
    CALL print("$!"), row + 1,
    CALL print("$! Executing Migration Monitoring Script..."),
    row + 1,
    CALL print("!"), row + 1,
    ...
    CALL print(concat("define oraclesystem '",dmsmj_full_source_connect_string,"' go")), row + 1,
    CALL print(concat("dm2_mig_monitoring ","'",dmsmj_full_source_connect_string,"', ","'",
     dmsmj_job_mode,"', ","'",dmsmj_file_loc,"' go")),
    row + 1,
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  IF (dm2_push_dcl(concat("SET FILE/PROT =(s:RWED, o:RWED, g:RWED, w:RWED) ",dmsmj_file_loc,
    "dm2_mig_monitoring.com"))=0)
   GO TO exit_script
  ENDIF
 ELSE
  SET dm_err->eproc = "Writing contents to dm2_mig_monitoring.ksh."
  SELECT INTO value(build(dmsmj_file_loc,"dm2_mig_monitoring.ksh"))
   FROM (dummyt d  WITH seq = 1)
   DETAIL
    ...
    row + 1,
    CALL print("#!"), row + 1,
    CALL print(build(". $cer_mgr/",logical("environment"),"_environment.ksh")), row + 1,
    CALL print("ccl <<!"),
    row + 1,
    CALL print("free define oraclesystem go"), row + 1,
    CALL print(concat("define oraclesystem '",dmsmj_full_source_connect_string,"' go")), row + 1,
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  IF (dm2_push_dcl(concat("chmod 777 ",dmsmj_file_loc,"dm2_mig_monitoring.ksh"))=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((dm2_sys_misc->cur_os="AXP"))
  IF (dm2_push_dcl(concat("SUBMIT /QUEUE=",dmr_batch_queue," /NAME=dm2_mig_monitoring ",
    dmsmj_file_loc,"dm2_mig_monitoring.com /NOLOG_FILE"))=0)
   GO TO exit_script
  ENDIF
 ELSE
  IF (dm2_push_dcl(concat("nohup ",dmsmj_file_loc,"dm2_mig_monitoring.ksh > /dev/null 2>&1 &"))=0)
   GO TO exit_script
  ENDIF
 ENDIF
#exit_script
 ...
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "The status of this job can be monitored via dm2_mig_menu."
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_submit_monitor_job completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_submit_mon")
END GO
