CREATE PROGRAM bbd_rpt_contactmethods:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 SELECT INTO "cer_temp:bbdcontactmethods.txt"
  c.*
  FROM code_value c
  PLAN (c
   WHERE c.code_set=14222
    AND c.code_value > 0)
  ORDER BY c.collation_seq
  HEAD PAGE
   col 1, "Cerner Health Systems",
   CALL center("C O N T A C T   M E T H O D S   R E P O R T",1,125),
   col 107, "Time:", col 121,
   curtime"hh:mm;;m", row + 1, col 107,
   "As of Date:", col 119, curdate"ddmmmyy;;d",
   row + 3, col 13, "Contact Method Display",
   col 46, "Contact Method Description", col 79,
  ...
  FOOT PAGE
   row 57, col 1, line,
   row + 1, col 1, "Report ID: BBD_RPT_CONTACTMETHODS",
   col 58, "Page:", col 64,
 ...
 IF (report_complete_ind="Y")
  ...
  SET reply->status_data.subeventstatus[count1].targetobjectname = "bbd_rpt_contactmethods"
