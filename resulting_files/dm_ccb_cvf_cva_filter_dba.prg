CREATE PROGRAM dm_ccb_cvf_cva_filter:dba
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (cva
   WHERE (cva.code_set=request->code_set)
    AND parser(filterstring_builder(dccd_filterstring,"cva",request)))
   JOIN (cv
   WHERE cv.code_value=cva.code_value)
  ORDER BY cva.code_set
  HEAD REPORT
   cv_cnt = 0
  DETAIL
   cv_cnt += 1
   ...
   ENDIF
   reply->cd_value_list[cv_cnt].code_value = cva.code_value, reply->cd_value_list[cv_cnt].display =
 ...
 IF (errcode != 0)
  SET errmsg = concat("Unable to read from code_value_alias while searching on alias:",errmsg)
  SET status = "F"
  GO TO exit_program
 ENDIF
#exit_program
