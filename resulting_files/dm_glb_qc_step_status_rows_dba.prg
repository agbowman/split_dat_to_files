CREATE PROGRAM dm_glb_qc_step_status_rows:dba
 ...
 SET reply->table_name = "QC_STEP_STATUS"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(qss.qc_step_status_id)
   FROM qc_step_status qss
   WHERE qss.qc_step_status_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(qss.qc_step_status_id)
  FROM qc_step_status qss
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    qss.rowid
    FROM qc_step_status qss
    WHERE parser(sbr_getrowidnotexists("qss.qc_step_status_id between curMinID and curMaxID","qss"))
     AND qss.step_complete_dt_tm <= cnvtdatetime((curdate - ldaystokeep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = qss.rowid
    WITH nocounter, maxqual(qss,value(rowsleft))
   ;end select
