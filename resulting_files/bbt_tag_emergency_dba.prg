CREATE PROGRAM bbt_tag_emergency:dba
 ...
 SET captions->emergency_transfusion = uar_i18ngetmessage(i18nhandle,"emergency_transfusion",
  "EMERGENCY TRANSFUSION TAG")
 ...
 SET captions->serial_number = uar_i18ngetmessage(i18nhandle,"serial_number","SERIAL NO:")
 ...
 SET captions->delay_in_transfusion = uar_i18ngetmessage(i18nhandle,"delay_in_transfusion",
  "BECAUSE DELAY IN TRANSFUSION WILL JEOPARDIZE THE")
 ...
 SET captions->by_time = uar_i18ngetmessage(i18nhandle,"by_time","BY:___________    &TIME:________")
 ...
 SET rpt_date = 0
 EXECUTE cpm_create_file_name_logical "bbt_tag_emer", "txt", "x"
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  DETAIL
   ...
   dispense_prvdr_name = trim(tag_request->taglist[d.seq].dispense_prvdr_name), rpt_row = 4, rpt_row
    += 2,
   row rpt_row, col 53, captions->emergency_transfusion,
   rpt_row += 2, row rpt_row, col 006,
   captions->patient, col 020, name_full_formatted,
   col 092, captions->unit_num, col 101,
   product_nbr_full
   IF (serial_nbr != null)
    rpt_row += 1, row rpt_row, col 092,
    captions->serial_number, col 103, serial_nbr
   ENDIF
   rpt_row += 1, row rpt_row, col 006,
   captions->med_rec, col 020, alias_mrn_formatted,
   col 055, captions->age, col 060,
   age, col 074, captions->sex,
   col 79, sex_disp, product_disp_30 = substring(1,30,product_disp),
   col 092, captions->product, col 101,
   product_disp_30, rpt_row += 1, person_aborh_disp = concat(trim(person_abo_disp)," ",trim(
     person_rh_disp)),
   person_aborh_disp_23 = substring(1,23,person_aborh_disp), product_aborh_disp = concat(trim(
     cur_abo_disp)," ",trim(cur_rh_disp)), product_aborh_disp_23 = substring(1,23,product_aborh_disp),
   row rpt_row, col 006, captions->financial_num,
   col 020, alias_fin_formatted, col 055,
   captions->location, col 060, patient_location,
   col 092, captions->donor_type, col 105,
   product_aborh_disp_23, rpt_row += 1, row rpt_row,
   col 006, captions->patient_type, col 020,
   person_aborh_disp_23, col 055, captions->blood_bank_id,
   col 070, bb_id_nbr, col 092,
   captions->unit_exp
   ...
   col 092, qty_vol_disp, rpt_row += 10,
   row rpt_row, col 006, captions->delay_in_transfusion,
   col 055, captions->authorize_admin, rpt_row += 2,
   row rpt_row, col 006, captions->uncrossmatched_blood,
   col 043, captions->attending_physician, rpt_row = 28,
   row rpt_row, col 004, captions->before_transfusion,
   col 055, captions->identified_recipient, col 096,
   captions->clerical_check, rpt_row += 1, row rpt_row,
   col 004, captions->of_the_wristband, col 054,
   captions->number_on_form, col 096, captions->released_date,
   rpt_row += 1, row rpt_row, col 004,
   captions->further_certify, col 059, captions->unit_number,
   col 096, captions->by_time, rpt_row += 1,
   row rpt_row, col 004, captions->stated_on_form,
   rpt_row += 1, row rpt_row, col 044,
   signature27, col 073, signature12,
   col 096, captions->transported_by, col 116,
   signature14, rpt_row += 1, row rpt_row,
   col 048, captions->transfusionist, col 073,
   captions->time_started, col 096, captions->received_date,
   rpt_row += 1, row rpt_row, col 096,
   captions->by_time, rpt_row += 1, row rpt_row,
   col 045, signature27, col 075,
   signature12, rpt_row += 1, row rpt_row,
   col 050, captions->transfusionist, col 075,
   captions->time_ended, rpt_row += 2, row rpt_row,
   col 006, captions->return_to_bb, col 054,
   captions->in_30_min, rpt_row += 1, row rpt_row,
   col 006, captions->refrigerator, rpt_row = 52,
   row rpt_row, col 094, captions->hospital_name,
   rpt_row += 1, row rpt_row, col 094,
   captions->hospital_address, rpt_row += 1, row rpt_row,
   col 094, captions->hospital_address, rpt_row += 1,
   row rpt_row, col 094, captions->clia_num,
   rpt_row += 1, row rpt_row, col 058,
   captions->original_to_chart, rpt_row += 1, row rpt_row,
   col 061, captions->copy_to_lab
  FOOT  d.seq
   IF (d.seq > 0
