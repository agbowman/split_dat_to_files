CREATE PROGRAM dm_si_error_reltn_rows:dba
 ...
 SET reply->table_name = "SI_ERROR_RELTN"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(s.si_error_reltn_id)
   FROM si_error_reltn s
   WHERE s.si_error_reltn_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(s.si_error_reltn_id)
  FROM si_error_reltn s
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    s.rowid
    FROM si_error_reltn s
    WHERE parser(sbr_getrowidnotexists("s.si_error_reltn_id between v_cur_min_id and v_cur_max_id",
      "s"))
