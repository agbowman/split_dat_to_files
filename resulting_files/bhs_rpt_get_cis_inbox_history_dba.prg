CREATE PROGRAM bhs_rpt_get_cis_inbox_history:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  HEAD b.session_id
   ...
   ENDIF
   data->batches[ml_batch_cnt].f_session_id = b.session_id, data->batches[ml_batch_cnt].s_log_time =
   format(bd.updt_dt_tm,"mm/dd/yyyy hh:mm:ss"), data->batches[ml_batch_cnt].s_params = b.parameters
  DETAIL
   ml_num = 0, ml_idx1 = locateval(ml_num,1,size(data->batches,5),b.session_id,data->batches[ml_num].
    f_session_id), ml_num = 0,
   ml_idx2 = locateval(ml_num,1,size(data->batches[ml_idx1].msgs,5),bd.detail_group,data->batches[
    ml_idx1].msgs[ml_num].l_detail_group)
   IF (ml_idx2=0)
   ...
   CASE (bd.parent_entity_name)
    OF "OriginalMessageID":
     data->batches[ml_idx1].msgs[ml_idx2].s_reference_id = bd.description,data->batches[ml_idx1].
     msgs[ml_idx2].s_msg_dt_tm = bd.msg
    OF "EventID":
    ...
    OF "CisTaskID":
     data->batches[ml_idx1].msgs[ml_idx2].s_cis_task_id = trim(cnvtstring(bd.parent_entity_id),3)
   ENDCASE
  FOOT REPORT
 ...
 SELECT INTO value( $OUTDEV)
  executiontime = substring(0,40,data->batches[d1.seq].s_log_time), msgcnt = cnvtstring(size(data->
    batches[d1.seq].msgs,5),5), parameters = substring(0,56,data->batches[d1.seq].s_params),
  ...
  HEAD REPORT
   ...
   curprog,
   CALL center("*** CIS to Patient Portal Message History ***",0,131), col 104,
  ...
  HEAD d1.seq
   ...
   row + 1, line_s, row + 1,
   col 0,
   CALL print(concat("BATCH: ",trim(cnvtstring(d1.seq),3))), col 11,
