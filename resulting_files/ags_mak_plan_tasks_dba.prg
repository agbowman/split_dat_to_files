CREATE PROGRAM ags_mak_plan_tasks:dba
 SET ags_mak_plan_tasks_mod = "000 10/06/06"
 CALL echo("***")
 CALL echo("***   BEG AGS_MAK_PLAN_TASKS")
 CALL echo("***")
 CALL echo(concat("MOD:",ags_mak_plan_tasks_mod))
 FREE RECORD item
 RECORD item(
   1 qual_knt = i4
   1 qual[*]
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_MAK_PLAN_TASKS"
 CALL echo("***")
 CALL echo("***   Find PLAN Batch Id Values")
 CALL echo("***")
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 SELECT INTO "nl:"
  min_id = min(r.ags_plan_data_id), max_id = max(r.ags_plan_data_id)
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("GET PLAN BATCH_ID VALUES :: Select Error :: ",trim(
    serrmsg))
  SET serrmsg = log->qual[log->qual_knt].smsg
  GO TO exit_script
 ENDIF
 CALL echo("***")
 CALL echo(build("***   AGS_PLAN_DATA min_batch_id :",min_batch_id))
 CALL echo(build("***   AGS_PLAN_DATA max_batch_id :",max_batch_id))
 CALL echo("***")
 CALL echo("***")
 CALL echo("***   Create PLAN Tasks")
 CALL echo("***")
 WHILE (min_batch_id <= max_batch_id
  AND min_batch_id >= 1)
   ...
   SET item->qual[item->qual_knt].task_type = "PLAN"
   ...
   SET item->qual[item->qual_knt].batch_program = "ags_plan_load"
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   SELECT INTO "nl:"
 ...
 IF ((item->qual_knt < 1))
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "No Plan tasks found"
  GO TO exit_script
 ENDIF
 CALL echo("***")
 ...
 COMMIT
 CALL echo("***")
 CALL echo("***   Update PLAN Data to WAITING")
 CALL echo("***")
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE AGS_PLAN_DATA ITEMS :: Update Error :: ",trim(
    serrmsg))
  SET serrmsg = log->qual[log->qual_knt].smsg
  GO TO exit_script
 ENDIF
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_MAK_PLAN_TASKS"
 SET script_ver = "000 10/06/06"
 CALL echo("***")
 CALL echo("***   END AGS_MAK_PLAN_TASKS")
 CALL echo("***")
END GO
