CREATE PROGRAM cv_da_add_cv_ecgclassification:dba
 IF (validate(stat)=0)
 ...
 SET stat = alterlist(id_list->item,size(request->ecgclassification,5))
 EXECUTE dm2_dar_get_bulk_seq "id_list->item", size(request->ecgclassification,5), "id",
 1, "CARD_VAS_SEQ"
 ...
 SET did = 0.0
 IF (validate(request->ecgclassification[nindex].normalcy_classification_id,- (0.00001)) <= 0.0)
  SET did = id_list->item[nindex].id
  SET request->ecgclassification[nindex].normalcy_classification_id = did
 ELSE
  SET did = request->ecgclassification[nindex].normalcy_classification_id
 ENDIF
 INSERT  FROM cv_normalcy_classification c
  SET c.cv_normalcy_classification_id = did, c.beg_effective_dt_tm = cnvtdatetime(transinfo->
    trans_dt_tm), c.classification_statement =
   ...
   ELSE 0.0
   ENDIF
   , c.end_effective_dt_tm = cnvtdatetime("31-DEC-2100 23:59:59.00"), c.activity_subtype_cd =
   activitysubtype,
   c.updt_dt_tm = cnvtdatetime(transinfo->trans_dt_tm), c.updt_id = reqinfo->updt_id, c.updt_task =
   reqinfo->updt_task,
   c.updt_applctx = reqinfo->updt_applctx, c.updt_cnt = 0, c.active_ind = 1.0
  WITH nocounter
 ;end insert
 IF (curqual=0)
 ...
 END ;Subroutine
#end_program
 CALL cv_log_msg_post("001 01/09/14 JT023123")
 CALL cv_log_msg_post("002 03/12/14 HK028252")
END GO
