CREATE PROGRAM bhs_ext_surg_consult_ord:dba
 ...
 DECLARE mf_cs200_consultsurgicalservicesappt_cd = f8 WITH protect, constant(uar_get_code_by(
   "DISPLAYKEY",200,"CONSULTSURGICALSERVICESAPPT"))
 DECLARE mf_cs200_consultoutsideimagecvsurgery_cd = f8 WITH protect, constant(uar_get_code_by(
   "DISPLAYKEY",200,"CONSULTOUTSIDEIMAGECVSURGERY"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   WHERE o.orig_order_dt_tm BETWEEN cnvtdatetime("01-JAN-2023 00:00:00") AND cnvtdatetime(
    "26-JAN-2024 23:59:59")
    AND o.active_ind=1
    AND o.catalog_cd IN (mf_cs200_consultsurgicalservicesappt_cd,
   mf_cs200_consultoutsideimagecvsurgery_cd, mf_cs200_consultsurgerybmc_cd,
   mf_cs200_consultsurgery_cd)
  ...
  HEAD o.order_id
   ...
   m_rec->qual[m_rec->l_cnt].oqual[m_rec->qual[m_rec->l_cnt].l_ocnt].s_order_dt = trim(format(o
     .orig_order_dt_tm,"MM/DD/YYYY HH:mm;;q"),3)
  WITH nocounter
 ;end select
 SET frec->file_name = "surgical_consult_orders_20240126.csv"
 SET frec->file_buf = "w"
 ...
 FOR (ml_idx1 = 1 TO m_rec->l_cnt)
   FOR (ml_idx2 = 1 TO m_rec->qual[ml_idx1].l_ocnt)
    SET frec->file_buf = build('"',m_rec->qual[ml_idx1].oqual[ml_idx2].s_provider,'","',m_rec->qual[
     ...
     s_order_status,
     '","',m_rec->qual[ml_idx1].oqual[ml_idx2].s_order_dt,'"',char(13),char(10))
    SET stat = cclio("WRITE",frec)
   ENDFOR
 ENDFOR
