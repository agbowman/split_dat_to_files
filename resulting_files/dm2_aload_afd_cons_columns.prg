CREATE PROGRAM dm2_aload_afd_cons_columns
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_indcol",".log","dm2_aload_afd_cons_columns")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_afd_cons_columns"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daacc_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daacc_file = vc WITH protect, noconstant( $2)
 DECLARE daacc_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daacc_batch = i4 WITH protect, noconstant(0)
 DECLARE daacc_rows_returned = i4 WITH protect, noconstant(1)
 DECLARE daacc_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daacc_pkg,daacc_file_loc_ret,daacc_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daacc_batch=0)
  SET daacc_batch = 5000
 ENDIF
 IF (daacc_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daacc_file_loc_ret,daacc_file), "dm2_aload_sch_conscol", daacc_batch
  CALL echo(concat(daacc_file_loc_ret,daacc_file))
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Remove extraneous DM_AFD_CONS_COLUMNS rows"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dm_afd_cons_columns a
   WHERE a.alpha_feature_nbr=daacc_pkg
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_afd_cons_columns completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_afd_cons_columns")
END GO
