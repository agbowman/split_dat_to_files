CREATE PROGRAM dm_rmc_dcl_all:dba
 ...
 SUBROUTINE drdc_file_success(dfs_name,dfs_file_name,dfs_error_ind)
   ...
   RETURN(null)
 END ;Subroutine
 IF ((validate(dclma_menu_ind,- (1))=- (1)))
  DECLARE dclma_menu_ind = i2 WITH protect, noconstant(1)
 ENDIF
 DECLARE dcla_file_name = vc WITH protect, noconstant("")
 DECLARE dcla_refresh_screen(rs_rec=vc(ref)) = null
 DECLARE dcla_print_report(dpr_name=vc,dpr_rec=vc(ref)) = i2
 SET dm_err->eproc = "Starting dm_rmc_dcl_all"
 IF (check_logfile("dm_rmc_dcl_all",".log","dm_rmc_dcl_all LOG")=0)
  SET dm_err->err_ind = 1
 ...
 CALL clear(1,1)
 SET width = 132
 CALL text(1,54,"RDDS Change Log Report")
 CALL text(2,1,"Env: ")
 CALL text(2,30,"Source:")
 CALL text(3,1,"REFCHG: ")
 CALL text(4,1,"NORDDS: ")
 CALL text(3,19,"MERGED: ")
 CALL text(4,19,"OLDVER: ")
 CALL text(3,37,"PROCES: ")
 CALL text(4,37,"ORPHAN: ")
 CALL text(3,55,"Investigate NOMV*: ")
 CALL text(4,55,"Non-investigate NOMV*: ")
 CALL text(3,88,"CHLD*: ")
 CALL text(4,88,"OTHER: ")
 CALL text(5,1,fillstring(132,"-"))
 CALL video(b)
 CALL text(10,56,"Generating Report (may take a few minutes)...")
 SET dclm_all->max_lines = 15
 EXECUTE dm_rmc_dcl_all_child
 IF ((dm_err->err_ind=1))
  GO TO exit_all
 ENDIF
 WHILE (true)
   CALL video(n)
   SET accept = scroll
   CALL dcla_refresh_screen(dclm_all)
   CALL accept(24,18,"X;CUS","E"
   ...
   CASE (curscroll)
    OF 0:
     IF (curaccept="E")
      GO TO exit_all
     ELSEIF (curaccept="S")
      SET dcla_file_name = drdc_get_name("RDDS Change Log Monitor","FULL_REPORT.CSV")
      IF (dcla_file_name != "-1")
       IF (dcla_print_report(dcla_file_name,dclm_all)=1)
        CALL drdc_file_success("RDDS Change Log Monitor",dcla_file_name,1)
       ELSE
        CALL drdc_file_success("RDDS Change Log Monitor",dcla_file_name,0)
       ENDIF
      ENDIF
     ENDIF
    OF 1:
    OF 6:
     IF (((dclm_all->cur_rs_pos+ dclm_all->max_lines) <= dclm_all->cnt))
      SET dclm_all->cur_rs_pos = (dclm_all->cur_rs_pos+ dclm_all->max_lines)
     ENDIF
    OF 2:
    OF 5:
     SET dclm_all->cur_rs_pos = greatest((dclm_all->cur_rs_pos - dclm_all->max_lines),0)
   ENDCASE
 ENDWHILE
 SUBROUTINE dcla_refresh_screen(rs_rec)
   ...
   CALL text(3,1,"REFCHG: ")
   SET rs_pos = locateval(rs_loop,1,rs_rec->type_cnt,"REFCHG",rs_rec->lt_qual[rs_loop].log_type)
   IF (rs_pos=0)
    SET rs_cnt = 0
   ELSE
    SET rs_cnt = rs_rec->lt_qual[rs_pos].lt_cnt
   ENDIF
   CALL text(3,(8+ (9 - size(trim(cnvtstring(rs_cnt))))),trim(cnvtstring(rs_cnt)))
   CALL text(4,1,"NORDDS: ")
   SET rs_pos = locateval(rs_loop,1,rs_rec->type_cnt,"NORDDS",rs_rec->lt_qual[rs_loop].log_type)
   IF (rs_pos=0)
    SET rs_cnt = 0
   ELSE
    SET rs_cnt = rs_rec->lt_qual[rs_pos].lt_cnt
   ENDIF
   CALL text(4,(8+ (9 - size(trim(cnvtstring(rs_cnt))))),trim(cnvtstring(rs_cnt)))
   CALL text(3,19,"MERGED: ")
   SET rs_pos = locateval(rs_loop,1,rs_rec->type_cnt,"MERGED",rs_rec->lt_qual[rs_loop].log_type)
   IF (rs_pos=0)
    SET rs_cnt = 0
   ELSE
    SET rs_cnt = rs_rec->lt_qual[rs_pos].lt_cnt
   ENDIF
   CALL text(3,(26+ (9 - size(trim(cnvtstring(rs_cnt))))),trim(cnvtstring(rs_cnt)))
   CALL text(4,19,"OLDVER: ")
   SET rs_pos = locateval(rs_loop,1,rs_rec->type_cnt,"OLDVER",rs_rec->lt_qual[rs_loop].log_type)
   IF (rs_pos=0)
    SET rs_cnt = 0
   ELSE
    SET rs_cnt = rs_rec->lt_qual[rs_pos].lt_cnt
   ENDIF
   CALL text(4,(26+ (9 - size(trim(cnvtstring(rs_cnt))))),trim(cnvtstring(rs_cnt)))
   CALL text(3,37,"PROCES: ")
   SET rs_pos = locateval(rs_loop,1,rs_rec->type_cnt,"PROCES",rs_rec->lt_qual[rs_loop].log_type)
   IF (rs_pos=0)
    SET rs_cnt = 0
   ELSE
    SET rs_cnt = rs_rec->lt_qual[rs_pos].lt_cnt
   ENDIF
   CALL text(3,(44+ (9 - size(trim(cnvtstring(rs_cnt))))),trim(cnvtstring(rs_cnt)))
   CALL text(4,37,"ORPHAN: ")
   SET rs_pos = locateval(rs_loop,1,rs_rec->type_cnt,"ORPHAN",rs_rec->lt_qual[rs_loop].log_type)
   IF (rs_pos=0)
    SET rs_cnt = 0
   ELSE
    SET rs_cnt = rs_rec->lt_qual[rs_pos].lt_cnt
   ENDIF
   CALL text(4,(44+ (9 - size(trim(cnvtstring(rs_cnt))))),trim(cnvtstring(rs_cnt)))
   CALL text(3,55,"Investigate NOMV*: ")
   SET rs_pos = locateval(rs_loop,1,rs_rec->type_cnt,"INV NOMV*",rs_rec->lt_qual[rs_loop].log_type)
   IF (rs_pos=0)
    SET rs_cnt = 0
   ELSE
    SET rs_cnt = rs_rec->lt_qual[rs_pos].lt_cnt
   ENDIF
   CALL text(3,(77+ (9 - size(trim(cnvtstring(rs_cnt))))),trim(cnvtstring(rs_cnt)))
   CALL text(4,55,"Non-investigate NOMV*: ")
   SET rs_pos = locateval(rs_loop,1,rs_rec->type_cnt,"NOMV*",rs_rec->lt_qual[rs_loop].log_type)
   IF (rs_pos=0)
    SET rs_cnt = 0
   ELSE
    SET rs_cnt = rs_rec->lt_qual[rs_pos].lt_cnt
   ENDIF
   CALL text(4,(77+ (9 - size(trim(cnvtstring(rs_cnt))))),trim(cnvtstring(rs_cnt)))
   CALL text(3,88,"CHLD*: ")
   SET rs_pos = locateval(rs_loop,1,rs_rec->type_cnt,"CHLD*",rs_rec->lt_qual[rs_loop].log_type)
   IF (rs_pos=0)
    SET rs_cnt = 0
   ELSE
    SET rs_cnt = rs_rec->lt_qual[rs_pos].lt_cnt
   ENDIF
   CALL text(3,(95+ (9 - size(trim(cnvtstring(rs_cnt))))),trim(cnvtstring(rs_cnt)))
   CALL text(4,88,"OTHER: ")
   SET rs_pos = locateval(rs_loop,1,rs_rec->type_cnt,"OTHER",rs_rec->lt_qual[rs_loop].log_type)
   IF (rs_pos=0)
    SET rs_cnt = 0
   ELSE
    SET rs_cnt = rs_rec->lt_qual[rs_pos].lt_cnt
   ENDIF
   CALL text(4,(95+ (9 - size(trim(cnvtstring(rs_cnt))))),trim(cnvtstring(rs_cnt)))
   SET rs_start = 5
    ...
    FOR (rs_stop_pos = 1 TO rs_rec->max_lines)
      IF (((rs_stop_pos+ rs_rec->cur_rs_pos) <= rs_rec->cnt))
       ...
       CALL text((rs_stop_pos+ rs_start),53,rs_rec->qual[(rs_stop_pos+ rs_rec->cur_rs_pos)].
        context_name)
       IF ((rs_rec->qual[(rs_stop_pos+ rs_rec->cur_rs_pos)].nomv_ind=1))
        CALL text((rs_stop_pos+ rs_start),80,"Investigation Required for LOG_TYPE")
       ENDIF
      ENDIF
    ENDFOR
   ...
   CALL text(24,1,"Command Options: _ (E)xit, (S)ave to CSV")
 END ;Subroutine
 SUBROUTINE dcla_print_report(dpr_name,dpr_rec)
   DECLARE dpr_temp_str = vc WITH protect, noconstant("")
   SELECT INTO value(dpr_name)
    FROM (dummyt d  WITH seq = dpr_rec->cnt)
    HEAD REPORT
     col 1, dpr_temp_str = concat("SOURCE: ",dpr_rec->src_name,",TARGET: ",dpr_rec->cur_name),
     dpr_temp_str,
     row + 1, dpr_temp_str = "TABLE_NAME,LOG_TYPE,COUNT,CONTEXT_NAME,INVESTIGATE", dpr_temp_str,
     row + 1
    DETAIL
     IF ((dpr_rec->qual[d.seq].nomv_ind=1))
      dpr_temp_str = concat('"',dpr_rec->qual[d.seq].table_name,'","',dpr_rec->qual[d.seq].log_type,
       '","',
       trim(cnvtstring(dpr_rec->qual[d.seq].row_cnt)),'","',dpr_rec->qual[d.seq].context_name,'","Y"'
       )
     ELSE
      dpr_temp_str = concat('"',dpr_rec->qual[d.seq].table_name,'","',dpr_rec->qual[d.seq].log_type,
       '","',
       trim(cnvtstring(dpr_rec->qual[d.seq].row_cnt)),'","',dpr_rec->qual[d.seq].context_name,'","N"'
       )
     ENDIF
     dpr_temp_str, row + 1
