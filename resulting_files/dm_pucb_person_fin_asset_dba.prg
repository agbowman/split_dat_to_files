CREATE PROGRAM dm_pucb_person_fin_asset:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PUCB_PERSON_FIN_ASSET"
 ...
 SUBROUTINE cust_ucb_del(dummy)
   UPDATE  FROM person_fin_asset
    SET active_ind = rchildren->qual1[det_cnt].prev_active_ind, active_status_cd = rchildren->qual1[
     ...
     parent_entity_id = request->xxx_uncombine[ucb_cnt].to_xxx_id, parent_entity_name = "PERSON"
    WHERE (person_fin_asset_id=rchildren->qual1[det_cnt].entity_id)
    WITH nocounter
   ;end update
   IF (curqual=0)
   ...
   IF (add_hist(rchildren->qual1[det_cnt].entity_id)=0)
    ...
    SET error_table = rchildren->qual1[det_cnt].entity_name
    SET request->error_message = substring(1,132,build(
      "Could not insert history for person_fin_asset with pk=",rchildren->qual1[det_cnt].entity_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 ...
 SUBROUTINE initialize_hist_id(null)
  IF (v_hist_init_ind=0)
   ...
   SET dcipht_request->transaction_reason_txt = "DM_PUCB_PERSON_FIN_ASSET"
