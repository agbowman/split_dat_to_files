CREATE PROGRAM cp_get_nodes
 ...
 SET nodedata->status_data.status = "F"
 CALL getnodes(null)
 SET nodedata->status_data.status = "S"
 SUBROUTINE getnodes(null)
   ...
   SELECT INTO "NL:"
    FROM cp_node cn
    PLAN (cn
     WHERE cn.cp_node_id > 0)
    ORDER BY cn.cp_node_id
    HEAD REPORT
     nodecnt = 0
    HEAD cn.cp_node_id
     nodecnt += 1
     IF (nodecnt > size(nodedata->node_list,5))
      stat = alterlist(nodedata->node_list,(nodecnt+ 10))
     ENDIF
     nodedata->node_list[nodecnt].concept = uar_get_code_display(cn.concept_cd), nodedata->node_list[
     nodecnt].concept_cd = cn.concept_cd, nodedata->node_list[nodecnt].node_type =
     uar_get_code_display(cn.node_type_cd),
     nodedata->node_list[nodecnt].node_type_cd = cn.node_type_cd, nodedata->node_list[nodecnt].
     node_name = cn.node_name, nodedata->node_list[nodecnt].node_id = cn.cp_node_id
    FOOT REPORT
     stat = alterlist(nodedata->node_list,nodecnt)
    WITH nocounter
   ;end select
...
#exit_script
 IF (validate(debug_ind,0)=1)
  CALL echorecord(nodedata)
 ENDIF
 CALL putjsonrecordtofile(nodedata)
