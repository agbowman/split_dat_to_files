CREATE PROGRAM advsr_meds_rec_add_ord_recon:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Order Reconciliation Request JSON" = "",
  'Admission Reconciliation Status ( "COMPLETE" or  "PARTIAL")' = "",
  'Discharge Reconciliation Status ( "COMPLETE" or  "PARTIAL")' = "",
  'Transfer  Reconciliation Status ( "COMPLETE" or  "PARTIAL")' = ""
  WITH outdev, reqjson, adm_recon_status,
  disch_recon_status, transf_recon_status
 RECORD reconreply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 DECLARE jrec = i4
 DECLARE jsonstr = vc WITH protect, noconstant(" ")
 IF (validate(request->blob_in))
  IF ((request->blob_in > " "))
   SET jsonstr = request->blob_in
  ELSE
   SET jsonstr = trim( $REQJSON)
  ENDIF
 ELSE
  SET jsonstr = trim( $REQJSON)
 ENDIF
 ...
 CALL echo(jsonstr)
 IF (validate(ord_recon_list)=0)
  RECORD ord_recon_list(
    1 encntr_id = f8
    1 performed_prsnl_id = f8
    1 ord_recons[*]
      2 recon_type_flag = i2
      2 no_known_meds_ind = i2
      2 to_action_seq = i4
      ...
      2 order_list[*]
        ...
        3 continue_order_ind = i4
        3 recon_order_action_mean = vc
        3 order_mnemonic = vc
  )
 ENDIF
 IF (jsonstr > " ")
  SET jrec = cnvtjsontorec(jsonstr)
 ENDIF
 ...
 FREE SET admit_recon
 RECORD admit_recon(
   1 encntr_id = f8
   1 performed_dt_tm = dq8
   1 performed_tz = i4
   1 performed_prsnl_id = f8
   1 recon_type_flag = i2
   1 no_known_meds_ind = i2
   1 order_list[*]
     2 order_id = f8
     2 clinical_display_line = vc
     2 continue_order_ind = i4
     2 recon_order_action_mean = vc
     2 order_mnemonic = vc
     2 simplified_display_line = vc
     2 order_proposal_id = f8
     2 recon_note_txt = vc
   1 recon_status_cd = f8
   1 next_loc_cd = f8
   1 person_id = f8
   1 cross_encntr_ind = i2
 )
 FREE SET transfer_recon
 RECORD transfer_recon(
   1 encntr_id = f8
   1 performed_dt_tm = dq8
   1 performed_tz = i4
   1 performed_prsnl_id = f8
 ...
 FREE SET discharge_recon
 RECORD discharge_recon(
   1 encntr_id = f8
   1 performed_dt_tm = dq8
   1 performed_tz = i4
   1 performed_prsnl_id = f8
 ...
 SUBROUTINE (getviewableencounters(personid=f8,prsnlid=f8,encntr_rec=vc(ref)) =null WITH protect)
   ...
   CALL log_message(build("Exit GetViewableEncounters(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 DECLARE logblobin(null) = null
 DECLARE getpersonid(null) = null
 DECLARE addorderrecon(null) = vc
 DECLARE addorderreltn(null) = vc
 DECLARE sscript_name = vc WITH protect, constant("advsr_meds_rec_add_ord_recon")
 DECLARE cur_dttm = dq8 WITH protect, constant(cnvtdatetime(sysdate))
 DECLARE cur_tz = i4 WITH protect, constant(curtimezonesys)
 DECLARE admin_recon = i2 WITH protect, constant(1)
 DECLARE disch_recon = i2 WITH protect, constant(3)
 DECLARE admit_recon_status_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4002695,trim(
     $ADM_RECON_STATUS)))
 DECLARE disch_recon_status_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4002695,trim(
     $DISCH_RECON_STATUS)))
 DECLARE transf_recon_status_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4002695,trim(
     $TRANSF_RECON_STATUS)))
 DECLARE script_version = vc WITH protect, noconstant(" ")
 DECLARE lstat = i4 WITH protect, noconstant(0)
 DECLARE temp_cntr = i4 WITH protect, noconstant(0)
 DECLARE temp_cnt = i4 WITH protect, noconstant(0)
 DECLARE resp_json_str = vc WITH protect
 DECLARE json_str = vc WITH protect
 DECLARE admit_recon_cnt = i4 WITH protect, noconstant(0)
 DECLARE transfer_recon_cnt = i4 WITH protect, noconstant(0)
 DECLARE discharge_recon_cnt = i4 WITH protect, noconstant(0)
 DECLARE person_id = f8 WITH protect, noconstant(0)
 IF (validate(ord_recon_list) != 0)
  CALL echorecord(ord_recon_list)
  CALL logblobin(null)
  CALL getpersonid(null)
  CALL addorderreltn(null)
  CALL addorderrecon(null)
 ENDIF
 GO TO set_return
 SUBROUTINE logblobin(null)
   SET audit_init_request->long_text = jsonstr
   SET audit_init_request->object_name = "ADVSR_MEDS_REC_ADD_ORD_RECON_BLOB_IN"
   SET audit_init_request->output_device = "MINE"
   SET audit_init_request->person_id = person_id
   SET audit_init_request->report_type = "REPORT"
   EXECUTE ccl_add_rpt_audit:dba  WITH replace("REQUEST","AUDIT_INIT_REQUEST"), replace("REPLY",
    "AUDIT_INIT_REPLY")
   IF ((audit_init_reply->report_audit_id > 0))
    UPDATE  FROM ccl_report_audit cra
     SET cra.end_dt_tm = cnvtdatetime(sysdate), cra.active_ind = 0, cra.status = "SUCCESS"
     WHERE (cra.report_event_id=audit_init_reply->report_audit_id)
    ;end update
    CALL errorhandler("logBlobIn()-q1","F","advsr_meds_rec_add_ord_recon ",reconreply)
   ENDIF
 END ;Subroutine
 SUBROUTINE getpersonid(null)
  SELECT INTO "nl:"
   FROM encounter e
   PLAN (e
    WHERE (e.encntr_id=ord_recon_list->encntr_id))
   HEAD e.encntr_id
    person_id = e.person_id
   WITH nocounter
  ;end select
  CALL echo(build("PERSON ID  : ",person_id))
 END ;Subroutine
 SUBROUTINE addorderreltn(null)
   DECLARE msg_text_id = f8 WITH protect, noconstant(0)
   DECLARE convert_reltn_type_cki = vc WITH protect, constant("CKI.CODEVALUE!12217247")
   DECLARE cancel_reltn_type_cki = vc WITH protect, constant("CKI.CODEVALUE!4100002641")
   DECLARE repeat_reltn_type_cki = vc WITH protect, constant("CKI.CODEVALUE!4100002642")
   DECLARE renew_reltn_type_cki = vc WITH protect, constant("CKI.CODEVALUE!4100273841")
   DECLARE order_reltn_code_set = i4 WITH protect, constant(4001987)
   DECLARE convert_reltn_type_cd = f8 WITH protect, noconstant(0)
   DECLARE cancel_reltn_type_cd = f8 WITH protect, noconstant(0)
   DECLARE repeat_reltn_type_cd = f8 WITH protect, noconstant(0)
   DECLARE renew_reltn_type_cd = f8 WITH protect, noconstant(0)
   DECLARE cur_reltn_type_cd = f8 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM code_value cv
    PLAN (cv
     WHERE cv.cki IN (convert_reltn_type_cki, cancel_reltn_type_cki, repeat_reltn_type_cki,
     renew_reltn_type_cki)
      AND ((cv.code_set+ 0)=order_reltn_code_set))
    DETAIL
     CASE (cv.cki)
      OF convert_reltn_type_cki:
       convert_reltn_type_cd = cv.code_value
      OF cancel_reltn_type_cki:
       cancel_reltn_type_cd = cv.code_value
      OF repeat_reltn_type_cki:
       repeat_reltn_type_cd = cv.code_value
      OF renew_reltn_type_cki:
       renew_reltn_type_cd = cv.code_value
     ENDCASE
    WITH nocounter
   ;end select
   CALL errorhandler("AddOrderReltn()-q1","F","advsr_meds_rec_add_ord_recon ",reconreply)
   SET temp_cnt = value(size(ord_recon_list->ord_recons,5))
   FOR (temp_cntr = 1 TO temp_cnt)
     IF ((ord_recon_list->ord_recons[temp_cntr].reltn_type_mean IN ("CONVERT_RX", "CONVERT_INPAT",
     "CANCEL REORD"))
      AND size(ord_recon_list->ord_recons[temp_cntr].order_list,5)=2)
      SELECT INTO "nl:"
       ...
       WITH format
      ;end select
      CALL errorhandler("AddOrderReltn() -> nextseqnum ","F","advsr_meds_rec_add_ord_recon",
       reconreply)
      IF (msg_text_id < 0)
       RETURN("F")
      ENDIF
      SET cur_reltn_type_cd = 0.00
      CASE (ord_recon_list->ord_recons[temp_cntr].reltn_type_mean)
       OF "CONVERT_RX":
        SET cur_reltn_type_cd = convert_reltn_type_cd
       OF "CONVERT_INPAT":
        SET cur_reltn_type_cd = convert_reltn_type_cd
       OF "CANCEL REORD":
        SET cur_reltn_type_cd = cancel_reltn_type_cd
      ENDCASE
      INSERT  FROM order_order_reltn oor
       SET oor.entered_by_prsnl_id = cnvtreal(ord_recon_list->performed_prsnl_id), oor
        .order_order_reltn_id = msg_text_id, oor.related_from_action_seq = ord_recon_list->
        ord_recons[temp_cntr].to_action_seq,
        oor.related_from_order_id = cnvtreal(ord_recon_list->ord_recons[temp_cntr].order_list[2].
         order_id), oor.related_to_order_id = cnvtreal(ord_recon_list->ord_recons[temp_cntr].
         order_list[1].order_id), oor.relation_created_dt_tm = cnvtdatetime(cur_dttm),
        oor.relation_created_tz = cur_tz, oor.relation_type_cd = cur_reltn_type_cd, oor.updt_dt_tm =
        cnvtdatetime(cur_dttm),
        oor.updt_id = reqinfo->updt_id, oor.updt_task = reqinfo->updt_task, oor.updt_cnt = 0,
        oor.updt_applctx = reqinfo->updt_applctx
       WITH nocounter
      ;end insert
      CALL errorhandler("AddOrderReltn()  -> INSERT into ORDER_ORDER_RELTN ","F",
       "advsr_meds_rec_add_ord_recon ",reconreply)
     ENDIF
   ENDFOR
   RETURN("S")
 END ;Subroutine
 SUBROUTINE addorderrecon(null)
   DECLARE r_cntr = i4 WITH protect, noconstant(0)
   DECLARE from_index = i4 WITH protect, noconstant(0)
   DECLARE to_size = i4 WITH protect, noconstant(0)
   DECLARE recon_status_cd_exists = i2 WITH protect, noconstant(0)
   SELECT
    cur_recon_type_flag = ord_recon_list->ord_recons[d1.seq].recon_type_flag, recon_seq = d1.seq
    FROM (dummyt d1  WITH seq = value(size(ord_recon_list->ord_recons,5)))
    PLAN (d1)
    ORDER BY cur_recon_type_flag
    HEAD REPORT
     row + 0
    HEAD cur_recon_type_flag
     row + 0
    DETAIL
     CASE (cur_recon_type_flag)
      OF 1:
       IF (admit_recon_cnt=0)
        admit_recon->encntr_id = cnvtreal(ord_recon_list->encntr_id), admit_recon->person_id =
        person_id, admit_recon->performed_dt_tm = cur_dttm,
        admit_recon->performed_tz = cur_tz, admit_recon->performed_prsnl_id = cnvtreal(ord_recon_list
         ->performed_prsnl_id), admit_recon->recon_type_flag = ord_recon_list->ord_recons[d1.seq].
        recon_type_flag,
        admit_recon->no_known_meds_ind = ord_recon_list->ord_recons[d1.seq].no_known_meds_ind
       ENDIF
       ,to_size = (admit_recon_cnt+ size(ord_recon_list->ord_recons[d1.seq].order_list,5)),
       IF (to_size > size(admit_recon->order_list,5))
        stat = alterlist(admit_recon->order_list,(to_size+ 9))
       ENDIF
       ,
       FOR (r_cntr = (admit_recon_cnt+ 1) TO to_size)
         from_index = (r_cntr - admit_recon_cnt), admit_recon->order_list[r_cntr].
         clinical_display_line = ord_recon_list->ord_recons[d1.seq].order_list[from_index].
         clinical_display_line, admit_recon->order_list[r_cntr].continue_order_ind = ord_recon_list->
         ord_recons[d1.seq].order_list[from_index].continue_order_ind,
         admit_recon->order_list[r_cntr].order_id = cnvtreal(ord_recon_list->ord_recons[d1.seq].
          order_list[from_index].order_id), admit_recon->order_list[r_cntr].order_mnemonic =
         ord_recon_list->ord_recons[d1.seq].order_list[from_index].order_mnemonic, admit_recon->
         order_list[r_cntr].recon_order_action_mean = ord_recon_list->ord_recons[d1.seq].order_list[
         from_index].recon_order_action_mean,
         admit_recon->order_list[r_cntr].simplified_display_line = ord_recon_list->ord_recons[d1.seq]
         .order_list[from_index].simplified_display_line
       ENDFOR
       ,admit_recon_cnt = to_size
      OF 2:
       IF (transfer_recon_cnt=0)
        transfer_recon->encntr_id = cnvtreal(ord_recon_list->encntr_id), transfer_recon->person_id =
        person_id, transfer_recon->performed_dt_tm = cur_dttm,
        transfer_recon->performed_tz = cur_tz, transfer_recon->performed_prsnl_id = cnvtreal(
         ord_recon_list->performed_prsnl_id), transfer_recon->recon_type_flag = ord_recon_list->
        ord_recons[d1.seq].recon_type_flag,
        transfer_recon->no_known_meds_ind = ord_recon_list->ord_recons[d1.seq].no_known_meds_ind
       ENDIF
       ,to_size = (transfer_recon_cnt+ size(ord_recon_list->ord_recons[d1.seq].order_list,5)),
       IF (to_size > size(transfer_recon->order_list,5))
        stat = alterlist(transfer_recon->order_list,(to_size+ 9))
       ENDIF
       ,
       FOR (r_cntr = (transfer_recon_cnt+ 1) TO to_size)
         from_index = (r_cntr - transfer_recon_cnt), transfer_recon->order_list[r_cntr].
         clinical_display_line = ord_recon_list->ord_recons[d1.seq].order_list[from_index].
         clinical_display_line, transfer_recon->order_list[r_cntr].continue_order_ind =
         ord_recon_list->ord_recons[d1.seq].order_list[from_index].continue_order_ind,
         transfer_recon->order_list[r_cntr].order_id = cnvtreal(ord_recon_list->ord_recons[d1.seq].
          order_list[from_index].order_id), transfer_recon->order_list[r_cntr].order_mnemonic =
         ord_recon_list->ord_recons[d1.seq].order_list[from_index].order_mnemonic, transfer_recon->
         order_list[r_cntr].recon_order_action_mean = ord_recon_list->ord_recons[d1.seq].order_list[
         from_index].recon_order_action_mean,
         transfer_recon->order_list[r_cntr].simplified_display_line = ord_recon_list->ord_recons[d1
         .seq].order_list[from_index].simplified_display_line
       ENDFOR
       ,transfer_recon_cnt = to_size
      OF 3:
       IF (discharge_recon_cnt=0)
        discharge_recon->encntr_id = cnvtreal(ord_recon_list->encntr_id), discharge_recon->person_id
         = person_id, discharge_recon->performed_dt_tm = cur_dttm,
        discharge_recon->performed_tz = cur_tz, discharge_recon->performed_prsnl_id = cnvtreal(
         ord_recon_list->performed_prsnl_id), discharge_recon->recon_type_flag = ord_recon_list->
        ord_recons[d1.seq].recon_type_flag,
        discharge_recon->no_known_meds_ind = ord_recon_list->ord_recons[d1.seq].no_known_meds_ind
       ENDIF
       ,to_size = (discharge_recon_cnt+ size(ord_recon_list->ord_recons[d1.seq].order_list,5)),
       IF (to_size > size(discharge_recon->order_list,5))
        stat = alterlist(discharge_recon->order_list,(to_size+ 9))
       ENDIF
       ,
       FOR (r_cntr = (discharge_recon_cnt+ 1) TO to_size)
         from_index = (r_cntr - discharge_recon_cnt), discharge_recon->order_list[r_cntr].
         clinical_display_line = ord_recon_list->ord_recons[d1.seq].order_list[from_index].
         clinical_display_line, discharge_recon->order_list[r_cntr].continue_order_ind =
         ord_recon_list->ord_recons[d1.seq].order_list[from_index].continue_order_ind,
         discharge_recon->order_list[r_cntr].order_id = cnvtreal(ord_recon_list->ord_recons[d1.seq].
          order_list[from_index].order_id), discharge_recon->order_list[r_cntr].order_mnemonic =
         ord_recon_list->ord_recons[d1.seq].order_list[from_index].order_mnemonic, discharge_recon->
         order_list[r_cntr].recon_order_action_mean = ord_recon_list->ord_recons[d1.seq].order_list[
         from_index].recon_order_action_mean,
         discharge_recon->order_list[r_cntr].simplified_display_line = ord_recon_list->ord_recons[d1
         .seq].order_list[from_index].simplified_display_line
       ENDFOR
       ,discharge_recon_cnt = to_size
     ENDCASE
    FOOT REPORT
     stat = alterlist(admit_recon->order_list,admit_recon_cnt), stat = alterlist(transfer_recon->
      order_list,transfer_recon_cnt), stat = alterlist(discharge_recon->order_list,
      discharge_recon_cnt),
     row + 0
    WITH nocounter, separator = " ", format
   ;end select
   CALL errorhandler("AddOrderRecon()","F","advsr_meds_rec_add_ord_recon ",reconreply)
   IF (admit_recon_cnt > 0)
    IF (admit_recon_status_cd > 0)
     SET admit_recon->recon_status_cd = admit_recon_status_cd
    ENDIF
    EXECUTE orm_add_order_reconciliation  WITH replace("REQUEST","ADMIT_RECON"), replace("REPLY",
     "RECONREPLY")
   ENDIF
   IF (transfer_recon_cnt > 0)
    IF (transf_recon_status_cd > 0)
     SET transfer_recon->recon_status_cd = transf_recon_status_cd
    ENDIF
    EXECUTE orm_add_order_reconciliation  WITH replace("REQUEST","TRANSFER_RECON"), replace("REPLY",
     "RECONREPLY")
   ENDIF
   IF (((discharge_recon_cnt > 0) OR ((discharge_recon->no_known_meds_ind=1))) )
    IF (disch_recon_status_cd > 0)
     SET discharge_recon->recon_status_cd = disch_recon_status_cd
    ENDIF
    EXECUTE orm_add_order_reconciliation  WITH replace("REQUEST","DISCHARGE_RECON"), replace("REPLY",
     "RECONREPLY")
   ENDIF
   CALL echorecord(admit_recon)
   CALL echorecord(transfer_recon)
   CALL echorecord(discharge_recon)
   RETURN("S")
 END ;Subroutine
 SUBROUTINE updatereconciliationstatus(null)
   DECLARE admit_recon_complete = i4 WITH protect, noconstant(0)
   DECLARE disch_recon_complete = i4 WITH protect, noconstant(0)
   DECLARE most_recent_admit_order_recon_id = f8 WITH protect, noconstant(0)
   DECLARE most_recent_disch_order_recon_id = f8 WITH protect, noconstant(0)
   DECLARE recon_cnt = i4 WITH protect, noconstant(0)
   DECLARE r_cntr = i4 WITH protect, noconstant(0)
   DECLARE complete_recon_status_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4002695,
     "COMPLETE"))
   IF (validate(recon_status_request)=0)
    RECORD recon_status_request(
      1 encounter_id = f8
      1 ppr_cd = f8
      ...
      1 rx_lookback_date_time = dq8
    ) WITH protect
   ENDIF
   IF (validate(recon_status_reply)=0)
    RECORD recon_status_reply(
      1 admission_status
        2 incomplete_ind = i2
        2 complete_ind = i2
   ...
   ENDIF
   SET medrecorders->disch_recon_status = 0
   SET medrecorders->admit_recon_status = 0
   SET recon_status_request->encounter_id = encounter_id
   SET recon_status_request->load_admission_status_ind = 1
   SET recon_status_request->load_discharge_status_ind = 1
   SET recon_status_request->inpatient_encounter_filter.focus_encounter_ind = 1
   SET recon_status_request->hide_discont_orders_amdin = 1
   SET recon_status_request->hide_discont_orders_disch = 1
   SET recon_status_request->override_org_security_ind = 1
   SET recon_status_request->rx_lookback_date_time = rx_lookback_date_time
   EXECUTE advsr_meds_rec_get_rec_status  WITH replace("REQUEST","RECON_STATUS_REQUEST"), replace(
    "REPLY","RECON_STATUS_REPLY")
   CALL echorecord(recon_status_reply)
   IF (size(recon_status_reply->admission_status,5) > 0)
    SET admit_recon_complete = recon_status_reply->admission_status[1].complete_ind
   ENDIF
   IF (size(recon_status_reply->discharge_status,5) > 0)
    SET disch_recon_complete = recon_status_reply->discharge_status[1].complete_ind
   ENDIF
   IF (((admit_recon_complete=1) OR (disch_recon_complete=1)) )
    FREE SET r_request
    FREE SET r_reply
    RECORD r_request(
      1 encntr_list[*]
    ...
    SET stat = alterlist(r_request->encntr_list,1)
    SET r_request->encntr_list[1].encntr_id = ord_recon_list->encntr_id
    SET stat = alterlist(r_request->recon_type_list,2)
    SET r_request->recon_type_list[1].recon_type = admin_recon
    SET r_request->recon_type_list[2].recon_type = disch_recon
    EXECUTE orm_get_order_reconciliations  WITH replace("REQUEST","R_REQUEST"), replace("REPLY",
     "R_REPLY")
    SET recon_cnt = size(r_reply->encntr_list,5)
    FOR (r_cntr = 1 TO recon_cnt)
      IF (most_recent_admit_order_recon_id=0
       AND (r_reply->encntr_list[r_cntr].recon_type_flag=admin_recon))
       SET most_recent_admit_order_recon_id = r_reply->encntr_list[r_cntr].order_recon_id
      ELSEIF (most_recent_disch_order_recon_id=0
       AND (r_reply->encntr_list[r_cntr].recon_type_flag=disch_recon))
       SET most_recent_disch_order_recon_id = r_reply->encntr_list[r_cntr].order_recon_id
      ENDIF
    ENDFOR
    IF (admit_recon_complete=1
     AND most_recent_admit_order_recon_id > 0.0)
     UPDATE  FROM order_recon
      SET recon_status_cd = complete_recon_status_cd
      WHERE order_recon_id=most_recent_admit_order_recon_id
     ;end update
    ENDIF
    IF (disch_recon_complete=1
     AND most_recent_disch_order_recon_id > 0.0)
     UPDATE  FROM order_recon
      SET recon_status_cd = complete_recon_status_cd
      WHERE order_recon_id=most_recent_disch_order_recon_id
     ;end update
    ENDIF
   ENDIF
   FREE RECORD recon_status_request
   FREE RECORD recon_status_reply
 END ;Subroutine
#set_return
 IF (( $OUTDEV != "NOFORMS"))
  CALL putjsonrecordtofile(reconreply)
 ENDIF
#exit_script
 SET script_version = "000 10/31/2010 RB018070"
END GO
