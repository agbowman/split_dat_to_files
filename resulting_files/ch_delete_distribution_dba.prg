CREATE PROGRAM ch_delete_distribution:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 SET failed = "T"
 SET table_name = "CHART_DISTRIBUTION"
 SET active_code = 0.0
 SET code_value1 = 0.0
 ...
 UPDATE  FROM chart_distribution c
  ...
  WITH nocounter
 ;end update
 UPDATE  FROM chart_dist_filter cdf
  SET cdf.active_ind = 0, cdf.active_status_cd = inactive_code, cdf.active_status_dt_tm =
   cnvtdatetime(curdate,curtime3),
   cdf.active_status_prsnl_id = reqinfo->updt_id
  WHERE (cdf.distribution_id=request->distribution_id)
  WITH nocounter
 ;end update
 UPDATE  FROM chart_dist_filter_value cdfv
  SET cdfv.active_ind = 0, cdfv.active_status_cd = inactive_code, cdfv.active_status_dt_tm =
   cnvtdatetime(curdate,curtime3),
   cdfv.active_status_prsnl_id = reqinfo->updt_id
  WHERE (cdfv.distribution_id=request->distribution_id)
  WITH nocounter
 ;end update
 IF (curqual > 0)
  SET failed = "F"
