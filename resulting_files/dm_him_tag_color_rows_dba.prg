CREATE PROGRAM dm_him_tag_color_rows:dba
 ...
 SET reply->table_name = "TAG_COLOR"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(tc.tag_color_id)
   FROM tag_color tc
   WHERE tc.tag_color_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    tc.rowid
    FROM tag_color tc
    WHERE tc.tag_color_id BETWEEN curminid AND curmaxid
     AND tc.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = tc.rowid
    WITH nocounter, maxqual(tc,value(rowsleft))
   ;end select
