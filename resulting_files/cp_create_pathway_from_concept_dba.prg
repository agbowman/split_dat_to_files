CREATE PROGRAM cp_create_pathway_from_concept:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Input Pathway Name" = "",
  "Input Concept Code" = 0.0,
  "Input Pathway Type Meaning" = "",
  "Create Options:" = 0
  WITH outdev, sinput_pathway_name, dinput_concept_cd,
  sinput_pathway_type_mean, lcreate_options
 ...
 FREE RECORD pwconfigreply
 RECORD pwconfigreply(
   1 pathway_id = f8
   1 pathway_name = vc
   1 concept_cd = f8
   1 node_list[*]
 ...
 DECLARE checkforexistingpathway(null) = f8 WITH protect
 DECLARE buildimportrequestfromconcept(conceptcd=f8,icompareconfig=i2) = null WITH protect
 DECLARE buildnewpathwayrequest(null) = null WITH protect
 DECLARE getconceptdetails(null) = null WITH protect
 DECLARE loaddefaultcomponentmapping(null) = null WITH protect
 DECLARE deletepathwayguts(null) = null WITH protect
 DECLARE dpathway_input_type_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003197,trim(
     $SINPUT_PATHWAY_TYPE_MEAN,3)))
 DECLARE dpathway_status_cd_active = f8 WITH protect, constant(uar_get_code_by("MEANING",4003198,
   "ACTIVE"))
 DECLARE dexistingpathwayid = f8 WITH protect, noconstant(0)
 DECLARE icompareconfig = i2 WITH protect, noconstant(0)
 ...
 SET dexistingpathwayid = checkforexistingpathway(null)
 CALL loaddefaultcomponentmapping(null)
 IF (dexistingpathwayid > 0)
  IF (( $LCREATE_OPTIONS=1))
   CALL deletepathwayguts(null)
  ELSEIF (( $LCREATE_OPTIONS=2))
   SET icompareconfig = 1
  ELSE
   CALL echo("Not supported at this Time!")
   SET mpagereply->status_data.status = "F"
   GO TO exit_script
  ENDIF
 ENDIF
 CALL createnewpathway(dexistingpathwayid,icompareconfig)
 SET mpagereply->status_data.status = "S"
 SUBROUTINE checkforexistingpathway(null)
   CALL log_message("Begin CheckForExistingPathway()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE dexistingpathwayid = f8 WITH noconstant(0), protect
   SELECT INTO "NL:"
    FROM cp_pathway cp
    WHERE (cp.cp_concept_cd= $DINPUT_CONCEPT_CD)
     AND cp.pathway_status_cd=dpathway_status_cd_active
     AND cp.pathway_type_cd=dpathway_input_type_cd
    DETAIL
     dexistingpathwayid = cp.cp_pathway_id
    WITH nocounter
   ;end select
   RETURN(dexistingpathwayid)
   CALL log_message(build("Exit CheckForExistingPathway(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (createnewpathway(dpathwayid=f8,icompareconfig=i2) =null WITH protect)
   CALL log_message("Begin CreateNewPathway()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   IF (icompareconfig=1)
    EXECUTE cp_load_pathway_config "NOFORMS", dpathwayid
    SET stat = moverec(reply->node_list,pwconfigreply->node_list)
    FREE RECORD reply
    CALL echorecord(pwconfigreply)
   ENDIF
   SET stat = alterlist(pw_rec->qual,1)
   SET pw_rec->qual[1].cp_pathway_id = dpathwayid
   SET pw_rec->qual[1].pathway_name =  $SINPUT_PATHWAY_NAME
   SET pw_rec->qual[1].pathway_type_cd = dpathway_input_type_cd
   SET pw_rec->qual[1].pathway_status_mean = "ACTIVE"
   SET pw_rec->qual[1].concept_cd =  $DINPUT_CONCEPT_CD
   CALL buildimportrequestfromconcept( $DINPUT_CONCEPT_CD)
   CALL echorecord(pw_rec)
   EXECUTE cp_import_pw_data
   SET mpagereply->pathway_id = pw_rec->qual[1].cp_pathway_id
   CALL log_message(build("Exit CreateNewPathway(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE buildimportrequestfromconcept(conceptcd)
   CALL log_message("Begin BuildImportRequestFromConcept()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE detail_reltn_concept_group = f8 WITH constant(uar_get_code_by("MEANING",4003134,
     "CONCEPTGROUP"))
   DECLARE x = i4 WITH private, noconstant(0)
   DECLARE y = i4 WITH private, noconstant(0)
   DECLARE z = i4 WITH private, noconstant(0)
   DECLARE pos = i4 WITH protect, noconstant(0)
   DECLARE comppos = i4 WITH protect, noconstant(0)
   DECLARE nodepos = i4 WITH protect, noconstant(0)
   DECLARE exnodepos = i4 WITH protect, noconstant(0)
   DECLARE excomppos = i4 WITH protect, noconstant(0)
   DECLARE num = i4 WITH protect, noconstant(0)
   DECLARE lcurdetailcnt = i4 WITH protect, noconstant(0)
   DECLARE dcurintentioncd = f8 WITH protect, noconstant(0)
   DECLARE dcurcomptypecd = f8 WITH protect, noconstant(0)
   DECLARE scurgroupmeaning = vc WITH protect, noconstant("")
   FREE RECORD conceptreq
   RECORD conceptreq(
     1 concept_cds[*]
       2 concept_cd = f8
       2 intention_list[*]
   ...
   FREE RECORD reply
   SET stat = alterlist(conceptreq->concept_cds,1)
   SET conceptreq->concept_cds[1].concept_cd = conceptcd
   EXECUTE cp_get_concept  WITH replace("REQUEST","CONCEPTREQ")
   IF ((((reply->status_data.status != "S")) OR (size(reply->concept_list,5) != 1)) )
    SET mpagereply->status_data.status = "F"
    CALL populate_subeventstatus_rec("Get Concept","Failed","cp_get_concept",reply->status_data.
     status,mpagereply)
    GO TO exit_script
   ENDIF
   FOR (x = 1 TO size(reply->concept_list[1].cg_list,5))
     SET nodepos = 0
     SET comppos = 0
     SET dcurintentioncd = reply->concept_list[1].cg_list[x].intention_cd
     IF ((pw_rec->qual[1].node_cnt > 0))
      SET nodepos = locateval(num,1,pw_rec->qual[1].node_cnt,dcurintentioncd,pw_rec->qual[1].
       node_list[num].intention_cd)
     ENDIF
     IF (nodepos=0)
      SET pw_rec->qual[1].node_cnt += 1
      SET stat = alterlist(pw_rec->qual[1].node_list,pw_rec->qual[1].node_cnt)
      SET nodepos = pw_rec->qual[1].node_cnt
      SET exnodepos = locateval(num,1,size(pwconfigreply->node_list,5),dcurintentioncd,pwconfigreply
       ->node_list[num].intention_cd)
      IF (exnodepos > 0)
       SET pw_rec->qual[1].node_list[nodepos].cp_node_id = pwconfigreply->node_list[exnodepos].
       cp_node_id
      ELSE
       SET pw_rec->qual[1].node_list[nodepos].concept_cd = conceptcd
       SET pw_rec->qual[1].node_list[nodepos].node_name = uar_get_code_display(dcurintentioncd)
       SET pw_rec->qual[1].node_list[nodepos].intention_cd = dcurintentioncd
      ENDIF
     ENDIF
     SET scurgroupmeaning = reply->concept_list[1].cg_list[x].concept_group_cd_mean
     CALL echo(build2("sCurGroupMeaning--------->",scurgroupmeaning))
     SET pos = locateval(num,1,size(componentmappings->qual,5),scurgroupmeaning,componentmappings->
      qual[num].concept_group_mean)
     CALL echo(build2("pos--------->",pos))
     IF (pos > 0)
      CALL echo(componentmappings->qual[pos].component_mean)
      SET dcurcomptypecd = uar_get_code_by("MEANING",4003130,componentmappings->qual[pos].
       component_mean)
      IF (dcurcomptypecd > 0)
       IF ((pw_rec->qual[1].node_list[nodepos].comp_cnt > 0))
        SET comppos = locateval(num,1,pw_rec->qual[1].node_list[nodepos].comp_cnt,dcurcomptypecd,
         pw_rec->qual[1].node_list[nodepos].comp_list[num].comp_type_cd)
       ENDIF
       IF (comppos=0)
        SET pw_rec->qual[1].node_list[nodepos].comp_cnt += 1
        SET comppos = pw_rec->qual[1].node_list[nodepos].comp_cnt
        SET stat = alterlist(pw_rec->qual[1].node_list[nodepos].comp_list,comppos)
        SET excomppos = locateval(num,1,size(pwconfigreply->node_list[exnodepos].component_list,5),
         dcurcomptypecd,pwconfigreply->node_list[exnodepos].component_list[num].comp_type_cd)
        IF (excomppos > 0)
         SET pw_rec->qual[1].node_list[nodepos].comp_list[comppos].cp_component_id = pwconfigreply->
         node_list[exnodepos].component_list[excomppos].cp_component_id
        ELSE
         SET pw_rec->qual[1].node_list[nodepos].comp_list[comppos].comp_type_cd = dcurcomptypecd
         SET pw_rec->qual[1].node_list[nodepos].comp_list[comppos].component_seq_txt = build("1:1:",
          cnvtstring(comppos))
        ENDIF
       ENDIF
      ENDIF
     ENDIF
   ENDFOR
   CALL log_message(build("Exit BuildImportRequestFromConcept(), Elapsed time in seconds:",((curtime3
      - begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE loaddefaultcomponentmapping(null)
   ...
   SELECT INTO "NL:"
    ...
    FOOT REPORT
     stat = alterlist(componentmappings->qual,cnt)
    WITH nocounter
   ;end select
   ...
   CALL log_message(build("Exit loadDefaultComponentMapping(), Elapsed time in seconds:",((curtime3
      - begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE deletepathwayguts(null)
   CALL log_message("In DeletePathwayGuts()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE x = i4 WITH private, noconstant(0)
   DECLARE y = i4 WITH private, noconstant(0)
   DECLARE num = i4 WITH protect, noconstant(0)
   DECLARE lnodesize = i4 WITH protect, noconstant(0)
   DECLARE lcomponentsize = i4 WITH protect, noconstant(0)
   DECLARE lcomponentcnt = i4 WITH protect, noconstant(0)
   EXECUTE cp_load_pathway_config "NOFORMS", dexistingpathwayid
   RECORD pathway_pks(
     1 component_cnt = i4
     1 component_ids[*]
       2 component_id = f8
   ...
   SET lnodesize = size(reply->node_list,5)
   SET stat = alterlist(pathway_pks->node_ids,lnodesize)
   SET pathway_pks->node_cnt = lnodesize
   FOR (x = 1 TO lnodesize)
     SET pathway_pks->node_ids[x].node_id = reply->node_list[x].cp_node_id
     SET lcomponentsize = size(reply->node_list[x].component_list,5)
     SET stat = alterlist(pathway_pks->component_ids,(pathway_pks->component_cnt+ lcomponentsize))
     FOR (y = 1 TO lcomponentsize)
      SET lcomponentcnt = (pathway_pks->component_cnt+ y)
      SET pathway_pks->component_ids[lcomponentcnt].component_id = reply->node_list[x].
      component_list[y].cp_component_id
     ENDFOR
     SET pathway_pks->component_cnt += lcomponentsize
   ENDFOR
   CALL echorecord(pathway_pks)
   IF ((pathway_pks->component_cnt > 0))
    DELETE  FROM cp_component_detail ccd
     WHERE expand(num,1,pathway_pks->component_cnt,ccd.cp_component_id,pathway_pks->component_ids[num
      ].component_id)
     WITH nocounter
    ;end delete
    CALL error_and_zero_check_rec(curqual,"CP_COMPONENT_DETAIL","DeletePathwayGuts",1,1,
     mpagereply)
    DELETE  FROM cp_component cc
     WHERE expand(num,1,pathway_pks->node_cnt,cc.cp_node_id,pathway_pks->node_ids[num].node_id)
     WITH nocounter
    ;end delete
    CALL error_and_zero_check_rec(curqual,"CP_COMPONENT","DeletePathwayGuts",1,1,
     mpagereply)
   ENDIF
   IF ((pathway_pks->node_cnt > 0))
    DELETE  FROM cp_node cn
     WHERE expand(num,1,pathway_pks->node_cnt,cn.cp_node_id,pathway_pks->node_ids[num].node_id)
     WITH nocounter
    ;end delete
    CALL error_and_zero_check_rec(curqual,"CP_NODE","DeletePathwayGuts",1,1,
     mpagereply)
   ENDIF
   SET reqinfo->commit_ind = 1
   FREE RECORD reply
   CALL log_message(build("Exit DeletePathwayGuts(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (validate(debug_ind,0))
  CALL echorecord(componentmappings)
