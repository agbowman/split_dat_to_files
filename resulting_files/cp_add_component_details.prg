CREATE PROGRAM cp_add_component_details
 PROMPT
 ...
 SUBROUTINE (setauditinitreplystatusbyprogramreplystatus(reply=vc(ref)) =null WITH protect)
  ...
  ENDIF
 END ;Subroutine
 DECLARE idetailsize = i2 WITH protect, noconstant(0)
 IF (validate(comp_details_reply) != 1)
  RECORD comp_details_reply(
    1 status_data
      2 status = c1
      2 subeventstatus[1]
        3 operationname = c25
 ...
 IF (validate(request->blob_in))
  IF ((request->blob_in > " "))
   CALL logblobindata("CP_ADD_COMPONENT_DETAILS",request->blob_in, $OUTDEV)
   IF (validate(debug_ind,0)=1)
    CALL echo(request->blob_in)
   ENDIF
   SET jrec = cnvtjsontorec(request->blob_in)
   IF (jrec != 1)
    SET comp_details_reply->status_data.status = "F"
    GO TO exit_script
   ENDIF
   FREE RECORD request
  ENDIF
 ENDIF
 SET idetailsize = size(add_comp_details->details,5)
 IF (idetailsize > 0)
  FOR (x = 1 TO idetailsize)
   EXECUTE cp_add_component_detail_json "MINE:", add_comp_details->details[x].cp_component_id,
   add_comp_details->details[x].comp_detail_reltn_cd,
   add_comp_details->details[x].component_entity_id, add_comp_details->details[x].
   component_entity_name, add_comp_details->details[x].component_ident,
   add_comp_details->details[x].component_text
  ...
  ENDFOR
 ENDIF
 SET comp_details_reply->status_data.status = "S"
#exit_script
 CALL setauditinitreplystatusbyprogramreplystatus(comp_details_reply)
 CALL putjsonrecordtofile(comp_details_reply)
 FREE RECORD add_comp_details
