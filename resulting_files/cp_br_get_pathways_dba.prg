CREATE PROGRAM cp_br_get_pathways:dba
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE 4003197_oncology_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003197,"ONCOLOGY"
   ))
 DECLARE 4003197_cpm_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003197,"CPM"))
 DECLARE 4003197_legacydebug_cd = f8 WITH constant(uar_get_code_by("MEANING",4003197,"LEGACYDEBUG")),
 protect
 DECLARE 4003198_active_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003198,"ACTIVE"))
 ...
 DECLARE getcarepathwaylist(null) = null
 SUBROUTINE PUBLIC::main(null)
   CALL log_message("Entering subroutine Main()",log_level_debug)
   SET reply->status_data.status = "F"
   CALL getcarepathwaylist(null)
   CALL log_message("Exiting subroutine Main()",log_level_debug)
 END ;Subroutine
 SUBROUTINE PUBLIC::getcarepathwaylist(null)
   CALL log_message("Entering subroutine getCarePathwayList()",log_level_debug)
   SELECT
    pathway_name_sort = substring(1,1000,cnvtupper(cp.pathway_name))
    FROM cp_pathway cp
    WHERE cp.pathway_type_cd IN (4003197_oncology_cd, 4003197_cpm_cd, 4003197_legacydebug_cd)
     AND cp.cp_pathway_id > 0.0
     AND cp.logical_domain_id=user_logic_domain_id
     AND cp.active_ind=1
     AND cp.pathway_status_cd=4003198_active_cd
    ORDER BY pathway_name_sort
    HEAD REPORT
     path_cnt = 0, stat = alterlist(reply->custom_filter_items,10)
    HEAD cp.pathway_name
     path_cnt += 1
     IF (size(reply->custom_filter_items,5) > path_cnt)
      stat = alterlist(reply->custom_filter_items,(path_cnt+ 10))
     ENDIF
     reply->custom_filter_items[path_cnt].parent_entity_id = cp.cp_pathway_id, reply->
     custom_filter_items[path_cnt].parent_entity_name = "CP_PATHWAY", reply->custom_filter_items[
     path_cnt].display = trim(cp.pathway_name),
     reply->custom_filter_items[path_cnt].status_ind = 1
    FOOT REPORT
     stat = alterlist(reply->custom_filter_items,path_cnt)
    WITH nocounter
   ;end select
   IF (curqual > 0)
    SET reply->status_data.status = "S"
   ...
   ENDIF
   CALL log_message("Exiting subroutine GetBundleData()",log_level_debug)
 END ;Subroutine
 CALL main(null)
#exit_script
 IF (validate(debug,0)=1)
  CALL bedlogmessage("Enhanced logging for cp_br_get_pathways")
  CALL echorecord(reply)
 ENDIF
