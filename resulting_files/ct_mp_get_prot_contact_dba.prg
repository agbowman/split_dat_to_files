CREATE PROGRAM ct_mp_get_prot_contact:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_prot_contact"
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getprotcontact(null) = null WITH protect
 DECLARE getcontactdetails(null) = null WITH protect
 DECLARE stat = i4 WITH protect, noconstant(0)
 SET public::"reply"->status_data.status = "F"
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_contact","Invalid JSON Request",
   PUBLIC::"reply")
  GO TO exit_script
 ENDIF
 SET stat = cnvtjsontorec( $JSONREQUEST)
 CALL getprotcontact(null)
 FREE RECORD request
 SET public::"reply"->status_data.status = "S"
 SUBROUTINE getprotcontact(null)
   CALL log_message("In GetProtContact()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE protcount = i4 WITH protect, noconstant(size(request->prot_list,5))
   DECLARE contactcount = i4 WITH protect, noconstant(0)
   DECLARE index = i4 WITH protect, noconstant(0)
   DECLARE emailaddresspref = i2 WITH protect, noconstant(0)
   IF (protcount > 0)
    ...
    EXECUTE ct_get_pref  WITH replace("REQUEST_STRUCT","CT_GET_PREF_REQUEST"), replace("REPLY",
     "CT_GET_PREF_REPLY")
    SET emailaddresspref = ct_get_pref_reply->pref_value
    FREE RECORD ct_get_pref_request
    FREE RECORD ct_get_pref_reply
    FOR (index = 1 TO protcount)
      SET contactcount = 0
      SELECT
       FROM prot_role pr,
        prsnl p,
        organization org
       PLAN (pr
        WHERE (pr.prot_amendment_id=request->prot_list[index].amendmentid)
       ...
       ORDER BY cnvtlower(p.name_last), cnvtlower(p.name_first)
       DETAIL
        contactcount += 1
        IF (mod(contactcount,10)=1)
         stat = alterlist(public::"reply"->prot_list[index]."prot_info"."protocol_contact",(
          contactcount+ 9))
        ENDIF
        public::"reply"->prot_list[index]."protocolId" = request->prot_list[index].protocolid,
        public::"reply"->prot_list[index]."amendmentId" = request->prot_list[index].amendmentid
        IF (p.person_id != 0)
         public::"reply"->prot_list[index].protocol_contact[contactcount]."person_id" = p.person_id,
         public::"reply"->prot_list[index].protocol_contact[contactcount]."person_full_name" = p
         .name_full_formatted
        ENDIF
        public::"reply"->prot_list[index].protocol_contact[contactcount]."role_name" =
        uar_get_code_display(pr.prot_role_cd), public::"reply"->prot_list[index].protocol_contact[
        contactcount]."organization_name" = org.org_name
       FOOT  pr.prot_amendment_id
        stat = alterlist(public::"reply"->prot_list[index]."prot_info"."protocol_contact",
         contactcount)
       WITH nocounter
      ;end select
      CALL getcontactdetails(null)
    ENDFOR
   ELSE
    CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_contact",
     "Did not receive any protocol ID",PUBLIC::"reply")
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit GetProtContact(), Elapsed time in seconds:",datetimediff(cnvtdatetime
      (sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getcontactdetails(null)
   CALL log_message("In GetContactDetails()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE emailcd = f8 WITH protect, noconstant(0.0)
   DECLARE busphonecd = f8 WITH protect, noconstant(0.0)
   DECLARE alphaphonecd = f8 WITH protect, noconstant(0.0)
   SET stat = uar_get_meaning_by_codeset(43,"BUSINESS",1,busphonecd)
   SET stat = uar_get_meaning_by_codeset(43,"PAGER BUS",1,alphaphonecd)
   SET stat = uar_get_meaning_by_codeset(212,"EMAIL",1,emailcd)
   IF (contactcount > 0)
    SELECT
     FROM phone ph,
      (dummyt d  WITH seq = value(contactcount))
     PLAN (d)
      JOIN (ph
      WHERE (ph.parent_entity_id=public::"reply"->prot_list[index].protocol_contact[d.seq].
      "person_id")
       AND ph.parent_entity_name="PERSON"
       AND ((ph.phone_type_cd=alphaphonecd) OR (ph.phone_type_cd=busphonecd))
       AND ph.active_ind=1)
     DETAIL
      IF (ph.phone_type_cd=alphaphonecd)
       public::"reply"->prot_list[index].protocol_contact[d.seq]."pager_num" = cnvtphone(ph.phone_num,
        ph.phone_format_cd)
      ELSEIF (ph.phone_type_cd=busphonecd)
       public::"reply"->prot_list[index].protocol_contact[d.seq]."phone_num" = cnvtphone(ph.phone_num,
        ph.phone_format_cd)
      ENDIF
     WITH nocounter
    ;end select
    IF (emailaddresspref=1)
     SELECT
      FROM address a,
       (dummyt d  WITH seq = value(contactcount))
      PLAN (d)
       ...
       WHERE a.parent_entity_name="PERSON"
        ...
        AND (a.parent_entity_id=public::"reply"->prot_list[index].protocol_contact[d.seq]."person_id"
       )
        AND a.address_type_cd=emailcd
        AND a.address_type_seq=1)
      DETAIL
       public::"reply"->prot_list[index].protocol_contact[d.seq]."email_addr" = a.street_addr
      WITH nocounter
     ;end select
    ELSE
     SELECT
      ...
      PLAN (d)
       JOIN (p
       WHERE p.end_effective_dt_tm > cnvtdatetime(sysdate)
        AND (p.person_id=public::"reply"->prot_list[index].protocol_contact[d.seq]."person_id"))
      DETAIL
       public::"reply"->prot_list[index].protocol_contact[d.seq]."email_addr" = p.email
      WITH nocounter
     ;end select
    ENDIF
   ENDIF
   CALL log_message(build("Exit GetContactDetails(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (g_debug_ind=1)
