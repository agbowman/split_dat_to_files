CREATE PROGRAM bed_aud_lab_orc_dta:dba
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY oc.activity_type_cd, cnvtupper(oc.primary_mnemonic), ptr.sequence
  HEAD oc.catalog_cd
 ...
 FOR (x = 1 TO temp->ocnt)
   ...
   SET reply->rowlist[row_nbr].celllist[11].string_value = temp->oqual[x].dept_name
   ...
   FOR (y = 1 TO temp->oqual[x].dcnt)
     ...
     SET reply->rowlist[row_nbr].celllist[9].string_value = temp->oqual[x].dqual[y].restrict_display
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("lab_orc_dta_audit.csv")
 ENDIF
