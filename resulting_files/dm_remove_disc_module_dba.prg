CREATE PROGRAM dm_remove_disc_module:dba
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 IF (check_logfile("dm_rmv_text",".log","DM_REMOVE_DISC_MODULE LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_REMOVE_DISC_MODULE..."
 DECLARE drto_det_id = f8 WITH protect, noconstant(0.0)
 DECLARE drto_log_id = f8 WITH protect, noconstant(0.0)
 IF ((validate(drop_rec->cnt,- (1))=- (1)))
 ...
 IF (((reflect(parameter(1,0)) != "I*"
  ...
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "Expected syntax:  dm_remove_disc_module <DM_TEXT_FIND_DETAIL_ID>, <DM_TEXT_FIND_LOG_ID> go"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_disc
 ENDIF
 SET drto_det_id =  $1
 ...
 SELECT INTO "NL:"
  ...
  WHERE d.script_name IS NOT null
   AND d.script_name > " "
   AND d.dm_text_find_detail_id=drto_det_id
   AND d.status_flag != 3
   AND  NOT ( EXISTS (
  (SELECT
   "x"
   FROM eks_module m,
    eks_modulestorage e
   WHERE trim(m.module_name)=d.script_name
    AND sqlpassthru(" m.module_name like (d.script_name||'%')")
    AND m.active_flag="A"
    AND  NOT (m.maint_validation IN ("EXAMPLE"))
    AND e.module_name=m.module_name
    AND e.version=m.version
    AND e.data_type=d.group_num)))
  HEAD REPORT
   stat = alterlist(drop_rec->qual,100)
  DETAIL
   drop_rec->cnt = (drop_rec->cnt+ 1)
   IF (mod(drop_rec->cnt,100)=1
    AND (drop_rec->cnt > 1))
    stat = alterlist(drop_rec->qual,(drop_rec->cnt+ 99))
   ENDIF
   drop_rec->qual[drop_rec->cnt].script_name = d.script_name, drop_rec->qual[drop_rec->cnt].
   dm_text_find_data_id = d.dm_text_find_data_id, drop_rec->qual[drop_rec->cnt].parent_entity_id = d
   .parent_entity_id,
   drop_rec->qual[drop_rec->cnt].parent_entity_name = d.parent_entity_name, drop_rec->qual[drop_rec->
   cnt].parent_entity_col = d.parent_entity_col, drop_rec->qual[drop_rec->cnt].search_col_name = d
   .search_col_name,
   drop_rec->qual[drop_rec->cnt].group_num = d.group_num, drop_rec->qual[drop_rec->cnt].exists_ind =
   0
  FOOT REPORT
   stat = alterlist(drop_rec->qual,drop_rec->cnt)
  WITH nocounter
 ;end select
 IF (check_error("Querying dm_text_find_data table") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(drto_log_id,"ERROR",concat("Getting DM_TEXT_FIND_DATA info: ",dm_err->emsg),
   0.0,0.0)
  GO TO exit_disc
 ENDIF
#exit_disc
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending DM_REMOVE_DISC_MODULE"
  CALL final_disp_msg("dm_remove_disc_module")
 ENDIF
END GO
