CREATE PROGRAM bhs_340b_macrohelix_visits:dba
 PROMPT
 ...
 RECORD 340b_visits(
   ...
   1 qual[*]
     ...
     2 c_location = c50
     2 c_facility = c50
     2 c_patientname_last = c50
     2 c_patientname_first = c50
     2 c_dob = c10
     2 c_zipcode = c12
     2 c_icdtype = c5
     2 c_diag_str = vc
 ...
 DECLARE f_mrn_cd = f8 WITH constant(uar_get_code_by_cki("CKI.CODEVALUE!8021"))
 DECLARE f_fin_cd = f8 WITH constant(uar_get_code_by_cki("CKI.CODEVALUE!2930"))
 ...
 DECLARE f_inp_cd = f8 WITH constant(uar_get_code_by_cki("CKI.CODEVALUE!17006"))
 DECLARE f_out_cd = f8 WITH constant(uar_get_code_by_cki("CKI.CODEVALUE!17007"))
 DECLARE f_home_addr = f8 WITH constant(uar_get_code_by_cki("CKI.CODEVALUE!4018"))
 DECLARE f_final_dx_type_cd = f8 WITH constant(uar_get_code_by_cki("CKI.CODEVALUE!13984"))
 DECLARE f_dx_principle_type_cd = f8 WITH constant(uar_get_code_by_cki("CKI.CODEVALUE!2971"))
 DECLARE f_icd10cm_src_voc_cd = f8 WITH constant(uar_get_code_by_cki("CKI.CODEVALUE!4101498946"))
 DECLARE f_icd9cm_src_voc_cd = f8 WITH constant(uar_get_code_by_cki("CKI.CODEVALUE!2953"))
 ...
 SET ms_report_title = build2("CHR_NY 340B Encounter Feed")
 IF (cnvtupper(substring(1,1,curdomain))="P")
  SET ms_filename = build2(trim(ms_fac_code,3),"_patient_encounter_",format(cnvtdatetime(sysdate),
    "YYYYMMDD;;d"),".txt")
 ELSE
  SET ms_filename = build2("test_",trim(ms_fac_code,3),"_patient_encounter_",format(cnvtdatetime(
 ...
 SELECT INTO "nl:"
  ...
  HEAD e.encntr_id
   ml_cnt += 1
   IF (size(340b_visits->qual,5) < ml_cnt)
    stat = alterlist(340b_visits->qual,(ml_cnt+ 9))
   ENDIF
   340b_visits->qual[ml_cnt].d_disch_dt_tm = e.disch_dt_tm
   IF (((e.disch_dt_tm=null) OR (e.disch_dt_tm=0)) )
    340b_visits->qual[ml_cnt].d_disch_dt_tm = c.service_dt_tm
   ENDIF
   340b_visits->qual[ml_cnt].c_disch_date = format(340b_visits->qual[ml_cnt].d_disch_dt_tm,
    "MM/DD/YYYY;;D"), 340b_visits->qual[ml_cnt].c_location = uar_get_code_description(e
    .loc_nurse_unit_cd), 340b_visits->qual[ml_cnt].c_facility = uar_get_code_display(e
    .loc_facility_cd),
   340b_visits->qual[ml_cnt].f_encntr_id = e.encntr_id, 340b_visits->qual[ml_cnt].f_person_id = e
   .person_id
  FOOT REPORT
   stat = alterlist(340b_visits->qual,ml_cnt), 340b_visits->l_cnt = ml_cnt, ml_cnt = 0
  WITH nocounter
 ;end select
 CALL echo("Stop of Main_Qual_data(null)")
 IF ((340b_visits->l_cnt != 0))
  CALL echo("Start of Get_fin(null)")
  SELECT INTO "nl:"
   FROM encntr_alias ea
   PLAN (ea
    WHERE expand(ml_loop,1,340b_visits->l_cnt,ea.encntr_id,340b_visits->qual[ml_loop].f_encntr_id)
   ...
   HEAD ea.encntr_id
    ml_pos = locateval(ml_loop,1,340b_visits->l_cnt,ea.encntr_id,340b_visits->qual[ml_loop].
     f_encntr_id)
   HEAD ea.encntr_alias_type_cd
    IF (ea.encntr_alias_type_cd=f_fin_cd)
     340b_visits->qual[ml_pos].c_fin = ea.alias
    ENDIF
    IF (ea.encntr_alias_type_cd=f_mrn_cd)
     340b_visits->qual[ml_pos].c_mrn = ea.alias
    ENDIF
   FOOT  ea.encntr_id
    ml_pos1 = ml_pos
    WHILE (ml_pos > 0)
      340b_visits->qual[ml_pos].c_fin = 340b_visits->qual[ml_pos1].c_fin, 340b_visits->qual[ml_pos].
      c_mrn = 340b_visits->qual[ml_pos1].c_mrn, ml_pos = locateval(ml_loop,(ml_pos+ 1),340b_visits->
       l_cnt,ea.encntr_id,340b_visits->qual[ml_loop].f_encntr_id)
    ENDWHILE
   WITH expand = 2, nocounter
  ;end select
  CALL echo("Stop of Get_fin(null)")
  CALL echo("Start of Get_name_dob(null)")
  SELECT INTO "nl:"
   FROM person p
   PLAN (p
    WHERE expand(ml_loop,1,340b_visits->l_cnt,p.person_id,340b_visits->qual[ml_loop].f_person_id)
     AND p.active_ind=1
     AND p.beg_effective_dt_tm <= cnvtdatetime(sysdate)
     AND p.end_effective_dt_tm > cnvtdatetime(sysdate))
   ORDER BY p.person_id
   HEAD p.person_id
    ml_pos = locateval(ml_loop,1,340b_visits->l_cnt,p.person_id,340b_visits->qual[ml_loop].
     f_person_id), 340b_visits->qual[ml_pos].c_patientname_first = trim(p.name_first,3), 340b_visits
    ->qual[ml_pos].c_patientname_last = trim(p.name_last,3),
    340b_visits->qual[ml_pos].c_dob = format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1
      ),"mm/dd/yy;;d")
   FOOT  p.person_id
    ml_pos1 = ml_pos
    WHILE (ml_pos > 0)
      340b_visits->qual[ml_pos].c_patientname_first = 340b_visits->qual[ml_pos1].c_patientname_first,
      340b_visits->qual[ml_pos].c_patientname_last = 340b_visits->qual[ml_pos1].c_patientname_last,
      340b_visits->qual[ml_pos].c_dob = 340b_visits->qual[ml_pos1].c_dob,
      ml_pos = locateval(ml_loop,(ml_pos+ 1),340b_visits->l_cnt,p.person_id,340b_visits->qual[ml_loop
       ].f_person_id)
    ENDWHILE
   WITH expand = 2, nocounter
  ;end select
  CALL echo("Stop of Get_name_dob(null)")
  CALL echo("Start of Get_address(null)")
  SELECT INTO "nl:"
   FROM address a
   PLAN (a
    WHERE expand(ml_loop,1,340b_visits->l_cnt,a.parent_entity_id,340b_visits->qual[ml_loop].
     f_person_id)
     ...
     AND a.address_type_cd=f_home_addr)
   ORDER BY a.parent_entity_id, a.beg_effective_dt_tm DESC
   HEAD a.parent_entity_id
    ml_pos = locateval(ml_loop,1,340b_visits->l_cnt,a.parent_entity_id,340b_visits->qual[ml_loop].
     f_person_id), 340b_visits->qual[ml_pos].c_zipcode = trim(a.zipcode,3)
   FOOT  a.parent_entity_id
    ml_pos1 = ml_pos
    WHILE (ml_pos > 0)
     340b_visits->qual[ml_pos].c_zipcode = 340b_visits->qual[ml_pos1].c_zipcode,ml_pos = locateval(
      ml_loop,(ml_pos+ 1),340b_visits->l_cnt,a.parent_entity_id,340b_visits->qual[ml_loop].
      f_person_id)
    ENDWHILE
   WITH nocounter, expand = 2
  ;end select
  CALL echo("Stop of Get_address(null)")
  CALL echo("Start of get_diagnosis(null)")
  SELECT INTO "nl:"
   FROM diagnosis dx,
    nomenclature n
   PLAN (dx
    WHERE expand(ml_loop,1,340b_visits->l_cnt,dx.encntr_id,340b_visits->qual[ml_loop].f_encntr_id)
     AND dx.active_ind=1
     AND dx.beg_effective_dt_tm <= cnvtdatetime(sysdate)
     AND dx.end_effective_dt_tm > cnvtdatetime(sysdate))
    JOIN (n
    WHERE n.nomenclature_id=dx.nomenclature_id
     AND n.source_vocabulary_cd IN (f_icd10cm_src_voc_cd, f_icd9cm_src_voc_cd))
   ORDER BY dx.encntr_id, dx.diag_priority
   HEAD dx.encntr_id
    ml_pos = locateval(ml_loop,1,340b_visits->l_cnt,dx.encntr_id,340b_visits->qual[ml_loop].
     f_encntr_id), diag_cnt = 0
   DETAIL
    diag_cnt += 1, stat = alterlist(340b_visits->qual[ml_pos].diag,diag_cnt), 340b_visits->qual[
    ml_pos].l_diag_cnt = diag_cnt,
    340b_visits->qual[ml_pos].diag[diag_cnt].c_diag = trim(n.source_identifier)
    IF (n.source_vocabulary_cd=f_icd10cm_src_voc_cd)
     340b_visits->qual[ml_pos].c_icdtype = "ICD10"
    ELSE
     340b_visits->qual[ml_pos].c_icdtype = "ICD9"
    ENDIF
   WITH nocounter, expand = 2
  ;end select
  CALL echo("Stop of get_diagnosis(null)")
  CALL echorecord(340b_visits)
  IF (( $S_MODE="2"))
   CALL echo("Start of get_screen_output(null)")
   FOR (ml_loop1 = 1 TO size(340b_visits->qual,5))
     FOR (ml_loop2 = 1 TO 24)
       IF ((ml_loop2 <= 340b_visits->qual[ml_loop1].l_diag_cnt))
        SET 340b_visits->qual[ml_loop1].c_diag_str = build2(trim(340b_visits->qual[ml_loop1].
          c_diag_str,3),trim(340b_visits->qual[ml_loop1].diag[ml_loop2].c_diag,3),"|")
       ENDIF
     ENDFOR
   ENDFOR
   SELECT INTO  $OUTDEV
    disch_date = 340b_visits->qual[d1.seq].c_disch_date, mrn = 340b_visits->qual[d1.seq].c_mrn, fin
     = 340b_visits->qual[d1.seq].c_fin,
    location = 340b_visits->qual[d1.seq].c_location, patientname_last = 340b_visits->qual[d1.seq].
    c_patientname_last, patientname_first = 340b_visits->qual[d1.seq].c_patientname_first,
    dob = 340b_visits->qual[d1.seq].c_dob, zipcode = 340b_visits->qual[d1.seq].c_zipcode, facility =
    340b_visits->qual[d1.seq].c_facility,
    icd10 = 340b_visits->qual[d1.seq].c_icdtype, diagnosis = 340b_visits->qual[d1.seq].c_diag_str
    FROM (dummyt d1  WITH seq = size(340b_visits->qual,5)),
     (dummyt d2  WITH seq = 1)
    PLAN (d1
     WHERE maxrec(d2,size(340b_visits->qual[d1.seq].diag,5)))
     JOIN (d2)
  ...
  ELSE
   ...
   IF (stat != 0)
    SET ms_output_string = notrim(check(concat("Date","|","MRNumber","|","Visit",
       "|","Location","|","LastName","|",
       "FirstName","|","DOB","|","ZipCode",
       "|","Facility","|","ICD","|",
       "Diagnosis1","|","Diagnosis2","|","Diagnosis3",
       "|","Diagnosis4","|","Diagnosis5","|",
       "Diagnosis6","|","Diagnosis7","|","Diagnosis8",
       "|","Diagnosis9","|","Diagnosis10","|",
       "Diagnosis11","|","Diagnosis12","|","Diagnosis13",
       "|","Diagnosis14","|","Diagnosis15","|",
       "Diagnosis16","|","Diagnosis17","|","Diagnosis18",
       "|","Diagnosis19","|","Diagnosis20","|",
       "Diagnosis21","|","Diagnosis22","|","Diagnosis23",
       "|","Diagnosis24")))
    SET frec->file_buf = build2(ms_output_string,ms_carriage_return,ms_line_feed)
    SET stat = cclio("WRITE",frec)
    FOR (ml_loop1 = 1 TO size(340b_visits->qual,5))
      IF (size(340b_visits->qual[ml_loop1].diag,5) > 0)
       SET ms_output_string = notrim(check(concat(trim(340b_visits->qual[ml_loop1].c_disch_date,3),
          "|",trim(340b_visits->qual[ml_loop1].c_mrn,3),"|",trim(340b_visits->qual[ml_loop1].c_fin,3),
          "|",trim(340b_visits->qual[ml_loop1].c_location,3),"|",trim(340b_visits->qual[ml_loop1].
           c_patientname_last,3),"|",
          trim(340b_visits->qual[ml_loop1].c_patientname_first,3),"|",trim(340b_visits->qual[ml_loop1
           ].c_dob,3),"|",trim(340b_visits->qual[ml_loop1].c_zipcode,3),
          "|",trim(340b_visits->qual[ml_loop1].c_facility,3),"|",trim(340b_visits->qual[ml_loop1].
           c_icdtype,3),"|")))
       FOR (ml_loop2 = 1 TO 24)
         IF ((ml_loop2 <= 340b_visits->qual[ml_loop1].l_diag_cnt))
          SET ms_output_string = build2(trim(ms_output_string,3),trim(340b_visits->qual[ml_loop1].
            diag[ml_loop2].c_diag,3),"|")
         ELSEIF ((((340b_visits->qual[ml_loop1].l_diag_cnt=0)) OR ((ml_loop2 > 340b_visits->qual[
         ml_loop1].l_diag_cnt))) )
          SET ms_output_string = build2(trim(ms_output_string,3))
         ENDIF
       ENDFOR
       SET frec->file_buf = notrim(concat(ms_output_string))
       SET stat = cclio("WRITE",frec)
       SET frec->file_buf = notrim(concat(ms_carriage_return,ms_line_feed))
       SET stat = cclio("WRITE",frec)
      ENDIF
    ENDFOR
   ENDIF
   ...
   IF (mn_is_ops_job != 1)
    SELECT INTO  $OUTDEV
     ...
     HEAD REPORT
      ...
      row 18, col 0,
      CALL print(build2("Total Rows:",size(340b_visits->qual,5))),
      row 22, col 0,
...
#exit_script
 FREE RECORD frec
 FREE RECORD 340b_visits
 SET last_mod = "001 04/25/2024 AM031915 BHS_MA 340B MacroFelix Encounter Visits Feed CCPS-300"
END GO
