CREATE PROGRAM dm2_gather_system_stats:dba
 ...
 ENDIF
 DECLARE dgss_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dgss_err_emsg = vc WITH protect, noconstant("")
 DECLARE dgss_err_eproc = vc WITH protect, noconstant("")
 DECLARE dgss_gather_params = vc WITH protect, noconstant("gathering_mode=>'INTERVAL',interval=>1440"
  )
 DECLARE dgss_gather_weekday = i2 WITH protect, noconstant(2)
 DECLARE dgss_gather_days = i2 WITH protect, noconstant(7)
 DECLARE dgss_gather_ind = i2 WITH protect, noconstant(0)
 DECLARE dgss_start_date = i4 WITH protect, noconstant(0)
 DECLARE dgss_dm_info_exists = i2 WITH protect, noconstant(0)
 DECLARE dgss_job_queue = i4 WITH protect, noconstant(0)
 DECLARE dgss_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dgss_reset_err_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_gather_sys_stats",".log","DM2_GATHER_SYSTEM_STATS LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_row(dpl_statistics,dpl_system_stats_gathering,dpl_no_prev_id,1)
 SET dgss_log_id = dm2_process_event_rs->dm_process_event_id
 SET dgss_err_ind = dm_err->err_ind
 SET dgss_err_emsg = dm_err->emsg
 SET dgss_err_eproc = dm_err->eproc
 SET dm_err->eproc = "Beginning DM2_GATHER_SYSTEM_STATS"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((validate(dm2_gather_anyway,- (1))=- (1))
  AND (validate(dm2_gather_anyway,- (2))=- (2)))
  DECLARE dm2_gather_anyway = i2 WITH protect, noconstant(0)
 ENDIF
 SET dm_err->eproc = "Determining if DM_INFO exists."
 IF (dm2_table_and_ccldef_exists("DM_INFO",dgss_dm_info_exists)=0)
  GO TO exit_script
 ENDIF
 IF (dgss_dm_info_exists=0)
  SET dm_err->emsg = "DM_INFO does not exist.  Skipping system stats gathering..."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Selecting next gather data from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2-RUNSTATS:SYSTEM-STATS"
   AND di.info_name="NEXT-GATHER"
  DETAIL
   dgss_gather_days = di.info_number, dgss_gather_weekday = di.info_long_id, dgss_gather_params = di
   .info_char
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inserting next gather dm_info row."
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2-RUNSTATS:SYSTEM-STATS", di.info_name = "NEXT-GATHER", di.info_number =
    dgss_gather_days,
    di.info_long_id = dgss_gather_weekday, di.info_char = dgss_gather_params
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Selecting info from sys.aux_stats$"
 SELECT INTO "nl:"
  FROM (sys.aux_stats$ sas)
  WHERE sas.sname="SYSSTATS_INFO"
  DETAIL
   CASE (build(sas.pname))
    OF "STATUS":
     IF (build(sas.pval2)="BADSTATS")
      dgss_gather_ind = 1
      IF ((dm_err->debug_flag > 0))
       CALL echo("BADSTATS present")
      ENDIF
     ENDIF
     ,
     IF (build(sas.pval2)="AUTOGATHERING")
      dgss_gather_ind = 0
      IF ((dm_err->debug_flag > 0))
       CALL echo("Skipping due to AUTOGATHERING")
      ENDIF
      CALL cancel(1)
     ENDIF
    OF "DSTART":
     dgss_start_date = cnvtdate2(substring(1,10,sas.pval2),"MM-DD-YYYY"),
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("Date of last system statistics start:",sas.pval2)),
      CALL echo(build("Numeric Date of last system statistics start:",dgss_start_date)),
      CALL echo(build("Numeric Date for today:",curdate)),
      CALL echo(build("Number of days between gathers:",dgss_gather_days)),
      CALL echo(build("Gather weekday (0 = Sunday):",dgss_gather_weekday)),
      CALL echo(build("Today's weekday:",weekday(curdate)))
     ENDIF
     ,
     IF (((dgss_start_date+ dgss_gather_days) <= curdate))
      dgss_gather_ind = 1
      IF ((dm_err->debug_flag > 0))
       CALL echo("Stat_date + Days_Between_Gather <= Today")
      ENDIF
     ELSEIF (dgss_start_date < curdate
      AND weekday(curdate)=dgss_gather_weekday)
      dgss_gather_ind = 1
      IF ((dm_err->debug_flag > 0))
       CALL echo("Stat_date < Today and Gather_Weekday = Todays_Weekday")
      ENDIF
     ENDIF
   ENDCASE
  WITH nocounter
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 IF (((dgss_gather_ind=1) OR (((dm2_gather_anyway=1) OR (curqual=0)) )) )
  SET dm_err->eproc = "Selecting job_queue_processes from v$parameter."
  SELECT INTO "nl:"
   FROM v$parameter vp
   WHERE vp.name="job_queue_processes"
   DETAIL
    dgss_job_queue = cnvtint(vp.value)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (dgss_job_queue < 1)
   SET dm_err->err_ind = 1
   SET dm_err->emsg =
   "Cannot issue DBMS_STATS.GATHER_SYSTEM_STATS because job_queue_processes is < 1"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  IF (dm2_push_cmd(build('RDB ASIS(" begin DBMS_STATS.GATHER_SYSTEM_STATS(',dgss_gather_params,
    '); end;") go'),1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
#exit_script
 ...
 SET dgss_reset_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_system_stats_gathering,dgss_log_id,1)
 SET dm_err->err_ind = dgss_reset_err_ind
 SET dm_err->eproc = "Ending DM2_GATHER_SYSTEM_STATS."
 CALL final_disp_msg("dm2_gather_sys_stats")
 SET dm_err->err_ind = dgss_err_ind
 SET dm_err->emsg = dgss_err_emsg
 SET dm_err->eproc = dgss_err_eproc
