CREATE PROGRAM dm2_assign_tspace:dba
 ...
 SUBROUTINE drr_chk_active_runners(dcar_dblink,dcar_count_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_assign_tspace",".log","DM2_ASSIGN_TSPACE LOG FILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "BEGIN DM2_ASSIGN_TSPACE"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE mac_tspace_type = c1 WITH public
 DECLARE tbl_index_idx = i4 WITH public
 DECLARE indx = i4 WITH public
 DECLARE ddtsp_idx = i4 WITH public
 DECLARE dat_lp = i4 WITH public, noconstant(1)
 DECLARE dat_lp1 = i4 WITH public, noconstant(1)
 DECLARE dat_cs_idx = i4 WITH public, noconstant(1)
 DECLARE dat_tgt_idx = i4 WITH protect, noconstant(0)
 DECLARE dat_tmp_tsp_idx = i4 WITH protect, noconstant(0)
 SET das_dtp->dtp_cnt = 0
 FREE RECORD tbl_index
 RECORD tbl_index(
   1 tcnt = i2
   1 max_tspace = vc
   1 max_cnt = i2
 ...
 SET tbl_index->tcnt = 0
 SET tbl_index->max_cnt = 0
 FREE RECORD das_ntp
 RECORD das_ntp(
   1 cnt = i4
   1 qual[*]
 ...
 SET dat_tsp_map->map_cnt = 0
 DECLARE dat_add_ntp(dan_seq=i4,dan_type=c1) = i4
 DECLARE dat_chk_prec(dcp_type=c1,dcp_seq=i4) = i4
 DECLARE dm2_assign_tspace_pass1(sbr_tgt_idx1=i4) = i4
 DECLARE dm2_assign_tspace_pass2(sbr_tgt_idx2=i4,sbr_tspace_type=c1) = i4
 DECLARE debug_disp_tgtsch(null) = null
 DECLARE dat_find_max_itspace(dfm_type=c1,dfm_t=i4) = vc
 SET dm_err->eproc = "LOAD TABLESPACE PRECEDENCE INTO MEMORY"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_set_autocommit(1)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Loading tablespace mappings."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  ...
  WHERE d.info_domain="DM2_TABLESPACE_MAPPING"
  DETAIL
   dat_tsp_map->map_cnt = (dat_tsp_map->map_cnt+ 1)
   IF (mod(dat_tsp_map->map_cnt,100)=1)
    stat = alterlist(dat_tsp_map->mapping,(dat_tsp_map->map_cnt+ 99))
   ENDIF
   dat_tsp_map->mapping[dat_tsp_map->map_cnt].from_tsp = d.info_name, dat_tsp_map->mapping[
   dat_tsp_map->map_cnt].to_tsp = d.info_char
  FOOT REPORT
   stat = alterlist(dat_tsp_map->mapping,dat_tsp_map->map_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 IF ((dm2_install_schema->dbase_type != "ADMIN"))
  SELECT INTO "nl:"
   d.table_name
   FROM dm_ts_precedence d
   WHERE d.precedence > 0
   ORDER BY d.table_name, d.precedence
   HEAD d.table_name
    das_dtp->dtp_cnt = (das_dtp->dtp_cnt+ 1), stat = alterlist(das_dtp->qual,das_dtp->dtp_cnt),
    das_dtp->qual[das_dtp->dtp_cnt].tname = d.table_name,
    das_dtp->qual[das_dtp->dtp_cnt].prec_cnt = 0
   DETAIL
    das_dtp->qual[das_dtp->dtp_cnt].prec_cnt = (das_dtp->qual[das_dtp->dtp_cnt].prec_cnt+ 1),
    dtp_pcnt = das_dtp->qual[das_dtp->dtp_cnt].prec_cnt, stat = alterlist(das_dtp->qual[das_dtp->
     dtp_cnt].prec,dtp_pcnt),
    das_dtp->qual[das_dtp->dtp_cnt].prec[dtp_pcnt].precedence = d.precedence, das_dtp->qual[das_dtp->
    dtp_cnt].prec[dtp_pcnt].data_tspace = trim(d.data_tablespace,3), das_dtp->qual[das_dtp->dtp_cnt].
    prec[dtp_pcnt].ind_tspace = trim(d.index_tablespace,3),
    das_dtp->qual[das_dtp->dtp_cnt].prec[dtp_pcnt].long_tspace = trim(d.long_tablespace,3), das_dtp->
    qual[das_dtp->dtp_cnt].prec[dtp_pcnt].data_extent_size = d.data_extent_size, das_dtp->qual[
    das_dtp->dtp_cnt].prec[dtp_pcnt].index_extent_size = d.index_extent_size
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (dtr_load_tspaces(dm2_install_schema->process_option)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "ASSIGN TABLESPACES FOR TABLES THAT ARE NEW OR HAVE SCHEMA DIFFERENCES"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((tgtsch->tbl_cnt=0))
  GO TO exit_script
 ENDIF
 IF ((tgtsch->diff_ind=1))
  FOR (dat_lp = 1 TO tgtsch->tbl_cnt)
    IF ((((tgtsch->tbl[dat_lp].diff_ind=1)) OR ((tgtsch->tbl[dat_lp].new_ind=1))) )
     SET tgtsch->tbl[dat_lp].dext_mgmt = dm2_db_options->new_tspace_type
     SET tgtsch->tbl[dat_lp].iext_mgmt = dm2_db_options->new_tspace_type
     SET tgtsch->tbl[dat_lp].lext_mgmt = dm2_db_options->new_tspace_type
     SET tgtsch->tbl[dat_lp].itspace_new_ind = 1
     SET tgtsch->tbl[dat_lp].ttspace_new_ind = 1
     SET tgtsch->tbl[dat_lp].ltspace_new_ind = 1
     CALL dm2_assign_tspace_pass1(dat_lp)
     IF ((dm_err->err_ind=1))
      GO TO exit_script
     ENDIF
     IF ((tgtsch->tbl[dat_lp].ttsp_set_ind=0))
      CALL dm2_assign_tspace_pass2(dat_lp,"D")
      IF ((dm_err->err_ind=1))
       GO TO exit_script
      ENDIF
      IF ((((tgtsch->tbl[dat_lp].ttsp_set_ind=0)) OR (trim(tgtsch->tbl[dat_lp].tspace_name)="")) )
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat("Could not find data tablespace ",tgtsch->tbl[dat_lp].tspace_name,
        " for table ",tgtsch->tbl[dat_lp].tbl_name," from DM_TS_PRECEDENCE")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_script
      ENDIF
     ENDIF
     IF ((tgtsch->tbl[dat_lp].itsp_set_ind=0))
      CALL dm2_assign_tspace_pass2(dat_lp,"I")
      IF ((dm_err->err_ind=1))
       GO TO exit_script
      ENDIF
      IF ((((tgtsch->tbl[dat_lp].itsp_set_ind=0)) OR (trim(tgtsch->tbl[dat_lp].ind_tspace)="")) )
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat("Could not find index tablespace ",tgtsch->tbl[dat_lp].ind_tspace,
        " for table ",tgtsch->tbl[dat_lp].tbl_name," from DM_TS_PRECEDENCE")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_script
      ENDIF
     ENDIF
     IF (currdb != "SQLSRV")
      IF ((tgtsch->tbl[dat_lp].ltsp_set_ind=0))
       CALL dm2_assign_tspace_pass2(dat_lp,"L")
       IF ((dm_err->err_ind=1))
        GO TO exit_script
       ENDIF
       IF ((((tgtsch->tbl[dat_lp].ltsp_set_ind=0)) OR (trim(tgtsch->tbl[dat_lp].long_tspace)="")) )
        SET dm_err->err_ind = 1
        SET dm_err->emsg = concat("Could not find lob tablespace ",tgtsch->tbl[dat_lp].long_tspace,
         " for table ",tgtsch->tbl[dat_lp].tbl_name," from DM_TS_PRECEDENCE")
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        GO TO exit_script
       ENDIF
      ENDIF
     ENDIF
     SET dat_tmp_tsp_idx = locateval(dat_tmp_tsp_idx,1,dat_tsp_map->map_cnt,tgtsch->tbl[dat_lp].
      tspace_name,dat_tsp_map->mapping[dat_tmp_tsp_idx].from_tsp)
     IF (dat_tmp_tsp_idx > 0)
      SET tgtsch->tbl[dat_lp].tspace_name = dat_tsp_map->mapping[dat_tmp_tsp_idx].to_tsp
      SET tgtsch->tbl[dat_lp].ttspace_assignment_choice = "CUSTOM TABLESPACE MAPPING"
     ENDIF
     SET dat_tmp_tsp_idx = locateval(dat_tmp_tsp_idx,1,dat_tsp_map->map_cnt,tgtsch->tbl[dat_lp].
      ind_tspace,dat_tsp_map->mapping[dat_tmp_tsp_idx].from_tsp)
     IF (dat_tmp_tsp_idx > 0)
      SET tgtsch->tbl[dat_lp].ind_tspace = dat_tsp_map->mapping[dat_tmp_tsp_idx].to_tsp
      SET tgtsch->tbl[dat_lp].itspace_assignment_choice = "CUSTOM TABLESPACE MAPPING"
     ENDIF
     SET dat_tmp_tsp_idx = locateval(dat_tmp_tsp_idx,1,dat_tsp_map->map_cnt,tgtsch->tbl[dat_lp].
      long_tspace,dat_tsp_map->mapping[dat_tmp_tsp_idx].from_tsp)
     IF (dat_tmp_tsp_idx > 0)
      SET tgtsch->tbl[dat_lp].long_tspace = dat_tsp_map->mapping[dat_tmp_tsp_idx].to_tsp
      SET tgtsch->tbl[dat_lp].ltspace_assignment_choice = "CUSTOM TABLESPACE MAPPING"
     ENDIF
     SET ddtsp_idx = 0
     SET ddtsp_idx = dtr_find_tspace(tgtsch->tbl[dat_lp].tspace_name)
     IF (ddtsp_idx > 0)
      SET tgtsch->tbl[dat_lp].dext_mgmt = ddtsp->qual[ddtsp_idx].ext_mgmt
      SET tgtsch->tbl[dat_lp].ttspace_new_ind = 0
      CALL dtr_eval_nonstd_tgt_tspace(ddtsp_idx)
      IF ((dm_err->err_ind=1))
       GO TO exit_script
      ENDIF
     ELSE
      IF (dat_add_ntp(dat_lp,"D")=0)
       GO TO exit_script
      ENDIF
     ENDIF
     SET ddtsp_idx = 0
     SET ddtsp_idx = dtr_find_tspace(tgtsch->tbl[dat_lp].ind_tspace)
     IF (ddtsp_idx > 0)
      SET tgtsch->tbl[dat_lp].iext_mgmt = ddtsp->qual[ddtsp_idx].ext_mgmt
      SET tgtsch->tbl[dat_lp].itspace_new_ind = 0
      CALL dtr_eval_nonstd_tgt_tspace(ddtsp_idx)
      IF ((dm_err->err_ind=1))
       GO TO exit_script
      ENDIF
     ELSE
      IF (dat_add_ntp(dat_lp,"I")=0)
       GO TO exit_script
      ENDIF
     ENDIF
     SET ddtsp_idx = 0
     SET ddtsp_idx = dtr_find_tspace(tgtsch->tbl[dat_lp].long_tspace)
     IF (ddtsp_idx > 0)
      SET tgtsch->tbl[dat_lp].lext_mgmt = ddtsp->qual[ddtsp_idx].ext_mgmt
      SET tgtsch->tbl[dat_lp].ltspace_new_ind = 0
      CALL dtr_eval_nonstd_tgt_tspace(ddtsp_idx)
      IF ((dm_err->err_ind=1))
       GO TO exit_script
      ENDIF
     ELSE
      IF (dat_add_ntp(dat_lp,"L")=0)
       GO TO exit_script
      ENDIF
     ENDIF
     IF ((dm_err->err_ind=1))
 ...
 IF ((dm2_install_schema->dbase_type != "ADMIN")
  AND (das_ntp->cnt > 0))
  FOR (dat_lp1 = 1 TO tgtsch->tbl_cnt)
   SET dat_cs_idx = tgtsch->tbl[dat_lp1].cur_idx
   IF ((tgtsch->tbl[dat_lp1].new_ind=1))
    IF (dat_chk_prec("D",dat_lp1)=0)
     GO TO exit_script
    ENDIF
    IF (dat_chk_prec("I",dat_lp1)=0)
     GO TO exit_script
    ENDIF
    IF (dat_chk_prec("L",dat_lp1)=0)
     GO TO exit_script
    ENDIF
   ELSEIF ((tgtsch->tbl[dat_lp1].diff_ind=1)
    AND (tgtsch->tbl[dat_lp1].ind_cnt > 0)
    AND (cur_sch->tbl[dat_cs_idx].ind_cnt=0))
    IF (dat_chk_prec("I",dat_lp1)=0)
     GO TO exit_script
    ENDIF
   ENDIF
 ...
 IF ((tgtsch->diff_ind=1))
  ...
  FOR (dat_lp = 1 TO tgtsch->tbl_cnt)
   IF ((dm_err->debug_flag >= 5))
    CALL echo(concat("Propagating ind_tspace and iext_mgmt for ",tgtsch->tbl[dat_lp].tbl_name))
   ENDIF
   FOR (dat_tgt_idx = 1 TO tgtsch->tbl[dat_lp].ind_cnt)
     IF ((dm_err->debug_flag >= 5))
      CALL echo(concat("processing index: ",tgtsch->tbl[dat_lp].ind[dat_tgt_idx].ind_name))
      CALL echo(concat("Before::",tgtsch->tbl[dat_lp].ind[dat_tgt_idx].tspace_name,"::",tgtsch->tbl[
        dat_lp].ind[dat_tgt_idx].ext_mgmt))
     ENDIF
     SET tgtsch->tbl[dat_lp].ind[dat_tgt_idx].tspace_name = tgtsch->tbl[dat_lp].ind_tspace
     SET tgtsch->tbl[dat_lp].ind[dat_tgt_idx].ext_mgmt = tgtsch->tbl[dat_lp].iext_mgmt
     IF ((dm_err->debug_flag >= 5))
      CALL echo(concat("After::",tgtsch->tbl[dat_lp].ind[dat_tgt_idx].tspace_name,"::",tgtsch->tbl[
        dat_lp].ind[dat_tgt_idx].ext_mgmt))
     ENDIF
   ENDFOR
  ENDFOR
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL debug_disp_tgtsch(null)
 ENDIF
 SUBROUTINE dm2_assign_tspace_pass1(sbr_tgt_idx1)
   DECLARE cs_idx = i4 WITH private, noconstant(0)
   DECLARE icnt2 = i4 WITH protect
   DECLARE dat_lmt_fnd = i2 WITH protect, noconstant(0)
   DECLARE dat_ext_mgmt = c1 WITH protect, noconstant(" ")
   SET dm_err->eproc = "Assign tablespace1"
   IF ((tgtsch->tbl[sbr_tgt_idx1].cur_idx > 0))
    SET cs_idx = tgtsch->tbl[sbr_tgt_idx1].cur_idx
    SET tgtsch->tbl[sbr_tgt_idx1].tspace_name = cur_sch->tbl[cs_idx].tspace_name
    SET tgtsch->tbl[sbr_tgt_idx1].ttsp_set_ind = 1
    IF (currdb="DB2UDB")
     SET tgtsch->tbl[sbr_tgt_idx1].ind_tspace = cur_sch->tbl[cs_idx].ind_tspace
     SET tgtsch->tbl[sbr_tgt_idx1].itsp_set_ind = 1
     SET tgtsch->tbl[sbr_tgt_idx1].long_tspace = cur_sch->tbl[cs_idx].long_tspace
     SET tgtsch->tbl[sbr_tgt_idx1].ltsp_set_ind = 1
    ELSEIF (currdb="ORACLE")
     IF ((cur_sch->tbl[cs_idx].ind_cnt > 0))
      SET dat_lmt_fnd = 0
      SET dat_lmt_fnd = locateval(dat_lmt_fnd,1,cur_sch->tbl[cs_idx].ind_cnt,"L",cur_sch->tbl[cs_idx]
       .ind[dat_lmt_fnd].ext_mgmt)
      IF (dat_lmt_fnd=0)
       SET dat_ext_mgmt = "D"
      ELSE
       IF ((dm_err->debug_flag > 1))
        CALL echo(concat("LMT found =",cur_sch->tbl[cs_idx].ind[dat_lmt_fnd].tspace_name))
       ENDIF
       SET dat_ext_mgmt = "L"
      ENDIF
      SET tgtsch->tbl[sbr_tgt_idx1].ind_tspace = dat_find_max_itspace(dat_ext_mgmt,cs_idx)
      IF ((tgtsch->tbl[sbr_tgt_idx1].ind_tspace > " "))
       SET tgtsch->tbl[sbr_tgt_idx1].itsp_set_ind = 1
       SET tgtsch->tbl[sbr_tgt_idx1].itspace_assignment_choice = "MOST POPULAR"
       IF ((dm_err->debug_flag > 1))
        CALL echo(concat("index Tspace =",tgtsch->tbl[sbr_tgt_idx1].ind_tspace))
       ENDIF
      ELSE
       SET dm_err->eproc =
       "Warning Message:  Unable to determine new index tablespace from existing indexes."
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
     ENDIF
    ELSEIF (currdb="SQLSRV")
     IF ((cur_sch->tbl[cs_idx].ind_cnt > 0))
      SET tbl_index->tcnt = 0
      SET tbl_index->max_cnt = 0
      SET tbl_index->max_tspace = ""
      SET stat = alterlist(tbl_index->qual,tbl_index->tcnt)
      FOR (icnt = 1 TO cur_sch->tbl[cs_idx].ind_cnt)
        IF (substring(1,2,cur_sch->tbl[cs_idx].ind[icnt].tspace_name) != "D_")
         SET tbl_index_idx = 0
         SET tbl_index_idx = locateval(indx,1,tbl_index->tcnt,cur_sch->tbl[cs_idx].ind[icnt].
          tspace_name,tbl_index->qual[indx].ind_tspace)
         IF (tbl_index_idx=0)
          SET tbl_index->tcnt = (tbl_index->tcnt+ 1)
          SET stat = alterlist(tbl_index->qual,tbl_index->tcnt)
          SET tbl_index->qual[tbl_index->tcnt].ind_tspace = cur_sch->tbl[cs_idx].ind[icnt].
          tspace_name
          SET tbl_index->qual[tbl_index->tcnt].ind_cnt = 1
         ELSE
          SET tbl_index->qual[tbl_index_idx].ind_cnt = (tbl_index->qual[tbl_index_idx].ind_cnt+ 1)
         ENDIF
        ENDIF
      ENDFOR
      SET tbl_index->max_cnt = 0
      FOR (icnt2 = 1 TO tbl_index->tcnt)
        IF ((tbl_index->qual[icnt2].ind_cnt > tbl_index->max_cnt))
         SET tbl_index->max_cnt = tbl_index->qual[icnt2].ind_cnt
         SET tbl_index->max_tspace = tbl_index->qual[icnt2].ind_tspace
        ENDIF
      ENDFOR
      IF ((tbl_index->max_tspace > " "))
       SET tgtsch->tbl[sbr_tgt_idx1].ind_tspace = tbl_index->max_tspace
       SET tgtsch->tbl[sbr_tgt_idx1].itsp_set_ind = 1
       SET tgtsch->tbl[sbr_tgt_idx1].itspace_assignment_choice = "MOST POPULAR"
      ELSE
       SET dm_err->eproc =
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dm2_assign_tspace_pass2(sbr_tgt_idx2,sbr_tspace_type)
   DECLARE gt_set_ind = i2 WITH private, noconstant(0)
   DECLARE mac_cnt1 = i2 WITH private, noconstant(1)
   DECLARE gt_tspace_name = vc WITH private, noconstant(" ")
   DECLARE gt_temp_tspace = vc WITH private, noconstant(" ")
   DECLARE das_dtp_idx = i4 WITH private, noconstant(0)
   SET dm_err->eproc = "Assign tablespace2"
   IF ((dm2_install_schema->dbase_type="ADMIN"))
    CASE (sbr_tspace_type)
     OF "D":
      SET tgtsch->tbl[sbr_tgt_idx2].tspace_name = "D_TOOLKIT"
      SET tgtsch->tbl[sbr_tgt_idx2].ttsp_set_ind = 1
     OF "I":
      SET tgtsch->tbl[sbr_tgt_idx2].ind_tspace = "I_TOOLKIT"
      SET tgtsch->tbl[sbr_tgt_idx2].itsp_set_ind = 1
     OF "L":
      SET tgtsch->tbl[sbr_tgt_idx2].long_tspace = "L_TOOLKIT"
      SET tgtsch->tbl[sbr_tgt_idx2].ltsp_set_ind = 1
    ENDCASE
   ELSE
    SET das_dtp_idx = locateval(das_dtp_idx,1,das_dtp->dtp_cnt,tgtsch->tbl[sbr_tgt_idx2].tbl_name,
     das_dtp->qual[das_dtp_idx].tname)
    IF (das_dtp_idx=0)
     RETURN(0)
    ENDIF
    WHILE ((mac_cnt1 <= das_dtp->qual[das_dtp_idx].prec_cnt)
     AND gt_set_ind=0)
      CASE (sbr_tspace_type)
       OF "D":
        SET gt_temp_tspace = das_dtp->qual[das_dtp_idx].prec[mac_cnt1].data_tspace
        SET gt_set_ind = 1
       OF "I":
        SET gt_temp_tspace = das_dtp->qual[das_dtp_idx].prec[mac_cnt1].ind_tspace
        SET gt_set_ind = 1
       OF "L":
        SET gt_temp_tspace = das_dtp->qual[das_dtp_idx].prec[mac_cnt1].long_tspace
        SET gt_set_ind = 1
      ENDCASE
      SET cur_tspace_idx = dtr_find_tspace(gt_temp_tspace)
      IF (cur_tspace_idx > 0)
       SET gt_set_ind = 1
       SET gt_tspace_name = gt_temp_tspace
      ELSE
       SET gt_set_ind = 0
      ENDIF
      SET mac_cnt1 = (mac_cnt1+ 1)
    ENDWHILE
    IF (gt_set_ind=0
     AND (das_dtp->qual[das_dtp_idx].prec_cnt > 0))
     CASE (sbr_tspace_type)
      OF "D":
       SET gt_tspace_name = das_dtp->qual[das_dtp_idx].prec[1].data_tspace
       SET gt_set_ind = 1
      OF "I":
       SET gt_tspace_name = das_dtp->qual[das_dtp_idx].prec[1].ind_tspace
       SET gt_set_ind = 1
      OF "L":
       SET gt_tspace_name = das_dtp->qual[das_dtp_idx].prec[1].long_tspace
       SET gt_set_ind = 1
     ENDCASE
    ENDIF
    CASE (sbr_tspace_type)
     OF "D":
      SET tgtsch->tbl[sbr_tgt_idx2].tspace_name = gt_tspace_name
      SET tgtsch->tbl[sbr_tgt_idx2].ttsp_set_ind = gt_set_ind
      SET tgtsch->tbl[sbr_tgt_idx2].ttspace_assignment_choice = "TABLESPACE PRECEDENCE"
     OF "I":
      SET tgtsch->tbl[sbr_tgt_idx2].ind_tspace = gt_tspace_name
      SET tgtsch->tbl[sbr_tgt_idx2].itsp_set_ind = gt_set_ind
      SET tgtsch->tbl[sbr_tgt_idx2].itspace_assignment_choice = "TABLESPACE PRECEDENCE"
     OF "L":
      SET tgtsch->tbl[sbr_tgt_idx2].long_tspace = gt_tspace_name
      SET tgtsch->tbl[sbr_tgt_idx2].ltsp_set_ind = gt_set_ind
      SET tgtsch->tbl[sbr_tgt_idx2].ltspace_assignment_choice = "TABLESPACE PRECEDENCE"
    ENDCASE
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dat_add_ntp(dan_seq,dan_type)
   SET dm_err->eproc = "Add new tablespace name to record das_ntp..."
   DECLARE dan_fnd = i4 WITH protect, noconstant(0)
   CASE (dan_type)
    OF "D":
     SET dan_fnd = locateval(dan_fnd,1,das_ntp->cnt,tgtsch->tbl[dan_seq].tspace_name,das_ntp->qual[
      dan_fnd].tspace_name)
     IF (dan_fnd=0)
      SET das_ntp->cnt = (das_ntp->cnt+ 1)
      SET stat = alterlist(das_ntp->qual,das_ntp->cnt)
      SET das_ntp->qual[das_ntp->cnt].tspace_name = tgtsch->tbl[dan_seq].tspace_name
     ENDIF
    OF "I":
     SET dan_fnd = locateval(dan_fnd,1,das_ntp->cnt,tgtsch->tbl[dan_seq].ind_tspace,das_ntp->qual[
      dan_fnd].tspace_name)
     IF (dan_fnd=0)
      SET das_ntp->cnt = (das_ntp->cnt+ 1)
      SET stat = alterlist(das_ntp->qual,das_ntp->cnt)
      SET das_ntp->qual[das_ntp->cnt].tspace_name = tgtsch->tbl[dan_seq].ind_tspace
     ENDIF
    OF "L":
     SET dan_fnd = locateval(dan_fnd,1,das_ntp->cnt,tgtsch->tbl[dan_seq].long_tspace,das_ntp->qual[
      dan_fnd].tspace_name)
     IF (dan_fnd=0)
      SET das_ntp->cnt = (das_ntp->cnt+ 1)
      SET stat = alterlist(das_ntp->qual,das_ntp->cnt)
      SET das_ntp->qual[das_ntp->cnt].tspace_name = tgtsch->tbl[dan_seq].long_tspace
     ENDIF
   ENDCASE
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dat_chk_prec(dcp_type,dcp_seq)
   SET dm_err->eproc = concat("Check if tablespace assigned to table ",tgtsch->tbl[dcp_seq].tbl_name,
    " has correct precedence.")
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   ENDIF
   DECLARE dcp_idx = i4 WITH protect, noconstant(0)
   DECLARE dcp_prec = i4 WITH protect, noconstant(0)
   DECLARE dcp_fnd = i4 WITH protect, noconstant(0)
   SET dcp_idx = locateval(dcp_idx,1,das_dtp->dtp_cnt,tgtsch->tbl[dcp_seq].tbl_name,das_dtp->qual[
    dcp_idx].tname)
   IF (dcp_idx=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Can not find table ",tgtsch->tbl[dcp_seq].tbl_name,
     " in table DM_TS_PRECEDENCE")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (dcp_type="D")
    FOR (dcp_prec = 1 TO das_dtp->qual[dcp_idx].prec_cnt)
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("tgt_ttspace =",tgtsch->tbl[dcp_seq].tspace_name))
      CALL echo(build("das_dtp_ttspace =",das_dtp->qual[dcp_idx].prec[dcp_prec].data_tspace))
     ENDIF
     IF ((tgtsch->tbl[dcp_seq].tspace_name=das_dtp->qual[dcp_idx].prec[dcp_prec].data_tspace))
      RETURN(1)
     ELSE
      SET dcp_fnd = 0
      SET dcp_fnd = locateval(dcp_fnd,1,das_ntp->cnt,das_dtp->qual[dcp_idx].prec[dcp_prec].
       data_tspace,das_ntp->qual[dcp_fnd].tspace_name)
      IF (dcp_fnd > 0)
       IF ((dm_err->debug_flag > 0))
        CALL echo(concat("Change table tablespace from ",tgtsch->tbl[dcp_seq].tspace_name," to ",
          das_dtp->qual[dcp_idx].prec[dcp_prec].data_tspace))
       ENDIF
       SET tgtsch->tbl[dcp_seq].tspace_name = das_dtp->qual[dcp_idx].prec[dcp_prec].data_tspace
       SET tgtsch->tbl[dcp_seq].ttspace_new_ind = 1
       SET tgtsch->tbl[dcp_seq].dext_mgmt = dm2_db_options->new_tspace_type
       SET dcp_prec = das_dtp->qual[dcp_idx].prec_cnt
      ENDIF
     ENDIF
    ENDFOR
   ELSEIF (dcp_type="I")
    FOR (dcp_prec = 1 TO das_dtp->qual[dcp_idx].prec_cnt)
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("tgt_itspace =",tgtsch->tbl[dcp_seq].ind_tspace))
      CALL echo(build("das_dtp_itspace =",das_dtp->qual[dcp_idx].prec[dcp_prec].ind_tspace))
     ENDIF
     IF ((tgtsch->tbl[dcp_seq].ind_tspace=das_dtp->qual[dcp_idx].prec[dcp_prec].ind_tspace))
      RETURN(1)
     ELSE
      SET dcp_fnd = 0
      SET dcp_fnd = locateval(dcp_fnd,1,das_ntp->cnt,das_dtp->qual[dcp_idx].prec[dcp_prec].ind_tspace,
       das_ntp->qual[dcp_fnd].tspace_name)
      IF (dcp_fnd > 0)
       IF ((dm_err->debug_flag > 0))
        CALL echo(concat("Change index tablespace from ",tgtsch->tbl[dcp_seq].ind_tspace," to ",
          das_dtp->qual[dcp_idx].prec[dcp_prec].ind_tspace))
       ENDIF
       SET tgtsch->tbl[dcp_seq].ind_tspace = das_dtp->qual[dcp_idx].prec[dcp_prec].ind_tspace
       SET tgtsch->tbl[dcp_seq].itspace_new_ind = 1
       SET tgtsch->tbl[dcp_seq].iext_mgmt = dm2_db_options->new_tspace_type
       SET dcp_prec = das_dtp->qual[dcp_idx].prec_cnt
      ENDIF
     ENDIF
    ENDFOR
   ELSEIF (dcp_type="L")
    FOR (dcp_prec = 1 TO das_dtp->qual[dcp_idx].prec_cnt)
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("tgt_ltspace =",tgtsch->tbl[dcp_seq].long_tspace))
      CALL echo(build("das_dtp_ltspace =",das_dtp->qual[dcp_idx].prec[dcp_prec].long_tspace))
     ENDIF
     IF ((tgtsch->tbl[dcp_seq].long_tspace=das_dtp->qual[dcp_idx].prec[dcp_prec].long_tspace))
      RETURN(1)
     ELSE
      SET dcp_fnd = 0
      SET dcp_fnd = locateval(dcp_fnd,1,das_ntp->cnt,das_dtp->qual[dcp_idx].prec[dcp_prec].
       long_tspace,das_ntp->qual[dcp_fnd].tspace_name)
      IF (dcp_fnd > 0)
       IF ((dm_err->debug_flag > 0))
        CALL echo(concat("Change lob tablespace from ",tgtsch->tbl[dcp_seq].long_tspace," to ",
          das_dtp->qual[dcp_idx].prec[dcp_prec].long_tspace))
       ENDIF
       SET tgtsch->tbl[dcp_seq].long_tspace = das_dtp->qual[dcp_idx].prec[dcp_prec].long_tspace
       SET tgtsch->tbl[dcp_seq].ltspace_new_ind = 1
       SET tgtsch->tbl[dcp_seq].lext_mgmt = dm2_db_options->new_tspace_type
       SET dcp_prec = das_dtp->qual[dcp_idx].prec_cnt
      ENDIF
     ENDIF
    ENDFOR
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dat_find_max_itspace(dfm_type,dfm_t)
   DECLARE itspace_found = i2 WITH protect, noconstant(0)
   DECLARE dfm_icnt = i4 WITH protect, noconstant(0)
   DECLARE dfm_icnt2 = i4 WITH protect, noconstant(0)
   SET tbl_index->tcnt = 0
   SET tbl_index->max_cnt = 0
   SET tbl_index->max_tspace = ""
   SET stat = alterlist(tbl_index->qual,tbl_index->tcnt)
   IF ((dm_err->debug_flag > 1))
    CALL echo(concat("EXT_MGMT =",dfm_type))
   ENDIF
   FOR (dfm_icnt = 1 TO cur_sch->tbl[dfm_t].ind_cnt)
    IF ((dm_err->debug_flag > 1))
     CALL echo(concat("index_name =",cur_sch->tbl[dfm_t].ind[dfm_icnt].ind_name))
     CALL echo(concat("itspace_name =",cur_sch->tbl[dfm_t].ind[dfm_icnt].tspace_name))
     CALL echo(concat("iext_mgmt =",cur_sch->tbl[dfm_t].ind[dfm_icnt].ext_mgmt))
    ENDIF
    IF ((cur_sch->tbl[dfm_t].ind[dfm_icnt].ext_mgmt=dfm_type))
     SET itspace_found = 0
     SET dfm_icnt2 = 1
     WHILE ((dfm_icnt2 <= tbl_index->tcnt)
      AND itspace_found=0)
      IF ((tbl_index->qual[dfm_icnt2].ind_tspace=cur_sch->tbl[dfm_t].ind[dfm_icnt].tspace_name))
       SET tbl_index->qual[dfm_icnt2].ind_cnt = (tbl_index->qual[dfm_icnt2].ind_cnt+ 1)
       IF ((tbl_index->qual[dfm_icnt2].ind_cnt > tbl_index->max_cnt))
        SET tbl_index->max_cnt = tbl_index->qual[dfm_icnt2].ind_cnt
        SET tbl_index->max_tspace = tbl_index->qual[dfm_icnt2].ind_tspace
       ENDIF
       SET itspace_found = 1
      ENDIF
      SET dfm_icnt2 = (dfm_icnt2+ 1)
     ENDWHILE
     IF (itspace_found=0)
      SET tbl_index->tcnt = (tbl_index->tcnt+ 1)
      SET stat = alterlist(tbl_index->qual,tbl_index->tcnt)
      SET tbl_index->qual[tbl_index->tcnt].ind_tspace = cur_sch->tbl[dfm_t].ind[dfm_icnt].tspace_name
      SET tbl_index->qual[tbl_index->tcnt].ind_cnt = (tbl_index->qual[tbl_index->tcnt].ind_cnt+ 1)
      IF ((tbl_index->tcnt=1))
       SET tbl_index->max_cnt = 1
       SET tbl_index->max_tspace = tbl_index->qual[tbl_index->tcnt].ind_tspace
      ENDIF
     ENDIF
    ENDIF
   ENDFOR
   RETURN(tbl_index->max_tspace)
 END ;Subroutine
 SUBROUTINE debug_disp_tgtsch(null)
   DECLARE ddt_log = vc
   SET ddt_log = build(dm2_install_schema->schema_prefix,dm2_install_schema->file_prefix,
    "_assignTsp.log")
   SELECT INTO value(ddt_log)
    d.seq
    ...
    DETAIL
     ...
     "data tablespace assignment choice = ", tgtsch->tbl[d.seq].ttspace_assignment_choice, row + 1,
     "index tablespace assignment choice = ", tgtsch->tbl[d.seq].itspace_assignment_choice, row + 1,
     "long tablespace assignment choice = ", tgtsch->tbl[d.seq].ltspace_assignment_choice, row + 1,
     "new ind = ", tgtsch->tbl[d.seq].new_ind, row + 1,
     "diff ind = ", tgtsch->tbl[d.seq].diff_ind
    WITH nocounter
   ;end select
   SELECT INTO value(ddt_log)
    d.seq
    FROM (dummyt d  WITH seq = value(das_dtp->dtp_cnt)),
     (dummyt dp  WITH seq = 100)
    PLAN (d)
     JOIN (dp
     WHERE (dp.seq <= das_dtp->qual[d.seq].prec_cnt))
    HEAD REPORT
     "table count = ", das_dtp->dtp_cnt, row + 1,
     "DAS_DTP"
    DETAIL
     row + 1, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", row + 1,
     "table number = ", d.seq, row + 1,
     "table_name = ", das_dtp->qual[d.seq].tname, row + 1,
     "data tablespace = ", das_dtp->qual[d.seq].prec[dp.seq].data_tspace, row + 1,
     "index tablespace = ", das_dtp->qual[d.seq].prec[dp.seq].ind_tspace, row + 1,
     "long tablespace = ", das_dtp->qual[d.seq].prec[dp.seq].long_tspace, row + 1,
     "data extent size = ", das_dtp->qual[d.seq].prec[dp.seq].data_extent_size, row + 1,
     "index extent size = ", das_dtp->qual[d.seq].prec[dp.seq].index_extent_size
    WITH nocounter, append
   ;end select
   IF ((dir_ui_misc->dm_process_event_id > 0))
    CALL drr_assign_file_to_installs("RPT:ASSIGNTSPACE",ddt_log,dir_ui_misc->dm_process_event_id)
   ENDIF
 END ;Subroutine
#exit_script
 SET stat = alterlist(tbl_index->qual,0)
 FREE RECORD tbl_index
 SET stat = alterlist(das_ntp->qual,0)
 FREE RECORD das_ntp
 SET stat = alterlist(dat_tsp_map->mapping,0)
 FREE RECORD dat_tsp_map
 SET dm_err->eproc = "DM2_ASSIGN_TSPACE COMPLETED"
 CALL final_disp_msg("dm2_assign_tspace")
END GO
