CREATE PROGRAM bhs_sys_trauma_one:dba
 PROMPT
 ...
 DECLARE mf_cs282_mult_cd = f8 WITH protect, constant(uar_get_code_by_cki("CKI.CODEVALUE!4200838485")
  )
 ...
 DECLARE mf_cs72_auditscore_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "AUDITSCORE"))
 DECLARE mf_cs356_traumanumber_cd = f8 WITH protect, noconstant(uar_get_code_by("DISPLAYKEY",356,
   "TRAUMANUMBER"))
 SET filename = build(logical("bhscust"),"/ftp/bhs_sys_trauma_one/t1_",format(curdate,"mmddyyyy;;d"),
  ".csv")
 SELECT INTO "nl:"
  ...
  PLAN (fn
   ...
   (SELECT
    ...
    WHERE ea.encntr_id=fn.encntr_id
     ...
     AND ((trim(ea.alias)="992????") OR (trim(ea.alias)="993????"))
     AND ea.active_ind=1
     AND  EXISTS (
    (SELECT
     ea2.encntr_id
  ...
  DETAIL
   t1->cnt += 1, stat = alterlist(t1->qual,t1->cnt), t1->qual[t1->cnt].eid = e.encntr_id
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM fn_omf_encntr fn,
  ...
  PLAN (fn
   WHERE (fn.checkout_dt_tm > (sysdate - 180))
    AND fn.tracking_group_cd=mf_bmced_trk_grp_cd)
   JOIN (ei
   WHERE ei.encntr_id=fn.encntr_id
    AND ei.info_sub_type_cd=mf_cs356_traumanumber_cd
    AND ei.active_ind=1
    AND ei.value_numeric > 0)
   JOIN (e
   WHERE e.encntr_id=ei.encntr_id
    AND e.active_ind=1
    AND e.loc_facility_cd=673936
    AND  NOT (e.encntr_type_cd IN (679658.00, 309310.00))
    AND ((e.disch_dt_tm = null) OR (e.disch_dt_tm BETWEEN cnvtdatetime((curdate - 1),0) AND
   cnvtdatetime((curdate - 1),235959)))
    AND  NOT (expand(ml_idx1,1,t1->cnt,e.encntr_id,t1->qual[ml_idx1].eid)))
  DETAIL
   t1->cnt += 1, stat = alterlist(t1->qual,t1->cnt), t1->qual[t1->cnt].eid = e.encntr_id
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (cnt=0)
    t1->cnt += 1, stat = alterlist(t1->qual,t1->cnt), t1->qual[t1->cnt].eid = o.encntr_id
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  PLAN (di
   ...
   JOIN (e
   WHERE e.encntr_id=di.info_long_id
    AND ((e.disch_dt_tm = null) OR (e.disch_dt_tm BETWEEN cnvtdatetime((curdate - 5),0) AND
   cnvtdatetime((curdate - 1),235959))) )
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = 0, cnt = locateval(ind,1,t1->cnt,e.encntr_id,t1->qual[ind].eid)
   IF (cnt=0)
    t1->cnt += 1, stat = alterlist(t1->qual,t1->cnt), t1->qual[t1->cnt].eid = e.encntr_id
   ENDIF
  WITH nocounter
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE (pi.person_id= Outerjoin(p.person_id))
    ...
    AND (pi.info_type_cd= Outerjoin(mf_user_def_cd))
    AND (pi.info_sub_type_cd= Outerjoin(mf_race1_cd)) )
   JOIN (fn
   WHERE (fn.encntr_id= Outerjoin(en.encntr_id))
    AND (fn.tracking_group_cd= Outerjoin(mf_bmced_trk_grp_cd)) )
  ORDER BY en.encntr_id
  HEAD en.encntr_id
   list->cnt += 1, stat = alterlist(list->qual,list->cnt), list->qual[list->cnt].eid = en.encntr_id,
   list->qual[list->cnt].acct = e.alias, list->qual[list->cnt].mrn = e2.alias, list->qual[list->cnt].
   last_name = p.name_last_key,
   list->qual[list->cnt].first_name = p.name_first_key
   IF (p.race_cd=mf_cs282_mult_cd)
    list->qual[list->cnt].race = trim(uar_get_code_display(pi.value_cd),3)
   ELSE
    list->qual[list->cnt].race = trim(uar_get_code_display(p.race_cd),3)
   ENDIF
   list->qual[list->cnt].dob = format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1),
    "mm/dd/yyyy hh:mm;;d"), list->qual[list->cnt].sex = uar_get_code_display(p.sex_cd), list->qual[
   list->cnt].address = concat(trim(ad.street_addr,3),trim(ad.street_addr2,3)),
   list->qual[list->cnt].city = ad.city, list->qual[list->cnt].state = ad.state, list->qual[list->cnt
   ].zip = ad.zipcode,
   list->qual[list->cnt].country = "USA", list->qual[list->cnt].checkout_dt_tm = format(fn
    .checkout_dt_tm,"mm/dd/yyyy hh:mm;;d"), list->qual[list->cnt].admit_date = concat(format(en
     .reg_dt_tm,"mm/dd/yyyy")," 00:00"),
   list->qual[list->cnt].checkin_dt_tm = list->qual[list->cnt].admit_date, list->qual[list->cnt].
   disch_date = format(en.disch_dt_tm,"mm/dd/yyyy hh:mm;;d"), list->qual[list->cnt].inpt_loc = " ",
   list->qual[list->cnt].ventilator = "0", list->qual[list->cnt].pregnant = "N", list->qual[list->cnt
   ].eddisch = " ",
   list->qual[list->cnt].mf_person_id = p.person_id, list->qual[list->cnt].ms_hispanic = "U", list->
   qual[list->cnt].mf_tot_icu_min = 0.0
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   tdx->cnt += 1, stat = alterlist(tdx->qual,tdx->cnt), tdx->qual[tdx->cnt].event_cd = vese1.event_cd
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (expand(ind,1,tdx->cnt,ce.event_cd,tdx->qual[ind].event_cd))
    cnt += 1, stat = alterlist(list->qual[d.seq].drug_result,cnt), list->qual[d.seq].drug_result[cnt]
    .display = concat(format(ce.event_end_dt_tm,"mm/dd/yyyy hh:mm;;d")," ",trim(uar_get_code_display(
       ce.event_cd),3)," ",trim(ce.result_val,3))
   ENDIF
   ...
   IF (ce.result_status_cd IN (mf_active_cd, mf_auth_cd, mf_altered_cd, mf_modified_cd, mf_final_cd))
    ...
    ENDIF
    IF (ce.event_cd=mf_cs72_auditscore_cd)
     list->qual[d.seq].s_audit_score = trim(ce.result_val,3)
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
  ...
  DETAIL
   ...
   "MICU", "PICU", "SICU", "HVCC"))
    ...
    IF (elh.end_effective_dt_tm > cnvtdatetime(sysdate))
     list->qual[d.seq].mf_tot_icu_min += datetimediff(cnvtdatetime(sysdate),elh.beg_effective_dt_tm,4
      )
    ELSE
     list->qual[d.seq].mf_tot_icu_min += datetimediff(elh.end_effective_dt_tm,elh.beg_effective_dt_tm,
      4)
    ENDIF
   ENDIF
  WITH nocounter
   ...
   FOR (x = 1 TO list->cnt)
     ...
     IF (size(list->qual[x].drug_result,5)=0)
      display2 = build('","'," ",'","',list->qual[x].ms_hispanic,'","',
       ...
       '","',list->qual[x].s_audit_score,'","',list->qual[x].s_crafft,'",'), displayline = build(
       display1,display2), col 1,
      displayline, row + 1
     ELSE
