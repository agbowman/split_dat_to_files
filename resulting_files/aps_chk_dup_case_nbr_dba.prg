CREATE PROGRAM aps_chk_dup_case_nbr:dba
 RECORD reply(
   1 manual_assign_ind = i2
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
...
#script
 ...
 SELECT INTO "nl:"
  ...
  WHERE (accession_str->accession_pool_id=pg.group_id)
   AND 1=pg.active_ind
  DETAIL
   reply->manual_assign_ind = pg.manual_assign_ind
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET reply->status_data.status = "F"
