CREATE PROGRAM bhs_rpt_tran_bld_20111210:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dfr
   ...
   WHERE dfa.dcp_forms_ref_id=dfr.dcp_forms_ref_id
    ...
    AND dfa.encntr_id IN (67484502.00, 67179539.00, 68754898.00))
   JOIN (ea
 ...
 SELECT INTO "nl:"
  ...
  HEAD ce2.event_cd
   ...
   ELSEIF (ce2.event_cd IN (mf_transfusionstarttime, autotransfusionstarttime))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_transfusionstarttime = format(
     cnvtdatetime(s_tran_result),"mm/dd/yy hh:mm;;d"), forms->patientslist[d1.seq].forminstanceslist[
    i_instance_cnt].d_transfusionstarttime = cnvtdatetime(s_tran_result), i_vital_cnt = (i_vital_cnt
    + 1)
   ...
   ELSEIF (ce2.event_cd IN (mf_transfusionstartplus15min, autotransfusionstartplus15min))
    ...
    format(cnvtdatetime(s_tran_result),"mm/dd/yy hh:mm;;d"), i_vital_cnt = (i_vital_cnt+ 1)
   ...
   ELSEIF (ce2.event_cd IN (mf_transfusionendtime, autotransfusionstoptime))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_transfusionendtime = format(
     cnvtdatetime(s_tran_result),"mm/dd/yy hh:mm;;d"), forms->patientslist[d1.seq].forminstanceslist[
    i_instance_cnt].d_transfusionendtime = cnvtdatetime(s_tran_result), i_vital_cnt = (i_vital_cnt+ 1
    )
   ...
   ELSEIF (ce2.event_cd=unitidnumberlotnumber)
    IF (findstring("=",ce2.result_val,1,0)=1)
     forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].unitidnumberlotnumber = build2('"',
      trim(ce2.result_val,3))
    ELSE
     forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].unitidnumberlotnumber = build2(ce2
      .result_val)
    ENDIF
 ...
 SELECT INTO value(trim(var_output))
  ...
  unit_id_lot_number = substring(1,80,trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].
  ...
  temperaturestart = substring(1,30,trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].
    s_transfusionstarttime)), temperatureroutestart = substring(1,25,trim(forms->patientslist[d1.seq]
 ...
 IF (( $FTPFILES=1))
  ...
  SET dclcom = concat("$cust_script/bhs_ftp_file.ksh ",filenamein,
   " 172.17.10.5 'bhs\cisftp' C!sftp01 Biovigilance2/testing")
  SET status = 0
