CREATE PROGRAM dcp_upd_prob_icd9_hli_syn_p:dba
 ...
 SET ms_range_prefix = "RANGE"
 SET mn_num_children = 5
 SET mf_min_range_id = 1.0
 SELECT INTO "nl:"
  max_val = max(nomenclature_id), min_val = min(nomenclature_id)
  FROM nomenclature
  DETAIL
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_PROB_ICD9_HLI_SYN",concat(
   "create global temporary table TEMP_PROB_ICD9_HLI_SYN(problem_instance_id number,",
   "nomenclature_id number) on commit preserve rows"),"Creating temporary table")
 CALL sbr_add_sql_stmt("INDEX","XIETEMPPROBICD9HLISYN",
  "create unique index XIETEMPPROBICD9HLISYN ON TEMP_PROB_ICD9_HLI_SYN(problem_instance_id)",
  "Creating index")
