CREATE PROGRAM dm_eem_transaction_rows:dba
 ...
 IF (v_days_to_keep < 30)
  ...
  SET reply->status_data.status = "F"
 ELSE
  SET reply->table_name = "EEM_TRANSACTION"
  SET reply->rows_between_commit = 50
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(et.transaction_id)
    FROM eem_transaction et
    WHERE et.transaction_id > 0
    DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     FROM eem_transaction et
     WHERE parser(sbr_getrowidnotexists("et.transaction_id between curMinID and curMaxID","et"))
      AND et.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,100)=1)
       stat = alterlist(reply->rows,(rows+ 99))
      ENDIF
      reply->rows[rows].row_id = et.rowid
     WITH nocounter, maxqual(et,value(rowsleft))
    ;end select
