CREATE PROGRAM bed_ens_iview_assay:dba
 ...
 DECLARE addassay(taskassayuid=vc,mnemonic=vc) = f8
 DECLARE addevent(taskassayuid=vc,taskassaycd=f8) = i2
 DECLARE sortassaysbeforesaving(dummyvar=i2) = i2
 IF (assay_cnt > 0)
  CALL bedaddlogstologgerfile("#### ENTERING INTO BED_ENS_IVIEW_ASSAY.PRG ####")
  ...
  FOR (xx = 1 TO assay_cnt)
    IF ((assaystosave->assays[xx].assay_action_flag=1))
     CALL addassay(assaystosave->assays[xx].task_assay_uid,trim(assaystosave->assays[xx].description,
       7))
    ENDIF
  ENDFOR
  IF (validate(request->iviewname)
   ...
   CALL bedaddlogstologgerfile("#### EXITING FROM BED_ENS_IVIEW_ASSAY.PRG ####")
...
#exit_script
 ...
 END ;Subroutine
 SUBROUTINE addassay(taskassayuid,mnemonic)
   ...
   CALL bedaddlogstologgerfile(build2("### taskAssayUID:",taskassayuid))
   ...
   ;end select
   IF (mnemonic != "")
    SET ensassayrequest->assay_list[1].display = mnemonic
   ENDIF
   SELECT INTO "nl:"
    FROM cnt_data_map m,
     cnt_code_value_key c1
   ...
   FOR (i = 1 TO rrcnt)
     CALL handlereferencerange(1,importedrrrec->list[i].rrfuid,0.0,add_flag)
   ENDFOR
   SET newtaskassaycd = executebedensassay(0)
   CALL bedaddlogstologgerfile(build2("### newTaskAssayCd:",newtaskassaycd))
   CALL addevent(taskassayuid,newtaskassaycd)
   ...
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   CALL bedaddlogstologgerfile(build2("### reference text curqual:",curqual))
   IF (curqual > 0)
   ...
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   CALL bedaddlogstologgerfile(build2("### Interp curqual:",curqual))
   IF (curqual > 0)
 ...
 END ;Subroutine
 SUBROUTINE addevent(taskassayuid,taskassaycd)
   CALL bedlogmessage("addEvent","Entering ...")
   CALL bedaddlogstologgerfile("### Entering addEvent ...")
   CALL bedaddlogstologgerfile(build2("### taskAssayUID:",taskassayuid," taskAssayCd:",taskassaycd))
   ...
   SELECT INTO "nl:"
    ...
    PLAN (d
     WHERE d.task_assay_uid=taskassayuid)
     JOIN (c
     WHERE c.code_value_uid=d.event_code_cduid
    ...
    WITH nocounter
   ;end select
   CALL bedaddlogstologgerfile(build2("### event_code_value:",event_code_value,"display:",display,
     " definition:",
   ...
   IF (curqual > 0)
    IF (event_code_value=0
     ...
     SELECT INTO "nl:"
      ...
      WITH nocounter
     ;end select
     CALL bedaddlogstologgerfile(build2("### again event_code_value:",event_code_value))
     IF (event_code_value=0)
      SET event_code_value = insertcodevalue(cdfmeaning,definition,description,display,codeset,
       conceptcki,cki)
      CALL bedaddlogstologgerfile(build2("### yet another event_code_value:",event_code_value))
    ...
    IF (event_code_value > 0
     ...
     UPDATE  FROM discrete_task_assay d
      ...
      WITH nocounter
     ;end update
     CALL bedaddlogstologgerfile(build2("### Updated discrete_task_assay $$ event_code_value:",
       event_code_value," taskAssayCd:",taskassaycd))
    ENDIF
   ENDIF
 ...
 SUBROUTINE sortassaysbeforesaving(dummyvar)
   ...
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   CALL bedaddlogstologgerfile(build2("### asCnt:",ascnt," interCnt:",intercnt))
   IF (ascnt=0
    AND intercnt=0)
