CREATE PROGRAM cp_br_create_path_from_concept:dba
 PROMPT
 ...
 DECLARE dpathway_status_cd_active = f8 WITH protect, constant(uar_get_code_by("MEANING",4003198,
   "ACTIVE"))
 DECLARE dconcept_cd = f8 WITH protect, constant( $DINPUT_CONCEPT_CD)
 DECLARE spathway_name = vc WITH protect, constant( $SINPUT_PATHWAY_NAME)
 DECLARE duser_logic_domain_id = f8 WITH protect, constant(getuserlogicaldomain(reqinfo->updt_id))
 ...
 SET existingpathwayid = checkforexistingactivepathway(spathway_name,dpathway_input_type_cd,
  duser_logic_domain_id)
 IF ((existingpathwayid > - (1)))
  IF (validate(debug_ind,0))
   CALL echo("Duplicate exists")
  ENDIF
  SET mpagereply->status_data.status = "F"
  GO TO exit_script
 ENDIF
 ...
 SUBROUTINE createnewpathway(null)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   EXECUTE cp_add_pathway "NOFORMS",  $SINPUT_PATHWAY_NAME, dpathway_input_type_cd,
   "ACTIVE", 0.0, dconcept_cd
   SET mpagereply->pathway_id = reply->pwlist[1].cp_pathway_id
