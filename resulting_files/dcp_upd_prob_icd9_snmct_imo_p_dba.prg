CREATE PROGRAM dcp_upd_prob_icd9_snmct_imo_p:dba
 ...
 IF (error(ms_errmsg,0) != 0)
  ...
  GO TO exit_program
 ENDIF
 IF (((dicd9 <= 0.0) OR (((dsnmct <= 0.0) OR (dimo <= 0.0)) )) )
  SET readme_data->status = "F"
  SET readme_data->message = "Code values for ICD9, SNMCT, or IMO in Codeset 400 do not exist"
  GO TO exit_program
 ENDIF
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_PROB_ICD9_SNMCT_IMO",concat(
   "create global temporary table TEMP_PROB_ICD9_SNMCT_IMO(person_id number,",
 ...
 CALL sbr_add_sql_stmt("INDEX","XIETEMPPROBICD9SNMCTIMO",
  "create index XIETEMPPROBICD9SNMCTIMO ON TEMP_PROB_ICD9_SNMCT_IMO(problem_instance_id)",
  "Creating index")
