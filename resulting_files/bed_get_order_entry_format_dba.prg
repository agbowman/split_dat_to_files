CREATE PROGRAM bed_get_order_entry_format:dba
 FREE SET reply
 RECORD reply(
   1 oe_format_list[*]
     2 oe_format_id = f8
 ...
 DECLARE acnt = i4 WITH protect, constant(size(request->alist,5))
 DECLARE oef_parse = vc WITH protect, noconstant("")
 DECLARE aff_parse = vc WITH protect
...
#exit_script
 ...
 SUBROUTINE getorderentryformats(dummyvar)
   SET stat = alterlist(reply->oe_format_list,1)
   SELECT DISTINCT INTO "NL:"
    ...
    DETAIL
     ...
     IF (listcount > 1)
      stat = alterlist(reply->oe_format_list,(listcount+ 1))
     ENDIF
   ...
   ;end select
   CALL bederrorcheck("Error[1] Failure in getting order entry formats")
 END ;Subroutine
 SUBROUTINE buildoefparserforrequestedactions(dummyvar)
   ...
   FOR (a = 1 TO acnt)
     ...
     SELECT INTO "nl:"
      ...
      WITH nocounter
     ;end select
     CALL bederrorcheck("Error[2] Failure in preparing parser text")
     IF (a=acnt)
   ...
   IF (validate(request->search_string,"") > " ")
    SET aff_parse = build("concat('*',cnvtupper(trim(request->search_string)),'*')))")
    SET oef_parse = build(oef_parse," and cnvtupper(oef.oe_format_name) in(patstring(",aff_parse)
   ENDIF
 END ;Subroutine
END GO
