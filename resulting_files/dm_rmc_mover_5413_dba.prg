CREATE PROGRAM dm_rmc_mover_5413:dba
 ...
 SUBROUTINE drmm_breakup_string(sbr_rs_reply)
   ...
   RETURN("S")
 END ;Subroutine
 DECLARE cust_tab_name = vc WITH protect, noconstant("")
 DECLARE cust_src_tab_name = vc WITH protect, noconstant("")
 DECLARE cust_src_sda_name = vc WITH protect, noconstant("")
 DECLARE cust_src_freq_id = f8 WITH protect, noconstant(0.0)
 DECLARE cust_src_def_sch_id = f8 WITH protect, noconstant(0.0)
 DECLARE cust_src_res_cd = f8 WITH protect, noconstant(0.0)
 DECLARE cust_src_beg_dt_tm = f8 WITH protect, noconstant(0.0)
 DECLARE cust_src_end_dt_tm = f8 WITH protect, noconstant(0.0)
 DECLARE cust_src_dow = vc WITH protect, noconstant("")
 DECLARE cust_tgt_def_sch_id = vc WITH protect, noconstant("")
 DECLARE cust_tgt_res_cd = vc WITH protect, noconstant("")
 DECLARE cust_tgt_freq_id = f8 WITH protect, noconstant(0.0)
 DECLARE cust_cnt = i4 WITH protect, noconstant(0)
 DECLARE cust_idcd_check = i4 WITH protect, noconstant(0)
 DECLARE cust_rpt_missing = vc WITH protect, noconstant("")
 DECLARE cust_new_id = f8 WITH protect, noconstant(0.0)
 DECLARE cust_from_id = f8 WITH protect, noconstant(0.0)
 DECLARE cust_app_rng_null_ind = i2 WITH protect, noconstant(0)
 DECLARE cust_apply_range = vc WITH protect, noconstant("")
 DECLARE cust_ndx = i4 WITH protect, noconstant(0)
 DECLARE cust_col_pos = i4 WITH protect, noconstant(0)
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET temp_col_cnt = get_col_pos(cust_tab_name,"FREQUENCY_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   SET cust_src_freq_id = cnvtreal(get_value(cust_tab_name,"FREQUENCY_ID","FROM"))
   SET cust_src_tab_name = dm2_get_rdds_tname("SCH_FREQ")
   SET cust_src_sda_name = dm2_get_rdds_tname("SCH_DEF_APPLY")
   SELECT INTO "NL:"
    FROM (parser(cust_src_sda_name) s)
    WHERE s.frequency_id=cust_src_freq_id
    DETAIL
     cust_src_def_sch_id = s.def_sched_id, cust_src_res_cd = s.resource_cd, cust_src_beg_dt_tm = s
     .beg_dt_tm,
     cust_src_end_dt_tm = s.end_dt_tm, cust_src_dow = s.days_of_week
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm_err->emsg = "Error obtaining data from SCH_DEF_APPLY in source."
    SET dm2_ref_data_reply->error_msg = "Error obtaining data from SCH_DEF_APPLY in source."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_5413
   ENDIF
   IF (curqual=0)
    ...
    "Could not identify a single related SCH_DEF_APPLY row in source.  Found no related row."
    SET dm_err->emsg = concat("Could not identify a single related SCH_DEF_APPLY ",
     "row in source.  Found no related row.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV63"
    GO TO exit_5413
   ELSEIF (curqual > 1)
    SET vers_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
    SET drdm_chg->log[vers_parent_drdm_row].chg_log_reason_txt =
    "Could not identify a single related SCH_DEF_APPLY row in source.  Found multiple related rows."
    SET dm_err->emsg = concat("Could not identify a single related SCH_DEF_APPLY ",
     "row in source.  Found multiple related rows.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV63"
   ENDIF
   SET cust_tgt_def_sch_id = select_merge_translate(cnvtstring(cust_src_def_sch_id),"SCH_DEF_SCHED")
   SET cust_tgt_res_cd = select_merge_translate(cnvtstring(cust_src_res_cd),"CODE_VALUE")
   IF (isnumeric(cust_tgt_def_sch_id) IN (1, 2)
    AND isnumeric(cust_tgt_res_cd) IN (1, 2))
    SET cust_col_pos = locateval(cust_ndx,1,size(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual,5),
     "APPLY_RANGE",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_ndx].column_name)
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_pos].check_null=1))
     SET cust_apply_range = "f.apply_range is NULL"
    ELSE
     SET cust_apply_range = "f.apply_range = RS_5413->to_values.apply_range"
    ENDIF
    SELECT INTO "NL:"
     FROM sch_freq f
     WHERE f.parent_table="SCH_DEF_APPLY"
      AND (f.days_of_week=rs_5413->to_values.days_of_week)
      AND (f.day_string=rs_5413->to_values.day_string)
      AND (f.end_type_cd=rs_5413->to_values.end_type_cd)
      AND (f.max_occurance=rs_5413->to_values.max_occurance)
      AND (f.freq_pattern_cd=rs_5413->to_values.freq_pattern_cd)
      AND (f.interval=rs_5413->to_values.interval)
      AND (f.freq_type_cd=rs_5413->to_values.freq_type_cd)
      AND (f.month_string=rs_5413->to_values.month_string)
      AND (f.pattern_option=rs_5413->to_values.pattern_option)
      AND (f.month_string=rs_5413->to_values.month_string)
      AND (f.pattern_option=rs_5413->to_values.pattern_option)
      AND (f.units=rs_5413->to_values.units)
      AND (f.units_cd=rs_5413->to_values.units_cd)
      AND (f.week_string=rs_5413->to_values.week_string)
      AND f.beg_dt_tm=cnvtdatetime(rs_5413->to_values.beg_dt_tm)
      AND f.end_dt_tm=cnvtdatetime(rs_5413->to_values.end_dt_tm)
      AND parser(cust_apply_range)
      AND  EXISTS (
     (SELECT
      "x"
      FROM sch_def_apply a
      WHERE a.def_apply_id=f.parent_id
       AND a.def_sched_id=cnvtreal(cust_tgt_def_sch_id)
       AND a.resource_cd=cnvtreal(cust_tgt_res_cd)
       AND a.days_of_week=cust_src_dow
       AND a.beg_dt_tm=cnvtdatetime(cust_src_beg_dt_tm)
       AND a.end_dt_tm=cnvtdatetime(cust_src_end_dt_tm)))
     DETAIL
      cust_cnt = (cust_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,cust_cnt), ui_query_eval_rec
      ->qual[cust_cnt].root_entity_attr = f.frequency_id
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "Error selecting from target."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_5413
    ENDIF
    SET cust_tgt_freq_id = evaluate_exec_ui_query(cust_cnt,temp_tbl_cnt,perm_col_cnt)
    IF (cust_tgt_freq_id < 0)
     IF ((cust_tgt_freq_id=- (3)))
      SET cust_tgt_freq_id = get_seq(cust_tab_name,"FREQUENCY_ID")
      IF (cust_tgt_freq_id > 0)
       CALL put_value(cust_tab_name,"FREQUENCY_ID",cnvtstring(cust_tgt_freq_id))
       SET current_merges = (current_merges+ 1)
       SET child_merge_audit->num[current_merges].action = "NEWSEQ"
       SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  FREQUENCY_ID")
      ELSE
       GO TO exit_5413
      ENDIF
     ELSE
    ...
    SET cust_from_id = cnvtreal(get_value(cust_tab_name,"FREQUENCY_ID","FROM"))
    IF ((dm2_ref_data_reply->error_ind=1))
     GO TO exit_5413
    ELSE
     SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_id)
    ENDIF
    SET cust_new_id = cnvtreal(get_value(cust_tab_name,"FREQUENCY_ID","TO"))
    IF ((dm2_ref_data_reply->error_ind=1))
     GO TO exit_5413
    ELSE
     SET dm2_ref_data_reply->qual[1].to_value = cnvtreal(cust_new_id)
    ENDIF
    CALL echo(build("P COLUMN = ",curmem))
   ELSE
    IF (isnumeric(cust_tgt_def_sch_id)=0)
     SET cust_rpt_missing = report_missing("SCH_DEF_SCHED","DEF_SCHED_ID",cust_src_def_sch_id)
    ELSE
     SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cust_src_res_cd)
    ENDIF
