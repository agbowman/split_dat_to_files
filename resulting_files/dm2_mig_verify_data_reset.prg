CREATE PROGRAM dm2_mig_verify_data_reset
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmvdr_owner = vc WITH protect, noconstant(cnvtupper( $1))
 DECLARE dmvdr_table = vc WITH protect, noconstant(cnvtupper( $2))
 DECLARE dmvdr_compare_ind = i2 WITH protect, noconstant(1)
 DECLARE dmvdr_chk_time = dq8 WITH protect, noconstant(0.0)
 IF (check_logfile("dm2migvtblrst",".log","dm2_mig_verify_data_reset")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_verify_data_reset"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dmr_prompt_retrieve_mig_data(0,0,1,0)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Check for stop compare row."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_VDATA_INFO"
   AND d.info_name="DM2_VDATA_STOP_COMPARE"
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Stop the compare."
  INSERT  FROM dm_info d
   SET d.info_domain = "DM2_VDATA_INFO", d.info_name = "DM2_VDATA_STOP_COMPARE"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  COMMIT
 ENDIF
 SET dmvdr_chk_time = cnvtdatetime(curdate,curtime3)
 WHILE (dmvdr_compare_ind=1)
   IF (dvdr_cleanup_stranded(null)=0)
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = "Check if any tables being compared."
   SELECT INTO "nl;"
    FROM dm_vdata_master d
    WHERE d.compare_status="COMPARING"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dmvdr_compare_ind = 0
   ENDIF
   IF (datetimediff(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),cnvtdatetimeutc(cnvtdatetime(
      dmvdr_chk_time)),4) >= 5)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Unable to stop compare. One or more tables are currently comparing."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (dmvdr_compare_ind=1)
    CALL pause(5)
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Resetting compare"
 UPDATE  FROM dm_vdata_master d
  SET d.range_beg_keyid = 0, d.range_end_keyid = 0, d.compare_status = "READY",
   d.mismatch_event_cnt = 0, d.mismatch_beg_keyid = 0, d.mismatch_end_keyid = 0,
   d.last_compare_dt_tm = cnvtdatetime("01-JAN-1900"), d.last_src_mod_dt_tm = cnvtdatetime(
    "01-JAN-1900"), d.last_compare_cnt = 0,
   d.last_match_dt_tm = cnvtdatetime("01-JAN-1900"), d.begin_dt_tm = cnvtdatetime("01-JAN-1900"), d
   .end_dt_tm = cnvtdatetime("01-JAN-1900"),
   d.last_match_cnt = 0, d.ttl_rows_compared = 0, d.prev_mismatch_row_cnt = 0,
   d.curr_mismatch_row_cnt = 0, d.lag_dt_tm = cnvtdatetime("01-JAN-1900"), d.mismatch_rowid = null,
   d.mismatch_row_dt_tm = cnvtdatetime("01-JAN-1900"), d.mismatch_unique_key_txt = null, d
   .message_txt = null
  WHERE d.owner_name=patstring(dmvdr_owner)
   AND d.table_name=patstring(dmvdr_table)
   AND  NOT (d.compare_status IN ("EXCLUDE", "NOCOMP-UOW"))
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 COMMIT
#exit_program
 SET dm_err->eproc = "Remove stop compare row."
 DELETE  FROM dm_info d
  WHERE d.info_domain="DM2_VDATA_INFO"
   AND d.info_name="DM2_VDATA_STOP_COMPARE"
  WITH nocounter
 ;end delete
 COMMIT
 SET dm_err->eproc = "dm2_mig_verify_data_reset completed"
 CALL final_disp_msg("dm2migvtblrst")
END GO
