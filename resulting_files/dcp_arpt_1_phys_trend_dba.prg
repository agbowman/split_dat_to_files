CREATE PROGRAM dcp_arpt_1_phys_trend:dba
 RECORD rpt_params(
   1 output_device = vc
   1 beg_dt_tm = dq8
   1 end_dt_tm = dq8
 ...
 RECORD pat_record(
   ...
   1 pat_data[*]
     ...
     2 elective_surg = vc
     2 readmit = vc
     2 no_activerx_pred = vc
     2 discharged_to_icu = vc
     2 visit_num = i4
     2 hosp_disch_dt = dq8
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (ra.icu_disch_dt_tm != cnvtdatetime("31-DEC-2100 00:00"))
    ...
    ENDIF
    IF (uar_get_code_meaning(ra.discharge_location_cd)="ICU_TRANSFER")
     pat_record->pat_data[indx].discharged_to_icu = "Y"
    ELSE
     pat_record->pat_data[indx].discharged_to_icu = "N"
    ENDIF
   ...
   ELSE
    pat_record->pat_data[indx].discharged_to_icu = "N", pat_record->pat_data[indx].icu_disch_stat =
    " ", pat_record->pat_data[indx].curr_icu_day = datetimediff(datetimeadd(cnvtdatetime(curdate,
       curtime3),1),ra.icu_admit_dt_tm,1),
    pat_record->pat_data[indx].act_or_curr_icu_day_label = "Current ICU Day:"
   ENDIF
   disease_category = uar_get_code_meaning(ra.disease_category_cd)
   IF (((disease_category="CARDARR") OR (((disease_category="CARDIOG") OR (((disease_category=
   "SVALVE") OR (((disease_category="SLIVERTR") OR (((disease_category="SRUPAOR") OR (((
   disease_category="SVHDCABG") OR (disease_category="SCABGVALV")) )) )) )) )) )) )
    pat_record->pat_data[indx].no_activerx_pred = "Y"
   ELSE
    pat_record->pat_data[indx].no_activerx_pred = "N"
   ENDIF
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (ea
   WHERE (ea.encntr_id=pat_record->pat_data[d.seq].encntr_id)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE rao.risk_adjustment_day_id=rad.risk_adjustment_day_id
    AND rao.equation_name IN ("ICU_DEATH", "ICU_LOS")
    AND rao.active_ind=1)
  ORDER BY d.seq, rad.cc_day
  DETAIL
   IF (rao.equation_name="ICU_LOS")
    IF (rad.cc_day=1)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   obs_value[indx].ap3 = rad.apache_iii_score
   ...
   IF (rad.activetx_ind=1)
    pat_record->pat_data[d.seq].obs_value[indx].act_tx = "Yes"
   ELSEIF (rad.activetx_ind=0)
    pat_record->pat_data[d.seq].obs_value[indx].act_tx = "No"
   ELSE
    pat_record->pat_data[d.seq].obs_value[indx].act_tx = "N/A"
   ENDIF
   IF (rad.meds_ind=0)
   ...
   pat_record->pat_data[d.seq].obs_value[indx].urine = rad.urine_24hr_output, pat_record->pat_data[d
   .seq].obs_value[indx].wbc = rad.worst_wbc_result, pat_record->pat_data[d.seq].obs_value[indx].
   hematocrit = rad.worst_hematocrit,
   pat_record->pat_data[d.seq].obs_value[indx].sodium = rad.worst_sodium_result, pat_record->
   pat_data[d.seq].obs_value[indx].bun = rad.worst_bun_result, pat_record->pat_data[d.seq].obs_value[
   indx].creatinine = rad.worst_creatinine_result,
   pat_record->pat_data[d.seq].obs_value[indx].glucose = rad.worst_glucose_result, pat_record->
   pat_data[d.seq].obs_value[indx].albumin = rad.worst_albumin_result, pat_record->pat_data[d.seq].
   obs_value[indx].bilirubin = rad.worst_bilirubin_result
   ...
   IF (rad.pa_line_today_ind=1)
    pat_record->pat_data[d.seq].obs_value[indx].pa_line = "Yes"
   ELSEIF (rad.pa_line_today_ind=0)
    pat_record->pat_data[d.seq].obs_value[indx].pa_line = "No"
   ELSE
    pat_record->pat_data[d.seq].obs_value[indx].pa_line = "N/A"
   ENDIF
   ...
   ELSE
    pat_record->pat_data[d.seq].obs_value[indx].aado2 = - (1.0)
   ENDIF
  WITH nocounter
 ;end select
 FOR (i = 1 TO pat_record->cnt)
   SET get_visit_parameters->risk_adjustment_id = pat_record->pat_data[i].ra_id
   EXECUTE cco_get_apache_visit_number
   SET pat_record->pat_data[i].visit_num = get_visit_reply->visit_number
 ENDFOR
 SELECT INTO rpt_params->output_device
  ...
  HEAD PAGE
   ...
   date_full_disp,
   CALL print(calcpos(600,y_pos)), "By Module: dcp_aprt_1_phys_trend",
   CALL print(calcpos(358,y_pos)), "APACHE For ICU", row + 1,
   y_pos = (y_pos+ 10), col 0, font80c,
   row + 1, y_pos = (y_pos+ 10),
   CALL print(calcpos(265,y_pos)),
   "Physiology Trend (Daily Worst Values)", row + 1, col 0,
   ...
   y_pos = (y_pos+ 30), start_pos = y_pos, row + 1
  DETAIL
   FOR (i = 1 TO pat_record->cnt)
     unit_bed = fillstring(50," "), unit_bed = pat_record->pat_data[i].unit, dob = format(pat_record
      ->pat_data[i].dob,"mm-dd-yyyy ;;d"),
     ...
     curr_icu_day_disp = format(pat_record->pat_data[i].curr_icu_day,"###;l")
     ...
     ENDIF
     visit_number_disp = format(pat_record->pat_data[i].visit_num,"##;l"),
     CALL print(calcpos(30,y_pos)), "APACHE Admission ICU: ",
     CALL print(calcpos(140,y_pos)), unit_bed,
     CALL print(calcpos(300,y_pos)),
     "Body System: ",
     CALL print(calcpos(415,y_pos)), pat_record->pat_data[i].system,
     CALL print(calcpos(510,y_pos)), "Admit Source: ",
     CALL print(calcpos(700,y_pos)),
     pat_record->pat_data[i].admit_source, row + 1, y_pos = (y_pos+ 10),
     CALL print(calcpos(30,y_pos)), "Patient Name:",
     CALL print(calcpos(140,y_pos)),
     pat_record->pat_data[i].name,
     CALL print(calcpos(300,y_pos)), "ICU Discharge Date:",
     CALL print(calcpos(415,y_pos)), icu_disch_dt_disp,
     CALL print(calcpos(510,y_pos)),
     "Chronic Health:",
     CALL print(calcpos(700,y_pos)), pat_record->pat_data[i].chronic,
     row + 1, y_pos = (y_pos+ 10),
     CALL print(calcpos(30,y_pos)),
     "Patient ID:",
     CALL print(calcpos(140,y_pos)), pat_record->pat_data[i].mrn,
     CALL print(calcpos(300,y_pos)), "ICU Discharge Status:",
     CALL print(calcpos(415,y_pos)),
     pat_record->pat_data[i].icu_disch_stat,
     CALL print(calcpos(510,y_pos)), "Elective Surgery (Y/N):",
     CALL print(calcpos(700,y_pos)), pat_record->pat_data[i].elective_surg, row + 1,
     y_pos = (y_pos+ 10),
     CALL print(calcpos(30,y_pos)), "Age/DOB:",
     CALL print(calcpos(140,y_pos)), age_dob,
     CALL print(calcpos(300,y_pos)),
     "ICU Admit Date:",
     CALL print(calcpos(415,y_pos)), icu_admit_dt_disp,
     CALL print(calcpos(510,y_pos)), "Readmission (Y/N):",
     CALL print(calcpos(700,y_pos)),
     pat_record->pat_data[i].readmit, row + 1, y_pos = (y_pos+ 10),
     CALL print(calcpos(30,y_pos)), "Service:",
     CALL print(calcpos(140,y_pos)),
     pat_record->pat_data[i].service,
     CALL print(calcpos(300,y_pos)), pat_record->pat_data[i].act_or_curr_icu_day_label,
     CALL print(calcpos(415,y_pos)), curr_icu_day_disp,
     CALL print(calcpos(510,y_pos)),
     "Excluded from Active Rx pred (Y/N):",
     CALL print(calcpos(700,y_pos)), pat_record->pat_data[i].no_activerx_pred,
     row + 1, y_pos = (y_pos+ 10),
     CALL print(calcpos(30,y_pos)),
     "Physician Name:",
     CALL print(calcpos(140,y_pos)), pat_record->pat_data[i].doc_name,
     CALL print(calcpos(300,y_pos)), "Predicted ICU LOS:",
     CALL print(calcpos(415,y_pos)),
     icu_pred_los_disp,
     CALL print(calcpos(510,y_pos)), "Discharged to another ICU (Y/N):",
     CALL print(calcpos(700,y_pos)), pat_record->pat_data[i].discharged_to_icu, row + 1,
     y_pos = (y_pos+ 10),
     CALL print(calcpos(30,y_pos)), "ICU Admit Diagnosis:",
     CALL print(calcpos(140,y_pos)), pat_record->pat_data[i].diagnosis,
     CALL print(calcpos(300,y_pos)),
     pat_record->pat_data[i].day_n_label,
     CALL print(calcpos(415,y_pos)), pat_record->pat_data[i].day_n,
     CALL print(calcpos(510,y_pos)), "Visit Number:",
     CALL print(calcpos(700,y_pos)),
     visit_number_disp, row + 1, y_pos = (y_pos+ 10),
     CALL print(calcpos(30,y_pos)), "Hospital Admit Date:",
     CALL print(calcpos(140,y_pos)),
     hosp_adm_dt_disp,
     CALL print(calcpos(300,y_pos)), "Admit Category:",
     CALL print(calcpos(415,y_pos)), pat_record->pat_data[i].therapy,
     CALL print(calcpos(510,y_pos)),
     "Hospital Discharge Date:",
     CALL print(calcpos(700,y_pos)), hosp_disch_dt_disp,
     row + 1, y_pos = (y_pos+ 10),
     CALL print(calcpos(510,y_pos)),
     "Hospital Discharge Status:",
     CALL print(calcpos(700,y_pos)), pat_record->pat_data[i].hosp_disch_stat,
     ...
     ENDIF
     prev_wbc = - (1.00), prev_hematocrit = - (1.00), prev_sodium = - (1.00),
     prev_bun = - (1.00), prev_creatinine = - (1.00), prev_glucose = - (1.00),
     prev_albumin = - (1.00), prev_bilirubin = - (1.00), prev_fio2 = - (1),
     prev_pao2 = - (1), prev_paco2 = - (1), prev_ph = - (1.00),
     prev_aado2 = - (1.00)
     FOR (y = 1 TO loop_cnt)
       ...
       CALL print(calcpos(30,y_pos)), "MAP"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "PA Line"
       FOR (x = start_indx TO col_max)
         column = (column+ 60),
         CALL print(calcpos(column,y_pos)), pat_record->pat_data[i].obs_value[x].pa_line
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "Act. Treatment"
       FOR (x = start_indx TO col_max)
         column = (column+ 60),
         CALL print(calcpos(column,y_pos)), pat_record->pat_data[i].obs_value[x].act_tx
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "Glascow C.S."
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(40,y_pos)), "Eyes"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(40,y_pos)), "Motor"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(40,y_pos)), "Verbal"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "Urine Out"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
         IF ((pat_record->pat_data[i].obs_value[x].urine=- (1.00)))
          urine = "N/A     "
         ELSE
          urine = format(pat_record->pat_data[i].obs_value[x].urine,"#####.##;l")
         ENDIF
         CALL print(calcpos(column,y_pos)), urine
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "WBC", wbc = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
         IF (x=1)
          IF ((pat_record->pat_data[i].obs_value[x].wbc=- (1.00)))
           wbc = "N/A   "
          ELSE
           wbc = format(pat_record->pat_data[i].obs_value[x].wbc,"####.##;l"), prev_wbc = pat_record
           ->pat_data[i].obs_value[x].wbc
          ENDIF
         ELSE
          IF ((pat_record->pat_data[i].obs_value[x].wbc=- (1.00)))
           IF ((prev_wbc=- (1.00)))
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "Hematocrit", hematocrit = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
         IF (x=1)
          ...
          ELSE
           ...
           prev_hematocrit = pat_record->pat_data[i].obs_value[x].hematocrit
          ENDIF
         ELSE
          IF ((pat_record->pat_data[i].obs_value[x].hematocrit=- (1.00)))
           IF ((prev_hematocrit=- (1.00)))
            hematocrit = "N/A   "
           ELSE
            hematocrit = build("(",format(prev_hematocrit,"###.##;l"),")")
           ENDIF
          ELSE
           hematocrit = format(pat_record->pat_data[i].obs_value[x].hematocrit,"###.##;l"),
           prev_hematocrit = pat_record->pat_data[i].obs_value[x].hematocrit
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "Sodium", sodium = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
         IF (x=1)
          IF ((pat_record->pat_data[i].obs_value[x].sodium=- (1.00)))
           sodium = "N/A   "
          ELSE
           sodium = format(pat_record->pat_data[i].obs_value[x].sodium,"###.##;l"), prev_sodium =
           pat_record->pat_data[i].obs_value[x].sodium
          ENDIF
         ELSE
          IF ((pat_record->pat_data[i].obs_value[x].sodium=- (1.00)))
           IF ((prev_sodium=- (1.00)))
            sodium = "N/A   "
           ELSE
            sodium = build("(",format(prev_sodium,"###.##;l"),")")
           ENDIF
          ELSE
           sodium = format(pat_record->pat_data[i].obs_value[x].sodium,"###.##;l"), prev_sodium =
           pat_record->pat_data[i].obs_value[x].sodium
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "BUN", bun = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
         IF (x=1)
          IF ((pat_record->pat_data[i].obs_value[x].bun=- (1.00)))
           bun = "N/A   "
          ELSE
           bun = format(pat_record->pat_data[i].obs_value[x].bun,"###.##;l"), prev_bun = pat_record->
           pat_data[i].obs_value[x].bun
          ENDIF
         ELSE
          IF ((pat_record->pat_data[i].obs_value[x].bun=- (1.00)))
           IF ((prev_bun=- (1.00)))
          ...
          ENDIF
         ENDIF
         CALL print(calcpos(column,y_pos)), bun
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "Creatinine", creatinine = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
         IF (x=1)
          ...
          ELSE
           ...
           prev_creatinine = pat_record->pat_data[i].obs_value[x].creatinine
          ENDIF
         ELSE
          IF ((pat_record->pat_data[i].obs_value[x].creatinine=- (1.00)))
           IF ((prev_creatinine=- (1.00)))
            creatinine = "N/A   "
           ELSE
            creatinine = build("(",format(prev_creatinine,"###.##;l"),")")
           ENDIF
          ELSE
           creatinine = format(pat_record->pat_data[i].obs_value[x].creatinine,"###.##;l"),
           prev_creatinine = pat_record->pat_data[i].obs_value[x].creatinine
       ...
       row + 1, y_pos = (y_pos+ 10),
       CALL print(calcpos(30,y_pos)),
       "Glucose", column = 120, glucose = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
         IF (x=1)
          IF ((pat_record->pat_data[i].obs_value[x].glucose=- (1.00)))
           glucose = "N/A   "
          ELSE
           glucose = format(pat_record->pat_data[i].obs_value[x].glucose,"####.##;l"), prev_glucose
            = pat_record->pat_data[i].obs_value[x].glucose
          ENDIF
         ELSE
          IF ((pat_record->pat_data[i].obs_value[x].glucose=- (1.00)))
           IF ((prev_glucose=- (1.00)))
            glucose = "N/A   "
           ELSE
            glucose = build("(",format(prev_glucose,"####.##;l"),")")
           ENDIF
          ELSE
           glucose = format(pat_record->pat_data[i].obs_value[x].glucose,"####.##;l"), prev_glucose
            = pat_record->pat_data[i].obs_value[x].glucose
          ENDIF
         ENDIF
         CALL print(calcpos(column,y_pos)), glucose
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "Albumin", albumin = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
         IF (x=1)
          IF ((pat_record->pat_data[i].obs_value[x].albumin=- (1.00)))
           albumin = "N/A   "
          ELSE
           albumin = format(pat_record->pat_data[i].obs_value[x].albumin,"###.##;l"), prev_albumin =
           pat_record->pat_data[i].obs_value[x].albumin
          ENDIF
         ELSE
          IF ((pat_record->pat_data[i].obs_value[x].albumin=- (1.00)))
           IF ((prev_albumin=- (1.00)))
            albumin = "N/A   "
           ELSE
            albumin = build("(",format(prev_albumin,"###.##;l"),")")
           ENDIF
          ELSE
           albumin = format(pat_record->pat_data[i].obs_value[x].albumin,"###.##;l"), prev_albumin =
           pat_record->pat_data[i].obs_value[x].albumin
          ENDIF
         ENDIF
         CALL print(calcpos(column,y_pos)), albumin
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "Bilirubin", bilirubin = "###.####"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
         IF (x=1)
          IF ((pat_record->pat_data[i].obs_value[x].bilirubin=- (1.00)))
           bilirubin = "N/A   "
          ELSE
           bilirubin = format(pat_record->pat_data[i].obs_value[x].bilirubin,"###.####;l"),
           prev_bilirubin = pat_record->pat_data[i].obs_value[x].bilirubin
          ENDIF
         ELSE
          IF ((pat_record->pat_data[i].obs_value[x].bilirubin=- (1.00)))
           IF ((prev_bilirubin=- (1.00)))
            bilirubin = "N/A   "
           ELSE
            bilirubin = build("(",format(prev_bilirubin,"###.####;l"),")")
           ENDIF
          ELSE
           bilirubin = format(pat_record->pat_data[i].obs_value[x].bilirubin,"###.####;l"),
           prev_bilirubin = pat_record->pat_data[i].obs_value[x].bilirubin
          ENDIF
         ENDIF
         CALL print(calcpos(column,y_pos)), bilirubin
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(30,y_pos)), "ABG", row + 1,
       y_pos = (y_pos+ 10),
       CALL print(calcpos(30,y_pos)), "Intub",
       intub = "###########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60),
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(40,y_pos)), "FiO2", fio2 = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(40,y_pos)), "PaO2", pao2 = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(40,y_pos)), "PaCO2", paco2 = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(40,y_pos)), "PH", ph = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
       ...
       ENDFOR
       row + 1, y_pos = (y_pos+ 10), column = 120,
       CALL print(calcpos(40,y_pos)), "AaDO2", aado2 = "##########"
       FOR (x = start_indx TO col_max)
         column = (column+ 60)
  ...
  FOOT PAGE
   ...
   CALL print(calcpos(30,540)), "() = carryover value", row + 1,
