CREATE PROGRAM dm_esi_log_rows:dba
 ...
 IF (days_to_keep < 0)
  ...
  GO TO the_end
 ENDIF
 SET reply->table_name = "ESI_LOG"
 SET reply->rows_between_commit = 500
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(el.esi_log_id)
   FROM esi_log el
   WHERE el.esi_log_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    el.rowid
    FROM esi_log el
    WHERE el.esi_log_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("el.create_dt_tm < cnvtdatetime(curdate - days_to_keep, 0)",
      "el"))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,10)=1)
     ...
     ENDIF
     reply->rows[rows].row_id = el.rowid
    WITH nocounter, maxqual(el,value(rowsleft))
   ;end select
