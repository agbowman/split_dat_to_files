CREATE PROGRAM bbt_cs_get_codes:dba
 RECORD reply(
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   reply->qual[count1].begin_effective_dt_tm = cnvtdatetime(c.begin_effective_dt_tm),
   reply->qual[count1].end_effective_dt_tm = cnvtdatetime(c.end_effective_dt_tm)
  WITH counter
 ;end select
 IF (curqual != 0)
  SET reply->status_data.status = "S"
