CREATE PROGRAM cp_del_pathway:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CP_DEL_PATHWAY"
 DECLARE verifyinputs(null) = null WITH protect
 DECLARE deleterows(null) = i4 WITH protect
 ...
 SET reply->status_data.status = "F"
 CALL getparametervalues(2,pw_rec)
 IF (validate(debug_ind,0)=1)
  CALL echorecord(pw_rec)
 ENDIF
 CALL verifyinputs(null)
 IF (size(reply->p_qual,5) > 0)
  CALL deleterows(0)
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE xsize = i4 WITH protect, constant(size(pw_rec->qual,5))
   DECLARE x = i4 WITH protect, noconstant(0)
   IF (xsize > 0)
    FOR (x = 1 TO xsize)
      IF ((pw_rec->qual[x].value > 0))
       SET codevaluevalid = checkpathwayid(pw_rec->qual[x].value)
       IF (codevaluevalid=0)
        SET reply->status_data.status = "F"
        ...
        SET reply->status_data.subeventstatus.targetobjectvalue = build2("Pathway ID for list seq[",
         trim(cnvtstring(x),3),"]",trim(cnvtstring(pw_rec->qual[x].value),3),
      ...
      ELSE
       ...
       SET reply->status_data.subeventstatus.operationstatus = "F"
       SET reply->status_data.subeventstatus.targetobjectname = "Validate"
       SET reply->status_data.subeventstatus.targetobjectvalue = build2("CP_PATHWAY_ID for list seq ",
 ...
 END ;Subroutine
 SUBROUTINE (checkpathwayid(pwid=f8) =i4 WITH protect)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (cp
     WHERE cp.cp_pathway_id=pwid)
     JOIN (cn
     WHERE (cn.cp_pathway_id= Outerjoin(cp.cp_pathway_id)) )
     JOIN (cnb
     WHERE (cnb.cp_node_id= Outerjoin(cn.cp_node_id)) )
    HEAD REPORT
     i = size(reply->p_qual,5)
    HEAD cp.cp_pathway_id
     i += 1, lstat = alterlist(reply->p_qual,i), reply->p_qual[i].cp_pathway_id = cp.cp_pathway_id,
     reply->p_qual[i].pathway_name = trim(cp.pathway_name), reply->p_qual[i].pathway_type_cd = cp
     .pathway_type_cd, reply->p_qual[i].pathway_status_cd = cp.pathway_status_cd,
     j = 0
    HEAD cn.cp_node_id
     IF (cn.cp_pathway_id > 0.0)
      j += 1
      IF (j > size(reply->p_qual[i].n_qual,5))
       lstat = alterlist(reply->p_qual[i].n_qual,(j+ 10))
      ENDIF
      reply->p_qual[i].n_qual[j].cp_node_id = cn.cp_node_id
     ENDIF
     k = 0
    DETAIL
     IF (cnb.cp_node_id > 0.0)
      k += 1
      IF (k > size(reply->p_qual[i].n_qual[j].cnb_qual,5))
       lstat = alterlist(reply->p_qual[i].n_qual[j].cnb_qual,(k+ 10))
      ENDIF
      reply->p_qual[i].n_qual[j].cnb_qual[k].cp_node_behavior_id = cnb.cp_node_behavior_id
     ENDIF
    FOOT  cn.cp_node_id
     IF (k > 0)
      lstat = alterlist(reply->p_qual[i].n_qual[j].cnb_qual,k), reply->p_qual[i].n_qual[j].cnb_flag
       = 1
     ENDIF
    FOOT  cp.cp_pathway_id
     IF (j > 0)
      lstat = alterlist(reply->p_qual[i].n_qual,j), reply->p_qual[i].n_flag = 1
     ENDIF
    FOOT REPORT
     lstat = alterlist(reply->p_qual,i)
    WITH nocounter
   ;end select
 ...
 SUBROUTINE deleterows(null)
   ...
   DECLARE isize = i4 WITH protect, constant(size(reply->p_qual,5))
   ...
   IF (checkdic("CP_PATHWAY","T",0) != 2)
    ...
    SET reply->status_data.subeventstatus.operationname = "PERFORM"
    SET reply->status_data.subeventstatus.operationstatus = "F"
    SET reply->status_data.subeventstatus.targetobjectname = "DELETE"
   ...
   ENDIF
   IF (validate(debug_ind,0)=1)
    CALL echo("*** Deleting the following values... ***")
    CALL echorecord(reply)
   ENDIF
   FOR (i = 1 TO isize)
     IF ((reply->p_qual[i].n_flag=1))
      IF (size(reply->p_qual[i].n_qual,5) > 0)
       FOR (j = 1 TO size(reply->p_qual[i].n_qual,5))
         IF ((reply->p_qual[i].n_qual[j].cnb_flag=1))
          DELETE  FROM cp_node_behavior cnb
           WHERE (cnb.cp_node_id=reply->p_qual[i].n_qual[j].cp_node_id)
           WITH nocounter
          ;end delete
          CALL error_and_zero_check_rec(1,"DELETE","ERROR DELETING FROM CP_NODE_BEHAVIOR",1,1,
           reply)
         ENDIF
       ENDFOR
       DELETE  FROM cp_node cn
        WHERE (cn.cp_pathway_id=reply->p_qual[i].cp_pathway_id)
        WITH nocounter
       ;end delete
       CALL error_and_zero_check_rec(1,"DELETE","ERROR DELETING FROM CP_NODE",1,1,
        reply)
      ENDIF
     ENDIF
     DELETE  FROM cp_pathway cp
      WHERE (cp.cp_pathway_id=reply->p_qual[i].cp_pathway_id)
      WITH nocounter
     ;end delete
     CALL error_and_zero_check_rec(1,"DELETE","ERROR DELETING FROM CP_PATHWAY",1,1,
      reply)
   ENDFOR
