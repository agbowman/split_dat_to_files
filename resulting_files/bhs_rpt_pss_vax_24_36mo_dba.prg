CREATE PROGRAM bhs_rpt_pss_vax_24_36mo:dba
 PROMPT
 ...
 CALL echo(build2("mf_CS93_IMM: ",mf_cs93_imm))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (vese
   ...
   WHERE cv.code_value=vese.event_cd
    AND cv.active_ind=1
    AND cv.display_key IN ("ACTHIBOLDTERM", "DAPTACELDTAPOLDTERM", "DIPHTHHEPBPERTUSSISACELPOLIOTET",
   "DIPHTHHAEMOPHILUSPERTUSSISTETPOLIO", "DIPHTHPERTUSSISACELTETANUSPOLIO",
   "FLUVACCINE", "FLULAVALOLDTERM", "FLUMISTOLDTERM", "FLUARIXOLDTERM", "FLUVIRINOLDTERM",
   "FLUVIRINPRESERVATIVEFREEOLDTERM", "FLUZONEOLDTERM", "FLUZONEPRESERVATIVEFREEOLDTERM",
   "FLUZONEPRESERVATIVEFREEPEDIOLDTERM", "HAEMOPHILUSBHEPATITISBVACCINE",
   "HEPATITISAADULTVACCINE", "HEPATITISAPEDIATRICVACCINE", "HEPATITISAVACCINEOLDTERM",
   "HEPATITISAHEPATITISBVACCINE", "HEPATITISBIMMUNEGLOBULIN",
   "HEPATITISBIMMUNEGLOBULINOLDTERM", "HEPATITISBVACCINEOLDTERM", "HEPATITISBVACCINEOLDTERM",
   "HIBTITEROLDTERM", "INFANRIXDTAPOLDTERM",
   "INFANRIXDTAPPRESERVEFREEOLDTERM", "LIQUIDPEDVAXHIBOLDTERM", "MMRIIOLDTERM",
   "MEASLEMUMPRUBELLAVARICELLAOLDTERM", "MEASLESMUMPSRUBELLAVIRUSVACCINE",
   "MEASLESMUMPSRUBELLAVARICELLAVIRUSVAC", "POLIOVACCINELIVEOLDTERM", "POLIOVIRUSVACCINEINACTIVATED",
   "ROTAVIRUSVACCINE", "TRIPEDIADTAPOLDTERM",
   "VARICELLAVIRUSVACCINE", "DIPHTHHAEMHEPBPERTACELPOLIOTETAN", "DIPHTHERIATETANUSPERTUSSISACELDTAP",
   "HEPATITISBADULTVACCINE", "HEPATITISBPEDIATRICVACCINE",
   "INFLUVIRUSVACH1N1INACTIVEOLDTERM", "INFLUVIRUSVACH1N1LIVEOLDTERM",
   "INFLUENZAVIRUSVACCINEINACTIVATED", "INFLUENZAVIRUSVACCINELIVE"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (e
   ...
   JOIN (d
   WHERE p.birth_dt_tm BETWEEN cnvtlookbehind("36,M",e.reg_dt_tm) AND cnvtlookbehind("24,M",e
    .reg_dt_tm))
 ...
 SELECT INTO value( $OUTDEV)
  ...
  fin = substring(1,40,m_rec->enc[d.seq].s_fin), vaccines = substring(1,500,m_rec->enc[d.seq].s_vax)
