CREATE PROGRAM dm2_refresh_local_meta_data:dba
 ...
 SUBROUTINE drcm_get_pk_str(dgps_rec,dgps_tab_name)
   ...
   RETURN(dgps_ret_str)
 END ;Subroutine
 DECLARE dm_rmc_get_optimizer(null) = i4
 SUBROUTINE dm_rmc_get_optimizer(null)
   DECLARE drgo_optimizer_flag = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Querying for Oracle optimizer_mode"
   SELECT INTO "nl:"
    FROM v$parameter v
    WHERE v.name="optimizer_mode"
    DETAIL
     IF (cnvtupper(v.value)="RULE")
      drgo_optimizer_flag = 1
     ELSE
      drgo_optimizer_flag = 2
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(- (1))
   ELSEIF (drgo_optimizer_flag=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Error determining optimizer_mode"
    RETURN(- (1))
   ENDIF
   RETURN(drgo_optimizer_flag)
 END ;Subroutine
 ...
 ENDIF
 DECLARE ms_log_prefix = vc WITH protect, constant("dm2_rshloc_metadata")
 IF (check_logfile(ms_log_prefix,".log","DM2_REFRESH_LOCAL_META_DATA")=0)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Error creating log file"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (validate(request->remote_env_id,1234)=1234
  AND validate(request->remote_env_id,5678)=5678)
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "The required REQUEST structure was not defined before DM2_REFRESH_LOCAL_META_DATA execution."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DECLARE mf_mx_commit = i4 WITH protect, constant(1000)
 DECLARE rlmd_remote_env_id = f8 WITH protect, constant(request->remote_env_id)
 DECLARE rlmd_loc_env_id = f8 WITH protect, constant(request->local_env_id)
 DECLARE ms_source_tbl = vc WITH protect, noconstant("")
 DECLARE event_source_tbl = vc WITH protect, noconstant("")
 DECLARE rlmd_source_event = vc WITH protect, noconstant("")
 DECLARE merge_link = vc WITH protect, noconstant("")
 DECLARE rlmd_reason = vc WITH protect, noconstant("")
 DECLARE rlmd_event_ind = i4 WITH protect, noconstant(0)
 DECLARE rlmd_ptam_ind = i4 WITH protect, noconstant(0)
 DECLARE rlmd_optimizer_flag = i4 WITH protect, noconstant(0)
 DECLARE rlmd_table_lvl_ind = i2 WITH protect, noconstant(0)
 DECLARE rlmd_where_str = vc WITH protect, noconstant("")
 DECLARE rlmd_dtd_qual = i4 WITH protect, noconstant(0)
 DECLARE rlmd_dcd_qual = i4 WITH protect, noconstant(0)
 DECLARE rlmd_cnt = i4 WITH protect, noconstant(0)
 DECLARE rlmd_md_tbl = vc WITH protect, noconstant(" ")
 IF (validate(request->table_name,"RS_ITEM_EXISTS") != "RS_ITEM_EXISTS")
  IF ((request->table_name > " ")
   AND (request->table_name != char(42)))
   SET rlmd_table_lvl_ind = 1
  ENDIF
  SET dm_err->eproc = concat("Gathering metadata for ",request->table_name)
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF ((request->post_link_name != ""))
  SET merge_link = request->post_link_name
 ELSEIF (rlmd_remote_env_id != 0)
  SET dm_err->eproc = "Getting post_link_name from dm_env_reltn."
  SELECT INTO "NL:"
   FROM dm_env_reltn r
   WHERE r.relationship_type="REFERENCE MERGE"
    AND r.parent_env_id=rlmd_remote_env_id
    AND r.child_env_id=rlmd_loc_env_id
   DETAIL
    merge_link = r.post_link_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Checking open events for current environment."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET rlmd_event_ind = check_open_event(rlmd_loc_env_id,0.0)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (rlmd_event_ind=1)
  ...
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "There is an open event for the current environment.  Meta-data cannot be refreshed while an event is open."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSEIF (rlmd_remote_env_id > 0.0)
  SET rlmd_event_ind = check_open_event(rlmd_loc_env_id,rlmd_remote_env_id)
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ...
 SELECT INTO "NL:"
  ...
  WHERE relationship_type="PENDING TARGET AS MASTER"
   AND parent_env_id=rlmd_remote_env_id
   AND child_env_id=rlmd_loc_env_id
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ...
 IF (curqual > 0)
  SET rlmd_ptam_ind = 1
 ENDIF
 SET dm_err->eproc = "Checking for reverse existing PTAM relationship."
 SELECT INTO "NL:"
  FROM dm_env_reltn
  WHERE relationship_type="PENDING TARGET AS MASTER"
   AND parent_env_id=rlmd_loc_env_id
   AND child_env_id=rlmd_remote_env_id
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ...
 IF (curqual > 0)
  SET rlmd_ptam_ind = 1
 ENDIF
 SET rlmd_optimizer_flag = dm_rmc_get_optimizer(null)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Calling dm2_rdds_metadata_refresh for local tables ..."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF (rlmd_remote_env_id > 0
  AND rlmd_loc_env_id > 0)
  SET dm_err->eproc = "Calling dm2_rdds_metadata_refresh for source tables ..."
  CALL disp_msg(" ",dm_err->logfile,0)
  EXECUTE dm2_rdds_metadata_refresh merge_link
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
 ...
 IF (rlmd_event_ind=2
  AND rlmd_ptam_ind=1)
  SET rlmd_reason = "Remote tables"
 ELSEIF (rlmd_event_ind=0
  AND rlmd_loc_env_id > 0
  AND rlmd_remote_env_id=0)
  SET rlmd_reason = "Local Views"
 ELSE
  SET rlmd_reason = "Local and Remote Views"
 ENDIF
 IF (rlmd_event_ind=2
  AND rlmd_ptam_ind=1
  AND merge_link != "")
  SET ms_source_tbl = concat("DM_TABLES_DOC_LOCAL",merge_link)
 ELSEIF (merge_link != "")
  SET ms_source_tbl = concat("DM_RDDS_TBL_DOC",merge_link)
 ENDIF
 IF (rlmd_table_lvl_ind=0)
  CALL parser("rdb truncate table dm_tables_doc_local go",1)
  CALL parser("rdb truncate table dm_columns_doc_local go",1)
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ELSE
  DELETE  FROM dm_tables_doc_local
   WHERE table_name=patstring(request->table_name)
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  DELETE  FROM dm_columns_doc_local
   WHERE table_name=patstring(request->table_name)
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF (rlmd_table_lvl_ind=0)
  SET rlmd_where_str = "1 = 1"
 ELSE
  IF (findstring(request->table_name,char(42),1,1) > 0)
   SET rlmd_where_str = concat("<suffix>.table_name = patstring('",trim(request->table_name),"')")
  ELSE
   SET rlmd_where_str = concat("<suffix>.table_name = '",trim(request->table_name),"'")
  ENDIF
 ENDIF
 IF (rlmd_event_ind=0
  AND rlmd_loc_env_id > 0
  AND rlmd_remote_env_id=0)
  SET dm_err->eproc = "Inserting to dm_tables_doc_local using local view..."
  CALL disp_msg(" ",dm_err->logfile,0)
  INSERT  FROM dm_tables_doc_local d
   (d.table_suffix, d.full_table_name, d.suffixed_table_name,
   d.merge_ui_query, d.merge_active_ind, d.mergeable_ind,
   d.merge_delete_ind, d.reference_ind, d.data_model_section,
   d.table_name)(SELECT
    t.table_suffix, t.full_table_name, t.suffixed_table_name,
    t.merge_ui_query, t.merge_active_ind, t.mergeable_ind,
    t.merge_delete_ind, t.reference_ind, t.data_model_section,
    t.table_name
    FROM dm_rdds_tbl_doc t
    WHERE parser(replace(rlmd_where_str,"<suffix>","t",0)))
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
   ...
   GO TO exit_program
  ELSE
   SET dm_err->eproc = concat("Inserted ",trim(cnvtstring(curqual)),
    " rows into DM_TABLES_DOC_LOCAL from DM_RDDS_TBL_DOC.")
   CALL disp_msg(" ",dm_err->logfile,0)
   COMMIT
  ENDIF
  SET rlmd_md_tbl = "DM_RDDS_TBL_DOC"
 ELSEIF (rlmd_event_ind=2
  AND rlmd_ptam_ind=1)
  SET dm_err->eproc = "Inserting to dm_tables_doc_local using source view  ..."
  CALL disp_msg(" ",dm_err->logfile,0)
  INSERT  FROM dm_tables_doc_local d
   (d.table_suffix, d.full_table_name, d.suffixed_table_name,
   d.merge_ui_query, d.merge_active_ind, d.mergeable_ind,
  ...
  ELSE
   SET dm_err->eproc = concat("Inserted ",trim(cnvtstring(curqual)),
    " rows into DM_TABLES_DOC_LOCAL from ",ms_source_tbl)
   CALL disp_msg(" ",dm_err->logfile,0)
   COMMIT
  ENDIF
  SET rlmd_md_tbl = ms_source_tbl
 ELSE
  SET dm_err->eproc = "Inserting to dm_tables_doc_local using local view..."
  CALL disp_msg(" ",dm_err->logfile,0)
  INSERT  FROM dm_tables_doc_local d
   ...
   d.table_name)(SELECT
    ...
    (SELECT
     r.table_name
     FROM dm_rdds_tbl_doc r,
      (parser(ms_source_tbl) rs)
     WHERE r.table_name=rs.table_name
      AND r.override_instance <= rs.override_instance))
     AND parser(replace(rlmd_where_str,"<suffix>","t",0)))
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  ENDIF
  SET dm_err->eproc =
  "Inserting to dm_tables_doc_local using local view for tables that only have RDDS CODE requirements..."
  CALL disp_msg(" ",dm_err->logfile,0)
  INSERT  FROM dm_tables_doc_local d
   (d.table_suffix, d.full_table_name, d.suffixed_table_name,
   d.merge_ui_query, d.merge_active_ind, d.mergeable_ind,
   ...
   d.table_name)(SELECT
    ...
    (SELECT
     ror.table_name
     FROM dm_rdds_override_req ror
     WHERE ror.active_ind=1
      AND  NOT ( EXISTS (
     (SELECT
      dror.table_name
      FROM dm_rdds_override_req dror
      WHERE dror.table_name=ror.table_name
       AND ((dror.req_type != "CODE") OR (dror.req_name != "RDDS")) )))))
     AND  NOT ( EXISTS (
    (SELECT
     1
     FROM dm_tables_doc_local d1
     WHERE d1.table_name=t.table_name)))
     AND parser(replace(rlmd_where_str,"<suffix>","t",0)))
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Found ",trim(cnvtstring(rlmd_dtd_qual))," out of ",trim(cnvtstring(
    rlmd_cnt))," rows in DM_TABLES_DOC_LOCAL")
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((rlmd_dtd_qual < (rlmd_cnt * 0.9)))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "ERROR: Detected missing rows in DM_TABLES_DOC_LOCAL"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (rlmd_event_ind=2
  AND rlmd_ptam_ind=1
  AND merge_link != "")
  SET ms_source_tbl = concat("DM_COLUMNS_DOC_LOCAL",merge_link)
 ELSEIF (merge_link != "")
  SET ms_source_tbl = concat("DM_RDDS_COL_DOC",merge_link)
 ENDIF
 IF (rlmd_event_ind=0
  AND rlmd_loc_env_id > 0
  AND rlmd_remote_env_id=0)
  SET dm_err->eproc = "Inserting to dm_columns_doc_local using local view  ..."
  CALL disp_msg(" ",dm_err->logfile,0)
  INSERT  FROM dm_columns_doc_local d
   (d.sequence_name, d.code_set, d.unique_ident_ind,
   d.root_entity_name, d.root_entity_attr, d.constant_value,
   d.parent_entity_col, d.exception_flg, d.defining_attribute_ind,
   d.merge_delete_ind, d.table_name, d.column_name)(SELECT
    t.sequence_name, t.code_set, t.unique_ident_ind,
    t.root_entity_name, t.root_entity_attr, t.constant_value,
    t.parent_entity_col, t.exception_flg, t.defining_attribute_ind,
    t.merge_delete_ind, t.table_name, t.column_name
    FROM dm_rdds_col_doc t
    WHERE parser(replace(rlmd_where_str,"<suffix>","t",0)))
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
  ...
  ELSE
   SET dm_err->eproc = concat("Inserted ",trim(cnvtstring(curqual)),
    " rows into DM_COLUMNS_DOC_LOCAL from dm_rdds_col_doc")
   CALL disp_msg(" ",dm_err->logfile,0)
   COMMIT
  ENDIF
  SET rlmd_md_tbl = "DM_RDDS_COL_DOC"
 ELSEIF (rlmd_event_ind=2
  AND rlmd_ptam_ind=1)
  SET dm_err->eproc = "Inserting to dm_columns_doc_local using source view  ..."
  CALL disp_msg(" ",dm_err->logfile,0)
  INSERT  FROM dm_columns_doc_local d
   (d.sequence_name, d.code_set, d.unique_ident_ind,
   d.root_entity_name, d.root_entity_attr, d.constant_value,
  ...
  ELSE
   SET dm_err->eproc = concat("Inserted ",trim(cnvtstring(curqual)),
    " rows into DM_COLUMNS_DOC_LOCAL from ",ms_source_tbl)
   CALL disp_msg(" ",dm_err->logfile,0)
   COMMIT
  ENDIF
  SET rlmd_md_tbl = ms_source_tbl
  ...
  IF (rlmd_optimizer_flag=1)
   INSERT  FROM dm_columns_doc_local d
    ...
    d.merge_delete_ind, d.table_name, d.column_name)(SELECT
     ...
     t.merge_delete_ind, t.table_name, t.column_name
     FROM dm_rdds_col_doc t
     WHERE list(t.table_name,t.column_name) IN (
     (SELECT
      r.table_name, r.column_name
      FROM dm_rdds_col_doc r,
       (parser(ms_source_tbl) rs)
      WHERE r.table_name=rs.table_name
       AND r.column_name=rs.column_name
       AND r.override_instance <= rs.override_instance))
      AND parser(replace(rlmd_where_str,"<suffix>","t",0)))
    WITH nocounter
   ;end insert
   ...
   ELSE
    ...
    COMMIT
   ENDIF
   SET dm_err->eproc = "Inserting to dm_columns_doc_local for code only overrides  ..."
   CALL disp_msg(" ",dm_err->logfile,0)
   INSERT  FROM dm_columns_doc_local d
    (d.sequence_name, d.code_set, d.unique_ident_ind,
    d.root_entity_name, d.root_entity_attr, d.constant_value,
    ...
    d.merge_delete_ind, d.table_name, d.column_name)(SELECT
     ...
     (SELECT
      1
      FROM dm_columns_doc_local d1
      WHERE d1.table_name=t.table_name
       AND d1.column_name=t.column_name)))
      AND parser(replace(rlmd_where_str,"<suffix>","t",0)))
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
  ...
  ELSE
   INSERT  FROM dm_columns_doc_local d
    ...
    d.merge_delete_ind, d.table_name, d.column_name)(SELECT
     ...
     (SELECT
      "x"
      FROM (parser(ms_source_tbl) rs)
      WHERE t.table_name=rs.table_name
       AND t.column_name=rs.column_name
       AND t.override_instance <= rs.override_instance))
      AND parser(replace(rlmd_where_str,"<suffix>","t",0)))
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
   ...
   INSERT  FROM dm_columns_doc_local d
    ...
    d.merge_delete_ind, d.table_name, d.column_name)(SELECT
     ...
     (SELECT
      "x"
      FROM dm_rdds_override_req ror
      WHERE ror.table_name=t.table_name
       AND ror.active_ind=1
       AND  NOT ( EXISTS (
      (SELECT
       dror.table_name
       FROM dm_rdds_override_req dror
    ...
    d.merge_delete_ind, d.table_name, d.column_name)(SELECT
     ...
     (SELECT
      ...
      WHERE r.table_name=t.table_name
       ...
       AND r.override_instance > t.override_instance))
      AND  NOT ( EXISTS (
     (SELECT
      1
      FROM dm_columns_doc_local d1
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Found ",trim(cnvtstring(rlmd_dcd_qual))," out of ",trim(cnvtstring(
    rlmd_cnt))," rows in DM_COLUMNS_DOC_LOCAL")
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((rlmd_dcd_qual < (rlmd_cnt * 0.9)))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "ERROR: Detected missing rows in DM_COLUMNS_DOC_LOCAL"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dm_err->err_ind != 1)
  AND rlmd_table_lvl_ind=0)
  IF (rlmd_remote_env_id > 0
   AND rlmd_loc_env_id > 0
   AND merge_link > " ")
   EXECUTE dm_rmc_check_overrides merge_link
  ELSE
   EXECUTE dm_rmc_check_overrides ""
  ENDIF
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
   SET dm_err->err_ind = 1
  ...
  SET dm_get_request->run_qual[1].run_time_flag = 6
  ...
  ENDIF
  SET stat = alterlist(auto_ver_request->qual,1)
  SET auto_ver_request->qual[1].rdds_event = "Local Meta-Data Refresh"
  SET auto_ver_request->qual[1].cur_environment_id = rlmd_loc_env_id
  SET auto_ver_request->qual[1].paired_environment_id = rlmd_remote_env_id
  SET auto_ver_request->qual[1].event_reason = rlmd_reason
  EXECUTE dm_rmc_auto_verify_setup
  IF ((auto_ver_reply->status="F"))
   ROLLBACK
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_program
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "Ending dm2_refresh_local_meta_data"
  CALL final_disp_msg(ms_log_prefix)
 ENDIF
END GO
