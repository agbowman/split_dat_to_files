CREATE PROGRAM bbd_rdm_updt_code_values:dba
 ...
 SUBROUTINE insertreqproc(nrequest,spfmt,ntargetreq,ndeststep,sservice,nreprocess,naction,srefpfmt,
  nseq)
   ...
   ENDCASE
 END ;Subroutine
 DECLARE script_name = c23 WITH protect, constant("bbd_rdm_updt_code_values")
 ...
 UPDATE  FROM code_value cv,
  ...
  SET cv.display = trim(requestin->list_0[d1.seq].display), cv.display_key = cnvtupper(cnvtalphanum(
     trim(requestin->list_0[d1.seq].display))), cv.description = trim(requestin->list_0[d1.seq].
    description),
   cv.definition = trim(requestin->list_0[d1.seq].definition)
  PLAN (d1
   WHERE size(trim(requestin->list_0[d1.seq].cki)) > 0)
   JOIN (cv
   WHERE cv.code_set=cnvtint(trim(requestin->list_0[d1.seq].code_set))
    AND cv.cdf_meaning=trim(requestin->list_0[d1.seq].cdf_meaning)
    AND cv.display=trim(requestin->list_0[d1.seq].old_display)
    AND cv.cki=trim(requestin->list_0[d1.seq].cki)
    AND cv.definition=trim(requestin->list_0[d1.seq].old_definition))
  WITH counter
 ;end update
 IF (error(error_msg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error updating first code_value load: ",error_msg)
  CALL logmsg(readme_data->message,((e_dispmsg+ e_logtofile)+ e_rdmstatus))
  GO TO script_end
 ENDIF
 CALL logmsg("Finished updating valid CKI values.",(e_dispmsg+ e_logtofile))
 UPDATE  FROM code_value cv,
   (dummyt d1  WITH seq = value(size(requestin->list_0,5)))
  SET cv.display = trim(requestin->list_0[d1.seq].display), cv.display_key = cnvtupper(cnvtalphanum(
     trim(requestin->list_0[d1.seq].display))), cv.description = trim(requestin->list_0[d1.seq].
 ...
 IF (error(error_msg,0) != 0)
  ...
  GO TO script_end
 ENDIF
 CALL logmsg("Finished updating where there are no valid CKI values.",(e_dispmsg+ e_logtofile))
