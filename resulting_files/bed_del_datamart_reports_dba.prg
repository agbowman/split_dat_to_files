CREATE PROGRAM bed_del_datamart_reports:dba
 ...
 SUBROUTINE delreportfilters(null)
   ...
   SELECT INTO "nl:"
    ...
    HEAD r.br_datamart_filter_id
     filtercnt = (filtercnt+ 1), stat = alterlist(delfilterrequest->filters,filtercnt),
     delfilterrequest->filters[filtercnt].br_datamart_filter_id = r.br_datamart_filter_id
    WITH nocounter
   ;end select
   CALL bederrorcheck("Failed to get filter ids.")
   EXECUTE bed_del_datamart_filters  WITH replace("REQUEST",delfilterrequest)
