CREATE PROGRAM acm_add_prsnl_reltn:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->prsnl_reltn_qual[xref->add[index].idx].prsnl_reltn_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(person_seq,nextval)
     FROM dual
     DETAIL
      acm_request->prsnl_reltn_qual[xref->add[index].idx].prsnl_reltn_id = cnvtreal(nextseqnum),
      reply->prsnl_reltn_qual[xref->add[index].idx].prsnl_reltn_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->prsnl_reltn_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->prsnl_reltn_qual[xref->add[index].idx].prsnl_reltn_id = acm_request->prsnl_reltn_qual[
    xref->add[index].idx].prsnl_reltn_id
   ENDIF
   SET reply->prsnl_reltn_qual[xref->add[index].idx].status = 0
   IF ((acm_request->prsnl_reltn_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->prsnl_reltn_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->prsnl_reltn_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->prsnl_reltn_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   prsnl_reltn p
  SET p.prsnl_reltn_id = acm_request->prsnl_reltn_qual[xref->add[d.seq].idx].prsnl_reltn_id, p
   .person_id = acm_request->prsnl_reltn_qual[xref->add[d.seq].idx].person_id, p.parent_entity_id =
   acm_request->prsnl_reltn_qual[xref->add[d.seq].idx].parent_entity_id,
   p.parent_entity_name = acm_request->prsnl_reltn_qual[xref->add[d.seq].idx].parent_entity_name, p
   .display_seq = acm_request->prsnl_reltn_qual[xref->add[d.seq].idx].display_seq, p.reltn_type_cd =
   acm_request->prsnl_reltn_qual[xref->add[d.seq].idx].reltn_type_cd,
   p.beg_effective_dt_tm = cnvtdatetime(acm_request->prsnl_reltn_qual[xref->add[d.seq].idx].
    beg_effective_dt_tm), p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_reltn_qual[xref->
    add[d.seq].idx].end_effective_dt_tm), p.active_ind = 1,
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->prsnl_reltn_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->prsnl_reltn_qual[xref->add[index].idx].status=0))
