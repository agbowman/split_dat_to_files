CREATE PROGRAM bhs_ma_multum_dai_audit_v2
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ede
   WHERE trim(ede.dlg_name)="MUL_MED!DRUGALLERGY"
    AND ede.trigger_order_id IN (407684405, 406263305, 407624756, 402925943, 402660939,
   404998097, 402034556, 411508879, 404593946, 407731104,
   411089870, 402080435, 405848453, 402371536, 405405633,
   410367751, 407517824, 411512040, 410707939, 406404536,
   408667262, 411951176, 407716215, 405111111, 408196936,
   408079412, 409379297, 405014258, 411932425, 402558616,
   401322494, 401322494, 405274896, 408543990, 410963042,
   408073545, 406322648, 404325858, 404646878, 411836366,
   400878925, 411961330, 405464637, 405715211, 402155272,
   400557405, 406645305, 406263484, 409558867, 397311848,
   410577831, 411293348, 402867675, 411402192, 405014638,
   402154063, 402691589, 410800429, 405992061, 411896350,
   411508329, 410256597, 398913209, 404700082, 410849879,
   411024491, 406449100, 406398155, 411402016, 403565483,
   404980864, 405873756, 409918164, 401465249, 402283625,
   404328101, 403301811, 410962192, 402925815, 400479058,
   404118438, 409935041, 408211097, 409315453, 406352036,
   411081831, 411611943, 402432135, 410463506, 405342009,
   403331958, 410526518, 404917393, 411805806, 408073348,
   411518855))
   JOIN (edea
  ...
  HEAD ede.trigger_entity_id
   ...
   t_record->list[t_record->list_cnt].prsnl_name = pr.name_full_formatted, t_record->list[t_record->
   list_cnt].prsnl_position_cd = pr.position_cd, t_record->list[t_record->list_cnt].fmrn = trim(
    cnvtalias(ea.alias,ea.alias_pool_cd)),
   t_record->list[t_record->list_cnt].patient_name = p.name_full_formatted, t_record->list[t_record->
   list_cnt].facility_cd = e.loc_facility_cd, t_record->list[t_record->list_cnt].active_ind = 1
  DETAIL
   IF (ede.long_text_id > 0)
    t_record->list[t_record->list_cnt].override_rsn_lt_id = ede.long_text_id
   ENDIF
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (a
   WHERE (a.allergy_id=t_record->list[d.seq].allergy_id))
   JOIN (n
  ...
  DETAIL
   ...
   t_record->list[d.seq].substance_type_cd = a.substance_type_cd, t_record->list[d.seq].
   reaction_class_cd = a.reaction_class_cd, t_record->list[d.seq].severity_cd = a.severity_cd
  WITH nocounter
 ;end select
 ...
 IF (findfile("dai_detail.xls")=1
  ...
  SET dclcom = "gzip dai_detail.xls"
  SET len = size(trim(dclcom))
  SET status = 0
  SET stat = dcl(dclcom,len,status)
  ...
  SET dclcom = concat('echo " " | mailx -s "',subject_line,'" ','-a "dai_detail.xls.gz" ',
