CREATE PROGRAM djh_person_name
 PROMPT
 ...
 ENDIF
 SELECT INTO  $OUTDEV
  pr.person_id, pr.name_last, pr.position_cd,
  ...
  p.name_first, p.name_first_key, p.name_full,
  p.name_initials, p.name_last, p.name_last_key,
  p.name_middle, p.name_middle_key, p.name_original,
  p.name_prefix, p.name_suffix, p.name_title,
  p.name_type_cd, p_name_type_disp = uar_get_code_display(p.name_type_cd), p.name_type_seq,
  p.person_id, p.person_name_id, p.updt_dt_tm,
  p.updt_id
  FROM prsnl pr,
   person_name p
  PLAN (pr
