CREATE PROGRAM dm_ecmb_encntr_cmnty_case:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "ENCNTR_CMNTY_CASE"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_ECMB_ENCNTR_CMNTY_CASE"
 ...
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count2].to_id = tu.encntr_cmnty_case_id
   FOOT REPORT
    IF (mod(v_cust_count2,10) != 0)
    ...
    IF (v_cust_count2 > 0)
     IF ((rreclist->from_rec[v_cust_loopcount].active_ind=1))
      IF (del_from(rreclist->from_rec[v_cust_loopcount].from_id,request->xxx_combine[icombine].
       ...
       GO TO exit_sub
      ENDIF
     ELSEIF ((rreclist->from_rec[v_cust_loopcount].active_ind=0))
 ...
 SUBROUTINE del_from(s_df_pk_id,s_df_to_fk_id,s_df_prev_act_ind,s_df_prev_act_status)
   UPDATE  FROM encntr_cmnty_case frm
    ...
    WHERE frm.encntr_cmnty_case_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "ENCNTR_CMNTY_CASE"
 ...
 SUBROUTINE upt_from(s_uf_pk_id,s_uf_to_fk_id)
   UPDATE  FROM encntr_cmnty_case frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.encntr_cmnty_case_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
