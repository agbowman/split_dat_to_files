CREATE PROGRAM bhs_ma_merge_list_rpt_ops:dba
 RECORD data(
 ...
 DECLARE equal_line = vc WITH public, constant(fillstring(12,"="))
 ...
 SELECT INTO "nl:"
  ...
  FROM person_combine pc,
   ...
   diagnosis dg
  PLAN (pc
   WHERE pc.updt_dt_tm >= cnvtdatetime((curdate - 1),curtime)
    AND pc.updt_dt_tm <= cnvtdatetime(curdate,curtime)
    AND pc.active_ind=1)
   JOIN (p
 ...
 SELECT INTO mine
  ...
  DETAIL
   FOR (x = 1 TO data->person_qual_cnt)
     ...
     CALL print(calcpos(xcol1,yrow)), "BWH MR #:",
     CALL print(calcpos(xcol3,yrow)),
     data->person_qual[x].from_bwh_mrn,
     CALL print(calcpos(xcol5,yrow)), "BWH MR #:",
     CALL print(calcpos(xcol7,yrow)), data->person_qual[x].to_bwh_mrn, rowplusone,
