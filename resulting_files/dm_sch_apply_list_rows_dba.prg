CREATE PROGRAM dm_sch_apply_list_rows:dba
 ...
 IF (v_days_to_keep < g_minimum_keep_days)
  ...
  SET reply->status_data.status = "F"
 ELSE
  SET reply->table_name = "SCH_APPLY_LIST"
  SET reply->rows_between_commit = g_rows_between_commit
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(sal.apply_list_id)
    FROM sch_apply_list sal
    WHERE sal.apply_list_id > 0
    DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     sal.rowid
     FROM sch_apply_list sal
     WHERE sal.apply_list_id BETWEEN curminid AND curmaxid
      AND sal.def_beg_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
      AND parser(sbr_getrowidnotexists("sal.version_dt_tm = cnvtdatetime('31-DEC-2100 00:00:00.00')",
       "sal"))
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,100)=1)
      ...
      ENDIF
      reply->rows[rows].row_id = sal.rowid
     WITH nocounter, maxqual(sal,value(rowsleft))
    ;end select
