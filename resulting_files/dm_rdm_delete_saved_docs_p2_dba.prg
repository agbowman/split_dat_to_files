CREATE PROGRAM dm_rdm_delete_saved_docs_p2:dba
 ...
 DECLARE sbr_update_dm_info(ps_domain=vc,ps_name=vc,ps_char=vc) = i2
 DECLARE sbr_insert_dm_info(ps_domain=vc,ps_name=vc,ps_char=vc) = i2
 DECLARE sbr_delete_dm_info(ps_domain=vc) = i2
 DECLARE sbr_wipe_ranges(ms_info_domain_nm=vc,ms_child_prefix=vc) = i2
 DECLARE sbr_count_children(ms_info_domain_nm=vc) = i4
 DECLARE sbr_parallel_debug_echo(ms_msg=vc) = null
 ...
 ENDIF
 SUBROUTINE sbr_insert_dm_info(ps_domain,ps_name,ps_char)
   ...
   INSERT  FROM dm_info di
    SET di.info_domain = ps_domain, di.info_name = ps_name, di.info_number = 0,
     di.info_date = cnvtdatetime(curdate,curtime3), di.info_char = ps_char
    WITH nocounter
   ;end insert
   IF (error(errmsg,0) != 0)
    ROLLBACK
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_update_dm_info(ps_domain,ps_name,ps_char)
   ...
   UPDATE  FROM dm_info di
    SET di.info_number = 0, di.info_date = cnvtdatetime(curdate,curtime3), di.info_char = ps_char
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_delete_dm_info(ps_domain)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_wipe_ranges(ms_info_domain_nm,ms_child_prefix)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_count_children(ms_info_domain_nm)
   ...
   RETURN(mn_count)
 END ;Subroutine
 SUBROUTINE sbr_parallel_debug_echo(ms_msg)
   IF (dm2_rdm_parallel_debug_ind=1)
    ...
    IF (ms_msg=patstring("Entering*?"))
     SET mn_num_tabs = (mn_num_tabs+ 1)
    ENDIF
   ENDIF
 END ;Subroutine
 DECLARE sbr_create_ccl_tmp_def(tbl_name=vc) = i2
 DECLARE sbr_drop_temp_table(ps_tbl_name=vc) = i2
 DECLARE sbr_sync_ccl_tmp_def(tbl_name=vc) = i2
 DECLARE sbr_create_temp_table(ps_tbl_name=vc) = null
 DECLARE sbr_add_sql_stmt(ms_obj_type=vc,ms_obj_name=vc,ms_sql=vc,ms_message=vc) = null
 DECLARE sbr_add_dropped_object(ms_obj_type=vc,ms_obj_name=vc,ms_message=vc) = null
 DECLARE sbr_run_sql(null) = i2
 DECLARE sbr_drop_sql_objects(null) = i2
 DECLARE sbr_truncate_table(tbl_name=vc) = i2
 DECLARE sbr_table_exists(tbl_name=vc) = i2
 ...
 SUBROUTINE sbr_drop_sql_objects(null)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_add_dropped_object(ms_obj_type,ms_obj_name,ms_message)
 ...
 END ;Subroutine
 SUBROUTINE sbr_add_sql_stmt(ms_obj_type,ms_obj_name,ms_sql,ms_message)
 ...
 END ;Subroutine
 SUBROUTINE sbr_sync_ccl_tmp_def(tbl_name)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_create_ccl_tmp_def(tbl_name)
 ...
 SUBROUTINE sbr_drop_temp_table(ps_tbl_name)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_truncate_table(tbl_name)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_table_exists(tbl_name)
 ...
 DECLARE sbr_run_child(ms_info_domain_nm=vc,ms_child_prefix=vc,ms_child_max_name=vc,ms_cc_script=vc,
  mf_max_range_id=f8,
  mf_min_range_id=f8) = i2
 DECLARE sbr_fetch_min_max(ms_info_domain_nm=vc,ms_child_prefix=vc,ms_child_max_name=vc,
  mf_max_range_id=f8(ref),mf_min_range_id=f8(ref)) = i2
 DECLARE sbr_claim_min_max_row(ms_info_domain_nm=vc,ms_child_prefix=vc,ms_child_max_name=vc) = i2
 DECLARE sbr_add_stats_row(mf_readme_id=i4,ms_range_name=vc,mf_min_batch=f8(ref),mf_max_batch=f8(ref)
  ) = i2
 ...
 SUBROUTINE sbr_dm2_rdm_resume_off_chk(null)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE sbr_claim_min_max_row(ms_info_domain_nm,ms_child_prefix,ms_child_max_name)
    ...
    IF (curqual=1)
     ...
     INSERT  FROM dm_info di
      SET di.info_domain = ms_info_domain_nm, di.info_name = concat(ms_child_max_name," ",cnvtstring(
       ...
       di.info_long_id = mf_range_id_increment, di.info_date = cnvtdatetime(curdate,curtime3), di
       .updt_cnt = 0,
   ...
   RETURN(0)
 END ;Subroutine
 SUBROUTINE sbr_fetch_min_max(ms_info_domain_nm,ms_child_prefix,ms_child_max_name,mf_max_range_id,
  mf_min_range_id)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_run_child(ms_info_domain_nm,ms_child_prefix,ms_child_max_name,ms_cc_script,
  mf_max_range_id,mf_min_range_id)
   ...
   WHILE (mf_cur_min_id <= mf_max_range_id)
     SET mf_runtime = 0
     SET dq_starttime = cnvtdatetime(curdate,curtime3)
     ...
     ELSEIF (mn_deadlock_ind=1)
      ...
      SET mn_deadlock_cnt = (mn_deadlock_cnt+ 1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_add_stats_row(mf_readme_id,ms_range_name,mf_min_batch,mf_max_batch)
   ...
   IF (curqual=0)
    ...
    INSERT  FROM dm_parallel_readme_stats dprs
     SET dprs.readme_stats_id = seq(dm_ref_seq,nextval), dprs.readme_id = mf_readme_id, dprs
      ...
      dprs.updt_dt_tm = cnvtdatetime(curdate,curtime3), dprs.updt_id = reqinfo->updt_id, dprs
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE sbr_run_parent2(mf_max_range_id=f8,ms_range_name=vc,ms_max_name=vc,ms_info_domain_nm=vc,
  ms_child_script=vc,
  mn_num_children=i4,full_scan_ind=i2) = i4
 DECLARE sbr_get_max_id(ms_info_domain_nm=vc) = f8
 DECLARE sbr_p2_check_for_prev_rdm(readme_id=i4,instance=i4) = i4
 SUBROUTINE sbr_get_max_id(ms_info_domain_nm)
   ...
   RETURN(max_id_eval)
 END ;Subroutine
 SUBROUTINE sbr_run_parent2(mf_max_range_id,ms_range_name,ms_max_name,ms_info_domain_nm,
  ms_child_script,mn_num_children,full_scan_ind)
   ...
   IF (full_scan_ind=0)
    ...
    IF (mf_min_range_id=0)
     ...
     RETURN(0)
    ELSE
     SET mf_min_range_id = (mf_min_range_id+ 1)
     CALL sbr_parallel_debug_echo(concat("Minimum ID: ",cnvtstring(mf_min_range_id)))
    ENDIF
   ENDIF
   IF (mf_min_range_id > mf_max_range_id)
    SET readme_data->status = "S"
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_p2_check_for_prev_rdm(readme_id,instance)
 ...
 DECLARE event_class_cd_doc = f8 WITH protect, noconstant(0.0)
 DECLARE event_class_cd_mdoc = f8 WITH protect, noconstant(0.0)
 DECLARE event_class_cd_grpdoc = f8 WITH protect, noconstant(0.0)
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 IF (((error(ms_errmsg,0) != 0) OR (task_status_pending_cd <= 0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error loading code value PENDING from codeset 79: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
  ...
  WITH nocounter
 ;end select
 IF (((error(ms_errmsg,0) != 0) OR (task_type_cd <= 0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error loading value ENDORSE from codeset 6026: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
  ...
  DETAIL
   event_class_cd_doc = cv.code_value
  WITH nocounter
 ;end select
 IF (((error(ms_errmsg,0) != 0) OR (event_class_cd_doc <= 0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error loading code value DOC from codeset 53: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
  ...
  DETAIL
   event_class_cd_mdoc = cv.code_value
  WITH nocounter
 ;end select
 IF (((error(ms_errmsg,0) != 0) OR (event_class_cd_mdoc <= 0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error loading code value MDOC from codeset 53: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
  ...
  PLAN (cv
   WHERE cv.code_set=53
    AND cv.cdf_meaning="GRPDOC")
  DETAIL
   event_class_cd_grpdoc = cv.code_value
  WITH nocounter
 ;end select
 IF (((error(ms_errmsg,0) != 0) OR (event_class_cd_grpdoc <= 0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error loading code value GRPDOC from codeset 53: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
  ...
  WITH nocounter
 ;end select
 IF (((error(ms_errmsg,0) != 0) OR (task_activity_cd_saved_doc <= 0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error loading code value SAVED DOC from codeset 6027: ",
   ms_errmsg)
  GO TO exit_program
 ENDIF
 SET ms_info_domain_nm = "DM_RDM_DELETE_NON_VIEWABLE_SAVED_DOCS"
 SET ms_range = "FINAL RANGE"
 SET ms_max_name_p = "MAX TASK_ACTIVITY_ID EVALUATED"
 SET ms_child_script = "dm_rdm_delete_saved_docs_cc"
 SET mn_num_children = 10
 SELECT INTO "n1:"
  max_id = max(ta.task_id)
  ...
  WHERE ta.active_ind=1
   ...
   AND  NOT (ta.event_class_cd IN (event_class_cd_doc, event_class_cd_mdoc, event_class_cd_grpdoc))
  DETAIL
   mf_max_range_id = cnvtreal(max_id)
  WITH nocounter
 ;end select
 SET mn_full_range_scan_ind = 0
 IF (sbr_run_parent2(mf_max_range_id,ms_range,ms_max_name_p,ms_info_domain_nm,ms_child_script,
  mn_num_children,mn_full_range_scan_ind) IN (2, 0))
  GO TO exit_program
 ELSE
  SET readme_data->status = "S"
...
#exit_program
 ...
 ELSE
  COMMIT
  CALL sbr_add_dropped_object("INDEX","XIETEMP_TASK_ACTIVITY","Dropping index for task id")
  CALL sbr_add_dropped_object("TABLE","TEMP_TASK_ACTIVITY","Dropping table")
