CREATE PROGRAM dm2_capture_schema:dba
 DECLARE dm2_mod = vc WITH private, constant("028")
 DECLARE def_tspace = vc WITH public
 DECLARE def_itspace = vc WITH public
 DECLARE def_ltspace = vc WITH public
 DECLARE dcs_source_rdbms = vc WITH public, noconstant("")
 ...
 SUBROUTINE drr_del_preserved_ts(dcdl_tgt_db_name)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE sf_ver = i4 WITH protect, constant(1)
 DECLARE dcs_cerinstall = vc WITH protect, constant("cer_install")
 DECLARE cs_sch_date_str = vc WITH protect
 DECLARE cs_sch_date_fmt = f8 WITH protect
 DECLARE cs_table_prefix = vc WITH protect
 DECLARE cs_sch_date_type = vc WITH protect
 DECLARE cs_file_prefix = vc WITH protect
 DECLARE cs_select_dir = vc WITH protect
 DECLARE mod_ind = i2 WITH protect, noconstant(1)
 DECLARE sf_desc = vc WITH protect
 DECLARE dcs_lnksrv = vc WITH public
 DECLARE dcs_inhouse_oracle_capture = i2 WITH pubilc, noconstant(0)
 DECLARE dcs_pull_from_dm_tables = i2 WITH public, noconstant(0)
 DECLARE dm2h_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2tsp_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2tbl_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2col_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2ind_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2icol_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2cons_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2ccol_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2seq_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2tdoc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2cdoc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dm2dtp_cnt = i4 WITH protect, noconstant(0)
 DECLARE cs_tcnt = i4 WITH protect, noconstant(0)
 DECLARE cs_tc_cnt = i4 WITH protect, noconstant(0)
 DECLARE cs_icnt = i4 WITH protect, noconstant(0)
 DECLARE cs_ic_cnt = i4 WITH protect, noconstant(0)
 DECLARE cs_ccnt = i4 WITH protect, noconstant(0)
 DECLARE cs_cc_cnt = i4 WITH protect, noconstant(0)
 DECLARE cs_seq_cnt = i4 WITH protect, noconstant(0)
 DECLARE val_write_privs(vwp_dir=vc,vwp_full_dir=vc) = i4
 DECLARE check_user_privs(null) = i2
 DECLARE init_schema_files(null) = i2
 DECLARE insert_dmheader(null) = i2
 DECLARE populate_sf_from_cs(null) = i2
 DECLARE get_dm_tables_schema(sbr_gdts_sch_dt=vc) = null
 DECLARE update_schema_instance(null) = i2
 DECLARE dcs_adj_for_clin_copy(null) = i2
 CALL check_logfile("dm2_capture_schema",".log","DM2_CAPTURE_SCHEMA LOG FILE")
 SET dm_err->eproc = "Begin dm2_capture_schema"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (validate(dm2_inhouse_oracle_capture,0)=1)
  SET dcs_inhouse_oracle_capture = 1
 ENDIF
 IF (validate(validate_schema_initiated_script,- (1))=1)
  IF ((dm_err->debug_flag > 0))
   SET dm_err->eproc = "DM2_CAPTURE_SCHEMA called from DM2_CAPTURE_DESIRED_SCHEMA."
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SET dcs_pull_from_dm_tables = 1
 ENDIF
 SET cs_sch_date_str =  $1
 SET sf_desc = cnvtupper(substring(1,30, $2))
 SET cs_table_prefix =  $3
 SET cs_select_dir = dcs_cerinstall
 IF (check_user_privs(null)=0)
  GO TO exit_script
 ENDIF
 SET dcs_source_rdbms = currdb
 IF (init_schema_files(null)=0)
  GO TO exit_script
 ENDIF
 IF (insert_dmheader(null)=0)
  GO TO exit_script
 ENDIF
 IF (dcs_pull_from_dm_tables=1)
  IF (get_dm_tables_schema(cs_sch_date_str)=0)
   GO TO exit_script
  ENDIF
 ELSE
  SET dm_err->eproc = "LOAD SNAPSHOT OF DATABASE SCHEMA INTO MEMORY"
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dm2_install_schema->schema_prefix = cs_sch_date_type
  SET dm2_install_schema->file_prefix = cnvtalphanum(format(cs_sch_date_fmt,"MM/DD/YYYY;;D"))
  EXECUTE dm2_get_cursch value(cs_table_prefix)
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
 ENDIF
 IF (populate_sf_from_cs(null)=0)
  GO TO exit_script
 ENDIF
 IF (dcs_pull_from_dm_tables=1)
  IF (update_schema_instance(null)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((dm_err->debug_flag > 0)
  ...
  SELECT INTO "nl:"
   ...
   FROM dmcons dcons
   DETAIL
    dm2cons_cnt = (dm2cons_cnt+ 1)
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   dccol.*
   FROM dmconscol dccol
   DETAIL
    dm2ccol_cnt = (dm2ccol_cnt+ 1)
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   ds.*
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    WHERE (cur_sch->tbl[d.seq].capture_ind=1))
   DETAIL
    cs_tcnt = (cs_tcnt+ 1), cs_tc_cnt = (cs_tc_cnt+ cur_sch->tbl[d.seq].tbl_col_cnt), cs_icnt = (
    cs_icnt+ cur_sch->tbl[d.seq].ind_cnt)
    FOR (x = 1 TO cur_sch->tbl[d.seq].ind_cnt)
      cs_ic_cnt = (cs_ic_cnt+ cur_sch->tbl[d.seq].ind[x].ind_col_cnt)
    ENDFOR
    cs_ccnt = (cs_ccnt+ cur_sch->tbl[d.seq].cons_cnt)
    FOR (x = 1 TO cur_sch->tbl[d.seq].cons_cnt)
      cs_cc_cnt = (cs_cc_cnt+ cur_sch->tbl[d.seq].cons[x].cons_col_cnt)
    ENDFOR
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   d.seq
   FROM (dummyt d  WITH seq = value(cur_sch->sequence_cnt))
   PLAN (d
    WHERE (cur_sch->sequence[d.seq].capture_ind=1))
   DETAIL
    cs_seq_cnt = (cs_seq_cnt+ 1)
   WITH nocounter
  ;end select
  SELECT INTO value(dm_err->logfile)
   ...
   PLAN (d)
   HEAD REPORT
    row + 2, col 30, "Tables in exception list",
    row + 1
   DETAIL
    row + 1, dm2_sch_except->tbl[d.seq].tbl_name
   WITH nocounter, append
  ;end select
  SELECT INTO value(dm_err->logfile)
   d.seq
   FROM (dummyt d  WITH seq = value(dm2_sch_except->seq_cnt))
   PLAN (d)
   HEAD REPORT
    row + 2, col 30, "Sequences in exception list",
    row + 1
   DETAIL
    row + 1, dm2_sch_except->seq[d.seq].seq_name
   WITH nocounter, append
  ;end select
  SELECT INTO value(dm_err->logfile)
   d.seq
   FROM (dummyt d  WITH seq = value(cur_sch->tbl_cnt))
   PLAN (d
    WHERE (cur_sch->tbl[d.seq].capture_ind=0))
   HEAD REPORT
    row + 2, col 30, "Tables with capture_ind = 0",
    row + 1
   DETAIL
    row + 1, cur_sch->tbl[d.seq].tbl_name
   WITH nocounter, append
  ;end select
  SELECT INTO value(dm_err->logfile)
   d.seq
   ...
   PLAN (d)
   HEAD REPORT
    row + 2, col 18, "Counts of rows in schema files vs. Cur_Sch record where capture_ind = 1",
    row + 1, row + 1, "DMHEADER:",
    col 20, dm2h_cnt, row + 1,
    "DMTABLE:", col 20, dm2tbl_cnt,
    col 35, "Cur_Sch table cnt:", col 70,
    cs_tcnt, row + 1, "DMCOLUMN:",
    col 20, dm2col_cnt, col 35,
    "Cur_Sch column cnt:", col 70, cs_tc_cnt,
    row + 1, "DMINDEX:", col 20,
    dm2ind_cnt, col 35, "Cur_Sch index cnt:",
    col 70, cs_icnt, row + 1,
    "DMINDCOL:", col 20, dm2icol_cnt,
    col 35, "Cur_Sch index column cnt:", col 70,
    cs_ic_cnt, row + 1, "DMCONS:",
    col 20, dm2cons_cnt, col 35,
    "Cur_Sch constraint cnt:", col 70, cs_ccnt,
    row + 1, "DMCONSCOL:", col 20,
    dm2ccol_cnt, col 35, "Cur_Sch cons column cnt:",
    col 70, cs_cc_cnt, row + 1,
    "DMSEQ:", col 20, dm2seq_cnt,
    col 35, "Cur_Sch sequence cnt:", col 70,
    cs_seq_cnt, row + 1, "DMTBLDOC:",
    col 20, dm2tdoc_cnt, row + 1,
    "DMCOLDOC:", col 20, dm2cdoc_cnt,
    row + 1, "DMTSPREC:", col 20,
    dm2dtp_cnt, row + 2, " "
   WITH nocounter, append
  ;end select
 ENDIF
 ...
 ENDIF
 GO TO exit_script
 SUBROUTINE val_write_privs(vwp_dir,vwp_full_dir)
   DECLARE dname_full = vc WITH protect
   DECLARE full_fname = vc WITH protect
   DECLARE prev_user_action = vc WITH protect
   CASE (dm2_sys_misc->cur_os)
    OF "AXP":
     SET dname_full = build(vwp_dir,":")
    OF "WIN":
     SET dname_full = build(vwp_full_dir,"\")
    ELSE
     SET dname_full = build(vwp_full_dir,"/")
   ENDCASE
   SET prev_user_action = dm_err->user_action
   IF (get_unique_file("dm2tst",".dat")=0)
    RETURN(0)
   ENDIF
   SET full_fname = build(vwp_dir,":",cnvtlower(dm_err->unique_fname))
   SELECT INTO value(full_fname)
    d.seq
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     row + 1, "This is a test of writing to ", vwp_dir
    WITH nocounter
   ;end select
   SET dm_err->user_action = concat("Please log in as a user that has full privileges to ",vwp_dir)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
 ...
 END ;Subroutine
 SUBROUTINE check_user_privs(null)
   SET dm_err->eproc = "Validating user privileges"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (val_user_privs(1)=0)
    RETURN(0)
   ENDIF
   IF (val_write_privs(cs_select_dir,dm2_install_schema->cer_install)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE init_schema_files(null)
   IF (dm2_val_sch_date_str(cs_sch_date_str)=0)
    RETURN(0)
   ENDIF
   SET dm2_sch_file->dest_dir_cclfmt = dcs_cerinstall
   SET dm_err->eproc = "Determining schema file prefix"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET cs_sch_date_fmt = cnvtdate2(cs_sch_date_str,"DD-MMM-YYYY")
   CASE (cnvtupper(currdbuser))
    OF "CDBA":
     SET cs_sch_date_type = "dm2a"
    OF "ADMIN_TMP":
     SET cs_sch_date_type = "dm2a"
    OF "V500":
     IF ((dm2_install_schema->process_option="CLIN COPY"))
      SET cs_sch_date_type = "dm2s"
     ELSE
      SET cs_sch_date_type = "dm2c"
     ENDIF
    ELSE
     SET dm_err->eproc = "Evaluating if valid user."
     SELECT INTO "nl:"
      FROM dm_tables_doc dtd
      WHERE dtd.owner=currdbuser
      WITH maxqual(dtd,1), nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      RETURN(0)
     ...
     IF (curqual=0)
      SET dm_err->err_ind = 1
      CALL disp_msg(concat(
        "You must be logged into the database as valid Cerner user to run this process. ",
        "Current database user is: ",currdbuser),dm_err->logfile,1)
      RETURN(0)
     ENDIF
     IF ((dm2_install_schema->process_option="CLIN COPY"))
      SET cs_sch_date_type = "dm2s"
     ...
     ENDIF
   ENDCASE
   SET dm2_sch_file->file_prefix = build(cs_sch_date_type,cnvtalphanum(format(cs_sch_date_fmt,
      "MM/DD/YYYY;;D")))
   IF (cs_sch_date_type="dm2a")
    SET dm_err->eproc = "Validating schema date used to capture Admin schema"
    CASE (dm2_sys_misc->cur_os)
     OF "AXP":
      IF (substring(2,1,cs_sch_date_str) != "2")
       SET dm_err->err_ind = 1
       CALL disp_msg("For AXP, admin schema files must have a day ending with the numeral 2",dm_err->
        logfile,1)
       RETURN(0)
      ENDIF
     OF "WIN":
      IF (substring(2,1,cs_sch_date_str) != "3")
       SET dm_err->err_ind = 1
       CALL disp_msg("For NT, admin schema files must have a day ending with the numeral 3",dm_err->
        logfile,1)
       RETURN(0)
      ENDIF
     OF "LNX":
      IF (substring(2,1,cs_sch_date_str) != "4")
       SET dm_err->err_ind = 1
       CALL disp_msg("For LNX, admin schema files must have a day ending with the numeral 4",dm_err->
        logfile,1)
       RETURN(0)
      ENDIF
     ELSE
      IF (substring(2,1,cs_sch_date_str) != "1")
       SET dm_err->err_ind = 1
       CALL disp_msg("For AIX and HPX, admin schema files must have a day ending with the numeral 1",
        dm_err->logfile,1)
       RETURN(0)
      ENDIF
    ENDCASE
   ...
   IF ((dm_err->debug_flag > 0))
    CALL echo("*************************************************")
    CALL echo(build("Schema Date(Parm 1):",cs_sch_date_str))
    CALL echo(build("Capture Description (Parm 2):",sf_desc))
    CALL echo(build("Table Prefix(Parm 3):",cs_table_prefix))
    CALL echo(build("Schema Date Prefix:",cs_sch_date_type))
    CALL echo(build("Destination Directory:",dm2_sch_file->dest_dir_cclfmt))
    CALL echo(build("Format for select Directory:",cs_select_dir))
    CALL echo(build("Capture Source RDBMS:",dcs_source_rdbms))
    CALL echo("*************************************************")
   ENDIF
   SET dm_err->eproc = "CREATE EMPTY SCHEMA FILES"
   CALL disp_msg(" ",dm_err->logfile,0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE insert_dmheader(null)
   SET dm_err->eproc = "POPULATE DMHEADER SCHEMA FILE WITH HEADER INFO"
   CALL disp_msg(" ",dm_err->logfile,0)
   INSERT  FROM dmheader dh
    SET dh.description = sf_desc, dh.admin_load_ind = 0, dh.source_rdbms = dcs_source_rdbms,
     dh.sf_version = sf_ver
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE populate_sf_from_cs(null)
   DECLARE max_tc_cnt = i4 WITH protect, noconstant(0)
   DECLARE max_i_cnt = i2 WITH protect, noconstant(0)
   DECLARE max_ic_cnt = i2 WITH protect, noconstant(0)
   DECLARE max_c_cnt = i2 WITH protect, noconstant(0)
   DECLARE max_cc_cnt = i2 WITH protect, noconstant(0)
   DECLARE ttl_tc_cnt = i4 WITH protect, noconstant(0)
   DECLARE ttl_ind_cnt = i4 WITH protect, noconstant(0)
   DECLARE ttl_icol_cnt = i4 WITH protect, noconstant(0)
   DECLARE ttl_cons_cnt = i4 WITH protect, noconstant(0)
   DECLARE ttl_ccol_cnt = i4 WITH protect, noconstant(0)
   IF (dm2_fill_sch_except("LOCAL")=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "POPULATE DMTABLE SCHEMA FILE WITH TABLE INFO"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((cur_sch->tbl_cnt > 0))
    IF (((currdbuser="CDBA") OR (currdbuser="ADMIN_TMP")) )
     SET def_tspace = "D_TOOLKIT"
     SET def_itspace = "I_TOOLKIT"
     SET def_ltspace = "D_TOOLKIT"
     SELECT INTO "nl:"
      d.seq
      FROM (dummyt d  WITH seq = value(cur_sch->tbl_cnt)),
       (dummyt d2  WITH seq = value(dm2_sch_except->tcnt))
      PLAN (d)
       JOIN (d2
       WHERE (dm2_sch_except->tbl[d2.seq].tbl_name=cur_sch->tbl[d.seq].tbl_name))
      DETAIL
       cur_sch->tbl[d.seq].capture_ind = 1
      WITH nocounter, outerjoin = d, dontexist
     ;end select
     IF (check_error(dm_err->eproc)=1)
      SET dm_err->eproc = "SET CAPTURE_IND"
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
    ELSEIF (dcs_pull_from_dm_tables=0)
     SET def_tspace = dm2_dft_clin_tspace
     SET def_itspace = dm2_dft_clin_itspace
     SET def_ltspace = dm2_dft_clin_ltspace
     SELECT
      IF (findstring(char(42),cs_table_prefix) > 0)
       FROM dm_tables_doc td,
        dm2_user_tables ut,
        dm_ts_precedence ts
       PLAN (ut)
        JOIN (td
        WHERE td.table_name=ut.table_name)
        JOIN (ts
        WHERE td.table_name=ts.table_name)
      ELSE
       FROM dm_tables_doc td,
        dm2_user_tables ut,
        dm_ts_precedence ts
       PLAN (ut)
        JOIN (td
        WHERE td.table_name=ut.table_name
         AND ((td.data_model_section=cs_table_prefix) OR (((td.table_name="DM_INFO") OR (td
        .table_name="DM_INFO2388")) )) )
        JOIN (ts
        WHERE td.table_name=ts.table_name)
      ENDIF
      INTO "nl:"
      ...
      HEAD td.table_name
       IF ((dm_err->debug_flag > 0))
        CALL echo(concat("Locating ",trim(ut.table_name)," in Cur_Sch to set capture_ind"))
       ENDIF
       tblx = locateval(tblx,1,cur_sch->tbl_cnt,ut.table_name,cur_sch->tbl[tblx].tbl_name), cur_sch->
       tbl[tblx].capture_ind = 1, cur_sch->tbl[tblx].reference_ind = td.reference_ind
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      SET dm_err->eproc = "GET TABLES TO CAPTURE FROM DM_TABLES_DOC"
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
     SELECT INTO "nl:"
      ...
      DETAIL
       IF ((dm_err->debug_flag > 0))
        CALL echo(concat("Exception table ",trim(cur_sch->tbl[d.seq].tbl_name),
         " in Cur_Sch set capture_ind to 0"))
       ENDIF
       cur_sch->tbl[d.seq].capture_ind = 0
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      SET dm_err->eproc = "SET CAPTURE_IND"
     ...
     SELECT
      IF (findstring(char(42),cs_table_prefix) > 0)
       FROM dm_info i
       WHERE i.info_domain="DM2_SCHEMA_INSTANCE"
      ELSE
       FROM dm_info i,
        dm_tables_doc td
       WHERE i.info_domain="DM2_SCHEMA_INSTANCE"
        AND i.info_name=td.table_name
        AND ((td.data_model_section=cs_table_prefix) OR (((td.table_name="DM_INFO") OR (td.table_name
       ="DM_INFO2388")) ))
      ENDIF
      INTO "nl:"
      i.info_number
      ...
      DETAIL
       IF ((dm_err->debug_flag > 0))
        CALL echo(concat("Locating ",trim(i.info_name)," in Cur_Sch for schema instance"))
       ENDIF
       tblx = locateval(tblx,1,cur_sch->tbl_cnt,i.info_name,cur_sch->tbl[tblx].tbl_name)
       IF (tblx > 0)
        cur_sch->tbl[tblx].schema_instance = i.info_number
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      SET dm_err->eproc = "SET CAPTURE_IND AND SCHEMA INSTANCE"
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
    ENDIF
    IF ((dm2_install_schema->process_option="CLIN COPY"))
     IF (dcs_adj_for_clin_copy(null)=0)
      RETURN(0)
     ENDIF
    ENDIF
    INSERT  FROM dmtable t,
     ...
     SET t.seq = 1, t.table_name = cur_sch->tbl[d.seq].tbl_name, t.tablespace_name = cur_sch->tbl[d
      .seq].tspace_name,
      t.index_tspace = def_itspace, t.index_tspace_ni = 0, t.long_tspace = cur_sch->tbl[d.seq].
      long_tspace,
      t.long_tspace_ni = cur_sch->tbl[d.seq].long_tspace_ni, t.long_tspace = def_ltspace, t
      .long_tspace_ni = 0,
      ...
      .bytes_allocated = cur_sch->tbl[d.seq].bytes_allocated,
      t.bytes_used = cur_sch->tbl[d.seq].bytes_used, t.schema_date = cur_sch->tbl[d.seq].schema_date,
      t.schema_instance = cur_sch->tbl[d.seq].schema_instance,
      t.alpha_feature_nbr = 0, t.feature_number = cur_sch->tbl[d.seq].mview_exists_ind, t.updt_dt_tm
       = cnvtdatetime(curdate,curtime3),
      t.tspace_type = evaluate(dm2_install_schema->process_option,"CLIN COPY",cur_sch->tbl[d.seq].
       ext_mgmt," "), t.long_tspace_type = evaluate(dm2_install_schema->process_option,"CLIN COPY",
       cur_sch->tbl[d.seq].lext_mgmt," "), t.max_ext = evaluate(dm2_install_schema->process_option,
       "CLIN COPY",cur_sch->tbl[d.seq].max_ext,0.0)
     PLAN (d
      WHERE (cur_sch->tbl[d.seq].capture_ind=1))
      JOIN (t
      WHERE (cur_sch->tbl[d.seq].tbl_name=t.table_name))
    ...
    SELECT INTO "nl:"
     ...
     PLAN (d)
     DETAIL
      IF ((cur_sch->tbl[d.seq].tbl_col_cnt > max_tc_cnt))
       max_tc_cnt = cur_sch->tbl[d.seq].tbl_col_cnt
      ENDIF
      ttl_tc_cnt = (ttl_tc_cnt+ cur_sch->tbl[d.seq].tbl_col_cnt)
     WITH nocounter
    ;end select
    IF (max_tc_cnt > 0)
     INSERT  FROM dmcolumn tc,
    ...
    SELECT INTO "nl:"
     ...
     PLAN (d)
     DETAIL
      IF ((cur_sch->tbl[d.seq].ind_cnt > max_i_cnt))
       max_i_cnt = cur_sch->tbl[d.seq].ind_cnt
      ENDIF
      ttl_ind_cnt = (ttl_ind_cnt+ cur_sch->tbl[d.seq].ind_cnt)
     WITH nocounter
    ;end select
    IF (max_i_cnt > 0)
     INSERT  FROM dmindex i,
      ...
      SET i.seq = 1, i.table_name = cur_sch->tbl[d.seq].tbl_name, i.index_name = cur_sch->tbl[d.seq].
       ind[d2.seq].ind_name,
       i.full_ind_name = cur_sch->tbl[d.seq].ind[d2.seq].full_ind_name, i.pct_increase = cur_sch->
       tbl[d.seq].ind[d2.seq].pct_increase, i.pct_free = cur_sch->tbl[d.seq].ind[d2.seq].pct_free,
       i.init_ext = cur_sch->tbl[d.seq].ind[d2.seq].init_ext, i.next_ext = cur_sch->tbl[d.seq].ind[d2
       .seq].next_ext, i.bytes_allocated = cur_sch->tbl[d.seq].ind[d2.seq].bytes_allocated,
       i.bytes_used = cur_sch->tbl[d.seq].ind[d2.seq].bytes_used, i.unique_ind = cur_sch->tbl[d.seq].
       ind[d2.seq].unique_ind, i.tspace_name = evaluate(dm2_install_schema->process_option,
        "CLIN COPY",cur_sch->tbl[d.seq].ind[d2.seq].tspace_name," "),
       i.tspace_type = evaluate(dm2_install_schema->process_option,"CLIN COPY",cur_sch->tbl[d.seq].
        ind[d2.seq].ext_mgmt," "), i.index_type = cur_sch->tbl[d.seq].ind[d2.seq].index_type, i
       .max_ext = evaluate(dm2_install_schema->process_option,"CLIN COPY",cur_sch->tbl[d.seq].ind[d2
        .seq].max_ext,0.0)
      PLAN (d
       WHERE (cur_sch->tbl[d.seq].capture_ind=1))
       JOIN (d2
     ...
     SELECT INTO "nl:"
      ...
      PLAN (d)
       ...
       WHERE (d2.seq <= cur_sch->tbl[d.seq].ind_cnt))
      DETAIL
       IF ((cur_sch->tbl[d.seq].ind[d2.seq].ind_col_cnt > max_ic_cnt))
        max_ic_cnt = cur_sch->tbl[d.seq].ind[d2.seq].ind_col_cnt
       ENDIF
       ttl_icol_cnt = (ttl_icol_cnt+ cur_sch->tbl[d.seq].ind[d2.seq].ind_col_cnt)
      WITH nocounter
     ;end select
     IF (max_ic_cnt > 0)
      INSERT  FROM dmindcol ic,
     ...
     ENDIF
    ELSE
     SET dm_err->eproc = "NO INDEXES IN CURRENT SCHEMA"
     IF (dcs_pull_from_dm_tables=1)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "No information found for indexes."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      RETURN(0)
     ELSE
      CALL disp_msg(" ",dm_err->logfile,0)
    ...
    SELECT INTO "nl:"
     ...
     PLAN (d)
     DETAIL
      IF ((cur_sch->tbl[d.seq].cons_cnt > max_c_cnt))
       max_c_cnt = cur_sch->tbl[d.seq].cons_cnt
      ENDIF
      ttl_cons_cnt = (ttl_cons_cnt+ cur_sch->tbl[d.seq].cons_cnt)
     WITH nocounter
    ;end select
    IF (max_c_cnt > 0)
     INSERT  FROM dmcons c,
      ...
      SET c.seq = 1, c.table_name = cur_sch->tbl[d.seq].tbl_name, c.constraint_name = cur_sch->tbl[d
       .seq].cons[d2.seq].cons_name,
       c.full_cons_name = cur_sch->tbl[d.seq].cons[d2.seq].full_cons_name, c.constraint_type =
       cur_sch->tbl[d.seq].cons[d2.seq].cons_type, c.status_ind = cur_sch->tbl[d.seq].cons[d2.seq].
       status_ind,
       c.r_constraint_name = cur_sch->tbl[d.seq].cons[d2.seq].r_constraint_name, c.parent_table_name
        = cur_sch->tbl[d.seq].cons[d2.seq].parent_table, c.parent_table_columns = cur_sch->tbl[d.seq]
       .cons[d2.seq].parent_table_columns,
     ...
     SELECT INTO "nl:"
      ...
      PLAN (d)
       ...
       WHERE (d2.seq <= cur_sch->tbl[d.seq].cons_cnt))
      DETAIL
       IF ((cur_sch->tbl[d.seq].cons[d2.seq].cons_col_cnt > max_cc_cnt))
        max_cc_cnt = cur_sch->tbl[d.seq].cons[d2.seq].cons_col_cnt
       ENDIF
       ttl_ccol_cnt = (ttl_ccol_cnt+ cur_sch->tbl[d.seq].cons[d2.seq].cons_col_cnt)
      WITH nocounter
     ;end select
     IF (max_cc_cnt > 0)
      INSERT  FROM dmconscol cc,
     ...
     ENDIF
    ELSE
     SET dm_err->eproc = "NO CONSTRAINTS IN CURRENT SCHEMA"
     IF (dcs_pull_from_dm_tables=1)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "No constraints found for the schema snapshot."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      RETURN(0)
     ELSE
      CALL disp_msg(" ",dm_err->logfile,0)
   ...
   ELSE
    ...
    ENDIF
   ENDIF
   IF (dcs_pull_from_dm_tables=0)
    SET dm_err->eproc = "POPULATE DMSEQ SCHEMA FILE WITH SEQUENCE INFO"
    CALL disp_msg(" ",dm_err->logfile,0)
    IF ((cur_sch->sequence_cnt > 0))
     SELECT INTO "nl:"
      d.seq
      FROM (dummyt d  WITH seq = value(cur_sch->sequence_cnt)),
       (dummyt d2  WITH seq = value(dm2_sch_except->seq_cnt))
      PLAN (d)
       JOIN (d2
       WHERE (dm2_sch_except->seq[d2.seq].seq_name=cur_sch->sequence[d.seq].seq_name))
      DETAIL
       cur_sch->sequence[d.seq].capture_ind = 1
      WITH nocounter, outerjoin = d, dontexist
     ;end select
     INSERT  FROM dmseq s,
       (dummyt d  WITH seq = value(cur_sch->sequence_cnt))
      SET s.seq = 1, s.sequence_name = cur_sch->sequence[d.seq].seq_name, s.min_value = cur_sch->
       sequence[d.seq].min_val,
       s.max_value = cur_sch->sequence[d.seq].max_val, s.increment_by = cur_sch->sequence[d.seq].
       increment_by, s.cycle_flag = cur_sch->sequence[d.seq].cycle_flag,
       s.last_number = cur_sch->sequence[d.seq].last_number, s.alpha_feature_nbr = 0, s
       .feature_number = 0,
     ...
     ENDIF
    ELSE
     SET dm_err->eproc = "NO SEQUENCES IN CURRENT SCHEMA"
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE get_dm_tables_schema(sbr_gdts_sch_dt)
   DECLARE gdts_table_cnt = i4 WITH protect, noconstant(0)
   DECLARE gdts_table_col_cnt = i4 WITH protect, noconstant(0)
   DECLARE gdts_alloc_unit = i4 WITH protect, noconstant(100)
   ...
   SET def_itspace = dm2_dft_clin_itspace
   SET def_ltspace = dm2_dft_clin_ltspace
   SET dm_err->eproc = "Getting table and column information."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    data_default_null = nullind(c.data_default)
    FROM dm_tables t,
     dm_columns c
    WHERE t.schema_date=cnvtdatetimeutc(sbr_gdts_sch_dt)
     AND c.schema_date=t.schema_date
     AND c.table_name=t.table_name
    ORDER BY c.table_name, c.column_name
    HEAD REPORT
     gdts_table_cnt = 0, gdts_table_col_cnt = 0,
     MACRO (data_default_cleanup)
      ...
      ((c.data_default="''") OR (c.data_default='""')) )) )) )) )
       cur_sch->tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].data_default_ni = 1
      ENDIF
     ENDMACRO
    HEAD c.table_name
     gdts_table_cnt = (gdts_table_cnt+ 1)
     IF (mod(gdts_table_cnt,gdts_alloc_unit)=1)
      stat = alterlist(cur_sch->tbl,(gdts_table_cnt+ (gdts_alloc_unit - 1)))
     ENDIF
     cur_sch->tbl[gdts_table_cnt].tbl_name = t.table_name, cur_sch->tbl[gdts_table_cnt].tspace_name
      = t.tablespace_name, cur_sch->tbl[gdts_table_cnt].pct_increase = t.pct_increase,
     cur_sch->tbl[gdts_table_cnt].pct_used = t.pct_used, cur_sch->tbl[gdts_table_cnt].pct_free = t
     .pct_free, cur_sch->tbl[gdts_table_cnt].init_ext = 0,
     cur_sch->tbl[gdts_table_cnt].next_ext = 0, cur_sch->tbl[gdts_table_cnt].capture_ind = 1, cur_sch
     ->tbl[gdts_table_cnt].schema_date = cnvtdatetime(sbr_gdts_sch_dt),
     cur_sch->tbl[gdts_table_cnt].schema_instance = 0, cur_sch->tbl[gdts_table_cnt].long_tspace =
     def_ltspace, cur_sch->tbl[gdts_table_cnt].long_tspace_ni = 0,
     cur_sch->tbl[gdts_table_cnt].ind_tspace = def_itspace, cur_sch->tbl[gdts_table_cnt].
     ind_tspace_ni = 0
    DETAIL
     gdts_table_col_cnt = (gdts_table_col_cnt+ 1)
     IF (mod(gdts_table_col_cnt,gdts_alloc_unit)=1)
      stat = alterlist(cur_sch->tbl[gdts_table_cnt].tbl_col,(gdts_table_col_cnt+ (gdts_alloc_unit - 1
       )))
     ENDIF
     cur_sch->tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].col_name = c.column_name, cur_sch->tbl[
     gdts_table_cnt].tbl_col[gdts_table_col_cnt].col_seq = c.column_seq
     IF (currdb="SQLSRV")
      IF (cnvtupper(c.data_type)="decimal*")
       cur_sch->tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].data_type = "decimal(38,10)"
      ELSE
       cur_sch->tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].data_type = c.data_type
      ENDIF
     ELSE
      cur_sch->tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].data_type = c.data_type
     ENDIF
     cur_sch->tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].data_length = c.data_length, cur_sch->
     tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].nullable = c.nullable, cur_sch->tbl[
     gdts_table_cnt].tbl_col[gdts_table_col_cnt].data_default_ni = data_default_null
     IF ((cur_sch->tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].data_default_ni=0))
      data_default_cleanup
     ENDIF
     IF ((cur_sch->tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].data_default_ni=0))
      cur_sch->tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].data_default = c.data_default
     ENDIF
     cur_sch->tbl[gdts_table_cnt].tbl_col[gdts_table_col_cnt].virtual_column = c.virtual_column
    FOOT  c.table_name
     stat = alterlist(cur_sch->tbl[gdts_table_cnt].tbl_col,gdts_table_col_cnt), cur_sch->tbl[
     gdts_table_cnt].tbl_col_cnt = gdts_table_col_cnt, gdts_table_col_cnt = 0
    FOOT REPORT
     stat = alterlist(cur_sch->tbl,gdts_table_cnt), cur_sch->tbl_cnt = gdts_table_cnt, gdts_table_cnt
      = 0
    WITH nocounter
   ;end select
   IF (check_error("Error capturing table and column information.")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Getting index information."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    tablespace_name_null = nullind(i.tablespace_name)
    FROM dm_index_columns c,
     dm_indexes i
    WHERE i.schema_date=cnvtdatetimeutc(sbr_gdts_sch_dt)
     AND c.table_name=i.table_name
     AND c.index_name=i.index_name
     AND c.schema_date=i.schema_date
    ORDER BY c.table_name, c.index_name, c.column_position
    HEAD REPORT
     fatal_error_ind = false
    HEAD c.table_name
     IF (fatal_error_ind=false)
      index_cnt = 0, index_col_cnt = 0, table_found_ind = false,
      tablex = 0, tablex = locateval(tablex,1,cur_sch->tbl_cnt,c.table_name,cur_sch->tbl[tablex].
       tbl_name)
      IF (tablex > 0)
       table_found_ind = true
      ELSE
       fatal_error_ind = true, fatal_error_eproc = "get_index_info", fatal_error_emsg = concat(
        "could not find table ",trim(c.table_name)," for index ",trim(i.index_name))
      ENDIF
     ENDIF
    HEAD c.index_name
     IF (table_found_ind=true
      ...
      IF (mod(index_cnt,gdts_alloc_unit)=1)
       stat = alterlist(cur_sch->tbl[tablex].ind,(index_cnt+ (gdts_alloc_unit - 1)))
      ENDIF
      cur_sch->tbl[tablex].ind[index_cnt].ind_name = i.index_name, cur_sch->tbl[tablex].ind[index_cnt
      ].pct_increase = i.pct_increase, cur_sch->tbl[tablex].ind[index_cnt].pct_free = i.pct_free,
      cur_sch->tbl[tablex].ind[index_cnt].init_ext = 0, cur_sch->tbl[tablex].ind[index_cnt].next_ext
       = 0, cur_sch->tbl[tablex].ind[index_cnt].tspace_name = i.tablespace_name,
      cur_sch->tbl[tablex].ind[index_cnt].full_ind_name = i.index_name, cur_sch->tbl[tablex].ind[
      index_cnt].index_type = i.index_type, cur_sch->tbl[tablex].ind[index_cnt].unique_ind = i
      .unique_ind
      IF (tablespace_name_null=0
       AND i.tablespace_name="null")
       cur_sch->tbl[tablex].ind[index_cnt].tspace_name_ni = 1
      ELSE
       cur_sch->tbl[tablex].ind[index_cnt].tspace_name_ni = tablespace_name_null
      ENDIF
     ENDIF
    DETAIL
     IF (table_found_ind=true
      ...
      IF (mod(index_col_cnt,gdts_alloc_unit)=1)
       stat = alterlist(cur_sch->tbl[tablex].ind[index_cnt].ind_col,(index_col_cnt+ (gdts_alloc_unit
         - 1)))
      ENDIF
      cur_sch->tbl[tablex].ind[index_cnt].ind_col[index_col_cnt].col_name = c.column_name, cur_sch->
      tbl[tablex].ind[index_cnt].ind_col[index_col_cnt].col_position = c.column_position
     ENDIF
    FOOT  c.index_name
     IF (table_found_ind=true
      AND fatal_error_ind=false)
      stat = alterlist(cur_sch->tbl[tablex].ind[index_cnt].ind_col,index_col_cnt), cur_sch->tbl[
      tablex].ind[index_cnt].ind_col_cnt = index_col_cnt, index_col_cnt = 0
     ENDIF
    FOOT  c.table_name
     IF (table_found_ind=true
      AND fatal_error_ind=false)
      stat = alterlist(cur_sch->tbl[tablex].ind,index_cnt), cur_sch->tbl[tablex].ind_cnt = index_cnt,
      index_cnt = 0
     ENDIF
    WITH nocounter
   ;end select
   IF (fatal_error_ind=true)
    ...
    SET dm_err->emsg = fatal_error_emsg
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (check_error("Error capturing index information.")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Getting constraint information."
   CALL disp_msg("",dm_err->logfile,0)
   SET tablex = 0
   SELECT INTO "nl:"
    FROM dm_cons_columns cc,
     dm_constraints c
    WHERE c.schema_date=cnvtdatetimeutc(sbr_gdts_sch_dt)
    ...
    ORDER BY c.constraint_type, c.table_name, c.constraint_name,
     cc.position
    HEAD REPORT
     fatal_error_ind = false, pk_constraint_cnt = 0
    HEAD c.constraint_type
     row + 0
    HEAD c.table_name
     IF (fatal_error_ind=false)
      table_found_ind = false, tablex = 0, tablex = locateval(tablex,1,cur_sch->tbl_cnt,c.table_name,
       cur_sch->tbl[tablex].tbl_name)
      IF (tablex > 0)
       table_found_ind = true, constraint_cnt = cur_sch->tbl[tablex].cons_cnt, constraint_col_cnt = 0
      ELSE
       fatal_error_ind = true, fatal_error_eproc = "get_constraint_info", fatal_error_emsg = concat(
        "could not find table ",trim(c.table_name)," for constraint ",trim(c.constraint_name))
      ENDIF
     ENDIF
    HEAD c.constraint_name
     IF (table_found_ind=true
      AND fatal_error_ind=false)
      constraint_cnt = (constraint_cnt+ 1), stat = alterlist(cur_sch->tbl[tablex].cons,constraint_cnt
       ), cur_sch->tbl[tablex].cons[constraint_cnt].cons_name = c.constraint_name,
      cur_sch->tbl[tablex].cons[constraint_cnt].full_cons_name = c.constraint_name, cur_sch->tbl[
      tablex].cons[constraint_cnt].cons_type = c.constraint_type, cur_sch->tbl[tablex].cons[
      constraint_cnt].r_constraint_name = c.r_constraint_name,
      cur_sch->tbl[tablex].cons[constraint_cnt].parent_table = c.parent_table_name, cur_sch->tbl[
      tablex].cons[constraint_cnt].status_ind = c.status_ind, cur_sch->tbl[tablex].cons[
      constraint_cnt].parent_table = c.parent_table_name,
      cur_sch->tbl[tablex].cons[constraint_cnt].parent_table_columns = c.parent_table_columns,
      cur_sch->tbl[tablex].cons[constraint_cnt].delete_rule = c.delete_rule
     ENDIF
    DETAIL
     IF (table_found_ind=true
      AND fatal_error_ind=false)
      constraint_col_cnt = (constraint_col_cnt+ 1)
      IF (mod(constraint_col_cnt,gdts_alloc_unit)=1)
       stat = alterlist(cur_sch->tbl[tablex].cons[constraint_cnt].cons_col,(constraint_col_cnt+ (
        gdts_alloc_unit - 1)))
      ENDIF
      cur_sch->tbl[tablex].cons[constraint_cnt].cons_col[constraint_col_cnt].col_name = cc
      .column_name, cur_sch->tbl[tablex].cons[constraint_cnt].cons_col[constraint_col_cnt].
      col_position = cc.position
     ENDIF
    FOOT  c.constraint_name
     IF (table_found_ind=true
      AND fatal_error_ind=false)
      stat = alterlist(cur_sch->tbl[tablex].cons[constraint_cnt].cons_col,constraint_col_cnt),
      cur_sch->tbl[tablex].cons[constraint_cnt].cons_col_cnt = constraint_col_cnt, constraint_col_cnt
       = 0
     ENDIF
    FOOT  c.table_name
     IF (table_found_ind=true
      AND fatal_error_ind=false)
      stat = alterlist(cur_sch->tbl[tablex].cons,constraint_cnt), cur_sch->tbl[tablex].cons_cnt =
      constraint_cnt, constraint_cnt = 0
     ENDIF
    WITH nocounter
   ;end select
   IF (fatal_error_ind=true)
    ...
    RETURN(0)
   ENDIF
   IF (check_error("Error capturing constraint information.")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(cur_sch,dm_err->logfile)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE update_schema_instance(null)
   IF (dcs_pull_from_dm_tables=0)
    SET dm_err->emsg = "Operation failed. Program was not executed from dm2_capture_desired_schema."
    SET dm_err->err_ind = 1
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "UPDATE DMTABLE SCHEMA FILE WITH SCHEMA INSTANCE INFO"
   CALL disp_msg(" ",dm_err->logfile,0)
   UPDATE  FROM dmtable dt,
     (dummyt d  WITH seq = value(size(new_schema->qual,5)))
    SET dt.schema_instance = new_schema->qual[d.seq].schema_instance
    PLAN (d)
     JOIN (dt
     WHERE (new_schema->qual[d.seq].table_name=dt.table_name)
      AND (new_schema->qual[d.seq].schema_instance > 0))
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcs_adj_for_clin_copy(null)
   DECLARE dacc_tbl_cnt = i4 WITH protect, noconstant(0)
   DECLARE dacc_idx_cnt = i4 WITH protect, noconstant(0)
   DECLARE dacc_tmp_init_ext = f8 WITH protect, noconstant(0.00)
   DECLARE dacc_tmp_next_ext = f8 WITH protect, noconstant(0.00)
   DECLARE dacc_ndx = i4 WITH protect, noconstant(0)
   DECLARE dacc_pres_t = i4 WITH protect, noconstant(0)
   FREE SET dcs_lob_tspace
   RECORD dcs_lob_tspace(
     1 cnt = i4
     1 tbl[*]
       2 table_name = vc
   ...
   SET dcs_lob_tspace->cnt = 0
   SET dm_err->eproc = "Load lob tablespace assignment for tables."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM user_lobs ul,
     user_segments us
    WHERE ul.segment_name=us.segment_name
     AND us.segment_type IN ("LOBSEGMENT", "LOB PARTITION", "LOB SUBPARTITION")
    ORDER BY ul.table_name, us.tablespace_name
    HEAD ul.table_name
     dcs_lob_tspace->cnt = (dcs_lob_tspace->cnt+ 1), stat = alterlist(dcs_lob_tspace->tbl,
      dcs_lob_tspace->cnt), dcs_lob_tspace->tbl[dcs_lob_tspace->cnt].table_name = trim(ul.table_name),
     dcs_lob_tspace->tbl[dcs_lob_tspace->cnt].tablespace_name = trim(us.tablespace_name),
     dcs_lob_tspace->tbl[dcs_lob_tspace->cnt].next_ext = us.next_extent
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag >= 5))
    CALL echorecord(dcs_lob_tspace)
   ENDIF
   IF ((dcs_lob_tspace->cnt > 0))
    FOR (dacc_tbl_cnt = 1 TO dcs_lob_tspace->cnt)
      SET dacc_tmp_tbl_cnt = 0
      SET dacc_tmp_tbl_cnt = locateval(dacc_tmp_tbl_cnt,1,value(cur_sch->tbl_cnt),dcs_lob_tspace->
       tbl[dacc_tbl_cnt].table_name,cur_sch->tbl[dacc_tmp_tbl_cnt].tbl_name)
      IF (dacc_tmp_tbl_cnt > 0)
       SET cur_sch->tbl[dacc_tmp_tbl_cnt].lext_mgmt = evaluate(dcs_lob_tspace->tbl[dacc_tbl_cnt].
        next_ext,0.0,"L","D")
       SET cur_sch->tbl[dacc_tmp_tbl_cnt].long_tspace = dcs_lob_tspace->tbl[dacc_tbl_cnt].
       tablespace_name
       SET cur_sch->tbl[dacc_tmp_tbl_cnt].long_tspace_ni = 0
      ENDIF
    ENDFOR
   ENDIF
   FREE SET dcs_lob_tspace
   IF ((drr_clin_copy_data->process="MIGRATION"))
    IF (validate(dm2_bypass_long_to_lob,- (1)) != 1)
     SET dm_err->eproc =
     "Determining space required for columns converting from LONGs to LOBs during migration"
     CALL disp_msg(" ",dm_err->logfile,0)
     SELECT INTO "nl:"
      d.long_tablespace, c.table_name, c.column_name
      ...
      PLAN (c
       ...
       WHERE c.table_name=d.table_name
        AND d.precedence=1)
      ORDER BY d.long_tablespace, c.table_name, c.column_name
      HEAD REPORT
       tspace_cnt = 0, tbl_cnt = 0
      HEAD d.long_tablespace
       tspace_cnt = (tspace_cnt+ 1), dcs_long_tspace->tspace_count = tspace_cnt, stat = alterlist(
        dcs_long_tspace->tspace,tspace_cnt),
       dcs_long_tspace->tspace[tspace_cnt].tspace_name = d.long_tablespace, tbl_cnt = 0
      DETAIL
       tbl_cnt = (tbl_cnt+ 1), dcs_long_tspace->tspace[tspace_cnt].tbl_cnt = tbl_cnt, stat =
       alterlist(dcs_long_tspace->tspace[tspace_cnt].tbl,tbl_cnt),
       dcs_long_tspace->tspace[tspace_cnt].tbl[tbl_cnt].table_name = c.table_name, dcs_long_tspace->
       tspace[tspace_cnt].tbl[tbl_cnt].column_name = c.column_name, dcs_long_tspace->tspace[
       tspace_cnt].bytes = (dcs_long_tspace->tspace[tspace_cnt].bytes+ dtr_lob_size),
       found = 0, found = locateval(found,1,value(cur_sch->tbl_cnt),dcs_long_tspace->tspace[
        tspace_cnt].tbl[tbl_cnt].table_name,cur_sch->tbl[found].tbl_name)
       IF (found > 0)
        cur_sch->tbl[found].lext_mgmt = "L", cur_sch->tbl[found].long_tspace = dcs_long_tspace->
        tspace[tspace_cnt].tspace_name, cur_sch->tbl[found].long_tspace_ni = 0
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      ...
      RETURN(0)
     ENDIF
     CALL echorecord(dcs_long_tspace)
    ENDIF
   ENDIF
   IF (drr_load_mixed_table_data(0)=0)
    RETURN(0)
   ENDIF
   FOR (dacc_tbl_cnt = 1 TO cur_sch->tbl_cnt)
     SET dacc_tmp_init_ext = cur_sch->tbl[dacc_tbl_cnt].init_ext
     SET dacc_tmp_next_ext = cur_sch->tbl[dacc_tbl_cnt].next_ext
     IF ((dm_err->debug_flag >= 5))
      CALL echo(build("table_name=",cur_sch->tbl[dacc_tbl_cnt].tbl_name))
     ENDIF
     SET dacc_ndx = 0
     SET dacc_ndx = locateval(dacc_ndx,1,value(drr_mixed_tables_data->cnt),cur_sch->tbl[dacc_tbl_cnt]
      .tbl_name,drr_mixed_tables_data->tbl[dacc_ndx].table_name)
     IF (dacc_ndx > 0)
      IF ((dm_err->debug_flag >= 5))
       CALL echo(concat(cur_sch->tbl[dacc_tbl_cnt].tbl_name," is a mixed table."))
      ENDIF
      CALL dm2_adj_init_next_ext(dm2_install_schema->data_to_move,2,cur_sch->tbl[dacc_tbl_cnt].
       tbl_name,dacc_tmp_init_ext,dacc_tmp_next_ext)
     ELSE
      CALL dm2_adj_init_next_ext(dm2_install_schema->data_to_move,cur_sch->tbl[dacc_tbl_cnt].
       reference_ind,cur_sch->tbl[dacc_tbl_cnt].tbl_name,dacc_tmp_init_ext,dacc_tmp_next_ext)
     ENDIF
     IF ((dm_err->debug_flag >= 5))
      CALL echo(build("dacc_tmp_init_ext=",dacc_tmp_init_ext,", dacc_tmp_next_ext=",dacc_tmp_next_ext,
        "."))
     ENDIF
     IF ((drr_clin_copy_data->process="MIGRATION"))
      SET cur_sch->tbl[dacc_tbl_cnt].init_ext = 0
      SET cur_sch->tbl[dacc_tbl_cnt].next_ext = 0
     ELSE
      SET cur_sch->tbl[dacc_tbl_cnt].init_ext = dacc_tmp_init_ext
      SET cur_sch->tbl[dacc_tbl_cnt].next_ext = dacc_tmp_next_ext
     ENDIF
     IF ((cur_sch->tbl[dacc_tbl_cnt].ext_mgmt="L"))
      SET cur_sch->tbl[dacc_tbl_cnt].max_ext = 0.0
     ENDIF
     IF ((cur_sch->tbl[dacc_tbl_cnt].ext_mgmt="L")
      AND (cur_sch->tbl[dacc_tbl_cnt].pct_free > 0.0)
      AND (cur_sch->tbl[dacc_tbl_cnt].pct_used=0.0))
      SET cur_sch->tbl[dacc_tbl_cnt].pct_used = 1
     ENDIF
     IF ((drr_clin_copy_data->process="REFRESH"))
      SET dacc_pres_t = locateval(dacc_pres_t,1,drr_preserved_tables_data->cnt,cur_sch->tbl[
       dacc_tbl_cnt].tbl_name,drr_preserved_tables_data->tbl[dacc_pres_t].table_name)
      IF (dacc_pres_t=0)
       SET cur_sch->tbl[dacc_tbl_cnt].capture_ind = 0
      ENDIF
     ENDIF
     FOR (dacc_idx_cnt = 1 TO cur_sch->tbl[dacc_tbl_cnt].ind_cnt)
       SET dacc_tmp_init_ext = cur_sch->tbl[dacc_tbl_cnt].ind[dacc_idx_cnt].init_ext
       SET dacc_tmp_next_ext = cur_sch->tbl[dacc_tbl_cnt].ind[dacc_idx_cnt].next_ext
       IF ((dm_err->debug_flag >= 5))
        CALL echo(build("index_name=",cur_sch->tbl[dacc_tbl_cnt].ind[dacc_idx_cnt].ind_name))
       ENDIF
       IF (dacc_ndx=0)
        CALL dm2_adj_init_next_ext(dm2_install_schema->data_to_move,cur_sch->tbl[dacc_tbl_cnt].
         reference_ind,cur_sch->tbl[dacc_tbl_cnt].tbl_name,dacc_tmp_init_ext,dacc_tmp_next_ext)
       ELSE
        IF ((dm_err->debug_flag >= 5))
         CALL echo(concat("index_name=",cur_sch->tbl[dacc_tbl_cnt].ind[dacc_idx_cnt].ind_name,
           " resides on a mixed table"))
        ENDIF
        CALL dm2_adj_init_next_ext(dm2_install_schema->data_to_move,2,cur_sch->tbl[dacc_tbl_cnt].
         tbl_name,dacc_tmp_init_ext,dacc_tmp_next_ext)
       ENDIF
       IF ((dm_err->debug_flag >= 5))
        CALL echo(build("dacc_tmp_init_ext=",dacc_tmp_init_ext,", dacc_tmp_next_ext=",
          dacc_tmp_next_ext,"."))
       ENDIF
       IF ((drr_clin_copy_data->process="MIGRATION"))
        SET cur_sch->tbl[dacc_tbl_cnt].ind[dacc_idx_cnt].init_ext = 0
        SET cur_sch->tbl[dacc_tbl_cnt].ind[dacc_idx_cnt].next_ext = 0
       ELSE
        SET cur_sch->tbl[dacc_tbl_cnt].ind[dacc_idx_cnt].init_ext = dacc_tmp_init_ext
        SET cur_sch->tbl[dacc_tbl_cnt].ind[dacc_idx_cnt].next_ext = dacc_tmp_next_ext
       ENDIF
       IF ((cur_sch->tbl[dacc_tbl_cnt].ind[dacc_idx_cnt].ext_mgmt="L"))
        SET cur_sch->tbl[dacc_tbl_cnt].ind[dacc_idx_cnt].max_ext = 0.0
       ENDIF
     ENDFOR
   ENDFOR
   RETURN(1)
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "SCHEMA FILES CREATED SUCCESSFULLY"
 CALL final_disp_msg("dm2_capture_schema")
END GO
