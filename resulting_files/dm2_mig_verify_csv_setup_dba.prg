CREATE PROGRAM dm2_mig_verify_csv_setup:dba
 ...
 SUBROUTINE dvdr_get_verify2(null)
  ...
  RETURN(1)
 END ;Subroutine
 IF ((validate(dmvcs_tab_list->cnt,- (1))=- (1))
  AND (validate(dmvcs_tab_list->cnt,- (2))=- (2)))
  FREE RECORD dmvcs_tab_list
  RECORD dmvcs_tab_list(
    1 cnt = i4
    1 qual[*]
      2 owner = vc
      2 table_name = vc
  )
  SET dmvcs_tab_list->cnt = 0
 ENDIF
 DECLARE dmvcs_filename = vc WITH protect, noconstant("")
 DECLARE dmvcs_idx = i4 WITH protect, noconstant(0)
 DECLARE dmvcs_owner = vc WITH protect, noconstant("")
 DECLARE dmvcs_table_name = vc WITH protect, noconstant("")
 DECLARE dmvcs_indx = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_verify_csv_setup",".log","dm2_mig_verify_csv_setup")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_verify_csv_setup"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dmvcs_filename = trim( $1,3)
 SET dm_err->eproc = "Verifying input parameters."
 IF (dmvcs_filename="")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat(
   "No parameter passed. Expected syntax: dm2_mig_verify_csv_setup '<csv file_name>' go")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dm2_findfile(dmvcs_filename)=0)
  SET dm_err->eproc = "Check if .csv file exists or not"
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat(" The ",trim(dmvcs_filename),
   " file does not exist in the CCLUSERDIR directory.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dmr_prompt_retrieve_mig_data(0,1,1,0)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Extracting information from ",trim(dmvcs_filename)," file")
 CALL disp_msg(" ",dm_err->logfile,0)
 FREE DEFINE rtl
 FREE SET file_loc
 SET logical file_loc value(dmvcs_filename)
 DEFINE rtl "file_loc"
 SELECT INTO "nl:"
  r.line
  FROM rtlt r
  HEAD REPORT
   dmvcs_tab_list->cnt = 0
  DETAIL
   IF (trim(r.line) != "")
    dmvcs_owner = trim(substring(1,(findstring(",",r.line,1,1) - 1),r.line),3), dmvcs_table_name =
    trim(substring((findstring(",",r.line,1,1)+ 1),textlen(r.line),r.line),3)
    IF (dmvcs_owner != "OWNER"
     AND dmvcs_table_name != "TABLE NAME")
     dmvcs_idx = 0, dmvcs_idx = locateval(dmvcs_idx,1,dmvcs_tab_list->cnt,dmvcs_owner,dmvcs_tab_list
      ->qual[dmvcs_idx].owner,
      dmvcs_table_name,dmvcs_tab_list->qual[dmvcs_idx].table_name)
     IF (dmvcs_idx=0)
      dmvcs_tab_list->cnt = (dmvcs_tab_list->cnt+ 1), stat = alterlist(dmvcs_tab_list->qual,
       dmvcs_tab_list->cnt), dmvcs_tab_list->qual[dmvcs_tab_list->cnt].owner = dmvcs_owner,
      dmvcs_tab_list->qual[dmvcs_tab_list->cnt].table_name = dmvcs_table_name
     ENDIF
    ENDIF
   ENDIF
  WITH nocounter
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 IF ((dmvcs_tab_list->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No content was loaded from csv file")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmvcs_tab_list)
 ENDIF
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = concat("Retrieve DB Link information")
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_VDATA_INFO"
 ...
 ENDIF
 SET dm_err->eproc = concat("Retrieve rows to compare information")
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_VDATA_INFO"
   AND di.info_name="DM2_VDATA_NUMROWS"
  DETAIL
   dmvd->rows_to_compare = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dmvd->rows_to_compare=0))
  SET dmvd->rows_to_compare = 500000
 ENDIF
 ...
 ENDIF
 FOR (dmvcs_idx = 1 TO dmvcs_tab_list->cnt)
   SET dmvd->owner = dmvcs_tab_list->qual[dmvcs_idx].owner
   SET dmvd->table_name = dmvcs_tab_list->qual[dmvcs_idx].table_name
   SET dm2_install_schema->curprog = "dm2_mig_verify_csv_setup"
   IF (locateval(dmvcs_indx,1,dmr_user_list->cnt,dmvcs_tab_list->qual[dmvcs_idx].owner,dmr_user_list
    ->qual[dmvcs_indx].user) > 0)
    SET dm_err->eproc = concat("Check if ",dmvcs_tab_list->qual[dmvcs_idx].table_name,
     " exists in target db")
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dba_tables d
     WHERE (d.owner=dmvcs_tab_list->qual[dmvcs_idx].owner)
      AND (d.table_name=dmvcs_tab_list->qual[dmvcs_idx].table_name)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    IF (curqual > 0)
     EXECUTE dm2_verify_data_setup
     IF ((dm_err->err_ind=1))
      GO TO exit_script
     ENDIF
    ELSE
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Table does not exist in target database"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
   ELSE
    SET dm_err->eproc = concat("Check if ",dmvcs_tab_list->qual[dmvcs_idx].owner,
     " user exists in list of migration users")
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Invalid migration user"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
 ENDFOR
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_mig_verify_csv_setup completed succesfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_verify_csv_setup")
END GO
