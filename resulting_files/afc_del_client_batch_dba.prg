CREATE PROGRAM afc_del_client_batch:dba
 ...
 CALL del_client_batch(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE del_client_batch(del_begin,del_end)
   FOR (x = del_begin TO del_end)
     ...
     UPDATE  FROM client_batch c
      SET c.active_ind = false, c.active_status_cd = nullcheck(active_code,request->client_batch[x].
        active_status_cd,
        IF ((request->client_batch[x].active_status_cd=0)) 0
        ELSE 1
        ENDIF
