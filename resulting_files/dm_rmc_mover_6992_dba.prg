CREATE PROGRAM dm_rmc_mover_6992:dba
 ...
 SET cust_column_cnt = get_col_pos(cust_tab_name,"CRITERIA_VALUE")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_column_cnt].translated = 0
 IF (is_translated(cust_tab_name,"CRITERIA_CD")=0)
  CALL is_translated(cust_tab_name,"ALL")
  GO TO exit_6992
 ENDIF
 SET cust_temp_cd = cnvtreal(get_value(cust_tab_name,"CRITERIA_CD","TO"))
 SELECT INTO "nl:"
  FROM code_value cv
  WHERE cv.code_value=cust_temp_cd
  DETAIL
   cust_temp_cdf_mean = cv.cdf_meaning
  WITH nocounter
 ;end select
 IF (check_error("Obtaining CDF meaning")=1)
  ...
  GO TO exit_6992
 ENDIF
 IF (cust_temp_cdf_mean <= " ")
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV22"
  CALL merge_audit("FAILREASON","ERROR: invalid cdf_meaning retuned by uar call",2)
  GO TO exit_6992
 ENDIF
 SET cust_from_value = get_value(cust_tab_name,"CRITERIA_VALUE","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_6992
 ENDIF
 CASE (cust_temp_cdf_mean)
  OF "BILLENTITY":
   SET cust_process_tab = "BILLING_ENTITY"
   SET cust_process_col = "BILLING_ENTITY_ID"
  OF "CHARGEGROUP":
   SET cust_process_tab = "BT_CONDITION"
   SET cust_process_col = "BT_CONDITION_ID"
  OF "FINCLASS":
  OF "ENCNTRTYPE":
   SET cust_process_tab = "CODE_VALUE"
   SET cust_process_col = "CODE_VALUE"
  OF "HEALTHPLAN":
   SET cust_process_tab = "HEALTH_PLAN"
   SET cust_process_col = "HEALTH_PLAN_ID"
  OF "INSURANCEORG":
   SET cust_process_tab = "ORGANIZATION"
   SET cust_process_col = "ORGANIZATION_ID"
  ELSE
   SET cust_err_val = get_err_val(null)
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV22"
    CALL merge_audit("FAILREASON",concat("ERROR: the cdf_meaning of ",cust_temp_cdf_mean,
      " is not supported at this time"),2)
   ENDIF
   GO TO exit_6992
 ENDCASE
 IF (cnvtreal(cust_from_value) > 0)
  SET cust_new_value = select_merge_translate(cust_from_value,cust_process_tab)
  IF (isnumeric(cust_new_value))
   CALL put_value(cust_tab_name,"CRITERIA_VALUE",cust_new_value)
  ELSE
   SET cust_rpt_missing = report_missing(cust_process_tab,cust_process_col,cnvtreal(cust_from_value))
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ENDIF
 ELSE
  CALL put_value(cust_tab_name,"CRITERIA_VALUE",cust_from_value)
 ENDIF
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_6992
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"BT_SELECTION_CRIT_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"BT_SELECTION_CRIT_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"BT_SELECTION_CRIT_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name," BT_SELECTION_CRIT_ID")
     ELSE
      GO TO exit_6992
     ENDIF
    ELSE
   ...
   SET cust_from_value = get_value(cust_tab_name,"BT_SELECTION_CRIT_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6992
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_value)
   ENDIF
   SET cust_new_value = get_value(cust_tab_name,"BT_SELECTION_CRIT_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6992
   ELSE
    SET dm2_ref_data_reply->qual[1].to_value = cnvtreal(cust_new_value)
   ENDIF
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"CRITERIA_VALUE")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg =
    "This row wasn't translated because of the CRITERIA_VALUE column"
