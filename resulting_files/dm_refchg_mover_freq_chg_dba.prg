CREATE PROGRAM dm_refchg_mover_freq_chg:dba
 ...
 ENDIF
 CALL check_logfile("dm_refchg_mover",".log","dm_refchg_mover_freq_chg LOGFILE")
 SET dm_err->eproc = "Beginning dm_refchg_mover_freq_chg"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET readme_data->status = "F"
 SET readme_data->message = "Starting dm_refchg_mover_freq_chg"
 DECLARE mfc_exist_flag = i4 WITH noconstant(0)
 DECLARE mfc_domain = vc
 DECLARE mfc_to_run = i4 WITH noconstant(0)
 DECLARE mfc_running = i4 WITH noconstant(0)
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
  ...
  WITH nocounter
 ;end select
 IF (check_error("Query 'RDDS CYCLE MOVER FREQUENCY' from dm_info table") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (mfc_exist_flag=0)
  INSERT  FROM dm_info
   SET info_domain = "DATA MANAGEMENT", info_name = "RDDS CYCLE MOVER FREQUENCY", info_number = 60,
    info_date = sysdate
   WITH nocounter
  ;end insert
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   mfc_domain = build("MERGE*",cnvtstring(di.info_number))
  WITH nocounter
 ;end select
 IF (check_error("Getting current environment id") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET readme_data->message = dm_err->emsg
  GO TO exit_program
 ENDIF
 CALL echo(mfc_domain)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE ((di.info_name="RDDS MOVERS RUNNING") OR (di.info_name="RDDS MOVERS TO RUN"))
   AND di.info_domain=patstring(mfc_domain)
   AND di.info_number > 0
  DETAIL
   IF (di.info_name="RDDS MOVERS RUNNING")
    mfc_running = di.info_number
   ELSEIF (di.info_name="RDDS MOVERS TO RUN")
    mfc_to_run = di.info_number
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error("Checking mover to and mover running") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET readme_data->message = dm_err->emsg
  GO TO exit_program
 ENDIF
 CALL echo(build("mfc_to_run = ",mfc_to_run,"and mfc_running =",mfc_running))
 IF (mfc_to_run=0
  AND mfc_running=0)
  DELETE  FROM dm_info
   ...
   WITH nocounter
  ;end delete
  IF (check_error("deleting DM2_CYCLE_DATA_MOVERS") != 0)
   ROLLBACK
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET readme_data->message = dm_err->emsg
   GO TO exit_program
 ...
 SET readme_data->message = "Script dm_refchg_mover_freq_chg finished successfully"
#exit_program
END GO
