CREATE PROGRAM bbt_get_inv_evnt_ant:dba
 RECORD reply(
   1 more_records = vc
   1 product_id = f8
   1 qual[*]
 ...
 SUBROUTINE get_products(nodata)
   SELECT INTO "nl:"
    ...
    PLAN (d_st)
     JOIN (st
     WHERE (st.special_testing_cd=request->antigen_data[d_st.seq].antigen_cd)
      AND ((st.product_id+ 0) > 0.0)
      AND st.active_ind=1)
     JOIN (d_pe)
     JOIN (pe
     WHERE pe.product_id=st.product_id
