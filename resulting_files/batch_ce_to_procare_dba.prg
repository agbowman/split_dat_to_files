CREATE PROGRAM batch_ce_to_procare:dba
 DECLARE fextracttypecd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",14135,"CLIN EVENT"))
 DECLARE itotalevents = i4 WITH protect, noconstant(0)
 DECLARE ievents2process = i4 WITH protect, noconstant(0)
 DECLARE iiterationidx = i4 WITH protect, noconstant(0)
 DECLARE log_filename = vc WITH constant("PROCARE_BATCH_CE")
 ...
 IF (idebugind=1)
  ...
  SET stempstr = build2(format(d_incrementalfrom,"@SHORTDATETIME")," AND ",format(d_incrementalto,
    "@SHORTDATETIME"))
  CALL omflogprint(concat("Processing Clinical Events updated between: ",stempstr))
  CALL omflogprint(" ")
 ENDIF
 MERGE INTO omf_clinical_event_st oces
 USING (SELECT
  ce.event_id, ce.encntr_id, ce.person_id,
  ce.order_id, ce.catalog_cd, ce.task_assay_cd,
  ce.event_cd, ce.normalcy_cd, ce.result_val,
  ce.parent_event_id, ce.event_title_text
  FROM clinical_event ce
  WHERE ce.updt_dt_tm BETWEEN cnvtdatetime(d_incrementalfrom) AND cnvtdatetime(d_incrementalto)
   AND ce.view_level > 0
   AND ce.valid_until_dt_tm=cnvtdatetime("31-DEC-2100"))
 CLIN_EVENT ON (oces.event_id=clin_event.event_id)
 WHEN MATCHED THEN
 (UPDATE
  SET oces.process_flag = 2, oces.encntr_id = clin_event.encntr_id, oces.person_id = clin_event
   .person_id,
   oces.order_id = clin_event.order_id, oces.catalog_cd = clin_event.catalog_cd, oces.task_assay_cd
    = clin_event.task_assay_cd,
   oces.event_cd = clin_event.event_cd, oces.normalcy_cd = clin_event.normalcy_cd, oces.result_val =
   clin_event.result_val,
   oces.service_dt_nbr = - (1), oces.service_min_nbr = - (1), oces.updt_id = reqinfo->updt_id,
   oces.updt_cnt = (oces.updt_cnt+ 1), oces.updt_task = reqinfo->updt_task, oces.updt_applctx =
   reqinfo->updt_applctx,
   oces.updt_dt_tm = cnvtdatetime(sysdate), oces.parent_event_id = clin_event.parent_event_id, oces
   .event_title_text = clin_event.event_title_text
  WHERE oces.event_id=clin_event.event_id
 ;end update
 )
 WHEN NOT MATCHED THEN
 (INSERT  FROM oces
  (oces.event_id, oces.process_flag, oces.encntr_id,
  oces.person_id, oces.order_id, oces.catalog_cd,
  oces.task_assay_cd, oces.event_cd, oces.normalcy_cd,
  oces.result_val, oces.service_dt_nbr, oces.service_min_nbr,
  oces.updt_id, oces.updt_cnt, oces.updt_task,
  oces.updt_applctx, oces.updt_dt_tm, oces.parent_event_id,
  oces.event_title_text)
  VALUES(clin_event.event_id, 1, clin_event.encntr_id,
  clin_event.person_id, clin_event.order_id, clin_event.catalog_cd,
  clin_event.task_assay_cd, clin_event.event_cd, clin_event.normalcy_cd,
  clin_event.result_val, - (1), - (1),
  reqinfo->updt_id, 0, reqinfo->updt_task,
  reqinfo->updt_applctx, cnvtdatetime(sysdate), clin_event.parent_event_id,
  clin_event.event_title_text)
 ;end insert
 )
 WITH nocounter, maxcommit = value(c_ccl_block_large)
 SET itotalevents = curqual
 IF (idebugind=1)
  SET stempstr = concat("Total Clinical Events to process: ",build(itotalevents))
  CALL omflogprint(stempstr)
 ENDIF
 WHILE (itotalevents > 0)
   SET iiterationidx += 1
   IF (itotalevents < value(c_ccl_block_large))
    SET ievents2process = itotalevents
   ELSE
    SET ievents2process = value(c_ccl_block_large)
   ENDIF
   IF (idebugind=1)
    SET stempstr = concat("   Iteration #",build(iiterationidx)," - Processing ",build(
      ievents2process)," clinical events.")
    CALL omflogprint(stempstr)
   ENDIF
   EXECUTE omf_clin_event_batch_ops value(ievents2process), 1
   IF (validate(omf_ins_interface_error_request->data)=1)
    IF (size(omf_ins_interface_error_request->data,5) > 0)
     IF (idebugind=1)
      SET stempstr = concat("      ERR: ",build(size(omf_ins_interface_error_request->data,5)),
       " clinical events could not be processed, see OMF_INTERFACE_ERROR table for further information."
       )
      CALL omflogprint(stempstr)
    ...
    ENDIF
   ENDIF
   SET itotalevents -= value(ievents2process)
   COMMIT
 ENDWHILE
...
#end_batch
 IF (idebugind=1)
  CALL omflogprint(concat("Procare Clinical Event batch complete with status: ",reply->status_data.
    status))
