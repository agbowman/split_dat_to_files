CREATE PROGRAM cv_chk_omf_cvgsts:dba
 SET error_msg = fillstring(200," ")
 SET failure = "F"
 SET v_count1 = 0
 SET v_cnt_indicator = 497
 SET v_cnt_pv_view = 5
 SET v_cnt_view_options = 458
 SET v_cnt_view_indicator = 933
 SET v_cnt_grid = 5
 SET v_cnt_grid_column = 40
 SET v_cnt_calc_indicator = 6
 ...
 IF (failure="T")
  ...
  IF (v_indicator_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_indicator_CVGSTS")
  ENDIF
  IF (v_pv_view_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_pv_view_CVGSTS")
  ENDIF
  IF (v_view_options_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_vo_type/omf_vo_type_display_CVGSTS")
  ENDIF
  IF (v_view_indicator_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_view_indicator_CVGSTS")
  ENDIF
  IF (v_grid_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_grid_CVGSTS")
  ENDIF
  IF (v_grid_column_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_grid_column_CVGSTS")
  ENDIF
  IF (v_calc_indicator_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_calc_indicator_CVGSTS")
  ENDIF
  CALL echo(error_msg)
 ELSE
  CALL echo("CVNet OMF STS Views successfully loaded!")
 ENDIF
END GO
