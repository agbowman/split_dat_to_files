CREATE PROGRAM dm2_cb_manage_all:dba
 ...
 SUBROUTINE dcm_error(null)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dcma_param_int = i4
 DECLARE dcma_param_vc = c132
 DECLARE dcma_ques_hold = i4
 DECLARE dcma_act_str = c132
 SET dcma_param_int = 0
 SET dcma_param_vc = ""
 SET dcma_ques_hold = 0
 SET dcma_act_str = ""
 FREE RECORD dcma_action
 RECORD dcma_action(
   1 act[*]
     2 action = vc
 )
 SET dcma_ques_in =  $1
 SET dcma_param_int =  $2
 IF (dcma_param_int=1)
  SET dcma_param_vc = "BUILD"
 ELSE
  SET dcma_param_vc = "DROP"
 ENDIF
 SET dcma_act_cnt = 0
 FOR (dcma_act_cnt = 1 TO size(dccs_master->question,5))
   IF ((dcma_ques_in=dccs_master->question[dcma_act_cnt].question_nbr))
    SET dcma_ques_hold = dcma_act_cnt
    SET dcma_act_cnt = size(dccs_master->question,5)
   ENDIF
 ENDFOR
 SET dcma_act_cnt = 0
 IF (size(dccs_master->question[dcma_ques_hold].object,5) > 0)
  SET dcma_act_cnt = (dcma_act_cnt+ 1)
  SET dcma_act_str = concat("dm2_cb_manage_schema ",trim(cnvtstring(dcma_ques_in)),",'",trim(
    dcma_param_vc),"'")
  SET stat = alterlist(dcma_action->act,dcma_act_cnt)
  SET dcma_action->act[dcma_act_cnt].action = concat(trim(dcma_act_str)," go")
 ENDIF
 IF (size(dccs_master->question[dcma_ques_hold].request,5) > 0)
  SET dcma_act_cnt = (dcma_act_cnt+ 1)
  SET dcma_act_str = concat("dm2_cb_manage_reqpro ",trim(cnvtstring(dcma_ques_in)),",",cnvtstring(
    dcma_param_int))
  SET stat = alterlist(dcma_action->act,dcma_act_cnt)
  SET dcma_action->act[dcma_act_cnt].action = concat(trim(dcma_act_str)," go")
 ENDIF
 SET dcma_act_cnt = 0
 FOR (dcma_act_cnt = 1 TO size(dcma_action->act,5))
   CALL execute_actions(dcma_action->act[dcma_act_cnt].action)
 ENDFOR
END GO
