CREATE PROGRAM dm_rmc_parse_pkw:dba
 ...
 DECLARE dm_trans3(sbr_tbl_name=vc,sbr_col_name=vc,sbr_from_val=f8,sbr_src_ind=i2,sbr_pe_tbl_name=vc)
  = vc
 DECLARE dm_trans4(sbr_re_name=vc,sbr_from_val_str=vc,sbr_src_ind=i2) = vc
 ...
 SUBROUTINE dm_trans2(sbr_tbl_name,sbr_col_name,sbr_from_val,sbr_src_ind)
   ...
   DECLARE dt_temp_tbl_cnt = i4
   DECLARE dt_temp_col_cnt = i4
   DECLARE dt_root_tbl_cnt = i4
   DECLARE dt_cur_tbl = i4
   DECLARE dt2_parent_drdm_row = i2 WITH protect, noconstant(0)
   IF (sbr_src_ind=0)
    SET to_val = "NOXLAT"
    SET dt_temp_tbl_cnt = locateval(index_var,1,perm_tbl_cnt,sbr_tbl_name,dm2_ref_data_doc->tbl_qual[
     index_var].table_name)
    ...
    IF (dt_root_tbl_cnt=0)
     SET dt_cur_tbl = temp_tbl_cnt
     SET dt_root_tbl_cnt = fill_rs("TABLE",dm2_ref_data_doc->tbl_qual[dt_temp_tbl_cnt].col_qual[
      dt_temp_col_cnt].root_entity_name)
     SET temp_tbl_cnt = dt_cur_tbl
    ENDIF
    IF ((dt_root_tbl_cnt=- (2)))
     RETURN("REQDDL")
    ENDIF
    IF ((dm2_ref_data_doc->tbl_qual[dt_root_tbl_cnt].mergeable_ind=0))
     SET dt2_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
     SET drdm_chg->log[dt2_parent_drdm_row].chg_log_reason_txt = concat("Column ",sbr_col_name,
      " references value ",trim(cnvtstring(sbr_from_val,20,1)),
      ", which comes from non-mergeable table ",
      dm2_ref_data_doc->tbl_qual[dt_temp_tbl_cnt].col_qual[dt_temp_col_cnt].root_entity_name,
      " in SOURCE.")
     RETURN("NOMV04")
    ENDIF
   ...
   IF (to_val="No Trans")
    ...
    IF (to_val="ORPHAN")
     SET dt2_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
     SET drdm_chg->log[dt2_parent_drdm_row].chg_log_reason_txt = concat("Column ",sbr_col_name,
      " with a value of ",trim(cnvtstring(sbr_from_val,20,1)),
 ...
 SUBROUTINE dm_trans3(sbr_tbl_name,sbr_col_name,sbr_from_val,sbr_src_ind,sbr_pe_tbl_name)
   ...
   DECLARE dt3_data_type = vc
   DECLARE dt3_num = i4
   DECLARE dt3_plsql_call = vc WITH protect, noconstant("")
   DECLARE dt3_prev_ctxt = vc WITH protect, noconstant("")
   DECLARE dt3_root_tbl_cnt = i4
   DECLARE dt3_parent_drdm_row = i2 WITH protect, noconstant(0)
   DECLARE dt3_colstring_ret = vc WITH protect, noconstant("")
   DECLARE dt3_col_datatype = i2 WITH protect, noconstant(0)
   DECLARE dt3_plsql_parent_row = f8 WITH protect, noconstant(0.0)
   DECLARE dt3_plsql_parm_loop = i4 WITH protect, noconstant(0)
   DECLARE dt3_parm_pos = i4 WITH protect, noconstant(0)
   DECLARE dt3_lms_return = i4 WITH protect, noconstant(0)
   DECLARE dt3_pe_name = vc WITH protect, noconstant("")
   FREE RECORD drt_request
   RECORD drt_request(
     1 sbr_p_e_name = vc
     1 sbr_p_e_col = i4
   )
   FREE RECORD drt_reply
   RECORD drt_reply(
     1 pe_name = vc
   )
   IF (sbr_from_val=0)
   ...
   IF (sbr_src_ind=0)
    ...
    IF ((dm2_ref_data_doc->tbl_qual[dt3_temp_tbl_cnt].col_qual[dt3_temp_col_cnt].object_name > " ")
     AND (dm2_ref_data_doc->tbl_qual[dt3_temp_tbl_cnt].col_qual[dt3_temp_col_cnt].execution_flag IN (
    1, 2)))
     SET dt3_plsql_ret = drmmc_call_custom_plsql(dt3_temp_tbl_cnt,dt3_temp_col_cnt,drdm_log_loop,1)
     RETURN(dt3_plsql_ret)
    ELSE
     IF ((dm2_ref_data_doc->tbl_qual[dt3_temp_tbl_cnt].col_qual[dt3_temp_col_cnt].exception_flg=1))
      RETURN(cnvtstring(sbr_from_val,20,1))
     ELSE
      IF ((dm2_ref_data_doc->tbl_qual[dt3_temp_tbl_cnt].col_qual[dt3_temp_col_cnt].root_entity_name
       IN ("", " ")))
       DECLARE evaluate_pe_name() = c255
       SET dt3_pe_name = dm2_ref_data_doc->tbl_qual[dt3_temp_tbl_cnt].col_qual[dt3_temp_col_cnt].
       parent_entity_col
       CALL parser("select into 'nl:' pe_name = evaluate_pe_name( ",0)
       IF (findstring(char(0),sbr_pe_tbl_name)=0)
        CALL parser("sbr_tbl_name,sbr_col_name,dt3_pe_name,sbr_pe_tbl_name)",0)
       ELSE
        CALL parser("sbr_tbl_name,sbr_col_name,dt3_pe_name,char(0))",0)
       ENDIF
       CALL parser("from dual detail dt3_pk_tab_name = pe_name with nocounter go",1)
       IF (((dt3_pk_tab_name="") OR (findstring(char(0),dt3_pk_tab_name) > 0)) )
       ...
       IF (dt3_pk_tab_name != "")
        IF (dt3_pk_tab_name != "INVALIDTABLE")
         ...
         ENDIF
         SET to_val = select_merge_translate(cnvtstring(sbr_from_val,20,1),dt3_pk_tab_name)
        ELSE
         SET to_val = "NOMV38"
         SET dt3_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
         SET drdm_chg->log[dt3_parent_drdm_row].chg_log_reason_txt = concat("Column ",dt3_pe_name,
          " contains a value which is either blank, null, or a char(0)")
        ENDIF
       ENDIF
       IF (to_val="No Trans")
        ...
        IF ((dm2_ref_data_doc->tbl_qual[dt3_root_tbl_cnt].mergeable_ind=0))
         SET to_val = "NOMV04"
         SET dt3_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
         SET drdm_chg->log[dt3_parent_drdm_row].chg_log_reason_txt = concat("Column ",sbr_col_name,
          " references value ",trim(cnvtstring(sbr_from_val,20,1)),
          ", which comes from non-mergeable table ",
          dt3_pk_tab_name," in SOURCE.")
        ELSE
      ...
      ELSE
       ...
       "PERSON")
        ...
        SET dm2_ref_data_doc->tbl_qual[dt3_temp_tbl_cnt].col_qual[dt3_temp_col_cnt].root_entity_name
         = "PRSNL"
       ENDIF
       IF ((dm2_ref_data_doc->tbl_qual[dt3_temp_tbl_cnt].root_col_name=dm2_ref_data_doc->tbl_qual[
       dt3_temp_tbl_cnt].col_qual[dt3_temp_col_cnt].column_name))
        SET dt3_prev_ctxt = select_merge_translate_rec->from_opt_ctxt
        SET select_merge_translate_rec->from_opt_ctxt = ""
       ENDIF
       SET to_val = select_merge_translate(cnvtstring(sbr_from_val,20,1),dm2_ref_data_doc->tbl_qual[
        dt3_temp_tbl_cnt].col_qual[dt3_temp_col_cnt].root_entity_name)
       IF ((dm2_ref_data_doc->tbl_qual[dt3_temp_tbl_cnt].root_col_name=dm2_ref_data_doc->tbl_qual[
       dt3_temp_tbl_cnt].col_qual[dt3_temp_col_cnt].column_name))
        SET select_merge_translate_rec->from_opt_ctxt = dt3_prev_ctxt
       ENDIF
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET dm_err->err_ind = 0
       ...
       IF (to_val="No Trans")
        ...
        IF ((dm2_ref_data_doc->tbl_qual[dt3_root_tbl_cnt].mergeable_ind=0))
         ...
         SET drdm_chg->log[dt3_parent_drdm_row].chg_log_reason_txt = concat("Column ",sbr_col_name,
          " references value ",trim(cnvtstring(sbr_from_val,20,1)),
          ", which comes from non-mergeable table ",
          dm2_ref_data_doc->tbl_qual[dt3_temp_tbl_cnt].col_qual[dt3_temp_col_cnt].root_entity_name,
          " in SOURCE.")
        ELSE
    ...
    ENDIF
   ELSE
    SET dt3_num = isnumeric(sbr_from_val)
    SET to_val = cnvtstring(sbr_from_val,20,1)
    SELECT INTO "NL:"
     dtal.*
     FROM dtableattr dta,
      dtableattrl dtal,
     ...
     PLAN (dta
      WHERE dta.table_name=sbr_tbl_name)
      JOIN (utc
      WHERE utc.table_name=dta.table_name
       AND utc.column_name=sbr_col_name)
      JOIN (dtal
      WHERE dtal.structtype="F"
       AND btest(dtal.stat,11)=0
       AND dtal.attr_name=utc.column_name)
     DETAIL
      dt3_data_type = concat(dtal.type,cnvtstring(dtal.len))
     WITH nocounter
    ;end select
    IF (dt3_num=2
     AND dt3_data_type="F8")
     IF (findstring(".0",to_val)=0)
      SET to_val = concat(to_val,".0")
     ENDIF
    ENDIF
   ...
   RETURN(to_val)
 END ;Subroutine
 SUBROUTINE dm_trans4(sbr_re_name,sbr_from_val_str,sbr_src_ind)
   DECLARE dm_refchg_b10tob62() = c10
   DECLARE dm_refchg_b62tob10() = f8
   DECLARE dt4_from_val = f8 WITH protect, noconstant(0.0)
   DECLARE dt4_tab_pos = i4 WITH protect, noconstant(0)
   DECLARE dt4_col_name = vc WITH protect, noconstant(" ")
   DECLARE dt4_trans3_return = vc WITH protect, noconstant(" ")
   DECLARE dt4_trans3_value = f8 WITH protect, noconstant(0.0)
   DECLARE dt4_cur_tbl = i4 WITH protect, noconstant(0)
   DECLARE dt4_return = vc WITH protect, noconstant("NOTRANS")
   IF (sbr_from_val_str="<DM2NULLVAL>")
    RETURN("^<DM2NULLVAL>^")
   ELSEIF (sbr_from_val_str=char(0))
    RETURN("char(0)")
   ENDIF
   IF (sbr_src_ind=1)
    RETURN(concat("^",sbr_from_val_str,"^"))
   ELSE
    SET dt4_tab_pos = locateval(dt4_tab_pos,1,perm_tbl_cnt,sbr_re_name,dm2_ref_data_doc->tbl_qual[
     dt4_tab_pos].table_name)
    IF (dt4_tab_pos=0)
     SET dt4_cur_tbl = temp_tbl_cnt
     SET dt4_tab_pos = fill_rs("TABLE",sbr_re_name)
     SET temp_tbl_cnt = dt4_cur_tbl
     IF ((dt4_tab_pos=- (1)))
      CALL disp_msg("Table can't be created from remote mover - DM_TRANS4",dm_err->logfile,1)
      RETURN("NOTRANS")
     ENDIF
     IF ((dt4_tab_pos=- (2)))
      RETURN("REQDDL")
     ENDIF
    ENDIF
    SET dt4_col_name = dm2_ref_data_doc->tbl_qual[dt4_tab_pos].root_col_name
    SELECT INTO "NL:"
     y = dm_refchg_b62tob10(sbr_from_val_str)
     FROM dual
     DETAIL
      dt4_from_val = y
     WITH nocounter
    ;end select
    IF (check_error("Converting B62 to B10 in DM_TRANS4")=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dm_err->err_ind = 0
     RETURN("NOTRANS")
    ENDIF
    SET dt4_trans3_return = dm_trans3(sbr_re_name,dt4_col_name,dt4_from_val,sbr_src_ind," ")
    IF (check_error("Calling the DM_TRANS3 subroutine from DM_TRANS4")=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dm_err->err_ind = 0
     RETURN("NOTRANS")
    ENDIF
    IF (isnumeric(dt4_trans3_return)=2)
     SET dt4_trans3_value = cnvtreal(dt4_trans3_return)
     SELECT INTO "NL:"
      y = dm_refchg_b10tob62(dt4_trans3_value)
      FROM dual
      DETAIL
       dt4_return = y, dt4_return = concat("^",dt4_return,"^")
      WITH nocounter
     ;end select
     IF (check_error("Converting B10 to B62 in DM_TRANS4")=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dm_err->err_ind = 0
      RETURN("NOTRANS")
     ENDIF
    ELSE
     SET dt4_return = concat("^",dt4_trans3_return,"^")
    ENDIF
   ENDIF
   RETURN(dt4_return)
 END ;Subroutine
 DECLARE drpp_table_name = vc WITH protect
 DECLARE v_alt_del_pk_ = vc WITH protect
 DECLARE src_pk_where = vc WITH protect
 DECLARE i_pkw = vc WITH protect
 DECLARE v_temp_idx = i2 WITH protect
 DECLARE v_temp = i2 WITH protect
 DECLARE v_temp2 = i2 WITH protect
 DECLARE v_delim = c1 WITH protect
 DECLARE v_stop = i2 WITH protect, noconstant(1)
 DECLARE v_col_cnt = i4 WITH protect
 DECLARE v_col_idx = i4 WITH protect
 FREE RECORD col_info
 RECORD col_info(
   1 total = i4
   1 cols[*]
     2 col_name = vc
 ...
 SET drpp_table_name = drpp_request->table_name
 SELECT INTO "NL:"
  ...
  FROM dtableattr dta,
   ...
   user_tab_cols utc
  PLAN (dta
   WHERE dta.table_name=drpp_table_name)
   JOIN (dtal
   WHERE dtal.structtype="F"
    AND btest(dtal.stat,11)=0)
   JOIN (utc
  ...
  HEAD REPORT
   col_cnt = 0
  DETAIL
   col_cnt = (col_cnt+ 1), stat = alterlist(col_info->cols,col_cnt), col_info->cols[col_cnt].col_name
    = dtal.attr_name
   IF (dtal.type="F")
    col_info->cols[col_cnt].data_type = "F8"
   ELSEIF (dtal.type="I")
    col_info->cols[col_cnt].data_type = "I4"
   ELSEIF (dtal.type="C")
    IF (btest(dtal.stat,13))
     col_info->cols[col_cnt].data_type = "VC"
    ELSE
     col_info->cols[col_cnt].data_type = build(dtal.type,dtal.len)
    ENDIF
   ELSEIF (dtal.type="Q")
    col_info->cols[col_cnt].data_type = "DQ8"
   ENDIF
  FOOT REPORT
   col_info->total = col_cnt
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (((findstring("DM_TRANS2",cnvtupper(drpp_request->pk_where),1,0) > 0) OR (((findstring(
  "DM_TRANS3",cnvtupper(drpp_request->pk_where),1,0) > 0) OR (findstring("DM_TRANSLATE",cnvtupper(
   drpp_request->pk_where),1,0) > 0)) )) )
  SET v_alt_del_pk = replace(drpp_request->pk_where,"<SOURCE_IND>","1",0)
  CALL parser(concat("set src_pk_where = ",v_alt_del_pk," go"),1)
 ELSE
  SET src_pk_where = drpp_request->pk_where
 ENDIF
 SET i_pkw = trim(substring(6,size(src_pk_where,1),trim(src_pk_where,3)),3)
 SET v_col_cnt = 0
 WHILE (v_stop=1)
   SET v_temp = findstring("=",i_pkw,1,0)
   SET v_temp2 = findstring("IS NULL",cnvtupper(i_pkw),1,0)
   IF (v_temp2 > 0)
    IF (((v_temp2 < v_temp) OR (v_temp=0)) )
     SET v_temp = v_temp2
     SET v_temp2 = - (1)
    ENDIF
   ENDIF
   IF (v_temp=0)
    SET v_stop = 0
   ELSE
    SET v_col_cnt = (v_col_cnt+ 1)
    SET stat = alterlist(drpp_reply->cols,v_col_cnt)
    SET drpp_reply->cols[v_col_cnt].col_name = trim(substring(1,(v_temp - 1),i_pkw),3)
    IF (v_temp2 >= 0)
     SET i_pkw = trim(substring((v_temp+ 1),size(i_pkw,1),i_pkw),3)
    ELSE
     SET i_pkw = trim(substring(v_temp,size(i_pkw,1),i_pkw),3)
    ENDIF
    SET v_temp = findstring(".",drpp_reply->cols[v_col_cnt].col_name,1,0)
    IF (v_temp > 0)
     SET drpp_reply->cols[v_col_cnt].col_name = cnvtupper(substring((v_temp+ 1),size(drpp_reply->
        cols[v_col_cnt].col_name,1),drpp_reply->cols[v_col_cnt].col_name))
    ENDIF
    SET v_col_idx = locateval(v_temp_idx,1,col_info->total,drpp_reply->cols[v_col_cnt].col_name,
     col_info->cols[v_temp_idx].col_name)
    SET drpp_reply->cols[v_col_cnt].data_type = col_info->cols[v_col_idx].data_type
    IF ((col_info->cols[v_col_idx].data_type="*C*"))
     SET v_delim = substring(1,1,i_pkw)
     IF (cnvtupper(v_delim)="C")
      SET v_temp = findstring(") AND",cnvtupper(i_pkw),1,0)
      IF (v_temp > 0)
       SET drpp_reply->cols[v_col_cnt].col_value = trim(substring(1,(v_temp+ 1),i_pkw),3)
       SET i_pkw = trim(substring((v_temp+ 5),size(i_pkw,1),i_pkw),3)
      ELSE
       SET drpp_reply->cols[v_col_cnt].col_value = i_pkw
       SET v_stop = 0
      ENDIF
     ELSEIF (cnvtupper(v_delim)="I")
      SET drpp_reply->cols[v_col_cnt].col_value = "NULL"
      SET v_temp = findstring("AND",cnvtupper(i_pkw),1,0)
      IF (v_temp > 0)
       SET i_pkw = trim(substring((v_temp+ 3),size(i_pkw,1),i_pkw),3)
      ELSE
       SET v_stop = 0
      ENDIF
     ELSE
      SET v_temp = findstring(v_delim,i_pkw,2,0)
      SET drpp_reply->cols[v_col_cnt].col_value = substring(1,v_temp,i_pkw)
      IF (v_temp < size(i_pkw,1))
       SET v_temp = findstring("AND",cnvtupper(i_pkw),v_temp,0)
       SET i_pkw = trim(substring((v_temp+ 3),size(i_pkw,1),i_pkw),3)
      ELSE
       SET v_stop = 0
      ENDIF
   ...
   ENDIF
 ENDWHILE
 SET drpp_reply->col_cnt = v_col_cnt
#exit_program
END GO
