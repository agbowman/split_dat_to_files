CREATE PROGRAM dm2_publish_dbstats_tab:dba
 ...
 SUBROUTINE dm2_adb_check(dac_db_link,dac_adb_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dpdt_inowner = vc WITH protect, noconstant(" ")
 DECLARE dpdt_intable = vc WITH protect, noconstant(" ")
 DECLARE dpdt_instatid = vc WITH protect, noconstant(" ")
 DECLARE dpdt_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dpdt_info_exists = i2 WITH protect, noconstant(0)
 DECLARE dpdt_step_beg_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dpdt_step_end_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dpdt_process_status = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dpdt_cmd = vc WITH protect, noconstant(" ")
 DECLARE dpdt_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dpdt_emsg = vc WITH protect, noconstant(" ")
 DECLARE dpdt_retry_attempts = i2 WITH protect, noconstant(0)
 DECLARE dpdt_retry_ceiling = i2 WITH protect, noconstant(10)
 DECLARE dpdt_allow_publish = i2 WITH protect, noconstant(0)
 DECLARE dpdt_blocking_str = vc WITH protect, noconstant("")
 DECLARE dpdt_in_parse_ind = i2 WITH protect, noconstant(0)
 DECLARE dpdt_wait_pref_ind = i2 WITH protect, noconstant(0)
 DECLARE dpdt_adb_ind = i2 WITH protect, noconstant(0)
 DECLARE import_table_stats_noinv(ownname=vc,tabname=vc,stattab=vc,statid=vc,statown=vc,
  cascade=i4,no_invalidate=i4,force=i4) = null WITH sql = "SYS.DBMS_STATS.IMPORT_TABLE_STATS",
 parameter
 DECLARE import_table_stats(ownname=vc,tabname=vc,stattab=vc,statid=vc,statown=vc,
  cascade=i4,force=i4) = null WITH sql = "SYS.DBMS_STATS.IMPORT_TABLE_STATS", parameter
 ...
 IF (check_logfile("dm2_pdbs_tab",".log","dm2_publish_dbstats_tab LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_publish_dbstats_tab"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dpdt_inowner = trim(cnvtupper( $1),3)
 SET dpdt_intable = trim( $2,3)
 SET dpdt_instatid = trim(cnvtupper( $3),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = "Parameter usage: dm2_publish_dbstats_tab  '<owner>','<table_name>','<STATID>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_add_detail_text("OWNER",dpdt_inowner)
 CALL dm2_process_log_add_detail_text("TABLE_NAME",dpdt_intable)
 CALL dm2_process_log_add_detail_text("TABLE_OWNER.TABLE_NAME",concat(dpdt_inowner,".",dpdt_intable))
 CALL dm2_process_log_add_detail_text("STATID",dpdt_instatid)
 ...
 SET dpdt_log_id = dm2_process_event_rs->dm_process_event_id
 IF (ddf_autosuccess_init(null)=0)
  GO TO exit_script
 ELSEIF ((dgdt_prefs->autosuccess_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Checking DM_PROCESS_QUEUE non-SUCCESS status for table (",dpdt_intable,
  ").")
 SELECT INTO "nl:"
  ...
  WHERE d.process_type IN (dpq_statistics, dpq_freq_statistics)
   ...
   AND d.object_type="TABLE"
   AND d.owner_name=dpdt_inowner
   AND d.object_name=dpdt_intable
   AND d.process_status != dpl_success
  DETAIL
   dpdt_process_status = d.process_status
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual > 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Publish request for TABLE object (",dpdt_intable,") ","for owner (",
   dpdt_inowner,
   ") is denied."," TABLE was found in DM_PROCESS_QUEUE in a non-SUCCESS status.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Checking DM_STAT_TABLE for statid existence for table (",dpdt_intable,
  ").")
 SELECT INTO "nl:"
  FROM dm_stat_table d
  WHERE d.statid=dpdt_instatid
   AND d.type="T"
   AND d.c5=dpdt_inowner
   AND d.c1=dpdt_intable
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual=0)
  SET dm_err->emsg = concat("Publish request for TABLE object (",dpdt_intable,") ","for owner (",
   dpdt_inowner,
   ") was bypassed due to statid (",dpdt_instatid,") related data not existing in DM_STAT_TABLE.")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Checking DBA_TABLES for existence of table (",dpdt_intable,").")
 SELECT INTO "nl:"
  FROM dba_tables d
  WHERE d.owner=dpdt_inowner
   AND d.table_name=dpdt_intable
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual=0)
  SET dm_err->emsg = concat("Publish request for TABLE object (",dpdt_intable,") ","for owner (",
   dpdt_inowner,
   ") was bypassed due to table ","not existing in DBA_TABLES.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  SET dm_err->eproc = concat("Deleting statistics for non-existent table (",dpdt_intable,
   ") from DM_STAT_TABLE, STATID (",dpdt_instatid,").")
  CALL disp_msg(" ",dm_err->logfile,0)
  CALL delete_table_stats(dpdt_inowner,concat('"',dpdt_intable,'"'),"DM_STAT_TABLE",dpdt_instatid,
   "V500",
   cnvtbool(true))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Importing statistics for table (",dpdt_intable,
  ") from DM_STAT_TABLE, STATID (",dpdt_instatid,").")
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (ddf_get_publish_retry(dpdt_retry_ceiling)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_adb_check("",dpdt_adb_ind)=0)
  GO TO exit_script
 ENDIF
 IF (ddf_check_wait_pref(dpdt_wait_pref_ind,dpdt_adb_ind)=0)
  GO TO exit_script
 ENDIF
 SET dpdt_retry_attempts = 0
 SET dpdt_allow_publish = 0
 WHILE (dpdt_allow_publish=0
  AND ((dpdt_retry_attempts < dpdt_retry_ceiling) OR ((dpdt_retry_ceiling=- (1)))) )
   SET dpdt_allow_publish = 1
   SET dpdt_blocking_str = ""
   IF (dpdt_wait_pref_ind=0)
    IF (ddf_check_in_parse(dpdt_inowner,dpdt_intable,dpdt_in_parse_ind,dpdt_blocking_str)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   IF (dpdt_in_parse_ind=1)
    SET dpdt_allow_publish = 0
   ENDIF
   IF ((dpdt_retry_ceiling=- (1))
    AND dpdt_allow_publish=0)
    SET dpdt_allow_publish = 1
    SET dm_err->eproc = "Parse detection overridden. Allowing Publish."
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF (dpdt_allow_publish=1)
    IF (validate(dfgd_freq_gather_ind,- (1))=1)
     CALL import_table_stats_noinv(dpdt_inowner,concat('"',dpdt_intable,'"'),"DM_STAT_TABLE",
      dpdt_instatid,"V500",
      cnvtbool(false),cnvtbool(false),cnvtbool(true))
    ELSE
     CALL import_table_stats(dpdt_inowner,concat('"',dpdt_intable,'"'),"DM_STAT_TABLE",dpdt_instatid,
      "V500",
    ...
    ENDIF
   ENDIF
   IF (dpdt_allow_publish=0)
    SET dpdt_retry_attempts = (dpdt_retry_attempts+ 1)
    IF (dpdt_retry_attempts < dpdt_retry_ceiling)
     SET dm_err->eproc = concat(dpdt_blocking_str," Attempt ",trim(cnvtstring(dpdt_retry_attempts)),
      " of ",trim(cnvtstring(dpdt_retry_ceiling)),
      ".")
     CALL disp_msg(" ",dm_err->logfile,0)
     CALL dm2_process_log_add_detail_text("PARSE_EVENT:TABLOG",dm_err->eproc)
     IF (dm2_process_log_dtl_row(dpdt_log_id,0)=0)
      GO TO exit_script
     ENDIF
     CALL pause(10)
    ELSE
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat(dpdt_blocking_str," Attempt ",trim(cnvtstring(dpdt_retry_attempts)),
      " of ",trim(cnvtstring(dpdt_retry_ceiling)),
      ". Aborting Publish")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     CALL dm2_process_log_add_detail_text("PARSE_EVENT:TABFAILURE",dm_err->eproc)
     IF (dm2_process_log_dtl_row(dpdt_log_id,1)=0)
      GO TO exit_script
     ENDIF
     GO TO exit_script
    ENDIF
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Get DM2GDBS:PUBLISHADJUSTPROC-TAB row from DM_INFO."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2GDBS:PUBLISHADJUSTPROC-TAB"
  DETAIL
   dpdt_procs->cnt = (dpdt_procs->cnt+ 1), stat = alterlist(dpdt_procs->qual,dpdt_procs->cnt),
   dpdt_procs->qual[dpdt_procs->cnt].proc_name = trim(d.info_name),
   dpdt_procs->qual[dpdt_procs->cnt].proc_cmd = trim(d.info_char)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dpdt_procs)
 ENDIF
 IF ((dpdt_procs->cnt > 0))
  FOR (dgdi_proccnt = 1 TO dpdt_procs->cnt)
    SET dpdt_cmd = dpdt_procs->qual[dgdi_proccnt].proc_cmd
    SET dpdt_cmd = replace(dpdt_cmd,"<OBJECT_TYPE>","TABLE")
    SET dpdt_cmd = replace(dpdt_cmd,"<OBJECT_OWNER>",dpdt_inowner)
    SET dpdt_cmd = replace(dpdt_cmd,"<OBJECT_NAME>",dpdt_intable)
    SET dpdt_cmd = replace(dpdt_cmd,"<STATID>",dpdt_instatid)
    SET dgdi_step_beg_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dpdt_cmd = concat("RDB ASIS(^ begin ",dpdt_cmd,"; end; ^) go")
    IF (dm2_push_cmd(dpdt_cmd,1)=0)
    ...
    CALL dm2_process_log_add_detail_number(concat(dpdt_procs->qual[dgdi_proccnt].proc_name,":SECS"),
     datetimediff(dgdi_step_end_dttm,dgdi_step_beg_dttm,5))
    CALL dm2_process_log_add_detail_text(concat(dpdt_procs->qual[dgdi_proccnt].proc_name,":CMD"),
     dpdt_cmd)
    IF (dm2_process_log_dtl_row(dpdt_log_id,0)=0)
     GO TO exit_script
    ENDIF
  ENDFOR
 ENDIF
 SET dm_err->eproc = concat("Deleting statistics for table (",dpdt_intable,
  ") from DM_STAT_TABLE, STATID (",dpdt_instatid,").")
 CALL disp_msg(" ",dm_err->logfile,0)
 CALL delete_table_stats(dpdt_inowner,concat('"',dpdt_intable,'"'),"DM_STAT_TABLE",dpdt_instatid,
  "V500",
...
#exit_script
 ...
 SET dpdt_err_ind = dm_err->err_ind
 SET dpdt_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_execution,dpdt_log_id,1)
 SET dm_err->err_ind = dpdt_err_ind
 SET dm_err->emsg = dpdt_emsg
 SET dm_err->eproc = "Ending dm2_publish_dbstats_tab"
