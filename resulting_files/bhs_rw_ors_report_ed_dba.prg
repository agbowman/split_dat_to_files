CREATE PROGRAM bhs_rw_ors_report_ed:dba
 FREE RECORD work
 RECORD work(
   1 e_cnt = i4
   1 encntrs[*]
 ...
 DECLARE mf_bmc_edau_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",220,"EDAU"))
 ...
 ELSEIF (ms_loc_fac="BWH")
  CALL echo("bwh")
  SET cs220_er_group_cd = uar_get_code_by("DISPLAYKEY",220,"ERALLBWH")
 ELSE
  CALL echo("BMC")
  ...
  PLAN (d)
   ...
   (SELECT
    ...
    WHERE lg2.parent_loc_cd=cs220_er_group_cd
     ...
     AND elh.loc_nurse_unit_cd != mf_bmc_edau_cd)))
...
#exit_script
 ...
 SET last_mod = "007 09/11/17 SH013356			  Filtered out inactive encounter rows"
END GO
