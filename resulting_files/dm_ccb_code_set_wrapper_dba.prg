CREATE PROGRAM dm_ccb_code_set_wrapper:dba
 ...
 IF (size(request->filterlist,5) > 1)
  SET reply->message = "Must have only one search term."
  SET failed = "T"
  GO TO exit_script
 ENDIF
 ...
 IF (size(request->filterlist,5) < 1)
  IF ((request->code_set=- (1)))
   SET reply->message = "No search values given."
   SET failed = "T"
   GO TO exit_script
  ELSE
   IF (checkprg("DM_CCB_CSF_GET_CS_BY_NUMBER") > 0)
    EXECUTE dm_ccb_csf_get_cs_by_number
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_get_cs_by_number does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ENDIF
 ELSEIF (cnvtupper(request->filterlist[1].table_name)="CODE_VALUE_SET")
  IF (cnvtupper(request->filterlist[1].column_name)="DISPLAY_KEY")
   IF (checkprg("DM_CCB_CSF_GET_CS_BY_DISPLAY") > 0)
    EXECUTE dm_ccb_csf_get_cs_by_display
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_get_cs_by_display does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ELSE
   ...
   GO TO exit_script
  ENDIF
 ELSEIF (cnvtupper(request->filterlist[1].table_name)="CODE_VALUE")
  IF (cnvtupper(request->filterlist[1].column_name)="DISPLAY_KEY")
   IF (checkprg("DM_CCB_CSF_DISPLAYFILTER") > 0)
    EXECUTE dm_ccb_csf_displayfilter
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_displayfilter does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ELSEIF (cnvtupper(request->filterlist[1].column_name)="DESCRIPTION")
   IF (checkprg("DM_CCB_CSF_DESCRIPTION_FILTER") > 0)
    EXECUTE dm_ccb_csf_description_filter
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_description_filter does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ELSEIF (cnvtupper(request->filterlist[1].column_name)="CDF_MEANING")
   IF (checkprg("DM_CCB_CSF_CDF_MEANING_FILTER") > 0)
    EXECUTE dm_ccb_csf_cdf_meaning_filter
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_cdf_meaning_filter does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ELSEIF (cnvtupper(request->filterlist[1].column_name)="CODE_VALUE")
   IF (checkprg("DM_CCB_CSF_CV_NUMBER") > 0)
    EXECUTE dm_ccb_csf_cv_number
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_cv_number does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ELSE
 ...
 ELSEIF (cnvtupper(request->filterlist[1].table_name)="CODE_VALUE_ALIAS")
  IF (cnvtupper(request->filterlist[1].column_name)="ALIAS")
   IF (checkprg("DM_CCB_CSF_CVA_FILTER") > 0)
    EXECUTE dm_ccb_csf_cva_filter
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_cva_filter does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ELSEIF (cnvtupper(request->filterlist[1].column_name)="CONTRIBUTOR_SOURCE_CD")
   IF (checkprg("DM_CCB_CSF_CVA_CNTRBTR_SRC") > 0)
    EXECUTE dm_ccb_csf_cva_cntrbtr_src
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_cva_cntrbtr_src does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ELSE
 ...
 ELSEIF (cnvtupper(request->filterlist[1].table_name)="CODE_VALUE_OUTBOUND")
  IF (cnvtupper(request->filterlist[1].column_name)="ALIAS")
   IF (checkprg("DM_CCB_CSF_CVO_FILTER") > 0)
    EXECUTE dm_ccb_csf_cvo_filter
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_cvo_filter does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ELSEIF (cnvtupper(request->filterlist[1].column_name)="CONTRIBUTOR_SOURCE_CD")
   IF (checkprg("DM_CCB_CSF_CVO_CNTRBTR_SRC") > 0)
    EXECUTE dm_ccb_csf_cvo_cntrbtr_src
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_cvo_cntrbtr_src does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ELSE
 ...
 ELSEIF (cnvtupper(request->filterlist[1].table_name)="CODE_VALUE_EXTENSION")
  IF (cnvtupper(request->filterlist[1].column_name)="FIELD_VALUE")
   IF (checkprg("DM_CCB_CSF_CV_EXTENSION") > 0)
    EXECUTE dm_ccb_csf_cv_extension
   ELSE
    SET reply->message = "Child script: dm_ccb_csf_cv_extension does not exist."
    SET failed = "T"
    GO TO exit_script
   ENDIF
  ELSE
 ...
 IF (error(errmsg,0) != 0)
  ...
  GO TO exit_script
 ENDIF
 IF (size(reply->code_set_list,5) < 1)
  SET reply->message = "No search results found."
  SET failed = "T"
  GO TO exit_script
 ENDIF
 GO TO exit_script
