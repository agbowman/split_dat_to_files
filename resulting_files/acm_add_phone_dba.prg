CREATE PROGRAM acm_add_phone:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->phone_qual[xref->add[index].idx].phone_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(phone_seq,nextval)
     FROM dual
     DETAIL
      acm_request->phone_qual[xref->add[index].idx].phone_id = cnvtreal(nextseqnum), reply->
      phone_qual[xref->add[index].idx].phone_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->phone_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->phone_qual[xref->add[index].idx].phone_id = acm_request->phone_qual[xref->add[index].
    idx].phone_id
   ENDIF
   SET reply->phone_qual[xref->add[index].idx].status = 0
   IF ((acm_request->phone_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->phone_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(curdate,
     curtime3)
   ENDIF
   IF ((acm_request->phone_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->phone_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   phone p
  SET p.phone_id = acm_request->phone_qual[xref->add[d.seq].idx].phone_id, p.beg_effective_dt_tm =
   cnvtdatetime(acm_request->phone_qual[xref->add[d.seq].idx].beg_effective_dt_tm), p
   .call_instruction = acm_request->phone_qual[xref->add[d.seq].idx].call_instruction,
   p.contact = acm_request->phone_qual[xref->add[d.seq].idx].contact, p.contributor_system_cd =
   IF ((acm_request->phone_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0)) acm_request->
    phone_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , p.description = acm_request->phone_qual[xref->add[d.seq].idx].description,
   p.end_effective_dt_tm = cnvtdatetime(acm_request->phone_qual[xref->add[d.seq].idx].
    end_effective_dt_tm), p.extension = acm_request->phone_qual[xref->add[d.seq].idx].extension, p
   .long_text_id = acm_request->phone_qual[xref->add[d.seq].idx].long_text_id,
   p.modem_capability_cd = acm_request->phone_qual[xref->add[d.seq].idx].modem_capability_cd, p
   .operation_hours = acm_request->phone_qual[xref->add[d.seq].idx].operation_hours, p.paging_code =
   acm_request->phone_qual[xref->add[d.seq].idx].paging_code,
   p.parent_entity_id = acm_request->phone_qual[xref->add[d.seq].idx].parent_entity_id, p
   .parent_entity_name = acm_request->phone_qual[xref->add[d.seq].idx].parent_entity_name, p
   .phone_format_cd = acm_request->phone_qual[xref->add[d.seq].idx].phone_format_cd,
   p.phone_num = acm_request->phone_qual[xref->add[d.seq].idx].phone_num, p.phone_type_cd =
   acm_request->phone_qual[xref->add[d.seq].idx].phone_type_cd, p.phone_type_seq = acm_request->
   phone_qual[xref->add[d.seq].idx].phone_type_seq,
   p.contact_method_cd = acm_request->phone_qual[xref->add[d.seq].idx].contact_method_cd, p
   .source_identifier = acm_request->phone_qual[xref->add[d.seq].idx].source_identifier, p
   .phone_num_key = trim(cnvtupper(cnvtalphanum(acm_request->phone_qual[xref->add[d.seq].idx].
      phone_num)),3),
   p.data_status_cd =
   IF ((acm_request->phone_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request->phone_qual[
   ...
   p.active_ind =
   IF ((((acm_request->phone_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((acm_request->
   phone_qual[xref->add[d.seq].idx].active_status_cd=reqdata->active_status_cd))) ) 1
   ELSE 0
   ENDIF
   , p.active_status_cd =
   IF ((acm_request->phone_qual[xref->add[d.seq].idx].active_status_cd > 0.0)) acm_request->
    phone_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->phone_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->phone_qual[xref->add[index].idx].status=0))
