CREATE PROGRAM dc_mp_get_vte
 ...
 SUBROUTINE organizesequence(rec,cond_seq,cond_id)
   ...
   CALL lhprint(build(";organizeSequence Process time: ",datetimediff(cnvtdatetime(curdate,curtime3),
      begin_dt_tm,5)))
 END ;Subroutine
 DECLARE ver_num = f8 WITH protect, constant(iqrversioncontrol("VTE_NHIQM",cnvtdatetime(curdate,
    curtime3)))
 IF (( $2 != null))
  SET jrec = cnvtjsontorec( $2)
  IF (validate(debug_dc_mp_get_vte_ind,0)=1)
 ...
 DECLARE iqr_current_version = f8 WITH constant(iqrversioncontrol("VTE_NHIQM",current_dt_tm)),
 protect
 DECLARE emeas_ver_num = i4 WITH constant(emeasversioncontrol("MU_CQM_EH_VTE",current_dt_tm)),
 protect
 DECLARE combinedviewtype = i2 WITH constant(getcombinedviewtype(iqr_current_version,emeas_ver_num,
   "MP_QM_VTE_2","MP_QM_EVTE_2",_flexid,
   _logicaldomainid)), protect
 DECLARE iqrvenuedefined = i2 WITH constant(checkvenuedefined(_flexid,_logicaldomainid,"MP_QM_VTE_2")
  ), protect
 DECLARE emeasvenuedefined = i2 WITH constant(checkvenuedefined(_flexid,_logicaldomainid,
   "MP_QM_EVTE_2")), protect
 IF (validate(debug_dc_mp_get_vte_ind,0)=1)
 ...
 ENDIF
 IF (checkemeasureenabled(_flexid,_logicaldomainid,"MP_QM_EVTE_2")=1
  AND emeas_ver_num=2015)
  IF (checkdic("LH_MP_ORG_E_VTE","P",0) > 0)
   EXECUTE lh_mp_org_e_vte  WITH replace("REQUEST",qmreq), replace("REPLY",vtereply), replace(
    "FLEXID",_flexid),
   replace("DOMAINID",_logicaldomainid)
   GO TO exit_script
  ENDIF
 ELSEIF (emeas_ver_num >= 2016)
  IF (checkdic("LH_MP_COMBINE_ORG_VTE","P",0) > 0)
   EXECUTE lh_mp_combine_org_vte  WITH replace("REQUEST",qmreq), replace("REPLY",vtereply), replace(
    "FLEXID",_flexid),
   replace("DOMAINID",_logicaldomainid), replace("VTE_IQR_IND",iqrvenuedefined), replace(
    "VTE_EMEAS_IND",emeasvenuedefined)
   GO TO exit_script
  ENDIF
 ELSE
  IF (ver_num >= 5.0
   AND ver_num < 5.2
   AND checkdic("LH_MP_COMBINE_ORG_VTE","P",0) > 0)
   EXECUTE lh_mp_combine_org_vte  WITH replace("REQUEST",qmreq), replace("REPLY",vtereply), replace(
    "FLEXID",_flexid),
   replace("DOMAINID",_logicaldomainid), replace("VTE_IQR_IND",iqrvenuedefined), replace(
    "VTE_EMEAS_IND",emeasvenuedefined)
...
#exit_script
 IF (validate(_memory_reply_string)=1)
  SET _memory_reply_string = cnvtrectojson(vtereply)
 ELSE
  CALL echojson(vtereply, $1)
 ENDIF
 SELECT INTO "nl:"
  DETAIL
   row + 0
