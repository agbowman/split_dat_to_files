CREATE PROGRAM cp_get_nomen_do_context:dba
 PROMPT
 ...
 DECLARE template_id = f8 WITH constant(cnvtint( $1)), protect
 ...
 DECLARE loadassociatedontologyconcepts(null) = null WITH protect
 DECLARE loaddiagnosisnomenclature(null) = null WITH protect
 IF ( NOT (validate(eventlist)))
  RECORD eventlist(
    1 cnt = i4
    1 qual[*]
      2 raw = vc
 ...
 SUBROUTINE loadassociatedontologyconcepts(null)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   DECLARE ncntr = i4 WITH noconstant(0), protect
   DECLARE vcntr = i4 WITH noconstant(0), protect
   DECLARE searchcntr = i4 WITH noconstant(0), protect
   DECLARE nindx = i4 WITH noconstant(0), protect
   CALL log_message("start loadAssociatedOntologyConcepts()",log_level_debug)
   SELECT INTO "nl:"
    FROM do_nomenclature_concept_vw dncv,
     do_concept dco,
     do_concept_alias dca,
     do_context_version dcv,
     do_context dc
    PLAN (dncv
     WHERE expand(vcntr,1,conditionstocheck->nomen_cnt,dncv.nomenclature_id,conditionstocheck->
      nomen_qual[ncntr].nomenclature_id))
     JOIN (dco
     WHERE dco.do_concept_id=dncv.do_concept_id)
     JOIN (dca
     WHERE dca.do_concept_id=dco.do_concept_id)
     JOIN (dcv
     WHERE dcv.do_context_version_id=dca.do_context_version_id)
     JOIN (dc
     WHERE dc.do_context_id=dcv.do_context_id)
    ORDER BY dca.do_concept_alias, dcv.do_context_id, dcv.do_context_version_nbr DESC
    HEAD dca.do_concept_alias
     nindx = locateval(searchcntr,1,conditionstocheck->nomen_cnt,dncv.nomenclature_id,
      conditionstocheck->nomen_qual[ncntr].nomenclature_id)
    HEAD dcv.do_context_id
     conditionstocheck->ont_concept_cnt += 1, stat = alterlist(conditionstocheck->ont_concept_qual,
      conditionstocheck->ont_concept_cnt), conditionstocheck->ont_concept_qual[conditionstocheck->
     ont_concept_cnt].condition_display = conditionstocheck->nomen_qual[nindx].condition_display,
     conditionstocheck->ont_concept_qual[conditionstocheck->ont_concept_cnt].nomenclature_id =
     conditionstocheck->nomen_qual[nindx].nomenclature_id, conditionstocheck->ont_concept_qual[
     conditionstocheck->ont_concept_cnt].do_concept_name = dco.do_concept_name, conditionstocheck->
     ont_concept_qual[conditionstocheck->ont_concept_cnt].do_context_name = dc.do_context_name,
     conditionstocheck->ont_concept_qual[conditionstocheck->ont_concept_cnt].do_context_version_nbr
      = dcv.do_context_version_nbr
    WITH nocounter
   ;end select
   CALL log_message(build("exit loadAssociatedOntologyConcepts, elapsed time in seconds:",((curtime3
      - start_tm)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE loaddiagnosisnomenclature(null)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   ...
   SELECT INTO "nl:"
    FROM diagnosis d,
     nomenclature n
    PLAN (d
     WHERE expand(idx,1,eventlist->cnt,d.diagnosis_id,eventlist->qual[idx].fvalue)
      AND d.active_status_cd=active_diagnosis_cd
      AND d.active_ind=1)
     JOIN (n
     WHERE n.nomenclature_id=d.nomenclature_id)
    ORDER BY n.nomenclature_id
    HEAD n.nomenclature_id
     conditionstocheck->nomen_cnt += 1, stat = alterlist(conditionstocheck->nomen_qual,
      conditionstocheck->nomen_cnt), conditionstocheck->nomen_qual[conditionstocheck->nomen_cnt].
     condition_display = d.diagnosis_display,
     conditionstocheck->nomen_qual[conditionstocheck->nomen_cnt].nomenclature_id = n.nomenclature_id
     CASE (uar_get_code_meaning(n.source_vocabulary_cd))
      OF "ICD9":
       conditionstocheck->nomen_qual[conditionstocheck->nomen_cnt].vocab_mean = "ICD9"
      OF "ICD10-CM":
       conditionstocheck->nomen_qual[conditionstocheck->nomen_cnt].vocab_mean = "ICD10CM"
      OF "ICD10":
       conditionstocheck->nomen_qual[conditionstocheck->nomen_cnt].vocab_mean = "ICD-10"
      OF "SNMCT":
       conditionstocheck->nomen_qual[conditionstocheck->nomen_cnt].vocab_mean = "SNMCT"
      ELSE
       conditionstocheck->nomen_qual[conditionstocheck->nomen_cnt].vocab_mean = uar_get_code_meaning(
        n.source_vocabulary_cd)
     ENDCASE
     conditionstocheck->nomen_qual[conditionstocheck->nomen_cnt].source_identifier = n
     .source_identifier, conditionstocheck->nomen_qual[conditionstocheck->nomen_cnt].source_string =
     n.source_string, conditionstocheck->nomen_qual[conditionstocheck->nomen_cnt].concept_cki = n
     .concept_cki
    WITH nocounter
   ;end select
   CALL log_message(build("exit loadAssociatedOntologyConcepts, elapsed time in seconds:",((curtime3
      - start_tm)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (eksdatamiscfetch(iqual=f8(value),misctype=vc(value),misclist=vc(ref)) =null WITH protect
  )
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   DECLARE sectionname = c5 WITH noconstant("Logic"), protect
   DECLARE qsize = i4 WITH protect
   DECLARE msize = i4 WITH protect
   DECLARE typefound = i2 WITH protect
   DECLARE idx = i4 WITH protect
   DECLARE stat = i4 WITH protect
   CALL log_message("start eksdataMiscFetch()",log_level_debug)
   IF (validate(eksdata))
    SET qsize = size(eksdata->tqual[3].qual,5)
    IF (qsize >= iqual)
     SET msize = size(eksdata->tqual[3].qual[iqual].data,5)
     SET stat = alterlist(misclist->qual,(msize - 1))
     IF (msize > 1)
      SET typefound = 0
      FOR (idx = 1 TO msize)
        IF ((eksdata->tqual[3].qual[iqual].data[idx].misc=misctype))
         SET typefound = 1
        ELSEIF (substring(1,1,trim(eksdata->tqual[3].qual[iqual].data[idx].misc,3))="<")
         SET typefound = 0
        ELSEIF (typefound)
         SET misclist->cnt += 1
         SET misclist->qual[misclist->cnt].raw = eksdata->tqual[3].qual[iqual].data[idx].misc
         SET misclist->qual[misclist->cnt].numericind = 0
         IF (isnumeric(eksdata->tqual[3].qual[iqual].data[idx].misc))
          SET misclist->qual[misclist->cnt].numericind = 1
          SET misclist->qual[misclist->cnt].fvalue = cnvtreal(eksdata->tqual[3].qual[iqual].data[idx]
           .misc)
         ENDIF
        ENDIF
      ENDFOR
     ELSE
      CALL log_message(concat("No ",misctype," values found in ",trim(sectionname)," template ",
        build(iqual)),log_level_debug)
     ENDIF
    ELSE
     CALL log_message(concat(trim(sectionname)," Section only has ",build(qsize),
       " template(s)....cannot access template ",build(iqual)),log_level_debug)
    ENDIF
   ELSE
    CALL log_message("eksdata structure does not exist",log_level_debug)
   ENDIF
   SET stat = alterlist(misclist->qual,misclist->cnt)
   CALL log_message(build("exit eksdataMiscFetch, elapsed time in seconds:",((curtime3 - start_tm)/
 ...
 SUBROUTINE main(null)
   ...
   CALL eksdatamiscfetch(template_id,"<DIAGNOSIS_ID>",eventlist)
   IF ((eventlist->cnt > 0))
    CALL loaddiagnosisnomenclature(null)
    CALL loadassociatedontologyconcepts(null)
   ENDIF
   CALL echorecord(conditionstocheck)
   IF ((conditionstocheck->ont_concept_cnt > 0))
    CALL log_message(build("Found ",cnvtstring(conditionstocheck->ont_concept_cnt)," do concepts."),
     log_level_debug)
    SET log_misc1 = cnvtrectojson(conditionstocheck)
    SET retval = 100
   ELSEIF ((conditionstocheck->nomen_cnt > 0))
    CALL log_message(build("Found ",cnvtstring(conditionstocheck->nomen_cnt)," nomenclature."),
     log_level_debug)
    SET log_misc1 = cnvtrectojson(conditionstocheck)
    SET retval = 100
   ELSE
    CALL log_message(build("Found no matching concepts."),log_level_debug)
    SET log_misc1 = ""
    SET retval = 0
    GO TO exit_script
   ENDIF
