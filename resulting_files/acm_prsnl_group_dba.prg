CREATE PROGRAM acm_prsnl_group:dba
 ...
 RECORD acm_request(
   ...
   1 prsnl_group_reltn_qual[*]
     ...
     2 data_status_cd = f8
     2 updt_cnt = i4
   1 prsnl_group_org_reltn_qual[*]
     2 prsnl_group_org_reltn_id = f8
     2 prsnl_group_id = f8
     2 prsnl_group_idx = i4
 ...
 FREE RECORD reply
 RECORD reply(
   1 prsnl_group_qual_cnt = i4
   1 prsnl_group_qual[*]
     2 prsnl_group_id = f8
     2 status = i2
   1 prsnl_group_reltn_qual_cnt = i4
   1 prsnl_group_reltn_qual[*]
     2 prsnl_group_reltn_id = f8
     2 status = i2
   1 prsnl_group_org_reltn_qual_cnt = i4
   1 prsnl_group_org_reltn_qual[*]
     2 prsnl_group_org_reltn_id = f8
     2 status = i2
 ...
 DECLARE idl1 = i4 WITH protect, noconstant(0)
 SET call_echo_ind = request->call_echo_ind
 IF (validate(request,"-1") != "-1")
  SET acm_request = request
  SET reply->prsnl_group_qual_cnt = size(request->prsnl_group_qual,5)
  SET reply->prsnl_group_reltn_qual_cnt = size(request->prsnl_group_reltn_qual,5)
  SET reply->prsnl_group_org_reltn_qual_cnt = size(request->prsnl_group_org_reltn_qual,5)
 ENDIF
 IF (size(acm_request->preprocess_qual,5) > 0)
  FOR (idl1 = 1 TO size(acm_request->preprocess_qual,5))
   EXECUTE value(cnvtupper(acm_request->preprocess_qual[idl1].prog_name)) idl1
   IF ((reply->preprocess_qual[idl1].status=0))
  ...
  ENDFOR
 ENDIF
 IF ((reply->prsnl_group_qual_cnt > 0))
  EXECUTE acm_write_prsnl_group
  IF ((reply->status_data.status != "S"))
   SET failed = execute_error
   SET table_name = build(table_name,": failed in execute acm_write_prsnl_group")
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((reply->prsnl_group_reltn_qual_cnt > 0))
  SET qual_cnt = 0
  SET qual_cnt = size(acm_request->prsnl_group_reltn_qual,5)
  FOR (idl1 = 1 TO qual_cnt)
    IF ((acm_request->prsnl_group_reltn_qual[idl1].prsnl_group_idx > 0))
     SET acm_request->prsnl_group_reltn_qual[idl1].prsnl_group_id = acm_request->prsnl_group_qual[
     acm_request->prsnl_group_reltn_qual[idl1].person_idx].prsnl_group_id
    ENDIF
  ENDFOR
  EXECUTE acm_write_prsnl_group_reltn
  IF ((reply->status_data.status != "S"))
   SET failed = execute_error
   SET table_name = build(table_name,": failed in execute acm_write_prsnl_group_reltn")
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((reply->prsnl_group_org_reltn_qual_cnt > 0))
  SET qual_cnt = 0
  SET qual_cnt = size(acm_request->prsnl_group_org_reltn_qual,5)
  FOR (idl1 = 1 TO qual_cnt)
    IF ((acm_request->prsnl_group_org_reltn_qual[idl1].prsnl_group_idx > 0))
     SET acm_request->prsnl_group_org_reltn_qual[idl1].prsnl_group_id = acm_request->
     prsnl_group_qual[acm_request->prsnl_group_org_reltn_qual[idl1].prsnl_group_idx].prsnl_group_id
    ENDIF
  ENDFOR
  EXECUTE acm_write_prsnl_group_org_reltn
  IF ((reply->status_data.status != "S"))
   SET failed = execute_error
   SET table_name = build(table_name,": failed in execute acm_write_prsnl_group_org_reltn")
   GO TO exit_script
  ENDIF
 ENDIF
 IF (size(acm_request->postprocess_qual,5) > 0)
  FOR (idl1 = 1 TO size(acm_request->postprocess_qual,5))
   EXECUTE value(cnvtupper(acm_request->postprocess_qual[idl1].prog_name)) idl1
   IF ((reply->postprocess_qual[idl1].status=0))
    SET failed = execute_error
