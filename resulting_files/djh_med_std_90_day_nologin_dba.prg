CREATE PROGRAM djh_med_std_90_day_nologin:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE"
  WITH outdev
 IF (findstring("@", $1) > 0)
  SET output_dest = build(format(cnvtdatetime(curdate,curtime3),"YYYYMMDDHHMMSS;;D"))
  SET email_ind = 1
 ELSE
  SET output_dest =  $1
  SET email_ind = 0
 ENDIF
 CALL echo(output_dest)
 DECLARE date_qual = dq8
 SET date_qual = cnvtlookbehind("90,D",cnvtdatetime(curdate,curtime3))
 CALL echo(format(date_qual,"YYYY/MM/DD;;D"))
 DECLARE output_string = vc
 SELECT DISTINCT INTO value(output_dest)
  pr.name_full_formatted, pr.username, position = uar_get_code_display(pr.position_cd),
  updt_dt_tm = format(pr.updt_dt_tm,"MM/DD/YYYY ;;D"), end_eff_dt_tm =
  IF (pr.end_effective_dt_tm <= cnvtdatetime((curdate - 45),000)) format(pr.end_effective_dt_tm,
    "MM/DD/YYYY;;D")
  ELSE ""
  ENDIF
  , act_stat_disp =
  IF (pr.active_status_cd != 188) uar_get_code_display(pr.active_status_cd)
  ELSE ""
  ENDIF
  FROM prsnl pr,
   omf_app_ctx_day_st oa
  PLAN (pr
   WHERE pr.position_cd=777650
    AND pr.username != "MEDICALSTUDENT"
    AND ((pr.active_ind=1
    AND pr.active_status_cd=188.00) OR (pr.active_status_cd != 188.00
    AND pr.end_effective_dt_tm >= cnvtdatetime((curdate - 45),000)
    AND  NOT (pr.person_id IN (
   (SELECT
    oai.person_id
    FROM omf_app_ctx_day_st oai
    WHERE oai.person_id=pr.person_id
     AND oai.start_day > cnvtdatetime((curdate - 90),000)))))) )
   JOIN (oa
   WHERE oa.person_id=outerjoin(pr.person_id))
  ORDER BY pr.name_full_formatted, oa.start_day DESC, 0
  HEAD REPORT
   col 1, ",", "User",
   ",", "Login", ",",
   "Position", ",", "Last Login",
   ",", "Update Date", ",",
   "End Date", ",", row + 1
  HEAD pr.name_full_formatted
   position = trim(uar_get_code_display(pr.position_cd)), last_login = format(oa.start_day,
    "YYYY/MM/DD;;D"), days_ago =
   IF (oa.start_day < curdate) datetimediff(cnvtdatetime(curdate,0),oa.start_day,1)
   ENDIF
   ,
   output_string = build(',"',pr.name_full_formatted,'","',pr.username,'","',
    position,'","',last_login,'","',updt_dt_tm,
    '","',act_stat_disp,'","',end_eff_dt_tm,'","',
    days_ago,'",'), col 1, output_string
   IF ( NOT (curendreport))
    row + 1
   ENDIF
  WITH format = variable, formfeed = none
 ;end select
 IF (email_ind=1)
  SET filename_in = trim(concat(output_dest,".dat"))
  SET filename_out = concat(format(curdate,"YYYY-MM-DD;;D"),"-Med-Stds.csv")
  DECLARE subject_line = vc
  SET subject_line = concat("Med Std List - Test")
  EXECUTE bhs_ma_email_file
  CALL emailfile(filename_in,filename_out, $1,subject_line,1)
 ENDIF
#end_prog
END GO
