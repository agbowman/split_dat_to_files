CREATE PROGRAM dm2_disable_mvj_eppr_pgr:dba
 ...
 SET readme_data->message = "Readme Failed: Starting script dm2_disable_mvj_eppr_pgr..."
 IF (check_logfile("dm2_dis_mvj_eppr_pgr",".log","dm2_disable_mvj_eppr_pgr")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_disable_mvj_eppr_pgr"
 CALL disp_msg("",dm_err->logfile,0)
 IF (dmvr_updt_mv_inst("IU","MVJ_EPPR_PGR",1,"DISABLED")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Updating DB-wide MVIEW row."
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ELSEIF (curqual=0)
  SET dm_err->eproc = "Inserting DB-wide MVIEW row."
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2_MVIEWS", di.info_name = "STATUS", di.info_number = 0
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
...
#exit_program
 IF ( NOT (dm_err->err_ind))
  ...
  SET readme_data->message = "Success: Disabling of MVJ_EPPR_PGR was successful."
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error disabling MVJ_EPPR_PGR:",dm_err->emsg)
 ENDIF
 CALL echorecord(readme_data)
 EXECUTE dm_readme_status
 SET dm_err->eproc = "Ending dm2_disable_mvj_eppr_pgr."
 CALL final_disp_msg("dm2_dis_mvj_eppr_pgr")
END GO
