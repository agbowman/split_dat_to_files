CREATE PROGRAM dc_mp_get_ami
 IF ( NOT (validate(amieval)))
  RECORD amieval(
    1 listcnt = i4
    1 colmncnt = i4
    1 list[*]
 ...
 SUBROUTINE iqrversioncontrolstring(category_topic_mean,beg_time)
   ...
   RETURN(version_number)
 END ;Subroutine
 DECLARE ver_num = f8 WITH protect, constant(iqrversioncontrol("AMI",cnvtdatetime(curdate,curtime3)))
 IF (( $2 != null))
  SET jrec = cnvtjsontorec( $2)
  IF (validate(debug_dc_mp_get_ami_ind,0)=1)
 ...
 DECLARE checkind = i2 WITH noconstant(0), protect
 DECLARE filterexisted = i2 WITH constant(checkfilterexisted("MP_QM_EAMI_2","QM_COMP_CONTROL")),
 protect
 DECLARE iqrvenuedefined = i2 WITH constant(checkvenuedefined(_flexid,_logicaldomainid,"MP_QM_AMI_2")
  ), protect
 DECLARE emeasvenuedefined = i2 WITH constant(checkvenuedefined(_flexid,_logicaldomainid,
   "MP_QM_EAMI_2")), protect
 DECLARE eamienabled = i2 WITH constant(checkemeasureenabled(_flexid,_logicaldomainid,"MP_QM_EAMI_2")
  ), protect
 IF (ver_num >= 5.0
  AND filterexisted=0)
  SET checkind = 3
 ELSEIF (ver_num >= 5.0
  ...
  AND eamienabled=0)
  SET checkind = 2
 ELSEIF (((filterexisted=0
  AND iqrvenuedefined=0) OR (filterexisted=1
  AND eamienabled=1)) )
  SET checkind = 1
 ENDIF
 DECLARE emeasversion = i4 WITH constant(emeasversioncontrol("MU_CQM_EH_AMI",cnvtdatetime(curdate,
    curtime3))), protect
 IF (emeasversion >= 2017)
  IF (checkdic("LH_MP_ORGANIZER_EAMI","P",0) > 0)
   EXECUTE lh_mp_organizer_eami  WITH replace("FLEXID",_flexid), replace("DOMAINID",_logicaldomainid)
  ENDIF
 ELSEIF (checkind != 0)
  IF (checkdic("LH_MP_COMBINE_ORG_AMI","P",0) > 0)
   EXECUTE lh_mp_combine_org_ami  WITH replace("REQUEST",qmreq), replace("REPLY",amireply), replace(
    "FLEXID",_flexid),
   replace("DOMAINID",_logicaldomainid), replace("VERSION",ver_num), replace("AMI_CHECK",checkind),
   replace("AMI_IQR_IND",iqrvenuedefined), replace("AMI_EMEAS_IND",emeasvenuedefined)
   GO TO exit_script
  ENDIF
 ENDIF
#exit_script
 IF (validate(_memory_reply_string)=1)
  SET _memory_reply_string = cnvtrectojson(amireply)
 ELSE
  CALL echojson(amireply, $1)
  SELECT INTO "nl:"
   DETAIL
    row + 0
   WITH nocounter
