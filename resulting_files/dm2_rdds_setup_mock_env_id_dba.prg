CREATE PROGRAM dm2_rdds_setup_mock_env_id:dba
 ...
 SUBROUTINE drmmi_backfill_mock_id(dbmi_env_id)
   ...
   RETURN(dbmi_mock_id)
 END ;Subroutine
 DECLARE drsm_mck_src_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drsm_cur_env = f8
 DECLARE drsm_rep_environment_id = f8 WITH protect, noconstant(0.0)
 DECLARE drsm_rep_environment_name = vc WITH protect, noconstant("")
 DECLARE drsm_continue = i2 WITH protect, noconstant(1)
 DECLARE drsm_src_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drsm_src_env_name = vc WITH protect, noconstant("")
 DECLARE drsm_env_str_name = vc WITH protect, noconstant("")
 DECLARE drsm_env_str = vc WITH protect, noconstant("")
 DECLARE drsm_rep_override = c1 WITH protect, noconstant("N")
 DECLARE drsm_rep_found = c1 WITH protect, noconstant("Y")
 DECLARE drsm_ret = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_rdds_mock_setup",".log","DM2_RDDS_SETUP_MOCK_ENV_ID LOG")=0)
  GO TO exit_drsm
 ENDIF
 IF (reflect(parameter(1,0)) != "I*"
  AND reflect(parameter(1,0)) != "F*")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("The parameter ",trim(cnvtstring( $1,20),3),
   " is not of the correct data type.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_drsm
 ENDIF
 SET drsm_src_env_id =  $1
 FREE RECORD drsm_env
 RECORD drsm_env(
   1 env_id = f8
   1 env_name = vc
 )
 ...
 SELECT INTO "NL:"
  FROM dm_environment de
  WHERE de.environment_id=drsm_src_env_id
  DETAIL
   drsm_src_env_name = de.environment_name
  WITH nocounter
 ;end select
 SELECT INTO "NL:"
  FROM dm_info a,
  ...
  DETAIL
   drsm_env->env_id = b.environment_id, drsm_env->env_name = b.environment_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "NL:"
  ...
  DETAIL
   drsm_rep_environment_id = d.environment_id, drsm_rep_environment_name = d.environment_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_drsm
 ENDIF
 WHILE (drsm_continue=1)
   SET drsm_rep_found = "Y"
   SET drsm_rep_override = "N"
   CALL clear(6,01)
   CALL text(07,05,
    "Is the current environment a copy of the final_target environment, for use in a mock RDDS move (Y/N)?"
    )
   ...
   IF (curaccept="Y")
    SET dm_err->eproc =
    "USER CONFIRMATION: CURRENT ENVIRONMENT IS A COPY OF FINAL_TARGET ENVIRONMENT"
    CALL disp_msg(" ",dm_err->logfile,0)
    IF (drsm_rep_environment_id > 0.0)
     SET drsm_env_str = concat(" and replicated environment of ",drsm_rep_environment_name,":",
      cnvtstring(drsm_rep_environment_id,20))
     CALL clear(10,01)
     CALL text(10,05,concat("It was detected that this environment was replicated from ",
       drsm_rep_environment_name,": ",cnvtstring(drsm_rep_environment_id,20)))
     CALL clear(11,01)
     CALL text(11,05,
      "This will be used as the final target environment for RDDS Mock Move. Is this correct (Y/N)?")
     CALL text(12,05,"ENTER CHOICE: ")
     SET accept = nopatcheck
     CALL accept(12,20,"P;CU","Y"
      WHERE curaccept IN ("Y", "N"))
     SET accept = patcheck
     CALL clear(20,01)
     SET drsm_rep_found = curaccept
     IF (drsm_rep_found="N")
      CALL clear(14,01)
      CALL text(14,05,
       "Warning: By choosing a different environment ID, your mock RDDS move might not work as intended."
       )
      CALL text(15,05,"Do you wish to continue and choose a different environment_id (Y/N)?")
      CALL text(16,05,"ENTER CHOICE: ")
      SET accept = nopatcheck
      CALL accept(16,20,"P;CU"
       WHERE curaccept IN ("Y", "N"))
      SET accept = patcheck
      SET drsm_rep_override = curaccept
     ELSE
      SET drsm_mck_src_env_id = drsm_rep_environment_id
     ENDIF
    ENDIF
    IF (((drsm_rep_environment_id <= 0.0) OR (drsm_rep_override="Y")) )
     SET drsm_mck_src_env_id = 0.0
     WHILE (drsm_mck_src_env_id=0.0)
       CALL clear(18,01)
       CALL text(18,05,"Enter the environment ID of the replicated environment: ")
       CALL clear(20,01)
       ...
       SELECT INTO "NL:"
        ...
        (SELECT
         ...
         WHERE (der.child_env_id=drsm_env->env_id)
          AND der.relationship_type="REFERENCE MERGE")))
         AND (de.environment_id != drsm_env->env_id)
       ...
       ENDIF
       SET validate = 1
       CALL accept(18,61,"P(15);CU","0.0")
       SET drsm_mck_src_env_id = cnvtreal(curaccept)
     ENDWHILE
    ELSEIF (drsm_rep_override="N")
     SET drsm_mck_src_env_id = drsm_rep_environment_id
    ENDIF
    SELECT INTO "NL:"
     FROM dm_environment de
     WHERE de.environment_id=drsm_mck_src_env_id
     DETAIL
      drsm_env_str_name = de.environment_name
     WITH nocounter
    ;end select
    SET drsm_env_str = concat(" and replicated environment of ",drsm_env_str_name,":",cnvtstring(
      drsm_mck_src_env_id,20))
    SET help = off
    SET validate = off
    CALL clear(18,01)
    CALL text(18,05,concat("User has selected MOCK DOMAIN setup for RDDS Merge process",drsm_env_str,
      "."))
    CALL text(19,05,concat(
      "Existing translations and sequence match rows from the replicated environment will be",
      " utilized by RDDS."))
    CALL text(20,05,concat(
      "If you have not done so already, please run DM2_RDDS_COPY_UTILITY in build source domain ",
      drsm_src_env_name,":",cnvtstring(drsm_src_env_id,20)))
    CALL text(22,05,"Please review and confirm the above settings")
    CALL text(23,05,"Continue (Y/N)?")
    SET accept = nopatcheck
    CALL accept(23,20,"P;CU"
     WHERE curaccept IN ("Y", "N"))
    SET accept = patcheck
    IF (curaccept="Y")
     SET drsm_continue = 0
     SET drsm_ret = drmmi_set_mock_id(drsm_env->env_id,drsm_mck_src_env_id,1)
     IF (drsm_ret < 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_drsm
     ENDIF
   ...
   ELSEIF (curaccept="N")
    SET drsm_continue = 0
    SET drsm_ret = drmmi_set_mock_id(drsm_env->env_id,0.0,0)
    IF (drsm_ret < 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_drsm
    ENDIF
