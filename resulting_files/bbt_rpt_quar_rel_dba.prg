CREATE PROGRAM bbt_rpt_quar_rel:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->quarantine_release = uar_i18ngetmessage(i18nhandle,"quarantine_release",
  "QUARANTINE RELEASE REASONS")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_quar_rels", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  cv.display, cv.description, cv.active_ind
  FROM code_value cv
  WHERE cv.code_set=1631
  HEAD REPORT
   select_ok_ind = 0
  HEAD PAGE
   ...
   col 46, captions->quarantine_release, row + 2,
