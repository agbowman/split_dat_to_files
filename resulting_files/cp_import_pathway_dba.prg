CREATE PROGRAM cp_import_pathway:dba
 PROMPT
  "Backend File Directory:" = "CCLUSERDIR:"
  WITH outdev
 ...
 SUBROUTINE (setauditinitreplystatusbyprogramreplystatus(reply=vc(ref)) =null WITH protect)
  ...
  ENDIF
 END ;Subroutine
 DECLARE addpathway(null) = null
 DECLARE setupcomponentdetail(idx=i4,nidx=i4,cidx=i4,cdidx=i4) = null
 DECLARE addstructureddoc(null) = null
 DECLARE current_date_time2 = dq8 WITH constant(curtime3), private
 IF (validate(request->blob_in))
  IF ((request->blob_in > " "))
   CALL logblobindata("CP_IMPORT_PATHWAY",request->blob_in, $OUTDEV)
   IF (validate(debug_ind,0)=1)
    CALL echo(request->blob_in)
   ENDIF
   SET jrec = cnvtjsontorec(request->blob_in)
   IF (jrec != 1)
    SET pw_rec->status_data.status = "F"
    SET pw_rec->status_data.subeventstatus.operationname = "Failed to parse blob in data"
    SET pw_rec->status_data.subeventstatus.operationstatus = "F"
    GO TO exit_script
   ENDIF
   IF (validate(debug_ind,0)=1)
 ...
 CALL addpathway(null)
 CALL addstructureddoc(null)
 SUBROUTINE addpathway(null)
   CALL log_message("Begin AddPathway()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE dconceptgroupcd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003133,"IS-A"))
   DECLARE dintentioncd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003278,"TREATMENTS"))
   IF (size(pw_rec->qual,5) > 0)
    FOR (idx1 = 1 TO size(pw_rec->qual,5))
      SET pw_rec->qual[idx1].pathway_type_cd = uar_get_code_by("MEANING",4003197,pw_rec->qual[idx1].
       pathway_type_mean)
      SET pw_rec->qual[idx1].pathway_status_cd = uar_get_code_by("MEANING",4003198,pw_rec->qual[idx1]
       .pathway_status_mean)
      SET pw_rec->qual[idx1].concept_cd = uar_get_code_by("DISPLAYKEY",4003132,pw_rec->qual[idx1].
       concept_display_key)
      CALL echo(build("pathway name: ",pw_rec->qual[idx1].pathway_name))
      EXECUTE cp_create_concept_pathway "NOFORMS", pw_rec->qual[idx1].pathway_name, pw_rec->qual[idx1
      ].pathway_type_mean
      IF ((pathwayreply->status_data.status != "S"))
       CALL echo("Pathway Create Failure")
       ROLLBACK
       GO TO exit_script
      ENDIF
      SET pw_rec->qual[idx1].cp_pathway_id = pathwayreply->pathway_id
      SET pw_rec->qual[idx1].concept_cd = pathwayreply->concept_cd
      SET pw_rec->qual[idx1].concept_group_cd = dconceptgroupcd
      SET pw_rec->qual[idx1].intention_cd = dintentioncd
      SET stat = addnodes(idx1)
    ENDFOR
   ENDIF
   CALL log_message(build("Exit AddPathway(), Elapsed time in seconds:",((curtime3 - begin_curtime3)
     / 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (addnodes(indx=i4) =i4)
   CALL log_message("In AddNodes()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE n = i4 WITH private, noconstant(0)
   DECLARE x = i4 WITH private, noconstant(0)
   DECLARE reqnodecnt = i4 WITH private, noconstant(0)
   DECLARE dnodeid = f8 WITH private, noconstant(0.0)
   FREE RECORD request
   RECORD request(
     1 cp_pathway_id = f8
     1 node_list[*]
   ...
   CALL echo(indx)
   SET request->cp_pathway_id = pw_rec->qual[indx].cp_pathway_id
   IF ((pw_rec->qual[indx].node_cnt > 0))
    SET stat = alterlist(request->node_list,pw_rec->qual[indx].node_cnt)
    FOR (n = 1 TO pw_rec->qual[indx].node_cnt)
      SET reqnodecnt += 1
      SET pw_rec->qual[indx].node_list[n].intention_cd = uar_get_code_by("MEANING",4003278,pw_rec->
       qual[indx].node_list[n].intention_mean)
      SET pw_rec->qual[indx].node_list[n].concept_cd = pw_rec->qual[indx].concept_cd
      SET request->node_list[reqnodecnt].concept_cd = pw_rec->qual[indx].node_list[n].concept_cd
      SET request->node_list[reqnodecnt].node_name = pw_rec->qual[indx].node_list[n].node_name
      SET request->node_list[reqnodecnt].intention_cd = pw_rec->qual[indx].node_list[n].intention_cd
      SET request->node_list[reqnodecnt].treatment_line_cd = pw_rec->qual[indx].node_list[n].
      treatment_line_cd
      SET request->node_list[reqnodecnt].category_mean = pw_rec->qual[indx].node_list[n].
      category_mean
      SET request->node_list[reqnodecnt].sequence = pw_rec->qual[indx].node_list[n].sequence
    ENDFOR
    IF (reqnodecnt > 0)
     SET reqnodecnt = 0
     CALL echorecord(request)
     EXECUTE cp_create_node
     CALL echo("CP_CREATE_NODE [REPLY]...")
     ...
     IF ((reply->status_data.status != "S"))
      ...
      GO TO exit_script
     ENDIF
     IF (size(reply->node_list,5) > 0)
      FOR (x = 1 TO pw_rec->qual[indx].node_cnt)
       SET reqnodecnt += 1
       SET pw_rec->qual[indx].node_list[x].cp_node_id = reply->node_list[reqnodecnt].cp_node_id
      ENDFOR
     ENDIF
    ENDIF
    FOR (x = 1 TO pw_rec->qual[indx].node_cnt)
      IF ((pw_rec->qual[indx].node_list[x].comp_cnt > 0))
       SET stat = addcomponents(indx,x)
      ENDIF
    ENDFOR
   ELSE
    CALL echo("Node list is empty.")
   ENDIF
   CALL log_message(build("Exit AddNodes(), Elapsed time in seconds:",((curtime3 - begin_curtime3)/
     100.0)),log_level_debug)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (addcomponents(idx=i4,nidx=i4) =i4)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE m = i4 WITH private, noconstant(0)
   DECLARE x = i4 WITH private, noconstant(0)
   ...
   RECORD request(
     1 component_list[*]
       ...
       2 component_display = vc
   )
   IF ((pw_rec->qual[idx].node_list[nidx].comp_cnt > 0))
    SET stat = alterlist(request->component_list,pw_rec->qual[idx].node_list[nidx].comp_cnt)
    CALL echo(pw_rec->qual[idx].node_list[nidx].comp_cnt)
    FOR (m = 1 TO pw_rec->qual[idx].node_list[nidx].comp_cnt)
      SET reqcompcnt += 1
      SET pw_rec->qual[idx].node_list[nidx].comp_list[m].cp_node_id = pw_rec->qual[idx].node_list[
      nidx].cp_node_id
      CALL echo(pw_rec->qual[idx].node_list[nidx].cp_node_id)
      CALL echo(pw_rec->qual[idx].node_list[nidx].comp_list[m].cp_node_id)
      SET pw_rec->qual[idx].node_list[nidx].comp_list[m].comp_type_cd = uar_get_code_by("MEANING",
       4003130,pw_rec->qual[idx].node_list[nidx].comp_list[m].comp_type_mean)
      IF ((pw_rec->qual[idx].node_list[nidx].comp_list[m].concept_group_cd > 0))
       SET pw_rec->qual[idx].node_list[nidx].comp_list[m].concept_group_cd = uar_get_code_by(
        "MEANING",4003133,pw_rec->qual[idx].node_list[nidx].comp_list[m].concept_group_mean)
      ENDIF
      CALL echo(pw_rec->qual[idx].node_list[nidx].comp_list[m].cp_node_id)
      SET request->component_list[reqcompcnt].cp_node_id = pw_rec->qual[idx].node_list[nidx].
      comp_list[m].cp_node_id
      SET request->component_list[reqcompcnt].comp_type_cd = pw_rec->qual[idx].node_list[nidx].
      comp_list[m].comp_type_cd
      SET request->component_list[reqcompcnt].concept_group_cd = pw_rec->qual[idx].node_list[nidx].
      comp_list[m].concept_group_cd
      SET request->component_list[reqcompcnt].component_seq_txt = pw_rec->qual[idx].node_list[nidx].
      comp_list[m].component_seq_txt
    ENDFOR
    SET stat = alterlist(request->component_list,reqcompcnt)
    IF (reqcompcnt > 0)
     SET reqcompcnt = 0
     CALL echorecord(request)
     EXECUTE cp_add_component
     IF ((reply->status_data.status != "S"))
      CALL echo("Component Create Failure")
      ROLLBACK
      GO TO exit_script
     ENDIF
     CALL echo("CP_ADD_COMPONENT [REPLY]...")
     CALL echorecord(reply)
     IF (size(reply->component_list,5) > 0)
      FOR (x = 1 TO pw_rec->qual[idx].node_list[nidx].comp_cnt)
       SET reqcompcnt += 1
       SET pw_rec->qual[idx].node_list[nidx].comp_list[x].cp_component_id = reply->component_list[
       reqcompcnt].cp_component_id
      ENDFOR
     ENDIF
    ENDIF
    FOR (x = 1 TO pw_rec->qual[idx].node_list[nidx].comp_cnt)
 ...
 SUBROUTINE (addcomponentdetails(idx=i4,nidx=i4,cidx=i4) =i4)
   ...
   DECLARE sentityname = vc WITH private, noconstant("")
   DECLARE scompident = vc WITH protect, noconstant("")
   ...
   IF ((pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_dtl_cnt > 0))
    FOR (j = 1 TO pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_dtl_cnt)
      SET scompident = pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_dtl_list[j].
      component_ident
      SET sentityname = pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_dtl_list[j].
      component_entity_name
      SET pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_dtl_list[j].comp_detail_reltn_cd =
      uar_get_code_by("MEANING",4003134,pw_rec->qual[idx].node_list[nidx].comp_list[cidx].
       comp_dtl_list[j].comp_detail_reltn_mean)
      IF ((pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_dtl_list[j].comp_detail_reltn_cd=
      - (1)))
       SET pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_dtl_list[j].comp_detail_reltn_cd =
       uar_get_code_by("MEANING",4003130,pw_rec->qual[idx].node_list[nidx].comp_list[cidx].
        comp_dtl_list[j].comp_detail_reltn_mean)
       IF ((pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_dtl_list[j].comp_detail_reltn_cd=
       - (1)))
        SET pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_dtl_list[j].comp_detail_reltn_cd
       ...
       ENDIF
      ENDIF
      IF (sentityname IN ("LONG_TEXT_REFERENCE", "")
       AND (pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_type_mean IN ("PATHWAY_DOC",
      "GUIDEDTRMNT")))
       CALL setupcomponentdetailandadd(idx,nidx,cidx,j)
      ENDIF
      IF (sentityname="PAT_ED_RELTN")
       SELECT INTO "nl:"
        FROM pat_ed_reltn per
        WHERE per.key_doc_ident=scompident
        WITH nocounter
       ;end select
       IF (curqual > 0)
        CALL setupcomponentdetailandadd(idx,nidx,cidx,j)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE setupcomponentdetailandadd(idx,nidx,cidx,cdidx)
   CALL log_message("Begin SetupComponentDetail()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE doccontentcd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003134,"DOCCONTENT"))
   FREE RECORD request
   RECORD request(
     1 comp_detail_list[*]
       2 cp_component_id = f8
   ...
   SET stat = alterlist(request->comp_detail_list,1)
   SET curalias comp_request request->comp_detail_list[1]
   SET curalias comp_rec pw_rec->qual[idx].node_list[nidx].comp_list[cidx].comp_dtl_list[cdidx]
   SET comp_rec->cp_component_id = pw_rec->qual[idx].node_list[nidx].comp_list[cidx].cp_component_id
   SET comp_request->cp_component_id = comp_rec->cp_component_id
   SET comp_request->comp_detail_reltn_cd = comp_rec->comp_detail_reltn_cd
   SET comp_request->component_entity_id = comp_rec->component_entity_id
   SET comp_request->component_entity_name = comp_rec->component_entity_name
   SET comp_request->component_ident = comp_rec->component_ident
   SET comp_request->component_text = comp_rec->component_text
   IF ((comp_rec->comp_detail_reltn_cd=doccontentcd))
    SET comp_request->version_nbr = 1
    SET comp_request->default_ind = 1
   ENDIF
   IF ((comp_rec->component_entity_name="LONG_TEXT_REFERENCE"))
    SET comp_request->component_entity_id = insertnewlongtext(comp_rec->cp_component_id,comp_rec->
     comp_detail_entity_disp)
    SET comp_rec->component_entity_id = comp_request->component_entity_id
    SET comp_request->version_nbr = 1
    SET comp_request->default_ind = 1
   ENDIF
   ...
   CALL echorecord(reply)
   IF (size(reply->comp_detail_list,5)=1)
    SET comp_rec->cp_component_detail_id = reply->comp_detail_list[1].cp_component_detail_id
   ENDIF
   CALL log_message(build("Exit SetupComponentDetail(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE insertnewlongtext(component_id,text)
   ...
   INSERT  FROM long_text_reference lt
    SET lt.active_ind = lsetactind, lt.active_status_cd = dactive_cd, lt.active_status_dt_tm =
     ...
     lt.parent_entity_id = component_id, lt.parent_entity_name = "CP_COMPONENT_DETAIL", lt
 ...
 SUBROUTINE getlongtextseqid(null)
   ...
   RETURN(newid)
 END ;Subroutine
 SUBROUTINE addstructureddoc(null)
   DECLARE isdocsize = i2 WITH protect, noconstant(0)
   DECLARE idoccnt = i2 WITH protect, noconstant(0)
   SET isdocsize = size(pw_rec->qual[1].structured_content,5)
   ...
   IF (isdocsize > 0)
    FOR (idoccnt = 1 TO isdocsize)
      SET xmltemplatesimportreq->template_cnt = isdocsize
      SET stat = alterlist(xmltemplatesimportreq->qual,idoccnt)
      SET xmltemplatesimportreq->qual[idoccnt].xml_blob = pw_rec->qual[1].structured_content[idoccnt]
      .sdoc_blob
    ENDFOR
    SET request->blob_in = cnvtrectojson(xmltemplatesimportreq)
    EXECUTE cp_import_sdoc_templates "MINE"
   ENDIF
 END ;Subroutine
 SET pw_rec->status_data.status = "S"
 CALL putjsonrecordtofile(pw_rec)
 COMMIT
#exit_script
 CALL setauditinitreplystatusbyprogramreplystatus(pw_rec)
