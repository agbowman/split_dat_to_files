CREATE PROGRAM cp_get_events_search:dba
 PROMPT
  "Output to File/Printer/MINE:" = "MINE",
  "Search String:" = "",
  "Input type:" = 0
  WITH outdev, search_string, inputtype
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE failed = i4 WITH public, noconstant(1)
 ...
 DECLARE input_search_str = vc WITH public, noconstant(" ")
 DECLARE event_code = i2 WITH public, constant(2)
 DECLARE inputtype_search = i2 WITH protect, constant(cnvtreal( $INPUTTYPE))
 DECLARE call_count = i4 WITH public, noconstant(0)
 DECLARE default_result_type_cd = f8 WITH public, noconstant(0)
 DECLARE type_code_set = i4 WITH protect, constant(289)
 DECLARE numerical_type = f8 WITH protect, constant(uar_get_code_by("MEANING",type_code_set,"3"))
 DECLARE calculation_type = f8 WITH protect, constant(uar_get_code_by("MEANING",type_code_set,"8"))
 DECLARE multi_type = f8 WITH protect, constant(uar_get_code_by("MEANING",type_code_set,"5"))
 DECLARE alpha_type = f8 WITH protect, constant(uar_get_code_by("MEANING",type_code_set,"2"))
 DECLARE freetext_type = f8 WITH protect, constant(uar_get_code_by("MEANING",type_code_set,"7"))
 DECLARE alpha_and_freetext_type = f8 WITH protect, constant(uar_get_code_by("MEANING",type_code_set,
   "21"))
 DECLARE multi_alpha_and_freetext_type = f8 WITH protect, constant(uar_get_code_by("MEANING",
   type_code_set,"22"))
 ...
 CALL getsuggestions(null)
 GO TO exit_script
 SUBROUTINE getsuggestions(null)
   SET input_search_str = cnvtalphanum(cnvtupper(replace( $SEARCH_STRING,"&#94;","^",0)))
   SET startstr = concat("cv.display_key = patstring('",input_search_str,"*')")
   SET containstr = concat("cv.display_key = patstring('*",input_search_str,
    "*') and cv.display_key != patstring('",input_search_str,"*')")
   SELECT INTO "nl:"
    cv.code_set, cv.definition, cv.description,
    cv.display, cv.display_key, vec.event_set_name,
    dta.default_result_type_cd, dta.event_cd
    FROM code_value cv,
     v500_event_code vec,
    ...
    DETAIL
     ...
     ENDIF
     IF (((inputtype_search=1
      AND ((dta.default_result_type_cd=numerical_type) OR (dta.default_result_type_cd=
     calculation_type)) ) OR (((inputtype_search=0
      AND ((dta.default_result_type_cd=multi_type) OR (((dta.default_result_type_cd=alpha_type) OR (
     ((dta.default_result_type_cd=alpha_and_freetext_type) OR (dta.default_result_type_cd=
     multi_alpha_and_freetext_type)) )) )) ) OR (inputtype_search=2
      AND dta.default_result_type_cd=freetext_type)) )) )
      reply->starts_with_list[st_cnt].result_cd = cv.code_value, reply->starts_with_list[st_cnt].
      result_disp = cv.display, reply->starts_with_list[st_cnt].result_desc = cv.description,
      reply->starts_with_list[st_cnt].type_flag = event_code, reply->starts_with_list[st_cnt].
      event_set_name = vec.event_set_name, reply->starts_with_list[st_cnt].task_assay_cd = dta
      .task_assay_cd
     ENDIF
    FOOT REPORT
    ...
    WITH nocounter
   ;end select
   SET failed = errors_handled("SELECT","V500_EVENT_CODE")
   IF (failed)
    GO TO exit_script
   ELSE
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     calculation_type)) ) OR (((inputtype_search=0
      AND ((dta.default_result_type_cd=multi_type) OR (((dta.default_result_type_cd=alpha_type) OR (
     ((dta.default_result_type_cd=alpha_and_freetext_type) OR (dta.default_result_type_cd=
     multi_alpha_and_freetext_type)) )) )) ) OR (inputtype_search=3
      AND dta.default_result_type_cd=freetext_type)) )) )
      reply->contains_list[cont_cnt].result_cd = cv.code_value, reply->contains_list[cont_cnt].
      result_disp = cv.display, reply->contains_list[cont_cnt].result_desc = cv.description,
      reply->contains_list[cont_cnt].type_flag = event_code, reply->contains_list[cont_cnt].
      event_set_name = vec.event_set_name, reply->contains_list[cont_cnt].task_assay_cd = dta
      .task_assay_cd
     ENDIF
    FOOT REPORT
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE errors_handled(operation_name,target_obj_name)
   SET call_count += 1
   DECLARE err_num = i4 WITH protect, noconstant(0)
   DECLARE err_msg = vc WITH protect, noconstant("")
   DECLARE error_code = i4 WITH protect, noconstant(0)
   SET error_code = error(err_msg,0)
   WHILE (error_code != 0)
     SET err_num += 1
     SET stat = alterlist(reply->status_data.subeventstatus,err_num)
     SET reply->status_data.subeventstatus[err_num].operationname = operation_name
     SET reply->status_data.subeventstatus[err_num].operationstatus = "F"
     SET reply->status_data.subeventstatus[err_num].targetobjectname = target_obj_name
     SET reply->status_data.subeventstatus[err_num].targetobjectvalue = concat(
      "Errors_handled call count: ",trim(cnvtstring(call_count))," Error: ",err_msg)
     SET error_code = error(err_msg,0)
   ENDWHILE
   IF (err_num > 0)
    RETURN(true)
