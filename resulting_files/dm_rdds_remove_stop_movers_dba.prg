CREATE PROGRAM dm_rdds_remove_stop_movers:dba
 ...
 SUBROUTINE dsdm_kill_mover(dkm_sid,dkm_serial,dkm_ignore_ind)
   ...
   RETURN(dkm_ret)
 END ;Subroutine
 DECLARE drrsm_cnt = i4 WITH protect, noconstant(0)
 DECLARE drrsm_idx = i4 WITH protect, noconstant(0)
 DECLARE drrsm_retry_cnt = i4 WITH protect, noconstant(3)
 DECLARE drrsm_remove_rdds_ctxt = vc WITH protect, noconstant(" ")
 DECLARE drrsm_env_id = f8 WITH protect, noconstant(0.0)
 FREE RECORD drrsm_process
 RECORD drrsm_process(
   1 list[*]
 ...
 DECLARE drrsm_get_process_info(dgpi_process=vc(ref)) = i4
 IF (check_logfile("dm_rdds_rm_mvrs",".log","DM_RDDS_REMOVE_STOP_MOVERS LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm_rdds_remove_stop_movers"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF ((inhouse_misc->inhouse_domain=1))
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    drrsm_remove_rdds_ctxt = di.info_char
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF (((drrsm_remove_rdds_ctxt="YES") OR ((inhouse_misc->inhouse_domain=0))) )
  SET dm_err->eproc = "Gather environment_id"
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="DATA MANAGEMENT"
    AND di.info_name="DM_ENV_ID"
   DETAIL
    drrsm_env_id = di.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Set movers to run to 0"
  UPDATE  FROM dm_info
   SET info_number = 0, updt_cnt = (updt_cnt+ 1), updt_id = reqinfo->updt_id,
    updt_task = reqinfo->updt_task, updt_applctx = reqinfo->updt_applctx, updt_dt_tm = cnvtdatetime(
     curdate,curtime3)
   WHERE info_domain=patstring(concat("MERGE*",cnvtstring(drrsm_env_id)))
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_program
  ENDIF
  SET drrsm_cnt = drrsm_get_process_info(drrsm_process)
  IF (drrsm_cnt < 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  WHILE (drrsm_retry_cnt > 0
   AND size(drrsm_process->list,5) > 0)
    CALL pause(600)
    FOR (drrsm_idx = 1 TO drrsm_cnt)
      CALL dsdm_kill_mover(drrsm_process->list[drrsm_idx].session_id,drrsm_process->list[drrsm_idx].
       serial_num,1)
    ENDFOR
    SET stat = initrec(drrsm_process)
    SET drrsm_cnt = drrsm_get_process_info(drrsm_process)
    IF (drrsm_cnt < 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    SET drrsm_retry_cnt = (drrsm_retry_cnt - 1)
  ENDWHILE
 ELSE
  SET dm_err->err_ind = 1
  ...
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 SUBROUTINE drrsm_get_process_info(dgpi_process)
   DECLARE dgpi_cnt = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    ...
    DETAIL
     dgpi_cnt = (dgpi_cnt+ 1)
     IF (mod(dgpi_cnt,100)=1)
      stat = alterlist(dgpi_process->list,(dgpi_cnt+ 99))
     ENDIF
     dgpi_process->list[dgpi_cnt].dm_refchg_process_id = d.dm_refchg_process_id, dgpi_process->list[
     dgpi_cnt].rdbhandle_value = d.rdbhandle_value, dgpi_process->list[dgpi_cnt].env_source_id = d
     .env_source_id,
     dgpi_process->list[dgpi_cnt].session_id = s.sid, dgpi_process->list[dgpi_cnt].serial_num = s
     .serial#
    FOOT REPORT
     stat = alterlist(dgpi_process->list,dgpi_cnt)
    WITH nocounter
   ;end select
   IF (check_error("Gather active movers to kill") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(- (1))
   ENDIF
   RETURN(dgpi_cnt)
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "...Ending dm_rdds_remove_movers"
 CALL final_disp_msg("dm_rdds_rm_mvrs")
END GO
