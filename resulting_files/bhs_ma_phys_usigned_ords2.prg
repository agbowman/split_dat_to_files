CREATE PROGRAM bhs_ma_phys_usigned_ords2
 PROMPT
 ...
 SET v_order_status_cd = uar_get_code_by("MEANING",14281,"ORDERED")
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   WHERE (oa.order_id=data->qual[d1.seq].order_id)
    AND oa.dept_status_cd=v_order_status_cd)
   JOIN (d2)
   JOIN (pr
  ...
  DETAIL
   IF (lt.long_text_id > 0)
    ...
    IF (comment_len <= pg_width
     AND eol1=0)
     cnt_comment += 1, stat = alterlist(data->qual[d1.seq].comments,cnt_comment), data->qual[d1.seq].
     comments[cnt_comment].comment = comment_string,
    ...
    IF (comment_len > pg_width
     ...
     WHILE (temp_pos <= comment_len)
       cnt_comment += 1, stat = alterlist(data->qual[d1.seq].comments,cnt_comment), data->qual[d1.seq
       ].comments[cnt_comment].comment = substring(temp_pos,pg_width,comment_string),
       data->qual[d1.seq].comment_cnt = cnt_comment, temp_pos += pg_width
     ENDWHILE
    ENDIF
    IF (eol1 > 0)
     temp_pos = 1
     WHILE (temp_pos <= comment_len)
       cnt_comment += 1, stat = alterlist(data->qual[d1.seq].comments,cnt_comment), data->qual[d1.seq
       ].comments[cnt_comment].comment = trim(substring(temp_pos,(eol1 - temp_pos),comment_string),3),
 ...
 SELECT INTO value(p_output)
  ...
  HEAD PAGE
   ...
   "{b/18}Ordering Physician", rowplusone2,
   CALL print(calcpos(x_col1,yrow)),
   "{b/12}Order Detail",
  ...
  HEAD mrn
   IF ((data->order_cnt > 0))
    ...
    CALL print(calcpos(xcol1,yrow)), "{b}", data->qual[d1.seq].pt_name,
    CALL print(calcpos(xcol6,yrow)), "{endb}", data->qual[d1.seq].fmrn,
