CREATE PROGRAM dm2_utc_set_ready_to_run:dba
 ...
 DECLARE dusrtr_idx = i4 WITH protect, noconstant(0)
 DECLARE dusrtr_priority_from = i4 WITH protect, noconstant(0)
 DECLARE dusrtr_priority_to = i4 WITH protect, noconstant(99999)
 DECLARE cur_priority_null = i2 WITH protect, noconstant(0)
 DECLARE dusrtr_calltime = f8 WITH protect, noconstant(0.0)
 DECLARE dm2_utc_bkgrnd_calling_script = vc WITH public, noconstant("DM2_UTC_SET_READY_TO_RUN")
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE load_priorities(null) = i2
 DECLARE update_ready_to_run_ind(cur_priority=i4) = i2
 DECLARE check_error_status(cur_priority=i4) = i2
 DECLARE dusrtr_dop = i4 WITH protect, noconstant(0)
 DECLARE dusrtr_cmd = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_utc_set_rdy2rn",".log","Starting dm2_utc_set_ready_to_run")=0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_utc_set_ready_to_run"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Validating input parameter"
 CALL disp_msg("",dm_err->logfile,0)
 IF (load_priorities(null)=0)
  GO TO end_program
 ENDIF
 SET ddl_in_error = 0
 SET priorityx = 1
 IF (check_error_status(priority_tmp->priorities[priorityx].priority)=0)
  GO TO end_program
 ENDIF
 WHILE ((priorityx <= priority_tmp->priority_cnt)
  AND ddl_in_error=0)
   SET dm_err->eproc = "Check for Migration Parrallel DDL Degree."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT
    IF ((dm2_install_schema->process_option="MIGRATION/UTC"))
     FROM dm_info@ref_data_link d
     WHERE d.info_domain="DM2_MIG_PARALLEL_DDL"
      AND d.info_name="DOP"
    ELSE
     FROM dm_info d
     WHERE d.info_domain="DM2_MIG_PARALLEL_DDL"
   ...
   ENDIF
   IF (dusrtr_dop > 0)
    SET dusrtr_cmd = concat("rdb alter session force parallel ddl parallel ",cnvtstring(dusrtr_dop),
     " go")
    IF (dm2_push_cmd(dusrtr_cmd,1)=0)
     GO TO end_program
    ENDIF
   ENDIF
   IF (dm2_cleanup_stranded_appl(null)=0)
    GO TO end_program
   ENDIF
   SET dm_err->eproc = build("*** EXECUTING PRIORITY: ",priority_tmp->priorities[priorityx].priority,
    " ***")
   CALL disp_msg("",dm_err->logfile,0)
   IF (update_ready_to_run_ind(priority_tmp->priorities[priorityx].priority)=0)
    GO TO end_program
   ENDIF
   EXECUTE dm2_utc_schema_runner
   IF ((dm_err->err_ind=1))
    GO TO end_program
   ENDIF
   SET cur_priority_running = 1
   SET cur_priority_null = 1
   SET dusrtr_calltime = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3))
   WHILE (((cur_priority_running=1) OR (cur_priority_null=1)) )
     IF (datetimediff(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),cnvtdatetimeutc(cnvtdatetime(
        dusrtr_calltime)),4) > 5)
      SET dusrtr_calltime = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3))
      IF (dm2_cleanup_stranded_appl(null)=0)
      ...
      ENDIF
     ENDIF
     IF (check_running_status(priority_tmp->priorities[priorityx].priority)=0)
      GO TO end_program
     ENDIF
     IF (((cur_priority_running=1) OR (cur_priority_null=1)) )
      CALL echo("**************************************************")
      CALL echo("Waiting on higher priority DDL to finish executing")
      CALL echo("**************************************************")
      CALL pause(15)
     ENDIF
   ENDWHILE
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("priority:",priority_tmp->priorities[priorityx].priority))
   ENDIF
   SET priorityx = (priorityx+ 1)
 ENDWHILE
 IF (ddl_in_error=1)
  SET dm_err->user_action = concat(dm_err->user_action," Schema Date: ",format(dusrtr_work->
    schema_date,"dd-mmm-yyyy;L;D"))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Execute_DDL_Command"
  SET dm_err->emsg = dm_err->user_action
  CALL disp_msg("One or more errors were encountered executing DDL",dm_err->logfile,1)
  GO TO end_program
 ENDIF
 SUBROUTINE load_priorities(null)
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = "Load the Priorities Record Structure"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    priority = ddol.priority
    FROM dm2_ddl_ops ddo,
     dm2_ddl_ops_log ddol
    WHERE (ddo.run_id=dm2_install_schema->run_id)
     AND ddol.run_id=ddo.run_id
     AND ddo.process_option != cpreview
     AND ddol.status = null
     AND ddol.priority > dusrtr_priority_from
     AND ddol.priority < dusrtr_priority_to
    GROUP BY ddo.run_id, ddol.priority
    ORDER BY ddol.priority
    HEAD REPORT
     priorityx = 0
    DETAIL
     priorityx = (priorityx+ 1), stat = alterlist(priority_tmp->priorities,priorityx), priority_tmp->
     priorities[priorityx].priority = priority
    FOOT REPORT
     priority_tmp->priority_cnt = priorityx
    WITH nocounter
   ;end select
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(priority_tmp)
   ENDIF
   IF (check_error("Load_Priorities")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE update_ready_to_run_ind(cur_priority)
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = "Update Ready to Run Indicator for Run Id"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   UPDATE  FROM dm2_ddl_ops_log ddol
    SET ddol.ready_to_run_ind = 1
    WHERE (ddol.run_id=dm2_install_schema->run_id)
     AND ddol.priority=cur_priority
   ...
   ENDIF
   IF (check_error("Update_Ready_To_Run_Ind")=1)
    ROLLBACK
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ELSE
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE check_error_status(cur_priority)
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = "Check the Error Status"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    ddo.schema_date, ddol.error_msg
    FROM dm2_ddl_ops ddo,
     dm2_ddl_ops_log ddol
    WHERE (ddo.run_id=dm2_install_schema->run_id)
     AND ddol.run_id=ddo.run_id
    ...
    DETAIL
     dusrtr_work->schema_date = ddo.schema_date, err_msg = ddol.error_msg
    WITH nocounter
   ;end select
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows In Error:",curqual))
   ENDIF
   IF (check_error("Check_Error_Status")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ELSE
    IF (curqual > 0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE check_running_status(cur_priority)
   DECLARE crs_running_or_null_cnt = i4 WITH protect, noconstant(0)
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = "Check Running Status for Priority"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Run id:",dm2_install_schema->run_id))
    CALL echo(build("Priority:",cur_priority))
   ENDIF
   SET cur_priority_running = 0
   SET cur_priority_null = 0
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=dm2_install_schema->run_id)
     AND ddol.priority=cur_priority
    ...
    DETAIL
     crs_running_or_null_cnt = (crs_running_or_null_cnt+ 1)
     IF (ddol.status="RUNNING")
      cur_priority_running = 1
     ELSE
      cur_priority_null = 1
     ENDIF
    WITH nocounter
   ;end select
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("Rows Currently Running or yet to run: ",cnvtstring(crs_running_or_null_cnt,4)))
   ENDIF
   IF (check_error("Check_Running_Status")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
#end_program
 SET dm_err->eproc = "Ending dm2_utc_set_ready_to_run"
 CALL final_disp_msg("dm2_utc_set_rdy2rn")
END GO
