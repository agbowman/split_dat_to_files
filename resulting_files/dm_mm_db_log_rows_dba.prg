CREATE PROGRAM dm_mm_db_log_rows:dba
 ...
 SET reply->table_name = "MM_DB_LOG"
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    mdl.rowid
    FROM mm_db_log mdl
    WHERE parser(sbr_getrowidnotexists("mdl.log_id between curMinID and curMaxID","mdl"))
     AND mdl.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = mdl.rowid
    WITH nocounter, maxqual(mdl,value(rowsleft))
   ;end select
