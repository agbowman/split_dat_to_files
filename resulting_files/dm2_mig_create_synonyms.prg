CREATE PROGRAM dm2_mig_create_synonyms
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmcs_cnt = i4 WITH protect, noconstant(0)
 FREE RECORD dmcs_syn_cmds
 RECORD dmcs_syn_cmds(
   1 cnt = i4
   1 qual[*]
     2 syn_cmd = vc
     2 syn_name = vc
 ...
 IF (check_logfile("dm2_mig_crtsyn",".log","dm2_mig_create_synonyms")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting Dm2_mig_create_synonyms"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Scan for synonyms and generate associated synonym creation DDL from SOURCE."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM (
   (
   (SELECT
    create_text = sqlpassthru("dbms_metadata.get_ddl('SYNONYM',d.synonym_name,d.owner)"), syn_name =
    d.synonym_name, syn_owner = d.owner,
    tadmcs_owner = d.table_owner, tadmcs_name = d.table_name, d.db_link
    FROM dba_synonyms d
   ...
   a)
  DETAIL
   dmcs_syn_cmds->cnt = (dmcs_syn_cmds->cnt+ 1), stat = alterlist(dmcs_syn_cmds->qual,dmcs_syn_cmds->
    cnt), dmcs_syn_cmds->qual[dmcs_syn_cmds->cnt].syn_cmd = a.create_text,
   dmcs_syn_cmds->qual[dmcs_syn_cmds->cnt].syn_name = a.syn_name, dmcs_syn_cmds->qual[dmcs_syn_cmds->
   cnt].syn_owner = a.syn_owner, dmcs_syn_cmds->qual[dmcs_syn_cmds->cnt].tbl_owner = a.tadmcs_owner,
   dmcs_syn_cmds->qual[dmcs_syn_cmds->cnt].tbl_name = a.tadmcs_name, dmcs_syn_cmds->qual[
   dmcs_syn_cmds->cnt].db_link = a.db_link
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dmcs_syn_cmds->cnt=0))
  SET dm_err->eproc = "No synonyms detected."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF (dmr_prompt_connect_data("TARGET","V500","CO")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determine which synonyms exist in TARGET."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_synonyms ds,
   (dummyt d  WITH seq = dmcs_syn_cmds->cnt)
  PLAN (d
   WHERE d.seq > 0)
   JOIN (ds
   WHERE (ds.synonym_name=dmcs_syn_cmds->qual[d.seq].syn_name)
    AND (ds.owner=dmcs_syn_cmds->qual[d.seq].syn_owner)
    AND (ds.table_name=dmcs_syn_cmds->qual[d.seq].tbl_name)
    AND (ds.table_owner=dmcs_syn_cmds->qual[d.seq].tbl_owner))
  DETAIL
   dmcs_syn_cmds->qual[d.seq].exists_ind = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (locateval(dmcs_cnt,1,dmcs_syn_cmds->cnt,0,dmcs_syn_cmds->qual[dmcs_cnt].exists_ind)=0)
  SET dm_err->eproc = "All synonyms exist in TARGET, no synonym creation required."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF ((validate(dmcs_report_option,- (1)) != - (1)))
  SET dm_err->eproc = "Generate report listing synonyms which will be created."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO mine
   FROM (dummyt d  WITH seq = dmcs_syn_cmds->cnt)
   HEAD REPORT
    CALL print("SYNONYM CREATION REPORT"), row + 2,
    CALL print(concat("THE FOLLOWING SYNONYMS WILL BE CREATED IN TARGET DATABASE ",dmr_mig_data->
   ...
   DETAIL
    IF ((dmcs_syn_cmds->qual[d.seq].exists_ind=0))
     col 2,
     CALL print(concat(dmcs_syn_cmds->qual[d.seq].syn_owner,".",dmcs_syn_cmds->qual[d.seq].syn_name,
      " on ",dmcs_syn_cmds->qual[d.seq].tbl_owner,
      ".",dmcs_syn_cmds->qual[d.seq].tbl_name,evaluate(dmcs_syn_cmds->qual[d.seq].db_link,"","",
       concat(" using ",dmcs_syn_cmds->qual[d.seq].db_link)))), row + 1
    ENDIF
  ...
  SET message = window
  CALL clear(1,1)
  CALL text(3,2,"Would you like to continue with synonym creation? Y/N:")
  CALL accept(3,57,"A;CU"," "
   WHERE curaccept IN ("Y", "N"))
  SET message = nowindow
  IF (curaccept="N")
   SET dm_err->eproc = "User elected to not create synonyms."
   CALL disp_msg("",dm_err->logfile,0)
   GO TO exit_script
  ENDIF
 ENDIF
 FOR (dmcs_cnt = 1 TO dmcs_syn_cmds->cnt)
   IF ((dmcs_syn_cmds->qual[dmcs_cnt].exists_ind=0))
    SET dm_err->eproc = concat("Creating synonym ",dmcs_syn_cmds->qual[dmcs_cnt].syn_owner,".",
     dmcs_syn_cmds->qual[dmcs_cnt].syn_name," on ",
     dmcs_syn_cmds->qual[dmcs_cnt].tbl_owner,".",dmcs_syn_cmds->qual[dmcs_cnt].tbl_name)
    CALL disp_msg("",dm_err->logfile,0)
    EXECUTE dm2_exec_child_cmd "CMD", concat("rdb asis(^",dmcs_syn_cmds->qual[dmcs_cnt].syn_cmd,
     "^) go")
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
...
#exit_script
 ...
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_create_synonyms completed"
 ENDIF
 CALL final_disp_msg("dm2_mig_crtsyn")
END GO
