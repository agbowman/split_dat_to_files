CREATE PROGRAM bhs_athn_get_message_list_v2
 ...
 DECLARE erxrenewal = f8 WITH protect, constant(uar_get_code_by("DISPLAY_KEY",6026,"ERXRENEWAL"))
 ...
 IF (( $6 > 0))
  SET where_params_prsnl_pool = build(" taa.assign_prsnl_group_id =", $6)
 ELSE
  SET where_params_prsnl_pool = build(" taa.assign_prsnl_id =", $2)
 ENDIF
 ...
 IF (( $4 > " ")
  ...
  IF (( $3="(Reminder)"))
   SET where_params_from_dt = build("taa.remind_dt_tm >= cnvtdatetime(",cnvtdatetimeutc2(date_line,
     "YYYY-MM-DD",time_line,"HH;mm;ss",0),")")
  ELSE
  ...
  IF (( $3="(Reminder)"))
   SET where_params_to_dt = build("taa.remind_dt_tm <= cnvtdatetime(",cnvtdatetimeutc2(date_line,
     "YYYY-MM-DD",time_line,"HH;mm;ss",0),")"," AND taa.remind_dt_tm <= SYSDATE")
  ELSE
 ...
 ELSE
  IF (( $3="(Reminder)"))
   SET where_params_from_dt = build("taa.remind_dt_tm between (SYSDATE - 30) and SYSDATE")
  ELSE
 ...
 RECORD msg_list(
   1 msgs[*]
     ...
     2 callername = vc
     2 callerphones = vc
     2 senderprsnlgroupid = f8
     2 senderprsnlgroupname = vc
     2 actionrequests[*]
       3 taskid = f8
 ...
 SELECT INTO "nl:"
  ...
  patient_name = trim(replace(replace(replace(replace(replace(p.name_full_formatted,"&","&amp;",0),
   ...
   replace(replace(replace(replace(replace(uar_get_code_display(ta.task_priority_cd),"&","&amp;",0),
       "<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3), remind_dt_tm = format(taa
  ...
  ELSE "FALSE"
  ENDIF
  , ta.msg_subject_cd, ta.msg_sender_prsnl_group_id,
  sender_prsnl_group = trim(replace(replace(replace(replace(replace(pg.prsnl_group_name,"&","&amp;",0
        ),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3)
  ...
  PLAN (ta
   ...
   WHERE ta.task_id=taa.task_id
    ...
    AND parser(where_params_prsnl_pool)
   ...
   JOIN (pg
   WHERE (pg.prsnl_group_id= Outerjoin(ta.msg_sender_prsnl_group_id))
    AND (pg.active_ind= Outerjoin(1)) )
  ORDER BY taa.updt_dt_tm DESC, ta.task_id
  HEAD ta.task_id
   vcnt += 1, stat = alterlist(msg_list->msgs,vcnt), msg_list->msgs[vcnt].taskid = ta.task_id,
   ...
   replyallowedindicator = reply_allowed, msg_list->msgs[vcnt].senderprsnlgroupid = cnvtint(ta
    .msg_sender_prsnl_group_id),
   msg_list->msgs[vcnt].senderprsnlgroupname = sender_prsnl_group, acnt = 0
  HEAD task_subact_id
   acnt += 1, stat = alterlist(msg_list->msgs[vcnt].actionrequests,acnt), msg_list->msgs[vcnt].
 ...
 IF (vcnt > 0)
  SELECT INTO "nl:"
   ...
   HEAD taa_rec.assign_prsnl_id
    rcnt += 1, stat = alterlist(msg_list->msgs[d1.seq].receivers,rcnt), msg_list->msgs[d1.seq].
 ...
 DECLARE vlt3 = vc WITH protect, noconstant(" ")
 DECLARE vlt4 = vc WITH protect, noconstant(" ")
 ...
 SELECT INTO value( $1)
  ...
  HEAD REPORT
   ...
   FOR (i = 1 TO size(msg_list->msgs,5))
     ...
     row + 1, vlt3 = build("<senderPoolId>",cnvtint(msg_list->msgs[i].senderprsnlgroupid),
      "</senderPoolId>"), col + 1,
     vlt3, row + 1, vlt4 = build("<senderPoolName>",msg_list->msgs[i].senderprsnlgroupname,
      "</senderPoolName>"),
     col + 1, vlt4, row + 1,
