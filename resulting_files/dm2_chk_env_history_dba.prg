CREATE PROGRAM dm2_chk_env_history:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_chk_env_hist",".log","DM2_CHK_ENV_HISTORY LOG FILE...") != 1)
  GO TO exit_program
 ENDIF
 IF ((dir_env_maint_rs->tgt_env_id=0))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Checking to see if record structure is filled out."
  SET dm_err->emsg = "Record structure was not filled out."
  CALL disp_msg(" ",dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Checking if there is data in alpha_features table for target environment."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "NL:"
  FROM dm_alpha_features_env e
  WHERE (e.environment_id=dir_env_maint_rs->tgt_env_id)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSEIF (curqual > 0)
  SET dir_env_maint_rs->tgt_hist_fnd = 1
  SET dm_err->eproc = "Package history found in dm_alpha_features_env for target_environment."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Checking if ther is data in dm_ocd_log table for target environment."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "NL:"
  FROM dm_ocd_log dol
  WHERE (dol.environment_id=dir_env_maint_rs->tgt_env_id)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual > 0)
  SET dir_env_maint_rs->tgt_hist_fnd = 1
  SET dm_err->eproc = "Package history found in dm_ocd_log for target_environment."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc =
 "Checking if ther is data in dm_pkt_setup_proc_hist table for target environment."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "NL:"
  FROM dm_pkt_setup_proc_hist h
  WHERE (h.environment_id=dir_env_maint_rs->tgt_env_id)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual > 0)
  SET dir_env_maint_rs->tgt_hist_fnd = 1
  SET dm_err->eproc = "Package history found in dm_pkt_setup_proc_hist for target_environment."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Checking if there is RDDS history"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (checkprg("DM2_RDDS_CHECK_ENV_HISTORY") != 0)
  EXECUTE dm2_rdds_check_env_history
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
 ENDIF
#exit_program
 SET dm_err->eproc = "Exiting dm2_chk_env_history."
 CALL final_disp_msg("dm2_chk_env_hist")
END GO
