CREATE PROGRAM dm2_setup_new_db:dba
 ...
 SUBROUTINE dm2_adb_check(dac_db_link,dac_adb_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsnd_command = vc WITH protect, noconstant("")
 DECLARE dsnd_adb_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_setup_new_db",".log","dm2_setup_new_db LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_setup_new_db"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ( NOT (dm2_get_rdbms_version(null)))
  GO TO exit_script
 ENDIF
 IF ((dm2_rdbms_version->level1 >= 10))
  IF (dm2_adb_check("",dsnd_adb_ind)=0)
   GO TO exit_script
  ENDIF
  IF (((trim(currdbuser)="SYS") OR (trim(currdbuser)="ADMIN"
   AND dsnd_adb_ind=1)) )
   IF ((dm2_rdbms_version->level1 >= 11))
    ...
    IF ( NOT (dm2_push_cmd(concat(
     ...
     GO TO exit_script
    ENDIF
    IF ((dcdur_user_data->create_user_method != "DBMS_METADATA_METHOD"))
     SET dm_err->eproc =
     ...
     ENDIF
    ELSE
     SET dm_err->eproc = "Skipping DEFAULT profile changes setting limits to unlimited."
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    IF ( NOT (dm2_push_cmd(
    ...
    IF ( NOT (dm2_push_cmd(dsnd_command,1)))
     GO TO exit_script
    ENDIF
    SET dsnd_command =
   ...
   ELSE
    ...
    IF ( NOT (dm2_push_cmd(^RDB ASIS("BEGIN DBMS_SCHEDULER.DISABLE('GATHER_STATS_JOB'); END;") GO^,1)
    ))
     GO TO exit_script
    ENDIF
  ...
  ELSE
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Unable to run this command connected as ",currdbuser," user.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
#exit_script
 SET dm_err->eproc = "Ending dm2_setup_new_db"
 CALL final_disp_msg("dm2_setup_new_db")
