CREATE PROGRAM dm_rx_pha_wh_upload_rows:dba
 ...
 SET reply->table_name = "PHA_WH_UPLOAD"
 ...
 FOR (i = 1 TO size(request->tokens,5))
   IF ((request->tokens[i].token_str="WHDAYS"))
    SET v_days_to_keep = ceil(cnvtreal(request->tokens[i].value))
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 30)
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"WHDAYS",
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(pq.pha_wh_upload_id)
    FROM pha_wh_upload pq
    WHERE pq.pha_wh_upload_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   seqval = max(pq.pha_wh_upload_id)
   FROM pha_wh_upload pq
   DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     pq.rowid
     FROM pha_wh_upload pq
     WHERE parser(sbr_getrowidnotexists("pq.pha_wh_upload_id between curMinID and curMaxID","pq"))
      AND pq.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
      AND pq.active_ind=0
     DETAIL
      ...
      ENDIF
      reply->rows[v_rec_count].row_id = pq.rowid
     WITH nocounter, maxqual(pq,value(rowsleft))
    ;end select
