CREATE PROGRAM dm_cdi_ac_module_launch_rows:dba
 ...
 SET reply->table_name = "CDI_AC_MODULE_LAUNCH"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(l.cdi_ac_module_launch_id)
    FROM cdi_ac_module_launch l
    WHERE l.cdi_ac_module_launch_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   seqval = max(l.cdi_ac_module_launch_id)
   FROM cdi_ac_module_launch l
   DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     l.rowid
     FROM cdi_ac_module_launch l
     WHERE parser(sbr_getrowidnotexists("l.cdi_ac_module_launch_id between curMinID and curMaxID","l"
       ))
      AND l.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
      AND  NOT ( EXISTS (
     (SELECT
      m.modulelaunchid
      FROM cdi_ac_batchmodule m
      WHERE m.modulelaunchid=l.modulelaunchid)))
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,50)=1)
       stat = alterlist(reply->rows,(rows+ 49))
      ENDIF
      reply->rows[rows].row_id = l.rowid
     WITH nocounter, maxqual(l,value(rowsleft))
    ;end select
    ...
    IF (v_err_code2 > 0)
     ...
     SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERROR",
      "Failed in row collection: %1","d",nullterm(v_errmsg2))
     SET reply->status_data.status = "F"
    ENDIF
