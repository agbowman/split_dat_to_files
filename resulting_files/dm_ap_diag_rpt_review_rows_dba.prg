CREATE PROGRAM dm_ap_diag_rpt_review_rows:dba
 ...
 DECLARE minimum_months_to_keep = i4 WITH protect, constant(24)
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    adrr.rowid
    FROM ap_diag_rpt_review adrr
    WHERE parser(sbr_getrowidnotexists("adrr.event_id between curMinID and curMaxID","adrr"))
     AND adrr.verified_dt_tm <= cnvtdatetime(purge_dt_tm)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,100)=1)
      stat = alterlist(reply->rows,(rows+ 99))
     ENDIF
     reply->rows[rows].row_id = adrr.rowid
    WITH nocounter, maxqual(adrr,value(rowsleft))
   ;end select
