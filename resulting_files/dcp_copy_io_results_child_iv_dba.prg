CREATE PROGRAM dcp_copy_io_results_child_iv:dba
 ...
 DECLARE bolus = f8 WITH public, noconstant(uar_get_code_by("MEANING",180,"BOLUS"))
 DECLARE infuse = f8 WITH public, noconstant(uar_get_code_by("MEANING",180,"INFUSE"))
 ...
 IF (((med < 0.0) OR (((ivparent < 0.0) OR (((powerchart73 < 0.0) OR (((bolus < 0.0) OR (((infuse <
 0.0) OR (((confirmed < 0.0) OR (ml < 0.0)) )) )) )) )) )) )
 ...
 IF ((powerchart73 > - (1.0)))
  SELECT INTO "nl:"
   ...
   WITH nocounter
  ;end select
  IF (((dcpgeneric=0.0) OR (error(string_struct->ms_err_msg,0) != 0)) )
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="COPY EXISTING IO RESULTS TO IO2G COMPATIBLE - IV"
 ...
 IF (curqual=0)
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="COPY EXISTING IO RESULTS TO IO2G COMPATIBLE PARENT - IV"
  ...
  IF (curqual=1)
   ...
   INSERT  FROM dm_info di
    SET di.info_domain = "COPY EXISTING IO RESULTS TO IO2G COMPATIBLE - IV", di.info_name = concat(
 ...
 WHILE (mf_cur_min_id <= mf_max_epr_id)
   ...
   ENDIF
   EXECUTE dcp_copy_io_results_iv value(mf_cur_min_id), value(mf_cur_max_id)
