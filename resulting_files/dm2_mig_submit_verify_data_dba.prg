CREATE PROGRAM dm2_mig_submit_verify_data:dba
 ...
 SUBROUTINE dm2_rename_login_default(sbr_rld_mode)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmsvd_job_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmsvd_loop_ind = i4 WITH protect, noconstant(0)
 DECLARE dmsvd_inst_loc = vc WITH protect, noconstant("")
 DECLARE dmsvd_file_name = vc WITH protect, noconstant("")
 IF (check_logfile("dm2migsubvdata",".log","DM2_MIG_SUBMIT_VERIFY_DATA LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_submit_verify_data"
 CALL disp_msg("",dm_err->logfile,0)
 IF (size(trim(reflect(parameter(1,0))),1) > 0)
  SET dmsvd_job_cnt = cnvtint(value(parameter(1,0)))
 ENDIF
 IF (dmsvd_job_cnt=0)
  SET dmsvd_job_cnt = 1
 ELSE
  SET dmsvd_job_cnt = least(10,dmsvd_job_cnt)
  CALL echo("Job count ceiling set at 10")
 ENDIF
 ...
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dmsvd_inst_loc = concat(trim(logical("cer_install")),"/")
 SET dmsvd_file_name = "dm2_mig_verify_data.ksh"
 SET dmsvd_inst_loc = build(dmsvd_inst_loc,dmsvd_file_name)
 SET dm_err->eproc = "Check if file exists or not"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (dm2_findfile(build(dmsvd_inst_loc))=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "failed to find file"
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Nohup ",trim(cnvtstring(dmsvd_job_cnt))," compare jobs")
 CALL disp_msg(" ",dm_err->logfile,0)
 FOR (dmsvd_loop_ind = 1 TO dmsvd_job_cnt)
   IF (dm2_push_dcl(concat("nohup ",dmsvd_inst_loc," > /dev/null 2>&1 &"))=0)
    GO TO exit_script
   ENDIF
 ENDFOR
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 CALL echo(build("The number of jobs submitted are :",dmsvd_job_cnt))
#exit_script
 SET dm_err->eproc = "Ending dm2_mig_submit_verify_data.prg"
 CALL final_disp_msg("dm2migsubvdata")
END GO
