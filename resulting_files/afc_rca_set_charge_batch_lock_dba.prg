CREATE PROGRAM afc_rca_set_charge_batch_lock:dba
 ...
 IF ((request->chargebatchid > 0.0))
  SELECT INTO "nl:"
   ...
   DETAIL
    stat = alterlist(chrgbatchreq->objarray,1), chrgbatchreq->objarray[1].charge_batch_id = b
    .charge_batch_id, chrgbatchreq->objarray[1].updt_cnt = b.updt_cnt
    IF ((request->lockind=0))
     IF ((chrgbatchreq->objarray[1].status_cd != cs4002322_failed_cd))
      IF (d.status_cd > 0)
       chrgbatchreq->objarray[1].status_cd = d.status_cd
      ELSE
       chrgbatchreq->objarray[1].status_cd = cs4002322_pending_cd
      ENDIF
     ENDIF
    ELSE
     chrgbatchreq->objarray[1].status_cd = cs4002322_locked_cd
    ENDIF
   WITH nocounter
  ;end select
  IF (size(chrgbatchreq->objarray,5) > 0)
   ...
   IF ((chrgbatchrep->status_data.status != "S"))
    CALL exitservicefailure("Failure updated charge batch.",go_to_exit_script)
   ENDIF
  ELSE
   CALL exitservicefailure("Applicable charge batch not found.",go_to_exit_script)
  ENDIF
 ELSE
  CALL exitservicefailure("Charge batch not specified.",go_to_exit_script)
 ENDIF
