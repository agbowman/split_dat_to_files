CREATE PROGRAM bhs_prax_get_patient_list
 ...
 SELECT INTO  $1
  ...
  HEAD e.encntr_id
   ...
   row + 1, v20 = build("<EncntrTypeClassCd>",cnvtint(e.encntr_type_class_cd),"</EncntrTypeClassCd>"),
   col + 1,
   v20, row + 1, v21 = build("<EncntrTypeClass>",trim(replace(replace(replace(replace(replace(
          uar_get_code_display(e.encntr_type_class_cd),"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'",
       "&apos;",0),'"',"&quot;",0),3),"</EncntrTypeClass>"),
   col + 1, v21, row + 1,
   v22 = build("<EncntrTypeCd>",cnvtint(e.encntr_type_cd),"</EncntrTypeCd>"), col + 1, v22,
   row + 1, v23 = build("<EncntrType>",trim(replace(replace(replace(replace(replace(
          uar_get_code_display(e.encntr_type_cd),"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'",
       "&apos;",0),'"',"&quot;",0),3),"</EncntrType>"), col + 1,
   v23, row + 1, v24 = build("<EncntrStatusCd>",cnvtint(e.encntr_status_cd),"</EncntrStatusCd>"),
   col + 1, v24, row + 1,
   ...
   v30,
   row + 1, v31 = build("<SexCd>",cnvtint(p.sex_cd),"</SexCd>"), col + 1,
   v31, row + 1, v32 = build("<Sex>",trim(replace(replace(replace(replace(replace(
          uar_get_code_display(p.sex_cd),"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),
      '"',"&quot;",0),3),"</Sex>"),
   col + 1, v32, row + 1,
   v33 = build("<Language>",trim(replace(replace(replace(replace(replace(uar_get_code_display(p
           .language_cd),"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3
     ),"</Language>"), col + 1, v33,
   row + 1, v34 = build("<AbsBirthDtTm>",format(p.abs_birth_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"),
    "</AbsBirthDtTm>"), col + 1,
   v34, row + 1, v35 = build("<MaritalStatus>",trim(replace(replace(replace(replace(replace(
          uar_get_code_display(p.marital_type_cd),"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'",
       "&apos;",0),'"',"&quot;",0),3),"</MaritalStatus>"),
   col + 1, v35, row + 1,
   v36 = build("<LastEncntrDtTm>",format(p.last_encntr_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"),
    "</LastEncntrDtTm>"), col + 1, v36,
   row + 1
   IF (((e.encntr_type_class_cd=inpatient_cd) OR (e.encntr_type_class_cd=observation_cd)) )
    v38 = build("<PatientType>","IP","</PatientType>")
   ELSEIF (e.encntr_type_class_cd=emergency_cd)
    v38 = build("<PatientType>","EMER","</PatientType>")
   ELSE
