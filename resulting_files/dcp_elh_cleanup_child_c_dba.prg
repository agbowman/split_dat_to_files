CREATE PROGRAM dcp_elh_cleanup_child_c:dba
 ...
 SET ms_child_c_info_domain_nm = "DCP_ELH_CLEANUP PROCESS"
 SET ms_child_c_max_name = "MAX ENCNTR_LOC_HIST_ID EVALUATED"
 CALL sbr_get_min_max(mf_min_id,mf_max_id)
 CALL echo("Processing...")
 CALL echo("the ENCNTR_LOC_HIST table")
 CALL parser(
  "rdb asis(^update  /*+  CERNER_README<DCP_ELH_CLEANUP_CHILD:Q1> */ encntr_loc_hist elh ^)")
 CALL parser(concat("asis(^ set elh.updt_task =  ",cnvtstring(reqinfo->updt_task)," , ^) "))
 CALL parser("asis(^ elh.end_effective_dt_tm = (select nvl(e.disch_dt_tm,e.depart_dt_tm) ^)")
 CALL parser("asis(^ from encounter e ^)")
 CALL parser("asis(^ where e.encntr_id = elh. encntr_id) ^)")
 CALL parser("asis(^ where exists ^)")
 CALL parser("asis(^ ( select 1 from encounter e ^)")
 CALL parser("asis(^ where e.encntr_id = elh. encntr_id and ^)")
 CALL parser("asis(^ (e.disch_dt_tm is not NULL or e.depart_dt_tm is not NULL) ^)")
 CALL parser(concat("asis(^ and (e.encntr_status_cd+0 in ( ",cnvtstring(dischargecd)," , ^) "))
 CALL parser(concat("asis(^  ",cnvtstring(cancelcd)," ))) ^) "))
 CALL parser(concat("asis(^ and elh.encntr_loc_hist_id >= ",cnvtstring(mf_min_id)," ^) "))
 CALL parser(concat("asis(^ and elh.encntr_loc_hist_id <= ",cnvtstring(mf_max_id)," ^) "))
 CALL parser(
  "asis(^ and elh.end_effective_dt_tm > to_date('2100/01/01:12:00:00am','yyyy/mm/dd:hh:mi:ssam') ^) go "
  )
 CALL echo(build("number processed:",curqual))
