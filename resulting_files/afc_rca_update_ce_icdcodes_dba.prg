CREATE PROGRAM afc_rca_update_ce_icdcodes:dba
 ...
 DECLARE dxsequence = i4 WITH protect, noconstant(0)
 DECLARE uptchargeeventdetail(null) = i2
 DECLARE savechargeeventdetailicdcodes(null) = i2
 CALL beginservice("257887.000")
 ...
 IF ( NOT (validate(request->chargebatchid,- (1)) > 0.0))
  CALL logmessage("Main","BatchID is zero",log_debug)
  CALL exitservicefailure("BatchID is zero",go_to_exit_script)
 ENDIF
 IF ( NOT (uptchargeeventdetail("")))
  CALL exitservicefailure("Failed updating Diagnosis Pointer on existing charge event",
   go_to_exit_script)
 ENDIF
 IF ( NOT (savechargeeventdetailicdcodes("")))
  CALL exitservicefailure("Failed inserting new ICD Codes on charge_event_detail_field",
   go_to_exit_script)
 ENDIF
 CALL exitservicesuccess("Exiting script")
 SUBROUTINE uptchargeeventdetail(null)
   CALL logmessage("uptChargeEventDetail","Begin sub",log_debug)
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = value(size(request->chargeevents,5))),
     charge_batch_detail c
    PLAN (d1)
     JOIN (c
     WHERE (c.charge_batch_detail_id=request->chargeevents[d1.seq].chargeeventid)
      AND (c.charge_batch_id=request->chargebatchid)
      AND (c.encntr_id=request->encounterid))
    HEAD REPORT
     loopcnt = 0
    DETAIL
     loopcnt = (loopcnt+ 1)
     IF (mod(loopcnt,50)=1)
      stat = alterlist(chargebatchdetailreq->objarray,(loopcnt+ 49))
     ENDIF
     chargebatchdetailreq->objarray[d1.seq].charge_batch_detail_id = c.charge_batch_detail_id,
     chargebatchdetailreq->objarray[d1.seq].diagnosis_pointer_txt = request->chargeevents[d1.seq].
     diagnosispointertext, chargebatchdetailreq->objarray[d1.seq].updt_cnt = c.updt_cnt
    FOOT REPORT
     stat = alterlist(chargebatchdetailreq->objarray,loopcnt)
    WITH nocounter
   ;end select
   IF (size(chargebatchdetailreq->objarray,5) > 0)
    ...
    IF ((chargebatchdetailrep->status_data.status != "S"))
     CALL logmessage("uptChargeEventDetail",
      "Error updating Diagnosis Pointer on existing charge event",log_debug)
     RETURN(false)
    ELSE
     RETURN(true)
    ENDIF
   ELSE
    CALL logmessage("uptChargeEventDetail","Failed while retrieving Existing charge event",log_debug)
    RETURN(false)
   ENDIF
   CALL logmessage("uptChargeEventDetail","End sub",log_debug)
 END ;Subroutine
 SUBROUTINE savechargeeventdetailicdcodes(null)
   CALL logmessage("saveChargeEventDetailICDCodes","Begin sub",log_debug)
   CALL logmessage("saveChargeEventDetailICDCodes","Query for detail codes",log_debug)
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = value(size(request->chargeevents,5))),
    ...
    PLAN (d1)
     JOIN (c
     WHERE (c.charge_batch_detail_id=request->chargeevents[d1.seq].chargeeventid)
      AND c.type_cd=cs4002321_diag_cd)
    HEAD REPORT
     loopcnt = 0
    DETAIL
     loopcnt = (loopcnt+ 1)
     IF (mod(loopcnt,50)=1)
      stat = alterlist(chargebatchdtlcdreq->objarray,(loopcnt+ 49))
     ENDIF
     chargebatchdtlcdreq->objarray[loopcnt].charge_batch_detail_code_id = c
     .charge_batch_detail_code_id, chargebatchdtlcdreq->objarray[loopcnt].active_ind = false,
     chargebatchdtlcdreq->objarray[loopcnt].updt_cnt = c.updt_cnt
    FOOT REPORT
     stat = alterlist(chargebatchdtlcdreq->objarray,loopcnt)
    WITH nocounter
   ;end select
   IF (size(chargebatchdtlcdreq->objarray,5) > 0)
    CALL logmessage("saveChargeEventDetailICDCodes","Inactivate detail codes",log_debug)
    ...
    IF ((chargebatchdtlcdrep->status_data.status != "S"))
     CALL logmessage("saveChargeEventDetailICDCodes","Error inactivating existing ICD codes",
      log_debug)
     RETURN(false)
    ENDIF
   ENDIF
   ...
   CALL logmessage("saveChargeEventDetailICDCodes","Load ICD codes",log_debug)
   SELECT INTO "nl:"
    chargeeventid = request->chargeevents[d1.seq].chargeeventid
    FROM (dummyt d1  WITH seq = value(size(request->chargeevents,5))),
     (dummyt d2  WITH seq = 1)
    PLAN (d1
     WHERE maxrec(d2,size(request->chargeevents[d1.seq].batchicdcodes,5)))
     JOIN (d2)
    ORDER BY chargeeventid
    HEAD REPORT
     loopcnt = 0
    HEAD chargeeventid
     dxsequence = 0
    DETAIL
     loopcnt = (loopcnt+ 1), dxsequence = (dxsequence+ 1)
     IF (mod(loopcnt,50)=1)
      stat = alterlist(chargebatchdtlcdreq->objarray,(loopcnt+ 49))
     ENDIF
     chargebatchdtlcdreq->objarray[loopcnt].charge_batch_detail_id = request->chargeevents[d1.seq].
     chargeeventid, chargebatchdtlcdreq->objarray[loopcnt].type_cd = cs4002321_diag_cd,
     chargebatchdtlcdreq->objarray[loopcnt].parent_entity_id = request->chargeevents[d1.seq].
     batchicdcodes[d2.seq].aliasid,
     chargebatchdtlcdreq->objarray[loopcnt].parent_entity_name = "NOMENCLATURE", chargebatchdtlcdreq
     ->objarray[loopcnt].type_ident = request->chargeevents[d1.seq].batchicdcodes[d2.seq].alias,
     chargebatchdtlcdreq->objarray[loopcnt].active_ind = 1,
     chargebatchdtlcdreq->objarray[loopcnt].priority_seq = dxsequence
    FOOT REPORT
     stat = alterlist(chargebatchdtlcdreq->objarray,loopcnt)
    WITH nocounter
   ;end select
   IF (size(chargebatchdtlcdreq->objarray,5) > 0)
    CALL logmessage("saveChargeEventDetailICDCodes","Insert ICD codes",log_debug)
    EXECUTE afc_da_add_charge_batch_dtl_cd  WITH replace("REQUEST",chargebatchdtlcdreq), replace(
     "REPLY",chargebatchdtlcdrep)
    IF ((chargebatchdtlcdrep->status_data.status != "S"))
     CALL logmessage("saveChargeEventDetailICDCodes","Failed inserting ICD codes",log_debug)
     RETURN(false)
    ELSE
     RETURN(true)
    ENDIF
   ENDIF
   RETURN(true)
   CALL logmessage("saveChargeEventDetailICDCodes","End sub",log_debug)
 END ;Subroutine
#exit_script
 IF (validate(debug,0)=1)
  CALL echorecord(request)
  CALL echorecord(chargebatchdetailreq)
  CALL echorecord(chargebatchdtlcdreq)
  CALL echorecord(chargebatchdetailrep)
  CALL echorecord(chargebatchdtlcdrep)
