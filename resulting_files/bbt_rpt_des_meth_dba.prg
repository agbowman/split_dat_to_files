CREATE PROGRAM bbt_rpt_des_meth:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->des_meths = uar_i18ngetmessage(i18nhandle,"des_meths","DESTRUCTION METHODS")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_des_meth", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  PLAN (cv
   WHERE cv.code_set=1609)
   JOIN (cd
   WHERE cd.code_set=outerjoin(1609)
  ...
  HEAD PAGE
   ...
   col 50, captions->des_meths, row + 2,
