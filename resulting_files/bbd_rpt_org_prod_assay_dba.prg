CREATE PROGRAM bbd_rpt_org_prod_assay:dba
 ...
 SET captions->rpt_title = uar_i18ngetmessage(i18nhandle,"rpt_title",
  "ORGANIZATION AND INVENTORY AREA / TASK ASSAY /")
 ...
 SET captions->rpt_id = uar_i18ngetmessage(i18nhandle,"rpt_id","Report ID: BBD_RPT_ORG_PROD_ASSAY")
 ...
 SELECT INTO "cer_temp:bbdorgprodassay.txt"
  product_disp = uar_get_code_display(p.product_cd), task_assay_disp = uar_get_code_display(t
   .task_assay_cd), t.active_ind,
  ...
  FROM accept_pos_prod_r p,
   ...
   organization o
  PLAN (p
   WHERE p.accept_pos_prod_id > 0)
   JOIN (t
   WHERE t.accept_pos_test_id=p.accept_pos_test_id)
   JOIN (s
   WHERE s.org_shipment_id=t.org_shipment_id)
   JOIN (o
   WHERE o.organization_id=outerjoin(s.organization_id))
  ORDER BY organization, task_assay_disp, product_disp
  HEAD PAGE
   ...
   captions->rpt_org, col 45, captions->rpt_task,
