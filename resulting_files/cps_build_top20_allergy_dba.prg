CREATE PROGRAM cps_build_top20_allergy:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ap
   ...
   WHERE nvp.parent_entity_id=ap.app_prefs_id
    ...
    AND nvp.pvc_name="TOP20_ALLERGY_COUNT")
  HEAD REPORT
 ...
 SET ierrcode = 0
 SELECT INTO "nl:"
  item_knt = count(a.allergy_instance_id), a.updt_id, a.substance_nom_id
  FROM allergy a
  PLAN (a
   WHERE a.updt_id > 0
    AND a.substance_nom_id > 0
    AND a.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
    AND a.beg_effective_dt_tm >= cnvtdatetime(anchor->dt_tm)
    AND a.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)
    AND a.active_ind=true)
  GROUP BY a.updt_id, a.substance_nom_id
  HAVING count(a.allergy_instance_id) >= low_knt
  ORDER BY a.updt_id, item_knt DESC
  HEAD REPORT
  ...
  HEAD a.updt_id
   ...
   ENDIF
   20_knt = 0, nknt = 0, hold->qual[knt].user_id = a.updt_id,
  ...
  DETAIL
   ...
   IF (20_knt <= 20)
    ...
    ENDIF
    hold->qual[knt].nomen[nknt].nomen_id = a.substance_nom_id, hold->qual[knt].nomen[nknt].item_knt
     = item_knt
   ENDIF
  FOOT  a.updt_id
 ...
 SET code_value = 0.0
 SET code_set = 25321
 SET cdf_meaning = "ALLERGY"
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,1,allergy_cd)
 IF (stat > 0)
  SET failed = select_error
 ...
 FOR (i = 1 TO hold->qual_knt)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (nc
     WHERE nc.parent_entity_name="PRSNL"
      ...
      AND nc.category_type_cd=allergy_cd
      AND nc.category_name="TOP_20_ALLERGY")
    DETAIL
   ...
   IF (curqual < 1)
    ...
    INSERT  FROM nomen_category nc
     SET nc.nomen_category_id = hold->qual[i].nomen_category_id, nc.category_name = "TOP_20_ALLERGY",
      nc.category_type_cd = allergy_cd,
