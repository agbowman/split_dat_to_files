CREATE PROGRAM afc_da_upt_charge_mod:dba
 SET afc_da_upt_charge_mod_vrsn = 20171207
 ...
 CALL upt_charge_mod(mndastart,mndastop)
 SUBROUTINE (upt_charge_mod(nstart=i4,nstop=i4) =null WITH protect)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (dt)
     JOIN (c
     WHERE (c.charge_mod_id=request->objarray[dt.seq].charge_mod_id))
    DETAIL
     mndamodrec += 1, mndamodfld = 0, reply->mod_objs[mndamodobj].mod_recs[mndamodrec].table_name =
     msdatablename,
     reply->mod_objs[mndamodobj].mod_recs[mndamodrec].pk_values = build(c.charge_mod_id)
     IF ((validate(request->objarray[dt.seq].charge_item_id,c.charge_item_id) != - (0.00001))
      AND validate(request->objarray[dt.seq].charge_item_id,c.charge_item_id) != c.charge_item_id)
      CALL logfieldmodified("CHARGE_ITEM_ID","F8",build(validate(request->objarray[dt.seq].
        charge_item_id,c.charge_item_id)),build(c.charge_item_id))
     ENDIF
     IF ((validate(request->objarray[dt.seq].charge_mod_type_cd,c.charge_mod_type_cd) != - (0.00001))
      AND validate(request->objarray[dt.seq].charge_mod_type_cd,c.charge_mod_type_cd) != c
     .charge_mod_type_cd)
      CALL logfieldmodified("CHARGE_MOD_TYPE_CD","F8",build(validate(request->objarray[dt.seq].
        charge_mod_type_cd,c.charge_mod_type_cd)),build(c.charge_mod_type_cd))
     ENDIF
     ...
     ELSE
      ...
      ENDIF
     ENDIF
     IF ((validate(request->objarray[dt.seq].charge_mod_source_cd,c.charge_mod_source_cd) != - (
     0.00001))
      AND validate(request->objarray[dt.seq].charge_mod_source_cd,c.charge_mod_source_cd) != c
     .charge_mod_source_cd)
      CALL logfieldmodified("CHARGE_MOD_SOURCE_CD","F8",build(validate(request->objarray[dt.seq].
        charge_mod_source_cd,c.charge_mod_source_cd)),build(c.charge_mod_source_cd))
     ENDIF
    WITH forupdate(c)
   ;end select
   FOR (i = 1 TO size(mrsstatus->objarray,5))
     ...
     ENDIF
   ENDFOR
   UPDATE  FROM charge_mod c,
     (dummyt dt  WITH seq = value(size(request->objarray,5)))
    SET c.charge_item_id =
     ...
     c.active_status_dt_tm = evaluate(validate(request->objarray[dt.seq].active_status_cd,c
      ...
      cnvtdatetime(transinfo->trans_dt_tm)), c.active_status_prsnl_id = reqinfo->updt_id, c
     .charge_mod_source_cd =
