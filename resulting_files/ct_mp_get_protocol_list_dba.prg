CREATE PROGRAM ct_mp_get_protocol_list:dba
 PROMPT
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "ct_mp_get_protocol_list"
 ...
 DECLARE script_begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
 DECLARE max_date = dq8 WITH constant(cnvtdatetime("31-DEC-2100 00:00:00"))
 DECLARE initservicewherestring = vc WITH protect, noconstant(" ")
 DECLARE protstatuswherestring = vc WITH protect, noconstant(" ")
 DECLARE roleprefwherestring = vc WITH protect, noconstant(" ")
 DECLARE protmnemonicinputstring = vc WITH protect, noconstant(" ")
 DECLARE protaliasinputstring = vc WITH protect, noconstant(" ")
 DECLARE protcount = i4 WITH protect, noconstant(0)
 DECLARE invalidcd = f8 WITH protect, constant(uar_get_code_by("MEANING",17274,"INVALID"))
 DECLARE otherinitservccd = f8 WITH protect, constant(uar_get_code_by("MEANING",17273,"OTHER"))
 DECLARE institutioncd = f8 WITH protect, constant(uar_get_code_by("MEANING",17296,"INSTITUTION"))
 DECLARE primarycd = f8 WITH protect, constant(uar_get_code_by("MEANING",17441,"PRIMARY"))
 DECLARE sponsortypecd = f8 WITH protect, constant(uar_get_code_by("MEANING",17271,"PRIMARY"))
 ...
 DECLARE getprotlist(null) = null WITH protect
 DECLARE buildprotstatuswherestring(null) = null WITH protect
 DECLARE buildinitservicewherestring(null) = null WITH protect
 DECLARE buildroleprefwherestring(null) = null WITH protect
 DECLARE getprotocolid(null) = null WITH protect
 DECLARE getamendmentid(null) = null WITH protect
 DECLARE checkprotcount(null) = null WITH protect
 SET reply->status_data.status = "F"
 ...
 IF (log_domain_exists_ind=2)
  ...
  SET log_domain_where_str = "pm.logical_domain_id = domain_reply->logical_domain_id"
 ENDIF
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_protocol_list","Invalid JSON Request",
   reply)
  GO TO exit_script
 ENDIF
 SET stat = cnvtjsontorec( $JSONREQUEST)
 SET protmnemonicinputstring = cnvtupper(cnvtalphanum(request->prot_mnemonic_search_string,3))
 SET protaliasinputstring = cnvtupper(cnvtalphanum(request->prot_alias_search_string,3))
 ...
 IF (org_security_ind=1)
  ...
  SET userorgstr =
  "expand(UserOrgItr, 1, UserOrgSize, pr.ORGANIZATION_ID,	 					  user_org_reply->organizations[UserOrgItr]->organization_id)"
 ELSE
  SET userorgstr = "1=1"
 ENDIF
 CALL echo(build("UserOrgStr: ",userorgstr))
 CALL getprotlist(null)
 SET reply->status_data.status = "S"
 SUBROUTINE getprotlist(null)
   CALL log_message("In GetProtList()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   CALL buildprotstatuswherestring(null)
   SET stat = initrec(ct_get_pref_request)
   SET stat = initrec(ct_get_pref_reply)
   CALL buildinitservicewherestring(null)
   CALL buildroleprefwherestring(null)
   FREE RECORD ct_get_pref_request
   FREE RECORD ct_get_pref_reply
   CALL getprotocolid(null)
   IF (textlen(protmnemonicinputstring) > 0)
    CALL filterbasedonmnemonic(null)
   ENDIF
   IF (textlen(protaliasinputstring) > 0)
    CALL filterbasedonalias(null)
   ENDIF
   CALL getamendmentid(null)
   IF (size(request->primary_sponsor_search_key_list,5) > 0)
    CALL filterbasedonsponsor(null)
   ENDIF
   IF (size(request->prot_pi_search_key_list,5) > 0)
    CALL filterbasedonpi(null)
   ENDIF
   IF (size(request->prot_contact_search_key_list,5) > 0)
    CALL filterbasedoncontacts(null)
   ENDIF
   CALL log_message(build("Exit GetProtList(), Elapsed time in seconds:",datetimediff(cnvtdatetime(
       sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE filterbasedonmnemonic(null)
   CALL log_message("In FilterBasedOnMnemonic()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE mnemonicregex = vc WITH protect, constant(concat("*",protmnemonicinputstring,"*"))
   DECLARE mnemonicfiltercount = i4 WITH protect, noconstant(0)
   DECLARE protocoliditr = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM prot_master pm
    PLAN (pm
     WHERE expand(protocoliditr,1,protcount,pm.prot_master_id,reply->prot_list[protocoliditr].
      protocolid)
      AND cnvtupper(cnvtalphanum(pm.primary_mnemonic,3))=patstring(mnemonicregex,1))
    DETAIL
     mnemonicfiltercount += 1, reply->prot_list[mnemonicfiltercount].protocolid = pm.prot_master_id
    WITH nocounter, orahintcbo(" GATHER_PLAN_STATISTICS ")
   ;end select
   SET protcount = mnemonicfiltercount
   SET stat = alterlist(reply->prot_list,mnemonicfiltercount)
   CALL log_message(build("Exit FilterBasedOnMnemonic(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5),"Protocol count after mnemonic filter: ",
     mnemonicfiltercount),log_level_debug)
   CALL checkprotcount(null)
 END ;Subroutine
 SUBROUTINE filterbasedonalias(null)
   CALL log_message("In FilterBasedOnAlias()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE aliasregex = vc WITH protect, constant(concat("*",protaliasinputstring,"*"))
   DECLARE aliasfiltercount = i4 WITH protect, noconstant(0)
   DECLARE protocoliditr = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM prot_master pm,
     prot_alias pal
    PLAN (pm
     WHERE expand(protocoliditr,1,protcount,pm.prot_master_id,reply->prot_list[protocoliditr].
      protocolid))
     JOIN (pal
     WHERE pal.prot_master_id=pm.prot_master_id
      AND pal.end_effective_dt_tm=cnvtdatetime(max_date)
      AND cnvtupper(cnvtalphanum(pal.prot_alias,3))=patstring(aliasregex,1))
    DETAIL
     aliasfiltercount += 1, reply->prot_list[aliasfiltercount].protocolid = pm.prot_master_id
    WITH nocounter, orahintcbo(" GATHER_PLAN_STATISTICS ")
   ;end select
   SET protcount = aliasfiltercount
   SET stat = alterlist(reply->prot_list,aliasfiltercount)
   CALL log_message(build("Exit FilterBasedOnAlias(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5),"Protocol count after alias filter: ",aliasfiltercount
     ),log_level_debug)
   CALL checkprotcount(null)
 END ;Subroutine
 SUBROUTINE filterbasedonsponsor(null)
   CALL log_message("In FilterBasedOnSponsor()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE sponsorfiltercount = i4 WITH protect, noconstant(0)
   DECLARE sponsorfiltersize = i4 WITH protect, constant(size(request->
     primary_sponsor_search_key_list,5))
   DECLARE sponsorfilteritr = i4 WITH protect, noconstant(0)
   DECLARE amendmentitr = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    ...
    PLAN (pm)
     ...
     WHERE pm.prot_master_id=pa.prot_master_id
      AND expand(amendmentitr,1,protcount,pa.prot_amendment_id,reply->prot_list[amendmentitr].
      amendmentid))
     JOIN (pgs
     WHERE pgs.prot_amendment_id=pa.prot_amendment_id
      AND pgs.primary_secondary_cd=sponsortypecd
      AND expand(sponsorfilteritr,1,sponsorfiltersize,pgs.organization_id,cnvtreal(request->
       primary_sponsor_search_key_list[sponsorfilteritr])))
    ORDER BY cnvtlower(pm.primary_mnemonic), pa.prot_master_id
    HEAD pa.prot_master_id
     sponsorfiltercount += 1, reply->prot_list[sponsorfiltercount].protocolid = pa.prot_master_id,
     reply->prot_list[sponsorfiltercount].amendmentid = pa.prot_amendment_id
    WITH nocounter, orahintcbo(" GATHER_PLAN_STATISTICS ")
   ;end select
   SET protcount = sponsorfiltercount
   SET stat = alterlist(reply->prot_list,sponsorfiltercount)
   CALL log_message(build("Exit FilterBasedOnSponsor(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5),"Protocol count after sponsor filter: ",
     sponsorfiltercount),log_level_debug)
   CALL checkprotcount(null)
 END ;Subroutine
 SUBROUTINE filterbasedonpi(null)
   CALL log_message("In FilterBasedOnPI()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE amendmentitr = i4 WITH protect, noconstant(0)
   DECLARE pifiltercount = i4 WITH protect, noconstant(0)
   DECLARE pifiltersize = i4 WITH protect, constant(size(request->prot_pi_search_key_list,5))
   DECLARE pifilteritr = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM prot_amendment pa,
     prot_role pr,
     prot_master pm
    PLAN (pm)
     ...
     WHERE pr.prot_amendment_id=pa.prot_amendment_id
      AND pr.end_effective_dt_tm=cnvtdatetime(max_date)
      AND pr.prot_role_cd=primarycd
      AND expand(pifilteritr,1,pifiltersize,pr.person_id,cnvtreal(request->prot_pi_search_key_list[
       pifilteritr])))
    ORDER BY cnvtlower(pm.primary_mnemonic), pa.prot_master_id
    HEAD pa.prot_master_id
     pifiltercount += 1, reply->prot_list[pifiltercount].protocolid = pa.prot_master_id, reply->
     prot_list[pifiltercount].amendmentid = pa.prot_amendment_id
    WITH nocounter, orahintcbo(" GATHER_PLAN_STATISTICS ")
   ;end select
   SET protcount = pifiltercount
   SET stat = alterlist(reply->prot_list,pifiltercount)
   CALL log_message(build("Exit FilterBasedOnPI(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5),"Protocol count after principal investigator filter: ",
     pifiltercount),log_level_debug)
   CALL checkprotcount(null)
 END ;Subroutine
 SUBROUTINE filterbasedoncontacts(null)
   CALL log_message("In FilterBasedOnContacts()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE amendmentitr = i4 WITH protect, noconstant(0)
   DECLARE contactsfiltercount = i4 WITH protect, noconstant(0)
   DECLARE contactsfiltersize = i4 WITH protect, constant(size(request->prot_contact_search_key_list,
     5))
   DECLARE contactsfilteritr = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM prot_amendment pa,
     prot_role pr,
     prot_master pm
    PLAN (pm)
     ...
     WHERE pr.prot_amendment_id=pa.prot_amendment_id
      ...
      AND expand(contactsfilteritr,1,contactsfiltersize,pr.person_id,cnvtreal(request->
       prot_contact_search_key_list[contactsfilteritr])))
    ORDER BY cnvtlower(pm.primary_mnemonic), pa.prot_master_id
    HEAD pa.prot_master_id
     contactsfiltercount += 1, reply->prot_list[contactsfiltercount].protocolid = pa.prot_master_id,
     reply->prot_list[contactsfiltercount].amendmentid = pa.prot_amendment_id
    WITH nocounter, orahintcbo(" GATHER_PLAN_STATISTICS ")
   ;end select
   SET protcount = contactsfiltercount
   SET stat = alterlist(reply->prot_list,contactsfiltercount)
   CALL log_message(build("Exit FilterBasedOnContacts(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5),"Protocol count after protocol contact filter: ",
     contactsfiltercount),log_level_debug)
   CALL checkprotcount(null)
 END ;Subroutine
 SUBROUTINE buildprotstatuswherestring(null)
   CALL log_message("In BuildProtStatusWhereString()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   SET ct_get_pref_request->pref_entry = "invalid_display"
   EXECUTE ct_get_pref  WITH replace("REQUEST_STRUCT","CT_GET_PREF_REQUEST"), replace("REPLY",
    "CT_GET_PREF_REPLY")
   IF ((ct_get_pref_reply->pref_value=0))
    SET protstatuswherestring = concat(protstatuswherestring,"pm.prot_status_cd != InvalidCd and ")
   ENDIF
   SET protstatuswherestring = concat(protstatuswherestring," pm.prot_status_cd = cvs.code_value")
   IF ((request->prot_status_cd > 0))
    SET protstatuswherestring = concat(protstatuswherestring," and pm.prot_status_cd = ",cnvtstring(
      request->prot_status_cd))
   ENDIF
   CALL log_message(build("Exit BuildProtStatusWhereString(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE buildinitservicewherestring(null)
   CALL log_message("In BuildInitServiceWhereString()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   SET initservicewherestring = "cvi.active_ind = 1 and pm.initiating_service_cd = cvi.code_value"
   IF ((request->init_service_cd=otherinitservccd))
    SET initservicewherestring = concat("( pm.initiating_service_cd = cvi.code_value and ",
     "cvi.end_effective_dt_tm <= cnvtdatetime(curdate,curtime3)) or ","pm.initiating_service_cd = ",
     cnvtstring(request->init_service_cd)," or ",
     "( cvi.active_ind = 0 and pm.initiating_service_cd = cvi.code_value)")
   ELSEIF ((request->init_service_cd > 0))
    SET initservicewherestring = concat(initservicewherestring," and pm.initiating_service_cd = ",
     cnvtstring(request->init_service_cd))
   ELSEIF (otherinitservccd > 0)
    SET initservicewherestring = "pm.initiating_service_cd = cvi.code_value"
   ENDIF
   CALL log_message(build("Exit BuildInitServiceWhereString(), Elapsed time in seconds:",datetimediff
     (cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE buildroleprefwherestring(null)
   CALL log_message("In BuildRolePrefWhereString()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   SET ct_get_pref_request->pref_entry = "protocol_display"
   EXECUTE ct_get_pref  WITH replace("REQUEST_STRUCT","CT_GET_PREF_REQUEST"), replace("REPLY",
    "CT_GET_PREF_REPLY")
   IF ((ct_get_pref_reply->pref_value=1))
    SET roleprefwherestring = build("(pr2.person_id = ",reqinfo->updt_id," or (pr2.person_id = ",0,
     " and pr2.position_cd = ",
     reqinfo->position_cd,"))")
   ELSE
    SET roleprefwherestring = "1 = 1"
   ENDIF
   CALL log_message(build("Exit BuildRolePrefWhereString(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getprotocolid(null)
   CALL log_message("In GetProtocolID()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   SELECT INTO "nl:"
    FROM prot_amendment pa,
     prot_master pm,
     ...
     prot_role pr2
    PLAN (cvi
     WHERE cvi.code_set=17273)
     JOIN (cvs
     WHERE cvs.active_ind=1
      AND cvs.code_set=17274)
     JOIN (pm
     WHERE parser(initservicewherestring)
      AND parser(protstatuswherestring)
      AND pm.end_effective_dt_tm=cnvtdatetime(max_date)
     ...
     WHERE pr.prot_amendment_id=pa.prot_amendment_id
      ...
      AND pr.prot_role_type_cd=institutioncd)
     JOIN (pr2
     WHERE pr2.prot_amendment_id=pa.prot_amendment_id
      AND pr2.end_effective_dt_tm=cnvtdatetime(max_date)
      AND parser(roleprefwherestring))
    ORDER BY cnvtlower(pm.primary_mnemonic)
    HEAD pm.prot_master_id
     protcount += 1
     IF (size(reply->prot_list,5) < protcount)
      stat = alterlist(reply->prot_list,(protcount+ 9))
     ENDIF
     reply->prot_list[protcount].protocolid = pm.prot_master_id
    WITH nocounter, orahintcbo(" GATHER_PLAN_STATISTICS ")
   ;end select
   SET stat = alterlist(reply->prot_list,protcount)
   CALL log_message(build("Exit GetProtocolID(), Elapsed time in seconds:",datetimediff(cnvtdatetime(
       sysdate),begin_date_time,5)),log_level_debug)
   CALL checkprotcount(null)
 END ;Subroutine
 SUBROUTINE getamendmentid(null)
   CALL log_message("In GetAmendmentID()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE protocoliditr = i4 WITH protect, noconstant(0)
   DECLARE protocolcount = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM prot_amendment pa,
     prot_master pm
    PLAN (pm
    ...
    ORDER BY cnvtlower(pm.primary_mnemonic), pa.amendment_nbr
    HEAD pa.prot_master_id
     protocolcount += 1, reply->prot_list[protocolcount].protocolid = pa.prot_master_id
    DETAIL
     reply->prot_list[protocolcount].amendmentid = pa.prot_amendment_id
    WITH nocounter, expand = 1, orahintcbo(" GATHER_PLAN_STATISTICS ")
   ;end select
   CALL log_message(build("Exit GetAmendmentID(), Elapsed time in seconds:",datetimediff(cnvtdatetime
      (sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE checkprotcount(null)
   IF (protcount <= 0)
    SET reply->status_data.status = "S"
    GO TO exit_script
   ENDIF
...
#exit_script
 ...
 FREE RECORD reply
 SET last_mod = "002"
 SET mod_date = "June 11th, 2020"
 CALL log_message(build("ct_mp_get_protocol_list script ends here, Elapsed time in seconds:",
   datetimediff(cnvtdatetime(sysdate),script_begin_date_time,5)),log_level_debug)
END GO
