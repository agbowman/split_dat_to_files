CREATE PROGRAM cva_get_outbound_tree:dba
 RECORD reply(
   1 qual[*]
     2 inner_qual[*]
       3 level = i4
  ...
  IF (size(reply->qual,5) > 0)
   ...
   WHILE (outidx < outcnt)
     ...
     SELECT INTO "nl:"
      ...
      PLAN (cvo
       WHERE expand(num,nstart,ntotal,cvo.code_value,reply->qual[outidx].inner_qual[num].location_cd)
        AND (cvo.contributor_source_cd=request->contributor_source_cd))
      HEAD cvo.code_value
       aliascnt = 0, inidx = locateval(num,nstart,ntotal,cvo.code_value,reply->qual[outidx].
        inner_qual[num].location_cd)
      DETAIL
       aliascnt += 1, stat = alterlist(reply->qual[outidx].inner_qual[inidx].qual,aliascnt), reply->
       qual[outidx].inner_qual[inidx].qual[aliascnt].alias = cvo.alias,
       reply->qual[outidx].inner_qual[inidx].qual[aliascnt].updt_cnt = cvo.updt_cnt
