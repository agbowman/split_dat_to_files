CREATE PROGRAM dm_oen_si_audit_rows:dba
 ...
 DECLARE actualrows = i4 WITH public, noconstant(0)
 ...
 IF (days_to_keep < 1)
  ...
  GO TO the_end
 ENDIF
 SET reply->table_name = "SI_AUDIT"
 SET reply->rows_between_commit = min(10000,max_rows)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(sa.si_audit_id)
   FROM si_audit sa
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(sa.si_audit_id)
  FROM si_audit sa
  DETAIL
   v_max_id = cnvtreal(maxval)
  WITH nocounter
 ;end select
 SET v_days_back = cnvtlookbehind(build(days_to_keep,",D"))
 SET v_cur_max_id = (v_cur_min_id+ (v_batch_size - 1))
 SET sparserstring = concat(
  "(sa.updt_dt_tm < cnvtdatetime(v_days_back) and (sa.retain_until_dt_tm is null",
  " or sa.retain_until_dt_tm < cnvtdatetime(curdate,curtime3)))")
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    sa.rowid
    FROM si_audit sa
    WHERE parser(sbr_getrowidnotexists(sparserstring,"sa"))
     AND sa.si_audit_id BETWEEN v_cur_min_id AND v_cur_max_id
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,10)=1)
      stat = alterlist(reply->rows,(rows+ 9))
     ENDIF
     reply->rows[rows].row_id = sa.rowid
    WITH nocounter, maxqual(sa,value(v_rows_left))
   ;end select
