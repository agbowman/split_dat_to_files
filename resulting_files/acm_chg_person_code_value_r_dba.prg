CREATE PROGRAM acm_chg_person_code_value_r:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->person_code_value_r_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
  ...
  SET p.person_code_value_r_id = acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].
   person_code_value_r_id, p.beg_effective_dt_tm = cnvtdatetime(acm_request->
    person_code_value_r_qual[xref->chg[d.seq].idx].beg_effective_dt_tm), p.end_effective_dt_tm =
   cnvtdatetime(acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].end_effective_dt_tm),
   p.code_set = acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].code_set, p.code_value =
   acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].code_value, p.contributor_system_cd =
   IF ((acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.person_id =
   IF ((acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].person_id > 0)) acm_request->
    person_code_value_r_qual[xref->chg[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].person_idx].
    person_id
   ENDIF
   , p.active_ind = acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].active_ind, p
   .active_status_cd = acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].active_status_cd,
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.person_code_value_r_id=acm_request->person_code_value_r_qual[xref->chg[d.seq].idx].
   person_code_value_r_id))
  WITH nocounter, status(reply->person_code_value_r_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->person_code_value_r_qual[xref->chg[index].idx].status != 1))
    SET failed = update_error
    SET table_name = "PERSON_CODE_VALUE_R"
    GO TO exit_script
   ENDIF
 ENDFOR
 IF (acm_hist_ind=1)
  EXECUTE acm_chg_person_cd_val_r_hist
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,p.person_code_value_r_id,acm_request->person_code_value_r_qual[xref->
     chg[t1].idx].person_code_value_r_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.person_code_value_r_id,acm_request->person_code_value_r_qual[
      xref->chg[t1].idx].person_code_value_r_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->person_code_value_r_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->person_code_value_r_qual[idx].status = - (1), reply->person_code_value_r_qual[idx].
      person_code_value_r_id = acm_request->person_code_value_r_qual[idx].person_code_value_r_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->person_code_value_r_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->person_code_value_r_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->person_code_value_r_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("CODE_SET,",chg_str)=0)
      acm_request->person_code_value_r_qual[idx].code_set = p.code_set
     ENDIF
     IF (findstring("CODE_VALUE,",chg_str)=0)
      acm_request->person_code_value_r_qual[idx].code_value = p.code_value
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->person_code_value_r_qual[idx].contributor_system_cd = p.contributor_system_cd
     ENDIF
     IF (findstring("PERSON_ID,",chg_str)=0)
      acm_request->person_code_value_r_qual[idx].person_id = p.person_id
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->person_code_value_r_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->person_code_value_r_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->person_code_value_r_qual[xref->chg[index].idx].status=0))
