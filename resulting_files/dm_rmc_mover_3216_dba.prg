CREATE PROGRAM dm_rmc_mover_3216:dba
 ...
 DECLARE cust_type_mean = vc
 DECLARE cust_pathway_catalog_id = f8
 DECLARE cust_new_seq = f8
 DECLARE cust_cnt_rows = i2
 DECLARE cust_desc = vc
 DECLARE cust_version = i2
 DECLARE cust_person_id = f8
 DECLARE cust_src_table = vc
 DECLARE cust_single_trans = i2
 DECLARE cust_trans_val = vc
 DECLARE cust_mult_loop = i4
 DECLARE cust_mult_cnt = i4
 DECLARE cust_version_pw_cat_id = f8
 DECLARE cust_chk_ind = i2
 DECLARE evaluate_id = f8
 DECLARE cust_loc_idx = i4
 DECLARE cust_loc_loop = i4
 DECLARE cust_nomv_chk = vc WITH protect, noconstant("")
 SET cust_chk_ind = 0
 FREE RECORD cust_mult_rs
 RECORD cust_mult_rs(
   1 qual[*]
     2 s_pw_cat_t_id = f8
     2 trans_ind = i2
 ...
 SET cust_pathway_catalog_id = cnvtreal(get_value(cust_tab_name,"PATHWAY_CATALOG_ID","FROM"))
 SET cust_version_pw_cat_id = cnvtreal(get_value(cust_tab_name,"VERSION_PW_CAT_ID","FROM"))
 SET cust_type_mean = get_value(cust_tab_name,"TYPE_MEAN","FROM")
 SET cust_desc = get_value(cust_tab_name,"DESCRIPTION","FROM")
 SET cust_version = cnvtint(get_value(cust_tab_name,"VERSION","FROM"))
 SET cust_person_id = cnvtreal(get_value(cust_tab_name,"REF_OWNER_PERSON_ID","FROM"))
 SET cust_src_table = dm2_get_rdds_tname("PW_CAT_RELTN")
 SET temp_col_cnt = get_col_pos(cust_tab_name,"PATHWAY_CATALOG_ID")
 IF (cust_pathway_catalog_id=cust_version_pw_cat_id)
  SET cust_chk_ind = 1
 ENDIF
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  SET cust_nomv_chk = select_merge_translate(cnvtstring(cust_pathway_catalog_id),cust_tab_name)
  ...
  IF (cust_no_query=1)
   IF (cust_type_mean="PHASE")
    SELECT INTO "NL:"
     FROM (parser(cust_src_table) p)
     WHERE p.pw_cat_t_id=cust_pathway_catalog_id
      AND p.type_mean="GROUP"
     DETAIL
      cust_mult_cnt = (cust_mult_cnt+ 1), stat = alterlist(cust_mult_rs->qual,cust_mult_cnt),
      cust_mult_rs->qual[cust_mult_cnt].s_pw_cat_t_id = p.pw_cat_s_id
     WITH nocounter
    ;end select
    IF (cust_mult_cnt=0)
     SET dm2_ref_data_reply->error_ind = 1
     ...
     GO TO exit_3216
    ENDIF
    FOR (cust_mult_loop = 1 TO cust_mult_cnt)
     SET cust_trans_val = select_merge_translate(cnvtstring(cust_mult_rs->qual[cust_mult_loop].
       s_pw_cat_t_id),"PATHWAY_CATALOG")
     IF (isnumeric(cust_trans_val))
      SET cust_mult_rs->qual[cust_mult_loop].t_pw_cat_t_id = cnvtreal(cust_trans_val)
      SET cust_mult_rs->qual[cust_mult_loop].trans_ind = 1
      SET cust_single_trans = 1
     ELSE
      SET cust_rpt_missing = report_missing("PATHWAY_CATALOG","PATHWAY_CATALOG_ID",cust_mult_rs->
       qual[cust_mult_loop].s_pw_cat_t_id)
      CALL evaluate_rpt_missing(cust_rpt_missing)
     ENDIF
    ENDFOR
    IF (cust_single_trans=0)
     SET cust_err_val = get_err_val(null)
     IF (cust_err_val=0)
      SET dm2_ref_data_reply->error_ind = 1
      SET dm2_ref_data_reply->error_msg =
      "This row wasn't translated because of the PW_CAT_S_ID on the PW_CAT_RELTN table"
      SET current_merges = 0
     ENDIF
     GO TO exit_3216
    ENDIF
    SET cust_loc_idx = locateval(cust_loc_loop,1,dm2_ref_data_doc->tbl_cnt,"PW_CAT_RELTN",
     dm2_ref_data_doc->tbl_qual[cust_loc_loop].table_name)
    IF (cust_loc_idx=0)
     SET cust_loc_loop = temp_tbl_cnt
     SET cust_loc_idx = fill_rs("TABLE","PW_CAT_RELTN")
     SET temp_tbl_cnt = cust_loc_loop
     IF ((cust_loc_idx=- (2)))
      SET drdm_log_loop = redirect_to_start_row(drdm_chg,drdm_log_loop,drdm_next_row_pos,
       drdm_max_rs_size)
      GO TO exit_3216
    ...
    ENDIF
    FOR (cust_mult_loop = 1 TO cust_mult_cnt)
      IF ((cust_mult_rs->qual[cust_mult_loop].trans_ind=1))
       SET cust_cnt_rows = 0
       SELECT INTO "NL:"
        FROM pathway_catalog p,
         pw_cat_reltn r
        PLAN (p
         WHERE p.description=cust_desc
          AND p.version=cust_version
          AND p.ref_owner_person_id=cust_person_id)
         JOIN (r
         WHERE (r.pw_cat_s_id=cust_mult_rs->qual[cust_mult_loop].t_pw_cat_t_id)
          AND r.pw_cat_t_id=p.pathway_catalog_id
          AND r.type_mean="GROUP"
          AND  NOT (list(r.pw_cat_s_id,r.pw_cat_t_id,r.type_mean) IN (
         (SELECT
          pw_cat_s_id, pw_cat_t_id, type_mean
          FROM pw_cat_reltn7585$r
          WHERE rdds_delete_ind=1
           AND rdds_status_flag < 9000
           AND (rdds_source_env_id=dm2_ref_data_doc->env_source_id)))))
        DETAIL
         cust_val = r.pw_cat_t_id, cust_cnt_rows = (cust_cnt_rows+ 1), stat = alterlist(
          ui_query_eval_rec->qual,cust_cnt_rows),
         ui_query_eval_rec->qual[cust_cnt_rows].root_entity_attr = r.pw_cat_t_id
        WITH nocounter
       ;end select
       SET evaluate_id = evaluate_exec_ui_query(cust_cnt_rows,temp_tbl_cnt,perm_col_cnt)
       IF (evaluate_id < 0)
        IF ((evaluate_id=- (2)))
         SET dm2_ref_data_reply->error_ind = 1
         ...
         SET current_merges = 0
         SET cust_mult_loop = cust_mult_cnt
        ENDIF
       ELSE
        CALL put_value(cust_tab_name,"PATHWAY_CATALOG_ID",cnvtstring(cust_val))
        SET cust_mult_loop = cust_mult_cnt
       ENDIF
      ENDIF
    ENDFOR
    SET cust_idcd_check = is_translated(cust_tab_name,"PATHWAY_CATALOG_ID")
    IF ((dm2_ref_data_reply->error_ind=1))
     GO TO exit_3216
    ELSEIF (cust_idcd_check=0)
     SET cust_new_seq = get_seq(cust_tab_name,"PATHWAY_CATALOG_ID")
     CALL put_value(cust_tab_name,"PATHWAY_CATALOG_ID",cnvtstring(cust_new_seq))
    ENDIF
   ELSE
    SET cust_val = query_target(temp_tbl_cnt,perm_col_cnt)
    IF (cust_val < 0)
     IF ((cust_val=- (3)))
      SET cust_val = get_seq(cust_tab_name,"PATHWAY_CATALOG_ID")
      IF (cust_val > 0)
       CALL put_value(cust_tab_name,"PATHWAY_CATALOG_ID",cnvtstring(cust_val))
       SET current_merges = (current_merges+ 1)
       SET child_merge_audit->num[current_merges].action = "NEWSEQ"
       SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  PATHWAY_CATALOG_ID")
      ELSE
       GO TO exit_3216
      ENDIF
     ELSE
  ...
  SET cust_from_cd = get_value(cust_tab_name,"PATHWAY_CATALOG_ID","FROM")
  IF ((dm2_ref_data_reply->error_ind=1))
   GO TO exit_3216
  ELSE
   SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
  ENDIF
  SET cust_new_cd = get_value(cust_tab_name,"PATHWAY_CATALOG_ID","TO")
  IF ((dm2_ref_data_reply->error_ind=1))
   GO TO exit_3216
  ELSE
 ...
 IF (cust_chk_ind=1)
  IF (is_translated(cust_tab_name,"PATHWAY_CATALOG_ID")=1
   AND is_translated(cust_tab_name,"VERSION_PW_CAT_ID")=0)
   SET cust_new_cd = get_value(cust_tab_name,"PATHWAY_CATALOG_ID","TO")
   CALL put_value(cust_tab_name,"VERSION_PW_CAT_ID",cust_new_cd)
  ENDIF
 ENDIF
