CREATE PROGRAM dm_bmdi_cqm_result_rows:dba
 ...
 END ;Subroutine
 DECLARE v_hours_to_keep = i4 WITH protect, noconstant(0)
 ...
 SET reply->table_name = "CQM_BMDI_RESULTS_QUE"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(cbrq.queue_id)
    FROM cqm_bmdi_results_que cbrq
    WHERE cbrq.queue_id > 0
    DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT
     IF (v_hours_to_keep=0)
      WHERE cbrq.queue_id BETWEEN curminid AND curmaxid
     ELSE
      WHERE parser(sbr_getrowidnotexists("cbrq.queue_id between curMinID and curMaxID","cbrq"))
       AND cbrq.updt_dt_tm < cnvtlookbehind(concat(trim(cnvtstring(v_hours_to_keep)),",H"))
     ENDIF
     INTO "nl:"
     cbrq.rowid
     FROM cqm_bmdi_results_que cbrq
     DETAIL
      ...
      ENDIF
      reply->rows[rows].row_id = cbrq.rowid
     WITH nocounter, maxqual(cbrq,value(rowsleft))
    ;end select
