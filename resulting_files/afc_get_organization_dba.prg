CREATE PROGRAM afc_get_organization:dba
 SET afc_get_organization_version = 007
 ...
 RECORD reply(
   ...
   1 organization[*]
     ...
     2 bill_org_payor_qual = i4
     2 bill_org_qual[*]
       3 org_payor_id = f8
       3 bill_org_type_cd = f8
       3 bill_org_type_disp = c40
       3 bill_org_type_desc = c40
       3 bill_org_type_mean = c12
       3 bill_org_type_id = f8
       3 bill_org_type_string = c50
       3 bill_org_type_ind = i2
       3 interface_file_cd = f8
       3 priority = i4
       3 active_ind = i2
 ...
 SET org_id = 0
 DECLARE lnumoforgs = i4 WITH public, noconstant(0)
 ...
 ENDIF
 IF ((((reqinfo->updt_task=951005)) OR ((reqinfo->updt_task=951060))) )
  SET maxqualrows = 9999
 ELSE
  SET maxqualrows = 10
 ENDIF
 SET code_set = 48
 SET cdf_meaning = "ACTIVE"
 SET stat = alterlist(reply->organization,(count1+ 10))
 SET count1 = 1
 SET x = 1
 SELECT
  IF (trim(cnvtupper(cnvtnlssort(request->orgname)))="A")
   PLAN (o
    WHERE o.active_ind=true
     AND cnvtnlssort(o.org_name_key) >= cnvtnlssort(cnvtalphanum(trim(cnvtupper(request->orgname),3))
     )
  ...
  ENDIF
  INTO "nl:"
  o.org_name, o.organization_id, o.org_name_key
  FROM organization o,
   code_value cv,
   org_type_reltn otr
  ORDER BY o.org_name_key, o.organization_id
  DETAIL
   IF (count1 <= maxqualrows)
    lnumoforgs += 1, stat = alterlist(reply->organization,count1), reply->organization[count1].
    org_name = o.org_name,
    reply->organization[count1].organization_id = o.organization_id, count1 += 1
   ENDIF
  WITH nocounter, maxqual(o,value(maxqualrows))
 ;end select
 SET reply->organization_qual = lnumoforgs
 SET stat = alterlist(reply->organization,lnumoforgs)
 ...
 ELSE
  SELECT INTO "nl:"
   ...
   FROM bill_org_payor b,
    (dummyt d1  WITH seq = value(reply->organization_qual))
   PLAN (d1)
    JOIN (b
    WHERE (b.organization_id=reply->organization[d1.seq].organization_id)
     AND b.active_ind=1
     AND b.beg_effective_dt_tm <= cnvtdatetime(sysdate)
     AND b.end_effective_dt_tm >= cnvtdatetime(sysdate))
   DETAIL
    count2 += 1, stat = alterlist(reply->organization[d1.seq].bill_org_qual,count2), reply->
    organization[d1.seq].bill_org_qual[count2].org_payor_id = b.org_payor_id,
    reply->organization[d1.seq].bill_org_qual[count2].bill_org_type_cd = b.bill_org_type_cd, reply->
    organization[d1.seq].bill_org_qual[count2].bill_org_type_string = b.bill_org_type_string, reply->
    organization[d1.seq].bill_org_qual[count2].bill_org_type_ind = b.bill_org_type_ind,
    reply->organization[d1.seq].bill_org_qual[count2].bill_org_type_id = b.bill_org_type_id, reply->
    organization[d1.seq].bill_org_qual[count2].interface_file_cd = b.interface_file_cd, reply->
    organization[d1.seq].bill_org_qual[count2].priority = b.priority,
    reply->organization[d1.seq].bill_org_payor_qual = count2
   WITH nocounter
  ;end select
 ENDIF
 IF (curqual=0)
