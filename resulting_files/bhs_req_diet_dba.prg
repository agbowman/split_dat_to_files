CREATE PROGRAM bhs_req_diet:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   orders->admit_diagnosis = e.reason_for_visit, orders->med_service = uar_get_code_display(e
    .med_service_cd),
   CALL echo(build2("facility: ",uar_get_code_display(e.loc_facility_cd))),
   CALL echo(build2("facility_cd: ",e.loc_facility_cd)),
   CALL echo(build2("bmc_cd: ",trim(cnvtstring(mf_bmc_cd))," bmc_psych_cd: ",trim(cnvtstring(
      mf_bmc_psych_cd))))
   IF ( NOT (e.loc_facility_cd IN (mf_bmc_cd, mf_bmc_psych_cd)))
    mn_print_ind = 1
   ENDIF
 ...
 IF ((orders->spoolout_ind=1))
  CALL echo(build2("print ind: ",mn_print_ind))
  IF (mn_print_ind=1)
   ...
   SELECT INTO value(request->printer_name)
      ...
      WHILE (tempstring > " "
       AND limit < 1000)
        ...
        IF (lfloc=0
         ...
         xrow = (xrow+ 12), tempstring = substring((pos+ 1),9999,tempstring)
        ELSE
         IF (((crloc < lfloc
          AND crloc > 0) OR (lfloc=0)) )
          printstring = substring(1,(crloc - 1),printstring),
          CALL print(calcpos(xcol,xrow)), printstring,
          row + 1, xrow = (xrow+ 12), tempstring = substring((crloc+ 2),9999,tempstring)
         ELSEIF (((lfloc < crloc
          AND lfloc > 0) OR (crloc=0)) )
          printstring = substring(1,(lfloc - 1),printstring),
          CALL print(calcpos(xcol,xrow)), printstring,
          row + 1, xrow = (xrow+ 12), tempstring = substring((lfloc+ 2),9999,tempstring)
         ENDIF
    ...
    DETAIL
     FOR (vv = 1 TO value(order_cnt))
       ...
       CALL print(calcpos(xcol,xrow)), "{b/14}Order Comments", xrow = (xrow+ 10)
       IF ((orders->qual[vv].com_ln_cnt > 0))
        ...
        ENDFOR
       ENDIF
       xrow = (xrow+ 20), xcol = 50,
       CALL print(calcpos(xcol,xrow)),
       "{b/4}Acct#: ", "{BCR/100}{FR/0}{CPI/6}{F/28/2}", orders->fnbr_barcode,
       row + 1, "{f/8}{cpi/12}", row + 1,
       xrow = (xrow+ 10), xcol = 370,
       CALL print(calcpos(xcol,xrow)),
       "{cpi/14}BHS_REQ_DIET ", orders->fnbr_barcode, row + 1
     ENDFOR
    WITH nocounter, maxrow = 800, maxcol = 750,
     dio = postscript
   ;end select
  ELSE
   CALL echo("send output to inbox for BMC, BMCINPTPSYCH")
   SET new_timedisp = cnvtstring(curtime3)
   SET ms_filename = build2("bhs_req_diet_",trim(new_timedisp),".dat")
   ...
   SELECT INTO value(ms_filename)
    ...
    PLAN (d1)
    HEAD REPORT
     col 42, orders->facility, row + 1,
     col 55, "Order Requisition", row + 1,
     col 1, "Patient Information", row + 1,
     col 1, "Patient: ", orders->name,
     row + 1, col 70, "MRN#:",
     orders->mrn
     IF (temp_cd > 0)
      temp_loc_disp = uar_get_code_display(temp_cd), row + 1, col 1,
     ...
     ELSE
      ...
      orders->location
     ENDIF
     col 70, "CMRN#: ", orders->cmrn,
     row + 1, col 1, "Attending MD: ",
     orders->attending, col 70, "Acct#: ",
     orders->fnbr, row + 1, col 1,
     "LOS: ", orders->los, " Days",
     col 40, "Admit Date: ", orders->admit_dt,
     col 70, "Patient Type: ", orders->pat_type,
     row + 1, col 1, "Allergies:"
     IF ((allergy->line_cnt > 0))
      FOR (zz = 1 TO allergy->line_cnt)
        IF (zz > 1)
         row + 1
        ENDIF
        col 10, allergy->line_qual[zz].line
      ENDFOR
     ENDIF
    DETAIL
     FOR (vv = 1 TO value(order_cnt))
       row + 1, col 1, "Procedure: ",
       orders->qual[vv].mnemonic, col 70, "Order Status: ",
       orders->qual[vv].status, row + 1, col 1,
       "Order ID: ", orders->qual[vv].order_id
       FOR (fsub = 1 TO 31)
         FOR (ww = 1 TO orders->qual[vv].d_cnt)
           IF ((((orders->qual[vv].d_qual[ww].group_seq=fsub)) OR (fsub=31
            AND (orders->qual[vv].d_qual[ww].print_ind=0))) )
            IF ((orders->qual[vv].d_qual[ww].value > " "))
             ...
             value, row + 1,
             col 1, orders->qual[vv].d_qual[ww].label_text, ":",
             col 50, tempstring, footer_idx = vv
            ENDIF
           ENDIF
         ENDFOR
       ENDFOR
       row + 1, col 1, "Ordering MD: ",
       orders->qual[vv].order_dr, col 70, "Order Date/Time: ",
       orders->qual[vv].signed_dt, row + 1, col 1,
       "Order Type: ", orders->qual[footer_idx].type, row + 1,
       col 1, "Order Entered by: ", orders->qual[footer_idx].enter_by,
       col 70, "Printed Date/Time: ", curdate,
       " ", curtime, row + 1,
       col 1, "Order Comments"
       IF ((orders->qual[footer_idx].com_ln_cnt > 0))
        ...
        FOR (w = 1 TO orders->qual[footer_idx].com_ln_cnt)
          col 1, orders->qual[footer_idx].com_ln_qual[w].com_line, row + 1
        ENDFOR
       ENDIF
       row + 1, col 1, "Acct:# ",
       orders->fnbr_barcode, col 70, "BHS_REQ_DIET",
       orders->fnbr_barcode
     ENDFOR
    WITH nocounter, maxrow = 800, maxcol = 750
   ;end select
   ...
   SET subject = concat("bhs_req_diet - ",orders->name)
   SET dclcom = concat('mail -s "',subject,'" ',"dietorders@baystatehealth.org < ",ms_filename)
...
#exit_script
 ...
 FREE RECORD pt
 FREE RECORD problem
 SET last_mod = "011 09/11/17 NA033934 SR 417074720 remove some of the requested fields"
END GO
