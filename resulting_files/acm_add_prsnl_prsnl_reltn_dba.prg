CREATE PROGRAM acm_add_prsnl_prsnl_reltn:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->prsnl_prsnl_reltn_qual[xref->add[index].idx].prsnl_prsnl_reltn_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(prsnl_seq,nextval)
     FROM dual
     DETAIL
      acm_request->prsnl_prsnl_reltn_qual[xref->add[index].idx].prsnl_prsnl_reltn_id = cnvtreal(
       nextseqnum), reply->prsnl_prsnl_reltn_qual[xref->add[index].idx].prsnl_prsnl_reltn_id =
      cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->prsnl_prsnl_reltn_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->prsnl_prsnl_reltn_qual[xref->add[index].idx].prsnl_prsnl_reltn_id = acm_request->
    prsnl_prsnl_reltn_qual[xref->add[index].idx].prsnl_prsnl_reltn_id
   ENDIF
   SET reply->prsnl_prsnl_reltn_qual[xref->add[index].idx].status = 0
   IF ((acm_request->prsnl_prsnl_reltn_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->prsnl_prsnl_reltn_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->prsnl_prsnl_reltn_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->prsnl_prsnl_reltn_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   prsnl_prsnl_reltn p
  SET p.prsnl_prsnl_reltn_id = acm_request->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].
   ...
   IF ((acm_request->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.data_status_cd = acm_request->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].data_status_cd, p
   .data_status_prsnl_id = acm_request->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].
   data_status_prsnl_id, p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_prsnl_reltn_qual[
    xref->add[d.seq].idx].end_effective_dt_tm),
   p.organization_id = acm_request->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].organization_id, p
   .person_id = acm_request->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].person_id, p
   .prsnl_prsnl_reltn_cd = acm_request->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].
   prsnl_prsnl_reltn_cd,
   p.related_person_id = acm_request->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].related_person_id,
   p.data_status_cd =
   IF ((acm_request->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request
    ->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].data_status_cd
   ELSE reqdata->data_status_cd
   ENDIF
   , p.data_status_dt_tm = cnvtdatetime(curdate,curtime3),
   p.data_status_prsnl_id = reqinfo->updt_id, p.active_ind = 1, p.active_status_cd = reqdata->
   active_status_cd,
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->prsnl_prsnl_reltn_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->prsnl_prsnl_reltn_qual[xref->add[index].idx].status=0))
