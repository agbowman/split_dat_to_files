CREATE PROGRAM dm_dcp_sticky_note_rows:dba
 ...
 DECLARE token_ndx = i4 WITH protect, noconstant(0)
 DECLARE retention_days = i4 WITH protect, noconstant(- (1))
 DECLARE interval = vc WITH protect, noconstant("")
 ...
 SET reply->table_name = "STICKY_NOTE"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 FOR (token_ndx = 1 TO size(request->tokens,5))
   CASE (request->tokens[token_ndx].token_str)
    OF "RETENTIONDAYS":
     SET retention_days = ceil(cnvtreal(request->tokens[token_ndx].value))
   ENDCASE
 ENDFOR
 IF (retention_days < 14)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"RETENTIONDAYS",
   "You must keep at least 14 days' worth of data.  You entered %1 or did not enter any value.","i",
   retention_days)
  GO TO exit_script
 ENDIF
 SET interval = build(retention_days,"D")
 SET target_dt_tm = cnvtlookbehind(interval,cnvtdatetime(curdate,curtime3))
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(sn.sticky_note_id)
   FROM sticky_note sn
   WHERE sn.sticky_note_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM sticky_note sn
    WHERE parser(sbr_getrowidnotexists("sn.sticky_note_id between curMinID and curMaxID","sn"))
     AND sn.end_effective_dt_tm < cnvtdatetime(target_dt_tm)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,100)=1)
      stat = alterlist(reply->rows,(rows+ 99))
     ENDIF
     reply->rows[rows].row_id = sn.rowid
    WITH nocounter, maxqual(sn,value(rowsleft))
   ;end select
