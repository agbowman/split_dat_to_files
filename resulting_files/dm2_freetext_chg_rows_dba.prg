CREATE PROGRAM dm2_freetext_chg_rows:dba
 ...
 DECLARE ddcr_dup_cnt = i4
 DECLARE ddcr_dup_loop = i4
 DECLARE ddcr_del_cnt = i4
 DECLARE ddcr_del_loop = i4
 DECLARE ddcr_error_ind = i2
 DECLARE ddcr_free_text = f8
 DECLARE ddcr_max_log = f8
 DECLARE ddcr_min_log = f8
 FREE RECORD del_holder
 RECORD del_holder(
   1 qual[*]
     2 log_id = f8
     2 d_ind = i2
 ...
 IF (check_logfile("dm2_freetext_",".log","DM2_FREETEXT LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_freetext_chg_rows"
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "NL:"
  FROM code_value c
  WHERE c.code_set=396
   AND c.display_key="FREETEXT"
  DETAIL
   ddcr_free_text = c.code_value
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error selecting free text code_value"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
 ...
 IF (ddcr_free_text > 0)
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  SET stat = alterlist(del_holder->qual,0)
  SET dm_err->eproc = "Deleting 'FREETEXT' rows from DM_CHG_LOG"
  CALL disp_msg("",dm_err->logfile,0)
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
  EXECUTE dm_readme_status
  SET ddcr_del_cnt = 0
  WHILE (ddcr_min_log <= ddcr_max_log)
    CALL echo("_________ ")
    CALL echo("New Batch")
    CALL echo(concat("Processing 1000 rows starting with log_id ",trim(cnvtstring(ddcr_min_log))))
    SET stat = alterlist(del_holder->qual,0)
    SET ddcr_del_cnt = 0
    SELECT INTO "NL:"
     FROM dm_chg_log d
     WHERE cnvtupper(d.table_name) IN ("ADDRESS", "PHONE", "ORGANIZATION")
      AND d.log_type="REFCHG"
      AND d.log_id >= ddcr_min_log
      AND (d.log_id < (ddcr_min_log+ 1000))
     HEAD REPORT
      ddcr_del_cnt = 0, stat = alterlist(del_holder->qual,1000)
     DETAIL
      ddcr_del_cnt = (ddcr_del_cnt+ 1)
      IF (ddcr_del_cnt > 1000)
       CALL cancel(1)
      ENDIF
      IF (mod(ddcr_del_cnt,1000)=1
       AND ddcr_del_cnt != 1)
       stat = alterlist(del_holder->qual,(ddcr_del_cnt+ 999))
      ENDIF
      del_holder->qual[ddcr_del_cnt].log_id = d.log_id, del_holder->qual[ddcr_del_cnt].pk_where = d
      .pk_where, del_holder->qual[ddcr_del_cnt].table_name = d.table_name
     FOOT REPORT
      stat = alterlist(del_holder->qual,ddcr_del_cnt)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Error selecting address rows from dm_chg_log"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ROLLBACK
     SET readme_data->status = "F"
     SET readme_data->message = dm_err->emsg
     GO TO exit_script
    ELSE
     FOR (ddcr_del_loop = 1 TO ddcr_del_cnt)
      EXECUTE dm2_freetext_chg_child ddcr_del_loop
      IF (ddcr_error_ind=1)
       SET ddcr_del_loop = ddcr_del_cnt
       SET ddcr_min_log = ddcr_max_log
      ENDIF
     ENDFOR
    ENDIF
    SET ddcr_min_log = (ddcr_min_log+ 1000)
  ENDWHILE
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "There is no free text code_value"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
 ...
 IF (ddcr_error_ind=1)
  GO TO exit_script
 ELSE
  SET readme_data->message = "Finished deleting all freetext rows in DM_CHG_LOG"
