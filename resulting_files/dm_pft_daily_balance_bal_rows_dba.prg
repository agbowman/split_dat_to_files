CREATE PROGRAM dm_pft_daily_balance_bal_rows:dba
 ...
 SET reply->table_name = "PFT_DAILY_BALANCE_BAL"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(pdbal.pft_daily_balance_bal_id)
   FROM pft_daily_balance_bal pdbal
   WHERE pdbal.pft_daily_balance_bal_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(pdbal.pft_daily_balance_bal_id)
  FROM pft_daily_balance_bal pdbal
  WHERE pdbal.pft_daily_balance_bal_id > 0.0
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM pft_daily_balance_bal pdbal
    WHERE pdbal.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND pdbal.pft_daily_balance_bal_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("pdbal.pft_daily_balance_bal_id != 0","pdbal"))
    DETAIL
     row_cnt = (row_cnt+ 1)
     IF (mod(row_cnt,100)=1)
      stat = alterlist(reply->rows,(row_cnt+ 99))
     ENDIF
     reply->rows[row_cnt].row_id = pdbal.rowid
    WITH nocounter, maxqual(pdbal,value(rowsleft))
   ;end select
