CREATE PROGRAM dm2_shc_rdbms_cnct_opt:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsr_err_status = c1 WITH protect, noconstant(" ")
 DECLARE dsr_key = vc WITH protect, noconstant(" ")
 DECLARE dsr_get_cnct_opt(null) = null
 IF (check_logfile("DM2_SHC_RDBMS",".log","DM2_SHC_RDBMS_CNCT_OPT LOG FILE")=0)
  SET dsr_err_status = "F"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "System Health Check working on RDBMS Connect Option..."
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF (cursys="AIX")
  SET dm_err->disp_dcl_err_ind = 0
  IF (dm2_push_dcl(concat("ps -fu oracle | grep -i ",dshc_env_info->database_name))=0)
   IF ((dm_err->err_ind=1))
    SET dsr_err_status = "E"
   ELSE
    IF ((dm_err->errtext=" "))
     SET dsr_err_status = "G"
    ELSE
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dsr_err_status = "E"
   ...
   ENDIF
  ELSE
   CALL dsr_get_cnct_opt(null)
  ENDIF
 ELSEIF (cursys="AXP")
  IF (dm2_push_dcl(concat("sh sys/proc=\*",dshc_env_info->database_name,"\*pmon\*"))=0)
   SET dsr_err_status = "E"
  ELSE
   FREE DEFINE rtl2
   FREE SET file_loc
   ...
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   IF (check_error("Check if any process found.") > 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dsr_err_status = "E"
   ELSEIF (curqual=0)
    SET dsr_err_status = "G"
  ...
  ENDIF
 ENDIF
 IF (dshc_add_check_to_rs("RDBMS CONNECT OPTION","Rdbms Connect Option",0,dsr_err_status,
  dshc_ctf_millennium)=0)
  SET dsr_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dsr_err_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 ...
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Rdbms Connect Option"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Rdbms Connect Option Set:"
 ...
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "Application Servers running on the same node as its corresponding Oracle database,",
  " should not be connecting to CCL through the Oracle listener.")
#exit_script
 ...
 IF (dsr_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_RDBMS_CNCT_OPT WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_RDBMS_CNCT_OPT WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_RDBMS_CNCT_OPT COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_RDBMS")
 SUBROUTINE dsr_get_cnct_opt(null)
   SET dsr_key = concat("\database\",dshc_env_info->database_name,"\node\",dshc_env_info->node_name)
   IF ((dm_err->debug_flag >= 1))
    CALL echo(dsr_key)
   ENDIF
   SET dshc_lreg_getp->key_name = dsr_key
   SET dshc_lreg_getp->propname = '"Rdbms Connect Option"'
   SET dshc_lreg_getp->value_type = 2
   IF (dshc_lreg_getp(null)=0)
    SET dsr_err_status = "E"
   ENDIF
   IF ((dm_err->debug_flag >= 1))
    CALL echo(build("propvalue =",dshc_lreg_getp->propvalue))
   ENDIF
   IF (dsr_err_status=" ")
    IF ((dshc_lreg_getp->propvalue > " "))
     SET dsr_err_status = "G"
    ELSE
     SET dsr_err_status = "R"
    ENDIF
   ENDIF
 END ;Subroutine
END GO
