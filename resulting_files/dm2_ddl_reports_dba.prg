CREATE PROGRAM dm2_ddl_reports:dba
 ...
 SUBROUTINE der_compile_oos_pkg(dcop_src_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_ddl_reports",".log","dm2_ddl_reports")=false)
  GO TO exit_program
 ENDIF
 DECLARE ddl_rpt_group_by_status(null) = i2
 DECLARE ddl_rpt_by_status(null) = i2
 DECLARE ddl_get_managed(null) = i2
 DECLARE ddl_get_addt_data(null) = i2
 DECLARE ddr_prog_exit = i2 WITH protect, noconstant(0)
 DECLARE ddr_install_criteria = vc WITH protect, noconstant("")
 DECLARE ddr_total_type = vc WITH protect, noconstant("")
 DECLARE ddr_error_str = vc WITH protect, noconstant("")
 DECLARE ddr_status_col_size = i4 WITH protect, noconstant(0)
 DECLARE ddr_op_type_col_size = i4 WITH protect, noconstant(0)
 DECLARE ddr_priority_col_size = i4 WITH protect, noconstant(0)
 DECLARE ddr_count_col_size = i4 WITH protect, noconstant(0)
 DECLARE ddr_table_col_size = i4 WITH protect, noconstant(0)
 DECLARE ddr_logfile_col_size = i4 WITH protect, noconstant(0)
 DECLARE ddr_dbname = vc WITH protect, noconstant(" ")
 DECLARE ddr_w_msg = vc WITH protect, noconstant(" ")
 DECLARE ddr_mng_msg = vc WITH protect, noconstant("* High Risk DDL")
 DECLARE ddr_temp_dir = vc WITH protect, noconstant("")
 SET ddr_w_msg = "* Estimated time calculated with insufficient schema operations history."
 IF ((dm_err->debug_flag=722))
  SET message = nowindow
 ELSE
  SET message = window
 ENDIF
 CALL clear(1,1)
 SET ddr_dbname = currdbname
 IF ((dm2rpting->prompt_mode="NOPROMPT"))
  SET dm_err->eproc = "Validating report requirements"
  IF ((dm2rpting->run_id=0))
   SET ddr_error_str = concat("RUN_ID (",trim(cnvtstring(dm2rpting->run_id)),
    ") entered is not valid.")
  ELSEIF ((dm2rpting->report_option="DM2NOTSET"))
   SET ddr_error_str = concat("REPORTING_OPTION: (",dm2rpting->report_option,
    ") entered is not valid. ")
  ELSEIF ((dm2rpting->install_mode="SCHEMA_DATE")
   AND (dm2rpting->schema_date=" "))
   SET ddr_error_str = concat("SCHEMA_DATE (",trim(dm2rpting->schema_date),") entered is not valid.")
  ELSEIF ((dm2rpting->install_mode="PACKAGE")
   AND (dm2rpting->package_number=0))
   SET ddr_error_str = concat("PACKAGE_NUMBER (",trim(cnvtstring(dm2rpting->package_number)),
    ") entered is not valid.")
  ELSEIF ((dm2rpting->install_mode="BATCH_INSTALL_PLAN")
   AND (dm2rpting->package_number > 0))
   SET ddr_error_str = concat("BATCH_INSTALL_PLAN (",trim(cnvtstring(dm2rpting->package_number)),
    ") entered is not valid.")
  ENDIF
  IF (ddr_error_str != "")
   CALL text(11,50,ddr_error_str)
   SET message = nowindow
   SET dm_err->err_ind = 1
   SET dm_err->emsg = ddr_error_str
   GO TO exit_program
  ENDIF
  IF (ddl_get_addt_data(null)=0)
   GO TO exit_program
  ENDIF
  IF (ddr_max_col_size(null)=0)
   GO TO exit_program
  ENDIF
  IF (ddl_get_managed(null)=0)
   GO TO exit_program
  ENDIF
  CASE (cnvtupper(dm2rpting->report_option))
   OF "DDL SUMMARY":
    IF ((dm2rpting->status_criteria="DM2NOTSET"))
     SET dm2rpting->status_criteria = "1 = 1"
    ENDIF
    CALL ddl_rpt_group_by_status(null)
   OF "DDL RUNNING":
    IF ((dm2rpting->status_criteria="DM2NOTSET"))
     SET dm2rpting->status_criteria = "ddol.status = 'RUNNING' "
    ENDIF
    CALL ddl_rpt_by_status(null)
   OF "DDL FAILED":
    IF ((dm2rpting->status_criteria="DM2NOTSET"))
     SET dm2rpting->status_criteria = concat(
      " not (ddol.status = 'ERROR' and substring(1,14,ddol.error_msg) = 'Application Id'",
      " and ddol.ready_to_run_ind = 0) and ddol.status = 'ERROR'")
    ENDIF
    CALL ddl_rpt_by_status(null)
   ELSE
    IF ((dm2rpting->status_criteria="DM2NOTSET"))
 ...
 ENDIF
 IF ((dm2rpting->run_id=0))
  SET dm_err->eproc = "Prompting for Install Type."
  IF ((dm_err->debug_flag > 721))
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  ENDIF
  IF (dcprom_ddlops_install_type(null)=0)
   GO TO exit_program
  ELSEIF ((dm2comprom->install_mode="DM2NOTSET"))
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Refreshing report information."
  IF ((dm_err->debug_flag > 721))
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  ENDIF
  CALL dm2rpt_refresh_from_dm2comprom(null)
 ENDIF
 IF (ddl_get_addt_data(null)=0)
  GO TO exit_program
 ENDIF
 ...
 IF (ddl_get_managed(null)=0)
  GO TO exit_program
 ENDIF
 WHILE (ddr_prog_exit=0)
   CALL clear(1,1)
   CALL text(1,1,"INSTALLATION REPORTING PROMPT")
   CALL box(2,1,20,130)
   CALL box(2,1,4,130)
   CALL text(3,2,concat("DATA MANAGEMENT INSTALLATION REPORTING               DATABASE: ",ddr_dbname)
    )
   CALL text(5,3,"1.   View Summary of Operation Types by Status")
   CALL text(6,3,"2.   View all Running Schema Operations ")
   CALL text(7,3,"3.   View all Failed Schema Operations ")
   CALL text(8,3,"4.   View all Schema Operations")
   CALL text(9,3,"5.   View all Schema Operations by Table")
   CALL text(11,6,"Your Selection? (99 to Exit)")
   CALL accept(11,35,"9(2)",99
    WHERE curaccept IN (1, 2, 3, 4, 5,
    99))
   CASE (curaccept)
    OF 1:
     SET dm2rpting->report_option = "DDL SUMMARY"
     SET dm2rpting->status_criteria = "1 = 1"
    OF 2:
     SET dm2rpting->status_criteria = "ddol.status = 'RUNNING' "
     SET dm2rpting->report_option = "DDL RUNNING"
    OF 3:
     SET dm2rpting->status_criteria = concat(
      " not (ddol.status = 'ERROR' and substring(1,14,ddol.error_msg) = 'Application Id'",
      " and ddol.ready_to_run_ind = 0) and ddol.status = 'ERROR'")
     SET dm2rpting->report_option = "DDL FAILED"
    OF 4:
     SET dm2rpting->report_option = "DDL ALL"
     SET dm2rpting->status_criteria = "1 = 1"
    OF 5:
     SET dm2rpting->report_option = "DDL ALL BY TABLE"
     SET dm2rpting->status_criteria = "1 = 1"
    OF 99:
     SET ddr_prog_exit = 1
   ENDCASE
   IF (ddr_prog_exit=0)
    SET dm_err->eproc = concat("Running option:",dm2rpting->report_option)
    IF ((dm_err->debug_flag > 721))
     CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    ENDIF
    IF ((dm2rpting->report_option="DDL SUMMARY"))
     CALL ddl_rpt_group_by_status(null)
    ELSE
     CALL ddl_rpt_by_status(null)
    ENDIF
   ENDIF
 ENDWHILE
 SUBROUTINE ddl_rpt_group_by_status(null)
   DECLARE dro_total_cnt = f8 WITH protect, noconstant(0.0)
   DECLARE dro_str = vc WITH protect, noconstant("")
   DECLARE dro_line = c132 WITH protect, constant(fillstring(132,"-"))
   DECLARE dro_total_est = f8 WITH protect, noconstant(0.0)
   DECLARE dro_total_act = f8 WITH protect, noconstant(0.0)
   DECLARE dro_warning = i4 WITH protect, noconstant(0)
   DECLARE dro_f_warning = i4 WITH protect, noconstant(0)
   DECLARE dro_est = f8 WITH protect, noconstant(0.0)
   DECLARE dro_act = f8 WITH protect, noconstant(0.0)
   DECLARE dro_ndx = i4 WITH protect, noconstant(0)
   DECLARE dro_ndx2 = i4 WITH protect, noconstant(0)
   DECLARE dro_priority = i4 WITH protect, noconstant(0)
   DECLARE dro_found_managed = i2 WITH protect, noconstant(0)
   DECLARE dro_status_hold = vc WITH protect, noconstant("")
   FREE RECORD dro_rpt
   RECORD dro_rpt(
     1 stat[*]
       2 rpt_status = vc
   ...
   SET dm_err->eproc = "Create report grouped by status."
   IF ((dm_err->debug_flag > 721))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   ENDIF
   SELECT
    IF ((dm2rpting->report_name != "DM2NOTSET"))INTO value(dm2rpting->report_name)
    ELSE INTO mine
    ENDIF
    ddol.op_type, ddol.status, ddol.act_duration,
    ddol_status_null = nullind(ddol.status)
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=dm2rpting->run_id)
    ORDER BY ddol.priority, ddol.op_type, ddol.status
    HEAD REPORT
     SUBROUTINE duration(d_value)
      IF (d_value > 0)
       d_hun = mod(ceil((d_value * 100.0)),100), duration_str = concat(trim(format((d_value/ 86400.0),
          "DD.HH:MM:SS;3;Z"),3),".",format(d_hun,"##;P0"))
      ELSEIF (d_value=0)
       duration_str = "0.00:00:00.00"
      ENDIF
      ,
      CALL print(duration_str)
     END ;Subroutine report
     , drbs_pos1 = 2, drbs_pos2 = (ddr_priority_col_size+ 2),
     drbs_pos3 = (((ddr_priority_col_size+ 2)+ ddr_op_type_col_size)+ 2), drbs_pos4 = (((((
     ddr_priority_col_size+ 2)+ ddr_op_type_col_size)+ 2)+ ddr_status_col_size)+ 2), drbs_pos5 = ((((
     ((ddr_priority_col_size+ 2)+ ddr_op_type_col_size)+ 2)+ ddr_status_col_size)+ 2)+
     ddr_count_col_size),
     op_status_cnt = 0
     IF ((dm2rpting->package_number < 0))
      dro_str = concat("Summary Report - Operation Type by Status for Batch Install Plan: ",trim(
        cnvtstring(abs(dm2rpting->package_number))))
     ELSE
      dro_str = concat("Summary Report - Operation Type by Status for ",evaluate(dm2rpting->
        package_number,0,concat("Schema Date:",dm2rpting->schema_date),concat("Package:",trim(
          cnvtstring(dm2rpting->package_number)))))
     ENDIF
     col 1, dro_str, row + 2
     IF ((dir_managed_ddl->managed_ddl_ind=1))
      col 1,
      "*******************WARNING:   High Risk DDL Operations have been detected.***************************",
      row + 1,
      col 1,
      "Refer to the Install Tools Overview Guide on Cerner.com for recommended system preparation activities.",
      row + 2
     ENDIF
     col 1, dro_line, row + 1,
     col drbs_pos5, "Actual", row + 1,
     col drbs_pos1, "Priority", col drbs_pos2,
     "Operation Type", col drbs_pos3, "Status",
     col drbs_pos4, "Count", col drbs_pos5,
     "Duration", row + 1, col drbs_pos5,
     "D:HH:MM:SS:CC", row + 1, col 1,
     dro_line, row + 1
    HEAD ddol.priority
     row + 0, stat = alterlist(dro_rpt->stat,0), op_status_cnt = 0
    DETAIL
     dro_ndx = 0, dro_status_hold = ""
     IF (op_status_cnt=0)
      op_status_cnt = (op_status_cnt+ 1), stat = alterlist(dro_rpt->stat,op_status_cnt)
      IF (ddol.status="ERROR"
       AND substring(1,14,ddol.error_msg)="Application Id"
       AND ddol.ready_to_run_ind=0)
       dro_rpt->stat[op_status_cnt].rpt_status = "READY TO RUN"
      ELSE
       dro_rpt->stat[op_status_cnt].rpt_status = evaluate(ddol_status_null,1,"READY TO RUN",ddol
        .status)
      ENDIF
     ELSE
      IF (ddol.status="ERROR"
       AND substring(1,14,ddol.error_msg)="Application Id"
       AND ddol.ready_to_run_ind=0)
       dro_status_hold = "READY TO RUN"
      ELSE
       dro_status_hold = evaluate(ddol_status_null,1,"READY TO RUN",ddol.status)
      ENDIF
      dro_ndx = locateval(dro_ndx,1,size(dro_rpt->stat,5),dro_status_hold,dro_rpt->stat[dro_ndx].
       rpt_status)
      IF (dro_ndx=0)
       op_status_cnt = (op_status_cnt+ 1), stat = alterlist(dro_rpt->stat,op_status_cnt), dro_rpt->
       stat[op_status_cnt].rpt_status = dro_status_hold
      ENDIF
     ENDIF
     IF ((dir_managed_ddl->managed_ddl_ind=1))
      dro_ndx2 = 0, dro_priority = ddol.priority
      IF (locateval(dro_ndx2,1,dir_managed_ddl->priority_cnt,dro_priority,dir_managed_ddl->
       priorities[dro_ndx2].priority) > 0)
       IF (locateval(dro_ndx2,1,dir_managed_ddl->table_cnt,ddol.table_name,dir_managed_ddl->tables[
        dro_ndx2].table_name) > 0)
        IF (dro_ndx=0)
         dro_rpt->stat[op_status_cnt].rpt_managed = 1
        ELSE
         dro_rpt->stat[dro_ndx].rpt_managed = 1
        ENDIF
       ENDIF
      ENDIF
     ENDIF
     IF (dro_ndx=0)
      dro_rpt->stat[op_status_cnt].rpt_act_dur = (dro_rpt->stat[op_status_cnt].rpt_act_dur+ ddol
      .act_duration), dro_rpt->stat[op_status_cnt].rpt_cnt = (dro_rpt->stat[op_status_cnt].rpt_cnt+ 1
      )
     ELSE
      dro_rpt->stat[dro_ndx].rpt_act_dur = (dro_rpt->stat[dro_ndx].rpt_act_dur+ ddol.act_duration),
      dro_rpt->stat[dro_ndx].rpt_cnt = (dro_rpt->stat[dro_ndx].rpt_cnt+ 1)
     ENDIF
    FOOT  ddol.priority
     row + 0, op_status_cnt = 0
     FOR (op_status_cnt = 1 TO size(dro_rpt->stat,5))
       IF ((dro_rpt->stat[op_status_cnt].rpt_managed=1))
        dro_str = "*", call reportmove('COL',(drbs_pos1 - 1),0), dro_str,
        dro_found_managed = 1
       ENDIF
       dro_total_cnt = (dro_total_cnt+ dro_rpt->stat[op_status_cnt].rpt_cnt), dro_str = "", dro_str
        = trim(cnvtstring(ddol.priority)),
       col drbs_pos1, dro_str, dro_str = "",
       col drbs_pos2, ddol.op_type, dro_str = dro_rpt->stat[op_status_cnt].rpt_status,
       col drbs_pos3, dro_str, col drbs_pos4,
       dro_rpt->stat[op_status_cnt].rpt_cnt";C", col drbs_pos5,
       CALL duration(dro_rpt->stat[op_status_cnt].rpt_act_dur),
       row + 1
     ENDFOR
    FOOT REPORT
     IF (size(dro_rpt->stat,5) > 0)
      row + 2, col 1, "TOTALS:",
      col drbs_pos4, dro_total_cnt";C;I", dro_total_act = sum(ddol.act_duration),
      col drbs_pos5,
      CALL duration(dro_total_act), row + 1
      IF (dro_found_managed=1)
       row + 1, col 1, ddr_mng_msg
      ENDIF
     ELSE
      row + 1, col 1, "No Data found."
     ENDIF
     row + 2, col 50, "END OF REPORT"
   ...
   IF (check_error(dm_err->eproc) != 0)
    GO TO exit_program
   ENDIF
   IF ((dm2rpting->report_name != "DM2NOTSET"))
    IF (dir_silmode_requested_ind=1)
     CALL dir_add_silmode_entry(trim(dm2rpting->report_option),dm2rpting->report_name)
     SET dm_err->eproc = concat("Silent Mode: ",trim(dm2rpting->report_option),
      " was bypassed.  Report exits"," in CCLUSERDIR under file name: ",dm2rpting->report_name)
     CALL disp_msg("",dm_err->logfile,0)
    ELSE
     IF ((dir_ui_misc->background_ind=0))
      IF (dm2_disp_file(dm2rpting->report_name,trim(dm2rpting->report_option))=0)
       GO TO exit_program
      ENDIF
     ENDIF
    ENDIF
    IF ((dir_ui_misc->dm_process_event_id > 0))
     IF (drr_assign_file_to_installs("RPT:SCHEMA ESTIMATES",dm2rpting->report_name,dir_ui_misc->
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE ddl_rpt_by_status(null)
   DECLARE drbs_rows_exist = i2 WITH protect, noconstant(0)
   DECLARE drbs_str = vc WITH protect, noconstant("")
   DECLARE drbs_line = c290 WITH protect, constant(fillstring(290,"-"))
   DECLARE drbs_calc_act_duration = f8 WITH protect, noconstant(0.0)
   DECLARE drbs_str_cntr = i4 WITH protect, noconstant(0)
   DECLARE drbs_tot_act_time = f8 WITH protect, noconstant(0.0)
   DECLARE drbs_tot_est_time = f8 WITH protect, noconstant(0.0)
   DECLARE drbs_tbl_hold = vc WITH protect, noconstant("")
   DECLARE drbs_warning = i2 WITH protect, noconstant(0)
   DECLARE drbs_f_warning = i2 WITH protect, noconstant(0)
   DECLARE drbs_est = f8 WITH protect, noconstant(0.0)
   DECLARE drbs_operation = vc WITH protect, noconstant("")
   DECLARE drbs_ndx = i4 WITH protect, noconstant(0)
   DECLARE drbs_priority = i4 WITH protect, noconstant(0)
   DECLARE drbs_logfile_tmp = vc WITH protect, noconstant("")
   DECLARE drbs_delim = vc WITH protect, noconstant(evaluate(dm2_sys_misc->cur_os,"AXP","]","/"))
   DECLARE drbs_pos1 = i2 WITH protect, noconstant(0)
   DECLARE drbs_pos2 = i2 WITH protect, noconstant(0)
   DECLARE drbs_pos3 = i2 WITH protect, noconstant(0)
   DECLARE drbs_pos4 = i2 WITH protect, noconstant(0)
   DECLARE drbs_pos5 = i2 WITH protect, noconstant(0)
   DECLARE drbs_pos6 = i2 WITH protect, noconstant(0)
   SET dm_err->eproc = "Create report by status."
   IF ((dm_err->debug_flag > 721))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   ENDIF
   SELECT
    IF ((dm2rpting->report_option="DDL ALL BY TABLE")
     AND (dm2rpting->report_name="DM2NOTSET"))INTO mine
     FROM dm2_ddl_ops_log ddol
     WHERE (ddol.run_id=dm2rpting->run_id)
     ORDER BY ddol.owner_name, ddol.table_name, ddol.priority,
      ddol.est_duration DESC, ddol.status
    ELSEIF ((dm2rpting->report_option="DDL ALL BY TABLE")
     AND (dm2rpting->report_name != "DM2NOTSET"))INTO value(dm2rpting->report_name)
     FROM dm2_ddl_ops_log ddol
     WHERE (ddol.run_id=dm2rpting->run_id)
     ORDER BY ddol.owner_name, ddol.table_name, ddol.priority,
      ddol.est_duration DESC, ddol.status
    ELSEIF ((dm2rpting->report_option="DDL ALL")
     AND (dm2rpting->report_name="DM2NOTSET"))INTO mine
     FROM dm2_ddl_ops_log ddol
     WHERE (ddol.run_id=dm2rpting->run_id)
     ORDER BY ddol.priority, ddol.owner_name, ddol.table_name,
      ddol.obj_name
    ELSEIF ((dm2rpting->report_option="DDL ALL")
     AND (dm2rpting->report_name != "DM2NOTSET"))INTO value(dm2rpting->report_name)
     FROM dm2_ddl_ops_log ddol
     ...
     ORDER BY ddol.priority, ddol.owner_name, ddol.table_name,
      ddol.obj_name
    ELSEIF ((dm2rpting->report_name="DM2NOTSET"))INTO mine
     FROM dm2_ddl_ops_log ddol
     WHERE (ddol.run_id=dm2rpting->run_id)
      AND parser(dm2rpting->status_criteria)
     ORDER BY ddol.priority, ddol.owner_name, ddol.table_name,
      ddol.obj_name
    ELSE INTO value(dm2rpting->report_name)
     FROM dm2_ddl_ops_log ddol
     WHERE (ddol.run_id=dm2rpting->run_id)
      AND parser(dm2rpting->status_criteria)
     ORDER BY ddol.priority, ddol.owner_name, ddol.table_name,
    ...
    HEAD REPORT
     SUBROUTINE duration(d_value)
      ...
      CALL print(duration_str)
     END ;Subroutine report
     , drbs_pos1 = 1, drbs_pos2 = (ddr_table_col_size+ 3),
     drbs_pos3 = ((drbs_pos2+ ddr_op_type_col_size)+ 2), drbs_pos4 = ((drbs_pos3+ ddr_status_col_size
     )+ 2), drbs_pos5 = ((drbs_pos4+ ddr_logfile_col_size)+ 2)
     IF ((dm2rpting->report_option="DDL RUNNING"))
      drbs_str = "RUNNING Operations for "
     ELSEIF ((dm2rpting->report_option="DDL FAILED"))
      drbs_str = "FAILED Operations for "
     ELSE
      IF ((dm2rpting->report_option="DDL ALL BY TABLE"))
       drbs_str = "ALL Operations by Table for "
      ELSEIF ((dm2rpting->report_option IN ("DDL ALL", "DDL SUMMARY")))
       drbs_str = "ALL Operations for "
      ELSE
       drbs_str = concat(dm2rpting->report_option," for ")
      ENDIF
     ENDIF
     IF ((dm2rpting->package_number < 0))
      drbs_str = concat(drbs_str," Batch Install Plan:",trim(cnvtstring(abs(dm2rpting->package_number
          ))))
     ELSE
      drbs_str = concat(drbs_str," ",evaluate(dm2rpting->package_number,0,concat("Schema Date:",
         dm2rpting->schema_date),concat("Package:",trim(cnvtstring(dm2rpting->package_number)))))
     ENDIF
     col 1, drbs_str, row + 2
     IF ( NOT ((dm2rpting->report_option IN ("DDL RUNNING", "DDL FAILED")))
      AND (dir_managed_ddl->managed_ddl_ind=1))
      col 1,
      "*******************WARNING:   High Risk DDL Operations have been detected.***************************",
      row + 1,
      col 1,
     ...
     IF ((dm2rpting->report_option != "DDL ALL BY TABLE"))
      col drbs_pos5, "Actual", row + 1,
      col drbs_pos1, "Table\Object Name", col drbs_pos2,
      "Operation Type", col drbs_pos3, "Status",
      col drbs_pos4, "Logfile", col drbs_pos5,
      "Duration", row + 1, col drbs_pos5,
      "D:HH:MM:SS:CC", row + 1, col 1,
      drbs_line, row + 1
     ENDIF
    DETAIL
     drbs_warning = 0
     IF (drbs_tbl_hold != build(ddol.owner_name,".",ddol.table_name)
      AND (dm2rpting->report_option="DDL ALL BY TABLE"))
      drbs_tbl_hold = build(ddol.owner_name,".",ddol.table_name), col drbs_pos5, "Actual",
      row + 1, col drbs_pos1, "Table\Object Name",
      col drbs_pos2, "Operation Type", col drbs_pos3,
      "Status", col drbs_pos4, "Logfile",
      col drbs_pos5, "Duration", row + 1,
      col drbs_pos5, "D:HH:MM:SS:CC", row + 1,
      col 1, drbs_line, row + 1
      IF (ddol.table_name="-1")
       IF (ddol.obj_name != "-1")
        drbs_str = ddol.obj_name
       ELSE
        drbs_str = "--"
       ENDIF
      ELSE
       IF (findstring(".",drbs_tbl_hold,1,0)=1)
        drbs_str = ddol.table_name
       ELSE
        drbs_str = build(ddol.owner_name,".",ddol.table_name)
       ENDIF
      ENDIF
      col 1, drbs_str
     ELSEIF ((dm2rpting->report_option="DDL ALL BY TABLE"))
      drbs_tbl_hold = build(ddol.owner_name,".",ddol.table_name), drbs_str = "", drbs_str =
      fillstring(value(drbs_pos5),"-"),
      col 5, drbs_str, row + 1
     ENDIF
     drbs_tbl_hold = build(ddol.owner_name,".",ddol.table_name), drbs_rows_exist = 1
     IF (ddol.status="RUNNING")
      drbs_calc_act_duration = round(datetimediff(cnvtdatetime(curdate,curtime3),ddol.begin_dt_tm,5),
       0), drbs_tot_act_time = (drbs_tot_act_time+ drbs_calc_act_duration)
     ELSE
      drbs_tot_act_time = (drbs_tot_act_time+ ddol.act_duration)
     ENDIF
     IF ((dm2rpting->report_option != "DDL ALL BY TABLE"))
      IF (ddol.table_name="-1")
       IF (ddol.obj_name != "-1")
      ...
      ELSE
       ...
       ENDIF
      ENDIF
      col drbs_pos1, drbs_str
     ENDIF
     col drbs_pos2, ddol.op_type
     IF (ddol.status="ERROR"
      AND substring(1,14,ddol.error_msg)="Application Id"
      AND ddol.ready_to_run_ind=0)
      col drbs_pos3, "READY TO RUN"
     ELSE
      drbs_str = "", drbs_str = evaluate(nullind(ddol.status),1,"READY TO RUN",ddol.status), col
      drbs_pos3,
      drbs_str
     ENDIF
     IF (ddr_temp_dir > ""
      AND ddol.op_type="*REMOTE*")
      drbs_logfile_tmp = build(ddr_temp_dir,ddol.logfile_name)
     ELSE
      drbs_logfile_tmp = ddol.logfile_name
     ENDIF
     col drbs_pos4, drbs_logfile_tmp
     CASE (ddol.status)
      OF "RUNNING":
       col drbs_pos5,
       CALL duration(drbs_calc_act_duration)
      ELSE
       IF (nullind(ddol.act_duration)=1)
        col drbs_pos5,
        CALL duration(0)
       ELSE
        col drbs_pos5,
        CALL duration(ddol.act_duration)
       ENDIF
     ENDCASE
     row + 1
     IF (ddol.status="ERROR")
      IF (substring(1,14,ddol.error_msg) != "Application Id")
       drbs_str = "", row + 1, col 5,
       "Error Message:"
       IF (size(trim(ddol.error_msg)) > 77)
        drbs_str_cntr = 1
        WHILE (drbs_str_cntr < size(trim(ddol.error_msg)))
          row + 1, drbs_str = fillstring(77," "), drbs_str = substring(drbs_str_cntr,77,trim(ddol
            .error_msg)),
          col 16, drbs_str, drbs_str_cntr = (drbs_str_cntr+ 77)
        ENDWHILE
       ELSE
        row + 1, drbs_str = fillstring(77," "), drbs_str = trim(ddol.error_msg),
        col 16, drbs_str
       ENDIF
       row + 2
      ELSE
       IF (ddol.ready_to_run_ind=1)
        row + 1, col 5, "Error Message:",
        row + 1, drbs_str = "DDL associated to inactive session.", col 16,
        drbs_str, row + 1, drbs_str = "Restart installation to reestablish with active session.",
        col 16, drbs_str, row + 1
       ENDIF
      ENDIF
     ENDIF
     IF (ddol.operation > " ")
      drbs_operation = ddol.operation
      IF (dir_ddl_token_replacement(drbs_operation)=0)
       dm_err->err_ind = 1
      ENDIF
      row + 1, drbs_str = "", col 5,
      "Operation:"
      CASE (ddol.op_type)
       OF "EXECUTE DM2_RUNSTATS":
        drbs_str = "",drbs_str = concat(trim(ddol.op_type)," for table: ",ddol.table_name),col 16,
        drbs_str,row + 1
       ELSE
        IF (size(trim(drbs_operation)) > 77)
         drbs_str_cntr = 1
         WHILE (drbs_str_cntr < size(trim(drbs_operation)))
           row + 1, drbs_str = fillstring(77," "), drbs_str = substring(drbs_str_cntr,77,trim(
             drbs_operation)),
           col 16, drbs_str, drbs_str_cntr = (drbs_str_cntr+ 77)
         ENDWHILE
        ELSE
         row + 1, drbs_str = fillstring(77," "), drbs_str = trim(drbs_operation),
         col 16, drbs_str
        ENDIF
        ,row + 2
      ENDCASE
     ENDIF
     IF (ddol.error_msg > " "
      AND ddol.status IN ("COMPLETE", "RUNNING")
      AND substring(1,14,ddol.error_msg) != "Application Id")
      drbs_str = "", row + 1, col 5,
      "Last Message:"
      IF (size(trim(ddol.error_msg)) > 77)
     ...
     ENDIF
     IF (drbs_warning=1
      AND (dm2rpting->process_option != "CLIN COPY*"))
      col 1, ddr_w_msg, row + 1
     ENDIF
     IF ( NOT ((dm2rpting->report_option IN ("DDL RUNNING", "DDL FAILED")))
      AND (dir_managed_ddl->managed_ddl_ind=1))
      drbs_ndx = 0, drbs_priority = ddol.priority
      IF (locateval(drbs_ndx,1,dir_managed_ddl->priority_cnt,drbs_priority,dir_managed_ddl->
       priorities[drbs_ndx].priority) > 0)
       IF (locateval(drbs_ndx,1,dir_managed_ddl->table_cnt,ddol.table_name,dir_managed_ddl->tables[
        drbs_ndx].table_name) > 0)
        col 1, ddr_mng_msg, row + 1
       ENDIF
      ENDIF
     ENDIF
     IF ((dm2rpting->report_option != "DDL ALL BY TABLE"))
     ...
     row + 1
    FOOT REPORT
     IF (drbs_rows_exist=0)
      row + 1, col 1, "No Data found."
     ELSE
      col 1, "TOTALS:", col drbs_pos5,
      CALL duration(drbs_tot_act_time), row + 1
     ENDIF
     row + 2, col 50, "END OF REPORT"
    WITH nocounter, maxcol = 300, formfeed = none,
     nullreport
   ...
   IF ((dm2rpting->report_name != "DM2NOTSET"))
    IF (dir_silmode_requested_ind=1)
     ...
     CALL disp_msg("",dm_err->logfile,0)
    ELSE
     IF (dm2_disp_file(dm2rpting->report_name,concat(trim(dm2rpting->report_option)," Report"))=0)
      GO TO exit_program
     ENDIF
    ENDIF
   ENDIF
 END ;Subroutine
 SUBROUTINE ddl_get_managed(null)
   DECLARE dgm_exist_ind = i2 WITH protect, noconstant(0)
   IF ((dir_managed_ddl->setup_complete=0))
    IF ((dm2rpting->process_option IN ("CLIN UPGRADE", "CLIN UPGRADE NR", "DATABASE MIGRATION",
    "PREVIEW")))
     IF (dm2_table_and_ccldef_exists("DM_INFO",dgm_exist_ind)=0)
      RETURN(0)
     ENDIF
     IF (dgm_exist_ind=1)
      IF (dir_managed_ddl_setup(dm2rpting->run_id)=0)
       RETURN(0)
      ENDIF
     ELSE
      SET dir_managed_ddl->setup_complete = 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddl_get_addt_data(null)
   IF ((dm2rpting->process_option="DM2NOTSET"))
    SET dm_err->eproc = "Retrieving process option."
    IF ((dm_err->debug_flag > 721))
     CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
     FROM dm2_ddl_ops d
     WHERE (d.run_id=dm2rpting->run_id)
     DETAIL
      dm2rpting->process_option = d.process_option
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ENDIF
   IF ((dm2rpting->process_option="CLIN COPY*"))
    IF (der_manage_admin_data(ddr_dbname,"DM2_ADMIN_DM_INFO","S","ALL","")=0)
     RETURN(0)
    ENDIF
    IF ((der_expimp_data->tgt_temp_dir > "")
     AND (der_expimp_data->tgt_temp_dir != "DM2NOTSET")
     AND size(der_expimp_data->tgt_temp_dir) < 51)
     IF (dm2_get_rdbms_version(null)=0)
      RETURN(0)
     ENDIF
     IF ((dm2_rdbms_version->level1 >= 11))
      SET ddr_temp_dir = build(der_expimp_data->tgt_temp_dir,"/",cnvtlower(ddr_dbname),"_exp/")
     ELSE
      SET ddr_temp_dir = build(der_expimp_data->tgt_temp_dir,"/log/")
     ENDIF
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddr_max_col_size(null)
   SET dm_err->eproc = "Getting Maximum Col Sizes"
   SET ddr_status_col_size = 0
   SET ddr_op_type_col_size = 0
   SET ddr_logfile_col_size = 0
   SET ddr_count_col_size = 17
   SET ddr_status_col_size = 14
   SET ddr_priority_col_size = 10
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE (( NOT (ddol.status="ERROR"
     AND substring(1,14,ddol.error_msg)="Application Id")) OR (ddol.status = null))
     AND (ddol.run_id=dm2rpting->run_id)
    DETAIL
     IF (size(trim(ddol.op_type,3)) > ddr_op_type_col_size)
      ddr_op_type_col_size = size(trim(ddol.op_type,3))
     ENDIF
     IF (size(trim(ddol.logfile_name,3)) > ddr_logfile_col_size)
      ddr_logfile_col_size = size(trim(ddol.logfile_name,3))
     ENDIF
     IF (size(build(trim(ddol.owner_name,3),".",trim(ddol.table_name,3))) > ddr_table_col_size)
      ddr_table_col_size = size(build(trim(ddol.owner_name,3),".",trim(ddol.table_name,3)))
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
   ...
   ELSE
    IF (curqual=0)
     SET ddr_status_col_size = 30
     SET ddr_op_type_col_size = 60
     SET ddr_logfile_col_size = (size(ddr_temp_dir)+ 40)
     SET ddr_table_col_size = 30
    ELSE
     IF (ddr_op_type_col_size < 16)
      SET ddr_op_type_col_size = 16
     ENDIF
     IF (ddr_logfile_col_size < 20)
      SET ddr_logfile_col_size = (size(ddr_temp_dir)+ 20)
     ELSE
      SET ddr_logfile_col_size = (size(ddr_temp_dir)+ ddr_logfile_col_size)
     ENDIF
    ENDIF
   ENDIF
   IF ((dm_err->debug_flag > 721))
    CALL echo(concat("ddr_temp_dir = ",ddr_temp_dir))
    CALL echo(build("ddr_op_type_col_size = ",ddr_op_type_col_size))
    CALL echo(build("ddr_status_col_size = ",ddr_status_col_size))
    CALL echo(build("ddr_count_col_size = ",ddr_count_col_size))
    CALL echo(build("ddr_logfile_col_size = ",ddr_logfile_col_size))
    CALL echo(build("ddr_table_col_size = ",ddr_table_col_size))
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_program
 SET message = nowindow
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM2_DDL_REPORTS completed successfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 CALL final_disp_msg("dm2_ddl_reports")
 IF ((dm_err->debug_flag > 721))
  CALL echorecord(dm2comprom)
  CALL echorecord(dm2rpting)
  CALL echorecord(dcprom_ocd)
  CALL echo(ddr_status_col_size)
  CALL echo(ddr_op_type_col_size)
 ENDIF
END GO
