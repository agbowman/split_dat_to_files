CREATE PROGRAM dm2_arc_get_info:dba
 ...
 ENDIF
 DECLARE dagi_errchk = i2
 SELECT INTO "nl:"
  d.beg_effective_dt_tm, d.end_effective_dt_tm, d.info_number,
  d.info_char, d.info_dt_tm
  FROM dm_arc_info d
  WHERE (d.info_domain=request->info_domain)
   AND (d.info_name=request->info_name)
  ...
  DETAIL
   cnt = (cnt+ 1)
   IF (mod(cnt,10)=1)
    stat = alterlist(reply->ranges,(cnt+ 9))
   ENDIF
   reply->ranges[cnt].info_number = d.info_number, reply->ranges[cnt].info_char = d.info_char, reply
   ->ranges[cnt].info_dt_tm = d.info_dt_tm,
   reply->ranges[cnt].beg_effective_dt_tm = d.beg_effective_dt_tm, reply->ranges[cnt].
   end_effective_dt_tm = d.end_effective_dt_tm
  FOOT REPORT
   stat = alterlist(reply->ranges,cnt)
  WITH nocounter
 ;end select
 SET dagi_errchk = arc_error_check("An error occurred during dm2_arc_get_info: ","ARCHIVE","PERSON")
END GO
