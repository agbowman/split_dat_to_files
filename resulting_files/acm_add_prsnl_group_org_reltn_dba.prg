CREATE PROGRAM acm_add_prsnl_group_org_reltn:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
  IF ((acm_request->prsnl_group_org_reltn_qual[xref->add[index].idx].prsnl_group_org_reltn_id <= 0.0)
  )
   SELECT INTO "nl:"
    nextseqnum = seq(prsnl_seq,nextval)
    FROM dual
    DETAIL
     acm_request->prsnl_group_org_reltn_qual[xref->add[index].idx].prsnl_group_org_reltn_id =
     cnvtreal(nextseqnum), reply->prsnl_group_org_reltn_qual[xref->add[index].idx].
     prsnl_group_org_reltn_id = cnvtreal(nextseqnum)
    WITH nocounter, format
   ;end select
   IF (curqual=0)
    SET reply->prsnl_group_org_reltn_qual[xref->add[index].idx].status = gen_nbr_error
    SET failed = gen_nbr_error
    GO TO exit_script
   ENDIF
  ELSE
   SET reply->prsnl_group_org_reltn_qual[xref->add[index].idx].prsnl_group_org_reltn_id = acm_request
   ->prsnl_group_org_reltn_qual[xref->add[index].idx].prsnl_group_org_reltn_id
  ENDIF
  SET reply->prsnl_group_org_reltn_qual[xref->add[index].idx].status = 0
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   prsnl_group_org_reltn p
  SET p.prsnl_group_org_reltn_id = acm_request->prsnl_group_org_reltn_qual[xref->add[d.seq].idx].
   prsnl_group_org_reltn_id, p.prsnl_group_id =
   IF ((acm_request->prsnl_group_org_reltn_qual[xref->add[d.seq].idx].prsnl_group_id > 0))
    acm_request->prsnl_group_org_reltn_qual[xref->add[d.seq].idx].prsnl_group_id
   ELSE reply->prsnl_group_qual[acm_request->prsnl_group_org_reltn_qual[xref->add[d.seq].idx].
    prsnl_group_idx].prsnl_group_id
   ENDIF
   , p.organization_id = acm_request->prsnl_group_org_reltn_qual[xref->add[d.seq].idx].
   organization_id,
   p.reltn_type_cd = acm_request->prsnl_group_org_reltn_qual[xref->add[d.seq].idx].reltn_type_cd, p
   .updt_cnt = 0, p.updt_dt_tm = cnvtdatetime(curdate,curtime3),
   p.updt_id = reqinfo->updt_id, p.updt_applctx = reqinfo->updt_applctx, p.updt_task = reqinfo->
   updt_task
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->prsnl_group_org_reltn_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->prsnl_group_org_reltn_qual[xref->add[index].idx].status=0))
