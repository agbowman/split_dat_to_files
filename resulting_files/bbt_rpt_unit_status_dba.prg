CREATE PROGRAM bbt_rpt_unit_status:dba
 ...
 SET stat = uar_get_meaning_by_codeset(1610,"15",code_cnt,shipped_code)
 IF (((mrn_code=0.0) OR (((dispose_code=0.0) OR (((transfuse_code=0.0) OR (((destroy_code=0.0) OR (((
 derivative_class_cd=0.0) OR (shipped_code=0.0)) )) )) )) )) )
  SET reply->status = "F"
  GO TO exit_script
 ENDIF
 RECORD aborh(
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   aborh->aborh_list[aborh_index].aborh_display = substring(1,13,cv1.display), aborh->aborh_list[
   aborh_index].abo_code = cv2.code_value, aborh->aborh_list[aborh_index].rh_code = cv3.code_value
   ...
   IF (naddproduct=1)
    IF (p.product_class_cd=derivative_class_cd)
     IF (pe.event_type_cd != dispose_code
      ...
      AND pe.event_type_cd != shipped_code)
      ndervalidevent += 1
     ENDIF
    ELSE
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  d_flg = decode(bp.seq,"BP",de.seq,"DE","XX"), pr.product_id, pr.product_nbr,
  pr.product_sub_nbr, pr.product_cat_cd, pr.cur_expire_dt_tm,
  product_class_display = uar_get_code_display(pr.product_class_cd), product_cat_display =
  uar_get_code_display(pr.product_cat_cd), product_display = uar_get_code_display(pr.product_cd),
  pe.person_id, pe.product_event_id, pe.event_type_cd,
  event_type_display = uar_get_code_display(pe.event_type_cd)"###############", bp.cur_abo_cd, bp
  .cur_rh_cd,
  de.cur_avail_qty, per.name_full_formatted"#########################", n_f_f = decode(per
   .name_full_formatted,per.name_full_formatted,fillstring(20," "))"####################",
  ...
  ORDER BY product_class_display, product_cat_display, product_display,
   cnvtdatetime(pr.cur_expire_dt_tm), pr.product_id, pr.product_nbr,
   pr.product_sub_nbr, event_type_display
  HEAD REPORT
   new_report = "Y", product_cd_hd = 0.0, select_ok_ind = 0,
   mrn = fillstring(20," ")
  HEAD PAGE
   new_page = "Y", row 0,
   ...
   row + 1, col 104, captions->user,
   col 111, reportbyusername"###############;R", inc_i18nhandle = 0,
   ...
   IF (sub_get_location_name != "<<INFORMATION NOT FOUND>>")
    ...
    ENDIF
   ENDIF
   row + 1, col 1, captions->blood_bank_owner,
   col 19, cur_owner_area_disp, row + 1,
   col 1, captions->inventory_area, col 17,
   cur_inv_area_disp, row + 2, col 4,
   captions->product_class, col 22, product_class_display,
   beg_dt_tm = cnvtdatetime(begin_date_time), row + 1, col 4,
   captions->product_category, col 22, product_cat_display,
   end_dt_tm = cnvtdatetime(end_date_time), row + 2, col 20,
   captions->begin_exp_date, col 47, beg_dt_tm"@DATETIMECONDENSED;;d",
   col 72, captions->ending_exp_date, col 96,
   end_dt_tm"@DATETIMECONDENSED;;d", row + 2,
   CALL center(captions->prod_number,1,25),
   CALL center(captions->expire,59,70), col 27, captions->prod_type,
   CALL center(captions->aborh,40,52), col 54, captions->quantity,
   CALL center(captions->date_time,59,70),
   CALL center(captions->status,72,84),
   CALL center(captions->name,86,105),
   CALL center(captions->mrn,107,125), row + 1,
   CALL center(captions->serial_number,1,25),
   row + 1, col 1, "-------------------------",
   col 27, "------------", col 40,
   "-------------", col 54, "----",
   col 59, "------------", col 72,
  ...
  DETAIL
   ...
   IF (new_prod_id="Y")
    ...
    ENDIF
    IF (d_flg="DE")
     qty = trim(cnvtstring(de.cur_avail_qty,4,0,r)), col 54, qty
    ENDIF
    dt_tm = cnvtdatetime(pr.cur_expire_dt_tm), col 59, dt_tm"@DATETIMECONDENSED;;d",
    col 72, event_type_display
    IF (n_f_f > " ")
     col 86, n_f_f
    ENDIF
    IF (alias="Y")
     mrn = cnvtalias(ea.alias,ea.alias_pool_cd), col 107, mrn"###################"
    ELSE
  ...
  WITH nocounter, nullreport, maxrow = 61,
   dontcare(per), dontcare(ea), compress,
   nolandscape
 ;end select
 SET rpt_cnt += 1
