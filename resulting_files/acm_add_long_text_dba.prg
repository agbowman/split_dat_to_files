CREATE PROGRAM acm_add_long_text:dba
 ...
 ENDIF
 IF ( NOT (validate(add_long_text_request)))
  SET failed = not_found
  SET table_name = "add_long_text_request was not defined by calling script"
  GO TO exit_script
 ENDIF
 IF ( NOT (validate(add_long_text_reply)))
 ...
 SET add_long_text_reply->qual_cnt = size(add_long_text_request->qual,5)
 SET stat = alterlist(add_long_text_reply->qual,add_long_text_reply->qual_cnt)
 IF ((add_long_text_reply->qual_cnt=0))
  SET failed = attribute_error
  SET table_name = "No long text rows passed in"
  GO TO exit_script
 ENDIF
 DECLARE comment_tz = i4 WITH protect, noconstant(0)
 IF (curutc > 0)
  SET comment_tz = curtimezoneapp
 ELSE
  SET comment_tz = 0
 ENDIF
 FOR (i = 1 TO add_long_text_reply->qual_cnt)
   SET generic_ind = true
   IF (generic_ind)
    IF ((add_long_text_request->qual[i].long_text_id=0))
     SELECT INTO "nl:"
      nextseqnum = seq(long_data_seq,nextval)
      FROM dual
      DETAIL
       add_long_text_reply->qual[i].long_text_id = nextseqnum, add_long_text_request->qual[i].
     ...
     ENDIF
    ELSE
     SET add_long_text_reply->qual[i].long_text_id = add_long_text_request->qual[i].long_text_id
    ENDIF
   ENDIF
   IF ( NOT (generic_ind))
    SET add_long_text_reply->qual[i].status = gen_nbr_error
 ...
 IF (checkdic("LONG_TEXT.UPDT_TZ","A",0) > 0)
  INSERT  FROM long_text t,
    (dummyt d  WITH seq = value(add_long_text_reply->qual_cnt))
   SET t.long_text_id = add_long_text_request->qual[d.seq].long_text_id, t.updt_cnt = 0, t.updt_dt_tm
     = cnvtdatetime(curdate,curtime3),
    t.updt_id = reqinfo->updt_id, t.updt_task = reqinfo->updt_task, t.updt_applctx = reqinfo->
    updt_applctx,
    t.active_ind = add_long_text_request->qual[d.seq].active_ind, t.active_status_cd =
    IF ((add_long_text_request->qual[d.seq].active_status_cd > 0)) add_long_text_request->qual[d.seq]
     .active_status_cd
    ELSE
     IF ((add_long_text_request->qual[d.seq].active_ind=true)) reqdata->active_status_cd
     ELSE reqdata->inactive_status_cd
     ENDIF
    ENDIF
    , t.active_status_dt_tm = cnvtdatetime(curdate,curtime3),
    t.active_status_prsnl_id = reqinfo->updt_id, t.parent_entity_name =
    IF ((add_long_text_request->qual[d.seq].parent_entity_name > " ")) trim(add_long_text_request->
      qual[d.seq].parent_entity_name)
    ELSE " "
    ENDIF
    , t.parent_entity_id = add_long_text_request->qual[d.seq].parent_entity_id,
    t.long_text = add_long_text_request->qual[d.seq].long_text, t.updt_tz = comment_tz
   PLAN (d)
    JOIN (t)
   WITH nocounter, status(add_long_text_reply->qual[d.seq].status)
  ;end insert
 ELSE
  INSERT  FROM long_text t,
    (dummyt d  WITH seq = value(add_long_text_reply->qual_cnt))
   SET t.long_text_id = add_long_text_request->qual[d.seq].long_text_id, t.updt_cnt = 0, t.updt_dt_tm
    ...
    ENDIF
    , t.parent_entity_id = add_long_text_request->qual[d.seq].parent_entity_id,
    t.long_text = add_long_text_request->qual[d.seq].long_text
   PLAN (d)
    JOIN (t)
   WITH nocounter, status(add_long_text_reply->qual[d.seq].status)
  ;end insert
 ENDIF
 FOR (i = 1 TO add_long_text_reply->qual_cnt)
   IF ((add_long_text_reply->qual[i].status=0))
    SET failed = insert_error
    SET add_long_text_reply->qual[i].status = insert_error
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
