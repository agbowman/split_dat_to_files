CREATE PROGRAM dm_tablespace_mapping:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dtm_toolset_usage = i2 WITH protect, noconstant(0)
 ...
 ELSEIF (dtm_toolset_usage=1)
  ...
  CALL echo(concat("Component Name:  ",curprog))
  CALL echo("*")
  CALL echo("Process Description:  Verifying toolset usage.  This tool has been replaced.")
  CALL echo("*")
  CALL echo(
   "Recommended Action(s):  To create Tablespace mappings, use the Tablespace Mapping option in DM2_DOMAIN_MAINT:"
   )
  CALL echo(
   "                        DM2_DOMAIN_MAINT -> Installation Configuration -> Tablespace Mapping")
  CALL echo("*")
  CALL echo(dm_err->asterisk_line)
  CALL echo("*")
  CALL echo("*")
...
#init
 ...
 DECLARE name = c30
 DECLARE char = c30
 DECLARE space_cnt = i4
 DECLARE count = i4
 DECLARE old_tablespace = c30
 DECLARE new_tablespace = c30
 DECLARE cern_tablespace = c30
 DECLARE pre_mod_ts = c30
 DECLARE post_mos_ts = c30
 DECLARE pre_del_ots = c30
 DECLARE pre_del_nts = c30
 DECLARE cerner_tablespace = c30
 DECLARE client_tablespace = c30
 CALL text(24,1,"Program is in Process, please wait....")
 DELETE  FROM dm_info d
  WHERE d.info_domain="TABLESPACE MAPPING"
   AND  NOT ( EXISTS (
  (SELECT
   u.tablespace_name
   FROM user_tablespaces u
   WHERE u.tablespace_name=d.info_char)))
 ;end delete
 COMMIT
 SET add_flag = 0
#menu
 CALL clear(1,1)
 CALL text(2,2,"Please enter V to view the existing tablespace mapping.")
 CALL text(4,2,"Please enter A to add a new tablespace mapping.")
 CALL text(6,2,"Please enter M to modify a existing tablespace mapping.")
 CALL text(8,2,"Please enter D to delete a existing tablespace mapping.")
 CALL text(24,2,"Select (V)iew, (A)dd, (M)odify, (D)elete, (Q)uit: ")
 CALL accept(24,61,"P;CU","Q"
  WHERE curaccept IN ("V", "A", "M", "D", "Q"))
 SET menu = " "
 SET menu = curaccept
 IF (menu="Q")
  GO TO end_program
...
#vi
 CALL clear(1,1)
 SELECT
  name = substring(1,30,d.info_name), char = substring(1,30,d.info_char)
  FROM dm_info d
  WHERE info_domain="TABLESPACE MAPPING*"
  ORDER BY substring(3,30,d.info_name), substring(1,2,d.info_name)
  HEAD PAGE
   col 5, "Cerner Tablespace name", col 45,
   "Client Tablespace name", row + 1
  DETAIL
   col 5, name, col 45,
   char, row + 1
  WITH nocounter
...
#ad
 IF (add_flag=0)
  CALL clear(1,1)
  CALL text(20,2,"Program is in process, Please wait....")
  RECORD tablespace(
    1 list[*]
      2 space = c30
  )
  SET space_cnt = 0
  SELECT DISTINCT INTO "NL:"
   d_a_tables.tablespace_name
   FROM dm_afd_tables d_a_tables
   WHERE  NOT ( EXISTS (
   (SELECT
    d.info_name, d.info_char
    FROM dm_info d
    WHERE d.info_domain="TABLESPACE MAPPING"
     AND ((d.info_name=d_a_tables.tablespace_name) OR (d.info_char=d_a_tables.tablespace_name)) )))
    AND d_a_tables.tablespace_name="D_*"
   DETAIL
    space_cnt = (space_cnt+ 1)
    IF (mod(space_cnt,10)=1)
     stat = alterlist(tablespace->list,(space_cnt+ 9))
    ENDIF
    tablespace->list[space_cnt].space = d_a_tables.tablespace_name
   WITH nocounter
  ;end select
  SELECT DISTINCT INTO "NL:"
   d_a_indexes.tablespace_name
   FROM dm_afd_indexes d_a_indexes
   WHERE  NOT ( EXISTS (
   (SELECT
    d.info_name, d.info_char
    FROM dm_info d
    WHERE d.info_domain="TABLESPACE MAPPING"
     AND ((d.info_name=d_a_indexes.tablespace_name) OR (d.info_char=d_a_indexes.tablespace_name)) )))
    AND d_a_indexes.tablespace_name="I_*"
   DETAIL
    space_cnt = (space_cnt+ 1)
    IF (mod(space_cnt,10)=1)
     stat = alterlist(tablespace->list,(space_cnt+ 9))
    ENDIF
    tablespace->list[space_cnt].space = d_a_indexes.tablespace_name
   WITH nocounter
  ;end select
  SET count = space_cnt
  SELECT INTO "nl:"
   ...
   (SELECT
    ...
    FROM dm_info d
    WHERE d.info_domain="TABLESPACE MAPPING"
     AND ((d.info_name=dt.tablespace_name) OR (d.info_char=dt.tablespace_name)) )))
    AND dt.tablespace_name IN ("I_*", "D_*")
   HEAD REPORT
    row + 1
   DETAIL
    exist = 0
    FOR (x = 1 TO count)
      IF ((dt.tablespace_name=tablespace->list[x].space))
       exist = (exist+ 1)
      ENDIF
    ENDFOR
    IF (exist=0)
     ...
     IF (mod(space_cnt,10)=1)
      stat = alterlist(tablespace->list,(space_cnt+ 9))
     ENDIF
     tablespace->list[space_cnt].space = dt.tablespace_name
    ENDIF
   FOOT REPORT
    stat = alterlist(tablespace->list,space_cnt)
   WITH nocounter
  ;end select
  SET add_flag = 1
 ENDIF
...
#old
 CALL clear(18,1)
 CALL text(2,2,"1, Choose one of the Cerner standard tablespace from help to add: ")
 SET help =
 SELECT INTO "nl:"
  table_space = tablespace->list[d.seq].space
  FROM (dummyt d  WITH seq = value(size(tablespace->list,5)))
  PLAN (d)
  ORDER BY substring(3,30,tablespace->list[d.seq].space), substring(1,2,tablespace->list[d.seq].space
    )
  WITH nocounter
 ;end select
 CALL accept(3,2,"P(30);CUSF")
 SET old_tablespace = fillstring(30," ")
 SET old_tablespace = curaccept
 SELECT INTO "nl:"
  info_name, info_char
  FROM dm_info
  WHERE info_domain="TABLESPACE MAPPING"
   AND ((info_name=old_tablespace) OR (info_char=old_tablespace))
  WITH nocounter
 ;end select
 IF (curqual != 0)
  CALL text(18,1,concat("This tablespace ",trim(old_tablespace),
    " already exists on the mapping table."))
  CALL text(20,1,
   "Please enter <A> to add a different tablespace, enter <B> to back to the main menu.")
  CALL accept(20,100,"P;CU","B"
   WHERE curaccept IN ("A", "B"))
  IF (curaccept="A")
   GO TO old
  ELSEIF (curaccept="B")
   GO TO menu
 ...
 ENDIF
#new
 CALL text(5,2,"2, Choose a client tablespace to map the cerner tablespace.")
 IF (old_tablespace="D_*")
  SET help =
  SELECT
   u_t.tablespace_name
   FROM user_tablespaces u_t
   WHERE u_t.tablespace_name="D_*"
    AND u_t.tablespace_name != old_tablespace
    AND  NOT ( EXISTS (
   (SELECT
    d.info_char
    FROM dm_info d
    WHERE d.info_domain="TABLESPACE MAPPING"
     AND ((d.info_char=u_t.tablespace_name) OR (d.info_name=u_t.tablespace_name)) )))
   ORDER BY u_t.tablespace_name
   WITH nocounter
  ;end select
 ELSEIF (old_tablespace="I_*")
  SET help =
  SELECT
   u_t.tablespace_name
   FROM user_tablespaces u_t
   WHERE u_t.tablespace_name="I_*"
    AND u_t.tablespace_name != old_tablespace
    AND  NOT ( EXISTS (
   (SELECT
    d.info_char
  ...
  ;end select
 ENDIF
 CALL accept(6,2,"P(30);CUSF")
 SET new_tablespace = fillstring(30," ")
 SET new_tablespace = curaccept
#correct_yn
 CALL video(n)
 SET help = off
 SET validate = off
 ...
 IF (curaccept="N")
  CALL clear(24,1)
  CALL text(24,1,"(C)ontinue to add or (B)ack to menu:")
  CALL accept(24,50,"P;CU","C"
  ...
  CALL clear(24,1)
  CALL text(24,1,"LINE #")
  CALL accept(24,10,"XX",1
   WHERE curaccept BETWEEN 0 AND 2)
  IF (curaccept=0)
   GO TO correct_yn
  ENDIF
  GO TO (old, new)curaccept
 ENDIF
 INSERT  FROM dm_info
  SET info_domain = "TABLESPACE MAPPING", info_name = old_tablespace, info_char = new_tablespace,
...
#mo
 CALL clear(1,1)
 CALL text(1,1,"Please enter a cerner tablespace name for modifying: ")
 SET help = pos(2,30,10,70)
 SET help =
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 CALL accept(1,60,"P(30);CUSF")
 SET cern_tablespace = fillstring(30," ")
 SET cern_tablespace = curaccept
 SET pre_mod_ts = fillstring(30," ")
 SELECT INTO "nl:"
  d.info_char
  FROM dm_info d
  WHERE d.info_domain="TABLESPACE MAPPING"
   AND d.info_name=cern_tablespace
  DETAIL
   pre_mod_ts = d.info_char
  WITH nocounter
 ;end select
 CALL text(3,1,concat("The current client tablespace mapped to the choosen cerner tablespace is ",
   trim(pre_mod_ts)))
 CALL text(5,1,"Please enter the new client tablespace name: ")
 SET help = pos(4,60,15,35)
 IF (cern_tablespace="D_*")
  SET help =
  SELECT
   u_t.tablespace_name
   FROM user_tablespaces u_t
 ...
 ELSEIF (cern_tablespace="I_*")
  ...
  ;end select
 ENDIF
 CALL accept(5,60,"P(30);CUSF")
 SET post_mod_ts = fillstring(30," ")
 SET post_mod_ts = curaccept
 CALL text(7,1,concat("The cerner tablespace is ",trim(cern_tablespace),
   " and the new tablespace map to it is ",trim(post_mod_ts)))
 CALL clear(24,1)
 CALL text(24,1,"CORRECT? (Y/N)")
 CALL accept(24,18,"P;CU","Y"
  WHERE curaccept IN ("Y", "N"))
 IF (curaccept="N")
  CALL clear(24,1)
  CALL text(24,1,"(C)ontinue to modify or (B)ack to menu:")
  CALL accept(24,50,"P;CU","C"
   WHERE curaccept IN ("C", "B"))
  IF (curaccept="B")
   GO TO menu
 ...
 ENDIF
 UPDATE  FROM dm_info
  SET info_char = post_mod_ts, updt_dt_tm = cnvtdatetime(curdate,curtime3)
  WHERE info_domain="TABLESPACE MAPPING"
   AND info_name=cern_tablespace
 ;end update
 COMMIT
 GO TO menu
#del
 CALL clear(1,1)
 CALL text(1,1,"Please enter a cerner tablespace name for deleting: ")
 SET cerner_tablespace = fillstring(30," ")
 SET client_tablespace = fillstring(30," ")
 SET help = pos(2,30,10,70)
 SET help =
 SELECT INTO "nl:"
  cerner_tablespace = substring(1,30,d.info_name), client_tablespace = substring(1,30,d.info_char)
 ...
 ;end select
 CALL accept(1,60,"P(30);CUSF")
 SET pre_del_ots = fillstring(30," ")
 SET pre_del_ots = curaccept
 SET pre_del_nts = fillstring(30," ")
 SELECT INTO "nl:"
  d.info_char
  FROM dm_info d
  WHERE d.info_domain="TABLESPACE MAPPING"
   AND d.info_name=pre_del_ots
  DETAIL
   pre_del_nts = d.info_char
  WITH nocounter
 ;end select
 CALL text(3,1,concat("The tablespace combination will be deleted are ",trim(pre_del_ots)," and ",
   trim(pre_del_nts)))
 CALL clear(24,1)
 CALL text(24,1,"CORRECT? (Y/N)")
 CALL accept(24,18,"P;CU","Y"
 ...
 IF (curaccept="N")
  CALL clear(24,1)
  CALL text(24,1,"(C)ontinue to delete or (B)ack to menu:")
  CALL accept(24,50,"P;CU","C"
   WHERE curaccept IN ("C", "B"))
  IF (curaccept="B")
   GO TO menu
 ...
 DELETE  FROM dm_info
  WHERE info_domain="TABLESPACE MAPPING"
   AND info_name=pre_del_ots
   AND info_char=pre_del_nts
 ;end delete
 COMMIT
 GO TO menu
#end_program
