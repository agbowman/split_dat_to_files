CREATE PROGRAM dm2_rr_resumable_menu
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_rr_res_menu",".log","dm2_rr_res_menu")=0)
  GO TO exit_program
 ENDIF
 SET width = 132
 ...
 IF ((dm2_rdbms_version->level1 < 9))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Monitor resumable session for Replicate/Refresh."
  SET dm_err->emsg = "Resumable is not supported for this Oracle version."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 IF (((drer_fill_email_list(drer_email_det->src_env,drer_email_det->tgt_env)=0) OR ((drer_email_list
 ->email_cnt=0))) )
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Getting email addresses from Admin DM_INFO."
  SET dm_err->emsg = "Failed to retrieve email addresses."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "Leaving dm2_rr_resumable_menu"
 CALL final_disp_msg("dm2_rr_res_menu")
END GO
