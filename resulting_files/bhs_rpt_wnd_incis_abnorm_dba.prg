CREATE PROGRAM bhs_rpt_wnd_incis_abnorm:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Select Facility" = 0,
  "Unit" = value(0.0),
  "Admit Start date(2 month Max Range)" = "CURDATE",
  "Admit End Date" = "CURDATE"
 ...
 IF (datetimediff(cnvtdatetime(cnvtdate2(ms_end_date,"dd-mmm-yyyy"),235959),cnvtdatetime(cnvtdate2(
    ms_start_date,"dd-mmm-yyyy"),0)) > 62)
  SET ms_error = "Date range exceeds 62 days."
 ...
 ELSE
  SELECT INTO "nl:"
   FROM encntr_alias fin,
    ...
    code_value fac
   PLAN (e
    WHERE (e.loc_facility_cd= $FNAME)
     AND operator(e.loc_nurse_unit_cd,opr_var, $F_UNIT)
     AND e.active_ind=1
     AND e.active_status_cd=mf_cs48_active
     AND e.reg_dt_tm BETWEEN cnvtdatetime(cnvtdate2(ms_start_date,"dd-mmm-yyyy"),0) AND cnvtdatetime(
     cnvtdate2(ms_end_date,"dd-mmm-yyyy"),235959))
    JOIN (ce
    ...
    mf_cs72_preventioninterventionsskin, mf_cs72_woundinterventions,
    mf_cs72_pressureinterventionsskin,
   ...
   HEAD e.encntr_id
    ...
    ENDIF
    wound->encntr[wound->l_patcnt].f_encntrid = e.encntr_id, wound->encntr[wound->l_patcnt].
    f_personid = p.person_id, wound->encntr[wound->l_patcnt].s_patient_name = trim(p
  ...
  SELECT INTO "nl:"
   ...
   HEAD o.order_id
    ...
    ELSEIF (ml_wnd_cnt=10)
     wound->encntr[ml_loc].s_wound_care_order_date10 = format(o.orig_order_dt_tm,
      "mm/dd/yyyy hh:mm;;q")
    ENDIF
    IF (ml_wnd_cnt=1)
     wound->encntr[ml_loc].s_wound_care_orders_dates = format(o.orig_order_dt_tm,
      "mm/dd/yyyy hh:mm;;q")
    ELSEIF (ml_wnd_cnt > 1)
  ...
  IF (size(wound->encntr,5) > 0)
   SELECT INTO  $OUTDEV
    ...
    pressure_wound_date_identified = substring(1,100,wound->encntr[d1.seq].
     ...
     ->encntr[d1.seq].s_pressure_injury_present_on_admission), pressure_injury_stage = substring(1,
     100,wound->encntr[d1.seq].s_pressure_injury_stage),
    braden_score = substring(1,100,wound->encntr[d1.seq].s_braden_score_adult), braden_q_score_pedi
     = substring(1,100,wound->encntr[d1.seq].s_braden_q_score_pedi), braden_q_score_newborn =
    substring(1,100,wound->encntr[d1.seq].s_braden_q_score_newborn),
    braden_score_date = substring(1,100,wound->encntr[d1.seq].s_braden_score_date), mattress_device
     = substring(1,100,wound->encntr[d1.seq].s_mattress_device), therapeutic_bed_order = substring(1,
     100,wound->encntr[d1.seq].s_therapeutic_bed_order),
    ...
    maintenance_interventions_skin = substring(1,100,wound->encntr[d1.seq].
     s_maintenance_interventions_skin), pressure_interventions_skin = substring(1,100,wound->encntr[
     d1.seq].s_pressure_interventions_skin), wound_interventions_skin = substring(1,100,wound->
     encntr[d1.seq].s_wound_interventions_skin),
    surgical_wound_interventions = substring(1,100,wound->encntr[d1.seq].
     s_surgical_wound_interventions), diaper_dermatitis_interventions = substring(1,100,wound->
     encntr[d1.seq].s_diaper_dermatitis_interventions), adhesive_interventions = substring(1,100,
     wound->encntr[d1.seq].s_adhesive_interventions),
