CREATE PROGRAM cp_get_concept
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getentitydisplays(null) = null WITH protect
 SUBROUTINE getentitydisplays(null)
   CALL log_message("In GetEntityDisplays()",log_level_debug)
   ...
   DECLARE xsize = i4 WITH private, constant(size(reply->concept_list,5))
   DECLARE ysize = i4 WITH private, noconstant(0)
   DECLARE zsize = i4 WITH private, noconstant(0)
   IF (xsize > 0)
    FOR (x = 1 TO xsize)
      IF ((reply->concept_list[x].concept_cd > 0.0))
       SET ysize = size(reply->concept_list[x].cg_list,5)
       IF (ysize > 0)
        FOR (y = 1 TO ysize)
         SET zsize = size(reply->concept_list[x].cg_list[y].cg_dtl_list,5)
         IF (zsize > 0)
          FOR (z = 1 TO zsize)
            IF ((reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_name="ALT_SEL_CAT"))
             SET reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_disp =
             getentitydisplay_altselcat(reply->concept_list[x].cg_list[y].cg_dtl_list[z].
              concept_entity_id)
            ENDIF
            IF ((reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_name=
            "V500_EVENT_SET_CODE"))
             SET reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_disp =
             getentitydisplay_eventsetcd(reply->concept_list[x].cg_list[y].cg_dtl_list[z].
              concept_entity_id)
            ENDIF
            IF ((reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_name=
            "OUTCOME_CATALOG"))
             SET reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_disp =
             getentitydisplay_outcomecatalog(reply->concept_list[x].cg_list[y].cg_dtl_list[z].
              concept_entity_id)
            ENDIF
            IF ((reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_name="ORDER_CATALOG"
            ))
             SET reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_disp =
             getentitydisplay_ordercatalog(reply->concept_list[x].cg_list[y].cg_dtl_list[z].
              concept_entity_id)
            ENDIF
            IF ((reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_name="NOMENCLATURE")
            )
             SET reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_disp =
             getentitydisplay_nomenclature(reply->concept_list[x].cg_list[y].cg_dtl_list[z].
              concept_entity_id)
            ENDIF
            IF ((reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_name=
            "BR_DATAMART_FILTER"))
             SET reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_disp =
             getentitydisplay_datamartfilter(reply->concept_list[x].cg_list[y].cg_dtl_list[z].
              concept_entity_id)
            ENDIF
            IF ((reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_name="PAT_ED_RELTN")
            )
             SET reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_disp =
             getentitydisplay_patedreltn(reply->concept_list[x].cg_list[y].cg_dtl_list[z].
              concept_entity_ident)
            ENDIF
            IF ((reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_name="HM_EXPECT"))
             SET reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_disp =
             getentitydisplay_hmexpect(reply->concept_list[x].cg_list[y].cg_dtl_list[z].
              concept_entity_id)
            ENDIF
            IF ((reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_name=
            "MLTM_DRUG_CATEGORIES"))
             SET reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_disp =
             getentitydisplay_drugcats(reply->concept_list[x].cg_list[y].cg_dtl_list[z].
              concept_entity_id)
            ENDIF
            IF ((reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_name="CODE_VALUE"))
             SET reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_disp =
             uar_get_code_display(reply->concept_list[x].cg_list[y].cg_dtl_list[z].concept_entity_id)
            ENDIF
          ENDFOR
         ENDIF
        ENDFOR
    ...
    ENDFOR
   ENDIF
   CALL log_message(build("Exit GetEntityDisplays(), Elapsed time in seconds:",((curtime3 -
 ...
 SUBROUTINE (getentitydisplay_outcomecatalog(val=f8) =vc WITH protect)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     val_display = trim(oc.description)
    WITH nocounter
   ;end select
 ...
 DECLARE conceptcdlistsize = i4 WITH noconstant(size(request->concept_cds,5)), protect
 ...
 CALL echo("GetEntityDisplays...")
 CALL getentitydisplays(null)
 SET reply->status_data.status = "S"
 SUBROUTINE verifyinputs(null)
   CALL log_message("In verifyInputs()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE x = i4 WITH noconstant(0), private
   IF (conceptcdlistsize <= 0)
    CALL reporterrorandexit("PERFORM","Validate","Concept Cd list size must be > 0")
   ENDIF
   FOR (x = 1 TO conceptcdlistsize)
     IF (size(request->concept_cds[x].intention_list,5)=0)
      SET stat = alterlist(request->concept_cds[x].intention_list,1)
     ENDIF
   ENDFOR
 ...
 SUBROUTINE getconcepts(null)
   ...
   SET stat = alterlist(reply->concept_list,size(request->concept_cds,5))
   FOR (x = 1 TO size(request->concept_cds,5))
     SET reply->concept_list[x].concept_cd = request->concept_cds[x].concept_cd
   ENDFOR
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = value(conceptcdlistsize)),
     (dummyt d2  WITH seq = 1),
     cp_concept_group ccg,
     cp_concept_group_dtl ccgd,
     cp_concept_group_dtl_attr ccgda
    PLAN (d1
     WHERE maxrec(d2,size(request->concept_cds[d1.seq].intention_list,5)))
     JOIN (d2)
     JOIN (ccg
     WHERE (ccg.concept_cd=request->concept_cds[d1.seq].concept_cd)
      AND (((ccg.intention_cd=request->concept_cds[d1.seq].intention_list[d2.seq].intention_cd)) OR (
     ccg.intention_cd > 0
      AND (request->concept_cds[d1.seq].intention_list[d2.seq].intention_cd=0.0))) )
     JOIN (ccgd
     WHERE (ccgd.cp_concept_group_id= Outerjoin(ccg.cp_concept_group_id)) )
     JOIN (ccgda
     WHERE (ccgda.cp_concept_group_dtl_id= Outerjoin(ccgd.cp_concept_group_dtl_id)) )
    ORDER BY ccg.concept_cd, ccgd.cp_concept_group_id, ccgd.cp_concept_group_dtl_id,
     ccgda.cp_concept_group_dtl_attr_id
    HEAD REPORT
     ccnt = 0
    HEAD ccg.concept_cd
     cgcnt = 0, ccnt += 1
     IF (ccnt > size(reply->concept_list,5))
      stat = alterlist(reply->concept_list,(ccnt+ 5))
     ENDIF
     reply->concept_list[ccnt].concept_cd = ccg.concept_cd
    HEAD ccg.cp_concept_group_id
     cdcnt = 0, cgcnt += 1
     IF (cgcnt > size(reply->concept_list[ccnt].cg_list,5))
      stat = alterlist(reply->concept_list[ccnt].cg_list,(cgcnt+ 5))
     ENDIF
     CALL echo(uar_get_code_meaning(ccg.concept_group_cd)), reply->concept_list[ccnt].cg_list[cgcnt].
     cp_concept_group_id = ccg.cp_concept_group_id, reply->concept_list[ccnt].cg_list[cgcnt].
     concept_group_cd = ccg.concept_group_cd,
     reply->concept_list[ccnt].cg_list[cgcnt].concept_group_cd_disp = uar_get_code_display(ccg
      .concept_group_cd), reply->concept_list[ccnt].cg_list[cgcnt].concept_group_cd_mean =
     uar_get_code_meaning(ccg.concept_group_cd), reply->concept_list[ccnt].cg_list[cgcnt].
     intention_cd = ccg.intention_cd,
     reply->concept_list[ccnt].cg_list[cgcnt].rule_ind = ccg.rule_ind
    HEAD ccgd.cp_concept_group_dtl_id
     cacnt = 0
     IF (ccgd.cp_concept_group_dtl_id > 0)
      cdcnt += 1
      IF (cdcnt > size(reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list,5))
       stat = alterlist(reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list,(cdcnt+ 5))
      ENDIF
      reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].cp_concept_group_dtl_id = ccgd
      .cp_concept_group_dtl_id, reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].
      concept_entity_id = ccgd.concept_entity_id, reply->concept_list[ccnt].cg_list[cgcnt].
      cg_dtl_list[cdcnt].concept_entity_ident = ccgd.concept_entity_ident,
      reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].concept_entity_name = ccgd
      .concept_entity_name, reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].
      concept_entity_text = ccgd.concept_entity_text
     ENDIF
    DETAIL
     IF (ccgda.cp_concept_group_dtl_attr_id > 0)
      cacnt += 1
      IF (cacnt > size(reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].cg_dtl_attr_list,5
       ))
       stat = alterlist(reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].cg_dtl_attr_list,
        (cacnt+ 5))
      ENDIF
      reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].cg_dtl_attr_list[cacnt].
      cp_concept_group_dtl_attr_id = ccgda.cp_concept_group_dtl_attr_id, reply->concept_list[ccnt].
      cg_list[cgcnt].cg_dtl_list[cdcnt].cg_dtl_attr_list[cacnt].attribute_entity_id = ccgda
      .attribute_entity_id, reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].
      cg_dtl_attr_list[cacnt].attribute_entity_name = ccgda.attribute_entity_name,
      reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].cg_dtl_attr_list[cacnt].
      attribute_entity_text = ccgda.attribute_entity_text, reply->concept_list[ccnt].cg_list[cgcnt].
      cg_dtl_list[cdcnt].cg_dtl_attr_list[cacnt].detail_attribute_type_cd = ccgda
      .detail_attribute_type_cd, reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].
      cg_dtl_attr_list[cacnt].detail_attribute_type_mean = uar_get_code_meaning(ccgda
       .detail_attribute_type_cd),
      reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].cg_dtl_attr_list[cacnt].
      detail_attribute_type_disp = uar_get_code_display(ccgda.detail_attribute_type_cd)
      IF (ccgda.attribute_entity_name="CODE_VALUE")
       reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].cg_dtl_attr_list[cacnt].
       attribute_entity_text = uar_get_code_display(ccgda.attribute_entity_id)
      ENDIF
     ENDIF
    FOOT  ccgd.cp_concept_group_dtl_id
     IF (cacnt > 0)
      stat = alterlist(reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list[cdcnt].cg_dtl_attr_list,
       cacnt)
     ENDIF
    FOOT  ccgd.cp_concept_group_id
     IF (cdcnt > 0)
      stat = alterlist(reply->concept_list[ccnt].cg_list[cgcnt].cg_dtl_list,cdcnt)
     ENDIF
    FOOT  ccg.concept_cd
     stat = alterlist(reply->concept_list[ccnt].cg_list,cgcnt)
    FOOT REPORT
     stat = alterlist(reply->concept_list,ccnt)
    WITH nocounter
   ;end select
   CALL error_and_zero_check_rec(curqual,"GETCONCEPTS","Get Concepts",1,0,
    reply)
