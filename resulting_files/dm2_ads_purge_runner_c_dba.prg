CREATE PROGRAM dm2_ads_purge_runner_c:dba
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_ads_purge_runner_c",".log","dm2_ads_purge_runner_c")=0)
  GO TO end_program
 ENDIF
 DECLARE ddr_purge_cnt = i4 WITH protect, noconstant(0)
 SET dm_err->eproc = "Starting dm2_ads_purge_runner_c"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Updating batch of rows to RUNNING"
 UPDATE  FROM dm2_ddl_ops_log ddol,
   (dummyt d  WITH seq = request->exec_cnt)
  SET ddol.status = "RUNNING", ddol.begin_dt_tm = cnvtdatetime(curdate,curtime3), ddol.logfile_name
    = dm_err->logfile,
   ddol.appl_id = currdbhandle, ddol.updt_cnt = (ddol.updt_cnt+ 1), ddol.updt_dt_tm = cnvtdatetime(
    curdate,curtime3)
  PLAN (d)
   JOIN (ddol
   WHERE (ddol.op_id=request->exec[d.seq].op_id))
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = build("OP_ID: ",request->exec[ddr_cnt].op_id," not found")
  CALL disp_msg("",dm_err->logfile,0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Running deletes..."
 CALL disp_msg("",dm_err->logfile,0)
 FOR (ddr_cnt = 1 TO request->exec_cnt)
   SET dm_err->eproc = build("Executing delete command for OP_ID ",request->exec[ddr_cnt].op_id)
   SET request->exec[ddr_cnt].operation = concat("rdb asis (^",replace(request->exec[ddr_cnt].
     operation,"<<TGTDBLINK>>","@PURGE_TGT"),"^) go")
   IF (dm2_push_cmd(request->exec[ddr_cnt].operation,1)=0)
    ROLLBACK
    SET dm_err->eproc = build("Updating OP_ID ",request->exec[ddr_cnt].op_id," to FAILED")
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = "FAILED", ddol.end_dt_tm = cnvtdatetime(curdate,curtime3), ddol.logfile_name
       = dm_err->logfile,
      ddol.error_msg = dm_err->emsg, ddol.updt_cnt = (ddol.updt_cnt+ 1), ddol.updt_dt_tm =
      cnvtdatetime(curdate,curtime3)
     WHERE (ddol.op_id=request->exec[ddr_cnt].op_id)
     WITH nocounter
    ;end update
    COMMIT
    IF (check_error(dm_err->eproc)=1)
   ...
   ELSE
    COMMIT
    IF ((request->exec[ddr_cnt].priority >= 1002))
     SET ddr_purge_cnt = curqual
     UPDATE  FROM dm_ads_purge_hist daph
      SET daph.purge_count = (daph.purge_count+ ddr_purge_cnt), daph.updt_cnt = (daph.updt_cnt+ 1),
       daph.updt_dt_tm = cnvtdatetime(curdate,curtime3),
       daph.updt_applctx = reqinfo->updt_applctx, daph.updt_id = reqinfo->updt_id, daph.updt_task =
       reqinfo->updt_task
      WHERE (daph.dm_ads_config_id=request->config_id)
       AND (daph.table_name=request->exec[ddr_cnt].table_name)
      WITH nocounter
     ;end update
     IF (curqual=0)
      INSERT  FROM dm_ads_purge_hist daph
       SET daph.dm_ads_purge_hist_id = seq(dm_clinical_seq,nextval), daph.dm_ads_config_id = request
        ->config_id, daph.config_name = request->config_name,
        daph.table_name = request->exec[ddr_cnt].table_name, daph.purge_count = ddr_purge_cnt, daph
        .purge_dt_tm = cnvtdatetime(curdate,curtime3),
        daph.purge_user = curuser, daph.updt_applctx = 0, daph.updt_cnt = 0,
        daph.updt_dt_tm = cnvtdatetime(curdate,curtime3), daph.updt_task = reqinfo->updt_task, daph
        .updt_id = reqinfo->updt_id
       WITH nocounter
      ;end insert
     ENDIF
    ENDIF
    SET dm_err->eproc = build("Updating OP_ID ",request->exec[ddr_cnt].op_id," to COMPLETE")
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = "COMPLETE", ddol.end_dt_tm = cnvtdatetime(curdate,curtime3), ddol.logfile_name
       = dm_err->logfile,
      ddol.updt_cnt = (ddol.updt_cnt+ 1), ddol.updt_dt_tm = cnvtdatetime(curdate,curtime3)
     WHERE (ddol.op_id=request->exec[ddr_cnt].op_id)
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
 ...
 ENDFOR
#end_program
 SET dm_err->eproc = "Ending dm2_ads_purge_runner_c"
 CALL final_disp_msg(" ")
END GO
