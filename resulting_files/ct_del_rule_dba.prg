CREATE PROGRAM ct_del_rule:dba
 ...
 CALL del_ct_rule(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE del_ct_rule(del_begin,del_end)
   FOR (x = del_begin TO del_end)
     ...
     UPDATE  FROM ct_rule c
      SET c.active_ind = false, c.active_status_cd = nullcheck(active_code,request->ct_rule[x].
        active_status_cd,
        IF ((request->ct_rule[x].active_status_cd=0)) 0
        ELSE 1
        ENDIF
