CREATE PROGRAM dm_rc_cloud_transaction_rows:dba
 ...
 SET reply->table_name = "RC_CLOUD_TRANSACTION"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(rct.rc_cloud_transaction_id)
   FROM rc_cloud_transaction rct
   WHERE rct.rc_cloud_transaction_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(rct.rc_cloud_transaction_id)
  FROM rc_cloud_transaction rct
  WHERE rct.rc_cloud_transaction_id > 0.0
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM rc_cloud_transaction rct
    WHERE rct.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND rct.rc_cloud_transaction_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("rct.rc_cloud_transaction_id != 0","rct"))
    DETAIL
     row_cnt += 1
     IF (mod(row_cnt,100)=1)
      stat = alterlist(reply->rows,(row_cnt+ 99))
     ENDIF
     reply->rows[row_cnt].row_id = rct.rowid
    WITH nocounter, maxqual(rct,value(rowsleft))
   ;end select
