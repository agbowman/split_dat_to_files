CREATE PROGRAM dash_rdm_import_filter:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Config String:" = "",
  "Dashboard ID:" = 0.0
  WITH outdev, configstr, dashboardid
 ...
 SET log_program_name = "DASH_RDM_IMPORT_FILTER"
 CALL log_message(concat("Begin script ",log_program_name))
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed during script DASH_RDM_IMPORT_FILTER..."
 ...
 SUBROUTINE importdata(null)
   ...
   IF (ok > 0)
    ...
    WHILE (lpos > 0)
      ...
      IF (ok > 0)
       CALL processfilter(listitem,0.0)
      ELSE
       SET ok = findattrbyname(listitem,"filterGroup",filtergroup)
       IF (ok > 0)
        CALL processfiltergroup(listitem)
       ELSE
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE processfilter(listitem,filtergroupid)
   DECLARE ok = i4 WITH protect, noconstant(0)
   DECLARE filterdef = vc WITH protect, noconstant("")
 ...
 END ;Subroutine
 SUBROUTINE processfiltergroup(listitem)
   DECLARE ok = i4 WITH protect, noconstant(0)
   ...
   IF (ok > 0)
    ...
    WHILE (lpos > 0)
      SET ok = findattrbyname(listitem,"filterDef",filterdef)
      IF (ok > 0)
       CALL processfilter(listitem,filtergroupid)
      ELSE
       CALL logandexit("I was unable to find the filter definition section.")
      ENDIF
      SET lpos = getnextlistitem(filters,lpos,listitem)
