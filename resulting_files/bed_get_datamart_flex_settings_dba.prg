CREATE PROGRAM bed_get_datamart_flex_settings:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (v
   ...
   JOIN (cvp
   WHERE cvp.code_value=f1.parent_entity_id
    AND cvp.active_ind=1)
   JOIN (cvl
   WHERE cvl.code_value=f2.parent_entity_id)
  ...
  HEAD f1.br_datamart_flex_id
   ...
   ELSEIF (f1.grouper_ind=1
    AND f1.grouper_flex_id > 0
    AND cvl.active_ind=1)
    flex_group_cnt = (flex_group_cnt+ 1), stat = alterlist(reply->flex_groups,flex_group_cnt), reply
    ->flex_groups[flex_group_cnt].parent_flex.parent_flex_id = f2.br_datamart_flex_id,
    reply->flex_groups[flex_group_cnt].parent_flex.parent_entity_name = f2.parent_entity_name, reply
    ->flex_groups[flex_group_cnt].parent_flex.parent_entity_id = f2.parent_entity_id, reply->
    flex_groups[flex_group_cnt].parent_flex.parent_entity_type_flag = f2.parent_entity_type_flag,
    reply->flex_groups[flex_group_cnt].child_flex.child_flex_id = f1.br_datamart_flex_id, reply->
    flex_groups[flex_group_cnt].child_flex.parent_entity_name = f1.parent_entity_name, reply->
    flex_groups[flex_group_cnt].child_flex.parent_entity_id = f1.parent_entity_id,
    reply->flex_groups[flex_group_cnt].child_flex.parent_entity_type_flag = f1
    .parent_entity_type_flag
   ENDIF
  WITH nocounter
 ;end select
 CALL bederrorcheck("Error 01: Unable to retrieve flexed settings")
#exit_script
