CREATE PROGRAM bed_aud_hlx_refrange:dba
 ...
 DECLARE fmtrefrange(nmindigits=i4,nmaxdigits=i4,nmindec=i4,drawval=f8,bscinotind=i2) = vc WITH
 protect
 SUBROUTINE fmtrefrange(nmindigits,nmaxdigits,nmindec,drawval,bscinotind)
   RETURN(uar_accformatresultstringscinote(nmindigits,nmaxdigits,nmindec,drawval,0,
    bscinotind))
 END ;Subroutine
 DECLARE fmtrefrangefromtemp(ndlistndx=i4,nrrfndx=i4,drawval=f8) = vc WITH protect
 SUBROUTINE fmtrefrangefromtemp(ndlistndx,nrrfndx,drawval)
   RETURN(fmtrefrange(temp->dlist[ndlistndx].rrf[nrrfndx].min_digits,temp->dlist[ndlistndx].rrf[
    nrrfndx].max_digits,temp->dlist[ndlistndx].rrf[nrrfndx].min_decimal_places,drawval,temp->dlist[
    ndlistndx].sci_notation_ind))
 END ;Subroutine
 SET serv_res_cnt = size(request->service_resources,5)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (c
   WHERE c.code_set=106
    AND c.cdf_meaning="HLX"
    AND c.active_ind=1)
  DETAIL
   hlx_cd = c.code_value, hlx_disp = c.display
  WITH nocounter
 ;end select
 DECLARE days_cd = f8
 DECLARE hours_cd = f8
 ...
 IF ((request->skip_volume_check_ind=0))
  IF (serv_res_cnt > 0)
   SELECT INTO "nl:"
    ...
    PLAN (dta
     WHERE dta.activity_type_cd=hlx_cd
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dta
   WHERE dta.active_ind=1
    AND dta.activity_type_cd=hlx_cd)
   JOIN (cvdta
  ...
  HEAD dta.task_assay_cd
   ...
   dta_result_type = rt.display, temp->dlist[tcnt].sci_notation_ind = dta.sci_notation_ind
  WITH nocounter
 ;end select
 IF (tcnt=0)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (found_ind=1)
    ...
    IF (rrf.normal_ind=1)
     temp->dlist[d.seq].rrf[rcnt].normal_low = rrf.normal_low
    ELSEIF (rrf.normal_ind=2)
     temp->dlist[d.seq].rrf[rcnt].normal_high = rrf.normal_high
    ELSEIF (rrf.normal_ind=3)
     temp->dlist[d.seq].rrf[rcnt].normal_low = rrf.normal_low, temp->dlist[d.seq].rrf[rcnt].
     normal_high = rrf.normal_high
    ENDIF
    temp->dlist[d.seq].rrf[rcnt].critical_ind = rrf.critical_ind
    IF (rrf.critical_ind=1)
     temp->dlist[d.seq].rrf[rcnt].critical_low = rrf.critical_low
    ELSEIF (rrf.critical_ind=2)
     temp->dlist[d.seq].rrf[rcnt].critical_high = rrf.critical_high
    ELSEIF (rrf.critical_ind=3)
     temp->dlist[d.seq].rrf[rcnt].critical_low = rrf.critical_low, temp->dlist[d.seq].rrf[rcnt].
     critical_high = rrf.critical_high
    ENDIF
    temp->dlist[d.seq].rrf[rcnt].review_ind = rrf.review_ind
    IF (rrf.review_ind=1)
     temp->dlist[d.seq].rrf[rcnt].review_low = rrf.review_low
    ELSEIF (rrf.review_ind=2)
     temp->dlist[d.seq].rrf[rcnt].review_high = rrf.review_high
    ELSEIF (rrf.review_ind=3)
     temp->dlist[d.seq].rrf[rcnt].review_low = rrf.review_low, temp->dlist[d.seq].rrf[rcnt].
     review_high = rrf.review_high
    ENDIF
    temp->dlist[d.seq].rrf[rcnt].linear_ind = rrf.linear_ind
    IF (rrf.linear_ind=1)
     temp->dlist[d.seq].rrf[rcnt].linear_low = rrf.linear_low
    ELSEIF (rrf.linear_ind=2)
     temp->dlist[d.seq].rrf[rcnt].linear_high = rrf.linear_high
    ELSEIF (rrf.linear_ind=3)
     temp->dlist[d.seq].rrf[rcnt].linear_low = rrf.linear_low, temp->dlist[d.seq].rrf[rcnt].
     linear_high = rrf.linear_high
    ENDIF
    temp->dlist[d.seq].rrf[rcnt].feasible_ind = rrf.feasible_ind
    IF (rrf.feasible_ind=1)
     temp->dlist[d.seq].rrf[rcnt].feasible_low = rrf.feasible_low
    ELSEIF (rrf.feasible_ind=2)
     temp->dlist[d.seq].rrf[rcnt].feasible_high = rrf.feasible_high
    ELSEIF (rrf.feasible_ind=3)
     temp->dlist[d.seq].rrf[rcnt].feasible_low = rrf.feasible_low, temp->dlist[d.seq].rrf[rcnt].
     feasible_high = rrf.feasible_high
    ENDIF
...
#skip_processing
 ...
 SET stat = alterlist(reply->collist,37)
 ...
 SET reply->collist[22].header_text = "Alpha Response/Chart Name"
 SET reply->collist[22].data_type = 1
 SET reply->collist[22].hide_ind = 0
 SET reply->collist[23].header_text = "MDI Alias"
 SET reply->collist[23].data_type = 1
 SET reply->collist[23].hide_ind = 0
 SET reply->collist[24].header_text = "Unique Identifier"
 SET reply->collist[24].data_type = 1
 SET reply->collist[24].hide_ind = 0
 SET reply->collist[25].header_text = "Vocabulary"
 SET reply->collist[25].data_type = 1
 SET reply->collist[25].hide_ind = 0
 SET reply->collist[26].header_text = "Alpha Default"
 SET reply->collist[26].data_type = 1
 SET reply->collist[26].hide_ind = 0
 SET reply->collist[27].header_text = "Reference"
 SET reply->collist[27].data_type = 1
 SET reply->collist[27].hide_ind = 0
 SET reply->collist[28].header_text = "Report Value"
 SET reply->collist[28].data_type = 1
 SET reply->collist[28].hide_ind = 0
 SET reply->collist[29].header_text = "Use Units of Measure"
 SET reply->collist[29].data_type = 1
 SET reply->collist[29].hide_ind = 0
 SET reply->collist[30].header_text = "Units of Measure"
 SET reply->collist[30].data_type = 1
 SET reply->collist[30].hide_ind = 0
 SET reply->collist[31].header_text = "Numeric Map Minimum Digits"
 SET reply->collist[31].data_type = 1
 SET reply->collist[31].hide_ind = 0
 SET reply->collist[32].header_text = "Numeric Map Maximum Digits"
 SET reply->collist[32].data_type = 1
 SET reply->collist[32].hide_ind = 0
 SET reply->collist[33].header_text = "Numeric Map Minimum Decimal Place"
 SET reply->collist[33].data_type = 1
 SET reply->collist[33].hide_ind = 0
 SET reply->collist[34].header_text = "Scientific Notation"
 SET reply->collist[34].data_type = 1
 SET reply->collist[34].hide_ind = 0
 SET reply->collist[35].header_text = "task_assay_cd"
 SET reply->collist[35].data_type = 1
 SET reply->collist[35].hide_ind = 1
 SET reply->collist[36].header_text = "service_resource_cd"
 SET reply->collist[36].data_type = 1
 SET reply->collist[36].hide_ind = 1
 SET reply->collist[37].header_text = "reference_range_factor_id"
 SET reply->collist[37].data_type = 1
 SET reply->collist[37].hide_ind = 1
 SET row_nbr = 0
 FOR (x = 1 TO tcnt)
  SET rcnt = size(temp->dlist[x].rrf,5)
  IF (((serv_res_cnt=0) OR (serv_res_cnt > 0
   AND rcnt > 0)) )
   SET skip_nrrf_ind = 0
   ...
   FOR (y = 1 TO rcnt)
     SET found_ind = 1
     IF (serv_res_cnt > 0
      AND (temp->dlist[x].rrf[y].service_resource="All"))
      SET found_ind = 0
      SELECT INTO "NL:"
     ...
     IF (found_ind=1)
      IF ((temp->dlist[x].rrf[y].skip_ind=0))
       ...
       IF ((temp->dlist[x].rrf[y].normal_ind=1))
        SET temp->dlist[x].rrf[y].nl_disp = fmtrefrangefromtemp(x,y,temp->dlist[x].rrf[y].normal_low)
       ELSEIF ((temp->dlist[x].rrf[y].normal_ind=2))
        SET temp->dlist[x].rrf[y].nh_disp = fmtrefrangefromtemp(x,y,temp->dlist[x].rrf[y].normal_high
         )
       ...
       IF ((temp->dlist[x].rrf[y].critical_ind=1))
        SET temp->dlist[x].rrf[y].cl_disp = fmtrefrangefromtemp(x,y,temp->dlist[x].rrf[y].
         critical_low)
       ELSEIF ((temp->dlist[x].rrf[y].critical_ind=2))
        SET temp->dlist[x].rrf[y].ch_disp = fmtrefrangefromtemp(x,y,temp->dlist[x].rrf[y].
         critical_high)
       ...
       IF ((temp->dlist[x].rrf[y].review_ind=1))
        SET temp->dlist[x].rrf[y].rl_disp = fmtrefrangefromtemp(x,y,temp->dlist[x].rrf[y].review_low)
       ELSEIF ((temp->dlist[x].rrf[y].review_ind=2))
        SET temp->dlist[x].rrf[y].rh_disp = fmtrefrangefromtemp(x,y,temp->dlist[x].rrf[y].review_high
         )
       ...
       IF ((temp->dlist[x].rrf[y].linear_ind=1))
        SET temp->dlist[x].rrf[y].ll_disp = fmtrefrangefromtemp(x,y,temp->dlist[x].rrf[y].linear_low)
       ELSEIF ((temp->dlist[x].rrf[y].linear_ind=2))
        SET temp->dlist[x].rrf[y].lh_disp = fmtrefrangefromtemp(x,y,temp->dlist[x].rrf[y].linear_high
         )
       ...
       IF ((temp->dlist[x].rrf[y].feasible_ind=1))
        SET temp->dlist[x].rrf[y].fl_disp = fmtrefrangefromtemp(x,y,temp->dlist[x].rrf[y].
         feasible_low)
       ELSEIF ((temp->dlist[x].rrf[y].feasible_ind=2))
        SET temp->dlist[x].rrf[y].fh_disp = fmtrefrangefromtemp(x,y,temp->dlist[x].rrf[y].
         feasible_high)
       ...
       SET stat = alterlist(reply->rowlist[row_nbr].celllist,37)
       ...
       ELSE
        SET reply->rowlist[row_nbr].celllist[5].string_value = trim(temp->dlist[x].rrf[y].
         sr_result_type)
       ENDIF
       SET reply->rowlist[row_nbr].celllist[6].string_value = concat(temp->dlist[x].rrf[y].af_disp,
        " ",temp->dlist[x].rrf[y].age_from_unit)
       SET reply->rowlist[row_nbr].celllist[7].string_value = concat(temp->dlist[x].rrf[y].at_disp,
        " ",temp->dlist[x].rrf[y].age_to_unit)
       SET reply->rowlist[row_nbr].celllist[8].string_value = trim(temp->dlist[x].rrf[y].
        unknown_age_str)
       SET reply->rowlist[row_nbr].celllist[9].string_value = trim(temp->dlist[x].rrf[y].sex)
       SET reply->rowlist[row_nbr].celllist[10].string_value = trim(temp->dlist[x].rrf[y].
        specimen_type)
       SET reply->rowlist[row_nbr].celllist[11].string_value = trim(temp->dlist[x].rrf[y].species)
       ...
       SET reply->rowlist[row_nbr].celllist[30].string_value = trim(temp->dlist[x].rrf[y].units)
       SET reply->rowlist[row_nbr].celllist[31].string_value = cnvtstring(temp->dlist[x].rrf[y].
        min_digits)
       SET reply->rowlist[row_nbr].celllist[32].string_value = cnvtstring(temp->dlist[x].rrf[y].
        max_digits)
       SET reply->rowlist[row_nbr].celllist[33].string_value = cnvtstring(temp->dlist[x].rrf[y].
        min_decimal_places)
       IF (temp->dlist[x].sci_notation_ind)
        SET reply->rowlist[row_nbr].celllist[34].string_value = "Yes"
       ENDIF
       IF ((temp->dlist[x].task_assay_cd > 0))
        SET reply->rowlist[row_nbr].celllist[35].string_value = cnvtstring(temp->dlist[x].
         task_assay_cd)
       ELSE
        SET reply->rowlist[row_nbr].celllist[35].string_value = " "
       ENDIF
       IF ((temp->dlist[x].rrf[y].service_resource_cd > 0))
        SET reply->rowlist[row_nbr].celllist[36].string_value = cnvtstring(temp->dlist[x].rrf[y].
         service_resource_cd)
       ELSE
        SET reply->rowlist[row_nbr].celllist[36].string_value = " "
       ENDIF
       IF ((temp->dlist[x].rrf[y].reference_range_factor_id > 0))
        SET reply->rowlist[row_nbr].celllist[37].string_value = cnvtstring(temp->dlist[x].rrf[y].
       ...
       FOR (z = 1 TO acnt)
         ...
         SET reply->rowlist[row_nbr].celllist[4].string_value = trim(temp->dlist[x].rrf[y].
          service_resource)
         SET reply->rowlist[row_nbr].celllist[5].string_value = "Alpha Response"
         SET reply->rowlist[row_nbr].celllist[22].string_value = trim(temp->dlist[x].rrf[y].al[z].
          term)
         SET reply->rowlist[row_nbr].celllist[23].string_value = trim(temp->dlist[x].rrf[y].al[z].
          mnemonic)
         SET reply->rowlist[row_nbr].celllist[24].string_value = trim(temp->dlist[x].rrf[y].al[z].
          long_desc)
         SET reply->rowlist[row_nbr].celllist[25].string_value = trim(temp->dlist[x].rrf[y].al[z].
          vocab)
         SET reply->rowlist[row_nbr].celllist[26].string_value = trim(temp->dlist[x].rrf[y].al[z].
          default)
         SET reply->rowlist[row_nbr].celllist[27].string_value = trim(temp->dlist[x].rrf[y].al[z].
          reference)
         SET reply->rowlist[row_nbr].celllist[28].string_value = trim(temp->dlist[x].rrf[y].al[z].
          result_process)
         SET reply->rowlist[row_nbr].celllist[29].string_value = trim(temp->dlist[x].rrf[y].al[z].
          use_units)
       ENDFOR
      ENDIF
     ENDIF
   ...
   IF (nrcnt > 0
    ...
    FOR (y = 1 TO 1)
      ...
      SET reply->rowlist[row_nbr].celllist[2].string_value = trim(temp->dlist[x].dtaname)
      SET reply->rowlist[row_nbr].celllist[3].string_value = trim(temp->dlist[x].dtadesc)
      SET reply->rowlist[row_nbr].celllist[4].string_value = trim(temp->dlist[x].norrf[y].
       service_resource)
      IF ((temp->dlist[x].norrf[y].sr_result_type="(None)"))
       SET reply->rowlist[row_nbr].celllist[5].string_value = " "
      ELSE
       SET reply->rowlist[row_nbr].celllist[5].string_value = trim(temp->dlist[x].norrf[y].
        sr_result_type)
      ENDIF
      IF ((temp->dlist[x].task_assay_cd > 0))
       SET reply->rowlist[row_nbr].celllist[35].string_value = cnvtstring(temp->dlist[x].
   ...
   IF (rcnt=0
    ...
    SET reply->rowlist[row_nbr].celllist[3].string_value = trim(temp->dlist[x].dtadesc)
    IF ((temp->dlist[x].task_assay_cd > 0))
     SET reply->rowlist[row_nbr].celllist[35].string_value = cnvtstring(temp->dlist[x].task_assay_cd)
    ELSE
     SET reply->rowlist[row_nbr].celllist[35].string_value = " "
    ENDIF
   ENDIF
...
#exit_script
 ...
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("hlx_refrange_audit.csv")
 ENDIF
