CREATE PROGRAM dm2_ref_data_mover:dba
 ...
 DECLARE id = f8
 DECLARE pre_link = vc
 DECLARE post_link = vc
 DECLARE source_id = f8
 DECLARE target_id = f8
 DECLARE dbase_con_str = vc
 IF (check_logfile("dm2_ref_data_mover",".log","DM2_REF_DATA_MOVER LOGFILE")=0)
  SET dm_err->err_ind = 1
 ...
 IF ((xxcclseclogin->loggedin != 1))
  SET dm_err->eproc = "Starting dm2_ref_data_mover"
...
#redo
 ...
 SELECT INTO "NL:"
  ...
  PLAN (di
   ...
   WHERE der.child_env_id=di.info_number
    ...
    AND der.parent_env_id=cnvtint(id))
  DETAIL
   pre_link = der.pre_link_name, post_link = der.post_link_name, source_id = der.parent_env_id,
   target_id = der.child_env_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (pre_link=""
  ...
  IF (currdb="ORACLE")
   ...
   SELECT INTO "NL:"
    ...
    WHERE d.info_domain="DATA MANAGEMENT"
     AND d.info_name="DM_ENV_ID"
    DETAIL
     target_id = d.info_number
    WITH nocounter
   ;end select
   SET post_link = concat("@MERGE",trim(cnvtstring(source_id)),trim(cnvtstring(target_id)))
   SET dbase_con_str = concat("DM_CHG_LOG",post_link)
   SELECT INTO "NL:"
    FROM (parser(dbase_con_str) d)
    WHERE d.target_env_id=target_id
  ...
  ELSE
   SET dm_err->eproc = "Starting dm2_ref_data_mover"
   SET dm_err->emsg =
   "Please run DM2_SETUP_REF_DATA_MOVER and try again. Pre_link_name and Post_link_name aren't filled out."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ELSE
  SET dbase_con_str = concat(value(pre_link),"DM_CHG_LOG",value(post_link))
  SELECT INTO "NL:"
   FROM (parser(dbase_con_str) d)
   WHERE d.target_env_id=target_id
    AND d.log_type="REFCHG"
 ...
 SET validate = off
 SET help = off
 SET str = concat("Do you want to start the merge with ",trim(cnvtstring(curaccept)),
  " as your source id. (Y/N)")
 CALL text(9,10,str)
 CALL accept(9,(72+ size(trim(cnvtstring(curaccept)))),"P;CU","Y"
  WHERE curaccept IN ("Y", "N"))
 IF (curaccept="N")
  GO TO redo
 ELSE
  SET message = nowindow
  SET dm_err->eproc = "EXECUTING DM2_REF_DATA_MOVER_MAIN"
 ENDIF
 SET dbase_con_str = concat(value(pre_link),"v$database",value(post_link))
 CALL parser(concat('select into "NL:" name from ',dbase_con_str," with nocounter go"),1)
 IF (curqual=0)
  SET dm_err->eproc = "Starting dm2_ref_data_mover"
  SET dm_err->emsg =
  "Please run DM2_SETUP_REF_DATA_MOVER and try again.  The database connection failed."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "EXECUTING DM2_REF_DATA_MOVER_MAIN"
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_ref_data_mover_main id, 0, 0
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "EXITING THE DM2_REF_DATA_MOVER"
 CALL echo("*")
 CALL echo("*")
 CALL echo("*")
 CALL echo("********************************************************************************")
 CALL echo("*")
 CALL echo("Exiting the DM2_REF_DATA_MOVER")
 CALL echo("*")
