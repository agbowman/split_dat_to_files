CREATE PROGRAM cv_frpt_avg_wait_drv:dba
 PROMPT
 ...
 SELECT
  IF (( $ORG_ID=0.0)
   ...
   PLAN (c
    ...
    WHERE cs.cv_proc_id=c.cv_proc_id
     AND cs.perf_start_dt_tm != null)
    JOIN (css
    WHERE cs.cv_step_id=css.cv_step_id
     AND css.arrive_dt_tm <= cs.perf_start_dt_tm)
    JOIN (p
    WHERE p.person_id=cs.perf_provider_id)
  ELSEIF (( $ORG_ID=0.0)
   AND org_sec_ind=1.0)
   FROM cv_proc c,
    cv_step cs,
  ...
  ENDIF
  INTO "NL:"
  p.name_full_formatted, c_catalog_disp = uar_get_code_display(c.catalog_cd)
  ORDER BY c_catalog_disp, p.name_full_formatted
  HEAD REPORT
  ...
  DETAIL
   ...
   IF (mod(nlistcnt,10)=1)
    stat = alterlist(reply_obj->cv_list,(nlistcnt+ 9)), stat = alterlist(audit_record->qual,(nlistcnt
     + 9))
   ENDIF
   reply_obj->cv_list[nlistcnt].rpl_catalog_disp = trim(c_catalog_disp), reply_obj->cv_list[nlistcnt]
   .rpl_full_name = trim(p.name_full_formatted), reply_obj->cv_list[nlistcnt].rpl_wait_minutes =
   IF (curutc=1) datetimediff(cs.perf_start_dt_tm,css.arrive_dt_tm,4)
   ELSE datetimediff(cnvtdatetimeutc(cs.perf_start_dt_tm,3),cnvtdatetimeutc(css.arrive_dt_tm,3),4)
   ENDIF
   ,
   reply_obj->cv_list[nlistcnt].rpl_mth_yr = c.action_dt_tm, reply_obj->cv_list[nlistcnt].rpl_day =
   css.arrive_dt_tm, audit_record->qual[nlistcnt].order_id = c.order_id
  FOOT REPORT
