CREATE PROGRAM apscassformat1:dba
  ...
  DETAIL
   ...
   FOR (r_index = 1 TO max_r)
    ...
    FOR (l_index = 1 TO max_l)
      accession = data->resrc[r_index].label[l_index].fmt_accession_nbr, patient_name = data->resrc[
      r_index].label[l_index].name_full_formatted, spec_display = data->resrc[r_index].label[l_index]
      .specimen_disp,
      spec_blk_tag_disp = data->resrc[r_index].label[l_index].spec_blk_tag_disp, row + 1, line1 =
      formatline(trim(accession),font_medium),
      line2 = formatline(trim(patient_name),font_small), line3 = formatline(concat(trim(
         spec_blk_tag_disp),space,trim(spec_display)),font_small), output = concat(header,command,
       tower_1,command,font_medium,
       line1,command,new_line,command,font_small,
       line2,command,new_line,line3,footer),
      row + 1, output
    ENDFOR
   ENDFOR
