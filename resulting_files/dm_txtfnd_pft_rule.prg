CREATE PROGRAM dm_txtfnd_pft_rule
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 DECLARE dtpr_cnt = i4 WITH protect, noconstant(0)
 DECLARE dtpr_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dtpr_emsg = vc WITH protect, noconstant(" ")
 DECLARE dtpr_idx = i4 WITH protect, noconstant(0)
 DECLARE dtpr_num = i4 WITH protect, noconstant(0)
 FREE RECORD dtpr_attrib
 RECORD dtpr_attrib(
   1 cnt = i4
   1 qual[*]
     2 column_name = vc
 ...
 IF (check_logfile("dm_tf_pft_rule",".log","DM_TXTFND_PFT_RULE LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_TXTFND_PFT_RULE..."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (dtla_detail_id=0.0)
  ...
  SELECT INTO "nl:"
   ...
   WHERE d.detail_type_flag=2
    AND d.active_ind=1
    AND d.detail_script_name="DM_TXTFND_PFT_RULE"
   DETAIL
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "No active data found in DM_TEXT_FIND_DETAIL for script DM_TXTFND_PFT_RULE."
   IF ((script_rec->dm_text_find_log_id > 0.0))
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
   ENDIF
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_TXTFND_ATTRIBUTE:PFT_RULE"
   AND  EXISTS (
  (SELECT
   "x"
   FROM user_tab_columns utc
   WHERE utc.table_name="PFT_RULE"
    AND utc.column_name=di.info_name))
  ORDER BY di.info_name
  DETAIL
   dtpr_attrib->cnt = (dtpr_attrib->cnt+ 1), stat = alterlist(dtpr_attrib->qual,dtpr_attrib->cnt),
   dtpr_attrib->qual[dtpr_attrib->cnt].column_name = di.info_name,
   dtpr_attrib->qual[dtpr_attrib->cnt].column_label = di.info_char, dtpr_attrib->qual[dtpr_attrib->
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No attribute data found for PFT_RULE."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for Attribute list:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ...
 SET dm_err->eproc = "Gathering Profit Rule FKs to LONG_TEXT_REFERENCE"
 SELECT DISTINCT INTO "nl:"
  pr.rule_long_text_id
  FROM pft_rule pr
  WHERE pr.rule_long_text_id > 0.0
   AND  EXISTS (
  (SELECT
   "x"
   FROM long_text_reference l
   WHERE l.long_text_id=pr.rule_long_text_id))
   AND  EXISTS (
  (SELECT
   "x"
   FROM code_value c
   WHERE c.code_value=pr.rule_type_cd))
  HEAD REPORT
   script_rec->dm_text_find_detail_id = dtla_detail_id, dtpr_cnt = 0
  DETAIL
   dtpr_cnt = (dtpr_cnt+ 1)
   IF (mod(dtpr_cnt,10)=1)
    stat = alterlist(dtg->qual,(dtpr_cnt+ 9))
   ENDIF
   dtg->qual[dtpr_cnt].pe_name = "LONG_TEXT_REFERENCE", dtg->qual[dtpr_cnt].pe_id = pr
   .rule_long_text_id, dtg->qual[dtpr_cnt].pe_col = "LONG_TEXT_ID",
   dtg->qual[dtpr_cnt].srch_col = "LONG_TEXT"
  FOOT REPORT
   dtg->cnt = dtpr_cnt, stat = alterlist(dtg->qual,dtg->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for PFT_RULE LONG_TEXT_REFERENCE FKs:",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 IF ((dtg->cnt > 0))
  SET dm_err->eproc = "Gathering attribute information from PFT_RULE"
  CALL parser("select into 'nl:'",0)
  CALL parser("  from pft_rule pr, code_value c",0)
  CALL parser(" where expand(dtpr_num, 1, dtg->cnt,pr.rule_long_text_id,dtg->qual[dtpr_num].pe_id)",0
   )
  CALL parser(" and c.code_value = pr.rule_type_cd",0)
  CALL parser(" order by pr.rule_long_text_id",0)
  CALL parser("head pr.rule_long_text_id",0)
  CALL parser(
   "   dtpr_pos = locateval(dtpr_num, 1, dtg->cnt,pr.rule_long_text_id,dtg->qual[dtpr_num].pe_id) ",0
   )
  CALL parser("detail",0)
  CALL parser("   dtg->qual[dtpr_pos].data_source = pr.rule_name",0)
  CALL parser("   dtg->qual[dtpr_pos].att_cnt = dtg->qual[dtpr_pos].att_cnt + 1",0)
  CALL parser("   stat = alterlist(dtg->qual[dtpr_pos].att_qual, dtg->qual[dtpr_pos].att_cnt)",0)
  CALL parser("   dtg->qual[dtpr_pos].att_qual[dtg->qual[dtpr_pos].att_cnt].tab_name = 'CODE_VALUE'",
   0)
  CALL parser("   dtg->qual[dtpr_pos].att_qual[dtg->qual[dtpr_pos].att_cnt].col_name = 'DESCRIPTION'",
   0)
  CALL parser(
   "   dtg->qual[dtpr_pos].att_qual[dtg->qual[dtpr_pos].att_cnt].col_txt = 'Rule Folder: '",0)
  CALL parser(
   "   dtg->qual[dtpr_pos].att_qual[dtg->qual[dtpr_pos].att_cnt].col_val = replace(c.description,'.','/')",
   0)
  FOR (dtpr_idx = 1 TO dtpr_attrib->cnt)
    CALL parser("   dtg->qual[dtpr_pos].att_cnt = dtg->qual[dtpr_pos].att_cnt + 1",0)
    CALL parser("   stat = alterlist(dtg->qual[dtpr_pos].att_qual, dtg->qual[dtpr_pos].att_cnt)",0)
    CALL parser(concat(
      "      dtg->qual[dtpr_pos].att_qual[dtg->qual[dtpr_pos].att_cnt].tab_name = 'PFT_RULE'"),0)
    CALL parser(concat(
      "   dtg->qual[dtpr_pos].att_qual[dtg->qual[dtpr_pos].att_cnt].col_name = dtpr_attrib->qual[",
      trim(cnvtstring(dtpr_idx)),"].column_name"),0)
    CALL parser(concat(
      "   dtg->qual[dtpr_pos].att_qual[dtg->qual[dtpr_pos].att_cnt].col_txt = dtpr_attrib->qual[",
      trim(cnvtstring(dtpr_idx)),"].column_label"),0)
    IF ((dtpr_attrib->qual[dtpr_idx].column_flg=3))
     CALL parser(concat(
       "   dtg->qual[dtpr_pos].att_qual[dtg->qual[dtpr_pos].att_cnt].col_val = format(pr.",
       dtpr_attrib->qual[dtpr_idx].column_name,",';;q')"),0)
    ELSEIF ((dtpr_attrib->qual[dtpr_idx].column_flg=2))
     CALL parser(concat(
       "   dtg->qual[dtpr_pos].att_qual[dtg->qual[dtpr_pos].att_cnt].col_val = cnvtstring(pr.",
       dtpr_attrib->qual[dtpr_idx].column_name,",20)"),0)
    ELSE
     CALL parser(concat("   dtg->qual[dtpr_pos].att_qual[dtg->qual[dtpr_pos].att_cnt].col_val = pr.",
       dtpr_attrib->qual[dtpr_idx].column_name),0)
    ENDIF
  ENDFOR
  ...
  IF (check_error(dm_err->eproc) != 0)
   ...
   CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Gathering object attributes:",
     dm_err->emsg),0.0,0.0)
   SET dtpr_err_ind = dm_err->err_ind
   SET dtpr_emsg = dm_err->emsg
   SET dm_err->err_ind = 0
  ENDIF
 ...
 ENDIF
#exit_script
 SET dm_err->eproc = "...Ending DM_TXTFND_PFT_RULE"
 CALL final_disp_msg("dm_tf_pft_rule")
 IF (dtpr_err_ind=1
  AND (dm_err->err_ind=0))
  SET dm_err->err_ind = dtpr_err_ind
  SET dm_err->emsg = dtpr_emsg
 ENDIF
END GO
