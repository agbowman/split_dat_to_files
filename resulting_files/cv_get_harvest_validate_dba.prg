CREATE PROGRAM cv_get_harvest_validate:dba
 ...
 IF (validate(request->dataset_id) != 1)
  CALL echo("No dataset_id in request")
  GO TO exit_script
 ELSEIF ((request->dataset_id=0.0))
  CALL echo("Dataset_id set to 0.0")
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM cv_dataset t
  WHERE (t.dataset_id=request->dataset_id)
   AND t.active_ind=1
  DETAIL
   dataset_internal_name = t.dataset_internal_name
  WITH nocounter
 ;end select
 IF (dataset_internal_name="ACC")
  EXECUTE cv_get_harvest_validate_acc
 ENDIF
#exit_script
 ...
 SUBROUTINE cv_log_destroyhandle(dummy)
   ...
   ENDIF
 END ;Subroutine
 DECLARE cv_get_harvest_validate_vrsn = vc WITH private, constant("MOD 002 BM9013 06/02/06")
END GO
