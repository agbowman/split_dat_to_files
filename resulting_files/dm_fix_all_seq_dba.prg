CREATE PROGRAM dm_fix_all_seq:dba
 PAINT
 ...
 SUBROUTINE dctx_restore_prev_contexts(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE incr_amount = f8 WITH protect, noconstant(0.0)
 DECLARE dfas_seq_name = vc WITH protect, noconstant(" ")
#draw_box
 CALL box(2,2,20,78)
 ...
 IF (check_logfile("dm_fix_all_sequences",".log","dm_fix_all_seq LOGFILE")=0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Beginning dm_fix_all_seq"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "NL:"
  ...
  FROM dm_columns_doc dcd,
   dm_tables_doc dtc
  PLAN (dtc
   WHERE dtc.drop_ind=0)
   JOIN (dcd
   WHERE dtc.table_name=dcd.table_name
 ...
 FOR (x = 1 TO row_count)
   ...
   ENDIF
 ENDFOR
 IF (one_seq_missing="T")
  SELECT
   table_name = requestin->list[d.seq].table_name, column_name = requestin->list[d.seq].column_name,
   sequence_name = requestin->list[d.seq].sequence_name,
   max_table_value = requestin->list[d.seq].max_table_value, next_sequence_value = requestin->list[d
   .seq].next_sequence_value
   FROM (dummyt d  WITH seq = value(row_count))
   WHERE (requestin->list[d.seq].sequence_missing="T")
   HEAD REPORT
    col 0, "Sequence Check Report", row + 2,
    col 0, "Ran on: ", curdate"mm/dd/yyyy;;d",
    col + 2, curtime, row + 1,
    row + 2, "Sequences that are missing", row + 1,
    "This program can not fix this issue! It can just find them.", row + 1
   HEAD PAGE
    col 0, "Table name", col 35,
    "Column name", col 70, "Sequence Name",
    row + 1
   DETAIL
    col 0, table_name, col 35,
    column_name, col 70, sequence_name,
    row + 1
   WITH nocounter, nullreport, maxcol = 400
  ;end select
 ENDIF
 IF (one_seq_out_of_seq="T")
  SELECT
   table_name = requestin->list[d.seq].table_name, column_name = requestin->list[d.seq].column_name,
   sequence_name = requestin->list[d.seq].sequence_name,
   max_table_value = requestin->list[d.seq].max_table_value, next_sequence_value = requestin->list[d
   .seq].next_sequence_value
   FROM (dummyt d  WITH seq = value(row_count))
   WHERE (requestin->list[d.seq].max_table_value > requestin->list[d.seq].next_sequence_value)
   HEAD REPORT
    col 0, "Sequence Check Report", row + 2,
    col 0, "Ran on: ", curdate"mm/dd/yyyy;;d",
    col + 2, curtime, row + 1,
    row + 2, "Sequences with problems", row + 2
   HEAD PAGE
    col 0, "Table name", col 35,
    "Column name", col 70, "Sequence Name",
    col 100, "Max table value", col 120,
    "Next seq value", row + 1
   DETAIL
    col 0, table_name, col 35,
    column_name, col 70, sequence_name,
    col 100, max_table_value, col 120,
    next_sequence_value, row + 1
   WITH nocounter, nullreport, maxcol = 400
  ;end select
  CALL box(2,2,20,78)
  CALL line(6,2,77,xhor)
  CALL text(4,10,"Sequence Check program")
  CALL text(8,10,"Correct sequences that the next value is too low")
  CALL text(24,1,"Correct sequences that are too low? (Y/N)")
  CALL accept(24,60,"P;CU","Y"
   WHERE curaccept IN ("Y", "N"))
  IF (curaccept="N")
   GO TO end_program
  ...
  IF (dctx_set_context("DM2_DDLPRIVS","YES")=0)
   GO TO end_program
  ENDIF
  FOR (z = 1 TO row_count)
    IF ((requestin->list[z].max_table_value > requestin->list[z].next_sequence_value))
     SET st_sname = fillstring(30," ")
     SET st_sname = cnvtupper(trim(requestin->list[z].sequence_name))
     SET st_min_val = 0.0
     SET st_min_val = requestin->list[z].max_table_value
     SET st_max_val = 0.0
     SET st_cycle_flg = 0
     SET st_incr_by = 0
     SET incr_amount = 0.0
     ...
     SELECT
      ...
      ENDIF
      INTO "nl:"
      u.sequence_name, u.increment_by
      WHERE u.sequence_name=st_sname
      DETAIL
       st_incr_by = u.increment_by
      WITH nocounter
     ;end select
     IF (curqual > 0)
      SET dcs_next_v = 0.0
      ...
      WHILE (dcs_next_v < st_min_val
       AND st_abort_cnt < 3)
        ...
        IF (mod(incr_amount,2) != 0)
         SET incr_amount = (incr_amount+ 1)
        ENDIF
        SET call_parser_string = concat("rdb alter sequence ",trim(st_sname)," increment by ",
         cnvtstring(incr_amount,20)," go")
 ...
 ELSE
  SELECT
   ...
   FROM dummyt
   DETAIL
    col 0, "all sequences are correct"
   WITH nocounter
  ;end select
 ENDIF
#end_program
