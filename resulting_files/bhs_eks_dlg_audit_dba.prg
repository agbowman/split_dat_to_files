CREATE PROGRAM bhs_eks_dlg_audit:dba
 PROMPT
  "Output to File/Printer/MINE (MINE):" = "MINE",
  "Begin Date, mmddyy (today):" = "CURDATE",
  "BeginTime, hhmm (0000):" = "0000",
  "End Date, mmddyy (today):" = "CURDATE",
  "End Time, hhmm (2359):" = "2359",
  "Module Name, pattern match OK (*):" = "*",
  "Output Type - (B)ackend CSV, (F)rontend CSV, or (R)eport (R):" = "F",
  "Show (S)ummary or (D)etails (D):" = "D",
  "Sort by (A)lert Recipient or (M)odule (M):" = "M"
 ...
 IF (eksdlgevent->qual_cnt)
  ...
  IF (( $OUTTYPE IN ("B", "F")))
   ...
   SELECT
    ...
    order_id = cnvtstring(eksdlgevent->qual[d1.seq].trigger_order_id), dlgdttm = format(cnvtdatetime(
      eksdlgevent->qual[d1.seq].updt_dt_tm),"yyyy/mm/dd hh:mm:ss;;d"), trigger = trim(
     uar_get_code_display(eksdlgevent->qual[d1.seq].trigger_entity_id)),
    recipient = trim(p.name_full_formatted), dlgname = substring(1,255,eksdlgevent->qual[d1.seq].
     dlg_name), reason = trim(uar_get_code_display(eksdlgevent->qual[d1.seq].override_reason_cd)),
    facility = trim(uar_get_code_display(eh.loc_facility_cd)), building = trim(uar_get_code_display(
      eh.loc_building_cd)), nurseunit = trim(uar_get_code_display(eh.loc_nurse_unit_cd)),
    room = trim(uar_get_code_display(eh.loc_room_cd)), bed = trim(uar_get_code_display(eh.loc_bed_cd)
     ), ft_reason =
    IF (lt.long_text_id > 0) substring(1,75,lt.long_text)
    ELSE " "
    ENDIF
    ,
    allergy = substring(1,50,eksdlgevent->qual[d1.seq].srcstring), interaction = trim(eksdlgevent->
     qual[d1.seq].catdisp), severity = eksdlgevent->qual[d1.seq].severity
    FROM (dummyt d1  WITH seq = eksdlgevent->qual_cnt),
     person p,
     encounter e,
     encntr_loc_hist eh,
