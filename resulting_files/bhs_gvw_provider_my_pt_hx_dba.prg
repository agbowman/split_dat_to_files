CREATE PROGRAM bhs_gvw_provider_my_pt_hx:dba
 RECORD m_request(
 ...
 DECLARE mf_providersmypatienthistory_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "PROVIDERSMYPATIENTHISTORY"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ce
   WHERE ce.person_id=mf_per_id
    AND ce.event_cd=mf_providersmypatienthistory_cd
    AND (ce.performed_prsnl_id=reqinfo->updt_id)
