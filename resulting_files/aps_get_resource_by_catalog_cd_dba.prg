CREATE PROGRAM aps_get_resource_by_catalog_cd:dba
...
#script
 ...
 SET stat = alterlist(reply->catalog_qual,10)
 SET catalog_cnt = 0
 ...
 ENDIF
 SELECT INTO "nl:"
  d.seq, orl.catalog_cd
  FROM (dummyt d  WITH seq = value(size(request->qual,5))),
   orc_resource_list orl
  PLAN (d)
   JOIN (orl
   WHERE (request->qual[d.seq].catalog_cd=orl.catalog_cd))
  HEAD REPORT
  ...
  DETAIL
   ...
   IF (isresourceviewable(service_resource_cd)=true)
    IF (resource_cnt=0)
     catalog_cnt += 1
     IF (mod(catalog_cnt,10)=1
      AND catalog_cnt != 1)
      stat = alterlist(reply->catalog_qual,(catalog_cnt+ 9))
     ENDIF
     stat = alterlist(reply->catalog_qual[catalog_cnt].service_resource_qual,10), reply->
     catalog_qual[catalog_cnt].catalog_cd = orl.catalog_cd
    ENDIF
    resource_cnt += 1
    IF (mod(resource_cnt,10)=1
     AND resource_cnt != 1)
     stat = alterlist(reply->catalog_qual[catalog_cnt].service_resource_qual,(resource_cnt+ 9))
    ENDIF
    reply->catalog_qual[catalog_cnt].service_resource_qual[resource_cnt].service_resource_cd = orl
    .service_resource_cd, reply->catalog_qual[catalog_cnt].service_resource_qual[resource_cnt].
    primary_ind = orl.primary_ind
   ENDIF
  FOOT  orl.catalog_cd
   IF (resource_cnt > 0)
    stat = alterlist(reply->catalog_qual[catalog_cnt].service_resource_qual,resource_cnt)
   ENDIF
  FOOT REPORT
   stat = alterlist(reply->catalog_qual,catalog_cnt)
  WITH nocounter
 ;end select
 IF (getresourcesecuritystatus(0)="F")
  SET reply->status_data.status = getresourcesecuritystatus(0)
  CALL populateressecstatusblock(0)
 ELSEIF (catalog_cnt=0)
  SET reply->status_data.status = "Z"
  SET reply->status_data.subeventstatus[1].operationname = "SELECT"
  SET reply->status_data.subeventstatus[1].operationstatus = "Z"
  SET reply->status_data.subeventstatus[1].targetobjectname = "TABLE"
  SET reply->status_data.subeventstatus[1].targetobjectvalue = "ORC_RESOURCE_LIST"
 ELSE
