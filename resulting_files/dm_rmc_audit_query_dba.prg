CREATE PROGRAM dm_rmc_audit_query:dba
...
","    char_0_ind = 1 "),0)
 ...
 SUBROUTINE dra_get_dcl_data(dgdd_tab_name,dgdd_global,dgdd_info,dgdd_info_loc)
   ...
   ENDIF
 END ;Subroutine
 DECLARE draq_direct_join(ds_md_loc=i4,ds_info_loc=i4,ds_info=vc(ref),ds_parse1=vc(ref),ds_parse2=vc(
   ref),
  ds_parse3=vc(ref),ds_parse4=vc(ref),ds_parse5=vc(ref)) = null
 DECLARE draq_outer_join(dl_md_loc=i4,dl_info_loc=i4,dl_info=vc(ref),dl_parse1=vc(ref),dl_parse2=vc(
   ref)) = null
 DECLARE draq_fcn_strs(dfs_info_loc=i4,dfs_info=vc(ref)) = null
 SUBROUTINE draq_fcn_strs(dfs_info_loc,dfs_info)
   DECLARE dfs_loop = i4 WITH protect, noconstant(0)
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("INFO_LOC:",trim(cnvtstring(dfs_info_loc))))
   ENDIF
   SET dfs_info->tables[dfs_info_loc].tgt_pkw_str = concat(dfs_info->tables[dfs_info_loc].
    tgt_pkwhere_function,"(")
   FOR (dfs_loop = 1 TO dfs_info->tables[dfs_info_loc].tgt_pkw_parm_cnt)
    IF (dfs_loop > 1)
     SET dfs_info->tables[dfs_info_loc].tgt_pkw_str = concat(dfs_info->tables[dfs_info_loc].
      tgt_pkw_str,",")
    ENDIF
    SET dfs_info->tables[dfs_info_loc].tgt_pkw_str = concat(dfs_info->tables[dfs_info_loc].
     tgt_pkw_str,"d.",dfs_info->tables[dfs_info_loc].tgt_pkw_parms[dfs_loop].parm_name)
   ENDFOR
   SET dfs_info->tables[dfs_info_loc].src_proc_pkw_str = concat(dfs_info->tables[dfs_info_loc].
    tgt_pkw_str,",'",drgi_global->db_link,"',0)")
   SET dfs_info->tables[dfs_info_loc].tgt_pkw_str = concat(dfs_info->tables[dfs_info_loc].tgt_pkw_str,
    ",' ',0)")
   SET dfs_info->tables[dfs_info_loc].src_pkw_str = concat(dfs_info->tables[dfs_info_loc].
    src_pkwhere_function,drgi_global->db_link,"(")
   FOR (dfs_loop = 1 TO dfs_info->tables[dfs_info_loc].src_pkw_parm_cnt)
    IF (dfs_loop > 1)
     SET dfs_info->tables[dfs_info_loc].src_pkw_str = concat(dfs_info->tables[dfs_info_loc].
      src_pkw_str,",")
    ENDIF
    SET dfs_info->tables[dfs_info_loc].src_pkw_str = concat(dfs_info->tables[dfs_info_loc].
     src_pkw_str,"d.",dfs_info->tables[dfs_info_loc].src_pkw_parms[dfs_loop].parm_name)
   ENDFOR
   SET dfs_info->tables[dfs_info_loc].src_pkw_str = concat(dfs_info->tables[dfs_info_loc].src_pkw_str,
    ")")
   IF ((dfs_info->tables[dfs_info_loc].tgt_filter_function > " "))
    SET dfs_info->tables[dfs_info_loc].tgt_filter_str = concat(dfs_info->tables[dfs_info_loc].
     tgt_filter_function,"('ADD',")
    FOR (dfs_loop = 1 TO dfs_info->tables[dfs_info_loc].tgt_filter_parm_cnt)
     IF (dfs_loop > 1)
      SET dfs_info->tables[dfs_info_loc].tgt_filter_str = concat(dfs_info->tables[dfs_info_loc].
       tgt_filter_str,",")
     ENDIF
     SET dfs_info->tables[dfs_info_loc].tgt_filter_str = concat(dfs_info->tables[dfs_info_loc].
      tgt_filter_str,"d.",dfs_info->tables[dfs_info_loc].tgt_f_parms[dfs_loop].parm_name,", d.",
      dfs_info->tables[dfs_info_loc].tgt_f_parms[dfs_loop].parm_name)
    ENDFOR
    SET dfs_info->tables[dfs_info_loc].tgt_filter_str = concat(dfs_info->tables[dfs_info_loc].
     tgt_filter_str,")")
   ELSE
    SET dfs_info->tables[dfs_info_loc].tgt_filter_str = "1"
   ENDIF
   IF ((dfs_info->tables[dfs_info_loc].src_filter_function > " "))
    SET dfs_info->tables[dfs_info_loc].src_filter_str = concat(dfs_info->tables[dfs_info_loc].
     src_filter_function,drgi_global->db_link,"('ADD',")
    FOR (dfs_loop = 1 TO dfs_info->tables[dfs_info_loc].src_filter_parm_cnt)
     IF (dfs_loop > 1)
      SET dfs_info->tables[dfs_info_loc].src_filter_str = concat(dfs_info->tables[dfs_info_loc].
       src_filter_str,",")
     ENDIF
     SET dfs_info->tables[dfs_info_loc].src_filter_str = concat(dfs_info->tables[dfs_info_loc].
      src_filter_str,"d.",dfs_info->tables[dfs_info_loc].src_f_parms[dfs_loop].parm_name,", d.",
      dfs_info->tables[dfs_info_loc].src_f_parms[dfs_loop].parm_name)
    ENDFOR
    SET dfs_info->tables[dfs_info_loc].src_filter_str = concat(dfs_info->tables[dfs_info_loc].
     src_filter_str,")")
   ELSE
    SET dfs_info->tables[dfs_info_loc].src_filter_str = "1"
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dfs_info)
   ENDIF
   RETURN(null)
 END ;Subroutine
 SUBROUTINE draq_direct_join(ds_md_loc,ds_info_loc,ds_info,ds_parse1,ds_parse2,ds_parse3,ds_parse4,
  ds_parse5)
   DECLARE ds_cnt = i4 WITH protect, noconstant(0)
   DECLARE ds_loop = i4 WITH protect, noconstant(0)
   DECLARE ds_tab_list = vc WITH protect, noconstant(" ")
   DECLARE ds_loop2 = i4 WITH protect, noconstant(0)
   DECLARE ds_info_col_loc = i4 WITH protect, noconstant(0)
   SET ds_cnt = 1
   SET stat = alterlist(ds_parse1->stmt,500)
   SET ds_parse1->stmt[ds_cnt].str = concat("insert into dm_rdds_",trim(cnvtstring(currdbhandle)),
    "_aud_gttd d (d.status")
   SET ds_cnt = (ds_cnt+ 1)
   FOR (ds_loop = 1 TO tmc_md_info->tbl_qual[ds_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].pk_ind=1))
      SET ds_parse1->stmt[ds_cnt].str = concat(ds_parse1->stmt[ds_cnt].str,", d.",tmc_md_info->
       tbl_qual[ds_md_loc].col_qual[ds_loop].column_name)
     ENDIF
   ENDFOR
   SET ds_parse1->stmt[ds_cnt].str = concat(ds_parse1->stmt[ds_cnt].str,")")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse1->stmt[ds_cnt].str = "(select 'INTGT'"
   FOR (ds_loop = 1 TO tmc_md_info->tbl_qual[ds_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].pk_ind=1))
      SET ds_parse1->stmt[ds_cnt].str = concat(ds_parse1->stmt[ds_cnt].str,", d1.",tmc_md_info->
       tbl_qual[ds_md_loc].col_qual[ds_loop].column_name)
     ENDIF
   ENDFOR
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse1->stmt[ds_cnt].str = concat(" from dm_rdds_",trim(cnvtstring(currdbhandle)),
    "_aud_gttd d1")
   FOR (ds_loop = 1 TO tmc_md_info->tbl_qual[ds_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].pk_ind=1)
      AND (tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].idcd_ind=1))
      SET ds_parse1->stmt[ds_cnt].str = concat(ds_parse1->stmt[ds_cnt].str,", v_rdds_audit_xlat v",
       trim(cnvtstring(ds_loop)))
     ENDIF
   ENDFOR
   SET ds_parse1->stmt[ds_cnt].str = concat(ds_parse1->stmt[ds_cnt].str,", ",tmc_md_info->tbl_qual[
    ds_md_loc].table_name," t")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse1->stmt[ds_cnt].str = " where d1.status = 'S_INTLSRCH'"
   FOR (ds_loop = 1 TO tmc_md_info->tbl_qual[ds_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].pk_ind=1))
      SET ds_cnt = (ds_cnt+ 1)
      IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].idcd_ind=1))
       IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].parent_entity_col > " "))
        SET ds_info_col_loc = locateval(ds_info_col_loc,1,ds_info->tables[ds_info_loc].column_cnt,
         tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].column_name,ds_info->tables[ds_info_loc].
         columns[ds_info_col_loc].column_name)
        SET ds_tab_list = concat("decode(nullval(d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[
         ds_loop].parent_entity_col,
         ",'RDDS_NONAME'),'RDDS_NONAME','INVALIDTABLE',' ','INVALIDTABLE','")
        FOR (ds_loop2 = 1 TO ds_info->tables[ds_info_loc].columns[ds_info_col_loc].pe_val_cnt)
          SET ds_tab_list = concat(ds_tab_list,ds_info->tables[ds_info_loc].columns[ds_info_col_loc].
           parent_entity_vals[ds_loop2].pe_name,"','",ds_info->tables[ds_info_loc].columns[
           ds_info_col_loc].parent_entity_vals[ds_loop2].pe_name,"','")
        ENDFOR
        SET ds_tab_list = concat(substring(1,(size(ds_tab_list) - 2),ds_tab_list),")")
       ELSE
        SET ds_tab_list = concat("'",tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].
         root_entity_name,"'")
       ENDIF
       SET ds_parse1->stmt[ds_cnt].str = concat(" and ((v",trim(cnvtstring(ds_loop)),
        ".from_value = d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].column_name," and v",
        trim(cnvtstring(ds_loop)),".table_name = ",ds_tab_list)
       SET ds_cnt = (ds_cnt+ 1)
       SET ds_parse1->stmt[ds_cnt].str = concat(" and t.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[
        ds_loop].column_name," = v",trim(cnvtstring(ds_loop)),".to_value)")
       IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].nullable != "N"))
        SET ds_cnt = (ds_cnt+ 1)
        SET ds_parse1->stmt[ds_cnt].str = concat(" or (t.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[
         ds_loop].column_name," is null and d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].
         column_name," is null)")
       ENDIF
       SET ds_parse1->stmt[ds_cnt].str = concat(ds_parse1->stmt[ds_cnt].str,")")
      ELSE
       SET ds_parse1->stmt[ds_cnt].str = concat(" and (t.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[
        ds_loop].column_name," = d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].column_name)
       IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].nullable != "N"))
        SET ds_parse1->stmt[ds_cnt].str = concat(ds_parse1->stmt[ds_cnt].str," or (t.",tmc_md_info->
         tbl_qual[ds_md_loc].col_qual[ds_loop].column_name," is null and d1.",tmc_md_info->tbl_qual[
         ds_md_loc].col_qual[ds_loop].column_name,
         " is null)")
       ENDIF
       SET ds_parse1->stmt[ds_cnt].str = concat(ds_parse1->stmt[ds_cnt].str,")")
      ENDIF
   ...
   ENDFOR
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse1->stmt[ds_cnt].str = ") with nocounter go"
   SET stat = alterlist(ds_parse1->stmt,ds_cnt)
   SET ds_cnt = 1
   SET stat = alterlist(ds_parse2->stmt,500)
   SET ds_parse2->stmt[ds_cnt].str = concat("rdb ASIS(^update DM_RDDS_",trim(currdbhandle),
    "_AUD_GTTD d^)")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse2->stmt[ds_cnt].str = concat("ASIS(^ set d.pk_where = ",ds_info->tables[ds_info_loc].
    src_proc_pkw_str,",^)")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse2->stmt[ds_cnt].str = concat("ASIS(^ d.filtered_ind = ",ds_info->tables[ds_info_loc].
    src_filter_str,",^)")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse2->stmt[ds_cnt].str = "ASIS(^ d.status = 'SOURCE'^)"
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse2->stmt[ds_cnt].str = concat(
    "ASIS(^where d.status = 'S_INTLSRCH' and not exists (select 'x' from DM_RDDS_",trim(currdbhandle),
    "_AUD_GTTD d1 where d1.status = 'INTGT' ^)")
   FOR (ds_loop = 1 TO tmc_md_info->tbl_qual[ds_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].pk_ind=1))
      SET ds_cnt = (ds_cnt+ 1)
      SET ds_parse2->stmt[ds_cnt].str = concat("ASIS(^ and (d.",tmc_md_info->tbl_qual[ds_md_loc].
       col_qual[ds_loop].column_name," = d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].
       column_name)
      IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].nullable != "N"))
       SET ds_parse2->stmt[ds_cnt].str = concat(ds_parse2->stmt[ds_cnt].str," ^)")
       SET ds_cnt = (ds_cnt+ 1)
       SET ds_parse2->stmt[ds_cnt].str = concat("ASIS(^ or d.",tmc_md_info->tbl_qual[ds_md_loc].
        col_qual[ds_loop].column_name," is null and d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[
        ds_loop].column_name," is null")
      ENDIF
      SET ds_parse2->stmt[ds_cnt].str = concat(ds_parse2->stmt[ds_cnt].str,")^)")
     ENDIF
   ENDFOR
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse2->stmt[ds_cnt].str = "ASIS(^ ) ^) go"
   SET stat = alterlist(ds_parse2->stmt,ds_cnt)
   SET ds_cnt = 1
   SET stat = alterlist(ds_parse3->stmt,500)
   SET ds_parse3->stmt[ds_cnt].str = concat("insert into dm_rdds_",trim(cnvtstring(currdbhandle)),
    "_aud_gttd d (d.status")
   SET ds_cnt = (ds_cnt+ 1)
   FOR (ds_loop = 1 TO tmc_md_info->tbl_qual[ds_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].pk_ind=1))
      SET ds_parse3->stmt[ds_cnt].str = concat(ds_parse3->stmt[ds_cnt].str,", d.",tmc_md_info->
       tbl_qual[ds_md_loc].col_qual[ds_loop].column_name)
     ENDIF
   ENDFOR
   SET ds_parse3->stmt[ds_cnt].str = concat(ds_parse3->stmt[ds_cnt].str,")")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse3->stmt[ds_cnt].str = "(select 'INSRC'"
   FOR (ds_loop = 1 TO tmc_md_info->tbl_qual[ds_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].pk_ind=1))
      SET ds_parse3->stmt[ds_cnt].str = concat(ds_parse3->stmt[ds_cnt].str,", d1.",tmc_md_info->
       tbl_qual[ds_md_loc].col_qual[ds_loop].column_name)
     ENDIF
   ENDFOR
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse3->stmt[ds_cnt].str = concat(" from dm_rdds_",trim(cnvtstring(currdbhandle)),
    "_aud_gttd d1")
   FOR (ds_loop = 1 TO tmc_md_info->tbl_qual[ds_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].pk_ind=1)
      AND (tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].idcd_ind=1))
      SET ds_parse3->stmt[ds_cnt].str = concat(ds_parse3->stmt[ds_cnt].str,", v_rdds_audit_xlat v",
       trim(cnvtstring(ds_loop)))
     ENDIF
   ENDFOR
   SET ds_parse3->stmt[ds_cnt].str = concat(ds_parse3->stmt[ds_cnt].str,", ",tmc_md_info->tbl_qual[
    ds_md_loc].table_name,drgi_global->db_link," t")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse3->stmt[ds_cnt].str = " where d1.status = 'T_INTLSRCH'"
   FOR (ds_loop = 1 TO tmc_md_info->tbl_qual[ds_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].pk_ind=1))
      SET ds_cnt = (ds_cnt+ 1)
      IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].idcd_ind=1))
       ...
       ELSE
        SET ds_tab_list = concat("'",tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].
         root_entity_name,"'")
       ENDIF
       SET ds_parse3->stmt[ds_cnt].str = concat(" and ((v",trim(cnvtstring(ds_loop)),
        ".to_value = d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].column_name," and v",
        trim(cnvtstring(ds_loop)),".table_name = ",ds_tab_list)
       SET ds_cnt = (ds_cnt+ 1)
       SET ds_parse3->stmt[ds_cnt].str = concat(" and t.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[
        ds_loop].column_name," = v",trim(cnvtstring(ds_loop)),".from_value)")
       IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].nullable != "N"))
        SET ds_cnt = (ds_cnt+ 1)
        SET ds_parse3->stmt[ds_cnt].str = concat(" or (t.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[
         ds_loop].column_name," is null and d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].
         column_name," is null)")
       ENDIF
       SET ds_parse3->stmt[ds_cnt].str = concat(ds_parse3->stmt[ds_cnt].str,")")
      ELSE
       SET ds_parse3->stmt[ds_cnt].str = concat(" and (t.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[
        ds_loop].column_name," = d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].column_name)
       IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].nullable != "N"))
        SET ds_parse3->stmt[ds_cnt].str = concat(ds_parse3->stmt[ds_cnt].str," or (t.",tmc_md_info->
         tbl_qual[ds_md_loc].col_qual[ds_loop].column_name," is null and d1.",tmc_md_info->tbl_qual[
         ds_md_loc].col_qual[ds_loop].column_name,
         " is null)")
       ENDIF
   ...
   ENDFOR
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse3->stmt[ds_cnt].str = ") with nocounter go"
   SET stat = alterlist(ds_parse3->stmt,ds_cnt)
   SET ds_cnt = 1
   SET stat = alterlist(ds_parse4->stmt,500)
   SET ds_parse4->stmt[ds_cnt].str = concat("rdb ASIS(^update DM_RDDS_",trim(currdbhandle),
    "_AUD_GTTD d^)")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse4->stmt[ds_cnt].str = concat("ASIS(^ set d.pk_where = ",ds_info->tables[ds_info_loc].
    tgt_pkw_str,",^)")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse4->stmt[ds_cnt].str = concat("ASIS(^ d.filtered_ind = ",ds_info->tables[ds_info_loc].
    tgt_filter_str,",^)")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse4->stmt[ds_cnt].str = "ASIS(^ d.status = 'TARGET'^)"
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse4->stmt[ds_cnt].str = concat(
    "ASIS(^where d.status = 'T_INTLSRCH' and not exists (select 'x' from DM_RDDS_",trim(currdbhandle),
    "_AUD_GTTD d1 where d1.status = 'INSRC' ^)")
   FOR (ds_loop = 1 TO tmc_md_info->tbl_qual[ds_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].pk_ind=1))
      SET ds_cnt = (ds_cnt+ 1)
      SET ds_parse4->stmt[ds_cnt].str = concat("ASIS(^ and (d.",tmc_md_info->tbl_qual[ds_md_loc].
       col_qual[ds_loop].column_name," = d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].
       column_name)
      IF ((tmc_md_info->tbl_qual[ds_md_loc].col_qual[ds_loop].nullable != "N"))
       SET ds_parse4->stmt[ds_cnt].str = concat(ds_parse4->stmt[ds_cnt].str," ^)")
       SET ds_cnt = (ds_cnt+ 1)
       SET ds_parse4->stmt[ds_cnt].str = concat("ASIS(^ or d.",tmc_md_info->tbl_qual[ds_md_loc].
        col_qual[ds_loop].column_name," is null and d1.",tmc_md_info->tbl_qual[ds_md_loc].col_qual[
        ds_loop].column_name," is null")
      ENDIF
      SET ds_parse4->stmt[ds_cnt].str = concat(ds_parse4->stmt[ds_cnt].str,")^)")
   ...
   ENDFOR
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse4->stmt[ds_cnt].str = "ASIS(^ ) ^) go"
   SET stat = alterlist(ds_parse4->stmt,ds_cnt)
   SET ds_cnt = 1
   SET stat = alterlist(ds_parse5->stmt,500)
   SET ds_parse5->stmt[ds_cnt].str = concat("rdb ASIS(^update DM_RDDS_",trim(currdbhandle),
    "_AUD_GTTD d^)")
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse5->stmt[ds_cnt].str =
   "ASIS(^ set d.pk_where_value = dbms_utility.get_hash_value(d.pk_where,0,1073741824.0)^)"
   SET ds_cnt = (ds_cnt+ 1)
   SET ds_parse5->stmt[ds_cnt].str =
   "ASIS(^where d.status in ('TARGET','SOURCE') and d.pk_where > ' ' ^) go"
   SET stat = alterlist(ds_parse5->stmt,ds_cnt)
   RETURN(null)
 END ;Subroutine
 SUBROUTINE draq_outer_join(dl_md_loc,dl_info_loc,dl_info,dl_parse1,dl_parse2)
   DECLARE dl_cnt = i4 WITH protect, noconstant(0)
   DECLARE dl_loop = i4 WITH protect, noconstant(0)
   DECLARE dl_view_cnt = i4 WITH protect, noconstant(0)
   DECLARE dl_tab_list = vc WITH protect, noconstant(" ")
   DECLARE dl_ind = i2 WITH protect, noconstant(0)
   DECLARE dl_info_col_loc = i4 WITH protect, noconstant(0)
   SET stat = alterlist(dl_parse1->stmt,500)
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = "RDB ASIS(^INSERT ALL ^)"
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = "ASIS(^ WHEN (("
   SET dl_ind = 0
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1))
      IF (dl_ind=1)
       SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," or (")
      ENDIF
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," t_",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," is null and f_",tmc_md_info->tbl_qual[
       dl_md_loc].col_qual[dl_loop].column_name,
       " is not null) ")
      SET dl_ind = 1
     ENDIF
   ENDFOR
   SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str,")^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = concat("ASIS(^  then into dm_rdds_",trim(cnvtstring(currdbhandle
      )),"_aud_gttd^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = "ASIS(^("
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].audit_flag=1))
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str,tmc_md_info->tbl_qual[
       dl_md_loc].col_qual[dl_loop].column_name,", ")
     ENDIF
   ENDFOR
   SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str,"^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str,
    " ASIS(^status, pk_where, pk_where_value, filtered_ind) ^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = concat(" ASIS(^ VALUES(^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = " ASIS(^ "
   SET dl_ind = 0
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].audit_flag=1))
      IF (dl_ind=1)
       SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str,",")
      ENDIF
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," f_",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name)
      SET dl_ind = 1
     ENDIF
   ENDFOR
   SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str,",'SOURCE',^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = concat("ASIS(^ ",replace(dl_info->tables[dl_info_loc].
     src_proc_pkw_str,"d.","f_"),",^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = concat("ASIS(^ dbms_utility.get_hash_value(",replace(dl_info->
     tables[dl_info_loc].src_proc_pkw_str,"d.","f_"),",0,1073741824.0),^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = concat("ASIS(^ ",replace(dl_info->tables[dl_info_loc].
     src_filter_str,"d.","f_"),")^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = "ASIS(^ select "
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].audit_flag IN (1, 2)))
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," s.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," F_",tmc_md_info->tbl_qual[dl_md_loc].
       col_qual[dl_loop].column_name,
       ", ")
     ENDIF
   ENDFOR
   SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str,"^)")
   ...
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1)
      AND (tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].idcd_ind=1))
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," t.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," T_",tmc_md_info->tbl_qual[dl_md_loc].
       col_qual[dl_loop].column_name,
       ", ")
     ELSEIF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1))
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," s.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," T_",tmc_md_info->tbl_qual[dl_md_loc].
       col_qual[dl_loop].column_name,
       ", ")
     ENDIF
   ENDFOR
   SET dl_parse1->stmt[dl_cnt].str = substring(1,(size(dl_parse1->stmt[dl_cnt].str) - 1),dl_parse1->
    stmt[dl_cnt].str)
   SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str,"^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = concat("ASIS(^ from dm_rdds_",trim(cnvtstring(currdbhandle)),
    "_aud_gttd s ")
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1)
      AND (tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].idcd_ind=1))
      SET dl_view_cnt = (dl_view_cnt+ 1)
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," left outer join ^)")
      SET dl_cnt = (dl_cnt+ 1)
      SET dl_parse1->stmt[dl_cnt].str = concat(" ASIS(^ v_rdds_audit_xlat v",trim(cnvtstring(
         dl_view_cnt))," on ^)")
      SET dl_cnt = (dl_cnt+ 1)
      SET dl_parse1->stmt[dl_cnt].str = " ASIS(^ "
      IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].parent_entity_col > " "))
       SET dl_info_col_loc = locateval(dl_info_col_loc,1,dl_info->tables[dl_info_loc].column_cnt,
        tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].column_name,dl_info->tables[dl_info_loc].
        columns[dl_info_col_loc].column_name)
       SET dl_tab_list = concat("decode(nvl(s.",tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].
        parent_entity_col,",'RDDS_NONAME'),'RDDS_NONAME','INVALIDTABLE',' ','INVALIDTABLE','")
       FOR (dl_loop2 = 1 TO dl_info->tables[dl_info_loc].columns[dl_info_col_loc].pe_val_cnt)
         SET dl_tab_list = concat(dl_tab_list,dl_info->tables[dl_info_loc].columns[dl_info_col_loc].
          parent_entity_vals[dl_loop2].pe_name,"','",dl_info->tables[dl_info_loc].columns[
          dl_info_col_loc].parent_entity_vals[dl_loop2].pe_name,"','")
       ENDFOR
       SET dl_tab_list = concat(substring(1,(size(dl_tab_list) - 2),dl_tab_list),")")
      ELSE
       SET dl_tab_list = concat("'",tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].
        root_entity_name,"'")
      ENDIF
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," s.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," = v",trim(cnvtstring(dl_view_cnt)),
       ".from_value and v",trim(cnvtstring(dl_view_cnt)),".table_name = ",dl_tab_list," and s.",
       tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," > 0.0 ")
     ENDIF
   ENDFOR
   SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str,"^)")
   SET dl_view_cnt = 0
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = concat(" ASIS(^ left outer join ",tmc_md_info->tbl_qual[
    dl_md_loc].table_name," t on ^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = concat(" ASIS(^")
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1)
      AND (tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].idcd_ind=1))
      SET dl_view_cnt = (dl_view_cnt+ 1)
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," ((t.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," = v",trim(cnvtstring(dl_view_cnt)),
       ".to_value ")
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," or (s.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," <= 0.0 and s.",tmc_md_info->tbl_qual[
       dl_md_loc].col_qual[dl_loop].column_name,
       " = t.",tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].column_name,")) ")
      IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].nullable != "N"))
       SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," or (t.",tmc_md_info->
        tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," is null and s.",tmc_md_info->tbl_qual[
        dl_md_loc].col_qual[dl_loop].column_name,
        " is null)")
      ENDIF
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str,") and ")
     ELSEIF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1))
      SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," (s.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," = t.",tmc_md_info->tbl_qual[dl_md_loc].
       col_qual[dl_loop].column_name)
      IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].nullable != "N"))
       SET dl_parse1->stmt[dl_cnt].str = concat(dl_parse1->stmt[dl_cnt].str," or (t.",tmc_md_info->
        tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," is null and s.",tmc_md_info->tbl_qual[
     ...
     ENDIF
   ENDFOR
   SET dl_parse1->stmt[dl_cnt].str = concat(substring(1,(size(dl_parse1->stmt[dl_cnt].str) - 4),
     dl_parse1->stmt[dl_cnt].str)," ^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = "ASIS(^ where s.status = 'S_INTLSRCH' ^)"
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse1->stmt[dl_cnt].str = " go "
   SET stat = alterlist(dl_parse1->stmt,dl_cnt)
   SET dl_cnt = 1
   SET dl_view_cnt = 0
   SET stat = alterlist(dl_parse2->stmt,500)
   SET dl_parse2->stmt[dl_cnt].str = "RDB ASIS(^INSERT ALL ^)"
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = " ASIS(^ WHEN (("
   SET dl_ind = 0
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1))
      IF (dl_ind=1)
       SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," or (")
      ENDIF
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," f_",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," is null and t_",tmc_md_info->tbl_qual[
       dl_md_loc].col_qual[dl_loop].column_name,
       " is not null )")
      SET dl_ind = 1
     ENDIF
   ENDFOR
   SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str,")^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = concat("ASIS(^ then into dm_rdds_",trim(cnvtstring(currdbhandle)
     ),"_aud_gttd^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = "ASIS(^("
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].audit_flag=1))
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str,tmc_md_info->tbl_qual[
       dl_md_loc].col_qual[dl_loop].column_name,", ")
     ENDIF
   ENDFOR
   SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str,"^)")
   ...
   SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str,
    "ASIS(^ status, pk_where, pk_where_value, filtered_ind) ^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = concat("ASIS(^ VALUES(")
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].audit_flag=1))
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str,"t_",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name,", ")
     ENDIF
   ENDFOR
   SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," 'TARGET',^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = concat("ASIS(^ ",replace(dl_info->tables[dl_info_loc].
     tgt_pkw_str,"d.","t_"),",^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = concat("ASIS(^ dbms_utility.get_hash_value(",replace(dl_info->
     tables[dl_info_loc].tgt_pkw_str,"d.","t_"),",0,1073741824.0),^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = concat("ASIS(^ ",replace(dl_info->tables[dl_info_loc].
     tgt_filter_str,"d.","t_"),")^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = "ASIS(^ select "
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].audit_flag IN (1, 2)))
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," t.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," T_",tmc_md_info->tbl_qual[dl_md_loc].
       col_qual[dl_loop].column_name,
       ", ")
     ENDIF
   ...
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1)
      AND (tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].idcd_ind=1))
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," s.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," F_",tmc_md_info->tbl_qual[dl_md_loc].
       col_qual[dl_loop].column_name,
       ", ")
     ELSEIF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1))
     ...
     ENDIF
   ENDFOR
   SET dl_parse2->stmt[dl_cnt].str = substring(1,(size(dl_parse2->stmt[dl_cnt].str) - 1),dl_parse2->
    stmt[dl_cnt].str)
   SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str,"^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = concat("ASIS(^ from dm_rdds_",trim(cnvtstring(currdbhandle)),
    "_aud_gttd t ")
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1)
      AND (tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].idcd_ind=1))
      SET dl_view_cnt = (dl_view_cnt+ 1)
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," left outer join ^)")
      SET dl_cnt = (dl_cnt+ 1)
      SET dl_parse2->stmt[dl_cnt].str = concat(" ASIS(^ v_rdds_audit_xlat v",trim(cnvtstring(
         dl_view_cnt))," on ^)")
      SET dl_cnt = (dl_cnt+ 1)
      SET dl_parse2->stmt[dl_cnt].str = " ASIS(^ "
      IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].parent_entity_col > " "))
       SET dl_info_col_loc = locateval(dl_info_col_loc,1,dl_info->tables[dl_info_loc].column_cnt,
        tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].column_name,dl_info->tables[dl_info_loc].
        columns[dl_info_col_loc].column_name)
       SET dl_tab_list = concat("decode(nvl(t.",tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].
        parent_entity_col,",'RDDS_NONAME'),'RDDS_NONAME','INVALIDTABLE',' ','INVALIDTABLE','")
       FOR (dl_loop2 = 1 TO dl_info->tables[dl_info_loc].columns[dl_info_col_loc].pe_val_cnt)
         SET dl_tab_list = concat(dl_tab_list,dl_info->tables[dl_info_loc].columns[dl_info_col_loc].
          parent_entity_vals[dl_loop2].pe_name,"','",dl_info->tables[dl_info_loc].columns[
      ...
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," t.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," = v",trim(cnvtstring(dl_view_cnt)),
       ".to_value and v",trim(cnvtstring(dl_view_cnt)),".table_name = ",dl_tab_list," and t.",
       tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," > 0.0 ")
     ENDIF
   ENDFOR
   SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str,"^)")
   SET dl_view_cnt = 0
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = concat(" ASIS(^ left outer join ",tmc_md_info->tbl_qual[
    dl_md_loc].table_name,drgi_global->db_link," s on ")
   FOR (dl_loop = 1 TO tmc_md_info->tbl_qual[dl_md_loc].col_cnt)
     IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1)
      AND (tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].idcd_ind=1))
      SET dl_view_cnt = (dl_view_cnt+ 1)
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," ((s.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," = v",trim(cnvtstring(dl_view_cnt)),
       ".from_value ")
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," or (t.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," <= 0.0 and t.",tmc_md_info->tbl_qual[
       dl_md_loc].col_qual[dl_loop].column_name,
       " = s.",tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].column_name,")) ")
      IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].nullable != "N"))
       SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," or (t.",tmc_md_info->
        tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," is null and s.",tmc_md_info->tbl_qual[
        dl_md_loc].col_qual[dl_loop].column_name,
        " is null)")
      ENDIF
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str,") and ")
     ELSEIF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].pk_ind=1))
      SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," (t.",tmc_md_info->
       tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," = s.",tmc_md_info->tbl_qual[dl_md_loc].
       col_qual[dl_loop].column_name)
      IF ((tmc_md_info->tbl_qual[dl_md_loc].col_qual[dl_loop].nullable != "N"))
       SET dl_parse2->stmt[dl_cnt].str = concat(dl_parse2->stmt[dl_cnt].str," or (t.",tmc_md_info->
        tbl_qual[dl_md_loc].col_qual[dl_loop].column_name," is null and s.",tmc_md_info->tbl_qual[
     ...
     ENDIF
   ENDFOR
   SET dl_parse2->stmt[dl_cnt].str = concat(substring(1,(size(dl_parse2->stmt[dl_cnt].str) - 4),
     dl_parse2->stmt[dl_cnt].str)," ^)")
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = "ASIS(^ where t.status = 'T_INTLSRCH' ^)"
   SET dl_cnt = (dl_cnt+ 1)
   SET dl_parse2->stmt[dl_cnt].str = " go "
   SET stat = alterlist(dl_parse2->stmt,dl_cnt)
   RETURN(null)
 END ;Subroutine
 DECLARE draq_tab_name = vc WITH protect, noconstant(" ")
 DECLARE draq_md_loc = i4 WITH protect, noconstant(0)
 DECLARE draq_qp_loc = i4 WITH protect, noconstant(0)
 DECLARE draq_loop = i4 WITH protect, noconstant(0)
 DECLARE draq_info_loc = i4 WITH protect, noconstant(0)
 DECLARE draq_col_str = vc WITH protect, noconstant(" ")
 DECLARE draq_src_qual = i4 WITH protect, noconstant(0)
 DECLARE draq_tgt_qual = i4 WITH protect, noconstant(0)
 DECLARE draq_limit = f8 WITH protect, noconstant(0.0)
 FREE RECORD draq_parse_rs1
 RECORD draq_parse_rs1(
   1 stmt[*]
     2 str = vc
 )
 FREE RECORD draq_parse_rs2
 RECORD draq_parse_rs2(
   1 stmt[*]
     2 str = vc
 )
 FREE RECORD draq_parse_rs3
 RECORD draq_parse_rs3(
   1 stmt[*]
     2 str = vc
 )
 FREE RECORD draq_parse_rs4
 RECORD draq_parse_rs4(
   1 stmt[*]
     2 str = vc
 )
 FREE RECORD draq_parse_rs5
 RECORD draq_parse_rs5(
   1 stmt[*]
     2 str = vc
 )
 IF (check_logfile("dm_rmc_audit_query",".log","dm_rmc_audit_query LOG FILE...") != 1)
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm_rmc_audit_query..."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (reflect(parameter(1,0)) != "C*")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = 'Expected syntax:  dm_rmc_audit_query "<Table Name to Audit>"'
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET draq_tab_name =  $1
 IF ((dm_err->debug_flag > 0))
  CALL echo(draq_tab_name)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="RDDS CONFIGURATION"
   AND d.info_name="SOLUTION AUDIT SIZE LIMIT"
  DETAIL
   draq_limit = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 ENDIF
 IF (curqual=0)
  SET draq_limit = 10000.00
  INSERT  FROM dm_info d
   SET d.info_domain = "RDDS CONFIGURATION", d.info_name = "SOLUTION AUDIT SIZE LIMIT", d.info_number
     = draq_limit
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
 ENDIF
 SET draq_md_loc = locateval(draq_md_loc,1,tmc_md_info->tbl_cnt,draq_tab_name,tmc_md_info->tbl_qual[
  draq_md_loc].table_name)
 IF (draq_md_loc=0)
  IF ((drgi_global->db_link <= " "))
   ...
   ENDIF
  ENDIF
  CALL drm_md_wrp(draq_tab_name,drgi_global->db_link)
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
  SET draq_md_loc = locateval(draq_md_loc,1,tmc_md_info->tbl_cnt,draq_tab_name,tmc_md_info->tbl_qual[
   draq_md_loc].table_name)
 ENDIF
 IF ((validate(dra_cnt,- (531)) != - (531)))
  SET draq_info_loc = dra_cnt
 ELSE
  SET draq_info_loc = locateval(draq_info_loc,1,table_info->tab_cnt,draq_tab_name,table_info->tables[
   draq_info_loc].table_name)
 ENDIF
 SET draq_qp_loc = locateval(draq_qp_loc,1,drqp_reply->tab_cnt,draq_tab_name,drqp_reply->tab_qual[
  draq_qp_loc].table_name)
 SET draq_col_str = "distinct 'S_INTLSRCH'"
 FOR (draq_loop = 1 TO tmc_md_info->tbl_qual[draq_md_loc].col_cnt)
   IF ((tmc_md_info->tbl_qual[draq_md_loc].col_qual[draq_loop].audit_flag IN (1, 2))
    AND (tmc_md_info->tbl_qual[draq_md_loc].col_qual[draq_loop].in_src_ind=1))
    SET draq_col_str = concat(draq_col_str,",",drqp_reply->tab_qual[draq_qp_loc].table_alias,".",
     tmc_md_info->tbl_qual[draq_md_loc].col_qual[draq_loop].column_name)
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Inserting source data subset into GTTD..."
 CALL disp_msg(" ",dm_err->logfile,0)
 CALL parser(concat("insert into dm_rdds_",trim(cnvtstring(currdbhandle)),"_aud_gttd(status"),0)
 FOR (draq_loop = 1 TO tmc_md_info->tbl_qual[draq_md_loc].col_cnt)
   IF ((tmc_md_info->tbl_qual[draq_md_loc].col_qual[draq_loop].audit_flag IN (1, 2)))
    CALL parser(concat(",",tmc_md_info->tbl_qual[draq_md_loc].col_qual[draq_loop].column_name),0)
   ENDIF
 ENDFOR
 CALL parser(")(",0)
 FOR (draq_loop = 1 TO drqp_reply->cnt)
   CALL parser(concat(replace(replace(drqp_reply->qual[draq_loop].text,"<COLUMN_LIST>",draq_col_str),
      "<DB_LINK>",drgi_global->db_link)),0)
 ENDFOR
 CALL parser(") with nocounter go",1)
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET draq_src_qual = curqual
 SET dm_err->eproc = "Inserting target data subset into GTTD..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET draq_col_str = replace(draq_col_str,"S_INTLSRCH","T_INTLSRCH")
 FOR (draq_loop = 1 TO tmc_md_info->tbl_qual[draq_md_loc].col_cnt)
   IF ((tmc_md_info->tbl_qual[draq_md_loc].col_qual[draq_loop].audit_flag IN (1, 2))
    AND (tmc_md_info->tbl_qual[draq_md_loc].col_qual[draq_loop].in_src_ind=0))
    SET draq_col_str = concat(draq_col_str,",",drqp_reply->tab_qual[draq_qp_loc].table_alias,".",
     tmc_md_info->tbl_qual[draq_md_loc].col_qual[draq_loop].column_name)
   ENDIF
 ENDFOR
 ...
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_program
 ENDIF
 SET draq_tgt_qual = curqual
 SET dm_err->eproc = "Creating function strings for query..."
 CALL disp_msg(" ",dm_err->logfile,0)
 CALL draq_fcn_strs(draq_info_loc,table_info)
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Building and executing audit query..."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (draq_src_qual < draq_limit
  AND draq_tgt_qual < draq_limit
  AND (table_info->tables[draq_info_loc].pk_cnt <= 1))
  CALL draq_direct_join(draq_md_loc,draq_info_loc,table_info,draq_parse_rs1,draq_parse_rs2,
   draq_parse_rs3,draq_parse_rs4,draq_parse_rs5)
 ELSE
  CALL draq_outer_join(draq_md_loc,draq_info_loc,table_info,draq_parse_rs1,draq_parse_rs2)
 ENDIF
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DRAQ_PARSE_RS1")
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DRAQ_PARSE_RS2")
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (size(draq_parse_rs3->stmt,5) > 0)
  EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DRAQ_PARSE_RS3")
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DRAQ_PARSE_RS4")
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DRAQ_PARSE_RS5")
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
