CREATE PROGRAM bed_rpt_file:dba
 ...
 DECLARE writereporttocsv(dummyvar=i2) = i2
 DECLARE closecsvwrittento(dummyvar=i2) = i2
 DECLARE row_string = vc
 DECLARE header_string = vc
 DECLARE cell_value = vc
 IF (validate(request->output_filename))
  SET output_filename = request->output_filename
 ELSE
  CALL bederror("Missing output file parameter.")
 ENDIF
 CALL preparecsvtowrite(0)
 CALL writereporttocsv(0)
 ...
 END ;Subroutine
 SUBROUTINE writereporttocsv(dummyvar)
   ...
   SET frec->file_buf = ""
   FOR (x = 1 TO size(reply->rowlist,5))
     FOR (y = 1 TO size(reply->rowlist[x].celllist,5))
       SET cell_value = " "
