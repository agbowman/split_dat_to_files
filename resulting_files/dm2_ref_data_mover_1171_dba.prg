CREATE PROGRAM dm2_ref_data_mover_1171:dba
 ...
 DECLARE cust_pe_name = vc WITH constant("APP_PREFS")
 ...
 SET nvp_tab_name = concat(value(dm2_ref_data_doc->pre_link_name),"NAME_VALUE_PREFS",value(
   dm2_ref_data_doc->post_link_name))
 SET from_id = get_value(cust_tab_name,"APP_PREFS_ID","FROM")
 ...
 ENDFOR
 IF (good_pref_ind=1)
  SET temp_col_cnt = get_col_pos(cust_tab_name,"APP_PREFS_ID")
  IF ((dm2_ref_data_reply->error_ind=1))
   GO TO exit_1171
  ENDIF
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
   ...
   IF (no_query=1)
    ...
    IF (val < 0)
     GO TO exit_1171
    ELSEIF (val=0)
     SET val = get_seq(cust_tab_name,"APP_PREFS_ID")
     IF (val > 1)
      CALL put_value(cust_tab_name,"APP_PREFS_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  APP_PREFS_ID")
     ELSE
      GO TO exit_1171
     ENDIF
    ELSE
    ...
    ENDIF
    SET new_cd = get_value(cust_tab_name,"APP_PREFS_ID","TO")
    IF ((dm2_ref_data_reply->error_ind=1))
     GO TO exit_1171
    ELSE
 ...
 ELSE
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "APP_PREFS_ID is related to a preference that isn't mergeable."
