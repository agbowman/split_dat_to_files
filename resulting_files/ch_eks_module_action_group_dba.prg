CREATE PROGRAM ch_eks_module_action_group:dba
 PROMPT
 ...
 DECLARE action_ident = i2 WITH protect, constant(9)
 ...
 DECLARE alogic_string = vc WITH persistscript, noconstant(" ")
 ...
 DECLARE log_pos = i4 WITH protect, noconstant(0)
 ...
 RECORD discern_rule_action_group(
   1 action_group_data[*]
     2 module_name = vc
     2 version = vc
     2 action_group = vc
     2 action_group_logic = vc
     2 action_group_operator = vc
     2 action_group_id = vc
     2 action_group_template = vc
     2 action_id = vc
 ) WITH protect
 ...
 SUBROUTINE (parsetemplate(txt=vc) =null)
   ...
   DECLARE token_cnt = i4 WITH protect, noconstant(size(actiontokens->list,5))
   ...
   WHILE (size(remainder) > 0
    AND limit < upper_bound)
     ...
     IF (limit != 1)
      SET record_cnt += 1
      SET stat = alterlist(discern_rule_action_group->action_group_data,record_cnt)
      SET discern_rule_action_group->action_group_data[record_cnt].action_group =
      discern_rule_action_group->action_group_data[(record_cnt - 1)].action_group
     ENDIF
     SET stat = alterlist(actiontokens->list,token_cnt)
     SET token = getnext(remainder,etx)
     IF ( NOT (isoperator(token)))
      SET template_cnt += 1
      SET action_template_cnt += 1
      SET actiontokens->list[token_cnt].index = template_cnt
      IF (token != not_found)
      ...
      ELSE
       SET remainder = null
      ENDIF
      SET discern_rule_action_group->action_group_data[record_cnt].action_group_template = token
      SET discern_rule_action_group->action_group_data[record_cnt].action_id = build("A",
       action_template_cnt)
     ELSE
      SET discern_rule_action_group->action_group_data[record_cnt].action_group_operator = token
     ENDIF
     SET discern_rule_action_group->action_group_data[record_cnt].action_group_id = build("G",
      template_cnt)
     SET discern_rule_action_group->action_group_data[record_cnt].module_name = module_name
     SET discern_rule_action_group->action_group_data[record_cnt].version = version
   ENDWHILE
   SET actiontokens->cnt = token_cnt
   SET stat = alterlist(actiontokens->list,token_cnt)
   SET stat = alterlist(discern_rule_action_group->action_group_data,record_cnt)
 ...
 SUBROUTINE (findtemplateend(txt=vc) =i4 WITH protect)
   ...
   RETURN(pos)
 END ;Subroutine
 SUBROUTINE (parseactiongroup(txt=vc) =null WITH protect)
   DECLARE upper_bound = i2 WITH protect, constant(2000)
   DECLARE remainder = vc WITH protect, noconstant(txt)
   DECLARE token = vc WITH protect, noconstant(" ")
   DECLARE logic_template = vc WITH protect, noconstant(" ")
   DECLARE op_string = vc WITH protect, noconstant(" ")
   DECLARE l_string = vc WITH protect, noconstant(" ")
   DECLARE logic_string = vc WITH protect, noconstant(" ")
   DECLARE limit = i4 WITH protect, noconstant(0)
   WHILE (size(remainder) > 0
    AND limit < upper_bound)
     SET limit += 1
     ...
     IF ( NOT (isoperator(token)))
      IF ((tokens->cnt > 0)
       AND isnumeric(token)=1)
       SET logic_template = build(tokens->list[(cnvtint(token)+ 1)].index)
      ELSE
       SET logic_template = token
      ENDIF
      SET logic_string = build2(logic_string," L",logic_template)
     ELSE
      SET logic_string = build2(logic_string," ",token)
     ENDIF
   ENDWHILE
   SET alogic_string = build2("IF",logic_string)
   IF (limit >= upper_bound)
    CALL cclexception(999,"E","Infinite loop in parseActionGroup")
   ENDIF
   RETURN(alogic_string)
 END ;Subroutine
 SUBROUTINE (parselogictemplate(txt=vc) =null)
   DECLARE upper_bound = i4 WITH protect, constant(2000)
   DECLARE token = vc WITH protect, noconstant(" ")
   DECLARE remainder = vc WITH protect, noconstant(" ")
   DECLARE parameter_list = vc WITH protect, noconstant(" ")
 ...
 DECLARE action_group = vc WITH protect, noconstant(" ")
 DECLARE grp_cnt = i4 WITH noconstant(0)
 DECLARE gcnt = i4 WITH noconstant(0)
 DECLARE acnt = i4 WITH noconstant(0)
 DECLARE record_cnt = i4 WITH noconstant(0)
 DECLARE action_template_cnt = i4 WITH noconstant(0)
 ...
 IF (validate(ctp_module_name))
  SET stat = alterlist(eksmod_reply->qual,2)
  SET eksmod_reply->qual[1].module_name = cnvtupper(ctp_module_name)
  SET eksmod_reply->qual[2].module_name = "PHA_STD_ACET_MAXDOSE_7"
 ...
 FOR (i = 1 TO size(eksmod_reply->qual,5))
   ...
   SET gcnt = 0
   SET limit = 0
   SET action_template_cnt = 0
   SET action_template_remainder = ""
   SET remainder = ""
   SET logic_template = " "
   SET action_template = " "
   SET alogic_string = " "
   IF (stat)
    ...
    SET pos = locateval(idx,1,size(eksmodstor_reply->stor,5),action_ident,eksmodstor_reply->stor[idx]
     .data_type)
    SET log_pos = locateval(idx,1,size(eksmodstor_reply->stor,5),logic,eksmodstor_reply->stor[idx].
     data_type)
    IF (log_pos > 0)
     IF (size(trim(eksmodstor_reply->stor[log_pos].ekm_info)) > 0)
      SET logic_template = eksmodstor_reply->stor[log_pos].ekm_info
     ELSE
      SET logic_template = eksmodstor_reply->stor[log_pos].ekm_info_blob
     ENDIF
    ENDIF
    CALL parselogictemplate(logic_template)
    IF (pos > 0)
    ...
    WHILE (size(action_template_remainder) > 0
     AND limit < upper_bound)
      ...
      IF (action_group != not_found)
       ...
       SET discern_rule_action_group->action_group_data[record_cnt].action_group = concat(
        "Action Group ",trim(cnvtstring(gcnt)))
       SET discern_rule_action_group->action_group_data[record_cnt].module_name = module_name
       SET discern_rule_action_group->action_group_data[record_cnt].version = version
       CALL parseactiongroup(action_group)
       SET discern_rule_action_group->action_group_data[record_cnt].action_group_logic =
       alogic_string
      ENDIF
      IF (action_template_remainder=remainder)
       SET remainder = " "
      ENDIF
      SET action_template_remainder = remainder
      CALL parsetemplate(action_template)
    ENDWHILE
    IF (limit >= upper_bound)
     CALL cclexception(999,"E","Infinite loop in printActionSection")
    ENDIF
   ENDIF
 ENDFOR
 CALL echo(size(discern_rule_action_group->action_group_data,5))
 FOR (i = 1 TO size(discern_rule_action_group->action_group_data,5))
   SET manualxml = ""
   IF (i > 1)
    SET manualxml = char(10)
   ENDIF
   SET manualxml = concat(manualxml,"<MODULE_NAME>",trim(discern_rule_action_group->
     action_group_data[i].module_name,3),"</MODULE_NAME>","<MODULE_VERSION>",
    trim(discern_rule_action_group->action_group_data[i].version,3),"</MODULE_VERSION>",
    "<ACTION_GROUP_OPERATOR>",trim(discern_rule_action_group->action_group_data[i].
     action_group_operator,3),"</ACTION_GROUP_OPERATOR>",
    "<ACTION_GROUP_ID>",trim(discern_rule_action_group->action_group_data[i].action_group_id,3),
    "</ACTION_GROUP_ID>","<ACTION_GROUP_TEMPLATE>",trim(discern_rule_action_group->action_group_data[
     i].action_group_template,3),
    "</ACTION_GROUP_TEMPLATE>","<ACTION_GROUP_LOGIC>",trim(discern_rule_action_group->
     action_group_data[i].action_group_logic,3),"</ACTION_GROUP_LOGIC>","<ACTION_ID>",
    trim(discern_rule_action_group->action_group_data[i].action_id,3),"</ACTION_ID>")
   SET stat = writetofile("ccluserdir:mill_rules_discern_rule_action_group_2023_07_05_00_02_40.dat",
    manualxml)
   IF (mod(i,100)=0)
