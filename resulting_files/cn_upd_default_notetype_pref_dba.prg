CREATE PROGRAM cn_upd_default_notetype_pref:dba
 ...
 ;end update
 UPDATE  FROM name_value_prefs nvp
  SET nvp.merge_name = "CODE_VALUE", nvp.merge_id =
   (SELECT
    nt.event_cd
    FROM note_type nt
    WHERE nt.note_type_id=nvp.merge_id)
  WHERE nvp.merge_name="NOTE_TYPE"
   AND nvp.pvc_name="pvNotes.DefaultNoteType"
   AND nvp.pvc_value != "0"
   AND nvp.pvc_value != " "
  WITH nocounter
 ;end update
