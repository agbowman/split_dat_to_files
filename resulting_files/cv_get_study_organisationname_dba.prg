CREATE PROGRAM cv_get_study_organisationname:dba
 IF (validate(stat)=0)
 ...
 SELECT INTO "nl:"
  FROM organization o,
   im_device_org_r i,
   im_acquired_study im
  PLAN (im
   WHERE (im.im_acquired_study_id=request->acquired_study_id))
   JOIN (i
   WHERE im.im_device_id=i.im_device_id)
   JOIN (o
   WHERE i.organization_id=o.organization_id)
  DETAIL
   reply->study_organisationname = o.org_name
 ...
 ENDIF
 CALL echo(build("Reply :",reply->study_organisationname))
 CALL echo(build("StatusData :",reply->status_data))
 CALL cv_log_msg_post("MOD 001 10/08/15 MG023115")
END GO
