CREATE PROGRAM dm_pft_wf_hist_rem_reltn_rows:dba
 ...
 SET reply->table_name = "PFT_WF_HIST_REMINDER_RELTN"
 ...
 IF (days_to_keep < 730)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must keep at least 730 day's worth of data. You entered %1 days or did not enter any value.",
   "i",days_to_keep)
  SET reply->status_data.status = "F"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(whrr.pft_wf_hist_reminder_reltn_id)
   FROM pft_wf_hist_reminder_reltn whrr
   WHERE whrr.pft_wf_hist_reminder_reltn_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(whrr.pft_wf_hist_reminder_reltn_id)
  FROM pft_wf_hist_reminder_reltn whrr
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    whrr.rowid
    FROM pft_wf_hist_reminder_reltn whrr
    WHERE whrr.pft_wf_hist_reminder_reltn_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists(
      "whrr.created_dt_tm < cnvtdatetime((curdate - days_to_keep), 0)","whrr"))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,500)=1)
      stat = alterlist(reply->rows,(rows+ 499))
     ENDIF
     reply->rows[rows].row_id = whrr.rowid
    WITH nocounter, maxqual(whrr,value(rowsleft))
   ;end select
