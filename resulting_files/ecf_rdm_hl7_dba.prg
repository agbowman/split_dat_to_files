CREATE PROGRAM ecf_rdm_hl7:dba
 ...
 SUBROUTINE kia_script_inc(pkg,ccl_obj)
   ...
   CALL parser(obj_prs)
 END ;Subroutine
 DECLARE kia_concept_filename = vc WITH public, constant("hl7_concept.csv")
 DECLARE kia_reltn_filename = vc WITH public, constant("hl7_relation.csv")
 DECLARE kia_nomen_csv_filename = vc WITH public, constant("hl7_nomen.csv")
 DECLARE kia_cur_ver_beg_dt = vc WITH public, noconstant("")
 SET kia_short_desc = "MOH_PROCEDURE Content Load"
 SET kia_rdm_log_name = "ecf_rdm_hl7.log"
 SET kia_rdm_nbr = 9535
 SET kia_rdm_vocab_mean = "HL7"
 SET readme_data->status = "F"
 SET readme_data->message = concat("Failed - ",trim(kia_long_desc))
 SET script_status = " "
 SET csv_stat = 1
 CALL kia_inhouse_chk(1)
 CALL kia_db2_check(1)
 EXECUTE kia_dm_dbimport "cer_install:hl7_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_error(1)
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET readme_data->status = "F"
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("ecf_rdm_hl7 Begin> ",format(cnvtdatetime(curdate,curtime3),
 ...
 CALL kia_chk_error(1)
 CALL kia_chk_status_info("kia_load_cmt_concept_by_ld_2","check")
 CALL kia_chk_error(1)
 IF (kia_obj_suc_ind=0)
  CALL echo("DELETING CMT_CONCEPT_LOAD_NS ROWS...")
  DELETE  FROM cmt_concept_load_ns c
   WHERE ((c.concept_cki="HL7*") OR (c.concept_cki="SNOMED*"))
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_concept_filename,"kia_imp_concept_load_ns","kia_imp_concept_load_ns.log")
  CALL kia_chk_error(1)
  EXECUTE kia_load_cmt_concept_by_ld_2 kia_rdm_vocab_mean
  CALL kia_chk_script_failure("kia_load_cmt_concept_by_ld_2")
  CALL kia_set_status_success("kia_load_cmt_concept_by_ld_2")
 ENDIF
 CALL kia_chk_status_info("kia_load_concept_reltn_2","check")
 CALL kia_chk_error(1)
 IF (kia_obj_suc_ind=0)
  CALL echo("DELETING CMT_CONCEPT_RELTN_LD_NS ROWS...")
  DELETE  FROM cmt_concept_reltn_ld_ns c
   WHERE c.concept_cki1="HL7*"
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_reltn_filename,"kia_imp_reltn_load_ns","kia_imp_reltn_load_ns.log")
  CALL kia_chk_error(1)
  EXECUTE kia_load_concept_reltn_2 kia_rdm_vocab_mean
  CALL kia_chk_script_failure("kia_load_concept_reltn_2")
  CALL kia_set_status_success("kia_load_concept_reltn_2")
 ENDIF
 CALL kia_chk_status_info("LOAD NOMEN BASELINE DATA","check")
 CALL kia_chk_error(1)
 IF (kia_obj_suc_ind=0)
  CALL my_echo("DELETING EXISTING HL7 CONTENT FROM NOMECLATURE_LOAD_NS TABLE...")
  DELETE  FROM nomenclature_load_ns n
   WHERE n.source_vocabulary_mean IN (kia_rdm_vocab_mean)
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_nomen_csv_filename,"kia_imp_nomen_load_ns","kia_imp_nomen_load_ns.log")
  CALL kia_chk_error(1)
  CALL kia_set_status_success("LOAD NOMEN BASELINE DATA")
 ENDIF
 CALL kia_chk_status_info("kia_upd_cmti_nomen_2","check")
 CALL kia_chk_error(1)
 IF (kia_obj_suc_ind=0)
  EXECUTE kia_upd_cmti_nomen_2 kia_rdm_vocab_mean
  CALL kia_chk_script_failure("kia_upd_cmti_nomen_2")
  CALL kia_set_status_success("kia_upd_cmti_nomen_2")
 ENDIF
 CALL kia_chk_status_info("kia_load_nomen_by_cmti_full_2","check")
 CALL kia_chk_error(1)
 IF (kia_obj_suc_ind=0)
  EXECUTE kia_load_nomen_by_cmti_full_2 kia_rdm_vocab_mean
  CALL kia_chk_script_failure("kia_load_nomen_by_cmti_full_2")
  CALL kia_set_status_success("kia_load_nomen_by_cmti_full_2")
 ENDIF
 CALL kia_chk_status_info("cmt_build_nsi","check")
 CALL kia_chk_error(1)
 IF (kia_obj_suc_ind=0)
  EXECUTE cmt_build_nsi
  CALL kia_chk_script_failure("cmt_build_nsi")
  CALL kia_set_status_success("cmt_build_nsi")
 ENDIF
 CALL kia_chk_status_info("LOAD_CONCEPT_BASELINE_CERNER_CONCEPTS","check")
 IF (kia_obj_suc_ind=0)
  DELETE  FROM cmt_concept_load_ns c
   WHERE c.concept_cki="CERNER*"
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  EXECUTE kia_sqlldr "cmt_concept_load_ns", "cerner_concepts.csv", 0
  CALL kia_chk_script_failure("kia_sqlldr")
  CALL kia_set_status_success("LOAD_CONCEPT_BASELINE_CERNER_CONCEPTS")
 ENDIF
 CALL kia_chk_status_info("kia_load_cmt_concept_by_ld_2","check")
 IF (kia_obj_suc_ind=0)
  EXECUTE kia_load_cmt_concept_by_ld_2 "CERNER"
  CALL kia_chk_script_failure("kia_load_cmt_concept_by_ld_2")
  CALL kia_set_status_success("kia_load_cmt_concept_by_ld_2")
 ENDIF
 CALL kia_chk_error(1)
 CALL kia_set_parse_row_success(kia_long_desc)
 CALL kia_chk_error(1)
