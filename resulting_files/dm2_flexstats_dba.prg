CREATE PROGRAM dm2_flexstats:dba
 ...
 DECLARE df_beg_time = dq8 WITH protect, noconstant(0.0)
 DECLARE df_beg_tbl_time = dq8 WITH protect, noconstant(0.0)
 DECLARE df_end_time = dq8 WITH protect, noconstant(0.0)
 DECLARE df_analy_time = f8 WITH protect, noconstant(0.0)
 DECLARE df_tbl_time = f8 WITH protect, noconstant(0.0)
 DECLARE df_noanalyze_trig = i2 WITH protect, noconstant(0)
 DECLARE df_level = vc WITH protect, noconstant("")
 DECLARE df_owner = vc WITH protect, noconstant("")
 DECLARE df_table_name = vc WITH protect, noconstant("")
 DECLARE df_est_pct = f8 WITH protect, noconstant(0.0)
 DECLARE df_method_opt = vc WITH protect, noconstant("")
 DECLARE df_block_sample = vc WITH protect, noconstant("")
 DECLARE df_cascade = vc WITH protect, noconstant("")
 DECLARE df_err_ind = i2 WITH protect, noconstant(0)
 DECLARE df_err_emsg = vc WITH protect, noconstant("")
 DECLARE df_err_eproc = vc WITH protect, noconstant("")
 DECLARE df_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE df_err_ind = i2 WITH protect, noconstant(0)
 DECLARE df_table_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE df_publish_val = vc WITH protect, noconstant("NOTSET")
 DECLARE df_force_str = vc WITH protect, noconstant(" ")
 DECLARE df_stack_idx = i4 WITH protect, noconstant(0)
 DECLARE df_cclversion = i4 WITH protect, noconstant(0)
 DECLARE df_force_val = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE alter_trigger_status(ats_trigger_name=vc,ats_from_status=vc,ats_to_status=vc) = null
 IF (check_logfile("dm2_flexstats",".log","dm2_flexstats LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_flexstats."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET df_err_ind = dm_err->err_ind
 SET df_err_emsg = dm_err->emsg
 SET df_err_eproc = dm_err->eproc
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET df_level = trim(cnvtupper( $1),3)
 SET df_owner = trim(cnvtupper( $2),3)
 SET df_table_name = trim(cnvtupper( $3),3)
 SET df_est_pct = cnvtreal( $4)
 SET df_method_opt = trim(cnvtupper( $5),3)
 SET df_block_sample = trim(cnvtupper( $6),3)
 SET df_cascade = trim(cnvtupper( $7),3)
 SET df_cclversion = (((cnvtint(currev) * 10000)+ (cnvtint(currevminor) * 100))+ cnvtint(currevminor2
  ))
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = concat(
   "Parameter usage: dm2_flexstats '<level>', '<owner>', '<table_name>', <est_pct>,",
   "'<method_opt>', '<block_sample>', '<cascade>'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating <level> input parameter."
 IF ( NOT (df_level="TABLE"))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Valid values are 'TABLE'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating <est_pct> input parameter."
 IF ( NOT (((df_est_pct > 0.0
  AND df_est_pct <= 100.0) OR ((df_est_pct=- (1.0)))) ))
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "Valid values are greater than 0.0 and less than or equal to 100.0, or -1 for AUTO_SAMPLE_SIZE."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF ((dm2_rdbms_version->level1 > 10))
  SET dm_err->eproc = "Autosuccess for Oracle 11 and above."
  CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Verifying table still exists in dba_tables"
 SELECT INTO "nl:"
  FROM dba_tables dt
  WHERE dt.table_name=df_table_name
   AND dt.owner=df_owner
  DETAIL
   df_table_exists_ind = 1
 ...
 IF (check_error(dm_err->eproc))
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Calling SYS.DBMS_STATS.GATHER_TABLE_STATS for ",df_table_name)
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dclcom1 = build('RDB ASIS(" begin SYS.DBMS_STATS.GATHER_TABLE_STATS(',"ownname=>'",df_owner,"'",
  ",tabname=>'",
  df_table_name,"'",",estimate_percent=>",evaluate(df_est_pct,- (1.0),
   "SYS.DBMS_STATS.AUTO_SAMPLE_SIZE",build(df_est_pct)),",method_opt=>'",
  df_method_opt,"'",evaluate(table_stats->table_degree,- (1)," ",build(" ,degree=>",table_stats->
    table_degree)),",block_sample=>",df_block_sample,
  ",cascade=>",df_cascade,df_force_str,'); end;") go')
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_add_detail_number(dpl_est_pct,df_est_pct)
 CALL dm2_process_log_add_detail_text(dpl_owner,df_owner)
 CALL dm2_process_log_add_detail_text(dpl_table,df_table_name)
 CALL dm2_process_log_add_detail_text(dpl_method_opt,df_method_opt)
 CALL dm2_process_log_add_detail_text(dpl_cmd,dclcom1)
 IF (df_table_exists_ind=0)
  CALL dm2_process_log_add_detail_text("GATHER_BYPASS_REASON","TABLE NAME NO LONGER EXISTS")
 ENDIF
 CALL dm2_process_log_row(dpl_statistics,dpl_table_stats_gathering,dpl_no_prev_id,1)
 SET df_log_id = dm2_process_event_rs->dm_process_event_id
 IF ((dm_err->debug_flag >= 1))
  SET df_beg_time = cnvtdatetime(curdate,curtime3)
 ENDIF
 IF (df_table_exists_ind=1)
  IF (dm_ora_pre_stat(df_table_name,df_owner,df_publish_val)=0)
   SET dm_err->err_ind = 0
   SET df_err_ind = 1
  ENDIF
  IF (df_err_ind != 1)
   IF (df_cclversion >= 80506)
    IF ((table_stats->table_degree=- (1)))
     DECLARE gather_table_stats(ownname=vc,tabname=vc,estimate_percent=f8,method_opt=vc,block_sample=
      i4,
      cascade=i4) = null WITH sql = "SYS.DBMS_STATS.GATHER_TABLE_STATS", parameter
     CALL gather_table_stats(df_owner,df_table_name,evaluate(df_est_pct,- (1.0),0.0,df_est_pct),
      df_method_opt,cnvtbool(evaluate(df_block_sample,"TRUE",true,false)),
      cnvtbool(evaluate(df_cascade,"TRUE",true,false)))
    ELSE
     DECLARE gather_table_stats(ownname=vc,tabname=vc,estimate_percent=f8,method_opt=vc,degree=i4,
      block_sample=i4,cascade=i4) = null WITH sql = "SYS.DBMS_STATS.GATHER_TABLE_STATS", parameter
     CALL gather_table_stats(df_owner,df_table_name,evaluate(df_est_pct,- (1.0),0.0,df_est_pct),
      df_method_opt,table_stats->table_degree,
      cnvtbool(evaluate(df_block_sample,"TRUE",true,false)),cnvtbool(evaluate(df_cascade,"TRUE",true,
        false)))
    ENDIF
   ELSE
   ...
   IF (check_error(dm_err->eproc)=1)
    IF ((dm_err->debug_flag >= 1))
     SET df_end_time = cnvtdatetime(curdate,curtime3)
     SET df_analy_time = datetimediff(df_end_time,df_beg_time,5)
     SET dm_err->eproc = build("Stats Gather time for table: ",df_table_name,"=",df_analy_time,
      " seconds.")
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SET dm_err->err_ind = 0
  ...
  ENDIF
  IF (df_err_ind != 1)
   IF (dm_ora_post_stat(df_table_name,df_owner)=0)
    SET dm_err->err_ind = 0
    SET df_err_ind = 1
   ENDIF
  ENDIF
 ENDIF
 SET dm2_process_event_rs->status = evaluate(df_err_ind,1,dpl_failure,dpl_success)
 SET dm2_process_event_rs->message = evaluate(df_err_ind,1,dm_err->emsg,"")
 CALL dm2_process_log_row(dpl_statistics,dpl_table_stats_gathering,df_log_id,1)
 IF (df_err_ind)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag >= 1))
  ...
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SUBROUTINE alter_trigger_status(ats_trigger_name,ats_from_status,ats_to_status)
   DECLARE ats_from_status_value = vc
   DECLARE ats_to_status_value = vc
   CASE (ats_from_status)
    OF "ENABLE":
     SET ats_from_status_value = "ENABLED"
    OF "DISABLE":
     SET ats_from_status_value = "DISABLED"
   ENDCASE
   CASE (ats_to_status)
    OF "ENABLE":
     SET ats_to_status_value = "ENABLED"
    OF "DISABLE":
     SET ats_to_status_value = "DISABLED"
   ENDCASE
   SET dm_err->eproc = concat("Checking status of trigger in dm2_user_triggers.")
   SELECT INTO "NL:"
    FROM dm2_user_triggers dut
    WHERE dut.trigger_name=ats_trigger_name
     AND dut.status=ats_from_status_value
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (curqual > 0)
    IF ((dm_err->debug_flag >= 4))
     SET dm_err->eproc = concat("Trigger ",ats_trigger_name," found in ",ats_from_status_value,
      " status.  Altering trigger...")
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SET dclcom1 = concat('RDB ASIS("ALTER TRIGGER ',ats_trigger_name," ",ats_to_status,'") GO')
    IF (dm2_push_cmd(dclcom1,1)=0)
     IF ((dm_err->debug_flag >= 4))
      SET dm_err->eproc = concat("Failed to alter trigger ",ats_trigger_name," from ",ats_from_status,
       " to ",
       ats_to_status,".  Script will continue.")
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     SET dm_err->err_ind = 0
     SET df_noanalyze_trig = 0
    ELSE
     IF ((dm_err->debug_flag >= 4))
      SET dm_err->eproc = concat("Trigger ",ats_trigger_name," status successfully altered from ",
       ats_from_status," to ",
       ats_to_status,".")
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     IF (ats_to_status="DISABLE")
      SET df_noanalyze_trig = 1
     ENDIF
    ENDIF
   ELSE
    IF ((dm_err->debug_flag >= 4))
     SET dm_err->eproc = concat("Trigger ",ats_trigger_name," already ",ats_to_status_value,
      ".  Alter bypassed.")
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
   ENDIF
 END ;Subroutine
#exit_script
 IF (df_noanalyze_trig=1)
  CALL alter_trigger_status("DM2_NOANALYZE","DISABLE","ENABLE")
 ENDIF
 CALL dm_ora_fin_clean(df_table_name,df_owner,df_publish_val)
 SET dm_err->eproc = "Ending dm2_flexstats."
 CALL final_disp_msg("dm2_flexstats")
 SET dm_err->err_ind = df_err_ind
 SET dm_err->emsg = df_err_emsg
 SET dm_err->eproc = df_err_eproc
