CREATE PROGRAM dm2_ref_data_mover_0858:dba
 ...
 SUBROUTINE insert_update_row(iur_temp_tbl_cnt,iur_perm_col_cnt)
   ...
   IF ((dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].merge_delete_ind=1)
    ...
    IF ((dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].child_flag=2))
     SET drdm_parser_cnt = 1
     FOR (iur_child_loop = 1 TO size(dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].child_qual,5))
       SET drdm_parser->statement[1].frag = concat("delete from ",dm2_ref_data_doc->tbl_qual[
        iur_temp_tbl_cnt].child_qual[iur_child_loop].child_name," where ")
       IF ((dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].child_qual[iur_child_loop].parent_tab_col
        != ""))
        SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
        SET drdm_parser->statement[drdm_parser_cnt].frag = concat(dm2_ref_data_doc->tbl_qual[
         iur_temp_tbl_cnt].child_qual[iur_child_loop].parent_tab_col," = '",dm2_ref_data_doc->
         tbl_qual[iur_temp_tbl_cnt].table_name,"' and ")
       ENDIF
       SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
       SET drdm_parser->statement[drdm_parser_cnt].frag = concat(dm2_ref_data_doc->tbl_qual[
        iur_temp_tbl_cnt].child_qual[iur_child_loop].parent_id_col," in (select ")
   ...
   IF ((dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].merge_delete_ind=1))
    IF ((dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].child_flag=2))
     ...
     FOR (iur_child_loop = 1 TO size(dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].child_qual,5))
       ...
       SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
       SET drdm_parser->statement[drdm_parser_cnt].frag = concat(dm2_ref_data_doc->tbl_qual[
        iur_temp_tbl_cnt].child_qual[iur_child_loop].parent_id_col," = ")
       FOR (iur_del_loop = 1 TO perm_col_cnt)
         IF ((dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].col_qual[iur_del_loop].pk_ind=1))
          IF (iur_del_ind >= 1)
           SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
 ...
 SUBROUTINE del_chg_log(sbr_table_name,sbr_log_type,sbr_target_id)
   ...
   IF ((update_cnt=drdm_chg->log[drdm_log_loop].updt_cnt))
    ...
    CALL parser(concat(" d.updt_cnt = d.updt_cnt + 1 ",
      " where (d.pk_where_value = drdm_chg->log[sbr_flex_pos].pk_where_value "),0)
    CALL parser(concat(" and d.target_env_id+0=dm2_ref_data_doc->env_target_id and ",
      " d.table_name=drdm_chg->log[sbr_flex_pos].table_name "),0)
    CALL parser(concat(" and d.pk_where=drdm_chg->log[sbr_flex_pos].pk_where and ",
      " d.log_type in ('REFCHG','NORDDS'"),0)
    CALL parser(")) or (d.log_id = drdm_chg->log[drdm_log_loop].log_id) go",1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET drdm_error_out_ind = 1
     SET dm_err->err_ind = 0
   ...
   SELECT
    ...
    DETAIL
     ...
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].data_default_ni = nullind(utc
      .data_default), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_qual_cnt].
 ...
 DECLARE nsi_imt_check = i2
 DECLARE orphan_mes = vc
 DECLARE ns_idcd_skip = i2
 DECLARE new_seq_ind = i2
 DECLARE ns_merge_audit(action=vc,text=vc) = null
 ...
 ENDFOR
 IF ((drdm_chg->log[drdm_log_loop].table_name="NORMALIZED_STRING_INDEX"))
  SET where_clause = concat(" ",value(drdm_chg->log[drdm_log_loop].pk_where))
  SET where_clause = trim(substring((findstring("=",where_clause,1,0)+ 1),size(where_clause),
    where_clause))
  SET drdm_srch_str = dm2_get_rdds_tname("NORMALIZED_STRING_INDEX")
  SET col_cnt = 1
  SET temp_col_cnt = 1
  ...
  ENDFOR
  SELECT INTO "NL:"
   var1 = nullind(dc.active_ind), var2 = nullind(dc.active_status_cd), var3 = nullind(dc
    .active_status_dt_tm),
   var4 = nullind(dc.active_status_prsnl_id), var5 = nullind(dc.beg_effective_dt_tm), var6 = nullind(
    dc.end_effective_dt_tm),
   var7 = nullind(dc.language_cd), var8 = nullind(dc.nomenclature_id), var9 = nullind(dc
    .normalized_string),
   var10 = nullind(dc.normalized_string_id), var11 = nullind(dc.normalized_string_nls), var12 =
   nullind(dc.updt_applctx),
   var13 = nullind(dc.updt_cnt), var14 = nullind(dc.updt_dt_tm), var15 = nullind(dc.updt_id),
   var16 = nullind(dc.updt_task)
   FROM (parser(drdm_srch_str) dc)
   WHERE normalized_string_id=cnvtreal(where_clause)
   DETAIL
    rs_0858->from_values.active_ind = dc.active_ind, rs_0858->from_values.active_status_cd = dc
    .active_status_cd, rs_0858->from_values.active_status_dt_tm = dc.active_status_dt_tm,
    rs_0858->from_values.active_status_prsnl_id = dc.active_status_prsnl_id, rs_0858->from_values.
    beg_effective_dt_tm = dc.beg_effective_dt_tm, rs_0858->from_values.end_effective_dt_tm = dc
    .end_effective_dt_tm,
    rs_0858->from_values.language_cd = dc.language_cd, rs_0858->from_values.nomenclature_id = dc
    .nomenclature_id, rs_0858->from_values.normalized_string = dc.normalized_string,
    rs_0858->from_values.normalized_string_id = dc.normalized_string_id, rs_0858->from_values.
    normalized_string_nls = dc.normalized_string_nls, rs_0858->from_values.updt_applctx = dc
    .updt_applctx,
    rs_0858->from_values.updt_cnt = dc.updt_cnt, rs_0858->from_values.updt_dt_tm = dc.updt_dt_tm,
    rs_0858->from_values.updt_id = dc.updt_id,
    rs_0858->from_values.updt_task = dc.updt_task, rs_0858->to_values.active_status_dt_tm = dc
    .active_status_dt_tm, rs_0858->to_values.beg_effective_dt_tm = dc.beg_effective_dt_tm,
    rs_0858->to_values.end_effective_dt_tm = dc.end_effective_dt_tm, rs_0858->to_values.
    normalized_string = dc.normalized_string, rs_0858->to_values.normalized_string_nls = dc
    .normalized_string_nls,
    rs_0858->to_values.updt_applctx = dc.updt_applctx, rs_0858->to_values.updt_cnt = dc.updt_cnt,
    rs_0858->to_values.updt_dt_tm = dc.updt_dt_tm,
    rs_0858->to_values.updt_task = dc.updt_task, col_num = locateval(index_var,1,perm_col_cnt,
     "ACTIVE_IND",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name),
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null = var1,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1, col_num = locateval(
     index_var,1,perm_col_cnt,"ACTIVE_STATUS_CD",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null
     = var2,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 0, col_num = locateval(
     index_var,1,perm_col_cnt,"ACTIVE_STATUS_DT_TM",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
     col_qual[index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].
    check_null = var3,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1, col_num = locateval(
     index_var,1,perm_col_cnt,"ACTIVE_STATUS_PRSNL_ID",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
     col_qual[index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].
    check_null = var4,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 0, col_num = locateval(
     index_var,1,perm_col_cnt,"BEG_EFFECTIVE_DT_TM",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
     col_qual[index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].
    check_null = var5,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1, col_num = locateval(
     index_var,1,perm_col_cnt,"END_EFFECTIVE_DT_TM",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
     col_qual[index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].
    check_null = var6,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1, col_num = locateval(
     index_var,1,perm_col_cnt,"LANGUAGE_CD",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null
     = var7,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 0, col_num = locateval(
     index_var,1,perm_col_cnt,"NOMENCLATURE_ID",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null
     = var8,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 0, col_num = locateval(
     index_var,1,perm_col_cnt,"NORMALIZED_STRING",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null
     = var9,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1, col_num = locateval(
     index_var,1,perm_col_cnt,"NORMALIZED_STRING_ID",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
     col_qual[index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].
    check_null = var10,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 0, col_num = locateval(
     index_var,1,perm_col_cnt,"NORMALIZED_STRING_NLS",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
     col_qual[index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].
    check_null = var11,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1, col_num = locateval(
     index_var,1,perm_col_cnt,"UPDT_APPLCTX",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null
     = var12,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1, col_num = locateval(
     index_var,1,perm_col_cnt,"UPDT_CNT",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[index_var]
     .column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null = var13,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1, col_num = locateval(
     index_var,1,perm_col_cnt,"UPDT_DT_TM",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null
     = var14,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1, col_num = locateval(
     index_var,1,perm_col_cnt,"UPDT_ID",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[index_var].
     column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null = var15,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 0, col_num = locateval(
     index_var,1,perm_col_cnt,"UPDT_TASK",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[index_var
     ].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null = var16,
    dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (curqual=0)
   ...
   UPDATE  FROM (parser(drdmm_srch_str) dc)
    SET dc.log_type = "NO SRC"
   ...
   SET dm_err->emsg = concat("A ",drdm_chg->log[drdm_log_loop].table_name," row ",trim(drdm_chg->log[
     drdm_log_loop].pk_where)," wasn't found in the source.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dm_err->emsg = concat("A ",drdm_chg->log[drdm_log_loop].table_name," row ",trim(drdm_chg->log[
  ...
  SET rs_0858->to_values.updt_id = 0
  SET current_merges = (current_merges+ 1)
  SET child_merge_audit->num[current_merges].action = "CONSTANT"
  SET child_merge_audit->num[current_merges].text = "NORMALIZED_STRING_INDEX  UPDT_ID"
  ...
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
  SET rs_0858->to_values.active_status_prsnl_id = 0
  SET current_merges = (current_merges+ 1)
  SET child_merge_audit->num[current_merges].action = "CONSTANT"
  SET child_merge_audit->num[current_merges].text = "NORMALIZED_STRING_INDEX  ACTIVE_STATUS_PRSNL_ID"
  SET rs_0858->log_id = drdm_chg->log[drdm_log_loop].log_id
  SET rs_0858->from_table_name = "NORMALIZED_STRING_INDEX"
  SET rs_0858->to_table_name = "NORMALIZED_STRING_INDEX"
  SET no_insert_update = 0
  SET found_rec = 0
  SET smt_ret_val = ""
  SET smt_ret_val = select_merge_translate(cnvtstring(rs_0858->from_values.active_status_cd),
   "CODE_VALUE")
  IF (smt_ret_val != "No Trans")
   SET rs_0858->to_values.active_status_cd = cnvtreal(smt_ret_val)
   SET found_rec = 1
  ENDIF
  IF (found_rec=1)
   SET col_num = locateval(index_var,1,perm_col_cnt,"ACTIVE_STATUS_CD",dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[index_var].column_name)
   SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
   SET current_merges = (current_merges+ 1)
   SET child_merge_audit->num[current_merges].action = "LOOKUPS"
   SET child_merge_audit->num[current_merges].text = "NORMALIZED_STRING_INDEX  ACTIVE_STATUS_CD"
  ELSE
   IF ((rs_0858->from_values.active_status_cd=0))
    SET rs_0858->to_values.active_status_cd = 0
    SET col_num = locateval(index_var,1,perm_col_cnt,"ACTIVE_STATUS_CD",dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[index_var].column_name)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
   ENDIF
  ...
  SET found_rec = 0
  SET smt_ret_val = ""
  SET smt_ret_val = select_merge_translate(cnvtstring(rs_0858->from_values.language_cd),"CODE_VALUE")
  IF (smt_ret_val != "No Trans")
   SET rs_0858->to_values.language_cd = cnvtreal(smt_ret_val)
   SET found_rec = 1
  ENDIF
  IF (found_rec=1)
   SET col_num = locateval(index_var,1,perm_col_cnt,"LANGUAGE_CD",dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[index_var].column_name)
   SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
   SET current_merges = (current_merges+ 1)
   SET child_merge_audit->num[current_merges].action = "LOOKUPS"
   SET child_merge_audit->num[current_merges].text = "NORMALIZED_STRING_INDEX  LANGUAGE_CD"
  ELSE
   IF ((rs_0858->from_values.language_cd=0))
    SET rs_0858->to_values.language_cd = 0
    SET col_num = locateval(index_var,1,perm_col_cnt,"LANGUAGE_CD",dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[index_var].column_name)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
   ENDIF
  ...
  SET found_rec = 0
  SET smt_ret_val = ""
  SET smt_ret_val = select_merge_translate(cnvtstring(rs_0858->from_values.nomenclature_id),
   "NOMENCLATURE")
  IF (smt_ret_val != "No Trans")
   SET rs_0858->to_values.nomenclature_id = cnvtreal(smt_ret_val)
   SET found_rec = 1
  ENDIF
  IF (found_rec=1)
   SET col_num = locateval(index_var,1,perm_col_cnt,"NOMENCLATURE_ID",dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[index_var].column_name)
   SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
   SET current_merges = (current_merges+ 1)
   SET child_merge_audit->num[current_merges].action = "LOOKUPS"
   SET child_merge_audit->num[current_merges].text = "NORMALIZED_STRING_INDEX  NOMENCLATURE_ID"
  ELSE
   IF ((rs_0858->from_values.nomenclature_id=0))
    SET rs_0858->to_values.nomenclature_id = 0
    SET col_num = locateval(index_var,1,perm_col_cnt,"NOMENCLATURE_ID",dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[index_var].column_name)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
   ENDIF
  ...
  SET found_rec = 0
  SET smt_ret_val = ""
  SET smt_ret_val = select_merge_translate(cnvtstring(rs_0858->from_values.normalized_string_id),
   "NORMALIZED_STRING_INDEX")
  IF (smt_ret_val != "No Trans")
   SET rs_0858->to_values.normalized_string_id = cnvtreal(smt_ret_val)
   SET found_rec = 1
  ENDIF
  IF (found_rec=1)
   SET col_num = locateval(index_var,1,perm_col_cnt,"NORMALIZED_STRING_ID",dm2_ref_data_doc->
    tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
   SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
   SET current_merges = (current_merges+ 1)
   SET child_merge_audit->num[current_merges].action = "LOOKUPS"
   SET child_merge_audit->num[current_merges].text = "NORMALIZED_STRING_INDEX  NORMALIZED_STRING_ID"
  ENDIF
  IF (drdm_error_out_ind=1)
   GO TO exit_0858
  ENDIF
  SET col_num = locateval(index_var,1,perm_col_cnt,"NORMALIZED_STRING_ID",dm2_ref_data_doc->tbl_qual[
   temp_tbl_cnt].col_qual[index_var].column_name)
  CALL echo("")
  CALL echo("")
  ...
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated=0))
   DECLARE src_vocab = i4
   DECLARE princ_type = i4
   DECLARE val = f8
   SET src_vocab = locateval(index_var,1,perm_col_cnt,"NOMENCLATURE_ID",dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[index_var].column_name)
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[src_vocab].translated=1))
    SET val = 0
    CALL echo("")
    CALL echo("")
    CALL echo("*******************QUERY TARGET***************************")
    CALL echo("")
    CALL echo("")
    SET norm_s = locateval(index_var,1,perm_col_cnt,"NORMALIZED_STRING",dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[index_var].column_name)
    SELECT INTO "NL:"
     FROM normalized_string_index n
     WHERE (n.nomenclature_id=rs_0858->to_values.nomenclature_id)
      AND n.normalized_string=evaluate(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[norm_s].
      check_null,1,null,0,evaluate(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[norm_s].
       check_space,1," ",0,rs_0858->to_values.normalized_string))
     DETAIL
      val = n.normalized_string_id
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (curqual=0)
     SELECT INTO "NL:"
      y = seq(nomenclature_seq,nextval)
      FROM dual
      DETAIL
       drdm_from_con = cnvtstring(y)
      WITH nocounter
     ;end select
     SET new_seq_ind = 1
     SET rs_0858->to_values.normalized_string_id = cnvtreal(drdm_from_con)
     SET src_vocab = locateval(index_var,1,perm_col_cnt,"NORMALIZED_STRING_ID",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[src_vocab].translated = 1
     SET current_merges = (current_merges+ 1)
     SET child_merge_audit->num[current_merges].action = "NEWSEQ"
     SET child_merge_audit->num[current_merges].text = concat("NORMALIZED_STRING_INDEX  ",
      "NORMALIZED_STRING_ID")
    ELSE
     SET rs_0858->to_values.normalized_string_id = val
     SET src_vocab = locateval(index_var,1,perm_col_cnt,"NORMALIZED_STRING_ID",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[src_vocab].translated = 1
     SET current_merges = (current_merges+ 1)
     ...
     SET child_merge_audit->num[current_merges].text = concat("NORMALIZED_STRING_INDEX   ",
      "NORMALIZED_STRING_ID")
    ENDIF
    CALL rdds_del_except("NORMALIZED_STRING_INDEX",cnvtreal(rs_0858->from_values.normalized_string_id
      ))
    SET nsi_imt_check = insert_merge_translate(cnvtreal(rs_0858->from_values.normalized_string_id),
     cnvtreal(rs_0858->to_values.normalized_string_id),"NORMALIZED_STRING_INDEX")
    IF (nsi_imt_check=1)
     IF (drdm_error_out_ind=1)
      ROLLBACK
      GO TO exit_0858
   ...
   ELSE
    CALL echo("")
    CALL echo("")
    CALL echo("THE UI IS NOT TRANSLATED")
    CALL echo("")
    CALL echo("")
    ROLLBACK
    CALL ns_merge_audit("FAILREASON","The UI is not translated.")
    IF (drdm_error_out_ind=1)
     ROLLBACK
     GO TO exit_0858
    ELSE
    ...
    IF (value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].idcd_ind)=1)
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].translated=0))
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].root_entity_name != ""
       ...
       root_entity_attr
       CALL parser(concat("set rpt_from = RS_858->from_values.",dm2_ref_data_doc->tbl_qual[
      ...
      IF (rpt_table != ""
       ...
       ELSE
        IF (except_log_type="ORPHAN")
         IF ((((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].unique_ident_ind=
         1)) OR ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].pk_ind=1))) )
          SET orphan_mes = concat("ORPHAN - ",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
           final_check_loop].column_name)
          ROLLBACK
          CALL ns_merge_audit("FAILREASON",orphan_mes)
          IF (drdm_error_out_ind=1)
           ROLLBACK
           GO TO exit_0858
          ELSE
         ...
         ELSE
          SET ns_idcd_skip = 1
          CALL parser(concat("set RS_858->to_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
      ...
      IF (ns_idcd_skip=0)
       SET idcd_check = 1
       SET dm_err->eproc = concat("This log_id ","wasn't translated becase of the ",dm2_ref_data_doc
       ...
       CALL echo("")
       ROLLBACK
       CALL ns_merge_audit("FAILREASON",dm_err->eproc)
       IF (drdm_error_out_ind=1)
        ROLLBACK
  ...
  IF (no_insert_update=1)
   SET insert_update_reason = concat(insert_update_reason," ",cnvtstring(drdm_chg->log[drdm_log_loop]
     .log_id))
   ROLLBACK
   CALL ns_merge_audit("FAILREASON",insert_update_reason)
   IF (drdm_error_out_ind=1)
    ROLLBACK
    GO TO exit_0858
   ELSE
  ...
  IF (idcd_check=0)
   ...
   CALL echo("")
   SELECT INTO "NL:"
    FROM normalized_string_index
    WHERE (normalized_string_id=rs_0858->to_values.normalized_string_id)
   ;end select
   SET norm_s = locateval(index_var,1,perm_col_cnt,"NORMALIZED_STRING",dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].col_qual[index_var].column_name)
   SET norm_nls = locateval(index_var,1,perm_col_cnt,"NORMALIZED_STRING_NLS",dm2_ref_data_doc->
    tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
   IF (curqual > 0)
    IF (new_seq_ind=1
     AND drdm_override_ind=0)
     ...
     SET nodelete_ind = 1
    ELSE
     UPDATE  FROM normalized_string_index dc
      SET dc.active_ind = evaluate(drdm_active_ind_merge,1,rs_0858->to_values.active_ind,0,evaluate(
         rs_0858->from_values.active_ind,0,0,1,dc.active_ind)), dc.active_status_cd = rs_0858->
       to_values.active_status_cd, dc.active_status_dt_tm = cnvtdatetime(rs_0858->to_values.
        active_status_dt_tm),
       dc.active_status_prsnl_id = rs_0858->to_values.active_status_prsnl_id, dc.beg_effective_dt_tm
        = cnvtdatetime(rs_0858->to_values.beg_effective_dt_tm), dc.end_effective_dt_tm = cnvtdatetime
       (rs_0858->to_values.end_effective_dt_tm),
       dc.language_cd = rs_0858->to_values.language_cd, dc.nomenclature_id = rs_0858->to_values.
       nomenclature_id, dc.normalized_string = evaluate(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
        col_qual[norm_s].check_null,1,null,0,evaluate(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
         col_qual[norm_s].check_space,1," ",0,rs_0858->to_values.normalized_string)),
       dc.normalized_string_nls = evaluate(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[norm_nls
        ].check_null,1,null,0,evaluate(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[norm_nls].
         check_space,1," ",0,rs_0858->to_values.normalized_string_nls)), dc.updt_applctx = rs_0858->
       to_values.updt_applctx, dc.updt_cnt = (dc.updt_cnt+ 1),
       dc.updt_dt_tm = cnvtdatetime(curdate,curtime3), dc.updt_id = rs_0858->to_values.updt_id, dc
       .updt_task = 4310001
      WHERE (dc.normalized_string_id=rs_0858->to_values.normalized_string_id)
      WITH nocounter
     ;end update
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     SET current_merges = (current_merges+ 1)
     SET child_merge_audit->num[current_merges].action = "UPDATE"
     SET child_merge_audit->num[current_merges].text = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
     table_name
    ENDIF
   ELSE
    INSERT  FROM normalized_string_index cd
     SET cd.active_ind = rs_0858->to_values.active_ind, cd.active_status_cd = rs_0858->to_values.
      active_status_cd, cd.active_status_dt_tm = cnvtdatetime(rs_0858->to_values.active_status_dt_tm),
      cd.active_status_prsnl_id = rs_0858->to_values.active_status_prsnl_id, cd.beg_effective_dt_tm
       = cnvtdatetime(rs_0858->to_values.beg_effective_dt_tm), cd.end_effective_dt_tm = cnvtdatetime(
       rs_0858->to_values.end_effective_dt_tm),
      cd.language_cd = rs_0858->to_values.language_cd, cd.nomenclature_id = rs_0858->to_values.
      nomenclature_id, cd.normalized_string = evaluate(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
       col_qual[norm_s].check_null,1,null,0,evaluate(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
        col_qual[norm_s].check_space,1," ",0,rs_0858->to_values.normalized_string)),
      cd.normalized_string_nls = evaluate(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[norm_nls]
       .check_null,1,null,0,evaluate(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[norm_nls].
        check_space,1," ",0,rs_0858->to_values.normalized_string_nls)), cd.updt_applctx = rs_0858->
      to_values.updt_applctx, cd.updt_cnt = 0,
      cd.updt_dt_tm = cnvtdatetime(curdate,curtime3), cd.updt_id = rs_0858->to_values.updt_id, cd
      .updt_task = 4310001,
      cd.normalized_string_id = rs_0858->to_values.normalized_string_id
    ;end insert
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET nodelete_ind = 1
   ...
   IF (nodelete_ind=0)
    ...
    IF ((update_cnt=drdm_chg->log[drdm_log_loop].updt_cnt))
     UPDATE  FROM (parser(drdmm_srch_str) dc)
      SET dc.log_type = "MERGED", dc.updt_dt_tm = cnvtdatetime(curdate,curtime3), dc.updt_cnt = (dc
       .updt_cnt+ 1)
      WHERE (dc.log_id=rs_0858->log_id)
      WITH nocounter
     ;end update
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     FOR (merge_loop = 1 TO current_merges)
      CALL ns_merge_audit(child_merge_audit->num[current_merges].action,child_merge_audit->num[
       current_merges].text)
      IF (drdm_error_out_ind=1)
       ROLLBACK
  ...
  ELSE
   IF (orphan_ind=1)
    ROLLBACK
    SET update_cnt = 0
    CALL orphan_child_tab("NORMALIZED_STRING_INDEX")
    IF (nodelete_ind=0)
     SET drdmm_srch_str = dm2_get_rdds_tname("DM_CHG_LOG")
     SELECT INTO "nl:"
      FROM (parser(drdmm_srch_str) dc)
     ...
     IF ((update_cnt=drdm_chg->log[drdm_log_loop].updt_cnt))
      UPDATE  FROM (parser(drdmm_srch_str) dc)
       SET dc.log_type = "ORPHAN", dc.updt_dt_tm = cnvtdatetime(curdate,curtime3), dc.updt_cnt = (dc
        .updt_cnt+ 1)
       WHERE (dc.log_id=rs_0858->log_id)
       WITH nocounter
      ;end update
  ...
  ENDIF
 ENDIF
 SUBROUTINE ns_merge_audit(action,text)
   DECLARE ma_next_seq = f8
   DECLARE ma_del_ind = i2
   IF (drdm_log_level=1
    AND  NOT (action IN ("INSERT", "UPDATE", "FAILREASON")))
   ...
   ELSE
    ...
    ELSE
     UPDATE  FROM dm_chg_log_audit dm
      SET dm.audit_dt_tm = cnvtdatetime(curdate,curtime3), dm.log_id = rs_0858->log_id, dm.action =
       action,
       dm.text = text, dm.table_name = "NORMALIZED_STRING_INDEX", dm.updt_dt_tm = cnvtdatetime(
        curdate,curtime3)
      WHERE dm.dm_chg_log_audit_id=ma_next_seq
      WITH nocounter
     ;end update
...
#exit_0858
 ...
 FREE SET nodelete_ind
 FREE SET nodelete_msg
 FREE SET idcd_msg
 FREE SET zero_rec
 FREE SET last_table
 FREE SET except_tab
