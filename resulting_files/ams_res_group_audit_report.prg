CREATE PROGRAM ams_res_group_audit_report
 PROMPT
 ...
 CALL inc_prompt(1)
 SET t_record->prompt_qual[t_record->prompt_qual_cnt].prompt_string = "Child Res Groups:"
 CALL inc_prompt(1)
 SET t_record->prompt_qual[t_record->prompt_qual_cnt].prompt_string = "Parent Res Groups:"
 CALL inc_prompt(1)
 SET t_record->prompt_qual[t_record->prompt_qual_cnt].prompt_string = "Res Types:"
 CALL inc_prompt(1)
 ...
 SET entity = evaluate(t_record->entity_id,0.0,"a.res_group_id > 0",
  "a.res_group_id = t_record->entity_id")
 SELECT INTO "nl:"
  a.res_group_id
  FROM sch_res_group a,
  ...
  DETAIL
   ...
   ENDIF
   t_record->qual[t_record->qual_cnt].res_group_id = a.res_group_id, t_record->qual[t_record->
   ...
   IF ((t_record->qual[t_record->qual_cnt].updt_id > 0))
    t_record->qual[t_record->qual_cnt].updt_name = concat("Person ","  (PERSON_ID:",trim(format(
       t_record->qual[t_record->qual_cnt].updt_id,";l;i"))," not found")
   ELSE
   ...
   IF ((t_record->qual[t_record->qual_cnt].active_prsnl_id > 0))
    t_record->qual[t_record->qual_cnt].active_prsnl_name = concat("Person ","  (PERSON_ID:",trim(
      format(t_record->qual[t_record->qual_cnt].active_prsnl_id,";l;i"))," not found")
   ELSE
    t_record->qual[t_record->qual_cnt].active_prsnl_name = "NOT ASSOCIATED"
   ENDIF
   t_record->qual[t_record->qual_cnt].child_qual_cnt = 0, t_record->qual[t_record->qual_cnt].
   parent_qual_cnt = 0, t_record->qual[t_record->qual_cnt].max_active_size = 0,
   t_record->qual[t_record->qual_cnt].date_set_qual_cnt = 0
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (a
   WHERE (a.res_group_id=t_record->qual[d.seq].res_group_id)
    AND a.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
   JOIN (b
   WHERE b.res_group_id=a.child_res_group_id
    AND b.res_group_id > 0
    AND b.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
  ORDER BY d.seq, a.seq_nbr
  HEAD d.seq
   t_record->qual[d.seq].child_qual_cnt = 0
  DETAIL
   t_record->qual[d.seq].child_qual_cnt = (t_record->qual[d.seq].child_qual_cnt+ 1)
   IF (mod(t_record->qual[d.seq].child_qual_cnt,10)=1)
    stat = alterlist(t_record->qual[d.seq].child_qual,(t_record->qual[d.seq].child_qual_cnt+ 9))
   ENDIF
   t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].res_group_id = b
   .res_group_id, t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].mnemonic = b
   .mnemonic, t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].seq_nbr = (a
   .seq_nbr+ 1),
   t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].active_status_cd = a
   .active_status_cd
   IF ((t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].active_status_cd > 0))
    t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].active_status_desc =
    uar_get_code_display(a.active_status_cd)
    IF ((t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].active_status_desc
     <= " "))
     t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].active_status_desc =
     concat("Code_value ",trim(format(a.active_status_cd,";l;i"))," not found")
    ENDIF
   ELSE
    t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].active_status_desc =
    "NOT ASSOCIATED"
   ENDIF
   t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].active_status_size = size(
    trim(t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].active_status_desc))
   IF ((t_record->qual[d.seq].child_qual[t_record->qual[d.seq].child_qual_cnt].active_status_size >
   t_record->qual[d.seq].max_active_size))
    t_record->qual[d.seq].max_active_size = t_record->qual[d.seq].child_qual[t_record->qual[d.seq].
    child_qual_cnt].active_status_size
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (a
   WHERE (a.child_res_group_id=t_record->qual[d.seq].res_group_id)
    AND a.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
   JOIN (b
  ...
  HEAD d.seq
   t_record->qual[d.seq].parent_qual_cnt = 0
  DETAIL
   t_record->qual[d.seq].parent_qual_cnt = (t_record->qual[d.seq].parent_qual_cnt+ 1)
   IF (mod(t_record->qual[d.seq].parent_qual_cnt,10)=1)
    stat = alterlist(t_record->qual[d.seq].parent_qual,(t_record->qual[d.seq].parent_qual_cnt+ 9))
   ENDIF
   t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].res_group_id = a
   .res_group_id, t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].mnemonic
    = b.mnemonic, t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].seq_nbr =
   (a.seq_nbr+ 1),
   t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].active_status_cd = a
   .active_status_cd
   IF ((t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].active_status_cd > 0
   ))
    t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].active_status_desc =
    uar_get_code_display(a.active_status_cd)
    IF ((t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].active_status_desc
     <= " "))
     t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].active_status_desc =
     concat("Code_value ",trim(format(a.active_status_cd,";l;i"))," not found")
    ENDIF
   ELSE
    ...
    "NOT ASSOCIATED"
   ENDIF
   t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].active_status_size = size
   (trim(t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].active_status_desc)
    )
   IF ((t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].parent_qual_cnt].active_status_size
    > t_record->qual[d.seq].max_active_size))
    t_record->qual[d.seq].max_active_size = t_record->qual[d.seq].parent_qual[t_record->qual[d.seq].
    parent_qual_cnt].active_status_size
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  ORDER BY d.seq
  HEAD d.seq
   t_record->qual[d.seq].res_type_qual_cnt = 0
  DETAIL
   t_record->qual[d.seq].res_type_qual_cnt = (t_record->qual[d.seq].res_type_qual_cnt+ 1)
   IF (mod(t_record->qual[d.seq].res_type_qual_cnt,10)=1)
    stat = alterlist(t_record->qual[d.seq].res_type_qual,(t_record->qual[d.seq].res_type_qual_cnt+ 9)
     )
   ENDIF
   t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].res_type_qual_cnt].res_group_cd = a
   .res_group_cd, t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].res_type_qual_cnt].
   mnemonic = uar_get_code_display(a.res_group_cd), t_record->qual[d.seq].res_type_qual[t_record->
   qual[d.seq].res_type_qual_cnt].active_status_cd = a.active_status_cd
   IF ((t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].res_type_qual_cnt].active_status_cd
    > 0))
    t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].res_type_qual_cnt].active_status_desc
     = uar_get_code_display(a.active_status_cd)
    IF ((t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].res_type_qual_cnt].
    active_status_desc <= " "))
     t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].res_type_qual_cnt].active_status_desc
      = concat("Code_value ",trim(format(t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].
        res_type_qual_cnt].active_status_cd,";l;i"))," not found")
    ENDIF
   ELSE
    t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].res_type_qual_cnt].active_status_desc
     = "NOT ASSOCIATED"
   ENDIF
   t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].res_type_qual_cnt].active_status_size =
   size(trim(t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].res_type_qual_cnt].
     active_status_desc))
   IF ((t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq].res_type_qual_cnt].
   active_status_size > t_record->qual[d.seq].max_active_size))
    t_record->qual[d.seq].max_active_size = t_record->qual[d.seq].res_type_qual[t_record->qual[d.seq]
    .res_type_qual_cnt].active_status_size
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
 ...
 FOR (i = 1 TO t_record->qual_cnt)
   CALL echo(build("   t_record->qual[",i,"]->res_group_id = ",t_record->qual[i].res_group_id))
   ...
   CALL echo(build("   t_record->qual[",i,"]->owner_person_id = ",t_record->qual[i].owner_person_id))
   CALL echo(build("   t_record->qual[",i,"]->owner_person_name = ",t_record->qual[i].
     owner_person_name))
   ...
   CALL echo(build("   t_record->qual[",i,"]->child_qual_cnt = ",t_record->qual[i].child_qual_cnt))
   FOR (j = 1 TO t_record->qual[i].child_qual_cnt)
     CALL echo(build("   t_record->qual[",i,",",j,"]->res_group_id = ",
       t_record->qual[i].child_qual[j].res_group_id))
     CALL echo(build("   t_record->qual[",i,",",j,"]->mnemonic = ",
       t_record->qual[i].child_qual[j].mnemonic))
     CALL echo(build("   t_record->qual[",i,",",j,"]->seq_nbr = ",
       t_record->qual[i].child_qual[j].seq_nbr))
     CALL echo(build("   t_record->qual[",i,",",j,"]->active_status_cd = ",
       t_record->qual[i].child_qual[j].active_status_cd))
     CALL echo(build("   t_record->qual[",i,",",j,"]->active_status_desc = ",
       t_record->qual[i].child_qual[j].active_status_desc))
   ENDFOR
   CALL echo(build("   t_record->qual[",i,"]->parent_qual_cnt = ",t_record->qual[i].parent_qual_cnt))
   FOR (j = 1 TO t_record->qual[i].parent_qual_cnt)
     CALL echo(build("   t_record->qual[",i,",",j,"]->res_group_id = ",
       t_record->qual[i].parent_qual[j].res_group_id))
     CALL echo(build("   t_record->qual[",i,",",j,"]->mnemonic = ",
       t_record->qual[i].parent_qual[j].mnemonic))
     CALL echo(build("   t_record->qual[",i,",",j,"]->seq_nbr = ",
       t_record->qual[i].parent_qual[j].seq_nbr))
     CALL echo(build("   t_record->qual[",i,",",j,"]->active_status_cd = ",
       t_record->qual[i].parent_qual[j].active_status_cd))
     CALL echo(build("   t_record->qual[",i,",",j,"]->active_status_desc = ",
       t_record->qual[i].parent_qual[j].active_status_desc))
   ENDFOR
   CALL echo(build("   t_record->qual[",i,"]->res_type_qual_cnt = ",t_record->qual[i].
     res_type_qual_cnt))
   FOR (j = 1 TO t_record->qual[i].res_type_qual_cnt)
     CALL echo(build("   t_record->qual[",i,",",j,"]->res_group_cd = ",
       t_record->qual[i].res_type_qual[j].res_group_cd))
     CALL echo(build("   t_record->qual[",i,",",j,"]->mnemonic = ",
       t_record->qual[i].res_type_qual[j].mnemonic))
     CALL echo(build("   t_record->qual[",i,",",j,"]->active_status_cd = ",
       t_record->qual[i].res_type_qual[j].active_status_cd))
     CALL echo(build("   t_record->qual[",i,",",j,"]->active_status_desc = ",
       t_record->qual[i].res_type_qual[j].active_status_desc))
   ENDFOR
   CALL echo(" ")
 ENDFOR
 FOR (i = 1 TO t_record->qual_cnt)
   ...
   SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
    substring(1,t_record->max_prompt_size,t_record->prompt_qual[6].prompt_string),evaluate(t_record->
     qual[i].child_qual_cnt,0,"NOT ASSOCIATED",concat("Mnemonic                           Status",
      substring(1,maxval(0,(t_record->qual[i].max_active_size - 6)),t_record->blank_line),"     Seq")
     ))
   FOR (i_child = 1 TO t_record->qual[i].child_qual_cnt)
     IF (i_child=1)
      CALL inc_data(1)
      SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat
     ...
     CALL inc_data(1)
     SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
      substring(1,t_record->max_prompt_size,t_record->blank_line),t_record->qual[i].child_qual[
      i_child].mnemonic,"     ",substring(1,t_record->qual[i].max_active_size,t_record->qual[i].
       child_qual[i_child].active_status_desc),"     ",
      format(t_record->qual[i].child_qual[i_child].seq_nbr,"###;r;i"))
   ENDFOR
   IF ((t_record->qual[i].child_qual_cnt > 0)
    AND (t_record->qual[i].parent_qual_cnt > 0))
    CALL inc_data(1)
   ...
   SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
    substring(1,t_record->max_prompt_size,t_record->prompt_qual[7].prompt_string),evaluate(t_record->
     qual[i].parent_qual_cnt,0,"NOT ASSOCIATED",concat("Mnemonic                           Status",
      substring(1,maxval(0,(t_record->qual[i].max_active_size - 6)),t_record->blank_line),"     Seq")
     ))
   FOR (i_parent = 1 TO t_record->qual[i].parent_qual_cnt)
     IF (i_parent=1)
      CALL inc_data(1)
      SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat
      (substring(1,t_record->max_prompt_size,t_record->blank_line),
     ...
     CALL inc_data(1)
     SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
      substring(1,t_record->max_prompt_size,t_record->blank_line),t_record->qual[i].parent_qual[
      i_parent].mnemonic,"     ",substring(1,t_record->qual[i].max_active_size,t_record->qual[i].
       parent_qual[i_parent].active_status_desc),"     ",
      format(t_record->qual[i].parent_qual[i_parent].seq_nbr,"###;r;i"))
   ENDFOR
   IF ((t_record->qual[i].parent_qual_cnt > 0)
    AND (t_record->qual[i].res_type_qual_cnt > 0))
    CALL inc_data(1)
    SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = " "
   ENDIF
   CALL inc_data(1)
   SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
    substring(1,t_record->max_prompt_size,t_record->prompt_qual[8].prompt_string),evaluate(t_record->
     qual[i].res_type_qual_cnt,0,"NOT ASSOCIATED",concat("Mnemonic                           Status",
      substring(1,maxval(0,(t_record->qual[i].max_active_size - 6)),t_record->blank_line))))
   FOR (i_res_type = 1 TO t_record->qual[i].res_type_qual_cnt)
     IF (i_res_type=1)
      CALL inc_data(1)
      SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat
      (substring(1,t_record->max_prompt_size,t_record->blank_line),
     ...
     CALL inc_data(1)
     SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
      substring(1,t_record->max_prompt_size,t_record->blank_line),t_record->qual[i].res_type_qual[
      i_res_type].mnemonic,"     ",substring(1,t_record->qual[i].max_active_size,t_record->qual[i].
       res_type_qual[i_res_type].active_status_desc))
   ENDFOR
   IF ((t_record->qual[i].res_type_qual_cnt > 0)
   ...
   SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
    substring(1,t_record->max_prompt_size,t_record->prompt_qual[9].prompt_string),evaluate(t_record->
     qual[i].date_set_qual_cnt,0,"NOT ASSOCIATED","Holiday Set"))
   FOR (ml_date_set_cnt = 1 TO t_record->qual[i].date_set_qual_cnt)
     ...
     CALL inc_data(1)
     SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
      substring(1,t_record->max_prompt_size,t_record->blank_line),substring(1,50,t_record->qual[i].
       date_set_qual[ml_date_set_cnt].date_set_mnemonic))
   ENDFOR
 ENDFOR
