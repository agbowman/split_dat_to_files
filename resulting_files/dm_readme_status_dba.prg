CREATE PROGRAM dm_readme_status:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE rs_table_exists(rte_in_table=vc,rte_out_exists_ind=i2(ref)) = i2
 DECLARE rs_table_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE rs_ecode = i4 WITH protect, noconstant(0)
 DECLARE rs_emsg = vc WITH protect, noconstant("")
 FREE SET rs_data
 RECORD rs_data(
   1 name = vc
   1 status = vc
 ...
 EXECUTE FROM 2000_update TO 2999_update_exit
 GO TO 9999_exit_program
 SUBROUTINE rs_table_exists(rte_in_table,rte_out_exists_ind)
   SET rte_out_exists_ind = 0
   SET rte_ccl_exists_ind = 0
   SET dm_err->eproc = concat("Verify if table ",rte_in_table," exists")
   ...
   SELECT INTO "nl:"
    a.table_name
    FROM user_tables u
    WHERE u.table_name=cnvtupper(trim(rte_in_table,3))
    DETAIL
     rte_out_exists_ind = 1
    WITH nocounter
   ;end select
   SET dm_err->ecode = error(dm_err->emsg,1)
   IF ((dm_err->ecode != 0))
    GO TO 9999_exit_program
   ENDIF
   SET dm_err->eproc = concat("Verify if CCL definition for table ",rte_in_table," exists")
   IF ((dm_err->debug_flag > 5))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF (dm2_table_and_ccldef_exists(cnvtupper(rte_in_table),rte_ccl_exists_ind)=0)
    GO TO 9999_exit_program
   ENDIF
   IF (rte_ccl_exists_ind > 0
    AND rte_out_exists_ind=1)
    SET rte_out_exists_ind = 1
    RETURN(1)
   ELSE
    IF (rte_out_exists_ind=1)
     SET rte_out_exists_ind = 0
     RETURN(0)
    ELSE
     SET rte_out_exists_ind = 0
 ...
 END ;Subroutine
#1000_initialize
 IF ( NOT ((dm2_rr_misc->dm2_toolset_usage IN ("Y", "N"))))
  SET dm_err->eproc = "Determining if DM_INFO exists."
  IF ((dm_err->debug_flag > 5))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SET rs_table_exists_ind = 0
  IF (dm2_table_and_ccldef_exists("DM_INFO",rs_table_exists_ind)=0)
   GO TO 9999_exit_program
  ENDIF
  IF (rs_table_exists_ind=0)
   SET dm2_rr_misc->dm2_toolset_usage = "Y"
  ELSE
   SET dm_err->eproc = "Check for DM_README_TOOLSET row."
   IF ((dm_err->debug_flag > 5))
 ...
 IF ((dm2_rr_misc->dm2_toolset_usage="Y"))
  ...
  IF ((dm2_rr_misc->env_id=0.0))
   SELECT INTO "nl:"
    ...
    PLAN (d
     ...
     WHERE d.info_number=de.environment_id)
    DETAIL
     dm2_rr_misc->env_id = de.environment_id
    WITH nocounter
   ;end select
   SET dm_err->ecode = error(dm_err->emsg,1)
   IF ((dm_err->ecode > 0))
  ...
  IF ((readme_data->status="S")
   AND validate(dm_readme_finished_ind,"X")="T")
   SET rs_table_exists_ind = 0
   IF (rs_table_exists("DM_XNTR_README_DATA",rs_table_exists_ind)=0)
    SET readme_data->status = "F"
    SET readme_data->message = substring(1,255,concat(
      "Missing ccl definition for table DM_XNTR_README_DATA:",dm_err->emsg))
   ENDIF
   IF (rs_table_exists_ind=1)
    SET dm_err->eproc = "Copy down executed readme information"
    UPDATE  FROM dm_xntr_readme_data d
     SET d.execute_dt_tm = cnvtdatetime(rs_data->start_dt_tm), d.updt_dt_tm = cnvtdatetime(curdate,
       curtime3), d.updt_cnt = (d.updt_cnt+ 1),
      d.updt_applctx = reqinfo->updt_applctx, d.updt_task = reqinfo->updt_task, d.updt_id = reqinfo->
      updt_id
     WHERE (d.readme_id=readme_data->readme_id)
      AND (d.readme_instance=readme_data->instance)
     WITH nocounter
    ;end update
    IF ((dm_err->ecode > 0))
     ROLLBACK
     SET readme_data->status = "F"
     SET readme_data->message = substring(1,255,concat(
       "Error occured copying XnTR readme data to local:",dm_err->emsg))
    ENDIF
    IF (curqual=0
     AND (dm_err->ecode=0))
     INSERT  FROM dm_xntr_readme_data d
      (d.dm_xntr_readme_data_id, d.readme_id, d.readme_instance,
      d.script, d.parent_readme_id, d.execute_dt_tm,
      d.updt_dt_tm, d.updt_task, d.updt_applctx,
      d.updt_id)(SELECT
       seq(dm_clinical_seq,nextval), r.readme_id, r.instance,
       r.script, r.parent_readme_id, cnvtdatetime(rs_data->start_dt_tm),
       cnvtdatetime(curdate,curtime3), reqinfo->updt_task, reqinfo->updt_applctx,
       reqinfo->updt_id
       FROM dm_readme r
       WHERE r.active_ind=1
        AND (r.readme_id=readme_data->readme_id)
        AND (r.instance=readme_data->instance)
        AND  EXISTS (
       (SELECT
        "x"
        FROM dm_xntr_readme_table x
        WHERE x.readme_id=r.readme_id
         AND x.readme_instance=r.instance))
        AND  NOT ( EXISTS (
       (SELECT
        "x"
        FROM dm_xntr_readme_data dx
        WHERE dx.readme_id=r.readme_id
         AND dx.readme_instance=r.instance))))
      WITH nocounter
     ;end insert
     SET dm_err->ecode = error(dm_err->emsg,1)
     IF ((dm_err->ecode > 0))
  ...
  CASE (readme_data->status)
   ...
   OF "R":
    IF (validate(dm_readme_finished_ind,"X")="T")
     SET rs_data->status = dm2_rr_reset
    ELSE
     SET rs_data->status = dm2_rr_running
    ENDIF
   ELSE
  ...
  IF ((rs_data->status=dm2_rr_done))
   ...
   UPDATE  FROM dm_ocd_log l
    SET l.status = rs_data->status, l.start_dt_tm = evaluate(l.ocd,readme_data->ocd,cnvtdatetimeutc(
     ...
     l.message = substring(1,255,readme_data->message), l.updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(
       curdate,curtime3))
    WHERE (l.environment_id=dm2_rr_misc->env_id)
     AND l.project_type=dm2_rr_readme
   ...
   IF ((dm_err->ecode > 0))
    ...
    GO TO 9999_exit_program
   ENDIF
  ELSEIF ((rs_data->status=dm2_rr_reset))
   SET dm_err->eproc = "Update DM_OCD_LOG so this readme so it can be picked up again"
   IF ((dm_err->debug_flag > 5))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   UPDATE  FROM dm_ocd_log l
    SET l.status = rs_data->status, l.start_dt_tm = null, l.end_dt_tm = null,
     l.updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)), l.message =
     "Reset so dm2_readme_runner can pick it up again"
    WHERE (l.ocd=readme_data->ocd)
     AND (l.environment_id=dm2_rr_misc->env_id)
     AND l.project_type=dm2_rr_readme
     AND l.project_name=cnvtstring(readme_data->readme_id)
  ...
  ELSE
   ...
   ENDIF
  ENDIF
  IF ((((rs_data->status=dm2_rr_done)) OR ((rs_data->status=dm2_rr_failed))) )
   SET dm_err->eproc = "Insert execution details into DM_OCD_LOG_HIST"
   IF ((dm_err->debug_flag > 5))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   INSERT  FROM dm_ocd_log_hist lh
    (lh.environment_id, lh.project_type, lh.project_name,
    lh.project_instance, lh.ocd, lh.batch_dt_tm,
    lh.status, lh.start_dt_tm, lh.end_dt_tm,
    lh.driver_count, lh.estimated_time, lh.message,
    lh.active_ind, lh.updt_dt_tm)(SELECT
     l.environment_id, l.project_type, l.project_name,
     l.project_instance, l.ocd, l.batch_dt_tm,
     l.status, l.start_dt_tm, l.end_dt_tm,
     l.driver_count, rs_data->seconds, l.message,
     1, cnvtdatetimeutc(cnvtdatetime(curdate,curtime3))
     FROM dm_ocd_log l
     WHERE (l.ocd=readme_data->ocd)
      AND (l.environment_id=dm2_rr_misc->env_id)
      AND l.project_name=cnvtstring(readme_data->readme_id)
    ...
    WITH nocounter
   ;end insert
   SET rs_ecode = error(rs_emsg,1)
   IF (rs_ecode > 0)
    CALL echo(concat("Error can be ignored:",rs_emsg))
   ENDIF
  ENDIF
  ...
  IF ((rs_data->inhouse_flag != 0))
   IF ((rs_data->status=dm2_rr_failed))
    SET rs_data->text = "Please run the readme log report in the backend domain for failure details."
   ELSE
 ...
 IF ((validate(dm2_debug_flag,- (1))=- (1))
  ...
  SET dm_rs_debug_flag = 0
 ELSE
  SET dm_rs_debug_flag = dm2_debug_flag
 ENDIF
 IF (dm_rs_debug_flag > 1)
  CALL echo(fillstring(110,"*"))
...
#2000_update
 SELECT INTO "nl:"
  ...
  WHERE (l.ocd=readme_data->ocd)
   ...
   AND (l.project_instance=readme_data->instance)
   AND l.batch_dt_tm=cnvtdatetime(readme_data->batch_dt_tm)
  DETAIL
   rs_data->start_dt_tm = l.start_dt_tm, rs_data->end_dt_tm = l.end_dt_tm, rs_data->feature = l.ocd
  WITH nocounter
 ;end select
 IF (curqual <= 0)
  CALL rm_log(rm_warning,"No log row found for status update.")
  GO TO 9999_exit_program
 ENDIF
 IF ((readme_data->status="S")
  AND validate(dm_readme_finished_ind,"X")="T")
  ...
  ELSE
   SELECT INTO "nl:"
    FROM user_tables ut
    WHERE ut.table_name="DM_XNTR_README_DATA"
    WITH nocounter
   ;end select
   IF ((dm_err->ecode > 0))
    ROLLBACK
    SET readme_data->status = "F"
    SET readme_data->message = substring(1,255,concat(
      "Error occured checking for DM_XNTR_README_DATA:",dm_err->emsg))
   ELSEIF (curqual > 0)
    ROLLBACK
    SET readme_data->status = "F"
    SET readme_data->message = substring(1,255,concat(
 ...
 CASE (readme_data->status)
  OF "S":
   IF (validate(dm_readme_finished_ind,"X")="T")
    SET rs_data->status = rm_done
   ELSE
    SET rs_data->status = rm_running
   ENDIF
  OF "F":
   IF (validate(dm_readme_finished_ind,"X")="T")
    SET rs_data->status = rm_failed
   ELSE
    SET rs_data->status = rm_running
   ENDIF
  OF "R":
   IF (validate(dm_readme_finished_ind,"X")="T")
    SET rs_data->status = rm_reset
   ELSE
    SET rs_data->status = rm_running
   ENDIF
  ELSE
 ...
 ELSE
  ...
  ENDIF
 ENDIF
 IF ((rs_data->status=rm_done))
  UPDATE  FROM dm_ocd_log l
   SET l.status = rs_data->status, l.start_dt_tm = cnvtdatetime(rs_data->start_dt_tm), l.end_dt_tm =
    cnvtdatetime(rs_data->end_dt_tm),
    l.message = substring(1,255,readme_data->message), l.updt_dt_tm = cnvtdatetime(curdate,curtime3)
   WHERE l.environment_id=rm_env_id
    AND l.project_type=rm_readme
    AND (l.project_name=rs_data->name)
   WITH nocounter
  ;end update
 ELSEIF ((rs_data->status=rm_reset))
  UPDATE  FROM dm_ocd_log l
   SET l.status = rs_data->status, l.start_dt_tm = null, l.end_dt_tm = null,
    l.updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)), l.message =
    "Reset so dm2_readme_runner can pick it up again"
 ...
 ENDIF
 IF (curqual <= 0)
  CALL rm_log(rm_warning,"Unable to update log row.")
  GO TO 9999_exit_program
 ENDIF
 ...
 IF ((rs_data->inhouse_flag=0))
  GO TO 2900_time
 ENDIF
 IF ((rs_data->status=rm_failed))
  SET rs_data->text = "Please run the readme log report in the backend domain for failure details."
 ELSE
  SET rs_data->text = substring(1,100,readme_data->message)
 ENDIF
 UPDATE  FROM dm_project_status_env s
  SET s.dm_status = rs_data->status, s.dm_status_dt_tm = cnvtdatetime(curdate,curtime3), s
   .dm_status_msg = rs_data->text
 ...
 ;end update
#2900_time
 IF ( NOT ((rs_data->status=rm_done)))
  GO TO 2999_update_exit
 ENDIF
 SELECT INTO "nl:"
  l.driver_count
  FROM dm_ocd_log l,
