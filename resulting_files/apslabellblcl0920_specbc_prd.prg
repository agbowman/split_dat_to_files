CREATE PROGRAM apslabellblcl0920_specbc_prd
 PROMPT
 ...
 SUBROUTINE detailsectionabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(1.940000), private
   IF (ncalc=rpt_render)
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_datamatrix,(offsetx+ 0.126),(offsety+ 0.281))
    ...
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_datamatrix,(offsetx+ 0.084),(offsety+ 1.354))
    SET rptbce->m_recsize = 88
    SET rptbce->m_width = 0.27
    SET rptbce->m_height = 0.27
    SET rptbce->m_rotation = 0
    ...
    SET rptbce->m_bprintinterp = 0
    SET _rptstat = uar_rptbarcodeex(_hreport,rptbce,build2(barcode_accession,char(0)))
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_datamatrix,(offsetx+ 1.084),(offsety+ 1.354))
    SET rptbce->m_recsize = 88
    SET rptbce->m_width = 0.27
    SET rptbce->m_height = 0.27
    SET rptbce->m_rotation = 0
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "APSLABELLBLCL0920_SPECBC_PRD"
   SET rptreport->m_pagewidth = 2.00
   SET rptreport->m_pageheight = 1.94
