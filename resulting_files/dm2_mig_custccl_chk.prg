CREATE PROGRAM dm2_mig_custccl_chk
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmccc_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_chk")
 DECLARE dmccc_mode = vc WITH protect, noconstant("")
 DECLARE dmccc_master_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccc_tgt_ind = i2 WITH protect, noconstant(0)
 DECLARE dmccc_master_ind = i2 WITH protect, noconstant(0)
 DECLARE dmccc_diff_users_ind = i2 WITH protect, noconstant(0)
 DECLARE dmccc_diff_scripts_ind = i2 WITH protect, noconstant(0)
 DECLARE dmccc_cont = i2 WITH protect, noconstant(1)
 DECLARE dmccc_ok = i2 WITH protect, noconstant(1)
 DECLARE dmccc_env = vc WITH protect, noconstant(value(logical("environment")))
 DECLARE dmccc_node = vc WITH protect, noconstant("")
 DECLARE dmccc_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmccc_acc = vc WITH protect, noconstant("")
 DECLARE dmccc_str = vc WITH protect, noconstant("")
 FREE RECORD dmccc_nodes
 RECORD dmccc_nodes(
   1 master_id = f8
   ...
   1 tgt_cnt = i4
   1 qual[*]
     2 dma_env_node_id = f8
     2 node_name = vc
     2 environment_id = f8
 )
 FREE RECORD diff_users
 RECORD diff_users(
   1 cnt = i4
   1 users[*]
     2 dma_duaf_id = f8
     ...
     2 tgt_cnt = i4
     2 tgts[*]
       3 dma_env_node_id = f8
       3 exists_flag = i2
 )
 FREE RECORD diff_scripts
 RECORD diff_scripts(
   1 cnt = i4
   1 scripts[*]
     2 dma_dprotect_id = f8
     2 script_name = vc
     2 group_nbr = i4
     2 node_name = vc
     2 binary_cnt = f8
     2 app_major_version = f8
     2 datestamp = vc
     2 timestamp = vc
     2 master_ind = i2
     2 tgt_cnt = i4
 ...
 DECLARE dmccc_get_tgt_master_info(null) = i2
 DECLARE dmccc_get_diff_users(dgdu_master_ind=i2,dgdu_tgt_ind=i2,dgdu_diff_users_ind=i2(ref)) = i2
 DECLARE dmccc_view_diff_users(dvdu_master_ind=i2,dvdu_tgt_ind=i2) = i2
 DECLARE dmccc_get_diff_scripts(dgds_master_ind=i2,dgds_tgt_ind=i2,dgds_diff_scripts_ind=i2(ref)) =
 i2
 DECLARE dmccc_view_diff_scripts(dvds_master_ind=i2,dvds_tgt_ind=i2) = i2
 SET width = 132
 IF (check_logfile(dmccc_logfile_prefix,".log","dm2_mig_custccl_chk")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_chk"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Validating inputs for dm2_mig_custccl_chk"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmccc_mode = cnvtupper( $1)
 IF (((check_error(dm_err->eproc) != 0) OR (dmccc_mode != "TARGET"
  AND dmccc_mode != "MERGE")) )
  SET dm_err->emsg = "Parameter usage: dm2_mig_custccl_chk <mode=TARGET/MERGE>"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dmccc_mode="TARGET")
  SET dmccc_tgt_ind = 1
 ELSEIF (dmccc_mode="MERGE")
  SET dmccc_master_ind = 1
 ENDIF
 IF (dmccc_get_tgt_master_info(null)=0)
  GO TO exit_program
 ENDIF
 WHILE (dmccc_cont=1)
   IF (dmccc_get_diff_users(dmccc_master_ind,dmccc_tgt_ind,dmccc_diff_users_ind)=0)
    GO TO exit_program
   ENDIF
   IF (dmccc_get_diff_scripts(dmccc_master_ind,dmccc_tgt_ind,dmccc_diff_scripts_ind)=0)
    GO TO exit_program
   ENDIF
   IF (((dmccc_diff_users_ind=1) OR (dmccc_diff_scripts_ind=1)) )
    SET dmccc_ok = 1
    WHILE (dmccc_ok=1)
      SET message = window
      SET width = 132
      CALL clear(1,1)
      CALL box(1,1,22,131)
      CALL text(1,2,"Custom CCL Migration: Target Difference Reports")
      CALL text(1,70,"DATE/TIME: ")
      CALL text(1,80,build(month(curdate),"/",day(curdate),"/",year(curdate),
        " ",hour(curtime),":",minute(curtime)))
      CALL text(3,2,concat("Target Environment:",dmccc_env))
      SET dmccc_str = dmccc_nodes->qual[1].node_name
      FOR (dmccc_cnt = 2 TO dmccc_nodes->tgt_cnt)
        SET dmccc_str = concat(dmccc_str,",",dmccc_nodes->qual[dmccc_cnt].node_name)
      ENDFOR
      CALL text(4,2,concat("Target Nodes: ",dmccc_str))
      IF (dmccc_master_ind=1)
       CALL text(5,2,concat("Master Node: ",dmccc_nodes->master_node_name))
       CALL text(6,2,concat(
         "There are differences in users and/or scripts between Master and Targets."))
      ELSE
       CALL text(6,2,concat("There are differences in users and/or scripts between Targets."))
       CALL text(7,2,concat("Please resolve all differences shown in the reports before continuing.")
        )
      ENDIF
      CALL text(10,2,"(U)ser Difference Report")
      CALL text(11,2,"(S)cript Difference Report")
      CALL text(12,2,"(C)ontinue")
      CALL text(13,2,"(Q)uit")
      CALL text(14,2,"Enter your choice:")
      CALL accept(14,22,"A;cu"," "
       WHERE curaccept IN ("U", "S", "C", "Q"))
      CASE (curaccept)
       OF "Q":
        SET message = nowindow
        SET dmccc_cont = 0
        SET dmccc_ok = 0
        SET dm_err->emsg = "User elected to quit from Target Difference Reports"
        SET dm_err->err_ind = 1
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        GO TO exit_program
       OF "U":
        IF (dmccc_diff_users_ind=0)
         CALL text(16,2,"There are NO differences in CCL users. Press <Enter> to continue")
         CALL accept(16,86,"A;CH"," ")
         CALL clear(16,2,129)
        ELSE
         SET message = nowindow
         IF (dmccc_view_diff_users(dmccc_master_ind,dmccc_tgt_ind)=0)
          GO TO exit_program
         ENDIF
        ENDIF
       OF "S":
        IF (dmccc_diff_scripts_ind=0)
         CALL text(16,2,"There are NO differences in CCL scripts. Press <Enter> to continue")
         CALL accept(16,86,"A;CH"," ")
         CALL clear(16,2,129)
        ELSE
         SET message = nowindow
         IF (dmccc_view_diff_scripts(dmccc_master_ind,dmccc_tgt_ind)=0)
          GO TO exit_program
         ENDIF
        ENDIF
       OF "C":
   ...
   ELSE
    ...
    IF (dmccc_master_ind=1)
     CALL text(5,2,concat("Master Node: ",dmccc_nodes->master_node_name))
     CALL text(6,2,concat("There are NO differences in users and scripts between Master and Targets."
       ))
    ELSE
     CALL text(6,2,concat("There are NO differences in users and scripts between Targets."))
    ENDIF
    CALL text(7,2,"Press <Enter> to continue")
    CALL accept(7,28,"A;CH"," ")
    CALL clear(16,2,129)
    SET message = nowindow
    SET dmccc_cont = 0
   ENDIF
 ENDWHILE
 GO TO exit_program
 SUBROUTINE dmccc_get_tgt_master_info(null)
   DECLARE dgti_cnt = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Gathering information for MASTER and all TARGETs"
   CALL disp_msg("",dm_err->logfile,0)
   SET dm_err->eproc = "Getting MASTER and TARGET dma_env_node_ids from dma_env_node"
   SELECT INTO "nl:"
    FROM dma_env_node_attr dena,
     dma_env_node dne
    WHERE dena.attr_name="CCMIG_SETUPTYPE"
     AND dne.dma_env_node_id=dena.dma_env_node_id
    HEAD REPORT
     dmccc_nodes->tgt_cnt = 0, stat = alterlist(dmccc_nodes->qual,0)
    DETAIL
     IF (dena.attr_value_txt="MASTER")
      dmccc_nodes->master_id = dena.dma_env_node_id, dmccc_nodes->master_node_name = dne.node_name,
      dmccc_nodes->master_env_id = dne.environment_id
     ELSE
      dmccc_nodes->tgt_cnt = (dmccc_nodes->tgt_cnt+ 1), stat = alterlist(dmccc_nodes->qual,
       dmccc_nodes->tgt_cnt), dmccc_nodes->qual[dmccc_nodes->tgt_cnt].dma_env_node_id = dne
      .dma_env_node_id,
      dmccc_nodes->qual[dmccc_nodes->tgt_cnt].node_name = dne.node_name, dmccc_nodes->qual[
      dmccc_nodes->tgt_cnt].environment_id = dne.environment_id
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dmccc_nodes)
   ENDIF
   IF ((((dmccc_nodes->tgt_cnt=0)) OR ((dmccc_nodes->master_id=0))) )
    SET dm_err->emsg = concat("No Valid MASTER and/or TARGET Information was returned. ",
     "No. of Targets: ",cnvtstring(dmccc_nodes->tgt_cnt),"Master ID: ",cnvtstring(dmccc_nodes->
      master_id))
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmccc_get_diff_users(dgdu_master_ind,dgdu_tgt_ind,dgdu_diff_users_ind)
   DECLARE dgdu_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgdu_tgt_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgdu_u_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgdu_ndx = i4 WITH protect, noconstant(0)
   IF (dgdu_master_ind=1)
    SET dm_err->eproc = "Retrieving users present in MASTER but not in TARGETS"
   ELSE
    SET dm_err->eproc = "Retrieving users not present in all TARGETs"
   ENDIF
   CALL disp_msg("",dm_err->logfile,0)
   IF ((((dmccc_nodes->tgt_cnt=0)) OR ((dmccc_nodes->master_id=0))) )
    SET dm_err->emsg = concat("No Valid MASTER and/or TARGET Information was returned. ",
     "No. of Targets: ",dmccc_nodes->tgt_cnt,"Master ID: ",dmccc_nodes->master_id)
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dgdu_diff_users_ind = 0
   SET dm_err->eproc = "Retrieving list of users that are present in each node"
   SELECT INTO "nl:"
    unq_user = concat(d.user_name,"|",trim(cnvtstring(d.group_nbr)),"|",d.group_name), d
    .dma_env_node_id
    FROM dma_env_node_duaf d
    ORDER BY d.user_name, d.group_nbr, d.group_name,
     d.dma_env_node_id
    HEAD REPORT
     diff_users->cnt = 0, stat = alterlist(diff_users->users,0)
    HEAD unq_user
     diff_users->cnt = (diff_users->cnt+ 1), stat = alterlist(diff_users->users,diff_users->cnt),
     diff_users->users[diff_users->cnt].dma_duaf_id = d.dma_duaf_id,
     diff_users->users[diff_users->cnt].user_name = d.user_name, diff_users->users[diff_users->cnt].
     group_nbr = d.group_nbr
     IF ((d.dma_env_node_id=dmccc_nodes->master_id))
      diff_users->users[diff_users->cnt].master_ind = 1
     ELSE
      diff_users->users[diff_users->cnt].tgt_cnt = 0, stat = alterlist(diff_users->users[diff_users->
       cnt].tgts,0)
     ENDIF
    DETAIL
     IF ((d.dma_env_node_id != dmccc_nodes->master_id))
      diff_users->users[diff_users->cnt].tgt_cnt = (diff_users->users[diff_users->cnt].tgt_cnt+ 1),
      stat = alterlist(diff_users->users[diff_users->cnt].tgts,diff_users->users[diff_users->cnt].
       tgt_cnt), diff_users->users[diff_users->cnt].tgts[diff_users->users[diff_users->cnt].tgt_cnt].
      dma_env_node_id = d.dma_env_node_id,
      diff_users->users[diff_users->cnt].tgts[diff_users->users[diff_users->cnt].tgt_cnt].exists_flag
       = 1
     ENDIF
    FOOT  unq_user
     IF (dgdu_master_ind=1)
      IF ((diff_users->users[diff_users->cnt].master_ind=1))
       IF ((dmccc_nodes->tgt_cnt != diff_users->users[diff_users->cnt].tgt_cnt))
        dgdu_diff_users_ind = 1
       ENDIF
      ENDIF
     ELSE
      IF ((dmccc_nodes->tgt_cnt != diff_users->users[diff_users->cnt].tgt_cnt)
       AND (diff_users->users[diff_users->cnt].tgt_cnt != 0))
       dgdu_diff_users_ind = 1
      ENDIF
     ENDIF
    WITH nocounter
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(diff_users)
    CALL echo(dgdu_diff_users_ind)
   ENDIF
   IF ((diff_users->cnt=0))
    SET dm_err->emsg = "No Valid TARGET Users were returned"
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmccc_get_diff_scripts(dgds_master_ind,dgds_tgt_ind,dgds_diff_scripts_ind)
   DECLARE dgds_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgds_tgt_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgds_u_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgds_ndx = i4 WITH protect, noconstant(0)
   IF (dgds_master_ind=1)
    SET dm_err->eproc = "Retrieving scripts present in MASTER but not in TARGETS"
   ELSE
    SET dm_err->eproc = "Retrieving scripts not present in all TARGETs"
   ENDIF
   CALL disp_msg("",dm_err->logfile,0)
   IF ((((dmccc_nodes->tgt_cnt=0)) OR ((dmccc_nodes->master_id=0))) )
    SET dm_err->emsg = concat("No Valid MASTER and/or TARGET Information was returned. ",
    ...
    RETURN(0)
   ENDIF
   SET dgds_diff_scripts_ind = 0
   SET dm_err->eproc = "Retrieving list of scripts that are present in each target"
   SELECT INTO "nl:"
    unq_script = concat(d.object_name,"|",trim(cnvtstring(d.group_nbr))), d.dma_env_node_id
    FROM dma_env_node_dprotect d
    ORDER BY d.object_name, d.group_nbr, d.dma_env_node_id
    HEAD REPORT
     diff_scripts->cnt = 0, stat = alterlist(diff_scripts->scripts,0)
    HEAD unq_script
     diff_scripts->cnt = (diff_scripts->cnt+ 1), stat = alterlist(diff_scripts->scripts,diff_scripts
      ->cnt), diff_scripts->scripts[diff_scripts->cnt].dma_dprotect_id = d.dma_dprotect_id,
     diff_scripts->scripts[diff_scripts->cnt].script_name = d.object_name, diff_scripts->scripts[
     diff_scripts->cnt].group_nbr = d.group_nbr, diff_scripts->scripts[diff_scripts->cnt].binary_cnt
      = d.binary_cnt,
     diff_scripts->scripts[diff_scripts->cnt].app_major_version = d.app_major_version_nbr,
     diff_scripts->scripts[diff_scripts->cnt].datestamp = d.datestamp_txt, diff_scripts->scripts[
     diff_scripts->cnt].timestamp = d.timestamp_txt
     IF ((d.dma_env_node_id=dmccc_nodes->master_id))
      diff_scripts->scripts[diff_scripts->cnt].master_ind = 1
     ELSE
      diff_scripts->scripts[diff_scripts->cnt].tgt_cnt = 0, stat = alterlist(diff_scripts->scripts[
       diff_scripts->cnt].tgts,0)
     ENDIF
    DETAIL
     IF ((d.dma_env_node_id != dmccc_nodes->master_id))
      diff_scripts->scripts[diff_scripts->cnt].tgt_cnt = (diff_scripts->scripts[diff_scripts->cnt].
      tgt_cnt+ 1), stat = alterlist(diff_scripts->scripts[diff_scripts->cnt].tgts,diff_scripts->
       scripts[diff_scripts->cnt].tgt_cnt), diff_scripts->scripts[diff_scripts->cnt].tgts[
      diff_scripts->scripts[diff_scripts->cnt].tgt_cnt].dma_env_node_id = d.dma_env_node_id,
      diff_scripts->scripts[diff_scripts->cnt].tgts[diff_scripts->scripts[diff_scripts->cnt].tgt_cnt]
      .exists_flag = 1
     ENDIF
    FOOT  unq_script
     IF (dgds_master_ind=1)
      IF ((diff_scripts->scripts[diff_scripts->cnt].master_ind=1))
       IF ((dmccc_nodes->tgt_cnt != diff_scripts->scripts[diff_scripts->cnt].tgt_cnt))
        dgds_diff_scripts_ind = 1
       ENDIF
      ENDIF
     ELSE
      IF ((dmccc_nodes->tgt_cnt != diff_scripts->scripts[diff_scripts->cnt].tgt_cnt)
       AND (diff_scripts->scripts[diff_scripts->cnt].tgt_cnt != 0))
       dgds_diff_scripts_ind = 1
      ENDIF
     ENDIF
    WITH nocounter
   ...
   IF ((dm_err->debug_flag > 0))
    IF ((dm_err->debug_flag=511))
     CALL echorecord(diff_scripts)
    ENDIF
    CALL echo(diff_scripts->cnt)
    CALL echo(dgds_diff_scripts_ind)
   ENDIF
   IF ((diff_scripts->cnt=0))
    SET dm_err->emsg = "No Valid TARGET scripts were returned"
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmccc_view_diff_users(dvdu_master_ind,dvdu_tgt_ind)
   DECLARE dvdu_cnt = i4 WITH protect, noconstant(0)
   DECLARE dvdu_cnt2 = i4 WITH protect, noconstant(0)
   DECLARE dvdu_ndx = i4 WITH protect, noconstant(0)
   DECLARE dvdu_cmp_flag = i2 WITH protect, noconstant(0)
   DECLARE dvdu_rpt_file = vc WITH protect, noconstant("")
   DECLARE dvdu_tgt_list = vc WITH protect, noconstant("")
   DECLARE dvdu_miss_tgt_list = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Generating User Difference Report"
   CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
   IF ((dmccc_nodes->tgt_cnt=0))
    SET dm_err->emsg = "No Valid TARGET Information was returned."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((diff_users->cnt=0))
    ...
    RETURN(0)
   ENDIF
   IF (get_unique_file("dm2_mig_cc_users",".dat")=0)
    GO TO exit_program
   ELSE
    SET dvdu_rpt_file = dm_err->unique_fname
   ENDIF
   SELECT INTO value(dvdu_rpt_file)
    FROM dummyt d
    HEAD REPORT
     CALL print(fillstring(250,"-")), row + 1, "User Difference Report",
     row + 1,
     CALL print(fillstring(250,"-")), row + 2
     IF (dvdu_master_ind=1)
      CALL print(concat("  Master Node: ",dmccc_nodes->master_node_name)), row + 1
     ENDIF
     CALL print("  List of Targets Compared: "), row + 1
     FOR (dvdu_cnt = 1 TO dmccc_nodes->tgt_cnt)
      CALL print(concat("  ",dmccc_nodes->qual[dvdu_cnt].node_name)),row + 1
     ENDFOR
     CALL print(fillstring(250,"-")), row + 2,
     CALL print("Mismatched CCL Users are shown below"),
     row + 2
    DETAIL
     col 0, "USER NAME ", col 40,
     "| GROUP NUMBER ", col 55, "| NODES ON WHICH USER IS MISSING ",
     row + 1,
     CALL print(fillstring(250,"-")), row + 2,
     dvdu_cnt = 0
     FOR (dvdu_cnt = 1 TO diff_users->cnt)
      dvdu_miss_tgt_list = "",
      IF (dvdu_master_ind=1)
       IF ((diff_users->users[dvdu_cnt].master_ind=1))
        IF ((diff_users->users[dvdu_cnt].tgt_cnt != dmccc_nodes->tgt_cnt))
         FOR (dvdu_cnt2 = 1 TO dmccc_nodes->tgt_cnt)
           IF (locateval(dvdu_ndx,1,diff_users->users[dvdu_cnt].tgt_cnt,dmccc_nodes->qual[dvdu_cnt2].
            dma_env_node_id,diff_users->users[dvdu_cnt].tgts[dvdu_ndx].dma_env_node_id)=0)
            dvdu_miss_tgt_list = concat(dvdu_miss_tgt_list,cnvtupper(dmccc_nodes->qual[dvdu_cnt2].
              node_name)," | ")
           ENDIF
         ENDFOR
        ENDIF
        IF (dvdu_miss_tgt_list != "")
         col 0,
         CALL print(trim(diff_users->users[dvdu_cnt].user_name)), col 40,
         "| ",
         CALL print(trim(build(diff_users->users[dvdu_cnt].group_nbr))), col 55,
         "| ",
         CALL print(dvdu_miss_tgt_list), row + 1
        ENDIF
       ENDIF
      ELSE
       IF ((diff_users->users[dvdu_cnt].tgt_cnt != dmccc_nodes->tgt_cnt)
        AND (diff_users->users[dvdu_cnt].tgt_cnt != 0))
        FOR (dvdu_cnt2 = 1 TO dmccc_nodes->tgt_cnt)
          IF (locateval(dvdu_ndx,1,diff_users->users[dvdu_cnt].tgt_cnt,dmccc_nodes->qual[dvdu_cnt2].
           dma_env_node_id,diff_users->users[dvdu_cnt].tgts[dvdu_ndx].dma_env_node_id)=0)
           dvdu_miss_tgt_list = concat(dvdu_miss_tgt_list,cnvtupper(dmccc_nodes->qual[dvdu_cnt2].
   ...
   IF (check_error(dm_err->eproc))
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Displaying Mismatched CCL Users Report"
   IF (dm2_disp_file(dvdu_rpt_file,"Mismatched CCL Users Report")=0)
    RETURN(0)
   ENDIF
 END ;Subroutine
 SUBROUTINE dmccc_view_diff_scripts(dvds_master_ind,dvds_tgt_ind)
   DECLARE dvds_cnt = i4 WITH protect, noconstant(0)
   DECLARE dvds_cnt2 = i4 WITH protect, noconstant(0)
   DECLARE dvds_ndx = i4 WITH protect, noconstant(0)
   DECLARE dvds_cmp_flag = i2 WITH protect, noconstant(0)
   DECLARE dvds_rpt_file = vc WITH protect, noconstant("")
   DECLARE dvds_tgt_list = vc WITH protect, noconstant("")
   DECLARE dvds_miss_tgt_list = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Generating Script Difference Report"
   CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
   IF ((dmccc_nodes->tgt_cnt=0))
    SET dm_err->emsg = "No Valid TARGET Information was returned."
    SET dm_err->err_ind = 1
   ...
   IF ((diff_scripts->cnt=0))
    ...
    RETURN(0)
   ENDIF
   IF (get_unique_file("dm2_mig_cc_scripts",".dat")=0)
    GO TO exit_program
   ELSE
    SET dvds_rpt_file = dm_err->unique_fname
   ENDIF
   SELECT INTO value(dvds_rpt_file)
    FROM dummyt d
    HEAD REPORT
     CALL print(fillstring(250,"-")), row + 1, "Script Difference Report",
     row + 1,
     CALL print(fillstring(250,"-")), row + 2
     IF (dvds_master_ind=1)
      CALL print(concat("  Master Node: ",dmccc_nodes->master_node_name)), row + 1
     ENDIF
     CALL print("  List of Targets Compared: "), row + 1
     FOR (dvds_cnt = 1 TO dmccc_nodes->tgt_cnt)
      CALL print(concat("  ",dmccc_nodes->qual[dvds_cnt].node_name)),row + 1
     ENDFOR
     CALL print(fillstring(250,"-")), row + 2,
     CALL print("Mismatched CCL scripts are shown below"),
     row + 2
    DETAIL
     col 0, "SCRIPT NAME ", col 40,
     "| GROUP NUMBER ", col 55, "| BINARY COUNT ",
     col 70, "| APP MAJOR VERSION ", col 90,
     "| DATESTAMP ", col 105, "| TIMESTAMP ",
     col 120, "| NODES ON WHICH SCRIPT IS MISSING ", row + 1,
     CALL print(fillstring(250,"-")), row + 2, dvds_cnt = 0
     FOR (dvds_cnt = 1 TO diff_scripts->cnt)
      dvds_miss_tgt_list = "",
      IF (dvds_master_ind=1)
       IF ((diff_scripts->scripts[dvds_cnt].master_ind=1))
        IF ((diff_scripts->scripts[dvds_cnt].tgt_cnt != dmccc_nodes->tgt_cnt))
         FOR (dvds_cnt2 = 1 TO dmccc_nodes->tgt_cnt)
           IF (locateval(dvds_ndx,1,diff_scripts->scripts[dvds_cnt].tgt_cnt,dmccc_nodes->qual[
            dvds_cnt2].dma_env_node_id,diff_scripts->scripts[dvds_cnt].tgts[dvds_ndx].dma_env_node_id
            )=0)
            dvds_miss_tgt_list = concat(dvds_miss_tgt_list,cnvtupper(dmccc_nodes->qual[dvds_cnt2].
              node_name)," | ")
           ENDIF
         ENDFOR
        ENDIF
        IF (dvds_miss_tgt_list != "")
         col 0,
         CALL print(trim(diff_scripts->scripts[dvds_cnt].script_name)), col 40,
         "| ",
         CALL print(trim(build(diff_scripts->scripts[dvds_cnt].group_nbr))), col 55,
         "| ",
         CALL print(trim(build(diff_scripts->scripts[dvds_cnt].binary_cnt))), col 70,
         "| ",
         CALL print(cnvtstring(diff_scripts->scripts[dvds_cnt].app_major_version)), col 90,
         "| ",
         CALL print(trim(diff_scripts->scripts[dvds_cnt].datestamp)), col 105,
         "| ",
         CALL print(trim(diff_scripts->scripts[dvds_cnt].timestamp)), col 120,
         "| ",
         CALL print(dvds_miss_tgt_list), row + 1
        ENDIF
       ENDIF
      ELSE
       IF ((diff_scripts->scripts[dvds_cnt].tgt_cnt != dmccc_nodes->tgt_cnt)
        AND (diff_scripts->scripts[dvds_cnt].tgt_cnt != 0))
        FOR (dvds_cnt2 = 1 TO dmccc_nodes->tgt_cnt)
          IF (locateval(dvds_ndx,1,diff_scripts->scripts[dvds_cnt].tgt_cnt,dmccc_nodes->qual[
           dvds_cnt2].dma_env_node_id,diff_scripts->scripts[dvds_cnt].tgts[dvds_ndx].dma_env_node_id)
          =0)
   ...
   IF (check_error(dm_err->eproc))
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Displaying Mismatched CCL Script Report"
   IF (dm2_disp_file(dvds_rpt_file,"Mismatched CCL Script Report")=0)
    RETURN(0)
   ENDIF
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_chk"
 CALL final_disp_msg(dmccc_logfile_prefix)
END GO
