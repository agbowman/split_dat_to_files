CREATE PROGRAM dm2_utc_schema_runner:dba
 DECLARE dusr_cleanup_timestamp = f8 WITH public
 DECLARE dusr_updt_cnt = i4 WITH public
 DECLARE dusr_updt_fnd_cnt = i4 WITH public
 DECLARE dusr_updt_stolen_cnt = i4 WITH public
 DECLARE dusr_updt_constolen_cnt = i4 WITH public
 DECLARE dusreidx = i4 WITH public
 DECLARE dusr_requery_threshold_cnt = i4 WITH public, constant(40)
 DECLARE dusr_cmd_rscbsy_retry_cnt = i4 WITH public, noconstant(0)
 DECLARE dusr_cmd_successful = i2 WITH public, noconstant(0)
 DECLARE dusr_cmd_rscbsy_error = i2 WITH public, noconstant(0)
 DECLARE dusr_cmd_firstexec = i2 WITH public, noconstant(0)
 DECLARE dusr_op_id = f8 WITH public, noconstant(0.0)
 DECLARE dusr_op_type = vc WITH public, noconstant(" ")
 DECLARE dusr_command = vc WITH public, noconstant(" ")
 DECLARE dusr_oe_command = vc WITH public, noconstant(" ")
 DECLARE dusr_begin_dt_tm = f8 WITH public, noconstant(0.0)
 DECLARE dusr_obj_name = vc WITH public, noconstant(" ")
 DECLARE dusr_dm_info_exists = i2 WITH public, noconstant(0)
 DECLARE dusr_logfile_prefix = vc WITH protect, constant("dm2_utc_sch_runner")
 DECLARE dusr_cmd = vc WITH protect, noconstant("")
 DECLARE dusr_serial_op_types = vc WITH protect, noconstant("('DM2NOTSET')")
 DECLARE dusr_wait = i2 WITH protect, noconstant(0)
 DECLARE dusr_wait_timestamp = f8 WITH protect, noconstant(0.0)
 DECLARE dusr_runner_type = vc WITH protect, noconstant("")
 DECLARE dusr_loadreadytorun(null) = i4
 DECLARE dusr_whereclause(dw_info_domain=vc,dw_clause=vc(ref)) = i2
 DECLARE dusr_check_whether_to_wait(dcwtw_wait=i2(ref)) = i4
 FREE RECORD dusr_execlist
 RECORD dusr_execlist(
   1 exec_cnt = i4
   1 exec[*]
     2 op_id = f8
 )
 SET dusr_execlist->exec_cnt = 0
 ...
 SUBROUTINE dum_cust_incl_abort_gen(dciag_tgt_dbname,dciag_src_orcl_ver,dciag_tgt_orcl_ver)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile(dusr_logfile_prefix,".log","dm2_utc_schema_runner")=0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_utc_schema_runner"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 SET dusr_updt_cnt = 0
 SET dusr_updt_fnd_cnt = 0
 SET dusr_updt_stolen_cnt = 0
 SET dusr_updt_constolen_cnt = 0
 IF ((dm2_install_schema->appl_id="NONE"))
  SET dm2_install_schema->appl_id = currdbhandle
 ENDIF
 IF (validate(dm2_utc_bkgrnd_calling_script,"X")="DM2_UTC_SET_READY_TO_RUN")
  SET dusr_runner_type = "MAIN_RUNNER"
 ELSE
  SET dusr_runner_type = "PARALLEL_RUNNER"
 ENDIF
 SET dm_err->eproc = "Determining if dm_info exist."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_table_and_ccldef_exists("DM_INFO",dusr_dm_info_exists)=0)
  GO TO end_program
 ENDIF
 IF (dusr_dm_info_exists=1)
  IF (dusr_whereclause("DM2_SERIAL_EXEC_OP_TYPE",dusr_serial_op_types)=0)
   GO TO end_program
  ENDIF
  UPDATE  FROM dm_info di
   SET di.info_char = dusr_runner_type, di.info_number = 1, di.info_long_id = 0,
    di.updt_applctx = 0, di.updt_cnt = 0, di.updt_id = 0,
    di.updt_task = reqinfo->updt_task
   WHERE di.info_domain="DM2_UTC_SCHEMA_RUNNER"
    AND (di.info_name=dm2_install_schema->appl_id)
   WITH nocounter
  ;end update
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  ELSEIF (curqual=0)
   SET dusr_cmd = concat(
    "rdb asis(^insert into dm_info di (di.info_domain,di.info_name,di.info_char, di.info_number, ",
    "di.info_long_id,di.updt_applctx,di.updt_cnt,di.updt_id, di.updt_task) ",
    "values ('DM2_UTC_SCHEMA_RUNNER', '",dm2_install_schema->appl_id,"', '",
    trim(dusr_runner_type),"', 1, 0, 0, 0, 0, ",trim(cnvtstring(reqinfo->updt_task)),")^) go")
   CALL dm2_push_cmd(dusr_cmd,1)
   IF (check_error("Inserting DM2_UTC_SCHEMA_RUNNER row to DM_INFO.")=1)
    ROLLBACK
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO end_program
   ELSE
 ...
 ENDIF
 SET dusr_wait = 1
 SET dusr_wait_timestamp = cnvtdatetime(curdate,curtime3)
 WHILE (dusr_wait=1)
  IF (dusr_loadreadytorun(1) != 1)
   GO TO end_program
  ENDIF
  IF ((dusr_execlist->exec_cnt=0)
   AND dusr_runner_type="PARALLEL_RUNNER")
   IF (dusr_check_whether_to_wait(dusr_wait)=0)
    GO TO end_program
   ENDIF
   IF (dusr_wait=1)
    IF (datetimediff(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),cnvtdatetimeutc(cnvtdatetime(
       dusr_cleanup_timestamp)),4) > 15)
  ...
  ENDIF
 ENDWHILE
 WHILE ((dusr_execlist->exec_cnt > 0))
   FOR (dusreidx = 1 TO dusr_execlist->exec_cnt)
     SET dusr_cmd_successful = 0
     SET dusr_cmd_rscbsy_retry_cnt = 0
     SET dusr_cmd_rscbsy_error = 0
     SET dusr_cmd_firstexec = 1
     WHILE (((dusr_cmd_firstexec=1) OR (dusr_cmd_successful=0
      AND dusr_cmd_rscbsy_error=1
      AND dusr_cmd_rscbsy_retry_cnt < cnvtint(dm2_db_options->resource_busy_maxretry))) )
       IF (dusr_cmd_rscbsy_retry_cnt > 1)
        CALL pause(3)
       ENDIF
       SET dusr_cmd_rscbsy_retry_cnt = (dusr_cmd_rscbsy_retry_cnt+ 1)
       EXECUTE dm2_utc_schema_runner_c
       IF ((dm_err->err_ind=1))
        GO TO end_program
       ENDIF
       SET dusr_cmd_firstexec = 0
     ENDWHILE
     IF (dusr_updt_constolen_cnt > dusr_requery_threshold_cnt)
      SET dusreidx = dusr_execlist->exec_cnt
      SET dusr_updt_constolen_cnt = 0
     ENDIF
   ENDFOR
   SET dusr_wait = 1
 ...
 ENDWHILE
 IF ((dm_err->debug_flag > 0))
  CALL echo(build("Total updt_cnt:",dusr_updt_cnt))
  CALL echo(build("Total updt_fnd_cnt:",dusr_updt_fnd_cnt))
  CALL echo(build("Total updt_stolen_cnt:",dusr_updt_stolen_cnt))
 ENDIF
 GO TO end_program
 SUBROUTINE dusr_loadreadytorun(null)
   SET dusr_execlist->exec_cnt = 0
   SET stat = alterlist(dusr_execlist->exec,dusr_execlist->exec_cnt)
   SELECT
    IF (dusr_serial_op_types != "('DM2NOTSET')")
     ...
     (SELECT
      ...
      AND ((parser(concat("ddol1.op_type in ",dusr_serial_op_types))
      AND  NOT ( EXISTS (
     (SELECT
      "x"
      FROM dm2_ddl_ops_log x
      WHERE parser(concat("x.op_type in ",dusr_serial_op_types))
       AND x.status="RUNNING")))) OR (parser(concat("ddol1.op_type not in ",dusr_serial_op_types))))
    ...
    INTO "nl:"
    DETAIL
     dusr_execlist->exec_cnt = (dusr_execlist->exec_cnt+ 1)
     IF (mod(dusr_execlist->exec_cnt,1000)=1)
      stat = alterlist(dusr_execlist->exec,(dusr_execlist->exec_cnt+ 999))
     ENDIF
     dusr_execlist->exec[dusr_execlist->exec_cnt].op_id = ddol1.op_id
    FOOT REPORT
     stat = alterlist(dusr_execlist->exec,dusr_execlist->exec_cnt)
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF ((dm_err->debug_flag > 3))
    CALL echorecord(dusr_execlist)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dusr_whereclause(dw_info_domain,dw_clause)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dusr_check_whether_to_wait(dcwtw_wait)
   IF (datetimediff(cnvtdatetime(curdate,curtime),cnvtdatetime(dusr_wait_timestamp),4) > 5)
    SET dusr_wait_timestamp = cnvtdatetime(curdate,curtime3)
    IF ((dm_err->debug_flag > 1))
     CALL echo(concat("Current date and time = ",format(cnvtdatetime(curdate,curtime),
        "mm/dd/yyyy hh:mm:ss;;d")))
     CALL echo(concat("Last run date and time = ",format(dusr_wait_timestamp,"mm/dd/yyyy hh:mm:ss;;d"
        )))
     CALL echo(concat("Number of minutes elapsed = ",cnvtstring(datetimediff(cnvtdatetime(curdate,
          curtime),cnvtdatetime(dusr_wait_timestamp),4),4,3)))
    ENDIF
 ...
 IF (dusr_dm_info_exists=1)
  ...
  ;end delete
  COMMIT
  IF (check_error("Removing DM2_UTC_SCHEMA_RUNNER row from dm_info.")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  ENDIF
 ENDIF
 SET dm_err->eproc = "Ending dm2_utc_schema_runner"
 CALL final_disp_msg(" ")
END GO
