CREATE PROGRAM dm2_tspace_driver:dba
 ...
 SUBROUTINE dm2_adb_check(dac_db_link,dac_adb_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_tspace_driver",".log","DM2_TSPACE_DRIVER LOG FILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "BEGIN DM2_TSPACE_DRIVER"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dtd_quit_menu_ind = i2 WITH public, noconstant(0)
 DECLARE dtd_space_needs_ind = i2 WITH protect, noconstant(0)
 DECLARE dtd_file_found = i2 WITH protect, noconstant(0)
 DECLARE dtd_answer_ret = vc WITH protect, noconstant("")
 DECLARE dtd_get_space_ind = i2 WITH protect, noconstant(0)
 DECLARE dtd_rpt_filename = vc WITH protect, noconstant("")
 DECLARE dtd_disp_rpt_ind = i2 WITH protect, noconstant(1)
 DECLARE dtd_database_diff_os = i2 WITH protect, noconstant(0)
 DECLARE dtd_database_remote = i2 WITH protect, noconstant(0)
 DECLARE dtd_cnt = i4 WITH protect, noconstant(0)
 DECLARE dtd_nidx_cnt = i4 WITH protect, noconstant(0)
 DECLARE dtd_disp_warning = i4 WITH protect, noconstant(0)
 DECLARE dtd_new_chk = i4 WITH protect, noconstant(0)
 DECLARE dtd_tspaces_dropped = i2 WITH protect, noconstant(0)
 DECLARE dtd_exist_rpt_filename = vc WITH protect, noconstant("")
 DECLARE dtd_df_added = i2 WITH protect, noconstant(0)
 DECLARE dtd_exist_needs_space = i2 WITH protect, noconstant(0)
 DECLARE dtd_ui_syspause_event = vc WITH protect, noconstant("SYSPAUSE:TSPACE1")
 DECLARE dtd_ui_install_event = vc WITH protect, noconstant("SCHEMA")
 DECLARE dtd_user_list = vc WITH protect, noconstant("")
 DECLARE dtd_adb_ind = i2 WITH protect, noconstant(0)
 DECLARE dtd_add_rpt_to_event_dtl(dard_rpt_in=vc) = i2
 DECLARE dtd_get_tmp_tspace_needs(null) = i2
 DECLARE dtd_add_space_to_existing(null) = i2
 DECLARE dtd_drop_empty_tspace(ddet_dropped=i2(ref)) = i2
 DECLARE dtd_load_datafiles(null) = i2
 DECLARE dtd_figure_df_to_add(dfda_added=i2(ref)) = i2
 DECLARE dctn_check_status(null) = i2
 DECLARE dtd_calc_tsp_needs_for_clin_copy(null) = i2
 DECLARE dtd_mode_prompt(dmp_answer_out=vc(ref)) = i2
 DECLARE dtd_disp_tspace_rpt(null) = i2
 DECLARE dtd_manual_process_tspace(null) = i2
 DECLARE dtd_post_prompt(dpp_answer_out=vc(ref)) = i2
 DECLARE dtd_pop_current_size(null) = i2
 DECLARE dtd_auto_success_obj_needs(null) = i2
 DECLARE dtd_load_readme_content(null) = i2
 DECLARE dtd_disp_temp_tspace_rpt(null) = i2
 DECLARE dtd_rpt_exist_tsp_needs_by_dg(detnbd_file=vc,detnbd_exist_space_needs=i2(ref)) = i2
 DECLARE dtd_ui_tspace_pause(dutp_plan_id=f8,dutp_event_id=f8,dutp_pause_event=vc,dutp_file_name=vc,
  dutp_install_event=vc) = i2
 DECLARE dtd_get_usersql_tspaces(dgut_user=vc) = i2
 DECLARE dtd_get_src_custom_users(dgscu_user_list=vc(ref)) = i2
 DECLARE dtd_set_tspace_quotas(null) = i2
 DECLARE dtd_load_tspace_quota_candidates(null) = i2
 ...
 FREE RECORD dtr_dg_for_ts
 RECORD dtr_dg_for_ts(
   1 ts[*]
     2 tspace_name = vc
     2 dg[*]
       3 disk_group = vc
 ...
 FREE RECORD dtd_bad_tspaces
 RECORD dtd_bad_tspaces(
   1 tspace_cnt = i4
   1 tspace[*]
 ...
 SET dtd_bad_tspaces->tspace_cnt = 0
 FREE RECORD dtd_user_tspaces
 RECORD dtd_user_tspaces(
   1 cnt = i4
   1 qual[*]
     2 tspace_name = vc
     2 temp_ind = i2
 )
 SET dtd_user_tspaces->cnt = 0
 FREE RECORD dtd_exception_list
 RECORD dtd_exception_list(
   1 cnt = i4
   1 qual[*]
     2 owner = vc
 )
 FREE RECORD dtd_ts_quota_cand
 RECORD dtd_ts_quota_cand(
   1 cnt = i4
   1 qual[*]
     2 tspace_name = vc
 )
 SET dtd_ts_quota_cand->cnt = 0
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_adb_check("",dtd_adb_ind)=0)
  GO TO exit_script
 ENDIF
 IF ((dm2_rr_spc_needs->space_needed=1))
  IF (dtd_load_readme_content(null)=0)
   GO TO exit_script
  ENDIF
  SET dtd_ui_syspause_event = "SYSPAUSE:TSPACE2"
  SET dtd_ui_install_event = "POST-SCHEMA_READMES"
 ENDIF
 IF ((dm2_install_schema->process_option != "CLIN COPY"))
  EXECUTE dm2_assign_tspace
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
 ENDIF
 IF (dtd_adb_ind=1)
  GO TO exit_script
 ENDIF
 IF ((dm2_install_schema->process_option="CHECK"))
  GO TO exit_script
 ENDIF
 IF ((dm2_install_schema->process_option != "CLIN COPY"))
  EXECUTE dm2_apply_sizing
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
 ENDIF
 SET dtd_exist_needs_space = 0
 IF ((dir_storage_misc->tgt_storage_type="ASM")
  AND (dm2_install_schema->process_option != "INHOUSE")
  AND (dm2_install_schema->dbase_type != "ADMIN"))
  IF (get_unique_file("dm2_tsp_needs_by_dg",".rpt")=0)
   GO TO exit_script
  ENDIF
  SET dtd_exist_rpt_filename = dm_err->unique_fname
  IF (validate(drrr_responsefile_in_use,0)=1)
   SET dtd_exist_rpt_filename = build(drrr_misc_data->active_dir,dtd_exist_rpt_filename)
  ENDIF
  IF ((dm2_install_schema->process_option="CLIN COPY"))
   IF (dtd_calc_tsp_needs_for_clin_copy(null)=0)
    GO TO exit_script
   ENDIF
   IF (dtd_drop_empty_tspace(dtd_tspaces_dropped)=0)
    GO TO exit_script
   ENDIF
   IF (dtd_tspaces_dropped=1)
   ...
   IF (locateval(dtd_new_chk,1,size(rtspace->qual,5),0,rtspace->qual[dtd_new_chk].new_ind) > 0)
    IF (dtd_load_datafiles(null)=0)
     GO TO exit_script
    ENDIF
    IF (dtd_figure_df_to_add(dtd_df_added)=0)
     GO TO exit_script
    ENDIF
    IF (dtd_df_added=1)
     IF (dtd_add_space_to_existing(null)=0)
      GO TO exit_script
     ENDIF
     IF (dtd_load_datafiles(null)=0)
      GO TO exit_script
     ENDIF
    ENDIF
    IF (dtd_rpt_exist_tsp_needs_by_dg(dtd_exist_rpt_filename,dtd_exist_needs_space)=0)
     GO TO exit_script
    ENDIF
   ENDIF
  ELSE
   EXECUTE dm2_calc_tspace_needs
   IF ((dm_err->err_ind=1))
    GO TO exit_script
   ENDIF
   IF (locateval(dtd_new_chk,1,size(rtspace->qual,5),0,rtspace->qual[dtd_new_chk].new_ind) > 0)
   ...
   ENDIF
   IF ((dir_ui_misc->dm_process_event_id > 0))
    IF (drr_assign_file_to_installs("RPT:TSPACENEEDS",dtd_exist_rpt_filename,dir_ui_misc->
   ...
   IF (dtd_rpt_exist_tsp_needs_by_dg(dtd_exist_rpt_filename,dtd_exist_needs_space)=0)
    GO TO exit_script
   ENDIF
   IF ((dm2_install_schema->process_option="PREVIEW"))
    IF (dtd_get_tmp_tspace_needs(null)=0)
     RETURN(0)
    ENDIF
    IF ((rtspace->temp_tspace_ttl_needed_mb > (rtspace->temp_tspace_ttl_mb - rtspace->
    temp_tspace_reserved_mb)))
     IF (dtd_disp_temp_tspace_rpt(null)=0)
      RETURN(0)
     ENDIF
    ENDIF
   ENDIF
  ENDIF
  IF (dtd_load_tspace_quota_candidates(null)=0)
   GO TO exit_script
  ENDIF
  IF (locateval(dtd_new_chk,1,size(rtspace->qual,5),1,rtspace->qual[dtd_new_chk].new_ind) > 0)
   IF ((dir_ui_misc->background_ind=1)
    ...
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "New Tablespaces must be pre-created for Unattended Installs."
    SET dm_err->user_action = "Re-Execute BATCHPREVIEW to identify and create required tablespaces."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
   ...
   IF (validate(drrr_responsefile_in_use,0)=0)
    IF ((dm2_install_schema->process_option="CLIN COPY")
     ...
     SET drer_email_det->status = "PAUSED"
     SET drer_email_det->status_dt_tm = cnvtdatetime(curdate,curtime3)
     SET drer_email_det->step = "Tablespace Prompt"
     SET drer_email_det->email_level = 1
     SET drer_email_det->logfile = dm_err->logfile
     SET drer_email_det->err_ind = dm_err->err_ind
     SET drer_email_det->eproc = dm_err->eproc
     SET drer_email_det->emsg = dm_err->emsg
     SET drer_email_det->user_action = dm_err->user_action
     CALL drer_add_body_text(concat("Tablespace maintenance menu was displayed at ",format(
        drer_email_det->status_dt_tm,";;q")),1)
     CALL drer_add_body_text(concat("User Action : Return to dm2_domain_maint main session and ",
       "choose an option to fulfill the tablespace requirements."),0)
     CALL drer_add_body_text(concat("Log file name is ccluserdir: ",drer_email_det->logfile),0)
     IF (drer_compose_email(null)=1)
      CALL drer_send_email(drer_email_det->subject,drer_email_det->file_name,drer_email_det->
       email_level)
     ENDIF
   ...
   IF ((dm_err->err_ind=1)
    AND dtd_quit_menu_ind=0)
    GO TO exit_script
   ELSEIF ((dm_err->err_ind=1)
    AND dtd_quit_menu_ind=1)
    IF ((dm2_install_schema->process_option != "PREVIEW"))
     GO TO exit_script
    ELSEIF ((dm2_install_schema->process_option="PREVIEW"))
     SET dm_err->err_ind = 0
     SET dm_err->emsg = ""
     SET dm_err->eproc = "User quit from DM2_DOMAIN_MAINT in PREVIEW mode, continue processing"
     CALL disp_msg(" ",dm_err->logfile,0)
     GO TO exit_script
    ENDIF
   ENDIF
  ...
  IF ((ddtsp->nonstd_tgt_ind=1))
   IF (dtr_rpt_nonstd_tspace(dtd_exist_rpt_filename,0)=0)
    GO TO exit_script
   ENDIF
   IF ((dir_ui_misc->dm_process_event_id > 0))
    IF (dtd_add_rpt_to_event_dtl(concat("TSPACE_NEEDS:",dtd_ui_install_event,":NONSTAND"),
  ...
  IF (dtd_exist_needs_space > 0)
   IF ((dir_ui_misc->background_ind=0))
    IF (validate(drrr_responsefile_in_use,0)=1)
     SET dm_err->eproc = concat(
      "Using response file - bypassing display of Space Needs Report by Disk Group.  ",
      "Report File may be found in:  ",dtd_exist_rpt_filename)
     CALL disp_msg(" ",dm_err->logfile,0)
     IF ((drer_email_list->email_cnt > 0))
      SET drer_email_det->msgtype = "PROGRESS"
      SET drer_email_det->status = "REPORT"
      SET drer_email_det->status_dt_tm = cnvtdatetime(curdate,curtime3)
      SET drer_email_det->step = "Tablespace needs report for diskgroups"
      SET drer_email_det->email_level = 1
      SET drer_email_det->logfile = dm_err->logfile
      SET drer_email_det->err_ind = dm_err->err_ind
      SET drer_email_det->eproc = dm_err->eproc
      SET drer_email_det->emsg = dm_err->emsg
      SET drer_email_det->user_action = dm_err->user_action
      SET drer_email_det->attachment = dtd_exist_rpt_filename
      CALL drer_add_body_text(concat("Tablespace needs report for diskgroups was generated at ",
        format(drer_email_det->status_dt_tm,";;q")),1)
      CALL drer_add_body_text(concat("Report file name : ",dtd_exist_rpt_filename),0)
      IF (drer_compose_email(null)=1)
       CALL drer_send_email(drer_email_det->subject,drer_email_det->file_name,drer_email_det->
        email_level)
      ENDIF
    ...
    ELSE
     IF ((dm2_install_schema->process_option="CLIN COPY")
      ...
      SET drer_email_det->emsg = dm_err->emsg
      SET drer_email_det->user_action = dm_err->user_action
      CALL drer_add_body_text(concat("Tablespace needs report for diskgroups was displayed at ",
        format(drer_email_det->status_dt_tm,";;q")),1)
      CALL drer_add_body_text(concat("User Action : Return to dm2_domain_maint main session and ",
        "review the tablespace needs report displayed on the screen.  Press <enter> to continue",
        " when there is enough free space for disk groups listed."),0)
      CALL drer_add_body_text(concat("Report file name is ccluserdir: ",dtd_exist_rpt_filename),0)
      IF (drer_compose_email(null)=1)
       CALL drer_send_email(drer_email_det->subject,drer_email_det->file_name,drer_email_det->
        email_level)
      ENDIF
   ...
   IF ((dir_ui_misc->background_ind=1)
    AND (dir_ui_misc->auto_install_ind=0))
    IF (dtd_ui_tspace_pause(abs(cnvtreal(tgtsch->alpha_feature_nbr)),dir_ui_misc->dm_process_event_id,
     dtd_ui_syspause_event,dtd_exist_rpt_filename,dtd_ui_install_event)=0)
     GO TO exit_script
    ENDIF
   ENDIF
  ENDIF
  IF (dtd_exist_needs_space > 0)
   WHILE ( NOT (dtd_answer_ret IN ("C", "Q"))
    ...
    IF (dtd_post_prompt(dtd_answer_ret)=0)
     RETURN(0)
    ENDIF
    CASE (dtd_answer_ret)
     OF "V":
     ...
     OF "Q":
      IF ((dm2_install_schema->process_option="PREVIEW"))
       SET dm_err->eproc = "User Chose to Quit from Tablespace Maintenance Prompt in PREVIEW mode."
       CALL disp_msg("",dm_err->logfile,0)
      ELSE
 ...
 IF ((dm2_install_schema->process_option != "INHOUSE")
  ...
  IF ((dm2_sys_misc->cur_db_os != dm2_sys_misc->cur_os))
   SET dtd_database_diff_os = 1
  ENDIF
  IF (dir_find_data_file(dtd_file_found)=0)
   GO TO exit_script
  ENDIF
  IF (dtd_file_found=0)
   SET dtd_database_remote = 1
  ENDIF
  IF ((dtr_tspace_misc->gen_id > 0))
   IF (dtd_auto_success_obj_needs(null)=0)
    GO TO exit_script
   ENDIF
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echo(build("dtd_space_needs_ind =",dtd_space_needs_ind))
  ENDIF
  IF ((dm2_install_schema->process_option="PREVIEW"))
   IF (dtd_get_tmp_tspace_needs(null)=0)
    RETURN(0)
  ...
  ;end select
  IF (check_error("Logging tablespace needs to the logfile") != 0)
   SET dm_err->err_ind = 1
   GO TO exit_script
  ENDIF
  IF (((dtd_database_remote=1) OR (dtd_database_diff_os=1))
   ...
   IF ((drr_clin_copy_data->process != "MIGRATION")
    ...
    CALL drer_add_body_text(concat(
      "Prompt for user to enter remote tablespace maintenance mode was displayed at ",format(
       drer_email_det->status_dt_tm,";;q")),1)
    CALL drer_add_body_text(concat("User Action : Return to dm2_domain_maint main session and ",
      "choose an option to fulfill tablespace requirements on remote database mode."),0)
    CALL drer_add_body_text(concat("Log file name is ccluserdir: ",drer_email_det->logfile),0)
    IF (drer_compose_email(null)=1)
     CALL drer_send_email(drer_email_det->subject,drer_email_det->file_name,drer_email_det->
      email_level)
   ...
   CALL clear(1,1)
   CALL box(2,1,22,132)
   CALL text(3,2,'About to run Tablespace Menu in "Remote" Mode')
   CALL text(4,2,
    "The database data files and disk information are not available on this node. In order ")
   CALL text(5,2,
    'to facilitate this process, tablespace menu will run in "Remote" mode. You will be asked ')
   CALL text(6,2,concat("to copy ",evaluate(dm2_sys_misc->cur_db_os,"AXP","COM","KSH"),
     " file to the database node; "))
   CALL text(7,2,concat("execute the ",evaluate(dm2_sys_misc->cur_db_os,"AXP","COM","KSH"),
     " file and then copy back several output files in order to complete "))
   CALL text(8,2,"tablespace processing. If this mode was not expected, ")
   CALL text(9,2,"please restart the process...")
   CALL text(15,2,
    ^Enter 'R' to continue tablespace menu in "Remote" mode or 'M' for "Manual" mode or 'Q' to quit:^
    )
   CALL accept(15,98,"p;cu","R"
    WHERE curaccept IN ("R", "Q", "M"))
   IF (curaccept="Q")
    SET dm_err->emsg = "User chose to quit from remote node prompt. Cannot continue"
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
   IF (curaccept="R")
    ...
    GO TO exit_script
   ELSEIF (curaccept="M")
    SET dtd_answer_ret = "M"
   ENDIF
  ELSEIF (((dtd_database_diff_os=1) OR (dtd_database_remote=1))
   AND (dm2_install_schema->process_option != "CLIN COPY")
   AND validate(dm2_use_tspace_anyway,1) != 722)
   SET dtd_answer_ret = "M"
  ELSEIF ((dm2_install_schema->process_option != "CLIN COPY")
   AND validate(dm2_use_tspace_anyway,1) != 722
   AND (dm2_sys_misc->cur_db_os IN ("HPX", "AIX")))
   SET dtd_answer_ret = "M"
  ELSEIF ((ddtsp->nonstd_tgt_ind=1))
   SET dtd_answer_ret = "M"
  ENDIF
  IF ((dir_ui_misc->background_ind=1))
   SET dtd_answer_ret = "UI"
  ENDIF
  IF (dtd_answer_ret="")
   IF ((dm2_install_schema->process_option="CLIN COPY")
    AND (drr_clin_copy_data->process != "MIGRATION")
    ...
    SET drer_email_det->user_action = dm_err->user_action
    CALL drer_add_body_text(concat(
      "Prompt for user to enter tablespace maintenance mode was displayed at ",format(drer_email_det
       ->status_dt_tm,";;q")),1)
    CALL drer_add_body_text(concat("User Action : Return to dm2_domain_maint main session and ",
      "choose an option to fulfill tablespace requirements."),0)
    CALL drer_add_body_text(concat("Log file name is ccluserdir: ",drer_email_det->logfile),0)
    IF (drer_compose_email(null)=1)
     CALL drer_send_email(drer_email_det->subject,drer_email_det->file_name,drer_email_det->
      email_level)
    ...
    CALL drer_reset_pre_err(null)
   ENDIF
   IF (dtd_mode_prompt(dtd_answer_ret)=0)
    GO TO exit_script
   ENDIF
  ENDIF
  IF (dtd_answer_ret="Q")
   IF ((dm2_install_schema->process_option="PREVIEW"))
    SET dm_err->err_ind = 0
    SET dm_err->emsg = ""
    SET dm_err->eproc = "User quit from Mode Prompt in PREVIEW mode, continue processing"
    CALL disp_msg(" ",dm_err->logfile,0)
    GO TO exit_script
   ELSE
  ...
  ELSEIF (dtd_answer_ret="A")
   ...
   ELSEIF ((dm_err->err_ind=1)
    ...
    ENDIF
   ENDIF
  ELSEIF (dtd_answer_ret="M")
   IF (dtd_rpt_filename="")
    IF (get_unique_file("dm2_tspace_needs",".rpt")=0)
     GO TO exit_script
    ELSE
     SET dtd_rpt_filename = dm_err->unique_fname
    ENDIF
   ENDIF
   IF (dtd_pop_current_size(null)=0)
    GO TO exit_script
   ENDIF
   WHILE (dtd_space_needs_ind=1)
    IF (dtd_manual_process_tspace(null)=0)
     GO TO exit_script
    ENDIF
    IF ((dm_err->debug_flag > 0))
     CALL echo(build("dtd_space_needs_ind =",dtd_space_needs_ind))
  ...
  ELSEIF (dtd_answer_ret="UI")
   ...
   IF (dtd_pop_current_size(null)=0)
    GO TO exit_script
   ENDIF
   IF (dtd_disp_tspace_rpt(null)=0)
    GO TO exit_script
   ENDIF
   IF (dtd_ui_tspace_pause(abs(cnvtreal(tgtsch->alpha_feature_nbr)),dir_ui_misc->dm_process_event_id,
    dtd_ui_syspause_event,dtd_rpt_filename,dtd_ui_install_event)=0)
    GO TO exit_script
   ENDIF
  ENDIF
 ENDIF
 IF ((dm2_install_schema->process_option != "CLIN COPY")
  AND (tgtsch->diff_ind=1))
  FOR (dtd_cnt = 1 TO tgtsch->tbl_cnt)
   IF ((tgtsch->tbl[dtd_cnt].new_ind=1)
    AND (tgtsch->tbl[dtd_cnt].dext_mgmt="L"))
    SET tgtsch->tbl[dtd_cnt].next_ext = 0
   ENDIF
   IF ((tgtsch->tbl[dtd_cnt].iext_mgmt="L"))
    FOR (dtd_nidx_cnt = 1 TO tgtsch->tbl[dtd_cnt].ind_cnt)
      IF ((tgtsch->tbl[dtd_cnt].ind[dtd_nidx_cnt].build_ind=1))
       SET tgtsch->tbl[dtd_cnt].ind[dtd_nidx_cnt].next_ext = 0
      ENDIF
    ENDFOR
   ENDIF
  ENDFOR
 ENDIF
 IF ((dm2_install_schema->process_option != "INHOUSE"))
  IF (dtd_set_tspace_quotas(null)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 GO TO exit_script
 SUBROUTINE dtd_add_rpt_to_event_dtl(dard_type_in,dard_rpt_in)
   ...
   SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
   SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
   SET dm2_process_event_rs->install_plan_id = abs(cnvtreal(tgtsch->alpha_feature_nbr))
   IF (dm2_process_log_row(dpl_package_install,dpl_report,dpl_no_prev_id,0)=0)
    RETURN(0)
   ENDIF
   FREE DEFINE rtl
   FREE SET dard_file_loc
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_get_tmp_tspace_needs(null)
   DECLARE dgttn_max_icnt = i4 WITH protect, noconstant(0)
   DECLARE dgttn_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgttn_ndx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Figure Temporary tablespace needs."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dm_err->eproc = "Get temporary tablespace name"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM user_users u
    DETAIL
     rtspace->temp_tspace_name = u.temporary_tablespace
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Get temporary tablespace file type"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM dba_temp_files d
    WHERE (d.tablespace_name=rtspace->temp_tspace_name)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual > 0)
    SET rtspace->temp_tspace_file_type = "tempfile"
   ELSE
    SET rtspace->temp_tspace_file_type = "datafile"
   ENDIF
   SET dm_err->eproc = "Get total size of temp tablespace"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT
    IF ((rtspace->temp_tspace_file_type="tempfile"))INTO "nl:"
     ttl_bytes = sum(((d.bytes/ 1024.0)/ 1024.0))
     FROM dba_temp_files d
     WHERE (d.tablespace_name=rtspace->temp_tspace_name)
    ELSE INTO "nl:"
     ttl_bytes = sum(((d.bytes/ 1024.0)/ 1024.0))
    ...
    ENDIF
    DETAIL
     rtspace->temp_tspace_ttl_mb = dm2ceil(ttl_bytes)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Get temp tablespace reserved percent and space ratio"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SET rtspace->temp_tspace_reserved_pct = 25
   SET rtspace->temp_tspace_ratio = 1.5
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="DM2_TEMP_SPACE"
     AND d.info_name IN ("RESERVED_PCT", "RATIO")
    DETAIL
     IF (d.info_name="RATIO")
      rtspace->temp_tspace_ratio = d.info_number
     ELSEIF (d.info_name="RESERVED_PCT")
      rtspace->temp_tspace_reserved_pct = d.info_number
     ENDIF
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    ...
    RETURN(0)
   ENDIF
   SET rtspace->temp_tspace_reserved_mb = dm2ceil(((rtspace->temp_tspace_ttl_mb * rtspace->
    temp_tspace_reserved_pct)/ 100))
   SET dm_err->eproc = "Get max number of indexes for tables in target schema"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    ...
    PLAN (d)
    DETAIL
     IF ((tgtsch->tbl[d.seq].ind_cnt > dgttn_max_icnt))
      dgttn_max_icnt = tgtsch->tbl[d.seq].ind_cnt
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Get top 10 largest indexes"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(tgtsch->tbl_cnt)),
     (dummyt d2  WITH seq = value(dgttn_max_icnt))
    PLAN (d)
     JOIN (d2
     WHERE (d2.seq <= tgtsch->tbl[d.seq].ind_cnt)
      AND (tgtsch->tbl[d.seq].ind[d2.seq].build_ind=1)
      AND (tgtsch->tbl[d.seq].ind[d2.seq].size > 0))
    ORDER BY tgtsch->tbl[d.seq].ind[d2.seq].size DESC
    DETAIL
     IF (dgttn_cnt < 10)
      IF (((size(rtspace->temp_tspace_indexlist,5)=0) OR (size(rtspace->temp_tspace_indexlist,5) > 0
       AND locateval(dgttn_ndx,1,size(rtspace->temp_tspace_indexlist,5),tgtsch->tbl[d.seq].tbl_name,
       rtspace->temp_tspace_indexlist[dgttn_ndx].tbl_name)=0)) )
       dgttn_cnt = (dgttn_cnt+ 1), stat = alterlist(rtspace->temp_tspace_indexlist,dgttn_cnt),
       rtspace->temp_tspace_indexlist[dgttn_cnt].tbl_name = tgtsch->tbl[d.seq].tbl_name,
       rtspace->temp_tspace_indexlist[dgttn_cnt].ind_name = tgtsch->tbl[d.seq].ind[d2.seq].ind_name,
       rtspace->temp_tspace_indexlist[dgttn_cnt].size_mb = dm2ceil(((tgtsch->tbl[d.seq].ind[d2.seq].
        size/ 1024.0)/ 1024.0)), rtspace->temp_tspace_ttl_needed_mb = (rtspace->
       temp_tspace_ttl_needed_mb+ (rtspace->temp_tspace_indexlist[dgttn_cnt].size_mb * rtspace->
       temp_tspace_ratio))
      ENDIF
     ENDIF
    WITH nocounter
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_post_prompt(dpp_answer_out)
   CALL clear(1,1)
   SET message = window
   ...
   CALL text(12,1,"(V)iew Tablespace Needs, (C)ontinue, (Q)uit:")
   IF (dtd_disp_warning > 0)
    IF ((ddtsp->nonstd_tgt_ind=1))
     CALL text(5,1,"IMPORTANT:  Unsupported tablespace configurations detected in this environment!")
     CALL text(7,1,"This process will not verify that needed space has been added.")
     CALL text(8,1,"Ensure that you have enough tablespace before continuing. ")
    ELSE
     CALL text(5,1,"WARNING:  Bypassing Tablespace Maintenance!")
     CALL text(7,1,
      "User has requested to continue install without completing required tablespace needs.")
     CALL text(8,1,"This may result in schema failures due to lack of adequate tablespace. ")
    ENDIF
   ENDIF
   CALL accept(12,46,"p;cu","V"
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_disp_temp_tspace_rpt(null)
   DECLARE ddttr_rpt_filename = vc WITH protect, noconstant("")
   DECLARE ddttr_cnt = i4 WITH protect, noconstant(0)
   DECLARE ddttr_space_avail = i4 WITH protect, noconstant(0)
   DECLARE ddttr_nbr = i4 WITH protect, noconstant(0)
   IF (get_unique_file("dm2_tmptsp_needs",".rpt")=0)
    RETURN(0)
   ELSE
    SET ddttr_rpt_filename = dm_err->unique_fname
   ENDIF
   SET dm_err->eproc = concat("Write report data to ",ddttr_rpt_filename)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(ddttr_rpt_filename)
    FROM dual
    HEAD REPORT
     CALL center("Temporary Tablespace Installation Requirements report",1,132), row + 2,
     CALL print(
     "This report provides estimates for additional temporary tablespace needed to support the expected schema operations."
     ),
     row + 1,
     CALL print(concat("For example, CREATE INDEX schema operations require temporary sort space ",
      " which can cause potential points of schema failure.")), row + 1,
     CALL print("Inadequate temporary tablespace can result in schema operations being either:"), row
      + 1,
     CALL print("1)  Suspended (when dba_resumable is enabled)"),
     row + 1,
     CALL print(
     "2)  Failed  (when dba_resumable is disabled or is allowed to exceed the time out period)."),
     row + 2,
     CALL print(
     "************************************************************************************************************"
     ), row + 1,
     CALL print(concat("Current Temporary Tablespace Information for ",currdbuser)),
     row + 1,
     CALL print(
     "************************************************************************************************************"
     ), row + 2,
     CALL print("                                  Current           Reserved          Current      "
     ), row + 1,
     CALL print("Temporary                         Size              Size              Available    "
     ),
     row + 1,
     CALL print("Tablespace Name                   (MBYTES)          (MBYTES)          (MBYTES)   "),
     row + 1,
     CALL print(
     "_______________                   ______________    ______________    __________       "), row
      + 1, col 0,
     rtspace->temp_tspace_name, col 37, rtspace->temp_tspace_ttl_mb,
     col 55, rtspace->temp_tspace_reserved_mb, ddttr_nbr = (rtspace->temp_tspace_ttl_mb - rtspace->
     temp_tspace_reserved_mb),
     col 69, ddttr_nbr, row + 2,
     CALL print(
     "************************************************************************************************************"
     ), row + 1,
     CALL print(
     "Estimated Temp Tablespace Needs based on Current Temp Tablespace Available and expected schema operations"
     ),
     row + 1,
     CALL print(
     "************************************************************************************************************"
     ), row + 2,
     CALL print("                                                    |------- ESTIMATED --------|"),
     row + 1,
     CALL print("Planned                                                                         "),
     row + 1,
     CALL print("Installation                                        Object            Additional"),
     row + 1,
     CALL print("Scheduler                                           Temp Space        Temp Space"),
     row + 1,
     CALL print("Runners      Index Name                             Usage (MB)        Needs (MB)"),
     row + 1,
     CALL print("-----------  ------------------------------         ---------         ------------ "
     ), row + 1,
     ddttr_space_avail = (rtspace->temp_tspace_ttl_mb - rtspace->temp_tspace_reserved_mb)
     FOR (ddttr_cnt = 1 TO size(rtspace->temp_tspace_indexlist,5))
       ddttr_space_avail = (ddttr_space_avail - (rtspace->temp_tspace_indexlist[ddttr_cnt].size_mb *
       rtspace->temp_tspace_ratio)), col 0, ddttr_cnt,
       col 14, rtspace->temp_tspace_indexlist[ddttr_cnt].ind_name, ddttr_nbr = (rtspace->
       temp_tspace_indexlist[ddttr_cnt].size_mb * rtspace->temp_tspace_ratio),
       col 50, ddttr_nbr
       IF (ddttr_space_avail < 0)
        ddttr_nbr = abs(ddttr_space_avail), col 69, ddttr_nbr
       ELSE
        col 77, "---"
       ENDIF
       row + 1
   ...
   ENDIF
   IF ((dir_ui_misc->dm_process_event_id > 0))
    IF (dtd_add_rpt_to_event_dtl(concat("TSPACE_NEEDS:",dtd_ui_install_event),ddttr_rpt_filename)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((dir_ui_misc->background_ind=0))
    IF (dm2_disp_file(ddttr_rpt_filename,concat("Report File may be found in CCLUSERDIR:",
      ddttr_rpt_filename))=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((dir_ui_misc->dm_process_event_id > 0))
    IF (drr_assign_file_to_installs("RPT:TEMP TABLESPACE NEEDS",ddttr_rpt_filename,dir_ui_misc->
     dm_process_event_id)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_disp_tspace_rpt(null)
   DECLARE ddt_type_disp = i2 WITH protect, noconstant(0)
   DECLARE ddt_ndx = i4 WITH protect, noconstant(0)
   DECLARE ddt_type_full = vc WITH protect, noconstant(" ")
   DECLARE ddt_bytes = f8 WITH protect, noconstant(0.0)
   DECLARE ddt_temp_ind = i2 WITH protect, noconstant(0)
   DECLARE ddt_locndx = i4 WITH protect, noconstant(0)
   IF ((dm2_install_schema->process_option="CLIN COPY"))
    IF (locateval(ddt_ndx,1,size(rtspace->qual,5),"L",rtspace->qual[ddt_ndx].ext_mgmt) > 0)
     SET ddt_type_disp = 1
    ENDIF
    IF (locateval(ddt_ndx,1,size(rtspace->qual,5),"D",rtspace->qual[ddt_ndx].ext_mgmt) > 0)
     IF (ddt_type_disp=1)
      SET ddt_type_disp = 3
     ELSE
      SET ddt_type_disp = 2
   ...
   ELSE
    ...
    ENDIF
   ENDIF
   IF (locateval(ddt_locndx,1,size(rtspace->qual,5),1,rtspace->qual[ddt_locndx].temp_ind) > 0)
    SET ddt_temp_ind = 1
   ENDIF
   SELECT INTO value(dtd_rpt_filename)
    FROM (dummyt d  WITH seq = size(rtspace->qual,5))
    PLAN (d
     WHERE d.seq > 0
    ...
    HEAD REPORT
     row + 1,
     CALL center("Tablespace Maintenance Report",1,80), row + 2,
     col 0,
     "The following report contains tablespaces and their space needs that must be completed before continuing.",
     row + 1,
     col 0,
     "Failure to complete this work may result in schema failures due to lack of adequate tablespace. ",
     row + 1,
     col 0,
     "Example SQL commands can be found following the 'Interpreting the Tablespace Maintenance Report' section.",
     row + 2
     IF ((dm2_rdbms_version->level1 >= 10)
      AND (dm2_sys_misc->cur_db_os="AIX"))
      col 0, "IMPORTANT:  Logical volumes must be created in big volume groups with the zero", row +
      1,
      col 3, "offset flag specified (see example mklv below).", row + 2
     ENDIF
     IF ((ddtsp->nonstd_tgt_ind=1))
      col 0, "****IMPORTANT****", row + 1,
      col 0,
      "Unsupported tablespace configurations have been detected in this environment, as denoted by an * below.",
      row + 1,
      col 0,
      'As a result,"Space Needed" values stated below have not factored in current free space of these',
      row + 1,
      col 0,
      "existing tablespaces. The installation process will not verify that space requirements have ",
      row + 1,
      col 0, "been met for these tablespaces before continuing with schema changes.", row + 2
     ENDIF
     col 0, "Tablespace Needs:      ", row + 1
     IF ((ddtsp->nonstd_tgt_ind=1))
      col 2, "* denotes tablespaces with unsupported configuration", row + 2
     ENDIF
     IF (ddt_temp_ind=1)
      col 2, "** denotes temporary tablespaces", row + 2
     ENDIF
     IF ((dm2_install_schema->process_option="CLIN COPY"))
      col 0, "                                  Current           Space                ", row + 1,
      col 0, "                                  Total Size        Needed            Extent   ", row
       + 1,
      col 0, "Tablespace Name                   (MBYTES)          (MBYTES)          Management   ",
      row + 1,
      col 0,
      "_______________                   ______________    ______________    __________       ", row
       + 1
     ELSEIF ((ddtsp->nonstd_tgt_ind=1))
      col 0, "                                  Current           Space         ", row + 1,
      col 0, "                                  Total Size        Needed        ", row + 1,
      col 0, "Tablespace Name                   (MBYTES)          (MBYTES)      ", row + 1,
      col 0, "_______________                   ______________    ______________", row + 1
     ELSE
      col 0, "                                  Current           Space                Minimum ", row
       + 1,
      col 0,
    ...
    DETAIL
     IF ((dm2_install_schema->process_option="CLIN COPY"))
      ddt_bytes = round(((rtspace->qual[d.seq].bytes_needed/ 1024.0)/ 1024.0),2), ddt_type_full =
      evaluate(rtspace->qual[d.seq].ext_mgmt,"L","LOCAL","DICTIONARY"), col 0,
      rtspace->qual[d.seq].tspace_name
      IF ((rtspace->qual[d.seq].new_ind=1))
       col 34, "NEW"
      ELSE
       ddt_bytes = round(((rtspace->qual[d.seq].cur_bytes_allocated/ 1024.0)/ 1024.0),2), col 34,
       ddt_bytes
      ENDIF
      ddt_bytes = round(((rtspace->qual[d.seq].bytes_needed/ 1024.0)/ 1024.0),2), col 52, ddt_bytes,
      col 70, ddt_type_full
     ELSE
      IF ((ddtsp->nonstd_tgt_ind=1))
       ddt_ndx = locateval(ddt_ndx,1,size(ddtsp->qual,5),rtspace->qual[d.seq].tspace_name,ddtsp->
        qual[ddt_ndx].tspace_name)
       IF (ddt_ndx > 0
        AND (ddtsp->qual[ddt_ndx].nonstd_tgt_ind=1))
        col 0, "* ", rtspace->qual[d.seq].tspace_name
       ELSE
        col 1, rtspace->qual[d.seq].tspace_name
       ENDIF
      ELSE
       col 1, rtspace->qual[d.seq].tspace_name
      ENDIF
      IF ((rtspace->qual[d.seq].temp_ind=1))
       col 0, "** ", rtspace->qual[d.seq].tspace_name
      ENDIF
      IF ((rtspace->qual[d.seq].new_ind=1))
       col 34, "NEW"
      ...
      IF ((ddtsp->nonstd_tgt_ind=0))
       ddt_bytes = round(((rtspace->qual[d.seq].chunk_size/ 1024.0)/ 1024.0),2), col 70, ddt_bytes,
       col 96, rtspace->qual[d.seq].chunks_needed
      ENDIF
     ENDIF
     row + 1
    FOOT REPORT
     row + 2, col 0, "Interpreting the Tablespace Maintenance Report:",
     row + 2, col 0, "Tablespace Name          Tablespace that needs additional space.",
     row + 2
     IF ((dm2_install_schema->process_option="CLIN COPY"))
      col 0,
      "Current Total Size       The size of existing tablespace or 'NEW' if tablespace does not exist.  ",
      row + 2,
      col 0,
      "Space Needed             Minimum amount of space that is required to complete schema changes (free space",
      row + 1,
      col 0,
      "                         in tablespace is considered when calculating this value).     ", row
       + 2,
      col 0,
      "Extent Management        The extent management setting that should be used when creating the tablespace.",
      row + 2
     ELSE
      IF ((ddtsp->nonstd_tgt_ind=0))
       col 0,
       '"Chunk"                  A chunk is a block of contiguous space within a tablespace.  ', row
        + 2
      ENDIF
      col 0,
      "Current Total Size       The size of existing tablespace or 'NEW' if tablespace does not exist.  ",
      ...
      IF ((ddtsp->nonstd_tgt_ind=0))
       ...
       "                         in tablespace is considered when calculating this value).     ", row
        + 2,
       col 0,
       "Minimum Chunk Size       The minimum data file size that can be added to an existing or new tablespace or",
       row + 1,
       col 0, "                         a minimum size that an existing data file can be extended.",
       row + 2,
       col 0,
       "Number of Chunks Needed  The minimum number of chunks needed to satisfy tablespace needs. ",
       row + 2
      ELSE
       col 0,
       "Space Needed             Minimum amount of space that is required to complete schema changes (free space",
       row + 1,
       col 0,
       "                         in tablespace is NOT considered when calculating this value).     ",
       row + 2
      ENDIF
     ENDIF
     col 0, "Tablespace Maintenance Examples:", row + 2
     IF ((dm2_rdbms_version->level1 >= 10)
      AND (dm2_sys_misc->cur_db_os="AIX"))
      col 0, "Create Logical Volume", row + 2,
      col 0, "$> mklv -y 'dtest_0000099' -w n -t 'raw' -T O bigvg1 10 disk1", row + 2
     ENDIF
     IF ((dm2_install_schema->process_option="CLIN COPY"))
      IF (ddt_type_disp IN (1, 3))
       col 0, "Create New Tablespace (Locally Managed) ", row + 2
       IF ((dm2_sys_misc->cur_db_os="AXP"))
        col 0, "  Sql> create tablespace D_A_SMALL datafile", row + 1,
        col 0, "    'data01:[v500.db_sample]d_a_small_001.dbs' size 15M", row + 1,
        col 0, "     extent management local autoallocate segment space management auto;", row + 2
       ELSEIF ((dm2_sys_misc->cur_db_os="AIX"))
        col 0, "  Sql> create tablespace D_A_SMALL datafile", row + 1,
        col 0, "    '/dev/rdsample_0000001' size 15M  ", row + 1,
        col 0, "    extent management local autoallocate segment space management auto;", row + 2
       ELSE
        col 0, "  Sql> create tablespace D_A_SMALL datafile", row + 1,
        col 0, "    '/dev/vgdb01/rdsample_0000001' size 15M  ", row + 1,
        col 0, "    extent management local autoallocate segment space management auto;", row + 2
       ENDIF
      ENDIF
      IF (ddt_type_disp IN (2, 3))
       ...
       ELSEIF ((dm2_sys_misc->cur_db_os="AIX"))
        col 0, "Create New Tablespace (Dictionary Managed)  ", row + 2,
        col 0, "Sql> create tablespace D_A_SMALL datafile", row + 1,
        col 0, "    '/dev/rdsample_0000001' size 15M  ", row + 1,
        col 0, "    extent management dictionary; ", row + 2
       ELSE
      ...
      IF (locateval(ddt_ndx,1,size(rtspace->qual,5),0,rtspace->qual[ddt_ndx].new_ind) > 0)
       ...
       ELSE
        col 0, "Add Data File to Existing Tablespace    ", row + 2,
        col 0, "  Sql> alter tablespace D_A_SMALL add datafile", row + 1,
        col 0, "    '/dev/vgdb01/rdtest_0000099' size 15M;", row + 2,
        col 0, "Extend an Existing Tablespace   ", row + 2,
        col 0, "  Sql> alter database test datafile ", row + 1,
        col 0, "    '/dev/vgdb01/rdtest_0000099' resize 31M;", row + 2
       ENDIF
      ENDIF
     ELSE
      IF (ddt_type_disp IN (2, 3))
      ...
      IF (ddt_type_disp IN (1, 3))
       IF ((dm2_db_options->new_tspace_type="L"))
        ...
        ELSE
         col 0, "Create New Tablespace  ", row + 2,
         col 0, "  Sql> create tablespace D_A_SMALL datafile", row + 1,
         col 0, "    '/dev/vgdb01/rdtest_0000099' size 15M", row + 1,
         col 0, "     extent management local autoallocate segment space management auto;", row + 2
        ENDIF
       ELSE
        IF ((dm2_sys_misc->cur_db_os="AXP"))
      ...
      ENDIF
      IF (ddt_temp_ind=1)
       col 0, "Create New Temporary Tablespace  ", row + 2,
       col 0, "  Sql> create temporary tablespace CER_TEMP", row + 1,
       col 0, " tempfile '/dev/example_path/cer_temp' size 100M;     ", row + 1
      ENDIF
     ENDIF
    WITH nocounter, format = variable, formfeed = none,
     maxcol = 512
   ;end select
   IF ((ddtsp->nonstd_tgt_ind=1))
    IF (dtr_rpt_nonstd_tspace(dtd_rpt_filename,0)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((dir_ui_misc->dm_process_event_id > 0))
    IF (dtd_add_rpt_to_event_dtl(concat("TSPACE_NEEDS:",dtd_ui_install_event),dtd_rpt_filename)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((dir_ui_misc->dm_process_event_id > 0))
    IF (drr_assign_file_to_installs("RPT:TABLESPACE NEEDS",dtd_rpt_filename,dir_ui_misc->
     dm_process_event_id)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((dir_ui_misc->background_ind=0))
    IF (dm2_disp_file(dtd_rpt_filename,concat("Report File may be found in CCLUSERDIR:",
      dtd_rpt_filename))=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_manual_process_tspace(null)
   DECLARE dmpt_answer_ret = vc WITH protect, noconstant("")
   DECLARE dmpt_ndx = i4 WITH protect, noconstant(0)
   IF (dtd_disp_rpt_ind=1)
    IF (dtd_disp_tspace_rpt(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (dtd_post_prompt(dmpt_answer_ret)=0)
    RETURN(0)
   ENDIF
   CASE (dmpt_answer_ret)
    OF "C":
     IF (locateval(dmpt_ndx,1,size(rtspace->qual,5),1,rtspace->qual[dmpt_ndx].new_ind)=0)
      SET dtd_disp_warning = (dtd_disp_warning+ 1)
     ENDIF
     SET dtd_space_needs_ind = 0
     SET dtr_tspace_misc->recalc_space_needs = 1
     IF ((dm2_install_schema->process_option="CLIN COPY"))
      IF (dtd_calc_tsp_needs_for_clin_copy(null)=0)
       RETURN(0)
      ENDIF
     ELSE
      ...
      ENDIF
     ENDIF
     SET dtd_disp_rpt_ind = 1
     IF (dtd_space_needs_ind=1)
      IF (dtd_pop_current_size(null)=0)
       GO TO exit_script
      ENDIF
      IF (dtd_disp_warning > 1)
       SET dtd_space_needs_ind = 0
       SET dm_err->eproc = "User has elected to not satisfy tablespace requirements."
       CALL disp_msg("",dm_err->logfile,0)
      ENDIF
     ENDIF
    OF "V":
     SET dtd_disp_warning = 0
     IF (dm2_disp_file(dtd_rpt_filename,concat("Report File may be found in CCLUSERDIR:",
       dtd_rpt_filename))=0)
      RETURN(0)
     ENDIF
     SET dtd_disp_rpt_ind = 0
    OF "Q":
     IF ((dm2_install_schema->process_option="PREVIEW"))
      SET dm_err->eproc =
      "User Chose to Quit from Tablespace Needs Prompt during Tablespace Assessment in PREVIEW mode."
      CALL disp_msg("",dm_err->logfile,0)
      SET dtd_space_needs_ind = 0
     ELSE
      SET dm_err->emsg =
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_mode_prompt(dmp_answer_out)
   CALL clear(1,1)
   SET message = window
   ...
   CALL accept(10,31,"p;cu"," "
    WHERE curaccept IN ("A", "M", "Q"))
   SET dmp_answer_out = curaccept
   CALL clear(1,1)
   SET message = nowindow
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dctn_check_status(null)
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   IF (check_error("checking status of tablespace opertations") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (curqual > 0)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dtd_pop_current_size(null)
  IF (size(rtspace->qual,5) > 0)
   SET dm_err->eproc = "Getting total size for existing tablespaces..."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    d.tablespace_name, total_bytes = sum(d.bytes)
    FROM dm2_dba_data_files d,
     (dummyt t  WITH seq = size(rtspace->qual,5))
    PLAN (t)
     JOIN (d
     WHERE (rtspace->qual[t.seq].tspace_name=d.tablespace_name)
      AND (rtspace->qual[t.seq].new_ind=0))
    GROUP BY d.tablespace_name
    DETAIL
     rtspace->qual[t.seq].cur_bytes_allocated = total_bytes
     IF ((dm_err->debug_flag > 5))
      CALL echo(build(d.tablespace_name,":",total_bytes))
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
  ...
  RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_calc_tsp_needs_for_clin_copy(null)
   DECLARE ddd_idx = i4 WITH protect, noconstant(0)
   DECLARE ddd_spot = i4 WITH protect, noconstant(0)
   DECLARE ddd_spot_end = i4 WITH protect, noconstant(0)
   DECLARE ddd_badtsp_filename = vc WITH protect, noconstant("")
   DECLARE ddd_accept = vc WITH protect, noconstant("")
   DECLARE dctnfcc_cntr = i4 WITH protect, noconstant(0)
   DECLARE dctnfcc_sys_em = vc WITH protect, noconstant("")
   DECLARE dctnfcc_temp_tspace_size = f8 WITH protect, noconstant(5242880.00)
   DECLARE dctnfcc_default_tspace_size = f8 WITH protect, noconstant(1048576.00)
   IF ((dtr_tspace_misc->recalc_space_needs=1))
    SET dm_err->eproc = "Checking if tablespaces have been created with space required..."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     d.tablespace_name, total_bytes = sum(d.bytes)
     FROM dm2_dba_data_files d
     GROUP BY d.tablespace_name
     DETAIL
      ddd_idx = locateval(ddd_idx,1,value(size(rtspace->qual,5)),d.tablespace_name,rtspace->qual[
       ddd_idx].tspace_name)
      IF (ddd_idx > 0)
       IF ((total_bytes < (rtspace->qual[ddd_idx].bytes_needed+ rtspace->qual[ddd_idx].
       cur_bytes_allocated)))
        rtspace->qual[ddd_idx].bytes_needed = ((rtspace->qual[ddd_idx].bytes_needed+ rtspace->qual[
        ddd_idx].cur_bytes_allocated) - total_bytes), rtspace->qual[ddd_idx].new_ind = 0,
        dtd_space_needs_ind = 1
        IF ((dm_err->debug_flag > 0))
         CALL echo(concat("Still need space for:",rtspace->qual[ddd_idx].tspace_name)),
         CALL echo(build("Bytes Needed:",rtspace->qual[ddd_idx].bytes_needed))
        ENDIF
       ELSE
        IF ((dm_err->debug_flag > 0))
         CALL echo(concat("Space satisfied for:",rtspace->qual[ddd_idx].tspace_name)),
         CALL echo(build("TOTAL:",total_bytes,",NEEDED:",rtspace->qual[ddd_idx].bytes_needed))
        ENDIF
        rtspace->qual[ddd_idx].bytes_needed = 0, rtspace->qual[ddd_idx].new_ind = 0
       ENDIF
      ENDIF
     WITH nocounter
    ;end select
    ...
    ENDIF
    IF (dtd_space_needs_ind=0)
     SET dm_err->eproc = "Checking if any tablespaces still need to be created..."
     CALL disp_msg(" ",dm_err->logfile,0)
     SET ddd_idx = locateval(ddd_idx,1,value(size(rtspace->qual,5)),1,rtspace->qual[ddd_idx].new_ind)
     IF (ddd_idx > 0)
      SET dtd_space_needs_ind = 1
      IF ((dm_err->debug_flag > 0))
       CALL echo(concat("Found new tablespace:",rtspace->qual[ddd_idx].tspace_name))
      ENDIF
     ELSE
      IF ((dm_err->debug_flag > 0))
       CALL echo("Did not find any new tablespaces in RTSPACE")
      ENDIF
     ENDIF
    ENDIF
    RETURN(1)
   ...
   CALL dsfi_load_schema_file_defs("tspace")
   IF ((((drr_clin_copy_data->process="RESTORE")
    AND (drr_preserved_tables_data->refresh_ind=1)) OR ((drr_clin_copy_data->process != "RESTORE")))
   )
    ...
    ENDIF
    IF ((dm_err->debug_flag > 2))
     CALL echorecord(dm2_sch_file)
    ENDIF
    SET dm_err->eproc = "Populating rTspace with tablespaces from schema file..."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dmtspace d
     WHERE d.tspace_name != "MISC"
     DETAIL
      ddd_idx = locateval(ddd_idx,1,value(size(rtspace->qual,5)),d.tspace_name,rtspace->qual[ddd_idx]
       .tspace_name)
      IF (ddd_idx=0)
       ddd_cnt = (rtspace->rtspace_cnt+ 1), stat = alterlist(rtspace->qual,ddd_cnt), rtspace->qual[
       ddd_cnt].tspace_name = d.tspace_name,
       rtspace->qual[ddd_cnt].cur_bytes_allocated = 0, rtspace->qual[ddd_cnt].bytes_needed = d
       .bytes_needed, rtspace->qual[ddd_cnt].ext_mgmt = d.ext_mgmt,
       rtspace->rtspace_cnt = ddd_cnt, rtspace->qual[ddd_cnt].new_ind = 1, rtspace->qual[ddd_cnt].
       chunk_size = 0.0,
       rtspace->qual[ddd_cnt].chunks_needed = 0
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     ...
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Closing schema files"
    CALL disp_msg(" ",dm_err->logfile,0)
    IF (close_sch_files(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((drr_preserved_tables_data->refresh_ind=1)
    ...
    IF ((dm2_sys_misc->cur_os IN ("AIX", "LNX", "HPX")))
     SET dm2_sch_file->src_dir_osfmt = drr_clin_copy_data->temp_location
     SET dm2_sch_file->dest_dir_osfmt = build(logical("cer_install"),"/")
    ELSEIF ((dm2_sys_misc->cur_os="AXP"))
     SET dm2_sch_file->src_dir_osfmt = drr_clin_copy_data->temp_location
     SET dm2_sch_file->dest_dir_osfmt = logical("cer_install")
    ENDIF
    SET dm2_sch_file->file_prefix = build("dm2s",drr_clin_copy_data->preserve_sch_dt)
    IF (copy_sch_files(null)=0)
     RETURN(0)
    ENDIF
    ...
    IF ((dm_err->debug_flag > 2))
     CALL echorecord(dm2_sch_file)
    ENDIF
    SET dm_err->eproc = "Populating rTspace with preserved tablespaces during Refresh"
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dmtspace d
     DETAIL
      ...
      IF (ddd_idx=0)
       IF ((dm_err->debug_flag > 2))
        CALL echo(concat("Add tspace ",d.tspace_name," to rtspace for preserve tables."))
       ENDIF
       ddd_cnt = (rtspace->rtspace_cnt+ 1), stat = alterlist(rtspace->qual,ddd_cnt), rtspace->qual[
       ddd_cnt].tspace_name = d.tspace_name,
       rtspace->qual[ddd_cnt].cur_bytes_allocated = 0, rtspace->qual[ddd_cnt].bytes_needed = (d
       .bytes_needed * (1+ (cnvtreal(dm2_install_schema->percent_tspace)/ 100))), rtspace->qual[
       ddd_cnt].ext_mgmt = d.ext_mgmt,
       rtspace->qual[ddd_cnt].new_ind = 1, rtspace->qual[ddd_cnt].chunk_size = 0.0, rtspace->qual[
       ddd_cnt].chunks_needed = 0,
       rtspace->rtspace_cnt = ddd_cnt
      ELSE
       rtspace->qual[ddd_idx].bytes_needed = (rtspace->qual[ddd_idx].bytes_needed+ (d.bytes_needed *
       (1+ (cnvtreal(dm2_install_schema->percent_tspace)/ 100))))
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
   IF ((drr_retain_db_users->cnt > 0)
    AND (validate(dm2_bypass_loading_user_tspaces_from_sql,- (1))=- (1)))
    SET dm_err->eproc = "Get SYSTEM tablespace extent management."
    SELECT INTO "nl:"
     FROM dba_tablespaces d
     WHERE d.tablespace_name="SYSTEM"
     DETAIL
      dctnfcc_sys_em = d.extent_management
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (curqual=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Unable to get EXTENT_MANAGEMENT for the SYSTEM tablespace."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    SET dm_err->eproc =
    "Loading new tablespaces to support database users being 'retained' from original Target."
    CALL disp_msg(" ",dm_err->logfile,0)
    FOR (dctnfcc_cntr = 1 TO drr_retain_db_users->cnt)
      SET dm_err->eproc = concat("Get sql for user ",drr_retain_db_users->user[dctnfcc_cntr].
       user_name,".")
      CALL disp_msg(" ",dm_err->logfile,0)
      IF (dtd_get_usersql_tspaces(drr_retain_db_users->user[dctnfcc_cntr].user_name)=0)
       RETURN(0)
      ENDIF
    ENDFOR
   ENDIF
   IF ((dir_storage_misc->tgt_storage_type="ASM")
    AND (drr_clin_copy_data->process != "RESTORE"))
    IF (dtd_get_src_custom_users(dtd_user_list)=0)
     RETURN(0)
    ENDIF
    IF (dtd_user_list != "")
     SET dctnfcc_sys_em = "LOCAL"
     SET dm_err->eproc = "Retrieve list of Source custom database users tablespaces."
     CALL disp_msg("",dm_err->logfile,0)
     SELECT INTO "nl:"
      FROM dba_users@ref_data_link du
      WHERE parser(concat("du.username in (",dtd_user_list,")"))
      ORDER BY du.username
      HEAD REPORT
       idx = 0
      DETAIL
       idx = 0, idx = locateval(idx,1,dtd_user_tspaces->cnt,du.default_tablespace,dtd_user_tspaces->
        qual[idx].tspace_name)
       IF (idx=0)
        dtd_user_tspaces->cnt = (dtd_user_tspaces->cnt+ 1), stat = alterlist(dtd_user_tspaces->qual,
         dtd_user_tspaces->cnt), dtd_user_tspaces->qual[dtd_user_tspaces->cnt].tspace_name = du
        .default_tablespace,
        dtd_user_tspaces->qual[dtd_user_tspaces->cnt].temp_ind = 0
       ELSEIF ((dtd_user_tspaces->qual[idx].temp_ind=1))
        dm_err->err_ind = 1, dm_err->emsg = "User default tablespace cannot be TEMP tablespace."
       ENDIF
       idx = 0, idx = locateval(idx,1,dtd_user_tspaces->cnt,du.temporary_tablespace,dtd_user_tspaces
        ->qual[idx].tspace_name)
       IF (idx=0)
        dtd_user_tspaces->cnt = (dtd_user_tspaces->cnt+ 1), stat = alterlist(dtd_user_tspaces->qual,
         dtd_user_tspaces->cnt), dtd_user_tspaces->qual[dtd_user_tspaces->cnt].tspace_name = du
        .temporary_tablespace,
        dtd_user_tspaces->qual[dtd_user_tspaces->cnt].temp_ind = 1
       ELSEIF ((dtd_user_tspaces->qual[idx].temp_ind=0))
        dm_err->err_ind = 1, dm_err->emsg = "User temp tablespace cannot be default tablespace."
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
   ENDIF
   IF ((dtd_user_tspaces->cnt > 0))
    SET dm_err->eproc = "Obtain DM_INFO row override for default/temp tspace size"
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm_info@ref_data_link d
     WHERE d.info_domain="DM2_DB_USER_SIZE"
     DETAIL
      IF (d.info_name="DEFAULT_TSPACE_SIZE")
       dctnfcc_default_tspace_size = d.info_number
      ELSEIF (d.info_name="TEMP_TSPACE_SIZE")
       dctnfcc_temp_tspace_size = d.info_number
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
    IF ((dm_err->debug_flag > 0))
     CALL echo(build("dctnfcc_default_tspace_size =",dctnfcc_default_tspace_size))
     CALL echo(build("dctnfcc_temp_tspace_size =",dctnfcc_temp_tspace_size))
    ENDIF
    FOR (dctnfcc_cntr = 1 TO dtd_user_tspaces->cnt)
      SET ddd_idx = 0
      SET ddd_idx = locateval(ddd_idx,1,rtspace->rtspace_cnt,dtd_user_tspaces->qual[dctnfcc_cntr].
       tspace_name,rtspace->qual[ddd_idx].tspace_name)
      IF (ddd_idx=0)
       IF ((dm_err->debug_flag > 2))
        CALL echo(concat("Add user tspace ",dtd_user_tspaces->qual[dctnfcc_cntr].tspace_name,
          " to rtspace."))
       ENDIF
       SET rtspace->rtspace_cnt = (rtspace->rtspace_cnt+ 1)
       SET stat = alterlist(rtspace->qual,rtspace->rtspace_cnt)
       SET rtspace->qual[rtspace->rtspace_cnt].tspace_name = dtd_user_tspaces->qual[dctnfcc_cntr].
       tspace_name
       SET rtspace->qual[rtspace->rtspace_cnt].cur_bytes_allocated = 0
       IF ((dtd_user_tspaces->qual[dctnfcc_cntr].temp_ind=1))
        SET rtspace->qual[rtspace->rtspace_cnt].bytes_needed = dctnfcc_temp_tspace_size
       ELSE
        SET rtspace->qual[rtspace->rtspace_cnt].bytes_needed = dctnfcc_default_tspace_size
       ENDIF
       SET rtspace->qual[rtspace->rtspace_cnt].ext_mgmt = evaluate(dctnfcc_sys_em,"LOCAL","L","D")
       SET rtspace->qual[rtspace->rtspace_cnt].new_ind = 1
       SET rtspace->qual[rtspace->rtspace_cnt].chunk_size = 0.0
       SET rtspace->qual[rtspace->rtspace_cnt].chunks_needed = 0
       SET rtspace->qual[rtspace->rtspace_cnt].temp_ind = dtd_user_tspaces->qual[dctnfcc_cntr].
       temp_ind
       SET rtspace->qual[rtspace->rtspace_cnt].user_tspace_ind = 1
      ENDIF
    ENDFOR
   ENDIF
   IF ((dir_storage_misc->tgt_storage_type != "ASM"))
    SET dm_err->eproc = "Finding tablespaces that do not have matching extent management"
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
     FROM dm2_dba_tablespaces ddt,
      (dummyt d  WITH seq = value(size(rtspace->qual,5)))
     PLAN (d
      WHERE d.seq > 0)
      JOIN (ddt
      WHERE (rtspace->qual[d.seq].tspace_name=ddt.tablespace_name)
       AND evaluate(rtspace->qual[d.seq].ext_mgmt,"D","DICTIONARY","L","LOCAL") != ddt
      .extent_management)
     HEAD REPORT
      ddd_cnt = 0
     DETAIL
      ddd_cnt = (ddd_cnt+ 1), stat = alterlist(dtd_bad_tspaces->tspace,ddd_cnt), dtd_bad_tspaces->
      tspace[ddd_cnt].tablespace_name = ddt.tablespace_name
     FOOT REPORT
      dtd_bad_tspaces->tspace_cnt = ddd_cnt
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
    IF ((dtd_bad_tspaces->tspace_cnt > 0))
     SET dm_err->eproc = "Get datafile names for mismatched tablespaces"
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     SELECT INTO "nl:"
      FROM dm2_dba_data_files df,
       (dummyt d  WITH seq = value(dtd_bad_tspaces->tspace_cnt))
      PLAN (d)
       JOIN (df
       WHERE (df.tablespace_name=dtd_bad_tspaces->tspace[d.seq].tablespace_name))
      ORDER BY df.tablespace_name
      HEAD REPORT
       ddd_cnt = 0
      HEAD df.tablespace_name
       ddd_cnt = 0, stat = alterlist(dtd_bad_tspaces->tspace[d.seq].dfile,ddd_cnt)
      DETAIL
       ddd_cnt = (ddd_cnt+ 1), stat = alterlist(dtd_bad_tspaces->tspace[d.seq].dfile,ddd_cnt),
       ddd_spot = 0
       IF ((dir_storage_misc->tgt_storage_type != "ASM"))
        IF ((dm2_sys_misc->cur_db_os IN ("AXP", "HPX")))
         dtd_bad_tspaces->tspace[d.seq].dfile[ddd_cnt].datafile_name = df.file_name
        ELSEIF ((dm2_sys_misc->cur_db_os="AIX"))
         ddd_spot = (findstring("/r",df.file_name,1,1)+ 1)
         IF (ddd_spot=1)
          ddd_spot = findstring("/",df.file_name,1,1)
         ENDIF
         ddd_spot_end = textlen(df.file_name), dtd_bad_tspaces->tspace[d.seq].dfile[ddd_cnt].
         datafile_name = substring((ddd_spot+ 1),(ddd_spot_end - ddd_spot),df.file_name)
        ENDIF
       ENDIF
      FOOT  df.tablespace_name
       dtd_bad_tspaces->tspace[d.seq].dfile_cnt = ddd_cnt
      FOOT REPORT
       ddd_cnt = 0
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      ...
      RETURN(0)
     ENDIF
     IF (get_unique_file("ddd_bad_tspace",".rpt")=0)
      RETURN(0)
     ELSE
      SET ddd_badtsp_filename = dm_err->unique_fname
     ENDIF
     SET dm_err->eproc = concat(
      "Display report to user which shows mismatched tablespaces. Report file:",ddd_badtsp_filename)
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     IF ((dm_err->debug_flag > 3))
      CALL echorecord(dtd_bad_tspaces)
     ENDIF
     SELECT INTO value(ddd_badtsp_filename)
      FROM (dummyt d  WITH seq = value(dtd_bad_tspaces->tspace_cnt))
      PLAN (d)
      HEAD REPORT
       col 0, "The following report contains the tablespaces that currently exist in TARGET", row + 1,
       col 0, "with a different extent management than what now exists in SOURCE.", row + 1,
       col 0, " ", row + 1,
       col 0, " ", row + 1,
       col 0,
       CALL print(evaluate(dir_storage_misc->tgt_storage_type,"RAW",
        "It is recommended that the following tablespaces and logical volumes be removed before proceeding.",
        "It is recommended that the following tablespaces be removed before proceeding.")), row + 1,
       col 0,
       "Failing to remove them may result in tablespace sizing issues as objects are created or data is loaded.",
       row + 1,
       col 0, "A drop tablespace SQL command example is located at the bottom of this report.", row
        + 1,
       col 0, " ", row + 1,
       col 0, " ", row + 1,
       col 0, "Tablespace Name", col 32,
       CALL print(evaluate(dm2_sys_misc->cur_db_os,"AXP","Datafiles","Logical Volumes")), row + 1,
       col 0,
       "__________________", col 32, "__________________",
       row + 1, col 0, " ",
       row + 1, col 0, " ",
       row + 1
      DETAIL
       col 0, dtd_bad_tspaces->tspace[d.seq].tablespace_name
       FOR (ddd_cnt = 1 TO dtd_bad_tspaces->tspace[d.seq].dfile_cnt)
         col 32, dtd_bad_tspaces->tspace[d.seq].dfile[ddd_cnt].datafile_name, row + 1
       ENDFOR
      FOOT REPORT
       col 0, " ", row + 1,
       col 0, " ", row + 1,
       col 0, " ", row + 1,
       col 0, " ", row + 1,
       col 0, "Drop tablespace command:", row + 1,
       CALL print(evaluate(dm2_sys_misc->cur_db_os,"AXP",
        "SQL> drop tablespace <TABLESPACE NAME> including contents and datafiles cascade constraints;",
        "SQL> drop tablespace <TABLESPACE NAME> including contents cascade constraints;")), row + 1
      WITH nocounter, format = variable, formfeed = none,
       maxcol = 512
     ;end select
     IF (check_error(dm_err->eproc)=1)
      ...
      RETURN(0)
     ENDIF
     SET dm_err->eproc = "Issue mismatched tablespace report and prompt user for action"
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     IF ((dm2_install_schema->process_option="CLIN COPY")
      ...
      SET drer_email_det->status = "PAUSED"
      SET drer_email_det->status_dt_tm = cnvtdatetime(curdate,curtime3)
      SET drer_email_det->step = "Mismatched Tablespace Report"
      SET drer_email_det->email_level = 1
      SET drer_email_det->logfile = dm_err->logfile
      SET drer_email_det->err_ind = dm_err->err_ind
      SET drer_email_det->eproc = dm_err->eproc
      SET drer_email_det->emsg = dm_err->emsg
      SET drer_email_det->user_action = dm_err->user_action
      CALL drer_add_body_text(concat("Mismatched Tablespace Report was displayed at ",format(
         drer_email_det->status_dt_tm,";;q")),1)
      CALL drer_add_body_text(concat("User Action : Return to dm2_domain_maint main session and ",
        "review mismatched tablespace report displayed on the screen.  ",
        "Follow instructions to cleanup tablespaces and press <enter> to continue."),0)
      CALL drer_add_body_text(concat("Report file name is ccluserdir: ",ddd_badtsp_filename),0)
      IF (drer_compose_email(null)=1)
       CALL drer_send_email(drer_email_det->subject,drer_email_det->file_name,drer_email_det->
        email_level)
      ENDIF
      CALL drer_reset_pre_err(null)
     ENDIF
     IF (dm2_disp_file(ddd_badtsp_filename,"Mismatched Tablespace Report")=0)
      RETURN(0)
     ENDIF
     WHILE ( NOT (ddd_accept IN ("Q", "C"))
      AND (dm_err->err_ind=0))
       ...
       CALL box(2,1,8,132)
       CALL text(3,2,
        "WARNING: Tablespace cleanup should be completed before continuing. Failing to complete the cleanup "
        )
       CALL text(4,2,
        "may result in tablespace sizing issues as objects are created or data is loaded.")
       CALL text(6,2,"(V)iew Tablespace Cleanup Report, (C)ontinue, (Q)uit:")
       CALL accept(6,56,"p;cu"," "
        WHERE curaccept IN ("Q", "V", "C"))
       CALL clear(1,1)
       SET ddd_accept = curaccept
       SET message = nowindow
       CASE (ddd_accept)
        OF "C":
         SET dm_err->eproc = "User elected to continue from mismatched tablespace report"
         CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
        OF "Q":
         SET dm_err->err_ind = 1
         SET dm_err->emsg = "User elected to quit from tablespace cleanup prompt."
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         RETURN(0)
        OF "V":
         SET dm_err->eproc = "Redisplaying mismatched tablespace report"
         IF ((dm_err->debug_flag > 0))
          CALL disp_msg(" ",dm_err->logfile,0)
         ENDIF
         IF (dm2_disp_file(ddd_badtsp_filename,"Mismatched Tablespace Report")=0)
    ...
    ENDIF
   ENDIF
   SET dm_err->eproc = "Determine if tablespace already exists in TARGET"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    ...
    DETAIL
     ddd_idx = locateval(ddd_idx,1,value(size(rtspace->qual,5)),d.tablespace_name,rtspace->qual[
      ddd_idx].tspace_name)
     IF (ddd_idx > 0)
      IF ((rtspace->qual[ddd_idx].user_tspace_ind=1))
       rtspace->qual[ddd_idx].cur_bytes_allocated = total_bytes, rtspace->qual[ddd_idx].new_ind = 0,
       rtspace->qual[ddd_idx].bytes_needed = 0
       IF ((dm_err->debug_flag > 0))
        CALL echo(d.tablespace_name),
        CALL echo(total_bytes),
        CALL echo(rtspace->qual[ddd_idx].bytes_needed),
        CALL echo(rtspace->qual[ddd_idx].user_tspace_ind)
       ENDIF
      ELSE
       rtspace->qual[ddd_idx].cur_bytes_allocated = total_bytes, rtspace->qual[ddd_idx].new_ind = 0
       IF ((dm_err->debug_flag > 0))
        ...
        CALL echo(rtspace->qual[ddd_idx].bytes_needed)
       ENDIF
       IF ((total_bytes < rtspace->qual[ddd_idx].bytes_needed))
        rtspace->qual[ddd_idx].bytes_needed = (rtspace->qual[ddd_idx].bytes_needed - total_bytes),
        dtd_space_needs_ind = 1
       ELSE
        rtspace->qual[ddd_idx].bytes_needed = 0
       ENDIF
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Determine if temporary tablespace(s) already exists in TARGET"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
   ...
   ENDIF
   CALL dsfi_load_schema_file_defs("table_info")
   SET dm_err->eproc = "Finished calculating tspace needs for 'CLIN COPY'"
   CALL disp_msg(" ",dm_err->logfile,0)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_auto_success_obj_needs(null)
   SET dm_err->eproc = "Update status on DM2_TSPACE_OBJ_SIZE to AUTO-SUCCESS"
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE dtd_install_type = vc WITH protect, noconstant(" ")
   DECLARE dtd_install_value = vc WITH protect, noconstant(" ")
   IF ((dm2_install_schema->schema_prefix="dm2o"))
    SET dtd_install_type = "PACKAGE"
    SET dtd_install_value = dm2_install_schema->file_prefix
   ELSE
    SET dtd_install_type = "SCHEMA_DATE"
    SET dtd_install_value = format(cnvtdate2(dm2_install_schema->file_prefix,"MMDDYYYY"),
     "DD-MMM-YYYY;;D")
   ENDIF
   UPDATE  FROM dm2_tspace_obj_size o
    SET o.install_status = "AUTO-SUCCESS", o.updt_dt_time = cnvtdatetime(curdate,curtime3)
    PLAN (o
     WHERE o.install_type=dtd_install_type
      AND o.install_type_value=dtd_install_value
      AND (o.gen_id=dtr_tspace_misc->gen_id)
      AND  NOT ( EXISTS (
     (SELECT
      "x"
      FROM dm2_tspace_size s
      WHERE o.install_type=s.install_type
       AND o.install_type_value=s.install_type_value
       AND o.tspace_name=s.tspace_name))))
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_load_readme_content(null)
   SET dm_err->eproc = "Set tgtidx and curidx for objects in DM2_RR_SPC_NEEDS record."
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE dlr_ttgtidx = i2 WITH protect, noconstant(0)
   DECLARE dlr_tcuridx = i2 WITH protect, noconstant(0)
   DECLARE dlr_log_file = vc WITH protect, noconstant(" ")
   FOR (dlr_t = 1 TO dm2_rr_spc_needs->tbl_cnt)
     SET dlr_ttgtidx = 0
     SET dlr_tcuridx = 0
     SET dlr_ttgtidx = locateval(dlr_ttgtidx,1,tgtsch->tbl_cnt,dm2_rr_spc_needs->tbl[dlr_t].
      table_name,tgtsch->tbl[dlr_ttgtidx].tbl_name)
     SET dm2_rr_spc_needs->tbl[dlr_t].tgt_idx = dlr_ttgtidx
     SET dlr_tcuridx = locateval(dlr_tcuridx,1,cur_sch->tbl_cnt,dm2_rr_spc_needs->tbl[dlr_t].
      table_name,cur_sch->tbl[dlr_tcuridx].tbl_name)
     SET dm2_rr_spc_needs->tbl[dlr_t].cur_idx = dlr_tcuridx
     IF (dlr_tcuridx=0)
      SET dm2_rr_spc_needs->tbl[dlr_t].skip_ind = 1
     ELSE
      SET dm2_rr_spc_needs->tbl[dlr_t].skip_ind = 0
     ENDIF
   ENDFOR
   IF ((dm_err->debug_flag >= 10))
    SET dlr_log_file = build(dm2_install_schema->schema_prefix,dm2_install_schema->file_prefix,
     "_rspace.log")
    CALL disp_msg("",dm_err->logfile,0)
    SET dm_err->eproc = concat("Write dm2_rr_spc_needs contents into file ",dlr_log_file)
    SELECT INTO value(dlr_log_file)
     d.seq
     FROM (dummyt d  WITH seq = value(dm2_rr_spc_needs->tbl_cnt))
     HEAD REPORT
      "table count = ", dm2_rr_spc_needs->tbl_cnt, row + 1,
      "space needed = ", dm2_rr_spc_needs->space_needed
     DETAIL
      row + 1, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", row + 1,
      "table number = ", d.seq, row + 1,
      "table_name = ", dm2_rr_spc_needs->tbl[d.seq].table_name, row + 1,
      "large_data_loaded  = ", dm2_rr_spc_needs->tbl[d.seq].large_data_loaded, row + 1,
      "skip_ind = ", dm2_rr_spc_needs->tbl[d.seq].skip_ind, row + 1,
      "insert_row_cnt  = ", dm2_rr_spc_needs->tbl[d.seq].insert_row_cnt, row + 1,
      "tgtsch table idx  = ", dm2_rr_spc_needs->tbl[d.seq].tgt_idx, row + 1,
      "cursch table idx = ", dm2_rr_spc_needs->tbl[d.seq].cur_idx, row + 1,
      "space needed  = ", dm2_rr_spc_needs->tbl[d.seq].space_needed, row + 1,
      "column update count", dm2_rr_spc_needs->tbl[d.seq].col_updt_cnt, row + 1,
      "index count", dm2_rr_spc_needs->tbl[d.seq].ind_cnt, row + 1
      IF ((dm2_rr_spc_needs->tbl[d.seq].col_updt_cnt > 0))
       row + 1, "UPDATE COLUMNS"
       FOR (tc_cnt = 1 TO dm2_rr_spc_needs->tbl[d.seq].col_updt_cnt)
         row + 1, col 3, "------------------",
         row + 1, col 3, "column name = ",
         dm2_rr_spc_needs->tbl[d.seq].col_updt[tc_cnt].column_name, row + 1, col 3,
         "update row cnt  = ", dm2_rr_spc_needs->tbl[d.seq].col_updt[tc_cnt].update_row_cnt, row + 1
       ENDFOR
      ENDIF
     WITH nocounter, format = variable, formfeed = none,
      maxrow = 1, maxcol = 512
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_load_datafiles(null)
   DECLARE dld_ts_ndx = i4 WITH protect, noconstant(0)
   ...
   SELECT INTO "nl:"
    ...
    HEAD REPORT
     dls_ts_cnt = 0
    HEAD df.tablespace_name
     dld_df_cnt = 0, dld_ts_ndx = 0, dld_ts_ndx = locateval(dld_ts_ndx,1,size(dtr_dg_for_ts->ts,5),df
      .tablespace_name,dtr_dg_for_ts->ts[dld_ts_ndx].tspace_name)
     IF (dld_ts_ndx=0)
      dls_ts_cnt = (dls_ts_cnt+ 1), stat = alterlist(dtr_dg_for_ts->ts,dls_ts_cnt), dtr_dg_for_ts->
      ts[dls_ts_cnt].tspace_name = df.tablespace_name
     ENDIF
     dld_ts_ndx = locateval(dld_ts_ndx,1,size(dtr_dg_for_ts->ts,5),df.tablespace_name,dtr_dg_for_ts->
      ts[dld_ts_ndx].tspace_name), dld_dg_cnt = 0
    DETAIL
     ...
     IF (locateval(dld_dg_ndx,1,size(dtr_dg_for_ts->ts[dld_ts_ndx].dg,5),rtspace->qual[d.seq].cont[
      dld_df_cnt].disk_name,dtr_dg_for_ts->ts[dld_ts_ndx].dg[dld_dg_ndx].disk_group)=0)
      dld_dg_cnt = (dld_dg_cnt+ 1), stat = alterlist(dtr_dg_for_ts->ts[dld_ts_ndx].dg,dld_dg_cnt),
      dtr_dg_for_ts->ts[dld_ts_ndx].dg[dld_dg_cnt].disk_group = rtspace->qual[d.seq].cont[dld_df_cnt]
      .disk_name
     ENDIF
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dtr_dg_for_ts)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_drop_empty_tspace(ddet_dropped)
   DECLARE dgtd_ts_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgtd_ndx = i4 WITH protect, noconstant(0)
   DECLARE dgtd_str = vc WITH protect, noconstant("")
   SET dtd_tspaces_dropped = 0
   SET dm_err->eproc = "Get list of tablespaces from DBA_SEGMENTS"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT DISTINCT INTO "nl:"
    ...
    HEAD REPORT
     dgtd_ts_cnt = 0
    DETAIL
     dgtd_ts_cnt = (dgtd_ts_cnt+ 1)
     IF (mod(dgtd_ts_cnt,50)=1)
      stat = alterlist(dtr_ts->tsp,(dgtd_ts_cnt+ 49))
     ENDIF
     dtr_ts->tsp[dgtd_ts_cnt].tspace_name = d.tablespace_name
    FOOT REPORT
     stat = alterlist(dtr_ts->tsp,dgtd_ts_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 1))
    CALL echorecord(dtr_ts)
   ENDIF
   SET dm_err->eproc = "Get list of tablespaces that are USERS default/temporary tablespaces."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT DISTINCT INTO "nl:"
    ...
    FROM (
     (
     (SELECT
      tablespace_name = d.default_tablespace
      FROM dba_users d
      WHERE ((1=1) UNION (
      (SELECT
       tablespace_name = d2.temporary_tablespace
       FROM dba_users d2)))
      WITH sqltype("c30")))
     x)
    ORDER BY x.tablespace_name
    DETAIL
     dgtd_ts_cnt = (dgtd_ts_cnt+ 1), stat = alterlist(dtr_ts->tsp,dgtd_ts_cnt), dtr_ts->tsp[
     dgtd_ts_cnt].tspace_name = x.tablespace_name
    FOOT REPORT
     stat = alterlist(dtr_ts->tsp,dgtd_ts_cnt)
    WITH nocounter
   ;end select
   ...
   IF ((dm_err->debug_flag > 1))
    ...
    CALL echorecord(rtspace)
   ENDIF
   SET dm_err->eproc = "Find empty tablespaces that are in rtspace"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   FOR (dgtd_ts_cnt = 1 TO size(rtspace->qual,5))
     IF ((rtspace->qual[dgtd_ts_cnt].new_ind=0))
      IF (((size(dtr_ts->tsp,5)=0) OR (locateval(dgtd_ndx,1,size(dtr_ts->tsp,5),rtspace->qual[
       dgtd_ts_cnt].tspace_name,dtr_ts->tsp[dgtd_ndx].tspace_name)=0)) )
       SET rtspace->qual[dgtd_ts_cnt].new_ind = 1
       SET dm_err->eproc = concat("Dropping:",rtspace->qual[dgtd_ts_cnt].tspace_name)
       IF ((dm_err->debug_flag > 0))
        CALL disp_msg(" ",dm_err->logfile,0)
       ENDIF
       SET ddet_dropped = 1
       SET dgtd_str = concat("rdb drop tablespace ",rtspace->qual[dgtd_ts_cnt].tspace_name,
        " including contents and datafiles cascade constraints go")
       IF (dm2_push_cmd(dgtd_str,1)=0)
        RETURN(0)
       ENDIF
      ENDIF
     ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_figure_df_to_add(dfda_added)
   DECLARE dfda_ndx = i4 WITH protect, noconstant(0)
   DECLARE dfda_df_add = i4 WITH protect, noconstant(0)
   DECLARE dfda_32g = f8 WITH protect, noconstant((((32.0 * 1024.0) * 1024.0) * 1024.0))
   DECLARE dfda_dg_ndx = i4 WITH protect, noconstant(0)
   DECLARE dfda_cont_cnt = i4 WITH protect, noconstant(0)
   DECLARE dfda_cnt = i4 WITH protect, noconstant(0)
   DECLARE dfda_dg_select = i4 WITH protect, noconstant(0)
   DECLARE dfda_last = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Allocate datafiles to compensate for space needs."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    tot_bytes = sum(db.bytes), db.tablespace_name
    FROM dm2_dba_free_space db,
     dm2_dba_tablespaces d
    WHERE db.tablespace_name=d.tablespace_name
     AND d.bigfile="NO"
     AND db.tablespace_name IN ("D_*", "L_*", "I_*")
    GROUP BY db.tablespace_name
    DETAIL
     dfda_dg_select = 0, dfda_last = 0, dfda_df_add = 0,
     dfda_ndx = locateval(dfda_ndx,1,size(rtspace->qual,5),db.tablespace_name,rtspace->qual[dfda_ndx]
      .tspace_name), dfda_dg_ndx = locateval(dfda_dg_ndx,1,size(dtr_dg_for_ts->ts,5),db
      .tablespace_name,dtr_dg_for_ts->ts[dfda_dg_ndx].tspace_name)
     IF (dfda_ndx > 0
      AND (rtspace->qual[dfda_ndx].user_tspace_ind=0))
      IF ((((rtspace->qual[dfda_ndx].bytes_needed - tot_bytes)/ dfda_32g) > 1))
       dfda_df_add = dm2ceil(((rtspace->qual[dfda_ndx].bytes_needed - tot_bytes)/ dfda_32g))
       FOR (dfda_cnt = 1 TO dfda_df_add)
         dfda_dg_select = (dfda_last+ 1)
         IF (dfda_dg_select > size(dtr_dg_for_ts->ts[dfda_dg_ndx].dg,5))
          dfda_dg_select = 1, dfda_last = 0
         ELSE
          dfda_last = dfda_dg_select
         ENDIF
         dfda_cont_cnt = (size(rtspace->qual[dfda_ndx].cont,5)+ 1), stat = alterlist(rtspace->qual[
          dfda_ndx].cont,dfda_cont_cnt), rtspace->qual[dfda_ndx].cont[dfda_cont_cnt].disk_name =
         dtr_dg_for_ts->ts[dfda_dg_ndx].dg[dfda_dg_select].disk_group,
         rtspace->qual[dfda_ndx].cont[dfda_cont_cnt].space_to_add = ((1.0 * 1024.0) * 1024.0),
         rtspace->qual[dfda_ndx].user_bytes_to_add = ((1.0 * 1024.0) * 1024.0), rtspace->qual[
         dfda_ndx].cont[dfda_cont_cnt].new_ind = 1,
         dfda_added = 1
       ENDFOR
      ENDIF
     ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_add_space_to_existing(null)
   DECLARE dase_ts_cnt = i4 WITH protect, noconstant(0)
   DECLARE dase_df_cnt = i4 WITH protect, noconstant(0)
   DECLARE dase_str = vc WITH protect, noconstant("")
   FOR (dase_ts_cnt = 1 TO size(rtspace->qual,5))
     IF ((rtspace->qual[dase_ts_cnt].new_ind=0)
      AND (rtspace->qual[dase_ts_cnt].user_bytes_to_add > 0))
      SET dm_err->eproc = concat("Adding ",trim(cnvtstring(size(rtspace->qual[dase_ts_cnt].cont,5))),
       " datafile to tablespace:",rtspace->qual[dase_ts_cnt].tspace_name)
      CALL disp_msg(" ",dm_err->logfile,0)
      SET dase_str = concat("rdb alter tablespace ",rtspace->qual[dase_ts_cnt].tspace_name,
       " add datafile ")
      FOR (dase_df_cnt = 1 TO size(rtspace->qual[dase_ts_cnt].cont,5))
        IF ((rtspace->qual[dase_ts_cnt].cont[dase_df_cnt].space_to_add > 0))
         IF (dase_df_cnt > 1)
          SET dase_str = concat(dase_str,",")
         ENDIF
         SET dase_str = concat(dase_str,"'+",rtspace->qual[dase_ts_cnt].cont[dase_df_cnt].disk_name,
          "' size ",trim(cnvtstring(convert_bytes(rtspace->qual[dase_ts_cnt].cont[dase_df_cnt].
             space_to_add,"b","m"))),
          "M autoextend on next 64M")
        ENDIF
      ENDFOR
      SET dase_str = concat(dase_str," go")
      IF (dm2_push_cmd(dase_str,1)=0)
       RETURN(0)
      ENDIF
     ENDIF
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_rpt_exist_tsp_needs_by_dg(detnbd_file,detnbd_exist_space_needs)
   ...
   SELECT INTO value(detnbd_file)
    ...
    HEAD REPORT
     ...
     IF ((ddtsp->nonstd_tgt_ind=1))
      col 0,
      "IMPORTANT:  Unsupported tablespace configurations have been detected in this environment.",
      row + 1,
      col 0, "            These tablespaces may have space issues causing this installation to fail.",
      row + 1,
      col 0, "            Review report following the [Tablespace Maintenance Examples/Glossary] ",
      row + 1,
      col 0, "            section for the list of tablespaces with unsupported configurations. ", row
       + 2
     ENDIF
    ...
    FOOT REPORT
     ...
     IF ((dm2_install_schema->process_option="CLIN COPY"))
      col 0,
      "Space Needed         Amount of space calculated for one or more tablespaces required to", row
       + 1,
      col 0,
     ...
     ELSE
      col 0,
      "Space Needed         Amount of space calculated to complete schema changes (free space in ",
      row + 1,
      col 0,
      "                        underlying tablespaces and disk groups are NOT considered when determining this value).",
      row + 2
     ENDIF
     col 0, "Free Space           Amount of disk group space available for use by the database.", row
      + 2
   ...
   ENDIF
   IF ((dir_ui_misc->dm_process_event_id > 0))
    IF (dtd_add_rpt_to_event_dtl(concat("TSPACE_NEEDS:",dtd_ui_install_event),detnbd_file)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_ui_tspace_pause(dutp_plan_id,dutp_event_id,dutp_pause_event,dutp_file_name,
  dutp_install_event)
   DECLARE dutp_warn_ind = i2 WITH protect, noconstant(1)
   DECLARE dutp_process_status = i2 WITH protect, noconstant(0)
   DECLARE dutp_wait_confirmed = i2 WITH protect, noconstant(0)
   DECLARE dutp_pause_install = i2 WITH protect, noconstant(1)
   DECLARE dutp_monitor_status = i2 WITH protect, noconstant(0)
   IF (drr_get_process_status("DM2_INSTALL_PACKAGE",cnvtreal(dutp_plan_id),dutp_process_status)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Obtain DM_INFO row override for tspace warning"
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="DM2_TSPACE_DRIVER"
     AND d.info_domain="UI_TSPACE_WARNING"
    DETAIL
     dutp_warn_ind = d.info_number
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (dutp_warn_ind=1)
    IF (drr_modify_install_status(dutp_plan_id,currdbhandle,2,concat("DM2_TSPACE_DRIVER:",
      dutp_pause_event),dutp_install_event)=0)
     RETURN(0)
    ENDIF
    CALL dm2_process_log_add_detail_text(dutp_pause_event,"ISSUED")
    SET dm2_process_event_rs->details[dm2_process_event_rs->detail_cnt].detail_date = cnvtdatetime(
     curdate,curtime3)
    IF (dm2_process_log_dtl_row(dutp_event_id,1)=0)
     RETURN(0)
    ENDIF
    SET dnotify->process = "INSTALLPLAN"
    SET dnotify->plan_id = dutp_plan_id
    SET dnotify->install_status = "PAUSED"
    SET dnotify->event = dutp_install_event
    SET dnotify->mode = dip_install_mode
    SET dnotify->msgtype = dpl_actionreq
    CALL dn_add_body_text("Installation has Paused.  Tablespace needs were detected.",0)
    CALL dn_add_body_text(" ",0)
    CALL dn_add_body_text("Please review tablespace needs to ensure that:",0)
    CALL dn_add_body_text("1) Install Failures do not occur",0)
    CALL dn_add_body_text(
     "2) Applications do not fail from Installation (potentially) fully consuming a tablespace.",0)
    CALL dn_add_body_text(" ",0)
    IF ((dm2_sys_misc->cur_os="AXP"))
     CALL dn_add_body_text("Please process tablespace needs at the following location:",0)
     CALL dn_add_body_text(concat("ccl | dm2_install_plan_menu | Select Plan (",trim(cnvtstring(
         dutp_plan_id)),") | Tablespace Needs Maintenance."),0)
    ELSE
     CALL dn_add_body_text("Please review tablespace needs at the following location:",0)
     CALL dn_add_body_text(concat("ccl | dm2_install_plan_menu | Select Plan (",trim(cnvtstring(
         dutp_plan_id)),") | ",concat("TSPACE_NEEDS:",dtd_ui_install_event),"."),0)
    ENDIF
    CALL dn_add_body_text(" ",0)
    CALL dn_add_body_text(concat("After reviewing and satisfying tablespace needs, ",
      "to proceed with the Installation, execute the following:"),0)
    CALL dn_add_body_text(" ",0)
    CALL dn_add_body_text(concat("ccl | dm2_install_plan_menu go | Select Plan (",trim(cnvtstring(
        dutp_plan_id)),")  | Modify Install Status | EXECUTE "),0)
    SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->status = dpl_complete
    SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dm2_process_event_rs->install_plan_id = dutp_plan_id
    CALL dm2_process_log_add_detail_text(dpl_audit_name,concat("EMAIL:",dutp_pause_event))
    CALL dm2_process_log_add_detail_text(dpl_audit_type,dpl_actionreq)
    IF (dn_notify(null)=0)
     RETURN(0)
    ENDIF
    WHILE (dutp_pause_install=1)
      IF (drr_get_process_status("DM2_INSTALL_PKG",cnvtreal(dutp_plan_id),dutp_process_status)=0)
       RETURN(0)
      ENDIF
      SET dir_ui_misc->install_status = dutp_process_status
      IF ((dir_ui_misc->install_status=0))
       SET dm_err->eproc = "Obtaining installation status"
       SET dm_err->err_ind = 1
       SET dm_err->emsg = "Install Status stopped, exiting dm2_tspace_driver"
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ELSEIF ((dir_ui_misc->install_status=2))
       IF (dipr_update_install_itinerary(dir_ui_misc->install_status,dm2_process_event_rs->
        itinerary_process_event_id,cnvtreal(dutp_plan_id))=0)
        RETURN(0)
       ENDIF
       CALL dpl_upd_dped_last_status(dir_ui_misc->dm_process_event_id,concat(
         "Installation paused for Tablespace Needs. ",dutp_install_event),0.0,cnvtdatetime(curdate,
         curtime3))
       IF (dir_perform_wait_interval(null)=0)
        RETURN(0)
       ENDIF
      ELSEIF ((dir_ui_misc->install_status=1))
       SET dutp_pause_install = 0
       IF (dipr_update_install_itinerary(dir_ui_misc->install_status,dm2_process_event_rs->
        itinerary_process_event_id,cnvtreal(dutp_plan_id))=0)
        RETURN(0)
       ENDIF
      ENDIF
      IF ((dir_ui_misc->install_status IN (1, 2)))
       SET dutp_monitor_status = 0
       IF (drr_get_process_status("DM2_INSTALL_MONITOR",cnvtreal(dutp_plan_id),dutp_monitor_status)=0
       )
        RETURN(0)
       ENDIF
       IF (dutp_monitor_status=0)
        IF (drr_submit_background_process(trim(currdbuser,3),dm2_install_schema->v500_p_word,
         dm2_install_schema->v500_connect_str,dir_batch_queue,dpl_install_monitor,
         cnvtreal(dutp_plan_id),dip_install_mode)=0)
         RETURN(0)
        ENDIF
       ENDIF
      ENDIF
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dtd_get_usersql_tspaces(dgut_user)
   DECLARE dgut_sql_name = vc WITH protect, noconstant("")
   DECLARE dgut_tmp_dir = vc WITH protect, noconstant("")
   DECLARE dgut_db_node = vc WITH protect, noconstant("")
   DECLARE dgut_tspace_strt = i4 WITH protect, noconstant(0)
   DECLARE dgut_tspace_end = i4 WITH protect, noconstant(0)
   DECLARE dgut_tspace_len = i4 WITH protect, noconstant(0)
   DECLARE dgut_tspace_name = vc WITH protect, noconstant("")
   DECLARE dgut_idx = i4 WITH protect, noconstant(0)
   IF ((((der_expimp_data->tgt_temp_dir="")) OR ((der_expimp_data->tgt_temp_dir="DM2NOTSET"))) )
    IF (der_manage_admin_data(trim(cnvtupper(currdbname),3),"DM2_ADMIN_DM_INFO","S","ALL","")=0)
     RETURN(0)
    ENDIF
    IF ((((der_expimp_data->tgt_temp_dir="")) OR ((der_expimp_data->tgt_temp_dir="DM2NOTSET"))) )
     SET dm_err->eproc = "Retrieve Target temporary export/import directory."
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Unable to get directory data."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   ...
   IF (check_error(dm_err->eproc) > 0)
    ...
    RETURN(0)
   ENDIF
   SET dgut_tmp_dir = build(der_expimp_data->tgt_temp_dir,"/",trim(cnvtlower(currdbname),3),"_exp/")
   SET dm_err->eproc = concat("Retrieve preserved Target user ",cnvtupper(dgut_user),
    " tablespace for database ",currdbname)
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_admin_dm_info di
    WHERE di.info_domain="DM2_REPLICATE_USER_TS"
     AND di.info_name=patstring(cnvtupper(build(trim(currdbname,3),"-DB-",dgut_user,"-*")))
    DETAIL
     dgut_idx = 0, dgut_idx = locateval(dgut_idx,1,dtd_user_tspaces->cnt,di.info_char,
      dtd_user_tspaces->qual[dgut_idx].tspace_name)
     IF (dgut_idx=0)
      dtd_user_tspaces->cnt = (dtd_user_tspaces->cnt+ 1), stat = alterlist(dtd_user_tspaces->qual,
       dtd_user_tspaces->cnt), dtd_user_tspaces->qual[dtd_user_tspaces->cnt].tspace_name = di
      .info_char,
      dtd_user_tspaces->qual[dtd_user_tspaces->cnt].temp_ind = 0
      IF (findstring("-TEMP",di.info_name,1,0) > 0)
       dtd_user_tspaces->qual[dtd_user_tspaces->cnt].temp_ind = 1
      ENDIF
     ELSE
      IF (findstring("-TEMP",di.info_name,1,0) > 0)
       dtd_user_tspaces->qual[dgut_idx].temp_ind = 1
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF ((dm_err->debug_flag > 2))
    CALL echorecord(dtd_user_tspaces)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_get_src_custom_users(dgscu_user_list)
   SET dm_err->eproc = "Determining if REF_DATA_LINK link for Source exists in Target"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM all_db_links dl
    WHERE dl.db_link="REF_DATA_LINK.*"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   SELECT INTO "nl:"
    ...
    HEAD REPORT
     dtd_exception_list->cnt = 0, stat = alterlist(dtd_exception_list->qual,0)
    DETAIL
     dtd_exception_list->cnt = (dtd_exception_list->cnt+ 1), stat = alterlist(dtd_exception_list->
      qual,dtd_exception_list->cnt), dtd_exception_list->qual[dtd_exception_list->cnt].owner = di
     .info_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = "Retrieve list of custom database users from Source not exists in Target."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_users@ref_data_link du
    ...
    ORDER BY du.username
    HEAD REPORT
     dgscu_user_list = "", idx = 0
    DETAIL
     idx = 0, idx = locateval(idx,1,dtd_exception_list->cnt,du.username,dtd_exception_list->qual[idx]
      .owner)
     IF (idx=0)
      IF ((dm_err->debug_flag > 0))
      ...
      ENDIF
      IF (dgscu_user_list="")
       dgscu_user_list = build("'",du.username,"',")
      ELSE
       dgscu_user_list = build(dgscu_user_list,"'",du.username,"',")
      ENDIF
     ELSE
      IF ((dm_err->debug_flag > 0))
     ...
     ENDIF
    FOOT REPORT
     dgscu_user_list = replace(dgscu_user_list,",","",2)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dtd_exception_list)
    CALL echo(concat("User List = ",dgscu_user_list))
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_set_tspace_quotas(null)
   DECLARE dstq_cnt = i4 WITH protect, noconstant(0)
   DECLARE dstq_str = vc WITH protect, noconstant("")
   DECLARE dstq_idx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Establish quotas for tablespaces."
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((dtd_ts_quota_cand->cnt=0))
    SET dm_err->eproc = "No tablespace to assign quotas for."
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   IF ((((dir_storage_misc->tgt_storage_type != "ASM")) OR ((dm2_rdbms_version->level1 < 11))) )
    SET dm_err->eproc = "Storage type and Oracle version does not qualify for tablespace quotas."
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   SET dm_err->eproc = "Check if user already has unlimited tablespace."
   SELECT INTO "nl:"
    FROM dba_sys_privs t
    WHERE t.privilege="UNLIMITED TABLESPACE"
     AND t.grantee=currdbuser
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ENDIF
   IF (curqual > 0)
    SET dm_err->eproc = concat("UNLIMITED TABLESPACE privilege already granted to ",currdbuser,
     ". No further action needed.")
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   FREE RECORD dstq_unlimited_quota
   RECORD dstq_unlimited_quota(
     1 cnt = i4
     1 qual[*]
       2 tspace_name = vc
   )
   SET dstq_unlimited_quota->cnt = 0
   SET dm_err->eproc = "Gather list of tablespaces with unlimited quota."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_ts_quotas dtq
    WHERE (dtq.max_bytes=- (1))
    HEAD REPORT
     dstq_unlimited_quota->cnt = 0, stat = alterlist(dstq_unlimited_quota->qual,0)
    DETAIL
     dstq_unlimited_quota->cnt = (dstq_unlimited_quota->cnt+ 1), stat = alterlist(
      dstq_unlimited_quota->qual,dstq_unlimited_quota->cnt), dstq_unlimited_quota->qual[
     dstq_unlimited_quota->cnt].tspace_name = dtq.tablespace_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ENDIF
   IF ((dtd_ts_quota_cand->cnt > 0))
    FOR (dstq_cnt = 1 TO size(dtd_ts_quota_cand->qual,5))
      SET dstq_idx = 0
      SET dstq_idx = locateval(dstq_idx,1,dstq_unlimited_quota->cnt,dtd_ts_quota_cand->qual[dstq_cnt]
       .tspace_name,dstq_unlimited_quota->qual[dstq_idx].tspace_name)
      IF (dstq_idx=0)
       SET dstq_str = concat("rdb alter user ",currdbuser," quota unlimited on ",dtd_ts_quota_cand->
        qual[dstq_cnt].tspace_name," go")
       IF (dm2_push_cmd(dstq_str,1)=0)
        RETURN(0)
       ENDIF
      ENDIF
    ENDFOR
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dtd_load_tspace_quota_candidates(null)
   IF ((rtspace->rtspace_cnt=0)
    AND (tspace_rec->tsp_rec_cnt=0))
    SET dm_err->eproc = "No tablespace to process."
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   IF ((rtspace->rtspace_cnt > 0))
    FOR (dstq_cnt = 1 TO size(rtspace->qual,5))
      IF ((rtspace->qual[dstq_cnt].temp_ind=0)
       AND (rtspace->qual[dstq_cnt].user_tspace_ind=0))
       SET dstq_idx = 0
       SET dstq_idx = locateval(dstq_idx,1,dtd_ts_quota_cand->cnt,rtspace->qual[dstq_cnt].tspace_name,
        dtd_ts_quota_cand->qual[dstq_idx].tspace_name)
       IF (dstq_idx=0)
        SET dtd_ts_quota_cand->cnt = (dtd_ts_quota_cand->cnt+ 1)
        SET stat = alterlist(dtd_ts_quota_cand->qual,dtd_ts_quota_cand->cnt)
        SET dtd_ts_quota_cand->qual[dtd_ts_quota_cand->cnt].tspace_name = rtspace->qual[dstq_cnt].
        tspace_name
       ENDIF
      ENDIF
    ENDFOR
   ENDIF
   IF ((tspace_rec->tsp_rec_cnt > 0))
    FOR (dstq_cnt = 1 TO size(tspace_rec->tsp,5))
      SET dstq_idx = 0
      SET dstq_idx = locateval(dstq_idx,1,dtd_ts_quota_cand->cnt,tspace_rec->tsp[dstq_cnt].tsp_name,
       dtd_ts_quota_cand->qual[dstq_idx].tspace_name)
      IF (dstq_idx=0)
       SET dtd_ts_quota_cand->cnt = (dtd_ts_quota_cand->cnt+ 1)
       SET stat = alterlist(dtd_ts_quota_cand->qual,dtd_ts_quota_cand->cnt)
       SET dtd_ts_quota_cand->qual[dtd_ts_quota_cand->cnt].tspace_name = tspace_rec->tsp[dstq_cnt].
       tsp_name
      ENDIF
    ENDFOR
   ENDIF
...
#exit_script
 ...
 FREE RECORD tspace_rec
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "...end DM2_TSPACE_DRIVER"
 ENDIF
 CALL final_disp_msg("dm2_tspace_driver")
END GO
