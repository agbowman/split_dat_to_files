CREATE PROGRAM dm2_pg_child_readme_c:dba
 ...
 DECLARE mf_maxqual_size = f8 WITH protect, noconstant(mf_update_size)
 DECLARE mf_curqual_var = f8 WITH protect, noconstant(0.0)
 DECLARE mf_updt_id_tag_c = f8 WITH protect, noconstant(1.1)
 DECLARE ms_maxqual_string = vc WITH protect, noconstant("")
 SET ms_child_c_info_domain_nm = "UPDATE EPR'S ENCNTR_TYPE_CD"
 SET ms_child_c_max_name = "MAX ENCNTR_PRSNL_RELTN_ID EVALUATED BY"
 CALL sbr_get_min_max(mf_min_id,mf_max_id)
 SET ms_maxqual_string = concat("rownum <=",cnvtstring(mf_maxqual_size))
 SET mf_curqual_var = mf_maxqual_size
 CALL echo("Processing...")
 CALL echo("")
 WHILE (mf_curqual_var=mf_maxqual_size)
   CALL echo("Processing...")
   CALL echo("Updating A Section of the Encntr_Prsnl_Reltn Table...")
   UPDATE  FROM encntr_prsnl_reltn epr
    SET epr.encntr_type_cd =
     (SELECT
      e.encntr_type_cd
      FROM encounter e
      WHERE e.encntr_id=epr.encntr_id), epr.updt_task = reqinfo->updt_task, epr.updt_id =
     mf_updt_id_tag_c
    WHERE epr.encntr_prsnl_reltn_id >= mf_min_id
     AND epr.encntr_prsnl_reltn_id <= mf_max_id
     AND ((epr.encntr_type_cd = null) OR (epr.encntr_type_cd=0))
     AND epr.updt_id != mf_updt_id_tag_c
     AND sqlpassthru(ms_maxqual_string)
    WITH nocounter
   ;end update
   SET mf_curqual_var = curqual
