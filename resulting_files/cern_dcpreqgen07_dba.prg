CREATE PROGRAM cern_dcpreqgen07:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   orders->dischg_dt = concat(format(datetimezone(e.disch_dt_tm,tz_index),"@SHORTDATE;;Q")," ",
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   DETAIL
    FOR (vv = 1 TO value(order_cnt))
      IF ((orders->qual[vv].display_ind=1))
       ...
       CALL print(calcpos(xcol,ycol)),
       "Ordering Type: ", orders->qual[vv].type, row + 1,
       xcol = 40, ycol = (ycol+ 11),
       CALL print(calcpos(xcol,ycol)),
   ...
   FOOT PAGE
    ...
    "{pos/410/709}", "Age: ", orders->age,
    "{pos/490/709}", "Sex: ", orders->sex,
    row + 1, "{pos/300/720}", "MD: ",
    orders->attending, row + 1, "{pos/490/731}",
    "Page # ", curpage, row + 1,
    "{cpi/10}{pos/30/693}{b}", "Hospital Name", "{cpi/10}{pos/60/708}{b}",
    "Laboratory Order Requisition", "{font/8}{cpi/12}{pos/50/720}", "dcpreqgen07"
