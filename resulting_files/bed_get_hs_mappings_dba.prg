CREATE PROGRAM bed_get_hs_mappings:dba
 ...
 SELECT INTO "nl:"
  FROM br_hlth_sntry_item h,
   ...
   code_value c
  PLAN (h
   WHERE (h.code_set=request->code_set))
   JOIN (r
  ...
  DETAIL
   ...
   display = c.display, reply->items[tcnt].reltns[r_tcnt].definition = c.definition,
   reply->items[tcnt].reltns[r_tcnt].description = c.description, reply->items[tcnt].reltns[r_tcnt].
   health_sentry_item_relation_id = r.br_hlth_sntry_mill_item_id
