CREATE PROGRAM bhs_rpt_rad_tat_tech_comm:dba
 PROMPT
 ...
 SELECT INTO  $OUTDEV
  ...
  HEAD section
   cntsection += 1, stat = alterlist(event->sectionlist,cntsection), event->sectionlist[cntsection].
  ...
  DETAIL
   IF (complete_to_final >= 0
    ...
    ordtat += complete_to_final, transtofinaltat += trans_to_final, comptotranstat +=
    complete_to_trans,
    cntorders += 1
    ...
    event->sectionlist[cntsection].orderlist[cntorders].complete_to_final = complete_to_final, stat
     = alterlist(event->sectionlist[cntsection].orderlist,cntorders), totalrptordcnt += 1,
    totalrptcfcnt += complete_to_final
    IF ((event->sectionlist[cntsection].orderlist[cntorders].position_cd=227480046))
     totalresidenttat += complete_to_final, cntresidentorders += 1
    ENDIF
    IF ((event->sectionlist[cntsection].orderlist[cntorders].position_cd=228838033))
     totalradiologisttat += complete_to_final, cntradiologistorders += 1
    ENDIF
   ENDIF
  FOOT  orders
   event->sectionlist[cntsection].totaltatcf += ordtat, event->sectionlist[cntsection].totaltatdf +=
   transtofinaltat, event->sectionlist[cntsection].totaltatcd += comptotranstat,
   event->sectionlist[cntsection].totalrestatcf += totalresidenttat, event->sectionlist[cntsection].
