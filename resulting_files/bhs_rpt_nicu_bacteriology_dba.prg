CREATE PROGRAM bhs_rpt_nicu_bacteriology:dba
 PROMPT
 ...
 DECLARE mf_cs72_weight_oz = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,"WEIGHTLBOZ"))
 DECLARE mf_cs72_birthweight = f8 WITH constant(uar_get_code_by("DESCRIPTION",72,"Birth weight")),
 protect
 ...
 DECLARE mf_cs120_ocfcompression = f8 WITH constant(uar_get_code_by("DISPLAYKEY",120,"OCFCOMPRESSION"
   )), protect
 SELECT INTO "nl:"
  ...
  PLAN (c
   ...
   JOIN (ese
   WHERE ese.event_set_cd=value(uar_get_code_by("DISPLAYKEY",93,"BACTERIOLOGY"))
   ...
   JOIN (pp
   WHERE pp.person_id=c.person_id)
   JOIN (ce1
   WHERE ce1.event_cd IN (mf_cs72_weight_oz)
    AND ce1.encntr_id=c.encntr_id
    AND ce1.person_id=c.person_id
   ...
   (SELECT
    min(ce1a.event_end_dt_tm)
    FROM clinical_event ce1a
    WHERE ce1a.encntr_id=ce1.encntr_id
     AND ce1a.person_id=ce1.person_id
     AND ce1a.valid_until_dt_tm > sysdate
     AND ce1a.view_level=1
     AND ce1a.result_status_cd IN (mf_cs8_altered_cd, mf_cs8_modified_cd, mf_cs8_auth_cd,
    mf_cs8_active_cd)
     AND ce1a.event_cd=ce1.event_cd)))
   JOIN (ce2
   WHERE ce2.event_cd IN (mf_cs72_birthweight)
    AND ce2.encntr_id=c.encntr_id
    AND ce2.person_id=c.person_id
   ...
   mf_cs8_active_cd)
    ...
    AND (ce2.event_end_dt_tm=
   (SELECT
    min(ce2a.event_end_dt_tm)
    FROM clinical_event ce2a
    WHERE ce2a.encntr_id=ce2.encntr_id
     AND ce2a.person_id=ce2.person_id
     AND ce2a.valid_until_dt_tm > sysdate
     AND ce2a.view_level=1
     AND ce2a.result_status_cd IN (mf_cs8_altered_cd, mf_cs8_modified_cd, mf_cs8_auth_cd,
    mf_cs8_active_cd)
     AND ce2a.event_cd=ce2.event_cd)))
  ...
  DETAIL
   ...
   blob->list[blob->cntblob].event_id = c.event_id, blob->list[blob->cntblob].source = trim(
    uar_get_code_display(ces.source_type_cd),3), blob->list[blob->cntblob].birth_wt = ce1.result_val
   IF (cnvtreal(ce2.result_val) > 0)
    mf_weight = (cnvtreal(ce2.result_val) * 2.20462), ml_weight_lbs = cnvtint(mf_weight),
    ml_weight_oz = cnvtint(round(((mf_weight - ml_weight_lbs) * 16),0))
    IF (ml_weight_oz=16)
     ml_weight_lbs += 1, ml_weight_oz = 0
    ENDIF
    blob->list[blob->cntblob].birth_wt = build3(3,ml_weight_lbs," lbs ",ml_weight_oz," oz")
   ENDIF
   blob->list[blob->cntblob].gestage = pp.gest_age_at_birth, blob->list[blob->cntblob].dob = trim(
    datebirthformat(p.birth_dt_tm,p.birth_tz,p.birth_prec_flag,"@SHORTDATETIME"),3)
   IF (ceb.compression_cd=mf_cs120_ocfcompression)
    ...
    ->cntblob].organism=null)) OR (((findstring("NO ANAEROBES ISOLATED",blob->list[blob->cntblob].
     organism,1,0) > 0) OR (((findstring("NO METHICILLIN RESISTANT STAPHYLOCOCCUS AUREUS ISOLATED",
     blob->list[blob->cntblob].organism,1,0) > 0) OR (findstring(
     "No significant microorganisms isolated",blob->list[blob->cntblob].organism,1,0) > 0)) )) )) ))
    )
 ...
 SELECT INTO  $OUTDEV
  ...
  birth_weight = substring(1,30,blob->list[d1.seq].birth_wt), gestation_age = blob->list[d1.seq].
  gestage, discharge_date = substring(1,30,blob->list[d1.seq].discharge_dt)
