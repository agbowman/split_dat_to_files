CREATE PROGRAM cps_add_sum_sheet:dba
 ...
 INSERT  FROM summary_sheet css
  SET css.summary_sheet_id = reply->summary_sheet_id, css.prsnl_id = request->prsnl_id, css.display
    = request->display,
   css.description = request->description, css.active_ind = 1, css.active_status_cd = active_code,
 ...
 IF (curqual <= 0)
  ...
  GO TO check_error
 ENDIF
 SET stat = alter(reply->sect,request->section_qual)
 FOR (i = 1 TO request->section_qual)
   SET cdf_meaning = fillstring(12," ")
   SET cdf_meaning = cnvtupper(request->section[i].subj_area_mean)
   SET code_set = 12004
   SET code_value = 0
   EXECUTE cpm_get_cd_for_cdf
   SET subj_area_cd = code_value
   SET cdf_meaning = fillstring(12," ")
   SET cdf_meaning = cnvtupper(request->section[i].section_type_mean)
   SET code_set = 12001
   SET code_value = 0
   EXECUTE cpm_get_cd_for_cdf
   ...
   INSERT  FROM summary_section ss
    SET ss.section_id = reply->sect[i].section_id, ss.prsnl_id = 0, ss.display = request->section[i].
     display,
   ...
   IF (curqual <= 0)
    ...
    GO TO check_error
   ENDIF
   IF ((request->section[i].sequence > 0))
    ...
    INSERT  FROM summary_section_r cds
     SET cds.summary_sheet_id = reply->summary_sheet_id, cds.section_id = reply->sect[i].section_id,
      cds.sequence = request->section[i].sequence,
      cds.default_expand_ind = 0, cds.active_ind = 1, cds.active_status_cd = active_code,
    ...
    ENDIF
   ENDIF
   IF ((request->section[i].attr_qual > 0))
    FOR (j = 1 TO request->section[i].attr_qual)
      SET cdf_meaning = fillstring(12," ")
      SET cdf_meaning = cnvtupper(request->section[i].attr[j].subj_area_dtl_mean)
      SET code_set = 12005
      SET code_value = 0
      EXECUTE cpm_get_cd_for_cdf
      SET subj_area_dtl_cd = code_value
      SET cdf_meaning = fillstring(12," ")
      SET cdf_meaning = cnvtupper(request->section[i].attr[j].detail_type_mean)
      SET code_set = 12002
      SET code_value = 0
      EXECUTE cpm_get_cd_for_cdf
      ...
      INSERT  FROM section_attribute csa
       SET csa.section_id = reply->sect[i].section_id, csa.column_num = request->section[i].attr[j].
        col_num, csa.subj_area_dtl_cd = subj_area_dtl_cd,
        csa.width =
        IF ((request->section[i].attr[j].width > 0)) request->section[i].attr[j].width
        ELSE 0
        ENDIF
        , csa.detail_type_cd = detail_type_cd, csa.detail_value = request->section[i].attr[j].
        detail_value,
        csa.output_mask = " ", csa.sort_direction_cd = 0, csa.height = 0,
        csa.sep_string = " ", csa.sep_str_length = 0, csa.trim_type_cd = 0,
        csa.trim_char = " ", csa.row_num = 0, csa.detail_sequence = request->section[i].attr[j].
        col_num,
   ...
   ENDIF
 ENDFOR
 SET stat = alter(reply->parent,(request->sect_with_child+ 10))
 FOR (k = 1 TO request->sect_with_child)
   ...
   SELECT INTO "nl:"
    ...
    FROM summary_section ss,
     code_value c
    PLAN (c
     WHERE c.cdf_meaning=cnvtupper(request->parent[k].parent_sect_mean)
   ...
   ENDIF
   SET count = 0
   SET stat = alterlist(reply->parent[k].child,request->parent[k].child_qual)
   SET table_name = "summary_section_child_ids"
   SELECT INTO "nl:"
    ss.section_id, c.code_value
    FROM summary_section ss,
     (dummyt d1  WITH seq = value(request->parent[k].child_qual)),
     code_value c
    PLAN (d1)
     JOIN (c
     WHERE c.cdf_meaning=cnvtupper(request->parent[k].child[d1.seq].child_sect_mean)
      AND c.code_set=12004
      AND c.active_ind=1)
     JOIN (ss
     WHERE c.code_value=ss.subject_area_cd
   ...
   INSERT  FROM section_section_r csr,
    ...
    SET csr.parent_id = reply->parent[k].parent_sect_id, csr.child_id = reply->parent[k].child[d1.seq
     ].child_sect_id, csr.sequence = request->parent[k].child[d1.seq].sequence,
...
#check_error
 ...
 GO TO end_program
#end_program
 SET pco_script_version = "002 10/03/02 SF3151"
END GO
