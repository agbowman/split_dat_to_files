CREATE PROGRAM dm_pft_chrg_grp_rprcs_hst_rows:dba
 ...
 SET reply->table_name = "PFT_CHRG_GRP_RPRCS_HIST"
 ...
 FOR (v_token_idx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF ((v_days_to_keep <= (curdate - cnvtdate(cnvtlookbehind("2,Y")))))
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(pcgrh.pft_chrg_grp_rprcs_hist_id)
   FROM pft_chrg_grp_rprcs_hist pcgrh
   WHERE pcgrh.pft_chrg_grp_rprcs_hist_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(pcgrh.pft_chrg_grp_rprcs_hist_id)
  FROM pft_chrg_grp_rprcs_hist pcgrh
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    pcgrh.rowid
    FROM pft_chrg_grp_rprcs_hist pcgrh
    WHERE pcgrh.pft_chrg_grp_rprcs_hist_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("pcgrh.pft_chrg_grp_rprcs_hist_id != 0","pcgrh"))
     AND pcgrh.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rowcnt += 1
     IF (mod(rowcnt,100)=1)
      stat = alterlist(reply->rows,(rowcnt+ 99))
     ENDIF
     reply->rows[rowcnt].row_id = pcgrh.rowid
    WITH nocounter, maxqual(pcgrh,value(rowsleft))
   ;end select
