CREATE PROGRAM bhs_tracy:dba
 PROMPT
  "Send Message to Screen" = "MINE",
  "Patient:" = "",
  "Enter EDD - Start Date:" = "SYSDATE",
  "Enter EDD - End Date:" = "SYSDATE",
  "Location" = 0,
  "Output to File/Printer/MINE" = "MINE"
  WITH messagetoscreen, lstperson, sdate,
  edate, location, outdev
 ...
 DECLARE mf_amb_cd = vc WITH protct, constant("AMBULATORY")
 ...
 DECLARE mf_masnobgyn_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",220,
   "BAYSTATEMASONSQ"))
 DECLARE mf_cd_set = i4 WITH protect, constant(220)
 ...
 DECLARE mf_cln_sum_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",14003,
   "CLINICIANSUMMARY"))
 ...
 DECLARE mf_sensitive_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",12033,"SENSITIVE"))
 ...
 DECLARE ml_ord_flag_0 = i2 WITH protect, constant(0)
 DECLARE ml_ord_flag_1 = i2 WITH protect, constant(1)
 DECLARE ml_ord_flag_2 = i2 WITH protect, constant(2)
 DECLARE ml_ord_flag_3 = i2 WITH protect, constant(3)
 ...
 DECLARE mf_discont_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",6004,"DISms_continue"
   ))
 ...
 DECLARE ml_head_ind = vc WITH protect, noconstant("N")
 ...
 DECLARE ml_tcnt = i2 WITH protect, noconstant(0)
 DECLARE ml_g = vc WITH protect, noconstant(" ")
 DECLARE ml_p = vc WITH protect, noconstant(" ")
 DECLARE ml_l = vc WITH protect, noconstant(" ")
 DECLARE ml_a = vc WITH protect, noconstant(" ")
 DECLARE ml_f = vc WITH protect, noconstant(" ")
 DECLARE ml_pt = vc WITH protect, noconstant(" ")
 ...
 DECLARE ml_pgc = i2 WITH protect, noconstant(0)
 ...
 SET md_beg_dt_tm = cnvtdatetime( $SDATE)
 SET md_end_dt_tm = cnvtdatetime( $EDATE)
 SET mf_location_cd = uar_get_code_display( $LOCATION)
 CALL echo("Validating User Input")
 SET md_start_date = cnvtdatetime(format(datetimefind(cnvtlookbehind("9M",cnvtdatetime(curdate,0)),
    "M","B","E"),";;Q"))
 IF (datetimediff(cnvtdatetime(md_beg_dt_tm),cnvtdatetime(md_start_date)) > 271.0)
  CALL echo("Hitting 271 rule")
  SET ms_output =  $MESSAGETOSCREEN
  SELECT INTO  $MESSAGETOSCREEN
   FROM dummyt
   HEAD REPORT
    ms_msg1 = "The Start Date range extends beyond 270 days .", ms_msg2 =
    "  Please Enter a date within the 270 days.", col 0,
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE pe.pregnancy_id=pi.pregnancy_id
    AND pe.est_delivery_dt_tm BETWEEN cnvtdatetime(md_beg_dt_tm) AND cnvtdatetime(md_end_dt_tm)
    AND pe.method_cd=mf_method_cd)
  ORDER BY pi.person_id
  HEAD REPORT
   ml_pat_cnt = 0
  DETAIL
   ml_pat_cnt = (ml_pat_cnt+ 1), stat = alterlist(prentl_sum->patient,ml_pat_cnt), prentl_sum->
   patient[ml_pat_cnt].mf_person_id = pi.person_id,
   prentl_sum->patient[ml_pat_cnt].ms_edd_dt = format(pe.est_delivery_dt_tm,"MM/DD/YY;;d"),
   prentl_sum->patient[ml_pat_cnt].ms_edd_method = uar_get_code_display(pe.method_cd)
  WITH nocounter
 ;end select
 IF (curqual < 1)
  GO TO exit_script
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE pe.pregnancy_id=pi.pregnancy_id
    AND pe.est_delivery_dt_tm BETWEEN cnvtdatetime(md_beg_dt_tm) AND cnvtdatetime(md_end_dt_tm)
    AND pe.method_cd=mf_lmp2_cd)
  ORDER BY pi.person_id
  HEAD REPORT
   ml_pat_cnt = 0
  DETAIL
   ml_pat_cnt = (ml_pat_cnt+ 1), stat = alterlist(prentl_sum->patient,ml_pat_cnt)
   IF (pe.method_cd > 0)
    prentl_sum->patient[ml_pat_cnt].ms_lmp_dt = format(pe.method_dt_tm,"MM/DD/YY;;d"), ms_lmp_ind =
    "Y"
   ENDIF
  WITH nocounter
 ;end select
 ...
 SELECT DISTINCT INTO "NL:"
  ...
  FROM (dummyt d  WITH seq = size(prentl_sum->patient,5)),
   ...
   person p
  PLAN (cv
   WHERE (cv.code_value= $LOCATION)
    AND cv.code_set=mf_cd_set
    AND cv.cdf_meaning=mf_amb_cd
    AND cv.active_ind=1)
   JOIN (d)
   JOIN (p
   WHERE (p.person_id=prentl_sum->patient[d.seq].mf_person_id))
   JOIN (e
  ...
  HEAD REPORT
   stat = 0, ml_e_cnt = 0
  DETAIL
   ml_e_cnt = (ml_e_cnt+ 1), stat = alterlist(prentl_sum->patient,ml_e_cnt), prentl_sum->patient[
   ml_e_cnt].mf_person_id = e.person_id,
   prentl_sum->patient[ml_e_cnt].mf_encntr_id = e.encntr_id, prentl_sum->patient[ml_e_cnt].
   ms_account_no = ea.alias, prentl_sum->patient[ml_e_cnt].ms_prsn_full = p.name_full_formatted,
   prentl_sum->patient[ml_e_cnt].ms_dob = format(p.birth_dt_tm,"mm/dd/yy;;d"), prentl_sum->patient[
   ml_e_cnt].ms_mrn = ea1.alias, prentl_sum->patient[ml_e_cnt].ms_lang = uar_get_code_display(p
    .language_cd),
   prentl_sum->patient[ml_e_cnt].mf_fac = e.location_cd
  WITH nocounter
 ;end select
 IF (curqual < 1)
  GO TO exit_script
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (dfa
   WHERE (dfa.person_id=prentl_sum->patient[d.seq].mf_person_id)
  ...
  HEAD REPORT
   ml_f_cnt = 0
  DETAIL
   ml_f_cnt = (ml_f_cnt+ 1), stat = alterlist(work_form->forms,ml_f_cnt), work_form->forms[ml_f_cnt].
   mf_person_id = dfa.person_id,
   work_form->forms[ml_f_cnt].mf_encntr_id = dfa.encntr_id, work_form->forms[ml_f_cnt].
   mf_dcp_forms_actv_id = dfa.dcp_forms_activity_id, work_form->forms[ml_f_cnt].ms_dcp_desc = dfa
   .description,
   work_form->forms[ml_f_cnt].ms_reference_nbr = trim(build2(dfa.dcp_forms_activity_id,"*"),3),
   work_form->forms[ml_f_cnt].ms_activity_dt = format(dfa.form_dt_tm,"MM-DD-YYYY hh:mm:ss;;d")
  WITH nocounter
 ;end select
 IF (curqual < 1)
  GO TO exit_script
 ...
 SELECT DISTINCT INTO "nl:"
  FROM (dummyt d  WITH seq = size(work_form->forms,5)),
   (dummyt d1  WITH seq = size(prentl_sum->patient,5)),
   clinical_event ce
  PLAN (d)
   JOIN (d1
   WHERE (prentl_sum->patient[d1.seq].mf_person_id=work_form->forms[d.seq].mf_person_id))
   JOIN (ce
  ...
  HEAD d.seq
   ml_acog_cnt = 0
  DETAIL
   ml_acog_cnt = (ml_acog_cnt+ 1), stat = alterlist(acog_data->acog,ml_acog_cnt)
   IF (ce.event_class_cd=mf_date_cd)
    ...
    ms_temp_val2 = concat(substring(5,2,ms_temp_val),"/",substring(7,2,ms_temp_val),"/",substring(1,4,
      ms_temp_val))
   ENDIF
   acog_data->acog[ml_acog_cnt].mf_encntr_id = ce.encntr_id
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (d)
   JOIN (a
   WHERE (a.person_id=prentl_sum->patient[d.seq].mf_person_id)
  ...
  HEAD d.seq
   ml_a_cnt = 0
  DETAIL
   ml_a_cnt = (ml_a_cnt+ 1), stat = alterlist(prentl_sum->patient[d.seq].allergies,ml_a_cnt)
   IF (ml_a_cnt=1)
    prentl_sum->patient[d.seq].allergies[ml_a_cnt].ms_allg_list = trim(n.source_string,3)
   ELSE
    prentl_sum->patient[d.seq].allergies[ml_a_cnt].ms_allg_list = concat(trim(prentl_sum->patient[d
      .seq].allergies,3),", ",n.source_string)
   ENDIF
  WITH nocounter
 ;end select
 CALL echorecord(prentl_sum)
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (d)
   JOIN (o
   WHERE (o.person_id=prentl_sum->patient[d.seq].mf_person_id)
   ...
   WHERE ce.order_id=o.order_id
    AND o.active_ind=1)
  ORDER BY d.seq, o.catalog_cd
  HEAD d.seq
   ml_l_cnt = 0
  DETAIL
   o.catalog_cd, ml_l_cnt = (ml_l_cnt+ 1), stat = alterlist(prentl_sum->patient[d.seq].lab_data,
    ml_l_cnt),
   prentl_sum->patient[d.seq].lab_data[ml_l_cnt].ms_lab_name = trim(uar_get_code_display(o.catalog_cd
     )), prentl_sum->patient[d.seq].lab_data[ml_l_cnt].ms_lab_rslt = trim(ce.result_val), prentl_sum
   ->patient[d.seq].lab_data[ml_l_cnt].ms_lab_dt = trim(format(o.orig_order_dt_tm,
     "dd-mmm-yyyy hh:mm;;d"))
 ...
 SELECT DISTINCT INTO "nl"
  ...
  FROM problem p,
   nomenclature n,
   (dummyt d  WITH seq = size(prentl_sum->patient,5))
  PLAN (d)
   JOIN (p
   WHERE (p.person_id=prentl_sum->patient[d.seq].mf_person_id)
    AND p.active_ind=1
  ...
  HEAD d.seq
   ml_prob_cnt = 0
  DETAIL
   ml_prob_cnt = (ml_prob_cnt+ 1), stat = alterlist(prentl_sum->patient[d.seq].problem_data,
    ml_prob_cnt)
   IF (p.nomenclature_id > 0)
    prentl_sum->patient[d.seq].problem_data[ml_prob_cnt].ms_problem_list = n.source_string, problem
     = n.source_string
   ELSE
    problem = p.problem_ftdesc, prentl_sum->patient[d.seq].problem_data[ml_prob_cnt].ms_problem_list
     = p.problem_ftdesc
   ENDIF
  WITH nocounter
 ;end select
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (d)
   ...
   mf_pendingrev_cd)
    AND o.template_order_flag IN (ml_ord_flag_0, ml_ord_flag_1)
    AND o.orig_ord_as_flag IN (ml_ord_flag_1, ml_ord_flag_2, ml_ord_flag_3))
   JOIN (od
  ...
  HEAD d.seq
   ml_med_cnt = 0
  DETAIL
   ml_med_cnt = (ml_med_cnt+ 1), stat = alterlist(prentl_sum->patient[d.seq].medication_data,
    ml_med_cnt), prentl_sum->patient[d.seq].medication_data[ml_med_cnt].ms_med_list = o
   .ordered_as_mnemonic
  WITH nocounter
 ;end select
 IF (curqual < 1)
  GO TO exit_script
 ENDIF
 CALL echo("Getting Gravida/Parithy Data")
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (d)
   JOIN (ce
   WHERE (ce.person_id=prentl_sum->patient[d.seq].mf_person_id)
    AND ce.event_cd IN (mf_gravida_cd, mf_parity_cd, mf_abortion_cd, mf_living_cd, mf_fullterm_cd,
   mf_parapreterm_cd)
    AND ce.valid_until_dt_tm >= cnvtdatetime(curdate,curtime3)
    AND ce.result_status_cd IN (mf_auth_cd, mf_altered_cd, mf_modified_cd))
   JOIN (d1
   WHERE (prentl_sum->patient[d1.seq].mf_person_id=ce.person_id))
  ORDER BY ce.event_cd, ce.result_val DESC
  HEAD ce.event_cd
   ml_pt_cnt = 1
  DETAIL
   ce.event_cd, ml_pt_cnt = (ml_pt_cnt+ 1), stat = alterlist(prentl_sum->patient[d1.seq].gar_par,
    ml_pt_cnt)
   IF (trim(ce.result_val) > "0")
    CASE (ce.event_cd)
     OF mf_gravida_cd:
      IF ( NOT (trim(ce.result_val) IN ("0", "", " ", null)))
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_gravida = cnvtint(trim(ce.result_val))
      ELSE
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_gravida = 0
      ENDIF
     OF mf_parity_cd:
      IF ( NOT (trim(ce.result_val) IN ("0", "", " ")))
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_parity = cnvtint(trim(ce.result_val))
      ELSE
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_parity = 0
      ENDIF
     OF mf_living_cd:
      IF ( NOT (trim(ce.result_val) IN ("0", "", " ")))
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_living = cnvtint(trim(ce.result_val))
      ELSE
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_living = 0
      ENDIF
     OF mf_abortion_cd:
      IF ( NOT (trim(ce.result_val) IN ("0", "", " ")))
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_abortion = cnvtint(trim(ce.result_val))
      ELSE
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_abortion = 0
      ENDIF
     OF mf_fullterm_cd:
      CALL echo(build("result value=",ce.result_val))
      IF ( NOT (ce.result_val IN ("0", "", " ")))
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_fullterm = cnvtint(trim(ce.result_val))
      ELSE
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_fullterm = 0
      ENDIF
     OF mf_parapreterm_cd:
      IF ( NOT (ce.result_val IN ("0", "", " ")))
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_parapreterm = cnvtint(trim(ce.result_val))
      ELSE
       prentl_sum->patient[d1.seq].gar_par[ml_pt_cnt].ml_parapreterm = 0
      ENDIF
    ENDCASE
   ENDIF
  WITH nocounter
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   mf_edema_cd)
    ...
    AND ce.valid_until_dt_tm >= cnvtdatetime(curdate,curtime3)
    AND ce.result_status_cd IN (mf_auth_cd, mf_altered_cd, mf_modified_cd))
  ORDER BY d.seq, ce.event_end_dt_tm_os
  HEAD d.seq
   ml_vs_cnt = 1, ml_h_cnt = 1, hold_dt = ce.event_end_dt_tm_os
  DETAIL
   ce.event_end_dt_tm_os, stat = alterlist(prentl_sum->patient[d.seq].visit_summary,ml_vs_cnt)
   CASE (ce.task_assay_cd)
    OF mf_gasage_cd:
     prentl_sum->patient[d.seq].visit_summary[ml_h_cnt].ms_gest_age = trim(ce.result_val)
    OF mf_fundalhg_cd:
     prentl_sum->patient[d.seq].visit_summary[ml_h_cnt].ms_fndal_hght = trim(ce.result_val)
    OF mf_presentation_cd:
     prentl_sum->patient[d.seq].visit_summary[ml_h_cnt].ms_presentation = trim(ce.result_val)
    OF mf_fhr_cd:
     prentl_sum->patient[d.seq].visit_summary[ml_h_cnt].ms_fhr = trim(ce.result_val)
    ...
    OF mf_cervstation_cd:
     ...
     visit_summary[ml_h_cnt].ms_cervical_exam = ms_cerv_data
    OF mf_systolic_cd:
     prentl_sum->patient[d.seq].visit_summary[ml_h_cnt].ms_systolic_bp = trim(ce.result_val)
    OF mf_diastolic_cd:
     prentl_sum->patient[d.seq].visit_summary[ml_h_cnt].ms_diastolic_bp = trim(ce.result_val)
    OF mf_weight_cd:
     prentl_sum->patient[d.seq].visit_summary[ml_h_cnt].ms_weight_kg = trim(ce.result_val)
    OF mf_edema_cd:
     prentl_sum->patient[d.seq].visit_summary[ml_h_cnt].ms_edema = trim(ce.result_val)
   ENDCASE
   prentl_sum->patient[d.seq].visit_summary[ml_h_cnt].ms_vdate = format(ce.event_end_dt_tm,
    "MM/DD/YYYY;;d")
   IF (hold_dt != ce.event_end_dt_tm)
    hold_dt = ce.event_end_dt_tm, ml_vs_cnt = (ml_vs_cnt+ 1), ml_h_cnt = (ml_h_cnt+ 1)
   ENDIF
  WITH nocounter, maxcol = 1500
 ;end select
 IF (curqual < 1)
  GO TO exit_script
 ENDIF
 CALL echo("Getting Clinician Summary Data")
 SELECT DISTINCT INTO "nl:"
  ce.event_end_dt_tm
  FROM (dummyt d  WITH seq = size(prentl_sum->patient,5)),
  ...
  PLAN (d)
   JOIN (d1
   WHERE (work_form->forms[d1.seq].mf_person_id=prentl_sum->patient[d.seq].mf_person_id))
   JOIN (ce
   WHERE (ce.person_id=work_form->forms[d1.seq].mf_person_id)
    ...
    AND ce.task_assay_cd=mf_cln_sum_cd
    AND ce.view_level=1
    AND ce.valid_until_dt_tm >= cnvtdatetime(curdate,curtime3)
  ...
  HEAD d.seq
   ml_cl_cnt = 0
  DETAIL
   ml_blob_size = cnvtint(cb.blob_length), ms_blob_in = fillstring(64000," "), ms_blob_out =
   fillstring(64000," "),
   ms_blob_rtf = fillstring(64000," "), ml_blob_ret_len = 0, ms_blob_in = cb.blob_contents
   IF (cb.compression_cd=mf_ocf_compression_cd)
    CALL uar_ocf_uncompress(ms_blob_in,ml_blob_size,ms_blob_out,64000,ml_blob_ret_len),
    CALL uar_rtf2(ms_blob_out,ml_blob_ret_len,ms_blob_rtf,64000,ml_blob_ret_len,1)
   ELSE
    CALL uar_rtf2(ms_blob_in,ml_blob_size,ms_blob_rtf,64000,ml_blob_ret_len,1)
   ENDIF
   ml_cl_cnt = (ml_cl_cnt+ 1), stat = alterlist(prentl_sum->patient[d.seq].clin_sum,ml_cl_cnt),
   prentl_sum->patient[d.seq].clin_sum[ml_cl_cnt].ms_clin_dt = format(ce.performed_dt_tm,
    "MM/DD/YY;;d"),
   prentl_sum->patient[d.seq].clin_sum[ml_cl_cnt].ms_clin_note = replace(trim(ms_blob_rtf,3)," \par ",
    char(12),0)
  WITH nocounter
 ;end select
 IF (curqual < 1)
 ...
 SUBROUTINE report_headerabs(ncalc,offsetx,offsety)
   ...
   DECLARE __location = vc WITH noconstant(build2(mf_location_cd,char(0))), protect
   IF (ncalc=rpt_render)
 ...
 SUBROUTINE allergy_listabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.344000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_allergies = f8 WITH noconstant(0.0), private
   IF ((prentl_sum->patient[ml_pd_prt].allergies[ml_alg_prt].ms_allg_list != ""))
    DECLARE __allergies = vc WITH noconstant(build2(prentl_sum->patient[ml_pd_prt].allergies[
      ml_alg_prt].ms_allg_list,char(0))), protect
   ENDIF
   IF (bcontinue=0)
    SET _remallergies = 1
   ENDIF
 ...
 SUBROUTINE labheaderabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.438000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
 ...
 SUBROUTINE problemheaderabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.365000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
 ...
 SUBROUTINE clinicalheaderabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.646000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_TRACY"
 ...
 SET ml_garpar = value(size(prentl_sum->patient.gar_par,5))
 FOR (ml_gar_par = 1 TO ml_garpar)
   SET ml_g = trim(cnvtstring(prentl_sum->patient.gar_par[ml_garpar].ml_gravida))
   SET ml_p = trim(cnvtstring(prentl_sum->patient.gar_par[ml_garpar].ml_parity))
   SET ml_l = trim(cnvtstring(prentl_sum->patient.gar_par[ml_garpar].ml_living))
   SET ml_a = trim(cnvtstring(prentl_sum->patient.gar_par[ml_garpar].ml_abortion))
   SET ml_f = trim(cnvtstring(prentl_sum->patient.gar_par[ml_garpar].ml_fullterm))
   SET ml_pt = trim(cnvtstring(prentl_sum->patient.gar_par[ml_garpar].ml_parapreterm))
 ENDFOR
 SET ms_grapar_line = concat("G",ml_g,",","P",ml_p,
  "(",ml_f,",",ml_pt,",",
  ml_a,",",ml_l,")")
 FOR (ml_pd_prt = 1 TO ml_size)
   IF (ml_pd_prt=1)
    SET ml_tcnt = (ml_tcnt+ 1)
   ...
   ELSE
    ...
    SET becont = 0
   ENDIF
   FOR (ml_alg_prt = 1 TO size(prentl_sum->patient[ml_pd_prt].allergies,5))
   ...
   IF ((mf_hld_person != prentl_sum->patient[ml_pd_prt].mf_person_id))
    SELECT INTO "nl:"
     ...
     DETAIL
      ...
      .seq].fin_cnt[ml_t_cnt].ml_tcnt = ml_tcnt,
      mf_hld_person = prentl_sum->patient[ml_pd_prt].mf_person_id, ml_tcnt = 0
     WITH nocounter
    ;end select
    CALL echorecord(prentl_sum)
   ENDIF
 ...
 FOR (ml_pd_prt = 1 TO ml_size)
   ...
   FOR (ml_alg_prt = 1 TO size(prentl_sum->patient[ml_pd_prt].allergies,5))
     IF ((((_yoffset+ allergy_list(rpt_calcheight,remain_space,becont))+ foot_report_section(
      rpt_calcheight)) > page_size))
      SET _yoffset = page_size
      SET ms_hpg = trim(concat("Page ",trim(cnvtstring(ml_pgc))," of ",trim(cnvtstring(prentl_sum->
          patient[ml_pd_prt].fin_cnt[1].ml_tcnt))))
      SET ms_tpg = trim(concat("Page ",cnvtstring(ml_pgc)," of ",trim(cnvtstring(prentl_sum->patient[
          ml_pd_prt].fin_cnt[1].ml_tcnt))))
      SET d0 = subfooter(rpt_render)
      SET d0 = foot_report_section(rpt_render)
      SET d0 = pagebreak(0)
      SET ml_pgc = (ml_pgc+ 1)
     ...
     WHILE (becont=1)
       ...
       SET ms_hpg = trim(concat("Page ",trim(cnvtstring(ml_pgc))," of ",trim(cnvtstring(prentl_sum->
           patient[ml_pd_prt].fin_cnt[1].ml_tcnt))))
       SET ms_tpg = trim(concat("Page ",cnvtstring(ml_pgc)," of ",trim(cnvtstring(prentl_sum->
           patient[ml_pd_prt].fin_cnt[1].ml_tcnt))))
       SET d0 = subfooter(rpt_render)
       SET d0 = foot_report_section(rpt_render)
       SET ml_pgc = (ml_pgc+ 1)
