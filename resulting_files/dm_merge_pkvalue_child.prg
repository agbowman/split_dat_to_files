CREATE PROGRAM dm_merge_pkvalue_child
 ...
 SET stat = alterlist(chg_hold->qual,0)
 SET log_cnt = 0
 SELECT INTO "nl:"
  d.log_id
  FROM dm_chg_log d
  WHERE ((d.pk_where_value+ 0)=0)
   AND cnvtupper(d.log_type) IN ("REFCHG", "MERGED", "ORPHAN", "NORDDS")
   AND d.log_id >= dmpb_min_log
   AND (d.log_id < (dmpb_min_log+ dmpb_batch))
  HEAD REPORT
   log_cnt = 0, stat = alterlist(chg_hold->qual,dmpb_batch)
  DETAIL
   log_cnt += 1, chg_hold->qual[log_cnt].log_id = d.log_id, chg_hold->qual[log_cnt].pk_where = d
   .pk_where
  FOOT REPORT
   stat = alterlist(chg_hold->qual,log_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading pk_where strings from DM_CHG_LOG into record structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  ...
  SET batch_stop = "Y"
 ENDIF
 CALL echo(concat("Processing ",trim(cnvtstring(dmpb_batch))," starting with log_id ",trim(cnvtstring
    (dmpb_min_log)),"."))
 CALL echo(
  "This program can be run in multiple CCL sessions to decrease the amount of processing time.")
 CALL echo("To do so run: DM_MERGE_PKVALUE_BACKFILL GO")
 SET readme_data->status = "F"
 SET readme_data->message = concat("Processing ",trim(cnvtstring(dmpb_batch)),
  " starting with log_id ",trim(cnvtstring(dmpb_min_log)),".")
 IF (dmpb_sa_run_ind=0)
  EXECUTE dm_readme_status
 ENDIF
 SET stat = alterlist(usi_request->qual,0)
 IF (batch_stop="N")
  IF (size(chg_hold->qual,5) > 0)
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = size(chg_hold->qual,5))
    HEAD REPORT
     stat = alterlist(usi_request->qual,size(chg_hold->qual,5))
    DETAIL
     usi_request->qual[d.seq].str_from = chg_hold->qual[d.seq].pk_where
    WITH nocounter
   ;end select
   EXECUTE dm_merge_unique_sql_id
   IF ((usi_reply->err_ind=0))
    SELECT INTO "nl:"
     FROM (dummyt d  WITH seq = size(usi_request->qual,5))
     DETAIL
      chg_hold->qual[d.seq].where_value = cnvtreal(usi_request->qual[d.seq].str_to)
     WITH nocounter
    ;end select
   ELSE
    SET dm_err->err_ind = 1
    ...
    ROLLBACK
    SET readme_data->status = "F"
    SET readme_data->message = usi_reply->err_msg
    SET batch_stop = "Y"
   ENDIF
  ENDIF
  IF (batch_stop="N")
   IF (size(usi_request->qual,5) > 0)
    CALL echo("Updating DM_CHG_LOG with pk_where_value.")
    UPDATE  FROM dm_chg_log c,
      (dummyt d  WITH seq = size(chg_hold->qual,5))
     SET c.pk_where_value = chg_hold->qual[d.seq].where_value
     PLAN (d)
      JOIN (c
      WHERE (chg_hold->qual[d.seq].log_id=c.log_id))
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Error updating pk_where_value in DM_CHG_LOG."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ROLLBACK
     SET readme_data->status = "F"
     SET readme_data->message = dm_err->emsg
