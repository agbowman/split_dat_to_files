CREATE PROGRAM cv_chk_omf_acc_indicator:dba
 SET request->setup_proc[1].success_ind = 1
 SET request->setup_proc[1].error_msg = fillstring(200," ")
 SET request->setup_proc[1].error_msg = "CV_OMF_ACC_INDICATOR Successful.  Count= "
 SET v_count1 = 0
 SET v_expected_count = 121
 SELECT INTO "nl:"
  table_count = count(*)
  FROM cv_omf_indicator coi,
   code_value cv
  WHERE coi.indicator_cd != 0
   AND coi.indicator_cd=cv.code_value
   AND coi.cdf_meaning="ACC*"
  DETAIL
 ...
 IF (v_count1 < v_expected_count)
  ...
  SET request->setup_proc[1].error_msg = concat("CV_OMF_ACC_INDICATOR: Expected ",trim(cnvtstring(
