CREATE PROGRAM bhs_rpt_fn_down_tareport110208:dba
 PROMPT
 ...
 SET history_p_illness = uar_get_code_by("displaykey",72,"HISTORYOFPRESENTILLNESS")
 ...
 IF (operation=1)
  ...
  FOR (li = 1 TO loc->encounter_cnt)
    SET time_marker = format(cnvtlookahead("1,S",cnvtdatetime(curdate,curtime3)),"YYYYMMDDHHMMSS;;D")
    ...
    SELECT INTO "nl:"
     ...
     c_sp_clin_cleared,
     c_sp_cleared_by, pat_clear_f_triage, history_p_illness))
     ...
     HEAD ce.event_cd
      ...
      ELSEIF (ce.event_cd=blood_p_sites)
       pat->blood_dt_tm = ce.event_end_dt_tm, pat->blood_p_sites_f_val = build2(trim(ce.result_val),
        "    (",format(ce.event_end_dt_tm,"MM/DD/YY HH:MM;;;D"),")")
      ELSEIF (ce.event_cd=history_p_illness)
       pat->h_p_i_dt_tm = build2("(",format(ce.event_end_dt_tm,"MM/DD/YY HH:MM;;;D"),")"), pat->
       history_p_illness_val = trim(ce.result_val)
      ENDIF
     DETAIL
    ...
    SELECT INTO value(loc->encounter_info[li].file_name)
     ...
     DETAIL
      ...
      ENDIF
      IF ((pat->history_p_illness_val=null))
       no_data
      ELSE
       y_pos = (y_pos+ 10), row + 1,
       CALL print(calcpos(x_pos,y_pos)),
       b, "History of Present Illness: ", eb,
       pat->h_p_i_dt_tm, y_pos = (y_pos+ 10), first = substring(1,120,pat->history_p_illness_val),
       second = substring(120,250,pat->history_p_illness_val), third = substring(250,300,pat->
        history_p_illness_val), row + 1,
       CALL print(calcpos(x_pos,y_pos)), first
       IF (second != null)
        y_pos = (y_pos+ 10), row + 1,
        CALL print(calcpos(x_pos,y_pos)),
        second
 ...
 IF (operation=0)
  ...
  SELECT INTO value( $OUTDEV)
   ...
   HEAD PAGE
    ...
    CALL print(calcpos(225,20)), "{F/5}{CPI/10}{U}",
    "FirstNet Triage / Assessment Report", "{endu}", row + 1,
    CALL print(calcpos(20,40)), "{F/4}{CPI/11}", b,
    "Patient Name: ", eb, pat->pat_name,
    row + 1,
