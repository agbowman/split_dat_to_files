CREATE PROGRAM acm_write_encounter:dba
 ...
 DECLARE ubound = i4 WITH protect, noconstant(reply->encounter_qual_cnt)
 SET stat = alterlist(reply->encounter_qual,reply->encounter_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->encounter_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->encounter_qual[index].encntr_id = acm_request->encounter_qual[index].encntr_id
     SET reply->encounter_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_encounter
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->act_cnt > 0))
  EXECUTE acm_act_encounter
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_encounter
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_encounter
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
