CREATE PROGRAM bed_aud_micro_ord_event_reltn:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=106
    ...
    AND cv.active_ind=1)
  DETAIL
   micro = cv.code_value, act_type = cv.display
  WITH nocounter
 ;end select
 SELECT INTO "NL:"
  FROM code_value cv
 ...
 IF ((request->skip_volume_check_ind=0))
  SELECT INTO "nl:"
   ...
   PLAN (oc
    WHERE oc.activity_type_cd=micro
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("micro_ord_to_evntcode_evntset.csv")
 ENDIF
