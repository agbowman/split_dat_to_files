CREATE PROGRAM cv_inst_updt_summary_data:dba
 ...
 DECLARE datefmt = vc
 DECLARE timefmt = vc
 DECLARE null_date = vc
 DECLARE datefmt2 = vc
 DECLARE add_ind = vc
 DECLARE chg_ind = vc
 DECLARE del_ind = vc
 DECLARE date = vc
 DECLARE ignore_ind = vc
 SET sum_failed = "F"
 SET datefmt = "dd-mmm-yyyy hh:mm:ss.cc;;d"
 SET timefmt = "HHMM;;d"
 SET null_date = "31-DEC-2100 00:00:00"
 SET datefmt2 = "MM/DD/YYYY;;d"
 SET add_ind = "AUTH"
 SET chg_ind = "MODIFIED"
 SET del_ind = "INERROR"
 SET date = "DATE"
 SET ignore_ind = "IGNORE"
 SET sum_failed = "F"
 SET reply->status_data.status = "F"
 SET proc_cnt = 0
 SET proc_cnt = size(cv_omf_rec->proc_data,5)
 SET cv_omf_rec->proc_start_dt_tm = cnvtdatetime(curdate,curtime3)
 SET cv_omf_rec->proc_dt_num = cnvtdate(cv_omf_rec->proc_start_dt_tm)
 SELECT INTO "NL:"
  cc.cv_case_id, cc.form_event_id
  FROM cv_case cc
 ...
 ;end select
 IF (curqual=0)
  SET cv_omf_rec->status_ind = cv_status_add
  CALL cv_log_message("A new record was sent for insertion! ")
 ELSE
  SET cv_omf_rec->status_ind = cv_status_del
  CALL cv_log_message("A previous record was sent for update or delete!")
 ENDIF
 DECLARE acc_ind = f8 WITH public, noconstant(0.0)
 DECLARE calling_calc = i2 WITH public, noconstant(0)
 SET calling_calc = getcvcontrol(cv_omf_rec->dataset[1].dataset_id,"CV_FLAG_CTRL_ACC_OR_STS_ETC")
 IF (calling_calc=1)
  SET acc_ind = cv_omf_rec->dataset[1].dataset_id
  EXECUTE cv_del_summary_calc_data
 ENDIF
 IF ((cv_omf_rec->status_ind=cv_status_add))
  DECLARE inerror_cd = f8 WITH protect, noconstant(0.0)
  DECLARE inerror_mean = vc WITH protect, constant("INERROR")
  DECLARE inerror_cs = i4 WITH protect, constant(8)
  DECLARE iret = i2
  SET register_cnt = size(register->rec,5)
  SET iret = uar_get_meaning_by_codeset(inerror_cs,inerror_mean,1,inerror_cd)
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(register_cnt))
   WHERE (register->rec[d.seq].result_status_cd=inerror_cd)
   WITH nocounter
  ;end select
  IF (curqual=register_cnt)
   CALL cv_log_message("This is the case of uncharting, exiting")
   GO TO commit_rollback_section
  ENDIF
 ENDIF
 IF ((cv_omf_rec->status_ind=cv_status_del))
  SELECT INTO "nl:"
   cp.procedure_id, cp.event_type_cd
   FROM cv_procedure cp,
    (dummyt d  WITH seq = value(size(cv_omf_rec->proc_data,5)))
   PLAN (d
    WHERE (cv_omf_rec->proc_data[d.seq].event_type_cd != 0.0)
     AND (cv_omf_rec->case_id != 0.0))
    JOIN (cp
    WHERE (cp.cv_case_id=cv_omf_rec->case_id)
     AND cp.procedure_id != 0.0
     AND (cp.event_type_cd=cv_omf_rec->proc_data[d.seq].event_type_cd))
   HEAD REPORT
    pro_type_cnt = 0
   DETAIL
    pro_type_cnt = (pro_type_cnt+ 1), cv_omf_rec->proc_data[d.seq].procedure_id = cp.procedure_id,
    cv_omf_rec->proc_data[d.seq].num_lesion = cp.num_lesion,
    cv_omf_rec->proc_data[d.seq].status_ind = cv_status_del
   WITH nocounter
  ;end select
  IF (curqual=0)
   SET cv_log_level = cv_log_debug
   CALL cv_log_message("Failed in select cv_procedure table, program continue!")
  ENDIF
  SELECT INTO "nl:"
   cl.lesion_id, cl.procedure_id
   FROM cv_lesion cl,
   ...
   PLAN (d1)
    ...
    JOIN (cl
    WHERE (cl.procedure_id=cv_omf_rec->proc_data[d1.seq].procedure_id)
     AND cl.lesion_id != 0.0
     AND (cl.parent_event_id=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].parent_event_id))
   DETAIL
    cv_omf_rec->proc_data[d1.seq].status_ind = cv_status_del, cv_omf_rec->proc_data[d1.seq].lesion[d2
    .seq].lesion_id = cl.lesion_id, cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].status_ind =
    cv_status_del
   WITH nocounter
  ;end select
  IF (curqual=0)
   SET cv_log_level = cv_log_debug
   CALL cv_log_message("Failed in select cv_lesion for lesion_id!")
  ENDIF
  DECLARE l_auth = c12 WITH public, constant("AUTH")
  DECLARE auth_dta = f8 WITH public, noconstant(0.0)
  DECLARE iret = i2 WITH public, noconstant(0)
  SET iret = uar_get_meaning_by_codeset(8,l_auth,1,auth_dta)
  DECLARE l_modify = c12 WITH public, constant("MODIFIED")
  DECLARE modify_dta = f8 WITH public, noconstant(0.0)
  DECLARE iret = i2 WITH public, noconstant(0)
  SET iret = uar_get_meaning_by_codeset(8,l_modify,1,modify_dta)
  SELECT INTO "nl:"
   ccad.event_cd
   FROM cv_case_abstr_data ccad,
    (dummyt d  WITH seq = value(size(cv_omf_rec->case_abstr_data,5)))
   PLAN (d
    WHERE (cv_omf_rec->case_abstr_data[d.seq].result_status_cd=modify_dta))
    JOIN (ccad
    WHERE (ccad.event_cd=cv_omf_rec->case_abstr_data[d.seq].event_cd)
     AND (ccad.cv_case_id=cv_omf_rec->case_id))
   DETAIL
    cv_omf_rec->case_abstr_data[d.seq].ins_upd_ind = 1
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(size(cv_omf_rec->case_abstr_data,5)))
   PLAN (d
    WHERE (cv_omf_rec->case_abstr_data[d.seq].result_status_cd=modify_dta)
     AND (cv_omf_rec->case_abstr_data[d.seq].ins_upd_ind=0))
   DETAIL
    cv_omf_rec->case_abstr_data[d.seq].result_status_cd = auth_dta, cv_omf_rec->case_abstr_data[d.seq
    ].result_status_meaning = l_auth
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   cpad.event_cd
   FROM (dummyt d1  WITH seq = value(proc_cnt)),
   ...
   PLAN (d1)
    ...
    WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].proc_abstr_data,5)
     AND (cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].result_status_cd=modify_dta))
    JOIN (cpad
    WHERE (cpad.event_cd=cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].event_cd)
     AND (cpad.procedure_id=cv_omf_rec->proc_data[d1.seq].procedure_id))
   DETAIL
    cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].ins_upd_ind = 1
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   FROM (dummyt d1  WITH seq = value(proc_cnt)),
   ...
   PLAN (d1)
    ...
    WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].proc_abstr_data,5)
     AND (cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].result_status_cd=modify_dta)
     AND (cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].ins_upd_ind=0))
   DETAIL
    cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].result_status_cd = auth_dta, cv_omf_rec->
    proc_data[d1.seq].proc_abstr_data[d2.seq].result_status_meaning = l_auth
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   clad.event_cd
   ...
   PLAN (d1)
    ...
    WHERE d3.seq <= size(cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data,5)
     AND (cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].result_status_cd=
    modify_dta))
    JOIN (clad
    WHERE (clad.event_cd=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].event_cd
    )
     AND (clad.lesion_id=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id))
   DETAIL
    cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].ins_upd_ind = 1
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   FROM (dummyt d1  WITH seq = value(proc_cnt)),
   ...
   PLAN (d1)
    ...
    WHERE d3.seq <= size(cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data,5)
     AND (cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].result_status_cd=
    modify_dta)
     AND (cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].ins_upd_ind=0))
   DETAIL
    cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].result_status_cd = auth_dta,
    cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].result_status_meaning =
    l_auth
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (ccdr
    WHERE (ccdr.dataset_id=cv_omf_rec->dataset[d.seq].dataset_id)
     AND (ccdr.cv_case_id=cv_omf_rec->case_id))
   DETAIL
    cv_omf_rec->dataset[d.seq].status_ind = cv_status_chg
   WITH nocounter
  ;end select
  IF (curqual=0)
   SET cv_log_level = cv_log_info
   CALL cv_log_message("Failed in select CV_CASE_DATASET_R for dataset_id, program continue!")
  ENDIF
  SELECT INTO "nl:"
   FROM cv_case cc
   WHERE (cc.form_event_id=cv_omf_rec->form_event_id)
    AND cc.form_event_id != 0.0
   DETAIL
    cv_omf_rec->proc_dt_num = cc.proc_dt_nbr, cv_omf_rec->num_proc = cc.num_proc, cv_omf_rec->
    admit_ind = cc.pat_adm_ind,
    cv_omf_rec->disch_ind = cc.pat_disch_ind, cv_omf_rec->death_ind = cc.death_ind, cv_omf_rec->
    los_adm_proc = cc.los_adm_proc,
    cv_omf_rec->los_proc_disch = cc.los_proc_disch
   WITH nocounter
  ;end select
  IF (curqual=0)
   SET cv_log_level = cv_log_info
   CALL cv_log_message("Failed in select cv_case for loading exist data, program continue!")
  ELSE
   SET cv_omf_rec->proc_start_dt_tm = cnvtdatetime(cv_omf_rec->proc_dt_num,1)
  ENDIF
 ELSE
  SELECT INTO "nl:"
  ...
  IF (curqual=0)
   ...
   SET sum_failed = "T"
   CALL cv_log_message("Failed in select case_id from the dual table, program exit!")
   GO TO commit_rollback_section
  ENDIF
 ENDIF
 FOR (proc_idx = 1 TO proc_cnt)
   IF ((cv_omf_rec->proc_data[proc_idx].status_ind != cv_status_del))
    SELECT INTO "nl:"
     nextseqnbr = seq(card_vas_seq,nextval)
     FROM dual
     DETAIL
      cv_omf_rec->proc_data[proc_idx].procedure_id = nextseqnbr, cv_omf_rec->num_proc = (cv_omf_rec->
      num_proc+ 1)
     WITH format, nocounter
    ;end select
    IF (curqual=0)
     SET cv_log_level = cv_log_info
     CALL cv_log_message("Failed in select dual table for proc_id, program continue!")
    ENDIF
   ENDIF
   SET les_cnt = size(cv_omf_rec->proc_data[proc_idx].lesion,5)
   FOR (les_idx = 1 TO les_cnt)
     IF ((cv_omf_rec->proc_data[proc_idx].lesion[les_idx].status_ind != cv_status_del))
      SELECT INTO "nl:"
       nextseqnbr = seq(card_vas_seq,nextval)
       FROM dual
       DETAIL
        cv_omf_rec->proc_data[proc_idx].lesion[les_idx].lesion_id = nextseqnbr, cv_omf_rec->
        proc_data[proc_idx].num_lesion = (cv_omf_rec->proc_data[proc_idx].num_lesion+ 1)
       WITH format, nocounter
      ;end select
      IF (curqual=0)
       SET cv_log_level = cv_log_info
       CALL cv_log_message("Failed in select dual table for lesion_id, program continue!")
      ENDIF
     ENDIF
   ENDFOR
 ENDFOR
 ...
 CALL echorecord(cv_omf_rec,"cer_temp:cv_omf_rec_mid.dat")
 IF ((cv_omf_rec->status_ind=cv_status_add))
  INSERT  FROM cv_case cc
   SET cc.cv_case_id = cv_omf_rec->case_id, cc.form_event_id = cv_omf_rec->form_event_id, cc.num_proc
    ...
    cc.age = cv_omf_rec->age_year, cc.age_group_cd = cv_omf_rec->age_group_cd, cc.sex_cd = cv_omf_rec
    ->sex_cd,
    ...
    cc.active_status_prsnl_id = reqinfo->updt_id, cc.data_status_dt_tm = cnvtdatetime(curdate,
    ...
    updt_applctx,
    cc.updt_cnt = 0, cc.updt_req = reqinfo->updt_req, cc.updt_id = reqinfo->updt_id
   WHERE (cv_omf_rec->case_id != 0.0)
   WITH nocounter
  ;end insert
  IF (curqual=0)
   SET cv_log_level = cv_log_audit
   SET sum_failed = "T"
   CALL cv_log_message("Failed in insert cv_case table, program exit!")
   GO TO commit_rollback_section
  ENDIF
 ENDIF
 INSERT  FROM cv_case_dataset_r ccdr,
   (dummyt d  WITH seq = value(size(cv_omf_rec->dataset,5)))
  SET ccdr.case_dataset_r_id = seq(card_vas_seq,nextval), ccdr.cv_case_id = cv_omf_rec->case_id, ccdr
   .dataset_id = cv_omf_rec->dataset[d.seq].dataset_id,
   ccdr.participant_nbr = cv_omf_rec->dataset[d.seq].participant_nbr, ccdr.status_cd = 0, ccdr
   .active_ind = 1,
   ccdr.active_status_dt_tm = cnvtdatetime(curdate,curtime3), ccdr.active_status_cd = reqdata->
   active_status_cd, ccdr.beg_effective_dt_tm = cnvtdatetime(curdate,curtime3),
   ccdr.end_effective_dt_tm = cnvtdatetime(null_date), ccdr.data_status_cd = reqdata->data_status_cd,
   ccdr.data_status_prsnl_id = reqinfo->updt_id,
   ccdr.active_status_prsnl_id = reqinfo->updt_id, ccdr.data_status_dt_tm = cnvtdatetime(curdate,
    curtime3), ccdr.updt_dt_tm = cnvtdatetime(curdate,curtime3),
   ccdr.updt_task = reqinfo->updt_task, ccdr.updt_app = reqinfo->updt_app, ccdr.updt_applctx =
   reqinfo->updt_applctx,
   ccdr.updt_cnt = 0, ccdr.updt_req = reqinfo->updt_req, ccdr.updt_id = reqinfo->updt_id
  PLAN (d
   WHERE (cv_omf_rec->dataset[d.seq].status_ind=cv_status_add)
    AND (cv_omf_rec->dataset[d.seq].dataset_id > 0))
   JOIN (ccdr)
  WITH nocounter
 ;end insert
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in insert cv_case_dataset_r table, program continue!")
 ENDIF
 SELECT INTO "nl:"
  ccad.event_id
  FROM cv_case_abstr_data ccad,
  ...
  PLAN (d)
   JOIN (ccad
   WHERE (cv_omf_rec->case_abstr_data[d.seq].event_id=ccad.event_id)
    AND trim(cv_omf_rec->case_abstr_data[d.seq].result_status_meaning)=add_ind)
  DETAIL
   cv_omf_rec->case_abstr_data[d.seq].result_status_meaning = ignore_ind
  WITH nocounter
 ;end select
 INSERT  FROM cv_case_abstr_data ccad,
  ...
  PLAN (d
   WHERE trim(cv_omf_rec->case_abstr_data[d.seq].result_status_meaning)=add_ind)
   JOIN (ccad)
  WITH nocounter
 ;end insert
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("failed in insert cv_case_abstr_data table, program continue!")
 ENDIF
 INSERT  FROM cv_procedure cp,
  ...
  SET cp.procedure_id = cv_omf_rec->proc_data[d.seq].procedure_id, cp.cv_case_id = cv_omf_rec->
   ...
   cp.los_adm_proc = cv_omf_rec->proc_data[d.seq].los_adm_proc, cp.los_proc_disch = cv_omf_rec->
   proc_data[d.seq].los_proc_disch, cp.num_lesion = cv_omf_rec->proc_data[d.seq].num_lesion,
   cp.active_ind = 1, cp.active_status_dt_tm = cnvtdatetime(curdate,curtime3), cp.active_status_cd =
   reqdata->active_status_cd,
   cp.beg_effective_dt_tm = cnvtdatetime(curdate,curtime3), cp.end_effective_dt_tm = cnvtdatetime(
    null_date), cp.data_status_cd = reqdata->data_status_cd,
   cp.data_status_prsnl_id = reqinfo->updt_id, cp.active_status_prsnl_id = reqinfo->updt_id, cp
   .data_status_dt_tm = cnvtdatetime(curdate,curtime3),
   cp.updt_dt_tm = cnvtdatetime(curdate,curtime3), cp.updt_task = reqinfo->updt_task, cp.updt_app =
   reqinfo->updt_app,
   cp.updt_applctx = reqinfo->updt_applctx, cp.updt_cnt = 0, cp.updt_req = reqinfo->updt_req,
   cp.updt_id = reqinfo->updt_id
  PLAN (d
   WHERE (cv_omf_rec->proc_data[d.seq].status_ind=cv_status_add)
    AND (cv_omf_rec->proc_data[d.seq].procedure_id != 0.0))
   JOIN (cp)
  WITH nocounter
 ;end insert
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in insert cv_procedure table, program continue!")
 ENDIF
 SELECT INTO "nl:"
  cpad.event_id
  FROM (dummyt d1  WITH seq = value(proc_cnt)),
  ...
  PLAN (d1)
   JOIN (d2
   WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].proc_abstr_data,5)
    AND trim(cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].result_status_meaning)=add_ind)
   JOIN (cpad
   WHERE (cpad.event_id=cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].event_id))
  DETAIL
   cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].result_status_meaning = ignore_ind
  WITH nocounter
 ;end select
 INSERT  FROM cv_proc_abstr_data cpad,
   (dummyt d1  WITH seq = value(proc_cnt)),
   (dummyt d2  WITH seq = value(cv_omf_rec->max_proc_abstr))
  SET cpad.proc_abstr_data_id = seq(card_vas_seq,nextval), cpad.procedure_id = cv_omf_rec->proc_data[
   d1.seq].proc_abstr_data[d2.seq].procedure_id, cpad.event_cd = cv_omf_rec->proc_data[d1.seq].
   proc_abstr_data[d2.seq].event_cd,
   cpad.nomenclature_id = cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].nomenclature_id, cpad
   .result_id = cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].result_id, cpad.event_id =
 ...
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in insert cv_proc_abstr_data table, program continue!")
 ENDIF
 INSERT  FROM cv_lesion cl,
  ...
  SET cl.lesion_id = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id, cl.procedure_id =
   ...
   cl.data_status_prsnl_id = reqinfo->updt_id, cl.active_status_prsnl_id = reqinfo->updt_id, cl
   .data_status_dt_tm = cnvtdatetime(curdate,curtime3),
  ...
  PLAN (d1)
   JOIN (d2
   WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].lesion,5)
    AND (cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].status_ind=cv_status_add)
    AND (cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id != 0.0))
   JOIN (cl)
  WITH nocounter
 ;end insert
 IF (curqual=0)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   JOIN (d3
   WHERE d3.seq <= size(cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data,5)
    AND trim(cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].
    result_status_meaning)=add_ind)
   JOIN (clad
   WHERE (clad.event_id=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].event_id)
   )
  DETAIL
   cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].result_status_meaning =
   ignore_ind
 ...
 INSERT  FROM cv_les_abstr_data clad,
   ...
   (dummyt d2  WITH seq = value(cv_omf_rec->max_lesion)),
   (dummyt d3  WITH seq = value(cv_omf_rec->max_lesion_abstr))
  SET clad.les_abstr_data_id = seq(card_vas_seq,nextval), clad.lesion_id = cv_omf_rec->proc_data[d1
   .seq].lesion[d2.seq].les_abstr_data[d3.seq].lesion_id, clad.event_type_cd = cv_omf_rec->proc_data[
   d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].event_type_cd,
   clad.event_cd = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].event_cd, clad
   .nomenclature_id = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].
   nomenclature_id, clad.result_id = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3
   .seq].result_id,
   clad.event_id = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].event_id, clad
   .result_val = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].result_val, clad
   .result_dt_tm = cnvtdatetime(cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].
    result_dt_tm),
 ...
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in insert cv_les_abstr_data table, program continue!")
 ENDIF
 IF ((cv_omf_rec->status_ind != cv_status_del))
  GO TO commit_rollback_section
 ENDIF
 ...
 ;end delete
 IF (curqual=0)
  CALL cv_log_message("Failed in deleting cv_les_abstr_data table, program continue!")
 ELSE
  CALL cv_log_message("Cv_les_abstr_data record was deleted, program continue!")
 ENDIF
 SELECT INTO "nl:"
  clad.lesion_id
  FROM cv_les_abstr_data clad,
  ...
  PLAN (d1)
   ...
   WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].lesion,5)
    AND (cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id != 0.0))
   JOIN (clad
   WHERE (clad.lesion_id=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id))
  DETAIL
   cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].status_ind = cv_status_chg
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in selecting cv_les_abstr_data table, program continue!")
 ENDIF
 SELECT INTO "nl:"
  cl.lesion_id
  FROM cv_lesion cl,
  ...
  PLAN (d1)
   JOIN (d2
   WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].lesion,5)
    AND (cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].status_ind=cv_status_del))
   JOIN (cl
   WHERE (cl.lesion_id=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id))
  DETAIL
   cv_omf_rec->proc_data[d1.seq].num_lesion = (cv_omf_rec->proc_data[d1.seq].num_lesion - 1)
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in selecting cv_procedure table, program continue!")
 ENDIF
 ...
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in deleting cv_lesion table, program continue!")
 ELSE
  CALL cv_log_message("Record in cv_lesion table was deleted, program continue!")
 ENDIF
 SELECT INTO "nl:"
  cp.updt_cnt
  FROM cv_procedure cp,
   (dummyt d  WITH seq = value(size(cv_omf_rec->proc_data,5)))
  PLAN (d
   WHERE (cv_omf_rec->proc_data[d.seq].status_ind=1)
    AND (cv_omf_rec->proc_data[d.seq].procedure_id != 0.0))
   JOIN (cp
   WHERE (cp.procedure_id=cv_omf_rec->proc_data[d.seq].procedure_id))
  DETAIL
   cv_omf_rec->proc_data[d.seq].updt_cnt = cp.updt_cnt
  WITH nocounter, forupdate(cp)
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in select cv_procedure for lock the row, program continue!")
 ENDIF
 UPDATE  FROM cv_procedure cp,
   (dummyt d  WITH seq = value(size(cv_omf_rec->proc_data,5)))
  SET cp.num_lesion = cv_omf_rec->proc_data[d.seq].num_lesion
  PLAN (d
 ...
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in update cv_procedure table, program continue!")
 ENDIF
 DELETE  FROM (dummyt d1  WITH seq = value(proc_cnt)),
   (dummyt d2  WITH seq = value(cv_omf_rec->max_proc_abstr)),
   cv_proc_abstr_data cpad
  SET cpad.seq = 1
  PLAN (d1)
   JOIN (d2
   WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].proc_abstr_data,5)
    AND trim(cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].result_status_meaning)=del_ind)
   JOIN (cpad
   WHERE (cpad.event_id=cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].event_id)
    AND (cpad.procedure_id=cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].procedure_id))
  WITH nocounter
 ;end delete
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in delete cv_proc_abstr_data table, program continue!")
 ENDIF
 SELECT INTO "nl:"
  capd.procedure_id
  ...
  DETAIL
   cv_omf_rec->proc_data[d.seq].status_ind = cv_status_chg
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   WHERE (cv_omf_rec->proc_data[d.seq].procedure_id != 0.0))
   JOIN (cl
   WHERE (cl.procedure_id=cv_omf_rec->proc_data[d.seq].procedure_id))
  DETAIL
   cv_omf_rec->proc_data[d.seq].status_ind = cv_status_chg
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("No lesion associated with this procedure, program continue!")
 ENDIF
 DELETE  FROM cv_procedure cp,
   (dummyt d1  WITH seq = value(proc_cnt))
  SET cp.seq = 1
  PLAN (d1
   WHERE (cv_omf_rec->proc_data[d1.seq].status_ind=cv_status_del))
   JOIN (cp
   WHERE (cp.procedure_id=cv_omf_rec->proc_data[d1.seq].procedure_id))
  WITH nocounter
 ;end delete
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in deleting cv_procedure table, program continue!")
 ENDIF
 DELETE  FROM cv_case_abstr_data ccad,
   (dummyt d  WITH seq = value(size(cv_omf_rec->case_abstr_data,5)))
  SET ccad.seq = 1
  PLAN (d
   WHERE trim(cv_omf_rec->case_abstr_data[d.seq].result_status_meaning)=del_ind)
   JOIN (ccad
   WHERE (ccad.event_id=cv_omf_rec->case_abstr_data[d.seq].event_id)
    AND (ccad.cv_case_id=cv_omf_rec->case_abstr_data[d.seq].case_id))
  WITH nocounter
 ;end delete
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in deleting cv_case_abstr_data table, program continue!")
 ENDIF
 SELECT INTO "nl:"
  ccad.cv_case_id
  FROM cv_case_abstr_data ccad,
   (dummyt d  WITH seq = value(size(cv_omf_rec->case_abstr_data,5)))
  PLAN (d
   WHERE (cv_omf_rec->case_abstr_data[d.seq].case_id != 0.0))
   JOIN (ccad
   WHERE (ccad.cv_case_id=cv_omf_rec->case_abstr_data[d.seq].case_id))
  DETAIL
   cv_omf_rec->status_ind = cv_status_chg
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in selecting cv_case_abstr_data table, program continue!")
 ENDIF
 SELECT INTO "nl:"
  cp.procedure_id
  FROM cv_procedure cp,
   (dummyt d  WITH seq = value(proc_cnt))
  PLAN (d
   WHERE (cv_omf_rec->proc_data[d.seq].case_id != 0.0))
   JOIN (cp
   WHERE (cp.cv_case_id=cv_omf_rec->proc_data[d.seq].case_id))
  DETAIL
   cv_omf_rec->status_ind = cv_status_chg
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   del_ds_cnt = 0
  DETAIL
   del_ds_cnt = (del_ds_cnt+ 1), stat = alterlist(del_data_rec->dataset,del_ds_cnt), del_data_rec->
   dataset[del_ds_cnt].dataset_r_id = ccdr.case_dataset_r_id
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
 ...
 SELECT INTO "nl:"
  ...
  FROM cv_case_field ccf,
   (dummyt d  WITH seq = value(size(del_data_rec->dataset,5)))
  PLAN (d)
   JOIN (ccf
   WHERE (ccf.case_dataset_r_id=del_data_rec->dataset[d.seq].dataset_r_id))
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = (cnt+ 1), stat = alterlist(del_data_rec->dataset[d.seq].case_field,cnt), del_data_rec->
   dataset[d.seq].case_field[cnt].case_field_id = ccf.case_field_id
  FOOT REPORT
   IF ((del_data_rec->max_case_field_cnt < cnt))
    del_data_rec->max_case_field_cnt = cnt
   ENDIF
  WITH nocounter
 ;end select
 IF (curqual=0)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (ccfr
   WHERE (ccfr.case_dataset_r_id=del_data_rec->dataset[d.seq].dataset_r_id))
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = (cnt+ 1), stat = alterlist(del_data_rec->dataset[d.seq].case_file_row,cnt), del_data_rec->
   dataset[d.seq].case_file_row[cnt].cv_case_file_row_id = ccfr.cv_case_file_row_id
  FOOT REPORT
   IF ((del_data_rec->max_case_file_row_cnt < cnt))
    del_data_rec->max_case_file_row_cnt = cnt
   ENDIF
  WITH nocounter
 ;end select
 IF (curqual=0)
 ...
 IF ((cv_omf_rec->status_ind=cv_status_del))
  DELETE  FROM cv_case_field ccf,
    (dummyt d  WITH seq = value(size(del_data_rec->dataset,5))),
    (dummyt d1  WITH seq = value(del_data_rec->max_case_field_cnt))
   SET ccf.seq = 1
   PLAN (d)
    JOIN (d1
    WHERE d1.seq <= size(del_data_rec->dataset[d.seq].case_field,5))
    JOIN (ccf
    WHERE (ccf.case_field_id=del_data_rec->dataset[d.seq].case_field[d1.seq].case_field_id))
   WITH nocounter
  ;end delete
  IF (curqual=0)
   SET cv_log_level = cv_log_info
  ...
  DELETE  FROM cv_case_file_row ccfr,
    (dummyt d  WITH seq = value(size(del_data_rec->dataset,5))),
    (dummyt d2  WITH seq = value(del_data_rec->max_case_file_row_cnt))
   SET ccfr.seq = 1
   PLAN (d)
    JOIN (d2
    WHERE d2.seq <= size(del_data_rec->dataset[d.seq].case_file_row,5))
    JOIN (ccfr
    WHERE (ccfr.cv_case_file_row_id=del_data_rec->dataset[d.seq].case_file_row[d2.seq].
    cv_case_file_row_id))
   WITH nocounter
  ;end delete
  IF (curqual=0)
  ...
  DELETE  FROM long_text lt,
   ...
   PLAN (d)
    ...
    WHERE d1.seq <= size(del_data_rec->dataset[d.seq].case_field,5))
    JOIN (lt
    WHERE (lt.parent_entity_id=del_data_rec->dataset[d.seq].case_field[d1.seq].case_field_id)
  ...
  DELETE  FROM long_text lt,
   ...
   PLAN (d)
    ...
    WHERE d2.seq <= size(del_data_rec->dataset[d.seq].case_file_row,5))
    JOIN (lt
    WHERE (lt.parent_entity_id=del_data_rec->dataset[d.seq].case_file_row[d2.seq].cv_case_file_row_id
    )
  ...
  DELETE  FROM cv_count_data ccd
   WHERE (ccd.parent_entity_id=cv_omf_rec->case_id)
    AND ccd.parent_entity_id != 0.0
  ...
  DELETE  FROM cv_case cc
   SET cc.seq = 1
   WHERE (cv_omf_rec->case_id=cc.cv_case_id)
   WITH nocounter
  ;end delete
  IF (curqual=0)
   SET cv_log_level = cv_log_info
   CALL cv_log_message("Failed in deleting cv_case table, program continue!")
  ENDIF
 ENDIF
 SET numb_proc = 0
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   WHERE (cp.procedure_id=cv_omf_rec->proc_data[d1.seq].procedure_id))
  DETAIL
   numb_proc = (numb_proc+ 1), cv_omf_rec->num_proc = numb_proc
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_debug
  CALL cv_log_message("Failed in selecting cv_procedure table, program continue!")
  SET cv_omf_rec->num_proc = numb_proc
 ENDIF
 SET proc_cnt = size(cv_omf_rec->proc_data,5)
 SET case_chg = 0
 IF ((((cv_omf_rec->status_ind=cv_status_chg)) OR ((cv_omf_rec->status_ind=cv_status_del))) )
  SELECT INTO "nl:"
   cc.updt_cnt
   FROM cv_case cc
   WHERE (cc.cv_case_id=cv_omf_rec->case_id)
  ...
  IF (curqual=0)
   SET sum_failed = "T"
   SET cv_log_level = cv_log_audit
   CALL cv_log_message("Failed in locking cv_case rows, program continue!")
   GO TO commit_rollback_section
  ENDIF
  UPDATE  FROM cv_case cc
   SET cc.cv_case_id = cv_omf_rec->case_id, cc.form_event_id = cv_omf_rec->form_event_id, cc.num_proc
    ...
    cc.los_adm_disch = cv_omf_rec->los_adm_disch, cc.los_adm_proc = cv_omf_rec->los_adm_proc, cc
    .los_proc_disch = cv_omf_rec->los_proc_disch,
    cc.pat_adm_ind = cv_omf_rec->admit_ind, cc.pat_disch_ind = cv_omf_rec->disch_ind, cc.death_ind =
    cv_omf_rec->death_ind,
    cc.form_id = cv_omf_rec->form_id, cc.chart_dt_tm = cnvtdatetime(cv_omf_rec->chart_dt_tm), cc
    .active_ind = 1,
    cc.active_status_dt_tm = cnvtdatetime(curdate,curtime3), cc.active_status_cd = reqdata->
    active_status_cd, cc.data_status_cd = reqdata->data_status_cd,
    cc.data_status_prsnl_id = reqinfo->updt_id, cc.active_status_prsnl_id = reqinfo->updt_id, cc
    .data_status_dt_tm = cnvtdatetime(curdate,curtime3),
    cc.updt_dt_tm = cnvtdatetime(curdate,curtime3), cc.updt_task = reqinfo->updt_task, cc.updt_app =
    reqinfo->updt_app,
    cc.updt_applctx = reqinfo->updt_applctx, cc.updt_cnt = (cc.updt_cnt+ 1), cc.updt_id = reqinfo->
    updt_id,
    cc.updt_req = reqinfo->updt_req
   WHERE (cc.cv_case_id=cv_omf_rec->case_id)
    AND cc.cv_case_id != 0.0
   WITH nocounter
  ;end update
  IF (curqual != case_chg)
   SET sum_failed = "T"
   SET cv_log_level = cv_log_audit
   CALL cv_log_message("Failed in update cv_case table, program continue!")
   GO TO commit_rollback_section
  ENDIF
 ENDIF
 SET dataset_chg = 0
 SELECT INTO "nl:"
  ...
  FROM cv_case_dataset_r ccdr,
   (dummyt d  WITH seq = value(size(cv_omf_rec->dataset,5)))
  PLAN (d
   WHERE (cv_omf_rec->dataset[d.seq].status_ind=cv_status_chg))
   JOIN (ccdr
   WHERE (ccdr.cv_case_id=cv_omf_rec->case_id)
    AND (ccdr.dataset_id=cv_omf_rec->dataset[d.seq].dataset_id))
  DETAIL
   dataset_chg = (dataset_chg+ 1)
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in select cv_case, program continue!")
 ENDIF
 SELECT INTO "nl:"
  ccdr.updt_cnt
  FROM cv_case_dataset_r ccdr,
  ...
  PLAN (d
   ...
   WHERE (ccdr.cv_case_id=cv_omf_rec->case_id)
    AND (ccdr.dataset_id=cv_omf_rec->dataset[d.seq].dataset_id))
  WITH nocounter, forupdate(ccdr)
 ;end select
 IF (curqual != dataset_chg)
  SET failure = "T"
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in select cv_case for lock the row!")
  GO TO commit_rollback_section
 ENDIF
 UPDATE  FROM cv_case_dataset_r ccdr,
   (dummyt d  WITH seq = value(size(cv_omf_rec->dataset,5)))
  SET ccdr.cv_case_id = cv_omf_rec->case_id, ccdr.dataset_id = cv_omf_rec->dataset[d.seq].dataset_id,
   ccdr.status_cd = 0,
   ...
   ccdr.data_status_prsnl_id = reqinfo->updt_id, ccdr.active_status_prsnl_id = reqinfo->updt_id, ccdr
   .data_status_dt_tm = cnvtdatetime(curdate,curtime3),
   ccdr.updt_dt_tm = cnvtdatetime(curdate,curtime3), ccdr.updt_task = reqinfo->updt_task, ccdr
   .updt_app = reqinfo->updt_app,
   ccdr.updt_applctx = reqinfo->updt_applctx, ccdr.updt_cnt = (ccdr.updt_cnt+ 1), ccdr.updt_req =
   reqinfo->updt_req,
 ...
 IF (curqual != dataset_chg)
  SET sum_failed = "T"
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in updating cv_case_dataset_r!")
  GO TO commit_rollback_section
 ENDIF
 SET case_abs_chg = 0
 SELECT INTO "nl:"
  ...
  FROM cv_case_abstr_data ccad,
   (dummyt d  WITH seq = value(size(cv_omf_rec->case_abstr_data,5)))
  PLAN (d
   WHERE (cv_omf_rec->case_abstr_data[d.seq].result_status_meaning=chg_ind))
   JOIN (ccad
   WHERE (ccad.event_id=cv_omf_rec->case_abstr_data[d.seq].event_id)
    AND (ccad.cv_case_id=cv_omf_rec->case_id))
  DETAIL
   case_abs_chg = (case_abs_chg+ 1)
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in select cv_case_abstr_data!")
 ENDIF
 SELECT INTO "nl:"
  ccad.updt_cnt
  FROM cv_case_abstr_data ccad,
 ...
 ;end select
 IF (curqual != case_abs_chg)
  SET sum_failed = "T"
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in select cv_case_abstr_data for lock the row!")
  GO TO commit_rollback_section
 ENDIF
 SET case_ab_cnt = size(cv_omf_rec->case_abstr_data,5)
 UPDATE  FROM cv_case_abstr_data ccad,
   (dummyt d  WITH seq = value(size(cv_omf_rec->case_abstr_data,5)))
  SET ccad.cv_case_id = cv_omf_rec->case_id, ccad.event_cd = cv_omf_rec->case_abstr_data[d.seq].
   event_cd, ccad.nomenclature_id = cv_omf_rec->case_abstr_data[d.seq].nomenclature_id,
   ccad.result_id = cv_omf_rec->case_abstr_data[d.seq].result_id, ccad.event_id = cv_omf_rec->
   case_abstr_data[d.seq].event_id, ccad.result_val = cv_omf_rec->case_abstr_data[d.seq].result_val,
   ccad.result_dt_tm = cnvtdatetime(cv_omf_rec->case_abstr_data[d.seq].result_dt_tm), ccad.active_ind
    = 1, ccad.active_status_dt_tm = cnvtdatetime(curdate,curtime3),
   ccad.active_status_cd = reqdata->active_status_cd, ccad.beg_effective_dt_tm = cnvtdatetime(curdate,
    curtime3), ccad.end_effective_dt_tm = cnvtdatetime(null_date),
   ccad.data_status_cd = reqdata->data_status_cd, ccad.data_status_prsnl_id = reqinfo->updt_id, ccad
   .active_status_prsnl_id = reqinfo->updt_id,
   ccad.data_status_dt_tm = cnvtdatetime(curdate,curtime3), ccad.updt_dt_tm = cnvtdatetime(curdate,
    curtime3), ccad.updt_task = reqinfo->updt_task,
   ccad.updt_app = reqinfo->updt_app, ccad.updt_applctx = reqinfo->updt_applctx, ccad.updt_cnt = (
   ccad.updt_cnt+ 1),
   ccad.updt_req = reqinfo->updt_req, ccad.updt_id = reqinfo->updt_id
  PLAN (d
   WHERE trim(cv_omf_rec->case_abstr_data[d.seq].result_status_meaning)=chg_ind)
   JOIN (ccad
   WHERE (ccad.event_id=cv_omf_rec->case_abstr_data[d.seq].event_id)
    AND (ccad.cv_case_id=cv_omf_rec->case_id))
  WITH nocounter
 ...
 IF (curqual != case_abs_chg)
  SET sum_failed = "T"
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in updating cv_case_abstr_data table!")
  GO TO commit_rollback_section
 ENDIF
 SET proc_chg = 0
 SELECT INTO "nl:"
  ...
  FROM cv_procedure cp,
   (dummyt d  WITH seq = value(size(cv_omf_rec->proc_data,5)))
  PLAN (d
   WHERE (cv_omf_rec->proc_data[d.seq].status_ind=cv_status_chg)
    AND (cv_omf_rec->proc_data[d.seq].procedure_id != 0.0))
   JOIN (cp
   WHERE (cp.procedure_id=cv_omf_rec->proc_data[d.seq].procedure_id))
  DETAIL
 ...
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in select cv_procedure, program continue!")
 ENDIF
 SELECT INTO "nl:"
  cp.updt_cnt
  FROM cv_procedure cp,
 ...
 IF (curqual != proc_chg)
  SET sum_failed = "T"
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in select cv_procedure for lock the row!")
  GO TO commit_rollback_section
 ENDIF
 UPDATE  FROM cv_procedure cp,
   (dummyt d  WITH seq = value(size(cv_omf_rec->proc_data,5)))
  SET cp.proc_physic_id = cv_omf_rec->proc_data[d.seq].proc_physician_id, cp.proc_start_dt_tm =
   ...
   proc_data[d.seq].proc_start_month, cp.proc_start_day = cv_omf_rec->proc_data[d.seq].proc_start_day,
   ...
   updt_applctx,
   cp.updt_cnt = (cp.updt_cnt+ 1), cp.updt_req = reqinfo->updt_req, cp.updt_id = reqinfo->updt_id
  PLAN (d
   WHERE (cv_omf_rec->proc_data[d.seq].status_ind=cv_status_chg)
    AND (cv_omf_rec->proc_data[d.seq].procedure_id != 0.0))
   JOIN (cp
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   JOIN (d2
   WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].proc_abstr_data,5)
    AND trim(cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].result_status_meaning)=chg_ind)
   JOIN (cpad
   WHERE (cpad.event_id=cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].event_id)
    AND (cpad.procedure_id=cv_omf_rec->proc_data[d1.seq].procedure_id))
  DETAIL
   proc_abs_chg = (proc_abs_chg+ 1)
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in select cv_proc_abstr_data, program continue!")
 ENDIF
 SELECT INTO "nl:"
  cpad.updt_cnt
  FROM cv_proc_abstr_data cpad,
  ...
  PLAN (d1)
   ...
   WHERE (cpad.event_id=cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].event_id)
    AND (cpad.procedure_id=cv_omf_rec->proc_data[d1.seq].procedure_id))
  WITH nocounter, forupdate(cpad)
 ;end select
 IF (curqual != proc_abs_chg)
  CALL cv_log_message(build("number to be changed:",proc_abs_chg))
  CALL cv_log_message(build("number changed:",curqual))
  SET sum_failed = "T"
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in select cv_proc_abstr_data for lock the row!")
  GO TO commit_rollback_section
 ENDIF
 UPDATE  FROM cv_proc_abstr_data cpad,
   (dummyt d1  WITH seq = value(proc_cnt)),
   (dummyt d2  WITH seq = value(cv_omf_rec->max_proc_abstr))
  SET cpad.procedure_id = cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].procedure_id, cpad
   .event_cd = cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].event_cd, cpad.nomenclature_id
    = cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].nomenclature_id,
   cpad.result_id = cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].result_id, cpad.event_id =
   cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].event_id, cpad.result_val = cv_omf_rec->
   proc_data[d1.seq].proc_abstr_data[d2.seq].result_val,
   cpad.result_dt_tm = cnvtdatetime(cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].
    result_dt_tm), cpad.active_ind = 1, cpad.active_status_dt_tm = cnvtdatetime(curdate,curtime3),
   cpad.active_status_cd = reqdata->active_status_cd, cpad.beg_effective_dt_tm = cnvtdatetime(curdate,
    curtime3), cpad.end_effective_dt_tm = cnvtdatetime(null_date),
   cpad.data_status_cd = reqdata->data_status_cd, cpad.data_status_prsnl_id = reqinfo->updt_id, cpad
   .active_status_prsnl_id = reqinfo->updt_id,
   cpad.data_status_dt_tm = cnvtdatetime(curdate,curtime3), cpad.updt_dt_tm = cnvtdatetime(curdate,
    curtime3), cpad.updt_task = reqinfo->updt_task,
   cpad.updt_app = reqinfo->updt_app, cpad.updt_applctx = reqinfo->updt_applctx, cpad.updt_cnt = (
   cpad.updt_cnt+ 1),
   cpad.updt_req = reqinfo->updt_req, cpad.updt_id = reqinfo->updt_id
  PLAN (d1)
   JOIN (d2
   WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].proc_abstr_data,5)
    AND trim(cv_omf_rec->proc_data[d1.seq].proc_abstr_data[d2.seq].result_status_meaning)=chg_ind)
 ...
 IF (curqual != proc_abs_chg)
  SET sum_failed = "T"
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in update cv_proc_abstr_data table!")
  GO TO commit_rollback_section
 ENDIF
 SET les_chg = 0
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   JOIN (d2
   WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].lesion,5)
    AND (cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].status_ind=cv_status_chg)
    AND (cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id != 0.0))
   JOIN (cl
   WHERE (cl.lesion_id=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id))
  DETAIL
 ...
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in select cv_lesion, program continue!")
 ENDIF
 SELECT INTO "nl:"
  cl.updt_cnt
  FROM cv_lesion cl,
 ...
 IF (curqual != les_chg)
  SET sum_failed = "T"
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in select cv_lesion for lock the row!")
  GO TO commit_rollback_section
 ENDIF
 UPDATE  FROM cv_lesion cl,
   (dummyt d1  WITH seq = value(proc_cnt)),
   (dummyt d2  WITH seq = value(cv_omf_rec->max_lesion))
  SET cl.lesion_id = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id, cl.procedure_id =
   cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].procedure_id, cl.parent_event_id = cv_omf_rec->
   ...
   cl.updt_dt_tm = cnvtdatetime(curdate,curtime3), cl.updt_task = reqinfo->updt_task, cl.updt_app =
   reqinfo->updt_app,
   cl.updt_applctx = reqinfo->updt_applctx, cl.updt_cnt = (cl.updt_cnt+ 1), cl.updt_req = reqinfo->
   updt_req,
   cl.updt_id = reqinfo->updt_id
  PLAN (d1)
   JOIN (d2
 ...
 IF (curqual != les_chg)
  SET sum_failed = "T"
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in update cv_lesion table!")
  GO TO commit_rollback_section
 ENDIF
 SET les_abs_chg = 0
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   WHERE d3.seq <= size(cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data,5)
    AND trim(cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].
    result_status_meaning)=chg_ind)
   JOIN (clad
   WHERE (clad.event_id=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].event_id)
    AND (clad.lesion_id=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id))
  DETAIL
   les_abs_chg = (les_abs_chg+ 1)
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET cv_log_level = cv_log_info
  CALL cv_log_message("Failed in select cv_les_abstr_data, program continue!")
 ENDIF
 SELECT INTO "nl:"
  clad.updt_cnt
  FROM cv_les_abstr_data clad,
  ...
  PLAN (d1)
   ...
   JOIN (d3
   WHERE d3.seq <= size(cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data,5)
    AND (cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].result_status_meaning=
   chg_ind))
   JOIN (clad
   WHERE (clad.event_id=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].event_id)
    AND (clad.lesion_id=cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].lesion_id))
  WITH nocounter, forupdate(clad)
 ;end select
 IF (curqual != les_abs_chg)
  SET sum_failed = "T"
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in select cv_les_abstr_data for lock the row!")
  GO TO commit_rollback_section
 ENDIF
 UPDATE  FROM cv_les_abstr_data clad,
   (dummyt d1  WITH seq = value(proc_cnt)),
   (dummyt d2  WITH seq = value(cv_omf_rec->max_lesion)),
   (dummyt d3  WITH seq = value(cv_omf_rec->max_lesion_abstr))
  SET clad.lesion_id = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].lesion_id,
   clad.event_type_cd = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].
   event_type_cd, clad.event_cd = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq]
   .event_cd,
   clad.nomenclature_id = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].
   nomenclature_id, clad.result_id = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3
   .seq].result_id, clad.result_id = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3
   .seq].event_id,
   clad.result_val = cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3.seq].result_val,
   clad.result_dt_tm = cnvtdatetime(cv_omf_rec->proc_data[d1.seq].lesion[d2.seq].les_abstr_data[d3
    .seq].result_dt_tm), clad.active_ind = 1,
   ...
   reqinfo->updt_applctx,
   clad.updt_cnt = (clad.updt_cnt+ 1), clad.updt_req = reqinfo->updt_req, clad.updt_id = reqinfo->
   updt_id
  PLAN (d1)
   JOIN (d2
   WHERE d2.seq <= size(cv_omf_rec->proc_data[d1.seq].lesion,5))
 ...
 IF (curqual != les_abs_chg)
  SET cv_log_level = cv_log_audit
  CALL cv_log_message("Failed in update cv_les_abstr_data table!")
  SET sum_failed = "T"
  GO TO commit_rollback_section
 ENDIF
 CALL echorecord(cv_omf_rec,"cer_temp:cv_omf_rec_af.dat")
#commit_rollback_section
 IF (sum_failed="T")
  SET reply->status_data.status = "F"
 ...
 FOR (n = 1 TO size(cv_omf_rec->dataset,5))
   ...
   CALL cv_log_message("cv_inst_updt...executed cv_get_harvest!!")
 ENDFOR
