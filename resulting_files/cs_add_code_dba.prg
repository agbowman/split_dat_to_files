CREATE PROGRAM cs_add_code:dba
 ...
 SET authcnt = 0
 SET cw_req = fillstring(40," ")
 SET cw_type = fillstring(15," ")
 ...
 SELECT INTO "nl:"
  cvs.add_access_ind, cvs.display_key_dup_ind, cvs.cdf_meaning_dup_ind,
  cvs.active_ind_dup_ind, cvs.display_dup_ind
  FROM code_value_set cvs
  WHERE (cvs.code_set=request->code_set)
  DETAIL
   add_access_ind = cvs.add_access_ind, display_key_dup_ind = cvs.display_key_dup_ind,
   cdf_meaning_dup_ind = cvs.cdf_meaning_dup_ind,
   active_ind_dup_ind = cvs.active_ind_dup_ind, display_dup_ind = cvs.display_dup_ind
  WITH nocounter
 ;end select
#start_loop
 FOR (y = y TO number_to_add)
   ...
   SET parser_buffer[3] = concat("where c.code_set = ",cnvtstring(request->code_set))
   SET parser_number = 3
   IF (display_key_dup_ind=1)
    SET parser_number = (parser_number+ 1)
    CALL cac_where(request->qual[y].display_key,"display_key")
   ENDIF
   IF (display_dup_ind=1)
    SET parser_number = (parser_number+ 1)
    CALL cac_where(request->qual[y].display,"display")
   ENDIF
   IF (cdf_meaning_dup_ind=1)
    IF ((request->qual[y].cdf_meaning > ""))
     SET parser_number = (parser_number+ 1)
     CALL cac_where(request->qual[y].cdf_meaning,"cdf_meaning")
    ELSE
     SET parser_number = (parser_number+ 1)
   ...
   IF (active_ind_dup_ind=1)
    SET parser_number = (parser_number+ 1)
    SET parser_buffer[parser_number] = concat("  and c.active_ind = ",cnvtstring(request->qual[y].
      active_ind))
   ENDIF
   IF (display_dup_ind=0
    ...
    SET parser_number = (parser_number+ 1)
    SET parser_buffer[parser_number] = concat("  and c.display = '",request->qual[y].display,"'")
   ENDIF
   SET parser_number = (parser_number+ 1)
...
#exit_script
 ...
 SUBROUTINE cac_where(cw_req,cw_type)
   IF (findstring("^",cw_req)=0)
    SET parser_buffer[parser_number] = concat("  and c.",trim(cw_type)," = ^",trim(cw_req),"^")
