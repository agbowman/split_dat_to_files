CREATE PROGRAM dm_rmc_mover_1492:dba
 ...
 DECLARE temp_filter_id = f8
 DECLARE temp_field_name = vc
 DECLARE temp_field_case = c3
 DECLARE from_value = vc
 DECLARE new_value = vc
 DECLARE from_cd = vc
 DECLARE new_cd = vc
 DECLARE temp_table_str = vc
 DECLARE temp_table_str2 = vc
 DECLARE no_query = i2
 DECLARE val = f8
 DECLARE ins_upd = i2
 ...
 SET temp_col_cnt = get_col_pos(cust_tab_name,"VALUE")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
 SET temp_filter_id = cnvtreal(get_value(cust_tab_name,"FILTER_ID","FROM"))
 SET temp_table_str = dm2_get_rdds_tname("PM_RPT_FILTER")
 SET temp_table_str2 = dm2_get_rdds_tname("PM_RPT_FIELD")
 SELECT INTO "nl:"
  p.field_id, prf.field_id, prf.field_name
  FROM (parser(temp_table_str) p),
   (parser(temp_table_str2) prf)
  PLAN (p
   WHERE p.filter_id=temp_filter_id)
   JOIN (prf
   WHERE prf.field_id=p.field_id)
  DETAIL
   temp_field_name = cnvtupper(prf.field_name)
  WITH nocounter
 ;end select
 SET temp_field_case = substring((textlen(temp_field_name) - 2),textlen(temp_field_name),
  temp_field_name)
 SET from_value = get_value(cust_tab_name,"VALUE","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_1492
 ENDIF
 CASE (temp_field_case)
  OF "_ID":
   IF (temp_field_name="PERSON_ID")
    IF (from_value > " ")
     SET new_value = select_merge_translate(from_value,"PERSON")
     IF (isnumeric(new_value))
      CALL put_value(cust_tab_name,"VALUE",new_value)
     ELSE
      SET cust_rpt_missing = report_missing("PRSNL","PERSON_ID",cnvtreal(from_value))
      CALL evaluate_rpt_missing(cust_rpt_missing)
     ENDIF
    ELSE
     CALL put_value(cust_tab_name,"VALUE",from_value)
    ENDIF
    IF ((dm2_ref_data_reply->error_ind=1))
     GO TO exit_1492
    ENDIF
   ELSE
    SET dm2_ref_data_reply->error_ind = 1
    CALL merge_audit("FAILREASON",concat("FAIL: column name - ",temp_field_name,
      " is unsupported at this time."),1)
    SET dm2_ref_data_reply->error_msg = "NOMV31"
    GO TO exit_1492
   ENDIF
  OF "_CD":
   SET from_cd = get_value(cust_tab_name,"VALUE","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1492
   ENDIF
   IF (cnvtreal(from_cd) > 0)
    SET new_cd = select_merge_translate(from_cd,"CODE_VALUE")
    IF (isnumeric(new_cd))
     CALL put_value(cust_tab_name,"VALUE",new_cd)
    ELSE
     SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(from_cd))
     CALL evaluate_rpt_missing(cust_rpt_missing)
    ENDIF
   ELSE
    CALL put_value(cust_tab_name,"VALUE",from_cd)
   ENDIF
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1492
   ENDIF
  ELSE
   ...
   CALL put_value(cust_tab_name,"VALUE",from_cd)
 ENDCASE
 SET temp_col_cnt = get_col_pos(cust_tab_name,"FILTER_VALUES_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1)
   ...
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"FILTER_VALUES_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"FILTER_VALUES_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  FILTER_VALUES_ID")
     ELSE
      GO TO exit_1492
     ENDIF
    ELSE
   ...
   SET from_cd = get_value(cust_tab_name,"FILTER_VALUES_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1492
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(from_cd)
   ENDIF
   SET new_cd = get_value(cust_tab_name,"FILTER_VALUES_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1492
   ELSE
 ...
 ENDIF
 IF (idcd_check=1)
  SET idcd_check = is_translated(cust_tab_name,"VALUE")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the VALUE column"
...
#exit_1492
 ...
 FREE SET temp_col_cnt
 FREE SET temp_filter_id
 FREE SET temp_field_name
 FREE SET temp_field_case
 FREE SET from_value
END GO
