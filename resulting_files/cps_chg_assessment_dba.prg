CREATE PROGRAM cps_chg_assessment:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 UPDATE  FROM dsm_assessment da,
  ...
  SET da.seq = 1, da.active_ind = request->qual[d.seq].active_ind, da.end_effective_dt_tm =
   IF ((request->qual[d.seq].active_ind=0)) cnvtdatetime(curdate,curtime3)
   ELSE cnvtdatetime("31-dec-2100 00:00:00.00")
   ENDIF
   ,
   da.active_status_cd =
   IF ((request->qual[d.seq].active_ind=0)) inactive
   ELSE active
   ENDIF
   , da.updt_dt_tm = cnvtdatetime(curdate,curtime3), da.updt_id = reqinfo->updt_id,
