CREATE PROGRAM bbt_rdm_updt_atr:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bbd_rdm_updt_atr"
 CALL logscriptstart(sreadme_name)
 CALL logstatus(build("Readme Failed:  Starting script ",sreadme_name,"."),"F")
 DECLARE err_ind = i4
 SET err_ind = 0
 CALL logmsg("Stamping application BBDShipTransfer - 225078",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225078)
 SELECT INTO "nl:"
  ap.*
  FROM application ap
  WHERE ap.application_number=225078
   AND trim(ap.object_name)="BBDShipTransfer.Application"
   AND ap.active_ind=1
  WITH nocounter
 ;end select
 IF (curqual=0)
  CALL logstatus(build(sreadme_name,
    " Failed! object_name of Application 225078 not updated or is not active."),"F")
  SET err_ind = 1
 ENDIF
 CALL logmsg("Stamping application BBDShippingTool - 225074",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225074)
 SELECT INTO "nl:"
  ap.*
  FROM application ap
  WHERE ap.application_number=225074
   AND ap.active_ind=1
  WITH nocounter
 ;end select
 IF (curqual=0)
  CALL logstatus(build(sreadme_name," Failed! Application 225074 is not active."),"F")
  SET err_ind = 1
 ENDIF
 IF (err_ind=0)
