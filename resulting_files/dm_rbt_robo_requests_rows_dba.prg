CREATE PROGRAM dm_rbt_robo_requests_rows:dba
 ...
 SET reply->table_name = "ROBOTICS_REQUESTS"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(rr.robotics_msg_id)
    FROM robotics_requests rr
    WHERE rr.robotics_msg_id > 0
    DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     rr.rowid
     FROM robotics_requests rr
     WHERE parser(sbr_getrowidnotexists("rr.robotics_msg_id between curMinID and curMaxID","rr"))
      AND rr.updt_dt_tm < cnvtdatetime((curdate - days_to_keep),curtime3)
      AND rr.robotics_msg_id > 0
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,50)=1)
       stat = alterlist(reply->rows,(rows+ 49))
      ENDIF
      reply->rows[rows].row_id = rr.rowid
     WITH maxqual(rr,value(rowsleft))
    ;end select
