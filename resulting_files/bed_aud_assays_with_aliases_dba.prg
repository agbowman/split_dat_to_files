CREATE PROGRAM bed_aud_assays_with_aliases:dba
  ...
  SELECT INTO "NL:"
   ...
   DETAIL
    IF (cva.alias > " ")
     ...
     temp->tqual[tcnt].alias = cva.alias, temp->tqual[tcnt].active_ind = cv1.active_ind, temp->tqual[
     tcnt].routed_ind = 0
    ENDIF
   WITH nocounter
  ;end select
 ...
 SET reply->collist[7].header_text = "Alias"
 SET reply->collist[7].data_type = 1
 SET reply->collist[7].hide_ind = 0
 SET reply->collist[8].header_text = "Active Indicator"
 ...
 FOR (x = 1 TO tcnt)
   IF (((scnt=0) OR (scnt > 0
    ...
    SET reply->rowlist[row_nbr].celllist[7].string_value = temp->tqual[x].alias
...
#exit_script
 ...
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("assays_with_aliases.csv")
 ENDIF
