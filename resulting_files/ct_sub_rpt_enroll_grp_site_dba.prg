CREATE PROGRAM ct_sub_rpt_enroll_grp_site:dba
 PROMPT
 ...
 DECLARE get_enroll_by_site(dummy) = null WITH protect
 ...
 DECLARE _bcontheadenrollments_enroll_org_idsection = i2 WITH noconstant(0), protect
 ...
 DECLARE _bcontfootenrollments_prot_master_idsection = i2 WITH noconstant(0), protect
 DECLARE _remprin_inv = i4 WITH noconstant(1), protect
 DECLARE _bcontfootenrollments_prot_master_idsection1 = i2 WITH noconstant(0), protect
 ...
 DECLARE _remoff_study_date = i4 WITH noconstant(1), protect
 DECLARE _remtotal_count = i4 WITH noconstant(1), protect
 DECLARE _bcontfootenrollments_enroll_org_idsection = i2 WITH noconstant(0), protect
 ...
 DECLARE _remtotal_sites = i4 WITH noconstant(1), protect
 ...
 SUBROUTINE get_enroll_by_site(dummy)
   SELECT
    IF (( $GROUPBY=0))
     enrollments_enroll_org_id = results->enrollments[d.seq].enroll_org_id,
     enrollments_primary_mnemonic = results->enrollments[d.seq].primary_mnemonic,
     enrollments_prot_master_id = results->enrollments[d.seq].prot_master_id,
     enrollments_initiating_service_cd = results->enrollments[d.seq].initiating_service_cd, pi_id =
     results->pis[d2.seq].pi_id, enrollments_person_id = results->enrollments[d.seq].person_id,
     reportlist_order_by_1 = parser(reportlist->order_by_1), reportlist_order_by_2 = parser(
      reportlist->order_by_2), enrollments_enroll_org_name = cnvtlower(results->enrollments[d.seq].
      enroll_org_name)
     ...
     ORDER BY enrollments_enroll_org_name, reportlist_order_by_1, reportlist_order_by_2
    ...
    reportlist_order_by_1 = parser(reportlist->order_by_1), reportlist_order_by_2 = parser(reportlist
     ->order_by_2), enrollments_enroll_org_name = cnvtlower(results->enrollments[d.seq].
     enroll_org_name)
    FROM (dummyt d  WITH seq = value(size(results->enrollments,5))),
     (dummyt d2  WITH seq = value(size(results->pis,5)))
    PLAN (d)
     JOIN (d2
     WHERE (results->pis[d2.seq].prot_master_id=results->enrollments[d.seq].prot_master_id))
    ORDER BY enrollments_enroll_org_id, enrollments_prot_master_id, enrollments_person_id,
     pi_id
    HEAD REPORT
    ...
    HEAD PAGE
     ...
     (rpt_render)
    HEAD enrollments_enroll_org_id
     ...
     ENDIF
     _bcontheadenrollments_enroll_org_idsection = 0, bfirsttime = 1
     WHILE (((_bcontheadenrollments_enroll_org_idsection=1) OR (bfirsttime=1)) )
       _bholdcontinue = _bcontheadenrollments_enroll_org_idsection, _fdrawheight =
       headenrollments_enroll_org_idsection(rpt_calcheight,(_fenddetail - _yoffset),_bholdcontinue)
       IF (((_bholdcontinue=1) OR (_fdrawheight > 0)) )
        IF ((_fenddetail >= (_yoffset+ _fdrawheight)))
         _fdrawheight += headenrollments_enroll_org_idsection_line(rpt_calcheight)
        ENDIF
       ENDIF
       IF (((_yoffset+ _fdrawheight) > _fenddetail))
        BREAK
       ELSEIF (_bholdcontinue=1
        AND _bcontheadenrollments_enroll_org_idsection=0)
        BREAK
       ENDIF
       dummy_val = headenrollments_enroll_org_idsection(rpt_render,(_fenddetail - _yoffset),
        _bcontheadenrollments_enroll_org_idsection), bfirsttime = 0
     ENDWHILE
     _fdrawheight = headenrollments_enroll_org_idsection_line(rpt_calcheight)
     IF (((_yoffset+ _fdrawheight) > _fenddetail))
      BREAK
     ENDIF
     dummy_val = headenrollments_enroll_org_idsection_line(rpt_render)
    HEAD enrollments_prot_master_id
    ...
    FOOT  pi_id
     row + 0
    FOOT  enrollments_person_id
     row + 0
    FOOT  enrollments_prot_master_id
     ...
     ENDIF
     tmp_prot = results->enrollments[d.seq].primary_mnemonic, tmp_pi = trim(tmp_pi), tmp_init_svc =
     trim(uar_get_code_display(results->enrollments[d.seq].initiating_service_cd)),
     _bcontfootenrollments_prot_master_idsection = 0, bfirsttime = 1
     WHILE (((_bcontfootenrollments_prot_master_idsection=1) OR (bfirsttime=1)) )
       _bholdcontinue = _bcontfootenrollments_prot_master_idsection, _fdrawheight =
       footenrollments_prot_master_idsection(rpt_calcheight,(_fenddetail - _yoffset),_bholdcontinue)
       IF (((_bholdcontinue=1) OR (_fdrawheight > 0)) )
        IF ((_fenddetail >= (_yoffset+ _fdrawheight)))
         _bholdcontinue = 0, _fdrawheight += footenrollments_prot_master_idsection1(rpt_calcheight,((
       ...
       IF (((_yoffset+ _fdrawheight) > _fenddetail))
        BREAK
       ELSEIF (_bholdcontinue=1
        AND _bcontfootenrollments_prot_master_idsection=0)
        BREAK
       ENDIF
       dummy_val = footenrollments_prot_master_idsection(rpt_render,(_fenddetail - _yoffset),
        _bcontfootenrollments_prot_master_idsection), bfirsttime = 0
     ENDWHILE
     ...
     FOR (idx = 2 TO prot_pi_cnt)
       tempstr = trim(countlist->prot_pis[idx].pi_name_full),
       _bcontfootenrollments_prot_master_idsection1 = 0, bfirsttime = 1
       WHILE (((_bcontfootenrollments_prot_master_idsection1=1) OR (bfirsttime=1)) )
         _bholdcontinue = _bcontfootenrollments_prot_master_idsection1, _fdrawheight =
         footenrollments_prot_master_idsection1(rpt_calcheight,(_fenddetail - _yoffset),
          _bholdcontinue)
         IF (((_yoffset+ _fdrawheight) > _fenddetail))
          BREAK
         ELSEIF (_bholdcontinue=1
          AND _bcontfootenrollments_prot_master_idsection1=0)
          BREAK
         ENDIF
         dummy_val = footenrollments_prot_master_idsection1(rpt_render,(_fenddetail - _yoffset),
          _bcontfootenrollments_prot_master_idsection1), bfirsttime = 0
       ENDWHILE
     ENDFOR
     stat = alterlist(countlist->prot_pis,0), prot_pi_cnt = 0
    FOOT  enrollments_enroll_org_id
     _bcontfootenrollments_enroll_org_idsection = 0, bfirsttime = 1
     WHILE (((_bcontfootenrollments_enroll_org_idsection=1) OR (bfirsttime=1)) )
       _bholdcontinue = _bcontfootenrollments_enroll_org_idsection, _fdrawheight =
       footenrollments_enroll_org_idsection(rpt_calcheight,(_fenddetail - _yoffset),_bholdcontinue)
       IF (((_yoffset+ _fdrawheight) > _fenddetail))
        BREAK
       ELSEIF (_bholdcontinue=1
        AND _bcontfootenrollments_enroll_org_idsection=0)
        BREAK
       ENDIF
       dummy_val = footenrollments_enroll_org_idsection(rpt_render,(_fenddetail - _yoffset),
        _bcontfootenrollments_enroll_org_idsection), bfirsttime = 0
     ENDWHILE
    FOOT PAGE
    ...
    FOOT REPORT
     tempstr = concat(label->total_sites," ",trim(cnvtstring(site_cnt,3))), _bcontfootreportsection
      = 0, bfirsttime = 1
 ...
 SUBROUTINE (headpagesection_lineabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (headenrollments_enroll_org_idsection(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) =f8 WITH
  protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headenrollments_enroll_org_idsectionabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (headenrollments_enroll_org_idsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,
  bcontinue=i2(ref)) =f8 WITH protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (headenrollments_enroll_org_idsection_line(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headenrollments_enroll_org_idsection_lineabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (headenrollments_enroll_org_idsection_lineabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH
  protect)
   DECLARE sectionheight = f8 WITH noconstant(0.060000), private
   IF (ncalc=rpt_render)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (footenrollments_prot_master_idsection(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) =f8 WITH
  protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = footenrollments_prot_master_idsectionabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (footenrollments_prot_master_idsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,
  bcontinue=i2(ref)) =f8 WITH protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (footenrollments_prot_master_idsection1(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) =f8
  WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = footenrollments_prot_master_idsection1abs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (footenrollments_prot_master_idsection1abs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,
  bcontinue=i2(ref)) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.150000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_prin_inv = f8 WITH noconstant(0.0), private
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (footenrollments_enroll_org_idsection(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) =f8 WITH
  protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = footenrollments_enroll_org_idsectionabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (footenrollments_enroll_org_idsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,
  bcontinue=i2(ref)) =f8 WITH protect)
 ...
 SUBROUTINE (footreportsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
  WITH protect)
   ...
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_total_sites = f8 WITH noconstant(0.0), private
   DECLARE drawheight_label_end_of_rpt = f8 WITH noconstant(0.0), private
   DECLARE __total_sites = vc WITH noconstant(build2(tempstr,char(0))), protect
   DECLARE __label_end_of_rpt = vc WITH noconstant(build2(label->end_of_rpt,char(0))), protect
   IF (bcontinue=0)
    SET _remtotal_sites = 1
   ...
   SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
   SET _holdremtotal_sites = _remtotal_sites
   IF (_remtotal_sites > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_remtotal_sites,((size(
        __total_sites) - _remtotal_sites)+ 1),__total_sites)))
    SET drawheight_total_sites = rptsd->m_height
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _remtotal_sites = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remtotal_sites,((size(__total_sites) -
       _remtotal_sites)+ 1),__total_sites)))))
     SET _remtotal_sites += rptsd->m_drawlength
    ELSE
     SET _remtotal_sites = 0
    ENDIF
    SET growsum += _remtotal_sites
   ENDIF
   SET rptsd->m_flags = 21
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   SET rptsd->m_x = (offsetx+ 0.000)
   SET rptsd->m_width = 10.500
   SET rptsd->m_height = drawheight_total_sites
   SET _dummyfont = uar_rptsetfont(_hreport,_courier9b0)
   IF (ncalc=rpt_render
    AND _holdremtotal_sites > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdremtotal_sites,((size
       (__total_sites) - _holdremtotal_sites)+ 1),__total_sites)))
   ELSE
    SET _remtotal_sites = _holdremtotal_sites
   ENDIF
   SET rptsd->m_flags = 20
   IF (bcontinue)
    SET rptsd->m_y = offsety
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CT_SUB_RPT_ENROLL_GRP_SITE"
