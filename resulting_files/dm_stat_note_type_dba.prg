CREATE PROGRAM dm_stat_note_type:dba
 ...
 ENDIF
 DECLARE ms_snapshot_type = vc WITH protect, constant("NOTE_TYPE")
 DECLARE error_msg = vc WITH noconstant("")
 DECLARE ds_cnt = i4
 DECLARE ms_snapshot_time = dq8 WITH constant(cnvtdatetime((curdate - 1),0))
 DECLARE ms_info_domain = vc WITH constant("DM_STAT_NOTE_TYPE")
 ...
 ENDIF
 CALL sbr_debug_timer("START_TOTAL","DM_STAT_NOTE_TYPE")
 CALL sbr_debug_timer("START","INSERTING NOTE DATA")
 SELECT INTO "nl:"
  n.note_type_id, description = trim(n.note_type_description)
  FROM note_type n
  HEAD REPORT
  ...
  DETAIL
   ...
   ENDIF
   dsr->qual[1].qual[ds_cnt].stat_name = "NOTE_TYPE", dsr->qual[1].qual[ds_cnt].stat_type = 0, dsr->
   qual[1].qual[ds_cnt].stat_seq = ds_cnt,
   dsr->qual[1].qual[ds_cnt].stat_number_val = n.note_type_id, dsr->qual[1].qual[ds_cnt].
 ...
 IF (error(error_msg,0) != 0)
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_debug_timer("END","INSERTING NOTE DATA")
 ...
 END ;Subroutine
#exit_program
 CALL sbr_debug_timer("END_TOTAL","dm_stat_note_type")
END GO
