CREATE PROGRAM dm_rmc_config_routines_test
 ...
 SUBROUTINE drcr_check_cbc_setup(i_source_env_id,i_target_env_id,ccs_msg)
   ...
   ENDIF
   IF (drcr_get_cutover_config(i_source_env_id,i_target_env_id)=0)
    SET drcr_ccs_info->return_msg = concat(
     "Auto-Cutover can not be setup, while using cutover by context.  Please change the environment relationship to use ",
     "Planned Cutover in DM_MERGE_DOMAIN_ADM.")
    SET drcr_ccs_info->return_ind = 1
    RETURN(null)
   ENDIF
   SELECT INTO "NL:"
   ...
   RETURN(null)
 END ;Subroutine
 IF (check_logfile("dm_test_config",".log","DM_RMC_CONFIG_ROUTINES_TEST LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_main
 ENDIF
 DECLARE drcrt_src_id = f8 WITH protect, noconstant(0.0)
 DECLARE drcrt_tgt_id = f8 WITH protect, noconstant(0.0)
 SET drcrt_src_id =  $1
 SET drcrt_tgt_id =  $2
 CALL drcr_check_cbc_setup(drcrt_src_id,drcrt_tgt_id,drcr_ccs_info)
#exit_main
END GO
