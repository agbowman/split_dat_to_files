CREATE PROGRAM dm2_regen_rdds_trigs:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dm_rdds_setup_check(check_env_id=f8) = i2
 SUBROUTINE dm_rdds_setup_check(check_env_id)
   IF (check_env_id <= 0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Invalid environment ID passed to dm_rdds_setup_check"
    RETURN(0)
   ENDIF
   SELECT INTO "NL:"
    FROM dm2_user_triggers d
    WHERE d.trigger_name="REFCHG*"
    WITH nocounter, maxqual(d,1)
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Error selecting RDDS related triggers."
    RETURN(0)
   ENDIF
   IF (curqual > 0)
    RETURN(1)
   ...
   SELECT INTO "NL:"
    FROM dm_env_reltn d
    WHERE ((d.parent_env_id=check_env_id) OR (d.child_env_id=check_env_id))
     AND d.relationship_type="REFERENCE MERGE"
    WITH nocounter, maxqual(d,1)
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Error selecting environment relationships."
    RETURN(0)
   ENDIF
   IF (curqual > 0)
    RETURN(1)
 ...
 SUBROUTINE dsi_get_tspace_info(gti_tspace_name,gti_obj_type)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE check_trg_tablespace(null) = i2
 SUBROUTINE check_trg_tablespace(null)
   DECLARE ctt_ret = i2 WITH noconstant(1)
   DECLARE ctt_ts_loc = i4
   DECLARE ctt_idx = i4
   DECLARE ctt_space = f8
   DECLARE ctt_margin = f8
   DECLARE ctt_required = f8
   SET ctt_ret = dsi_get_tspace_info("SYSTEM","TABLESPACE")
   IF (ctt_ret=0)
    RETURN(ctt_ret)
   ENDIF
   SET ctt_ts_loc = locateval(ctt_idx,1,size(space_info->tspace,5),"SYSTEM",space_info->tspace[
    ctt_idx].tspace_name)
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="RDDS CONFIGURATION"
     AND di.info_name="SYSTEM FREE SPACE"
    DETAIL
     ctt_margin = cnvtreal(di.info_char), ctt_required = di.info_number
    WITH nocounter
   ;end select
   IF (check_error("Check DM_INFO row for SYSTEM FREE SPACE requirement") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dm_err->err_ind = 1
    RETURN(0)
   ENDIF
   IF (curqual=0)
    SET ctt_margin = 0
    SET ctt_required = 2048
   ENDIF
   SET ctt_space = (((space_info->tspace[ctt_ts_loc].free_spc/ 1024.0)+ ctt_margin) - ctt_required)
   IF (ctt_space < 0)
    SET ctt_ret = 0
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Not enough SYSTEM tablespace avaliable. Please add: ",trim(cnvtstring(
       abs(ctt_space))),"KB to the SYSTEM tablespace")
   ENDIF
   RETURN(ctt_ret)
 END ;Subroutine
 ...
 FREE RECORD drrt_md_request
 RECORD drrt_md_request(
   1 remote_env_id = f8
   1 local_env_id = f8
   1 post_link_name = vc
 )
 DECLARE drrt_cs_del_ind = i2
 DECLARE drrt_environment = f8
 DECLARE drrt_regen_ind = i2
 DECLARE drrt_ret = i2
 DECLARE v_trigger_name = vc WITH protect, noconstant("")
 DECLARE drrt_skip = i4 WITH protect, noconstant(0)
 DECLARE drrt_cnt = i4 WITH protect, noconstant(0)
 DECLARE drrt_status = vc WITH protect, noconstant("")
 SET readme_data->status = "F"
 IF (validate(trg_regen_reason,"X")="X")
  DECLARE trg_regen_reason = vc
  SET trg_regen_reason = "Automatic regeneration via readme"
 ENDIF
 IF (check_logfile("dm2_regen_trig_",".log","dm2_regen_trig LOG FILE...") != 1)
  SET dm_err->emsg = "Unable to Create Log File"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_regen_rdds_trigs..."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "NL:"
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="DM_ENV_ID"
  DETAIL
   drrt_environment = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg = "Error selecting RDDS related triggers."
  GO TO exit_script
 ENDIF
 IF (drrt_environment=0)
  SET dm_err->emsg = "Error: No valid environment_id was set.  Please run DM_SET_ENV_ID"
  GO TO exit_script
 ENDIF
 SET drrt_regen_ind = dm_rdds_setup_check(drrt_environment)
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 IF (drrt_regen_ind=1)
  SET drrt_ret = check_trg_tablespace(null)
  IF (drrt_ret=0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   SET readme_data->status = "F"
   GO TO exit_script
  ENDIF
  IF (check_open_event(drrt_environment,0.0)=0)
   SET drrt_md_request->remote_env_id = 0
   SET drrt_md_request->local_env_id = drrt_environment
   EXECUTE dm2_refresh_local_meta_data  WITH replace("REQUEST","DRRT_MD_REQUEST")
   IF (check_error(dm_err->eproc)=1)
    GO TO exit_script
   ENDIF
  ENDIF
  SET dm_err->eproc = "Querying dm_info for list of current state tables"
  SELECT DISTINCT INTO "nl:"
   di.info_char
   FROM dm_info di
   WHERE di.info_domain="RDDS CURRENT STATE OBJECTS"
   ...
   (SELECT
    "x"
    FROM user_tables ut
    WHERE ut.table_name=di.info_char))
   HEAD REPORT
    drrt_cnt = 0
   DETAIL
    drrt_cnt = (drrt_cnt+ 1)
    IF (mod(drrt_cnt,10)=1)
     stat = alterlist(drrt_cs_func->qual,(drrt_cnt+ 9))
    ENDIF
    drrt_cs_func->qual[drrt_cnt].tab_name = di.info_char
   FOOT REPORT
    drrt_cs_func->tab_cnt = drrt_cnt, stat = alterlist(drrt_cs_func->qual,drrt_cs_func->tab_cnt)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Gathering correct pk_where and ptam template functions."
  FOR (drrt_cnt = 0 TO drrt_cs_func->tab_cnt)
    SET drrt_cs_func->qual[drrt_cnt].pkw_func_name = drcs_get_func_name(drrt_cs_func->qual[drrt_cnt].
     tab_name,"PK_WHERE")
    IF (check_error(dm_err->eproc) != 0)
     GO TO exit_script
    ENDIF
    SET drrt_cs_func->qual[drrt_cnt].ptam_func_name = drcs_get_func_name(drrt_cs_func->qual[drrt_cnt]
     .tab_name,"PTAM")
    IF (check_error(dm_err->eproc) != 0)
     GO TO exit_script
    ENDIF
    IF ((((drrt_cs_func->qual[drrt_cnt].pkw_func_name <= " ")) OR ((drrt_cs_func->qual[drrt_cnt].
    ptam_func_name <= " "))) )
     SET drrt_skip = 1
     SET drrt_cnt = drrt_cs_func->tab_cnt
    ENDIF
  ENDFOR
  IF (drrt_skip=0)
   SET drrt_cnt = 0
   SET dm_err->eproc = "Querying user_objects to verify current state functions exist"
   SELECT INTO "nl:"
    FROM user_objects uo,
     (dummyt d  WITH seq = value(drrt_cs_func->tab_cnt))
    PLAN (d)
     JOIN (uo
     WHERE uo.object_type="FUNCTION"
      AND uo.status="VALID"
      AND (((uo.object_name=drrt_cs_func->qual[d.seq].pkw_func_name)) OR ((uo.object_name=
     drrt_cs_func->qual[d.seq].ptam_func_name))) )
    HEAD REPORT
     drrt_cnt = 0
    DETAIL
     drrt_cnt = (drrt_cnt+ 1)
    FOOT REPORT
     IF ((drrt_cnt != (drrt_cs_func->tab_cnt * 2)))
      drrt_skip = 1
     ENDIF
    WITH nocounter
   ;end select
  ...
  ENDIF
  IF (drrt_skip=0)
   SET dm_err->eproc = "Exploding out trigger regen tasks..."
   CALL disp_msg("",dm_err->logfile,0)
   SET drrt_status = drtq_delete_task_process("REFRESH TRIGGER PROCESS")
   IF (drrt_status="F")
    GO TO exit_script
   ENDIF
   EXECUTE dm_rmc_explode_trig_task
   IF (check_error(dm_err->eproc) != 0)
    GO TO exit_script
   ELSE
    SET dm_err->eproc = "Trigger regen tasks exploded successfully."
    SET readme_data->status = "S"
    SET readme_data->message = "Trigger regen tasks exploded successfully."
   ENDIF
  ELSE
   SET readme_data->message = "Missing required pl/sql functions trigger regeneration was skipped."
   SET readme_data->status = "S"
  ENDIF
 ELSE
  SET drrt_status = drtq_delete_task_process("REFRESH TRIGGER PROCESS")
  IF (drrt_status="F")
   GO TO exit_script
  ENDIF
  SET readme_data->message = "Trigger regeneration not needed."
  SET readme_data->status = "S"
 ENDIF
#exit_script
