CREATE PROGRAM ce_get_lr_children_all:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1
   ...
   JOIN (lr
   WHERE expand(idx,nstart,(nstart+ (nsize - 1)),lr.event_id,request->event_list[idx].event_id))
   JOIN (ce
   WHERE ce.event_id=lr.linked_event_id
    AND ce.valid_from_dt_tm=lr.valid_from_dt_tm
    AND ce.record_status_cd != record_status_deleted)
  ORDER BY ce.event_id, lr.event_id, ce.valid_until_dt_tm
  HEAD ce.event_id
   donothing = 0
  HEAD lr.event_id
   eventcnt += 1
