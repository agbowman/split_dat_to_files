CREATE PROGRAM ags_exe_plan_load:dba
 SET ags_exe_plan_load_mod = "000 10/06/06"
 CALL echo("<===== AGS_EXE_PLAN_LOAD Begin =====>")
 CALL echo(concat("MOD:",ags_exe_plan_load_mod))
 FREE RECORD reply
 RECORD reply(
   1 status_data
     2 status = c1
 ...
 DECLARE check_for_dups = i2 WITH public, noconstant(1)
 ...
 DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_exe_plan_load_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_EXE_PLAN_LOAD"
 ...
 WHILE (ipos < bs_length)
   ...
   CASE (field_name)
    ...
    OF "CHECK_FOR_DUPS":
     SET check_for_dups = cnvtint(field_data)
    ELSE
     SET failed = input_error
     SET table_name = "WORKING_BATCH_SELECTION"
     SET ilog_status = 1
...
#skip_parse
 ...
 FOR (exe_idx = 1 TO work->qual_knt)
   ...
   IF (current_task_id > 0)
    EXECUTE ags_plan_load value(current_task_id)
   ELSE
    SET failed = input_error
