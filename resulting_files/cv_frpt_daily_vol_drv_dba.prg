CREATE PROGRAM cv_frpt_daily_vol_drv:dba
 PROMPT
 ...
 IF (qrlstartdate > qrlenddate)
  GO TO exit_script
 ENDIF
 DECLARE signed_cd = f8 WITH constant(uar_get_code_by("MEANING",4000341,"SIGNED"))
 ...
 SELECT
  IF (( $ORG_ID=0.0)
   AND org_sec_ind=0.0)
   FROM cv_proc c
   WHERE c.proc_status_cd=signed_cd
    AND c.action_dt_tm >= cnvtdatetime(qrlstartdate)
    AND c.action_dt_tm < cnvtdatetime(qrlenddate)
  ELSEIF (( $ORG_ID=0.0)
   AND org_sec_ind=1.0)DISTINCT
   FROM cv_proc c,
    encounter e
   WHERE c.proc_status_cd=signed_cd
    AND c.action_dt_tm >= cnvtdatetime(qrlstartdate)
  ...
  DETAIL
   ...
   reply_obj->cv_list[nlistcnt].rpl_mth_yr = c.action_dt_tm, audit_record->qual[nlistcnt].order_id =
   c.order_id
  FOOT REPORT
