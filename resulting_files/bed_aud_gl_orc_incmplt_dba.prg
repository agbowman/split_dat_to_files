CREATE PROGRAM bed_aud_gl_orc_incmplt:dba
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (o.activity_type_cd=apat_cd
    ...
    temp->olist[o_cnt].no_coll_req_allinstr_ind = 0, temp->olist[o_cnt].no_coll_req_instr_ind = 0
   ELSE
    temp->olist[o_cnt].no_coll_req_allinstr_ind = 1, temp->olist[o_cnt].no_coll_req_instr_ind = 1
   ENDIF
 ...
 FOR (x = 1 TO temp->o_cnt)
   ...
   .no_routing_ind=1)) OR ((temp->olist[x].no_coll_req_allinstr_ind=1))) )) )) )
