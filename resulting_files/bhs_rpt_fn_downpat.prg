CREATE PROGRAM bhs_rpt_fn_downpat
 PROMPT
 ...
 DECLARE _times36b0 = i4 WITH noconstant(0), protect
   ...
   SELECT
    ...
    PLAN (e
     ...
     WHERE e.encntr_id=ea.encntr_id
      ...
      AND ea.active_ind=1)
    HEAD REPORT
     _d0 = e.reg_dt_tm, _d1 = p.name_last_key, _d2 = p.name_first_key,
     _d3 = ea.alias, _fenddetail = (rptreport->m_pageheight - rptreport->m_marginbottom),
 ...
 SUBROUTINE detailsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(format(e.reg_dt_tm,"@SHORTDATETIME"),
      char(0)))
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_times36b0)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Trauma Report ",char(0)))
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 1.500)
    SET rptsd->m_width = 4.167
    SET rptsd->m_height = 0.646
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Downtime Patients",char(0)))
    SET rptsd->m_flags = 0
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_RPT_FN_DOWNPAT"
 ...
 SUBROUTINE _createfonts(dummy)
   ...
   SET _times36b0 = uar_rptcreatefont(_hreport,rptfont)
   SET rptfont->m_pointsize = 10
