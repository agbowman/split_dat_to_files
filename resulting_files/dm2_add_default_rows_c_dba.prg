CREATE PROGRAM dm2_add_default_rows_c:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dadrc_logfile_name = vc WITH protect, constant("dm2_adddrc_")
 ...
 DECLARE dadrc_blocking_str = vc WITH protect, noconstant("")
 DECLARE dadrc_loop_var = i2 WITH protect, noconstant(0)
 DECLARE dadrc_in_parse_ind = i2 WITH protect, noconstant(1)
 DECLARE dadrc_time_in_sec = i2 WITH protect, noconstant(0)
 DECLARE dadr_ins_default_row(tbl_idx=i4) = i2
 DECLARE dadr_create_ora_trigger(sub_trig_name=vc,sub_idx=i4) = i2
 IF (check_logfile(dadrc_logfile_name,".log","DM2_ADD_DEFAULT_ROWS_C.PRG ...") != 1)
  GO TO exit_program
 ENDIF
 IF (validate(dadr_tname," ")=" ")
  SET dm_err->eproc = concat(
   " DM2_ADD_DEFAULT_ROWS_C Error. This a sub-program for DM2_ADD_DEFAULT_ROWS and cannot",
   " run by itself.  Please execute DM2_ADD_DEFAULT_ROWS instead.")
 ...
 SUBROUTINE dadr_ins_default_row(sub_idx)
   ...
   DECLARE didr_ts_default = vc WITH protect, noconstant(" ")
   DECLARE didr_ndx = i4 WITH protect, noconstant(0)
   DECLARE didr_fs = c9 WITH protect, noconstant("")
   ...
   FOR (idx2 = 1 TO dadr_tbl->tbl[sub_idx].col_cnt)
     IF ((dadr_tbl->tbl[sub_idx].col[idx2].pk_ind=1)
      AND (dadr_tbl->tbl[sub_idx].col[idx2].data_type IN ("TIMESTAMP*", "DATE")))
      IF ((dadr_tbl->tbl[sub_idx].col[idx2].data_type="DATE"))
       ...
       IF (substring(1,6,didr_date_default)="TODATE"
        ...
        CASE (substring(7,2,didr_date_default))
         ...
         OF "12":
          SET dadr_mon = "DEC"
        ENDCASE
        SET dadr_ccl_datefmt = concat("CNVTDATETIMEUTC('",substring(9,2,didr_date_default),"-",
         dadr_mon,"-",
         substring(11,4,didr_date_default)," ",substring(15,2,didr_date_default),":",substring(17,2,
          didr_date_default),
         ":",substring(19,2,didr_date_default),"',0)")
       ELSE
        SET dadr_ccl_datefmt = "CNVTDATETIMEUTC('31-DEC-2100 00:00:00',0)"
        SET dadr_tbl->tbl[sub_idx].col[idx2].data_default =
        "TO_DATE('12/31/2100 00:00:00','MM/DD/YYYY HH24:MI:SS')"
       ENDIF
      ENDIF
      IF ((dadr_tbl->tbl[sub_idx].col[idx2].data_type="TIMESTAMP*"))
       SET didr_ts_default = cnvtupper(cnvtalphanum(dadr_tbl->tbl[sub_idx].col[idx2].data_default))
       SET didr_ndx = findstring("MMDDYYYYHH24MISSFF",didr_ts_default,1,0)
       IF (substring(1,11,didr_ts_default)="TOTIMESTAMP"
        AND didr_ndx > 0)
        CASE (substring(12,2,didr_ts_default))
         OF "01":
          SET dadr_mon = "JAN"
         OF "02":
          SET dadr_mon = "FEB"
         ...
         OF "12":
          SET dadr_mon = "DEC"
        ENDCASE
        SET didr_fs = format(substring(26,(didr_ndx - 26),didr_ts_default),"#########;L")
        SET didr_fs = build(didr_fs,fillstring(value((9 - size(trim(didr_fs)))),"0"))
        SET dadr_ccl_datefmt = concat("CNVTTIMESTAMP(CNVTDATETIMEUTC('",substring(14,2,
          didr_ts_default),"-",dadr_mon,"-",
         substring(16,4,didr_ts_default)," ",substring(20,2,didr_ts_default),":",substring(22,2,
          didr_ts_default),
         ":",substring(24,2,didr_ts_default),"',0)",",",didr_fs,
         ")")
        CALL echo(concat("dadr_ccl_datefmt = <",dadr_ccl_datefmt,">"))
       ELSE
        SET dadr_ccl_datefmt = "CNVTTIMESTAMP(CNVTDATETIMEUTC('31-DEC-2100 00:00:00',0), 000000000)"
        SET dadr_tbl->tbl[sub_idx].col[idx2].data_default =
        "TO_TIMESTAMP('12/31/2100 00:00:00.000000000','MM/DD/YYYY HH24:MI:SS.FF')"
       ENDIF
      ENDIF
     ...
     IF ((dadr_tbl->tbl[sub_idx].col[idx2].nullable="N")
      ...
      CASE (trim(dadr_tbl->tbl[sub_idx].col[idx2].data_type))
       ...
       OF patstring("TIMESTAMP*",0):
        SET dadr_ccl_datefmt = "SYSTIMESTAMP"
       ELSE
     ...
     IF (dadr_valid_table=1)
      IF ((dadr_tbl->tbl[sub_idx].col[idx2].pk_ind=1))
       SET dadr_of_text = concat(dadr_of_text," ",trim(dadr_tbl->tbl[sub_idx].col[idx2].col_name,3),
        ",")
       IF (trim(dadr_tbl->tbl[sub_idx].col[idx2].data_type,3) IN ("DATE", "TIMESTAMP*"))
   ...
   IF (dadr_valid_table=1)
    ...
    IF (curqual=0
     AND dadr_error_found=0)
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat("Call parsing insert of default row, command is: ",dadr_insert_sql))
     ENDIF
     CALL parser(dadr_insert_sql,1)
     SET dm_err->ecode = error(dm_err->emsg,1)
     IF ((dm_err->ecode > 0))
      ROLLBACK
      SET dadr_valid_table = 0
      SET dm_err->eproc = concat("*** Warning : Unable to insert the default row for table: ",trim(
        dadr_tbl->tbl[sub_idx].tname),"  Reason code: ",dm_err->emsg)
      SET dm_err->user_action = "This message is only a warning and can be ignored. "
      CALL disp_msg("",dm_err->logfile,0)
      SET dadr_warning_cnt = (dadr_warning_cnt+ 1)
     ELSE
      COMMIT
 ...
 SUBROUTINE dadr_create_ora_trigger(sub_idx)
   ...
   ENDIF
   IF (dadr_retry_ceiling > 0)
    WHILE (dadrc_in_parse_ind=1
     AND dadrc_loop_var < dadr_retry_ceiling)
      IF (dir_check_in_parse(currdbuser,dadr_tbl->tbl[sub_idx].tname,dadrc_in_parse_ind,
       dadrc_blocking_str)=0)
       RETURN(0)
      ENDIF
      SET dadrc_loop_var = (dadrc_loop_var+ 1)
      IF (dadrc_in_parse_ind=1)
       SET dm_err->eproc = concat("Pausing 10 seconds due to hard parse event detected on ",trim(
         currdbuser),".",dadr_tbl->tbl[sub_idx].tname,". Attempt ",
        trim(cnvtstring(dadrc_loop_var)),"/",trim(cnvtstring(dadr_retry_ceiling)),".")
       CALL disp_msg(" ",dm_err->logfile,0)
       CALL pause(10)
       IF (dadrc_loop_var=dadr_retry_ceiling)
        SET dadrc_time_in_sec = (dadr_retry_ceiling * 10)
        SET dm_err->eproc = concat("Validating ",trim(dadr_tbl->tbl[sub_idx].tname),
         " table is NOT part of a long ","parse event prior to creation of default row trigger.")
        SET dm_err->emsg = concat("Parse event found against ",trim(dadr_tbl->tbl[sub_idx].tname),
         " table, cannot ","continue with trigger creation. Wait time exceeded (",trim(cnvtstring(
           dadrc_time_in_sec)),
         " seconds). ",dadrc_blocking_str)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET dadr_error_found = 1
        SET dadr_error_cnt = (dadr_error_cnt+ 1)
        SET dm_err->err_ind = 0
    ...
    ENDWHILE
   ENDIF
   SET dadr_text = concat(" CREATE OR REPLACE TRIGGER ",trim(cnvtupper(currdbuser)),".",trim(
     dcot_del_trig_name))
   IF ((dadr_tbl->tbl[sub_idx].trig_version > 0))
    SET dadr_text = concat(dadr_text," /*Version_",trim(cnvtstring(dadr_tbl->tbl[sub_idx].
       trig_version)),"*/")
   ENDIF
   SET dadr_text = concat(dadr_text," BEFORE DELETE OR UPDATE ")
   IF (trim(dadr_tbl->tbl[sub_idx].tname) != "CODE_VALUE")
    SET dadr_text = concat(dadr_text," ",dadr_of_text)
   ENDIF
   SET dadr_text = concat(dadr_text," ON ",trim(cnvtupper(currdbuser)),".",trim(dadr_tbl->tbl[sub_idx
     ].tname),
   ...
   ENDIF
   SET dadr_text = concat(dadr_text," BEGIN ",
    "if NVL(SYS_CONTEXT('CERNER','FIRE_REFCHG_TRG_MVR'),'YES')='YES' then ",
    "   RAISE_APPLICATION_ERROR(-20100,'The default row for table ",trim(dadr_tbl->tbl[sub_idx].tname
     ),
    " cannot be updated or deleted.'); ","end if;"," END;")
