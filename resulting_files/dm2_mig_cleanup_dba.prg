CREATE PROGRAM dm2_mig_cleanup:dba
 ...
 SUBROUTINE dm2_rename_login_default(sbr_rld_mode)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD dmc_user_priv
 RECORD dmc_user_priv(
   1 cnt = i4
   1 qual[*]
     2 username = vc
 )
 DECLARE dmc_file = vc WITH protect, noconstant(concat(dmr_mig_data->dm2_mig_log,"dm2_mig_config.txt"
   ))
 DECLARE dmc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmc_str = vc WITH protect, noconstant("")
 DECLARE dmc_queue_fnd_ret = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_cleanup",".log","dm2_mig_cleanup LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_cleanup"
 CALL disp_msg("",dm_err->logfile,0)
 IF (dmr_validate_src_and_tgt(0,1)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Executing in MODE:",dmcr_cleanup->mode," for User:",evaluate(
   dmcr_cleanup->user,"DM2NOTSET","ALL",dmcr_cleanup->user))
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dmcr_cleanup->mode="DOWNTIME")
  AND (dmcr_cleanup->user != "DM2NOTSET"))
  SET dm_err->eproc = "Validate that user is migration user"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2_MIG_USER"
    AND (d.info_name=dmcr_cleanup->user)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat(dmcr_cleanup->user," is not a valid migration user.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 ...
 CALL clear(1,1)
 IF ((dmcr_cleanup->mode != "DOWNTIME"))
  IF (dm2_findfile(dmc_file) > 0)
   SET dm_err->eproc = concat("Attempting to remove ",dmc_file)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF (remove(dmc_file)=0)
    SET dm_err->emsg = concat("Unable to remove ",dmc_file)
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
  ENDIF
  SET dm_err->eproc = "Get list of users with quota on I_DM2MIG"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM dba_ts_quotas d
   WHERE d.tablespace_name="I_DM2MIG"
   HEAD REPORT
    dmc_user_priv->cnt = 0
   DETAIL
    dmc_user_priv->cnt = (dmc_user_priv->cnt+ 1), stat = alterlist(dmc_user_priv->qual,dmc_user_priv
     ->cnt), dmc_user_priv->qual[dmc_user_priv->cnt].username = d.username
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  IF ((dmc_user_priv->cnt > 0))
   FOR (dmc_cnt = 1 TO dmc_user_priv->cnt)
     SET dm_err->eproc = concat("Remove I_DM2MIG quota for user ",dmc_user_priv->qual[dmc_cnt].
      username)
     CALL disp_msg(" ",dm_err->logfile,0)
     IF (dm2_push_cmd(concat("rdb alter user ",dmc_user_priv->qual[dmc_cnt].username,
       " quota 0 on I_DM2MIG go"),1)=0)
      GO TO exit_script
     ENDIF
   ENDFOR
  ENDIF
 ...
 IF ((dmcr_cleanup->mode="DOWNTIME")
  AND (dmcr_cleanup->user != "DM2NOTSET"))
  SET dm_err->eproc = concat("Clean DB Objects for ",trim(dmcr_cleanup->user)," user.")
  CALL disp_msg(" ",dm_err->logfile,0)
  EXECUTE dm2_mig_clean_dbobjects
  IF (dm_err->err_ind)
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((((dmcr_cleanup->mode != "DOWNTIME")) OR ((dmcr_cleanup->mode="DOWNTIME")
  AND (dmcr_cleanup->user="DM2NOTSET"))) )
  SET dm_err->eproc = "Stop Archive and Monitoring Jobs."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dmr_get_gg_dir(null)=0)
   GO TO exit_script
  ENDIF
  IF ((dm2_sys_misc->cur_os="AXP"))
   IF (dmr_check_batch_queue(dmr_batch_queue,dmc_queue_fnd_ret)=0)
    GO TO exit_script
   ENDIF
  ENDIF
  IF (((dmc_queue_fnd_ret=1
   AND (dm2_sys_misc->cur_os="AXP")) OR ((dm2_sys_misc->cur_os != "AXP"))) )
   IF (dmr_stop_job("MON",0)=0)
    IF ((dm_err->emsg="GGDELIVERY or GGCAPTURE do not exist. Unable to stop job."))
     SET dm_err->err_ind = 0
     CALL echo("***** THE ABOVE MESSAGES ARE ACCEPTABLE *****")
    ELSE
     GO TO exit_script
    ENDIF
   ENDIF
   IF (dmr_stop_job("ARCH",0)=0)
    IF ((dm_err->emsg="GGDELIVERY or GGCAPTURE do not exist. Unable to stop job."))
     SET dm_err->err_ind = 0
     CALL echo("***** THE ABOVE MESSAGES ARE ACCEPTABLE *****")
    ELSE
   ...
   ENDIF
  ENDIF
  SET dm_err->eproc = "Stop Compare."
  CALL disp_msg(" ",dm_err->logfile,0)
  EXECUTE dm2_mig_stop_verify_data
  IF ((dm_err->err_ind > 0))
   GO TO exit_script
  ENDIF
 ENDIF
 SET dmc_queue_fnd_ret = 0
 IF ((dmcr_cleanup->mode != "DOWNTIME"))
  IF ((dm2_sys_misc->cur_os="AXP"))
   SET dm_err->eproc = concat("Remove migration queue:",dmr_batch_queue)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dmr_check_batch_queue(dmr_batch_queue,dmc_queue_fnd_ret)=0)
    GO TO exit_script
   ENDIF
   IF (dmc_queue_fnd_ret=1)
    SET dmc_str = concat("$stop/queue/next ",dmr_batch_queue)
    IF (dm2_push_dcl(dmc_str)=0)
     GO TO exit_script
    ENDIF
    SET dmc_str = concat("$delete/queue ",dmr_batch_queue)
    IF (dm2_push_dcl(dmc_str)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   IF (dm2_push_dcl(concat("delete ccluserdir:dm2_cpyarc_",char(42),".com;",char(42)))=0)
    GO TO exit_script
   ENDIF
  ENDIF
 ENDIF
 IF ((dmcr_cleanup->mode != "DOWNTIME"))
  SET dm_err->eproc = "Clean DM_INFO."
  CALL disp_msg(" ",dm_err->logfile,0)
  EXECUTE dm2_mig_clean_dminfo
  IF (dm_err->err_ind)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Clean DM2_COLINIT_DBMIG dm_info rows"
  CALL disp_msg(" ",dm_err->logfile,0)
  DELETE  FROM dm_info
   WHERE info_domain="DM2_COLINIT_DBMIG"
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  COMMIT
  SET dm_err->eproc = "Clean NFS Mounts."
  CALL disp_msg(" ",dm_err->logfile,0)
  EXECUTE dm2_mig_clean_nfs
  IF (dm_err->err_ind)
   GO TO exit_script
  ENDIF
 ENDIF
...
#exit_script
 ...
 CALL clear(1,1)
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_cleanup completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_cleanup")
END GO
