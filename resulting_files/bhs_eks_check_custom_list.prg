CREATE PROGRAM bhs_eks_check_custom_list
 PROMPT
 ...
 ENDIF
 SELECT INTO "NL:"
  dpl.name, dplce.encntr_id
  FROM dcp_patient_list dpl,
   prsnl pr,
   dcp_pl_custom_entry dplce1,
  ...
  PLAN (dpl
   ...
   JOIN (dplce1
   WHERE outerjoin(dpl.patient_list_id)=dplce1.patient_list_id
    AND dplce1.person_id=outerjoin(var_person_id))
   JOIN (dplce2
