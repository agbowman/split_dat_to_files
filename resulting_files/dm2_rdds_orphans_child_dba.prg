CREATE PROGRAM dm2_rdds_orphans_child:dba
 ...
 SUBROUTINE get_err_val(null)
  ...
  RETURN(sbr_err_val)
 END ;Subroutine
 FREE RECORD chg_log_except
 RECORD chg_log_except(
   1 qual[*]
     2 table_name = vc
     2 column_name = vc
     2 from_value = f8
 ...
 DECLARE except_cnt = i4
 DECLARE dcle_loop = i4
 DECLARE dcle_table_name = vc
 DECLARE dcle_except_tab = vc
 DECLARE dcle_fnd = i4
 DECLARE dcle_srch = i4
 DECLARE dcle_fnd_ind = i2
 DECLARE dcle_col_ind = i2
 DECLARE dcle_task_ind = i2
 DECLARE dcle_prev_tab = vc
 DECLARE dcle_code_set = f8
 SET dcle_except_tab = concat(value(dm2_ref_data_doc->pre_link_name),"DM_CHG_LOG_EXCEPTION",value(
   dm2_ref_data_doc->post_link_name))
 SELECT INTO "NL:"
  FROM (parser(dcle_except_tab) d)
  WHERE (d.target_env_id=dm2_ref_data_doc->env_target_id)
   AND d.log_type="NOXLAT"
   AND d.from_value > 0
  ORDER BY d.table_name
  HEAD REPORT
   except_cnt = 0, stat = alterlist(chg_log_except->qual,1000)
  DETAIL
   except_cnt = (except_cnt+ 1), chg_log_except->qual[except_cnt].table_name = d.table_name,
   chg_log_except->qual[except_cnt].column_name = d.column_name,
   chg_log_except->qual[except_cnt].from_value = d.from_value
  FOOT REPORT
   stat = alterlist(chg_log_except->qual,except_cnt)
 ...
 IF (curqual=0)
  SET dcle_quit = 1
 ELSE
  SET dm_err->eproc = "Searching for orphaned data"
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dcle_prev_tab = ""
  FOR (dcle_loop = 1 TO except_cnt)
    SET dcle_task_ind = 0
    SET dcle_table_name = concat(dm2_ref_data_doc->pre_link_name,chg_log_except->qual[dcle_loop].
     table_name,dm2_ref_data_doc->post_link_name)
    IF (dcle_prev_tab != dcle_table_name)
     SET dcle_col_ind = 0
     FREE RANGE p
     CALL parser(concat("range of p is ",chg_log_except->qual[dcle_loop].table_name," go"),1)
     SET dcle_col_ind = evaluate(validate(p.updt_task,- (999999)),- (999999),0,1)
     SET dcle_prev_tab = dcle_table_name
    ENDIF
    SET dcle_fnd_ind = 0
    IF ((chg_log_except->qual[dcle_loop].column_name=""))
     SET stat = alterlist(cust_rs->qual,0)
     IF ((chg_log_except->qual[dcle_loop].table_name="DCP_FORMS_REF"))
      SET stat = alterlist(cust_rs->qual,2)
      SET cust_rs->qual[1].column_name = "DCP_FORM_INSTANCE_ID"
      SET cust_rs->qual[2].column_name = "DCP_FORMS_REF_ID"
     ELSEIF ((chg_log_except->qual[dcle_loop].table_name="DCP_SECTION_REF"))
      SET stat = alterlist(cust_rs->qual,2)
      SET cust_rs->qual[1].column_name = "DCP_SECTION_INSTANCE_ID"
      SET cust_rs->qual[2].column_name = "DCP_SECTION_REF_ID"
     ELSE
      SET dcle_fnd = locateval(dcle_srch,1,dguc_reply->rs_tbl_cnt,chg_log_except->qual[dcle_loop].
       table_name,dguc_reply->dtd_hold[dcle_srch].tbl_name)
      IF (dcle_fnd != 0)
       SET dcle_fnd_ind = 0
       IF ((dguc_reply->dtd_hold[dcle_fnd].pk_cnt >= 1))
        FOR (dcle_srch = 1 TO dguc_reply->dtd_hold[dcle_fnd].pk_cnt)
          IF ((((dguc_reply->dtd_hold[dcle_fnd].pk_hold[dcle_srch].pk_name="*ID")) OR ((dguc_reply->
          dtd_hold[dcle_fnd].pk_hold[dcle_srch].pk_name="CODE_VALUE"))) )
           SET stat = alterlist(cust_rs->qual,(size(cust_rs->qual,5)+ 1))
           SET cust_rs->qual[size(cust_rs->qual,5)].column_name = dguc_reply->dtd_hold[dcle_fnd].
           pk_hold[dcle_srch].pk_name
          ENDIF
        ENDFOR
       ENDIF
      ENDIF
     ENDIF
     FOR (dcle_srch = 1 TO size(cust_rs->qual,5))
       CALL parser(concat("select into 'NL:' from ",dcle_table_name," d where d.",cust_rs->qual[
         dcle_srch].column_name," = chg_log_except->qual[dcle_loop].from_value"),0)
       IF (dcle_col_ind=1)
        CALL parser(concat(" detail dcle_fnd_ind = 1 if (d.updt_task in (15301, 4310001)) ",
          " dcle_task_ind=1 endif with nocounter go"),1)
       ELSE
        CALL parser(" detail dcle_fnd_ind = 1 with nocounter go",1)
       ENDIF
       IF (check_error(dm_err->eproc)=1)
        SET dcle_loop = except_cnt
        SET dm_err->err_ind = 1
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       ENDIF
       IF (dcle_fnd_ind >= 1)
        ROLLBACK
        IF (dcle_task_ind=1)
         UPDATE  FROM (parser(dcle_except_tab) d)
          SET d.log_type = "UPDTSK", d.column_name = cust_rs->qual[dcle_srch].column_name
          WHERE (d.table_name=chg_log_except->qual[dcle_loop].table_name)
           AND (d.from_value=chg_log_except->qual[dcle_loop].from_value)
           AND (d.target_env_id=dm2_ref_data_doc->env_target_id)
          WITH nocounter
         ;end update
        ELSE
         UPDATE  FROM (parser(dcle_except_tab) d)
          SET d.log_type = "INSRC", d.column_name = cust_rs->qual[dcle_srch].column_name
          WHERE (d.table_name=chg_log_except->qual[dcle_loop].table_name)
           AND (d.from_value=chg_log_except->qual[dcle_loop].from_value)
           AND (d.target_env_id=dm2_ref_data_doc->env_target_id)
          WITH nocounter
        ...
        ENDIF
        COMMIT
        SET dcle_srch = size(cust_rs->qual,5)
       ENDIF
     ENDFOR
     IF (dcle_fnd_ind=0)
      ROLLBACK
     ...
     ENDIF
    ELSE
     CALL parser(concat("select into 'NL:' from ",dcle_table_name," d where d.",chg_log_except->qual[
       dcle_loop].column_name," = chg_log_except->qual[dcle_loop].from_value"),0)
     CALL parser(" detail dcle_fnd_ind = 1 ",0)
     IF (dcle_col_ind=1)
      CALL parser("if (d.updt_task in (15301,4310001)) dcle_task_ind=1 endif ",0)
     ENDIF
     IF ((chg_log_except->qual[dcle_loop].table_name="CODE_VALUE"))
      CALL parser(" dcle_code_set = d.code_set ",0)
     ENDIF
     CALL parser("with nocounter go",1)
     IF (check_error(dm_err->eproc)=1)
      SET dm_err->err_ind = 1
     ...
     ELSE
      IF (dcle_fnd_ind=1)
       IF (dcle_task_ind=1
        AND dcle_code_set != 93)
        UPDATE  FROM (parser(dcle_except_tab) d)
         SET d.log_type = "UPDTSK"
         WHERE (d.table_name=chg_log_except->qual[dcle_loop].table_name)
          AND (d.from_value=chg_log_except->qual[dcle_loop].from_value)
          AND (d.target_env_id=dm2_ref_data_doc->env_target_id)
         WITH nocounter
        ;end update
       ELSEIF (dcle_code_set=93)
        UPDATE  FROM (parser(dcle_except_tab) d)
         SET d.log_type = "NOMV03"
         WHERE (d.table_name=chg_log_except->qual[dcle_loop].table_name)
          AND (d.column_name=chg_log_except->qual[dcle_loop].column_name)
          AND (d.from_value=chg_log_except->qual[dcle_loop].from_value)
          AND (d.target_env_id=dm2_ref_data_doc->env_target_id)
         WITH nocounter
        ;end update
...
#orphan_child
 FREE SET except_cnt
 FREE SET dcle_loop
 FREE SET dcle_table_name
 FREE SET dcle_except_tab
 FREE SET dcle_fnd
 FREE SET dcle_srch
 FREE SET dcle_fnd_ind
END GO
