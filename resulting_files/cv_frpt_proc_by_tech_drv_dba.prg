CREATE PROGRAM cv_frpt_proc_by_tech_drv:dba
 PROMPT
 ...
 IF (qrlstartdate > qrlenddate)
  GO TO exit_script
 ENDIF
 DECLARE step_completed_var = f8 WITH constant(uar_get_code_by("MEANING",4000440,"COMPLETED"))
 ...
 SELECT
  IF (( $ORG_ID=0.0)
   ...
   FROM cv_step cs,
    ...
    person p
   PLAN (cs
    WHERE cs.step_status_cd=step_completed_var
     AND cs.perf_start_dt_tm >= cnvtdatetime(qrlstartdate)
  ...
  DETAIL
   ...
   IF (mod(nlistcnt,10)=1)
    stat = alterlist(reply_obj->cv_list,(nlistcnt+ 9)), stat = alterlist(audit_record->qual,(nlistcnt
     + 9))
   ENDIF
   reply_obj->cv_list[nlistcnt].rpl_task_disp = trim(uar_get_code_display(csr.task_assay_cd)),
   reply_obj->cv_list[nlistcnt].rpl_tech_name = trim(p.name_full_formatted), audit_record->qual[
   nlistcnt].order_id = c.order_id
  FOOT REPORT
 ...
 ENDIF
#exit_script
 CALL cv_log_msg_post("MOD 009 16/08/2018 RT050705")
END GO
