CREATE PROGRAM dm2_aload_afd_tables
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_cvs",".log","dm2_aload_afd_tables")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_afd_tables"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daat_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daat_file = vc WITH protect, noconstant( $2)
 DECLARE daat_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daat_batch = i4 WITH protect, noconstant(0)
 DECLARE daat_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE daat_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daat_pkg,daat_file_loc_ret,daat_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daat_batch=0)
  SET daat_batch = 5000
 ENDIF
 IF (daat_expected_rows=0)
  WHILE ((dm_err->err_ind=0)
   AND daat_rows_returned=10000)
    SET dm_err->eproc = "Delete AFD Table Rows"
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    DELETE  FROM dm_afd_tables d
     WHERE d.alpha_feature_nbr=daat_pkg
    ...
    ELSE
     COMMIT
    ENDIF
    SET daat_rows_returned = curqual
    IF ((dm_err->debug_flag > 0))
     CALL echo(build("Rows deleted:",daat_rows_returned))
    ENDIF
  ENDWHILE
 ELSE
  EXECUTE dm2_dbimport concat(daat_file_loc_ret,daat_file), "dm2_aload_sch_tab", daat_batch
  CALL echo(concat(daat_file_loc_ret,daat_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_afd_tables completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_afd_tables")
END GO
