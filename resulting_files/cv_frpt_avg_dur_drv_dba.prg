CREATE PROGRAM cv_frpt_avg_dur_drv:dba
 PROMPT
 ...
 SELECT
  IF (( $ORG_ID=0.0)
   ...
   FROM cv_proc c,
    ...
    prsnl p
   PLAN (cs
    WHERE cs.perf_start_dt_tm >= cnvtdatetime(qrlstartdate)
     AND cs.perf_start_dt_tm < cnvtdatetime(qrlenddate)
     AND cs.perf_stop_dt_tm >= cs.perf_start_dt_tm)
    JOIN (css
    WHERE css.cv_step_id=cs.cv_step_id)
    JOIN (c
    WHERE c.cv_proc_id=cs.cv_proc_id
     AND ((c.order_physician_id+ 0) > 0.0))
    JOIN (p
  ...
  DETAIL
   ...
   IF (mod(nlistcnt,10)=1)
    stat = alterlist(reply_obj->cv_list,(nlistcnt+ 9)), stat = alterlist(audit_record->qual,(nlistcnt
     + 9))
   ENDIF
   reply_obj->cv_list[nlistcnt].rpl_catalog_disp = trim(uar_get_code_display(c.catalog_cd)),
   reply_obj->cv_list[nlistcnt].rpl_full_name = trim(p.name_full_formatted), reply_obj->cv_list[
   nlistcnt].rpl_dur_min =
   IF (curutc=1) datetimediff(cs.perf_stop_dt_tm,cs.perf_start_dt_tm,4)
   ELSE datetimediff(cnvtdatetimeutc(cs.perf_stop_dt_tm,3),cnvtdatetimeutc(cs.perf_start_dt_tm,3),4)
   ENDIF
   ,
   reply_obj->cv_list[nlistcnt].rpl_mth_yr = c.action_dt_tm, reply_obj->cv_list[nlistcnt].rpl_day =
   cs.perf_start_dt_tm, audit_record->qual[nlistcnt].order_id = c.order_id
  FOOT REPORT
