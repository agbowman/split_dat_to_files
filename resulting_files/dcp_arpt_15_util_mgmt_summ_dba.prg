CREATE PROGRAM dcp_arpt_15_util_mgmt_summ:dba
 RECORD rpt_params(
   1 output_device = vc
   1 beg_dt_tm = dq8
   1 end_dt_tm = dq8
 ...
 ENDIF
 CALL echo("creating custom RDBMS database table cust_co_risk_rec")
 SELECT INTO TABLE cust_co_risk_rec
  risk_id = type("f8"), icu_cd = type("f8"), readmit_ind = type("i2")
  WITH organization = "TEMPRDB", index(risk_id)
 ;end select
 CALL echo("inserting into table: cust_co_risk_rec")
 IF ((rpt_params->date_type=4))
  INSERT  FROM cust_co_risk_rec
   (risk_id, icu_cd, readmit_ind)(SELECT
    risk_id = ra.risk_adjustment_id, icu_cd = ra.admit_icu_cd, readmit_ind = ra.readmit_ind
 ...
 ELSE
  ...
  ;end insert
 ENDIF
 CALL echo(build("curqual = ",curqual," rpt_params->date_type = ",rpt_params->date_type))
 SELECT INTO "nl:"
  FROM cust_co_risk_rec ccrr
  DETAIL
   risk_record->total_admit = (risk_record->total_admit+ 1)
   IF (ccrr.readmit_ind=1)
    risk_record->total_readmit = (risk_record->total_readmit+ 1)
   ENDIF
  WITH nocounter
 ;end select
 CALL echo(build("risk_record->total_admit=",risk_record->total_admit," risk_record->total_readmit=",
   risk_record->total_readmit))
 ...
 SELECT INTO "nl:"
  FROM risk_adjustment r,
   ...
   cust_co_risk_rec crr
  PLAN (crr)
   JOIN (r
   WHERE r.risk_adjustment_id=crr.risk_id)
   JOIN (ra
 ...
 SELECT INTO "nl:"
  ...
  PLAN (crr)
   JOIN (r
   WHERE crr.risk_id=r.risk_adjustment_id)
   JOIN (ra
   WHERE ra.risk_adjustment_id=r.risk_adjustment_id
    AND ra.active_ind=1
    AND ra.cc_day=1)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (crr)
   JOIN (rae
   WHERE rae.risk_adjustment_id=crr.risk_id
 ...
 SELECT INTO "nl:"
  ...
  PLAN (crr)
   JOIN (ra
   WHERE ra.risk_adjustment_id=crr.risk_id)
   JOIN (rad
