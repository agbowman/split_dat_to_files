CREATE PROGRAM dm_info_rows:dba
 ...
 IF (v_days_to_keep < 2)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"k1",
   "You must keep at least 2 days' worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
  SET reply->status_data.status = "F"
 ELSE
  SELECT INTO "nl:"
   ...
   WHERE di.info_domain="CODE SET UPDATE"
    AND parser(sbr_getrowidnotexists(
     "di.updt_dt_tm < cnvtdatetime(curdate - v_days_to_keep,curtime3)","di"))
   DETAIL
    row_cnt = (row_cnt+ 1)
    IF (mod(row_cnt,50)=1)
     stat = alterlist(reply->rows,(row_cnt+ 49))
    ENDIF
    reply->rows[row_cnt].row_id = di.rowid
   WITH nocounter, maxqual(di,value(cnvtint(request->max_rows)))
  ;end select
  SET v_err_code2 = error(v_errmsg2,1)
  IF (v_err_code2 > 0)
   SET reply->err_code = v_err_code2
   SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"CODEERROR",
    "Failed in 'CODE SET UPDATE' collection: %1","s",nullterm(v_errmsg2))
   SET reply->status_data.status = "F"
   GO TO exit_script
  ENDIF
  IF (((request->max_rows - row_cnt) > 0))
   SELECT INTO "nl:"
    d.rowid
    FROM dm_info d
    WHERE d.info_domain="RDDS SEQ MATCH:*"
     AND datetimediff(cnvtdatetime(curdate,curtime3),d.info_date) >= 365
    DETAIL
     row_cnt = (row_cnt+ 1)
     IF (mod(row_cnt,50)=1)
      stat = alterlist(reply->rows,(row_cnt+ 49))
    ...
    WITH nocounter, maxqual(d,value((request->max_rows - row_cnt)))
   ;end select
   SET v_err_code2 = error(v_errmsg2,0)
   IF (v_err_code2 > 0)
    SET reply->err_code = v_err_code2
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"RDDSERROR",
     "Failed in 'RDDS SEQ MATCH' collection: %1","s",nullterm(v_errmsg2))
    SET reply->status_data.status = "F"
    GO TO exit_script
   ENDIF
  ENDIF
