CREATE PROGRAM dcpreqgen05:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   orders->sex = uar_get_code_display(p.sex_cd), orders->age = cnvtage(p.birth_dt_tm), orders->dob =
   format(datetimezone(p.birth_dt_tm,p.birth_tz,2),"@SHORTDATE"),
   orders->admit_dt = format(e.reg_dt_tm,"@SHORTDATE"), orders->facility = uar_get_code_display(e
    .loc_facility_cd), orders->nurse_unit = uar_get_code_display(e.loc_nurse_unit_cd),
  ...
  HEAD o.order_id
   ...
   IF (curutc > 0)
    orders->qual[ord_cnt].order_dt = concat(format(datetimezone(oa.order_dt_tm,oa.order_tz),
      "@SHORTDATE")," ",datetimezonebyindex(oa.order_tz,offset,daylight,7,oa.order_dt_tm))
   ELSE
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   PLAN (d1)
   HEAD REPORT
    print_header = "Y", first_page = "Y", "{LPI/8}{CPI/12}{FONT/8}",
    "  ", row + 1, line1 = fillstring(80,"_"),
    saved_pos = 0
   HEAD PAGE
    line9 = fillstring(80,"_"), temp = fillstring(50," "), spaces = fillstring(20," "),
    pri = fillstring(20," "), iv = fillstring(20," "), o2 = fillstring(20," "),
    indications = fillstring(20," "), dt_tm_type = 0, start_dt_tm = fillstring(20," "),
    transport = fillstring(20," "), wgt = fillstring(20," "), wgt_ind = fillstring(20," "),
    "{CPI/8}{POS/1/70}", "", "{CPI/10}",
    CALL center(orders->facility,1,187), row + 1, print_time = format(cnvtdatetime(curdate,curtime),
     "@SHORTDATETIMENOSEC"),
    "{cpi/12}{pos/210/130}", "Print Date/Time:  ", print_time,
    row + 1, "{cpi/12}{pos/85/150}{b}", line1,
    "{endb}", row + 1, "{cpi/12}{pos/85/170}",
    "Room: ", orders->room, row + 1,
    "{cpi/12}{pos/85/190}", "Financial #: ", orders->fnbr,
    row + 1, "{cpi/12}{pos/340/190}", "Medical Record #: ",
    orders->mrn, row + 1, "{cpi/12}{pos/85/210}",
    "Patient Name:  ",
    CALL print(trim(orders->name,3)), row + 1,
    "{cpi/12}{pos/340/210}", "DOB: ", orders->dob,
    row + 1, "{cpi/12}{pos/340/225}", "Age: ",
    orders->age, row + 1, "{cpi/12}{pos/340/240}",
    "Sex: ", orders->sex, row + 1,
    "{cpi/12}{pos/85/255}", "Allergies: "
    ...
    ENDIF
    "{cpi/12}{pos/85/280}", "Chief Complaint:  "
    IF (textlen(orders->admit_diagnosis) > 50)
     CALL print(trim(concat(substring(1,47,orders->admit_diagnosis),"..."))), row + 1
    ELSE
     CALL print(orders->admit_diagnosis), row + 1
    ENDIF
    "{cpi/12}{pos/85/300}{b}", line1, "{endb}",
    row + 1
    IF (saved_pos > 0)
     "{B}{CPI/10}{POS/1/90}",
     CALL center(build(cnvtupper(orders->qual[saved_pos].catalog)," REQUISITION"),1,190), "{endb}",
     row + 1, "{CPI/10}{POS/1/110}",
     CALL center(concat("*******************",trim(uar_get_code_display(orders->qual[saved_pos].
        action_type_cd)),"*******************"),1,190),
     row + 1, "{CPI/12}{POS/85/720}{B}", "Order ID: ",
     "{ENDB}", "{CPI/12}{POS/130/720}", orders->qual[saved_pos].order_id"#########",
     row + 1, "{cpi/12}{pos/85/320}", "{b}",
     "Procedure:   ", orders->qual[saved_pos].mnemonic, "{endb}",
     "{cpi/12}{pos/85/340}", "Scheduled Start Date/Time: ", start_dt_tm,
     row + 1, "{cpi/12}{pos/85/360}", "Ordering Physician:  ",
     orders->qual[saved_pos].order_dr, row + 1, "{cpi/12}{pos/340/360}",
     "Attending Physician:  ", orders->attending, row + 1,
     "{cpi/12}{pos/85/380}", "Entered by: ", orders->qual[saved_pos].enter_by,
     row + 1, "{cpi/12}{pos/340/380}", "Ordered Date/Time: ",
     orders->qual[saved_pos].order_dt, row + 1, "{cpi/12}{pos/85/400}{b}",
     line1, "{endb}", saved_pos = 0
    ENDIF
   DETAIL
    FOR (vv = 1 TO value(ord_cnt))
      IF ((orders->qual[vv].display_ind=1))
       IF (first_page="N")
        BREAK
       ENDIF
       first_page = "N", "{B}{CPI/10}{POS/1/90}",
       CALL center(build(cnvtupper(orders->qual[vv].catalog)," REQUISITION"),1,190),
       "{endb}", row + 1, "{CPI/10}{POS/1/110}",
       CALL center(concat("*******************",trim(uar_get_code_display(orders->qual[vv].
          action_type_cd)),"*******************"),1,190), row + 1, "{LPI/8}{CPI/12}{FONT/8}",
       "  ", row + 1, "{CPI/12}{POS/85/720}{B}",
       "Order ID: ", "{ENDB}", "{CPI/12}{POS/130/720}",
       orders->qual[vv].order_id"#########", row + 1, "{cpi/12}{pos/85/320}",
       "{b}", "Procedure:   ", orders->qual[vv].mnemonic,
       "{endb}", "{cpi/12}{pos/85/340}", "Scheduled Start Date/Time: ",
       start_dt_tm, row + 1, "{cpi/12}{pos/85/360}",
       "Ordering Physician:  ", orders->qual[vv].order_dr, row + 1,
       "{cpi/12}{pos/340/360}", "Attending Physician:  ", orders->attending,
       row + 1, "{cpi/12}{pos/85/380}", "Entered by: ",
       orders->qual[vv].enter_by, row + 1, "{cpi/12}{pos/340/380}",
       "Ordered Date/Time: ", orders->qual[vv].order_dt, row + 1,
       "{cpi/12}{pos/85/400}{b}", line1, "{endb}",
       xcol = 85, ycol = 420
       FOR (fsub = 1 TO 31)
         FOR (ww = 1 TO orders->qual[vv].d_cnt)
           IF ((((orders->qual[vv].d_qual[ww].group_seq=fsub)) OR (fsub=31
            ...
            IF (textlen(trim(orders->qual[vv].d_qual[ww].value,3)) > 0)
             ...
             "  ", row + 1, xcol = 212
             IF (textlen(orders->qual[vv].d_qual[ww].label_text) > 30)
              ycol = (ycol+ 12)
             ENDIF
             IF ((orders->qual[vv].d_qual[ww].value_cnt > 1))
              FOR (dsub = 1 TO orders->qual[vv].d_qual[ww].value_cnt)
                ...
                IF (ycol > 640
                 ...
                 CALL print(calcpos(xcol,ycol)), "**Continued on next page**", saved_pos = vv,
                 BREAK, xcol = 85, ycol = 420,
                 "{CPI/13}",
                 CALL print(calcpos(xcol,ycol)),
      ...
      IF ((orders->qual[vv].comment_ind=1)
       ...
       CALL print(calcpos(xcol,ycol)), "Order Comments: ", ycol = (ycol+ 12)
    ...
    ENDFOR
   FOOT PAGE
    "{cpi/12}{pos/85/680}", "Ordering Physician:  ", orders->qual[saved_pos].order_dr,
    row + 1, "{cpi/12}{pos/340/680}", "Attending Physician:  ",
    orders->attending, row + 1, "{cpi/12}{pos/85/700}",
    "Entered by: ", orders->qual[saved_pos].enter_by, row + 1,
    "{cpi/12}{pos/340/700}", "Ordered Date/Time: ", orders->qual[saved_pos].order_dt,
    row + 1
