CREATE PROGRAM bbt_get_auto_dir_alert_info:dba
 ...
 IF (((issued_event_type_cd=0.0) OR (((transfused_event_type_cd=0.0) OR (((destroyed_event_type_cd=
 0.0) OR (((shipped_event_type_cd=0.0) OR (((dcurrentnametypecd=0.0) OR (((dcombineaddcd=0.0) OR (
 dactivestatuscd=0.0)) )) )) )) )) )) )
 ...
 IF (trim(request->retrieve_data)="T")
  ...
  SELECT INTO "nl:"
   ...
   pe.event_status_flag, pe.event_prsnl_id, p.product_id,
   p.product_nbr, p.product_cd, p.cur_expire_dt_tm,
   bp.cur_abo_cd, bp.cur_rh_cd, bp.supplier_prefix
   ...
   PLAN (d_p)
    JOIN (pr
    WHERE (pr.product_id=products->productlist[d_p.seq].product_id)
     AND pr.product_id > 0
   ...
   HEAD REPORT
    lreplycnt = 0, cnt = 0
   HEAD pr.product_id
    lreplycnt += 1, stat = alterlist(reply->qual,lreplycnt), reply->qual[lreplycnt].product_id = pe
    .product_id,
    ...
    reply->qual[lreplycnt].cur_rh_cd = bp.cur_rh_cd, reply->qual[lreplycnt].locked_ind = pr
    .locked_ind
    IF (pr.locked_ind=1)
   ...
   HEAD pe.product_event_id
    IF (bad_product != "T")
     IF (((pe.event_type_cd=destroyed_event_type_cd) OR (((pe.event_type_cd=transfused_event_type_cd)
      OR (((pe.event_type_cd=issued_event_type_cd) OR (pe.event_type_cd=shipped_event_type_cd)) ))
     )) )
      bad_product = "T"
     ELSE
      good_product = "T", count2 += 1, stat = alterlist(reply->qual[lreplycnt].qual2,count2),
      reply->qual[lreplycnt].qual2[count2].product_event_id = pe.product_event_id, reply->qual[
      lreplycnt].qual2[count2].event_type_cd = pe.event_type_cd
   ...
   FOOT  pr.product_id
    IF (good_product="T"
     AND bad_product="F")
     reply->qual[lreplycnt].nbr_of_states = count2, reply->qual[lreplycnt].good_product_ind = "T",
     cnt += 1
    ELSE
   ...
   FOR (lidx = 1 TO lreplycnt)
     ...
     SELECT INTO "nl:"
      ...
      FROM person_combine_det pcd,
       ...
       person_combine pc
      PLAN (ad
       WHERE (ad.product_id=reply->qual[lidx].product_id))
       JOIN (pcd
       WHERE pcd.entity_id=ad.product_event_id
 ...
 ELSE
  ...
  SELECT INTO "nl:"
   ...
   HEAD pr.product_id
    lreplycnt += 1, stat = alterlist(reply->qual,lreplycnt), reply->qual[lreplycnt].product_id = pe
    .product_id,
    reply->qual[lreplycnt].product_cd = pr.product_cd, reply->qual[lreplycnt].product_nbr = pr
    .product_nbr, reply->qual[lreplycnt].product_sub_nbr = pr.product_sub_nbr,
