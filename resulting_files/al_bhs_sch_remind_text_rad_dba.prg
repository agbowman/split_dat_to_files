CREATE PROGRAM al_bhs_sch_remind_text_rad:dba
 EXECUTE bhs_hlp_ftp
 ...
 DECLARE ms_file = vc WITH protect, constant(concat("rad_reminder_text_",trim(cnvtstring(rand(0),20),
    3),"_",format(cnvtdatetime(sysdate),"YYYYMMDDHHMMSS;;d"),".dat"))
 ...
 IF (ms_weekday IN ("Saturday", "Sunday"))
  ...
  GO TO exit_script
 ELSEIF (ms_weekday="Monday")
  SET mf_start_date = cnvtdatetime((curdate+ 2),000000)
  SET mf_stop_date = cnvtdatetime((curdate+ 3),235959)
 ELSEIF (ms_weekday IN ("Tuesday", "Wednesday"))
  SET mf_start_date = cnvtdatetime((curdate+ 3),000000)
  SET mf_stop_date = cnvtdatetime((curdate+ 3),235959)
 ELSEIF (ms_weekday="Thursday")
  SET mf_start_date = cnvtdatetime((curdate+ 3),000000)
  SET mf_stop_date = cnvtdatetime((curdate+ 4),235959)
 ELSEIF (ms_weekday="Friday")
  SET mf_start_date = cnvtdatetime((curdate+ 4),000000)
  SET mf_stop_date = cnvtdatetime((curdate+ 4),235959)
 ENDIF
 SELECT INTO "nl:"
  FROM sch_appt sa,
  ...
  ORDER BY sa.person_id, sa.beg_dt_tm, ph.phone_type_seq
  HEAD REPORT
   rem->cnt = 0
  DETAIL
   IF (size(trim(replace(replace(replace(trim(ph.phone_num,3),")",""),"(",""),"-",""),3)) > 0
    ...
    mf_cs200_mmplmtseedinmammoright_cd)
     ...
     ml_skip_cnt += 1
    ELSE
     ml_idx2 = locateval(ml_idx1,1,rem->cnt,sa.person_id,rem->qual[ml_idx1].f_person_id,
      trim(format(sa.beg_dt_tm,"MMDDYYYY;;q"),3),rem->qual[ml_idx1].s_appt_dt)
     IF (ml_idx2=0)
      rem->cnt += 1, stat = alterlist(rem->qual,rem->cnt), rem->qual[rem->cnt].f_person_id = sa
      .person_id,
      rem->qual[rem->cnt].ms_first_name = trim(pn.name_first,3), rem->qual[rem->cnt].ms_last_name =
      trim(pn.name_last,3), rem->qual[rem->cnt].mf_department_number = sa.appt_location_cd,
      rem->qual[rem->cnt].mf_location_number = sa.appt_location_cd, rem->qual[rem->cnt].
      mf_sch_event_id = sa.sch_event_id, rem->qual[rem->cnt].ms_location_name = trim(
       uar_get_code_display(sa.appt_location_cd),3),
      rem->qual[rem->cnt].ms_provider_number = "542", rem->qual[rem->cnt].ms_appt_dt_tm = trim(format
       (sa.beg_dt_tm,"YYYY-MM-DD HH:MM ;;q"),3), rem->qual[rem->cnt].ms_visit_type = trim(
       uar_get_code_display(o.catalog_cd),3),
      rem->qual[rem->cnt].ms_visit_type_sch_alias = trim(cvo.alias,3), rem->qual[rem->cnt].
      ms_phone_num = replace(replace(replace(trim(ph.phone_num,3),")",""),"(",""),"-","")
      IF (sed.oe_field_display_value="Spanish")
       rem->qual[rem->cnt].ms_lang = "Spanish"
      ELSE
       rem->qual[rem->cnt].ms_lang = "English"
      ENDIF
      rem->qual[rem->cnt].s_appt_dt = trim(format(sa.beg_dt_tm,"MMDDYYYY;;q"),3)
     ENDIF
    ENDIF
   ENDIF
  WITH nocounter
 ...
 IF ((rem->cnt > 0))
  SELECT INTO value(ms_file)
   ...
   DETAIL
    FOR (ml_idx = 1 TO rem->cnt)
      ms_str = concat(rem->qual[ml_idx].ms_phone_num,"|",rem->qual[ml_idx].ms_lang,"|",trim(
       ...
       rem->qual[ml_idx].ms_visit_type_sch_alias,"|","Y","|","|",
       trim(cnvtstring(rem->qual[ml_idx].mf_department_number,20),3),"|","|","|",rem->qual[ml_idx].
       ms_phone_num,
       "|","|","|","|","|",
       "|","|","|","|","|",
       "|","|","|","|"),
   ...
   WITH nocounter, maxcol = 500, format = none,
    noheading, maxrow = 1
  ;end select
  SET ms_ftp_cmd = concat("put ",ms_file)
  SET ms_ftp_path = "ciscore/radiology/reminder"
  SET ms_ftp_host = "172.17.10.5"
  SET ms_ftp_username = '"bhs\cisftp"'
  SET ms_ftp_password = "C!sftp01"
  ...
  SET ms_dclcom = concat("mv ",ms_file," ",trim(logical("BHSCUST"),3),"/radiology/reminder/",
   ms_file)
  CALL echo(ms_dclcom)
  CALL dcl(ms_dclcom,size(trim(ms_dclcom)),ml_stat)
 ENDIF
