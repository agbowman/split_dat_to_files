CREATE PROGRAM dm_rx_pha_queueerr_rows:dba
 ...
 SET reply->table_name = "PHA_QUEUEERR"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 DECLARE i = i2 WITH noconstant(0)
 DECLARE v_pyxis_days_to_keep = i4 WITH protect, noconstant(- (1))
 FOR (i = 1 TO size(request->tokens,5))
   IF ((request->tokens[i].token_str="PYXISDAYSTOKEEP"))
    SET v_pyxis_days_to_keep = ceil(cnvtreal(request->tokens[i].value))
   ENDIF
 ENDFOR
 IF (v_pyxis_days_to_keep < 30)
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"PYXISDAYSTOKEEP",
   "You must keep at least 30 days' worth of data.  You entered %1 days or did not enter any value.",
   "i",v_pyxis_days_to_keep)
  GO TO the_end
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(qe.queue_id)
    FROM pha_queueerr qe
    WHERE qe.queue_id > 0
    DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     qe.rowid
     FROM pha_queueerr qe
     WHERE parser(sbr_getrowidnotexists("qe.queue_id between curMinID and curMaxID","qe"))
      AND qe.updt_dt_tm < cnvtdatetime((curdate - v_pyxis_days_to_keep),curtime3)
      AND ((qe.request_nbr=360015) OR (qe.request_nbr=360150))
     DETAIL
      v_rec_count += 1
      IF (mod(v_rec_count,20)=1)
       stat = alterlist(reply->rows,(v_rec_count+ 19))
      ENDIF
      reply->rows[v_rec_count].row_id = qe.rowid
     WITH nocounter, maxqual(qe,value(rowsleft))
    ;end select
