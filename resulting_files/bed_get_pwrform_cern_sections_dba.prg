CREATE PROGRAM bed_get_pwrform_cern_sections:dba
 ...
 END ;Subroutine
 DECLARE aresectionsdifferent(importedsectionuid=vc,existingsectionid=f8,formid=f8,formuid=vc,
  r_statuslevel=vc(ref),
  bailoutind=i2) = i2
 DECLARE aredescriptionsdifferent(importedsectionuid=vc,existingsectionid=f8) = i2
 DECLARE aresequencesdifferent(importedsectionuid=vc,existingsectionid=f8) = i2
 DECLARE areinputsdifferent(importedsectionuid=vc,existingsectionid=f8) = i2
 DECLARE areformsdifferent(formuid=vc,formid=f8) = i2
 DECLARE areassaysdifferent(importedsectionuid=vc,formuid=vc) = i2
 SUBROUTINE areformsdifferent(formuid,formid)
   CALL bedlogmessage("areFormsDifferent","Entering ...")
   DECLARE idescription = vc WITH protect, noconstant("")
   DECLARE edescription = vc WITH protect, noconstant("")
   DECLARE idefinition = vc WITH protect, noconstant("")
   DECLARE edefinition = vc WITH protect, noconstant("")
   DECLARE idonechartind = i2 WITH protect, noconstant(0)
   DECLARE edonechartind = i2 WITH protect, noconstant(0)
   DECLARE ienforcereq = i2 WITH protect, noconstant(0)
   DECLARE eenforcereq = i2 WITH protect, noconstant(0)
   DECLARE iflags = i4 WITH protect, noconstant(0)
   DECLARE eflags = i4 WITH protect, noconstant(0)
   DECLARE ieventcd = f8 WITH protect, noconstant(0)
   DECLARE eeventcd = f8 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM cnt_pf_key2 pk2,
     cnt_powerform cp
    PLAN (pk2
     WHERE pk2.form_uid=formuid)
     JOIN (cp
     WHERE cp.form_uid=outerjoin(pk2.form_uid))
    ORDER BY pk2.form_uid
    HEAD pk2.form_uid
     idescription = pk2.form_description, idefinition = pk2.form_definition, idonechartind = cp
     .done_charting_ind,
     ienforcereq = cp.enforce_required_ind, iflags = cp.form_flag, ieventcd = cp.form_event_cd
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    FROM dcp_forms_ref d
    WHERE d.dcp_forms_ref_id=formid
    ORDER BY d.dcp_forms_ref_id
    HEAD d.dcp_forms_ref_id
     edescription = d.description, edefinition = d.definition, edonechartind = d.done_charting_ind,
     eenforcereq = d.enforce_required_ind, eflags = d.flags, eeventcd = d.event_cd
    WITH nocounter
   ;end select
   IF (idefinition != edefinition)
    CALL logdebuginfo(build2("Form definition diff: ",idefinition,"::",edefinition))
    RETURN(true)
   ENDIF
   IF (idescription != edescription)
    CALL logdebuginfo(build2("Form description diff: ",idescription,"::",edescription))
    RETURN(true)
   ENDIF
   IF (idonechartind != edonechartind)
    CALL logdebuginfo(build2("Form doneChartInd diff: ",idonechartind,"::",edonechartind))
    RETURN(true)
   ENDIF
   IF (ienforcereq != eenforcereq)
    CALL logdebuginfo(build2("Form enforceReq diff: ",ienforcereq,"::",eenforcereq))
    RETURN(true)
   ENDIF
   IF (iflags != eflags)
    CALL logdebuginfo(build2("Form flags diff: ",iflags,"::",eflags))
    RETURN(true)
   ENDIF
   IF (ieventcd != eeventcd)
    CALL logdebuginfo(build2("Form event diff: ",ieventcd,"::",eeventcd))
    RETURN(true)
   ENDIF
   CALL bedlogmessage("areFormsDifferent","Exiting ...")
   RETURN(false)
 END ;Subroutine
 SUBROUTINE aresectionsdifferent(importedsectionuid,existingsectionid,formid,formuid,r_statuslevel,
  bailoutind)
   CALL bedlogmessage("areSectionsDifferent","Entering ...")
   DECLARE aresectiondifferent = i2 WITH protect, noconstant(false)
   CALL logdebuginfo(build2("Evaluating sections: ",importedsectionuid,"::",existingsectionid))
   IF (aredescriptionsdifferent(importedsectionuid,existingsectionid))
    CALL logdebuginfo("Sections descriptions are different")
    SET r_statuslevel = modified_section
    IF (bailoutind=1)
     RETURN(true)
    ELSE
     SET aresectiondifferent = true
    ENDIF
   ENDIF
   IF (formid > 0)
    IF (aresequencesdifferent(importedsectionuid,existingsectionid,formid))
     CALL logdebuginfo("Sections sequences are different")
     SET r_statuslevel = modified_section
     IF (bailoutind=1)
      RETURN(true)
     ELSE
    ...
    ENDIF
   ENDIF
   IF (areinputsdifferent(importedsectionuid,existingsectionid))
    CALL logdebuginfo("Sections inputs are different")
    SET r_statuslevel = modified_input
    IF (bailoutind=1)
     RETURN(true)
    ELSE
     SET aresectiondifferent = true
    ENDIF
   ENDIF
   IF (areassaysdifferent(importedsectionuid,formuid))
    CALL logdebuginfo("Assays are different")
    SET r_statuslevel = modified_assay
    IF (bailoutind=1)
     RETURN(true)
    ELSE
     SET aresectiondifferent = true
    ENDIF
   ENDIF
   CALL bedlogmessage("areSectionsDifferent","Exiting ...")
   RETURN(aresectiondifferent)
 END ;Subroutine
 SUBROUTINE aredescriptionsdifferent(importedsectionuid,existingsectionid)
   CALL bedlogmessage("areDescriptionsDifferent","Entering ...")
   DECLARE idefinition = vc WITH protect, noconstant("")
   DECLARE edefinition = vc WITH protect, noconstant("")
   DECLARE idescription = vc WITH protect, noconstant("")
   DECLARE edescription = vc WITH protect, noconstant("")
   SELECT INTO "nl:"
    FROM cnt_section_key2 sk2
    WHERE sk2.section_uid=importedsectionuid
    DETAIL
     idefinition = sk2.section_definition, idescription = sk2.section_description
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    FROM dcp_section_ref d
    WHERE d.dcp_section_ref_id=existingsectionid
     AND d.active_ind=true
    DETAIL
     edefinition = d.definition, edescription = d.description
    WITH nocounter
   ;end select
   CALL logdebuginfo(build2("Evaluating section: ",idefinition))
   IF (idescription != edescription)
    CALL logdebuginfo(build2("Section description diff: ",idescription,"::",edescription))
    RETURN(true)
   ENDIF
   CALL bedlogmessage("areDescriptionsDifferent","Exiting ...")
   RETURN(false)
 END ;Subroutine
 SUBROUTINE aresequencesdifferent(importedsectionuid,existingsectionid,formid)
   CALL bedlogmessage("areSequencesDifferent","Entering ...")
   DECLARE iseq = i4 WITH protect, noconstant(0)
   DECLARE eseq = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM dcp_forms_ref r,
     dcp_forms_def d
    PLAN (r
     WHERE r.dcp_forms_ref_id=formid
      AND r.active_ind=true)
     JOIN (d
     WHERE d.dcp_form_instance_id=r.dcp_form_instance_id
      AND d.dcp_forms_ref_id=r.dcp_forms_ref_id
      AND d.dcp_section_ref_id=existingsectionid)
    DETAIL
     eseq = d.section_seq
    WITH nocounter
   ;end select
   ...
   SELECT INTO "nl:"
    ...
    WHERE r.section_uid=importedsectionuid
     AND r.form_uid=form_uid
    DETAIL
     iseq = r.section_seq
    WITH nocounter
   ;end select
   IF (eseq != iseq)
    CALL logdebuginfo(build2("Section seq diff: ",iseq,"::",eseq))
    RETURN(true)
   ENDIF
   CALL bedlogmessage("areSequencesDifferent","Exiting ...")
   RETURN(false)
 END ;Subroutine
 SUBROUTINE areinputsdifferent(importedsectionuid,existingsectionid)
   CALL bedlogmessage("areInputsDifferent","Entering ...")
   IF (((importedsectionuid="") OR (existingsectionid=0)) )
    CALL bederror("Invalid sectionUID or existingSectionId. Null is not allowed.")
   ENDIF
   ...
   IF ( NOT (validate(getimportedinputsreply,0)))
   ...
   ENDIF
   SET getimportedinputsrequest->section_uid = importedsectionuid
   ...
   IF ( NOT (validate(getexistinginputsreply,0)))
   ...
   ENDIF
   IF (size(getimportedinputsreply->inputs,5) != size(getexistinginputsreply->inputs,5))
    CALL logdebuginfo("Sizes of replies don't match")
    RETURN(true)
   ENDIF
   IF (isinputmodified(getimportedinputsreply,getexistinginputsreply,true))
    RETURN(true)
   ENDIF
   CALL bedlogmessage("areInputsDifferent","Exiting ...")
   RETURN(false)
 END ;Subroutine
 SUBROUTINE areassaysdifferent(importedsectionuid,formuid)
   CALL bedlogmessage("areAssaysDifferent","Entering ...")
   DECLARE assaycnt = i4 WITH protect, noconstant(0)
   FREE RECORD tempimportedassays
   RECORD tempimportedassays(
     1 assays[*]
       2 task_assay_uid = vc
   )
   SELECT INTO "nl:"
    FROM cnt_input i
    PLAN (i
     WHERE i.section_uid=importedsectionuid
      AND i.merge_name="DISCRETE_TASK_ASSAY")
    DETAIL
     assaycnt = (assaycnt+ 1), stat = alterlist(tempimportedassays->assays,assaycnt),
     tempimportedassays->assays[assaycnt].task_assay_uid = i.task_assay_uid
    WITH nocounter
   ;end select
   FOR (assayinx = 1 TO assaycnt)
     IF (isassaymodified(tempimportedassays->assays[assayinx].task_assay_uid,formuid))
      RETURN(true)
     ENDIF
   ENDFOR
   CALL bedlogmessage("areAssaysDifferent","Exiting ...")
   RETURN(false)
 END ;Subroutine
 CALL bedbeginscript(0)
 DECLARE form_uid = vc WITH protect, constant(request->form_uid)
 DECLARE formrefid = f8 WITH protect, noconstant(request->dcp_form_ref_id)
 DECLARE modifiedformind = i2 WITH protect, noconstant(0)
 DECLARE sectionsizewithoutremoved = i4 WITH protect, noconstant(0)
 DECLARE getsectionsbyformuid(formuid=vc) = null
 DECLARE getsections(dummyvar=i2) = null
 DECLARE addsectionmatchesbyname(dummyvar=i2) = null
 DECLARE setconditionind(dummyvar=i2) = null
 DECLARE setformmodifiedstatus(dummyvar=i2) = null
 DECLARE setsectionmodifiedstatus(formid=f8,formuid=vc) = null
 DECLARE logdebuginfo(desc=vc) = i2
 SET reply->form_modified_status = none
 IF (form_uid > " ")
  IF (formrefid=0)
   SELECT INTO "nl:"
    FROM cnt_pf_key2 ck
    WHERE ck.form_uid=form_uid
    DETAIL
     formrefid = ck.dcp_forms_ref_id
    WITH nocounter
   ;end select
   CALL bederrorcheck("Error 001: Failed to retrieve dcp_forms_ref_id from cnt_pf_key2 table")
   IF (formrefid=0)
    SET reply->form_modified_status = added
   ENDIF
  ENDIF
  CALL getsectionsbyformuid(form_uid)
  CALL addsectionmatchesbyname(0)
  SET sectionsizewithoutremoved = size(reply->sections,5)
  IF (sectionsizewithoutremoved > 0)
   CALL setconditionind(0)
   CALL setsectionmodifiedstatus(formrefid,form_uid)
  ENDIF
  IF ((reply->form_modified_status != added))
   IF (modifiedformind=true)
    SET reply->form_modified_status = modified
   ENDIF
  ENDIF
 ELSE
  CALL getsections(0)
...
#exit_script
 ...
 SUBROUTINE getsectionsbyformuid(formuid)
   CALL bedlogmessage("getSectionsByFormUid","Entering ...")
   DECLARE cnt = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM cnt_pf_section_r sr,
     cnt_section_key2 sk,
     cnt_section s,
     dcp_section_ref d
    PLAN (sr
     WHERE sr.form_uid=formuid)
     JOIN (sk
     WHERE sk.section_uid=sr.section_uid)
     JOIN (s
     WHERE s.section_uid=sk.section_uid
      AND s.active_ind=1)
     JOIN (d
     WHERE d.dcp_section_ref_id=outerjoin(sk.dcp_section_ref_id)
      AND d.active_ind=outerjoin(1))
    ORDER BY sr.section_seq
    DETAIL
     cnt = (cnt+ 1), stat = alterlist(reply->sections,cnt), reply->sections[cnt].section_uid = sk
     .section_uid,
     reply->sections[cnt].description = sk.section_definition, reply->sections[cnt].width = s
     .section_width, reply->sections[cnt].height = s.section_height,
     reply->sections[cnt].sequence = sr.section_seq, reply->sections[cnt].dcp_section_ref_id = sk
     .dcp_section_ref_id, reply->sections[cnt].match_description = d.definition,
     reply->sections[cnt].match_name = d.description, reply->sections[cnt].match_width = d.width,
     reply->sections[cnt].match_height = d.height,
     reply->sections[cnt].ignore_ind = sr.ignore_ind, reply->sections[cnt].name = sk
     .section_description, reply->sections[cnt].modified_status = none
    WITH nocounter
   ;end select
   CALL bederrorcheck("Error 002: Failed to retrieve sections by form UID")
   CALL bedlogmessage("getSectionsByFormUid","Exiting ...")
 END ;Subroutine
 SUBROUTINE addsectionmatchesbyname(dummyvar)
   CALL bedlogmessage("addSectionMatchesByName","Entering ...")
   IF (size(reply->sections,5) > 0)
    SELECT INTO "nl:"
     FROM (dummyt d  WITH seq = size(reply->sections,5)),
      dcp_section_ref r
     PLAN (d
      WHERE (reply->sections[d.seq].dcp_section_ref_id=0))
      JOIN (r
      WHERE (r.definition=reply->sections[d.seq].description)
       AND r.active_ind=true)
     DETAIL
      reply->sections[d.seq].dcp_section_ref_id = r.dcp_section_ref_id, reply->sections[d.seq].
      match_description = r.definition, reply->sections[d.seq].match_name = r.description,
      reply->sections[d.seq].match_width = r.width, reply->sections[d.seq].match_height = r.height
     WITH nocounter
    ;end select
   ENDIF
   CALL bederrorcheck(
    "Error 003: Failed to retrieve matching sections in millennium based on section's defintions")
   CALL bedlogmessage("addSectionMatchesByName","Entering ...")
 END ;Subroutine
 SUBROUTINE getsections(dummyvar)
   CALL bedlogmessage("getSections","Entering ...")
   DECLARE cnt = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM cnt_section_key2 sk,
     cnt_section s,
    ...
    PLAN (sk)
     ...
     WHERE d.dcp_section_ref_id=outerjoin(sk.dcp_section_ref_id)
      AND d.active_ind=outerjoin(1))
    ORDER BY sk.section_description
    HEAD sk.section_uid
     cnt = (cnt+ 1), stat = alterlist(reply->sections,cnt), reply->sections[cnt].section_uid = sk
     .section_uid,
     reply->sections[cnt].description = sk.section_definition, reply->sections[cnt].width = s
     .section_width, reply->sections[cnt].height = s.section_height,
     reply->sections[cnt].dcp_section_ref_id = sk.dcp_section_ref_id, reply->sections[cnt].
     match_description = d.definition, reply->sections[cnt].match_name = d.description,
     reply->sections[cnt].match_width = d.width, reply->sections[cnt].match_height = d.height, reply
     ->sections[cnt].name = sk.section_description,
     reply->sections[cnt].modified_status = none
    WITH nocounter
   ;end select
   CALL bederrorcheck("Error 004: Failed to retrieve sections")
   CALL bedlogmessage("getSections","Entering ...")
 END ;Subroutine
 SUBROUTINE setconditionind(dummyvar)
   CALL bedlogmessage("setConditionInd","Entering ...")
   DECLARE secindex = i4 WITH protect, noconstant(0)
   DECLARE num = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(sectionsizewithoutremoved)),
     cnt_input i
    PLAN (d)
     JOIN (i
     WHERE (i.section_uid=reply->sections[d.seq].section_uid)
      AND i.pvc_name="conditional_section")
    DETAIL
     reply->sections[d.seq].condition_ind = 1
    WITH nocounter
   ;end select
   CALL bederrorcheck("Error 005: Failed to retrieve condition_ind")
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = value(sectionsizewithoutremoved)),
     cnt_section s
    PLAN (d1)
     JOIN (s
     WHERE (s.section_uid=reply->sections[d1.seq].section_uid))
    DETAIL
     IF (s.section_flag=1)
      reply->sections[d1.seq].conditional_section_ind = 1, reply->sections[d1.seq].condition_ind = 1
     ENDIF
    WITH nocounter
   ;end select
   CALL bederrorcheck("Error 006: Failed to retrieve conditional_section_ind and condition_ind")
   CALL bedlogmessage("setConditionInd","Exiting ...")
 END ;Subroutine
 SUBROUTINE setsectionmodifiedstatus(formid,formuid)
   CALL bedlogmessage("setSectionModifiedStatus","Entering ...")
   DECLARE num = i4 WITH protect, noconstant(1)
   DECLARE cnt = i4 WITH protect, noconstant(0)
   DECLARE secsize = i4 WITH protect, noconstant(0)
   DECLARE statuslevel = vc WITH protect, noconstant("")
   FREE RECORD getsectionsrequest
   RECORD getsectionsrequest(
     1 dcp_forms_ref_id = f8
     1 description = vc
     1 search_type_flag = vc
     1 max_reply = i4
   )
   FREE RECORD getsectionsreply
   RECORD getsectionsreply(
     1 sections[*]
       2 dcp_section_ref_id = f8
       2 description = vc
       2 width = i4
   ...
   IF (formid > 0)
    SET getsectionsrequest->dcp_forms_ref_id = formid
    EXECUTE bed_get_pwrform_sections  WITH replace("REQUEST",getsectionsrequest), replace("REPLY",
     getsectionsreply)
    IF ((getsectionsreply->status_data.status != "S"))
     CALL bederror("bed_get_pwrform_sections did not return success")
    ENDIF
    SET secsize = size(getsectionsreply->sections,5)
   ENDIF
   FOR (k = 1 TO secsize)
     SET num = 1
     SET loc1 = locateval(num,1,sectionsizewithoutremoved,getsectionsreply->sections[k].
      dcp_section_ref_id,reply->sections[num].dcp_section_ref_id)
     IF (loc1=0)
      SET cnt = (size(reply->sections,5)+ 1)
      SET stat = alterlist(reply->sections,cnt)
      SET reply->sections[cnt].description = getsectionsreply->sections[k].description
      SET reply->sections[cnt].width = getsectionsreply->sections[k].width
      SET reply->sections[cnt].height = getsectionsreply->sections[k].height
      SET reply->sections[cnt].sequence = getsectionsreply->sections[k].sequence
      SET reply->sections[cnt].dcp_section_ref_id = getsectionsreply->sections[k].dcp_section_ref_id
      SET reply->sections[cnt].match_description = getsectionsreply->sections[k].description
      SET reply->sections[cnt].match_name = getsectionsreply->sections[k].name
      SET reply->sections[cnt].match_width = getsectionsreply->sections[k].width
      SET reply->sections[cnt].match_height = getsectionsreply->sections[k].height
      SET reply->sections[cnt].ignore_ind = 0
      SET reply->sections[cnt].name = getsectionsreply->sections[k].name
      SET reply->sections[cnt].modified_status = removed
      SET reply->sections[cnt].modified_status_level = none
      SET modifiedformind = true
     ENDIF
   ENDFOR
   FOR (jk = 1 TO sectionsizewithoutremoved)
     CALL logdebuginfo(build2("SECTION: ",reply->sections[jk].description))
     SET reply->sections[jk].modified_status_level = none
     IF ((reply->sections[jk].dcp_section_ref_id > 0))
      SET num4 = 1
      SET loc1 = locateval(num4,1,secsize,reply->sections[jk].dcp_section_ref_id,getsectionsreply->
       sections[num4].dcp_section_ref_id)
      IF (loc1 > 0)
       IF (aresectionsdifferent(reply->sections[jk].section_uid,reply->sections[jk].
        dcp_section_ref_id,formrefid,formuid,statuslevel,
        1))
        SET reply->sections[jk].modified_status = modified
        SET reply->sections[jk].modified_status_level = statuslevel
        SET modifiedformind = true
       ENDIF
      ELSE
       CALL logdebuginfo("Section is new to the form")
       SET reply->sections[jk].modified_status = modified
       SET modifiedformind = true
      ENDIF
     ELSE
      SET reply->sections[jk].modified_status = added
      SET modifiedformind = true
     ENDIF
   ENDFOR
   CALL bedlogmessage("setSectionModifiedStatus","Exiting ...")
 END ;Subroutine
END GO
