CREATE PROGRAM dm_ocmb_problem_drr:dba
 ...
 IF (debug_ind=1)
  CALL echo(build("********DM_OCMB_PROBLEM_DRR - from_xxx_id = ",request->xxx_combine[icombine].
    from_xxx_id))
  CALL echo(build("********DM_OCMB_PROBLEM_DRR - to_xxx_id = ",request->xxx_combine[icombine].
    to_xxx_id))
 ENDIF
 SELECT INTO "nl:"
  frm.*
  FROM problem0859drr frm
 ...
 IF (v_cust_count1 > 0)
  ...
  ;end select
  SELECT INTO "nl:"
   FROM pregnancy_instance2029drr pi1,
    pregnancy_instance2029drr pi2
   PLAN (pi1
 ...
 SUBROUTINE del_from(s_df_pk_id,s_df_prev_act_ind,s_df_prev_act_status,s_df_prev_end_eff_dt_tm)
   UPDATE  FROM problem0859drr frm
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "PROBLEM0859DRR"
