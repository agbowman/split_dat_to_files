...
#ukr_common_rpt_exit
 ...
 IF (trust_id=0.0)
  GO TO exit_program
 ENDIF
 DECLARE ae_cd = f8 WITH noconstant(uar_get_code_by("DISPLAYKEY",71,"EMERGENCYDEPARTMENT"))
 DECLARE cnn_cd = f8 WITH noconstant(uar_get_code_by("MEANING",4,"MRN"))
 DECLARE trust_rel_code = f8 WITH noconstant(uar_get_code_by("MEANING",369,"NHSTRUSTCHLD"))
 ...
 SELECT DISTINCT INTO  $OUTDEV
  cnn = cnvtalias(pa.alias,pa.alias_pool_cd), name1 = p.name_full_formatted, sex1 = substring(1,1,
   uar_get_code_display(p.sex_cd)),
  dob1 = format(p.birth_dt_tm,"DD-MMM-YYYY HH:MM;;;D"), p_id1 = p.person_id, nhs_num1 = cnvtalias(pa2
   .alias,pa2.alias_pool_cd),
  name2 = p1.name_full_formatted, sex2 = substring(1,1,uar_get_code_display(p1.sex_cd)), dob2 =
  format(p1.birth_dt_tm,"DD-MMM-YYYY HH:MM;;;D"),
  p_id2 = p1.person_id, nhs_num2 = cnvtalias(pa3.alias,pa3.alias_pool_cd)
  FROM encounter e,
   person p,
   person p1,
   ...
   person_alias pa3
  PLAN (e
   WHERE (e.encntr_type_cd= $ENC_TYPE)
    AND ((e.encntr_type_cd=ae_cd
    AND e.arrive_dt_tm >= cnvtdatetime(cnvtdate2(start_date,"DD-MMM-YYYY"),0)
    AND e.arrive_dt_tm < cnvtdatetime(cnvtdate2(end_date,"DD-MMM-YYYY"),235959)) OR (e.encntr_type_cd
    != ae_cd
    AND e.reg_dt_tm >= cnvtdatetime(cnvtdate2(start_date,"DD-MMM-YYYY"),0)
    AND e.reg_dt_tm < cnvtdatetime(cnvtdate2(end_date,"DD-MMM-YYYY"),235959)
    AND ((e.organization_id=trust_id) OR (e.organization_id IN (
   (SELECT
    oor.related_org_id
   ...
   JOIN (pa
   WHERE pa.alias_pool_cd=oa.alias_pool_cd
    AND pa.active_ind=1
    AND pa.beg_effective_dt_tm < cnvtdatetime(curdate,curtime3)
    AND pa.end_effective_dt_tm > cnvtdatetime(curdate,curtime3))
   JOIN (pa1
   WHERE pa1.alias=pa.alias
    AND pa1.person_id != pa.person_id
    AND pa1.alias_pool_cd=oa.alias_pool_cd
   ...
   WHERE pa2.person_id=outerjoin(p.person_id)
    AND pa2.person_alias_type_cd=outerjoin(pa_ssn_cd)
   ...
   JOIN (pa3
   WHERE pa3.person_id=outerjoin(p1.person_id)
    AND pa3.person_alias_type_cd=outerjoin(pa_ssn_cd)
    AND pa3.active_ind=outerjoin(1)
    AND pa3.beg_effective_dt_tm < outerjoin(cnvtdatetime(curdate,curtime3))
    AND pa3.end_effective_dt_tm > outerjoin(cnvtdatetime(curdate,curtime3)))
  ORDER BY cnn
  WITH nocounter, format, separator = " "
 ;end select
#exit_program
