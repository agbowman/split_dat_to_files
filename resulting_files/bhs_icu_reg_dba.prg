CREATE PROGRAM bhs_icu_reg:dba
 PROMPT
 ...
 DECLARE _times14bi0 = i4 WITH noconstant(0), protect
 DECLARE _times100 = i4 WITH noconstant(0), protect
 DECLARE _times10b0 = i4 WITH noconstant(0), protect
 DECLARE _pen10s1c0 = i4 WITH noconstant(0), protect
 ...
 SUBROUTINE head_sectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_times14bi0)
 ...
 SUBROUTINE detail_sectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _dummypen = uar_rptsetpen(_hreport,_pen10s1c0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.157),(offsetx+ 10.000),(offsety
     + 0.157))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_ICU_REG"
 ...
 SUBROUTINE _createfonts(dummy)
   ...
   SET _times14bi0 = uar_rptcreatefont(_hreport,rptfont)
   SET rptfont->m_pointsize = 10
   SET rptfont->m_italic = rpt_off
 ...
 SUBROUTINE _createpens(dummy)
   ...
   SET _pen10s1c0 = uar_rptcreatepen(_hreport,rptpen)
 END ;Subroutine
 ...
 DECLARE mf_picu_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",220,"PICU"))
 ...
 FOR (ml_loop_cnt = 1 TO size(m_info->pat,5))
   ...
   SET mf_rem_space = (mf_page_size - (_yoffset+ line_section(rpt_calcheight)))
   IF ((mf_rem_space < (detail_section(rpt_calcheight)+ 0.32)))
    SET d0 = pagebreak(0)
    SET d0 = head_section(rpt_render)
    SET d0 = line_section(rpt_render)
   ENDIF
