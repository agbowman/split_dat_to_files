CREATE PROGRAM dm2_delete_code_value:dba
 DECLARE single_cv = f8 WITH public, noconstant(0.0)
 DECLARE v_trigger_str = vc WITH public, noconstant(" ")
 DECLARE success_cnt = i2 WITH public, noconstant(0)
 DECLARE cnt = i2 WITH public, noconstant(0)
 DECLARE get_triggers(null) = i4
 DECLARE delete_single_cv(null) = i4
 DECLARE delete_multiple_cv(null) = i4
 ...
 SET single_cv =  $1
 SET stat = check_logfile("dm2_del_cv",".log","DM2_DELETE_CODE_VALUE BEGIN")
 IF (single_cv > 0)
  IF ( NOT (get_triggers(null)))
   SET reply->status_data.status = "F"
   GO TO exit_script
  ENDIF
  IF (delete_single_cv(null))
   COMMIT
  ELSE
   ROLLBACK
   SET reply->status_data.status = "F"
   SET dm_err->eproc = dm_err->emsg
   CALL disp_msg(" ",dm_err->unique_fname,1)
   GO TO exit_script
  ENDIF
 ELSE
  IF ( NOT (validate(dm2_del_cv,0)))
   RECORD dm2_del_cv(
     1 cv_cnt = i4
     1 qual[*]
       2 code_value = f8
  ...
  ENDIF
  IF (size(dm2_del_cv->qual,5) > 0)
   IF ( NOT (get_triggers(null)))
    SET reply->status_data.status = "F"
    GO TO exit_script
   ENDIF
   IF (delete_multiple_cv(null))
    COMMIT
   ELSE
    ROLLBACK
    SET reply->status_data.status = "F"
   ...
   ENDIF
  ELSE
   SET dm_err->eproc = "dm2_del_cv contains no records"
   CALL disp_msg(" ",dm_err->unique_fname,1)
   GO TO exit_script
  ENDIF
 ENDIF
 SUBROUTINE get_triggers(null)
   SET dm_err->eproc = "Drop Trigger Begin"
   CALL disp_msg(" ",dm_err->unique_fname,0)
   IF (currdb="DB2UDB")
    IF ( NOT (dm2_get_suffixed_tablename("CODE_VALUE")))
     RETURN(0)
    ENDIF
    SET trig->table_name = dm2_table->suffixed_table_name
    SELECT INTO "nl:"
     t.trigger_name
     FROM dm2_user_triggers t
     WHERE (t.table_name=dm2_table->suffixed_table_name)
      AND t.trigger_name=patstring("TRG_CODE_VALUE_D*")
     ORDER BY t.table_name
     HEAD REPORT
      cnt = 0
     HEAD t.table_name
      cnt = (cnt+ 1), stat = alterlist(trig->qual,cnt), trig->table_name = t.table_name
     DETAIL
      len = textlen(trim(t.trigger_name,3))
      IF (substring((len - 1),len,trim(t.trigger_name,3))="$C")
       trig->qual[cnt].trg_c_exists = 1, trig->qual[cnt].trg_c_name = trim(t.trigger_name,3)
      ELSE
       trig->qual[cnt].trg_reg_exists = 1, trig->qual[cnt].trg_reg_name = trim(t.trigger_name,3)
      ENDIF
     WITH nocounter
    ;end select
   ELSEIF (currdb="ORACLE")
    SET trig->table_name = "CODE_VALUE"
    SELECT INTO "nl:"
     t.trigger_name
     FROM dm2_user_triggers t
     WHERE t.table_name="CODE_VALUE"
      AND t.trigger_name=patstring("TRG_CODE_VALUE_DELETE*")
     ORDER BY t.table_name
     HEAD REPORT
      cnt = 0
     HEAD t.table_name
      cnt = (cnt+ 1), stat = alterlist(trig->qual,cnt), trig->table_name = trim(t.table_name)
     DETAIL
      len = textlen(trim(t.trigger_name))
      IF (substring((len - 1),len,trim(t.trigger_name))="$C")
       trig->qual[cnt].trg_c_exists = 1, trig->qual[cnt].trg_c_name = trim(t.trigger_name)
      ELSE
       trig->qual[cnt].trg_reg_exists = 1, trig->qual[cnt].trg_reg_name = trim(t.trigger_name)
      ENDIF
     WITH nocounter
    ;end select
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE delete_single_cv(null)
   SET dm_err->eproc = "Delete Single CV Begin"
   CALL disp_msg(" ",dm_err->unique_fname,0)
   IF (currdb="DB2UDB")
    IF (size(trig->qual,5) > 0)
     FOR (x = 1 TO size(trig->qual,5))
       IF ((trig->qual[x].trg_c_exists=1)
        AND (trig->qual[x].trg_reg_exists=1))
        IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trig->qual[x].trg_c_name," go"),1)))
         RETURN(0)
        ENDIF
        IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trig->qual[x].trg_reg_name," go"),1)))
         RETURN(0)
        ENDIF
        SET trig->trigger_name = trig->qual[x].trg_reg_name
       ELSEIF ((trig->qual[x].trg_c_exists=1))
        IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trig->qual[x].trg_c_name," go"),1)))
         RETURN(0)
        ENDIF
        SET trig->trigger_name = trig->qual[x].trg_c_name
       ELSE
        IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trig->qual[x].trg_reg_name," go"),1)))
         RETURN(0)
        ENDIF
     ...
     ENDFOR
    ELSE
     SET trig->trigger_name = "TRG_CODE_VALUE_DEL"
    ENDIF
    DELETE  FROM code_value c
     WHERE c.code_value=single_cv
     WITH nocounter
    ;end delete
    IF (check_error("DM2_DELETE_CODE_VALUE:Delete_Single_CV"))
     RETURN(0)
    ENDIF
    SET v_trigger_str = concat("create trigger ",trig->trigger_name," AFTER DELETE ","on ",trig->
     table_name,
     " referencing old as old for each row mode db2sql"," when (old.code_set not in ",
     "(10172,6020,1310,1312,1319,14252,72,93,2200,2201,2203,",
     "2206,2208,2210,14229,14230,14231,14232,14233,14248,14249,14250,14251)) "," SIGNAL SQLSTATE ",
     "'","75000","'","(","'",
     "No deletes on code value allowed.","'",")")
   ...
   ELSEIF (currdb="ORACLE")
    IF (size(trig->qual,5) > 0)
     ...
     ENDFOR
    ELSE
     SET trig->trigger_name = "TRG_CODE_VALUE_DELETE"
    ENDIF
    DELETE  FROM code_value c
     WHERE c.code_value=single_cv
     WITH nocounter
    ...
    SET v_trigger_str = concat("create trigger ",trig->trigger_name," AFTER DELETE ","on ",trig->
     table_name,
     " referencing old as old for each row when (old.code_set not in ",
     "(10172,6020,1310,1312,1319,14252,72,93,2200,2201,2203,",
     "2206,2208,2210,14229,14230,14231,14232,14233,14248,14249,14250,14251)) ","begin ",
     "raise_application_error(-20100,",
     "'","No deletes allowed on the code value table.","'","); end;")
    SET v_trigger_str = concat("rdb asis(^",trim(v_trigger_str,3),"^) end go")
    IF ( NOT (dm2_push_cmd(v_trigger_str,1)))
     RETURN(0)
    ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE delete_multiple_cv(null)
   SET dm_err->eproc = "Delete Multiple CV Begin"
   CALL disp_msg(" ",dm_err->unique_fname,0)
   IF (currdb="DB2UDB")
    IF (size(trig->qual,5) > 0)
     FOR (x = 1 TO size(trig->qual,5))
     ...
     ENDFOR
    ELSE
     SET trig->trigger_name = "TRG_CODE_VALUE_D"
    ENDIF
    FOR (x = 1 TO value(dm2_del_cv->cv_cnt))
     DELETE  FROM code_value c
      WHERE (c.code_value=dm2_del_cv->qual[x].code_value)
      WITH nocounter
     ;end delete
     IF (check_error("DM2_DELETE_CODE_VALUE:Delete_Mulitple_CV"))
      RETURN(0)
     ENDIF
    ENDFOR
    SET v_trigger_str = concat("create trigger ",trig->trigger_name," AFTER DELETE ","on ",trig->
...
#exit_script
 IF ((dm_err->err_ind=1))
  SET dm_err->eproc = "EXITING DM2_DELETE_CODE_VALUE WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_DELETE_CODE_VALUE COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("dm2_del_cv")
END GO
