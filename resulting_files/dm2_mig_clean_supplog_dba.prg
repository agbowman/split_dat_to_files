CREATE PROGRAM dm2_mig_clean_supplog:dba
 ...
 DECLARE dmcs_pos = i4 WITH protect, noconstant(0)
 DECLARE dmcs_loc = i4 WITH protect, noconstant(0)
 DECLARE dmcs_idx = i4 WITH protect, noconstant(0)
 DECLARE dmcs_max_cnt = i4 WITH protect, noconstant(10)
 DECLARE dmcs_ownr_tbl_grp = vc WITH protect, noconstant("")
 DECLARE dmcs_object_name = vc WITH protect, noconstant("")
 DECLARE dmcs_info_idx = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_clean_suplog",".log","dm2_mig_clean_supplog LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_clean_supplog"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 CALL clear(1,1)
 CALL box(1,1,24,131)
 CALL text(2,2,concat("Database Migration Supp Log Group Cleanup is about to be performed on ",
   dmr_mig_data->tgt_db_name,"."))
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_SUPP_LOG"
   AND di.info_name="VERIFY_CNT"
  DETAIL
   dmcs_max_cnt = di.info_number
  WITH nocounter
 ;end select
 IF (check_error("Check for supp log validation cnt override.")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Query DM_INFO for migration supplemental log groups."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain IN ("DM2_SUPP_LOG_ADD_TRANDATA", "DM2_SUPP_LOG_ADD_TRANDATA_DELTA")
  DETAIL
   dmcs_cleanup->cnt = (dmcs_cleanup->cnt+ 1), stat = alterlist(dmcs_cleanup->qual,dmcs_cleanup->cnt),
   dmcs_loc = findstring(".",di.info_name,1,0),
   dmcs_cleanup->qual[dmcs_cleanup->cnt].owner = substring(1,(dmcs_loc - 1),di.info_name), dmcs_pos
    = findstring(".",di.info_name,1,1), dmcs_cleanup->qual[dmcs_cleanup->cnt].table_name = substring(
    (dmcs_loc+ 1),((dmcs_pos - dmcs_loc) - 1),di.info_name),
   dmcs_object_name = substring((dmcs_pos+ 1),size(di.info_name),di.info_name)
   IF (dmcs_object_name="SYS*")
    dmcs_cleanup->qual[dmcs_cleanup->cnt].object_name = ""
   ELSE
    dmcs_cleanup->qual[dmcs_cleanup->cnt].object_name = dmcs_object_name
   ENDIF
   dmcs_cleanup->qual[dmcs_cleanup->cnt].ownr_tbl_grp = di.info_name, dmcs_cleanup->qual[dmcs_cleanup
   ->cnt].info_char = di.info_char
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dmcs_cleanup->cnt < dmcs_max_cnt))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Validating min rows returned from DM_INFO to drop Supplemental log groups."
  SET dm_err->emsg = build("Rows returned from DM_INFO are less than min count (",dmcs_max_cnt,").")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining list of migration supplemental log groups."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_log_groups d
  WHERE d.log_group_name != "D7A*"
  ORDER BY d.owner, d.table_name, d.log_group_name
  DETAIL
   dmcs_idx = 0
   IF (d.log_group_name="GGS*")
    dmcs_ownr_tbl_grp = concat(trim(d.owner),".",trim(d.table_name),".",trim(d.log_group_name)),
    dmcs_idx = locateval(dmcs_idx,1,dmcs_cleanup->cnt,dmcs_ownr_tbl_grp,dmcs_cleanup->qual[dmcs_idx].
     ownr_tbl_grp)
    IF (dmcs_idx > 0)
     dmcr_cleanup->cnt = (dmcr_cleanup->cnt+ 1), stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->
      cnt), dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = dmcs_cleanup->qual[dmcs_idx].owner,
     dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = dmcs_cleanup->qual[dmcs_idx].object_name,
     dmcr_cleanup->qual[dmcr_cleanup->cnt].table_name = dmcs_cleanup->qual[dmcs_idx].table_name,
     dmcr_cleanup->qual[dmcr_cleanup->cnt].object_type = "SUPPLEMENTAL LOG GROUP",
     dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order = 2
    ENDIF
   ELSEIF (d.log_group_name="SYS*")
    dmcs_info_idx = 0
    IF (d.log_group_type="PRIMARY KEY LOGGING"
     AND d.always="ALWAYS"
     AND d.generated="GENERATED NAME")
     dmcs_info_idx = locateval(dmcs_info_idx,1,dmcs_cleanup->cnt,"PK_A_G",dmcs_cleanup->qual[
      dmcs_info_idx].info_char,
      trim(d.table_name),dmcs_cleanup->qual[dmcs_info_idx].table_name,trim(d.owner),dmcs_cleanup->
      qual[dmcs_info_idx].owner)
     IF (dmcs_info_idx > 0)
      dmcr_cleanup->cnt = (dmcr_cleanup->cnt+ 1), stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->
       cnt), dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = dmcs_cleanup->qual[dmcs_info_idx].owner,
      dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = d.log_group_name, dmcr_cleanup->qual[
      dmcr_cleanup->cnt].table_name = dmcs_cleanup->qual[dmcs_info_idx].table_name, dmcr_cleanup->
      qual[dmcr_cleanup->cnt].object_type = "SUPPLEMENTAL LOG GROUP",
      dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order = 2
     ENDIF
    ELSEIF (d.log_group_type="UNIQUE KEY LOGGING"
     AND d.always="CONDITIONAL"
     AND d.generated="GENERATED NAME")
     dmcs_info_idx = locateval(dmcs_info_idx,1,dmcs_cleanup->cnt,"UK_C_G",dmcs_cleanup->qual[
      dmcs_info_idx].info_char,
      trim(d.table_name),dmcs_cleanup->qual[dmcs_info_idx].table_name,trim(d.owner),dmcs_cleanup->
      qual[dmcs_info_idx].owner)
     IF (dmcs_info_idx > 0)
      ...
      dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order = 2
     ENDIF
    ELSEIF (d.log_group_type="FOREIGN KEY LOGGING"
     AND d.always="CONDITIONAL"
     AND d.generated="GENERATED NAME")
     dmcs_info_idx = locateval(dmcs_info_idx,1,dmcs_cleanup->cnt,"FK_C_G",dmcs_cleanup->qual[
      dmcs_info_idx].info_char,
      trim(d.table_name),dmcs_cleanup->qual[dmcs_info_idx].table_name,trim(d.owner),dmcs_cleanup->
      qual[dmcs_info_idx].owner)
     IF (dmcs_info_idx > 0)
 ...
 IF (dmcr_cleanup->confirm_ind)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Update DM_INFO rows for Supplemental log groups."
 CALL disp_msg("",dm_err->logfile,0)
 UPDATE  FROM dm_info d
  SET d.info_number = 2
  WHERE d.info_domain IN ("DM2_SUPP_LOG_ADD_TRANDATA", "DM2_SUPP_LOG_ADD_TRANDATA_DELTA")
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 GO TO exit_script
#exit_script
 SET dm_err->eproc = "Ending dm2_mig_clean_supplog."
 CALL final_disp_msg("dm2_mig_clean_suplog")
END GO
