CREATE PROGRAM dm2_arc_backfill:dba
 ...
 SET readme_data->message = "Backfill last_accessed_dt_tm columns"
 EXECUTE dm_readme_status
 IF (check_logfile("dm2_arc_backfill",".log","DM2_ARC_BACKFILL LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_program
 ENDIF
 DECLARE dab_count = i4 WITH noconstant(0)
 DECLARE dab_range = i4 WITH constant(3000)
 DECLARE dab_min_value = f8 WITH noconstant(0.0)
 DECLARE dab_max_value = f8 WITH noconstant(0.0)
 FREE RECORD dab_person
 RECORD dab_person(
   1 list[dab_range]
     2 person_id = f8
     2 max_date = dq8
 ...
 SELECT INTO "nl:"
  pmin = min(p.person_id), pmax = max(p.person_id)
  FROM person p
  WHERE p.ft_entity_id=0
   AND ((p.last_accessed_dt_tm = null) OR (p.last_accessed_dt_tm > cnvtdatetime(sysdate)))
  DETAIL
   dab_min_value = pmin, dab_max_value = pmax
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  GO TO exit_program
 ENDIF
 WHILE (dab_min_value <= dab_max_value)
   SET dab_count = 0
   SELECT INTO "nl:"
    p.person_id
    FROM person p
    WHERE p.person_id BETWEEN dab_min_value AND (dab_min_value+ dab_range)
     AND p.ft_entity_id=0
     AND ((p.last_accessed_dt_tm = null) OR (p.last_accessed_dt_tm > cnvtdatetime(sysdate)))
    DETAIL
     dab_count += 1, dab_person->list[dab_count].person_id = p.person_id, dab_person->list[dab_count]
     .updt_dt_tm = least(p.updt_dt_tm,cnvtdatetime(sysdate))
    WITH nocounter
   ;end select
   SET dab_min_value += dab_range
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET readme_data->status = "F"
    SET readme_data->message = dm_err->emsg
   ...
   IF (dab_count > 0)
    SELECT INTO "nl:"
     reg = max(e.reg_dt_tm), disch = max(e.disch_dt_tm)
     FROM encounter e,
      (dummyt d  WITH seq = dab_count)
     PLAN (d)
      JOIN (e
      WHERE (e.person_id=dab_person->list[d.seq].person_id)
       AND e.reg_dt_tm < cnvtdatetime(sysdate))
     DETAIL
      dab_person->list[d.seq].max_date = cnvtdatetime(greatest(reg,disch))
      IF ((dab_person->list[d.seq].max_date > cnvtdatetime(sysdate)))
       dab_person->list[d.seq].max_date = cnvtdatetime(0,0)
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc) != 0)
    ...
    UPDATE  FROM person p,
      (dummyt d  WITH seq = dab_count)
     SET p.seq = 1, p.last_accessed_dt_tm = evaluate(cnvtdatetime(dab_person->list[d.seq].max_date),
       cnvtdatetime(0,0),cnvtdatetime(dab_person->list[d.seq].updt_dt_tm),cnvtdatetime(dab_person->
        list[d.seq].max_date))
     PLAN (d)
      JOIN (p
      WHERE (p.person_id=dab_person->list[d.seq].person_id))
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc) != 0)
     ROLLBACK
 ...
 SET readme_data->message = "The tables were updated successfully"
#exit_program
 EXECUTE dm_readme_status
 CALL echorecord(readme_data)
 SET dm_err->eproc = "...Ending dm2_arc_backfill"
 CALL final_disp_msg("dm2_arc_backfill")
END GO
