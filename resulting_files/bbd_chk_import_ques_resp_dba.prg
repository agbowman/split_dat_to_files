CREATE PROGRAM bbd_chk_import_ques_resp:dba
 ...
 SET sreadme_name = "bbd_chk_ques_resp"
 DECLARE failed_ind = i2
 SET failed_ind = 0
 DECLARE response_count = i2
 SET response_count = 0
 DECLARE question_code_set = i4
 SET question_code_set = 1661
 DECLARE response_code_set = i4
 SET response_code_set = 1659
 DECLARE ssn_cdf = c12
 SET ssn_cdf = "SSN"
 DECLARE donor_name_cdf = c12
 SET donor_name_cdf = "DNRNAME"
 DECLARE donor_number_cdf = c12
 SET donor_number_cdf = "DNRNBR"
 DECLARE yes_cdf = c12
 SET yes_cdf = "Y"
 DECLARE no_cdf = c12
 SET no_cdf = "N"
 DECLARE patient_name_cdf = c12
 SET patient_name_cdf = "PAT NAME"
 DECLARE mrn_cdf = c12
 SET mrn_cdf = "MED NBR"
 DECLARE dispose_cdf = c12
 SET dispose_cdf = "DISPOSE"
 DECLARE quarantine_cdf = c12
 SET quarantine_cdf = "QUAR"
 DECLARE ssn_cd = f8
 SET ssn_cd = 0.0
 DECLARE donor_name_cd = f8
 SET donor_name_cd = 0.0
 DECLARE donor_number_cd = f8
 SET donor_number_cd = 0.0
 DECLARE yes_cd = f8
 SET yes_cd = 0.0
 DECLARE no_cd = f8
 SET no_cd = 0.0
 DECLARE patient_name_cd = f8
 SET patient_name_cd = 0.0
 DECLARE mrn_cd = f8
 SET mrn_cd = 0.0
 DECLARE dispose_cd = f8
 SET dispose_cd = 0.0
 DECLARE quarantine_cd = f8
 SET quarantine_cd = 0.0
 ...
 DECLARE dispose_quar_cdf = c12
 SET dispose_quar_cdf = "DISPQUARPROD"
 ...
 DECLARE dispose_quar_cd = f8
 SET dispose_quar_cd = 0.0
...
#start_script
 ...
 SELECT INTO "nl:"
  ...
  FROM code_value c
  PLAN (c
   WHERE c.code_set=response_code_set
    AND c.cdf_meaning IN (ssn_cdf, donor_name_cdf, donor_number_cdf, yes_cdf, no_cdf,
   patient_name_cdf, mrn_cdf, dispose_cdf, quarantine_cdf)
    AND c.active_ind=1)
  DETAIL
   CASE (c.cdf_meaning)
    OF ssn_cdf:
     ssn_cd = c.code_value
    OF donor_name_cdf:
     donor_name_cd = c.code_value
    OF donor_number_cdf:
     donor_number_cd = c.code_value
    OF yes_cdf:
     yes_cd = c.code_value
    OF no_cdf:
     no_cd = c.code_value
    OF patient_name_cdf:
     patient_name_cd = c.code_value
    OF mrn_cdf:
     mrn_cd = c.code_value
    OF dispose_cdf:
     dispose_cd = c.code_value
    OF quarantine_cdf:
     quarantine_cd = c.code_value
   ENDCASE
  WITH nocounter
 ;end select
 IF (((ssn_cd=0.0) OR (((donor_name_cd=0.0) OR (((donor_number_cd=0.0) OR (((yes_cd=0.0) OR (((no_cd=
 0.0) OR (((patient_name_cd=0.0) OR (((mrn_cd=0.0) OR (((dispose_cd=0.0) OR (quarantine_cd=0.0)) ))
 )) )) )) )) )) )) )
  SET failed_ind = 1
  SET msg = "Missing required code value from 1659."
 ...
 SELECT INTO "nl:"
  ...
  PLAN (c
   WHERE c.code_set=question_code_set
    AND c.cdf_meaning IN (donor_prompt_cdf, patient_prompt_cdf, default_measure_cdf, dispose_quar_cdf,
   donor_race_cdf,
 ...
 dispose_quar_cd=0.0) OR (((donor_race_cd=0.0) OR (((don_results_quar_cd=0.0) OR (((
 recruitment_days_cd=0.0) OR (interview_cd=0.0)) )) )) )) )) )) )) )
  SET failed_ind = 1
 ...
 SET response_count = 0
 SELECT INTO "nl:"
  vr.question_cd, vr.response_cd
  FROM valid_response vr
  PLAN (vr
   WHERE vr.question_cd=donor_prompt_cd
    AND vr.response_cd IN (ssn_cd, donor_name_cd, donor_number_cd))
  DETAIL
 ...
 IF (response_count != 3)
  SET failed_ind = 1
  SET msg = concat("Missing values from the VALID_RESPONSE table --> ",donor_prompt_cdf)
  CALL logmsg(msg,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  GO TO exit_script
 ENDIF
 SET response_count = 0
 SELECT INTO "nl:"
  ...
  FROM valid_response vr
  PLAN (vr
   WHERE vr.question_cd=patient_prompt_cd
    AND vr.response_cd IN (patient_name_cd, mrn_cd, ssn_cd))
  DETAIL
   response_count = (response_count+ 1)
  WITH nocounter
 ;end select
 IF (response_count != 3)
  SET failed_ind = 1
  SET msg = concat("Missing values from the VALID_RESPONSE table --> ",patient_prompt_cdf)
  CALL logmsg(msg,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  GO TO exit_script
 ENDIF
 SET response_count = 0
 SELECT INTO "nl:"
  ...
  FROM valid_response vr
  PLAN (vr
   WHERE vr.question_cd=default_measure_cd
    AND vr.response_cd=0.0)
  DETAIL
   response_count = (response_count+ 1)
  WITH nocounter
 ;end select
 IF (response_count != 1)
  SET failed_ind = 1
  SET msg = concat("Missing values from the VALID_RESPONSE table --> ",default_measure_cdf)
  CALL logmsg(msg,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  GO TO exit_script
 ENDIF
 SET response_count = 0
 SELECT INTO "nl:"
  ...
  FROM valid_response vr
  PLAN (vr
   WHERE vr.question_cd=dispose_quar_cd
    AND vr.response_cd IN (dispose_cd, quarantine_cd))
  DETAIL
   response_count = (response_count+ 1)
  WITH nocounter
 ;end select
 IF (response_count != 2)
  SET failed_ind = 1
  SET msg = concat("Missing values from the VALID_RESPONSE table --> ",dispose_quar_cdf)
  CALL logmsg(msg,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  GO TO exit_script
 ENDIF
 SET response_count = 0
 SELECT INTO "nl:"
  ...
  FROM valid_response vr
  PLAN (vr
   WHERE vr.question_cd=donor_race_cd
    AND vr.response_cd IN (yes_cd, no_cd))
  DETAIL
   response_count = (response_count+ 1)
  WITH nocounter
 ;end select
 IF (response_count != 2)
  SET failed_ind = 1
  SET msg = concat("Missing values from the VALID_RESPONSE table --> ",donor_race_cdf)
  CALL logmsg(msg,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  GO TO exit_script
 ENDIF
 SET response_count = 0
 SELECT INTO "nl:"
  ...
  FROM valid_response vr
  PLAN (vr
   WHERE vr.question_cd=don_results_quar_cd
    AND vr.response_cd=0.0)
  DETAIL
   response_count = (response_count+ 1)
  WITH nocounter
 ...
 IF (response_count != 1)
  SET failed_ind = 1
  SET msg = concat("Missing values from the VALID_RESPONSE table --> ",don_results_quar_cdf)
  CALL logmsg(msg,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  GO TO exit_script
 ENDIF
 SET response_count = 0
 SELECT INTO "nl:"
  ...
  FROM valid_response vr
  PLAN (vr
   WHERE vr.question_cd=recruitment_days_cd
    AND vr.response_cd=0.0)
  DETAIL
   response_count = (response_count+ 1)
  WITH nocounter
 ...
 IF (response_count != 1)
  SET failed_ind = 1
  SET msg = concat("Missing values from the VALID_RESPONSE table --> ",recruitment_days_cdf)
  CALL logmsg(msg,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  GO TO exit_script
 ENDIF
 SET response_count = 0
 SELECT INTO "nl:"
  ...
  FROM valid_response vr
  PLAN (vr
   WHERE vr.question_cd=interview_cd
    AND vr.response_cd IN (yes_cd, no_cd))
  DETAIL
   response_count = (response_count+ 1)
  WITH nocounter
 ...
 IF (response_count != 2)
  SET failed_ind = 1
  SET msg = concat("Missing values from the VALID_RESPONSE table --> ",interview_cdf)
  CALL logmsg(msg,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  GO TO exit_script
 ENDIF
#exit_script
 IF (failed_ind=1)
  ROLLBACK
  CALL logstatus("Failed to insert all response rows successfully.","F")
  CALL logmsg("Failed to insert all response rows successfully.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
 ELSE
  CALL logstatus("Inserted all response rows successfully.","S")
  CALL logmsg("Inserted all response rows successfully.",((e_rdmstatus+ e_dispmsg)+ e_logtofile))
 ENDIF
