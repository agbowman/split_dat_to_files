CREATE PROGRAM bbt_get_code_set_by_mean:dba
 RECORD reply(
   1 qual[*]
     2 code_value = f8
     2 cdf_meaning = c12
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE (cv.code_set=request->code_set)
    AND ((trim(request->cdf_meaning) <= chk_no_cdf_meaning) OR (trim(request->cdf_meaning) >
   chk_no_cdf_meaning
 ...
 IF (curqual=0)
  ...
  SET reply->status_data.subeventstatus[count1].targetobjectname = "bbt_get_code_set_by_mean"
