CREATE PROGRAM bhs_athn_allpatient_ls
...
")
  ...
  SELECT INTO "nl:"
   ...
   PLAN (ed
    ...
    WHERE e.active_ind=1
     AND parser(where_enc_type_params)
 ...
 IF (((cv_nu != 0) OR (cv_encnt_id != 0)) )
  ...
  SELECT INTO  $1
   ...
   HEAD e.encntr_id
    ...
    e_type_class_cd = build("<EncounterTypeClassId>",e_encntr_type_class_cd,"</EncounterTypeClassId>"
     ), col + 1, e_type_class_cd,
    row + 1, e_type_class_disp = build("<EncounterTypeClassDisp>",e_encntr_type_class_disp,
     "</EncounterTypeClassDisp>"), col + 1,
    e_type_class_disp, row + 1, e_type_class_mean = build("<EncounterTypeClassMean>",
     e_encntr_type_class_mean,"</EncounterTypeClassMean>"),
    col + 1, e_type_class_mean, row + 1,
 ...
 ELSEIF (cv_nu=0)
  SELECT INTO  $1
   ...
   e_encntr_type_class_mean = uar_get_code_meaning(e.encntr_type_class_cd)
   FROM encounter e,
    person p,
    person_alias pa,
    encntr_alias ea,
   ...
   HEAD e.encntr_id
    ...
    p_patienttype, row + 1, e_type_class_cd = build("<EncounterTypeClassId>",e_encntr_type_class_cd,
     "</EncounterTypeClassId>"),
    col + 1, e_type_class_cd, row + 1,
    e_type_class_disp = build("<EncounterTypeClassDisp>",e_encntr_type_class_disp,
     "</EncounterTypeClassDisp>"), col + 1, e_type_class_disp,
    row + 1, e_type_class_mean = build("<EncounterTypeClassMean>",e_encntr_type_class_mean,
     "</EncounterTypeClassMean>"), col + 1,
    e_type_class_mean, row + 1, ph_home = fillstring(200," "),
