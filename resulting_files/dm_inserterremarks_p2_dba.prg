CREATE PROGRAM dm_inserterremarks_p2:dba
 ...
 DECLARE errmsg = vc WITH protect, noconstant("")
 DECLARE f_cs18736_ar = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs20849_patient = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs18735_open = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs18935_canceled = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs20549_exp_reim_adj = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs24730_estimatedreimbursement_cd = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs29904_infoonly_cd = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs48_active_status_cd = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs18937_reversal_cd = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs25753_reversal_cd = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs20569_posted_cd = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs24730_denial_res_cd = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs29904_denial_type_cd = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs26913_denial_rrmark_cd = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs26913_denial_rrmark_alias_cd = f8 WITH protect, noconstant(0.0)
 DECLARE fsystemid = f8 WITH protect, noconstant(0.0)
 DECLARE f_cs18649_adjust_cd = f8 WITH protect, noconstant(0.0)
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="ADJUST"
   AND c.code_set=18649
   AND c.active_ind=1
  DETAIL
   f_cs18649_adjust_cd = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs18649_adjust_cd=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve Adjustment code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="DOLLARAMT"
   AND c.code_set=26913
   AND c.active_ind=1
  DETAIL
   f_cs26913_denial_rrmark_cd = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs26913_denial_rrmark_cd=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve Dollar Amount code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="ALIAS"
   AND c.code_set=26913
   AND c.active_ind=1
  DETAIL
   f_cs26913_denial_rrmark_alias_cd = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs26913_denial_rrmark_alias_cd=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve Alias code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="ER"
   AND c.code_set=24730
   AND c.active_ind=1
  DETAIL
   f_cs24730_denial_res_cd = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs24730_denial_res_cd=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve ER code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="INFOONLY"
   AND c.code_set=29904
   AND c.active_ind=1
  DETAIL
   f_cs29904_denial_type_cd = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs29904_denial_type_cd=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve Info only code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="A/R"
   AND c.code_set=18736
   AND c.active_ind=1
  DETAIL
   f_cs18736_ar = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs18736_ar=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve A/R code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="PATIENT"
   AND c.code_set=20849
   AND c.active_ind=1
  DETAIL
   f_cs20849_patient = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs20849_patient=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve patient code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="OPEN"
   AND c.code_set=18735
   AND c.active_ind=1
  DETAIL
   f_cs18735_open = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs18735_open=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve open code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="CANCELED"
   AND c.code_set=18935
   AND c.active_ind=1
  DETAIL
   f_cs18935_canceled = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs18935_canceled=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve Canceled code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="EXP REIM ADJ"
   AND c.code_set=20549
   AND c.active_ind=1
  DETAIL
   f_cs20549_exp_reim_adj = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs20549_exp_reim_adj=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve EXP REIM ADJ code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM prsnl p
  WHERE ((p.name_first_key="SYSTEM") OR (p.name_last_key="SYSTEM"
   AND p.active_ind=1))
  DETAIL
   fsystemid = p.person_id
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (fsystemid=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve system id : ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="POSTED"
   AND c.code_set=20569
   AND c.active_ind=1
  DETAIL
   f_cs20569_posted_cd = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs20569_posted_cd=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve posted code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="ACTIVE"
   ...
   AND c.active_ind=1
  DETAIL
   f_cs48_active_status_cd = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs48_active_status_cd=0.0)) )
  SET readme_data->status = "F"
 ...
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="REVERSAL"
   AND c.code_set=18937
   AND c.active_ind=1
  DETAIL
   f_cs18937_reversal_cd = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs18937_reversal_cd=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve reversal code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.cdf_meaning="REVERSAL"
   AND c.code_set=25753
   AND c.active_ind=1
  DETAIL
   f_cs25753_reversal_cd = c.code_value
  WITH nocounter
 ;end select
 IF (((error(errmsg,0) != 0) OR (f_cs25753_reversal_cd=0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to retrieve reversal 25753 code value: ",errmsg)
  GO TO exit_program
 ENDIF
 SET ms_info_domain_nm = "DM_INSERTERREMARKS_README"
 SET ms_range = "MAX acct_id RANGE 6 CAT 5"
 SET ms_max_name_p = "MAX acct_id EVALUATED"
 SET ms_child_script = "dm_insertERremarks_cc"
 SET mn_num_children = 5
 SELECT INTO "nl:"
  max_val = max(e.encntr_id)
  FROM encounter e
  PLAN (e
   WHERE e.encntr_id > 0.0
    AND e.reg_dt_tm >= cnvtdatetime("01-JAN-2018")
    AND e.reg_dt_tm <= cnvtdatetime(sysdate)
    AND e.active_ind=1)
  DETAIL
