CREATE PROGRAM bsc_audit_detail_prompt_inst:dba
 ...
 SET readme_data->message = "Readme failure: starting bsc_audit_detail_prompt_inst.prg"
 SET strdir = "cer_install"
 CALL importform("bsc_earlylate_audit_detail.dat")
 CALL importform("bsc_oudose_audit_detail.dat")
 CALL importform("bsc_patmm_audit_detail.dat")
 CALL importform("bsc_scan_compliance.dat")
 IF (errorcount=0)
 ...
 SUBROUTINE importform(strimport)
   ...
   IF (findfile(nullterm(strimportname))=0)
    ...
    SET readme_data->message = concat("%CCL-F-BSC_AUDIT_DETAIL_PROMPT_INST FILE NOT FOUND '",trim(
      strimportname,3),"'")
   ELSE
