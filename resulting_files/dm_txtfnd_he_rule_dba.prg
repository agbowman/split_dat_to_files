CREATE PROGRAM dm_txtfnd_he_rule:dba
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 DECLARE dthr_idx = i4 WITH protect, noconstant(0)
 DECLARE dthr_num = i4 WITH protect, noconstant(0)
 DECLARE dthr_new_ind = i2 WITH protect, noconstant(0)
 DECLARE dthr_rule_pos = i4 WITH protect, noconstant(0)
 DECLARE dthr_det_name = vc WITH protect, noconstant("")
 DECLARE dthr_flag = i4 WITH protect, noconstant(0)
 DECLARE dthr_error_ind = i2 WITH protect, noconstant(0)
 DECLARE dthr_error_msg = vc WITH protect, noconstant("")
 DECLARE dthr_long_ind = i2 WITH protect, noconstant(0)
 DECLARE dthr_pe_name = vc WITH protect, noconstant("")
 DECLARE dthr_pe_id = f8 WITH protect, noconstant(0.0)
 DECLARE dthr_data_source = vc WITH protect, noconstant("")
 FREE RECORD dthr_temp_rec
 RECORD dthr_temp_rec(
   1 cnt = i4
   1 qual[*]
     2 long_text_id = f8
     2 active_display = vc
     2 rule_group_display = vc
     2 rule_group_cdf = vc
     2 active_cdf = vc
     2 updt_dt_tm = dq8
     2 active_ind = i2
 ...
 FREE RECORD dthr_dtfd
 RECORD dthr_dtfd(
   1 cnt = i4
   1 qual[*]
     2 dm_text_find_data_id = f8
     2 parent_entity_id = f8
     2 parent_entity_name = vc
     2 data_source = vc
     2 compile_dt_tm = dq8
 ...
 FREE RECORD dthr_prog
 RECORD dthr_prog(
   1 cnt = i4
   1 qual[*]
     2 line_data = vc
 )
 IF (check_logfile("dm_tf_he_rule",".log","DM_TXTFND_HE_RULE LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_TXTFND_HE_RULE..."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((validate(dtla_detail_id,- (123.0))=- (123.0)))
  IF ((script_rec->dm_text_find_detail_id=0.0))
   DECLARE dtla_detail_id = f8 WITH protect, noconstant(0.0)
   SET dtla_detail_id = 0.0
  ELSE
   DECLARE dtla_detail_id = f8 WITH protect, noconstant(0.0)
 ...
 IF ((script_rec->dm_text_find_log_id=0.0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No DM_TEXT_FIND_LOG_ID provided. Unable to proceed."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dtla_detail_id=0.0)
  SET dm_err->eproc = "Querying for DM_TEXT_FIND_DETAIL_ID."
  SELECT INTO "nl:"
   FROM dm_text_find_detail d
   WHERE d.detail_type_flag=2
    AND d.active_ind=1
    AND d.detail_script_name="DM_TXTFND_HE_RULE"
   DETAIL
    dtla_detail_id = d.dm_text_find_detail_id
   WITH nocounter
  ;end select
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "No active data found in DM_TEXT_FIND_DETAIL for script DM_TXTFND_HE_RULE."
   IF ((script_rec->dm_text_find_log_id > 0.0))
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
   ENDIF
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "NL:"
  ...
  WHERE u.table_name="LONG_TEXT_REFERENCE"
   AND u.column_name="LONG_TEXT"
 ...
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Gathering LONG_TEXT data_type:",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 SET script_rec->dm_text_find_detail_id = dtla_detail_id
 SET dm_err->eproc = "Gathering Health Expert FKs to LONG_TEXT_REFERENCE"
 SELECT INTO "nl:"
  FROM he_rule_group h,
   code_value c,
   code_value c2
  WHERE h.rule_text_id > 0.0
   AND c.code_set=48
   AND c.cdf_meaning IN ("ACTIVE", "DRAFT")
   AND c.code_value=h.active_status_cd
   AND c2.code_set=4002671
   AND c2.code_value=h.rule_group_cd
   AND  EXISTS (
  (SELECT
   "x"
   FROM long_text_reference l
   WHERE l.long_text_id=h.rule_text_id))
  DETAIL
   dthr_temp_rec->cnt = (dthr_temp_rec->cnt+ 1), stat = alterlist(dthr_temp_rec->qual,dthr_temp_rec->
    cnt), dthr_temp_rec->qual[dthr_temp_rec->cnt].long_text_id = h.rule_text_id,
   dthr_temp_rec->qual[dthr_temp_rec->cnt].active_display = c.display, dthr_temp_rec->qual[
   dthr_temp_rec->cnt].active_cdf = c.cdf_meaning, dthr_temp_rec->qual[dthr_temp_rec->cnt].
   rule_group_display = c2.display,
   dthr_temp_rec->qual[dthr_temp_rec->cnt].rule_group_cdf = c2.cdf_meaning, dthr_temp_rec->qual[
   dthr_temp_rec->cnt].active_ind = h.active_ind, dthr_temp_rec->qual[dthr_temp_rec->cnt].updt_dt_tm
    = cnvtdatetime(h.updt_dt_tm)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for HE_RULE_GROUP LONG_TEXT_REFERENCE FKs:",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 FOR (dthr_idx = 1 TO dthr_temp_rec->cnt)
   IF (dthr_long_ind=1)
    INSERT  FROM dm_clob_gttd d
     (d.dm_clob_id, d.dm_clob, d.dm_clob_name)(SELECT
      l.long_text_id, sqlpassthru("to_lob(l.long_text)"), ""
      FROM long_text_reference l
      WHERE (l.long_text_id=dthr_temp_rec->qual[dthr_idx].long_text_id))
     WITH nocounter
    ;end insert
   ELSE
    INSERT  FROM dm_clob_gttd d
   ...
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
      "Moving JSON rule over to DM_CLOB_GTTD:",dm_err->emsg),0.0,0.0)
    GO TO exit_script
   ENDIF
   CALL parser(concat("rdb asis(^begin dm_json_package.dm_parse_he_rule_json(",trim(cnvtstring(
       dthr_temp_rec->qual[dthr_idx].long_text_id,20,2)),"); end;^) go"),1)
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Parsing JSON object apart:",
      dm_err->emsg),0.0,0.0)
    GO TO exit_script
   ENDIF
   SELECT INTO "NL:"
    bloblen = textlen(d.dm_clob)
    FROM dm_clob_gttd d
    ORDER BY d.dm_clob_id
    HEAD REPORT
     outbuf = fillstring(32767," ")
    DETAIL
     offset = 0, retlen = 0, dthr_temp_rec->qual[dthr_idx].rule_cnt = (dthr_temp_rec->qual[dthr_idx].
     rule_cnt+ 1),
     stat = alterlist(dthr_temp_rec->qual[dthr_idx].rule_qual,dthr_temp_rec->qual[dthr_idx].rule_cnt),
     dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_temp_rec->qual[dthr_idx].rule_cnt].rule_name = d
     .dm_clob_name, retlen = blobget(outbuf,offset,d.dm_clob),
     dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_temp_rec->qual[dthr_idx].rule_cnt].rule_data = " "
     WHILE (retlen > 0)
       IF ((dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_temp_rec->qual[dthr_idx].rule_cnt].rule_data
       =" "))
        dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_temp_rec->qual[dthr_idx].rule_cnt].rule_data =
        notrim(outbuf)
       ELSE
        dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_temp_rec->qual[dthr_idx].rule_cnt].rule_data =
        notrim(concat(notrim(dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_temp_rec->qual[dthr_idx].
           rule_cnt].rule_data),notrim(substring(1,retlen,outbuf))))
       ENDIF
       offset = (offset+ retlen), retlen = blobget(outbuf,offset,d.dm_clob)
     ENDWHILE
     dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_temp_rec->qual[dthr_idx].rule_cnt].rule_data = trim
     (dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_temp_rec->qual[dthr_idx].rule_cnt].rule_data),
     dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_temp_rec->qual[dthr_idx].rule_cnt].data_source =
     concat(dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_temp_rec->qual[dthr_idx].rule_cnt].rule_name,
      ":",dthr_temp_rec->qual[dthr_idx].active_cdf,":",dthr_temp_rec->qual[dthr_idx].rule_group_cdf)
    WITH nocounter, rdbarrayfetch = 1
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying individual rules:",
      dm_err->emsg),0.0,0.0)
    GO TO exit_script
   ENDIF
   ROLLBACK
 ...
 SELECT INTO "NL:"
  FROM dm_text_find_data d
  WHERE d.dm_text_find_detail_id=dtla_detail_id
   AND d.status_flag != 3
  DETAIL
   dthr_dtfd->cnt = (dthr_dtfd->cnt+ 1), stat = alterlist(dthr_dtfd->qual,dthr_dtfd->cnt), dthr_dtfd
   ->qual[dthr_dtfd->cnt].dm_text_find_data_id = d.dm_text_find_data_id,
   dthr_dtfd->qual[dthr_dtfd->cnt].parent_entity_id = d.parent_entity_id, dthr_dtfd->qual[dthr_dtfd->
   cnt].parent_entity_name = d.parent_entity_name, dthr_dtfd->qual[dthr_dtfd->cnt].data_source = d
   .data_source,
   dthr_dtfd->qual[dthr_dtfd->cnt].compile_dt_tm = cnvtdatetime(d.compile_dt_tm)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying DM_TEXT_FIND_DATA:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 FOR (dthr_idx = 1 TO dthr_temp_rec->cnt)
   FOR (dthr_num = 1 TO dthr_temp_rec->qual[dthr_idx].rule_cnt)
     IF (size(dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_num].data_source) > 255)
      SET dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_num].data_source = substring(1,255,
       dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_num].data_source)
     ENDIF
     SET dthr_new_ind = 0
     SET script_rec->long_cnt = 0
     SET script_rec->script_cnt = 0
     SET stat = alterlist(script_rec->long_qual,0)
     SET stat = alterlist(script_rec->script_qual,0)
     SET stat = initrec(dtg)
     SET stat = initrec(dthr_prog)
     SET dthr_rule_pos = locateval(dthr_rule_pos,1,dthr_dtfd->cnt,dthr_temp_rec->qual[dthr_idx].
      rule_qual[dthr_num].data_source,dthr_dtfd->qual[dthr_rule_pos].data_source)
     IF (dthr_rule_pos > 0)
      SET dthr_dtfd->qual[dthr_rule_pos].rule_match = 1
      IF (cnvtdatetime(dthr_temp_rec->qual[dthr_idx].updt_dt_tm) > cnvtdatetime(dthr_dtfd->qual[
       dthr_rule_pos].compile_dt_tm))
       SET dthr_new_ind = 1
       SET dthr_pe_name = dthr_dtfd->qual[dthr_rule_pos].parent_entity_name
       SET dthr_pe_id = dthr_dtfd->qual[dthr_rule_pos].parent_entity_id
       SET dthr_data_source = dthr_dtfd->qual[dthr_rule_pos].data_source
      ENDIF
     ELSE
      SET dthr_new_ind = 1
      SET dthr_pe_name = "LONG_TEXT_REFERENCE"
      SET dthr_pe_id = 0.0
      SET dthr_data_source = dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_num].data_source
     ENDIF
     IF (dthr_new_ind=1)
      SET dthr_new_ind = 1
      SET dtg->cnt = (dtg->cnt+ 1)
      SET stat = alterlist(dtg->qual,dtg->cnt)
      SET dtg->qual[dtg->cnt].pe_name = dthr_pe_name
      SET dtg->qual[dtg->cnt].pe_id = dthr_pe_id
      SET dtg->qual[dtg->cnt].pe_col = "LONG_TEXT_ID"
      SET dtg->qual[dtg->cnt].srch_col = "LONG_TEXT"
      SET dtg->qual[dtg->cnt].compile_dt_tm = cnvtdatetime(dthr_temp_rec->qual[dthr_idx].updt_dt_tm)
      SET dtg->qual[dtg->cnt].data_source = dthr_data_source
      SET dtg->qual[dtg->cnt].att_cnt = (dtg->qual[dtg->cnt].att_cnt+ 1)
      SET stat = alterlist(dtg->qual[dtg->cnt].att_qual,dtg->qual[dtg->cnt].att_cnt)
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].tab_name = "HE_RULE_GROUP"
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_name = "ACTIVE_STATUS_CD"
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = dthr_temp_rec->qual[
      dthr_idx].active_display
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_txt = "Rule Status: "
      SET dtg->qual[dtg->cnt].att_cnt = (dtg->qual[dtg->cnt].att_cnt+ 1)
      SET stat = alterlist(dtg->qual[dtg->cnt].att_qual,dtg->qual[dtg->cnt].att_cnt)
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].tab_name = "HE_RULE_GROUP"
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_name = "RULE_GROUP_CD"
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = dthr_temp_rec->qual[
      dthr_idx].rule_group_display
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_txt = "Rule Group: "
      SET dtg->qual[dtg->cnt].att_cnt = (dtg->qual[dtg->cnt].att_cnt+ 1)
      SET stat = alterlist(dtg->qual[dtg->cnt].att_qual,dtg->qual[dtg->cnt].att_cnt)
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].tab_name = "HE_RULE_GROUP"
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_name = "ACTIVE_IND"
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = cnvtstring(
       dthr_temp_rec->qual[dthr_idx].active_ind)
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_txt = "Active Indicator: "
      SET dtg->qual[dtg->cnt].att_cnt = (dtg->qual[dtg->cnt].att_cnt+ 1)
      SET stat = alterlist(dtg->qual[dtg->cnt].att_qual,dtg->qual[dtg->cnt].att_cnt)
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].tab_name = "HE_RULE_GROUP"
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_name = "UPDT_DT_TM"
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = format(cnvtdatetime(
        dthr_temp_rec->qual[dthr_idx].updt_dt_tm),"YYYY-MMM-DD HH:MM;;Q")
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_txt =
      "Last Change Date/TIme: "
      SET dtg->qual[dtg->cnt].att_cnt = (dtg->qual[dtg->cnt].att_cnt+ 1)
      SET stat = alterlist(dtg->qual[dtg->cnt].att_qual,dtg->qual[dtg->cnt].att_cnt)
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].tab_name = "LONG_TEXT_REFERENCE"
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_name = "LONG_TEXT"
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_val = dthr_temp_rec->qual[
      dthr_idx].rule_qual[dthr_num].rule_name
      SET dtg->qual[dtg->cnt].att_qual[dtg->qual[dtg->cnt].att_cnt].col_txt = "Rule Name: "
      IF ((dtg->qual[dtg->cnt].pe_id=0.0))
       SELECT INTO "NL:"
        y = seq(long_data_seq,nextval)
        FROM dual d
        DETAIL
         dtg->qual[dtg->cnt].pe_id = y
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc) != 0)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
          "Popping new LONG_DATA_SEQ value:",dm_err->emsg),0.0,0.0)
        GO TO exit_script
       ENDIF
      ENDIF
      CALL dst_transfer_data(script_rec,dtg)
      IF (check_error(dm_err->eproc) != 0)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dthr_error_ind = dm_err->err_ind
       SET dthr_error_msg = dm_err->emsg
       SET dm_err->err_ind = 0
      ENDIF
      SELECT INTO "NL:"
       FROM dm_text_find d,
        dm_text_find_detail f
       WHERE f.dm_text_find_detail_id=dtla_detail_id
        AND f.dm_text_find_detail_id > 0.0
        AND d.dm_text_find_id=f.dm_text_find_id
       DETAIL
        dthr_det_name = trim(d.find_name), dthr_flag = f.detail_type_flag
       WITH nocounter
      ;end select
      IF (check_error("Getting DM_TEXT_FIND information") != 0)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
         "Getting DM_TEXT_FIND info: ",dm_err->emsg),0.0,0.0)
       GO TO exit_script
      ENDIF
      IF (curqual=0)
       SET dm_err->err_ind = 1
       SET dm_err->emsg = "No data found for detail_id."
       CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_script
      ENDIF
      SET dthr_prog->cnt = 1
      SET stat = alterlist(dthr_prog->qual,1)
      SET dthr_prog->qual[1].line_data = dthr_temp_rec->qual[dthr_idx].rule_qual[dthr_num].rule_data
      CALL dst_insert_data(1,script_rec,dthr_det_name,dthr_prog,dthr_flag)
      IF (check_error("Calling DST_INSERT_DATA") != 0)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dm_err->err_ind = 0
       IF (dthr_error_ind=0)
        SET dthr_error_msg = dm_err->emsg
       ENDIF
       SET dthr_error_ind = 1
      ENDIF
     ENDIF
   ENDFOR
 ENDFOR
 FOR (dthr_rule_pos = 1 TO dthr_dtfd->cnt)
   IF ((dthr_dtfd->qual[dthr_rule_pos].rule_match=0))
    DELETE  FROM long_text_reference l
     WHERE (l.long_text_id=dthr_dtfd->qual[dthr_rule_pos].parent_entity_id)
     WITH nocounter
    ;end delete
    IF (check_error("Deleting non-existent rule information") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Deleting rule info: ",
       dm_err->emsg),0.0,0.0)
     ROLLBACK
     GO TO exit_script
    ELSE
 ...
 ENDFOR
#exit_script
 SET dm_err->eproc = "...Ending DM_TXTFND_HE_RULE"
 CALL final_disp_msg("dm_tf_he_rule")
 IF (dthr_error_ind=1
  AND (dm_err->err_ind=0))
  SET dm_err->err_ind = dthr_error_ind
  SET dm_err->emsg = dthr_error_msg
 ENDIF
END GO
