CREATE PROGRAM bhs_clairvia_clin_doc_br_stand:dba
 PROMPT
  "Printer" = "MINE",
  "FACILITY" = 0,
 ...
 SUBROUTINE (loadbrflexvalues(flex_parent_entity_id=f8) =null WITH protect)
 ...
 SUBROUTINE (getbedrockfilterindexbymeaning(filtermeaning=vc) =i4 WITH protect)
   ...
   RETURN(filter_index)
 END ;Subroutine
 SUBROUTINE (copytofiltervaluesrecord(fromrec=vc(ref),filter_index=i4) =null WITH protect)
   ...
   ENDFOR
 END ;Subroutine
 SUBROUTINE (copyfiltervaluesrecord(filter_index=i4,value_index=i4,torec=vc(ref)) =null WITH protect)
   SET torec->values_cnt += 1
 ...
 END ;Subroutine
 SUBROUTINE (getencounterfacilitycd(encntr_id=f8) =f8 WITH protect)
 ...
 SUBROUTINE loadbrflexflag(null)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (loadbrfilter(filter_index=i4) =null WITH protect)
   ...
   SET cntx = 0
 END ;Subroutine
 SUBROUTINE (loadbrvalues(filter_index=i4) =null WITH protect)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (getbestfilterbatchsize(querysize=i4,groupsize=i4) =i4 WITH protect)
   IF (querysize <= 1)
    RETURN(querysize)
   ELSEIF (querysize <= 5)
    RETURN(5)
 ...
 END ;Subroutine
 SUBROUTINE (getbrfilters(allind=i2,report_meaning=vc(value,""),cache_ind=i2(value,0),logdom_ind=i2(
   value,1)) =vc WITH protect)
    ...
    IF (allind=1)
     SELECT INTO "nl:"
      ...
      PLAN (b
       ...
       JOIN (bdf
       WHERE (bdf.br_datamart_flex_id= Outerjoin(bdv.br_datamart_flex_id)) )
       JOIN (n
 ...
 END ;Subroutine
 SUBROUTINE (loadbrcategorydetails(category_meaning=vc) =null WITH protect)
   SELECT INTO "nl:"
    FROM br_datamart_category b
    PLAN (b
 ...
 END ;Subroutine
 SUBROUTINE (buildbrquerycriteria(report_meaning=vc) =null WITH protect)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (getbrflexedfilters(allind=i2,report_meaning=vc(value,"")) =vc WITH protect)
   DECLARE cache_key = vc WITH protect
   DECLARE cache_val = vc WITH protect
   SET cache_key = buildbrcachekey(null)
   IF (textlen(trim(cache_key,3)) > 0)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (retrievebrfiltersbyflex(allind=i2,report_meaning=vc(value,"")) =vc WITH protect)
   DECLARE cntx = i4 WITH protect, noconstant(0)
    ...
    IF (filter_length > 0)
     ...
     SELECT INTO "nl:"
      ...
      HEAD REPORT
       cnt = 0
      DETAIL
       cnt += 1, br_cache_key->filters[cnt].filter_mean = filter_mean
      WITH nocounter
     ;end select
    ENDIF
 ...
 DECLARE begin_dt_tm = dq8 WITH noconstant(cnvtdatetime( $STARTDTTM)), protect
 ...
 SUBROUTINE parsedatepromptwithcurtime3(date_str)
   ...
   DECLARE _date = i4 WITH constant(_parsedate(date_str)), private
   IF (_date=0.0)
    SET _return_val = cnvtdatetime(sysdate)
   ELSE
...
#exit_script
 IF ((transaction->trans_qual_cnt > 0))
  ...
  IF (gl_bhs_prod_flag=1)
   SET ms_dclcom = concat("mv ",trim(cnvtlower(filewrite),3)," ",build(logical("bhscust"),
     "/ftp/bhs_clairvia_clin_doc_br_stand/"),trim(cnvtlower(filename),3))
  ELSE
   SET ms_dclcom = concat(
    "$cust_script/bhs_sftp_file.ksh ciscoreftp3@transfer.baystatehealth.org:/clairviabdrkt ",trim(
     cnvtlower(filewrite),3))
  ENDIF
  CALL echo(ms_dclcom)
  ...
  DETAIL
   ...
   IF ((transaction->trans_qual_cnt > 0))
    CALL print(build2("Extract file written to: ",trim(filewrite,3)," The file should contain ",trim(
      cnvtstringchk(transaction->trans_qual_cnt),3)," records.")), row + 1,
    CALL print(build2("SFTP status: ",ml_stat)),
    row + 1,
    CALL print(ms_dclcom)
   ELSE
