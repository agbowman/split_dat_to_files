CREATE PROGRAM disch_signed_ords_20080514
 PROMPT
 ...
 SET beg_dt_tm = cnvtdatetime(cnvtdate("02012005"),0)
 SET end_dt_tm = cnvtdatetime(cnvtdate("02282005"),235959)
 SET data->facility = uar_get_code_display(p_facility_cd)
 ...
 SELECT INTO "nl:"
  ...
  days_since_disch = datetimecmp(cnvtdatetime(curdate,curtime3),e.disch_dt_tm), last_loc =
  ...
  PLAN (orv
   ...
   WHERE e.encntr_id=o.encntr_id
    ...
    AND e.loc_facility_cd=p_facility_cd)
   JOIN (p
  ...
  HEAD orv.order_id
   ...
   qual[cnt].location = last_loc,
   data->qual[cnt].pt_name = substring(1,30,p.name_full_formatted), data->qual[cnt].birth_dt_tm = p
   .birth_dt_tm, data->qual[cnt].acct_nbr = substring(1,15,trim(ea1.alias,3)),
   data->qual[cnt].fmrn = substring(1,15,trim(ea2.alias,3)), data->qual[cnt].cmrn = substring(1,15,
    trim(pa.alias,3)), data->qual[cnt].clin_display = trim(o.clinical_display_line,3),
   data->qual[cnt].days_since_disch = days_since_disch, data->qual[cnt].cosign_phys = substring(1,30,
    pr.name_full_formatted), data->qual[cnt].cosign_dt_tm = orv.review_dt_tm
  FOOT REPORT
   IF (cnt > 0)
    stat = alterlist(data->qual,cnt), data->order_cnt = cnt
   ENDIF
  WITH nocounter, outerjoin = d1, outerjoin = d2,
   outerjoin = d3, nullreport
 ;end select
 SELECT INTO "nl:"
 ...
 SELECT INTO value(p_output)
  ...
  ORDER BY mrn
  HEAD REPORT
   yrow = 15, linestr = fillstring(120,"_"), xcol1 = 10,
   ...
   ENDMACRO
  HEAD PAGE
   "{f/8}{cpi/14}{lpi/8}", row + 1, signing_date = format(beg_dt_tm,"mm/dd/yyyy;;d"),
   CALL print(calcpos(xcol2,yrow)), "{b}ORDERS SIGNED ON {endb}", signing_date,
  ...
  HEAD mrn
   IF ((data->order_cnt > 0))
    ...
    CALL print(calcpos(xcol1,yrow)),
    data->qual[d1.seq].pt_name,
    CALL print(calcpos(xcol6,yrow)), data->qual[d1.seq].fmrn,
    CALL print(calcpos(xcol7,yrow)), data->qual[d1.seq].cmrn,
    CALL print(calcpos(xcol8,yrow)),
    data->qual[d1.seq].acct_nbr,
    CALL print(calcpos(xcol9,yrow)), data->qual[d1.seq].encntr_type,
  ...
  DETAIL
   IF ((data->order_cnt > 0))
    cosign_date_time = format(data->qual[d1.seq].cosign_dt_tm,"mm/dd/yyyy hh:mm:ss;;q"),
    CALL print(calcpos(x_col1,yrow)), "Cosigned by: ",
    data->qual[d1.seq].cosign_phys, " on ", cosign_date_time,
    rowplusone2,
    CALL print(calcpos(x_col1,yrow)), data->qual[d1.seq].order_mnem,
    CALL print(calcpos(x_col3,yrow)), data->qual[d1.seq].order_id,
    CALL print(calcpos(x_col4,yrow)),
    data->qual[d1.seq].ord_status
    ...
    CALL print(calcpos(xcol1,yrow)),
    "{f/8}{cpi/14}{lpi/8}", linestr, rowplusone
   ENDIF
  FOOT PAGE
