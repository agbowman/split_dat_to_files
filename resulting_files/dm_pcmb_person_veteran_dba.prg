CREATE PROGRAM dm_pcmb_person_veteran:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "PERSON_VETERAN"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_PERSON_VETERAN"
 ...
 SUBROUTINE (add_to(s_at_from_pk_id=f8,s_at_to_pk_id=f8) =i4)
   ...
   IF (size(dm_cmb_cust_cols->col,5)=0)
    SET dm_cmb_cust_cols->tbl_name = "PERSON_VETERAN"
    SET dm_cmb_cust_cols->sub_select_from_tbl = "PERSON_VETERAN"
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "PERSON_VETERAN"
   ...
   IF (v_at_hist_ind=1)
    IF (add_hist(s_at_to_pk_id)=0)
     ...
     SET request->error_message = substring(1,132,build(
       "Could not insert history for person_veteran with pk=",s_at_to_pk_id))
     RETURN(0)
    ENDIF
   ELSEIF (v_at_hist_ind=0)
    SET failed = insert_error
    SET request->error_message = substring(1,132,build(
      "Could not insert history for person_veteran with pk val=",s_at_to_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =i4)
   UPDATE  FROM person_veteran frm
   ...
   IF (add_hist(s_df_pk_id)=0)
    SET failed = insert_error
    SET request->error_message = substring(1,132,build(
      "Could not insert history for person_veteran with pk=",s_df_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (upt_to(s_ut_to_id=f8,s_ut_from_id=f8) =i4)
   ...
   DECLARE col_pers_cnt = i4 WITH protect, noconstant(0)
   ...
   SELECT INTO "NL:"
    ...
    FROM dtable t,
     dtableattr a,
     dtableattrl l
    WHERE t.table_name="PERSON_VETERAN"
    ...
    DETAIL
     CASE (l.attr_name)
      ...
      ELSE
       ...
       ENDIF
       ,col_pers_cnt += 1,
       IF (mod(col_pers_cnt,100)=1)
        stat = alterlist(pv_columns->pers_vet,(col_pers_cnt+ 99))
       ENDIF
       ,pv_columns->pers_vet[col_pers_cnt].column_name = l.attr_name,
       IF (l.type="F")
        pv_columns->pers_vet[col_pers_cnt].column_type = "F8"
       ELSEIF (l.type="I")
        pv_columns->pers_vet[col_pers_cnt].column_type = "I4"
       ELSEIF (l.type="C")
        IF (btest(l.stat,13))
         pv_columns->pers_vet[col_pers_cnt].column_type = "VC"
        ELSE
         pv_columns->pers_vet[col_pers_cnt].column_type = build(l.type,l.len)
        ENDIF
       ELSEIF (l.type="Q")
        pv_columns->pers_vet[col_pers_cnt].column_type = "DQ8"
       ENDIF
     ENDCASE
    FOOT REPORT
     stat = alterlist(pv_columns->pers_vet,col_pers_cnt)
    WITH nocounter
   ;end select
   SET updt_comb_det = " SELECT into 'nl:'"
   SET updt_comb_det = concat(updt_comb_det," FROM PERSON_VETERAN p")
   SET updt_comb_det = concat(updt_comb_det," WHERE p.person_id = s_ut_to_id")
   SET updt_comb_det = concat(updt_comb_det," DETAIL")
   FOR (col_cnt2 = 1 TO col_pers_cnt)
     IF ((pv_columns->pers_vet[col_cnt2].column_type IN ("C*", "VC*")))
      SET updt_comb_det = concat(updt_comb_det," pv_columns->pers_vet[",trim(cnvtstring(col_cnt2)),
       "]->to_value = p.",pv_columns->pers_vet[col_cnt2].column_name)
     ELSEIF ((pv_columns->pers_vet[col_cnt2].column_type="DQ8"))
      SET updt_comb_det = concat(updt_comb_det," pv_columns->pers_vet[",trim(cnvtstring(col_cnt2)),
       "]->to_value = format(p.",pv_columns->pers_vet[col_cnt2].column_name,
       ",';;Q')")
     ELSE
      SET updt_comb_det = concat(updt_comb_det," pv_columns->pers_vet[",trim(cnvtstring(col_cnt2)),
       "]->to_value = cnvtstring(p.",pv_columns->pers_vet[col_cnt2].column_name,
       ")")
     ENDIF
   ENDFOR
   SET updt_comb_det = concat(updt_comb_det," WITH NOCOUNTER, NOTRIM go")
   CALL parser(updt_comb_det,1)
   FOR (pcp_cols = 1 TO size(pv_columns->pers_vet,5))
    SET log_val_ind = cmb_save_column_value("PERSON_VETERAN",s_ut_to_id,pv_columns->pers_vet[pcp_cols
     ].column_name,pv_columns->pers_vet[pcp_cols].column_type,null,
     pv_columns->pers_vet[pcp_cols].to_value)
    IF (log_val_ind=0)
     SET failed = insert_error
     SET request->error_message = substring(1,132,build(
       "Could not insert column value to combine_det_value table for person_veteran with pk=",
       s_ut_to_id))
     RETURN(0)
    ENDIF
   ENDFOR
   ...
   ENDIF
   SET updt_stmt = concat("UPDATE into PERSON_VETERAN TU SET (",updt_col_to,updt_add_col_name_to,
    ") (SELECT ",updt_col_from,
    updt_add_col_val_from," FROM PERSON_VETERAN FRM WHERE FRM.person_id = s_ut_from_id )",
   ...
   IF (add_hist(s_ut_to_id)=0)
    SET failed = insert_error
    SET request->error_message = substring(1,132,build(
      "Could not insert history for person_veteran with pk=",s_ut_to_id))
    RETURN(0)
   ENDIF
   FREE SET pv_columns
   RETURN(1)
 ...
 SUBROUTINE (add_hist(s_at_to_parent_pk_id=f8) =i4)
   ...
   SET dm_cmb_cust_cols2->add_col_val[1].col_name = "PERSON_VETERAN_HIST_ID"
   ...
   IF (size(dm_cmb_cust_cols2->col,5)=0)
    SET dm_cmb_cust_cols2->tbl_name = "PERSON_VETERAN_HIST"
    SET dm_cmb_cust_cols2->sub_select_from_tbl = "PERSON_VETERAN"
 ...
 SUBROUTINE initialize_hist_id(null)
  IF (v_hist_init_ind=0)
   ...
   SET dcipht_request->transaction_reason_txt = "DM_PCMB_PERSON_VETERAN"
