CREATE PROGRAM cp_import_pw_data:dba
 ...
 SUBROUTINE (addnodes(idx=i4) =i4)
   ...
   SET request->cp_pathway_id = pw_rec->qual[idx].cp_pathway_id
   IF ((pw_rec->qual[idx].node_cnt > 0))
    SET stat = alterlist(request->node_list,pw_rec->qual[idx].node_cnt)
    FOR (j = 1 TO pw_rec->qual[idx].node_cnt)
      IF ((pw_rec->qual[idx].node_list[j].cp_node_id=0))
       SET reqnodecnt += 1
       SET request->node_list[reqnodecnt].concept_cd = pw_rec->qual[idx].node_list[j].concept_cd
       SET request->node_list[reqnodecnt].node_name = pw_rec->qual[idx].node_list[j].node_name
       SET request->node_list[reqnodecnt].intention_cd = pw_rec->qual[idx].node_list[j].intention_cd
       SET request->node_list[reqnodecnt].treatment_line_cd = pw_rec->qual[idx].node_list[j].
       treatment_line_cd
      ENDIF
    ENDFOR
    SET stat = alterlist(request->node_list,reqnodecnt)
    IF (reqnodecnt > 0)
     SET reqnodecnt = 0
     ...
     IF (size(reply->node_list,5) > 0)
      FOR (x = 1 TO pw_rec->qual[idx].node_cnt)
        IF ((pw_rec->qual[idx].node_list[x].cp_node_id=0))
         SET reqnodecnt += 1
         SET pw_rec->qual[idx].node_list[x].cp_node_id = reply->node_list[reqnodecnt].cp_node_id
        ENDIF
      ENDFOR
     ENDIF
    ENDIF
   ...
   IF ((pw_rec->qual[idx].node_list[nidx].comp_cnt > 0))
    ...
    FOR (j = 1 TO pw_rec->qual[idx].node_list[nidx].comp_cnt)
      IF ((pw_rec->qual[idx].node_list[nidx].comp_list[j].cp_component_id=0))
       SET reqcompcnt += 1
       SET pw_rec->qual[idx].node_list[nidx].comp_list[j].cp_node_id = pw_rec->qual[idx].node_list[
       nidx].cp_node_id
       SET request->component_list[reqcompcnt].cp_node_id = pw_rec->qual[idx].node_list[nidx].
       comp_list[j].cp_node_id
