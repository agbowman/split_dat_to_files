CREATE PROGRAM dm_get_unique_columns:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE check_override_tbl(null) = null
 DECLARE log_str = vc
 DECLARE all_tables = i2
 DECLARE v_where_str = vc WITH protect, noconstant(" ")
 DECLARE ccl_version = f8 WITH noconstant(8000.0)
 DECLARE table_dtl = vc
 DECLARE column_dtl = vc
 DECLARE tbl_cnt = i4
 DECLARE col_cnt = i4
 DECLARE pk_loop_cnt = i4 WITH noconstant(0), public
 DECLARE c_tbl_cnt = i4 WITH noconstant(0), public
 DECLARE final_cnt = i4
 DECLARE dguc_dcd_tname = vc
 DECLARE dguc_tbl_name_find = vc
 DECLARE tbl_num = i4 WITH protect
 DECLARE col_num = i4 WITH protect
 DECLARE tbl_pos = i4 WITH protect
 DECLARE col_pos = i4 WITH protect
 DECLARE v_start_point = i4 WITH protect, noconstant(0)
 FREE RECORD indexes
 RECORD indexes(
   1 data[*]
     2 name = vc
 )
 FREE RECORD dguc_reply2
 RECORD dguc_reply2(
   1 rs_tbl_cnt = i4
   1 dtd_hold[*]
     2 tbl_name = vc
 ...
 IF ((dguc_request->is_mrg_ind=1))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "dguc_request->is_mrg_ind is set to 1. This is not a supported setting."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dguc_reply->dguc_err_ind = 1
  SET dguc_reply->dguc_err_msg = dm_err->emsg
  GO TO exit_program
 ENDIF
 IF ((dguc_request->local_tables_ind=1))
  SET table_dtl = "DM_TABLES_DOC_LOCAL"
  SET column_dtl = "DM_COLUMNS_DOC_LOCAL"
 ELSE
  IF ((dguc_request->current_remote_db=0))
   SET table_dtl = "DM_RDDS_TBL_DOC"
   SET column_dtl = "DM_RDDS_COL_DOC"
  ELSE
   IF (trim(dguc_request->db_link,3)="")
    SET dm_err->err_ind = 1
    SET dm_err->emsg = build("database link is not popluated")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dguc_reply->dguc_err_ind = 1
    SET dguc_reply->dguc_err_msg = dm_err->emsg
    GO TO exit_program
 ...
 ENDIF
 IF (currdb="DB2UDB")
  IF ( NOT (findstring("*",dguc_request->what_tables)))
   SELECT INTO "nl:"
    FROM dm_tables_doc d
    WHERE (d.table_name=dguc_request->what_tables)
    DETAIL
     dguc_tbl_name_find = d.suffixed_table_name
    WITH nocounter
   ;end select
  ELSE
   SET dguc_tbl_name_find = dguc_request->what_tables
  ENDIF
 ELSE
  SET dguc_tbl_name_find = dguc_request->what_tables
 ENDIF
 IF ((dguc_request->only_special_ind=0)
  AND (dguc_request->is_mrg_ind=0)
  AND (dguc_request->is_ref_ind=0)
  AND (dguc_request->what_tables=char(42)))
  SET all_tables = 1
 ENDIF
 SET dm_err->eproc = "Getting list of tables to gather primary key information"
 CALL disp_msg("",dm_err->logfile,0)
 IF ((dguc_request->only_special_ind != 1))
  IF ((dguc_request->is_ref_ind=1))
   SET v_where_str = concat(v_where_str," (dtd.reference_ind = 1 or dtd.table_name IN (",
    "select rt.table_name from DM_RDDS_REFMRG_TABLES rt))")
  ELSE
   SET v_where_str = concat(v_where_str," 1 = 1")
  ENDIF
  IF (currdb="DB2")
   SET v_where_str = concat(v_where_str," and dtd.table_name = dtd.suffixed_table_name ")
  ENDIF
  IF (dguc_tbl_name_find != char(42))
   SET v_where_str = concat(v_where_str," and dtd.table_name = patstring(dguc_tbl_name_find)")
  ENDIF
 ENDIF
 SET dm_err->eproc = "Getting primary key info from tables with top-level primary key."
 CALL disp_msg("",dm_err->logfile,0)
 IF ((dguc_request->current_remote_db=1)
  AND (dguc_request->local_tables_ind=0))
  SELECT INTO "nl:"
   FROM user_tab_columns dt,
    dm_tables_doc dtd,
    user_constraints uc,
    user_cons_columns ucc
   WHERE parser(v_where_str)
    AND uc.table_name=dtd.table_name
    AND uc.constraint_type="P"
    AND uc.constraint_name=ucc.constraint_name
    AND uc.table_name=ucc.table_name
    AND dt.table_name=ucc.table_name
    AND dt.column_name=ucc.column_name
   ORDER BY dtd.table_name, ucc.position
   HEAD REPORT
    c_tbl_cnt = 0
   HEAD dtd.table_name
    c_tbl_cnt = (c_tbl_cnt+ 1)
    IF (mod(c_tbl_cnt,100)=1)
     stat = alterlist(dguc_reply2->dtd_hold,(c_tbl_cnt+ 99))
    ENDIF
    dguc_reply2->dtd_hold[c_tbl_cnt].tbl_name = trim(dtd.table_name), dguc_reply2->dtd_hold[c_tbl_cnt
    ].tbl_suffix = trim(dtd.table_suffix), col_cnt = 0
   DETAIL
    col_cnt = (col_cnt+ 1), stat = alterlist(dguc_reply2->dtd_hold[c_tbl_cnt].pk_hold,col_cnt),
    dguc_reply2->dtd_hold[c_tbl_cnt].pk_hold[col_cnt].ui_yes = 1,
    dguc_reply2->dtd_hold[c_tbl_cnt].pk_hold[col_cnt].pk_name = dt.column_name, dguc_reply2->
    dtd_hold[c_tbl_cnt].pk_hold[col_cnt].pk_datatype = dt.data_type
   FOOT  dtd.table_name
    dguc_reply2->dtd_hold[c_tbl_cnt].ui_found = 1
   WITH nocounter
  ;end select
  IF (check_error(" Gathering primary key information.") != 0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Error selecting to gather primary key information."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dguc_reply->dguc_err_ind = 1
   SET dguc_reply->dguc_err_msg = "Error selecting to gather primary key information."
   RETURN(1)
  ENDIF
  SET v_start_point = (c_tbl_cnt+ 1)
  SELECT INTO "nl:"
   ...
   (SELECT
    ...
    WHERE uc.table_name=dtd.table_name
     AND uc.constraint_type="P")))
   DETAIL
    c_tbl_cnt = (c_tbl_cnt+ 1)
    IF (mod(c_tbl_cnt,100)=1)
    ...
    ENDIF
    dguc_reply2->dtd_hold[c_tbl_cnt].tbl_name = trim(dtd.table_name), dguc_reply2->dtd_hold[c_tbl_cnt
    ].tbl_suffix = trim(dtd.table_suffix)
   FOOT REPORT
    stat = alterlist(dguc_reply2->dtd_hold,c_tbl_cnt)
   WITH nocounter
  ;end select
  SET dguc_reply2->rs_tbl_cnt = c_tbl_cnt
  SET dm_err->eproc = "Searching for tables that have unique indexes."
  CALL disp_msg("",dm_err->logfile,0)
  SET stat = alterlist(indexes->data,((dguc_reply2->rs_tbl_cnt - v_start_point)+ 1))
  SELECT INTO "nl:"
   ui.index_name
   FROM user_indexes ui,
    (dummyt d  WITH seq = dguc_reply2->rs_tbl_cnt)
   PLAN (d
    WHERE d.seq >= v_start_point)
    JOIN (ui
    WHERE (ui.table_name=dguc_reply2->dtd_hold[d.seq].tbl_name)
     AND ui.table_owner="V500"
     AND (( NOT (ui.table_name IN ("REF_TEXT", "ASSAY_RESOURCE_LOT"))
     AND ui.uniqueness="UNIQUE") OR (ui.table_name IN ("REF_TEXT", "ASSAY_RESOURCE_LOT")
     AND ui.index_name="XPK*")) )
   DETAIL
    indexes->data[((d.seq - v_start_point)+ 1)].name = ui.index_name
   WITH maxqual(ui,1), nocounter
  ;end select
  SELECT INTO "nl:"
   FROM user_tab_columns dt,
   ...
   PLAN (d
    WHERE d.seq >= v_start_point
     AND (indexes->data[((d.seq - v_start_point)+ 1)].name > " "))
    JOIN (uic
    WHERE (uic.table_name=dguc_reply2->dtd_hold[d.seq].tbl_name)
     AND (uic.index_name=indexes->data[((d.seq - v_start_point)+ 1)].name))
    JOIN (dt
    WHERE dt.table_name=uic.table_name
     AND dt.column_name=uic.column_name)
   ORDER BY uic.column_position
   HEAD REPORT
    col_cnt = 0
   DETAIL
    col_cnt = (col_cnt+ 1), stat = alterlist(dguc_reply2->dtd_hold[d.seq].pk_hold,col_cnt),
    dguc_reply2->dtd_hold[d.seq].pk_hold[col_cnt].pk_name = dt.column_name,
    dguc_reply2->dtd_hold[d.seq].pk_hold[col_cnt].pk_datatype = dt.data_type, dguc_reply2->dtd_hold[d
    .seq].pk_hold[col_cnt].ui_yes = 1, dguc_reply2->dtd_hold[d.seq].ui_found = 1
   WITH nocounter
  ;end select
  IF (check_error(" Gathering unique index information.") != 0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Error selecting to gather unique index information."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dguc_reply->dguc_err_ind = 1
   SET dguc_reply->dguc_err_msg = "Error selecting to gather unique index information."
   RETURN(1)
  ENDIF
  SET dm_err->eproc = "Searching for tables that have unique identifiers."
  CALL disp_msg("",dm_err->logfile,0)
  FOR (dguc_cnt = v_start_point TO dguc_reply2->rs_tbl_cnt)
    IF ((dguc_reply2->dtd_hold[dguc_cnt].ui_found=0))
     SET dguc_dcd_tname = "dm_rdds_col_doc"
     SELECT INTO "nl:"
      FROM dm_rdds_tbl_doc drtd,
       (parser(concat("dm_rdds_tbl_doc",dguc_request->db_link)) drtdr)
      PLAN (drtdr
       WHERE (drtdr.table_name=dguc_reply2->dtd_hold[dguc_cnt].tbl_name))
       JOIN (drtd
       WHERE drtdr.table_name=drtd.table_name)
      DETAIL
       IF (drtdr.override_instance < drtd.override_instance)
        dguc_dcd_tname = build("dm_rdds_col_doc",dguc_request->db_link)
       ENDIF
      WITH nocounter
     ;end select
     SELECT INTO "nl:"
      FROM user_tab_columns dt,
       (parser(dguc_dcd_tname) dcd)
      PLAN (dcd
       WHERE (dcd.table_name=dguc_reply2->dtd_hold[dguc_cnt].tbl_name)
        AND dcd.unique_ident_ind=1)
       JOIN (dt
       WHERE dt.table_name=dcd.table_name
        AND dt.column_name=dcd.column_name)
      ORDER BY dt.column_name
      HEAD REPORT
       col_cnt = 0
      DETAIL
       col_cnt = (col_cnt+ 1), stat = alterlist(dguc_reply2->dtd_hold[dguc_cnt].pk_hold,col_cnt),
       dguc_reply2->dtd_hold[dguc_cnt].pk_hold[col_cnt].ui_yes = 1,
       dguc_reply2->dtd_hold[dguc_cnt].pk_hold[col_cnt].pk_name = dt.column_name, dguc_reply2->
       dtd_hold[dguc_cnt].pk_hold[col_cnt].pk_datatype = dt.data_type, dguc_reply2->dtd_hold[dguc_cnt
       ].ui_found = 1
      WITH nocounter
     ;end select
     IF (check_error("Gathering unique identifier column information.") != 0)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Error gathering unique identifier column information."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dguc_reply->dguc_err_ind = 1
      SET dguc_reply->dguc_err_msg = "Error gathering unique identifier column information."
      RETURN(1)
     ENDIF
    ENDIF
  ENDFOR
  SET final_cnt = 0
  SET dm_err->eproc = "Moving to reply record structure."
  CALL disp_msg("",dm_err->logfile,0)
  SET stat = alterlist(dguc_reply->dtd_hold,0)
  SET stat = alterlist(dguc_reply2->dtd_hold,dguc_reply2->rs_tbl_cnt)
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = dguc_reply2->rs_tbl_cnt)
   PLAN (d
    WHERE (dguc_reply2->dtd_hold[d.seq].ui_found=1))
   DETAIL
    final_cnt = (final_cnt+ 1)
    IF (mod(final_cnt,100)=1)
     stat = alterlist(dguc_reply->dtd_hold,(final_cnt+ 99))
    ENDIF
    dguc_reply->dtd_hold[final_cnt].tbl_name = dguc_reply2->dtd_hold[d.seq].tbl_name, dguc_reply->
    dtd_hold[final_cnt].tbl_suffix = dguc_reply2->dtd_hold[d.seq].tbl_suffix
    FOR (pk_loop_cnt = 1 TO size(dguc_reply2->dtd_hold[d.seq].pk_hold,5))
      IF ((dguc_reply2->dtd_hold[d.seq].pk_hold[pk_loop_cnt].pk_name != ""))
       dguc_reply->dtd_hold[final_cnt].pk_cnt = (dguc_reply->dtd_hold[final_cnt].pk_cnt+ 1), stat =
       alterlist(dguc_reply->dtd_hold[final_cnt].pk_hold,dguc_reply->dtd_hold[final_cnt].pk_cnt),
       dguc_reply->dtd_hold[final_cnt].pk_hold[dguc_reply->dtd_hold[final_cnt].pk_cnt].pk_name =
       dguc_reply2->dtd_hold[d.seq].pk_hold[pk_loop_cnt].pk_name,
       dguc_reply->dtd_hold[final_cnt].pk_hold[dguc_reply->dtd_hold[final_cnt].pk_cnt].pk_datatype =
       dguc_reply2->dtd_hold[d.seq].pk_hold[pk_loop_cnt].pk_datatype
      ENDIF
    ENDFOR
   FOOT REPORT
    stat = alterlist(dguc_reply->dtd_hold,final_cnt), dguc_reply->rs_tbl_cnt = final_cnt
   WITH nocounter
  ;end select
  IF ((dguc_reply->rs_tbl_cnt=0))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Error moving to dguc_reply record structure."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dguc_reply->dguc_err_ind = 1
   SET dguc_reply->dguc_err_msg = "Error moving to  dguc_reply record structure"
   RETURN(0)
  ENDIF
 ELSE
  SET tbl_cnt = 0
  SET col_cnt = 0
  SELECT INTO "NL:"
   FROM (parser(table_dtl) dtd),
    (parser(column_dtl) dcd)
   ...
   HEAD dcd.table_name
    tbl_cnt = (tbl_cnt+ 1), stat = alterlist(dguc_reply->dtd_hold,tbl_cnt), dguc_reply->dtd_hold[
    tbl_cnt].tbl_name = trim(dtd.table_name),
    dguc_reply->dtd_hold[tbl_cnt].tbl_suffix = trim(dtd.table_suffix), col_cnt = 0
   DETAIL
    IF (dcd.table_name=dcd.root_entity_name
     AND dcd.column_name=dcd.root_entity_attr)
     col_cnt = (col_cnt+ 1), stat = alterlist(dguc_reply->dtd_hold[tbl_cnt].pk_hold,col_cnt),
     dguc_reply->dtd_hold[tbl_cnt].pk_cnt = (dguc_reply->dtd_hold[tbl_cnt].pk_cnt+ 1),
     dguc_reply->dtd_hold[tbl_cnt].pk_hold[col_cnt].pk_name = dcd.column_name
    ENDIF
   WITH nocounter, orahintcbo("leading(dcd,dtd)")
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Getting unique columns from tables with non-top level primary keys."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "NL:"
   dtd.*
   FROM (parser(table_dtl) dtd),
   ...
   (SELECT
    ...
    FROM user_tab_columns u,
     (parser(column_dtl) cd)
    WHERE cd.table_name=cd.root_entity_name
     AND cd.column_name=cd.root_entity_attr
     AND cd.table_name=dcd.table_name
     AND cd.column_name=u.column_name
     AND u.table_name=cd.table_name
    WITH orahintcbo("leading(cd)"))))
    AND parser(v_where_str)
   ...
   WITH nocounter, orahintcbo("leading(dcd,dtd)")
  ;end select
  SET dguc_reply->rs_tbl_cnt = tbl_cnt
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dguc_reply->dguc_err_ind = 1
  ...
  SELECT INTO "nl:"
   utc.data_type
   FROM user_tab_columns utc
   WHERE expand(tbl_num,1,size(dguc_reply->dtd_hold,5),utc.table_name,dguc_reply->dtd_hold[tbl_num].
    tbl_name)
   ORDER BY utc.table_name
   HEAD utc.table_name
    tbl_pos = 0, tbl_pos = locateval(tbl_num,1,size(dguc_reply->dtd_hold,5),utc.table_name,dguc_reply
     ->dtd_hold[tbl_num].tbl_name)
   DETAIL
    col_pos = 0, col_pos = locateval(col_num,1,size(dguc_reply->dtd_hold[tbl_pos].pk_hold,5),utc
     .column_name,dguc_reply->dtd_hold[tbl_pos].pk_hold[col_num].pk_name)
    IF (col_pos > 0)
     dguc_reply->dtd_hold[tbl_pos].pk_hold[col_pos].pk_datatype = utc.data_type
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
 ENDIF
 CALL check_override_tbl(null)
 SET dm_err->eproc = "Creating audit report of tables without identifiable primary key."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO value(dm_err->logfile)
  FROM (dummyt d  WITH seq = dguc_reply->rs_tbl_cnt)
  PLAN (d
   WHERE (dguc_reply->dtd_hold[d.seq].pk_cnt=0))
  HEAD REPORT
   log_str = "*********Exception Report for tables with no Identifier Information.*********", log_str,
   row + 1,
   row + 1, pk_cnt = 0
  DETAIL
   log_str = "", log_str = dguc_reply->dtd_hold[d.seq].tbl_name
   IF (size(dguc_reply->dtd_hold[d.seq].pk_hold,5)=0)
    log_str = concat(log_str,":::NO PRIMARY KEY, UNIQUE INDEX, UNIQUE IDENTIFIER"), pk_cnt = (pk_cnt
    + 1)
   ENDIF
   log_str, row + 1
  FOOT REPORT
   row + 1, log_str = concat("Number with no primary key information: ",trim(cnvtstring(pk_cnt)),"."),
   log_str,
   row + 1, row + 1, log_str = "*********End of Report*********",
   log_str, row + 1
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dguc_reply->dguc_err_ind = 0
 SUBROUTINE check_override_tbl(null)
   DECLARE cot_cnt = i4 WITH noconstant(0)
   DECLARE lor_loc_idx = i4 WITH noconstant(0)
   SET dm_err->eproc = "Checking primary key overrides on select tables"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_info di,
     user_tab_columns utc
    PLAN (di
     WHERE di.info_domain="RDDS PK OVERRIDE:*"
      AND  NOT (di.info_name IN ("GROUP_REFERENCE", "ONLINE_ITEM_DEFINITION", "INPUT_FORM_REFERENCE")
     ))
     JOIN (utc
     WHERE utc.table_name=di.info_name
      AND utc.column_name=di.info_char)
    ORDER BY utc.table_name
    HEAD utc.table_name
     lor_loc_idx = 0, lor_loc_idx = locateval(lor_loc_idx,1,dguc_reply->rs_tbl_cnt,trim(di.info_name,
       3),dguc_reply->dtd_hold[lor_loc_idx].tbl_name)
     IF (lor_loc_idx > 0)
      stat = alterlist(dguc_reply->dtd_hold[lor_loc_idx].pk_hold,0)
     ENDIF
     cot_cnt = 0
    DETAIL
     IF (lor_loc_idx > 0)
      cot_cnt = (cot_cnt+ 1), stat = alterlist(dguc_reply->dtd_hold[lor_loc_idx].pk_hold,cot_cnt),
      dguc_reply->dtd_hold[lor_loc_idx].pk_hold[cot_cnt].pk_name = trim(di.info_char,3),
      dguc_reply->dtd_hold[lor_loc_idx].pk_hold[cot_cnt].pk_datatype = utc.data_type
     ENDIF
    FOOT  utc.table_name
     IF (lor_loc_idx > 0)
      dguc_reply->dtd_hold[lor_loc_idx].pk_cnt = cot_cnt
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   SELECT INTO "nl:"
    FROM user_tab_columns utc,
     dm_info di,
     (parser(table_dtl) dtd)
    PLAN (di
     WHERE di.info_domain="RDDS PK OVERRIDE:*"
      AND di.info_name IN ("GROUP_REFERENCE", "ONLINE_ITEM_DEFINITION", "INPUT_FORM_REFERENCE"))
     JOIN (dtd
     WHERE dtd.table_name=di.info_name
      AND dtd.mergeable_ind=1)
     JOIN (utc
     WHERE utc.table_name=di.info_name
      AND utc.column_name=di.info_char)
    ORDER BY di.info_name
