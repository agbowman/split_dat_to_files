CREATE PROGRAM cp_build_get_hlth_maint:dba
 PROMPT
 ...
 FREE RECORD 966302_req
 RECORD 966302_req(
   1 last_load_dt_tm = dq8
 )
 DECLARE lapp_id = i4 WITH protect, constant(3202004)
 DECLARE ltask_id = i4 WITH protect, constant(3202004)
 DECLARE lrequest_id = i4 WITH protect, constant(966302)
 DECLARE x = i4 WITH noconstant(0), protect
 DECLARE y = i4 WITH noconstant(0), protect
 DECLARE z = i4 WITH noconstant(0), protect
 DECLARE lseriessize = i4 WITH noconstant(0), protect
 ...
 SET stat = tdbexecute(lapp_id,ltask_id,lrequest_id,"REC",966302_req,
  "REC",966302_rep)
 FOR (x = 1 TO size(966302_rep->sched,5))
   SET lseriessize = size(966302_rep->sched[x].series,5)
   SET stat = alterlist(mpagereply->expect_list,(mpagereply->expect_cnt+ lseriessize))
   FOR (y = 1 TO lseriessize)
     IF (size(966302_rep->sched[x].series[y].expect,5) > 0)
      SET mpagereply->expect_cnt += 1
      SET mpagereply->expect_list[mpagereply->expect_cnt].expect_name = 966302_rep->sched[x].series[y
      ].expect[1].expect_name
      SET mpagereply->expect_list[mpagereply->expect_cnt].expect_id = 966302_rep->sched[x].series[y].
      expect[1].expect_id
     ENDIF
   ENDFOR
 ENDFOR
 SET mpagereply->status_data.status = 966302_rep->status_data.status
 IF (validate(debug_ind,0))
  CALL echorecord(966302_rep)
