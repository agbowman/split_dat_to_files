CREATE PROGRAM dm2_mig_resumable_sessions:dba
 ...
 SUBROUTINE dm2_rename_login_default(sbr_rld_mode)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmrsm_cont_rep = i2 WITH protect, noconstant(1)
 IF (check_logfile("dm2_mig_res_sess",".log","dm2_mig_resumable_sessions")=0)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag=722))
  SET message = nowindow
 ...
 ENDIF
 SET width = 132
 WHILE (dmrsm_cont_rep=1)
   SET message = window
   CALL clear(1,1)
   CALL box(1,1,24,131)
   CALL text(2,2,"Database Migration Resumable Sesssion Menu")
   CALL text(4,2,"Do you want to view SOURCE or TARGET Resumable Sessions?")
   CALL text(6,2,"1. SOURCE Resumable Sessions")
   CALL text(7,2,"2. TARGET Resumable Sessions")
   CALL text(10,2,"Your Selection (0 to Exit)?")
   CALL accept(10,38,"9;",0
    WHERE curaccept IN (0, 1, 2))
   CASE (curaccept)
    OF 0:
     SET dmrsm_cont_rep = 0
    OF 1:
     SET message = nowindow
...
#exit_script
 ...
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_mig_resumable_sessions has completed"
 ENDIF
 CALL final_disp_msg("dm2_mig_res_sess")
END GO
