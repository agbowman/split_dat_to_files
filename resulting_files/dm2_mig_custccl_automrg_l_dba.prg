CREATE PROGRAM dm2_mig_custccl_automrg_l:dba
 ...
 ENDIF
 DECLARE dmcal_size = i4 WITH protect, noconstant(0)
 ...
 SET readme_data->message = "Readme Failed: Starting script dm2_mig_custccl_automrg_l.prg..."
 SET dmcal_size = size(requestin->list_0,5)
 IF (check_logfile("dm2_mig_cc_automrg",".log","dm2_mig_custccl_automrg_l")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_automrg_l"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Reading data from input CSV file and performing type conversion"
 SET stat = alterlist(copy_requestin->list,dmcal_size)
 FOR (i = 1 TO dmcal_size)
   SET copy_requestin->list[i].info_domain = trim(requestin->list_0[i].info_domain)
   SET copy_requestin->list[i].info_name = trim(requestin->list_0[i].info_name)
   SET copy_requestin->list[i].info_number = cnvtreal(requestin->list_0[i].info_number)
   SET copy_requestin->list[i].exists_ind = 0
 ENDFOR
 ...
 ENDIF
 SET dm_err->eproc = "Determining existance of automrg dm_info rows"
 SELECT INTO "nl:"
  FROM dm_info di,
   (dummyt d  WITH seq = dmcal_size)
  PLAN (d)
   JOIN (di
   WHERE (di.info_domain=copy_requestin->list[d.seq].info_domain)
    AND (di.info_name=copy_requestin->list[d.seq].info_name))
  DETAIL
 ...
 IF (check_error(dm_err->eproc) > 0)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Inserting automrg dm_info rows"
 INSERT  FROM dm_info di,
   (dummyt d  WITH seq = dmcal_size)
  SET di.info_domain = copy_requestin->list[d.seq].info_domain, di.info_name = copy_requestin->list[d
   .seq].info_name, di.info_number = copy_requestin->list[d.seq].info_number
  PLAN (d
 ...
 ELSE
  COMMIT
 ENDIF
 SET dm_err->eproc = "Updating automrg dm_info rows"
 UPDATE  FROM dm_info di,
   (dummyt d  WITH seq = dmcal_size)
  SET di.info_number = copy_requestin->list[d.seq].info_number
  PLAN (d
   WHERE (copy_requestin->list[d.seq].exists_ind=1))
   JOIN (di
   WHERE (di.info_domain=copy_requestin->list[d.seq].info_domain)
    AND (di.info_name=copy_requestin->list[d.seq].info_name))
  WITH nocounter
...
#exit_program
 IF ( NOT (dm_err->err_ind))
  ...
  SET readme_data->message = "Success: Automrg Data Loaded Successfully"
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error when loading automrg dm_info rows:",dm_err->emsg)
 ENDIF
 FREE RECORD copy_requestin
 SET dm_err->eproc = "Ending dm2_mig_custccl_automrg_l."
 CALL final_disp_msg("dm2_mig_custccl_automrg_l")
END GO
