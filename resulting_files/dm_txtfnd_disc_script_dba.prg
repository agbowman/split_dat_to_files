CREATE PROGRAM dm_txtfnd_disc_script:dba
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 IF (check_logfile("dm_tf_dsc_ccl",".log","DM_TXTFND_DISC_SCRIPT LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_TXTFND_DISC_SCRIPT..."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (dtla_detail_id=0.0)
  ...
  SELECT INTO "nl:"
   ...
   WHERE d.detail_type_flag=2
    AND d.active_ind=1
    AND d.detail_script_name="DM_TXTFND_DISC_SCRIPT"
   DETAIL
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "No active data found in DM_TEXT_FIND_DETAIL for script DM_TXTFND_DISC_SCRIPT."
   IF ((script_rec->dm_text_find_log_id > 0.0))
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
   ENDIF
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SET dm_err->eproc = "Gathering Discern scripts"
 SELECT INTO "NL:"
  FROM dprotect p,
   dcompile c
  PLAN (p
   ...
   WHERE "P"=c.object
    ...
    AND 0=c.qual)
  DETAIL
   dtg->cnt = (dtg->cnt+ 1), stat = alterlist(dtg->qual,dtg->cnt), dtg->qual[dtg->cnt].s_name = c
   .object_name,
   dtg->qual[dtg->cnt].data_source = c.object_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for flat file Discern scripts:",dm_err->emsg),0.0,0.0)
  GO TO exit_scrpit
 ENDIF
 SELECT INTO "NL:"
  FROM dprotect p
  WHERE p.object="P"
   AND p.ccl_version=3
  DETAIL
   dtg->cnt = (dtg->cnt+ 1), stat = alterlist(dtg->qual,dtg->cnt), dtg->qual[dtg->cnt].s_name = p
   .object_name,
   dtg->qual[dtg->cnt].data_source = p.object_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for RDBMS Discern scripts:",dm_err->emsg),0.0,0.0)
  GO TO exit_scrpit
 ENDIF
 SET dm_err->eproc = "Transferring data to SCRIPT_REC"
 CALL dst_transfer_data(script_rec,dtg)
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Transferring data between structures:",dm_err->emsg),0.0,0.0)
  GO TO exit_scrpit
 ENDIF
 ...
 ENDIF
#exit_script
 SET dm_err->eproc = "...Ending DM_TXTFND_DISC_SCRIPT"
 CALL final_disp_msg("dm_tf_dsc_ccl")
END GO
