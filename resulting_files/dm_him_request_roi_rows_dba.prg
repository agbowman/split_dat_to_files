CREATE PROGRAM dm_him_request_roi_rows:dba
 ...
 SET reply->table_name = "HIM_REQUEST"
 ...
 DECLARE v_complete_cd = f8 WITH protect, noconstant(0.0)
 DECLARE v_canceled_cd = f8 WITH protect, noconstant(0.0)
 DECLARE v_rejected_cd = f8 WITH protect, noconstant(0.0)
 FREE RECORD code_list
 RECORD code_list(
   1 qual[*]
     2 code_value = f8
 )
 ...
 FOR (tok_ndx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 2190)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"KEEPDAYS",
   "You must keep at least 2190 days' (6 years') worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM code_value cv
  WHERE cv.code_set=14120
   AND cv.cdf_meaning="ROI"
   AND cv.active_ind=1
  HEAD REPORT
   cv_cnt = 0
  DETAIL
   cv_cnt = (cv_cnt+ 1), stat = alterlist(code_list->qual,cv_cnt), code_list->qual[cv_cnt].code_value
    = cv.code_value
  WITH nocounter
 ;end select
 SET v_complete_cd = uar_get_code_by("MEANING",14172,"COMPLETE")
 SET v_canceled_cd = uar_get_code_by("MEANING",14172,"CANCELED")
 SET v_rejected_cd = uar_get_code_by("MEANING",14172,"REJECTED")
 IF (v_complete_cd=0.0)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"COMPLETE",
   "Unable to find code value for CDF meaning 'COMPLETE' in codeset 14172.")
  GO TO exit_script
 ENDIF
 IF (v_canceled_cd=0.0)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"CANCELED",
   "Unable to find code value for CDF meaning 'CANCELED' in codeset 14172.")
  GO TO exit_script
 ENDIF
 IF (v_rejected_cd=0.0)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"REJECTED",
   "Unable to find code value for CDF meaning 'REJECTED' in codeset 14172.")
  GO TO exit_script
 ENDIF
 IF (size(code_list->qual,5)=0)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"ROI",
   "Unable to find code values for CDF meaning 'ROI' in codeset 14120.")
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(hr.him_request_id)
   FROM him_request hr
   WHERE hr.him_request_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    hr.rowid
    FROM him_request hr
    WHERE parser(sbr_getrowidnotexists("hr.him_request_id between curMinID and curMaxID","hr"))
     AND hr.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND ((hr.request_status_cd+ 0) IN (v_complete_cd, v_rejected_cd, v_canceled_cd))
     AND expand(loop,1,size(code_list->qual,5),hr.request_type_cd,code_list->qual[loop].code_value)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = hr.rowid
    WITH nocounter, maxqual(hr,value(rowsleft))
   ;end select
