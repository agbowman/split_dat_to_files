CREATE PROGRAM dm_oucb_preg_instance2029drr:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  IF (debug_ind=1)
   CALL echo("********DM_OUCB_PREG_INSTANCE2029DRR - maintaining exception")
  ENDIF
  ...
  SET dcem_request->qual[1].script_name = "DM_OUCB_PREG_INSTANCE2029DRR"
 ...
 ENDIF
 IF (debug_ind=1)
  CALL echo(build("********DM_OUCB_PREG_INSTANCE2029DRR - FROM_XXX_ID = ",request->xxx_uncombine[
    ucb_cnt].from_xxx_id))
  CALL echo(build("********DM_OUCB_PREG_INSTANCE2029DRR - TO_XXX_ID = ",request->xxx_uncombine[
    ucb_cnt].to_xxx_id))
  CALL echo(build("********DM_OUCB_PREG_INSTANCE2029DRR - data_number = ",rchildren->qual1[det_cnt].
    entity_pk[1].data_number))
  CALL echo(build("********DM_OUCB_PREG_INSTANCE2029DRR - combine_action_cd = ",rchildren->qual1[
 ...
 SUBROUTINE cust_ucb_upt(null)
  UPDATE  FROM pregnancy_instance2029drr p1
   ...
   (SELECT
    p2.pregnancy_id
    FROM pregnancy_instance2029drr p2
    WHERE (p2.pregnancy_instance_id=rchildren->qual1[det_cnt].entity_pk[1].data_number)))
   WITH nocounter
  ;end update
 ...
 SUBROUTINE cust_ucb_del2(null)
   UPDATE  FROM pregnancy_instance2029drr p1
    ...
    WHERE (p1.pregnancy_instance_id=rchildren->qual1[det_cnt].entity_pk[1].data_number)
    WITH nocounter
   ;end update
   IF (curqual=0)
