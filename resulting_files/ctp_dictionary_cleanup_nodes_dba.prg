CREATE PROGRAM ctp_dictionary_cleanup_nodes:dba
 PROMPT
 ...
 SUBROUTINE (_::setatr(app_num=i4,task_num=i4,req_num=i4) =null WITH protect)
 ...
 SUBROUTINE _::crmgetrequest(null)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (_::samenode(node=vc) =i2 WITH protect)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (_::crmperformas(node_name=vc,service_name=vc) =i2 WITH protect)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (_::crmnodeperform(node_name=vc) =i2 WITH protect)
 ...
 SUBROUTINE _::crmend(null)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (_::logerror(value=vc) =null)
 ...
 END ;Subroutine
 SUBROUTINE (_::getstatus(message=vc(ref)) =i2 WITH protect)
  SET message = PRIVATE::err_msg
  RETURN(PRIVATE::success)
 END ;Subroutine
 END; class scope:init
 WITH copy = 0
 SUBROUTINE (executecleanup(node_name=vc,msg=vc(ref)) =i2 WITH protect)
   ...
   DECLARE out_file_name = vc WITH protect, noconstant(" ")
   DECLARE DISCERN::vcclrunprogram = null WITH protect, class(crmsrv_perform)
   EXECUTE cpm_create_file_name "ctpdict", "dat"
   SET out_file_name = cpm_cfn_info->file_name_full_path
   ...
   IF (DISCERN::vcclrunprogram.crmbegin(0))
    IF (DISCERN::vcclrunprogram.crmgetrequest(0))
     ...
     SET stat = uar_srvsetstring(hreq,"program_name","CTP_DICTIONARY_CLEANUP")
     SET stat = uar_srvsetstring(hreq,"output_device","NL:")
     SET stat = uar_srvsetstring(hreq,"IsBlob","1")
     SET stat = uar_srvsetstring(hreq,"params",nullterm(build('"',out_file_name,'"')))
   ...
   RETURN(success)
 END ;Subroutine
 SUBROUTINE (getnodenames(domain=vc) =i2 WITH protect)
 ...
 CALL getnodenames(curdomain)
 FOR (idx = 1 TO nodes->cnt)
   IF ( NOT (executecleanup(nodes->qual[idx].name,message)))
    SET nodes->qual[idx].msg = message
   ENDIF
 ENDFOR
 SELECT INTO  $OUTDEV
  FROM (dummyt d  WITH seq = nodes->cnt)
  HEAD REPORT
   "NODES:", row + 2
  DETAIL
   nodes->qual[d.seq].name, row + 1
   IF (size(trim(nodes->qual[d.seq].msg)) > 0)
    "---->", nodes->qual[d.seq].msg, row + 1
   ENDIF
  WITH format = variable, maxcol = 1000
 ;end select
#abort
END GO
