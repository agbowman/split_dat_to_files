CREATE PROGRAM dm2_aload_adoc_df
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_aload_adoc_df",".log","dm2_aload_adoc_df")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_adoc_df"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dadf_cnt = i4 WITH protect, noconstant(0)
 DECLARE dadf_str = vc WITH protect, noconstant("")
 DECLARE dadf_req_size = i4 WITH protect, noconstant(0)
 DECLARE dadf_admin_db_link = vc WITH protect, noconstant("")
 DECLARE dadf_fail_ind = i2 WITH protect, noconstant(0)
 FREE RECORD dadf_maint
 RECORD dadf_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET dadf_maint->num_of_inserts = 0
 SET dadf_maint->num_of_updates = 0
 SET dadf_maint->num_of_none = 0
 SET dm_err->eproc = "Get admin db link."
 IF (dir_get_admin_db_link(0,dadf_admin_db_link,dadf_fail_ind)=0)
  GO TO exit_program
 ENDIF
 SET dadf_req_size = size(requestin->list_0,5)
 SET stat = alterlist(dadf_maint->qual,dadf_req_size)
 IF (dac_pop_coldic_rec("DM_FLAGS_ALL")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  SET dm_err->eproc = "Determining maintenance type for DM_FLAGS_ALL."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dadf_req_size))
  DETAIL
   dadf_maint->qual[d.seq].maint_type = "I", dadf_maint->num_of_inserts = (dadf_maint->num_of_inserts
   + 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 SELECT INTO "nl:"
  FROM (parser(concat("cdba.dm_flags@",dadf_admin_db_link)) df),
   (dummyt d  WITH seq = value(dadf_req_size))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (df
   WHERE df.owner=currdbuser
    AND (df.table_name=requestin->list_0[d.seq].table_name)
    AND (df.column_name=requestin->list_0[d.seq].column_name)
    AND df.flag_value=cnvtreal(requestin->list_0[d.seq].flag_value))
  ORDER BY df.table_name, df.column_name, df.flag_value
  DETAIL
   IF (cnvtint(requestin->list_0[d.seq].updt_cnt) > df.updt_cnt)
    dadf_maint->qual[d.seq].maint_type = "U", dadf_maint->num_of_updates = (dadf_maint->
    num_of_updates+ 1), dadf_maint->num_of_inserts = (dadf_maint->num_of_inserts - 1)
   ELSE
    dadf_maint->qual[d.seq].maint_type = "N", dadf_maint->num_of_none = (dadf_maint->num_of_none+ 1),
    dadf_maint->num_of_inserts = (dadf_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dadf_maint)
  CALL echorecord(requestin)
 ENDIF
 SET dac_col_list->tbl = concat("cdba.dm_flags@",dadf_admin_db_link)
 IF ((dadf_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(dadf_req_size) )"),0)
  ...
  COMMIT
 ENDIF
 IF ((dadf_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where dadf_maint->qual[d.seq].maint_type = 'U' ",0)
  ...
  IF (dm2_push_cmd(
   " and cnvtreal(requestin->list_0[d.seq].flag_value) = dcv.flag_value  with nocounter go",1)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  IF ((curqual != dadf_maint->num_of_updates))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Updated:",curqual,". Does not match expected rows updated:",
    dadf_maint->num_of_updates)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_adoc_df completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_adoc_df")
END GO
