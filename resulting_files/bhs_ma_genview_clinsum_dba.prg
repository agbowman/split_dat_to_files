CREATE PROGRAM bhs_ma_genview_clinsum:dba
 ...
 DECLARE equal_line = c90 WITH public, constant(fillstring(96,"="))
 ...
 IF ((request->visit_cnt > 0))
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    dlrec->seq[dd.seq].orders[cnt].misc = build(cnvtint(o.order_id),"\",uar_get_code_display(o
      .catalog_type_cd),"\",uar_get_code_display(o.activity_type_cd))
  ...
  SELECT INTO "nl:"
   mso =
   IF (o.med_order_type_cd IN (iv_cd, intermittent_cd)) 4
   ELSEIF (o.prn_ind=0
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   ORDER BY c.encntr_id, c.event_cd, cnvtdatetime(c.event_end_dt_tm)
   HEAD c.encntr_id
    cnt = 0, stat = alterlist(dlrec->seq[dd.seq].events,10)
   HEAD c.event_cd
    col + 0
   DETAIL
    IF (uar_get_code_display(c.event_cd) > " ")
     cnt = (cnt+ 1)
     IF (mod(cnt,10)=1)
     ...
     dlrec->seq[dd.seq].events[cnt].event_dt_tm = substring(1,14,format(c.event_end_dt_tm,
       "@SHORTDATETIME;;Q")), dlrec->seq[dd.seq].events[cnt].result = concat(" {b}",trim(c.result_val
       )," ",trim(uar_get_code_display(c.result_units_cd)),"{endb}"), dlrec->seq[dd.seq].events[cnt].
     misc = build(cnvtint(c.event_cd),"\",cv.display_key)
    ENDIF
   FOOT  c.event_cd
    col + 0
   FOOT  c.encntr_id
  ...
  SELECT INTO request->output_device
   ...
   HEAD REPORT
    "{f/8}{cpi/14}{lpi/8}", row + 1, xcol1 = 26,
    xcol2 = 38, xcol3 = 86, xcol4 = 140,
    xcol5 = 200, xcol7 = 270, xcol8 = 340,
    xcol9 = 375, xcol10 = 380, xcol11 = 410,
    xcol12 = 480, xcolvar = xcol1, yrow1 = 5,
    last_encntr_id = 0.0, mso[5] = "Unknown             ", mso[4] = "Continuous Infusions",
    ...
    ENDIF
    MACRO (title_print)
     CALL print(calcpos(xcol1,yrow1)), equal_line, row + 1,
     yrow1 = (yrow1+ 7),
     CALL print(calcpos(xcol1,yrow1)), "{b}",
     title_string, "{endb}", last_title = title_string,
     rowplusone2
    ENDMACRO
    ,
    MACRO (calcpos_print)
   ...
   HEAD PAGE
    ...
    CALL print(calcpos(xcol10,yrow1)), rpt_title, rowplusone2,
    CALL print(calcpos(xcol1,yrow1)), equal_line, rowplusone2,
    CALL print(calcpos(xcol1,yrow1)), "{b}", dlrec->seq[i].name_full_formatted,
    "{endb}",
    CALL print(calcpos(xcol10,yrow1)), dlrec->seq[i].nurse_unit,
    " ", dlrec->seq[i].pt_loc, rowplusone2,
    CALL print(calcpos(xcol2,yrow1)), "MRN: ", dlrec->seq[i].mrn,
    CALL print(calcpos(xcol4,yrow1)), "DOB: ", dlrec->seq[i].birth_dt_tm"mm/dd/yy;;D",
    CALL print(calcpos(xcol7,yrow1)), "<teaching team>", rowplusone2,
    CALL print(calcpos((xcol2 - 5),yrow1)), "Acct #: ", dlrec->seq[i].account_nbr,
    CALL print(calcpos((xcol4 - 3),yrow1)), "Admit: ", dlrec->seq[i].admit_dt,
    CALL print(calcpos(xcol7,yrow1)), dlrec->seq[i].attenddoc_name, rowplusone2,
    CALL print(calcpos(xcol1,yrow1)), equal_line
    IF (page_cnt > 1)
     rowplusone2
    ENDIF
    IF (page_cnt > 1
    ...
    ENDIF
    CALL print(calcpos(xcol1,660)), equal_line, rowplusone2,
    CALL print(calcpos(xcol1,675)), dlrec->seq[i].name_full_formatted,
    CALL print(calcpos(xcol5,675)),
    "Acct #:", dlrec->seq[i].account_nbr,
    CALL print(calcpos(xcol11,675)),
    "Pg ", page_cnt"###;l", row + 1
   DETAIL
    FOR (i = 1 TO dlrec->encntr_total)
      ...
      FOR (a = 1 TO dlrec->seq[i].problem_total)
        CALL print(calcpos(xcol1,yrow1)), dlrec->seq[i].problem[a].beg_effective_dt_tm, " ",
        dlrec->seq[i].problem[a].full_text, rowplusone
      ENDFOR
      ...
      FOR (a = 1 TO dlrec->seq[i].number_of_orders)
        ...
        dlrec->seq[i].orders[a].order_mnemonic,
        CALL print(calcpos(xcol10,yrow1)), dlrec->seq[i].orders[a].misc,
        rowplusone
      ENDFOR
      lastflag = 0, title_string = "All Events:", title_print
      FOR (a = 1 TO dlrec->seq[i].total_events)
        CALL print(calcpos(xcol1,yrow1)), dlrec->seq[i].events[a].event_dt_tm, " ",
        dlrec->seq[i].events[a].event_cd_disp, " ", dlrec->seq[i].events[a].result,
        CALL print(calcpos(xcol10,yrow1)), dlrec->seq[i].events[a].misc, rowplusone
      ENDFOR
      colgroup = 0, lastparent_event_id = 0.0, lastdate = fillstring(20," "),
      lastflag = 0, title_string = "               ", title_print
      ...
      ENDFOR
      rowplusone, lastflag = 0, title_print
      ...
      FOR (a = 1 TO dlrec->seq[i].number_of_meds)
        ...
        rowplusone, tempstring = dlrec->seq[i].meds[a].display_line, xcolvar = xcol2,
        line_wrap, "{endb}{color/0}"
    ...
    row + 1
   WITH maxcol = 366, maxrow = 166, dio = 36
  ;end select
