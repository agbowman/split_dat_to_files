CREATE PROGRAM bhs_rpt_acpoe_dcorders_al
 PROMPT
 ...
 SET m_info->f_facility_id = value( $F_LOCATION)
 SELECT INTO "nl:"
 ...
 ENDIF
 CALL echoxml(m_info,"al_test.txt")
 SELECT INTO "nl:"
 ...
 SELECT INTO  $OUTDEV
  ...
  original_ord_dt_tm = substring(1,20,m_info->orders[d.seq].s_orig_ord_dt_tm), order_desc = substring
  (1,50,concat(m_info->orders[d.seq].s_order_name,": ",m_info->orders[d.seq].s_ord_desc)), loc =
   $F_LOCATION
  FROM (dummyt d  WITH seq = value(size(m_info->orders,5)))
  PLAN (d)
