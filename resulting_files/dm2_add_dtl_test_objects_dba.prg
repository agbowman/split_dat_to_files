CREATE PROGRAM dm2_add_dtl_test_objects:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE seq_cnt = i4 WITH private
 DECLARE tbl_cnt = i4 WITH private
 DECLARE col_cnt = i4 WITH private
 DECLARE ind_cnt = i4 WITH private
 DECLARE idc_cnt = i4 WITH private
 DECLARE con_cnt = i4 WITH private
 DECLARE mco_cnt = i4 WITH private
 DECLARE bld_cnt = i4 WITH private
 DECLARE dld_cnt = i4 WITH private
 DECLARE spr_cnt = i4 WITH private
 DECLARE sch_type = vc WITH protect
 DECLARE sch_pre = vc WITH protect
 DECLARE file_prefix = vc WITH protect
 DECLARE sch_loc = vc WITH protect
 DECLARE tspace_suffix = vc WITH protect
 DECLARE obj_static_name = vc WITH protect
 DECLARE search_file = vc WITH protect
 DECLARE admin_load_ind = i2 WITH protect, noconstant(0)
 DECLARE header_desc = vc WITH protect
 CALL check_logfile("dm2_add_objs",".log","DM2_ADD_DTL_TEST_OBJECTS")
 SET sch_type =  $1
 SET file_prefix =  $2
 SET sch_loc =  $3
 CASE (cnvtupper(sch_type))
  OF "A":
   SET sch_pre = "dm2a"
  OF "C":
   SET sch_pre = "dm2c"
  OF "O":
   SET sch_pre = "dm2o"
 ENDCASE
 SET dm_err->eproc = concat("Processing schema files - ",cnvtlower(trim(sch_pre)),cnvtlower(trim(
    file_prefix)))
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm2_sch_file->dest_dir_cclfmt = sch_loc
 SET dm2_sch_file->file_prefix = build(sch_pre,file_prefix)
 SET dm2_sch_file->dest_dir_osfmt = build(logical(dm2_sch_file->dest_dir_cclfmt),dm2_sch_file->
  ending_punct)
 SET search_file = build(dm2_sch_file->dest_dir_osfmt,sch_pre,file_prefix,dm2_sch_file->qual[1].
  file_suffix,".dat")
 CALL echo("**********************************************************************")
 CALL echo(concat("dest dir (ccl fmt) = ",dm2_sch_file->dest_dir_cclfmt))
 CALL echo(concat("file prefix = ",dm2_sch_file->file_prefix))
 CALL echo(concat("dest dir (os fmt) = ",dm2_sch_file->dest_dir_osfmt))
 CALL echo("**********************************************************************")
 SET dm_err->eproc = concat("Searching for schema file - ",search_file)
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (findfile(search_file)=0)
  SET dm_err->eproc = "Creating schema files...."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (create_sch_files(null)=0)
   GO TO exit_program
  ENDIF
 ELSE
  SET dm_err->eproc = "Opening schema files...."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (open_sch_files(1)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Removing test objects if already in schema files...."
 CALL disp_msg(" ",dm_err->logfile,0)
 DELETE  FROM dmseq
  WHERE sequence_name="DTL_*"
 ;end delete
 IF (check_error("Removing test sequences...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DELETE  FROM dmtable
  WHERE table_name="DTL_*"
 ;end delete
 IF (check_error("Removing test tables...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DELETE  FROM dmcolumn
  WHERE table_name="DTL_*"
 ;end delete
 IF (check_error("Removing test columns...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DELETE  FROM dmindex
  WHERE table_name="DTL_*"
 ;end delete
 IF (check_error("Removing test indexes...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DELETE  FROM dmindcol
  WHERE table_name="DTL_*"
 ;end delete
 IF (check_error("Removing test index columns...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DELETE  FROM dmcons
  WHERE table_name="DTL_*"
 ;end delete
 IF (check_error("Removing test constraints...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DELETE  FROM dmconscol
  WHERE table_name="DTL_*"
 ;end delete
 IF (check_error("Removing test constraint columns...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (cnvtupper(sch_type)="A")
  SET header_desc = "Admin Schema"
  SET tspace_suffix = "_TOOLKIT"
  SET obj_static_name = "DTL_ADM_"
 ELSEIF (cnvtupper(sch_type)="C")
  SET header_desc = "Clinical Schema"
  SET tspace_suffix = "_A_SMALL"
  SET obj_static_name = "DTL_CLI_"
 ELSE
  SET header_desc = "Clinical Package Schema"
  SET admin_load_ind = 1
  SET tspace_suffix = "_A_SMALL"
  SET obj_static_name = "DTL_PKG_"
 ENDIF
 SET dm_err->eproc = "Inserting test objects into schema files...."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Select from dmheader..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dmheader h
  WITH nocounter
 ...
 ELSE
  IF (curqual=0)
   SET dm_err->eproc = "Inserting row into dmheader...."
   CALL disp_msg(" ",dm_err->logfile,0)
   INSERT  FROM dmheader dh
    SET dh.description = header_desc, dh.admin_load_ind = admin_load_ind, dh.source_rdbms = "ORACLE",
     dh.sf_version = 1
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
 ...
 SET stat = alterlist(tst_dmseq->seqn,1)
 SET seq_cnt = 0
 SET seq_cnt = (seq_cnt+ 1)
 SET tst_dmseq->seqn[seq_cnt].sequence_name = build(concat(obj_static_name,"TESTSEQ"))
 SET tst_dmseq->seqn[seq_cnt].min_value = 1
 SET tst_dmseq->seqn[seq_cnt].max_value = 1000000
 SET tst_dmseq->seqn[seq_cnt].increment_by = 1
 SET tst_dmseq->seqn[seq_cnt].cycle_flag = "Y"
 SET tst_dmseq->seqn[seq_cnt].last_number = 1
 SET tst_dmseq->seqn[seq_cnt].alpha_feature_nbr = 0
 SET tst_dmseq->seqn[seq_cnt].feature_number = 0
 SET tst_dmseq->seqn[seq_cnt].updt_dt_tm = cnvtdatetime(curdate,curtime3)
 SET stat = alterlist(tst_dmseq->seqn,seq_cnt)
 SET size_tst_dmseq = size(tst_dmseq->seqn,5)
 INSERT  FROM dmseq ds,
   (dummyt d  WITH seq = size_tst_dmseq)
  SET ds.sequence_name = tst_dmseq->seqn[d.seq].sequence_name, ds.min_value = tst_dmseq->seqn[d.seq].
   min_value, ds.max_value = tst_dmseq->seqn[d.seq].max_value,
   ds.increment_by = tst_dmseq->seqn[d.seq].increment_by, ds.cycle_flag = tst_dmseq->seqn[d.seq].
   cycle_flag, ds.last_number = tst_dmseq->seqn[d.seq].last_number,
   ds.alpha_feature_nbr = tst_dmseq->seqn[d.seq].alpha_feature_nbr, ds.feature_number = tst_dmseq->
   seqn[d.seq].feature_number, ds.updt_dt_tm = tst_dmseq->seqn[d.seq].updt_dt_tm
  PLAN (d)
   JOIN (ds
   WHERE (tst_dmseq->seqn[d.seq].sequence_name=ds.sequence_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 IF (check_error("Inserting test sequences...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FREE RECORD tst_dmtable
 RECORD tst_dmtable(
   1 tbl[*]
     2 table_name = vc
     2 index_tspace_ni = i2
     2 long_tspace_ni = i2
     2 init_ext = f8
     2 next_ext = f8
     2 pct_increase = f8
 ...
 SET stat = alterlist(tst_dmtable->tbl,2)
 SET tbl_cnt = 0
 SET tbl_cnt = (tbl_cnt+ 1)
 SET tst_dmtable->tbl[tbl_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmtable->tbl[tbl_cnt].index_tspace_ni = 0
 SET tst_dmtable->tbl[tbl_cnt].long_tspace_ni = 0
 SET tst_dmtable->tbl[tbl_cnt].init_ext = 0
 SET tst_dmtable->tbl[tbl_cnt].next_ext = 0
 SET tst_dmtable->tbl[tbl_cnt].pct_increase = 0
 SET tst_dmtable->tbl[tbl_cnt].pct_used = 40
 SET tst_dmtable->tbl[tbl_cnt].pct_free = 10
 SET tst_dmtable->tbl[tbl_cnt].bytes_allocated = 0
 SET tst_dmtable->tbl[tbl_cnt].bytes_used = 0
 SET tst_dmtable->tbl[tbl_cnt].schema_date = cnvtdatetime("01-jan-1900")
 SET tst_dmtable->tbl[tbl_cnt].alpha_feature_nbr = 0
 SET tst_dmtable->tbl[tbl_cnt].feature_number = 0
 SET tst_dmtable->tbl[tbl_cnt].schema_instance = 2
 SET tst_dmtable->tbl[tbl_cnt].updt_dt_tm = cnvtdatetime(curdate,curtime3)
 SET tbl_cnt = (tbl_cnt+ 1)
 SET tst_dmtable->tbl[tbl_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmtable->tbl[tbl_cnt].index_tspace_ni = 0
 SET tst_dmtable->tbl[tbl_cnt].long_tspace_ni = 0
 SET tst_dmtable->tbl[tbl_cnt].init_ext = 0
 SET tst_dmtable->tbl[tbl_cnt].next_ext = 0
 ...
 SET tst_dmtable->tbl[tbl_cnt].schema_instance = 2
 SET tst_dmtable->tbl[tbl_cnt].updt_dt_tm = cnvtdatetime(curdate,curtime3)
 SET stat = alterlist(tst_dmtable->tbl,tbl_cnt)
 SET size_tst_dmtable = size(tst_dmtable->tbl,5)
 INSERT  FROM dmtable dt,
   (dummyt d  WITH seq = size_tst_dmtable)
  SET dt.table_name = tst_dmtable->tbl[d.seq].table_name, dt.tablespace_name = concat("D",
    tspace_suffix), dt.index_tspace = concat("I",tspace_suffix),
   dt.index_tspace_ni = tst_dmtable->tbl[d.seq].index_tspace_ni, dt.long_tspace = concat("D",
    tspace_suffix), dt.long_tspace_ni = tst_dmtable->tbl[d.seq].long_tspace_ni,
   dt.init_ext = tst_dmtable->tbl[d.seq].init_ext, dt.next_ext = tst_dmtable->tbl[d.seq].next_ext, dt
   .pct_increase = tst_dmtable->tbl[d.seq].pct_increase,
   dt.pct_used = tst_dmtable->tbl[d.seq].pct_used, dt.pct_free = tst_dmtable->tbl[d.seq].pct_free, dt
   .bytes_allocated = tst_dmtable->tbl[d.seq].bytes_allocated,
   dt.bytes_used = tst_dmtable->tbl[d.seq].bytes_used, dt.schema_date = tst_dmtable->tbl[d.seq].
   schema_date, dt.alpha_feature_nbr = tst_dmtable->tbl[d.seq].alpha_feature_nbr,
   dt.feature_number = tst_dmtable->tbl[d.seq].feature_number, dt.schema_instance = tst_dmtable->tbl[
   d.seq].schema_instance, dt.updt_dt_tm = tst_dmtable->tbl[d.seq].updt_dt_tm
  PLAN (d)
   JOIN (dt
   WHERE (tst_dmtable->tbl[d.seq].table_name=dt.table_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 IF (check_error("Inserting test tables...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FREE RECORD tst_dmcolumn
 RECORD tst_dmcolumn(
   1 col[*]
     2 table_name = vc
     2 column_name = vc
     ...
     2 nullable = vc
     2 data_default = vc
     2 data_default_ni = i2
 )
 SET stat = alterlist(tst_dmcolumn->col,15)
 SET col_cnt = 0
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL1_ID"
 SET tst_dmcolumn->col[col_cnt].column_seq = 1
 SET tst_dmcolumn->col[col_cnt].data_type = "NUMBER"
 SET tst_dmcolumn->col[col_cnt].data_length = 22
 SET tst_dmcolumn->col[col_cnt].nullable = "N"
 SET tst_dmcolumn->col[col_cnt].data_default = "0"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 0
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL2_CD"
 SET tst_dmcolumn->col[col_cnt].column_seq = 2
 SET tst_dmcolumn->col[col_cnt].data_type = "NUMBER"
 SET tst_dmcolumn->col[col_cnt].data_length = 22
 SET tst_dmcolumn->col[col_cnt].nullable = "N"
 SET tst_dmcolumn->col[col_cnt].data_default = "0"
 ...
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL3"
 SET tst_dmcolumn->col[col_cnt].column_seq = 3
 SET tst_dmcolumn->col[col_cnt].data_type = "CHAR"
 SET tst_dmcolumn->col[col_cnt].data_length = 20
 SET tst_dmcolumn->col[col_cnt].nullable = "Y"
 SET tst_dmcolumn->col[col_cnt].data_default = "'TEST DEFAULT'"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 0
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL4"
 SET tst_dmcolumn->col[col_cnt].column_seq = 4
 SET tst_dmcolumn->col[col_cnt].data_type = "DATE"
 SET tst_dmcolumn->col[col_cnt].data_length = 7
 SET tst_dmcolumn->col[col_cnt].nullable = "Y"
 SET tst_dmcolumn->col[col_cnt].data_default = "SYSDATE"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 0
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL5"
 SET tst_dmcolumn->col[col_cnt].column_seq = 5
 SET tst_dmcolumn->col[col_cnt].data_type = "DATE"
 SET tst_dmcolumn->col[col_cnt].data_length = 7
 SET tst_dmcolumn->col[col_cnt].nullable = "Y"
 SET tst_dmcolumn->col[col_cnt].data_default =
 "TO_DATE('12/31/2100 00:00:00','MM/DD/YYYY HH24:MI:SS')"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 0
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL6"
 SET tst_dmcolumn->col[col_cnt].column_seq = 6
 SET tst_dmcolumn->col[col_cnt].data_type = "VARCHAR2"
 SET tst_dmcolumn->col[col_cnt].data_length = 100
 SET tst_dmcolumn->col[col_cnt].nullable = "Y"
 SET tst_dmcolumn->col[col_cnt].data_default = "'NULL'"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 0
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL7"
 SET tst_dmcolumn->col[col_cnt].column_seq = 7
 SET tst_dmcolumn->col[col_cnt].data_type = "FLOAT"
 SET tst_dmcolumn->col[col_cnt].data_length = 22
 SET tst_dmcolumn->col[col_cnt].nullable = "Y"
 SET tst_dmcolumn->col[col_cnt].data_default = "0.0"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 0
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL8"
 SET tst_dmcolumn->col[col_cnt].column_seq = 8
 SET tst_dmcolumn->col[col_cnt].data_type = "LONG"
 SET tst_dmcolumn->col[col_cnt].data_length = 0
 SET tst_dmcolumn->col[col_cnt].nullable = "Y"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 1
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL9"
 SET tst_dmcolumn->col[col_cnt].column_seq = 9
 SET tst_dmcolumn->col[col_cnt].data_type = "RAW"
 SET tst_dmcolumn->col[col_cnt].data_length = 32
 SET tst_dmcolumn->col[col_cnt].nullable = "Y"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 1
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL10"
 SET tst_dmcolumn->col[col_cnt].column_seq = 10
 SET tst_dmcolumn->col[col_cnt].data_type = "ROWID"
 SET tst_dmcolumn->col[col_cnt].data_length = 10
 SET tst_dmcolumn->col[col_cnt].nullable = "Y"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 1
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL1_ID"
 SET tst_dmcolumn->col[col_cnt].column_seq = 1
 SET tst_dmcolumn->col[col_cnt].data_type = "NUMBER"
 SET tst_dmcolumn->col[col_cnt].data_length = 22
 ...
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL2"
 SET tst_dmcolumn->col[col_cnt].column_seq = 2
 SET tst_dmcolumn->col[col_cnt].data_type = "LONG RAW"
 SET tst_dmcolumn->col[col_cnt].data_length = 0
 SET tst_dmcolumn->col[col_cnt].nullable = "Y"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 1
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL3_ID"
 SET tst_dmcolumn->col[col_cnt].column_seq = 3
 SET tst_dmcolumn->col[col_cnt].data_type = "NUMBER"
 SET tst_dmcolumn->col[col_cnt].data_length = 22
 SET tst_dmcolumn->col[col_cnt].nullable = "N"
 ...
 SET col_cnt = (col_cnt+ 1)
 SET tst_dmcolumn->col[col_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmcolumn->col[col_cnt].column_name = "COL4_CD"
 SET tst_dmcolumn->col[col_cnt].column_seq = 4
 SET tst_dmcolumn->col[col_cnt].data_type = "NUMBER"
 SET tst_dmcolumn->col[col_cnt].data_length = 22
 SET tst_dmcolumn->col[col_cnt].nullable = "N"
 SET tst_dmcolumn->col[col_cnt].data_default = "0"
 SET tst_dmcolumn->col[col_cnt].data_default_ni = 0
 SET stat = alterlist(tst_dmcolumn->col,col_cnt)
 SET size_tst_dmcolumn = size(tst_dmcolumn->col,5)
 INSERT  FROM dmcolumn dc,
   (dummyt d  WITH seq = size_tst_dmcolumn)
  SET dc.table_name = tst_dmcolumn->col[d.seq].table_name, dc.column_name = tst_dmcolumn->col[d.seq].
   column_name, dc.column_seq = tst_dmcolumn->col[d.seq].column_seq,
   dc.data_type = tst_dmcolumn->col[d.seq].data_type, dc.data_length = tst_dmcolumn->col[d.seq].
   data_length, dc.nullable = tst_dmcolumn->col[d.seq].nullable,
   dc.data_default = tst_dmcolumn->col[d.seq].data_default, dc.data_default_ni = tst_dmcolumn->col[d
   .seq].data_default_ni
  PLAN (d)
   JOIN (dc
   WHERE (tst_dmcolumn->col[d.seq].table_name=dc.table_name)
    AND (tst_dmcolumn->col[d.seq].column_name=dc.column_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 IF (check_error("Inserting test table columns...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FREE RECORD tst_dmindex
 RECORD tst_dmindex(
   1 ind[*]
     2 table_name = vc
     2 index_name = vc
 ...
 SET stat = alterlist(tst_dmindex->ind,4)
 SET ind_cnt = 0
 SET ind_cnt = (ind_cnt+ 1)
 SET tst_dmindex->ind[ind_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmindex->ind[ind_cnt].index_name = build(concat("XPK",obj_static_name,"TESTA"))
 SET tst_dmindex->ind[ind_cnt].full_ind_name = build(concat("XPK",obj_static_name,"TESTA"))
 SET tst_dmindex->ind[ind_cnt].pct_increase = 0
 SET tst_dmindex->ind[ind_cnt].pct_free = 10
 SET tst_dmindex->ind[ind_cnt].init_ext = 100000
 SET tst_dmindex->ind[ind_cnt].next_ext = 100000
 SET tst_dmindex->ind[ind_cnt].unique_ind = 1
 SET tst_dmindex->ind[ind_cnt].bytes_allocated = 0
 SET tst_dmindex->ind[ind_cnt].bytes_used = 0
 SET ind_cnt = (ind_cnt+ 1)
 SET tst_dmindex->ind[ind_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmindex->ind[ind_cnt].index_name = build(concat("XAK1",obj_static_name,"TESTA"))
 SET tst_dmindex->ind[ind_cnt].full_ind_name = build(concat("XAK1",obj_static_name,"TESTA"))
 SET tst_dmindex->ind[ind_cnt].pct_increase = 0
 SET tst_dmindex->ind[ind_cnt].pct_free = 10
 SET tst_dmindex->ind[ind_cnt].init_ext = 100000
 SET tst_dmindex->ind[ind_cnt].next_ext = 100000
 ...
 SET ind_cnt = (ind_cnt+ 1)
 SET tst_dmindex->ind[ind_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmindex->ind[ind_cnt].index_name = build(concat("XIE1",obj_static_name,"TESTA"))
 SET tst_dmindex->ind[ind_cnt].full_ind_name = build(concat("XIE1",obj_static_name,"TESTA"))
 SET tst_dmindex->ind[ind_cnt].pct_increase = 0
 SET tst_dmindex->ind[ind_cnt].pct_free = 10
 SET tst_dmindex->ind[ind_cnt].init_ext = 100000
 SET tst_dmindex->ind[ind_cnt].next_ext = 100000
 SET tst_dmindex->ind[ind_cnt].unique_ind = 0
 SET tst_dmindex->ind[ind_cnt].bytes_allocated = 0
 SET tst_dmindex->ind[ind_cnt].bytes_used = 0
 SET ind_cnt = (ind_cnt+ 1)
 SET tst_dmindex->ind[ind_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmindex->ind[ind_cnt].index_name = build(concat("XPK",obj_static_name,"TESTB"))
 SET tst_dmindex->ind[ind_cnt].full_ind_name = build(concat("XPK",obj_static_name,"TESTB"))
 SET tst_dmindex->ind[ind_cnt].pct_increase = 0
 SET tst_dmindex->ind[ind_cnt].pct_free = 10
 SET tst_dmindex->ind[ind_cnt].init_ext = 100000
 SET tst_dmindex->ind[ind_cnt].next_ext = 100000
 ...
 SET tst_dmindex->ind[ind_cnt].bytes_allocated = 0
 SET tst_dmindex->ind[ind_cnt].bytes_used = 0
 SET stat = alterlist(tst_dmindex->ind,ind_cnt)
 SET size_tst_dmindex = size(tst_dmindex->ind,5)
 INSERT  FROM dmindex di,
   (dummyt d  WITH seq = size_tst_dmindex)
  SET di.table_name = tst_dmindex->ind[d.seq].table_name, di.index_name = tst_dmindex->ind[d.seq].
   index_name, di.full_ind_name = tst_dmindex->ind[d.seq].full_ind_name,
   di.pct_increase = tst_dmindex->ind[d.seq].pct_increase, di.pct_free = tst_dmindex->ind[d.seq].
   pct_free, di.init_ext = tst_dmindex->ind[d.seq].init_ext,
   di.next_ext = tst_dmindex->ind[d.seq].next_ext, di.unique_ind = tst_dmindex->ind[d.seq].unique_ind,
   di.bytes_allocated = tst_dmindex->ind[d.seq].bytes_allocated,
   di.bytes_used = tst_dmindex->ind[d.seq].bytes_used
  PLAN (d)
   JOIN (di
   WHERE (tst_dmindex->ind[d.seq].table_name=di.table_name)
    AND (tst_dmindex->ind[d.seq].index_name=di.index_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 IF (check_error("Inserting test indexes...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FREE RECORD tst_dmindcol
 RECORD tst_dmindcol(
   1 idc[*]
     2 table_name = vc
     2 index_name = vc
 ...
 SET stat = alterlist(tst_dmindcol->idc,8)
 SET idc_cnt = 0
 SET idc_cnt = (idc_cnt+ 1)
 SET tst_dmindcol->idc[idc_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmindcol->idc[idc_cnt].index_name = build(concat("XPK",obj_static_name,"TESTA"))
 SET tst_dmindcol->idc[idc_cnt].column_name = "COL1_ID"
 SET tst_dmindcol->idc[idc_cnt].column_position = 1
 SET idc_cnt = (idc_cnt+ 1)
 SET tst_dmindcol->idc[idc_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmindcol->idc[idc_cnt].index_name = build(concat("XAK1",obj_static_name,"TESTA"))
 SET tst_dmindcol->idc[idc_cnt].column_name = "COL2_CD"
 SET tst_dmindcol->idc[idc_cnt].column_position = 1
 SET idc_cnt = (idc_cnt+ 1)
 SET tst_dmindcol->idc[idc_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmindcol->idc[idc_cnt].index_name = build(concat("XAK1",obj_static_name,"TESTA"))
 SET tst_dmindcol->idc[idc_cnt].column_name = "COL3"
 SET tst_dmindcol->idc[idc_cnt].column_position = 2
 SET idc_cnt = (idc_cnt+ 1)
 SET tst_dmindcol->idc[idc_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmindcol->idc[idc_cnt].index_name = build(concat("XIE1",obj_static_name,"TESTA"))
 SET tst_dmindcol->idc[idc_cnt].column_name = "COL5"
 SET tst_dmindcol->idc[idc_cnt].column_position = 1
 SET idc_cnt = (idc_cnt+ 1)
 SET tst_dmindcol->idc[idc_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmindcol->idc[idc_cnt].index_name = build(concat("XIE1",obj_static_name,"TESTA"))
 SET tst_dmindcol->idc[idc_cnt].column_name = "COL6"
 SET tst_dmindcol->idc[idc_cnt].column_position = 2
 SET idc_cnt = (idc_cnt+ 1)
 SET tst_dmindcol->idc[idc_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmindcol->idc[idc_cnt].index_name = build(concat("XPK",obj_static_name,"TESTB"))
 SET tst_dmindcol->idc[idc_cnt].column_name = "COL1_ID"
 SET tst_dmindcol->idc[idc_cnt].column_position = 1
 SET stat = alterlist(tst_dmindcol->idc,idc_cnt)
 SET size_tst_dmindcol = size(tst_dmindcol->idc,5)
 INSERT  FROM dmindcol i,
   (dummyt d  WITH seq = size_tst_dmindcol)
  SET i.table_name = tst_dmindcol->idc[d.seq].table_name, i.index_name = tst_dmindcol->idc[d.seq].
   index_name, i.column_name = tst_dmindcol->idc[d.seq].column_name,
   i.column_position = tst_dmindcol->idc[d.seq].column_position
  PLAN (d)
   JOIN (i
   WHERE (tst_dmindcol->idc[d.seq].table_name=i.table_name)
    AND (tst_dmindcol->idc[d.seq].index_name=i.index_name)
    AND (tst_dmindcol->idc[d.seq].column_name=i.column_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 IF (check_error("Inserting test index columns...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FREE RECORD tst_dmcons
 RECORD tst_dmcons(
   1 con[*]
     2 table_name = vc
     2 constraint_name = vc
 ...
 SET stat = alterlist(tst_dmcons->con,5)
 SET con_cnt = 0
 SET con_cnt = (con_cnt+ 1)
 SET tst_dmcons->con[con_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcons->con[con_cnt].constraint_name = build(concat("XPK",obj_static_name,"TESTA"))
 SET tst_dmcons->con[con_cnt].full_cons_name = build(concat("XPK",obj_static_name,"TESTA"))
 SET tst_dmcons->con[con_cnt].constraint_type = "P"
 SET tst_dmcons->con[con_cnt].status_ind = 1
 SET tst_dmcons->con[con_cnt].r_constraint_name = " "
 SET tst_dmcons->con[con_cnt].parent_table_name = " "
 SET tst_dmcons->con[con_cnt].parent_table_columns = " "
 SET con_cnt = (con_cnt+ 1)
 SET tst_dmcons->con[con_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcons->con[con_cnt].constraint_name = build(concat("XAK1",obj_static_name,"TESTA"))
 SET tst_dmcons->con[con_cnt].full_cons_name = build(concat("XAK1",obj_static_name,"TESTA"))
 SET tst_dmcons->con[con_cnt].constraint_type = "U"
 SET tst_dmcons->con[con_cnt].status_ind = 1
 SET tst_dmcons->con[con_cnt].r_constraint_name = " "
 SET tst_dmcons->con[con_cnt].parent_table_name = " "
 SET tst_dmcons->con[con_cnt].parent_table_columns = " "
 SET con_cnt = (con_cnt+ 1)
 SET tst_dmcons->con[con_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmcons->con[con_cnt].constraint_name = build(concat("XPK",obj_static_name,"TESTB"))
 SET tst_dmcons->con[con_cnt].full_cons_name = build(concat("XPK",obj_static_name,"TESTB"))
 SET tst_dmcons->con[con_cnt].constraint_type = "P"
 SET tst_dmcons->con[con_cnt].status_ind = 1
 SET tst_dmcons->con[con_cnt].r_constraint_name = " "
 SET tst_dmcons->con[con_cnt].parent_table_name = " "
 ...
 SET con_cnt = (con_cnt+ 1)
 SET tst_dmcons->con[con_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmcons->con[con_cnt].constraint_name = build(concat("XFK001",obj_static_name,"TESTB"))
 SET tst_dmcons->con[con_cnt].full_cons_name = build(concat("XFK001",obj_static_name,"TESTB"))
 SET tst_dmcons->con[con_cnt].constraint_type = "R"
 SET tst_dmcons->con[con_cnt].status_ind = 0
 SET tst_dmcons->con[con_cnt].r_constraint_name = build(concat("XPK",obj_static_name,"TESTA"))
 SET tst_dmcons->con[con_cnt].parent_table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmcons->con[con_cnt].parent_table_columns = "COL1_ID"
 SET stat = alterlist(tst_dmcons->con,con_cnt)
 SET size_tst_dmcons = size(tst_dmcons->con,5)
 INSERT  FROM dmcons c,
   (dummyt d  WITH seq = size_tst_dmcons)
  SET c.table_name = tst_dmcons->con[d.seq].table_name, c.constraint_name = tst_dmcons->con[d.seq].
   constraint_name, c.full_cons_name = tst_dmcons->con[d.seq].full_cons_name,
   c.constraint_type = tst_dmcons->con[d.seq].constraint_type, c.status_ind = tst_dmcons->con[d.seq].
   status_ind, c.r_constraint_name = tst_dmcons->con[d.seq].r_constraint_name,
   c.parent_table_name = tst_dmcons->con[d.seq].parent_table_name, c.parent_table_columns =
   tst_dmcons->con[d.seq].parent_table_columns
  PLAN (d)
   JOIN (c
   WHERE (tst_dmcons->con[d.seq].table_name=c.table_name)
    AND (tst_dmcons->con[d.seq].constraint_name=c.constraint_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 IF (check_error("Inserting test constraints...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FREE RECORD tst_dmconscol
 RECORD tst_dmconscol(
   1 mco[*]
     2 table_name = vc
     2 constraint_name = vc
     2 column_name = vc
 ...
 SET stat = alterlist(tst_dmconscol->mco,6)
 SET mco_cnt = 0
 SET mco_cnt = (mco_cnt+ 1)
 SET tst_dmconscol->mco[mco_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmconscol->mco[mco_cnt].constraint_name = build(concat("XPK",obj_static_name,"TESTA"))
 SET tst_dmconscol->mco[mco_cnt].column_name = "COL1_ID"
 SET tst_dmconscol->mco[mco_cnt].position = 1
 SET mco_cnt = (mco_cnt+ 1)
 SET tst_dmconscol->mco[mco_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmconscol->mco[mco_cnt].constraint_name = build(concat("XAK1",obj_static_name,"TESTA"))
 SET tst_dmconscol->mco[mco_cnt].column_name = "COL2_CD"
 SET tst_dmconscol->mco[mco_cnt].position = 1
 SET mco_cnt = (mco_cnt+ 1)
 SET tst_dmconscol->mco[mco_cnt].table_name = build(concat(obj_static_name,"TESTA"))
 SET tst_dmconscol->mco[mco_cnt].constraint_name = build(concat("XAK1",obj_static_name,"TESTA"))
 SET tst_dmconscol->mco[mco_cnt].column_name = "COL3"
 SET tst_dmconscol->mco[mco_cnt].position = 2
 SET mco_cnt = (mco_cnt+ 1)
 SET tst_dmconscol->mco[mco_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmconscol->mco[mco_cnt].constraint_name = build(concat("XPK",obj_static_name,"TESTB"))
 SET tst_dmconscol->mco[mco_cnt].column_name = "COL1_ID"
 SET tst_dmconscol->mco[mco_cnt].position = 1
 SET mco_cnt = (mco_cnt+ 1)
 SET tst_dmconscol->mco[mco_cnt].table_name = build(concat(obj_static_name,"TESTB"))
 SET tst_dmconscol->mco[mco_cnt].constraint_name = build(concat("XFK001",obj_static_name,"TESTB"))
 SET tst_dmconscol->mco[mco_cnt].column_name = "COL3_ID"
 SET tst_dmconscol->mco[mco_cnt].position = 1
 SET stat = alterlist(tst_dmconscol->mco,mco_cnt)
 SET size_tst_dmconscol = size(tst_dmconscol->mco,5)
 INSERT  FROM dmconscol dsc,
   (dummyt d  WITH seq = size_tst_dmconscol)
  SET dsc.table_name = tst_dmconscol->mco[d.seq].table_name, dsc.constraint_name = tst_dmconscol->
   mco[d.seq].constraint_name, dsc.column_name = tst_dmconscol->mco[d.seq].column_name,
   dsc.position = tst_dmconscol->mco[d.seq].position
  PLAN (d)
   JOIN (dsc
   WHERE (tst_dmconscol->mco[d.seq].table_name=dsc.table_name)
    AND (tst_dmconscol->mco[d.seq].constraint_name=dsc.constraint_name)
    AND (tst_dmconscol->mco[d.seq].column_name=dsc.column_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 IF (check_error("Inserting test constraint columns...")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (cnvtupper(sch_type)="O")
  FREE RECORD tst_dmtbldoc
  RECORD tst_dmtbldoc(
    1 bld[*]
      2 table_name = vc
  ...
  SET stat = alterlist(tst_dmtbldoc->bld,4)
  SET bld_cnt = 0
  SET bld_cnt = (bld_cnt+ 1)
  SET tst_dmtbldoc->bld[bld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmtbldoc->bld[bld_cnt].data_model_section = "Test table A"
  SET tst_dmtbldoc->bld[bld_cnt].description = "This is a test table A"
  SET tst_dmtbldoc->bld[bld_cnt].definition = " "
  SET tst_dmtbldoc->bld[bld_cnt].static_rows = 0
  SET tst_dmtbldoc->bld[bld_cnt].updt_cnt = 0
  SET tst_dmtbldoc->bld[bld_cnt].reference_ind = 0
  SET tst_dmtbldoc->bld[bld_cnt].human_reqd_ind = 0
  SET tst_dmtbldoc->bld[bld_cnt].drop_ind = 0
  SET tst_dmtbldoc->bld[bld_cnt].table_suffix = "9974"
  SET tst_dmtbldoc->bld[bld_cnt].full_table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmtbldoc->bld[bld_cnt].suffixed_table_name = build(obj_static_name,"TESTA","9974")
  SET tst_dmtbldoc->bld[bld_cnt].person_cmb_trigger_type = " "
  SET tst_dmtbldoc->bld[bld_cnt].default_row_ind = 0
  SET tst_dmtbldoc->bld[bld_cnt].encntr_cmb_trigger_type = " "
  SET bld_cnt = (bld_cnt+ 1)
  SET tst_dmtbldoc->bld[bld_cnt].table_name = build(obj_static_name,"TESTA","9974")
  SET tst_dmtbldoc->bld[bld_cnt].data_model_section = "Test table A"
  SET tst_dmtbldoc->bld[bld_cnt].description = "This is a test table A"
  SET tst_dmtbldoc->bld[bld_cnt].definition = " "
  SET tst_dmtbldoc->bld[bld_cnt].static_rows = 0
  ...
  SET tst_dmtbldoc->bld[bld_cnt].encntr_cmb_trigger_type = " "
  SET bld_cnt = (bld_cnt+ 1)
  SET tst_dmtbldoc->bld[bld_cnt].table_name = build(concat(obj_static_name,"TESTB"))
  SET tst_dmtbldoc->bld[bld_cnt].data_model_section = "Test table B"
  SET tst_dmtbldoc->bld[bld_cnt].description = "This is a test table B"
  SET tst_dmtbldoc->bld[bld_cnt].definition = " "
  SET tst_dmtbldoc->bld[bld_cnt].static_rows = 0
  SET tst_dmtbldoc->bld[bld_cnt].updt_cnt = 0
  SET tst_dmtbldoc->bld[bld_cnt].reference_ind = 0
  SET tst_dmtbldoc->bld[bld_cnt].human_reqd_ind = 0
  SET tst_dmtbldoc->bld[bld_cnt].drop_ind = 0
  SET tst_dmtbldoc->bld[bld_cnt].table_suffix = "9975"
  SET tst_dmtbldoc->bld[bld_cnt].full_table_name = build(concat(obj_static_name,"TESTB"))
  SET tst_dmtbldoc->bld[bld_cnt].suffixed_table_name = build(obj_static_name,"TESTB","9975")
  SET tst_dmtbldoc->bld[bld_cnt].person_cmb_trigger_type = " "
  SET tst_dmtbldoc->bld[bld_cnt].default_row_ind = 0
  SET tst_dmtbldoc->bld[bld_cnt].encntr_cmb_trigger_type = " "
  SET bld_cnt = (bld_cnt+ 1)
  SET tst_dmtbldoc->bld[bld_cnt].table_name = build(obj_static_name,"TESTB","9975")
  SET tst_dmtbldoc->bld[bld_cnt].data_model_section = "Test table B"
  SET tst_dmtbldoc->bld[bld_cnt].description = "This is a test table B"
  SET tst_dmtbldoc->bld[bld_cnt].definition = " "
  SET tst_dmtbldoc->bld[bld_cnt].static_rows = 0
  ...
  SET tst_dmtbldoc->bld[bld_cnt].default_row_ind = 0
  SET tst_dmtbldoc->bld[bld_cnt].encntr_cmb_trigger_type = " "
  SET stat = alterlist(tst_dmtbldoc->bld,bld_cnt)
  SET size_tst_dmtbldoc = size(tst_dmtbldoc->bld,5)
  INSERT  FROM dmtbldoc bl,
    (dummyt d  WITH seq = size_tst_dmtbldoc)
   SET bl.table_name = tst_dmtbldoc->bld[d.seq].table_name, bl.data_model_section = tst_dmtbldoc->
    bld[d.seq].data_model_section, bl.description = tst_dmtbldoc->bld[d.seq].description,
    bl.definition = tst_dmtbldoc->bld[d.seq].definition, bl.static_rows = tst_dmtbldoc->bld[d.seq].
    static_rows, bl.updt_cnt = tst_dmtbldoc->bld[d.seq].updt_cnt,
    bl.reference_ind = tst_dmtbldoc->bld[d.seq].reference_ind, bl.human_reqd_ind = tst_dmtbldoc->bld[
    d.seq].human_reqd_ind, bl.drop_ind = tst_dmtbldoc->bld[d.seq].drop_ind,
    bl.suffixed_table_name = tst_dmtbldoc->bld[d.seq].suffixed_table_name, bl.table_suffix =
    tst_dmtbldoc->bld[d.seq].table_suffix, bl.full_table_name = tst_dmtbldoc->bld[d.seq].
    full_table_name,
    bl.person_cmb_trigger_type = tst_dmtbldoc->bld[d.seq].person_cmb_trigger_type, bl.default_row_ind
     = tst_dmtbldoc->bld[d.seq].default_row_ind, bl.encntr_cmb_trigger_type = tst_dmtbldoc->bld[d.seq
    ].encntr_cmb_trigger_type
   PLAN (d)
    JOIN (bl
    WHERE (tst_dmtbldoc->bld[d.seq].table_name=bl.table_name))
   WITH nocounter, outerjoin = d, dontexist
  ;end insert
  FREE RECORD tst_dmcoldoc
  RECORD tst_dmcoldoc(
    1 dld[*]
      2 table_name = vc
      2 column_name = vc
  ...
  SET stat = alterlist(tst_dmcoldoc->dld,30)
  SET dld_cnt = 0
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL1_ID"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  SET tst_dmcoldoc->dld[dld_cnt].flag_ind = 0
  SET tst_dmcoldoc->dld[dld_cnt].updt_cnt = 0
  SET tst_dmcoldoc->dld[dld_cnt].unique_ident_ind = 1
  SET tst_dmcoldoc->dld[dld_cnt].root_entity_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].root_entity_attr = " "
  SET tst_dmcoldoc->dld[dld_cnt].constant_value = " "
  SET tst_dmcoldoc->dld[dld_cnt].parent_entity_col = " "
  SET tst_dmcoldoc->dld[dld_cnt].exception_flg = 0
  SET tst_dmcoldoc->dld[dld_cnt].defining_attribute_ind = 0
  SET tst_dmcoldoc->dld[dld_cnt].merge_updateable_ind = 0
  SET tst_dmcoldoc->dld[dld_cnt].nls_col_ind = 0
  SET tst_dmcoldoc->dld[dld_cnt].absolute_date_ind = 0
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL2_CD"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL3"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL4"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL5"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL6"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL7"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL8"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL9"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL10"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET tst_dmcoldoc->dld[dld_cnt].absolute_date_ind = 0
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTA","9974"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL1_ID"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  ...
  SET tst_dmcoldoc->dld[dld_cnt].absolute_date_ind = 0
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTB"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL1_ID"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTB"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL2"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTB"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL3_ID"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTB"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL4_CD"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  SET tst_dmcoldoc->dld[dld_cnt].definition = " "
  ...
  SET tst_dmcoldoc->dld[dld_cnt].absolute_date_ind = 0
  SET dld_cnt = (dld_cnt+ 1)
  SET tst_dmcoldoc->dld[dld_cnt].table_name = build(concat(obj_static_name,"TESTB","9975"))
  SET tst_dmcoldoc->dld[dld_cnt].column_name = "COL1_ID"
  SET tst_dmcoldoc->dld[dld_cnt].sequence_name = " "
  SET tst_dmcoldoc->dld[dld_cnt].code_set = 0
  SET tst_dmcoldoc->dld[dld_cnt].description = "This is a test column"
  ...
  SET tst_dmcoldoc->dld[dld_cnt].nls_col_ind = 0
  SET tst_dmcoldoc->dld[dld_cnt].absolute_date_ind = 0
  SET stat = alterlist(tst_dmcoldoc->dld,dld_cnt)
  SET size_tst_dmcoldoc = size(tst_dmcoldoc->dld,5)
  INSERT  FROM dmcoldoc dod,
    (dummyt d  WITH seq = size_tst_dmcoldoc)
   SET dod.table_name = tst_dmcoldoc->dld[d.seq].table_name, dod.column_name = tst_dmcoldoc->dld[d
    .seq].column_name, dod.sequence_name = tst_dmcoldoc->dld[d.seq].sequence_name,
    dod.code_set = tst_dmcoldoc->dld[d.seq].code_set, dod.description = tst_dmcoldoc->dld[d.seq].
    description, dod.definition = tst_dmcoldoc->dld[d.seq].definition,
    dod.flag_ind = tst_dmcoldoc->dld[d.seq].flag_ind, dod.updt_cnt = tst_dmcoldoc->dld[d.seq].
    updt_cnt, dod.unique_ident_ind = tst_dmcoldoc->dld[d.seq].unique_ident_ind,
    dod.root_entity_name = tst_dmcoldoc->dld[d.seq].root_entity_name, dod.root_entity_attr =
    tst_dmcoldoc->dld[d.seq].root_entity_attr, dod.constant_value = tst_dmcoldoc->dld[d.seq].
    constant_value,
    dod.parent_entity_col = tst_dmcoldoc->dld[d.seq].parent_entity_col, dod.exception_flg =
    tst_dmcoldoc->dld[d.seq].exception_flg, dod.defining_attribute_ind = tst_dmcoldoc->dld[d.seq].
    defining_attribute_ind,
    dod.merge_updateable_ind = tst_dmcoldoc->dld[d.seq].merge_updateable_ind, dod.nls_col_ind =
    tst_dmcoldoc->dld[d.seq].nls_col_ind, dod.absolute_date_ind = tst_dmcoldoc->dld[d.seq].
    absolute_date_ind
   PLAN (d)
    JOIN (dod
    WHERE (tst_dmcoldoc->dld[d.seq].table_name=dod.table_name)
     AND (tst_dmcoldoc->dld[d.seq].column_name=dod.column_name))
   WITH nocounter, outerjoin = d, dontexist
  ;end insert
  FREE RECORD tst_dmtsprec
  RECORD tst_dmtsprec(
    1 spr[*]
      2 table_name = vc
      2 precedence = i4
  ...
  SET stat = alterlist(tst_dmtsprec->spr,4)
  SET spr_cnt = 0
  SET spr_cnt = (spr_cnt+ 1)
  SET tst_dmtsprec->spr[spr_cnt].table_name = build(concat(obj_static_name,"TESTA"))
  SET tst_dmtsprec->spr[spr_cnt].precedence = 1
  SET tst_dmtsprec->spr[spr_cnt].data_tablespace = concat("D",tspace_suffix)
  SET tst_dmtsprec->spr[spr_cnt].data_extent_size = 163840
  SET tst_dmtsprec->spr[spr_cnt].index_tablespace = concat("I",tspace_suffix)
  SET tst_dmtsprec->spr[spr_cnt].index_extent_size = 163840
  SET tst_dmtsprec->spr[spr_cnt].long_tablespace = concat("D",tspace_suffix)
  SET tst_dmtsprec->spr[spr_cnt].long_extent_size = 163840
  SET tst_dmtsprec->spr[spr_cnt].updt_cnt = 0
  SET spr_cnt = (spr_cnt+ 1)
  SET tst_dmtsprec->spr[spr_cnt].table_name = build(concat(obj_static_name,"TESTA","9974"))
  SET tst_dmtsprec->spr[spr_cnt].precedence = 1
  SET tst_dmtsprec->spr[spr_cnt].data_tablespace = concat("D",tspace_suffix)
  SET tst_dmtsprec->spr[spr_cnt].data_extent_size = 163840
  SET tst_dmtsprec->spr[spr_cnt].index_tablespace = concat("I",tspace_suffix)
  ...
  SET tst_dmtsprec->spr[spr_cnt].updt_cnt = 0
  SET spr_cnt = (spr_cnt+ 1)
  SET tst_dmtsprec->spr[spr_cnt].table_name = build(concat(obj_static_name,"TESTB"))
  SET tst_dmtsprec->spr[spr_cnt].precedence = 1
  SET tst_dmtsprec->spr[spr_cnt].data_tablespace = concat("D",tspace_suffix)
  SET tst_dmtsprec->spr[spr_cnt].data_extent_size = 163840
  SET tst_dmtsprec->spr[spr_cnt].index_tablespace = concat("I",tspace_suffix)
  ...
  SET tst_dmtsprec->spr[spr_cnt].updt_cnt = 0
  SET spr_cnt = (spr_cnt+ 1)
  SET tst_dmtsprec->spr[spr_cnt].table_name = build(concat(obj_static_name,"TESTB","9975"))
  SET tst_dmtsprec->spr[spr_cnt].precedence = 1
  SET tst_dmtsprec->spr[spr_cnt].data_tablespace = concat("D",tspace_suffix)
  SET tst_dmtsprec->spr[spr_cnt].data_extent_size = 163840
  SET tst_dmtsprec->spr[spr_cnt].index_tablespace = concat("I",tspace_suffix)
  ...
  SET tst_dmtsprec->spr[spr_cnt].long_extent_size = 163840
  SET tst_dmtsprec->spr[spr_cnt].updt_cnt = 0
  SET stat = alterlist(tst_dmtsprec->spr,spr_cnt)
  SET size_tst_dmtsprec = size(tst_dmtsprec->spr,5)
  INSERT  FROM dmtsprec s,
    (dummyt d  WITH seq = size_tst_dmtsprec)
   SET s.table_name = tst_dmtsprec->spr[d.seq].table_name, s.precedence = tst_dmtsprec->spr[d.seq].
    precedence, s.data_tablespace = tst_dmtsprec->spr[d.seq].data_tablespace,
    s.data_extent_size = tst_dmtsprec->spr[d.seq].data_extent_size, s.index_tablespace = tst_dmtsprec
    ->spr[d.seq].index_tablespace, s.index_extent_size = tst_dmtsprec->spr[d.seq].index_extent_size,
    s.long_tablespace = tst_dmtsprec->spr[d.seq].long_tablespace, s.long_extent_size = tst_dmtsprec->
    spr[d.seq].long_extent_size, s.updt_cnt = tst_dmtsprec->spr[d.seq].updt_cnt
   PLAN (d)
    JOIN (s
    WHERE (tst_dmtsprec->spr[d.seq].table_name=s.table_name)
     AND (tst_dmtsprec->spr[d.seq].precedence=s.precedence))
   WITH nocounter, outerjoin = d, dontexist
  ;end insert
 ENDIF
 FREE RECORD tst_dmtable
 ...
 FREE RECORD tst_dmtsprec
#exit_program
 SET dm_err->eproc = "DM2_ADD_DTL_TEST_OBJECTS COMPLETED SUCCESSFULLY"
 CALL final_disp_msg("dm2_add_objs")
END GO
