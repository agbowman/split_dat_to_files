CREATE PROGRAM bhs_rw_get_powerform_data:dba
 PROMPT
  "     DCP_FORMS_REF_ID: " = "",
  "DCP_SECTION_REF_ID(S): " = "",
  "Complete RTF Document: " = 1,
  "            PERSON_ID: " = 0.00,
  "            ENCNTR_ID: " = 0.00
 RECORD run_info(
   1 dcp_forms_ref_id = f8
   1 dcp_forms_act_id = f8
   1 form_disp = vc
   1 s_cnt = i4
   1 sections[*]
 ...
 IF (cnvtreal( $1) > 0.00)
  SET run_info->dcp_forms_ref_id = cnvtreal( $1)
 ELSE
  CALL echo("No DCP_FORMS_REF_ID given. Exitting Script")
  GO TO exit_script
 ENDIF
 IF (cnvtreal( $2) > 0.00)
  SET run_info->s_cnt = 1
  SET stat = alterlist(run_info->sections,1)
  SET run_info->sections[1].dcp_section_ref_id = cnvtreal( $2)
 ELSE
  DECLARE subslot = i4 WITH noconstant(1)
  WHILE (reflect(parameter(2,subslot)) > " "
   AND cnvtreal(parameter(2,subslot)) > 0.00)
    SET run_info->s_cnt = (run_info->s_cnt+ 1)
    SET stat = alterlist(run_info->sections,run_info->s_cnt)
    SET run_info->sections[run_info->s_cnt].dcp_section_ref_id = cnvtreal(parameter(2,subslot))
    SET subslot = (subslot+ 1)
  ENDWHILE
  FREE SET subslot
 ENDIF
 IF (cnvtint( $3)=1)
  SET run_info->complete_rtf = 1
 ELSE
  SET run_info->complete_rtf = 0
 ENDIF
 IF (cnvtreal( $4)=0.00)
  CALL echo("No PERSON_ID given. Exitting Script")
  GO TO exit_script
 ELSE
  SET run_info->person_id = cnvtreal( $4)
 ENDIF
 IF (cnvtreal( $5)=0.00)
  CALL echo("No ENCNTR_ID given. Exitting Script")
  GO TO exit_script
 ELSE
  SET run_info->encntr_id = cnvtreal( $5)
 ENDIF
 IF (reflect(parameter(6,0)) > " ")
  IF (cnvtreal(parameter(6,0)) > 0.00)
   SET run_info->dcp_forms_act_id = cnvtreal(parameter(6,0))
  ENDIF
 ENDIF
 ...
 DECLARE cs18189_primaryeventid_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",18189,
   "PRIMARYEVENTID"))
 DECLARE cs6004_ordered_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6004,"ORDERED"))
 DECLARE cs106_pharmacy_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",106,"PHARMACY"))
 DECLARE cs12038_recorder_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",12038,"RECORDER"))
 DECLARE cs12038_responsible_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",12038,
   "RESPONSIBLEMANAGING"))
 DECLARE cs48_active_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",48,"ACTIVE"))
 IF (validate(cs8_inerror_cd)=0)
  DECLARE cs8_inerror_cd = f8 WITH constant(uar_get_code_by("MEANING",8,"INERROR"))
 ENDIF
 IF (validate(cs53_grp_cd)=0)
  DECLARE cs53_grp_cd = f8 WITH constant(uar_get_code_by("MEANING",53,"GRP"))
 ENDIF
 DECLARE cs120_ocfcompression_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",120,"OCFCOMPRESSION"
   ))
 DECLARE cs53_date_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",53,"DATE"))
 DECLARE cs53_num_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",53,"NUM"))
 DECLARE cs53_txt_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",53,"TXT"))
 DECLARE tmp_tag = vc
 DECLARE tmp_s_cnt = i4
 SELECT DISTINCT INTO "NL:"
  ...
  FROM dcp_forms_activity dfa,
   ...
   dcp_forms_def dfd
  PLAN (dfa
   WHERE (run_info->dcp_forms_ref_id=dfa.dcp_forms_ref_id)
    AND (dfa.encntr_id=run_info->encntr_id)
    AND dfa.form_status_cd != cs8_inerror_cd
    AND (((run_info->dcp_forms_act_id=0.00)) OR ((run_info->dcp_forms_act_id > 0.00)
    AND (run_info->dcp_forms_act_id=dfa.dcp_forms_activity_id))) )
   JOIN (dfac
   WHERE dfa.dcp_forms_activity_id=dfac.dcp_forms_activity_id
    AND dfac.component_cd=cs18189_primaryeventid_cd)
   JOIN (ce
   ...
   WHERE cnvtreal(ce.collating_seq)=dsr.dcp_section_ref_id
    ...
    AND (((run_info->s_cnt=0)) OR (expand(tmp_s_cnt,1,run_info->s_cnt,dsr.dcp_section_ref_id,run_info
    ->sections[tmp_s_cnt].dcp_section_ref_id))) )
   JOIN (dfr2
  ...
  HEAD dfa.dcp_forms_activity_id
   ref_info->form_desc = dfa.description
  HEAD dsr.definition
 ...
 SELECT INTO "NL:"
  ...
  HEAD ce1.event_id
   ...
   IF (ce1.event_class_cd=cs53_txt_cd)
    IF (csr.event_id > 0.0)
     act_info->ctrls[c_cnt].event_tag = trim(csr.string_result_text,3)
    ELSE
     act_info->ctrls[c_cnt].event_tag = trim(ce1.result_val,3)
    ENDIF
   ELSEIF (ce1.event_class_cd=cs53_num_cd)
    ...
    ELSE
     act_info->ctrls[c_cnt].event_tag = trim(ce1.result_val,3)
    ENDIF
   ELSEIF (ce1.event_class_cd=cs53_date_cd)
   ...
   IF (cb.event_id > 0.0)
    ...
    IF (cb.compression_cd=cs120_ocfcompression_cd)
  ...
  DETAIL
   IF (ccr.event_id > 0.0)
    ...
    act_info->ctrls[c_cnt].nomen[n_cnt].nomenclature = trim(n.source_string,3)
   ENDIF
  FOOT  ce1.event_id
   act_info->ctrls[c_cnt].nomen_cnt = n_cnt
   IF (n_cnt > 0)
    FOR (n = 1 TO n_cnt)
      tmp_tag = build2(tmp_tag,", ",trim(act_info->ctrls[c_cnt].nomen[n].nomenclature,3))
    ENDFOR
    tmp_tag = trim(substring(3,(size(tmp_tag) - 2),tmp_tag),3)
    IF ((tmp_tag != act_info->ctrls[c_cnt].event_tag))
     IF (trim(act_info->ctrls[c_cnt].event_tag,3) <= " ")
      act_info->ctrls[c_cnt].event_tag = trim(tmp_tag,3)
     ELSE
      act_info->ctrls[c_cnt].event_tag = build2(tmp_tag,", ",trim(act_info->ctrls[c_cnt].event_tag,3)
       )
     ENDIF
    ENDIF
   ENDIF
...
#get_grids
 ...
 SELECT INTO "NL:"
  ...
  PLAN (d1
   ...
   JOIN (ce1
   WHERE (ref_info->sections[d1.seq].event_id=ce1.parent_event_id)
    AND (ce1.event_cd=ref_info->sections[d1.seq].inputs[d2.seq].grid_event_cd)
    AND ce1.valid_until_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00"))
   JOIN (ce2
   WHERE outerjoin(ce1.event_id)=ce2.parent_event_id
    AND ce2.valid_until_dt_tm=outerjoin(cnvtdatetime("31-DEC-2100 00:00:00")))
   JOIN (pr1
   WHERE outerjoin(ce2.performed_prsnl_id)=pr1.person_id)
   JOIN (cen
   WHERE outerjoin(ce2.event_id)=cen.event_id
   ...
   JOIN (pr2
   WHERE outerjoin(ce3.performed_prsnl_id)=pr2.person_id)
   JOIN (ccr1
   WHERE outerjoin(ce2.event_id)=ccr1.event_id
    AND ccr1.valid_until_dt_tm=outerjoin(cnvtdatetime("31-DEC-2100 00:00:00")))
   JOIN (ccr2
   WHERE outerjoin(ce3.event_id)=ccr2.event_id
   ...
   JOIN (cdr
   WHERE outerjoin(ce3.event_id)=cdr.event_id
    AND cdr.valid_until_dt_tm=outerjoin(cnvtdatetime("31-DEC-2100 00:00:00")))
   JOIN (csr
   WHERE outerjoin(ce3.event_id)=csr.event_id
    AND csr.valid_until_dt_tm=outerjoin(cnvtdatetime("31-DEC-2100 00:00:00")))
  HEAD REPORT
  ...
  HEAD ce1.event_id
   ...
   d2.seq].input_type,
   act_info->grids[g_cnt].lvl2_event_id = ce1.event_id, act_info->grids[g_cnt].grid_desc =
   uar_get_code_display(ce1.event_cd), r_cnt = 0,
   c_cnt = 0, n_cnt = 0
  HEAD ce2.event_id
   r_cnt = (r_cnt+ 1), stat = alterlist(act_info->grids[g_cnt].rows,r_cnt), act_info->grids[g_cnt].
   rows[r_cnt].row_event_id = ce2.event_id,
   act_info->grids[g_cnt].rows[r_cnt].row_event_cd = ce2.event_cd, act_info->grids[g_cnt].rows[r_cnt]
   .task_assay_cd = ce2.task_assay_cd, act_info->grids[g_cnt].rows[r_cnt].row_desc =
   uar_get_code_display(ce2.event_cd),
   act_info->grids[g_cnt].rows[r_cnt].row_seq = cnvtint(ce2.collating_seq), c_cnt = 0, n_cnt = 0
  HEAD ce3.event_id
   IF (ce3.event_id > 0.00)
    tmp_c = 0, stat = locateval(tmp_c,1,c_cnt,ce3.event_id,act_info->grids[g_cnt].rows[r_cnt].cols[
     tmp_c].col_event_id)
    IF ((act_info->grids[g_cnt].rows[r_cnt].cols[tmp_c].col_event_id != ce3.event_id))
     c_cnt = (c_cnt+ 1), stat = alterlist(act_info->grids[g_cnt].rows[r_cnt].cols,c_cnt), act_info->
     grids[g_cnt].rows[r_cnt].cols[c_cnt].col_event_id = ce3.event_id,
     act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].col_event_cd = ce3.event_cd, act_info->grids[
     g_cnt].rows[r_cnt].cols[c_cnt].task_assay_cd = ce3.task_assay_cd, act_info->grids[g_cnt].rows[
     r_cnt].cols[c_cnt].col_desc = uar_get_code_display(ce3.event_cd),
     act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].col_seq = cnvtint(ce3.collating_seq), act_info->
     grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag = ce3.event_tag, act_info->grids[g_cnt].rows[
     r_cnt].cols[c_cnt].event_class_cd = ce3.event_class_cd,
     act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].event_end_dt_tm = ce3.event_end_dt_tm, act_info->
     grids[g_cnt].rows[r_cnt].cols[c_cnt].valid_until_dt_tm = ce3.valid_until_dt_tm, act_info->grids[
     g_cnt].rows[r_cnt].cols[c_cnt].prsnl_name = pr2.name_full_formatted
     IF (ce3.event_class_cd=cs53_txt_cd)
      IF (csr.event_id > 0.0)
       act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag = csr.string_result_text
      ELSE
       act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag = ce3.result_val
      ENDIF
     ELSEIF (ce3.event_class_cd=cs53_num_cd)
      IF (ce2.result_units_cd > 0.0)
       act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag = concat(trim(ce3.result_val,3)," ",
        uar_get_code_display(ce3.result_units_cd))
      ELSE
       act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag = ce3.result_val
      ENDIF
     ELSEIF (ce3.event_class_cd=cs53_date_cd)
      IF (cdr.date_type_flag=0)
   ...
   ELSE
    ...
    ].rows[r_cnt].cols[c_cnt].col_event_id = ce2.event_id,
    act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].col_event_cd = ce2.event_cd, act_info->grids[g_cnt
    ].rows[r_cnt].cols[c_cnt].task_assay_cd = ce2.task_assay_cd, act_info->grids[g_cnt].rows[r_cnt].
    cols[c_cnt].col_desc = uar_get_code_display(ce2.event_cd),
    act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].col_seq = cnvtint(ce2.collating_seq), act_info->
    grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag = ce2.event_tag, act_info->grids[g_cnt].rows[r_cnt
    ].cols[c_cnt].event_end_dt_tm = ce2.event_end_dt_tm,
    act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].valid_until_dt_tm = ce2.valid_until_dt_tm,
  ...
  FOOT  ce3.event_id
   IF ((act_info->grids[g_cnt].rows[r_cnt].cols[tmp_c].col_event_id != ce3.event_id))
    ...
    IF (n_cnt > 0)
     FOR (n = 1 TO n_cnt)
       IF (tmp_tag=" ")
        tmp_tag = trim(act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].nomen[n].nomenclature,3)
       ELSE
        tmp_tag = build2(tmp_tag,", ",trim(act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].nomen[n].
          nomenclature,3))
       ENDIF
     ENDFOR
     IF ((tmp_tag != act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag))
      IF (trim(act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag,3) <= " ")
       act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag = trim(tmp_tag,3)
      ELSE
       act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag = build2(tmp_tag,", ",trim(act_info->
         grids[g_cnt].rows[r_cnt].cols[c_cnt].event_tag,3))
      ENDIF
     ENDIF
    ENDIF
   ENDIF
  ...
  FOOT  ce2.event_id
   IF (cen.ce_event_note_id > 0.00
    AND lb.parent_entity_id > 0.00)
    blob_in = fillstring(32000," "), blob_out = fillstring(32000," "), blob_out2 = fillstring(32000,
     " "),
    blob_out3 = fillstring(32000," "), blob_ret_len = 0
    IF (cen.compression_cd=cs120_ocfcompression_cd)
    ...
    ENDIF
    CALL uar_rtf(blob_out2,textlen(blob_out2),blob_out3,32000,32000,0), c_cnt = (c_cnt+ 1), act_info
    ->grids[g_cnt].rows[r_cnt].col_cnt = c_cnt,
    stat = alterlist(act_info->grids[g_cnt].rows[r_cnt].cols,c_cnt), act_info->grids[g_cnt].rows[
    r_cnt].cols[c_cnt].col_desc = "Comments", act_info->grids[g_cnt].rows[r_cnt].cols[c_cnt].
    event_tag = trim(blob_out3,3)
   ENDIF
 ...
 SELECT INTO "NL:"
  ...
  PLAN (a
   ...
   WHERE outerjoin(a.allergy_id)=ac.allergy_id
    ...
    AND ac.active_status_cd=outerjoin(cs48_active_cd)
   ...
   WHERE outerjoin(a.allergy_id)=r.allergy_id
    ...
    AND r.active_status_cd=outerjoin(cs48_active_cd)
 ...
 SELECT INTO "NL:"
  ...
  PLAN (p
   WHERE (run_info->person_id=p.person_id)
    AND p.active_ind=1
    AND p.active_status_cd=cs48_active_cd
    AND p.end_effective_dt_tm=cnvtdatetime("31-DEC-2100 23:59:59"))
   JOIN (ppr1
   WHERE outerjoin(p.problem_id)=ppr1.problem_id
    AND ppr1.problem_reltn_cd=outerjoin(cs12038_responsible_cd)
    AND ppr1.active_ind=outerjoin(1)
    AND ppr1.active_status_cd=outerjoin(cs48_active_cd)
   ...
   WHERE outerjoin(p.problem_id)=ppr2.problem_id
    AND ppr2.problem_reltn_cd=outerjoin(cs12038_recorder_cd)
    AND ppr2.active_ind=outerjoin(1)
    AND ppr2.active_status_cd=outerjoin(cs48_active_cd)
   ...
   WHERE outerjoin(p.problem_id)=pc.problem_id
    ...
    AND pc.active_status_cd=outerjoin(cs48_active_cd)
 ...
 SELECT INTO "NL:"
  ...
  PLAN (o
   WHERE (o.encntr_id=run_info->encntr_id)
    ...
    AND o.order_status_cd=cs6004_ordered_cd)
   JOIN (oa
 ...
 SELECT INTO "NL:"
  ...
  HEAD REPORT
   ...
   MACRO (write_ctrl_entry)
    ...
    ENDIF
    wrote_entry_ind = 1, cell_1_value = trim(act_info->ctrls[tmp_slot_ind].ctrl_desc,3), cell_2_value
     = trim(act_info->ctrls[tmp_slot_ind].event_tag,3),
    write_tbl_row
   ENDMACRO
   ,
    ...
    ELSE
     ...
     FOR (a = 1 TO act_info->allergy_cnt)
       ...
       reply->text = build2(reply->text,beg_italize," "), reply->text = build2(reply->text,trim(
         cnvtstring(a))," out of ",trim(cnvtstring(act_info->allergy_cnt)),":"), reply->text = build2
       (reply->text,end_italize," ",end_line," "),
   ...
   MACRO (write_problems)
    ...
    ELSE
     reply->text = build2(reply->text,beg_underline," "), reply->text = build2(reply->text,
      "Problems (Total of ",trim(cnvtstring(act_info->problem_cnt)),"):"), reply->text = build2(reply
     ...
     FOR (p = 1 TO act_info->problem_cnt)
       ...
       ENDIF
       reply->text = build2(reply->text,beg_italize," "), reply->text = build2(reply->text,trim(
         cnvtstring(p))," out of ",trim(cnvtstring(act_info->problem_cnt)),":"), reply->text = build2
       (reply->text,end_italize," ",end_line," "),
      ...
      FOR (a = 1 TO act_info->med_cnt)
        IF ((act_info->meds[a].type_ind="M"))
         ...
         ELSE
          reply->text = build2(reply->text,end_line," ")
         ENDIF
         temp_meds_cnt = (temp_meds_cnt+ 1), reply->text = build2(reply->text,beg_italize," "), reply
         ->text = build2(reply->text,trim(cnvtstring(temp_meds_cnt))," out of ",trim(cnvtstring(
            admin_meds_cnt)),":"),
         reply->text = build2(reply->text,end_italize," ",end_line," "), cell_1_value = trim(act_info
          ->meds[a].ordered_as_mnemonic), merge_cell_ind = 1,
      ...
      FOR (p = 1 TO act_info->med_cnt)
        IF ((act_info->meds[p].type_ind="P"))
         ...
         ->text = build2(reply->text,trim(cnvtstring(temp_meds_cnt))," out of ",trim(cnvtstring(
            prescribe_meds_cnt)),":"),
         reply->text = build2(reply->text,end_italize," ",end_line," "), cell_1_value = trim(act_info
          ->meds[p].ordered_as_mnemonic), merge_cell_ind = 1,
   ...
   ENDMACRO
  DETAIL
   IF ((run_info->complete_rtf=1))
    reply->text = build2(beg_doc," ",end_para," ",beg_text,
     " ",end_line,char(10),char(13))
   ELSE
    reply->text = build2(reply->text,char(10),char(13))
   ENDIF
   reply->text = build2(reply->text,"\b ",ref_info->form_desc,"\b0",end_line,
    char(10),char(13))
   ...
   ELSE
    reply->text = build2(reply->text," ",end_para," ",end_line,
     char(10),char(13))
   ENDIF
