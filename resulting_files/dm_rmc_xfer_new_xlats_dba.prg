CREATE PROGRAM dm_rmc_xfer_new_xlats:dba
 ...
 SUBROUTINE xta_dbl_hlv(i_gap)
   ...
   RETURN(xdh_gap)
 END ;Subroutine
 IF (check_logfile("DM_RMC_XNX",".log","DM_RMC_XNX LogFile")=0)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF ((validate(request->src_env_id,- (99))=- (99)))
  FREE RECORD request
  RECORD request(
    1 src_env_id = f8
    1 trg_env_id = f8
  ...
  SET request->src_env_id =  $1
  SET request->trg_env_id =  $2
  SET request->new_trg_env_id =  $3
  SET request->dm_rdds_chain_id =  $4
 ENDIF
 DECLARE xnx_ret = vc
 DECLARE xnx_row_cnt = i4
 DECLARE xnx_logging_val = i4
 DECLARE xnx_continue_ind = i2
 DECLARE xnx_row_qual = i4
 DECLARE xnx_qual = f8
 DECLARE xnx_low = f8
 DECLARE xnx_high = f8
 DECLARE xnx_max = f8
 DECLARE xnx_gap = f8
 DECLARE xnx_info_domain = vc
 DECLARE xnx_info_domain_done = vc
 SET xnx_logging_val = 202
 SET xnx_ret = dm_check_syn(null)
 IF (xnx_ret="F")
  SET dm_err->eproc = "Error occured while checking synonyms"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Obtaining count of rows"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  y = count(*)
  FROM dm_rdds_dmt drd
  WHERE (drd.dm_rdds_chain_id=request->dm_rdds_chain_id)
   AND (drd.env_source_id=request->src_env_id)
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Updating dm_rdds_chain row to state_flag = 2"
 CALL disp_msg(" ",dm_err->logfile,0)
 UPDATE  FROM dm_rdds_chain drc
  SET drc.state_flag = 2, drc.rows_to_process = xnx_row_cnt, drc.rows_processed = 0
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (di.info_number < 2000000000)
    xnx_row_qual = di.info_number
   ELSE
    xnx_row_qual = 2000000000
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO end_program
 ENDIF
 IF (((curqual=0) OR (xnx_row_qual < 100)) )
  SET xnx_row_qual = 100000
 ENDIF
 SET xnx_continue_ind = 0
 WHILE (xnx_continue_ind=0)
   SET dm_err->eproc = concat("Cleaning up translations found in current domain: ",cnvtstring(request
     ->new_trg_env_id))
   CALL disp_msg(" ",dm_err->logfile,0)
   DELETE  FROM dm_merge_translate dmt
    WHERE (dmt.env_source_id=request->src_env_id)
     AND (dmt.env_target_id=request->new_trg_env_id)
    WITH maxqual(dmt,value(xnx_row_qual)), nocounter
   ;end delete
   SET xnx_qual = curqual
   IF (check_error(dm_err->eproc)=1)
    SET xnx_row_qual = xta_dbl_hlv(cnvtreal(xnx_row_qual))
    IF (xnx_row_qual <= 0)
     GO TO end_program
    ENDIF
   ELSE
    IF (xnx_qual < xnx_row_qual)
     SET xnx_continue_ind = 1
    ENDIF
    COMMIT
   ENDIF
   SELECT INTO "nl:"
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO end_program
   ENDIF
   IF (xnx_logging_val=302)
    GO TO end_program
   ENDIF
 ENDWHILE
 SELECT INTO "nl:"
  y = max(drd.dm_rdds_dmt_id)
  FROM dm_rdds_dmt drd
  WHERE (drd.dm_rdds_chain_id=request->dm_rdds_chain_id)
   AND (drd.env_source_id=request->src_env_id)
   AND (drd.env_target_id=request->trg_env_id)
 ...
 IF (((curqual=0) OR (xnx_row_qual < 100)) )
  SET xnx_row_qual = 100000
 ENDIF
 SET xnx_gap = xnx_row_qual
 SET xnx_high = (xnx_low+ xnx_gap)
 SET xnx_continue_ind = 0
 WHILE (xnx_continue_ind=0)
   SET dm_err->eproc = concat("Inserting translations in dm_merge_translate for : ",cnvtstring(
     request->new_trg_env_id))
   CALL disp_msg(" ",dm_err->logfile,0)
   INSERT  FROM dm_merge_translate dmt
    ...
    dmt.updt_id, dmt.updt_task, dmt.updt_cnt,
    dmt.updt_applctx, dmt.updt_dt_tm)(SELECT
     drd.from_value, drd.table_name, drd.env_source_id,
     request->new_trg_env_id, drd.to_value, 11,
     reqinfo->updt_id, reqinfo->updt_task, 0,
     reqinfo->updt_applctx, sysdate
     FROM dm_rdds_dmt drd
     WHERE (drd.env_source_id=request->src_env_id)
      AND (drd.env_target_id=request->trg_env_id)
      AND (drd.dm_rdds_chain_id=request->dm_rdds_chain_id)
      AND drd.dm_rdds_dmt_id >= value(xnx_low)
      AND drd.dm_rdds_dmt_id < value(xnx_high))
    WITH nocounter
   ;end insert
   SET xnx_qual = curqual
   IF (check_error(dm_err->eproc)=1)
    SET xnx_gap = xta_dbl_hlv(xnx_gap)
    IF ((xnx_gap=- (1)))
     GO TO end_program
    ENDIF
    SET xnx_high = (xnx_low+ xnx_gap)
   ELSE
    ...
    IF (xnx_high > xnx_max)
     SET xnx_continue_ind = 1
    ELSE
     SET xnx_low = xnx_high
     SET xnx_high = (xnx_high+ xnx_gap)
    ENDIF
   ENDIF
   IF (xnx_qual > 0)
    ...
    CALL disp_msg(" ",dm_err->logfile,0)
    UPDATE  FROM dm_rdds_chain drc
     SET drc.rows_processed = (drc.rows_processed+ xnx_qual), drc.action_dt_tm = sysdate
     WHERE (drc.dm_rdds_chain_id=request->dm_rdds_chain_id)
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Clean up seqmatch rows in current domain"
 CALL disp_msg(" ",dm_err->logfile,0)
 DELETE  FROM dm_info di
  WHERE di.info_domain="MERGE*SEQMATCH*"
   AND di.info_name IN (
  (SELECT DISTINCT
   dcd.sequence_name
 ...
 DECLARE xnx_info_domain = vc
 DECLARE xnx_info_domain_done = vc
 SET xnx_info_domain = concat("MERGE",trim(cnvtstring(request->src_env_id)),trim(cnvtstring(request->
    new_trg_env_id)),"SEQMATCH")
 SET xnx_info_domain_done = concat("MERGE",trim(cnvtstring(request->src_env_id)),trim(cnvtstring(
    request->new_trg_env_id)),"SEQMATCHDONE2")
 SET dm_err->eproc = "Inserting seqmatch rows in current domain"
 CALL disp_msg(" ",dm_err->logfile,0)
 INSERT  FROM dm_info
  (info_name, info_domain, info_char,
  info_number)(SELECT DISTINCT
   dcd.sequence_name, xnx_info_domain, "DM_RMC_XFER_NEW_XLATS",
   0
   FROM dm_columns_doc dcd,
    dm_tables_doc dtd
   WHERE ((dtd.reference_ind=1) OR (dtd.table_name IN (
 ...
 ELSE
  COMMIT
 ENDIF
 SET dm_err->eproc = "Inserting seqmatch done rows in current domain"
 CALL disp_msg(" ",dm_err->logfile,0)
 INSERT  FROM dm_info
  (info_name, info_domain, info_char,
  info_number)(SELECT DISTINCT
   dcd.sequence_name, xnx_info_domain_done, "DM_RMC_XFER_NEW_XLATS",
   0
   FROM dm_columns_doc dcd,
    dm_tables_doc dtd
   WHERE ((dtd.reference_ind=1) OR (dtd.table_name IN (
 ...
 ELSE
  COMMIT
 ENDIF
 SET xnx_logging_val = 102
#end_program
 IF ((request->dm_rdds_chain_id > 0))
  UPDATE  FROM dm_rdds_chain drc
   SET drc.state_flag = xnx_logging_val, drc.error_msg = substring(1,255,dm_err->emsg), drc
