CREATE PROGRAM alert_link_add_context:dba
 PROMPT
  "Key: " = "",
  "Value:" = ""
  WITH context_key, context_value
 ...
 SUBROUTINE check_log_level(arg_log_level)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "alert_link_add_context"
 DECLARE setcontext(null) = null WITH protect
 CALL log_message(concat("Begin script alert_link_add_context: ",log_program_name),log_level_debug)
 CALL setcontext(null)
 CALL log_message(concat("Exit script alert_link_add_context: ",log_program_name),log_level_debug)
 SUBROUTINE setcontext(null)
   CALL log_message("In setContext()",log_level_debug)
   IF (validate(ejs_request) > 0)
    CALL log_message(build("Key: ", $CONTEXT_KEY),log_level_debug)
    CALL log_message(build("Value: ", $CONTEXT_VALUE),log_level_debug)
    SET rec_pos = (size(ejs_request->context,5)+ 1)
    SET stat = alterlist(ejs_request->context,rec_pos)
    SET ejs_request->context[rec_pos].ckey =  $CONTEXT_KEY
    SET ejs_request->context[rec_pos].cvalue =  $CONTEXT_VALUE
    SET retval = 100
   ELSE
    CALL log_message("ejs_request structure not defined.",log_level_debug)
    SET retval = - (1)
   ENDIF
   CALL log_message("exit setContext()",log_level_debug)
 END ;Subroutine
