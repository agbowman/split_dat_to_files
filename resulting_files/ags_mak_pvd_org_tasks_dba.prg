CREATE PROGRAM ags_mak_pvd_org_tasks:dba
 CALL echo("***")
 CALL echo("***   BEG AGS_MAK_PVD_ORG_TASKS")
 CALL echo("***")
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_MAK_PVD_ORG_TASKS"
 CALL echo("***")
 ...
 IF (((min_batch_id < 1) OR (max_batch_id < 1)) )
  ...
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "No ORG task found"
  GO TO exit_script
 ENDIF
 CALL echo("***")
 CALL echo("***   Insert ORG Task")
 CALL echo("***")
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 INSERT  FROM ags_task t
  SET t.ags_task_id = seq(gs_seq,nextval), t.ags_job_id = data_info->ags_job_id, t.task_type = "ORG",
   t.batch_program = "ags_load_org", t.batch_start_id = min_batch_id, t.batch_end_id = max_batch_id,
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_MAK_PVD_ORG_TASKS"
 CALL echo("***")
 CALL echo("***   END AGS_MAK_PVD_ORG_TASKS")
 CALL echo("***")
