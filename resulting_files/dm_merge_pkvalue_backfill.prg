CREATE PROGRAM dm_merge_pkvalue_backfill
 ...
 DECLARE dmpb_max_log = f8
 DECLARE dmpb_min_log = f8
 DECLARE batch_stop = c1
 DECLARE dmpb_cnt = i4
 DECLARE dmpb_loop = i4
 DECLARE dmpb_sa_run_ind = i2
 DECLARE dmpb_instance = i4
 DECLARE dmpb_high_val = i4
 DECLARE dmpb_colon_pos = i4
 DECLARE dmpb_old_dt_tm_ind = i2
 DECLARE dmpb_qual_ind = i2
 DECLARE dmpb_batch = i4 WITH constant(2000)
 ...
 IF (check_logfile("dm_bkfill_pkval_",".log","DM_bkfill_pkval LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_main
 ENDIF
 SET dm_err->eproc = "Starting dm_merge_pkvalue_backfill"
 CALL disp_msg("",dm_err->logfile,0)
 IF ((readme_data->readme_id > 0))
  SET dmpb_sa_run_ind = 0
  DELETE  FROM dm_info
   WHERE info_domain="DATA MANAGEMENT"
    AND info_name="BACKFILL INSTANCE:*"
   WITH nocounter
  ;end delete
  DELETE  FROM dm_info
   WHERE info_domain="DATA MANAGEMENT"
    AND info_name="BACKFILL BATCH"
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Error clearing out dm_info batch rows"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  COMMIT
 ELSE
  SET dmpb_sa_run_ind = 1
 ENDIF
 SET dmpb_cnt = 0
 IF (dmpb_sa_run_ind=1)
  SELECT INTO "NL:"
   FROM dm_info d
   WHERE d.info_domain="DATA MANAGEMENT"
    AND d.info_name="BACKFILL INSTANCE:*"
   DETAIL
    dmpb_cnt += 1, stat = alterlist(instance_val->qual,dmpb_cnt), instance_val->qual[dmpb_cnt].name
     = d.info_name
   WITH nocounter, forupdatewait(d)
  ;end select
  SET dmpb_high_val = 0
  FOR (dmpb_loop = 1 TO dmpb_cnt)
    SET dmpb_colon_pos = 0
    SET dmpb_colon_pos = findstring(":",instance_val->qual[dmpb_loop].name,1,0)
    IF (dmpb_colon_pos != 0)
     SET instance_val->qual[dmpb_loop].num = cnvtint(substring((dmpb_colon_pos+ 1),(size(instance_val
        ->qual[dmpb_loop].name,1) - dmpb_colon_pos),instance_val->qual[dmpb_loop].name))
     IF ((dmpb_high_val < instance_val->qual[dmpb_loop].num))
      SET dmpb_high_val = instance_val->qual[dmpb_loop].num
     ENDIF
    ENDIF
  ENDFOR
  SET dmpb_instance = (dmpb_high_val+ 1)
  INSERT  FROM dm_info d
   SET d.info_domain = "DATA MANAGEMENT", d.info_name = concat("BACKFILL INSTANCE:",cnvtstring(
      dmpb_instance)), d.updt_dt_tm = cnvtdatetime(sysdate),
    d.info_number = 0
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
 ...
 SELECT INTO "nl:"
  ...
  FROM dm_chg_log d
  DETAIL
   rptbuf = reportinfo(2), dmpb_max_log = cnvtreal(substring(1,15,rptbuf))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading max log_id from DM_CHG_LOG"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
 ...
 IF (dmpb_max_log=0)
  ...
  SET readme_data->message = "DM_CHG_LOG doesn't contain any rows to change."
  GO TO exit_script
 ENDIF
#redo
 SET dmpb_cnt = 0
#try_again
 ...
 IF (dmpb_cnt < 5)
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DATA MANAGEMENT"
    AND d.info_name="BACKFILL BATCH"
   DETAIL
    dmpb_min_log = d.info_number
   WITH nocounter, forupdatewait(d)
  ;end select
  IF (curqual=0)
   SELECT INTO "nl:"
    ...
    FROM dm_chg_log d
    DETAIL
     rptbuf = reportinfo(2), dmpb_min_log = cnvtreal(substring(1,15,rptbuf))
    WITH nocounter
   ;end select
   INSERT  FROM dm_info d
    SET d.info_domain = "DATA MANAGEMENT", d.info_name = "BACKFILL BATCH", d.info_number =
     dmpb_min_log,
  ...
  ELSE
   UPDATE  FROM dm_info d
    SET d.info_number = (dmpb_min_log+ dmpb_batch), d.updt_dt_tm = cnvtdatetime(sysdate)
    WHERE d.info_name="BACKFILL BATCH"
   ...
   IF (dmpb_sa_run_ind=1)
    UPDATE  FROM dm_info
     ...
     WHERE info_domain="DATA MANAGEMENT"
      AND info_name=concat("BACKFILL INSTANCE:",cnvtstring(dmpb_instance))
     WITH nocounter
    ;end update
    IF (curqual=0)
     INSERT  FROM dm_info d
 ...
 ELSE
  ...
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 IF (dmpb_min_log <= dmpb_max_log)
  SET batch_stop = "N"
  CALL echo("_________ ")
  CALL echo("New Batch")
  SET readme_data->status = "F"
  SET readme_data->message = "New Batch"
  EXECUTE dm_merge_pkvalue_child
  IF (batch_stop="N")
   GO TO redo
  ELSE
   GO TO exit_script
 ...
 ELSE
  IF (dmpb_sa_run_ind=1)
   UPDATE  FROM dm_info
    SET info_number = 1, updt_dt_tm = cnvtdatetime(sysdate)
    WHERE info_domain="DATA MANAGEMENT"
     AND info_name=concat("BACKFILL INSTANCE:",cnvtstring(dmpb_instance))
    WITH nocounter
   ;end update
   ...
   ENDIF
   COMMIT
   CALL echo("This stand-alone instance of DM_MERGE_PKVALUE_BACKFILL has finished")
  ELSE
   SET dmpb_qual_ind = 1
   SET dmpb_old_dt_tm_ind = 0
   WHILE (dmpb_qual_ind != 0
    AND dmpb_old_dt_tm_ind=0)
     ...
     SELECT INTO "NL:"
      ...
      DETAIL
       IF (datetimediff(cnvtdatetime(sysdate),d.updt_dt_tm,4) > 10)
        dmpb_old_dt_tm_ind = 1
       ENDIF
      WITH nocounter
     ;end select
     IF (curqual=0)
     ...
     ENDIF
   ENDWHILE
   IF (dmpb_old_dt_tm_ind=0)
    SET readme_data->message = "Finished back-filling pk_where_value in DM_CHG_LOG"
    SET readme_data->status = "S"
   ELSE
