CREATE PROGRAM bed_aud_iview_cnt_report:dba
 ...
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(req_cnt)),
   cnt_wv_key wv,
   cnt_wv_section_r wvsr,
   cnt_wv_section_key sk,
   cnt_wv_section_item_r wvsi,
   cnt_wv_item_key wvi,
   v500_event_set_code vsi,
   v500_event_set_code ves
  PLAN (d)
   JOIN (wv
   WHERE (wv.working_view_uid=request->iviews[d.seq].view_uid)
    AND wv.active_ind=1)
   JOIN (wvsr
   WHERE wvsr.working_view_uid=wv.working_view_uid)
   JOIN (sk
   WHERE sk.wv_section_uid=wvsr.wv_section_uid)
   JOIN (wvsi
   WHERE wvsi.wv_section_uid=sk.wv_section_uid)
   JOIN (wvi
   WHERE outerjoin(wvi.wv_item_uid)=wvsi.wv_item_uid)
   JOIN (vsi
  ...
  ORDER BY wv.display_name, wv.cnt_wv_key_id, sk.cnt_wv_section_key_id,
   wvi.cnt_wv_item_key_id
  HEAD REPORT
   wvcnt = 0, tot_wvcnt = 0, stat = alterlist(temp->wvs,100)
  HEAD wv.cnt_wv_key_id
   wvcnt = (wvcnt+ 1), tot_wvcnt = (tot_wvcnt+ 1)
   IF (wvcnt > 100)
    stat = alterlist(temp->wvs,(tot_wvcnt+ 100)), wvcnt = 1
   ENDIF
   temp->wvs[tot_wvcnt].id = wv.cnt_wv_key_id, temp->wvs[tot_wvcnt].name = wv.display_name, wscnt = 0,
   tot_wscnt = 0, stat = alterlist(temp->wvs[tot_wvcnt].sections,100)
  HEAD sk.cnt_wv_section_key_id
   wicnt = 0, tot_wicnt = 0, wvs_set = 0
  HEAD wvi.cnt_wv_item_key_id
   IF (wvs_set=0
    AND ((sk.section_type_flag=1) OR (wvi.cnt_wv_item_key_id > 0)) )
    wvs_set = 1, wscnt = (wscnt+ 1), tot_wscnt = (tot_wscnt+ 1)
    IF (wscnt > 100)
     stat = alterlist(temp->wvs[tot_wvcnt].sections,(tot_wscnt+ 100)), wscnt = 1
    ENDIF
    temp->wvs[tot_wvcnt].sections[tot_wscnt].id = sk.cnt_wv_section_key_id, temp->wvs[tot_wvcnt].
    sections[tot_wscnt].name = sk.display_name, temp->wvs[tot_wvcnt].sections[tot_wscnt].
    event_set_name = sk.event_set_name,
   ...
   ENDIF
   IF (wvi.cnt_wv_item_key_id > 0)
    wicnt = (wicnt+ 1), tot_wicnt = (tot_wicnt+ 1)
    IF (wicnt > 100)
     stat = alterlist(temp->wvs[tot_wvcnt].sections[tot_wscnt].subsection,(tot_wicnt+ 100)), wicnt =
     1
    ENDIF
    temp->wvs[tot_wvcnt].sections[tot_wscnt].subsection[tot_wicnt].id = wvi.wv_item_uid
    IF (cnvtupper(wvi.parent_event_set_name) != cnvtupper(sk.event_set_name))
    ...
    temp->wvs[tot_wvcnt].sections[tot_wscnt].subsection[tot_wicnt].disp_assoc = wvi.disp_assoc_ind
    IF (wvi.disp_assoc_ind=1)
     temp->wvs[tot_wvcnt].sections[tot_wscnt].subsection[tot_wicnt].disp_assoc_name = wvi
     .primitive_event_set_name
    ENDIF
   ENDIF
  FOOT  sk.cnt_wv_section_key_id
   stat = alterlist(temp->wvs[tot_wvcnt].sections[tot_wscnt].subsection,tot_wicnt)
  FOOT  wv.cnt_wv_key_id
   ...
   IF (sub_cnt > 0)
    SELECT INTO "nl:"
     ...
     PLAN (d
      ...
      JOIN (wvi
      WHERE (wvi.wv_item_uid=temp->wvs[x].sections[y].subsection[d.seq].id))
      JOIN (dta
      WHERE wvi.task_assay_guid=dta.task_assay_uid)
      JOIN (lk
      WHERE lk.label_template_id=outerjoin(dta.label_template_id))
      JOIN (lr
      WHERE lr.cnt_ds_label_key_uid=outerjoin(lk.cnt_ds_label_key_uid))
      JOIN (dk
      WHERE dk.cnt_ds_key_uid=outerjoin(lr.cnt_ds_key_uid))
     ...
     DETAIL
      ...
      IF (dk.cnt_ds_key_id > 0)
       temp->wvs[x].sections[y].subsection[d.seq].details[etot_cnt].dynamic_type = "Template", temp->
       wvs[x].sections[y].subsection[d.seq].details[etot_cnt].dynamic_id = dk.cnt_ds_key_id, temp->
       wvs[x].sections[y].subsection[d.seq].details[etot_cnt].dynamic_uid = dk.cnt_ds_key_uid
      ENDIF
     FOOT  d.seq
    ...
    SELECT INTO "nl:"
     ...
     PLAN (d
      WHERE (temp->wvs[x].sections[y].subsection[d.seq].disp_assoc=1))
      JOIN (dta
      WHERE (dta.wv_item_uid=temp->wvs[x].sections[y].subsection[d.seq].id))
     ORDER BY d.seq, dta.task_assay_uid
     HEAD d.seq
     ...
     HEAD dta.task_assay_uid
      ...
      ENDIF
      temp->wvs[x].sections[y].subsection[d.seq].da_list[etot_cnt].dta_uid = dta.task_assay_uid
     FOOT  d.seq
      ...
      IF ((temp->wvs[x].sections[y].subsection[z].disp_assoc=1))
       ...
       IF (da_cnt > 0)
        SELECT INTO "nl:"
         ...
         PLAN (d)
          JOIN (dta
          WHERE dta.task_assay_uid=outerjoin(cnvtupper(temp->wvs[x].sections[y].subsection[z].
            da_list[d.seq].dta_uid)))
          JOIN (lk
          WHERE lk.label_template_id=outerjoin(dta.label_template_id))
          JOIN (lr
          WHERE lr.cnt_ds_label_key_uid=outerjoin(lk.cnt_ds_label_key_uid))
         ...
         DETAIL
          ...
          ENDIF
          temp->wvs[x].sections[y].subsection[z].details[etot_cnt].da_prim_event_set = dta.mnemonic,
          ...
          IF (dk.cnt_ds_key_id > 0)
           ...
           wvs[x].sections[y].subsection[z].details[etot_cnt].dynamic_id = dk.cnt_ds_key_id, temp->
           wvs[x].sections[y].subsection[z].details[etot_cnt].dynamic_uid = dk.cnt_ds_key_uid
          ENDIF
         FOOT REPORT
        ...
        IF ((temp->wvs[x].sections[y].subsection[z].details[a].dynamic_id > 0))
         SELECT INTO "nl:"
          FROM cnt_ds_label_r lr,
           ...
           v500_event_set_code vsi
          PLAN (lr
           WHERE (lr.cnt_ds_key_uid=temp->wvs[x].sections[y].subsection[z].details[a].dynamic_uid))
           JOIN (sr
           ...
           JOIN (ck
           WHERE ck.code_value_uid=outerjoin(dta.event_code_cduid))
           JOIN (vsi
           WHERE cnvtupper(vsi.event_set_name)=outerjoin(cnvtupper(ck.event_set_name)))
          ...
          HEAD dta.task_assay_uid
           ...
           dadd[dy_size].label_id = sek.cnt_ds_sec_element_key_id,
           stat = add_label(x,y,z,vsi.event_set_name,vsi.event_set_cd_disp,
            dta.mnemonic)
          WITH nocounter
         ;end select
 ...
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("iview_cnt_design_audit.csv")
 ENDIF
