CREATE PROGRAM bhs_prax_recomm_action_list
 SELECT INTO  $1
  ...
  hr_updt_dt_tm = format(hr.updt_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"), hr_first_due_dt_tm = format(hr
   .first_due_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"), h.priority_meaning,
  ...
  hr.last_satisfied_by_id, p.name_full_formatted, hr_last_satisfaction_dt_tm = format(hr
   .last_satisfaction_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"),
  hra_satisfaction_dt_tm = format(hra.satisfaction_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"), hra_reason_disp
   = trim(replace(replace(replace(replace(replace(uar_get_code_display(hra.reason_cd),"&","&amp;",0),
       "<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3), hr.status_flag,
  hr_status =
