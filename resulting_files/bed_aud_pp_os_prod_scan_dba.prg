CREATE PROGRAM bed_aud_pp_os_prod_scan:dba
 ...
 DECLARE uom_compare(os_uom_id=i2,med_uom_id=i2) = i2
 ...
 DECLARE populate_powerplan_desc(null) = null
 DECLARE powerplan_order_list_extract(null) = null
 ...
 RECORD order_list(
   1 order_info[*]
     ...
     2 os_info[*]
       ...
       3 os_str_uom = vc
       3 os_str_uom_id = f8
       3 os_str_cnvt_uom_id = f8
       3 os_vol_dose = vc
       3 os_vol_uom = vc
       3 os_vol_uom_id = f8
       3 os_vol_cnvt_uom_id = f8
       3 os_route = vc
       3 os_route_id = f8
       3 os_form = vc
       3 os_form_id = f8
 ...
 RECORD pathway_cat(
   1 pathway_info[*]
     2 pathway_id = f8
     2 pathway_desc = vc
     2 pathway_subphase_ind = i2
     2 pathway_join_id = f8
 ) WITH protect
 SET stat = alterlist(pathway_cat->pathway_info,500)
 CALL syn_prod_link_extract(null)
 CALL route_form_comp_populate(null)
 CALL uom_extract(null)
 CALL populate_powerplan_desc(null)
 CALL powerplan_order_list_extract(null)
 ...
 SUBROUTINE route_form_comp_populate(null)
  SELECT INTO "nl:"
   ...
   HEAD r.route_cd
    ...
    ENDIF
    route_form->route[r_cnt].route_cd = r.route_cd, f_cnt = 0, stat = alterlist(route_form->route[
     r_cnt].form,10)
   HEAD r.form_cd
    f_cnt = (f_cnt+ 1)
    IF (mod(f_cnt,10)=1
 ...
 SUBROUTINE no_product_format(ff,rf,fac)
   ...
   RETURN(output)
 END ;Subroutine
 SUBROUTINE populate_powerplan_desc(null)
  SELECT INTO "nl:"
   FROM pw_cat_flex pwcf,
    pathway_catalog pc,
    pw_cat_reltn pcrp
   PLAN (pwcf
    WHERE pwcf.parent_entity_name="CODE_VALUE"
     AND pwcf.parent_entity_id IN (0, request->vv_facility))
    JOIN (pc
    WHERE pc.pathway_catalog_id=pwcf.pathway_catalog_id
     AND pc.active_ind=1)
    JOIN (pcrp
    WHERE pcrp.pw_cat_s_id=outerjoin(pc.pathway_catalog_id))
   ...
   DETAIL
    ...
    IF (mod(pw_cnt,100)=1
     AND pw_cnt > 500)
     stat = alterlist(pathway_cat->pathway_info,(pw_cnt+ 99))
    ENDIF
    pathway_cat->pathway_info[pw_cnt].pathway_id = pc.pathway_catalog_id, pathway_cat->pathway_info[
    pw_cnt].pathway_desc = pc.display_description
    IF (pc.sub_phase_ind=1)
     pathway_cat->pathway_info[pw_cnt].pathway_subphase_ind = 1
    ENDIF
    IF (pcrp.pw_cat_s_id=0)
     pathway_cat->pathway_info[pw_cnt].pathway_join_id = pc.pathway_catalog_id
    ELSE
     IF (pcrp.type_mean="GROUP")
      pathway_cat->pathway_info[pw_cnt].pathway_join_id = pcrp.pw_cat_t_id
     ELSEIF (pcrp.type_mean="SUBPHASE")
      pathway_cat->pathway_info[pw_cnt].pathway_join_id = pcrp.pw_cat_s_id
     ENDIF
    ENDIF
   FOOT REPORT
    stat = alterlist(pathway_cat->pathway_info,pw_cnt)
   WITH nocounter
  ;end select
  CALL bederrorcheck("PopPowerPlanDesc")
 END ;Subroutine
 SUBROUTINE powerplan_order_list_extract(null)
  SELECT INTO "nl:"
   pp_id = pathway_cat->pathway_info[d1.seq].pathway_id
   FROM (dummyt d1  WITH seq = value(size(pathway_cat->pathway_info,5))),
    pathway_comp pcmp,
   ...
   PLAN (d1)
    JOIN (pcmp
    WHERE (pcmp.pathway_catalog_id=pathway_cat->pathway_info[d1.seq].pathway_join_id)
     AND pcmp.parent_entity_name="ORDER_CATALOG_SYNONYM")
    JOIN (ocs
    WHERE ocs.synonym_id=pcmp.parent_entity_id
     AND ocs.catalog_type_cd=catalog_type_pharmacy
    ...
    JOIN (osd)
    JOIN (ofm)
   ORDER BY pp_id, ocs.synonym_id, os.order_sentence_id,
    osd.oe_field_id
   HEAD REPORT
    pp_cnt = 0, syn_cnt = 0, os_cnt = 0
   HEAD pp_id
    pp_cnt = (pp_cnt+ 1)
   HEAD ocs.synonym_id
    syn_cnt = (syn_cnt+ 1)
    ...
    ENDIF
    order_list->order_info[syn_cnt].set_desc = pathway_cat->pathway_info[d1.seq].pathway_desc,
    order_list->order_info[syn_cnt].set_id = pathway_cat->pathway_info[d1.seq].pathway_join_id,
    order_list->order_info[syn_cnt].subphase_ind = pathway_cat->pathway_info[d1.seq].
    pathway_subphase_ind,
    order_list->order_info[syn_cnt].catalog_cd = ocs.catalog_cd, order_list->order_info[syn_cnt].
    synonym = ocs.mnemonic, order_list->order_info[syn_cnt].synonym_id = ocs.synonym_id,
    stat = alterlist(order_list->order_info[syn_cnt].os_info,10), os_cnt = 0
   HEAD os.order_sentence_id
    os_cnt = (os_cnt+ 1)
    IF (mod(os_cnt,10)=1
     AND os_cnt > 10)
     stat = alterlist(order_list->order_info[syn_cnt].os_info,(os_cnt+ 9))
    ENDIF
    order_list->order_info[syn_cnt].os_info[os_cnt].os_id = os.order_sentence_id, order_list->
    order_info[syn_cnt].os_info[os_cnt].os_disp_line = os.order_sentence_display_line
   HEAD osd.oe_field_id
    CASE (ofm.oe_field_meaning)
     OF "STRENGTHDOSE":
      order_list->order_info[syn_cnt].os_info[os_cnt].os_str_dose = osd.oe_field_display_value
     OF "STRENGTHDOSEUNIT":
      order_list->order_info[syn_cnt].os_info[os_cnt].os_str_uom = osd.oe_field_display_value,
      order_list->order_info[syn_cnt].os_info[os_cnt].os_str_uom_id = osd.default_parent_entity_id
     OF "VOLUMEDOSE":
      order_list->order_info[syn_cnt].os_info[os_cnt].os_vol_dose = osd.oe_field_display_value
     OF "VOLUMEDOSEUNIT":
      order_list->order_info[syn_cnt].os_info[os_cnt].os_vol_uom = osd.oe_field_display_value,
      order_list->order_info[syn_cnt].os_info[os_cnt].os_vol_uom_id = osd.default_parent_entity_id
     OF "RXROUTE":
      order_list->order_info[syn_cnt].os_info[os_cnt].os_route = osd.oe_field_display_value,
      order_list->order_info[syn_cnt].os_info[os_cnt].os_route_id = osd.default_parent_entity_id
     OF "DRUGFORM":
      order_list->order_info[syn_cnt].os_info[os_cnt].os_form = osd.oe_field_display_value,order_list
      ->order_info[syn_cnt].os_info[os_cnt].os_form_id = osd.default_parent_entity_id
    ENDCASE
   FOOT  os.order_sentence_id
    order_list->order_info[syn_cnt].os_info[os_cnt].os_route_form_err = route_form_comp(order_list->
     order_info[syn_cnt].os_info[os_cnt].os_route_id,order_list->order_info[syn_cnt].os_info[os_cnt].
     os_form_id), order_list->order_info[syn_cnt].os_info[os_cnt].facility_err = 0, order_list->
    order_info[syn_cnt].os_info[os_cnt].form_form_err = 0,
    order_list->order_info[syn_cnt].os_info[os_cnt].route_form_err = 0
   FOOT  ocs.synonym_id
    stat = alterlist(order_list->order_info[syn_cnt].os_info,os_cnt)
   FOOT REPORT
    stat = alterlist(order_list->order_info,syn_cnt)
   WITH nocounter
  ;end select
  CALL bederrorcheck("PowerPlanOrderExtract")
 END ;Subroutine
 SUBROUTINE populate_reply(null)
   DECLARE total_col = i4 WITH protect, constant(24)
   ...
   SET reply->collist[2].header_text = "Sub Phase Indicator"
   SET reply->collist[2].data_type = 2
   SET reply->collist[2].hide_ind = 0
   SET reply->collist[3].header_text = "Synonym"
   SET reply->collist[3].data_type = 1
   SET reply->collist[3].hide_ind = 0
   SET reply->collist[4].header_text = "OS ID"
   SET reply->collist[4].data_type = 2
   SET reply->collist[4].hide_ind = 0
   SET reply->collist[5].header_text = "Order Sentence Display"
   SET reply->collist[5].data_type = 1
   SET reply->collist[5].hide_ind = 0
   SET reply->collist[6].header_text = "Order Sentence Strength"
   SET reply->collist[6].data_type = 1
   SET reply->collist[6].hide_ind = 0
   SET reply->collist[7].header_text = "Order Sentence Strength Unit"
   SET reply->collist[7].data_type = 1
   SET reply->collist[7].hide_ind = 0
   SET reply->collist[8].header_text = "Order Sentence Volume"
   SET reply->collist[8].data_type = 1
   SET reply->collist[8].hide_ind = 0
   SET reply->collist[9].header_text = "Order Sentence Volume Unit"
   SET reply->collist[9].data_type = 1
   SET reply->collist[9].hide_ind = 0
   SET reply->collist[10].header_text = "Order Sentence Route"
   SET reply->collist[10].data_type = 1
   SET reply->collist[10].hide_ind = 0
   SET reply->collist[11].header_text = "Order Sentence Drug Form"
   SET reply->collist[11].data_type = 1
   SET reply->collist[11].hide_ind = 0
   SET reply->collist[12].header_text = "Ready Ind"
   SET reply->collist[12].data_type = 2
   SET reply->collist[12].hide_ind = 0
   SET reply->collist[13].header_text = "Product Label Description"
   SET reply->collist[13].data_type = 1
   SET reply->collist[13].hide_ind = 0
   SET reply->collist[14].header_text = "Product Strength"
   SET reply->collist[14].data_type = 1
   SET reply->collist[14].hide_ind = 0
   SET reply->collist[15].header_text = "Product Strength Unit"
   SET reply->collist[15].data_type = 1
   SET reply->collist[15].hide_ind = 0
   SET reply->collist[16].header_text = "Product Volume"
   SET reply->collist[16].data_type = 1
   SET reply->collist[16].hide_ind = 0
   SET reply->collist[17].header_text = "Product Volume Unit"
   SET reply->collist[17].data_type = 1
   SET reply->collist[17].hide_ind = 0
   SET reply->collist[18].header_text = "Product Drug Form"
   SET reply->collist[18].data_type = 1
   SET reply->collist[18].hide_ind = 0
   SET reply->collist[19].header_text = "NDC"
   SET reply->collist[19].data_type = 1
   SET reply->collist[19].hide_ind = 0
   SET reply->collist[20].header_text = "Strength Mismatch"
   SET reply->collist[20].data_type = 1
   SET reply->collist[20].hide_ind = 0
   SET reply->collist[21].header_text = "Volume mismatch"
   SET reply->collist[21].data_type = 1
   SET reply->collist[21].hide_ind = 0
   SET reply->collist[22].header_text = "Route Form Incompatibility"
   SET reply->collist[22].data_type = 1
   SET reply->collist[22].hide_ind = 0
   SET reply->collist[23].header_text = "Linked?"
   SET reply->collist[23].data_type = 1
   SET reply->collist[23].hide_ind = 0
   SET reply->collist[24].header_text = "Status"
   ...
   SELECT INTO "nl:"
    ...
    set_id = order_list->order_info[d1.seq].set_id, syn_id = order_list->order_info[d1.seq].
    synonym_id, cat_cd = order_list->order_info[d1.seq].catalog_cd,
    os_id = order_list->order_info[d1.seq].os_info[d2.seq].os_id
    ...
    PLAN (d1
     ...
     JOIN (mfoi2)
    ORDER BY set_id, syn_id, os_id,
     mi1.item_id
    HEAD REPORT
     set_cnt = 0, cat_cnt = 0, syn_cnt = 0,
     os_cnt = 0, med_cnt = 0
    HEAD set_id
     set_cnt = (set_cnt+ 1)
    HEAD syn_id
    ...
    HEAD mi1.item_id
     ...
     IF (med_disqualify_ind=0)
      ...
      ELSE
       str_uom_err_ind = uom_compare(cnvtint(order_list->order_info[d1.seq].os_info[d2.seq].
         os_str_uom_id),cnvtint(md.strength_unit_cd))
       IF (str_uom_err_ind=2)
        ...
        IF (os_conv_str_ind=0
         ...
         os_str_uom_cnvt = uom_conversion(order_list->order_info[d1.seq].os_info[d2.seq].
          os_str_uom_id)
        ENDIF
        os_conv_str_ind = 1, str_uom_err_ind = uom_compare(os_str_uom_cnvt,med_str_uom_cnvt)
       ENDIF
       vol_uom_err_ind = uom_compare(cnvtint(order_list->order_info[d1.seq].os_info[d2.seq].
         os_vol_uom_id),cnvtint(md.volume_unit_cd))
       IF (vol_uom_err_ind=2)
        ...
        IF (os_conv_vol_ind=0
         ...
         os_vol_uom_cnvt = uom_conversion(order_list->order_info[d1.seq].os_info[d2.seq].
          os_vol_uom_id)
        ENDIF
        os_conv_vol_ind = 1, vol_uom_err_ind = uom_compare(os_vol_uom_cnvt,med_vol_uom_cnvt)
       ENDIF
    ...
    FOOT  os_id
     ...
     IF (os_no_products_ind=1)
      ...
      string_value = order_list->order_info[d1.seq].set_desc, reply->rowlist[rcnt].celllist[2].
      double_value = order_list->order_info[d1.seq].subphase_ind,
      reply->rowlist[rcnt].celllist[3].string_value = order_list->order_info[d1.seq].synonym, reply->
      rowlist[rcnt].celllist[4].double_value = order_list->order_info[d1.seq].os_info[d2.seq].os_id,
      reply->rowlist[rcnt].celllist[5].string_value = order_list->order_info[d1.seq].os_info[d2.seq].
      os_disp_line,
      reply->rowlist[rcnt].celllist[6].string_value = order_list->order_info[d1.seq].os_info[d2.seq].
      os_str_dose, reply->rowlist[rcnt].celllist[7].string_value = order_list->order_info[d1.seq].
      os_info[d2.seq].os_str_uom, reply->rowlist[rcnt].celllist[8].string_value = order_list->
      ...
      os_vol_uom, reply->rowlist[rcnt].celllist[10].string_value = order_list->order_info[d1.seq].
      os_info[d2.seq].os_route, reply->rowlist[rcnt].celllist[11].string_value = order_list->
      order_info[d1.seq].os_info[d2.seq].os_form,
      reply->rowlist[rcnt].celllist[12].double_value = 0, reply->rowlist[rcnt].celllist[13].
      string_value = no_product_output, reply->rowlist[rcnt].celllist[14].string_value = " ",
      ...
      string_value = " ", reply->rowlist[rcnt].celllist[20].string_value = " ",
      reply->rowlist[rcnt].celllist[21].string_value = " ", reply->rowlist[rcnt].celllist[22].
      string_value = order_list->order_info[d1.seq].os_info[d2.seq].os_route_form_err, reply->
      rowlist[rcnt].celllist[23].string_value = " ",
      reply->rowlist[rcnt].celllist[24].string_value = "NOT READY"
     ELSE
      FOR (x = 1 TO size(order_list->order_info[d1.seq].os_info[d2.seq].med_info,5))
        rcnt = (rcnt+ 1), stat = alterlist(reply->rowlist,rcnt), stat = alterlist(reply->rowlist[rcnt
         ].celllist,total_col),
        reply->rowlist[rcnt].celllist[1].string_value = order_list->order_info[d1.seq].set_desc,
        reply->rowlist[rcnt].celllist[2].double_value = order_list->order_info[d1.seq].subphase_ind,
        reply->rowlist[rcnt].celllist[3].string_value = order_list->order_info[d1.seq].synonym,
        reply->rowlist[rcnt].celllist[4].double_value = order_list->order_info[d1.seq].os_info[d2.seq
        ].os_id, reply->rowlist[rcnt].celllist[5].string_value = order_list->order_info[d1.seq].
        os_info[d2.seq].os_disp_line, reply->rowlist[rcnt].celllist[6].string_value = order_list->
        order_info[d1.seq].os_info[d2.seq].os_str_dose,
        reply->rowlist[rcnt].celllist[7].string_value = order_list->order_info[d1.seq].os_info[d2.seq
        ].os_str_uom, reply->rowlist[rcnt].celllist[8].string_value = order_list->order_info[d1.seq].
        os_info[d2.seq].os_vol_dose, reply->rowlist[rcnt].celllist[9].string_value = order_list->
        order_info[d1.seq].os_info[d2.seq].os_vol_uom,
        reply->rowlist[rcnt].celllist[10].string_value = order_list->order_info[d1.seq].os_info[d2
        .seq].os_route, reply->rowlist[rcnt].celllist[11].string_value = order_list->order_info[d1
        .seq].os_info[d2.seq].os_form, reply->rowlist[rcnt].celllist[12].double_value = os_chk_ind,
        reply->rowlist[rcnt].celllist[13].string_value = order_list->order_info[d1.seq].os_info[d2
        .seq].med_info[x].med_desc, reply->rowlist[rcnt].celllist[14].string_value = build(order_list
         ->order_info[d1.seq].os_info[d2.seq].med_info[x].med_str), reply->rowlist[rcnt].celllist[15]
        .string_value = order_list->order_info[d1.seq].os_info[d2.seq].med_info[x].med_str_uom,
        reply->rowlist[rcnt].celllist[16].string_value = build(order_list->order_info[d1.seq].
         os_info[d2.seq].med_info[x].med_vol), reply->rowlist[rcnt].celllist[17].string_value =
        order_list->order_info[d1.seq].os_info[d2.seq].med_info[x].med_vol_uom, reply->rowlist[rcnt].
        celllist[18].string_value = order_list->order_info[d1.seq].os_info[d2.seq].med_info[x].
        med_form,
        reply->rowlist[rcnt].celllist[19].string_value = order_list->order_info[d1.seq].os_info[d2
        .seq].med_info[x].med_ndc, reply->rowlist[rcnt].celllist[20].string_value = order_list->
        order_info[d1.seq].os_info[d2.seq].med_info[x].str_uom_err_output, reply->rowlist[rcnt].
        ...
        .seq].os_route_form_err, reply->rowlist[rcnt].celllist[23].string_value = order_list->
        order_info[d1.seq].os_info[d2.seq].med_info[x].med_linked_disp, reply->rowlist[rcnt].
        celllist[24].string_value = order_list->order_info[d1.seq].os_info[d2.seq].med_info[x].
        med_ready_display
      ENDFOR
     ENDIF
   ...
   IF ((reply->high_volume_flag IN (1, 2)))
    ...
    SET reply->output_filename = build("bed_aud_pp_os_prod_scan.csv")
   ENDIF
