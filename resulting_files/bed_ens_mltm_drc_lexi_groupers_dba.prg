CREATE PROGRAM bed_ens_mltm_drc_lexi_groupers:dba
 FREE SET request_kia
 RECORD request_kia(
 ...
 FREE SET inact_premise
 RECORD inact_premise(
   1 premises[*]
 ...
 SET tot_cnt = 0
 SET cnt2 = 0
 SET cnt = size(request->groupers,5)
 ...
 IF (cnt > 0)
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (m
    WHERE (m.grouper_id=request->groupers[d.seq].grouper_id))
    JOIN (drc
    WHERE drc.dose_range_check_name=m.grouper_name)
   ORDER BY d.seq
   HEAD REPORT
    dcnt = 0, dtot_cnt = 0, stat = alterlist(inact_drc->drc_ids,100)
   HEAD d.seq
    dcnt = (dcnt+ 1), dtot_cnt = (dtot_cnt+ 1)
    IF (dcnt > 100)
     stat = alterlist(inact_drc->drc_ids,(dtot_cnt+ 100)), dcnt = 1
    ENDIF
    inact_drc->drc_ids[dtot_cnt].drc_id = drc.dose_range_check_id
   FOOT REPORT
    stat = alterlist(inact_drc->drc_ids,dtot_cnt)
   WITH nocounter
  ;end select
  IF (dtot_cnt > 0)
   SET ptot_cnt = 0
   SELECT INTO "nl:"
    ...
    PLAN (d)
     JOIN (dp
     WHERE (dp.dose_range_check_id=inact_drc->drc_ids[d.seq].drc_id)
      AND dp.parent_premise_id != 0.0
      AND dp.active_ind=1
      AND dp.relational_operator_flag=1
      AND dp.value1=18.0
      AND dp.premise_type_flag=1)
     JOIN (cv
     WHERE cv.code_value=dp.value_unit_cd
      AND cv.code_set=54
      AND cv.cdf_meaning="YEARS"
      AND cv.active_ind=1)
    ORDER BY dp.parent_premise_id
    HEAD REPORT
     pcnt = 0, ptot_cnt = 0, stat = alterlist(inact_premise->premises,100)
    HEAD dp.parent_premise_id
     pcnt = (pcnt+ 1), ptot_cnt = (ptot_cnt+ 1)
     IF (pcnt > 100)
      stat = alterlist(inact_premise->premises,(ptot_cnt+ 100)), pcnt = 1
     ENDIF
     inact_premise->premises[ptot_cnt].parent_premise_id = dp.parent_premise_id
    FOOT REPORT
     stat = alterlist(inact_premise->premises,ptot_cnt)
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = dtot_cnt),
    ...
    ORDER BY dp.parent_premise_id
    HEAD REPORT
     pcnt = 0, stat = alterlist(inact_premise->premises,(ptot_cnt+ 100))
    HEAD dp.parent_premise_id
     pcnt = (pcnt+ 1), ptot_cnt = (ptot_cnt+ 1)
     IF (pcnt > 100)
      stat = alterlist(inact_premise->premises,(ptot_cnt+ 100)), pcnt = 1
   ...
   SELECT INTO "nl:"
    ...
    PLAN (d)
     ...
     WHERE (dp.dose_range_check_id=inact_drc->drc_ids[d.seq].drc_id)
      AND dp.parent_premise_id != 0.0
      AND dp.active_ind=1
      AND dp.value_unit_cd > 0.0
      AND dp.premise_type_flag=1)
     JOIN (cv
     WHERE cv.code_value=dp.value_unit_cd
      AND cv.code_set=54
      AND cv.cdf_meaning IN ("SECONDS", "MINUTES", "HOURS", "DAYS", "WEEKS",
     "MONTHS")
      AND cv.active_ind=1)
    ORDER BY dp.parent_premise_id
    HEAD REPORT
   ...
   IF (ptot_cnt > 0)
    ...
    UPDATE  FROM drc_premise d,
      (dummyt d2  WITH seq = value(ptot_cnt))
     SET d.active_ind = 0, d.updt_cnt = (d.updt_cnt+ 1), d.updt_dt_tm = cnvtdatetime(curdate,curtime3
       ),
      d.updt_id = reqinfo->updt_id, d.updt_task = reqinfo->updt_task, d.updt_applctx = reqinfo->
      updt_applctx
     PLAN (d2)
      JOIN (d
      WHERE (d.drc_premise_id=inact_premise->premises[d2.seq].parent_premise_id)
       AND d.active_ind=1)
     WITH nocounter
    ;end update
    SET ierrcode = error(serrmsg,1)
    ...
    SELECT INTO "nl:"
     ...
     PLAN (d2)
      JOIN (d
      WHERE (d.drc_premise_id=inact_premise->premises[d2.seq].parent_premise_id))
     ORDER BY d2.seq, d.ver_seq
     DETAIL
      inact_premise->premises[d2.seq].ver_seq = (d.ver_seq+ 1)
     WITH nocounter
    ;end select
    FOR (ins_cnt = 1 TO ptot_cnt)
     INSERT  FROM drc_premise_ver dp
      (dp.active_ind, dp.concept_cki, dp.dose_range_check_id,
      dp.drc_premise_id, dp.multum_case_id, dp.parent_ind,
      dp.parent_premise_id, dp.premise_type_flag, dp.relational_operator_flag,
      dp.value1, dp.value1_string, dp.value2,
      dp.value2_string, dp.value_type_flag, dp.value_unit_cd,
      dp.ver_seq, dp.updt_cnt, dp.updt_dt_tm,
      dp.updt_task, dp.updt_applctx, dp.updt_id)(SELECT
       x.active_ind, x.concept_cki, x.dose_range_check_id,
       inact_premise->premises[ins_cnt].parent_premise_id, x.multum_case_id, x.parent_ind,
       x.parent_premise_id, x.premise_type_flag, x.relational_operator_flag,
       x.value1, x.value1_string, x.value2,
       x.value2_string, x.value_type_flag, x.value_unit_cd,
       inact_premise->premises[ins_cnt].ver_seq, 0, cnvtdatetime(curdate,curtime3),
       reqinfo->updt_task, reqinfo->updt_applctx, reqinfo->updt_id
       FROM drc_premise x
       WHERE (x.drc_premise_id=inact_premise->premises[ins_cnt].parent_premise_id))
      PLAN (dp)
      WITH nocounter
     ;end insert
     IF (curqual=0)
      SET error_flag = "Y"
      SET reply->error_msg = concat("Unable to insert drc_premise: ",trim(cnvtstring(inact_premise->
         premises[ins_cnt].parent_premise_id))," on drc_premise_ver.")
      GO TO exit_script
     ENDIF
    ENDFOR
   ENDIF
  ...
  SELECT INTO "nl:"
   ...
   HEAD m.drc_identifier
    ...
    IF (cnvtupper(m.liver_desc) IN ("YES", "Y", "1"))
    ...
    active_ind = 1, temp_kia->drc_obj_list[d.seq].drc_obj.qualifier_list[cnt2].qualifier.
    multum_case_id = m.multum_case_id,
   ...
   DETAIL
    ...
    qualifier_list[cnt2].qualifier.dose_range_list[cnt4].dose_range.dose_unit = cv2.display, temp_kia
    ->drc_obj_list[d.seq].drc_obj.qualifier_list[cnt2].qualifier.dose_range_list[cnt4].dose_range.
    dose_unit_cki = m.dose_unit_cki,
    temp_kia->drc_obj_list[d.seq].drc_obj.qualifier_list[cnt2].qualifier.dose_range_list[cnt4].
  ...
  SELECT INTO "nl:"
   ...
   HEAD m.grouper_id
    ...
    drc_obj.build_contributor = "MULTUM INSTALL", temp_kia->drc_obj_list[d.seq].drc_obj.active_ind =
    1,
    cnt2 = size(temp_kia->drc_obj_list[d.seq].drc_obj.qualifier_list,5), tot_cnt2 = 0, stat =
    alterlist(temp_kia->drc_obj_list[d.seq].drc_obj.qualifier_list,(cnt2+ 10))
   HEAD m.drc_identifier
    cnt2 = (cnt2+ 1), tot_cnt2 = (tot_cnt2+ 1)
    IF (tot_cnt2 > 10)
     stat = alterlist(temp_kia->drc_obj_list[d.seq].drc_obj.qualifier_list,(cnt2+ 10)), tot_cnt2 = 1
  ...
  FOR (gcnt = 1 TO kia_size)
    ...
    IF (((exe_cnt=50) OR (gcnt=kia_size)) )
     SET request_kia->import_method_flag = 1
     SET request_kia->domain_flag = 0
     FREE SET reply
