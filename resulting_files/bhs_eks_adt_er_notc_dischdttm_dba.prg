CREATE PROGRAM bhs_eks_adt_er_notc_dischdttm:dba
 PROMPT
 ...
 SELECT INTO "NL:"
  ...
  DETAIL
   log_misc1 = format(cnvtdatetime(e.disch_dt_tm),"MM/DD/YYYY HH:MM;;q"),
   CALL echo(cnvtdatetime(e.disch_dt_tm))
  WITH nocounter
 ;end select
 IF (curqual > 0)
  SET retval = 100
  SET log_message = build("disc_dt_tm for encntrId:",encntr_id," _ ",log_misc1)
 ELSE
  SET logmisc1 = "(Failed to find Discharge date)"
 ...
 SELECT INTO  $OUTDEV
  ...
  HEAD REPORT
   msg1 =
   IF (textlen(log_misc1) > 0) build("DischDtTm:",log_misc1)
