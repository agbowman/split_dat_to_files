CREATE PROGRAM aps_processing_grp_r_fix:dba
 RECORD temp(
   1 qual[*]
     2 grouper_cd = f8
     2 sequence = i4
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   temp->qual[cnt].grouper_cd = ap.grouper_cd, temp->qual[cnt].sequence = ap.sequence, temp->qual[cnt
   ].code_value_ind = cv_exists
  FOOT REPORT
  ...
  UPDATE  FROM ap_processing_grp_r ap,
   ...
   PLAN (d)
    ...
    WHERE (temp->qual[d.seq].grouper_cd=ap.grouper_cd)
     AND (temp->qual[d.seq].sequence=ap.sequence))
   WITH nocounter
  ;end update
  COMMIT
 ENDIF
