CREATE PROGRAM atr_del_task_from_app:dba
 RECORD internal(
   1 qual[*]
     2 updt_flag = i2
 )
 ...
 IF ((request->feature_number=0))
  DELETE  FROM application_task_r r,
   ...
   PLAN (d)
    JOIN (r
    WHERE (r.task_number=request->qual[d.seq].task_number)
     AND (r.application_number=request->application_number))
  ...
  ENDIF
 ELSE
  SET stat = alterlist(internal->qual,number_total)
  SELECT INTO "nl:"
   r.application_number
   FROM dm_application_task_r r,
   ...
   HEAD REPORT
    number_to_update = 0
   DETAIL
    number_to_update += 1, internal->qual[d.seq].updt_flag = 1
   WITH nocounter, forupdate(r)
  ;end select
  IF (number_to_update > 0)
   UPDATE  FROM dm_application_task_r r,
     (dummyt d  WITH seq = value(number_total))
    SET r.seq = 1, r.deleted_ind = 1, r.updt_dt_tm = cnvtdatetime(sysdate),
     r.updt_task = reqinfo->updt_task, r.updt_id = reqinfo->updt_id, r.updt_cnt = (r.updt_cnt+ 1),
     r.updt_applctx = reqinfo->updt_applctx, r.schema_date = cnvtdatetimeutc(request->schema_date,2)
    PLAN (d
     WHERE (internal->qual[d.seq].updt_flag=1))
     JOIN (r
     WHERE (r.application_number=request->application_number)
      AND (r.feature_number=request->feature_number)
      AND (r.task_number=request->qual[d.seq].task_number))
   ...
   ;end update
   IF (curqual=0)
    CALL result_status("Update","F","dm_application_task_r","Update failed.")
    SET reqinfo->commit_ind = 0
    GO TO exit_script
   ENDIF
  ENDIF
  IF (number_to_update != number_total)
   INSERT  FROM dm_application_task_r r,
     (dummyt d  WITH seq = value(number_total))
    SET r.application_number = request->application_number, r.task_number = request->qual[d.seq].
     task_number, r.deleted_ind = 1,
   ...
   ;end insert
   IF (curqual=0)
    CALL result_status("Insert","F","dm_application_task_r","Insert failed.")
    SET reqinfo->commit_ind = 0
    GO TO exit_script
   ENDIF
  ENDIF
