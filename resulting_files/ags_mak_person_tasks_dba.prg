CREATE PROGRAM ags_mak_person_tasks:dba
 CALL echo("***")
 CALL echo("***   BEG AGS_MAK_PERSON_TASKS")
 CALL echo("***")
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_MAK_PERSON_TASKS"
 CALL echo("***")
 CALL echo("***   Find PERSON Batch Id Values")
 CALL echo("***")
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 SELECT INTO "nl:"
  little_id = min(o.ags_person_data_id), big_id = max(o.ags_person_data_id)
  FROM ags_person_data o
  PLAN (o
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("GET PERSON BATCH_ID VALUES :: Select Error :: ",trim(
    serrmsg))
  SET serrmsg = log->qual[log->qual_knt].smsg
  GO TO exit_script
 ENDIF
 CALL echo("***")
 CALL echo(build("***   AGS_PERSON_DATA min_batch_id :",min_batch_id))
 CALL echo(build("***   AGS_PERSON_DATA max_batch_id :",max_batch_id))
 CALL echo("***")
 CALL echo("***")
 CALL echo("***   Create PERSON Tasks")
 CALL echo("***")
 WHILE (min_batch_id <= max_batch_id
  AND min_batch_id >= 1)
   ...
   SET item->qual[item->qual_knt].task_type = "PERSON"
   ...
   SET item->qual[item->qual_knt].batch_program = "ags_load_person"
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   SELECT INTO "nl:"
 ...
 IF ((item->qual_knt < 1))
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "No PERSON tasks found"
  GO TO exit_script
 ENDIF
 CALL echo("***")
 ...
 COMMIT
 CALL echo("***")
 CALL echo("***   Update PERSON Data to WAITING")
 CALL echo("***")
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_MAK_PERSON_TASKS"
 CALL echo("***")
 CALL echo("***   END AGS_MAK_PERSON_TASKS")
 CALL echo("***")
