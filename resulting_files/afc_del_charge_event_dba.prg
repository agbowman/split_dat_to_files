CREATE PROGRAM afc_del_charge_event:dba
 ...
 CALL del_charge_event(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE del_charge_event(del_begin,del_end)
   FOR (x = del_begin TO del_end)
     ...
     UPDATE  FROM charge_event c
      SET c.active_ind = false, c.active_status_cd = nullcheck(active_code,request->charge_event[x].
        active_status_cd,
