CREATE PROGRAM bhs_prax_problem_review
 FREE RECORD result
 RECORD result(
   1 problem_cnt = i4
   1 problem[*]
 ...
 ELSEIF (( $3 <= 0.0))
  ...
  GO TO exit_script
 ENDIF
 DECLARE problemidparam = vc WITH protect, noconstant("")
 ...
 SET startpos = 1
 SET problemidparam = trim( $2,3)
 CALL echo(build2("PROBLEMIDPARAM IS: ",problemidparam))
 WHILE (size(problemidparam) > 0)
   SET endpos = (findstring(";",problemidparam,1) - 1)
   IF (endpos <= 0)
    SET endpos = size(problemidparam)
   ENDIF
   CALL echo(build("ENDPOS:",endpos))
   IF (startpos < endpos)
    SET param = substring(1,endpos,problemidparam)
    CALL echo(build("PARAM:",param))
    SET result->problem_cnt = (result->problem_cnt+ 1)
    SET stat = alterlist(result->problem,result->problem_cnt)
    SET result->problem[result->problem_cnt].problem_id = cnvtreal(param)
   ENDIF
   SET problemidparam = substring((endpos+ 2),(size(problemidparam) - endpos),problemidparam)
   CALL echo(build("PROBLEMIDPARAM:",problemidparam))
   CALL echo(build("SIZE(PROBLEMIDPARAM):",size(problemidparam)))
 ENDWHILE
 ...
 SELECT INTO "NL:"
  ...
  HEAD p.problem_id
   ...
   IF (pos > 0)
    ...
    result->problem[pos].annotated_display = p.annotated_display, result->problem[pos].problem_ftdesc
     = p.problem_ftdesc, result->problem[pos].classification_cd = p.classification_cd,
    result->problem[pos].confirmation_status_cd = p.confirmation_status_cd, result->problem[pos].
    qualifier_cd = p.qualifier_cd, result->problem[pos].life_cycle_status_cd = p.life_cycle_status_cd,
    ...
    result->problem[pos].ranking_cd = p.ranking_cd, result->problem[pos].probability = p.probability,
    result->problem[pos].onset_dt_flag = p.onset_dt_flag,
    ...
    result->problem[pos].person_aware_prognosis_cd = p.person_aware_prognosis_cd, result->problem[pos
  ...
  HEAD ppr.problem_prsnl_id
   IF (pos > 0
    ...
    problem[pos].problem_prsnl[ppr_cnt].problem_prsnl_id = ppr.problem_prsnl_id, result->problem[pos]
    .problem_prsnl[ppr_cnt].problem_reltn_prsnl_id = ppr.problem_reltn_prsnl_id,
    result->problem[pos].problem_prsnl[ppr_cnt].beg_effective_dt_tm = ppr.beg_effective_dt_tm, result
    ->problem[pos].problem_prsnl[ppr_cnt].end_effective_dt_tm = ppr.end_effective_dt_tm
   ENDIF
  WITH nocounter, time = 30
 ;end select
 FOR (idx = 1 TO result->problem_cnt)
   IF ((result->problem[idx].person_id != result->problem[1].person_id))
    CALL echo("PROBLEM ID INVALID: MUST BELONG TO SAME PERSON...EXITING")
    GO TO exit_script
   ENDIF
 ENDFOR
  ...
  SELECT INTO value(moutputdevice)
   ...
   HEAD REPORT
    ...
    FOR (idx = 1 TO size(result->problem,5))
      ...
      row + 1, v3 = build("<ReviewDtTm>",format(result->problem[idx].review_dt_tm,
        "MM/DD/YYYY HH:MM;;D"),"</ReviewDtTm>"), col + 1,
      v3, row + 1, col + 1,
      "</Problem>", row + 1
    ENDFOR
