CREATE PROGRAM acm_chg_encntr_alias_hist:dba
 ...
 RECORD temp_record(
   1 encntr_alias_hist_qual[*]
     2 encntr_alias_hist_id = f8
 ...
 DECLARE tracking_bit = i4 WITH noconstant(1)
 ...
 ENDFOR
 SET stat = alterlist(temp_record->encntr_alias_hist_qual,xref->chg_cnt)
 SET eah_qual_cntr = 0
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(xref->chg_cnt))
  PLAN (d)
  DETAIL
   item_idx = xref->chg[d.seq].idx, chg_str = acm_request->encntr_alias_qual[item_idx].chg_str,
   chgbit = 0
   IF (findstring("ALIAS,",chg_str) != 0)
    chgbit = bor(1,chgbit)
   ENDIF
   IF (chgbit > 0)
    eah_qual_cntr = (eah_qual_cntr+ 1), temp_record->encntr_alias_hist_qual[eah_qual_cntr].change_bit
     = chgbit, temp_record->encntr_alias_hist_qual[eah_qual_cntr].item_idx = item_idx,
    temp_record->encntr_alias_hist_qual[eah_qual_cntr].encntr_alias_id = acm_request->
    encntr_alias_qual[item_idx].encntr_alias_id, temp_record->encntr_alias_hist_qual[eah_qual_cntr].
    encntr_id = acm_request->encntr_alias_qual[item_idx].encntr_id, temp_record->
    encntr_alias_hist_qual[eah_qual_cntr].pm_hist_tracking_id = acm_request->encntr_alias_qual[
    item_idx].pm_hist_tracking_id,
    temp_record->encntr_alias_hist_qual[eah_qual_cntr].transaction_dt_tm = acm_request->
    encntr_alias_qual[item_idx].transaction_dt_tm, temp_record->encntr_alias_hist_qual[eah_qual_cntr]
    .alias = acm_request->encntr_alias_qual[item_idx].alias
   ELSE
    reply->encntr_alias_qual[item_idx].status = 1
   ENDIF
  WITH nocounter
 ;end select
 SET stat = alterlist(temp_record->encntr_alias_hist_qual,eah_qual_cntr)
 IF (eah_qual_cntr > 0)
  INSERT  FROM encntr_alias_hist eah,
    (dummyt d  WITH seq = value(eah_qual_cntr))
   SET eah.encntr_alias_hist_id = seq(encounter_seq,nextval), eah.encntr_alias_id = temp_record->
    encntr_alias_hist_qual[d.seq].encntr_alias_id, eah.encntr_id = temp_record->
    encntr_alias_hist_qual[d.seq].encntr_id,
    eah.pm_hist_tracking_id = temp_record->encntr_alias_hist_qual[d.seq].pm_hist_tracking_id, eah
    .transaction_dt_tm = cnvtdatetime(temp_record->encntr_alias_hist_qual[d.seq].transaction_dt_tm),
    eah.alias = temp_record->encntr_alias_hist_qual[d.seq].alias,
    eah.tracking_bit = tracking_bit, eah.change_bit = temp_record->encntr_alias_hist_qual[d.seq].
    change_bit, eah.active_ind = 1,
    eah.active_status_cd = reqdata->active_status_cd, eah.active_status_dt_tm = cnvtdatetime(curdate,
     curtime3), eah.active_status_prsnl_id = reqinfo->updt_id,
    eah.updt_dt_tm = cnvtdatetime(curdate,curtime3), eah.updt_id = reqinfo->updt_id, eah.updt_applctx
     = reqinfo->updt_applctx,
    eah.updt_task = reqinfo->updt_task, eah.updt_cnt = updt_cnt
   PLAN (d)
    JOIN (eah)
   WITH nocounter, status(reply->encntr_alias_qual[temp_record->encntr_alias_hist_qual[d.seq].
    item_idx].status)
  ;end insert
 ENDIF
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->encntr_alias_qual[xref->chg[index].idx].status=0))
    ...
    SET table_name = "ENCNTR_ALIAS_HIST"
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
