CREATE PROGRAM bhs_cc_acog_genview:dba
 ...
 SELECT INTO "NL:"
  ...
  PLAN (dfa
   ...
   WHERE dfac.parent_entity_id=ce.parent_event_id
    ...
    AND  NOT (ce.event_title_text IN (work->form_desc, "ACOG History")))
   JOIN (dsr
 ...
 IF ((work->s_cnt=0))
  ...
  GO TO print_output
 ENDIF
 SET reply->text = build2(beg_doc,"\fs20\b\ul ACOG History\b0\ul0\par",newline)
