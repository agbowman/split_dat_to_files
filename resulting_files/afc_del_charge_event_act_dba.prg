CREATE PROGRAM afc_del_charge_event_act:dba
 ...
 CALL del_charge_event_act(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE del_charge_event_act(del_begin,del_end)
   FOR (x = del_begin TO del_end)
     SET active_code = 0.0
     IF ((request->charge_event_act[x].active_status_cd=0))
      SELECT INTO "nl:"
       FROM code_value c
       WHERE c.code_set=48
     ...
     UPDATE  FROM charge_event_act c
      SET c.active_ind = false, c.active_status_cd = nullcheck(active_code,request->charge_event_act[
        x].active_status_cd,
        IF ((request->charge_event_act[x].active_status_cd=0)) 0
        ELSE 1
        ENDIF
        ), c.active_status_prsnl_id = reqinfo->updt_id,
       c.active_status_dt_tm = cnvtdatetime(curdate,curtime), c.updt_cnt = (c.updt_cnt+ 1), c
