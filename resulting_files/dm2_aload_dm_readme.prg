CREATE PROGRAM dm2_aload_dm_readme
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_dm_readme",".log","dm2_aload_dm_readme")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dm_readme"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dadr_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dadr_file = vc WITH protect, noconstant( $2)
 DECLARE dadr_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dadr_batch = i4 WITH protect, noconstant(0)
 DECLARE dadr_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dadr_pkg,dadr_file_loc_ret,dadr_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dadr_batch=0)
  SET dadr_batch = 5000
 ENDIF
 IF (dadr_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dadr_file_loc_ret,dadr_file), "dm2_aload_rdm_dr", dadr_batch
  CALL echo(concat(dadr_file_loc_ret,dadr_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dm_readme completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_dm_readme")
END GO
