CREATE PROGRAM dm2_export_dbstats_ind:dba
 ...
 SUBROUTINE ddf_context_lock_maint(dclm_mode,dclm_process,dclm_process_code,dclm_owner,
  dclm_table_name,dclm_object_id,dclm_lock_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dedi_inowner = vc WITH protect, noconstant(" ")
 DECLARE dedi_inindex = vc WITH protect, noconstant(" ")
 DECLARE dedi_in_to = vc WITH protect, noconstant(" ")
 DECLARE dedi_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dedi_step_beg_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dedi_step_end_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dedi_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dedi_emsg = vc WITH protect, noconstant(" ")
 DECLARE export_index_stats(ownname=vc,indname=vc,stattab=vc,statid=vc,statown=vc) = null WITH sql =
 "SYS.DBMS_STATS.EXPORT_INDEX_STATS", parameter
 DECLARE delete_index_stats(ownname=vc,indname=vc,stattab=vc,statid=vc,statown=vc) = null WITH sql =
 "SYS.DBMS_STATS.DELETE_INDEX_STATS", parameter
 IF (check_logfile("dm2_edbs_ind",".log","dm2_export_dbstats_ind LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_export_dbstats_ind"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dedi_inowner = trim(cnvtupper( $1),3)
 SET dedi_inindex = trim( $2,3)
 SET dedi_in_to = trim(cnvtupper( $3),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = concat("Parameter usage: dm2_export_dbstats_ind  '<index_owner>'",
   "'<index_name>','<TO>:[STATID]''")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_add_detail_text("OWNER",dedi_inowner)
 CALL dm2_process_log_add_detail_text("INDEX_NAME",dedi_inindex)
 CALL dm2_process_log_add_detail_text("INDEX_OWNER.INDEX_NAME",concat(dedi_inowner,".",dedi_inindex))
 CALL dm2_process_log_add_detail_text("TO",dedi_in_to)
 ...
 SET dedi_log_id = dm2_process_event_rs->dm_process_event_id
 ...
 SELECT INTO "nl:"
  ...
  WHERE d.owner=dedi_inowner
   AND d.index_name=dedi_inindex
   AND d.num_rows IS NOT null
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSEIF (curqual=0)
  SET dm_err->eproc = build("AutoSuccessing: No statistics found in DBA_IND_STATISTICS for (",
   dedi_inowner,".",dedi_inindex,").")
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = build("Deleting Index Stats from DM_STAT_TABLE for (",dedi_inowner,".",
  dedi_inindex,"), STATID (",
  dedi_in_to,")")
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 SET dedi_step_beg_dttm = cnvtdatetime(curdate,curtime3)
 CALL delete_index_stats(dedi_inowner,concat('"',dedi_inindex,'"'),"DM_STAT_TABLE",dedi_in_to,"V500")
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dedi_step_end_dttm = cnvtdatetime(curdate,curtime3)
 CALL dm2_process_log_add_detail_number("DELETE_INDEX_STATS:SECS",datetimediff(dedi_step_end_dttm,
   dedi_step_beg_dttm,5))
 IF (dm2_process_log_dtl_row(dedi_log_id,0)=0)
  GO TO exit_script
 ENDIF
 SET dedi_step_beg_dttm = cnvtdatetime(curdate,curtime3)
 SET dm_err->eproc = build("Exporting LIVE Stats to DM_STAT_TABLE for (",dedi_inowner,".",
  dedi_inindex,") using STATID (",
  dedi_in_to,")")
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 CALL export_index_stats(dedi_inowner,concat('"',dedi_inindex,'"'),"DM_STAT_TABLE",dedi_in_to,"V500")
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dedi_step_end_dttm = cnvtdatetime(curdate,curtime3)
 CALL dm2_process_log_add_detail_number("EXPORT_INDEX_STATS:SECS",datetimediff(dedi_step_end_dttm,
   dedi_step_beg_dttm,5))
 IF (dm2_process_log_dtl_row(dedi_log_id,0)=0)
  GO TO exit_script
 ENDIF
#exit_script
 ...
 SET dedi_err_ind = dm_err->err_ind
 SET dedi_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_execution,dedi_log_id,1)
 SET dm_err->err_ind = dedi_err_ind
 SET dm_err->emsg = dedi_emsg
 SET dm_err->eproc = "Ending dm2_export_dbstats_ind"
 CALL final_disp_msg("dm2_edbs_ind")
END GO
