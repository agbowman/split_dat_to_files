CREATE PROGRAM bb_rdm_updt_code_values:dba
 ...
 SUBROUTINE insertreqproc(nrequest,spfmt,ntargetreq,ndeststep,sservice,nreprocess,naction,srefpfmt,
  nseq)
   ...
   ENDCASE
 END ;Subroutine
 DECLARE script_name = c23 WITH protect, constant("bb_rdm_updt_code_values")
 DECLARE error_msg = c255 WITH protect, noconstant(fillstring(255," "))
 DECLARE error_status = i2 WITH protect, noconstant(0)
 DECLARE list_count = i4 WITH protect, noconstant(0)
 DECLARE i_idx = i4 WITH protect, noconstant(0)
 DECLARE stat = i2 WITH protect, noconstant(0)
 CALL logscriptstart(script_name)
 SET error_status = error(error_msg,1)
 SET list_count = size(requestin->list_0,5)
 IF (list_count=0)
  CALL logmsg("No records read from input file.  Nothing to process.",(e_dispmsg+ e_logtofile))
  GO TO script_end
 ENDIF
 FOR (i_idx = 1 TO list_count)
   SELECT
    IF (size(trim(requestin->list_0[i_idx].cki)) > 0)
     WHERE c.code_set=cnvtint(trim(requestin->list_0[i_idx].code_set))
      AND c.cdf_meaning=trim(requestin->list_0[i_idx].cdf_meaning)
      AND c.cki=trim(requestin->list_0[i_idx].cki)
    ELSE
     WHERE c.code_set=cnvtint(trim(requestin->list_0[i_idx].code_set))
      AND c.cdf_meaning=trim(requestin->list_0[i_idx].cdf_meaning)
    ENDIF
   ...
   ;end select
   IF (curqual != 1)
    CALL logmsg("Error retrieving code_value information.",(e_dispmsg+ e_logtofile))
    CALL logstatus(build(script_name," Execution failed - unable to retrieve code_value information."
      ),"F")
    GO TO script_end
   ENDIF
   EXECUTE gm_code_value0619_def "U"
   ...
   SET gm_u_code_value0619_req->force_updt_ind = 1
   SET stat = 1
   IF (size(trim(requestin->list_0[i_idx].display)) > 0)
    SET stat = gm_u_code_value0619_vc("DISPLAY",trim(requestin->list_0[i_idx].display),1,0,0)
   ENDIF
   IF (stat=1
    AND size(trim(requestin->list_0[i_idx].description)) > 0)
    SET stat = gm_u_code_value0619_vc("DESCRIPTION",trim(requestin->list_0[i_idx].description),1,0,0)
   ENDIF
   IF (stat=1
    AND size(trim(requestin->list_0[i_idx].definition)) > 0)
    SET stat = gm_u_code_value0619_vc("DEFINITION",trim(requestin->list_0[i_idx].definition),1,0,0)
   ENDIF
   IF (stat=1
    AND ((trim(requestin->list_0[i_idx].active_ind)="1") OR (trim(requestin->list_0[i_idx].active_ind
    )="0")) )
    SET stat = gm_u_code_value0619_i2("ACTIVE_IND",cnvtint(trim(requestin->list_0[i_idx].active_ind)),
     1,0,0)
   ENDIF
   IF (stat=1
    AND trim(requestin->list_0[i_idx].active_ind)="1")
    SET stat = gm_u_code_value0619_f8("ACTIVE_TYPE_CD",reqdata->active_status_cd,1,0,0)
   ELSEIF (stat=1
    AND trim(requestin->list_0[i_idx].active_ind)="0")
    SET stat = gm_u_code_value0619_f8("ACTIVE_TYPE_CD",reqdata->inactive_status_cd,1,0,0)
   ENDIF
   IF (stat=1
    AND trim(requestin->list_0[i_idx].active_ind)="1")
    SET stat = gm_u_code_value0619_dq8("ACTIVE_DT_TM",cnvtdatetime(curdate,curtime3),1,0,0)
   ENDIF
   IF (stat=1
    AND trim(requestin->list_0[i_idx].active_ind)="1")
    SET stat = gm_u_code_value0619_dq8("BEGIN_EFFECTIVE_DT_TM",cnvtdatetime(curdate,curtime3),1,0,0)
   ENDIF
   IF (stat=1
    AND trim(requestin->list_0[i_idx].active_ind)="0")
    SET stat = gm_u_code_value0619_dq8("INACTIVE_DT_TM",cnvtdatetime(curdate,curtime3),1,0,0)
   ELSEIF (stat=1
    AND trim(requestin->list_0[i_idx].active_ind)="1")
    SET stat = gm_u_code_value0619_dq8("INACTIVE_DT_TM",cnvtdatetime(0,0),1,0,0)
   ENDIF
   IF (stat=1
    AND trim(requestin->list_0[i_idx].active_ind)="0")
    SET stat = gm_u_code_value0619_dq8("END_EFFECTIVE_DT_TM",cnvtdatetime(curdate,curtime3),1,0,0)
   ELSEIF (stat=1
    AND trim(requestin->list_0[i_idx].active_ind)="1")
    SET stat = gm_u_code_value0619_dq8("END_EFFECTIVE_DT_TM",cnvtdatetime("31-DEC-2100"),1,0,0)
   ENDIF
   IF (stat=1)
    SET stat = gm_u_code_value0619_i4("CODE_SET",cnvtint(trim(requestin->list_0[i_idx].code_set)),1,0,
     1)
   ENDIF
   IF (stat=1)
    SET stat = gm_u_code_value0619_vc("CDF_MEANING",trim(requestin->list_0[i_idx].cdf_meaning),1,0,1)
   ENDIF
   IF (stat=1
    AND size(trim(requestin->list_0[i_idx].cki)) > 0)
    SET stat = gm_u_code_value0619_vc("CKI",requestin->list_0[i_idx].cki,1,0,1)
   ENDIF
   IF (stat=1)
   ...
   ELSE
    ROLLBACK
    CALL logstatus(build(script_name," Execution Failed.  Unable to update code_set ",requestin->
      list_0[i_idx].code_set,"."),"F")
    GO TO script_end
   ENDIF
   FREE RECORD gm_u_code_value0619_req
   FREE RECORD gm_u_code_value0619_rep
 ENDFOR
 CALL logstatus(build(script_name," Execution Successful."),"S")
#script_end
 FREE RECORD gm_u_code_value0619_req
 FREE RECORD gm_u_code_value0619_rep
 CALL logscriptend(script_name)
END GO
