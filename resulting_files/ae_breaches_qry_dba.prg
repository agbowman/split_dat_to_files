CREATE PROGRAM ae_breaches_qry:dba
 PROMPT
...
#ukr_common_rpt_exit
 ...
 IF (stat=c_function_success)
  SET stat = prefacc_addreport("ae_breaches_qry",pref_report_index)
 ...
 SELECT
  IF (pref_dbr_value="yes")
   ...
   week_number = getweeknumber(aedata->qual[d.seq].arrivaltime,0), day_of_week = cnvtint(weekday(
     aedata->qual[d.seq].arrivaltime)), mrn = substring(1,15,trim(aedata->qual[d.seq].mrn_list[1].mrn,
     3)),
   ...
   fin_no = substring(1,25,trim(aedata->qual[d.seq].fin,3)), att_no = aedata->qual[d.seq].attendance,
   attendance_type = substring(1,40,trim(aedata->qual[d.seq].attendancetype,3)),
   referral_source = substring(1,40,trim(aedata->qual[d.seq].referralsource,3)), stream = substring(1,
    40,trim(aedata->qual[d.seq].stream,3)), arrival_time = format(aedata->qual[d.seq].arrivaltime,
    "dd-mmm-yyyy hh:mm;;d"),
   arrival_hour = cnvtint(hour(aedata->qual[d.seq].arrivaltime)), assessment_time = format(aedata->
    qual[d.seq].assessmenttime,"dd-mmm-yyyy hh:mm;;d"), arrival_to_assessment =
   ...
   ENDIF
   ,
   clinician_assign_time = format(aedata->qual[d.seq].cliniciantime,"DD-MMM-YYYY HH:MM;;D"), dta_time
    = format(aedata->qual[d.seq].dtatime,"dd-mmm-yyyy hh:mm;;d"), dta_to_disposal =
   ...
   formattohhmm(aedata->qual[d.seq].arrivaltime,aedata->qual[d.seq].checkouttime),
   breach_reason = substring(1,255,aedata->qual[d.seq].breach_reason), disposition = substring(1,40,
    trim(aedata->qual[d.seq].disposition,3)), site_code = substring(1,5,trim(aedata->qual[d.seq].
     treatment_site_code,3))
  ELSE
   day_of_month = cnvtint(day(aedata->qual[d.seq].arrivaltime)), month_of_year = cnvtint(month(aedata
     ->qual[d.seq].arrivaltime)), year = cnvtint(year(aedata->qual[d.seq].arrivaltime)),
   week_number = getweeknumber(aedata->qual[d.seq].arrivaltime,0), day_of_week = cnvtint(weekday(
   ...
   checkout_personnel = substring(1,50,trim(aedata->qual[d.seq].checkoutpersonnel,3)), los =
   formattohhmm(aedata->qual[d.seq].arrivaltime,aedata->qual[d.seq].checkouttime),
   disposition = substring(1,40,trim(aedata->qual[d.seq].disposition,3)), site_code = substring(1,5,
    trim(aedata->qual[d.seq].treatment_site_code,3))
  ENDIF
  INTO  $OUTDEV
  FROM (dummyt d  WITH seq = listcount)
  PLAN (d
   WHERE cnvtint(datetimediff(aedata->qual[d.seq].checkouttime,aedata->qual[d.seq].arrivaltime,3))
    >= 4)
  WITH nocounter, format, separator = " "
 ;end select
#exit_program
