CREATE PROGRAM ccl_label_dio_2x2
 PROMPT
 ...
 SELECT INTO  $1
  ...
  DETAIL
   ...
   _id, col + 5, stext,
   "{F/1}{CPI/28}", row + 1, stext = concat(_courier," (28)"),
   col 1, "Id:", _id,
   col + 5, stext"{lpi/12}{cpi/28}{font/1}", row + 1,
   stext = concat(_courier," (LPI12,CPI28)"), col 1, "Id:",
   _id, col + 5, stext"{F/5}{CPI/25}",
   col 1, "Id:", _id,
   ...
   stext, row + 1, stext = concat(_timesroman," (20)"),
   col + 3, "{F/5}{CPI/20}", stext,
   row + 2, col 1, "{F/28}{lpi/6}{cpi/16}{bcr/300}{CPI/8}",
   "*39393939*", row + 1, stext = concat(_timesroman," (20)"),
   col 1, "{F/5}{CPI/20}", stext,
   row + 1
