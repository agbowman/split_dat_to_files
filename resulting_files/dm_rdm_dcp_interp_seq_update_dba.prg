CREATE PROGRAM dm_rdm_dcp_interp_seq_update:dba
 ...
 ENDIF
 SUBROUTINE (dctx_set_context(dsc_i_attr_name=vc,dsc_i_value=vc) =i2)
 ...
 SET readme_data->message = "Readme failed: starting script dm_rdm_dcp_interp_seq_update."
 DECLARE errmsg = vc WITH protect, noconstant("")
 DECLARE context_val = vc WITH protect, noconstant("NO")
 DECLARE interpseqval = f8 WITH protect, noconstant(0)
 DECLARE carenetseqval = f8 WITH protect, noconstant(0)
 DECLARE fixinc = f8 WITH protect, noconstant(0)
 DECLARE increment_by = f8 WITH protect, noconstant(0)
 DECLARE sequencename = vc WITH protect, noconstant("DCP_INTERP_SEQ")
 SELECT INTO "nl:"
  d.last_number
  FROM dba_sequences d
  WHERE d.sequence_name=sequencename
  DETAIL
   interpseqval = d.last_number
  WITH nocounter
 ;end select
 IF (error(errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to get next value from ",cnvtupper(sequencename),": ",
   errmsg)
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  d.last_number
  FROM dba_sequences d
  WHERE d.sequence_name="CARENET_SEQ"
  DETAIL
   carenetseqval = d.last_number
  WITH nocounter
 ;end select
 IF (error(errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to get carenet_seq value : ",errmsg)
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  ds.increment_by
 ...
 IF (error(errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to get increment value from ",cnvtupper(sequencename),
   ": ",errmsg)
  GO TO exit_script
 ENDIF
 IF (interpseqval < carenetseqval)
  SET increment_by = ((carenetseqval+ 1000000) - interpseqval)
  SELECT INTO "nl:"
   FROM session_context sc
   WHERE sc.namespace="CERNER"
    AND sc.attribute="DM2_DDLPRIVS"
   DETAIL
    context_val = sc.value
   WITH nocounter
  ;end select
  IF (error(errmsg,0) != 0)
   SET readme_data->status = "F"
   SET readme_data->message = "Failed to get the context value from session_context table"
   GO TO exit_script
  ENDIF
  IF (dctx_set_context("DM2_DDLPRIVS","YES")=0)
   SET readme_data->status = "F"
   SET readme_data->message = "Failed, not able to set the context for value Yes"
   GO TO exit_script
  ENDIF
  IF (setincrement(increment_by)=0)
   SET readme_data->status = "F"
   SET readme_data->message = concat("Failed to increase increment value: ",errmsg)
   GO TO exit_script
  ENDIF
  SELECT INTO "nl:"
   seqval = parser(concat("seq(",cnvtupper(sequencename),", nextval)"))
   FROM dual
   DETAIL
    interpseqval = seqval
   WITH nocounter
  ...
  IF (setincrement(fixinc)=0)
   SET readme_data->status = "F"
   SET readme_data->message = concat("Failed to reset increment value: ",errmsg)
   GO TO exit_script
  ENDIF
  IF (dctx_set_context("DM2_DDLPRIVS",context_val)=0)
   SET readme_data->status = "F"
   SET readme_data->message =
   "Failed, not able to set the context for value No within the main block"
   GO TO exit_script
  ENDIF
 ENDIF
...
#exit_script
 IF (dctx_set_context("DM2_DDLPRIVS",context_val)=0)
  SET readme_data->status = "F"
  SET readme_data->message = "Failed, not able to set the context - outside of exit block"
  GO TO exit_script
 ENDIF
 CALL echorecord(readme_data)
 EXECUTE dm_readme_status
 SUBROUTINE (setincrement(increment=f8) =i2)
   CALL parser(concat("rdb asis(^ alter sequence ",sequencename," increment by ",cnvtstring(increment
      ),"^) go"))
   IF (error(errmsg,0) != 0)
    RETURN(0)
   ENDIF
