CREATE PROGRAM bhs_rpt_surg_sched_audit:dba
 PROMPT
  ...
  ms_end_dt
 DECLARE ms_beg_dt_tm = vc WITH protect, constant(concat( $MS_BEG_DT," 00:00:00"))
 DECLARE ms_end_dt_tm = vc WITH protect, constant(concat( $MS_END_DT," 23:59:59"))
 ...
 SUBROUTINE (sec_headabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Date Range Entered : ",char(0)))
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.313)
 ...
 SUBROUTINE (sec_fields1abs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Sched Date",char(0)))
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.001)
 ...
 SUBROUTINE (sec_body2abs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __apptdt = vc WITH noconstant(build2(m_case->qual[ml_idx1].s_start_date,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET rptsd->m_width = 0.688
    SET rptsd->m_height = 0.251
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__apptdt)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 ...
 SUBROUTINE (sec_body1abs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __datetime = vc WITH noconstant(build2(m_case->qual[ml_idx1].s_sched_date,char(0))),
   protect
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (sec_scheduler(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = sec_schedulerabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (sec_schedulerabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.200000), private
   DECLARE __schedname = vc WITH noconstant(build2(concat(m_case->qual[ml_idx1].s_sched_l_name,",",
      m_case->qual[ml_idx1].s_sched_f_name),char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET rptsd->m_height = 0.188
    SET _dummyfont = uar_rptsetfont(_hreport,_times80)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__schedname)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_RPT_SURG_SCHED_AUDIT"
 ...
 SELECT INTO "nl:"
  ...
  ps_sched_dt = format(sea.action_dt_tm,"MMDDYYYY;;D")
  ...
  PLAN (sea
   ...
   WHERE sc.sch_event_id=sea.sch_event_id
    AND (sc.sched_surg_area_cd= $MF_SCHED_LOC_CD)
    AND sc.active_ind=1
  ...
  ORDER BY ps_sched_dt, pr.name_last_key, pr.name_first_key,
   ps_or_room
  HEAD REPORT
   m_case->l_cnt = 0
  HEAD sc.surg_case_nbr_formatted
   ...
   m_case->qual[m_case->l_cnt].s_sched_date = format(sea.action_dt_tm,"MM/DD/YYYY;;D"), m_case->qual[
   m_case->l_cnt].f_sched_date = sea.action_dt_tm, m_case->qual[m_case->l_cnt].s_sched_f_name = trim(
    pr.name_first_key,3),
   m_case->qual[m_case->l_cnt].s_sched_l_name = trim(pr.name_last_key,3), m_case->qual[m_case->l_cnt]
   .s_case_nbr = sc.surg_case_nbr_formatted, m_case->qual[m_case->l_cnt].s_start_date = format(sc
    .sched_start_dt_tm,"MM/DD/YYYY;;D"),
 ...
 IF ((m_case->l_cnt > 0))
  SET ms_day = concat("Date Scheduled : ",format(m_case->qual[1].f_sched_date,"WWWWWWWWW;;D")," ",
   m_case->qual[1].s_sched_date)
  SET d0 = sec_day(rpt_render)
  SET ms_prev_date = m_case->qual[1].s_sched_date
 ...
 SET d0 = sec_line1(rpt_render)
 FOR (ml_idx1 = 1 TO m_case->l_cnt)
   SET ms_day = concat("Date Scheduled : ",format(m_case->qual[ml_idx1].f_sched_date,"WWWWWWWWW;;D"),
    " ",m_case->qual[ml_idx1].s_sched_date)
   SET ms_cur_date = m_case->qual[ml_idx1].s_sched_date
   ...
   SET mf_rem_space = (mf_page_size - (((((_yoffset+ sec_body1(rpt_calcheight))+ sec_line2(
    rpt_calcheight))+ sec_scheduler(rpt_calcheight))+ sec_comment(rpt_calcheight,5.0,ml_continue))+ (
   0.63 * m_case->qual[ml_idx1].p_cnt)))
   IF (mf_rem_space <= 0.5)
    SET _yoffset = 10.18
    SET d0 = sec_foot(rpt_render)
    SET d0 = pagebreak(0)
   ...
   SET d0 = sec_scheduler(rpt_render)
