CREATE PROGRAM dac_create_a_nls_triggers:dba
 ...
 IF (sbr_check_for_a_nls(null)=0)
  ...
  GO TO exit_script
 ENDIF
 FREE RECORD dcant_columns
 RECORD dcant_columns(
   1 columns[*]
     2 column_name = vc
     2 column_length = i4
   1 nls_columns[*]
 ...
 DECLARE dcant_tablename = vc WITH protect, noconstant("")
 DECLARE dcant_suffixedtablename = vc WITH protect, noconstant("")
 DECLARE dcant_columncount = i4 WITH protect, noconstant(0)
 DECLARE dcant_nlscolumncount = i4 WITH protect, noconstant(0)
 DECLARE dcant_columnidx = i4 WITH protect, noconstant(0)
 DECLARE dcant_nlscolumnname = vc WITH protect, noconstant("")
 DECLARE dcant_triggername = vc WITH protect, noconstant("")
 DECLARE dcant_createtriggerstmt = vc WITH protect, noconstant("")
 DECLARE dcant_nlscolumn_when_clause = vc WITH protect, noconstant("")
 DECLARE dcant_nlslang = vc WITH protect, constant(sbr_get_nls_lang(null))
 DECLARE dcant_errmsg = vc WITH protect, noconstant("")
 DECLARE dcant_loop = i4 WITH protect, noconstant(0)
 DECLARE dcant_lvalidx = i4 WITH protect, noconstant(0)
 DECLARE dcant_trg_drop_ind = i2 WITH protect, noconstant(0)
 DECLARE dcant_sd_in_use_ind = i2 WITH protect, noconstant(0)
 DECLARE dcant_trig_version = i4 WITH protect, noconstant(- (1))
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 IF (error(dcant_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to look up CERN_NLS_SORT: ",dcant_errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  SET readme_data->status = "F"
  SET readme_data->message = "CERN_NLS_SORT function does not exist"
  GO TO exit_script
 ENDIF
 IF (dsr_sd_in_use_check("DM2NOTSET",dcant_sd_in_use_ind)=0)
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
  GO TO exit_script
 ENDIF
 IF (dcant_sd_in_use_ind=1)
  ...
  ENDIF
 ENDIF
 SET dcant_tablename = cnvtupper(trim( $1,3))
 SELECT INTO "nl:"
  FROM dm_tables_doc dtd
  WHERE dtd.table_name=dcant_tablename
  DETAIL
   dcant_suffixedtablename = dtd.suffixed_table_name
  WITH nocounter
 ;end select
 IF (error(dcant_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to look up suffixed table name: ",dcant_errmsg)
  GO TO exit_script
 ELSEIF (((curqual=0) OR (dcant_suffixedtablename <= " ")) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("No suffixed table name found for table '",dcant_tablename,"'")
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM user_tab_columns utc
  WHERE utc.table_name=dcant_tablename
  ORDER BY utc.column_id
  DETAIL
   dcant_columncount = (dcant_columncount+ 1)
   IF (mod(dcant_columncount,10)=1)
    stat = alterlist(dcant_columns->columns,(dcant_columncount+ 9))
   ENDIF
   dcant_columns->columns[dcant_columncount].column_name = utc.column_name, dcant_columns->columns[
   dcant_columncount].column_length = utc.data_length
  FOOT REPORT
   stat = alterlist(dcant_columns->columns,dcant_columncount)
  WITH nocounter
 ;end select
 IF (error(dcant_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to find columns for table '",dcant_tablename,"': ",
   dcant_errmsg)
  GO TO exit_script
 ELSEIF (dcant_columncount=0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("No columns found for table '",dcant_tablename,"'")
  GO TO exit_script
 ENDIF
 FOR (dcant_loop = 1 TO dcant_columncount)
   IF ((dcant_columns->columns[dcant_loop].column_name != patstring("*_NLS")))
    SET dcant_nlscolumnname = concat(dcant_columns->columns[dcant_loop].column_name,"_A_NLS")
    SET dcant_columnidx = locateval(dcant_lvalidx,1,dcant_columncount,dcant_nlscolumnname,
     dcant_columns->columns[dcant_lvalidx].column_name)
    IF (dcant_columnidx > 0)
     SET dcant_nlscolumncount = (dcant_nlscolumncount+ 1)
     SET stat = alterlist(dcant_columns->nls_columns,dcant_nlscolumncount)
     SET dcant_columns->nls_columns[dcant_nlscolumncount].base_column_name = dcant_columns->columns[
     dcant_loop].column_name
     SET dcant_columns->nls_columns[dcant_nlscolumncount].nls_column_name = dcant_nlscolumnname
     SET dcant_columns->nls_columns[dcant_nlscolumncount].nls_column_length = dcant_columns->columns[
     dcant_columnidx].column_length
    ENDIF
   ENDIF
 ENDFOR
 IF (error(dcant_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed in matching of _A_NLS column names: ",dcant_errmsg)
  GO TO exit_script
 ENDIF
 IF (dcant_nlscolumncount=0)
  SET readme_data->status = "F"
  SET readme_data->message = "No _A_NLS columns matched"
  GO TO exit_script
 ENDIF
 FOR (dcant_loop = 1 TO dcant_nlscolumncount)
   SET dcant_triggername = build("TRG_",dcant_suffixedtablename,"_ANLS",dcant_loop)
   SET dcant_nlscolumn_when_clause = build("*OLD.",cnvtupper(dcant_columns->nls_columns[dcant_loop].
     nls_column_name),"*")
   SET dcant_trig_version = - (1)
   IF (dsr_sd_get_trigger_version(currdbuser,"TRG*ANLS*",dcant_tablename,dcant_nlscolumn_when_clause,
    dcant_trig_version)=0)
    SET readme_data->status = "F"
    SET readme_data->message = dm_err->emsg
    GO TO exit_script
   ENDIF
   SET dcant_trg_drop_ind = 0
   IF (dcant_sd_in_use_ind=1
    AND dcant_trig_version <= 0)
    IF (dsr_check_object_state(currdbuser,"TRIGGER",dcant_triggername,"DROPPED",dcant_trg_drop_ind)=0
    )
     SET readme_data->status = "F"
     SET readme_data->message = dm_err->emsg
     GO TO exit_script
    ENDIF
   ENDIF
   IF (((dcant_trig_version > 0) OR (dcant_trg_drop_ind=1)) )
    CALL echo(concat("Schema Deployment now managing trigger ",dcant_triggername))
   ELSE
    SET dcant_createtriggerstmt = concat("CREATE OR REPLACE TRIGGER ",dcant_triggername,char(10),
     "BEFORE INSERT OR UPDATE OF ",dcant_columns->nls_columns[dcant_loop].base_column_name,
     " ON ",dcant_tablename,char(10),"FOR EACH ROW ",char(10),
     "WHEN ((old.",dcant_columns->nls_columns[dcant_loop].nls_column_name," IS NULL) OR ","(old.",
     dcant_columns->nls_columns[dcant_loop].base_column_name,
     " <> ","new.",dcant_columns->nls_columns[dcant_loop].base_column_name,"))",char(10),
     "BEGIN ",char(10),"  :new.",dcant_columns->nls_columns[dcant_loop].nls_column_name," := ",
     " substr(cern_nls_sort(:new.",dcant_columns->nls_columns[dcant_loop].base_column_name,
     ", 'NLS_SORT=",dcant_nlslang,"_AI'), ",
     "1, ",build(dcant_columns->nls_columns[dcant_loop].nls_column_length),");",char(10),"end;")
    SET dcant_createtriggerstmt = concat("rdb asis(^ ",dcant_createtriggerstmt," ^) go")
    CALL parser(dcant_createtriggerstmt)
    IF (error(dcant_errmsg,0) != 0)
     SET readme_data->status = "F"
     SET readme_data->message = concat("Failed to build '",dcant_triggername,"': ",dcant_errmsg)
     GO TO exit_script
    ENDIF
    EXECUTE dm_readme_include_sql_chk value(dcant_triggername), "TRIGGER"
    IF ((dm_sql_reply->status != "S"))
     SET readme_data->status = "F"
 ...
 ENDFOR
 SET readme_data->status = "S"
 SET readme_data->message = concat("A_NLS triggers created for table '",dcant_tablename,"'")
#exit_script
 FREE RECORD dcant_columns
END GO
