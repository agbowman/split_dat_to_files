CREATE PROGRAM dm2_rdds_reset_child:dba
 ...
 UPDATE  FROM dm_chg_log d
  ...
  WHERE ((d.target_env_id+ 0)=from_id)
   AND d.log_id >= drcc_min_cnt
   AND (d.log_id < (drcc_min_cnt+ 3000))
   AND cnvtupper(d.log_type) IN ("MERGED", "PROCES")
  WITH nocounter
 ;end update
 IF (check_error("ERROR: Reseting data in DM_CHG_LOG ") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET copy_err_ind = 1
  GO TO exit_child
 ELSE
 ...
 UPDATE  FROM dm_chg_log d
  ...
  WHERE ((d.target_env_id+ 0)=from_id)
   AND d.log_id >= drcc_min_cnt
   AND (d.log_id < (drcc_min_cnt+ 3000))
   AND cnvtupper(d.log_type)="NOMV00"
  WITH nocounter
 ;end update
 IF (check_error("ERROR: Reseting data in DM_CHG_LOG ") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
