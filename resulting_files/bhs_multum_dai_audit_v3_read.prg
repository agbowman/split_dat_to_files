CREATE PROGRAM bhs_multum_dai_audit_v3_read
 PROMPT
 ...
 CALL echo(build("cat_code_size:",size(cat_codes->list,5)))
 CALL echo(cat_codes->list[1].catalog_cd)
 CALL echo(cat_codes->list[2].catalog_cd)
 CALL echo(cat_codes->list[3].catalog_cd)
 CALL echo(cat_codes->list[4].catalog_cd)
 CALL echo(cat_codes->list[5].catalog_cd)
 SET min_oid = 0
 SET max_oid = 0
 SELECT INTO "nl:"
  ...
  FOOT REPORT
   min_oid = min(oa.order_id), max_oid = max(oa.order_id)
  WITH nocounter
 ;end select
 FOR (x = 1 TO size(cat_codes->list,5))
  CALL echo(build("cat code:",cat_codes->list[x].catalog_cd))
  SELECT INTO "nl:"
   FROM orders o
   PLAN (o
    WHERE o.order_id >= min_oid
     AND o.order_id < max_oid
     AND ((o.catalog_cd+ 0)=cat_codes->list[x].catalog_cd))
   FOOT REPORT
    cat_codes->list[x].cnt = count(o.order_id)
   WITH nocounter
  ;end select
 ENDFOR
 CALL echo("end of order catalog")
 SELECT INTO "nl:"
  FROM long_text lt,
   (dummyt d  WITH seq = value(size(eks->list,5)))
  PLAN (d)
 ...
 FOR (l = 1 TO 4)
   ...
   ;end select
   IF (email_ind=1)
    SET dclcom = concat("$cust_script/bhs_ftp_file.ksh ",output_destfile,
