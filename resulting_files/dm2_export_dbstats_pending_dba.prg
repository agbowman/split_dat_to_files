CREATE PROGRAM dm2_export_dbstats_pending:dba
 ...
 SUBROUTINE ddf_context_lock_maint(dclm_mode,dclm_process,dclm_process_code,dclm_owner,
  dclm_table_name,dclm_object_id,dclm_lock_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dedp_inowner = vc WITH protect, noconstant(" ")
 DECLARE dedp_intable = vc WITH protect, noconstant(" ")
 DECLARE dedp_in_from = vc WITH protect, noconstant(" ")
 DECLARE dedp_in_to = vc WITH protect, noconstant(" ")
 DECLARE dedp_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dedp_step_beg_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dedp_step_end_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dedp_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dedp_emsg = vc WITH protect, noconstant(" ")
 DECLARE export_pending_stats(ownname=vc,tabname=vc,stattab=vc,statid=vc,statown=vc) = null WITH sql
 ...
 IF (check_logfile("dm2_edbs_pend",".log","dm2_export_dbstats_pending LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_export_dbstats_pending"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dedp_inowner = trim(cnvtupper( $1),3)
 SET dedp_intable = trim( $2,3)
 SET dedp_in_to = trim(cnvtupper( $3),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = concat("Parameter usage: dm2_export_dbstats_pending  '<table_owner>'",
   "'<table_name>','<TO>:[STATID]'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_add_detail_text("OWNER",dedp_inowner)
 CALL dm2_process_log_add_detail_text("TABLE_NAME",dedp_intable)
 CALL dm2_process_log_add_detail_text("TABLE_OWNER.TABLE_NAME",concat(dedp_inowner,".",dedp_intable))
 CALL dm2_process_log_add_detail_text("TO",dedp_in_to)
 ...
 SET dedp_log_id = dm2_process_event_rs->dm_process_event_id
 IF (ddf_autosuccess_init(null)=0)
  GO TO exit_script
 ELSEIF ((dgdt_prefs->autosuccess_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = build("Deleting Table Stats from DM_STAT_TABLE for (",dedp_inowner,".",
  dedp_intable,"), STATID (",
  dedp_in_to,")")
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 SET dedp_step_beg_dttm = cnvtdatetime(curdate,curtime3)
 CALL delete_table_stats(dedp_inowner,concat('"',dedp_intable,'"'),"DM_STAT_TABLE",dedp_in_to,"V500",
  cnvtbool(true))
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dedp_step_end_dttm = cnvtdatetime(curdate,curtime3)
 CALL dm2_process_log_add_detail_number("DELETE_TABLE_STATS:SECS",datetimediff(dedp_step_end_dttm,
   dedp_step_beg_dttm,5))
 IF (dm2_process_log_dtl_row(dedp_log_id,0)=0)
  GO TO exit_script
 ENDIF
 SET dedp_step_beg_dttm = cnvtdatetime(curdate,curtime3)
 SET dm_err->eproc = build("Exporting Pending Stats to DM_STAT_TABLE for (",dedp_inowner,".",
  dedp_intable,") using STATID (",
  dedp_in_to,")")
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 CALL export_pending_stats(dedp_inowner,concat('"',dedp_intable,'"'),"DM_STAT_TABLE",dedp_in_to,
  "V500")
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dedp_step_end_dttm = cnvtdatetime(curdate,curtime3)
 CALL dm2_process_log_add_detail_number("EXPORT_PENDING_STATS:SECS",datetimediff(dedp_step_end_dttm,
   dedp_step_beg_dttm,5))
 IF (dm2_process_log_dtl_row(dedp_log_id,0)=0)
  GO TO exit_script
 ENDIF
#exit_script
 ...
 SET dedp_err_ind = dm_err->err_ind
 SET dedp_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_execution,dedp_log_id,1)
 SET dm_err->emsg = dedp_emsg
 SET dm_err->err_ind = dedp_err_ind
 SET dm_err->eproc = "Ending dm2_export_dbstats_pending"
 CALL final_disp_msg("dm2_edbs_pend")
END GO
