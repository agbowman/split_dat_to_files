CREATE PROGRAM dm2_rdds_post_env_copy_maint:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_rdds_pt_env_cp",".log","dm2_rdds_post_env_copy_maint LOG...")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_rdds_post_env_copy_maint"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (checkprg("DM2_RDDS_POST_ENV_COPY_WRP") != 0)
  EXECUTE dm2_rdds_post_env_copy_wrp
  IF ((dm_err->err_ind != 0))
   GO TO exit_program
  ENDIF
 ELSE
  IF ((((dir_env_maint_rs->src_env_id=0.0)) OR ((dir_env_maint_rs->tgt_env_id=0.0))) )
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Source environment ID or target environment ID is not populated"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  EXECUTE dm2_rdds_metadata_reset dir_env_maint_rs->tgt_env_id
  IF ((dm_err->err_ind != 0))
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Executing dm_rdds_drop_xlat_function"
  CALL disp_msg(" ",dm_err->logfile,0)
  EXECUTE dm_rdds_drop_xlat_function
  IF ((dm_err->err_ind != 0))
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Executing dm_rdds_drop_reg_triggers"
  CALL disp_msg(" ",dm_err->logfile,0)
  EXECUTE dm_rdds_drop_reg_triggers
  IF ((dm_err->err_ind != 0))
   GO TO exit_program
  ENDIF
  EXECUTE dm2_rdds_copy_req_install_env dir_env_maint_rs->src_env_id, dir_env_maint_rs->tgt_env_id
  IF ((dm_err->err_ind != 0))
   GO TO exit_program
  ENDIF
  EXECUTE dm2_rdds_event_copy dir_env_maint_rs->src_env_id, dir_env_maint_rs->tgt_env_id
  IF ((dm_err->err_ind != 0))
   GO TO exit_program
  ENDIF
  UPDATE  FROM dm_info d
   SET d.info_number = dir_env_maint_rs->src_env_id, d.updt_dt_tm = cnvtdatetime(curdate,curtime3)
  ...
  IF (curqual=0)
   INSERT  FROM dm_info d
    SET d.info_domain = "RDDS REPLICATE INFO", d.info_name = "DOMAIN REPLICATE SOURCE", d.info_number
      = dir_env_maint_rs->src_env_id,
...
#exit_program
 IF ((dm_err->err_ind != 0))
  ROLLBACK
 ENDIF
 SET dm_err->eproc = "...Ending dm2_rdds_post_env_copy_maint"
 CALL final_disp_msg("dm2_rdds_post_env_copy_maint")
END GO
