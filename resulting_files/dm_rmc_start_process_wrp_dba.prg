CREATE PROGRAM dm_rmc_start_process_wrp:dba
 ...
 IF (check_logfile("dm_rmc_start",".log","DM_RMC_START_PROCESS_WRP LOGFILE")=0)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning DM_RMC_START_PROCESS_WRP"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (reflect(parameter(1,0)) != "C*")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Expected syntax:  dm_rmc_start_process_wrp <PROCESS_NAME>"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drsp_data->process_name =  $1
 SET dm_err->eproc = "Gathering number of processes"
 SET drsp_data->num_procs = gather_num_procs("Starting background processes")
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((drsp_data->num_procs > 0))
  CALL dbase_connect(drsp_data)
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (daf_is_not_blank(drsp_data->db_sid)
   AND daf_is_not_blank(drsp_data->db_password))
   EXECUTE dm_rmc_start_task_process  WITH replace("REQUEST","DRSP_DATA")
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   ...
   SET width = 132
   CALL box(1,1,5,132)
   CALL text(3,47,"***  Starting background processes ***")
   CALL text(7,3,concat("The ",trim(cnvtstring(drsp_data->num_procs)),
     " processes were successfully started."))
   CALL text(8,3,"To track the progress, run the command below.")
   CALL text(9,6,concat("DM_RMC_TASK_MON '",drsp_data->process_name,"' go"))
   CALL text(20,3,"Press ENTER to continue")
   CALL accept(20,28,"P;E"," ")
   RETURN(null)
  ENDIF
 ENDIF
#exit_program
