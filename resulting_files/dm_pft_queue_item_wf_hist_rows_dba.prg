CREATE PROGRAM dm_pft_queue_item_wf_hist_rows:dba
 ...
 END ;Subroutine
 DECLARE table_name = vc WITH protect, constant("PFT_QUEUE_ITEM_WF_HIST")
 DECLARE token_days_to_keep = vc WITH protect, constant("DAYSTOKEEP")
 DECLARE min_age_in_days = i4 WITH protect, constant(180)
 DECLARE batch_size = f8 WITH protect, constant(50000.0)
 ...
 DECLARE purgedatetime = dq8 WITH protect, noconstant(0.0)
 DECLARE i18nhandle = i4 WITH private, noconstant(0)
 DECLARE errormessage = vc WITH protect, noconstant("")
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 SET reply->err_code = - (1)
 SET reply->status_data.status = "F"
 SET reply->table_name = table_name
 SET reply->rows_between_commit = minval(100,v_rows_left)
 FOR (v_token_idx = 1 TO size(request->tokens,5))
   IF ((request->tokens[v_token_idx].token_str=token_days_to_keep))
    SET v_days_to_keep = ceil(cnvtreal(request->tokens[v_token_idx].value))
    GO TO next_step
   ENDIF
 ENDFOR
#next_step
 IF (v_days_to_keep < min_age_in_days)
  SET errormessage = concat("You must keep at least ",cnvtstring(min_age_in_days,3),
   " days' worth of data.  You entered %1 days or did not enter any value.")
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",errormessage,"i",v_days_to_keep)
  GO TO exit_script
 ENDIF
 SET purgedatetime = cnvtagedatetime(0,0,0,v_days_to_keep)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(pqiwh.pft_queue_item_wf_hist_id)
   FROM pft_queue_item_wf_hist pqiwh
   WHERE pqiwh.pft_queue_item_wf_hist_id > 0.0
   DETAIL
    v_cur_min_id = maxval(cnvtreal(seqval),1.0)
   WITH nocounter
  ;end select
 ELSE
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(pqiwh.pft_queue_item_wf_hist_id)
  FROM pft_queue_item_wf_hist pqiwh
  WHERE pqiwh.pft_queue_item_wf_hist_id > 0.0
  DETAIL
   v_max_id = cnvtreal(seqval)
  WITH nocounter
 ;end select
 SET v_cur_max_id = (v_cur_min_id+ (batch_size - 1))
  ...
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    ...
    PLAN (pqiwh
     WHERE  NOT ( EXISTS (
     (SELECT
      pqi.pft_queue_item_id
      FROM pft_queue_item pqi
      WHERE pqiwh.pft_queue_item_id=pqi.pft_queue_item_id)))
      AND pqiwh.pft_queue_item_wf_hist_id BETWEEN v_cur_min_id AND v_cur_max_id
      AND parser(sbr_getrowidnotexists("pqiwh.updt_dt_tm <= cnvtdatetime(purgeDateTime)","pqiwh"))
      AND pqiwh.pft_queue_item_wf_hist_id != 0)
    DETAIL
     v_row_cnt += 1
     IF (mod(v_row_cnt,50)=1)
      stat = alterlist(reply->rows,(v_row_cnt+ 49))
     ENDIF
     reply->rows[v_row_cnt].row_id = pqiwh.rowid
    WITH nocounter, maxqual(pqiwh,value(v_rows_left))
   ;end select
   ...
   SET v_cur_max_id = (v_cur_min_id+ (batch_size - 1))
   SET v_rows_left = (request->max_rows - v_row_cnt)
 ENDWHILE
