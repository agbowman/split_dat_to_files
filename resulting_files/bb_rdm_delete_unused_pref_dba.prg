CREATE PROGRAM bb_rdm_delete_unused_pref:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bb_rdm_delete_unused_pref"
 ...
 DECLARE nobsoletedcnt = i2
 DECLARE smsg = vc
 DECLARE found = i2
 SET nobsoletedcnt = size(requestin->list_0,5)
 IF (nobsoletedcnt=0)
  SET smsg = "No records read from input file.  Nothing to do."
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  GO TO script_failed
 ENDIF
 SET stat = alterlist(recobsolete->questions,nobsoletedcnt)
 FOR (i = 1 TO nobsoletedcnt)
   SET recobsolete->questions[i].scdf_meaning = requestin->list_0[i].scdf_meaning
 ENDFOR
 SET smsg = build("Getting ready to delete ",cnvtstring(nobsoletedcnt)," records read from file.")
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 SET smsg = "Gathering code_value information..."
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 SELECT INTO "nl:"
  c.code_value
  FROM code_value c,
   (dummyt d  WITH seq = value(nobsoletedcnt))
  PLAN (d)
   JOIN (c
   WHERE c.code_set=1661
    AND (c.cdf_meaning=recobsolete->questions[d.seq].scdf_meaning))
  DETAIL
   recobsolete->questions[d.seq].dcode_value = c.code_value
  WITH nocounter
 ;end select
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  CALL logmsg("Error retrieving code_value information for cdf_meanings.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,
    "Execution failed - unable to retrieve code_value information for cdf_meanings."),"F")
  COMMIT
  GO TO script_failed
 ENDIF
 SET smsg = build("Preparing to delete rows from dependency table...")
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 DELETE  FROM dependency dp,
   (dummyt d  WITH seq = value(nobsoletedcnt))
  SET dp.seq = 1
  PLAN (d)
   JOIN (dp
   WHERE (recobsolete->questions[d.seq].dcode_value=dp.question_cd))
  WITH nocounter
 ;end delete
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  CALL logmsg("Error deleting rows from dependency table.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,"Execution failed - unable to delete rows from dependency table."
    ),"F")
  COMMIT
  GO TO script_failed
 ENDIF
 SET smsg = build("Records deleted.")
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 SET smsg = build("Checking dependency table data...")
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 SET found = 0
 SELECT INTO "nl:"
  FROM dependency dp,
 ...
 IF (nerrorstatus=0)
  IF (found != 0)
   CALL logmsg("Row(s) marked for deletion still exists on dependency table.",(e_dispmsg+ e_logtofile
    ))
   ROLLBACK
   CALL logstatus(build(sreadme_name,
     "Execution failed - Row(s) marked for deletion still exists on dependency table.."),"F")
   COMMIT
   GO TO script_failed
  ELSE
   SET smsg = build("Row(s) not found, deletion successful.")
   CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  ENDIF
 ELSE
  CALL logmsg("Error checking dependency table.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,"Execution failed - unable to check rows on dependency table."),
   "F")
  COMMIT
  GO TO script_failed
 ENDIF
 SET smsg = build("Preparing to delete rows from valid response table...")
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 DELETE  FROM valid_response v,
   (dummyt d  WITH seq = value(nobsoletedcnt))
  SET v.seq = 1
  PLAN (d)
   JOIN (v
   WHERE (recobsolete->questions[d.seq].dcode_value=v.question_cd))
  WITH nocounter
 ;end delete
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  CALL logmsg("Error deleting rows from valid_response table.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,
    "Execution failed - unable to delete rows from valid_response table."),"F")
  COMMIT
  GO TO script_failed
 ENDIF
 SET smsg = build("Checking valid_response table data...")
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 SET found = 0
 SELECT INTO "nl:"
  FROM valid_response v,
   (dummyt d  WITH seq = value(nobsoletedcnt))
  PLAN (d)
   JOIN (v
   WHERE (recobsolete->questions[d.seq].dcode_value=v.question_cd))
 ...
 IF (nerrorstatus=0)
  IF (found != 0)
   CALL logmsg("Row(s) marked for deletion still exists on valid_response table.",(e_dispmsg+
    e_logtofile))
   ROLLBACK
   CALL logstatus(build(sreadme_name,
     "Execution failed - Row(s) marked for deletion still exists on valid_response table.."),"F")
   COMMIT
   GO TO script_failed
  ELSE
   SET smsg = build("Row(s) not found, deletion successful.")
  ...
  ENDIF
 ELSE
  CALL logmsg("Error checking valid_response table.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,
    "Execution failed - unable to check rows on valid_response table."),"F")
  COMMIT
  GO TO script_failed
 ENDIF
 SET smsg = build("Records deleted.")
 ...
 DELETE  FROM answer a,
  ...
  PLAN (d)
   JOIN (a
   WHERE (recobsolete->questions[d.seq].dcode_value=a.question_cd))
  WITH nocounter
 ;end delete
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  CALL logmsg("Error deleting rows from answer table.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,"Execution failed - unable to delete rows from answer table."),
   "F")
  COMMIT
  GO TO script_failed
 ENDIF
 SET smsg = build("Checking answer table data...")
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 SET found = 0
 SELECT INTO "nl:"
  FROM answer a,
 ...
 IF (nerrorstatus=0)
  IF (found != 0)
   CALL logmsg("Row(s) marked for deletion still exists on answer table.",(e_dispmsg+ e_logtofile))
   ROLLBACK
   CALL logstatus(build(sreadme_name,
     "Execution failed - Row(s) marked for deletion still exists on answer table.."),"F")
   COMMIT
   GO TO script_failed
  ELSE
   SET smsg = build("Row(s) not found, deletion successful.")
 ...
 ELSE
  CALL logmsg("Error checking dependency table.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,"Execution failed - unable to check rows on answer table."),"F")
  COMMIT
  GO TO script_failed
 ENDIF
 DELETE  FROM question q,
  ...
  PLAN (d)
   JOIN (q
   WHERE (recobsolete->questions[d.seq].dcode_value=q.question_cd))
  WITH nocounter
 ;end delete
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  CALL logmsg("Error deleting rows from question table.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,"Execution failed - unable to delete rows from question table."),
   "F")
  COMMIT
  GO TO script_failed
 ENDIF
 SET smsg = build("Checking question table data...")
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 SET found = 0
 SELECT INTO "nl:"
  FROM question q,
 ...
 IF (nerrorstatus=0)
  IF (found != 0)
   CALL logmsg("Row(s) marked for deletion still exists on question table.",(e_dispmsg+ e_logtofile))
   ROLLBACK
   CALL logstatus(build(sreadme_name,
     "Execution failed - Row(s) marked for deletion still exists on question table.."),"F")
   COMMIT
   GO TO script_failed
  ELSE
   SET smsg = build("Row(s) not found, deletion successful.")
 ...
 ELSE
  CALL logmsg("Error checking dependency table.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,"Execution failed - unable to check rows on question table."),"F"
   )
  COMMIT
  GO TO script_failed
 ENDIF
 UPDATE  FROM code_value c,
   (dummyt d  WITH seq = value(nobsoletedcnt))
  SET c.active_ind = 0, c.active_type_cd = 0
  PLAN (d)
   JOIN (c
   WHERE (recobsolete->questions[d.seq].dcode_value=c.code_value))
  WITH nocounter
 ;end update
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  CALL logmsg("Error inactivating rows from the code_value table.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,
    "Execution failed - unable to inactivate rows from the code_value table."),"F")
  COMMIT
  GO TO script_failed
 ENDIF
 SET smsg = build("Checking code_value table data...")
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 SET found = 0
 SELECT INTO "nl:"
  FROM code_value c,
 ...
 IF (nerrorstatus=0)
  IF (found != 0)
   CALL logmsg("Row(s) marked for inactivation still active on code_value table.",(e_dispmsg+
    e_logtofile))
   ROLLBACK
   CALL logstatus(build(sreadme_name,
     "Execution failed - Row(s) marked for inactivation still active on code_value table."),"F")
   COMMIT
   GO TO script_failed
  ELSE
   SET smsg = build("Active row(s) not found, inactivation successful.")
   CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  ENDIF
 ELSE
  CALL logmsg("Error checking dependency table.",(e_dispmsg+ e_logtofile))
  ROLLBACK
  CALL logstatus(build(sreadme_name,
    "Execution failed - unable to inactivate rows on code_value table."),"F")
  COMMIT
  GO TO script_failed
 ENDIF
 COMMIT
