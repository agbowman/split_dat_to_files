CREATE PROGRAM bb_rdm_add_component_zero_row:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bb_rdm_add_component_zero_row"
 CALL logscriptstart(sreadme_name)
 COMMIT
 SET serrormsg = fillstring(255," ")
 SET nerrorstatus = error(serrormsg,1)
 CALL logmsg("Checking for existance of zero row on the component table...",(e_dispmsg+ e_logtofile))
 SET found = 0
 SELECT INTO "nl:"
  *
  FROM component
 ...
 IF (nerrorstatus=0)
  IF (found=0)
   CALL logmsg("No zero row found.",(e_dispmsg+ e_logtofile))
   CALL logmsg("Adding zero row using program 'dm_add_zero_rows'.",(e_dispmsg+ e_logtofile))
   EXECUTE dm_add_zero_rows "COMPONENT"
  ELSE
   CALL logmsg("Zero row found.  Nothing to do.",(e_dispmsg+ e_logtofile))
  ENDIF
 ELSE
  CALL logmsg("Error selecting from component table.",(e_dispmsg+ e_logtofile))
  CALL logstatus(build(sreadme_name,"Execution failed - unable to select from component table."),"F")
  COMMIT
  GO TO script_failed
 ENDIF
 SET found = 0
 ...
 IF (nerrorstatus=0)
  IF (found=0)
   CALL logmsg("No zero row found.",(e_dispmsg+ e_logtofile))
   CALL logstatus(build(sreadme_name,"Execution failed - zero still does not exist."),"F")
   COMMIT
   GO TO script_failed
  ELSE
   CALL logmsg("Zero row found.  Row check passed.",(e_dispmsg+ e_logtofile))
  ENDIF
 ELSE
  CALL logmsg("Error selecting from component table.",(e_dispmsg+ e_logtofile))
  CALL logstatus(build(sreadme_name,"Execution failed - unable to select from component table."),"F")
  ...
  GO TO script_failed
 ENDIF
 CALL logstatus(build(sreadme_name,"Execution completed successfully."),"S")
 COMMIT
#script_failed
