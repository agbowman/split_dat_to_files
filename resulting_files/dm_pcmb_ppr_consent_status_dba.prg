CREATE PROGRAM dm_pcmb_ppr_consent_status:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "dm_pcmb_ppr_consent_status"
 ...
 SELECT
  ...
  DETAIL
   v_cust_count1 += 1
   IF (mod(v_cust_count1,100)=1)
    stat = alterlist(rreclist->from_rec,(v_cust_count1+ 99))
   ENDIF
   ...
   organization_id = frm.organization_id, rreclist->from_rec[v_cust_count1].consent_status_id = frm
   .consent_status_id,
   rreclist->from_rec[v_cust_count1].consent_policy_id = frm.consent_policy_id, rreclist->from_rec[
   v_cust_count1].consent_type_cd = frm.consent_type_cd
  FOOT REPORT
 ...
 IF (v_cust_count1 > 0)
  SELECT
   ...
   ELSE
    WHERE (tu.person_id=request->xxx_combine[icombine].to_xxx_id)
     AND (tu.encntr_id=request->xxx_combine[icombine].encntr_id)
   ENDIF
   INTO "nl:"
   tu.*
   ...
   DETAIL
    v_cust_count2 += 1
    IF (mod(v_cust_count2,100)=1)
     stat = alterlist(rreclist->to_rec,(v_cust_count2+ 99))
    ENDIF
    rreclist->to_rec[v_cust_count2].to_id = tu.person_id, rreclist->to_rec[v_cust_count2].active_ind
    ...
    organization_id = tu.organization_id, rreclist->to_rec[v_cust_count2].consent_status_id = tu
    .consent_status_id,
    rreclist->to_rec[v_cust_count2].consent_policy_id = tu.consent_policy_id, rreclist->to_rec[
    v_cust_count2].consent_type_cd = tu.consent_type_cd
   FOOT REPORT
  ...
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
    ...
    FOR (v_cust_loopcount2 = 1 TO v_cust_count2)
      IF ((rreclist->from_rec[v_cust_loopcount].active_ind=1)
       ...
       AND (rreclist->from_rec[v_cust_loopcount].organization_id=0.0)
       AND (rreclist->to_rec[v_cust_loopcount2].organization_id=0.0)
       AND (rreclist->from_rec[v_cust_loopcount].consent_policy_id=rreclist->to_rec[v_cust_loopcount2
      ].consent_policy_id)
       AND (rreclist->from_rec[v_cust_loopcount].consent_type_cd=rreclist->to_rec[v_cust_loopcount2].
      consent_type_cd))
       SET active_consents_present = 1
       SET found_index = v_cust_loopcount2
      ELSEIF ((rreclist->from_rec[v_cust_loopcount].active_ind=1)
       AND (rreclist->to_rec[v_cust_loopcount2].active_ind=1)
       AND (rreclist->from_rec[v_cust_loopcount].organization_id > 0)
       AND (rreclist->from_rec[v_cust_loopcount].organization_id=rreclist->to_rec[v_cust_loopcount2].
      organization_id)
       AND (rreclist->from_rec[v_cust_loopcount].consent_policy_id=rreclist->to_rec[v_cust_loopcount2
      ].consent_policy_id)
       AND (rreclist->from_rec[v_cust_loopcount].consent_type_cd=rreclist->to_rec[v_cust_loopcount2].
    ...
    IF (active_consents_present=1)
     ...
     upd_dt_time))
      IF (del_to(rreclist->to_rec[found_index].consent_status_id,rreclist->to_rec[found_index].
       active_ind,rreclist->to_rec[found_index].active_status_cd)=0)
       GO TO exit_sub
      ENDIF
