CREATE PROGRAM dm_eucb_encntr_plan_cob:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_EUCB_ENCNTR_PLAN_COB"
 ...
 SUBROUTINE cust_ucb_del(dummy)
   UPDATE  FROM encntr_plan_cob
    ...
    WHERE (encntr_plan_cob_id=rchildren->qual1[det_cnt].entity_id)
    WITH nocounter
   ;end update
   IF (curqual=0)
   ...
   CALL add_hist(rchildren->qual1[det_cnt].entity_id)
   RETURN(1)
 END ;Subroutine
 ...
 SUBROUTINE cust_ucb_upt(dummy)
   UPDATE  FROM encntr_plan_cob
    SET updt_cnt = (updt_cnt+ 1), updt_id = reqinfo->updt_id, updt_applctx = reqinfo->updt_applctx,
     updt_task = reqinfo->updt_task, updt_dt_tm = cnvtdatetime(sysdate), encntr_id = request->
 ...
 SUBROUTINE initialize_hist_id(null)
  IF (v_hist_init_ind=0)
   ...
   SET dcipht_request->transaction_reason_txt = "ENCNTR_UCB_ENCNTR_PLAN_COB"
