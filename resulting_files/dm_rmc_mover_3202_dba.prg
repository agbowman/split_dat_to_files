CREATE PROGRAM dm_rmc_mover_3202:dba
 ...
 DECLARE r_tab_name_dfr = vc
 DECLARE r_tab_name_dfd = vc
 DECLARE from_desc = vc
 DECLARE drm3202_context_name = vc
 DECLARE drs_sc_parent_ind = i2
 DECLARE mxlat_loop = i4
 DECLARE drm_context = vc
 DECLARE drm_inst_id = f8
 DECLARE drm_forms_oct = i4
 DECLARE drm_parent_drdm_row = i2
 DECLARE cust_nomv_chk = vc WITH protect, noconstant("")
 DECLARE drm_ptam_result = f8 WITH protect, noconstant(0.0)
 DECLARE del_$r_sub(sbr_tab_name_dfd,sbr_tab_name_dfr) = null
 ...
 IF ((global_mover_rec->ptam_ind=1))
  FREE RECORD drm_drpbr_request
  RECORD drm_drpbr_request(
    1 table_name = vc
    1 table_suffix = vc
    1 match_str_tspace_cnt = i4
    1 src_match_result = f8
  ...
  FREE RECORD drm_drpbr_reply
  RECORD drm_drpbr_reply(
    1 nomv_type = vc
    1 blocking_log_id = f8
    1 error_ind = i2
    1 reason_text = vc
  )
  SET drm_drpbr_request->table_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
  SET drm_drpbr_request->table_suffix = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix
  SET drm_drpbr_request->src_env_id = dm2_ref_data_doc->env_source_id
  SET drm_drpbr_request->trg_env_id = dm2_ref_data_doc->env_target_id
  SET drm_drpbr_request->src_match_result = drdm_chg->log[drdm_log_loop].ptam_match_result
 ENDIF
 SET r_tab_name_dfr = cutover_tab_name(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,"")
 SET r_tab_name_dfd = cutover_tab_name("DCP_FORMS_DEF","")
 ...
 IF (active_ind=0)
  SELECT INTO "NL:"
   ...
   WHERE d.dcp_forms_ref_id=cnvtreal(ref_vc)
    AND d.active_ind=1
   DETAIL
    inactivate_ind = 0, drm_inst_id = d.dcp_form_instance_id
   WITH nocounter
  ;end select
 ...
 CALL echo("")
 CALL echo("")
 SET dm_err->eproc = "Checking if dcp_forms_ref is a single commit parent"
 SET drs_sc_parent_ind = 0
 SELECT INTO "NL:"
  FROM dm_refchg_tab_r r
  WHERE r.parent_table="DCP_FORMS_REF"
  DETAIL
   drs_sc_parent_ind = 1
  WITH nocounter
 ;end select
 ...
 ELSEIF (inactivate_ind=0
  ...
  IF (drs_sc_parent_ind=1)
   CALL trigger_proc_call(cust_tab_name,"",drdm_chg->log[drdm_log_loop].src_cntxt_name,
    "DCP_FORM_INSTANCE_ID",drm_inst_id)
  ENDIF
  FOR (mxlat_loop = 1 TO size(missing_xlats->qual,5))
    IF ((missing_xlats->qual[mxlat_loop].table_name="DCP_FORMS_REF")
     AND (missing_xlats->qual[mxlat_loop].missing_value=drm_inst_id))
     SET missing_xlats->qual[mxlat_loop].processed_ind = 1
    ENDIF
  ENDFOR
  SET drm_forms_oct = orphan_child_tab(cust_tab_name,"OLDVER","",cnvtreal(rs_3202->from_values.
 ...
 IF (((active_ind=1) OR (inactivate_ind=1)) )
  ...
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[ref_num].translated=0))
   ...
   IF ((ref_val=- (3)))
    IF (inactivate_ind=1)
     CALL echo("dm_rmc_mover_3202:The inactive form doesn't exist in target")
     ...
     FOR (mxlat_loop = 1 TO size(missing_xlats->qual,5))
       IF ((missing_xlats->qual[mxlat_loop].table_name="DCP_FORMS_REF")
        AND (missing_xlats->qual[mxlat_loop].missing_value IN (rs_3202->from_values.dcp_forms_ref_id,
       rs_3202->from_values.dcp_form_instance_id)))
        SET missing_xlats->qual[mxlat_loop].processed_ind = 1
       ENDIF
     ENDFOR
     IF (drdm_error_out_ind=1)
  ...
  SET drm_ptam_result = drmm_get_ptam_match_result(0.0,dm2_ref_data_doc->env_source_id,
   ...
   "FROM")
  IF ((global_mover_rec->ptam_ind=1))
   SET drm_drpbr_request->trg_match_result = drm_ptam_result
   EXECUTE dm_rmc_ptam_block  WITH replace("REQUEST","DRM_DRPBR_REQUEST"), replace("REPLY",
    "DRM_DRPBR_REPLY")
   IF (check_error(dm_err->eproc)=1)
    SET dm2_ref_data_reply->error_msg = dm_err->emsg
    SET dm2_ref_data_reply->error_ind = 1
    GO TO exit_3202
   ENDIF
   IF ((drm_drpbr_reply->blocking_log_id > 0))
    SET drdm_chg->log[drdm_log_loop].blocking_log_id = drm_drpbr_reply->blocking_log_id
    SET drm_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
    SET drdm_chg->log[drm_parent_drdm_row].chg_log_reason_txt = drm_drpbr_reply->reason_text
    SET dm2_ref_data_reply->error_ind = 1
    SET drdm_mini_loop_status = drm_drpbr_reply->nomv_type
    SET drdm_no_trans_ind = 1
    SET dm2_ref_data_reply->error_msg = drm_drpbr_reply->nomv_type
    CALL orphan_child_tab(cust_tab_name,drm_drpbr_reply->nomv_type,"DCP_FORM_INSTANCE_ID",cnvtreal(
      rs_3202->from_values.dcp_form_instance_id))
    GO TO exit_3202
   ENDIF
  ENDIF
  IF (cust_beg_eff_ind=1)
   ...
   IF (curqual=0)
    SELECT INTO "NL:"
     FROM dcp_forms_ref3202$r dfr
     WHERE dfr.dcp_form_instance_id=inst_val
      AND dfr.active_ind=1
      AND dfr.rdds_status_flag < 9000
     WITH nocounter
    ;end select
    IF (curqual=0)
     SELECT INTO "NL:"
      ...
      FROM dcp_forms_ref dfr
      WHERE ((dfr.dcp_form_instance_id=inst_val) UNION (
      (SELECT
       dcp_form_instance_id
       FROM dcp_forms_ref3202$r
       WHERE dcp_form_instance_id=inst_val
        AND rdds_status_flag < 9000)))
      WITH nocounter
     ;end select
     IF (curqual != 0)
      SET dm2_ref_data_reply->error_ind = 1
      SET dm2_ref_data_reply->error_msg = "NOMV30"
      SET stat = alterlist(dm2_ref_data_reply->qual,0)
      CALL merge_audit("FAILREASON",
  ...
  IF (idcd_check=1)
   IF ((dm2_ref_data_reply->error_ind=1))
    ...
    GO TO exit_3202
   ENDIF
   SET from_id = cnvtreal(get_value(cust_tab_name,"DCP_FORMS_REF_ID","TO"))
   SET from_desc = get_value(cust_tab_name,"DESCRIPTION","FROM")
   IF (cust_beg_eff_ind != 1)
    IF ((global_mover_rec->context_ind=0))
     SET drm3202_context_name = drdm_chg->log[drdm_log_loop].context_name
    ELSE
     SET drm3202_context_name = drdm_oracle_context
    ENDIF
    CALL del_$r_sub(r_tab_name_dfd,r_tab_name_dfr)
    SELECT INTO "NL:"
     FROM (parser(r_tab_name_dfr) dfr)
     WHERE (dfr.dcp_forms_ref_id=rs_3202->to_values.dcp_forms_ref_id)
      AND dfr.active_ind=0
      AND (dfr.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
      AND dfr.rdds_status_flag < 9000
     DETAIL
      drm_context = dfr.rdds_context_name
     WITH nocounter
    ;end select
    IF (curqual > 0)
     SET drm3202_context_name = evaluate_context_name(drm_context,drm3202_context_name)
     UPDATE  FROM (parser(r_tab_name_dfr) dfr)
      SET dfr.rdds_context_name = drm3202_context_name, dfr.rdds_dt_tm = cnvtdatetime(curdate,
        curtime3), dfr.rdds_log_id = drdm_chg->log[drdm_log_loop].log_id,
       dfr.rdds_ptam_match_result = drm_ptam_result
      WHERE (dfr.dcp_forms_ref_id=rs_3202->to_values.dcp_forms_ref_id)
       AND dfr.active_ind=0
       AND (dfr.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
       AND dfr.rdds_status_flag < 9000
     ...
     ENDIF
    ELSE
     INSERT  FROM (parser(r_tab_name_dfr) d)
      SET d.description = from_desc, d.dcp_form_instance_id = seq(carenet_seq,nextval), d
       .dcp_forms_ref_id = from_id,
       d.active_ind = 0, d.end_effective_dt_tm = cnvtdatetime(curdate,curtime3), d.rdds_status_flag
        = 0,
       d.rdds_dt_tm = cnvtdatetime(curdate,curtime3), d.rdds_log_id = drdm_chg->log[drdm_log_loop].
       log_id, d.rdds_context_name = drm3202_context_name,
       d.rdds_source_env_id = dm2_ref_data_doc->env_source_id, d.rdds_delete_ind = 0, d
       .rdds_ptam_match_result = drm_ptam_result
      WITH nocounter
     ;end insert
     IF (((findstring("ORA-00001:",dm_err->emsg) > 0) OR (((findstring("ORA-02049:",dm_err->emsg) > 0
     ) OR (findstring("ORA-4020:",dm_err->emsg) > 0)) )) )
  ...
  ENDIF
 ENDIF
 SUBROUTINE del_$r_sub(sbr_tab_name_dfd,sbr_tab_name_dfr)
   DECLARE v_context = vc WITH protect
   SELECT INTO "NL:"
    FROM (parser(sbr_tab_name_dfr) dfr)
    WHERE (dfr.dcp_forms_ref_id=rs_3202->to_values.dcp_forms_ref_id)
     AND dfr.active_ind=1
     AND (dfr.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
     AND dfr.rdds_status_flag < 9000
    DETAIL
     v_context = dfr.rdds_context_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (drs_sc_parent_ind=1)
    ...
    DELETE  FROM (parser(sbr_tab_name_dfd) dfd)
     WHERE dfd.dcp_form_instance_id IN (
     (SELECT
      dcp_form_instance_id
      FROM (parser(sbr_tab_name_dfr) dfr)
      WHERE (dfr.dcp_forms_ref_id=rs_3202->to_values.dcp_forms_ref_id)
       ...
       AND (dfr.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
       AND dfr.rdds_status_flag < 9000))
      AND (dfd.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
      AND dfd.rdds_status_flag < 9000
     WITH nocounter
    ;end delete
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ENDIF
   DELETE  FROM (parser(sbr_tab_name_dfr) dfr)
    WHERE (dfr.dcp_forms_ref_id=rs_3202->to_values.dcp_forms_ref_id)
     AND dfr.active_ind=1
     AND (dfr.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
     AND dfr.rdds_status_flag < 9000
