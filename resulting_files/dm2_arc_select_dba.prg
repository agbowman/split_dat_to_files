CREATE PROGRAM dm2_arc_select:dba
...
#arc_person_exit
 ...
 ENDIF
 FOR (ps_ndx = 1 TO size(pers_arc->statements[s_ndx].archive_insert,5))
  IF ((pers_arc->statements[s_ndx].archive_insert[ps_ndx].from_ind > 0)
   AND first_from_ind=0)
   SET first_from_ind = 1
  ENDIF
  IF (first_from_ind > 0)
   SET from_loc = findstring("from",pers_arc->statements[s_ndx].archive_insert[ps_ndx].stmt)
   IF (found_ind=0)
    SET found_ind = 1
    SET from_str = substring(from_loc,((size(pers_arc->statements[s_ndx].archive_insert[ps_ndx].stmt,
      1) - from_loc)+ 1),pers_arc->statements[s_ndx].archive_insert[ps_ndx].stmt)
   ELSE
    SET from_str = pers_arc->statements[s_ndx].archive_insert[ps_ndx].stmt
   ENDIF
   SET from_str = arc_replace(from_str,pers_arc->statements[s_ndx].archive_insert[ps_ndx].link_ind,0,
    pers_arc->statements[s_ndx].archive_insert[ps_ndx].arc_entity_ind,pers_arc->arc_db[v_link_db].
    pre_link_name,
    pers_arc->arc_db[v_link_db].post_link_name,v_archive_entity_id)
   IF (ps_ndx=size(pers_arc->statements[s_ndx].archive_insert,5))
   ...
   ELSEIF ((ps_ndx=(size(pers_arc->statements[s_ndx].archive_insert,5) - 1))
    AND findstring(") go",pers_arc->statements[s_ndx].archive_insert[(ps_ndx+ 1)].stmt)=0)
