CREATE PROGRAM afc_add_tier_group:dba
 DECLARE versionnbr = vc
 SET versionnbr = "001"
 CALL echo(build("AFC_ADD_TIER_GROUP Version: ",versionnbr))
 RECORD reply(
   1 qual[1]
     2 code_value = f8
     2 display_key = c40
 ...
 IF (logicaldomainsinuse)
  ...
  SELECT INTO "nl:"
   ...
   WITH nocounter
  ;end select
  FOR (count1 = 1 TO size(request->qual,5))
    SET request->qual[count1].extension_cnt = 2
    SET stat = alterlist(request->qual[count1].extension_data,2)
    SET request->qual[count1].extension_data[1].field_name = "LockUser"
    SET request->qual[count1].extension_data[1].field_type = 2
    SET request->qual[count1].extension_data[1].field_value = username
    SET request->qual[count1].extension_data[2].field_name = "Logical Domain"
    SET request->qual[count1].extension_data[2].field_type = 1
    SET request->qual[count1].extension_data[2].field_value = cnvtstring(curlogicaldomainid,17,2)
  ENDFOR
 ENDIF
 EXECUTE cs_add_code  WITH replace("REQUEST",request), replace("REPLY",reply)
#end_program
