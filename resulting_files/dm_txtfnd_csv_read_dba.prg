CREATE PROGRAM dm_txtfnd_csv_read:dba
 ...
 SUBROUTINE dm2_get_program_stack(null)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm_txtfnd_csv",".log","DM_TXTFND_CSV_READ LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_read
 ENDIF
 DECLARE dtcr_loop = i4 WITH protect, noconstant(0)
 DECLARE dtcr_size = i4 WITH protect, noconstant(0)
 SET dtcr_size = size(requestin->list_0,5)
 SET script_rec->script_cnt = dtcr_size
 SET stat = alterlist(script_rec->script_qual,dtcr_size)
 FOR (dtcr_loop = 1 TO dtcr_size)
  SET script_rec->script_qual[dtcr_loop].script_name = requestin->list_0[dtcr_loop].script_name
  SET script_rec->script_qual[dtcr_loop].data_source = requestin->list_0[dtcr_loop].script_name
 ENDFOR
 IF (check_error("Reading in csv data") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Transferring CSV data:",dm_err
    ->emsg),0.0,0.0)
  GO TO exit_read
 ENDIF
