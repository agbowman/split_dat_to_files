CREATE PROGRAM acm_rdm_legacy_comm_pref_p:dba
 ...
 ENDIF
 SELECT INTO "nl:"
  max_val = max(p.person_id), min_val = min(p.person_id)
  FROM person_patient p
  WHERE p.person_id > 0
   AND p.active_ind=1
   AND p.active_status_cd=dactivecd
  DETAIL
   mf_max_range_id = cnvtreal(max_val), mf_min_range_id = cnvtreal(min_val)
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_RDM_MIGRATE_PPC",concat(
   "create global temporary table TEMP_RDM_MIGRATE_PPC(person_id number,",
   "contact_method_cd number, phone_contact_type_cd number, pm_hist_tracking_id number)",
   " on commit preserve rows"),"Creating temporary table")
 CALL sbr_add_sql_stmt("INDEX","TEMPPPCINDEX",
  "create unique index TEMPPPCINDEX ON TEMP_RDM_MIGRATE_PPC(person_id)","Creating index")
