CREATE PROGRAM dm2_aload_adm_purge_token
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_tkn",".log","dm2_aload_adm_purge_token")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_adm_purge_token"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daaptkn_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daaptkn_file = vc WITH protect, noconstant( $2)
 DECLARE daaptkn_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daaptkn_batch = i4 WITH protect, noconstant(0)
 DECLARE daaptkn_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daaptkn_pkg,daaptkn_file_loc_ret,daaptkn_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daaptkn_batch=0)
  SET daaptkn_batch = 5000
 ENDIF
 IF (daaptkn_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daaptkn_file_loc_ret,daaptkn_file), "dm2_aload_prgtpl_tkn",
  daaptkn_batch
  CALL echo(concat(daaptkn_file_loc_ret,daaptkn_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_adm_purge_token completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_adm_purge_token")
END GO
