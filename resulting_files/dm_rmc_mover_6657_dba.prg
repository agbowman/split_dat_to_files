CREATE PROGRAM dm_rmc_mover_6657:dba
 ...
 SET cust_pe_id = get_value(cust_tab_name,"PARENT_ENTITY_ID","FROM")
 SET cust_pft_id = get_value(cust_tab_name,"PFT_PROVIDER_NBR_RELTN_ID","FROM")
 IF (cust_pe_name="*_CD")
  SET cust_process_tab = "CODE_VALUE"
 ELSE
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV23"
  CALL disp_msg(concat("ERROR: The parent_entity_name of ",cust_pe_name,
    " is not supported at this time"),dm_err->logfile,0)
  GO TO exit_6657
 ENDIF
 IF (cust_process_tab > " ")
  IF (cnvtreal(cust_pe_id) > 0)
   SET cust_new_value = select_merge_translate(cust_pe_id,cust_process_tab)
   IF (isnumeric(cust_new_value))
    CALL put_value(cust_tab_name,"PARENT_ENTITY_ID",cust_new_value)
   ELSE
    SET cust_rpt_missing = report_missing(cust_process_tab,"CODE_VALUE",cnvtreal(cust_pe_id))
    CALL evaluate_rpt_missing(cust_rpt_missing)
   ENDIF
  ELSE
   CALL put_value(cust_tab_name,"PARENT_ENTITY_ID",cust_pe_id)
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"PFT_PROVIDER_NBR_RELTN_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"PFT_PROVIDER_NBR_RELTN_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"PFT_PROVIDER_NBR_RELTN_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,
       "  PFT_PROVIDER_NBR_RELTN_ID")
     ELSE
      GO TO exit_6657
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"PFT_PROVIDER_NBR_RELTN_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6657
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"PFT_PROVIDER_NBR_RELTN_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6657
   ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"PARENT_ENTITY_ID")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg =
    "This row wasn't translated because of the PARENT_ENTITY_ID column"
   ENDIF
