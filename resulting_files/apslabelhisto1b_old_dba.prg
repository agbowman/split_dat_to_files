CREATE PROGRAM apslabelhisto1b_old:dba
 RECORD label(
   1 qual[*]
     2 field[*]
       3 data = c15
...
#script
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   lcnt = 0
  DETAIL
   lcnt = (lcnt+ 1), stat = alterlist(label->qual,lcnt), stat = alterlist(label->qual[lcnt].field,7),
   ...
   label->qual[lcnt].field[4].data = substring(1,15,data->resrc[d1.seq].label[d2.seq].
    ...
    current_dt_tm_string), label->qual[lcnt].field[6].data = substring(4,15,data->resrc[d1.seq].
    label[d2.seq].accession_nbr),
   label->qual[lcnt].field[7].data = substring(1,15,data->resrc[d1.seq].label[d2.seq].
    supplemental_tag)
  WITH nocounter
 ;end select
 SET col->count = 4
 ...
 SET col->qual[2].xpos = 68
 SET col->qual[2].ypos = 0
 SET col->qual[3].xpos = 136
 SET col->qual[3].ypos = 0
 SET col->qual[4].xpos = 204
   ...
   FOR (labs = 1 TO ((size(label->qual,5)/ col->count)+ 1))
    FOR (ncol = 1 TO col->count)
     ...
     IF (nlab <= size(label->qual,5))
      ...
      CALL print(calcpos(((home_x_pos+ col->qual[ncol].xpos)+ 0),((home_y_pos+ col->qual[ncol].ypos)
       + 21))),
      "{LPI/8}{CPI/18}{FONT/0}", label->qual[nlab].field[4].data,
      CALL print(calcpos(((home_x_pos+ col->qual[ncol].xpos)+ 37),((home_y_pos+ col->qual[ncol].ypos)
       + 21))),
      "{LPI/8}{CPI/18}{FONT/0}", label->qual[nlab].field[7].data, row + 1,
      CALL print(calcpos(((home_x_pos+ col->qual[ncol].xpos)+ 0),((home_y_pos+ col->qual[ncol].ypos)
      ...
      CALL print(calcpos(((home_x_pos+ col->qual[ncol].xpos)+ 2),((home_y_pos+ col->qual[ncol].ypos)
       + 34))), "{LPI/8}{CPI/18}{FONT/0}",
      label->qual[nlab].field[5].data, row + 1,
      CALL print(calcpos(((home_x_pos+ col->qual[ncol].xpos)+ 10),((home_y_pos+ col->qual[ncol].ypos)
       + 50))),
      "{FONT/0/1}{LPI/12}{CPI/34}", "California Hospital",
      CALL print(calcpos(((home_x_pos+ col->qual[ncol].xpos)+ 14),((home_y_pos+ col->qual[ncol].ypos)
       + 54))),
      "{FONT/0/1}{LPI/12}{CPI/34}", "Medical Center", row + 1
     ENDIF
    ENDFOR
    ,
    IF (nlab <= size(label->qual,5))
