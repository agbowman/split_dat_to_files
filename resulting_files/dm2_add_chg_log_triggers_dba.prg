CREATE PROGRAM dm2_add_chg_log_triggers:dba
 ...
 DECLARE daclt_module_name = vc WITH protect, constant("RDDS_PROCESS")
 DECLARE daclt_action_name = vc WITH protect, constant("RDDS_DDL")
 DECLARE daclt_original_module = vc WITH protect, noconstant(" ")
 DECLARE daclt_original_action = vc WITH protect, noconstant(" ")
 DECLARE daclt_module_set_ind = i2 WITH protect, noconstant(0)
 DECLARE daclt_version_nbr = i4 WITH protect, constant(2)
 DECLARE daclt_version_str = vc WITH protect, constant("Trigger Version")
 IF (validate(rdds_skip_parse_check,- (1)) > 0)
  SET rdds_skip_parse_check = 1
 ELSE
  SET rdds_skip_parse_check = 0
 ENDIF
 IF (validate(rdds_force_trig_regen,- (1)) > 0)
  SET rdds_force_trig_regen = 1
 ELSE
  SET rdds_force_trig_regen = 0
 ENDIF
 ...
 RECORD daclt_trg_data(
   ...
   1 qual[*]
     ...
     2 mergeable_ind = i2
     2 reference_ind = i2
     2 version_match_ind = i2
     2 filter_function = vc
     2 filter_skip_recreate_ind = i2
     2 filter_parm[*]
       3 column_name = vc
       3 parm_name = vc
       3 data_type = vc
     ...
     2 del_pkw_hash = f8
     2 del_pkw_skip_recreate_ind = i2
     2 del_pkw_parm[*]
       3 column_name = vc
       3 data_type = vc
       3 nullable = c1
     ...
     2 colstring_skip_recreate_ind = i2
     2 colstring_parm[*]
       3 column_name = vc
       3 data_type = vc
     2 ptam_match_function = vc
     2 ptam_hash = f8
     2 ptam_skip_recreate_ind = i2
     2 ptam_parm[*]
       3 column_name = vc
       3 data_type = vc
       3 exist_ind = i2
 ...
 ENDIF
 IF ((dm2_rdbms_version->level1 >= 11))
  IF (dmai_get_cur_mod_act(daclt_original_module,daclt_original_action)=0)
   GO TO exit_program
  ELSE
   SET daclt_module_set_ind = 1
  ENDIF
  CALL dmai_set_mod_act(daclt_module_name,daclt_action_name)
 ENDIF
 ...
 IF (check_open_event(daclt_src_id,0.0)=0)
  IF (trg_regen_reason != "Automatic Regeneration via open event"
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   IF (daf_is_not_blank(daclt_pair_name))
 ...
 IF ((daclt_trg_data->table_cnt > 0))
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    IF ((drgtd_reply->qual[d.seq].metadata_change_ind=0))
     daclt_trg_data->qual[d.seq].pkw_skip_recreate_ind = 1, daclt_trg_data->qual[d.seq].
     del_pkw_skip_recreate_ind = 1, daclt_trg_data->qual[d.seq].ptam_skip_recreate_ind = 1
    ENDIF
   WITH nocounter
  ;end select
 ENDIF
 ...
 FOR (daclt_num = 1 TO daclt_trg_data->table_cnt)
   IF ((daclt_trg_data->qual[daclt_num].table_name="PRSNL"))
    ...
    IF (daclt_p_ndx > daclt_num)
     ...
     IF ((daclt_ptam_env_list->ptam_on_ind=1)
      ...
      ENDIF
     ENDIF
     IF ( NOT (drcdv_check_version(daclt_version_str,concat("REFCHG",daclt_trg_data->qual[daclt_p_ndx
       ].table_suffix,"ADD"),daclt_version_nbr)))
      SET daclt_trg_data->qual[daclt_p_ndx].pkw_skip_recreate_ind = 0
      SET daclt_trg_data->qual[daclt_p_ndx].del_pkw_skip_recreate_ind = 0
      SET daclt_trg_data->qual[daclt_p_ndx].ptam_skip_recreate_ind = 0
     ELSE
      SET daclt_trg_data->qual[daclt_p_ndx].version_match_ind = 1
     ENDIF
     ...
     CALL create_refchg_triggers_ora(daclt_p_ndx,v_stmt_cnt,v_drop_cnt)
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
     IF ((dm_err->debug_flag > 1))
      CALL echorecord(daclt_trg_data)
     ENDIF
    ENDIF
   ENDIF
   IF ((((daclt_trg_data->qual[daclt_num].table_name != "PERSON")) OR (locateval(daclt_ndx,1,
    ...
    IF ((daclt_ptam_env_list->ptam_on_ind=1)
     ...
     ENDIF
    ENDIF
    IF ((( NOT (drcdv_check_version(daclt_version_str,concat("REFCHG",daclt_trg_data->qual[daclt_num]
      .table_suffix,"ADD"),daclt_version_nbr))) OR (rdds_force_trig_regen=1)) )
     SET daclt_trg_data->qual[daclt_num].pkw_skip_recreate_ind = 0
     SET daclt_trg_data->qual[daclt_num].del_pkw_skip_recreate_ind = 0
     SET daclt_trg_data->qual[daclt_num].ptam_skip_recreate_ind = 0
    ELSE
     SET daclt_trg_data->qual[daclt_num].version_match_ind = 1
    ENDIF
    SET dm_err->eproc = "Calling create_refchg_filter_function_ora()."
    ...
    ENDIF
    SET dm_err->eproc = "Calling create_refchg_trggers_ora()."
    CALL create_refchg_triggers_ora(daclt_num,v_stmt_cnt,v_drop_cnt)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
 ...
 SUBROUTINE create_refchg_filter_function_ora(i_trg_ndx,io_drop_cnt,io_stmt_cnt)
   ...
   DECLARE s_filter_version = i4 WITH protect, noconstant(0)
   DECLARE s_filter_str = vc WITH protect, constant("Filter Version")
   ...
   ENDIF
   SELECT INTO "NL:"
    drf.filter_type, drf.statement_ind, drf.filter_version_nbr
    ...
    DETAIL
     ...
     s_filter_version = drf.filter_version_nbr
    WITH nocounter
   ;end select
   ...
   IF (s_omf_prsnl_group_flg=1)
    ...
    ;end select
    IF ((daclt_trg_data->qual[i_trg_ndx].version_match_ind=1)
     AND drcdv_check_version(s_filter_str,s_drop_func_name,s_filter_version)
     AND s_filter_ind=1)
     SET daclt_trg_data->qual[i_trg_ndx].filter_function = s_drop_func_name
     SET daclt_trg_data->qual[i_trg_ndx].filter_skip_recreate_ind = 1
    ELSE
    ...
    ELSE
     ...
     SELECT INTO "nl:"
      ...
      WITH nocounter
     ;end select
     IF ((daclt_trg_data->qual[i_trg_ndx].filter_skip_recreate_ind=1))
      RETURN
     ENDIF
     ...
     CALL stmt_push(io_stmt_cnt,concat('ASIS("',char(10),"--",s_filter_str,": ",
       cnvtstring(s_filter_version),char(10),'")'))
 ...
 SUBROUTINE create_refchg_triggers_ora(i_trg_ndx,io_stmt_cnt,io_drop_cnt)
   ...
   DECLARE cto_disable_str = vc WITH protect, noconstant(" ")
   DECLARE cto_ret_str = vc WITH protect, noconstant(" ")
   DECLARE cto_in_parse_ind = i2 WITH protect, noconstant(0)
   DECLARE cto_retry_cnt = i4 WITH protect, noconstant(5)
   DECLARE cto_idx = i4 WITH protect, noconstant(0)
   DECLARE cto_person_chg_ind = i2 WITH protect, noconstant(0)
   SET csa_tbl_cnt = 0
   SET csa_col_cnt = 0
   ...
   ENDIF
   IF (rdds_skip_parse_check=0
    AND (dm2_rdbms_version->level1 >= 11))
    FOR (cto_idx = 1 TO cto_retry_cnt)
     IF (ddf_check_in_parse("V500",daclt_trg_data->qual[i_trg_ndx].table_name,cto_in_parse_ind,
      cto_ret_str)=0)
      GO TO exit_program
     ENDIF
     IF (cto_in_parse_ind=1
      AND cto_idx < cto_retry_cnt)
      CALL pause(3)
     ELSEIF (cto_in_parse_ind=1)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = cto_ret_str
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ELSE
      SET cto_idx = (cto_retry_cnt+ 1)
     ENDIF
    ENDFOR
   ENDIF
   IF ((daclt_trg_data->qual[i_trg_ndx].table_name="PRSNL"))
    SET i_person_ndx = locateval(ic_idx,1,daclt_trg_data->table_cnt,"PERSON",daclt_trg_data->qual[
     ic_idx].table_name)
    IF ((((daclt_trg_data->qual[i_person_ndx].ptam_skip_recreate_ind=0)) OR ((daclt_trg_data->qual[
    i_person_ndx].colstring_skip_recreate_ind=0))) )
     SET cto_person_chg_ind = 1
    ENDIF
   ENDIF
   IF ((((daclt_trg_data->qual[i_trg_ndx].version_match_ind=0)) OR ((((daclt_trg_data->qual[i_trg_ndx
   ].filter_skip_recreate_ind=0)) OR ((((daclt_trg_data->qual[i_trg_ndx].pkw_skip_recreate_ind=0))
    OR ((((daclt_trg_data->qual[i_trg_ndx].del_pkw_skip_recreate_ind=0)) OR ((((daclt_trg_data->qual[
   i_trg_ndx].ptam_skip_recreate_ind=0)) OR ((((daclt_trg_data->qual[i_trg_ndx].
   colstring_skip_recreate_ind=0)) OR (((rdds_force_trig_regen=1) OR (cto_person_chg_ind=1)) )) ))
   )) )) )) )) )
    FOR (s_temp_loop = 1 TO 2)
     IF (s_temp_loop=1)
      SET s_trig_suffix = "_TEMP"
      IF ((dm2_rdbms_version->level1 >= 12))
       SET cto_disable_str = " DISABLE "
      ENDIF
     ELSE
      SET s_trig_suffix = " "
      SET cto_disable_str = " "
     ...
     FOR (type_ndx = 1 TO 3)
       ...
       IF (s_event="UPDATE"
        ...
        SELECT INTO "NL:"
         ...
         DETAIL
          ic_idx = 0, ic_idx = locateval(ic_idx2,1,s_col_cnt,utc.column_name,s_updt_cols->qual[
           ic_idx2].column_name)
          IF (ic_idx=0)
           s_col_cnt = (s_col_cnt+ 1)
           IF (mod(s_col_cnt,10)=1)
       ...
       CALL stmt_push(io_stmt_cnt,'ASIS(" begin ")')
       CALL stmt_push(io_stmt_cnt,concat('ASIS("',char(10),"--",daclt_version_str,": ",
         cnvtstring(daclt_version_nbr),char(10),'")'))
       IF (s_event="UPDATE"
        AND cto_lob_ind=0)
        CALL stmt_push(io_stmt_cnt,concat('ASIS(" ',ic_string,' ")'))
       ENDIF
       CALL stmt_push(io_stmt_cnt,^ASIS(" dm2_context_control('REFCHG TRIGGER','Y');")^)
       IF (daf_is_not_blank(daclt_trg_data->qual[i_trg_ndx].filter_function))
        ...
        FOR (fp_ndx = 1 TO size(daclt_trg_data->qual[i_trg_ndx].filter_parm,5))
         ...
         ENDIF
         CALL stmt_push(io_stmt_cnt,concat('ASIS(",:new.',daclt_trg_data->qual[i_trg_ndx].
           filter_parm[fp_ndx].column_name,",:old.",daclt_trg_data->qual[i_trg_ndx].filter_parm[
           fp_ndx].column_name,'")'))
        ENDFOR
       ...
       IF (s_type="UPD")
        ...
        CALL stmt_push(io_stmt_cnt,concat(^ASIS(" proc_refchg_ins_dcl('^,daclt_trg_data->qual[
          ...
          s_del_ind_str,",",evaluate(dm2_cl_trg_updt_cols->qual[updt_ndx].updt_id_exist_ind,1,
           ":new.updt_id",0,"0"),",",evaluate(dm2_cl_trg_updt_cols->qual[updt_ndx].
           updt_task_exist_ind,1,":new.updt_task",0,"0"),
       ...
       IF ((daclt_trg_data->qual[i_trg_ndx].table_name="PRSNL")
        ...
        SET i_person_ndx = locateval(ic_idx,1,daclt_trg_data->table_cnt,"PERSON",daclt_trg_data->
         qual[ic_idx].table_name)
       ...
       IF ((daclt_ptam_env_list->ptam_on_ind=1))
        ...
        SET cto_circ_pos = locateval(cto_circ_pos,1,dcltr_circ->tbl_cnt,daclt_trg_data->qual[
         i_trg_ndx].table_name,dcltr_circ->tbl_qual[cto_circ_pos].table_name)
        IF (cto_circ_pos > 0)
         IF ((dm_err->debug_flag > 1))
         ...
         FOR (cto_circ_idx = 1 TO dcltr_circ->tbl_qual[cto_circ_pos].circ_cnt)
           IF ((dcltr_circ->tbl_qual[cto_circ_pos].circ_qual[cto_circ_idx].circ_exist_ind=1)
            ...
            IF (daf_is_not_blank(dcltr_circ->tbl_qual[cto_circ_pos].circ_qual[cto_circ_idx].
             fk_name_col))
        ...
        ELSEIF ((daclt_trg_data->qual[i_trg_ndx].table_name="MED_INGRED_SET"))
         ...
         CALL stmt_push(io_stmt_cnt,concat(
           ...
           " nvl(:v2,''null_vaLue_CHeck'') and mi.rdds_source_env_id in(",daclt_ptam_env_list->
           env_ids,
           " )) and m.rdds_delete_ind = 0 and m.rdds_status_flag = 0 and m.rdds_source_env_id in(",
           daclt_ptam_env_list->env_ids,^ )'; ")^))
         CALL stmt_push(io_stmt_cnt,concat(
 ...
 SUBROUTINE create_sc_proc_ora(i_dtd_ndx,i_found_ndx,io_stmt_cnt)
   ...
   IF (s_found2_ndx > 0
    ...
    CALL stmt_push(io_stmt_cnt,concat('asis("select ',refchg_tab_r->child[i_found_ndx].parent[1].
      parent_id_col,'")'))
    IF (daf_is_not_blank(refchg_tab_r->child[i_found_ndx].parent[1].parent_tab_col))
   ...
   ELSE
    ...
    FOR (loop_ndx = 1 TO size(refchg_tab_r->child[i_found_ndx].parent,5))
      IF (daf_is_not_blank(refchg_tab_r->child[i_found_ndx].parent[loop_ndx].md_value_str)
       AND daf_is_not_blank(refchg_tab_r->child[i_found_ndx].parent[loop_ndx].md_var_str))
       SET daclt_md_flag = 1
      ENDIF
    ENDFOR
    IF (daclt_md_flag=1)
    ...
    ENDIF
    CALL stmt_push(io_stmt_cnt,concat('asis("begin")'))
    IF (((daf_is_blank(refchg_tab_r->child[i_found_ndx].parent[1].md_var_str)
     AND daf_is_blank(refchg_tab_r->child[i_found_ndx].parent[1].md_value_str)) OR ((refchg_tab_r->
    ...
    IF (size(refchg_tab_r->child[i_found_ndx].parent,5)=1)
     IF (daf_is_not_blank(refchg_tab_r->child[i_found_ndx].parent[1].md_var_str)
      AND daf_is_not_blank(refchg_tab_r->child[i_found_ndx].parent[1].md_value_str))
    ...
    ELSE
     FOR (loop_ndx = 1 TO size(refchg_tab_r->child[i_found_ndx].parent,5))
      ...
      ELSE
       IF (daf_is_not_blank(refchg_tab_r->child[i_found_ndx].parent[loop_ndx].md_var_str)
        AND daf_is_not_blank(refchg_tab_r->child[i_found_ndx].parent[loop_ndx].md_value_str))
        CALL stmt_push(io_stmt_cnt,concat(^asis("if i_select_str > ' ' then")^))
        CALL stmt_push(io_stmt_cnt,concat('asis("execute immediate i_select_str into ',refchg_tab_r->
    ...
    IF ((refchg_tab_r->child[i_found_ndx].child_table="NAME_VALUE_PREFS"))
     ...
     CALL stmt_push(io_stmt_cnt,concat('asis(", 0.0);")'))
     CALL stmt_push(io_stmt_cnt,concat('asis("end if;")'))
    ELSEIF (daf_is_blank(refchg_tab_r->child[i_found_ndx].parent[1].md_var_str)
     AND daf_is_blank(refchg_tab_r->child[i_found_ndx].parent[1].md_value_str))
     CALL stmt_push(io_stmt_cnt,concat('asis("proc_refchg_ins_log(v_parent_table, v_pk_where, ")'))
     CALL stmt_push(io_stmt_cnt,concat(
       'asis("dbms_utility.get_hash_value(v_pk_where, 0, 1073741824),")'))
     CALL stmt_push(io_stmt_cnt,concat(
   ...
   FOR (s_dtd_ndx = i_dtd_find TO i_upto_value)
     IF (((i_single_tab_ind=0) OR ((rs_dtd_reply->dtd_hold[s_dtd_ndx].process_ind=1))) )
      ...
      IF (s_found_ndx > 0)
       ...
       ELSEIF (daf_is_not_blank(refchg_tab_r->child[s_found_ndx].parent[1].md_value_str)
        AND daf_is_not_blank(refchg_tab_r->child[s_found_ndx].parent[1].md_var_str))
       ...
       CALL stmt_push(io_stmt_cnt,^ASIS("   v_trig_type := 'INS/UPD'; ")^)
       CALL stmt_push(io_stmt_cnt,'ASIS(" end if; ")')
       IF (daf_is_not_blank(rs_dtd_reply->dtd_hold[s_dtd_ndx].filter_function))
       ...
       IF ((refchg_tab_r->child[s_found_ndx].child_table IN ("NAME_VALUE_PREFS")))
        ...
        FOR (loop_ndx = 1 TO size(refchg_tab_r->child[s_found_ndx].parent,5))
          ...
          "DCP_INPUT_REF", "CONFIG_PREFS", "DCP_FORMS_REF", "PREDEFINED_PREFS"))))
           ...
           IF (daf_is_not_blank(refchg_tab_r->child[s_found_ndx].parent[loop_ndx].md_var_str)
            AND daf_is_not_blank(refchg_tab_r->child[s_found_ndx].parent[loop_ndx].md_value_str))
           ...
           ENDIF
          ELSE
           IF (daf_is_not_blank(s_else_list))
       ...
       ELSE
        CALL stmt_push(io_stmt_cnt,concat('asis("else")'))
       ENDIF
       IF (daf_is_not_blank(refchg_tab_r->child[s_found_ndx].parent[1].parent_tab_col))
 ...
 SUBROUTINE create_xlat_triggers(i_dtd_ndx,io_stmt_cnt)
   ...
   IF (cxl_find_tbl_ndx > 0)
    ...
    IF (daf_is_blank(cxl_trig_ins))
     SET cxl_trig_ins = concat("REFCHGXLAT",daclt_trg_data->qual[i_dtd_ndx].table_suffix,"INS")
    ENDIF
    IF (daf_is_blank(cxl_trig_del))
     SET cxl_trig_del = concat("REFCHGXLAT",daclt_trg_data->qual[i_dtd_ndx].table_suffix,"DEL")
    ENDIF
    IF (daf_is_blank(cxl_trig_upd))
     SET cxl_trig_upd = concat("REFCHGXLAT",daclt_trg_data->qual[i_dtd_ndx].table_suffix,"UPD")
    ENDIF
 ...
 SUBROUTINE xlat_trigger_check(i_trigger_name,i_trigger_name_c,i_cxl_drop)
   IF (daf_is_not_blank(i_trigger_name)
    AND daf_is_not_blank(i_trigger_name_c))
   ...
   ELSEIF (daf_is_not_blank(i_trigger_name))
    RETURN(i_trigger_name)
   ELSEIF (daf_is_not_blank(i_trigger_name_c))
 ...
 END ;Subroutine
#exit_program
 IF (daclt_module_set_ind=1)
  CALL dmai_set_mod_act(daclt_original_module,daclt_original_action)
 ENDIF
 IF ((dm_err->err_ind=0)
 ...
 ELSEIF ((dm_err->err_ind=1)
  AND (((dguc_reply->dguc_err_ind=0)) OR (daf_is_blank(dguc_reply->dguc_err_msg))) )
  SET dguc_reply->dguc_err_ind = 1
