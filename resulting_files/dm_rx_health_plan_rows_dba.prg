CREATE PROGRAM dm_rx_health_plan_rows:dba
 ...
 DECLARE service_type_cd = f8 WITH constant(uar_get_code_by("MEANING",27137,"PRESCRIPTION"))
 ...
 SET reply->table_name = "HEALTH_PLAN"
 ...
 ELSE
  SELECT INTO "nl:"
   hp.rowid
   FROM health_plan hp
   WHERE parser(sbr_getrowidnotexists("hp.data_status_cd = data_status_cd","hp"))
    AND hp.service_type_cd=service_type_cd
    AND hp.health_plan_id > 0
    AND  NOT (hp.health_plan_id IN (
   (SELECT
    prpr.health_plan_id
    FROM person_rx_plan_reltn prpr
    WHERE prpr.end_effective_dt_tm > cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND prpr.person_rx_plan_reltn_id > 0)))
   DETAIL
    row_cnt = (row_cnt+ 1)
    IF (mod(row_cnt,50)=1)
     stat = alterlist(reply->rows,(row_cnt+ 49))
    ENDIF
    reply->rows[row_cnt].row_id = hp.rowid
   FOOT REPORT
    stat = alterlist(reply->rows,row_cnt)
   WITH nocounter, maxqual(hp,value(request->max_rows))
  ;end select
