CREATE PROGRAM bbt_tag_component:dba
 RECORD antibody(
   1 antibodylist[10]
     2 antibody_cd = f8
     2 antibody_disp = c15
 ...
 SET captions->received_date = uar_i18ngetmessage(i18nhandle,"received_date",
  "RECEIVED           DATE")
 SET captions->by_time = uar_i18ngetmessage(i18nhandle,"by_time","BY:___________     &TIME________")
 ...
 SET rpt_date = 0
 EXECUTE cpm_create_file_name_logical "bbt_tag_comp", "txt", "x"
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  DETAIL
   ...
   dispense_prvdr_name = trim(tag_request->taglist[d.seq].dispense_prvdr_name), rpt_row = 3, rpt_row
    += 2,
   row rpt_row, col 53, captions->component_transfusion,
   rpt_row += 1, row rpt_row, col 006,
   captions->tech, col 020, tech_name,
   col 092, captions->date, col 105,
   pe_event_dt_tm"@SHORTDATE", rpt_row += 1, row rpt_row,
   col 006, captions->patient, col 020,
   name_full_formatted, col 092, captions->unit_num,
   col 101, product_nbr_full
   IF (serial_nbr != null)
   ...
   col 079, sex_disp, product_disp_30 = substring(1,30,product_disp),
   ...
   col 055, captions->pooled_unit_count, col 074,
   cmpnt_cnt"###", col 092, qty_vol_disp,
   ...
   rpt_row += 1, row rpt_row, col 048,
   captions->transfusionist, col 073, captions->time_started,
   row rpt_row, col 096, captions->received_date,
   ...
   col 006, captions->leukofiltered, col 033,
   captions->irradiated, col 060, captions->cmv_neg,
   col 090, captions->vol_reduced, rpt_row += 1,
   row rpt_row, col 006, captions->autologous,
   col 033, captions->directed, col 060,
   captions->hla_typed, col 090, captions->hgb_s_neg,
   rpt_row += 1, row rpt_row, col 006,
   captions->iga_deficient, col 033, captions->other,
   col 060, captions->absc, rpt_row += 2,
   row rpt_row, col 006, captions->return_to_bb,
   col 054, captions->in_30_min, rpt_row += 1,
   row rpt_row, col 006, captions->refrigerator,
   rpt_row = 52, row rpt_row, col 094,
   captions->hospital_name, rpt_row += 1, row rpt_row,
   col 094, captions->hospital_address, rpt_row += 1,
   row rpt_row, col 094, captions->hospital_address,
   rpt_row += 1, row rpt_row, col 094,
   captions->clia_num, rpt_row += 1, row rpt_row,
   col 058, captions->original_to_chart, rpt_row += 1,
   row rpt_row, col 061, captions->copy_to_lab
  FOOT  d.seq
   IF (d.seq > 0
