CREATE PROGRAM dm_rcm_upd_third_party_acct:dba
 ...
 SET readme_data->message = "Readme failed: starting script dm_rcm_upd_third_party_acct..."
 ...
 IF (error(errmsg,0) > 0)
  ...
  SET readme_data->message = concat("Failed to retrieve CURASPAN code value: ",errmsg)
  GO TO exit_script
 ENDIF
 IF (third_party_type_cd < 1)
  SET readme_data->status = "S"
  SET readme_data->message = "Failed to retrieve CURASPAN code value"
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM code_value cv
 ...
 IF (attribute_type_cd < 1)
  SET readme_data->status = "S"
  SET readme_data->message = "Failed to retrieve AUTHORITY code value"
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM code_value cv
 ...
 IF (active_status_cd < 1)
  SET readme_data->status = "S"
  SET readme_data->message = "Failed to retrieve ACTIVE code value"
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
