CREATE PROGRAM bed_rec_lablay_nonlab_srvarea:dba
         ...
         IF ((hold->ilist[i].dlist[d].slist[s].sslist[ss].multiplexor_ind=1))
          ...
          SELECT INTO "nl:"
           ...
           PLAN (lrc
            ...
            JOIN (l
            WHERE l.location_cd=lrc.location_cd
             AND l.active_ind=1)
           DETAIL
            IF (l.discipline_type_cd != lab_ct_cd)
             found_bad_calendar = 1
            ENDIF
           WITH nocounter
          ;end select
          IF (found_bad_calendar=1)
           SET nonlab_srvarea_ind = 1
          ENDIF
         ENDIF
         IF (nonlab_srvarea_ind=0)
