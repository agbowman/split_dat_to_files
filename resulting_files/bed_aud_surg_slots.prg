CREATE PROGRAM bed_aud_surg_slots
 SET last_mod = "120022"
 CALL echo("**** BED_AUD_THR_SURG_SLOTS.PRG LAST MOD: 120022 ****")
...
#bed_error_subroutines_exit
 ...
 SELECT INTO "nl:"
  ...
  PLAN (schr
   ...
   JOIN (sdr
   WHERE sdr.resource_cd=schr.resource_cd
    AND sdr.beg_effective_dt_tm <= cnvtdatetime(currentdatetime)
    AND sdr.end_effective_dt_tm >= cnvtdatetime(currentdatetime)
    AND sdr.active_ind=1)
   JOIN (sds
   WHERE sds.def_sched_id=sdr.def_sched_id
    AND sds.beg_effective_dt_tm <= cnvtdatetime(currentdatetime)
    AND sds.end_effective_dt_tm >= cnvtdatetime(currentdatetime)
    AND sds.active_ind=1)
   JOIN (sdl
   WHERE sds.def_sched_id=sdl.def_sched_id
    AND sdl.beg_effective_dt_tm <= cnvtdatetime(currentdatetime)
    AND sdl.end_effective_dt_tm >= cnvtdatetime(currentdatetime)
    AND sdl.active_ind=1)
   JOIN (sst
   WHERE sst.slot_type_id=outerjoin(sdl.slot_type_id)
    AND sst.active_ind=outerjoin(1))
   JOIN (sfs
   WHERE sfs.sch_flex_id=outerjoin(sst.sch_flex_id))
   JOIN (cv_schr
   WHERE cv_schr.code_value=outerjoin(schr.service_resource_cd)
    AND cv_schr.active_ind=outerjoin(1))
  ORDER BY sds.mnemonic, sdl.def_sched_id, sdl.vis_beg_units DESC
  HEAD REPORT
   rowindex = 0
  DETAIL
   tcnt = (tcnt+ 1), stat = alterlist(temp->tqual,tcnt), temp->tqual[tcnt].template_name = sds
   .mnemonic,
   temp->tqual[tcnt].template_desc = sds.description, temp->tqual[tcnt].operating_room = cv_schr
   .display, temp->tqual[tcnt].slot_name = sdl.description,
   temp->tqual[tcnt].flex_rule = sfs.mnemonic
   IF ((sdl.vis_beg_units=- (1)))
    temp->tqual[tcnt].release_time = " "
   ELSE
    temp->tqual[tcnt].release_time = cnvtstring(sdl.vis_beg_units)
   ENDIF
   temp->tqual[tcnt].release_units = sdl.vis_beg_units_meaning, temp->tqual[tcnt].person_id = schr
   .person_id, temp->tqual[tcnt].item_id = schr.item_id,
   temp->tqual[tcnt].service_resource_cd = schr.service_resource_cd
  WITH nocounter
 ;end select
 FOR (t = 1 TO tcnt)
 ...
 SET row_nbr = 0
 FOR (x = 1 TO tcnt)
   IF ((temp->tqual[x].surgical_resource_ind=1))
    SET row_nbr = (row_nbr+ 1)
    SET stat = alterlist(reply->rowlist,row_nbr)
    SET stat = alterlist(reply->rowlist[row_nbr].celllist,7)
    SET reply->rowlist[row_nbr].celllist[1].string_value = temp->tqual[x].template_name
    SET reply->rowlist[row_nbr].celllist[2].string_value = temp->tqual[x].template_desc
    SET reply->rowlist[row_nbr].celllist[3].string_value = temp->tqual[x].slot_name
    SET reply->rowlist[row_nbr].celllist[4].string_value = temp->tqual[x].operating_room
    SET reply->rowlist[row_nbr].celllist[5].string_value = temp->tqual[x].flex_rule
    SET reply->rowlist[row_nbr].celllist[6].string_value = temp->tqual[x].release_time
    SET reply->rowlist[row_nbr].celllist[7].string_value = temp->tqual[x].release_units
   ENDIF
 ENDFOR
END GO
