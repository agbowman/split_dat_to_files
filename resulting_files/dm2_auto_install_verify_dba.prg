CREATE PROGRAM dm2_auto_install_verify:dba
 ...
 DECLARE daiv_load_reply(dlr_process=vc,dlr_msg=vc,dlr_status=i2) = i2
 DECLARE daiv_clinical_credentials(null) = i2
 DECLARE daiv_admin_credentials(null) = i2
 DECLARE daiv_clinical_app_os(null) = i2
 DECLARE daiv_clinical_db_os(null) = i2
 DECLARE daiv_admin_link(dal_admin_link_name=vc(ref)) = i2
 DECLARE daiv_clinicaldb_storage(null) = i2
 DECLARE daiv_admindb_storage(das_link=vc) = i2
 DECLARE daiv_clinicaldb_ver(null) = i2
 DECLARE daiv_admindb_ver(dav_link=vc) = i2
 DECLARE daiv_unattended_setup(null) = i2
 DECLARE daiv_scheduler_setup(null) = null
 DECLARE daiv_notification_setup(null) = i2
 DECLARE daiv_package_history(null) = i2
 DECLARE daiv_clinical_diskgroup_setup(null) = i2
 DECLARE daiv_auto_install_status(null) = i2
 DECLARE daiv_db_conn_check(ddcc_user=vc,ddcc_pword=vc,ddcc_cnnct_str=vc,ddcc_admin_ind=i2) = i2
 DECLARE daiv_chk_ccl_tabledef_sync(null) = i2
 DECLARE daiv_adb_ind = i2 WITH protect, noconstant(0)
 DECLARE daiv_idx = i4 WITH protect, noconstant(0)
 DECLARE daiv_admin_link_name = vc WITH protect, noconstant(" ")
 DECLARE daiv_clin_user_name = vc WITH protect, noconstant(" ")
 DECLARE daiv_admin_user_name = vc WITH protect, noconstant(" ")
 SET reply->status_data.status = "F"
 SET reply->auto_install_ind = 0
 IF (check_logfile("dm2_install_verify",".log","dm2_auto_install_verify")=0)
  SET reply->status_data.subeventstatus[1].operationname = "LOG FILE CREATION"
  SET reply->status_data.subeventstatus[1].operationstatus = "F"
  SET reply->status_data.subeventstatus[1].targetobjectname = "check_logfile"
  SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_auto_install_verify"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Validating currdbuser is v500 or not"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (currdbuser="V500")
  IF (daiv_load_reply("V500 user",concat(trim(currdbuser)," is a valid user"),1)=0)
   GO TO exit_program
  ENDIF
 ELSE
  IF (daiv_load_reply("V500 user",concat(trim(currdbuser)," is an invalid user"),0)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 ...
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 IF (dm2_adb_check("",daiv_adb_ind)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_clinical_credentials(null)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_admin_credentials(null)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_clinical_app_os(null)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_clinical_db_os(null)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_admin_link(daiv_admin_link_name)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_clinicaldb_storage(null)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_admindb_storage(daiv_admin_link_name)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_clinicaldb_ver(null)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_admindb_ver(daiv_admin_link_name)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_db_conn_check(daiv_clin_user_name,dm2_install_schema->v500_p_word,dm2_install_schema->
  v500_connect_str,0)=0)
  GO TO exit_program
 ENDIF
 IF (daiv_db_conn_check(daiv_admin_user_name,dm2_install_schema->cdba_p_word,dm2_install_schema->
  cdba_connect_str,1)=0)
  GO TO exit_program
 ENDIF
 IF (locateval(daiv_idx,1,reply->check_list_cnt,0,reply->check_list[daiv_idx].check_passed_ind)=0)
  IF (daiv_unattended_setup(null)=0)
   GO TO exit_program
  ENDIF
  IF (daiv_scheduler_setup(null)=0)
   GO TO exit_program
  ENDIF
  IF (daiv_notification_setup(null)=0)
   GO TO exit_program
  ENDIF
  IF (daiv_package_history(null)=0)
   GO TO exit_program
  ENDIF
  IF (daiv_clinical_diskgroup_setup(null)=0)
   GO TO exit_program
  ENDIF
  IF (daiv_chk_ccl_tabledef_sync(null)=0)
   GO TO exit_program
  ENDIF
  IF (daiv_auto_install_status(null)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 GO TO exit_program
 SUBROUTINE daiv_clinical_credentials(null)
   DECLARE dcc_cnct_str_len = i4 WITH protect, noconstant(0)
   DECLARE dcc_tmp_str_loc = i4 WITH protect, noconstant(0)
   DECLARE dcc_tmp_str_loc1 = i4 WITH protect, noconstant(0)
   DECLARE dcc_tmp_str_loc2 = i4 WITH protect, noconstant(0)
   DECLARE dcc_tmp_str_loc3 = i4 WITH protect, noconstant(0)
   DECLARE dcc_dminfo_cnt = i2 WITH protect, noconstant(0)
   DECLARE dcc_cnct_strng = vc WITH protect, noconstant(" ")
   DECLARE dcc_db_name = vc WITH protect, noconstant(" ")
   DECLARE dcc_pwd = vc WITH protect, noconstant(" ")
   SET dm_err->eproc = "Querying for Clinical credentials"
   CALL disp_msg("",dm_err->logfile,0)
   SET dm_err->eproc = "Querying dm_info"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    tmp_cnt = count(*)
    FROM dm_info d
    WHERE d.info_domain="DATA MANAGEMENT"
     AND d.info_name IN ("DM_ENV_NAME", "DM_ENV_ID")
    DETAIL
     dcc_dminfo_cnt = tmp_cnt
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Querying dm_info"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "Clin Credentials"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF (dcc_dminfo_cnt < 2)
    IF (daiv_load_reply("Clin Credentials","Missing dm_info rows related to Clinical credentials",0)=
    0)
     RETURN(0)
    ENDIF
    RETURN(1)
   ENDIF
   SET dm_err->eproc = "Querying dm_environment"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_environment de,
     dm_info d1,
     dm_info d2
    WHERE d1.info_domain="DATA MANAGEMENT"
     AND d1.info_name="DM_ENV_ID"
     AND d2.info_domain="DATA MANAGEMENT"
     AND d2.info_name="DM_ENV_NAME"
     AND d1.info_number=de.environment_id
     AND d2.info_char=de.environment_name
    DETAIL
     dcc_db_name = cnvtupper(trim(de.database_name)), dcc_cnct_strng = trim(de.v500_connect_string)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Querying dm_environment"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "Clin Credentials"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF (curqual=0)
    IF (daiv_load_reply("Clin Credentials","Missing Environment rows",0)=0)
     RETURN(0)
    ENDIF
    RETURN(1)
   ENDIF
   IF (dcc_db_name != "")
    SET dm2_install_schema->target_dbase_name = dcc_db_name
   ELSE
    IF (daiv_load_reply("Clin Credentials","Missing Clinical DB name",0)=0)
     RETURN(0)
    ENDIF
    RETURN(1)
   ENDIF
   IF (dcc_cnct_strng="")
    IF (daiv_load_reply("Clin Credentials","Missing Clinical DB credentials",0)=0)
     RETURN(0)
    ENDIF
    RETURN(1)
   ENDIF
   SET dcc_cnct_str_len = textlen(trim(dcc_cnct_strng))
   SET dcc_tmp_str_loc = findstring("/",dcc_cnct_strng,1,0)
   SET dcc_tmp_str_loc1 = findstring("@",dcc_cnct_strng,1,0)
   SET dcc_tmp_str_loc2 = (dcc_tmp_str_loc1 - dcc_tmp_str_loc)
   SET dcc_tmp_str_loc3 = (dcc_cnct_str_len - dcc_tmp_str_loc1)
   IF (((dcc_cnct_str_len < 5) OR (((dcc_tmp_str_loc <= 1) OR (((dcc_tmp_str_loc1=0) OR (((
   dcc_tmp_str_loc2 <= 1) OR (dcc_tmp_str_loc3 <= 0)) )) )) )) )
    IF (daiv_load_reply("Clin Credentials","Missing Clinical DB credentials",0)=0)
     RETURN(0)
    ENDIF
    RETURN(1)
   ENDIF
   SET daiv_clin_user_name = trim(substring(1,(dcc_tmp_str_loc - 1),dcc_cnct_strng))
   SET dm2_install_schema->v500_p_word = trim(substring((dcc_tmp_str_loc+ 1),(dcc_tmp_str_loc2 - 1),
     dcc_cnct_strng))
   SET dm2_install_schema->v500_connect_str = trim(substring((dcc_tmp_str_loc1+ 1),(dcc_tmp_str_loc3
     + 1),dcc_cnct_strng))
   IF (daiv_load_reply("Clin Credentials",concat("Clinical DB credentials for ",daiv_clin_user_name,
     " are found."),1)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_admin_credentials(null)
   DECLARE dac_admin_conn = vc WITH protect, noconstant("")
   DECLARE dac_cnct_str_len = i4 WITH protect, noconstant(0)
   DECLARE dac_tmp_str_loc = i4 WITH protect, noconstant(0)
   DECLARE dac_tmp_str_loc1 = i4 WITH protect, noconstant(0)
   DECLARE dac_tmp_str_loc2 = i4 WITH protect, noconstant(0)
   DECLARE dac_tmp_str_loc3 = i4 WITH protect, noconstant(0)
   DECLARE dac_admin_status = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Obtain Admin Credentials"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dac_admin_status = dad_get_admin_conn(dac_admin_conn)
   IF (dac_admin_status=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Admin Credentials"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "dad_get_admin_conn"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ELSEIF (dac_admin_status=2)
    SET dm_err->eproc = "Above Error message can be Ignored"
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dm_err->err_ind = 0
   ENDIF
   IF (dac_admin_conn="")
    IF (daiv_load_reply("Admin Credentials","Missing Admin DB credentials",0)=0)
     RETURN(0)
    ENDIF
    RETURN(1)
   ENDIF
   SET dac_cnct_str_len = textlen(trim(dac_admin_conn))
   SET dac_tmp_str_loc = findstring("/",dac_admin_conn,1,0)
   SET dac_tmp_str_loc1 = findstring("@",dac_admin_conn,1,0)
   SET dac_tmp_str_loc2 = (dac_tmp_str_loc1 - dac_tmp_str_loc)
   SET dac_tmp_str_loc3 = (dac_cnct_str_len - dac_tmp_str_loc1)
   IF (((dac_cnct_str_len < 5) OR (((dac_tmp_str_loc <= 1) OR (((dac_tmp_str_loc1=0) OR (((
   dac_tmp_str_loc2 <= 1) OR (dac_tmp_str_loc3 <= 0)) )) )) )) )
    IF (daiv_load_reply("Admin Credentials","Missing Admin DB credentials",0)=0)
     RETURN(0)
    ENDIF
    RETURN(1)
   ENDIF
   SET daiv_admin_user_name = trim(substring(1,(dac_tmp_str_loc - 1),dac_admin_conn))
   SET dm2_install_schema->cdba_p_word = trim(substring((dac_tmp_str_loc+ 1),(dac_tmp_str_loc2 - 1),
     dac_admin_conn))
   SET dm2_install_schema->cdba_connect_str = trim(substring((dac_tmp_str_loc1+ 1),(dac_tmp_str_loc3
     + 1),dac_admin_conn))
   IF (daiv_load_reply("Admin Credentials",concat("Admin DB credentials for ",daiv_admin_user_name,
     " are found"),1)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_clinical_app_os(null)
   SET dm_err->eproc = "Obtain Clinical App O/S"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ( NOT ((dm2_sys_misc->cur_os IN ("AIX", "HPX", "LNX"))))
    IF (daiv_load_reply("Clin O/S",concat(dm2_sys_misc->cur_os," is not an UNIX O/S"),0)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("Clin O/S",concat(dm2_sys_misc->cur_os," is the Clinical APP O/S"),1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_clinical_db_os(null)
   SET dm_err->eproc = "Obtain Admin DB O/S"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ( NOT ((dm2_sys_misc->cur_db_os IN ("AIX", "HPX", "LNX"))))
    IF (daiv_load_reply("Admin O/S",concat(dm2_sys_misc->cur_db_os," is not an UNIX O/S"),0)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("Admin O/S",concat(dm2_sys_misc->cur_db_os," is the Admin DB O/S"),1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_admin_link(dal_admin_link_name)
   DECLARE dal_fail_ind = i2 WITH protect, noconstant(0)
   SET dal_admin_link_name = ""
   IF (dir_get_admin_db_link(1,dal_admin_link_name,dal_fail_ind)=0)
    SET dm_err->emsg = "Invalid DBlink"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Admin link"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "dal_admin_link_name"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF (dal_fail_ind=1)
    IF (daiv_load_reply("Admin link","Invalid admin db link",0)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("Admin link",concat(dal_admin_link_name," is the valid admin db link"),1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_clinicaldb_storage(null)
   SET dm_err->eproc = "Obtain Clinical DB Storage type"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dir_get_storage_type("")=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "ClinDb storage"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "daiv_clinicaldb_storage"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF ((dir_storage_misc->cur_storage_type != "ASM"))
    IF (daiv_load_reply("ClinDb storage","Invalid Clinical DB storage type",0)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("ClinDb storage","ASM is the valid Clinical DB storage type",1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_admindb_storage(das_link)
   SET dm_err->eproc = "Obtain Admin DB Storage type"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dir_get_storage_type(das_link)=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Admin DB storage"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "dir_get_storage_type"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF ((dir_storage_misc->src_storage_type != "ASM"))
    IF (daiv_load_reply("Admindb storage","Invalid Admin DB storage type",0)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("Admindb storage","ASM is the valid Admin DB storage type",1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_clinicaldb_ver(null)
   SET dm_err->eproc = "Check for Clinical DB version"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_get_rdbms_version(null)=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "ClinDB Version"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "dm2_get_rdbms_version"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF ((((dm2_rdbms_version->level1 > 11)) OR ((dm2_rdbms_version->level1 >= 11)
    AND (dm2_rdbms_version->level2 >= 1))) )
    IF (daiv_load_reply("ClinDB Version",build(dm2_rdbms_version->level1,".",dm2_rdbms_version->
      level2," is the valid Clinical DB Version"),1)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("ClinDB Version","Invalid Clinical DB version",0)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_admindb_ver(dav_link)
   SET dm_err->eproc = "Check for Admin Db version"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_get_remote_rdbms_version(dav_link)=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "AdminDB version"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "dm2_get_remote_rdbms_version"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF ((((dm2_remote_rdbms_version->level1 > 11)) OR ((dm2_remote_rdbms_version->level1 >= 11)
    AND (dm2_remote_rdbms_version->level2 >= 1))) )
    IF (daiv_load_reply("AdminDB Version",build(dm2_remote_rdbms_version->level1,".",
      dm2_remote_rdbms_version->level2," is the valid Admin DB Version"),1)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("AdminDB Version","Invalid Admin DB version",0)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_unattended_setup(null)
   SET dm_err->eproc = "Check for Unattended Installation Setup"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dpl_ui_chk(dpl_package_install)=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Unattended Setup"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "dpl_ui_chk"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF ((dm2_process_event_rs->ui_allowed_ind=0))
    IF (daiv_load_reply("Unattended Setup","Unattended Installation not setup",0)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("Unattended Setup","Unattended Installation has been properly setup.",1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_scheduler_setup(null)
   SET dm_err->eproc = "Check for Installation Scheduler setup"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (drr_get_flexible_schedule(null)=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Installation scheduler"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "drr_get_flexible_schedule"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF ((drr_flex_sched->status=0))
    IF (daiv_load_reply("Installation scheduler","Installation scheduler is not enabled",0)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("Installation scheduler","Installation scheduler is enabled",1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_notification_setup(null)
   SET dm_err->eproc = "Check for Notification Setup"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dn_get_notify_settings(null)=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Notification setup"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "dn_get_notify_settings"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF ((dnotify->status=1))
    IF (daiv_load_reply("Notification setup",concat("Notifications has been enabled for ",dnotify->
      email_address_list),1)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("Notification setup","Notifications disabled",0)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_package_history(null)
   DECLARE dph_valid_ind = i2 WITH protect, noconstant(0)
   DECLARE dph_env_id = f8 WITH protect, noconstant(0.0)
   SET dm_err->eproc = "Check for Package History"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dph_valid_ind = 0
   IF (dir_check_for_package(dph_valid_ind,dph_env_id)=0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Package History"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "dm2_auto_install_verify"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF (dph_valid_ind=0)
    IF (dph_env_id=0)
     IF (daiv_load_reply("Package History","Failed to find env id",0)=0)
      RETURN(0)
     ENDIF
    ELSE
     IF (daiv_load_reply("Package History",build3(3,"No package history found for ",dph_env_id),0)=0)
      RETURN(0)
     ENDIF
    ENDIF
   ELSE
    IF (daiv_load_reply("Package History",build3(3,"Package history found for ",dph_env_id),1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_clinical_diskgroup_setup(null)
   DECLARE dcds_use_initprm_assign_dg_ind = i2 WITH noconstant(0)
   DECLARE dcds_assign_dg_override = vc WITH noconstant("NOT_SET")
   DECLARE dcds_diskgroup = vc WITH noconstant("NOT_SET")
   SET dm_err->eproc = "Obtain Clinical Diskgroup"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (daiv_adb_ind=0)
    IF (dir_get_dg_data(cnvtint(dm2_db_options->use_initprm_assign_dg_ind),dm2_db_options->
     assign_dg_override,dcds_diskgroup)=0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET reply->status_data.subeventstatus[1].operationname = "Clin Diskgroup"
     SET reply->status_data.subeventstatus[1].operationstatus = "F"
     SET reply->status_data.subeventstatus[1].targetobjectname = "dir_get_dg_data"
     SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
     RETURN(0)
    ENDIF
   ENDIF
   IF (((dcds_diskgroup != "NOT_SET") OR (daiv_adb_ind=1)) )
    IF (daiv_load_reply("Clin Diskgroup",concat(dcds_diskgroup," diskgroup exists."),1)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("Clin Diskgroup","No diskgroup found",0)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_auto_install_status(null)
   SET dm_err->eproc = "Auto install status"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_info d
   ...
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "select dm_info"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "Auto Install Status"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   IF (curqual=0)
    IF (daiv_load_reply("Auto Install Status","Auto install status is disabled",0)=0)
     RETURN(0)
    ENDIF
   ELSE
    IF (daiv_load_reply("Auto Install Status","Auto install status is enabled",1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_db_conn_check(ddcc_user,ddcc_pword,ddcc_cnnct_str,ddcc_admin_ind)
   DECLARE ddcc_connect_string = vc WITH protect, noconstant("")
   DECLARE ddcc_clin_dbname_main = vc WITH protect, noconstant("")
   DECLARE ddcc_admin_dbname_main = vc WITH protect, noconstant("")
   DECLARE ddcc_db_to_test = vc WITH protect, noconstant("")
   DECLARE ddcc_file_name = vc WITH protect, noconstant("")
   DECLARE ddcc_file_prefix = vc WITH protect, noconstant("")
   DECLARE ddcc_exec_cmd = vc WITH protect, noconstant("")
   DECLARE ddcc_pos1 = i4 WITH protect, noconstant(0)
   DECLARE ddcc_pos2 = i4 WITH protect, noconstant(0)
   DECLARE ddcc_idx = i4 WITH protect, noconstant(0)
   DECLARE ddcc_dbname = vc WITH protect, noconstant("")
   DECLARE ddcc_currdbhandle = vc WITH protect, noconstant("")
   IF (ddcc_admin_ind=1)
    SET ddcc_db_to_test = "Admin"
   ELSE
    SET ddcc_db_to_test = "Clin"
   ENDIF
   SET dm_err->eproc = concat(ddcc_db_to_test," Db connection check")
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (ddcc_admin_ind=1)
    IF (locateval(ddcc_idx,1,reply->check_list_cnt,"Admin Credentials",reply->check_list[ddcc_idx].
     check_name,
     0,reply->check_list[ddcc_idx].check_passed_ind) > 0)
     IF (daiv_load_reply("Admin DB connection check",reply->check_list[ddcc_idx].check_txt,0)=0)
      RETURN(0)
     ENDIF
     RETURN(1)
    ENDIF
   ELSE
    IF (locateval(ddcc_idx,1,reply->check_list_cnt,"Clin Credentials",reply->check_list[ddcc_idx].
     check_name,
     0,reply->check_list[ddcc_idx].check_passed_ind) > 0)
     IF (daiv_load_reply("Clin DB connection check",reply->check_list[ddcc_idx].check_txt,0)=0)
      RETURN(0)
     ENDIF
     RETURN(1)
    ENDIF
   ENDIF
   IF (ddcc_admin_ind=1)
    IF (dm2_get_remote_dbase_name(daiv_admin_link_name,ddcc_admin_dbname_main)=0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET reply->status_data.subeventstatus[1].operationname = "Remote database name"
     SET reply->status_data.subeventstatus[1].operationstatus = "F"
     SET reply->status_data.subeventstatus[1].targetobjectname = concat(ddcc_db_to_test,
      " Db connection check")
     SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
     RETURN(0)
    ENDIF
    IF (ddcc_admin_dbname_main="")
     SET reply->status_data.subeventstatus[1].operationname = "Validate Admin DBname"
     SET reply->status_data.subeventstatus[1].operationstatus = "F"
     SET reply->status_data.subeventstatus[1].targetobjectname = concat(ddcc_db_to_test,
      " Db connection check")
     SET reply->status_data.subeventstatus[1].targetobjectvalue = "Missing Admindb name"
     RETURN(0)
    ENDIF
    SET ddcc_file_prefix = concat("dm2_conn_check_",ddcc_admin_dbname_main)
   ELSE
    SET ddcc_clin_dbname_main = currdbname
    SET ddcc_file_prefix = concat("dm2_conn_check_",ddcc_clin_dbname_main)
   ENDIF
   IF (get_unique_file(ddcc_file_prefix,".ksh")=0)
    SET reply->status_data.subeventstatus[1].operationname = "Unique file name"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = concat(ddcc_db_to_test,
     " Db connection check")
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   SET ddcc_file_name = dm_err->unique_fname
   SET ddcc_connect_string = build("'",ddcc_user,"/",ddcc_pword,"@",
    ddcc_cnnct_str,"'")
   SELECT INTO trim(ddcc_file_name)
    DETAIL
     col 0, "#!/usr/bin/ksh", row + 1,
     col 0, "#", row + 1,
     col 0, ". $cer_mgr/",
     CALL print(trim(cnvtlower(logical("environment")))),
     "_environment.ksh", row + 1, col 0,
     "$cer_exe/cclora <<!", row + 1, col 0,
     "free define oraclesystem go", row + 1, col 0,
     "define oraclesystem ", ddcc_connect_string, " go",
     row + 1, col 0, "call echo (concat('<daiv_currdbhandle_value>=<',trim(currdbhandle),'>')) go ",
     row + 1, col 0, "call echo (concat('<daiv_db_name_value>=<',trim(currdbname),'>')) go",
     row + 1, col 0, "exit",
     row + 1, col 0, "!",
     row + 1
    WITH nocounter, maxrow = 1, format = variable,
   ...
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Creating KSH file"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = concat(ddcc_db_to_test,
     " Db connection check")
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   SET ddcc_exec_cmd = concat("chmod 777 $CCLUSERDIR/",ddcc_file_name)
   IF (dm2_push_dcl(ddcc_exec_cmd)=0)
    SET reply->status_data.subeventstatus[1].operationname = "Change .ksh permissions"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = concat(ddcc_db_to_test,
     " Db connection check")
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
    RETURN(0)
   ENDIF
   SET ddcc_exec_cmd = concat(". $CCLUSERDIR/",ddcc_file_name)
   SET dm_err->disp_dcl_err_ind = 0
   IF (dm2_push_dcl(ddcc_exec_cmd)=0)
    IF ((dm_err->err_ind=1))
     SET reply->status_data.subeventstatus[1].operationname = "Executing .ksh file"
     SET reply->status_data.subeventstatus[1].operationstatus = "F"
     SET reply->status_data.subeventstatus[1].targetobjectname = concat(ddcc_db_to_test,
      " Db connection check")
     SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
   ...
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dm_err)
   ENDIF
   SET dm_err->eproc = "Parsing .ksh logfile"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   FREE DEFINE rtl
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     IF ((dm_err->debug_flag > 0))
      CALL echo(t.line)
     ENDIF
     IF (findstring("<daiv_currdbhandle_value>=<",t.line,1,0) > 0)
      ddcc_pos1 = findstring("<",t.line,1,1), ddcc_pos2 = (findstring(">",t.line,1,1) - (ddcc_pos1+ 1
      )), ddcc_currdbhandle = substring((ddcc_pos1+ 1),ddcc_pos2,t.line)
     ENDIF
     IF (ddcc_currdbhandle != "")
      IF (findstring("<daiv_db_name_value>=<",t.line,1,0) > 0)
       ddcc_pos1 = findstring("<",t.line,1,1), ddcc_pos2 = (findstring(">",t.line,1,1) - (ddcc_pos1+
       1)), ddcc_dbname = substring((ddcc_pos1+ 1),ddcc_pos2,t.line)
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status_data.subeventstatus[1].operationname = "Parsing .ksh logfile"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = concat(ddcc_db_to_test,
     " Db connection check")
    SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
   ...
   ENDIF
   IF (ddcc_admin_ind=1)
    IF (ddcc_dbname=ddcc_admin_dbname_main)
     IF (daiv_load_reply("Admin DB connection check",concat("Admin DB is ",ddcc_admin_dbname_main),1)
     =0)
      RETURN(0)
     ENDIF
    ELSE
     IF (daiv_load_reply("Admin DB connection check","Admin DB name mismatch",0)=0)
      RETURN(0)
     ENDIF
    ENDIF
   ELSE
    IF (ddcc_dbname=ddcc_clin_dbname_main)
     IF (daiv_load_reply("Clin DB connection check",concat("Clinical DB is ",ddcc_clin_dbname_main),1
      )=0)
      RETURN(0)
     ENDIF
    ELSE
     IF (daiv_load_reply("Clin DB connection check","CLin DB name mismatch",0)=0)
      RETURN(0)
     ENDIF
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_load_reply(dlr_process,dlr_msg,dlr_status)
   SET reply->check_list_cnt = (reply->check_list_cnt+ 1)
   SET stat = alterlist(reply->check_list,reply->check_list_cnt)
   SET reply->check_list[reply->check_list_cnt].check_name = dlr_process
   SET reply->check_list[reply->check_list_cnt].check_txt = dlr_msg
   SET reply->check_list[reply->check_list_cnt].check_passed_ind = dlr_status
   SET dm_err->eproc = dlr_msg
   CALL disp_msg(" ",dm_err->logfile,0)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE daiv_chk_ccl_tabledef_sync(null)
   DECLARE dccts_nodes = vc WITH protect, noconstant("")
   DECLARE dccts_bypass_node_check = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Check for app node entries in dm_info and insert if they do not exist"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dan_insert_app_nodes(null)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Validate cronjob for app node CCL Tabledef sync"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dan_bypass_node_check(dccts_bypass_node_check)=0)
    RETURN(0)
   ENDIF
   IF (dccts_bypass_node_check=1)
    SET dm_err->eproc = "Bypassing validation of cron job setup for app node CCL Tabledef sync"
    CALL disp_msg(" ",dm_err->logfile,0)
    IF (daiv_load_reply("CCL Tabledef Sync",
     "Bypassing validation of cron job setup for app node CCL Tabledef sync",1)=0)
     RETURN(0)
    ENDIF
   ELSE
    SET dm_err->eproc = "Validate cronjob for app node CCL Tabledef sync"
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dccts_nodes = dan_get_missing_cron_node(null)
    IF ((dan_nodes_list->cnt > 1))
     IF (dccts_nodes="ERROR")
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Errored out in dan_get_missing_cron_node"
      SET reply->status_data.subeventstatus[1].operationname = "Check for multi-nodes"
      SET reply->status_data.subeventstatus[1].operationstatus = "F"
      SET reply->status_data.subeventstatus[1].targetobjectname = "daiv_chk_ccl_tabledef_sync"
      SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
      RETURN(0)
     ELSEIF (dccts_nodes != "")
      IF (daiv_load_reply("CCL Tabledef Sync",concat(
        "Missing cronjob for dm2_exec_sync_app_tabledef_wrapper.ksh setup on nodes: ",dccts_nodes),0)
      =0)
       RETURN(0)
      ENDIF
     ELSE
      IF (daiv_load_reply("CCL Tabledef Sync",
       "Cronjob for dm2_exec_sync_app_tabledef_wrapper.ksh  has been setup.",1)=0)
       RETURN(0)
      ENDIF
     ENDIF
    ELSE
...
#exit_program
 IF (locateval(daiv_idx,1,reply->check_list_cnt,0,reply->check_list[daiv_idx].check_passed_ind)=0)
  SET reply->auto_install_ind = 1
 ENDIF
 IF ((dm_err->err_ind=0))
  SET reply->status_data.status = "S"
  SET dm_err->eproc = "dm2_auto_install_verify completed"
 ENDIF
 IF ((dm_err->debug_flag >= 2))
  CALL echorecord(reply)
  CALL echorecord(dm2_install_schema)
 ENDIF
 CALL final_disp_msg("dm2_auto_install_verify")
END GO
