CREATE PROGRAM br_get_client_info:dba
 FREE SET reply
 RECORD reply(
   1 item_list[*]
     2 br_client_id = f8
     ...
     IF (repcnt2 > 0
      ...
      FOR (x = 1 TO repcnt2)
        SELECT INTO "nl:"
         ...
         PLAN (b
          WHERE value(reply->item_list[cloop].prsnl_list[x].br_prsnl_id)=b.br_prsnl_id
           AND b.item_type IN ("STEP", "STEPCAT")
           AND (b.br_client_id=request->item_list[cloop].br_client_id))
         HEAD REPORT
          scnt = 0, sccnt = 0
         DETAIL
          IF (b.item_type="STEP")
           scnt = (scnt+ 1), stat = alterlist(reply->item_list[cloop].prsnl_list[x].user_slist,scnt),
           reply->item_list[cloop].prsnl_list[x].user_slist[scnt].item_mean = b.item_mean,
           reply->item_list[cloop].prsnl_list[x].user_slist[scnt].item_type = b.item_type, reply->
           item_list[cloop].prsnl_list[x].user_slist[scnt].item_disp = b.item_display
          ELSE
           sccnt = (sccnt+ 1), stat = alterlist(reply->item_list[cloop].prsnl_list[x].user_sclist,
            sccnt), reply->item_list[cloop].prsnl_list[x].user_sclist[sccnt].item_mean = b.item_mean,
           reply->item_list[cloop].prsnl_list[x].user_sclist[sccnt].item_type = b.item_type, reply->
           item_list[cloop].prsnl_list[x].user_sclist[sccnt].item_disp = b.item_display
          ENDIF
   ...
   IF ((request->get_sol_ind=1))
    ...
    SELECT INTO "nl:"
     ...
     HEAD b.br_client_item_reltn_id
      ...
      reply->item_list[cloop].sol_list[cnt].sol_display = b.item_display, reply->item_list[cloop].
      sol_list[cnt].sequence = cnt, reply->item_list[cloop].sol_list[cnt].solution_type_flag = b
      .solution_type_flag,
      status0_ind = 0, status1_ind = 0, status2_ind = 0,
      cnt2 = 0
     DETAIL
      cnt2 = (cnt2+ 1), stat = alterlist(reply->item_list[cloop].sol_list[cnt].step_list,cnt2), reply
      ->item_list[cloop].sol_list[cnt].step_list[cnt2].step_mean = bcss.step_mean,
      reply->item_list[cloop].sol_list[cnt].step_list[cnt2].step_type = bs.step_type, reply->
      item_list[cloop].sol_list[cnt].step_list[cnt2].step_disp = bcir.item_display, reply->item_list[
      cloop].sol_list[cnt].step_list[cnt2].status_flag = bcir.status_flag,
    ...
    FOR (x = 1 TO sol_cnt)
     ...
     FOR (y = 1 TO step_cnt)
       ...
       SELECT INTO "nl:"
        ...
        PLAN (bsp
         WHERE (bsp.step_mean=reply->item_list[cloop].sol_list[x].step_list[y].step_mean))
        DETAIL
         paramcnt = (paramcnt+ 1), stat = alterlist(reply->item_list[cloop].sol_list[x].step_list[y].
          step_param_list,paramcnt), reply->item_list[cloop].sol_list[x].step_list[y].
         step_param_list[paramcnt].param_name = bsp.parameter_name,
         reply->item_list[cloop].sol_list[x].step_list[y].step_param_list[paramcnt].param_value = bsp
         .parameter_value, reply->item_list[cloop].sol_list[x].step_list[y].step_param_list[paramcnt]
         .param_seq = bsp.parameter_seq
        WITH nocounter, skipbedrock = 1
       ;end select
     ENDFOR
    ENDFOR
