CREATE PROGRAM bhs_finding_favor:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Name to search (wildcard * accepted)" = "",
  "Email Address:" = ""
  WITH outdev, s_search, s_email
 RECORD m_rec(
   1 m_list[*]
     2 mf_pathway_id = f8
     2 mf_synonym_id = f8
 ...
 SELECT INTO "nl:"
  ...
  PLAN (asl
   ...
   (SELECT
    ...
    WHERE o.mnemonic_key_cap=parser(ms_search)))) OR (asl.pathway_catalog_id IN (
   (SELECT
    pcg.pathway_catalog_id
    FROM pathway_catalog pcg
    WHERE pcg.description_key=parser(ms_search))))) )
   JOIN (o
