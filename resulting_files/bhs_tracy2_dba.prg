CREATE PROGRAM bhs_tracy2:dba
 ...
 SET ms_location_display = uar_get_code_display( $LOCATION)
 ...
 IF (( $LSTPERSON <= ""))
  ...
  GO TO exit_script
 ENDIF
 SET md_start_date = cnvtdatetime(format(datetimefind(cnvtlookbehind("8M",cnvtdatetime(md_end_dt_tm)),
    "M","B","E"),";;Q"))
 CALL echo(build("md_start_date2",format(md_start_date,"MM/DD/YY HH:MM;;q")))
 IF (datetimediff(cnvtdatetime(md_end_dt_tm),cnvtdatetime(md_start_date)) > 275.0)
  CALL echo("Hitting 294 rule")
  SET ms_output =  $MESSAGETOSCREEN
  SELECT INTO  $MESSAGETOSCREEN
   FROM dummyt
   HEAD REPORT
    ms_msg1 = "The Start Date range extends beyond 294 days .", ms_msg2 =
    "  Please Enter a date within the 294 days.", col 0,
    "{PS/792 0 translate 90 rotate/}", y_pos = 18, row + 1,
    "{F/1}{CPI/7}",
    CALL print(calcpos(36,(y_pos+ 0))), ms_msg1,
    row + 2, ms_msg2
 ...
 IF (( $LOCATION <= 0))
  SELECT INTO  $MESSAGETOSCREEN
   FROM dummyt
   HEAD REPORT
    ms_msg1 = "You have not provided a location.", ms_msg2 = "  Please retry.", col 0,
    "{PS/792 0 translate 90 rotate/}", y_pos = 18, row + 1,
    "{F/1}{CPI/7}",
    CALL print(calcpos(36,(y_pos+ 0))), ms_msg1,
    row + 2, ms_msg2
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  DETAIL
   ...
   IF (pe.method_cd > 0)
    prentl_sum->patient[ml_pat_cnt].mf_person_id = pi.person_id, prentl_sum->patient[ml_pat_cnt].
    ms_lmp_dt = format(pe.method_dt_tm,"MM/DD/YY;;d"), ms_lmp_ind = "Y"
   ENDIF
  WITH nocounter
 ;end select
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  DETAIL
   ...
   IF (pe.method_cd > 0)
    prentl_sum->patient[ml_pat_cnt].mf_person_id = pi.person_id, prentl_sum->patient[ml_pat_cnt].
    ms_edd_dt = format(pe.est_delivery_dt_tm,"MM/DD/YY;;q"), prentl_sum->patient[ml_pat_cnt].
 ...
 SUBROUTINE report_headerabs(ncalc,offsetx,offsety)
   ...
   DECLARE __location = vc WITH noconstant(build2(ms_location_display,char(0))), protect
   IF (ncalc=rpt_render)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_TRACY2"
