CREATE PROGRAM ct_core_ens_cd_value
 FREE RECORD reply
 RECORD reply(
   1 curqual = i4
   1 qual[*]
 ...
 FOR (xvar = 1 TO req_cnt)
   ...
   ELSE
    SET display_key = trim(cnvtupper(cnvtalphanum(substring(1,40,request->cd_value_list[xvar].display
        ))))
   ENDIF
   CALL build_wherestr(xvar)
   CASE (request->cd_value_list[xvar].action_type_flag)
    OF 1:
    ...
    OF 3:
     ...
     SET reply->qual[xvar].error_msg = concat("Deletes are not allowed ","for the CODE_VALUE table.")
     GO TO exit_script
    ELSE
     SET failed = "T"
     SET stat = alterlist(reply->qual,xvar)
     ...
     SET reply->qual[xvar].error_msg = build("Could not recognize"," action_type_flag:",request->
      cd_value_list[xvar].action_type_flag,".")
     GO TO exit_script
   ENDCASE
 ENDFOR
 GO TO exit_script
 SUBROUTINE (build_wherestr(aidx=i4) =null)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     IF (cvs.add_access_ind=1)
      add_access_ind = 1
     ENDIF
     IF (cvs.chg_access_ind=1)
      chg_access_ind = 1
     ENDIF
     ...
     IF (cvs.active_ind_dup_ind=1)
      ...
      active_ind < 0))) )
       ...
       reply->qual[aidx].error_msg = "Active indicator must be valid"
      ELSE
       IF (wherestr > " ")
     ...
     IF (cvs.cdf_meaning_dup_ind=1)
      IF (wherestr > " ")
       wherestr = concat(wherestr," and")
      ENDIF
      IF (trim(substring(1,12,request->cd_value_list[aidx].cdf_meaning)) > " ")
       wherestr = concat(trim(wherestr,3)," cv.cdf_meaning = ",
        "request->cd_value_list[aidx]->cdf_meaning")
      ELSE
       wherestr = concat(trim(wherestr,3)," cv.cdf_meaning = NULL")
      ENDIF
     ENDIF
     IF (cvs.display_dup_ind=1)
      IF (wherestr > " ")
       wherestr = concat(wherestr," and")
      ENDIF
      IF (trim(substring(1,40,request->cd_value_list[aidx].display)) > " ")
       wherestr = concat(trim(wherestr,3)," cv.display = ","request->cd_value_list[aidx]->display")
      ELSE
       wherestr = concat(trim(wherestr,3)," cv.display = NULL")
      ENDIF
     ENDIF
     IF (cvs.display_key_dup_ind=1)
      ...
      ENDIF
      IF (display_key > " ")
       wherestr = concat(trim(wherestr,3)," cv.display_key = display_key")
      ELSE
       wherestr = concat(trim(wherestr,3)," cv.display_key = NULL")
      ENDIF
     ENDIF
     IF (cvs.definition_dup_ind=1)
      IF (wherestr > " ")
       wherestr = concat(wherestr," and")
      ENDIF
      IF (trim(request->cd_value_list[aidx].definition,3) > " ")
       wherestr = concat(trim(wherestr,3)," cv.definition = ",
        "request->cd_value_list[aidx]->definition")
      ELSE
       wherestr = concat(trim(wherestr,3)," cv.definition = NULL")
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (insert_cd_value(aidx=i4) =null)
   IF (wherestr > " ")
    SELECT INTO "nl:"
     cv.code_value
     FROM code_value cv
    ...
    IF (curqual > 0)
     ...
     SET reply->qual[aidx].error_msg = concat("Code value could not be ",
   ...
   INSERT  FROM code_value cv
    SET cv.active_dt_tm =
     IF ((request->cd_value_list[aidx].active_ind=1)) cnvtdatetime(sysdate)
     ELSE null
     ENDIF
     ...
     IF ((request->cd_value_list[aidx].begin_effective_dt_tm <= 0)) cnvtdatetime(sysdate)
     ...
     ENDIF
     , cv.inactive_dt_tm =
     IF ((request->cd_value_list[aidx].active_ind=0)) cnvtdatetime(sysdate)
     ELSE null
     ENDIF
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (update_cd_value(aidx=i4) =null)
   IF (wherestr > " ")
    SELECT INTO "nl:"
     cv.code_value
     FROM code_value cv
   ...
   UPDATE  FROM code_value cv
    SET cv.active_dt_tm =
     IF ((request->cd_value_list[aidx].active_ind != old_active_ind)
      AND (request->cd_value_list[aidx].active_ind=1)) cnvtdatetime(sysdate)
     ...
     , cv.inactive_dt_tm =
     IF ((request->cd_value_list[aidx].active_ind != old_active_ind)
      AND (request->cd_value_list[aidx].active_ind=0)) cnvtdatetime(sysdate)
     ELSEIF ((request->cd_value_list[aidx].active_ind != old_active_ind)
...
#exit_script
 ...
 ENDIF
 SET script_version = "005 01/27/05 CERFTS"
END GO
