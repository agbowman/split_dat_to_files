CREATE PROGRAM dm2_mig_setup_info
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmsi_adm_ind = i2 WITH protect, noconstant(0)
 DECLARE dmsi_src_ind = i2 WITH protect, noconstant(0)
 DECLARE dmsi_src_sys_ind = i2 WITH protect, noconstant(0)
 DECLARE dmsi_tgt_ind = i2 WITH protect, noconstant(0)
 DECLARE dmsi_file = vc WITH protect, noconstant(concat(dmr_mig_data->dm2_mig_log,
   "dm2_mig_config.txt"))
 DECLARE dmsi_str = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_mig_setup_info",".log","dm2_mig_setup_info")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_setup_info"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_findfile(dmsi_file) > 0)
  SET message = window
  CALL clear(1,1)
  CALL box(1,1,6,132)
  CALL text(2,2,
   "ADVISORY: This operation should only be performed if the intention is to add or refresh connection information."
   )
  CALL text(4,2,"Do you wish to continue? (Y or N):")
  CALL accept(4,39,"P;CU"," "
   WHERE curaccept IN ("Y", "N"))
  SET message = nowindow
  IF (curaccept="Y")
   SET dm_err->eproc = concat("Attempting to remove ",dmsi_file)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF (remove(dmsi_file)=0)
    SET dm_err->emsg = concat("Unable to remove ",dmsi_file)
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
 ...
 SET dmsi_adm_ind = 1
 SET dmsi_src_ind = 1
 SET dmsi_src_sys_ind = 1
 SET message = window
 CALL clear(1,1)
 CALL box(1,1,6,132)
 CALL text(3,2,concat("Is the TARGET database available? (Y or N):"))
 CALL accept(3,46,"P;CU"," "
  WHERE curaccept IN ("Y", "N"))
 SET dmsi_tgt_ind = evaluate(curaccept,"Y",1,0)
 SET message = nowindow
 IF (dmr_prompt_retrieve_mig_data(dmsi_adm_ind,dmsi_src_ind,dmsi_tgt_ind,dmsi_src_sys_ind)=0)
  GO TO exit_program
 ENDIF
 IF (dmr_store_off_mig_data(null)=0)
  GO TO exit_program
 ENDIF
 IF (dmsi_tgt_ind=1)
  SET dmsi_str = concat("define ORACLESYSTEM 'v500/",trim(dmr_mig_data->src_v500_pwd),"@",trim(
    dmr_mig_data->src_v500_cnct_str),"' go")
  SET dm_err->eproc = "Create dm2migcon_src.ccl file in CCLUSERDIR..."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "dm2migcon_src.ccl"
   d.*
   FROM dual d
   DETAIL
    col 0, "%D NOECHO", row + 1,
    col 0, "free define oraclesystem go", row + 1,
    col 0, dmsi_str, row + 1,
    col 0, "set dm2_skip_create_date_check=1 go", row + 1,
    col 0,
    "select into noforms concat(                                                                 ",
    row + 1,
    col 0,
    "'Connected to instance ',trim(r2.instance_number),' of database ',trim(r2.database_name),   ",
    row + 1,
    col 0,
    "' running on host ',trim(r2.host_name),                                                     ",
    row + 1,
    col 0,
    "' as user ',trim(r2.user_name),'.') from                                                    ",
    row + 1,
    col 0,
    "((                                                                                          ",
    row + 1,
    ...
    row + 1,
    col 0,
    ^  database_name=sqlpassthru("sys_context('userenv','db_name')")                             ^,
    row + 1,
    col 0,
    ^  ,user_name=sqlpassthru("sys_context('userenv','session_user')")                           ^,
    row + 1,
    col 0,
    ^  ,host_name=sqlpassthru("sys_context('userenv','server_host')")                            ^,
    row + 1,
    col 0,
    ^  ,instance_number=sqlpassthru("sys_context('userenv','instance')")                         ^,
    row + 1,
    col 0,
    "  from dual                                                                                 ",
    row + 1,
    col 0,
    "  with sqltype('VC','VC','VC','VC')) r2)                                                    ",
    row + 1,
    col 0,
    "with nocounter go                                                                           ",
    row + 1
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_program
  ENDIF
  SET dmsi_str = concat("define ORACLESYSTEM 'v500/",trim(dmr_mig_data->tgt_v500_pwd),"@",trim(
    dmr_mig_data->tgt_v500_cnct_str),"' go")
  SET dm_err->eproc = "Create dm2migcon_tgt.ccl file in CCLUSERDIR..."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "dm2migcon_tgt.ccl"
   d.*
   FROM dual d
   DETAIL
    col 0, "%D NOECHO", row + 1,
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_mig_setup_info completed successfully"
 ENDIF
 CALL final_disp_msg("dm2_mig_setup_info")
END GO
