CREATE PROGRAM dm_cs_cpp_error_log_rows:dba
 ...
 END ;Subroutine
 DECLARE dm_cs_cpp_error_log_rows_version = vc WITH private, noconstant("224651.FT.001")
 ...
 SET reply->table_name = "CS_CPP_ERROR_LOG"
 ...
 ELSE
  ...
  SELECT INTO "nl:"
   ...
   WHERE parser(sbr_getrowidnotexists("e.error_dt_tm < cnvtdatetime(v_days_back)","e"))
    AND ((e.cs_cpp_error_log_id+ 0) > 0)
   DETAIL
    v_rows = (v_rows+ 1), stat = alterlist(reply->rows,v_rows), reply->rows[v_rows].row_id = e.rowid
   WITH nocounter, maxqual(e,value(request->max_rows))
  ;end select
