CREATE PROGRAM ecf_rdm_imo_secondary_xmap:dba
 ...
 DECLARE terminology_group = vc WITH public, noconstant("IMO SECONDARY XMAP")
 SET kia_tbl_desc = "IMO SECONDARY XMAP"
 SET kia_short_desc = "IMO SECONDARY XMAP"
 SET kia_rdm_log_name = "ecf_rdm_imo_secondary_xmap.log"
 SET kia_rdm_nbr = 11113
 ...
 IF (curqual > 0)
  IF (info_char_var != "US")
   CALL echo("This is not a IMO US Domain.")
   SET readme_data->status = "F"
   SET readme_data->message = "Failure - Cannot load IMO US content in a Non-US CMT Domain"
   GO TO exit_script
  ENDIF
 ELSE
  INSERT  FROM dm_info
   SET info_domain = "External Content Factory", info_char = "US", info_name = "IMO_DIALECT",
 ...
 EXECUTE kia_dm_dbimport "cer_install:imo_secondary_xmap_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("ecf_rdm_imo_secondary_xmap Begin> ",format(cnvtdatetime(curdate,curtime3),
