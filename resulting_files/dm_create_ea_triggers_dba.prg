CREATE PROGRAM dm_create_ea_triggers:dba
 ...
 SET trg_debug_flag = 0
 SET trig_exist_flag = 0
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: starting dm_create_ea_triggers.prg script."
 SET stat = check_logfile("dm_ea_trg",".log","DM_REBUILD_EA_TRIGGERS Begin")
 IF (validate(dm2_debug_flag,0) > 0
  AND validate(dm2_debug_flag,- (1)) > 0)
  SET trg_debug_flag = dm2_debug_flag
 ENDIF
 SET trig_exist_flag = select_triggers(null)
 IF (trig_exist_flag=2)
  IF (check_sequence(null) > 0)
 ...
 SUBROUTINE check_sequence(null)
   ...
   IF (trg_debug_flag > 1)
    CALL echo("***")
    CALL echo(build("DM_CLINICAL_SEQ last_number=",last_number))
    CALL echo(build("DM_ENTITY_ACTIVITY max_number=",max_number))
    CALL echo("***")
   ENDIF
   IF (max_number > last_number)
    SET readme_data->status = "F"
    SET readme_data->message =
    "Readme failed - DM_CLINICAL_SEQ is lower than values in DM_ENTITY_ACTIVITY"
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
