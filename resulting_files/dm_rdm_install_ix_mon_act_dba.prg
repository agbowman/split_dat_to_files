CREATE PROGRAM dm_rdm_install_ix_mon_act:dba
 ...
 SET readme_data->message = "Readme Failed: Starting script dm_rdm_install_ix_mon_act..."
 DECLARE errmsg = vc WITH protect, noconstant("")
 DECLARE dsj_retry_cnt = i4 WITH public, noconstant(0)
 DECLARE dsj_missing = i2 WITH public, noconstant(1)
 DECLARE dsj_maxretry = i4 WITH public, noconstant(20)
 DECLARE dsrj_cnt_idx = i2 WITH protect, noconstant(0)
 IF (currdb != "ORACLE")
 ...
 IF ((dm2_rdbms_version->level1 < 11))
  ...
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql "cer_install:DM2_ORACLE_VERSION.sql"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql_chk "DM2_ORACLE_VERSION", "VIEW"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql "cer_install:DM2_ORACLE_LEVEL.sql"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql_chk "DM2_ORACLE_LEVEL", "FUNCTION"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql "cer_install:DM2_CHECK_OBJ_IN_PARSE.sql"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql_chk "DM2_CHECK_OBJ_IN_PARSE", "FUNCTION"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ...
 EXECUTE dm_readme_include_sql "cer_install:DM2_IX_MONITORING_ACTIVATE.sql"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 SET dris_skip_test_file = 0
 EXECUTE dm_readme_include_sql_chk "DM2_IX_MONITORING_ACTIVATE", "PROCEDURE"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ...
 SELECT INTO "NL:"
  ...
  WHERE dsj.job_name="DM2_DAILY_IX_MON_ACTIVATE"
   AND dsj.owner="V500"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
 ...
 IF (curqual > 0)
  SELECT INTO "nl:"
   dsrj_qry_tmp = count(*)
   FROM dba_scheduler_running_jobs dsrj
   WHERE dsrj.owner="V500"
    AND dsrj.job_name="DM2_DAILY_IX_MON_ACTIVATE"
   DETAIL
    dsrj_cnt_idx = dsrj_qry_tmp
   WITH nocounter
  ;end select
  ...
  ENDIF
  IF (dsrj_cnt_idx > 0)
   IF (dm2_push_cmd(concat(^RDB ASIS("BEGIN DBMS_SCHEDULER.STOP_JOB('^,v500,dm2_daily_ix_mon_activate,
     ^',force=>true);END;")GO^),1)=0)
    IF (findstring("ORA-27366",dm_err->emsg) > 0
     AND findstring("DM2_DAILY_IX_MON_ACTIVATE",dm_err->emsg) > 0)
     SET dm_err->err_ind = 0
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
   ENDIF
  ENDIF
  CALL parser(
   "RDB ASIS (^ BEGIN DBMS_SCHEDULER.DROP_JOB(job_name=> 'V500.DM2_DAILY_IX_MON_ACTIVATE',defer=> TRUE); END;^) go ",
   1)
  IF (error(errmsg,0) > 0)
   SET readme_data->status = "F"
   SET readme_data->message = concat("Failed to drop job V500.DM2_DAILY_IX_MON_ACTIVATE: ",errmsg)
   GO TO exit_script
  ENDIF
 ENDIF
 CALL parser("RDB ASIS (^ BEGIN DBMS_SCHEDULER.CREATE_JOB (^)",0)
 CALL parser("ASIS (^    job_name=> 'DM2_DAILY_IX_MON_ACTIVATE',^)",0)
 CALL parser("ASIS (^    job_type=> 'STORED_PROCEDURE',^)",0)
 CALL parser("ASIS (^    job_action=> 'V500.DM2_IX_MONITORING_ACTIVATE',^)",0)
 CALL parser("ASIS (^    start_date => SYSTIMESTAMP,^)",0)
 CALL parser("ASIS (^    repeat_interval => 'FREQ=DAILY;BYHOUR=01;BYMINUTE=30',^)",0)
 ...
 IF (error(errmsg,0) > 0)
  ...
  SET readme_data->message = concat(
   "Failed to create DM2_DAILY_IX_MON_ACTIVATE job using call parser: ",errmsg)
  GO TO exit_script
 ENDIF
 WHILE (dsj_missing=1
  AND dsj_retry_cnt < dsj_maxretry)
   SET dsj_missing = 0
   SELECT INTO "NL:"
    FROM dba_scheduler_jobs dsj
    WHERE dsj.job_name="DM2_DAILY_IX_MON_ACTIVATE"
   ...
   IF (error(errmsg,0) > 0)
    ...
    SET readme_data->message = concat("Failed to select from DBA_SCHEDULER_JOBS: ",errmsg)
    GO TO exit_script
   ELSEIF ((dsj_retry_cnt=(dsj_maxretry - 1)))
    SET readme_data->status = "F"
    SET readme_data->message = concat("Failed to create DM2_DAILY_IX_MON_ACTIVATE job: ",errmsg)
    GO TO exit_script
   ELSEIF (curqual=0)
    SET dsj_missing = 1
   ENDIF
   IF (dsj_missing=1)
    CALL pause(3)
    SET dsj_retry_cnt = (dsj_retry_cnt+ 1)
   ENDIF
 ENDWHILE
 SET readme_data->status = "S"
