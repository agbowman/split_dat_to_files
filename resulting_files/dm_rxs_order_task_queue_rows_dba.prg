CREATE PROGRAM dm_rxs_order_task_queue_rows:dba
 ...
 DECLARE pending_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4904,"PENDING"))
 DECLARE block_size = i4 WITH protect, constant(200)
 ...
 SET reply->table_name = "RXS_ORDER_TASK_QUEUE"
 SET reply->rows_between_commit = min(200,request->max_rows)
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    minval = min(rotq.rxs_order_task_queue_id)
    FROM rxs_order_task_queue rotq
    WHERE rotq.rxs_order_task_queue_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   maxval = max(rotq.rxs_order_task_queue_id)
   FROM rxs_order_task_queue rotq
   DETAIL
  ...
  WHILE (v_cur_min_id <= v_max_id
   AND v_rows_left > 0)
    CALL echo(build("v_cur_min_id: ",v_cur_min_id,"v_max_id: ",v_max_id,"v_rows_left: ",
      v_rows_left))
    SELECT INTO "nl:"
     rotq.rowid
     FROM rxs_order_task_queue rotq
     WHERE rotq.rxs_order_task_queue_id BETWEEN v_cur_min_id AND v_cur_max_id
      AND parser(sbr_getrowidnotexists("rotq.updt_dt_tm < cnvtdatetime(v_days_back)","rotq"))
      AND rotq.rxs_order_task_queue_id > 0
      AND rotq.state_cd != pending_cd
     DETAIL
      v_rows = (v_rows+ 1)
      IF (mod(v_rows,block_size)=1)
       stat = alterlist(reply->rows,((v_rows+ block_size) - 1))
      ENDIF
      reply->rows[v_rows].row_id = rotq.rowid
     WITH nocounter, maxqual(rotq,value(v_rows_left))
    ;end select
...
#exit_script
 CALL echo("Last mod date: 5/28/15 last mod version:000")
END GO
