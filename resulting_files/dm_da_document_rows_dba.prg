CREATE PROGRAM dm_da_document_rows:dba
 ...
 SET reply->table_name = "DA_DOCUMENT"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(da.da_document_id)
   FROM da_document da
   WHERE da.da_document_id > 0
   DETAIL
 ...
 IF ((reply->err_code > 0))
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"INITERROR","Failed in purge init: %1","s",
   nullterm(reply->err_msg))
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  seqval = max(da.da_document_id)
  FROM da_document da
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    da.rowid
    FROM da_document da
    WHERE parser(sbr_getrowidnotexists("da.da_document_id between curMinID and curMaxID","da"))
     AND da.valid_until_dt_tm < cnvtdatetime(curdate,0000)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = da.rowid
    WITH nocounter, maxqual(da,value(rowsleft))
   ;end select
   SET reply->err_code = error(reply->err_msg,1)
   IF ((reply->err_code > 0))
    SET reply->status_data.status = "F"
