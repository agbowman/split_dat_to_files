CREATE PROGRAM ct_pt_mp_get_prscrn_job_stats:dba
 PROMPT
 ...
 SUBROUTINE (populatereplywitherrormessageifany(reply=vc(ref)) =vc)
   ...
   RETURN(errormessage)
 END ;Subroutine
 SET log_program_name = "ct_pt_mp_get_prscrn_job_stats"
 FREE RECORD request
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_pt_mp_get_prscrn_job_stats",
 ...
 SET reply->status_data.status = "F"
 SELECT INTO "nl:"
  FROM ct_prot_prescreen_job_info cpp,
   ct_prescreen_job cpj
  PLAN (cpp
   WHERE (cpp.person_id=request->person_id))
   JOIN (cpj
   WHERE cpj.ct_prescreen_job_id=cpp.ct_prescreen_job_id)
  ORDER BY cpj.job_start_dt_tm DESC
  HEAD cpp.person_id
   reply->completed_flag = cpp.completed_flag
  WITH nocounter
 ;end select
 IF (curqual > 0)
  SET reply->status_data.status = "S"
 ELSE
  SET reply->completed_flag = 1
