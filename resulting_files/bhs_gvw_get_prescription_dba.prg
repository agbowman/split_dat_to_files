CREATE PROGRAM bhs_gvw_get_prescription:dba
 ...
 IF ((prsn_orders->l_cnt > 0))
  FOR (l_idx = 1 TO prsn_orders->l_cnt)
    IF (l_idx=1)
     SET ms_text_temp = concat(ms_rh2b,"{Prescriptions}",ms_reol)
     SET reply->text = concat(reply->text," ",ms_text_temp)
    ENDIF
    SET ms_text_temp = concat(ms_wb,"{",prsn_orders->list[l_idx].s_order_name,"}",ms_reol,
     ms_wr,"{",prsn_orders->list[l_idx].s_gen_order_disp_line,"}",ms_reol,
     "{Next Dose: }",ms_reol,ms_reol)
    SET reply->text = concat(reply->text," ",ms_text_temp)
  ENDFOR
 ELSE
  SET ms_text_temp = concat(ms_rh2b," No new prescriptions during this visit.")
  SET reply->text = concat(reply->text," ",ms_text_temp)
 ENDIF
