CREATE PROGRAM dm_workflow_task_q_hist_rows:dba
 ...
 SET reply->table_name = "WORKFLOW_TASK_QUEUE_HIST"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(wtq.workflow_task_queue_hist_id)
   FROM workflow_task_queue_hist wtq
   WHERE wtq.workflow_task_queue_hist_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(wtq.workflow_task_queue_hist_id)
  FROM workflow_task_queue_hist wtq
  WHERE wtq.workflow_task_queue_hist_id > 0.0
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    w1.rowid
    FROM workflow_task_queue_hist w1
    PLAN (w1
     WHERE w1.workflow_task_queue_hist_id BETWEEN curminid AND curmaxid
      AND parser(sbr_getrowidnotexists("w1.updt_dt_tm <= cnvtdatetime(purge_dt_tm)","w1"))
      AND w1.workflow_task_queue_hist_id != 0
      AND w1.process_dt_tm < cnvtdatetime(curdate,curtime))
    DETAIL
     row_cnt += 1
     IF (mod(row_cnt,50)=1)
