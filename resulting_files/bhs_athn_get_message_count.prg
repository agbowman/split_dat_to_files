CREATE PROGRAM bhs_athn_get_message_count
 ...
 SELECT INTO "NL:"
  ...
  HEAD t.task_id
   ...
   "count") OR (((t_event_class_mean="rad") OR (t_event_class_mean="resultdoc")) )) )) )) )) )) ))
   )) )) )) )
    total_count += 1, res_count += 1
   ENDIF
  WITH time = 20
 ;end select
 SELECT INTO "NL:"
 ...
 SELECT DISTINCT INTO "NL:"
  ...
  PLAN (o
   WHERE o.to_prsnl_id=prsnl_id
    ...
    AND o.notification_dt_tm BETWEEN (sysdate - 30) AND sysdate)
   JOIN (oa
  ...
  HEAD o.order_notification_id
   total_count += 1, ord_count += 1, inbox_ord_count += 1
  WITH time = 30
 ;end select
 SELECT INTO  $1
  FROM dummyt d1
