CREATE PROGRAM bhs_rpt_deed_report_112808:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  FROM tracking_checkin tc,
   ...
   order_action oa
  PLAN (tc
   WHERE tc.checkout_dt_tm BETWEEN cnvtdatetime(cnvtdate(110108),0) AND cnvtdatetime(cnvtdate(121308),
    0))
   JOIN (ti
  ...
  HEAD REPORT
   p_cnt = 0
  HEAD tc.tracking_id
   p_cnt = (p_cnt+ 1), pat->pat_cnt = p_cnt, stat = alterlist(pat->pat_count,p_cnt),
   ...
   (p.sex_cd), pat->pat_count[p_cnt].pat_age = cnvtage(p.birth_dt_tm),
 ...
 SELECT INTO "nl:"
  ...
  HEAD tid
   l_cnt = 0
  DETAIL
   l_cnt = (l_cnt+ 1), stat = alterlist(pat->pat_count[d.seq].last_location,l_cnt), pat->pat_count[d
   .seq].last_location[l_cnt].unit = uar_get_code_display(tl.loc_nurse_unit_cd),
 ...
 IF (findfile("deeds_rpt.csv")=1)
  ...
  CALL emailfile(filename_in,filename_in,"naser.sanjar@bhs.org",subject_line,1)
 ENDIF
#exit_script
END GO
