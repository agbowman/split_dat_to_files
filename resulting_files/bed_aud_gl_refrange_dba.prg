  ...
  DETAIL
   ...
   IF (found_ind=1)
    ...
    ELSEIF (rrf.critical_ind=2)
     temp->dlist[d.seq].rrf[rcnt].critical_high = format(rrf.critical_high,
      "##########.##########;I;f")
    ...
    ELSEIF (rrf.feasible_ind=2)
     temp->dlist[d.seq].rrf[rcnt].feasible_high = format(rrf.feasible_high,
      "##########.##########;I;f")
    ...
    IF (cvdelta.code_value > 0)
     ...
     IF (rrf.delta_minutes > 0)
      ...
      ELSE
       ...
       ELSE
        temp->dlist[d.seq].rrf[rcnt].delta_time_value = rrf.delta_minutes, temp->dlist[d.seq].rrf[
        rcnt].delta_time_unit = "Min"
       ENDIF
      ENDIF
     ENDIF
    ENDIF
...
#skip_processing
 ...
 SUBROUTINE convert_range_number(row_nbr)
   ...
   FOR (ptr = 1 TO size(trim(text_nbr),3))
     ...
     IF (text_char > " "
      ...
      SET start_pos = ptr
     ENDIF
     IF (text_char=curlocale("DECIMAL"))
   ...
   RETURN(1)
 END ;Subroutine
 SET stat = alterlist(reply->collist,41)
 ...
 SET reply->collist[24].header_text = "MDI Alias"
 SET reply->collist[24].data_type = 1
 SET reply->collist[24].hide_ind = 0
 SET reply->collist[25].header_text = "Unique Identifier"
 SET reply->collist[25].data_type = 1
 SET reply->collist[25].hide_ind = 0
 SET reply->collist[26].header_text = "Vocabulary"
   ...
   FOR (y = 1 TO rcnt)
     ...
     IF (found_ind=1)
      IF ((temp->dlist[x].rrf[y].skip_ind=0))
       ...
       SET stat = alterlist(reply->rowlist[row_nbr].celllist,41)
       ...
       SET reply->rowlist[row_nbr].celllist[31].string_value = trim(temp->dlist[x].rrf[y].units)
       SET reply->rowlist[row_nbr].celllist[32].string_value = trim(temp->dlist[x].rrf[y].
       ...
       IF ((temp->dlist[x].rrf[y].delta_time_value > 0))
        SET reply->rowlist[row_nbr].celllist[33].string_value = cnvtstring(temp->dlist[x].rrf[y].
         delta_time_value)
       ENDIF
       SET reply->rowlist[row_nbr].celllist[34].string_value = trim(temp->dlist[x].rrf[y].
        delta_time_unit)
       SET reply->rowlist[row_nbr].celllist[35].string_value = temp->dlist[x].rrf[y].delta_value_disp
       ...
       SET reply->rowlist[row_nbr].celllist[37].string_value = cnvtstring(temp->dlist[x].rrf[y].
        max_digits)
       SET reply->rowlist[row_nbr].celllist[38].string_value = cnvtstring(temp->dlist[x].rrf[y].
        min_decimal_places)
       SET reply->rowlist[row_nbr].celllist[39].double_value = temp->dlist[x].task_assay_cd
       SET reply->rowlist[row_nbr].celllist[40].double_value = temp->dlist[x].rrf[y].
       service_resource_cd
       SET reply->rowlist[row_nbr].celllist[41].double_value = temp->dlist[x].rrf[y].
