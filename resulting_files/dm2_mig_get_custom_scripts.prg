CREATE PROGRAM dm2_mig_get_custom_scripts
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmgcs_logfile_prefix = vc WITH protect, constant("dm2_mig_get_custom")
 DECLARE dmgcs_temp_line = vc WITH protect, noconstant("")
 DECLARE dmgcs_tmp_str = vc WITH protect, noconstant("")
 DECLARE dmgcs_hold_eproc = vc WITH protect, noconstant("")
 DECLARE dmgcs_hold_emsg = vc WITH protect, noconstant("")
 DECLARE dmgcs_env = vc WITH protect, noconstant("")
 DECLARE dmgcs_node = vc WITH protect, noconstant("")
 DECLARE dmgcs_csv_file = vc WITH protect, noconstant("")
 SET dmgcs_csv_file = concat(dm2_install_schema->ccluserdir,"dm2_script_data.csv")
 DECLARE dmgcs_rpt_file = vc WITH protect, noconstant("dm2_script_diff.rpt")
 FREE RECORD dmgcs_scripts
 RECORD dmgcs_scripts(
   1 cnt = i4
   1 qual[*]
     2 script_name = vc
     2 source_name = vc
 ...
 SET width = 132
 IF (check_logfile(dmgcs_logfile_prefix,".log","dm2_mig_get_custom_scripts")=0)
  GO TO exit_program
 ENDIF
 SET dmgcs_env = logical("environment")
 SET dmgcs_node = curnode
 IF (cursys="AXP")
  SET message = window
  CALL clear(1,1)
  CALL box(1,1,5,130)
  CALL text(2,5,concat("This process will capture all scripts in SOURCE (",dmgcs_env,
    " environment on node ",dmgcs_node,")"))
  CALL text(4,5,"Do you want to (C)ontinue or (Q)uit?")
  CALL accept(4,60,"P;CU"," "
   WHERE curaccept IN ("C", "Q"))
  SET message = nowindow
  IF (curaccept="Q")
   SET dm_err->eproc = concat("Capturing scripts in ",dmgcs_env," on node ",dmgcs_node)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "User choose to quit."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = concat("Capture all scripts from ",dmgcs_env," on node ",dmgcs_node," in ",
   dmgcs_csv_file)
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO value(dmgcs_csv_file)
   FROM dprotect p
   WHERE p.object="P"
   ORDER BY p.object_name
   DETAIL
    dmgcs_temp_line = build("^",trim(p.object_name),"^,^",trim(p.source_name),"^,^",
     trim(p.user_name),"^,^",format(p.datestamp,"DD-MMM-YY;;D"),"^,^",format(p.timestamp,
      "HH:MM:SS;2;m"),
     "^"), col 0, dmgcs_temp_line,
    row + 1
  ...
  CALL clear(1,1)
  CALL box(1,1,10,130)
  CALL text(2,5,"Please complete following steps:")
  CALL text(4,5,
   "1, Copy dm2_script_data.csv in lower case from CCLUSERDIR in Source to Target (Application Node) CCLUSERDIR."
   )
  CALL text(6,5,"2, Run script dm2_mig_get_custom_scripts in TARGET.")
  CALL text(8,5,"Upon successful completion of these steps, enter 'C' to continue :")
  CALL accept(8,80,"P;CU"," "
 ...
 ELSE
  ...
  CALL box(1,1,6,130)
  CALL text(2,5,
   "This process will compare scripts in dm2_script_data.csv captured from SOURCE dictionary with scripts in "
   )
  CALL text(3,5,concat("Target (",dmgcs_env," environment on node ",dmgcs_node,")"))
  CALL text(5,5,"Do you want to (C)ontinue or (Q)uit? ")
  CALL accept(5,60,"P;CU"," "
   WHERE curaccept IN ("C", "Q"))
  SET message = nowindow
  IF (curaccept="Q")
   SET dm_err->eproc = "Source and Target scripts comparison confirmation."
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "User choose to quit."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (dm2_findfile(dmgcs_csv_file)=0)
   SET dm_err->err_ind = 1
   SET dm_err->eproc = concat("Verify file ",dmgcs_csv_file," exists.")
   SET dm_err->emsg = concat("File ",dmgcs_csv_file," not found.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = concat("Load ",dmgcs_csv_file," in record.")
  CALL disp_msg("",dm_err->logfile,0)
  FREE DEFINE rtl3
  SET logical file_loc dmgcs_csv_file
  DEFINE rtl3 "file_loc"
  SELECT INTO "nl:"
   FROM rtl3t t
   WHERE t.line > " "
   HEAD REPORT
    cnt = 0, tmp_strt = 0, tmp_end = 0,
    tmp_abs_end = 0, tmp_row_cnt = 0
   DETAIL
    cnt = (cnt+ 1)
    IF (mod(cnt,1000)=1)
     stat = alterlist(dmgcs_scripts->qual,(cnt+ 999))
    ENDIF
    tmp_abs_end = findstring(",",t.line,1,1), tmp_strt = 0, tmp_end = 0,
    tmp_row_cnt = 0
    WHILE (tmp_end < tmp_abs_end)
      tmp_row_cnt = (tmp_row_cnt+ 1), tmp_strt = findstring("^",t.line,(tmp_end+ 1),0), tmp_end =
      findstring("^",t.line,(tmp_strt+ 1),0),
      dmgcs_tmp_str = substring((tmp_strt+ 1),((tmp_end - tmp_strt) - 1),t.line)
      CASE (tmp_row_cnt)
       OF 1:
        dmgcs_scripts->qual[cnt].script_name = dmgcs_tmp_str
       OF 2:
        dmgcs_scripts->qual[cnt].source_name = dmgcs_tmp_str
       OF 3:
        dmgcs_scripts->qual[cnt].user_name = dmgcs_tmp_str
       OF 4:
        dmgcs_scripts->qual[cnt].datestamp = dmgcs_tmp_str
       OF 5:
        dmgcs_scripts->qual[cnt].timestamp = dmgcs_tmp_str
      ENDCASE
    ENDWHILE
   FOOT REPORT
    stat = alterlist(dmgcs_scripts->qual,cnt), dmgcs_scripts->cnt = cnt
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF ((dm_err->debug_flag > 10))
   CALL echorecord(dmgcs_scripts)
  ENDIF
  SET dm_err->eproc = concat("Store scripts in Source not in Target in ",dmgcs_rpt_file)
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO value(dmgcs_rpt_file)
   FROM dprotect p
   WHERE p.object="P"
   ORDER BY p.object_name
   HEAD REPORT
    tmp_line = fillstring(180,"-"), src_script = 1, src_script_cnt = 0,
    row + 1,
    CALL center("Scripts in SOURCE dictionary NOT in TARGET dictionary",1,126), row + 1,
    CALL center("(Report can be found in CCLUSERDIR:dm2_script_diff.rpt)",1,126), row + 1, row + 1,
    col 1, "Script Name", col 35,
    "Location of Script code in SOURCE domain", col 117, "User Name",
    col 137, "Datestamp", col 147,
    "Timestamp", row + 1, col 1,
    tmp_line, row + 1
   DETAIL
    FOR (src_script_cnt = src_script TO dmgcs_scripts->cnt)
      IF ((dmgcs_scripts->qual[src_script_cnt].script_name < p.object_name))
       col 1, dmgcs_scripts->qual[src_script_cnt].script_name, col 35,
       dmgcs_scripts->qual[src_script_cnt].source_name, col 117, dmgcs_scripts->qual[src_script_cnt].
       user_name,
       col 137, dmgcs_scripts->qual[src_script_cnt].datestamp, col 147,
       dmgcs_scripts->qual[src_script_cnt].timestamp, row + 1
       IF ((dm_err->debug_flag > 1))
        CALL echo(concat("SOURCE script ",dmgcs_scripts->qual[src_script_cnt].script_name,
         " was not found in TARGET"))
       ENDIF
       src_script = (src_script+ 1)
      ELSEIF ((dmgcs_scripts->qual[src_script_cnt].script_name=p.object_name))
       src_script = (src_script+ 1)
      ELSE
       src_script_cnt = (dmgcs_scripts->cnt+ 1)
      ENDIF
    ENDFOR
   FOOT REPORT
    FOR (src_script_cnt = src_script TO dmgcs_scripts->cnt)
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_program
  ENDIF
  IF (dm2_disp_file(dmgcs_rpt_file,"Source and Target Scripts Difference Report")=0)
   GO TO exit_program
  ENDIF
 ENDIF
 GO TO exit_program
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "DM2_MIG_GET_CUSTOM_SCRIPTS completes successfully"
 CALL final_disp_msg(dmgcs_logfile_prefix)
END GO
