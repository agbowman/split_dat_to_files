CREATE PROGRAM dm2_remove_mvj_eppr_pgr:dba
 ...
 SET readme_data->message = "Readme Failed: Starting script dm2_remove_mvj_eppr_pgr..."
 IF (check_logfile("dm2_rm_mvj_eppr_pgr",".log","dm2_remove_mvj_eppr_pgr")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_remove_mvj_eppr_pgr"
 CALL disp_msg("",dm_err->logfile,0)
 IF (dmvr_updt_mv_inst("IU","MVJ_EPPR_PGR",1,"DISABLED")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Deleting any existing MVJ_EPPR_PGR preference from NAME_VALUE_PREFS..."
 DELETE  FROM name_value_prefs n
  WHERE n.pvc_name="MATERIALIZED_VIEWS"
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ELSE
  COMMIT
 ENDIF
 EXECUTE dm2_mv_drop_bytable "ENCNTR_PRSNL_RELTN"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 EXECUTE dm2_mv_drop_bytable "ENCNTR_DOMAIN"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 EXECUTE dm2_mv_drop_bytable "PRSNL_GROUP_RELTN"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 EXECUTE dm2_mv_drop_bytable "TEAM_MEM_PPR_RELTN"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 GO TO exit_program
#exit_program
 IF ( NOT (dm_err->err_ind))
  ...
  SET readme_data->message = "Success: Disabling and Removal of MVJ_EPPR_PGR was successful."
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error removing/disabling MVJ_EPPR_PGR:",dm_err->emsg)
 ENDIF
 CALL echorecord(readme_data)
 EXECUTE dm_readme_status
 SET dm_err->eproc = "Ending dm2_remove_mvj_eppr_pgr."
 CALL final_disp_msg("dm2_rm_mvj_eppr_pgr")
END GO
