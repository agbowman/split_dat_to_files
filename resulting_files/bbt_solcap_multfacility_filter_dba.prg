CREATE PROGRAM bbt_solcap_multfacility_filter:dba
 RECORD facilitylist(
   1 locs[*]
     2 facilitycd = f8
     2 preference_value = i4
 )
 ...
 SUBROUTINE (bbtgetprodtagverifypreference(facility_code=f8(value)) =vc)
   ...
   ENDIF
 END ;Subroutine
 DECLARE constant_yes = c12 WITH protect, constant("YES")
 DECLARE constant_no = c12 WITH protect, constant("NO")
 DECLARE flex_on_ind = i2 WITH protect, noconstant(0)
 ...
 DECLARE pref_value = i4 WITH protect, noconstant(0.0)
 SET stat = alterlist(reply->solcap,1)
 SET reply->solcap[1].identifier = "2013.1.00038.2"
 SET reply->solcap[1].degree_of_use_num = 0
 SET reply->solcap[1].distinct_user_count = 0
 SET reply->solcap[1].degree_of_use_str = constant_yes
 FREE SET res_count
 ...
 SELECT
  ...
  DETAIL
   ...
   IF (facility_index_found > 0)
    pref_value = facilitylist->locs[facility_index_found].preference_value
   ELSE
    ...
    IF (bbtgetflexspecenableflexexpiration(test_facility_cd)=1)
     facilitylist->locs[fac_list_size].preference_value = bbtgetflexfilterbyfacility(test_facility_cd
      )
    ELSE
     facilitylist->locs[fac_list_size].preference_value = 0
    ENDIF
    pref_value = facilitylist->locs[fac_list_size].preference_value
   ENDIF
   res_count += 1
   IF (mod(res_count,10)=1)
    stat = alterlist(reply->solcap[1].facility,(res_count+ 9))
   ENDIF
   reply->solcap[1].facility[res_count].display = cv.display
   IF (pref_value=1)
    reply->solcap[1].facility[res_count].value_str = constant_yes
   ELSE
    reply->solcap[1].facility[res_count].value_str = constant_no
   ENDIF
  FOOT REPORT
   stat = alterlist(reply->solcap[1].facility,res_count), stat = alterlist(facilitylist->locs,
    fac_list_size)
  WITH nocounter
 ;end select
END GO
