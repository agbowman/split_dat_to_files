CREATE PROGRAM bhs_rpt_aco_measure:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (e
   ...
   WHERE p.person_id=e.person_id
    AND cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1) < cnvtlookbehind("65,Y"))
   JOIN (ea1
  ...
  DETAIL
   ...
   format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1),"MM/DD/YYYY;;q"), pinfo->qual[
   pinfo->l_cnt].s_person_fname = trim(p.name_first_key,3),
   ...
   trim(uar_get_code_display(e.encntr_type_cd),3), pinfo->qual[pinfo->l_cnt].s_age = substring(1,(
    findstring(" ",trim(cnvtage(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1)),3))+ 1),
    trim(cnvtage(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1)),3)),
 ...
 IF ((pinfo->l_cnt > 0))
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    IF (ml_idx2 > 0)
     pinfo->qual[ml_idx2].l_mcnt += 1, stat = alterlist(pinfo->qual[ml_idx2].meds,pinfo->qual[ml_idx2
      ].l_mcnt), pinfo->qual[ml_idx2].meds[pinfo->qual[ml_idx2].l_mcnt].f_catalog_cd = o.catalog_cd,
