CREATE PROGRAM dm_him_coding_specialty_rows:dba
 ...
 SET reply->table_name = "CODING_SPECIALTY"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(cs.coding_id)
   FROM coding_specialty cs
   WHERE cs.coding_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    cs.rowid
    FROM coding_specialty cs
    WHERE parser(sbr_getrowidnotexists("cs.coding_id between curMinID and curMaxID","cs"))
     AND cs.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = cs.rowid
    WITH nocounter, maxqual(cs,value(rowsleft))
   ;end select
   SET reply->err_code = error(reply->err_msg,1)
   IF ((reply->err_code > 0))
    SET reply->status_data.status = "F"
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERROR",
     "Failed in row collection: %1","cs",nullterm(reply->err_msg))
    GO TO exit_script
   ENDIF
