CREATE PROGRAM dm2_mig_setup_users:dba
 ...
 FREE RECORD dmsu_orauser
 RECORD dmsu_orauser(
   1 cnt = i4
   1 qual[*]
     2 user = vc
 )
 FREE RECORD dmsu_temp_orauser
 RECORD dmsu_temp_orauser(
   1 cnt = i4
   1 qual[*]
     2 user = vc
 )
 FREE RECORD dmsu_users_list
 RECORD dmsu_users_list(
   1 cnt = i4
   1 qual[*]
     2 user_name = vc
 )
 DECLARE dmsu_user_name = vc WITH protect, noconstant("")
 DECLARE dmsu_iter = i4 WITH protect, noconstant(0)
 DECLARE dmsu_index = i4 WITH protect, noconstant(0)
 DECLARE dmsu_remove_location = i4 WITH protect, noconstant(0)
 DECLARE dmsu_valid_data_types = vc WITH protect, noconstant("")
 DECLARE dmsu_setup_mig_users_cont = i2 WITH protect, noconstant(1)
 DECLARE dmsu_continue_anyway = i2 WITH protect, noconstant(0)
 DECLARE dmsu_table_count = i4 WITH protect, noconstant(0)
 DECLARE dmsu_purge_string = vc WITH protect, noconstant(" ")
 DECLARE dmsu_managed_tables = vc WITH protect, noconstant("")
 DECLARE dmsu_user_loc = i4 WITH protect, noconstant(0)
 DECLARE dmsu_csv_name = vc WITH public, constant( $1)
 DECLARE dmsu_file_loc = vc WITH protect, noconstant(" ")
 DECLARE dmsu_idx = i4 WITH protect, noconstant(0)
 DECLARE dmsu_invalid_users = vc WITH protect, noconstant(" ")
 DECLARE dmsu_invalid_usr_ind = i2 WITH protect, noconstant(0)
 DECLARE dmsu_backup_users_file = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm2_mig_setup_users",".log","dm2_mig_setup_users LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_setup_users"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmsu_file_loc = concat(trim(logical("cer_install")),"/")
 SET dmsu_backup_users_file = dmsu_file_loc
 SET dmsu_file_loc = build(dmsu_file_loc,dmsu_csv_name)
 SET dm_err->eproc = "Check if .csv file exists or not"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (dm2_findfile(build(dmsu_file_loc))=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat(" The ",trim(dmsu_csv_name),
   " file does not exist in the CER_INSTALL directory.")
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Create backup audit file for the list of uptime users selected"
 IF ((dm2_sys_misc->cur_os != "AXP"))
  IF (get_unique_file("dm2dmsu_list",".dat")=0)
   GO TO exit_script
  ELSE
   SET dmsu_backup_users_file = build(dmsu_backup_users_file,dm_err->unique_fname)
  ENDIF
  IF (dm2_push_dcl(concat("cp ",dmsu_file_loc," ",dmsu_backup_users_file))=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Unable to copy users to backup file"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Determining if schema setup is already complete from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
 ...
 IF (curqual)
  ...
  CALL clear(1,1)
  CALL box(1,1,24,131)
  CALL text(2,2,"Setup Migration Users")
  CALL text(21,2,concat(
    "Adding or removing users while a migration is in progress is not recommended.  ",
    "Please contact Cerner support before proceeding."))
  CALL text(22,2,"(C)ontinue anyways, (Q)uit: ")
  CALL accept(22,30,"P;CU","Q"
   WHERE curaccept IN ("C", "Q"))
  CALL clear(1,1)
  SET message = nowindow
  SET dmsu_continue_anyway = 1
  IF (curaccept="Q")
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Load list of Oracle specific users."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_ORACLE_USER"
  DETAIL
   dmsu_orauser->cnt = (dmsu_orauser->cnt+ 1), stat = alterlist(dmsu_orauser->qual,dmsu_orauser->cnt),
   dmsu_orauser->qual[dmsu_orauser->cnt].user = d.info_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF ((dmr_mig_data->src_ora_level1 > 11))
  SET dm_err->eproc = "Check in dba_users with oracle_maintained flag."
  SELECT INTO "nl:"
   FROM dba_users du
   WHERE du.oracle_maintained="Y"
   DETAIL
    IF (locateval(dmsu_index,1,dmsu_orauser->cnt,du.username,dmsu_orauser->qual[dmsu_index].user)=0)
     dmsu_temp_orauser->cnt = (dmsu_temp_orauser->cnt+ 1), stat = alterlist(dmsu_temp_orauser->qual,
      dmsu_temp_orauser->cnt), dmsu_temp_orauser->qual[dmsu_temp_orauser->cnt].user = du.username
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  IF ((dmsu_temp_orauser->cnt > 0))
   SET dm_err->eproc = "Insert into dm_info from dba_users with oracle_maintained flag."
   INSERT  FROM dm_info di,
     (dummyt d  WITH seq = value(dmsu_temp_orauser->cnt))
    SET di.info_domain = "DM2_ORACLE_USER", di.info_name = dmsu_temp_orauser->qual[d.seq].user
    PLAN (d)
     JOIN (di)
    WITH nocounter
   ;end insert
  ...
  ENDIF
 ENDIF
 IF ((dmsu_orauser->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No list of Oracle users found."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Deleting existing user rows from dm_info."
 DELETE  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_USER"
  WITH nocounter
 ;end delete
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Retrieving all dba_users"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dba_users du
  DETAIL
   dmsu_users_list->cnt = (dmsu_users_list->cnt+ 1), stat = alterlist(dmsu_users_list->qual,
    dmsu_users_list->cnt), dmsu_users_list->qual[dmsu_users_list->cnt].user_name = du.username
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 FREE DEFINE rtl
 FREE SET file_loc
 SET logical file_loc value(build("cer_install:",dmsu_csv_name))
 DEFINE rtl "file_loc"
 SET dm_err->eproc = concat("Adding users from ",trim(dmsu_csv_name)," file")
 SELECT INTO "nl:"
  r.line
  FROM rtlt r
  HEAD REPORT
  ...
  DETAIL
   IF (trim(r.line) != "")
    dmsu_user_name = trim(cnvtupper(r.line),3)
    IF (locateval(dmsu_index,1,dmsu_users_list->cnt,dmsu_user_name,dmsu_users_list->qual[dmsu_index].
     user_name) > 0
     AND locateval(dmsu_index,1,dmsu_orauser->cnt,dmsu_user_name,dmsu_orauser->qual[dmsu_index].user)
    =0
     AND  NOT (dmsu_user_name IN ("GG", "CERN_DBSTATS", "CMS_DBA", "PATROL", "PATROL1",
    "PATROL2", "GGDDL", "GGVD", "GGREPAIR"))
     AND dmsu_user_name != "C##*")
     dmsu_idx = 0, dmsu_idx = locateval(dmsu_index,1,dmr_user_list->cnt,dmsu_user_name,dmr_user_list
      ->qual[dmsu_index].user)
     IF (dmsu_idx=0)
      dmr_user_list->cnt = (dmr_user_list->cnt+ 1), stat = alterlist(dmr_user_list->qual,
       dmr_user_list->cnt), dmr_user_list->qual[dmr_user_list->cnt].user = dmsu_user_name,
      dmr_user_list->change_ind = 1
     ENDIF
    ELSE
     dm_err->err_ind = 1, dmsu_invalid_usr_ind = 1
     IF (textlen(dmsu_invalid_users)=0)
      dmsu_invalid_users = dmsu_user_name
     ELSE
      dmsu_invalid_users = concat(dmsu_invalid_users,",",dmsu_user_name)
     ENDIF
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 IF ((dmr_user_list->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No users added to migration process")
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  IF (dmsu_invalid_usr_ind=1)
   SET dm_err->emsg = concat("Invalid user ",trim(dmsu_invalid_users)," in  ",trim(dmsu_csv_name),
    " file")
  ENDIF
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(dmr_user_list)
 ENDIF
 SET dm_err->eproc = "Existence of V500 user"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (locateval(dmsu_index,1,dmr_user_list->cnt,"V500",dmr_user_list->qual[dmsu_index].user)=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Failed to find V500 user in ",trim(dmsu_csv_name)," file")
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Inserting new user rows into dm_info."
 INSERT  FROM dm_info di,
   (dummyt d  WITH seq = value(dmr_user_list->cnt))
  SET di.info_domain = "DM2_MIG_USER", di.info_name = dmr_user_list->qual[d.seq].user, di.info_char
    = evaluate(dmsu_continue_anyway,1,"USER BYPASSED WARNING","")
  PLAN (d)
   JOIN (di)
  WITH nocounter
 ;end insert
 ...
 COMMIT
 SET dm_err->eproc =
 "Deleting existing migration exclusion rows except for manual exclusions from dm_info."
 DELETE  FROM dm_info di
  WHERE di.info_domain IN ("DM2_MIG_EXCL", "DM2_MIG_DT_USER")
   AND di.info_number >= 16
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ENDIF
 SET rpt_rec->cnt = 0
 SET stat = alterlist(rpt_rec->qual,0)
 SET dm_err->eproc = "Loading materialized views."
 SELECT INTO "nl:"
  FROM dba_tables a
  WHERE  NOT (a.owner IN (
  (SELECT
   info_name
   FROM dm_info
  ...
  (SELECT
   ...
   WHERE a.owner=b.owner
    AND a.table_name=b.mview_name))) OR (a.table_name="MV*"
   AND a.owner="V500"))
  DETAIL
   rpt_rec->cnt = (rpt_rec->cnt+ 1), stat = alterlist(rpt_rec->qual,rpt_rec->cnt), rpt_rec->qual[
   rpt_rec->cnt].owner = a.owner,
   rpt_rec->qual[rpt_rec->cnt].table_name = a.table_name, rpt_rec->qual[rpt_rec->cnt].reason_cd = 17,
   rpt_rec->qual[rpt_rec->cnt].exclude_flag = "0"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Loading materialized view logs."
 SELECT INTO "nl:"
  FROM dba_tables a
  WHERE  NOT (a.owner IN (
  (SELECT
  ...
  (SELECT
   "x"
   FROM dba_mview_logs b
   WHERE a.owner=b.log_owner
    AND a.table_name=b.log_table))
  DETAIL
   IF (locateval(dmsu_index,1,rpt_rec->cnt,a.owner,rpt_rec->qual[dmsu_index].owner,
    a.table_name,rpt_rec->qual[dmsu_index].table_name)=0)
    rpt_rec->cnt = (rpt_rec->cnt+ 1), stat = alterlist(rpt_rec->qual,rpt_rec->cnt), rpt_rec->qual[
    rpt_rec->cnt].owner = a.owner,
    rpt_rec->qual[rpt_rec->cnt].table_name = a.table_name, rpt_rec->qual[rpt_rec->cnt].reason_cd = 17,
    rpt_rec->qual[rpt_rec->cnt].exclude_flag = "0"
 ...
 ENDIF
 IF ((rpt_rec->cnt > 0))
  SET dm_err->eproc = "Check for exclusions that are non-doc."
  SELECT INTO "nl:"
   FROM dm_tables_doc dt,
    (dummyt d  WITH seq = value(rpt_rec->cnt))
   PLAN (d
    WHERE (rpt_rec->qual[d.seq].reason_cd=17))
    JOIN (dt
    WHERE (dt.table_name=rpt_rec->qual[d.seq].table_name))
   DETAIL
    rpt_rec->qual[d.seq].reason_cd = 16
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Automate exclusion of external tables"
 SELECT INTO "nl:"
  ...
  DETAIL
   rpt_rec->cnt = (rpt_rec->cnt+ 1), stat = alterlist(rpt_rec->qual,rpt_rec->cnt), rpt_rec->qual[
   rpt_rec->cnt].owner = a.owner,
   rpt_rec->qual[rpt_rec->cnt].table_name = a.table_name, rpt_rec->qual[rpt_rec->cnt].reason_cd = 21,
   rpt_rec->qual[rpt_rec->cnt].exclude_flag = "0"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dmsu_managed_tables = ""
 IF (dmr_load_managed_tables(dmsu_managed_tables)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Loading list of valid data types from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIGRATION"
   AND di.info_name="VALID_DATA_TYPES"
  HEAD REPORT
   dmsu_valid_data_types = concat(
    "'CHAR','VARCHAR2','VARCHAR','NUMBER','FLOAT','LONG','LONG RAW','DATE','BLOB','CLOB','ROWID',",
    "'TIMESTAMP(1)','TIMESTAMP(2)','TIMESTAMP(3)','TIMESTAMP(4)','TIMESTAMP(5)',",
    "'TIMESTAMP(6)','TIMESTAMP(7)','TIMESTAMP(8)','TIMESTAMP(9)'")
  DETAIL
   dmsu_valid_data_types = di.info_char
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Loading tables with invalid data types."
 SELECT INTO "nl:"
  FROM dba_tab_columns dtc,
   dba_tables dt
  WHERE parser(concat("dtc.data_type not in (",dmsu_valid_data_types,")"))
   AND parser(concat(" dtc.table_name not in (",dmsu_managed_tables,")"))
   AND dtc.owner IN (
  (SELECT
   info_name
   FROM dm_info
  ...
  HEAD dtc.owner
   row + 0
  HEAD dtc.table_name
   IF (locateval(dmsu_index,1,rpt_rec->cnt,dtc.owner,rpt_rec->qual[dmsu_index].owner,
    dtc.table_name,rpt_rec->qual[dmsu_index].table_name)=0)
    rpt_rec->cnt = (rpt_rec->cnt+ 1), stat = alterlist(rpt_rec->qual,rpt_rec->cnt), rpt_rec->qual[
    rpt_rec->cnt].owner = dtc.owner,
    rpt_rec->qual[rpt_rec->cnt].table_name = dtc.table_name, rpt_rec->qual[rpt_rec->cnt].reason_cd =
    20, rpt_rec->qual[rpt_rec->cnt].exclude_flag = "1"
   ENDIF
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Mark tables with spaces in the name as NOMOVE"
 SELECT INTO "nl:"
  ...
  (SELECT
   ...
   FROM dm_info
   WHERE info_domain="DM2_MIG_USER"))
   AND ((dtc.table_name="* *") OR (dtc.column_name="* *"))
   AND  EXISTS (
  (SELECT
   "x"
   FROM dba_tables z
   WHERE dtc.owner=z.owner
    AND dtc.table_name=z.table_name))
  ORDER BY dtc.owner, dtc.table_name
  HEAD dtc.owner
   row + 0
  HEAD dtc.table_name
   IF (locateval(dmsu_index,1,rpt_rec->cnt,dtc.owner,rpt_rec->qual[dmsu_index].owner,
    ...
    rpt_rec->cnt].owner = dtc.owner,
    rpt_rec->qual[rpt_rec->cnt].table_name = dtc.table_name, rpt_rec->qual[rpt_rec->cnt].reason_cd =
    22, rpt_rec->qual[rpt_rec->cnt].exclude_flag = "0"
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Loading tables with duplicate column names by case."
 SELECT INTO "nl:"
  dtc.owner, dtc.table_name, cnvtlower(dtc.column_name)
  FROM dba_tab_columns dtc
  WHERE dtc.owner IN (
  (SELECT
   info_name
   FROM dm_info
   WHERE info_domain="DM2_MIG_USER"))
  GROUP BY dtc.owner, dtc.table_name, cnvtlower(dtc.column_name)
  HAVING count(*) > 1
  ORDER BY dtc.owner, dtc.table_name
  HEAD dtc.owner
   row + 0
  HEAD dtc.table_name
   IF (locateval(dmsu_index,1,rpt_rec->cnt,dtc.owner,rpt_rec->qual[dmsu_index].owner,
    ...
    rpt_rec->cnt].owner = dtc.owner,
    rpt_rec->qual[rpt_rec->cnt].table_name = dtc.table_name, rpt_rec->qual[rpt_rec->cnt].reason_cd =
    18, rpt_rec->qual[rpt_rec->cnt].exclude_flag = "1"
   ENDIF
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Loading tables with column names that are oracle data types."
 SELECT INTO "nl:"
  FROM dba_tab_columns dtc,
   dba_tables d
  WHERE dtc.owner IN (
  (SELECT
   ...
   FROM dm_info
   WHERE info_domain="DM2_MIG_USER"))
   AND dtc.column_name IN ("BLOB", "FLOAT", "DATE", "CLOB", "CHAR",
  "LONG", "NUMBER", "FLOAT", "DATE", "CHAR",
  "NUMBER", "BFILE", "NCHAR", "UROWID", "DECIMAL",
  "INTEGER", "SMALLINT", "RAW")
  ...
  HEAD dtc.table_name
   IF (locateval(dmsu_index,1,rpt_rec->cnt,dtc.owner,rpt_rec->qual[dmsu_index].owner,
    ...
    rpt_rec->cnt].owner = dtc.owner,
    rpt_rec->qual[rpt_rec->cnt].table_name = dtc.table_name, rpt_rec->qual[rpt_rec->cnt].reason_cd =
    19, rpt_rec->qual[rpt_rec->cnt].exclude_flag = "1"
   ENDIF
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Loading tables owned by excluded users."
 SELECT INTO "nl:"
  FROM dba_users dt
  WHERE  NOT (dt.username IN (
  (SELECT
   info_name
   FROM dm_info
   WHERE info_domain="DM2_ORACLE_USER")))
   AND  NOT (dt.username IN (
  (SELECT
   info_name
   FROM dm_info
   WHERE info_domain="DM2_MIG_USER")))
   AND  NOT (dt.username IN ("CERN_DBSTATS", "CMS_DBA", "PATROL", "PATROL1", "PATROL2",
  "GGDDL", "GGVD", "GGREPAIR"))
   AND dt.username != "C##*"
  ORDER BY dt.username
  DETAIL
   rpt_rec->cnt = (rpt_rec->cnt+ 1), stat = alterlist(rpt_rec->qual,rpt_rec->cnt), rpt_rec->qual[
   rpt_rec->cnt].owner = dt.username,
   rpt_rec->qual[rpt_rec->cnt].reason_cd = 40, rpt_rec->qual[rpt_rec->cnt].exclude_flag = "1",
   rpt_rec->qual[rpt_rec->cnt].exclude_owner = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Mark designated users as NOMOVE users."
 SELECT INTO "nl:"
  FROM dba_users dt
  WHERE dt.username IN ("CMS_DBA", "PATROL", "PATROL1", "PATROL2", "GGDDL",
  "GGVD", "GGREPAIR")
  ORDER BY dt.username
  HEAD dt.username
   dmsu_user_loc = locateval(dmsu_index,1,rpt_rec->cnt,dt.username,rpt_rec->qual[dmsu_index].owner)
   IF (dmsu_user_loc=0)
    rpt_rec->cnt = (rpt_rec->cnt+ 1), stat = alterlist(rpt_rec->qual,rpt_rec->cnt), rpt_rec->qual[
    rpt_rec->cnt].owner = dt.username,
    rpt_rec->qual[rpt_rec->cnt].reason_cd = 40, rpt_rec->qual[rpt_rec->cnt].exclude_flag = "0",
    rpt_rec->qual[rpt_rec->cnt].exclude_owner = 1
   ELSE
    rpt_rec->qual[dmsu_user_loc].owner = dt.username, rpt_rec->qual[dmsu_user_loc].reason_cd = 40,
    rpt_rec->qual[dmsu_user_loc].exclude_flag = "0",
    rpt_rec->qual[dmsu_user_loc].exclude_owner = 1
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Exclude tables owned by GG user."
 SELECT INTO "nl:"
  FROM dba_tables dt
  WHERE dt.owner="GG"
  ...
  HEAD dt.owner
   row + 0
  HEAD dt.table_name
   dmsu_user_loc = locateval(dmsu_index,1,rpt_rec->cnt,dt.owner,rpt_rec->qual[dmsu_index].owner,
    dt.table_name,rpt_rec->qual[dmsu_index].table_name)
   IF (dmsu_user_loc=0)
    rpt_rec->cnt = (rpt_rec->cnt+ 1), stat = alterlist(rpt_rec->qual,rpt_rec->cnt), rpt_rec->qual[
    rpt_rec->cnt].owner = dt.owner,
    rpt_rec->qual[rpt_rec->cnt].table_name = dt.table_name, rpt_rec->qual[rpt_rec->cnt].reason_cd =
    23, rpt_rec->qual[rpt_rec->cnt].exclude_flag = "0"
   ELSE
    rpt_rec->qual[dmsu_user_loc].owner = dt.owner, rpt_rec->qual[dmsu_user_loc].table_name = dt
    .table_name, rpt_rec->qual[dmsu_user_loc].reason_cd = 23,
    rpt_rec->qual[dmsu_user_loc].exclude_flag = "0"
   ENDIF
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Mark DM2_DDL_OPS1 and DM2_DDL_OPS_LOG1 as nomove tables "
 SELECT INTO "nl:"
  FROM dba_tables dt
  WHERE dt.table_name IN ("DM2_DDL_OPS1", "DM2_DDL_OPS_LOG1")
   AND dt.owner="V500"
  ORDER BY dt.owner, dt.table_name
  HEAD dt.owner
   row + 0
  HEAD dt.table_name
   ...
   IF (dmsu_user_loc=0)
    ...
    rpt_rec->cnt].owner = dt.owner,
    rpt_rec->qual[rpt_rec->cnt].table_name = dt.table_name, rpt_rec->qual[rpt_rec->cnt].reason_cd =
    24, rpt_rec->qual[rpt_rec->cnt].exclude_flag = "0"
   ELSE
    rpt_rec->qual[dmsu_user_loc].owner = dt.owner, rpt_rec->qual[dmsu_user_loc].table_name = dt
    .table_name, rpt_rec->qual[dmsu_user_loc].reason_cd = 24,
    rpt_rec->qual[dmsu_user_loc].exclude_flag = "0"
   ENDIF
  WITH nocounter, nullreport
 ;end select
 ...
 ENDIF
 IF ((rpt_rec->cnt > 0))
  SET dm_err->eproc = "Inserting migration exclusion rows into dm_info."
  INSERT  FROM dm_info di,
    (dummyt d  WITH seq = value(rpt_rec->cnt))
   SET di.info_domain = evaluate(rpt_rec->qual[d.seq].exclude_owner,0,"DM2_MIG_EXCL",
     "DM2_MIG_DT_USER"), di.info_name = evaluate(rpt_rec->qual[d.seq].exclude_owner,0,concat(rpt_rec
      ->qual[d.seq].owner,".",rpt_rec->qual[d.seq].table_name),rpt_rec->qual[d.seq].owner), di
    .info_number = rpt_rec->qual[d.seq].reason_cd,
    di.info_char = rpt_rec->qual[d.seq].exclude_flag
   PLAN (d)
    JOIN (di)
   WITH nocounter
  ;end insert
 ...
 COMMIT
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(rpt_rec)
 ENDIF
 IF (dmr_user_list->change_ind)
  EXECUTE dm2_mig_report
  IF (dm_err->err_ind)
   GO TO exit_script
  ENDIF
 ENDIF
...
#exit_script
 ...
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "Dm2_mig_setup_users completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_setup_users")
END GO
