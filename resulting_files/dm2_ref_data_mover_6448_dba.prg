CREATE PROGRAM dm2_ref_data_mover_6448:dba
 ...
 DECLARE cust_rpt_miss = vc
 DECLARE cust_conv_id = vc
 DECLARE cust_trig_val = vc
 DECLARE cust_track_group = vc
 DECLARE cust_act_val = vc
 DECLARE cust_to_conv_id = vc
 DECLARE cust_to_trig_val = vc
 DECLARE cust_to_track_group = vc
 DECLARE cust_to_act_val = vc
 DECLARE cust_conv_id_pos = i4
 DECLARE cust_trig_val_pos = i4
 DECLARE cust_track_group_pos = i4
 DECLARE cust_act_val_pos = i4
 DECLARE cust_act_type = vc
 DECLARE cust_rpt_miss = vc
 DECLARE cust_err_val = i4
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_conv_id_pos = get_col_pos(cust_tab_name,"CONVERSATION_ID")
 SET cust_trig_val_pos = get_col_pos(cust_tab_name,"TRIGGER_VALUE")
 SET cust_track_group_pos = get_col_pos(cust_tab_name,"TRACK_GROUP")
 SET cust_act_val_pos = get_col_pos(cust_tab_name,"ACTION_VALUE")
 SET cust_conv_id = get_value(cust_tab_name,"CONVERSATION_ID","FROM")
 SET cust_trig_val = get_value(cust_tab_name,"TRIGGER_VALUE","FROM")
 SET cust_track_group = get_value(cust_tab_name,"TRACK_GROUP","FROM")
 SET cust_act_val = get_value(cust_tab_name,"ACTION_VALUE","FROM")
 SET cust_act_type = get_value(cust_tab_name,"ACTION_TYPE","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_6448
 ENDIF
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_conv_id_pos].translated = 0
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_trig_val_pos].translated = 0
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_track_group_pos].translated = 0
 IF (cnvtreal(cust_conv_id) >= 4250510
  AND ((cnvtreal(cust_conv_id) <= 4250517) OR (cnvtreal(cust_conv_id) >= 4250610
  AND cnvtreal(cust_conv_id) <= 4250617)) )
  CALL put_value(cust_tab_name,"CONVERSATION_ID",cust_conv_id)
 ELSE
  IF (cnvtreal(cust_conv_id) > 0)
   SET cust_to_conv_id = select_merge_translate(cust_conv_id,"CODE_VALUE")
   SET stat = alterlist(rdds_exception->qual,1)
   SET rdds_exception->qual[1].tab_col_name = "CONVERSATION_ID"
   SET rdds_exception->qual[1].tru_tab_name = "CODE_VALUE"
   SET rdds_exception->qual[1].tru_col_name = "CODE_VALUE"
   IF (cust_to_conv_id != "No Trans")
    CALL put_value(cust_tab_name,"CONVERSATION_ID",cust_to_conv_id)
   ENDIF
  ELSE
   CALL put_value(cust_tab_name,"CONVERSATION_ID","0")
  ENDIF
 ENDIF
 IF (cnvtint(cust_trig_val) > 0)
  SET cust_to_trig_val = select_merge_translate(cust_trig_val,"CODE_VALUE")
  IF (cust_to_trig_val != "No Trans")
   CALL put_value(cust_tab_name,"TRIGGER_VALUE",cust_to_trig_val)
  ENDIF
 ELSE
  CALL put_value(cust_tab_name,"TRIGGER_VALUE","0")
 ENDIF
 IF (cnvtint(cust_track_group) > 0)
  SET cust_to_track_group = select_merge_translate(cust_track_group,"CODE_VALUE")
  IF (cust_to_track_group != "No Trans")
   CALL put_value(cust_tab_name,"TRACK_GROUP",cust_to_track_group)
  ENDIF
 ELSE
  CALL put_value(cust_tab_name,"TRACK_GROUP","0")
 ENDIF
 IF (cnvtint(cust_act_val) > 0)
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_act_val_pos].translated = 0
  IF (cust_act_type="SETEVENTS")
   SET cust_to_act_val = select_merge_translate(cust_act_val,"TRACK_EVENT")
   IF (cust_to_act_val != "No Trans")
    CALL put_value(cust_tab_name,"ACTION_VALUE",cust_to_act_val)
   ENDIF
  ELSEIF (cust_act_type="CHKOUT")
   SET cust_to_act_val = select_merge_translate(cust_act_val,"CODE_VALUE")
   IF (cust_to_act_val != "No Trans")
    CALL put_value(cust_tab_name,"ACTION_VALUE",cust_to_act_val)
   ENDIF
  ENDIF
 ELSE
  CALL put_value(cust_tab_name,"ACTION_VALUE","0")
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"TRACK_TRIGGER_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1)
   ...
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"TRACK_TRIGGER_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"TRACK_TRIGGER_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  TRACK_TRIGGER_ID")
     ELSE
      GO TO exit_6448
     ENDIF
    ELSE
   ...
   SET from_id = get_value(cust_tab_name,"TRACK_TRIGGER_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6448
   ELSE
 ...
 CALL echo("")
 CALL echo("")
 SET idcd_check = is_translated(cust_tab_name,"TRIGGER_VALUE")
 IF (idcd_check=1)
  SET idcd_check = 0
  SET idcd_check = is_translated(cust_tab_name,"TRACK_GROUP")
  IF (idcd_check=1)
   SET idcd_check = 0
   SET idcd_check = is_translated(cust_tab_name,"ACTION_VALUE")
   IF (idcd_check=1)
    SET idcd_check = 0
    SET idcd_ind = 1
   ...
   ELSE
    IF (cust_act_type="SETEVENTS")
     SET cust_rpt_miss = report_missing("TRACK_EVENT","TRACK_EVENT_ID",cnvtreal(cust_act_val))
    ELSEIF (cust_act_type="CHKOUT")
     SET cust_rpt_miss = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_act_val))
    ENDIF
    CALL evaluate_rpt_missing(cust_rpt_miss)
    SET cust_err_val = get_err_val(null)
    IF (cust_err_val=0)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = concat(
      "This log_id wasn't translated because there was no translation for ",
      "the ACTION_VALUE column.")
    ENDIF
   ENDIF
  ELSE
   SET cust_rpt_miss = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_track_group))
   CALL evaluate_rpt_missing(cust_rpt_miss)
   SET cust_err_val = get_err_val(null)
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = concat(
     "This log_id wasn't translated because there was no translation for ","the TRACK_GROUP column.")
   ENDIF
  ENDIF
 ELSE
  SET cust_rpt_miss = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_trig_val))
  CALL evaluate_rpt_missing(cust_rpt_miss)
  SET cust_err_val = get_err_val(null)
  IF (cust_err_val=0)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = concat(
    "This log_id wasn't translated because there was no translation for ","the TRIGGER_VALUE column."
    )
  ENDIF
 ENDIF
#exit_6448
 ...
 FREE SET cust_prem_flag
 FREE SET cust_value1
 FREE SET cust_to_value1
 FREE SET cust_value1_pos
END GO
