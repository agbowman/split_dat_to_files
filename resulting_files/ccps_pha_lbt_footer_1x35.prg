CREATE PROGRAM ccps_pha_lbt_footer_1x35
 PROMPT
 ...
 SUBROUTINE check_secabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.273),(offsety+ 0.109),(offsetx+ 1.086),(offsety+
     0.109))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 1.512),(offsety+ 0.109),(offsetx+ 2.220),(offsety+
     0.109))
    IF ((params->label_per="DOSE")
     AND  NOT (com_msg=concat("execute ",trim(params->order_header_object)," '",print_file,"',1 go"))
    )
     SET _rptstat = uar_rptline(_hreport,(offsetx+ 2.731),(offsety+ 0.109),(offsetx+ 3.419),(offsety
      + 0.109))
    ENDIF
 ...
 SUBROUTINE delivery_secabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptbarcodeex(_hreport,rptbce,build2(build("T",trim(cnvtstring(label_data->
         dispense.id))),char(0)))
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_aztec,(offsetx+ 0.835),(offsety+ 0.021))
    SET rptbce->m_recsize = 88
    SET rptbce->m_width = 0.32
    SET rptbce->m_height = 0.30
    SET rptbce->m_rotation = 0
    ...
    SET _rptstat = uar_rptbarcodeex(_hreport,rptbce,build2(build("T",trim(cnvtstring(label_data->
         dispense.id))),char(0)))
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_aztec,(offsetx+ 2.262),(offsety+ 0.021))
    SET rptbce->m_recsize = 88
    SET rptbce->m_width = 0.32
    SET rptbce->m_height = 0.30
    SET rptbce->m_rotation = 0
    ...
    SET _rptstat = uar_rptbarcodeex(_hreport,rptbce,build2(build("T",trim(cnvtstring(label_data->
         dispense.id))),char(0)))
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_aztec,(offsetx+ 3.095),(offsety+ 0.021))
    SET rptbce->m_recsize = 88
    SET rptbce->m_width = 0.32
    SET rptbce->m_height = 0.30
    SET rptbce->m_rotation = 0
    ...
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_aztec,(offsetx+ 0.418),(offsety+ 0.021))
    SET rptbce->m_recsize = 88
    SET rptbce->m_width = 0.32
    SET rptbce->m_height = 0.30
    SET rptbce->m_rotation = 0
    ...
    SET _rptstat = uar_rptbarcodeex(_hreport,rptbce,build2(build("T",trim(cnvtstring(label_data->
         dispense.id))),char(0)))
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_aztec,(offsetx+ 2.679),(offsety+ 0.021))
    SET rptbce->m_recsize = 88
    SET rptbce->m_width = 0.32
    SET rptbce->m_height = 0.30
    SET rptbce->m_rotation = 0
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CCPS_PHA_LBT_FOOTER_1X35"
