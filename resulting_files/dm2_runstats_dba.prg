CREATE PROGRAM dm2_runstats:dba
 ...
 DECLARE dm2_runstats_ind = i2 WITH protect, constant(1)
 ...
 DECLARE dr_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dr_iterator = i4 WITH protect, noconstant(0)
 DECLARE dr_queue_table_exists_ind = i2 WITH protect, noconstant(validate(drt_queue_table_exists_ind,
   0))
 DECLARE dr_reset_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dr_use_auto_sample_size_ind = i2 WITH protect, noconstant(0)
 DECLARE dr_info_exists = i2 WITH protect, noconstant(0)
 DECLARE dr_realstats_ind = i2 WITH protect, noconstant(0)
 DECLARE dr_list_stale_report(null) = i2
 IF (check_logfile("dm2_runstats",".log","dm2_runstats LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dr_err_ind = dm_err->err_ind
 SET dr_err_emsg = dm_err->emsg
 SET dr_err_eproc = dm_err->eproc
 SET dm_err->eproc = "Beginning dm2_runstats"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg =
  "Parameter usage: dm2_runstats  '<owner>','<table_name>','<gather option>',<stale pct value>"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 SET dr_log_id = dm2_process_event_rs->dm_process_event_id
 ...
 IF ((dm_err->err_ind=1))
  ...
  ENDIF
 ENDIF
 IF (dm2_table_and_ccldef_exists("DM_INFO",dr_info_exists)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_script
 ...
 IF ((dm2_rdbms_version->level1 > 10))
  SET dm_err->eproc = "Oracle Version > 10. Auto-successing dm2_runstats."
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dr_realstats_ind = 1
  GO TO exit_script
 ENDIF
 IF (dr_info_exists)
  SET dm_err->eproc = "Checking for override for USE_AUTO_SAMPLE_SIZE from DM_INFO"
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="DM2_RUNSTATS"
    AND di.info_name="USE_AUTO_SAMPLE_SIZE"
   DETAIL
    dr_use_auto_sample_size_ind = di.info_number
   WITH nocounter
  ;end select
  ...
  ENDIF
 ENDIF
 IF (dr_queue_table_exists_ind != 1)
  SET dm_err->eproc = "Check for existance of DM_RPOCESS_QUEUE in dba_tables"
  SELECT INTO "nl:"
   FROM dba_tables dt
   WHERE dt.table_name="DM_PROCESS_QUEUE"
    AND dt.owner=currdbuser
   DETAIL
    dr_queue_table_exists_ind = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc))
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (currdb="ORACLE")
  ...
  ENDIF
  IF ((dm2_rdbms_version->level1 < 11))
   IF ((table_stats->table_name=char(42))
    AND (table_stats->owner != "CDBA"))
    SET dm_err->eproc = "ORACLE database detected, gathering database system statistics..."
    CALL disp_msg(" ",dm_err->logfile,0)
    EXECUTE dm2_gather_system_stats
    IF ((dm_err->err_ind=1))
     SET dm_err->err_ind = 0
    ENDIF
   ENDIF
  ...
  IF ((dm2_db_options->dbstats_gather_method="DBMS_STATS"))
   ...
   ELSE
    IF (dri_get_ind_est_perc(null)=0)
     GO TO exit_script
    ENDIF
    SET table_stats->cascade = evaluate(index_list->est_pct,0.0,"TRUE","FALSE")
    ...
    FOR (item_count = 1 TO table_stats->tbl_cnt)
      IF ((table_stats->stats_cnt[item_count].gather_ind=1))
       SET dm_err->eproc = build("Calling dm2_flexstats 'TABLE'",",'",table_stats->stats_cnt[
        item_count].owner,"'",",'",
        table_stats->stats_cnt[item_count].tabname,"'",",",evaluate(dr_use_auto_sample_size_ind,1,- (
         1.0),table_stats->stats_cnt[item_count].est_pct),",'",
        table_stats->stats_cnt[item_count].method_opt,"'",",'",table_stats->stats_cnt[item_count].
        block_sample,"','",
        table_stats->cascade,"'")
       CALL disp_msg(" ",dm_err->logfile,0)
       IF ((((program_stack_rs->qual[1].name="DM2_ROUTINE_TASKS")) OR (validate(dm2_queue_rows_only,0
        )=1))
        AND dr_queue_table_exists_ind)
        SET dpq_process_queue->process_type = dpq_statistics
        SET dpq_process_queue->op_type = dpq_gather
        SET dpq_process_queue->owner_name = table_stats->stats_cnt[item_count].owner
        SET dpq_process_queue->object_type = dpq_table
        SET dpq_process_queue->op_method = dpq_db
        SET dpq_process_queue->object_name = table_stats->stats_cnt[item_count].tabname
        SET dpq_process_queue->priority = 150
        SET dpq_process_queue->routine_tasks_ind = 1
        SET dpq_process_queue->operation_txt = build("execute dm2_flexstats 'TABLE','",table_stats->
         stats_cnt[item_count].owner,"','",table_stats->stats_cnt[item_count].tabname,"',",
         evaluate(dr_use_auto_sample_size_ind,1,- (1.0),table_stats->stats_cnt[item_count].est_pct),
         ",'",table_stats->stats_cnt[item_count].method_opt,"','",table_stats->stats_cnt[item_count].
         block_sample,
         "','",table_stats->cascade,"' GO")
        IF (table_stats->stats_cnt[item_count].initial_gather_ind)
         SET dm_err->eproc = "Deleting initial_stats_only rows from dm_info"
         DELETE  FROM dm_info di
          WHERE di.info_domain="DM2_RUNSTATS_INITIAL_STATS_ONLY"
           AND di.info_name=concat(table_stats->stats_cnt[item_count].owner,".",table_stats->
        ...
        ENDIF
       ELSE
        EXECUTE dm2_flexstats "TABLE", value(table_stats->stats_cnt[item_count].owner), value(
         table_stats->stats_cnt[item_count].tabname),
        value(evaluate(dr_use_auto_sample_size_ind,1,- (1.0),table_stats->stats_cnt[item_count].
          est_pct)), value(table_stats->stats_cnt[item_count].method_opt), value(table_stats->
         stats_cnt[item_count].block_sample),
        value(table_stats->cascade)
       ENDIF
       SET dm_err->err_ind = 0
       IF ((table_stats->cascade="FALSE"))
        IF ((((program_stack_rs->qual[1].name="DM2_ROUTINE_TASKS")) OR (validate(dm2_queue_rows_only,
         0)=1))
         AND dr_queue_table_exists_ind)
         IF (dri_fill_index_list(table_stats->stats_cnt[item_count].owner,table_stats->stats_cnt[
          item_count].tabname)=0)
          GO TO exit_script
         ELSE
          FOR (dr_iterator = 1 TO index_list->index_cnt)
            SET dm_err->eproc = build("Writing process_queue row for: dm2_gather_index_stats 'INDEX'",
             ",'",index_list->owner,"'",",'",
             index_list->qual[dr_iterator].index_name,"'",",",evaluate(dr_use_auto_sample_size_ind,1,
              - (1.0),index_list->qual[dr_iterator].est_pct))
            CALL disp_msg(" ",dm_err->logfile,0)
            SET dpq_process_queue->process_type = dpq_statistics
            SET dpq_process_queue->op_type = dpq_gather
            SET dpq_process_queue->owner_name = index_list->owner
            SET dpq_process_queue->object_type = dpq_index
            SET dpq_process_queue->op_method = dpq_db
            SET dpq_process_queue->object_name = index_list->qual[dr_iterator].index_name
            SET dpq_process_queue->priority = 250
            SET dpq_process_queue->routine_tasks_ind = 1
            SET dpq_process_queue->operation_txt = build("execute dm2_gather_index_stats 'INDEX' ",
             ",'",index_list->owner,"','",index_list->qual[dr_iterator].index_name,
             "',",evaluate(dr_use_auto_sample_size_ind,1,- (1.0),index_list->qual[dr_iterator].
              est_pct)," go")
            IF (dpq_process_queue_row(null)=0)
             SET dm_err->err_ind = 0
            ENDIF
         ...
         ENDIF
        ELSE
         SET dm_err->eproc = build("Calling dm2_gather_index_stats 'TABLE'",",'",table_stats->
          stats_cnt[item_count].owner,"'",",'",
          table_stats->stats_cnt[item_count].tabname,"'",",",evaluate(dr_use_auto_sample_size_ind,1,
           - (1.0),index_list->est_pct))
         CALL disp_msg(" ",dm_err->logfile,0)
         EXECUTE dm2_gather_index_stats "TABLE", value(table_stats->stats_cnt[item_count].owner),
         value(table_stats->stats_cnt[item_count].tabname),
         value(evaluate(dr_use_auto_sample_size_ind,1,- (1.0),index_list->est_pct))
         SET dm_err->err_ind = 0
        ENDIF
       ENDIF
      ENDIF
    ...
    IF ((dm_err->debug_flag >= 1))
     ...
     SET dm_err->eproc = build("Stats Gather time for all tables and indexes selected = ",
      dr_analy_time," minutes.")
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
   ENDIF
  ELSE
 ...
 SUBROUTINE dr_list_stale_report(null)
   ...
   SELECT INTO build(dlsr_report_destination)
    ...
    DETAIL
     ...
     col 31, table_stats->stats_cnt[d.seq].stale_pct";L", col 37,
     table_stats->stats_cnt[d.seq].num_rows";L", col 48, table_stats->stats_cnt[d.seq].table_mods";L",
     col 59, table_stats->stats_cnt[d.seq].last_analyzed"MM/DD/YY HH:MM;;Q", col 74,
     table_stats->stats_cnt[d.seq].last_mod_dt_tm"MM/DD/YY HH:MM;;Q"
    FOOT REPORT
     row + 1,
...
#exit_script
 ...
 SET dm2_process_event_rs->message = evaluate(dm_err->err_ind,1,dm_err->emsg,evaluate(
   dr_realstats_ind,1,"'Oracle Version > 10. Auto-successing dm2_runstats.'",""))
 SET dr_reset_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_execution,dr_log_id,1)
 SET dm_err->err_ind = dr_reset_err_ind
 SET dm_err->eproc = "Ending dm2_runstats."
 CALL final_disp_msg("dm2_runstats")
