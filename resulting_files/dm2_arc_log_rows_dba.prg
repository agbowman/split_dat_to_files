CREATE PROGRAM dm2_arc_log_rows:dba
 ...
 SET reply->table_name = "DM_ARC_LOG"
 ...
 SELECT INTO "nl:"
  dal.rowid
  FROM dm_arc_log dal
  WHERE dal.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
   AND parser(sbr_getrowidnotexists("dal.dm_arc_log_id+0 > 0","dal"))
  DETAIL
   v_rows = (v_rows+ 1)
   IF (mod(v_rows,50)=1)
    stat = alterlist(reply->rows,(v_rows+ 49))
   ENDIF
   reply->rows[v_rows].row_id = dal.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,v_rows)
  WITH nocounter, maxqual(dal,request->max_rows)
 ;end select
