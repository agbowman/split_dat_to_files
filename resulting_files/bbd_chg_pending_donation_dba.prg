CREATE PROGRAM bbd_chg_pending_donation:dba
 RECORD reply(
   1 encounter_id = f8
   1 status_data
     2 status = c1
 ...
 DECLARE person_reltn_id = f8 WITH protect, noconstant(0.0)
 ...
 DECLARE nbr_of_add_exceptions = i4 WITH protect, noconstant(size(request->add_qual,5))
 DECLARE nbr_of_update_exceptions = i4 WITH protect, noconstant(size(request->update_qual,5))
 DECLARE found_row = i4 WITH protect, noconstant(0)
 DECLARE donor_org_reltn_ind = i4 WITH protect, noconstant(0)
 DECLARE cdf_meaning = c12 WITH protect, noconstant(fillstring(12," "))
 DECLARE person_to_add = f8 WITH protect, noconstant(0.0)
 DECLARE successful_action = c1 WITH protect, noconstant(" ")
 ...
 IF (code_value=0)
  ...
  SET reply->status_data.subeventstatus[1].sourceobjectvalue = "bbd_chg_pending_donation"
  ...
  GO TO exit_script
 ENDIF
 SET bbd_contact_type_cd = code_value
 SET code_value = 0.0
 SET code_cnt = 1
 SET stat = uar_get_meaning_by_codeset(69,"BLOODDONOR",code_cnt,code_value)
 IF (code_value=0)
  SET failed = "T"
  SET reply->status_data.subeventstatus[1].sourceobjectname = "script"
  SET reply->status_data.subeventstatus[1].sourceobjectvalue = "bbd_chg_pending_donation"
  ...
  GO TO exit_script
 ENDIF
 SET encntr_type_class_code = code_value
 SELECT INTO "nl:"
  c.child_code_value
 ...
 SET code_value = 0.0
 SET code_cnt = 1
 SET stat = uar_get_meaning_by_codeset(89,"CERNER",code_cnt,code_value)
 IF (code_value=0)
  SET failed = "T"
  SET reply->status_data.subeventstatus[1].sourceobjectname = "script"
  ...
  GO TO exit_script
 ENDIF
 SET contributor_system = code_value
 SET code_value = 0.0
 SET code_cnt = 1
 SET stat = uar_get_meaning_by_codeset(8,"AUTH",code_cnt,code_value)
 IF (code_value=0)
  SET failed = "T"
  SET reply->status_data.subeventstatus[1].sourceobjectname = "script"
  SET reply->status_data.subeventstatus[1].sourceobjectvalue = "bbd_chg_donor_elig"
  ...
  GO TO exit_script
 ENDIF
 SET contact_type_cd = code_value
 SET code_value = 0.0
 SET code_cnt = 1
 SET stat = uar_get_meaning_by_codeset(351,"BBRECIPIENT",code_cnt,code_value)
 IF (code_value=0)
  SET failed = "T"
  SET reply->status_data.subeventstatus[1].sourceobjectname = "script"
  SET reply->status_data.subeventstatus[1].sourceobjectvalue = "bbd_chg_donor_elig"
  ...
  GO TO exit_script
 ENDIF
 SET person_reltn_type_code = code_value
 SET code_value = 0.0
 SET code_cnt = 1
 ...
 IF (code_value=0)
  ...
  GO TO exit_script
 ENDIF
 SET encntr_status_code = code_value
 SET code_value = 0.0
 SET code_cnt = 1
 ...
 IF (code_value=0)
  ...
  GO TO exit_script
 ENDIF
 SET contact_status_code = code_value
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.code_set=40
 ...
 UPDATE  FROM encounter e
  SET e.updt_cnt = (e.updt_cnt+ 1), e.updt_dt_tm = cnvtdatetime(curdate,curtime3), e.updt_id =
   reqinfo->updt_id,
   e.updt_task = reqinfo->updt_task, e.updt_applctx = reqinfo->updt_applctx, e.encntr_class_cd = 0,
   e.encntr_type_cd = encntr_type_code, e.encntr_type_class_cd = encntr_type_class_code, e
   .encntr_status_cd = encntr_status_code,
   e.pre_reg_dt_tm = null, e.pre_reg_prsnl_id = 0, e.reg_dt_tm = cnvtdatetime(request->contact_dt_tm),
   e.reg_prsnl_id = reqinfo->updt_id, e.est_arrive_dt_tm = null, e.est_depart_dt_tm = null,
   e.arrive_dt_tm = null, e.depart_dt_tm = null, e.admit_type_cd = 0,
   e.admit_src_cd = 0, e.admit_mode_cd = 0, e.admit_with_medication_cd = 0,
   e.referring_comment = null, e.disch_disposition_cd = 0, e.disch_to_loctn_cd = 0,
   e.preadmit_nbr = null, e.preadmit_testing_cd = 0, e.readmit_cd = 0,
   e.accommodation_cd = 0, e.accommodation_request_cd = 0, e.alt_result_dest_cd = 0,
   e.ambulatory_cond_cd = 0, e.courtesy_cd = 0, e.diet_type_cd = 0,
   e.isolation_cd = 0, e.med_service_cd = 0, e.result_dest_cd = 0,
   e.confid_level_cd = 0, e.vip_cd = 0, e.name_last_key = null,
   e.name_first_key = null, e.name_full_formatted = null, e.name_last = null,
   e.name_first = null, e.name_phonetic = null, e.sex_cd = 0,
   e.birth_dt_cd = 0, e.birth_dt_tm = null, e.species_cd = 0,
   e.data_status_cd = contact_type_cd, e.data_status_dt_tm = cnvtdatetime(curdate,curtime3), e
   .data_status_prsnl_id = reqinfo->updt_id,
   e.contributor_system_cd = contributor_system, e.location_cd = 0, e.loc_facility_cd = 0,
   e.loc_building_cd = 0, e.loc_nurse_unit_cd = 0, e.loc_room_cd = 0,
   e.loc_bed_cd = 0, e.disch_dt_tm = null, e.guarantor_type_cd = 0,
   e.loc_temp_cd = 0, e.organization_id = 0, e.reason_for_visit = null,
   e.encntr_financial_id = 0, e.name_first_synonym_id = 0, e.financial_class_cd = 0,
   e.bbd_procedure_cd = request->bbd_procedure_cd
  WHERE (e.encntr_id=request->encntr_id)
   AND (e.person_id=request->person_id)
  WITH nocounter
 ;end update
 IF (curqual=0)
  ...
  GO TO exit_script
 ELSE
  SET reply->encounter_id = request->encntr_id
 ENDIF
 SELECT INTO "nl:"
  dc.*
  FROM bbd_donor_contact dc
  WHERE (dc.contact_id=request->contact_id)
   AND (dc.person_id=request->person_id)
   AND dc.active_ind=1
  WITH counter, forupdate(dc)
 ;end select
 UPDATE  FROM bbd_donor_contact dc
  SET dc.organization_id = request->organization_id, dc.updt_applctx = reqinfo->updt_applctx, dc
   .updt_dt_tm = cnvtdatetime(curdate,curtime3),
   dc.updt_id = reqinfo->updt_id, dc.updt_task = reqinfo->updt_task, dc.updt_cnt = 0,
   dc.contact_type_cd = bbd_contact_type_cd, dc.init_contact_prsnl_id = reqinfo->updt_id, dc
   .contact_outcome_cd = 0,
   dc.contact_dt_tm = cnvtdatetime(request->contact_dt_tm), dc.needed_dt_tm = cnvtdatetime(request->
    needed_dt), dc.contact_status_cd = contact_status_code,
   dc.inventory_area_cd = request->inventory_area_cd, dc.owner_area_cd = request->owner_area_cd
  WHERE (dc.contact_id=request->contact_id)
   AND (dc.encntr_id=request->encntr_id)
   AND (dc.person_id=request->person_id)
  WITH nocounter
 ;end update
 IF (curqual=0)
 ...
 IF ((request->update_encntr_person_reltn=0))
  SELECT INTO "nl:"
   ...
   WHERE (e.encntr_person_reltn_id=request->encntr_person_reltn_id)
   DETAIL
    found_row = 1
   WITH nocounter, forupdate(e)
  ;end select
  IF (found_row=1)
   UPDATE  FROM encntr_person_reltn e
    SET e.active_ind = 0, e.active_status_cd = reqdata->inactive_status_cd, e.active_status_dt_tm =
     cnvtdatetime(curdate,curtime3),
     e.active_status_prsnl_id = reqinfo->updt_id, e.updt_cnt = (e.updt_cnt+ 1), e.updt_dt_tm =
     cnvtdatetime(curdate,curtime3),
 ...
 ELSE
  ...
  IF (found_row=1)
   UPDATE  FROM encntr_person_reltn e
    SET e.active_ind = 1, e.active_status_cd = reqdata->active_status_cd, e.active_status_dt_tm =
     cnvtdatetime(curdate,curtime3),
     e.active_status_prsnl_id = reqinfo->updt_id, e.person_reltn_cd = 0, e.related_person_reltn_cd =
     request->relation_cd,
     e.related_person_id = request->recipient_id, e.updt_cnt = (e.updt_cnt+ 1), e.updt_dt_tm =
     cnvtdatetime(curdate,curtime3),
     e.updt_id = reqinfo->updt_id, e.updt_task = reqinfo->updt_task, e.updt_applctx = reqinfo->
     updt_applctx,
     e.beg_effective_dt_tm = cnvtdatetime(curdate,curtime3), e.end_effective_dt_tm = cnvtdatetime(
      "31-DEC-2100 23:59:59:99"), e.data_status_cd = contact_type_cd,
     e.data_status_dt_tm = cnvtdatetime(curdate,curtime3), e.data_status_prsnl_id = reqinfo->updt_id,
     e.contributor_system_cd = contributor_system,
     e.contact_role_cd = 0, e.genetic_relationship_ind = 1, e.living_with_ind = 0,
     e.visitation_allowed_cd = 0, e.priority_seq = 0, e.free_text_cd = 0,
     e.ft_rel_person_name = null, e.internal_seq = 0
    WHERE (e.encntr_person_reltn_id=request->encntr_person_reltn_id)
    WITH nocounter
   ;end update
   IF (curqual=0)
  ...
  ELSE
   ...
   INSERT  FROM encntr_person_reltn p
    SET p.encntr_person_reltn_id = new_encntr_person_reltn_id, p.person_reltn_type_cd =
     person_reltn_type_code, p.encntr_id = request->encntr_id,
     ...
     p.contributor_system_cd = contributor_system,
...
#person_donor
 ...
 ENDIF
 SET counter = 0
 FOR (counter = 1 TO nbr_of_update_exceptions)
   SET person_to_add = request->person_id
   SELECT INTO "nl:"
    b.*
    FROM bb_exception b
    WHERE (b.exception_id=request->update_qual[counter].exception_id)
     AND b.person_id=person_to_add
     AND (b.updt_cnt=request->update_qual[counter].updt_cnt)
     AND b.active_ind=1
   ...
   IF (curqual=0)
    SET failed = "T"
    SET reply->status = "F"
    SET reply->status_data.subeventstatus[1].sourceobjectname = "bbd_chg_pending_donation.prg"
    ...
    SET reply->status_data.subeventstatus[1].targetobjectvalue = "Error on locking exceptions."
    SET reply->status_data.subeventstatus[1].sourceobjectqual = 1
    GO TO exit_script
   ENDIF
   UPDATE  FROM bb_exception b
    SET b.override_reason_cd = request->update_qual[counter].override_reason_cd, b.updt_cnt = (b
   ...
   IF (curqual=0)
    ...
    SET reply->status_data.subeventstatus[1].operationname = "Update"
    SET reply->status_data.subeventstatus[1].targetobjectname = "bb_exception"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = "Error on updating exceptions."
    SET reply->status_data.subeventstatus[1].sourceobjectqual = 1
    GO TO exit_script
   ENDIF
 ENDFOR
 FOR (counter = 1 TO nbr_of_add_exceptions)
   SET person_to_add = request->person_id
   SET exception_status = "I"
   SET modify = nopredeclare
   CALL add_bb_exception(request->add_qual[counter].exception_type_mean,request->add_qual[counter].
    override_reason_cd,person_to_add,request->contact_id,bbd_contact_type_cd)
   SET modify = predeclare
   IF (exception_status="S")
    SET failed = "F"
   ELSE
    SET failed = "T"
    SET reply->status_data.subeventstatus[1].sourceobjectname = "bbd_chg_pending_donation"
    SET reply->status_data.subeventstatus[1].operationname = "insert"
    SET reply->status_data.subeventstatus[1].targetobjectvalue = "Error on inserting exception"
    SET reply->status_data.subeventstatus[1].sourceobjectqual = 1
    SET reply->status_data.subeventstatus[1].sub_event_dt_tm = cnvtdatetime(curdate,curtime)
    GO TO exit_script
   ENDIF
