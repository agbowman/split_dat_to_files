CREATE PROGRAM bhs_eks_nurse_witness:dba
 PROMPT
 ...
 protect
 DECLARE mf_witnessreqd_cd = f8 WITH constant(validatecodevalue("DISPLAYKEY",72,
   "BLOODPRODUCTRNWITNESSREQUIRED")), protect
 DECLARE mf_bptransfusedwitreqd_cd = f8 WITH constant(validatecodevalue("DESCRIPTION",72,
   "Blood Products Transfused-RN Wit Reqd")), protect
 ...
 DECLARE inprogress = f8 WITH constant(validatecodevalue("MEANING",8,"IN PROGRESS")), protect
 DECLARE bloodbankproduct = f8 WITH constant(validatecodevalue("DISPLAYKEY",106,"BLOODBANKPRODUCT")),
 protect
 DECLARE dcpgeneric = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,"DCPGENERICCODE")), protect
 ...
 IF (( $RULETYPE="DISCERN"))
  ...
  SET ce_request->clin_detail_list[1].clinical_event_id = 782514839
 ELSEIF (( $RULETYPE="TIMER"))
  ...
  DECLARE qualcnt = i4
  DECLARE alert_id = f8
  FOR (x = 1 TO size(eksdata->tqual[3].qual,5))
   CALL echo("inside qual for")
   IF ((eksdata->tqual[3].qual[x].template_name IN ("EKS_TIMER_SOURCE_L*")))
    CALL echo(build("found template",x))
    SET qualcnt = x
   ENDIF
  ENDFOR
  FOR (x = 1 TO size(eksdata->tqual[3].qual[qualcnt].data,5))
   CALL echo("inside data for loop")
   IF ((eksdata->tqual[3].qual[qualcnt].data[x].misc IN ("*ALERT_ID*")))
    CALL echo(build("found alert_id",x))
    SET alert_id = cnvtreal(eksdata->tqual[3].qual[qualcnt].data[(x+ 1)].misc)
   ENDIF
  ENDFOR
  SELECT INTO "nl:"
   FROM eks_alert_escalation e
   PLAN (e
    WHERE e.alert_id=alert_id)
   HEAD REPORT
    cnt = 0
   HEAD e.alert_id
    cnt += 1, stat = alterlist(ce_request->clin_detail_list,cnt), ce_request->clin_detail_list[cnt].
    clinical_event_id = cnvtreal(e.subject_text),
    log_message = concat(log_message,"clinical_event id:",cnvtstring(ce_request->clin_detail_list[cnt
      ].clinical_event_id))
   WITH nocounter
  ;end select
  IF (curqual <= 0)
   ...
   GO TO exit_program
  ENDIF
 ELSEIF (( $RULETYPE="CLINICAL_EVENT"))
  FREE RECORD ce_request
  RECORD ce_request(
    1 clin_detail_list[*]
      2 clinical_event_id = f8
  ...
  DECLARE benw_idx = i4
  SET benw_idx = size(request->clin_detail_list,5)
  DECLARE mn_i = i4
  IF (benw_idx > 0)
   SET stat = alterlist(ce_request->clin_detail_list,benw_idx)
   FOR (mn_i = 0 TO benw_idx)
     SET ce_request->clin_detail_list[mn_i].clinical_event_id = request->clin_detail_list[mn_i].
     clinical_event_id
     SET log_message = concat(log_message," clin_event_id: ",build(ce_request->clin_detail_list[mn_i]
       .clinical_event_id))
     SET log_misc1 = cnvtstring(ce_request->clin_detail_list[mn_i].clinical_event_id)
   ENDFOR
  ENDIF
 ENDIF
 IF (size(ce_request->clin_detail_list,5) <= 0)
  SET log_message = concat(log_message," No ce_request->clin_detail_list found ")
  GO TO exit_program
 ENDIF
  ...
  *
  FROM clinical_event ce,
   (dummyt d  WITH seq = size(ce_request->clin_detail_list,5))
  PLAN (d)
   JOIN (ce
   WHERE (ce.clinical_event_id=ce_request->clin_detail_list[d.seq].clinical_event_id)
  ...
  ORDER BY ce.valid_from_dt_tm, ce.clinical_event_id
  HEAD REPORT
  ...
  HEAD ce.clinical_event_id
   ...
   .clinical_event_id,
   eventinfo->parent_event_id = ce.event_id, eventinfo->valid_from_dt_tm = ce.valid_from_dt_tm,
   eventinfo->formname = uar_get_code_display(ce.event_cd),
   eventinfo->formtype = ce.event_cd, eventinfo->result_stats_cd = ce.result_status_cd, refstart = (
   findstring("!",ce.reference_nbr,1)+ 1),
   refend = (findstring("!",ce.reference_nbr,refstart) - 1), reflen = (refend - refstart),
   CALL echo(build(refstart,":",refend,":",reflen)),
   eventinfo->refid = concat("*",trim(substring(refstart,reflen,ce.reference_nbr),3),"*")
  WITH nocounter
 ;end select
 IF (curqual=0)
 ...
 SELECT
  ...
  PLAN (ce
   WHERE ce.encntr_id=encntrid
    AND ce.valid_from_dt_tm=cnvtdatetime(eventinfo->valid_from_dt_tm)
    AND ce.result_status_cd IN (authverified, altered, modified, inprogress)
   ...
   infusionstarttime, chartingreason, mf_witnessreqd_cd))
   JOIN (cedr
  ...
  HEAD ce.event_cd
   ...
   ELSEIF (ce.event_cd IN (nursestatementofattestation, dcpgeneric))
   ...
   ELSEIF (ce.event_cd=chartingreason
    ...
    eventinfo->donotsendalerts = 1, log_message = concat(log_message,
     "!!! Do Not Send alert 4 hour event !!!")
   ELSEIF (ce.event_cd=mf_witnessreqd_cd
    AND cnvtupper(trim(ce.result_val,3))="NO")
    eventinfo->donotsendalerts = 1, log_message = concat(log_message,
     "!!! Alerts not sent - Witness not required !!!")
   ENDIF
  WITH nocounter
 ;end select
 ...
 IF ((((eventinfo->result_stats_cd IN (altered, modified))) OR (( $RULETYPE="TIMER"))) )
  ...
  SELECT
   ...
   PLAN (ce
    ...
    WHERE ce2.parent_event_id=ce.event_id
     AND ce2.result_status_cd IN (authverified, altered, modified, inprogress))
    JOIN (t1)
    JOIN (ce3
    WHERE ce3.parent_event_id=ce2.event_id
     AND ce3.result_status_cd IN (authverified, altered, modified, inprogress)
     AND ce3.event_cd IN (nursewitness, nursewitnesstransfusion, nursestatementofattestation,
    patchappliedtime, mf_bptransfusedwitreqd_cd,
    mf_witnessreqd_cd, chartingreason))
    JOIN (cedr
   ...
   HEAD ce3.event_cd
    ...
    ELSEIF (ce3.event_cd=infusionstarttime)
     eventinfo->infusiontime = format(cnvtdatetime(cedr.result_dt_tm),";;q")
    ELSEIF (ce3.event_cd=mf_bptransfusedwitreqd_cd
     AND trim(ce3.event_tag,3) IN ("Rh Immune Globulin: Intramuscular",
    "Rh Immune Globulin: Intravenous"))
     eventinfo->donotsendalerts = 1, log_message = concat(log_message,
      "!!!RH Immune blobulin - do not send alert !!!")
    ELSEIF (ce3.event_cd=chartingreason
     AND cnvtupper(trim(ce3.event_tag,3)) IN ("*4 HOUR CHECK*", "*CLINICIAN DOSE*"))
     eventinfo->donotsendalerts = 1, log_message = concat(log_message,
      "!!!4 HOUR CHECK/CLINICIAN DOSE - do not send alert !!!")
    ELSEIF (ce3.event_cd=mf_witnessreqd_cd
     AND cnvtupper(trim(ce3.result_val,3))="NO")
     eventinfo->donotsendalerts = 1, log_message = concat(log_message,
      "!!! Alerts not sent - Witness Required = NO !!!")
    ENDIF
 ...
 SELECT INTO "NL:"
  ...
  PLAN (p
   ...
   "BHS Nursing Student", "BHS Onco RN", "BHS SN RN", "BHS OB RESIDENT", "BHS OB RN",
   "BHS OB/GYN MD", "BHS SN Manager"))) )
  HEAD REPORT
   cnt = 0
  DETAIL
...
#exit_program
 ...
 END ;Subroutine
 SET last_mod =
 "001 02/16/2017 SN050530 SR#414837079 Update Rule to include: BHS SN RN and BHS SN Manager ."
END GO
