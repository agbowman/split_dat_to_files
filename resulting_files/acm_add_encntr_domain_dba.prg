CREATE PROGRAM acm_add_encntr_domain:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->encntr_domain_qual[xref->add[index].idx].encntr_domain_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(encounter_seq,nextval)
     FROM dual
     DETAIL
      acm_request->encntr_domain_qual[xref->add[index].idx].encntr_domain_id = cnvtreal(nextseqnum),
      reply->encntr_domain_qual[xref->add[index].idx].encntr_domain_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->encntr_domain_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->encntr_domain_qual[xref->add[index].idx].encntr_domain_id = acm_request->
    encntr_domain_qual[xref->add[index].idx].encntr_domain_id
   ENDIF
   SET reply->encntr_domain_qual[xref->add[index].idx].status = 0
   IF ((acm_request->encntr_domain_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->encntr_domain_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->encntr_domain_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->encntr_domain_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   encntr_domain e
  SET e.encntr_domain_id = acm_request->encntr_domain_qual[xref->add[d.seq].idx].encntr_domain_id, e
   .beg_effective_dt_tm = cnvtdatetime(acm_request->encntr_domain_qual[xref->add[d.seq].idx].
    beg_effective_dt_tm), e.encntr_domain_type_cd = acm_request->encntr_domain_qual[xref->add[d.seq].
   idx].encntr_domain_type_cd,
   e.encntr_id =
   IF ((acm_request->encntr_domain_qual[xref->add[d.seq].idx].encntr_id > 0)) acm_request->
    encntr_domain_qual[xref->add[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->encntr_domain_qual[xref->add[d.seq].idx].encntr_idx].
    encntr_id
   ENDIF
   , e.end_effective_dt_tm = cnvtdatetime(acm_request->encntr_domain_qual[xref->add[d.seq].idx].
    end_effective_dt_tm), e.loc_bed_cd = acm_request->encntr_domain_qual[xref->add[d.seq].idx].
   loc_bed_cd,
   e.loc_building_cd = acm_request->encntr_domain_qual[xref->add[d.seq].idx].loc_building_cd, e
   .loc_facility_cd = acm_request->encntr_domain_qual[xref->add[d.seq].idx].loc_facility_cd, e
   .loc_nurse_unit_cd = acm_request->encntr_domain_qual[xref->add[d.seq].idx].loc_nurse_unit_cd,
   e.loc_room_cd = acm_request->encntr_domain_qual[xref->add[d.seq].idx].loc_room_cd, e
   .med_service_cd = acm_request->encntr_domain_qual[xref->add[d.seq].idx].med_service_cd, e
   .person_id =
   IF ((acm_request->encntr_domain_qual[xref->add[d.seq].idx].person_id > 0)) acm_request->
    encntr_domain_qual[xref->add[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->encntr_domain_qual[xref->add[d.seq].idx].person_idx].
    person_id
   ENDIF
   ,
   e.active_ind =
   IF ((((acm_request->encntr_domain_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((
   acm_request->encntr_domain_qual[xref->add[d.seq].idx].active_status_cd=reqdata->active_status_cd)
   )) ) 1
   ELSE 0
   ENDIF
   , e.active_status_cd =
   IF ((acm_request->encntr_domain_qual[xref->add[d.seq].idx].active_status_cd > 0.0)) acm_request->
    encntr_domain_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (e)
  WITH nocounter, status(reply->encntr_domain_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->encntr_domain_qual[xref->add[index].idx].status=0))
