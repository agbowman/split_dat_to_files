CREATE PROGRAM dm_pcmb_charge:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PCMB_CHARGE"
  ...
  GO TO end_of_program
 ENDIF
 IF ("Z"=validate(dm_pcmb_charge_vrsn,"Z"))
  DECLARE dm_pcmb_charge_vrsn = vc WITH noconstant("479576.000")
 ENDIF
 SET dm_pcmb_charge_vrsn = "479576.000"
 CALL echorecord(request,"ccluserdir:percmb.dat")
 FREE SET rreclist
 RECORD rreclist(
   1 from_rec[*]
     2 from_id = f8
 ...
 FREE RECORD pcc_requestmain
 RECORD pcc_requestmain(
   1 cstart = i4
   1 cend = i4
 )
 FREE SET rcolumns
 ...
 FREE RECORD pcc_excl
 RECORD pcc_excl(
   1 excl_cnt = i4
   1 qual[*]
     2 column_name = vc
 )
 DECLARE pcc_idx = i4 WITH protect, noconstant(0)
 SET count1 = 0
 SET count2 = 0
 SET col_count = 0
 SET mod_count = 0
 ...
 SET v_ocii_exists = 0
 SET v_comma_str = " "
 DECLARE charge_originally_offset = i2
 DECLARE ssuspensedesc = vc WITH public, noconstant("")
 DECLARE dm_cmb_blocks = i4
 ...
 IF (319_mrn IN (0.0, null))
  ...
  GO TO end_of_program
 ENDIF
 DECLARE 13019_finnbr_alias = f8 WITH public, noconstant(0.0)
 ...
 IF (13019_finnbr_alias IN (0.0, null))
  ...
  GO TO end_of_program
 ENDIF
 DECLARE 319_fin_nbr = f8 WITH public, noconstant(0.0)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   pcc_excl->excl_cnt += 1, stat = alterlist(pcc_excl->qual,pcc_excl->excl_cnt), pcc_excl->qual[
   pcc_excl->excl_cnt].column_name = utc.column_name
  WITH nocounter
 ;end select
 SELECT INTO "NL:"
  l.attr_name
  ...
  WHERE t.table_name="CHARGE"
   ...
   AND l.structtype="F"
   AND btest(l.stat,11)=0
   AND  NOT (expand(pcc_idx,1,pcc_excl->excl_cnt,l.attr_name,pcc_excl->qual[pcc_idx].column_name))
  DETAIL
   col_count += 1, stat = alterlist(rcolumns->col,col_count)
 ...
 SET stat = alterlist(pcc_excl->qual,0)
 SELECT INTO "nl:"
  FROM user_tab_cols utc
 ...
 SELECT INTO "NL:"
  ...
  WHERE t.table_name="CHARGE_MOD"
   ...
   AND  NOT (expand(pcc_idx,1,pcc_excl->excl_cnt,l.attr_name,pcc_excl->qual[pcc_idx].column_name))
  DETAIL
   mod_count += 1, stat = alterlist(modcolumns->col,mod_count), modcolumns->col[mod_count].col_name
    = l.attr_name
  WITH nocounter
 ;end select
 ...
 IF ((request->xxx_combine[icombine].encntr_id > 0))
  SELECT INTO "nl:"
   t.organization_id
   FROM encounter t
   WHERE (t.encntr_id=request->xxx_combine[icombine].encntr_id)
   DETAIL
    to_payor_id = t.organization_id
   WITH nocounter
  ;end select
 ENDIF
 SELECT
  ...
  DETAIL
   count1 += 1
   IF (mod(count1,100)=1)
    stat = alterlist(rreclist->from_rec,(count1+ 100))
   ENDIF
   rreclist->from_rec[count1].from_id = frm.charge_item_id, rreclist->from_rec[count1].to_person_id
    = request->xxx_combine[icombine].to_xxx_id, rreclist->from_rec[count1].to_encntr_id = request->
   xxx_combine[icombine].encntr_id,
   rreclist->from_rec[count1].encntr_id = frm.encntr_id, rreclist->from_rec[count1].payor_id = frm
   .payor_id, rreclist->from_rec[count1].item_price = frm.item_price,
   rreclist->from_rec[count1].item_extended_price = frm.item_extended_price, rreclist->from_rec[
   count1].charge_type_cd = frm.charge_type_cd, rreclist->from_rec[count1].posted_cd = frm.posted_cd,
   rreclist->from_rec[count1].posted_dt_tm = frm.posted_dt_tm, rreclist->from_rec[count1].process_flg
    = frm.process_flg, rreclist->from_rec[count1].credited_dt_tm = frm.credited_dt_tm,
   rreclist->from_rec[count1].beg_effective_dt_tm = frm.beg_effective_dt_tm, rreclist->from_rec[
   count1].activity_type_cd = frm.activity_type_cd, rreclist->from_rec[count1].interface_file_id =
   frm.interface_file_id
  FOOT REPORT
   stat = alterlist(rreclist->from_rec,count1)
  WITH forupdatewait(frm)
 ;end select
 SELECT INTO "nl:"
  FROM (dummyt frm  WITH seq = size(rreclist->from_rec,5)),
   interface_file i
  PLAN (frm
   WHERE (rreclist->from_rec[frm.seq].interface_file_id > 0.0))
   JOIN (i
   WHERE (i.interface_file_id=rreclist->from_rec[frm.seq].interface_file_id)
    AND i.active_ind=true)
  DETAIL
   rreclist->from_rec[frm.seq].profit_type_cd = i.profit_type_cd
  WITH nocounter
 ;end select
 CALL echorecord(rreclist,"ccluserdir:recpcmb.dat")
 DECLARE new_combine_ind = i2 WITH noconstant(true)
 IF ((request->xxx_combine[icombine].encntr_id > 0))
  IF (validate(request->xxx_uncombine,char(128)) != char(128))
   SELECT INTO "nl:"
    FROM person_combine pc,
     ...
     interface_file i
    PLAN (pc
     WHERE (pc.from_person_id=request->xxx_combine[icombine].to_xxx_id)
      AND (pc.to_person_id=request->xxx_combine[icombine].from_xxx_id))
     JOIN (pcd
     WHERE pcd.person_combine_id=pc.person_combine_id
      AND pcd.entity_name="CHARGE"
      AND pcd.combine_action_cd=add)
     JOIN (c
     WHERE c.charge_item_id=pcd.entity_id)
     JOIN (i
 ...
 SELECT INTO "nl:"
  ...
  WHERE d.info_domain="DM COMBINE"
   AND d.info_name="PCMB CHARGE BATCH SIZE"
  DETAIL
 ...
 ENDIF
 SET pcc_requestmain->cstart = 0
 SET pcc_requestmain->cend = 0
 WHILE ((pcc_requestmain->cend < count1))
   SET pcc_requestmain->cstart = (pcc_requestmain->cend+ 1)
   SET pcc_requestmain->cend = least(count1,(pcc_requestmain->cend+ dm_cmb_blocks))
   CALL echo(build("Performing combine for records: ",pcc_requestmain->cstart," to:",pcc_requestmain
     ->cend))
   CALL echo("-----")
   EXECUTE person_cmb_charge_child
 ENDWHILE
#skip_processing
 FREE SET rreclist
 FREE SET rcolumns
