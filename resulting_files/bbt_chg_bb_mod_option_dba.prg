CREATE PROGRAM bbt_chg_bb_mod_option:dba
 RECORD reply(
   1 option_id = f8
   1 new_prods[*]
 ...
 RECORD isbtpoolseq(
   1 org_qual[*]
     ...
     2 seq_nbr = i4
     2 mod_pool_nbr_id = f8
 )
 SET reply->status_data.status = "F"
 SET modify = predeclare
 DECLARE system_dt_tm = q8 WITH constant(cnvtdatetime(sysdate))
 DECLARE display_key_new = vc WITH noconstant(" ")
 DECLARE option_id_new = f8 WITH noconstant(0.0)
 ...
 DECLARE seq_nbr_new = i4 WITH noconstant(0)
 ...
 DECLARE boptionidfound = i2 WITH noconstant(0)
 SET display_key_new = cnvtupper(cnvtalphanum(request->display))
 SET option_id_new = 0.0
 SELECT INTO "nl:"
  mo.display_key
  ...
  PLAN (mo
   WHERE mo.display_key=display_key_new
    AND mo.active_ind=1)
  DETAIL
   option_id_new = mo.option_id, boptionidfound = 1
  WITH nocounter
 ;end select
 IF (option_id_new > 0.0)
  UPDATE  FROM bb_mod_option mo
   SET mo.end_effective_dt_tm = cnvtlookbehind("1,HUN",cnvtdatetime(system_dt_tm)), mo.active_ind = 0,
    mo.active_status_cd = reqdata->inactive_status_cd,
    ...
    updt_id,
    mo.updt_cnt = (mo.updt_cnt+ 1)
   PLAN (mo
    WHERE mo.option_id=option_id_new)
   WITH nocounter
  ;end update
 ...
 INSERT  FROM bb_mod_option mo
  SET mo.option_id = reply->option_id, mo.display = request->display, mo.display_key =
   display_key_new,
   mo.beg_effective_dt_tm = cnvtdatetime(system_dt_tm), mo.end_effective_dt_tm = cnvtdatetime(
 ...
 INSERT  FROM (dummyt d  WITH seq = value(mnp_cnt)),
  ...
  SET mnp.mod_new_prod_id = reply->new_prods[d.seq].mod_new_prod_id, mnp.option_id = reply->option_id,
   ...
   mnp.orig_plasma_prod_cd = request->new_prods[d.seq].orig_plasma_prod_cd, mnp.new_product_cd =
   request->new_prods[d.seq].new_product_cd, mnp.quantity = request->new_prods[d.seq].quantity,
 ...
 IF ((request->generate_prod_nbr_ind=1))
  ...
  SELECT INTO "nl:"
   ...
   PLAN (mpn
    WHERE mpn.option_id=option_id_new
     AND (mpn.prefix=request->prod_nbr_prefix)
     AND mpn.year=yr)
   DETAIL
    seq_nbr_new = mpn.seq_nbr
   WITH nocounter
  ;end select
  SET error_check = error(errmsg,0)
  IF (error_check != 0)
  ...
  ENDIF
  IF (curqual=0)
   SET seq_nbr_new = (request->prod_nbr_starting_nbr - 1)
  ENDIF
  SELECT INTO "nl:"
   y = seq(reference_seq,nextval)
   FROM dual
  ...
  INSERT  FROM bb_mod_pool_nbr mpn
   SET mpn.mod_pool_nbr_id = bb_mod_pool_nbr_id, mpn.option_id = reply->option_id, mpn.prefix =
    ...
    mpn.year = yr, mpn.seq_nbr = seq_nbr_new, mpn.updt_applctx = reqinfo->updt_applctx,
    mpn.updt_task = reqinfo->updt_task, mpn.updt_dt_tm = cnvtdatetime(sysdate), mpn.updt_id = reqinfo
    ->updt_id,
    mpn.updt_cnt = 0
   WITH nocounter
  ;end insert
  ...
  ENDIF
 ENDIF
 IF ((request->generate_isbt_nbr_ind=1)
  AND boptionidfound=1)
  SET yr = year(cnvtdatetime(sysdate))
  SELECT INTO "nl:"
   mpn.option_id, mpn.year
   FROM bb_mod_pool_nbr mpn
   PLAN (mpn
    WHERE mpn.option_id=option_id_new
     AND mpn.year=yr
     AND mpn.organization_id > 0.0)
   HEAD REPORT
    org_cnt = 0
   DETAIL
    org_cnt += 1
    IF (org_cnt > size(isbtpoolseq->org_qual,5))
     stat = alterlist(isbtpoolseq->org_qual,(org_cnt+ 9))
    ENDIF
    isbtpoolseq->org_qual[org_cnt].organization_id = mpn.organization_id, isbtpoolseq->org_qual[
    org_cnt].seq_nbr = mpn.seq_nbr
   FOOT REPORT
    stat = alterlist(isbtpoolseq->org_qual,org_cnt)
   WITH nocounter
  ;end select
  SET error_check = error(errmsg,0)
  IF (error_check != 0)
  ...
  ENDIF
  IF (curqual > 0)
   FOR (cnt = 1 TO org_cnt)
    SELECT INTO "nl:"
     y = seq(reference_seq,nextval)
     FROM dual
     DETAIL
      isbtpoolseq->org_qual[cnt].mod_pool_nbr_id = y
     WITH format, counter
    ;end select
    IF (curqual=0)
     SET errmsg = "Unable to obtain reference sequence id"
    ...
    ENDIF
   ENDFOR
   INSERT  FROM (dummyt d  WITH seq = org_cnt),
     bb_mod_pool_nbr mpn
    SET mpn.mod_pool_nbr_id = isbtpoolseq->org_qual[d.seq].mod_pool_nbr_id, mpn.option_id = reply->
     option_id, mpn.prefix = request->prod_nbr_prefix,
     mpn.year = yr, mpn.seq_nbr = isbtpoolseq->org_qual[d.seq].seq_nbr, mpn.organization_id =
     isbtpoolseq->org_qual[d.seq].organization_id,
    ...
    PLAN (d)
     JOIN (mpn
     WHERE (mpn.mod_pool_nbr_id=isbtpoolseq->org_qual[d.seq].mod_pool_nbr_id))
    WITH nocounter
   ;end insert
   SET error_check = error(errmsg,0)
   IF (error_check != 0)
