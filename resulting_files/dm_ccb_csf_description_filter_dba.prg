CREATE PROGRAM dm_ccb_csf_description_filter:dba
 ...
 SELECT INTO "nl:"
  ...
  cvs.display, cnt = count(cv.description)
  FROM code_value cv,
   code_value_set cvs
  PLAN (cv
  ...
  WITH nocounter, orahintcbo("INDEX(O XIE9CODE_VALUE)")
 ;end select
 SET errcode = error(errmsg,1)
 IF (errcode != 0)
  SET errmsg = concat(
   "Unable to read from CODE_VALUE or CODE_VALUE_SET while searching on description:",errmsg)
  SET status = "F"
  GO TO exit_program
 ENDIF
#exit_program
