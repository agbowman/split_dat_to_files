CREATE PROGRAM dm2_rdds_val_reltn:dba
 ...
 SUBROUTINE dm2_rdds_init_display(null)
   ...
   CALL clear(1,1)
 END ;Subroutine
 DECLARE drvr_source_env_id = f8
 DECLARE drvr_target_env_id = f8
 DECLARE drvr_dblink = vc
 DECLARE drvr_find_reltns = vc
 DECLARE drvr_find_xlats_flg = i2
 DECLARE drvr_find_seqmatch_flg = i2
 DECLARE drvr_table_name = vc
 DECLARE drvr_cleanup_ind = i2
 DECLARE drvr_check_xlats = vc
 DECLARE drvr_continue_flg = i2
 IF (check_logfile("dm2_rdds_val_reltn",".log","DM2_RDDS_VAL_RELTN LOG")=0)
  GO TO exit_drvr
 ENDIF
 SET drvr_source_env_id =  $1
 SET drvr_continue_flg = 1
 SET drvr_target_env_id = 0
 SET drvr_dblink = ""
 SET drvr_find_reltns = ""
 SET drvr_find_xlats_flg = 0
 SET drvr_find_seqmatch_flg = 0
 SET drvr_table_name = ""
 SET drvr_cleanup_ind = 0
 SET drvr_check_xlats = ""
 SET dm_err->eproc = "BEGIN VALIDATION OF THE SOURCE/TARGET RELATIONSHIP"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "GATHERING ENVIRONMENT INFORMATION"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "NL:"
  d.info_number
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="DM_ENV_ID"
  DETAIL
   drvr_target_env_id = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "NL:"
  d.post_link_name
  FROM dm_env_reltn d
  WHERE d.parent_env_id=drvr_source_env_id
   AND d.child_env_id=drvr_target_env_id
   AND d.relationship_type="REFERENCE MERGE"
  DETAIL
   drvr_dblink = d.post_link_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_drvr
 ENDIF
 IF (drvr_continue_flg=1)
  SET drvr_find_reltns = dm2_rdds_chk_rel_val_rows(drvr_source_env_id,drvr_target_env_id,drvr_dblink)
  IF ((dm_err->err_ind=1))
   GO TO exit_drvr
  ELSEIF (drvr_find_reltns="NONE_FOUND")
   SET drvr_check_xlats = dm2_check_xlat_seqmatch(drvr_source_env_id,drvr_target_env_id,drvr_dblink)
   IF ((dm_err->err_ind=1))
    GO TO exit_drvr
   ELSEIF (drvr_check_xlats="XLAT_FOUND")
    CALL dm2_rdds_init_display(null)
    CALL text(05,05,"No validation information exists for this source/target relationship,")
    CALL text(06,05,"however translations and/or sequence match rows exist.")
    CALL text(07,05,"You should contact the Cerner Immediate Answer Center before proceeding.")
    CALL text(08,05,"Please choose one of the following options:")
    CALL text(10,10,
     "1) I have already contacted the IAC. I am certain that existing translations and sequence")
    CALL text(11,12,"   matches for this relationship are valid.")
    CALL text(12,10,
     "2) I have already contacted the IAC. I need to delete existing translations and sequence")
    CALL text(13,12,"   matches for this relationship.")
    CALL text(14,10,"3) Quit.")
    CALL text(15,10,"ENTER CHOICE:")
    CALL accept(15,30,"9"
     WHERE curaccept IN (1, 2, 3))
    SET message = nowindow
    IF (curaccept=1)
     EXECUTE dm2_rdds_write_rel_val_rows value(drvr_source_env_id), value(drvr_target_env_id), value(
      drvr_dblink)
     IF ((dm_err->err_ind=1))
      GO TO exit_drvr
     ENDIF
     SET dm_err->eproc = "USER CONFIRMATION:NO REFRESH OCCURRED, RELTN ROWS WRITTEN"
     CALL disp_msg(" ",dm_err->logfile,0)
    ELSEIF (curaccept=2)
     SET dm_err->eproc = "USER CONFIRMATION:REFRESH OCCURRED. MUST DELETE TRANSLATIONS/SEQMATCH"
     CALL disp_msg(" ",dm_err->logfile,0)
     CALL dm2_rdds_init_display(null)
     CALL text(05,05,
      "Translations and sequence matches for the source/target you specified will be permanently")
     CALL text(06,05,"removed from the source and target databases. Do you want to continue? (Y/N)")
     CALL text(07,05,"databases. Do you want to continue? (Y/N)")
     CALL text(08,05,"ENTER CHOICE:")
     CALL accept(08,30,"P;CU"
      WHERE curaccept IN ("Y", "N"))
     SET message = nowindow
     IF (curaccept="Y")
      SET dm_err->eproc = "USER CONFIRMATION: CLEAN UP TRANSLATIONS/SEQMATCH"
      CALL disp_msg(" ",dm_err->logfile,0)
      SET drvr_cleanup_ind = 1
     ELSEIF (curaccept="N")
      CALL clear(1,1)
    ...
    ELSEIF (curaccept=3)
     ...
     GO TO exit_drvr
    ENDIF
   ELSEIF (drvr_check_xlats="NONE_FOUND")
    EXECUTE dm2_rdds_write_rel_val_rows value(drvr_source_env_id), value(drvr_target_env_id), value(
     drvr_dblink)
    IF ((dm_err->err_ind=1))
     GO TO exit_drvr
    ENDIF
   ENDIF
  ELSEIF (((drvr_find_reltns="SRC_MISSING") OR (drvr_find_reltns="TGT_MISSING")) )
   SET drvr_check_xlats = dm2_check_xlat_seqmatch(drvr_source_env_id,drvr_target_env_id,drvr_dblink)
   IF ((dm_err->err_ind=1))
    GO TO exit_drvr
   ELSEIF (drvr_check_xlats="XLAT_FOUND")
    CALL dm2_rdds_init_display(null)
    CALL text(05,05,
     "It appears that one of the databases in this source/target relationship has been refreshed.")
    CALL text(06,05,
     "Existing translations and sequence match rows for a given relationship become invalid after")
    CALL text(07,05,
     "a database refresh and must be removed. If you believe you have received this message in error,"
     )
    CALL text(08,05,
     "please contact Cerner's Immediate Answer Center. Please choose one of the following options:")
    CALL text(10,10,"1) Remove Existing translations and sequence matches for this relationship")
    CALL text(11,10,"2) Quit")
    CALL text(12,10,"ENTER CHOICE:")
    CALL accept(12,30,"9"
    ...
    IF (curaccept=2)
     ...
     GO TO exit_drvr
    ELSEIF (curaccept=1)
     SET dm_err->eproc = "USER CONFIRMATION: REMOVE EXISTING TRANSLATIONS/SEQMATCH"
     CALL disp_msg(" ",dm_err->logfile,0)
     CALL dm2_rdds_init_display(null)
     CALL text(05,05,
      "Translations and sequence matches for the source/target you specified will be permanently")
     CALL text(06,05,"removed from the source and target databases. Do you want to continue? (Y/N)")
     CALL text(07,05,"ENTER CHOICE")
     CALL accept(07,30,"P;CU"
      WHERE curaccept IN ("Y", "N"))
     SET message = nowindow
     IF (curaccept="Y")
      SET dm_err->eproc = "USER CONFIRMATION: REMOVE EXISTING TRANSLATIONS/SEQMATCH"
   ...
   ELSEIF (drvr_check_xlats="NONE_FOUND")
    ...
    ENDIF
   ENDIF
  ELSEIF (drvr_find_reltns="MISMATCH_FOUND")
   CALL dm2_rdds_init_display(null)
   CALL text(05,05,"Results of src/tgt relationship validation require that you contact ")
   CALL text(06,05,"Cerner's Immediate Answer Center")
   CALL accept(06,50,"P;CU","Y")
   SET message = nowindow
   SET dm_err->eproc =
   "Results of src/tgt relationship validation require that you contact Cerner's Immediate Answer Center"
   CALL disp_msg(" ",dm_err->logfile,1)
   SET dm_err->err_ind = 1
   GO TO exit_drvr
  ENDIF
  IF (drvr_cleanup_ind=1)
   SET dm_err->eproc = "CLEANING UP TRANSLATIONS LOCALLY"
   CALL disp_msg(" ",dm_err->logfile,0)
   EXECUTE dm2_rdds_cleanup_xlat value(drvr_source_env_id), value(drvr_target_env_id), "DM2NOLINK"
   IF ((dm_err->err_ind=1))
    GO TO exit_drvr
   ENDIF
   SET dm_err->eproc = "CLEANING UP TRANSLATIONS IN SOURCE"
   CALL disp_msg(" ",dm_err->logfile,0)
   EXECUTE dm2_rdds_cleanup_xlat value(drvr_target_env_id), value(drvr_source_env_id), value(
    drvr_dblink)
   IF ((dm_err->err_ind=1))
    GO TO exit_drvr
   ENDIF
   ...
   CALL dm2_rdds_init_display(null)
   CALL text(05,05,
    "Cleanup process complete.  If you are not already in the process of running Sequence Match,")
   CALL text(06,05,"please do so now via dm_merge_domain_adm.")
   CALL accept(06,50,"P;CU","Y")
   SET message = nowindow
  ENDIF
 ENDIF
#exit_drvr
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Exiting Relationship Validation: RDDS SOURCE/TARGET RELATIONSHIP VALIDATED"
 ENDIF
 CALL final_disp_msg("dm2_rdds_val_reltn")
END GO
