CREATE PROGRAM ecf_rdm_cpt_inactive_bill:dba
 ...
 ENDIF
 SET kia_rdm_log_name = "ecf_rdm_cpt_inactive_bill.log"
 SET kia_rdm_nbr = 11755
 SET readme_data->status = "F"
 SET readme_data->message = concat("Failed - ","Updating the bill_item_modifier table")
 SET script_status = " "
 DECLARE rdm_errmsg = vc WITH protect, noconstant("")
 CALL kia_inhouse_chk(1)
 CALL kia_db2_check(1)
 DECLARE asterisk = vc
 SET asterisk = fillstring(130,"*")
 IF (validate(bill_item_debug)=0)
  DECLARE bill_item_debug = i2 WITH noconstant(0)
 ENDIF
 IF (bill_item_debug=1)
  SET trace = echoprog
  SET trace = echosub
 ...
 ENDIF
 FREE RECORD inactivenomenupdate
 RECORD inactivenomenupdate(
   1 nomencnt = i4
   1 bill_item_mod[*]
     2 bill_item_id = f8
     ...
     2 old_nomen_id = f8
     2 new_nomen_id = f8
     2 old_bim_beg_date = dq8
     2 new_bim_beg_date = dq8
     2 prior_bill_item_mod_id = f8
     2 prior_old_bim_end_date = dq8
 )
 SELECT INTO "nl:"
  ...
  PLAN (n
   ...
   (SELECT
    ...
    FROM code_value
    WHERE code_set=400
     AND cdf_meaning IN ("CPT4", "HCPCS")
     AND active_ind=1))
    AND n.principle_type_cd IN (
   (SELECT
    code_value
    FROM code_value
    WHERE code_set=401
     AND cdf_meaning="PROCEDURE"
     AND active_ind=1))
    AND n.active_ind=false
    AND n.concept_cki != null)
   JOIN (n2
   WHERE n2.source_identifier=n.source_identifier
    AND n2.nomenclature_id != n.nomenclature_id
    AND n2.source_vocabulary_cd=n.source_vocabulary_cd
    AND n2.principle_type_cd=n.principle_type_cd
    AND n2.primary_vterm_ind=true
    AND n2.end_effective_dt_tm > cnvtdatetime(sysdate)
    AND n2.active_ind=true
    AND n2.concept_cki != null
    AND n2.beg_effective_dt_tm != n2.end_effective_dt_tm
    AND  NOT ( EXISTS (
   (SELECT
    n3.nomenclature_id
    FROM nomenclature n3
    WHERE n3.source_identifier=n2.source_identifier
     AND n3.nomenclature_id != n2.nomenclature_id
     AND n3.source_vocabulary_cd=n2.source_vocabulary_cd
     AND n3.principle_type_cd=n2.principle_type_cd
     AND n3.primary_vterm_ind=true
     AND n3.active_ind=true
     AND n3.concept_cki != null
     AND n3.beg_effective_dt_tm > n2.end_effective_dt_tm))))
   JOIN (bim
   WHERE bim.key3_id=n.nomenclature_id
    AND bim.bill_item_type_cd IN (
   (SELECT
    code_value
    FROM code_value
    WHERE code_set=13019
     AND cdf_meaning="BILL CODE"
     AND active_ind=1))
   ...
   (SELECT
    ...
    WHERE bim2.bill_item_id=bim.bill_item_id
     AND bim2.bill_item_type_cd=bim.bill_item_type_cd
     AND bim2.beg_effective_dt_tm > bim.end_effective_dt_tm
     AND bim2.key3_id=n2.nomenclature_id
     AND bim2.key6=bim.key6
     AND bim2.key1_id=bim.key1_id
     AND bim2.active_ind=true))))
   JOIN (bim2
   WHERE bim2.bill_item_id=bim.bill_item_id
    AND bim2.bill_item_type_cd=bim.bill_item_type_cd
    AND bim2.end_effective_dt_tm < bim.beg_effective_dt_tm
    AND bim2.key6=bim.key6
    AND bim2.key1_id=bim.key1_id
    AND bim2.active_ind=true)
   JOIN (d
   WHERE bim2.end_effective_dt_tm BETWEEN cnvtlookbehind("1,D",bim.beg_effective_dt_tm) AND bim
   .beg_effective_dt_tm)
  ORDER BY bim.bill_item_mod_id
  HEAD bim.bill_item_mod_id
   inactivenomenupdate->nomencnt = (inactivenomenupdate->nomencnt+ 1)
   IF ((inactivenomenupdate->nomencnt > size(inactivenomenupdate->bill_item_mod,5)))
    stat = alterlist(inactivenomenupdate->bill_item_mod,(inactivenomenupdate->nomencnt+ 100))
   ENDIF
   inactivenomenupdate->bill_item_mod[inactivenomenupdate->nomencnt].bill_item_id = bim.bill_item_id,
   inactivenomenupdate->bill_item_mod[inactivenomenupdate->nomencnt].bill_item_mod_id = bim
   .bill_item_mod_id, inactivenomenupdate->bill_item_mod[inactivenomenupdate->nomencnt].new_nomen_id
    = n2.nomenclature_id,
   inactivenomenupdate->bill_item_mod[inactivenomenupdate->nomencnt].old_nomen_id = n.nomenclature_id,
   inactivenomenupdate->bill_item_mod[inactivenomenupdate->nomencnt].old_bim_beg_date = bim
   .beg_effective_dt_tm, inactivenomenupdate->bill_item_mod[inactivenomenupdate->nomencnt].
   new_bim_beg_date = n2.beg_effective_dt_tm,
   inactivenomenupdate->bill_item_mod[inactivenomenupdate->nomencnt].prior_bill_item_mod_id = bim2
   .bill_item_mod_id, inactivenomenupdate->bill_item_mod[inactivenomenupdate->nomencnt].
   prior_old_bim_end_date = bim2.end_effective_dt_tm
  FOOT REPORT
   stat = alterlist(inactivenomenupdate->bill_item_mod,inactivenomenupdate->nomencnt)
  WITH nocounter
 ;end select
 IF (error(rdm_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed in retrieve the content",rdm_errmsg)
  GO TO exit_script
 ENDIF
 IF ((inactivenomenupdate->nomencnt > 0))
  UPDATE  FROM bill_item_modifier bi,
    (dummyt d  WITH seq = value(inactivenomenupdate->nomencnt))
   SET bi.beg_effective_dt_tm = cnvtdatetime(inactivenomenupdate->bill_item_mod[d.seq].
     new_bim_beg_date), bi.key3_id = inactivenomenupdate->bill_item_mod[d.seq].new_nomen_id, bi
    .updt_cnt = (bi.updt_cnt+ 1),
    bi.updt_dt_tm = cnvtdatetime(curdate,curtime3), bi.updt_id = reqinfo->updt_id, bi.updt_applctx =
    10012022
   PLAN (d)
    JOIN (bi
    WHERE (bi.bill_item_mod_id=inactivenomenupdate->bill_item_mod[d.seq].bill_item_mod_id)
     AND bi.bill_item_mod_id != 0)
   WITH nocounter
  ;end update
  IF (error(rdm_errmsg,0) != 0)
   SET readme_data->status = "F"
   SET readme_data->message = concat("Failed updating the end effected nomenclature Id's: ",
    rdm_errmsg)
   ROLLBACK
   GO TO exit_script
  ENDIF
  ...
  UPDATE  FROM bill_item_modifier bi,
    (dummyt d  WITH seq = value(inactivenomenupdate->nomencnt))
   SET bi.end_effective_dt_tm = cnvtlookbehind("1,S",inactivenomenupdate->bill_item_mod[
     inactivenomenupdate->nomencnt].new_bim_beg_date), bi.updt_cnt = (bi.updt_cnt+ 1), bi.updt_dt_tm
     = cnvtdatetime(curdate,curtime3),
    bi.updt_id = reqinfo->updt_id, bi.updt_applctx = 10012022
   PLAN (d)
    JOIN (bi
    WHERE (bi.bill_item_mod_id=inactivenomenupdate->bill_item_mod[d.seq].prior_bill_item_mod_id)
     AND bi.bill_item_mod_id != 0)
   WITH nocounter
  ;end update
  IF (error(rdm_errmsg,0) != 0)
 ...
 IF (bill_item_debug=1)
  ...
  SET trace = noechosub
  SET trace = nordbbind
  SET trace = nordbbind2
  SET trace = nordbdebug
  SET trace = nordbdebug1
  SET trace = nordbdebug2
  SET trace = nocostall
