CREATE PROGRAM bhs_athn_refuse_request_v2
 RECORD arequest(
   1 lst[*]
 ...
 DECLARE refused_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",103,"REFUSED"))
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (((cep.action_type_cd=review_cd) OR (cep.action_type_cd=sign_cd))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ta
   WHERE (ta.event_id=t_record->event_id)
    AND ta.task_activity_cd IN (reviewresult_cd, signresult_cd)
    AND ta.active_ind=1)
   JOIN (taa
 ...
 SELECT INTO "nl:"
  ...
  HEAD cep.ce_event_prsnl_id
   ...
   refused_cd, arequest->lst[1].action_comment = trim(comment_text,3),
