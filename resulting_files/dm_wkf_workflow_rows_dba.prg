CREATE PROGRAM dm_wkf_workflow_rows:dba
 ...
 SET reply->table_name = "WKF_WORKFLOW"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(ww.wkf_workflow_id)
    FROM wkf_workflow ww
    WHERE ww.wkf_workflow_id != 0
     AND ww.updt_dt_tm < cnvtdatetime((curdate - idaystokeep),curtime3)
    DETAIL
  ...
  SET stat = alterlist(reply->rows,request->max_rows)
  WHILE (dcurminid <= dmaxid
   AND irowsleft > 0)
    SELECT INTO "nl:"
     ww.rowid
     FROM wkf_workflow ww
     WHERE parser(sbr_getrowidnotexists("ww.wkf_workflow_id between dCurMinID and dCurMaxID","ww"))
     DETAIL
      irowcnt += 1
      IF ((irowcnt > request->max_rows)
       AND mod(irowcnt,100)=1)
       stat = alterlist(reply->rows,(irowcnt+ 99))
      ENDIF
      reply->rows[irowcnt].row_id = ww.rowid
     WITH nocounter, maxqual(ww,value(irowsleft))
    ;end select
    SET ierrcode2 = 0
