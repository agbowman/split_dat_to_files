CREATE PROGRAM bhs_phys_missing_spi:dba
 PROMPT
 ...
 SET spicd = uar_get_code_by("display",263,"Surescripts Prescriber ID")
 RECORD md_alias(
   1 qual[*]
     2 name = vc
     2 stat_cd = f8
 ...
 SELECT INTO "nl:"
  ...
  PLAN (pr
   ...
   pr.position_cd=psychmd) OR (((pr.position_cd=pulmd) OR (((pr.position_cd=renmd) OR (((pr
   .position_cd=thormd) OR (pr.position_cd=urolmd)) )) )) )) )) )) )) )) )) )) )) )) )) )) )) )) ))
   )) )) )) )) )) )) )) )) ))
    AND ((pr.username="EN*") OR (((pr.username="PN*") OR (pr.username="CN*")) ))
    AND pr.username != "*EN00796*"
    AND pr.username != "*EN43613*"
    AND pr.username != "*EN45342*"
    AND pr.username != "*EN43887*"
    AND pr.username != "*EN06028*"
    AND pr.username != "*EN45033*"
    AND pr.username != "*EN07393*"
    AND pr.username != "*EN44469*"
    AND pr.username != "*EN03941*"
    AND pr.username != "*EN40101*"
    AND pr.username != "*EN46404*"
    AND pr.username != "*EN00793*"
    AND pr.username != "*EN45729*"
    AND pr.username != "*EN43202*"
    AND pr.username != "*EN03418*")
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   IF (pa.prsnl_alias_type_cd=115572481
    AND pa.active_ind=1)
    md_alias->qual[d.seq].spi_nbr = concat("SPI-",pa.alias)
   ENDIF
 ...
 SELECT INTO value(output_dest)
  ...
  HEAD REPORT
   ...
   FOR (y = 1 TO size(md_alias->qual[d.seq],5))
     IF ((md_alias->qual[y].spi_nbr="")
      AND (md_alias->qual[y].npi_nbr > ""))
      xperson_id = format(md_alias->qual[y].personid,"#########"), output_string = build(',"',
       xperson_id,'","',md_alias->qual[y].stat_descr,'","',
       md_alias->qual[y].name,'","',md_alias->qual[y].username,'","',md_alias->qual[y].position,
       '","',format(md_alias->qual[y].demog_updt,"yyyy-mm-dd;;d"),'","',md_alias->qual[y].dea_nbr,
       '","',
       md_alias->qual[y].npi_nbr,'","',md_alias->qual[y].spi_nbr,'","',md_alias->qual[y].org_id,
       '","',md_alias->qual[y].ext_id,'","',md_alias->qual[y].doc_upin,'","',
       md_alias->qual[y].license_nbr,'",'), col 1,
      output_string
      IF ( NOT (curendreport))
       row + 1
      ENDIF
   ...
   col 1, output_string
  WITH format = variable, formfeed = none, maxcol = 400
 ;end select
 IF (email_ind=1)
  SET filename_in = trim(concat(output_dest,".dat"))
  SET filename_out = concat(format(curdate,"YYYY-MM-DD;;D"),"_PHYS_missing_SPI.csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog,"-V1.x - PHYS missing SPI ",curnode)
