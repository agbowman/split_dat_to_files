CREATE PROGRAM dm_pft_sys_activity_log_rows:dba
 ...
 SET reply->table_name = "PFT_SYSTEM_ACTIVITY_LOG"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(psal.pft_system_activity_log_id)
   FROM pft_system_activity_log psal
   WHERE psal.pft_system_activity_log_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(psal.pft_system_activity_log_id)
  FROM pft_system_activity_log psal
  WHERE psal.pft_system_activity_log_id > 0.0
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM pft_system_activity_log psal
    WHERE psal.pft_system_activity_log_id BETWEEN curminid AND curmaxid
     AND psal.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND parser(sbr_getrowidnotexists("psal.pft_system_activity_log_id != 0","psal"))
    DETAIL
     row_cnt += 1
     IF (mod(row_cnt,100)=1)
      stat = alterlist(reply->rows,(row_cnt+ 99))
     ENDIF
     reply->rows[row_cnt].row_id = psal.rowid
    WITH nocounter, maxqual(psal,value(rowsleft))
   ;end select
