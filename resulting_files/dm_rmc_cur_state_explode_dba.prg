CREATE PROGRAM dm_rmc_cur_state_explode:dba
 ...
 DECLARE drcse_tbl_pos = i4 WITH protect, noconstant(0)
 DECLARE drcse_tab_name = vc WITH protect, noconstant("")
 DECLARE drcse_num = i4 WITH protect, noconstant(0)
 DECLARE drcse_pk_where = vc WITH protect, noconstant("")
 DECLARE drcse_pkw = vc WITH protect, noconstant("")
 DECLARE drcse_col_loop = i4 WITH protect, noconstant(0)
 DECLARE drcse_temp = i4 WITH protect, noconstant(0)
 DECLARE drcse_table_suffix = vc WITH protect, noconstant("")
 DECLARE drcse_total = i4 WITH protect, noconstant(0)
 DECLARE drcse_row_type = vc WITH protect, noconstant("")
 DECLARE drcse_par_name = vc WITH protect, noconstant("")
 DECLARE drcse_par_pos = i4 WITH protect, noconstant(0)
 DECLARE drcse_temp_pkw = vc WITH protect, noconstant("")
 DECLARE drcse_loop = i4 WITH protect, noconstant(0)
 DECLARE drcse_par_suffix = vc WITH protect, noconstant("")
 DECLARE drcse_new_func_ind = i2 WITH protect, noconstant(0)
 SET drcse_tbl_pos = request->tbl_pos
 SET drcse_par_pos = request->top_level_pos
 SET drcse_par_name = dm2_ref_data_doc->tbl_qual[drcse_par_pos].table_name
 IF ((request->source_ind=1))
  SET drcse_tab_name = dm2_get_rdds_tname(request->table_name)
  SET drcse_pk_where = replace(request->pk_where,"<MERGE_LINK>",dm2_ref_data_doc->post_link_name,0)
 ELSE
  SET drcse_tab_name = request->table_name
  SET drcse_pk_where = replace(request->pk_where,"<MERGE_LINK>","",0)
 ENDIF
 SET drcse_table_suffix = concat("t",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].suffix)
 SET drcse_par_suffix = concat("t",dm2_ref_data_doc->tbl_qual[drcse_par_pos].suffix)
 SET dm_err->eproc = "Beginning dm_rmc_cur_state_explode."
 FREE RECORD cust_cs_rows
 CALL parser("record cust_cs_rows (",0)
 CALL parser("1 trailing_space_ind = i2",0)
 CALL parser(" 1 qual[*]",0)
 IF ((dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].merge_delete_ind=1))
  SET drcse_row_type = "MD"
  FOR (drcse_col_loop = 1 TO dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_cnt)
    IF ((dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].merge_delete_ind=1))
     CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].
       column_name," = ",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].data_type
       ),0)
     CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].
       column_name,"_NULLIND = i2 "),0)
     IF ((dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].data_type="VC"))
      CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].
        column_name,"_len = i4 "),0)
     ENDIF
     SET drcse_cols->total = (drcse_cols->total+ 1)
     SET stat = alterlist(drcse_cols->col_qual,drcse_cols->total)
     SET drcse_cols->col_qual[drcse_cols->total].column_name = dm2_ref_data_doc->tbl_qual[
     drcse_tbl_pos].col_qual[drcse_col_loop].column_name
     SET drcse_cols->col_qual[drcse_cols->total].data_type = dm2_ref_data_doc->tbl_qual[drcse_tbl_pos
     ].col_qual[drcse_col_loop].data_type
    ENDIF
  ENDFOR
 ELSEIF ((dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].version_ind=1))
  SET drcse_row_type = "UI"
  FOR (drcse_col_loop = 1 TO dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_cnt)
    IF ((dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].unique_ident_ind=1))
     CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].
       column_name," = ",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].data_type
       ),0)
     CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].
  ...
  ENDFOR
 ELSE
  SET drcse_row_type = "PK"
  FOR (drcse_col_loop = 1 TO dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_cnt)
    IF ((dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].pk_ind=1))
     CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].
       column_name," = ",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].data_type
       ),0)
     CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[drcse_tbl_pos].col_qual[drcse_col_loop].
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Populating cust_cs_rows rs."
 IF ((((dm2_ref_data_doc->tbl_qual[drcse_par_pos].version_ind=1)) OR (drcse_par_name IN (
 "DCP_SECTION_REF", "DCP_FORMS_REF"))) )
  SET drcse_loop = 1
 ELSE
  SET drcse_loop = 3
 ENDIF
 SET drcse_temp_pkw = drcse_pk_where
 WHILE (drcse_loop <= 3
  AND size(cust_cs_rows->qual,5)=0)
   IF (drcse_loop=1)
    SET drcse_pk_where = replace(drcse_temp_pkw,"<VERS_CLAUSE>"," ACTIVE_IND = 1 ",0)
   ELSEIF (drcse_loop=2)
    IF ((dm2_ref_data_doc->tbl_qual[drcse_par_pos].effective_col_ind=1))
     SET drcse_pk_where = replace(drcse_temp_pkw,"<VERS_CLAUSE>",concat(drcse_par_suffix,".",
       dm2_ref_data_doc->tbl_qual[drcse_par_pos].beg_col_name,
       "<= cnvtdatetime(curdate,curtime3) and ",drcse_par_suffix,
       ".",dm2_ref_data_doc->tbl_qual[drcse_par_pos].end_col_name,
       ">= cnvtdatetime(curdate,curtime3) "),0)
    ELSE
     SET drcse_loop = 3
     SET drcse_pk_where = replace(drcse_temp_pkw,"<VERS_CLAUSE>"," 1 = 1 ",0)
    ENDIF
   ELSE
    SET drcse_pk_where = replace(drcse_temp_pkw,"<VERS_CLAUSE>"," 1 = 1 ",0)
   ENDIF
   CALL parser("select distinct into 'nl:'",0)
   FOR (drcse_col_loop = 1 TO drcse_cols->total)
     IF (drcse_col_loop > 1)
      CALL parser(" , ",0)
     ENDIF
     CALL parser(concat(drcse_table_suffix,".",drcse_cols->col_qual[drcse_col_loop].column_name))
     CALL parser(concat(", var",cnvtstring(drcse_col_loop)," = nullind(",drcse_table_suffix,".",
       drcse_cols->col_qual[drcse_col_loop].column_name,")"),0)
     IF ((drcse_cols->col_qual[drcse_col_loop].data_type="VC"))
      CALL parser(concat(", len",cnvtstring(drcse_col_loop)," = length(",drcse_table_suffix,".",
        drcse_cols->col_qual[drcse_col_loop].column_name,")"),0)
     ENDIF
   ENDFOR
   CALL parser(concat(" from  ",drcse_tab_name," ",drcse_table_suffix),0)
   CALL parser(drcse_pk_where,0)
   CALL parser(
    " detail drcse_total = drcse_total + 1 stat = alterlist(cust_cs_rows->qual, drcse_total) ",0)
   FOR (drcse_col_loop = 1 TO drcse_cols->total)
     CALL parser(concat("cust_cs_rows->qual[drcse_total].",drcse_cols->col_qual[drcse_col_loop].
       column_name," = ",drcse_table_suffix,".",
       drcse_cols->col_qual[drcse_col_loop].column_name),0)
     CALL parser(concat("cust_cs_rows->qual[drcse_total].",drcse_cols->col_qual[drcse_col_loop].
       column_name,"_NULLIND = var",trim(cnvtstring(drcse_col_loop))),0)
     IF ((drcse_cols->col_qual[drcse_col_loop].data_type="VC"))
      CALL parser("cust_cs_rows->trailing_space_ind = 1")
      CALL parser(concat("cust_cs_rows->qual[drcse_total].",drcse_cols->col_qual[drcse_col_loop].
        column_name,"_len = len",trim(cnvtstring(drcse_col_loop))),0)
     ENDIF
   ENDFOR
   CALL parser(" with nocounter,notrim go",1)
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   SET drcse_loop = (drcse_loop+ 1)
 ENDWHILE
 SET stat = alterlist(reply->qual,size(cust_cs_rows->qual,5))
 SELECT INTO "NL:"
  FROM dm_pk_where d
  WHERE (d.table_name=request->table_name)
   AND d.delete_ind=0
   AND findstring("i_cs_pk_ind",d.pk_where) > 0
  DETAIL
   drcse_new_func_ind = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  GO TO exit_program
 ENDIF
 FOR (drcse_temp = 1 TO size(cust_cs_rows->qual,5))
   SET drcse_pkw = drdm_create_pk_where(drcse_temp,drcse_tbl_pos,drcse_row_type)
   IF (drcse_pkw="")
    CALL echo("An error occurred during drdm_create_pk_where.")
    SET reply->status = "F"
    GO TO exit_program
   ENDIF
   IF (drcse_new_func_ind=1)
    CALL drmm_get_col_string(drcse_pkw,0.0,request->table_name,dm2_ref_data_doc->tbl_qual[
     drcse_tbl_pos].suffix,dm2_ref_data_doc->post_link_name)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET reply->status = "F"
     GO TO exit_program
    ENDIF
    SET drcse_pkw = drmm_get_exploded_pkw(request->table_name,dm2_ref_data_doc->tbl_qual[
     drcse_tbl_pos].suffix,dm2_ref_data_doc->post_link_name)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET reply->status = "F"
    ...
    ENDIF
   ENDIF
   SET reply->total = (reply->total+ 1)
   SET reply->qual[reply->total].pk_where = drcse_pkw
 ENDFOR
 SET reply->status = "S"
#exit_program
END GO
