CREATE PROGRAM bb_rdm_chg_seqs:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bb_rdm_chg_seqs"
 CALL logscriptstart(sreadme_name)
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: Starting bb_rdm_chg_seqs.prg script"
 FREE RECORD mobile_prefs
 RECORD mobile_prefs(
   1 list[*]
     2 mpref_id = f8
     2 new_id = f8
 ...
 DECLARE info_name = c18 WITH constant("MOBILE_PREF_SEQ")
 DECLARE pathnet_bbt = c11 WITH constant("PATHNET_BBT")
 DECLARE messagelog = c132 WITH public, noconstant("")
 DECLARE loopsize = i4 WITH protect, noconstant(50)
 DECLARE last = i4 WITH protect, noconstant(0)
 DECLARE done = i2 WITH protect, noconstant(0)
 DECLARE first = i4 WITH protect, noconstant(0)
 DECLARE success_ind = i2 WITH protect, noconstant(0)
 DECLARE last_seq = f8 WITH protect, noconstant(0.0)
 SELECT INTO "nl:"
  FROM dm_info dm
 ...
 IF (curqual=0)
  SELECT INTO "nl:"
   m.mobile_pref_id, seqn = seq(pathnet_seq,nextval)
   FROM bbd_mobile_pref m
   WHERE m.mobile_pref_id != 0.00
   HEAD REPORT
    count = 0
   DETAIL
    count = (count+ 1)
    IF (mod(count,10)=1)
     stat = alterlist(mobile_prefs->list,(count+ 9))
    ENDIF
    mobile_prefs->list[count].mpref_id = m.mobile_pref_id, mobile_prefs->list[count].new_id = seqn
   FOOT REPORT
    stat = alterlist(mobile_prefs->list,count), last_seq = max(seqn)
   WITH nocounter
  ;end select
  IF (count=0)
  ...
  ELSE
   WHILE (count > 0)
     SET last = least(count,loopsize)
     SELECT INTO "nl:"
      mp.mobile_pref_id
      FROM bbd_mobile_pref mp,
       (dummyt d  WITH seq = last)
      PLAN (d)
       JOIN (mp
       WHERE (mp.mobile_pref_id=mobile_prefs->list[(first+ d.seq)].mpref_id))
      WITH nocounter, forupdate(mp)
     ;end select
     UPDATE  FROM bbd_mobile_pref mp,
       (dummyt d  WITH seq = last)
      SET mp.mobile_pref_id = mobile_prefs->list[(first+ d.seq)].new_id
      PLAN (d)
       JOIN (mp
       WHERE (mp.mobile_pref_id=mobile_prefs->list[(first+ d.seq)].mpref_id))
      WITH nocounter
     ...
     IF (curqual=0)
      ...
      CALL logstatus(build(sreadme_name,
        " Execution Failed.  No rows found for update on BBD_MOBILE_PREF."),"F")
      SET readme_data->status = "F"
      SET readme_data->message = "No records found for update."
      GO TO exit_script
     ELSE
      COMMIT
      CALL logstatus(build(sreadme_name,"Update Successful."),"S")
     ENDIF
     SET first = (first+ loopsize)
     SET count = (count - loopsize)
   ENDWHILE
   SELECT INTO "nl:"
    m.mobile_pref_id
    FROM bbd_mobile_pref m
    WHERE m.mobile_pref_id != 0.0
    HEAD REPORT
     max_seq = 0
    DETAIL
     row + 0
    FOOT REPORT
     max_seq = max(m.mobile_pref_id)
     IF (max_seq=last_seq)
      success_ind = 1
     ENDIF
    WITH nocounter
   ...
   IF (success_ind=0)
    ROLLBACK
    CALL logstatus(build(sreadme_name," Execution Failed.  Unable to update BBD_MOBILE_PREF."),"F")
    SET readme_data->status = "F"
    SET readme_data->message = "BBD_MOBILE_PREF mobile_pref_id not updated correctly."
    GO TO exit_script
   ELSE
    INSERT  FROM dm_info di
     SET di.info_number = 0, di.info_name = info_name, di.info_domain = pathnet_bbt,
    ...
    ELSE
     ...
     SET readme_data->status = "F"
     SET readme_data->message = "DM_INFO table update failed."
    ENDIF
   ENDIF
  ENDIF
 ELSE
  SET messagelog = "Readme has already been run"
  CALL logstatus(build(sreadme_name,"--Readme has already been run"),"S")
  CALL logmsg(messagelog,(e_dispmsg+ e_logtofile))
  SET readme_data->status = "S"
  SET readme_data->message = "Readme has already been run."
 ENDIF
#exit_script
 EXECUTE dm_readme_status
 CALL echorecord(readme_data)
