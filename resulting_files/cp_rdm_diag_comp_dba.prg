CREATE PROGRAM cp_rdm_diag_comp:dba
 ...
 ENDIF
 IF (validate(comp_detail_rec) != 1)
  RECORD comp_detail_rec(
    1 comp_details[*]
      2 component_text = vc
 ...
 ENDIF
 DECLARE getcpdiagramssettings(null) = null
 DECLARE createstddiagcomp(null) = null
 DECLARE setstddiagdetails(null) = null
 DECLARE deletecpdiagramcomp(null) = null
 DECLARE refimagecd = f8 WITH constant(uar_get_code_by("MEANING",4003134,"REFIMAGE")), protect
 DECLARE cpdiagcd = f8 WITH constant(uar_get_code_by("MEANING",4003130,"DIAGRAM")), protect
 DECLARE diagreportmean = vc WITH constant("DIAGRAM"), protect
 SET readme_data->status = "F"
 SET readme_data->message = "Readme failure: Starting Script cp_rdm_diag_comp."
 CALL getcpdiagramssettings(null)
 CALL createstddiagcomp(null)
 CALL setstddiagdetails(null)
 CALL deletecpdiagramcomp(null)
 SUBROUTINE getcpdiagramssettings(null)
   CALL log_message("Begin getCPDiagramsSettings()",log_level_debug)
   ...
   SELECT INTO "nl:"
    FROM cp_component_detail ccd,
     ...
     cp_pathway cp
    PLAN (ccd
     WHERE ccd.component_detail_reltn_cd=refimagecd
      AND ccd.component_text > "")
     JOIN (cc
     WHERE cc.cp_component_id=ccd.cp_component_id)
     JOIN (cn
    ...
    DETAIL
     idx += 1, stat = alterlist(comp_detail_rec->comp_details,idx), comp_detail_rec->comp_details[idx
     ].component_text = ccd.component_text,
     comp_detail_rec->comp_details[idx].cp_pathway_id = cp.cp_pathway_id, comp_detail_rec->
     comp_details[idx].cp_node_id = cn.cp_node_id, comp_detail_rec->comp_details[idx].category_mean
      = cn.category_mean,
     comp_detail_rec->comp_details[idx].cp_component_id = cc.cp_component_id, comp_detail_rec->
     comp_details[idx].cp_component_detail_id = ccd.cp_component_detail_id
    WITH nocounter
   ;end select
   IF (size(comp_detail_rec->comp_details,5) < 1)
    SET readme_data->status = "F"
    SET readme_data->message = "Failed. No Qualifications."
    GO TO exit_script
   ENDIF
   IF (validate(debug_ind,0)=1)
    CALL echorecord(comp_detail_rec)
   ENDIF
   CALL log_message(build("Exit getCPDiagramsSettings(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE createstddiagcomp(null)
   CALL log_message("Begin createStdDiagComp()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE i = i4 WITH noconstant(1), protect
   FOR (i = 1 TO size(comp_detail_rec->comp_details,5))
     EXECUTE cp_br_add_component_json "MINE", comp_detail_rec->comp_details[i].cp_node_id,
     "{'COMPONENT_JSON': {'COMPONENT_TYPE_CODE_LIST': [{'TYPE_CODE':'STDDIAG'}]}}",
     ""
   ENDFOR
   CALL log_message(build("Exit createStdDiagComp(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE setstddiagdetails(null)
   CALL log_message("Begin setStdDiagDetails()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE j = i4 WITH noconstant(1), protect
   RECORD brvalue_request(
   ...
   FOR (j = 1 TO size(comp_detail_rec->comp_details,5))
     SET stat = initrec(brvalue_request)
     SELECT INTO "nl:"
      FROM br_datamart_category bdc,
       br_datamart_filter bdf
      PLAN (bdc
       WHERE (bdc.category_mean=comp_detail_rec->comp_details[j].category_mean))
       JOIN (bdf
       WHERE bdf.br_datamart_category_id=bdc.br_datamart_category_id
        AND bdf.filter_mean="WF_DIAGRAM_URL")
      DETAIL
       brvalue_request->br_datamart_category_id = bdf.br_datamart_category_id, stat = alterlist(
        brvalue_request->filter,1), brvalue_request->filter[1].br_datamart_filter_id = bdf
       .br_datamart_filter_id,
       brvalue_request->filter[1].filter_mean = "WF_DIAGRAM_URL", stat = alterlist(brvalue_request->
        filter[1].value,1), brvalue_request->filter[1].value[1].freetext_desc = comp_detail_rec->
       comp_details[j].component_text
      WITH nocounter
     ;end select
     IF (validate(debug_ind,0)=1)
      CALL echorecord(brvalue_request)
     ENDIF
     EXECUTE bed_ens_datamart_values  WITH replace("REQUEST",brvalue_request)
   ENDFOR
   CALL log_message(build("Exit setStdDiagDetails(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE deletecpdiagramcomp(null)
   CALL log_message("Begin deleteCPDiagramComp()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE k = i4 WITH noconstant(1), protect
   ...
   FOR (k = 1 TO size(comp_detail_rec->comp_details,5))
     SET stat = initrec(comp_del_request)
     SET stat = alterlist(comp_del_request->qual,1)
     SET comp_del_request->cp_node_id = comp_detail_rec->comp_details[k].cp_node_id
     SET comp_del_request->qual[1].cp_component_id = comp_detail_rec->comp_details[k].cp_component_id
     SET comp_del_request->qual[1].comp_type_cd = cpdiagcd
     SET comp_del_request->qual[1].report_mean = diagreportmean
     IF (validate(debug_ind,0)=1)
      CALL echorecord(comp_del_request)
     ENDIF
     EXECUTE cp_br_del_component
   ENDFOR
   CALL log_message(build("Exit deleteCPDiagramComp(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
