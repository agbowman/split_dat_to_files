CREATE PROGRAM bbd_rpt_cont_org:dba
 ...
 SET captions->rpt_title = uar_i18ngetmessage(i18nhandle,"rpt_title",
  "CONTAINER TYPE / ORGANIZATION AND INVENTORY AREA")
 ...
 SET captions->rpt_org = uar_i18ngetmessage(i18nhandle,"rpt_org",
  "           Organization/Inventory Area")
 ...
 SET captions->rpt_id = uar_i18ngetmessage(i18nhandle,"rpt_id","Report ID: BBD_RPT_CONT_ORG")
 ...
 SET captions->rpt_bbd_cont = uar_i18ngetmessage(i18nhandle,"rpt_bbd_cont","bbd_rpt_cont_org")
 ...
 SELECT INTO "cer_temp:bbdcontorg.txt"
  container_type_disp = uar_get_code_display(c.container_type_cd), c.*, o.org_name,
  org_inv_area =
  IF (c.inventory_area_cd <= 0) substring(1,62,o.org_name)
  ELSE substring(1,62,uar_get_code_display(c.inventory_area_cd))
  ENDIF
  , org_inv_area_key =
  IF (c.inventory_area_cd <= 0) cnvtupper(substring(1,62,o.org_name))
  ELSE cnvtupper(substring(1,62,uar_get_code_display(c.inventory_area_cd)))
  ENDIF
  FROM container_org_r c,
   organization o
  PLAN (c
   WHERE c.container_org_id > 0)
   JOIN (o
   WHERE o.organization_id=outerjoin(c.organization_id))
  ORDER BY container_type_disp, org_inv_area_key, c.active_status_dt_tm DESC
  HEAD PAGE
   ...
   curdate"@DATECONDENSED;;d", row + 3, col 7,
   captions->rpt_container, col 42, captions->rpt_org,
   col 113, captions->rpt_active, row + 1,
   col 7, "------------------------------", col 42,
   "----------------------------------------------------------------", col 113, "------",
   row + 1
  HEAD container_type_disp
   col 7, container_type_disp"##############################"
  HEAD org_inv_area_key
   col 42, org_inv_area
   IF (c.active_ind=1)
