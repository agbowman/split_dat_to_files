CREATE PROGRAM bhs_rpt_no_code_stat:dba
 PROMPT
 ...
 DECLARE mf_comfortmeasureonly_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",200,
   "COMFORTMEASUREONLY"))
 DECLARE mf_comfortmeasuresonly_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",200,
   "COMFORTMEASURESONLY"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ed
   ...
   WHERE e.encntr_id=ed.encntr_id
    ...
    AND e.reg_dt_tm BETWEEN cnvtdatetime((curdate - 200),curtime) AND cnvtdatetime(curdate,curtime)
   ...
   mf_limitedresuscitation_cd, mf_fullcodeconfirmed_cd, mf_fullcodepresumed_cd,
   mf_comfortmeasureonly_cd, mf_comfortmeasuresonly_cd)
    AND o.orig_order_dt_tm BETWEEN cnvtdatetime((curdate - 200),curtime) AND cnvtdatetime(curdate,
    curtime)
  ...
  HEAD e.encntr_id
   ...
   IF (ml_cnt > size(m_rec->enc,5))
    CALL alterlist(m_rec->enc,(ml_cnt+ 99))
   ENDIF
   m_rec->enc[ml_cnt].f_encntr_id = e.encntr_id
  FOOT REPORT
   m_rec->l_enc_cnt = ml_cnt,
   CALL alterlist(m_rec->enc,ml_cnt)
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  facility = uar_get_code_display(ed.loc_facility_cd), nurse_unit = uar_get_code_display(ed
   .loc_nurse_unit_cd), room = uar_get_code_display(ed.loc_room_cd),
  ...
  PLAN (ed
   WHERE parser(ms_facility_p)
    AND parser(ms_nurse_unit_p)
    AND  NOT (expand(ml_expand,1,m_rec->l_enc_cnt,ed.encntr_id,m_rec->enc[ml_expand].f_encntr_id))
    AND ed.active_ind=1
    AND ed.active_status_cd=mf_active_cd
    AND ed.end_effective_dt_tm=cnvtdatetime("31-DEC-2100"))
   JOIN (e
  ...
  ORDER BY facility, nurse_unit, room,
   bed, p.name_full_formatted
  HEAD REPORT
   ml_cnt = 0
  HEAD e.encntr_id
   ml_cnt += 1
   ...
   .name_full_formatted, m_rec->qual[ml_cnt].s_reason = e.reason_for_visit,
   m_rec->qual[ml_cnt].s_attending = pr.name_full_formatted
  FOOT REPORT
 ...
 IF (textlen(trim( $S_RECIPIENTS,3)) > 1)
  ...
  SET ms_subject = build2("No Code Status Report ",trim(format(sysdate,"mmm-dd-yyyy hh:mm ;;d"),3))
  SET frec->file_buf = "w"
  SET stat = cclio("OPEN",frec)
  SET frec->file_buf = build('"FACILITY",','"NURSE UNIT",','"PATIENT",','"ROOM/BED",','"MRN#",',
   '"FIN#",','"ATTENDING PHYSICIAN",','"REASON FOR VISIT",',char(13))
  ...
  FOR (ml_cnt = 1 TO m_rec->l_qual_cnt)
   SET frec->file_buf = build('"',trim(m_rec->qual[ml_cnt].s_facility,3),'","',trim(m_rec->qual[
    ...
    trim(m_rec->qual[ml_cnt].s_patient_name,3),'","',trim(m_rec->qual[ml_cnt].s_room_bed,3),'","',
    trim(m_rec->qual[ml_cnt].s_mrn,3),
    '","',trim(m_rec->qual[ml_cnt].s_fin,3),'","',trim(m_rec->qual[ml_cnt].s_attending,3),'","',
    trim(m_rec->qual[ml_cnt].s_reason,3),'"',char(13))
   SET stat = cclio("WRITE",frec)
  ENDFOR
 ...
 ELSE
  SELECT INTO value( $OUTDEV)
   facility = substring(1,50,m_rec->qual[d.seq].s_facility), nurse_unit = substring(1,50,m_rec->qual[
    d.seq].s_nurse_unit), patient = substring(1,100,m_rec->qual[d.seq].s_patient_name),
   room_bed = substring(1,50,m_rec->qual[d.seq].s_room_bed), mrn = substring(1,50,m_rec->qual[d.seq].
    s_mrn), fin = substring(1,50,m_rec->qual[d.seq].s_fin),
   attending_physician = substring(1,100,m_rec->qual[d.seq].s_attending), reason_for_visit =
   substring(1,100,m_rec->qual[d.seq].s_reason)
   FROM (dummyt d  WITH seq = m_rec->l_qual_cnt)
