CREATE PROGRAM dcp_insert_cv_group:dba
...
#script
 ...
 FOR (x = 1 TO p_cnt)
   ...
   ENDIF
   SET c_cnt = size(request->parent_qual[x].child_qual,5)
   FOR (y = 1 TO c_cnt)
     SET request->parent_qual[x].child_qual[y].child_code_value = 0.0
   ENDFOR
 ENDFOR
 SELECT INTO "nl:"
  cv1.code_value
  ...
  DETAIL
   ...
   CALL echo(build("child cv :",cv2.code_value)), insert_cnt = (insert_cnt+ 1)
 ...
 INSERT  FROM code_value_group cvg,
  ...
  PLAN (d1)
   JOIN (d2
   WHERE d2.seq <= size(request->parent_qual[d1.seq].child_qual,5)
    AND (request->parent_qual[d1.seq].child_qual[d2.seq].child_code_value != 0.0))
   JOIN (cvg)
  WITH nocounter
 ;end insert
 IF (insert_cnt > 0
