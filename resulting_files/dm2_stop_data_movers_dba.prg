CREATE PROGRAM dm2_stop_data_movers:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsdm_kill_mover(dkm_sid=f8,dkm_serial=f8,dkm_ignore_ind=i2) = c1
 SUBROUTINE dsdm_kill_mover(dkm_sid,dkm_serial,dkm_ignore_ind)
   DECLARE dkm_ret = c1 WITH protect, noconstant(" ")
   CALL parser(concat("rdb alter system kill session '",trim(cnvtstring(dkm_sid,20)),",",trim(
      cnvtstring(dkm_serial,20)),"' go"),1)
   IF (check_error("Kill Oracle session for hanging mover") > 0)
    IF (dkm_ignore_ind=1)
     SET dm_err->user_action = "No action needed; the above message can be safely ignored"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     SET dm_err->err_ind = 0
     SET dm_err->emsg = ""
     SET dkm_ret = "S"
   ...
   RETURN(dkm_ret)
 END ;Subroutine
 DECLARE dsrdm_logfile = vc WITH protect, noconstant(" ")
 IF ((dm_err->logfile IN ("NONE", "DM2_LOGFILE_NOTSET")))
  SET dsrdm_logfile = dm_err->logfile
  SET dm_err->logfile = "DM2_LOGFILE_NOTSET"
 ELSE
  SET dsrdm_logfile = dm_err->logfile
 ENDIF
 SET dm_err->eproc = "Beginning dm2_stop_data_mover"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dsrdm_env_id = f8 WITH public, noconstant(0.0)
 DECLARE dsrdm_info_domain = vc
 DECLARE dsrdm_cnt = i4 WITH public, noconstant(0)
 DECLARE dsrdm_flag = i4 WITH public, noconstant(0)
 DECLARE dsrdm_file_name = vc
 DECLARE drp_cnt = i4 WITH public, noconstant(0)
 DECLARE dsrdm_dcl = vc
 DECLARE drp_info_number = i4 WITH public, noconstant(75)
 DECLARE dsrdm_writing_hang_file = vc WITH public, constant("WRITING HANG FILE")
 DECLARE dsrdm_orphaned_mover = vc WITH public, constant("ORPHANED MOVER")
 DECLARE dsrdm_hanging_mover = vc WITH public, constant("HANGING MOVER")
 FREE RECORD dsrdm_process
 RECORD dsrdm_process(
   1 list[*]
     2 dm_refchg_process_id = f8
     2 rdbhandle_value = f8
     2 env_source_id = f8
     2 session_id = f8
     2 serial_num = f8
 )
 FREE RECORD dsrdm_orp_prcess
 RECORD dsrdm_orp_prcess(
   1 list[*]
     2 domain = vc
     2 number = i4
 )
 SET dm_err->eproc = "Setting mover stop time"
 CALL disp_msg(" ",dm_err->logfile,0)
 UPDATE  FROM dm_info d
  ...
  WITH nocounter
 ;end update
 IF (check_error("Update dm_info with the new stop date time")=1)
  ROLLBACK
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ...
 IF (curqual=0)
  INSERT  FROM dm_info d
   ...
   WITH nocounter
  ;end insert
  IF (check_error("Insert into dm_info with the new stop date time")=1)
   ROLLBACK
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Pause 15 seconds before proceeding..."
 CALL disp_msg("",dm_err->logfile,0)
 FOR (lp = 1 TO 8)
   CALL pause(2)
 ENDFOR
 SELECT INTO "nl:"
  ...
  DETAIL
   dsrdm_env_id = a.info_number
  WITH nocounter
 ;end select
 IF (dsrdm_env_id=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Fatal Error: current environment id not found in DM_INFO"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dsrdm_flag = 1
 SELECT INTO "nl:"
  ...
  WHERE di.info_domain="DM_STAT_GATHER"
   AND di.info_name="DM2_CYCLE_DATA_MOVERS"
  DETAIL
   drp_info_number = 0, drp_info_number = (di.info_number+ 15)
  WITH nocounter
 ;end select
 IF (dsrdm_flag=1)
  SELECT INTO "nl:"
   dat = datetimediff(cnvtdatetime(curdate,curtime3),d.last_action_dt_tm,4)
   FROM dm_refchg_process d,
    gv$session s
   WHERE d.rdbhandle_value=s.audsid
    AND d.refchg_type="MOVER PROCESS"
    AND  NOT (d.refchg_status IN (dsrdm_writing_hang_file, dsrdm_orphaned_mover, dsrdm_hanging_mover)
   )
   DETAIL
    CALL echo(dat)
    IF (dat > drp_info_number)
     dsrdm_cnt = (dsrdm_cnt+ 1)
     IF (mod(dsrdm_cnt,100)=1)
      stat = alterlist(dsrdm_process->list,(dsrdm_cnt+ 99))
     ENDIF
     dsrdm_process->list[dsrdm_cnt].dm_refchg_process_id = d.dm_refchg_process_id, dsrdm_process->
     list[dsrdm_cnt].rdbhandle_value = d.rdbhandle_value, dsrdm_process->list[dsrdm_cnt].
     env_source_id = d.env_source_id,
     dsrdm_process->list[dsrdm_cnt].session_id = s.sid, dsrdm_process->list[dsrdm_cnt].serial_num = s
     .serial#
    ENDIF
   FOOT REPORT
    stat = alterlist(dsrdm_process->list,dsrdm_cnt)
   WITH nocounter
  ;end select
  IF (check_error("Populate the process record") != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  CALL echo(dsrdm_cnt)
  FOR (dsrdm_lp = 1 TO dsrdm_cnt)
    UPDATE  FROM dm_refchg_process d
     SET d.refchg_status = dsrdm_writing_hang_file
     WHERE (d.dm_refchg_process_id=dsrdm_process->list[dsrdm_lp].dm_refchg_process_id)
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
     ROLLBACK
  ...
  ENDFOR
  FOR (dsrdm_lp = 1 TO dsrdm_cnt)
    CALL dsdm_kill_mover(dsrdm_process->list[dsrdm_lp].session_id,dsrdm_process->list[dsrdm_lp].
     serial_num,1)
    SET dsrdm_dcl = build("DM_CHG_LOG@MERGE",cnvtstring(dsrdm_process->list[dsrdm_lp].env_source_id,
      20,0),cnvtstring(dsrdm_env_id,20,0))
    SELECT INTO build("HANGING_MOVER_",trim(cnvtstring(dsrdm_process->list[dsrdm_lp].rdbhandle_value,
        20,0)))
     FROM (value(dsrdm_dcl) dcl)
     WHERE dcl.target_env_id=dsrdm_env_id
      AND dcl.log_type="PROCES"
      AND dcl.rdbhandle=trim(cnvtstring(dsrdm_process->list[dsrdm_lp].rdbhandle_value,20,0))
     HEAD REPORT
      CALL print(format(curdate,"MM/DD/YY;;D")),
      CALL print(format(curtime3,";;Q"))
     DETAIL
      CALL print(dcl.log_id),
      CALL print(dcl.target_env_id),
      CALL print(dcl.table_name),
      row + 1,
      CALL print(substring(1,1000,dcl.pk_where)), row + 1,
      CALL print(dcl.rdbhandle),
      CALL print(dcl.delete_ind),
      CALL print(dcl.context_name)
     WITH nocounter, append, maxcol = 1001
    ;end select
    UPDATE  FROM (value(dsrdm_dcl) d)
     SET d.log_type = "REFCHG", d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_cnt = (d
      .updt_cnt+ 1)
     WHERE d.target_env_id=dsrdm_env_id
      AND d.log_type="PROCES"
      AND d.rdbhandle=trim(cnvtstring(dsrdm_process->list[dsrdm_lp].rdbhandle_value,20,0))
     WITH nocounter
    ;end update
    IF (check_error("Update DM_CHG_LOG rows back to REFCHG") != 0)
     ROLLBACK
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    UPDATE  FROM dm_refchg_process d
     SET d.refchg_status = dsrdm_hanging_mover, d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d
      .updt_cnt = (d.updt_cnt+ 1)
     WHERE (d.dm_refchg_process_id=dsrdm_process->list[dsrdm_lp].dm_refchg_process_id)
     WITH nocounter
    ;end update
    IF (check_error("Update DM_REFCHG_PROCESS rows to HANGING MOVER") != 0)
     ROLLBACK
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
  ENDFOR
  UPDATE  FROM dm_refchg_process d
   SET d.refchg_type = "ORPHANED MOVER", d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_cnt =
    (d.updt_cnt+ 1)
   WHERE d.refchg_type="MOVER PROCESS"
    AND  NOT (d.rdbhandle_value IN (
   (SELECT
    s.audsid
    FROM gv$session s)))
   WITH nocounter
  ;end update
  IF (check_error("Update DM_REFCHG_PROCESS rows to ORPHANED MOVER")=1)
   ROLLBACK
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SELECT INTO "nl:"
   ...
   WHERE d.refchg_type="MOVER PROCESS"
    ...
    AND cnvtupper(d.log_file)="DM2_RUN_DATA_MVR*.LOG"
   GROUP BY d.env_source_id
   DETAIL
    drp_cnt = (drp_cnt+ 1)
    IF (mod(drp_cnt,10)=1)
     stat = alterlist(dsrdm_orp_prcess->list,(drp_cnt+ 9))
    ENDIF
    dsrdm_orp_prcess->list[drp_cnt].domain = concat("MERGE",trim(cnvtstring(d.env_source_id,20,0)),
     trim(cnvtstring(dsrdm_env_id,20,0))), dsrdm_orp_prcess->list[drp_cnt].number = cnt
   FOOT REPORT
    stat = alterlist(dsrdm_orp_prcess->list,drp_cnt)
   WITH nocounter
  ;end select
  IF (check_error("Checking running movers")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 CALL updt_dm_info(concat('d.info_domain = "MERGE*',trim(cnvtstring(dsrdm_env_id,20,0)),'"'),0)
 FOR (lp = 1 TO drp_cnt)
   CALL updt_dm_info(concat('d.info_domain = "',dsrdm_orp_prcess->list[lp].domain,'"'),
    dsrdm_orp_prcess->list[lp].number)
 ENDFOR
 SUBROUTINE updt_dm_info(udi_domain,dit_number)
   SET dm_err->eproc = concat("Updating number of mover running for ",udi_domain)
   CALL disp_msg(" ",dm_err->logfile,0)
   UPDATE  FROM dm_info d
    SET d.info_number = dit_number, d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_cnt = (d
     .updt_cnt+ 1)
    WHERE parser(udi_domain)
     AND d.info_name="RDDS MOVERS RUNNING"
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
...
#exit_program
 ROLLBACK
 SET dm_err->eproc = "...Ending dm2_stop_data_mover"
 CALL final_disp_msg(" ")
 SET dm_err->logfile = dsrdm_logfile
END GO
