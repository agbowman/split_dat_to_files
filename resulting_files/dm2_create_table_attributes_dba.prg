CREATE PROGRAM dm2_create_table_attributes:dba
 DECLARE c_mod = vc WITH private, constant("000")
 DECLARE cta_logfile_prefix = vc WITH protect, constant("dm2_create_tblattr")
 DECLARE cta_tname = vc WITH protect, noconstant(" ")
 DECLARE cta_replace_ind = i4 WITH protect, noconstant(0)
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile(cta_logfile_prefix,".log","DM2_INSTALL_SCHEMA LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->debug_flag = 1
 SET dm_err->eproc = "Beginning Create Table Attributes Process"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET cta_tname = cnvtupper( $1)
 IF (cta_tname=" ")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Input table name to create table attributes is invalid."
  SET dm_err->eproc = "Input Table Name Validation"
  SET dm_err->user_action = "Please enter a table name to create table attributes."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET cta_replace_ind =  $2
 IF ( NOT (cta_replace_ind IN (0, 1)))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Trigger replace indicator of ",trim(cnvtstring(cta_replace_ind,4,0)),
   " is invalid.")
  SET dm_err->eproc = "Trigger Replace Indicator Validation"
  SET dm_err->user_action = "Please specify a valid trigger replace indicator (0 or 1)."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Create CCL Definition for the Table(s)"
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE oragen3 cta_tname
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Create Zero Row and Zero Row Triggers for Table(s)"
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm_add_zero_rows cta_tname
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Create Active Indicator Triggers for Table(s)"
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm_create_active_ind_trig cta_tname
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Combine Triggers for Table(s)"
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_combine_triggers cta_tname
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Grant SELECT privileges to the V500READ user for Table(s)"
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_grant_privileges cta_tname
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
#exit_program
 SET dm_err->eproc = "Ending Create Table Attributes Process.  Process Completed Successfully."
 CALL final_disp_msg(cta_logfile_prefix)
END GO
