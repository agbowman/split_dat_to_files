CREATE PROGRAM dm2_diload_indextypedata:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ddi_logfile_prefix = vc WITH protect, constant("dm2_diloadit")
 DECLARE ddi_substat_vc = vc WITH protect, noconstant(" ")
 DECLARE ddi_cnt = i4 WITH protect, noconstant(0)
 DECLARE ddi_insert_cnt_ver = i4 WITH protect, noconstant(0)
 DECLARE ddi_update_cnt_ver = i4 WITH protect, noconstant(0)
 DECLARE ddi_table_name = vc WITH protect, noconstant(" ")
 DECLARE ddi_index_name = vc WITH protect, noconstant(" ")
 DECLARE ddix = i4 WITH protect, noconstant(0)
 IF (validate(diload->qual[1].dml_type,"@")="@"
  AND validate(diload->qual[1].dml_type,"#")="#")
  FREE RECORD diload
  RECORD diload(
    1 diload_cnt = i4
    1 qual[*]
 ...
 IF (validate(dml_chk->qual[1].dml_type,"@")="@"
  AND validate(dml_chk->qual[1].dml_type,"#")="#")
  FREE RECORD dml_chk
  RECORD dml_chk(
    1 dml_cnt = i4
    1 dml_insert_cnt = i4
 ...
 IF (check_logfile(ddi_logfile_prefix,".log","DM2_DILOAD_INDEXTYPEDATA LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET readme_data->message = "Starting readme... "
 SET dm_err->eproc = "Starting dm2_diload_indextypedata..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET ddi_substat_vc = dm2_table_exists("DM_INFO")
 CASE (ddi_substat_vc)
  OF "N":
   SET dm_err->err_ind = 1
   SET dm_err->err_msg = "DM_INFO not found - Cannot load override index content."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDCASE
 IF (ddi_substat_vc != "F")
  GO TO exit_program
 ...
 ENDIF
 SET ddi_cnt = size(requestin->list_0,5)
 IF (ddi_cnt < 1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Loading Index override content from csv."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(ddi_cnt)),
   dm_tables_doc dtd
  PLAN (d
   WHERE d.seq > 0
    AND cnvtupper(trim(requestin->list_0[d.seq].rdbms,3))=currdb)
   JOIN (dtd
   WHERE dtd.full_table_name=cnvtupper(trim(requestin->list_0[d.seq].table_name,3))
    AND dtd.table_name=dtd.full_table_name)
  HEAD REPORT
   ddi_work_cnt = 0, dml_chk->dml_cnt = 0, dml_chk->dml_insert_cnt = 0,
   dml_chk->dml_update_cnt = 0, dml_chk->num_of_none = 0
  DETAIL
   ddi_work_cnt = (ddi_work_cnt+ 1)
   IF (mod(ddi_work_cnt,50)=1)
    stat = alterlist(diload->qual,(ddi_work_cnt+ 50)), stat = alterlist(dml_chk->qual,(ddi_work_cnt+
     50))
   ENDIF
   diload->qual[ddi_work_cnt].info_domain = concat("DM2_",cnvtupper(trim(requestin->list_0[d.seq].
      rdbms,3)),"_INDEX_TYPE")
   CASE (currdb)
    OF "ORACLE":
     ddi_table_name = cnvtupper(trim(requestin->list_0[d.seq].table_name,3)),ddi_index_name =
     cnvtupper(trim(requestin->list_0[d.seq].index_name,3))
    ELSE
     ddi_table_name = cnvtupper(trim(dtd.suffixed_table_name,3)),ddi_index_name = cnvtupper(build(
       substring(1,12,trim(requestin->list_0[d.seq].index_name,3)),trim(dtd.table_suffix,3)))
   ENDCASE
   diload->qual[ddi_work_cnt].info_name = cnvtupper(concat(ddi_table_name,":",ddi_index_name)),
   diload->qual[ddi_work_cnt].info_char = cnvtupper(trim(requestin->list_0[d.seq].index_type,3)),
   diload->qual[ddi_work_cnt].info_number = cnvtint(requestin->list_0[d.seq].baseline_schema_instance
    ),
   diload->qual[ddi_work_cnt].updt_applctx = cnvtint(requestin->list_0[d.seq].updt_applctx), diload->
   qual[ddi_work_cnt].updt_cnt = cnvtint(requestin->list_0[d.seq].updt_cnt), diload->qual[
   ddi_work_cnt].updt_id = cnvtint(requestin->list_0[d.seq].updt_id),
   diload->qual[ddi_work_cnt].updt_task = cnvtint(requestin->list_0[d.seq].updt_task), dml_chk->qual[
   ddi_work_cnt].dml_type = "I", dml_chk->dml_insert_cnt = (dml_chk->dml_insert_cnt+ 1),
   ddi_work_cnt = (ddi_work_cnt+ 1)
   IF (mod(ddi_work_cnt,50)=1)
    stat = alterlist(diload->qual,(ddi_work_cnt+ 50)), stat = alterlist(dml_chk->qual,(ddi_work_cnt+
     50))
   ENDIF
   diload->qual[ddi_work_cnt].info_domain = diload->qual[(ddi_work_cnt - 1)].info_domain, diload->
   qual[ddi_work_cnt].info_name = concat(ddi_table_name,":",trim(substring(1,28,ddi_index_name)),"$C"
    ), diload->qual[ddi_work_cnt].info_char = diload->qual[(ddi_work_cnt - 1)].info_char,
   diload->qual[ddi_work_cnt].info_number = diload->qual[(ddi_work_cnt - 1)].info_number, diload->
   qual[ddi_work_cnt].updt_applctx = diload->qual[(ddi_work_cnt - 1)].updt_applctx, diload->qual[
   ddi_work_cnt].updt_cnt = diload->qual[(ddi_work_cnt - 1)].updt_cnt,
   diload->qual[ddi_work_cnt].updt_id = diload->qual[(ddi_work_cnt - 1)].updt_id, diload->qual[
   ddi_work_cnt].updt_task = diload->qual[(ddi_work_cnt - 1)].updt_task, dml_chk->qual[ddi_work_cnt].
   dml_type = "I",
   dml_chk->dml_insert_cnt = (dml_chk->dml_insert_cnt+ 1)
  FOOT REPORT
   stat = alterlist(diload->qual,ddi_work_cnt), diload->diload_cnt = ddi_work_cnt, stat = alterlist(
    dml_chk->qual,ddi_work_cnt),
   dml_chk->dml_cnt = ddi_work_cnt
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating index types."
 CALL disp_msg(" ",dm_err->logfile,0)
 FOR (ddix = 1 TO diload->diload_cnt)
   IF (findstring("$C",diload->qual[ddix].info_name)=0)
    CASE (currdb)
     OF "ORACLE":
      IF ((diload->qual[ddix].info_char != "BITMAP"))
       SET dm_err->err_ind = 1
      ENDIF
     OF "SQLSRV":
      IF ((diload->qual[ddix].info_char != "CLUSTERED"))
       SET dm_err->err_ind = 1
      ENDIF
    ENDCASE
    IF ((dm_err->err_ind=1))
     SET dm_err->emsg = concat("An invalid Index Type (",diload->qual[ddix].info_char,
      ") was found for ","Table:Index (",diload->qual[ddix].info_name,
      ").")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Marking Index override data for insert or update."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((diload->diload_cnt > 0))
  SELECT INTO "nl:"
   FROM dm_info di,
    (dummyt t  WITH seq = value(diload->diload_cnt))
   PLAN (t)
    JOIN (di
    WHERE (di.info_domain=diload->qual[t.seq].info_domain)
     AND (di.info_name=diload->qual[t.seq].info_name))
   DETAIL
    dml_chk->dml_insert_cnt = (dml_chk->dml_insert_cnt - 1)
    IF ((diload->qual[t.seq].updt_cnt >= di.updt_cnt))
     dml_chk->qual[t.seq].dml_type = "U", dml_chk->dml_update_cnt = (dml_chk->dml_update_cnt+ 1)
    ELSE
     dml_chk->qual[t.seq].dml_type = "N", dml_chk->num_of_none = (dml_chk->num_of_none+ 1)
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(diload)
  CALL echorecord(dml_chk)
 ENDIF
 IF ((dml_chk->dml_insert_cnt > 0))
  SET dm_err->eproc = " Inserting Index Override data into DM_INFO"
  CALL disp_msg(" ",dm_err->logfile,0)
  INSERT  FROM dm_info di,
    (dummyt t  WITH seq = value(diload->diload_cnt))
   SET di.info_domain = diload->qual[t.seq].info_domain, di.info_name = diload->qual[t.seq].info_name,
    di.info_char = diload->qual[t.seq].info_char,
    di.info_number = diload->qual[t.seq].info_number, di.updt_applctx = diload->qual[t.seq].
    updt_applctx, di.updt_cnt = diload->qual[t.seq].updt_cnt,
    di.updt_id = diload->qual[t.seq].updt_id, di.updt_task = diload->qual[t.seq].updt_task
   PLAN (t
  ...
  ;end insert
  IF (dm2_debug_flag > 0)
   CALL echo(build("Actual rows inserted into dm_info=",curqual))
  ENDIF
  SET ddi_insert_cnt_ver = curqual
  IF (check_error(dm_err->eproc) != 0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF ((dml_chk->dml_update_cnt > 0))
  SET dm_err->eproc = " Updating Index Override data"
  CALL disp_msg(" ",dm_err->logfile,0)
  ...
  ;end update
  IF ((dm_err->debug_flag > 0))
   CALL echo(build("Actual rows updated into dm_info=",curqual))
  ENDIF
  SET ddi_update_cnt_ver = curqual
  IF (check_error(dm_err->eproc) != 0)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Verifying insert/update count actuals against expected."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dml_chk->dml_insert_cnt=ddi_insert_cnt_ver)
  AND (dml_chk->dml_update_cnt=ddi_update_cnt_ver))
  SET dm_err->eproc = "Insert/Update count verifications are correct !!"
  CALL disp_msg("",dm_err->logfile,0)
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = build("Insert/Update count verifications failed.  Inserts expected: (",dml_chk->
   dml_insert_cnt,").  Inserts actual:(",ddi_insert_cnt_ver,").  Updates expected: (",
   dml_chk->dml_update_cnt,").  Updates actual:(",ddi_update_cnt_ver,").")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
...
#exit_program
 IF ((dm_err->err_ind=1))
  SET readme_data->status = "F"
  SET readme_data->message = build("Readme Failed. ",dm_err->emsg)
  ROLLBACK
 ELSE
  SET readme_data->status = "S"
  SET readme_data->message = concat("Readme Successful. Index Types loaded successfully.")
  COMMIT
 ENDIF
 ...
 ENDIF
 SET dm_err->eproc = "ENDING DM2_DILOAD_INDEXTYPEDATA"
 CALL final_disp_msg(ddi_logfile_prefix)
END GO
