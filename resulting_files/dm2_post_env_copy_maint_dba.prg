CREATE PROGRAM dm2_post_env_copy_maint:dba
 ...
 SUBROUTINE drr_del_preserved_ts(dcdl_tgt_db_name)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dpecm_src_env_name = vc
 DECLARE dpecm_tgt_env_name = vc
 SET dpecm_src_env_name = ""
 SET dpecm_tgt_env_name = ""
 IF (check_logfile("dm2_post_env_cpy",".log","DM2_POST_ENV_COPY_MAINT LOG FILE...") != 1)
  GO TO exit_program
 ENDIF
 IF ((dir_env_maint_rs->tgt_env_id=0))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Checking if target environment_id record structure has been filled out"
  SET dm_err->emsg = "Target environment id not valued prior to script execution."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSEIF ((dir_env_maint_rs->src_env_id=0))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Checking if source environment_id record structure has been filled out"
  SET dm_err->emsg = "Source environment id not valued prior to script execution."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (checkprg("DM2_RDDS_POST_ENV_COPY_MAINT") != 0)
  EXECUTE dm2_rdds_post_env_copy_maint
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
 ENDIF
 ...
 SELECT INTO "NL:"
  d.environment_name
  FROM dm_environment d
  WHERE (d.environment_id=dir_env_maint_rs->src_env_id)
  DETAIL
   dpecm_src_env_name = d.environment_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "NL:"
  d.environment_name
  FROM dm_environment d
  WHERE (d.environment_id=dir_env_maint_rs->tgt_env_id)
  DETAIL
   dpecm_tgt_env_name = d.environment_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((((dir_env_maint_rs->process="DM2NOTSET")) OR (validate(dm2_bypass_adm_csv_load,- (1))=1)) )
  EXECUTE dm2_copy_ocd_hist dpecm_src_env_name, dpecm_tgt_env_name
  IF ((dm_err->err_ind=1))
   ROLLBACK
   GO TO exit_program
  ENDIF
 ELSEIF ((dir_env_maint_rs->process="REPLICATE"))
  EXECUTE dm2_load_env_history_wrapper drr_env_hist_misc->path, dpecm_src_env_name,
  dpecm_tgt_env_name
  IF ((dm_err->err_ind=1))
   ROLLBACK
   GO TO exit_program
 ...
 ENDIF
 IF ((drr_clin_copy_data->purge_chosen_ind=1))
  IF (checkprg("DM2_ADS_PURGE_POST_ENV_COPY") != 0)
   EXECUTE dm2_ads_purge_post_env_copy
   IF ((dm_err->err_ind=1))
    GO TO exit_program
   ENDIF
  ENDIF
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "Exiting dm2_post_copy_maint."
 CALL final_disp_msg("dm2_post_env_cpy")
END GO
