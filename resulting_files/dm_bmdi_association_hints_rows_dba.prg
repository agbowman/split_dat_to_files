CREATE PROGRAM dm_bmdi_association_hints_rows:dba
 ...
 SET reply->table_name = "BMDI_ASSOCIATION_HINTS"
 ...
 ELSE
  ...
  WHILE (v_cur_min_id <= v_max_id
   AND v_rows_left > 0)
    SELECT
     IF (v_days_to_keep=0)
      WHERE parser(sbr_getrowidnotexists("bah.hint_id between v_cur_min_id and v_cur_max_id","bah"))
     ELSE
      WHERE bah.hint_id BETWEEN v_cur_min_id AND v_cur_max_id
       AND parser(sbr_getrowidnotexists("bah.updt_dt_tm < cnvtdatetime(v_days_back)","bah"))
     ENDIF
     INTO "nl:"
     bah.rowid
     FROM bmdi_association_hints bah
     DETAIL
      ...
      ENDIF
      reply->rows[v_rows].row_id = bah.rowid
     WITH nocounter, maxqual(bah,value(v_rows_left))
    ;end select
