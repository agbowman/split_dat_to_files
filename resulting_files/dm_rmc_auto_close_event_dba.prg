CREATE PROGRAM dm_rmc_auto_close_event:dba
 ...
 SUBROUTINE dm2_get_program_stack(null)
   ...
   ENDIF
 END ;Subroutine
 IF (validate(ddoer_request)=0)
  FREE RECORD ddoer_request
  RECORD ddoer_request(
    1 cur_env_id = f8
  )
 ENDIF
 IF (validate(ddoer_reply)=0)
  FREE RECORD ddoer_reply
  RECORD ddoer_reply(
    1 err_ind = i2
    1 err_msg = vc
  )
 ...
 ENDIF
 DECLARE drace_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drace_src_id = f8 WITH protect, noconstant(0.0)
 DECLARE drace_event_flg = i4 WITH protect, noconstant(0)
 DECLARE drace_event_ind = i2 WITH protect, noconstant(0)
 DECLARE drace_event_name = vc WITH protect, noconstant(" ")
 DECLARE drace_idx = i4 WITH protect, noconstant(0)
 DECLARE drace_max_dt_tm = f8 WITH protect, noconstant(cnvtdatetime("01-JAN-1900 00:00:00"))
 DECLARE drace_cut_dt_tm = f8 WITH protect, noconstant(0.0)
 DECLARE drace_num_days = i4 WITH protect, noconstant(120)
 DECLARE drace_close_event = i2 WITH protect, noconstant(0)
 FREE RECORD r_table
 RECORD r_table(
   1 tabs[*]
     2 tab_name = vc
 ...
 IF (check_logfile("dm_rmc_auto_",".log","DM_RMC_AUTO_CLOSE_EVENT LOGFILE")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning DM_RMC_AUTO_CLOSE_EVENT"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Gathering env_id"
 SELECT INTO "nl:"
  FROM dm_info a,
   dm_environment b
  PLAN (a
 ...
 IF (drace_env_id=0.0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to find valid environment_id."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET ddoer_request->cur_env_id = drace_env_id
 EXECUTE dm_del_old_env_reltn
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drace_event_flg = drtq_check_task_process("OPEN EVENT PROCESS")
 IF (drace_event_flg IN (1, 2))
  SELECT INTO "NL:"
   ...
   WHERE drel.rdds_event="Begin Reference Data Sync"
    ...
    AND drel.cur_environment_id=drace_env_id
   ...
   (SELECT
    ...
    WHERE d.cur_environment_id=drace_env_id
     AND d.rdds_event="End Reference Data Sync"
     AND d.rdds_event_key="ENDREFERENCEDATASYNC")))
   DETAIL
    drace_src_id = drel.paired_environment_id, drace_event_name = drel.event_reason, drace_event_ind
     = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
  ...
  ENDIF
 ENDIF
 IF (drace_event_ind=1)
  SET dm_err->eproc = "Create missing $R definitions"
  EXECUTE dm_rmc_oragen_r
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET drcec_request->cur_env_id = drace_env_id
  EXECUTE dm_rmc_close_event_chk
  IF ((drcec_reply->ret_val IN (0, - (1))))
   CALL disp_msg(drcec_reply->ret_msg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
  FOR (drace_idx = 1 TO r_table->tabs_cnt)
   SELECT INTO "nl:"
    max_dt = max(r.rdds_dt_tm)
    FROM (parser(r_table->tabs[drace_idx].tab_name) r)
    WHERE 1=1
    DETAIL
     IF (drace_max_dt_tm < max_dt)
      drace_max_dt_tm = max_dt
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
   ...
   ENDIF
  ENDFOR
  IF (drace_max_dt_tm=cnvtdatetime("01-JAN-1900 00:00:00"))
   SELECT INTO "nl:"
    FROM dm_env_reltn d
    WHERE d.parent_env_id=drace_src_id
     AND d.child_env_id=drace_env_id
   ...
   IF (check_error(dm_err->eproc) != 0)
    ...
    GO TO exit_program
   ELSEIF (curqual > 0)
    SET dm_err->eproc = "No rows found in the $R tables event will not be closed."
    CALL disp_msg(" ",dm_err->logfile,0)
    GO TO exit_program
   ENDIF
   SET drace_close_event = 1
  ENDIF
  SET dm_err->eproc = "Gathering last Cutover Started event time."
  SELECT INTO "nl:"
   max_dt = max(event_dt_tm)
   FROM dm_rdds_event_log d
   WHERE d.cur_environment_id=drace_env_id
    AND d.rdds_event_key IN ("CUTOVERSTARTED", "AUTOCUTOVERSTARTED")
   DETAIL
    drace_cut_dt_tm = max_dt
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (drace_max_dt_tm < drace_cut_dt_tm)
   SET dm_err->eproc = "Gathering last Cutover Finished event time."
   SELECT INTO "nl:"
    max_dt = max(d.event_dt_tm)
    FROM dm_rdds_event_log d
    WHERE d.cur_environment_id=drace_env_id
     AND d.rdds_event_key IN ("CUTOVERFINISHED", "AUTOCUTOVERFINISHED")
    DETAIL
     drace_cut_dt_tm = max_dt
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    ...
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = "Gathering max table cutover date from dm_info."
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="RDDS CONFIGURATION"
     AND d.info_name=concat("LAST SUCCESSFUL CUT-OVER:",trim(cnvtstring(drace_env_id)))
    DETAIL
     IF (d.info_date > drace_cut_dt_tm)
      drace_cut_dt_tm = d.info_date
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    ...
    GO TO exit_program
   ENDIF
   IF (drace_cut_dt_tm > 0.0)
    SET drace_max_dt_tm = drace_cut_dt_tm
   ENDIF
  ENDIF
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="RDDS CONFIGURATION"
    AND di.info_name="AUTO CLOSE EVENT DELAY"
   DETAIL
    drace_num_days = di.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (((drace_max_dt_tm < cnvtlookbehind(concat(cnvtstring(drace_num_days),",D"),cnvtdatetime(curdate,
    curtime3))) OR (drace_close_event=1)) )
   EXECUTE dm_rmc_bookmark_end drace_src_id, drace_env_id, drace_event_name
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
  ENDIF
 ELSE
  SET dm_err->eproc = "No open event was detected."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
#exit_program
END GO
