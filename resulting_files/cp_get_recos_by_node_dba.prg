CREATE PROGRAM cp_get_recos_by_node:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "CP Node Id" = 0
  WITH outdev, cp_node_id
 DECLARE cp_node_id = f8 WITH protect, constant( $CP_NODE_ID)
 DECLARE treatopts_comp_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003130,
   "TREATMENTOPT"))
 DECLARE gt_comp_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003130,"GUIDEDTRMNT"))
 ...
 DECLARE order_sentence = vc WITH protect, constant("ORDER_SENTENCE")
 DECLARE alt_sel_cat_entity_name = vc WITH protect, constant("ALT_SEL_CAT")
 IF (validate(recommendations) != 1)
 ...
 SUBROUTINE main(null)
   SET recommendations->status_data.status = "F"
   CALL getallrecommendedorders(null)
   SET recommendations->status_data.status = "S"
   CALL putjsonrecordtofile(recommendations)
 END ;Subroutine
 SUBROUTINE getallrecommendedorders(null)
   ...
   DECLARE rsize = i4 WITH protect, noconstant(size(recommendations->qual,5))
   DECLARE acntr = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM cp_component cc2,
     cp_component_detail ccd
    PLAN (cc2
     WHERE cc2.cp_node_id=cp_node_id
      AND cc2.comp_type_cd IN (treatopts_comp_cd, gt_comp_cd))
     JOIN (ccd
   ...
   SELECT INTO "nl:"
    ...
    HEAD c.alt_sel_category_id
     IF (c.alt_sel_category_id > 0.00)
      rsize += 1, stat = alterlist(recommendations->qual,rsize), recommendations->qual[rsize].value
       = c.long_description,
      recommendations->qual[rsize].id = c.alt_sel_category_id, recommendations->qual[rsize].
      reaction_entity_name = alt_sel_cat_entity_name
     ENDIF
    HEAD ocs.synonym_id
     row + 0
    HEAD os.order_sentence_id
     IF (ocs.synonym_id > 0.00
      AND os.order_sentence_id > 0.00)
      rsize += 1, stat = alterlist(recommendations->qual,rsize), recommendations->qual[rsize].value
       = build(ocs.mnemonic," (",trim(os.order_sentence_display_line,3),")"),
      recommendations->qual[rsize].id = os.order_sentence_id, recommendations->qual[rsize].
      reaction_entity_name = order_sentence
     ELSEIF (ocs.synonym_id > 0.00
      AND os.order_sentence_id=0.00)
      rsize += 1, stat = alterlist(recommendations->qual,rsize), recommendations->qual[rsize].value
       = ocs.mnemonic,
      recommendations->qual[rsize].id = ocs.synonym_id, recommendations->qual[rsize].
      reaction_entity_name = order_cat_synonym
     ENDIF
    HEAD pc.pathway_catalog_id
     IF (pc.pathway_catalog_id > 0.00)
      rsize += 1, stat = alterlist(recommendations->qual,rsize), recommendations->qual[rsize].value
       = pcs.synonym_name,
      recommendations->qual[rsize].id = pcs.pathway_catalog_id, recommendations->qual[rsize].
      reaction_entity_name = pathway_catalog
     ENDIF
    HEAD rcs.regimen_cat_synonym_id
     IF (rcs.regimen_cat_synonym_id > 0.00)
      rsize += 1, stat = alterlist(recommendations->qual,rsize), recommendations->qual[rsize].value
       = rcs.synonym_display,
      recommendations->qual[rsize].id = rcs.regimen_catalog_id, recommendations->qual[rsize].
      reaction_entity_name = regimen_catalog
     ENDIF
    FOOT REPORT
     recommendations->cnt = rsize, stat = alterlist(recommendations->qual,rsize)
...
#exit_script
 IF (validate(debug_ind,0)=1)
  CALL echorecord(recommendations)
 ENDIF
END GO
