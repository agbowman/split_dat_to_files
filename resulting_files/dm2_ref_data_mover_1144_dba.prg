CREATE PROGRAM dm2_ref_data_mover_1144:dba
 ...
 DECLARE cust_to_value = vc
 DECLARE cust_field_pos = i4
 DECLARE cust_new_value = f8
 DECLARE cust_rpt_missing = vc
 DECLARE cust_err_val = i4
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_new_value = cnvtreal(get_value(cust_tab_name,"FLEX_CONTROL_ID","FROM"))
 SET cust_field_pos = get_col_pos(cust_tab_name,"FIELD")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_field_pos].translated = 0
 SET cust_field_value = get_value(cust_tab_name,"FIELD","FROM")
 IF (((isnumeric(cust_field_value)=0) OR (cust_field_value=" ")) )
  CALL put_value(cust_tab_name,"FIELD",cust_field_value)
 ELSE
  SET cust_to_value = select_merge_translate(cust_field_value,"CODE_VALUE")
  IF (cust_to_value="No Trans")
   SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_field_value))
   CALL evaluate_rpt_missing(cust_rpt_missing)
   SET cust_err_val = get_err_val(null)
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg =
    "This row wasn't translated because the FIELD column wasn't translated"
   ENDIF
   GO TO exit_1144
  ELSE
   CALL put_value(cust_tab_name,"FIELD",cust_to_value)
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"FLEX_CONTROL_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"FLEX_CONTROL_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"FLEX_CONTROL_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  FLEX_CONTROL_ID")
     ELSE
      GO TO exit_1144
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"FLEX_CONTROL_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1144
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"FLEX_CONTROL_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1144
   ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"FIELD")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the FIELD column"
    IF ((global_mover_rec->one_pass_ind=0))
     ROLLBACK
    ENDIF
   ENDIF
