CREATE PROGRAM dm2_aload_ocd_readme_comp
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_df",".log","dm2_aload_ocd_readme_comp")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_ocd_readme_comp"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dacomp_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dacomp_file = vc WITH protect, noconstant( $2)
 DECLARE dacomp_passive = i2 WITH protect, noconstant( $3)
 DECLARE dacomp_expected_rows = i4 WITH protect, noconstant( $4)
 DECLARE dacomp_batch = i4 WITH protect, noconstant(0)
 DECLARE dacomp_file_loc_ret = vc WITH protect, noconstant("")
 DECLARE dacomp_fnd_ind = i2 WITH protect, noconstant(0)
 SET dm_err->eproc = "Verify OCD_README_COMPONENT table and CCLDEF exist."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SET dacomp_fnd_ind = checkdic("OCD_README_COMPONENT","T",0)
 IF (dacomp_fnd_ind=0
  AND dacomp_passive=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "OCD_README_COMPONENT table/ccldef does not exist. Table is not passive."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSEIF (dacomp_fnd_ind=0
  AND dacomp_passive=1)
  SET dm_err->eproc = "OCD_README_COMPONENT does not exist, but is passive. Continue."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (dac_prelim(dacomp_pkg,dacomp_file_loc_ret,dacomp_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dacomp_batch=0)
  SET dacomp_batch = 5000
 ENDIF
 IF (dacomp_expected_rows > 0)
  EXECUTE dm2_dbimport concat(dacomp_file_loc_ret,dacomp_file), "dm2_aload_rdm_orc", dacomp_batch
  CALL echo(concat(dacomp_file_loc_ret,dacomp_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_ocd_readme_comp completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_ocd_readme_comp")
END GO
