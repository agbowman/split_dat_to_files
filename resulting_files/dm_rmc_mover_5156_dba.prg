CREATE PROGRAM dm_rmc_mover_5156:dba
 ...
 DECLARE cust_xlat_col = vc
 DECLARE cust_process_val = vc
 ...
 SET cust_val2 = trim(get_value(cust_tab_name,"VAL2","FROM"))
 SET cust_val4 = trim(get_value(cust_tab_name,"VAL4","FROM"))
 ...
 CASE (cust_val1)
  ...
  OF "1":
  OF "4":
   SET cust_xlat_col = "NONE"
  OF "5":
   SET cust_process_tab = "OMF_PV_ITEMS"
   SET cust_process_col = "OMF_PV_ITEM_ID"
   SET cust_xlat_col = "VAL4"
   SET cust_process_val = cust_val4
  OF "7":
   SET cust_process_tab = "CODE_VALUE"
   SET cust_process_col = "CODE_VALUE"
   SET cust_process_val = cust_val2
   SET cust_xlat_col = "VAL2"
  ELSE
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV54"
   CALL disp_msg(concat("ERROR: the value of VAL1 is not an expected value"),dm_err->logfile,0)
   GO TO exit_5156
 ENDCASE
 IF (cust_process_tab > " ")
  SET cust_column_cnt = get_col_pos(cust_tab_name,cust_xlat_col)
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_column_cnt].translated = 0
  SET cust_new_value = select_merge_translate(cust_process_val,cust_process_tab)
  IF (isnumeric(cust_new_value))
   CALL put_value(cust_tab_name,cust_xlat_col,cust_new_value)
  ELSE
   SET cust_rpt_missing = report_missing(cust_process_tab,cust_process_col,cnvtreal(cust_process_val)
    )
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"COMPONENT_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"COMPONENT_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"COMPONENT_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  COMPONENT_ID")
     ELSE
      GO TO exit_5156
     ENDIF
    ELSE
   ...
   SET cust_from_id = get_value(cust_tab_name,"COMPONENT_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5156
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_id)
   ENDIF
   SET cust_new_id = get_value(cust_tab_name,"COMPONENT_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5156
   ELSE
 ...
 IF (cust_idcd_check=1)
  IF (cust_xlat_col != "NONE")
   SET cust_idcd_check = is_translated(cust_tab_name,cust_xlat_col)
