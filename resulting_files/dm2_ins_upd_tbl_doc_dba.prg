CREATE PROGRAM dm2_ins_upd_tbl_doc:dba
 ...
 ENDIF
 DECLARE iutdp_logfile_prefix = vc WITH protect, constant("dm2_ins_upd_tbl_doc")
 DECLARE iutdp_cnt = i4 WITH protect, noconstant(0)
 DECLARE iutdp_curqual = i4 WITH protect, noconstant(0)
 IF (check_logfile(iutdp_logfile_prefix,".log","DM2_INS_UPD_TBL_DOC LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning maintenance on DM_TABLES_DOC...."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (validate(tbl_doc->qual[1].table_name,"@")="@")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Record structure required for this process is not established."
  SET dm_err->eproc = "Confirming record structure [tbl_doc] existence."
  SET dm_err->user_action = "Please establish required record structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (validate(tbl_doc_maint->qual[1].maint_type,"@")="@")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Record structure required for this process is not established."
  SET dm_err->eproc = "Confirming record structure [tbl_doc_maint] existence."
  SET dm_err->user_action = "Please establish required record structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determine if process running in an in-house domain"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
 ...
 ELSEIF (curqual=1)
  IF ((dm2_install_schema->process_option="INHOUSE")
   AND currdb="SQLSRV")
   SET dm_err->eproc = "Process running in a SQLSRV/in-house domain, maintenance will be performed."
   CALL disp_msg(" ",dm_err->logfile,0)
  ELSE
   SET dm_err->emsg = "Process running in an in-house domain, no maintenance will be performed."
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF ((tbl_doc_maint->num_of_inserts > 0))
  IF (dm2_set_autocommit(1)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Performing insert maintenance on DM_TABLES_DOC."
  CALL disp_msg(" ",dm_err->logfile,0)
  CALL dm2_push_cmd("insert into dm_tables_doc  dtd, ",0)
  CALL dm2_push_cmd("  (dummyt d with seq = value(size(tbl_doc->qual,5))) ",0)
  CALL dm2_push_cmd(" set dtd.table_name = tbl_doc->qual[d.seq]->table_name ",0)
  CALL dm2_push_cmd("  ,dtd.data_model_section = tbl_doc->qual[d.seq]->data_model_section ",0)
  CALL dm2_push_cmd("  ,dtd.description = tbl_doc->qual[d.seq]->description ",0)
  CALL dm2_push_cmd("  ,dtd.definition = tbl_doc->qual[d.seq]->definition ",0)
  CALL dm2_push_cmd("  ,dtd.primary_update_script = tbl_doc->qual[d.seq]->primary_update_script ",0)
  CALL dm2_push_cmd("  ,dtd.primary_insert_script = tbl_doc->qual[d.seq]->primary_insert_script ",0)
  CALL dm2_push_cmd("  ,dtd.primary_delete_script = tbl_doc->qual[d.seq]->primary_delete_script ",0)
  CALL dm2_push_cmd("  ,dtd.static_size_flg = tbl_doc->qual[d.seq]->static_size_flg ",0)
  CALL dm2_push_cmd("  ,dtd.static_rows = tbl_doc->qual[d.seq]->static_rows ",0)
  CALL dm2_push_cmd("  ,dtd.reads_flg = tbl_doc->qual[d.seq]->reads_flg ",0)
  CALL dm2_push_cmd("  ,dtd.update_flg = tbl_doc->qual[d.seq]->update_flg ",0)
  CALL dm2_push_cmd("  ,dtd.insert_flg = tbl_doc->qual[d.seq]->insert_flg ",0)
  CALL dm2_push_cmd("  ,dtd.delete_flg = tbl_doc->qual[d.seq]->delete_flg ",0)
  CALL dm2_push_cmd("  ,dtd.core_ind = tbl_doc->qual[d.seq]->core_ind ",0)
  CALL dm2_push_cmd("  ,dtd.updt_dt_tm = cnvtdatetime(curdate,curtime3) ",0)
  CALL dm2_push_cmd("  ,dtd.updt_cnt = tbl_doc->qual[d.seq]->updt_cnt ",0)
  CALL dm2_push_cmd("  ,dtd.updt_applctx = 0 ",0)
  CALL dm2_push_cmd("  ,dtd.updt_task = 0 ",0)
  CALL dm2_push_cmd("  ,dtd.updt_id = 0 ",0)
  CALL dm2_push_cmd("  ,dtd.bytes_per_row = tbl_doc->qual[d.seq]->bytes_per_row ",0)
  CALL dm2_push_cmd("  ,dtd.reference_ind = tbl_doc->qual[d.seq]->reference_ind ",0)
  CALL dm2_push_cmd("  ,dtd.pct_free = tbl_doc->qual[d.seq]->pct_free ",0)
  CALL dm2_push_cmd("  ,dtd.pct_used = tbl_doc->qual[d.seq]->pct_used ",0)
  CALL dm2_push_cmd("  ,dtd.bpr_mean = tbl_doc->qual[d.seq]->bpr_mean ",0)
  CALL dm2_push_cmd("  ,dtd.bpr_min = tbl_doc->qual[d.seq]->bpr_min ",0)
  CALL dm2_push_cmd("  ,dtd.bpr_max = tbl_doc->qual[d.seq]->bpr_max ",0)
  CALL dm2_push_cmd("  ,dtd.bpr_std_dev = tbl_doc->qual[d.seq]->bpr_std_dev ",0)
  CALL dm2_push_cmd("  ,dtd.human_reqd_ind = tbl_doc->qual[d.seq]->human_reqd_ind ",0)
  CALL dm2_push_cmd("  ,dtd.purge_except_ind = tbl_doc->qual[d.seq]->purge_except_ind ",0)
  CALL dm2_push_cmd("  ,dtd.freelist_cnt = tbl_doc->qual[d.seq]->freelist_cnt ",0)
  IF ((validate(tbl_doc->qual[1].drop_ind,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dtd.drop_ind = tbl_doc->qual[d.seq]->drop_ind ",0)
  ENDIF
  IF (validate(tbl_doc->qual[1].table_suffix,"@") != "@")
   CALL dm2_push_cmd("  ,dtd.table_suffix = tbl_doc->qual[d.seq]->table_suffix ",0)
  ENDIF
  IF (validate(tbl_doc->qual[1].full_table_name,"@") != "@")
   CALL dm2_push_cmd("  ,dtd.full_table_name = tbl_doc->qual[d.seq]->full_table_name ",0)
  ENDIF
  IF (validate(tbl_doc->qual[1].suffixed_table_name,"@") != "@")
   CALL dm2_push_cmd("  ,dtd.suffixed_table_name = tbl_doc->qual[d.seq]->suffixed_table_name ",0)
  ENDIF
  IF ((validate(tbl_doc->qual[1].default_row_ind,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dtd.default_row_ind = tbl_doc->qual[d.seq]->default_row_ind ",0)
  ENDIF
  IF (validate(tbl_doc->qual[1].person_cmb_trigger_type,"@") != "@")
   CALL dm2_push_cmd(
    "  ,dtd.person_cmb_trigger_type = tbl_doc->qual[d.seq]->person_cmb_trigger_type ",0)
  ENDIF
  IF (validate(tbl_doc->qual[1].encntr_cmb_trigger_type,"@") != "@")
   CALL dm2_push_cmd(
    "  ,dtd.encntr_cmb_trigger_type = tbl_doc->qual[d.seq]->encntr_cmb_trigger_type ",0)
  ENDIF
  IF (validate(tbl_doc->qual[1].merge_ui_query,"@") != "@")
   CALL dm2_push_cmd("  ,dtd.merge_ui_query = tbl_doc->qual[d.seq]->merge_ui_query ",0)
  ENDIF
  IF ((validate(tbl_doc->qual[1].mergeable_ind,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dtd.mergeable_ind = tbl_doc->qual[d.seq]->mergeable_ind ",0)
  ENDIF
  IF ((validate(tbl_doc->qual[1].merge_delete_ind,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dtd.merge_delete_ind = tbl_doc->qual[d.seq]->merge_delete_ind ",0)
  ENDIF
  IF ((validate(tbl_doc->qual[1].merge_active_ind,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dtd.merge_active_ind = tbl_doc->qual[d.seq]->merge_active_ind ",0)
  ENDIF
  IF ((validate(tbl_doc->qual[1].data_disp_flag,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dtd.data_disp_flag = tbl_doc->qual[d.seq]->data_disp_flag ",0)
  ENDIF
  CALL dm2_push_cmd(' plan d where tbl_doc_maint->qual[d.seq]->maint_type = "I" ',0)
  CALL dm2_push_cmd(" join dtd where tbl_doc->qual[d.seq]->table_name = dtd.table_name ",0)
  CALL dm2_push_cmd("with nocounter go",1)
  IF ((dm_err->err_ind=0))
   COMMIT
   SET iutdp_curqual = (iutdp_curqual+ curqual)
  ELSE
   ROLLBACK
   GO TO exit_program
  ENDIF
  ...
  ENDIF
 ENDIF
 IF ((tbl_doc_maint->num_of_updates > 0))
  IF (currdb="DB2UDB")
   SET trace = rdbskipcast
  ENDIF
  IF (dm2_set_autocommit(1)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Performing update maintenance on DM_TABLES_DOC."
  CALL disp_msg(" ",dm_err->logfile,0)
  CALL dm2_push_cmd("update into dm_tables_doc  dtd, ",0)
  CALL dm2_push_cmd("  (dummyt d with seq = value(size(tbl_doc->qual,5))) ",0)
  CALL dm2_push_cmd(" set dtd.data_model_section = tbl_doc->qual[d.seq]->data_model_section ",0)
  CALL dm2_push_cmd("  ,dtd.description = tbl_doc->qual[d.seq]->description ",0)
  CALL dm2_push_cmd("  ,dtd.definition = tbl_doc->qual[d.seq]->definition ",0)
  CALL dm2_push_cmd("  ,dtd.primary_update_script = tbl_doc->qual[d.seq]->primary_update_script ",0)
  CALL dm2_push_cmd("  ,dtd.primary_insert_script = tbl_doc->qual[d.seq]->primary_insert_script ",0)
  ...
  IF ((validate(tbl_doc->qual[1].data_disp_flag,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dtd.data_disp_flag = tbl_doc->qual[d.seq]->data_disp_flag ",0)
  ENDIF
  CALL dm2_push_cmd(' plan d where tbl_doc_maint->qual[d.seq]->maint_type = "U" ',0)
  CALL dm2_push_cmd(" join dtd where tbl_doc->qual[d.seq]->table_name = dtd.table_name ",0)
  CALL dm2_push_cmd("with nocounter go",1)
  IF ((dm_err->err_ind=0))
   COMMIT
  ...
  ENDIF
  IF (currdb="DB2UDB")
   SET trace = nordbskipcast
  ENDIF
 ENDIF
 IF ((tbl_doc_maint->num_of_fixes > 0))
  IF (currdb="DB2UDB")
   SET trace = rdbskipcast
  ENDIF
  IF (dm2_set_autocommit(1)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Performing update maintenance on DM_TABLES_DOC for 8.0 and 8.1 columns."
  CALL disp_msg(" ",dm_err->logfile,0)
  CALL dm2_push_cmd("update into dm_tables_doc  dtd, ",0)
  CALL dm2_push_cmd("  (dummyt d with seq = value(size(tbl_doc->qual,5))) ",0)
  CALL dm2_push_cmd(" set dtd.drop_ind = tbl_doc->qual[d.seq]->drop_ind ",0)
  CALL dm2_push_cmd("  ,dtd.table_suffix = tbl_doc->qual[d.seq]->table_suffix ",0)
  CALL dm2_push_cmd("  ,dtd.full_table_name = tbl_doc->qual[d.seq]->full_table_name ",0)
  CALL dm2_push_cmd("  ,dtd.suffixed_table_name = tbl_doc->qual[d.seq]->suffixed_table_name ",0)
  CALL dm2_push_cmd("  ,dtd.default_row_ind = tbl_doc->qual[d.seq]->default_row_ind ",0)
  CALL dm2_push_cmd("  ,dtd.person_cmb_trigger_type = tbl_doc->qual[d.seq]->person_cmb_trigger_type ",
   0)
  CALL dm2_push_cmd("  ,dtd.encntr_cmb_trigger_type = tbl_doc->qual[d.seq]->encntr_cmb_trigger_type ",
   0)
  IF (validate(tbl_doc->qual[1].merge_ui_query,"@") != "@")
   CALL dm2_push_cmd("  ,dtd.merge_ui_query = tbl_doc->qual[d.seq]->merge_ui_query ",0)
  ENDIF
  ...
  IF ((validate(tbl_doc->qual[1].data_disp_flag,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dtd.data_disp_flag = tbl_doc->qual[d.seq]->data_disp_flag ",0)
  ENDIF
  CALL dm2_push_cmd(' plan d where tbl_doc_maint->qual[d.seq]->maint_type = "F" ',0)
  CALL dm2_push_cmd(" join dtd where tbl_doc->qual[d.seq]->table_name = dtd.table_name ",0)
  CALL dm2_push_cmd("with nocounter go",1)
  IF ((dm_err->err_ind=0))
   COMMIT
 ...
 IF (iutdp_curqual > 0
  ...
  AND checkprg("DM_RMC_AUTO_VERIFY_SETUP") > 0)
  SET stat = alterlist(auto_ver_request->qual,1)
  SET auto_ver_request->qual[1].rdds_event = "Meta-Data Change"
  SET auto_ver_request->qual[1].cur_environment_id = 0
  SET auto_ver_request->qual[1].paired_environment_id = 0
  SET auto_ver_request->qual[1].event_reason = "DM_TABLES_DOC change by package install"
...
#exit_program
 ...
 SET dm_err->eproc = "Maintenance on DM_TABLES_DOC Complete"
 CALL final_disp_msg(iutdp_logfile_prefix)
END GO
