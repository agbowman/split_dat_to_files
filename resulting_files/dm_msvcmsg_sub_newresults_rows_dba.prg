CREATE PROGRAM dm_msvcmsg_sub_newresults_rows:dba
 ...
 SET reply->table_name = "SUBSCRIPTION_NEW_RESULTS"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(snr.subscription_new_results_id)
   FROM subscription_new_results snr
   WHERE snr.subscription_new_results_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(snr.subscription_new_results_id)
  FROM subscription_new_results snr
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    snr.rowid
    FROM subscription_new_results snr
    WHERE snr.subscription_new_results_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists("snr.expiration_dt_tm < cnvtdatetime(curdate, curtime3)","snr")
     )
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
     ...
     ENDIF
     reply->rows[v_rows].row_id = snr.rowid
