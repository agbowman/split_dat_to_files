CREATE PROGRAM dm2_replicate_data_sync:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drds_reapply_templates(null) = i2
 DECLARE drds_insert_rst_chkpnt(dirc_chkpnt_name=vc,dirc_chkpnt_status=i2) = i2
 IF (check_logfile("dm2_rep_data_sync",".log","dm2_replicate_data_sync")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning data sync following database replicate..."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (validate(drrr_responsefile_in_use,0)=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "The replicate data sync process can only be executed where response file is leveraged (via dm2_rrr)."
  SET dm_err->eproc = "Verify script execution context."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 SET dm2_install_schema->dbase_name = cnvtupper(drrr_rf_data->tgt_db_name)
 ...
 IF ((xxcclseclogin->loggedin=0))
  EXECUTE cclseclogin2 drrr_rf_data->tgt_high_priv_user, drrr_rf_data->tgt_domain_name, drrr_rf_data
  ->tgt_high_priv_user_pwd
  IF ((xxcclseclogin->loggedin=0))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("CCL security login failed (User = ",trim(drrr_rf_data->
     tgt_high_priv_user)," / Domain = ",trim(drrr_rf_data->tgt_domain_name),").")
   SET dm_err->eproc = "Executing CCL Security Login."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ELSE
   SET dm_err->eproc = "CCL Security Login established."
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
 ELSE
  SET dm_err->eproc = "CCL Security Login already established."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF ((drrr_rf_data->tgt_db_copy_type="REFERENCE")
  AND (drrr_rf_data->tgt_reapply_sched_templates="YES"))
  IF (drds_reapply_templates(null)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 GO TO exit_program
 SUBROUTINE drds_reapply_templates(null)
   DECLARE drt_end_dt_tm = f8 WITH protect, noconstant(0.0)
   DECLARE drt_cnt = i2 WITH protect, noconstant(0)
   DECLARE drt_name = vc WITH protect, noconstant("RESET_DATA")
   DECLARE drt_ts = f8 WITH protect, noconstant(0.0)
   DECLARE drt_handle = vc WITH protect, noconstant("DM2NOTSET")
   DECLARE drt_status = vc WITH protect, noconstant("DM2NOTSET")
   DECLARE drt_code_value = f8 WITH protect, noconstant(0.0)
   DECLARE drt_cv_status = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Beginning Reapply Scheduling Templates..."
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (((checkprg("SCH_UTL_RESET_APPLY_BEG_DATE")=0) OR (((checkprg("SCH_UTL_REAPPLY_DEFSCHED_ALL")=0
   ) OR (((checkprg("SCH_UTL_SYSTEM_PREF")=0) OR (checkprg("SCH_OPS_FREQ_DEFSCHED")=0)) )) )) )
    SET dm_err->eproc = "Verify Scheduling script(s) exist in dictionary."
    SET dm_err->emsg = "Scheduling script(s) to reapply templates missing from dictionary."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET drt_cv_status = uar_get_meaning_by_codeset(23002,"DEFSCHED",1,drt_code_value)
   IF (((drt_cv_status != 0) OR (drt_code_value <= 0.0)) )
    SET dm_err->eproc =
    "Issue UAR call to verify access to CODE_VALUE content (via CPM Code Cache Manager)."
    SET dm_err->emsg = "Unable to retrieve code_value via UAR call."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc =
   'Get earliest "completed" timestamp when Scheduling tables were imported (from dm2_ddl_ops_log).'
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=
    (SELECT
     ...
     WHERE r.process_option="CLIN COPY"
      AND r.last_checkpoint="CLIN_COPY_COMPLETE"))
     AND ddol.op_type="EXPORT/IMPORT*"
     AND ddol.table_name IN ("SCH_FREQ", "SCH_DEF_APPLY", "SCH_NOTIFY", "SCH_APPLY_LIST")
     AND ddol.status="COMPLETE"
    DETAIL
     IF (drt_end_dt_tm=0.0)
      drt_end_dt_tm = cnvtdatetime(ddol.end_dt_tm)
     ELSEIF (cnvtdatetime(ddol.end_dt_tm) < cnvtdatetime(drt_end_dt_tm))
      drt_end_dt_tm = cnvtdatetime(ddol.end_dt_tm)
     ENDIF
     drt_cnt = (drt_cnt+ 1)
    WITH nocounter
   ...
   ENDIF
   IF (drt_cnt != 4)
    SET dm_err->emsg = "Unable to retrieve all scheduling tables import timestamps."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("Timestamp of Scheduling tables imports = ",format(drt_end_dt_tm,
       "mm/dd/yyyy hh:mm:ss;;d")))
   ENDIF
   SET dm_err->eproc = "Get reappy scheduling template checkpoint (from dm_info)."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET drt_cnt = 0
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="DM2_REAPPLY_SCHED_TEMPLATES"
     AND di.info_name IN ("RESET_DATA", "REAPPLY_TEMPLATES")
    DETAIL
     drt_name = di.info_name, drt_ts = cnvtdatetime(di.info_date), drt_handle = di.info_char,
     drt_status = evaluate(di.info_number,1.0,"RUNNING",2.0,"COMPLETE"), drt_cnt = (drt_cnt+ 1)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (drt_cnt > 1)
    SET dm_err->emsg =
    "Multiple checkpoints found.  Unable to determine state of Reapply Scheduling Templates automated process."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Evaluate reappy scheduling template checkpoint status."
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (drt_status="RUNNING")
    SET drt_status = dm2_get_appl_status(drt_handle)
    IF (drt_status="E")
     RETURN(0)
    ELSE
     IF (drt_status="A")
      SET drt_status = "RUNNING"
     ELSE
   ...
   IF (drt_status="RUNNING")
    SET dm_err->err_ind = 1
    SET dm_err->emsg =
    "Another process is currently reapplying scheduling templates.  Exiting Script."
    SET dm_err->eproc = "Determining if reapply scheduling templates actively running."
    SET dm_err->user_action = "Please wait until other process completes and try again."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (drt_status != "DM2NOTSET"
    AND cnvtdatetime(drt_ts) < cnvtdatetime(drt_end_dt_tm))
    SET dm_err->eproc = "Reset reappy scheduling template checkpoint (Scheduling tables reimported)."
    CALL disp_msg(" ",dm_err->logfile,0)
    SET drt_name = "RESET_DATA"
    SET drt_ts = 0.0
    SET drt_handle = "DM2NOTSET"
    SET drt_status = "DM2NOTSET"
   ENDIF
   IF (drt_name="RESET_DATA")
    IF ((drrr_rf_data->tgt_reapply_sched_templates_reset="YES"))
     IF (drt_status="DM2NOTSET")
      SET dm_err->eproc =
      "Executing reset begin date for Reapply Scheduling Templates (script SCH_UTL_RESET_APPLY_BEG_DATE)..."
      CALL disp_msg(" ",dm_err->logfile,0)
      EXECUTE sch_utl_reset_apply_beg_date
      IF (check_error("Reset begin date for Scheduling Templates...")=1)
       SET dm_err->eproc =
       'Evaluate execution of Reapply Scheduling Templates "reset begin date" step (script SCH_UTL_RESET_APPLY_BEG_DATE).'
       SET dm_err->emsg =
       "Errors detected during script execution. Resolve errors before restarting this process."
       SET dm_err->err_ind = 1
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ENDIF
      COMMIT
     ELSEIF (drt_status IN ("FAILED", "COMPLETE"))
      SET dm_err->eproc = "Skipping reset begin date for Scheduling Templates (previously executed)."
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
    ELSE
     SET dm_err->eproc =
     "Skipping reset begin date for Scheduling Templates as requested via response file."
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    IF (drt_status="DM2NOTSET")
     SET dm_err->eproc =
     "Executing reset data for Reapply Scheduling Templates (script SCH_UTL_REAPPLY_DEFSCHED_ALL)..."
     CALL disp_msg(" ",dm_err->logfile,0)
     IF (drds_insert_rst_chkpnt(drt_name,1)=0)
      RETURN(0)
     ENDIF
     EXECUTE sch_utl_reapply_defsched_all 0
     IF (check_error("Reset data for Scheduling Templates...")=1)
      SET dm_err->eproc =
      'Evaluate execution of Reapply Scheduling Templates "reset data" step (script SCH_UTL_REAPPLY_DEFSCHED_ALL).'
      SET dm_err->emsg =
      "Errors detected during script execution. Resolve errors before restarting this process."
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
     IF (drds_insert_rst_chkpnt(drt_name,2)=0)
      RETURN(0)
     ENDIF
    ELSEIF (drt_status="COMPLETE")
     SET dm_err->eproc = "Skipping reset data for Scheduling Templates (previously executed)."
     CALL disp_msg(" ",dm_err->logfile,0)
    ELSEIF (drt_status="FAILED")
     SET dm_err->eproc =
     'Evaluate execution of Reapply Scheduling Templates "reset data" step (script SCH_UTL_REAPPLY_DEFSCHED_ALL).'
     SET dm_err->emsg =
     "Errors detected during previous script execution. Resolve errors before restarting this process."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    SET drt_name = "REAPPLY_TEMPLATES"
    SET drt_ts = 0.0
    SET drt_handle = "DM2NOTSET"
    SET drt_status = "DM2NOTSET"
   ELSE
   ...
   IF (drt_status="DM2NOTSET")
    SET dm_err->eproc =
    "Executing setting of DEFRANGE system pref for scheduling templates (script SCH_UTL_SYSTEM_PREF)..."
    CALL disp_msg(" ",dm_err->logfile,0)
    EXECUTE sch_utl_system_pref "DEFRANGE", drrr_rf_data->tgt_reapply_sched_templates_range
    IF (check_error("Set DEFRANGE system pref for scheduling templates...")=1)
     SET dm_err->eproc =
     'Evaluate execution of Reapply Scheduling Templates "set DEFRANGE system pref" step (script SCH_UTL_SYSTEM_PREF).'
     SET dm_err->emsg =
     "Errors detected during script execution. Resolve errors before restarting this process."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSEIF (drt_status IN ("FAILED", "COMPLETE"))
    SET dm_err->eproc =
    "Skipping setting of DEFRANGE system pref for scheduling templates (previously executed)."
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF (drt_status IN ("DM2NOTSET", "FAILED"))
    IF (drt_status="DM2NOTSET")
     SET dm_err->eproc = "Executing reapply scheduling templates (script SCH_OPS_FREQ_DEFSCHED)..."
     CALL disp_msg(" ",dm_err->logfile,0)
    ELSE
     SET dm_err->eproc =
     "Re-Executing reapply scheduling templates (script SCH_OPS_FREQ_DEFSCHED)..."
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    IF (drds_insert_rst_chkpnt(drt_name,1)=0)
     RETURN(0)
    ...
    IF (check_error("Reapply scheduling templates...")=1)
     SET dm_err->eproc =
     'Evaluate execution of Reapply Scheduling Templates "reapply templates" step (script SCH_OPS_FREQ_DEFSCHED).'
     SET dm_err->emsg =
     "Errors detected during script execution. Resolve errors before restarting this process."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ELSEIF (drt_status="COMPLETE")
    SET dm_err->eproc = "Skipping reapply of Scheduling Templates (previously executed)."
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drds_insert_rst_chkpnt(dirc_chkpnt_name,dirc_chkpnt_status)
   SET dm_err->eproc = 'Query for Scheduling "Reapply Templates" checkpoint row.'
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="DM2_REAPPLY_SCHED_TEMPLATES"
     AND di.info_name IN ("RESET_DATA", "REAPPLY_TEMPLATES")
   ...
   ENDIF
   IF (curqual > 0)
    SET dm_err->eproc = 'Remove Scheduling "Reapply Templates" checkpoint row.'
    DELETE  FROM dm_info di
     WHERE di.info_domain="DM2_REAPPLY_SCHED_TEMPLATES"
      AND di.info_name IN ("RESET_DATA", "REAPPLY_TEMPLATES")
     WITH nocounter
    ...
    ENDIF
   ENDIF
   SET dm_err->eproc = concat('Insert Scheduling "Reapply Templates" checkpoint row (',
    dirc_chkpnt_name,") with ",evaluate(dirc_chkpnt_status,1,"RUNNING",2,"COMPLETE")," status.")
   CALL disp_msg(" ",dm_err->logfile,0)
   INSERT  FROM dm_info di
    SET di.info_domain = "DM2_REAPPLY_SCHED_TEMPLATES", di.info_name = dirc_chkpnt_name, di.info_char
      = currdbhandle,
     di.info_number = dirc_chkpnt_status, di.info_date = cnvtdatetime(curdate,curtime3)
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM2_REPLICATE_DATA_SYNC COMPLETED"
 ENDIF
 CALL final_disp_msg("dm2_rep_data_sync")
END GO
