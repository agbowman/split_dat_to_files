CREATE PROGRAM bhs_ma_saved_powernote:dba
 PROMPT
 ...
 IF (( $SPREADVIEW=0))
  ...
  DECLARE noresults(ncalc=i2) = f8 WITH protect
  DECLARE noresultsabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
  ...
  SUBROUTINE detailsecabs(ncalc,offsetx,offsety)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE noresults(ncalc)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = noresultsabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE noresultsabs(ncalc,offsetx,offsety)
    ...
    IF (ncalc=rpt_render)
     ...
     SET _oldfont = uar_rptsetfont(_hreport,_times100)
     SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("No Unsigned Powernotes Were Found",
       char(0)))
     SET _yoffset = (offsety+ sectionheight)
    ENDIF
    RETURN(sectionheight)
  ...
  SUBROUTINE initializereport(dummy)
    ...
    SET rptreport->m_reportname = "BHS_MA_SAVED_POWERNOTE"
  ...
  SELECT
   ...
   PLAN (s
    ...
    JOIN (e
    WHERE e.encntr_id=s.encounter_id
     AND (e.loc_nurse_unit_cd= $MF_UNIT)
     AND e.disch_dt_tm IS NOT null)
    JOIN (p
    WHERE p.person_id=s.author_id)
    JOIN (pe
 ...
 ELSE
  ...
  IF (curqual=0)
   SELECT INTO  $OUTDEV
    HEAD PAGE
     col + 50, "No Unsigned Powernotes Found"
    WITH nocounter
   ;end select
  ENDIF
 ENDIF
