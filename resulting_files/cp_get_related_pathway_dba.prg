CREATE PROGRAM cp_get_related_pathway:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Concept Code:" = 0.0
  WITH outdev, concept_cd
 FREE RECORD related_pathways
 RECORD related_pathways(
   1 cnt = i4
   1 pathways[*]
     2 cp_pathway_id = f8
     2 cp_pathway_name = vc
     2 cp_pathway_type = vc
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CP_GET_RELATED_PATHWAY"
 DECLARE getrelatedpathway(null) = null WITH protect
 DECLARE lstat = i4 WITH protect, noconstant(0)
 DECLARE concept_cd = f8 WITH protect, constant( $CONCEPT_CD)
 ...
 DECLARE active_pathway_status_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003198,
   "ACTIVE"))
 CALL getrelatedpathway(null)
 SUBROUTINE getrelatedpathway(null)
   CALL log_message("In getRelatedPathway()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   IF (concept_cd != 0.0)
    SELECT INTO "nl:"
     FROM cp_pathway cp
     WHERE cp.cp_concept_cd=concept_cd
      AND ((cp.pathway_status_cd+ 0)=active_pathway_status_cd)
     HEAD cp.cp_pathway_id
      related_pathways->cnt += 1, stat = alterlist(related_pathways->pathways,related_pathways->cnt),
      related_pathways->pathways[related_pathways->cnt].cp_pathway_id = cp.cp_pathway_id,
      related_pathways->pathways[related_pathways->cnt].cp_pathway_name = cp.pathway_name
      IF (uar_get_code_display(cp.pathway_type_cd)="Care Process Model")
       related_pathways->pathways[related_pathways->cnt].cp_pathway_type = "CPM"
      ELSE
       related_pathways->pathways[related_pathways->cnt].cp_pathway_type = uar_get_code_display(cp
        .pathway_type_cd)
      ENDIF
     WITH nocounter
    ;end select
   ENDIF
   IF ((related_pathways->cnt > 0))
    SET related_pathways->status_data.status = "S"
    SET related_pathways->status_data.subeventstatus[1].targetobjectname = "Data Found"
   ELSE
    SET related_pathways->status_data.status = "Z"
    SET related_pathways->status_data.subeventstatus[1].targetobjectname = "No-Data Found"
   ENDIF
...
#exit_script
 IF (validate(debug_ind,0)=1)
  CALL echorecord(related_pathways)
 ENDIF
 IF (( $OUTDEV != "NOFORMS"))
  CALL putjsonrecordtofile(related_pathways)
  FREE RECORD related_pathways
 ENDIF
