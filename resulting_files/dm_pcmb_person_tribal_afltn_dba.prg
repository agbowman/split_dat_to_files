CREATE PROGRAM dm_pcmb_person_tribal_afltn:dba
 ...
 RECORD rreclist(
   1 from_rec[*]
     ...
     2 tribal_entity_nomenclature_id = f8
   1 to_rec[*]
     2 to_id = f8
     2 active_ind = i4
     2 active_status_cd = f8
 ...
 DECLARE del_to(s_df_pk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) = i4
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "PERSON_TRIBAL_AFLTN"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_PERSON_TRIBAL_AFLTN"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count_from].from_id = frm.person_tribal_afltn_id, rreclist->from_rec[
   v_cust_count_from].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count_from].
   active_status_cd = frm.active_status_cd,
   rreclist->from_rec[v_cust_count_from].tribal_entity_nomenclature_id = frm
   .tribal_entity_nomenclature_id
 ...
 IF (v_cust_count_from > 0)
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count_to].to_id = tu.person_tribal_afltn_id, rreclist->to_rec[
    v_cust_count_to].active_ind = tu.active_ind, rreclist->to_rec[v_cust_count_to].active_status_cd
     = tu.active_status_cd,
    rreclist->to_rec[v_cust_count_to].tribal_entity_nomenclature_id = tu
    .tribal_entity_nomenclature_id
  ...
  ENDIF
  FOR (v_cust_loopcount = 1 TO v_cust_count_from)
    SET to_id_present = 0
    FOR (v_cust_loopcount2 = 1 TO v_cust_count_to)
      IF ((rreclist->from_rec[v_cust_loopcount].tribal_entity_nomenclature_id=rreclist->to_rec[
      v_cust_loopcount2].tribal_entity_nomenclature_id))
       SET to_id_present = 1
      ENDIF
    ENDFOR
    IF (to_id_present=1)
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =i4)
   UPDATE  FROM person_tribal_afltn frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.active_status_dt_tm =
     ...
     cnvtdatetime(trans_dt_tm)
    WHERE frm.person_tribal_afltn_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "PERSON_TRIBAL_AFLTN"
   ...
   IF (curqual=0)
    ...
    SET request->error_message = substring(1,132,build(
      "Could not inactivate the row for person_tribal_afltn with pk val=",s_df_pk_id))
    RETURN(0)
   ENDIF
   IF (add_hist(s_df_pk_id)=0)
    SET failed = insert_error
    SET request->error_message = substring(1,132,build(
      "Could not insert history for person_tribal_afltn with pk=",s_df_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM person_tribal_afltn frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.person_tribal_afltn_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
   ...
   IF (curqual=0)
    ...
    SET request->error_message = substring(1,132,build(
      "Could not update the row for person_tribal_afltn with pk val=",s_uf_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (add_hist(s_at_to_parent_pk_id=f8) =i4)
   ...
   SET dm_cmb_cust_cols2->add_col_val[1].col_name = "PERSON_TRIBAL_AFLTN_HIST_ID"
   ...
   SET dm_cmb_cust_cols2->add_col_val[3].col_name = "UPDT_DT_TM"
   ...
   SET dm_cmb_cust_cols2->where_col_val[1].col_name = "PERSON_TRIBAL_AFLTN_ID"
   SET dm_cmb_cust_cols2->where_col_val[1].col_value = build(s_at_to_parent_pk_id)
   IF (size(dm_cmb_cust_cols2->col,5)=0)
    SET dm_cmb_cust_cols2->tbl_name = "PERSON_TRIBAL_AFLTN_HIST"
    SET dm_cmb_cust_cols2->sub_select_from_tbl = "PERSON_TRIBAL_AFLTN"
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0)
    ...
    SET dcipht_request->transaction_reason_txt = "PERSON_CMB_PERSON_TRIBAL_AFFILIATION"
