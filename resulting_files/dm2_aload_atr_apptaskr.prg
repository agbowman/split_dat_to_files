CREATE PROGRAM dm2_aload_atr_apptaskr
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_apptaskr",".log","dm2_aload_atr_apptaskr")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_atr_apptaskr"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daatr_cnt = i4 WITH protect, noconstant(0)
 DECLARE daatr_str = vc WITH protect, noconstant("")
 IF (dac_pop_coldic_rec("DM_OCD_APP_TASK_R")=0)
  GO TO exit_program
 ENDIF
 ...
 CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
   " dcv ,(dummyt d with seq = size(requestin->list_0,5))"),0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_atr_apptaskr completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_atr_apptaskr")
END GO
