CREATE PROGRAM dcp_get_tasks_by_person_sec:dba
 DECLARE program_version = vc WITH private, constant("004")
 ...
 IF (person_count > 0)
  ...
  IF (task_type_count > 0)
   SET expandclauses = concat(trim(expandclauses),
    " and expand(task_type_index, 1, task_type_count, ta.task_type_cd+0,",
    "request->task_type_list[task_type_index].task_type_cd)")
  ENDIF
  IF (task_status_count > 0)
   SET expandclauses = concat(trim(expandclauses),
    " and expand(task_status_index, 1, task_status_count, ta.task_status_cd+0,",
    "request->task_status_list[task_status_index].status_cd)")
  ENDIF
  IF (task_class_count > 0)
   SET expandclauses = concat(trim(expandclauses),
    " and expand(task_class_index, 1, task_class_count, ta.task_class_cd+0,",
    "request->task_class_list[task_class_index].class_cd)")
  ENDIF
  IF (location_count > 0)
   SET expandclauses = concat(trim(expandclauses),
    " and expand(location_index, 1, location_count, ta.location_cd+0,",
    "request->location_list[location_index].location_cd)")
  ENDIF
  DECLARE idx = i4
  DECLARE ntotal = i4
  DECLARE ntotal2 = i4
  ...
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(person_count)),
    task_activity ta
   PLAN (d)
    JOIN (ta
    WHERE (ta.person_id=request->person_list[d.seq].person_id)
     AND (ta.person_id > context->last_person_id)
