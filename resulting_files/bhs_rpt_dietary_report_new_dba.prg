CREATE PROGRAM bhs_rpt_dietary_report_new:dba
 PROMPT
 ...
 DECLARE non_sel_tray_cd = f8
 DECLARE meal_w_assist_cd = f8
 DECLARE meal_delievery_cd = f8
 ...
 SET non_sel_tray_cd = uar_get_code_by("DISPLAYKEY",200,"MEALSERVICENONSELECTTRAYSERVICE")
 SET meal_w_assist_cd = uar_get_code_by("DISPLAYKEY",200,"MEALSERVICEASSISTWITHMENUSELECTION")
 SET meal_delievery_cd = uar_get_code_by("DISPLAYKEY",200,"MEALSERVICEDELIVERY")
 ...
 SELECT
  ...
  HEAD e.encntr_id
   ...
   diet->qual[cnt].has_diet_order = 0, diet->qual[cnt].has_clear_order = 0, diet->qual[cnt].
   has_non_sel_try = 0,
   diet->qual[cnt].has_w_assist = 0
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   WHERE (o.encntr_id=diet->qual[d.seq].encntr_id)
    AND o.active_ind=1
    AND (o.order_status_cd= $ORDER_STATUS)
    AND o.activity_type_cd IN (nut_services_consult_cd, supplements_cd, diets_cd, diet_spec_inst)
    AND  EXISTS (
   (SELECT
    oc.encntr_id
    FROM orders oc
    WHERE oc.encntr_id=o.encntr_id
     AND oc.catalog_cd=meal_delievery_cd
     AND oc.active_ind=1
     AND (oc.order_status_cd= $ORDER_STATUS)
     AND (( EXISTS (
    (SELECT
     od.order_id
     ...
     WHERE od.order_id=oc.order_id
      ...
      AND od.oe_field_display_value="Non-select Tray Service"))) OR (oc.catalog_cd=non_sel_tray_cd))
   )))
  ORDER BY o.encntr_id, o.order_id
  HEAD o.encntr_id
   ord_cnt = 0
  DETAIL
   CALL echo(o.order_mnemonic), ord_cnt = (ord_cnt+ 1), comment_cnt = 0
   IF (mod(ord_cnt,10)=1)
    stat = alterlist(diet->qual[d.seq].orders,(ord_cnt+ 9))
   ENDIF
   diet->qual[d.seq].orders[ord_cnt].order_id = o.order_id, diet->qual[d.seq].orders[ord_cnt].
  ...
  PLAN (d
   ...
   (SELECT
    ...
    (SELECT
     ...
     WHERE od.order_id=oc.order_id
      AND od.oe_field_meaning="OTHER"
      AND od.oe_field_display_value="Assist with menu selection"))) OR (o.catalog_cd=meal_w_assist_cd
    )) )))
  ORDER BY o.encntr_id, o.order_id
  HEAD o.encntr_id
   ord_cnt = 0
 ...
 SELECT INTO  $OUTDEV
  ...
  PLAN (d
   WHERE ((( $TPN_OPTION=1)) OR (((( $TPN_OPTION=2)
    ...
    AND (diet->qual[d.seq].has_clear_order=1)) OR (((( $TPN_OPTION=5)
    AND (diet->qual[d.seq].has_non_sel_try=1)) OR (( $TPN_OPTION=6)
