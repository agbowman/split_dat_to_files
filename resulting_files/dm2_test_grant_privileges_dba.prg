CREATE PROGRAM dm2_test_grant_privileges:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE tst_log_pre = vc WITH protect, constant("dm2_test_grant")
 DECLARE tst_tname = vc WITH protect, noconstant(" ")
 IF (check_logfile(tst_log_pre,".log","DM2_TEST_GRANT_PRIVILEGES LOGFILE")=0)
  GO TO exit_program
 ENDIF
 CALL echo('1.  Testing dm2_grant_privileges.prg using "PERSON4859" as input parameter')
 SET tst_tname = "PERSON4859 "
 SET dm_err->eproc = 'Testing dm2_grant_privileges.prg using "PERSON4859" as input parameter'
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_grant_privileges tst_tname
 CALL echo("  ")
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 CALL echo(
  "2.  Testing dm2_grant_privileges.prg without providing an input parameter - causes error!")
 SET tst_tname = " "
 SET dm_err->eproc = "Testing dm2_grant_privileges.prg without providing an input parameter"
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_grant_privileges
 SET dm_err->err_ind = 0
 CALL echo("  ")
 CALL echo('3.  Testing dm2_grant_privileges.prg using " " as input parameter')
 SET tst_tname = " "
 SET dm_err->eproc = 'Testing dm2_grant_privileges.prg using " " as input parameter'
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_grant_privileges tst_tname
 SET dm_err->err_ind = 0
 CALL echo("  ")
 CALL echo('4.  Testing dm2_grant_privileges.prg using "ABN*" as input parameter')
 SET tst_tname = "ABN*"
 SET dm_err->eproc = 'Testing dm2_grant_privileges.prg using "ABN*" as input parameter'
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_grant_privileges tst_tname
 CALL echo("  ")
 CALL echo('5.  Testing dm2_grant_privileges.prg using "PRSN" as input parameter')
 SET tst_tname = "PRSN"
 SET dm_err->eproc = 'Testing dm2_grant_privileges.prg using "PRSN" as input parameter'
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_grant_privileges tst_tname
 CALL echo("  ")
#exit_program
 SET dm_err->eproc = "Ending Test Grant Privileges Process.  Process Completed Successfully."
