CREATE PROGRAM dm2_mig_final_trans:dba
 ...
 END ;Subroutine
 SET width = 132
 DECLARE dmft_archive_job_work(null) = i2
 DECLARE dmft_prompt_wait(dpaw_selection_ret=vc(ref),dpaw_mode=vc,dpaw_info=vc) = i2
 DECLARE dmft_get_submit_time(dgst_time_ret=dq8(ref)) = i2
 DECLARE dmft_final_trans_work(dftw_final_insert_ind=i2) = i2
 DECLARE dmft_select_final_trans(dsft_exists=i2(ref),dsft_inst_nbr=i2) = i2
 DECLARE dmft_tspace_work(null) = i2
 DECLARE dmft_insert_row(dir_info_domain=vc,dir_info_name=vc) = i2
 DECLARE dmft_log_switch(dlw_nbr_to_switch=i4) = i2
 DECLARE dmft_log_status(dls_mode=vc) = i2
 DECLARE dmft_gather_info(null) = i2
 DECLARE dmft_disp_report(null) = i2
 DECLARE dmft_disp_screen(null) = i2
 DECLARE dmft_eval_checkpoints(null) = i2
 DECLARE dmft_trg_owner = vc WITH protect, noconstant(" ")
 DECLARE dmft_trg_name = vc WITH protect, noconstant(" ")
 DECLARE dmft_strng = vc WITH protect, noconstant(" ")
 DECLARE dmft_exists = i2 WITH protect, noconstant(0)
 DECLARE dmft_exit_ind = i2 WITH protect, noconstant(0)
 DECLARE dmft_selection = vc WITH protect, noconstant(" ")
 DECLARE dmft_cappump_used = i2 WITH protect, noconstant(0)
 DECLARE dmft_monitor_only = i2 WITH protect, noconstant(0)
 DECLARE dmft_mock_trans = i2 WITH protect, noconstant(0)
 DECLARE dmft_write_trans_trail_ind = i2 WITH protect, noconstant(1)
 DECLARE dmft_src_db_os = vc WITH protect, noconstant("")
 DECLARE dmft_idx = i4 WITH protect, noconstant(0)
 DECLARE dmft_del_stopped_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmft_mode_fail = i2 WITH protect, noconstant(0)
 IF ((validate(dm2_mig_finaltrans_monitor_only,- (1)) != - (1)))
  SET dmft_monitor_only = 1
 ENDIF
 IF ((validate(dm2_mig_mock_finaltrans_ind,- (1)) != - (1)))
  SET dmft_mock_trans = 1
 ENDIF
 FREE RECORD dmft_cstr
 RECORD dmft_cstr(
   1 cstr_cnt = i2
   1 qual[*]
     2 cstr = vc
 ...
 FREE RECORD dmft_screen
 RECORD dmft_screen(
   1 ds_start_final_ind = i2
   1 ds_end_final_ind = i2
   1 ds_read_only_complete = i2
   1 ds_insert_final_trans = i2
   1 ds_archive_copy_init = i2
   1 ds_final_detected_in_trail = i2
   1 ds_delivery_fully_processed = i2
   1 ds_delivdi_started = i2
   1 ds_final_trans_in_tgt = i2
   1 ds_delivdi_stop = i2
   1 ds_stopped_delivdi = i2
   1 ds_capture_stop = i2
   1 ds_delivery_stop = i2
   1 ds_cappump_stop = i2
   1 ds_stopped_capture = i2
   1 ds_stopped_cappump = i2
   1 ds_stopped_delmgr = i2
   1 ds_stopped_capmgr = i2
   1 ds_stopped_delivery = i2
   1 ds_cappump_status = vc
   1 ds_capture_status = vc
   1 ds_mgr_status = vc
   1 ds_trailseq = vc
   1 ds_trailrba = vc
   1 ds_cappump_seq = vc
   ...
   1 deliv[*]
     ...
     2 seq = vc
     2 lag = dq8
   1 ds_delivery_idx = i4
   1 ds_delivcy_idx = i4
   1 ds_delivdi_idx = i4
   1 ds_delivo_idx = i4
   1 ds_delivch_idx = i4
   1 ds_delivce_idx = i4
   1 ds_delivery_mgr_status = vc
 )
 FREE RECORD dmft_trgs
 RECORD dmft_trgs(
   1 trgs_cnt = i2
   1 qual[*]
     2 owner = vc
 ...
 SET dmft_screen->ds_start_final_ind = 0
 SET dmft_screen->ds_end_final_ind = 0
 SET dmft_screen->ds_read_only_complete = 0
 SET dmft_screen->ds_insert_final_trans = 0
 SET dmft_screen->ds_archive_copy_init = 0
 SET dmft_screen->ds_final_detected_in_trail = 0
 SET dmft_screen->ds_delivery_fully_processed = 0
 SET dmft_screen->ds_delivdi_started = 0
 SET dmft_screen->ds_final_trans_in_tgt = 0
 SET dmft_screen->ds_delivdi_stop = 0
 SET dmft_screen->ds_stopped_delivdi = 0
 SET dmft_screen->ds_capture_stop = 0
 SET dmft_screen->ds_delivery_stop = 0
 SET dmft_screen->ds_cappump_stop = 0
 SET dmft_screen->ds_stopped_capture = 0
 SET dmft_screen->ds_stopped_cappump = 0
 SET dmft_screen->ds_stopped_delmgr = 0
 SET dmft_screen->ds_stopped_capmgr = 0
 SET dmft_screen->ds_stopped_delivery = 0
 SET dmft_screen->ds_cappump_status = "DM2NOTSET"
 SET dmft_screen->ds_capture_status = "DM2NOTSET"
 SET dmft_screen->ds_mgr_status = "DM2NOTSET"
 SET dmft_screen->ds_trailseq = "0"
 SET dmft_screen->ds_trailrba = "0"
 SET dmft_screen->ds_cappump_seq = "0"
 SET dmft_screen->ds_cappump_rba = "0"
 SET dmft_screen->ds_cappump_lag = "DM2NOTSET"
 SET dmft_screen->ds_capture_lag = "DM2NOTSET"
 SET dmft_screen->ds_delivery_mgr_status = "DM2NOTSET"
 SET dmft_screen->ds_delivery_idx = 0
 SET dmft_screen->ds_delivcy_idx = 0
 SET dmft_screen->ds_delivdi_idx = 0
 SET dmft_screen->ds_delivo_idx = 0
 SET dmft_screen->ds_delivch_idx = 0
 SET dmft_screen->ds_delivce_idx = 0
 SET stat = alterlist(dmft_screen->deliv,0)
 SET dmft_screen->deliv_cnt = 0
 SET stat = alterlist(dmft_trgs->qual,0)
 SET dmft_trgs->trgs_cnt = 0
 IF ((dm2_install_schema->curprog="NONE"))
  SET dmft_mode_fail = 1
 ENDIF
 IF (check_logfile("dm2_migfnltrs",".log","dm2_mig_final_trans")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_final_trans"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ENDIF
 IF (dmft_mode_fail=1)
  SET dm_err->logfile = "DM2NOTSET"
  SET dm_err->eproc = "Validate script standalone status."
  SET dm_err->emsg = "DM2_MIG_FINAL_TRANS cannot be run standalone!"
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Get delivery names stored in admin."
 SELECT INTO "nl:"
  FROM dm2_admin_dm_info d
  WHERE d.info_domain="DM2MIG_DELDATA_NAME"
  ORDER BY d.info_name
  DETAIL
   dmft_screen->deliv_cnt = (dmft_screen->deliv_cnt+ 1), stat = alterlist(dmft_screen->deliv,
    dmft_screen->deliv_cnt), dmft_screen->deliv[dmft_screen->deliv_cnt].name = cnvtupper(trim(d
     .info_name)),
   dmft_screen->deliv[dmft_screen->deliv_cnt].status = "DM2NOTSET", dmft_screen->deliv[dmft_screen->
   deliv_cnt].rba = "DM2NOTSET", dmft_screen->deliv[dmft_screen->deliv_cnt].seq = "DM2NOTSET",
   dmft_screen->deliv[dmft_screen->deliv_cnt].lag = cnvtdatetimeutc("01-JAN-1900 12:00:00",2)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 SET dmft_idx = 0
 SET dmft_screen->ds_delivery_idx = locateval(dmft_idx,1,dmft_screen->deliv_cnt,"DELIVERY",
  dmft_screen->deliv[dmft_idx].name)
 SET dmft_idx = 0
 SET dmft_screen->ds_delivdi_idx = locateval(dmft_idx,1,dmft_screen->deliv_cnt,"DELIVDI",dmft_screen
  ->deliv[dmft_idx].name)
 SET dmft_idx = 0
 SET dmft_screen->ds_delivo_idx = locateval(dmft_idx,1,dmft_screen->deliv_cnt,"DELIVO",dmft_screen->
  deliv[dmft_idx].name)
 SET dmft_idx = 0
 SET dmft_screen->ds_delivce_idx = locateval(dmft_idx,1,dmft_screen->deliv_cnt,"DELIVCE",dmft_screen
  ->deliv[dmft_idx].name)
 SET dmft_idx = 0
 SET dmft_screen->ds_delivcy_idx = locateval(dmft_idx,1,dmft_screen->deliv_cnt,"DELIVCY",dmft_screen
  ->deliv[dmft_idx].name)
 SET dmft_idx = 0
 SET dmft_screen->ds_delivch_idx = locateval(dmft_idx,1,dmft_screen->deliv_cnt,"DELIVCH",dmft_screen
  ->deliv[dmft_idx].name)
 IF ((((dmft_screen->ds_delivery_idx=0)) OR ((((dmft_screen->ds_delivdi_idx=0)) OR ((((dmft_screen->
 ds_delivo_idx=0)) OR ((((dmft_screen->ds_delivce_idx=0)) OR ((((dmft_screen->ds_delivcy_idx=0)) OR (
 (dmft_screen->ds_delivch_idx=0))) )) )) )) )) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Not all default deliveries found in Admin dm_info."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 IF (dmft_monitor_only=0)
  IF (dmft_eval_checkpoints(null)=0)
   GO TO exit_program
  ENDIF
  IF ((dmft_screen->ds_end_final_ind=1))
   SET dm_err->eproc = "Final Transaction process already completed."
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_program
  ENDIF
  IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
   ...
   ENDIF
  ENDIF
  SET dmft_src_db_os = dm2_sys_misc->cur_db_os
  IF ((dmr_mig_data->src_node_cnt > 1))
   FOR (dmft_cnt = 1 TO dmr_mig_data->src_node_cnt)
     IF ((dmr_mig_data->src_nodes[dmft_cnt].instance_number > 1))
      SET dm2_install_schema->dbase_name = dmr_mig_data->src_nodes[dmft_cnt].instance_name
      SET dm2_install_schema->u_name = "V500"
      EXECUTE dm2_connect_to_dbase "PC"
      IF ((dm_err->err_ind=1))
       GO TO exit_program
      ENDIF
      SET dmft_cstr->cstr_cnt = (dmft_cstr->cstr_cnt+ 1)
      SET stat = alterlist(dmft_cstr->qual,dmft_cstr->cstr_cnt)
      SET dmft_cstr->qual[dmft_cstr->cstr_cnt].cstr = dm2_install_schema->connect_str
      SET dmft_cstr->qual[dmft_cstr->cstr_cnt].inst_nbr = dmr_mig_data->src_nodes[dmft_cnt].
      instance_number
     ENDIF
   ENDFOR
   IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
   ...
   ENDIF
  ENDIF
  IF ((dmft_screen->ds_start_final_ind=0))
   IF (dmft_insert_row("DM2MIG_DELDATA","START_FINAL_TRANS")=0)
    GO TO exit_program
   ENDIF
   IF (dmft_mock_trans=1)
    IF (dmft_insert_row("DM2MIG_DELDATA","START_MOCK_FINAL_TRANS")=0)
     GO TO exit_program
    ENDIF
   ENDIF
   SET dmft_screen->ds_start_final_ind = 1
  ENDIF
 ENDIF
 SET dm_err->eproc = "Check if CAPPUMP in use."
 SELECT INTO "nl:"
  FROM dm2_admin_dm_info d
  WHERE d.info_domain="DM2MIG_CAPDATA"
   AND d.info_name="CAPPUMP_STATUS"
   AND (d.info_number != - (1))
  DETAIL
   dmft_cappump_used = 1
  WITH nocounter
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat(evaluate(dmft_cappump_used,1,"Cappump in use.","Cappump not in use."))
 CALL disp_msg(" ",dm_err->logfile,0)
 WHILE (dmft_exit_ind=0)
   IF ((dmft_screen->ds_end_final_ind=1))
    IF (dmft_prompt_wait(dmft_selection,"MAIN","Final Transaction processing complete.")=0)
     GO TO exit_program
    ENDIF
   ELSE
    IF (dmft_prompt_wait(dmft_selection,"MAIN"," ")=0)
     GO TO exit_program
    ENDIF
   ENDIF
   IF (dmft_mock_trans=1
    AND dmft_write_trans_trail_ind=1)
    IF ((dmft_screen->deliv[dmft_screen->ds_delivdi_idx].seq="DM2NOTSET"))
     SET message = nowindow
     CALL clear(1,1)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "DELIVDI SeqNo has not been written to admin yet"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    IF (dmft_insert_row("DM2MIG_DELDATA",concat("MOCK_FINAL_TRANS_TRAIL:",dmft_screen->deliv[
      dmft_screen->ds_delivdi_idx].seq))=0)
     GO TO exit_program
    ENDIF
    SET dmft_write_trans_trail_ind = 0
   ENDIF
   IF (dmft_selection="Q")
    SET message = nowindow
    GO TO exit_program
   ELSEIF (dmft_selection="C")
    SET dmft_exit_ind = 1
   ENDIF
   IF (dmft_monitor_only=0)
    IF ((dmft_screen->ds_read_only_complete != 1)
     AND (dmft_screen->ds_start_final_ind=1))
     SELECT INTO "nl:"
      d.owner, d.trigger_name
      FROM dba_triggers d
      WHERE d.owner=currdbuser
       AND d.table_name="DM_INFO"
       AND ((d.trigger_name="TRG*LUTS") OR (d.trigger_name="TRG*TXNSCN"))
      DETAIL
       dmft_trgs->trgs_cnt = (dmft_trgs->trgs_cnt+ 1), stat = alterlist(dmft_trgs->qual,dmft_trgs->
        trgs_cnt), dmft_trgs->qual[dmft_trgs->trgs_cnt].owner = d.owner,
       dmft_trgs->qual[dmft_trgs->trgs_cnt].name = d.trigger_name
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
     IF ((dmft_trgs->trgs_cnt > 0))
      IF (dctx_set_context("TRG_LUTS_CHK","NO")=0)
       GO TO exit_program
      ENDIF
      FOR (dmft_idx = 1 TO dmft_trgs->trgs_cnt)
        SET dmft_trg_owner = dmft_trgs->qual[dmft_idx].owner
        SET dmft_trg_name = dmft_trgs->qual[dmft_idx].name
        SET dmft_trg_name = concat(dmft_trg_owner,".",dmft_trg_name)
        SET dmft_strng = concat(" rdb asis (' ALTER TRIGGER ",dmft_trg_name," DISABLE ') go")
        IF (dm2_push_cmd(dmft_strng,1)=0)
         CALL echo(dmft_strng)
         GO TO exit_program
        ENDIF
      ENDFOR
     ENDIF
     IF ((dmr_mig_data->src_ora_level1 >= 11))
      ...
      SELECT INTO "nl:"
       FROM dba_tables d
       WHERE d.owner="V500"
        AND d.table_name="DM_TXN_TRACKING"
       DETAIL
        IF (d.read_only="YES")
         dmft_idx = 0
        ELSE
      ...
      ENDIF
      IF (dmft_idx > 0)
       SET dmft_strng = concat(
        " rdb asis (' alter /* DM2MIG SOURCE DDL ONLY */ table v500.dm_txn_tracking read only ') go")
       IF (dm2_push_cmd(dmft_strng,1)=0)
        GO TO exit_program
       ENDIF
      ENDIF
     ENDIF
     IF (dmft_tspace_work(null)=0)
      GO TO exit_program
     ENDIF
     IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","READ_ONLY_COMPLETE")=0)
      GO TO exit_program
     ENDIF
     SET dmft_screen->ds_read_only_complete = 1
    ENDIF
    IF ((dmft_screen->ds_delivdi_stop != 1)
     AND (dmft_screen->ds_read_only_complete=1))
     IF (dmft_insert_row("DM2MIG_DELDATA","STOP_DELIVDI")=0)
      GO TO exit_program
     ENDIF
     SET dmft_screen->ds_delivdi_stop = 1
    ENDIF
    IF ((dmft_screen->ds_delivdi_stop=1)
     AND (dmft_screen->ds_stopped_delivdi != 1))
     IF ( NOT ((dmft_screen->deliv[dmft_screen->ds_delivdi_idx].status IN ("RUNNING", "DM2NOTSET",
     "ABENDED"))))
      SET dmft_screen->ds_stopped_delivdi = 1
      IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","STOPPED_DELIVDI")=0)
       GO TO exit_program
      ENDIF
     ELSE
      SET dmft_screen->ds_stopped_delivdi = 2
     ENDIF
    ENDIF
    IF ((dmft_screen->ds_insert_final_trans != 1)
     AND (dmft_screen->ds_stopped_delivdi=1))
     IF (dmft_final_trans_work(0)=0)
      GO TO exit_program
     ENDIF
     IF (dmft_final_trans_work(1)=0)
      GO TO exit_program
     ENDIF
     IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","INSERT_FINAL_TRANS")=0)
      GO TO exit_program
     ENDIF
     SET dmft_screen->ds_insert_final_trans = 1
    ENDIF
    IF (dmft_src_db_os="AXP")
     IF ((dmft_screen->ds_archive_copy_init != 1)
      AND (dmft_screen->ds_insert_final_trans=1))
      SET dmft_screen->ds_archive_copy_init = 2
      IF (dmft_archive_job_work(null)=0)
       GO TO exit_program
      ENDIF
      IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","ARCHIVE_COPY_INIT")=0)
       GO TO exit_program
      ENDIF
      SET dmft_screen->ds_archive_copy_init = 1
     ENDIF
    ELSE
     IF ((dmft_screen->ds_archive_copy_init != 1)
      AND (dmft_screen->ds_insert_final_trans=1))
    ...
    ENDIF
    IF ((dmft_screen->ds_archive_copy_init=1)
     AND (dmft_screen->ds_final_detected_in_trail != 1))
     SET dmft_screen->ds_final_detected_in_trail = 2
    ENDIF
    IF ((dmft_screen->ds_capture_stop != 1)
     AND (dmft_screen->ds_final_detected_in_trail=1))
     IF (dmft_insert_row("DM2MIG_CAPDATA","STOP_CAPTURE")=0)
      GO TO exit_program
     ENDIF
     SET dmft_screen->ds_capture_stop = 1
    ENDIF
    IF ((dmft_screen->ds_cappump_stop != 1)
     AND (dmft_screen->ds_final_detected_in_trail=1))
     IF (dmft_insert_row("DM2MIG_CAPDATA","STOP_CAPPUMP")=0)
      GO TO exit_program
     ENDIF
     SET dmft_screen->ds_cappump_stop = 1
    ENDIF
    IF ((dmft_screen->ds_stopped_capture != 1)
     AND (dmft_screen->ds_capture_stop=1))
     IF ( NOT ((dmft_screen->ds_capture_status IN ("RUNNING", "DM2NOTSET", "ABENDED"))))
      SET dmft_screen->ds_stopped_capture = 1
      IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","STOPPED_CAPTURE")=0)
       GO TO exit_program
      ENDIF
     ELSE
      SET dmft_screen->ds_stopped_capture = 2
     ENDIF
    ENDIF
    IF ((dmft_screen->ds_stopped_cappump != 1)
     AND (dmft_screen->ds_cappump_stop=1)
     AND dmft_cappump_used=0)
     SET dmft_screen->ds_stopped_cappump = 1
     IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","STOPPED_CAPPUMP")=0)
      GO TO exit_program
     ENDIF
    ENDIF
    IF ((dmft_screen->ds_stopped_cappump != 1)
     AND (dmft_screen->ds_cappump_stop=1))
     IF ( NOT ((dmft_screen->ds_cappump_status IN ("RUNNING", "DM2NOTSET", "ABENDED"))))
      SET dmft_screen->ds_stopped_cappump = 1
      IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","STOPPED_CAPPUMP")=0)
       GO TO exit_program
      ENDIF
     ELSE
      SET dmft_screen->ds_stopped_cappump = 2
     ENDIF
    ENDIF
    IF ((dmft_screen->ds_stopped_cappump=1)
     AND (dmft_screen->ds_stopped_capture=1)
     AND (dmft_screen->ds_stopped_capmgr != 1))
     IF ( NOT ((dmft_screen->ds_mgr_status IN ("RUNNING", "DM2NOTSET"))))
      SET dmft_screen->ds_stopped_capmgr = 1
      IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","STOPPED_CAPMGR")=0)
       GO TO exit_program
      ENDIF
     ELSE
      SET dmft_screen->ds_stopped_capmgr = 2
     ENDIF
    ENDIF
    IF ((dmft_screen->ds_stopped_capmgr=1)
     AND (dmft_screen->ds_delivery_fully_processed != 1))
     SET dmft_screen->ds_delivery_fully_processed = 2
    ENDIF
    IF ((dmft_screen->ds_delivery_fully_processed=1)
     AND (dmft_screen->ds_delivdi_started != 1))
     SET dmft_screen->ds_delivdi_started = 2
    ENDIF
    IF ((dmft_screen->ds_final_trans_in_tgt != 1)
     AND (dmft_screen->ds_delivdi_started=1))
     IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
      IF (dmr_prompt_connect_data("TARGET","V500","CO")=0)
       GO TO exit_program
      ENDIF
     ENDIF
     IF (dmft_select_final_trans(dmft_exists,- (1))=0)
      RETURN(0)
     ENDIF
     IF ((dmft_exists=(dmr_mig_data->src_node_cnt+ 1)))
      IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","FINAL_TRANS_IN_TGT")=0)
       GO TO exit_program
      ENDIF
      SET dmft_screen->ds_final_trans_in_tgt = 1
      SET dm_err->eproc = "Final Transaction exists in TARGET"
      CALL disp_msg(" ",dm_err->logfile,0)
     ELSE
      IF ((dmft_screen->ds_delivdi_started=1))
       SET dmft_screen->ds_final_trans_in_tgt = 2
      ENDIF
     ENDIF
    ENDIF
    IF ((dmft_screen->ds_delivery_stop != 1)
     AND (dmft_screen->ds_final_trans_in_tgt=1))
     IF (dmft_insert_row("DM2MIG_DELDATA","STOP_DELIVERY")=0)
      GO TO exit_program
     ENDIF
     SET dmft_screen->ds_delivery_stop = 1
    ENDIF
    IF ((dmft_screen->ds_stopped_capmgr=1)
     AND (dmft_screen->ds_delivery_stop=1)
     AND (dmft_screen->ds_stopped_delivery != 1))
     SET dmft_del_stopped_cnt = 0
     FOR (dmft_idx = 1 TO dmft_screen->deliv_cnt)
       IF ( NOT ((dmft_screen->deliv[dmft_idx].status IN ("RUNNING", "DM2NOTSET", "ABENDED"))))
        SET dmft_del_stopped_cnt = (dmft_del_stopped_cnt+ 1)
       ENDIF
     ENDFOR
     IF ((dmft_del_stopped_cnt=dmft_screen->deliv_cnt))
      SET dmft_screen->ds_stopped_delivery = 1
      IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","STOPPED_DELIVERY")=0)
       GO TO exit_program
      ENDIF
     ELSE
      SET dmft_screen->ds_stopped_delivery = 2
     ENDIF
    ENDIF
    IF ((dmft_screen->ds_stopped_delivery=1)
     AND (dmft_screen->ds_stopped_delmgr != 1))
     IF ( NOT ((dmft_screen->ds_delivery_mgr_status IN ("RUNNING", "DM2NOTSET"))))
      SET dmft_screen->ds_stopped_delmgr = 1
      IF (dmft_insert_row("DM2MIG_FINAL_TRANS_CHECKPOINT","STOPPED_DELMGR")=0)
       GO TO exit_program
      ENDIF
     ELSE
      SET dmft_screen->ds_stopped_delmgr = 2
     ENDIF
    ENDIF
    IF ((dmft_screen->ds_stopped_delmgr=1)
     AND (dmft_screen->ds_end_final_ind=0))
     IF (dmft_insert_row("DM2MIG_DELDATA","END_FINAL_TRANS")=0)
      GO TO exit_program
     ENDIF
     SET dmft_screen->ds_end_final_ind = 1
    ENDIF
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dmft_screen)
   ENDIF
 ENDWHILE
 IF (dmft_monitor_only=0)
  IF ((dmft_screen->ds_end_final_ind=1))
   SET message = nowindow
   CALL clear(1,1)
   SET dm_err->eproc = "Final Transaction has been delivered to Target."
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
 ENDIF
 GO TO exit_program
 SUBROUTINE dmft_eval_checkpoints(null)
   SET dm_err->eproc = "Query for checkpoints"
   SELECT INTO "nl:"
    FROM dm2_admin_dm_info d
    WHERE d.info_domain IN ("DM2MIG_FINAL_TRANS_CHECKPOINT", "DM2MIG_DELDATA", "DM2MIG_CAPDATA")
     AND d.info_name IN ("END_FINAL_TRANS", "START_FINAL_TRANS", "READ_ONLY_COMPLETE",
    "INSERT_FINAL_TRANS", "ARCHIVE_COPY_INIT",
    "FINAL_TRANS_IN_TRAIL", "DELIVERIES_FULLY_PROCESSED", "DELIVDI_INITIATED", "FINAL_TRANS_IN_TGT",
    "STOP_CAPTURE",
    "STOP_CAPPUMP", "STOP_DELIVERY", "STOP_DELIVDI", "STOPPED_DELIVDI", "STOPPED_CAPTURE",
    "STOPPED_CAPPUMP", "STOPPED_DELIVERY", "STOPPED_CAPMGR", "STOPPED_DELMGR")
    DETAIL
     CASE (d.info_name)
      OF "START_FINAL_TRANS":
       dmft_screen->ds_start_final_ind = 1
      OF "END_FINAL_TRANS":
       dmft_screen->ds_end_final_ind = 1
      OF "STOP_DELIVDI":
       dmft_screen->ds_delivdi_stop = 1
      OF "STOPPED_DELIVDI":
       dmft_screen->ds_stopped_delivdi = 1
      OF "READ_ONLY_COMPLETE":
       dmft_screen->ds_read_only_complete = 1
      OF "INSERT_FINAL_TRANS":
       dmft_screen->ds_insert_final_trans = 1
      OF "ARCHIVE_COPY_INIT":
       dmft_screen->ds_archive_copy_init = 1
      OF "FINAL_TRANS_IN_TRAIL":
       dmft_screen->ds_final_detected_in_trail = 1
      OF "DELIVERIES_FULLY_PROCESSED":
       dmft_screen->ds_delivery_fully_processed = 1
      OF "DELIVDI_INITIATED":
       dmft_screen->ds_delivdi_started = 1
      OF "FINAL_TRANS_IN_TGT":
       dmft_screen->ds_final_trans_in_tgt = 1
      OF "STOP_CAPTURE":
       dmft_screen->ds_capture_stop = 1
      OF "STOP_CAPPUMP":
       dmft_screen->ds_cappump_stop = 1
      OF "STOP_DELIVERY":
       dmft_screen->ds_delivery_stop = 1
      OF "STOPPED_CAPTURE":
       dmft_screen->ds_stopped_capture = 1
      OF "STOPPED_CAPPUMP":
       dmft_screen->ds_stopped_cappump = 1
      OF "STOPPED_DELIVERY":
       dmft_screen->ds_stopped_delivery = 1
      OF "STOPPED_CAPMGR":
       dmft_screen->ds_stopped_capmgr = 1
      OF "STOPPED_DELMGR":
       dmft_screen->ds_stopped_delmgr = 1
     ENDCASE
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_disp_report(null)
   DECLARE ddr_report_file = vc WITH protect, noconstant(" ")
   DECLARE ddr_idx = i4 WITH protect, noconstant(0)
   DECLARE ddr_report_title = vc WITH protect, noconstant(" ")
   DECLARE ddr_str = vc WITH protect, noconstant(" ")
   SET message = nowindow
   SET ddr_str = fillstring(100,"-")
   IF (dmft_monitor_only=0)
    SET ddr_report_title = "Migration Final Transaction Report"
    SET ddr_report_file = "dm2_mig_fnl_trxn.rpt"
   ELSE
    SET ddr_report_title = "Migration GG Status Monitor Report"
    SET ddr_report_file = "dm2_mig_gg_monitor.rpt"
   ENDIF
   SET dm_err->eproc = concat("Write report data to ",ddr_report_file)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(ddr_report_file)
    FROM dual
    HEAD REPORT
     row + 1, col 1, ddr_report_title,
     row + 1, col 1,
     CALL print(concat("Current Date/Time : ",format(cnvtdatetime(curdate,curtime3),"@LONGDATETIME"))
     ),
     row + 2
    DETAIL
     row + 1, col 1, ddr_str,
     row + 1, col 1,
     CALL print(concat("Capture Mgr. Status : ",dmft_screen->ds_mgr_status)),
     row + 1, col 1,
     CALL print(concat("Delivery Mgr. Status: ",dmft_screen->ds_delivery_mgr_status)),
     row + 1, col 1, ddr_str,
     row + 1, col 1, "Process",
     col 14, "Status", col 29,
     "Rba", col 44, "Seq",
     col 60, "Lag", row + 1,
     col 1, ddr_str, row + 1,
     col 1, "Capture", col 14,
     dmft_screen->ds_capture_status, col 29, dmft_screen->ds_trailrba,
     col 44, dmft_screen->ds_trailseq, col 60,
     "n/a"
     IF (dmft_cappump_used=1)
      row + 1, col 1, "Cappump",
      col 14, dmft_screen->ds_cappump_status, col 29,
      dmft_screen->ds_cappump_rba, col 44, dmft_screen->ds_cappump_seq,
      col 60, "n/a"
     ENDIF
     row + 1, col 1, ddr_str,
     row + 1
     FOR (ddr_idx = 1 TO dmft_screen->deliv_cnt)
       col 1, dmft_screen->deliv[ddr_idx].name, col 14,
       dmft_screen->deliv[ddr_idx].status, col 29, dmft_screen->deliv[ddr_idx].rba,
       col 44, dmft_screen->deliv[ddr_idx].seq, col 60,
       CALL print(concat(evaluate(dmft_screen->deliv[ddr_idx].lag,cnvtdatetimeutc(
          "01-JAN-1900 12:00:00",2),"-1",format(datetimediff(cnvtdatetimeutc(cnvtdatetime(curdate,
             curtime3),2),dmft_screen->deliv[ddr_idx].lag,8),"####.##:##:##")))), row + 1
     ENDFOR
     IF (dmft_monitor_only=0)
      col 1, ddr_str, row + 1,
      col 1, "Final Transaction Progress", row + 1,
      col 1,
      CALL print(concat(evaluate(dmft_screen->ds_start_final_ind,1,"| Complete |",2,"| Waiting  |",
        "|          |")," Begin Final Transaction")), row + 1,
      col 1,
      CALL print(concat(evaluate(dmft_screen->ds_read_only_complete,1,"| Complete |",2,"| Waiting  |",
        "|          |")," Tablespaces to Read Only")), row + 1,
      col 1,
      CALL print(concat(evaluate(dmft_screen->ds_stopped_delivdi,1,"| Complete |",2,"| Waiting  |",
        "|          |")," Stop DelivDI")), row + 1,
      col 1,
      CALL print(concat(evaluate(dmft_screen->ds_insert_final_trans,1,"| Complete |",2,"| Waiting  |",
        "|          |")," Insert Final Transaction"))
      IF (dmft_src_db_os != "AXP")
       row + 1, col 1, "| n/a      | Cycle and Copy Archive Logs"
      ELSE
       row + 1, col 1,
       CALL print(concat(evaluate(dmft_screen->ds_archive_copy_init,1,"| Complete |",2,"| Waiting  |",
         "|          |")," Cycle and Copy Archive Logs"))
      ENDIF
      row + 1, col 1,
      CALL print(concat(evaluate(dmft_screen->ds_final_detected_in_trail,1,"| Complete |",2,
        "| Waiting  |",
        "|          |")," Final Transaction in Trail")),
      row + 1, col 1,
      CALL print(concat(evaluate(dmft_screen->ds_stopped_capture,1,"| Complete |",2,"| Waiting  |",
        "|          |")," Stop Capture"))
      IF (dmft_cappump_used=0)
       row + 1, col 1, "| n/a      | Stop Cappump"
      ELSE
       row + 1, col 1,
       CALL print(concat(evaluate(dmft_screen->ds_stopped_cappump,1,"| Complete |",2,"| Waiting  |",
         "|          |")," Stop Cappump"))
      ENDIF
      row + 1, col 1,
      CALL print(concat(evaluate(dmft_screen->ds_stopped_capmgr,1,"| Complete |",2,"| Waiting  |",
        "|          |")," Stop Capture Mgr.")),
      row + 1, col 1,
      CALL print(concat(evaluate(dmft_screen->ds_delivery_fully_processed,1,"| Complete |",2,
        "| Waiting  |",
        "|          |")," Deliveries Fully Processed")),
      row + 1, col 1,
      CALL print(concat(evaluate(dmft_screen->ds_delivdi_started,1,"| Complete |",2,"| Waiting  |",
        "|          |")," Start DelivDI")),
      row + 1, col 1,
      CALL print(concat(evaluate(dmft_screen->ds_final_trans_in_tgt,1,"| Complete |",2,"| Waiting  |",
        "|          |")," Final Transaction in Target")),
      row + 1, col 1,
      CALL print(concat(evaluate(dmft_screen->ds_stopped_delivery,1,"| Complete |",2,"| Waiting  |",
        "|          |")," Stop all Deliveries")),
      row + 1, col 1,
      CALL print(concat(evaluate(dmft_screen->ds_stopped_delmgr,1,"| Complete |",2,"| Waiting  |",
        "|          |")," Stop Delivery Mgr."))
     ENDIF
    FOOT REPORT
     row + 1, col 0, "END OF REPORT"
    WITH nocounter, maxcol = 300, format = variable,
     formfeed = none, maxrow = 1, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = concat("Displaying report data in ",ddr_report_file)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   FREE SET file_loc
   SET logical file_loc value(ddr_report_file)
   FREE DEFINE rtl2
   DEFINE rtl2 "file_loc"
   SELECT INTO "nl:"
    t.line
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_disp_screen(null)
   IF ((dm_err->debug_flag != 722))
    SET message = window
   ENDIF
   CALL clear(1,1)
   CALL box(1,1,23,131)
   IF (dmft_monitor_only=0)
    CALL text(1,1,"Database Migration Final Transaction")
   ELSE
    CALL text(1,1,"Database Migration GG Status Monitor")
   ENDIF
   CALL text(3,1,"|Process")
   CALL text(3,14,"|Status")
   CALL text(3,29,"|Rba")
   CALL text(3,44,"|Seq")
   CALL text(3,60,"|Lag")
   CALL text(4,2,fillstring(125,"-"))
   CALL text(6,2,"Capture")
   CALL text(6,15,dmft_screen->ds_capture_status)
   CALL text(6,30,dmft_screen->ds_trailrba)
   CALL text(6,45,dmft_screen->ds_trailseq)
   CALL text(6,60,"n/a")
   IF (dmft_cappump_used=1)
    CALL text(7,2,"Cappump")
    CALL text(7,15,dmft_screen->ds_cappump_status)
    CALL text(7,30,dmft_screen->ds_cappump_rba)
    CALL text(7,45,dmft_screen->ds_cappump_seq)
    CALL text(7,60,"n/a")
   ENDIF
   IF ((dmft_screen->ds_end_final_ind=1))
    CALL text(24,2,"(C)ontinue:")
   ELSE
    CALL text(24,2,"(R)efresh,(Q)uit:")
   ENDIF
   IF (dmft_monitor_only=0)
    CALL text(3,90,"| Final Transaction Progress")
    CALL text(5,90,concat(evaluate(dmft_screen->ds_start_final_ind,1,"| Complete |",2,"| Waiting  |",
       "|          |")," Begin Final Transaction"))
    CALL text(6,90,concat(evaluate(dmft_screen->ds_read_only_complete,1,"| Complete |",2,
       "| Waiting  |",
       "|          |")," Tablespaces to Read Only"))
    CALL text(7,90,concat(evaluate(dmft_screen->ds_stopped_delivdi,1,"| Complete |",2,"| Waiting  |",
       "|          |")," Stop DelivDI"))
    CALL text(8,90,concat(evaluate(dmft_screen->ds_insert_final_trans,1,"| Complete |",2,
       "| Waiting  |",
       "|          |")," Insert Final Transaction"))
    IF (dmft_src_db_os != "AXP")
     CALL text(9,90,"| n/a      | Cycle and Copy Archive Logs")
    ELSE
     CALL text(9,90,concat(evaluate(dmft_screen->ds_archive_copy_init,1,"| Complete |",2,
        "| Waiting  |",
        "|          |")," Cycle and Copy Archive Logs"))
    ENDIF
    CALL text(10,90,concat(evaluate(dmft_screen->ds_final_detected_in_trail,1,"| Complete |",2,
       "| Waiting  |",
       "|          |")," Final Transaction in Trail"))
    CALL text(11,90,concat(evaluate(dmft_screen->ds_stopped_capture,1,"| Complete |",2,"| Waiting  |",
       "|          |")," Stop Capture"))
    IF (dmft_cappump_used=0)
     CALL text(12,90,"| n/a      | Stop Cappump")
    ELSE
     CALL text(12,90,concat(evaluate(dmft_screen->ds_stopped_cappump,1,"| Complete |",2,
        "| Waiting  |",
        "|          |")," Stop Cappump"))
    ENDIF
    CALL text(13,90,concat(evaluate(dmft_screen->ds_stopped_capmgr,1,"| Complete |",2,"| Waiting  |",
       "|          |")," Stop Capture Mgr."))
    CALL text(14,90,concat(evaluate(dmft_screen->ds_delivery_fully_processed,1,"| Complete |",2,
       "| Waiting  |",
       "|          |")," Deliveries Fully Processed"))
    CALL text(15,90,concat(evaluate(dmft_screen->ds_delivdi_started,1,"| Complete |",2,"| Waiting  |",
       "|          |")," Start DelivDI"))
    CALL text(16,90,concat(evaluate(dmft_screen->ds_final_trans_in_tgt,1,"| Complete |",2,
       "| Waiting  |",
       "|          |")," Final Transaction in Target"))
    CALL text(17,90,concat(evaluate(dmft_screen->ds_stopped_delivery,1,"| Complete |",2,
       "| Waiting  |",
       "|          |")," Stop all Deliveries"))
    CALL text(18,90,concat(evaluate(dmft_screen->ds_stopped_delmgr,1,"| Complete |",2,"| Waiting  |",
       "|          |")," Stop Delivery Mgr."))
   ENDIF
   CALL text(19,2,fillstring(125,"-"))
   CALL text(20,2,concat("Capture Mgr. Status : ",dmft_screen->ds_mgr_status))
   CALL text(21,2,concat("Delivery Mgr. Status: ",dmft_screen->ds_delivery_mgr_status))
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_insert_row(dir_info_domain,dir_info_name)
   SET dm_err->eproc = concat("Check for ",dir_info_domain,".",dir_info_name," row")
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_admin_dm_info
    WHERE info_domain=dir_info_domain
     AND info_name=dir_info_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->eproc = concat("Inserting ",dir_info_domain,".",dir_info_name," row")
    CALL disp_msg(" ",dm_err->logfile,0)
    INSERT  FROM dm2_admin_dm_info
     SET info_domain = dir_info_domain, info_name = dir_info_name
     WITH nocounter
    ;end insert
    IF (check_error(dm_err->eproc)=1)
     ROLLBACK
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_log_status(dls_mode)
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = "Update the script status"
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   UPDATE  FROM dm_info d
    SET d.info_number = dm_err->err_ind, d.info_char = substring(1,100,dm_err->eproc), d.updt_cnt = (
     d.updt_cnt+ 1),
     d.info_date = cnvtdatetime(curdate,curtime3)
    WHERE d.info_domain="DM2_MIG_FINAL_TRANS"
     AND d.info_name=dls_mode
    WITH nocounter
   ;end update
   COMMIT
   IF (curqual=0)
    INSERT  FROM dm_info d
     SET d.info_number = dm_err->err_ind, d.info_char = substring(1,100,dm_err->eproc), d.updt_cnt =
      1,
      d.info_date = cnvtdatetime(curdate,curtime3), d.info_domain = "DM2_MIG_FINAL_TRANS", d
      .info_name = dls_mode
     WITH nocounter
    ;end insert
    COMMIT
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_gather_info(null)
   DECLARE dgi_cnt = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Gather capture and delivery information from Admin."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM dm2_admin_dm_info di
    WHERE di.info_domain IN ("DM2MIG_CAPDATA", "DM2MIG_DELDATA")
    DETAIL
     IF (di.info_domain="DM2MIG_CAPDATA"
      AND di.info_name="CAPPUMP_TRAILRBA")
      dmft_screen->ds_cappump_rba = trim(cnvtstring(di.info_number))
     ENDIF
     IF (di.info_domain="DM2MIG_CAPDATA"
      AND di.info_name="CAPPUMP_TRAILSEQ")
      dmft_screen->ds_cappump_seq = trim(cnvtstring(di.info_number))
     ENDIF
     IF (di.info_domain="DM2MIG_CAPDATA"
      AND di.info_name="TRAILRBA")
      dmft_screen->ds_trailrba = trim(cnvtstring(di.info_number))
     ENDIF
     IF (di.info_domain="DM2MIG_CAPDATA"
      AND di.info_name="CAPPUMP_STATUS")
      dmft_screen->ds_cappump_status = di.info_char
     ENDIF
     IF (di.info_domain="DM2MIG_CAPDATA"
      AND di.info_name="CAPTURE_STATUS")
      dmft_screen->ds_capture_status = di.info_char
     ENDIF
     IF (di.info_domain="DM2MIG_CAPDATA"
      AND di.info_name="MGR_STATUS")
      dmft_screen->ds_mgr_status = di.info_char
     ENDIF
     IF (di.info_domain="DM2MIG_CAPDATA"
      AND di.info_name="TRAILSEQ")
      dmft_screen->ds_trailseq = trim(cnvtstring(di.info_number))
     ENDIF
     IF (di.info_domain="DM2MIG_DELDATA"
      AND di.info_name="DELIVERY_MGR_STATUS")
      dmft_screen->ds_delivery_mgr_status = di.info_char
     ENDIF
     FOR (dgi_cnt = 1 TO dmft_screen->deliv_cnt)
       IF (di.info_domain="DM2MIG_DELDATA"
        AND di.info_name=build(dmft_screen->deliv[dgi_cnt].name,"_STATUS"))
        dmft_screen->deliv[dgi_cnt].status = di.info_char
       ENDIF
       IF (di.info_domain="DM2MIG_DELDATA"
        AND di.info_name=build(dmft_screen->deliv[dgi_cnt].name,"_RBA"))
        dmft_screen->deliv[dgi_cnt].rba = trim(cnvtstring(di.info_number))
       ENDIF
       IF (di.info_domain="DM2MIG_DELDATA"
        AND di.info_name=build(dmft_screen->deliv[dgi_cnt].name,"_SEQ"))
        dmft_screen->deliv[dgi_cnt].seq = trim(cnvtstring(di.info_number))
       ENDIF
       IF (di.info_domain="DM2MIG_DELDATA"
        AND di.info_name=build(dmft_screen->deliv[dgi_cnt].name,"_LAG"))
        dmft_screen->deliv[dgi_cnt].lag = di.info_date
       ENDIF
     ENDFOR
    WITH nocounter
   ;end select
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_select_final_trans(dsft_exists,dsft_inst_nbr)
   SET dm_err->eproc = concat("Check for Final Transaction row:",build(dsft_inst_nbr))
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT
    IF ((dsft_inst_nbr=- (1)))
     FROM dm_info
     WHERE info_domain="DATABASE MIGRATION"
      AND info_name IN ("FINAL TRANSACTION", "FINAL TXN*")
    ELSEIF (dsft_inst_nbr=0)
     FROM dm_info
     WHERE info_domain="DATABASE MIGRATION"
      AND info_name="FINAL TRANSACTION"
    ELSE
     FROM dm_info
     WHERE info_domain="DATABASE MIGRATION"
      AND info_name=concat("FINAL TXN",trim(cnvtstring(dsft_inst_nbr)))
    ENDIF
    INTO "nl:"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dsft_exists = curqual
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_tspace_work(null)
   SET message = nowindow
   CALL clear(1,1)
   DECLARE dtw_cnt = i4 WITH protect, noconstant(0)
   DECLARE dtw_instr = vc WITH protect, noconstant("")
   FREE RECORD dtw_tsp
   RECORD dtw_tsp(
     1 tsp_cnt = i4
     1 qual[*]
   ...
   SET dm_err->eproc = "Gathering tablespaces to exclude from setting to READ ONLY"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_admin_dm_info di
    WHERE di.info_domain="DM2MIGTSPACE BYPASS"
     AND di.info_name="EXCLUDE_READONLY_TSPACE"
    HEAD REPORT
     dtw_instr =
     "ut.tablespace_name not in ('D_GGDDL','T_GGDDL','D_724ACCESS','I_724ACCESS','SYSTEM','SYSAUX'"
    DETAIL
     dtw_instr = concat(dtw_instr,di.info_char)
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   SELECT INTO "nl:"
    ...
    FROM (
     ...
     (SELECT
      t.tablespace_name
      FROM user_tables t
      WHERE ((t.table_name="DM_INFO") UNION (
      (SELECT DISTINCT
       i.tablespace_name
       FROM user_indexes i
       WHERE i.table_name="DM_INFO")))
      WITH sqltype("c30")))
     x)
    DETAIL
     dtw_instr = concat(dtw_instr,",'",trim(x.tablespace_name),"'")
    FOOT REPORT
     dtw_instr = concat(dtw_instr,")")
    WITH nocounter, nullreport
   ;end select
   IF ((dm_err->debug_flag > 0))
    CALL echo(dtw_instr)
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Gathering list of tablespaces to put into READ ONLY Mode"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM user_tablespaces ut
    WHERE ut.contents="PERMANENT"
     AND ut.status="ONLINE"
     AND parser(dtw_instr)
    DETAIL
     dtw_tsp->tsp_cnt = (dtw_tsp->tsp_cnt+ 1), stat = alterlist(dtw_tsp->qual,dtw_tsp->tsp_cnt),
     dtw_tsp->qual[dtw_tsp->tsp_cnt].tsp_cmd = concat("rdb alter tablespace ",build(ut
       .tablespace_name)," read only go"),
     dtw_tsp->qual[dtw_tsp->tsp_cnt].tsp_name = ut.tablespace_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   FOR (dtw_cnt = 1 TO dtw_tsp->tsp_cnt)
     SET dm_err->eproc = concat("Putting ",build(dtw_tsp->qual[dtw_cnt].tsp_name),
      " into READ ONLY Mode")
     CALL disp_msg(" ",dm_err->logfile,0)
     IF (dm2_push_cmd(dtw_tsp->qual[dtw_cnt].tsp_cmd,1)=0)
      RETURN(0)
     ENDIF
   ENDFOR
   IF ((dm_err->debug_flag != 722))
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_final_trans_work(dftw_final_insert_ind)
   DECLARE dftw_exists = i2 WITH protect, noconstant(0)
   DECLARE dftw_cnt = i4 WITH protect, noconstant(0)
   IF (dftw_final_insert_ind=1)
    IF (dmft_select_final_trans(dftw_exists,0)=0)
     RETURN(0)
    ENDIF
    IF (dftw_exists=0)
     SET dm_err->eproc = "Insert Master Final Transaction Row into SOURCE"
     CALL disp_msg(" ",dm_err->logfile,0)
     INSERT  FROM dm_info
      SET info_domain = "DATABASE MIGRATION", info_name = "FINAL TRANSACTION", updt_dt_tm =
       cnvtdatetime(curdate,curtime3)
      WITH nocounter
     ;end insert
     IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(1)
   ENDIF
   FOR (dftw_cnt = 1 TO dmr_mig_data->src_node_cnt)
     IF (dftw_cnt > 1)
      SET dm2_install_schema->connect_str = dmft_cstr->qual[(dftw_cnt - 1)].cstr
      SET dm2_install_schema->u_name = "V500"
      ...
      ENDIF
     ENDIF
     IF (dmft_select_final_trans(dftw_exists,evaluate(dftw_cnt,1,1,dmft_cstr->qual[(dftw_cnt - 1)].
       inst_nbr))=0)
      RETURN(0)
     ENDIF
     IF (dftw_exists=0)
      SET dm_err->eproc = "Insert Final Transaction Row into SOURCE"
      CALL disp_msg(" ",dm_err->logfile,0)
      INSERT  FROM dm_info
       SET info_domain = "DATABASE MIGRATION", info_name = concat("FINAL TXN",trim(cnvtstring(
           evaluate(dftw_cnt,1,1,dmft_cstr->qual[(dftw_cnt - 1)].inst_nbr)))), updt_dt_tm =
        cnvtdatetime(curdate,curtime3)
       WITH nocounter
      ;end insert
      IF (check_error(dm_err->eproc)=1)
      ...
      ENDIF
     ELSE
      SET dm_err->eproc = "Final Transaction Row already in SOURCE"
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
   ENDFOR
   IF ((dmr_mig_data->src_node_cnt > 1))
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_log_switch(dlw_nbr_to_switch)
   DECLARE dls_cnt = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = concat("Switching logfiles ",build(dlw_nbr_to_switch)," times")
   CALL disp_msg(" ",dm_err->logfile,0)
   FOR (dls_cnt = 1 TO dlw_nbr_to_switch)
     IF (dm2_push_cmd("rdb alter system archive log current go",1)=0)
      RETURN(0)
     ENDIF
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_archive_job_work(null)
   DECLARE dajw_cnt = i4 WITH protect, noconstant(0)
   DECLARE dajw_copy_job_performing = i2 WITH protect, noconstant(0)
   DECLARE dajw_archive_job_performing = i2 WITH protect, noconstant(0)
   DECLARE dajw_job_chk_time = dq8 WITH protect, noconstant(0.0)
   DECLARE dajw_job_submit_time = dq8 WITH protect, noconstant(0.0)
   DECLARE dajw_selection = vc WITH protect, noconstant("")
   DECLARE dajw_ndx = i4 WITH protect, noconstant(0)
   DECLARE dajw_found_job_ind = i2 WITH protect, noconstant(0)
   FREE RECORD dajw_logs
   RECORD dajw_logs(
     1 log_cnt = i4
     1 qual[*]
       2 l_seq = f8
   ...
   SET message = nowindow
   SET dm_err->eproc = "Get count of thread 1 archive logs"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    bg_cnt_val = count(v.sequence#)
    FROM v$log v
    WHERE v.thread#=1
    DETAIL
     dajw_logs->log_cnt = bg_cnt_val
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (dmft_log_switch(dajw_logs->log_cnt)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Waiting 1 Minute to switch logfiles once more"
   CALL disp_msg(" ",dm_err->logfile,0)
   CALL pause(60)
   SET dm_err->eproc = "Get Archive Log information for second switch"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dajw_logs->log_cnt = 0
   SELECT INTO "nl:"
    FROM v$log v
    WHERE v.thread#=1
    DETAIL
     dajw_logs->log_cnt = (dajw_logs->log_cnt+ 1), stat = alterlist(dajw_logs->qual,dajw_logs->
      log_cnt), dajw_logs->qual[dajw_logs->log_cnt].l_seq = v.sequence#
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (dmft_log_switch(dajw_logs->log_cnt)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Verifying that logfiles were switched"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM v$log v
    WHERE v.thread#=1
     AND expand(dajw_ndx,1,dajw_logs->log_cnt,v.sequence#,dajw_logs->qual[dajw_ndx].l_seq)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual > 0)
    SET dm_err->eproc = "Sequences still exist in V$LOG. Switching logfiles once more"
    CALL disp_msg(" ",dm_err->logfile,0)
    CALL echorecord(dajw_logs)
    IF (dmft_log_switch(dajw_logs->log_cnt)=0)
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Verifying that logfiles were switched"
    ...
    IF (curqual > 0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Unable to switch logfiles successfully"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     CALL echorecord(dajw_logs)
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->eproc = "Waiting 15 seconds for Oracle to write Redo to Archive"
   CALL disp_msg(" ",dm_err->logfile,0)
   CALL pause(15)
   IF (dmft_get_submit_time(dajw_job_chk_time)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Check if Archive Copy job is executing"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dmr_get_queue_contents(dmr_batch_queue)=0)
    RETURN(0)
   ENDIF
   CALL echorecord(dmr_queue)
   SET dajw_archive_job_performing = 1
   SET dajw_found_job_ind = 0
   SET dm_err->eproc = "Check if Archive Log Copy job is running"
   WHILE (dajw_archive_job_performing=1)
     SET dajw_archive_job_performing = 0
     FOR (dajw_cnt = 1 TO dmr_queue->cnt)
       IF ((dmr_queue->qual[dajw_cnt].jobname="DM2_MIG_COPY_ARCHIVELOGS"))
        SET dajw_found_job_ind = 1
        IF (findstring("Executing",dmr_queue->qual[dajw_cnt].status,1,0) > 0)
         SET dm_err->eproc = concat("Waiting for entry:job, ",trim(cnvtstring(dmr_queue->qual[
            dajw_cnt].entry)),":",dmr_queue->qual[dajw_cnt].jobname,", in queue ",
          dmr_batch_queue," to complete")
         CALL disp_msg(" ",dm_err->logfile,0)
         SET dajw_archive_job_performing = 1
        ENDIF
        SET dajw_cnt = (dmr_queue->cnt+ 1)
       ENDIF
     ENDFOR
     IF (dajw_found_job_ind=0)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Archive Log Copy Job is not running"
      SET dm_err->user_action = "Please start the Archive Log Copy job"
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
     IF (dajw_archive_job_performing=1)
      SET dm_err->eproc = "Prompt user for Archive Copy Job wait on execution"
      CALL disp_msg(" ",dm_err->logfile,0)
      IF (dmft_prompt_wait(dajw_selection,"EXECUTION_CHECK","Wait for Archive Copy execution.")=0)
       RETURN(0)
      ENDIF
      IF ((dm_err->debug_flag != 722))
       SET message = window
      ...
      CASE (dajw_selection)
       ...
       OF "Q":
        SET message = nowindow
        SET dm_err->emsg = "User elected to quit from archive copy job execution wait"
        SET dm_err->err_ind = 1
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        RETURN(0)
      ENDCASE
   ...
   FOR (dajw_cnt = 1 TO dmr_queue->cnt)
     IF ((dmr_queue->qual[dajw_cnt].jobname="DM2_MIG_COPY_ARCHIVELOGS")
      AND findstring("Holding",dmr_queue->qual[dajw_cnt].status,1,0) > 0)
      SET dm_err->eproc = concat("Releasing entry:job, ",trim(cnvtstring(dmr_queue->qual[dajw_cnt].
         entry)),":",dmr_queue->qual[dajw_cnt].jobname,", in queue ",
       dmr_batch_queue)
      CALL disp_msg(" ",dm_err->logfile,0)
      IF (dm2_push_dcl(concat("set entry ",trim(cnvtstring(dmr_queue->qual[dajw_cnt].entry)),
        " /release"))=0)
       RETURN(0)
      ENDIF
      SET dajw_cnt = (dmr_queue->cnt+ 1)
     ENDIF
   ENDFOR
   IF (dmft_get_submit_time(dajw_job_submit_time)=0)
    RETURN(0)
   ENDIF
   CALL echo(format(dajw_job_submit_time,"DD-MMM-YYYY HH:MM:SS;;q"))
   CALL echo(format(dajw_job_chk_time,"DD-MMM-YYYY HH:MM:SS;;q"))
   SET dm_err->eproc = "Prompt user for Archive Copy Job submit wait status"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dajw_selection = ""
   WHILE ( NOT (dajw_selection IN ("Q", "B"))
    AND dajw_job_submit_time <= dajw_job_chk_time)
     IF (dmft_prompt_wait(dajw_selection,"SUBMIT_CHECK",format(dajw_job_submit_time,
       "DD-MMM-YYYY HH:MM:SS;;q"))=0)
      RETURN(0)
     ENDIF
     IF ((dm_err->debug_flag != 722))
      SET message = window
     ...
     CASE (dajw_selection)
      OF "R":
       IF (dmft_get_submit_time(dajw_job_submit_time)=0)
        RETURN(0)
       ENDIF
       IF (dajw_job_submit_time > dajw_job_chk_time)
        SET dm_err->eproc = "Archive Job submit time current. Continuing downtime steps."
        CALL disp_msg(" ",dm_err->logfile,0)
       ENDIF
      OF "Q":
       SET message = nowindow
       SET dm_err->emsg = "User elected to quit from archive copy verification"
       SET dm_err->err_ind = 1
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
     ENDCASE
   ...
   ENDIF
   CALL echorecord(dmr_queue)
   SET dajw_copy_job_performing = 1
   WHILE (dajw_copy_job_performing=1)
    SET dajw_copy_job_performing = 0
    FOR (dajw_cnt = 1 TO dmr_queue->cnt)
      IF (findstring("DM2_CPY",dmr_queue->qual[dajw_cnt].jobname,1,1) > 0)
       SET dajw_copy_job_performing = 1
       SET dajw_cnt = (dmr_queue->cnt+ 1)
       SET dm_err->eproc = "Prompt user for Archive Copy Job wait complete status"
       CALL disp_msg(" ",dm_err->logfile,0)
       IF (dmft_prompt_wait(dajw_selection,"COMPLETE_CHECK","Wait for Archive Copy to complete.")=0)
        RETURN(0)
       ENDIF
       IF ((dm_err->debug_flag != 722))
        SET message = window
       ...
       CASE (dajw_selection)
        ...
        OF "Q":
         SET message = nowindow
         SET dm_err->emsg = "User elected to quit from archive copy job completion wait"
         SET dm_err->err_ind = 1
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         RETURN(0)
       ENDCASE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_prompt_wait(dpaw_selection_ret,dpaw_mode,dpaw_info)
   IF (dmft_eval_checkpoints(null)=0)
    GO TO exit_program
   ENDIF
   IF (dmft_gather_info(null)=0)
    GO TO exit_program
   ENDIF
   IF (dmft_disp_report(null)=0)
    GO TO exit_program
   ENDIF
   IF (dmft_disp_screen(null)=0)
    GO TO exit_program
   ENDIF
   IF (dpaw_info > " ")
    CALL text(24,46,concat("Informational:",dpaw_info))
   ENDIF
   SET accept = time(10)
   IF ((dmft_screen->ds_end_final_ind=1))
    CALL accept(24,14,"P;CU"
     WHERE curaccept IN ("C"))
   ELSE
    CALL accept(24,21,"P;CU","R"
     WHERE curaccept IN ("R", "Q"))
   ENDIF
   SET dpaw_selection_ret = curaccept
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmft_get_submit_time(dgst_time_ret)
   SET dm_err->eproc = "Determining existing Archive Copy Job submit time"
   CALL disp_msg("",dm_err->logfile,0)
   SET dgst_time_ret = 0.0
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="DM2_MIG_DATA"
     AND di.info_name="ARCH_LAST_TIMESTAMP"
    DETAIL
     dgst_time_ret = cnvtdatetime(di.info_date)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->emsg = "Unable to determine existing Archive Copy Job submit time"
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
...
#exit_program
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  CALL dmr_prompt_connect_data("TARGET","V500","CO")
 ENDIF
 CALL dmft_log_status("END")
 IF ((dm_err->err_ind=0))
  IF (dmft_monitor_only=1)
   SET dm_err->eproc = "GG status monitoring completed"
  ELSE
   SET dm_err->eproc = "dm2_mig_final_trans completed"
  ENDIF
 ENDIF
 CALL final_disp_msg("dm2_mig_final_trans")
END GO
