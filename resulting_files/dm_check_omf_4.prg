CREATE PROGRAM dm_check_omf_4
  ...
  *
  ...
  WHERE me.cover_period_to_fl06 > cnvtdatetime(curdate,curtime3)
 ...
 IF (curqual != 0)
  UPDATE  FROM ub92_mon_encounter_error mee,
   ...
   SET mee.error_cd = 00503, mee.status_flg = "R", mee.updt_dt_tm = cnvtdatetime(curdate,curtime3),
  ...
  IF (curqual=0)
   INSERT  FROM ub92_mon_encounter_error mee,
     (dummyt d  WITH seq = value(data->count))
    SET mee.error_cd = 00503, mee.reporting_period = cnvtdatetime(startdatesstring), mee
  ...
  *
  ...
  WHERE mpp.cover_period_to_fl06 > cnvtdatetime(curdate,curtime3)
  ...
  *
  ...
  WHERE md.cover_period_to_fl06 > cnvtdatetime(curdate,curtime3)
