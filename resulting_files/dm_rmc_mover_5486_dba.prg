CREATE PROGRAM dm_rmc_mover_5486:dba
 ...
 IF (cust_disp_val <= " ")
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV27"
  CALL merge_audit("FAILREASON","ERROR: pft_criteria_name was invalid",1)
  GO TO exit_5486
 ENDIF
 ...
 CASE (cust_disp_val)
  ...
  OF "Current Billing Entity":
  OF "Selected Billing Entity":
  ...
  ELSE
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV27"
   CALL merge_audit("FAILREASON",concat("ERROR: the display value of ",cust_disp_val,
     " is not supported at this time"),1)
   GO TO exit_5486
 ENDCASE
 IF (cust_process_tab > " ")
  IF (cnvtreal(cust_from_value) > 0)
   SET new_value = select_merge_translate(cust_from_value,cust_process_tab)
 ...
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_5486
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"SELECTED_CRITERIA_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1)
   ...
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"SELECTED_CRITERIA_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"SELECTED_CRITERIA_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  SELECTED_CRITERIA_ID"
       )
     ELSE
      GO TO exit_5486
     ENDIF
   ...
   SET from_cd = get_value(cust_tab_name,"SELECTED_CRITERIA_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5486
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(from_cd)
   ENDIF
   SET new_cd = get_value(cust_tab_name,"SELECTED_CRITERIA_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5486
   ELSE
