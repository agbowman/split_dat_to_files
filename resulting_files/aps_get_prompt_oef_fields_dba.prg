CREATE PROGRAM aps_get_prompt_oef_fields:dba
 ...
 DECLARE dactiontypecd = f8 WITH protect, noconstant(0.0)
 RECORD reply(
   1 oe_field_qual[*]
     2 oe_field_id = f8
     2 oe_field_display = vc
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 SET dactiontypecd = uar_get_code_by("MEANING",6003,"ORDER")
 IF (dactiontypecd < 1)
  CALL populate_subeventstatus("UAR","F","MEANING","CS 6003 - ORDER")
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  ...
  PLAN (oc
   WHERE (oc.catalog_cd=request->specimen_catalog_cd))
   JOIN (off
   WHERE off.oe_format_id=oc.oe_format_id
    AND off.action_type_cd=dactiontypecd)
   JOIN (oef
   WHERE oef.oe_field_id=off.oe_field_id)
  HEAD REPORT
   loefieldcnt = 0
  DETAIL
   loefieldcnt = (loefieldcnt+ 1)
   IF (mod(loefieldcnt,10)=1)
    stat = alterlist(reply->oe_field_qual,(loefieldcnt+ 9))
   ENDIF
   reply->oe_field_qual[loefieldcnt].oe_field_id = oef.oe_field_id, reply->oe_field_qual[loefieldcnt]
   .oe_field_display = oef.description
  FOOT REPORT
   stat = alterlist(reply->oe_field_qual,loefieldcnt)
  WITH nocounter
 ;end select
 IF (curqual=0)
