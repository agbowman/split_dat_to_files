CREATE PROGRAM acm_del_person_code_value_r:dba
 ...
 DECLARE t_val = i4 WITH noconstant(xref->del_cnt), protect
 DECLARE f_val = i4 WITH noconstant(1), protect
 DECLARE idx = i4 WITH noconstant(0), protect
 DECLARE del_cnt = i4 WITH noconstant(xref->del_cnt), protect
 ...
 ELSE
  ...
  ENDWHILE
 ENDIF
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
   person_code_value_r p
  SET p.seq = 1
  PLAN (d)
   JOIN (p
   WHERE (p.person_id=acm_request->person_code_value_r_qual[xref->del[d.seq].idx].person_id)
    AND (p.code_set=acm_request->person_code_value_r_qual[xref->del[d.seq].idx].code_set)
    AND p.person_code_value_r_id > 0)
  WITH nocounter, status(reply->person_code_value_r_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->person_code_value_r_qual[xref->del[index].idx].status != 1))
    SET failed = delete_error
    SET table_name = "PERSON_CODE_VALUE_R"
    GO TO exit_script
   ENDIF
 ENDFOR
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    FROM person_code_value_r p
    WHERE expand(t1,f_val,t_val,p.person_code_value_r_id,acm_request->person_code_value_r_qual[xref->
     del[t1].idx].person_code_value_r_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.person_code_value_r_id,acm_request->person_code_value_r_qual[
      xref->del[t1].idx].person_code_value_r_id,
      max_val), idx = xref->del[t2].idx
     ...
     ENDIF
    WITH nocounter, forupdatewait(t), time = 5
   ;end select
   IF (failed)
    SET table_name = "PERSON_CODE_VALUE_R"
    GO TO exit_script
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->person_code_value_r_qual[xref->del[index].idx].status=0))
