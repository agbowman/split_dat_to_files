CREATE PROGRAM dcp_get_tasks_by_encntr_assign:dba
 ...
 DECLARE encounter_count = i4 WITH constant(size(request->encntr_list,5)), protect
 ...
 IF (assign_prsnl_count > 0
  ...
  IF (task_status_count > 0)
   SET expandclauses = concat(trim(expandclauses),
    " and expand(task_status_index,1,task_status_count,ta.task_status_cd+0,request->task_status_list[task_status_index].status_cd)"
    )
  ENDIF
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (ta
    WHERE (ta.encntr_id=request->encntr_list[d.seq].encntr_id)
     AND ((ta.person_id+ 0) > context->last_person_id)
     AND parser(trim(expandclauses)))
