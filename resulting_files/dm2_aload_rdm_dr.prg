CREATE PROGRAM dm2_aload_rdm_dr
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_rdm_dr",".log","dm2_aload_rdm_dr")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_rdm_dr"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dardr_req_size = i4 WITH protect, noconstant(0)
 SET dardr_req_size = size(requestin->list_0,5)
 IF (dac_pop_coldic_rec("DM_README")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(requestin)
 ENDIF
 SET dm_err->eproc = "Remove existing readme data from dm_readme for readmes on package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 DELETE  FROM dm_readme r,
   (dummyt d  WITH seq = value(dardr_req_size))
  SET r.seq = 0
  PLAN (d)
   JOIN (r
   WHERE r.readme_id=cnvtint(requestin->list_0[d.seq].readme_id)
    AND r.instance=cnvtint(requestin->list_0[d.seq].instance))
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
   " dcv ,(dummyt d with seq = value(dardr_req_size) )"),0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_rdm_dr completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_rdm_dr")
END GO
