CREATE PROGRAM dm_him_purge_prsnl_suspension:dba
 ...
 SET reply->table_name = "PRSNL_SUSPENSION"
 ...
 ELSE
  ...
  SET v_rows = 0
  SELECT INTO "nl:"
   FROM prsnl_suspension ps
   WHERE parser(sbr_getrowidnotexists("ps.updt_dt_tm < cnvtdatetime(v_days_back)","ps"))
    AND ps.prsnl_suspension_id != 0
    AND ps.susp_end_dt_tm < cnvtdatetime(curdate,curtime3)
   DETAIL
    v_rows = (v_rows+ 1)
    IF (mod(v_rows,50)=1)
     stat = alterlist(reply->rows,(v_rows+ 49))
    ENDIF
    reply->rows[v_rows].row_id = ps.rowid
   FOOT REPORT
    stat = alterlist(reply->rows,v_rows)
   WITH nocounter, maxqual(ps,value(request->max_rows))
  ;end select
