CREATE PROGRAM dm_ocmb_patient_event_deta_drr:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "dm_ocmb_patient_event_deta_drr"
 ...
 SELECT INTO "nl:"
  ...
  FROM patient_event_deta3085drr frm
 ...
 SUBROUTINE (del_from(event_index=i4,s_df_pk_id=f8,s_df_prev_act_ind=i2,s_df_to_fk_id=f8) =i4)
   ...
   DECLARE expindex = i4
   UPDATE  FROM patient_event_deta3085drr frm
   ...
   FOR (i = 1 TO detailsize)
     ...
     SET request->xxx_combine_det[icombinedet].entity_name = "PATIENT_EVENT_DETA3085DRR"
     ...
     ELSE
      SET request->xxx_combine_det[icombinedet].attribute_name = "PATIENT_EVENT_DETA3085DRR"
     ...
     IF (add_detail_hist(eventlist->patient_events[event_index].patient_event_details[i].
      ...
      SET request->error_message = substring(1,132,build(
        "Could not insert del history for PATIENT_EVENT_DETA3085DRR with pk=",eventlist->
        patient_events[event_index].patient_event_details[i].patient_event_detail_id))
      RETURN(0)
     ENDIF
   ENDFOR
   UPDATE  FROM patient_event4050drr frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.active_status_dt_tm =
     cnvtdatetime(trans_dt_tm),
     frm.active_status_prsnl_id = reqinfo->updt_id, frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id =
     reqinfo->updt_id,
   ...
   SET request->xxx_combine_det[icombinedet].attribute_name = "PATIENT_EVENT4050DRR"
   SET stat = alterlist(request->xxx_combine_det[icombinedet].entity_pk,1)
   ...
   IF (add_event_hist(eventlist->patient_events[event_index].patient_event_id)=0)
    SET failed = insert_error
    SET request->error_message = substring(1,132,build(
      "Could not insert del history for PATIENT_EVENT4050DRR with pk=",eventlist->patient_events[
      event_index].patient_event_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   ...
   IF (curqual=0)
    ...
    SET request->error_message = build(
     "Couldn't update PATIENT_EVENT_DETA3085DRR record with PATIENT_EVENT_DETAIL_ID = ",s_uf_pk_id)
    RETURN(0)
   ENDIF
   IF (add_detail_hist(s_uf_pk_id)=0)
    SET failed = insert_error
    SET request->error_message = substring(1,132,build(
      "Could not insert updt history for PATIENT_EVENT_DETA3085DRR with pk=",s_uf_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0)
    ...
    SET dcipht_request->transaction_reason_txt = "DM_OCMB_PATIENT_EVENT_DETA_DRR"
