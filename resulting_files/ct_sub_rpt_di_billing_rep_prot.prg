CREATE PROGRAM ct_sub_rpt_di_billing_rep_prot
 PROMPT
 ...
 SUBROUTINE layout_query(dummy)
   SELECT DISTINCT
    ...
    o.protocol_order_id, pw.description, p.name_full_formatted,
    p.birth_dt_tm, pa.alias, org.org_name,
    ...
    HEAD REPORT
     ...
     _d3 = pw.description, _d4 = o.orig_order_dt_tm, _d5 = oa.action_dt_tm,
     _d6 = ps.name_full_formatted, _d7 = o.ordered_as_mnemonic, _fenddetail = (rptreport->
 ...
 SUBROUTINE (headpm_primary_mnemonicsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.094),(offsetx+ 8.480),(offsety+
     0.094))
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 1.089),(offsetx+ 8.501),(offsety+
     1.089))
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
 ...
 SUBROUTINE (heado_person_idsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __fieldname2 = vc WITH noconstant(build2(label->enroll_id,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
 ...
 SUBROUTINE (heado_order_mnemonicsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __fieldname3 = vc WITH noconstant(build2(label->encntr_type,char(0))), protect
   DECLARE __fieldname4 = vc WITH noconstant(build2(label->order_name,char(0))), protect
   DECLARE __fieldname5 = vc WITH noconstant(build2(label->order_type,char(0))), protect
   DECLARE __fieldname21 = vc WITH noconstant(build2(uar_get_code_display(o.catalog_type_cd),char(0))
    ), protect
   DECLARE __fieldname8 = vc WITH noconstant(build2(label->order_id,char(0))), protect
   DECLARE __fieldname0 = vc WITH noconstant(build2(label->action_date,char(0))), protect
   DECLARE __fieldname1 = vc WITH noconstant(build2(label->res_acc,char(0))), protect
   DECLARE __fieldname7 = vc WITH noconstant(build2(label->order_placed_date,char(0))), protect
   DECLARE __fieldname2 = vc WITH noconstant(build2(label->order_status,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CT_SUB_RPT_DI_BILLING_REP_PROT"
