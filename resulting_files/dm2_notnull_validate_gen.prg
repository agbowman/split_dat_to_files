CREATE PROGRAM dm2_notnull_validate_gen
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dnvg_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dnvg_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dnvg_iter = i4 WITH protect, noconstant(0)
 FREE RECORD dnvg_list
 RECORD dnvg_list(
   01 constraint_cnt = i4
   01 constraint_qual[*]
     02 constraint_name = vc
     02 constraint_type = vc
     02 owner_name = vc
     02 table_name = vc
     02 search_condition = vc
     02 column_name = vc
     02 index_exists_ind = i2
     02 valid_data_type_ind = i2
     02 dpq_row_exists_ind = i2
     02 valid_object_ind = i2
 )
 DECLARE dnvg_get_constraint_list(null) = i2
 IF (check_logfile("dm2_nnv_gen",".log","dm2_notnull_validate_gen")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_notnull_validate_gen"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF (dm2_process_log_row(dpl_notnull_validate,dpl_execution,dpl_no_prev_id,1)=0)
  GO TO exit_program
 ENDIF
 SET dnvg_log_id = dm2_process_event_rs->dm_process_event_id
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = build("Processing constrains to generate not null validate operations")
 CALL disp_msg("",dm_err->logfile,0)
 IF (dnvg_get_constraint_list(null)=0)
  GO TO exit_program
 ENDIF
 IF ((dnvg_list->constraint_cnt=0))
  SET dm_err->eproc = "No NON-VALIDATED constraints found in user_constraints"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ELSE
  SET dm_err->eproc =
  "Inserting dm_process_queue rows for constraint validation for all constraints."
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 FOR (dnvg_iter = 1 TO dnvg_list->constraint_cnt)
   IF ((dnvg_list->constraint_qual[dnvg_iter].index_exists_ind=1)
    AND (dnvg_list->constraint_qual[dnvg_iter].valid_data_type_ind=1)
    AND (dnvg_list->constraint_qual[dnvg_iter].dpq_row_exists_ind=0)
    AND (dnvg_list->constraint_qual[dnvg_iter].valid_object_ind=1))
    SET dm_err->eproc = build("Current constraint (<",dnvg_iter,"> of <",dnvg_list->constraint_cnt,
     ">) : ",
     dnvg_list->constraint_qual[dnvg_iter].constraint_name)
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
    SET dpq_process_queue->process_type = dpq_notnull_validate
    SET dpq_process_queue->op_type = dpq_validate
    SET dpq_process_queue->owner_name = value(currdbuser)
    SET dpq_process_queue->object_type = dpq_constraint
    SET dpq_process_queue->op_method = dpq_db
    SET dpq_process_queue->object_name = trim(dnvg_list->constraint_qual[dnvg_iter].constraint_name)
    SET dpq_process_queue->priority = 350
    SET dpq_process_queue->routine_tasks_ind = 1
    SET dpq_process_queue->operation_txt = concat("EXECUTE DM2_NOTNULL_VALIDATE ","'",dnvg_list->
     constraint_qual[dnvg_iter].owner_name,"',","'",
     dnvg_list->constraint_qual[dnvg_iter].table_name,"',","'",dnvg_list->constraint_qual[dnvg_iter].
     column_name,"',",
     "'",dnvg_list->constraint_qual[dnvg_iter].constraint_name,"' GO")
 ...
 ENDFOR
 GO TO exit_program
 SUBROUTINE dnvg_get_constraint_list(null)
   DECLARE dgcl_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgcl_idx = i4 WITH protect, noconstant(0)
   DECLARE dgcl_iter = i4 WITH protect, noconstant(0)
   SET dm_err->eproc =
   "Populating the dnvg_list->constraint record structure with list of constraints from user_constraints"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM user_constraints uc
    WHERE uc.owner=value(currdbuser)
     AND uc.constraint_type="C"
     AND uc.validated="NOT VALIDATED"
     AND uc.status="ENABLED"
    ORDER BY uc.table_name, uc.constraint_name
    HEAD REPORT
     dnvg_list->constraint_cnt = 0, stat = alterlist(dnvg_list->constraint_qual,dnvg_list->
      constraint_cnt)
    DETAIL
     IF (findstring("ISNOTNULL",trim(uc.search_condition,4)) > 0)
      dnvg_list->constraint_cnt = (dnvg_list->constraint_cnt+ 1)
      IF (mod(dnvg_list->constraint_cnt,10)=1)
       stat = alterlist(dnvg_list->constraint_qual,(dnvg_list->constraint_cnt+ 9))
      ENDIF
      dnvg_list->constraint_qual[dnvg_list->constraint_cnt].constraint_name = trim(uc.constraint_name
       ), dnvg_list->constraint_qual[dnvg_list->constraint_cnt].constraint_type = trim(uc
       .constraint_type), dnvg_list->constraint_qual[dnvg_list->constraint_cnt].owner_name = trim(uc
       .owner),
      dnvg_list->constraint_qual[dnvg_list->constraint_cnt].table_name = trim(uc.table_name),
      dnvg_list->constraint_qual[dnvg_list->constraint_cnt].search_condition = trim(uc
       .search_condition), dnvg_list->constraint_qual[dnvg_list->constraint_cnt].column_name =
      replace(replace(trim(uc.search_condition),"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_$#",
        "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_$#",3),"ISNOTNULL","",0),
      dnvg_list->constraint_qual[dnvg_list->constraint_cnt].index_exists_ind = 0, dnvg_list->
      constraint_qual[dnvg_list->constraint_cnt].valid_data_type_ind = 1, dnvg_list->constraint_qual[
      dnvg_list->constraint_cnt].dpq_row_exists_ind = 0,
      dnvg_list->constraint_qual[dnvg_list->constraint_cnt].valid_object_ind = 1
     ENDIF
    FOOT REPORT
     stat = alterlist(dnvg_list->constraint_qual,dnvg_list->constraint_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   SET dm_err->eproc =
   "Filtering out constraints which have already been generated in dm_process_queue"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_process_queue dpq
    WHERE dpq.process_type=dpq_notnull_validate
     AND dpq.op_type=dpq_validate
     AND dpq.owner_name=value(currdbuser)
     AND dpq.object_type=dpq_constraint
    DETAIL
     dgcl_idx = 0, dgcl_idx = locateval(dgcl_iter,1,dnvg_list->constraint_cnt,dpq.object_name,
      dnvg_list->constraint_qual[dgcl_iter].constraint_name)
     IF (dgcl_idx > 0)
      dnvg_list->constraint_qual[dgcl_idx].dpq_row_exists_ind = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Filtering out constraints with no corresponding index in user_ind_colums"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM user_ind_columns uic
    DETAIL
     IF ((dnvg_list->constraint_qual[dgcl_iter].dpq_row_exists_ind=0))
      dgcl_idx = 0, dgcl_idx = locateval(dgcl_iter,1,dnvg_list->constraint_cnt,uic.table_name,
       dnvg_list->constraint_qual[dgcl_iter].table_name,
       uic.column_name,dnvg_list->constraint_qual[dgcl_iter].column_name)
      IF (dgcl_idx > 0)
       dnvg_list->constraint_qual[dgcl_idx].index_exists_ind = 1
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Filtering out constraints with invalid data types"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM user_tab_columns utc,
     (dummyt d  WITH seq = value(dnvg_list->constraint_cnt))
    PLAN (d
     WHERE d.seq > 0
      AND (dnvg_list->constraint_qual[d.seq].dpq_row_exists_ind=0)
      AND (dnvg_list->constraint_qual[d.seq].index_exists_ind=1))
     JOIN (utc
     WHERE (utc.table_name=dnvg_list->constraint_qual[d.seq].table_name)
      AND (utc.column_name=dnvg_list->constraint_qual[d.seq].column_name)
      AND utc.data_type IN ("LONG", "LONG RAW", "BLOB", "CLOB"))
    DETAIL
     dnvg_list->constraint_qual[d.seq].valid_data_type_ind = 0
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF ((dm2_rdbms_version->level1 >= 10))
    SET dm_err->eproc = "Filtering out recyclebin objects"
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     ...
     PLAN (d
      WHERE d.seq > 0
       AND (dnvg_list->constraint_qual[d.seq].dpq_row_exists_ind=0)
       AND (dnvg_list->constraint_qual[d.seq].index_exists_ind=1)
       AND (dnvg_list->constraint_qual[d.seq].valid_data_type_ind=1))
      JOIN (dr
      WHERE dr.type="TABLE"
       AND (dr.object_name=dnvg_list->constraint_qual[d.seq].table_name))
     DETAIL
      dnvg_list->constraint_qual[d.seq].valid_object_ind = 0
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_program
 ...
 SET dnvg_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_notnull_validate,dpl_execution,dnvg_log_id,1)
 SET dm2_process_rs->process_name = ""
 SET dm_err->err_ind = dnvg_err_ind
 SET dm_err->eproc = "Ending dm2_notnull_validate_gen"
 CALL final_disp_msg("dm2_nnv_gen")
END GO
