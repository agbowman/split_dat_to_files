 ...
 SELECT INTO "nl:"
  ...
  PLAN (t
   WHERE (request->person_id=t.person_id)
    AND t.active_ind=1)
   JOIN (pe
   WHERE t.product_event_id=pe.product_event_id)
   JOIN (p
  ...
  DETAIL
   ...
   ENDIF
   reply->qual[lreplycnt].donation_number = p.product_nbr, reply->qual[lreplycnt].product_id = p
   .product_id, reply->qual[lreplycnt].product_sub_nbr = p.product_sub_nbr,
   reply->qual[lreplycnt].product_cd = p.product_cd, reply->qual[lreplycnt].supplier_prefix = bp
   .supplier_prefix, reply->qual[lreplycnt].abo_cd = bp.cur_abo_cd,
   reply->qual[lreplycnt].rh_cd = bp.cur_rh_cd, reply->qual[lreplycnt].product_event_id = pe
   .product_event_id, reply->qual[lreplycnt].trans_dt_tm = pe.event_dt_tm,
   reply->qual[lreplycnt].trans_vol = t.transfused_vol, reply->qual[lreplycnt].unit_meas_cd = p
   .cur_unit_meas_cd, reply->qual[lreplycnt].history_upload_ind = 0,
   reply->qual[lreplycnt].electronic_entry_flag = p.electronic_entry_flag, reply->qual[lreplycnt].
   serial_nbr_txt = p.serial_number_txt
  FOOT REPORT
   row + 0
  WITH nocounter
 ;end select
   ...
   FOR (lidx = 1 TO lreplycnt)
     ...
     SELECT INTO "nl:"
      ...
      FROM person_combine_det pcd,
       ...
       person_combine pc
      PLAN (t
       WHERE (t.product_event_id=reply->qual[lidx].product_event_id))
       JOIN (pcd
       WHERE pcd.entity_id=t.product_event_id
     ...
     SELECT INTO "nl:"
      ...
      PLAN (t
       WHERE (t.product_id=reply->qual[lidx].product_id))
       JOIN (pnh
 ...
 SELECT INTO "nl:"
  ...
  PLAN (hpe
   WHERE (hpe.person_id=request->person_id)
    AND hpe.event_type_cd=transfused_event_type_cd
    AND hpe.active_ind=1)
   JOIN (hp
   WHERE hp.product_id=hpe.product_id)
  ORDER BY hpe.event_dt_tm
  DETAIL
   lreplycnt += 1
   IF (mod(lreplycnt,10)=1)
    stat = alterlist(reply->qual,(lreplycnt+ 9))
   ENDIF
   reply->qual[lreplycnt].donation_number = hp.product_nbr, reply->qual[lreplycnt].product_sub_nbr =
   hp.product_sub_nbr, reply->qual[lreplycnt].supplier_prefix = hp.supplier_prefix,
   reply->qual[lreplycnt].product_cd = hp.product_cd, reply->qual[lreplycnt].abo_cd = hp.abo_cd,
   reply->qual[lreplycnt].rh_cd = hp.rh_cd,
   reply->qual[lreplycnt].cross_reference = hp.cross_reference, reply->qual[lreplycnt].upload_dt_tm
    = hp.upload_dt_tm, reply->qual[lreplycnt].contributor_system_cd = hp.contributor_system_cd,
   reply->qual[lreplycnt].trans_dt_tm = hpe.event_dt_tm, reply->qual[lreplycnt].trans_vol = hpe
   .volume, reply->qual[lreplycnt].unit_meas_cd = hp.unit_meas_cd,
   reply->qual[lreplycnt].history_upload_ind = 1, reply->qual[lreplycnt].electronic_entry_flag = 0
  WITH nocounter
 ;end select
