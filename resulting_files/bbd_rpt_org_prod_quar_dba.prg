CREATE PROGRAM bbd_rpt_org_prod_quar:dba
 ...
 SET captions->rpt_title = uar_i18ngetmessage(i18nhandle,"rpt_title",
  "ORGANIZATION AND INVENTORY AREA / QUARANTINE REASON /")
 SET captions->rpt_title_2 = uar_i18ngetmessage(i18nhandle,"rpt_title_2","PRODUCT REPORT")
 ...
 SET captions->rpt_products = uar_i18ngetmessage(i18nhandle,"rpt_products","Products")
 ...
 SET captions->rpt_id = uar_i18ngetmessage(i18nhandle,"rpt_id","Report ID: BBD_RPT_ORG_PROD_QUAR")
 ...
 SET captions->rpt_bbd_cont = uar_i18ngetmessage(i18nhandle,"rpt_bbd_cont","bbd_rpt_org_prod_assay")
 ...
 SELECT INTO "cer_temp:bbdorgprodquar.txt"
  product_disp = uar_get_code_display(p.product_cd), quar_reason_disp = uar_get_code_display(q
   .quar_reason_cd), q.active_ind,
  s.organization_id, p.*, organization =
  IF (s.inventory_area_cd > 0) substring(1,30,uar_get_code_display(s.inventory_area_cd))
  ELSE substring(1,30,o.org_name)
  ENDIF
  FROM accept_quar_prod_r p,
   ...
   organization o
  PLAN (p
   WHERE p.accept_quar_prod_id > 0)
   JOIN (q
   WHERE q.accept_quar_reason_id=p.accept_quar_reason_id)
   JOIN (s
   WHERE s.org_shipment_id=q.org_shipment_id)
   JOIN (o
   WHERE o.organization_id=outerjoin(s.organization_id))
  ORDER BY organization, quar_reason_disp, product_disp,
  ...
  HEAD PAGE
   ...
   curdate"@DATECONDENSED;;d", row + 3, col 2,
   captions->rpt_org, col 45, captions->rpt_quar_reason,
   col 70, captions->rpt_active, col 85,
   captions->rpt_products, col 110, captions->rpt_active,
   row + 1, col 2, "------------------------------",
   col 45, "--------------------", col 70,
   "----------", col 85, "--------------------",
   col 110, "----------", row + 1
  ...
  HEAD quar_reason_disp
   col 45, quar_reason_disp"####################"
   IF (q.active_ind=1)
    col 73, captions->rpt_yes
   ELSE
    col 73, captions->rpt_no
   ENDIF
  HEAD product_disp
   col 85, product_disp"####################"
   IF (p.active_ind=1)
    col 112, captions->rpt_yes
   ELSE
    col 112, captions->rpt_no
   ENDIF
   row + 1
   IF (row > 56)
    BREAK
