CREATE PROGRAM bhs_rpt_powerform_extract:dba
 PROMPT
 ...
 IF (((findstring("@", $EMAIL) > 0) OR (cnvtupper( $EMAIL)="OPSJOB")) )
  IF (findstring("@", $EMAIL) > 0)
   SET email_ind = 1
  ENDIF
  SET var_output = concat(trim("powerform"))
  SET filedelimiter1 = '"'
  SET filedelimiter2 = ","
 ELSE
  SELECT INTO  $OUTDEV
 ...
 SELECT INTO concat(var_output)
  dcp_forms_ref_id = dfr.dcp_forms_ref_id, definition = trim(dfr.definition,3), description = trim(
   dfr.description,3),
  beg_effective_dt_tm = trim(format(dfr.beg_effective_dt_tm,";;q"),3), end_effective_dt_tm = trim(
   format(dfr.end_effective_dt_tm,";;q"),3)
  FROM dcp_forms_ref dfr
  PLAN (dfr
   WHERE dfr.end_effective_dt_tm >= cnvtdatetime(curdate,curtime)
    AND dfr.active_ind=1
    AND ((( $RUNTYPE="ALL")) OR (( $RUNTYPE="UPDATE")
    AND dfr.updt_dt_tm >= cnvtdatetime((curdate -  $DAYS),0)))
    AND  NOT (cnvtupper(dfr.description) IN ("ZZ*"))
    AND  NOT (cnvtupper(dfr.definition) IN ("ZZ*")))
  HEAD REPORT
   tline = build(char(34),"dcp_forms_ref_id",char(34),char(44),char(34),
    "definition",char(34),char(44),char(34),"description",
    char(34),char(44),char(34),"beg_effective_dt_tm",char(34),
    char(44),char(34),"end_effective_dt_tm",char(34),char(44)), col 0, tline,
   row + 1
  DETAIL
   tline = build(dcp_forms_ref_id,char(44),char(34),definition,char(34),
    char(44),char(34),description,char(34),char(44),
    char(34),beg_effective_dt_tm,char(34),char(44),char(34),
    end_effective_dt_tm,char(34),char(44)), col 0, tline,
   row + 1
