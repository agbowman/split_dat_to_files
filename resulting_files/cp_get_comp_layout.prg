CREATE PROGRAM cp_get_comp_layout
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "CP_NODE_ID" = 0.0
  WITH outdev, cpnodeid
 IF (validate(comp_layout) != 1)
  RECORD comp_layout(
    1 components[*]
      2 cp_component_id = f8
      2 cp_comp_display = vc
      2 group = i2
      2 column = i2
      2 row = i2
    1 status_data
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getcomponentdetails(null) = null WITH protect
 DECLARE verified = i2 WITH noconstant(0), protect
 DECLARE cp_node_id = f8 WITH constant( $CPNODEID), protect
 ...
 CALL getcomponentdetails(null)
 SET comp_layout->status_data.status = "S"
 SUBROUTINE getcomponentdetails(null)
   CALL log_message("In getComponentDetails()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE cp_size = i4 WITH noconstant(0), private
   SELECT INTO "nl:"
    FROM cp_component cc
    PLAN (cc
     WHERE cc.cp_node_id=cp_node_id)
    ORDER BY cc.component_seq_txt
    HEAD cc.cp_component_id
     cp_size = (size(comp_layout->components,5)+ 1), stat = alterlist(comp_layout->components,cp_size
      ), comp_layout->components[cp_size].cp_component_id = cc.cp_component_id,
     comp_layout->components[cp_size].cp_comp_display = uar_get_code_display(cc.comp_type_cd),
     comp_layout->components[cp_size].group = cnvtint(substring(1,1,cc.component_seq_txt)),
     comp_layout->components[cp_size].column = cnvtint(substring(3,1,cc.component_seq_txt)),
     comp_layout->components[cp_size].row = cnvtint(substring(5,1,cc.component_seq_txt))
    WITH nocounter
   ;end select
   CALL echorecord(comp_layout)
   CALL log_message(build("Exit getComponentDetails(),Elapsed time in seconds:",((curtime3 -
...
#exit_script
 IF (validate(debug_ind,0)=1)
  CALL echorecord(comp_layout)
 ENDIF
 CALL putjsonrecordtofile(comp_layout)
