CREATE PROGRAM bsc_cs_task_sel_sort_pref
 ...
 DECLARE degree_of_use = vc WITH noconstant("NO")
 ...
 SET reply->solcap[1].degree_of_use_str = degree_of_use
 IF (degree_of_use="YES")
  SET reply->solcap[1].degree_of_use_num = 1
 ENDIF
   ...
   IF (size(pref_reply->app.nv,5) > 0)
    SELECT INTO "nl:"
     ...
     DETAIL
      ...
      reply->solcap[1].other[2].value[res_count].value_str = pref_reply->app.nv[dprefno.seq].
      pvc_value, degree_of_use = "YES"
     FOOT REPORT
   ...
   IF (size(pref_reply->app.nv,5) > 0)
    SELECT INTO "nl:"
     ...
     DETAIL
      ...
      solcap[1].position[res_count].value_num = encounters, degree_of_use = "YES"
     FOOT REPORT
   ...
   IF (size(pref_reply->app.nv,5) > 0)
    SELECT INTO "nl:"
     ...
     DETAIL
      ...
      degree_of_use = "YES"
     FOOT REPORT
      stat = alterlist(reply->solcap[1].other[1].value,res_count), reply->solcap[1].
      distinct_user_count = res_count
     WITH nocounter
    ;end select
   ENDIF
 END ;Subroutine
