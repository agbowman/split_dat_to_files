CREATE PROGRAM dm2_aload_sch_indcol
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_sch_ind_col",".log","dm2_aload_sch_ind_col")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_sch_ind_col"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dasi_cnt = i4 WITH protect, noconstant(0)
 DECLARE dasi_str = vc WITH protect, noconstant("")
 DECLARE dasi_req_size = i4 WITH protect, noconstant(0)
 DECLARE dasi_ndx = i4 WITH protect, noconstant(0)
 FREE RECORD dasi_maint
 RECORD dasi_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET dasi_maint->num_of_inserts = 0
 SET dasi_maint->num_of_updates = 0
 SET dasi_maint->num_of_none = 0
 SET dasi_req_size = size(requestin->list_0,5)
 SET stat = alterlist(dasi_maint->qual,dasi_req_size)
 IF (dac_pop_coldic_rec("DM_AFD_INDEX_COLUMNS")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  SET dm_err->eproc = "Determining maintenance type for DM_AFD_INDEX_COLUMNS."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dasi_req_size))
  DETAIL
   dasi_maint->qual[d.seq].maint_type = "I", dasi_maint->num_of_inserts = (dasi_maint->num_of_inserts
   + 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 SELECT INTO "nl:"
  FROM dm_afd_tables dat,
   dm_afd_index_columns dai,
   (dummyt d  WITH seq = value(dasi_req_size))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (dat
   ...
   WHERE dai.owner=dat.owner
    ...
    AND (dai.index_name=requestin->list_0[d.seq].index_name)
    AND (dai.column_name=requestin->list_0[d.seq].column_name))
  ORDER BY dai.table_name, dai.index_name, dai.column_name
  DETAIL
   IF (cnvtint(requestin->list_0[d.seq].schema_instance) < dat.schema_instance)
    dasi_maint->qual[d.seq].maint_type = "N", dasi_maint->num_of_none = (dasi_maint->num_of_none+ 1),
    dasi_maint->num_of_inserts = (dasi_maint->num_of_inserts - 1)
   ...
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm) != cnvtdatetime(dai.updt_dt_tm))
    dasi_maint->qual[d.seq].maint_type = "U", dasi_maint->num_of_updates = (dasi_maint->
    num_of_updates+ 1), dasi_maint->num_of_inserts = (dasi_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dasi_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((dasi_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(dasi_req_size) )"),0)
  ...
  COMMIT
 ENDIF
 IF ((dasi_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where dasi_maint->qual[d.seq].maint_type = 'U' ",0)
  ...
  CALL dm2_push_cmd(" and requestin->list_0[d.seq].index_name = dcv.index_name ",0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_sch_ind_col completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_sch_ind_col")
END GO
