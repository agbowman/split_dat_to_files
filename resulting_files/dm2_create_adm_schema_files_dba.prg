CREATE PROGRAM dm2_create_adm_schema_files:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dcasf_get_next_date(dummy=i4) = i4
 DECLARE dcasf_populate_big_three(dummy=i4) = i4
 DECLARE dcasf_cas_curdate = q8 WITH protect
 DECLARE dcasf_dest = vc WITH protect
 DECLARE dcasf_prefix = vc WITH protect
 DECLARE dcasf_cas_sch_date_str = vc WITH public
 DECLARE dcasf_cas_sch_date_nbr = vc WITH public
 DECLARE dcasf_logfile = vc WITH protect, constant("dm2_create_adm_sch")
 DECLARE i_cnt = i2 WITH protect, noconstant(0)
 DECLARE t_cnt = i2 WITH protect, noconstant(0)
 DECLARE tc_cnt = i2 WITH protect, noconstant(0)
 DECLARE i_count = i2 WITH protect, noconstant(0)
 DECLARE ic_cnt = i2 WITH protect, noconstant(0)
 DECLARE c_cnt = i2 WITH protect, noconstant(0)
 DECLARE cc_cnt = i2 WITH protect, noconstant(0)
 DECLARE t_count = i2 WITH protect, noconstant(0)
 DECLARE td_cnt = i2 WITH protect, noconstant(0)
 DECLARE cd_cnt = i2 WITH protect, noconstant(0)
 DECLARE tp_cnt = i2 WITH protect, noconstant(0)
 DECLARE size_dmcolumn = i2 WITH protect
 DECLARE size_dmindcol = i2 WITH protect
 DECLARE size_dmcons = i2 WITH protect
 DECLARE size_dmconscol = i2 WITH protect
 DECLARE size_dmtbldoc = i2 WITH protect
 DECLARE size_dmcoldoc = i2 WITH protect
 DECLARE size_dmtsprec = i2 WITH protect
 DECLARE dcasf_lnksrv = vc WITH protect, noconstant(" ")
 DECLARE dcasf_tbl_name = vc WITH protect, noconstant(" ")
 SET dcasf_tbl_name =  $1
 FREE SET r2
 RECORD r2(
   1 rdate = dq8
 )
 SET r2->rdate =  $2
 CALL check_logfile(dcasf_logfile,".log","DM2_CREATE_ADM_SCHEMA_FILES LOGFILE")
 SET dm_err->eproc = "Beginning Create Schema Process"
 CALL disp_msg("",dm_err->logfile,0)
 IF (dm2_set_autocommit(1)=0)
  GO TO exit_program
 ENDIF
 IF (currdb="SQLSRV")
  ...
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_name="INHOUSE ORACLE LINKED SERVER"
    AND d.info_domain="DM2TOOLS"
   DETAIL
    dcasf_lnksrv = d.info_char
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF (dcasf_get_next_date(1) != 1)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SET dm2_install_schema->file_prefix = dcasf_cas_sch_date_str
 SET dm2_sch_file->dest_dir_cclfmt = "CCLUSERDIR"
 SET dm2_sch_file->file_prefix = build("dm2c",cnvtalphanum(format(dcasf_cas_curdate,"MM/DD/YYYY;;DD")
   ))
 IF ((dm_err->debug_flag > 1))
  SET dm_err->eproc = concat("File prefix is <",dm2_sch_file->file_prefix,">.")
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 CALL create_sch_files(null)
 SET dm_err->eproc = "Populate DMHEADER."
 INSERT  FROM dmheader dh
  SET dh.description = dcasf_cas_sch_date_str, dh.admin_load_ind = 0, dh.source_rdbms = "ORACLE",
   dh.sf_version = dm2_sch_file->sf_ver
  WITH nocounter
 ;end insert
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (dm2_set_autocommit(1)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Populate Get qualified date from target tables."
 IF (currdb="SQLSRV")
  SET modify openquery dcasf_lnksrv
 ENDIF
 SELECT INTO "nl:"
  FROM dm_adm_tables dat
  WHERE dat.schema_date=cnvtdatetime(r2->rdate)
   AND dat.table_name=dcasf_tbl_name
  ORDER BY dat.table_name
  DETAIL
   t_cnt = (t_cnt+ 1), stat = alterlist(dmtable->list,t_cnt), dmtable->list[t_cnt].table_name = dat
   .table_name,
   dmtable->list[t_cnt].pct_used = dat.pct_used, dmtable->list[t_cnt].pct_free = dat.pct_free,
   dmtable->list[t_cnt].schema_date = cnvtdatetime(dat.schema_date),
   dmtable->list[t_cnt].updt_dt_tm = cnvtdatetime(curdate,curtime3), dmtable->list[t_cnt].
   schema_instance = dat.schema_instance
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (t_cnt > 0)
  ...
  INSERT  FROM dmtable dt
   SET dt.table_name = dmtable->list[t_count].table_name, dt.tablespace_name = dm2_dft_clin_tspace,
    dt.index_tspace = dm2_dft_clin_itspace,
    dt.index_tspace_ni = 0, dt.long_tspace = dm2_dft_clin_ltspace, dt.long_tspace_ni = 0,
    dt.init_ext = dm2_dft_extsize, dt.next_ext = 100000, dt.pct_increase = dm2_dft_extsize,
    dt.pct_used = dmtable->list[t_count].pct_used, dt.pct_free = dmtable->list[t_count].pct_free, dt
    .bytes_allocated = 0,
    dt.bytes_used = 0, dt.schema_date = dmtable->list[t_count].schema_date, dt.alpha_feature_nbr = 0,
    dt.feature_number = 0, dt.updt_dt_tm = dmtable->list[t_count].updt_dt_tm, dt.schema_instance =
    dmtable->list[t_count].schema_instance
   WITH nocounter
  ;end insert
  SET dm_err->eproc = "Insert into DMTABLE."
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ...
 SELECT INTO "NL:"
  ...
  WHERE dac.table_name=dcasf_tbl_name
   AND dac.schema_date=cnvtdatetime(r2->rdate)
  ORDER BY dac.table_name, dac.column_seq
  DETAIL
   tc_cnt = (tc_cnt+ 1), stat = alterlist(dmcolumn->list,tc_cnt), dmcolumn->list[tc_cnt].table_name
    = dac.table_name,
   dmcolumn->list[tc_cnt].column_name = dac.column_name, dmcolumn->list[tc_cnt].column_seq = dac
   .column_seq, dmcolumn->list[tc_cnt].data_type = dac.data_type,
   dmcolumn->list[tc_cnt].data_length = dac.data_length, dmcolumn->list[tc_cnt].nullable = dac
   .nullable, dmcolumn->list[tc_cnt].data_default = dac.data_default
   IF (((size(trim(dac.data_default),1)=0) OR (((nullind(dac.data_default)=1) OR (dac.data_default=
   "''")) )) )
    dmcolumn->list[tc_cnt].data_default_ni = 1
   ENDIF
  WITH nocounter
 ;end select
 SET dm_err->eproc = "Selecting from dm_adm_columns."
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (currdb="SQLSRV")
  SET modify = noopenquery
 ENDIF
 SET size_dmcolumn = size(dmcolumn->list,5)
 INSERT  FROM dmcolumn dc,
   (dummyt d  WITH seq = size_dmcolumn)
  SET dc.table_name = dmcolumn->list[d.seq].table_name, dc.column_name = dmcolumn->list[d.seq].
   column_name, dc.column_seq = dmcolumn->list[d.seq].column_seq,
   dc.data_type = dmcolumn->list[d.seq].data_type, dc.data_length = dmcolumn->list[d.seq].data_length,
   dc.nullable = dmcolumn->list[d.seq].nullable,
   dc.data_default = dmcolumn->list[d.seq].data_default, dc.data_default_ni = dmcolumn->list[d.seq].
   data_default_ni
  PLAN (d)
   JOIN (dc
   WHERE (dmcolumn->list[d.seq].table_name=dc.table_name)
    AND (dmcolumn->list[d.seq].column_name=dc.column_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 SET dm_err->eproc = "Inserting into dmcolumn."
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 SELECT INTO "NL:"
  FROM dm_adm_indexes dai
  WHERE dai.table_name=dcasf_tbl_name
   AND dai.schema_date=cnvtdatetime(r2->rdate)
  ORDER BY dai.table_name, dai.index_name
  DETAIL
   i_cnt = (i_cnt+ 1), stat = alterlist(dmindex->list,i_cnt), dmindex->list[i_cnt].table_name = dai
   .table_name,
   dmindex->list[i_cnt].index_name = dai.index_name, dmindex->list[i_cnt].full_ind_name = dai
   .index_name, dmindex->list[i_cnt].pct_increase = dai.pct_increase,
   dmindex->list[i_cnt].pct_free = dai.pct_free, dmindex->list[i_cnt].unique_ind = dai.unique_ind
  WITH nocounter
 ;end select
 SET dm_err->eproc = "Selecting from dm_adm_indexes."
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 WHILE (i_count < i_cnt)
   ...
   INSERT  FROM dmindex di
    SET di.table_name = dmindex->list[i_count].table_name, di.index_name = dmindex->list[i_count].
     index_name, di.full_ind_name = dmindex->list[i_count].full_ind_name,
     di.pct_increase = dmindex->list[i_count].pct_increase, di.pct_free = dmindex->list[i_count].
     pct_free, di.init_ext = dm2_dft_extsize,
     di.next_ext = dm2_dft_extsize, di.unique_ind = dmindex->list[i_count].unique_ind, di
     .bytes_allocated = 0,
     di.bytes_used = 0
    WITH nocounter
   ;end insert
   SET dm_err->eproc = "Inserting into dmindex."
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
 ...
 ENDIF
 SELECT INTO "NL:"
  FROM dm_adm_index_columns dac
  WHERE dac.table_name=dcasf_tbl_name
   AND dac.schema_date=cnvtdatetime(r2->rdate)
  ORDER BY dac.table_name, dac.index_name, dac.column_position
  DETAIL
   ic_cnt = (ic_cnt+ 1), stat = alterlist(dmindcol->list,ic_cnt), dmindcol->list[ic_cnt].table_name
    = dac.table_name,
   dmindcol->list[ic_cnt].index_name = dac.index_name, dmindcol->list[ic_cnt].column_name = dac
   .column_name, dmindcol->list[ic_cnt].column_position = dac.column_position
  WITH nocounter
 ;end select
 SET dm_err->eproc = "Selecting from dm_adm_index_columns."
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (currdb="SQLSRV")
  SET modify = noopenquery
 ENDIF
 SET size_dmindcol = size(dmindcol->list,5)
 INSERT  FROM dmindcol dic,
   (dummyt d  WITH seq = size_dmindcol)
  SET dic.table_name = dmindcol->list[d.seq].table_name, dic.index_name = dmindcol->list[d.seq].
   index_name, dic.column_name = dmindcol->list[d.seq].column_name,
   dic.column_position = dmindcol->list[d.seq].column_position
  PLAN (d)
   JOIN (dic
   WHERE (dmindcol->list[d.seq].table_name=dic.table_name)
    AND (dmindcol->list[d.seq].column_name=dic.column_name)
    AND (dmindcol->list[d.seq].index_name=dic.index_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 SET dm_err->eproc = "Inserting into dmindcol."
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 SELECT INTO "NL:"
  FROM dm_adm_constraints dac
  WHERE dac.table_name=dcasf_tbl_name
   AND dac.schema_date=cnvtdatetime(r2->rdate)
   AND dac.delete_ind=0
  ORDER BY dac.table_name, dac.constraint_name
  DETAIL
   c_cnt = (c_cnt+ 1), stat = alterlist(dmcons->list,c_cnt), dmcons->list[c_cnt].table_name = dac
   .table_name,
   dmcons->list[c_cnt].constraint_name = dac.constraint_name, dmcons->list[c_cnt].full_cons_name =
   dac.constraint_name, dmcons->list[c_cnt].constraint_type = dac.constraint_type,
   dmcons->list[c_cnt].status_ind = dac.status_ind, dmcons->list[c_cnt].r_constraint_name = dac
   .r_constraint_name, dmcons->list[c_cnt].parent_table_name = dac.parent_table_name,
   dmcons->list[c_cnt].parent_table_columns = dac.parent_table_columns
  WITH nocounter
 ;end select
 SET dm_err->eproc = "Selecting from dm_adm_constraints."
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (currdb="SQLSRV")
  SET modify = noopenquery
 ENDIF
 SET size_dmcons = size(dmcons->list,5)
 INSERT  FROM dmcons dcs,
   (dummyt d  WITH seq = size_dmcons)
  SET dcs.table_name = dmcons->list[d.seq].table_name, dcs.constraint_name = dmcons->list[d.seq].
   constraint_name, dcs.full_cons_name = dmcons->list[d.seq].full_cons_name,
   dcs.constraint_type = dmcons->list[d.seq].constraint_type, dcs.status_ind = dmcons->list[d.seq].
   status_ind, dcs.r_constraint_name = dmcons->list[d.seq].r_constraint_name,
   dcs.parent_table_name = dmcons->list[d.seq].parent_table_name, dcs.parent_table_columns = dmcons->
   list[d.seq].parent_table_columns
  PLAN (d)
   JOIN (dcs
   WHERE (dmcons->list[d.seq].table_name=dcs.table_name)
    AND (dmcons->list[d.seq].constraint_name=dcs.constraint_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 SET dm_err->eproc = "Inserting into DMCONS."
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET cc_cnt = 0
 FREE RECORD dmconscol
 RECORD dmconscol(
   1 list[*]
     2 table_name = vc
     2 constraint_name = vc
 ...
 SELECT INTO "NL:"
  FROM dm_adm_constraints dac,
   dm_adm_cons_columns dacc
  WHERE dac.table_name=dcasf_tbl_name
   AND dac.schema_date=cnvtdatetime(r2->rdate)
   AND dac.table_name=dacc.table_name
   AND dac.schema_date=dacc.schema_date
   AND dac.constraint_name=dacc.constraint_name
   AND dac.delete_ind=0
  ORDER BY dacc.table_name, dacc.constraint_name, dacc.position
  DETAIL
   cc_cnt = (cc_cnt+ 1), stat = alterlist(dmconscol->list,cc_cnt), dmconscol->list[cc_cnt].table_name
    = dacc.table_name,
   dmconscol->list[cc_cnt].constraint_name = dacc.constraint_name, dmconscol->list[cc_cnt].
   column_name = dacc.column_name, dmconscol->list[cc_cnt].position = dacc.position
  WITH nocounter
 ;end select
 SET dm_err->eproc = "Selecting from dm_adm_constraints/dm_adm_cons_columns."
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (currdb="SQLSRV")
  SET modify = noopenquery
 ENDIF
 SET size_dmconscol = size(dmconscol->list,5)
 INSERT  FROM dmconscol cc,
   (dummyt d  WITH seq = size_dmconscol)
  SET cc.table_name = dmconscol->list[d.seq].table_name, cc.constraint_name = dmconscol->list[d.seq].
   constraint_name, cc.column_name = dmconscol->list[d.seq].column_name,
   cc.position = dmconscol->list[d.seq].position
  PLAN (d)
   JOIN (cc
   WHERE (dmconscol->list[d.seq].table_name=cc.table_name)
    AND (dmconscol->list[d.seq].constraint_name=cc.constraint_name)
    AND (dmconscol->list[d.seq].column_name=cc.column_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 SET dm_err->eproc = "Inserting into DMCONSCOL."
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (currdb="SQLSRV")
  IF (dcasf_populate_big_three(1) != 1)
   SET dm_err->err_ind = 1
   GO TO exit_program
  ENDIF
 ENDIF
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE dcasf_get_next_date(dummy1)
   SET dm_err->eproc = build("***> Getting next date to user for schema file <***")
   CALL disp_msg("",dm_err->logfile,0)
   SET dm_err->eproc = "Checking to see if an INHOUSE SCHEMA DATE row exists for possible insert."
   DECLARE dgnd_dm_info_fnd_ind = i2 WITH protect, noconstant(0)
   DECLARE dgnd_ccludir_fnd_ind = i2 WITH protect, noconstant(1)
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="DM2TOOLS"
     AND d.info_name="INHOUSE SCHEMA DATE"
    DETAIL
     dcasf_cas_curdate = d.info_date
    WITH nocounter, forupdate(d)
   ;end select
   IF (check_error(dm_err->eproc)=0)
    IF (curqual=0)
     SET dcasf_cas_curdate = cnvtdatetime("30-JUN-2003 00:00:00")
    ELSE
     SET dgnd_dm_info_fnd_ind = 1
     SET dcasf_cas_curdate = datetimeadd(dcasf_cas_curdate,1)
    ENDIF
   ELSE
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ROLLBACK
    RETURN(0)
   ENDIF
   WHILE (dgnd_ccludir_fnd_ind=1)
     SET dcasf_cas_sch_date_str = format(dcasf_cas_curdate,"DD-MMM-YYYY;;DD")
     SET dm2_install_schema->process_option = "INHOUSE"
     CASE (dm2_val_file_prefix(dcasf_cas_sch_date_str))
      OF 0:
       SET dm_err->user_action = "NONE"
       IF ((dm_err->debug_flag > 0))
        SET dm_err->eproc = concat("No CCL files found for schema date candidate <",
         dcasf_cas_sch_date_str,">")
        CALL disp_msg(" ",dm_err->logfile,0)
       ENDIF
       IF ((dm_err->err_ind=1))
        ROLLBACK
        RETURN(0)
       ENDIF
       SET dgnd_ccludir_fnd_ind = 0
      OF 1:
       SET dm_err->user_action = "NONE"
       SET dcasf_cas_curdate = datetimeadd(dcasf_cas_curdate,1)
       SET dgnd_ccludir_fnd_ind = 1
     ENDCASE
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat("dcasf_cas_sch_date_str=",dcasf_cas_sch_date_str))
      CALL echo(build("dgnd_ccludir_fnd_ind=",dgnd_ccludir_fnd_ind))
     ENDIF
   ENDWHILE
   IF (dgnd_dm_info_fnd_ind=0)
    SET dm_err->eproc = "Inserting INHOUSE SCHEMA DATE row to DM_INFO for the first time."
    INSERT  FROM dm_info d
     SET d.info_domain = "DM2TOOLS", d.info_name = "INHOUSE SCHEMA DATE", d.info_date = cnvtdatetime(
       dcasf_cas_curdate),
      d.info_long_id = 0, d.updt_applctx = 0, d.updt_dt_tm = cnvtdatetime(curdate,curtime3),
      d.updt_cnt = 0, d.updt_id = 0
     WITH nocounter
    ;end insert
   ELSE
    UPDATE  FROM dm_info d
     SET d.info_date = cnvtdatetime(dcasf_cas_curdate)
     WHERE info_domain="DM2TOOLS"
      AND info_name="INHOUSE SCHEMA DATE"
     WITH nocounter
    ;end update
   ENDIF
   IF (check_error(dm_err->eproc)=0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcasf_populate_big_three(dummy1)
   FREE RECORD dmtbldoc
   RECORD dmtbldoc(
     1 list[*]
       2 table_name = vc
   ...
   SELECT INTO "NL:"
    FROM dm_tables_doc dtd
    WHERE dtd.full_table_name=dcasf_tbl_name
    ORDER BY dtd.table_name
    DETAIL
     td_cnt = (td_cnt+ 1), stat = alterlist(dmtbldoc->list,td_cnt), dmtbldoc->list[td_cnt].table_name
      = dtd.table_name,
     dmtbldoc->list[td_cnt].data_model_section = dtd.data_model_section, dmtbldoc->list[td_cnt].
     description = dtd.description, dmtbldoc->list[td_cnt].definition = dtd.definition,
     dmtbldoc->list[td_cnt].static_rows = dtd.static_rows, dmtbldoc->list[td_cnt].updt_cnt = dtd
     .updt_cnt, dmtbldoc->list[td_cnt].reference_ind = dtd.reference_ind,
     dmtbldoc->list[td_cnt].human_reqd_ind = dtd.human_reqd_ind, dmtbldoc->list[td_cnt].drop_ind =
     dtd.drop_ind, dmtbldoc->list[td_cnt].table_suffix = dtd.table_suffix,
     dmtbldoc->list[td_cnt].full_table_name = dtd.full_table_name, dmtbldoc->list[td_cnt].
     suffixed_table_name = dtd.suffixed_table_name, dmtbldoc->list[td_cnt].person_cmb_trigger_type =
     dtd.person_cmb_trigger_type,
     dmtbldoc->list[td_cnt].encntr_cmb_trigger_type = dtd.encntr_cmb_trigger_type, dmtbldoc->list[
     td_cnt].default_row_ind = dtd.default_row_ind, dmtbldoc->list[td_cnt].merge_ui_query = dtd
     .merge_ui_query,
     dmtbldoc->list[td_cnt].mergeable_ind = dtd.mergeable_ind, dmtbldoc->list[td_cnt].
     merge_delete_ind = dtd.merge_delete_ind, dmtbldoc->list[td_cnt].merge_active_ind = dtd
     .merge_active_ind,
     dmtbldoc->list[td_cnt].data_disp_flag = dtd.data_disp_flag
    WITH nocounter
   ;end select
   SET dm_err->eproc = "Selecting from dm_tables_doc."
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (currdb="SQLSRV")
    SET modify = noopenquery
   ENDIF
   SET size_dmtbldoc = size(dmtbldoc->list,5)
   IF (size_dmtbldoc > 0)
    INSERT  FROM dmtbldoc td,
      (dummyt d  WITH seq = size_dmtbldoc)
     SET td.table_name = dmtbldoc->list[d.seq].table_name, td.data_model_section = dmtbldoc->list[d
      .seq].data_model_section, td.description = dmtbldoc->list[d.seq].description,
      td.definition = dmtbldoc->list[d.seq].definition, td.static_rows = dmtbldoc->list[d.seq].
      static_rows, td.updt_cnt = dmtbldoc->list[d.seq].updt_cnt,
      td.reference_ind = dmtbldoc->list[d.seq].reference_ind, td.human_reqd_ind = dmtbldoc->list[d
      .seq].human_reqd_ind, td.drop_ind = dmtbldoc->list[d.seq].drop_ind,
      td.suffixed_table_name = dmtbldoc->list[d.seq].suffixed_table_name, td.table_suffix = dmtbldoc
      ->list[d.seq].table_suffix, td.full_table_name = dmtbldoc->list[d.seq].full_table_name,
      td.person_cmb_trigger_type = dmtbldoc->list[d.seq].person_cmb_trigger_type, td
      .encntr_cmb_trigger_type = dmtbldoc->list[d.seq].encntr_cmb_trigger_type, td.default_row_ind =
      dmtbldoc->list[d.seq].default_row_ind,
      td.merge_ui_query = dmtbldoc->list[d.seq].merge_ui_query, td.mergeable_ind = dmtbldoc->list[d
      .seq].mergeable_ind, td.merge_delete_ind = dmtbldoc->list[d.seq].merge_delete_ind,
      td.merge_active_ind = dmtbldoc->list[d.seq].merge_active_ind, td.data_disp_flag = dmtbldoc->
      list[d.seq].data_disp_flag
     PLAN (d)
      JOIN (td
      WHERE (dmtbldoc->list[d.seq].table_name=td.table_name))
     WITH nocounter, outerjoin = d, dontexist
    ;end insert
    SET dm_err->eproc = "Inserting into dmtbldoc."
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   FREE RECORD dmcoldoc
   RECORD dmcoldoc(
     1 list[*]
       2 table_name = vc
       2 column_name = vc
   ...
   SELECT INTO "NL:"
    ...
    PLAN (dtd
     WHERE dtd.full_table_name=dcasf_tbl_name)
     JOIN (dcd
     WHERE dtd.table_name=dcd.table_name)
    ORDER BY dcd.table_name
    DETAIL
     tc_cnt = (tc_cnt+ 1), stat = alterlist(dmcoldoc->list,tc_cnt), dmcoldoc->list[tc_cnt].table_name
      = dcd.table_name,
     dmcoldoc->list[tc_cnt].column_name = dcd.column_name, dmcoldoc->list[tc_cnt].sequence_name = dcd
     .sequence_name, dmcoldoc->list[tc_cnt].code_set = dcd.code_set,
     dmcoldoc->list[tc_cnt].description = dcd.description, dmcoldoc->list[tc_cnt].definition = dcd
     .definition, dmcoldoc->list[tc_cnt].flag_ind = dcd.flag_ind,
     dmcoldoc->list[tc_cnt].updt_cnt = dcd.updt_cnt, dmcoldoc->list[tc_cnt].unique_ident_ind = dcd
     .unique_ident_ind, dmcoldoc->list[tc_cnt].root_entity_name = dcd.root_entity_name,
     dmcoldoc->list[tc_cnt].root_entity_attr = dcd.root_entity_attr, dmcoldoc->list[tc_cnt].
     constant_value = dcd.constant_value, dmcoldoc->list[tc_cnt].parent_entity_col = dcd
     .parent_entity_col,
     dmcoldoc->list[tc_cnt].exception_flg = dcd.exception_flg, dmcoldoc->list[tc_cnt].
     defining_attribute_ind = dcd.defining_attribute_ind, dmcoldoc->list[tc_cnt].merge_updateable_ind
      = dcd.merge_updateable_ind,
     dmcoldoc->list[tc_cnt].nls_col_ind = dcd.nls_col_ind, dmcoldoc->list[tc_cnt].absolute_date_ind
      = dcd.absolute_date_ind, dmcoldoc->list[tc_cnt].full_table_name = dtd.full_table_name,
     dmcoldoc->list[tc_cnt].merge_delete_ind = dcd.merge_delete_ind, dmcoldoc->list[tc_cnt].
     tz_rule_flag = dcd.tz_rule_flag
    FOOT REPORT
     stat = alterlist(dmcoldoc->list,tc_cnt)
    WITH nocounter
   ;end select
   SET dm_err->eproc = "Selecting from dm_colums_doc/dm_tables_doc."
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (currdb="SQLSRV")
    SET modify = noopenquery
   ENDIF
   SET size_dmcoldoc = size(dmcoldoc->list,5)
   IF (size_dmcoldoc > 0)
    INSERT  FROM dmcoldoc cd,
      (dummyt d  WITH seq = size_dmcoldoc)
     SET cd.table_name = dmcoldoc->list[d.seq].table_name, cd.column_name = dmcoldoc->list[d.seq].
      column_name, cd.sequence_name = dmcoldoc->list[d.seq].sequence_name,
      cd.code_set = dmcoldoc->list[d.seq].code_set, cd.description = dmcoldoc->list[d.seq].
      description, cd.definition = dmcoldoc->list[d.seq].definition,
      cd.flag_ind = dmcoldoc->list[d.seq].flag_ind, cd.updt_cnt = dmcoldoc->list[d.seq].updt_cnt, cd
      .unique_ident_ind = dmcoldoc->list[d.seq].unique_ident_ind,
      cd.root_entity_name = dmcoldoc->list[d.seq].root_entity_name, cd.root_entity_attr = dmcoldoc->
      list[d.seq].root_entity_attr, cd.constant_value = dmcoldoc->list[d.seq].constant_value,
      cd.parent_entity_col = dmcoldoc->list[d.seq].parent_entity_col, cd.exception_flg = dmcoldoc->
      list[d.seq].exception_flg, cd.defining_attribute_ind = dmcoldoc->list[d.seq].
      defining_attribute_ind,
      cd.merge_updateable_ind = dmcoldoc->list[d.seq].merge_updateable_ind, cd.nls_col_ind = dmcoldoc
      ->list[d.seq].nls_col_ind, cd.absolute_date_ind = dmcoldoc->list[d.seq].absolute_date_ind,
      cd.merge_delete_ind = dmcoldoc->list[d.seq].merge_delete_ind, cd.tz_rule_flag = dmcoldoc->list[
      d.seq].tz_rule_flag
     PLAN (d)
      JOIN (cd
      WHERE (dmcoldoc->list[d.seq].table_name=cd.table_name)
       AND (dmcoldoc->list[d.seq].column_name=cd.column_name))
     WITH nocounter, outerjoin = d, dontexist
    ;end insert
    SET dm_err->eproc = "Inserting into dmcoldoc."
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   FREE RECORD dmtsprec
   RECORD dmtsprec(
     1 list[*]
       2 table_name = vc
       2 precedence = i4
   ...
   ENDIF
   SELECT INTO "NL:"
    FROM dm_ts_precedence dtp,
     dm_tables_doc dtd
    PLAN (dtd
     WHERE dtd.full_table_name=dcasf_tbl_name)
     JOIN (dtp
     WHERE dtd.table_name=dtp.table_name)
    ORDER BY dtp.table_name
    DETAIL
     tp_cnt = (tp_cnt+ 1), stat = alterlist(dmtsprec->list,tp_cnt), dmtsprec->list[tp_cnt].table_name
      = dtp.table_name,
     dmtsprec->list[tp_cnt].precedence = dtp.precedence, dmtsprec->list[tp_cnt].data_tablespace = dtp
     .data_tablespace, dmtsprec->list[tp_cnt].data_extent_size = dtp.data_extent_size,
     dmtsprec->list[tp_cnt].index_tablespace = dtp.index_tablespace, dmtsprec->list[tp_cnt].
     index_extent_size = dtp.index_extent_size, dmtsprec->list[tp_cnt].long_tablespace = dtp
     .long_tablespace,
     dmtsprec->list[tp_cnt].long_extent_size = dtp.long_extent_size, dmtsprec->list[tp_cnt].
     full_table_name = dtd.full_table_name, dmtsprec->list[tp_cnt].updt_cnt = dtp.updt_cnt
    FOOT REPORT
     stat = alterlist(dmtsprec->list,tp_cnt)
    WITH nocounter
   ;end select
   SET dm_err->eproc = "Selecting from dm_ts_precedence/dm_tables_doc."
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (currdb="SQLSRV")
    SET modify = noopenquery
   ENDIF
   SET size_dmtsprec = size(dmtsprec->list,5)
   IF (size_dmtsprec > 0)
    INSERT  FROM dmtsprec tp,
      (dummyt d  WITH seq = size_dmtsprec)
     SET tp.table_name = dmtsprec->list[d.seq].table_name, tp.precedence = dmtsprec->list[d.seq].
      precedence, tp.data_tablespace = dmtsprec->list[d.seq].data_tablespace,
      tp.data_extent_size = dmtsprec->list[d.seq].data_extent_size, tp.index_tablespace = dmtsprec->
      list[d.seq].index_tablespace, tp.index_extent_size = dmtsprec->list[d.seq].index_extent_size,
      tp.long_tablespace = dmtsprec->list[d.seq].long_tablespace, tp.long_extent_size = dmtsprec->
      list[d.seq].long_extent_size, tp.updt_cnt = dmtsprec->list[d.seq].updt_cnt
     PLAN (d)
      JOIN (tp
      WHERE (dmtsprec->list[d.seq].table_name=tp.table_name)
       AND (dmtsprec->list[d.seq].precedence=tp.precedence))
     WITH nocounter, outerjoin = d, dontexist
    ;end insert
    SET dm_err->eproc = "Inserting into dmtsprec."
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SET dm_err->eproc = "DM2_CREATE_ADM_SCHEMA_FILES COMPLETED"
 CALL final_disp_msg(" ",dm_err->logfile,0)
#exit_program
 IF (currdb="SQLSRV")
  SET modify = noopenquery
 ENDIF
