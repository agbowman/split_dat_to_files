CREATE PROGRAM bb_rdm_import_ques_resp:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bb_rdm_import_ques_resp"
 ...
 SET num_responses = size(requestin->list_0,5)
 IF (num_responses=0)
  SET success_ind = 0
 ...
 ENDIF
 SET q_cnt = 1
 SET smsg = build("Number of responses going to import->",num_responses)
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 FOR (q_cnt = 1 TO num_responses)
   SET smsg = build("Import response number->",q_cnt)
   CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
   SET code_set = 0
   ...
   IF (module_cd=0)
    SET success_ind = 0
    SET smsg = build("Module code lookup failed->",cdf_meaning)
    CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    GO TO exit_script
   ENDIF
   SET code_set = 0
   ...
   IF (process_cd=0)
    SET success_ind = 0
    SET smsg = build("Process code lookup failed->",cdf_meaning)
    CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    GO TO exit_script
   ENDIF
   SET code_set = 0
   ...
   IF (question_cd=0)
    SET success_ind = 0
    SET smsg = build("Question code lookup failed->",cdf_meaning)
    CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    GO TO exit_script
   ENDIF
   IF ((requestin->list_0[q_cnt].response_mean="0"))
    SET response_cd = 0.0
   ELSE
    SET code_set = 0
    SET code_set = response_code_set
    SET response_cd = 0.0
    SET cdf_meaning = requestin->list_0[q_cnt].response_mean
    SET response_cd = getcodevaluefromcdf(code_set,cdf_meaning)
    IF (response_cd=0)
     SET success_ind = 0
     SET smsg = build("Response code lookup failed->",cdf_meaning)
     CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
     GO TO exit_script
    ENDIF
   ENDIF
   SELECT INTO "nl:"
    ...
    WHERE r.question_cd=question_cd
     AND r.response_cd=response_cd
    WITH nocounter
   ;end select
   IF (curqual=0
    AND success_ind=1)
    INSERT  FROM valid_response r
     SET r.module_cd = module_cd, r.question_cd = question_cd, r.process_cd = process_cd,
      r.response_cd = response_cd, r.updt_id = 1, r.updt_task = reqinfo->updt_task,
      r.updt_applctx = 1, r.updt_cnt = 0, r.updt_dt_tm = cnvtdatetime(curdate,curtime3)
     WITH nocounter
    ;end insert
    IF (curqual=0)
     SET success_ind = 0
     SET smsg = build("Unsuccessful import for response number->",q_cnt)
     CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
     GO TO exit_script
    ENDIF
   ENDIF
...
#exit_script
 IF (success_ind=0)
  SET smsg = build("Unsuccessful import valid responses.")
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  CALL logstatus("Execution Failed","F")
  CALL echorecord(readme_data)
  ROLLBACK
 ELSE
  SET smsg = build("Successful import valid responses.")
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
