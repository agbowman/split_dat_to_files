CREATE PROGRAM acm_add_nomen_entity_r:dba
 ...
 ENDIF
 IF ( NOT (validate(add_nomen_entity_r_request)))
  SET failed = not_found
  SET table_name = "add_nomen_entity_r_request was not defined by calling script"
  GO TO exit_script
 ENDIF
 IF ( NOT (validate(add_nomen_entity_r_reply)))
 ...
 SET add_nomen_entity_r_reply->qual_cnt = size(add_nomen_entity_r_request->qual,5)
 SET stat = alterlist(add_nomen_entity_r_reply->qual,add_nomen_entity_r_reply->qual_cnt)
 IF ((add_nomen_entity_r_reply->qual_cnt=0))
  SET failed = attribute_error
  SET table_name = "No nomenclature entity relationships passed in"
  GO TO exit_script
 ENDIF
 FOR (i = 1 TO add_nomen_entity_r_reply->qual_cnt)
   SET generic_ind = true
   IF (generic_ind)
    IF ((add_nomen_entity_r_request->qual[i].nomen_entity_reltn_id=0))
     SELECT INTO "nl:"
      nextseqnum = seq(entity_reltn_seq,nextval)
      FROM dual
      DETAIL
       add_nomen_entity_r_reply->qual[i].nomen_entity_reltn_id = nextseqnum,
       add_nomen_entity_r_request->qual[i].nomen_entity_reltn_id = nextseqnum
      WITH nocounter
     ;end select
     IF (curqual=0)
     ...
     ENDIF
    ELSE
     SET add_nomen_entity_r_reply->qual[i].nomen_entity_reltn_id = add_nomen_entity_r_request->qual[i
     ].nomen_entity_reltn_id
    ENDIF
   ENDIF
   IF ( NOT (generic_ind))
    SET add_nomen_entity_r_reply->qual[i].status = gen_nbr_error
    SET failed = gen_nbr_error
    SET table_name = "Failed to generate a sequence number"
    GO TO exit_script
   ENDIF
 ENDFOR
 INSERT  FROM nomen_entity_reltn t,
   (dummyt d  WITH seq = value(add_nomen_entity_r_reply->qual_cnt))
  SET t.nomen_entity_reltn_id = add_nomen_entity_r_request->qual[d.seq].nomen_entity_reltn_id, t
   .nomenclature_id = add_nomen_entity_r_request->qual[d.seq].nomenclature_id, t.parent_entity_name
    =
   IF ((add_nomen_entity_r_request->qual[d.seq].parent_entity_name > " ")) trim(
     add_nomen_entity_r_request->qual[d.seq].parent_entity_name)
   ELSE " "
   ENDIF
   ,
   t.parent_entity_id = add_nomen_entity_r_request->qual[d.seq].parent_entity_id, t.child_entity_name
    =
   IF ((add_nomen_entity_r_request->qual[d.seq].child_entity_name > " ")) trim(
     add_nomen_entity_r_request->qual[d.seq].child_entity_name)
   ELSE " "
   ENDIF
   , t.child_entity_id = add_nomen_entity_r_request->qual[d.seq].child_entity_id,
   t.reltn_type_cd = add_nomen_entity_r_request->qual[d.seq].reltn_type_cd, t.freetext_display = trim
   (add_nomen_entity_r_request->qual[d.seq].freetext_display), t.person_id =
   add_nomen_entity_r_request->qual[d.seq].person_id,
   t.encntr_id = add_nomen_entity_r_request->qual[d.seq].encntr_id, t.active_ind =
   add_nomen_entity_r_request->qual[d.seq].active_ind, t.beg_effective_dt_tm = cnvtdatetime(curdate,
    curtime3),
   t.end_effective_dt_tm = cnvtdatetime("31-DEC-2100 00:00:00.00"), t.updt_dt_tm = cnvtdatetime(
    curdate,curtime3), t.updt_id = reqinfo->updt_id,
   t.updt_task = reqinfo->updt_task, t.updt_cnt = 0, t.updt_applctx = reqinfo->updt_applctx,
   t.activity_type_cd = add_nomen_entity_r_request->qual[d.seq].activity_type_cd, t.priority =
   add_nomen_entity_r_request->qual[d.seq].priority, t.reltn_subtype_cd = add_nomen_entity_r_request
   ->qual[d.seq].reltn_subtype_cd,
   t.order_action_sequence = add_nomen_entity_r_request->qual[d.seq].order_action_sequence, t
   .inactive_order_action_sequence = add_nomen_entity_r_request->qual[d.seq].
   inactive_order_action_sequence
  PLAN (d)
   JOIN (t)
  WITH nocounter, status(add_nomen_entity_r_reply->qual[d.seq].status)
 ;end insert
 FOR (i = 1 TO add_nomen_entity_r_reply->qual_cnt)
   IF ((add_nomen_entity_r_reply->qual[i].status=0))
    SET failed = insert_error
    SET add_nomen_entity_r_reply->qual[i].status = insert_error
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
