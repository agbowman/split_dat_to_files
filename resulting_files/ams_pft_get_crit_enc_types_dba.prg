CREATE PROGRAM ams_pft_get_crit_enc_types:dba
 ...
 IF (size(pft_reply->encountertypes,5) > 0)
  ...
  FOR (loopcnt = 1 TO size(pft_reply->encountertypes,5))
    SET recordpos = getnextrecord(0)
    SET stat = setstringfield(recordpos,disppos,substring(1,100,pft_reply->encountertypes[loopcnt].
      encntrtypedisplay))
    SET stat = setrealfield(recordpos,valuepos,pft_reply->encountertypes[loopcnt].encntrtypecd)
  ENDFOR
