CREATE PROGRAM dm2_create_views:dba
 DECLARE dcv_tables = vc WITH protect, constant(cnvtupper( $1))
 DECLARE dcv_tbl_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcv_vcnt = i4 WITH protect, noconstant(0)
 DECLARE dcv_logfile = vc WITH protect, constant("dm2_create_views")
 DECLARE dcv_call_by_etu_ind = i2 WITH protect, noconstant(0)
 DECLARE dcv_acnt = i4 WITH protect, noconstant(0)
 DECLARE dcv_pkg_process = i4 WITH protect, noconstant(0)
 DECLARE dcv_pkg_num = i4 WITH protect, noconstant(0)
 FREE RECORD dm_objects
 RECORD dm_objects(
   1 obj_cnt[*]
     2 obj_name = vc
     2 suffix = vc
 ...
 IF (validate(dm2_etu->mode," ")="PACKAGE")
  SET dcv_call_by_etu_ind = 1
  SET dcv_pkg_process = 1
  SET dcv_pkg_num = dm2_etu->pkg_num
 ELSEIF (validate(dm2_etu->mode," ") != " ")
  SET dcv_pkg_process = 0
  SET dcv_call_by_etu_ind = 1
 ENDIF
 IF (check_logfile(dcv_logfile,".log","DM2_CREATE_VIEWS LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "BEGINNING DM2_CREATE_VIEWS"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (currdbuser != "V500"
  AND validate(dm2_create_views_for_user,0)=0)
  SET dm_err->eproc = "Exiting DM2_CREATE_VIEWS due to current database user."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (dcv_pkg_process=1)
  SET dm_err->eproc = concat(
   "ORACLE DATABASE DETECTED:  VERIFYING SYNONYM EXISTENCE FOR TABLES ON PACKAGE: ",trim(cnvtstring(
     dcv_pkg_num)))
  CALL disp_msg(" ",dm_err->logfile,0)
 ELSE
  SET dm_err->eproc = concat("ORACLE DATABASE DETECTED:  VERIFYING SYNONYM EXISTENCE FOR: ",trim(
    dcv_tables))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT
  IF (dcv_pkg_process=1)
   FROM dm_afd_tables dat,
    dm2_user_tables dut,
    dm2_dba_synonyms s
   WHERE dat.alpha_feature_nbr=dcv_pkg_num
    AND dat.owner=currdbuser
    AND dat.table_name=dut.table_name
    AND dut.table_name != "PLAN_TABLE"
    AND outerjoin(dut.table_name)=s.synonym_name
    AND s.owner=outerjoin("PUBLIC")
  ELSE
   FROM dm2_user_tables dut,
    dm2_dba_synonyms s
   WHERE dut.table_name=patstring(dcv_tables)
    AND dut.table_name != "PLAN_TABLE"
    AND outerjoin(dut.table_name)=s.synonym_name
    AND s.owner=outerjoin("PUBLIC")
  ENDIF
  ...
  HEAD REPORT
   dcv_tbl_cnt = 0
  DETAIL
   IF (((dut.table_name != s.synonym_name) OR (dut.table_name != s.table_name)) )
    IF ((dm_err->debug_flag > 0))
     CALL echo("NEW PUBLIC SYNONYM NEEDED OR IS INCORRECT")
    ENDIF
    dcv_tbl_cnt = (dcv_tbl_cnt+ 1)
    IF (mod(dcv_tbl_cnt,50)=1)
     stat = alterlist(dm_objects->obj_cnt,(dcv_tbl_cnt+ 49))
    ENDIF
    dm_objects->obj_cnt[dcv_tbl_cnt].obj_name = trim(dut.table_name)
   ENDIF
   IF (s.synonym_name != " "
    AND dut.table_name != s.table_name)
    IF ((dm_err->debug_flag > 0))
     CALL echo("RECREATE OF PUBLIC SYNONYM NEEDED, SO DO THE DROP")
    ENDIF
    dm_objects->obj_cnt[dcv_tbl_cnt].drop_ind = 1
   ENDIF
  FOOT REPORT
   stat = alterlist(dm_objects->obj_cnt,dcv_tbl_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (dcv_tbl_cnt=0)
  IF (dcv_pkg_process=1)
   SET dm_err->eproc = concat("SYNONYNM(S) EXISTS FOR TABLES ON PACKAGE ",trim(cnvtstring(dcv_pkg_num
      )),", NO SYNONYM(S) CREATED")
   CALL disp_msg(" ",dm_err->logfile,0)
  ELSE
   SET dm_err->eproc = concat("SYNONYNM(S) EXISTS FOR ",dcv_tables,", NO SYNONYM(S) CREATED")
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  GO TO exit_script
 ENDIF
 SET stat = alterlist(dm_objects->obj_cnt,dcv_tbl_cnt)
 FOR (dcv_vcnt = 1 TO dcv_tbl_cnt)
  EXECUTE dm2_create_views_c
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
 ENDFOR
#exit_script
 SET dm_err->eproc = "ENDING DM2_CREATE_VIEWS"
 CALL final_disp_msg(dcv_logfile)
 FREE RECORD dm_objects
END GO
