CREATE PROGRAM cp_add_component
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE concept_grp_cd_codeset = i4 WITH constant(4003133), protect
   IF (complistsize <= 0)
   ...
   FOR (x = 1 TO complistsize)
     CALL verifynonzerocodevalue(request->component_list[x].comp_type_cd,comp_type_cd_codeset,
      "Component Type")
     CALL checkcodevalue(request->component_list[x].concept_group_cd,concept_grp_cd_codeset)
     SELECT INTO "NL:"
      FROM cp_node cn
      WHERE (cn.cp_node_id=request->component_list[x].cp_node_id)
      WITH nocounter
     ;end select
     IF (curqual=0)
 ...
 SUBROUTINE insertrows(null)
   ...
   FOR (x = 1 TO size(request->component_list,5))
     ...
     INSERT  FROM cp_component cc
      SET cc.cp_node_id = request->component_list[x].cp_node_id, cc.comp_type_cd = request->
       component_list[x].comp_type_cd, cc.concept_group_cd = request->component_list[x].
       concept_group_cd,
       cc.component_seq_txt = request->component_list[x].component_seq_txt, cc.cp_component_id =
       cp_component_id, cc.updt_dt_tm = cnvtdatetime(sysdate),
       cc.updt_id = reqinfo->updt_id, cc.updt_task = reqinfo->updt_task, cc.updt_cnt = 0,
       cc.updt_applctx = reqinfo->updt_applctx, cc.component_display = request->component_list[x].
       component_display
      WITH nocounter
     ;end insert
     IF (curqual=0)
     ...
     ELSE
      ...
      SET reply->component_list[x].cp_node_id = request->component_list[x].cp_node_id
      SET reply->component_list[x].concept_group_cd = request->component_list[x].concept_group_cd
      SET reply->component_list[x].component_seq_txt = request->component_list[x].component_seq_txt
      SET reply->component_list[x].component_display = request->component_list[x].component_display
     ENDIF
   ENDFOR
