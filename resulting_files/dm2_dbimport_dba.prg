CREATE PROGRAM dm2_dbimport:dba
 ...
 SUBROUTINE dm2_get_program_stack(null)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_dbimport",".log","dm2_dbimport")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_dbimport"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 SET rdm_line_data->str = " "
 DECLARE rdm_line_len = i4 WITH protect, noconstant(0)
 DECLARE rdm_check_pos = i4 WITH protect, noconstant(0)
 DECLARE rdm_field_total = i4 WITH protect, noconstant(0)
 DECLARE rdm_for_cnt = i4 WITH protect, noconstant(1)
 DECLARE rdm_stat = i4 WITH protect, noconstant(0)
 DECLARE rdm_col_size = i4 WITH protect, noconstant(0)
 DECLARE dm_dbi_load_script = vc WITH protect, noconstant(" ")
 DECLARE dm_dbi_blocks = i4 WITH protect, noconstant(0)
 DECLARE dm_dbi_file_loc = vc WITH protect, noconstant(" ")
 DECLARE dm_dbi_csv_rows = i4 WITH protect, noconstant(0)
 DECLARE dm_dbi_start_row = i4 WITH protect, noconstant(1)
 DECLARE dm_dbi_end_row = i4 WITH protect, noconstant(1)
 DECLARE dm_unique_logical = vc WITH protect, noconstant(concat("dm_unique_logical",trim(cnvtstring(
     curtime))))
 DECLARE dm_dbi_rtl3_ind = i2 WITH protect, noconstant(0)
 DECLARE delim = vc WITH protect, noconstant("")
 SET dm_dbi_file_loc =  $1
 SET dm_dbi_load_script = cnvtupper( $2)
 SET dm_dbi_blocks =  $3
 IF (dm2_push_cmd(concat("set logical ",trim(value(dm_unique_logical),3),' "',trim(dm_dbi_file_loc,3),
   '" go'),1)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Check if CSV file exists:",dm_dbi_file_loc)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (findfile(dm_dbi_file_loc)=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Csv file could not be found"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Check if load script exists:",dm_dbi_load_script)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "NL:"
  FROM dprotect d
  WHERE d.object="P"
   AND d.object_name=dm_dbi_load_script
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Load program could not be found"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Check for new Block size from DM_INFO"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_DBIMPORT"
   AND d.info_name="BLOCK_MAX"
  DETAIL
   dm_dbi_blocks = d.info_number
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determine rows in csv file"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 FREE DEFINE rtl2
 ...
 IF (dm_dbi_rtl3_ind=1)
  ...
  FREE DEFINE rtl3
  DEFINE rtl3 value(dm_unique_logical)
 ELSE
  FREE DEFINE rtl2
  DEFINE rtl2 value(dm_unique_logical)
 ENDIF
 IF ((dm_err->debug_flag >= 2))
  ...
  CALL echo("*")
 ENDIF
 SET dm_err->eproc = "Parse column headings of csv file"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT
 ...
 IF ((dm_err->debug_flag >= 2))
  ...
  CALL echo("*")
 ENDIF
 SET dm_err->eproc = "Create requestin record structure"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SET rdm_stat = alterlist(rec_info->list_0,10)
 FOR (rdm_for_cnt = 1 TO rdm_col_size)
   IF (mod(rdm_for_cnt,10)=1
    AND rdm_for_cnt != 1)
    SET rdm_stat = alterlist(rec_info->list_0,(rdm_for_cnt+ 9))
   ENDIF
 ...
 FOR (rdm_for_cnt = 1 TO rdm_col_size)
  ...
  IF ((dm_err->debug_flag > 2))
   CALL echo(rec_info->list_0[rdm_for_cnt].rec_line)
  ENDIF
 ENDFOR
 CALL parser(")")
 IF ((dm_err->debug_flag > 2))
 ...
 WHILE ((dm_err->err_ind=0)
  AND dm_dbi_start_row < dm_dbi_csv_rows)
   ...
   CALL echo("-----")
   SET dm_err->eproc = "Populate the requestin structure"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   EXECUTE dm2_rdm_dbimport dm_dbi_file_loc, dm_dbi_start_row, dm_dbi_end_row,
   dm_dbi_rtl3_ind
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = concat("Execute the load script:",dm_dbi_load_script)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF (dm2_push_cmd(concat("execute ",dm_dbi_load_script," go"),1)=0)
    GO TO exit_script
   ENDIF
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM2_DBIMPORT completed."
 ENDIF
 CALL final_disp_msg("dm2_dbimport")
 ...
 FREE SET dm_dbi_load_script
 FREE SET dm_dbi_blocks
 FREE SET dm_dbi_file_loc
 FREE SET dm_dbi_csv_rows
 FREE SET dm_dbi_start_row
 FREE SET dm_dbi_end_row
 FREE SET dm_unique_logical
