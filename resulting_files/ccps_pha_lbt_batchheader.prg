CREATE PROGRAM ccps_pha_lbt_batchheader
 PROMPT
 ...
 DECLARE batch_header_sec(ncalc=i2) = f8 WITH protect
 DECLARE batch_header_secabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 ...
 END ;Subroutine
 SUBROUTINE batch_header_sec(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = batch_header_secabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE batch_header_secabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.920000), private
   DECLARE __label_data_batch_description = vc WITH noconstant(build2(build("Batch Name: ",trim(
       label_data->batch_description)),char(0))), protect
   DECLARE __fill_period = vc WITH noconstant(build2(build("Fill Period From: ",trim(label_data->
       fill_cycle.start_dt_tm_formatted)," To: ",trim(label_data->fill_cycle.stop_dt_tm_formatted)),
     char(0))), protect
   DECLARE __run_id = vc WITH noconstant(build2(build("Run_id: ",cnvtint(label_data->run.id)),char(0)
     )), protect
   DECLARE __run_user = vc WITH noconstant(build2(build("Run User: ",trim(label_data->run.
       user_username)),char(0))), protect
   DECLARE __run_date = vc WITH noconstant(build2(build("Printed: ",format(sysdate,
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__label_data_batch_description)
    SET rptsd->m_flags = 16
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 0.002)
    SET rptsd->m_width = 2.917
    SET rptsd->m_height = 0.208
    SET _dummyfont = uar_rptsetfont(_hreport,_default120)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("** START OF BATCH **",char(0)))
    SET rptsd->m_flags = 0
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__fill_period)
    SET rptsd->m_y = (offsety+ 0.625)
    SET rptsd->m_x = (offsetx+ 0.002)
    SET rptsd->m_width = 2.917
    SET rptsd->m_height = 0.146
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__run_id)
    SET rptsd->m_y = (offsety+ 0.771)
    SET rptsd->m_x = (offsetx+ 0.002)
    SET rptsd->m_width = 2.917
    SET rptsd->m_height = 0.146
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__run_user)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CCPS_PHA_LBT_BATCHHEADER"
 ...
 SELECT INTO "NL:"
  ...
  FROM output_dest o,
   ...
   printer p
  PLAN (o
   WHERE cnvtupper(o.name)=cnvtupper(trim(printer)))
   JOIN (d
 ...
 SET _fdrawheight = batch_header_sec(rpt_calcheight)
 IF (((_yoffset+ _fdrawheight) > _fenddetail))
  CALL pagebreak(0)
 ENDIF
 SET dummy_val = batch_header_sec(rpt_render)
