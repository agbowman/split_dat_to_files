CREATE PROGRAM cv_get_process_algorithm:dba
 ...
 IF (validate(predmort_only_event,0) != 0)
  IF (predmort_only_event=1)
   CALL cv_log_message("Leaving cv_get_process_algorithm: predmort only")
   GO TO exit_script
  ENDIF
 ENDIF
 ...
 ENDIF
 IF (validate(sections,"notdefined")="notdefined")
  RECORD sections(
    1 max_sec = i4
    1 max_input = i4
    1 seclist[*]
 ...
 IF ( NOT (validate(csm_name_type_current,0)))
  ...
  END ;Subroutine
 ENDIF
 DECLARE added_longtext = i4 WITH protect
 DECLARE cnt_algorithm = i4 WITH protect
 DECLARE cnt_component = i4 WITH protect
 DECLARE cnt_operator = i4 WITH protect
 DECLARE cnt_stack = i4 WITH protect
 DECLARE cnt_token = i4 WITH protect
 DECLARE failure = vc WITH protect, noconstant("F")
 IF (cnvtstring(validate(i,"notdef"))="notdef")
  DECLARE i = i4 WITH protect
 ELSE
  SET i = 0
 ENDIF
 DECLARE algidx = i4 WITH protect
 DECLARE j = i4 WITH protect
 ...
 DECLARE start = i4 WITH protect
 IF (cnvtstring(validate(stat,"notdef"))="notdef")
  DECLARE stat = i4 WITH protect
 ELSE
  SET stat = 0
 ENDIF
 DECLARE stop = i4 WITH protect
 DECLARE val = i4 WITH protect, noconstant(1)
 DECLARE username = vc WITH protect
 DECLARE record_status_cd = f8 WITH protect
 DECLARE num_format_cd = f8 WITH protect
 DECLARE alpha_format_cd = f8 WITH protect
 DECLARE action_status_cd = f8 WITH protect
 DECLARE action_status_cd_v = f8 WITH protect
 DECLARE action_status_cd_p = f8 WITH protect
 DECLARE action_status_cd_m = f8 WITH protect
 DECLARE num_cd = f8 WITH protect
 DECLARE text_cd = f8 WITH protect
 DECLARE result_status_cd = f8 WITH protect
 DECLARE event_reltn_cd = f8 WITH protect
 DECLARE update_event = i2 WITH protect, noconstant(0)
 DECLARE contributor_system_cd = f8 WITH protect
 ...
 DECLARE hstce = i4 WITH protect
 DECLARE hlsep = i4 WITH protect
 DECLARE hlsce = i4 WITH protect
 DECLARE grp_cd = f8 WITH protect
 DECLARE dcpgeneric_cd = f8 WITH protect
 DECLARE peid_grandparent = f8 WITH protect
 DECLARE clinical_event_id = f8 WITH protect
 DECLARE srvstat = i4 WITH protect
 DECLARE hcetype = i4 WITH protect
 DECLARE hcestruct = i4 WITH protect
 DECLARE hce = i4 WITH protect
 DECLARE hce2 = i4 WITH protect
 DECLARE hce3 = i4 WITH protect
 DECLARE hce4 = i4 WITH protect
 DECLARE modified_cd = f8 WITH protect
 DECLARE name_last = vc WITH protect
 DECLARE name_first = vc WITH protect
 DECLARE name_middle = vc WITH protect
 DECLARE surg_dt = vc WITH protect
 DECLARE predmort_xref_id = f8 WITH protect
 DECLARE discern_description = vc WITH protect, noconstant(" ")
 DECLARE discern_result_val = vc WITH protect, noconstant(" ")
 DECLARE lnumval = f8 WITH protect
 DECLARE rnumval = f8 WITH protect
 DECLARE mnumval = f8 WITH protect
 DECLARE lstrval = vc WITH protect
 DECLARE rstrval = vc WITH protect
 DECLARE alg_form_name = vc WITH protect, noconstant(" ")
 DECLARE alg_form_collating_seq = vc WITH protect, noconstant(" ")
 DECLARE token_alloc_size = i4 WITH protect, constant(500)
 DECLARE stack_alloc_size = i4 WITH protect, constant(100)
 DECLARE calc_components(prm_cc_idx=i4) = null
 DECLARE discern_notify(model=vc,result=vc) = i4
 DECLARE get_age(age=vc) = vc
 DECLARE get_fieldtype(index=i4) = i4
 DECLARE get_operands_fieldtype(nbr_operands=i4) = i4
 DECLARE get_value(index=i4) = vc
 DECLARE popstack(cv_dummy=i4) = vc
 DECLARE process_tokens(prm_idx=i4,prm_comp_type_flag=i4) = null
 DECLARE pushstack(psvalue=vc,pstype=i4) = i4
 DECLARE set_initial_token(prm_sit_idx=i4,prm_comp_type_flag=i4) = null
 EXECUTE crmrtl
 EXECUTE srvrtl
 SET reply->status_data.status = "F"
 CALL cv_log_message("read all algorithms for the current dataset")
 SELECT INTO "NL:"
  a.algorithm_id
  FROM cv_algorithm a,
   cv_component c,
  ...
  PLAN (a
   WHERE (a.dataset_id=cv_hrv_rec->dataset_id)
  ...
  DETAIL
   ...
   IF (lc.long_text_id != 0.0)
    algcomp->algorithm[cnt_algorithm].component[cnt_component].long_text = lc.long_text
   ENDIF
   algcomp->algorithm[cnt_algorithm].component[cnt_component].component_type_flag = c
   .component_type_flag
   IF (c.modifier != 0
    AND c.modifier != 1)
  ...
  WITH nocounter
 ;end select
 CALL cv_log_message("dump algcomp record")
 ...
 CALL get_person_info(0)
 CALL cv_log_message("process validations")
 DECLARE validated_cnt = i2 WITH protect, noconstant(0)
 DECLARE failed_cnt = i2 WITH protect, noconstant(0)
 IF (validate(calc_components_flag,0) != 0)
  IF (calc_components_flag=1)
   FOR (calc_start = 1 TO size(algcomp->algorithm,5))
     IF ((algcomp->algorithm[calc_start].description=calc_cmp->alg_desc))
      CALL echo("*******************************************")
      CALL echo("calc_components called!")
      CALL calc_components(calc_start)
      CALL echo("After calc_components!")
      CALL echo("*******************************************")
     ENDIF
   ENDFOR
  ENDIF
 ENDIF
 WHILE (val <= size(algcomp->algorithm,5))
  IF ((((algcomp->algorithm[val].validation_id=algcomp->algorithm[val].algorithm_id)) OR ((algcomp->
  algorithm[val].validation_id=0.0))) )
   SET validated_cnt = (validated_cnt+ 1)
   SET token->description = algcomp->algorithm[val].description
   CALL echo("*******************************************")
   CALL cv_log_message(build("validating:",algcomp->algorithm[val].description))
   CALL echo("*******************************************")
   CALL set_initial_token(val,cv_component_type_validation)
   CALL process_tokens(val,cv_component_type_validation)
   CALL get_response_values(0)
   CALL get_values_from_cv_hrv_rec(0)
   CALL do_postfix(0)
   ...
   IF (cnvtreal(stack->stack[1].value) != 0.0)
    CALL cv_log_message(build("validation succeeded for:",algcomp->algorithm[val].description,
      " RESULT:",stack->stack[1].value))
    SET algcomp->algorithm[val].result_val = stack->stack[1].value
    SET algcomp->algorithm[val].validated_ind = 1
    FOR (j = 1 TO size(algcomp->algorithm,5))
      IF ((algcomp->algorithm[val].algorithm_id=algcomp->algorithm[j].validation_id))
       SET algcomp->algorithm[j].validated_ind = algcomp->algorithm[val].validated_ind
       SET algcomp->algorithm[j].result_val = algcomp->algorithm[val].result_val
      ENDIF
    ENDFOR
    CALL cv_log_message("***********************************************")
    CALL cv_log_message(build("validation succeeded for:",algcomp->algorithm[val].description))
    CALL cv_log_message("***********************************************")
    IF ((((algcomp->algorithm[val].description="AVR OR MVR PREDMORT")) OR ((algcomp->algorithm[val].
    description="AVR+CABG OR MVR+CABG PREDMORT"))) )
     SET val = size(algcomp->algorithm,5)
    ENDIF
   ELSE
    CALL cv_log_message("***********************************************")
    CALL cv_log_message(build("validation failed for:",algcomp->algorithm[val].description))
    CALL cv_log_message("***********************************************")
  ...
  SET val = (val+ 1)
 ENDWHILE
 CALL echo(build("validated_cnt: ",validated_cnt))
 CALL echo(build("failed_cnt: ",failed_cnt))
 IF (failed_cnt=validated_cnt)
  CALL echo("All validations failed, exit!!")
  CALL cv_log_message("Dumping algcomp to cer_temp:cv_algcomp.dat:")
  CALL echorecord(algcomp,"cer_temp:cv_algcomp.dat")
  CALL cv_log_message("Dumping cv_hrv_rec to cer_temp:cv_hrv_rec_alg_failed.dat:")
  CALL echorecord(cv_hrv_rec,"cer_temp:cv_hrv_rec_alg_failed.dat")
  IF ((cv_hrv_rec->harvest_rec[1].top_parent_event_id > 0))
   CALL cv_log_message("Found valid event_id; posting to clinical event")
   CALL post_to_clinical_event(0)
  ELSE
   CALL cv_log_message("Did not find valid event_id; not posting to clinical event")
  ENDIF
  CALL post_to_cv_case_abstr_data(0)
  GO TO exit_script
 ENDIF
 CALL echo(build("Completed While"))
 DECLARE new_line = c2 WITH protect, constant(concat(char(13),char(10)))
 FOR (algidx = 1 TO size(algcomp->algorithm,5))
   IF ((algcomp->algorithm[algidx].validated_ind > 0))
    CALL echo("*******************************************")
    CALL cv_log_message(build("evaluating:",algcomp->algorithm[algidx].description))
    CALL echo("*******************************************")
    CALL set_initial_token(algidx,cv_component_type_evaluation)
    CALL process_tokens(algidx,cv_component_type_evaluation)
    CALL get_response_values(0)
    CALL get_values_from_cv_hrv_rec(0)
    CALL do_postfix(0)
    CALL cv_log_message("dump token")
    EXECUTE cv_log_struct  WITH replace(request,token)
    CALL cv_log_message("*****************************************")
    CALL cv_log_message(build("result:",stack->stack[cnt_stack].value))
    CALL cv_log_message("*****************************************")
    CALL cv_log_message("dump stack")
    SET algcomp->algorithm[algidx].result_val = stack->stack[cnt_stack].value
    SET discern_description = "***** The Report of the Predicted Risk Scores *****"
    SET discern_result_val = build(discern_result_val,algcomp->algorithm[algidx].description," = ",
     trim(algcomp->algorithm[algidx].result_val,3),new_line)
   ENDIF
 ENDFOR
 IF ((cv_hrv_rec->harvest_rec[1].top_parent_event_id > 0.0))
  CALL cv_log_message("Found valid event_id; posting to clinical event")
  CALL post_to_clinical_event(0)
  CALL post_to_cv_case_abstr_data(0)
 ELSE
  ...
  CALL post_to_cv_case_abstr_data(0)
 ENDIF
 CALL echo(discern_description)
 CALL echo(discern_result_val)
 CALL echo("**************************************************")
 CALL discern_notify(discern_description,discern_result_val)
#exit_script
 IF ((validate(timers->qual[1].count,- (1)) > - (1)))
  CALL echorecord(timers)
 ENDIF
 FREE RECORD token
 FREE RECORD stack
 ...
 SUBROUTINE cv_log_destroyhandle(dummy)
   ...
   ENDIF
 END ;Subroutine
 DECLARE cv_get_process_algorithm_vrsn = vc WITH private, constant("MOD 031 03/28/06 BM9013")
END GO
