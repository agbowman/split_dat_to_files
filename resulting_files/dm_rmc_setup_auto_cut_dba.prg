CREATE PROGRAM dm_rmc_setup_auto_cut:dba
 ...
 SUBROUTINE drmmi_backfill_mock_id(dbmi_env_id)
   ...
   RETURN(dbmi_mock_id)
 END ;Subroutine
 DECLARE drsac_cut_cnt = i4 WITH protect, noconstant(0)
 DECLARE drsac_task_flag = vc WITH protect, noconstant("")
 DECLARE drsac_loop = i4 WITH protect, noconstant(0)
 DECLARE drsac_event_name = vc WITH protect, noconstant("")
 DECLARE drsac_ptam_ind = i2 WITH protect, noconstant(0)
 DECLARE drsac_cbc_ind = i2 WITH protect, noconstant(0)
 DECLARE drsac_mock_ind = i2 WITH protect, noconstant(0)
 IF ((validate(drsac_request->environment_id,- (99.0))=- (99.0)))
 ...
 ENDIF
 IF (check_logfile("dm_rmc_setup_ac",".log","DM_RMC_RUN_SETUP_CUT LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_sac
 ENDIF
 IF ((((drsac_request->environment_id <= 0.0)) OR ((((drsac_request->source_env_id <= 0.0)) OR ((((
 drsac_request->cut_proc_cnt <= 0)) OR ((((drsac_request->db_pw <= " ")) OR ((((drsac_request->
 db_conn <= " ")) OR ((drsac_request->cut_dt_tm <= 0.0))) )) )) )) )) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "One or more values proveded in the request structure were incorrectly filled out."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_sac
 ENDIF
 SET dm_err->eproc = "Looking for active cutover processes"
 SELECT INTO "NL:"
  cnt = count(*)
  FROM dm_refchg_process p
  WHERE p.refchg_type=patstring("CUTOVER PROCESS*")
 ...
 IF (drsac_cut_cnt > 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "There is already a cutover in process, Scheduled Auto-Cutover will not be setup."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_sac
 ENDIF
 SET dm_err->eproc = "Looking for scheduled auto cutover rows in DM_REFCHG_TASK_QUEUE"
 SET drsac_cut_cnt = drtq_check_task_process("SCHEDULED AUTO-CUTOVER")
 IF (drsac_cut_cnt IN (2, 3, 1))
  CALL get_lock("SCHEDULED AUTO-CUTOVER","SETUP AUTO-CUTOVER",0,drl_reply)
  IF ((drl_reply->status="F"))
   GO TO exit_sac
  ELSEIF ((drl_reply->status="Z"))
   SET dm_err->err_ind = 1
   SET dm_err->emsg =
   "There is already another setup occurring for scheduled auto-cutover. This process did not perform anything."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_sac
  ENDIF
  IF (drsac_cut_cnt IN (2, 3))
   SET drsac_task_flag = drtq_delete_task_process("SCHEDULED AUTO-CUTOVER")
   IF (drsac_task_flag="F")
    GO TO exit_sac
   ENDIF
  ENDIF
 ELSEIF ((drsac_cut_cnt=- (1)))
  GO TO exit_sac
 ELSEIF (drsac_cut_cnt=4)
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "There is already a cutover in process, Scheduled Auto-Cutover will not be setup."
  GO TO exit_sac
 ENDIF
 SET dm_err->eproc = "Writing SCHEDULED AUTO-CUTOVER rows to DM_REFCHG_TASK_QUEUE"
 SET task_info->process_name = "SCHEDULED AUTO-CUTOVER"
 SET task_info->environment_id = drsac_request->environment_id
 SET task_info->total = (6+ drsac_request->cut_proc_cnt)
 SET stat = alterlist(task_info->qual,task_info->total)
 SET task_info->qual[1].task_desc = "Checking for $R CCL definitions"
 SET task_info->qual[1].task_level = 50
 SET task_info->qual[1].task_name = "execute DM_RMC_ORAGEN_R go"
 SET task_info->qual[1].task_reply = ""
 SET task_info->qual[1].task_request = ""
 SET task_info->qual[2].task_desc = "Stopping RDDS Movers"
 SET task_info->qual[2].task_level = 50
 SET task_info->qual[2].task_name = "execute DM2_STOP_REF_DATA_MOVER go"
 SET task_info->qual[2].task_reply = ""
 SET task_info->qual[2].task_request = ""
 SET task_info->qual[3].task_desc = "Running Dual Build Report"
 SET task_info->qual[3].task_level = 100
 SET task_info->qual[3].task_name = concat("execute DM_RMC_EXEC_DB_RPT ",trim(cnvtstring(
    drsac_request->source_env_id,20,1)),", ",trim(cnvtstring(drsac_request->environment_id,20,1)),
  " go")
 SET task_info->qual[3].task_reply = ""
 SET task_info->qual[3].task_request = ""
 SET task_info->qual[4].task_desc = "Acknowledging Dual Build Report"
 SET task_info->qual[4].task_level = 110
 SET task_info->qual[4].task_name = "execute DM_RMC_ACK_DUAL_BUILD go"
 SET task_info->qual[4].task_reply = ""
 SET task_info->qual[4].task_request = concat(
  "<REC_DEF>record dradb_request(1 src_id = f8 1 tgt_id = f8 1 event_reason = vc ",
  "1 ack_dt_tm = dq8) go</REC_DEF><VAL_SET>set dradb_request->src_id = ",trim(cnvtstring(
    drsac_request->source_env_id,20,1))," go</VAL_SET><VAL_SET>set dradb_request->tgt_id = ",trim(
   cnvtstring(drsac_request->environment_id,20,1)),
  " go</VAL_SET>",
  "<VAL_SET>set dradb_request->event_reason = 'Forced Auto-Cutover Acknowledge' go</VAL_SET>")
 FOR (drsac_loop = 1 TO drsac_request->cut_proc_cnt)
   SET task_info->qual[(drsac_loop+ 4)].task_desc = concat("Running Cutover Session ",trim(cnvtstring
     (drsac_loop)))
   SET task_info->qual[(drsac_loop+ 4)].task_level = 200
   SET task_info->qual[(drsac_loop+ 4)].task_name = concat('execute DM_RMC_CUTOVER_WRAPPER "',trim(
     cnvtstring(drsac_request->source_env_id,20,1)),'", "',trim(cnvtstring(drsac_loop)),'", " " go')
   SET task_info->qual[(drsac_loop+ 4)].task_reply = concat(
    "<REC_DEF>record DRCW_REPLY (1 status = c1 1 status_msg = vc) go",
    "</REC_DEF><ERR_IND>drcw_reply->status</ERR_IND><ERR_MSG>drcw_reply->status_msg</ERR_MSG>",
    "<NO_ERR_RESULT>S</NO_ERR_RESULT><ERR_TYPE>VC</ERR_TYPE>")
   SET task_info->qual[(drsac_loop+ 4)].task_request = ""
 ENDFOR
 SET task_info->qual[(drsac_request->cut_proc_cnt+ 5)].task_desc = "Gathering counts on $R tables"
 SET task_info->qual[(drsac_request->cut_proc_cnt+ 5)].task_level = 300
 SET task_info->qual[(drsac_request->cut_proc_cnt+ 5)].task_name = "execute DM_RMC_GET_R_INFO go"
 SET task_info->qual[(drsac_request->cut_proc_cnt+ 5)].task_reply = ""
 SET task_info->qual[(drsac_request->cut_proc_cnt+ 5)].task_request = ""
 SET task_info->qual[(drsac_request->cut_proc_cnt+ 6)].task_desc =
 "Logging Auto Cutover Started Event"
 SET task_info->qual[(drsac_request->cut_proc_cnt+ 6)].task_level = 150
 SET task_info->qual[(drsac_request->cut_proc_cnt+ 6)].task_name =
 "execute DM_RMC_AUTO_VERIFY_SETUP go"
 SET task_info->qual[(drsac_request->cut_proc_cnt+ 6)].task_request = concat(
  ...
  "<VAL_SET>set auto_ver_request->qual[1].rdds_event = 'Auto-Cutover Started' go</VAL_SET> ",
  "<VAL_SET>set auto_ver_request->qual[1].cur_environment_id = ",trim(cnvtstring(drsac_request->
    environment_id,20,1))," go</VAL_SET> ",
  "<VAL_SET>set auto_ver_request->qual[1].paired_environment_id = ",trim(cnvtstring(drsac_request->
    source_env_id,20,1)),
  " go</VAL_SET> ","<VAL_SET>set auto_ver_request->qual[1].event_reason = '' go</VAL_SET>")
 SET task_info->qual[(drsac_request->cut_proc_cnt+ 6)].task_reply = concat(
 ...
 IF ((drmmi_get_mock_id(drsac_request->environment_id)=drsac_request->environment_id))
  SET task_info->total = (task_info->total+ 1)
  SET stat = alterlist(task_info->qual,task_info->total)
  SET drsac_event_name = concat("Auto ",format(cnvtdatetime(curdate,curtime3),cclfmt->shortdatetime))
  SELECT INTO "NL:"
   FROM dm_env_reltn er
   WHERE (er.parent_env_id=drsac_request->source_env_id)
    AND (er.child_env_id=drsac_request->environment_id)
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_sac
  ELSEIF (curqual > 0)
   SET drsac_ptam_ind = 1
  ELSE
   SET drsac_ptam_ind = 0
  ENDIF
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="RDDS_CONFIGURATION"
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_sac
  ENDIF
  SET task_info->qual[task_info->total].task_desc = "Starting Open Event Process"
  SET task_info->qual[task_info->total].task_level = 350
  SET task_info->qual[task_info->total].task_name = concat("execute dm_rmc_wrp_open_event ",trim(
    cnvtstring(drsac_request->source_env_id,20,1)),",",trim(cnvtstring(drsac_request->environment_id,
     20,1)),",'",
   drsac_event_name,"',1,",trim(cnvtstring(drsac_ptam_ind)),",1,",trim(cnvtstring(drsac_cbc_ind)),
   " go")
  SET task_info->qual[task_info->total].task_request = ""
  SET task_info->qual[task_info->total].task_reply = ""
 ENDIF
 SET drsac_task_flag = drtq_insert_task_process(task_info,1)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->err_ind = 1
  ROLLBACK
  GO TO exit_sac
 ENDIF
 SET dm_err->eproc = "Adding SCHEDULED CUTOVER configuration rows to DM_INFO"
 UPDATE  FROM dm_info d
  SET d.info_char = concat(drsac_request->db_pw,"::",drsac_request->db_conn,"::",drsac_request->
    db_batch), d.info_number = drsac_request->cut_proc_cnt, d.info_date = cnvtdatetime(drsac_request
    ->cut_dt_tm)
  WHERE d.info_domain="RDDS CONFIGURATION"
   AND d.info_name=concat("SCHEDULED CUTOVER:",trim(cnvtstring(drsac_request->environment_id,20)))
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  INSERT  FROM dm_info d
   SET d.info_char = concat(drsac_request->db_pw,"::",drsac_request->db_conn,"::",drsac_request->
     db_batch), d.info_number = drsac_request->cut_proc_cnt, d.info_date = cnvtdatetime(drsac_request
     ->cut_dt_tm),
    d.info_domain = "RDDS CONFIGURATION", d.info_name = concat("SCHEDULED CUTOVER:",trim(cnvtstring(
       drsac_request->environment_id,20)))
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Updating RDDS STOP TIME to stop movers"
 UPDATE  FROM dm_info d
 ...
 IF (curqual=0)
  INSERT  FROM dm_info d
   SET d.info_date = cnvtdatetime(curdate,curtime3), d.info_domain = "DATA MANAGEMENT", d.info_name
     = "RDDS STOP TIME"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Adding DM_STAT_GATHER row to kick cutover off"
 UPDATE  FROM dm_info d
  SET info_number = 15, info_char = "ROUTINE"
 ...
 IF (curqual=0)
  INSERT  FROM dm_info
   SET info_number = 15, info_char = "ROUTINE", info_domain = "DM_STAT_GATHER",
    info_name = "DM_RMC_RUN_AUTO_CUT"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
 ...
 COMMIT
#exit_sac
 CALL remove_lock("SCHEDULED AUTO-CUTOVER","SETUP AUTO-CUTOVER",currdbhandle,drl_reply)
END GO
