CREATE PROGRAM dm2_shc_ccludir_cpcty:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dscc_err_status = c1 WITH protect, noconstant(" ")
 DECLARE dscc_pos = i4 WITH protect, noconstant(0)
 DECLARE dscc_beg_pos = i4 WITH protect, noconstant(0)
 DECLARE dscc_end_pos = i4 WITH protect, noconstant(0)
 DECLARE dscc_file_cnt = i4 WITH protect, noconstant(0)
 DECLARE dscc_file_size = f8 WITH protect, noconstant(0.0)
 DECLARE dscc_temp = vc WITH protect, noconstant(" ")
 IF (check_logfile("DM2_SHC_CCL_CPCTY",".log","DM2_SHC_CCLUDIR_CPCTY LOG FILE")=0)
  SET dscc_err_status = "F"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "System Health Check working on CCLUSERDIR capacity..."
 CALL disp_msg("",dm_err->logfile,0)
 ...
 ENDIF
 IF (cursys="AIX")
  IF (dm2_push_dcl("ls -lR $CCLUSERDIR | wc -l")=0)
   SET dscc_err_status = "E"
  ELSE
   FREE DEFINE rtl2
   FREE SET file_loc
   SET logical file_loc value(dm_err->errfile)
   ...
   SELECT INTO "nl:"
    ...
    FROM rtl2t r
    DETAIL
     dscc_file_cnt = cnvtint(build(r.line))
    WITH nocounter
   ;end select
   IF (check_error(concat("Parse out ccluserdir file count.")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dscc_err_status = "E"
   ENDIF
   IF ((dm_err->debug_flag >= 1))
    CALL echo(build("file_cnt =",dscc_file_cnt))
   ENDIF
   IF (dm2_push_dcl("du -s -k $CCLUSERDIR")=0)
    SET dsce_err_status = "E"
   ELSE
    SET dscc_err_status = "R"
    FREE DEFINE rtl2
    FREE SET file_loc
    SET logical file_loc value(dm_err->errfile)
    DEFINE rtl2 "file_loc"
    SELECT INTO "nl:"
     ...
     FROM rtl2t r
     DETAIL
      dscc_pos = findstring("/",r.line), dscc_file_size = cnvtreal(substring(1,(dscc_pos - 2),r.line)
       )
     WITH nocounter
    ;end select
    IF (check_error(concat("Parse out ccluserdir file count.")) != 0)
    ...
    ENDIF
    IF ((dm_err->debug_flag >= 1))
     CALL echo(build("file_size =",dscc_file_size))
    ENDIF
   ENDIF
  ENDIF
 ELSEIF (cursys="AXP")
  IF (dm2_push_dcl("dir ccluserdir /size/total")=0)
   SET dscc_err_status = "E"
  ELSE
   SET dscc_err_status = "R"
   FREE DEFINE rtl2
   ...
   SELECT INTO "nl:"
    ...
    FROM rtl2t r
    DETAIL
     dscc_beg_pos = findstring("Total of ",r.line)
     IF (dscc_beg_pos > 0)
      dscc_end_pos = findstring(" files",r.line), dscc_file_cnt = cnvtint(substring((dscc_beg_pos+ 9),
        ((dscc_end_pos - dscc_beg_pos) - 9),r.line))
      IF ((dm_err->debug_flag >= 1))
       CALL echo(build("file_cnt =",dscc_file_cnt))
      ENDIF
      dscc_beg_pos = (dscc_end_pos+ 8), dscc_temp = substring(dscc_beg_pos,dscc_end_pos,r.line),
      dscc_end_pos = findstring(" ",dscc_temp),
      dscc_file_size = cnvtreal(substring(dscc_beg_pos,(dscc_end_pos - 1),r.line))
      IF ((dm_err->debug_flag >= 1))
       CALL echo(build("file_size =",dscc_file_size))
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(concat("Parse out ccluserdir file count and file size.")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dscc_err_status = "E"
   ENDIF
  ENDIF
 ENDIF
 IF (dshc_add_check_to_rs("CCLUSERDIR CAPACITY","CCLUSERDIR Capacity:  # of Files and Total",0,
  dscc_err_status,dshc_ctf_millennium)=0)
  SET dscc_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dscc_err_status="E")
  GO TO exit_script
 ENDIF
 ...
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Number of Files"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Number of Files:"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 1
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].nbr_value = dscc_file_cnt
 SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
 SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
 SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Total Size"
 IF (cursys="AIX")
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Total Size (kb):"
 ELSEIF (cursys="AXP")
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Total Size (blocks):"
 ENDIF
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 1
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].nbr_value = dscc_file_size
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "Make sure the number of files in the CCLUSERDIR directory is managed.  ",
  "Having a large number of files can degrade system performance.  This directory is ",
  "mostly used for log and temporary files and should not be used to store permanent files.  ",
  "One thing to consider is that when the system was created, this directory contained ",
  "only the login_default.ccl file.")
#exit_script
 IF ((dm_err->debug_flag >= 5))
  CALL echorecord(dshc)
  CALL echorecord(dm_err)
 ENDIF
 IF (dscc_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_CCLUDIR_CPCTY WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_CCLUDIR_CPCTY WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_CCLUDIR_CPCTY COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_CCL_CPCTY")
END GO
