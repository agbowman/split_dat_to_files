CREATE PROGRAM dm_rxs_worklist_event_rows:dba
 ...
 SET reply->table_name = "RXS_WORKLIST_EVENT"
 ...
 ELSE
  SELECT INTO "nl:"
   rwe.rowid
   FROM rxs_worklist_event rwe
   WHERE parser(sbr_getrowidnotexists("rwe.updt_dt_tm < cnvtdatetime(v_days_back)","rwe"))
    AND rwe.rxs_worklist_event_id != 0
   HEAD REPORT
    row_cnt = 0
   DETAIL
    row_cnt = (row_cnt+ 1)
    ...
    ENDIF
    reply->rows[row_cnt].row_id = rwe.rowid
   FOOT REPORT
    stat = alterlist(reply->rows,row_cnt)
   WITH nocounter, maxqual(rwe,value(request->max_rows))
  ;end select
