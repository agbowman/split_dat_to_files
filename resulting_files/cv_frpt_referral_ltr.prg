CREATE PROGRAM cv_frpt_referral_ltr
 PROMPT
 ...
 IF (validate(i18nuar_def,999)=999)
  ...
  persist
 ENDIF
 EXECUTE cv_frpt_referral_ltr_drv "NL:",  $START_DATE,  $END_DATE,
  $PHYSICIANID,  $ACTIVITYSUBTYPE,  $PROCEDURESTATUS,
  $ORG_ID
 ...
 DECLARE headprimaryphyssection(ncalc=i2) = f8 WITH protect
 DECLARE headprimaryphyssectionabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE detailsection(ncalc=i2) = f8 WITH protect
 DECLARE detailsectionabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE footprimaryphyssection(ncalc=i2) = f8 WITH protect
 DECLARE footprimaryphyssectionabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 ...
 DECLARE _times1210485760 = i4 WITH noconstant(0), protect
 DECLARE _times10b10485760 = i4 WITH noconstant(0), protect
 ...
 DECLARE strlayout = vc WITH protect
 ...
 SUBROUTINE get_reply(dummy)
  ...
  ENDIF
  SELECT
   referral_rep_primary_phy = substring(1,30,reply->referral_rep[d1.seq].primary_phy),
   referral_rep_description = substring(1,40,reply->referral_rep[d1.seq].description),
   referral_rep_proc_disp = substring(1,40,reply->referral_rep[d1.seq].proc_disp),
   referral_rep_status_disp = substring(1,30,reply->referral_rep[d1.seq].status_disp),
   referral_rep_proc_date = reply->referral_rep[d1.seq].proc_date, referral_rep_gen_date = reply->
   referral_rep[d1.seq].gen_date
   FROM (dummyt d1  WITH seq = value(size(reply->referral_rep,5)))
   PLAN (d1)
   ORDER BY referral_rep_primary_phy
   HEAD REPORT
    _d0 = referral_rep_primary_phy, _d1 = referral_rep_description, _d2 = referral_rep_proc_disp,
    _d3 = referral_rep_status_disp, _d4 = referral_rep_proc_date, sdate = format(cnvtdatetime(
    ...
    ndaylightvar = 0, stimezone = datetimezonebyindex(curtimezoneapp,noffsetvar,ndaylightvar,7,
     cnvtdatetime(curdate,curtime)), nproccnt = 0,
    ntotalproc = 0, tgraph = 0, tgraph_cnt = 0,
    max_tgraph = 0, graph_max = 0, p_curr = 0,
    p_max = 0, curparsedone = 0, param_pos = 1,
    param_sep = "|", param_string = nullterm(trim( $PROCEDURESTATUS))
    WHILE ((param_pos != - (1)))
      curpos = findstring(param_sep,param_string,param_pos)
      IF (curpos=0)
       curpos = (size(param_string,1)+ 1), curparsedone = 1
      ENDIF
      strtitle = trim(substring(param_pos,(curpos - param_pos),param_string),3)
      IF (strtitle="CANCELLED")
       strtitle = "Cancelled"
      ELSEIF (strtitle="COMPLETED")
       strtitle = "Completed"
      ELSEIF (strtitle="SAVED")
       strtitle = "Saved"
      ELSEIF (strtitle="UNSIGNED")
       strtitle = "Unsigned"
      ENDIF
      IF (((strtitle="Cancelled") OR (((strtitle="Completed") OR (((strtitle="Saved") OR (strtitle=
      "Unsigned")) )) )) )
       IF (size(strtitle) > 0)
        IF (size(strlayout) > 0)
         strlayout = build2(strlayout,", ",trim(strtitle),"  ")
        ELSE
         strlayout = build(strtitle,"  ")
        ENDIF
       ENDIF
      ENDIF
      param_pos = (curpos+ size(param_sep,1))
      IF (curparsedone=1)
       param_pos = - (1)
      ENDIF
    ENDWHILE
   ...
   HEAD referral_rep_primary_phy
    _fdrawheight = headprimaryphyssection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = headprimaryphyssection(rpt_render)
   DETAIL
    nproccnt = (nproccnt+ 1), tgraph_cnt = (1+ tgraph_cnt), _fdrawheight = detailsection(
     rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = detailsection(rpt_render)
   FOOT  referral_rep_primary_phy
    ntotalproc = (ntotalproc+ nproccnt), tgraph = tgraph_cnt, p_curr = tgraph_cnt
    IF (p_curr >= p_max)
     p_max = p_curr
    ENDIF
    _fdrawheight = footprimaryphyssection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = footprimaryphyssection(rpt_render), nproccnt = 0, tgraph_cnt = 0,
    stat = alterlist(rptgraphrec->m_labels,_nfieldscnt), rptgraphrec->m_labels[_nfieldscnt].label =
    build2(
     IF (trim(referral_rep_primary_phy)=null) "No Provider Entered"
     ELSE referral_rep_primary_phy
     ENDIF
     ,char(0)), stat = alterlist(rptgraphrec->m_series[1].y_values,_nfieldscnt),
    rptgraphrec->m_series[1].y_values[_nfieldscnt].y_f8 = tgraph, _nfieldscnt = (_nfieldscnt+ 1)
   FOOT PAGE
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.188),(offsety+ 1.255),(offsetx+ 7.188),(offsety+
     1.255))
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
      IF (strlayout=uar_i18ngetmessage(_hi18nhandle,"ALLREPORTS",
       "Saved, Unsigned, Completed, Cancelled")) build2(uar_i18ngetmessage(_hi18nhandle,
         "ALLREFLETTERS","All Referral letters"))
      ELSEIF (size(strlayout) > 0) build2(strlayout,uar_i18ngetmessage(_hi18nhandle,"REFLETTER",
         " Referral letter(s)"))
      ELSE build2(uar_i18ngetmessage(_hi18nhandle,"ALLLETTERS","All Referral letter(s)"))
      ENDIF
      ,char(0)))
 ...
 SUBROUTINE headpagesectionabs(ncalc,offsetx,offsety)
   ...
   IF ( NOT (((( $U_VIEW=0)) OR (( $U_VIEW=2)))
    AND size(reply->referral_rep,5) > 0))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    ...
    SET rptsd->m_height = 0.250
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "LETTERDESCRIPTION","Letter Description"),char(0)))
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 5.438)
    SET rptsd->m_width = 0.688
    SET rptsd->m_height = 0.250
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,"DATE",
       "Date"),char(0)))
    SET rptsd->m_y = (offsety+ 0.063)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE headprimaryphyssection(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headprimaryphyssectionabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headprimaryphyssectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
      IF (trim(referral_rep_primary_phy) != null) referral_rep_primary_phy
      ENDIF
      ,char(0)))
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 0.250)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
      IF (trim(referral_rep_primary_phy)=null) uar_i18ngetmessage(_hi18nhandle,"NOPROVIDER",
        "No Provider Found")
      ENDIF
      ,char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
 ...
 SUBROUTINE detailsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.250
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(referral_rep_description,char(0)))
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 2.750)
    SET rptsd->m_width = 2.625
    SET rptsd->m_height = 0.250
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(referral_rep_proc_disp,char(0)))
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 5.375)
    SET rptsd->m_width = 0.938
    SET rptsd->m_height = 0.250
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(format(referral_rep_proc_date,
       "@SHORTDATE;;D"),char(0)))
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 6.375)
    SET rptsd->m_width = 1.063
    SET rptsd->m_height = 0.250
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(referral_rep_status_disp,char(0)))
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE footprimaryphyssection(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = footprimaryphyssectionabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE footprimaryphyssectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.500),(offsety+ 0.063),(offsetx+ 7.250),(offsety+
     0.063))
    ...
    SET _dummyfont = uar_rptsetfont(_hreport,_times10b10485760)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(nproccnt),char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
 ...
 SUBROUTINE footreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.260
    SET _oldfont = uar_rptsetfont(_hreport,_times1210485760)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(uar_i18ngetmessage(_hi18nhandle,
        "GTOTAL","Overall Referral letters: "),cnvtstring(ntotalproc)),char(0)))
 ...
 SUBROUTINE graphsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptgraphrec->m_fleft = (0.438000+ offsetx)
    SET rptgraphrec->m_ftop = (0.125000+ offsety)
    SET rptgraphrec->m_fwidth = 6.000000
    SET rptgraphrec->m_fheight = 3.125000
    SET rptgraphrec->m_stitle = ""
    SET rptgraphrec->m_ssubtitle = ""
    SET rptgraphrec->m_sxtitle = ""
    SET rptgraphrec->m_sytitle = "Total Referral Letters"
 ...
 SUBROUTINE nodatasectionabs(ncalc,offsetx,offsety)
   ...
   IF ( NOT (size(reply->referral_rep,5) <= 0))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 16
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CV_FRPT_REFERRAL_LTR"
 ...
 SUBROUTINE _createfonts(dummy)
   ...
   SET _times10b10485760 = uar_rptcreatefont(_hreport,rptfont)
   SET rptfont->m_pointsize = 12
   SET rptfont->m_bold = rpt_off
   SET _times1210485760 = uar_rptcreatefont(_hreport,rptfont)
   SET rptfont->m_pointsize = 10
   SET rptfont->m_bold = rpt_on
   SET rptfont->m_rgbcolor = rpt_black
