CREATE PROGRAM acm_del_encounter:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_id=acm_request->encounter_qual[xref->del[d.seq].idx].encntr_id))
  WITH nocounter, status(reply->encounter_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->encounter_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    FROM encounter e
    WHERE expand(t1,f_val,t_val,e.encntr_id,acm_request->encounter_qual[xref->del[t1].idx].encntr_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,e.encntr_id,acm_request->encounter_qual[xref->del[t1].idx].
      encntr_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->encounter_qual[xref->del[index].idx].status=0))
