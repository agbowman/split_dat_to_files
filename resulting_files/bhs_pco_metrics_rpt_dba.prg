CREATE PROGRAM bhs_pco_metrics_rpt:dba
 ...
 DECLARE mf_sunquest_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",89,"SUNQUEST"))
 DECLARE mf_esi_entry_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",29520,"ESI"))
 DECLARE mf_performed_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",21,"PERFORM"))
 DECLARE mf_doc_disp_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",53,"DOC"))
 DECLARE mf_grp_disp_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",53,"GRP"))
 DECLARE mf_num_disp_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",53,"NUM"))
 DECLARE mf_txt_disp_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",53,"TXT"))
 ...
 IF (validate(request->batch_selection))
  ...
  SET email_list =  $1
 ELSE
  SET t_record->beg_date = cnvtdatetime("01-MAR-2009 00:00:00")
  SET t_record->end_date = cnvtdatetime("31-MAR-2009 23:59:59")
 ...
 SELECT INTO "nl:"
  ...
  PLAN (pe)
   ...
   WHERE ce.event_id=cep.event_id
    ...
    AND ce.valid_until_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00")
    AND trim(ce.accession_nbr) IN ("", " ", null)
    AND  NOT (ce.entry_mode_cd IN (781113.00, 882489.00, 882491.00)))
   JOIN (ce1
   WHERE ce1.parent_event_id=ce.event_id
    AND ce1.event_class_cd=doc_cd
    AND  NOT (ce1.entry_mode_cd IN (781113.00, 882489.00, 882491.00)))
   JOIN (ceb
 ...
 SET nbuckets = value((1+ ((ntotal - 1)/ nbucketsize)))
 SET indx1 = 1
 CALL echo("Dicated Notes for Appts")
 DECLARE tmp_ind = i4 WITH noconstant(0)
 SELECT INTO "nl:"
  ce.encntr_id
  FROM (dummyt d  WITH seq = nbuckets),
   clinical_event ce,
  ...
  PLAN (d
   ...
   WHERE expand(indx,nstart,(nstart+ (nbucketsize - 1)),ce.encntr_id,t_record->encntr_appt_qual[indx]
    .encntr_id)
    AND ((ce.event_class_cd+ 0) IN (mf_doc_disp_cd, mf_grp_disp_cd, mf_num_disp_cd, mf_txt_disp_cd))
    AND ((ce.contributor_system_cd+ 0)=mf_sunquest_cd)
    AND ((ce.valid_until_dt_tm+ 0)=cnvtdatetime("31-DEC-2100 00:00:00"))
    AND ce.entry_mode_cd=mf_esi_entry_cd)
   JOIN (cep
   WHERE cep.event_id=ce.event_id
    AND ((cep.action_status_cd+ 0)=completed_cd)
    AND ((cep.action_type_cd+ 0)=mf_performed_cd)
  ...
  HEAD ce.encntr_id
   ...
   IF (idx > 0)
    t_record->encntr_appt_qual[idx].doc_ind = 1, t_record->encntr_appt_qual[idx].encntr_id = 0,
    t_record->encntr_appt_qual[indx].n_dictated_ind = 1
   ELSE
    IF (tmp_ind=0)
     CALL echo("found row on db not in list"),
     CALL echo(concat("encntr_id: ",cnvtstring(ce.encntr_id))),
     CALL echo(concat("performed_prsnl_id: ",cnvtstring(ce.performed_prsnl_id))),
     tmp_ind = 1
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 CALL echo("check for dictated results")
 FOR (x = 1 TO size(t_record->encntr_appt_qual,5))
   IF ((t_record->encntr_appt_qual[x].doc_ind=1))
    CALL echo("found one")
    CALL echo(concat("encntr_id: ",cnvtstring(t_record->encntr_appt_qual[x].encntr_id)))
   ENDIF
 ENDFOR
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   IF ((t_record->encntr_appt_qual[d.seq].n_dictated_ind=1))
    t_record->phys_qual[idx].l_dictated_cnt = (t_record->phys_qual[idx].l_dictated_cnt+ 1)
   ENDIF
  WITH nocounter
 ;end select
 SET etime = format(cnvtdatetime(curdate,curtime3),";;q")
 ...
 SELECT INTO "pco_metrics.xls"
  ...
  HEAD REPORT
   ...
   t_line, row + 1, t_line = concat("Physcian Name",char(9),"Position",char(9),"Location",
    ...
    "Signed Clinical Notes",char(9),"Dictated Notes",char(9),
    "Forwarded Documents Modified and Signed",
    char(9),"Total Documents Signed",char(9),"Forwarded Documents Signed",char(9),
    "Forwarded Documents Reviewed",char(9),"Forwarded Documents Not Signed or Reviewed",char(9),
    "Forwarded Orders Not Signed or Reviewed",
    char(9),"Unread Messages",char(9),"Results Endorsed",char(9),
    "Manual Satisfiers into HM",char(9),"Orders",char(9),"Charges",
    char(9),"Number of Patients Seen",char(9),"Number of Appts with Docs",char(9),
    "% Appointments with Signed Documents",char(9)),
   col 0, t_line, row + 1
  ...
  HEAD phys_id
   t_line = concat(trim(pr.name_full_formatted),char(9),trim(t_record->phys_qual[d.seq].position),
    ...
    trim(cnvtstring(t_record->phys_qual[d.seq].signed_cn_cnt)),char(9),trim(cnvtstring(t_record->
      phys_qual[d.seq].l_dictated_cnt)),char(9),trim(cnvtstring(t_record->phys_qual[d.seq].
      fwd_doc_cnt)),
    char(9),trim(cnvtstring(total_docs)),char(9),trim(cnvtstring(t_record->phys_qual[d.seq].
      fwd_doc_sign_cnt)),char(9),
    trim(cnvtstring(t_record->phys_qual[d.seq].fwd_doc_rev_cnt)),char(9),trim(cnvtstring(t_record->
      phys_qual[d.seq].fwd_doc_not_sign_cnt)),char(9),trim(cnvtstring(t_record->phys_qual[d.seq].
      fwd_ord_not_sign_cnt)),
    char(9),trim(cnvtstring(t_record->phys_qual[d.seq].unread_msgs)),char(9),trim(cnvtstring(t_record
      ->phys_qual[d.seq].endorse_cnt)),char(9),
    trim(cnvtstring(t_record->phys_qual[d.seq].man_sat_hm_cnt)),char(9),trim(cnvtstring(t_record->
      phys_qual[d.seq].orders_cnt)),char(9),trim(cnvtstring(t_record->phys_qual[d.seq].charges_cnt)),
    char(9),trim(cnvtstring(t_record->phys_qual[d.seq].sch_appt_cnt)),char(9),trim(cnvtstring(
      t_record->phys_qual[d.seq].encntr_appt_doc_cnt)),char(9),
    trim(cnvtstring(percent_signed_docs)),char(9)), col 0, t_line,
   row + 1
 ...
 IF (findfile("pco_metrics.xls")=1
  ...
  SET dclcom = concat('echo " " | mailx -s "',subject_line,'" ','-a "pco_metrics.xls.gz" ',
   '-a "pn_type_counts.xls.gz" ',
   email_list)
  SET len = size(trim(dclcom))
  SET status = 0
  SET stat = dcl(dclcom,len,status)
