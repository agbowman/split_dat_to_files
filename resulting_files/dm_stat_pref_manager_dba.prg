CREATE PROGRAM dm_stat_pref_manager:dba
 RECORD frec(
   1 file_desc = i4
   1 file_offset = i4
   1 file_dir = i4
 ...
 ENDIF
 DECLARE ms_snapshot_type = vc WITH protect, constant("CHUNK_PREF_MAN.2")
 DECLARE ms_snapshot_num = i4 WITH protect, constant(114)
 ...
 DECLARE chunk_size = i4 WITH noconstant(100000)
 DECLARE ms_info_domain = vc WITH constant("DM_STAT_PREF_MAN.2")
 ...
 IF (curqual=0)
  ...
  ;end insert
  DELETE  FROM dm_info
   WHERE info_domain="DM_STAT_PREF_MAN"
  ;end delete
  COMMIT
 ...
 SELECT INTO "nl:"
  dvsm_min = min(pe.entry_id), dvsm_max = max(pe.entry_id)
  FROM prefdir_entrydata pe
  WHERE pe.updt_dt_tm > cnvtdatetime(ms_last_run_time)
   AND pe.updt_dt_tm <= cnvtdatetime(ms_this_run_time)
   AND ((entry_data="*prefcontext:default*") OR (((entry_data="*prefcontext:facility*") OR (
  entry_data="*prefcontext:position*")) ))
   AND entry_data="*prefvalue*"
   AND entry_data="*prefentry*"
  DETAIL
 ...
 FOR (itr = 1 TO num_chunks)
   SELECT INTO "nl:"
    dist_name = trim(pe.dist_name), entry_data = trim(pe.entry_data)
    FROM prefdir_entrydata pe
    WHERE pe.updt_dt_tm > cnvtdatetime(ms_last_run_time)
     AND pe.updt_dt_tm <= cnvtdatetime(ms_this_run_time)
     AND pe.entry_id >= curmin
     AND pe.entry_id < curmax
     AND ((entry_data="*prefcontext:default*") OR (((entry_data="*prefcontext:facility*") OR (
    entry_data="*prefcontext:position*")) ))
     AND entry_data="*prefvalue*"
     AND entry_data="*prefentry*"
    ...
    DETAIL
     ...
     ENDIF
     dsr->qual[qualcnt].qual[ds_cnt].stat_name = "CHUNK_PREF_MAN", dsr->qual[qualcnt].qual[ds_cnt].
     stat_clob_val = substring(1,4000,build(dist_name,"||",entry_data)), dsr->qual[qualcnt].qual[
     ds_cnt].stat_type = 0,
