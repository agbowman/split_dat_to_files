CREATE PROGRAM dm2_cbo_all_sql_rpt
 ...
 SUBROUTINE dcr_get_instance_info(dgii_instance_name,dgii_instance_nbr)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_cbosqlrpt",".log","Dm2_cbo_all_sql_rpt")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating input parameters."
 FREE RECORD dcasr_line
 RECORD dcasr_line(
   1 cnt = i4
   1 qual[*]
     2 str = vc
 )
 DECLARE dcasr_sqlid = vc WITH protect, noconstant( $1)
 DECLARE dcasr_fname = vc WITH protect, noconstant( $2)
 DECLARE dcasr_user_name = vc WITH protect, noconstant(trim(cnvtupper(currdbuser)))
 DECLARE dcasr_database_name = vc WITH protect, noconstant(trim(cnvtupper(currdbname)))
 DECLARE dcasr_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcasr_cnt2 = i4 WITH protect, noconstant(0)
 DECLARE dcasr_cnt3 = i4 WITH protect, noconstant(0)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->user_action = "Please verify correct number and format of input parameters."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_cbo_all_sql_rpt"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dcasr_fname="MINE")
  SET message = window
  CALL clear(1,1)
  CALL text(4,2,"Please provide a SQL_ID:")
  CALL accept(4,30,"P(30);c",dcasr_sqlid)
  SET dcasr_sqlid = curaccept
  CALL clear(1,1)
  SET message = nowindow
 ENDIF
 EXECUTE dm2_cbo_sql_with_binds_rpt dcasr_sqlid, "TEMP"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 CALL dcasr_add_line("SQL STATEMENT WITH BINDS")
 CALL dcasr_add_line(" ")
 IF ((dcr_sql->txt_cnt=0))
  CALL dcasr_add_line("Unable to retrieve statement with binds.")
  CALL dcasr_add_line(" ")
 ENDIF
 FOR (dcasr_cnt = 1 TO dcr_sql->txt_cnt)
   CALL dcasr_add_line(trim(substring(1,490,dcr_sql->txt[dcasr_cnt].sql_txt_with_binds)))
 ENDFOR
 CALL dcasr_add_line(" ")
 EXECUTE dm2_cbo_plan_estimate_rpt dcasr_sqlid, "TEMP"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 CALL dcasr_add_line("SQL PLAN ESTIMATE")
 CALL dcasr_add_line(" ")
 IF ((dcr_sqlplan->cnt=0))
  CALL dcasr_add_line("Unable to retrieve data for plan estimate.")
  CALL dcasr_add_line(" ")
 ENDIF
 FOR (dcasr_cnt = 1 TO dcr_sqlplan->cnt)
   CALL dcasr_add_line(dcr_sqlplan->qual[dcasr_cnt].plan_line)
 ENDFOR
 CALL dcasr_add_line(" ")
 EXECUTE dm2_cbo_plan_actual_rpt dcasr_sqlid, "TEMP"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 CALL dcasr_add_line("SQL PLAN ACTUAL")
 CALL dcasr_add_line(" ")
 IF ((dcr_sqlplan->cnt=0))
  CALL dcasr_add_line("Unable to retrieve data for actual plan.")
  CALL dcasr_add_line(" ")
 ENDIF
 FOR (dcasr_cnt = 1 TO dcr_sqlplan->cnt)
   CALL dcasr_add_line(dcr_sqlplan->qual[dcasr_cnt].plan_line)
 ENDFOR
 CALL dcasr_add_line(" ")
 SET dm_err->eproc = "Obtain plan objects"
 SELECT DISTINCT INTO "nl:"
  v.object_name
  FROM gv$sql_plan v
  WHERE v.object_type="TABLE"
   AND v.sql_id=dcasr_sqlid
   AND (v.child_number=
  (SELECT
   max(child_number)
   FROM gv$sql_plan
   WHERE sql_id=dcasr_sqlid))
  DETAIL
   dcrdata->req_cnt = (dcrdata->req_cnt+ 1), stat = alterlist(dcrdata->req_list,dcrdata->req_cnt),
   dcrdata->req_list[dcrdata->req_cnt].tbl_name = v.object_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 EXECUTE dm2_dbstats_table_rpt "NOPROMPT", "DM2REQLIST", "TEMP",
 "LIVE"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Generate SQL report"
 SELECT INTO value(dcasr_fname)
  FROM (dummyt d  WITH seq = 1)
  DETAIL
   CALL print("SQL ALL REPORT"),
   CALL center(concat("Report Date/Time: ",format(cnvtdatetime(curdate,curtime3),
     "DD-MMM-YY HH:MM:SS;;D")),1,132), row + 1,
   row + 1,
   CALL print(concat("User Name        : ",dcasr_user_name)), row + 1,
   CALL print(concat("Database Name    : ",dcasr_database_name)), row + 1,
   CALL print(concat("Sql_Id           : ",dcasr_sqlid)),
   row + 1, col 0,
   CALL print(fillstring(132,"*")),
   row + 1, row + 1
   FOR (dcasr_cnt = 1 TO dcasr_line->cnt)
    CALL print(dcasr_line->qual[dcasr_cnt].str),row + 1
   ENDFOR
   col 0, "TABLE STATISTIC REPORT", row + 1,
   col 0, "Table Criteria:"
   FOR (dcasr_cnt = 1 TO dcrdata->req_cnt)
     col 20, dcrdata->req_list[dcasr_cnt].tbl_name, row + 1
   ENDFOR
   IF ((dcrdata->table_cnt=0))
    row + 1, row + 1, col 0,
    "^^^^^^^^^^^^^^^^ Table does not exist ^^^^^^^^^^^^^^^^"
   ELSE
    FOR (dcasr_cnt = 1 TO dcrdata->table_cnt)
      row + 1, row + 1, col 0,
      "Table Name:", col 12, dcrdata->tables[dcasr_cnt].table_name,
      row + 1, row + 1, col 0,
      "|------------------Table Statistics-----------------|", col 66,
      "|----------------Table Modifications----------------|",
      row + 1, col 22, "   Avg",
      row + 1, col 22, "   Row",
      col 29, "   Sample", col 54,
      "Globl", col 60, " User",
      row + 1, col 0, "  Num_Rows",
      col 11, "    Blocks", col 22,
      "Length", col 29, "     Size",
      col 39, " Last_Analyzed", col 54,
      "Stats", col 60, "Stats",
      col 66, "   Inserts", col 77,
      "   Updates", col 88, "   Deletes",
      col 99, "     Timestamp", col 114,
      "Trunc", row + 1
      IF ((dcrdata->tables[dcasr_cnt].last_analyzed_null_ind=0))
       IF ((dcrdata->tables[dcasr_cnt].num_rows < 9999999999.0))
        col 0, dcrdata->tables[dcasr_cnt].num_rows"##########"
       ELSE
        col 0, "**********"
       ENDIF
       IF ((dcrdata->tables[dcasr_cnt].blocks < 9999999999.0))
        col 11, dcrdata->tables[dcasr_cnt].blocks"##########"
       ELSE
        col 11, "**********"
       ENDIF
       col 22, dcrdata->tables[dcasr_cnt].avg_row_length"######"
       IF ((dcrdata->tables[dcasr_cnt].sample_size < 999999999.0))
        col 29, dcrdata->tables[dcasr_cnt].sample_size"#########"
       ELSE
        col 29, "*********"
       ENDIF
       col 39, dcrdata->tables[dcasr_cnt].last_analyzed"MM/DD/YY HH:MM;;Q"
      ELSE
       col 0, "^^^^^^^^^^^^^^^^ No Table Statistics ^^^^^^^^^^^^^^^^"
      ENDIF
      col 54, dcrdata->tables[dcasr_cnt].global_stats"#####;R", col 60,
      dcrdata->tables[dcasr_cnt].user_stats"#####;R"
      IF ((dcrdata->tables[dcasr_cnt].table_mods_null_ind=0))
       IF ((dcrdata->tables[dcasr_cnt].inserts < 9999999999.0))
        col 66, dcrdata->tables[dcasr_cnt].inserts"##########"
       ELSE
        col 66, "**********"
       ENDIF
       IF ((dcrdata->tables[dcasr_cnt].updates < 9999999999.0))
        col 77, dcrdata->tables[dcasr_cnt].updates"##########"
       ELSE
        col 77, "**********"
       ENDIF
       IF ((dcrdata->tables[dcasr_cnt].deletes < 9999999999.0))
        col 88, dcrdata->tables[dcasr_cnt].deletes"##########"
       ELSE
        col 88, "**********"
       ENDIF
       col 99, dcrdata->tables[dcasr_cnt].timestamp"MM/DD/YY HH:MM;;Q", col 114,
       dcrdata->tables[dcasr_cnt].truncated"#####;R"
      ELSE
      ...
      row + 1
      FOR (dcasr_cnt2 = 1 TO dcrdata->tables[dcasr_cnt].index_cnt)
        IF (mod(dcasr_cnt2,7)=1)
         row + 1, col 66, " Avg",
         col 71, " Avg", row + 1,
         col 66, "Leaf", col 71,
         "Leaf", row + 1, col 42,
         ...
         col 57, "    Leaf", col 66,
         "Blk/", col 71, "Blk/",
         col 76, "  Cluster", col 86,
         "    Sample", col 112, "Globl",
         col 118, " User", row + 1,
         ...
         col 71, " Key", col 76,
         "   Factor", col 86, "      Size",
         col 97, " Last_Analyzed", col 112,
         "Stats", col 118, "Stats",
         row + 1
        ENDIF
        row + 1, col 0, dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].index_name
        "##############################"
        IF ((dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].last_analyzed_null_ind=0))
         IF ((dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].num_rows < 9999999999.0))
          col 31, dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].num_rows"##########"
         ELSE
          col 31, "**********"
         ENDIF
         IF ((dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].distinct_keys < 9999999999.0))
          col 42, dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].distinct_keys"##########"
         ELSE
          col 42, "**********"
         ENDIF
         col 53, dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].b_lvl"###", col 57,
         dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].leaf_blocks"########", col 66, dcrdata->
         tables[dcasr_cnt].indexes[dcasr_cnt2].avg_leaf_blocks_per_key"####",
         col 71, dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].avg_data_blocks_per_key"####"
         IF ((dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].clustering_factor < 999999999.0))
          col 76, dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].clustering_factor"#########"
         ELSE
          col 76, "*********"
         ENDIF
         col 86, dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].sample_size"##########", col 97,
         dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].last_analyzed"MM/DD/YY HH:MM;;Q"
        ELSE
         col 31, "^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ No Index Statistics ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^"
        ENDIF
        col 112, dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].global_stats"#####;R", col 118,
        dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].user_stats"#####;R"
        FOR (dcasr_cnt3 = 1 TO dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].index_column_cnt)
          IF (dcasr_cnt3=1)
           dcrdata->tables[dcasr_cnt].index_column_concat = "("
          ENDIF
          dcrdata->tables[dcasr_cnt].index_column_concat = notrim(concat(dcrdata->tables[dcasr_cnt].
            index_column_concat,dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].index_columns[
            dcasr_cnt3].column_name," | "))
          IF ((dcasr_cnt3=dcrdata->tables[dcasr_cnt].indexes[dcasr_cnt2].index_column_cnt))
           dcrdata->tables[dcasr_cnt].index_column_concat = concat(substring(1,(size(dcrdata->tables[
              dcasr_cnt].index_column_concat) - 3),dcrdata->tables[dcasr_cnt].index_column_concat),
            ")")
          ENDIF
        ENDFOR
        IF (size(dcrdata->tables[dcasr_cnt].index_column_concat) >= 123)
         dcrdata->tables[dcasr_cnt].index_column_concat = concat(substring(1,120,dcrdata->tables[
           dcasr_cnt].index_column_concat),"...)")
        ENDIF
        row + 1, col 2, dcrdata->tables[dcasr_cnt].index_column_concat
      ENDFOR
      row + 1, row + 1,
      CALL print(fillstring(126,"-")),
      row + 1, row + 1, col 0,
      "* = CCL using literals for this column"
      FOR (dcasr_cnt2 = 1 TO dcrdata->tables[dcasr_cnt].column_cnt)
        IF (mod(dcasr_cnt2,16)=1)
         row + 1, col 82, "   Avg",
         row + 1, col 32, "       Num",
         col 54, "       Num", col 65,
         "  Num", col 71, "    Sample",
         col 82, "Column", col 89,
         "Globl", col 95, " User",
         ...
         "      Size", col 82, "Length",
         col 89, "Stats", col 95,
         "Stats", col 101, " Last Analyzed",
         row + 1
        ENDIF
        IF ((dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].lit_column_ind=1))
         row + 1, col 0, "*",
         dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].column_name"##############################"
        ELSE
         row + 1, col 0, " ",
         dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].column_name"##############################"
        ENDIF
        IF ((dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].stat_null_ind=0))
         IF ((dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].num_distinct < 9999999999.0))
          col 32, dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].num_distinct"##########"
         ELSE
          col 32, "**********"
         ENDIF
         col 44, dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].density"#.#######;;F"
         IF ((dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].num_nulls < 9999999999.0))
          col 54, dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].num_nulls"##########"
         ELSE
          col 54, "**********"
         ENDIF
         col 65, dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].num_buckets"#####"
         IF ((dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].sample_size < 9999999999.0))
          col 71, dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].sample_size"##########"
         ELSE
          col 71, "**********"
         ENDIF
         col 82, dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].avg_column_length"######", col 89,
         dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].global_stats"#####;R", col 95, dcrdata->
         tables[dcasr_cnt].columns[dcasr_cnt2].user_stats"#####;R",
         col 101, dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].last_analyzed"MM/DD/YY HH:MM;;Q"
         IF ((dcrdata->stat_type="LIVE")
          AND (dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].histogram_type != "NONE"))
          row + 1, row + 1, col 1,
          CALL print(concat(dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].histogram_type,
           " histogram for ",dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].column_name)), row + 1
          IF ((dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].histogram_type="HEIGHT BALANCED"))
           col 31,
           "^^^^^^^^^^^^^^^^^ Histogram information not displayed for this type of histogram ^^^^^^^^^^^^^^^^",
           row + 1
          ELSE
           col 1, "Column Value", col 31,
           "Frequency", row + 1
           FOR (dcasr_cnt3 = 1 TO dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].histogram_cnt)
             col 1, dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].histogram[dcasr_cnt3].col_val, col
              31,
             CALL print(trim(cnvtstring(dcrdata->tables[dcasr_cnt].columns[dcasr_cnt2].histogram[
               dcasr_cnt3].rows_per_key))), row + 1
           ENDFOR
          ENDIF
          IF (mod((dcasr_cnt2+ 1),16) != 1)
           row + 1, col 82, "   Avg",
           row + 1, col 32, "       Num",
           col 54, "       Num", col 65,
           "  Num", col 71, "    Sample",
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE dcasr_add_line(dal_line)
   SET dcasr_line->cnt = (dcasr_line->cnt+ 1)
   SET stat = alterlist(dcasr_line->qual,dcasr_line->cnt)
   SET dcasr_line->qual[dcasr_line->cnt].str = dal_line
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Dm2_cbo_all_sql_rpt completed"
 CALL final_disp_msg("dm2_cbosqlrpt")
END GO
