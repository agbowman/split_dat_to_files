CREATE PROGRAM dm2_aload_sch_part_subpart_obj
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_sch_part_subpart_obj",".log","dm2_aload_sch_part_subpart_obj")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_sch_part_subpart_obj"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daspso_cnt = i4 WITH protect, noconstant(0)
 DECLARE daspso_str = vc WITH protect, noconstant("")
 DECLARE daspso_req_size = i4 WITH protect, noconstant(0)
 DECLARE daspso_ndx = i4 WITH protect, noconstant(0)
 FREE RECORD daspso_maint
 RECORD daspso_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET daspso_maint->num_of_inserts = 0
 SET daspso_maint->num_of_updates = 0
 SET daspso_maint->num_of_none = 0
 SET daspso_req_size = size(requestin->list_0,5)
 SET stat = alterlist(daspso_maint->qual,daspso_req_size)
 IF (dac_pop_coldic_rec("DM_AFD_PART_SUBPART_OBJECTS")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining maintenance type for DM_AFD_PART_SUBPART_OBJECTS."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(daspso_req_size))
  DETAIL
   daspso_maint->qual[d.seq].maint_type = "I", daspso_maint->num_of_inserts = (daspso_maint->
   num_of_inserts+ 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 SELECT INTO "nl:"
  FROM dm_afd_tables dat,
   ...
   (dummyt d  WITH seq = value(daspso_req_size))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (dat
   ...
   JOIN (dapso
   WHERE dapso.owner=dat.owner
    AND dapso.alpha_feature_nbr=cnvtint(requestin->list_0[d.seq].alpha_feature_nbr)
    AND (dapso.table_name=requestin->list_0[d.seq].table_name)
    AND (dapso.object_name=requestin->list_0[d.seq].object_name)
    AND dapso.table_name=dat.table_name)
  ORDER BY dapso.table_name, dapso.object_name
  DETAIL
   IF (cnvtint(requestin->list_0[d.seq].schema_instance) < dat.schema_instance)
    daspso_maint->qual[d.seq].maint_type = "N", daspso_maint->num_of_none = (daspso_maint->
    num_of_none+ 1), daspso_maint->num_of_inserts = (daspso_maint->num_of_inserts - 1)
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm)=cnvtdatetime(dapso.updt_dt_tm))
    daspso_maint->qual[d.seq].maint_type = "N", daspso_maint->num_of_none = (daspso_maint->
    num_of_none+ 1), daspso_maint->num_of_inserts = (daspso_maint->num_of_inserts - 1)
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm) != cnvtdatetime(dapso.updt_dt_tm))
    daspso_maint->qual[d.seq].maint_type = "U", daspso_maint->num_of_updates = (daspso_maint->
    num_of_updates+ 1), daspso_maint->num_of_inserts = (daspso_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF ((daspso_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(daspso_req_size) )"),0)
  ...
  COMMIT
 ENDIF
 IF ((daspso_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where daspso_maint->qual[d.seq].maint_type = 'U' ",0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_sch_part_subpart_obj completed."
 ENDIF
 CALL final_disp_msg("dm2_aload_sch_part_subpart_obj")
END GO
