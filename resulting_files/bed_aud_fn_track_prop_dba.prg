CREATE PROGRAM bed_aud_fn_track_prop:dba
 ...
 SET reply->collist[1].header_text = "Tracking Property Display"
 SET reply->collist[1].data_type = 1
 SET reply->collist[1].hide_ind = 0
 SET reply->collist[2].header_text = "Tracking Property Description"
 SET reply->collist[2].data_type = 1
 SET reply->collist[2].hide_ind = 0
 SET reply->collist[3].header_text = "Tracking Property Definition"
 ...
 SET reply->collist[7].header_text = "code_value"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   IF (p.person_id > 0)
    reply->rowlist[row_nbr].celllist[6].string_value = p.name_full_formatted
   ENDIF
   reply->rowlist[row_nbr].celllist[7].double_value = cv.code_value
  WITH nocounter
 ;end select
 SET reply->status_data.status = "S"
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("firstnet_track_prop_audit.csv")
 ENDIF
