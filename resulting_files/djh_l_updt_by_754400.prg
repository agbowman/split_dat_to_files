CREATE PROGRAM djh_l_updt_by_754400
 PROMPT
 ...
 SELECT INTO  $OUTDEV
  ...
  "@SHORTDATETIME",
  newdt = format(p.updt_dt_tm,"yyyy-mm-dd"), p.updt_id"#########", p.updt_task,
  p.create_prsnl_id"#########", p1.name_full_formatted, newdays = format( $DYBK,"##"),
  ...
  HEAD PAGE
   ...
   CALL print(calcpos(222,(y_pos+ 0))),
   "User IDs Added or Changed - Except Nursing Students - Pos Cd 457's", row + 1,
   ...
   "Date", row + 1,
   CALL print(calcpos(486,(y_pos+ 35))),
   "|  Date / Time  |",
   CALL print(calcpos(510,(y_pos+ 27))), "Change",
   CALL print(calcpos(580,(y_pos+ 34))), "Created / Changed by", row + 1,
   y_pos = (y_pos+ 60)
  DETAIL
   IF (((y_pos+ 97) >= 612))
    y_pos = 0, BREAK
   ...
   statcd = evaluate(p.active_status_cd,188.00,"",192.00,"C",
    194.00,"S","x"), row + 1,
   ...
   CALL print(calcpos(578,(y_pos+ 0))), name_full_formatted2, y_pos = (y_pos+ 13)
  FOOT PAGE
  ...
  FOOT REPORT
   ...
   "Recs Added/Changed:", xcnt = format(count(p.name_full_formatted),"###"), row + 1,
   CALL print(calcpos(121,(y_pos+ 0))), xcnt, row + 1,
