CREATE PROGRAM bed_ens_concept_mill_matches:dba
 ...
 ENDIF
 IF ( NOT (validate(identstosave,0)))
  FREE RECORD identstosave
  RECORD identstosave(
    1 ident[*]
      2 code_set = i4
      2 display = vc
      2 parseidentifierstring = vc
      2 identifier = vc
      2 definition = vc
 ...
 DECLARE selectedmatchescnt = i4 WITH protect, noconstant(1)
 DECLARE identcnt = i4 WITH protect, noconstant(1)
 DECLARE ignoreit = i2 WITH protect, noconstant(0)
 DECLARE updateidentstosave(request,identstosave) = null
 DECLARE updatecodevalueidentifiersforrelatedcodesets(identtosave=vc(ref)) = null
 IF (size(request->selectedmatches,5) > 0)
  CALL updateidentstosave(request)
 ENDIF
 FOR (identcnt = 1 TO size(identstosave->ident,5))
   SET ignoreit = false
   SET identtosave->code_set = identstosave->ident[identcnt].code_set
   SET identtosave->display = identstosave->ident[identcnt].display
   SET identtosave->parseidentifierstring = identstosave->ident[identcnt].parseidentifierstring
   SET identtosave->identifier = identstosave->ident[identcnt].identifier
   SET identtosave->definition = identstosave->ident[identcnt].definition
   SET identtosave->activity_type_cd = identstosave->ident[identcnt].activity_type_cd
   IF ((identtosave->code_set=93))
    SET identtosave->code_value = geteventsetcodefromeventsetname(identstosave->ident[identcnt].
     display)
    IF ((identtosave->code_value=0))
     SET ignoreit = true
    ENDIF
   ELSEIF ((identtosave->code_set=14003))
    SET identtosave->code_value = gettaskassaycdfrommnemonicandacvitytype(identstosave->ident[
     identcnt].display,identstosave->ident[identcnt].activity_type_cd)
    IF ((identtosave->code_value=0))
     SET ignoreit = true
    ENDIF
   ELSEIF ((identtosave->code_set=72))
    IF ((identstosave->ident[identcnt].code_value > 0))
     SET identtosave->code_value = identstosave->ident[identcnt].code_value
    ELSE
     SET identtosave->code_value = getcodevaluefromdisplayandcodeset(identtosave->display,identtosave
      ->code_set,identtosave->definition)
    ENDIF
    IF ((identtosave->code_value=0))
     SET ignoreit = true
    ENDIF
   ELSE
    SET identtosave->code_value = 0.0
   ENDIF
   IF (validate(debug,0)=1)
    CALL echorecord(identtosave)
   ENDIF
   IF (ignoreit=false)
    IF (table_type="NOMENCLATURE")
     CALL updatenomenclatureidentifiers(identtosave)
    ELSEIF (((table_type="CODE") OR (table_type="ORDERABLE")) )
     IF ((identtosave->code_set > 0))
      CALL updatecodevalueidentifiers(identtosave)
      IF (isrelatedcodeset(identtosave->code_set)
       AND identifier_type="CONCEPTCKI")
       CALL updatecodevalueidentifiersforrelatedcodesets(identtosave)
      ENDIF
     ELSE
      CALL updateordercatalogidentifiers(identtosave)
     ENDIF
    ENDIF
   ENDIF
 ENDFOR
#exit_script
 ...
 SUBROUTINE updateidentstosave(dummyvar)
   CALL bedlogmessage("updateIdentsToSave","Entering ...")
   DECLARE codeset = vc WITH protect, noconstant("")
   DECLARE notfnd = vc WITH protect, constant("<not_found>")
   DECLARE num = i4 WITH protect, noconstant(1)
   DECLARE cnt = i4 WITH protect, noconstant(0)
   FOR (selectedmatchescnt = 1 TO size(request->selectedmatches,5))
     SET num = 1
     SET codeset = ""
     IF ((request->selectedmatches[selectedmatchescnt].parsecodesetstring=""))
      SET request->selectedmatches[selectedmatchescnt].parsecodesetstring = "0"
     ENDIF
     WHILE (codeset != notfnd)
       SET codeset = piece(request->selectedmatches[selectedmatchescnt].parsecodesetstring,",",num,
        notfnd)
       IF (codeset != notfnd)
        SET cnt = (size(identstosave->ident,5)+ 1)
        SET stat = alterlist(identstosave->ident,cnt)
        IF (cnvtupper(trim(codeset))="ORDER CATALOG")
         SET identstosave->ident[cnt].code_set = - (1)
        ELSE
         SET identstosave->ident[cnt].code_set = cnvtreal(trim(codeset))
        ENDIF
        SET identstosave->ident[cnt].display = request->selectedmatches[selectedmatchescnt].display
        IF (identifier_type="CKI")
         SET identstosave->ident[cnt].parseidentifierstring = "cki"
        ELSEIF (identifier_type="CDFMEANING")
         SET identstosave->ident[cnt].parseidentifierstring = "cdf_meaning"
        ELSEIF (identifier_type="CONCEPTCKI")
         SET identstosave->ident[cnt].parseidentifierstring = "concept_cki"
        ENDIF
        SET identstosave->ident[cnt].identifier = request->selectedmatches[selectedmatchescnt].
        identifier
        SET identstosave->ident[cnt].definition = request->selectedmatches[selectedmatchescnt].
        definition
        SET identstosave->ident[cnt].activity_type_cd = request->selectedmatches[selectedmatchescnt].
        activity_type_cd
        SET identstosave->ident[cnt].code_value = request->selectedmatches[selectedmatchescnt].
        code_value
       ENDIF
       SET num = (num+ 1)
     ENDWHILE
   ENDFOR
   IF (validate(debug,0)=1)
    CALL echorecord(identstosave)
   ENDIF
   CALL bedlogmessage("updateIdentsToSave","Exiting ...")
 END ;Subroutine
 SUBROUTINE updatecodevalueidentifiersforrelatedcodesets(identtosave)
   CALL bedlogmessage("updateCodeValueIdentifiersForRelatedCodeSets","Entering ...")
   DECLARE parseidentifierstring = vc WITH protect, constant(identtosave->parseidentifierstring)
   DECLARE identifier = vc WITH protect, constant(identtosave->identifier)
   DECLARE display = vc WITH protect, constant(identtosave->display)
   DECLARE codevalue = f8 WITH protect, noconstant(0.0)
   DECLARE fidx = i4 WITH protect, noconstant(0)
   DECLARE fidx2 = i4 WITH protect, noconstant(0)
   ...
   IF ((identtosave->code_set=72))
    SET codevalue = identtosave->code_value
    SET rcnt = (size(relatedcodesetsrequest->event_codes,5)+ 1)
    SET stat = alterlist(relatedcodesetsrequest->event_codes,rcnt)
    SET relatedcodesetsrequest->event_codes[rcnt].code_value = codevalue
    SET relatedcodesetsrequest->event_codes[rcnt].concept_cki = identifier
    SET relatedcodesetsrequest->event_codes[rcnt].display = display
    CALL geteventsetfromeventcode(relatedcodesetsrequest,eventsetreply)
    FOR (fidx = 1 TO size(eventsetreply->event_set_reltns,5))
      SET identtosave->code_set = 93
      SET identtosave->display = eventsetreply->event_set_reltns[fidx].display
      SET identtosave->parseidentifierstring = parseidentifierstring
      SET identtosave->identifier = identifier
      SET identtosave->code_value = eventsetreply->event_set_reltns[fidx].event_set_cd
      CALL updatecodevalueidentifiers(identtosave)
    ENDFOR
    CALL getdtafromeventcode(relatedcodesetsrequest,dtacodesreply)
    FOR (fidx = 1 TO size(dtacodesreply->dta_code_reltns,5))
      SET identtosave->code_set = 14003
      SET identtosave->display = dtacodesreply->dta_code_reltns[fidx].display
      SET identtosave->parseidentifierstring = parseidentifierstring
      SET identtosave->identifier = identifier
      SET identtosave->code_value = dtacodesreply->dta_code_reltns[fidx].task_assay_cd
      CALL updatecodevalueidentifiers(identtosave)
    ENDFOR
   ELSEIF ((identtosave->code_set=93))
    SET rcnt = (size(relatedcodesetsrequest->event_sets,5)+ 1)
    SET stat = alterlist(relatedcodesetsrequest->event_sets,rcnt)
    SET relatedcodesetsrequest->event_sets[rcnt].event_set_name = display
    SET relatedcodesetsrequest->event_sets[rcnt].concept_cki = identifier
    CALL geteventcodefromeventset(relatedcodesetsrequest,eventcodesreply)
    FOR (fidx = 1 TO size(eventcodesreply->eventcodereplystructure,5))
      FOR (fidx2 = 1 TO size(eventcodesreply->eventcodereplystructure[fidx].event_codes,5))
        SET identtosave->code_set = 72
        SET identtosave->display = eventcodesreply->eventcodereplystructure[fidx].event_codes[fidx2].
        display
        SET identtosave->parseidentifierstring = parseidentifierstring
        SET identtosave->identifier = identifier
        SET identtosave->code_value = eventcodesreply->eventcodereplystructure[fidx].event_codes[
        fidx2].code_value
        CALL updatecodevalueidentifiers(identtosave)
      ENDFOR
    ENDFOR
    ...
    ENDFOR
   ELSEIF ((identtosave->code_set=14003))
    SET codevalue = gettaskassaycdfrommnemonicandacvitytype(identtosave->display,identtosave->
     activity_type_cd)
    SET rcnt = (size(relatedcodesetsrequest->dtas,5)+ 1)
    SET stat = alterlist(relatedcodesetsrequest->dtas,rcnt)
    SET relatedcodesetsrequest->dtas[rcnt].code_value = codevalue
    SET relatedcodesetsrequest->dtas[rcnt].concept_cki = identifier
    SET relatedcodesetsrequest->dtas[rcnt].display = display
    CALL geteventcodefromdta(relatedcodesetsrequest,eventcodesreplyfromdta)
    FOR (fidx = 1 TO size(eventcodesreplyfromdta->event_codes,5))
      SET identtosave->code_set = 72
      SET identtosave->display = eventcodesreplyfromdta->event_codes[fidx].display
      SET identtosave->parseidentifierstring = parseidentifierstring
      SET identtosave->identifier = identifier
      SET identtosave->code_value = eventcodesreplyfromdta->event_codes[fidx].event_cd
      CALL updatecodevalueidentifiers(identtosave)
    ENDFOR
    CALL geteventsetfromeventcode(relatedcodesetsrequest,eventsetreply)
    FOR (fidx = 1 TO size(eventsetreply->event_set_reltns,5))
    ...
    ENDFOR
   ENDIF
   CALL bedlogmessage("updateCodeValueIdentifiersForRelatedCodeSets","Exiting ...")
 END ;Subroutine
END GO
