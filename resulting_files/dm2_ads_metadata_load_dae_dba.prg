CREATE PROGRAM dm2_ads_metadata_load_dae:dba
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE damld_iter = i4 WITH protect, noconstant(0)
 DECLARE damld_cnt = i4 WITH protect, noconstant(0)
 DECLARE damld_iter2 = i4 WITH protect, noconstant(0)
 DECLARE damld_cur_dae_id = f8 WITH protect, noconstant(0.0)
 DECLARE damld_updt_flag = i2 WITH protect, noconstant(0)
 DECLARE damld_insert_flag = i2 WITH protect, noconstant(0)
 DECLARE damld_found = i2 WITH protect, noconstant(0)
 DECLARE damld_extract_cnt = i4 WITH protect, noconstant(0)
 DECLARE damld_invalid_ind = i2 WITH protect, noconstant(0)
 DECLARE damld_invalid_msg = vc WITH protect, noconstant("")
 DECLARE damld_debug_table_name = vc WITH protect, noconstant("")
 DECLARE damld_table_name = vc WITH protect, noconstant("")
 DECLARE damld_debug_table_extract_nbr = i4 WITH protect, noconstant(0)
 IF ( NOT (check_logfile("dm2_ads_load_dae",".log","dm2_ads_metadata_load_dae LOGFILE")))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_ads_metadata_load_dae"
 CALL disp_msg("",dm_err->logfile,0)
 IF (ds_scramble_init(null)=0)
  GO TO exit_program
 ENDIF
 IF (dar_get_config_list(1)=0)
  GO TO exit_program
 ENDIF
 IF (size(requestin->list_0,5) > 0)
  FOR (damld_iter = 1 TO size(requestin->list_0,5))
    SET dm_err->eproc = build("Processing object <",damld_iter,"> of <",size(requestin->list_0,5),
     ">.",
     requestin->list_0[damld_iter].table_name,";",requestin->list_0[damld_iter].table_extract_nbr)
    CALL disp_msg("",dm_err->logfile,0)
    IF ((dm_err->debug_flag=511))
     SET dm2scramble->in_text = requestin->list_0[damld_iter].table_name
     IF (ds_scramble(null)=0)
      GO TO exit_program
     ENDIF
     SET damld_debug_table_name = dm2scramble->out_text
     CALL echo(concat("Table NameSCR: ",requestin->list_0[damld_iter].table_name))
     CALL echo(concat("Table Name: ",damld_debug_table_name))
     CALL echo(concat("Table Extract Nbr: ",build(requestin->list_0[damld_iter].table_extract_nbr)))
    ENDIF
    SET requestin->list_0[damld_iter].table_name = replace(check(requestin->list_0[damld_iter].
      table_name," ")," ","",0)
    SET requestin->list_0[damld_iter].driver_table_name = replace(check(requestin->list_0[damld_iter]
      .driver_table_name," ")," ","",0)
    SET requestin->list_0[damld_iter].extract_method = replace(check(requestin->list_0[damld_iter].
      extract_method," ")," ","",0)
    SET requestin->list_0[damld_iter].data_class_type = replace(check(requestin->list_0[damld_iter].
      data_class_type," ")," ","",0)
    SET requestin->list_0[damld_iter].driver_keycol_name = replace(check(requestin->list_0[damld_iter
      ].driver_keycol_name," ")," ","",0)
    SET requestin->list_0[damld_iter].driver_rankcol_name = replace(check(requestin->list_0[
      damld_iter].driver_rankcol_name," ")," ","",0)
    SET requestin->list_0[damld_iter].expimp_parent_table_name = replace(check(requestin->list_0[
      damld_iter].expimp_parent_table_name," ")," ","",0)
    SET requestin->list_0[damld_iter].where_clause = check(requestin->list_0[damld_iter].where_clause,
     " ")
    SET requestin->list_0[damld_iter].table_comment = check(requestin->list_0[damld_iter].
     table_comment," ")
    SET damld_invalid_ind = 0
    IF ((dads_list->extract_qual[damld_iter].active_ind=1))
     SET dm_err->eproc = "Validating current csv row"
     IF ( NOT (cnvtupper(requestin->list_0[damld_iter].extract_method) IN (dpl_full, dpl_byconfig,
     dpl_static, dpl_intervalpct, dpl_nomove)))
      SET damld_invalid_ind = 1
      SET damld_invalid_msg = concat("Invalid Extract Method(",dads_list->extract_qual[damld_iter].
       extract_method,"). Valid values: ",dpl_full,", ",
       dpl_byconfig,", ",dpl_static,", ",dpl_intervalpct,
       ", ",dpl_nomove)
     ELSEIF ( NOT (cnvtupper(requestin->list_0[damld_iter].data_class_type) IN (dpl_act, dpl_ref,
     dpl_ref_mix, dpl_act_mix, dpl_mix)))
      SET damld_invalid_ind = 1
      SET damld_invalid_msg = concat("Invalid Data Class Type. Valid values: ",dpl_act,", ",dpl_ref,
       ", ",
       dpl_ref_mix,", ",dpl_act_mix,", ",dpl_mix)
     ELSEIF (cnvtupper(requestin->list_0[damld_iter].extract_method) IN (dpl_byconfig, dpl_static)
      AND cnvtint(requestin->list_0[damld_iter].apply_where_ind)=0)
      SET damld_invalid_ind = 1
      SET damld_invalid_msg = concat("Apply_where_ind MUST be set for extract methods: ",dpl_byconfig,
       ", ",dpl_static)
     ELSEIF (cnvtupper(requestin->list_0[damld_iter].extract_method) IN (dpl_full, dpl_intervalpct)
      AND cnvtint(requestin->list_0[damld_iter].apply_where_ind)=1)
      SET damld_invalid_ind = 1
      SET damld_invalid_msg = concat("Apply_where_ind CANNOT be set for extract methods: ",dpl_full,
       ", ",dpl_intervalpct)
     ELSEIF (((trim(requestin->list_0[damld_iter].extract_method)="") OR (trim(requestin->list_0[
      damld_iter].where_clause)=""))
      AND cnvtint(requestin->list_0[damld_iter].apply_where_ind)=1)
      SET damld_invalid_ind = 1
      SET damld_invalid_msg = "Extract must have a valid expimp parent table and where clause"
     ELSEIF (cnvtint(requestin->list_0[damld_iter].active_ind)=1
      AND cnvtint(requestin->list_0[damld_iter].expimp_level_nbr)=999)
      SET damld_invalid_ind = 1
      SET damld_invalid_msg =
      "Active Extracts must have a valid expimp level number (between 1 and 20)"
     ELSEIF (cnvtint(requestin->list_0[damld_iter].expimp_level_nbr) > 19)
      SET damld_invalid_ind = 1
      SET damld_invalid_msg =
      "Active Extracts must have a valid expimp level number (between 1 and 19)"
     ENDIF
    ENDIF
    IF (damld_invalid_ind=1)
     SET dm_err->err_ind = 1
     SET dm_err->eproc = concat("Current CSV Row is Invalid for table ",requestin->list_0[damld_iter]
      .table_name," - (",build(requestin->list_0[damld_iter].table_extract_nbr),")")
     SET dm_err->emsg = damld_invalid_msg
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    SET damld_updt_flag = 0
    SET damld_insert_flag = 0
    SET dm_err->eproc = "Check if row in DM_ADS_EXTRACT needs to be updated."
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
     FROM dm_ads_extract dae
     WHERE dae.owner_name=cnvtupper(requestin->list_0[damld_iter].owner_name)
      AND dae.table_name=cnvtupper(requestin->list_0[damld_iter].table_name)
      AND dae.table_extract_nbr=cnvtint(requestin->list_0[damld_iter].table_extract_nbr)
      AND dae.table_extract_instance_nbr=cnvtint(requestin->list_0[damld_iter].
      table_extract_instance_nbr)
     DETAIL
      damld_cur_dae_id = dae.dm_ads_extract_id
      IF (cnvtint(requestin->list_0[damld_iter].updt_cnt) > dae.updt_cnt)
       damld_updt_flag = 1
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc))
     ...
     GO TO exit_program
    ELSEIF (curqual=0)
     SET damld_insert_flag = 1
    ENDIF
    IF (damld_updt_flag=1)
     SET dm_err->eproc = "Extract exists: Updating row in DM_ADS_EXTRACT"
     CALL disp_msg("",dm_err->logfile,0)
     UPDATE  FROM dm_ads_extract dae
      SET dae.active_ind = cnvtint(requestin->list_0[damld_iter].active_ind), dae.driver_table_name
        = cnvtupper(requestin->list_0[damld_iter].driver_table_name), dae.data_class_type = cnvtupper
       (requestin->list_0[damld_iter].data_class_type),
       dae.extract_method = cnvtupper(requestin->list_0[damld_iter].extract_method), dae
       .driver_table_ind = cnvtint(requestin->list_0[damld_iter].driver_table_ind), dae
       .driver_keycol_name = cnvtupper(requestin->list_0[damld_iter].driver_keycol_name),
       dae.driver_rankcol_name = cnvtupper(requestin->list_0[damld_iter].driver_rankcol_name), dae
       .apply_where_ind = cnvtint(requestin->list_0[damld_iter].apply_where_ind), dae
       .expimp_parent_table_name = cnvtupper(requestin->list_0[damld_iter].expimp_parent_table_name),
       dae.expimp_level_nbr = cnvtint(requestin->list_0[damld_iter].expimp_level_nbr), dae
       .where_clause = trim(requestin->list_0[damld_iter].where_clause), dae.table_comment =
       requestin->list_0[damld_iter].table_comment,
       dae.dupdel_skip_ind = cnvtint(requestin->list_0[damld_iter].dupdel_skip_ind), dae.updt_cnt =
       cnvtint(requestin->list_0[damld_iter].updt_cnt)
      WHERE dae.dm_ads_extract_id=damld_cur_dae_id
       AND dae.owner_name=cnvtupper(requestin->list_0[damld_iter].owner_name)
       AND dae.table_name=cnvtupper(requestin->list_0[damld_iter].table_name)
       AND dae.table_extract_nbr=cnvtint(requestin->list_0[damld_iter].table_extract_nbr)
       AND dae.table_extract_instance_nbr=cnvtint(requestin->list_0[damld_iter].
       table_extract_instance_nbr)
     ...
     ENDIF
    ENDIF
    IF (damld_insert_flag=1)
     SET dm_err->eproc = "Getting next seq value"
     SELECT INTO "nl:"
      x = seq(dm_clinical_seq,nextval)
      FROM dual
      DETAIL
       damld_cur_dae_id = cnvtreal(x)
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc))
      GO TO exit_program
     ENDIF
     SET dm_err->eproc = "Inserting new extract row into DM_ADS_EXTRACT."
     CALL disp_msg("",dm_err->logfile,0)
     INSERT  FROM dm_ads_extract dae
      SET dae.dm_ads_extract_id = damld_cur_dae_id, dae.owner_name = cnvtupper(requestin->list_0[
        damld_iter].owner_name), dae.table_name = cnvtupper(requestin->list_0[damld_iter].table_name),
       dae.table_extract_nbr = cnvtint(requestin->list_0[damld_iter].table_extract_nbr), dae
       .table_extract_instance_nbr = cnvtint(requestin->list_0[damld_iter].table_extract_instance_nbr
        ), dae.active_ind = cnvtint(requestin->list_0[damld_iter].active_ind),
       dae.driver_table_name = cnvtupper(requestin->list_0[damld_iter].driver_table_name), dae
       .data_class_type = cnvtupper(requestin->list_0[damld_iter].data_class_type), dae
       .extract_method = cnvtupper(requestin->list_0[damld_iter].extract_method),
       dae.driver_table_ind = cnvtint(requestin->list_0[damld_iter].driver_table_ind), dae
       .driver_keycol_name = cnvtupper(requestin->list_0[damld_iter].driver_keycol_name), dae
       .driver_rankcol_name = cnvtupper(requestin->list_0[damld_iter].driver_rankcol_name),
       dae.apply_where_ind = cnvtint(requestin->list_0[damld_iter].apply_where_ind), dae
       .expimp_parent_table_name = cnvtupper(requestin->list_0[damld_iter].expimp_parent_table_name),
       dae.expimp_level_nbr = cnvtint(requestin->list_0[damld_iter].expimp_level_nbr),
       dae.where_clause = trim(requestin->list_0[damld_iter].where_clause), dae.table_comment = trim(
        requestin->list_0[damld_iter].table_comment), dae.dupdel_skip_ind = cnvtint(requestin->
        list_0[damld_iter].dupdel_skip_ind),
       dae.updt_cnt = cnvtint(requestin->list_0[damld_iter].updt_cnt), dae.updt_id = 0, dae
       .updt_dt_tm = cnvtdatetime(curdate,curtime3),
       dae.updt_task = 15301, dae.updt_applctx = 0
      WITH nocounter
     ;end insert
     IF (check_error(dm_err->eproc))
      SET dm_err->err_ind = 1
     ...
     ENDIF
    ENDIF
    IF (((damld_insert_flag=1) OR (damld_updt_flag=1))
     AND cnvtint(requestin->list_0[damld_iter].driver_table_ind)=1)
     IF ((dads_list->config_cnt > 0))
      SET dm2scramble->in_text = requestin->list_0[damld_iter].table_name
      IF (ds_scramble(null)=0)
       GO TO exit_program
      ENDIF
      SET damld_table_name = build(dm2scramble->out_text)
      SET damld_table_name = replace(check(damld_table_name," ")," ","",0)
      SET dm_err->eproc = "Verify that table exists in domain"
      SELECT INTO "nl:"
       FROM dba_tables dt
       WHERE dt.owner=cnvtupper(requestin->list_0[damld_iter].owner_name)
        AND dt.table_name=cnvtupper(damld_table_name)
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       SET dm_err->err_ind = 1
      ...
      IF (curqual > 0)
       IF (damld_insert_flag=1)
        SET dm_err->eproc = "Inserting new row into dm_ads_config_extract for new metadata"
        CALL disp_msg("",dm_err->logfile,0)
        INSERT  FROM dm_ads_config_extract dace,
          (dummyt d  WITH seq = value(dads_list->config_cnt))
         SET dace.dm_ads_config_extract_id = cnvtreal(seq(dm_clinical_seq,nextval)), dace
          .dm_ads_config_id = dads_list->config_qual[d.seq].config_id, dace.dm_ads_extract_id =
          damld_cur_dae_id,
          dace.status_txt = dpl_incomplete, dace.message_txt = "New Driver Table"
         PLAN (d
          WHERE d.seq > 0)
          JOIN (dace)
         WITH nocounter
        ...
        IF (check_error(dm_err->eproc)=1)
         ...
         GO TO exit_program
        ENDIF
       ELSEIF (damld_updt_flag=1)
        SET dm_err->eproc = "Updating Status in dm_ads_config_extract for new metadata"
        CALL disp_msg("",dm_err->logfile,0)
        UPDATE  FROM dm_ads_config_extract dace,
          (dummyt d  WITH seq = value(dads_list->config_cnt))
         SET dace.status_txt = dpl_incomplete, dace.message_txt = "New ADS Metadata"
         PLAN (d
          WHERE d.seq > 0)
          JOIN (dace
          WHERE (dace.dm_ads_config_id=dads_list->config_qual[d.seq].config_id)
           AND dace.dm_ads_extract_id=damld_cur_dae_id)
         WITH nocounter
        ;end update
        IF (check_error(dm_err->eproc)=1)
         SET dm_err->err_ind = 1
  ...
  ENDFOR
 ENDIF
 FOR (damld_iter2 = 1 TO dads_list->config_cnt)
   SET dm_err->eproc = "Checking Config Status in dm_ads_config_extract for new metadata"
   SELECT INTO "nl:"
    FROM dm_ads_config_extract dace
    WHERE (dace.dm_ads_config_id=dads_list->config_qual[damld_iter2].config_id)
     AND dace.status_txt=dpl_incomplete
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   ENDIF
   IF (curqual > 0)
    IF (dar_sync_config_status(dads_list->config_qual[damld_iter2].config_id)=0)
     GO TO exit_program
    ENDIF
   ENDIF
 ENDFOR
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM_ADS_EXTRACT was successfully loaded."
  CALL disp_msg("",dm_err->logfile,0)
 ELSE
  SET dm2scramble->in_text = requestin->list_0[damld_iter].table_name
  ...
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 SET dm_err->eproc = "Ending dm2_ads_metadata_load_dae."
 CALL final_disp_msg("dm2_ads_load_dae")
END GO
