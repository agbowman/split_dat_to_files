CREATE PROGRAM dm2_mig_copy_archivelog:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmcal_dcl_status = i2 WITH protect, noconstant(0)
 DECLARE dmcal_dcl_cmd = vc WITH protect, noconstant("")
 DECLARE dmcal_database_name = vc WITH protect, noconstant("")
 DECLARE dmcal_file_loc = vc WITH protect, noconstant("")
 DECLARE dmcal_arch_log_format = vc WITH protect, noconstant("")
 DECLARE dmcal_dest = vc WITH protect, noconstant("")
 DECLARE dmcal_name = vc WITH protect, noconstant("")
 DECLARE dmcal_ext = vc WITH protect, noconstant("")
 DECLARE dmcal_name_no_ext = vc WITH protect, noconstant("")
 DECLARE dmcal_cmd = vc WITH protect, noconstant("")
 DECLARE dmcal_nfsarch_loc = vc WITH protect, noconstant("")
 DECLARE dmcal_err_log = vc WITH protect, noconstant("")
 DECLARE dmcal_err_flag = i2 WITH protect, noconstant(0)
 DECLARE dmcal_email_msg = vc WITH protect, noconstant("")
 DECLARE dmcal_action = vc WITH protect, noconstant("")
 DECLARE dmcal_subject = vc WITH protect, noconstant("")
 DECLARE dmcal_cnt = i2 WITH protect, noconstant(0)
 DECLARE dmcal_fname = vc WITH protect, noconstant("")
 DECLARE dmcal_bypass_compare = i2 WITH protect, noconstant(0)
 DECLARE dmcal_use_ftp = i2 WITH protect, noconstant(0)
 DECLARE dmcal_delete_before_copy = i2 WITH protect, noconstant(0)
 DECLARE dmcal_ftp_user = vc WITH protect, noconstant("")
 DECLARE dmcal_ftp_pass = vc WITH protect, noconstant("")
 DECLARE dmcal_ftp_node = vc WITH protect, noconstant("")
 DECLARE dmcal_copy_retry_cnt = i4 WITH protect, noconstant(10)
 DECLARE dmcal_copy_retry_wait_time = i4 WITH protect, noconstant(60)
 DECLARE dmcal_copy_com_file = vc WITH protect, noconstant("")
 DECLARE dmcal_write_err_log(dcel_emsg=vc,dcel_body=vc,dcel_action=vc,dcel_log_file=vc) = i2
 DECLARE dmcal_create_ftp_com(dcfc_op_type=vc) = i2
 SET dm_err->logfile = "DM2_LOGFILE_NOTSET"
 SET dm_err->eproc = "Validating inputs for dm2_mig_copy_archivelog"
 SET dmcal_dest =  $1
 SET dmcal_name =  $2
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg =
  "Parameter usage: dm2_mig_copy_archivelog <archive log destination>, <archive log name>"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 IF (findstring(";",dmcal_name) > 0)
  SET dmcal_name = substring(1,(findstring(";",dmcal_name) - 1),dmcal_name)
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Archive Log Location: ",dmcal_dest))
  CALL echo(concat("Archive Log Name: ",dmcal_name))
 ENDIF
 SET dmcal_fname = substring((textlen(cnvtstring(cnvtdatetime(curdate,curtime3),17)) - 7),8,
  cnvtstring(cnvtdatetime(curdate,curtime3),17))
 SET dm_err->errfile = concat("dm2ca_",dmcal_fname,trim(currdbhandle),".err")
 SET dm_err->logfile = concat("dm2ca_",dmcal_fname,trim(currdbhandle),".log")
 SET dmcal_copy_com_file = concat("dm2cl_",dmcal_fname,trim(currdbhandle),".com")
 IF (init_logfile(dm_err->logfile,"DM2_MIG_COPY_ARCHIVELOG LOGFILE")=0)
  GO TO exit_script
 ENDIF
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_copy_archivelog"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmcal_database_name = cnvtupper(currdbname)
 SET dmcal_action = concat("dm2_mig_copy_archivelog '",dmcal_dest,"','",dmcal_name,"' go")
 SET dmcal_subject = concat("DATABASE MIGRATION : Process to copy archive logfile ",dmcal_name,
  " failed")
 SET dmcal_email_msg = concat(
  "After resolving error execute the following ccl script, after connecting to the Source Database ",
  "to get the archive log copied successfully:")
 SET dm_err->eproc = "Determining current OS."
 IF (cursys != "AXP")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "dm2_mig_copy_archivelog can only be used from a VMS node."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF ((dmr_emails->cnt=0))
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining existing miscellaneous log file location from dm_info."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name IN ("MISC_FILE_LOCATION", "BYPASS_COPY_COMPARE", "TARGET_FTP_LOGIN",
  "ARCH_COPY_RETRY_COUNT", "ARCH_COPY_RETRY_WAIT_TIME",
  "ARCH_DELETE_BEFORE_COPY")
  DETAIL
   CASE (di.info_name)
    OF "ARCH_DELETE_BEFORE_COPY":
     dmcal_delete_before_copy = 1
    OF "ARCH_COPY_RETRY_COUNT":
     dmcal_copy_retry_cnt = di.info_number
    OF "ARCH_COPY_RETRY_WAIT_TIME":
     dmcal_copy_retry_wait_time = di.info_number
    OF "BYPASS_COPY_COMPARE":
     dmcal_bypass_compare = 1
    OF "MISC_FILE_LOCATION":
     dmcal_file_loc = di.info_char
    OF "TARGET_FTP_LOGIN":
     dmcal_use_ftp = 1,dmcal_ftp_node = substring(1,(findstring("|||",di.info_char,1,0) - 1),di
      .info_char),dmcal_ftp_user = substring((findstring("|||",di.info_char,1,0)+ 3),(findstring(
       "|||",di.info_char,1,1) - (findstring("|||",di.info_char,1,0)+ 3)),di.info_char),
     dmcal_ftp_pass = substring((findstring("|||",di.info_char,1,1)+ 3),textlen(di.info_char),di
      .info_char)
   ENDCASE
  WITH nocounter
 ;end select
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No location for storage of Misc. files is found",
   "Please set up the archive log copy job using dm2_mig_menu.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Misc File Location: ",dmcal_file_loc))
 ENDIF
 IF (dmcal_use_ftp=0)
  SET dm_err->eproc = "Check that NFSARCH directory exists."
  CALL disp_msg("",dm_err->logfile,0)
  IF (dmr_check_directory(build("NFSARCH:[V500.DB_",dmcal_database_name,".ARCH]"))=0)
   SET dm_err->emsg = build("NFSARCH:[V500.DB_",dmcal_database_name,
    ".ARCH] is not available or does not have write privs.")
   GO TO exit_script
  ENDIF
  SET dmcal_nfsarch_loc = concat("NFSARCH:[V500.DB_",dmcal_database_name,".ARCH]")
 ELSE
  SET dm_err->eproc = "Using FTP for file copy, bypassing use of NFSARCH."
  CALL disp_msg("",dm_err->logfile,0)
  SET dmcal_nfsarch_loc = concat("/nfsarch/v500/db_",cnvtlower(dmcal_database_name),"/arch/")
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dmcal_arch_log_format = vp.value, dmcal_arch_log_format = concat(substring(1,(findstring("%",
      dmcal_arch_log_format) - 1),dmcal_arch_log_format),"*.",trim(substring((findstring(".",
       dmcal_arch_log_format,1,1)+ 1),size(dmcal_arch_log_format),dmcal_arch_log_format)))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (((curqual=0) OR (dmcal_arch_log_format="")) )
  SET dm_err->err_ind = 1
 ...
 ENDIF
 IF (dmcal_use_ftp=0)
  SET dm_err->eproc = concat("Copying archive log: ",dmcal_name," from ",dmcal_dest," to ",
   dmcal_nfsarch_loc)
  CALL disp_msg("",dm_err->logfile,0)
 ELSE
  SET dm_err->eproc = concat("FTP archive log: ",dmcal_name," from ",dmcal_dest," to ",
   dmcal_ftp_node,":",dmcal_nfsarch_loc)
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SET dmcal_ext = trim(substring((findstring(".",dmcal_arch_log_format,1,1)+ 1),size(
    dmcal_arch_log_format),dmcal_arch_log_format))
 SET dmcal_name_no_ext = trim(substring(1,(findstring(".",dmcal_name) - 1),dmcal_name))
 SET dmcal_err_log = concat(dmcal_file_loc,"dm2_cpyarc_",trim(replace(dmcal_name,"0123456789",
    "0123456789",3)),".log")
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Archive log format: ",dmcal_arch_log_format))
  CALL echo(concat("name without extension: ",dmcal_name_no_ext))
  CALL echo(concat("Error Log Location (file sent out in emails): ",dmcal_err_log))
 ENDIF
 IF (dmcal_use_ftp=0)
  WHILE (dmcal_cnt < dmcal_copy_retry_cnt
   AND dmcal_err_flag=0)
   SET dmcal_cmd = concat("copy ",dmcal_dest,dmcal_name," ",dmcal_nfsarch_loc,
    dmcal_name)
   IF (dm2_push_dcl(dmcal_cmd)=0)
    IF (findstring("%COPY-E-OPEN",dm_err->errtext,1,1) > 0)
     SET dmcal_cnt = (dmcal_cnt+ 1)
     CALL echo(concat("ARCHIVE LOG IS CURRENTLY BEING WRITTEN TO. WAITING ",build(
        dmcal_copy_retry_wait_time)," seconds..."))
     CALL pause(dmcal_copy_retry_wait_time)
     IF ((dmcal_cnt=(dmcal_copy_retry_cnt - 1)))
      SET dmcal_err_flag = 1
     ELSEIF (dm2_findfile(build(dmcal_nfsarch_loc,dmcal_name)) > 0)
      SET dmcal_cmd = concat("del ",dmcal_nfsarch_loc,dmcal_name,";*")
      IF (dm2_push_dcl(dmcal_cmd)=0)
       SET dmcal_err_flag = 1
      ENDIF
     ENDIF
    ...
    ENDIF
   ELSE
    SET dmcal_cnt = dmcal_copy_retry_cnt
   ENDIF
  ENDWHILE
 ELSE
  IF (dmcal_delete_before_copy=1)
   IF (dmcal_create_ftp_com("DELETE")=0)
    GO TO exit_script
   ENDIF
   SET dm_err->disp_dcl_err_ind = 0
   SET dmcal_cmd = concat("@",dmcal_copy_com_file)
   CALL dm2_push_dcl(dmcal_cmd)
   IF ((dm_err->debug_flag < 10))
    SET dm_err->eproc = "Cleanup delete scipt"
    IF (dm2_push_dcl(concat("delete ",dmcal_copy_com_file,";\*"))=0)
     SET dm_err->err_ind = 0
     CALL disp_msg("Error is ignorable.",dm_err->logfile,0)
    ENDIF
   ENDIF
  ENDIF
  IF (dmcal_create_ftp_com("PUT")=0)
   GO TO exit_script
  ENDIF
  WHILE (dmcal_cnt < dmcal_copy_retry_cnt
   AND dmcal_err_flag=0)
    ...
    SET dmcal_cmd = concat("@",dmcal_copy_com_file)
    CALL dm2_push_dcl(dmcal_cmd)
    CALL parse_errfile(dm_err->errfile)
    IF ((dm_err->debug_flag > 0))
     CALL echorecord(dm_err)
     CALL echo(dm_err->errtext)
    ENDIF
    IF (findstring("226 ",dm_err->errtext,1,1)=0)
     SET dmcal_cnt = (dmcal_cnt+ 1)
     CALL echo(concat("ARCHIVE LOG IS CURRENTLY BEING WRITTEN TO. WAITING ",build(
        dmcal_copy_retry_wait_time)," seconds..."))
     CALL pause(dmcal_copy_retry_wait_time)
    ...
    ELSE
     SET dmcal_cnt = dmcal_copy_retry_cnt
     IF ((dm_err->debug_flag < 10))
      SET dm_err->eproc = "Cleanup copy scipt"
      IF (dm2_push_dcl(concat("delete ",dmcal_copy_com_file,";\*"))=0)
       SET dm_err->err_ind = 0
       CALL disp_msg("Error is ignorable.",dm_err->logfile,0)
      ENDIF
 ...
 ENDIF
 IF (dmcal_err_flag=1)
  SET dmcal_subject = "DATABASE MIGRATION : Failure while copying archive logfile!"
  SET dmcal_err_flag = 1
  CALL dmcal_write_err_log(dm_err->errtext,dmcal_email_msg,dmcal_action,dmcal_err_log)
  GO TO exit_script
 ENDIF
 IF (dmcal_bypass_compare=0
  AND dmcal_use_ftp=0)
  SET dm_err->eproc = concat("Comparing the contents of ",dmcal_name,
   " in the original location to the new NFSARCH location")
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SET dmcal_cmd = concat("backup/compare/log ",dmcal_dest,dmcal_name," ",dmcal_nfsarch_loc,
   dmcal_name)
  IF (dm2_push_dcl(dmcal_cmd)=0)
   SET dmcal_subject = "DATABASE MIGRATION : Failed file compare after copying archive logfile!"
   SET dmcal_err_flag = 1
   CALL dmcal_write_err_log(concat("File compare failed ",dm_err->errtext),dmcal_email_msg,
    dmcal_action,dmcal_err_log)
   GO TO exit_script
  ENDIF
 ELSE
  IF (dmcal_use_ftp=0)
   SET dm_err->eproc = concat("Bypassing the comparison of ",dmcal_name,
    " in the original location to the new NFSARCH location")
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
 ENDIF
 GO TO exit_script
 SUBROUTINE dmcal_create_ftp_com(dcfc_op_type)
   SET dm_err->eproc = concat("Writing com script for FTP: ",dmcal_copy_com_file," for ",dcfc_op_type,
    " operation.")
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(dmcal_copy_com_file)
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     CALL print("FTP"), row + 1,
     CALL print(concat("connect ",dmcal_ftp_node)),
     row + 1,
     CALL print(dmcal_ftp_user), row + 1,
     CALL print(dmcal_ftp_pass), row + 1,
     CALL print("set type image"),
     row + 1
     IF (dcfc_op_type="DELETE")
      CALL print(concat('delete "',dmcal_nfsarch_loc,cnvtupper(dmcal_name),'"')), row + 1
     ELSE
      CALL print(concat("put ",dmcal_dest,dmcal_name," ",dmcal_nfsarch_loc,
       cnvtupper(dmcal_name))), row + 1
     ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmcal_write_err_log(dcel_emsg,dcel_body,dcel_action,dcel_log_file)
   SET dm_err->eproc = concat("Writing error contents to logfile: ",dcel_log_file)
   CALL disp_msg("",dm_err->logfile,0)
   SET dm_err->err_ind = 0
   SELECT INTO value(build(dcel_log_file))
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     CALL print("Error Log:"), row + 3,
     CALL print(concat(" ",dcel_log_file)),
     row + 4,
     CALL print("  "), row + 2,
     CALL print("Error Message:"), row + 3,
     CALL print(concat(" ",dcel_emsg)),
     row + 4,
     CALL print("  "), row + 2,
     CALL print("User Action:"), row + 3,
     CALL print(concat(" ",dcel_body)),
     row + 4,
     CALL print("  "), row + 2,
     CALL print(concat(" ",dcel_action)), row + 1
    WITH nocounter, maxcol = 1000, format = variable,
     maxrow = 1
   ;end select
   IF (check_error(dm_err->eproc)=1)
 ...
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "Ending dm2_mig_copy_archivelog."
 CALL final_disp_msg("dm2_mig_copy_arcl")
 IF (cursys="AXP")
  IF (size(dmr_emails->email_list) >= 1
   AND (dm_err->err_ind=1))
   SET dm_err->err_ind = 0
   IF (dmcal_err_flag=1)
    CALL dmr_send_email(dmcal_subject,dmr_emails->email_list,dmcal_err_log)
   ELSE
    CALL dmr_send_email("DATABASE MIGRATION : Severe error during single archive log copy process!",
     dmr_emails->email_list,dm_err->logfile)
   ENDIF
  ENDIF
  IF (dmcal_file_loc != logical("CCLUSERDIR"))
   SET dm_err->err_ind = 0
   CALL dm2_push_dcl(concat("COPY ",dm_err->logfile," ",dmcal_file_loc))
   SET dm_err->err_ind = 0
   CALL dm2_push_dcl(concat("DELETE ",dm_err->logfile,";*"))
  ENDIF
  SET dmcal_dcl_cmd = concat("DELETE ",dm_err->errfile,";*")
  IF ((dm_err->debug_flag > 0))
   CALL echo(dmcal_dcl_cmd)
  ENDIF
  CALL dcl(dmcal_dcl_cmd,size(dmcal_dcl_cmd),dmcal_dcl_status)
 ENDIF
END GO
