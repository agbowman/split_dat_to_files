CREATE PROGRAM al_bhs_get_cis_inbox_msg:dba
 PROMPT
  "Inbox ID:" = "",
 ...
 DECLARE ms_inbox_id = vc WITH protect, constant(trim( $S_INBOX_ID))
 ...
 SELECT INTO "nl:"
  ...
  HEAD taa.task_id
   pl_cnt = (pl_cnt+ 1), stat = alterlist(m_rec->msg,pl_cnt), m_rec->msg[pl_cnt].s_reference_id = ta2
   ...
   IF (ta1.event_id=0.0)
    ...
    ps_blob_out = trim(lt.long_text), ms_blob_rtf = fillstring(10000," "),
    CALL uar_rtf2(ps_blob_out,textlen(ps_blob_out),blob_rtf,size(blob_rtf),blob_return_len2,1),
    ms_tmp = trim(blob_rtf), m_rec->msg[pl_cnt].s_msg_body = trim(ms_tmp)
   ELSE
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   pl_cnt = 0
  HEAD cv.code_value
   pl_cnt = (pl_cnt+ 1), stat = alterlist(m_rec->phone_cd,pl_cnt), m_rec->phone_cd[pl_cnt].s_disp =
   trim(cv.display_key),
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (textlen(trim(cb.blob_contents)) > 0)
    blob_size = cnvtint(cb.blob_length), blob_out_detail = fillstring(128000," "),
    blob_compressed_trimmed = fillstring(128000," "),
    blob_uncompressed = fillstring(128000," "), blob_rtf = fillstring(128000," "), blob_out_detail =
    fillstring(128000," "),
    blob_compressed_trimmed = trim(cb.blob_contents), blob_return_len = 0, blob_return_len2 = 0,
    ps_blob_out = fillstring(128000," ")
    IF (cb.compression_cd=mf_comp_cd)
     CALL echo("compressed"),
     CALL uar_ocf_uncompress(blob_compressed_trimmed,size(blob_compressed_trimmed),blob_uncompressed,
     size(blob_uncompressed),blob_return_len), ps_blob_out = blob_uncompressed,
