CREATE PROGRAM dm2_rdds_copy_child:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drcu_min_cnt = f8 WITH protect
 DECLARE drcc_to_id = f8 WITH protect
 DECLARE drcc_from_id = f8 WITH protect
 DECLARE drcc_range_inc = f8 WITH protect
 SET drcu_min_cnt =  $1
 SET drcc_to_id =  $2
 SET drcc_from_id =  $3
 SET drcc_range_inc =  $4
 IF (check_logfile("dm2_rdds_copy_util",".log","DM2_RDDS_COPY_UTIL LOGFILE")=0)
  ...
  GO TO exit_child
 ENDIF
 SET drcu_copy_err_ind = 0
 CALL parser("insert into dm_chg_log (")
 FOR (drcc_for_cnt = 1 TO drcc_col_count)
   CALL parser(concat(trim(columns->list[drcc_for_cnt].name),","))
 ENDFOR
 CALL parser("TARGET_ENV_ID, LOG_ID, LOG_TYPE, CHG_DT_TM)")
 CALL parser("(select ")
 FOR (drcc_for_cnt = 1 TO drcc_col_count)
   CALL parser(build(" d.",trim(columns->list[drcc_for_cnt].name),","))
 ENDFOR
 CALL parser("drcc_to_id,")
 CALL parser("seq(dm_clinical_seq, nextval),")
 IF (drcu_question1="Y")
  CALL parser(
   "evaluate(d.log_type, 'MERGED', 'REFCHG','PROCES','REFCHG','NOMV14','REFCHG','HOLDNG','REFCHG', d.log_type),"
   )
 ELSE
  CALL parser(
   "evaluate(d.log_type, 'PROCES', 'REFCHG', 'HOLDNG', 'REFCHG', 'NOMV14', 'REFCHG', d.log_type),")
 ENDIF
 CALL parser(" cnvtdatetime(curdate,curtime3)")
 CALL parser("from dm_chg_log d")
 CALL parser("where d.target_env_id+0 = drcc_from_id")
 CALL parser(" and d.log_id >= drcu_min_cnt")
 CALL parser(" and d.log_id < drcu_min_cnt + drcc_range_inc")
 IF (drcu_question1="N"
  AND drcu_question2="N")
  CALL parser(" and d.log_type IN ('REFCHG', 'NORDDS', 'PROCES', 'NOMV14', 'HOLDNG')")
 ENDIF
 CALL parser(" and not exists(select 'X' from dm_chg_log dc")
 CALL parser("             where dc.pk_where_value = d.pk_where_value")
 CALL parser("              and cnvtupper(dc.table_name) = d.table_name")
 CALL parser("              and dc.pk_where = d.pk_where")
 CALL parser("              and dc.delete_ind = d.delete_ind")
 CALL parser("              and dc.context_name = d.context_name")
 CALL parser("              and dc.target_env_id + 0 = drcc_to_id")
 IF (drcu_question1="Y")
  CALL parser(concat("     and dc.log_type = evaluate(d.log_type, 'MERGED', 'REFCHG',",
    "'PROCES','REFCHG','NOMV14','REFCHG','HOLDNG','REFCHG', d.log_type)"))
 ELSE
  CALL parser(concat("     and dc.log_type = evaluate(d.log_type, ",
    "'PROCES','REFCHG','NOMV14','REFCHG','HOLDNG','REFCHG', d.log_type)"))
 ENDIF
 CALL parser("with orahintcbo('INDEX (DC XIE2DM_CHG_LOG)') )")
 CALL parser("with nocounter, orahintcbo('INDEX (D XPKDM_CHG_LOG)')) go")
 IF (check_error("ERROR occurred when inserting rows into dm_chg_log") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET drcu_copy_err_ind = 1
  ROLLBACK
 ELSE
  COMMIT
 ENDIF
#exit_child
 CALL final_disp_msg("dm2_rdds_copy_child")
END GO
