CREATE PROGRAM br_add_lighthouse_ccn:dba
 ...
 SET readme_data->message = "Readme Failed: Starting <br_add_lighthouse_ccn.prg> script"
 ...
 SELECT INTO "nl:"
  FROM br_step bs
  PLAN (bs
   WHERE bs.step_mean="LIGHTREPORTSCCN")
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed checking existence of LIGHTREPORTSCCN row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_step bs
   SET bs.step_mean = "LIGHTREPORTSCCN", bs.step_disp =
    "Meaningful Use CCN and Eligible Provider Setup", bs.step_type = "IMPMAINT",
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcir.br_client_id=1
   AND bcir.item_type="STEP"
   AND bcir.item_mean="LIGHTREPORTSCCN"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to verify existence of STEP/LIGHTREPORTSCCN row: ",errmsg
   )
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_client_item_reltn bcir
   SET bcir.br_client_item_reltn_id = seq(bedrock_seq,nextval), bcir.br_client_id = 1, bcir.item_type
     = "STEP",
    bcir.item_mean = "LIGHTREPORTSCCN", bcir.item_display =
    "Meaningful Use CCN and Eligible Provider Setup", bcir.step_cat_mean = "CORE",
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcss.br_client_id=1
   ...
   AND bcss.step_mean="LIGHTREPORTSCCN"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to check for existence of COREL/LIGHTREPORTSCCN row: ",
   errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  SELECT INTO "nl:"
  ...
  INSERT  FROM br_client_sol_step bcss
   SET bcss.br_client_id = 1, bcss.solution_mean = "COREL", bcss.step_mean = "LIGHTREPORTSCCN",
 ...
 SET readme_data->message = "Readme Succeeded: <br_add_lighthouse_ccn.prg> script"
#exit_script
