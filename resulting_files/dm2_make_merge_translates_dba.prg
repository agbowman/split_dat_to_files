CREATE PROGRAM dm2_make_merge_translates:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE src_table = vc WITH protect
 DECLARE mrgins_flag = i4 WITH protect, noconstant(0)
 DECLARE mrgsrc_env_id = i4 WITH protect, noconstant(0)
 DECLARE cur_env_id = i4 WITH protect, noconstant(0)
 DECLARE dmmt_pre_link = vc WITH protect
 DECLARE dmmt_post_link = vc WITH protect
 IF (check_logfile("dm2_make_mrg_trans",".log","DM2_MAKE_MERGE_TRANSALTES LOG FILE")=0)
  GO TO exit_program
 ENDIF
 SET mrgsrc_env_id =  $1
 SELECT INTO "nl:"
  ...
  DETAIL
   cur_env_id = a.info_number
  WITH nocounter
 ;end select
 IF (cur_env_id=0)
  SET dm_err->emsg = "ERROR: target environment id doesn't exist"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->ecode = 1
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM dm_env_reltn d
  WHERE d.child_env_id=cur_env_id
   AND d.parent_env_id=mrgsrc_env_id
  DETAIL
   dmmt_pre_link = d.pre_link_name, dmmt_post_link = d.post_link_name
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET dm_err->emsg = "ERROR: Target and Source relation counld not be found"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->ecode = 1
  GO TO exit_program
 ENDIF
 SET src_table = build(dmmt_pre_link,"DM_MERGE_TRANSLATE",dmmt_post_link)
 FREE RECORD mrg_tbl
 RECORD mrg_tbl(
   1 tbl_cnt = i4
   1 tbl[*]
 ...
 SET mrg_tbl->tbl_cnt = 0
 SET stat = alterlist(mrg_tbl->tbl,0)
 SELECT DISTINCT INTO "nl:"
  t.table_name
  FROM dm_merge_translate t
  PLAN (t
   WHERE t.env_source_id=0
  ...
  DETAIL
   tcnt = (tcnt+ 1)
   IF (mod(tcnt,500)=1)
    stat = alterlist(mrg_tbl->tbl,(tcnt+ 499))
   ENDIF
   mrg_tbl->tbl[tcnt].name = t.table_name
  FOOT REPORT
   stat = alterlist(mrg_tbl->tbl,tcnt), mrg_tbl->tbl_cnt = tcnt
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET dm_err->emsg = "ERROR: Merge template counld not be found"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->ecode = 1
  GO TO exit_program
 ENDIF
 FOR (ti = 1 TO mrg_tbl->tbl_cnt)
   SET dm_err->eproc = concat("Attempting to insert data for table ",mrg_tbl->tbl[ti].name)
   CALL disp_msg(" ",dm_err->logfile,0)
   EXECUTE dm2_make_merge_trans_tbl value(mrg_tbl->tbl[ti].name), value(mrgsrc_env_id), value(
    cur_env_id),
   value(src_table)
   IF ((dm_err->err_ind > 0))
    SET ti = (mrg_tbl->tbl_cnt+ 1)
   ENDIF
 ENDFOR
#exit_program
END GO
