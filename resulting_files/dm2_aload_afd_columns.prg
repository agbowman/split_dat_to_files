CREATE PROGRAM dm2_aload_afd_columns
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_col",".log","dm2_aload_afd_columns")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_afd_columns"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 DECLARE daac_rows_returned = i4 WITH protect, noconstant(1)
 ...
 IF (daac_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daac_file_loc_ret,daac_file), "dm2_aload_sch_col", daac_batch
  CALL echo(concat(daac_file_loc_ret,daac_file))
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Remove extraneous DM_AFD_COLUMNS rows"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dm_afd_columns a
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_afd_columns completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_afd_columns")
END GO
