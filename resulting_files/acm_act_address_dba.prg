CREATE PROGRAM acm_act_address:dba
 ...
 FOR (index = 1 TO xref->act_cnt)
   SET reply->address_qual[xref->act[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->act_cnt)),
   address a
  SET a.active_ind = xref->act[d.seq].active_ind, a.active_status_cd =
   ...
   a.active_status_dt_tm = cnvtdatetime(sysdate), a.updt_cnt = (a.updt_cnt+ 1), a.updt_dt_tm =
   cnvtdatetime(sysdate),
   a.updt_id = reqinfo->updt_id, a.updt_applctx = reqinfo->updt_applctx, a.updt_task = reqinfo->
   updt_task
  PLAN (d)
   JOIN (a
   WHERE (a.address_id=acm_request->address_qual[xref->act[d.seq].idx].address_id))
  WITH nocounter, status(reply->address_qual[xref->act[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->act_cnt)
   IF ((reply->address_qual[xref->act[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    FROM address a
    WHERE expand(t1,f_val,t_val,a.address_id,acm_request->address_qual[xref->act[t1].idx].address_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,a.address_id,acm_request->address_qual[xref->act[t1].idx].
      address_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->address_qual[xref->act[index].idx].status=0))
