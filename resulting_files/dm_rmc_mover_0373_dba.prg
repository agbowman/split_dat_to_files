CREATE PROGRAM dm_rmc_mover_0373:dba
 ...
 SELECT INTO "NL:"
  FROM code_value cv
  WHERE cv.cdf_meaning="CARRIER"
   AND cv.code_set=370
   AND cv.active_ind=1
  DETAIL
   oplan_type = cv.code_value
  WITH nocounter
 ;end select
 ...
 IF (hp_check=0)
  SET cust_nomv_chk = select_merge_translate(cnvtstring(cust_hp_id),cust_tab_name)
