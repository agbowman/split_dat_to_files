CREATE PROGRAM dm_pucb_person_tribal_afltn:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PUCB_PERSON_TRIBAL_AFLTN"
 ...
 SUBROUTINE cust_ucb_del(dummy)
   UPDATE  FROM person_tribal_afltn
    ...
    WHERE (person_tribal_afltn_id=rchildren->qual1[det_cnt].entity_id)
    WITH nocounter
   ;end update
   IF (curqual=0)
    ...
    SET error_table = rchildren->qual1[det_cnt].entity_name
    SET request->error_message = substring(1,132,build(
      "Uncombine failed to reactive for person_tribal_afltn with pk val=",rchildren->qual1[det_cnt].
      entity_id))
    RETURN(0)
   ENDIF
   ...
   IF (add_hist(rchildren->qual1[det_cnt].entity_id)=0)
    ...
    SET error_table = rchildren->qual1[det_cnt].entity_name
    SET request->error_message = substring(1,132,build(
      "Could not insert history for person_tribal_afltn with pk=",rchildren->qual1[det_cnt].entity_id
      ))
    RETURN(0)
   ENDIF
   RETURN(1)
 ...
 SUBROUTINE cust_ucb_upt(dummy)
   ...
   IF (curqual=0)
    ...
    SET error_table = rchildren->qual1[det_cnt].entity_name
    SET request->error_message = substring(1,132,build(
      "Uncombine failed to update for person_tribal_afltn with pk val=",rchildren->qual1[det_cnt].
      entity_id))
    RETURN(0)
   ENDIF
   SET activity_updt_cnt += 1
 ...
 SUBROUTINE initialize_hist_id(null)
  IF (v_hist_init_ind=0)
   ...
   SET dcipht_request->transaction_reason_txt = "DM_PUCB_PERSON_TRIBAL_AFLTN"
