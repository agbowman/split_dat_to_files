CREATE PROGRAM acm_add_address:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->address_qual[xref->add[index].idx].address_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(address_seq,nextval)
     FROM dual
     DETAIL
      acm_request->address_qual[xref->add[index].idx].address_id = cnvtreal(nextseqnum), reply->
      address_qual[xref->add[index].idx].address_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->address_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->address_qual[xref->add[index].idx].address_id = acm_request->address_qual[xref->add[
    index].idx].address_id
   ENDIF
   SET reply->address_qual[xref->add[index].idx].status = 0
   IF ((acm_request->address_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->address_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(curdate,
     curtime3)
   ENDIF
   IF ((acm_request->address_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->address_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   address a
  SET a.address_id = acm_request->address_qual[xref->add[d.seq].idx].address_id, a.address_format_cd
    = acm_request->address_qual[xref->add[d.seq].idx].address_format_cd, a.address_info_status_cd =
   acm_request->address_qual[xref->add[d.seq].idx].address_info_status_cd,
   a.address_type_cd = acm_request->address_qual[xref->add[d.seq].idx].address_type_cd, a
   .address_type_seq = acm_request->address_qual[xref->add[d.seq].idx].address_type_seq, a
   .beg_effective_dt_tm = cnvtdatetime(acm_request->address_qual[xref->add[d.seq].idx].
    beg_effective_dt_tm),
   a.city = acm_request->address_qual[xref->add[d.seq].idx].city, a.contact_name = acm_request->
   address_qual[xref->add[d.seq].idx].contact_name, a.contributor_system_cd =
   IF ((acm_request->address_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0)) acm_request->
    address_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   a.country = acm_request->address_qual[xref->add[d.seq].idx].country, a.country_cd = acm_request->
   address_qual[xref->add[d.seq].idx].country_cd, a.county = acm_request->address_qual[xref->add[d
   .seq].idx].county,
   a.county_cd = acm_request->address_qual[xref->add[d.seq].idx].county_cd, a.end_effective_dt_tm =
   cnvtdatetime(acm_request->address_qual[xref->add[d.seq].idx].end_effective_dt_tm), a.mail_stop =
   acm_request->address_qual[xref->add[d.seq].idx].mail_stop,
   a.operation_hours = acm_request->address_qual[xref->add[d.seq].idx].operation_hours, a
   .parent_entity_id = acm_request->address_qual[xref->add[d.seq].idx].parent_entity_id, a
   .parent_entity_name = acm_request->address_qual[xref->add[d.seq].idx].parent_entity_name,
   a.postal_barcode_info = acm_request->address_qual[xref->add[d.seq].idx].postal_barcode_info, a
   .residence_cd = acm_request->address_qual[xref->add[d.seq].idx].residence_cd, a.residence_type_cd
    = acm_request->address_qual[xref->add[d.seq].idx].residence_type_cd,
   a.state = acm_request->address_qual[xref->add[d.seq].idx].state, a.state_cd = acm_request->
   address_qual[xref->add[d.seq].idx].state_cd, a.street_addr = acm_request->address_qual[xref->add[d
   .seq].idx].street_addr,
   a.street_addr2 = acm_request->address_qual[xref->add[d.seq].idx].street_addr2, a.street_addr3 =
   acm_request->address_qual[xref->add[d.seq].idx].street_addr3, a.street_addr4 = acm_request->
   address_qual[xref->add[d.seq].idx].street_addr4,
   a.zipcode = acm_request->address_qual[xref->add[d.seq].idx].zipcode, a.zip_code_group_cd =
   acm_request->address_qual[xref->add[d.seq].idx].zip_code_group_cd, a.district_health_cd =
   acm_request->address_qual[xref->add[d.seq].idx].district_health_cd,
   a.primary_care_cd = acm_request->address_qual[xref->add[d.seq].idx].primary_care_cd, a.zipcode_key
    = trim(cnvtupper(cnvtalphanum(acm_request->address_qual[xref->add[d.seq].idx].zipcode)),3), a
   .comment_txt = acm_request->address_qual[xref->add[d.seq].idx].comment_txt,
   a.source_identifier = acm_request->address_qual[xref->add[d.seq].idx].source_identifier, a
   .postal_identifier = acm_request->address_qual[xref->add[d.seq].idx].postal_identifier, a
   .postal_identifier_key = trim(cnvtupper(cnvtalphanum(acm_request->address_qual[xref->add[d.seq].
      idx].postal_identifier)),3),
   a.data_status_cd = reqdata->data_status_cd, a.data_status_dt_tm = cnvtdatetime(curdate,curtime3),
   a.data_status_prsnl_id = reqinfo->updt_id,
   a.active_ind =
   IF ((((acm_request->address_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((acm_request->
   address_qual[xref->add[d.seq].idx].active_status_cd=reqdata->active_status_cd))) ) 1
   ELSE 0
   ENDIF
   , a.active_status_cd =
   IF ((acm_request->address_qual[xref->add[d.seq].idx].active_status_cd > 0.0)) acm_request->
    address_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (a)
  WITH nocounter, status(reply->address_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->address_qual[xref->add[index].idx].status=0))
