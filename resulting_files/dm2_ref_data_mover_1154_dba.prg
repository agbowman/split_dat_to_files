CREATE PROGRAM dm2_ref_data_mover_1154:dba
 ...
 SET dmt_select = 0
 SET dmt_cnt = 0
 SET null_check = get_nullind(cust_tab_name,"PARENT_ENTITY_NAME")
 SET from_vc = get_value(cust_tab_name,"PARENT_ENTITY_NAME","FROM")
 SET from_id = get_value(cust_tab_name,"BILL_ORG_TYPE_ID","FROM")
 ...
 ENDIF
 IF (dmt_select=1
  AND cnvtreal(from_id) > 0)
  SET stat = alterlist(rdds_exception->qual,1)
  SET rdds_exception->qual[1].tab_col_name = "BILL_ORG_PAYOR_ID"
  ...
  IF (to_id != "No Trans")
   CALL put_value(cust_tab_name,"BILL_ORG_TYPE_ID",to_id)
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1154
   ENDIF
  ELSE
   SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(from_id))
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"ORG_PAYOR_ID")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_1154
 ENDIF
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1)
   ...
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"ORG_PAYOR_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"ORG_PAYOR_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  ORG_PAYOR_ID")
     ELSE
      GO TO exit_1154
     ENDIF
    ELSE
   ...
   SET from_id = get_value(cust_tab_name,"ORG_PAYOR_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1154
   ELSE
