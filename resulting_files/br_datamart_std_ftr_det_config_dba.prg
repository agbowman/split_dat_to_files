CREATE PROGRAM br_datamart_std_ftr_det_config:dba
 ...
 SET readme_data->message = "Readme Failed: Starting <br_datamart_std_ftr_det_config.prg> script"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   fcount += 1, stat = alterlist(filter->qual,fcount), filter->qual[fcount].id = b
   .br_datamart_filter_id,
   filter->qual[fcount].field_mean = requestin->list_0[d.seq].oe_field_mean
   IF ((requestin->list_0[d.seq].required_ind="Yes"))
    filter->qual[fcount].required_ind = 1
   ELSE
    filter->qual[fcount].required_ind = 0
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(fcount)),
 ...
 INSERT  FROM br_datamart_filter_detail b,
   (dummyt d  WITH seq = value(fcount))
 ...
 SET readme_data->message = "Readme Succeeded: <br_datamart_std_ftr_det_config.prg> script"
#exit_script
