CREATE PROGRAM dm2_utc_gen_dst_dates:dba
 ...
 SUBROUTINE dum_cust_incl_abort_gen(dciag_tgt_dbname,dciag_src_orcl_ver,dciag_tgt_orcl_ver)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dug_start_year = i4 WITH protect, noconstant( $1)
 DECLARE dug_end_year = i4 WITH protect, noconstant( $2)
 DECLARE dug_timezone_name = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm2_utc_gen_dst",".log","dm2_utc_gen_dst_dates")=false)
  GO TO exit_script
 ENDIF
 IF ((validate(dm2_utc_gen_dst_dates_override,- (1))=- (1)))
  SET dm_err->eproc = "This script is Obsolete."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF (dum_set_timezone(dug_timezone_name)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM2_UTC_GEN_DST_DATES"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dum_auto_dst_date(dug_start_year,dug_end_year)=0)
  GO TO exit_script
 ENDIF
 IF (dum_disp_dst_rpt(dug_timezone_name)=0)
  GO TO exit_script
 ENDIF
#exit_script
 SET dm_err->eproc = "Ending DM2_UTC_GEN_DST_DATES"
 CALL final_disp_msg("dm2_utc_gen_dst")
END GO
