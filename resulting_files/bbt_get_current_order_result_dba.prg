CREATE PROGRAM bbt_get_current_order_result:dba
 ...
 IF (stat=1)
  ...
  SET reply->status_data.subeventstatus[count1].targetobjectname = "bbt_get_current_order_result"
  SET reply->status_data.subeventstatus[count1].targetobjectvalue =
  "could not get Current name type code_value"
  GO TO exit_script
 ENDIF
 ...
 IF (stat=1)
  ...
  SET reply->status_data.subeventstatus[count1].targetobjectname = "bbt_get_current_order_result"
  SET reply->status_data.subeventstatus[count1].targetobjectvalue =
  "could not get Active Status code_value"
  GO TO exit_script
 ENDIF
 ...
 IF (stat=1)
  ...
  SET reply->status_data.subeventstatus[count1].targetobjectname = "bbt_get_current_order_result"
  SET reply->status_data.subeventstatus[count1].targetobjectvalue =
  "could not get combine action Add code_value"
  GO TO exit_script
 ENDIF
 ...
 FOR (cnt = 1 TO mean_cnt)
   ...
   IF (stat=1)
    ...
    SET reply->status_data.subeventstatus[count1].targetobjectname = "bbt_get_current_order_result"
    SET reply->status_data.subeventstatus[count1].targetobjectvalue =
    "could not get order_type code_value"
    GO TO exit_script
   ELSE
 ...
 FOR (cnt = 1 TO mean_cnt)
   ...
   IF (stat=1)
    ...
    SET reply->status_data.subeventstatus[count1].targetobjectname = "bbt_get_current_order_result"
    SET reply->status_data.subeventstatus[count1].targetobjectvalue =
    "could not get result_type code_value"
    GO TO exit_script
   ELSE
 ...
 SELECT INTO "nl:"
  o.order_id, o.order_mnemonic, r.task_assay_cd,
  r.result_status_cd, d.seq, dta.mnemonic,
  pr.perform_result_id, pr.nomenclature_id, pr.result_value_alpha,
  pr.result_code_set_cd, pr.result_value_numeric, re.event_dt_tm,
  re.event_personnel_id, pnl.username, co.specimen_id,
  co.drawn_dt_tm, accession_number = decode(aor.seq,uar_fmt_accession(aor.accession,size(aor
     .accession,1))," "), nomenclature_id_field = decode(n.seq,n.nomenclature_id,0.0),
  concept_source_cd_field = decode(n.seq,n.concept_source_cd,0.0), concept_identifier_field = decode(
   n.seq,n.concept_identifier,"")
  FROM (dummyt d  WITH seq = value(mean_cnt)),
  ...
  PLAN (d)
   JOIN (sd
   WHERE (sd.bb_processing_cd=request->resultmeanlist[d.seq].bb_processing_cd))
   JOIN (o
   ...
   WHERE dta.task_assay_cd=r.task_assay_cd
    AND (dta.bb_result_processing_cd=request->resultmeanlist[d.seq].bb_result_processing_cd))
   JOIN (pr
   ...
   JOIN (co
   WHERE co.container_id=pr.container_id
   ...
   JOIN (d_seo)
   JOIN (seo
   WHERE seo.specimen_id=co.specimen_id
    AND seo.active_ind=1)
  ORDER BY cnvtdatetime(re.event_dt_tm)
  HEAD REPORT
   select_ok_ind = 0, lcount = 0, facility_count = 0,
   facilityidxhold = 0
  DETAIL
  ...
  FOOT REPORT
   ...
   ENDIF
  WITH nocounter, dontcare(n), outerjoin(d_seo),
   nullreport
 ;end select
 IF (lcount > 0)
  SELECT INTO "nl:"
 ...
 FOR (lidx = 1 TO lcount)
   SET facilityidxhold = locateval(x_idx,1,size(facilityinfo->facilities,5),reply->qual[lidx].
   ...
   IF (facilityidxhold <= 0)
    ...
    ENDIF
    SET facilityinfo->facilities[facility_count].testing_facility_cd = reply->qual[lidx].
    testing_facility_cd
   ...
   IF ((flex_param_out->flex_on_ind=1))
    IF ((reply->qual[lidx].expiration_dt_tm=0.0))
     SET reply->qual[lidx].expiration_dt_tm = getflexexpirationforperson(request->person_id,0.0,reply
      ->qual[lidx].drawn_dt_tm,reply->qual[lidx].encntr_facility_cd,0)
  ...
  FOR (lidx = 1 TO lcount)
    ...
    SELECT INTO "nl:"
     ...
     FROM person_combine_det pcd,
      person_combine pc
     PLAN (pcd
      WHERE (pcd.entity_id=reply->qual[lidx].order_id)
    ...
    SELECT INTO "nl:"
     ...
     PLAN (pnh
      WHERE pnh.person_id=dcurrentpersonid
 ...
 IF (select_ok_ind != 1)
  ...
  SET reply->status_data.subeventstatus[count1].targetobjectname = "bbt_get_current_order_result"
  SET reply->status_data.subeventstatus[count1].targetobjectvalue =
  "select for orders/results failed"
 ENDIF
#exit_script
 ...
 IF ((request->debug_ind=1))
  ...
  CALL echo(build("STATUS: ",reply->status_data.status," / ",item_cnt,reply->status_data.
    subeventstatus[1].targetobjectvalue))
  FOR (item = 1 TO item_cnt)
    CALL echo(build("REPLY INFO ",item,": ",reply->qual[item].encntr_id," / ",
      reply->qual[item].order_id," / ",reply->qual[item].order_mnemonic," / ",reply->qual[item].
      bb_processing_cd,
      " / ",reply->qual[item].result_id," / ",reply->qual[item].task_assay_cd," / ",
      reply->qual[item].mnemonic," / ",reply->qual[item].bb_result_processing_cd," / ",reply->qual[
      item].result_status_cd,
      " / ",reply->qual[item].container_id," / ",reply->qual[item].specimen_id," / ",
      reply->qual[item].drawn_dt_tm," / ",reply->qual[item].perform_result_id," / ",reply->qual[item]
      .pr_nomenclature_id,
      " / ",reply->qual[item].result_value_alpha," / ",reply->qual[item].nomenclature_id," / ",
      reply->qual[item].concept_identifier," / ",reply->qual[item].concept_source_cd," / ",reply->
      qual[item].result_code_set_cd,
      " / ",reply->qual[item].result_value_numeric," / ",reply->qual[item].event_dt_tm," / ",
      reply->qual[item].event_personnel_id," / ",reply->qual[item].username," / ",reply->qual[item].
      accession,
      " / ",reply->qual[item].unformatted_accession," / ",reply->qual[item].accession_id," / ",
      reply->qual[item].historical_name," / ",reply->qual[item].interface_flag," / ",reply->qual[item
 ...
 SUBROUTINE populatereply(null)
   ...
   SET reply->qual[lcount].interface_status_flag = pr.interface_status_flag
