CREATE PROGRAM dm_rdm_upd_erx_synonym_ids_cc:dba
 ...
 SET ms_child_c_info_domain_nm = "DM_RDM_UPD_ERX_SYNONYM_IDS"
 SET ms_child_c_max_name = "MAX IB_RX_REQ_DRUG_ID EVALUATED"
 CALL sbr_get_min_max(mf_min_id,mf_max_id)
 CALL echo(build("Processing IB_RX_REQ_DRUG_IDs from [",mf_min_id,"] to [",mf_max_id,"]..."))
 CALL echo("")
 INSERT  FROM temp_ib_rx_req_drug temp
  (temp.ib_rx_req_drug_id)(SELECT
   irrd.ib_rx_req_drug_id
   FROM ib_rx_req_drug irrd,
    ib_rx_req irr
   WHERE irrd.synonym_id=0
    AND irrd.ib_rx_req_drug_id BETWEEN mf_min_id AND mf_max_id
    AND irr.ib_rx_req_id=irrd.ib_rx_req_id
 ...
 IF (sbr_handle_cc_error(null)=0)
  GO TO exit_program
 ENDIF
 UPDATE  FROM ib_rx_req_drug irrd
  SET irrd.synonym_id = null, irrd.updt_cnt = (irrd.updt_cnt+ 1), irrd.updt_dt_tm = cnvtdatetime(
    sysdate),
   irrd.updt_id = reqinfo->updt_id, irrd.updt_applctx = reqinfo->updt_applctx, irrd.updt_task =
   reqinfo->updt_task
  WHERE irrd.ib_rx_req_drug_id IN (
  (SELECT
   temp.ib_rx_req_drug_id
   FROM temp_ib_rx_req_drug temp))
  WITH nocounter
 ;end update
 IF (sbr_handle_cc_error(null)=0)
  GO TO exit_program
 ...
 ENDIF
#exit_program
 CALL sbr_truncate_table("TEMP_IB_RX_REQ_DRUG")
END GO
