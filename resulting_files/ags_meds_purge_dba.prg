CREATE PROGRAM ags_meds_purge:dba
 PROMPT
  "Backout Type (J, I, P) =" = "J",
  "AGS_JOB_ID (0.0) = " = 0.0
  WITH cbackout_type, djob_id
 CALL echo("***")
 CALL echo("***   BEG :: AGS_MEDS_PURGE")
 CALL echo("***")
 ...
 IF ( NOT (validate(log,0)))
  ...
  DECLARE sstatus_file_name = vc WITH private, noconstant(concat("ags_meds_purge_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_MEDS_PURGE"
 ...
 IF (the_type="P"
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ENDIF
    purge_rec->qual[sknt].mil_item_id = a.gs_med_claim_id, purge_rec->qual[sknt].delete_item_id = a
    .ags_meds_data_id, purge_rec->qual[sknt].delete_person_id = a.person_id,
 ...
 ELSEIF (the_type="I")
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "INVALID PURGE >> MEDS by Item Not Allowed"
  GO TO exit_script
 ELSE
  SET failed = input_error
  SET ilog_status = 1
 ...
 SELECT INTO "nl:"
  ...
  HEAD d.seq
   purge_rec->qual[d.seq].bo_item_id = a.ags_meds_data_id
  WITH nocounter
 ;end select
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("SELECT ERROR AGS_MEDS_DATA (backout items)>> ",trim(
    serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 DELETE  FROM gs_med_claim_alias g,
  ...
  PLAN (d
   ...
   JOIN (g
   WHERE (g.gs_med_claim_id=purge_rec->qual[d.seq].mil_item_id))
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("DELETE ERROR GS_MED_CLAIM_ALIAS >> ",trim(serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 DELETE  FROM gs_med_claim g,
   (dummyt d  WITH seq = value(purge_rec->qual_knt))
  SET g.seq = 1
  PLAN (d
   WHERE d.seq > 0
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("DELETE ERROR GS_MED_CLAIM >> ",trim(serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 UPDATE  FROM ags_meds_data a,
   (dummyt d  WITH seq = value(purge_rec->qual_knt))
  SET a.status = "BACK OUT", a.status_dt_tm = cnvtdatetime(curdate,curtime3), a.gs_med_claim_id = 0.0,
   a.prescriber_id = 0.0, a.person_id = 0.0
  PLAN (d
   WHERE d.seq > 0
    AND (purge_rec->qual[d.seq].bo_item_id > 0))
   JOIN (a
   WHERE (a.ags_meds_data_id=purge_rec->qual[d.seq].bo_item_id))
  WITH nocounter
 ;end update
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_MEDS_DATA (backout items >> ",trim(
    serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 ...
 UPDATE  FROM ags_meds_data a,
  ...
  PLAN (d
   WHERE d.seq > 0
    ...
    AND (purge_rec->qual[d.seq].delete_item_id > 0))
   JOIN (a
   WHERE (a.ags_meds_data_id=purge_rec->qual[d.seq].delete_item_id))
  WITH nocounter
 ;end update
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_MEDS_DATA (delete items) >> ",trim(
    serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("DELETE ERROR AGS_MEDS_DATA >> ",trim(serrmsg))
  GO TO exit_script
 ENDIF
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_MEDS_PURGE"
 IF (define_logging_sub=true)
  CALL echo("***")
