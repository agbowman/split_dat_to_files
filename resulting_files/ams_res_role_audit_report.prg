CREATE PROGRAM ams_res_role_audit_report
 PROMPT
 ...
 CALL inc_prompt(1)
 SET t_record->prompt_qual[t_record->prompt_qual_cnt].prompt_string = "Meaning:"
 CALL inc_prompt(1)
 ...
 CALL inc_prompt(1)
 SET t_record->prompt_qual[t_record->prompt_qual_cnt].prompt_string = "Resources:"
 CALL inc_prompt(1)
 ...
 SET entity = evaluate(t_record->entity_id,0.0,"a.sch_role_cd > 0",
  "a.sch_role_cd = t_record->entity_id")
 SELECT INTO "nl:"
  a.sch_role_cd
  FROM sch_role a,
  ...
  DETAIL
   ...
   ENDIF
   t_record->qual[t_record->qual_cnt].sch_role_cd = a.sch_role_cd
   IF ((t_record->qual[t_record->qual_cnt].sch_role_cd > 0))
    t_record->qual[t_record->qual_cnt].role_meaning = concat("Person ",trim(format(t_record->qual[
       t_record->qual_cnt].sch_role_cd,";l;i"))," not found")
   ELSE
    t_record->qual[t_record->qual_cnt].role_meaning = "NOT ASSOCIATED"
   ENDIF
   t_record->qual[t_record->qual_cnt].mnemonic = a.mnemonic, t_record->qual[t_record->qual_cnt].
   description = a.description, t_record->qual[t_record->qual_cnt].info_sch_text_id = a
   .info_sch_text_id
   ...
   ENDIF
   t_record->qual[t_record->qual_cnt].resource_qual_cnt = 0, t_record->qual[t_record->qual_cnt].
   list_qual_cnt = 0, t_record->qual[t_record->qual_cnt].max_active_size = 0
  WITH nocounter
 ;end select
 ...
 FOR (i = 1 TO t_record->qual_cnt)
   SELECT INTO "nl:"
    a.sch_role_cd
    FROM sch_booking a
    WHERE (a.sch_role_cd=t_record->qual[i].sch_role_cd)
    DETAIL
 ...
 ENDFOR
 SELECT INTO "nl:"
  a.resource_cd, b.mnemonic
  FROM (dummyt d  WITH seq = value(t_record->qual_cnt)),
   sch_res_role a,
   sch_resource b
  PLAN (d)
   JOIN (a
   WHERE (a.sch_role_cd=t_record->qual[d.seq].sch_role_cd)
    AND a.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
   JOIN (b
   WHERE b.resource_cd=a.resource_cd
    AND b.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
  ORDER BY d.seq, b.mnemonic_key
  HEAD d.seq
   t_record->qual[d.seq].resource_qual_cnt = 0
  DETAIL
   t_record->qual[d.seq].resource_qual_cnt = (t_record->qual[d.seq].resource_qual_cnt+ 1)
   IF (mod(t_record->qual[d.seq].resource_qual_cnt,10)=1)
    stat = alterlist(t_record->qual[d.seq].resource_qual,(t_record->qual[d.seq].resource_qual_cnt+ 9)
     )
   ENDIF
   t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt].resource_cd = a
   .resource_cd, t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt].
   mnemonic = b.mnemonic, t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt
   ].active_status_cd = a.active_status_cd
   IF ((t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt].active_status_cd
    > 0))
    t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt].active_status_desc
     = uar_get_code_display(a.active_status_cd)
    IF ((t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt].
    active_status_desc <= " "))
     t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt].active_status_desc
      = concat("Code_value ",trim(format(t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].
        resource_qual_cnt].active_status_cd,";l;i"))," not found")
    ENDIF
   ELSE
    t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt].active_status_desc
     = "NOT ASSOCIATED"
   ENDIF
   t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt].active_status_size =
   size(trim(t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt].
     active_status_desc))
   IF ((t_record->qual[d.seq].resource_qual[t_record->qual[d.seq].resource_qual_cnt].
   active_status_size > t_record->qual[d.seq].max_active_size))
    t_record->qual[d.seq].max_active_size = t_record->qual[d.seq].resource_qual[t_record->qual[d.seq]
    .resource_qual_cnt].active_status_size
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE (a.sch_role_cd=t_record->qual[d.seq].sch_role_cd)
    AND a.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
   JOIN (b
   WHERE b.res_list_id=a.res_list_id
    AND b.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
  ORDER BY d.seq, b.mnemonic_key
  HEAD d.seq
   t_record->qual[d.seq].list_qual_cnt = 0
  DETAIL
   t_record->qual[d.seq].list_qual_cnt = (t_record->qual[d.seq].list_qual_cnt+ 1)
   IF (mod(t_record->qual[d.seq].list_qual_cnt,10)=1)
    stat = alterlist(t_record->qual[d.seq].list_qual,(t_record->qual[d.seq].list_qual_cnt+ 9))
   ENDIF
   t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].res_list_id = b.res_list_id,
   t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].mnemonic = b.mnemonic,
   t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].list_role_id = a.list_role_id,
   t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].list_role_mnemonic = a
   .description, t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].
   active_status_cd = a.active_status_cd
   IF ((t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].active_status_cd > 0))
    t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].active_status_desc =
    uar_get_code_display(a.active_status_cd)
    IF ((t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].active_status_desc <=
    " "))
     t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].active_status_desc = concat
     ("Code_value ",trim(format(t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].
        active_status_cd,";l;i"))," not found")
    ENDIF
   ELSE
    t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].active_status_desc =
    "NOT ASSOCIATED"
   ENDIF
   t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].active_status_size = size(
    trim(t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].active_status_desc))
   IF ((t_record->qual[d.seq].list_qual[t_record->qual[d.seq].list_qual_cnt].active_status_size >
   t_record->qual[d.seq].max_active_size))
    t_record->qual[d.seq].max_active_size = t_record->qual[d.seq].list_qual[t_record->qual[d.seq].
    list_qual_cnt].active_status_size
   ENDIF
  WITH nocounter
 ;end select
 CALL echo(build("t_record->qual_cnt = ",t_record->qual_cnt))
 FOR (i = 1 TO t_record->qual_cnt)
   CALL echo(build("   t_record->qual[",i,"]->sch_role_cd = ",t_record->qual[i].sch_role_cd))
   CALL echo(build("   t_record->qual[",i,"]->role_meaning = ",t_record->qual[i].role_meaning))
   ...
   CALL echo(build("   t_record->qual[",i,"]->resource_qual_cnt = ",t_record->qual[i].
     resource_qual_cnt))
   FOR (j = 1 TO t_record->qual[i].resource_qual_cnt)
     CALL echo(build("   t_record->qual[",i,",",j,"]->resource_cd = ",
       t_record->qual[i].resource_qual[j].resource_cd))
     CALL echo(build("   t_record->qual[",i,",",j,"]->mnemonic = ",
       t_record->qual[i].resource_qual[j].mnemonic))
     CALL echo(build("   t_record->qual[",i,",",j,"]->active_status_cd = ",
       t_record->qual[i].resource_qual[j].active_status_cd))
     CALL echo(build("   t_record->qual[",i,",",j,"]->active_status_desc = ",
       t_record->qual[i].resource_qual[j].active_status_desc))
   ENDFOR
   CALL echo(build("   t_record->qual[",i,"]->list_qual_cnt = ",t_record->qual[i].list_qual_cnt))
   FOR (j = 1 TO t_record->qual[i].list_qual_cnt)
     CALL echo(build("   t_record->qual[",i,",",j,"]->res_list_id = ",
       t_record->qual[i].list_qual[j].res_list_id))
     CALL echo(build("   t_record->qual[",i,",",j,"]->mnemonic = ",
       t_record->qual[i].list_qual[j].mnemonic))
     CALL echo(build("   t_record->qual[",i,",",j,"]->list_role_id = ",
       t_record->qual[i].list_qual[j].list_role_id))
     CALL echo(build("   t_record->qual[",i,",",j,"]->list_role_mnemonic = ",
       t_record->qual[i].list_qual[j].list_role_mnemonic))
     CALL echo(build("   t_record->qual[",i,",",j,"]->active_status_cd = ",
       t_record->qual[i].list_qual[j].active_status_cd))
     CALL echo(build("   t_record->qual[",i,",",j,"]->active_status_desc = ",
       t_record->qual[i].list_qual[j].active_status_desc))
   ENDFOR
   CALL echo(build("   t_record->qual[",i,"]->sch_booking_exists = ",t_record->qual[i].
     sch_booking_exists))
   CALL echo(build("   t_record->qual[",i,"]->sch_appt_exists = ",t_record->qual[i].sch_appt_exists))
   CALL echo(" ")
 ENDFOR
 FOR (i = 1 TO t_record->qual_cnt)
   ...
   FOR (j = 1 TO format_text_reply->qual_cnt)
    ...
    ENDIF
   ENDFOR
   SET format_text_request->raw_text = t_record->qual[i].role_meaning
   SET format_text_request->chars_per_line = t_record->field_column_size
   CALL format_text(1)
   FOR (j = 1 TO format_text_reply->qual_cnt)
    CALL inc_data(1)
   ...
   FOR (j = 1 TO format_text_reply->qual_cnt)
    ...
    ELSE
     SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
      substring(1,t_record->max_prompt_size,t_record->prompt_qual[4].prompt_string),trim(
       format_text_reply->qual[j].text_string))
    ENDIF
   ENDFOR
   CALL inc_data(1)
   ...
   CALL inc_data(1)
   SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
    substring(1,t_record->max_prompt_size,t_record->prompt_qual[6].prompt_string),substring(1,
   ...
   SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
    substring(1,t_record->max_prompt_size,t_record->prompt_qual[7].prompt_string),evaluate(t_record->
     qual[i].resource_qual_cnt,0,"NOT ASSOCIATED",concat("Mnemonic                           Status",
      substring(1,maxval(0,(t_record->qual[i].max_active_size - 6)),t_record->blank_line))))
   FOR (i_resource = 1 TO t_record->qual[i].resource_qual_cnt)
     IF (i_resource=1)
      CALL inc_data(1)
      SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat
     ...
     CALL inc_data(1)
     SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
      substring(1,t_record->max_prompt_size,t_record->blank_line),t_record->qual[i].resource_qual[
      i_resource].mnemonic,"     ",substring(1,t_record->qual[i].max_active_size,t_record->qual[i].
       resource_qual[i_resource].active_status_desc))
   ENDFOR
   IF ((t_record->qual[i].resource_qual_cnt > 0)
    AND (t_record->qual[i].list_qual_cnt > 0))
    CALL inc_data(1)
   ...
   SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
    substring(1,t_record->max_prompt_size,t_record->prompt_qual[8].prompt_string),evaluate(t_record->
     qual[i].list_qual_cnt,0,"NOT ASSOCIATED",concat("Mnemonic                           Status",
      substring(1,maxval(0,(t_record->qual[i].max_active_size - 6)),t_record->blank_line),
      "     Role Description                   ")))
   FOR (i_list = 1 TO t_record->qual[i].list_qual_cnt)
     IF (i_list=1)
      CALL inc_data(1)
      SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat
      (substring(1,t_record->max_prompt_size,t_record->blank_line),
     ...
     CALL inc_data(1)
     SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
      substring(1,t_record->max_prompt_size,t_record->blank_line),t_record->qual[i].list_qual[i_list]
      .mnemonic,"     ",substring(1,t_record->qual[i].max_active_size,t_record->qual[i].list_qual[
       i_list].active_status_desc),"     ",
      t_record->qual[i].list_qual[i_list].list_role_mnemonic,"     ")
   ENDFOR
   CALL inc_data(1)
   SET generic_print_request->data_qual[generic_print_request->data_qual_cnt].data_string = concat(
