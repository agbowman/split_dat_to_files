CREATE PROGRAM dm_ecmb_ce_io_total_result:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_CE_IO_TOTAL_RESULT"
 ...
 ELSE
  ...
  IF (v_cust_count1 > 0)
   ...
   FOR (v_cust_loopcount = 1 TO v_cust_count2)
     IF (upt_to(rreclist->to_rec[v_cust_count2].to_id)=0)
      GO TO exit_sub
     ENDIF
   ENDFOR
   FOR (v_cust_loopcount = 1 TO v_cust_count1)
     IF (upt_from(rreclist->from_rec[v_cust_loopcount].from_id,request->xxx_combine[icombine].
      to_xxx_id,rreclist->from_rec[v_cust_loopcount].suspect_flag)=0)
      GO TO exit_sub
     ENDIF
   ENDFOR
  ENDIF
 ...
 SUBROUTINE upt_from(s_uf_pk_id,s_uf_to_fk_id,suspect_flag)
   UPDATE  FROM ce_io_total_result frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ->updt_applctx,
     frm.updt_task = reqinfo->updt_task, frm.updt_dt_tm = cnvtdatetime(sysdate), frm.suspect_flag =
     suspect_flag,
     frm.encntr_id = s_uf_to_fk_id
