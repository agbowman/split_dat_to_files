CREATE PROGRAM dm2_aload_dtr
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dbis_ins_cols = vc WITH protect, noconstant("")
 DECLARE dbis_adm_col_str = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_aload_dtr",".log","dm2_aload_dtr")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dtr"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Remove DM_TABLE_RELATIONSHIPS rows with lower or equal table_instance."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 DELETE  FROM dm_table_relationships a
  WHERE a.table_name IN (
  (SELECT DISTINCT
   cv.table_name
   FROM dm_table_relationships_gttp cv
   WHERE cv.table_name=a.table_name
    AND cv.table_instance >= a.table_instance))
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Insert higher or equal instance rows into DM_TABLE_RELATIONSHIPS."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 FOR (dbis_cnt = 1 TO size(dac_col_list->col,5))
  SET dbis_ins_cols = concat(dbis_ins_cols,dac_col_list->col[dbis_cnt].col_name)
  IF (dbis_cnt != size(dac_col_list->col,5))
   SET dbis_ins_cols = concat(dbis_ins_cols,",")
  ENDIF
 ENDFOR
 SET dbis_ins_cols = check(dbis_ins_cols)
 SET dbis_adm_col_str = trim(replace(dbis_ins_cols,"UPDT_DT_TM","SYSDATE",2),3)
 SET dbis_adm_col_str = trim(replace(dbis_adm_col_str,"UPDT_APPLCTX",cnvtstring(reqinfo->updt_applctx
    ),2),3)
 SET dbis_adm_col_str = trim(replace(dbis_adm_col_str,"UPDT_ID",cnvtstring(reqinfo->updt_id),2),3)
 SET dbis_adm_col_str = trim(replace(dbis_adm_col_str,"UPDT_TASK",cnvtstring(reqinfo->updt_task),2),3
  )
 SET dbis_adm_col_str = trim(replace(dbis_adm_col_str,"UPDT_CNT","0",2),3)
 CALL dm2_push_cmd(concat("rdb insert into DM_TABLE_RELATIONSHIPS dcv "),0)
 CALL dm2_push_cmd(concat("  ( "),0)
 CALL dm2_push_cmd(concat(" ",dbis_ins_cols," "),0)
 CALL dm2_push_cmd(concat("  )( select "),0)
 CALL dm2_push_cmd(concat(" ",dbis_adm_col_str," "),0)
 CALL dm2_push_cmd(concat("  from DM_TABLE_RELATIONSHIPS_GTTP cv  "),0)
 CALL dm2_push_cmd(concat(" where cv.table_name not in "),0)
 CALL dm2_push_cmd(concat(" (select distinct(a.table_name) from DM_TABLE_RELATIONSHIPS a)) "),0)
 CALL dm2_push_cmd(concat("  go "),1)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  GO TO exit_program
 ...
 COMMIT
#exit_program
 SET dm_err->eproc = "Truncating table DM_TABLE_RELATIONSHIPS_GTTP"
 CALL dm2_push_cmd("rdb truncate table DM_TABLE_RELATIONSHIPS_GTTP go",1)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dtr completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_dtr")
END GO
