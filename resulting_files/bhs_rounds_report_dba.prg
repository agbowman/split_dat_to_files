CREATE PROGRAM bhs_rounds_report:dba
 FREE RECORD dlrec
 RECORD dlrec(
   1 encntr_total = i4
   1 seq[*]
 ...
 DECLARE mode_of_delivery_o2_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
 ...
 ELSE
  ...
  SET printer_disp =  $1
  SET dlrec->seq[1].encntr_id = 29100757
 ENDIF
 SELECT INTO "nl:"
  e.encntr_id
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dd)
   ...
   o2_sat_cd, mode_of_delivery_o2_cd, weight_cd)
  ...
  HEAD ce.encntr_id
   ...
   ENDIF
   stat = alterlist(dlrec->seq[dd.seq].weights,3), low_temp_result = fillstring(10," "),
   high_temp_result = fillstring(10," "),
   low_pulse_result = fillstring(10," "), high_pulse_result = fillstring(10," "), low_sbp_result =
   fillstring(10," "),
   high_sbp_result = fillstring(10," "), low_dbp_result = fillstring(10," "), high_dbp_result =
   fillstring(10," "),
   low_rr_result = fillstring(10," "), high_rr_result = fillstring(10," "), low_o2sat_result =
   fillstring(10," "),
   high_o2sat_result = fillstring(10," ")
  HEAD ce.event_cd
  ...
  DETAIL
   ...
   ELSEIF (ce.event_cd=weight_cd)
    ...
    IF (weight_cnt <= 3)
     dlrec->seq[dd.seq].weights[weight_cnt].weight_unit = trim(uar_get_code_display(ce
       .result_units_cd)), dlrec->seq[dd.seq].weights[weight_cnt].weight_value = trim(format(cnvtreal
       (ce.result_val),"###.##")), dlrec->seq[dd.seq].weights[weight_cnt].weight_dt_tm = trim(format(
  ...
  FOOT  ce.encntr_id
   ...
   IF (weight_cnt < 3)
    stat = alterlist(dlrec->seq[dd.seq].weights,weight_cnt)
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  PLAN (dd)
   ...
   WHERE ce.order_id=o.order_id
    ...
    AND  NOT (ce.result_status_cd IN (inerror_cd, notdone_cd, pending_cd))
    AND ce.result_val > " ")
  ORDER BY ce.encntr_id, ce.order_id DESC, ce.event_id
  HEAD ce.encntr_id
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dd)
   ...
   WHERE sn.parent_entity_name="PERSON"
    ...
    AND sn.sticky_note_type_cd=rounds_note_cd
 ...
 SELECT INTO value(printer_disp)
  ...
  HEAD encntr_id
   ...
   IF ((dlrec->seq[d1.seq].total_vitals > 0))
    ...
    dlrec->seq[d1.seq].vitals[1].mode_of_delivery, ") ", ycol = (ycol+ 8),
    row + 1
   ELSE
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
   ...
   xcol = 30, row + 1,
   CALL print(calcpos(xcol,ycol)),
   "{b}{u}Rounds Notes{endb}{endu}", ycol = (ycol+ 8), row + 1
   ...
   ELSE
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "No Rounds notes found on patient.", ycol = (ycol+ 8), row + 1
   ENDIF
  FOOT  encntr_id
   IF ((req2->pagebreak=2))
    BREAK
