CREATE PROGRAM dm2_run_data_movers_asynch:dba
 ...
 SET appid = 5000
 SET taskid = 5000
 DECLARE drdma_logfile = vc WITH protect, noconstant(" ")
 IF ((dm_err->logfile IN ("NONE", "DM2_LOGFILE_NOTSET")))
  SET drdma_logfile = dm_err->logfile
  SET dm_err->logfile = "DM2_LOGFILE_NOTSET"
 ELSE
  SET drdma_logfile = dm_err->logfile
 ENDIF
 SET dm_err->eproc = "Beginning dm2_run_data_movers_asynch"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET reqid = 4410008
 ...
 IF (happ=0)
  ...
  IF (iret != 0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("return value(App) =",iret)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO end_script
  ENDIF
 ENDIF
 SET dm_err->eproc = build("Application Handle is: ",happ)
 CALL disp_msg(" ",dm_err->logfile,0)
 SET iret = uar_crmbegintask(happ,taskid,htask)
 IF (((iret != 0) OR (htask=0)) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = build("return value(Task) =",iret)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_script
 ENDIF
 SET dm_err->eproc = build("Task Handle is: ",htask)
 CALL disp_msg(" ",dm_err->logfile,0)
 SET iret = uar_crmbeginreq(htask,0,reqid,hreq)
 IF (((iret != 0) OR (hreq=0)) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = build("return value(Req) =",iret)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_script
 ENDIF
 SET dm_err->eproc = build("Request Handle is: ",hreq)
 CALL disp_msg(" ",dm_err->logfile,0)
 SET iret = uar_crmperform(hreq)
 IF (iret != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = build("return value(Perform) =",iret)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_script
 ENDIF
#end_script
 SET dm_err->eproc = "clear up the memory usage"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (hreq > 0)
 ...
 ENDIF
 IF (happ > 0)
  SET iret = uar_crmendapp(happ)
 ENDIF
 SET dm_err->eproc = "...Ending dm2_run_data_movers_asynch"
 CALL final_disp_msg(" ")
 SET dm_err->logfile = drdma_logfile
END GO
