CREATE PROGRAM dm_starter_cv_extension_mpd:dba
 SELECT INTO dm_starter_cv_extension_mpd
  d.*
  FROM dual d
  DETAIL
 ...
 IF (trim(ref)="C")
  ...
  FOR (cnt = 1 TO list->count)
    ...
    SELECT INTO dm_starter_cv_extension_mpd
     ...
     DETAIL
      ...
      row + 1, "1 UPDT_DT_TM = dq8", row + 1,
      "1 UPDT_ID = f8", row + 1, "1 UPDT_CNT = i4",
      row + 1, "1 UPDT_APPLCTX = i4", row + 1,
      "1 UPDT_TASK = f8", row + 1, ")",
      row + 1, "go", row + 1,
      ...
      row + 1, tempstr = build('set dmrequest->schema_date = cnvtdatetime("',dcv.schema_date,'") go'),
      tempstr,
  ...
  SET cnt = 0
  FOR (cnt = 1 TO list->count)
    UPDATE  FROM dm_feature_code_sets_env dmf
     SET dmf.code_set_env_status = "1"
     WHERE (dmf.code_set=list->qual[cnt].code_set)
      AND dmf.environment=env
      AND (dmf.feature_number=
     (SELECT
      df.feature_number
      FROM dm_features df
      WHERE df.feature_status=ref_status))
     WITH nocounter
    ;end update
  ENDFOR
 ELSE
  ...
  SELECT INTO "NL:"
   dcf.schema_date
   FROM dm_code_value_extension dcf
  ...
  FOR (cnt = 1 TO list->count)
   ...
   SELECT INTO dm_starter_cv_extension_mpd
    ...
    DETAIL
     ...
     "1 active_ind = i2", row + 1, "1 field_type = i4",
     ...
     " go", row + 1, tempstr = "set dmrequest->field_name =  ",
