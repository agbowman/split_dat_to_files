CREATE PROGRAM ecf_rdm_icd10vemd_con_rel_ext:dba
 ...
 SUBROUTINE kia_script_inc(pkg,ccl_obj)
   ...
   CALL parser(obj_prs)
 END ;Subroutine
 DECLARE kia_concept_filename = vc WITH public, constant("icd10vemd_concept.csv")
 DECLARE kia_reltn_filename = vc WITH public, constant("icd10vemd_relation.csv")
 DECLARE kia_ext_csv_filename = vc WITH public, constant("icd10vemd_cmt_extension.csv")
 DECLARE kia_ext10_csv_filename = vc WITH public, constant("icd10vemd_icd_extension.csv")
 DECLARE kia_cur_ver_beg_dt = vc WITH public, noconstant("")
 SET kia_tbl_desc = "ICD10-AMVEMD concept reltn"
 SET kia_short_desc = "Load the ICD10-AM VEMD Content"
 SET kia_rdm_log_name = "ecf_rdm_icd10vmed_con_rel_ext.log"
 SET kia_rdm_nbr = 9952
 SET kia_rdm_vocab_mean = "ICD10-AMVEMD"
 SET readme_data->status = "F"
 ...
 EXECUTE kia_dm_dbimport "cer_install:icd10vemd_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET readme_data->status = "F"
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("kia_rdm_icd10amvemd Begin> ",format(cnvtdatetime(curdate,curtime3),
 ...
 IF (kia_obj_suc_ind=0)
  ...
  DELETE  FROM cmt_concept_load_ns c
   WHERE ((c.concept_cki="ICD10-AMVEMD*") OR (c.concept_cki="SNOMED*"))
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
 ...
 IF (kia_obj_suc_ind=0)
  ...
  DELETE  FROM cmt_concept_reltn_ld_ns c
   WHERE c.concept_cki1="ICD10-AMVEMD*"
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
 ...
 IF (kia_obj_suc_ind != 1)
  ...
  DELETE  FROM cmt_concept_extension c
   WHERE c.concept_cki="ICD10-AMVEMD*"
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_ext_csv_filename,"kia_imp_cmt_concept_ext","kia_imp_icd10-amvemd_ext.log")
  CALL kia_set_status_success("kia_imp_cmt_concept_ext")
 ENDIF
 CALL parser("rdb truncate table icd10_extension reuse storage go")
 CALL kia_load_file(kia_ext10_csv_filename,"kia_imp_icd10_ext","kia_imp_icd10_ext.log")
 CALL kia_chk_error(1)
