CREATE PROGRAM dm_rebuild_ea_triggers:dba
 ...
 DECLARE get_trig_name(null) = i4 WITH protect
 DECLARE select_triggers(null) = i4 WITH protect
 DECLARE check_sequence(null) = i4 WITH protect
 RECORD trig(
   1 qual[*]
     2 table_name = vc
     2 trigger_name = vc
 ...
 SET stat = check_logfile("dm_ea_trg",".log","DM_REBUILD_EA_TRIGGERS Begin")
 IF ( NOT (get_trig_name(null)))
  SET readme_data->status = "F"
  SET readme_data->message = "Readme Failed -No rows selected from dm_entity_activity_trigger table"
  GO TO exit_script
 ENDIF
 IF (select_triggers(null)=2)
  IF (check_sequence(null))
   EXECUTE dm_build_ea_triggers
   GO TO exit_script
  ELSE
   GO TO exit_script
  ENDIF
 ELSEIF (select_triggers(null)=1)
  SET readme_data->status = "S"
  SET readme_data->message = "Readme Successful - No triggers to update"
  GO TO exit_script
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = "Readme Failed - Error selecting triggers"
  GO TO exit_script
 ENDIF
 SUBROUTINE get_trig_name(null)
   DECLARE gtn_tab_cnt = i4
   DECLARE gtn_len = i4
   SET gtn_trg_name = fillstring(35," ")
   SET gtn_t_name = fillstring(35," ")
   SELECT INTO "nl:"
    ea.table_name
    ...
    HEAD REPORT
     gtn_tab_cnt = 0
    DETAIL
     gtn_tab_cnt = (gtn_tab_cnt+ 1)
     IF (mod(gtn_tab_cnt,10)=1)
      stat = alterlist(trig->qual,(gtn_tab_cnt+ 9))
     ENDIF
     trig->qual[gtn_tab_cnt].table_name = ea.table_name
     IF (currdb="ORACLE")
      trig->qual[gtn_tab_cnt].trigger_name = cnvtupper(concat(trim(substring(1,28,concat(trim(
            substring(1,27,concat("TRG",trim(trig->qual[gtn_tab_cnt].table_name,3))),3),"_EA")),3),
        "*"))
     ELSEIF (currdb="DB2UDB")
      gtn_len = textlen(trim(trig->qual[gtn_tab_cnt].table_name,3)), gtn_trg_name = concat("trg",trim
       (substring((gtn_len - 3),gtn_len,trig->qual[gtn_tab_cnt].table_name),3),"_EA"), trig->qual[
      gtn_tab_cnt].trigger_name = cnvtupper(concat(trim(gtn_trg_name,3),"*"))
     ENDIF
    FOOT REPORT
     stat = alterlist(trig->qual,gtn_tab_cnt)
    WITH nocounter
   ;end select
   IF (gtn_tab_cnt > 0)
    RETURN(1)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE select_triggers(null)
   SET errmsg = fillstring(132," ")
   IF (currdb="ORACLE")
    FOR (st_cnt = 1 TO size(trig->qual,5))
     SELECT INTO "nl:"
      u.trigger_name
      FROM user_triggers u
      WHERE u.trigger_name=patstring(trig->qual[st_cnt].trigger_name)
   ...
   ELSEIF (currdb="DB2UDB")
    FOR (st_cnt = 1 TO size(trig->qual,5))
     SELECT INTO "nl:"
      ...
      WHERE dm.trigger_name=patstring(trig->qual[st_cnt].trigger_name)
      WITH nocounter
     ;end select
     IF (error(errmsg,1))
      RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE check_sequence(null)
   DECLARE last_number = f8
   DECLARE max_number = f8
   SELECT INTO "nl:"
    s.last_number
    FROM dm2_user_sequences s
    WHERE s.sequence_name="DM_CLINICAL_SEQ"
    DETAIL
   ...
   IF (curqual=0)
    SET readme_data->status = "F"
    SET readme_data->message = "Readme failed - DM_CLINICAL_SEQ doesn't exist"
    RETURN(0)
   ENDIF
   SELECT INTO "nl:"
    max_nbr = max(e.entity_activity_id)
    FROM dm_entity_activity e
    DETAIL
     max_number = max_nbr
    WITH nocounter
   ;end select
   IF (max_number > last_number)
    SET max_number = (max_number+ 10000)
    CALL parser("rdb drop sequence dm_clinical_seq go")
    EXECUTE dm_add_sequence "dm_clinical_seq", value(max_number), 0,
    0, 0
    COMMIT
