CREATE PROGRAM dm_rmc_mover_3292:dba
 ...
 DECLARE cust_seg_cd_to = f8
 DECLARE cust_iur_context_name = vc
 DECLARE cust_from_cd = vc
 DECLARE cust_iur_next_log = i4
 DECLARE cust_err_val = i4
 DECLARE cust_seg_grp_cd = vc WITH protect, noconstant(" ")
 DECLARE cust_par_tab_name = vc WITH protect, noconstant(" ")
 DECLARE cust_from_seg_cd = vc WITH protect, noconstant(" ")
 DECLARE cust_to_val = vc WITH protect, noconstant(" ")
 DECLARE cust_rpt_missing = vc WITH protect, noconstant(" ")
 DECLARE cust_ptam_result = f8 WITH protect, noconstant(0.0)
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_par_tab_name = dm2_get_rdds_tname("SEGMENT_REFERENCE")
 SET dm_err->eproc = "Beginning dm_rmc_mover_3292"
 SET cust_from_seg_cd = get_value(cust_tab_name,"SEG_CD","FROM")
 SET dm_err->eproc = "Querying parent for seg_grp_cd"
 SELECT INTO "nl:"
  FROM (parser(cust_par_tab_name) t)
  WHERE seg_cd=cnvtreal(cust_from_seg_cd)
  DETAIL
   cust_seg_grp_cd = trim(cnvtstring(t.seg_grp_cd,20,2))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  GO TO exit_3292
 ENDIF
 SET cust_to_val = select_merge_translate(cust_seg_grp_cd,"CODE_VALUE")
 IF (drdm_debug_row_ind > 0)
  CALL echo(concat("CUST_TO_VAL",cust_to_val))
 ENDIF
 IF ( NOT (isnumeric(cust_to_val)))
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "SEG_GRP_CD could not be translated."
  SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_seg_grp_cd))
  CALL evaluate_rpt_missing(cust_rpt_missing)
  GO TO exit_3292
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"SEG_GRP_SEQ_R_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"SEG_GRP_SEQ_R_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"SEG_GRP_SEQ_R_ID",cnvtstring(cust_val,20,1))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  SEG_GRP_SEQ_R_ID")
     ELSE
      GO TO exit_3292
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"SEG_GRP_SEQ_R_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_3292
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_id = get_value(cust_tab_name,"SEG_GRP_SEQ_R_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_3292
   ELSE
 ...
 IF (cust_idcd_check=1)
  IF ((drdm_chg->log[drdm_log_loop].md_delete_ind=1))
   CALL drmm_get_ptam_match_query(drdm_chg->log[drdm_log_loop].log_id,dm2_ref_data_doc->tbl_qual[
    temp_tbl_cnt].table_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,dm2_ref_data_doc->
    post_link_name)
   SET cust_ptam_result = drmm_get_ptam_match_result(0.0,dm2_ref_data_doc->env_source_id,
    dm2_ref_data_doc->env_target_id,dm2_ref_data_doc->post_link_name,0,
    "FROM")
   SET dm_err->eproc = "Get context to from seg_grp_seq_r$R"
   SELECT DISTINCT INTO "nl:"
    rdds_context_name
    FROM seg_grp_seq_r3292$r r
    WHERE r.rdds_status_flag < 9000
     AND (r.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
     AND r.rdds_ptam_match_result=cust_ptam_result
    DETAIL
     cust_iur_context_name = r.rdds_context_name
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_3292
   ENDIF
   SET drdm_chg->log[drdm_log_loop].md_context_name = evaluate_context_name(cust_iur_context_name,
    drdm_chg->log[drdm_log_loop].context_name)
   SET cust_iur_next_log = drdm_chg->log[drdm_log_loop].next_to_process
   WHILE (cust_iur_next_log != 0
    AND  NOT ((drdm_chg->log[drdm_log_loop].process_flg IN (4, 5))))
    IF ((drdm_chg->log[cust_iur_next_log].table_name=cust_tab_name))
     SET drdm_chg->log[cust_iur_next_log].md_context_name = drdm_chg->log[drdm_log_loop].
     md_context_name
    ENDIF
    SET cust_iur_next_log = drdm_chg->log[cust_iur_next_log].next_to_process
   ENDWHILE
   SET dm_err->eproc = "Performing delete on SEG_GRP_SEQ_R3292$R"
   DELETE  FROM seg_grp_seq_r3292$r r
    WHERE r.rdds_status_flag < 9000
     AND (r.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
     AND r.rdds_ptam_match_result=cust_ptam_result
    WITH nocounter
  ...
  ENDIF
  SET cust_new_id = get_value(cust_tab_name,"SEG_GRP_SEQ_R_ID","TO")
  SET dm_err->eproc = "Performing delete on current row"
  DELETE  FROM seg_grp_seq_r3292$r r
   WHERE r.rdds_status_flag < 9000
    AND (r.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
    AND r.seg_grp_seq_r_id=cnvtreal(concat(cust_new_id,".0"))
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSE
  ...
  IF (cust_err_val=0)
   ...
   SET dm2_ref_data_reply->error_msg = concat(
    "This row wasn't translated because not all columns were translated.")
