CREATE PROGRAM bhs_sfe_outcomes_summary9:dba
 PROMPT
 ...
 DECLARE expand_stop = i4 WITH noconstant(150), protect
 CALL echo("2")
 ...
 SELECT INTO "NL:"
  ...
  PLAN (dfr
   ...
   WHERE dfa.dcp_forms_ref_id=dfr.dcp_forms_ref_id
    ...
    AND ((dfa.beg_activity_dt_tm+ 0) < cnvtdatetime(end_date_qual))
    AND ((dfa.last_activity_dt_tm+ 0) > cnvtdatetime(beg_date_qual)))
   JOIN (e
  ...
  HEAD dfa.encntr_id
   ...
   ENDIF
   formactlist->qual[countl].encntr_id = dfa.encntr_id, formactlist->qual[countl].person_id = dfa
   .person_id
  FOOT REPORT
 ...
 IF (mod(actual_size,150) != 0)
  SET expand_total = (actual_size+ (150 - mod(actual_size,150)))
  ...
  FOR (idx = (actual_size+ 1) TO expand_total)
   ...
   SET formactlist->qual[countl].person_id = formactlist->qual[countl].person_id
  ENDFOR
 ELSE
 ...
 SELECT INTO "NL:"
  ...
  FROM clinical_event ce,
   person p,
   encntr_alias ea,
   (dummyt d  WITH seq = value((expand_total/ 150)))
  PLAN (d
   WHERE assign(expand_start,evaluate(d.seq,1,1,(expand_start+ 150)))
    AND assign(expand_stop,(expand_start+ 149)))
   JOIN (ce
   WHERE expand(num,expand_start,expand_stop,ce.person_id,formactlist->qual[num].person_id)
    AND ce.event_cd IN (smokingcessation, assessmentstage, pharmacotherapypastuse, healthrisks,
   yearssmoking,
   ...
   smokinghistoryandmanagementform, smokingcessationresourcesinformation)
    AND ce.event_end_dt_tm > cnvtdatetime(beg_date_qual)
  ...
  HEAD REPORT
   stat = alterlist(sfeinfo->detailqual,100), count = 0, pos = 0,
   locnum = 0
  HEAD ce.person_id
   ...
   .encntr_id
  DETAIL
   pos = locateval(locnum,1,size(formlist->formqual,5),ce.encntr_id,formactlist->qual[testcnt].
    encntr_id)
   IF (pos > 0)
    sfecnt = (sfecnt+ 1), stat = alterlist(sfeinfo->qual,sfecnt), sfeinfo->qual[sfecnt].person_id =
    ce.person_id,
 ...
 SELECT INTO "NL:"
  ...
  PLAN (d)
   ...
   WHERE (o.encntr_id=sfeinfo->detailqual[d.seq].encntr_id)
    ...
    AND ((o.catalog_cd+ 0) IN (patientwantsreferralquitsmokingtask,
   patientwantsreferraltoquitsmokingtask, cpgnicotinereplacementtherapy, nicotine, bupropion,
   varenicline))
