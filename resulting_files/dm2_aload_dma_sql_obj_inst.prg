CREATE PROGRAM dm2_aload_dma_sql_obj_inst
 ...
 SUBROUTINE dad_get_admin_conn(i_conn_info)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dadsoi_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dadsoi_file = vc WITH protect, noconstant(trim( $2,3))
 DECLARE dadsoi_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE dadsoi_batch_num = i4 WITH protect, noconstant(10000)
 DECLARE dadsoi_file_path = vc WITH protect, noconstant(" ")
 DECLARE dadsoi_table_name = vc WITH protect, constant("DMA_DSOI_STAGE")
 DECLARE dadsoi_admin_conn = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm2_aload_dsoi",".log","DM2_ALOAD_DSOI LogFile...") != 1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dma_sql_obj_inst"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dadsoi_expected_rows > 0)
  IF (dac_prelim(dadsoi_pkg,dadsoi_file_path,dadsoi_batch_num)=0)
   GO TO exit_program
  ENDIF
  IF (dad_get_admin_conn(dadsoi_admin_conn) != 1)
   GO TO exit_program
  ENDIF
  EXECUTE dm2_exe_sqlldr dadsoi_file, dadsoi_file_path, dadsoi_table_name,
  dadsoi_admin_conn
  IF ((dm_err->err_ind=1))
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dma_sql_obj_inst completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_dma_sql_obj_inst")
END GO
