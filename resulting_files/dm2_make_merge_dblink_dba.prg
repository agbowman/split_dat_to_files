CREATE PROGRAM dm2_make_merge_dblink:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 CALL check_logfile("dm2_make_mrg_dblink",".log","DM2_MAKE_MERGE_DBLINK")
 SET dm_err->eproc = "Beginning dm2_make_merge_dblink"
 CALL disp_msg("dm2_make_merge_dblink ",dm_err->logfile,0)
 DECLARE dmmd_dblink_name = vc WITH protect
 DECLARE dmmd_tnsname = vc WITH protect
 DECLARE dmmd_username = vc WITH protect
 DECLARE dmmd_pwd = vc WITH protect
 DECLARE dmmd_node = vc WITH protect
 DECLARE dmmd_drop_db = vc WITH protect
 DECLARE dmmd_dblink = vc WITH protect
 DECLARE dmmd_err_code = i2 WITH protect, noconstant(0)
 DECLARE nick_cnt = i4 WITH protect, noconstant(0)
 DECLARE user_src_tbl = vc WITH protect
 DECLARE c_nick_stat = vc WITH protect
 DECLARE c_server_stat = vc WITH protect
 DECLARE errmsg_server = vc WITH protect
 SET dmmd_username = trim( $3)
 SET dmmd_pwd = trim( $4)
 IF (currdb="ORACLE")
  SET dmmd_dblink_name = trim( $1)
  SET dmmd_tnsname = trim( $2)
  SET dmmd_drop_db = concat("rdb drop database link ",dmmd_dblink_name," go")
  SELECT INTO "nl:"
   FROM dba_db_links d
   WHERE findstring(cnvtupper(dmmd_dblink_name),d.db_link) > 0
   WITH nocounter
  ;end select
  IF (curqual)
   CALL echo("dropping duplicate database link")
   SET dmmd_err_code = dm2_push_cmd(dmmd_drop_db,1)
   IF (dmmd_err_code=0)
    GO TO exit_program
   ENDIF
  ENDIF
  SET dmmd_dblink = concat("create database link ",dmmd_dblink_name," connect to ",dmmd_username,
   ' identified by "',
   dmmd_pwd,'" using ',"'",dmmd_tnsname,"'")
  CALL parser(concat("rdb asis(^",dmmd_dblink,"^) go"),1)
  IF (dmmd_err_code=0)
   GO TO exit_program
  ELSE
   CALL echo("Confirming the new database link...")
   SET dmda_confirm = concat("select into 'nl:' name from v$database@",dmmd_dblink_name,
    " with nocounter go")
   CALL echo(dmda_confirm)
   SET dmmd_err_code = dm2_push_cmd(dmda_confirm,1)
   IF (dmmd_err_code=0)
    GO TO exit_program
   ENDIF
  ENDIF
 ELSE
  ...
  SET dm2_server_link->wrapper = "DRDA"
  SET dm2_server_link->server_name = dmmd_server
  SET dm2_server_link->drop_server_ind = 0
  SET dm2_server_link->server_rdbms = "DB2"
  SET dm2_server_link->server_type = "DB2/6000"
  SET dm2_server_link->server_version = "8.1"
  SET dm2_server_link->user = dmmd_username
  SET dm2_server_link->password = dmmd_pwd
  SET dm2_server_link->dbase = dmmd_dbase
  SET dm2_server_link->node = dmmd_node
  SET dm2_server_link->hostname = "NONE"
  IF (dm2_create_server_link(null)=0)
   GO TO exit_program
  ENDIF
  SET user_src_tbl = build(dmmd_server,".DM2_USER_TABLES")
  SELECT INTO "nl:"
   FROM dm_tables_doc dtd,
    dm2_dba_synonyms dds,
    dm2_user_tables dut
   PLAN (dtd
    WHERE dtd.reference_ind=1
     AND dtd.mergeable_ind=1)
    JOIN (dds
    WHERE outerjoin(dtd.table_name)=dds.synonym_name
     AND dds.owner=outerjoin(dmmd_server))
    JOIN (dut
    WHERE dut.table_name=dtd.table_name)
   ORDER BY dtd.table_name
   DETAIL
    nick_cnt = (nick_cnt+ 1)
    IF (mod(nick_cnt,100)=1)
     stat = alterlist(dmda_tbl->list,(nick_cnt+ 99))
    ENDIF
    dmda_tbl->list[nick_cnt].tbl_name = dtd.table_name, dmda_tbl->list[nick_cnt].drop_ind = 0
    IF (dds.synonym_name > " ")
     dmda_tbl->list[nick_cnt].drop_ind = 1
    ENDIF
   FOOT REPORT
    stat = alterlist(dmda_tbl->list,nick_cnt), dmda_tbl->tbl_cnt = nick_cnt
   WITH nocounter
  ;end select
  CALL echorecord(dmda_tbl)
  SET dm2_nickname_info->local_owner = dmmd_server
  SET dm2_nickname_info->server = dmmd_server
  SET dm2_nickname_info->remote_owner = "V500"
  FOR (nlp_cnt = 1 TO dmda_tbl->tbl_cnt)
    SET dm2_nickname_info->nickname = dmda_tbl->list[nlp_cnt].tbl_name
    SET dm2_nickname_info->drop_ind = dmda_tbl->list[nlp_cnt].drop_ind
    SET dm2_nickname_info->create_ind = 1
    SET dm2_nickname_info->remote_table = dmda_tbl->list[nlp_cnt].tbl_name
    IF (dm2_create_nickname(null)=0)
     GO TO exit_program
    ELSE
     EXECUTE oragen3 concat(dmmd_server,".",dmda_tbl->list[nlp_cnt].tbl_name)
     IF ((dm_err->err_ind=1))
      GO TO exit_program
     ENDIF
    ENDIF
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "...Ending dm2_make_merge_dblink"
 CALL final_disp_msg("dm2_make_merge_dblink")
END GO
