CREATE PROGRAM acm_chg_encntr_code_value_r:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->encntr_code_value_r_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   encntr_code_value_r e
  SET e.encntr_code_value_r_id = acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].
   encntr_code_value_r_id, e.beg_effective_dt_tm = cnvtdatetime(acm_request->
    encntr_code_value_r_qual[xref->chg[d.seq].idx].beg_effective_dt_tm), e.end_effective_dt_tm =
   cnvtdatetime(acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].end_effective_dt_tm),
   e.code_set = acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].code_set, e.code_value =
   acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].code_value, e.contributor_system_cd =
   IF ((acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   e.encntr_id =
   IF ((acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].encntr_id > 0)) acm_request->
    encntr_code_value_r_qual[xref->chg[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].encntr_idx]
    .encntr_id
   ENDIF
   , e.active_ind = acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].active_ind, e
   .active_status_cd = acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].active_status_cd,
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_code_value_r_id=acm_request->encntr_code_value_r_qual[xref->chg[d.seq].idx].
   encntr_code_value_r_id))
  WITH nocounter, status(reply->encntr_code_value_r_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->encntr_code_value_r_qual[xref->chg[index].idx].status != 1))
    SET failed = update_error
    SET table_name = "ENCNTR_CODE_VALUE_R"
    GO TO exit_script
   ENDIF
 ENDFOR
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,e.encntr_code_value_r_id,acm_request->encntr_code_value_r_qual[xref->
     chg[t1].idx].encntr_code_value_r_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,e.encntr_code_value_r_id,acm_request->encntr_code_value_r_qual[
      xref->chg[t1].idx].encntr_code_value_r_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((e.updt_cnt=acm_request->encntr_code_value_r_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->encntr_code_value_r_qual[idx].status = - (1), reply->encntr_code_value_r_qual[idx].
      encntr_code_value_r_id = acm_request->encntr_code_value_r_qual[idx].encntr_code_value_r_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->encntr_code_value_r_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_code_value_r_qual[idx].beg_effective_dt_tm = e.beg_effective_dt_tm
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_code_value_r_qual[idx].end_effective_dt_tm = e.end_effective_dt_tm
     ENDIF
     IF (findstring("CODE_SET,",chg_str)=0)
      acm_request->encntr_code_value_r_qual[idx].code_set = e.code_set
     ENDIF
     IF (findstring("CODE_VALUE,",chg_str)=0)
      acm_request->encntr_code_value_r_qual[idx].code_value = e.code_value
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->encntr_code_value_r_qual[idx].contributor_system_cd = e.contributor_system_cd
     ENDIF
     IF (findstring("ENCNTR_ID,",chg_str)=0)
      acm_request->encntr_code_value_r_qual[idx].encntr_id = e.encntr_id
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->encntr_code_value_r_qual[idx].active_ind = e.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->encntr_code_value_r_qual[idx].active_status_cd = e.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->encntr_code_value_r_qual[xref->chg[index].idx].status=0))
