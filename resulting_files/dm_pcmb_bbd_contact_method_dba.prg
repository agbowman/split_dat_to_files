CREATE PROGRAM dm_pcmb_bbd_contact_method:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "BBD_CONTACT_METHOD"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_BBD_CONTACT_METHOD"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   .active_status_cd,
   rreclist->from_rec[v_cust_count1].contact_method_id = frm.contact_method_id, rreclist->from_rec[
   v_cust_count1].contact_method_cd = frm.contact_method_cd
  FOOT REPORT
 ...
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    rreclist->to_rec[v_cust_count2].contact_method_id = tu.contact_method_id, rreclist->to_rec[
    v_cust_count2].contact_method_cd = tu.contact_method_cd
   FOOT REPORT
  ...
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
   SELECT INTO "nl:"
    ...
    FROM bbd_contact_method tu
    WHERE (tu.person_id=request->xxx_combine[icombine].to_xxx_id)
     AND (tu.contact_method_cd=rreclist->from_rec[v_cust_loopcount].contact_method_cd)
     AND tu.active_ind=1
    WITH forupdate(tu)
   ;end select
   IF (curqual=0)
    CALL upt_from(rreclist->from_rec[v_cust_loopcount].contact_method_id,request->xxx_combine[
     icombine].to_xxx_id)
   ELSE
    CALL del_from(rreclist->from_rec[v_cust_loopcount].contact_method_id,request->xxx_combine[
 ...
 SUBROUTINE del_from(s_df_pk_id,s_df_to_fk_id,s_df_prev_act_ind,s_df_prev_act_status)
   UPDATE  FROM bbd_contact_method frm
    ...
    WHERE frm.contact_method_id=s_df_pk_id
    WITH nocounter
   ;end update
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "BBD_CONTACT_METHOD"
 ...
 SUBROUTINE upt_from(s_uf_pk_id,s_uf_to_fk_id)
   UPDATE  FROM bbd_contact_method frm
    ...
    WHERE frm.contact_method_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet = (icombinedet+ 1)
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
