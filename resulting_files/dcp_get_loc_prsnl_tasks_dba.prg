CREATE PROGRAM dcp_get_loc_prsnl_tasks:dba
 RECORD internal(
   1 task_list[*]
     2 task_id = f8
 )
 ...
 SELECT
  IF (types_to_get > 0
   ...
   FROM (dummyt d  WITH seq = value(tasks_to_get)),
    (dummyt d2  WITH seq = value(locations_to_get)),
   ...
   PLAN (d)
    ...
    JOIN (d5)
    JOIN (ta
    WHERE (((ta.task_id=internal->task_list[d.seq].task_id)) OR ((ta.location_cd=request->
    location_list[d2.seq].location_cd)))
