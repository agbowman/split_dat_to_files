CREATE PROGRAM cps_del_spclty_disp_cntl:dba
 ...
 CALL del_spclty_disp_cntl(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE del_spclty_disp_cntl(del_begin,del_end)
   FOR (x = del_begin TO del_end)
     ...
     UPDATE  FROM spclty_disp_cntl s
      SET s.active_ind = false, s.active_status_cd = nullcheck(active_code,request->spclty_disp_cntl[
        x].active_status_cd,
        IF ((request->spclty_disp_cntl[x].active_status_cd=0)) 0
        ELSE 1
        ENDIF
