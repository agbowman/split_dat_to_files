CREATE PROGRAM dm2_mig_import_remote:dba
 ...
 DECLARE dmir_lf_name = vc WITH protect, noconstant("dm2_mig_impremote")
 IF (check_logfile(dmir_lf_name,".log","Starting DM2_MIG_IMPORT_REMOTE.")=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (dmr_prompt_retrieve_mig_data(1,1,0,0)=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(dmr_mig_data)
 ENDIF
 EXECUTE dm_env_import_remote
#exit_program
 SET dm_err->eproc = "Finished executing dm2_mig_import_remote."
 CALL final_disp_msg(dmir_lf_name)
END GO
