CREATE PROGRAM bed_get_hp_wo_insco:dba
 ...
 SET insco_cd = 0.0
 SET insco_cd = uar_get_code_by("MEANING",278,"INSCO")
 ...
 ENDIF
 DECLARE hp_parse = vc
 SET hp_parse = "h.health_plan_id > 0 and h.active_ind = 1"
 ...
 SELECT INTO "nl:"
  ...
  PLAN (h
   ...
   JOIN (opr
   WHERE opr.health_plan_id=outerjoin(h.health_plan_id)
    AND opr.active_ind=outerjoin(1))
   JOIN (o
   WHERE o.organization_id=outerjoin(opr.organization_id)
    AND o.active_ind=outerjoin(1))
   JOIN (otr
   WHERE otr.organization_id=outerjoin(o.organization_id)
    AND otr.org_type_cd=outerjoin(insco_cd)
    AND otr.active_ind=outerjoin(1))
  ORDER BY h.health_plan_id, otr.organization_id
  HEAD h.health_plan_id
   add_hp_ind = 1
  HEAD otr.organization_id
   IF (otr.organization_id > 0)
    add_hp_ind = 0
   ENDIF
  FOOT  otr.organization_id
   IF (add_hp_ind=1)
    cnt = (cnt+ 1), stat = alterlist(reply->health_plans,cnt), reply->health_plans[cnt].id = h
    .health_plan_id,
    reply->health_plans[cnt].name = h.plan_name
   ENDIF
  WITH nocounter
 ;end select
 CALL bederrorcheck("error retrieving health plans")
#exit_script
