CREATE PROGRAM afc_cs_add_code
 RECORD reply(
   1 qual[1]
     2 code_value = f8
     2 display_key = c40
 ...
 IF ( NOT (validate(getlogicaldomainsystemuser)))
  ...
  END ;Subroutine
 ENDIF
 CALL beginservice("307322.001")
 DECLARE curlogicaldomainid = f8 WITH protect, noconstant(0.0)
 DECLARE cvcode_value = f8 WITH protect, noconstant(0.0)
 DECLARE iindex = i4
 ...
 SELECT INTO "nl:"
  FROM code_value cv
  WHERE (cv.display=request->qual[iindex].display)
   AND (cv.code_set=request->code_set)
  DETAIL
   cvcode_value = cv.code_value
  WITH nocounter
 ;end select
 IF (cvcode_value > 0.0)
  SELECT INTO "nl:"
   FROM bill_item b
   WHERE b.ext_parent_reference_id=cvcode_value
    AND b.logical_domain_id=curlogicaldomainid
    AND b.active_ind=1
  ;end select
  IF (curqual > 0)
   SET reply->qual[iindex].display_key = request->qual[iindex].display_key
   SET reply->qual[iindex].code_value = 0.0
   SET reply->qual[iindex].rec_status = "D"
  ELSE
   SET reply->qual[iindex].display_key = request->qual[iindex].display_key
   SET reply->qual[iindex].code_value = cvcode_value
   SET reply->qual[iindex].rec_status = "S"
  ENDIF
 ELSE
  EXECUTE cs_add_code
 ENDIF
