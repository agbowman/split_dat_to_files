CREATE PROGRAM dm2_pkindex_assign
 IF ((validate(pkind_request->pkcolcnt,- (1))=- (1))
  AND (validate(pkind_request->pkcolcnt,- (2))=- (2)))
  RECORD pkind_request(
    1 apply_ddl_ind = i2
    1 pkcolcnt = i4
    1 pkcollist[*]
      2 colname = vc
  ...
  SET pkind_request->apply_ddl_ind = 1
 ENDIF
 IF ((validate(pkind_reply->new_pkindex_ind,- (1))=- (1))
  AND (validate(pkind_reply->new_pkindex_ind,- (2))=- (2)))
  RECORD pkind_reply(
    1 new_pkindex_ind = i2
    1 ddl_command = vc
  )
 ENDIF
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dpa_matching_ind = i2 WITH protect, noconstant(0)
 DECLARE dpa_tab_name = vc WITH protect, noconstant("")
 DECLARE dpa_for_iterator = i4 WITH protect, noconstant(0)
 DECLARE dpa_while_iterator = i4 WITH protect, noconstant(1)
 IF (check_logfile("dm2_pkindex_assign",".log","dm2_pkindex LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dpa_tab_name = trim(cnvtupper( $1),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = "Parameter usage: dm2_pkindex_assign  '<table_name>'"
  GO TO exit_program
 ENDIF
 IF (currdb != "ORACLE")
  SET dm_err->emsg = "This program should only be used on an Oracle RDBMS."
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Identifying Oracle Version."
 IF (dm2_get_rdbms_version(null) != 1)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 IF ((((dm2_rdbms_version->level1 * 100)+ dm2_rdbms_version->level2) < 902))
  SET dm_err->emsg = "This program should only be run on Oracle versions 9.2.x and higher."
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting constraint info from USER_CONSTRAINTS."
 SELECT INTO "nl:"
  FROM user_constraints uc,
   dm2_user_cons_columns ducc
  PLAN (uc
   WHERE uc.table_name=dpa_tab_name
    AND uc.constraint_type="P")
   JOIN (ducc
   WHERE ducc.table_name=dpa_tab_name
    AND ducc.constraint_name=uc.constraint_name)
  ORDER BY ducc.position DESC
  HEAD REPORT
   pkind_request->curr_pkname = uc.constraint_name, pkind_request->curr_pkindname = uc.index_name,
   pkind_request->pkcolcnt = ducc.position,
   stat = alterlist(pkind_request->pkcollist,pkind_request->pkcolcnt)
  DETAIL
   pkind_request->pkcollist[ducc.position].colname = ducc.column_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  GO TO exit_program
 ENDIF
 IF (((trim(pkind_request->curr_pkindname)="") OR (trim(pkind_request->curr_pkname)="")) )
  SET dm_err->eproc = concat("Table: ",dpa_tab_name,
   " has no constraints, or there was a problem getting constraint from USER_CONSTRAINTS")
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF ((pkind_request->pkcolcnt=0))
  SET dm_err->emsg = "Could not get constraint columns from DM2_USER_CONS_COLUMNS."
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting index columns from DM2_USER_IND_COLUMNS"
 SELECT INTO "nl:"
  FROM dm2_user_ind_columns duic
  PLAN (duic
   WHERE duic.table_name=dpa_tab_name)
  ORDER BY duic.index_name, duic.column_position DESC
  HEAD REPORT
   pkind_request->indexcnt = 0, tmp = 0
  HEAD duic.index_name
   pkind_request->indexcnt = (pkind_request->indexcnt+ 1)
   IF ((pkind_request->indexcnt > tmp))
    tmp = (tmp+ 5), stat = alterlist(pkind_request->indexlist,tmp)
   ENDIF
   pkind_request->indexlist[pkind_request->indexcnt].index_name = duic.index_name, pkind_request->
   indexlist[pkind_request->indexcnt].indcolcnt = duic.column_position, stat = alterlist(
    pkind_request->indexlist[pkind_request->indexcnt].indexcollist,pkind_request->indexlist[
    pkind_request->indexcnt].indcolcnt)
  DETAIL
   pkind_request->indexlist[pkind_request->indexcnt].indexcollist[duic.column_position].colname =
   duic.column_name
  FOOT REPORT
   stat = alterlist(pkind_request->indexlist,pkind_request->indexcnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  GO TO exit_program
 ENDIF
 IF ((pkind_request->indexcnt=0))
  SET dm_err->emsg = "Could not get index information from DM2_USER_IND_COLUMNS."
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Checking list for index equality."
 FOR (dpa_for_iterator = 1 TO pkind_request->indexcnt)
   IF ((pkind_request->indexlist[dpa_for_iterator].indcolcnt=pkind_request->pkcolcnt))
    SET dpa_matching_ind = 1
    SET dpa_while_iterator = 1
    WHILE ((dpa_while_iterator <= pkind_request->indexlist[dpa_for_iterator].indcolcnt)
     AND dpa_matching_ind=1)
     IF ((pkind_request->indexlist[dpa_for_iterator].indexcollist[dpa_while_iterator].colname !=
     pkind_request->pkcollist[dpa_while_iterator].colname))
      SET dpa_matching_ind = 0
     ENDIF
     SET dpa_while_iterator = (dpa_while_iterator+ 1)
    ENDWHILE
    IF (dpa_matching_ind=1)
     IF ((pkind_request->indexlist[dpa_for_iterator].index_name != pkind_request->curr_pkindname))
      SET pkind_reply->new_pkindex_ind = 1
      SET pkind_reply->ddl_command = concat('ALTER TABLE "',dpa_tab_name,
       '" MODIFY PRIMARY KEY USING INDEX "',pkind_request->indexlist[dpa_for_iterator].index_name,'"'
       )
     ENDIF
     SET dpa_for_iterator = (pkind_request->indexcnt+ 1)
    ENDIF
   ENDIF
 ENDFOR
 IF ((pkind_request->apply_ddl_ind=1)
  AND (pkind_reply->new_pkindex_ind=1))
  SET dm_err->eproc = "Applying command"
  CALL dm2_push_cmd(concat("RDB ASIS(^",pkind_reply->ddl_command,"^) go"),1)
  IF (check_error(dm_err->eproc) != 0)
   GO TO exit_program
  ENDIF
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_pkindex_assign completed succesfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 CALL final_disp_msg("dm2_pkindex_assign")
END GO
