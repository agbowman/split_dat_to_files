CREATE PROGRAM dm2_stop_ref_data_mover:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsrd_cnt = i4 WITH protect, noconstant(1)
 DECLARE dsrd_limit = dq8 WITH protect, noconstant(0.0)
 CALL check_logfile("stop_ref_data_mover",".log","DM2_STOP_REF_DATA_MOVER LOGFILE")
 SET dm_err->eproc = "Beginning dm2_stop_ref_data_mover"
 CALL disp_msg(" ",dm_err->logfile,0)
 UPDATE  FROM dm_info d
  ...
  WITH nocounter
 ;end update
 IF (check_error("Update dm_info with the new date time")=1)
  ROLLBACK
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ...
 IF (curqual=0)
  INSERT  FROM dm_info d
   ...
   WITH nocounter
  ;end insert
  IF (check_error("Insert into dm_info with the new date time")=1)
   ROLLBACK
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
 ...
 ENDIF
 COMMIT
 SET dsrd_limit = cnvtlookahead("5,MIN",cnvtdatetime(curdate,curtime3))
 WHILE (dsrd_cnt != 0)
   CALL pause(15)
   SELECT INTO "NL:"
    cnt = count(*)
    ...
    WITH nocounter
   ;end select
   IF (check_error("Check for movers running still")=1)
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (cnvtdatetime(curdate,curtime3) >= cnvtdatetime(dsrd_limit))
    SET dm_err->err_ind = 1
    SET dm_err->emsg =
    "It has been 5 minutes since Mover Stop Time was set, and movers are still running."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dsrd_cnt = 0
   ENDIF
 ENDWHILE
#exit_program
 SET dm_err->eproc = "...Ending dm2_stop_ref_data_mover"
 CALL final_disp_msg("dm2_stop_ref_data_mover")
END GO
