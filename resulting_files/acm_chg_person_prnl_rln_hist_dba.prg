CREATE PROGRAM acm_chg_person_prnl_rln_hist:dba
 ...
 RECORD temp_record(
   1 person_prnl_rln_hist_qual[*]
     2 person_id = f8
     2 person_prsnl_reltn_hist_id = f8
 ...
 ENDFOR
 SET stat = alterlist(temp_record->person_prnl_rln_hist_qual,xref->chg_cnt)
 SET pprh_qual_cntr = 0
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(xref->chg_cnt))
  PLAN (d)
  DETAIL
   item_idx = xref->chg[d.seq].idx, chg_str = acm_request->person_prsnl_reltn_qual[item_idx].chg_str,
   chgbit = 0
   ...
   ENDIF
   IF (chgbit > 0)
    pprh_qual_cntr = (pprh_qual_cntr+ 1), temp_record->person_prnl_rln_hist_qual[pprh_qual_cntr].
    change_bit = chgbit, temp_record->person_prnl_rln_hist_qual[pprh_qual_cntr].item_idx = item_idx,
    temp_record->person_prnl_rln_hist_qual[pprh_qual_cntr].person_id = acm_request->
    person_prsnl_reltn_qual[item_idx].person_id, temp_record->person_prnl_rln_hist_qual[
    pprh_qual_cntr].person_prsnl_reltn_id = acm_request->person_prsnl_reltn_qual[item_idx].
    person_prsnl_reltn_id, temp_record->person_prnl_rln_hist_qual[pprh_qual_cntr].pm_hist_tracking_id
     = acm_request->person_prsnl_reltn_qual[item_idx].pm_hist_tracking_id,
    temp_record->person_prnl_rln_hist_qual[pprh_qual_cntr].transaction_dt_tm = acm_request->
    person_prsnl_reltn_qual[item_idx].transaction_dt_tm, temp_record->person_prnl_rln_hist_qual[
    pprh_qual_cntr].person_prsnl_r_cd = acm_request->person_prsnl_reltn_qual[item_idx].
    person_prsnl_r_cd, temp_record->person_prnl_rln_hist_qual[pprh_qual_cntr].prsnl_person_id =
    acm_request->person_prsnl_reltn_qual[item_idx].prsnl_person_id
   ELSE
    reply->person_prsnl_reltn_qual[item_idx].status = 1
   ENDIF
  WITH nocounter
 ;end select
 SET stat = alterlist(temp_record->person_prnl_rln_hist_qual,pprh_qual_cntr)
 IF (pprh_qual_cntr > 0)
  INSERT  FROM person_prsnl_reltn_hist pprh,
    (dummyt d  WITH seq = value(pprh_qual_cntr))
   SET pprh.person_id = temp_record->person_prnl_rln_hist_qual[d.seq].person_id, pprh
    .person_prsnl_reltn_hist_id = seq(person_seq,nextval), pprh.person_prsnl_reltn_id = temp_record->
    person_prnl_rln_hist_qual[d.seq].person_prsnl_reltn_id,
    pprh.pm_hist_tracking_id = temp_record->person_prnl_rln_hist_qual[d.seq].pm_hist_tracking_id,
    pprh.transaction_dt_tm = cnvtdatetime(temp_record->person_prnl_rln_hist_qual[d.seq].
     transaction_dt_tm), pprh.person_prsnl_r_cd = temp_record->person_prnl_rln_hist_qual[d.seq].
    person_prsnl_r_cd,
    pprh.prsnl_person_id = temp_record->person_prnl_rln_hist_qual[d.seq].prsnl_person_id, pprh
    .tracking_bit = tracking_bit, pprh.change_bit = temp_record->person_prnl_rln_hist_qual[d.seq].
    change_bit,
    pprh.active_ind = 1, pprh.active_status_cd = reqdata->active_status_cd, pprh.active_status_dt_tm
     = cnvtdatetime(curdate,curtime3),
    pprh.active_status_prsnl_id = reqinfo->updt_id, pprh.updt_dt_tm = cnvtdatetime(curdate,curtime3),
    pprh.updt_id = reqinfo->updt_id,
    pprh.updt_applctx = reqinfo->updt_applctx, pprh.updt_task = reqinfo->updt_task, pprh.updt_cnt =
    updt_cnt
   PLAN (d)
    JOIN (pprh)
   WITH nocounter, status(reply->person_prsnl_reltn_qual[temp_record->person_prnl_rln_hist_qual[d.seq
    ].item_idx].status)
  ;end insert
 ENDIF
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->person_prsnl_reltn_qual[xref->chg[index].idx].status=0))
    ...
    SET table_name = "PERSON_PRSNL_RELTN_HIST"
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
