CREATE PROGRAM aps_rdm_upd_acc_templates:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_rdm_upd_acc_templates"
 ...
 CALL logmsg("Getting ready to begin reading for AP Template settings.",((e_dispmsg+ e_rdmstatus)+
  e_logtofile))
 SELECT INTO "nl:"
 ...
 SET count = 0
 CALL logmsg("Getting ready to begin updating ap_accn_template_detail table.",((e_dispmsg+
 ...
 WHILE (aatd_cntr >= 1)
   ...
   IF (curqual != value(aatd_commit_recs_whole))
    ROLLBACK
    CALL logmsg("Failed to update ap_accn_template_detail table.",((e_dispmsg+ e_logtofile)+
     e_rdmstatus))
