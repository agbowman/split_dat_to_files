CREATE PROGRAM dm_pft_arwb_data_ext_rows:dba
 ...
 DECLARE min_days_to_keep_limit = i4 WITH protect, constant(90)
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 SET reply->table_name = "PFT_ARWB_DATA_EXT"
 ...
 IF (v_days_to_keep <= min_days_to_keep_limit)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1","You must keep at least",
   min_days_to_keep_limit,
   " days' or 1 years worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(p.pft_arwb_data_ext_id)
   FROM pft_arwb_data_ext p
   WHERE p.pft_arwb_data_ext_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(p.pft_arwb_data_ext_id)
  FROM pft_arwb_data_ext p
  WHERE p.pft_arwb_data_ext_id > 0.0
  DETAIL
  ...
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM pft_arwb_data_ext p,
     ...
     pft_encntr pe
    PLAN (p
     WHERE p.pft_arwb_data_ext_id BETWEEN curminid AND curmaxid
      AND parser(sbr_getrowidnotexists("p.pft_arwb_data_ext_id != 0","p")))
     JOIN (e
     ...
     WHERE pe.encntr_id=e.encntr_id
      ...
      AND pe.last_extraction_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
      AND pe.extraction_ind=true)
    ...
    HEAD p.encntr_id
     ...
     ENDIF
     reply->rows[row_cnt].row_id = p.rowid
    WITH nocounter, maxqual(p,value(rowsleft))
   ;end select
