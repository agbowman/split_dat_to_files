CREATE PROGRAM bhs_sn_censitrac_rpt:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Begin Date" = "CURDATE",
  "End Date" = "CURDATE"
  WITH outdev, ms_beg_date, ms_end_date
 DECLARE mf_beg_dt_tm = vc WITH protect, noconstant("")
 DECLARE mf_end_dt_tm = vc WITH protect, noconstant("")
 ...
 DECLARE mf_equip_mstr_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",11001,
   "EQUIPMENTMASTER"))
 DECLARE ms_filename = vc WITH protect, constant("sn_censitrac_out.dat")
 SET ms_beg_dt_tm = concat(trim( $MS_BEG_DATE)," 00:00:00")
 SET ms_end_dt_tm = concat(trim( $MS_END_DATE)," 23:59:59")
 SELECT INTO "NL:"
  FROM code_value cv
