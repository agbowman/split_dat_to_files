CREATE PROGRAM cp_pregnancy_versions:dba
 ...
 DECLARE dummy_void = i2 WITH protect, constant(0)
 DECLARE egadays = i4 WITH protect, noconstant(0)
 DECLARE egadisplay = vc WITH protect, noconstant
 DECLARE entered_dt_str = vc WITH protect, noconstant
 ...
 DECLARE buildeddtext(null) = null
...
#failure
 ...
 IF (error_code != 0)
  SET reply->status_data.status = "F"
  CALL fillsubeventstatus("ERROR","F","cp_pregnancy_versions",error_msg)
 ...
 SUBROUTINE loadpregnancyhistory(null)
   ...
   SET pregrequest->person_id = request->person_id
   SET pregrequest->encntr_id = request->encntr_id
   SET modify = nopredeclare
   EXECUTE dcp_get_pregnancy_versions  WITH replace("REQUEST",pregrequest), replace("REPLY",pregreply
    )
   SET modify = predeclare
   IF ((pregreply->status_data.status="S"))
    SET phx_data_ind = 1
   ELSEIF ((pregreply->status_data.status="Z"))
    SET phx_data_ind = 0
   ELSE
    SET failure_ind = true
    CALL fillsubeventstatus("EXECUTE","F","cp_pregnancy_versions",
     "dcp_get_pregnancy_versions: failure retrieving pregnancy history")
    GO TO failure
   ENDIF
   FREE RECORD pregrequest
 END ;Subroutine
   ...
   SELECT
    ...
    DETAIL
     IF (active_preg_ind > 0)
      ...
      IF (((preg_org_sec_ind=0) OR ((((request->encntr_id > 0)) OR (size(organizations->
       ...
       FOR (i = 1 TO eddsize)
         ...
         FOR (j = 1 TO linecnt)
          IF ((print_pregnancy->edd_list[i].instance_status=modified))
           col 10, print_pregnancy->edd_list[i].edd_wrap[j].edd_line
          ELSE
           col 5, print_pregnancy->edd_list[i].edd_wrap[j].edd_line
          ENDIF
          ,row + 1
         ENDFOR
         row + 1
     ...
     FOR (i = 1 TO childsize)
       ...
       pregnancy_id))
        IF ((print_pregnancy->child_list[i].instance_status=modified))
         col 15, print_pregnancy->child_list[i].instance_header, row + 1,
         col 15, print_pregnancy->child_list[i].child_header
        ELSE
         col 10, print_pregnancy->child_list[i].child_header
        ENDIF
       ELSEIF ((print_pregnancy->child_list[i].instance_status=deleted))
        col 0, print_pregnancy->child_list[i].instance_header, row + 1,
        col 10, print_pregnancy->child_list[i].child_header
       ELSE
       ...
       FOR (j = 1 TO linecnt)
        IF ((print_pregnancy->child_list[i].instance_status=modified))
         col 15, print_pregnancy->child_list[i].child_wrap[j].child_line
        ELSE
         col 10, print_pregnancy->child_list[i].child_wrap[j].child_line
        ENDIF
        ,row + 1
       ENDFOR
       row + 1
       ...
       child_list[(i+ 1)].pregnancy_id))) )
        IF (size(print_pregnancy->child_list[(i - childseq)].edd_list,5) > 0)
         ...
         FOR (j = 1 TO size(print_pregnancy->child_list[(i - childseq)].edd_list,5))
           ...
           FOR (k = 1 TO linecnt)
            IF ((print_pregnancy->child_list[(i - childseq)].edd_list[j].instance_status=modified))
             col 15, print_pregnancy->child_list[(i - childseq)].edd_list[j].edd_wrap[k].edd_line
            ELSE
 ...
 SUBROUTINE fillhxeddrequest(null)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (d1)
     JOIN (pe
     WHERE expand(idx,1,totalchildcnt,pe.pregnancy_id,print_pregnancy->child_list[idx].pregnancy_id)
    ...
    ORDER BY pe.pregnancy_id, pe.entered_dt_tm DESC, pe.pregnancy_estimate_id
    HEAD REPORT
     eddidx = 0
 ...
 SUBROUTINE buildeddtext(null)
   DECLARE flatedd_size = i4 WITH noconstant(size(flatedd->edd_list,5)), protect
   ...
   SELECT INTO "nl:"
    ...
    ORDER BY flatedd->edd_list[d1.seq].sort_status DESC, flatedd->edd_list[d1.seq].sort_dt_tm DESC
    HEAD REPORT
     eddcnt = 0
    HEAD edd_id
     eddcnt += 1,
     CALL addeddtext(false,true)
    WITH maxcol = 130
   ;end select
   ...
   FOR (x = 1 TO size(print_pregnancy->edd_list,5))
     IF ((print_pregnancy->edd_list[x].instance_status=modified))
      CALL wrap_text(print_pregnancy->edd_list[x].edd_text,65,65)
     ELSE
   ...
   SET stat = alterlist(flatedd->edd_list,0)
 ...
 SUBROUTINE buildhxeddtext(null)
   ...
   DECLARE childidx = i4 WITH noconstant(0)
   DECLARE eddcnt = i4 WITH noconstant(0)
   SELECT INTO "nl:"
    pregnancy_id = flatedd->edd_list[d1.seq].pregnancy_id, edd_id = flatedd->edd_list[d1.seq].
    pregnancy_estimate_id
    FROM (dummyt d1  WITH seq = eddlistsize)
    ORDER BY pregnancy_id, flatedd->edd_list[d1.seq].sort_status DESC, flatedd->edd_list[d1.seq].
     sort_dt_tm DESC
    HEAD REPORT
    ...
    HEAD edd_id
     ...
     IF (childidx != prevchildidx)
      eddcnt = 0
     ENDIF
     eddcnt += 1, prevchildidx = childidx,
     CALL addeddtext(true,true)
    FOOT  edd_id
   ...
   FOR (i = 1 TO size(print_pregnancy->child_list,5))
     FOR (x = 1 TO size(print_pregnancy->child_list[i].edd_list,5))
       IF ((print_pregnancy->child_list[i].edd_list[x].instance_status=modified))
        CALL wrap_text(print_pregnancy->child_list[i].edd_list[x].edd_text,65,65)
       ELSE
 ...
 DECLARE last_mod = vc WITH protect, noconstant("MOD 001 - 13/04/17")
 CALL echo(last_mod)
END GO
