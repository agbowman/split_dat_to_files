CREATE PROGRAM dm_him_coding_rows:dba
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    c.rowid
    FROM coding c
    WHERE parser(sbr_getrowidnotexists("c.coding_id between curMinID and curMaxID","c"))
     AND c.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
