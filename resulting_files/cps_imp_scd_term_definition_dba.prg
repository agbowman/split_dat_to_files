CREATE PROGRAM cps_imp_scd_term_definition:dba
 ...
 SET log_file_name = build("SCD_IMP_TERM_DEF_",format(curdate,"yymmdd;;d"),".log")
...
#start_loop
 ...
 FOR (x = x TO qual)
   ...
   ;end select
   IF (curqual < 1)
    SET error_line = trim(concat("ERROR >NONEXISTENT SCR_TERM_DEF_ID  : scr_term_def_id = ",trim(
