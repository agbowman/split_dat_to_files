CREATE PROGRAM bed_get_datamart_defaults:dba
 FREE SET reply
 RECORD reply(
   1 filter[*]
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     IF (f.filter_category_mean="CE_GROUP")
      group_ind = 1
     ENDIF
     IF (f.filter_category_mean="CE_BP")
      group_ind = 1
     ENDIF
   ...
   IF (group_ind=1)
    SELECT INTO "nl:"
     ...
     PLAN (b
      ...
      JOIN (c
      WHERE c.code_set=b.code_set
       AND ((cnvtupper(trim(c.display))=cnvtupper(trim(b.cv_display))
       AND c.display > " ") OR (((cnvtupper(trim(c.description))=cnvtupper(trim(b.cv_description))
       AND c.description > " ") OR (((c.cdf_meaning=b.unique_identifier
       AND c.cdf_meaning > " ") OR (((c.concept_cki=b.unique_identifier
       AND c.concept_cki > " ") OR (c.cki=b.unique_identifier
       AND c.cki > " ")) )) )) ))
      ...
      DETAIL
       ...
       FOR (z = 1 TO size(reply->filter[x].default,5))
         IF (n.nomenclature_id > 0.0
          AND (reply->filter[x].default[z].code_value=n.nomenclature_id))
          found = 1
         ENDIF
       ENDFOR
       IF (found=0)
