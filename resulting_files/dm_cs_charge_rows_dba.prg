CREATE PROGRAM dm_cs_charge_rows:dba
 DECLARE dm_cs_charge_rows = vc WITH private, noconstant("206564.FT.005")
  ...
  WHILE (curmindate <= maxdate
   AND rowsleft > 0)
    IF (retention_mode=1
     ...
     SELECT INTO "nl:"
      FROM charge c,
       interface_file i
      PLAN (c
       WHERE c.updt_dt_tm >= cnvtdatetime(curmindate)
        AND c.updt_dt_tm <= cnvtdatetime(curmaxdate)
        AND parser(sbr_getrowidnotexists("c.charge_item_id+0 > 0","c"))
       ...
       (SELECT
        ...
        WHERE (cea.charge_event_act_id=(c.charge_event_act_id+ 0))
         AND ((cea.updt_dt_tm+ 0) < cnvtdatetime((curdate - days_to_keep),curtime3)))))
       JOIN (i
      ...
      DETAIL
       ...
       ENDIF
       reply->rows[v_rows].row_id = c.rowid
     ...
     IF (validate(cs_purge_dbg,"N")="Y")
      CALL echo(build("rows after order_id = 0 remove query: ",value(v_rows)))
     ENDIF
     IF (rowsleft > 0)
      SELECT INTO "nl:"
       FROM charge c,
       ...
       DETAIL
        IF (retention_mode=1)
         dup_check = locateval(num,1,size(reply->rows,5),c.rowid,reply->rows[num].row_id)
        ENDIF
        IF (dup_check=0)
         v_rows = (v_rows+ 1)
         IF (mod(v_rows,100)=1)
    ...
    IF (rowsleft > 0)
     ...
     SET rowsleft = (request->max_rows - v_rows)
     IF (validate(cs_purge_dbg,"N")="Y")
      CALL echo(build("inactive rows qualified: ",value(v_rows)))
     ENDIF
     IF (rowsleft > 0)
      SELECT INTO "nl:"
       FROM charge c,
      ...
      SET rowsleft = (request->max_rows - v_rows)
      IF (validate(cs_purge_dbg,"N")="Y")
       CALL echo(build("rows after missing charge_event query: ",value(v_rows)))
      ENDIF
     ENDIF
    ENDIF
  ...
  IF ((request->purge_flag=c_audit))
   IF (size(reply->rows,5) > 0)
    UPDATE  FROM charge c,
     ...
     SET c.updt_task = 951270.0
     PLAN (d)
      JOIN (c
      WHERE (c.rowid=reply->rows[d.seq].row_id))
     WITH nocounter
    ;end update
    COMMIT
   ENDIF
 ...
 SUBROUTINE sbr_get_min_date(null)
   ...
   ENDIF
   UPDATE  FROM charge c
    SET c.updt_dt_tm = cnvtdatetime(curdate,curtime3)
    WHERE c.charge_item_id=0.0
    WITH nocounter
   ;end update
   SET v_err_code2 = error(v_errmsg2,0)
   IF (v_err_code2 != 0)
    ...
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"UPDATEERROR1",
     "Failed to set c.updt_dt_tm to curdate:%1","s",nullterm(v_errmsg2))
    ROLLBACK
    GO TO exit_script
   ELSE
    COMMIT
   ...
   ENDIF
   UPDATE  FROM charge c
    SET c.updt_dt_tm = cnvtdatetime(zerorowdate)
    WHERE c.charge_item_id=0.0
    WITH nocounter
   ;end update
   SET v_err_code2 = error(v_errmsg2,0)
   IF (v_err_code2 != 0)
    ...
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"UPDATEERROR2",
     "Failed to set c.updt_dt_tm to original date:%1","s",nullterm(v_errmsg2))
    ROLLBACK
    GO TO exit_script
   ELSE
    COMMIT
