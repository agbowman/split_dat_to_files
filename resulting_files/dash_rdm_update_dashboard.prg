CREATE PROGRAM dash_rdm_update_dashboard
 PROMPT
 ...
 SET log_program_name = "DASH_RDM_UPDATE_DASHBOARD"
 ...
 SUBROUTINE updatedata(null)
   ...
   SET substrlength = (textlen(json) - substrlength)
   SET json = trim(substring(endpos,substrlength,json))
   CALL log_message("I have removed the Dashboard Template section.<br>")
   SET startpos = findstring("if (typeof dashMaster",json,1,0)
   IF (startpos=0)
   ...
   SET substrlength = (textlen(json) - substrlength)
   SET json = trim(substring(endpos,substrlength,json))
   CALL log_message("I have removed the Dashboard Instance section.<br>")
   SET startpos = findstring("if (typeof dashMaster",json,1,0)
   IF (startpos=0)
   ...
   SET layoutjson = substring(layoutsectionstartpos,(textlen(tmpjson) - layoutsectionstartpos),
    tmpjson)
   SET layoutsect = replace(layoutjson,"layout = ","")
   CALL log_message("I've extracted the Layout section.")
   SET substrlength = (textlen(json) - substrlength)
   SET json = trim(substring(endpos,substrlength,json))
   CALL log_message("I have removed the Layout section.<br>")
   SET startpos = findstring("if (typeof dashMaster",json,1,0)
   IF (startpos=0)
    CALL logandexit("I couldn't identify the Page Filters section.")
   ENDIF
   SET endpos = findstring("if (typeof dashMaster",json,(startpos+ 1),1)
   IF (endpos=0)
   ...
   SELECT INTO "nl:"
    ...
    WHERE dd.dashboard_name=templatename
     AND dd.active_ind=1
    DETAIL
     dashboardid = dd.dash_dashboard_id
    WITH nocounter
   ;end select
   INSERT  FROM dash_dashboard
    ...
    active_ind, template_ind, last_updt_prsnl_id,
    updt_dt_tm, updt_cnt, end_effective_dt_tm)(SELECT
     seq(reference_seq,nextval), orig_dash_dashboard_id, dashboard_name,
     dashboard_template_name, org_id, dash_type_id,
     content_data_txt, shipped_ind, beg_effective_dt_tm,
     0, template_ind, last_updt_prsnl_id,
     updt_dt_tm, updt_cnt, cnvtdatetime(curdate,curtime3)
     FROM dash_dashboard dd
     WHERE dd.dash_dashboard_id=dashboardid)
   ;end insert
   IF (error(errmsg,0) > 0)
    CALL logandexit(concat("Failed trying to archive the Dashboard Template record. ",errmsg))
   ELSE
    CALL log_message(concat("I have archived the old Dashboard Template record: ",templatename,"."))
   ENDIF
   SELECT INTO "nl:"
    FROM dash_dashboard dd
    WHERE dd.dash_dashboard_id=dashboardid
    DETAIL
     updatecnt = dd.updt_cnt
    WITH nocounter
   ;end select
   IF (error(errmsg,0) > 0)
    CALL logandexit(concat("Failed trying to get the current update count.",errmsg))
   ENDIF
   UPDATE  FROM dash_dashboard
    SET content_data_txt = layoutsect, last_updt_prsnl_id = 0.0, updt_dt_tm = cnvtdatetime(curdate,
      curtime3),
     updt_cnt = (updatecnt+ 1), beg_effective_dt_tm = cnvtdatetime(curdate,curtime3)
    WHERE dash_dashboard_id=dashboardid
   ;end update
   IF (error(errmsg,0) > 0)
    CALL logandexit(concat("Failed updating the Dashboard Template record.",errmsg))
   ENDIF
   IF ( NOT (pagefilterjson=""))
    EXECUTE dash_rdm_update_filter pagefilterjson, templatename
    IF ((readme_data->status != "S"))
     GO TO exit_script
    ELSE
     SET readme_data->status = "F"
     SET readme_data->message =
     "Readme Failed: Continuing script dash_rdm_import_dashboard after filter update..."
    ENDIF
   ENDIF
   IF ( NOT (componentsjson=""))
    EXECUTE dash_rdm_update_comp componentsjson, dashboardid
    IF ((readme_data->status != "S"))
     GO TO exit_script
    ELSE
     SET readme_data->status = "F"
     SET readme_data->message =
     "Readme Failed: Continuing script dash_rdm_import_dashboard after first comp update..."
    ENDIF
   ENDIF
   CALL log_message(concat("Dashboard ",instancename," was updated successfully."))
   COMMIT
