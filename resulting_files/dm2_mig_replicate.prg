CREATE PROGRAM dm2_mig_replicate
 ...
 SUBROUTINE drr_del_preserved_ts(dcdl_tgt_db_name)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmr_using_dbx = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_replicate",".log","DM2_MIG_REPLICATE LOG FILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "BEGIN DM2_MIG_REPLICATE"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dmr_prompt_retrieve_mig_data(1,1,1,0)=0)
  GO TO exit_program
 ENDIF
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Verify Source Setup has been completed."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_MIG_SETUP"
   AND d.info_name="SCHEMA_SETUP_COMPLETE"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->emsg = "Source Setup has not been completed."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Checking if DBX is in use"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  dbx_cnt = count(*)
  FROM dm2_admin_dm_info di
 ...
 ENDIF
 IF (dmr_using_dbx > 0)
  SET dm2_mig_dbx_in_use = 1
 ENDIF
 IF (dmr_prompt_connect_data("TARGET","V500","CO")=0)
  GO TO exit_program
 ENDIF
 SET dm2_bypass_env_hist_chk = 1
 SET dm2_allow_same_db_name = 1
 SET dm2_install_schema->menu_driver = "dm2_domain_maint"
 SET dm2_install_schema->process_option = "CLIN COPY"
 SET drr_clin_copy_data->process = "MIGRATION"
 ...
 ENDIF
 IF (dmr_using_dbx=0)
  SET dm_err->eproc = "Determine if all DDL has completed successfully."
  SELECT INTO "nl:"
   FROM dm2_ddl_ops_log d
   WHERE (d.run_id=dm2_install_schema->run_id)
    AND ((d.status="ERROR") OR (d.status = null))
   WITH nocounter, maxqual(d,1)
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF (curqual > 0)
   SET dm_err->emsg = "There are remaining DDL operations to execute."
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Determine if all DDL has completed."
  SELECT INTO "nl:"
   FROM dm2_ddl_ops_log d
   WHERE (d.run_id=dm2_install_schema->run_id)
    AND d.status="COMPLETE"
 ...
 IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Remove status of DM2_MIG_REPLICATE."
 DELETE  FROM dm_info d
  WHERE d.info_domain="DM2_MIG_REPLICATE"
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Insert success status for DM2_MIG_REPLICATE."
 INSERT  FROM dm_info d
  SET d.info_domain = "DM2_MIG_REPLICATE", d.info_name = "SUCCESSFUL_EXECUTION"
  WITH nocounter
 ;end insert
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
...
#exit_program
 IF ((dm_err->err_ind=1))
  ...
  CALL echo("*")
 ENDIF
 SET dm_err->eproc = "DM2_MIG_REPLICATE completes successfully"
 CALL final_disp_msg("dm2_mig_replicate")
END GO
