CREATE PROGRAM dm2_verify_data_rpt
 ...
 SUBROUTINE dvdr_get_verify2(null)
  ...
  RETURN(1)
 END ;Subroutine
 DECLARE dvdr_cnt = i4 WITH protect, noconstant(0)
 DECLARE dvdr_cmp_flag = i2 WITH protect, noconstant(0)
 DECLARE dvdr_rpt_file = vc WITH protect, noconstant("")
 DECLARE dvdr_final_row_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_vdata_rpt",".log","dm2_vdata_rpt LOGFILE")=0)
  GO TO exit_script
 ENDIF
 ...
 IF ((((dvdr_rpt_dtl_info->src_db_name="DM2NOTSET")) OR ((dvdr_rpt_dtl_info->tgt_db_name="DM2NOTSET")
  ...
  GO TO exit_script
 ENDIF
 IF (dvdr_get_verify2(null)=0)
  GO TO exit_script
 ENDIF
 IF ((dmvd->use_verify2_ind=1))
  EXECUTE dm2_verify_data_rpt2
  GO TO exit_script
 ENDIF
 ...
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 IF (get_unique_file("dm2_vdata_rpt",".rpt")=0)
  GO TO exit_script
 ELSE
  SET dvdr_rpt_file = dm_err->unique_fname
 ENDIF
 SELECT INTO value(dvdr_rpt_file)
  FROM dummyt d
  HEAD REPORT
   ...
   CALL print(concat("  Source Database: ",dvdr_rpt_dtl_info->src_db_name,"@",dvdr_rpt_dtl_info->
    db_link)),
   row + 1,
   CALL print(concat("  Target Database: ",dvdr_rpt_dtl_info->tgt_db_name)), row + 2,
   CALL print(fillstring(250,"-")), row + 1
   IF ((dvdr_rpt_dtl_info->tbls_compared=0))
    "Compare Summary Report: No tables have been compared.", row + 1
   ELSEIF ((dvdr_rpt_dtl_info->tbls_compared=dvdr_rpt_dtl_info->tbls_matched))
    "Compare Summary Report: All tables matched.", row + 1
   ELSEIF ((dvdr_rpt_dtl_info->tbls_compared=dvdr_rpt_dtl_info->tbls_complete)
    AND (dvdr_rpt_dtl_info->tbls_complete > 0))
    "Compare Summary Report: All tables matched in final compare.", row + 1
   ELSEIF ((dvdr_rpt_dtl_info->tbls_mm_down_cnt > 0))
    "Compare Summary Report: Table mismatches found in final compare.", row + 1
   ELSEIF ((dvdr_rpt_dtl_info->tbls_mm_cnt > 0))
    "Compare Summary Report: Table mismatches found.", row + 1
   ELSE
    IF (dvdr_final_row_ind=1)
     "Compare Summary Report: Final Compare in Progress", row + 1
    ELSE
     "Compare Summary Report: Compare in Progress", row + 1
    ENDIF
   ENDIF
   CALL print(fillstring(250,"-")), row + 2, col 0,
   "Owners Compared:", col 40,
   CALL print(concat(cnvtstring(dvdr_rpt_dtl_info->num_owners)," -- ",dvdr_rpt_dtl_info->owners_list)
   ),
   row + 2, col 0, "Compare Statistics:",
   row + 1
   IF (dvdr_final_row_ind=1)
    col 5, "Final Compare Start:", col 40,
    CALL print(format(dvdr_rpt_dtl_info->final_row_dt_tm,"DD-MMM-YY HH:MM;;D")), row + 2, col 5,
    "Final Compare Mismatched:", col 40,
    CALL print(cnvtstring(dvdr_rpt_dtl_info->tbls_mm_down_cnt)),
    col 50,
    CALL print(concat("(",build(dvdr_rpt_dtl_info->downtime_mm_pct),"%)")), row + 1,
    col 5, "Final Compare Matched:", col 40,
    CALL print(cnvtstring(dvdr_rpt_dtl_info->tbls_complete)), col 50,
    CALL print(concat("(",build(dvdr_rpt_dtl_info->complete_pct),"%)")),
    row + 1, col 5, "Final Compare Pending:",
    col 40,
    CALL print(cnvtstring(dvdr_rpt_dtl_info->yet_to_be_compared)), col 50,
    CALL print(concat("(",build(dvdr_rpt_dtl_info->yet_to_be_compared_pct),"%)")), row + 2
   ELSE
    col 5, "Compare Mismatched:", col 40,
    CALL print(cnvtstring(dvdr_rpt_dtl_info->tbls_mm_cnt)), col 50,
    CALL print(concat("(",build(dvdr_rpt_dtl_info->mismatch_pct),"%)")),
    row + 1, col 5, "Compare Matched:",
    col 40,
    CALL print(cnvtstring(dvdr_rpt_dtl_info->tbls_matched)), col 50,
    CALL print(concat("(",build(dvdr_rpt_dtl_info->mtch_pct),"%)")), row + 1, col 5,
    "Compare Pending:", col 40,
    CALL print(cnvtstring(dvdr_rpt_dtl_info->yet_to_be_compared)),
    col 50,
    CALL print(concat("(",build(dvdr_rpt_dtl_info->yet_to_be_compared_pct),"%)")), row + 2
   ENDIF
   col 5, "Eligible for Compare:", col 40,
   CALL print(cnvtstring(dvdr_rpt_dtl_info->tbls_compared)), row + 1, col 5,
   "Excluded from Compare:", col 40,
   CALL print(cnvtstring(dvdr_rpt_dtl_info->tbls_excluded)),
   row + 2
   IF ((dvdr_rpt_dtl_info->comp_dt_min > 0.0))
    col 5, "Cycle Time Start:", col 40,
    CALL print(format(dvdr_rpt_dtl_info->comp_dt_min,"DD-MMM-YY HH:MM;;D")), row + 1, col 5,
    "Cycle Time End:", col 40,
    CALL print(format(dvdr_rpt_dtl_info->comp_dt_max,"DD-MMM-YY HH:MM;;D")),
    row + 1, col 5, "Cycle Time (Minutes):",
    col 40,
    CALL print(format(datetimediff(dvdr_rpt_dtl_info->comp_dt_max,dvdr_rpt_dtl_info->comp_dt_min,4),
     ";L")), row + 2
   ENDIF
   dvdr_cmp_flag = 0
   FOR (dvdr_cnt = 1 TO dvdr_rpt_dtl_info->tbl_cnt)
     IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].status=dvdr_comparing))
      IF (dvdr_cmp_flag=0)
       col 5, "Tables Currently Being Compared: ", dvdr_cmp_flag = 1
      ENDIF
      col 40,
      CALL print(concat(dvdr_rpt_dtl_info->tbls[dvdr_cnt].owner,".",dvdr_rpt_dtl_info->tbls[dvdr_cnt]
       .table_name)), row + 1
     ENDIF
   ENDFOR
   row + 2
  DETAIL
   IF (dvdr_final_row_ind=1
    AND (dvdr_rpt_dtl_info->tbls_mm_down_cnt > 0))
    CALL print(fillstring(250,"-")), row + 1, "Final Compare Mismatched Tables",
    row + 1,
    CALL print(fillstring(250,"-")), row + 2,
    col 1, " ", col 41,
    "| CONSEC. ", col 50, "| # KEYS",
    ...
    "| ", col 186, "| ",
    row + 1, col 1, " ",
    col 41, "|MISMATCH ", col 50,
    "| MIS- ", col 64, "| LAST COMPARE ",
    col 81, "| LAST MIN KEY ", col 96,
    "| LAST MIN KEY ", col 111, "| CURRENT MAX ",
    col 126, "| LAST MAX KEY ", col 141,
    "| # KEYS ", col 156, "| TOTAL # KEYS ",
    col 171, "| ORIGINAL MIN ", col 186,
    "| LAST # KEYS ", row + 1, col 1,
    "OWNER.TABLE_NAME", col 41, "| EVENTS  ",
    col 50, "| MATCHED ", col 64,
    "| DT / TM ", col 81, "| MISMATCHED ",
    col 96, "| COMPARED ", col 111,
    "| KEY ID ", col 126, "| COMPARED ",
    col 141, "| REMAINING ", col 156,
    "| COMPARED ", col 171, "| KEY ID ",
    col 186, "| COMPARED ", row + 1,
    CALL print(fillstring(250,"-")), row + 2, dvdr_cnt = 0
    FOR (dvdr_cnt = 1 TO dvdr_rpt_dtl_info->tbl_cnt)
      IF (dvdr_final_row_ind=1
       AND (dvdr_rpt_dtl_info->tbls[dvdr_cnt].status=dvdr_mismatch)
       AND (dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_compare_dt_tm > dvdr_rpt_dtl_info->final_row_dt_tm
      ))
       col 0,
       CALL print(concat(trim(substring(1,10,dvdr_rpt_dtl_info->tbls[dvdr_cnt].owner)),".",
        dvdr_rpt_dtl_info->tbls[dvdr_cnt].table_name))
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].mismatch_event_cnt=- (1)))
        col 41, "| ", "N/A"
       ELSE
        IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].total_rows_compared > 0))
         col 41, "|*",
         CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].mismatch_event_cnt,";L"))
        ELSE
         col 41, "| ",
         CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].mismatch_event_cnt,";L"))
        ENDIF
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_rows_mismatched=- (1)))
        col 50, "| ", "N/A"
       ELSE
        col 50, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_rows_mismatched,";L"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_compare_dt_tm=cnvtdatetime("01-JAN-1900")))
        col 64, "| ", "N/A"
       ELSE
        col 64, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_compare_dt_tm,"DD-MMM-YY HH:MM;;D"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_mismatch_keyid_min > 9999999999999.0))
        col 81, "| ", "*************"
       ELSEIF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_mismatch_keyid_min=- (1)))
        col 81, "| ", "N/A"
       ELSE
        col 81, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_mismatch_keyid_min,";L"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_comp_keyid_min > 9999999999999.0))
        col 96, "| ", "*************"
       ELSEIF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_comp_keyid_min=- (1)))
        col 96, "| ", "N/A"
       ELSE
        col 96, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_comp_keyid_min,";L"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].max_src_keyid > 9999999999999.0))
        col 111, "| ", "*************"
       ELSEIF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].max_src_keyid=- (1)))
        col 111, "| ", "N/A"
       ELSE
        col 111, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].max_src_keyid,";L"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_comp_keyid_max > 9999999999999.0))
        col 126, "| ", "*************"
       ELSEIF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_comp_keyid_max=- (1)))
        col 126, "| ", "N/A"
       ELSE
        col 126, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_comp_keyid_max,";L"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].key_range_remaining > 9999999999999.0))
        col 141, "| ", "*************"
       ELSEIF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].key_range_remaining=- (1)))
        col 141, "| ", "N/A"
       ELSE
        col 141, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].key_range_remaining,";L"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].total_rows_compared > 9999999999999.0))
        col 156, "| ", "*************"
       ELSEIF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].total_rows_compared=- (1)))
        col 156, "| ", "N/A"
       ELSE
        col 156, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].total_rows_compared,";L"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].orig_start_key > 9999999999999.0))
        col 171, "| ", "*************"
       ELSEIF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].orig_start_key=- (1)))
        col 171, "| ", "N/A"
       ELSE
        col 171, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].orig_start_key,";L"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_rows_compared > 9999999999999.0))
        col 186, "| ", "*************"
       ELSEIF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_rows_compared=- (1)))
        col 186, "| ", "N/A"
       ELSE
        col 186, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_rows_compared,";L"))
       ENDIF
       row + 1
      ENDIF
    ENDFOR
   ENDIF
   IF ((dvdr_rpt_dtl_info->tbls_mm_cnt > 0))
    row + 2,
    CALL print(fillstring(250,"-")), row + 1,
    "Mismatched Tables", row + 1,
    CALL print(fillstring(250,"-")),
    row + 2, col 1, " ",
    col 41, "| CONSEC. ", col 50,
    "| # KEYS", col 64, "| ",
    col 81, "| ", col 96,
    "| ", col 111, "| ",
    col 126, "| ", col 141,
    ...
    " ", col 41, "|MISMATCH ",
    col 50, "| MIS- ", col 64,
    "| LAST COMPARE ", col 81, "| LAST MIN KEY ",
    col 96, "| LAST MIN KEY ", col 111,
    "| CURRENT MAX ", col 126, "| LAST MAX KEY ",
    col 141, "| # KEYS ", col 156,
    "| TOTAL # KEYS ", col 171, "| ORIGINAL MIN ",
    col 186, "| LAST # KEYS ", row + 1,
    col 1, "OWNER.TABLE_NAME", col 41,
    "| EVENTS  ", col 50, "| MATCHED ",
    col 64, "| DT / TM ", col 81,
    "| MISMATCHED ", col 96, "| COMPARED ",
    col 111, "| KEY ID ", col 126,
    "| COMPARED ", col 141, "| REMAINING ",
    col 156, "| COMPARED ", col 171,
    "| KEY ID ", col 186, "| COMPARED ",
    row + 1,
    CALL print(fillstring(250,"-")), row + 2,
    dvdr_cnt = 0
    FOR (dvdr_cnt = 1 TO dvdr_rpt_dtl_info->tbl_cnt)
      IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].status=dvdr_mismatch)
       AND (((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_compare_dt_tm <= dvdr_rpt_dtl_info->
      final_row_dt_tm)) OR ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].status=dvdr_mismatch)
       AND (dvdr_rpt_dtl_info->final_row_dt_tm=0.0))) )
       col 0,
       CALL print(concat(trim(substring(1,10,dvdr_rpt_dtl_info->tbls[dvdr_cnt].owner)),".",
        dvdr_rpt_dtl_info->tbls[dvdr_cnt].table_name))
   ...
   row + 2,
   CALL print(fillstring(250,"-")), row + 1,
   "Tables Excluded From Compare", row + 1,
   CALL print(fillstring(250,"-")),
   row + 2
   IF ((dvdr_rpt_dtl_info->tbl_cnt > 0))
    col 0, "OWNER.TABLE_NAME", col 40,
    "| COMPARE EXCLUSION REASON ", row + 1,
    CALL print(fillstring(250,"-")),
    row + 2
    FOR (dvdr_cnt = 1 TO dvdr_rpt_dtl_info->tbl_cnt)
      IF ((((dvdr_rpt_dtl_info->tbls[dvdr_cnt].status IN (dvdr_uow_status, dvdr_nocomp_size,
      dvdr_exclude, dvdr_error))) OR ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].excl_reason > " "))) )
       col 0,
       CALL print(concat(trim(substring(1,10,dvdr_rpt_dtl_info->tbls[dvdr_cnt].owner)),".",
        dvdr_rpt_dtl_info->tbls[dvdr_cnt].table_name)), col 40,
       "| ",
       CALL print(substring(1,190,trim(dvdr_rpt_dtl_info->tbls[dvdr_cnt].excl_reason))), row + 1
      ENDIF
    ENDFOR
   ENDIF
   IF ((dvdr_rpt_dtl_info->yet_to_be_compared > 0))
    row + 2,
    CALL print(fillstring(250,"-")), row + 1
    IF (dvdr_final_row_ind=1)
     "Tables Pending Final Compare", row + 1
    ELSE
     "Tables Pending Compare", row + 1
    ENDIF
    CALL print(fillstring(250,"-")), row + 2
    IF ((dvdr_rpt_dtl_info->tbls_compared=0))
     "Validation summary: No tables have been compared.", row + 2
    ENDIF
    IF ((dvdr_rpt_dtl_info->tbl_cnt > 0))
     col 0, "OWNER.TABLE_NAME", row + 1,
     CALL print(fillstring(250,"-")), row + 2
     FOR (dvdr_cnt = 1 TO dvdr_rpt_dtl_info->tbl_cnt)
       IF ((((dvdr_rpt_dtl_info->tbls[dvdr_cnt].status=dvdr_ready)
        AND (dvdr_rpt_dtl_info->tbls[dvdr_cnt].excl_reason="")) OR (((dvdr_final_row_ind=1
        AND (dvdr_rpt_dtl_info->tbls[dvdr_cnt].status=dvdr_match)) OR (dvdr_final_row_ind=1
        AND (dvdr_rpt_dtl_info->tbls[dvdr_cnt].status=dvdr_mismatch)
        AND (dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_compare_dt_tm < dvdr_rpt_dtl_info->
       final_row_dt_tm))) )) )
        col 0,
        CALL print(concat(trim(substring(1,10,dvdr_rpt_dtl_info->tbls[dvdr_cnt].owner)),".",
         dvdr_rpt_dtl_info->tbls[dvdr_cnt].table_name)), row + 1
       ENDIF
     ENDFOR
    ENDIF
   ENDIF
   IF ((dvdr_rpt_dtl_info->tbls_matched > 0))
    row + 2,
    CALL print(fillstring(250,"-")), row + 1,
    "Matched Tables", row + 1,
    CALL print(fillstring(250,"-")),
    row + 2, col 1, " ",
    col 41, "| LAST MAX KEY ", col 56,
    "| CURRENT MAX ", col 71, "| LAST COMPARE ",
    col 88, "| LAST SRC MOD ", col 106,
    "| ORIGINAL MIN ", col 121, "| LAST MIN KEY ",
    col 136, "| # KEYS ", col 151,
    "| TOTAL # KEYS ", col 166, "| LAST # KEYS ",
    row + 1, col 1, "OWNER.TABLE_NAME",
    col 41, "| COMPARED ", col 56,
    "| KEY ID ", col 71, "| DT / TM ",
    col 88, "| TIMESTAMP ", col 106,
    "| KEY ID ", col 121, "| COMPARED ",
    col 136, "| REMAINING ", col 151,
    "| COMPARED ", col 166, "| COMPARED ",
    row + 1,
    CALL print(fillstring(250,"-")), row + 2
    FOR (dvdr_cnt = 1 TO dvdr_rpt_dtl_info->tbl_cnt)
      IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].status=dvdr_match))
       col 0,
       CALL print(concat(trim(substring(1,10,dvdr_rpt_dtl_info->tbls[dvdr_cnt].owner)),".",
        dvdr_rpt_dtl_info->tbls[dvdr_cnt].table_name))
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_comp_keyid_max > 9999999999999.0))
       ...
       ELSE
        ...
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_compare_dt_tm,"DD-MMM-YY HH:MM;;D"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_src_mod_dt_tm=cnvtdatetime("01-JAN-1900")))
        col 88, "| ", "N/A"
       ELSE
        col 88, "| ",
        CALL print(format(dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_src_mod_dt_tm,"DD-MMM-YY HH:MM;;D"))
       ENDIF
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].orig_start_key > 9999999999999.0))
        col 106, "| ", "*************"
       ELSEIF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].orig_start_key=- (1)))
   ...
   IF (dvdr_final_row_ind=1)
    row + 1,
    CALL print(fillstring(250,"-")), row + 1,
    "Final Compare Matched Tables", row + 1,
    CALL print(fillstring(250,"-")),
    row + 2
    IF ((dvdr_rpt_dtl_info->tbls_complete=0))
     "Validation summary: No tables compared during final compare.", row + 1
    ELSEIF ((dvdr_rpt_dtl_info->tbl_cnt > 0))
     col 1, " ", col 41,
     "| LAST COMPARE ", row + 1, col 1,
     "OWNER.TABLE_NAME", col 41, "| DT / TM ",
     row + 1,
     CALL print(fillstring(250,"-")), row + 2
     FOR (dvdr_cnt = 1 TO dvdr_rpt_dtl_info->tbl_cnt)
       IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].status=dvdr_complete))
        col 0,
        CALL print(concat(trim(substring(1,10,dvdr_rpt_dtl_info->tbls[dvdr_cnt].owner)),".",
         dvdr_rpt_dtl_info->tbls[dvdr_cnt].table_name))
        IF ((dvdr_rpt_dtl_info->tbls[dvdr_cnt].last_compare_dt_tm=cnvtdatetime("01-JAN-1900")))
 ...
 IF (dm2_disp_file(dvdr_rpt_file,"Summary Report")=0)
  GO TO exit_script
 ENDIF
 IF (check_error(dm_err->eproc))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_verify_data_rpt completed successfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 CALL final_disp_msg("dm2_verify_data_rpt")
END GO
