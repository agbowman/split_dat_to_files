CREATE PROGRAM acm_act_service_category_hist:dba
 ...
 FOR (index = 1 TO xref->act_cnt)
   SET reply->service_category_hist_qual[xref->act[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->act_cnt)),
  ...
  SET s.active_ind = xref->act[d.seq].active_ind, s.active_status_cd =
   ...
   s.active_status_dt_tm = cnvtdatetime(sysdate), s.updt_cnt = (s.updt_cnt+ 1), s.updt_dt_tm =
   cnvtdatetime(sysdate),
   s.updt_id = reqinfo->updt_id, s.updt_applctx = reqinfo->updt_applctx, s.updt_task = reqinfo->
   updt_task
  PLAN (d)
   JOIN (s
   WHERE (s.svc_cat_hist_id=acm_request->service_category_hist_qual[xref->act[d.seq].idx].
   svc_cat_hist_id))
  WITH nocounter, status(reply->service_category_hist_qual[xref->act[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->act_cnt)
   IF ((reply->service_category_hist_qual[xref->act[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,s.svc_cat_hist_id,acm_request->service_category_hist_qual[xref->act[
     t1].idx].svc_cat_hist_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,s.svc_cat_hist_id,acm_request->service_category_hist_qual[xref->
      act[t1].idx].svc_cat_hist_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->service_category_hist_qual[xref->act[index].idx].status=0))
