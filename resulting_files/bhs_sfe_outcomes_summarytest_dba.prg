CREATE PROGRAM bhs_sfe_outcomes_summarytest:dba
 PROMPT
 ...
 DECLARE expand_stop = i4 WITH noconstant(40), protect
 CALL echo("2")
 ...
 DECLARE node_name = vc
 SET x = 0
 SET node_name = curnode
 CALL echo(node_name)
 ...
 IF (( $LOCATION > 1))
  ...
  IF (mod(actual_size,40) != 0)
   SET expand_total = (actual_size+ (40 - mod(actual_size,40)))
  ...
  SELECT INTO "NL:"
   FROM encounter e,
    encntr_loc_hist el,
    (dummyt d  WITH seq = value((expand_total/ 40)))
   PLAN (d
    WHERE assign(expand_start,evaluate(d.seq,1,1,(expand_start+ 40)))
     AND assign(expand_stop,(expand_start+ 39)))
    JOIN (e
    WHERE expand(num,expand_start,expand_stop,e.encntr_id,formactlisttemp->qual[num].encntr_id)
     ...
     AND e.loc_facility_cd IN (bfmc, bfmcinptpsych)) OR (( $LOCATION=7))) )) )) )) )) )) ) OR (e
    .encntr_id=0)) )
    JOIN (el
    WHERE el.encntr_id=e.encntr_id
     AND ((el.encntr_id=0) OR ((((el.loc_nurse_unit_cd= $AUNIT)
     AND ( $AUNIT > 0)) OR (( $AUNIT=0)
     AND ((( $LOCATION=2)
   ...
   HEAD e.encntr_id
    IF (e.encntr_id > 0)
     countl = (countl+ 1), formactlisttemp->qual[countl].encntr_id = e.encntr_id, formactlisttemp->
     qual[countl].person_id = e.person_id
    ENDIF
   WITH counter
  ;end select
 ELSE
 ...
 SELECT INTO "NL:"
  ...
  FROM clinical_event ce,
   (dummyt d  WITH seq = size(formactlist->qual,5))
  PLAN (d)
   JOIN (ce
   WHERE operator(ce.reference_nbr,"like",patstring(formactlist->qual[d.seq].dcp_forms_activity_idvc,
     1))
    AND ((ce.encntr_id+ 0)=formactlist->qual[d.seq].encntr_id)
  ...
  HEAD REPORT
   ...
   count = 0
  HEAD ce.encntr_id
   CALL echo(ce.encntr_id), sfeinfo->personcnt = (sfeinfo->personcnt+ 1), col + 1,
   "_________________________________________________________________________", row + 1, count = (
   count+ 1)
   IF (mod(count,1000)=1
    AND count != 1)
  ...
  HEAD o.catalog_cd
   ...
   IF (pos > 0)
    IF (o.catalog_cd=patientwantsreferralquitsmokingtask)
     CALL echo("mlhtask not compl"), sfeinfo->mlhfmcpatientwantsrefer = (sfeinfo->
     mlhfmcpatientwantsrefer+ 1)
     IF (ta.task_status_cd=taskcomplete)
      CALL echo("task not compl"), sfeinfo->mlhfmcpatientwantsrefercompl = (sfeinfo->
      mlhfmcpatientwantsrefercompl+ 1)
     ENDIF
    ELSEIF (o.catalog_cd=patientwantsreferraltoquitsmokingtask)
     CALL echo("bmctask not compl"), sfeinfo->bmcpatientwantsrefer = (sfeinfo->bmcpatientwantsrefer+
     1)
     IF (ta.task_status_cd=taskcomplete)
      CALL echo("bmctask  compl"), sfeinfo->bmcpatientwantsrefercompl = (sfeinfo->
      bmcpatientwantsrefercompl+ 1)
     ENDIF
    ELSEIF (o.catalog_cd IN (cpgnicotinereplacementtherapy, nicotine, bupropion, varenicline))
     CALL echo(build2(nrtgiven,"_",sfeinfo->detailqual[pos].requestnrt))
     IF ((((nrtgiven < sfeinfo->detailqual[pos].requestnrt)) OR (nrtgiven=0
      AND (sfeinfo->detailqual[pos].requestnrt=0))) )
      nrtgiven = (nrtgiven+ 1),
      CALL echo("@@@@@@@@@@@"),
      CALL echo(build("nrtGiven",nrtgiven)),
