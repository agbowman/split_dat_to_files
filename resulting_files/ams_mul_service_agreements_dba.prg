CREATE PROGRAM ams_mul_service_agreements:dba
 PROMPT
  ...
  HEAD r.line
   ...
   IF (size(trim(line1),1) > 0)
    ...
    IF (count > 1)
     ...
     ENDIF
     file_content->qual[row_count].health_plan_name = piece(line1,",",1,"Not Found"), file_content->
     qual[row_count].service_agree_name = piece(line1,",",2,"Not Found")
    ENDIF
   ENDIF
  FOOT REPORT
 ...
 IF (( $AUDITCOMMIT="Audit"))
  SELECT INTO  $OUTDEV
   health_plan_name = substring(1,30,file_content->qual[d1.seq].health_plan_name), service_agree_name
    = substring(1,30,file_content->qual[d1.seq].service_agree_name)
   FROM (dummyt d1  WITH seq = value(size(file_content->qual,5)))
   PLAN (d1)
   WITH nocounter, separator = " ", format
  ;end select
 ELSE
  EXECUTE ams_mul_service_agree_main:dba
