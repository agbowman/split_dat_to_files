CREATE PROGRAM dm_txtfnd_oen_script
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 DECLARE dtos_cnt = i4 WITH protect, noconstant(0)
 DECLARE dtos_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dtos_emsg = vc WITH protect, noconstant(" ")
 DECLARE dtos_det_name = vc WITH protect, noconstant(" ")
 DECLARE dtos_flag = i4 WITH protect, noconstant(0)
 DECLARE dtos_idx = i4 WITH protect, noconstant(0)
 DECLARE dtos_pos = i4 WITH protect, noconstant(0)
 DECLARE dtos_num = i4 WITH protect, noconstant(0)
 SET modify maxvarlen 268435456
 FREE RECORD dtos_prog
 RECORD dtos_prog(
   1 cnt = i4
   1 qual[*]
     2 line_data = vc
 )
 FREE RECORD dtos_attrib
 RECORD dtos_attrib(
   1 cnt = i4
   1 qual[*]
     2 column_name = vc
 ...
 IF (check_logfile("dm_tf_oen",".log","DM_TXTFND_OEN_SCRIPT LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_TXTFND_OEN_SCRIPT..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET modify maxvarlen 268435456
 ...
 IF (dtla_detail_id=0.0)
  ...
  SELECT INTO "nl:"
   ...
   WHERE d.detail_type_flag=2
    AND d.active_ind=1
    AND d.detail_script_name="DM_TXTFND_OEN_SCRIPT"
   DETAIL
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "No active data found in DM_TEXT_FIND_DETAIL for script DM_TXTFND_OEN_SCRIPT."
   IF ((script_rec->dm_text_find_log_id > 0.0))
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
   ENDIF
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "NL:"
  ...
  DETAIL
   dtos_det_name = d.find_name, dtos_flag = d2.detail_type_flag
  WITH nocounter
 ;end select
 IF (check_error("Getting DM_TEXT_FIND information") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_TXTFND_ATTRIBUTE:OEN_SCRIPT"
   AND  EXISTS (
  (SELECT
   "x"
   FROM user_tab_columns utc
   WHERE utc.table_name="OEN_SCRIPT"
    AND utc.column_name=di.info_name))
  ORDER BY di.info_name
  DETAIL
   dtos_attrib->cnt = (dtos_attrib->cnt+ 1), stat = alterlist(dtos_attrib->qual,dtos_attrib->cnt),
   dtos_attrib->qual[dtos_attrib->cnt].column_name = di.info_name,
   dtos_attrib->qual[dtos_attrib->cnt].column_label = di.info_char, dtos_attrib->qual[dtos_attrib->
   cnt].column_flg = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No attribute data found for OEN_SCRIPT."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for Attribute list:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Gathering OEN scripts"
 ...
 DECLARE to_date() = dq8
 SELECT INTO "nl:"
  os.script_name, dtos_dt = to_date(concat(nullval(os.updt_date,format(curdate,"MM/DD/YYYY;;D"))," ",
    nullval(os.updt_time,format(curtime,"HH:MM:SS;;M"))),"MM/DD/YYYY HH24:MI:SS")
  FROM oen_script os
  WHERE (( NOT ( EXISTS (
  (SELECT
   "x"
   ...
   WHERE d.dm_text_find_detail_id=dtla_detail_id
    AND d.script_name=trim(cnvtupper(os.script_name))
    AND d.find_name=dtos_det_name)))) OR ( EXISTS (
  (SELECT
   "x"
   FROM dm_text_find_data dt
   WHERE dt.dm_text_find_detail_id=dtla_detail_id
    AND dt.script_name=trim(cnvtupper(os.script_name))
    AND dt.find_name=dtos_det_name
    AND dt.compile_dt_tm < to_date(concat(nullval(os.updt_date,format(curdate,"MM/DD/YYYY;;D"))," ",
     nullval(os.updt_time,format(curtime,"HH:MM:SS;;M"))),"MM/DD/YYYY HH24:MI:SS")))))
  HEAD REPORT
   script_rec->dm_text_find_detail_id = dtla_detail_id, dtos_cnt = 0
  DETAIL
   dtos_cnt = (dtos_cnt+ 1)
   IF (mod(dtos_cnt,10)=1)
    stat = alterlist(dtg->qual,(dtos_cnt+ 9))
   ENDIF
   dtg->qual[dtos_cnt].s_name = cnvtupper(os.script_name), dtg->qual[dtos_cnt].compile_dt_tm =
   dtos_dt
  FOOT REPORT
   dtg->cnt = dtos_cnt, stat = alterlist(dtg->qual,dtg->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for Open Engine scripts:",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 IF ((dtg->cnt > 0))
  CALL echorecord(dtos_attrib)
  SET dm_err->eproc = "Gathering attribute information from oen_script"
  CALL parser("select into 'nl:'",0)
  CALL parser("  from oen_script os",0)
  CALL parser(
   " where expand(dtos_num, 1, dtg->cnt,cnvtupper(os.script_name), dtg->qual[dtos_num].s_name)",0)
  CALL parser(" order by os.script_name")
  CALL parser("head os.script_name",0)
  CALL parser(
   "   dtos_pos = locateval(dtos_num, 1, dtg->cnt,cnvtupper(os.script_name), dtg->qual[dtos_num].s_name) ",
   0)
  CALL parser("detail",0)
  CALL parser("   dtg->qual[dtos_pos].data_source = os.script_desc",0)
  FOR (dtos_idx = 1 TO dtos_attrib->cnt)
    CALL parser("   dtg->qual[dtos_pos].att_cnt = dtg->qual[dtos_pos].att_cnt + 1",0)
    CALL parser("   stat = alterlist(dtg->qual[dtos_pos].att_qual, dtg->qual[dtos_pos].att_cnt)",0)
    CALL parser(concat(
      "      dtg->qual[dtos_pos].att_qual[dtg->qual[dtos_pos].att_cnt].tab_name = 'OEN_SCRIPT'"),0)
    CALL parser(concat(
      "   dtg->qual[dtos_pos].att_qual[dtg->qual[dtos_pos].att_cnt].col_name = dtos_attrib->qual[",
      trim(cnvtstring(dtos_idx)),"].column_name"),0)
    CALL parser(concat(
      "   dtg->qual[dtos_pos].att_qual[dtg->qual[dtos_pos].att_cnt].col_txt = dtos_attrib->qual[",
      trim(cnvtstring(dtos_idx)),"].column_label"),0)
    IF ((dtos_attrib->qual[dtos_idx].column_flg=3))
     CALL parser(concat(
       "   dtg->qual[dtos_pos].att_qual[dtg->qual[dtos_pos].att_cnt].col_val = format(os.",
       dtos_attrib->qual[dtos_idx].column_name,",';;q')"),0)
    ELSEIF ((dtos_attrib->qual[dtos_idx].column_flg=2))
     CALL parser(concat(
       "   dtg->qual[dtos_pos].att_qual[dtg->qual[dtos_pos].att_cnt].col_val = cnvtstring(os.",
       dtos_attrib->qual[dtos_idx].column_name,",20)"),0)
    ELSE
     CALL parser(concat("   dtg->qual[dtos_pos].att_qual[dtg->qual[dtos_pos].att_cnt].col_val = os.",
       dtos_attrib->qual[dtos_idx].column_name),0)
    ENDIF
  ENDFOR
  CALL parser("with nocounter go",1)
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Gathering object attributes:",
     dm_err->emsg),0.0,0.0)
   GO TO exit_script
  ENDIF
  ...
  ENDIF
 ENDIF
 FOR (dtos_idx = 1 TO script_rec->script_cnt)
   SET stat = initrec(dtos_prog)
   SET dm_err->eproc = "Gathering script body from OEN_SCRIPT"
   SELECT INTO "NL:"
    bloblen = textlen(os.script_body)
    FROM oen_script os
    WHERE (cnvtupper(os.script_name)=script_rec->script_qual[dtos_idx].script_name)
    HEAD REPORT
     outbuf = fillstring(32767," "), offset = 0, retlen = 0
    DETAIL
     retlen = blobget(outbuf,offset,os.script_body), dtos_prog->cnt = 1, stat = alterlist(dtos_prog->
      qual,1),
     dtos_prog->qual[1].line_data = " "
     WHILE (retlen > 0)
       IF ((dtos_prog->qual[1].line_data=" "))
        dtos_prog->qual[1].line_data = notrim(outbuf)
       ELSE
        dtos_prog->qual[1].line_data = notrim(concat(notrim(dtos_prog->qual[1].line_data),notrim(
           substring(1,retlen,outbuf))))
       ENDIF
       offset = (offset+ retlen), retlen = blobget(outbuf,offset,os.script_body)
     ENDWHILE
     dtos_prog->qual[1].line_data = trim(dtos_prog->qual[1].line_data)
   ...
   IF (check_error(dm_err->eproc) != 0)
    ...
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for script body:",
      dm_err->emsg),0.0,0.0)
    SET dtos_err_ind = dm_err->err_ind
    SET dtos_emsg = dm_err->emsg
    SET dm_err->err_ind = 0
   ELSE
    CALL dst_insert_data(dtos_idx,script_rec,dtos_det_name,dtos_prog,dtos_flag)
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dtos_err_ind = dm_err->err_ind
     SET dtos_emsg = dm_err->emsg
 ...
 ENDFOR
#exit_script
 SET dm_err->eproc = "...Ending DM_TXTFND_OEN_SCRIPT"
 CALL final_disp_msg("dm_tf_oen")
 IF (dtos_err_ind=1
  AND (dm_err->err_ind=0))
  SET dm_err->err_ind = dtos_err_ind
  SET dm_err->emsg = dtos_emsg
 ENDIF
END GO
