CREATE PROGRAM bed_aud_hlx_assay_event_reltn:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=106
    ...
    AND cv.active_ind=1)
  DETAIL
   hlx = cv.code_value, act_type = cv.display
  WITH nocounter
 ;end select
 SELECT INTO "NL:"
  FROM code_value cv
 ...
 IF ((request->skip_volume_check_ind=0))
  SELECT INTO "nl:"
   ...
   PLAN (dta
    WHERE dta.activity_type_cd=hlx
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("hlx_assays_to_evntcd_evntset.csv")
 ENDIF
