CREATE PROGRAM acm_add_person_rx_plan_coverag:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
  IF ((acm_request->person_rx_plan_coverage_qual[xref->add[index].idx].person_rx_plan_coverage_id <=
  0.0))
   SELECT INTO "nl:"
    nextseqnum = seq(person_seq,nextval)
    FROM dual
    DETAIL
     acm_request->person_rx_plan_coverage_qual[xref->add[index].idx].person_rx_plan_coverage_id =
     cnvtreal(nextseqnum), reply->person_rx_plan_coverage_qual[xref->add[index].idx].
     person_rx_plan_coverage_id = cnvtreal(nextseqnum)
    WITH nocounter, format
   ;end select
   IF (curqual=0)
    SET reply->person_rx_plan_coverage_qual[xref->add[index].idx].status = gen_nbr_error
    SET failed = gen_nbr_error
    GO TO exit_script
   ENDIF
  ELSE
   SET reply->person_rx_plan_coverage_qual[xref->add[index].idx].person_rx_plan_coverage_id =
   acm_request->person_rx_plan_coverage_qual[xref->add[index].idx].person_rx_plan_coverage_id
  ENDIF
  SET reply->person_rx_plan_coverage_qual[xref->add[index].idx].status = 0
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   person_rx_plan_coverage p
  SET p.person_rx_plan_coverage_id = acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].
   person_rx_plan_coverage_id, p.beg_service_dt_tm =
   IF ((acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].beg_service_dt_tm > 0))
    cnvtdatetime(acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].beg_service_dt_tm)
   ELSE null
   ENDIF
   , p.coverage_status_cd = acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].
   coverage_status_cd,
   p.end_service_dt_tm =
   IF ((acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].end_service_dt_tm > 0))
    cnvtdatetime(acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].end_service_dt_tm)
   ELSE null
   ENDIF
   , p.person_rx_plan_reltn_id =
   IF ((acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].person_rx_plan_reltn_id > 0))
     acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].person_rx_plan_reltn_id
   ELSE reply->person_rx_plan_reltn_qual[acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].
    idx].person_rx_plan_reltn_idx].person_rx_plan_reltn_id
   ENDIF
   , p.service_type_cd = acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].
   service_type_cd,
   p.service_type_txt = acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].
   service_type_txt, p.active_ind =
   IF ((((acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR (
   (acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].active_status_cd=reqdata->
   active_status_cd))) ) 1
   ELSE 0
   ENDIF
   , p.active_status_cd =
   IF ((acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].active_status_cd > 0.0))
    acm_request->person_rx_plan_coverage_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->person_rx_plan_coverage_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->person_rx_plan_coverage_qual[xref->add[index].idx].status=0))
