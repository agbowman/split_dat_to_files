CREATE PROGRAM cern_dcp_rpt_diet_orders:dba
 RECORD temp(
   1 cnt = i2
   1 qual[*]
     2 order_id = f8
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   age = trim(cnvtage(cnvtdate(p.birth_dt_tm),curdate),3), dob = concat(format(datetimezone(p
     ...
     .birth_dt_tm)), name = substring(1,30,p.name_full_formatted),
   ...
   bed = substring(1,10,uar_get_code_display(e.loc_bed_cd)), location = concat(trim(unit),"/",trim(
     room),"/",trim(bed)), tz_index = datetimezonebyname(trim(t.time_zone)),
   offset = 0, daylight = 0, date = concat(format(datetimezone(e.reg_dt_tm,tz_index),"@SHORTDATE"),
    " ",datetimezonebyindex(tz_index,offset,daylight,7,datetimezone(e.reg_dt_tm,tz_index))),
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   temp->qual[temp->cnt].mnemonic = o.hna_order_mnemonic, temp->qual[temp->cnt].date = concat(format(
     datetimezone(o.orig_order_dt_tm,o.orig_order_tz),"@SHORTDATETIME")," ",datetimezonebyindex(o
     .orig_order_tz,offset,daylight,7,o.orig_order_dt_tm)), temp->qual[temp->cnt].status =
 ...
 SELECT INTO request->output_device
  ...
  HEAD PAGE
   ...
   "{pos/30/110}{b}{u}Date", row + 1, "{pos/130/110}{b}{u}Orderable",
   row + 1, "{pos/250/110}{b}{u}Details/Comments", row + 1,
  ...
  DETAIL
   FOR (x = 1 TO temp->cnt)
     ...
     temp->qual[x].status, row + 1, xcol = 130,
     scol = ycol
