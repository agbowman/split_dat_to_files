CREATE PROGRAM ags_pvd_org_load:dba
 PROMPT
  "TASK_ID (0.0) = " = 0
  WITH dtask_id
 CALL echo("***")
 CALL echo("***   BEG: AGS_PVD_ORG_LOAD")
 CALL echo("***")
 ...
 IF ((validate(failed,- (1))=- (1)))
  ...
  DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_pvd_org_load_",format(
  ...
  SET log->qual[log->qual_knt].smsg = "BEG >> AGS_PVD_ORG_LOAD"
 ...
 DECLARE icombine = i2 WITH protect, constant(4)
 DECLARE imove = i2 WITH protect, constant(5)
 DECLARE isplit = i2 WITH protect, constant(6)
 DECLARE ierror = i2 WITH protect, constant(7)
 DECLARE ispecial = i2 WITH protect, constant(8)
 ...
 DECLARE ifoundspecialaction = i2 WITH public, noconstant(false)
 FREE RECORD wlist
 RECORD wlist(
   1 qual_knt = i4
   1 qual[*]
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   icutoverflag = cnvtint(di.info_number)
  WITH nocounter
 ;end select
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
 ...
 IF (icutoverflag < 1)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "Validate Cut-Over :: Input Error :: Migration Not Complete"
  SET serrmsg = log->qual[log->qual_knt].smsg
  GO TO exit_script
 ENDIF
 CALL echo("***")
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (a
   WHERE a.ags_job_id=djobid
    ...
    AND a.primary_ind=1
    AND a.status IN ("WAITING", "IN ERROR", "BACKOUT", "LOADING"))
 ...
 WHILE (sbeglinkalias < sendlinkalias
  AND imaincontinue=true)
   ...
   SET stat = initrec(prev_rec)
   SET log->qual_knt = (log->qual_knt+ 1)
   SET stat = alterlist(log->qual,log->qual_knt)
   SET log->qual[log->qual_knt].smsgtype = "INFO"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("sBegLinkAlias : ",trim(cnvtstring(sbeglinkalias)))
   SET log->qual_knt = (log->qual_knt+ 1)
   SET stat = alterlist(log->qual,log->qual_knt)
   SET log->qual[log->qual_knt].smsgtype = "INFO"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("sEndLinkAlias : ",trim(cnvtstring(sendlinkalias)))
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   SELECT DISTINCT INTO "nl:"
    FROM ags_org_data a
    ...
    HEAD a.provdir_link_alias
     ...
     provdir_alias = trim(a.provdir_alias,3), wlist->qual[knt].ags_org_data_id = a.ags_org_data_id,
     wlist->qual[knt].run_nbr = a.run_nbr, wlist->qual[knt].org_id = a.organization_id, wlist->qual[
     knt].location_cd = a.location_cd,
     wlist->qual[knt].name = trim(a.name,3)
    ...
    IF ((contrib_rec->list_knt > 0))
      ...
      IF ((wlist->qual[data_rec->qual[kidx].wlist_idx].contrib_idx > 0))
       ...
       ELSE
        ...
        contrib_idx].orglnkuuid_type_cd < 1))) )
         SET wlist->qual[data_rec->qual[kidx].wlist_idx].status = "IN ERROR"
         SET wlist->qual[data_rec->qual[kidx].wlist_idx].stat_msg = concat(trim(wlist->qual[data_rec
           ->qual[kidx].wlist_idx].stat_msg),"[pla-pool-type]")
        ENDIF
       ENDIF
      ENDIF
     ...
     FOR (kidx = 1 TO data_rec->qual_knt)
       ...
       IF ((data_rec->qual[kidx].status="IN ERROR"))
        ...
        SET data_rec->qual[kidx].action_flag = ierror
     ...
     SELECT INTO "nl:"
      ...
      PLAN (d
       WHERE (data_rec->qual[d.seq].contrib_idx > 0)
        AND (data_rec->qual[d.seq].action_flag != ierror)
        AND (data_rec->qual[d.seq].provdir_link_alias > " ")
        AND (data_rec->qual[d.seq].provdir_alias > " "))
       JOIN (a
       WHERE (a.provdir_alias=data_rec->qual[d.seq].provdir_alias)
        AND (a.run_nbr < data_rec->qual[d.seq].run_nbr))
      ORDER BY a.provdir_link_alias, a.provdir_alias, a.run_nbr
      FOOT  a.provdir_alias
       data_rec->qual[d.seq].old_pla = trim(a.provdir_link_alias,3)
       IF ((data_rec->qual[d.seq].old_pla != data_rec->qual[d.seq].provdir_link_alias))
        data_rec->qual[d.seq].action_flag = isplit
       ENDIF
      WITH nocounter
     ;end select
     SET ierrcode = error(serrmsg,1)
     IF (ierrcode > 0)
      ...
      SET log->qual[log->qual_knt].smsgtype = "ERROR"
      SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
      SET log->qual[log->qual_knt].smsg = concat("Load Previous State :: Select Error :: ",trim(
        serrmsg))
      SET serrmsg = log->qual[log->qual_knt].smsg
      GO TO exit_script
     ENDIF
     ...
     SELECT INTO "nl:"
      ...
      PLAN (d
       WHERE (data_rec->qual[d.seq].contrib_idx > 0)
        AND (data_rec->qual[d.seq].action_flag=isplit)
        AND (data_rec->qual[d.seq].provdir_link_alias > " ")
        AND (data_rec->qual[d.seq].provdir_alias > " "))
       JOIN (a
       WHERE (a.provdir_link_alias=data_rec->qual[d.seq].provdir_link_alias)
        AND (a.run_nbr < data_rec->qual[d.seq].run_nbr)
        AND a.primary_ind=1)
      ORDER BY a.provdir_link_alias, a.provdir_alias, a.run_nbr
      FOOT  a.provdir_link_alias
       data_rec->qual[d.seq].action_flag = imove
      WITH nocounter
     ;end select
     SET ierrcode = error(serrmsg,1)
     IF (ierrcode > 0)
     ...
     SELECT INTO "nl:"
      ...
      PLAN (d
       ...
       JOIN (a
       WHERE (a.provdir_link_alias=wlist->qual[d.seq].provdir_link_alias)
        AND (a.run_nbr < wlist->qual[d.seq].run_nbr))
      ORDER BY d.seq, a.run_nbr, a.ags_org_data_id
      FOOT  d.seq
       IF (a.organization_id > 0)
        wlist->qual[d.seq].org_id = a.organization_id, wlist->qual[d.seq].location_cd = a.location_cd
        IF ((a.name != wlist->qual[d.seq].name))
         wlist->qual[d.seq].org_action_flag = iupdate, wlist->qual[d.seq].location_action_flag =
         iupdate
        ELSE
         wlist->qual[d.seq].org_action_flag = inoaction, wlist->qual[d.seq].location_action_flag =
         inoaction
        ENDIF
       ENDIF
      WITH nocounter
     ...
     IF (ierrcode > 0)
      ...
      SET log->qual[log->qual_knt].smsgtype = "ERROR"
      SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
      SET log->qual[log->qual_knt].smsg = concat("Check ORG AGS Table :: Select Error :: ",trim(
        serrmsg))
      SET serrmsg = log->qual[log->qual_knt].smsg
      GO TO exit_script
     ENDIF
     ...
     IF (ierrcode > 0)
      ...
      SET log->qual[log->qual_knt].smsgtype = "ERROR"
      SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
      SET log->qual[log->qual_knt].smsg = concat("Check ORG Alias Table :: Select Error :: ",trim(
        serrmsg))
      SET serrmsg = log->qual[log->qual_knt].smsg
      GO TO exit_script
     ENDIF
     CALL echo("***")
     CALL echo("***   Does Address Exist")
     CALL echo("***")
     SET ierrcode = error(serrmsg,1)
     SET ierrcode = 0
     SELECT INTO "nl:"
      ...
      PLAN (d
       WHERE (data_rec->qual[d.seq].contrib_idx > 0)
        AND (((data_rec->qual[d.seq].address_id < 1)) OR ((((data_rec->qual[d.seq].bus_phone_id < 1))
        OR ((data_rec->qual[d.seq].fax_phone_id < 1))) )) )
       JOIN (a
       WHERE (a.provdir_alias=data_rec->qual[d.seq].provdir_alias)
        AND (a.provdir_link_alias=data_rec->qual[d.seq].provdir_link_alias)
        AND (a.run_nbr < data_rec->qual[d.seq].run_nbr))
      ORDER BY d.seq, a.run_nbr, a.ags_org_data_id
      FOOT  d.seq
       IF (a.address_id > 0
        AND (data_rec->qual[d.seq].address_id < 1))
        data_rec->qual[d.seq].address_id = a.address_id
        IF ((data_rec->qual[d.seq].address_action_flag != idelete))
         data_rec->qual[d.seq].address_action_flag = iupdate
        ENDIF
       ENDIF
       IF (a.bus_phone_id > 0
        AND (data_rec->qual[d.seq].bus_phone_id < 1))
        data_rec->qual[d.seq].bus_phone_id = a.bus_phone_id
        IF ((data_rec->qual[d.seq].phone_action_flag != idelete))
         data_rec->qual[d.seq].phone_action_flag = iupdate
        ENDIF
       ENDIF
       IF (a.fax_phone_id > 0
        AND (data_rec->qual[d.seq].fax_phone_id < 1))
        data_rec->qual[d.seq].fax_phone_id = a.fax_phone_id
        IF ((data_rec->qual[d.seq].fax_action_flag != idelete))
         data_rec->qual[d.seq].fax_action_flag = iupdate
        ENDIF
       ENDIF
      WITH nocounter
     ;end select
     ...
     IF (ierrcode > 0)
      ...
      SET log->qual[log->qual_knt].smsgtype = "ERROR"
      SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
      SET log->qual[log->qual_knt].smsg = concat("Check Contact AGS Table :: Select Error :: ",trim(
        serrmsg))
      SET serrmsg = log->qual[log->qual_knt].smsg
      GO TO exit_script
     ENDIF
     ...
     FOR (midx = 1 TO data_rec->qual_knt)
       IF ((data_rec->qual[midx].primary_ind=1)
        AND (data_rec->qual[midx].action_flag IN (isplit, imove)))
        SET wlist->qual[data_rec->qual[midx].wlist_idx].org_action_flag = data_rec->qual[midx].
        action_flag
        SET wlist->qual[data_rec->qual[midx].wlist_idx].location_action_flag = data_rec->qual[midx].
        action_flag
       ENDIF
       IF ((wlist->qual[data_rec->qual[midx].wlist_idx].org_id < 1)
        AND  NOT ((wlist->qual[data_rec->qual[midx].wlist_idx].org_action_flag IN (isplit, imove))))
        SET ierrcode = error(serrmsg,1)
        SET ierrcode = 0
        SELECT INTO "nl:"
       ...
       IF ((wlist->qual[data_rec->qual[midx].wlist_idx].location_cd < 1)
        AND  NOT ((wlist->qual[data_rec->qual[midx].wlist_idx].location_action_flag IN (isplit, imove
       ))))
        SET ierrcode = error(serrmsg,1)
        SET ierrcode = 0
        SELECT INTO "nl:"
     ...
     INSERT  FROM organization o,
      ...
      SET o.organization_id = wlist->qual[d.seq].org_id, o.contributor_system_cd = contrib_rec->list[
       ...
       qual[d.seq].contrib_idx].contributor_system_cd, o.active_status_dt_tm = cnvtdatetime(dates->
        now_dt_tm),
     ...
     SELECT INTO "nl:"
      ...
      PLAN (d
       WHERE (wlist->qual[data_rec->qual[oa_rec->qual[d.seq].data_rec_idx].wlist_idx].org_id > 0)
        AND  NOT ((data_rec->qual[oa_rec->qual[d.seq].data_rec_idx].action_flag IN (isplit, imove))))
       JOIN (a
     ...
     INSERT  FROM organization_alias o,
      ...
      PLAN (d
       WHERE (oa_rec->qual[d.seq].action_flag=iinsert)
        ...
        AND  NOT ((wlist->qual[data_rec->qual[oa_rec->qual[d.seq].data_rec_idx].wlist_idx].
       org_action_flag IN (isplit, imove)))
     ...
     INSERT  FROM address a,
      ...
      PLAN (d
       WHERE (data_rec->qual[d.seq].address_id > 0)
        ...
        AND  NOT ((data_rec->qual[d.seq].action_flag IN (isplit, imove))))
       JOIN (a)
      WITH nocounter
     ;end insert
     SET ierrcode = error(serrmsg,1)
     ...
     UPDATE  FROM address a,
       (dummyt d  WITH seq = value(data_rec->qual_knt))
      SET a.address_type_seq =
       IF ((data_rec->qual[d.seq].primary_ind=1)) 0
       ELSE 99
       ENDIF
       , a.street_addr = data_rec->qual[d.seq].street_addr, a.street_addr2 = data_rec->qual[d.seq].
       street_addr2,
       a.city = data_rec->qual[d.seq].city, a.state = data_rec->qual[d.seq].state, a.zipcode =
       data_rec->qual[d.seq].zipcode,
       a.county = data_rec->qual[d.seq].county, a.country = data_rec->qual[d.seq].country, a
       .contributor_system_cd = contrib_rec->list[data_rec->qual[d.seq].contrib_idx].
       contributor_system_cd,
       a.beg_effective_dt_tm = cnvtdatetime(dates->now_dt_tm), a.end_effective_dt_tm = cnvtdatetime(
        dates->end_dt_tm), a.updt_cnt = (a.updt_cnt+ 1),
       a.updt_dt_tm = cnvtdatetime(dates->now_dt_tm), a.updt_id = contrib_rec->list[data_rec->qual[d
       .seq].contrib_idx].prsnl_person_id, a.updt_applctx = iappnumber,
       a.updt_task = iappnumber
      PLAN (d
     ...
     UPDATE  FROM phone p,
       (dummyt d  WITH seq = value(data_rec->qual_knt))
      SET p.phone_type_seq =
       IF ((data_rec->qual[d.seq].primary_ind=1)) 0
       ELSE 99
       ENDIF
       , p.phone_num = data_rec->qual[d.seq].phone_num, p.contributor_system_cd = contrib_rec->list[
       data_rec->qual[d.seq].contrib_idx].contributor_system_cd,
       p.beg_effective_dt_tm = cnvtdatetime(dates->now_dt_tm), p.end_effective_dt_tm = cnvtdatetime(
        dates->end_dt_tm), p.updt_cnt = (p.updt_cnt+ 1),
       p.updt_dt_tm = cnvtdatetime(dates->now_dt_tm), p.updt_id = contrib_rec->list[data_rec->qual[d
       .seq].contrib_idx].prsnl_person_id, p.updt_applctx = iappnumber,
       p.updt_task = iappnumber
      PLAN (d
       WHERE (data_rec->qual[d.seq].bus_phone_id > 0)
        AND (data_rec->qual[d.seq].contrib_idx > 0)
     ...
     INSERT  FROM phone p,
      ...
      SET p.phone_id = data_rec->qual[d.seq].fax_phone_id, p.parent_entity_name = "ORGANIZATION", p
       ...
       ENDIF
       , p.phone_format_cd = 0,
       p.phone_num = data_rec->qual[d.seq].fax_num, p.description = "", p.contact = "",
       p.call_instruction = "", p.modem_capability_cd = 0, p.extension = "",
       p.paging_code = "", p.beg_effective_mm_dd = 0, p.end_effective_mm_dd = 0,
       p.contributor_system_cd = contrib_rec->list[data_rec->qual[d.seq].contrib_idx].
       contributor_system_cd, p.data_status_cd = dauthdatastatuscd, p.data_status_dt_tm =
     ...
     UPDATE  FROM phone p,
      ...
      SET p.phone_type_seq =
       ...
       ELSE 99
       ENDIF
       , p.phone_num = data_rec->qual[d.seq].fax_num, p.contributor_system_cd = contrib_rec->list[
       data_rec->qual[d.seq].contrib_idx].contributor_system_cd,
       p.beg_effective_dt_tm = cnvtdatetime(dates->now_dt_tm), p.end_effective_dt_tm = cnvtdatetime(
        dates->end_dt_tm), p.updt_cnt = (p.updt_cnt+ 1),
       p.updt_dt_tm = cnvtdatetime(dates->now_dt_tm), p.updt_id = contrib_rec->list[data_rec->qual[d
     ...
     UPDATE  FROM ags_org_data o,
       (dummyt d  WITH seq = value(data_rec->qual_knt))
      SET o.status = "HOLD", o.stat_msg =
       IF ((data_rec->qual[d.seq].action_flag=isplit)) concat("[SPLIT]",trim(data_rec->qual[d.seq].
          stat_msg))
       ELSE concat("[MOVE]",trim(data_rec->qual[d.seq].stat_msg))
       ENDIF
       , o.status_dt_tm = cnvtdatetime(dates->now_dt_tm)
      PLAN (d
       WHERE (data_rec->qual[d.seq].ags_org_data_id > 0)
        AND (data_rec->qual[d.seq].contrib_idx > 0)
        AND (data_rec->qual[d.seq].action_flag IN (isplit, imove)))
       JOIN (o
       WHERE (o.ags_org_data_id=data_rec->qual[d.seq].ags_org_data_id))
      WITH nocounter
     ;end update
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_PVD_ORG_LOAD"
 IF (define_logging_sub=true)
 ...
 ENDIF
 CALL echo("***")
 CALL echo("***   END: AGS_PVD_ORG_LOAD")
 CALL echo("***")
