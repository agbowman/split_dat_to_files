CREATE PROGRAM ct_pt_mp_add_prescreen_job:dba
 PROMPT
 ...
 SUBROUTINE (nextsequence(x=i2) =f8)
   ...
   RETURN(nsequence)
 END ;Subroutine
 RECORD eval_prescreen(
   1 person_id = f8
   1 screener_id = f8
   1 job_id = f8
 )
 SET log_program_name = "ct_pt_mp_add_prescreen_job"
 FREE RECORD request
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_pt_mp_add_prescreen_job","Invalid JSON Request",
   "reply")
  GO TO exit_script
 ENDIF
 ...
 DECLARE prescreen_job_id = f8 WITH protect, noconstant(0)
 DECLARE prescreen_job_info_id = f8 WITH protect, noconstant(0)
 DECLARE pending_job_status_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",17917,"PENDING")
  )
 DECLARE prescreen_job_type = i2 WITH protect, constant(3)
 SET reply->status_data.status = "F"
 IF ((request->person_id <= 0))
  SET reply->status_data.subeventstatus[1].targetobjectvalue = "Invalid patient ID"
  GO TO exit_script
 ENDIF
 SET prescreen_job_id = nextsequence(0)
 INSERT  FROM ct_prescreen_job cj
  SET cj.ct_prescreen_job_id = prescreen_job_id, cj.job_start_dt_tm = cnvtdatetime(sysdate), cj
   .job_status_cd = pending_job_status_cd,
   cj.job_type_flag = prescreen_job_type, cj.prsnl_id = reqinfo->updt_id, cj.updt_applctx = reqinfo->
   updt_applctx,
   cj.updt_cnt = 0, cj.updt_dt_tm = cnvtdatetime(sysdate), cj.updt_id = reqinfo->updt_id,
   cj.updt_task = reqinfo->updt_task
  WITH nocounter
 ;end insert
 IF (curqual=0)
  SET reply->status_data.subeventstatus[1].targetobjectvalue =
  "Error inserting row into ct_prescreen_job"
  GO TO exit_script
 ENDIF
 COMMIT
 SET prescreen_job_info_id = nextsequence(0)
 INSERT  FROM ct_prot_prescreen_job_info cpj
  SET cpj.ct_prot_prescreen_job_info_id = prescreen_job_info_id, cpj.ct_prescreen_job_id =
   prescreen_job_id, cpj.completed_flag = 0,
   cpj.person_id = request->person_id, cpj.prot_master_id = 0, cpj.updt_applctx = reqinfo->
   updt_applctx,
   cpj.updt_cnt = 0, cpj.updt_dt_tm = cnvtdatetime(sysdate), cpj.updt_id = reqinfo->updt_id,
   cpj.updt_task = reqinfo->updt_task
  WITH nocounter
 ;end insert
 IF (curqual=0)
  SET reply->status_data.subeventstatus[1].targetobjectvalue =
  "Error inserting row into ct_prot_prescreen_job_info"
  GO TO exit_script
 ENDIF
 COMMIT
 SET eval_prescreen->person_id = request->person_id
 SET eval_prescreen->screener_id = reqinfo->updt_id
 SET eval_prescreen->job_id = prescreen_job_id
 SET stat = tdbexecute(4150006,4150039,4159943,"REC",eval_prescreen,
  "REC",reply_out)
