CREATE PROGRAM dm2_dbstats_chk_rpt:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_dbstats_chk",".log","dm2_dbstats_chk")=0)
  GO TO exit_program
 ENDIF
 IF ((dm2_db_options->dbstats_chk_rpt="NOT_SET"))
  EXECUTE dm2_set_db_options
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
 ENDIF
 IF ((dm2_db_options->dbstats_chk_rpt="N"))
  IF (validate(docd_reply->status,"2") != "2")
   SET docd_reply->status = "C"
  ENDIF
  SET dm_err->eproc = "Database Statistics Report is not elected to be displayed."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 FREE RECORD drc_tbldoc
 RECORD drc_tbldoc(
   1 drc_tbl_cnt = i4
   1 qual[*]
     2 drc_tname = vc
 )
 DECLARE drc_tab_name = vc WITH protect, constant(cnvtupper( $1))
 DECLARE drc_num_days_old_stat = i4 WITH protect, noconstant( $2)
 DECLARE drc_row = i4 WITH protect, noconstant(0)
 DECLARE drc_oldstatsfound_ind = i2 WITH protect, noconstant(0)
 DECLARE drc_promptoptionselected = vc WITH protect, noconstant("C")
 DECLARE drc_display_oldstats_warning(ddow_mode_in=i2) = i2
 DECLARE drc_gen_oldstats_rpt(gor_report_prefix=vc) = i2
 DECLARE drc_cnt = i4 WITH protect, noconstant(0)
 DECLARE drc_pass = i2 WITH protect, noconstant(0)
 IF ((dm_err->debug_flag > 2)
  AND validate(dm2_force_threshold_days,72278) != 72278)
  SET drc_num_days_old_stat = dm2_force_threshold_days
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(build("drc_tab_name:",drc_tab_name))
  CALL echo(build("Days_old_stat:",drc_num_days_old_stat))
 ENDIF
 SET dm_err->eproc = "Validating days threshold"
 IF (drc_num_days_old_stat=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Days threshold is zero"
  SET dm_err->user_action = "Days threshold must be greater than zero"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ENDIF
 SET dm_err->eproc = "Gathering table doc information."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_tables_doc d
  WHERE d.full_table_name=patstring(drc_tab_name)
   AND d.full_table_name=d.table_name
  DETAIL
   drc_cnt = (drc_cnt+ 1)
   IF (mod(drc_cnt,10)=1)
    stat = alterlist(drc_tbldoc->qual,(drc_cnt+ 9))
   ENDIF
   drc_tbldoc->qual[drc_cnt].drc_tname = d.table_name
  FOOT REPORT
   drc_tbldoc->drc_tbl_cnt = drc_cnt, stat = alterlist(drc_tbldoc->qual,drc_cnt)
  WITH nocounter
 ;end select
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(drc_tbldoc)
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (((curqual=0) OR ((drc_tbldoc->drc_tbl_cnt=0))) )
  IF (currdbuser="V500")
   SET dm_err->emsg = "No tables selected for stats"
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ELSE
   SET dm_err->eproc =
   "Gathering table doc information - No tables selected for stats for current user."
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Gathering table stat times."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET drc_oldstatsfound_ind = 1
 SET drc_cnt = 0
 SELECT INTO "nl:"
  analyze_tab_ind = nullind(t.last_analyzed)
  FROM dm2_user_tables t,
   dm_tables_doc d
  WHERE t.table_name=d.table_name
   AND t.table_name=patstring(drc_tab_name)
  DETAIL
   IF (drc_oldstatsfound_ind=1)
    IF ((dm_err->debug_flag > 1))
     CALL echo("Check 2"),
     CALL echo(build("DIFF:",cnvtint(dm2ceil(datetimediff(cnvtdatetime(curdate,curtime3),t
         .last_analyzed))))),
     CALL echo(build("Days old in:",drc_num_days_old_stat)),
     CALL echo(build("Last analyzed:",format(t.last_analyzed,"dd-mmm-yyyy hh:mm:ss;;d"))),
     CALL echo(build("NULLIND:",analyze_tab_ind)),
     CALL echo(t.table_name)
    ENDIF
    IF (analyze_tab_ind=0
     AND cnvtint(dm2ceil(datetimediff(cnvtdatetime(curdate,curtime3),t.last_analyzed))) <=
    drc_num_days_old_stat)
     drc_oldstatsfound_ind = 0
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 ...
 IF (drc_oldstatsfound_ind=1)
  IF (dir_silmode_requested_ind=1)
   IF (drc_gen_oldstats_rpt(dm2_install_schema->logfile_prefix) != 1)
    GO TO exit_program
   ENDIF
  ELSE
   IF (drc_display_oldstats_warning(1) != 1)
    GO TO exit_program
   ENDIF
  ENDIF
 ELSE
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE drc_display_oldstats_warning(ddow_mode_in)
   DECLARE ddow_text = vc WITH protect, noconstant("")
   SET message = window
   SET width = 132
   CALL clear(1,1)
   SET drc_row = 1
   CALL text(drc_row,2,"DATABASE STATISTICS CHECK REPORT")
   SET drc_row = (drc_row+ 3)
   CALL text(drc_row,2,concat(
     "Database statistics for objects in this environment are not current (older than ",trim(
      cnvtstring(drc_num_days_old_stat))," days)."))
   SET drc_row = (drc_row+ 1)
   CALL text(drc_row,2,
    "Without current database statistics, the following operations could be adversely affected:")
   SET drc_row = (drc_row+ 1)
   CALL text(drc_row,2,
    "- Tablespace resizing estimates needed for successful schema installation may not be accurate.")
   SET drc_row = (drc_row+ 1)
   CALL text(drc_row,2,"- Index sizing parameters (initial and next) may be too small.")
   SET drc_row = (drc_row+ 1)
   CALL text(drc_row,2,"- Time estimates for readme steps may not be accurate.")
   SET drc_row = (drc_row+ 2)
   CALL text(drc_row,2,"Having current database statistics (within 30 days) is recommended.")
   SET drc_row = (drc_row+ 2)
   CALL text(drc_row,2,"Enter 'C' to continue the installation.")
   SET drc_row = (drc_row+ 1)
   CALL text(drc_row,2,"Enter 'Q' to quit installation.")
   SET drc_row = (drc_row+ 1)
   CALL text(drc_row,2,
    "Enter 'D' to discontinue this prompt in this environment (not recommended for PROD environments)."
    )
   SET drc_row = (drc_row+ 2)
   CALL text(drc_row,2,"Selection:")
   CALL accept(drc_row,13,"p;cu","Q"
    WHERE curaccept IN ("C", "Q", "D"))
   SET drc_promptoptionselected = curaccept
   SET dm_err->eproc = "Evaluating user selection."
   CASE (drc_promptoptionselected)
    OF "Q":
     IF (validate(docd_reply->status,"2") != "2"
     ...
     ELSE
      SET dm_err->emsg = "User has decided to continue installation at a later time."
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     ENDIF
     RETURN(0)
    OF "C":
     ...
     ELSE
      SET dm_err->eproc = "User has decided to continue installation without current statistics."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     ENDIF
    OF "D":
     SET dm_err->eproc = "Setting option to discontinue prompt display."
     UPDATE  FROM dm_info d
      SET d.info_char = "N", d.updt_task = 72278
      WHERE d.info_domain=concat("DM2_",currdb,"_DB_OPTION")
       AND d.info_name="DBSTATS_CHK_RPT"
      WITH nocounter
     ;end update
     IF (check_error(dm_err->eproc) > 0)
      ROLLBACK
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drc_gen_oldstats_rpt(gor_report_prefix)
   DECLARE ddcr_filename = vc WITH protect, noconstant("")
   SET ddcr_filename = concat(gor_report_prefix,"dbstatschk.log")
   SET dm_err->eproc = "Generating dbstats check report to a file"
   SELECT INTO value(ddcr_filename)
    FROM dummyt
    DETAIL
     col 1, "DATABASE STATISTICS CHECK REPORT", row + 1,
     row + 1, col 1,
     CALL print(concat(
      "Database statistics for objects in this environment are not current (older than ",trim(
       cnvtstring(drc_num_days_old_stat))," days).")),
     ...
     "- Tablespace resizing estimates needed for successful schema installation may not be accurate.",
     row + 1, col 1,
     "- Time estimates for readme steps may not be accurate.", row + 1, col 1,
     "- Time estimates for readme steps may not be accurate.", row + 1, row + 1,
     col 1, "Having current database statistics (within 30 days) is recommended.", row + 1,
     row + 1, col 1, "Silent Install requested, so this report was bypassed."
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   CALL dir_add_silmode_entry("Database Statistics Check Prompt",ddcr_filename)
   IF ( NOT (validate(docd_reply->status,"X")="X"
    AND validate(docd_reply->status,"Y")="Y"))
    SET docd_reply->status = "C"
   ENDIF
   SET dm_err->eproc = concat(
    "Silent Mode:  Database Statistics Check Prompt was bypassed.  Report exits",
    " in CCLUSERDIR under file name: ",ddcr_filename)
   CALL disp_msg("",dm_err->logfile,0)
   RETURN(1)
 END ;Subroutine
#exit_program
 SET message = nowindow
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM2_DBSTATS_CHK_RPT completed."
 ENDIF
 CALL final_disp_msg("dm2_dbstats_chk")
 FREE RECORD drc_tbldoc
 IF ((dm_err->debug_flag > 0))
