CREATE PROGRAM dm_mic_ang_status_rows:dba
 ...
 SET reply->table_name = "MIC_ANG_STATUS"
 SET reply->rows_between_commit = 20
 DECLARE purge_dt_tm = dq8 WITH protect, noconstant(0.0)
 DECLARE days_to_keep = i4 WITH protect, noconstant(0)
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(mas.ang_run_id)
   FROM mic_ang_status mas
   WHERE mas.ang_run_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    mas.rowid
    FROM mic_ang_status mas
    WHERE parser(sbr_getrowidnotexists("mas.ang_run_id between curMinID and curMaxID","mas"))
     AND mas.run_dt_tm <= cnvtdatetime(purge_dt_tm)
     AND  NOT ( EXISTS (
    (SELECT
     msr.ang_run_id
     FROM mic_act_ang_sum_rpt msr
     WHERE msr.ang_run_id=mas.ang_run_id
      AND  EXISTS (
     (SELECT
      maa.order_id
      FROM mic_ang_automated maa
      WHERE maa.order_id=msr.order_id
       AND maa.complete_ind=0)))))
     AND  NOT ( EXISTS (
    (SELECT
     maa.ang_run_id
     FROM mic_ang_automated maa
     WHERE maa.ang_run_id=mas.ang_run_id
      AND maa.complete_ind=0)))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
     ...
     ENDIF
     reply->rows[rows].row_id = mas.rowid
    WITH nocounter, maxqual(mas,value(rowsleft))
   ;end select
