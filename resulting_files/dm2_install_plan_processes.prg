CREATE PROGRAM dm2_install_plan_processes
 ...
 ENDIF
 DECLARE dipp_logfile_prefix = vc WITH protect, constant("dm2_inst_plan_procs")
 DECLARE dipp_continue_loop = i2 WITH protect, noconstant(1)
 DECLARE dipp_option = vc WITH protect, noconstant(" ")
 DECLARE dipp_ret_msg = vc WITH protect, noconstant(" ")
 IF (validate(dipp_sched->readme_cnt,8)=8
  AND validate(dipp_sched->readme_cnt,9)=9
  AND validate(dipp_sched->schema_cnt,8)=8
  AND validate(dipp_sched->cnt,9)=9)
  FREE RECORD dipp_sched
  RECORD dipp_sched(
    1 readme_cnt = i2
    1 schema_cnt = i2
    1 readme_qual[*]
  ...
  SET dipp_sched->readme_cnt = 0
  SET dipp_sched->schema_cnt = 0
  SET dipp_sched->hour24_crossed_idx = 0
 ENDIF
 IF (validate(dipp_runners->cnt,8)=8
  AND validate(dipp_runners->cnt,9)=9)
  FREE RECORD dipp_runners
  RECORD dipp_runners(
    1 cnt = i2
    1 qual[*]
      2 process_id = vc
  ...
  SET dipp_runners->cnt = 0
  SET dipp_runners->file_name = " "
 ENDIF
 IF (validate(dipp_procs->cnt,8)=8
  AND validate(dipp_procs->cnt,9)=9)
  FREE RECORD dipp_procs
  RECORD dipp_procs(
    1 cnt = i2
    1 qual[*]
      2 plan_id = f8
  ...
  SET dipp_procs->cnt = 0
  SET dipp_procs->file_name = " "
 ENDIF
 DECLARE dipp_load_sched_to_screen(dpms_strt_line=i2,dpms_line_ctr=i2) = null
 DECLARE dipp_load_flex_schedule(null) = i2
 DECLARE dipp_load_runners_to_screen(dlrts_start_line=i2,dlrts_ret_msg=vc(ref)) = i2
 DECLARE dipp_load_procs_to_screen(dlpts_start_line=i2,dlpts_ret_msg=vc(ref)) = i2
 DECLARE dipp_write_runners_to_file(null) = i2
 SET width = 132
 IF (check_logfile(dipp_logfile_prefix,".log","dm2_install_plan_processes")=0)
  CALL dipr_disp_error_msg(null)
  GO TO exit_program
 ENDIF
 IF ((drr_flex_sched->pkg_number="DM2NOTSET"))
  ...
  ENDIF
 ENDIF
 WHILE (dipp_continue_loop=1)
   SET message = window
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL text(2,2,"INSTALL PLAN MENU [ACTIVE INSTALL PROCESSES]")
   CALL text(2,70,"DATE/TIME: ")
   CALL text(2,80,concat(build(month(curdate),"/",day(curdate),"/",year(curdate)),"  ",build(hour(
       curtime),":",minute(curtime))))
   IF (dipp_load_procs_to_screen(5,dipp_ret_msg)=0)
    GO TO exit_program
   ENDIF
   IF (dipp_load_runners_to_screen(10,dipp_ret_msg)=0)
    RETURN(0)
   ENDIF
   CALL text(23,2,concat(
     "(R)efresh Screen, (S)how Schedule, (V)iew Suspended Sessions, (B)ack :      ",
     "* Screen auto-refreshes every 2 minutes"))
   IF (dipp_ret_msg > " ")
    CALL text(22,4,dipp_ret_msg)
    SET dipp_ret_msg = " "
   ENDIF
   SET accept = time(120)
   CALL accept(23,72,"P;CU","R"
    WHERE curaccept IN ("R", "B", "r", "b", "V",
    "v", "s", "S"))
   SET accept = time(0)
   SET dipp_option = cnvtupper(curaccept)
   SET message = nowindow
   CASE (dipp_option)
    OF "B":
     SET dipp_continue_loop = 0
    OF "R":
     SET dipp_continue_loop = 1
    OF "V":
     IF (dm2_get_rdbms_version(null)=0)
      GO TO exit_program
     ENDIF
     IF ((dm2_rdbms_version->level1 >= 9))
      ...
      ENDIF
     ELSE
      CALL text(22,2,"Option available on Oracle Version 9 or greater")
      CALL pause(2)
     ENDIF
     SET dipp_continue_loop = 1
    OF "S":
     IF (dipp_load_flex_schedule(null)=0)
      RETURN(0)
     ENDIF
     SET message = window
     CALL clear(1,1)
     ...
     CALL text(5,2,"README (R) SCHEMA (S)")
     ...
     CALL text(8,1,"(R)")
     CALL text(12,1,"(S)")
     CALL dipp_load_sched_to_screen(8,1)
     CALL text(23,2,"Press <enter> to go back.")
     CALL accept(23,26,"p;cu"," "
      WHERE curaccept IN (" "))
     SET message = nowindow
   ENDCASE
 ENDWHILE
 GO TO exit_program
 SUBROUTINE dipp_load_flex_schedule(null)
   DECLARE dlfs_cnt = i2 WITH protect, noconstant(0)
   DECLARE dlfs_hold_time = vc WITH protect, noconstant("")
   IF (drr_get_flexible_schedule(null)=0)
    CALL dipr_disp_error_msg(null)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Loading schedule for display..."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dipp_sched->readme_cnt = 4
   SET stat = alterlist(dipp_sched->readme_qual,4)
   FOR (dlfs_cnt = 1 TO drr_flex_sched->readme_time_periods)
     SET dipp_sched->readme_qual[dlfs_cnt].time_period = dlfs_cnt
     SET dipp_sched->readme_qual[dlfs_cnt].start_time_hhmm = drr_flex_sched->readme_schedule[dlfs_cnt
     ].start_time
     SET dlfs_hold_time = cnvtupper(format(dipp_sched->readme_qual[dlfs_cnt].start_time_hhmm,"HH;;s")
      )
     SET dipp_sched->readme_qual[dlfs_cnt].start_time_hh = cnvtint(trim(replace(replace(
         dlfs_hold_time,"AM","",0),"PM","",0),3))
     IF ((dipp_sched->readme_qual[dlfs_cnt].start_time_hh=0))
      SET dipp_sched->readme_qual[dlfs_cnt].start_time_hh = 12
     ENDIF
     IF (findstring("AM",dlfs_hold_time) > 0)
      SET dipp_sched->readme_qual[dlfs_cnt].start_time_am_pm = "AM"
     ELSE
      SET dipp_sched->readme_qual[dlfs_cnt].start_time_am_pm = "PM"
     ENDIF
     SET dipp_sched->readme_qual[dlfs_cnt].end_time_hhmm = drr_flex_sched->readme_schedule[dlfs_cnt].
     end_time
     SET dlfs_hold_time = cnvtupper(format(dipp_sched->readme_qual[dlfs_cnt].end_time_hhmm,"HH;;s"))
     SET dipp_sched->readme_qual[dlfs_cnt].end_time_hh = cnvtint(trim(replace(replace(dlfs_hold_time,
         "AM","",0),"PM","",0),3))
     IF ((dipp_sched->readme_qual[dlfs_cnt].end_time_hh=0))
      SET dipp_sched->readme_qual[dlfs_cnt].end_time_hh = 12
     ENDIF
     IF (findstring("AM",dlfs_hold_time) > 0)
      SET dipp_sched->readme_qual[dlfs_cnt].end_time_am_pm = "AM"
     ELSE
      SET dipp_sched->readme_qual[dlfs_cnt].end_time_am_pm = "PM"
     ENDIF
     SET dipp_sched->readme_qual[dlfs_cnt].num_of_runners = drr_flex_sched->readme_schedule[dlfs_cnt]
     .num_of_runners
   ENDFOR
   IF ((drr_flex_sched->readme_time_periods < 4))
    SET dlfs_cnt = (drr_flex_sched->readme_time_periods+ 1)
    FOR (dlfs_cnt = dlfs_cnt TO dipp_sched->readme_cnt)
      SET dipp_sched->readme_qual[dlfs_cnt].time_period = dlfs_cnt
      SET dipp_sched->readme_qual[dlfs_cnt].start_time_hhmm = 0
      SET dipp_sched->readme_qual[dlfs_cnt].start_time_hh = 0
      SET dipp_sched->readme_qual[dlfs_cnt].start_time_am_pm = "  "
      SET dipp_sched->readme_qual[dlfs_cnt].end_time_hhmm = 0
      SET dipp_sched->readme_qual[dlfs_cnt].end_time_hh = 0
      SET dipp_sched->readme_qual[dlfs_cnt].end_time_am_pm = "  "
      SET dipp_sched->readme_qual[dlfs_cnt].num_of_runners = 0
    ENDFOR
   ENDIF
   SET dipp_sched->schema_cnt = 4
   SET stat = alterlist(dipp_sched->schema_qual,4)
   FOR (dlfs_cnt = 1 TO drr_flex_sched->schema_time_periods)
     SET dipp_sched->schema_qual[dlfs_cnt].time_period = dlfs_cnt
     SET dipp_sched->schema_qual[dlfs_cnt].start_time_hhmm = drr_flex_sched->schema_schedule[dlfs_cnt
     ].start_time
     SET dlfs_hold_time = cnvtupper(format(dipp_sched->schema_qual[dlfs_cnt].start_time_hhmm,"HH;;s")
      )
     SET dipp_sched->schema_qual[dlfs_cnt].start_time_hh = cnvtint(trim(replace(replace(
         dlfs_hold_time,"AM","",0),"PM","",0),3))
     IF ((dipp_sched->schema_qual[dlfs_cnt].start_time_hh=0))
      SET dipp_sched->schema_qual[dlfs_cnt].start_time_hh = 12
     ENDIF
     IF (findstring("AM",dlfs_hold_time) > 0)
      SET dipp_sched->schema_qual[dlfs_cnt].start_time_am_pm = "AM"
     ELSE
      SET dipp_sched->schema_qual[dlfs_cnt].start_time_am_pm = "PM"
     ENDIF
     SET dipp_sched->schema_qual[dlfs_cnt].end_time_hhmm = drr_flex_sched->schema_schedule[dlfs_cnt].
     end_time
     SET dlfs_hold_time = cnvtupper(format(dipp_sched->schema_qual[dlfs_cnt].end_time_hhmm,"HH;;s"))
     SET dipp_sched->schema_qual[dlfs_cnt].end_time_hh = cnvtint(trim(replace(replace(dlfs_hold_time,
         "AM","",0),"PM","",0),3))
     IF ((dipp_sched->schema_qual[dlfs_cnt].end_time_hh=0))
      SET dipp_sched->schema_qual[dlfs_cnt].end_time_hh = 12
     ENDIF
     IF (findstring("AM",dlfs_hold_time) > 0)
      SET dipp_sched->schema_qual[dlfs_cnt].end_time_am_pm = "AM"
     ELSE
      SET dipp_sched->schema_qual[dlfs_cnt].end_time_am_pm = "PM"
     ENDIF
     SET dipp_sched->schema_qual[dlfs_cnt].num_of_runners = drr_flex_sched->schema_schedule[dlfs_cnt]
     .num_of_runners
   ENDFOR
   IF ((drr_flex_sched->schema_time_periods < 4))
    SET dlfs_cnt = (drr_flex_sched->schema_time_periods+ 1)
    FOR (dlfs_cnt = dlfs_cnt TO dipp_sched->schema_cnt)
      SET dipp_sched->schema_qual[dlfs_cnt].time_period = dlfs_cnt
      SET dipp_sched->schema_qual[dlfs_cnt].start_time_hhmm = 0
      SET dipp_sched->schema_qual[dlfs_cnt].start_time_hh = 0
      SET dipp_sched->schema_qual[dlfs_cnt].start_time_am_pm = "  "
      SET dipp_sched->schema_qual[dlfs_cnt].end_time_hhmm = 0
      SET dipp_sched->schema_qual[dlfs_cnt].end_time_hh = 0
      SET dipp_sched->schema_qual[dlfs_cnt].end_time_am_pm = "  "
      SET dipp_sched->schema_qual[dlfs_cnt].num_of_runners = 0
    ENDFOR
   ENDIF
   IF ((dm_err->debug_flag > 622))
    SET message = nowindow
    CALL echorecord(dipp_sched)
    SET message = window
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dipp_load_sched_to_screen(dpms_strt_line,dpms_line_ctr)
   DECLARE dlsts_cnt = i4 WITH protect, noconstant(0)
   DECLARE dlsts_hold_time = vc WITH protect, noconstant(" ")
   DECLARE dlsts_row = i2
   DECLARE dlsts_col = i2
   SET dlsts_row = dpms_strt_line
   SET dlsts_col = 7
   FOR (dlsts_cnt = 1 TO dipp_sched->readme_cnt)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dlsts_cnt))
     SET dlsts_col = (dlsts_col+ 11)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dipp_sched->readme_qual[dlsts_cnt].start_time_hh))
     SET dlsts_col = (dlsts_col+ 3)
     CALL text(dlsts_row,dlsts_col,dipp_sched->readme_qual[dlsts_cnt].start_time_am_pm)
     SET dlsts_col = (dlsts_col+ 10)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dipp_sched->readme_qual[dlsts_cnt].end_time_hh))
     SET dlsts_col = (dlsts_col+ 3)
     CALL text(dlsts_row,dlsts_col,dipp_sched->readme_qual[dlsts_cnt].end_time_am_pm)
     SET dlsts_col = (dlsts_col+ 10)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dipp_sched->readme_qual[dlsts_cnt].num_of_runners))
     SET dlsts_row = (dlsts_row+ dpms_line_ctr)
     SET dlsts_col = 7
   ENDFOR
   FOR (dlsts_cnt = 1 TO dipp_sched->schema_cnt)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dlsts_cnt))
     SET dlsts_col = (dlsts_col+ 11)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dipp_sched->schema_qual[dlsts_cnt].start_time_hh))
     SET dlsts_col = (dlsts_col+ 3)
     CALL text(dlsts_row,dlsts_col,dipp_sched->schema_qual[dlsts_cnt].start_time_am_pm)
     SET dlsts_col = (dlsts_col+ 10)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dipp_sched->schema_qual[dlsts_cnt].end_time_hh))
     SET dlsts_col = (dlsts_col+ 3)
     CALL text(dlsts_row,dlsts_col,dipp_sched->schema_qual[dlsts_cnt].end_time_am_pm)
     SET dlsts_col = (dlsts_col+ 10)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dipp_sched->schema_qual[dlsts_cnt].num_of_runners))
     SET dlsts_row = (dlsts_row+ dpms_line_ctr)
     SET dlsts_col = 7
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dipp_load_runners_to_screen(dlrts_start_line,dlrts_ret_msg)
   ...
   ENDIF
   SET dipp_runners->cnt = 0
   SET stat = alterlist(dipp_runners->qual,0)
   SELECT INTO "nl:"
    FROM dm_info di,
     gv$session s,
     gv$instance i,
    ...
    PLAN (di
     ...
     JOIN (s
     WHERE cnvtstring(s.audsid)=trim(di.info_name,3))
     JOIN (i
    ...
    DETAIL
     IF (((p.qcsid=0) OR (p.sid=p.qcsid)) )
      dlrts_idx = 0
      IF ((dipp_runners->cnt > 0))
       dlrts_idx = locateval(dlrts_idx,1,dipp_runners->cnt,trim(di.info_name,3),dipp_runners->qual[
        dlrts_idx].appl_id)
      ENDIF
      IF (dlrts_idx=0)
       dipp_runners->cnt = (dipp_runners->cnt+ 1)
       IF (mod(dipp_runners->cnt,10)=1)
        stat = alterlist(dipp_runners->qual,(dipp_runners->cnt+ 9))
       ENDIF
       dipp_runners->qual[dipp_runners->cnt].appl_id = trim(di.info_name,3), dipp_runners->qual[
       dipp_runners->cnt].bkgrnd_runner_ind = 0, dipp_runners->qual[dipp_runners->cnt].working_on =
       " ",
       dipp_runners->qual[dipp_runners->cnt].active_ind = di.info_number, dipp_runners->qual[
       dipp_runners->cnt].num_w_applid = 1, dipp_runners->qual[dipp_runners->cnt].process_id = s
       .process,
       dipp_runners->qual[dipp_runners->cnt].host_name = i.host_name, dipp_runners->qual[dipp_runners
       ->cnt].instance_name = i.instance_name, dlrts_idx = dipp_runners->cnt
      ELSE
       dipp_runners->qual[dlrts_idx].num_w_applid = (dipp_runners->qual[dlrts_idx].num_w_applid+ 1)
      ENDIF
      IF (di.info_domain="DM2_BACKGROUND_RUNNER")
       dipp_runners->qual[dlrts_idx].bkgrnd_runner_ind = 1
      ENDIF
      IF (di.info_domain="DM2_SCHEMA_RUNNER")
       dipp_runners->qual[dlrts_idx].working_on = "SCHEMA"
      ELSEIF (di.info_domain="DM2_README_RUNNER")
       dipp_runners->qual[dlrts_idx].working_on = "READMES"
      ENDIF
     ENDIF
    FOOT REPORT
     stat = alterlist(dipp_runners->qual,dipp_runners->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL dipr_disp_error_msg(null)
    RETURN(0)
   ENDIF
   FOR (dlrts_cnt = 1 TO dipp_runners->cnt)
     IF ((dipp_runners->qual[dlrts_cnt].active_ind=0))
      SET dipp_runners->qual[dlrts_cnt].status = "Stopping"
     ELSE
      IF ((dipp_runners->qual[dlrts_cnt].working_on=" ")
       AND (dipp_runners->qual[dlrts_cnt].bkgrnd_runner_ind=1))
       SET dipp_runners->qual[dlrts_cnt].status = "Idle"
      ELSE
       IF ((dipp_runners->qual[dlrts_cnt].working_on="SCHEMA"))
        SELECT INTO "nl:"
         FROM dm2_ddl_ops_log ddol
         WHERE ddol.status IN ("RUNNING", "COMPLETE")
          AND (ddol.appl_id=dipp_runners->qual[dlrts_cnt].appl_id)
         ORDER BY ddol.begin_dt_tm
         HEAD REPORT
          dipp_runners->qual[dlrts_cnt].num_completed = 0
         DETAIL
          dipp_runners->qual[dlrts_cnt].num_completed = (dipp_runners->qual[dlrts_cnt].num_completed
          + 1), dipp_runners->qual[dlrts_cnt].status = evaluate(ddol.status,"RUNNING","Running",
           "Idle"), dipp_runners->qual[dlrts_cnt].current_last_op = ddol.op_type
         WITH nocounter
        ;end select
        ...
        IF (curqual=0)
         SET dipp_runners->qual[dlrts_cnt].status = "Idle"
         SET dipp_runners->qual[dlrts_cnt].current_last_op = " "
         SET dipp_runners->qual[dlrts_cnt].num_completed = 0
        ELSE
         IF ((dipp_runners->qual[dlrts_cnt].status="Running"))
          SET dipp_runners->qual[dlrts_cnt].num_completed = (dipp_runners->qual[dlrts_cnt].
          num_completed - 1)
         ENDIF
         SET dlrts_len = findstring("-",dipp_runners->qual[dlrts_cnt].current_last_op)
         IF (dlrts_len > 0)
          SET dipp_runners->qual[dlrts_cnt].current_last_op = trim(substring(1,(dlrts_len - 1),
            dipp_runners->qual[dlrts_cnt].current_last_op))
         ENDIF
         SET dlrts_len = findstring("(",dipp_runners->qual[dlrts_cnt].current_last_op)
         IF (dlrts_len > 0)
          SET dipp_runners->qual[dlrts_cnt].current_last_op = trim(substring(1,(dlrts_len - 1),
       ...
       ELSE
        ...
        SELECT INTO "nl:"
         ...
         WHERE (d.environment_id=dm2_rr_misc->env_id)
          ...
          AND (d.appl_ident=dipp_runners->qual[dlrts_cnt].appl_id)
         ORDER BY d.start_dt_tm
         HEAD REPORT
          dipp_runners->qual[dlrts_cnt].num_completed = 0
         DETAIL
          dipp_runners->qual[dlrts_cnt].num_completed = (dipp_runners->qual[dlrts_cnt].num_completed
          + 1), dipp_runners->qual[dlrts_cnt].status = evaluate(d.status,dm2_rr_running,"Running",
           "Idle"), dipp_runners->qual[dlrts_cnt].current_last_op = concat("Readme ",trim(d
            .project_name,3))
         WITH nocounter
        ;end select
   ...
   IF ((dm2_rdbms_version->level1 >= 9))
    SELECT INTO "nl:"
     ...
     DETAIL
      dlrts_idx = locateval(dlrts_idx,1,size(dipp_runners->qual,5),trim(cnvtstring(v.audsid,0)),
       dipp_runners->qual[dlrts_idx].appl_id)
      IF (dlrts_idx > 0)
       dipp_runners->qual[dlrts_idx].status = u.status
      ENDIF
     WITH nocounter
    ;end select
   ...
   IF ((dm_err->debug_flag > 622))
    SET message = nowindow
    CALL echorecord(dipp_runners)
    SET message = window
   ENDIF
   SET dlrts_row = (dlrts_row+ 1)
   IF ((dipp_runners->cnt > 10))
    IF (dipp_write_runners_to_file(null)=0)
     RETURN(0)
    ENDIF
    SET dlrts_max = 10
    SET dlrts_ret_msg = concat("Total runners currently processing exceeded display limit.  See ",
     trim(dipp_runners->file_name,3)," for complete list.")
   ELSE
    IF ((dipp_runners->file_name > " "))
     IF (remove(dipp_runners->file_name)=0)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = concat("Could not remove ",trim(dipp_runners->file_name,3),
       " from ccluserdir.")
     ...
     SET dipp_runners->file_name = " "
    ENDIF
    SET dlrts_max = dipp_runners->cnt
    SET dlrts_ret_msg = " "
   ENDIF
   FOR (dlrts_cnt = 1 TO dlrts_max)
     CALL text(dlrts_row,2,cnvtstring(dlrts_cnt))
     CALL text(dlrts_row,9,substring(1,13,dipp_runners->qual[dlrts_cnt].process_id))
     CALL text(dlrts_row,23,substring(1,19,dipp_runners->qual[dlrts_cnt].host_name))
     CALL text(dlrts_row,43,substring(1,12,dipp_runners->qual[dlrts_cnt].instance_name))
     CALL text(dlrts_row,56,substring(1,26,dipp_runners->qual[dlrts_cnt].appl_id))
     CALL text(dlrts_row,83,substring(1,12,dipp_runners->qual[dlrts_cnt].status))
     CALL text(dlrts_row,96,substring(1,34,dipp_runners->qual[dlrts_cnt].current_last_op))
     CALL text(dlrts_row,121,substring(1,10,cnvtstring(dipp_runners->qual[dlrts_cnt].num_completed)))
     SET dlrts_row = (dlrts_row+ 1)
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dipp_load_procs_to_screen(dlpts_start_line,dlpts_ret_msg)
   DECLARE dlpts_idx = i2 WITH protect, noconstant(0)
   DECLARE dlpts_cnt = i4 WITH protect, noconstant(0)
   DECLARE dlpts_row = i2
   DECLARE dlpts_col = i2
   DECLARE dlpts_env_hold = f8 WITH protect, noconstant(0.0)
   DECLARE dlpts_max = i2 WITH protect, noconstant(0)
   DECLARE dlpts_file_name = vc WITH protect, noconstant(" ")
   DECLARE dlpts_len = i4 WITH protect, noconstant(0)
   DECLARE dlpts_dpe_id = f8 WITH protect, noconstant(0.0)
   DECLARE dlpts_optimizer_hint = vc WITH protect, noconstant("")
   SET dlpts_optimizer_hint = concat(" LEADING(DP DPE DPED)","INDEX(DP XAK1DM_PROCESS)",
    "INDEX(DPE XIE1DM_PROCESS_EVENT)","INDEX(DPED XIE1DM_PROCESS_EVENT_DTL) ")
   SET dlpts_row = dlpts_start_line
   CALL clear(dlpts_row,1)
   CALL text(dlpts_row,2,"Plan Id")
   CALL text(dlpts_row,12,"Process")
   CALL text(dlpts_row,32,"Process Id")
   CALL text(dlpts_row,44,"AudSid")
   CALL text(dlpts_row,54,"Host Name")
   CALL text(dlpts_row,74,"Instance")
   CALL text(dlpts_row,84,"Status")
   SET dm_err->eproc = concat("Getting install process statuses...")
   IF ((dm_err->debug_flag > 622))
    SET message = nowindow
    CALL disp_msg(" ",dm_err->logfile,0)
    SET message = window
   ENDIF
   SET dipp_procs->cnt = 0
   SET stat = alterlist(dipp_procs->qual,0)
   SELECT INTO "nl:"
    FROM dm_process dp,
     dm_process_event dpe,
     dm_process_event_dtl dped,
     ...
     gv$px_session p
    PLAN (dp
     WHERE dp.process_name IN (value(dpl_package_install), value(dpl_install_monitor))
      AND dp.action_type=value(dpl_execution))
     JOIN (dpe
     WHERE dp.dm_process_id=dpe.dm_process_id
      AND dpe.install_plan_id=cnvtreal(drr_flex_sched->pkg_number)
      AND  NOT (dpe.event_status IN (value(dpl_failed), value(dpl_complete), value(dpl_success),
     value(dpl_failure))))
     JOIN (dped
     WHERE dped.dm_process_event_id=dpe.dm_process_event_id
      AND dped.detail_type="AUDSID")
     JOIN (s
     WHERE cnvtstring(s.audsid)=trim(dped.detail_text,3))
     JOIN (i
     WHERE s.inst_id=i.inst_id)
     JOIN (p
     WHERE outerjoin(s.sid)=p.sid)
    ORDER BY dp.process_name
    HEAD REPORT
     dipp_procs->cnt = 0
    DETAIL
     IF (((p.qcsid=0) OR (p.sid=p.qcsid)) )
      dlpts_idx = 0
      IF ((dipp_procs->cnt > 0))
       dlpts_idx = locateval(dlpts_idx,1,dipp_procs->cnt,trim(dped.detail_text,3),dipp_procs->qual[
        dlpts_idx].audsid)
      ENDIF
      IF (dlpts_idx=0)
       dipp_procs->cnt = (dipp_procs->cnt+ 1)
       IF (mod(dipp_procs->cnt,10)=1)
        stat = alterlist(dipp_procs->qual,(dipp_procs->cnt+ 9))
       ENDIF
       dipp_procs->qual[dipp_procs->cnt].plan_id = dpe.install_plan_id, dipp_procs->qual[dipp_procs->
       cnt].dpe_id = dpe.dm_process_event_id, dipp_procs->qual[dipp_procs->cnt].process = evaluate(dp
        .process_name,value(dpl_install_monitor),"MONITOR","INSTALL"),
       dipp_procs->qual[dipp_procs->cnt].process_id = s.process, dipp_procs->qual[dipp_procs->cnt].
       audsid = trim(dped.detail_text,3), dipp_procs->qual[dipp_procs->cnt].host_name = i.host_name,
       dipp_procs->qual[dipp_procs->cnt].instance_name = i.instance_name, dipp_procs->qual[dipp_procs
       ->cnt].status = dpe.event_status, dlpts_idx = dipp_procs->cnt
      ENDIF
     ENDIF
    FOOT REPORT
     stat = alterlist(dipp_procs->qual,dipp_procs->cnt)
    WITH nocounter, orahintcbo(value(dlpts_optimizer_hint))
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL dipr_disp_error_msg(null)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Retrieving process mode for current package install execution"
   IF ((dm_err->debug_flag > 0))
    CALL dipr_disp_error_msg(null)
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = size(dipp_procs->qual,5)),
     dm_process_event_dtl dped
    PLAN (d
     WHERE d.seq > 0)
     JOIN (dped
     WHERE (dped.dm_process_event_id=dipp_procs->qual[d.seq].dpe_id)
      AND dped.detail_type=value(dpl_install_mode))
    DETAIL
     dipp_procs->qual[d.seq].process = concat(dipp_procs->qual[d.seq].process,":",dped.detail_text)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL dipr_disp_error_msg(null)
   ...
   IF ((dm_err->debug_flag > 511))
    SET message = nowindow
    CALL echorecord(dipp_procs)
    SET message = window
   ENDIF
   SET dlpts_row = (dlpts_row+ 1)
   FOR (dlpts_cnt = 1 TO dipp_procs->cnt)
     CALL text(dlpts_row,2,substring(1,9,cnvtstring(dipp_procs->qual[dlpts_cnt].plan_id)))
     CALL text(dlpts_row,12,substring(1,19,dipp_procs->qual[dlpts_cnt].process))
     CALL text(dlpts_row,32,substring(1,12,dipp_procs->qual[dlpts_cnt].process_id))
     CALL text(dlpts_row,44,substring(1,9,dipp_procs->qual[dlpts_cnt].audsid))
     CALL text(dlpts_row,54,substring(1,19,dipp_procs->qual[dlpts_cnt].host_name))
     CALL text(dlpts_row,74,substring(1,9,dipp_procs->qual[dlpts_cnt].instance_name))
     CALL text(dlpts_row,84,substring(1,10,dipp_procs->qual[dlpts_cnt].status))
     SET dlpts_row = (dlpts_row+ 1)
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dipp_write_runners_to_file(null)
   SET dwrtf_timestamp = format(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),";;q")
   IF ((dipp_runners->file_name=" "))
    SET dipp_runners->file_name = build("drr_flex_sched_runners_",trim(currdbhandle,3),".rpt")
   ENDIF
   SET dm_err->eproc = "Creating Installation Schedule Runners Report"
   SELECT INTO value(dipp_runners->file_name)
    FROM dual
    HEAD REPORT
    ...
    DETAIL
     ...
     "Status", row + 2
     FOR (dlrts_cnt = 1 TO dipp_runners->cnt)
       col 2, dlrts_cnt"##", proc_id = substring(1,13,dipp_runners->qual[dlrts_cnt].process_id),
       col 9, proc_id, host_name = substring(1,19,dipp_runners->qual[dlrts_cnt].host_name),
       col 23, host_name, instance_name = substring(1,12,dipp_runners->qual[dlrts_cnt].instance_name),
       col 43, instance_name, appl_id = substring(1,26,dipp_runners->qual[dlrts_cnt].appl_id),
       col 56, appl_id, col 83,
       dipp_runners->qual[dlrts_cnt].status, row + 1
     ENDFOR
    FOOT REPORT
     row + 0
    WITH nocounter, format = variable, formfeed = none,
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "LEAVING dm2_install_plan_processes"
 CALL final_disp_msg(dipp_logfile_prefix)
END GO
