CREATE PROGRAM cp_add_concept:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CP_ADD_CONCEPT"
 ...
 ENDIF
 SET conceptreply->status_data.status = "F"
 SET request->cd_value_list[1].action_type_flag = 1
 SET request->cd_value_list[1].code_set = concept_cs
 SET request->cd_value_list[1].definition =  $CDISP
 SET request->cd_value_list[1].description =  $CDISP
 SET request->cd_value_list[1].display =  $CDISP
 SET request->cd_value_list[1].active_ind = 1
 SET trace = recpersist
 EXECUTE core_ens_cd_value
 SET stat = moverec(reply,conceptreply)
#set_return
 IF (validate(debug_ind,0))
  CALL echorecord(conceptreply)
 ENDIF
 IF ((conceptreply->status_data.status="S"))
  COMMIT
 ELSE
  ROLLBACK
 ENDIF
#exit_script
 IF (( $OUTDEV != "NOFORMS"))
  CALL putjsonrecordtofile(conceptreply)
 ENDIF
