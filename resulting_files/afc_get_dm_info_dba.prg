CREATE PROGRAM afc_get_dm_info:dba
 DECLARE versionnbr = vc
 SET versionnbr = "002"
 CALL echo(build("AFC_GET_DM_INFO Version: ",versionnbr))
 RECORD reply(
   1 dm_info_qual = i2
   1 dm_info[*]
     2 info_name = vc
 ...
 DECLARE dminfoindicator = i2 WITH protect, noconstant(false)
 DECLARE pftconfigindicator = i2 WITH protect, noconstant(false)
 ...
 IF (trim(request->info_name) != "")
  SELECT INTO "nl:"
   ...
   DETAIL
    count1 += 1, stat = alterlist(reply->dm_info,count1), reply->dm_info[count1].info_name = di
    .info_name,
    reply->dm_info[count1].info_date = di.info_date, reply->dm_info[count1].info_char = di.info_char,
    reply->dm_info[count1].info_number = di.info_number,
    reply->dm_info[count1].info_long_id = di.info_long_id
   WITH nocounter
  ;end select
 ELSE
  IF (size(request->info,5) > 0)
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(size(request->info,5))),
     dm_info di
    PLAN (d)
     JOIN (di
     WHERE di.info_domain="CHARGE SERVICES"
      AND (di.info_name=request->info[d.seq].info_name)
      AND di.info_domain_id=curlogicaldomainid)
    DETAIL
     count1 += 1, stat = alterlist(reply->dm_info,count1), reply->dm_info[count1].info_name = di
     .info_name,
 ...
 ELSE
  SET dminfoindicator = true
 ENDIF
 IF (validate(request->pft_config_qual)
  AND validate(reply->pft_config_qual))
  IF ((request->pft_config_qual > 0))
   IF (size(request->pft_config_info,5) > 0)
    SELECT INTO "nl:"
     FROM (dummyt d  WITH seq = value(size(request->pft_config_info,5))),
      pft_config pc
     PLAN (d)
      JOIN (pc
      WHERE pc.info_domain_txt="CHARGE SERVICES"
       AND (pc.info_name=request->pft_config_info[d.seq].info_name)
       AND pc.info_domain_id=curlogicaldomainid)
     DETAIL
      count2 += 1, stat = alterlist(reply->pft_config,count2), reply->pft_config[count2].info_name =
      pc.info_name,
      reply->pft_config[count2].info_dt_tm = pc.info_dt_tm, reply->pft_config[count2].info_nbr = pc
      .info_nbr
     WITH nocounter
    ;end select
    IF (curqual=0)
     ...
     SET reply->status_data.subeventstatus[2].targetobjectname = "TABLE"
     SET reply->status_data.subeventstatus[2].targetobjectvalue = "PFT_CONFIG"
     SET reply->status_data.status = "Z"
    ELSE
     SET pftconfigindicator = true
    ENDIF
    SET reply->pft_config_qual = count2
   ENDIF
  ENDIF
 ENDIF
 IF (((pftconfigindicator) OR (dminfoindicator)) )
