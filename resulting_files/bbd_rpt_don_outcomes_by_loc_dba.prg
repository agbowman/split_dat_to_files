CREATE PROGRAM bbd_rpt_don_outcomes_by_loc:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 DECLARE line15 = c15 WITH protect, constant(fillstring(15,"-"))
 DECLARE line18 = c18 WITH protect, constant(fillstring(18,"-"))
 DECLARE line131 = c131 WITH protect, constant(fillstring(131,"-"))
 DECLARE script_name = c27 WITH protect, constant("bbd_rpt_don_outcomes_by_loc")
 ...
 IF (size(trim(request->batch_selection)) > 0)
  ...
  CALL check_outcome_cd(script_name)
 ENDIF
 ...
 SET captions->rpt_title = uar_i18ngetmessage(i18nhandle,"rpt_title",
  "SUMMARY OF DONATION OUTCOMES BY DONATION LOCATION")
 ...
 SET captions->rpt_don_outcome = uar_i18ngetmessage(i18nhandle,"rpt_don_outcome","Donation Outcome:")
 ...
 SET captions->rpt_don_proc = uar_i18ngetmessage(i18nhandle,"rpt_don_proc","Donation Procedure")
 ...
 IF (id_cd=0.0)
  SET reply->status_data.status = "F"
  CALL subevent_add("bbd_rpt_don_outcomes_by_loc.prg","F","uar_get_code_by",
   "Unable to retrieve the code_value for the cdf_meaning in code_set 4.")
  SET reply->status = "F"
  GO TO exit_script
 ENDIF
 EXECUTE cpm_create_file_name_logical "bbd_don_outcomes", "txt", "x"
 SELECT
  ...
  drawn_date = cnvtdate(bdr.drawn_dt_tm), name = substring(1,20,per.name_full_formatted), donor_id =
  substring(1,15,cnvtalias(pa.alias,pa.alias_pool_cd)),
  bdr_procedure_disp = substring(1,15,uar_get_code_display(bdr.procedure_cd)), bdr_outcome_disp =
  substring(1,15,uar_get_code_display(bdr.outcome_cd)), product_number = substring(1,20,concat(trim(p
     .product_nbr)," ",trim(p.product_sub_nbr))),
  p_product_disp = substring(1,20,uar_get_code_display(p.product_cd)), aborh = substring(1,10,concat(
    trim(uar_get_code_display(d.abo_cd))," ",trim(uar_get_code_display(d.rh_cd)))),
  p_cur_owner_area_disp = uar_get_code_display(p.cur_owner_area_cd),
  p_cur_inv_area_disp = uar_get_code_display(p.cur_inv_area_cd), e_location_disp =
  ...
  PLAN (bdr
   WHERE bdr.drawn_dt_tm BETWEEN cnvtdatetime(request->beg_dt_tm) AND cnvtdatetime(request->end_dt_tm
    ...
    AND (((request->outcome_cd > 0.0)
    AND (bdr.outcome_cd=request->outcome_cd)) OR ((request->outcome_cd=0.0))) )
   JOIN (bdp
   WHERE bdp.donation_results_id=outerjoin(bdr.donation_result_id))
   JOIN (p
   WHERE p.product_id=outerjoin(bdp.product_id)
   ...
   JOIN (per
   WHERE per.person_id=bdr.person_id)
   JOIN (e
   WHERE e.encntr_id=outerjoin(bdr.encntr_id)
    AND (((request->donation_location_cd=e.loc_facility_cd)) OR ((request->donation_location_cd=0.0)
   )) )
   JOIN (pa
   WHERE pa.person_id=per.person_id
    AND pa.person_alias_type_cd=outerjoin(id_cd)
    AND pa.active_ind=outerjoin(1))
   JOIN (d
   WHERE d.person_id=outerjoin(per.person_id)
    AND d.active_ind=outerjoin(1))
  ORDER BY p.cur_owner_area_cd, p.cur_inv_area_cd, e.loc_facility_cd,
   drawn_date, bdr_procedure_disp, per.name_last,
   per.name_first, product_number
  HEAD REPORT
   first_page = 1
  HEAD PAGE
   row 0,
   ...
   col 18, p_cur_owner_area_disp, row + 1,
   col 0, captions->rpt_inv_area, col 18,
   p_cur_inv_area_disp, row + 1, col 0,
   captions->rpt_don_loc, col 20, e_location_disp,
   row + 1, col 0, captions->rpt_don_outcome
   IF ((request->outcome_cd=0.0))
    col 19, captions->rpt_all
   ELSE
    col 19, bdr_outcome_disp
   ENDIF
   row + 2, col 0, captions->rpt_don_date,
   row + 1, col 0, captions->rpt_donor,
   col 22, captions->rpt_donor_id, col 39,
   captions->rpt_don_proc, col 59, captions->rpt_outcome,
   col 76, captions->rpt_prod_nbr, col 98,
   captions->head_products, col 120, captions->rpt_aborh,
   row + 1, col 0, line20,
   col 22, line15, col 39,
   line18, col 59, line15,
   col 76, line20, col 98,
   line20, col 120, line8,
   row + 1
  ...
  HEAD p.cur_inv_area_cd
   ...
   ELSE
    BREAK
   ENDIF
   cur_dloc = e.location_cd
  HEAD e.loc_facility_cd
   IF (e.location_cd != cur_dloc)
    cur_dloc = e.loc_facility_cd, BREAK
   ENDIF
  HEAD drawn_date
   IF (row > 55)
    BREAK
   ENDIF
   col 0, drawn_date"@SHORTDATE;;D", row + 1
  DETAIL
   IF (row > 56)
    BREAK
   ENDIF
   col 0, name";r", col 22,
   donor_id, col 39, bdr_procedure_disp,
   col 59, bdr_outcome_disp, col 76,
   product_number, col 98, p_product_disp,
   col 120, aborh, row + 1
  FOOT  drawn_date
   row + 1
 ...
 IF (error_check != 0)
  SET reply->status_data.status = "F"
  CALL subevent_add("Select Donation Outcomes.","F","bbd_rpt_don_outcomes_by_loc.prg",errmsg)
  GO TO exit_script
 ENDIF
#set_status
 IF (curqual=0)
