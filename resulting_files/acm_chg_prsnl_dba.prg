CREATE PROGRAM acm_chg_prsnl:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->prsnl_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 ELSE
  ...
  WHILE (chg_cnt > 0)
    CALL getexistingrows(max_val)
    SET chg_cnt = (chg_cnt - max_val)
    SET f_val = (t_val+ 1)
    IF (chg_cnt > max_val)
     SET t_val = (t_val+ max_val)
    ELSE
     SET t_val = (t_val+ chg_cnt)
    ENDIF
  ENDWHILE
 ENDIF
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   prsnl p
  SET p.person_id = acm_request->prsnl_qual[xref->chg[d.seq].idx].person_id, p.beg_effective_dt_tm =
   cnvtdatetime(acm_request->prsnl_qual[xref->chg[d.seq].idx].beg_effective_dt_tm), p
   .contributor_system_cd =
   IF ((acm_request->prsnl_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0)) acm_request->
    prsnl_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_qual[xref->chg[d.seq].idx].
    end_effective_dt_tm), p.name_first = acm_request->prsnl_qual[xref->chg[d.seq].idx].name_first, p
   .name_first_key = trim(cnvtupper(cnvtalphanum(acm_request->prsnl_qual[xref->chg[d.seq].idx].
      name_first)),3),
   p.name_first_key_nls = acm_request->prsnl_qual[xref->chg[d.seq].idx].name_first_key_nls, p
   .name_full_formatted = acm_request->prsnl_qual[xref->chg[d.seq].idx].name_full_formatted, p
   .name_last = acm_request->prsnl_qual[xref->chg[d.seq].idx].name_last,
   p.name_last_key = trim(cnvtupper(cnvtalphanum(acm_request->prsnl_qual[xref->chg[d.seq].idx].
      name_last)),3), p.name_last_key_nls = acm_request->prsnl_qual[xref->chg[d.seq].idx].
   name_last_key_nls, p.position_cd = acm_request->prsnl_qual[xref->chg[d.seq].idx].position_cd,
   p.prsnl_type_cd = acm_request->prsnl_qual[xref->chg[d.seq].idx].prsnl_type_cd, p.create_dt_tm =
   IF ((acm_request->prsnl_qual[xref->chg[d.seq].idx].create_dt_tm > 0)) cnvtdatetime(acm_request->
     prsnl_qual[xref->chg[d.seq].idx].create_dt_tm)
   ELSE null
   ENDIF
   , p.create_prsnl_id = acm_request->prsnl_qual[xref->chg[d.seq].idx].create_prsnl_id,
   p.department_cd = acm_request->prsnl_qual[xref->chg[d.seq].idx].department_cd, p.email =
   acm_request->prsnl_qual[xref->chg[d.seq].idx].email, p.free_text_ind = acm_request->prsnl_qual[
   xref->chg[d.seq].idx].free_text_ind,
   p.ft_entity_id = acm_request->prsnl_qual[xref->chg[d.seq].idx].ft_entity_id, p.ft_entity_name =
   acm_request->prsnl_qual[xref->chg[d.seq].idx].ft_entity_name, p.log_access_ind = acm_request->
   prsnl_qual[xref->chg[d.seq].idx].log_access_ind,
   p.log_level = acm_request->prsnl_qual[xref->chg[d.seq].idx].log_level, p.password = acm_request->
   prsnl_qual[xref->chg[d.seq].idx].password, p.physician_ind = acm_request->prsnl_qual[xref->chg[d
   .seq].idx].physician_ind,
   p.physician_status_cd = acm_request->prsnl_qual[xref->chg[d.seq].idx].physician_status_cd, p
   .prim_assign_loc_cd = acm_request->prsnl_qual[xref->chg[d.seq].idx].prim_assign_loc_cd, p
   .section_cd = acm_request->prsnl_qual[xref->chg[d.seq].idx].section_cd,
   p.username = acm_request->prsnl_qual[xref->chg[d.seq].idx].username, p.active_ind = acm_request->
   prsnl_qual[xref->chg[d.seq].idx].active_ind, p.active_status_cd = acm_request->prsnl_qual[xref->
   chg[d.seq].idx].active_status_cd,
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.person_id=acm_request->prsnl_qual[xref->chg[d.seq].idx].person_id))
  WITH nocounter, status(reply->prsnl_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->prsnl_qual[xref->chg[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM prsnl p
    WHERE expand(t1,f_val,t_val,p.person_id,acm_request->prsnl_qual[xref->chg[t1].idx].person_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.person_id,acm_request->prsnl_qual[xref->chg[t1].idx].person_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->prsnl_qual[idx].updt_cnt)) OR ((acm_request->force_updt_ind=1)))
     )
      reply->prsnl_qual[idx].status = - (1), reply->prsnl_qual[idx].person_id = acm_request->
      prsnl_qual[idx].person_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->prsnl_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->prsnl_qual[idx].contributor_system_cd = p.contributor_system_cd
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("NAME_FIRST,",chg_str)=0)
      acm_request->prsnl_qual[idx].name_first = p.name_first
     ENDIF
     IF (findstring("NAME_FIRST_KEY_NLS,",chg_str)=0)
      acm_request->prsnl_qual[idx].name_first_key_nls = p.name_first_key_nls
     ENDIF
     IF (findstring("NAME_FULL_FORMATTED,",chg_str)=0)
      acm_request->prsnl_qual[idx].name_full_formatted = p.name_full_formatted
     ENDIF
     IF (findstring("NAME_LAST,",chg_str)=0)
      acm_request->prsnl_qual[idx].name_last = p.name_last
     ENDIF
     IF (findstring("NAME_LAST_KEY_NLS,",chg_str)=0)
      acm_request->prsnl_qual[idx].name_last_key_nls = p.name_last_key_nls
     ENDIF
     IF (findstring("POSITION_CD,",chg_str)=0)
      acm_request->prsnl_qual[idx].position_cd = p.position_cd
     ENDIF
     IF (findstring("PRSNL_TYPE_CD,",chg_str)=0)
      acm_request->prsnl_qual[idx].prsnl_type_cd = p.prsnl_type_cd
     ENDIF
     IF (findstring("CREATE_DT_TM,",chg_str)=0)
      acm_request->prsnl_qual[idx].create_dt_tm = p.create_dt_tm
     ENDIF
     IF (findstring("CREATE_PRSNL_ID,",chg_str)=0)
      acm_request->prsnl_qual[idx].create_prsnl_id = p.create_prsnl_id
     ENDIF
     IF (findstring("DEPARTMENT_CD,",chg_str)=0)
      acm_request->prsnl_qual[idx].department_cd = p.department_cd
     ENDIF
     IF (findstring("EMAIL,",chg_str)=0)
      acm_request->prsnl_qual[idx].email = p.email
     ENDIF
     IF (findstring("FREE_TEXT_IND,",chg_str)=0)
      acm_request->prsnl_qual[idx].free_text_ind = p.free_text_ind
     ENDIF
     IF (findstring("FT_ENTITY_ID,",chg_str)=0)
      acm_request->prsnl_qual[idx].ft_entity_id = p.ft_entity_id
     ENDIF
     IF (findstring("FT_ENTITY_NAME,",chg_str)=0)
      acm_request->prsnl_qual[idx].ft_entity_name = p.ft_entity_name
     ENDIF
     IF (findstring("LOG_ACCESS_IND,",chg_str)=0)
      acm_request->prsnl_qual[idx].log_access_ind = p.log_access_ind
     ENDIF
     IF (findstring("LOG_LEVEL,",chg_str)=0)
      acm_request->prsnl_qual[idx].log_level = p.log_level
     ENDIF
     IF (findstring("PASSWORD,",chg_str)=0)
      acm_request->prsnl_qual[idx].password = p.password
     ENDIF
     IF (findstring("PHYSICIAN_IND,",chg_str)=0)
      acm_request->prsnl_qual[idx].physician_ind = p.physician_ind
     ENDIF
     IF (findstring("PHYSICIAN_STATUS_CD,",chg_str)=0)
      acm_request->prsnl_qual[idx].physician_status_cd = p.physician_status_cd
     ENDIF
     IF (findstring("PRIM_ASSIGN_LOC_CD,",chg_str)=0)
      acm_request->prsnl_qual[idx].prim_assign_loc_cd = p.prim_assign_loc_cd
     ENDIF
     IF (findstring("SECTION_CD,",chg_str)=0)
      acm_request->prsnl_qual[idx].section_cd = p.section_cd
     ENDIF
     IF (findstring("USERNAME,",chg_str)=0)
      acm_request->prsnl_qual[idx].username = p.username
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->prsnl_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->prsnl_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
     IF (((findstring("ACTIVE_IND,",chg_str) != 0) OR (findstring("ACTIVE_STATUS_CD,",chg_str) != 0
     )) )
      active_status_prsnl_id = reqinfo->updt_id, active_status_dt_tm = cnvtdatetime(curdate,curtime3)
     ELSE
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_qual[xref->chg[index].idx].status=0))
