CREATE PROGRAM acm_del_service_category_hist:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (s
   WHERE (s.svc_cat_hist_id=acm_request->service_category_hist_qual[xref->del[d.seq].idx].
   svc_cat_hist_id))
  WITH nocounter, status(reply->service_category_hist_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->service_category_hist_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,s.svc_cat_hist_id,acm_request->service_category_hist_qual[xref->del[
     t1].idx].svc_cat_hist_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,s.svc_cat_hist_id,acm_request->service_category_hist_qual[xref->
      del[t1].idx].svc_cat_hist_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->service_category_hist_qual[xref->del[index].idx].status=0))
