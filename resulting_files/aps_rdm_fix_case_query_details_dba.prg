CREATE PROGRAM aps_rdm_fix_case_query_details:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_rdm_fix_case_query_details"
 CALL logscriptstart(sreadme_name)
 SELECT INTO "nl:"
  u.column_name
  FROM user_tab_columns u
  WHERE u.table_name="AP_CASE_QUERY_DETAILS"
   AND u.column_name="QUERY_DETAIL_ID"
  WITH nocounter
 ;end select
 IF (curqual=0)
  CALL parser("rdb alter table ap_case_query_details add (query_detail_id number) go")
  CALL parser("ORAGEN3 'AP_CASE_QUERY_DETAILS' GO")
 ENDIF
 UPDATE  FROM ap_case_query_details acqd
  SET acqd.query_detail_id = seq(pathnet_seq,nextval), acqd.updt_dt_tm = cnvtdatetime(curdate,
    curtime3), acqd.updt_id = 999999,
 ...
 ;end update
#exit_script
 CALL logstatus("Updating of case_query_id field complete.","S")
