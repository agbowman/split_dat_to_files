CREATE PROGRAM dm_purge_toc_default_view_rows:dba
 ...
 SET reply->table_name = "TOC_DEFAULT_VIEW"
 SET reply->rows_between_commit = 100
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 SELECT INTO "nl:"
  tdv.rowid
  FROM toc_default_view tdv
  WHERE parser(sbr_getrowidnotexists("tdv.prsnl_id > 0","tdv"))
   AND tdv.prsnl_id IN (
  (SELECT
   p.person_id
   FROM prsnl p
  ...
  DETAIL
   row_cnt = (row_cnt+ 1)
   IF (mod(row_cnt,10)=1)
    stat = alterlist(reply->rows,(row_cnt+ 9))
   ENDIF
   reply->rows[row_cnt].row_id = tdv.rowid
  FOOT REPORT
