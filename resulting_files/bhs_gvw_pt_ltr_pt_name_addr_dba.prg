CREATE PROGRAM bhs_gvw_pt_ltr_pt_name_addr:dba
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   ms_city_state_zip = concat(trim(a.city),", ",trim(evaluate(a.state_cd,0.0,a.state,
    ...
    (p.name_first)," ",trim(p.name_last),
    "}",ms_reol,trim(ms_street_addr),ms_reol,"{",
    trim(ms_city_state_zip),"}",ms_reol,ms_rtfeof)
  WITH nocounter
 ;end select
#exit_script
