CREATE PROGRAM bhs_rpt_surg_sched_info:dba
 PROMPT
  ...
  ms_end_dt, s_output_type, s_emails
 DECLARE mf_cs23056_email = f8 WITH constant(uar_get_code_by("DISPLAYKEY",23056,"EMAIL")), protect
 ...
 DECLARE ms_var_out_consult = vc WITH noconstant("surg_upcoming_sched.csv"), protect
 DECLARE ms_report_name = vc WITH noconstant("Patient Info for Scheduled Surgeries"), protect
 EXECUTE reportrtl
 ...
 SUBROUTINE (sec_body1abs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   protect
   DECLARE __emails = vc WITH noconstant(build2(m_case->qual[ml_idx1].s_email,char(0))), protect
   DECLARE __mobile_numbers = vc WITH noconstant(build2(m_case->qual[ml_idx1].s_mobile_num,char(0))),
   protect
   DECLARE __primary_health_plan = vc WITH noconstant(build2(m_case->qual[ml_idx1].
     s_primary_heatlh_plan,char(0))), protect
   DECLARE __emails1 = vc WITH noconstant(build2(m_case->qual[ml_idx1].s_cmrn,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET rptsd->m_height = 0.209
    SET _dummyfont = uar_rptsetfont(_hreport,_times8b0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Mobile Number:",char(0)))
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.188)
    ...
    SET rptsd->m_width = 0.417
    SET rptsd->m_height = 0.209
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Email: ",char(0)))
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.188)
    ...
    SET rptsd->m_height = 0.209
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Primary Heatlh Plan:",char(0)))
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    ...
    SET rptsd->m_height = 0.209
    SET _dummyfont = uar_rptsetfont(_hreport,_times80)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__emails)
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.188)
    ...
    SET rptsd->m_width = 0.938
    SET rptsd->m_height = 0.209
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__mobile_numbers)
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.188)
    ...
    SET rptsd->m_height = 0.209
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__primary_health_plan)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.209)
    SET rptsd->m_x = (offsetx+ 0.000)
    SET rptsd->m_width = 0.417
    SET rptsd->m_height = 0.209
    SET _dummyfont = uar_rptsetfont(_hreport,_times8b0)
    ...
    SET rptsd->m_height = 0.209
    SET _dummyfont = uar_rptsetfont(_hreport,_times80)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__emails1)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_RPT_SURG_SCHED_INFO"
 ...
 IF (findstring("@", $S_EMAILS) > 0)
  SET ms_beg_dt_tm = format(datetimefind(cnvtlookahead("0,D",cnvtdatetime(curdate,0)),"D","B","B"),
   "DD-MMM-YYYY hh:mm:ss;;D")
  SET ms_end_dt_tm = format(datetimefind(cnvtlookahead("30,D",cnvtdatetime(curdate,235959)),"D","E",
 ...
 SELECT INTO "nl:"
  ...
  PLAN (sc
   ...
   WHERE (epr.active_ind= Outerjoin(1))
    AND (epr.encntr_id= Outerjoin(sc.encntr_id))
   ...
   WHERE (email.parent_entity_id= Outerjoin(p.person_id))
    ...
    AND (email.phone_type_cd= Outerjoin(value(uar_get_code_by("DISPLAYKEY",43,"EXTERNALSECURE"))))
  ...
  HEAD sc.surg_case_nbr_formatted
   ...
   m_case->qual[m_case->l_cnt].s_case_nbr = sc.surg_case_nbr_formatted, m_case->qual[m_case->l_cnt].
   s_start_date = format(sc.sched_start_dt_tm,"DD-MMM-YYYY;;D"), m_case->qual[m_case->l_cnt].
   s_start_time = format(sc.sched_start_dt_tm,"HH:MM:SS;;M"),
   m_case->qual[m_case->l_cnt].s_date_full = format(sc.sched_start_dt_tm,"@SHORTDATETIME"), m_case->
   qual[m_case->l_cnt].f_case_date = sc.sched_start_dt_tm, m_case->qual[m_case->l_cnt].s_or_room =
   uar_get_code_display(sc.sched_op_loc_cd),
   m_case->qual[m_case->l_cnt].f_personid = p.person_id, m_case->qual[m_case->l_cnt].s_pat_name = p
   .name_full_formatted, m_case->qual[m_case->l_cnt].s_pat_type = uar_get_code_display(sc
    .sched_pat_type_cd),
   m_case->qual[m_case->l_cnt].s_cmrn = pa.alias, m_case->qual[m_case->l_cnt].s_mrn = trim(ea.alias),
   m_case->qual[m_case->l_cnt].s_pat_dob = format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p
      .birth_tz),1),"mm/dd/yyyy;;d"),
   m_case->qual[m_case->l_cnt].f_surg_case_id = sc.surg_case_id, m_case->qual[m_case->l_cnt].
   s_latex_allergy = trim(sed.oe_field_display_value,3), m_case->qual[m_case->l_cnt].s_email = trim(
    email.phone_num,3),
   m_case->qual[m_case->l_cnt].s_mobile_num = trim(ph.phone_num,3), m_case->qual[m_case->l_cnt].
   s_primary_heatlh_plan = trim(hp.plan_name,3)
 ...
 ;end select
 SELECT INTO "nl:"
  FROM (dummyt d1  WITH seq = size(m_case->qual,5)),
   phone_hist phh
  PLAN (d1
   WHERE (m_case->qual[d1.seq].s_email=null))
   JOIN (phh
   WHERE (phh.parent_entity_id=m_case->qual[d1.seq].f_personid)
    AND phh.parent_entity_name="PERSON_PATIENT"
    AND phh.contact_method_cd=mf_cs23056_email
    AND phh.phone_num="*@*"
    AND phh.active_ind=1
    AND phh.parent_beg_effective_dt_tm <= cnvtdatetime(sysdate)
    AND phh.parent_end_effective_dt_tm >= cnvtdatetime(sysdate))
  ORDER BY d1.seq, phh.phone_id, phh.transaction_dt_tm DESC
  HEAD d1.seq
   m_case->qual[d1.seq].s_email = trim(phh.phone_num,3)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (scp
   ...
   JOIN (pg
   WHERE (pg.prsnl_group_id= Outerjoin(scp.sched_surg_specialty_id))
  ...
  HEAD scp.surg_case_proc_id
   ...
   IF (pg.prsnl_group_id > 0)
    m_case->qual[ml_idx2].proc[m_case->qual[ml_idx2].p_cnt].s_surg_specialty = trim(pg
     .prsnl_group_name,3)
   ENDIF
 ...
 IF (( $S_OUTPUT_TYPE=1))
  IF (findstring("@", $S_EMAILS)=0)
   SELECT INTO  $OUTDEV
    surgical_case_nbr = substring(1,30,m_case->qual[d1.seq].s_case_nbr), location = substring(1,100,
     m_case->qual[d1.seq].s_location), start_date = substring(1,30,m_case->qual[d1.seq].s_start_date),
    qual_s_start_time = substring(1,30,m_case->qual[d1.seq].s_start_time), or_room = substring(1,30,
     m_case->qual[d1.seq].s_or_room), pat_name = substring(1,100,m_case->qual[d1.seq].s_pat_name),
    pat_type = substring(1,30,m_case->qual[d1.seq].s_pat_type), dob = substring(1,30,m_case->qual[d1
     .seq].s_pat_dob), procedure_name = substring(1,100,m_case->qual[d1.seq].proc[d2.seq].s_proc_name
     ),
    ancillary_code = substring(1,50,m_case->qual[d1.seq].proc[d2.seq].s_ancillary), modifier =
    substring(1,30,m_case->qual[d1.seq].proc[d2.seq].s_modifier), mrn = substring(1,30,m_case->qual[
     d1.seq].s_mrn),
    cmrn = substring(1,30,m_case->qual[d1.seq].s_cmrn), email = substring(1,100,m_case->qual[d1.seq].
     s_email), mobile_num = substring(1,50,m_case->qual[d1.seq].s_mobile_num),
    primary_heatlh_plan = substring(1,70,m_case->qual[d1.seq].s_primary_heatlh_plan),
    primary_surgeon_name = substring(1,100,m_case->qual[d1.seq].proc[d2.seq].s_prim_surg_name),
    surgical_specialty = substring(1,30,m_case->qual[d1.seq].proc[d2.seq].s_surg_specialty),
    latex_allergy = substring(1,30,m_case->qual[d1.seq].s_latex_allergy), anes_type = substring(1,30,
     m_case->qual[d1.seq].proc[d2.seq].s_anes_type), duration = substring(1,30,m_case->qual[d1.seq].
     proc[d2.seq].s_duration),
    total_dur = m_case->qual[d1.seq].l_tot_dur
    FROM (dummyt d1  WITH seq = size(m_case->qual,5)),
     (dummyt d2  WITH seq = 1)
    PLAN (d1
     WHERE maxrec(d2,size(m_case->qual[d1.seq].proc,5)))
     JOIN (d2)
    ORDER BY location
    WITH nocounter, separator = " ", format
   ;end select
  ELSEIF (findstring("@", $S_EMAILS) > 0)
   ...
   SET frec->file_buf = build('"Surgical Case Number",','"Location",','"Start Date",','"Start Time",',
    '"OR Room",',
    '"Patient Name",','"Patien Type",','"DOB",','"Procedure Name",','"Ancillary Code",',
    '"Modifier",','"MRN",','"CMRN",','"Email",','"Mobile Number",',
    '"primary_heatlh_plan",','"Primary Surgeon Name",','"surgical_specialty",','"Latex Allergy",',
    '"Anesthesia Type",',
    '"Procedure Duration",','"Total Duration",',char(13))
   SET stat = cclio("WRITE",frec)
   FOR (ml_loop1 = 1 TO size(m_case->qual,5))
     FOR (ml_loop2 = 1 TO size(m_case->qual[ml_loop1].proc,5))
      SET frec->file_buf = build('"',substring(1,30,m_case->qual[ml_loop1].s_case_nbr),'","',
       substring(1,30,m_case->qual[ml_loop1].s_case_nbr),'","',
       substring(1,30,m_case->qual[ml_loop1].s_start_date),'","',substring(1,30,m_case->qual[ml_loop1
        ].s_start_time),'","',substring(1,30,m_case->qual[ml_loop1].s_or_room),
       '","',substring(1,100,m_case->qual[ml_loop1].s_pat_name),'","',substring(1,30,m_case->qual[
        ml_loop1].s_pat_type),'","',
       substring(1,30,m_case->qual[ml_loop1].s_pat_dob),'","',substring(1,100,m_case->qual[ml_loop1].
        proc[ml_loop2].s_proc_name),'","',substring(1,50,m_case->qual[ml_loop1].proc[ml_loop2].
        s_ancillary),
       '","',substring(1,30,m_case->qual[ml_loop1].proc[ml_loop2].s_modifier),'","',substring(1,30,
        m_case->qual[ml_loop1].s_mrn),'","',
       substring(1,30,m_case->qual[ml_loop1].s_cmrn),'","',substring(1,100,m_case->qual[ml_loop1].
        s_email),'","',substring(1,50,m_case->qual[ml_loop1].s_mobile_num),
       '","',substring(1,70,m_case->qual[ml_loop1].s_primary_heatlh_plan),'","',substring(1,100,
        m_case->qual[ml_loop1].proc[ml_loop2].s_prim_surg_name),'","',
       substring(1,30,m_case->qual[ml_loop1].proc[ml_loop2].s_surg_specialty),'","',substring(1,30,
        m_case->qual[ml_loop1].s_latex_allergy),'","',substring(1,30,m_case->qual[ml_loop1].proc[
        ml_loop2].s_anes_type),
       '","',substring(1,30,m_case->qual[ml_loop1].proc[ml_loop2].s_duration),'","',m_case->qual[
       ml_loop1].l_tot_dur,'"',
       char(13))
      SET stat = cclio("WRITE",frec)
     ENDFOR
   ...
   SET ms_subject = build2(ms_report_name," for GI Next 30 Days ","from:",trim(substring(1,11,
      ms_beg_dt_tm),3)," to ",
    trim(substring(1,11,ms_end_dt_tm),3))
