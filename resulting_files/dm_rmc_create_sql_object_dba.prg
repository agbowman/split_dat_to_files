CREATE PROGRAM dm_rmc_create_sql_object:dba
 ...
 SUBROUTINE drmmi_backfill_mock_id(dbmi_env_id)
   ...
   RETURN(dbmi_mock_id)
 END ;Subroutine
 DECLARE drsl_obj_name = vc WITH protect, noconstant("")
 DECLARE drsl_src_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drsl_env_no = f8 WITH protect, noconstant(0.0)
 DECLARE drsl_tgt_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drsl_obj_cnt = i4 WITH protect, noconstant(0)
 DECLARE drsl_text_num = i4 WITH protect, noconstant(0)
 DECLARE dcf_continue_ind = i2 WITH protect, noconstant(0)
 DECLARE drcf_valid_cnt = i4 WITH protect, noconstant(0)
 DECLARE drcf_status = vc WITH protect, noconstant("")
 DECLARE drsl_mock_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drsl_obj_file = vc WITH protect, noconstant("")
 DECLARE drsl_ccl_vers = i4 WITH protect, constant(((currevminor2+ (currevminor * 100))+ (currev *
  10000)))
 DECLARE drsl_utc_cur_dt_tm = vc WITH protect, noconstant("")
 DECLARE drsl_cbc_setting = vc WITH protect, noconstant("0")
 FREE RECORD drsl_objects
 RECORD drsl_objects(
   1 qual[*]
     2 object_name = vc
     2 table_name = vc
 ...
 IF (validate(reply->status_data,"XYZ")="XYZ")
  FREE RECORD reply
  RECORD reply(
    1 status_data
      2 status = c1
 ...
 IF (check_logfile("dm_create_sql",".log","dm_create_sql LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm_rmc_create_sql_object..."
 CALL disp_msg("",dm_err->logfile,0)
 IF (validate(request->object_name,"zzz")="zzz"
  AND validate(request->object_name,"yyy")="yyy")
  SET drsl_obj_name =  $1
  SET drsl_src_env_id =  $2
 ELSE
  SET drsl_obj_name = request->object_name
  SET drsl_src_env_id = request->source_env_id
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
 ...
 SELECT INTO "nl:"
  ...
  WHERE drso.active_ind=1
   AND drso.object_name=patstring(cnvtupper(trim(drsl_obj_name,3)))
  ORDER BY drso.creation_level
  DETAIL
   drsl_obj_cnt = (drsl_obj_cnt+ 1), stat = alterlist(drsl_objects->qual,drsl_obj_cnt), drsl_objects
   ->qual[drsl_obj_cnt].object_name = drso.object_name,
   drsl_objects->qual[drsl_obj_cnt].table_name = drso.table_name, drsl_objects->qual[drsl_obj_cnt].
   column_name = drso.column_name, drsl_objects->qual[drsl_obj_cnt].object_type = drso.object_type
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  ...
  WHERE di.info_domain="RDDS ENV PAIR"
   AND di.info_name=concat(trim(cnvtstring(drsl_src_env_id,20)),"::",trim(cnvtstring(drsl_tgt_env_id,
     20)))
  DETAIL
   drsl_env_no = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual=0)
  SELECT INTO "nl:"
   drsl_seq = seq(dm_clinical_seq,nextval)
   FROM dual
   DETAIL
    drsl_env_no = drsl_seq
   WITH nocounter
  ;end select
  INSERT  FROM dm_info
   SET info_domain = "RDDS ENV PAIR", info_name = concat(trim(cnvtstring(drsl_src_env_id,20)),"::",
     trim(cnvtstring(drsl_tgt_env_id,20))), info_number = drsl_env_no
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  ENDIF
 ENDIF
 SET drsl_mock_env_id = drmmi_get_mock_id(drsl_tgt_env_id)
 IF (drsl_mock_env_id < 0.0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->err_ind = 1
  SET reply->status_data.status = "F"
  GO TO exit_program
 ENDIF
 SET drsl_obj_file = concat("dm_rmc_sql_obj",trim(cnvtstring(currdbhandle)),".sql")
 IF (curutc=0)
  SET drsl_utc_cur_dt_tm = " SYSDATE "
 ELSE
  SET drsl_utc_cur_dt_tm = " SYS_EXTRACT_UTC(SYSTIMESTAMP) "
 ENDIF
 FOR (drsl_objs = 1 TO size(drsl_objects->qual,5))
   SET dcf_continue_ind = 1
   SELECT INTO "nl:"
    FROM user_tab_columns utc
    WHERE (utc.table_name=drsl_objects->qual[drsl_objs].table_name)
     AND (utc.column_name=drsl_objects->qual[drsl_objs].column_name)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    GO TO exit_program
   ELSEIF (curqual=0
    AND trim(drsl_objects->qual[drsl_objs].table_name) > " ")
    CALL echo("Column does not exist in this domain, object will not be created.")
    SET dcf_continue_ind = 0
   ENDIF
   IF (dcf_continue_ind=1)
    SELECT INTO "nl:"
     FROM dm_refchg_sql_obj_parm drsp
     WHERE (drsp.object_name=drsl_objects->qual[drsl_objs].object_name)
      AND  NOT ( EXISTS (
     (SELECT
      "x"
      FROM user_tab_columns utc
      WHERE utc.column_name=drsp.column_name
       AND (utc.table_name=drsl_objects->qual[drsl_objs].table_name))))
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ELSEIF (curqual > 0
     AND trim(drsl_objects->qual[drsl_objs].table_name) > " ")
     CALL echo(concat(
       "One of the columns listed as a parameter for the object does not exist on the table",
    ...
    IF (dcf_continue_ind=1)
     ...
     SELECT INTO "nl:"
      drsot.sql_text
      FROM dm_refchg_sql_obj_text drsot
      WHERE (drsot.object_name=drsl_objects->qual[drsl_objs].object_name)
      ORDER BY drsot.line_number
      DETAIL
       drsl_text_num = (drsl_text_num+ 1)
       IF (mod(drsl_text_num,10)=1)
        stat = alterlist(drsl_objects->qual[drsl_objs].text,(drsl_text_num+ 9))
       ENDIF
       drsl_objects->qual[drsl_objs].text[drsl_text_num].line_number = drsot.line_number,
       drsl_objects->qual[drsl_objs].text[drsl_text_num].sql_text = drsot.sql_text
      FOOT REPORT
       stat = alterlist(drsl_objects->qual[drsl_objs].text,drsl_text_num)
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     ENDIF
     SET drsl_text_num = 0
     SELECT INTO value(drsl_obj_file)
      FROM (dummyt d  WITH seq = size(drsl_objects->qual[drsl_objs].text,5))
      DETAIL
       drsl_text_num = (drsl_text_num+ 1), drsl_objects->qual[drsl_objs].text[d.seq].sql_text =
       replace(replace(replace(replace(replace(replace(drsl_objects->qual[drsl_objs].text[d.seq].
             sql_text,"::SOURCE::",trim(cnvtstring(drsl_src_env_id,20)),0),"::TARGET::",trim(
             cnvtstring(drsl_tgt_env_id,20)),0),"::SOURCE TARGET MAPPING::",trim(cnvtstring(
             drsl_env_no,20)),0),"::MOCK::",trim(cnvtstring(drsl_mock_env_id,20)),0),
         "::UTC CUR DATE::",trim(drsl_utc_cur_dt_tm),0),"::CBC SETTING IND::",drsl_cbc_setting,0),
       CALL print(concat(drsl_objects->qual[drsl_objs].text[d.seq].sql_text,char(10))),
       row + 1
       IF (drsl_text_num=size(drsl_objects->qual[drsl_objs].text,5))
        CALL print("/")
       ENDIF
      WITH nocounter, nullreport, format = variable,
       maxrow = 1, maxcol = 300, formfeed = none
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dm_err->err_ind = 1
      ...
      GO TO exit_program
     ENDIF
     IF (drsl_ccl_vers > 80501)
      CALL parser(concat(" rdb read '",value(drsl_obj_file),"' with full end go"),1)
     ELSE
      CALL parser(concat(" rdb read '",value(drsl_obj_file),"' end go"),1)
     ENDIF
     SELECT INTO "nl:"
      FROM user_objects uo
      WHERE uo.object_name=replace(drsl_objects->qual[drsl_objs].object_name,
       "::SOURCE TARGET MAPPING::",trim(cnvtstring(drsl_env_no,20)))
       AND (uo.object_type=drsl_objects->qual[drsl_objs].object_type)
       AND uo.status="VALID"
      DETAIL
       drcf_valid_cnt = (drcf_valid_cnt+ 1)
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     ELSEIF (curqual=0)
      SET reply->status_data.status = "F"
      SET dm_err->emsg = concat("Object ",drsl_objects->qual[drsl_objs].object_name,
       " failed creation.")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
...
#exit_program
 ...
 IF ((reply->status_data.status="S"))
  SET dm_err->emsg = "All functions from dm_refchg_sql_obj have been created successfully."
 ENDIF
