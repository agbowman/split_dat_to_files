CREATE PROGRAM ccps_pha_lbt_header
 PROMPT
 ...
 DECLARE facility_sec(ncalc=i2) = f8 WITH protect
 DECLARE facility_secabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 ...
 SUBROUTINE finalizereport(ssendreport)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE facility_sec(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = facility_secabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE facility_secabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.130000), private
   DECLARE __facility_description = vc WITH noconstant(build2(
     IF (print_file=build(logical("cer_temp"),dir_char,"phadrvr_",pdq,"_",
      cnvtint(data->run_id),"_",label_rec_ndx,"_foot.dat")) build2(trim(label_data->facility.
        description)," - Disp Rec")
     ELSE trim(label_data->facility.description)
     ENDIF
     ,char(0))), protect
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__facility_description)
 ...
 SUBROUTINE order_info_secabs(ncalc,offsetx,offsety)
   ...
   DECLARE __start_due_date = vc WITH noconstant(build2(
     IF (print_file=build(logical("cer_temp"),dir_char,"phadrvr_",pdq,"_",
      cnvtint(data->run_id),"_",label_rec_ndx,"_head.dat")) "** Bag Label **"
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CCPS_PHA_LBT_HEADER"
 ...
 SET _fdrawheight = facility_sec(rpt_calcheight)
 IF (((_yoffset+ _fdrawheight) > _fenddetail))
  CALL pagebreak(0)
 ENDIF
 SET dummy_val = facility_sec(rpt_render)
