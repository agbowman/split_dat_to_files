CREATE PROGRAM dm2_capture_schema_test:dba
 DECLARE c_mod = vc WITH private, constant("002")
 ...
 DECLARE cs_dest_dir = vc WITH protect
 DECLARE cs_ccl_dir = vc WITH protect
 ...
 DECLARE val_write_privs(vwp_dir=vc) = i4
 ...
 ENDIF
 IF ((dm2_sys_misc->cur_os="AXP"))
  SET cs_dest_dir = "cer_install"
  SET cs_ccl_dir = "cer_install"
 ELSE
  SET cs_dest_dir = "$cer_install"
  SET cs_ccl_dir = "cer_install"
 ENDIF
 IF (val_write_privs(cs_ccl_dir)=0)
  GO TO exit_script
 ENDIF
 ...
 CASE (cnvtupper(currdbuser))
  ...
  ELSE
   ...
   CALL disp_msg(concat(
     "You must be logged into the database as either CDBA or V500 to run this process. ",
     "Current database user is: ",currdbuser))
   GO TO exit_script
 ENDCASE
 SET cs_file_prefix = build(cs_sch_date_type,cnvtalphanum(format(cs_sch_date_fmt,"MM/DD/YYYY;;D")))
 SET dm_err->eproc = "CREATE EMPTY SCHEMA FILES"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (create_sch_files(cs_file_prefix,cs_dest_dir)=0)
  GO TO exit_script
 ENDIF
 ...
 CALL disp_msg(" ",dm_err->logfile,0)
 INSERT  FROM dmheader dh
  SET dh.description = sf_desc, dh.admin_load_ind = 0, dh.source_rdbms = currdb,
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "POPULATE DMTSPACE SCHEMA FILE WITH TABLESPACE INFO"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((cur_sch->tspace_cnt > 0))
  INSERT  FROM dmtspace ts,
    (dummyt d  WITH seq = value(cur_sch->tspace_cnt))
   SET ts.seq = 1, ts.tablespace_name = cur_sch->tspace[d.seq].tspace_name, ts.init_extent = cur_sch
    ->tspace[d.seq].initial_extent,
    ts.next_extent = cur_sch->tspace[d.seq].next_extent, ts.pct_increase = cur_sch->tspace[d.seq].
    pct_increase, ts.tspace_type = cur_sch->tspace[d.seq].tspace_type,
    ts.tspace_type_ni = cur_sch->tspace[d.seq].tspace_type_ni, ts.pagesize = cur_sch->tspace[d.seq].
    pagesize, ts.nodegroup = cur_sch->tspace[d.seq].nodegroup,
    ts.nodegroup_ni = cur_sch->tspace[d.seq].nodegroup_ni, ts.bufferpool_name = cur_sch->tspace[d.seq
    ].bufferpool_name, ts.bufferpool_name_ni = cur_sch->tspace[d.seq].bufferpool_name_ni,
    ts.schema_date = cnvtdatetime(cs_sch_date_str), ts.alpha_feature_nbr = 0, ts.feature_number = 0,
    ts.updt_dt_tm = cnvtdatetime(curdate,curtime3)
   PLAN (d)
    JOIN (ts
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "NO TABLESPACES IN CURRENT SCHEMA"
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SET dm_err->eproc = "POPULATE DMTABLE SCHEMA FILE WITH TABLE INFO"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((cur_sch->tbl_cnt > 0))
  ...
  ELSEIF (currdbuser="V500")
   SELECT INTO "nl:"
    ...
    PLAN (d)
     JOIN (td
     WHERE (td.table_name=cur_sch->tbl[d.seq].tbl_name))
    DETAIL
     cur_sch->tbl[d.seq].capture_ind = 1
    WITH nocounter
   ;end select
   SELECT INTO "dm2_no_capture"
    d.seq
    FROM (dummyt d  WITH seq = value(cur_sch->tbl_cnt))
    PLAN (d
     WHERE (cur_sch->tbl[d.seq].capture_ind=0))
    HEAD REPORT
     col 1, "tables with capture_ind=0", row + 2
    DETAIL
  ...
  INSERT  FROM dmtable t,
    (dummyt d  WITH seq = value(cur_sch->tbl_cnt))
   SET t.seq = 1, t.table_name = cur_sch->tbl[d.seq].tbl_name, t.tablespace_name = cur_sch->tbl[d.seq
    ].tspace_name,
    t.index_tspace = cur_sch->tbl[d.seq].ind_tspace, t.index_tspace_ni = cur_sch->tbl[d.seq].
    ind_tspace_ni, t.long_tspace = cur_sch->tbl[d.seq].long_tspace,
    t.long_tspace_ni = cur_sch->tbl[d.seq].long_tspace_ni, t.init_ext = cur_sch->tbl[d.seq].init_ext,
    t.next_ext = cur_sch->tbl[d.seq].next_ext,
    t.pct_increase = cur_sch->tbl[d.seq].pct_increase, t.pct_used = cur_sch->tbl[d.seq].pct_used, t
    .pct_free = cur_sch->tbl[d.seq].pct_free,
    t.schema_date = cnvtdatetime(cs_sch_date_str), t.alpha_feature_nbr = 0, t.feature_number = 0,
 ...
 IF ((cur_sch->sequence_cnt > 0))
  INSERT  FROM dmseq s,
   ...
   SET s.seq = 1, s.sequence_name = cur_sch->sequence[d.seq].seq_name, s.min_value = cur_sch->
    ...
    s.schema_date = cnvtdatetime(cs_sch_date_str), s.alpha_feature_nbr = 0, s.feature_number = 0,
...
#exit_script
 IF ((dm_err->debug_flag=1))
  ...
  SELECT INTO value(dm_err->logfile)
   ...
   PLAN (d)
   HEAD REPORT
    row + 2, col 25, "Counts of rows in schema files vs. Cur_Sch record",
    row + 1, row + 1, "DMHEADER:",
    col 20, dm2h_cnt, row + 1,
    "DMTSPACE:", col 20, dm2tsp_cnt,
    col 35, "Cur_Sch tspace cnt:", col 70,
    cur_sch->tspace_cnt, row + 1, "DMTABLE:",
    col 20, dm2tbl_cnt, col 35,
    "Cur_Sch table cnt:", col 70, cur_sch->tbl_cnt,
    row + 1, "DMCOLUMN:", col 20,
    dm2col_cnt, col 35, "Cur_Sch column cnt:",
    col 70, ttl_tc_cnt, row + 1,
    "DMINDEX:", col 20, dm2ind_cnt,
    col 35, "Cur_Sch index cnt:", col 70,
    ttl_ind_cnt, row + 1, "DMINDCOL:",
    col 20, dm2icol_cnt, col 35,
    "Cur_Sch index column cnt:", col 70, ttl_icol_cnt,
    row + 1, "DMCONS:", col 20,
    dm2cons_cnt, col 35, "Cur_Sch constraint cnt:",
    col 70, ttl_cons_cnt, row + 1,
    "DMCONSCOL:", col 20, dm2ccol_cnt,
    col 35, "Cur_Sch cons column cnt:", col 70,
    ttl_ccol_cnt, row + 1, "DMSEQ:",
    col 20, dm2seq_cnt, col 35,
    "Cur_Sch index column cnt:", col 70, cur_sch->sequence_cnt,
    row + 1, "DMTBLDOC:", col 20,
    dm2tdoc_cnt, row + 1, "DMCOLDOC:",
    col 20, dm2cdoc_cnt, row + 2,
    " "
 ...
 ENDIF
 SUBROUTINE val_write_privs(vwp_dir)
