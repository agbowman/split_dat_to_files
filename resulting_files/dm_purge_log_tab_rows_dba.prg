CREATE PROGRAM dm_purge_log_tab_rows:dba
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     jl.rowid
     FROM dm_purge_job_log jl
     WHERE parser(sbr_getrowidnotexists("jl.log_id between curMinID and curMaxID","jl"))
      AND ((jl.updt_dt_tm+ 0) < cnvtdatetime((curdate - v_days_to_keep),curtime3))
     DETAIL
      row_cnt = (row_cnt+ 1)
      IF (mod(row_cnt,10)=1)
       stat = alterlist(reply->rows,(row_cnt+ 9))
      ENDIF
      reply->rows[row_cnt].row_id = jl.rowid
     WITH nocounter, maxqual(jl,value(rowsleft))
    ;end select
