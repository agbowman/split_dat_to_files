CREATE PROGRAM bbt_add_hist_person_antigen:dba
 ...
 DECLARE script_name = c40 WITH public, constant("bbt_add_hist_person_antigen")
 ...
 FOR (i = 1 TO size(request->qual,5))
   ...
   ENDIF
   IF (check_if_attribute_exists(request->qual[i].person_id,request->qual[i].antigen_cd,request->
    contributor_system_cd)=0)
    GO TO exit_script
   ENDIF
   IF (exists_ind=0)
    IF (insert_attribute(request->qual[i].person_id,request->qual[i].encntr_id,request->qual[i].
     antigen_cd,1,item_dt_tm,
 ...
 SUBROUTINE (perform_data_validation(person_id_new=f8,encounter_id=f8,attribute_cd=f8,code_set_new=i4
   ...
   ELSE
    IF (curqual=0)
     CALL fill_out_status_data("F","Validate attribute_cd",concat("Antigen_cd ",trim(cnvtstring(
 ...
 SUBROUTINE (insert_attribute(person_id_new=f8,encounter_id=f8,attribute_cd=f8,active_ind_new=i2,
  active_status_dt_tm=dq8,active_status_prsnl_id_new=f8,contributor_system_cd_new=f8) =i2)
   ...
   ;end select
   INSERT  FROM person_antigen pa
    SET pa.person_antigen_id = new_pathnet_id, pa.person_id = person_id_new, pa.encntr_id =
     encounter_id,
     pa.antigen_cd = attribute_cd, pa.active_ind = active_ind_new, pa.active_status_cd =
   ...
   ELSE
    IF (curqual=0)
     CALL fill_out_status_data("F","Insert person_antigen",concat("Unable to insert antigen ",trim(
