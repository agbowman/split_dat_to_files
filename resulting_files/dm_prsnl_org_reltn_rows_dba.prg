CREATE PROGRAM dm_prsnl_org_reltn_rows:dba
 ...
 SET reply->table_name = "PRSNL_ORG_RELTN"
 SET reply->rows_between_commit = 100
 SET i18nhandle = 0
 SET i18nhandleinit = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 DECLARE batch_size = f8 WITH protect, noconstant(50000.0)
 DECLARE max_id = f8 WITH protect, noconstant(0.0)
 DECLARE cur_min_id = f8 WITH protect, noconstant(1.0)
 DECLARE cur_max_id = f8 WITH protect, noconstant(0.0)
 DECLARE rows_left = i4 WITH protect, noconstant(request->max_rows)
 DECLARE rows = i4 WITH protect, noconstant(0)
 DECLARE errcode2 = i4 WITH protect, noconstant(0)
 DECLARE errmsg2 = vc WITH protect, noconstant("")
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(por.prsnl_org_reltn_id)
   FROM prsnl_org_reltn por
   WHERE por.prsnl_org_reltn_id > 0.0
   DETAIL
    cur_min_id = maxval(cnvtreal(minval),1.0)
   WITH nocounter
  ;end select
 ELSE
  SET cur_min_id = sbr_fetch_starting_id(null)
 ENDIF
 SELECT INTO "nl:"
  maxval = max(por.prsnl_org_reltn_id)
  FROM prsnl_org_reltn por
  DETAIL
   max_id = cnvtreal(maxval)
  WITH nocounter
 ;end select
 SET cur_max_id = (cur_min_id+ (batch_size - 1))
 WHILE (cur_min_id <= max_id
  AND rows_left > 0)
   SELECT INTO "nl:"
    por.rowid
    FROM prsnl_org_reltn por
    WHERE por.prsnl_org_reltn_id BETWEEN cur_min_id AND cur_max_id
     AND parser(sbr_getrowidnotexists(
      "por.end_effective_dt_tm < cnvtdatetimeutc(SYSDATE) or por.active_ind = 0","por"))
     AND por.prsnl_org_reltn_id != 0.0
    DETAIL
     rows += 1
     IF (mod(rows,100)=1)
      stat = alterlist(reply->rows,(rows+ 99))
     ENDIF
     reply->rows[rows].row_id = por.rowid
    WITH nocounter, maxqual(por,value(rows_left))
   ;end select
   SET errcode2 = error(errmsg2,1)
   IF (errcode2 != 0)
    SET reply->err_code = errcode2
    SET reply->status_data.status = "F"
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s2","Failed in row collection: %1","s",
     nullterm(errmsg2))
    GO TO exit_script
   ENDIF
   CALL sbr_update_starting_id(cur_min_id)
   SET cur_min_id = (cur_max_id+ 1)
   SET cur_max_id = (cur_min_id+ (batch_size - 1))
   SET rows_left = (request->max_rows - rows)
 ENDWHILE
