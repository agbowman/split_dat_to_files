CREATE PROGRAM bhs_athn_add_clin_doc_v3
 FREE RECORD req969530
 RECORD req969530(
   1 ensure_type = i2
   1 ppr_cd = f8
   ...
   1 format_cd = f8
   1 workflow_id = f8
   1 dqr_reference_id = vc
 )
 FREE RECORD rep969530
 RECORD rep969530(
   1 document_event_id = f8
   1 version = i4
   1 document_section_event_id = f8
   1 status_cd = f8
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   req969530->ppr_cd = epr.encntr_prsnl_r_cd
  WITH nocounter, time = 30
 ;end select
 SET req969530->ensure_type = evaluate( $14,1,1,2)
 SET req969530->patient_id =  $2
 SET req969530->encntr_id =  $3
 SET req969530->event_cd =  $4
 SET req969530->author_id =  $5
 SET req969530->user_id =  $5
 SET req969530->title = trim(replace(replace(replace(replace(replace(replace(replace(replace(replace(
            $8,"&amp;","&",0),"&lt;","<",0),"&gt;",">",0),"&apos;","'",0),"&quot;",'"',0),"&rsquo;",
      "'",0),"&lsquo;","'",0),"&hellip;","",0),"<br>","",0),3)
 SET req969530->action_dt_tm = action_dt_tm
 SET req969530->action_tz = 126
 SET req969530->service_dt_tm = action_dt_tm
 SET req969530->service_tz = 126
 SET req969530->content = t_blob
 SET req969530->format_cd = uar_get_code_by("MEANING",23, $13)
 SET stat = tdbexecute(600005,3202004,969530,"REC",req969530,
  "REC",rep969530)
 CALL echorecord(rep969530)
 IF ((rep969530->status_data.status="S"))
  SET oreply->status = "S"
  SET stat = alterlist(oreply->rb_list,1)
  SET oreply->rb_list[1].parent_event_id = rep969530->document_event_id
  SET oreply->rb_list[1].event_id = rep969530->document_section_event_id
 ELSE
  SET oreply->status = "F"
 ENDIF
#exit_script
