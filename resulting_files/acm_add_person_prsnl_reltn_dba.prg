CREATE PROGRAM acm_add_person_prsnl_reltn:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->person_prsnl_reltn_qual[xref->add[index].idx].person_prsnl_reltn_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(person_seq,nextval)
     FROM dual
     DETAIL
      acm_request->person_prsnl_reltn_qual[xref->add[index].idx].person_prsnl_reltn_id = cnvtreal(
       nextseqnum), reply->person_prsnl_reltn_qual[xref->add[index].idx].person_prsnl_reltn_id =
      cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->person_prsnl_reltn_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->person_prsnl_reltn_qual[xref->add[index].idx].person_prsnl_reltn_id = acm_request->
    person_prsnl_reltn_qual[xref->add[index].idx].person_prsnl_reltn_id
   ENDIF
   SET reply->person_prsnl_reltn_qual[xref->add[index].idx].status = 0
   IF ((acm_request->person_prsnl_reltn_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->person_prsnl_reltn_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime
    (curdate,curtime3)
   ENDIF
   IF ((acm_request->person_prsnl_reltn_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->person_prsnl_reltn_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime
    ("31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   person_prsnl_reltn p
  SET p.person_prsnl_reltn_id = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].
   person_prsnl_reltn_id, p.beg_effective_dt_tm = cnvtdatetime(acm_request->person_prsnl_reltn_qual[
    xref->add[d.seq].idx].beg_effective_dt_tm), p.contributor_system_cd =
   IF ((acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.end_effective_dt_tm = cnvtdatetime(acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].
    end_effective_dt_tm), p.free_text_cd = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx]
   .free_text_cd, p.ft_prsnl_name = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].
   ft_prsnl_name,
   p.internal_seq = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].internal_seq, p
   .manual_create_by_id = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].
   manual_create_by_id, p.manual_create_dt_tm =
   IF ((acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].manual_create_dt_tm > 0))
    cnvtdatetime(acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].manual_create_dt_tm)
   ELSE null
   ENDIF
   ,
   p.manual_create_ind = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].manual_create_ind,
   p.manual_inact_by_id = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].
   manual_inact_by_id, p.manual_inact_dt_tm =
   IF ((acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].manual_inact_dt_tm > 0))
    cnvtdatetime(acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].manual_inact_dt_tm)
   ELSE null
   ENDIF
   ,
   p.manual_inact_ind = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].manual_inact_ind,
   p.notification_cd = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].notification_cd, p
   .person_id =
   IF ((acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].person_id > 0)) acm_request->
    person_prsnl_reltn_qual[xref->add[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].person_idx].
    person_id
   ENDIF
   ,
   p.person_prsnl_r_cd = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].person_prsnl_r_cd,
   p.priority_seq = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].priority_seq, p
   .prsnl_person_id = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].prsnl_person_id,
   p.source_identifier = acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].source_identifier,
   p.data_status_cd =
   IF ((acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request
    ->person_prsnl_reltn_qual[xref->add[d.seq].idx].data_status_cd
   ...
   IF ((((acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((
   acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].active_status_cd=reqdata->
   active_status_cd))) ) 1
   ELSE 0
   ENDIF
   , p.active_status_cd =
   IF ((acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].active_status_cd > 0.0))
    acm_request->person_prsnl_reltn_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->person_prsnl_reltn_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->person_prsnl_reltn_qual[xref->add[index].idx].status=0))
