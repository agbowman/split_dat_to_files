CREATE PROGRAM bbt_rpt_dispos_reas:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->dispose_reasons = uar_i18ngetmessage(i18nhandle,"dispose_reasons","DISPOSE REASONS")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_dispo_rsn", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  PLAN (cv
   WHERE cv.code_set=1608)
   JOIN (d_cd
   WHERE d_cd.seq=1)
   JOIN (cd
   WHERE cd.code_set=1608
  ...
  HEAD PAGE
   ...
   col 51, captions->dispose_reasons, row + 2,
