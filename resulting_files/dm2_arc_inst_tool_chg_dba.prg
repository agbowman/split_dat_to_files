CREATE PROGRAM dm2_arc_inst_tool_chg:dba
 IF ((validate(dasc_request->tbl_cnt,- (1))=- (1)))
  FREE RECORD dasc_request
  RECORD dasc_request(
    1 tabs[*]
      2 table_name = vc
  )
 ENDIF
 IF (validate(dasc_reply->status_data.status,"X")="X")
  FREE RECORD dasc_reply
  RECORD dasc_reply(
    1 status_data
      2 status = c1
      2 subeventstatus[1]
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(daac_reply->archive_ind,- (1))=- (1)))
  FREE RECORD daac_reply
  RECORD daac_reply(
    1 archive_ind = i2
    1 status_data
      2 status = c1
 ...
 ENDIF
 IF (check_logfile("dm2_arc_inst_chg",".log","DM2_ARC_INST_TOOL_CHG LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DECLARE aitc_cnt = i4 WITH noconstant(0)
 SET dm_err->eproc = "starting DM2_ARC_INST_TOOL_CHG"
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_arc_active_check  WITH replace("REPLY","DAAC_REPLY")
 IF ((daac_reply->archive_ind != 1))
  SET dm_err->eproc = "It is not ARCHIVE database, exiting..."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  table_name
  FROM dm2_ddl_ops_log dol
  WHERE (dol.run_id= $1)
   AND ((dol.op_type IN ("ADD COLUMN", "CREATE TABLE")) OR (dol.op_type IN ("CREATE FK CONSTRAINT")
   AND dol.obj_name="XARC*"))
  DETAIL
   aitc_cnt = (aitc_cnt+ 1)
   IF (mod(aitc_cnt,10)=1)
    stat = alterlist(dasc_request->tabs,(aitc_cnt+ 9))
   ENDIF
   dasc_request->tabs[aitc_cnt].table_name = dol.table_name
  FOOT REPORT
   stat = alterlist(dasc_request->tabs,aitc_cnt)
  WITH nocounter
 ;end select
 IF (curqual=0)
  IF (check_error("Gather schema changes information") != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ELSE
   SET dm_err->eproc = "There are no schema changes for Archive database"
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_program
  ENDIF
 ENDIF
 EXECUTE dm2_arc_schema_change  WITH replace("REQUEST","DASC_REQUEST"), replace("REPLY","DASC_REPLY")
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
#exit_program
 FREE RECORD dasc_request
 FREE RECORD dasc_reply
 SET dm_err->eproc = "...Ending dm2_arc_inst_tool_chg"
 CALL final_disp_msg("dm2_arc_inst_chg")
END GO
