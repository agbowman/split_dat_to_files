CREATE PROGRAM bhs_athn_order_detail_ls_v2
 ...
 SELECT DISTINCT INTO  $1
  ...
  .stop_duration,
  oc.stop_duration_unit_cd, stop_duration_unit = uar_get_code_meaning(oc.stop_duration_unit_cd), t
  .reference_task_id,
  od1.action_sequence, dx_string = trim(replace(replace(replace(replace(replace(order_list->qual[d1
        .seq].dx_string,"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),
  oc_stop_type_meaning = uar_get_code_meaning(oc.stop_type_cd),
  oc.disable_order_comment_ind, o_protocol_order_id = trim(replace(cnvtstring(o.protocol_order_id),
    ".0*","",0),3), oc.ref_text_mask
  FROM (dummyt d1  WITH seq = size(order_list->qual,5)),
   orders o,
   order_action oa,
   prsnl prl,
  ...
  HEAD REPORT
   ...
   row + 1
  HEAD o.order_id
   IF (((o.dcp_clin_cat_cd=10576) OR (oc.dcp_clin_cat_cd=10576)) )
    header_grp = build("<","StandardLaboratoryOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=10573) OR (oc.dcp_clin_cat_cd=10573)) )
    header_grp = build("<","StandardRadiologyOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=10581) OR (oc.dcp_clin_cat_cd=10581)) )
    header_grp = build("<","AdmitTransferDischarge",">")
   ELSEIF (((o.dcp_clin_cat_cd=10571) OR (oc.dcp_clin_cat_cd=10571)) )
    header_grp = build("<","StandardDirectCare",">")
   ELSEIF (((o.dcp_clin_cat_cd=10574) OR (oc.dcp_clin_cat_cd=10574)) )
    header_grp = build("<","StandardDietOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=10584) OR (oc.dcp_clin_cat_cd=10584)) )
    header_grp = build("<","StandardChargeOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=2064789) OR (oc.dcp_clin_cat_cd=2064789)) )
    header_grp = build("<","NonCategorizedOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=10578) OR (oc.dcp_clin_cat_cd=10578)) )
    header_grp = build("<","NursingOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=10582) OR (oc.dcp_clin_cat_cd=10582)) )
    header_grp = build("<","RespiratoryOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=10583) OR (oc.dcp_clin_cat_cd=10583)) )
    header_grp = build("<","RehabServicesOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=10579) OR (oc.dcp_clin_cat_cd=10579)) )
    header_grp = build("<","SpecialServicesOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=10585) OR (oc.dcp_clin_cat_cd=10585)) )
    header_grp = build("<","VitalOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=559676402) OR (oc.dcp_clin_cat_cd=559676402)) )
    header_grp = build("<","MedicalSuppliesOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=10568) OR (oc.dcp_clin_cat_cd=10568)) )
    header_grp = build("<","ActivityOrder",">")
   ELSEIF (((o.dcp_clin_cat_cd=10580) OR (oc.dcp_clin_cat_cd=10580)) )
    header_grp = build("<","CareSet",">")
   ELSEIF (((o.dcp_clin_cat_cd=10570) OR (oc.dcp_clin_cat_cd=10570)) )
    header_grp = build("<","Condition",">")
   ELSE
    header_grp = build("<","NonCategorizedOrder",">")
   ENDIF
   row + 1, col + 1, header_grp,
   ...
   v15, row + 1, v18 = build("<OriginalOrderingProviderId>",cnvtint(oa.order_provider_id),
    "</OriginalOrderingProviderId>"),
   col + 1, v18, row + 1,
   v19 = build("<OriginalOrderingProviderName>",originalorderingprovidername,
    "</OriginalOrderingProviderName>"), col + 1, v19,
   row + 1, v20 = build("<LastUpdateProviderId>",cnvtint(o.last_update_provider_id),
    "</LastUpdateProviderId>"), col + 1,
   v20, row + 1, v21 = build("<LastUpdateProviderName>",lastupdateprsnlname,
    "</LastUpdateProviderName>"),
   col + 1, v21, row + 1,
   v22 = build("<","OriginalOrderCommunicationType",">"), col + 1, v22,
   row + 1, v23 = build("<Display>",communication_type_disp,"</Display>"), col + 1,
   v23, row + 1, v24 = build("<Meaning>",communication_type_meaning,"</Meaning>"),
   col + 1, v24, row + 1,
   v25 = build("<Value>",cnvtint(o.latest_communication_type_cd),"</Value>"), col + 1, v25,
   row + 1, v26 = build("</","OriginalOrderCommunicationType",">"), col + 1,
   v26, row + 1, v27 = build("<","OrderStatus",">"),
   col + 1, v27, row + 1,
   ...
   row + 1, v32 = build("<NurseReviewIndicator>",nursereviewindicator,"</NurseReviewIndicator>"), col
    + 1,
   v32, row + 1, v33 = build("<ProviderCosignFlag>",providercosignflag,"</ProviderCosignFlag>"),
   col + 1, v33, row + 1,
   v34 = build("<PharmacistReviewFlag>",pharmacyreviewflag,"</PharmacistReviewFlag>"), col + 1, v34,
   row + 1, v35 = build("<ClinicalDisplayLine>",clinicaldisplayline,"</ClinicalDisplayLine>"), col +
   1,
   v35, row + 1, v36 = build("<","CatalogType",">"),
   col + 1, v36, row + 1,
   v37 = build("<Display>",catalogtype_display,"</Display>"), col + 1, v37,
   row + 1, v38 = build("<Meaning>",catalogtype_meaning,"</Meaning>"), col + 1,
   v38, row + 1, v39 = build("<Value>",cnvtint(o.catalog_type_cd),"</Value>"),
   col + 1, v39, row + 1,
   v40 = build("</","CatalogType",">"), col + 1, v40,
   row + 1, v41 = build("<","ActivityType",">"), col + 1,
   v41, row + 1, v42 = build("<Display>",activitytype_display,"</Display>"),
   col + 1, v42, row + 1,
   v43 = build("<Meaning>",uar_get_code_meaning(o.activity_type_cd),"</Meaning>"), col + 1, v43,
   row + 1, v44 = build("<Value>",cnvtint(o.activity_type_cd),"</Value>"), col + 1,
   v44, row + 1, v45 = build("</","ActivityType",">"),
   col + 1, v45, row + 1,
   v46 = build("<","ClinicalCategory",">"), col + 1, v46,
   row + 1, v47 = build("<Display>",clinicalcategory,"</Display>"), col + 1,
   v47, row + 1, v48 = build("<Meaning>",uar_get_code_meaning(o.dcp_clin_cat_cd),"</Meaning>"),
   col + 1, v48, row + 1,
   v49 = build("<Value>",cnvtint(o.dcp_clin_cat_cd),"</Value>"), col + 1, v49,
   row + 1, v50 = build("</","ClinicalCategory",">"), col + 1,
   v50, row + 1, v51 = build("<","StopType",">"),
   col + 1, v51, row + 1,
   v52 = build("<Display>",stop_type_disp,"</Display>"), col + 1, v52,
   row + 1, v53 = build("<Meaning>",stoptype_meaning,"</Meaning>"), col + 1,
   v53, row + 1, v54 = build("<Value>",cnvtint(o.stop_type_cd),"</Value>"),
   col + 1, v54, row + 1,
   v55 = build("</","StopType",">"), col + 1, v55,
   row + 1, v56 = build("<OrderableTypeFlag>",orderable_type_flag,"</OrderableTypeFlag>"), col + 1,
   v56, row + 1, v57 = build("<ActionSequence>",cnvtint(oa.action_sequence),"</ActionSequence>"),
   col + 1, v57, row + 1,
   v58 = build("<UpdateDateTime>",updt_dt,"</UpdateDateTime>"), col + 1, v58,
   row + 1, v59 = build("<StatusDateTime>",status_dt,"</StatusDateTime>"), col + 1,
   v59, row + 1, v60 = build("<PrescriptionOrderId>",cnvtint(o.prescription_order_id),
    "</PrescriptionOrderId>"),
   col + 1, v60, row + 1,
   v61 = build("<","Accession",">"), col + 1, v61,
   row + 1, v62 = build("<AccessionId>",cnvtint(ac.accession_id),"</AccessionId>"), col + 1,
   v62, row + 1, v63 = build("<UnformattedNumber>",unformattednumber,"</UnformattedNumber>"),
   col + 1, v63, row + 1,
   v64 = build("<FormattedNumber>",formattednumber,"</FormattedNumber>"), col + 1, v64,
   row + 1, v65 = build("</","Accession",">"), col + 1,
   v65, row + 1, v66 = build("<","ActivitySubType",">"),
   col + 1, v66, row + 1,
   v67 = build("<Display>",activity_subtype,"</Display>"), col + 1, v67,
   row + 1, v68 = build("<Meaning>",uar_get_code_meaning(oc.activity_subtype_cd),"</Meaning>"), col
    + 1,
   v68, row + 1, v69 = build("<Value>",cnvtint(oc.activity_subtype_cd),"</Value>"),
   col + 1, v69, row + 1,
   v70 = build("</","ActivitySubType",">"), col + 1, v70,
   ...
   col + 1, v102, row + 1,
   v103 = build("<PathwayPlan>",trim(replace(replace(replace(replace(replace(p.display_description,
          "&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),
    "</PathwayPlan>"), col + 1, v103,
   row + 1, v104 = build("<ProposalStatus>",op_proposal_status_disp,"</ProposalStatus>"), col + 1,
   v104, row + 1, v105 = build("<DCDisplayDays>",cnvtint(oc.dc_display_days),"</DCDisplayDays>"),
   col + 1, v105, row + 1,
   v106 = build("<DefaultStopDuration>",cnvtint(oc.stop_duration),"</DefaultStopDuration>"), col + 1,
   v106,
   row + 1, v107 = build("<StopDurationUnitCd>",cnvtint(oc.stop_duration_unit_cd),
    "</StopDurationUnitCd>"), col + 1,
   v107, row + 1, v108 = build("<StopDurationUnit>",stop_duration_unit,"</StopDurationUnit>"),
   col + 1, v108, row + 1,
   v1081 = build("<CatStopTypeCd>",cnvtint(oc.stop_type_cd),"</CatStopTypeCd>"), col + 1, v1081,
   row + 1, v1082 = build("<CatStopTypeMeaning>",oc_stop_type_meaning,"</CatStopTypeMeaning>"), col
    + 1,
   v1082, row + 1, v1083 = build("<DisableOrderCommentFlag>",cnvtint(oc.disable_order_comment_ind),
    "</DisableOrderCommentFlag>"),
   col + 1, v1083, row + 1,
   v1084 = build("<ProtocolOrderId>",o_protocol_order_id,"</ProtocolOrderId>"), col + 1, v1084,
   row + 1, v1085 = build("<RefTextMask>",
    ...
    ENDIF
    ,"</RefTextMask>"), col + 1,
   v1085, row + 1, v109 = build("<RefTaskID>",cnvtint(t.reference_task_id),"</RefTaskID>"),
   col + 1, v109, row + 1,
   v110 = build("<DxString>",dx_string,"</DxString>"), col + 1, v110,
   row + 1
