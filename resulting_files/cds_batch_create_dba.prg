CREATE PROGRAM cds_batch_create:dba
 PROMPT
  ...
  "File Version" = "6.1          ",
  "Mars Report" = 0
  WITH outdev, nacs, batch_type,
  version, mars_report_id
 SET last_mod = "817537"
 IF (validate(parent_prog)=0)
  DECLARE parent_prog = vc WITH public, constant("CDS_BATCH_CREATE")
 ENDIF
...
#ukr_error_subroutines_exit
 SET last_mod = "153584"
 SUBROUTINE (getorgidbynacscode(nacs_code=vc) =vc)
   DECLARE nhs_org_alias_cd = f8 WITH protected, noconstant(uar_get_code_by("MEANING",334,
     "NHSORGALIAS"))
   DECLARE nhs_trust_cd = f8 WITH protected, noconstant(uar_get_code_by("MEANING",278,"NHSTRUST"))
   DECLARE strust = vc WITH protected, nonconstant
   SELECT INTO "nl:"
    org = build(otr.organization_id,"|",trim(o.org_name))
    FROM organization_alias oa,
     organization o,
     org_type_reltn otr
    PLAN (oa
     WHERE oa.alias=nacs_code
      AND ((oa.org_alias_type_cd+ 0)=nhs_org_alias_cd)
      AND oa.beg_effective_dt_tm <= cnvtdatetime(sysdate)
      AND oa.end_effective_dt_tm >= cnvtdatetime(sysdate)
      AND oa.active_ind=1)
     JOIN (otr
     WHERE otr.organization_id=oa.organization_id
   ...
   ;end select
   IF (curqual > 1)
    CALL adderrormsg(failure,"PROMPT",failure,"NACS CODE",
     "More than one oganisation matches the NACS code entered")
    GO TO exit_script
   ENDIF
   RETURN(strust)
 END ;Subroutine
 SET trust_name_id = getorgidbynacscode( $NACS)
 IF (parent_prog="CDS_BATCH_CREATE")
  IF (trim(value( $BATCH_TYPE),3)="")
   CALL validationfailuremsg(value( $OUTDEV),build2(curprog,
     " Error: User did not select a CDS Type from the prompt."),log_level_info)
  ENDIF
 ENDIF
 EXECUTE ukr_cds_create_extracts  $OUTDEV, "", "",
  $BATCH_TYPE, 0, trust_name_id,
  $VERSION, 0, "CDS_BATCH_CREATE",
  $MARS_REPORT_ID
#exit_script
 IF (parent_prog="CDS_BATCH_CREATE")
