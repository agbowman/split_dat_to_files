CREATE PROGRAM dm2_publish_dbstats_ind:dba
 ...
 SUBROUTINE dm2_adb_check(dac_db_link,dac_adb_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dpdi_inowner = vc WITH protect, noconstant(" ")
 DECLARE dpdi_inobject = vc WITH protect, noconstant(" ")
 DECLARE dpdi_instatid = vc WITH protect, noconstant(" ")
 DECLARE dpdi_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dpdi_step_beg_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dpdi_step_end_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dpdi_process_status = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dpdi_tabname = vc WITH protect, noconstant(" ")
 DECLARE dpdi_tabowner = vc WITH protect, noconstant(" ")
 DECLARE dpdi_cmd = vc WITH protect, noconstant(" ")
 DECLARE dpdi_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dpdi_emsg = vc WITH protect, noconstant(" ")
 DECLARE dpdi_retry_attempts = i2 WITH protect, noconstant(0)
 DECLARE dpdi_retry_ceiling = i2 WITH protect, noconstant(0)
 DECLARE dpdi_allow_publish = i2 WITH protect, noconstant(0)
 DECLARE dpdi_blocking_str = vc WITH protect, noconstant("")
 DECLARE dpdi_in_parse_ind = i2 WITH protect, noconstant(0)
 DECLARE dpdi_from_finish_ind = i4 WITH protect, noconstant(0)
 DECLARE dpdi_wait_pref_ind = i2 WITH protect, noconstant(0)
 DECLARE dpdi_adb_ind = i2 WITH protect, noconstant(0)
 DECLARE import_index_stats(ownname=vc,indname=vc,stattab=vc,statid=vc,statown=vc,
  force=i4) = null WITH sql = "SYS.DBMS_STATS.IMPORT_INDEX_STATS", parameter
 DECLARE import_index_stats_noinv(ownname=vc,indname=vc,stattab=vc,statid=vc,statown=vc,
  no_invalidate=i4,force=i4) = null WITH sql = "SYS.DBMS_STATS.IMPORT_INDEX_STATS", parameter
 DECLARE delete_table_stats(ownname=vc,tabname=vc,stattab=vc,statid=vc,statown=vc,
  cascade_indexes=i4) = null WITH sql = "SYS.DBMS_STATS.DELETE_TABLE_STATS", parameter
 FREE RECORD dpdi_procs
 RECORD dpdi_procs(
   1 cnt = i4
   1 qual[*]
     2 proc_name = vc
 ...
 IF (check_logfile("dm2_pdbs_tab",".log","dm2_publish_dbstats_ind LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_publish_dbstats_ind"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dpdi_inowner = trim(cnvtupper( $1),3)
 SET dpdi_inobject = trim( $2,3)
 SET dpdi_instatid = trim(cnvtupper( $3),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = "Parameter usage: dm2_publish_dbstats_ind  '<owner>','<index_name>','<STATID>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_add_detail_text("OWNER",dpdi_inowner)
 CALL dm2_process_log_add_detail_text("INDEX_NAME",dpdi_inobject)
 CALL dm2_process_log_add_detail_text("STATID",dpdi_instatid)
 ...
 SET dpdi_log_id = dm2_process_event_rs->dm_process_event_id
 IF (ddf_autosuccess_init(null)=0)
  GO TO exit_script
 ELSEIF ((dgdt_prefs->autosuccess_ind=1))
  GO TO exit_script
 ENDIF
 IF (locateval(dpdi_from_finish_ind,1,program_stack_rs->cnt,"DM2_FINISH_INDEX",program_stack_rs->
  qual[dpdi_from_finish_ind].name)=0)
  SET dm_err->eproc = concat("Checking DM_PROCESS_QUEUE non-SUCCESS status for index (",dpdi_inobject,
   ").")
  SELECT INTO "nl:"
   FROM dm_process_queue d
   WHERE d.process_type IN (dpq_statistics, dpq_freq_statistics)
    AND d.op_type="GATHER"
    AND d.object_type="INDEX"
    AND d.owner_name=dpdi_inowner
    AND d.object_name=dpdi_inobject
    AND d.process_status != "SUCCESS"
   DETAIL
    dpdi_process_status = d.process_status
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ELSEIF (curqual > 0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Publish request for TABLE object (",dpdi_inobject,") ","for owner (",
    dpdi_inowner,
    ") is denied."," INDEX was found in DM_PROCESS_QUEUE with status (",dpdi_process_status,").")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = concat("Checking DM_STAT_TABLE for statid existence for index (",dpdi_inobject,
  ").")
 SELECT INTO "nl:"
  FROM dm_stat_table d
  WHERE d.statid=dpdi_instatid
   AND d.type="I"
   AND d.c1=dpdi_inobject
   AND d.c5=dpdi_inowner
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSEIF (curqual=0)
  SET dm_err->emsg = concat("Publish request for INDEX object (",dpdi_inobject,") ","for owner (",
   dpdi_inowner,
   ") was bypassed due to statid (",dpdi_instatid,") related data not existing in DM_STAT_TABLE.")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Checking DBA_INDEXES for existence of table (",dpdi_inobject,").")
 SELECT INTO "nl:"
  FROM dba_indexes d
  WHERE d.owner=dpdi_inowner
   AND d.index_name=dpdi_inobject
  DETAIL
   dpdi_tabname = d.table_name, dpdi_tabowner = d.table_owner
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual=0)
  SET dm_err->emsg = concat("Publish request for INDEX object (",dpdi_inobject,") ","for owner (",
   dpdi_inowner,
   ") was bypassed due object ","not existing in DBA_INDEXES.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 CALL dm2_process_log_add_detail_text("INDEX_OWNER.INDEX_NAME",concat(dpdi_inowner,".",dpdi_inobject)
  )
 CALL dm2_process_log_add_detail_text("TABLE_OWNER.INDEX_NAME",concat(dpdi_tabowner,".",dpdi_inobject
   ))
 IF (dm2_process_log_dtl_row(dpdi_log_id,0)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Importing statistics to the LIVE Oracle dictionary for INDEX (",
  dpdi_inobject,") from DM_STAT_TABLE, STATID (",dpdi_instatid,").")
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (ddf_get_publish_retry(dpdi_retry_ceiling)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_adb_check("",dpdi_adb_ind)=0)
  GO TO exit_script
 ENDIF
 IF (ddf_check_wait_pref(dpdi_wait_pref_ind,dpdi_adb_ind)=0)
  GO TO exit_script
 ENDIF
 SET dpdi_retry_attempts = 0
 SET dpdi_allow_publish = 0
 WHILE (dpdi_allow_publish=0
  AND ((dpdi_retry_attempts < dpdi_retry_ceiling) OR ((dpdi_retry_ceiling=- (1)))) )
   SET dpdi_allow_publish = 1
   SET dpdi_blocking_str = ""
   IF (dpdi_wait_pref_ind=0)
    IF (ddf_check_in_parse(dpdi_tabowner,dpdi_tabname,dpdi_in_parse_ind,dpdi_blocking_str)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   IF (dpdi_in_parse_ind=1)
    SET dpdi_allow_publish = 0
   ENDIF
   IF ((dpdi_retry_ceiling=- (1))
    AND dpdi_allow_publish=0)
    SET dpdi_allow_publish = 1
    SET dm_err->eproc = "Parse detection overridden. Allowing Publish."
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF (dpdi_allow_publish=1)
    IF (validate(dfgd_freq_gather_ind,- (1))=1)
     CALL import_index_stats_noinv(dpdi_inowner,concat('"',dpdi_inobject,'"'),"DM_STAT_TABLE",
      dpdi_instatid,"V500",
      cnvtbool(false),cnvtbool(true))
    ELSE
     CALL import_index_stats(dpdi_inowner,concat('"',dpdi_inobject,'"'),"DM_STAT_TABLE",dpdi_instatid,
      "V500",
      cnvtbool(true))
    ENDIF
    ...
    ENDIF
   ENDIF
   IF (dpdi_allow_publish=0)
    SET dpdi_retry_attempts = (dpdi_retry_attempts+ 1)
    IF (dpdi_retry_attempts < dpdi_retry_ceiling)
     SET dm_err->eproc = concat(dpdi_blocking_str," Attempt ",trim(cnvtstring(dpdi_retry_attempts)),
      " of ",trim(cnvtstring(dpdi_retry_ceiling)),
      ".")
     CALL disp_msg(" ",dm_err->logfile,0)
     CALL dm2_process_log_add_detail_text("PARSE_EVENT:INDLOG",dm_err->eproc)
     IF (dm2_process_log_dtl_row(dpdi_log_id,0)=0)
      GO TO exit_script
     ENDIF
     CALL pause(10)
    ELSE
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat(dpdi_blocking_str," Attempt ",trim(cnvtstring(dpdi_retry_attempts)),
      " of ",trim(cnvtstring(dpdi_retry_ceiling)),
      ". Aborting Publish")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     CALL dm2_process_log_add_detail_text("PARSE_EVENT:INDFAILURE",dm_err->eproc)
     IF (dm2_process_log_dtl_row(dpdi_log_id,1)=0)
      GO TO exit_script
     ENDIF
     GO TO exit_script
    ENDIF
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Get DM2GDBS:PUBLISHADJUSTPROC-IND row from DM_INFO."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2GDBS:PUBLISHADJUSTPROC-IND"
  DETAIL
   dpdi_procs->cnt = (dpdi_procs->cnt+ 1), stat = alterlist(dpdi_procs->qual,dpdi_procs->cnt),
   dpdi_procs->qual[dpdi_procs->cnt].proc_name = trim(d.info_name),
   dpdi_procs->qual[dpdi_procs->cnt].proc_cmd = trim(d.info_char)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dpdi_procs)
 ENDIF
 IF ((dpdi_procs->cnt > 0))
  FOR (dgdi_proccnt = 1 TO dpdi_procs->cnt)
    SET dpdi_cmd = dpdi_procs->qual[dgdi_proccnt].proc_cmd
    SET dpdi_cmd = replace(dpdi_cmd,"<OBJECT_TYPE>","INDEX")
    SET dpdi_cmd = replace(dpdi_cmd,"<OBJECT_OWNER>",dpdi_inowner)
    SET dpdi_cmd = replace(dpdi_cmd,"<OBJECT_NAME>",dpdi_inobject)
    SET dpdi_cmd = replace(dpdi_cmd,"<STATID>",dpdi_instatid)
    SET dgdi_step_beg_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dpdi_cmd = concat("RDB ASIS(^ begin ",dpdi_cmd,"; end; ^) go")
    IF (dm2_push_cmd(dpdi_cmd,1)=0)
     CALL dm2_process_log_add_detail_text(concat(dpdt_procs->qual[dgdi_proccnt].proc_name,":ERROR"),
      dm_err->emsg)
     IF (dm2_process_log_dtl_row(dgdi_log_id,1)=0)
      GO TO exit_script
     ENDIF
     SET dm_err->err_ind = 0
     SET dm_err->emsg = " "
    ENDIF
    SET dgdi_step_end_dt_tm = cnvtdatetime(curdate,curtime3)
    CALL dm2_process_log_add_detail_number(concat(dpdi_procs->qual[dgdi_proccnt].proc_name,":SECS"),
     datetimediff(dgdi_step_end_dttm,dgdi_step_beg_dttm,5))
    CALL dm2_process_log_add_detail_text(concat(dpdi_procs->qual[dgdi_proccnt].proc_name,":CMD"),
     dpdi_cmd)
    IF (dm2_process_log_dtl_row(dgdi_log_id,0)=0)
     GO TO exit_script
    ENDIF
  ENDFOR
 ENDIF
 SET dm_err->eproc = concat("Deleting all statistics from DM_STAT_TABLE for STATID (",dpdi_instatid,
  ").")
 CALL disp_msg(" ",dm_err->logfile,0)
 CALL delete_table_stats(dpdi_tabowner,concat('"',dpdi_tabname,'"'),"DM_STAT_TABLE",dpdi_instatid,
  "V500",
  cnvtbool(true))
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_script
 ...
 SET dpdi_err_ind = dm_err->err_ind
 SET dpdi_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_execution,dpdi_log_id,1)
 SET dm_err->err_ind = dpdi_err_ind
 SET dm_err->emsg = dpdi_emsg
 SET dm_err->eproc = "Ending dm2_publish_dbstats_ind"
 CALL final_disp_msg("dm2_pdbs_tab")
END GO
