CREATE PROGRAM ce_event_action_modifier_upd:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 SET error_code = 0
 UPDATE  FROM ce_event_action_modifier t,
  ...
  PLAN (d)
   JOIN (t
   WHERE (t.ce_event_action_modifier_id=request->lst[d.seq].ce_event_action_modifier_id))
