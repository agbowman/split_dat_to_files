CREATE PROGRAM djh_l_nurse_stud_sns
 PROMPT
 ...
 ENDIF
 SELECT INTO  $OUTDEV
  p.active_ind, p.updt_id"#########", actcd = evaluate(p.active_status_cd,188.0," ",194.0,"S",
   "-"),
  p.username, newdt = format(p.updt_dt_tm,"yyyy-mm-dd"), p.name_full_formatted,
  p.position_cd"#########", p_position_disp = uar_get_code_display(p.position_cd), p
  .beg_effective_dt_tm,
  p.updt_dt_tm"@SHORTDATETIME", p.updt_id"#########", p.updt_task,
  p.create_prsnl_id"#########", newdays = format( $DYBK,"##"), expr1 = curprog,
