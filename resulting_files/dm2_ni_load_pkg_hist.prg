CREATE PROGRAM dm2_ni_load_pkg_hist
 ...
 SUBROUTINE dehr_load_adm_tables(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dnlph_build_col_list(null) = i2
 DECLARE dnlph_req_size = i4 WITH protect, noconstant(0)
 DECLARE dnlph_req = i4 WITH protect, noconstant(0)
 DECLARE dnlph_col = i4 WITH protect, noconstant(0)
 DECLARE dnlph_rows_returned = i4 WITH protect, noconstant(10000)
 IF (check_logfile("dm2_ni_load_pkg_hist",".log","dm2_ni_load_pkg_hist")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_ni_load_pkg_hist."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dm_err->debug_flag=722))
  CALL echorecord(requestin)
 ENDIF
 IF ((drr_env_hist_misc->qual[dnlphw_idx].load=0))
  IF (dehr_verify_cols_load(drr_env_hist_misc->qual[dnlphw_idx].table_name)=0)
   GO TO exit_script
  ENDIF
  SET dnlph_rows_returned = 10000
  SET dm_err->eproc = concat("Delete rows from ",drr_env_hist_misc->qual[dnlphw_idx].table_name,
   " for current environment_id")
  CALL disp_msg(" ",dm_err->logfile,0)
  WHILE (dnlph_rows_returned=10000)
    DELETE  FROM (parser(drr_env_hist_misc->qual[dnlphw_idx].table_name) d)
     WHERE parser(build("d.environment_id = ",dm2_create_dom->env_id))
    ...
    IF (check_error(dm_err->eproc)=1)
     ...
     GO TO exit_script
    ENDIF
    SET dnlph_rows_returned = curqual
    IF ((dm_err->debug_flag > 0))
     CALL echo(build("Rows deleted:",dnlph_rows_returned))
    ENDIF
  ENDWHILE
  SET drr_env_hist_misc->qual[dnlphw_idx].load = 1
 ENDIF
 SET dnlph_req_size = size(requestin->list_0,5)
 SET dm_err->eproc = "Replace '~' with ',' in character strings."
 CALL disp_msg("",dm_err->logfile,0)
 FOR (dnlph_req = 1 TO dnlph_req_size)
   FOR (dnlph_col = 1 TO dehr_column->cnt)
     IF ((dehr_column->qual[dnlph_col].data_type IN ("CHAR", "VARCHAR2", "VARCHAR")))
      IF (dm2_push_cmd(build("set requestin->list_0[dnlph_req].",dehr_column->qual[dnlph_col].
        col_name,"= replace(requestin->list_0[dnlph_req].",dehr_column->qual[dnlph_col].col_name,
        ", '~', ',', 0) go"),1)=0)
       GO TO exit_script
      ENDIF
     ENDIF
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Insert into ",drr_env_hist_misc->qual[dnlphw_idx].table_name)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 CALL dm2_push_cmd(concat("insert into ",drr_env_hist_misc->qual[dnlphw_idx].table_name,
   " dcv ,(dummyt d with seq = value(dnlph_req_size) )"),0)
 CALL dm2_push_cmd("set",0)
 IF (dnlph_build_col_list(null)=0)
  GO TO exit_script
 ENDIF
 CALL dm2_push_cmd("plan d ",0)
 IF (dm2_push_cmd(" join dcv with nocounter go",1)=0)
  GO TO exit_script
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 COMMIT
 GO TO exit_script
 SUBROUTINE dnlph_build_col_list(null)
   DECLARE dbcl_cnt = i4 WITH protect, noconstant(0)
   FOR (dbcl_cnt = 1 TO dehr_column->cnt)
    IF ((dehr_column->qual[dbcl_cnt].data_type IN ("CHAR", "VARCHAR2", "VARCHAR")))
     IF (dm2_push_cmd(concat("dcv.",dehr_column->qual[dbcl_cnt].col_name," = ",
       "evaluate(requestin->list_0[d.seq].",dehr_column->qual[dbcl_cnt].col_name,
       ",'::DM2NULLVALUE::',","null, requestin->list_0[d.seq].",dehr_column->qual[dbcl_cnt].col_name,
       ")"),0)=0)
      RETURN(0)
     ENDIF
    ELSEIF ((dehr_column->qual[dbcl_cnt].data_type="DATE"))
     IF (dm2_push_cmd(concat("dcv.",dehr_column->qual[dbcl_cnt].col_name," = ",
       "evaluate(requestin->list_0[d.seq].",dehr_column->qual[dbcl_cnt].col_name,
       ",'::DM2NULLVALUE::',null, cnvtdatetime(requestin->list_0[d.seq].",dehr_column->qual[dbcl_cnt]
       .col_name,"))"),0)=0)
      RETURN(0)
     ENDIF
    ELSEIF ((dehr_column->qual[dbcl_cnt].data_type="NUMBER"))
     IF ((dehr_column->qual[dbcl_cnt].col_name="ENVIRONMENT_ID"))
      IF (dm2_push_cmd(build("dcv.",dehr_column->qual[dbcl_cnt].col_name," = ",dm2_create_dom->env_id
        ),0)=0)
       RETURN(0)
      ENDIF
     ELSEIF ((dehr_column->qual[dbcl_cnt].col_name="DM_RDDS_RIE_ID"))
      IF (dm2_push_cmd(build("dcv.",dehr_column->qual[dbcl_cnt].col_name," = ","seq(dm_seq,nextval)"),
       0)=0)
       RETURN(0)
      ENDIF
     ELSE
      IF (dm2_push_cmd(build("dcv.",dehr_column->qual[dbcl_cnt].col_name," = ",
        "evaluate(requestin->list_0[d.seq].",dehr_column->qual[dbcl_cnt].col_name,
        ",'::DM2NULLVALUE::',null, ","cnvtint(requestin->list_0[d.seq].",dehr_column->qual[dbcl_cnt].
        col_name,"))"),0)=0)
       RETURN(0)
      ENDIF
     ENDIF
    ELSEIF ((dehr_column->qual[dbcl_cnt].data_type="FLOAT"))
     IF (dm2_push_cmd(concat("dcv.",dehr_column->qual[dbcl_cnt].col_name," = ",
       "evaluate(requestin->list_0[d.seq].",dehr_column->qual[dbcl_cnt].col_name,
       ",'::DM2NULLVALUE::',null, cnvtreal(requestin->list_0[d.seq].",dehr_column->qual[dbcl_cnt].
       col_name,"))"),0)=0)
      RETURN(0)
     ENDIF
    ELSE
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Column Name:",dehr_column->qual[dbcl_cnt].col_name,". Data_Type:",
      dehr_column->qual[dbcl_cnt].col_type," is not recognizable by load script.")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    IF ((dbcl_cnt != dehr_column->cnt))
     CALL dm2_push_cmd(",",0)
    ENDIF
   ENDFOR
   RETURN(1)
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_ni_load_pkg_hist completed"
 ENDIF
 CALL final_disp_msg("dm2_ni_load_pkg_hist")
END GO
