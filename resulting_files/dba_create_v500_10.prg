CREATE PROGRAM dba_create_v500_10
 SET message = window
 ...
 SUBROUTINE aix_gen_db_creation_script(x)
   ...
   SELECT INTO concat("script_dir","/create_db.sql")
    a.file_name, a.file_size, a.disk_name
    FROM dm_env_files a
     ...
     DETAIL
      ...
      CASE (a.file_type)
       OF "UNDO":
        ...
        CALL print(cnvtlower(trim(a.tablespace_name)))row + 1,
        IF (osver="HPX")
         col 3, "datafile '",
         CALL print(trim(a.disk_name)),
         "/r",
       ...
       OF "TEMP":
        ...
        IF (osver="HPX")
         ...
         CALL print(cnvtstring(a.file_size)), " reuse"
        ELSE
         col 3, "tempfile '/dev/r",
         CALL print(trim(a.file_name)),
         "' size ",
         CALL print(cnvtstring(a.file_size)), " reuse"
        ENDIF
        ,row + 1,col 3,"extent management local uniform size 5M"
      ENDCASE
     WITH nocounter, format = stream, noheading,
      append, formfeed = none, maxrow = 1
    ;end select
   ...
   IF ((dm_env_import_request->base_oracle_version_int >= 10))
    SELECT INTO concat("script_dir","/create_db.sql")
     ...
     FROM dm_env_files a
     WHERE a.environment_id=env_id
      AND a.file_type="SYSAUX"
     DETAIL
      IF (osver="HPX")
       col 1, "sysaux datafile '",
       CALL print(trim(a.disk_name)),
       "/r",
       CALL print(trim(a.file_name)), "' size ",
       CALL print(cnvtstring(a.file_size)), " reuse"
      ELSE
       col 1,
       CALL print(concat("sysaux datafile '/dev/r",trim(a.file_name),"' size ",cnvtstring(a.file_size
         )," reuse"))
      ENDIF
     WITH nocounter, format = stream, noheading,
      append, formfeed = none, maxrow = 1
