CREATE PROGRAM dm_mdi_appl_ack_rows:dba
 ...
 SET reply->table_name = "MDI_APPL_ACK"
 ...
 IF (days_to_keep < 5)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"DAYSTOKEEP",
   "You must keep at least 5 days of data.  You entered %1 or did not enter any value.","i",
   days_to_keep)
  SET reply->status_data.status = "F"
  GO TO the_end
 ENDIF
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    a.rowid
    FROM mdi_appl_ack a
    WHERE a.order_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists(
      "a.create_dt_tm < cnvtdatetime((curdate - days_to_keep), curtime3)","a"))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,10)=1)
