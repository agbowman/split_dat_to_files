CREATE PROGRAM djh_l_er_rn_ta_w_orgs
 PROMPT
 ...
 SELECT INTO  $OUTDEV
  ...
  pr.position_cd, pr_position_disp = uar_get_code_display(pr.position_cd)
  FROM prsnl pr
  WHERE pr.active_ind=1
   AND ((pr.position_cd=36409588) OR (((pr.position_cd=36572363) OR (((pr.position_cd=1465246) OR (pr
  .position_cd=1465245)) )) ))
  ORDER BY o.org_name, pr.position_cd, pr.name_full_formatted,
   p.person_id
  HEAD REPORT
   y_pos = 18,
  ...
  FOOT  p.person_id
   IF (((y_pos+ 77) >= 792))
    y_pos = 0, BREAK
   ENDIF
   name_full_formatted1 = substring(1,30,pr.name_full_formatted), username1 = substring(1,9,pr
    .username), row + 1,
   "{F/0}{CPI/14}",
