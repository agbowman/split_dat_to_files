CREATE PROGRAM dm_pucb_workflow_task_queue:dba
 ...
 DECLARE ispersonuncombinequeueable = i2 WITH protect, noconstant(true)
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PUCB_WORKFLOW_TASK_QUEUE"
  ...
  GO TO exit_sub
 ENDIF
 CALL processpersonuncombtowtp(0)
 SUBROUTINE (processpersonuncombtowtp(dummy=i2) =null)
   IF ((request->parent_table="PERSON")
    AND (request->xxx_uncombine[ucb_cnt].encntr_id=0.0))
    SET wtptaskrequest->smode = "PERSON_UNCOMBINE"
   ...
   IF ((wtptaskrequest->cmb_qual[1].to_xxx_id != 0))
    SELECT INTO "nl:"
     ...
     PLAN (pcd
      WHERE (pcd.entity_id=wtptaskrequest->cmb_qual[1].to_xxx_id)
       AND pcd.entity_name="WORKFLOW_TASK_QUEUE"
       AND pcd.active_ind=true)
    ;end select
    IF (curqual=0)
     SET ispersonuncombinequeueable = false
    ENDIF
   ENDIF
   IF (usewtpframework)
    IF (ispersonuncombinequeueable)
