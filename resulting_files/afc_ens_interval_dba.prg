CREATE PROGRAM afc_ens_interval:dba
 SET false = 0
 SET true = 1
 SET gen_nbr_error = 3
 SET insert_error = 4
 ...
 SET hafc_ens_interval = 0
 SET istatus = 0
 SET serrmsg = fillstring(132," ")
 SET reply->status_data.status = "F"
 SET table_name = "INTERVAL_TABLE"
 IF ((request->interval_table_qual > 0))
  SET reply->interval_table_qual = request->interval_table_qual
  SET stat = alterlist(reply->interval_table,request->interval_table_qual)
  FOR (inx0 = 1 TO request->interval_table_qual)
    CASE (request->interval_table[inx0].action_type)
     OF "ADD":
      CALL echo("Add interval")
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_add_interval
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "UPT":
      CALL echo("Upt interval")
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_upt_interval
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "DEL":
      CALL echo("Del interval")
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_del_interval
      IF (failed != false)
       GO TO check_error
      ENDIF
     ELSE
