CREATE PROGRAM dm2_grant_privileges:dba
 DECLARE dm2_mod = vc WITH private, constant("004")
 DECLARE gp_logfile_prefix = vc WITH protect, constant("dm2_grant_privileges")
 DECLARE gp_tname = vc WITH protect, noconstant(" ")
 ...
 SET gp_tlist_cnt = 0
 DECLARE gp_cnt = i4 WITH protect, noconstant(0)
 IF (check_logfile(gp_logfile_prefix,".log","DM2_GRANT_PRIVILEGES LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning Grant Privileges Process"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (currdb IN ("ORACLE", "SQLSRV"))
  ...
  GO TO exit_program
 ENDIF
 SET gp_tname = build(cnvtupper(trim( $1,3)))
 IF (gp_tname=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Input table name to execute grant privileges is invalid."
  SET dm_err->eproc = "Input Table Name Validation"
  SET dm_err->user_action = "Please enter a table name to execute grant privileges."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Loading table(s) that meet input qualification into memory"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  ut.table_name
  FROM dm2_user_tables ut
  WHERE ut.table_name=patstring(gp_tname)
  DETAIL
   gp_tlist->gp_tlist_cnt = (gp_tlist->gp_tlist_cnt+ 1), stat = alterlist(gp_tlist->qual,gp_tlist->
    gp_tlist_cnt), gp_tlist->qual[gp_tlist->gp_tlist_cnt].gp_tname = ut.table_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((gp_tlist->gp_tlist_cnt > 0))
  SET dm_err->eproc = "Granting SELECT privileges to user V500READ for qualified table(s)"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  FOR (gp_cnt = 1 TO gp_tlist->gp_tlist_cnt)
   EXECUTE dm2_grant_privileges_c
   IF ((dm_err->err_ind=1))
    GO TO exit_program
   ENDIF
  ENDFOR
  SET dm_err->eproc = concat("Total tables qualified for SELECT privileges: ",trim(cnvtstring(
     gp_tlist->gp_tlist_cnt,4,0)))
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
 ELSE
  SET dm_err->eproc = concat("No tables qualified to grant privileges for input table name: ",
   gp_tname)
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
...
#exit_program
 FREE RECORD gp_tlist
 SET dm_err->eproc = "Ending Grant Privileges Process.  Process Completed Successfully."
 CALL final_disp_msg(gp_logfile_prefix)
END GO
