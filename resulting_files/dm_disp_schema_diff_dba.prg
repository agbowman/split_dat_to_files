CREATE PROGRAM dm_disp_schema_diff:dba
 ...
 IF ((tgtdb->tbl_cnt=0))
  ...
  GO TO end_program
 ENDIF
 SET diff_file = fillstring(30," ")
 IF ((fs_proc->ocd_number > 0))
  SET diff_file = build("dm_ocd_diff_",fs_proc->ocd_number,".txt")
 ELSE
  SET diff_file = build(cnvtlower(fs_proc->file_prefix),"_diff.txt")
 ENDIF
 SET diff_count = 0
 SELECT INTO value(diff_file)
  FROM dual
  DETAIL
   ...
   FOR (t_tbl = 1 TO tgtdb->tbl_cnt)
     IF ((tgtdb->tbl[t_tbl].new_ind=1))
      ...
      FOR (t_col = 1 TO tgtdb->tbl[t_tbl].tbl_col_cnt)
        ...
        data_length"#####",
        col 56, tgtdb->tbl[t_tbl].tbl_col[t_col].nullable, col 63,
        tgtdb->tbl[t_tbl].tbl_col[t_col].data_default, row + 1
      ENDFOR
       ...
       FOR (t_col = 1 TO tgtdb->tbl[t_tbl].tbl_col_cnt)
         ...
         diff_nullable_ind > 0)) OR ((tgtdb->tbl[t_tbl].tbl_col[t_col].diff_default_ind > 0))) )) ))
          ...
          IF (c_col > 0)
           ...
           data_length"#####",
           col 64, curdb->tbl[c_tbl].tbl_col[c_col].nullable, col 71,
           curdb->tbl[c_tbl].tbl_col[c_col].data_default, row + 1, col 0,
           "Target", col 10, tgtdb->tbl[t_tbl].tbl_col[t_col].col_name,
           ...
           t_col].nullable,
           col 71, tgtdb->tbl[t_tbl].tbl_col[t_col].data_default, row + 1
           ...
           IF ((tgtdb->tbl[t_tbl].tbl_col[t_col].downtime_ind=1))
            col 10, "NOTE: This schema change requires downtime.", row + 1
           ENDIF
           row + 1
          ENDIF
         ENDIF
       ...
       FOR (t_ind = 1 TO tgtdb->tbl[t_tbl].ind_cnt)
         IF ((tgtdb->tbl[t_tbl].ind[t_ind].new_ind=1))
          ...
          IF ((tgtdb->tbl[t_tbl].ind[t_ind].downtime_ind=1))
           "NOTE: This schema change requires downtime.", row + 1
          ENDIF
          row + 1
         ELSE
       ...
       FOR (t_cons = 1 TO tgtdb->tbl[t_tbl].cons_cnt)
         ...
         IF ((tgtdb->tbl[t_tbl].cons[t_cons].new_ind=1))
          ...
          IF ((tgtdb->tbl[t_tbl].cons[t_cons].downtime_ind=1))
           "NOTE: This schema change will be made in downtime.", row + 1
          ENDIF
          row + 1
         ELSE
 ...
 FREE DEFINE rtl
 FREE SET file_loc
 SET logical file_loc value(diff_file)
 DEFINE rtl "file_loc"
 SELECT
  r.line
  FROM rtlt r
