CREATE PROGRAM cv_da_upt_cv_proc:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_upt_cv_proc_vrsn = vc WITH private, constant("20091117")
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     .activity_subtype_cd)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].ed_review_ind,c.ed_review_ind) != - (1))
      AND validate(request->objarray[i].ed_review_ind,c.ed_review_ind) != c.ed_review_ind)
      alteredind = 1
     ENDIF
 ...
 END ;Subroutine
#end_program
 CALL cv_log_msg_post("003 09/03/18 VJ043510")
END GO
