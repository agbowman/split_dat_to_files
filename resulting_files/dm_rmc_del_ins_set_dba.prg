CREATE PROGRAM dm_rmc_del_ins_set:dba
 ...
 ENDIF
 IF (check_logfile("dm_rmc_del_ins",".log","dm_rmc_del_ins_set LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_del
 ENDIF
 DECLARE drdis_meaning = vc WITH protect, noconstant("")
 DECLARE drdis_ltr_id = f8 WITH protect, noconstant(0.0)
 IF (reflect(parameter(1,0)) != "C*")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = 'Expected syntax: dm_rmc_del_ins_set "<INS_MEANING>"'
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_del
 ENDIF
 SET dm_err->eproc = "Starting DM_RMC_DEL_INS_SET..."
 SET drdis_meaning =  $1
 SELECT INTO "NL:"
  FROM dm_refchg_instruction d
  WHERE d.ins_meaning=drdis_meaning
  DETAIL
   drdis_ltr_id = d.instruction_text_id
  WITH nocounter
 ;end select
 ...
 IF (curqual=0)
  GO TO exit_del
 ENDIF
 DELETE  FROM dm_refchg_instruction d
  WHERE d.ins_meaning=drdis_meaning
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc) != 0)
 ...
 DELETE  FROM long_text_reference
  WHERE long_text_id=drdis_ltr_id
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SET auto_ver_request->qual[1].rdds_event = "Instruction Set Deleted"
 SET auto_ver_request->qual[1].paired_environment_id = 0.0
 SET auto_ver_request->qual[1].event_reason = drdis_meaning
 SELECT INTO "NL:"
  FROM dm_info d
