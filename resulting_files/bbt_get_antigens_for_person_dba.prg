 ...
 SELECT INTO "nl:"
  ...
  PLAN (pa
   WHERE (pa.person_id=request->person_id)
    AND pa.active_ind=1)
   JOIN (r
   WHERE r.result_id=pa.result_id)
   JOIN (o
   WHERE o.order_id=r.order_id)
  ORDER BY pa.antigen_cd, o.current_start_dt_tm DESC
  HEAD REPORT
   err_cnt = 0, lreplycnt = 0, stat = alterlist(reply->qual,lreplycnt)
  HEAD pa.antigen_cd
   lreplycnt += 1, stat = alterlist(reply->qual,lreplycnt), reply->qual[lreplycnt].antigen_cd = pa
   .antigen_cd,
   reply->qual[lreplycnt].person_antigen_id = pa.person_antigen_id, reply->qual[lreplycnt].order_id
    = o.order_id, reply->qual[lreplycnt].anchor_dt_tm = o.current_start_dt_tm
   ...
   FOR (lidx = 1 TO lreplycnt)
     ...
     SELECT INTO "nl:"
      ...
      FROM person_combine_det pcd,
       person_combine pc
      PLAN (pcd
       WHERE (reply->qual[lidx].order_id > 0)
     ...
     SELECT INTO "nl:"
      ...
      PLAN (pnh
       WHERE pnh.person_id=dcurrentpersonid
        ...
        AND pnh.transaction_dt_tm <= cnvtdatetime(datetimeadd(reply->qual[lidx].anchor_dt_tm,dminute)
        )
        AND  NOT ( EXISTS (
       (SELECT
        pcd.entity_id
