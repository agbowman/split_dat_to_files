CREATE PROGRAM bhs_rpt_wing_missing_note:dba
 PROMPT
 ...
 DECLARE mf_pending_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",14233,"PENDING"))
 DECLARE mf_noshow_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",14233,"NOSHOW"))
 DECLARE mf_rescheduled_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",14233,"RESCHEDULED")
  )
 DECLARE mf_confirmed_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",14233,"CONFIRMED"))
 ...
 IF (substring(1,1,ms_data_type)="L")
  FOR (ml_cnt = 1 TO cnvtint(substring(2,(size(ms_data_type) - 1),ms_data_type)))
   ...
   IF (ml_cnt=1)
    SET ms_fac_p = concat(" cv.code_value in (",trim(ms_tmp_str))
   ELSE
 ...
 ELSE
  ...
  SET ms_fac_p = concat(" cv.code_value = ",trim(ms_fac_p))
 ENDIF
 SELECT INTO "nl:"
  FROM code_value cv
  WHERE cv.code_set=220
   ...
   AND parser(ms_fac_p)
   AND  NOT (cv.display IN ("NEURO PSYCH"))
 ...
 IF (substring(1,1,ms_data_type)="L")
  FOR (ml_cnt = 1 TO cnvtint(substring(2,(size(ms_data_type) - 1),ms_data_type)))
   ...
   IF (ml_cnt=1)
    SET ms_enc_type_p = concat(" cv.code_value in (",trim(ms_tmp_str))
   ELSE
 ...
 ELSE
  ...
  SET ms_enc_type_p = concat(" cv.code_value = ",trim(ms_enc_type_p))
 ENDIF
 SELECT INTO "nl:"
  FROM code_value cv
  WHERE cv.code_set=71
   ...
   AND cv.display_key IN ("ACTIVECMTYOFFICEVISIT", "DISCHRECUROFFICEVISIT", "DISCHARGEDOUTPATIENT",
  "OFFICEVISIT", "ONETIMEOP",
  "OUTPATIENT", "OUTPATIENTONETIME", "OUTPATIENTRECURRING", "PRECMTYOFFICEVISIT", "PREOFFICEVISIT",
  "PRERECUROFFICEVISIT", "PREOUTPATIENTONETIME", "PREOUTPT", "REACTIVATE", "RECUROFFICEVISIT",
  "TRIAGE")
  HEAD REPORT
   etype->l_cnt = 0
  DETAIL
   etype->l_cnt += 1, stat = alterlist(etype->qual,etype->l_cnt), etype->qual[etype->l_cnt].
   f_enc_type = cv.code_value,
   etype->qual[etype->l_cnt].s_enc_type_disp = cv.display
  WITH nocounter
 ;end select
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (pr
   ...
   WHERE sa.person_id=pr.person_id
    ...
    AND  NOT (sa.sch_state_cd IN (mf_canceled_cd, mf_pending_cd, mf_noshow_cd, mf_rescheduled_cd,
   mf_confirmed_cd))
   ...
   WHERE sep.encntr_id=e.encntr_id
    ...
    AND expand(ml_exp3,1,etype->l_cnt,e.encntr_type_cd,etype->qual[ml_exp3].f_enc_type))
   JOIN (se
  ...
  HEAD e.encntr_id
   ...
   ms_appt_stat = uar_get_code_display(sa.sch_state_cd),
   m_rec->enc[ml_cnt].s_appt_type = trim(uar_get_code_display(se.appt_type_cd),3), m_rec->enc[ml_cnt]
   .mf_enc_cd = e.encntr_id, m_rec->enc[ml_cnt].ml_ind = 0,
   ml_cnt2 = 0
  HEAD pr.person_id
   ml_cnt2 += 1,
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (ce
   ...
   WHERE pr.active_ind=1
    AND pr.active_status_cd=mf_active_cd
    AND pr.person_id=cep.action_prsnl_id
    AND expand(ml_exp,1,size(m_rec->pos,5),pr.position_cd,m_rec->pos[ml_exp].f_cd))
 ...
 SELECT INTO "nl:"
  ...
  HEAD e.encntr_id
   ...
   ENDIF
   m_rec->pat[ml_cnt].s_encntr_id = trim(cnvtstring(e.encntr_id),3), m_rec->pat[ml_cnt].s_appt_stat
    = trim(m_rec->enc[d1.seq].ms_appt_stat), m_rec->pat[ml_cnt].s_appt_type = trim(m_rec->enc[d1.seq]
    .s_appt_type,3),
   m_rec->pat[ml_cnt].s_enc_beg_dt = m_rec->enc[d1.seq].ms_beg_dt_tm, m_rec->pat[ml_cnt].s_enc_end_dt
    = m_rec->enc[d1.seq].ms_end_dt_tm, m_rec->pat[ml_cnt].s_encntr_type = trim(uar_get_code_display(e
     .encntr_type_cd),3),
   m_rec->pat[ml_cnt].s_fac = trim(uar_get_code_display(e.loc_facility_cd),3), m_rec->pat[ml_cnt].
   s_pat_name = trim(p.name_full_formatted,3), m_rec->pat[ml_cnt].s_mrn = trim(ea2.alias,3),
   m_rec->pat[ml_cnt].s_fin = trim(ea1.alias,3), ml_cnt2 = 0
  HEAD pr.person_id
   ml_cnt2 += 1,
 ...
 SELECT INTO value( $OUTDEV)
  ...
  mrn = trim(substring(1,200,m_rec->pat[d1.seq].s_mrn)), fin = trim(substring(1,200,m_rec->pat[d1.seq
    ...
    ms_provider_name)),
  appt_type = substring(1,100,m_rec->pat[d1.seq].s_appt_type)
 ...
 IF (size(trim(ms_log),3) > 0)
  SELECT INTO value( $OUTDEV)
   ...
   HEAD REPORT
    ...
    row + 1,
    CALL print(build2("mf_FACILITY_CD: ",ms_fac_p))
   WITH nocounter
  ;end select
 ENDIF
 FREE RECORD m_rec
