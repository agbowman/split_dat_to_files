CREATE PROGRAM dcpreqgen07:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 ENDFOR
 FOR (x = 1 TO order_cnt)
   FOR (z = 1 TO orders->qual[x].d_cnt)
     IF (size(orders->qual[x].display_line,1) > 0)
      SET pt->line_cnt = 0
      SET max_length = 65
      IF ((orders->qual[x].d_qual[z].special_ind=1))
       EXECUTE dcp_parse_text value(orders->qual[x].d_qual[z].value), value(max_length)
      ...
      IF (first_time="Y")
       IF (size(oi.ordered_as_mnemonic,1) > 0)
        ...
        IF (mnem_length >= mnemonic_size
         ...
         mnemonic_line = concat(trim(oi.ordered_as_mnemonic),"..., ",trim(oi
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   HEAD PAGE
    ...
    xcol = 300,
    CALL print(calcpos(xcol,ycol)), "{b}Fin # ",
   ...
   DETAIL
    FOR (vv = 1 TO value(ord_cnt))
      IF ((orders->qual[vv].display_ind=1))
       ...
       CALL print(calcpos(xcol,ycol)), "{cpi/14}Ordering MD: ", orders->qual[vv].order_dr,
       row + 1, xcol = 40, ycol = (ycol+ 11),
       CALL print(calcpos(xcol,ycol)), "Ordering Type: ", orders->qual[vv].type,
       row + 1, xcol = 40, ycol = (ycol+ 11),
       CALL print(calcpos(xcol,ycol)), "Order Action: ", orders->qual[vv].action,
       row + 1, xcol = 30, ycol = (ycol+ 15),
       CALL print(calcpos(xcol,ycol)), "{cpi/12}{b}Procedure: ", orders->qual[vv].mnemonic,
       "{endb}", row + 1, savev = vv,
       ...
       FOR (x = 1 TO orders->qual[vv].d_cnt)
         IF ((orders->qual[vv].d_qual[x].print_ind=0))
          ...
          ENDIF
          CALL print(calcpos(xcol,ycol))
          IF ((orders->qual[vv].d_qual[x].special_ind=0))
           IF (((size(orders->qual[vv].d_qual[x].label_text)+ size(orders->qual[vv].d_qual[x].value))
            > 57))
            CALL print(substring(1,57,concat(orders->qual[vv].d_qual[x].label_text,": ",orders->qual[
              vv].d_qual[x].value))), "..."
           ELSE
            orders->qual[vv].d_qual[x].label_text, ": ", orders->qual[vv].d_qual[x].value
           ENDIF
          ELSEIF ((orders->qual[vv].d_qual[x].special_ind=1))
           ...
           FOR (zz = 1 TO orders->qual[vv].disp_ln_cnt)
             ...
             ycol = (ycol+ 12)
             IF (ycol > 630
              AND (zz < orders->qual[vv].disp_ln_cnt))
              CALL print(calcpos(130,ycol)), "**Continued on next page**", saved_pos = vv,
              BREAK, xcol = 30, ycol = 100,
              CALL print(calcpos(xcol,ycol)),
   ...
   FOOT PAGE
    ...
    "  ", curtime, "{endb}",
    row + 1, "{cpi/12}{pos/300/687}", "Name: ",
    "{pos/345/687}", orders->name, "{pos/465/687}",
    "Room/Bed: ", orders->room, "-",
    orders->bed, row + 1, "{pos/300/698}",
    "Acct #: ", orders->fnbr, "{pos/465/698}",
    "ADM: ", orders->admit_dt, row + 1,
    "{pos/300/709}", "Med Rec # ", orders->mrn,
    "{pos/440/709}", "Age: ", orders->age,
    "{pos/510/709}", "Sex: ", orders->sex,
    row + 1, "{pos/300/720}", "MD: ",
    orders->attending, row + 1, "{pos/490/731}",
    "Page # ", curpage, row + 1,
    "{cpi/10}{pos/30/693}{b}", orders->facility, "{endb}"
