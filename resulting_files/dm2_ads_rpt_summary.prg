CREATE PROGRAM dm2_ads_rpt_summary
 ...
 END ;Subroutine
 DECLARE dars_logfile_prefix = vc WITH protect, constant("dm2_ads_rsum")
 DECLARE dars_cnt = i4 WITH protect, noconstant(0)
 DECLARE dars_rpt_file = vc WITH protect, noconstant("")
 SET width = 132
 IF (check_logfile(dars_logfile_prefix,".log","dm2_ads_rpt_summary")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_ads_rpt_summary"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF (dar_get_config_list(0)=0)
  GO TO exit_program
 ENDIF
 FOR (dars_cnt = 1 TO dads_list->config_cnt)
   IF (dar_sync_config_status(dads_list->config_qual[dars_cnt].config_id)=0)
    GO TO exit_program
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Generating ADS Summary Report"
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 IF (get_unique_file("dm2_ads_rsum",".dat")=0)
  GO TO exit_program
 ELSE
  SET dars_rpt_file = dm_err->unique_fname
 ENDIF
 SELECT INTO value(dars_rpt_file)
  FROM dummyt d
  HEAD REPORT
   CALL print(fillstring(250,"-")), row + 1, col 0,
   CALL print("ACTIVITY DATA SAMPLE MENU [SUMMARY REPORT]"), col 80,
   CALL print(build("REPORT DATE/TIME:",format(cnvtdatetime(curdate,curtime3),
     "DD-MMM-YYYY HH:MM:SS;;D"))),
   row + 1,
   CALL print(build("File Name: CCLUSERDIR:",dars_rpt_file)), row + 1,
   CALL print(fillstring(250,"-")), row + 2
  DETAIL
   col 0, "SAMPLE NAME ", col 40,
   "| SAMPLE STATUS ", col 60, "| SAMPLE METHOD ",
   col 80, "| PERCENT ", col 90,
   "| LAST MODIFIED DT_TM ", col 120, "| LAST SAMPLE MESSAGE ",
   row + 1,
   CALL print(fillstring(250,"-")), row + 2,
   dars_cnt = 0
   FOR (dars_cnt = 1 TO dads_list->config_cnt)
     col 0,
     CALL print(build(dads_list->config_qual[dars_cnt].config_name)), col 40,
     "| ",
     CALL print(build(dads_list->config_qual[dars_cnt].config_status)), col 60,
     "| ",
     CALL print(build(dads_list->config_qual[dars_cnt].config_method)), col 80,
     "| ",
     CALL print(build(evaluate(dads_list->config_qual[dars_cnt].config_method,dpl_custom,"N/A",build(
        dads_list->config_qual[dars_cnt].config_pct)))), col 90,
     "| ",
     CALL print(format(cnvtdatetime(dads_list->config_qual[dars_cnt].config_updt_dt_tm),";;Q")), col
     120,
     "| ",
     CALL print(build(substring(1,130,dads_list->config_qual[dars_cnt].last_msg))), row + 1
   ENDFOR
 ...
 IF (check_error(dm_err->eproc))
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Displaying ADS Summary Report"
 IF (dm2_disp_file(dars_rpt_file,"ADS Summary Report")=0)
  GO TO exit_program
 ENDIF
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Ending dm2_ads_rpt_summary"
 CALL final_disp_msg(dars_logfile_prefix)
END GO
