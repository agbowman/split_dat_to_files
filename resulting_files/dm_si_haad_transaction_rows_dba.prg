CREATE PROGRAM dm_si_haad_transaction_rows:dba
 ...
 SET reply->table_name = "SI_HAAD_TRANSACTION"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(ht.si_haad_transaction_id)
   FROM si_haad_transaction ht
   WHERE ht.si_haad_transaction_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(ht.si_haad_transaction_id)
  FROM si_haad_transaction ht
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT DISTINCT INTO "nl:"
    ht.rowid
    FROM si_haad_transaction ht
    WHERE ht.si_haad_transaction_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND ht.completed_dt_tm != null
     AND ((ht.si_haad_file_download_id=0) OR ( NOT (ht.si_haad_file_download_id IN (
    (SELECT
     si_haad_file_download_id
    ...
    DETAIL
     ...
     ENDIF
     reply->rows[v_rows].row_id = ht.rowid
    WITH nocounter, maxqual(ht,value(v_rows_left))
   ;end select
