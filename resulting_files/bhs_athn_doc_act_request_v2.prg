CREATE PROGRAM bhs_athn_doc_act_request_v2
 RECORD orequest(
   1 req[*]
     2 ensure_type = i2
     2 version_dt_tm = dq8
 ...
 DECLARE comment_text = vc WITH noconstant(trim( $8,3))
 ...
 ENDIF
 DECLARE message_text = vc WITH noconstant(trim( $10,3))
 IF (textlen(message_text) > 0)
  SET srequest->param = message_text
  EXECUTE bhs_athn_format_str_param  WITH replace("REQUEST","SREQUEST"), replace("REPLY","SREPLY")
  SET message_text = sreply->param
 ENDIF
 ...
 FOR (i = 1 TO t_record->prsnl_cnt)
   ...
   SET orequest->req[i].event_prsnl.request_dt_tm = cnvtdatetimeutc2(date_line,"YYYY-MM-DD",time_line,
    "HH;mm;ss",0)
   SET orequest->req[i].event_prsnl.request_comment = comment_text
 ...
 SET orequest->message_item.message_text = message_text
