CREATE PROGRAM cv_utl_upd_dm_prefs_notify:dba
 PROMPT
  "Enter Dataset Name:(e.g. ACC02, STS, STS02)[STS] = " = "STS",
  "Enter Target Name:(e.g. DISCERN_NOTIFY )[DISCERN_NOTIFY] = " = "DISCERN_NOTIFY",
  "Enter Action Name:(e.g. HARVEST)[HARVEST] = " = "HARVEST",
  "Enter Action Indicator (e.g. On = 1, Off = 0 [1] = " = "1"
 DECLARE ctrl_idx = i4 WITH private, noconstant(0)
 DECLARE pref_failed = c1 WITH private, noconstant("T")
 DECLARE stat = i4 WITH protect
 IF (((trim(cnvtupper( $1))=" ") OR (((trim(cnvtupper( $2))=" ") OR (((trim(cnvtupper( $3))=" ") OR (
 trim(cnvtupper( $4))=" ")) )) )) )
  CALL echo("******************************************")
 ...
 SET cv_action->action_list[1].pref_str = trim(cnvtupper( $1))
 SET cv_action->action_list[1].pref_name = trim(cnvtupper( $2))
 SET cv_action->action_list[1].pref_section = trim(cnvtupper( $3))
 SET cv_action->action_list[1].pref_nbr = cnvtint(trim(cnvtupper( $4)))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE dp.pref_domain=cvnet_cnst
    ...
    AND dp.pref_str=trim(cnvtupper(cv_action->action_list[d.seq].pref_str),3))
  DETAIL
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (cd
   WHERE cd.dataset_internal_name=trim(cnvtupper(cv_action->action_list[d.seq].pref_str)))
  DETAIL
...
#exit_script
 ...
 ELSE
  ...
  FREE RECORD request
 ENDIF
 DECLARE cv_utl_upd_dm_prefs_notify_vrsn = vc WITH private, constant("MOD 001 05/31/06 BM9013")
END GO
