CREATE PROGRAM dm2_replicate_rdds_cleanup:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (validate(drrc_cleanup_drop_list->cnt,1)=1
  AND validate(drrc_cleanup_drop_list->cnt,2)=2)
  FREE RECORD drrc_cleanup_drop_list
  RECORD drrc_cleanup_drop_list(
    1 cnt = i4
    1 qual[*]
      2 owner = vc
      2 table_name = vc
 ...
 ENDIF
 DECLARE drrc_iter = i4 WITH protect, noconstant(0)
 DECLARE drrc_purge_string = vc WITH protect, noconstant("")
 DECLARE drrc_drop_cmd = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_rep_rdds_clean",".log","dm2_replicate_rdds_cleanup LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_replicate_rdds_cleanup"
 CALL disp_msg("",dm_err->logfile,0)
 IF (currdbuser != "V500")
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Selecting list of rdds tables from dm2_dba_tables."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm2_dba_tables ddt
  WHERE ddt.owner="V500"
   AND ddt.table_name="*$R"
  ORDER BY ddt.table_name
  HEAD REPORT
   drrc_cleanup_drop_list->cnt = 0, stat = alterlist(drrc_cleanup_drop_list->qual,
    drrc_cleanup_drop_list->cnt)
  DETAIL
   drrc_cleanup_drop_list->cnt = (drrc_cleanup_drop_list->cnt+ 1)
   IF (mod(drrc_cleanup_drop_list->cnt,25)=1)
    stat = alterlist(drrc_cleanup_drop_list->qual,(drrc_cleanup_drop_list->cnt+ 24))
   ENDIF
   drrc_cleanup_drop_list->qual[drrc_cleanup_drop_list->cnt].owner = ddt.owner,
   drrc_cleanup_drop_list->qual[drrc_cleanup_drop_list->cnt].table_name = ddt.table_name
  FOOT REPORT
   stat = alterlist(drrc_cleanup_drop_list->qual,drrc_cleanup_drop_list->cnt)
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "No RDDS tables found in target domain"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(drrc_cleanup_drop_list)
 ENDIF
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_script
 ENDIF
 IF ((dm2_rdbms_version->level1 <= 9))
  SET drrc_purge_string = " "
 ELSE
  SET drrc_purge_string = "PURGE"
 ENDIF
 SET dm_err->eproc = "Dropping rdds tables from target database."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 FOR (drrc_iter = 1 TO drrc_cleanup_drop_list->cnt)
  SET drrc_drop_cmd = concat('RDB ASIS(^DROP TABLE "',drrc_cleanup_drop_list->qual[drrc_iter].owner,
   '"."',drrc_cleanup_drop_list->qual[drrc_iter].table_name,'" CASCADE CONSTRAINTS ',
   drrc_purge_string," ^) GO")
  IF ((dm_err->debug_flag=511))
   CALL echo(drrc_drop_cmd)
  ELSE
   IF (dm2_push_cmd(drrc_drop_cmd,1)=0)
    GO TO exit_script
   ENDIF
  ENDIF
 ENDFOR
...
#exit_script
 FREE RECORD drrc_cleanup_drop_list
 SET dm_err->eproc = "Ending dm2_replicate_rdds_cleanup."
 CALL final_disp_msg("dm2_rep_rdds_clean")
END GO
