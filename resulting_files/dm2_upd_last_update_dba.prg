CREATE PROGRAM dm2_upd_last_update:dba
 SET compile = debug
 ...
 DECLARE cdm_info = vc WITH protect, constant("DM_INFO")
 DECLARE last_update_row_found = i2 WITH protect, noconstant(0)
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE find_last_update_row() = i2
 DECLARE insert_last_update_row() = i2
 DECLARE update_last_update_row() = i2
 IF (check_logfile("dm2_upd_last_update",".log","Starting dm2_upd_last_update")=false)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_upd_last_update"
 CALL disp_msg("",dm_err->logfile,0)
 IF (currdbuser="CDBA")
  SET dm_err->eproc = "User Last Update Row Not Updated On The Admin Database."
  CALL disp_msg("",dm_err->logfile,0)
 ELSE
  CASE (dm2_table_exists(value(cdm_info)))
   OF "E":
    GO TO end_program
   OF "N":
    SET dm_err->eproc = "DM_INFO table does not exist. User Last Update Row Not Updated."
    CALL disp_msg("",dm_err->logfile,0)
   OF "F":
    IF (find_last_update_row(null)=false)
     GO TO end_program
    ENDIF
    IF (last_update_row_found=false)
     IF (insert_last_update_row(null)=false)
      GO TO end_program
     ENDIF
    ELSE
     IF (update_last_update_row(null)=false)
      GO TO end_program
     ENDIF
    ENDIF
  ENDCASE
 ENDIF
 SUBROUTINE find_last_update_row(null)
   IF ((dm_err->debug_flag=1))
    SET dm_err->eproc = "Find the Last Update Row"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain=cdomain_data_mgt
     AND di.info_name=cname_user_last_updt
    WITH nocounter
   ;end select
   IF (check_error("Find_Last_Update_Row")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(false)
   ELSE
    IF (curqual > 0)
     SET last_update_row_found = true
    ELSE
     SET last_update_row_found = false
    ENDIF
   ENDIF
   RETURN(true)
 END ;Subroutine
 SUBROUTINE insert_last_update_row(null)
   IF ((dm_err->debug_flag=1))
    SET dm_err->eproc = "Insert Last Update Row on dm_info"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   INSERT  FROM dm_info i
    ...
    WITH nocounter
   ;end insert
   IF (check_error("Insert_Last_Update_Row")=true)
    ROLLBACK
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(false)
   ENDIF
   IF ((dm_err->debug_flag=1))
    CALL echo(build("User Last Update Row Inserted: ",curqual))
   ENDIF
   COMMIT
   RETURN(true)
 END ;Subroutine
 SUBROUTINE update_last_update_row(null)
   IF ((dm_err->debug_flag=1))
    SET dm_err->eproc = "Update Last Update Row on dm_info"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   UPDATE  FROM dm_info i
    ...
    WITH nocounter
   ;end update
   IF (check_error("Update_Last_Update_Row")=true)
    ROLLBACK
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(false)
   ENDIF
   IF ((dm_err->debug_flag=1))
    CALL echo(build("User Last Update Row Updated: ",curqual))
   ENDIF
   COMMIT
   RETURN(true)
 END ;Subroutine
#end_program
 SET dm_err->eproc = "Ending dm2_upd_last_update"
 CALL final_disp_msg(" ")
END GO
