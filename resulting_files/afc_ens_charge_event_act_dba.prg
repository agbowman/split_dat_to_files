CREATE PROGRAM afc_ens_charge_event_act:dba
 ...
 IF (validate(reply->status_data.status,"Z")="Z")
  ...
  SET stat = alterlist(reply->charge_event_act,reply->charge_event_act_qual)
 ENDIF
 SET hafc_ens_charge_event_act = 0
 SET istatus = 0
 ...
 CASE (request->action_type)
  OF "ADD":
   EXECUTE afc_add_charge_event_act
   IF (failed != false)
    GO TO check_error
   ENDIF
  OF "UPT":
   EXECUTE afc_upt_charge_event_act
   IF (failed != false)
    GO TO check_error
   ENDIF
  OF "DEL":
   EXECUTE afc_del_charge_event_act
   IF (failed != false)
    GO TO check_error
   ENDIF
  ELSE
