CREATE PROGRAM dm_rmc_mover_0775:dba
 ...
 SUBROUTINE drmm_breakup_string(sbr_rs_reply)
   ...
   RETURN("S")
 END ;Subroutine
 DECLARE cs0775_tab_name = vc
 DECLARE cust_question_cd = vc
 DECLARE cust_response_flag = i4
 DECLARE cs0775_from_value = vc
 DECLARE cs0775_process_tab = vc
 DECLARE cs0775_process_col = vc
 ...
 SET cs0775_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cs0775_link = dm2_ref_data_doc->post_link_name
 SET cust_column_cnt = get_col_pos(cs0775_tab_name,"ANSWER")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_column_cnt].translated = 0
 SET cust_question_cd = get_value(cs0775_tab_name,"QUESTION_CD","FROM")
 SET cs0775_src_tbl = dm2_get_rdds_tname("QUESTION")
 SELECT INTO "NL:"
  FROM (parser(cs0775_src_tbl) q)
  WHERE q.question_cd=cnvtreal(cust_question_cd)
  DETAIL
   cust_response_flag = q.response_flag
  WITH nocounter
 ;end select
 IF (check_error("Obtaining QUESTION_CD value.")=1)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "Could not obtain QUESTION_CD value"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_0775
 ENDIF
 IF (curqual=0)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV55"
  CALL disp_msg("ERROR: relationship not found on QUESTION table",dm_err->logfile,0)
  GO TO exit_0775
 ENDIF
 SET cs0775_from_value = get_value(cs0775_tab_name,"ANSWER","FROM")
 IF (((cust_response_flag=0) OR (cust_response_flag=1)) )
  SET cs0775_process_tab = "CODE_VALUE"
  SET cs0775_process_col = "CODE_VALUE"
  SET cust_new_value = select_merge_translate(cs0775_from_value,cs0775_process_tab)
  IF (isnumeric(cust_new_value))
   CALL put_value(cs0775_tab_name,"ANSWER",cust_new_value)
  ELSE
   SET cust_rpt_missing = report_missing(cs0775_process_tab,cs0775_process_col,cnvtreal(
     cs0775_from_value))
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ENDIF
 ELSEIF (cust_response_flag > 1
  AND cust_response_flag < 6)
  CALL put_value(cs0775_tab_name,"ANSWER",cs0775_from_value)
 ELSE
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV55"
  CALL disp_msg("ERROR: Unexpected RESPONSE_FLAG value.",dm_err->logfile,0)
  GO TO exit_0775
 ENDIF
 ...
 SET cust_idcd_check = is_translated(cs0775_tab_name,"ALL")
 IF (drdm_error_out_ind=1)
  GO TO exit_0775
 ENDIF
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cs0775_tab_name,"ANSWER")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the ANSWER column"
