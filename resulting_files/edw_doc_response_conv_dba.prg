CREATE PROGRAM edw_doc_response_conv:dba
 ...
 SET syscmd = "mv $cer_extract/*DOCRESP_O_* $cer_extract/docresp.dat"
 SET len = size(trim(syscmd))
 SET status = 0
 CALL dcl(syscmd,len,status)
 IF (findfile("ccluserdir:DOCRESP.dat") != 0)
  CALL echo("processing DOCRESP.dat")
  DEFINE rtl2 "CCLUSERDIR:DOCRESP.DAT"
  SET rtl2_defined = 1
 ENDIF
 IF (rtl2_defined=1)
  SELECT INTO "nl:"
  ...
  SET return_val = 1
 ENDIF
 SET syscmd = "mv $cer_extract/docresp.dat $cer_extract/docrespdone.dat"
 SET len = size(trim(syscmd))
 SET status = 0
 CALL dcl(syscmd,len,status)
 SELECT INTO "nl:"
  ...
  PLAN (d
   WHERE line_cnt > 0)
   JOIN (da
   WHERE (da.dcp_forms_activity_id=reply_orphans->qual[d.seq].orphan_key))
   JOIN (dac
 ...
 WHILE (keys_start <= keys_end)
   ...
   SELECT INTO "nl:"
    ...
    HEAD ce2.event_id
     IF (ce2.event_class_cd != grp_cd)
      ...
      stat = alterlist(doc_response->qual[d.seq].results,i1), doc_response->qual[d.seq].results[i1].
      ce2_event_id = ce2.event_id, doc_response->qual[d.seq].results[i1].event_sk = ce2.event_id,
      doc_response->qual[d.seq].results[i1].parent_event_sk = ce2.parent_event_id, doc_response->
      qual[d.seq].results[i1].event_class_cd = ce2.event_class_cd, doc_response->qual[d.seq].results[
      i1].section_event_sk = doc_response->qual[d.seq].ce1_event_id,
      doc_response->qual[d.seq].results[i1].doc_response_sk = concat(trim(cnvtstring(ce2.event_id,16),
        3),"~-3"), doc_response->qual[d.seq].results[i1].doc_input_sk = build(cnvtstring(doc_response
        ->qual[d.seq].dcp_form_instance_id,16),"~",cnvtstring(doc_response->qual[d.seq].
        dcp_section_instance_id,16),"~",cnvtstring(ce2.event_cd,16))
     ELSE
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     ENDIF
     doc_response->qual[d1.seq].results[d2.seq].response_value = trim(format(cnvtdatetime(cdr
        .result_dt_tm,3),"MM/DD/YYYY HH:MM;;D"),3), doc_response->qual[d1.seq].results[d2.seq].
     doc_response_sk = concat(trim(cnvtstring(cdr.event_id,16),3),"~-2"), doc_response->qual[d1.seq].
     results[d2.seq].doc_response_seq = "-2",
    ...
    HEAD ccr.sequence_nbr
     ...
     IF (cnt=1)
      doc_response->qual[d1.seq].results[d2.seq].alpha_response_sk = trim(cnvtstring(n
        .nomenclature_id,16),3), doc_response->qual[d1.seq].results[d2.seq].response_value = trim(n
       .source_string,3), doc_response->qual[d1.seq].results[d2.seq].doc_response_sk = concat(trim(
        cnvtstring(ccr.event_id,16),3),"~",trim(cnvtstring(ccr.sequence_nbr),3)),
     ...
     ELSE
      ...
      doc_response->qual[d1.seq].results[i1].alpha_response_sk = trim(cnvtstring(n.nomenclature_id,16
        ),3), doc_response->qual[d1.seq].results[i1].response_value = trim(n.source_string,3),
   ...
   SELECT INTO "nl:"
    ...
    ORDER BY d1.seq, d2.seq, csr.event_id,
     csr.valid_until_dt_tm
    HEAD REPORT
     MACRO (check_set_ind)
    ...
    DETAIL
     ...
     CALL print(trim(datetimezoneformat(evaluate(curutc,1,doc_response->qual[d1.seq].results[d2.seq].
        date_time_response_dt_tm,0,cnvtdatetimeutc(doc_response->qual[d1.seq].results[d2.seq].
         date_time_response_dt_tm,3)),utc_timezone_index,"MM/DD/YYYY HH:mm"))), v_bar,
