CREATE PROGRAM bbt_tag_emerg_barcode:dba
 RECORD antibody(
   1 antibodylist[10]
     2 antibody_cd = f8
     2 antibody_disp = c15
 ...
 EXECUTE cpm_create_file_name_logical "bbt_tag_emerg_bar", "dat", "x"
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  DETAIL
   ...
   nfont_0_15,
   CALL print(calcpos(200,36)), captions->emergency_transfusion, row + 1,
   ...
   patient_location, row + 1, nfont_31_9,
   nypos = 72
   ...
   row + 1, nypos += 18,
   CALL print(calcpos(nxpos3,nypos)),
   captions->prepared_by, row + 1,
   CALL print(calcpos(nxpos4,nypos)),
   tech_name, row + 1, nypos += 18,
   CALL print(calcpos(nxpos3,nypos)), captions->prepared_dt_tm, row + 1
   IF (pe_event_dt_tm > 0)
   ...
   nfont_0_16, rpt_row = (row+ 12), row rpt_row,
   col 006, captions->delay_in_transfusion, col 055,
   captions->authorize_admin, rpt_row += 2, row rpt_row,
   col 006, captions->uncrossmatched_blood, col 043,
   captions->attending_physician, rpt_row += 2, row rpt_row,
   col 004, captions->return_to_bb, col 052,
   captions->in_30_min, rpt_row += 1, row rpt_row,
   col 004, captions->refrigerator, nfont_0_16,
   rpt_row += 2, row rpt_row, col 002,
   captions->before_transfusion, col 053, captions->identified_recipient,
   col 094, captions->clerical_check, rpt_row += 1,
   row rpt_row, col 002, captions->of_the_wristband,
   col 052, captions->number_on_form, col 094,
   captions->released_by, rpt_row += 1, row rpt_row,
   col 002, captions->further_certify, col 057,
   captions->unit_number, col 094, captions->date_time,
   rpt_row += 1, row rpt_row, col 002,
   captions->stated_on_form, rpt_row += 1, row rpt_row,
   col 041, signature27, col 071,
   signature12, col 094, captions->transported_by,
   rpt_row += 1, row rpt_row, col 046,
   captions->transfusionist, col 071, captions->time_started,
   col 094, captions->received_by, rpt_row += 1,
   row rpt_row, col 094, captions->date_time,
   rpt_row += 1, row rpt_row, col 041,
   signature27, col 071, signature12,
   rpt_row += 1, row rpt_row, col 046,
   captions->transfusionist, col 071, captions->time_ended,
   rpt_row += 7, row rpt_row, col 082,
   ...
   col 038, captions->original_to_chart, rpt_row += 1,
   row rpt_row, col 041, captions->copy_to_lab
  FOOT  d.seq
   IF (d.seq > 0
