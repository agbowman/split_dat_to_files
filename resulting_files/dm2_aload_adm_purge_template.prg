CREATE PROGRAM dm2_aload_adm_purge_template
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_tpl",".log","dm2_aload_adm_purge_template")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_adm_purge_template"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daapt_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daapt_file = vc WITH protect, noconstant( $2)
 DECLARE daapt_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daapt_batch = i4 WITH protect, noconstant(0)
 DECLARE daapt_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daapt_pkg,daapt_file_loc_ret,daapt_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daapt_batch=0)
  SET daapt_batch = 5000
 ENDIF
 IF (daapt_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daapt_file_loc_ret,daapt_file), "dm2_aload_prgtpl_templ", daapt_batch
  CALL echo(concat(daapt_file_loc_ret,daapt_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_adm_purge_template completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_adm_purge_template")
END GO
