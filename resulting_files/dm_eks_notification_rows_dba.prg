CREATE PROGRAM dm_eks_notification_rows:dba
 ...
 SET reply->table_name = "EKS_NOTIFICATIONS"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(e.notification_id)
   FROM eks_notifications e
   WHERE e.notification_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    e.rowid
    FROM eks_notifications e
    WHERE parser(sbr_getrowidnotexists("e.notification_id between curMinID and curMaxID","e"))
     AND e.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = e.rowid
    WITH nocounter, maxqual(e,value(rowsleft))
   ;end select
