CREATE PROGRAM afc_batch_now_release:dba
 SET afc_batch_now_release_vrsn = "708659.006"
 ...
 SET stat = uar_get_meaning_by_codeset(6004,"COMPLETED",1,orders_complete)
 SET max_rows = 15000
 SET done = 0
 SET found_some = 0
 WHILE (done=0)
   ...
   ELSE
    SET done = 1
    CALL echo("No more held charges qualified to be released.")
   ENDIF
 ENDWHILE
 IF (found_some=0)
 ...
 SUBROUTINE (getheldchargesoforggroupwithoutorderidpresent(porggroupcodevalue=f8,prcount=i4(ref)) =i2
  ...
  SELECT INTO "nl:"
   ...
   PLAN (c
    WHERE c.process_flg=held_flg
     AND c.order_id=0
     AND c.service_dt_tm < cnvtdatetime(curdate,curtime))
    JOIN (bop
    WHERE bop.organization_id=c.payor_id
     AND bop.bill_org_type_cd=cs13031_orggroup_cd
     AND bop.bill_org_type_id=porggroupcodevalue
