CREATE PROGRAM bhs_athn_add_prob_comment
 RECORD orequest(
   1 person_id = f8
   1 problem_cnt = i4
   1 problem[*]
 ...
 SET orequest->problem[orequest->problem_cnt].problem_id =  $4
 SET orequest->problem[orequest->problem_cnt].comment_only = 1
 SET orequest->problem[orequest->problem_cnt].problem_comment_cnt = 1
 SET stat = alterlist(orequest->problem[orequest->problem_cnt].problem_comment,orequest->problem[
  orequest->problem_cnt].problem_comment_cnt)
 SET orequest->problem[orequest->problem_cnt].problem_comment[1].comment_dt_tm = cnvtdatetime(curdate,
  curtime)
 SET orequest->problem[orequest->problem_cnt].problem_comment[1].comment_prsnl_id =  $3
 SET t_request->param =  $5
 EXECUTE bhs_athn_format_str_param  WITH replace("REQUEST",t_request), replace("REPLY",t_reply)
 SET orequest->problem[orequest->problem_cnt].problem_comment[1].problem_comment = t_reply->param
 SET stat = tdbexecute(600005,3202004,963035,"REC",orequest,
  "REC",oreply)
 FOR (i = 1 TO size(oreply->problem_list,5))
   FOR (j = 1 TO size(oreply->problem_list[i].comment_list,5))
     SET out_rec->problem_comment_id = cnvtstring(oreply->problem_list[i].comment_list[j].
      problem_comment_id)
   ENDFOR
 ENDFOR
