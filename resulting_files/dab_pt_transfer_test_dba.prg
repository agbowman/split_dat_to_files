CREATE PROGRAM dab_pt_transfer_test:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ELSEIF ((pt_transfer->facility IN ("MLH", "BMLH")))
    ...
    ENDIF
   ELSEIF ((pt_transfer->facility IN ("BWH", "BWHINPTPSYCH")))
    IF (phloc.parent_entity_id > 0)
     pt_transfer->facility_phone = cnvtphone(phloc.phone_num,phloc.phone_format_cd,2)
    ELSE
     pt_transfer->facility_phone = " "
    ENDIF
   ELSEIF ((pt_transfer->facility IN ("BNH", "BNHINPTPSYCH")))
    IF (phloc.parent_entity_id > 0)
     pt_transfer->facility_phone = cnvtphone(phloc.phone_num,phloc.phone_format_cd,2)
    ELSE
     pt_transfer->facility_phone = " "
 ...
 SELECT INTO value(printer_disp)
  ...
  FOOT REPORT
   ...
   xcol, ycol, " moveto (",
   page_string, ") dup stringwidth pop 2 div neg 0 rmoveto show/}"
  WITH dio = postscript, maxcol = 12000, check
 ;end select
#exit_script
 CALL echorecord(pt_transfer)
