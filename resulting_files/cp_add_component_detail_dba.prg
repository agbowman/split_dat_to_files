CREATE PROGRAM cp_add_component_detail:dba
 ...
 protect
 DECLARE doclayouttypecd = f8 WITH constant(uar_get_code_by("MEANING",4003134,"DOCLAYOUT")), protect
 DECLARE doceventstypecd = f8 WITH constant(uar_get_code_by("MEANING",4003134,"DOCEVENTS")), protect
 DECLARE doctermtypecd = f8 WITH constant(uar_get_code_by("MEANING",4003134,"DOCTERMDEC")), protect
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE complistsize = i4 WITH noconstant(size(request->comp_detail_list,5)), protect
   DECLARE comp_detail_reltn_cd_codeset = i4 WITH constant(4003134), protect
   IF (complistsize <= 0)
    CALL reporterrorandexit("PERFORM","Validate","Component Detail list must be nonzero in size.")
   ENDIF
   FOR (x = 1 TO complistsize)
     SELECT INTO "NL:"
      FROM cp_component cc
      WHERE (cc.cp_component_id=request->comp_detail_list[x].cp_component_id)
      WITH nocounter
     ;end select
     IF (curqual=0)
      CALL reporterrorandexit("PERFORM","Validate","cp_component_id is invalid.")
     ENDIF
     IF ((request->comp_detail_list[x].component_entity_name="")
      AND (request->comp_detail_list[x].component_entity_id=0.0))
      IF ((request->comp_detail_list[x].component_ident="")
       AND (request->comp_detail_list[x].component_text=""))
       CALL reporterrorandexit("PERFORM","Validate",
        "Either entity_id and entity_name or ident and text must be filled out.")
      ENDIF
     ENDIF
   ENDFOR
 ...
 SUBROUTINE insertrows(null)
   ...
   ENDIF
   SET stat = alterlist(reply->comp_detail_list,size(request->comp_detail_list,5))
   FOR (x = 1 TO size(request->comp_detail_list,5))
     SET cp_component_detail_id = 0.0
     SELECT INTO "nl:"
      nextseqnum = seq(reference_seq,nextval)
      FROM dual
      DETAIL
       cp_component_detail_id = nextseqnum
      WITH nocounter
     ;end select
     CALL echo(build2("Next cp_component_detail_id:",cp_component_detail_id))
     IF (cp_component_detail_id=0.0)
      CALL reporterror("PERFORM","INSERT",
       " Unable to generate cp_component_detail_id from REFERENCE_SEQ.")
      ROLLBACK
      GO TO exit_script
     ENDIF
     INSERT  FROM cp_component_detail ccd
      SET ccd.cp_component_id = request->comp_detail_list[x].cp_component_id, ccd
       .component_detail_reltn_cd = request->comp_detail_list[x].comp_detail_reltn_cd, ccd
       .component_entity_id = request->comp_detail_list[x].component_entity_id,
       ccd.component_entity_name = request->comp_detail_list[x].component_entity_name, ccd
       .component_ident = request->comp_detail_list[x].component_ident, ccd.component_text = request
       ->comp_detail_list[x].component_text,
       ccd.cp_component_detail_id = cp_component_detail_id, ccd.collation_seq = validate(request->
        comp_detail_list[x].collation_seq,0), ccd.selected_ind = validate(request->comp_detail_list[x
        ].selected_ind,0),
       ccd.updt_dt_tm = cnvtdatetime(sysdate), ccd.updt_id = reqinfo->updt_id, ccd.updt_task =
       reqinfo->updt_task,
       ccd.updt_cnt = 0, ccd.updt_applctx = reqinfo->updt_applctx, ccd.version_nbr = request->
       comp_detail_list[x].version_nbr,
       ccd.version_text = request->comp_detail_list[x].version_text, ccd.version_flag = request->
       comp_detail_list[x].version_flag, ccd.source_flag = request->comp_detail_list[x].source_flag,
       ccd.default_ind = request->comp_detail_list[x].default_ind
      WITH nocounter
     ;end insert
     CALL error_and_zero_check(curqual,"INSERT","cp_add_component_detail",1,1)
     SET reply->comp_detail_list[x].comp_detail_reltn_cd = request->comp_detail_list[x].
     comp_detail_reltn_cd
     SET reply->comp_detail_list[x].component_entity_id = request->comp_detail_list[x].
     component_entity_id
     SET reply->comp_detail_list[x].component_entity_name = request->comp_detail_list[x].
     component_entity_name
     SET reply->comp_detail_list[x].component_ident = request->comp_detail_list[x].component_ident
     SET reply->comp_detail_list[x].component_text = request->comp_detail_list[x].component_text
     SET reply->comp_detail_list[x].cp_component_id = request->comp_detail_list[x].cp_component_id
     SET reply->comp_detail_list[x].cp_component_detail_id = cp_component_detail_id
     SET reply->comp_detail_list[x].collation_seq = validate(request->comp_detail_list[x].
      collation_seq,0)
     SET reply->comp_detail_list[x].selected_ind = validate(request->comp_detail_list[x].selected_ind,
      0)
     SET reply->comp_detail_list[x].version_nbr = request->comp_detail_list[x].version_nbr
     SET reply->comp_detail_list[x].version_text = request->comp_detail_list[x].version_text
     SET reply->comp_detail_list[x].version_flag = request->comp_detail_list[x].version_flag
     SET reply->comp_detail_list[x].source_flag = request->comp_detail_list[x].source_flag
     SET reply->comp_detail_list[x].default_ind = request->comp_detail_list[x].default_ind
   ENDFOR
