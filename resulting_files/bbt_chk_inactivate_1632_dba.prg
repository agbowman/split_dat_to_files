...
#exit_script
 IF (((active_ind=0) OR (curqual=0)) )
  ...
  CALL logmsg("Inactivation of Numeric value on code_set 1632 was successful.",((e_rdmstatus+
   e_dispmsg)+ e_logtofile))
 ELSE
  ROLLBACK
  CALL logstatus("Execution failed - see log message for more details.","F")
  CALL logmsg("Inactivation of Numeric value on code_set 1632 failed.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
 ENDIF
