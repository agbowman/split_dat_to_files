CREATE PROGRAM bhs_rpt_pmat_missing_groupb:dba
 PROMPT
 ...
 protect
 DECLARE groupbstrepculture = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,"GROUPBSTREPCULTURE")),
 protect
  ...
  PLAN (e
   ...
   (SELECT
    ...
    WHERE ce.person_id=p.person_id
     AND ce.event_cd=groupbstrepculture
     AND ce.event_start_dt_tm >= cnvtdatetime((curdate - 30),curtime3)))))
   JOIN (pi
