CREATE PROGRAM dm_async_exec_weekly:dba
 ...
 SUBROUTINE async_exec_script(appid,taskid,reqid,scriptname)
   ...
   ENDIF
 END ;Subroutine
 DECLARE forceperiodic = i4
 CALL getdebugrow("x")
 IF (((datetimefind(cnvtdatetime(curdate,0),"W","B","B")=cnvtdatetime(curdate,0)) OR (forceperiodic=1
 )) )
  SET stat = alterlist(async_exec->qual,1)
  SET async_exec->qual[1].appid = 3202004
  SET async_exec->qual[1].taskid = 3202004
  SET async_exec->qual[1].reqid = 4420564
  SET async_exec->qual[1].scriptname = "bed_run_recommendations_esm"
 ENDIF
 SUBROUTINE getdebugrow(x)
   SELECT INTO "nl:"
    ...
    WHERE info_domain="DM_STAT_GATHER_DEBUG"
     AND info_name="FORCE_PERIODIC"
    DETAIL
     forceperiodic = di.info_number
    WITH nocounter
   ;end select
   IF (curqual=0)
    INSERT  FROM dm_info
     SET info_domain = "DM_STAT_GATHER_DEBUG", info_name = "FORCE_PERIODIC", info_number = 0
     WITH nocounter
    ;end insert
    COMMIT
   ENDIF
