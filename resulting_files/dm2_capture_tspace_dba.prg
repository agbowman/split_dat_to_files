CREATE PROGRAM dm2_capture_tspace:dba
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dct_logfile_prefix = vc WITH protect, constant("dm2_capture_tspace")
 DECLARE dcs_source_rdbms = vc WITH protect, noconstant("")
 DECLARE dct_sch_date_str = vc WITH protect
 DECLARE dct_sch_date_fmt = f8 WITH protect
 DECLARE dct_data_to_move = vc WITH protect
 DECLARE dct_sch_date_type = vc WITH protect
 DECLARE dct_file_prefix = vc WITH protect
 DECLARE dct_select_dir = vc WITH protect
 DECLARE dct_mod_ind = i2 WITH protect, noconstant(1)
 DECLARE dct_work_cnt = i4 WITH protect, noconstant(0)
 DECLARE dct_size_tmp = f8 WITH protect, noconstant(0.0)
 DECLARE dct_ora_block = f8 WITH protect, constant((5.0 * dm2_block_size))
 DECLARE dct_validate_and_prep(null) = i4
 DECLARE dct_val_write_privs(dct_dir=vc,dct_full_dir=vc) = i4
 DECLARE dct_load_source_objects(null) = i4
 DECLARE dct_adjust_object_allocations(dctaoa_opt=vc) = i4
 DECLARE dct_rollup_to_target(null) = i4
 DECLARE dct_assign_ddlops_tspace(null) = i4
 DECLARE dct_debuglogs(null) = i4
 DECLARE dct_tablespace_sizing_adjust(null) = null
 DECLARE dct_adjust_object_extents(null) = i4
 IF ((validate(dct_obj->object_cnt,- (1))=- (1)))
  FREE SET dct_obj
  RECORD dct_obj(
    1 extent_adj_ind = i2
    1 object_cnt = i4
  ...
  SET stat = alterlist(dct_obj->object,10)
  SET dct_obj->object_cnt = 0
  SET dct_obj->extent_adj_ind = 0
 ENDIF
 IF ((validate(dct_tspace->tspace_count,- (1))=- (1)))
  FREE SET dct_tspace
  RECORD dct_tspace(
    1 tspace_count = i4
    1 tspace[*]
  ...
  SET dct_tspace->tspace_count = 0
 ENDIF
 IF ((validate(dct_dtd->tbl_cnt,- (1))=- (1)))
  FREE SET dct_dtd
  RECORD dct_dtd(
    1 tbl_cnt = i4
    1 tbl[*]
  ...
  SET dct_dtd->tbl_cnt = 0
 ENDIF
 FREE RECORD dct_ts_exclusions
 RECORD dct_ts_exclusions(
   1 ts_cnt = i4
   1 qual[*]
     2 tspace_name = vc
 )
 FREE RECORD dct_cust_ref
 RECORD dct_cust_ref(
   1 tbl_cnt = i4
   1 tbl[*]
     2 table_name = vc
 )
 SET dct_cust_ref->tbl_cnt = 0
 IF (check_logfile(dct_logfile_prefix,".log","DM2_CAPTURE_TSPACE LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM2_CAPTURE_TSPACE"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm2_sch_file->dest_dir_cclfmt = "cer_install"
 SET dct_select_dir = "cer_install"
 SET dct_source_rdbms = trim(currdb)
 IF (dct_validate_and_prep(1) != 1)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo("*************************************************")
  CALL echo(concat("Schema Date(Parm 1)..........",dct_sch_date_str))
  CALL echo(concat("Data To Move (Parm 2)........",dct_data_to_move))
  CALL echo(concat("Schema Date Prefix...........",dct_sch_date_type))
  CALL echo(concat("Destination Directory........",dm2_sch_file->dest_dir_cclfmt))
  CALL echo(concat("Format for select Directory..",dct_select_dir))
  CALL echo(concat("Capture Source RDBMS.........",dct_source_rdbms))
  CALL echo("*************************************************")
 ENDIF
 SET dm_err->eproc = "LOAD SNAPSHOT OF TABLESPACE INFORMATION INTO MEMORY"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((drr_clin_copy_data->process="MIGRATION"))
  SET dm_err->eproc = "Determining system-related tablespace names from dba_rollback_segs."
  SELECT DISTINCT INTO "nl:"
   drs.tablespace_name
   FROM dba_rollback_segs drs
   HEAD REPORT
    dct_ts_exclusions->ts_cnt = 0, stat = alterlist(dct_ts_exclusions->qual,0)
   DETAIL
    dct_ts_exclusions->ts_cnt = (dct_ts_exclusions->ts_cnt+ 1), stat = alterlist(dct_ts_exclusions->
     qual,dct_ts_exclusions->ts_cnt), dct_ts_exclusions->qual[dct_ts_exclusions->ts_cnt].tspace_name
     = drs.tablespace_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Determining system-related tablespace names from dba_tablespaces."
  SELECT DISTINCT INTO "nl:"
   dt.tablespace_name
   FROM dba_tablespaces dt
   WHERE dt.contents IN ("UNDO", "TEMPORARY")
   DETAIL
    dct_ts_exclusions->ts_cnt = (dct_ts_exclusions->ts_cnt+ 1), stat = alterlist(dct_ts_exclusions->
     qual,dct_ts_exclusions->ts_cnt), dct_ts_exclusions->qual[dct_ts_exclusions->ts_cnt].tspace_name
     = dt.tablespace_name
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Determining space required for tablespaces during migration"
  SELECT INTO "nl:"
   ddf.tablespace_name, space_required = sum(ddf.bytes)
   FROM dba_data_files ddf
   WHERE "AVAILABLE"=ddf.status
    AND  NOT (ddf.tablespace_name IN ("SYSTEM", "SYSAUX"))
   GROUP BY ddf.tablespace_name
   ORDER BY ddf.tablespace_name
   HEAD REPORT
    dct_tspace->tspace_count = 0, found = 0
   DETAIL
    found = 0, found = locateval(found,1,dct_ts_exclusions->ts_cnt,ddf.tablespace_name,
     dct_ts_exclusions->qual[found].tspace_name)
    IF (found=0)
     dct_tspace->tspace_count = (dct_tspace->tspace_count+ 1)
     IF (mod(dct_tspace->tspace_count,50)=1)
      stat = alterlist(dct_tspace->tspace,(dct_tspace->tspace_count+ 49))
     ENDIF
     dct_tspace->tspace[dct_tspace->tspace_count].tspace_name = ddf.tablespace_name, dct_tspace->
     tspace[dct_tspace->tspace_count].bytes_adjusted = (space_required * (1+ (cnvtreal(
      dm2_install_schema->percent_tspace)/ 100))), dct_tspace->tspace[dct_tspace->tspace_count].
     tspace_type = "L"
    ENDIF
   FOOT REPORT
    stat = alterlist(dct_tspace->tspace,dct_tspace->tspace_count)
   WITH nocounter, nullreport
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  IF (validate(dm2_bypass_long_to_lob,- (1)) != 1
   AND (dcs_long_tspace->tspace_count > 0))
   SET dm_err->eproc = "Add long to lob space to dct_tspace"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM (dummyt t  WITH seq = dcs_long_tspace->tspace_count)
    ORDER BY dcs_long_tspace->tspace[t.seq].tspace_name
    HEAD REPORT
     found = 0
    DETAIL
     found = 0, found = locateval(found,1,dct_tspace->tspace_count,dcs_long_tspace->tspace[t.seq].
      tspace_name,dct_tspace->tspace[found].tspace_name)
     IF (found=0)
      dct_tspace->tspace_count = (dct_tspace->tspace_count+ 1), stat = alterlist(dct_tspace->tspace,
       dct_tspace->tspace_count), dct_tspace->tspace[dct_tspace->tspace_count].tspace_name =
      dcs_long_tspace->tspace[t.seq].tspace_name,
      dct_tspace->tspace[dct_tspace->tspace_count].bytes_adjusted = dcs_long_tspace->tspace[t.seq].
      bytes, dct_tspace->tspace[dct_tspace->tspace_count].tspace_type = "L",
      CALL echo(concat("New long tablespace ",trim(dcs_long_tspace->tspace[t.seq].tspace_name),
       " was added to dct_tspace."))
     ELSE
      dct_tspace->tspace[found].bytes_adjusted = (dct_tspace->tspace[found].bytes_adjusted+
      dcs_long_tspace->tspace[t.seq].bytes),
      CALL echo(concat("Long tablespace ",trim(dcs_long_tspace->tspace[t.seq].tspace_name),
       " already exists in dct_tspace."))
     ENDIF
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Loading space usage information for SOURCE Tables, Indexes and Lobs"
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dct_load_source_objects(1) != 1)
   GO TO exit_script
  ENDIF
  IF ((dm_err->debug_flag >= 5))
   CALL echorecord(dct_obj,"dct_before")
  ENDIF
  IF (drr_load_mixed_table_data(0)=0)
   GO TO exit_script
  ENDIF
  IF (dct_data_to_move IN ("REF", "ADS"))
   IF (drr_get_mixtbl_ref_rows(trim(cnvtupper(currdbname)))=0)
    GO TO exit_script
   ENDIF
  ENDIF
  SET dm_err->eproc = "Adjusting Target object allocations..."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dct_adjust_object_allocations(dct_data_to_move) != 1)
   GO TO exit_script
  ENDIF
  IF ((dct_obj->extent_adj_ind=1))
   IF (dct_adjust_object_extents(null)=0)
    GO TO exit_script
   ENDIF
  ENDIF
  IF ((dm_err->debug_flag >= 5))
   CALL echorecord(dct_obj,"dct_after")
  ENDIF
  SET dm_err->eproc = "Rolling up TARGET sizing to tablespace level..."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dct_rollup_to_target(1) != 1)
   GO TO exit_script
  ENDIF
  CALL dct_tablespace_sizing_adjust(null)
  IF ((drr_clin_copy_data->process != "REFRESH"))
   IF (dir_dm2_tables_tspace_assign(null)=0)
    GO TO exit_script
   ENDIF
   IF (dct_assign_ddlops_tspace(1) != 1)
    GO TO exit_script
   ENDIF
  ENDIF
 ENDIF
 SET dm_err->eproc = "Load dmtspace schema file description into memory"
 CALL disp_msg("",dm_err->logfile,0)
 IF (dsfi_load_schema_file_defs("TSPACE") != 1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "CREATE EMPTY DMTSPACE SCHEMA FILE"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (create_sch_files(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Populating DMTSPACE schema file with tablespace information."
 CALL disp_msg(" ",dm_err->logfile,0)
 INSERT  FROM dmtspace t,
   (dummyt d  WITH seq = value(dct_tspace->tspace_count))
  SET t.seq = 1, t.tspace_name = dct_tspace->tspace[d.seq].tspace_name, t.bytes_needed = dct_tspace->
   tspace[d.seq].bytes_adjusted,
   t.ext_mgmt = dct_tspace->tspace[d.seq].tspace_type, t.updt_dt_tm = cnvtdatetime(curdate,curtime3)
  PLAN (d)
   JOIN (t
   WHERE (dct_tspace->tspace[d.seq].tspace_name=t.tspace_name))
  WITH nocounter, outerjoin = d, dontexist
 ;end insert
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 GO TO exit_script
 SUBROUTINE dct_validate_and_prep(null)
   DECLARE dct_iv_return_status = i2 WITH protect, noconstant(1)
   IF (currdb IN ("DB2UDB", "SQLSRV"))
    SET dm_err->eproc = concat("This program does not run on ",currdb,".")
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Validating user privileges"
   ...
   IF (val_user_privs(1)=0)
    RETURN(0)
   ENDIF
   IF (dct_val_write_privs(dct_select_dir,dm2_install_schema->cer_install)=0)
    RETURN(0)
   ENDIF
   SET dct_sch_date_str = trim( $1)
   IF (dm2_val_sch_date_str(dct_sch_date_str)=0)
    RETURN(0)
   ENDIF
   SET dct_sch_date_fmt = cnvtdate2(dct_sch_date_str,"DD-MMM-YYYY")
   IF ((dm2_install_schema->process_option="CLIN COPY"))
    SET dct_sch_date_type = "dm2s"
   ELSE
    SET dct_sch_date_type = "dm2c"
   ENDIF
   SET dm2_sch_file->file_prefix = build(dct_sch_date_type,cnvtalphanum(format(dct_sch_date_fmt,
      "MM/DD/YYYY;;D")))
   SET dct_data_to_move = cnvtupper(trim( $2))
   IF ( NOT (dct_data_to_move IN ("ALL", "REF", "ADS")))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Data to move option ",dct_data_to_move," is invalid.")
    SET dm_err->eproc = "Data to Move Option Validation"
    SET dm_err->user_action = "Please specify ALL or REF"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   ...
   SELECT INTO "nl:"
    ...
    WHERE dtd.table_name=dtd.full_table_name
    HEAD REPORT
     dct_dtd->tbl_cnt = 0, dct_work_cnt = 0
    DETAIL
     dct_work_cnt = (dct_work_cnt+ 1)
     IF (mod(dct_work_cnt,50)=1)
      stat = alterlist(dct_dtd->tbl,(dct_work_cnt+ 49))
     ENDIF
     dct_dtd->tbl[dct_work_cnt].table_name = trim(dtd.table_name), dct_dtd->tbl[dct_work_cnt].
     reference_ind = dtd.reference_ind
    FOOT REPORT
     stat = alterlist(dct_dtd->tbl,dct_work_cnt), dct_dtd->tbl_cnt = dct_work_cnt
    WITH nocounter
   ;end select
   SET dm_err->eproc = "Loading up Cerner documented tables."
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (dct_data_to_move="REF")
    SET dm_err->eproc = "Load custom ref tablespace."
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm_info d
     WHERE d.info_domain="DM2_CUST_REF_TSPACE"
     HEAD REPORT
      dct_cust_ref->tbl_cnt = 0
     DETAIL
      dct_cust_ref->tbl_cnt = (dct_cust_ref->tbl_cnt+ 1), stat = alterlist(dct_cust_ref->tbl,
       dct_cust_ref->tbl_cnt), dct_cust_ref->tbl[dct_cust_ref->tbl_cnt].table_name = trim(d.info_name
       )
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dct_load_source_objects(null)
   DECLARE dctlso_locval = i4 WITH protect, noconstant(0)
   DECLARE dctlso_locval2 = i4 WITH protect, noconstant(0)
   DECLARE dctlso_locidx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Loading space usage information for SOURCE Tables..."
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((dm_err->debug_flag > 2))
    CALL echo(build("drr_clin_copy_data->process = ",drr_clin_copy_data->process))
    CALL echo(build("drr_preserved_tables_data->cnt = ",drr_preserved_tables_data->cnt))
   ENDIF
   SELECT INTO "nl:"
    FROM dm2_user_tables dut,
     user_segments us
    WHERE dut.table_name=us.segment_name
     AND us.segment_type IN ("TABLE", "TABLE PARTITION", "TABLE SUBPARTITION")
    ORDER BY dut.table_name
    HEAD REPORT
     dct_obj->object_cnt = 0, dct_obj->extent_adj_ind = 0, dct_work_cnt = 0,
     preserve_tbl = 0
    HEAD dut.table_name
     dctlso_locval = 0, dctlso_locval = locateval(dctlso_locval,1,dct_dtd->tbl_cnt,trim(dut
       .table_name),dct_dtd->tbl[dctlso_locval].table_name)
    DETAIL
     preserve_tbl = 0
     IF ((drr_clin_copy_data->process="REFRESH")
      AND (drr_preserved_tables_data->cnt > 0)
      AND locateval(preserve_tbl,1,drr_preserved_tables_data->cnt,dut.table_name,
      drr_preserved_tables_data->tbl[preserve_tbl].table_name)=0)
      IF ((dm_err->debug_flag > 2))
       CALL echo(concat("Skip table = ",dut.table_name))
      ENDIF
     ELSE
      IF ((dm_err->debug_flag > 2))
       IF ((drr_clin_copy_data->process="REFRESH")
        AND (drr_preserved_tables_data->cnt > 0))
        CALL echo(concat("Preserve table = ",dut.table_name))
       ENDIF
      ENDIF
      dct_work_cnt = (dct_work_cnt+ 1)
      IF (mod(dct_work_cnt,50)=1)
       stat = alterlist(dct_obj->object,(dct_work_cnt+ 49))
      ENDIF
      dct_obj->object[dct_work_cnt].object_type = "TABLE", dct_obj->object[dct_work_cnt].object_name
       = trim(dut.table_name), dct_obj->object[dct_work_cnt].table_name = trim(dut.table_name),
      dct_obj->object[dct_work_cnt].object_tablespace = trim(us.tablespace_name), dct_obj->object[
      dct_work_cnt].object_size_allocated = us.bytes, dct_obj->object[dct_work_cnt].
      object_size_adjusted = us.bytes
      IF (us.segment_type="TABLE")
       dct_obj->object[dct_work_cnt].init_ext = dut.initial_extent, dct_obj->object[dct_work_cnt].
       next_ext = dut.next_extent
      ELSE
       dct_obj->object[dct_work_cnt].init_ext = us.initial_extent, dct_obj->object[dct_work_cnt].
       next_ext = us.next_extent
      ENDIF
      dct_obj->object[dct_work_cnt].new_init_ext = 0, dct_obj->object[dct_work_cnt].new_next_ext = 0
      IF (dctlso_locval > 0)
       dct_obj->object[dct_work_cnt].object_documented = 1, dct_obj->object[dct_work_cnt].
       reference_ind = dct_dtd->tbl[dctlso_locval].reference_ind
      ELSE
       dct_obj->object[dct_work_cnt].object_documented = 0, dct_obj->object[dct_work_cnt].
       reference_ind = 0
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   SET dm_err->eproc = "Gathering segment information for Source tables."
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SELECT INTO "nl:"
    FROM dm2_user_tables dut,
     user_segments us
    WHERE dut.cluster_name=us.segment_name
     AND dut.cluster_name IS NOT null
     AND us.segment_type="CLUSTER"
    ORDER BY dut.table_name
    HEAD REPORT
     dctlso_locidx = 0
    HEAD dut.cluster_name
     dctlso_locidx = locateval(dctlso_locidx,1,dct_work_cnt,dut.table_name,dct_obj->object[
      dctlso_locidx].object_name)
     IF (dctlso_locidx=0)
      dct_work_cnt = (dct_work_cnt+ 1)
      IF (mod(dct_work_cnt,50)=1)
       stat = alterlist(dct_obj->object,(dct_work_cnt+ 49))
      ...
      object_size_adjusted = us.bytes,
      dct_obj->object[dct_work_cnt].init_ext = dut.initial_extent, dct_obj->object[dct_work_cnt].
      next_ext = dut.next_extent, dct_obj->object[dct_work_cnt].new_init_ext = 0,
      dct_obj->object[dct_work_cnt].new_next_ext = 0, dctlso_locval = 0, dctlso_locval = locateval(
       dctlso_locval,1,dct_dtd->tbl_cnt,dut.table_name,dct_dtd->tbl[dctlso_locval].table_name)
      IF (dctlso_locval > 0)
       dct_obj->object[dct_work_cnt].object_documented = 1, dct_obj->object[dct_work_cnt].
       reference_ind = dct_dtd->tbl[dctlso_locval].reference_ind
      ELSE
    ...
    WITH nocounter
   ;end select
   SET dm_err->eproc = "Gathering segment information for Source Cluster tables."
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((drr_clin_copy_data->process != "REFRESH"))
    SET dm_err->eproc = "Loading space usage information for SOURCE Indexes..."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm2_user_indexes dui,
      user_segments us
     WHERE dui.index_name=us.segment_name
      AND us.segment_type IN ("INDEX", "CLUSTER", "INDEX PARTITION", "INDEX SUBPARTITION")
     ORDER BY dui.table_name, dui.index_name
     HEAD dui.table_name
      dctlso_locval = 0, dctlso_locval = locateval(dctlso_locval,1,dct_dtd->tbl_cnt,trim(dui
        .table_name),dct_dtd->tbl[dctlso_locval].table_name)
     DETAIL
      dct_work_cnt = (dct_work_cnt+ 1)
      IF (mod(dct_work_cnt,50)=1)
       stat = alterlist(dct_obj->object,(dct_work_cnt+ 49))
      ENDIF
      dct_obj->object[dct_work_cnt].object_type = "INDEX", dct_obj->object[dct_work_cnt].object_name
       = trim(dui.index_name), dct_obj->object[dct_work_cnt].table_name = trim(dui.table_name),
      dct_obj->object[dct_work_cnt].object_tablespace = trim(us.tablespace_name), dct_obj->object[
      dct_work_cnt].object_size_allocated = us.bytes, dct_obj->object[dct_work_cnt].
      object_size_adjusted = us.bytes,
      dct_obj->object[dct_work_cnt].init_ext = dui.initial_extent, dct_obj->object[dct_work_cnt].
      next_ext = dui.next_extent, dct_obj->object[dct_work_cnt].new_init_ext = 0,
      dct_obj->object[dct_work_cnt].new_next_ext = 0
      IF (dctlso_locval > 0)
       dct_obj->object[dct_work_cnt].object_documented = 1, dct_obj->object[dct_work_cnt].
       reference_ind = dct_dtd->tbl[dctlso_locval].reference_ind
      ELSE
     ...
     WITH nocounter
    ;end select
    SET dm_err->eproc = "Gathering segment information for Source indexes."
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->eproc = "Loading space usage information for SOURCE Lobs..."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM user_lobs ul,
     user_segments us1
    WHERE ((ul.segment_name=us1.segment_name) OR (ul.index_name=us1.segment_name))
     AND us1.segment_type IN ("LOBSEGMENT", "LOB PARTITION", "LOBINDEX", "INDEX PARTITION",
    "LOB SUBPARTITION",
    "INDEX SUBPARTITION")
    ORDER BY ul.table_name
    HEAD REPORT
     preserve_tbl = 0, skip = 0
    HEAD ul.table_name
     dctlso_locval = 0, dctlso_locval = locateval(dctlso_locval,1,dct_dtd->tbl_cnt,trim(ul.table_name
       ),dct_dtd->tbl[dctlso_locval].table_name), preserve_tbl = 0,
     skip = 0
     IF ((drr_clin_copy_data->process="REFRESH")
      AND (drr_preserved_tables_data->cnt > 0)
      AND locateval(preserve_tbl,1,drr_preserved_tables_data->cnt,ul.table_name,
      drr_preserved_tables_data->tbl[preserve_tbl].table_name)=0)
      skip = 1
      IF ((dm_err->debug_flag > 2))
       CALL echo(concat("Skip table = ",ul.table_name))
      ENDIF
     ELSE
      IF ((dm_err->debug_flag > 2))
       IF ((drr_clin_copy_data->process="REFRESH")
        AND (drr_preserved_tables_data->cnt > 0))
        CALL echo(concat("Preserve table = ",ul.table_name))
       ENDIF
      ENDIF
     ENDIF
    DETAIL
     IF (skip=0)
      ...
      IF (mod(dct_work_cnt,50)=1)
       stat = alterlist(dct_obj->object,(dct_work_cnt+ 49))
      ENDIF
      dct_obj->object[dct_work_cnt].object_type = trim(us1.segment_type), dct_obj->object[
      dct_work_cnt].object_name = trim(us1.segment_name), dct_obj->object[dct_work_cnt].table_name =
      trim(ul.table_name),
      dct_obj->object[dct_work_cnt].object_tablespace = trim(us1.tablespace_name), dct_obj->object[
      dct_work_cnt].object_size_allocated = us1.bytes, dct_obj->object[dct_work_cnt].
      object_size_adjusted = us1.bytes,
      dct_obj->object[dct_work_cnt].new_init_ext = 0, dct_obj->object[dct_work_cnt].new_next_ext = 0
      IF (dctlso_locval > 0)
       dct_obj->object[dct_work_cnt].object_documented = 1, dct_obj->object[dct_work_cnt].
       reference_ind = dct_dtd->tbl[dctlso_locval].reference_ind
    ...
    WITH nocounter
   ;end select
   SET dm_err->eproc = "Gathering segment information for Source Lobs."
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET stat = alterlist(dct_obj->object,dct_work_cnt)
   SET dct_obj->object_cnt = dct_work_cnt
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dct_adjust_object_allocations(dctaoa_opt)
   DECLARE dctaoa_1m = f8 WITH protect, constant(1048576.0)
   DECLARE dctaoa_160k = f8 WITH protect, constant(163840.0)
   DECLARE dctaoa_tmp_init_ext = f8 WITH protect, noconstant(0.00)
   DECLARE dctaoa_tmp_next_ext = f8 WITH protect, noconstant(0.00)
   DECLARE dctaoa_tmp_new_init_ext = f8 WITH protect, noconstant(0.00)
   DECLARE dctaoa_tmp_new_next_ext = f8 WITH protect, noconstant(0.00)
   DECLARE dctaoa_ndx = i4 WITH protect, noconstant(0)
   DECLARE dctaoa_tbl = i4 WITH protect, noconstant(0)
   DECLARE dctaoa_mix_pct = f8 WITH protect, noconstant(0.0)
   DECLARE dctaoa_refmixed_adj = f8 WITH protect, noconstant(0.0)
   DECLARE dctaoa_driver_idx = i4 WITH protect, noconstant(0)
   DECLARE dctaoa_driver_pct = f8 WITH protect, noconstant(0.0)
   DECLARE dctaoa_driver_cnt = i4 WITH protect, noconstant(0)
   DECLARE dctaoa_dae_cnt = i4 WITH protect, noconstant(0)
   DECLARE dctaoa_cust_ref = i4 WITH protect, noconstant(0)
   IF ((drr_clin_copy_data->ads_chosen_ind=1))
    FREE RECORD dct_dae_data
    RECORD dct_dae_data(
      1 tbl_cnt = i4
      1 qual[*]
        2 table_name = vc
    ...
    FREE RECORD dct_dae_driver
    RECORD dct_dae_driver(
      1 config_pct = f8
      1 tbl_cnt = i4
      1 qual[*]
        2 table_name = vc
    ...
    SET dm_err->eproc = "Load DM_ADS_CONFIG config_pct."
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm_ads_config d
     WHERE (d.dm_ads_config_id=drr_clin_copy_data->ads_config_id)
     DETAIL
      dct_dae_driver->config_pct = (d.sample_percent_nbr/ 100)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Load Driver table key count."
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm_ads_config_extract dace,
      dm_ads_config_driver dacd,
      dm_ads_extract dae
     PLAN (dace
      WHERE (dace.dm_ads_config_id=drr_clin_copy_data->ads_config_id))
      JOIN (dacd
      WHERE dace.dm_ads_config_id=dacd.dm_ads_config_id
       AND dace.dm_ads_extract_id=dacd.dm_ads_extract_id)
      JOIN (dae
     ...
     HEAD REPORT
      key_cnt = 0
     HEAD dae.table_name
      dct_dae_driver->tbl_cnt = (dct_dae_driver->tbl_cnt+ 1), stat = alterlist(dct_dae_driver->qual,
       dct_dae_driver->tbl_cnt), dct_dae_driver->qual[dct_dae_driver->tbl_cnt].table_name = dae
      .table_name,
      dct_dae_driver->qual[dct_dae_driver->tbl_cnt].src_num_rows = - (1), key_cnt = 0
     DETAIL
      key_cnt = (key_cnt+ 1)
     FOOT  dae.table_name
      dct_dae_driver->qual[dct_dae_driver->tbl_cnt].dacd_key_cnt = key_cnt
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
    FOR (dctaoa_driver_cnt = 1 TO dct_dae_driver->tbl_cnt)
      SET dm2scramble->in_text = dct_dae_driver->qual[dctaoa_driver_cnt].table_name
      IF (ds_scramble(null)=0)
       RETURN(0)
      ENDIF
      IF ((dm_err->debug_flag > 0))
       CALL echo(concat("Unscrambled:",dct_dae_driver->qual[dctaoa_driver_cnt].table_name))
       CALL echo(concat("Scrambled:",dm2scramble->out_text))
      ENDIF
      SET dct_dae_driver->qual[dctaoa_driver_cnt].table_name = dm2scramble->out_text
    ENDFOR
    SET dm_err->eproc = "Load Source tables size."
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     ui.table_name, maxindnumrows = max(ui.num_rows)
     FROM dm_user_indexes_actual_stats ui,
      (dummyt d  WITH seq = value(dct_dae_driver->tbl_cnt))
     PLAN (d)
      JOIN (ui
      WHERE (ui.table_name=dct_dae_driver->qual[d.seq].table_name)
     ...
     HEAD REPORT
      tbl_idx = 0
     FOOT  ui.table_name
      tbl_idx = 0, tbl_idx = locateval(tbl_idx,1,dct_dae_driver->tbl_cnt,ui.table_name,dct_dae_driver
       ->qual[tbl_idx].table_name), dct_dae_driver->qual[tbl_idx].src_num_rows = maxindnumrows
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
    FOR (dctaoa_tbl = 1 TO dct_dae_driver->tbl_cnt)
      IF ( NOT ((dct_dae_driver->qual[dctaoa_tbl].src_num_rows IN (- (1), 0)))
       AND (dct_dae_driver->qual[dctaoa_tbl].dacd_key_cnt < dct_dae_driver->qual[dctaoa_tbl].
      src_num_rows))
       SET dct_dae_driver->qual[dctaoa_tbl].actual_pct = (dct_dae_driver->qual[dctaoa_tbl].
       dacd_key_cnt/ dct_dae_driver->qual[dctaoa_tbl].src_num_rows)
      ELSE
       SET dct_dae_driver->qual[dctaoa_tbl].actual_pct = dct_dae_driver->config_pct
      ENDIF
    ENDFOR
    IF ((dm_err->debug_flag > 5))
     CALL echorecord(dct_dae_driver)
    ENDIF
    SET dm_err->eproc =
    "Classify the tablespace estimating behavior by table according to data_class_type and extract_method."
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm_ads_extract dae
     ORDER BY dae.table_name
     HEAD dae.table_name
      dct_dae_data->tbl_cnt = (dct_dae_data->tbl_cnt+ 1), stat = alterlist(dct_dae_data->qual,
       dct_dae_data->tbl_cnt), dct_dae_data->qual[dct_dae_data->tbl_cnt].table_name = dae.table_name,
      dct_dae_data->qual[dct_dae_data->tbl_cnt].driver_table_name = dae.driver_table_name
     DETAIL
      IF (dae.extract_method="ALL")
       dct_dae_data->qual[dct_dae_data->tbl_cnt].all_ind = 1
      ELSEIF (dae.extract_method="NOMOVE")
       dct_dae_data->qual[dct_dae_data->tbl_cnt].nomove_ind = 1
      ELSEIF (dae.data_class_type="REFERENCE-MIXED")
       dct_dae_data->qual[dct_dae_data->tbl_cnt].refmixed_ind = 1
      ELSE
       dct_dae_data->qual[dct_dae_data->tbl_cnt].pct_ind = 1
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     ...
     RETURN(0)
    ENDIF
    FOR (dctaoa_dae_cnt = 1 TO dct_dae_data->tbl_cnt)
      SET dm2scramble->in_text = dct_dae_data->qual[dctaoa_dae_cnt].table_name
      IF (ds_scramble(null)=0)
       RETURN(0)
      ENDIF
      IF ((dm_err->debug_flag > 0))
       CALL echo(concat("Unscrambled:",dct_dae_data->qual[dctaoa_dae_cnt].table_name))
       CALL echo(concat("Scrambled:",dm2scramble->out_text))
      ENDIF
      SET dct_dae_data->qual[dctaoa_dae_cnt].table_name = dm2scramble->out_text
      SET dm2scramble->out_text = ""
      SET dm2scramble->in_text = dct_dae_data->qual[dctaoa_dae_cnt].driver_table_name
      IF (ds_scramble(null)=0)
       RETURN(0)
      ENDIF
      IF ((dm_err->debug_flag > 0))
       CALL echo(concat("Unscrambled:",dct_dae_data->qual[dctaoa_dae_cnt].driver_table_name))
       CALL echo(concat("Scrambled:",dm2scramble->out_text))
      ENDIF
      SET dct_dae_data->qual[dctaoa_dae_cnt].driver_table_name = dm2scramble->out_text
    ENDFOR
    IF ((dm_err->debug_flag > 5))
     CALL echorecord(dct_dae_data)
     CALL echorecord(dct_dae_driver)
    ENDIF
   ENDIF
   FOR (dct_work_cnt = 1 TO dct_obj->object_cnt)
     IF ((dm_err->debug_flag >= 5))
      CALL echo(concat("Adjusting object allocations for object",dct_obj->object[dct_work_cnt].
        object_name))
     ENDIF
     SET dctaoa_tmp_init_ext = dct_obj->object[dct_work_cnt].init_ext
     SET dctaoa_tmp_next_ext = dct_obj->object[dct_work_cnt].next_ext
     IF ((dm_err->debug_flag >= 5))
      CALL echo(build("before::",dctaoa_tmp_init_ext,"::",dctaoa_tmp_next_ext))
     ENDIF
     SET dctaoa_ndx = 0
     SET dctaoa_ndx = locateval(dctaoa_ndx,1,value(drr_mixed_tables_data->cnt),dct_obj->object[
      dct_work_cnt].table_name,drr_mixed_tables_data->tbl[dctaoa_ndx].table_name)
     IF (dctaoa_ndx > 0)
      IF ((dm_err->debug_flag >= 5))
       CALL echo(concat(dct_obj->object[dct_work_cnt].table_name," is a mixed table."))
      ENDIF
      CALL dm2_adj_init_next_ext(dctaoa_opt,2,dct_obj->object[dct_work_cnt].table_name,
       dctaoa_tmp_init_ext,dctaoa_tmp_next_ext)
     ELSE
      CALL dm2_adj_init_next_ext(evaluate(dctaoa_opt,"ADS","ALL",dctaoa_opt),evaluate(dct_obj->
        object[dct_work_cnt].object_documented,0,1,dct_obj->object[dct_work_cnt].reference_ind),
       dct_obj->object[dct_work_cnt].table_name,dctaoa_tmp_init_ext,dctaoa_tmp_next_ext)
     ENDIF
     SET dct_obj->object[dct_work_cnt].init_ext = dctaoa_tmp_init_ext
     SET dct_obj->object[dct_work_cnt].next_ext = dctaoa_tmp_next_ext
     IF ((dm_err->debug_flag >= 5))
      CALL echo(build("after::",dctaoa_tmp_init_ext,"::",dctaoa_tmp_next_ext))
     ENDIF
     IF (dctaoa_opt="REF")
      IF ((dct_obj->object[dct_work_cnt].object_documented=1)
       AND (dct_obj->object[dct_work_cnt].reference_ind=0)
       AND dctaoa_ndx=0
       AND locateval(dctaoa_cust_ref,1,dct_cust_ref->tbl_cnt,dct_obj->object[dct_work_cnt].table_name,
       dct_cust_ref->tbl[dctaoa_cust_ref].table_name)=0)
       IF ((dct_obj->object[dct_work_cnt].object_size_allocated > dctaoa_1m))
        SET dct_obj->object[dct_work_cnt].object_size_adjusted = dctaoa_1m
       ELSEIF ((dct_obj->object[dct_work_cnt].object_size_allocated < dctaoa_160k))
        SET dct_obj->object[dct_work_cnt].object_size_adjusted = dctaoa_160k
       ENDIF
       IF ((dm_err->debug_flag >= 5))
        CALL echo(build("documented object w refind = 0. object_size allocated, adjusted::",dct_obj->
          object[dct_work_cnt].object_size_allocated,"::",dct_obj->object[dct_work_cnt].
          object_size_adjusted))
       ENDIF
      ELSE
       IF (dctaoa_ndx > 0
        AND (drr_mixed_tables_data->tbl[dctaoa_ndx].ref_num_rows_set_ind=1))
        IF ((drr_mixed_tables_data->tbl[dctaoa_ndx].ref_num_rows=0))
         SET dct_obj->object[dct_work_cnt].object_size_adjusted = dctaoa_160k
         IF ((dct_obj->object[dct_work_cnt].init_ext > 0)
          AND (dct_obj->object[dct_work_cnt].object_type != "LOB*"))
          SET dct_obj->object[dct_work_cnt].new_init_ext = dctaoa_160k
          SET dct_obj->object[dct_work_cnt].new_next_ext = dctaoa_160k
          SET dct_obj->extent_adj_ind = 1
         ENDIF
        ELSEIF ((drr_mixed_tables_data->tbl[dctaoa_ndx].num_rows > drr_mixed_tables_data->tbl[
        dctaoa_ndx].ref_num_rows))
         SET dct_size_tmp = ((dct_obj->object[dct_work_cnt].object_size_adjusted - dct_obj->object[
         dct_work_cnt].init_ext) * (drr_mixed_tables_data->tbl[dctaoa_ndx].ref_num_rows/
         drr_mixed_tables_data->tbl[dctaoa_ndx].num_rows))
         IF ((dct_obj->object[dct_work_cnt].init_ext > 0)
          AND (dct_obj->object[dct_work_cnt].object_type != "LOB*"))
          SET dct_obj->object[dct_work_cnt].new_init_ext = ((dct_obj->object[dct_work_cnt].
          object_size_adjusted - dct_obj->object[dct_work_cnt].init_ext) * (drr_mixed_tables_data->
          tbl[dctaoa_ndx].ref_num_rows/ drr_mixed_tables_data->tbl[dctaoa_ndx].num_rows))
          SET dct_obj->object[dct_work_cnt].new_init_ext = dm2_adj_size(dct_obj->object[dct_work_cnt]
           .new_init_ext,dctaoa_160k)
          IF ((dct_obj->object[dct_work_cnt].new_init_ext < dctaoa_160k))
           SET dct_obj->object[dct_work_cnt].new_init_ext = dctaoa_160k
          ENDIF
          SET dct_obj->object[dct_work_cnt].new_next_ext = ((dct_obj->object[dct_work_cnt].
          object_size_adjusted - dct_obj->object[dct_work_cnt].init_ext) * (drr_mixed_tables_data->
          tbl[dctaoa_ndx].ref_num_rows/ drr_mixed_tables_data->tbl[dctaoa_ndx].num_rows))
          SET dct_obj->object[dct_work_cnt].new_next_ext = dm2_adj_size(dct_obj->object[dct_work_cnt]
           .new_next_ext,dctaoa_160k)
          IF ((dct_obj->object[dct_work_cnt].new_next_ext < dctaoa_160k))
           SET dct_obj->object[dct_work_cnt].new_next_ext = dctaoa_160k
          ENDIF
          SET dct_obj->extent_adj_ind = 1
         ENDIF
         IF ((dm_err->debug_flag > 0))
          CALL echo(build("SIZE ADJUSTED:",dct_obj->object[dct_work_cnt].object_size_adjusted))
          CALL echo(build("INIT_EXT:",dct_obj->object[dct_work_cnt].init_ext))
          CALL echo(build("REF_NUM_ROWS:",drr_mixed_tables_data->tbl[dctaoa_ndx].ref_num_rows))
          CALL echo(build("NUM_ROWS:",drr_mixed_tables_data->tbl[dctaoa_ndx].num_rows))
          CALL echo(build("SIZE_TMP:",dct_size_tmp))
          CALL echo(build("NEW_INIT_EXT:",dct_obj->object[dct_work_cnt].new_init_ext))
          CALL echo(build("NEW_NEXT_EXT:",dct_obj->object[dct_work_cnt].new_next_ext))
         ENDIF
         IF (dct_size_tmp < dctaoa_160k)
          SET dct_obj->object[dct_work_cnt].object_size_adjusted = dctaoa_160k
         ELSE
          SET dct_obj->object[dct_work_cnt].object_size_adjusted = (dct_obj->object[dct_work_cnt].
          new_init_ext+ dm2_adj_size(dct_size_tmp,dct_obj->object[dct_work_cnt].new_next_ext))
         ENDIF
        ELSE
         SET dct_size_tmp = dm2_adj_size((dct_obj->object[dct_work_cnt].object_size_adjusted -
          dct_obj->object[dct_work_cnt].init_ext),dct_obj->object[dct_work_cnt].next_ext)
         SET dct_obj->object[dct_work_cnt].object_size_adjusted = (dct_obj->object[dct_work_cnt].
         init_ext+ dct_size_tmp)
         IF ((dm_err->debug_flag > 0))
          CALL echo(build("SIZE ADJUSTED:",dct_obj->object[dct_work_cnt].object_size_adjusted))
          CALL echo(build("INIT_EXT:",dct_obj->object[dct_work_cnt].init_ext))
          CALL echo(build("REF_NUM_ROWS:",drr_mixed_tables_data->tbl[dctaoa_ndx].ref_num_rows))
        ...
        ENDIF
       ELSE
        SET dct_size_tmp = dm2_adj_size((dct_obj->object[dct_work_cnt].object_size_adjusted - dct_obj
         ->object[dct_work_cnt].init_ext),dct_obj->object[dct_work_cnt].next_ext)
        SET dct_obj->object[dct_work_cnt].object_size_adjusted = (dct_obj->object[dct_work_cnt].
        init_ext+ dct_size_tmp)
        IF ((dm_err->debug_flag >= 5))
         CALL echo(build(
           "nondocumented or reference object. refind, object_size allocated, adjusted::",dct_obj->
           object[dct_work_cnt].reference_ind,"::",dct_obj->object[dct_work_cnt].
           object_size_allocated,"::",
           dct_obj->object[dct_work_cnt].object_size_adjusted))
        ENDIF
       ENDIF
      ENDIF
     ELSEIF (dctaoa_opt="ALL")
      SET dct_size_tmp = dm2_adj_size((dct_obj->object[dct_work_cnt].object_size_adjusted - dct_obj->
       object[dct_work_cnt].init_ext),dct_obj->object[dct_work_cnt].next_ext)
      SET dct_obj->object[dct_work_cnt].object_size_adjusted = (dct_obj->object[dct_work_cnt].
      init_ext+ dct_size_tmp)
      IF ((dm_err->debug_flag >= 5))
       CALL echo(build("option was ALL.  object_size allocated, adjusted::",dct_obj->object[
         dct_work_cnt].object_size_allocated,"::",dct_obj->object[dct_work_cnt].object_size_adjusted)
        )
      ENDIF
     ELSEIF (dctaoa_opt="ADS")
      SET dct_size_tmp = 0
      SET dctaoa_refmixed_adj = 1.0
      SET dctaoa_mix_pct = 0.0
      SET dctaoa_driver_pct = 0.0
      SET dctaoa_tbl = 0
      SET dctaoa_tbl = locateval(dctaoa_tbl,1,dct_dae_data->tbl_cnt,dct_obj->object[dct_work_cnt].
       table_name,dct_dae_data->qual[dctaoa_tbl].table_name)
      IF (dctaoa_tbl > 0)
       IF ((dct_dae_data->qual[dctaoa_tbl].refmixed_ind=1))
        IF (dctaoa_ndx > 0
         AND (drr_mixed_tables_data->tbl[dctaoa_ndx].ref_num_rows_set_ind=1))
         IF ((drr_mixed_tables_data->tbl[dctaoa_ndx].num_rows > drr_mixed_tables_data->tbl[dctaoa_ndx
         ].ref_num_rows))
          SET dctaoa_mix_pct = (drr_mixed_tables_data->tbl[dctaoa_ndx].ref_num_rows/
          drr_mixed_tables_data->tbl[dctaoa_ndx].num_rows)
         ENDIF
         IF ((dm_err->debug_flag >= 5))
          CALL echo(concat("Table ",drr_mixed_tables_data->tbl[dctaoa_ndx].table_name,
            " is MIXED table."))
          CALL echo(build("ref_num_rows = ",drr_mixed_tables_data->tbl[dctaoa_ndx].ref_num_rows))
          CALL echo(build("num_rows = ",drr_mixed_tables_data->tbl[dctaoa_ndx].num_rows))
          CALL echo(build("dctaoa_mix_pct = ",dctaoa_mix_pct))
         ENDIF
         IF (dctaoa_mix_pct > 0.0)
          SET dct_size_tmp = (dct_size_tmp+ (dct_obj->object[dct_work_cnt].object_size_adjusted *
          dctaoa_mix_pct))
         ENDIF
         SET dctaoa_refmixed_adj = (1 - dctaoa_mix_pct)
        ENDIF
       ENDIF
       IF ((dct_dae_data->qual[dctaoa_tbl].all_ind=1))
        SET dct_size_tmp = (dct_size_tmp+ dct_obj->object[dct_work_cnt].object_size_adjusted)
       ELSEIF ((dct_dae_data->qual[dctaoa_tbl].pct_ind=1))
        SET dctaoa_driver_idx = 0
        SET dctaoa_driver_idx = locateval(dctaoa_driver_idx,1,dct_dae_driver->tbl_cnt,dct_dae_data->
         qual[dctaoa_tbl].driver_table_name,dct_dae_driver->qual[dctaoa_driver_idx].table_name)
        IF (dctaoa_driver_idx > 0)
         SET dctaoa_driver_pct = dct_dae_driver->qual[dctaoa_driver_idx].actual_pct
        ELSE
         SET dctaoa_driver_pct = dct_dae_driver->config_pct
        ENDIF
        SET dct_size_tmp = (dct_size_tmp+ ((dct_obj->object[dct_work_cnt].object_size_adjusted *
        dctaoa_refmixed_adj) * dctaoa_driver_pct))
       ELSE
        IF ((dm_err->debug_flag > 0))
         CALL echo("Space Needs ignored for...")
         CALL echo(build("TABLE:",dct_obj->object[dct_work_cnt].table_name))
         CALL echo(build("SEGMENT_TYPE:",dct_obj->object[dct_work_cnt].object_type))
         CALL echo(build("SEGMENT_NAME:",dct_obj->object[dct_work_cnt].object_name))
        ENDIF
       ENDIF
      ELSE
       SET dct_size_tmp = (dct_size_tmp+ dct_obj->object[dct_work_cnt].object_size_adjusted)
      ENDIF
      IF ((dct_obj->object[dct_work_cnt].init_ext > dct_size_tmp))
       SET dct_size_tmp = dct_obj->object[dct_work_cnt].init_ext
      ENDIF
      SET dct_obj->object[dct_work_cnt].object_size_adjusted = dm2_adj_size(dct_size_tmp,dct_obj->
       object[dct_work_cnt].next_ext)
     ENDIF
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dct_rollup_to_target(null)
   DECLARE dctrtt_idx = i4 WITH protect, noconstant(0)
   DECLARE dctrtt_locval = i4 WITH protect, noconstant(0)
   DECLARE dctrtt_locval2 = i4 WITH protect, noconstant(0)
   IF ((dm2_install_schema->percent_tspace=0))
    SET dm_err->eproc = "Percent Growth not valued, defaulting to 10%..."
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dm2_install_schema->percent_tspace = 10
   ENDIF
   SET dm_err->eproc = "Rolling Segment information to Tablespace Level..."
   CALL disp_msg(" ",dm_err->logfile,0)
   FOR (dctrtt_idx = 1 TO dct_obj->object_cnt)
     IF (((dctrtt_idx=1) OR ((dct_obj->object[dctrtt_idx].object_tablespace != dct_obj->object[(
     dctrtt_idx - 1)].object_tablespace))) )
      SET dctrtt_locval = 0
      SET dctrtt_locval = locateval(dctrtt_locval,1,dct_tspace->tspace_count,dct_obj->object[
       dctrtt_idx].object_tablespace,dct_tspace->tspace[dctrtt_locval].tspace_name)
      IF (dctrtt_locval=0)
       SET dct_tspace->tspace_count = (dct_tspace->tspace_count+ 1)
       SET stat = alterlist(dct_tspace->tspace,dct_tspace->tspace_count)
       SET dctrtt_locval = dct_tspace->tspace_count
       SET dct_tspace->tspace[dctrtt_locval].tspace_name = dct_obj->object[dctrtt_idx].
       object_tablespace
       SET dct_tspace->tspace[dctrtt_locval].bytes = dct_obj->object[dctrtt_idx].object_size_adjusted
       SET dctrtt_locval2 = 0
       SET dctrtt_locval2 = locateval(dctrtt_locval2,1,ddtsp->tsp_cnt,dct_obj->object[dctrtt_idx].
        object_tablespace,ddtsp->qual[dctrtt_locval2].tspace_name)
       IF (dctrtt_locval2=0)
        SET dm_err->emsg = concat("Unable to determine Extent Management for tablespace ",dct_obj->
         object[dctrtt_idx].object_tablespace)
        SET dm_err->err_ind = 1
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        RETURN(0)
       ELSE
        SET dct_tspace->tspace[dctrtt_locval].tspace_type = ddtsp->qual[dctrtt_locval2].ext_mgmt
       ENDIF
      ELSE
       SET dct_tspace->tspace[dctrtt_locval].bytes = (dct_tspace->tspace[dctrtt_locval].bytes+
       dct_obj->object[dctrtt_idx].object_size_adjusted)
      ENDIF
     ELSE
      SET dct_tspace->tspace[dctrtt_locval].bytes = (dct_tspace->tspace[dctrtt_locval].bytes+ dct_obj
      ->object[dctrtt_idx].object_size_adjusted)
     ENDIF
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dct_val_write_privs(dct_dir,dct_full_dir)
   DECLARE dname_full = vc WITH protect
   DECLARE full_fname = vc WITH protect
   DECLARE prev_user_action = vc WITH protect
   CASE (dm2_sys_misc->cur_os)
    OF "AXP":
     SET dname_full = build(dct_dir,":")
    OF "WIN":
     SET dname_full = build(dct_full_dir,"\")
    ELSE
     SET dname_full = build(dct_full_dir,"/")
   ENDCASE
   SET prev_user_action = dm_err->user_action
   IF (get_unique_file("dm2tst2",".dat")=0)
    RETURN(0)
   ENDIF
   SET full_fname = build(dct_dir,":",cnvtlower(dm_err->unique_fname))
   SELECT INTO value(full_fname)
    d.seq
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     row + 1, "This is a test of writing to ", dct_dir
    WITH nocounter
   ;end select
   SET dm_err->user_action = concat("Please log in as a user that has full privileges to ",dct_dir)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((dm2_sys_misc->cur_os="AXP"))
    IF (dm2_push_dcl(concat("del ",dname_full,cnvtlower(dm_err->unique_fname),";"))=0)
     RETURN(0)
    ENDIF
   ELSEIF ((dm2_sys_misc->cur_os="WIN"))
    IF (dm2_push_dcl(concat("del ",dname_full,cnvtlower(dm_err->unique_fname)))=0)
    ...
    ENDIF
   ELSE
    IF (dm2_push_dcl(concat("rm ",dname_full,cnvtlower(dm_err->unique_fname)))=0)
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->user_action = prev_user_action
 END ;Subroutine
 SUBROUTINE dct_assign_ddlops_tspace(null)
   SET dm_err->eproc = "Validating default tablespaces for dm2_tables."
   IF ((dir_tools_tspaces->data_tspace="NONE"))
    SET dm_err->emsg = "Required default table tablespace for dm2_tables was not found."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((dir_tools_tspaces->index_tspace="NONE"))
    SET dm_err->emsg = "Required default index tablespace for dm2_tables was not found."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dctadt_locval = 0
   SET dctadt_locval = locateval(dctadt_locval,1,dct_tspace->tspace_count,dir_tools_tspaces->
    data_tspace,dct_tspace->tspace[dctadt_locval].tspace_name)
   IF ((dm_err->debug_flag > 2))
    CALL echo(build("Bytes adjusted before bump for t-tablespace:",dir_tools_tspaces->data_tspace,
      " is ",dct_tspace->tspace[dctadt_locval].bytes_adjusted))
   ENDIF
   IF (dctadt_locval > 0)
    SET dct_tspace->tspace[dctadt_locval].bytes_adjusted = (dct_tspace->tspace[dctadt_locval].
    bytes_adjusted+ 20971520)
   ELSE
    SET dm_err->emsg = "Unable to find default tablespace in dct_tspace master record structure."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dctadt_locval = 0
   SET dctadt_locval = locateval(dctadt_locval,1,dct_tspace->tspace_count,dir_tools_tspaces->
    index_tspace,dct_tspace->tspace[dctadt_locval].tspace_name)
   IF ((dm_err->debug_flag > 2))
    CALL echo(build("Bytes adjusted before bump for i-tablespace:",dir_tools_tspaces->index_tspace,
      " is ",dct_tspace->tspace[dctadt_locval].bytes_adjusted))
   ENDIF
   IF (dctadt_locval > 0)
    SET dct_tspace->tspace[dctadt_locval].bytes_adjusted = (dct_tspace->tspace[dctadt_locval].
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dct_debuglogs(null)
   DECLARE dctdidx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Writing debug logfiles to ccluserdir..."
   CALL disp_msg("",dm_err->logfile,0)
   IF ((dct_tspace->tspace_count > 0))
    SELECT INTO "dm2_captsp1.log"
     FROM (dummyt d  WITH seq = value(dct_tspace->tspace_count))
     HEAD REPORT
      "******************************************************", row + 1,
      "* DM2_CAPTURE_TSPACE - DCD_TSPACE RECORD STRUCTURE DUMP",
      row + 1, "* Tspace count = ",
      CALL print(build(dct_tspace->tspace_count)),
      row + 1, "******************************************************"
     DETAIL
      row + 1, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", row + 1,
      "seq = ",
      CALL print(build(d.seq)), row + 1,
      "tspace_name = ", dct_tspace->tspace[d.seq].tspace_name, row + 1,
      "bytes = ",
      CALL print(build(dct_tspace->tspace[d.seq].bytes)), row + 1,
      "bytes_adjusted = ",
      CALL print(build(dct_tspace->tspace[d.seq].bytes_adjusted)), row + 1,
      "tspace_type = ", dct_tspace->tspace[d.seq].tspace_type
     WITH nocounter, format = variable, formfeed = none,
      maxrow = 1, maxcol = 512
    ;end select
   ENDIF
   IF ((dct_obj->object_cnt > 0))
    SELECT INTO "dm2_captsp2.log"
     FROM (dummyt d  WITH seq = value(dct_obj->object_cnt))
     HEAD REPORT
      "******************************************************", row + 1,
      "* DM2_CAPTURE_TSPACE - DCD_OBJ RECORD STRUCTURE DUMP",
      row + 1, "* Object count = ",
      CALL print(build(dct_obj->object_cnt)),
      row + 1, "******************************************************"
     DETAIL
      row + 1, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", row + 1,
      "seq = ",
      CALL print(build(d.seq)), row + 1,
      "object_type = ", dct_obj->object[d.seq].object_type, row + 1,
      "object_name = ", dct_obj->object[d.seq].object_name, row + 1,
      "table_name = ", dct_obj->object[d.seq].table_name, row + 1,
      "object_size_allocated = ",
      CALL print(build(dct_obj->object[d.seq].object_size_allocated)), row + 1,
      "object_size_adjusted = ",
      CALL print(build(dct_obj->object[d.seq].object_size_adjusted)), row + 1,
      "object_tablespace = ", dct_obj->object[d.seq].object_tablespace, row + 1,
      "object_documented = ",
      CALL print(build(dct_obj->object[d.seq].object_documented)), row + 1,
      "reference_ind = ",
      CALL print(build(dct_obj->object[d.seq].reference_ind))
      IF ((dct_obj->object[d.seq].object_size_allocated != dct_obj->object[d.seq].
      object_size_adjusted))
       row + 1, "Size adjustment !!"
      ENDIF
     WITH nocounter, format = variable, formfeed = none,
      maxrow = 1, maxcol = 512
    ;end select
   ENDIF
   IF ((dm2_ind_tspace_assign->cnt > 0))
    SELECT INTO "dm2_captsp3.log"
     FROM (dummyt d  WITH seq = value(dm2_ind_tspace_assign->cnt))
     HEAD REPORT
      "******************************************************", row + 1,
      "* DM2_CAPTURE_TSPACE - dm2_ind_tspace_assign RECORD STRUCTURE DUMP",
      row + 1, "* dm2_ind_tspace_assign count = ",
      CALL print(build(dm2_ind_tspace_assign->cnt)),
      row + 1, "******************************************************"
     DETAIL
      row + 1, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", row + 1,
      "seq = ",
      CALL print(build(d.seq)), row + 1,
      "table_name = ", dm2_ind_tspace_assign->qual[d.seq].table_name, row + 1,
      "index_tspace = ", dm2_ind_tspace_assign->qual[d.seq].index_tspace, row + 1,
      "index_tspace_cnt = ",
      CALL print(build(dm2_ind_tspace_assign->qual[d.seq].index_tspace_cnt)), row + 1,
      "tspace_cnt = ",
      CALL print(build(dm2_ind_tspace_assign->qual[d.seq].tspace_cnt))
      FOR (dctdidx = 1 TO dm2_ind_tspace_assign->qual[d.seq].tspace_cnt)
        row + 1, "         seq = ",
        CALL print(build(dctdidx)),
        row + 1, "         tspace_name = ", dm2_ind_tspace_assign->qual[d.seq].tspace[dctdidx].
        tspace_name,
        row + 1, "         ind_cnt = ",
        CALL print(build(dm2_ind_tspace_assign->qual[d.seq].tspace[dctdidx].ind_cnt))
      ENDFOR
     WITH nocounter, format = variable, formfeed = none,
      maxrow = 1, maxcol = 512
    ;end select
   ENDIF
 END ;Subroutine
 SUBROUTINE dct_tablespace_sizing_adjust(null)
   DECLARE dtsa_adj_factor = f8 WITH protect, noconstant(0.00)
   DECLARE dtsa_lmt_adj_size = f8 WITH protect, noconstant(0.00)
   DECLARE dtsa_pct_bytes_adj = f8 WITH protect, noconstant(0.00)
   DECLARE dtsa_lmt_bytes_adj = f8 WITH protect, noconstant(0.00)
   DECLARE dtsa_mb_in_bytes = f8 WITH protect, constant(1048576.00)
   DECLARE dtsa_kb_in_bytes = f8 WITH protect, constant(1024.00)
   SET dtsa_adj_factor = (1+ (cnvtreal(dm2_install_schema->percent_tspace)/ 100))
   FOR (dtsa_idx = 1 TO dct_tspace->tspace_count)
     SET dtsa_pct_bytes_adj = (dct_tspace->tspace[dtsa_idx].bytes * dtsa_adj_factor)
     IF ((dm_err->debug_flag > 0))
      CALL echo("**************************************************")
      CALL echo(build("tspace_name =",dct_tspace->tspace[dtsa_idx].tspace_name))
      CALL echo(build("tspace_type =",dct_tspace->tspace[dtsa_idx].tspace_type))
      CALL echo(build("bytes       =",dct_tspace->tspace[dtsa_idx].bytes))
     ENDIF
     IF ((dct_tspace->tspace[dtsa_idx].tspace_type="L"))
      IF ((dct_tspace->tspace[dtsa_idx].bytes > 0)
       AND (dct_tspace->tspace[dtsa_idx].bytes <= dtsa_mb_in_bytes))
       SET dtsa_lmt_adj_size = (dtsa_kb_in_bytes * 64)
      ELSEIF ((dct_tspace->tspace[dtsa_idx].bytes > dtsa_mb_in_bytes)
       AND (dct_tspace->tspace[dtsa_idx].bytes <= (dtsa_mb_in_bytes * 64)))
       SET dtsa_lmt_adj_size = dtsa_mb_in_bytes
      ELSEIF ((dct_tspace->tspace[dtsa_idx].bytes > (dtsa_mb_in_bytes * 64))
       AND (dct_tspace->tspace[dtsa_idx].bytes <= (dtsa_mb_in_bytes * dtsa_kb_in_bytes)))
       SET dtsa_lmt_adj_size = (dtsa_mb_in_bytes * 8)
      ELSEIF ((dct_tspace->tspace[dtsa_idx].bytes > (dtsa_mb_in_bytes * dtsa_kb_in_bytes)))
       SET dtsa_lmt_adj_size = (dtsa_mb_in_bytes * 64)
      ENDIF
      SET dtsa_lmt_bytes_adj = (dct_tspace->tspace[dtsa_idx].bytes+ dtsa_lmt_adj_size)
      IF ((dm_err->debug_flag > 0))
       CALL echo(build("adj_factor  =",(dtsa_pct_bytes_adj - dct_tspace->tspace[dtsa_idx].bytes)))
       CALL echo(build("lmt_adjust  =",dtsa_lmt_adj_size))
      ENDIF
      IF (dtsa_pct_bytes_adj > dtsa_lmt_bytes_adj)
       SET dct_tspace->tspace[dtsa_idx].bytes_adjusted = dtsa_pct_bytes_adj
       IF ((dm_err->debug_flag > 0))
        CALL echo("Using standard adjustment.")
       ENDIF
      ELSE
       SET dct_tspace->tspace[dtsa_idx].bytes_adjusted = dtsa_lmt_bytes_adj
       IF ((dm_err->debug_flag > 0))
        CALL echo("Using LMT adjustment.")
       ENDIF
      ENDIF
     ELSE
      SET dct_tspace->tspace[dtsa_idx].bytes_adjusted = dtsa_pct_bytes_adj
     ENDIF
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("bytes_adj   =",dct_tspace->tspace[dtsa_idx].bytes_adjusted))
      CALL echo("**************************************************")
     ENDIF
   ENDFOR
 END ;Subroutine
 SUBROUTINE dct_adjust_object_extents(null)
   DECLARE daoe_tmp_init_ext = f8 WITH protect, noconstant(0.00)
   DECLARE daoe_tmp_next_ext = f8 WITH protect, noconstant(0.00)
   SET dm_err->eproc = "Open Schema Files"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((dm_err->debug_flag > 0))
    CALL echo(dm2_sch_file->dest_dir_cclfmt)
    CALL echo(dm2_sch_file->file_prefix)
   ENDIF
   IF (open_sch_files(1)=0)
    RETURN(0)
   ENDIF
   FOR (dct_work_cnt = 1 TO dct_obj->object_cnt)
     IF ((((dct_obj->object[dct_work_cnt].new_init_ext > 0)) OR ((dct_obj->object[dct_work_cnt].
     new_next_ext > 0))) )
      SET dm_err->eproc = concat("Adjusting extent sizes for object",dct_obj->object[dct_work_cnt].
       object_name)
      CALL disp_msg(" ",dm_err->logfile,0)
      IF ((dct_obj->object[dct_work_cnt].new_init_ext > 0))
       SET daoe_tmp_init_ext = dct_obj->object[dct_work_cnt].new_init_ext
      ELSE
       SET daoe_tmp_init_ext = dct_obj->object[dct_work_cnt].init_ext
      ENDIF
      IF ((dct_obj->object[dct_work_cnt].new_next_ext > 0))
       SET daoe_tmp_next_ext = dct_obj->object[dct_work_cnt].new_next_ext
      ELSE
       SET daoe_tmp_next_ext = dct_obj->object[dct_work_cnt].next_ext
      ENDIF
      IF ((dm_err->debug_flag > 0))
       CALL echo(build("current::",dct_obj->object[dct_work_cnt].init_ext,"::",dct_obj->object[
         dct_work_cnt].next_ext))
       CALL echo(build("new::",daoe_tmp_init_ext,"::",daoe_tmp_next_ext))
      ENDIF
      IF ((dct_obj->object[dct_work_cnt].init_ext > 0)
       AND (dct_obj->object[dct_work_cnt].object_type="TABLE"))
       SET dm_err->eproc = concat("Updating DMTABLE for object",dct_obj->object[dct_work_cnt].
        object_name)
       UPDATE  FROM dmtable dt
        SET dt.init_ext = daoe_tmp_init_ext, dt.next_ext = daoe_tmp_next_ext
        WHERE (dt.table_name=dct_obj->object[dct_work_cnt].object_name)
        WITH nocounter
       ;end update
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ...
      ENDIF
      IF ((dct_obj->object[dct_work_cnt].init_ext > 0)
       AND (dct_obj->object[dct_work_cnt].object_type="INDEX"))
       SET dm_err->eproc = concat("Updating DMINDEX for object",dct_obj->object[dct_work_cnt].
        object_name)
       UPDATE  FROM dmindex di
        SET di.init_ext = daoe_tmp_init_ext, di.next_ext = daoe_tmp_next_ext
        WHERE (di.table_name=dct_obj->object[dct_work_cnt].object_name)
        WITH nocounter
       ;end update
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_script
 IF ((dm_err->debug_flag > 1))
  CALL dct_debuglogs(1)
 ENDIF
 FREE RECORD dct_obj
 FREE RECORD dct_tspace
 FREE RECORD dct_dtd
 SET dm_err->eproc = "DM2_CAPTURE_TSPACE COMPLETED"
 CALL final_disp_msg(dct_logfile_prefix)
END GO
