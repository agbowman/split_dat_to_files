CREATE PROGRAM dm2_run_data_movers:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drdm_qry_cnt = i4 WITH protect, noconstant(0)
 DECLARE drdm_info_nm = vc
 DECLARE drdm_flag = i2 WITH protect, noconstant(0)
 DECLARE drdm_env_id = i4 WITH protect, noconstant(0)
 DECLARE drdm_exe_flag = i4 WITH protect, noconstant(0)
 DECLARE rdm_rt_idx = i4 WITH protect, noconstant(0)
 DECLARE drdm_planned_ind = i4
 DECLARE drdm_auto_ind = i4
 IF (validate(dm_mover_run,"VALID")="VALID")
  DECLARE dm_mover_run = vc
  SET dm_mover_run = "Automatic start via server"
 ENDIF
 CALL check_logfile("dm2_run_data_mvr",".log","DM2_RUN_DATA_MOVERS LOGFILE")
 SET dm_err->eproc = "Beginning dm2_run_data_movers"
 CALL disp_msg(" ",dm_err->logfile,0)
 FREE RECORD drdm_src
 RECORD drdm_src(
   1 src_cnt = i4
   1 cur_env_id = f8
   1 list[*]
 ...
 SET dm_err->eproc = "Populate the record structure with domain info"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="DM_ENV_ID"
  DETAIL
   drdm_env_id = di.info_number
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM dm_env_reltn der
  WHERE der.child_env_id=drdm_env_id
   AND ((der.relationship_type="REFERENCE MERGE"
   AND der.post_link_name IS NOT null
   AND der.post_link_name != " ") OR (der.relationship_type IN ("PLANNED CUTOVER", "AUTO CUTOVER")))
  ORDER BY der.relationship_type DESC
  DETAIL
   IF (der.relationship_type="REFERENCE MERGE")
    drdm_qry_cnt = (drdm_qry_cnt+ 1)
    IF (mod(drdm_qry_cnt,10)=1)
     stat = alterlist(drdm_src->list,(drdm_qry_cnt+ 9))
    ENDIF
    drdm_src->list[drdm_qry_cnt].src_id = der.parent_env_id, drdm_src->list[drdm_qry_cnt].info_domain
     = build("MERGE",cnvtstring(der.parent_env_id),cnvtstring(der.child_env_id))
   ELSEIF (der.relationship_type="PLANNED CUTOVER")
    rdm_rt_idx = 0, rdm_rt_idx = locateval(rdm_rt_idx,1,drdm_qry_cnt,der.parent_env_id,drdm_src->
     list[rdm_rt_idx].src_id)
    IF (rdm_rt_idx > 0)
     drdm_src->list[rdm_rt_idx].planned_ind = 1
    ENDIF
   ELSE
    rdm_rt_idx = 0, rdm_rt_idx = locateval(rdm_rt_idx,1,drdm_qry_cnt,der.parent_env_id,drdm_src->
     list[rdm_rt_idx].src_id)
    IF (rdm_rt_idx > 0)
     drdm_src->list[rdm_rt_idx].auto_ind = 1
    ENDIF
   ENDIF
  FOOT REPORT
   stat = alterlist(drdm_src->list,drdm_qry_cnt), drdm_src->cur_env_id = drdm_env_id, drdm_src->
   src_cnt = drdm_qry_cnt
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg = "Error from query on DM_INFO and DM_ENV_RELTN"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  ...
  DETAIL
   drdm_flag = di.info_number
  WITH nocounter
 ;end select
 IF (check_error("Check if the CUTOVER is on") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drdm_planned_ind = 0
 SET drdm_auto_ind = 0
 FOR (dlp_cnt = 1 TO drdm_src->src_cnt)
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE (di.info_domain=drdm_src->list[dlp_cnt].info_domain)
     AND di.info_name IN ("RDDS MOVERS TO RUN", "RDDS MOVERS RUNNING")
    DETAIL
     IF (di.info_name="RDDS MOVERS TO RUN")
      drdm_src->list[dlp_cnt].mover_to_run = di.info_number
     ELSE
      drdm_src->list[dlp_cnt].mover_running = di.info_number
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    ...
    GO TO exit_program
   ENDIF
   IF ((drdm_src->list[dlp_cnt].planned_ind=0)
    AND (drdm_src->list[dlp_cnt].auto_ind=0))
    SET drdm_src->list[dlp_cnt].planned_ind = 1
   ENDIF
   IF ((drdm_src->list[dlp_cnt].mover_to_run > 0)
    AND (drdm_src->list[dlp_cnt].planned_ind=1))
    SET drdm_planned_ind = (drdm_planned_ind+ 1)
   ENDIF
   IF ((drdm_src->list[dlp_cnt].auto_ind=1)
    AND (drdm_src->list[dlp_cnt].mover_to_run > 0))
    SET drdm_auto_ind = (drdm_auto_ind+ 1)
   ENDIF
 ENDFOR
 IF (((drdm_auto_ind > 1) OR (drdm_planned_ind > 1)) )
  SET dm_err->eproc = concat("Multiple sources are trying to start movers, ",
   "please stop all sources that should not currently be running movers. ")
  CALL disp_msg(" ",dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FOR (dlp_cnt = 1 TO drdm_src->src_cnt)
   IF ((drdm_src->list[dlp_cnt].mover_to_run > 0))
    IF ((drdm_src->list[dlp_cnt].mover_running < drdm_src->list[dlp_cnt].mover_to_run))
     IF ((((drdm_src->list[dlp_cnt].planned_ind=1)) OR ((drdm_src->list[dlp_cnt].auto_ind=1)
      AND drdm_planned_ind=0)) )
      SET dm_err->eproc = concat("User requested ",trim(cnvtstring(drdm_src->list[dlp_cnt].
         mover_to_run))," movers to run, ","There are currently ",trim(cnvtstring(drdm_src->list[
         dlp_cnt].mover_running)),
       " mover(s) running for domain ",cnvtstring(drdm_src->list[dlp_cnt].src_id))
      CALL disp_msg(" ",dm_err->logfile,0)
      SET dm_err->eproc = "Update RDDS MOVERS RUNNING on table DM_INFO"
      UPDATE  FROM dm_info
       SET info_number = (info_number+ 1), updt_dt_tm = sysdate, updt_cnt = (updt_cnt+ 1)
       WHERE (info_domain=drdm_src->list[dlp_cnt].info_domain)
        AND info_name="RDDS MOVERS RUNNING"
       WITH nocounter
      ;end update
      IF (curqual=0)
       SET dm_err->eproc = "Insert RDDS MOVERS RUNNING on table DM_INFO"
       INSERT  FROM dm_info
        SET info_number = 1, info_domain = drdm_src->list[dlp_cnt].info_domain, info_name =
         "RDDS MOVERS RUNNING",
      ...
      ENDIF
      COMMIT
      SET dm_err->eproc = "Starting reference data mover...."
      CALL disp_msg(" ",dm_err->logfile,0)
      IF (drdm_flag=0)
       EXECUTE dm2_ref_data_mover_main drdm_src->list[dlp_cnt].src_id, 0, 0
      ELSE
       EXECUTE dm_rmc_mover_main drdm_src->list[dlp_cnt].src_id, 0, 0
      ENDIF
      IF ((dm_err->err_ind=1))
       UPDATE  FROM dm_info
        SET info_number = (info_number - 1), updt_dt_tm = sysdate, updt_cnt = (updt_cnt+ 1)
       ...
       GO TO exit_program
      ENDIF
      SET dlp_cnt = drdm_src->src_cnt
     ELSE
      SET dm_err->eproc = concat(
       "Requested number of movers already running for another source domain")
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
    ELSE
     SET dm_err->eproc = concat("Requested number of movers already running for source domain ",
      cnvtstring(drdm_src->list[dlp_cnt].src_id))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
   ENDIF
 ENDFOR
#exit_program
 SET dm_err->eproc = "...Ending dm2_run_data_movers"
 CALL final_disp_msg("dm2_run_data_mvr")
END GO
