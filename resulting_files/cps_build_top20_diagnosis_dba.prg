CREATE PROGRAM cps_build_top20_diagnosis:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ap
   ...
   WHERE nvp.parent_entity_id=ap.app_prefs_id
    ...
    AND nvp.pvc_name="TOP20_DIAGNOSIS_COUNT")
  HEAD REPORT
 ...
 SET ierrcode = 0
 SELECT INTO "nl:"
  item_knt = count(d.diagnosis_id), d.updt_id, d.nomenclature_id
  FROM diagnosis d
  PLAN (d
   WHERE d.updt_id > 0
    AND d.nomenclature_id > 0
    AND d.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
    AND d.beg_effective_dt_tm >= cnvtdatetime(anchor->dt_tm)
    AND d.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)
    AND d.active_ind=true)
  GROUP BY d.updt_id, d.nomenclature_id
  HAVING count(d.diagnosis_id) >= low_knt
  ORDER BY d.updt_id, item_knt DESC
  HEAD REPORT
  ...
  HEAD d.updt_id
   ...
   ENDIF
   20_knt = 0, nknt = 0, hold->qual[knt].user_id = d.updt_id,
  ...
  DETAIL
   ...
   IF (20_knt <= 20)
    ...
    ENDIF
    hold->qual[knt].nomen[nknt].nomen_id = d.nomenclature_id, hold->qual[knt].nomen[nknt].item_knt =
    item_knt
   ENDIF
  FOOT  d.updt_id
 ...
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,1,diagnosis_cd)
 IF (stat > 0)
  SET failed = select_error
 ...
 FOR (i = 1 TO hold->qual_knt)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (nc
     WHERE nc.parent_entity_name="PRSNL"
      ...
      AND nc.category_type_cd=diagnosis_cd
      AND nc.category_name="TOP_20_DIAGNOSIS")
    DETAIL
   ...
   IF (curqual < 1)
    ...
    INSERT  FROM nomen_category nc
     SET nc.nomen_category_id = hold->qual[i].nomen_category_id, nc.category_name =
      "TOP_20_DIAGNOSIS", nc.category_type_cd = diagnosis_cd,
