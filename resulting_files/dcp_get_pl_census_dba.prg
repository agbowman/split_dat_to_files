CREATE PROGRAM dcp_get_pl_census:dba
 ...
 END ;Subroutine
 DECLARE rexecute = i2 WITH noconstant(0)
 ...
 IF ((criteria->locations.unit_cnt=0)
  AND (criteria->service_cnt=0))
  CALL echo("Definition Error: No location or medical service specified.")
 ELSE
  SET rexecute = executeprimaryselect(1)
  IF (rexecute > 0)
   CALL executeprimaryselect(2)
  ENDIF
  SET stat = alterlist(reply->patients,patient_cnt)
 ...
 SUBROUTINE (executeprimaryselect(pass=i4) =i2)
   ...
   IF ((criteria->time_frame_flag=notimeparameter))
    CALL parser("from ENCNTR_DOMAIN ELH")
   ELSE
    CALL parser("from ENCNTR_LOC_HIST ELH")
   ENDIF
   ...
   ENDIF
   CALL parser("plan ELH where")
   CALL parseencntrlochist(1)
   CALL parseencounterbyencntr("E.encntr_id = ELH.encntr_id",0)
   ...
   ENDIF
   IF ((criteria->vreltn_cnt != 0))
    CALL parseepr("E.encntr_id")
   ELSEIF ((criteria->lreltn_cnt != 0))
    CALL parseppr("P.person_id")
   ENDIF
   CALL parser("order by ELH.encntr_id, ELH.end_effective_dt_tm desc")
   IF (pass=1)
    CALL parser("head report")
    CALL parser("patient_cnt = 0")
   ENDIF
   CALL parser("head ELH.encntr_id")
   ...
   IF (continue=0)
    ...
    CALL parser("stat = alterlist(reply->patients, patient_cnt)")
   ENDIF
   CALL parser("with nocounter ,orahint('index(ELH XIE16ENCNTR_LOC_HIST)') go")
   RETURN(continue)
 END ;Subroutine
 SUBROUTINE (parseencntrlochist(x=i4) =null)
   IF ((criteria->locations.unit_cnt > 0))
    CALL parser(
     " expand (locX, 1, criteria->locations.unit_cnt, ELH.loc_nurse_unit_cd, criteria->locations.units[locX].unit_cd)"
     )
    IF ((criteria->locations.level=room_level))
     CALL parser(
      " and expand (locY, 1, criteria->locations.room_cnt, ELH.loc_room_cd, criteria->locations.rooms[locY].room_cd)"
      )
    ELSEIF ((criteria->locations.level=bed_level))
     CALL parser(
      " and expand (locY, 1, criteria->locations.bed_cnt, ELH.loc_bed_cd, criteria->locations.beds[locY].bed_cd)"
      )
    ENDIF
   ELSEIF ((criteria->service_cnt > 0))
    CALL parser(
     " expand (serviceX, 1, criteria->service_cnt, ELH.med_service_cd, criteria->services[serviceX].service_cd)"
     )
   ENDIF
   IF ((criteria->time_frame_flag=timerangeparameter))
    CALL parser("and ELH.end_effective_dt_tm >= cnvtdatetime(criteria->beg_effective_dt_tm)")
    CALL parser("and ELH.beg_effective_dt_tm <= cnvtdatetime(criteria->end_effective_dt_tm)")
   ELSE
    CALL parser("and ELH.end_effective_dt_tm >= cnvtdatetime(criteria->end_effective_dt_tm)")
    CALL parser("and ELH.beg_effective_dt_tm >= cnvtdatetime(criteria->beg_effective_dt_tm)")
    CALL parser("and ELH.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)")
   ENDIF
   CALL parser("and ELH.active_ind = 1")
   IF ((criteria->service_cnt > 0)
    AND (criteria->locations.unit_cnt > 0))
    CALL parser(
     "and expand (serviceX, 1, criteria->service_cnt, ELH.med_service_cd, criteria->services[serviceX].service_cd)"
     )
   ENDIF
   IF ((criteria->time_frame_flag != notimeparameter)
    AND (criteria->type_cnt > 0))
    CALL parser(
     "and expand (typeX, 1, criteria->type_cnt, ELH.encntr_type_cd+0, criteria->types[typeX].encntr_type_cd)"
     )
   ENDIF
   IF (patient_cnt > 0)
    CALL parser(
     "and not expand (continueX, 1, patient_cnt, ELH.encntr_id, reply->patients[continueX].encntr_id)"
     )
   ENDIF
 END ;Subroutine
END GO
