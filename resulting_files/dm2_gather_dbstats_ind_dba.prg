CREATE PROGRAM dm2_gather_dbstats_ind:dba
 ...
 SUBROUTINE dmai_get_cur_mod_act(dgcma_mod_out,dmgca_act_out)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dgdi_inowner = vc WITH protect, noconstant(" ")
 DECLARE dgdi_inindex = vc WITH protect, noconstant(" ")
 DECLARE dgdi_inmode = vc WITH protect, noconstant(" ")
 DECLARE dgdi_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dgdi_step_beg_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dgdi_step_end_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dgdi_cmd = vc WITH protect, noconstant(" ")
 DECLARE dgdi_export_statid = vc WITH protect, noconstant(" ")
 DECLARE dgdi_proccnt = i4 WITH protect, noconstant(0)
 DECLARE dgdi_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dgdi_emsg = vc WITH protect, noconstant(" ")
 DECLARE dgdi_wh_cnt = i4 WITH protect, noconstant(0)
 DECLARE dgdi_lock_available_ind = i2 WITH protect, noconstant(0)
 DECLARE dgdi_lock_obtained_ind = i2 WITH protect, noconstant(0)
 DECLARE dgdi_retry_ind = i2 WITH protect, noconstant(1)
 DECLARE dgdi_retry_count = i2 WITH protect, noconstant(0)
 DECLARE dgdi_table_name = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dgdi_table_owner = vc WITH protect, noconstant("")
 DECLARE dgdi_module_name = vc WITH protect, constant("DM_STATS_PROCESS")
 DECLARE dgdi_action_name = vc WITH protect, constant("GATHER_DBSTATS_IND")
 DECLARE dgdi_original_module = vc WITH protect, noconstant("")
 DECLARE dgdi_original_action = vc WITH protect, noconstant("")
 DECLARE dgdi_object_id = f8 WITH protect, noconstant(0.0)
 ...
 IF (check_logfile("dm2_gdbs_ind",".log","dm2_gather_dbstats_ind LOGFILE")=0)
  GO TO exit_script
 ENDIF
 IF (dmai_get_cur_mod_act(dgdi_original_module,dgdi_original_action)=0)
  GO TO exit_script
 ENDIF
 CALL dmai_set_mod_act(dgdi_module_name,dgdi_action_name)
 SET dm_err->eproc = "Beginning dm2_gather_dbstats_ind"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dgdi_inowner = trim(cnvtupper( $1),3)
 SET dgdi_inindex = trim( $2,3)
 SET dgdi_inmode = trim(cnvtupper( $3),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg =
  "Parameter usage: dm2_gather_dbstats_ind  '<owner>','<table_name>','<STAGE|ACTUALONLY|PREVIEW>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating <mode> input parameter."
 IF ( NOT (dgdi_inmode IN ("STAGE", "ACTUALONLY", "PENDING", "PREVIEW")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Valid values for $3 <mode> are: STAGE | ACTUALONLY | PREVIEW "
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dgdt_prefs->in_object_type = "INDEX"
 SET dgdt_prefs->in_object_owner = dgdi_inowner
 SET dgdt_prefs->in_object_name = dgdi_inindex
 SET dgdt_prefs->in_mode = dgdi_inmode
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_add_detail_text(dpl_owner,dgdt_prefs->in_object_owner)
 CALL dm2_process_log_add_detail_text(dpl_index,dgdt_prefs->in_object_name)
 ...
 SET dgdi_log_id = dm2_process_event_rs->dm_process_event_id
 SELECT INTO "nl:"
  FROM dba_indexes d
  WHERE (d.owner=dgdt_prefs->in_object_owner)
   AND (d.index_name=dgdt_prefs->in_object_name)
   AND d.status != "UNUSABLE"
  DETAIL
   dgdi_table_name = d.table_name, dgdi_table_owner = d.table_owner
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dgdi_table_name="DM2NOTSET")
  SET dgdi_retry_ind = 0
 ELSE
  IF (ddf_get_object_id(dgdi_table_owner,dgdi_table_name,dgdi_object_id)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 WHILE (dgdi_retry_ind=1)
   IF (ddf_context_lock_maint("CHECK","STAT",2,dgdi_table_owner,dgdi_table_name,
    dgdi_object_id,dgdi_lock_available_ind)=0)
    GO TO exit_script
   ENDIF
   IF (dgdi_lock_available_ind=1)
    IF (ddf_context_lock_maint("GET","STAT",1,dgdi_table_owner,dgdi_table_name,
     dgdi_object_id,dgdi_lock_obtained_ind)=0)
     GO TO exit_script
    ENDIF
    IF (dgdi_lock_obtained_ind=1)
     SET dgdi_retry_ind = 0
    ENDIF
   ENDIF
   IF (dgdi_lock_obtained_ind=0)
    IF (dgdi_retry_count > 2)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Unable to obtain gather stats lock for Index: ",dgdt_prefs->
      in_object_name,
      ". Index stats will be re-gathered during next cycle. This error can be ignored.")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ELSE
     ...
     CALL disp_msg(" ",dm_err->logfile,0)
     CALL pause(30)
     SET dgdi_retry_count = (dgdi_retry_count+ 1)
    ENDIF
   ENDIF
 ENDWHILE
 EXECUTE dm2_set_dbstats_prefs "INDEX", dgdt_prefs->in_object_owner, dgdt_prefs->in_object_name,
 dgdt_prefs->in_mode
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dgdt_prefs)
 ENDIF
 IF ((((dm_err->err_ind=1)) OR ((((dgdt_prefs->autosuccess_ind=1)) OR (dgdi_inmode="PREVIEW")) )) )
  GO TO exit_script
 ENDIF
 CALL dm2_process_log_add_detail_number(dpl_est_pct,cnvtreal(dgdt_prefs->est_pct_idx))
 CALL dm2_process_log_add_detail_text(dpl_table,dgdt_prefs->in_table_name)
 CALL dm2_process_log_add_detail_text("INDEX_OWNER.INDEX_NAME",concat(dgdt_prefs->in_object_owner,".",
   dgdt_prefs->in_object_name))
 CALL dm2_process_log_add_detail_text("TABLE_OWNER.INDEX_NAME",concat(dgdt_prefs->table_owner,".",
   dgdt_prefs->in_object_name))
 IF (dm2_process_log_dtl_row(dgdi_log_id,0)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = build("Set resumable on for index (",dgdt_prefs->in_object_owner,".",dgdt_prefs
  ->in_object_name,")")
 SET dgdi_cmd = concat("RDB ALTER SESSION ENABLE RESUMABLE TIMEOUT 28800 NAME '",dgdt_prefs->
  in_object_name,"' GO")
 IF (dm2_push_cmd(dgdi_cmd,1)=0)
  CALL disp_msg(" ",dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 EXECUTE dm2_alter_dbstats_session "IND"
 FOR (dgdi_wh_cnt = 1 TO 3)
   SET dm_err->eproc = build("Gathering Index statistics for (",dgdt_prefs->in_object_owner,".",
    dgdt_prefs->in_object_name,")")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   SET dgdi_step_beg_dttm = cnvtdatetime(curdate,curtime3)
   CALL gather_index_stats(dgdt_prefs->in_object_owner,concat('"',dgdt_prefs->in_object_name,'"'),
    cnvtreal(dgdt_prefs->est_pct_idx),dgdt_prefs->degree_idx,cnvtbool(true))
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
   SET dgdi_step_end_dttm = cnvtdatetime(curdate,curtime3)
   CALL dm2_process_log_add_detail_number("GATHER_INDEX_STATS:SECS",datetimediff(dgdi_step_end_dttm,
     dgdi_step_beg_dttm,5))
   IF (dm2_process_log_dtl_row(dgdi_log_id,0)=0)
    GO TO exit_script
   ENDIF
   IF (dgdi_inmode IN ("STAGE", "ACTUALONLY"))
    CASE (dgdi_inmode)
     OF "STAGE":
      SET dgdi_export_statid = concat("DMSTG_I",dgdt_prefs->in_object_id)
     OF "ACTUALONLY":
      SET dgdi_export_statid = "ACTUAL"
    ENDCASE
    EXECUTE dm2_export_dbstats_pending dgdt_prefs->table_owner, dgdt_prefs->in_table_name,
    dgdi_export_statid
    IF ((dm_err->err_ind=1))
     GO TO exit_script
    ENDIF
    SET dm_err->eproc = "Verify stats were exported to DM_STAT_TABLE"
    SELECT INTO "nl:"
     FROM dm_stat_table dst
     WHERE dst.statid=dgdi_export_statid
      AND (dst.c1=dgdt_prefs->in_object_name)
      AND (dst.c5=dgdt_prefs->in_object_owner)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
    IF (((curqual > 0) OR ((dgdt_prefs->in_table_name != cnvtupper(dgdt_prefs->in_table_name)))) )
     SET dgdi_wh_cnt = 4
    ELSE
     IF (dgdi_wh_cnt=3)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Index stats export was retried. No pending stats were found."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_script
     ENDIF
    ENDIF
   ...
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Get DM2GDBS:CUSTADJUSTPROC-IND row from DM_INFO."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2GDBS:STAGEADJUSTPROC-IND"
  DETAIL
   dgdi_procs->cnt = (dgdi_procs->cnt+ 1), stat = alterlist(dgdi_procs->qual,dgdi_procs->cnt),
   dgdi_procs->qual[dgdi_procs->cnt].proc_name = trim(d.info_name),
   dgdi_procs->qual[dgdi_procs->cnt].proc_cmd = trim(d.info_char)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dgdi_procs)
 ENDIF
 IF ((dgdi_procs->cnt > 0))
  FOR (dgdi_proccnt = 1 TO dgdi_procs->cnt)
    SET dgdi_cmd = dgdi_procs->qual[dgdi_proccnt].proc_cmd
    SET dgdi_cmd = replace(dgdi_cmd,"<OBJECT_TYPE>","INDEX")
    SET dgdi_cmd = replace(dgdi_cmd,"<OBJECT_OWNER>",dgdt_prefs->in_object_owner)
    SET dgdi_cmd = replace(dgdi_cmd,"<OBJECT_NAME>",dgdt_prefs->in_object_name)
    SET dgdi_cmd = replace(dgdi_cmd,"<MODE>",dgdi_inmode)
    SET dgdi_cmd = replace(dgdi_cmd,"<STATID>",dgdi_export_statid)
    SET dgdi_step_beg_dt_tm = cnvtdatetime(curdate,curtime3)
    SET dgdi_cmd = concat("RDB ASIS(^ begin ",dgdi_cmd,"; end; ^) go")
    IF (dm2_push_cmd(dgdi_cmd,1)=0)
     CALL disp_msg(" ",dm_err->logfile,1)
     IF ((dm_err->err_ind=1))
      SET dm_err->err_ind = 0
    ...
    CALL dm2_process_log_add_detail_number(concat(dgdi_procs->qual[dgdi_proccnt].proc_name,":SECS"),
     datetimediff(dgdi_step_end_dttm,dgdi_step_beg_dttm,5))
    CALL dm2_process_log_add_detail_text(concat(dgdi_procs->qual[dgdi_proccnt].proc_name,":CMD"),
     dgdi_cmd)
    IF (dm2_process_log_dtl_row(dgdi_log_id,0)=0)
     GO TO exit_script
    ENDIF
...
#exit_script
 ...
 SET dgdi_err_ind = dm_err->err_ind
 SET dgdi_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_index_stats_gathering,dgdi_log_id,1)
 IF (dgdi_lock_obtained_ind=1)
  CALL ddf_context_lock_maint("RELEASE","STAT",1,dgdi_table_owner,dgdt_prefs->in_table_name,
   dgdi_object_id,dgdi_lock_available_ind)
 ENDIF
 SET dm_err->err_ind = dgdi_err_ind
 SET dm_err->emsg = dgdi_emsg
 SET dm_err->eproc = "Ending dm2_gather_dbstats_ind"
 CALL dmai_set_mod_act(dgdi_original_module,dgdi_original_action)
 CALL final_disp_msg("dm2_gdbs_ind")
END GO
