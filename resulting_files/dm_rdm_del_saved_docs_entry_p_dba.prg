CREATE PROGRAM dm_rdm_del_saved_docs_entry_p:dba
 ...
 DECLARE sbr_divide_rows(mf_max_range_id=f8,mf_min_range_id=f8,ms_child_prefix=vc,ms_info_domain=vc,
  mn_num_children=i4) = i2
 DECLARE sbr_handle_rollback(ms_info_domain_nm=vc,mn_num_children=i4) = i2
 DECLARE sbr_check_for_prev_rdm(mn_readme_id=i4,mn_instance=i4) = i4
 DECLARE sbr_run_parent(mf_max_range_id=f8,mf_min_range_id=f8,ms_info_domain_nm=vc,ms_range_prefix=vc,
  mn_num_children=i4) = i4
 DECLARE sbr_wipe_parallel_stats(mf_readme_id=f8) = i2
 SUBROUTINE sbr_check_for_prev_rdm(mn_readme_id,mn_instance,domain_name,range_prefix)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE sbr_handle_rollback(ms_info_domain_nm,mn_num_children)
 ...
 SUBROUTINE sbr_add_ranges(ms_info_domain_nm,mn_num_children)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_divide_rows(mf_max_range_id,mf_min_range_id,ms_child_prefix,ms_info_domain,
  mn_num_children)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_child_prefix,ms_info_domain_nm,
  mn_num_children)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_wipe_parallel_stats(mf_readme_id)
 ...
 DECLARE mf_min_range_id_powerform = f8 WITH protect, noconstant(0.0)
 DECLARE mf_max_range_id_wkfdoccomp = f8 WITH protect, noconstant(0.0)
 DECLARE mf_min_range_id_wkfdoccomp = f8 WITH protect, noconstant(0.0)
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 IF (((error(ms_errmsg,0) != 0) OR (task_status_deleted_cd <= 0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error loading code value DELETED from codeset 79: ",ms_errmsg)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cv.code_value
 ...
 SET ms_range_prefix = "MAX TASK_ACTIVITY_ID RANGE"
 SET mn_num_children = 10
 SELECT INTO "n1:"
  min_id = min(ta.task_id), max_id = max(ta.task_id)
  ...
  DETAIL
   mf_min_range_id_powerform = cnvtreal(min_id), mf_max_range_id_powerform = cnvtreal(max_id)
  WITH nocounter
 ;end select
 ...
 SELECT INTO "n1:"
  ...
  WHERE ta.active_ind=1
   ...
   AND ce.valid_until_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00")
  DETAIL
   mf_min_range_id_wkfdoccomp = cnvtreal(min_id), mf_max_range_id_wkfdoccomp = cnvtreal(max_id)
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
  ...
  GO TO exit_program
 ENDIF
 IF (mf_min_range_id_powerform <= mf_min_range_id_wkfdoccomp)
  SET mf_min_range_id = mf_min_range_id_powerform
 ELSE
  SET mf_min_range_id = mf_min_range_id_wkfdoccomp
 ENDIF
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_ENTRYMODE_TASK_ACTIVITY",concat(
   "create global temporary table TEMP_ENTRYMODE_TASK_ACTIVITY(task_id number)",
   "on commit preserve rows"),"Creating temporary table")
 CALL sbr_add_sql_stmt("INDEX","XIETEMP_TASK_ACTIVITY",
  "create index XIETEMP_TASK_ACTIVITY ON TEMP_ENTRYMODE_TASK_ACTIVITY(task_id)","Creating index")
 IF (sbr_run_sql(null)=0)
  GO TO exit_program
 ENDIF
 SET readme_data->status = "S"
