CREATE PROGRAM dm2_mig_custccl_dropdupccl
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmccdd_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_dropdup")
 DECLARE dmccdd_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccdd_cont = i2 WITH protect, noconstant(1)
 DECLARE dmccdd_env = vc WITH protect, noconstant("")
 DECLARE dmccdd_node = vc WITH protect, noconstant("")
 DECLARE dmccdd_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccdd_os_type = vc WITH protect, noconstant("")
 DECLARE dmccdd_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccdd_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmccdd_acc = vc WITH protect, noconstant("")
 FREE RECORD dmccdd_dup_scripts
 RECORD dmccdd_dup_scripts(
   1 cnt = i4
   1 qual[*]
     2 script_name = vc
     2 object_type = vc
 ...
 DECLARE dmccdd_get_dup_scripts(null) = i2
 DECLARE dmccdd_view_dup_rpt(dvdr_id=f8) = i2
 SET width = 132
 IF (check_logfile(dmccdd_logfile_prefix,".log","dm2_mig_custccl_dropdupccl")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_dropdupccl"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmccdd_env = cnvtupper(trim(logical("environment")))
 SET dmccdd_node = trim(curnode)
 SET dmccdd_os_type = trim(cursys2)
 SET dm_err->eproc = "Getting environment id for current node/env"
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE de.environment_name=dmccdd_env
  DETAIL
   dmccdd_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  ...
  WHERE dne.node_name=dmccdd_node
   AND dne.environment_id=dmccdd_env_id
  DETAIL
   dmccdd_id = dne.dma_env_node_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dmccdd_env))
  CALL echo(concat("Current Environment Id: ",cnvtstring(dmccdd_env_id)))
  CALL echo(concat("Current Node: ",dmccdd_node))
  CALL echo(concat("Current Environment: ",dmccdd_os_type))
  CALL echo(concat("Current Node/Env ID: ",cnvtstring(dmccdd_id)))
 ENDIF
 IF (dmccdd_get_dup_scripts(null)=0)
  GO TO exit_program
 ENDIF
 IF ((dmccdd_dup_scripts->cnt > 0))
  SET message = window
  SET width = 132
  CALL clear(1,1)
  CALL box(1,1,22,131)
  CALL text(1,2,"Custom CCL Migration: Duplicate CCL Script Report")
  CALL text(1,70,"DATE/TIME: ")
  CALL text(1,80,build(month(curdate),"/",day(curdate),"/",year(curdate),
    " ",hour(curtime),":",minute(curtime)))
  CALL text(3,2,concat("Environment:",dmccdd_env," (",trim(cnvtstring(dmccdd_env_id)),")"))
  CALL text(4,2,concat("Node:",dmccdd_node))
  CALL text(6,2,
   "There are CCL script names that are duplicate to a Group 0 CCL script having the same name")
  CALL text(7,2,"The non-Group 0 scripts have been marked for removal from the CCL Dictionary")
  CALL text(9,2,"(V)iew list of duplicate scripts, (Q)uit: ")
  CALL accept(9,44,"A;cu"," "
   WHERE curaccept IN ("V", "Q"))
  SET dmccdd_acc = curaccept
  IF (dmccdd_acc="V")
   WHILE (dmccdd_cont=1)
     SET message = nowindow
     IF (dmccdd_view_dup_rpt(dmccdd_id)=0)
      GO TO exit_program
     ENDIF
     SET message = window
     SET width = 132
     CALL clear(1,1)
     CALL box(1,1,22,131)
     CALL text(1,2,"Custom CCL Migration: Duplicate CCL Confirmation")
     CALL text(1,70,"DATE/TIME: ")
     CALL text(1,80,build(month(curdate),"/",day(curdate),"/",year(curdate),
       " ",hour(curtime),":",minute(curtime)))
     CALL text(3,2,concat("Environment:",dmccdd_env," (",trim(cnvtstring(dmccdd_env_id)),")"))
     CALL text(4,2,concat("Node:",dmccdd_node))
     CALL text(6,2,
      "Please confirm that the previously reported CCL Scripts can be removed from the current CCL Dictionary"
      )
     CALL text(9,2,"(V)iew Duplicates, (R)emove duplicates and continue, (Q)uit: ")
     CALL accept(9,62,"A;cu"," "
      WHERE curaccept IN ("V", "R", "Q"))
     SET message = nowindow
     IF (curaccept="R")
      SET dmccdd_cont = 0
     ELSEIF (curaccept="Q")
      SET dmccdd_cont = 0
      SET dm_err->emsg = "User elected to quit from Target Duplicate CCL Confirmation"
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
  ...
  ELSEIF (dmccdd_acc="Q")
   SET message = nowindow
   SET dm_err->emsg = "User elected to quit from Target Duplicate CCL Script Report"
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ELSE
  SET dm_err->eproc = "No Duplicate CCL Scripts were found"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmccdd_dup_scripts)
 ENDIF
 SET dm_err->eproc = "Drop Duplicate Scripts in TARGET"
 FOR (dmccdd_cnt = 1 TO dmccdd_dup_scripts->cnt)
   IF (dm2_push_cmd(concat("drop program ",dmccdd_dup_scripts->qual[dmccdd_cnt].script_name,":GROUP",
     cnvtstring(dmccdd_dup_scripts->qual[dmccdd_cnt].group_nbr)," go"),1)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Deleting row for dropped script in dma_env_node_dprotect"
   DELETE  FROM dma_env_node_dprotect dend
    WHERE (dend.object_name=dmccdd_dup_scripts->qual[dmccdd_cnt].script_name)
     AND (dend.group_nbr=dmccdd_dup_scripts->qual[dmccdd_cnt].group_nbr)
     AND dend.dma_env_node_id=dmccdd_id
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
 ...
 ENDFOR
 GO TO exit_program
 SUBROUTINE dmccdd_get_dup_scripts(null)
   SET dm_err->eproc = "Retrieving list of duplicate group0 scripts from dprotect."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dprotect d1,
     dprotect d2
    PLAN (d1
     WHERE d1.object="P"
      ...
      AND d1.group < 99)
     JOIN (d2
     WHERE d1.object=d2.object
      AND d1.object_name=d2.object_name
      AND d2.group=0)
    ORDER BY d1.object_name
    HEAD REPORT
     dmccdd_dup_scripts->cnt = 0, stat = alterlist(dmccdd_dup_scripts->qual,0)
    DETAIL
     dmccdd_dup_scripts->cnt = (dmccdd_dup_scripts->cnt+ 1), stat = alterlist(dmccdd_dup_scripts->
      qual,dmccdd_dup_scripts->cnt), dmccdd_dup_scripts->qual[dmccdd_dup_scripts->cnt].script_name =
     d1.object_name,
     dmccdd_dup_scripts->qual[dmccdd_dup_scripts->cnt].object_type = d1.object, dmccdd_dup_scripts->
     qual[dmccdd_dup_scripts->cnt].user_name = d1.user_name, dmccdd_dup_scripts->qual[
     dmccdd_dup_scripts->cnt].source_name = d1.source_name,
     dmccdd_dup_scripts->qual[dmccdd_dup_scripts->cnt].group_nbr = d1.group
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmccdd_view_dup_rpt(dvdr_id)
   ...
   CALL disp_msg("",dm_err->logfile,0)
   IF (get_unique_file("dm2_mig_cc_dup",".dat")=0)
    GO TO exit_program
   ELSE
   ...
   SELECT INTO value(dvdr_rpt_file)
    FROM dummyt d
    HEAD REPORT
     CALL print(fillstring(250,"-")), row + 1, "Duplicate Script Report",
     row + 1,
     CALL print(fillstring(250,"-")), row + 2
    DETAIL
     col 0, "DUPLICATE CCL SCRIPT NAME ", col 60,
     "| USER NAME ", col 80, "| GROUP NUMBER ",
     row + 1,
     CALL print(fillstring(250,"-")), row + 2,
     dvdr_cnt = 0
     FOR (dvdr_cnt = 1 TO dmccdd_dup_scripts->cnt)
       col 0,
       CALL print(trim(dmccdd_dup_scripts->qual[dvdr_cnt].script_name)), col 60,
       "| ",
       CALL print(trim(dmccdd_dup_scripts->qual[dvdr_cnt].user_name)), col 80,
       "| ",
       CALL print(cnvtstring(dmccdd_dup_scripts->qual[dvdr_cnt].group_nbr)), row + 1
     ENDFOR
   ...
   IF (check_error(dm_err->eproc))
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Displaying Duplicate CCL Script Report"
   IF (dm2_disp_file(dvdr_rpt_file,"Duplicate Report")=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc =
   "Insert a row into dma_env_node_attr admin table for duplicate script report location (if row does not exist)"
   SELECT INTO "nl:"
    FROM dma_env_node_attr dena
    WHERE dena.dma_env_node_id=dvdr_id
     AND dena.attr_name="CCMIG_DUP_CCL_RPT"
    WITH nocounter
   ;end select
   IF (curqual=0)
    SET dm_err->eproc = "Inserting row for ccl duplicate script report"
    INSERT  FROM dma_env_node_attr dena
     SET dena.dma_attr_id = seq(dm_seq,nextval), dena.dma_env_node_id = dvdr_id, dena.attr_name =
      "CCMIG_DUP_CCL_RPT",
      dena.attr_value_txt = dvdr_rpt_file
     WITH nocounter
    ;end insert
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ELSE
    SET dm_err->eproc = "Updating row for ccl duplicate script report"
    UPDATE  FROM dma_env_node_attr dena
     SET dena.attr_value_txt = dvdr_rpt_file
     WHERE dena.dma_env_node_id=dvdr_id
      AND dena.attr_name="CCMIG_DUP_CCL_RPT"
     WITH nocounter
    ;end update
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_dropdupccl"
 CALL final_disp_msg(dmccdd_logfile_prefix)
END GO
