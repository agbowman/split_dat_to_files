CREATE PROGRAM acm_del_prsnl_org_reltn_type:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.prsnl_org_reltn_type_id=acm_request->prsnl_org_reltn_type_qual[xref->del[d.seq].idx].
   prsnl_org_reltn_type_id))
  WITH nocounter, status(reply->prsnl_org_reltn_type_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->prsnl_org_reltn_type_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_org_reltn_type_qual[xref->del[index].idx].status=0))
