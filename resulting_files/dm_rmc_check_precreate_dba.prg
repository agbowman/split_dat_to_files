CREATE PROGRAM dm_rmc_check_precreate:dba
 ...
 ENDIF
 DECLARE drcp_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drcp_dblink = vc WITH protect, noconstant("")
 DECLARE drcp_db_name = vc WITH protect, noconstant("")
 DECLARE drcp_tab_cnt = i4 WITH protect, noconstant(0)
 DECLARE drcp_status = c1 WITH protect, noconstant("F")
 SET dm_err->eproc = "Starting dm_rmc_execute_task"
 IF (check_logfile("dm_rmc_precreate",".log","DM_RMC_PRECREATE LOG")=0)
  SET dm_err->err_ind = 1
 ...
 IF (((reflect(parameter(1,0)) != "F*"
  AND reflect(parameter(1,0)) != "I*") OR (reflect(parameter(2,0)) != "C*")) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Expected syntax:  dm_rmc_check_precreate <env_id>,<post_link_name>")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drcp_env_id =  $1
 SET drcp_dblink =  $2
 SET dm_err->eproc = "Checking for existing $R tables."
 SELECT INTO "nl:"
  FROM user_tables ut
 ...
 SET dm_err->eproc = "Gathering list of tables from dm_chg_log in source."
 SELECT DISTINCT INTO "nl:"
  ...
  (SELECT
   "x"
   FROM (parser(concat("DM_CHG_LOG",drcp_dblink)) d)
   WHERE d.target_env_id=drcp_env_id
    AND d.log_type="REFCHG"
    AND d.table_name=ut.table_name))
  HEAD REPORT
   drcp_tab_cnt = 0, task_info->environment_id = drcp_env_id, task_info->process_name =
   drtw_process_name
  DETAIL
   drcp_tab_cnt = (drcp_tab_cnt+ 1)
   IF (mod(drcp_tab_cnt,100)=1)
    stat = alterlist(task_info->qual,(drcp_tab_cnt+ 99))
   ENDIF
   task_info->qual[drcp_tab_cnt].task_desc = concat("Creating $R table: ",trim(ut.table_name,3)),
   task_info->qual[drcp_tab_cnt].task_level = (drtw_task_level+ 50), task_info->qual[drcp_tab_cnt].
   task_name =
   'execute DM_RMC_CREATE_R_TABLE with replace("REPLY","DCRT_REPLY"),replace("REQUEST","DCRT_REQUEST") go',
   task_info->qual[drcp_tab_cnt].task_reply = concat(
    "<REC_DEF>record DCRT_REPLY (1 status_data 2 status = c1 2 logfile = vc) go</REC_DEF>",
    "<ERR_IND>dcrt_reply->status_data.status</ERR_IND><ERR_MSG> </ERR_MSG><NO_ERR_RESULT>S</NO_ERR_RESULT>",
    "<ERR_TYPE>VC</ERR_TYPE>"), task_info->qual[drcp_tab_cnt].task_request = concat(
    "<REC_DEF>record DCRT_REQUEST( 1 database_name = vc 1 table_name = vc 1 trig_ind = i2) go</REC_DEF>",
    "<VAR_DEF>declare drcrt_precreate_ind = i2 go</VAR_DEF><VAL_SET>set drcrt_precreate_ind = 1 go</VAL_SET>",
    "<VAL_SET>set dcrt_request->database_name = '",drcp_db_name,"' go</VAL_SET><VAL_SET>set ",
    "dcrt_request->table_name = '",trim(ut.table_name,3),
    "' go</VAL_SET><VAL_SET>set dcrt_request->trig_Ind = 1 go</VAL_SET>")
  FOOT REPORT
   task_info->total = drcp_tab_cnt, stat = alterlist(task_info->qual,task_info->total)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 IF ((task_info->total > 0))
  ...
  SET task_info->qual[task_info->total].task_desc = "Creating all $R CCL definitions"
  ...
  SET drcp_status = drtq_insert_task_process(task_info,0)
  IF (drcp_status="F")
 ...
 ENDIF
#exit_program
 CALL final_disp_msg("dm_rmc_check_precreate")
END GO
