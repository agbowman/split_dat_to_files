CREATE PROGRAM bhs_dcp_get_pl_careteam:dba
 RECORD orgs(
   1 qual[*]
     2 org_id = f8
     2 confid_level = i4
 ...
 DECLARE patient_status_flag = i4
 DECLARE arg_nbr = i4 WITH noconstant(cnvtint(size(request->arguments,5)))
 DECLARE encntr_id = vc WITH noconstant(fillstring(3000," "))
 ...
 FOR (counter = 1 TO arg_nbr)
   CASE (request->arguments[counter].argument_name)
    ...
    OF "careteam_id":
     SET prsnl_group_id = cnvtint(request->arguments[counter].parent_entity_id)
    OF "facility_filter":
     SET fac_filter = concat(fac_filter,cnvtstring(request->arguments[counter].parent_entity_id),",")
     SET fac_ind = 1
   ENDCASE
 ENDFOR
 CALL echo(build("PRSNL_GROUP_ID=",prsnl_group_id))
 SET temp_dt_tm = datetimeadd(cnvtdatetime(curdate,curtime3),- ((patient_status_minutes/ 1440.0)))
 IF (patient_status_flag=0)
  SET encntr_where = "e.encntr_id = ce.encntr_id and (e.active_ind = 1 or e.encntr_id = 0)"
 ELSEIF (patient_status_flag=1)
  SET encntr_where = concat("e.encntr_id = ce.encntr_id and e.active_ind = 1",
 ...
 ENDIF
 SELECT INTO "nl:"
  FROM dcp_pl_custom_entry ce,
   encounter e,
   person p,
   dcp_pl_prioritization pr
  PLAN (ce
   WHERE ce.prsnl_group_id=prsnl_group_id)
   JOIN (p
