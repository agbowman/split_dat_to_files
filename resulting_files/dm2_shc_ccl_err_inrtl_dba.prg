CREATE PROGRAM dm2_shc_ccl_err_inrtl:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsce_err_status = c1 WITH protect, noconstant(" ")
 DECLARE dsce_rtlfile = vc WITH protect, noconstant(" ")
 DECLARE dsce_file = vc WITH protect, noconstant(" ")
 DECLARE dsce_del_cmd = vc WITH protect, noconstant(" ")
 FREE RECORD dsce_rtlerr
 RECORD dsce_rtlerr(
   1 rtl_cnt = i4
   1 rtl[*]
     2 rtlerr = vc
 )
 IF (check_logfile("DM2_SHC_CCLERRRTL",".log","DM2_SHC_CCL_ERR_INRTL LOG FILE")=0)
  SET dsce_err_status = "F"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "System Health Check working on CCL Errors in RTL Files..."
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF (get_unique_file("dm2_dshc_output",".dat"))
  SET dsce_file = dm_err->errfile
  SET dm_err->errfile = dm_err->unique_fname
  SET dm_err->disp_dcl_err_ind = 0
  IF (cursys="AIX")
   IF (dm2_push_dcl("grep ^%CCL-E $CCLUSERDIR/rtlsrv\*.log")=0)
    IF ((dm_err->err_ind=1))
     SET dsce_err_status = "E"
    ELSEIF (findstring("Cannot open",dm_err->errtext,1) > 0)
     SET dsce_err_status = "G"
    ELSE
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dsce_err_status = "E"
   ...
   ELSE
    ...
    SELECT INTO "nl:"
     ...
     HEAD REPORT
      beg_pos = 0, errlen = 0
     DETAIL
      beg_pos = 0, errlen = 0, beg_pos = findstring("rtl",r.line)
      IF (beg_pos > 0)
       errlen = textlen(trim(r.line)), dsce_rtlerr->rtl_cnt += 1, stat = alterlist(dsce_rtlerr->rtl,
        dsce_rtlerr->rtl_cnt),
       dsce_rtlerr->rtl[dsce_rtlerr->rtl_cnt].rtlerr = substring(beg_pos,errlen,r.line)
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(concat("Parse out the rtl errors.")) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dsce_err_status = "E"
    ENDIF
   ENDIF
  ELSEIF (cursys="AXP")
   IF (dm2_push_dcl("$search ccluserdir:rtl\*.log %CCL-E /nowarnings")=0)
    IF ((dm_err->err_ind=1))
     SET dsce_err_status = "E"
    ELSEIF (findstring("file not found",dm_err->errtext,1) > 0)
     SET dsce_err_status = "G"
    ELSE
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSE
    ...
    SELECT INTO "nl:"
     ...
     FROM rtl2t r
     HEAD REPORT
      file_pos = 0, err_pos = 0, errlen = 0
     DETAIL
      file_pos = 0, err_pos = 0, errlen = 0,
      file_pos = findstring("RTL",r.line)
      IF (file_pos > 0)
       errlen = textlen(trim(r.line)), dsce_rtlfile = "", dsce_rtlfile = substring(file_pos,errlen,r
        .line)
      ELSE
       err_pos = findstring("%CCL-E",r.line)
       IF (err_pos > 0)
        errlen = textlen(trim(r.line)), dsce_rtlerr->rtl_cnt += 1, stat = alterlist(dsce_rtlerr->rtl,
         dsce_rtlerr->rtl_cnt),
        dsce_rtlerr->rtl[dsce_rtlerr->rtl_cnt].rtlerr = concat(dsce_rtlfile,": ",substring(err_pos,
          errlen,r.line))
       ENDIF
      ENDIF
     WITH nocounter
   ...
   ENDIF
  ENDIF
  SET dm_err->errfile = dsce_file
  FREE DEFINE rtl2
  FREE SET file_loc
  IF (cursys="AIX")
   SET dsce_del_cmd = concat("rm -f $CCLUSERDIR/",dm_err->unique_fname)
  ELSEIF (cursys="AXP")
   SET dsce_del_cmd = concat("delete ccluserdir:",dm_err->unique_fname,";*")
  ENDIF
  IF (dm2_push_dcl(dsce_del_cmd)=0)
   SET dsce_err_status = "E"
  ENDIF
 ELSE
  SET dsce_err_status = "E"
 ENDIF
 IF (dshc_add_check_to_rs("CCL ERRORS IN RTL FILES","CCL Errors in RTL Files",0,dsce_err_status,
  dshc_ctf_millennium)=0)
  SET dsce_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dsce_err_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dsce_rtlerr->rtl_cnt))
  DETAIL
   ...
   dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "CCL errors in RTL* files", dshc->check[dshc->
   chk_cnt].desc_detail[cnt].value_prefix = "NOT_SET", dshc->check[dshc->chk_cnt].desc_detail[cnt].
   dtl_desc_text = "NOT_SET",
   dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2, dshc->check[dshc->chk_cnt].desc_detail[cnt].
   str_value = dsce_rtlerr->rtl[d.seq].rtlerr
  WITH nocounter
 ;end select
 IF (check_error(concat("Write out description details.")) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dsce_err_status = "F"
 ENDIF
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "If many CCL errors are being logged, access to the physical disk holding ",
  "the CCLUSERDIR directory can become slower affecting application performance.  ",
  "These CCL error messages may also be an indication that application functionality ",
  "is not correct.  <SPACE>Log points with the IAC to correct any CCL errors logged in ",
  "server rtl files.")
#exit_script
 ...
 ENDIF
 IF (dsce_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_CCL_ERR_INRTL WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_CCL_ERR_INRTL WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_CCL_ERR_INRTL COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_CCLERRRTL")
 FREE RECORD dsce_rtlerr
END GO
