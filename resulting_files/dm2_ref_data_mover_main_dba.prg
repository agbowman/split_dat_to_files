CREATE PROGRAM dm2_ref_data_mover_main:dba
 ...
 IF (check_logfile("dm2_data_mover",".log","DM2_DATA_MOVER LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_main
 ENDIF
 DECLARE drdm_get_merge_del_rows(sbr_t_name=vc,sbr_pk_where=vc) = i4
 DECLARE drdm_validate(sbr_dv_t_name=vc,sbr_dv_pk_where=vc,sbr_dv_table_cnt=i4) = i2
 DECLARE drdm_create_pk_where(dcpw_cnt=i4,dcpw_table_cnt=i4,dcpw_type=vc) = vc
 DECLARE create_dcl_row(cdr_t_name=vc,cdr_pk_where=vc,cdr_idx_pos=i4) = i2
 ...
 DECLARE reset_updt_cnt(ruc_rs_idx=i4) = null
 ...
 FREE RECORD drdm_pair_info
 RECORD drdm_pair_info(
   1 qual[*]
     2 log_id = f8
     2 updt_cnt = i4
 )
 SET stat = alterlist(drdm_pair_info->qual,10)
 SET cutover_flag = 1
 IF (cutover_flag=1)
  SET dm_err->eproc = "RDDS is currently configured to run in cutover mode, the program will exit"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_main
 ENDIF
 SELECT INTO "nl:"
 ...
 IF (curqual=0)
  ...
  ENDIF
 ENDIF
 SET drdm_context_active_ind = 0
 SELECT INTO "NL:"
  FROM dm_info d
  WHERE d.info_domain="RDDS CONTEXT"
   AND d.info_name="ACTIVE_IND"
  DETAIL
   drdm_context_active_ind = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSE
  ...
  SELECT INTO "NL:"
   ...
   DETAIL
    drdm_context_string = d.info_char
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 SET drdm_oracle_context = concat("DEFAULT RDDS ",trim(cnvtstring(dm2_ref_data_doc->env_source_id)),
  " TO ",trim(cnvtstring(dm2_ref_data_doc->env_target_id)))
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="RDDS CONTEXT"
   AND d.info_name="CONTEXT TO SET"
  DETAIL
   drdm_oracle_context = d.info_char
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_main
 ENDIF
 IF (((trim(drdm_oracle_context)="") OR (trim(cnvtupper(drdm_oracle_context))="NULL")) )
  SET drdm_oracle_context = concat("DEFAULT RDDS ",trim(cnvtstring(dm2_ref_data_doc->env_source_id)),
   " TO ",trim(cnvtstring(dm2_ref_data_doc->env_target_id)))
 ENDIF
 IF (drdm_oracle_context="ALL"
  AND drdm_context_active_ind=1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Illegal Context Setup.  Please use DM_MERGE_DOMAIN_ADM to setup contexts."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_main
 ELSEIF (locateval(simple_loop,1,drdm_num_contexts,drdm_oracle_context,drdm_contexts->qual[
  simple_loop].values) > 0
  ...
  GO TO exit_main
 ELSE
  EXECUTE dm2_set_context "RDDS_CONTEXT", drdm_oracle_context
 ENDIF
 SELECT INTO "NL:"
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
 ...
 IF (curqual=0)
  ...
  ELSE
   COMMIT
  ENDIF
 ELSEIF (((drdm_batch_size < 1) OR (drdm_batch_size > 200)) )
  SET drdm_batch_size = 20
 ...
 IF ((dm2_ref_data_doc->pre_link_name="")
  ...
  AND currdb="ORACLE")
  SET dm2_ref_data_doc->post_link_name = concat("@MERGE",trim(cnvtstring( $1)),trim(cnvtstring(
 ...
 IF (dm2_rdds_chk_rel_val_rows(dm2_ref_data_doc->env_source_id,dm2_ref_data_doc->env_target_id,
  ...
  GO TO exit_main
 ENDIF
 EXECUTE dm2_rdds_metadata_refresh dm2_ref_data_doc->post_link_name
 ...
 ENDIF
 IF (curqual != 0)
  DELETE  FROM (parser(drdmm_srch_str) d)
   WHERE log_id=0
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL echo("")
   CALL echo("The default row needs to be removed from the DM_CHG_LOG table in the source.")
   CALL echo("")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_main
  ELSE
 ...
 SET drdmm_srch_str = dm2_get_rdds_tname("DM_CHG_LOG")
 SET drdm_info_name = concat("MERGE",trim(cnvtstring(dm2_ref_data_doc->env_source_id)),trim(
   cnvtstring(dm2_ref_data_doc->env_target_id)))
 IF (drdm_proc_tab != "")
  SET drdm_info_name = concat("TAB",drdm_info_name)
 ENDIF
 SET drdm_dcl_qual = 1
 WHILE (drdm_dcl_qual > 0)
   UPDATE  FROM (parser(drdmm_srch_str) d)
    SET d.log_type = "REFCHG", d.rdbhandle = null, d.updt_dt_tm = sysdate
    WHERE d.log_type="PROCES"
     AND (d.target_env_id=dm2_ref_data_doc->env_target_id)
     AND  NOT (d.rdbhandle IN (
    (SELECT
     audsid
     FROM gv$session)))
    WITH maxqual(d,10000)
   ;end update
...
#try_again
 IF (drdm_debug_row_ind=1)
  IF ((dm2_rdds_rec->mode="OS"))
   SET drdm_where_stmt =
   " log_id = drdm_row_start and log_type in(DRDM_LOG_TYPE,DRDM_NORDDS,'ORPHAN','OLDVER') "
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    drdm_chg->log[drdm_log_cnt].pk_where = d.pk_where, drdm_chg->log[drdm_log_cnt].table_name = d
    ...
    drdm_chg->log[drdm_log_cnt].context_name = d.context_name
    IF (d.delete_ind=0)
 ...
 ELSE
  ...
  WHILE (drdm_timeout_cnt < 5)
    SET drdm_where_stmt = concat("d.log_type in(DRDM_LOG_TYPE,DRDM_NORDDS) ",
     " and d.target_env_id  = dm2_ref_data_doc->env_target_id"," and d.updt_dt_tm < sysdate ")
    IF (drdm_proc_tab > "")
    ...
    IF (check_error("While selecting rows from the dm_chg_log ")=1)
     IF (findstring("ORA-02049",dm_err->emsg,1,0) > 0)
  ...
  ELSE
   UPDATE  FROM (parser(drdmm_srch_str) d)
    SET d.log_type = "PROCES", d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.rdbhandle =
     currdbhandle
 ...
 WHILE (drdm_complete_ind=0)
   IF ((drdm_chg->log[drdm_log_loop].table_name="TIER_MATRIX"))
    SET drdm_chg->log[drdm_log_loop].validate_ind = 0
    CALL reset_updt_cnt(drdm_log_loop)
   ENDIF
   ...
   IF ((drdm_chg->log[drdm_log_loop].validate_ind=1))
    ...
    ELSE
     ...
     SET drdm_valid_rows_cnt = drdm_validate(drdm_chg->log[drdm_log_loop].table_name,drdm_chg->log[
      drdm_log_loop].pk_where,temp_tbl_cnt)
     IF (drdm_log_loop != drdm_valid_rows_cnt)
      SET drdm_chg->log[drdm_log_loop].process_flg = 5
      IF (drdm_valid_rows_cnt < 0)
       IF ((drdm_valid_rows_cnt=- (3)))
        SET drdm_top_level_ind = 0
        ...
        WHILE (drdm_top_level_ind=0)
          SET drdm_tl_id_col = ""
          SET drdm_tl_tab_name = ""
          SET drdm_tl_name_col = ""
          SELECT INTO "NL:"
           FROM dm_refchg_tab_r d
           WHERE d.child_table=drdm_tpc_tab_name
           DETAIL
            drdm_tl_id_col = d.parent_id_col, drdm_tl_tab_name = d.parent_table, drdm_tl_name_col = d
            .parent_tab_col
           WITH nocounter
          ;end select
          SET drdmv_source_tab = dm2_get_rdds_tname(drdm_tpc_tab_name)
          SET drdmv_suffix = dm2_rdds_get_tbl_alias(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix)
          IF (curqual=0)
           SET drdm_top_level_ind = 1
          ELSE
           CALL parser(concat(" select into 'NL:' from ",drdmv_source_tab," ",drdmv_suffix),0)
           CALL parser(drdm_tpc_pk_where,0)
           CALL parser(concat(" detail drdm_tl_id_val = ",drdmv_suffix,".",drdm_tl_id_col),0)
           IF (drdm_tl_name_col != "")
            CALL parser(concat(" drdm_tl_name_val = ",drdmv_suffix,".",drdm_tl_name_col),0)
           ELSE
            SET drdm_tl_name_val = ""
           ENDIF
           CALL parser(" with nocounter go",1)
           IF (curqual=0)
            SET drdm_no_insert_ind = 1
            SET drdm_top_level_ind = 1
           ELSE
            IF (drdm_tl_name_val="")
             SET drdm_tl_name_val = drdm_tl_tab_name
            ENDIF
            SET temp_tbl_cnt = locateval(simple_loop,1,dm2_ref_data_doc->tbl_cnt,drdm_tl_name_val,
             dm2_ref_data_doc->tbl_qual[simple_loop].table_name)
            IF (temp_tbl_cnt=0)
             SET temp_tbl_cnt = fill_rs("TABLE",drdm_tl_name_val)
            ENDIF
            FOR (simple_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
              IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name=dm2_ref_data_doc->tbl_qual[
              temp_tbl_cnt].col_qual[simple_loop].root_entity_name)
               AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[simple_loop].root_entity_attr=
              dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[simple_loop].column_name))
               SET drdm_tl_col_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[simple_loop].
               column_name
              ENDIF
            ENDFOR
            SET drdm_tpc_pk_where = concat(" WHERE t",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
             ".",drdm_tl_col_name," = drdm_tl_id_val")
            SET drdm_tpc_tab_name = drdm_tl_name_val
           ENDIF
          ENDIF
        ENDWHILE
       ENDIF
       IF (drdm_valid_rows_cnt IN (- (1), - (2), - (3))
        ...
        IF (drdm_valid_rows_cnt IN (- (1), - (2)))
         ...
         SET drdm_tpc_context = drdm_chg->log[drdm_log_loop].context_name
        ENDIF
        SET drdm_proc_call_ind = trigger_proc_call(drdm_tpc_tab_name,drdm_tpc_pk_where,
         drdm_tpc_context,"",0.0)
       ENDIF
      ENDIF
      IF (drdm_proc_call_ind=0)
       IF ((drdm_valid_rows_cnt=- (4)))
      ...
      IF (drdm_log_loop=0)
       ...
       CALL merge_audit("BATCH END",concat("The end of a batch was reached at log_id ",trim(
          cnvtstring(drdm_chg->log[drdm_log_loop].log_id))),1)
      ENDIF
     ENDIF
     IF (drdm_log_loop != 0
      ...
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].merge_delete_ind=1))
       SET dm_err->eproc = concat("Exploding an MD row  ",trim(cnvtstring(drdm_chg->log[drdm_log_loop
          ].log_id)))
       SET drdm_related_rows_cnt = drdm_get_merge_del_rows(drdm_chg->log[drdm_log_loop].table_name,
        drdm_chg->log[drdm_log_loop].pk_where,temp_tbl_cnt)
   ...
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].mergeable_ind != 0)
    ...
    IF ((drdm_chg->log[drdm_log_loop].process_flg IN (1, 2, 3, 6))
     ...
     IF (temp_tbl_cnt=0)
      SET temp_tbl_cnt = fill_rs("TABLE",drdm_chg->log[drdm_log_loop].table_name)
     ENDIF
     SET drdm_child_cnt = drdm_get_child_rows(drdm_chg->log[drdm_log_loop].table_name,drdm_chg->log[
      drdm_log_loop].pk_where,temp_tbl_cnt,drdm_chg->log[drdm_log_loop].next_to_process)
   ...
   IF (drdm_log_loop != 0
    ...
    tbl_qual[temp_tbl_cnt].reference_ind=0))) )
     ...
     ELSE
      COMMIT
     ENDIF
    ELSEIF ((drdm_chg->log[drdm_log_loop].process_flg IN (1, 2, 3, 6)))
    ...
    ELSE
     ...
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].mergeable_ind=1)
      AND (drdm_chg->log[drdm_log_loop].process_flg IN (1, 2, 3, 6)))
      SET drdm_no_trans_ind = 0
      IF ((drdm_chg->log[drdm_log_loop].delete_ind=1))
       EXECUTE dm2_ref_data_mover_main_child
      ELSEIF ((drdm_chg->log[drdm_log_loop].table_name="CODE_VALUE"))
       EXECUTE dm2_ref_data_mover_0619
      ELSEIF ((drdm_chg->log[drdm_log_loop].table_name="TIER_MATRIX"))
       EXECUTE dm2_ref_data_mover_1153
     ...
     IF (((drdm_no_trans_ind=1) OR (nodelete_ind=1)) )
      IF (drdm_mini_loop_status IN ("ORPHAN", "OLDVER", "BADLOG", "NOMV*"))
       IF ((global_mover_rec->one_pass_ind=0))
        ROLLBACK
       ENDIF
       IF ((drdm_chg->log[drdm_log_loop].process_flg != 6))
    ...
    IF ((((drdm_chg->log[drdm_log_loop].next_to_process=0)) OR (drdm_log_loop=0)) )
     ...
     CALL merge_audit("BATCH END",concat("The end of a batch was reached at log_id ",trim(cnvtstring(
         drdm_chg->log[drdm_log_loop].log_id))),1)
     CALL del_chg_log(drdm_chg->log[drdm_log_loop].table_name,"REFCHG",dm2_ref_data_doc->
      env_target_id)
    ELSE
     IF (drdm_log_loop <= drdm_log_cnt
      ...
      CALL del_chg_log(drdm_chg->log[drdm_log_loop].table_name,"REFCHG",dm2_ref_data_doc->
       env_target_id)
     ENDIF
     SET temp_tbl_cnt = locateval(simple_loop,1,dm2_ref_data_doc->tbl_cnt,drdm_chg->log[drdm_log_loop
      ].table_name,dm2_ref_data_doc->tbl_qual[simple_loop].table_name)
     ...
     IF (drdm_log_loop <= drdm_log_cnt
      ...
      IF ((global_mover_rec->one_pass_ind=1))
       SET drdm_log_loop = reprocess_move(drdm_log_loop,drdm_log_cnt)
      ENDIF
      IF (drdm_log_loop <= drdm_log_cnt)
       SET drdm_eraseable_ind = 0
       SET drdm_next_row_pos = (drdm_log_cnt+ 1)
 ...
 SUBROUTINE drdm_dcle_proc(null)
   ...
   ;end select
   IF (curqual > 0
    AND drdm_exception_time <= cnvtlookbehind("5H",sysdate))
    ROLLBACK
    UPDATE  FROM dm_info d
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE drdm_validate(sbr_dv_t_name,sbr_dv_pk_where,sbr_dv_table_cnt)
   ...
   DECLARE dv_table_suffix = vc
   DECLARE dv_s_tab_name = vc
   DECLARE dv_qual_cnt = i4
   DECLARE dv_pk_where_stmt = vc
   DECLARE dv_parent_table = vc
   DECLARE dv_col_name = vc
   DECLARE dv_type = vc
   ...
   DECLARE dv_fake_update = i4
   DECLARE dv_pe_name = vc
   DECLARE dv_pe_id = f8
   DECLARE dv_filter_ret = i2
   SET dv_table_suffix = dm2_rdds_get_tbl_alias(dm2_ref_data_doc->tbl_qual[sbr_dv_table_cnt].suffix)
   SET dv_s_tab_name = dm2_get_rdds_tname(sbr_dv_t_name)
   SET dv_qual_cnt = 0
   ...
   IF ((dm2_ref_data_doc->tbl_qual[sbr_dv_table_cnt].merge_delete_ind=1))
    ...
    IF (dv_pkw_col_cnt=dv_md_col_cnt)
     ...
     WHILE (dv_end_ind=0)
       SET dv_dummy_cnt = 0
       SET dv_dummy_cnt = findstring(" AND ",sbr_dv_pk_where,dv_and_pos,0)
       IF (dv_dummy_cnt=0)
        SET dv_end_ind = 1
       ELSE
        ...
        IF (dv_delim_start < size(sbr_dv_pk_where))
         ...
         IF (dv_delim_stop > 0)
          SET dv_col_value = substring((dv_delim_start+ 1),(dv_delim_stop - (dv_delim_start+ 1)),
           sbr_dv_pk_where)
          IF (findstring(" AND ",dv_col_value,0,0) > 0)
           SET dv_and_cnt = (dv_and_cnt - 1)
          ENDIF
   ...
   ELSE
    ...
    ENDIF
   ENDIF
   IF (sbr_dv_t_name="NAME_VALUE_PREFS")
    SET dv_fake_update = 0
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].child_flag = 2
    SET dv_pe_name = ""
    CALL parser(concat("select into 'nl:' from ",dv_s_tab_name," ",dv_table_suffix),0)
    CALL parser(concat(sbr_dv_pk_where," detail dv_pe_name = ",dv_table_suffix,
      ".PARENT_ENTITY_NAME with nocounter go"),1)
    IF (dv_pe_name != "")
     SELECT INTO "NL:"
      FROM dtable d
      WHERE d.table_name=dv_pe_name
      WITH nocounter
     ;end select
     IF (curqual=0)
      CALL echo(concat("The parent_entity_name value of ",dv_pe_name," is not a real table_name"))
     ELSE
      SET dv_fake_update = 0
      SELECT INTO "NL:"
       FROM dm_refchg_tab_r d
       WHERE d.child_table=sbr_dv_t_name
        AND d.parent_table=dv_pe_name
        AND d.process_type != "MOVER ONLY"
       DETAIL
        dv_col_name = d.parent_id_col, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].child_flag = 2,
        dv_fake_update = 1
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
     ...
     ENDIF
    ELSE
     CALL echo("No parent_entity_name value returned")
    ENDIF
   ELSEIF (sbr_dv_t_name IN ("LONG_TEXT", "LONG_TEXT_REFERENCE", "LONG_BLOB_REFERENCE"))
    SET dv_fake_update = 0
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].child_flag = 1
    SET dv_pe_name = ""
    SET dv_pe_id = 0
    CALL parser(concat("select into 'nl:' from ",dv_s_tab_name," ",dv_table_suffix),0)
    CALL parser(concat(sbr_dv_pk_where," detail dv_pe_name = ",dv_table_suffix,".PARENT_ENTITY_NAME "
      ),0)
    CALL parser(concat(" dv_pe_id = ",dv_table_suffix,".PARENT_ENTITY_ID with nocounter go"),1)
    IF (dv_pe_name != "")
     SELECT INTO "NL:"
      FROM dtable d
      WHERE d.table_name=dv_pe_name
     ...
     ELSE
      ...
      SELECT INTO "NL:"
       ...
       WHERE d.child_table=sbr_dv_t_name
        ...
        AND d.process_type="MOVER ONLY"
       DETAIL
        dv_col_name = d.parent_id_col, dv_fake_update = 1
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (dv_fake_update=1
    AND  NOT (sbr_dv_t_name IN ("LONG_TEXT", "LONG_TEXT_REFERENCE", "LONG_BLOB_REFERENCE")))
    RETURN(- (3))
   ELSEIF (dv_fake_update=1
    AND sbr_dv_t_name IN ("LONG_TEXT", "LONG_TEXT_REFERENCE", "LONG_BLOB_REFERENCE"))
    RETURN(- (3))
   ELSEIF (sbr_dv_t_name != "NAME_VALUE_PREFS")
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].child_flag = 1
   ENDIF
   RETURN(dv_ret_cnt)
 END ;Subroutine
 SUBROUTINE create_dcl_row(cdr_t_name,cdr_pk_where,cdr_idx_pos)
   DECLARE cdr_pk_where_value = f8
   DECLARE cdr_dcl_name = vc
   DECLARE cdr_log_id = f8
   DECLARE cdr_updt_cnt = i4
   SET cdr_dcl_name = dm2_get_rdds_tname("DM_CHG_LOG")
   SET stat = alterlist(usi_request->qual,1)
   SET usi_request->qual[1].str_from = cdr_pk_where
   EXECUTE dm_merge_unique_sql_id
   IF ((usi_reply->err_ind != 0))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = usi_reply->err_msg
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ROLLBACK
    RETURN(1)
   ELSE
    SET cdr_pk_where_value = cnvtreal(usi_request->qual[1].str_to)
    SELECT INTO "NL:"
     FROM (parser(cdr_dcl_name) d)
     WHERE d.pk_where_value=cdr_pk_where_value
      AND cnvtupper(d.table_name)=cdr_t_name
      AND d.pk_where=cdr_pk_where
      AND cnvtupper(d.log_type)="REFCHG"
      AND ((d.target_env_id+ 0)=dm2_ref_data_doc->env_target_id)
     DETAIL
      cdr_log_id = d.log_id, cdr_updt_cnt = d.updt_cnt
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (curqual=0)
     ...
     INSERT  FROM dm_chg_log d
      SET d.log_id = cdr_log_id, d.log_type = "REFCHG", d.table_name = cdr_t_name,
       d.target_env_id = dm2_ref_data_doc->env_target_id, d.pk_where = cdr_pk_where, d.pk_where_value
        = cdr_pk_where_value,
       d.updt_cnt = cdr_updt_cnt, d.chg_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_dt_tm =
       cnvtdatetime(curdate,curtime3),
       d.delete_ind = 0, d.updt_id = 0, d.updt_applctx = 0,
       d.updt_task = 0
      WITH nocounter
     ;end insert
     IF (check_error(dm_err->eproc)=1)
     ...
     ENDIF
    ELSE
     SET cdr_updt_cnt = (cdr_updt_cnt+ 1)
     UPDATE  FROM dm_chg_log d
      SET d.updt_cnt = cdr_updt_cnt
      WHERE d.pk_where_value=cdr_pk_where_value
       AND cnvtupper(d.table_name)=cdr_t_name
       AND d.pk_where=cdr_pk_where
       AND cnvtupper(d.log_type)="REFCHG"
     ...
     ENDIF
    ENDIF
    SET drdm_chg->log[cdr_idx_pos].log_id = cdr_log_id
    SET drdm_chg->log[cdr_idx_pos].updt_cnt = cdr_updt_cnt
   ENDIF
   RETURN(0)
 END ;Subroutine
 SUBROUTINE drdm_get_merge_del_rows(sbr_t_name,sbr_pk_where,sbr_tbl_cnt)
   ...
   CALL parser(
    " detail gmdr_qual_cnt = gmdr_qual_cnt + 1 stat = alterlist(cust_cs_rows->qual, gmdr_qual_cnt) ",
    0)
   FOR (gmdr_col_loop = 1 TO dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].col_cnt)
     IF ((dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].col_qual[gmdr_col_loop].pk_ind=1))
   ...
   IF (gmdr_top_level > 0)
    FOR (gmdr_col_loop = 1 TO gmdr_qual_cnt)
      CALL parser(concat("set gmdr_pk_value = cust_cs_rows->qual[gmdr_col_loop].",dm2_ref_data_doc->
        tbl_qual[sbr_tbl_cnt].col_qual[gmdr_top_level].column_name," go"),1)
      SET gmdr_xlat_return = select_merge_translate(cnvtstring(gmdr_pk_value),dm2_ref_data_doc->
       tbl_qual[sbr_tbl_cnt].table_name)
      IF (gmdr_xlat_return="No Trans")
       SET drdm_parser->statement[1].frag = concat("select into 'nl:' y=seq(",dm2_ref_data_doc->
        tbl_qual[sbr_tbl_cnt].col_qual[gmdr_top_level].sequence_name,", nextval) ")
       SET drdm_parser->statement[2].frag = "from dual detail gmdr_new_seq = y with nocounter go"
       CALL parse_statements(2)
       SET mmc_imt_check = insert_merge_translate(gmdr_pk_value,gmdr_new_seq,dm2_ref_data_doc->
        tbl_qual[sbr_tbl_cnt].table_name)
      ENDIF
    ENDFOR
    IF ((((global_mover_rec->one_pass_ind=0)) OR (drdm_log_loop <= drdm_log_cnt)) )
     COMMIT
    ENDIF
   ENDIF
   FOR (gmdr_col_loop = 1 TO gmdr_qual_cnt)
      ...
      IF ((dm2_ref_data_doc->tbl_qual[dcpw_table_cnt].col_qual[dcpw_col_loop].merge_delete_ind=1))
       ...
       "I4", "I2")))
        CALL parser(concat("set dcpw_pk_str = concat(dcpw_pk_str, ' ",dcpw_suffix,".",
          ...
          col_qual[dcpw_col_loop].column_name,")),'.0') go"),1)
     ...
     next_to_process=0))) )
      IF ((drdm_chg->log[fnc_log_loop].next_to_process != 0))
       ...
       IF ((dm2_ref_data_doc->tbl_qual[fnc_ndx_pos].child_flag=0))
        SELECT INTO "NL:"
         FROM dm_refchg_tab_r d
         WHERE (d.child_table=drdm_chg->log[fnc_log_loop].table_name)
          AND d.process_type != "MOVER ONLY"
         DETAIL
          dm2_ref_data_doc->tbl_qual[fnc_ndx_pos].child_flag = 2
         WITH nocounter
        ;end select
        IF (curqual=0)
       ...
       IF ((dm2_ref_data_doc->tbl_qual[fnc_ndx_pos].child_flag=1))
        ...
        ENDIF
       ELSE
        IF ((drdm_chg->log[fnc_log_loop].delete_ind=1))
         SET fnc_return_pos = fnc_log_loop
         SET fnc_done_ind = 1
        ELSE
         SET fnc_log_loop = drdm_chg->log[fnc_log_loop].next_to_process
 ...
 SUBROUTINE drdm_get_child_rows(sbr_t_name,sbr_pk_where,sbr_tbl_cnt,sbr_next_row)
   ...
   IF ((dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_flag=0))
    SELECT INTO "NL:"
     FROM dm_refchg_tab_r d
     WHERE d.parent_table=sbr_t_name
     ORDER BY d.process_seq
     DETAIL
      dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_flag = 2, gcr_child_cnt = (gcr_child_cnt+ 1),
      stat = alterlist(dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual,gcr_child_cnt),
      dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[gcr_child_cnt].child_name = d.child_table,
      dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[gcr_child_cnt].parent_id_col = d
      .parent_id_col, dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[gcr_child_cnt].
      parent_tab_col = d.parent_tab_col
     WITH nocounter
    ;end select
    IF (curqual=0)
    ...
    ENDIF
   ENDIF
   IF ((dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_flag=2))
    SET gcr_child_cnt = size(dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual,5)
    FOR (gcr_child_loop = 1 TO gcr_child_cnt)
      SET gcr_col_pass_cnt = 0
      SET gcr_qual_cnt = 0
      SELECT INTO "NL:"
       FROM (parser(gcr_user_tab) d)
       WHERE (d.table_name=dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[gcr_child_loop].
       child_name)
       DETAIL
        dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[gcr_child_loop].in_src_ind = 1
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(- (1))
      ENDIF
      IF ((dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[gcr_child_loop].in_src_ind=1))
       SET gcr_child_tbl_cnt = locateval(gcr_loop,1,size(dm2_ref_data_doc->tbl_qual,5),
        dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[gcr_child_loop].child_name,
        dm2_ref_data_doc->tbl_qual[gcr_loop].table_name)
       IF (gcr_child_tbl_cnt=0)
        SET gcr_loop = temp_tbl_cnt
        SET gcr_child_tbl_cnt = fill_rs("TABLE",dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[
         gcr_child_loop].child_name)
        SET temp_tbl_cnt = gcr_loop
       ENDIF
       IF (gcr_child_tbl_cnt=0)
        CALL echo("Could not find meta-data on child table.")
        RETURN(- (1))
       ENDIF
       SET gcr_table_suffix = dm2_rdds_get_tbl_alias(dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].
        suffix)
       SET gcr_s_tab_name = dm2_get_rdds_tname(dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[
        gcr_child_loop].child_name)
       FREE RECORD cust_cs_rows
       CALL parser("record cust_cs_rows (",0)
       CALL parser(" 1 qual[*]",0)
       FOR (gcr_col_loop = 1 TO dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_cnt)
         IF ((dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_qual[gcr_col_loop].pk_ind=1))
          CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_qual[
            gcr_col_loop].column_name," = ",dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_qual[
            gcr_col_loop].data_type),0)
          CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_qual[
            gcr_col_loop].column_name,"_NULLIND = i2 "),0)
         ENDIF
       ENDFOR
       CALL parser(") go",1)
       IF (check_error(dm_err->eproc)=1)
       ...
       FOR (gcr_col_loop = 1 TO dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_cnt)
         IF ((dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_qual[gcr_col_loop].pk_ind=1))
          IF (gcr_temp_cnt > 0)
           CALL parser(" , ",0)
          ENDIF
          SET gcr_temp_cnt = (gcr_temp_cnt+ 1)
          CALL parser(concat("var",cnvtstring(gcr_col_loop)," = nullind(",gcr_table_suffix,".",
            dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_qual[gcr_col_loop].column_name,")"),0)
         ENDIF
       ENDFOR
       CALL parser(concat(" from  ",gcr_s_tab_name," ",gcr_table_suffix," where "),0)
       IF ((dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[gcr_child_loop].parent_tab_col != "")
       )
        CALL parser(concat(gcr_table_suffix,".",dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[
          gcr_child_loop].parent_tab_col," = '",sbr_t_name,
          "' and "),0)
       ENDIF
       CALL parser(concat(gcr_table_suffix,".",dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].parent_qual[
         gcr_child_loop].parent_id_col," in (select "),0)
       FOR (gcr_col_loop = 1 TO dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].col_cnt)
         IF ((dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].col_qual[gcr_col_loop].pk_ind=1))
          CALL parser(concat(gcr_p_table_suffix,".",dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].col_qual[
            gcr_col_loop].column_name," from ",gcr_p_tab_source,
            " ",gcr_p_table_suffix),0)
         ENDIF
       ENDFOR
       CALL parser(concat(" ",sbr_pk_where,") "),0)
       FOR (gcr_col_loop = 1 TO dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_cnt)
         IF ((dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_qual[gcr_col_loop].pk_ind=1)
          AND (dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_qual[gcr_col_loop].data_type="F8"))
          SET gcr_col_pass_cnt = (gcr_col_pass_cnt+ 1)
          IF (gcr_col_pass_cnt=1)
           CALL parser(concat(" and (",gcr_table_suffix,".",dm2_ref_data_doc->tbl_qual[
             gcr_child_tbl_cnt].col_qual[gcr_col_loop].column_name," != 0"),0)
          ELSE
           CALL parser(concat(" or ",gcr_table_suffix,".",dm2_ref_data_doc->tbl_qual[
             gcr_child_tbl_cnt].col_qual[gcr_col_loop].column_name," != 0"),0)
          ENDIF
         ENDIF
       ENDFOR
       CALL parser(
        ") detail gcr_qual_cnt = gcr_qual_cnt + 1 stat = alterlist(cust_cs_rows->qual, gcr_qual_cnt) ",
        0)
       FOR (gcr_col_loop = 1 TO dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_cnt)
         IF ((dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_qual[gcr_col_loop].pk_ind=1))
          CALL parser(concat("cust_cs_rows->qual[gcr_qual_cnt].",dm2_ref_data_doc->tbl_qual[
            gcr_child_tbl_cnt].col_qual[gcr_col_loop].column_name," = ",gcr_table_suffix,".",
            dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].col_qual[gcr_col_loop].column_name),0)
          CALL parser(concat("cust_cs_rows->qual[gcr_qual_cnt].",dm2_ref_data_doc->tbl_qual[
            gcr_child_tbl_cnt].col_qual[gcr_col_loop].column_name,"_NULLIND = var",trim(cnvtstring(
              gcr_col_loop))),0)
         ENDIF
       ENDFOR
       CALL parser(" with nocounter go",1)
       IF (check_error(dm_err->eproc)=1)
        ...
        RETURN(- (1))
       ENDIF
       FOR (gcr_col_loop = 1 TO gcr_qual_cnt)
        SET gcr_pk_where_stmt = drdm_create_pk_where(gcr_col_loop,gcr_child_tbl_cnt,"PK")
        IF (gcr_pk_where_stmt != "")
         IF (drdm_next_row_pos > drdm_max_rs_size)
          SET drdm_max_rs_size = (drdm_max_rs_size+ 10)
          SET stat = alterlist(drdm_chg->log,drdm_max_rs_size)
         ENDIF
         CALL reinitialize_drdm(drdm_next_row_pos)
         IF (gcr_col_loop=1
          AND gcr_rtm_rs_size=drdm_log_loop)
          SET gcr_rtm_rs_size = drdm_next_row_pos
         ENDIF
         SET drdm_chg->log[drdm_next_row_pos].next_to_process = (drdm_next_row_pos+ 1)
         SET drdm_chg->log[drdm_next_row_pos].table_name = dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].
         parent_qual[gcr_child_loop].child_name
         SET drdm_chg->log[drdm_next_row_pos].pk_where = gcr_pk_where_stmt
         SET drdm_chg->log[drdm_next_row_pos].validate_ind = 0
         SET drdm_chg->log[drdm_next_row_pos].log_id = drdm_chg->log[drdm_log_loop].log_id
         SET drdm_chg->log[drdm_next_row_pos].updt_cnt = drdm_chg->log[drdm_log_loop].updt_cnt
         SET drdm_chg->log[drdm_next_row_pos].commit_ind = 0
         ...
         ELSE
          SET drdm_chg->log[drdm_next_row_pos].process_flg = 2
          SET drdm_chg->log[drdm_next_row_pos].status_flg = 2
         ENDIF
         IF ((drdm_chg->log[drdm_log_loop].par_location=0))
          SET drdm_chg->log[drdm_next_row_pos].par_location = drdm_log_loop
         ELSE
          SET drdm_chg->log[drdm_next_row_pos].par_location = drdm_chg->log[drdm_log_loop].
          par_location
         ENDIF
         IF ((dm2_ref_data_doc->tbl_qual[gcr_child_tbl_cnt].merge_delete_ind=1))
          IF (gcr_col_loop=1)
           SET drdm_chg->log[drdm_next_row_pos].md_delete_ind = 1
          ELSE
           SET drdm_chg->log[drdm_next_row_pos].md_delete_ind = 0
         ...
         ELSE
          SET drdm_chg->log[drdm_next_row_pos].md_delete_ind = 0
         ENDIF
         IF ((dm2_ref_data_doc->tbl_qual[sbr_tbl_cnt].merge_delete_ind=0))
          IF (gcr_col_loop=gcr_qual_cnt)
           SET drdm_chg->log[drdm_next_row_pos].commit_ind = 1
           SET drdm_chg->log[drdm_log_loop].commit_ind = 0
          ENDIF
         ENDIF
         SET drdm_next_row_pos = (drdm_next_row_pos+ 1)
        ELSE
      ...
      ENDIF
    ENDFOR
    IF (gcr_rtm_rs_size != drdm_log_loop)
     IF (sbr_next_row >= gcr_rtm_rs_size)
      SET drdm_chg->log[(drdm_next_row_pos - 1)].next_to_process = 0
     ELSE
      SET drdm_chg->log[(drdm_next_row_pos - 1)].next_to_process = sbr_next_row
     ENDIF
     SET drdm_chg->log[(drdm_next_row_pos - 1)].process_flg = drdm_chg->log[drdm_log_loop].
     process_flg
    ENDIF
   ENDIF
   ...
   IF ((global_mover_rec->one_pass_ind=1))
    ...
    FOR (amx_loop = 1 TO amx_rs_size)
      IF ((missing_xlats->qual[amx_loop].processed_ind=0))
       ...
       IF (amx_tab_pos=0)
        SET amx_ndx_search = temp_tbl_cnt
        SET amx_tab_pos = fill_rs("TABLE",missing_xlats->qual[amx_loop].table_name)
        SET temp_tbl_cnt = amx_ndx_search
       ENDIF
       IF ((dm2_ref_data_doc->tbl_qual[amx_tab_pos].child_flag=0))
        SELECT INTO "NL:"
         FROM dm_refchg_tab_r d
         WHERE (d.child_table=dm2_ref_data_doc->tbl_qual[amx_tab_pos].table_name)
          AND d.process_type != "MOVER ONLY"
         DETAIL
          dm2_ref_data_doc->tbl_qual[amx_tab_pos].child_flag = 2
         WITH nocounter
        ;end select
        IF (curqual=0)
         SET dm2_ref_data_doc->tbl_qual[amx_tab_pos].child_flag = 1
        ENDIF
       ENDIF
       IF ((((dm2_ref_data_doc->tbl_qual[amx_tab_pos].child_flag=2)) OR ((((dm2_ref_data_doc->
       tbl_qual[amx_tab_pos].table_name="TIER_MATRIX")) OR ( NOT ((dm2_ref_data_doc->tbl_qual[
       amx_tab_pos].custom_script IN ("", " "))))) )) )
        SET amx_loop = amx_rs_size
        SET amx_return_ind = 1
    ...
    FOR (amx_loop = 1 TO amx_rs_size)
      IF ((missing_xlats->qual[amx_loop].processed_ind=0))
       ...
       IF ((dm2_ref_data_doc->tbl_qual[amx_tab_pos].merge_delete_ind=1))
        SET amx_s_tab_name = dm2_get_rdds_tname(dm2_ref_data_doc->tbl_qual[amx_tab_pos].table_name)
        FREE RECORD cust_cs_rows
        CALL parser("record cust_cs_rows (",0)
        CALL parser(" 1 qual[*]",0)
        FOR (amx_ndx_search = 1 TO dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_cnt)
          IF ((dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_qual[amx_ndx_search].merge_delete_ind=1))
           CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_qual[amx_ndx_search].
             column_name," = ",dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_qual[amx_ndx_search].
             data_type),0)
           CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_qual[amx_ndx_search].
             column_name,"_NULLIND = i2 "),0)
          ENDIF
        ...
        FOR (amx_ndx_search = 1 TO dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_cnt)
          IF ((dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_qual[amx_ndx_search].merge_delete_ind=1))
           IF (amx_temp_cnt > 0)
            CALL parser(" , ",0)
           ENDIF
           SET amx_temp_cnt = (amx_temp_cnt+ 1)
           CALL parser(concat("var",cnvtstring(amx_ndx_search)," = nullind(",amx_suffix,".",
             dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_qual[amx_ndx_search].column_name,")"),0)
          ENDIF
        ENDFOR
        CALL parser(concat(" from  ",amx_s_tab_name," ",amx_suffix," where "),0)
        CALL parser(concat(amx_suffix,".",missing_xlats->qual[amx_loop].column_name," =",trim(
           cnvtstring(missing_xlats->qual[amx_loop].missing_value,15)),
          ".0"),0)
        IF ((missing_xlats->qual[amx_loop].table_name IN ("VIEW_PREFS", "VIEW_COMP_PREFS",
        "DETAIL_PREFS")))
         CALL parser(concat(" and ",amx_suffix,".PRSNL_ID  = 0 "),0)
        ENDIF
        CALL parser(
         " detail amx_qual_cnt = amx_qual_cnt + 1 stat = alterlist(cust_cs_rows->qual, amx_qual_cnt) ",
         0)
        FOR (amx_ndx_search = 1 TO dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_cnt)
          IF ((dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_qual[amx_ndx_search].merge_delete_ind=1))
           CALL parser(concat("cust_cs_rows->qual[amx_qual_cnt].",dm2_ref_data_doc->tbl_qual[
             amx_tab_pos].col_qual[amx_ndx_search].column_name," = ",amx_suffix,".",
             dm2_ref_data_doc->tbl_qual[amx_tab_pos].col_qual[amx_ndx_search].column_name),0)
           CALL parser(concat("cust_cs_rows->qual[amx_qual_cnt].",dm2_ref_data_doc->tbl_qual[
             amx_tab_pos].col_qual[amx_ndx_search].column_name,"_NULLIND = var",trim(cnvtstring(
               amx_ndx_search))),0)
          ENDIF
        ENDFOR
        CALL parser(" with nocounter go",1)
        ...
        IF (amx_qual_cnt=0)
         ...
         SET amx_qual_cnt = 1
         SET amx_s_tab_name = dm2_get_rdds_tname("DM_CHG_LOG_EXCEPTION")
         ROLLBACK
         UPDATE  FROM (parser(amx_s_tab_name) d)
          SET d.log_type = "NOMV10"
          WHERE (d.target_env_id=dm2_ref_data_doc->env_target_id)
           AND (d.table_name=missing_xlats->qual[amx_loop].table_name)
           AND (d.column_name=missing_xlats->qual[amx_loop].column_name)
           AND (d.from_value=missing_xlats->qual[amx_loop].missing_value)
          WITH nocounter
         ;end update
         IF (check_error(dm_err->eproc)=1)
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         ...
         ENDIF
         IF (curqual=0)
          INSERT  FROM (parser(amx_s_tab_name) d)
           SET d.log_type = "NOMV10", d.target_env_id = dm2_ref_data_doc->env_target_id, d.table_name
             = missing_xlats->qual[amx_loop].table_name,
            d.column_name = missing_xlats->qual[amx_loop].column_name, d.from_value = missing_xlats->
            qual[amx_loop].missing_value
           WITH nocounter
          ;end insert
          IF (check_error(dm_err->eproc)=1)
           CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         ...
         COMMIT
        ELSE
         SET amx_pk_where_stmt = drdm_create_pk_where(amx_qual_cnt,amx_tab_pos,"MD")
         IF (amx_pk_where_stmt="")
          RETURN(amx_return_pos)
         ENDIF
        ENDIF
       ELSE
        SET amx_pk_where_stmt = concat("WHERE ",amx_suffix,".",missing_xlats->qual[amx_loop].
         column_name," =",
         trim(cnvtstring(missing_xlats->qual[amx_loop].missing_value,15)),".0")
       ENDIF
       ...
       IF (amx_first_added_ind=0)
        ...
        SET drdm_next_row_pos = (drdm_next_row_pos+ 1)
       ENDIF
       IF (amx_loop=amx_rs_size)
        SET drdm_chg->log[drdm_next_row_pos].next_to_process = amx_log_loop
       ELSE
        SET drdm_chg->log[drdm_next_row_pos].next_to_process = (drdm_next_row_pos+ 1)
       ENDIF
       SET drdm_chg->log[drdm_next_row_pos].table_name = missing_xlats->qual[amx_loop].table_name
       SET drdm_chg->log[drdm_next_row_pos].pk_where = amx_pk_where_stmt
       SET drdm_chg->log[drdm_next_row_pos].validate_ind = 1
       SET drdm_chg->log[drdm_next_row_pos].log_id = drdm_chg->log[amx_log_loop].log_id
       SET drdm_chg->log[drdm_next_row_pos].updt_cnt = 0
       SET drdm_chg->log[drdm_next_row_pos].commit_ind = drdm_chg->log[amx_log_loop].commit_ind
 ...
 SUBROUTINE parse_contexts(pc_context_str,pc_delim,pc_context_rs)
   ...
   RETURN(pc_num_found)
 END ;Subroutine
 SUBROUTINE reset_updt_cnt(ruc_rs_idx)
   DECLARE ruc_pair_size = i4
   DECLARE ruc_loop = i4
   DECLARE ruc_tab_name = vc
   SET ruc_pair_size = size(drdm_pair_info->qual,5)
   FOR (ruc_loop = 1 TO ruc_pair_size)
    SET drdm_pair_info->qual[ruc_loop].log_id = 0
    SET drdm_pair_info->qual[ruc_loop].updt_cnt = 0
   ENDFOR
   SET ruc_loop = 0
   SET ruc_tab_name = dm2_get_rdds_tname("DM_CHG_LOG")
   SELECT INTO "nl:"
    FROM (parser(ruc_tab_name) d)
    WHERE ((d.target_env_id+ 0)=dm2_ref_data_doc->env_target_id)
     AND (d.table_name=drdm_chg->log[ruc_rs_idx].table_name)
     AND (d.pk_where=drdm_chg->log[ruc_rs_idx].pk_where)
     AND (d.pk_where_value=drdm_chg->log[ruc_rs_idx].pk_where_value)
     AND d.log_type IN ("REFCHG", "NORDDS", "PROCES")
    DETAIL
     ruc_loop = (ruc_loop+ 1)
     IF (ruc_loop > ruc_pair_size)
      ruc_pair_size = (ruc_pair_size+ 10), stat = alterlist(drdm_pair_info->qual,ruc_pair_size)
     ENDIF
     drdm_pair_info->qual[ruc_loop].log_id = d.log_id, drdm_pair_info->qual[ruc_loop].updt_cnt = d
     .updt_cnt
    WITH nocounter
   ;end select
   SET ruc_pair_size = locateval(ruc_loop,1,size(drdm_pair_info->qual,5),drdm_chg->log[ruc_rs_idx].
    log_id,drdm_pair_info->qual[ruc_loop].log_id)
   SET drdm_chg->log[ruc_rs_idx].updt_cnt = drdm_pair_info->qual[ruc_pair_size].updt_cnt
 END ;Subroutine
#done
