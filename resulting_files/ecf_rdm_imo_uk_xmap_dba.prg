CREATE PROGRAM ecf_rdm_imo_uk_xmap:dba
 ...
 DECLARE terminology_group = vc WITH public, noconstant("UK")
 SET kia_tbl_desc = "IMO UK XMAP"
 SET kia_short_desc = "IMO UK XMAP"
 SET kia_rdm_log_name = "ecf_rdm_imo_uk_xmap.log"
 SET kia_rdm_nbr = 10186
 SET kia_rdm_vocab_mean = "IMO"
 SET readme_data->status = "F"
 SET readme_data->message = concat("Failed - ",trim(kia_tbl_desc))
 SET script_status = " "
 SET csv_stat = 1
 DECLARE cur_internal = c1 WITH noconstant("F")
 SELECT INTO "nl:"
  FROM dm_info d
  PLAN (d
   WHERE d.info_domain="External Content Factory"
    AND d.info_name="INTERNAL_BUILD")
  DETAIL
   cur_internal = "S"
  WITH nocounter
 ;end select
 ...
 IF (curqual > 0)
  IF (info_char_var != "UK")
   CALL echo("This is not a IMO UK Domain.")
   SET readme_data->status = cur_internal
   SET readme_data->message = "Failure - Cannot load IMO UK content in a Non-UK CMT Domain"
   GO TO exit_script
  ENDIF
 ELSE
  INSERT  FROM dm_info
   SET info_domain = "External Content Factory", info_char = "UK", info_name = "IMO_DIALECT",
    info_date = cnvtdatetime(curdate,curtime3), info_number = 1.0, info_long_id = 0,
 ...
 EXECUTE kia_dm_dbimport "cer_install:imo_uk_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("ecf_rdm_imo_uk_xmap Begin> ",format(cnvtdatetime(curdate,curtime3),
 ...
 IF (kia_obj_suc_ind=0)
  EXECUTE kia_load_xmap kia_rdm_vocab_mean, terminology_group
 ...
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(10182,"IMO UK NOMEN"))
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(10183,"IMO UK CONCEPT"))
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(10184,"IMO UK RELTN"))
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(10186,"IMO UK XMAP"))
