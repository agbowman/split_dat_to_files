CREATE PROGRAM acm_get_pds_excep_by_person:dba
 IF (validate(reply,"-999")="-999")
  RECORD reply(
    1 pds_exception_id = f8
    1 source_version_number = vc
    1 status_data
      2 status = c1
      2 subeventstatus[1]
        3 operationname = c25
 ...
 SET failed = false
 SELECT INTO "nl:"
  ppp.pm_post_process_id, ppp.source_version_number
  FROM pm_post_process ppp
  WHERE (ppp.person_id=request->person_id)
   AND ((ppp.organization_id+ 0)=request->organization_id)
   AND ((ppp.pm_post_process_type_cd+ 0)=pdsexception_code)
  DETAIL
   count = (count+ 1), reply->pds_exception_id = ppp.pm_post_process_id, reply->source_version_number
    = ppp.source_version_number,
   reply->process_status_cd = ppp.process_status_cd
  WITH nocounter
 ;end select
 IF (count=0)
  SET failed = none_found
  SET table_name = "no records found"
  GO TO exit_script
 ENDIF
 IF (count > 1)
  SET failed = duplicate_error
  SET table_name = "multiple records were found"
  GO TO exit_script
 ENDIF
#exit_script
 IF (failed=false)
  SET reply->status_data.status = "S"
 ELSEIF (failed=none_found)
