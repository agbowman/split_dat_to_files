CREATE PROGRAM dm_rmc_rpt_gen_audit:dba
 ...
 DECLARE drrga_query_loop = i4 WITH protect, noconstant(0)
 DECLARE drrga_alt_loop = i4 WITH protect, noconstant(0)
 DECLARE drrga_tab_idx = i4 WITH protect, noconstant(0)
 DECLARE drrga_pk_cnt = i4 WITH protect, noconstant(0)
 DECLARE drrga_pk_col = vc WITH protect, noconstant(" ")
 DECLARE drrga_comma_ind = i2 WITH protect, noconstant(0)
 ...
 FREE RECORD drgcq_request
 RECORD drgcq_request(
   1 table_name = vc
   1 db_link = vc
   1 query_type = i4
 )
 IF (validate(drrga_reply)=0)
 ...
 ENDIF
 IF (check_logfile("DM_RMC_RPT_GEN",".log","DM_RMC_RPT_GEN_AUDIT LogFile")=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_program
 ENDIF
 IF (validate(drrga_request)=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No DRRGA_REQUEST structure defined."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF (daf_is_blank(drrga_request->table_name))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No table name supplied through DRRGA_REQUEST structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF (daf_is_blank(drrga_request->db_link))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No database link supplied through DRRGA_REQUEST structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 SET drgcq_request->query_type = 1
 SET drgcq_request->db_link = drrga_request->db_link
 SET drgcq_request->table_name = drrga_request->table_name
 EXECUTE dm_rmc_gen_comp_query
 IF (check_error("Executing DM_RMC_GEN_COMP_QUERY")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 SET drrga_tab_idx = locateval(drrga_tab_idx,1,tmc_md_info->tbl_cnt,drrga_request->table_name,
  tmc_md_info->tbl_qual[drrga_tab_idx].table_name)
 IF (drrga_tab_idx=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No metadata was loaded for table ",drrga_request->table_name)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 FOR (drrga_alt_loop = 1 TO tmc_md_info->tbl_qual[drrga_tab_idx].col_cnt)
   IF ((tmc_md_info->tbl_qual[drrga_tab_idx].col_qual[drrga_alt_loop].pk_ind=1))
    SET drrga_pk_cnt = (drrga_pk_cnt+ 1)
    IF ((tmc_md_info->tbl_qual[drrga_tab_idx].col_qual[drrga_alt_loop].data_type="F8"))
     SET drrga_pk_col = tmc_md_info->tbl_qual[drrga_tab_idx].col_qual[drrga_alt_loop].column_name
    ENDIF
   ENDIF
 ENDFOR
 FOR (drrga_query_loop = 1 TO query_data->query_cnt)
   SET drrga_reply->query_cnt = (drrga_reply->query_cnt+ 1)
   SET stat = alterlist(drrga_reply->query_qual,drrga_reply->query_cnt)
   SET drrga_comma_ind = 0
   IF ((query_data->query_cnt=1))
    SET drrga_reply->query_qual[drrga_query_loop].query_name = drrga_request->table_name
   ELSE
    SET drrga_reply->query_qual[drrga_query_loop].query_name = concat(drrga_request->table_name,"::",
     trim(cnvtstring(drrga_query_loop)))
   ENDIF
   SET drrga_reply->query_qual[drrga_query_loop].query_text = "SELECT into '<FILE_NAME>' "
   FOR (drrga_alt_loop = 1 TO query_data->query_qual[drrga_query_loop].col_cnt)
     IF ((query_data->query_qual[drrga_query_loop].col_qual[drrga_alt_loop].display_flag=1))
      IF (drrga_comma_ind=1)
       SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
        drrga_query_loop].query_text,",")
      ENDIF
      SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
       drrga_query_loop].query_text," ",query_data->query_qual[drrga_query_loop].col_qual[
       drrga_alt_loop].column_alias," = ",query_data->query_qual[drrga_query_loop].col_qual[
       drrga_alt_loop].table_alias,
       ".",query_data->query_qual[drrga_query_loop].col_qual[drrga_alt_loop].column_name)
      SET drrga_comma_ind = 1
     ENDIF
   ENDFOR
   SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
    drrga_query_loop].query_text," <SELECT_TAG>")
   FOR (drrga_alt_loop = 1 TO query_data->query_qual[drrga_query_loop].table_cnt)
     IF ((query_data->query_qual[drrga_query_loop].table_qual[drrga_alt_loop].join_type_flag=1))
      SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
       drrga_query_loop].query_text," FROM ",query_data->query_qual[drrga_query_loop].table_qual[
       drrga_alt_loop].table_name,"<DB_LINK_TAG> ",query_data->query_qual[drrga_query_loop].
       table_qual[drrga_alt_loop].table_alias)
     ELSEIF ((query_data->query_qual[drrga_query_loop].table_qual[drrga_alt_loop].join_type_flag=2))
      SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
       drrga_query_loop].query_text," ,(LEFT JOIN ",query_data->query_qual[drrga_query_loop].
       table_qual[drrga_alt_loop].table_name,"<DB_LINK_TAG> ",query_data->query_qual[drrga_query_loop
       ].table_qual[drrga_alt_loop].table_alias,
       " on (",query_data->query_qual[drrga_query_loop].table_qual[drrga_alt_loop].join_on_clause,
       ")) ")
     ELSEIF ((query_data->query_qual[drrga_query_loop].table_qual[drrga_alt_loop].join_type_flag=3))
      SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
       drrga_query_loop].query_text," ,(RIGHT JOIN ",query_data->query_qual[drrga_query_loop].
       table_qual[drrga_alt_loop].table_name,"<DB_LINK_TAG> ",query_data->query_qual[drrga_query_loop
       ].table_qual[drrga_alt_loop].table_alias,
       " on (",query_data->query_qual[drrga_query_loop].table_qual[drrga_alt_loop].join_on_clause,
       ")) ")
   ...
   ENDFOR
   SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
    drrga_query_loop].query_text," , DM_REFCHG_COMP_GTTD S")
   FOR (drrga_alt_loop = 1 TO query_data->query_qual[drrga_query_loop].table_cnt)
    SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
     drrga_query_loop].query_text," ",query_data->query_qual[drrga_query_loop].table_qual[
     drrga_alt_loop].join_clause)
    IF (drrga_alt_loop=1)
     IF (drrga_pk_cnt=1
      AND daf_is_not_blank(drrga_pk_col))
      SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
       drrga_query_loop].query_text," "," JOIN S WHERE S.R_PTAM_HASH_VALUE = t",tmc_md_info->
       tbl_qual[drrga_tab_idx].suffix,".",
       drrga_pk_col," and S.STATUS = <DOMAIN_FLAG> ")
     ELSE
      SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
       drrga_query_loop].query_text," "," JOIN S WHERE S.R_ROWID  = t",tmc_md_info->tbl_qual[
       drrga_tab_idx].suffix,".",
       "ROWID and S.STATUS = <DOMAIN_FLAG> ")
     ENDIF
    ENDIF
   ENDFOR
   SET drrga_reply->query_qual[drrga_query_loop].query_text = concat(drrga_reply->query_qual[
    drrga_query_loop].query_text,
    ^ with nocounter, pcformat('"', ';',1), format=stream, maxcol=10000,check <WITH_TAG> ^)
 ENDFOR
#end_program
END GO
