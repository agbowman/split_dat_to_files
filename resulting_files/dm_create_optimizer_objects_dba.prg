CREATE PROGRAM dm_create_optimizer_objects:dba
 ...
 SUBROUTINE dcr_get_opt_mode_by_name(opt_name)
   RETURN(evaluate(opt_name,"RULE",1,"CHOOSE",2,
    ...
    "ALL_ROWS",8,0))
 END ;Subroutine
 DECLARE dcoo_cmd = vc WITH protect, noconstant(" ")
 DECLARE dcoo_desired_db_trigger = vc WITH protect, noconstant(" ")
 DECLARE dcoo_current_db_trigger = vc WITH protect, noconstant(" ")
 DECLARE dcoo_object_exists = i2 WITH protect, noconstant(0)
 DECLARE dcoo_object_status = vc WITH protect, noconstant("")
 DECLARE dcoo_procedure_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE dcoo_get_object_status(dgos_type=vc,dgos_name=vc,dgos_exists=i2(ref),dgos_status=vc(ref)) =
 i2
 DECLARE dcoo_drop_object(ddo_type=vc,ddo_name=vc) = i2
 IF (check_logfile("dm_create_opt_obj",".log","dm_create_opt_obj")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_CREATE_OPTIMIZER_OBJECTS"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (currdb != "ORACLE")
 ...
 IF ((dm2_rdbms_version->level1 > 10))
  SET dm_err->eproc = "Oracle Version > 10. Auto-successing dm_create_optimizer_objects."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 ...
 IF ((dcr_optvalues->implementer_opt_mode="RULE")
  ...
  IF (dcoo_drop_object("TRIGGER","TRG_DM_OPTIMIZER_INIT")=0)
   GO TO exit_script
  ENDIF
  IF (dcoo_drop_object("PROCEDURE","DM_OPTIMIZER_INIT")=0)
   GO TO exit_script
  ENDIF
  IF (dcoo_drop_object("TRIGGER","TRG_DM_DB_OPTIMIZER_INIT")=0)
   GO TO exit_script
  ENDIF
  IF (dcoo_drop_object("FUNCTION","DM2_GET_HASH_VALUE")=0)
   GO TO exit_script
  ENDIF
  GO TO exit_script
 ENDIF
 IF (dcoo_get_object_status("PROCEDURE","DM_OPTIMIZER_INIT",dcoo_procedure_exists_ind,
  dcoo_object_status)=0)
  GO TO exit_script
 ENDIF
 IF (dcoo_get_object_status("TRIGGER","TRG_DM_OPTIMIZER_INIT",dcoo_object_exists,dcoo_object_status)=
 0)
  GO TO exit_script
 ENDIF
 IF (((dcoo_object_exists=0) OR (((dcoo_object_status="INVALID") OR (dcoo_procedure_exists_ind)) )) )
  SET dm_err->eproc = "Creating trigger trg_dm_optimizer_init"
  SET dcoo_cmd = concat(
   " rdb asis(^ CREATE OR REPLACE TRIGGER TRG_DM_OPTIMIZER_INIT AFTER LOGON ON DATABASE",
   " DECLARE c_dm_info_exists char(1) := 'N'; CURSOR dmoi IS select ",
   " di.info_char from v500.DM_INFO di where di.info_domain = 'DM_SET_SESSION_PARAMETERS' order by di.info_number;",
   " BEGIN IF (USER LIKE 'V500%') THEN BEGIN select 'Y' into c_dm_info_exists from DBA_TABLES where owner = 'V500'",
  ...
  =0)
   GO TO exit_script
  ENDIF
  IF (((dcoo_object_exists=0) OR (dcoo_object_status="INVALID")) )
   SET dm_err->emsg = "Trigger TRG_DM_OPTIMIZER_INIT has not been created or is invalid."
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (dcoo_procedure_exists_ind)
  IF (dcoo_drop_object("PROCEDURE","DM_OPTIMIZER_INIT")=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Getting current trigger body from user_triggers."
 SELECT INTO "nl:"
  FROM user_triggers ut
  WHERE "TRG_DM_DB_OPTIMIZER_INIT"=ut.trigger_name
  DETAIL
   dcoo_current_db_trigger = ut.trigger_body
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining what are the system parameters from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE "DM_SET_SYSTEM_PARAMETERS"=di.info_domain
  ORDER BY di.info_number
  HEAD REPORT
   dcoo_desired_db_trigger = "BEGIN "
  DETAIL
   dcoo_desired_db_trigger = concat(dcoo_desired_db_trigger," EXECUTE IMMEDIATE '",trim(di.info_char,
     3),"'; ")
  FOOT REPORT
   dcoo_desired_db_trigger = concat(dcoo_desired_db_trigger,"EXCEPTION WHEN OTHERS THEN NULL; END;")
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(build("Current trigger body:[",dcoo_current_db_trigger,"]"))
  CALL echo(build("Desired trigger body:[",dcoo_desired_db_trigger,"]"))
 ENDIF
 IF (trim(dcoo_desired_db_trigger,3)="")
  IF (dcoo_drop_object("TRIGGER","TRG_DM_DB_OPTIMIZER_INIT")=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (trim(dcoo_desired_db_trigger,3) != ""
  AND trim(dcoo_current_db_trigger,3) != trim(dcoo_desired_db_trigger,3))
  SET dm_err->eproc = "Creating trigger trg_dm_db_optimizer_init"
  CALL disp_msg("",dm_err->logfile,0)
  SET dcoo_cmd = concat(
   "RDB ASIS(^ CREATE OR REPLACE TRIGGER TRG_DM_DB_OPTIMIZER_INIT AFTER STARTUP ON DATABASE ",
   dcoo_desired_db_trigger," ^) GO")
  IF (dm2_push_cmd(dcoo_cmd,1)=0)
   GO TO exit_script
  ENDIF
  IF (dcoo_get_object_status("TRIGGER","TRG_DM_DB_OPTIMIZER_INIT",dcoo_object_exists,
   dcoo_object_status)=0)
   GO TO exit_script
  ENDIF
  IF (((dcoo_object_exists=0) OR (dcoo_object_status="INVALID")) )
   SET dm_err->emsg = "Trigger TRG_DM_DB_OPTIMIZER_INIT has not been created or is invalid."
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (dcoo_get_object_status("FUNCTION","DM2_GET_HASH_VALUE",dcoo_object_exists,dcoo_object_status)=0)
  GO TO exit_script
 ENDIF
 IF (((dcoo_object_exists=0) OR (dcoo_object_status="INVALID")) )
  SET dm_err->eproc = "Creating function dm2_get_hash_value"
  CALL disp_msg("",dm_err->logfile,0)
  SET dcoo_cmd = concat("RDB ASIS(^ CREATE OR REPLACE FUNCTION DM2_GET_HASH_VALUE(str VARCHAR2)",
   "RETURN FLOAT AS BEGIN RETURN(DBMS_UTILITY.GET_HASH_VALUE(str, 0, 1073741824)); END; ^) GO")
  IF (dm2_push_cmd(dcoo_cmd,1)=0)
   GO TO exit_script
  ENDIF
  IF (dcoo_get_object_status("FUNCTION","DM2_GET_HASH_VALUE",dcoo_object_exists,dcoo_object_status)=0
  ...
  ENDIF
  IF (((dcoo_object_exists=0) OR (dcoo_object_status="INVALID")) )
   SET dm_err->emsg = "FUNCTION DM2_GET_HASH_VALUE has not been created or is invalid."
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ...
 IF ((dm2_rdbms_version->level1 >= 11))
  SET dm_err->eproc = "Set default value of no_invalidate parameter to FALSE"
  CALL disp_msg("",dm_err->logfile,0)
  SET dcoo_cmd = concat(
   "RDB ASIS(^ begin dbms_stats.set_global_prefs(pname=>'NO_INVALIDATE', pvalue=>'FALSE'); end; ^) GO"
   )
  IF (dm2_push_cmd(dcoo_cmd,1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SUBROUTINE dcoo_get_object_status(dgos_type,dgos_name,dgos_exists,dgos_status)
   SET dgos_exists = 0
   SET dgos_status = ""
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcoo_drop_object(ddo_type,ddo_name)
   DECLARE ddo_exists = i2 WITH protect, noconstant(0)
   DECLARE ddo_status = vc WITH protect, noconstant("")
   SET dm_err->eproc = concat("Dropping the ",ddo_name," ",ddo_type)
   CALL disp_msg("",dm_err->logfile,0)
   IF (dcoo_get_object_status(ddo_type,ddo_name,ddo_exists,ddo_status)=0)
    RETURN(0)
   ENDIF
   IF (ddo_exists=1)
    IF (dm2_push_cmd(concat("rdb drop ",ddo_type," ",ddo_name," go"),1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "Ending DM_CREATE_OPTIMIZER_OBJECTS."
 CALL final_disp_msg("dm_create_opt_obj")
END GO
