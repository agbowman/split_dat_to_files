CREATE PROGRAM bbt_rpt_reagent_cells
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->reason_meanings = uar_i18ngetmessage(i18nhandle,"reason_meanings",
  "REASONS WITH MEANINGS")
 SET captions->reagent_cells = uar_i18ngetmessage(i18nhandle,"reagent_cells","REAGENT CELLS")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_reagent_cell", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  cv.display, cv.description, cv.cdf_meaning,
  cv.active_ind, cd.cdf_meaning, cd.display
  FROM code_value cv,
   common_data_foundation cd,
   (dummyt d_cd  WITH seq = 1)
  PLAN (cv
   WHERE cv.code_set=1603)
   JOIN (d_cd
   WHERE d_cd.seq=1)
   JOIN (cd
   WHERE cd.code_set=1603
  ...
  HEAD PAGE
   ...
   col 49, captions->reason_meanings, row + 1,
   col 53, captions->reagent_cells, row + 2,
  ...
  WITH nullreport, nocounter, compress,
   nolandscape, dontcare = cd, outerjoin = d_cd
 ;end select
