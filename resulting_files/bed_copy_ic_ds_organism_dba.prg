CREATE PROGRAM bed_copy_ic_ds_organism:dba
 ...
 SET org_size = size(request->copy_orgs,5)
 IF (org_size=0)
  GO TO exit_script
 ENDIF
 SET updt_tot_cnt = 0
 ...
 SELECT INTO "NL:"
  ...
  PLAN (d)
   ...
   WHERE (dsc.facility_cd=request->facility_cd)
    AND (dsc.organism_cd=request->copy_orgs[d.seq].organism_cd))
  DETAIL
 ...
 SELECT INTO "NL:"
  ...
  PLAN (dsc
   WHERE (dsc.organism_cd=request->source_organism_cd)
  ...
  DETAIL
   FOR (x = 1 TO org_size)
     ...
     temp_req->req_items[new_item_tot_cnt].antibiotic_cd = dsc.antibiotic_cd, temp_req->req_items[
     new_item_tot_cnt].organism_cd = request->copy_orgs[x].organism_cd,
