CREATE PROGRAM dm2_tablespace_maint:dba
 ...
 SUBROUTINE dcprom_install_type(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dm2_option_selected = i2 WITH protect, noconstant(0)
 DECLARE dm2_environment_name = vc WITH protect, noconstant("")
 DECLARE dm2_dbase_name = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_tablespace_maint",".log","dm2_tablespace_maint")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting environment name and database name."
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 SELECT INTO "nl:"
  ...
  WHERE i.info_domain="DATA MANAGEMENT"
   AND i.info_name="DM_ENV_ID"
   AND i.info_number=d.environment_id
  DETAIL
   dm2_environment_name = cnvtupper(d.environment_name), dm2_dbase_name = cnvtupper(d.database_name)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  GO TO exit_program
 ...
 IF ((dm_err->debug_flag > 5))
  CALL echorecord(dm2menureq)
 ENDIF
 IF ((dm2menureq->prompt_mode="PROMPT"))
  SET dm_err->eproc = "Prompting for Options"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  CALL dm2t_options_prompt(null)
 ELSE
  SET dm_err->eproc = "Install Type set"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  SET rtspace->install_type = dm2menureq->install_type
 ENDIF
 SET dm_err->eproc = "Executing DM2_TSPACE_MENU"
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 EXECUTE dm2_tspace_menu
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SUBROUTINE dm2t_options_prompt(null)
   CALL clear(1,1)
   CALL text(2,2,"DM2_TABLESPACE_MAINT")
   CALL box(1,1,21,132)
   CALL text(4,3,"  *****   TABLESPACE MAINTENANCE  ***** ")
   CALL text(5,3,concat("  Environment: ",trim(dm2_environment_name),"     Database:",trim(
      dm2_dbase_name)))
   CALL line(6,1,132,xhor)
   CALL text(8,2,"1.  Add / Extend Tablespace(s) needed for a Package Install.")
   CALL text(9,2,"2.  Add / Extend Tablespace(s) needed for a Schema Date Install.")
   CALL text(10,2,"3.  Add / Extend Tablespace(s) needed for a Batch Install.")
   CALL text(11,2,"4.  Add / Extend Tablespace(s) (User Defined List).")
   CALL text(13,10,"Your Selection (0 to Exit)?")
   CALL accept(13,39,"N;",1
   ...
   CASE (curaccept)
    ...
    OF 1:
     SET dm2comprom->install_mode = "PACKAGE"
     SET dm2menureq->menu_option = 1
     SET dm2menureq->install_type = "PACKAGE"
     SET dm_err->eproc = "Prompting for Install Type Value for Package"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
     IF (dcprom_tspacesize_install_type(null)=0)
      GO TO exit_program
     ENDIF
     SET rtspace->install_type = "PACKAGE"
     SET rtspace->install_type_value = dm2comprom->package_number_vc
    OF 2:
     SET dm2menureq->menu_option = 2
     SET dm2comprom->install_mode = "SCHEMA_DATE"
     SET dm2menureq->install_type = "SCHEMA_DATE"
     SET dm_err->eproc = "Prompting for Install Type Value for Schema Date"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
     IF (dcprom_tspacesize_install_type(null)=0)
      GO TO exit_program
     ENDIF
     SET rtspace->install_type = "SCHEMA_DATE"
     SET rtspace->install_type_value = dm2comprom->schema_date
    OF 3:
     SET dm2comprom->install_mode = "BATCH_INSTALL_PLAN"
     SET dm2menureq->menu_option = 1
     SET dm2menureq->install_type = "BATCH_INSTALL_PLAN"
     SET dm_err->eproc = "Prompting for Install Type Value for Batch Install Plan"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
     IF (dcprom_tspacesize_install_type(null)=0)
      GO TO exit_program
     ENDIF
     SET rtspace->install_type = "PACKAGE"
     SET rtspace->install_type_value = cnvtstring(dm2comprom->package_number)
    OF 4:
     SET dm_err->eproc = "Executing in Manual Mode"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
     SET dm2menureq->install_type = "MANUAL"
     SET dm2menureq->menu_option = 3
     SET rtspace->install_type = "MANUAL"
     SET dm2comprom->install_mode = "MANUAL"
   ENDCASE
 END ;Subroutine
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM2_TABLESPACE_MAINT has completed"
 ENDIF
 CALL final_disp_msg("dm2_tablespace_maint")
 IF ((dm_err->debug_flag > 5))
  SET message = nowindow
  CALL echorecord(dm2menureq)
