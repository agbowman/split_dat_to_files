CREATE PROGRAM cern_dcpreqgen03:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   action = uar_get_code_display(oa.action_type_cd), orders->qual[ord_cnt].iv_ind = o.iv_ind
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   HEAD PAGE
    ...
    CALL center(orders->facility,1,220), row + 1, "{cpi/12}{pos/1/45}",
    CALL center("Place client info here",1,220), row + 1, "{cpi/12}{pos/1/57}",
    CALL center("2800 Rockcreek Pkwy",1,220), row + 1, "{cpi/12}{pos/1/70}",
    CALL center("Kansas City, MO 64117",1,220), row + 1, "{cpi/14}{pos/40/110}",
    ...
    orders->admit_diagnosis, "{cpi/14}{pos/40/140}", "Attending MD: ",
    orders->attending, "{cpi/14}{pos/40/150}", "Height as of ",
    CALL print(trim(orders->height_dt_tm)), ": ", orders->height,
    row + 1, "{cpi/14}{pos/380/150}", "Patient Location: ",
    orders->location, row + 1, "{cpi/14}{pos/40/160}",
    "Weight as of ",
    CALL print(trim(orders->weight_dt_tm)), ": ",
    orders->weight, "{cpi/14}{pos/380/160}", "Patient Type: ",
    orders->pat_type, row + 1, "{cpi/2}{pos/35/190}{b}",
    line1, row + 1
   DETAIL
    FOR (vv = 1 TO value(order_cnt))
      IF ((orders->qual[vv].display_ind=1))
       ...
       ENDIF
       "{cpi/12}{pos/40/250}{b}", "Orderable: ", orders->qual[vv].mnemonic,
       xcol = 40, ycol = 270
       FOR (fsub = 1 TO 31)
         FOR (ww = 1 TO orders->qual[vv].d_cnt)
           IF ((((orders->qual[vv].d_qual[ww].group_seq=fsub)) OR (fsub=31
            ...
            IF ((orders->qual[vv].d_qual[ww].value > spaces))
             "{cpi/12}",
             CALL print(calcpos(xcol,ycol)),
             CALL print(orders->qual[vv].d_qual[ww].label_text),
             ":  ",
             CALL print(orders->qual[vv].d_qual[ww].value), row + 1,
             ycol = (ycol+ 20)
            ENDIF
           ENDIF
         ENDFOR
       ...
       IF ((orders->qual[vv].comment_ind=1)
        ...
        FOR (com_cnt = 1 TO ocnt)
          IF (com_cnt=1)
           CALL print(calcpos(xcol,ycol)), "Comments:  ", xcol = 100
          ELSE
           xcol = 100, ycol = (ycol+ 15)
          ENDIF
        ...
        IF ((orders->qual[vv].com_ln_cnt > 7))
         xcol = 100, ycol = (ycol+ 15),
         CALL print(calcpos(xcol,ycol)),
         "{cpi/14}", "**** Please check chart for further comments ****"
        ENDIF
       ENDIF
      ENDIF
    ENDFOR
   FOOT PAGE
    ...
    "{cpi/13}{pos/45/700}", "PRINT INITIATED: ",
    print_time, row + 1, "{cpi/13}{pos/255/700}",
    "BY: ", orders->qual[1].enter_by, row + 1,
    "{font/8}{cpi/12}{pos/50/720}", "dcpreqgen03"
