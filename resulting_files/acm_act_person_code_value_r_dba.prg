CREATE PROGRAM acm_act_person_code_value_r:dba
 ...
 FOR (index = 1 TO xref->act_cnt)
   SET reply->person_code_value_r_qual[xref->act[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->act_cnt)),
  ...
  SET p.active_ind = xref->act[d.seq].active_ind, p.active_status_cd =
   ...
   p.active_status_dt_tm = cnvtdatetime(sysdate), p.updt_cnt = (p.updt_cnt+ 1), p.updt_dt_tm =
   cnvtdatetime(sysdate),
   p.updt_id = reqinfo->updt_id, p.updt_applctx = reqinfo->updt_applctx, p.updt_task = reqinfo->
   updt_task
  PLAN (d)
   JOIN (p
   WHERE (p.person_code_value_r_id=acm_request->person_code_value_r_qual[xref->act[d.seq].idx].
   person_code_value_r_id))
  WITH nocounter, status(reply->person_code_value_r_qual[xref->act[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->act_cnt)
   IF ((reply->person_code_value_r_qual[xref->act[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->person_code_value_r_qual[xref->act[index].idx].status=0))
