CREATE PROGRAM bhs_pa_sched2_orders:dba
 PROMPT
 ...
 ENDIF
 IF (datetimediff(cnvtdatetime(cnvtdate( $EDATE),2359),cnvtdatetime(cnvtdate( $BDATE),0000),1) > 365)
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
  ...
  ;end select
  GO TO exit_prg
 ELSEIF ((datetimediff(cnvtdatetime(cnvtdate( $EDATE),2359),cnvtdatetime(cnvtdate( $BDATE),0000),1)
  < - (1)))
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
 ...
 SELECT INTO "NL:"
  ...
  HEAD ord.to_prsnl_id
   ...
   temp->qual[qualcnt].catalogcd = o.catalog_cd, temp->qual[qualcnt].orddate = format(o
    .orig_order_dt_tm,"MM-DD-YYYY HH:MM"), temp->qual[qualcnt].orderstatus = uar_get_code_display(o
    .order_status_cd),
   temp->qual[qualcnt].patacctid = cnvtreal(ea.alias), temp->qual[qualcnt].patname = p
   .name_full_formatted
  HEAD ord.order_notification_id
   IF (ord.notification_status_flag=4)
    temp->qual[qualcnt].ordforwarded = 1, temp->qual[qualcnt].fordnotid = ord.order_notification_id,
    temp->qual[qualcnt].ftoattd = 1,
    temp->qual[qualcnt].forwarddt = format(ord.notification_dt_tm,"MM-DD-YYYY HH:MM")
   ENDIF
 ...
 SELECT INTO "nl:"
  ...
  HEAD ord.parent_order_notification_id
   ...
   ENDIF
   temp->qual[d1.seq].ftoname = pr.name_full_formatted
 ...
 IF (qualcnt > 0)
  IF (email_ind=0)
   SELECT INTO  $OUTDEV
    apname = temp->qual[d2.seq].apname, order_id = temp->qual[d2.seq].oid, forwarded_to_attending =
    IF ((temp->qual[d2.seq].ftoattd=1)) "yes"
    ELSE "no"
    ENDIF
    ,
    ord_forwarded =
    IF (temp->qual[d2.seq].ordforwarded) "yes"
    ELSE "no"
    ENDIF
    , forwarded_to = temp->qual[d2.seq].ftoname, received_date = temp->qual[d2.seq].receiveddt,
    ...
    finalordnotifstatus = temp->qual[qualcnt].finalordnotifstatus, patient_acct_num = temp->qual[d2
    .seq].patacctid, patient_name = temp->qual[d2.seq].patname,
    drug_mnemonic = temp->qual[d2.seq].drugmnemonic, encntr_id = temp->qual[d2.seq].eid, catalog_cd
     = temp->qual[d2.seq].catalogcd,
  ...
  ELSE
   SELECT INTO value(var_output)
    ...
    ENDIF
    , forward_date = temp->qual[d2.seq].forwarddt, order_status = temp->qual[d2.seq].orderstatus,
    orignal_orde_date = temp->qual[d2.seq].orddate, finalordnotifstatus = temp->qual[qualcnt].
    finalordnotifstatus, patient_acct_num = temp->qual[d2.seq].patacctid,
    patient_name = temp->qual[d2.seq].patname, drug_mnemonic = temp->qual[d2.seq].drugmnemonic,
    encntr_id = temp->qual[d2.seq].eid,
    catalog_cd = temp->qual[d2.seq].catalogcd, order_notification_id = temp->qual[d2.seq].fordnotid
    FROM (dummyt d2  WITH seq = value(qualcnt))
    PLAN (d2)
    ORDER BY apname
