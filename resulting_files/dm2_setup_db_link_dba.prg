CREATE PROGRAM dm2_setup_db_link:dba
 ...
 SUBROUTINE eos_req_add_object(oname,otype,onr,rlen,radd)
   SET eos_request->obj_cnt += 1
   SET eos_ao_stat = alterlist(eos_request->obj,eos_request->obj_cnt)
   SET eos_request->obj[eos_request->obj_cnt].obj_name = oname
   SET eos_request->obj[eos_request->obj_cnt].obj_type = otype
   SET eos_request->obj[eos_request->obj_cnt].owner = onr
   SET eos_request->obj[eos_request->obj_cnt].row_length = rlen
   SET eos_request->obj[eos_request->obj_cnt].rows_to_add = radd
 END ;Subroutine
 IF (validate(i18nuar_def,999)=999)
  ...
  persist
 ENDIF
 DECLARE cur_env_id = i4 WITH protect, noconstant(0)
 DECLARE cur_env_name = vc WITH protect
 DECLARE dsrdm_db_link = vc WITH protect
 DECLARE dsrdm_tnsname = vc WITH protect
 DECLARE dsrdm_username = vc WITH protect
 DECLARE dsrdm_pwd = vc WITH protect
 DECLARE src_env_id = i4 WITH protect, noconstant(0)
 DECLARE src_env_name = vc WITH protect
 DECLARE dsrdm_con = i2 WITH protect, noconstant(0)
 DECLARE dsrdm_size = i4 WITH protect, noconstant(0)
 DECLARE dsrdm_poslink = i4 WITH protect, noconstant(0)
 DECLARE dsrdm_posuser = i4 WITH protect, noconstant(0)
 DECLARE dsrdm_holder = vc WITH protect
 DECLARE dsrdm_confirm = vc WITH protect
 DECLARE dsrdm_link_con = i2 WITH protect
 DECLARE dmt_total_row = i4 WITH protect, noconstant(0)
 DECLARE der_post_link = vc WITH protect
 DECLARE link_fail_ind = i2 WITH protect, noconstant(1)
 DECLARE dsrdm_dbase = vc WITH protect
 DECLARE dsrdm_node = vc WITH protect
 DECLARE dblink_i18n_handle = i4
 SET dblink_i18n_handle = 0
 SET stat = uar_i18nlocalizationinit(dblink_i18n_handle,curprog,"",curcclrev)
 CALL check_logfile("dm2_setup_rdt_mover",".log","DM2_SETUP_REF_DATA_MOVER LOGFILE")
 SET dm_err->eproc = "Beginning dm2_setup_ref_data_mover"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info a,
   dm_environment b
  ...
  DETAIL
   cur_env_id = b.environment_id, cur_env_name = b.environment_name
  WITH nocounter
 ;end select
 IF (cur_env_id=0)
 ...
 WHILE (dsrdm_link_con=0)
   ...
   SET dsrdm_username = " "
   SET dsrdm_pwd = " "
   CALL text(3,(66 - (textlen(request->menu_title_text)/ 2)),request->menu_title_text)
   WHILE (dsrdm_con=0)
     CALL text(6,3,uar_i18nbuildmessage(dblink_i18n_handle,"k1","Please input %1 ENVIRONMENT ID:",
       "s",request->environment_type))
     CALL text(23,05,uar_i18ngetmessage(dblink_i18n_handle,"k2",
       "HELP: Press <SHIFT><F5>   0 to exit "))
     IF ((request->environment_type="ARCHIVE"))
      SET help =
      SELECT INTO "nl:"
       ...
       (SELECT
        ...
        WHERE der.parent_env_id=cur_env_id
         AND (der.relationship_type=request->der_relationship_type))))
        AND  NOT (de.environment_id IN (
       (SELECT
        dae.env_id
        FROM dm_arc_env dae)))
     ...
     ELSE
      ...
      SELECT INTO "nl:"
       ...
       WHERE de.environment_id=der.parent_env_id
        AND der.child_env_id=cur_env_id
        AND (der.relationship_type=request->der_relationship_type)
       ORDER BY de.environment_name
       WITH nocounter
      ;end select
     ENDIF
     CALL accept(6,40,"P(15);CU","0")
     IF (curaccept="0")
      SET help = off
      SET validate = off
      GO TO exit_program
     ELSEIF (cnvtreal(curaccept)=0)
      CALL text(20,3,uar_i18ngetmessage(dblink_i18n_handle,"k3",
        "Please input a number for this field!"))
      CALL pause(3)
     ELSE
      IF ((request->environment_type="ARCHIVE"))
       SET dsrdm_con = 1
      ELSE
       SELECT INTO "nl:"
        der.parent_env_id
        FROM dm_env_reltn der
        WHERE der.child_env_id=cur_env_id
       ...
       ELSE
        CALL clear(20,1)
        CALL text(20,3,uar_i18ngetmessage(dblink_i18n_handle,"k4","Invalid environment ID"))
        CALL pause(3)
       ENDIF
      ENDIF
     ENDIF
   ...
   SET validate = off
   CALL clear(20,1)
   SET src_env_id = cnvtint(curaccept)
   SELECT INTO "nl:"
    FROM dm_environment de
    WHERE de.environment_id=src_env_id
    DETAIL
     src_env_name = de.environment_name
    WITH nocounter
   ;end select
   SET dsrdm_db_link = build(request->dsrdm_db_link_prefix,src_env_id,cur_env_id)
   SET dsrdm_confirm = build(request->dsrdm_db_link_prefix,"SETUP",src_env_id,cur_env_id)
   SELECT INTO "nl:"
    FROM dm_environment d
    WHERE d.environment_id=src_env_id
    DETAIL
     dsrdm_holder = d.v500_connect_string
    WITH nocounter
   ;end select
   SET dsrdm_size = size(dsrdm_holder,1)
   SET dsrdm_poslink = findstring("@",dsrdm_holder)
   SET dsrdm_posuser = findstring("/",dsrdm_holder)
   IF (dsrdm_size != 0)
    IF (dsrdm_posuser != 0)
     SET dsrdm_username = substring(1,(dsrdm_posuser - 1),dsrdm_holder)
    ENDIF
    IF (dsrdm_poslink != 0)
     SET dsrdm_tnsname = substring((dsrdm_poslink+ 1),(dsrdm_size - dsrdm_poslink),dsrdm_holder)
     SET dsrdm_pwd = substring((dsrdm_posuser+ 1),((dsrdm_poslink - dsrdm_posuser) - 1),dsrdm_holder)
    ELSE
     SET dsrdm_pwd = substring((dsrdm_posuser+ 1),(dsrdm_size - dsrdm_posuser),dsrdm_holder)
    ENDIF
   ENDIF
   CALL text(7,3,uar_i18ngetmessage(dblink_i18n_handle,"k5","Input user name: "))
   CALL accept(7,40,"P(20);CU",dsrdm_username)
   SET dsrdm_username = curaccept
   CALL text(8,3,uar_i18ngetmessage(dblink_i18n_handle,"k6","Input password: "))
   CALL accept(8,40,"P(20);CU",dsrdm_pwd)
   SET dsrdm_pwd = curaccept
   IF (currdb="ORACLE")
    CALL text(9,3,uar_i18ngetmessage(dblink_i18n_handle,"k7","Input tnsname for this environment: "))
    CALL accept(9,40,"P(20);CU",dsrdm_tnsname)
    SET dsrdm_tnsname = curaccept
    CALL text(12,3,uar_i18ngetmessage(dblink_i18n_handle,"k8","Creating database link... "))
    EXECUTE dm2_make_merge_dblink dsrdm_db_link, dsrdm_tnsname, dsrdm_username,
    dsrdm_pwd, " "
   ELSEIF (currdb="DB2UDB")
    CALL text(9,3,uar_i18ngetmessage(dblink_i18n_handle,"k9","Input remote database name: "))
    CALL accept(9,40,"P(20);CU")
    SET dsrdm_dbase = curaccept
    CALL text(10,3,uar_i18ngetmessage(dblink_i18n_handle,"k10","Input remote node name: "))
    CALL accept(10,40,"P(20);CU")
    SET dsrdm_node = curaccept
    CALL text(12,3,uar_i18ngetmessage(dblink_i18n_handle,"k11","Creating server... "))
    EXECUTE dm2_make_merge_dblink dsrdm_db_link, dsrdm_dbase, dsrdm_username,
    dsrdm_pwd, dsrdm_node
   ENDIF
   IF (check_error(concat("ERROR: Create database link")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    CALL clear(11,1)
    CALL text(12,3,uar_i18ngetmessage(dblink_i18n_handle,"k12",
      "Could not create database link/server for the reason"))
    CALL text(13,3,dm_err->emsg)
    CALL text(15,3,uar_i18ngetmessage(dblink_i18n_handle,"k13","Retry or Quit (R/Q)"))
    CALL accept(15,50,"P;CU","Q"
     WHERE curaccept IN ("Q", "R"))
    IF (curaccept="Q")
     SET dsrdm_link_con = 1
    ENDIF
    SET dm_err->err_ind = 0
   ELSE
    IF ((request->environment_type="ARCHIVE"))
     IF (currdb="ORACLE")
      INSERT  FROM dm_arc_env der
       SET der.env_id = src_env_id, der.post_link_name = concat("@",dsrdm_db_link), der.pre_link_name
         = " ",
        der.updt_id = reqinfo->updt_id, der.updt_task = reqinfo->updt_task, der.updt_applctx =
        reqinfo->updt_applctx,
        der.updt_dt_tm = cnvtdatetime(sysdate), der.updt_cnt = 0
      ;end insert
     ELSE
      INSERT  FROM dm_arc_env der
       SET der.env_id = src_env_id, der.pre_link_name = concat(dsrdm_db_link,"."), der.post_link_name
         = " ",
        der.updt_id = reqinfo->updt_id, der.updt_task = reqinfo->updt_task, der.updt_applctx =
        reqinfo->updt_applctx,
        der.updt_dt_tm = cnvtdatetime(sysdate), der.updt_cnt = 0
    ...
    ELSE
     IF (currdb="ORACLE")
      SET der_post_link = build("@",dsrdm_db_link)
      UPDATE  FROM dm_env_reltn
       SET post_link_name = der_post_link, pre_link_name = null
       WHERE parent_env_id=src_env_id
        AND child_env_id=cur_env_id
        AND (relationship_type=request->der_relationship_type)
       WITH nocounter
      ;end update
      COMMIT
     ELSE
      SET der_pre_link = build(dsrdm_db_link,".")
      UPDATE  FROM dm_env_reltn
       SET pre_link_name = der_pre_link, post_link_name = null
       WHERE parent_env_id=src_env_id
        AND child_env_id=cur_env_id
        AND (relationship_type=request->der_relationship_type)
       WITH nocounter
      ;end update
      COMMIT
      CALL text(13,3,uar_i18nbuildmessage(dblink_i18n_handle,"k14","Server %1 was created","s",
        dsrdm_db_link))
     ENDIF
    ENDIF
    SET dsrdm_link_con = 1
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "...Ending dm2_setup_db_link.prg"
 CALL final_disp_msg("dm2_setup_db_link")
END GO
