CREATE PROGRAM dm2_aload_afd_index_columns
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_indcol",".log","dm2_aload_afd_index_columns")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_afd_index_columns"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daaic_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daaic_file = vc WITH protect, noconstant( $2)
 DECLARE daaic_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daaic_batch = i4 WITH protect, noconstant(0)
 DECLARE daaic_rows_returned = i4 WITH protect, noconstant(1)
 DECLARE daaic_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daaic_pkg,daaic_file_loc_ret,daaic_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daaic_batch=0)
  SET daaic_batch = 5000
 ENDIF
 IF (daaic_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daaic_file_loc_ret,daaic_file), "dm2_aload_sch_indcol", daaic_batch
  CALL echo(concat(daaic_file_loc_ret,daaic_file))
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Remove extraneous DM_AFD_INDEX_COLUMNS rows"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dm_afd_index_columns a
   WHERE a.alpha_feature_nbr=daaic_pkg
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_afd_index_columns completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_afd_index_columns")
END GO
