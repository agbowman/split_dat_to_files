CREATE PROGRAM dm2_aload_adoc_dtd
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_adoc_dtd",".log","dm2_aload_adoc_dtd")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_adoc_dtd"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE datd_cnt = i4 WITH protect, noconstant(0)
 DECLARE datd_str = vc WITH protect, noconstant("")
 DECLARE datd_req_size = i4 WITH protect, noconstant(0)
 DECLARE datd_ndx = i4 WITH protect, noconstant(0)
 FREE RECORD datd_maint
 RECORD datd_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET datd_maint->num_of_inserts = 0
 SET datd_maint->num_of_updates = 0
 SET datd_maint->num_of_none = 0
 SET datd_req_size = size(requestin->list_0,5)
 SET stat = alterlist(datd_maint->qual,datd_req_size)
 IF (dac_pop_coldic_rec("DM_TABLES_DOC")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  SET dm_err->eproc = "Determining maintenance type for DM_TABLES_DOC."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(datd_req_size))
  DETAIL
   datd_maint->qual[d.seq].maint_type = "I", datd_maint->num_of_inserts = (datd_maint->num_of_inserts
   + 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 SELECT INTO "nl:"
  FROM dm_tables_doc dat
  WHERE dat.owner=currdbuser
   AND (dat.table_name >= requestin->list_0[1].table_name)
   AND (dat.table_name <= requestin->list_0[datd_req_size].table_name)
  ORDER BY dat.table_name
  DETAIL
   datd_ndx = locateval(datd_ndx,1,datd_req_size,dat.table_name,requestin->list_0[datd_ndx].
    table_name)
   IF (datd_ndx > 0)
    IF (cnvtint(requestin->list_0[datd_ndx].updt_cnt) >= dat.updt_cnt)
     datd_maint->qual[datd_ndx].maint_type = "U", datd_maint->num_of_updates = (datd_maint->
     num_of_updates+ 1), datd_maint->num_of_inserts = (datd_maint->num_of_inserts - 1)
    ELSE
     datd_maint->qual[datd_ndx].maint_type = "N", datd_maint->num_of_none = (datd_maint->num_of_none
     + 1), datd_maint->num_of_inserts = (datd_maint->num_of_inserts - 1)
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(datd_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((datd_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(datd_req_size) )"),0)
  ...
  CALL dm2_push_cmd("plan d where datd_maint->qual[d.seq].maint_type = 'I' ",0)
  IF (dm2_push_cmd(" join dcv  with nocounter go",1)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  SET dadtd_curqual = (dadtd_curqual+ curqual)
  IF ((curqual != datd_maint->num_of_inserts))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Inserted:",curqual,". Does not match expected rows inserted:",
    datd_maint->num_of_inserts)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
  COMMIT
 ENDIF
 IF ((datd_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where datd_maint->qual[d.seq].maint_type = 'U' ",0)
  CALL dm2_push_cmd("join dcv where requestin->list_0[d.seq].table_name = dcv.table_name ",0)
  CALL dm2_push_cmd("           and dcv.owner = currdbuser ",0)
  IF (dm2_push_cmd("with nocounter go",1)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  SET dadtd_curqual = (dadtd_curqual+ curqual)
  IF ((curqual != datd_maint->num_of_updates))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Updated:",curqual,". Does not match expected rows updated:",
    datd_maint->num_of_updates)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_adoc_dtd completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_adoc_dtd")
END GO
