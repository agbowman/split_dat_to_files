CREATE PROGRAM dm_rx_rx_claim_downtime_rows:dba
 ...
 SET reply->table_name = "RX_CLAIM_DOWNTIME"
 ...
 DECLARE v_rec_count = i2 WITH noconstant(0)
 DECLARE v_days_to_keep = i4 WITH protect, noconstant(- (1))
 FOR (i = 1 TO size(request->tokens,5))
   IF ((request->tokens[i].token_str="DOWNDAYS"))
    SET v_days_to_keep = ceil(cnvtreal(request->tokens[i].value))
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 30)
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"DOWNDAYS",
   "You must keep at least 30 days' worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
 ELSE
  SELECT INTO "nl:"
   cd.rowid
   FROM rx_claim_downtime cd
   WHERE parser(sbr_getrowidnotexists(
     "cd.updt_dt_tm < cnvtdatetime(curdate - v_days_to_keep, curtime3)","cd"))
    AND cd.active_ind=0
    AND ((cd.rx_claim_downtime_id+ 0) > 0.0)
   DETAIL
    v_rec_count = (v_rec_count+ 1)
    IF (mod(v_rec_count,20)=1)
     stat = alterlist(reply->rows,(v_rec_count+ 19))
    ENDIF
    reply->rows[v_rec_count].row_id = cd.rowid
   FOOT REPORT
    stat = alterlist(reply->rows,v_rec_count)
   WITH nocounter, maxqual(cd,value(request->max_rows))
  ;end select
