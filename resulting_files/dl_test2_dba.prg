CREATE PROGRAM dl_test2:dba
 ...
 SELECT INTO  $1
  ...
  ORDER BY pl.name_full_formatted, orv.order_id
  HEAD REPORT
   c = 0
  HEAD pl.name_full_formatted
   ...
   " ************", row + 1, col 1,
   starline, row + 1, m = (c+ 1)
  DETAIL
   c = (c+ 1)
   IF (mod(c,10)=1)
    stat = alterlist(dlrec->seq,(c+ 9))
   ENDIF
   dlrec->seq[c].encntr_id = o.encntr_id, dlrec->seq[c].order_person = order_person, dlrec->seq[c].
   pt_name = pt_name,
   dlrec->seq[c].order_id = o.order_id, dlrec->seq[c].order_mnemonic = substring(1,80,o
    .order_mnemonic), dlrec->seq[c].order_comment_ind = comment_flag,
   dlrec->seq[c].clinical_display_line = o.clinical_display_line, col 5, pt_name,
  ...
  FOOT  pl.name_full_formatted
   dlrec->seq[m].sign_doctor = pl.name_full_formatted
  FOOT REPORT
   dlrec->ord_total = c, stat = alterlist(dlrec->seq,c)
  WITH maxcol = 999, maxqual(orv,40)
 ;end select
 SELECT INTO "nl:"
 ...
 SELECT INTO  $1
  HEAD REPORT
   ...
   ENDMACRO
  HEAD PAGE
   CALL print(calcpos(xcol5,yrow1)), "{b}", dlrec->seq[1].org_name,
   "{endb}",
  ...
  DETAIL
   FOR (c = 1 TO dlrec->ord_total)
     ...
     CALL print(calcpos(xcol12,yrow1)),
     dlrec->seq[c].encntr_type, rowplusone,
     ...
     wrapcol = 110, tempstring = dlrec->seq[c].clinical_display_line, line_wrap,
     CALL print(calcpos(xcol2,yrow1)), "Order Comment: ", xcolvar = (xcol3+ 10),
     wrapcol = 100, tempstring = dlrec->seq[c].order_comment, line_wrap
  ...
  FOOT PAGE
   ...
   IF ((c < dlrec->ord_total))
    CALL print(calcpos(290,540)), "Continued", row + 1
   ENDIF
   CALL print(calcpos(xcol1,560)), "Page ", curpage"###",
   row + 1, ycol1 = 5
  FOOT REPORT
   row + 1,
   CALL print(calcpos(260,(yrow1+ 20))), "**** End of Report ****",
   row + 1, ycol1 = 5
