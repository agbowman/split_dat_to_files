CREATE PROGRAM dm2_shc_mill_prg_rtns:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsm_err_status = c1 WITH protect, noconstant(" ")
 DECLARE dsm_act = i4 WITH protect, noconstant(0)
 DECLARE dsm_inact = i4 WITH protect, noconstant(0)
 FREE RECORD dsm_purge_routines
 RECORD dsm_purge_routines(
   1 cnt = i4
   1 active_cnt = i4
   1 inactive_cnt = i4
   1 active_pr[*]
 ...
 IF (check_logfile("DM2_SHC_MILLPRGRTNS",".log","DM2_SHC_MILL_PRG_RTNS LOG FILE")=0)
  SET dsm_err_status = "F"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "System Health Check working on Millennium Purge Routines"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 ENDIF
 SELECT DISTINCT INTO "nl:"
  j.active_flag, t.program_str
  FROM dm_purge_job j,
   dm_purge_template t
  WHERE j.template_nbr=t.template_nbr
  ORDER BY j.active_flag
  DETAIL
   dsm_purge_routines->cnt += 1
   IF (j.active_flag)
    dsm_purge_routines->active_cnt += 1, stat = alterlist(dsm_purge_routines->active_pr,
     dsm_purge_routines->active_cnt), dsm_purge_routines->active_pr[dsm_purge_routines->active_cnt].
    routine_name = t.program_str
   ELSE
    dsm_purge_routines->inactive_cnt += 1, stat = alterlist(dsm_purge_routines->inactive_pr,
     dsm_purge_routines->inactive_cnt), dsm_purge_routines->inactive_pr[dsm_purge_routines->
    inactive_cnt].routine_name = t.program_str
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error("Determine what purge routines are active.") > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 IF ((dm_err->err_ind=1))
  SET dsm_err_status = "E"
 ELSE
  SET dsm_err_status = "R"
 ENDIF
 IF (dshc_add_check_to_rs("MILLENNIUM PURGE ROUTINES","Millennium Purge Routines",0,dsm_err_status,
 ...
 IF ((dsm_purge_routines->cnt=0))
  ...
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Millennium Purge Routine"
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "NOT_SET"
  SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text =
  "No Active or Inactive Purge Routines Found"
 ELSE
  IF ((dsm_purge_routines->active_cnt=0))
   SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
   SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
   SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
   SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Active Purge Routine"
   SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "NOT_SET"
   SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "No Active Purge Routines Found"
  ELSE
   FOR (dsm_act = 1 TO dsm_purge_routines->active_cnt)
     SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
     SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
     SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
     SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Active Purge Routine"
     ...
     IF (dsm_act=1)
      SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text =
      "The following purge routines are active:"
     ELSE
      SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
     ENDIF
     SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2
     SET dshc->check[dshc->chk_cnt].desc_detail[cnt].str_value = dsm_purge_routines->active_pr[
     dsm_act].routine_name
   ENDFOR
  ENDIF
  IF ((dsm_purge_routines->inactive_cnt=0))
   SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
   SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
   SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
   SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Inactive Purge Routine"
   SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "NOT_SET"
   SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text =
   "<SPACE>No Inactive Purge Routines Found"
  ELSE
   FOR (dsm_inact = 1 TO dsm_purge_routines->inactive_cnt)
     SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
     SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
     SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
     SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Inactive Purge Routine"
     ...
     IF (dsm_inact=1)
      SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text =
      "The following purge routines are inactive:"
     ELSE
      SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
     ENDIF
     SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2
     SET dshc->check[dshc->chk_cnt].desc_detail[cnt].str_value = dsm_purge_routines->inactive_pr[
     dsm_inact].routine_name
   ENDFOR
  ENDIF
 ENDIF
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "Based on the installed Cerner Solutions, please ensure that the appropriate purge ",
  "routines are running correctly.  These purge routines are normally configured in ",
  "DM Purge Job Manager.")
#exit_script
 ...
 IF (dsm_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_MILL_PRG_RTNS WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_MILL_PRG_RTNS WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_MILL_PRG_RTNS COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_MILLPRGRTNS")
 FREE RECORD dsm_purge_routines
END GO
