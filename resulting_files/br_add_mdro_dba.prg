CREATE PROGRAM br_add_mdro:dba
 ...
 SET readme_data->message = "Readme Failed: Starting <br_add_mdro.prg> script"
 ...
 SELECT INTO "nl:"
  FROM br_step bs
  PLAN (bs
   WHERE bs.step_mean="MDROWIZ")
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed checking existence of MDRO row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_step bs
   SET bs.step_mean = "MDROWIZ", bs.step_disp = "Multiple Drug Resistant Organisms", bs.step_type =
    "IMPMAINT",
 ...
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to check for existence of SOLUTION/COREMDRO row: ",errmsg
   )
  GO TO exit_script
 ELSEIF (curqual=0)
 ...
 ELSEIF (curqual=0)
  INSERT  FROM br_client_item_reltn bcir
   SET bcir.br_client_item_reltn_id = seq(bedrock_seq,nextval), bcir.br_client_id = 1, bcir.item_type
     = "STEP",
    bcir.item_mean = "MDROWIZ", bcir.item_display = " Multiple Drug Resistant Organisms", bcir
    .step_cat_mean = "CORE",
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcss.br_client_id=1
   ...
   AND bcss.step_mean="MDROWIZ"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to check for existence of COREIC/MDROWIZ row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  SELECT INTO "nl:"
   ...
   FROM br_client_sol_step bcss
   PLAN (bcss
    WHERE bcss.solution_mean="MDROWIZ")
   DETAIL
  ...
  INSERT  FROM br_client_sol_step bcss
   SET bcss.br_client_id = 1, bcss.solution_mean = "COREIC", bcss.step_mean = "MDROWIZ",
 ...
 SET readme_data->message = "Readme Succeeded: <br_add_mdro.prg> script"
#exit_script
