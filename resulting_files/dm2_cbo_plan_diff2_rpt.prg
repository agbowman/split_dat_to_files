CREATE PROGRAM dm2_cbo_plan_diff2_rpt
 ...
 SUBROUTINE dcr_get_instance_info(dgii_instance_name,dgii_instance_nbr)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_plndif2rpt",".log","dm2_cbo_plan_diff_rpt")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating input parameters."
 DECLARE dcpdr_envid = f8 WITH protect, noconstant(0)
 DECLARE dcpdr_search_string = vc WITH protect, noconstant( $1)
 DECLARE dcpdr_env_name = vc WITH protect, noconstant("")
 DECLARE dcpdr_env_dbname = vc WITH protect, noconstant("")
 DECLARE dcpdr_exclude_dt_str = vc WITH protect, noconstant( $2)
 DECLARE dcpdr_exclude_dt = f8 WITH protect, noconstant(cnvtdatetime(dcpdr_exclude_dt_str))
 DECLARE dcpdr_fname = vc WITH protect, noconstant( $3)
 DECLARE dcpdr_piece_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_sql_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_sql_cnt2 = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_sqltxt_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_plantxt_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_plan_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_tmp_sqlid = vc WITH protect, noconstant("")
 DECLARE dcpdr_tmp_sqlid_prev = vc WITH protect, noconstant("")
 DECLARE dcpdr_cnt2 = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_max_plan_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_cur_plan_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_env_ndx = i4 WITH protect, noconstant(0)
 DECLARE dcpdr_curplan_hash = f8 WITH protect, noconstant(0.0)
 DECLARE dcpdr_header_display = i2 WITH protect, noconstant(0)
 DECLARE dcpdr_ndx = i4 WITH protect, noconstant(0)
 FREE RECORD dcpdr_plans
 RECORD dcpdr_plans(
   1 cnt = i4
   1 qual[*]
     2 phash = f8
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting Dm2_cbo_plan_diff2_rpt"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF ((dm2_rdbms_version->level1 < 11))
  SET dm_err->eproc = "Dm2_cbo_plan_diff2_rpt used only for Oracle 11 and above. Exiting."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (dcpdr_fname="MINE")
  ...
  CALL text(4,2,"Please provide a date from which plan data should be displayed:")
  CALL accept(4,69,"99DAAAD9999D99D99D99;CU",format(cnvtdatetime((curdate - 1),curtime3),
    "DD-MMM-YYYY:HH:MM:SS;;Q")
   WHERE curaccept=format(cnvtdatetime(curaccept),"DD-MMM-YYYY:HH:MM:SS;;Q"))
  SET dcpdr_exclude_dt_str = curaccept
  SET dcpdr_exclude_dt = cnvtdatetime(dcpdr_exclude_dt_str)
  CALL text(5,2,"Filter String:")
  CALL accept(6,2,"P(120);CU","*"
   WHERE curaccept > " ")
  SET dcpdr_search_string = curaccept
  IF (dcpdr_search_string != char(42))
   SET dcpdr_search_string = concat(char(42),dcpdr_search_string,char(42))
  ENDIF
  SET message = nowindow
  CALL clear(1,1)
 ENDIF
 CALL dcr_add_sql_id("",0.0,1)
 CALL dcr_add_break_str("",1)
 CALL dcr_add_break_str(notrim(" AND "),0)
 CALL dcr_add_break_str(notrim(" WHERE "),0)
 CALL dcr_add_break_str(notrim(" FROM "),0)
 CALL dcr_add_break_str(notrim(" SELECT "),0)
 CALL dcr_add_break_str(notrim(","),0)
 CALL dcr_add_break_str(notrim(" "),0)
 SET dm_err->eproc = concat("Obtain SQL ids with SQL text matching ",dcpdr_search_string,
  " from SQL cache.")
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT
  IF (dcpdr_search_string=char(42))
   FROM (
    (
    (SELECT
     v.sql_fulltext, v.sql_id, v.plan_hash_value,
     v.child_number, first_time = sqlpassthru("to_date(v.first_load_time, 'YYYY-MM-DD/HH24:MI:SS')"),
     last_time = sqlpassthru("to_date(v.last_load_time, 'YYYY-MM-DD/HH24:MI:SS')"),
     firstget = ceil((v.buffer_gets/ evaluate(v.executions,0,1,v.executions))), lastget = ceil((v
      .buffer_gets/ evaluate(v.executions,0,1,v.executions)))
     FROM gv$sql v
     WHERE v.parsing_schema_name=trim(cnvtupper(currdbuser))
      AND sqlpassthru("to_date(v.last_load_time, 'YYYY-MM-DD/HH24:MI:SS')") > cnvtdatetime(
      dcpdr_exclude_dt)
      AND  EXISTS (
     (SELECT
      "x"
      FROM dm_sqlplan_hist d
      WHERE d.sql_id=v.sql_id
       AND d.plan_hash_value != v.plan_hash_value
       AND d.last_load_dt_tm > cnvtdatetime(dcpdr_exclude_dt)))
     WITH sqltype("VC32000","VC","F8","F8","DQ8",
       "DQ8","F8","F8")))
    v)
   ORDER BY v.sql_id, v.plan_hash_value, v.child_number DESC
  ELSE
   FROM (
    (
    (SELECT
     ...
     firstget = ceil((v.buffer_gets/ evaluate(v.executions,0,1,v.executions))), lastget = ceil((v
      .buffer_gets/ evaluate(v.executions,0,1,v.executions)))
     FROM gv$sql v
     WHERE v.sql_text=patstring(dcpdr_search_string)
  ...
  HEAD v.sql_id
   row + 0,
   CALL dcr_add_sql_id(v.sql_id,0.0,0),
   CALL dcr_add_plan_data(v.sql_id,0.0,v.plan_hash_value,v.first_time,v.last_time,v.firstget,v
   .lastget,1),
   CALL dcr_add_orig_str("",1),
   CALL dcr_add_orig_str(v.sql_fulltext,0),
   CALL dcr_add_sql_txt("",0.0,"",1),
   CALL dcr_split_text(null)
   FOR (dcpdr_piece_cnt = 1 TO dcrstrrec->orig_str[1].piece_cnt)
     CALL dcr_add_sql_txt(v.sql_id,0.0,dcrstrrec->orig_str[1].piece[dcpdr_piece_cnt].str,0)
   ENDFOR
   dcpdr_cur_plan_cnt = 0
  HEAD v.plan_hash_value
   CALL dcr_add_plan_data(v.sql_id,0.0,v.plan_hash_value,v.first_time,v.last_time,v.firstget,v
   .lastget,0),
   CALL dcr_add_child_number(v.sql_id,v.plan_hash_value,v.child_number,0.0)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dcr_plan_data->cnt > 0))
  SET dm_err->eproc = "Obtain SQL Ids from plan history for reporting."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dm_sqlplan_hist dsh,
    (dummyt d  WITH seq = value(dcr_plan_data->cnt))
   PLAN (d
    WHERE d.seq > 0)
    JOIN (dsh
    WHERE (dsh.sql_id=dcr_plan_data->qual[d.seq].sql_id))
   ORDER BY dsh.environment_id, dsh.sql_id, dsh.plan_hash_value
   HEAD dsh.environment_id
    row + 0
   HEAD dsh.sql_id
    CALL dcr_add_sql_id(dsh.sql_id,dsh.environment_id,0),
    CALL dcr_add_plan_data(dsh.sql_id,dsh.environment_id,dsh.plan_hash_value,dsh.first_load_dt_tm,dsh
    .last_load_dt_tm,dsh.first_gets_exec,dsh.last_gets_exec,1), dcpdr_cur_plan_cnt = 0
   DETAIL
    CALL dcr_add_plan_data(dsh.sql_id,dsh.environment_id,dsh.plan_hash_value,dsh.first_load_dt_tm,dsh
    .last_load_dt_tm,dsh.first_gets_exec,dsh.last_gets_exec,0),
    CALL dcr_add_child_number(dsh.sql_id,dsh.plan_hash_value,- (1.0),dsh.environment_id),
    dcpdr_cur_plan_cnt = (dcpdr_cur_plan_cnt+ 1)
   FOOT  dsh.sql_id
    dcpdr_max_plan_cnt = greatest(dcpdr_cur_plan_cnt,dcpdr_max_plan_cnt)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF (dm2_get_env_data(0,dcpdr_envid)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Obtain Environment names."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (d.environment_id=dcpdr_envid)
    dcpdr_env_name = d.environment_name, dcpdr_env_dbname = d.database_name
   ENDIF
   FOR (dcpdr_env_ndx = 1 TO dcr_plan_data->cnt)
     IF ((dcr_plan_data->qual[dcpdr_env_ndx].env_id=0))
      dcr_plan_data->qual[dcpdr_env_ndx].env_name = "SQL CACHE"
     ELSE
      IF ((d.environment_id=dcr_plan_data->qual[dcpdr_env_ndx].env_id))
       dcr_plan_data->qual[dcpdr_env_ndx].env_name = d.environment_name
      ENDIF
     ENDIF
   ENDFOR
  WITH nocounter
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dcr_plan_data)
 ENDIF
 IF ((dcr_plan_data->cnt > 0))
  SET dm_err->eproc = "Obtain Child Number from SQL cache for SQL ids obtained from plan history."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   envid = dcr_plan_data->qual[d.seq].env_id
   FROM gv$sql v,
    (dummyt d  WITH seq = dcr_plan_data->cnt),
    (dummyt d2  WITH seq = dcpdr_max_plan_cnt)
   PLAN (d
    WHERE d.seq > 0
     AND (dcr_plan_data->qual[d.seq].env_id > 0))
    JOIN (d2
    WHERE (d2.seq <= dcr_plan_data->qual[d.seq].plan_cnt))
    JOIN (v
    WHERE (v.sql_id=dcr_plan_data->qual[d.seq].sql_id)
     AND (v.plan_hash_value=dcr_plan_data->qual[d.seq].plans[d2.seq].plan_hash_value))
   ORDER BY envid, v.sql_id, v.plan_hash_value,
    v.child_number DESC
   HEAD envid
    row + 0
   HEAD v.sql_id
    row + 0
   HEAD v.plan_hash_value
    CALL dcr_add_child_number(v.sql_id,v.plan_hash_value,v.child_number,dcr_plan_data->qual[d.seq].
    env_id)
   WITH nocounter
  ;end select
  IF ((dm_err->debug_flag > 0))
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Obtain plan data for reporting."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 FOR (dcpdr_sql_cnt = 1 TO dcr_plan_data->cnt)
   IF ((dcr_plan_data->qual[dcpdr_sql_cnt].sql_text_cnt > 0))
    FOR (dcpdr_plan_cnt = 1 TO dcr_plan_data->qual[dcpdr_sql_cnt].plan_cnt)
      IF ((dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].child_number >= 0))
       IF (dcr_init_sqlplan(null)=0)
        GO TO exit_program
       ENDIF
       SET dcr_sqlplan->sql_id = dcr_plan_data->qual[dcpdr_sql_cnt].sql_id
       SET dcr_sqlplan->child_number = dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].
       child_number
       SET dcr_sqlplan->sqltext = ""
       SET dcr_sqlplan->mode = "CURSOR"
       SET dcr_sqlplan->plan_hash = dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].
       plan_hash_value
       IF (dcr_get_plan(null)=0)
        GO TO exit_program
       ENDIF
       CALL dcr_add_plan_txt(dcr_plan_data->qual[dcpdr_sql_cnt].sql_id,dcr_plan_data->qual[
        dcpdr_sql_cnt].plans[dcpdr_plan_cnt].plan_hash_value,dcr_plan_data->qual[dcpdr_sql_cnt].
        plans[dcpdr_plan_cnt].child_number,dcr_plan_data->qual[dcpdr_sql_cnt].env_id," ",
        1)
       FOR (dcpdr_cnt = 1 TO dcr_sqlplan->cnt)
         CALL dcr_add_plan_txt(dcr_plan_data->qual[dcpdr_sql_cnt].sql_id,dcr_plan_data->qual[
          dcpdr_sql_cnt].plans[dcpdr_plan_cnt].plan_hash_value,dcr_plan_data->qual[dcpdr_sql_cnt].
          plans[dcpdr_plan_cnt].child_number,dcr_plan_data->qual[dcpdr_sql_cnt].env_id,dcr_sqlplan->
          qual[dcpdr_cnt].plan_line,
          0)
       ENDFOR
      ENDIF
    ENDFOR
 ...
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dcr_plan_data)
 ENDIF
 SET dm_err->eproc = "Build SQLPLAN HISTORY report."
 SELECT INTO value(dcpdr_fname)
  FROM (dummyt d  WITH seq = 1)
  DETAIL
   CALL print("PLAN HISTORY REPORT"),
   CALL center(concat("Report Date/Time: ",format(cnvtdatetime(curdate,curtime3),
     "DD-MMM-YY HH:MM:SS;;D")),1,132), row + 1,
   row + 1,
   CALL print(concat("Environment Name:",dcpdr_env_name)), row + 1,
   CALL print(concat("Database Name:",dcpdr_env_dbname)), row + 1,
   CALL print(concat("Dates After:",dcpdr_exclude_dt_str)),
   row + 1,
   CALL print(concat("Search String:",dcpdr_search_string)), row + 1,
   row + 1
   IF ((dcr_plan_data->cnt=0))
    CALL print("NO SQL IDS AVAILABLE"), row + 1
   ENDIF
   FOR (dcpdr_sql_cnt = 1 TO dcr_plan_data->cnt)
     IF ((dcr_plan_data->qual[dcpdr_sql_cnt].sql_text_cnt > 0)
      AND (dcr_plan_data->qual[dcpdr_sql_cnt].env_id=0.0))
      row + 1,
      CALL print(fillstring(132,"*")), row + 1,
      CALL print(concat("SQL_ID: ",dcr_plan_data->qual[dcpdr_sql_cnt].sql_id)), row + 1, row + 1
      FOR (dcpdr_sqltxt_cnt = 1 TO dcr_plan_data->qual[dcpdr_sql_cnt].sql_text_cnt)
       CALL print(dcr_plan_data->qual[dcpdr_sql_cnt].sql_text[dcpdr_sqltxt_cnt].txt),row + 1
      ENDFOR
      row + 1, col 0,
      CALL print("ENVIRONMENT"),
      col 30,
      CALL print("PLAN HASH VALUE"), col 50,
      CALL print("FIRST_LOAD_TIME"), col 70,
      CALL print("FIRST GETS/EXEC"),
      col 90,
      CALL print("LAST_LOAD_TIME"), col 110,
      CALL print("LAST GETS/EXEC"), row + 1, col 0,
      CALL print("-----------"), col 30,
      CALL print("---------------"),
      col 50,
      CALL print("---------------"), col 70,
      ...
      col 110,
      CALL print("--------------"), row + 1
      FOR (dcpdr_sql_cnt2 = 1 TO dcr_plan_data->cnt)
        IF ((dcr_plan_data->qual[dcpdr_sql_cnt].sql_id=dcr_plan_data->qual[dcpdr_sql_cnt2].sql_id))
         FOR (dcpdr_plan_cnt = 1 TO dcr_plan_data->qual[dcpdr_sql_cnt2].plan_cnt)
           col 0,
           CALL print(dcr_plan_data->qual[dcpdr_sql_cnt2].env_name), col 30,
           CALL print(trim(cnvtstring(dcr_plan_data->qual[dcpdr_sql_cnt2].plans[dcpdr_plan_cnt].
             plan_hash_value,17))), col 50,
           CALL print(format(cnvtdatetime(dcr_plan_data->qual[dcpdr_sql_cnt2].plans[dcpdr_plan_cnt].
             first_load_dt_tm),"DD-MMM-YY HH:MM:SS;;D")),
           col 70,
           CALL print(trim(cnvtstring(dcr_plan_data->qual[dcpdr_sql_cnt2].plans[dcpdr_plan_cnt].
             first_gets_exec,17))), col 90,
           CALL print(format(cnvtdatetime(dcr_plan_data->qual[dcpdr_sql_cnt2].plans[dcpdr_plan_cnt].
             last_load_dt_tm),"DD-MMM-YY HH:MM:SS;;D")), col 110,
           CALL print(trim(cnvtstring(dcr_plan_data->qual[dcpdr_sql_cnt2].plans[dcpdr_plan_cnt].
             last_gets_exec,17))),
           row + 1
         ENDFOR
        ENDIF
      ENDFOR
      dcpdr_plans->cnt = 0, stat = alterlist(dcpdr_plans->qual,0)
      FOR (dcpdr_sql_cnt2 = 1 TO dcr_plan_data->cnt)
        IF ((dcr_plan_data->qual[dcpdr_sql_cnt].sql_id=dcr_plan_data->qual[dcpdr_sql_cnt2].sql_id))
         FOR (dcpdr_plan_cnt = 1 TO dcr_plan_data->qual[dcpdr_sql_cnt2].plan_cnt)
           IF ((((dcpdr_plans->cnt=0)) OR (locateval(dcpdr_ndx,1,dcpdr_plans->cnt,dcr_plan_data->
            qual[dcpdr_sql_cnt2].plans[dcpdr_plan_cnt].plan_hash_value,dcpdr_plans->qual[dcpdr_ndx].
            phash)=0)) )
            dcpdr_plans->cnt = (dcpdr_plans->cnt+ 1), stat = alterlist(dcpdr_plans->qual,dcpdr_plans
             ->cnt), dcpdr_plans->qual[dcpdr_plans->cnt].phash = dcr_plan_data->qual[dcpdr_sql_cnt2].
            plans[dcpdr_plan_cnt].plan_hash_value
            IF ((((dcr_plan_data->qual[dcpdr_sql_cnt2].plans[dcpdr_plan_cnt].plan_text_cnt=0)) OR (((
            (dcr_plan_data->qual[dcpdr_sql_cnt2].plans[dcpdr_plan_cnt].plan_text[1].txt=
            "SQL_ID:*cannot be found*")) OR ((dcr_plan_data->qual[dcpdr_sql_cnt2].plans[
            dcpdr_plan_cnt].plan_text[1].txt="NOTE: cannot fetch plan*"))) )) )
             row + 1,
             CALL print(concat("PLAN HASH VALUE ",trim(cnvtstring(dcr_plan_data->qual[dcpdr_sql_cnt2]
                .plans[dcpdr_plan_cnt].plan_hash_value,17))," - NO PLAN AVAILABLE")), row + 1
            ELSE
             row + 1,
             CALL print(concat("PLAN HASH VALUE ",trim(cnvtstring(dcr_plan_data->qual[dcpdr_sql_cnt2]
                .plans[dcpdr_plan_cnt].plan_hash_value,17)))), row + 1
             FOR (dcpdr_plantxt_cnt = 1 TO dcr_plan_data->qual[dcpdr_sql_cnt2].plans[dcpdr_plan_cnt].
             plan_text_cnt)
              CALL print(dcr_plan_data->qual[dcpdr_sql_cnt2].plans[dcpdr_plan_cnt].plan_text[
              dcpdr_plantxt_cnt].txt),row + 1
             ENDFOR
            ENDIF
           ENDIF
         ENDFOR
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Dm2_cbo_plan_diff2_rpt completed"
 CALL final_disp_msg("dm2_plndif2rpt")
END GO
