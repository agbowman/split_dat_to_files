CREATE PROGRAM dm_risk_adjustment_day_rows:dba
 ...
 SET reply->table_name = "RISK_ADJUSTMENT_DAY"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    minval = min(rad.risk_adjustment_day_id)
    FROM risk_adjustment_day rad
    WHERE rad.risk_adjustment_day_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   maxval = max(rad.risk_adjustment_day_id)
   FROM risk_adjustment_day rad
   DETAIL
  ...
  WHILE (v_cur_min_id <= v_max_id
   AND v_rows_left > 0)
    SELECT INTO "nl:"
     rad.rowid
     FROM risk_adjustment_day rad
     WHERE parser(sbr_getrowidnotexists("RAD.updt_dt_tm < cnvtdatetime(v_days_back)","RAD"))
      AND rad.active_ind=0
      AND rad.risk_adjustment_day_id BETWEEN v_cur_min_id AND v_cur_max_id
      AND  EXISTS (
     (SELECT
     ...
     DETAIL
      ...
      ENDIF
      reply->rows[v_rows].row_id = rad.rowid
     WITH nocounter, maxqual(rad,value(v_rows_left))
    ;end select
