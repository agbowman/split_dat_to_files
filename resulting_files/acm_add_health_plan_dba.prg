CREATE PROGRAM acm_add_health_plan:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->health_plan_qual[xref->add[index].idx].health_plan_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(health_plan_seq,nextval)
     FROM dual
     DETAIL
      acm_request->health_plan_qual[xref->add[index].idx].health_plan_id = cnvtreal(nextseqnum),
      reply->health_plan_qual[xref->add[index].idx].health_plan_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->health_plan_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->health_plan_qual[xref->add[index].idx].health_plan_id = acm_request->health_plan_qual[
    xref->add[index].idx].health_plan_id
   ENDIF
   SET reply->health_plan_qual[xref->add[index].idx].status = 0
   IF ((acm_request->health_plan_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->health_plan_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->health_plan_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->health_plan_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   health_plan h
  SET h.health_plan_id = acm_request->health_plan_qual[xref->add[d.seq].idx].health_plan_id, h
   .baby_coverage_cd = acm_request->health_plan_qual[xref->add[d.seq].idx].baby_coverage_cd, h
   .beg_effective_dt_tm = cnvtdatetime(acm_request->health_plan_qual[xref->add[d.seq].idx].
    beg_effective_dt_tm),
   h.comb_baby_bill_cd = acm_request->health_plan_qual[xref->add[d.seq].idx].comb_baby_bill_cd, h
   .end_effective_dt_tm = cnvtdatetime(acm_request->health_plan_qual[xref->add[d.seq].idx].
    end_effective_dt_tm), h.financial_class_cd = acm_request->health_plan_qual[xref->add[d.seq].idx].
   financial_class_cd,
   h.ft_entity_id = acm_request->health_plan_qual[xref->add[d.seq].idx].ft_entity_id, h
   .ft_entity_name = acm_request->health_plan_qual[xref->add[d.seq].idx].ft_entity_name, h.group_name
    = acm_request->health_plan_qual[xref->add[d.seq].idx].group_name,
   h.group_nbr = acm_request->health_plan_qual[xref->add[d.seq].idx].group_nbr, h.pat_bill_pref_flag
    = acm_request->health_plan_qual[xref->add[d.seq].idx].pat_bill_pref_flag, h.plan_class_cd =
   acm_request->health_plan_qual[xref->add[d.seq].idx].plan_class_cd,
   h.plan_desc = acm_request->health_plan_qual[xref->add[d.seq].idx].plan_desc, h.plan_name =
   acm_request->health_plan_qual[xref->add[d.seq].idx].plan_name, h.plan_type_cd = acm_request->
   health_plan_qual[xref->add[d.seq].idx].plan_type_cd,
   h.policy_nbr = acm_request->health_plan_qual[xref->add[d.seq].idx].policy_nbr, h
   .pri_concurrent_ind = acm_request->health_plan_qual[xref->add[d.seq].idx].pri_concurrent_ind, h
   .product_cd = acm_request->health_plan_qual[xref->add[d.seq].idx].product_cd,
   h.sec_concurrent_ind = acm_request->health_plan_qual[xref->add[d.seq].idx].sec_concurrent_ind, h
   .data_status_cd =
   IF ((acm_request->health_plan_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request->
    health_plan_qual[xref->add[d.seq].idx].data_status_cd
   ELSE reqdata->data_status_cd
   ENDIF
   , h.data_status_dt_tm = cnvtdatetime(curdate,curtime3),
   h.data_status_prsnl_id = reqinfo->updt_id, h.active_ind =
   IF ((((acm_request->health_plan_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((acm_request
   ->health_plan_qual[xref->add[d.seq].idx].active_status_cd=reqdata->active_status_cd))) ) 1
   ELSE 0
   ENDIF
   , h.active_status_cd =
   IF ((acm_request->health_plan_qual[xref->add[d.seq].idx].active_status_cd > 0.0)) acm_request->
    health_plan_qual[xref->add[d.seq].idx].active_status_cd
   ELSE reqdata->active_status_cd
   ENDIF
   ,
   h.active_status_prsnl_id = reqinfo->updt_id, h.active_status_dt_tm = cnvtdatetime(curdate,curtime3
    ), h.updt_cnt = 0,
   h.updt_dt_tm = cnvtdatetime(curdate,curtime3), h.updt_id = reqinfo->updt_id, h.updt_applctx =
   reqinfo->updt_applctx,
   h.updt_task = reqinfo->updt_task
  PLAN (d)
   JOIN (h)
  WITH nocounter, status(reply->health_plan_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->health_plan_qual[xref->add[index].idx].status=0))
