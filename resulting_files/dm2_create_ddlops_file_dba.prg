CREATE PROGRAM dm2_create_ddlops_file:dba
 DECLARE dcdf_v_command_to_execute = vc WITH private, noconstant(" ")
 DECLARE dcdf_ddlops_file_name = vc WITH public, noconstant(" ")
 DECLARE dcdf_database = vc WITH private, noconstant(" ")
 DECLARE dcdf_col_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcdf_col_list = vc WITH protect, noconstant(" ")
 DECLARE dcdf_oper_is_long = i2 WITH protect, noconstant(0)
 DECLARE dcdf_load_record(dlr_ndx=i4,dlr_pre_cond=vc,dlr_cmd=vc,dlr_reset=i2) = i2
 DECLARE dcdf_update_logfile_name = i2 WITH protect, noconstant(0)
 DECLARE dcdf_update_obj_name = i2 WITH protect, noconstant(0)
 DECLARE dcdf_product_info = vc WITH protect, noconstant("")
 DECLARE dcdf_oracle_edition = vc WITH protect, noconstant("")
 DECLARE dcdf_index_online_option = vc WITH protect, noconstant("")
 DECLARE dcdf_v_col_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcdf_v_oper_is_long = i2 WITH protect, noconstant(0)
 DECLARE dcdf_v_update_logfile_name = i2 WITH protect, noconstant(0)
 DECLARE dcdf_v_update_obj_name = i2 WITH protect, noconstant(0)
 DECLARE dcdf_table_column_cnt = i4 WITH protect, noconstant(0)
 ...
 IF (dm2_prg_maint("BEGIN") != 1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "*> BEGINNING DM2_CREATE_DDLOPS_FILE.PRG...."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ( NOT (currdbuser IN ("V500", "CDBA")))
  SET dm_err->eproc = "Check for Oracle Standard Edition override."
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="DM2_ORACLE_EDITION_OVERRIDE"
   DETAIL
    dcdf_oracle_edition = di.info_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF (curqual > 0)
   SET dm_err->eproc = concat("Check for Oracle Edition override.  Override found with value of ",
    trim(dcdf_oracle_edition),".")
   SET dcdf_oracle_edition = trim(cnvtupper(dcdf_oracle_edition))
  ELSE
   SET dm_err->eproc = "Getting product component information for non-v500 database user."
   SELECT INTO "nl:"
    FROM product_component_version p
    WHERE cnvtupper(p.product)="ORACLE*"
    DETAIL
     dcdf_product_info = cnvtupper(p.product)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSEIF (curqual=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Product component info not found."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ENDIF
   IF (findstring("ENTERPRISE",dcdf_product_info)=0)
    SET dcdf_oracle_edition = "STANDARD"
   ENDIF
   SET dm_err->eproc = concat('Check for Oracle Edition.  "',evaluate(dcdf_oracle_edition,"STANDARD",
     trim(dcdf_oracle_edition),"Non-Standard"),'" found.')
  ENDIF
 ENDIF
 IF (dcdf_oracle_edition="STANDARD")
  SET dcdf_index_online_option = ""
 ELSE
  SET dcdf_index_online_option = "online"
 ENDIF
 SET dcdf_v_command_to_execute = concat("rdb create table DM2_DDL_OPS1",
  " (RUN_ID NUMBER NOT NULL, GEN_DT_TM DATE, OCD NUMBER, SCHEMA_DATE DATE, ",
  " PROCESS_OPTION VARCHAR2(50), LAST_CHECKPOINT VARCHAR2(50) "," ) tablespace %TABLETSPACE% go ")
 CALL dcdf_load_record(30,"call pc_table_not_exists('DM2_DDL_OPS1') go",dcdf_v_command_to_execute,1)
 SET dcdf_v_command_to_execute = concat("rdb alter table DM2_DDL_OPS1 add constraint XPKDM2DDLOPS1",
  " primary key (RUN_ID) using index tablespace %INDEXTSPACE% go ")
 CALL dcdf_load_record(50,"call pc_constraint_not_exists('DM2_DDL_OPS1','XPKDM2DDLOPS1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = "call create_view('DM2_DDL_OPS','DM2_DDL_OPS1') go"
 CALL dcdf_load_record(60,"call pc_view_not_exists('DM2_DDL_OPS') go",dcdf_v_command_to_execute,0)
 IF ((validate(tgtsch->tbl_cnt,- (1)) != - (1)))
  SET dcdf_v_command_to_execute = "call ddt_update_tgtsch(1,'DM2_DDL_OPS1',0,0) go"
  CALL dcdf_load_record(80," ",dcdf_v_command_to_execute,0)
 ENDIF
 SET dcdf_table_column_cnt = 40
 SET dcdf_v_command_to_execute = concat("rdb create table DM2_DDL_OPS_LOG1"," (",
  "  OP_ID NUMBER not null,","  RUN_ID NUMBER NOT NULL, ","  TABLE_NAME VARCHAR2(30) NOT NULL, ",
  "  OBJ_NAME VARCHAR2(150) NOT NULL, ","  OP_TYPE VARCHAR2(60) NOT NULL, ",
  "  PRIORITY NUMBER NOT NULL, ","  TABLESPACE_NAME VARCHAR2(30), ","  SCHEMA_INSTANCE NUMBER,",
  "  SCHEMA_DATE DATE,","  GEN_ID NUMBER, ","  GEN_DT_TM DATE,","  PARENT_OP_ID NUMBER NULL,",
  "  PARENT_EXECUTION_ORDER NUMBER NULL,",
  "  BEGIN_DT_TM DATE, END_DT_TM DATE, ","  EST_DURATION FLOAT, ACT_DURATION FLOAT, ",
  "  ROW_CNT NUMBER, ","  STATUS VARCHAR2(30), ","  ERROR_MSG VARCHAR2(4000), ",
  "  LOGFILE_NAME VARCHAR2(80),  ","  READY_TO_RUN_IND NUMBER, ","  OPERATION CLOB, ",
  "  ON_ERROR_OPERATION VARCHAR2(4000), ","  OWNER_NAME VARCHAR2(30) DEFAULT 'V500' ,",
  "  JOB_NAME VARCHAR2(30)                  ,","  EI_METHOD_FLAG VARCHAR2(30)            ,",
  "  EI_EXP_PARFILE_OPTIONS VARCHAR2(1000)  ,","  EI_IMP_PARFILE_OPTIONS VARCHAR2(1000)  ,",
  "  IGNORABLE_ERROR_CODES VARCHAR2(500)    ,",
  "  ROWS_PROCESSED_VALUE NUMBER        ,","  TABLE_SIZE_MB_VALUE NUMBER         ,",
  "  EI_TABLE_EXISTS_ACTION VARCHAR2(30)    ,","  EI_WHERE_CLAUSE VARCHAR2(4000)         ,",
  "  EI_DATAPUMP_EXEC VARCHAR2(30)          ,",
  "  UPDT_CNT NUMBER DEFAULT 0              ,","  UPDT_DT_TM DATE DEFAULT SYSDATE        ,",
  "  APPL_ID VARCHAR2(255) ,","  REFERENCED_OBJECT VARCHAR2(128) ,","  WAIT_VAL NUMBER) ",
  "  tablespace %TABLETSPACE% "," go")
 CALL dcdf_load_record(110,"call pc_table_not_exists('DM2_DDL_OPS_LOG1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = concat(
  "rdb alter table DM2_DDL_OPS_LOG1 add constraint XPKDM2DDLOPSLOG1",
  " primary key (OP_ID) using index tablespace %INDEXTSPACE% go ")
 CALL dcdf_load_record(130,"call pc_constraint_not_exists('DM2_DDL_OPS_LOG1','XPKDM2DDLOPSLOG1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = concat(
  "rdb create index XIE2DM2DDLOPSLOG1 on DM2_DDL_OPS_LOG1 (STATUS) "," logging ",trim(
   dcdf_index_online_option,3)," tablespace %INDEXTSPACE% go ")
 CALL dcdf_load_record(131,"call pc_index_not_exists('DM2_DDL_OPS_LOG1','XIE2DM2DDLOPSLOG1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = concat("rdb create index XIE3DM2DDLOPSLOG1 on DM2_DDL_OPS_LOG1",
  " (READY_TO_RUN_IND,EST_DURATION) "," logging ",trim(dcdf_index_online_option,3),
  " tablespace %INDEXTSPACE% go ")
 CALL dcdf_load_record(132,"call pc_index_not_exists('DM2_DDL_OPS_LOG1','XIE3DM2DDLOPSLOG1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = concat("rdb create index XIE4DM2DDLOPSLOG1 on DM2_DDL_OPS_LOG1",
  " (RUN_ID,STATUS,TABLE_NAME) "," logging ",trim(dcdf_index_online_option,3),
  " tablespace %INDEXTSPACE% go ")
 CALL dcdf_load_record(133,"call pc_index_not_exists('DM2_DDL_OPS_LOG1','XIE4DM2DDLOPSLOG1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = concat("rdb create index XIE5DM2DDLOPSLOG1 on DM2_DDL_OPS_LOG1",
  " (OP_TYPE,TABLE_NAME) "," logging ",trim(dcdf_index_online_option,3),
  " tablespace %INDEXTSPACE% go ")
 CALL dcdf_load_record(134,"call pc_index_not_exists('DM2_DDL_OPS_LOG1','XIE5DM2DDLOPSLOG1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = concat("rdb create index XIE6DM2DDLOPSLOG1 on DM2_DDL_OPS_LOG1",
  " (APPL_ID,STATUS) "," logging ",trim(dcdf_index_online_option,3)," tablespace %INDEXTSPACE% go "
  )
 CALL dcdf_load_record(135,"call pc_index_not_exists('DM2_DDL_OPS_LOG1','XIE6DM2DDLOPSLOG1') go",
  dcdf_v_command_to_execute,0)
 SET dm_err->eproc = "Evaluate the need for additional columns on DM2_DDL_OPS_LOG1"
 SELECT INTO "nl:"
  FROM user_tab_columns u
  WHERE u.table_name IN ("DM2_DDL_OPS_LOG1", "DM2_DDL_OPS_LOG")
   AND  NOT (u.column_name IN ("LAST_UTC_TS", "TXN_ID_TEXT", "INST_ID"))
  ORDER BY u.table_name
  HEAD REPORT
   owner_name_fnd = 0, job_name_fnd = 0, ei_method_flag_fnd = 0,
   ei_exp_parfile_options_fnd = 0, ei_imp_parfile_options_fnd = 0, ignorable_error_codes_fnd = 0,
   rows_processed_value_fnd = 0, table_size_mb_value_fnd = 0, ei_table_exists_action_fnd = 0,
   ei_where_clause_fnd = 0, ei_datapump_exec_fnd = 0, updt_cnt_fnd = 0,
   updt_dt_tm_fnd = 0, referenced_object_fnd = 0, wait_val_fnd = 0,
   dcdf_v_col_cnt = 0, dcdf_v_oper_is_long = 0, dcdf_v_update_logfile_name = 0,
   dcdf_v_update_obj_name = 0
  DETAIL
   IF (u.table_name="DM2_DDL_OPS_LOG1")
    CASE (u.column_name)
     OF "OWNER_NAME":
      owner_name_fnd = 1
     OF "JOB_NAME":
      job_name_fnd = 1
     OF "EI_METHOD_FLAG":
      ei_method_flag_fnd = 1
     OF "EI_EXP_PARFILE_OPTIONS":
      ei_exp_parfile_options_fnd = 1
     OF "EI_IMP_PARFILE_OPTIONS":
      ei_imp_parfile_options_fnd = 1
     OF "IGNORABLE_ERROR_CODES":
      ignorable_error_codes_fnd = 1
     OF "ROWS_PROCESSED_VALUE":
      rows_processed_value_fnd = 1
     OF "TABLE_SIZE_MB_VALUE":
      table_size_mb_value_fnd = 1
     OF "EI_TABLE_EXISTS_ACTION":
      ei_table_exists_action_fnd = 1
     OF "EI_WHERE_CLAUSE":
      ei_where_clause_fnd = 1
     OF "EI_DATAPUMP_EXEC":
      ei_datapump_exec_fnd = 1
     OF "UPDT_CNT":
      updt_cnt_fnd = 1
     OF "UPDT_DT_TM":
      updt_dt_tm_fnd = 1
     OF "REFERENCED_OBJECT":
      referenced_object_fnd = 1
     OF "WAIT_VAL":
      wait_val_fnd = 1
    ENDCASE
    dcdf_col_cnt = (dcdf_col_cnt+ 1)
    IF (u.column_name="OPERATION"
     AND u.data_type="LONG")
     dcdf_oper_is_long = 1
    ELSEIF (u.column_name="LOGFILE_NAME"
     AND u.data_length=40)
     dcdf_update_logfile_name = 1
    ELSEIF (u.column_name="OBJ_NAME"
     AND u.data_length=60)
     dcdf_update_obj_name = 1
    ENDIF
   ELSE
    dcdf_v_col_cnt = (dcdf_v_col_cnt+ 1)
    IF (u.column_name="OPERATION"
     AND u.data_type="LONG")
     dcdf_v_oper_is_long = 1
    ELSEIF (u.column_name="LOGFILE_NAME"
     AND u.data_length=40)
     dcdf_v_update_logfile_name = 1
    ELSEIF (u.column_name="OBJ_NAME"
     AND u.data_length=60)
     dcdf_v_update_obj_name = 1
    ENDIF
   ENDIF
  FOOT  u.table_name
   IF (u.table_name="DM2_DDL_OPS_LOG1")
    dcdf_col_list = " "
    IF (owner_name_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",owner_name varchar2(30) default 'V500'")
    ENDIF
    IF (job_name_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",job_name varchar2(30)")
    ENDIF
    IF (ei_method_flag_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",ei_method_flag varchar2(30)")
    ENDIF
    IF (ei_exp_parfile_options_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",ei_exp_parfile_options varchar2(1000)")
    ENDIF
    IF (ei_imp_parfile_options_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",ei_imp_parfile_options varchar2(1000)")
    ENDIF
    IF (ignorable_error_codes_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",ignorable_error_codes varchar2(500)")
    ENDIF
    IF (rows_processed_value_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",rows_processed_value number")
    ENDIF
    IF (table_size_mb_value_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",table_size_mb_value number")
    ENDIF
    IF (ei_table_exists_action_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",ei_table_exists_action varchar2(30)")
    ENDIF
    IF (ei_where_clause_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",ei_where_clause varchar2(4000)")
    ENDIF
    IF (ei_datapump_exec_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",ei_datapump_exec varchar2(30)")
    ENDIF
    IF (updt_cnt_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",updt_cnt number default 0")
    ENDIF
    IF (updt_dt_tm_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",updt_dt_tm date default sysdate")
    ENDIF
    IF (referenced_object_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",referenced_object varchar2(128)")
    ENDIF
    IF (wait_val_fnd=0)
     dcdf_col_list = concat(dcdf_col_list,",wait_val number")
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF (dcdf_oper_is_long=1)
  SET dcdf_v_command_to_execute = "rdb alter table dm2_ddl_ops_log1 modify (operation CLOB) go"
  CALL dcdf_load_record(136," ",dcdf_v_command_to_execute,0)
  SET dcdf_v_command_to_execute = "rdb alter index XPKDM2DDLOPSLOG1 rebuild go"
  CALL dcdf_load_record(137," ",dcdf_v_command_to_execute,0)
  SET dcdf_v_command_to_execute = "rdb alter index XIE2DM2DDLOPSLOG1 rebuild go"
  CALL dcdf_load_record(138," ",dcdf_v_command_to_execute,0)
  SET dcdf_v_command_to_execute = "rdb alter index XIE3DM2DDLOPSLOG1 rebuild go"
  CALL dcdf_load_record(139," ",dcdf_v_command_to_execute,0)
  SET dcdf_v_command_to_execute = "rdb alter index XIE4DM2DDLOPSLOG1 rebuild go"
  CALL dcdf_load_record(140," ",dcdf_v_command_to_execute,0)
  SET dcdf_v_command_to_execute = "rdb alter index XIE5DM2DDLOPSLOG1 rebuild go"
  CALL dcdf_load_record(141," ",dcdf_v_command_to_execute,0)
  SET dcdf_v_command_to_execute = "rdb alter index XIE6DM2DDLOPSLOG1 rebuild go"
  CALL dcdf_load_record(142," ",dcdf_v_command_to_execute,0)
 ENDIF
 IF (dcdf_update_logfile_name=1)
  SET dcdf_v_command_to_execute =
  "rdb alter table dm2_ddl_ops_log1 modify (logfile_name VARCHAR2(80)) go"
  CALL dcdf_load_record(143," ",dcdf_v_command_to_execute,0)
 ENDIF
 IF (dcdf_update_obj_name=1)
  SET dcdf_v_command_to_execute =
  "rdb alter table dm2_ddl_ops_log1 modify (OBJ_NAME VARCHAR2(150)) go"
  CALL dcdf_load_record(143," ",dcdf_v_command_to_execute,0)
 ENDIF
 IF (dcdf_col_cnt > 0
  AND dcdf_col_list != " ")
  SET dcdf_v_command_to_execute = concat("rdb alter table dm2_ddl_ops_log1 add (",substring(2,textlen
    (dcdf_col_list),dcdf_col_list),") go")
  CALL dcdf_load_record(144," ",dcdf_v_command_to_execute,0)
 ENDIF
 IF (((dcdf_oper_is_long=1) OR (dcdf_col_cnt > 0
  AND ((dcdf_col_list != " ") OR (((dcdf_update_logfile_name=1) OR (dcdf_update_obj_name=1)) )) )) )
  SET dcdf_v_command_to_execute = "oragen3 'dm2_ddl_ops_log*' go"
  CALL dcdf_load_record(146," ",dcdf_v_command_to_execute,0)
 ENDIF
 IF (((dcdf_v_col_cnt != dcdf_table_column_cnt) OR (((dcdf_v_oper_is_long=1) OR (((
 dcdf_v_update_logfile_name=1) OR (dcdf_v_update_obj_name=1)) )) )) )
  SET dcdf_v_command_to_execute = "call create_view('DM2_DDL_OPS_LOG','DM2_DDL_OPS_LOG1') go"
  CALL dcdf_load_record(150," ",dcdf_v_command_to_execute,0)
 ENDIF
 IF ((validate(tgtsch->tbl_cnt,- (1)) != - (1)))
  SET dcdf_v_command_to_execute = "call ddt_update_tgtsch(1,'DM2_DDL_OPS_LOG1',0,0) go"
  CALL dcdf_load_record(160," ",dcdf_v_command_to_execute,0)
 ENDIF
 SET dcdf_v_command_to_execute = concat("rdb create table DM2_TSPACE_SIZE1",
  "(INSTALL_TYPE VARCHAR2(30) NOT NULL, ","INSTALL_TYPE_VALUE VARCHAR2(30) NOT NULL,",
  "TSPACE_NAME VARCHAR2(30) NOT NULL, ","BYTES_NEEDED FLOAT, ",
  "ERR_MSG VARCHAR2(500), ","EXTENT_SIZE_BYTES FLOAT, ","CHUNK_SIZE FLOAT, ",
  "INSTALL_STATUS VARCHAR2(20), ","NEW_IND NUMBER, ",
  "UPDT_DT_TM DATE, EXTENT_MANAGEMENT VARCHAR(10)) ","tablespace %TABLETSPACE% go")
 CALL dcdf_load_record(1000,"call pc_table_not_exists('DM2_TSPACE_SIZE1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = concat("rdb alter table DM2_TSPACE_SIZE1 ",
  "add constraint XPKDM2TSPACE_SIZE1 ","primary key (INSTALL_TYPE, INSTALL_TYPE_VALUE, TSPACE_NAME) ",
  "using index tablespace %INDEXTSPACE% go")
 CALL dcdf_load_record(1020,
  "call pc_constraint_not_exists('DM2_TSPACE_SIZE1','XPKDM2TSPACE_SIZE1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = "call create_view('DM2_TSPACE_SIZE','DM2_TSPACE_SIZE1') go "
 CALL dcdf_load_record(1030,"call pc_view_not_exists('DM2_TSPACE_SIZE') go",dcdf_v_command_to_execute,
  0)
 IF ((validate(tgtsch->tbl_cnt,- (1)) != - (1)))
  SET dcdf_v_command_to_execute = "call ddt_update_tgtsch(1,'DM2_TSPACE_SIZE1',0,0) go  "
  CALL dcdf_load_record(1050," ",dcdf_v_command_to_execute,0)
 ENDIF
 SET dcdf_v_command_to_execute = concat("rdb create table DM2_TSPACE_OBJ_SIZE1",
  " (INSTALL_TYPE VARCHAR2(30) NOT NULL, INSTALL_TYPE_VALUE VARCHAR2(30) NOT NULL, ",
  " GEN_ID NUMBER NOT NULL,REQUEST_TYPE VARCHAR2(30) NOT NULL, ",
  " TABLE_NAME VARCHAR2(30) NOT NULL,OBJ_TYPE VARCHAR2(30) NOT NULL, ",
  " OBJ_NAME VARCHAR2(30) NOT NULL, TSPACE_NAME VARCHAR2(30) NOT NULL, ",
  " BYTES_NEEDED FLOAT, INSTALL_STATUS VARCHAR2(20),UPDT_DT_TIME DATE ",
  " ) tablespace %TABLETSPACE% go ")
 CALL dcdf_load_record(200,"call pc_table_not_exists('DM2_TSPACE_OBJ_SIZE1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = concat(
  "rdb alter table DM2_TSPACE_OBJ_SIZE1 add constraint XPKDM2OBJ_NEEDS1",
  " primary key (INSTALL_TYPE, INSTALL_TYPE_VALUE, GEN_ID, REQUEST_TYPE, TABLE_NAME,",
  " OBJ_TYPE, OBJ_NAME) using index tablespace %INDEXTSPACE% go ")
 CALL dcdf_load_record(250,
  "call pc_constraint_not_exists('DM2_TSPACE_OBJ_SIZE1','XPKDM2OBJ_NEEDS1') go",
  dcdf_v_command_to_execute,0)
 SET dcdf_v_command_to_execute = "call create_view('DM2_TSPACE_OBJ_SIZE','DM2_TSPACE_OBJ_SIZE1') go"
 CALL dcdf_load_record(260,"call pc_view_not_exists('DM2_TSPACE_OBJ_SIZE') go",
  dcdf_v_command_to_execute,0)
 IF ((validate(tgtsch->tbl_cnt,- (1)) != - (1)))
  SET dcdf_v_command_to_execute = "call ddt_update_tgtsch(1,'DM2_TSPACE_OBJ_SIZE1',0,0) go"
  CALL dcdf_load_record(280," ",dcdf_v_command_to_execute,0)
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(ddta_ddlops_xrec)
 ENDIF
 SUBROUTINE dcdf_load_record(dlr_ndx,dlr_pre_cond,dlr_cmd,dlr_reset)
   IF (dlr_reset=1)
    SET ddta_ddlops_xrec->ddlx_cnt = 0
    SET stat = alterlist(ddta_ddlops_xrec->ddlxrec_idx,0)
   ENDIF
   SET ddta_ddlops_xrec->ddlx_cnt = (ddta_ddlops_xrec->ddlx_cnt+ 1)
   SET stat = alterlist(ddta_ddlops_xrec->ddlxrec_idx,ddta_ddlops_xrec->ddlx_cnt)
   SET ddta_ddlops_xrec->ddlxrec_idx[ddta_ddlops_xrec->ddlx_cnt].ddlx_db = "ORACLE"
   SET ddta_ddlops_xrec->ddlxrec_idx[ddta_ddlops_xrec->ddlx_cnt].ddlx_ndx = dlr_ndx
   SET ddta_ddlops_xrec->ddlxrec_idx[ddta_ddlops_xrec->ddlx_cnt].ddlx_pre_condition = dlr_pre_cond
   SET ddta_ddlops_xrec->ddlxrec_idx[ddta_ddlops_xrec->ddlx_cnt].ddlx_pre_condition_met = 1
   SET ddta_ddlops_xrec->ddlxrec_idx[ddta_ddlops_xrec->ddlx_cnt].ddlx_command_to_execute = dlr_cmd
   SET ddta_ddlops_xrec->ddlxrec_idx[ddta_ddlops_xrec->ddlx_cnt].ddlx_on_error = ""
   RETURN(1)
 END ;Subroutine
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "**>EXITING DM2_CREATE_DDLOPS_FILE.PRG... (ALL COMMANDS SUCCESSFUL)."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
