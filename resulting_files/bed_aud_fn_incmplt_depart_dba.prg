CREATE PROGRAM bed_aud_fn_incmplt_depart:dba
 ...
 SET reply->collist[4].header_text = "code_value"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (p.person_id > 0)
    reply->rowlist[row_nbr].celllist[3].string_value = p.name_full_formatted
   ENDIF
   reply->rowlist[row_nbr].celllist[4].double_value = cv.code_value
  WITH nocounter
 ;end select
 SET reply->status_data.status = "S"
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("firstnet_incmplt_depart_audit.csv")
 ENDIF
