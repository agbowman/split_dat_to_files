CREATE PROGRAM dm2_clean_nonmerge_rows:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD tbl_list
 RECORD tbl_list(
   1 qual[*]
     2 table_name = vc
     2 del_ind = i2
 )
 DECLARE dcnr_tbl_cnt = i4
 DECLARE dcnr_tbl_loop = i4
 DECLARE dcnr_del_ind = i2
 IF (check_logfile("dm2_cln_nonmrg_",".log","dm2_cln_nonmrg LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_cln_nonmrg..."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT DISTINCT INTO "NL:"
  d.table_name
  FROM dm_chg_log d
  WHERE d.log_type="REFCHG"
   AND  NOT (d.table_name IN (
  (SELECT
   rt.table_name
   FROM dm_rdds_refmrg_tables rt)))
  HEAD REPORT
   dcnr_tbl_cnt = 0, stat = alterlist(tbl_list->qual,100)
  DETAIL
   dcnr_tbl_cnt = (dcnr_tbl_cnt+ 1)
   IF (mod(dcnr_tbl_cnt,100)=1
    AND dcnr_tbl_cnt != 1)
    stat = alterlist(tbl_list->qual,(dcnr_tbl_cnt+ 99))
   ENDIF
   tbl_list->qual[dcnr_tbl_cnt].table_name = d.table_name
  FOOT REPORT
   stat = alterlist(tbl_list->qual,dcnr_tbl_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading distinct tables from DM_CHG_LOG into record structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
 ...
 ENDIF
 IF (dcnr_tbl_cnt=0)
  SET readme_data->message = "SUCCESS - There are no rows in DM_CHG_LOG to clean"
  SET readme_data->status = "S"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Finding mergeable tables..."
 CALL disp_msg("",dm_err->logfile,0)
 SET readme_data->status = "F"
 SET readme_data->message = dm_err->emsg
 ...
 SELECT INTO "NL:"
  FROM dm_tables_doc t,
   (dummyt d  WITH seq = value(dcnr_tbl_cnt))
  PLAN (d)
   JOIN (t
   WHERE (t.table_name=tbl_list->qual[d.seq].table_name)
    AND t.mergeable_ind=0)
  DETAIL
   tbl_list->qual[d.seq].del_ind = 1, dcnr_del_ind = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error selecting mergeable tables from DM_TABLES_DOC"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
 ...
 IF (dcnr_del_ind=0)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Updating dm_chg_log..."
 CALL disp_msg("",dm_err->logfile,0)
 SET readme_data->status = "F"
 SET readme_data->message = "Updating dm_chg_log.."
 EXECUTE dm_readme_status
 FOR (dcnr_tbl_loop = 1 TO dcnr_tbl_cnt)
   IF ((tbl_list->qual[dcnr_tbl_loop].del_ind=1))
    UPDATE  FROM dm_chg_log d
     SET d.log_type = "NORDDS"
     WHERE (d.table_name=tbl_list->qual[dcnr_tbl_loop].table_name)
      AND d.log_type="REFCHG"
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc) != 0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Error updating non-mergeable tables in DM_CHG_LOG"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ROLLBACK
     SET readme_data->status = "F"
     SET readme_data->message = dm_err->emsg
     SET dcnr_tbl_loop = dcnr_tbl_cnt
    ELSE
     COMMIT
    ENDIF
   ENDIF
 ...
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ELSE
  SET readme_data->message = "Finished update of DM_CHG_LOG"
  SET readme_data->status = "S"
 ENDIF
#exit_script
