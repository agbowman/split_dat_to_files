CREATE PROGRAM dm_pcmb_immunization_modifier:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "IMMUNIZATION_MODIFIER"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_IMMUNIZATION_MODIFIER"
 ...
 IF ((request->xxx_combine[icombine].encntr_id=0))
  SELECT INTO "nl:"
   FROM immunization_modifier im
   WHERE (im.person_id=request->xxx_combine[icombine].from_xxx_id)
   HEAD REPORT
   ...
   DETAIL
    ...
    ENDIF
    rreclist->from_rec[v_cust_count1].from_id = im.immunization_modifier_id
   FOOT REPORT
 ...
 ELSE
  SELECT INTO "nl:"
   ...
   PLAN (im
    WHERE (im.person_id=request->xxx_combine[icombine].from_xxx_id))
    JOIN (ce
    WHERE (ce.encntr_id=request->xxx_combine[icombine].encntr_id)
     AND ce.valid_until_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00")
     AND ce.event_id=im.event_id)
   HEAD REPORT
    stat = alterlist(rreclist->from_rec,10)
   DETAIL
    v_cust_count1 += 1
 ...
 IF (v_cust_count1 > 0)
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
   SELECT INTO "nl:"
    FROM immunization_modifier im
    WHERE (im.immunization_modifier_id=rreclist->from_rec[v_cust_loopcount].from_id)
    WITH forupdatewait(im)
   ;end select
 ...
 SUBROUTINE upt_from(s_uf_pk_id,s_uf_to_fk_id)
   UPDATE  FROM immunization_modifier frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.immunization_modifier_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "IMMUNIZATION_MODIFIER"
