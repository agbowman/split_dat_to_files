CREATE PROGRAM dm_rmc_dcl_mon:dba
 ...
 SUBROUTINE drdc_file_success(dfs_name,dfs_file_name,dfs_error_ind)
   ...
   RETURN(null)
 END ;Subroutine
 DECLARE dclmc_menu_ind = i2 WITH protect, noconstant(1)
 DECLARE dclma_menu_ind = i2 WITH protect, noconstant(1)
 DECLARE dclmi_menu_ind = i2 WITH protect, noconstant(1)
 DECLARE dclm_refresh_screen(rs_rec=vc(ref)) = null
 DECLARE dclm_show_help(dsh_rec=vc(ref)) = null
 DECLARE dclm_move_rs(dmr_rs=vc(ref),dmr_all=vc(ref),dmr_type=vc) = null
 SET dm_err->eproc = "Starting dm_rmc_dcl_mon"
 ...
 CALL clear(1,1)
 SET width = 132
 CALL text(1,54,"RDDS Change Log Monitor")
 CALL text(2,1,"Env: ")
 CALL text(2,30,"Source:")
 CALL text(2,81,"Open Event:")
 CALL text(3,1,"Number of Movers:")
 ...
 WHILE (true)
   ...
   SET accept = time(30)
   SET accept = scroll
   CALL dclm_refresh_screen(dclm_rs)
   CALL accept(24,18,"X;CUS","R"
    WHERE curaccept IN ("R", "E", "I", "H", "S"))
   CASE (curscroll)
    OF 0:
     CASE (curaccept)
      OF "R":
       SET dclm_rs->cur_rs_pos = 0
       CALL video(b)
       EXECUTE dm_rmc_dcl_mon_child
       IF ((dm_err->err_ind=1))
        GO TO exit_mon
      ...
      OF "I":
       SET accept = notime
       CALL dclm_move_rs(dclm_rs,dclm_issues,"ISSUES")
      ...
      OF "H":
       SET accept = notime
       SET accept = noscroll
       CALL dclm_show_help(dclm_rs)
       SET accept = time(30)
       SET accept = scroll
      OF "S":
       SET accept = notime
       CALL dclm_move_rs(dclm_rs,dclm_all,"ALL")
       EXECUTE dm_rmc_dcl_all
       IF ((dm_err->err_ind=1))
        GO TO exit_mon
       ENDIF
       SET accept = time(30)
    ...
    OF 1:
    OF 6:
     IF (((dclm_rs->cur_rs_pos+ dclm_rs->max_lines) <= dclm_rs->cnt))
      SET dclm_rs->cur_rs_pos = (dclm_rs->cur_rs_pos+ dclm_rs->max_lines)
     ENDIF
    OF 2:
    OF 5:
     SET dclm_rs->cur_rs_pos = greatest((dclm_rs->cur_rs_pos - dclm_rs->max_lines),0)
   ENDCASE
 ENDWHILE
 SUBROUTINE dclm_refresh_screen(rs_rec)
   ...
   CALL text(2,(132 - size(concat("Open Event: ",rs_rec->oe_name))),concat("Open Event: ",rs_rec->
     oe_name))
   CALL text(3,1,concat("Number of Movers: ",trim(cnvtstring(rs_rec->num_procs))))
   ...
   IF ((rs_rec->no_qual_msg=""))
    CALL text(rs_start,1,"TABLE_NAME")
    CALL text(rs_start,35,"LOG_TYPE")
    CALL text(rs_start,45,"COUNT")
    CALL text(rs_start,53,"CONTEXT_NAME")
    SET rs_rec->max_lines = (22 - rs_start)
    FOR (rs_stop_pos = 1 TO rs_rec->max_lines)
      IF (((rs_stop_pos+ rs_rec->cur_rs_pos) <= rs_rec->cnt))
       CALL text((rs_stop_pos+ rs_start),1,rs_rec->qual[(rs_stop_pos+ rs_rec->cur_rs_pos)].table_name
        )
       CALL text((rs_stop_pos+ rs_start),35,rs_rec->qual[(rs_stop_pos+ rs_rec->cur_rs_pos)].log_type)
       CALL text((rs_stop_pos+ rs_start),45,build(rs_rec->qual[(rs_stop_pos+ rs_rec->cur_rs_pos)].
         row_cnt))
       CALL text((rs_stop_pos+ rs_start),53,rs_rec->qual[(rs_stop_pos+ rs_rec->cur_rs_pos)].
        context_name)
      ENDIF
    ENDFOR
    ...
    ENDIF
    CALL box((rs_start+ 2),80,(rs_start+ 8),131)
    IF ((rs_rec->cur_tier=0))
     CALL text((rs_start+ 3),82,"Displaying counts of DM_CHG_LOG delete rows")
    ELSEIF ((rs_rec->cur_tier=999))
     CALL text((rs_start+ 3),82,"Displaying counts of DM_CHG_LOG non-delete")
     CALL text((rs_start+ 4),82,concat("rows. Table level ",trim(cnvtstring((rs_rec->max_tier+ 1))),
       " of ",trim(cnvtstring((rs_rec->max_tier+ 1)))))
    ELSE
     CALL text((rs_start+ 3),82,"Displaying counts of DM_CHG_LOG non-delete")
     CALL text((rs_start+ 4),82,concat("rows. Table level ",trim(cnvtstring(rs_rec->cur_tier))," of ",
       trim(cnvtstring((rs_rec->max_tier+ 1)))))
    ENDIF
    CALL text((rs_start+ 5),82,"Issues found for")
    CALL text((rs_start+ 6),84,"Change Log   : ")
    CALL text((rs_start+ 7),84,"Mover Process: ")
    IF ((rs_rec->nomv_ind=1))
     CALL text((rs_start+ 6),100,"Yes")
    ELSE
     CALL text((rs_start+ 6),100,"No")
    ENDIF
    IF ((rs_rec->mover_stale_ind=1))
     CALL text((rs_start+ 7),100,"Yes")
    ELSE
     CALL text((rs_start+ 7),100,"No")
    ENDIF
   ELSE
   ...
   CALL text(24,1,concat("Command Options: __ (R)efresh, (H)elp, (S)how All, (I)ssues, (E)xit"))
 END ;Subroutine
 SUBROUTINE dclm_show_help(dsh_rec)
   ...
   CALL text(2,(132 - size(concat("Open Event: ",dsh_rec->oe_name))),concat("Open Event: ",dsh_rec->
     oe_name))
   CALL text(3,1,concat("Number of Movers: ",trim(cnvtstring(dsh_rec->num_procs))))
   IF ((dsh_rec->nomv_ind=1))
    CALL text(3,30,"Change Log Issues found: Yes")
   ELSE
    CALL text(3,30,"Change Log Issues found: No")
   ENDIF
   IF ((dsh_rec->mover_stale_ind=1))
    CALL text(3,60,"Mover Process Issues found: Yes")
   ELSE
    CALL text(3,60,"Mover Process Issues found: No")
   ENDIF
   CALL text(3,96,concat("Last cycle time: ",dsh_rec->cycle_time))
   CALL text(4,1,"Contexts being merged:")
   SET dsh_temp_str = replace(dsh_rec->ctp_str,"::",", ",0)
   SET dsh_start = drdc_wrap_menu_lines(dsh_temp_str,4,24,"",2,
    109)
   CALL text(dsh_start,1,fillstring(132,"-"))
   SET dsh_start = (dsh_start+ 1)
   CALL text(dsh_start,1,
    "This report displays a summary of source change log rows that are about to be processed by movers in target domain."
    )
   CALL text((dsh_start+ 1),1,concat(
     "This report should be used when actively running movers in target domain so that progress of change log ",
     "rows can be monitored."))
   SET dsh_start = (dsh_start+ 2)
   CALL text(dsh_start,1,
    "An RDDS Event must be open in current target environment before using this report.")
   SET dsh_start = (dsh_start+ 2)
   CALL text(dsh_start,1,"The following change log rows from source are included in the report:")
   CALL text((dsh_start+ 1),5,"- rows for this target environment")
   CALL text((dsh_start+ 2),5,"- rows with context name selected for merge")
   SET dsh_partial_str = "- rows with LOG_TYPEs of:"
   FOR (dsh_stop_pos = 1 TO dsh_rec->reporting_cnt)
     IF (dsh_stop_pos=1)
      SET dsh_partial_str = concat(dsh_partial_str," '",dsh_rec->reporting_qual[dsh_stop_pos].
       log_type,"'")
     ELSE
      SET dsh_partial_str = concat(dsh_partial_str,", '",dsh_rec->reporting_qual[dsh_stop_pos].
       log_type,"'")
     ENDIF
   ENDFOR
   CALL text((dsh_start+ 3),5,dsh_partial_str)
   SET dsh_start = (dsh_start+ 5)
   CALL text(dsh_start,1,concat(
     "Mover Process Issues indicates that a mover has not picked up a new batch of rows for over an hour,",
     " which might indicate looping."))
   CALL text((dsh_start+ 1),1,concat(
     "Change Log issues indicate that 1 or more rows were found in an end-state that requires investigation, ",
     "per the RDDS User guide."))
   SET dsh_start = (dsh_start+ 3)
   CALL text(dsh_start,1,concat(
     "The (S)how All option will show counts of all LOG_TYPEs and contexts in the source ",
     "change log for the current target. This can "))
   CALL text((dsh_start+ 1),1,"take a while to run.")
   SET dsh_start = (dsh_start+ 3)
   CALL text(dsh_start,1,"Press Enter to (E)xit __")
   CALL accept(dsh_start,23,"X;CUS","E"
    WHERE curaccept IN ("E"))
 END ;Subroutine
 SUBROUTINE dclm_move_rs(dmr_rs,dmr_all,dmr_type)
   DECLARE dmr_loop = i4 WITH protect, noconstant(0)
   SET dmr_all->cur_id = dmr_rs->cur_id
   SET dmr_all->cur_name = dmr_rs->cur_name
   SET dmr_all->src_id = dmr_rs->src_id
   SET dmr_all->src_name = dmr_rs->src_name
   SET dmr_all->oe_name = dmr_rs->oe_name
   SET dmr_all->db_link = dmr_rs->db_link
   SET dmr_all->cur_rs_pos = 0
   SET dmr_all->max_lines = 15
   IF (dmr_type="ISSUES")
    SET dmr_all->ctp_str = dmr_rs->ctp_str
    SET dmr_all->context_cnt = dmr_rs->context_cnt
    SET stat = alterlist(dmr_all->context_qual,dmr_all->context_cnt)
    FOR (dmr_loop = 1 TO dmr_all->context_cnt)
      SET dmr_all->context_qual[dmr_loop].values = dmr_rs->context_qual[dmr_loop].values
    ENDFOR
   ENDIF
   IF (dmr_type="ALL")
    SET dmr_all->no_qual_msg = ""
    SET dmr_all->audit_cnt = dmr_rs->audit_cnt
    SET stat = alterlist(dmr_all->audit_qual,dmr_all->audit_cnt)
    FOR (dmr_loop = 1 TO dmr_all->audit_cnt)
      SET dmr_all->audit_qual[dmr_loop].log_type = dmr_rs->audit_qual[dmr_loop].log_type
    ENDFOR
   ENDIF
   RETURN(null)
 END ;Subroutine
