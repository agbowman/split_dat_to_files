CREATE PROGRAM bhs_l_med_student
 PROMPT
 ...
 SELECT INTO  $OUTDEV
  ...
  WHERE p.position_cd=227477522
   ...
   AND p.updt_dt_tm BETWEEN cnvtdatetime(cnvtdate( $STDT),0) AND cnvtdatetime(cnvtdate( $ENDDT),2400)
  ORDER BY p.username
  HEAD PAGE
   col 40, "Position Code = ", p.position_cd"###########",
   " - ", p_position_disp, row + 2,
   col 1, "  ln", col 10,
   "Act", col 18, "Stat",
   col 30, "LogIn", col 78,
   " Begin", col 90, "  End",
