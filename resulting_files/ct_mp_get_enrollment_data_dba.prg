CREATE PROGRAM ct_mp_get_enrollment_data:dba
 PROMPT
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "ct_mp_get_enrollment_data"
 ...
 IF (((( $PROTMASTERID=0)) OR (( $PERSONID=0))) )
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_general_info","Invalid  Request",PUBLIC::
   "reply")
  GO TO exit_script
 ENDIF
 ...
 SUBROUTINE getpersoninfo(null)
   ...
   SELECT INTO "NL:"
    ...
    ORDER BY cnvtupper(p.name_last), cnvtupper(p.name_first)
    HEAD p.person_id
