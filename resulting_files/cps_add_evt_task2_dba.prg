CREATE PROGRAM cps_add_evt_task2:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 UPDATE  FROM task_activity ta
  SET ta.person_id = request->person_id, ta.order_id = request->order_id, ta.encntr_id = request->
   encntr_id,
   ta.reference_task_id = request->reference_task_id, ta.task_type_cd = request->task_type_cd, ta
   .task_status_cd = request->task_status_cd,
   ta.updt_dt_tm = cnvtdatetime(curdate,curtime3), ta.updt_id = reqinfo->updt_id, ta.updt_task =
   reqinfo->updt_task,
   ta.updt_cnt = 0, ta.updt_applctx = reqinfo->updt_applctx, ta.event_id = request->event_id,
   ta.event_class_cd = request->event_class_cd, ta.task_activity_cd = request->task_activity_cd, ta
   .msg_subject = request->msg_subject,
   ta.task_create_dt_tm = cnvtdatetime(curdate,curtime3), ta.catalog_cd = 0, ta.active_ind = 1,
   ta.active_status_cd = reqdata->active_status_cd, ta.active_status_dt_tm = cnvtdatetime(curdate,
    curtime3), ta.active_status_prsnl_id = reqinfo->updt_id,
   ta.task_class_cd = 0, ta.msg_text_id = msg_text_id, ta.msg_sender_id = request->request_prsnl_id,
   ta.location_cd = 0, ta.catalog_type_cd = 0, ta.careset_id = 0
 ...
 UPDATE  FROM task_activity_assignment taa
  SET taa.assign_prsnl_id = request->assign_prsnl_id, taa.active_ind = 1, taa.beg_eff_dt_tm =
  ...
  WHERE (taa.task_activity_assign_id=request->task_activity_assign_id)
 ;end update
 IF (curqual < 1)
  SET ierrcode = error(serrmsg,1)
  IF (ierrcode > 0)
