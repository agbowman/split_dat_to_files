CREATE PROGRAM dm_refresh_afd_cva:dba
 SELECT INTO dm_refresh_afd_cva
  d.*
  FROM dual d
  DETAIL
 ...
 FOR (cnt = 1 TO list->count)
  ...
  SELECT INTO dm_refresh_afd_cva
   ...
   DETAIL
    ...
    "1 cki = vc", row + 1, "1 active_ind = i2",
    row + 1, "1 alias_type_meaning = vc", row + 1,
    "1 contributor_source_disp = vc", row + 1, ")",
    row + 1, "go", row + 1,
    ...
    row + 1, tempstr = "set dmrequest->alias ", tempstr,
    row + 1, tempstr = build('"',replace(dcv.alias,'"',"'",0),'" go'), tempstr,
    row + 1, tempstr = "set dmrequest->alias_type_meaning = ", tempstr,
    row + 1, tempstr = build('"',replace(dcv.alias_type_meaning,'"',"'",0),'" go'), tempstr,
    row + 1, tempstr = "set dmrequest->contributor_source_disp = ", tempstr,
    row + 1, tempstr = build('"',replace(dca.display,'"',"'",0),'" go'), tempstr,
    ...
    "set reqinfo->updt_applctx = 111 go", row + 1, "execute dm_code_value_alias go",
    row + 1, row + 1
   WITH nocounter, maxrow = 1, maxcol = 512,
