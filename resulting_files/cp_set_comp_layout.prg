CREATE PROGRAM cp_set_comp_layout
 PROMPT
 ...
 CALL getcomponentlayout(null)
 CALL updatecomponents(null)
 SUBROUTINE getcomponentlayout(null)
   CALL log_message("In getComponentLayout()",log_level_debug)
   ...
   CALL log_message(build("Exit getComponentLayout(),Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE updatecomponents(null)
   DECLARE len = i4 WITH noconstant(0), protect
   SET len = size(comp_layout->components,5)
   IF (len > 0)
    UPDATE  FROM (dummyt d  WITH seq = value(len)),
      cp_component cc
     SET cc.component_seq_txt = comp_layout->components[d.seq].component_seq_txt, cc.updt_cnt = (cc
      .updt_cnt+ 1), cc.updt_dt_tm = cnvtdatetime(sysdate),
      cc.updt_applctx = reqinfo->updt_applctx, cc.updt_task = reqinfo->updt_task
     PLAN (d)
      JOIN (cc
      WHERE (cc.cp_component_id=comp_layout->components[d.seq].cp_component_id))
     WITH nocounter
    ;end update
    COMMIT
    SET comp_reply->status_data.status = "S"
   ELSE
    SET comp_reply->status_data.status = "F"
   ENDIF
 END ;Subroutine
#exit_script
 CALL putjsonrecordtofile(comp_reply)
