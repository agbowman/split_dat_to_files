CREATE PROGRAM bhs_gvw_sn_specimen:dba
 FREE RECORD m_surg
 RECORD m_surg(
   1 slst[*]
     2 c_specimens_ordered = c3
     2 c_specimen_sent = c3
     2 c_culture_ordered = c3
     2 s_specimen_sent_to = vc
     2 s_specimen_type = vc
 ) WITH protect
 ...
 DECLARE mf_culture_ordered_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "SNCULTURESORDERED"))
 DECLARE mf_specimen_sent_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "SNSPECIMENSENT"))
 DECLARE mf_specimen_sent_to_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "SNSPECIMENSENTTO"))
 DECLARE mf_specimens_ordered_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "SNSPECIMENSORDERED"))
 DECLARE mf_specimen_type_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "SNSPECIMENTYPE"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (sc
   ...
   WHERE ifr.input_form_cd=sh.input_form_cd
    ...
    AND ifr.display IN ("Cultures and Specimens")
   ...
   WHERE ce2.parent_event_id=ce1.event_id
    ...
    AND ce2.event_cd IN (mf_culture_ordered_cd, mf_specimen_sent_cd, mf_specimen_sent_to_cd,
   mf_specimens_ordered_cd, mf_specimen_type_cd)
  ...
  ORDER BY ifr.display, ce2.collating_seq, ce2.event_cd,
   ce2.event_end_dt_tm DESC
  HEAD REPORT
   ml_sknt = 0
  HEAD ifr.display
   null
  HEAD ce2.collating_seq
   IF (ifr.display="Cultures and Specimens")
    ml_sknt += 1, s0 = alterlist(m_surg->slst,ml_sknt), m_surg->slst[ml_sknt].c_specimen_sent = "No"
   ENDIF
  HEAD ce2.event_cd
   CASE (ce2.event_cd)
    OF mf_culture_ordered_cd:
     m_surg->slst[ml_sknt].c_culture_ordered = trim(ce2.result_val)
    OF mf_specimen_sent_cd:
     m_surg->slst[ml_sknt].c_specimen_sent = trim(ce2.result_val)
    OF mf_specimen_sent_to_cd:
     m_surg->slst[ml_sknt].s_specimen_sent_to = trim(ce2.result_val)
    OF mf_specimens_ordered_cd:
     m_surg->slst[ml_sknt].c_specimens_ordered = trim(ce2.result_val)
    OF mf_specimen_type_cd:
     m_surg->slst[ml_sknt].s_specimen_type = replace(ce2.result_val," # ",", ")
   ENDCASE
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM dummyt d1
  HEAD REPORT
   ms_text_temp = ms_rhead, ms_text_temp = concat(ms_text_temp,ms_rh2b,"{Cultures and Specimens}",
    ms_reol)
   IF (size(m_surg->slst,5) > 0)
    FOR (sx = 1 TO size(m_surg->slst,5))
     ms_text_temp = concat(ms_text_temp,ms_wr,"{Cultures Order: ",m_surg->slst[sx].c_culture_ordered,
      "}",
      ms_rtab,"{Specimens Order: ",m_surg->slst[sx].c_specimens_ordered,"}",ms_rtab,
      "{Sent to: ",m_surg->slst[sx].s_specimen_sent_to,"}",ms_reol),ms_text_temp = concat(
      ms_text_temp,ms_wr,ms_hi,"{Specimens: ",m_surg->slst[sx].s_specimen_type,
      "}",ms_reol)
    ENDFOR
   ENDIF
