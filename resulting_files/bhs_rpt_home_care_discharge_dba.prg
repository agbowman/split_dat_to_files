CREATE PROGRAM bhs_rpt_home_care_discharge:dba
 PROMPT
  ...
  *
  ...
  PLAN (e
   ...
   WHERE ph.parent_entity_id=e.person_id
    ...
    AND outerjoin(1)=ph.phone_type_seq
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 CALL echo("Get Blood Pressure/Venipuncture and Wearing BP/Venipuncture Restriction Band")
 SELECT INTO "nl:"
  FROM clinical_event ce
  PLAN (ce
   WHERE (ce.person_id=info->person_id)
    ...
    AND ce.result_status_cd=auth)
  ORDER BY ce.event_cd, ce.updt_dt_tm DESC
  HEAD ce.event_cd
   IF (ce.event_cd=mf_bloodpressurevenipuncture)
    info->ms_bloodpressurevenipuncture = ce.result_val
   ELSEIF (ce.event_cd=mf_wearing_bpv_band)
    info->ms_mf_wearing_bpv_band = ce.result_val
   ENDIF
  WITH nocounter
 ;end select
 IF ((info->ms_bloodpressurevenipuncture=null))
  SET info->ms_bloodpressurevenipuncture = "No result found"
 ENDIF
 ...
 DECLARE wearing_bpv_band(ncalc=i2) = f8 WITH protect
 DECLARE wearing_bpv_bandabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE bloodpressurevenipuncture(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE bloodpressurevenipunctureabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref))
  = f8 WITH protect
 ...
 DECLARE _rembloodpressureveni = i4 WITH noconstant(1), protect
 DECLARE _bcontbloodpressurevenipuncture = i2 WITH noconstant(0), protect
 ...
 SUBROUTINE allergytitlesecabs(ncalc,offsetx,offsety)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE wearing_bpv_band(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = wearing_bpv_bandabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE wearing_bpv_bandabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.270000), private
   DECLARE __ms0mf_wearing_bpv_band = vc WITH noconstant(build2(info->ms_mf_wearing_bpv_band,char(0))
    ), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    ...
    SET rptsd->m_height = 0.260
    SET _dummyfont = uar_rptsetfont(_hreport,_times140)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__ms0mf_wearing_bpv_band)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE bloodpressurevenipuncture(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = bloodpressurevenipunctureabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE bloodpressurevenipunctureabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.650000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_bloodpressureveni = f8 WITH noconstant(0.0), private
   DECLARE __bloodpressureveni = vc WITH noconstant(build2(info->ms_bloodpressurevenipuncture,char(0)
     )), protect
   IF (bcontinue=0)
    SET _rembloodpressureveni = 1
   ENDIF
   SET rptsd->m_flags = 5
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   SET _oldfont = uar_rptsetfont(_hreport,_times140)
   SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
   SET _holdrembloodpressureveni = _rembloodpressureveni
   IF (_rembloodpressureveni > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_rembloodpressureveni,((
       size(__bloodpressureveni) - _rembloodpressureveni)+ 1),__bloodpressureveni)))
    SET drawheight_bloodpressureveni = rptsd->m_height
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _rembloodpressureveni = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_rembloodpressureveni,((size(
        __bloodpressureveni) - _rembloodpressureveni)+ 1),__bloodpressureveni)))))
     SET _rembloodpressureveni = (_rembloodpressureveni+ rptsd->m_drawlength)
    ELSE
     SET _rembloodpressureveni = 0
    ENDIF
    SET growsum = (growsum+ _rembloodpressureveni)
   ENDIF
   SET rptsd->m_flags = 4
   ...
   SET rptsd->m_x = (offsetx+ 0.313)
   SET rptsd->m_width = 7.125
   SET rptsd->m_height = drawheight_bloodpressureveni
   SET _dummyfont = uar_rptsetfont(_hreport,_times140)
   IF (ncalc=rpt_render
    AND _holdrembloodpressureveni > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdrembloodpressureveni,
       ((size(__bloodpressureveni) - _holdrembloodpressureveni)+ 1),__bloodpressureveni)))
   ELSE
    SET _rembloodpressureveni = _holdrembloodpressureveni
   ENDIF
   IF (ncalc=rpt_render)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
 ...
 IF (size(info->immun,5) > 0)
  ...
  ENDFOR
 ENDIF
 SET mf_remain_space = (10.0 - _yoffset)
 IF ((((_yoffset+ bloodpressurevenipuncture(rpt_calcheight,mf_remain_space,becont))+ linesec(
  rpt_calcheight)) > 10.0))
  SET d0 = pagebreak(0)
  SET d0 = bloodpressurevenipuncture(rpt_render,mf_remain_space,becont)
  SET continued = ""
  SET mf_remain_space = (page_size - _yoffset)
 ENDIF
 WHILE (becont=1)
   SET d0 = pagebreak(0)
   SET d0 = linesec(rpt_render)
 ...
 SET d0 = bloodpressurevenipuncture(rpt_render,mf_remain_space,becont)
 SET mf_remain_space = (10.0 - _yoffset)
 IF ((((_yoffset+ wearing_bpv_band(rpt_calcheight))+ linesec(rpt_calcheight)) > 10.0))
  SET d0 = pagebreak(0)
  SET d0 = wearing_bpv_band(rpt_render,mf_remain_space,becont)
  SET continued = ""
  SET mf_remain_space = (page_size - _yoffset)
 ENDIF
 SET d0 = linesec(rpt_render)
