CREATE PROGRAM bed_rec_dup_check_pharm_ords:dba
 ...
 SELECT INTO "nl:"
  FROM br_rec b,
   br_long_text bl
  PLAN (b
   WHERE b.rec_mean="ORDDUPCHKPHARMORDERS2")
   JOIN (bl
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ap
   ...
   WHERE nvp.parent_entity_name="APP_PREFS"
    ...
    AND nvp.pvc_name="MULDUPCHECKPREF"
 ...
 IF (detail_mode=1)
  IF (((powerchart_found=0) OR (powerchart_pref_row_exists=0)) )
   ...
   SET tempreply->rowlist[row_tot_cnt].celllist[2].string_value = "MULDUPCHECKPREF"
  ...
  HEAD ap.application_number
   ...
   IF (priv_exists_ind=1
    ...
    IF (nvp.name_value_prefs_id > 0
     ...
     row_tot_cnt].celllist[2].string_value = "MULDUPCHECKPREF", tempreply->rowlist[row_tot_cnt].
