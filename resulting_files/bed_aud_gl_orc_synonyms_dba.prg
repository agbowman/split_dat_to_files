 ...
 SELECT INTO "nl:"
  ...
  HEAD ocs.synonym_id
   syncnt = (syncnt+ 1), temp->oqual[ocnt].syncnt = syncnt, stat = alterlist(temp->oqual[ocnt].
    synqual,syncnt),
   temp->oqual[ocnt].synqual[syncnt].synonym = ocs.mnemonic, temp->oqual[ocnt].synqual[syncnt].format
    = oef.oe_format_name, temp->oqual[ocnt].synqual[syncnt].synonym_type = cv3.display
   IF (ocs.hide_flag=1)
