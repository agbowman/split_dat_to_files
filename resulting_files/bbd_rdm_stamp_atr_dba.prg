CREATE PROGRAM bbd_rdm_stamp_atr:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bbd_rdm_stamp_atr"
 CALL logscriptstart(sreadme_name)
 CALL logstatus(build("Readme Failed:  Starting script ",sreadme_name,"."),"F")
 DECLARE err_ind = i4
 SET err_ind = 0
 CALL logmsg("Stamping application BBDOutcomesReasons - 225039",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225039)
 CALL logmsg("Stamping application BBDDonationProcedures - 225040",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225040)
 CALL logmsg("Stamping application BBDBagTypes - 225046",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225046)
 CALL logmsg("Stamping application BBDDonationResults - 225043",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225043)
 CALL logmsg("Stamping application BBDRegistration - 225054",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225054)
 CALL logmsg("Stamping application BBDRecruitingRslts - 225075",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225075)
 CALL logmsg("Stamping application BBDMiscContact - 225077",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225077)
 SELECT INTO "nl:"
  ap.*
  FROM application ap
  WHERE ap.application_number IN (225039, 225040, 225046, 225043, 225054,
  225075, 225077)
   AND ap.active_ind=1
  WITH nocounter
 ;end select
 IF (curqual > 0)
  CALL logstatus(build(sreadme_name," Failed! Application numbers not inactived."),"F")
  SET err_ind = 1
 ENDIF
 CALL logmsg("Stamping application BBDDbPrefWizard - 225057",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225057)
 CALL logmsg("Stamping application BBReportMaintenance - 225060",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225060)
 CALL logmsg("Stamping application BBDDonationsPending - 225066",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225066)
 SELECT INTO "nl:"
  ap.*
  FROM application ap
  WHERE ap.application_number IN (225057, 225060, 225066)
   AND ap.direct_access_ind=1
  WITH nocounter
 ;end select
 IF (curqual > 0)
  CALL logstatus(build(sreadme_name," Failed! direct_access_ind's not set to 0."),"F")
  SET err_ind = 1
 ENDIF
 CALL logmsg("Stamping application BBDMiscContact - 225077",(e_dispmsg+ e_logtofile))
 CALL stamp_app(225077)
 SELECT INTO "nl:"
  ap.*
  FROM application ap
  WHERE ap.application_number=225077
   AND trim(ap.object_name)="BBDMiscContact.Appication"
  WITH nocounter
 ;end select
 IF (curqual > 0)
  CALL logstatus(build(sreadme_name," Failed! object_name of Application 225077 not updated."),"F")
  SET err_ind = 1
 ENDIF
 IF (err_ind=0)
