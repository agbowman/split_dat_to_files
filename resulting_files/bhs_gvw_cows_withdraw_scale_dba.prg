CREATE PROGRAM bhs_gvw_cows_withdraw_scale:dba
 IF (validate(reply)=0)
  RECORD reply(
    1 status_data[1]
      2 status = c4
 ...
 IF (validate(request)=0)
  ...
  SET request->visit[1].encntr_id = 102894298.0
 ...
 DECLARE mf_op_with_score_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "OPIATEWITHDRAWALSCORE"))
 DECLARE mf_med_intervent_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "MEDICATIONINTERVATIONS"))
 DECLARE mf_encntr_id = f8 WITH protect, constant(request->visit[1].encntr_id)
 DECLARE mf_auth_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",8,"AUTH"))
 DECLARE mf_alt_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",8,"ALTERED"))
 DECLARE mf_mod_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",8,"MODIFIED"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (e
   ...
   WHERE ce.encntr_id=e.encntr_id
    ...
    AND ce.event_cd IN (mf_op_with_score_cd, mf_med_intervent_cd)
  ...
  HEAD REPORT
   p_pos = 0, p_ndx = 0
  DETAIL
   p_pos = locateval(p_ndx,1,size(m_rec->list,5),ce.parent_event_id,m_rec->list[p_ndx].
    f_parent_event_id)
   IF (p_pos=0)
    m_rec->cnt = (m_rec->cnt+ 1), stat = alterlist(m_rec->list,m_rec->cnt), p_pos = m_rec->cnt,
    m_rec->list[p_pos].f_parent_event_id = ce.parent_event_id
   ENDIF
   IF (ce.event_cd=mf_op_with_score_cd)
    m_rec->list[p_pos].s_score_dt_tm = trim(format(ce.event_end_dt_tm,"dd-mmm-yyyy hh:mm;;d")), m_rec
    ->list[p_pos].s_score_result = trim(ce.result_val)
   ELSEIF (ce.event_cd=mf_med_intervent_cd)
    m_rec->list[p_pos].s_meds_intervent_result = trim(ce.result_val)
   ENDIF
  WITH nocounter
 ;end select
 SET reply->text = concat(ms_rhead,ms_rh2bu,"Opiate Withdrawal Score",ms_rtab,
  "Medication Intervention",
  ms_rtab,"Date/Time",ms_reol)
 FOR (ml_cnt = 1 TO size(m_rec->list,5))
   SET reply->text = concat(reply->text,ms_rh2r,ms_rtab,m_rec->list[ml_cnt].s_score_result,ms_rtab,
    ms_rtab,ms_rtab,m_rec->list[ml_cnt].s_meds_intervent_result,ms_rtab,ms_rtab,
