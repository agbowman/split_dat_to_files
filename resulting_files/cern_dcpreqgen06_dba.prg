CREATE PROGRAM cern_dcpreqgen06:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   tz_index = datetimezonebyname(trim(t.time_zone)), orders->dob = concat(format(datetimezone(p
      .birth_dt_tm,p.birth_tz),"@SHORTDATE;;Q")," ",datetimezonebyindex(p.birth_tz,offset,daylight,7,
     p.birth_dt_tm)), orders->admit_dt = concat(format(datetimezone(e.reg_dt_tm,tz_index),
     "@SHORTDATE;;Q")," ",datetimezonebyindex(tz_index,offset,daylight,7,e.reg_dt_tm)),
   orders->dischg_dt = concat(format(datetimezone(e.disch_dt_tm,tz_index),"@SHORTDATETIME;;Q")," ",
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (c.event_cd=height_cd)
    ...
    orders->height_dt_tm = concat(format(datetimezone(c.updt_dt_tm,c.updt_tz),"@SHORTDATETIME;;Q"),
     " ",datetimezonebyindex(c.performed_tz,offset,daylight,7,c.updt_dt_tm))
   ELSEIF (c.event_cd=weight_cd)
    orders->weight = concat(trim(c.event_tag)," ",trim(uar_get_code_display(c.result_units_cd))),
    orders->weight_dt_tm = concat(format(datetimezone(c.updt_dt_tm,c.updt_tz),"@SHORTDATETIME;;Q"),
     " ",datetimezonebyindex(c.performed_tz,offset,daylight,7,c._updt_dt_tm))
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   orders->qual[ord_cnt].mnemonic = o.hna_order_mnemonic, orders->qual[ord_cnt].order_dt = concat(
    format(datetimezone(oa.order_dt_tm,oa.order_tz),"@SHORTDATETIME;;Q")," ",datetimezonebyindex(oa
     ...
     datetimezone(o.orig_order_dt_tm,o.orig_order_tz),"@SHORTDATETIME;;Q")," ",datetimezonebyindex(o
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   HEAD PAGE
    row 0, col 5, "{font/13}",
    row + 3, col 60, "{cpi/8}{B}Hospital Name",
    row + 3, col 55, "{cpi/10}Order Requisition",
    row + 1, col 5, "{cpi/12}",
    xrow = 90, xcol = 44, row + 1,
    col 1,
    CALL print(calcpos(xcol,xrow)), "{box/85/1}",
    xrow = (xrow+ 10), xcol = 50,
    CALL print(calcpos(xcol,xrow)),
    "Patient Information", xrow = (xrow+ 25), xcol = 50,
    row + 1, col 1,
    CALL print(calcpos(xcol,xrow)),
    "Patient:", xcol = 125,
    CALL print(calcpos(xcol,xrow)),
    orders->name, xcol = 380,
    CALL print(calcpos(xcol,xrow)),
    "MRN#:", xcol = 425,
    CALL print(calcpos(xcol,xrow)),
    orders->mrn, xrow = (xrow+ 14), xcol = 50,
    row + 1, col 1,
    CALL print(calcpos(xcol,xrow)),
    "Location:", xcol = 125,
    CALL print(calcpos(xcol,xrow)),
    orders->location, xcol = 380,
    CALL print(calcpos(xcol,xrow)),
    "Fin#:", xcol = 425,
    CALL print(calcpos(xcol,xrow)),
    orders->fnbr, xrow = (xrow+ 14), xcol = 50,
    row + 1, col 1,
    CALL print(calcpos(xcol,xrow)),
    "Attending MD:", xcol = 125,
    CALL print(calcpos(xcol,xrow)),
    orders->attending, xrow = (xrow+ 20), xcol = 50,
    row + 1, col 1,
    CALL print(calcpos(xcol,xrow)),
    "LOS:", xcol = 80,
    CALL print(calcpos(xcol,xrow)),
    orders->los, " Days", xcol = 220,
    CALL print(calcpos(xcol,xrow)), "Admit Date:", xcol = 280,
    CALL print(calcpos(xcol,xrow)), orders->admit_dt, xcol = 380,
    CALL print(calcpos(xcol,xrow)), "Patient Type:", xcol = 450,
    CALL print(calcpos(xcol,xrow)), orders->pat_type, xrow = (xrow+ 20),
    xcol = 50, row + 1, col 1,
    CALL print(calcpos(xcol,xrow)), "Age:", xcol = 80,
    CALL print(calcpos(xcol,xrow)), orders->age, xcol = 220,
    CALL print(calcpos(xcol,xrow)), "DOB:", xcol = 250,
    CALL print(calcpos(xcol,xrow)), orders->dob, xcol = 380,
    CALL print(calcpos(xcol,xrow)), "Height:", xcol = 425,
    CALL print(calcpos(xcol,xrow)), orders->height, xrow = (xrow+ 14),
    xcol = 50, row + 1, col 1,
    CALL print(calcpos(xcol,xrow)), "Sex:", xcol = 100,
    CALL print(calcpos(xcol,xrow)), orders->sex, xcol = 380,
    CALL print(calcpos(xcol,xrow)), "Weight:", xcol = 425,
    CALL print(calcpos(xcol,xrow)), orders->weight, xrow = (xrow+ 20),
    xcol = 50, row + 1, col 1,
   ...
   DETAIL
    FOR (vv = 1 TO value(order_cnt))
      ...
      CALL print(calcpos(xcol,xrow)), orders->qual[vv].accession, xcol = 50,
      xrow = (xrow+ 17), row + 1
      FOR (fsub = 1 TO 31)
        FOR (ww = 1 TO orders->qual[vv].d_cnt)
          IF ((((orders->qual[vv].d_qual[ww].group_seq=fsub)) OR (fsub=31
           ...
           CALL print(calcpos(xcol,xrow)), orders->qual[vv].d_qual[ww].label_text,
           ":", xcol = (xcol+ 280),
           CALL print(calcpos(xcol,xrow))
           IF (size(orders->qual[vv].d_qual[ww].value) < 45)
            CALL print(orders->qual[vv].d_qual[ww].value)
           ELSE
            pos = 1, eol = 0
            WHILE ((pos < (size(orders->qual[vv].d_qual[ww].value)+ 1))
             AND substring((pos+ 1),(size(orders->qual[vv].d_qual[ww].value) - pos),orders->qual[vv].
             d_qual[ww].value) > spaces)
              bw = pos, ew = findstring(" ",trim(orders->qual[vv].d_qual[ww].value),(bw+ 1))
              IF (ew=0)
               ew = (size(orders->qual[vv].d_qual[ww].value)+ 1)
              ENDIF
              len = (ew - bw), z = movestring(orders->qual[vv].d_qual[ww].value,bw,temp,1,len), temp
               = trim(temp)
              IF (((eol+ len) < 45))
               temp, eol = (eol+ len)
              ELSE
               row + 1, eol = 0, xrow = (xrow+ 12),
               CALL print(calcpos(xcol,xrow)), temp, eol = len
              ENDIF
      ...
      CALL print(calcpos(xcol,xrow)), "Ordering MD:", xcol = 130,
      ...
      xcol = 50, row + 1,
      CALL print(calcpos(xcol,xrow)),
      "Ordering Type:", xcol = 130,
      CALL print(calcpos(xcol,xrow)),
      orders->qual[vv].type, xrow = (xrow+ 12), xcol = 50,
      row + 1,
      CALL print(calcpos(xcol,xrow)), "Ordering entered by:",
      xcol = 160,
      ...
      CALL print(calcpos(xcol,xrow)), "Ordering Comments", xrow = (xrow+ 10)
      ...
      ENDIF
    ENDFOR
    "{font/8}{cpi/12}{pos/50/720}", "dcpreqgen06"
