CREATE PROGRAM dm2_rdds_write_rel_val_rows:dba
 ...
 SUBROUTINE dm2_rdds_init_display(null)
   ...
   CALL clear(1,1)
 END ;Subroutine
 DECLARE rel_val_source_env_id = f8
 DECLARE rel_val_target_env_id = f8
 DECLARE rel_val_source_dblink = vc
 DECLARE rel_val_src_table_name = vc
 DECLARE rel_val_unique_string = vc
 DECLARE rel_val_info_domain = vc
 SET rel_val_source_env_id =  $1
 SET rel_val_target_env_id =  $2
 SET rel_val_source_dblink =  $3
 SET rel_val_src_table_name = ""
 SET rel_val_unique_string = ""
 SET rel_val_info_domain = ""
 IF (check_logfile("dm2_rdds_write_rvr",".log","DM2_RDDS_WRITE_REL_VAL_ROWS LOG")=0)
  GO TO exit_rel_val
 ENDIF
 IF (((trim(rel_val_source_dblink)="") OR (((rel_val_source_env_id=0) OR (rel_val_source_env_id=0))
 )) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "INVALID DATABASE LINK, OR SOURCE_ID, OR TARGET_ID FOR THE GIVEN SOURCE/TARGET RELATIONSHIP"
  ...
  GO TO exit_rel_val
 ENDIF
 IF (get_unique_file("RELTN","VAL")=1)
  SET rel_val_unique_string = cnvtupper(dm_err->unique_fname)
 ELSE
  GO TO exit_rel_val
 ENDIF
 SET rel_val_src_table_name = concat("DM_INFO",trim(rel_val_source_dblink))
 SET dm_err->eproc = "STARTING PROCESS FOR WRITING RELATIONSHIP VALIDATION ROWS"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Set up local relationship validation"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET rel_val_info_domain = dm2_write_info_domain("MERGE",rel_val_source_env_id,rel_val_target_env_id,
  " ")
 DELETE  FROM dm_info d
  WHERE d.info_name="RELATIONSHIP VALIDATION"
   AND d.info_domain=rel_val_info_domain
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_rel_val
 ...
 INSERT  FROM dm_info d
  SET d.info_domain = rel_val_info_domain, d.info_name = "RELATIONSHIP VALIDATION", d.info_char =
   rel_val_unique_string
 ;end insert
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_rel_val
 ENDIF
 SET dm_err->eproc = "Set up relationship validation in source"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET rel_val_info_domain = dm2_write_info_domain("MERGE",rel_val_target_env_id,rel_val_source_env_id,
  " ")
 DELETE  FROM (parser(rel_val_src_table_name) d)
  WHERE d.info_name="RELATIONSHIP VALIDATION"
   AND d.info_domain=rel_val_info_domain
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_rel_val
 ENDIF
 INSERT  FROM (parser(rel_val_src_table_name) d)
  SET d.info_domain = rel_val_info_domain, d.info_name = "RELATIONSHIP VALIDATION", d.info_char =
   rel_val_unique_string
 ;end insert
 IF (check_error(dm_err->eproc)=1)
...
#exit_rel_val
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Exiting Process for writing Relationship Validation Rows: Process Successful"
 ENDIF
 CALL final_disp_msg("dm2_rdds_write_rvr")
 IF ((dm_err->err_ind=1))
  ROLLBACK
 ENDIF
END GO
