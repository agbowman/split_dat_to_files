CREATE PROGRAM dm2_tsprec_load_child:dba
 ...
 SET dm_err->eproc = "Loading DM_TS_PRECEDENCE data to RS."
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dtl_cnt = (dtl_cnt+ 1)
   IF (mod(dtl_cnt,100)=1)
    stat = alterlist(ts_prec->qual,(dtl_cnt+ 99))
   ENDIF
   ts_prec->qual[dtl_cnt].table_name = requestin->list_0[d.seq].table_name, ts_prec->qual[dtl_cnt].
   precedence = cnvtint(requestin->list_0[d.seq].precedence), ts_prec->qual[dtl_cnt].data_tablespace
    = requestin->list_0[d.seq].data_tablespace,
   ts_prec->qual[dtl_cnt].data_extent_size = cnvtreal(requestin->list_0[d.seq].data_extent_size),
   ts_prec->qual[dtl_cnt].index_tablespace = requestin->list_0[d.seq].index_tablespace, ts_prec->
   qual[dtl_cnt].index_extent_size = cnvtreal(requestin->list_0[d.seq].index_extent_size),
   ts_prec->qual[dtl_cnt].long_tablespace = requestin->list_0[d.seq].long_tablespace, ts_prec->qual[
   dtl_cnt].long_extent_size = cnvtreal(requestin->list_0[d.seq].long_extent_size), ts_prec->qual[
   dtl_cnt].updt_cnt = cnvtint(requestin->list_0[d.seq].updt_cnt)
  FOOT REPORT
   stat = alterlist(ts_prec->qual,dtl_cnt), ts_prec->cnt = dtl_cnt
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF ((dm_err->debug_flag > 10))
  ...
  CALL echorecord(ts_prec)
 ENDIF
 EXECUTE dm2_chk_ts_prec 0
 IF ((dm_err->err_ind=1))
  SET readme_data->status = "F"
 ...
 SET readme_data->message = "DM2_TSPREC_LOAD completed successfully."
#exit_program
 ...
 FREE RECORD ts_prec
 FREE RECORD ts_prec_maint
 CALL final_disp_msg("dm2_tsprec_load")
END GO
