CREATE PROGRAM dm2_aload_code_value_alias
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_cva",".log","dm2_aload_code_value_alias")=0)
  GO TO exit_program
 ENDIF
 DECLARE dacva_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dacva_file = vc WITH protect, noconstant( $2)
 DECLARE dacva_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dacva_batch = i4 WITH protect, noconstant(5000)
 DECLARE dacva_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE dacva_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dacva_pkg,dacva_file_loc_ret,dacva_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dacva_batch=0)
  SET dacva_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_AFD_CODE_VALUE_ALIAS for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND dacva_rows_returned=10000)
   DELETE  FROM dm_afd_code_value_alias d
    WHERE d.alpha_feature_nbr=dacva_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET dacva_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",dacva_rows_returned))
   ENDIF
 ENDWHILE
 IF (dacva_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dacva_file_loc_ret,dacva_file), "dm2_aload_cs_cva", dacva_batch
  CALL echo(concat(dacva_file_loc_ret,dacva_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_code_value_alias completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_code_value_alias")
END GO
