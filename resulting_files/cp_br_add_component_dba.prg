CREATE PROGRAM cp_br_add_component:dba
 PROMPT
 ...
 DECLARE addbedrockcomponent(null) = null WITH protect
 DECLARE callbedrockwizard(null) = null WITH protect
 ...
 IF ((request->bedrock_wizard_ind=1))
  CALL insertrows(null)
 ELSE
  CALL addbedrockcomponent(null)
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE complistsize = i4 WITH noconstant(size(request->component_list,5)), protect
   ...
   SELECT INTO "NL:"
    FROM cp_node cn
    WHERE (cn.cp_node_id=request->cp_node_id)
    WITH nocounter
   ;end select
   IF (curqual=0)
    CALL reporterrorandexit("PERFORM","Validate","cp_node_id is invalid.")
   ELSE
    SET reply->cp_node_id = request->cp_node_id
   ENDIF
   FOR (x = 1 TO complistsize)
     CALL verifynonzerocodevalue(request->component_list[x].comp_type_cd,comp_type_cd_codeset,
      "Component Type")
   ENDFOR
   CALL log_message(build("Exit verifyInputs(), Elapsed time in seconds:",((curtime3 - begin_curtime3
     )/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE addbedrockcomponent(null)
   CALL log_message("In addBedrockComponent()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE i = i4 WITH protect, noconstant(0)
   DECLARE csize = i4 WITH protect, noconstant(size(request->component_list,5))
   SET stat = alterlist(br_maintain_req->new_components,csize)
   SET br_maintain_req->cp_node_id = request->cp_node_id
   SET br_maintain_req->new_components_cnt = csize
   FOR (i = 0 TO csize)
     SET br_maintain_req->new_components[i].comp_type_cd = request->component_list[i].comp_type_cd
   ENDFOR
   CALL log_message(build("Exit addBedrockComponent(), Elapsed time in seconds:",((curtime3 -
 ...
 SUBROUTINE callbedrockwizard(null)
   ...
   IF (validate(debug_ind,0)=1)
    CALL echo(build("Reply in callBedrockWizard of cp_br_add_component script"))
    CALL echorecord(br_maintain_rep)
   ENDIF
   IF ((br_maintain_rep->status_data.status="S"))
    SET reply->status_data.status = "S"
    SET reply->br_wizard_name = br_maintain_rep->category_display
    CALL insertrows(null)
   ELSE
    SET reply->status_data.status = br_maintain_rep->status_data.status
    SET stat = moverec(br_maintain_rep->status_data,reply->status_data)
    GO TO exit_script
   ENDIF
 ...
 SUBROUTINE insertrows(null)
   ...
   ENDIF
   SET stat = alterlist(reply->component_list,size(request->component_list,5))
   FOR (x = 1 TO size(request->component_list,5))
     SET cp_component_id = 0.0
     SELECT INTO "nl:"
      nextseqnum = seq(reference_seq,nextval)
      FROM dual
      DETAIL
       cp_component_id = nextseqnum
      WITH nocounter
     ;end select
     CALL echo(build2("Next CP_COMPONENT_ID:",cp_component_id))
     IF (cp_component_id=0.0)
      CALL reporterror("PERFORM","INSERT"," Unable to generate CP_COMPONENT_ID from REFERENCE_SEQ.")
      ROLLBACK
      GO TO exit_script
     ENDIF
     INSERT  FROM cp_component cc
      SET cc.cp_node_id = request->cp_node_id, cc.comp_type_cd = request->component_list[x].
       comp_type_cd, cc.component_seq_txt = request->component_list[x].component_seq_txt,
       cc.cp_component_id = cp_component_id, cc.updt_dt_tm = cnvtdatetime(sysdate), cc.updt_id =
       reqinfo->updt_id,
       cc.updt_task = reqinfo->updt_task, cc.updt_cnt = 0, cc.updt_applctx = reqinfo->updt_applctx
      WITH nocounter
     ;end insert
     IF (curqual=0)
      CALL reporterror("PERFORM","INSERT","Unable to insert row into CP_COMPONENT table.")
      ROLLBACK
      GO TO exit_script
     ELSE
      SET reply->component_list[x].comp_type_cd = request->component_list[x].comp_type_cd
      SET reply->component_list[x].cp_component_id = cp_component_id
      SET reply->component_list[x].component_seq_txt = request->component_list[x].component_seq_txt
     ENDIF
   ENDFOR
