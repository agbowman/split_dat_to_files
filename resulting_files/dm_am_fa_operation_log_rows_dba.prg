CREATE PROGRAM dm_am_fa_operation_log_rows:dba
 ...
 DECLARE operidentcnt = i4 WITH noconstant(0)
 DECLARE prevoperationcnt = i4 WITH noconstant(0)
 DECLARE tempidx = i4 WITH noconstant(0)
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 SET reply->table_name = "AM_FA_OPERATION_LOG"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(afol.am_fa_operation_log_id)
   FROM am_fa_operation_log afol
   WHERE afol.am_fa_operation_log_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(afol.am_fa_operation_log_id)
  FROM am_fa_operation_log afol
  WHERE afol.am_fa_operation_log_id > 0.0
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    rowid_list = listagg(afol.rowid,";") WITHIN GROUP(
    ORDER BY afol.operation_ident, afol.am_fa_operation_log_id DESC), afol.operation_ident,
    operation_count = count(afol.operation_ident)
    FROM am_fa_operation_log afol
    PLAN (afol
     WHERE afol.am_fa_operation_log_id BETWEEN curminid AND curmaxid
      AND afol.am_fa_operation_log_id != 0
      AND parser(sbr_getrowidnotexists("afol.operation_dt_tm <= cnvtdatetime(purge_dt_tm)","afol")))
    GROUP BY afol.operation_ident
    HAVING (count(afol.operation_ident)=
     (SELECT
      count(afol2.operation_ident)
      FROM am_fa_operation_log afol2
      WHERE afol2.operation_ident=afol.operation_ident))
    DETAIL
     prevoperationcnt = operation_count
     IF (((rowsleft - operation_count) >= 0))
      stat = initrec(temp_rows), operidentcnt = arraysplit(temp_rows->rows[tempidx].row_id,tempidx,
       rowid_list,";",1)
      FOR (loopcnt = 1 TO size(temp_rows->rows,5))
        row_cnt += 1
        IF (mod(row_cnt,100)=1)
         stat = alterlist(reply->rows,(row_cnt+ 99))
        ENDIF
        reply->rows[row_cnt].row_id = temp_rows->rows[loopcnt].row_id, rowsleft -= 1
      ENDFOR
     ENDIF
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF (rowsleft != 0
    AND ((rowsleft - prevoperationcnt) < 0)
    AND batch_num > batch_ndx)
    SET rowsleft = 0
   ENDIF
