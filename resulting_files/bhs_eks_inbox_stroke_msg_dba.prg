CREATE PROGRAM bhs_eks_inbox_stroke_msg:dba
 DECLARE mf_ordermd = f8 WITH protect
 DECLARE order_action_type = f8 WITH constant(uar_get_code_by("MEANING",6003,"ORDER")), protect
 DECLARE subject = vc
 SET personid = trigger_personid
 SET encntrid = trigger_encntrid
 SET orderid = trigger_orderid
 SET pid = 0
 SET retval = 100
 IF (validate(eksdata)=0)
  RECORD eksdata(
    1 tqual[4]
      2 temptype = c10
      2 qual[*]
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   mf_ordermd = oa.order_provider_id
  WITH nocounter
 ;end select
 IF (curqual > 0)
  ...
  SET inboxrequest->msg_text = trim(eksdata->bldmsg[1].text,3)
  SET inboxrequest->msg_subject_cd = 0
  SET inboxrequest->msg_subject = "MD Stroke Measure Alert"
  ...
  SET log_message = build("order id found order action - ",orderid,"message sent to ",mf_ordermd)
 ELSE
  SET retval = 0
  SET log_message = build("order id not in order action =",orderid)
 ENDIF
END GO
