CREATE PROGRAM dm_rmc_mover_3155:dba
 ...
 CALL put_value(cust_tab_name,"ACCT_BALANCE","0")
 CALL put_value(cust_tab_name,"ADJUSTMENT_BALANCE","0")
 CALL put_value(cust_tab_name,"APPLIED_PAYMENT_BALANCE","0")
 CALL put_value(cust_tab_name,"BAD_DEBT_BALANCE","0")
 CALL put_value(cust_tab_name,"CHARGE_BALANCE","0")
 CALL put_value(cust_tab_name,"HI_ACCT_BALANCE","0")
 CALL put_value(cust_tab_name,"PENDING_ACCT_BAL","0")
 CALL put_value(cust_tab_name,"UNAPPLIED_PAYMENT_BALANCE","0")
 SET translate_check = is_translated(cust_tab_name,"ACCT_ID")
 IF (translate_check=0)
  SET cust_nomv_chk = select_merge_translate(cnvtstring(cust_acct_id),cust_tab_name)
  ...
  IF (type_trans_check=1)
   ...
   IF (((tgt_be_id="NOMV21") OR (tgt_at_id="NOMV21")) )
    SET insert_update_reason = "The primary key value has been deleted from target."
    SET no_insert_update = 1
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV21"
    SET drdm_mini_loop_status = "NOMV21"
    GO TO exit_3155
   ELSEIF ((( NOT (isnumeric(tgt_be_id))) OR ( NOT (isnumeric(tgt_at_id)))) )
    SET dm2_ref_data_reply->error_ind = 1
   ...
   SELECT INTO "NL:"
    ...
    DETAIL
     ...
     ENDIF
     cust_acct->qual[ui_cnt].acct_id = a.acct_id, stat = alterlist(ui_query_eval_rec->qual,ui_cnt),
     ui_query_eval_rec->qual[ui_cnt].root_entity_attr = a.at_acct_reltn_id
    FOOT REPORT
   ...
   IF (error(dm2_ref_data_reply->error_msg,0) > 0)
    ...
    GO TO exit_3155
   ENDIF
   SET evaluate_id = evaluate_exec_ui_query(ui_cnt,temp_tbl_cnt,perm_col_cnt)
   IF (evaluate_id < 0)
    IF ((evaluate_id=- (3)))
    ...
    ELSEIF ((evaluate_id=- (2)))
     ...
     CALL merge_audit("FAILREASON","There were multiples returned from the UI",1)
     IF (drdm_error_out_ind=1)
      ROLLBACK
      GO TO exit_3155
     ENDIF
    ...
    ENDIF
   ELSE
    CALL put_value(cust_tab_name,"ACCT_ID",cnvtstring(evaluate_id))
   ENDIF
