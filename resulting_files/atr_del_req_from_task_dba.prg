CREATE PROGRAM atr_del_req_from_task:dba
 RECORD internal(
   1 qual[*]
     2 updt_flag = i2
 )
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
       3 operationstatus = c1
       3 targetobjectname = c25
       3 targetobjectvalue = vc
 )
 SET reply->status_data.status = "F"
 SET number_to_delete = size(request->qual,5)
 SET number_total = size(request->qual,5)
 SET number_to_update = 0
 SET i = size(reply->status_data.subeventstatus,5)
 SUBROUTINE result_status(opname,opstat,targetname,targetvalue)
   SET stat = alter(reply->status_data.subeventstatus[i],i)
   SET reply->status_data.subeventstatus[i].operationname = opname
   SET reply->status_data.subeventstatus[i].operationstatus = opstat
   SET reply->status_data.subeventstatus[i].targetobjectname = targetname
   SET reply->status_data.subeventstatus[i].targetobjectvalue = targetvalue
   SET i += 1
 END ;Subroutine
 IF ((request->feature_number=0))
  DELETE  FROM task_request_r r,
    (dummyt d  WITH seq = value(number_to_delete))
   SET r.seq = 1
   PLAN (d)
    JOIN (r
    WHERE (r.task_number=request->task_number)
     AND (r.request_number=request->qual[d.seq].request_number))
   WITH nocounter
  ;end delete
  IF (curqual=number_to_delete)
   SET reply->status_data.status = "S"
   SET reqinfo->commit_ind = 1
  ELSE
   SET reqinfo->commit_ind = 0
  ENDIF
 ELSE
  SET stat = alterlist(internal->qual,number_total)
  SELECT INTO "nl:"
   r.task_number
   FROM dm_task_request_r r,
    (dummyt d  WITH seq = value(number_total))
   PLAN (d)
    JOIN (r
    WHERE (r.task_number=request->task_number)
     AND (r.feature_number=request->feature_number)
     AND (r.request_number=request->qual[d.seq].request_number))
   HEAD REPORT
    number_to_update = 0
   DETAIL
    number_to_update += 1, internal->qual[d.seq].updt_flag = 1
   WITH nocounter, forupdate(r)
  ;end select
  IF (number_to_update > 0)
   UPDATE  FROM dm_task_request_r r,
     (dummyt d  WITH seq = value(number_total))
    SET r.seq = 1, r.deleted_ind = 1, r.updt_dt_tm = cnvtdatetime(sysdate),
     r.updt_task = reqinfo->updt_task, r.updt_id = reqinfo->updt_id, r.updt_cnt = (r.updt_cnt+ 1),
     r.updt_applctx = reqinfo->updt_applctx, r.schema_date = cnvtdatetimeutc(request->schema_date,2)
    PLAN (d
     WHERE (internal->qual[d.seq].updt_flag=1))
     JOIN (r
     WHERE (r.task_number=request->task_number)
      AND (r.feature_number=request->feature_number)
      AND (r.request_number=request->qual[d.seq].request_number))
    WITH nocounter
   ;end update
   IF (curqual=0)
    CALL result_status("Update","F","dm_task_request_r","Update failed.")
    SET reqinfo->commit_ind = 0
    GO TO exit_script
   ENDIF
  ENDIF
  IF (number_to_update != number_total)
   INSERT  FROM dm_task_request_r r,
     (dummyt d  WITH seq = value(number_total))
    SET r.task_number = request->task_number, r.request_number = request->qual[d.seq].request_number,
     r.deleted_ind = 1,
     r.updt_dt_tm = cnvtdatetime(sysdate), r.updt_task = reqinfo->updt_task, r.updt_id = reqinfo->
     updt_id,
     r.updt_cnt = 0, r.updt_applctx = reqinfo->updt_applctx, r.feature_number = request->
     feature_number,
     r.schema_date = cnvtdatetimeutc(request->schema_date,2)
    PLAN (d
     WHERE (internal->qual[d.seq].updt_flag != 1))
     JOIN (r)
    WITH nocounter
   ;end insert
   IF (curqual=0)
    CALL result_status("Insert","F","dm_task_request_r","Insert failed.")
    SET reqinfo->commit_ind = 0
    GO TO exit_script
   ENDIF
  ENDIF
 ENDIF
 SET reply->status_data.status = "S"
 SET reqinfo->commit_ind = 1
#exit_script
END GO
