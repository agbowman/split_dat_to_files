CREATE PROGRAM dm2_import_ccl_grants
 ...
 END ;Subroutine
 DECLARE dicg_logfile_prefix = vc WITH protect, constant("dm2_imp_ccl_grants")
 DECLARE dicg_node = vc WITH protect, noconstant("")
 DECLARE dicg_num = i4 WITH protect, noconstant(0)
 DECLARE dicg_idx = i4 WITH protect, noconstant(0)
 DECLARE dicg_pos = i4 WITH protect, noconstant(0)
 DECLARE dicg_ccl_ver = i4 WITH protect, constant((((cnvtint(currev) * 10000)+ (cnvtint(currevminor)
   * 100))+ cnvtint(currevminor2)))
 DECLARE dicg_continue = i2 WITH protect, noconstant(1)
 DECLARE dicg_directory = vc WITH protect, noconstant("")
 DECLARE dicg_file_name = vc WITH protect, noconstant("")
 DECLARE dicg_file = vc WITH protect, noconstant("")
 DECLARE dicg_str_pos = i4 WITH protect, noconstant(0)
 DECLARE dicg_str_pos2 = i4 WITH protect, noconstant(0)
 DECLARE dicg_str = vc WITH protect, noconstant("")
 DECLARE dicg_dpl_id = f8 WITH protect, noconstant(0.0)
 DECLARE dicg_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dicg_err_msg = vc WITH protect, noconstant("")
 FREE RECORD dicg_data
 RECORD dicg_data(
   1 cnt = i4
   1 qual[*]
     2 script_name = vc
 ...
 IF (check_logfile(dicg_logfile_prefix,".log","dm2_import_ccl_grants")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_import_ccl_grants"
 CALL disp_msg("",dm_err->logfile,0)
 SET dicg_node = cnvtupper(trim(curnode))
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Node: ",cnvtupper(dicg_node)))
 ENDIF
 ...
 IF ((dm2_rdbms_version->level1 < 11))
  SET dm_err->eproc = "Bypassing dm2_import_ccl_grants on oracle version < 11"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF (dicg_ccl_ver < 80601)
  SET dm_err->eproc = "Bypassing dm2_import_ccl_grants on CCL version < 8.7.1"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF (cursys="AXP")
  SET dm_err->eproc = "Bypassing dm2_import_ccl_grants on VMS Operating System"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 ...
 IF (dm2_process_log_row(dpl_ccl_grant,dpl_execution,dpl_no_prev_id,1)=0)
  GO TO exit_script
 ENDIF
 SET dicg_dpl_id = dm2_process_event_rs->dm_process_event_id
 IF (validate(dicg_import_fname,"DM2NOTSET")="DM2NOTSET")
  WHILE (dicg_continue=1)
    SET message = window
    CALL clear(1,1)
    CALL box(1,1,3,132)
    CALL box(1,1,24,132)
    CALL text(2,2,"IMPORT CCL GRANTS [FILE LOCATION]")
    CALL text(2,70,"DATE/TIME: ")
    CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
    CALL text(5,5,"Please provide the location from where the CCL grants will be imported.")
    CALL text(7,5,"Use an absolute directory path (i.e. without OS logicals)")
    CALL text(9,5,"Directory:")
    CALL text(10,5,"File Name:")
    CALL accept(9,17,"P(80);C",dicg_directory
     WHERE curaccept > " ")
    SET dicg_directory = curaccept
    CALL accept(10,17,"P(30);C",dicg_file_name
     WHERE curaccept > " ")
    SET dicg_file_name = curaccept
    CALL text(12,5,"**Note 1: Please ensure that this script is run on ALL app nodes")
    CALL text(13,5,
     "**Note 2: Millennium Servers associated with any of the CCL scripts in the import file ")
    CALL text(14,5,"          should be cycled to ensure the grant will take effect")
    ...
    CASE (curaccept)
     OF "Q":
      ...
      GO TO exit_script
     OF "C":
      IF (dm2_find_dir(dicg_directory)=0)
      ...
      ELSE
       SET dicg_continue = 0
       SET dicg_file = build(dicg_directory,"/",dicg_file_name)
       SET message = nowindow
      ENDIF
     OF "M":
      SET dicg_continue = 1
  ...
  ENDWHILE
 ELSE
  SET dicg_file = trim(dicg_import_fname)
 ENDIF
 SET dm_err->eproc = concat("Verifying import file exists: ",dicg_file)
 IF ( NOT (dm2_findfile(dicg_file) > 0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Unable to find CCL grants import file: ",dicg_file)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 CALL dm2_process_log_add_detail_text("IMPORT CCL GRANTS: FILE NAME",dicg_file)
 CALL dm2_process_log_dtl_row(dicg_dpl_id,0)
 SET dm_err->eproc = concat("Load ",dicg_file," into  dicg_data record structure.")
 CALL disp_msg("",dm_err->logfile,0)
 FREE DEFINE rtl3
 SET logical file_loc dicg_file
 DEFINE rtl3 "file_loc"
 SELECT INTO "nl:"
  FROM rtl3t t
  WHERE t.line > " "
  DETAIL
   dicg_str_pos = 0, dicg_str_pos = findstring("::",trim(t.line),1,0)
   IF (dicg_str_pos > 0)
    dicg_data->cnt = (dicg_data->cnt+ 1), stat = alterlist(dicg_data->qual,dicg_data->cnt), dicg_str
     = substring(1,(dicg_str_pos - 1),trim(t.line)),
    dicg_str_pos2 = findstring(":",trim(dicg_str),1,0)
    IF (dicg_str_pos2 > 0)
     dicg_data->qual[dicg_data->cnt].script_name = substring(1,(dicg_str_pos2 - 1),trim(dicg_str)),
     dicg_data->qual[dicg_data->cnt].group = evaluate(substring((dicg_str_pos2+ 1),size(trim(dicg_str
         )),trim(dicg_str)),"DBA",0,1)
    ENDIF
    dicg_data->qual[dicg_data->cnt].grant_cmd_full = trim(substring((dicg_str_pos+ 2),size(trim(t
        .line)),trim(t.line)))
   ENDIF
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dicg_data)
 ENDIF
 SET dm_err->eproc = "Applying grants to CCL dictionary"
 CALL disp_msg("",dm_err->logfile,0)
 FOR (dicg_num = 1 TO dicg_data->cnt)
   SET dm_err->eproc = "Verifying script existence in dprotect."
   SELECT INTO "nl:"
    FROM dprotect d
    WHERE d.object="P"
     AND (d.group=dicg_data->qual[dicg_num].group)
     AND (d.object_name=dicg_data->qual[dicg_num].script_name)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   IF (curqual > 0)
    IF ((dm_err->debug_flag > 0))
     CALL echo(trim(dicg_data->qual[dicg_num].grant_cmd_full))
    ENDIF
    SET dicg_data->qual[dicg_num].script_exists = 1
    CALL parser(trim(dicg_data->qual[dicg_num].grant_cmd_full))
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
    CALL dm2_process_log_add_detail_text("SCRIPT NAME::GRANT CMD",build(dicg_data->qual[dicg_num].
      script_name,":",dicg_data->qual[dicg_num].group,"::",dicg_data->qual[dicg_num].grant_cmd_full))
   ENDIF
 ENDFOR
 CALL dm2_process_log_dtl_row(dicg_dpl_id,0)
 ...
 IF ((dm_err->debug_flag=722))
  CALL echorecord(dpc_data)
 ENDIF
 SET dm_err->eproc = "Verifying grants have been applied successfully"
 CALL disp_msg("",dm_err->logfile,0)
 FOR (dicg_num = 1 TO dicg_data->cnt)
   IF ((dicg_data->qual[dicg_num].script_exists=1))
    SET dicg_pos = locateval(dicg_idx,1,dpc_data->grant_cnt,dicg_data->qual[dicg_num].script_name,
     dpc_data->grants[dicg_idx].script_name,
     dicg_data->qual[dicg_num].group,dpc_data->grants[dicg_idx].group)
    IF (((dicg_pos=0) OR ((dpc_data->grants[dicg_idx].rule_grant_ind != 1))) )
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Error verifying grants applied for: ",dicg_data->qual[dicg_num].
      script_name,":",cnvtstring(dicg_data->qual[dicg_num].group))
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
   ENDIF
...
#exit_script
 ...
 SET dicg_err_ind = dm_err->err_ind
 SET dicg_err_msg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_ccl_grant,dpl_execution,dicg_dpl_id,1)
 SET dm2_process_rs->process_name = ""
 SET dm_err->err_ind = dicg_err_ind
 SET dm_err->emsg = dicg_err_msg
 SET dm_err->eproc = "Ending dm2_import_ccl_grants"
 CALL final_disp_msg(dicg_logfile_prefix)
END GO
