CREATE PROGRAM dm2_ref_data_mover_est:dba
 ...
 SUBROUTINE get_err_val(null)
  ...
  RETURN(sbr_err_val)
 END ;Subroutine
 DECLARE rel_type_con = c15 WITH constant("REFERENCE MERGE")
 DECLARE estimate = i4 WITH constant(15380)
 DECLARE env = f8
 DECLARE mover_est = i4
 DECLARE drdm_srch_str = vc
 DECLARE valid_input_ind = i2
 DECLARE str = vc
 DECLARE drdm_count = i4
 DECLARE id = f8
 DECLARE src_name = vc
 DECLARE cnt = i4
 IF (check_logfile("mover estimator",".log","DM2_REF_DATA_MOVER LOGFILE")=0)
  SET dm_err->err_ind = 1
 ...
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="MERGE ESTIMATE"
  DETAIL
   mover_est = d.info_number
  WITH nocounter
 ;end select
 IF (curqual=0)
  INSERT  FROM dm_info d
   SET d.info_domain = "DATA MANAGEMENT", d.info_name = "MERGE ESTIMATE", d.info_number = estimate
   WITH nocounter
  ;end insert
  COMMIT
  SET mover_est = estimate
 ENDIF
 SET stat = alterlist(rec->num,100)
 SET valid_input_ind = 0
...
#redo
 ...
 CALL clear(1,1)
 CALL box(1,1,23,132)
 CALL text(3,57,"SOURCE ENVIRONMENTS")
 CALL line(5,1,131,horizontal)
 CALL text(7,10,"Please select source environment: ")
 CALL text(22,2,"For <HELP> press 'Shift + F5'")
 SET id_cnt = 0
 SET help = pos(10,10,10,100)
 SET help =
 SELECT INTO "NL:"
 ...
 SELECT INTO "NL:"
  ...
  PLAN (di
   ...
   JOIN (de
   WHERE de.environment_id=der.parent_env_id
    AND de.environment_id=id)
  DETAIL
   valid_input_ind = 1
  WITH nocounter
 ;end select
 IF (valid_input_ind=0)
 ...
 SELECT INTO "NL:"
  FROM dm_env_reltn dr,
   ...
   dm_environment dme
  PLAN (dif
   WHERE dif.info_domain="DATA MANAGEMENT"
    AND dif.info_name="DM_ENV_ID")
   JOIN (dr
   WHERE dr.child_env_id=dif.info_number
    AND dr.relationship_type="REFERENCE MERGE")
   JOIN (dme
   WHERE dme.environment_id=dr.parent_env_id
    AND dme.environment_id=id)
  DETAIL
   dm2_ref_data_doc->pre_link_name = dr.pre_link_name, dm2_ref_data_doc->post_link_name = dr
   .post_link_name, dm2_ref_data_doc->env_source_id = dr.parent_env_id,
   dm2_ref_data_doc->env_target_id = dr.child_env_id, src_name = dme.environment_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO done
 ENDIF
 SET drdm_srch_str = concat(value(dm2_ref_data_doc->pre_link_name),"dm_chg_log",dm2_ref_data_doc->
  post_link_name)
 CALL echo("*")
 CALL echo("*")
 CALL echo("*")
 CALL echo("********************************************************************************")
 CALL echo("*")
 CALL echo("Calculating Estimate . . .")
 CALL echo("*")
 CALL echo("********************************************************************************")
 CALL echo("*")
 CALL echo("*")
 ...
 SELECT INTO "NL:"
  thi = count(*)
  FROM (parser(drdm_srch_str) d)
  WHERE (target_env_id=dm2_ref_data_doc->env_target_id)
   AND log_type="REFCHG"
  DETAIL
   drdm_count = thi
  WITH nocounter
 ;end select
 SET cnt = drdm_count
 SET drdm_count = (drdm_count * 1.3)
 IF (drdm_count > 0)
  SET drdm_count = (drdm_count/ mover_est)
  SET message = window
  CALL video(n)
  CALL clear(1,1)
  CALL box(1,1,23,132)
  SET str = concat("For source environment ",src_name,", the mover should finish processing ",trim(
    cnvtstring(cnt))," log rows in approximately ",
   trim(cnvtstring(drdm_count))," hours.")
  CALL text(3,10,str)
  CALL text(5,10,
   "This estimate assumes one instance of dm2_ref_data_mover running. Running multiple instances of dm2_ref_data_mover"
   )
  CALL text(6,10,
   "will reduce the time required for this process, but may impact overall performance in the domain."
   )
  CALL line(8,1,131,"HORIZONTAL")
  CALL text(10,10,"Would you like to run the estimator on another source environment? (Y/N)")
  CALL accept(10,82,"P;CU","N"
 ...
 ELSE
  ...
  CALL clear(1,1)
  CALL box(1,1,23,132)
  CALL text(3,10,"There are no mergeable rows for that source environment.")
  CALL line(5,1,131,"HORIZONTAL")
  CALL text(7,10,"Would you like to run the estimator on another source environment? (Y/N)")
  CALL accept(7,82,"P;CU","N"
   WHERE curaccept IN ("Y", "N"))
  IF (curaccept="Y")
   GO TO redo
  ENDIF
 ENDIF
#exit_program
 SET dm_err->eproc = "EXITING THE DM2_REF_DATA_MOVER_EST"
 CALL echo("*")
 CALL echo("*")
 CALL echo("*")
 CALL echo("********************************************************************************")
 CALL echo("*")
 CALL echo("Exiting the DM2_REF_DATA_MOVER_EST")
 CALL echo("*")
 CALL echo(concat("Log file is ccluserdir:",dm_err->logfile))
 CALL echo("*")
 CALL echo("********************************************************************************")
 CALL echo("*")
 CALL echo("*")
