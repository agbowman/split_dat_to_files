CREATE PROGRAM dm_ccb_csf_displayfilter:dba
 ...
 SELECT
  ...
  ELSE
   PLAN (cv
    WHERE parser(filterstring_builder(dccd_filterstring,"cv",request))
     AND cv.code_set > 0)
    JOIN (cvs
    WHERE cvs.code_set=cv.code_set)
  ENDIF
  ...
  cvs.display, cnt = count(cv.display_key)
  FROM code_value cv,
   code_value_set cvs
  ...
  WITH nocounter, orahintcbo("INDEX( cv XIE3CODE_VALUE)")
 ;end select
 SET errcode = error(errmsg,1)
 IF (errcode != 0)
  SET errmsg = concat("Unable to read from CODE_VALUE or CODE_VALUE_SET:",errmsg)
  SET status = "F"
  GO TO exit_program
 ENDIF
#exit_program
