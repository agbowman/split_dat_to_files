CREATE PROGRAM ams_ld_import_users
 PROMPT
  "Enter Groups to Load" = "",
  "Enter Position Display to Use" = "",
  "Enter the Import file to load" = "",
  "Enter the last name addition" = "",
  "Enter the title to add" = "",
  "Enter the client suffix" = ""
  WITH group, pos, file,
  suffix, title, client
 EXECUTE cclseclogin
 FREE RECORD request
 ...
 SET file_name = concat("ccluserdir:", $FILE)
 SET ntitle =  $TITLE
 SET nsuffix = concat(" (", $SUFFIX,")")
 SET nclient = concat("#", $CLIENT)
  ...
  IF ((import->qual[a].action_ind=1))
   ...
   IF ((import->qual[a].person_id=0))
    IF ((import->qual[a].load_ind=1))
     ...
     SET request->person_list[cnt].name_title = ntitle
   ...
   ELSE
    UPDATE  FROM person_name p
     SET p.name_title = ntitle, p.updt_id = 4801, p.updt_dt_tm = cnvtdatetime(curdate,curtime3)
