CREATE PROGRAM dm2_aload_sch_tab
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_sch_tab",".log","dm2_aload_sch_tab")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_sch_tab"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dast_cnt = i4 WITH protect, noconstant(0)
 DECLARE dast_str = vc WITH protect, noconstant("")
 DECLARE dast_req_size = i4 WITH protect, noconstant(0)
 DECLARE dast_ndx = i4 WITH protect, noconstant(0)
 DECLARE dast_req_del_size = i4 WITH protect, noconstant(0)
 DECLARE dast_maint_del_size = i4 WITH protect, noconstant(0)
 FREE RECORD dast_maint
 RECORD dast_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET dast_maint->num_of_inserts = 0
 SET dast_maint->num_of_updates = 0
 SET dast_maint->num_of_none = 0
 SET dast_maint->num_of_deletes = 0
 SET dast_req_size = size(requestin->list_0,5)
 SET stat = alterlist(dast_maint->qual,dast_req_size)
 IF (dac_pop_coldic_rec("DM_AFD_TABLES")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining maintenance type for DM_AFD_TABLES."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dast_req_size))
  DETAIL
   dast_maint->qual[d.seq].maint_type = "I", dast_maint->num_of_inserts = (dast_maint->num_of_inserts
   + 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 SELECT
  IF ((requestin->list_0[1].row_number="1")
   AND cnvtint(requestin->list_0[dast_req_size].row_number)=daat_expected_rows)INTO "nl:"
   FROM dm_afd_tables dat
   WHERE dat.alpha_feature_nbr=cnvtint(requestin->list_0[1].alpha_feature_nbr)
    AND dat.owner=currdbuser
  ELSEIF ((requestin->list_0[1].row_number="1"))INTO "nl:"
   FROM dm_afd_tables dat
   WHERE dat.alpha_feature_nbr=cnvtint(requestin->list_0[1].alpha_feature_nbr)
    AND dat.owner=currdbuser
    AND (dat.table_name <= requestin->list_0[dast_req_size].table_name)
  ELSEIF (cnvtint(requestin->list_0[dast_req_size].row_number)=daat_expected_rows)INTO "nl:"
   FROM dm_afd_tables dat
   WHERE dat.alpha_feature_nbr=cnvtint(requestin->list_0[1].alpha_feature_nbr)
    AND dat.owner=currdbuser
    AND (dat.table_name >= requestin->list_0[1].table_name)
  ELSE INTO "nl:"
   FROM dm_afd_tables dat
   WHERE dat.alpha_feature_nbr=cnvtint(requestin->list_0[1].alpha_feature_nbr)
    AND dat.owner=currdbuser
  ...
  DETAIL
   dast_ndx = locateval(dast_ndx,1,dast_req_size,dat.table_name,requestin->list_0[dast_ndx].
    table_name)
   IF (dast_ndx > 0)
    IF (cnvtint(requestin->list_0[dast_ndx].schema_instance) > dat.schema_instance)
     dast_maint->qual[dast_ndx].maint_type = "U", dast_maint->num_of_updates = (dast_maint->
     num_of_updates+ 1), dast_maint->num_of_inserts = (dast_maint->num_of_inserts - 1)
    ELSEIF (cnvtint(requestin->list_0[dast_ndx].schema_instance)=dat.schema_instance
     AND cnvtdatetime(requestin->list_0[dast_ndx].updt_dt_tm) != cnvtdatetime(dat.updt_dt_tm))
     dast_maint->qual[dast_ndx].maint_type = "U", dast_maint->num_of_updates = (dast_maint->
     num_of_updates+ 1), dast_maint->num_of_inserts = (dast_maint->num_of_inserts - 1)
    ELSEIF (cnvtint(requestin->list_0[dast_ndx].schema_instance)=dat.schema_instance
     AND cnvtdatetime(requestin->list_0[dast_ndx].updt_dt_tm)=cnvtdatetime(dat.updt_dt_tm))
     dast_maint->qual[dast_ndx].maint_type = "N", dast_maint->num_of_none = (dast_maint->num_of_none
     + 1), dast_maint->num_of_inserts = (dast_maint->num_of_inserts - 1)
    ELSEIF (cnvtint(requestin->list_0[dast_ndx].schema_instance) < dat.schema_instance)
     dast_maint->qual[dast_ndx].maint_type = "N", dast_maint->num_of_none = (dast_maint->num_of_none
     + 1), dast_maint->num_of_inserts = (dast_maint->num_of_inserts - 1),
     csvcontent->prev_sch_inst_on_pkg = cnvtint(requestin->list_0[1].alpha_feature_nbr)
    ENDIF
   ELSE
    dast_maint_del_size = (size(requestin->list_0,5)+ 1), dast_req_del_size = (size(dast_maint->qual,
     5)+ 1), stat = alterlist(requestin->list_0,dast_req_del_size),
    stat = alterlist(dast_maint->qual,dast_maint_del_size), dast_maint->num_of_deletes = (dast_maint
    ->num_of_deletes+ 1), dast_maint->qual[dast_maint_del_size].maint_type = "D",
    requestin->list_0[dast_req_del_size].table_name = dat.table_name, requestin->list_0[
    dast_req_del_size].alpha_feature_nbr = trim(cnvtstring(dat.alpha_feature_nbr))
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echo(requestin->list_0[dast_req_size].row_number)
  CALL echo(daat_expected_rows)
  CALL echorecord(dast_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((dast_maint->num_of_inserts > 0))
  SET dm_err->eproc = concat("Insert into ",dac_col_list->tbl," based upon Fileversion:",requestin->
   list_0[1].fileversion)
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(dast_req_size) )"),0)
  CALL dm2_push_cmd("set",0)
  IF (dac_build_col_list(null)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd(",dcv.owner = currdbuser ",0)
  CALL dm2_push_cmd("plan d where dast_maint->qual[d.seq].maint_type = 'I' ",0)
  CALL dm2_push_cmd("join dcv  ",0)
  IF (dm2_push_cmd("with nocounter go",1)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  IF ((curqual != dast_maint->num_of_inserts))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Inserted:",curqual,". Does not match expected rows inserted:",
    dast_maint->num_of_inserts)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
  COMMIT
 ENDIF
 IF ((dast_maint->num_of_updates > 0))
  SET dm_err->eproc = concat("Update into ",dac_col_list->tbl," based upon Fileversion:",requestin->
   list_0[1].fileversion)
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  CALL dm2_push_cmd(concat("update into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(dast_req_size) )"),0)
  CALL dm2_push_cmd("set",0)
  IF (dac_build_col_list(null)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where dast_maint->qual[d.seq].maint_type = 'U' ",0)
  CALL dm2_push_cmd("join dcv where dcv.owner = currdbuser ",0)
  CALL dm2_push_cmd("and requestin->list_0[d.seq].table_name = dcv.table_name ",0)
  IF (dm2_push_cmd(
   " and cnvtint(requestin->list_0[d.seq].alpha_feature_nbr)= dcv.alpha_feature_nbr with nocounter go",
   1)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  IF ((curqual != dast_maint->num_of_updates))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Updated:",curqual,". Does not match expected rows updated:",
    dast_maint->num_of_updates)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
  COMMIT
 ENDIF
 IF ((dast_maint->num_of_deletes > 0))
  SET dm_err->eproc = concat("Delete from ",dac_col_list->tbl," based upon Fileversion:",requestin->
   list_0[1].fileversion)
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  CALL dm2_push_cmd(concat("delete from ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(size(requestin->list_0,5)) )"),0)
  CALL dm2_push_cmd("set dcv.seq = 0 ",0)
  CALL dm2_push_cmd("plan d where dast_maint->qual[d.seq].maint_type = 'D' ",0)
  CALL dm2_push_cmd("join dcv where dcv.owner = currdbuser ",0)
  CALL dm2_push_cmd("and requestin->list_0[d.seq].table_name = dcv.table_name ",0)
  IF (dm2_push_cmd(
   " and cnvtint(requestin->list_0[d.seq].alpha_feature_nbr)= dcv.alpha_feature_nbr with nocounter go",
   ...
   GO TO exit_program
  ENDIF
  IF ((curqual != dast_maint->num_of_deletes))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Deleted:",curqual,". Does not match expected rows deleted:",
    dast_maint->num_of_deletes)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_sch_tab completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_sch_tab")
END GO
