CREATE PROGRAM dm2_aload_task_req_r
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_tskreqrel",".log","dm2_aload_task_req_r")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_task_req_r"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daatrr_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daatrr_file = vc WITH protect, noconstant( $2)
 DECLARE daatrr_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE daatrr_batch = i4 WITH protect, noconstant(0)
 DECLARE daatrr_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE daatrr_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daatrr_pkg,daatrr_file_loc_ret,daatrr_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daatrr_batch=0)
  SET daatrr_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_OCD_TASK_REQ_R for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND daatrr_rows_returned=10000)
   DELETE  FROM dm_ocd_task_req_r d
    WHERE d.alpha_feature_nbr=daatrr_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET daatrr_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",daatrr_rows_returned))
   ENDIF
 ENDWHILE
 IF (daatrr_row_cnt > 0)
  EXECUTE dm2_dbimport concat(daatrr_file_loc_ret,daatrr_file), "dm2_aload_atr_taskreqr",
  daatrr_batch
  CALL echo(concat(daatrr_file_loc_ret,daatrr_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_task_req_r completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_task_req_r")
END GO
