CREATE PROGRAM cmb_vr_test_prsnl_combine:dba
 ...
 RECORD cmb_test_values(
   ...
   1 combine2_id = f8
   1 test_stage = i2
   1 person_detail_cnt = i4
   1 person_noop_cnt = i4
   1 prsnl_detail_cnt = i4
   1 prsnl_noop_cnt = i4
 )
 ...
 IF ((validate(cmb_test_from_prsnl_id,- (99.00)) != - (99.00))
  AND (validate(cmb_test_to_prsnl_id,- (99.00)) != - (99.00)))
  SET cmb_test_values->cmb_from_id = cmb_test_from_prsnl_id
  SET cmb_test_values->cmb_to_id = cmb_test_to_prsnl_id
 ELSE
  SELECT INTO "nl:"
   p1.person_id, p2.person_id
   FROM person p1,
    person p2
   WHERE p1.person_id > 1000.0
    AND p2.person_id > 1000.0
    AND p1.name_last_key=p2.name_last_key
    AND p1.person_id != p2.person_id
    AND p1.active_ind=1
    AND p1.beg_effective_dt_tm <= cnvtdatetime(sysdate)
    AND p1.end_effective_dt_tm >= cnvtdatetime(sysdate)
    AND p1.deceased_dt_tm = null
    AND p2.active_ind=1
    AND p2.beg_effective_dt_tm <= cnvtdatetime(sysdate)
    AND p2.end_effective_dt_tm >= cnvtdatetime(sysdate)
    AND p2.deceased_dt_tm = null
    AND  EXISTS (
   (SELECT
    1
    FROM prsnl pr1
    WHERE pr1.person_id=p1.person_id
     AND pr1.active_ind=1))
    AND  EXISTS (
   (SELECT
    1
    FROM prsnl pr2
    WHERE pr2.person_id=p2.person_id
     AND pr2.active_ind=1))
   DETAIL
    cmb_test_values->cmb_from_id = p1.person_id, cmb_test_values->cmb_to_id = p2.person_id
   WITH maxqual(p1,1), nocounter
  ;end select
 ENDIF
 FREE RECORD request
 RECORD request(
 ...
 SELECT INTO "nl:"
  c.combine_id
  FROM combine c
  WHERE (c.from_id=cmb_test_values->cmb_from_id)
  DETAIL
   cmb_test_values->combine2_id = c.combine_id
  WITH nocounter
 ;end select
 CALL echo("==================================================================================")
 CALL echo("Listing out the core Combine IDs...")
 CALL echo(build2("Person Combine ID: ",cmb_test_values->combine_id))
 CALL echo(build2("Prsnl Combine ID:  ",cmb_test_values->combine2_id))
 CALL echo("==================================================================================")
 IF ((cmb_test_values->combine_id=0))
  CALL echo("")
  GO TO exit_script
 ENDIF
 IF ((cmb_test_values->combine2_id=0))
  GO TO exit_script
 ENDIF
 SET cmb_test_values->person_detail_cnt = 0
 SET cmb_test_values->person_noop_cnt = 0
 SELECT INTO "nl:"
  pcd.person_combine_det_id, pcd.combine_action_cd
  FROM person_combine_det pcd
  WHERE (pcd.person_combine_id=cmb_test_values->combine_id)
  DETAIL
   IF (pcd.combine_action_cd=noop)
    cmb_test_values->person_noop_cnt += 1
   ELSE
    cmb_test_values->person_detail_cnt += 1
   ENDIF
  WITH nocounter
 ;end select
 SET cmb_test_values->prsnl_detail_cnt = 0
 SET cmb_test_values->prsnl_noop_cnt = 0
 SELECT INTO "nl:"
  cd.combine_detail_id, cd.combine_action_cd
  FROM combine_detail cd
  WHERE (cd.combine_id=cmb_test_values->combine2_id)
  DETAIL
   IF (cd.combine_action_cd=noop)
    cmb_test_values->prsnl_noop_cnt += 1
   ELSE
    cmb_test_values->prsnl_detail_cnt += 1
   ENDIF
  WITH nocounter
 ;end select
 CALL echo("==================================================================================")
 CALL echo("Listing out the combined PERSON row counts...")
 CALL echo(build2("True Rows Combined: ",cmb_test_values->person_detail_cnt))
 CALL echo(build2("NOOP Rows Combined: ",cmb_test_values->person_noop_cnt))
 CALL echo("Listing out the combined PRSNL row counts...")
 CALL echo(build2("True Rows Combined: ",cmb_test_values->prsnl_detail_cnt))
 CALL echo(build2("NOOP Rows Combined: ",cmb_test_values->prsnl_noop_cnt))
 CALL echo("==================================================================================")
 ...
 ELSE
  ...
  GO TO exit_script
 ENDIF
 DELETE  FROM person_combine_det
  WHERE (person_combine_id=cmb_test_values->combine_id)
  WITH nocounter
 ;end delete
 DELETE  FROM person_combine
  WHERE (person_combine_id=cmb_test_values->combine_id)
  WITH nocounter
 ;end delete
 DELETE  FROM combine_detail
  WHERE (combine_id=cmb_test_values->combine2_id)
  WITH nocounter
 ;end delete
 DELETE  FROM combine
  WHERE (combine_id=cmb_test_values->combine2_id)
