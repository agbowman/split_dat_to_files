CREATE PROGRAM bhs_prax_get_followups
 FREE RECORD result
 RECORD result(
   1 person_id = f8
   1 encntr_id = f8
...
#exit_script
    ...
    HEAD REPORT
     ...
     FOR (idx = 1 TO size(rep4250783->follow_up,5))
       ...
       v3, row + 1, v4 = build("<ProvName>",trim(replace(replace(replace(replace(replace(rep4250783->
          ...
          "&quot;",0),3),"</ProvName>"),
       col + 1, v4, row + 1,
       v5 = build("<FollowUpDtTm>",format(rep4250783->follow_up[idx].followup_dt_tm,
         "MM/DD/YYYY HH:MM;;D"),"</FollowUpDtTm>"), col + 1, v5,
       ...
       v9, row + 1, v10 = build("<Address>",trim(replace(replace(replace(replace(replace(rep4250783->
              follow_up[idx].add_long_text,"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),
          '"',"&quot;",0),3),"</Address>"),
       col + 1, v10, row + 1,
       ...
       v15, row + 1, v16 = build("<AddressTypeCd>",cnvtint(rep4250783->follow_up[idx].address_type_cd
         ),"</AddressTypeCd>"),
       col + 1, v16, row + 1,
       ...
       row + 1, v18 = build("<QuickPickCd>",cnvtint(rep4250783->follow_up[idx].quick_pick_cd),
        "</QuickPickCd>"), col + 1,
       v18, row + 1, v19 = build("<FollowupNeededInd>",rep4250783->follow_up[idx].followup_needed_ind,
        "</FollowupNeededInd>"),
       col + 1, v19, row + 1,
 ...
 SUBROUTINE callgetfollowuplist(null)
   ...
   SET req4250783->pat_ed_domain_cd =  $3
