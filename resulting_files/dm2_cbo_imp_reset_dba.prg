CREATE PROGRAM dm2_cbo_imp_reset:dba
 ...
 FREE RECORD dcir_scripts
 RECORD dcir_scripts(
   1 cnt = i4
   1 qual[*]
     2 name = vc
 )
 DECLARE dcir_last_run_dt_tm = dq8 WITH protect, noconstant(0.0)
 DECLARE dcir_temp_script_name = vc WITH protect, noconstant("")
 DECLARE dcir_temp_query_nbr = i4 WITH protect, noconstant(0)
 DECLARE dcir_reset_all_row_created = i4 WITH protect, constant(5)
 DECLARE dcir_reset_all_table_trunc = i4 WITH protect, constant(10)
 DECLARE dcir_main_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dcir_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dcir_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dcir_update_last_run(null) = i2
 DECLARE dcir_reset_all(null) = i2
 DECLARE dcir_reset(null) = i2
 SUBROUTINE dcir_update_last_run(null)
   SET dm_err->eproc = "Updating CBO Implementer last run date/time in dm_info."
   UPDATE  FROM dm_info di
    SET di.info_date = cnvtdatetime(curdate,curtime3)
    WHERE di.info_domain="DM_CBO_IMPLEMENTER"
     AND di.info_name="LAST_RESET_RUN_DT_TM"
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
   ...
   ENDIF
   IF ( NOT (curqual))
    SET dm_err->eproc = "Inserting CBO Implementer last run date/time into dm_info."
    INSERT  FROM dm_info di
     SET di.info_domain = "DM_CBO_IMPLEMENTER", di.info_name = "LAST_RESET_RUN_DT_TM", di.info_date
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcir_reset_all(null)
   DECLARE dcir_cmd_text = vc WITH protect, noconstant("")
   SET dm2_process_event_rs->status = dpl_executing
   CALL dm2_process_log_row(dpl_cbo,dpl_cbo_reset_all,dpl_no_prev_id,1)
   SET dcir_log_id = dm2_process_event_rs->dm_process_event_id
   SET dm_err->eproc = "Starting reset_all"
   CALL disp_msg("",dm_err->logfile,0)
   SET dm_err->eproc = "Deleting all individual resets from dm_info as part of a reset_all."
   ...
   ENDIF
   COMMIT
   SET dm_err->eproc = "Inserting individual reset_all rows into dm_info."
   INSERT  FROM dm_info di
    (di.info_domain, di.info_name, di.info_number)(SELECT DISTINCT
     dcir_info_domain = "DM_CBO_IMPLEMENTER:RESET_OBJECT", dspm.ccl_script_name, dcir_info_number =
     dcir_reset_all_row_created
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Deleting reset_all row from dm_info."
   DELETE  FROM dm_info di
    WHERE di.info_domain="DM_CBO_IMPLEMENTER"
   ...
   ENDIF
   COMMIT
   SET dcir_cmd_text = "RDB TRUNCATE TABLE dm_sql_perf_dtl GO"
   SET dm_err->eproc = dcir_cmd_text
   IF (dm2_push_cmd(dcir_cmd_text,1)=0)
    RETURN(0)
   ENDIF
   SET dcir_cmd_text = "RDB TRUNCATE TABLE dm_sql_perf_text GO"
   SET dm_err->eproc = dcir_cmd_text
   IF (dm2_push_cmd(dcir_cmd_text,1)=0)
    RETURN(0)
   ENDIF
   SET dcir_cmd_text = "RDB TRUNCATE TABLE dm_sql_perf GO"
   SET dm_err->eproc = dcir_cmd_text
   IF (dm2_push_cmd(dcir_cmd_text,1)=0)
    RETURN(0)
   ENDIF
   SET dcir_cmd_text = "RDB TRUNCATE TABLE dm_sql_perf_master GO"
   SET dm_err->eproc = dcir_cmd_text
   IF (dm2_push_cmd(dcir_cmd_text,1)=0)
    RETURN(0)
   ENDIF
   SET dm2_process_event_rs->status = dpl_success
   CALL dm2_process_log_row(dpl_cbo,dpl_cbo_reset_all,dcir_log_id,1)
   SET dm_err->eproc = "Reset_all complete"
   CALL disp_msg("",dm_err->logfile,0)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcir_reset(null)
   DECLARE dcir_iter = i4 WITH protect, noconstant(0)
   DECLARE dcir_iter2 = i4 WITH protect, noconstant(0)
   FOR (dcir_iter = 1 TO dcir_resets->cnt)
     IF (((dcir_resets->scripts[dcir_iter].all_query_ind) OR (dcir_resets->scripts[dcir_iter].
     recompile_ind)) )
      SET dm_err->eproc = concat("Resetting script ",dcir_resets->scripts[dcir_iter].name)
      CALL disp_msg("",dm_err->logfile,0)
      SET dm_err->eproc = "Deleting from dm_sql_perf_dtl for an entire script."
      DELETE  FROM dm_sql_perf_dtl dspd
       WHERE dspd.dm_sql_id IN (
       (SELECT
        dsp.dm_sql_id
        FROM dm_sql_perf dsp
        WHERE (dsp.ccl_script_name=dcir_resets->scripts[dcir_iter].name)))
       WITH nocounter
      ;end delete
      IF (check_error(dm_err->eproc)=1)
       ROLLBACK
       ...
       RETURN(0)
      ENDIF
      SET dm_err->eproc = "Deleting from dm_sql_perf_text for an entire script."
      DELETE  FROM dm_sql_perf_text dspt
       WHERE dspt.dm_sql_id IN (
       (SELECT
        dsp.dm_sql_id
        FROM dm_sql_perf dsp
        WHERE (dsp.ccl_script_name=dcir_resets->scripts[dcir_iter].name)))
      ...
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET dm_err->eproc = "Deleting from dm_sql_perf for an entire script."
      DELETE  FROM dm_sql_perf dsp
       WHERE (dsp.ccl_script_name=dcir_resets->scripts[dcir_iter].name)
       WITH nocounter
      ;end delete
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET dm_err->eproc = "Deleting query tuning data for a script from dm_sql_perf_master."
      DELETE  FROM dm_sql_perf_master dspm
       WHERE (dspm.ccl_script_name=dcir_resets->scripts[dcir_iter].name)
        AND dspm.ccl_query_nbr > evaluate(dcir_resets->scripts[dcir_iter].recompile_ind,1,- (1),0)
       WITH nocounter
      ;end delete
      IF (check_error(dm_err->eproc)=1)
       ROLLBACK
       ...
       RETURN(0)
      ENDIF
      IF ( NOT (dcir_resets->scripts[dcir_iter].recompile_ind))
       SET dm_err->eproc = "Updating script tuning data for a script in dm_sql_perf_master."
       UPDATE  FROM dm_sql_perf_master dspm
        SET dspm.tgt_opt_mode = 0, dspm.last_change_dt_tm = cnvtdatetime(curdate,curtime3)
        WHERE (dspm.ccl_script_name=dcir_resets->scripts[dcir_iter].name)
        WITH nocounter
       ;end update
       IF (check_error(dm_err->eproc)=1)
      ...
      COMMIT
      SET dm2_process_event_rs->status = dpl_success
      CALL dm2_process_log_add_detail_text(dpl_script_name,dcir_resets->scripts[dcir_iter].name)
      CALL dm2_process_log_row(dpl_cbo,evaluate(dcir_resets->scripts[dcir_iter].recompile_ind,1,
        dpl_cbo_reset_script_recompile,dpl_cbo_reset_script_manual),dpl_no_prev_id,1)
     ELSE
      FOR (dcir_iter2 = 1 TO dcir_resets->scripts[dcir_iter].query_cnt)
        SET dm_err->eproc = concat("Resetting script ",dcir_resets->scripts[dcir_iter].name," Query ",
         build(dcir_resets->scripts[dcir_iter].queries[dcir_iter2].query_nbr))
        CALL disp_msg("",dm_err->logfile,0)
        SET dm_err->eproc = "Deleting from dm_sql_perf_dtl for a single query number in a script."
        DELETE  FROM dm_sql_perf_dtl dspd
         WHERE dspd.dm_sql_id IN (
         (SELECT
          dsp.dm_sql_id
          FROM dm_sql_perf dsp
          WHERE (dsp.ccl_script_name=dcir_resets->scripts[dcir_iter].name)
           AND (dsp.ccl_query_nbr=dcir_resets->scripts[dcir_iter].queries[dcir_iter2].query_nbr)))
         WITH nocounter
        ;end delete
        IF (check_error(dm_err->eproc)=1)
         ROLLBACK
         ...
         RETURN(0)
        ENDIF
        SET dm_err->eproc = "Deleting from dm_sql_perf_text for a single query number in a script."
        DELETE  FROM dm_sql_perf_text dspt
         WHERE dspt.dm_sql_id IN (
         (SELECT
          dsp.dm_sql_id
        ...
        IF (check_error(dm_err->eproc)=1)
         ...
         RETURN(0)
        ENDIF
        SET dm_err->eproc = "Deleting from dm_sql_perf for a single query number in a script."
        DELETE  FROM dm_sql_perf dsp
         WHERE (dsp.ccl_script_name=dcir_resets->scripts[dcir_iter].name)
          AND (dsp.ccl_query_nbr=dcir_resets->scripts[dcir_iter].queries[dcir_iter2].query_nbr)
         WITH nocounter
        ...
        ENDIF
        SET dm_err->eproc =
        "Updating script tuning data for a single query in a script in dm_sql_perf_master."
        UPDATE  FROM dm_sql_perf_master dspm
         SET dspm.tgt_opt_mode = 0, dspm.last_change_dt_tm = cnvtdatetime(curdate,curtime3)
         WHERE (dspm.ccl_script_name=dcir_resets->scripts[dcir_iter].name)
          AND (dspm.ccl_query_nbr=dcir_resets->scripts[dcir_iter].queries[dcir_iter2].query_nbr)
         WITH nocounter
        ;end update
        IF (check_error(dm_err->eproc)=1)
         ROLLBACK
         ...
         RETURN(0)
        ENDIF
        SET dm_err->eproc = "Deleting reset row from dm_info."
        DELETE  FROM dm_info di
         WHERE di.info_domain="DM_CBO_IMPLEMENTER:RESET_OBJECT"
          AND di.info_name=concat(dcir_resets->scripts[dcir_iter].name,":",cnvtstring(dcir_resets->
           scripts[dcir_iter].queries[dcir_iter2].query_nbr))
         WITH nocounter
        ;end delete
        IF (check_error(dm_err->eproc)=1)
         ROLLBACK
        ...
        SET dm2_process_event_rs->status = dpl_success
        CALL dm2_process_log_add_detail_text(dpl_script_name,dcir_resets->scripts[dcir_iter].name)
        CALL dm2_process_log_add_detail_number(dpl_query_nbr,dcir_resets->scripts[dcir_iter].queries[
         dcir_iter2].query_nbr)
        CALL dm2_process_log_row(dpl_cbo,dpl_cbo_reset_query_manual,dpl_no_prev_id,1)
      ENDFOR
     ENDIF
   ENDFOR
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_cbo_imp_reset",".log","dm2_cbo_imp_reset LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_cbo_imp_reset"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_row(dpl_cbo,dpl_execution,dpl_no_prev_id,1)
 SET dcir_main_log_id = dm2_process_event_rs->dm_process_event_id
 EXECUTE dm2_script_version
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if RESET_ALL has been initiated in dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_CBO_IMPLEMENTER"
   AND di.info_name="RESET_ALL"
 ...
 ENDIF
 IF (curqual)
  IF (dcir_reset_all(null)=0)
   GO TO exit_script
  ENDIF
  IF (dcir_update_last_run(null)=0)
   GO TO exit_script
  ENDIF
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining last reset run date/time from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_CBO_IMPLEMENTER"
   AND di.info_name="LAST_RESET_RUN_DT_TM"
  HEAD REPORT
   dcir_last_run_dt_tm = cnvtdatetime(1,0)
  DETAIL
   dcir_last_run_dt_tm = di.info_date
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining list of script names the CBO IMPLEMENTER is monitoring."
 SELECT DISTINCT INTO "nl:"
  dspm.ccl_script_name
  FROM dm_sql_perf_master dspm
  ORDER BY dspm.ccl_script_name
  HEAD REPORT
   stat = initrec(dcir_scripts)
  DETAIL
   dcir_scripts->cnt = (dcir_scripts->cnt+ 1)
   IF (mod(dcir_scripts->cnt,50)=1)
    stat = alterlist(dcir_scripts->qual,(dcir_scripts->cnt+ 49))
   ENDIF
   dcir_scripts->qual[dcir_scripts->cnt].name = dspm.ccl_script_name
  FOOT REPORT
   stat = alterlist(dcir_scripts->qual,dcir_scripts->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining new/modified scripts from dm_sql_perf_script_version."
 SELECT INTO "nl:"
  FROM dm_sql_perf_script_version dspsv
  WHERE dspsv.new_version_dt_tm > cnvtdatetime(dcir_last_run_dt_tm)
  ORDER BY dspsv.script_name
  HEAD REPORT
   locval = 0, stat = initrec(dcir_resets)
  DETAIL
   IF (locatevalsort(locval,1,dcir_scripts->cnt,dspsv.script_name,dcir_scripts->qual[locval].name) >
   0)
    dcir_resets->cnt = (dcir_resets->cnt+ 1)
    IF (mod(dcir_resets->cnt,10)=1)
     stat = alterlist(dcir_resets->scripts,(dcir_resets->cnt+ 9))
    ENDIF
    dcir_resets->scripts[dcir_resets->cnt].name = dspsv.script_name, dcir_resets->scripts[dcir_resets
    ->cnt].recompile_ind = 1
   ENDIF
  FOOT REPORT
   stat = alterlist(dcir_resets->scripts,dcir_resets->cnt)
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Querying for list of resets from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_CBO_IMPLEMENTER:RESET_OBJECT"
  ...
  HEAD REPORT
   locval = 0, found = 0
  DETAIL
   IF ( NOT (assign(found,findstring(":",di.info_name))))
    dcir_temp_script_name = di.info_name, dcir_temp_query_nbr = - (1)
   ELSE
    dcir_temp_script_name = substring(1,(found - 1),di.info_name), dcir_temp_query_nbr = cnvtint(
     substring((found+ 1),size(di.info_name),di.info_name))
   ENDIF
   IF (locatevalsort(locval,1,dcir_scripts->cnt,dcir_temp_script_name,dcir_scripts->qual[locval].name
    ) > 0)
    IF ( NOT (assign(locval,locateval(locval,1,dcir_resets->cnt,dcir_temp_script_name,dcir_resets->
      scripts[locval].name))))
     dcir_resets->cnt = (dcir_resets->cnt+ 1), stat = alterlist(dcir_resets->scripts,dcir_resets->cnt
      ), dcir_resets->scripts[dcir_resets->cnt].name = dcir_temp_script_name,
     locval = dcir_resets->cnt
    ENDIF
    IF ((dcir_temp_query_nbr=- (1)))
     dcir_resets->scripts[locval].all_query_ind = 1, dcir_resets->scripts[locval].dm_info_ind = 1
    ELSEIF ( NOT (dcir_resets->scripts[locval].all_query_ind)
     AND  NOT (dcir_resets->scripts[locval].recompile_ind))
     dcir_resets->scripts[locval].query_cnt = (dcir_resets->scripts[locval].query_cnt+ 1), stat =
     alterlist(dcir_resets->scripts[locval].queries,dcir_resets->scripts[locval].query_cnt),
     dcir_resets->scripts[locval].queries[dcir_resets->scripts[locval].query_cnt].query_nbr =
     dcir_temp_query_nbr,
     dcir_resets->scripts[locval].dm_info_ind = 1
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(dcir_resets)
 ENDIF
 IF (dcir_reset(null)=0)
  GO TO exit_script
 ENDIF
 IF (dcir_update_last_run(null)=0)
  GO TO exit_script
 ENDIF
#exit_script
 SET dcir_err_ind = dm_err->err_ind
 SET dm2_process_event_rs->status = evaluate(dcir_err_ind,1,dpl_failure,dpl_success)
 CALL dm2_process_log_row(dpl_cbo,dpl_execution,dcir_main_log_id,1)
 SET dm_err->err_ind = dcir_err_ind
 SET dm_err->eproc = "Ending dm2_cbo_imp_reset"
 CALL final_disp_msg("dm2_cbo_imp_reset")
