CREATE PROGRAM ags_prsnl_purge:dba
 PROMPT
  "Backout Type (J, I, P) =" = "J",
  "AGS_JOB_ID (0.0) = " = 0.0
  WITH cbackout_type, djob_id
 CALL echo("***")
 CALL echo("***   BEG :: AGS_PRSNL_PURGE")
 CALL echo("***")
 ...
 IF ( NOT (validate(log,0)))
  ...
  DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_prsnl_purge_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_PRSNL_PURGE"
 ...
 ELSEIF (the_type="J"
  ...
  SELECT INTO "nl:"
   ...
   HEAD a.person_id
    ...
    .ags_prsnl_data_id, purge_rec->qual[sknt].delete_person_id = a.person_id,
 ...
 ELSEIF (the_type="I")
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "INVALID PURGE >> PRSNL by Item Not Allowed"
  GO TO exit_script
 ELSE
  SET failed = input_error
  SET ilog_status = 1
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY d.seq
  HEAD d.seq
   purge_rec->qual[d.seq].is_a_person = true
  WITH nocounter
 ;end select
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("SELECT ERROR AGS_PERSON_DATA (PERSON Check)>> ",trim(
    serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 ...
 SELECT INTO "nl:"
  ...
  HEAD d.seq
   purge_rec->qual[d.seq].bo_item_id = a.ags_prsnl_data_id
  WITH nocounter
 ;end select
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("SELECT ERROR AGS_PRSNL_DATA (backout items)>> ",trim(
    serrmsg))
  GO TO exit_script
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   JOIN (a2
   WHERE (a2.prescriber_id=purge_rec->qual[d.seq].mil_item_id)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   WHERE d.seq > 0
    AND (purge_rec->qual[d.seq].mil_item_id > 0))
   JOIN (a2
   WHERE (a2.ordering_person_id=purge_rec->qual[d.seq].mil_item_id))
  HEAD REPORT
   sknt = 0, stat = alterlist(sub_item_rec->qual,10)
  HEAD a2.event_id
   sknt = (sknt+ 1)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   WHERE d.seq > 0
    AND (purge_rec->qual[d.seq].mil_item_id > 0))
   JOIN (h
   WHERE (h.billing_provider_id=purge_rec->qual[d.seq].mil_item_id))
   JOIN (a2
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   WHERE d.seq > 0
    AND (purge_rec->qual[d.seq].mil_item_id > 0))
   JOIN (h
   WHERE (h.attending_person_id=purge_rec->qual[d.seq].mil_item_id))
   JOIN (a2
   WHERE a2.hea_claim_visit_id=h.hea_claim_visit_id)
  HEAD REPORT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   WHERE d.seq > 0
    AND (purge_rec->qual[d.seq].mil_item_id > 0))
   JOIN (h
   WHERE (h.admitting_person_id=purge_rec->qual[d.seq].mil_item_id))
   JOIN (a2
   WHERE a2.hea_claim_visit_id=h.hea_claim_visit_id)
  HEAD REPORT
   sknt = sub_item_rec->qual_knt, stat = alterlist(sub_item_rec->qual,(sknt+ 10))
 ...
 DELETE  FROM address a,
  ...
  PLAN (d
   WHERE d.seq > 0
    AND (purge_rec->qual[d.seq].mil_item_id > 0)
    AND (purge_rec->qual[d.seq].is_a_person=false))
   JOIN (a
 ...
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 DELETE  FROM (dummyt d  WITH seq = value(purge_rec->qual_knt)),
   prsnl_alias a
  SET a.seq = 1
  PLAN (d
   WHERE d.seq > 0
 ...
 DELETE  FROM person_prsnl_reltn a,
  ...
  PLAN (d
   WHERE d.seq > 0
    AND (purge_rec->qual[d.seq].mil_item_id > 0))
   JOIN (a
   WHERE (a.prsnl_person_id=purge_rec->qual[d.seq].mil_item_id))
  WITH nocounter, maxcommit = 5000
 ;end delete
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
 ...
 DELETE  FROM person_name a,
  ...
  PLAN (d
   WHERE d.seq > 0
    AND (purge_rec->qual[d.seq].mil_item_id > 0)
    AND (purge_rec->qual[d.seq].is_a_person=true))
   JOIN (a
   WHERE (a.person_id=purge_rec->qual[d.seq].mil_item_id)
 ...
 UPDATE  FROM ags_prsnl_data a,
  ...
  PLAN (d
   ...
   JOIN (a
   WHERE (a.ags_prsnl_data_id=purge_rec->qual[d.seq].bo_item_id))
  WITH nocounter
 ;end update
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_PRSNL_DATA (backout items >> ",trim(
    serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 ...
 UPDATE  FROM ags_prsnl_data a,
  ...
  PLAN (d
   WHERE d.seq > 0
    ...
    AND (purge_rec->qual[d.seq].delete_item_id > 0))
   JOIN (a
   WHERE (a.ags_prsnl_data_id=purge_rec->qual[d.seq].delete_item_id))
  WITH nocounter
 ;end update
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_PRSNL_DATA (delete items) >> ",trim(
    serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("DELETE ERROR AGS_PRSNL_DATA >> ",trim(serrmsg))
  GO TO exit_script
 ENDIF
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_PRSNL_PURGE"
 IF (define_logging_sub=true)
  CALL echo("***")
