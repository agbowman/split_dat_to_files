CREATE PROGRAM ams_irc_import_users
 PROMPT
  "Enter Groups to Load" = "",
  "Enter Position Display to Use" = ""
  WITH group, pos
 EXECUTE cclseclogin
 EXECUTE ams_define_toolkit_common:003
 FREE RECORD request
 RECORD request(
   1 audit_mode_ind = i2
   1 person_list[*]
 ...
 DECLARE script_name = vc WITH protect, constant("AMS_IRC_IMPORT_USERS")
 ...
 FREE DEFINE rtl2
 DEFINE rtl2 "ccluserdir:ircuserimport.csv"
 SELECT INTO "nl:"
  r.line
  FROM rtl2t r
  HEAD REPORT
 ...
 FOR (a = 1 TO size(import->qual,5))
  ...
  IF ((import->qual[a].action_ind=1))
   ...
   IF ((import->qual[a].person_id=0))
    IF ((import->qual[a].load_ind=1))
     ...
     SET request->person_list[cnt].name_title = "Cerner IRC"
   ...
   ELSE
    UPDATE  FROM person_name p
     SET p.name_title = "Cerner IRC", p.updt_id = 4801, p.updt_dt_tm = cnvtdatetime(curdate,curtime3)
  ...
  ELSEIF ((import->qual[a].action_ind=2))
   ...
   IF ((import->qual[a].person_id > 0))
    ...
    IF (curqual > 0)
     UPDATE  FROM prsnl p
      SET p.name_last = concat(trim(import->qual[a].name_last)," (IRC)"), p.name_last_key = cnvtupper
       (concat(trim(import->qual[a].name_last),"IRC")), p.name_full_formatted = concat(trim(import->
         qual[a].name_last)," (IRC), ",trim(import->qual[a].name_first))
      WHERE (p.person_id=import->qual[a].person_id)
      WITH nocounter
     ;end update
     UPDATE  FROM person pr
      SET pr.name_last = concat(trim(import->qual[a].name_last)," (IRC)"), pr.name_last_key =
       cnvtupper(concat(import->qual[a].name_last),"IRC"), pr.name_full_formatted = concat(trim(
         import->qual[a].name_last)," (IRC), ",trim(import->qual[a].name_first))
      WHERE (pr.person_id=import->qual[a].person_id)
      WITH nocounter
     ;end update
    ENDIF
    UPDATE  FROM prsnl p
