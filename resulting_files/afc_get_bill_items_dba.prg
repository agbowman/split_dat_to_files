CREATE PROGRAM afc_get_bill_items:dba
 ...
 IF (validate(copytracemessages,char(128))=char(128))
  ...
  END ;Subroutine
 ENDIF
 DECLARE afc_get_bill_items_vrsn = vc
 SET afc_get_bill_items_vrsn = "FT.469360.021"
 RECORD reply(
   1 list[*]
     2 bill_items[*]
       3 ext_owner_cd = f8
       ...
       3 active_ind = i2
       3 physician_qual_cd = f8
       3 beg_effective_dt_tm = dq8
       3 end_effective_dt_tm = dq8
       3 alpha_response_ind = i4
       3 misc_ind = i2
       3 stats_only_ind = i2
       3 workload_only_ind = i2
       3 bill_item_mods_bid = f8
 ...
 DECLARE ibisec = i2 WITH protect, noconstant(0)
 DECLARE iuploadind = i2 WITH protect, noconstant(0)
 DECLARE logicaldomainid = f8 WITH noconstant(0.0), protect
 DECLARE count1 = i4 WITH protect, noconstant(0)
 DECLARE mainlistcount = i4 WITH protect, noconstant(0)
 DECLARE maxlistsize = i4 WITH protect, constant(65534)
 ...
 SET ext_owner_disp = ""
 SET curqual1 = 0
 DECLARE iuploadind = i2
 SET iuploadind = 0
 DECLARE upload_cd = f8
 DECLARE ibischedsec = i2
 SET ibischedsec = 0
 DECLARE 26078_bill_item = f8
 DECLARE alpha_resp = f8
 SET code_set = 13016
 SET cdf_meaning = "ALPHA RESP"
 SET cnt1 = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cnt1,code_value)
 SET alpha_resp = code_value
 DECLARE add_on_owner_code = f8
 SET code_set = 106
 SET cdf_meaning = "AFC ADD SPEC"
 SET cnt1 = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cnt1,code_value)
 SET add_on_owner_code = code_value
 DECLARE add_on_code = f8
 SET code_set = 13019
 SET cdf_meaning = "ADD ON"
 SET cnt1 = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cnt1,code_value)
 SET add_on_code = code_value
 DECLARE ord_cat = f8
 SET code_set = 13016
 ...
 CALL echo(build("UPLOAD_CD: ",upload_cd))
 ...
 ENDIF
 CALL initsecuritypreference(ibisec,iuploadind)
  ...
  OF "P":
   IF (ibisec=1)
    ...
    SELECT
     ...
     ELSEIF ((request->ext_owner_cd=0)
      ...
      PLAN (por
       ...
       WHERE b.bill_item_id=cor.key1_id
        ...
        AND (((request->workload_only_ind=1)) OR ((request->workload_only_ind != 1)
        AND ((b.workload_only_ind=0) OR (b.workload_only_ind=null)) )) )
     ELSE
     ENDIF
     INTO "nl:"
     b.*
     ...
     DETAIL
      count1 += 1
      IF (mod(count1,maxlistsize)=1)
       IF (count1 > 1)
        stat = alterlist(reply->list[mainlistcount].bill_items,maxlistsize), count1 = 1
       ENDIF
       mainlistcount += 1, stat = alterlist(reply->list,mainlistcount)
      ENDIF
      IF (mod(count1,10)=1)
       stat = alterlist(reply->list[mainlistcount].bill_items,(count1+ 9))
      ENDIF
      reply->list[mainlistcount].bill_items[count1].bill_item_id = b.bill_item_id, reply->list[
      mainlistcount].bill_items[count1].ext_parent_reference_id = b.ext_parent_reference_id, reply->
      list[mainlistcount].bill_items[count1].ext_parent_contributor_cd = b.ext_parent_contributor_cd,
      reply->list[mainlistcount].bill_items[count1].ext_child_reference_id = b.ext_child_reference_id,
      reply->list[mainlistcount].bill_items[count1].ext_child_contributor_cd = b
      .ext_child_contributor_cd
      IF (trim(b.ext_description)=" ")
       reply->list[mainlistcount].bill_items[count1].ext_description = "BLANK"
      ELSE
       reply->list[mainlistcount].bill_items[count1].ext_description = trim(b.ext_description)
      ENDIF
      reply->list[mainlistcount].bill_items[count1].ext_short_desc = trim(b.ext_short_desc), reply->
      list[mainlistcount].bill_items[count1].ext_owner_cd = b.ext_owner_cd, msstat = assign(validate(
        reply->list[mainlistcount].bill_items[count1].ext_sub_owner_cd,0.0),b.ext_sub_owner_cd),
      reply->list[mainlistcount].bill_items[count1].parent_qual_cd = b.parent_qual_cd, reply->list[
      mainlistcount].bill_items[count1].charge_point_cd = b.charge_point_cd, reply->list[
      mainlistcount].bill_items[count1].physician_qual_cd = b.physician_qual_cd,
      reply->list[mainlistcount].bill_items[count1].beg_effective_dt_tm = cnvtdatetime(b
       .beg_effective_dt_tm), reply->list[mainlistcount].bill_items[count1].end_effective_dt_tm =
      cnvtdatetime(b.end_effective_dt_tm), reply->list[mainlistcount].bill_items[count1].active_ind
       = b.active_ind,
      reply->list[mainlistcount].bill_items[count1].misc_ind = b.misc_ind, reply->list[mainlistcount]
      .bill_items[count1].stats_only_ind = b.stats_only_ind, reply->list[mainlistcount].bill_items[
      count1].workload_only_ind = b.workload_only_ind,
      reply->list[mainlistcount].bill_items[count1].late_chrg_excl_ind = b.late_chrg_excl_ind
     WITH nocounter
    ;end select
    IF (mainlistcount >= 1)
     SET stat = alterlist(reply->list[mainlistcount].bill_items,count1)
    ENDIF
   ELSE
    CALL echo("Case of P")
    SELECT
   ...
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = size(reply->list,5)),
     (dummyt d2  WITH seq = size(reply->list[d1.seq].bill_items,5)),
     bill_item b
    PLAN (d1
     WHERE maxrec(d2,size(reply->list[d1.seq].bill_items,5)))
     JOIN (d2)
     JOIN (b
     WHERE (b.ext_parent_reference_id=reply->list[d1.seq].bill_items[d2.seq].ext_parent_reference_id)
      ...
      AND b.logical_domain_enabled_ind=true) OR (b.logical_domain_enabled_ind=false)) )
    DETAIL
     reply->list[d1.seq].bill_items[d2.seq].careset_ind = 1
    WITH nocounter
   ;end select
  OF "C":
   CALL echo("Case is C")
   SELECT
    ...
    DETAIL
     ...
     IF (mod(count1,10)=1)
      stat = alterlist(reply->list[mainlistcount].bill_items,(count1+ 9))
     ENDIF
     reply->list[mainlistcount].bill_items[count1].bill_item_id = b.bill_item_id,
     CALL echo(concat("bill_item_id: ",cnvtstring(b.bill_item_id,17,2))),
     CALL echo(concat("    workload_only_ind: ",cnvtstring(b.workload_only_ind))),
     reply->list[mainlistcount].bill_items[count1].ext_parent_reference_id = b
     .ext_parent_reference_id, reply->list[mainlistcount].bill_items[count1].
     ext_parent_contributor_cd = b.ext_parent_contributor_cd, reply->list[mainlistcount].bill_items[
     count1].ext_child_reference_id = b.ext_child_reference_id,
     reply->list[mainlistcount].bill_items[count1].ext_child_contributor_cd = b
     .ext_child_contributor_cd
     IF (trim(b.ext_description)=" ")
      reply->list[mainlistcount].bill_items[count1].ext_description = "BLANK"
     ...
     list[mainlistcount].bill_items[count1].ext_owner_cd = b.ext_owner_cd, msstat = assign(validate(
       reply->list[mainlistcount].bill_items[count1].ext_sub_owner_cd,0.0),b.ext_sub_owner_cd),
     reply->list[mainlistcount].bill_items[count1].parent_qual_cd = b.parent_qual_cd, reply->list[
     mainlistcount].bill_items[count1].charge_point_cd = b.charge_point_cd, reply->list[mainlistcount
     ].bill_items[count1].physician_qual_cd = b.physician_qual_cd,
     reply->list[mainlistcount].bill_items[count1].beg_effective_dt_tm = cnvtdatetime(b
      .beg_effective_dt_tm), reply->list[mainlistcount].bill_items[count1].end_effective_dt_tm =
     cnvtdatetime(b.end_effective_dt_tm), reply->list[mainlistcount].bill_items[count1].active_ind =
     b.active_ind,
   ...
   SELECT INTO "nl:"
    ...
    PLAN (b
     ...
     WHERE bm.bill_item_id=b.bill_item_id
      AND bm.bill_item_type_cd=add_on_code
      AND bm.active_ind=1
      AND bm.key3_id=logicaldomainid)
     JOIN (b2
     WHERE b2.bill_item_id=bm.key1_id
      AND b2.active_ind=1)
    DETAIL
     count1 += 1
     IF (mod(count1,maxlistsize)=1)
      ...
      mainlistcount += 1, stat = alterlist(reply->list,mainlistcount)
     ENDIF
     stat = alterlist(reply->list[mainlistcount].bill_items,count1), reply->list[mainlistcount].
     bill_items[count1].bill_item_id = b2.bill_item_id, reply->list[mainlistcount].bill_items[count1]
     .bill_item_mods_bid = bm.bill_item_id,
     reply->list[mainlistcount].bill_items[count1].ext_parent_reference_id = b2
     .ext_parent_reference_id, reply->list[mainlistcount].bill_items[count1].
     ext_parent_contributor_cd = b2.ext_parent_contributor_cd, reply->list[mainlistcount].bill_items[
     count1].ext_child_reference_id = b2.ext_child_reference_id,
     reply->list[mainlistcount].bill_items[count1].ext_child_contributor_cd = b2
     .ext_child_contributor_cd
     IF (trim(b2.ext_description)=" ")
      reply->list[mainlistcount].bill_items[count1].ext_description = bm.key6
     ELSE
      reply->list[mainlistcount].bill_items[count1].ext_description = trim(b2.ext_description)
     ENDIF
     reply->list[mainlistcount].bill_items[count1].ext_short_desc = trim(b2.ext_short_desc), reply->
     list[mainlistcount].bill_items[count1].ext_owner_cd = b2.ext_owner_cd, msstat = assign(validate(
       reply->list[mainlistcount].bill_items[count1].ext_sub_owner_cd,0.0),b2.ext_sub_owner_cd),
     reply->list[mainlistcount].bill_items[count1].parent_qual_cd = b2.parent_qual_cd, reply->list[
     mainlistcount].bill_items[count1].charge_point_cd = b2.charge_point_cd, reply->list[
     mainlistcount].bill_items[count1].physician_qual_cd = b2.physician_qual_cd,
     reply->list[mainlistcount].bill_items[count1].beg_effective_dt_tm = cnvtdatetime(b2
      .beg_effective_dt_tm), reply->list[mainlistcount].bill_items[count1].end_effective_dt_tm =
     cnvtdatetime(b2.end_effective_dt_tm), reply->list[mainlistcount].bill_items[count1].active_ind
      = b2.active_ind,
     reply->list[mainlistcount].bill_items[count1].misc_ind = b2.misc_ind, reply->list[mainlistcount]
     .bill_items[count1].stats_only_ind = b2.stats_only_ind, reply->list[mainlistcount].bill_items[
     count1].workload_only_ind = b.workload_only_ind,
     reply->list[mainlistcount].bill_items[count1].late_chrg_excl_ind = b.late_chrg_excl_ind
    WITH nocounter
   ;end select
  ...
  OF "O":
   SELECT
    IF ((request->ext_parent_reference_id != 0)
     AND iuploadind=0)
     WHERE (((b.ext_child_reference_id=request->ext_parent_reference_id)
      AND b.ext_parent_reference_id=0) OR ((b.ext_parent_reference_id=request->
     ext_parent_reference_id)
      ...
      AND ((b.logical_domain_id=logicaldomainid
      AND b.logical_domain_enabled_ind=true) OR (b.logical_domain_enabled_ind=false)) ))
    ELSEIF ((request->ext_parent_reference_id != 0))
     WHERE (((b.ext_child_reference_id=request->ext_parent_reference_id)
      AND b.ext_parent_reference_id=0) OR ((b.ext_parent_reference_id=request->
     ext_parent_reference_id)
      AND b.ext_child_contributor_cd=alpha_resp
  ...
  OF "A":
   ...
   SELECT
    ...
    DETAIL
     ...
     count1].workload_only_ind = b.workload_only_ind,
     reply->list[mainlistcount].bill_items[count1].late_chrg_excl_ind = b.late_chrg_excl_ind,
     CALL echo(build("Found item: ",b.ext_description))
    WITH nocounter
   ;end select
   IF (mainlistcount >= 1)
    SET stat = alterlist(reply->list[mainlistcount].bill_items,count1)
 ...
 ENDIF
 SUBROUTINE (initsecuritypreference(pbillitemsecurity=i2(ref),puploadind=i2(ref)) =i2)
   ...
   SET afc_dm_request->info[2].info_name = "BILLITEM UPLOAD"
   ...
   IF ((afc_dm_reply->status_data.status="S"))
    FOR (nrepcount = 1 TO size(afc_dm_reply->dm_info,5))
     IF ((afc_dm_reply->dm_info[nrepcount].info_name="BILL ITEM SECURITY")
      ...
      SET pbillitemsecurity = true
     ENDIF
     IF ((afc_dm_reply->dm_info[nrepcount].info_name="BILLITEM UPLOAD")
      AND (afc_dm_reply->dm_info[nrepcount].info_char="Y"))
      SET puploadind = true
     ENDIF
    ENDFOR
