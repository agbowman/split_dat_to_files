CREATE PROGRAM bhs_behav_id_upd:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Select Patient:" = 0,
  "***INACTIVATE CARE PLAN***:" = 0,
  "Effective Date of Care Plan:" = "SYSDATE",
  ...
  "Approval:" = "",
  "Enter Approval text:" = ""
  WITH outdev, f_behav_id, n_inactivate,
  s_plan_effective_dt_tm, s_review_due_dt_tm, s_plan_reason,
  s_plan_reason_ft, s_rationale_ft, s_clin_contact,
  s_clin_contact_ft, s_phys_name, s_pcp_name,
  s_interventions, s_goals_outcomes, s_approval,
  s_approval_ft
 EXECUTE reportrtl
 ...
 SUBROUTINE sec_headabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("ED Care Plan",char(0)))
 ...
 SUBROUTINE sec_introabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(concat(
       "This patient has an ED Care Plan currently in place.",_crlf,"The Care Plan is as follows:"),
      char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
 ...
 SUBROUTINE sec_inactivateabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
      "The following ED Care Plan has been inactivated.",char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "bhs_behav_id_upd"
 ...
 DECLARE mn_apprv_param = i2 WITH protect, constant(15)
 ...
 IF (( $N_INACTIVATE=0))
  SET m_rec->s_beg_dt_tm = trim( $S_PLAN_EFFECTIVE_DT_TM)
  ...
  IF (substring(1,1,ms_data_type)="C")
   IF (findstring("OTHER", $S_PLAN_REASON)=0)
    SET m_rec->s_reason = trim(replace( $S_PLAN_REASON,char(160)," ",0))
   ENDIF
  ELSEIF (substring(1,1,ms_data_type)="L")
   ...
   ENDFOR
  ENDIF
  IF (trim(replace( $S_PLAN_REASON_FT,char(160)," ",0)) > " ")
   IF (trim(m_rec->s_reason) > " ")
    SET m_rec->s_reason = concat(m_rec->s_reason,"|OTHER-",trim(replace(replace( $S_PLAN_REASON_FT,
        ":","-",0),char(160)," ",0)))
   ELSE
    SET m_rec->s_reason = concat(m_rec->s_reason,"OTHER-",trim(replace(replace( $S_PLAN_REASON_FT,":",
        "-",0),char(160)," ",0)))
   ENDIF
  ENDIF
  SET m_rec->s_rationale = trim(replace(replace( $S_RATIONALE_FT,":","-",0),char(160)," ",0))
  SET m_rec->s_goal = trim(replace(replace( $S_GOALS_OUTCOMES,":","-",0),char(160)," ",0))
  ...
  IF (trim(replace( $S_CLIN_CONTACT_FT,char(160)," ",0)) > " ")
   ...
   ENDIF
  ENDIF
  SET ms_data_type = build(reflect(parameter(mn_apprv_param,0)))
  IF (substring(1,1,ms_data_type)="C")
   IF (findstring("OTHER", $S_APPROVAL)=0)
    SET m_rec->s_approval = trim(replace( $S_APPROVAL,char(160)," ",0))
   ENDIF
  ELSEIF (substring(1,1,ms_data_type)="L")
   FOR (ml_cnt = 1 TO cnvtint(substring(2,(textlen(ms_data_type) - 1),ms_data_type)))
     SET ms_tmp = parameter(mn_apprv_param,ml_cnt)
     SET ms_tmp = trim(ms_tmp)
     IF (findstring("OTHER",ms_tmp)=0)
      IF (trim(m_rec->s_approval) <= " ")
       SET m_rec->s_approval = trim(ms_tmp)
      ELSE
       SET m_rec->s_approval = concat(m_rec->s_approval,"|",trim(ms_tmp))
      ENDIF
     ENDIF
   ENDFOR
  ENDIF
  IF (trim(replace( $S_APPROVAL_FT,char(160)," ",0)) > " ")
   IF (trim(m_rec->s_approval) > " ")
    SET m_rec->s_approval = concat(m_rec->s_approval,"|OTHER-",trim(replace(replace( $S_APPROVAL_FT,
        ":","-",0),char(160)," ",0)))
   ELSE
    SET m_rec->s_approval = concat(m_rec->s_approval,"OTHER-",trim(replace(replace( $S_APPROVAL_FT,
        ":","-",0),char(160)," ",0)))
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   IF (((trim(m_rec->s_approval) != trim(b.approval)) OR (((trim(m_rec->s_clin_contact) != trim(b
    .clinical_contact)) OR (((trim(m_rec->s_goal) != trim(b.goal)) OR (((trim(m_rec->s_intervention)
    != trim(b.intervention)) OR (((trim(m_rec->s_rationale) != trim(b.rationale)) OR (((trim(m_rec->
    s_reason) != trim(b.reason)) OR (cnvtdatetime(m_rec->s_review_dt_tm) != b.review_dt_tm)) )) ))
   )) )) )) )
 ...
 IF (mn_update=1)
  ...
  IF (( $N_INACTIVATE=0))
   ...
   INSERT  FROM bhs_pat_behav_ident b
    SET b.active_ind = 1, b.approval = trim(m_rec->s_approval), b.beg_effective_dt_tm = cnvtdatetime(
      m_rec->s_beg_dt_tm),
     ...
     b.goal = m_rec->s_goal, b.intervention = m_rec->s_intervention, b.person_id = m_rec->f_person_id,
     b.rationale = m_rec->s_rationale, b.reason = trim(m_rec->s_reason), b.review_dt_tm =
     cnvtdatetime(m_rec->s_review_dt_tm),
     b.updt_cnt = 0, b.updt_dt_tm = sysdate, b.updt_person_id = reqinfo->updt_id
    WITH nocounter
   ;end insert
   COMMIT
  ENDIF
 ...
 ENDIF
 CALL sbr_printline("Rationale","")
 CALL sbr_printline("Effective Date of Care Plan-",m_rec->s_beg_dt_tm)
 CALL sbr_printline("Due for Review-",m_rec->s_review_dt_tm)
 CALL sbr_printline("Reason for Care Plan-","")
 SET ml_pos = 0
 SET ml_start = 1
 ...
 ENDIF
 CALL sbr_printline("Rationale-",m_rec->s_rationale)
 ...
 CALL sbr_printline("Goals & Outcomes","")
 CALL sbr_printline("",m_rec->s_goal)
 CALL sbr_printline("Approval","")
 SET ml_pos = 0
 SET ml_start = 1
 IF (findstring("|",m_rec->s_approval))
  WHILE (findstring("|",m_rec->s_approval,ml_start) > 0)
    SET ml_pos = findstring("|",m_rec->s_approval,ml_start)
    SET ms_tmp = substring(ml_start,(ml_pos - ml_start),m_rec->s_approval)
    SET ml_start = (ml_pos+ 1)
    CALL sbr_printline("",ms_tmp)
  ENDWHILE
  SET ms_tmp = substring(ml_start,((textlen(m_rec->s_approval) - ml_start)+ 1),m_rec->s_approval)
  CALL sbr_printline("",ms_tmp)
 ELSE
  CALL sbr_printline("",m_rec->s_approval)
 ENDIF
