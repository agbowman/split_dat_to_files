CREATE PROGRAM dm2_dbstats_menu:dba
 ...
 ENDIF
 IF (check_logfile("dm2_dbstats_menu",".log","DM2_DBSTATS_MENU LOGFILE")=0)
  GO TO exit_script
 ENDIF
 WHILE (true)
   SET message = window
   ...
   CALL clear(1,1)
   CALL box(1,1,24,131)
   CALL text(2,2,"Database Statistics Gathering Menu")
   CALL text(5,10,"1. Show table statistics gathering history report")
   CALL text(6,10,"2. Manage statistics overrides <UNAVAILABLE>")
   CALL text(7,10,"3. Manage statistics execution window")
   ...
   CASE (curaccept)
    OF 0:
     GO TO exit_script
    OF 1:
     EXECUTE dm2_dbstats_report
     IF ((dm_err->err_ind=1))
      GO TO exit_script
     ENDIF
    OF 3:
     IF (dpq_manage_end_time(1,dpq_statistics,0) != 1)
      GO TO exit_script
     ENDIF
   ENDCASE
 ENDWHILE
#exit_script
 ...
 ELSE
  SET dm_err->eproc = "DM2_DBSTATS_MENU completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_dbstats_menu")
