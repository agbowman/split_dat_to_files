CREATE PROGRAM dm_da_ref_report_event_rows:dba
 ...
 END ;Subroutine
 DECLARE dpsd_tablename = vc WITH protect, constant("REF_REPORT_EVENT")
 DECLARE dpsd_weeks_to_keep_min = i4 WITH protect, constant(1)
 DECLARE dpsd_weeks_to_keep = i4 WITH protect, noconstant(- (1))
 DECLARE dpsd_ndx = i2 WITH protect, noconstant(0)
 DECLARE dpsd_row_count = i4 WITH protect, noconstant(0)
 DECLARE dpsd_i18n_handle = i4 WITH protect, noconstant(0)
 DECLARE dpsd_archi_version = f8 WITH protect, noconstant(sbr_get_purge_archi_version(null))
 DECLARE dpsd_parser_stmt = vc WITH protect, noconstant("")
 DECLARE dpsd_environment_id = f8 WITH protect, noconstant(0.0)
 SET reply->status_data.status = "F"
 SET reply->err_code = 0
 SET reply->table_name = dpsd_tablename
 SET reply->rows_between_commit = 1
 SET stat = uar_i18nlocalizationinit(dpsd_i18n_handle,curprog,"",curcclrev)
 IF (check_logfile("DM_DA_REF_REPORT_EV",".log","DM_DA_REF_REPORT_EVENT_ROWS log file")=0)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(dpsd_i18n_handle,"LOGERROR",
   "Failed to initialize log file: %1","s",nullterm(dm_err->emsg))
  SET reply->err_code = dm_err->ecode
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Begin DM_DA_REF_REPORT_EVENT_ROWS"
 CALL disp_msg(" ",dm_err->logfile,0)
 FOR (dpsd_ndx = 1 TO size(request->tokens,5))
   IF (cnvtupper(request->tokens[dpsd_ndx].token_str)="WEEKSTOKEEP")
    SET dpsd_weeks_to_keep = ceil(cnvtreal(request->tokens[dpsd_ndx].value))
   ENDIF
 ENDFOR
 IF (dpsd_weeks_to_keep < dpsd_weeks_to_keep_min)
  SET reply->status_data.status = "F"
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(dpsd_i18n_handle,"BADWEEKS",
   "You must keep at least 1 week's worth of data.  You entered %1 or did not enter any value.","i",
   dpsd_weeks_to_keep)
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  ...
  DETAIL
   dpsd_environment_id = di.info_number
  WITH nocounter
 ;end select
 IF (dpsd_environment_id=0.0)
  SET reply->status_data.status = "F"
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18ngetmessage(dpsd_i18n_handle,"NOENVID","No environment ID was found.")
  GO TO exit_script
 ENDIF
 IF (dpsd_archi_version >= 2.0)
  SET dpsd_parser_stmt = sbr_getrowidnotexists("rre.environment_id = dpsd_environment_id","rre")
 ELSE
  SET dpsd_parser_stmt = "rre.environment_id = dpsd_environment_id"
 ENDIF
 SELECT INTO "nl:"
  rre.rowid
  FROM ref_report_event rre
  WHERE ((rre.end_dt_tm < cnvtdatetime((curdate - (7 * dpsd_weeks_to_keep)),curtime3)) OR (rre
  .begin_dt_tm < cnvtdatetime((curdate - 7),curtime3)
   AND rre.status != "COMPLETE"))
   AND parser(dpsd_parser_stmt)
  DETAIL
   dpsd_row_count = (dpsd_row_count+ 1)
   IF (mod(dpsd_row_count,50)=1)
    stat = alterlist(reply->rows,(dpsd_row_count+ 49))
   ENDIF
   reply->rows[dpsd_row_count].row_id = rre.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,dpsd_row_count)
  WITH nocounter, maxqual(rre,value(request->max_rows))
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET reply->err_code = 1
  SET reply->err_msg = uar_i18nbuildmessage(dpsd_i18n_handle,"COLLECTERR",
   "Failure during collection of rows for purging: %1","s",nullterm(dm_err->emsg))
  GO TO exit_script
 ENDIF
 SET reply->status_data.status = "S"
 SET reply->err_code = 0
#exit_script
 SET dm_err->eproc = "Ending DM_DA_REF_REPORT_EVENT_ROWS."
 CALL final_disp_msg("DM_DA_REF_REPORT_EV")
END GO
