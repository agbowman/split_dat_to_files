CREATE PROGRAM dm_sys_outputctx_rows:dba
 ...
 SET reply->table_name = "OUTPUTCTX"
 ...
 FOR (tok_ndx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 1.0)
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"k1",
   "You must keep at least one day's worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
  SET reply->status_data.status = "F"
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(o.handle_id)
    FROM outputctx o
    WHERE o.handle_id > 0
    DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     o.rowid
     FROM outputctx o
     WHERE o.handle_id BETWEEN curminid AND curmaxid
      AND parser(sbr_getrowidnotexists(
       "o.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep), curtime3)","o"))
      AND  NOT ( EXISTS (
     (SELECT
      cr.chart_request_id
      FROM chart_request cr
      WHERE cr.handle_id=o.handle_id)))
      AND  NOT ( EXISTS (
     (SELECT
      crr.report_request_id
      FROM cr_report_request crr
      WHERE crr.rrd_handle_id=o.handle_id)))
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,50)=1)
       stat = alterlist(reply->rows,(rows+ 49))
      ENDIF
      reply->rows[rows].row_id = o.rowid
     WITH nocounter, maxqual(o,value(rowsleft))
    ;end select
