CREATE PROGRAM cps_chk_mrg_nomen_dups:dba
 ...
 SET false = 0
 SET continue = true
 SET log_file_name = "CPS_MRG_NOMEN_DUPS.LOG"
 SELECT INTO "nl:"
  err_msg = r.line
  FROM rtl2t r
  HEAD REPORT
 ...
 IF (curqual < 1)
  ...
  SET request->setup_proc[1].error_msg = concat("FAILURE : Could not find CCLUSERDIR:",trim(
    log_file_name),"  ",format(cnvtdatetime(curdate,curtime3),"dd-mmm-yyyy hh:mm;;q"))
  GO TO exit_script
 ENDIF
 WHILE (knt >= 1
  AND continue=true)
   IF (substring(28,7,err_log->qual[knt].msg)="SUCCESS")
    ...
    IF (datetimediff(cnvtdatetime(beg_dt_tm),cnvtdatetime(substring(37,20,err_log->qual[knt].msg)))
     ...
     SET request->setup_proc[1].error_msg = concat("SUCCESS : Merging duplicate nomenclature items ",
      format(cnvtdatetime(curdate,curtime3),"dd-mmm-yyyy hh:mm;;q"))
     GO TO exit_script
    ENDIF
   ELSEIF (substring(28,7,err_log->qual[knt].msg)="WARNING")
    SET continue = false
    IF (datetimediff(cnvtdatetime(beg_dt_tm),cnvtdatetime(substring(37,20,err_log->qual[knt].msg)))
     < time_dif)
     SET request->setup_proc[1].success_ind = 1
     SET request->setup_proc[1].error_msg = concat("WARNING : Merging duplicate nomenclature items ",
   ...
   ELSEIF (substring(28,7,err_log->qual[knt].msg)="FAILURE")
    ...
    IF (datetimediff(cnvtdatetime(beg_dt_tm),cnvtdatetime(substring(37,20,err_log->qual[knt].msg)))
     < time_dif)
     SET request->setup_proc[1].success_ind = 0
     SET request->setup_proc[1].error_msg = concat("FAILURE : Merging duplicate nomenclature items ",
      format(cnvtdatetime(curdate,curtime3),"dd-mmm-yyyy hh:mm;;q"),"  :See log file CCLUSERDIR:",
      trim(log_file_name)," for details")
     GO TO exit_script
    ENDIF
