CREATE PROGRAM cp_delete_old_distr_rows_14119:dba
 FREE SET work_array
 RECORD work_array(
   1 work_element[*]
     2 wk_encntr_id = f8
 ...
 SET log_cnt = 0
 CALL update_log("*******************************************")
 CALL update_log(build("BEGIN -- CP_DELETE_OLD_DISTR_ROWS_14119"," - ",format(beg_time,
...
#exit_script
 ...
 ENDIF
 SET end_time = cnvtdatetime(curdate,curtime3)
 CALL update_log(build("END -- CP_DELETE_OLD_DISTR_ROWS_14119"," - ",format(end_time,
    "mm/dd/yyyy hh:mm:ss;;d")))
 SET exec_time = 0
