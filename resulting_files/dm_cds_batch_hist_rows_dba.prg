CREATE PROGRAM dm_cds_batch_hist_rows:dba
 ...
 SET reply->table_name = "CDS_BATCH_HIST"
 ...
 FOR (tok_ndx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 100)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must keep at least 100 days' worth of data.  You entered %1 days or did not enter any value.",
   "i",days_to_keep)
  GO TO exit_script
 ELSE
  ...
  SELECT INTO "nl:"
   c.cds_batch_hist_id
   FROM cds_batch_hist c
   WHERE parser(sbr_getrowidnotexists("c.updt_dt_tm < cnvtdatetime(v_days_back)","c"))
    AND ((c.cds_batch_hist_id+ 0) != 0)
   ORDER BY c.cds_batch_dt_tm
   HEAD REPORT
    v_rows = 0
   DETAIL
    v_rows = (v_rows+ 1)
   ...
   WITH nocounter, maxqual(c,value(request->max_rows))
  ;end select
