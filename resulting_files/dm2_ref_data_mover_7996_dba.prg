CREATE PROGRAM dm2_ref_data_mover_7996:dba
 ...
 DECLARE cust_par_t_name = vc
 DECLARE cust_action_id = f8
 DECLARE cust_action_value = vc
 DECLARE cust_action_val_pos = i4
 DECLARE cust_to_action_val = vc
 ...
 SET cust_par_t_name = dm2_get_rdds_tname("SA_REF_ACTION")
 SELECT INTO "NL:"
  FROM (parser(cust_par_t_name) sa)
  WHERE sa.sa_ref_action_id=cust_action_id
  DETAIL
 ...
 IF (cust_type_flag=4)
  ...
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"SA_REF_MACRO_ACTION_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"SA_REF_MACRO_ACTION_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"SA_REF_MACRO_ACTION_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,
       "  SA_REF_MACRO_ACTION_ID")
     ELSE
      GO TO exit_7996
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"SA_REF_MACRO_ACTION_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_7996
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"SA_REF_MACRO_ACTION_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_7996
   ELSE
