CREATE PROGRAM dm2_install_plan_menu_mon
 ...
 SUBROUTINE dipma_check_db_arch_mode(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dipmm_logfile_prefix = vc WITH protect, constant("dm2_inst_plan_menu")
 DECLARE dipmm_continue_loop = i2 WITH protect, noconstant(1)
 DECLARE dipmm_accept = vc WITH protect, noconstant("")
 DECLARE dipmm_mon_not_set = i2 WITH protect, noconstant(0)
 DECLARE dipmm_planmon_status = i2 WITH protect, noconstant(1)
 DECLARE dipmm_cur_planmon_status = i2 WITH protect, noconstant(0)
 DECLARE dipmm_planmon_interval = f8 WITH protect, noconstant(15.0)
 DECLARE dipmm_uname = vc WITH protect, noconstant("V500")
 DECLARE dipmm_pwd = vc WITH protect, noconstant("")
 DECLARE dipmm_connect_str = vc WITH protect, noconstant("")
 DECLARE dipmm_upd_dminfo_ind = i2 WITH protect, noconstant(0)
 DECLARE dipmm_main_loop = i4 WITH protect, noconstant(1)
 DECLARE dipmm_archmon_status = i4 WITH protect, noconstant(0)
 DECLARE dipmm_arch_notify_threshold = f8 WITH protect, noconstant(0.0)
 DECLARE dipmm_arch_pause_threshold = f8 WITH protect, noconstant(0.0)
 SET width = 132
 IF (check_logfile(dipmm_logfile_prefix,".log","dm2_install_plan_menu_mon")=0)
  CALL dipr_disp_error_msg(null)
  GO TO exit_program
 ENDIF
 ...
 IF ((dipm_misc_data->install_plan_id=0))
  ...
  ENDIF
 ENDIF
 IF ((((dipm_misc_data->cur_dpe_id=0)) OR ((dipm_misc_data->cur_mode="DM2NOTSET"))) )
  IF (dipr_get_cur_dpe_data(dipm_misc_data->install_plan_id)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 WHILE (dipmm_main_loop=1)
   SET dipmm_mon_not_set = 0
   SET dm_err->eproc = "Retrieving previous values for monitoring settings"
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="DM2_INSTALL_MONITOR:PLANSETTINGS"
     AND (cnvtreal(di.info_name)=dipm_misc_data->install_plan_id)
    DETAIL
     dipmm_planmon_status = di.info_number, dipmm_planmon_interval = di.info_long_id
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL dipr_disp_error_msg(null)
   ...
   IF (dipma_check_db_arch_mode(null)=0)
    IF ((dm_err->err_ind=1))
     ...
     GO TO exit_program
    ENDIF
    SET dipmm_archmon_status = 0
   ELSE
    IF (dipma_get_arch_thresholds(dipmm_arch_pause_threshold,dipmm_arch_notify_threshold)=0)
     IF ((dm_err->err_ind=1))
      CALL dipr_disp_error_msg(null)
      GO TO exit_program
     ENDIF
     SET dipmm_archmon_status = 0
    ELSE
     SET dipmm_archmon_status = 1
    ENDIF
   ENDIF
   SET message = window
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL text(2,2,"INSTALL PLAN MENU [MONITORING]")
   ...
   CALL text(7,2,concat("1) Manage Install Plan Monitoring   [",evaluate(dipmm_planmon_status,1,"ON",
      "OFF"),"]"))
   IF (dipmm_planmon_status=1)
    CALL text(8,2,concat("  A) Manage Archive Log Monitoring  [",evaluate(dipmm_archmon_status,1,"ON",
       "OFF"),"]*"))
   ELSE
    CALL text(8,2,concat("  A) Manage Archive Log Monitoring  [",evaluate(dipmm_planmon_status,1,"ON",
       "OFF"),"]*"))
   ENDIF
   CALL text(23,2,"* this option can only be turned ON if Install Plan Monitoring is ON")
   CALL text(10,2,"Your selection or 0 to go back: ")
   CALL accept(10,36,"P;CU","0"
    WHERE curaccept IN ("0", "1", "A"))
   CASE (curaccept)
    OF "0":
     SET dipmm_main_loop = 0
    OF "1":
     SET dipmm_continue_loop = 1
     WHILE (dipmm_continue_loop=1)
       SET message = window
       CALL clear(1,1)
       CALL box(1,1,3,132)
       CALL text(2,2,"INSTALL PLAN [MANAGE MONITORING]")
       CALL text(2,70,"DATE/TIME: ")
       CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
       CALL text(5,2,"Plan Number: ")
       CALL text(5,18,drr_flex_sched->pkg_number)
       CALL text(7,2,"Monitoring Status: ")
       CALL text(7,21,evaluate(dipmm_planmon_status,1,"ON","OFF"))
       CALL text(8,2,concat("Monitoring Interval: Every ",trim(cnvtstring(dipmm_planmon_interval)),
         " Minutes"))
       CALL text(10,2,"(M)odify, (S)ave, (B)ack: ")
       CALL accept(10,30,"p;cu","S"
        WHERE curaccept IN ("M", "S", "B"))
       CASE (curaccept)
        OF "B":
         SET dipmm_continue_loop = 0
        OF "M":
         SET help = pos(4,80,10,50)
         SET help = fix("ON,OFF")
         CALL clear(7,25,11)
         CALL accept(7,25,"A(3);CUF")
         IF (curhelp > 0)
          SET dipmm_planmon_status = evaluate(curhelp,1,1,2,0)
         ELSE
          SET dipmm_planmon_status = evaluate(curaccept,"ON",1,0)
         ENDIF
         IF (dipmm_planmon_status=1)
          CALL clear(8,23,20)
          CALL text(8,40,"Enter value between 1 and 1440")
          CALL accept(8,24,"9(4)",dipmm_planmon_interval
           WHERE curaccept >= 1
            AND curaccept <= 1440)
          SET dipmm_planmon_interval = cnvtreal(curaccept)
         ENDIF
         SET help = off
        OF "S":
         IF (dipmm_planmon_status=0)
          IF ((dipm_misc_data->cur_dpe_status IN (value(dpl_executing), value(dpl_paused))))
           IF ((dipm_misc_data->cur_method="UNATTENDED"))
            CALL text(12,2,"Monitoring cannot be turned OFF while Unattended Installs are executing."
             )
            CALL text(13,2,"Press <Enter> to go back.")
            CALL accept(13,28,"p;cu"," "
             WHERE curaccept IN (" "))
           ELSE
            CALL text(12,2,
             "Monitoring will be shut off AFTER the current monitoring interval has expired.")
            CALL text(13,2,"Press <Enter> to go back.")
            CALL accept(13,28,"p;cu"," "
             WHERE curaccept IN (" "))
            SET dipmm_upd_dminfo_ind = 1
           ENDIF
          ELSE
           SET dipmm_upd_dminfo_ind = 1
          ENDIF
         ELSE
          SET message = nowindow
          SET dipmm_upd_dminfo_ind = 1
          SET dm_err->eproc = "Obtaining Current Monitoring Status"
          IF (drr_get_process_status("DM2_INSTALL_MONITOR",dipm_misc_data->install_plan_id,
           dipmm_cur_planmon_status)=0)
           GO TO exit_program
          ENDIF
          IF ((drr_flex_sched->status=1)
           AND dipmm_cur_planmon_status=0
           AND (dipm_misc_data->cur_dpe_status IN (value(dpl_executing), value(dpl_paused))))
           ...
           SELECT INTO "nl:"
            ...
            (SELECT
             ...
             WHERE di.info_domain="DATA MANAGEMENT"
              AND di.info_name="DM_ENV_ID"))
            DETAIL
             dipmm_pwd = substring((findstring("/",d.v500_connect_string,1,0)+ 1),((findstring("@",d
               .v500_connect_string,1,0) - findstring("/",d.v500_connect_string,1,0)) - 1),d
              .v500_connect_string), dipmm_connect_str = substring((findstring("@",d
               .v500_connect_string,1,0)+ 1),((textlen(trim(d.v500_connect_string)) - findstring("@",
               d.v500_connect_string,1,0))+ 1),d.v500_connect_string)
            WITH nocounter
           ;end select
           IF (check_error(dm_err->eproc)=1)
            CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
            GO TO exit_program
           ENDIF
           IF (((curqual=0) OR (((dipmm_pwd="") OR (dipmm_connect_str="")) )) )
            SET dm_err->err_ind = 1
            SET dm_err->emsg = "Unable to obtain connection information."
            CALL dipr_disp_error_msg(null)
            GO TO exit_program
           ENDIF
           IF (drr_submit_background_process(dipmm_uname,dipmm_pwd,dipmm_connect_str,dir_batch_queue,
            dpl_install_monitor,
            dipm_misc_data->install_plan_id,dipm_misc_data->cur_mode)=0)
            GO TO exit_program
           ENDIF
          ENDIF
         ENDIF
         IF (dipmm_upd_dminfo_ind=1)
          IF (dipmm_mon_not_set=1)
           SET dm_err->eproc = "Inserting Monitoring Settings into dm_info"
           IF ((dm_err->debug_flag > 0))
            CALL dipr_disp_error_msg(null)
           ENDIF
           INSERT  FROM dm_info di
            SET di.info_domain = "DM2_INSTALL_MONITOR:PLANSETTINGS", di.info_name = drr_flex_sched->
             pkg_number, di.info_number = dipmm_planmon_status,
             di.info_long_id = dipmm_planmon_interval
            WITH nocounter
           ;end insert
           IF (check_error(dm_err->eproc)=1)
            ROLLBACK
           ...
           ENDIF
          ELSE
           SET dm_err->eproc = "Updating Monitoring Settings into dm_info"
           IF ((dm_err->debug_flag > 0))
            CALL dipr_disp_error_msg(null)
           ENDIF
           UPDATE  FROM dm_info di
            SET di.info_number = dipmm_planmon_status, di.info_long_id = dipmm_planmon_interval
            WHERE di.info_domain="DM2_INSTALL_MONITOR:PLANSETTINGS"
             AND (di.info_name=drr_flex_sched->pkg_number)
            WITH nocounter
           ;end update
     ...
     ENDWHILE
    OF "A":
     EXECUTE dm2_install_plan_menu_arch dipmm_planmon_status
     IF ((dm_err->err_ind=1))
      GO TO exit_program
     ENDIF
   ENDCASE
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "LEAVING dm2_install_plan_menu_mon"
 CALL final_disp_msg(dipmm_logfile_prefix)
END GO
