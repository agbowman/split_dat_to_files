CREATE PROGRAM cp_get_concept_json:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Concept Code" = 0.0,
  "Intention List" = ""
  WITH outdev, inputconceptcode, inputintentionlist
 ...
 FREE RECORD concept_request
 RECORD concept_request(
   1 concept_cds[*]
     2 concept_cd = f8
     2 intention_list[*]
       3 intention_cd = f8
 ...
 CALL getparametervalues(3,intention_cds)
 SET stat = alterlist(concept_request->concept_cds,1)
 SET concept_request->concept_cds[1].concept_cd =  $INPUTCONCEPTCODE
 IF ((intention_cds->cnt > 0))
  SET stat = alterlist(concept_request->concept_cds[1].intention_list,intention_cds->cnt)
  FOR (x = 1 TO intention_cds->cnt)
    SET concept_request->concept_cds[1].intention_list[x].intention_cd = intention_cds->qual[x].value
  ENDFOR
 ENDIF
 EXECUTE cp_get_concept  WITH replace("REQUEST","CONCEPT_REQUEST")
#exit_script
 CALL echo("sending json to front")
 CALL putjsonrecordtofile(reply)
 IF (validate(debug_ind,1)=1)
  CALL echorecord(intention_cds)
  CALL echorecord(concept_request)
