CREATE PROGRAM dm2_create_noanalyze_trig:dba
 ...
 DECLARE dcat_trigger_enabled = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_create_noanalyze",".log","DM2_CREATE_NOANALYZE_TRIG LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_create_noanalyze_trig."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET readme_data->status = "F"
 SET readme_data->message = "**** FAILED - Starting script dm2_create_noanalyze_trig ****"
 IF (currdb != "ORACLE")
 ...
 IF ((dm2_rdbms_version->level1 < 9))
  ...
  "Auto-Success.  Readme should not run unless in running Oracle 9 or higher"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining initial status for dm2_noanalyze trigger from user_triggers."
 SET readme_data->message = dm_err->eproc
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM user_triggers ut
  WHERE ut.trigger_name="DM2_NOANALYZE"
   AND ut.status="ENABLED"
  DETAIL
   dcat_trigger_enabled = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql "cer_install:dm2_noanalyze_trigger.sql"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = "**** FAILED - Trigger dm2_noanalyze not created"
  SET dm_err->err_ind = 1
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql_chk "dm2_noanalyze", "trigger"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = "**** FAILED - Trigger dm2_noanalyze does not exist"
  SET dm_err->err_ind = 1
  GO TO exit_script
 ENDIF
 IF (dcat_trigger_enabled=0)
  IF (dm2_push_cmd("RDB ALTER TRIGGER DM2_NOANALYZE DISABLE GO",1)=0)
   SET readme_data->status = "F"
   SET readme_data->message = "Alter trigger failed on dm2_noanalyze"
   SET dm_err->err_ind = 1
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Determining final status for dm2_noanalyze trigger from user_triggers."
 SET readme_data->message = dm_err->eproc
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM user_triggers ut
  WHERE ut.trigger_name="DM2_NOANALYZE"
   AND trim(ut.triggering_event,3)="ANALYZE"
   AND ut.trigger_type="BEFORE EVENT"
   AND ut.status=evaluate(dcat_trigger_enabled,0,"DISABLED","ENABLED")
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual > 0)
  SET readme_data->status = "S"
  SET readme_data->message = "SUCCESS - Verification of trigger dm2_noanalyze successful "
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = "FAILED - Verification of trigger dm2_noanalyze failed"
  SET dm_err->err_ind = 1
  GO TO exit_script
 ENDIF
#exit_script
 ...
 ELSE
  ...
  CALL echo("##################################################")
 ENDIF
 SET dm_err->eproc = "Ending dm2_create_noanalyze_trig."
 CALL final_disp_msg("dm2_create_noanalyze")
END GO
