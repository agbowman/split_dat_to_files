CREATE PROGRAM bed_get_ens_tasks_form:dba
 FREE SET reply
 RECORD reply(
   1 status_data
     2 status_list[*]
 ...
 SET stat = alterlist(pvrequest->pvlist,3)
 SET stat = alterlist(pvreply->pvlist,3)
 ...
 SET dummy_parm2 = 0
 IF ((request->action="0"))
  SET formbr_auth_exists = 0
  CALL complete_formbr_vp(dummy_parm1)
  ...
  IF ((vpreply->vplist[1].view_prefs_id > 0))
   CALL complete_formbr_vp_nvp(dummy_parm1)
   ...
   IF ((nvpreply->nvplist[1].name_value_prefs_id > 0)
    ...
    CALL complete_formbr_vcp(dummy_parm1)
    ...
    IF ((vcpreply->vcplist[1].view_comp_prefs_id > 0))
     CALL complete_formbr_vcp_nvp(dummy_parm1)
     ...
     IF ((nvpreply->nvplist[1].name_value_prefs_id > 0)
      ...
      SET formbr_auth_exists = 1
     ENDIF
    ENDIF
   ENDIF
  ENDIF
  SET adhoc_auth_exists = 0
  CALL complete_adhoc_vp(dummy_parm1)
  SET vprequest->vplist[1].action_flag = "0"
  SET trace = recpersist
  EXECUTE bed_get_ens_view_prefs  WITH replace("REQUEST",vprequest), replace("REPLY",vpreply)
  IF ((vpreply->vplist[1].view_prefs_id > 0))
   CALL complete_adhoc_vp_nvp(dummy_parm1)
   SET nvprequest->nvplist[1].parent_entity_id = vpreply->vplist[1].view_prefs_id
   SET nvprequest->nvplist[2].parent_entity_id = vpreply->vplist[1].view_prefs_id
   SET nvprequest->nvplist[1].action_flag = "0"
   SET nvprequest->nvplist[2].action_flag = "0"
   ...
   IF ((nvpreply->nvplist[1].name_value_prefs_id > 0)
    AND (nvpreply->nvplist[2].name_value_prefs_id > 0))
    CALL complete_adhoc_vcp(dummy_parm1)
    SET vcprequest->vcplist[1].action_flag = "0"
    SET trace = recpersist
    EXECUTE bed_get_ens_view_comp_prefs  WITH replace("REQUEST",vcprequest), replace("REPLY",vcpreply
     )
    IF ((vcpreply->vcplist[1].view_comp_prefs_id > 0))
     CALL complete_adhoc_vcp_nvp(dummy_parm1)
     SET nvprequest->nvplist[1].parent_entity_id = vcpreply->vcplist[1].view_comp_prefs_id
     SET nvprequest->nvplist[2].parent_entity_id = vcpreply->vcplist[1].view_comp_prefs_id
     SET nvprequest->nvplist[3].parent_entity_id = vcpreply->vcplist[1].view_comp_prefs_id
     SET nvprequest->nvplist[4].parent_entity_id = vcpreply->vcplist[1].view_comp_prefs_id
  ...
  IF ((dpreply->dplist[1].detail_prefs_id > 0))
   SELECT INTO "NL:"
    ...
    WHERE nvp.parent_entity_name="DETAIL_PREFS"
     ...
     AND nvp.pvc_name="COMMAND_33461"
  ...
  SET docmodunc_priv_allowed = 0
  SET sign_priv_allowed = 0
  SET pvrequest->pvlist[1].priv_cdf_meaning = "VIEWPOWERFRM"
  SET pvrequest->pvlist[2].priv_cdf_meaning = "DOCMODIFYUNC"
  SET pvrequest->pvlist[3].priv_cdf_meaning = "SIGNPOWERFRM"
  ...
  IF ((pvreply->pvlist[2].privilege_id=0))
   SET docmodunc_priv_allowed = 1
  ENDIF
  IF ((pvreply->pvlist[3].privilege_id=0))
   SET sign_priv_allowed = 1
  ENDIF
  ...
  IF (formbr_auth_exists=1
   AND adhoc_auth_exists=1
   AND menu_auth_value="1"
   AND view_priv_allowed=1
   AND docmodunc_priv_allowed=1
   AND sign_priv_allowed=1)
 ...
 ELSEIF ((request->action="2"))
  SET viewform_off_on_ind = request->task_list[1].on_off_ind
  SET saveform_off_on_ind = request->task_list[2].on_off_ind
  SET signform_off_on_ind = request->task_list[3].on_off_ind
  IF (((viewform_off_on_ind=1) OR (((saveform_off_on_ind=1) OR (signform_off_on_ind=1)) )) )
   CALL complete_formbr_vp(dummy_parm1)
   SET vprequest->vplist[1].action_flag = "0"
   SET trace = recpersist
   EXECUTE bed_get_ens_view_prefs  WITH replace("REQUEST",vprequest), replace("REPLY",vpreply)
   ...
   IF ((vpreply->vplist[1].view_prefs_id > 0))
    ...
    ELSEIF ((nvpreply->nvplist[2].name_value_prefs_id=0))
     ...
     SET nvprequest->nvplist[1].pvc_value = "12"
     SET nvprequest->nvplist[1].action_flag = "1"
     SET trace = recpersist
     EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
      nvpreply)
   ...
   IF ((vcpreply->vcplist[1].view_comp_prefs_id > 0))
    ...
    ELSE
     ...
     IF (hold_nvp5=0)
      ...
      SET nvprequest->nvplist[1].pvc_value = "33248"
      SET trace = recpersist
      EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
       nvpreply)
     ENDIF
  ...
  ELSE
   ...
   ENDIF
  ENDIF
  IF (((saveform_off_on_ind=1) OR (signform_off_on_ind=1)) )
   CALL complete_adhoc_vp(dummy_parm1)
   SET vprequest->vplist[1].action_flag = "0"
   SET trace = recpersist
   EXECUTE bed_get_ens_view_prefs  WITH replace("REQUEST",vprequest), replace("REPLY",vpreply)
   ...
   IF ((vcpreply->vcplist[1].view_comp_prefs_id > 0))
    ...
    ELSE
     ...
     IF (hold_nvp5=0)
      SET nvprequest->nvplist[1].pvc_name = "COMMAND_ID"
      SET nvprequest->nvplist[1].pvc_value = "33250"
      SET trace = recpersist
      EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
       nvpreply)
     ENDIF
  ...
  SET nvprequest->nvplist[1].pvc_name = "COMMAND_33461"
  IF (((saveform_off_on_ind=1) OR (signform_off_on_ind=1)) )
   IF (nvp_exists > 0)
  ...
  IF (signform_off_on_ind=1)
   IF (view_priv_allowed=0)
    ...
    SET view_priv_allowed = 1
   ENDIF
   IF (docmodunc_priv_allowed=0)
    SET pvrequest->pvlist[1].priv_cdf_meaning = "DOCMODIFYUNC"
    SET pvrequest->pvlist[1].action_flag = "3"
    SET trace = recpersist
    EXECUTE bed_get_ens_privilege  WITH replace("REQUEST",pvrequest), replace("REPLY",pvreply)
    SET docmodunc_priv_allowed = 1
   ENDIF
   IF (sign_priv_allowed=0)
    SET pvrequest->pvlist[1].priv_cdf_meaning = "SIGNPOWERFRM"
    SET pvrequest->pvlist[1].action_flag = "3"
    SET trace = recpersist
    EXECUTE bed_get_ens_privilege  WITH replace("REQUEST",pvrequest), replace("REPLY",pvreply)
    SET sign_priv_allowed = 1
   ENDIF
  ELSEIF (saveform_off_on_ind=1)
   IF (view_priv_allowed=0)
    SET pvrequest->pvlist[1].priv_cdf_meaning = "VIEWPOWERFRM"
    SET pvrequest->pvlist[1].action_flag = "3"
    SET trace = recpersist
 ...
 ENDIF
 GO TO exitscript
 SUBROUTINE complete_adhoc_vp(dummy_parm2)
   ...
   SET vprequest->vplist[1].view_name = "ADHOCCHART"
   SET vprequest->vplist[1].view_seq = 0
 END ;Subroutine
 SUBROUTINE complete_adhoc_vp_nvp(dummy_parm2)
   ...
   SET nvprequest->nvplist[1].pvc_value = "Ad Hoc Charting"
   SET nvprequest->nvplist[2].parent_entity_name = "VIEW_PREFS"
   SET nvprequest->nvplist[2].pvc_name = "DISPLAY_SEQ"
   SET nvprequest->nvplist[2].pvc_value = "1"
 END ;Subroutine
 SUBROUTINE complete_adhoc_vcp(dummy_parm2)
   ...
   SET vcprequest->vcplist[1].view_name = "ADHOCCHART"
   SET vcprequest->vcplist[1].view_seq = 0
   SET vcprequest->vcplist[1].comp_name = "ADHOCCHART"
   SET vcprequest->vcplist[1].comp_seq = 0
 END ;Subroutine
 SUBROUTINE complete_adhoc_vcp_nvp(dummy_parm2)
   ...
   SET nvprequest->nvplist[1].pvc_value = "PVAdHocCharting.dll"
   ...
   SET nvprequest->nvplist[5].pvc_value = "33250"
 END ;Subroutine
 SUBROUTINE complete_formbr_vp(dummy_parm2)
   SET stat = alterlist(vprequest->vplist,1)
   SET stat = alterlist(vpreply->vplist,1)
   SET vprequest->vplist[1].application_number = request->application_number
   SET vprequest->vplist[1].position_cd = request->position_cd
   SET vprequest->vplist[1].prsnl_id = request->prsnl_id
   SET vprequest->vplist[1].frame_type = "CHART"
   SET vprequest->vplist[1].view_name = "FORMBROWSER"
   SET vprequest->vplist[1].view_seq = 0
 END ;Subroutine
 SUBROUTINE complete_formbr_vp_nvp(dummy_parm2)
   SET stat = alterlist(nvprequest->nvplist,2)
   SET stat = alterlist(nvpreply->nvplist,2)
   SET nvprequest->nvplist[1].parent_entity_name = "VIEW_PREFS"
   SET nvprequest->nvplist[1].pvc_name = "VIEW_CAPTION"
   SET nvprequest->nvplist[1].pvc_value = "Form Browser"
   SET nvprequest->nvplist[2].parent_entity_name = "VIEW_PREFS"
   SET nvprequest->nvplist[2].pvc_name = "DISPLAY_SEQ"
   SET nvprequest->nvplist[2].pvc_value = "12"
 END ;Subroutine
 SUBROUTINE complete_formbr_vcp(dummy_parm2)
   SET stat = alterlist(vcprequest->vcplist,1)
   SET stat = alterlist(vcpreply->vcplist,1)
   SET vcprequest->vcplist[1].application_number = request->application_number
   SET vcprequest->vcplist[1].position_cd = request->position_cd
   SET vcprequest->vcplist[1].prsnl_id = request->prsnl_id
   SET vcprequest->vcplist[1].view_name = "FORMBROWSER"
   SET vcprequest->vcplist[1].view_seq = 0
   SET vcprequest->vcplist[1].comp_name = "FORMBROWSER"
   SET vcprequest->vcplist[1].comp_seq = 0
 END ;Subroutine
 SUBROUTINE complete_formbr_vcp_nvp(dummy_parm2)
   SET stat = alterlist(nvprequest->nvplist,5)
   SET stat = alterlist(nvpreply->nvplist,5)
   SET nvprequest->nvplist[1].parent_entity_name = "VIEW_COMP_PREFS"
   SET nvprequest->nvplist[1].pvc_name = "COMP_DLLNAME"
   SET nvprequest->nvplist[1].pvc_value = "PVFormBrowser.dll"
   SET nvprequest->nvplist[2].parent_entity_name = "VIEW_COMP_PREFS"
   SET nvprequest->nvplist[2].pvc_name = "COMP_TYPE"
   SET nvprequest->nvplist[2].pvc_value = "0"
   SET nvprequest->nvplist[3].parent_entity_name = "VIEW_COMP_PREFS"
   ...
   SET nvprequest->nvplist[5].parent_entity_name = "VIEW_COMP_PREFS"
   SET nvprequest->nvplist[5].pvc_name = "COMMAND_ID"
   SET nvprequest->nvplist[5].pvc_value = "33248"
 END ;Subroutine
#exitscript
END GO
