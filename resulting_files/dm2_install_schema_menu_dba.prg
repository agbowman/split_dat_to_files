CREATE PROGRAM dm2_install_schema_menu:dba
 ...
 SUBROUTINE dctx_restore_prev_contexts(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dm2_file_prefix = vc WITH protect, noconstant("  -  -    ")
 DECLARE dm2_dbase_name = vc WITH protect, noconstant(" ")
 DECLARE ism_process = vc WITH protect, noconstant(" ")
 DECLARE ism_file_prefix = vc WITH protect, noconstant(" ")
 DECLARE ism_logfile_prefix = vc WITH protect, noconstant(" ")
 DECLARE ism_quit_process_ind = i2 WITH protect, noconstant(0)
 DECLARE dism_date_str2 = vc WITH protect, noconstant(" ")
 DECLARE dism_connect_str = vc WITH protect, noconstant(" ")
 DECLARE dism_stats_daysold = i2 WITH protect, constant(30)
 DECLARE dm2_force_connect_string = i2 WITH public, noconstant(0)
 DECLARE get_list_of_files(null) = i2
 ...
 CALL clear(1,1)
 CALL box(1,1,5,132)
 CALL text(3,54,"***  DM2 INSTALL SCHEMA MENU ***")
 CALL text(7,8,"This program is used to install/upgrade schema.  Options are:")
 CALL text(9,8,"1) Create ADMIN database (inactive)")
 CALL text(10,8,"2) Create non-production database (inactive) ")
 CALL text(11,8,"3) Create production database (inactive)")
 CALL text(12,8,
  "4) Inactive - Launch from DM2_DOMAIN_MAINT->Replicate/Refresh a Domain->Create a Copy of a Clinical Database"
  )
 CALL text(13,8,"5) Upgrade ADMIN database (inactive) ")
 CALL text(14,8,"6) Upgrade clinical database")
 CALL text(15,8,"7) Upgrade clinical database, with no reports (inactive) ")
 CALL text(16,8,"8) View schema differences ")
 CALL text(17,8,"9) View Tablespace needs, schema differences and schema estimates")
 CALL text(18,7,"10) View Installation Reports ")
 CALL text(20,8,">>> Please choose a number from the menu above (99 to exit): ")
 SET fini = 0
 WHILE (fini=0)
   CALL accept(20,69,"9(2)")
   SET menu_mode = curaccept
   IF (menu_mode=99)
    SET ism_quit_process_ind = 1
    GO TO exit_program
   ELSE
    IF ( NOT (menu_mode IN (6, 8, 9, 10)))
     CALL clear(20,8,124)
     CALL text(20,8,concat("Invalid option.  Please enter a valid (active) option:"))
    ELSE
     SET fini = 1
    ENDIF
   ENDIF
 ENDWHILE
 IF (menu_mode != 10)
  IF (get_list_of_files(null)=0)
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Getting help list of schema files to select from."
   CALL disp_msg("Error encountered trying to get a list of schema files",dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 CALL clear(1,1)
 CASE (menu_mode)
  OF 6:
   CALL text(2,3,"Upgrade clinical database...",w)
  OF 8:
   CALL text(2,3,"View schema differences...",w)
  OF 9:
   CALL text(2,3,"View Tablespace needs and schema differences...",w)
  OF 10:
   CALL text(2,3,"View Installation Reports...",w)
 ENDCASE
 IF (menu_mode=10)
  SET dm2rpting->prompt_mode = "PROMPT"
  SET dm2comprom->install_mode = "DM2NOTSET"
  ...
  GO TO exit_program
 ENDIF
 CALL text(6,8,">>> Enter schema date (list in mm-dd-yyyy format) :  ")
 CALL text(8,8,">>> Enter 'C' to continue or 'Q' to quit (C or Q) :")
 SET help =
 SELECT INTO "nl:"
  format(substring((findstring("dm2",cnvtlower(r.line),1,1)+ 4),8,trim(r.line)),"##-##-####")
  FROM rtlt r
  WHERE findstring("dm2",cnvtlower(r.line)) > 0
  WITH nocounter
 ;end select
 SET validate =
 SELECT INTO "nl:"
  r.line
  FROM rtlt r
  WHERE r.line=curaccept
  WITH nocounter
 ;end select
 SET validate = 2
 CALL accept(6,65,"p(10);CSF")
 SET validate = off
 SET dm2_file_prefix = curaccept
 SET ism_file_prefix = cnvtalphanum(dm2_file_prefix)
 CALL accept(8,65,"A;cu","C"
  WHERE curaccept IN ("Q", "C"))
 ...
 ENDIF
 SET message = nowindow
 SET ism_logfile_prefix = build("dm2c",ism_file_prefix,"_")
 SET dm_err->logfile = "NONE"
 IF (check_logfile(ism_logfile_prefix,".log","DM2_INSTALL_SCHEMA_MENU LOGFILE")=0)
  GO TO exit_program
 ENDIF
 ...
 ELSE
  ...
  SET dm_err->user_action = concat("Please connect to clinical database as valid user. ")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 ...
 IF ((dm_err->debug_flag > 0))
  CALL echo(dm_err->asterisk_line)
  CALL echo(build("ism::reqinfo->updt_task=",reqinfo->updt_task))
  CALL echo(dm_err->asterisk_line)
 ENDIF
 SET dm2_install_schema->logfile_prefix = substring(1,(findstring(".",dm_err->logfile) - 1),dm_err->
  logfile)
 SET dm2_dbase_name = currdbname
 SET dm2_install_schema->menu_driver = "dm2_install_schema_menu"
 SET dm2_install_schema->file_prefix = ism_file_prefix
 SET dm2_install_schema->schema_prefix = "dm2c"
 CASE (menu_mode)
  OF 6:
 ...
 ENDCASE
 SET dm2_install_schema->dbase_name = dm2_dbase_name
 ...
 EXECUTE dm2_dbstats_chk_rpt "*", value(dism_stats_daysold)
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 ...
 "NONE"
 GO TO exit_program
 SUBROUTINE get_list_of_files(null)
   DECLARE str2 = vc WITH protect
   SET message = nowindow
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET str2 = concat("dir/version=1/columns=1 cer_install:dm2c*_h.dat ")
    IF (dm2_push_dcl(value(str2))=0)
     RETURN(0)
    ENDIF
   ELSEIF ((dm2_sys_misc->cur_os="WIN"))
    SET str2 = concat("dir ",dm2_install_schema->cer_install,"\dm2c*_h.dat /B")
    IF (dm2_push_dcl(value(str2))=0)
     RETURN(0)
    ENDIF
   ELSE
    SET str2 = 'find $cer_install -name "dm2c*_h.dat" -print'
    IF (dm2_push_dcl(value(str2))=0)
     RETURN(0)
    ENDIF
   ENDIF
   ...
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   IF (((check_error(dm_err->eproc)=1) OR (curqual=0)) )
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Getting help list of schema files to select from."
    SET message = nowindow
    CALL disp_msg("No schema files found. Process Terminated!",dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET message = window
   RETURN(1)
...
#exit_program
 ...
 IF ((dm_err->debug_flag > 1))
  ...
  ;end select
 ENDIF
 IF (ism_quit_process_ind=1)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "User quit install schema process.  Process Terminated!"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 ENDIF
 IF (menu_mode != 10)
  CALL parser("free define oraclesystem go",1)
  IF (menu_mode IN (6, 7, 8, 9))
  ...
  IF ((dm2_install_schema->p_word != "NONE"))
   IF ((dm2_install_schema->connect_str > " "))
    SET dism_connect_str = build(dm2_install_schema->u_name,"/",dm2_install_schema->p_word,"@",
     dm2_install_schema->connect_str)
   ELSE
    SET dism_connect_str = build(dm2_install_schema->u_name,"/",dm2_install_schema->p_word)
   ENDIF
   CALL parser(concat("define oraclesystem '",trim(dism_connect_str),"' go"),1)
  ELSE
   CALL echo("*")
   CALL echo("**************************************************************************")
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "DM2_INSTALL_SCHEMA_MENU COMPLETED SUCCESSFULLY"
 CALL final_disp_msg(ism_logfile_prefix)
END GO
