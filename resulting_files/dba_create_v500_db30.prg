CREATE PROGRAM dba_create_v500_db30
 SET message = window
 ...
 SUBROUTINE aix_crt_raw_device(x)
   ...
   SELECT INTO concat("/tmp/","v500_create_",cnvtlower(dbname),".ksh")
    ...
    DETAIL
     ...
     IF (cnt <= control_cnt)
      ...
      row + 1, temp_vgname = cnvtlower(trim(b.volume_group)), col 1,
      "mklv -y '",
      CALL print(trim(a.file_name)), "' -t 'raw' ",
      CALL print(temp_vgname), " ",
      CALL print(trim(cnvtstring(t_size))),
      " ",
      CALL print(trim(a.disk_name)), row + 1,
      col 1, "if [ $? != 0 ] ", row + 1,
      col 1, "then", row + 1,
      col 1, " echo ",
      CALL print('"'),
      ...
      CALL print('"'), row + 1,
      col 1, " export returned_status=1", row + 1,
      col 1, "else", row + 1,
      col 1, " chmod 600 /dev/r",
      CALL print(trim(a.file_name)),
      row + 1, col 1, " chown oracle.dba /dev/r",
   ...
   SELECT INTO concat("/tmp/","v500_create_",cnvtlower(dbname),".ksh")
    ...
    FOOT REPORT
     ...
     CALL print('"'), row + 1, col 1,
     'echo "Update Cerner Registry ..."', row + 1, col 1,
     ". /tmp/v500_crt_",
     CALL print(cnvtlower(dbname)), "_81.ksh",
     row + 1, col 1, "fi"
    WITH nocounter, format = stream, append,
     noheading, formfeed = none, maxrow = 1
   ;end select
