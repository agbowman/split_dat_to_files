CREATE PROGRAM bbt_get_person_antibodies:dba
 RECORD reply(
   1 all_antibodies[*]
     2 active_ind = i2
     2 person_antibody_id = f8
     2 person_antibody_disp = vc
     2 added_by = vc
     2 date_added = dq8
     2 removed_by = vc
     2 date_removed = dq8
 ...
 DECLARE result_id = f8 WITH protected, noconstant(0.0)
 DECLARE antibody_cnt = i4 WITH protected, noconstant(0)
 DECLARE persantibodyindex = i4 WITH protected, noconstant(0)
 DECLARE pos = i4 WITH protected, noconstant(0)
 DECLARE historic = vc WITH public
 ...
 SET historic = uar_i18ngetmessage(i18nhandle,"historical","Historical")
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY pa.person_antibody_id, pa.updt_dt_tm
  HEAD REPORT
   antibody_cnt = 0, stat = alterlist(reply->all_antibodies,(antibody_cnt+ 10))
  HEAD pa.person_antibody_id
   row + 0
  DETAIL
   IF (pa.contributor_system_cd > 0)
    pos = locateval(persantibodyindex,1,size(reply->all_antibodies,5),pa.person_antibody_id,reply->
     all_antibodies[persantibodyindex].person_antibody_id)
    IF (pos <= 0
     AND pa.active_status_cd != combined_cd
     AND pa.active_status_cd != inactive_cd)
     antibody_cnt = (antibody_cnt+ 1)
     IF (antibody_cnt > size(reply->all_antibodies,5))
      stat = alterlist(reply->all_antibodies,(antibody_cnt+ 10))
     ENDIF
     reply->all_antibodies[antibody_cnt].active_ind = pa.active_ind, reply->all_antibodies[
     antibody_cnt].person_antibody_id = pa.person_antibody_id, reply->all_antibodies[antibody_cnt].
     person_antibody_disp = uar_get_code_display(pa.antibody_cd),
     reply->all_antibodies[antibody_cnt].added_by = historic, reply->all_antibodies[antibody_cnt].
     updt_cnt = pa.updt_cnt
     IF (pa.active_ind=0)
      reply->all_antibodies[antibody_cnt].removed_by = prsnl2.username, reply->all_antibodies[
      antibody_cnt].date_removed = cnvtdatetime(pa.removed_dt_tm), reply->all_antibodies[antibody_cnt
      ].removal_reason = uar_get_code_display(pa.removal_reason_cd),
      reply->all_antibodies[antibody_cnt].removal_notes = pa.removal_notes
     ENDIF
    ENDIF
   ELSE
    IF (pa.active_ind=1
    ...
    .result_status_cd=corrected_cd)) )) )
     ...
     antibody_cnt].person_antibody_id = pa.person_antibody_id, reply->all_antibodies[antibody_cnt].
     person_antibody_disp = uar_get_code_display(pa.antibody_cd),
     reply->all_antibodies[antibody_cnt].added_by = prsnl1.username, reply->all_antibodies[
     antibody_cnt].date_added = cnvtdatetime(pr.perform_dt_tm), reply->all_antibodies[antibody_cnt].
     updt_cnt = pa.updt_cnt
    ENDIF
    IF (pa.active_ind=0
     AND pa.active_status_cd != combined_cd
    ...
    IF (pa.active_ind=0
     ...
     reply->all_antibodies[antibody_cnt].removed_by = prsnl2.username, reply->all_antibodies[
     antibody_cnt].date_removed = cnvtdatetime(pa.removed_dt_tm), reply->all_antibodies[antibody_cnt]
     .removal_notes = removed_res_correction
    ENDIF
   ENDIF
  FOOT  pa.person_antibody_id
   row + 0
  FOOT REPORT
   stat = alterlist(reply->all_antibodies,antibody_cnt)
