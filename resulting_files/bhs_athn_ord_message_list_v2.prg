CREATE PROGRAM bhs_athn_ord_message_list_v2
 RECORD orequest(
   1 anchor_dt_tm = dq8
   1 prsnl_knt = i4
   1 prsnl[*]
 ...
 DECLARE pg_name = vc
 IF (( $3=1))
  SET orequest->prsnl_knt = 1
  SET stat = alterlist(orequest->prsnl,1)
  SET orequest->prsnl[1].prsnl_id =  $2
  SET orequest->prsnl[1].flag_knt = 5
  SET stat = alterlist(orequest->prsnl[1].flag,5)
  SET orequest->prsnl[1].flag[1].flag_type = 1
  SET orequest->prsnl[1].flag[2].flag_type = 2
  SET orequest->prsnl[1].flag[3].flag_type = 3
  SET orequest->prsnl[1].flag[4].flag_type = 4
  SET orequest->prsnl[1].flag[5].flag_type = 5
 ENDIF
 IF (( $3=2))
  SELECT INTO "nl:"
   FROM proxy p
...
#pools
 ...
 ;end select
 SELECT INTO "nl:"
  FROM msg_category mc
  PLAN (mc
   WHERE (mc.prsnl_group_id= $2)
    AND mc.msg_category_type_cd=orders_cd)
  HEAD mc.msg_category_id
   prequest->configuration[1].msg_category_config_id = mc.msg_category_id
  WITH nocounter, time = 30
 ;end select
 SET date_line = substring(1,10, $4)
 SET time_line = substring(12,8, $4)
