CREATE PROGRAM bhs_eks_modified_labs
 ...
 SELECT INTO "nl:"
  FROM clinical_event ce,
   ...
   person p
  PLAN (ce
   WHERE ce.clinical_event_id=ce_id
  ...
  HEAD REPORT
   ...
   corrected_result = ""
  DETAIL
   eid = ce.encntr_id, oldresultdisp = concat(trim(uar_get_code_display(ce2.event_cd)),": ",trim(ce2
     .result_val)," ",trim(uar_get_code_display(ce2.result_units_cd)),
    " Resulted on ",format(ce2.clinsig_updt_dt_tm,"mm/dd/yyyy hh:mm;;q")), newresultdisp = concat(
    trim(uar_get_code_display(ce.event_cd)),": ",trim(ce.result_val)," ",trim(uar_get_code_display(ce
      .result_units_cd)),
 ...
 IF (curqual > 0)
  ...
  SET log_message = concat(
   ...
   " to a laboratory result for ",pat_name," who you are listed as the attending"," physician. The ",
   procedure," previously resulted as [",orig_result,"]"," on [",
   orig_date,"] for this patient was modified / updated to [",corrected_result,"]"," on [",
   corrected_date,"].",
  ...
  SET log_misc1 = concat(
   ...
   " or the attending physician or both. The ",
   procedure," previously resulted as [",orig_result,"]"," on [",
   orig_date,"] for this patient was modified / updated to [",corrected_result,"]"," on [",
   corrected_date,"].",
   " For additional question please contact Laboratory on-call manager (Beeper: 4-5227)")
 ELSE
  SET log_message = concat(build("cliical event ID:",ce_id))
 ENDIF
END GO
