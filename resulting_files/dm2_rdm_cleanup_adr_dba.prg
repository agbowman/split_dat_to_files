CREATE PROGRAM dm2_rdm_cleanup_adr:dba
 ...
 ENDIF
 DECLARE drca_logfile_name = vc WITH protect, noconstant("dm2cleanadr")
 DECLARE dyn_text = vc WITH protect, noconstant(" ")
 FREE RECORD tlist
 RECORD tlist(
   1 tcnt = i4
   1 trig[*]
 ...
 IF (check_logfile(drca_logfile_name,".log","DM2_RDM_CLEAN_ADR.PRG ...") != 1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "BEGINNING DM2_RDM_CLEAN_ADR.PRG ..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "NL:"
  FROM dm2_user_triggers dut
  WHERE ((dut.trigger_name="TRG*ZDEL*") OR (((dut.trigger_name="TRG_LONG_TEXT_DELETE*") OR (dut
  .trigger_name="TRG_RRF_DELETE*")) ))
  HEAD REPORT
   tlist->tcnt = 0
  DETAIL
   tlist->tcnt = (tlist->tcnt+ 1), stat = alterlist(tlist->trig,tlist->tcnt), tlist->trig[tlist->tcnt
   ].trigname = dut.trigger_name
  WITH nocounter
 ;end select
 FOR (ix1 = 1 TO tlist->tcnt)
   SET dyn_text = concat("RDB DROP TRIGGER ",tlist->trig[ix1].trigname," GO")
   SET dm_err->eproc = concat("Executing: ",dyn_text)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_push_cmd(dyn_text,1) != 1)
    SET dm_err->eproc = "Dropping ZDEL trigger"
    SET dm_err->err_ind = 1
    GO TO exit_program
   ENDIF
 ENDFOR
 ...
 IF ((dm_err->err_ind != 1))
  SET dm_err->debug_flag = 214
  EXECUTE dm2_undo_default_rows "*"
  SET dm_err->debug_flag = 1
 ENDIF
 EXECUTE dm2_add_default_rows "*"
#exit_program
 IF ((dm_err->err_ind=1))
  SET readme_data->status = "F"
  SET readme_data->message = concat(
   "An error occurred during the processing of a dm2_rdm_cleanup_adr.   See log file (",dm_err->
   logfile,") for details.")
 ELSE
  SET readme_data->status = "S"
  SET readme_data->message = "Default Row cleanup processing was successful."
 ENDIF
 EXECUTE dm_readme_status
 SET dm_err->eproc = "ENDING DM2_RDM_CLEANUP_ADR.PRG"
 CALL final_disp_msg(drca_logfile_name)
 FREE RECORD tlist
END GO
