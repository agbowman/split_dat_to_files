CREATE PROGRAM bhs_req_04:dba
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   orders->admit_dt = format(e.reg_dt_tm,"@SHORTDATETIME"), orders->dischg_dt = format(e.disch_dt_tm,
    "@SHORTDATETIME")
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   WHERE (o.order_id=request->order_qual[d1.seq].order_id)
    ...
    AND o.order_status_cd != inprocess)
   JOIN (oa
   ...
   (SELECT DISTINCT
    ...
    WHERE cve.code_set=14281
     AND cve.field_name="REQ_PRINT_FLG"
     AND substring(8,1,cve.field_value)="Y")))) )
   JOIN (pl
 ...
 FOR (x = 1 TO order_cnt)
   ...
   ENDIF
 ENDFOR
 IF ((request->order_qual[1].encntr_id=0.00))
  SELECT INTO "nl:"
   FROM (dummyt d1  WITH seq = value(size(request->order_qual,5))),
    order_detail od
   PLAN (d1)
    JOIN (od
    WHERE (od.order_id=request->order_qual[d1.seq].order_id)
     AND od.oe_field_id=12594.00)
   HEAD REPORT
    initial = 1
   HEAD od.oe_field_display_value
    IF (initial=1)
     orders->admit_diagnosis = trim(od.oe_field_display_value,3), initial = 0
    ELSE
     orders->admit_diagnosis = concat(orders->admit_diagnosis,", ",trim(od.oe_field_display_value,3))
    ENDIF
   WITH nocounter
  ;end select
 ELSE
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   pl_cnt += 1,
   CALL alterlist(m_rec->cd,pl_cnt), m_rec->cd[pl_cnt].f_cd = cv.code_value,
   m_rec->cd[pl_cnt].s_disp = trim(cv.display,3)
  WITH nocounter
 ;end select
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(ms_filename)
   ...
   PLAN (d1
    WHERE expand(ml_exp,1,size(m_rec->cd,5),orders->qual[d1.seq].f_catalog_cd,m_rec->cd[ml_exp].f_cd)
    )
   HEAD REPORT
  ...
  IF (curqual > 0)
   ...
   SET dclcom = concat('mail -s "',subject,'" ',"equipmentmanagement@baystatehealth.org < ",
    ms_filename)
  ...
  SELECT INTO value(request->printer_name)
   FROM (dummyt d1  WITH seq = value(size(orders->qual,5)))
   PLAN (d1
    WHERE  NOT (expand(ml_exp,1,size(m_rec->cd,5),orders->qual[d1.seq].f_catalog_cd,m_rec->cd[ml_exp]
     .f_cd)))
   HEAD REPORT
    CALL echo("printing this one"), first_page = "Y", spaces = fillstring(50," "),
    footer_start_pos = 0, temp_s = fillstring(1000,""), temp_s2 = fillstring(45,""),
    MACRO (line_wrap)
     limit = 0, cr = char(10)
     WHILE (tempstring > " "
      AND limit < 1000)
    ...
    , footer_idx = 0
   HEAD PAGE
    CALL echo("head page"), "{font/8}", row + 1,
    xrow = 70, xcol = 190
    IF (ml_print_draw_only=1)
     xcol = 150,
     CALL print(calcpos(xcol,xrow)),
     "{cpi/12}{B}PLEASE DRAW & SEND TESTING TO BAYSTATE MEDICAL CENTER"
    ENDIF
    xrow += 10, xcol = 190,
    CALL print(calcpos(1,xrow)),
    "{cpi/12}{B}", spos = (324 - cnvtint((cnvtint((size(orders->facility)/ 2)) * 12))),
    CALL print(calcpos(spos,xrow)),
    orders->facility, row + 1, xrow += 10,
    xcol = 250,
    CALL print(calcpos(xcol,xrow)), "{cpi/12}{B}Order Requisition",
    row + 1, xrow += 10, xcol = 44,
    "{cpi/12}", row + 1, row + 1,
    col 1,
    CALL print(calcpos(xcol,xrow)), "{box/85/1}",
    xrow += 10, xcol = 50,
    CALL echo("patient information"),
    CALL print(calcpos(xcol,xrow)), "{b/19}Patient Information", xrow += 25,
    xcol = 50, row + 1, col 1,
    ...
    CALL print(calcpos(xcol,xrow)), orders->mrn, xrow += 14,
    xcol = 50
    IF (temp_cd > 0)
     temp_loc_disp = uar_get_code_display(temp_cd), row + 1, col 1,
    ...
    xcol = 380,
    CALL print(calcpos(xcol,xrow)), "Trauma #:",
    xcol = 425,
    CALL print(calcpos(xcol,xrow)), orders->trauma,
    xrow += 14, xcol = 50, row + 1,
    col 1,
    ...
    "Weight:", xcol = 425,
    CALL print(calcpos(xcol,xrow)),
    orders->weight, xrow += 14, xcol = 50,
    xcol = 380,
    CALL print(calcpos(xcol,xrow)), orders->braden_score,
    xrow -= 14, xcol = 50, xrow += 20,
    xcol = 50, row + 1, col 1,
    ...
    xcol = 145, tempstring = orders->admit_diagnosis, maxlen = 75,
    line_wrap, xrow += 14, xcol = 50,
    xrow += 20, xcol = 44, row + 1,
   ...
   DETAIL
    ...
    IF (first_page="N")
     BREAK
    ENDIF
    first_page = "N", footer_start_pos = (750 - (90+ (15 * orders->qual[d1.seq].com_ln_cnt))), xrow
     += 25,
    xcol = 50, row + 1,
    CALL print(calcpos(xcol,xrow)),
    "Procedure: ", xcol = 110,
    CALL print(calcpos(xcol,xrow)),
    orders->qual[d1.seq].mnemonic, xcol = 380,
    CALL print(calcpos(xcol,xrow)),
    "Order Status:", xcol = 460,
    CALL print(calcpos(xcol,xrow)),
    orders->qual[d1.seq].status, xrow += 14, xcol = 50,
    row + 1,
    CALL print(calcpos(xcol,xrow)), "Order ID:",
    xcol = 90,
    CALL print(calcpos(xcol,xrow)), orders->qual[d1.seq].order_id,
    xcol = 50, xrow += 17, row + 1
    IF ((orders->qual[d1.seq].f_fut_facilty_cd > 0))
     CALL print(calcpos(xcol,xrow)), "Future Order Facility: ", orders->qual[d1.seq].s_fut_facilty,
     row + 1, xcol = 50, xrow += 11,
     CALL print(calcpos(xcol,xrow)), "Future Order Unit: ", orders->qual[d1.seq].s_fut_unit,
     row + 1, xcol = 50, xrow += 15
    ENDIF
    FOR (fsub = 1 TO 31)
      FOR (ww = 1 TO orders->qual[d1.seq].d_cnt)
        IF ((((orders->qual[d1.seq].d_qual[ww].group_seq=fsub)) OR (fsub=31
         ...
         IF ((orders->qual[d1.seq].d_qual[ww].value > " "))
          ...
          ww].value,
          xcol = 330
          IF ((orders->qual[d1.seq].d_qual[ww].label_text="Test(s) Requested*"))
           limit = 0, maxlen = textlen(tempstring), xpos = 0,
           line = 0, num_line = 0, tempstring = replace(tempstring," ","-",0),
           CALL echo(build("maxlen :",maxlen))
           WHILE (limit <= maxlen)
             limit += 1,
             CALL echo(build("limit :",limit))
             IF (((substring(limit,1,tempstring)=char(10)) OR (limit > maxlen)) )
              temp_s = replace(temp_s,"-"," ",0)
              IF (xpos > 45)
               num_line = (maxlen/ 45)
               FOR (line = 0 TO (num_line+ 1))
                temp_s2 = substring(((line * 45)+ 1),45,temp_s),
                IF (temp_s2 > "")
                 CALL echo(build("temp_s2 :",temp_s2)),
                 CALL echo(build("temp_s :",temp_s)),
                 CALL print(calcpos(xcol,xrow)),
                 temp_s2, row + 1, xrow += 12,
                 temp_s2 = fillstring(45,"")
                ENDIF
               ENDFOR
              ELSE
               CALL echo(build("temp_s :",temp_s)), temp_s2 = substring(1,45,temp_s),
               CALL print(calcpos(xcol,xrow)),
               temp_s2, row + 1, xrow += 12
              ENDIF
              temp_s = fillstring(1000,""), xpos = 0
             ELSE
              xpos += 1, c = substring(limit,1,tempstring),
              CALL echo(build(" c :",c)),
              temp_s = concat(trim(temp_s),trim(c))
             ENDIF
           ENDWHILE
          ELSE
           maxlen = 45, line_wrap
          ENDIF
          IF ((xrow > (footer_start_pos - 25)))
           footer_idx = d1.seq, BREAK, row + 1,
           xrow += 25
          ENDIF
         ENDIF
        ENDIF
    ...
    ENDFOR
   FOOT PAGE
    xrow = footer_start_pos, xcol = 50, row + 1,
    CALL print(calcpos(xcol,xrow)), "Ordering MD:", xcol = 134,
    CALL print(calcpos(xcol,xrow)), orders->qual[footer_idx].order_dr, xcol = 350,
    CALL print(calcpos(xcol,xrow)), "Order Date/Time:", xcol = 450,
    CALL print(calcpos(xcol,xrow)), orders->qual[footer_idx].signed_dt, xrow += 12,
    xcol = 50, row + 1,
    CALL print(calcpos(xcol,xrow)),
    "Order Type:", xcol = 134,
    CALL print(calcpos(xcol,xrow)),
    orders->qual[footer_idx].type, xrow += 12, xcol = 50,
    row + 1,
    CALL print(calcpos(xcol,xrow)), "Order entered by:",
    xcol = 134,
    CALL print(calcpos(xcol,xrow)), orders->qual[footer_idx].enter_by,
    xcol = 350,
    ...
    IF ((orders->qual[footer_idx].com_ln_cnt > 0))
     IF ((orders->qual[footer_idx].com_ln_cnt > 8))
      orders->qual[footer_idx].com_ln_cnt = 8
     ENDIF
     FOR (w = 1 TO orders->qual[footer_idx].com_ln_cnt)
       xrow += 15, xcol = 70, row + 1,
       CALL print(calcpos(xcol,xrow)), orders->qual[footer_idx].com_ln_qual[w].com_line
     ENDFOR
    ENDIF
    xrow += 30, xcol = 470,
    CALL print(calcpos(xcol,xrow)),
    "{cpi/14}", curprog, row + 1
