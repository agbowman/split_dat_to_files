CREATE PROGRAM bed_ens_cd_value:dba
 RECORD reply(
   1 curqual = i4
   1 qual[*]
     2 status = i2
 ...
 DECLARE ignoreduplicatecheck = i2 WITH protect, noconstant(0)
 DECLARE next_seq = f8 WITH protect, noconstant(0.0)
 DECLARE seq_string = vc WITH protect, noconstant(" ")
 DECLARE errorcode = i4 WITH protect, noconstant(0)
 SET reply->status_data.status = "F"
 SET stat = alterlist(reply->qual,req_cnt)
 SET reply->curqual = req_cnt
 IF (validate(request->ignore_duplicate_checks))
  SET ignoreduplicatecheck = request->ignore_duplicate_checks
 ENDIF
      ...
      IF (wherestr > " ")
       IF (trim(substring(1,12,request->cd_value_list[aidx].cdf_meaning)) > " ")
        wherestr = concat(wherestr," and cv.cdf_meaning = ",'"',request->cd_value_list[aidx].
         cdf_meaning,'"')
       ELSE
        IF (cvs.display_dup_ind=0
         AND cvs.display_key_dup_ind=0
         AND cvs.definition_dup_ind=0
         AND cvs.active_ind_dup_ind=0)
         failed = "T", stat = alterlist(reply->qual,aidx), reply->qual[aidx].status = 0,
         reply->error_msg = "CDF Meaning must be valid"
        ELSE
         wherestr = concat(wherestr," and cv.cdf_meaning = NULL")
        ENDIF
       ENDIF
      ELSE
       IF (trim(substring(1,12,request->cd_value_list[aidx].cdf_meaning)) > " ")
       ...
       ELSE
        IF (cvs.display_dup_ind=0
         ...
         reply->error_msg = "CDF Meaning must be valid"
        ELSE
         wherestr = concat(wherestr,"cv.cdf_meaning = NULL")
        ENDIF
       ENDIF
      ENDIF
     ENDIF
 ...
 SUBROUTINE update_cd_value(aidx)
   ...
   UPDATE  FROM code_value cv
    SET cv.active_dt_tm =
     ...
     ENDIF
     , cv.cdf_meaning =
     IF ((request->cd_value_list[aidx].cdf_meaning IN (" ", '""'))) cv.cdf_meaning
     ELSE trim(cnvtupper(substring(1,12,request->cd_value_list[aidx].cdf_meaning)),3)
     ENDIF
     ,
     cv.collation_seq =
     IF ((request->cd_value_list[aidx].collation_seq <= 0)) cv.collation_seq
     ELSE request->cd_value_list[aidx].collation_seq
     ENDIF
     , cv.concept_cki =
     IF ((request->cd_value_list[aidx].concept_cki IN (" ", '""'))) cv.concept_cki
     ELSE trim(request->cd_value_list[aidx].concept_cki,3)
     ENDIF
     , cv.cki =
     IF ((request->cd_value_list[aidx].cki IN (" ", '""'))) cv.cki
     ELSE trim(request->cd_value_list[aidx].cki,3)
     ENDIF
     ,
     cv.definition =
     IF ((request->cd_value_list[aidx].definition IN (" ", '""'))) cv.definition
     ELSE trim(request->cd_value_list[aidx].definition,3)
     ENDIF
     , cv.description =
     IF ((request->cd_value_list[aidx].description IN (" ", '""'))) cv.description
     ELSE trim(substring(1,60,request->cd_value_list[aidx].description),3)
     ENDIF
     , cv.display =
     IF ((request->cd_value_list[aidx].display IN (" ", '""'))) cv.display
     ELSE trim(substring(1,40,request->cd_value_list[aidx].display),3)
     ENDIF
     ,
     cv.display_key =
     IF (display_key IN (" ", '""')) cv.display_key
     ELSE display_key
     ENDIF
     , cv.end_effective_dt_tm =
 ...
 SUBROUTINE inactivate_cd_value(aidx)
   UPDATE  FROM code_value cv
    SET cv.active_ind = 0, cv.active_type_cd = inactive, cv.inactive_dt_tm = cnvtdatetime(curdate,
      curtime3),
     cv.end_effective_dt_tm = cnvtdatetime(curdate,curtime), cv.updt_applctx = reqinfo->updt_applctx,
