CREATE PROGRAM dm_bb_qc_group_act_rows:dba
 ...
 SET reply->table_name = "BB_QC_GROUP_ACTIVITY"
 ...
 FOR (ltokenidx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (lmonthstokeep < 120)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"INVALIDMNTH",
   "You must enter 120 or greater for the months to keep token.  You entered %1 or did not enter any value.",
   "i",lmonthstokeep)
  GO TO exit_script
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  WHERE parser(sbr_getrowidnotexists("bqga.scheduled_dt_tm+0 < cnvtdatetime(dPurgeDtTm)","bqga"))
   AND ((bqga.lock_prsnl_id+ 0)=0)
   AND ((bqga.group_id+ 0) > 0)
   AND ((bqga.group_activity_id+ 0) > 0)
  HEAD REPORT
   row_cnt = 0
  DETAIL
   row_cnt = (row_cnt+ 1)
   ...
   ENDIF
   reply->rows[row_cnt].row_id = bqga.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,row_cnt)
  WITH nocounter, maxqual(bqga,value(request->max_rows))
 ;end select
