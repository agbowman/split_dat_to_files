CREATE PROGRAM cv_frpt_avg_wait_proc
 PROMPT
 ...
 IF (validate(i18nuar_def,999)=999)
  ...
  persist
 ENDIF
 EXECUTE cv_frpt_avg_wait_drv "NL:",  $START_DATE,  $END_DATE,
  $ORG_ID
 ...
 SUBROUTINE get_reply_obj(dummy)
  ...
  SELECT
   ...
   cv_list_rpl_wait_minutes = reply_obj->cv_list[d1.seq].rpl_wait_minutes,
   ...
   HEAD REPORT
    _d0 = cv_day, sdate = format(cnvtdatetime( $START_DATE),"@SHORTDATE;;Q"), edate = format(
     cnvtdatetime( $END_DATE),"@SHORTDATE;;Q"),
    gtotal_avg = 0, total_cnt = 0, total_avg = 0,
    tday_avg = 0, p_curr = 0, p_max = 0,
    ...
    tproc_performed = 0, proc_performed = 0, tmonth_avg = 0,
    month_avg = 0, month_cnt = 0, tgraph = 0,
    max_tgraph = 0, graph_max = 0, _fenddetail = (rptreport->m_pageheight - rptreport->m_marginbottom
    ),
   ...
   HEAD cv_month
    ...
    day_avg = 0, day_cnt = 0, _fdrawheight = headmonthsection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
   ...
   DETAIL
    total_avg = (total_avg+ cv_list_rpl_wait_minutes), total_cnt = (total_cnt+ 1), day_cnt = (day_cnt
    + 1),
    day_avg = (day_avg+ cv_list_rpl_wait_minutes), proc_performed = (proc_performed+ 1), _fdrawheight
   ...
   FOOT  cv_day
    tday_avg = avg(cv_list_rpl_wait_minutes), tproc_performed = (tproc_performed+ proc_performed),
   ...
   FOOT  cv_month
    tmonth_avg = (day_avg/ day_cnt), tgraph = tmonth_avg, p_curr = tmonth_avg
    IF (p_curr >= p_max)
     p_max = p_curr
    ENDIF
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "HeadReportSection_FieldName0","Average Patient Wait Time by Procedures"),char(0)))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.063),(offsety+ 1.255),(offsetx+ 6.501),(offsety+
     1.255))
    SET rptsd->m_flags = 16
    SET rptsd->m_y = (offsety+ 1.063)
    SET rptsd->m_x = (offsetx+ 0.725)
 ...
 SUBROUTINE headpagesectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.250
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "HeadPageSection_FieldName1","Performed Day"),char(0)))
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 5.688)
    SET rptsd->m_width = 1.313
    SET rptsd->m_height = 0.250
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "HeadPageSection_FieldName3","Average (min)"),char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
 ...
 SUBROUTINE headdaysectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(format(cv_day,"##")),char(0
       )))
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
   ENDIF
   RETURN(sectionheight)
 ...
 SUBROUTINE footmonthsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(trim(cnvtstring(tproc_performed
         )),uar_i18ngetmessage(_hi18nhandle,"PROCEDUREPERFORMEDIN"," procedure(s) performed in")," ",
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 2.000),(offsety+ 0.063),(offsetx+ 6.813),(offsety+
     0.063))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
 ...
 SUBROUTINE footreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _dummyfont = uar_rptsetfont(_hreport,_times100)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(total_cnt,uar_i18ngetmessage(
        _hi18nhandle,"PROCEDUREPERFORMED"," Procedures performed averaging: "),trim(cnvtstring(
         gtotal_avg))),uar_i18ngetmessage(_hi18nhandle,"MIN"," min"),char(0)))
    SET _dummypen = uar_rptsetpen(_hreport,_pen14s3c0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.125),(offsety+ 0.131),(offsetx+ 6.417),(offsety+
     0.131))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
 ...
 SUBROUTINE graphsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptgraphrec->m_fheight = 3.000000
    SET rptgraphrec->m_stitle = ""
    SET rptgraphrec->m_ssubtitle = ""
    SET rptgraphrec->m_sxtitle = uar_i18ngetmessage(_hi18nhandle,"MONTHYR","Month-Year")
    ...
    SET rptgraphrec->m_sytitle = uar_i18ngetmessage(_hi18nhandle,"AVERAGEGRAPH","Average")
 ...
 SUBROUTINE nodatasectionabs(ncalc,offsetx,offsety)
   ...
   IF ( NOT (((( $START_DATE >  $END_DATE)) OR (size(reply_obj->cv_list,5) <= 0)) ))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CV_FRPT_AVG_WAIT_PROC"
