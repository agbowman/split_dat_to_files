CREATE PROGRAM bhs_rpt_disch_order_stats_v2
 PROMPT
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  FROM bhs_physician_team phy,
   prsnl pr
  PLAN (phy
   WHERE (phy.team= $FILENAME)
    AND phy.active_ind=1)
   JOIN (pr
   WHERE pr.person_id=phy.person_id)
  ORDER BY pr.person_id, 0
  DETAIL
   work->u_cnt = (work->u_cnt+ 1), stat = alterlist(work->users,work->u_cnt), work->users[work->u_cnt
   ].username = trim(pr.username),
   work->users[work->u_cnt].name_full_formatted = pr.name_full_formatted, work->users[work->u_cnt].
