CREATE PROGRAM acm_chg_person_person_reltn:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->person_person_reltn_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
  ...
  SET p.person_person_reltn_id = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].
   person_person_reltn_id, p.beg_effective_dt_tm = cnvtdatetime(acm_request->
    person_person_reltn_qual[xref->chg[d.seq].idx].beg_effective_dt_tm), p.contact_role_cd =
   acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].contact_role_cd,
   p.contributor_system_cd =
   IF ((acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , p.copy_correspondence_cd = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].
   copy_correspondence_cd, p.end_effective_dt_tm = cnvtdatetime(acm_request->
    person_person_reltn_qual[xref->chg[d.seq].idx].end_effective_dt_tm),
   p.free_text_cd = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].free_text_cd, p
   .ft_rel_person_name = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].
   ft_rel_person_name, p.genetic_relationship_ind = acm_request->person_person_reltn_qual[xref->chg[d
   .seq].idx].genetic_relationship_ind,
   p.living_with_ind = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].living_with_ind, p
   .person_id =
   IF ((acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].person_id > 0)) acm_request->
    person_person_reltn_qual[xref->chg[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].person_idx].
    person_id
   ENDIF
   , p.person_reltn_cd = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].person_reltn_cd,
   p.person_reltn_type_cd = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].
   person_reltn_type_cd, p.priority_seq = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx]
   .priority_seq, p.related_person_id = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].
   related_person_id,
   p.related_person_reltn_cd = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].
   related_person_reltn_cd, p.visitation_allowed_cd = acm_request->person_person_reltn_qual[xref->
   chg[d.seq].idx].visitation_allowed_cd, p.source_identifier = acm_request->
   person_person_reltn_qual[xref->chg[d.seq].idx].source_identifier,
   p.relation_seq = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].relation_seq, p
   .active_ind = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].active_ind, p
   .active_status_cd = acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].active_status_cd,
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.person_person_reltn_id=acm_request->person_person_reltn_qual[xref->chg[d.seq].idx].
   person_person_reltn_id))
  WITH nocounter, status(reply->person_person_reltn_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->person_person_reltn_qual[xref->chg[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM person_person_reltn p
    WHERE expand(t1,f_val,t_val,p.person_person_reltn_id,acm_request->person_person_reltn_qual[xref->
     chg[t1].idx].person_person_reltn_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.person_person_reltn_id,acm_request->person_person_reltn_qual[
      xref->chg[t1].idx].person_person_reltn_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->person_person_reltn_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->person_person_reltn_qual[idx].status = - (1), reply->person_person_reltn_qual[idx].
      person_person_reltn_id = acm_request->person_person_reltn_qual[idx].person_person_reltn_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->person_person_reltn_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("CONTACT_ROLE_CD,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].contact_role_cd = p.contact_role_cd
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].contributor_system_cd = p.contributor_system_cd
     ENDIF
     IF (findstring("COPY_CORRESPONDENCE_CD,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].copy_correspondence_cd = p.copy_correspondence_cd
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("FREE_TEXT_CD,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].free_text_cd = p.free_text_cd
     ENDIF
     IF (findstring("FT_REL_PERSON_NAME,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].ft_rel_person_name = p.ft_rel_person_name
     ENDIF
     IF (findstring("GENETIC_RELATIONSHIP_IND,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].genetic_relationship_ind = p
      .genetic_relationship_ind
     ENDIF
     IF (findstring("LIVING_WITH_IND,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].living_with_ind = p.living_with_ind
     ENDIF
     IF (findstring("PERSON_ID,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].person_id = p.person_id
     ENDIF
     IF (findstring("PERSON_RELTN_CD,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].person_reltn_cd = p.person_reltn_cd
     ENDIF
     IF (findstring("PERSON_RELTN_TYPE_CD,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].person_reltn_type_cd = p.person_reltn_type_cd
     ENDIF
     IF (findstring("PRIORITY_SEQ,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].priority_seq = p.priority_seq
     ENDIF
     IF (findstring("RELATED_PERSON_ID,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].related_person_id = p.related_person_id
     ENDIF
     IF (findstring("RELATED_PERSON_RELTN_CD,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].related_person_reltn_cd = p.related_person_reltn_cd
     ENDIF
     IF (findstring("VISITATION_ALLOWED_CD,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].visitation_allowed_cd = p.visitation_allowed_cd
     ENDIF
     IF (findstring("SOURCE_IDENTIFIER,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].source_identifier = p.source_identifier
     ENDIF
     IF (findstring("RELATION_SEQ,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].relation_seq = p.relation_seq
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->person_person_reltn_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->person_person_reltn_qual[xref->chg[index].idx].status=0))
