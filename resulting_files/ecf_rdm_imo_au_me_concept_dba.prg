CREATE PROGRAM ecf_rdm_imo_au_me_concept:dba
 ...
 SUBROUTINE kia_script_inc(pkg,ccl_obj)
   ...
   CALL parser(obj_prs)
 END ;Subroutine
 DECLARE kia_concept_filename = vc WITH public, constant("imo_au_me_concept.csv")
 DECLARE kia_cur_ver_beg_dt = vc WITH public, noconstant("")
 SET kia_tbl_desc = "IMO-AU-ME CONCEPT"
 SET kia_short_desc = "IMO-AU-ME CONCEPT"
 SET kia_rdm_log_name = "ecf_rdm_imo_au_me_concept.log"
 SET kia_rdm_nbr = 9928
 ...
 CALL log_sub(concat("ecf_rdm_imo_au_me_concept Begin> ",format(cnvtdatetime(curdate,curtime3),
 ...
 CALL kia_chk_status_info("LOAD IMO-AU-ME CONCEPT BASELINE DATA","check")
 IF (kia_obj_suc_ind=0)
  CALL echo("DELETING CMT_CONCEPT_LOAD_NS ROWS...")
  DELETE  FROM cmt_concept_load_ns c
   WHERE ((c.concept_cki="IMO_AU-ME*") OR (c.concept_cki="SNOMED*"))
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_concept_filename,"kia_imp_concept_load_ns","kia_imp_concept_load_ns.log")
  CALL kia_set_status_success("LOAD IMO_AU-ME CONCEPT BASELINE DATA")
 ENDIF
 ...
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(9929,"IMO-AU-ME NOMEN"))
 SET readme_data->status = "S"
