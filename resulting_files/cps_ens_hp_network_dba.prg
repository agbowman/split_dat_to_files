CREATE PROGRAM cps_ens_hp_network:dba
 RECORD reply(
   1 network[*]
     2 id = f8
 ...
 SET number_of_actions = size(request->network,5)
 SET stat = alterlist(reply->network,number_of_actions)
 SET failed = 0
 SET active_code = 0.0
