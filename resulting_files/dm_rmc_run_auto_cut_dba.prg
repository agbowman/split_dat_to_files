CREATE PROGRAM dm_rmc_run_auto_cut:dba
 ...
 SUBROUTINE drtq_view_task_process(i_process_name)
   ...
   SET message = nowindow
 END ;Subroutine
 DECLARE drrac_run_ind = i2 WITH protect, noconstant(0)
 DECLARE drrac_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drrac_run_dt_tm = dq8 WITH protect, noconstant(0.0)
 DECLARE drrac_run_num = i4 WITH protect, noconstant(0)
 DECLARE drrac_status_flag = i4 WITH protect, noconstant(0)
 DECLARE drrac_dbpw = vc WITH protect, noconstant("")
 DECLARE drrac_dbconn = vc WITH protect, noconstant("")
 DECLARE drrac_batch = vc WITH protect, noconstant("")
 DECLARE drrac_char = vc WITH protect, noconstant("")
 DECLARE drrac_loop = i4 WITH protect, noconstant(0)
 DECLARE drrac_min_lvl = i4 WITH protect, noconstant(0)
 DECLARE drrac_remove_me(null) = null
 DECLARE drrac_run_background(drb_batch=vc,drb_pw=vc,drb_conn=vc) = null
 IF (validate(drrac_reply->status,"$")="$")
  FREE RECORD drrac_reply
  RECORD drrac_reply(
    1 status = c1
  )
 ENDIF
 IF (check_logfile("dm_rmc_run_ac",".log","DM_RMC_RUN_AUTO_CUT LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_rac
 ENDIF
 SET dm_err->eproc = "Getting environment_id"
 SELECT INTO "NL:"
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="DM_ENV_ID"
  DETAIL
   drrac_env_id = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->err_ind = 1
  SET drrac_reply->status = "F"
  GO TO exit_rac
 ENDIF
 IF (curqual=0)
  SET dm_err->emsg = "There was no environment_id found in DM_INFO.  Please run DM_SET_ENV_ID."
  SET dm_err->err_ind = 1
  SET drrac_reply->status = "F"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_rac
 ENDIF
 SET dm_err->eproc = "Getting SCHEDULED CUTOVER configuration information"
 SELECT INTO "NL:"
  FROM dm_info d
  WHERE d.info_domain="RDDS CONFIGURATION"
   AND d.info_name=concat("SCHEDULED CUTOVER:",trim(cnvtstring(drrac_env_id,20)))
  DETAIL
   drrac_run_dt_tm = d.info_date, drrac_run_num = d.info_number, drrac_char = d.info_char,
   drrac_dbpw = substring(1,(findstring("::",drrac_char,1,0) - 1),drrac_char), drrac_dbconn =
   substring((findstring("::",drrac_char,1,0)+ 2),((findstring("::",drrac_char,1,1) - 2) - findstring
    ("::",drrac_char,1,0)),drrac_char), drrac_batch = substring((findstring("::",drrac_char,1,1)+ 2),
    size(drrac_char),drrac_char)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->emsg = "There was no cutover configuration found in DM_INFO."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET drrac_reply->status = "F"
  CALL drrac_remove_me(null)
  GO TO exit_rac
 ENDIF
 SET dm_err->eproc = "Checking status of SCHEDULED AUTO-CUTOVER rows in DM_REFCHG_TASK_QUEUE"
 IF (drrac_run_dt_tm <= cnvtdatetime(curdate,curtime3))
  SET drrac_status_flag = drtq_check_task_process("SCHEDULED AUTO-CUTOVER")
  IF (drrac_status_flag IN (1, 2))
   SET drrac_reply->status = "Z"
   CALL drrac_remove_me(null)
   GO TO exit_rac
  ELSEIF (drrac_status_flag IN (- (1), 4))
   SET drrac_reply->status = "Z"
   GO TO exit_rac
  ELSEIF (drrac_status_flag=3)
   SELECT INTO "nl:"
    x = min(drtq.task_level)
    FROM dm_refchg_task_queue drtq
    WHERE drtq.process_name="SCHEDULED AUTO-CUTOVER"
     AND drtq.task_status="ERROR"
    DETAIL
     drrac_min_lvl = x
    WITH nocounter
   ...
   IF (drrac_min_lvl > 0)
    UPDATE  FROM dm_refchg_task_queue drtq
     SET drtq.task_status = "QUEUED", drtq.task_retry_cnt = (drtq.task_retry_cnt - 1)
     WHERE drtq.process_name="SCHEDULED AUTO-CUTOVER"
      AND drtq.task_status="ERROR"
      AND drtq.task_level=drrac_min_lvl
      AND drtq.task_retry_cnt > 0
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc) > 0)
     ROLLBACK
   ...
   IF (drrac_min_lvl=0)
    SET drrac_run_ind = 1
    SET dm_err->eproc = "Attempting to start KSH/COM files"
    FOR (drrac_loop = 1 TO drrac_run_num)
     CALL drrac_run_background(drrac_batch,drrac_dbpw,drrac_dbconn)
     IF ((dm_err->err_ind=1))
      GO TO exit_rac
     ELSE
      CALL pause(5)
     ENDIF
    ENDFOR
    SET drrac_reply->status = "S"
   ELSE
    SET dm_err->err_ind = 1
    SET dm_err->emsg =
    "There are ERROR rows that have exceeded the retry limit.  Cutover will not be started."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 0
    SET drrac_reply->status = "F"
   ENDIF
 ...
 ELSE
  SET drrac_reply->status = "Z"
 ENDIF
 SUBROUTINE drrac_remove_me(null)
   DELETE  FROM dm_info d
    WHERE d.info_domain="DM_STAT_GATHER"
     AND d.info_name="DM_RMC_RUN_AUTO_CUT"
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   DELETE  FROM dm_info d
    WHERE d.info_domain="RDDS CONFIGURATION"
     AND d.info_name=patstring("SCHEDULED CUTOVER:*")
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   COMMIT
 END ;Subroutine
 SUBROUTINE drrac_run_background(drb_batch,drb_pw,drb_conn)
   DECLARE drb_logfile = vc WITH protect, noconstant("")
   DECLARE drb_nohup_logfile = vc WITH protect, noconstant("")
   DECLARE drb_execute_str = vc WITH protect, noconstant("")
   DECLARE drb_dcl_stat = i4 WITH protect, noconstant(0)
   SET drb_logfile = dm_err->unique_fname
   IF (get_unique_file("rdds_run_proc",".log")=0)
    SET drb_nohup_logfile = "rdds_run_proc.log"
    SET dm_err->err_ind = 0
   ELSE
    SET drb_nohup_logfile = dm_err->unique_fname
   ENDIF
   SET dm_err->unique_fname = drb_logfile
   IF (cursys="AXP")
    SET drb_execute_str = concat("SUBMIT /QUE=",drb_batch," cer_proc:rdds_run_proc.com /param=(",
     '"SCHEDULE AUTO-CUTOVER",',drb_pw,
     ",",drb_conn,") /log=CCLUSERDIR:",drb_nohup_logfile)
   ELSE
    SET drb_execute_str = concat("nohup $cer_proc/rdds_run_proc.ksh 'SCHEDULED AUTO-CUTOVER' ",drb_pw,
     " ",drb_conn," > $CCLUSERDIR/",
     drb_nohup_logfile," 2>&1 &")
   ENDIF
   CALL echo(drb_execute_str)
   CALL dcl(drb_execute_str,size(drb_execute_str),drb_dcl_stat)
   IF (drb_dcl_stat=0)
    SET dm_err->emsg = concat("Error connecting to: ",drb_dcl_stat)
    SET dm_err->err_ind = 1
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   RETURN(null)
