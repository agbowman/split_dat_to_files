CREATE PROGRAM dm_dd_emr_extract_rows:dba
 ...
 SET reply->table_name = "DD_EMR_EXTRACT"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(ex.dd_emr_extract_id)
    FROM dd_emr_extract ex
    WHERE ex.dd_emr_extract_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   seqval = max(ex.dd_emr_extract_id)
   FROM dd_emr_extract ex
   DETAIL
  ...
  WHILE (dcurminid <= dmaxid
   AND irowsleft > 0)
    SELECT INTO "nl:"
     ex.rowid
     FROM dd_emr_extract ex
     WHERE parser(sbr_getrowidnotexists("ex.DD_EMR_EXTRACT_ID between dCurMinID and dCurMaxID","ex"))
      AND ex.updt_dt_tm < cnvtdatetime((curdate - idaystokeep),curtime3)
      AND ex.dd_emr_extract_id != 0
     DETAIL
      irowcnt = (irowcnt+ 1)
      IF (mod(irowcnt,100)=1)
       stat = alterlist(reply->rows,(irowcnt+ 99))
      ENDIF
      reply->rows[irowcnt].row_id = ex.rowid
     WITH nocounter, maxqual(ex,value(irowsleft))
    ;end select
