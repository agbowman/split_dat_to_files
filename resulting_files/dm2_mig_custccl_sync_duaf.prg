CREATE PROGRAM dm2_mig_custccl_sync_duaf
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmccsd_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_sync_duaf")
 DECLARE dmccsd_master_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccsd_target_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccsd_env = vc WITH protect, noconstant("")
 DECLARE dmccsd_node = vc WITH protect, noconstant("")
 DECLARE dmccsd_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccsd_cnt = i4 WITH protect, noconstant(0)
 FREE RECORD dmccsd_diff_users
 RECORD dmccsd_diff_users(
   1 cnt = i4
   1 qual[*]
     2 user_name = vc
     2 group_nbr = i4
 ...
 DECLARE dmccsd_get_diff_users(dgfu_master_id=f8,dgfu_target_id=f8) = i2
 SET width = 132
 IF (check_logfile(dmccsd_logfile_prefix,".log","dm2_mig_custccl_sync_duaf")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_sync_duaf"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmccsd_env = cnvtupper(trim(logical("environment")))
 SET dmccsd_node = trim(curnode)
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE de.environment_name=dmccsd_env
  DETAIL
   dmccsd_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dmccsd_env))
  CALL echo(concat("Current Environment Id: ",cnvtstring(dmccsd_env_id)))
  CALL echo(concat("Current Node: ",dmccsd_node))
 ENDIF
 SET dm_err->eproc = "Get env_node_id for MASTER"
 SELECT INTO "nl:"
  FROM dma_env_node_attr dena
  WHERE dena.attr_name="CCMIG_SETUPTYPE"
   AND dena.attr_value_txt="MASTER"
  DETAIL
   dmccsd_master_id = dena.dma_env_node_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dmccsd_master_id=0)
  SET dm_err->emsg = "A valid entry for MASTER does not exist in the dma_env_node_attr table"
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Get env_node_id for current TARGET"
 SELECT INTO "nl:"
  FROM dma_env_node_attr dena
  WHERE dena.attr_name="CCMIG_SETUPTYPE"
   AND dena.attr_value_txt="TARGET"
   AND (dena.dma_env_node_id=
  (SELECT
   dma_env_node_id
   FROM dma_env_node dne
   WHERE dne.node_name=dmccsd_node
    AND dne.environment_id=dmccsd_env_id))
  DETAIL
   dmccsd_target_id = dena.dma_env_node_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dmccsd_target_id=0)
  SET dm_err->emsg =
  "A valid TARGET entry does not exist in the dma_env_node_attr table for the current node/environment"
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dmccsd_get_diff_users(dmccsd_master_id,dmccsd_target_id)=0)
  GO TO exit_program
 ENDIF
 IF ((dmccsd_diff_users->cnt=0))
  SET dm_err->eproc = "No difference in users between MASTER and TARGET"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmccsd_diff_users)
 ENDIF
 SET dm_err->eproc = "Creating additional cclusers in TARGET"
 CALL disp_msg("",dm_err->logfile,0)
 FOR (dmccsd_cnt = 1 TO dmccsd_diff_users->cnt)
   IF (dm2_push_cmd(concat("create ccluser ",dmccsd_diff_users->qual[dmccsd_cnt].user_name," to ",
     dmccsd_diff_users->qual[dmccsd_cnt].group_name," go"),1)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc =
   "Insert new rows into dma_env_node_duaf admin table for new user in current TARGET"
   INSERT  FROM dma_env_node_duaf dend
    SET dend.dma_duaf_id = seq(dm_seq,nextval), dend.dma_env_node_id = dmccsd_target_id, dend
     .user_name = dmccsd_diff_users->qual[dmccsd_cnt].user_name,
     dend.group_name = dmccsd_diff_users->qual[dmccsd_cnt].group_name, dend.group_nbr =
     dmccsd_diff_users->qual[dmccsd_cnt].group_nbr, dend.create_dt_tm = cnvtdatetime(
      dmccsd_diff_users->qual[dmccsd_cnt].create_dt_tm),
     dend.datestamp_txt = dmccsd_diff_users->qual[dmccsd_cnt].datestamp_txt, dend.timestamp_txt =
     dmccsd_diff_users->qual[dmccsd_cnt].timestamp_txt, dend.cclcount_nbr = dmccsd_diff_users->qual[
     dmccsd_cnt].cclcount_nbr
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
 ...
 ENDFOR
 GO TO exit_program
 SUBROUTINE dmccsd_get_diff_users(dgfu_master_id,dgfu_target_id)
   SET dm_err->eproc =
   "Retrieving list of users that are present in MASTER but not in current TARGET."
   SELECT INTO "nl:"
    FROM dma_env_node_duaf d1
    WHERE d1.dma_env_node_id=dgfu_master_id
     AND  NOT ( EXISTS (
    (SELECT
     "x"
     FROM dma_env_node_duaf d2
     WHERE d2.dma_env_node_id=dgfu_target_id
      AND d2.user_name=d1.user_name)))
    HEAD REPORT
     dmccsd_diff_users->cnt = 0, stat = alterlist(dmccsd_diff_users->qual,0)
    DETAIL
     dmccsd_diff_users->cnt = (dmccsd_diff_users->cnt+ 1), stat = alterlist(dmccsd_diff_users->qual,
      dmccsd_diff_users->cnt), dmccsd_diff_users->qual[dmccsd_diff_users->cnt].user_name = d1
     .user_name,
     dmccsd_diff_users->qual[dmccsd_diff_users->cnt].group_nbr = d1.group_nbr, dmccsd_diff_users->
     qual[dmccsd_diff_users->cnt].group_name = d1.group_name, dmccsd_diff_users->qual[
     dmccsd_diff_users->cnt].create_dt_tm = cnvtdatetime(d1.create_dt_tm),
     dmccsd_diff_users->qual[dmccsd_diff_users->cnt].datestamp_txt = d1.datestamp_txt,
     dmccsd_diff_users->qual[dmccsd_diff_users->cnt].timestamp_txt = d1.timestamp_txt,
     dmccsd_diff_users->qual[dmccsd_diff_users->cnt].cclcount_nbr = d1.cclcount_nbr
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_sync_duaf"
 CALL final_disp_msg(dmccsd_logfile_prefix)
END GO
