CREATE PROGRAM bhs_athn_updt_assmnt_plan
 RECORD t_rec(
   1 a_line = vc
   1 dx_line = vc
   1 d_line = vc
 ...
 ELSE
  IF (( $5 !=  $6))
   EXECUTE bhs_athn_add_doc_segment "mine",  $5,  $7,
    $9, "", ""
   GO TO exit_script
  ENDIF
  IF (( $5= $6))
   SELECT INTO "nl:"
    ...
    WITH nocounter, separator = " ", format,
     time = 10
   ;end select
   SET t_blob = concat(t_blob,trim( $9,3))
  ENDIF
 ENDIF
 ...
 IF (( $4 != "ASSESSMENT"))
  FOR (i = 1 TO t_rec->diag_cnt)
    IF ((t_rec->diag_qual[i].key_name= $4))
     SET t_rec->diag_qual[i].plan_str = t_blob
     SET t_rec->diag_qual[i].last_updated =  $8
     SET t_rec->diag_qual[i].last_updated_by = prsnl_string
    ENDIF
  ENDFOR
 ENDIF
 SET out_rec->dx_line = "["
...
#exit_script
 IF (( $6=1))
  IF (( $4="ASSESSMENT"))
   SET out_rec->a_line = concat(
    '[{"KEY_NAME":"ASSESSMENT","CONTENTIDX":{"LABEL":{"PROPIDX":{"TEXT":""}},"ASSESSMENT":{"PROPIDX":{"TEXT":"',
    t_blob,'","LAST_UPDATED":', $8,',"USER_DEFINED":1,"LAST_UPDATED_BY":"',
    prsnl_string,'"}}},"HIDDEN_IND":0,"REMOVABLE_IND":""}]')
   EXECUTE uhs_mpg_upd_rounding_sum "mine",  $2, "ASSESSMENT",
   "", "", out_rec->a_line,
   "1"
  ENDIF
  IF (( $4 != "ASSESSMENT"))
