CREATE PROGRAM cp_suggest_pathways_by_reason:dba
 PROMPT
  "Pathways JSON" = "",
  "Person ID" = 0.0,
  "Encounter ID" = 0.0,
  "Output to File/Printer/MINE: " = "MINE"
  WITH inputjson, person_id, encounter_id,
  outdev
 ...
 DECLARE pid = f8 WITH constant(validate(trigger_personid, $2)), protect
 DECLARE eid = f8 WITH constant(validate(trigger_encntrid, $3)), protect
 DECLARE prsnl_id = f8 WITH constant(reqinfo->updt_id), protect
 DECLARE pathways_json = vc WITH constant(trim( $1)), protect
 ...
 protect
 DECLARE rfv_triggering_type_mean = vc WITH constant("REASON_FOR_VISIT"), protect
 DECLARE dx_triggering_type_mean = vc WITH constant("THIS_VISIT_PROBLEM"), protect
 DECLARE diag_triggering_type_mean = vc WITH constant("DIAGNOSIS"), protect
 DECLARE ont_cond_triggering_type_mean = vc WITH constant("DISCERN_ONTOLOGY_CONDITION"), protect
 DECLARE order_triggering_type_mean = vc WITH constant("ORDER_ADD_TO_SCRATCHPAD"), protect
 DECLARE rule_type_mean = vc WITH constant("SUGGESTED_BY_RULE"), protect
 DECLARE suggest_by_rfv_act_det_type_mean = vc WITH constant("SUGGESTBYRFV"), protect
 DECLARE suggest_by_dx_act_det_type_mean = vc WITH constant("SUGGESTBYDX"), protect
 DECLARE suggest_by_order_act_det_type_mean = vc WITH constant("SUGGESTBYORD"), protect
 DECLARE suggest_by_rule_mean = vc WITH constant("RULESUGGEST"), protect
 DECLARE sys_suggest_act_type_mean = vc WITH constant("SYSPWSUGGEST"), protect
 ...
 DECLARE checkactiveproposedpathways(null) = null WITH protect
 ...
 SUBROUTINE checkactiveproposedpathways(null)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   DECLARE idx = i4 WITH noconstant(0), protect
   DECLARE pathidx = i4 WITH noconstant(0), protect
   DECLARE searchcntr = i4 WITH noconstant(0), protect
   CALL log_message("start checkActiveProposedPathways()",log_level_debug)
   SELECT INTO "nl:"
    FROM cp_pathway_activity cpa
    PLAN (cpa
     WHERE cpa.person_id=pid
      AND cpa.end_effective_dt_tm > cnvtdatetime(sysdate)
      AND expand(idx,1,pathwaystosuggest->cnt,cpa.cp_pathway_id,pathwaystosuggest->qual[idx].
      cp_pathway_id))
    ORDER BY cpa.cp_pathway_id, cpa.pathway_instance_id, cpa.beg_effective_dt_tm DESC
    HEAD cpa.pathway_instance_id
     IF (((cpa.pathway_activity_status_cd=active_act_status_cd) OR (cpa.pathway_activity_status_cd=
     proposed_act_status_cd
      AND cpa.encntr_id=eid)) )
      pathidx = locateval(searchcntr,1,pathwaystosuggest->cnt,cpa.cp_pathway_id,pathwaystosuggest->
       qual[searchcntr].cp_pathway_id)
      IF (pathidx > 0)
       pathwaystosuggest->qual[pathidx].active_pathway_exists_ind = 1
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   CALL log_message(build("exit checkActiveProposedPathways, elapsed time in seconds:",((curtime3 -
     start_tm)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE main(null)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   DECLARE pathwaycntr = i4 WITH protect
   DECLARE suggestedpathwaynames = vc WITH noconstant(""), protect
   ...
   IF (textlen(pathways_json) > 0)
    ...
    IF ((pathwaystosuggest->cnt > 0))
     CALL checkactiveproposedpathways(null)
     IF (validate(debug_ind))
      CALL echorecord(pathwaystosuggest)
     ENDIF
     FOR (pathwaycntr = 1 TO pathwaystosuggest->cnt)
       IF ((pathwaystosuggest->qual[pathwaycntr].active_pathway_exists_ind=0))
        SET cur_pathway_instance_id = 0
        SET trace = recpersist
        EXECUTE cp_add_pathway_activity:dba "NOFORMS", pid, eid,
        reqinfo->updt_id, pathwaystosuggest->qual[pathwaycntr].cp_pathway_id, "PROPOSED",
        0, 1 WITH replace("MPAGEREPLY","ACTIVITYREPLY")
        SET trace = norecpersist
        IF (validate(debug_ind))
         CALL echorecord(activityreply)
        ENDIF
        SET cur_pathway_instance_id = activityreply->pathway_instance_id
        IF ((activityreply->status_data.status="S")
         ...
         SET actionreq->actions[1].action_type_mean = sys_suggest_act_type_mean
         SET actionreq->actions[1].action_dt_tm = cur_dt_tm
         SET stat = alterlist(actionreq->actions[1].details,1)
         IF ((pathwaystosuggest->qual[pathwaycntr].criteria_type_mean IN (dx_triggering_type_mean,
         diag_triggering_type_mean, ont_cond_triggering_type_mean)))
          SET actionreq->actions[1].details[1].cp_action_detail_type_mean =
          suggest_by_dx_act_det_type_mean
         ELSEIF ((pathwaystosuggest->qual[pathwaycntr].criteria_type_mean=rfv_triggering_type_mean))
          SET actionreq->actions[1].details[1].cp_action_detail_type_mean =
          suggest_by_rfv_act_det_type_mean
         ELSEIF ((pathwaystosuggest->qual[pathwaycntr].criteria_type_mean=order_triggering_type_mean)
         )
          SET actionreq->actions[1].details[1].cp_action_detail_type_mean =
          suggest_by_order_act_det_type_mean
         ELSEIF ((pathwaystosuggest->qual[pathwaycntr].criteria_type_mean=rule_type_mean))
          SET actionreq->actions[1].details[1].cp_action_detail_type_mean = suggest_by_rule_mean
         ENDIF
         SET actionreq->actions[1].details[1].action_detail_text = str_decodeuri(pathwaystosuggest->
          qual[pathwaycntr].trigger_display)
         SET actionreq->actions[1].details[1].action_detail_entity_name = pathwaystosuggest->qual[
         pathwaycntr].trigger_parent_entity_name
         SET actionreq->actions[1].details[1].action_detail_entity_id = pathwaystosuggest->qual[
         pathwaycntr].trigger_parent_entity_id
         IF (validate(debug_ind))
         ...
         IF ((actionreply->status_data.status="S"))
          IF (suggestedpathwaynames="")
           SET suggestedpathwaynames = str_decodeuri(pathwaystosuggest->qual[pathwaycntr].
            pathway_name)
          ELSE
           SET suggestedpathwaynames = build2(suggestedpathwaynames,", ",str_decodeuri(
             pathwaystosuggest->qual[pathwaycntr].pathway_name))
          ENDIF
         ELSE
     ...
     ENDFOR
     IF (suggestedpathwaynames > " ")
      SET log_misc1 = build("Suggesting Care Pathway(s): ",suggestedpathwaynames)
      SET retval = 100
      SET reqinfo->commit_ind = 1
     ELSE
      SET log_misc1 = "No Care Pathways suggested"
      SET retval = 0
     ENDIF
    ELSE
     CALL log_message(build("Found ",cnvtstring(pathwaystosuggest->cnt)," pathways; cannot start."),
      log_level_debug)
     SET log_misc1 = ""
     SET retval = 0
     GO TO exit_script
 ...
 CALL main(null)
#exit_script
 CALL putjsonrecordtofile(reply_record)
