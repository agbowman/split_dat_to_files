CREATE PROGRAM bhs_athn_add_dyndoc_v2
 FREE RECORD result
 RECORD result(
 ...
 FREE RECORD req969500
 RECORD req969500(
   1 patient_id = f8
   1 encounter_id = f8
   1 dd_ref_template_id = f8
 ) WITH protect
 FREE RECORD rep969500
 RECORD rep969500(
   1 document
     2 attribute
       3 author_id = f8
 ...
 SET result->message_seq =  $8
 SET result->message_total =  $9
 IF (( $2 <= 0.0))
 ...
 ELSEIF (( $3 <= 0.0))
  ...
  GO TO exit_script
 ELSEIF (( $4 <= 0.0))
  CALL echo("INVALID EVENT CD PARAMETER...EXITING")
  GO TO exit_script
 ELSEIF (( $5 <= 0.0))
  CALL echo("INVALID PERSONNEL ID PARAMETER...EXITING")
  GO TO exit_script
 ELSEIF ((((result->message_seq <= 0)) OR ((result->message_seq > result->message_total))) )
  CALL echo("INVALID MESSAGE SEQUENCE PARAMETER...EXITING")
  GO TO exit_script
 ELSEIF ((result->message_total <= 0))
  CALL echo("INVALID MESSAGE TOTAL PARAMETER...EXITING")
  GO TO exit_script
 ELSEIF (( $13 <= 0.0))
  CALL echo("INVALID DD TEMPLATE ID PARAMETER...EXITING")
  GO TO exit_script
 ENDIF
 SELECT INTO "NL:"
...
#exit_script
 ...
 IF (size(trim(moutputdevice,3)) > 0)
  SELECT INTO value(moutputdevice)
   ...
   HEAD REPORT
    ...
    v1, row + 1, v2 = build("<MDocEventID>",cnvtint(result->mdoc_event_id),"</MDocEventID>"),
    col + 1, v2, row + 1,
    v3 = build("<SegmentFileName>",trim(replace(replace(replace(replace(replace(result->
           segment_file_name,"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",
       0),3),"</SegmentFileName>"), col + 1, v3,
    row + 1, col + 1, "</ReplyMessage>",
    row + 1
 ...
 SUBROUTINE formatparameters(null)
   ...
   IF (textlen(trim( $11,3)))
    SET req_decode_str->blob =  $11
 ...
 SUBROUTINE concatenatemessagesegments(null)
   ...
   ELSEIF ((result->message_seq=result->message_total))
    FOR (idx = 1 TO (result->message_total - 1))
      SET temp_file = concat("CER_TEMP:", $10,"_",trim(cnvtstring(idx),3),".TMP")
   ...
   ELSE
    SET temp_file = concat("CER_TEMP:", $10,"_",trim(cnvtstring(result->message_seq),3),".TMP")
 ...
 SUBROUTINE callopendocument(null)
   ...
   DECLARE requestid = i4 WITH constant(969500)
   SET req969500->patient_id =  $2
   SET req969500->encounter_id =  $3
   SET req969500->dd_ref_template_id =  $13
   CALL echorecord(req969500)
   SET stat = tdbexecute(applicationid,taskid,requestid,"REC",req969500,
    "REC",rep969500,1)
   IF (stat > 0)
    ...
    RETURN(fail)
   ENDIF
   CALL echorecord(rep969500)
   IF ((rep969500->status_data.status="S")
    AND size(rep969500->contributions,5) > 0)
    SET result->dd_contribution_id = rep969500->contributions[1].contribution_id
    SET result->dd_session_id = rep969500->contributions[1].dd_session_id
    RETURN(success)
   ENDIF
   RETURN(fail)
 END ;Subroutine
 SUBROUTINE callsavedocument(null)
   ...
   SET req969502->author_id =  $5
   SET req969502->encounter_id =  $3
   SET req969502->event_cd =  $4
   SET req969502->patient_id =  $2
   SET req969502->service_dt_tm = cnvtdatetime( $6)
   SET req969502->service_tz = app_tz
   ...
   SET stat = alterlist(req969502->contributions,1)
   SET req969502->contributions[1].author_id =  $5
   SET req969502->contributions[1].dd_contribution_id = result->dd_contribution_id
   SET req969502->contributions[1].dd_session_id = result->dd_session_id
   SET req969502->contributions[1].event_cd =  $4
   SET req969502->contributions[1].html_text = result->html_text
   SET req969502->contributions[1].title_text = result->subject
   SET req969502->contributions[1].ensure_type = evaluate( $12,1,1,2)
   ...
   IF ((rep969502->status_data.status="S"))
    SET result->mdoc_event_id = rep969502->mdoc_event_id
    RETURN(success)
   ENDIF
   RETURN(fail)
 END ;Subroutine
