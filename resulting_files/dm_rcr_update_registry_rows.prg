CREATE PROGRAM dm_rcr_update_registry_rows
 ...
 SET reply->table_name = "RCR_UPDATE_REGISTRY"
 ...
 FOR (v_token_idx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 1825)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must keep at least 1825 days worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
 ELSE
  SELECT INTO "nl:"
   rur.rowid
   FROM rcr_update_registry rur
   WHERE rur.create_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
    AND parser(sbr_getrowidnotexists("ruq.rcr_update_registry_id != 0","ruq"))
   DETAIL
    v_rows = (v_rows+ 1)
    IF (mod(v_rows,50)=1)
