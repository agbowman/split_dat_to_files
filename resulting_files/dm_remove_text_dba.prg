CREATE PROGRAM dm_remove_text:dba
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 IF (check_logfile("dm_rmv_text",".log","DM_REMOVE_TEXT LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_REMOVE_TEXT..."
 DECLARE drt_loop = i4 WITH protect, noconstant(0)
 DECLARE drt_idx = i4 WITH protect, noconstant(0)
 DECLARE drt_error_ind = i2 WITH protect, noconstant(0)
 DECLARE drt_error_msg = vc WITH protect, noconstant("")
 DECLARE drt_start_dt_tm = f8 WITH protect, noconstant(0.0)
 DECLARE drt_end_dt_tm = f8 WITH protect, noconstant(0.0)
 DECLARE drt_lookup_data(dld_detail_id=f8,drop_rec=vc(ref),dld_log_id=f8) = null
 DECLARE drt_remove_data(drd_rec=vc(ref),drd_log_id=f8) = null
 FREE RECORD remove_rec
 RECORD remove_rec(
   1 cnt = i4
   1 qual[*]
 ...
 IF ((daf_icd9_request->parent_log_id=0.0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No PARENT_LOG_ID supplied in the REQUEST structure.  Required to run script."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET drt_start_dt_tm = cnvtdatetime(curdate,curtime3)
 IF ((daf_icd9_request->text_find_detail_id > 0.0))
  SELECT INTO "NL:"
   FROM dm_text_find_detail c,
    dm_text_find d
   WHERE (((c.dm_text_find_detail_id=daf_icd9_request->text_find_detail_id)
    AND c.detail_type_flag IN (2, 4)) OR (c.active_ind=0
    AND c.detail_type_flag IN (2, 4)))
    AND d.dm_text_find_id=c.dm_text_find_id
    AND c.dm_text_find_detail_id > 0.0
   DETAIL
    remove_rec->cnt = (remove_rec->cnt+ 1), stat = alterlist(remove_rec->qual,remove_rec->cnt),
    remove_rec->qual[remove_rec->cnt].dm_text_find_detail_id = c.dm_text_find_detail_id,
    remove_rec->qual[remove_rec->cnt].det_meaning = c.detail_meaning
   WITH nocounter
  ;end select
 ELSEIF ((daf_icd9_request->find_category > " "))
  SELECT INTO "NL:"
   FROM dm_text_find_cat fc,
    dm_text_find_cat_r r,
    dm_text_find d,
    dm_text_find_detail c
   WHERE (fc.find_category=daf_icd9_request->find_category)
    AND fc.active_ind=1
    AND r.dm_text_find_cat_id=fc.dm_text_find_cat_id
    AND r.active_ind=1
    AND d.dm_text_find_id=r.dm_text_find_id
    AND c.dm_text_find_id=d.dm_text_find_id
    AND c.detail_type_flag IN (2, 4)
    AND c.dm_text_find_detail_id > 0.0
   DETAIL
    remove_rec->cnt = (remove_rec->cnt+ 1), stat = alterlist(remove_rec->qual,remove_rec->cnt),
 ...
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No data found in the request structure"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(daf_icd9_request->parent_log_id,"ERROR",dm_err->emsg,0.0,0.0)
  GO TO exit_script
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(daf_icd9_request->parent_log_id,"ERROR",concat(
    "Getting DM_TEXT_FIND_DETAIL info: ",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 SELECT INTO "NL:"
  FROM (dummyt d  WITH seq = remove_rec->cnt),
   dm_info i
  PLAN (d)
   JOIN (i
   WHERE i.info_domain="DM_TEXT_FIND_CONFIGURATIONS"
    AND i.info_name=concat("CUSTOM REMOVE:",trim(remove_rec->qual[d.seq].det_meaning)))
  DETAIL
   remove_rec->qual[d.seq].cust_remove = i.info_char
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(daf_icd9_request->parent_log_id,"ERROR",concat(
    "Looking for custom remove scripts: ",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 FOR (drt_loop = 1 TO remove_rec->cnt)
   SET drop_rec->cnt = 0
   SET stat = alterlist(drop_rec->qual,0)
   IF ((remove_rec->qual[drt_loop].cust_remove > " "))
    CALL parser(concat("execute ",remove_rec->qual[drt_loop].cust_remove," ",trim(cnvtstring(
        remove_rec->qual[drt_loop].dm_text_find_detail_id,20,1)),", ",
      trim(cnvtstring(daf_icd9_request->parent_log_id,20,1))," GO"),1)
    IF (check_error(dm_err->eproc)=1)
     IF (drt_error_ind=0)
      SET drt_error_msg = dm_err->emsg
     ENDIF
     SET drt_error_ind = 1
     SET dm_err->err_ind = 0
    ENDIF
    CALL drt_remove_data(drop_rec,daf_icd9_request->parent_log_id)
    IF (check_error(dm_err->eproc)=1)
     IF (drt_error_ind=0)
      SET drt_error_msg = dm_err->emsg
     ENDIF
    ...
    ENDIF
   ELSE
    CALL drt_lookup_data(remove_rec->qual[drt_loop].dm_text_find_detail_id,drop_rec,daf_icd9_request
     ->parent_log_id)
    IF (check_error(dm_err->eproc)=1)
     IF (drt_error_ind=0)
      SET drt_error_msg = dm_err->emsg
   ...
   ENDIF
 ENDFOR
 SUBROUTINE drt_lookup_data(dld_detail_id,dld_rec,dld_log_id)
   DECLARE dld_idx = i4 WITH protect, noconstant(0)
   DECLARE dld_first_cnt = i4 WITH protect, noconstant(0)
   DECLARE dld_found_ind = i2 WITH protect, noconstant(0)
   DECLARE dld_error_ind = i2 WITH protect, noconstant(0)
   DECLARE dld_error_msg = vc WITH protect, noconstant("")
   FREE RECORD dld_parse
   RECORD dld_parse(
     1 stmt[*]
       2 str = vc
   )
   SELECT INTO "NL:"
    FROM dm_text_find_data d
    WHERE d.script_name IS NOT null
     AND d.script_name > " "
     AND d.dm_text_find_detail_id=dld_detail_id
     AND d.status_flag != 3
     AND d.dm_text_find_data_id > 0.0
     AND d.node_name=curnode
    HEAD REPORT
     stat = alterlist(dld_rec->qual,100)
    DETAIL
     dld_rec->cnt = (dld_rec->cnt+ 1)
     IF (mod(dld_rec->cnt,100)=1
      AND (dld_rec->cnt > 1))
      stat = alterlist(dld_rec->qual,(dld_rec->cnt+ 99))
     ENDIF
     dld_rec->qual[dld_rec->cnt].script_name = d.script_name, dld_rec->qual[dld_rec->cnt].
     dm_text_find_data_id = d.dm_text_find_data_id, dld_rec->qual[dld_rec->cnt].parent_entity_id = d
     .parent_entity_id,
     dld_rec->qual[dld_rec->cnt].parent_entity_name = d.parent_entity_name, dld_rec->qual[dld_rec->
     cnt].parent_entity_col = d.parent_entity_col, dld_rec->qual[dld_rec->cnt].search_col_name = d
     .search_col_name,
     dld_rec->qual[dld_rec->cnt].group_num = d.group_num
    WITH nocounter
   ;end select
   IF (check_error("Querying dm_text_find_data table") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    CALL dst_add_log_det(dld_log_id,"ERROR",concat("Querying DM_TEXT_FIND_DATA: ",dm_err->emsg),0.0,
     0.0)
    SET dld_error_msg = dm_err->emsg
    SET dld_error_ind = 1
    SET dm_err->err_ind = 0
   ENDIF
   IF ((dld_rec->cnt > 0))
    SELECT INTO "NL:"
     ...
     PLAN (d
      ...
      WHERE "P"=c.object
       ...
       AND 0=c.qual)
     DETAIL
      dld_idx = locateval(dld_idx,1,dld_rec->cnt,d.object_name,dld_rec->qual[dld_idx].script_name,
       d.group,dld_rec->qual[dld_idx].group_num)
      IF (dld_idx > 0)
       dld_rec->qual[dld_idx].exists_ind = 1
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error("Querying dprotect table") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     CALL dst_add_log_det(dld_log_id,"ERROR",concat("Querying DPROTECT: ",dm_err->emsg),0.0,0.0)
     SET dld_error_msg = dm_err->emsg
     SET dld_error_ind = 1
     SET dm_err->err_ind = 0
     SET dld_rec->cnt = 0
     SET stat = alterlist(dld_rec->qual,0)
    ENDIF
   ENDIF
   SET dld_first_cnt = dld_rec->cnt
   SELECT INTO "NL:"
    FROM dm_text_find_data d
    WHERE ((d.script_name = null) OR (d.script_name <= " "))
     AND d.dm_text_find_detail_id=dld_detail_id
     AND d.status_flag != 3
     AND d.dm_text_find_data_id > 0.0
     AND d.parent_entity_name > " "
     AND d.parent_entity_id > 0.0
     AND d.parent_entity_col > " "
     AND d.search_col_name > " "
     AND d.node_name=curnode
    DETAIL
     dld_rec->cnt = (dld_rec->cnt+ 1)
     IF (mod(dld_rec->cnt,100)=1)
      stat = alterlist(dld_rec->qual,(dld_rec->cnt+ 99))
     ENDIF
     dld_rec->qual[dld_rec->cnt].script_name = "", dld_rec->qual[dld_rec->cnt].dm_text_find_data_id
      = d.dm_text_find_data_id, dld_rec->qual[dld_rec->cnt].parent_entity_id = d.parent_entity_id,
     dld_rec->qual[dld_rec->cnt].parent_entity_name = d.parent_entity_name, dld_rec->qual[dld_rec->
     cnt].parent_entity_col = d.parent_entity_col, dld_rec->qual[dld_rec->cnt].search_col_name = d
     .search_col_name,
     dld_rec->qual[dld_rec->cnt].group_num = 0
    FOOT REPORT
     stat = alterlist(dld_rec->qual,dld_rec->cnt)
    WITH nocounter
   ;end select
   IF (check_error("Querying dm_text_find_data table") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dld_rec->cnt > 0)
    AND (dld_rec->cnt != dld_first_cnt))
    SET stat = alterlist(dld_parse->stmt,3)
    FOR (dld_idx = (dld_first_cnt+ 1) TO dld_rec->cnt)
      SET dld_found_ind = 0
      SET dld_parse->stmt[1].str = concat("select into 'NL:' from ",dld_rec->qual[dld_idx].
       parent_entity_name," l")
      SET dld_parse->stmt[2].str = concat(" where l.",dld_rec->qual[dld_idx].parent_entity_col," = ",
       "dld_rec->qual[",trim(cnvtstring(dld_idx)),
       "].parent_entity_id")
      SET dld_parse->stmt[3].str = " detail dld_found_ind = 1 with nocounter go"
      EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DLD_PARSE")
      IF (check_error("Querying dm_text_find_data table") != 0)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       CALL dst_add_log_det(dld_log_id,"ERROR",concat("Querying LONG data: ",dm_err->emsg),0.0,0.0)
       IF (dld_error_ind=0)
        SET dld_error_msg = dm_err->emsg
       ENDIF
       SET dld_error_ind = 1
       ...
       SET dld_found_ind = 1
      ENDIF
      SET dld_rec->qual[dld_idx].exists_ind = dld_found_ind
    ENDFOR
   ENDIF
   SET dm_err->err_ind = dld_error_ind
   SET dm_err->emsg = dld_error_msg
   RETURN(null)
 END ;Subroutine
 SUBROUTINE drt_remove_data(drd_rec,drd_log_id)
   SET dm_err->eproc = "Removing dropped script objects from DM_TEXT_FIND_DATA table."
   DECLARE drd_loop = i4 WITH protect, noconstant(0)
   DECLARE drd_error_ind = i2 WITH protect, noconstant(0)
   DECLARE drd_error_msg = vc WITH protect, noconstant("")
   FREE RECORD drd_parse
   RECORD drd_parse(
     1 stmt[*]
       2 str = vc
   )
   SET stat = alterlist(drd_parse->stmt,3)
   FOR (drd_loop = 1 TO drd_rec->cnt)
     IF ((drd_rec->qual[drd_loop].exists_ind=0))
      IF ((drd_rec->qual[drd_loop].script_name > " "))
       SET drd_parse->stmt[1].str = concat("delete from ",drd_rec->qual[drd_loop].parent_entity_name,
        " l")
       SET drd_parse->stmt[2].str = concat(" where l.",drd_rec->qual[drd_loop].parent_entity_col,
        " = ","drd_rec->qual[",trim(cnvtstring(drd_loop)),
        "].parent_entity_id")
       SET drd_parse->stmt[3].str = " with nocounter go"
       EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DRD_PARSE")
       IF (check_error("Removing long data") != 0)
        ROLLBACK
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        IF (drd_error_ind=0)
         SET drd_error_msg = dm_err->emsg
        ENDIF
        SET drd_error_ind = 1
        SET dm_err->err_ind = 0
        CALL dst_add_log_det(drd_log_id,"ERROR",concat("Removing LONG data: ",dm_err->emsg),0.0,0.0)
       ENDIF
      ENDIF
      UPDATE  FROM dm_text_find_data d
       SET d.status_flag = 3, d.updt_id = reqinfo->updt_id, d.updt_task = reqinfo->updt_task,
        d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_cnt = (d.updt_cnt+ 1), d.updt_applctx
         = reqinfo->updt_applctx
       WHERE (d.dm_text_find_data_id=drd_rec->qual[drd_loop].dm_text_find_data_id)
      ;end update
      IF (check_error("Removing DM_TEXT_FIND_DATA data") != 0)
       ROLLBACK
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       IF (drd_error_ind=0)
        SET drd_error_msg = dm_err->emsg
       ...
       SET drd_error_ind = 1
       SET dm_err->err_ind = 0
       CALL dst_add_log_det(drd_log_id,"ERROR",concat("Updating DM_TEXT_FIND_DATA status: ",dm_err->
         emsg),0.0,0.0)
      ENDIF
      COMMIT
     ENDIF
   ENDFOR
   SET dm_err->err_ind = drd_error_ind
   SET dm_err->emsg = drd_error_msg
   RETURN(null)
 END ;Subroutine
#exit_script
 SET drt_end_dt_tm = cnvtdatetime(curdate,curtime3)
 IF (drt_error_ind=1)
  SET dm_err->err_ind = drt_error_ind
  SET dm_err->emsg = drt_error_msg
 ENDIF
 IF ((dm_err->err_ind=1))
  SET daf_icd9_reply->status = "F"
  SET daf_icd9_reply->message = dm_err->emsg
  IF ((daf_icd9_request->parent_log_id > 0.0))
   CALL dst_add_log_det(daf_icd9_request->parent_log_id,"ERROR","The call to DM_REMOVE_TEXT failed.",
    drt_start_dt_tm,drt_end_dt_tm)
  ENDIF
 ELSE
  SET daf_icd9_reply->status = "S"
  SET daf_icd9_reply->message = "DM_REMOVE_TEXT completed without error."
  IF ((daf_icd9_request->parent_log_id > 0.0))
   CALL dst_add_log_det(daf_icd9_request->parent_log_id,"SUCCESS",
    "The call to DM_REMOVE_TEXT completed successfully.",drt_start_dt_tm,drt_end_dt_tm)
  ENDIF
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending DM_REMOVE_TEXT"
  CALL final_disp_msg("dm_remove_text")
 ENDIF
END GO
