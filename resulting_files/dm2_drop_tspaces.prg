CREATE PROGRAM dm2_drop_tspaces
 ...
 SUBROUTINE dtr_load_clin_tspaces(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ddt_file_found = i2 WITH protect, noconstant(0)
 DECLARE ddt_database_remote = i2 WITH protect, noconstant(0)
 DECLARE ddt_ts_cnt = i4 WITH protect, noconstant(0)
 DECLARE ddt_lv_cnt = i4 WITH protect, noconstant(0)
 DECLARE ddt_spot = i4 WITH protect, noconstant(0)
 DECLARE ddt_spot_end = i4 WITH protect, noconstant(0)
 DECLARE ddt_ndx = i4 WITH protect, noconstant(0)
 DECLARE ddt_str = vc WITH protect, noconstant("")
 DECLARE ddt_standalone = i2 WITH protect, noconstant(0)
 DECLARE ddt_dbname = vc WITH protect, noconstant("")
 DECLARE ddt_selection = vc WITH protect, noconstant("")
 DECLARE ddt_finish = i2 WITH protect, noconstant(0)
 DECLARE ddt_disp_report(null) = i2
 DECLARE ddt_get_tspace_to_drop(null) = i2
 FREE RECORD ddt_ts
 RECORD ddt_ts(
   1 drop_cnt = i4
   1 tsp[*]
 ...
 IF ((dm2_install_schema->curprog="NONE"))
  SET ddt_standalone = 1
 ENDIF
 IF (check_logfile("dm2_drop_tspaces",".log","dm2_drop_tspaces")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_drop_tspaces"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (currdb != "ORACLE")
  SET dm_err->err_ind = 1
 ...
 IF (ddt_standalone=1)
  ...
  SET ddt_selection = curaccept
  SET message = nowindow
  IF (ddt_selection="Y")
   SET dm_err->eproc = "User elected to drop tablespaces."
   CALL disp_msg(" ",dm_err->logfile,0)
  ELSE
   SET dm_err->eproc = "User elected to NOT drop tablespaces. Exiting Script"
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF ((dir_storage_misc->cur_storage_type="RAW"))
  IF ((dm2_sys_misc->cur_db_os != dm2_sys_misc->cur_os))
   SET ddt_database_remote = 1
  ELSE
   IF (dir_find_data_file(ddt_file_found)=0)
    GO TO exit_program
   ENDIF
   IF (ddt_file_found=0)
    SET ddt_database_remote = 1
 ...
 ENDIF
 IF (curuser != "ROOT"
  AND ddt_database_remote=0
  AND (dir_storage_misc->cur_storage_type="RAW"))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "User must be logged in as ROOT"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dm2_get_dbase_name(ddt_dbname)=0)
  GO TO exit_program
 ENDIF
 IF (ddt_dbname="")
  SET dm_err->err_ind = 1
  ...
  GO TO exit_program
 ENDIF
 SET dm2_install_schema->dbase_name = ddt_dbname
 IF (ddt_get_tspace_to_drop(null)=0)
  GO TO exit_program
 ENDIF
 IF (size(ddt_ts->tsp,5)=0)
  SET dm_err->eproc = "No tablespaces available to drop."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF ((ddt_ts->drop_cnt=0))
  SET dm_err->eproc = "No empty tablespaces available to drop."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (ddt_database_remote=1)
  IF (ddt_disp_report(null)=0)
   GO TO exit_program
  ENDIF
  WHILE (ddt_finish=0)
   IF (ddt_selection != "V")
    ...
    ENDIF
    IF (size(ddt_ts->tsp,5)=0)
     SET dm_err->eproc = "No work to do, exit script"
     CALL disp_msg(" ",dm_err->logfile,0)
     SET ddt_finish = 1
    ENDIF
   ENDIF
   IF (ddt_finish=0)
    SET dm_err->eproc = "Prompt user for selection"
    SET ddt_selection = ""
    SET message = window
    CALL clear(1,1)
    CALL box(1,1,8,132)
    CALL text(3,2,
     "ERROR:  Not all of the Cerner clinical tablespaces were removed.  Please remove tablespaces before proceeding."
     )
    CALL text(4,2,"(V)iew Tablespace Drop Report, (C)ontinue, (Q)uit: ")
    CALL accept(4,53,"p;cu",""
     WHERE curaccept IN ("V", "C", "Q"))
    SET ddt_selection = curaccept
    SET message = nowindow
    CASE (ddt_selection)
     OF "V":
      SET dm_err->eproc = "User elected to view report."
      CALL disp_msg(" ",dm_err->logfile,0)
      IF (ddt_disp_report(null)=0)
       GO TO exit_program
      ENDIF
     OF "C":
      SET dm_err->eproc = "User elected to continue."
      CALL disp_msg(" ",dm_err->logfile,0)
     OF "Q":
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "User elected to quit from drop tablespace program"
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
    ENDCASE
   ENDIF
  ...
  GO TO exit_program
 ENDIF
 FOR (ddt_ts_cnt = 1 TO size(ddt_ts->tsp,5))
   IF ((ddt_ts->tsp[ddt_ts_cnt].drop_ind=1))
    IF ((ddt_ts->tsp[ddt_ts_cnt].ts_offline=0))
     SET dm_err->eproc = concat("Taking ",ddt_ts->tsp[ddt_ts_cnt].tspace_name,
      " OFFLINE in prep for drop.")
     CALL disp_msg(" ",dm_err->logfile,0)
     SET ddt_str = concat("rdb alter tablespace ",ddt_ts->tsp[ddt_ts_cnt].tspace_name," offline go")
     IF (dm2_push_cmd(ddt_str,1)=0)
      GO TO exit_program
     ENDIF
    ENDIF
    IF ((dir_storage_misc->cur_storage_type IN ("ASM", "AXP")))
     SET dm_err->eproc = concat("Dropping and removing datafiles for:",ddt_ts->tsp[ddt_ts_cnt].
      tspace_name)
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     IF ((dm2_sys_misc->cur_db_os="AXP"))
      SET ddt_str = concat("rdb drop tablespace ",ddt_ts->tsp[ddt_ts_cnt].tspace_name,
       " including contents and datafiles cascade constraints go")
     ELSE
      SET ddt_str = concat("rdb drop tablespace ",ddt_ts->tsp[ddt_ts_cnt].tspace_name,
       " including contents cascade constraints go")
     ENDIF
     IF (dm2_push_cmd(ddt_str,1)=0)
      GO TO exit_program
     ENDIF
    ELSE
     SET dm_err->eproc = concat("Dropping:",ddt_ts->tsp[ddt_ts_cnt].tspace_name)
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     SET ddt_str = concat("rdb drop tablespace ",ddt_ts->tsp[ddt_ts_cnt].tspace_name,
     ...
     IF (dm2_push_cmd(ddt_str,1)=0)
      GO TO exit_program
     ENDIF
     FOR (ddt_lv_cnt = 1 TO size(ddt_ts->tsp[ddt_ts_cnt].df,5))
       SET dm_err->eproc = concat("Removing:",ddt_ts->tsp[ddt_ts_cnt].df[ddt_lv_cnt].df_drop)
       IF ((dm_err->debug_flag > 0))
        CALL disp_msg(" ",dm_err->logfile,0)
       ENDIF
       IF ((dm2_sys_misc->cur_db_os="AIX"))
        SET ddt_str = concat("rmlv -f ",ddt_ts->tsp[ddt_ts_cnt].df[ddt_lv_cnt].df_drop)
       ELSEIF ((dm2_sys_misc->cur_db_os="HPX"))
        SET ddt_str = concat("lvremove -f ",ddt_ts->tsp[ddt_ts_cnt].df[ddt_lv_cnt].df_drop)
       ENDIF
       IF (dm2_push_dcl(ddt_str)=0)
        GO TO exit_program
       ENDIF
     ENDFOR
    ENDIF
 ...
 ENDFOR
 GO TO exit_program
 SUBROUTINE ddt_get_tspace_to_drop(null)
   DECLARE dgttd_add_to_list = i2 WITH protect, noconstant(0)
   FOR (ddt_ts_cnt = 1 TO size(ddt_ts->tsp,5))
     SET stat = alterlist(ddt_ts->tsp[ddt_ts_cnt].df,0)
   ENDFOR
   SET stat = alterlist(ddt_ts->tsp,0)
   SET ddt_ts->drop_cnt = 0
   ...
   SELECT DISTINCT INTO "nl:"
    ...
    HEAD REPORT
     ddt_ts_cnt = 0
    DETAIL
     IF (substring(1,2,d.tablespace_name) IN ("D_", "L_", "I_")
      AND d.tablespace_name != "D_SYSCERXDB")
      ddt_ts_cnt = (ddt_ts_cnt+ 1)
      IF (mod(ddt_ts_cnt,50)=1)
       stat = alterlist(ddt_ts->tsp,(ddt_ts_cnt+ 49))
      ENDIF
      ddt_ts->tsp[ddt_ts_cnt].tspace_name = d.tablespace_name
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 2))
    CALL echorecord(ddt_ts)
   ENDIF
   SET dm_err->eproc = "Get list of default tablespaces from DBA_USERS"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT DISTINCT INTO "nl:"
    ...
    FROM dba_users d
    ORDER BY d.default_tablespace
    DETAIL
     IF (substring(1,2,d.default_tablespace) IN ("D_", "L_", "I_")
      AND d.default_tablespace != "D_SYSCERXDB")
      ddt_ndx = 0, ddt_ndx = locateval(ddt_ndx,1,size(ddt_ts->tsp,5),d.default_tablespace,ddt_ts->
       tsp[ddt_ndx].tspace_name)
      IF (ddt_ndx=0)
       ddt_ts_cnt = (ddt_ts_cnt+ 1)
       IF (mod(ddt_ts_cnt,50)=1)
        stat = alterlist(ddt_ts->tsp,(ddt_ts_cnt+ 49))
       ENDIF
       ddt_ts->tsp[ddt_ts_cnt].tspace_name = d.default_tablespace
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   ...
   IF ((dm_err->debug_flag > 0))
    IF ((dtrt->cnt > 0))
     SET dm_err->eproc = concat(
      "Empty tablespaces that qualify for drop are those tablespaces slated to store Millennium (v500) ",
      "objects and that are NOT used as a database user's default tablespace.  This will prevent 'custom' tablespaces ",
      "and tablespaces used as a database user's default tablespace from being dropped.")
     CALL disp_msg(" ",dm_err->logfile,0)
    ELSE
     SET dm_err->eproc = concat("Empty tablespaces that qualify for drop are those tablespaces ",
      "that are NOT used as a database user's default tablespace.  This will not prevent 'custom' tablespaces ",
      "from being dropped, only tablespaces used as a database user's default tablespace will NOT be dropped."
      )
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
   ENDIF
   SET dm_err->eproc =
   "Get list of datafile names for tablespaces that are not in list from dba_segments"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    dbf_file_len = textlen(u.file_name)
    FROM dba_data_files u
    ORDER BY u.tablespace_name
    HEAD REPORT
     ddt_ndx = 0, ddt_lv_cnt = 0
    HEAD u.tablespace_name
     dgttd_add_to_list = 0
     IF (substring(1,2,u.tablespace_name) IN ("D_", "L_", "I_")
      AND u.tablespace_name != "D_SYSCERXDB")
      ddt_ndx = 0, ddt_ndx = locateval(ddt_ndx,1,size(ddt_ts->tsp,5),u.tablespace_name,ddt_ts->tsp[
       ddt_ndx].tspace_name)
      IF (ddt_ndx=0)
       IF ((dtrt->cnt > 0))
        ddt_ndx = 0, ddt_ndx = locateval(ddt_ndx,1,dtrt->cnt,u.tablespace_name,dtrt->qual[ddt_ndx].
         tspace_name)
        IF (ddt_ndx > 0)
         dgttd_add_to_list = 1
        ENDIF
       ELSE
        dgttd_add_to_list = 1
       ENDIF
       IF (dgttd_add_to_list=1)
        ddt_ts_cnt = (ddt_ts_cnt+ 1)
        IF (mod(ddt_ts_cnt,50)=1)
         stat = alterlist(ddt_ts->tsp,(ddt_ts_cnt+ 49))
        ENDIF
        ddt_ts->tsp[ddt_ts_cnt].tspace_name = u.tablespace_name, ddt_ts->tsp[ddt_ts_cnt].drop_ind = 1
        IF (u.status="OFFLINE")
         ddt_ts->tsp[ddt_ts_cnt].ts_offline = 1
        ENDIF
        ddt_ts->drop_cnt = (ddt_ts->drop_cnt+ 1)
       ENDIF
      ENDIF
     ENDIF
     ddt_lv_cnt = 0
    DETAIL
     IF (dgttd_add_to_list=1
      AND (dir_storage_misc->cur_storage_type="RAW"))
      ddt_lv_cnt = (ddt_lv_cnt+ 1), stat = alterlist(ddt_ts->tsp[ddt_ts_cnt].df,ddt_lv_cnt), ddt_spot
       = 0
      IF ((dm2_sys_misc->cur_db_os IN ("AXP")))
       ddt_ts->tsp[ddt_ts_cnt].df[ddt_lv_cnt].df_name = u.file_name
      ELSEIF ((dm2_sys_misc->cur_db_os="AIX"))
       ddt_spot = (findstring("/r",u.file_name,1,1)+ 1)
       IF (ddt_spot=1)
        ddt_spot = findstring("/",u.file_name,1,1)
       ENDIF
       ddt_spot_end = dbf_file_len, ddt_ts->tsp[ddt_ts_cnt].df[ddt_lv_cnt].df_name = substring((
        ddt_spot+ 1),(ddt_spot_end - ddt_spot),u.file_name), ddt_ts->tsp[ddt_ts_cnt].df[ddt_lv_cnt].
       df_drop = substring((ddt_spot+ 1),(ddt_spot_end - ddt_spot),u.file_name)
      ELSEIF ((dm2_sys_misc->cur_db_os="HPX"))
       ddt_ts->tsp[ddt_ts_cnt].df[ddt_lv_cnt].df_drop = replace(u.file_name,"/r","/",2), ddt_ts->tsp[
       ddt_ts_cnt].df[ddt_lv_cnt].df_name = u.file_name
      ENDIF
     ENDIF
    FOOT REPORT
     stat = alterlist(ddt_ts->tsp,ddt_ts_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddt_disp_report(null)
   SET dm_err->eproc = "Since running from a remote database, just display the remote report."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "MINE"
    FROM (dummyt d  WITH seq = size(ddt_ts->tsp,5))
    PLAN (d
     WHERE d.seq > 0)
    HEAD REPORT
     col 0, "TABLESPACE DROP REPORT", row + 3,
     col 0,
     "The following report lists existing Cerner clinical tablespaces that do not contain objects.",
     row + 2,
     col 0,
     "The following tablespaces and logical volumes need to be removed before proceeding.   A drop ",
     row + 1,
     col 0,
     "tablespace SQL command example is located at the bottom of this report (Note:  Issuing the below drop ",
     row + 1,
     col 0, "tablespace command does not remove the logical volume).", row + 2,
     col 0, "Tablespace Name", col 32,
     "Logical Volumes", row + 1, col 0,
     "__________________", col 32, "__________________",
     row + 1, col 0, " ",
     row + 1, col 0, " ",
     row + 1
    DETAIL
     IF ((ddt_ts->tsp[d.seq].drop_ind=1))
      col 0, ddt_ts->tsp[d.seq].tspace_name
      FOR (ddd_cnt = 1 TO size(ddt_ts->tsp[d.seq].df,5))
        col 32, ddt_ts->tsp[d.seq].df[ddd_cnt].df_name, row + 1
      ENDFOR
     ENDIF
    FOOT REPORT
     col 0, " ", row + 1,
     ...
     col 0, "SQL> drop tablespace <TABLESPACE NAME> including contents cascade constraints;", row + 1
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "dm2_drop_tspaces completed"
 IF (ddt_standalone=1)
  ...
  CALL disp_msg("DATABASE CONNECTION REMOVED. RE-ENTER CCL TO ESTABLISH NEW CONNECTION.",dm_err->
   logfile,1)
 ENDIF
 CALL final_disp_msg("dm2_drop_tspaces")
END GO
