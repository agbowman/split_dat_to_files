CREATE PROGRAM dm_eks_dlg_event_rows:dba
 ...
 DECLARE blnspecrows = i2 WITH protect, noconstant(0)
 DECLARE strwhereclause = vc WITH protect
 ...
 FOR (tok_ndx = 1 TO size(request->tokens,5))
  ...
  ENDIF
  IF ((request->tokens[tok_ndx].token_str="ROWSTOPURGE"))
   SET blnspecrows = 1
   CASE (request->tokens[tok_ndx].value)
    OF "1":
     SET strwhereclause = " !='MUL_MED!*'"
    OF "2":
     SET strwhereclause = " ='MUL_MED!*'"
    ELSE
     SET blnspecrows = 0
   ...
   SELECT
    IF (blnspecrows)
     ...
     WHERE parser(sbr_getrowidnotexists("ede.dlg_event_id between curMinID and curMaxID","ede"))
      AND parser(build("ede.dlg_name",strwhereclause))
      AND ede.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
    ELSE
     FROM eks_dlg_event ede
     WHERE parser(sbr_getrowidnotexists("ede.dlg_event_id between curMinID and curMaxID","ede"))
      AND ede.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
    ...
    WITH nocounter, maxqual(ede,value(rowsleft))
   ;end select
