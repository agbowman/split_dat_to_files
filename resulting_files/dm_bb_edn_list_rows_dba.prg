CREATE PROGRAM dm_bb_edn_list_rows:dba
 ...
 DECLARE min_days_to_keep = i4 WITH protect, constant(365)
 DECLARE daystokeep = i4 WITH protect, noconstant(0)
 ...
 SET reply->table_name = "BB_EDN_ADMIN"
 SET reply->rows_between_commit = 50
 FOR (tokenidx = 1 TO size(request->tokens,5))
   IF ((request->tokens[tokenidx].token_str="DAYSTOKEEP"))
    SET daystokeep = ceil(cnvtreal(request->tokens[tokenidx].value))
    SET tokenidx = size(request->tokens,5)
   ENDIF
 ENDFOR
 DECLARE earliest_admin_dt_tm = dq8 WITH protect, constant(datetimeadd(current_dt_tm,(0 - daystokeep)
   ))
 IF (daystokeep < min_days_to_keep)
  SET reply->status_data.status = "F"
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"DAYS_ERROR",
 ...
 SELECT INTO "nl:"
  ...
  WHERE bea.admin_dt_tm < cnvtdatetime(earliest_admin_dt_tm)
   AND bea.edn_complete_ind=1
   AND parser(sbr_getrowidnotexists("bea.bb_edn_admin_id+0 > 0","bea"))
  DETAIL
   rowcnt = (rowcnt+ 1)
   IF (size(reply->rows,5) < rowcnt)
    stat = alterlist(reply->rows,(rowcnt+ 49))
   ENDIF
   reply->rows[rowcnt].row_id = bea.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,rowcnt)
  WITH nocounter, maxqual(admin,value(request->max_rows))
 ;end select
 ...
 IF (lerrcode != 0)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERR",
   "Failed in collection of rows for purging: %1","s",nullterm(serrmsg))
  GO TO exit_script
 ENDIF
