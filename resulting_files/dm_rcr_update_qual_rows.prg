CREATE PROGRAM dm_rcr_update_qual_rows
 ...
 SET reply->table_name = "RCR_UPDATE_QUAL"
 ...
 FOR (v_token_idx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 183)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must keep at least 183 days worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(ruq.rcr_update_qual_id)
   FROM rcr_update_qual ruq
   WHERE ruq.rcr_update_qual_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(ruq.rcr_update_qual_id)
  FROM rcr_update_qual ruq
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    ruq.rowid
    FROM rcr_update_qual ruq
    WHERE ruq.rcr_update_qual_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND ruq.create_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND parser(sbr_getrowidnotexists("ruq.rcr_update_qual_id != 0","ruq"))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,500)=1)
      stat = alterlist(reply->rows,(v_rows+ 499))
     ENDIF
     reply->rows[v_rows].row_id = ruq.rowid
    WITH nocounter, maxqual(ruq,value(v_rows_left))
   ;end select
