CREATE PROGRAM dm_rmc_query_parser:dba
 ...
 SUBROUTINE drqp_remove_comments(drc_text)
   ...
   RETURN(null)
 END ;Subroutine
 DECLARE drqp_file_name = vc WITH protect, noconstant("")
 DECLARE drqp_start_qual = i4 WITH protect, noconstant(0)
 DECLARE drqp_stop_qual = i4 WITH protect, noconstant(0)
 DECLARE drqp_select_qual = i4 WITH protect, noconstant(0)
 DECLARE drqp_select_char = i4 WITH protect, noconstant(0)
 DECLARE drqp_idx = i4 WITH protect, noconstant(0)
 DECLARE drqp_file_ind = i2 WITH protect, noconstant(0)
 DECLARE drqp_temp = vc WITH protect, noconstant(" ")
 DECLARE drqp_line_cnt = i4 WITH protect, noconstant(0)
 DECLARE drqp_fname = vc WITH protect, noconstant("")
 DECLARE drqp_start_pos = i4 WITH protect, noconstant(0)
 DECLARE drqp_stop_pos = i4 WITH protect, noconstant(0)
 DECLARE drqp_search_str = vc WITH protect, noconstant(" ")
 DECLARE drqp_replace_str = vc WITH protect, noconstant(" ")
 DECLARE drqp_search_pos = i4 WITH protect, noconstant(0)
 DECLARE drqp_search_qual = i4 WITH protect, noconstant(0)
 DECLARE drqp_done_ind = i2 WITH protect, noconstant(0)
 DECLARE drqp_orig_search = i2 WITH protect, noconstant(0)
 FREE RECORD drqp_text
 RECORD drqp_text(
   1 cnt = i4
   1 qual[*]
     2 text = vc
 )
 IF (check_logfile("dm_rmc_query",".log","DM_RMC_QUERY_PARSER LOG")=0)
  SET dm_err->err_ind = 1
 ...
 IF (reflect(parameter(1,0)) != "C*")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = 'Expected syntax:  dm_rmc_query_parser "<FILE_NAME>" go'
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSE
  SET drqp_file_name = trim( $1,3)
 ENDIF
 IF (drqp_file_name <= " ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("The parameter passed in did not contain a file_name.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (findstring(":",drqp_file_name)=0
  AND findstring("/",drqp_file_name)=0)
  SET drqp_file_name = concat("CCLUSERDIR:",trim(drqp_file_name,3))
 ENDIF
 SET drqp_file_ind = findfile(value(drqp_file_name),4)
 IF (drqp_file_ind=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("The ",drqp_file_name,
   " file could not be found, or script did not have read access on it.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET logical drqp_fname value(drqp_file_name)
 FREE DEFINE rtl2
 DEFINE rtl2 "drqp_fname"
 SELECT INTO "NL:"
  t.line
  FROM rtl2t t
  HEAD REPORT
   stat = alterlist(drqp_text->qual,10), drqp_text->cnt = 0
  DETAIL
   drqp_temp = t.line
   IF (size(trim(drqp_temp)) > 0)
    drqp_text->cnt = (drqp_text->cnt+ 1)
    IF (mod(drqp_text->cnt,10)=1
     AND (drqp_text->cnt >= 10))
     stat = alterlist(drqp_text->qual,(drqp_text->cnt+ 9))
    ENDIF
    drqp_line_cnt = (drqp_line_cnt+ 1), drqp_text->qual[drqp_text->cnt].text = t.line
   ENDIF
  FOOT REPORT
   stat = alterlist(drqp_text->qual,drqp_text->cnt)
 ...
 IF ((drqp_text->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No query was found in the ",drqp_file_name," file.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 CALL drqp_remove_comments(drqp_text)
 SET drqp_pos->qual_pos = 0
 SET drqp_pos->char_pos = 0
 CALL drqp_find_clause("DETAIL",drqp_text,1,1,drqp_pos,
  0)
 IF ((((drqp_pos->qual_pos > 0)) OR ((drqp_pos->char_pos > 0))) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("A DETAIL clause was found in the ",drqp_file_name,
   " file.  This query is not in a supported format")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET drqp_pos->qual_pos = 0
 SET drqp_pos->char_pos = 0
 CALL drqp_find_clause("GROUP BY",drqp_text,1,1,drqp_pos,
  0)
 IF ((((drqp_pos->qual_pos > 0)) OR ((drqp_pos->char_pos > 0))) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("A GROUP BY clause was found in the ",drqp_file_name,
   " file.  This query is not in a supported format")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET drqp_pos->qual_pos = 0
 SET drqp_pos->char_pos = 0
 CALL drqp_find_clause("SELECT",drqp_text,1,1,drqp_pos,
  2)
 IF (check_error("Looking for SELECT clause") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((drqp_pos->qual_pos=0)
  ...
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("The SELECT clause was not found in the ",drqp_file_name," file.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET drqp_done_ind = 1
  GO TO exit_script
 ELSE
  SET drqp_select_qual = drqp_pos->qual_pos
  SET drqp_select_char = drqp_pos->char_pos
 ENDIF
 SET drqp_temp = ""
 SET drqp_pos->qual_pos = 0
 SET drqp_pos->char_pos = 0
 CALL drqp_find_clause("FROM",drqp_text,drqp_select_qual,drqp_select_char,drqp_pos,
  0)
 ...
 IF ((drqp_pos->qual_pos=0)
  AND (drqp_pos->char_pos=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("The FROM clause was not found in the ",drqp_file_name," file.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET drqp_start_pos = drqp_pos->char_pos
 SET drqp_start_qual = drqp_pos->qual_pos
 SET drqp_temp = ""
 IF (drqp_start_qual=drqp_select_qual)
  SET drqp_temp = trim(substring((drqp_select_char+ 6),(drqp_start_pos - (drqp_select_char+ 6)),
    drqp_text->qual[drqp_start_qual].text),3)
  SET drqp_text->qual[drqp_start_qual].text = replace(drqp_text->qual[drqp_start_qual].text,drqp_temp,
   "<COLUMN_LIST>",0)
 ELSE
  FOR (drqp_idx = drqp_select_qual TO drqp_start_qual)
    IF (drqp_idx=drqp_select_qual)
     SET drqp_temp = trim(substring((drqp_select_char+ 6),size(drqp_text->qual[drqp_idx].text),
       drqp_text->qual[drqp_idx].text),3)
     SET drqp_text->qual[drqp_idx].text = concat(trim(substring(1,(drqp_select_char+ 6),drqp_text->
        qual[drqp_idx].text),3)," <COLUMN_LIST>")
    ELSEIF (drqp_idx=drqp_start_qual)
     SET drqp_temp = concat(drqp_temp," ",trim(substring(1,(drqp_start_pos - 1),drqp_text->qual[
        drqp_idx].text),3))
     SET drqp_text->qual[drqp_idx].text = trim(substring(drqp_start_pos,size(drqp_text->qual[drqp_idx
        ].text),drqp_text->qual[drqp_idx].text),3)
    ELSE
     SET drqp_temp = concat(drqp_temp," ",trim(drqp_text->qual[drqp_idx].text,3))
     SET drqp_text->qual[drqp_idx].text = ""
    ENDIF
  ENDFOR
 ENDIF
 IF ((drqp_text->cnt > 1))
  SET drqp_idx = 1
  WHILE ((drqp_idx < (drqp_text->cnt+ 1)))
   IF ((drqp_text->qual[drqp_idx].text <= " "))
    FOR (drqp_line_cnt = (drqp_idx+ 1) TO drqp_text->cnt)
      SET drqp_text->qual[(drqp_line_cnt - 1)].text = drqp_text->qual[drqp_line_cnt].text
    ENDFOR
    SET drqp_text->cnt = (drqp_text->cnt - 1)
    SET drqp_idx = (drqp_idx - 1)
   ENDIF
   SET drqp_idx = (drqp_idx+ 1)
  ENDWHILE
 ENDIF
 SET stat = alterlist(drqp_text->qual,drqp_text->cnt)
 IF (cnvtupper(drqp_text->qual[drqp_text->cnt].text)=patstring("* GO"))
  SET drqp_text->qual[drqp_text->cnt].text = trim(substring(1,(size(drqp_text->qual[drqp_text->cnt].
     text) - 2),drqp_text->qual[drqp_text->cnt].text))
 ELSEIF (cnvtupper(drqp_text->qual[drqp_text->cnt].text)="GO")
  SET drqp_text->cnt = (drqp_text->cnt - 1)
  SET stat = alterlist(drqp_text->qual,drqp_text->cnt)
 ENDIF
 SET drqp_reply->cnt = drqp_text->cnt
 SET stat = alterlist(drqp_reply->qual,drqp_reply->cnt)
 FOR (drqp_idx = 1 TO drqp_reply->cnt)
   SET drqp_reply->qual[drqp_idx].text = drqp_text->qual[drqp_idx].text
 ENDFOR
 CALL drqp_parse_columns(drqp_temp,drqp_reply)
 IF (check_error("Parsing column list apart") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET drqp_done_ind = 0
 SET drqp_search_pos = 1
 SET drqp_search_qual = 1
 SET drqp_orig_search = 1
 WHILE (drqp_done_ind=0)
   SET drqp_temp = ""
   SET drqp_pos->qual_pos = 0
   SET drqp_pos->char_pos = 0
   CALL drqp_find_clause("FROM",drqp_text,drqp_search_qual,drqp_search_pos,drqp_pos,
    0)
   IF (check_error("Looking for FROM clause") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ...
   IF ((drqp_pos->qual_pos=0)
    AND (drqp_pos->char_pos=0)
    AND drqp_orig_search=1)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("The FROM clause was not found in the ",drqp_file_name," file.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET drqp_done_ind = 1
    GO TO exit_script
   ELSEIF ((drqp_pos->qual_pos=0)
    AND (drqp_pos->char_pos=0)
    AND drqp_orig_search=0)
    SET drqp_done_ind = 1
   ENDIF
   IF (drqp_done_ind=0)
    SET drqp_start_pos = drqp_pos->char_pos
    ...
    SET drqp_pos->qual_pos = 0
    SET drqp_pos->char_pos = 0
    CALL drqp_find_clause("WHERE",drqp_text,drqp_start_qual,(drqp_start_pos+ 1),drqp_pos,
     0)
    IF (check_error("Looking for WHERE clause") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
    IF ((drqp_pos->qual_pos=0)
     AND (drqp_pos->char_pos=0))
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("The WHERE clause was not found in the ",drqp_file_name," file.")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET drqp_done_ind = 1
     GO TO exit_script
    ENDIF
    SET drqp_stop_pos = drqp_pos->char_pos
    SET drqp_stop_qual = drqp_pos->qual_pos
    SET drqp_pos->qual_pos = 0
    SET drqp_pos->char_pos = 0
    CALL drqp_find_clause("PLAN",drqp_text,drqp_start_qual,(drqp_start_pos+ 1),drqp_pos,
     0)
    IF (check_error("Looking for PLAN clause") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
    IF ((drqp_pos->qual_pos > 0)
     ...
     SET dm_err->err_ind = 1
     SET dm_err->emsg =
     "The PLAN/JOIN CCL query syntax is not allowed in this application.  Please change query to not have PLAN/JOIN syntax."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
    IF (drqp_stop_pos=1)
     SET drqp_stop_qual = (drqp_stop_qual - 1)
     SET drqp_stop_pos = size(drqp_text->qual[drqp_stop_qual].text)
    ELSE
     SET drqp_stop_pos = (drqp_stop_pos - 1)
    ENDIF
    SET drqp_orig_search = 0
    SET drqp_search_qual = drqp_stop_qual
    SET drqp_search_pos = drqp_stop_pos
    SET drqp_temp = ""
    IF (drqp_start_qual=drqp_stop_qual)
     SET drqp_temp = trim(substring((drqp_start_pos+ 5),(drqp_stop_pos - (drqp_start_pos+ 4)),
       drqp_text->qual[drqp_start_qual].text),3)
    ELSE
     FOR (drqp_idx = drqp_start_qual TO drqp_stop_qual)
       IF (drqp_idx=drqp_start_qual)
        SET drqp_temp = trim(substring((drqp_start_pos+ 5),size(drqp_text->qual[drqp_idx].text),
          drqp_text->qual[drqp_idx].text),3)
       ELSEIF (drqp_idx=drqp_stop_qual)
        SET drqp_temp = concat(drqp_temp," ",trim(substring(1,drqp_stop_pos,drqp_text->qual[drqp_idx]
           .text),3))
       ELSE
        SET drqp_temp = concat(drqp_temp," ",trim(drqp_text->qual[drqp_idx].text,3))
       ENDIF
     ENDFOR
    ENDIF
    CALL drqp_parse_tables(drqp_temp,drqp_reply)
    IF (check_error("Parsing table list apart") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
    IF ((drqp_reply->tab_cnt=0))
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("No table name's found in query.")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET drqp_done_ind = 1
     GO TO exit_script
    ENDIF
    FOR (drqp_idx = drqp_start_qual TO drqp_stop_qual)
      IF (drqp_idx=drqp_start_qual
       AND drqp_start_pos > 1)
       SET drqp_temp = substring(drqp_start_pos,size(drqp_reply->qual[drqp_idx].text),drqp_reply->
        qual[drqp_idx].text)
      ELSEIF (drqp_idx=drqp_stop_qual
       AND drqp_stop_pos < size(drqp_reply->qual[drqp_idx].text))
       SET drqp_temp = substring(1,drqp_stop_pos,drqp_reply->qual[drqp_idx].text)
      ELSE
       SET drqp_temp = drqp_reply->qual[drqp_idx].text
      ENDIF
      FOR (drqp_cnt = 1 TO drqp_reply->tab_cnt)
        IF ((drqp_reply->tab_qual[drqp_cnt].dummyt_ind=0))
         SET drqp_search_str = drqp_reply->tab_qual[drqp_cnt].table_name
         IF ((drqp_reply->tab_qual[drqp_cnt].table_alias=patstring("sfx*")))
          SET drqp_replace_str = concat(drqp_reply->tab_qual[drqp_cnt].table_name,"<DB_LINK> ",
           drqp_reply->tab_qual[drqp_cnt].table_alias)
          SET drqp_temp = trim(replace(concat(" ",drqp_temp," "),concat(" ",drqp_search_str," ",
             drqp_reply->tab_qual[drqp_cnt].table_alias," "),concat(" ",drqp_replace_str," "),0),3)
          SET drqp_temp = trim(replace(concat(" ",drqp_temp," "),concat(" ,",drqp_search_str," ",
             drqp_reply->tab_qual[drqp_cnt].table_alias," "),concat(" ,",drqp_replace_str," "),0),3)
          SET drqp_temp = trim(replace(concat(" ",drqp_temp," "),concat(" ",drqp_search_str," ",
             drqp_reply->tab_qual[drqp_cnt].table_alias,", "),concat(" ",drqp_replace_str,", "),0),3)
          CALL echo(concat("Searching: ",drqp_temp," :"))
          CALL echo(concat("For: ",drqp_search_str," :"))
          CALL echo(concat("To replace with: ",drqp_replace_str," :"))
         ELSE
          SET drqp_replace_str = concat(drqp_reply->tab_qual[drqp_cnt].table_name,"<DB_LINK>")
          SET drqp_temp = trim(replace(concat(" ",drqp_temp," "),concat(" ",drqp_search_str," "),
            concat(" ",drqp_replace_str," "),0),3)
          SET drqp_temp = trim(replace(concat(" ",drqp_temp," "),concat(" ,",drqp_search_str," "),
            concat(" ,",drqp_replace_str," "),0),3)
          SET drqp_temp = trim(replace(concat(" ",drqp_temp," "),concat(" ",drqp_search_str,", "),
            concat(" ",drqp_replace_str,", "),0),3)
      ...
      IF (drqp_idx=drqp_start_qual
       AND drqp_start_pos > 1)
       SET drqp_reply->qual[drqp_idx].text = concat(substring(1,(drqp_start_pos - 1),drqp_reply->
         qual[drqp_idx].text),drqp_temp)
      ELSEIF (drqp_idx=drqp_stop_qual
       AND drqp_stop_pos < size(drqp_reply->qual[drqp_idx].text))
       SET drqp_reply->qual[drqp_idx].text = concat(drqp_temp,substring(drqp_stop_pos,size(drqp_reply
          ->qual[drqp_idx].text),drqp_reply->qual[drqp_idx].text))
      ELSE
       SET drqp_reply->qual[drqp_idx].text = drqp_temp
      ENDIF
    ENDFOR
   ENDIF
 ENDWHILE
 FOR (drqp_cnt = 1 TO drqp_reply->tab_cnt)
   SET drqp_reply->tab_qual[drqp_cnt].table_name = cnvtupper(drqp_reply->tab_qual[drqp_cnt].
    table_name)
 ENDFOR
#exit_script
END GO
