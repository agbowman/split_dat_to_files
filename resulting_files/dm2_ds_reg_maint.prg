CREATE PROGRAM dm2_ds_reg_maint
 ...
 END ;Subroutine
 DECLARE ddrm_prelim(null) = i2
 DECLARE ddrm_get_reg_exp(dgre_file_name=vc,dgre_exp_key=vc) = i2
 DECLARE ddrm_get_srv_reg(dgsr_domain_name=vc,dgsr_node_name=vc) = i2
 DECLARE ddrm_get_services(null) = i2
 DECLARE ddrm_perform_reg_updates(dpru_set_key=vc) = i2
 DECLARE ddrm_confirm_reg_updates(dcru_confirm_ret=vc(ref)) = i2
 DECLARE ddrm_confirm_rdbms_reg_update(dcrru_confirm_ret=vc(ref)) = i2
 DECLARE ddrm_create_report(dcr_file_ret=vc(ref)) = i2
 DECLARE ddrm_rdbms_conn_reg_update(drcru_key=vc,drcru_prop=vc,drcru_prop_val=vc) = i2
 DECLARE ddrm_confirm_rdbms_default_connect(dcrdc_confirm_ret=vc(ref)) = i2
 DECLARE ddrm_rpt_file = vc WITH protect, noconstant("")
 DECLARE ddrm_ret_val = vc WITH protect, noconstant("")
 DECLARE ddrm_conn_ret_val = vc WITH protect, noconstant("")
 FREE RECORD srvreg
 RECORD srvreg(
   1 failover_str = vc
   1 cer_reg_formatted = vc
   1 cer_rdbms_connect_val = vc
   1 cer_rdbms_connect_key = vc
   1 cer_default_connect_key = vc
   1 set_key = vc
   1 root_key = vc
   1 platform_val = vc
   1 mode = vc
   ...
   1 domain_name = vc
   1 node_name = vc
   1 default_connect = vc
   1 max_servername_size = i2
   1 oper_cnt = i4
   1 cnt = i2
   1 srv[*]
     2 server_number = i4
     2 server_name = vc
     2 curr_connect_svc = vc
     2 new_connect_svc = vc
     2 managed_ind = i2
     2 create_prop_key = i2
     2 prop_op = vc
 )
 IF (check_logfile("dm2_ds_reg_maint",".log","dm2_ds_reg_maint")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_ds_reg_maint"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (ddrm_prelim(null)=0)
  GO TO exit_program
 ENDIF
 IF (ddrm_get_services(null)=0)
  GO TO exit_program
 ENDIF
 IF (ddrm_create_report(ddrm_rpt_file)=0)
  GO TO exit_program
 ENDIF
 IF (dm2_disp_file(ddrm_rpt_file,"Database Services Report")=0)
  GO TO exit_program
 ENDIF
 WHILE ( NOT (ddrm_ret_val IN ("C", "Q")))
  IF ((srvreg->oper_cnt > 0))
   IF (ddrm_confirm_reg_updates(ddrm_ret_val)=0)
    GO TO exit_program
   ENDIF
  ELSE
   SET ddrm_ret_val = "C"
  ...
  CASE (ddrm_ret_val)
   OF "C":
    IF (ddrm_confirm_rdbms_reg_update(ddrm_conn_ret_val)=0)
     GO TO exit_program
    ENDIF
    CASE (ddrm_conn_ret_val)
     OF "Q":
      SET dm_err->eproc = "User chose to exit from cer_rdbms_connect confirmation."
      CALL disp_msg(" ",dm_err->logfile,0)
      GO TO exit_program
    ENDCASE
    SET ddrm_conn_ret_val = ""
    IF (ddrm_confirm_rdbms_default_connect(ddrm_conn_ret_val)=0)
     GO TO exit_program
    ENDIF
    CASE (ddrm_conn_ret_val)
     OF "Q":
      SET dm_err->eproc = "User chose to exit from default connection confirmation."
      CALL disp_msg(" ",dm_err->logfile,0)
      GO TO exit_program
    ENDCASE
   OF "R":
    ...
    ENDIF
   OF "Q":
    SET dm_err->eproc = "User chose to not perform registry updates."
    CALL disp_msg(" ",dm_err->logfile,0)
    GO TO exit_program
  ENDCASE
 ENDWHILE
 GO TO exit_program
 SUBROUTINE ddrm_prelim(null)
   DECLARE dp_log_ret = vc WITH protect, noconstant("")
   DECLARE dp_dbase_name = vc WITH protect, noconstant("")
   DECLARE dp_str = vc WITH protect, noconstant("")
   DECLARE dp_ret_val = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Perform preliminary setup."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SET srvreg->mode =  $1
   IF ( NOT ((srvreg->mode IN ("ASSIGN ALL", "UNASSIGN ALL"))))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Invalid mode of execution entered.")
    SET dm_err->user_action = 'Valid modes are "ASSIGN ALL" and "UNASSIGN ALL"'
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET srvreg->node_name = trim(curnode)
   IF (ddr_get_env_logical(dp_log_ret)=0)
    RETURN(0)
   ENDIF
   SET srvreg->env_name = dp_log_ret
   IF (ddr_validate_user(dp_log_ret)=0)
    RETURN(0)
   ENDIF
   IF (dm2_get_dbase_name(dp_dbase_name)=0)
    RETURN(0)
   ENDIF
   SET srvreg->db_name = cnvtlower(dp_dbase_name)
   SET dm_err->eproc = "Obtain database instance ids."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM gv$instance g
    ORDER BY g.instance_number DESC
    DETAIL
     srvreg->failover_str = concat(srvreg->failover_str,"/@",trim(g.instance_name))
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Obtain Domain name."
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET dp_str = concat("\\environment\",srvreg->env_name," Domain")
   ELSE
    SET dp_str = concat("\\\environment\\",srvreg->env_name," Domain")
   ENDIF
   IF (ddr_lreg_oper("GET",dp_str,dp_ret_val)=0)
    RETURN(0)
   ENDIF
   IF (dp_ret_val="NOPARMRETURNED")
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Unable to obtain Domain Name:",dp_str)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET srvreg->domain_name = dp_ret_val
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET srvreg->set_key = concat("\\node\",srvreg->node_name,"\domain\",srvreg->domain_name,
     "\servers")
    SET srvreg->root_key = concat("\\system\node\",srvreg->node_name,"\domain\",srvreg->domain_name,
     "\servers")
   ELSE
    SET srvreg->set_key = concat("\\\node\\",srvreg->node_name,"\\domain\\",srvreg->domain_name,
     "\\servers")
    SET srvreg->root_key = concat("\\\system\\node\\",srvreg->node_name,"\\domain\\",srvreg->
     domain_name,"\\servers")
   ENDIF
   SET dm_err->eproc = "Obtain connect option."
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET srvreg->cer_default_connect_key = concat("\\database\",srvreg->db_name,"\node\",srvreg->
     node_name)
   ELSE
    SET srvreg->cer_default_connect_key = concat("\\\database\\",srvreg->db_name,"\\node\\",srvreg->
     node_name)
   ENDIF
   SET dp_str = concat(srvreg->cer_default_connect_key,' "Rdbms Connect Option"')
   IF (ddr_lreg_oper("GET",dp_str,dp_ret_val)=0)
    RETURN(0)
   ENDIF
   IF (dp_ret_val="NOPARMRETURNED")
    SET dm_err->eproc = concat("Using ORACLE_SID for default connect:",logical("ORACLE_SID"))
    CALL disp_msg(" ",dm_err->logfile,0)
    SET srvreg->default_connect = trim(cnvtlower(logical("ORACLE_SID")))
   ELSE
    SET srvreg->default_connect = dp_ret_val
   ENDIF
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET srvreg->cer_reg_formatted = trim(cnvtlower(logical("cer_reg")))
   ELSE
    SET srvreg->cer_reg_formatted = concat(trim(cnvtlower(logical("cer_reg"))),"/")
   ENDIF
   IF (ddrm_get_srv_reg(srvreg->domain_name,srvreg->node_name)=0)
    RETURN(0)
   ENDIF
   CASE (dm2_sys_misc->cur_os)
    OF "AIX":
     SET srvreg->platform_val = "aixrs6000"
    OF "HPX":
     SET srvreg->platform_val = "hpuxia64"
    OF "LNX":
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Not valid for UNIX platform"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    OF "AXP":
     SET srvreg->platform_val = "vmsalpha"
   ENDCASE
   SET srvreg->cer_rdbms_connect_val = concat("/c5/r1/t5/@fail_",srvreg->db_name,"/@active_",srvreg->
    db_name,"/@",
    srvreg->db_name,srvreg->failover_str)
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET srvreg->cer_rdbms_connect_key = concat("\\environment\",srvreg->env_name,"\definitions\",
     srvreg->platform_val,"\environment")
   ELSE
    SET srvreg->cer_rdbms_connect_key = concat("\\\environment\\",srvreg->env_name,"\\definitions\\",
     srvreg->platform_val,"\\environment")
   ENDIF
   SET dp_str = concat(srvreg->cer_rdbms_connect_key," cer_rdbms_connect")
   IF (ddr_lreg_oper("GET",dp_str,dp_ret_val)=0)
    RETURN(0)
   ENDIF
   IF (dp_ret_val="NOPARMRETURNED")
    SET dm_err->eproc = concat("Using default connect:",srvreg->cer_rdbms_connect_val)
    CALL disp_msg(" ",dm_err->logfile,0)
   ELSE
    SET srvreg->cer_rdbms_connect_val = dp_ret_val
   ENDIF
   CALL dm2_process_log_add_detail_text(dpl_host_name,srvreg->node_name)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddrm_rdbms_conn_reg_update(drcru_key,drcru_prop,drcru_prop_val)
   DECLARE drcru_str = vc WITH protect, noconstant
   DECLARE drcru_ret = vc WITH protect, noconstant
   DECLARE drcru_log_id = f8 WITH protect, noconstant(0.0)
   SET dm_err->eproc = concat('Set "',drcru_prop,'" property to ',drcru_prop_val)
   CALL disp_msg("",dm_err->logfile,0)
   SET drcru_str = concat(drcru_key,' "',drcru_prop,'" "',drcru_prop_val,
    '"')
   CALL dm2_process_log_add_detail_text(dpl_cmd,drcru_str)
   CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpl_no_prev_id,1)
   SET drcru_log_id = dm2_process_event_rs->dm_process_event_id
   IF (ddr_lreg_oper("SET",drcru_str,drcru_ret)=0)
    SET dm2_process_event_rs->status = dpl_failure
    SET dm2_process_event_rs->message = concat(dm_err->emsg,". retval:",drcru_ret)
    CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,drcru_log_id,1)
    RETURN(0)
   ENDIF
   SET dm2_process_event_rs->status = dpl_success
   CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,drcru_log_id,1)
   SET drcru_str = concat(drcru_key,' "',drcru_prop,'"')
   CALL dm2_process_log_add_detail_text(dpl_cmd,drcru_str)
   CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpl_no_prev_id,1)
   SET drcru_log_id = dm2_process_event_rs->dm_process_event_id
   IF (ddr_lreg_oper("GET",drcru_str,drcru_ret)=0)
    SET dm2_process_event_rs->status = dpl_failure
    SET dm2_process_event_rs->message = concat(dm_err->emsg,". retval:",drcru_ret)
    CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,drcru_log_id,1)
    RETURN(0)
   ENDIF
   IF (cnvtlower(trim(drcru_ret,3)) != cnvtlower(trim(drcru_prop_val,3)))
    SET message = nowindow
    SET dm2_process_event_rs->status = dpl_failure
    SET dm2_process_event_rs->message = concat(dm_err->emsg,". retval:",drcru_ret)
    CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,drcru_log_id,1)
    SET dm_err->emsg = concat('Unable to set "',drcru_prop,'" property to ',drcru_prop_val)
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddrm_confirm_rdbms_default_connect(dcrdc_confirm_ret)
   DECLARE dcrdc_str = vc WITH protect, noconstant("")
   DECLARE dcrdc_default = vc WITH protect, noconstant("")
   DECLARE dcrdc_current = vc WITH protect, noconstant("")
   SET dcrdc_current = srvreg->default_connect
   SET dcrdc_default = concat("@active_",srvreg->db_name)
   SET message = window
   SET width = 132
   WHILE ( NOT (dcrdc_confirm_ret IN ("C", "Q")))
     CALL clear(1,1)
     CALL box(1,1,18,131)
     CALL text(2,2,"Manage Millennium Server Default Database Connection")
     SET dcrdc_str = concat("Environment: ",srvreg->env_name)
     CALL text(4,2,dcrdc_str)
     SET dcrdc_str = concat("Domain     : ",srvreg->domain_name)
     CALL text(5,2,dcrdc_str)
     SET dcrdc_str = concat("Database   : ",srvreg->env_name)
     CALL text(6,2,dcrdc_str)
     CALL text(8,2,"Please confirm the following registry entry update:")
     SET dcrdc_str = concat("Key: ",srvreg->cer_default_connect_key)
     CALL text(10,2,dcrdc_str)
     CALL text(11,2,"Property: Rdbms Connect Option")
     SET dcrdc_str = concat("Current Value: ",dcrdc_current)
     CALL text(12,2,dcrdc_str)
     SET dcrdc_str = concat("New Value: ",dcrdc_default)
     CALL text(13,2,dcrdc_str)
     CALL text(15,2,"(C)ontinue, (M)odify, (Q)uit: ")
     CALL accept(15,33,"A;cu"," "
      WHERE curaccept IN ("C", "M", "Q"))
     SET dcrdc_confirm_ret = curaccept
     IF (dcrdc_confirm_ret="M")
      CALL accept(13,13,"P(90);C",dcrdc_default
       WHERE  NOT (trim(curaccept)=""))
      SET srvreg->default_connect = curaccept
      SET dcrdc_default = curaccept
     ENDIF
   ENDWHILE
   CALL clear(1,1)
   SET message = nowindow
   IF ( NOT (dcrdc_confirm_ret IN ("M", "Q"))
    AND dcrdc_current != dcrdc_default)
    SET srvreg->default_connect = dcrdc_default
    IF (ddrm_rdbms_conn_reg_update(srvreg->cer_default_connect_key,"Rdbms Connect Option",srvreg->
     default_connect)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddrm_confirm_rdbms_reg_update(dcrru_confirm_ret)
   DECLARE dcrru_str = vc WITH protect, noconstant("")
   DECLARE dcrru_default = vc WITH protect, noconstant("")
   DECLARE dcrru_current = vc WITH protect, noconstant("")
   SET message = window
   SET width = 132
   SET dcrru_current = srvreg->cer_rdbms_connect_val
   SET dcrru_default = concat("/c5/r1/t5/@fail_",srvreg->db_name,"/@active_",srvreg->db_name,"/@",
    srvreg->db_name,srvreg->failover_str)
   WHILE ( NOT (dcrru_confirm_ret IN ("C", "Q")))
     CALL clear(1,1)
     CALL box(1,1,18,131)
     CALL text(2,2,"Manage Millennium Server Usage of Cerner Database Services")
     SET dcrru_str = concat("Environment: ",srvreg->env_name)
     CALL text(4,2,dcrru_str)
     SET dcrru_str = concat("Domain     : ",srvreg->domain_name)
     CALL text(5,2,dcrru_str)
     SET dcrru_str = concat("Database   : ",srvreg->env_name)
     CALL text(6,2,dcrru_str)
     CALL text(8,2,"Please confirm the following registry entry update:")
     SET dcrru_str = concat("Key: ",srvreg->cer_rdbms_connect_key)
     CALL text(10,2,dcrru_str)
     CALL text(11,2,"Property: cer_rdbms_connect")
     SET dcrru_str = concat("Current Value: ",dcrru_current)
     CALL text(12,2,dcrru_str)
     SET dcrru_str = concat("New Value: ",dcrru_default)
     CALL text(13,2,dcrru_str)
     CALL text(15,2,"(C)ontinue, (M)odify, (Q)uit: ")
     CALL accept(15,33,"A;cu"," "
      WHERE curaccept IN ("C", "M", "Q"))
     SET dcrru_confirm_ret = curaccept
     IF (dcrru_confirm_ret="M")
      CALL accept(13,13,"P(90);C",dcrru_default
       WHERE  NOT (trim(curaccept)=""))
      SET dcrru_default = curaccept
      SET srvreg->cer_rdbms_connect_val = curaccept
     ENDIF
   ENDWHILE
   CALL clear(1,1)
   SET message = nowindow
   IF ( NOT (dcrru_confirm_ret IN ("M", "Q"))
    AND dcrru_current != dcrru_default)
    SET srvreg->cer_rdbms_connect_val = dcrru_default
    IF (ddrm_rdbms_conn_reg_update(srvreg->cer_rdbms_connect_key,"cer_rdbms_connect",srvreg->
     cer_rdbms_connect_val)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddrm_confirm_reg_updates(dcru_confirm_ret)
   SET message = window
   SET width = 132
   CALL clear(1,1)
   CALL box(1,1,10,131)
   CALL text(2,2,"Manage Millennium Server Usage of Cerner Database Services")
   CALL text(4,2,
    "Registry updates are about to be made that will affect how Millennium servers connect to the database."
    )
   CALL text(5,2,"Please review the registry updates before proceeding.")
   CALL text(8,2,"(C)ontinue, (R)eview updates, (Q)uit: ")
   CALL accept(8,41,"A;cu"," "
    WHERE curaccept IN ("C", "R", "Q"))
   SET dcru_confirm_ret = curaccept
   CALL clear(1,1)
   SET message = nowindow
   IF (dcru_confirm_ret="C")
    IF (ddrm_perform_reg_updates(srvreg->set_key)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddrm_create_report(dcr_file_ret)
   DECLARE dcr_cnt = i4 WITH protect, noconstant(0)
   DECLARE dcr_line = vc WITH protect, noconstant(fillstring(129,"-"))
   DECLARE dcr_str = vc WITH protect, noconstant("")
   IF (get_unique_file("ddrm_reg_update",".rpt")=0)
    RETURN(0)
   ELSE
    SET dcr_file_ret = dm_err->unique_fname
   ENDIF
   SET dm_err->eproc = "Create report file for displaying registry settings to user."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO value(dcr_file_ret)
    HEAD REPORT
     row + 1, dcr_col = 0, col 1,
     "Manage Millennium Application Server Usage of Cerner Database Services", row + 2, col 1,
     "This process will manage the assignment of Cerner Database Services to Millennium Application Servers.",
     row + 1, col 1,
     "The following assignments will determine how a Millennium Application Server will connect to the database.",
     row + 1, col 1,
     'Upon confirmation, this process will assign (if different) the "New Database Service" to the listed ',
     row + 1, col 1,
     "Millennium Applcation server in the Cerner registry. Currently running servers will not utilize the ",
     row + 1, col 1,
     "newly assigned Cerner Database Service until they are cycled (a reconnection occurs).", row + 2,
     col 1,
     "Environment: ", srvreg->env_name, row + 1,
     col 1, "Domain     : ", srvreg->domain_name,
     row + 1, col 1, "Database   : ",
     srvreg->db_name, row + 2, col 1,
     "Note: Asterisk denotes a registry operation will assign New Database Service Name to Current Database Service Name.",
     row + 1, dcr_str = concat(trim(cnvtstring(srvreg->oper_cnt))," of ",trim(cnvtstring(srvreg->cnt)
       )," servers require updates."),
     col 7, dcr_str, row + 3,
     col 1, "Server", col 25,
     "Server", call reportmove('COL',(26+ srvreg->max_servername_size),0), "Current",
     call reportmove('COL',((25+ srvreg->max_servername_size)+ 22),0), "New", call reportmove('COL',(
     ((25+ srvreg->max_servername_size)+ 22)+ 22),0),
     "Update", row + 1, col 1,
     "Number", col 25, "Name",
     call reportmove('COL',(26+ srvreg->max_servername_size),0), "Database Service", call reportmove(
     'COL',((25+ srvreg->max_servername_size)+ 22),0),
     "Database Service", call reportmove('COL',(((25+ srvreg->max_servername_size)+ 22)+ 22),0),
     "Required",
     row + 1, col 1, dcr_line,
     row + 2
    DETAIL
     FOR (dcr_cnt = 1 TO srvreg->cnt)
       col 1, srvreg->srv[dcr_cnt].server_number, col 25,
       srvreg->srv[dcr_cnt].server_name
       IF ((srvreg->srv[dcr_cnt].curr_connect_svc=""))
        dcr_str = "<Not Assigned>"
       ELSE
        dcr_str = srvreg->srv[dcr_cnt].curr_connect_svc
       ENDIF
       call reportmove('COL',(26+ srvreg->max_servername_size),0), dcr_str
       IF ((srvreg->srv[dcr_cnt].new_connect_svc=""))
        dcr_str = "<Not Assigned>"
       ELSE
        dcr_str = srvreg->srv[dcr_cnt].new_connect_svc
       ENDIF
       call reportmove('COL',((25+ srvreg->max_servername_size)+ 22),0), dcr_str
       IF ((srvreg->srv[dcr_cnt].prop_op != ""))
        dcr_str = "*", call reportmove('COL',(((25+ srvreg->max_servername_size)+ 22)+ 22),0),
        dcr_str
       ENDIF
       row + 1
     ENDFOR
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddrm_perform_reg_updates(dpru_set_key)
   DECLARE dpru_str = vc WITH protect, noconstant("")
   DECLARE dpru_cnt = i4 WITH protect, noconstant(0)
   DECLARE dpru_ret = vc WITH protect, noconstant("")
   DECLARE dpru_log_id = f8 WITH protect, noconstant(0.0)
   FOR (dpru_cnt = 1 TO srvreg->cnt)
     IF ((srvreg->srv[dpru_cnt].managed_ind=1)
      AND (srvreg->srv[dpru_cnt].prop_op="ADD")
      AND (srvreg->srv[dpru_cnt].create_prop_key=1))
      IF ((dm2_sys_misc->cur_os="AXP"))
       SET dpru_str = concat(dpru_set_key,"\",trim(cnvtstring(srvreg->srv[dpru_cnt].server_number)),
        "\prop")
      ELSE
       SET dpru_str = concat(dpru_set_key,"\\",trim(cnvtstring(srvreg->srv[dpru_cnt].server_number)),
        "\\prop")
      ENDIF
      CALL dm2_process_log_add_detail_text(dpl_cmd,dpru_str)
      CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpl_no_prev_id,1)
      SET dpru_log_id = dm2_process_event_rs->dm_process_event_id
      IF (ddr_lreg_oper("CREATE",dpru_str,dpru_ret)=0)
       SET dm2_process_event_rs->status = dpl_failure
       SET dm2_process_event_rs->message = concat(dm_err->emsg,". retval:",dpru_ret)
       CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpru_log_id,1)
       RETURN(0)
      ENDIF
      SET dm2_process_event_rs->status = dpl_success
      CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpru_log_id,1)
     ...
     IF ((srvreg->srv[dpru_cnt].managed_ind=1)
      AND (srvreg->srv[dpru_cnt].prop_op="ADD"))
      SET dm_err->eproc = concat('Set "Rdbms Connect Service" property for server ',trim(cnvtstring(
         srvreg->srv[dpru_cnt].server_number)),' to "',srvreg->srv[dpru_cnt].new_connect_svc,'"')
      CALL disp_msg("",dm_err->logfile,0)
      IF ((dm2_sys_misc->cur_os="AXP"))
       SET dpru_str = concat(dpru_set_key,"\",trim(cnvtstring(srvreg->srv[dpru_cnt].server_number)),
        '\prop\ "rdbms connect service" "',srvreg->srv[dpru_cnt].new_connect_svc,
        '"')
      ELSE
       SET dpru_str = concat(dpru_set_key,"\\",trim(cnvtstring(srvreg->srv[dpru_cnt].server_number)),
        '\\prop\\ "rdbms connect service" "',srvreg->srv[dpru_cnt].new_connect_svc,
      ...
      CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpl_no_prev_id,1)
      SET dpru_log_id = dm2_process_event_rs->dm_process_event_id
      IF (ddr_lreg_oper("SET",dpru_str,dpru_ret)=0)
       SET dm2_process_event_rs->status = dpl_failure
       SET dm2_process_event_rs->message = concat(dm_err->emsg,". retval:",dpru_ret)
       CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpru_log_id,1)
       RETURN(0)
      ...
      CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpl_no_prev_id,1)
      SET dpru_log_id = dm2_process_event_rs->dm_process_event_id
      IF (ddr_lreg_oper("GET",dpru_str,dpru_ret)=0)
       SET dm2_process_event_rs->status = dpl_failure
       SET dm2_process_event_rs->message = concat(dm_err->emsg,". retval:",dpru_ret)
       CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpru_log_id,1)
       RETURN(0)
      ENDIF
      IF (cnvtlower(trim(dpru_ret,3)) != cnvtlower(trim(srvreg->srv[dpru_cnt].new_connect_svc,3))
       AND (srvreg->srv[dpru_cnt].new_connect_svc != ""))
       SET message = nowindow
       SET dm2_process_event_rs->status = dpl_failure
       SET dm2_process_event_rs->message = concat(dm_err->emsg,". retval:",dpru_ret)
       CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpru_log_id,1)
       SET dm_err->emsg = concat('Unable to set "rdbms connect service" property for server:',trim(
         cnvtstring(srvreg->srv[dpru_cnt].server_number)),' to "',srvreg->srv[dpru_cnt].
        new_connect_svc,'"')
       SET dm_err->err_ind = 1
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
     ...
     ENDIF
     IF ((srvreg->srv[dpru_cnt].managed_ind=1)
      AND (srvreg->srv[dpru_cnt].prop_op="REMOVE"))
      SET dm_err->eproc = concat('Remove "rdbms connect service" property from server ',trim(
        cnvtstring(srvreg->srv[dpru_cnt].server_number)))
      CALL disp_msg("",dm_err->logfile,0)
      IF ((dm2_sys_misc->cur_os="AXP"))
       SET dpru_str = concat(dpru_set_key,"\",trim(cnvtstring(srvreg->srv[dpru_cnt].server_number)),
        '\prop\ "rdbms connect service"')
      ...
      CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpl_no_prev_id,1)
      SET dpru_log_id = dm2_process_event_rs->dm_process_event_id
      IF (ddr_lreg_oper("DEL",dpru_str,dpru_ret)=0)
       SET dm2_process_event_rs->status = dpl_failure
       SET dm2_process_event_rs->message = concat(dm_err->emsg,". retval:",dpru_ret)
       CALL dm2_process_log_row(dpl_db_services,dpl_svc_reg_upd,dpru_log_id,1)
       RETURN(0)
      ...
      IF (ddr_lreg_oper("GET",dpru_str,dpru_ret)=0)
       ...
       RETURN(0)
      ENDIF
      IF (dpru_ret != "NOPARMRETURNED")
       SET message = nowindow
       SET dm_err->emsg = concat('Unable to remove "rdbms connect service" property for server:',trim
        (cnvtstring(srvreg->srv[dpru_cnt].server_number)))
       SET dm_err->err_ind = 1
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dm2_process_event_rs->status = dpl_failure
   ...
   ENDFOR
 END ;Subroutine
 SUBROUTINE ddrm_get_services(null)
   DECLARE dgs_ndx = i4 WITH protect, noconstant(0)
   DECLARE dgs_search = vc WITH protect, noconstant("")
   DECLARE dgs_prop_ndx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Obtain Server to Services for Cerner Default Template"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_ds_template tmp,
     dm_ds_server srv,
     dm_ds_service svc,
     dm_ds_server_reltn rel
    PLAN (tmp
     WHERE tmp.template_name="DEFAULT")
     JOIN (rel
     WHERE tmp.dm_ds_template_id=rel.dm_ds_template_id)
     JOIN (svc
     WHERE rel.dm_ds_service_id=svc.dm_ds_service_id)
     JOIN (srv
     WHERE rel.dm_ds_server_id=srv.dm_ds_server_id)
    HEAD REPORT
     srvreg->cnt = 0
    DETAIL
     dgs_search = concat("\",trim(cnvtstring(srv.server_nbr))), dgs_ndx = locateval(dgs_ndx,1,ddr_reg
      ->skcnt,dgs_search,ddr_reg->skey[dgs_ndx].skeyname)
     IF (dgs_ndx > 0)
      srvreg->cnt = (srvreg->cnt+ 1), stat = alterlist(srvreg->srv,srvreg->cnt), srvreg->srv[srvreg->
      cnt].server_number = srv.server_nbr
      IF ((srvreg->mode="UNASSIGN ALL"))
       srvreg->srv[srvreg->cnt].new_connect_svc = ""
      ELSE
       srvreg->srv[srvreg->cnt].new_connect_svc = svc.service_name
      ENDIF
      dgs_prop_ndx = 0, dgs_prop_ndx = locateval(dgs_prop_ndx,1,ddr_reg->skey[dgs_ndx].spcnt,
       "servername",ddr_reg->skey[dgs_ndx].sprop[dgs_prop_ndx].spropname), srvreg->srv[srvreg->cnt].
      server_name = ddr_reg->skey[dgs_ndx].sprop[dgs_prop_ndx].spropval
      IF ((srvreg->max_servername_size < size(srvreg->srv[srvreg->cnt].server_name)))
       srvreg->max_servername_size = size(srvreg->srv[srvreg->cnt].server_name)
      ENDIF
      srvreg->srv[srvreg->cnt].managed_ind = 1, dgs_search = concat("\",trim(cnvtstring(srv
         .server_nbr)),"\prop"), dgs_ndx = locateval(dgs_ndx,1,ddr_reg->skcnt,dgs_search,ddr_reg->
       skey[dgs_ndx].skeyname)
      IF (dgs_ndx > 0)
       dgs_prop_ndx = 0, dgs_prop_ndx = locateval(dgs_prop_ndx,1,ddr_reg->skey[dgs_ndx].spcnt,
        "rdbms connect service",ddr_reg->skey[dgs_ndx].sprop[dgs_prop_ndx].spropname)
       IF (dgs_prop_ndx > 0)
        srvreg->srv[srvreg->cnt].curr_connect_svc = ddr_reg->skey[dgs_ndx].sprop[dgs_prop_ndx].
        spropval
       ELSE
        IF ((dm_err->debug_flag > 0))
         CALL echo(concat("Server ",build(srvreg->srv[srvreg->cnt].server_number),
          ' did not have "rdbms connect service" , initialize curr_connect_svc'))
        ENDIF
        srvreg->srv[srvreg->cnt].curr_connect_svc = ""
       ENDIF
      ELSE
       srvreg->srv[srvreg->cnt].curr_connect_svc = "", srvreg->srv[srvreg->cnt].create_prop_key = 1
       IF ((dm_err->debug_flag > 0))
        CALL echo(concat("Server ",build(srvreg->srv[srvreg->cnt].server_number),
         "did not have any properties , initialize curr_connect_svc"))
       ENDIF
      ENDIF
      IF ((srvreg->srv[srvreg->cnt].curr_connect_svc != srvreg->srv[srvreg->cnt].new_connect_svc)
       AND (srvreg->mode="ASSIGN ALL"))
       srvreg->srv[srvreg->cnt].prop_op = "ADD", srvreg->oper_cnt = (srvreg->oper_cnt+ 1)
      ELSEIF ((srvreg->srv[srvreg->cnt].curr_connect_svc != srvreg->srv[srvreg->cnt].new_connect_svc)
       AND (srvreg->srv[srvreg->cnt].new_connect_svc="")
       AND (srvreg->mode="UNASSIGN ALL"))
       srvreg->srv[srvreg->cnt].prop_op = "REMOVE", srvreg->oper_cnt = (srvreg->oper_cnt+ 1)
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF ((srvreg->cnt=0))
    SET dm_err->emsg = "No services available for servers in default services template."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((srvreg->max_servername_size < 20))
    SET srvreg->max_servername_size = 20
   ENDIF
   IF ((dm_err->debug_flag > 1))
    CALL echorecord(srvreg)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddrm_get_srv_reg(dgsr_domain_name,dgsr_node_name)
   DECLARE dgsr_key = vc WITH protect, noconstant("")
   DECLARE dgsr_file_name = vc WITH protect, noconstant("srvregexp")
   DECLARE dgsr_file_name_full = vc WITH protect, noconstant(concat(srvreg->cer_reg_formatted,
     dgsr_file_name,".reg"))
   SET dgsr_key = srvreg->root_key
   IF (ddrm_get_reg_exp(dgsr_file_name,dgsr_key)=0)
    RETURN(0)
   ENDIF
   IF (ddr_pop_reg_struct(1,dgsr_file_name_full,1)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddrm_get_reg_exp(dgre_file_name,dgre_exp_key)
   DECLARE dgre_cmd_file = vc WITH protect, noconstant("")
   DECLARE dgre_file_date = f8 WITH protect, noconstant(0.0)
   DECLARE dgre_cmd = vc WITH protect, noconstant("")
   DECLARE dgre_suffix = vc WITH protect, noconstant(";*")
   DECLARE dgre_file_full = vc WITH protect, noconstant("")
   DECLARE dgre_file_trim = vc WITH protect, noconstant("")
   DECLARE dgre_reg_file = vc WITH protect, noconstant("")
   SET dgre_reg_file = concat(srvreg->cer_reg_formatted,dgre_file_name,".reg ")
   SET dm_err->eproc = concat("Remove:",dgre_reg_file)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   IF (dm2_findfile(dgre_reg_file) > 0)
    IF (dm2_push_dcl(concat(evaluate(dm2_sys_misc->cur_os,"AXP","del","rm")," ",evaluate(dm2_sys_misc
       ->cur_os,"AXP",concat(dgre_reg_file,dgre_suffix),dgre_reg_file)))=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (get_unique_file("get_reg",evaluate(dm2_sys_misc->cur_os,"AXP",".com",".ksh"))=0)
    RETURN(0)
   ELSE
    SET dgre_cmd_file = dm_err->unique_fname
   ENDIF
   SET dm_err->eproc = concat("Create file to export REGISTRY info:",dgre_cmd_file)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(dgre_cmd_file)
    DETAIL
     IF ((dm2_sys_misc->cur_os="AXP"))
      CALL print("$mcr cer_exe:lregview"), row + 1,
      CALL print(concat("copy ",dgre_exp_key," \\",dgre_file_name," -sub")),
      row + 1
     ELSE
      CALL print("$cer_exe/lregview <<!"), row + 1,
      CALL print(concat("copy ",dgre_exp_key," \\\",dgre_file_name," -sub")),
   ...
   IF (check_error(dm_err->eproc) != 0)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = concat("Export REGISTRY info to ",dgre_reg_file)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET dgre_cmd = concat("@",dgre_cmd_file)
   ELSE
    SET dgre_cmd = concat(". $CCLUSERDIR/",dgre_cmd_file)
   ENDIF
   SET dm_err->disp_dcl_err_ind = 0
   IF (dm2_push_dcl(dgre_cmd)=0)
    IF ((dm_err->err_ind=1))
     RETURN(0)
    ENDIF
   ELSE
   ...
   IF (((findstring("bad syntax",dm_err->errtext,1,1) > 0) OR (findstring("key not found",dm_err->
    errtext,1,1) > 0)) )
    SET dm_err->emsg = concat("Error exporting REGISTRY:",dgre_cmd)
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "dm2_ds_reg_maint completed"
 CALL final_disp_msg("dm2_ds_reg_maint")
END GO
