CREATE PROGRAM acm_act_phone:dba
 ...
 FOR (index = 1 TO xref->act_cnt)
   SET reply->phone_qual[xref->act[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->act_cnt)),
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.phone_id=acm_request->phone_qual[xref->act[d.seq].idx].phone_id))
  WITH nocounter, status(reply->phone_qual[xref->act[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->act_cnt)
   IF ((reply->phone_qual[xref->act[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    FROM phone p
    WHERE expand(t1,f_val,t_val,p.phone_id,acm_request->phone_qual[xref->act[t1].idx].phone_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.phone_id,acm_request->phone_qual[xref->act[t1].idx].phone_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->phone_qual[xref->act[index].idx].status=0))
