CREATE PROGRAM djh_ma_phys_180_day_nologin
 PROMPT
 ...
 SET date_qual = cnvtlookbehind("180,D",cnvtdatetime(curdate,curtime3))
 ...
 SELECT DISTINCT INTO value(output_dest)
  ...
  PLAN (pr
   WHERE pr.active_ind=1
    ...
    AND pr.updt_dt_tm < cnvtdatetime((curdate - 180),0)
    ...
    AND pr.username != "SUSP*"
    AND pr.username != "SPND*"
    AND pr.username != "HIST"
    AND pr.username != "REHABMED"
    AND pr.username != "TRANSFUSE"
    AND pr.position_cd IN (925824, 925830, 925831, 925832, 925833,
   925834, 925835, 925836, 925837, 925841,
   925842, 925843, 925844, 925845, 925846,
   925847, 925848, 925851, 925852, 925824,
   925825, 925826, 925827, 925828, 96630,
   719476, 966300, 966301, 925850, 1646210)
    AND  NOT (pr.person_id IN (
   (SELECT
    oai.person_id
    FROM omf_app_ctx_day_st oai
    WHERE oai.person_id=pr.person_id
     AND oai.start_day > cnvtdatetime((curdate - 180),000)))))
   JOIN (oa
 ...
 IF (email_ind=1)
  ...
  SET subject_line = concat(curprog," V1 - Baystate Health CIS PHYS Acnts Inactive 180 days")
