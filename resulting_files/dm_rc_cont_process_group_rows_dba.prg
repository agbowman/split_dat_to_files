CREATE PROGRAM dm_rc_cont_process_group_rows:dba
 ...
 SET reply->table_name = "RC_CONT_PROCESS_GROUP"
 SET reply->rows_between_commit = 100
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 DECLARE min_days_to_keep_limit = i4 WITH protect, constant(180)
 ...
 FOR (v_token_idx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < min_days_to_keep_limit)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   ...
   "ii",min_days_to_keep_limit,
   v_days_to_keep)
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(pg.rc_cont_process_group_id)
   FROM rc_cont_process_group pg
   WHERE pg.rc_cont_process_group_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(pg.rc_cont_process_group_id)
  FROM rc_cont_process_group pg
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    pg.rowid
    FROM rc_cont_process_group pg
    WHERE pg.rc_cont_process_group_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(getpurgequalparserstring("pg"),"pg"))
    DETAIL
     v_rows += 1
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = pg.rowid
    WITH nocounter, maxqual(pg,value(v_rows_left))
   ;end select
 ...
 SUBROUTINE (getpurgequalparserstring(ptablealias=vc) =vc)
   RETURN(build2("(","(",ptablealias,".active_ind = 0)"," or (",
    ptablealias,".active_ind = 1"," and not exists (select null from rc_cont_process rcp",
    " where rcp.rc_cont_process_group_id = ",ptablealias,
    ".rc_cont_process_group_id)"," and not exists (select null from rc_cont_process_hist rcph",
    " where rcph.rc_cont_process_group_id = ",ptablealias,".rc_cont_process_group_id)",
    ")",")"," and ",ptablealias,".updt_dt_tm < cnvtdatetime(curdate - v_days_to_keep, 0)"))
 END ;Subroutine
#exit_script
END GO
