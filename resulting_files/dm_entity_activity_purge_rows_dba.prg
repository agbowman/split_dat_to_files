CREATE PROGRAM dm_entity_activity_purge_rows:dba
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(ea.entity_activity_id)
    FROM dm_entity_activity ea
    WHERE ea.entity_activity_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   seqval = max(ea.entity_activity_id)
   FROM dm_entity_activity ea
   DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     ea.rowid
     FROM dm_entity_activity ea
     WHERE parser(sbr_getrowidnotexists("ea.entity_activity_id between curMinID and curMaxID","ea"))
      AND ((ea.updt_dt_tm+ 0) < cnvtdatetime(cnvtlookbehind(lookbehindstring)))
      AND  NOT (trim(ea.parent_entity_name)="DM_ENTITY_ACTIVITY"
      AND ((ea.parent_entity_id+ 0)=0.0))
     DETAIL
      row_cnt = (row_cnt+ 1)
     ...
     WITH nocounter, maxqual(ea,value(rowsleft))
    ;end select
    ...
    IF (v_err_code2 != 0)
     ...
     SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERROR",
      "Failed in row collection: %1","s",v_errmsg2)
     GO TO exit_script
    ENDIF
  ...
  IF (v_err_code2 != 0)
   ...
   SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"UPDATEERROR",
    "Failed to update control record: %1","s",v_errmsg2)
   ROLLBACK
   GO TO exit_script
  ELSE
   COMMIT
  ...
  IF (curqual=0)
   INSERT  FROM dm_entity_activity ea
    SET ea.entity_activity_id = seq(dm_clinical_seq,nextval), ea.parent_entity_name =
     "DM_ENTITY_ACTIVITY", ea.parent_entity_id = 0.0,
   ...
   IF (v_err_code2 != 0)
    ...
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"INSERTERROR",
     "Failed to insert control record: %1","s",v_errmsg2)
    ROLLBACK
    GO TO exit_script
   ELSE
    COMMIT
