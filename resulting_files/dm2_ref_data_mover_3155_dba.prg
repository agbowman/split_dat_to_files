CREATE PROGRAM dm2_ref_data_mover_3155:dba
 ...
 END ;Subroutine
 DECLARE from_cd = vc
 DECLARE src_at_id = f8
 DECLARE src_be_id = f8
 DECLARE acct_type = f8
 DECLARE tgt_at_id = vc
 DECLARE tgt_be_id = vc
 DECLARE cust_acct_id = f8
 DECLARE ui_cnt = i4
 DECLARE val = f8
 DECLARE ins_upd = i2
 DECLARE idcd_check = i2
 DECLARE cust_tab_name = vc
 DECLARE at_tab_name = vc
 DECLARE be_tab_name = vc
 DECLARE translate_check = i2
 DECLARE type_trans_check = i2
 DECLARE at_tab_string = vc
 DECLARE be_tab_string = vc
 DECLARE acct_cnt = i4
 DECLARE acct_rows = i4
 DECLARE acct_insert_update(iur_temp_tbl_cnt=i4,iur_perm_col_cnt=i4) = i2
 FREE RECORD cust_acct
 RECORD cust_acct(
   1 qual[*]
     2 acct_id = f8
     2 acct_ind = i2
 ...
 SET at_tab_name = "AT_ACCT_RELTN"
 SET be_tab_name = "BE_AT_RELTN"
 SET cust_acct_id = cnvtreal(get_value(cust_tab_name,"ACCT_ID","FROM"))
 SET acct_type = cnvtreal(get_value(cust_tab_name,"ACCT_TYPE_CD","TO"))
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_3155
 ENDIF
 SET ui_cnt = 0
 SET type_trans_check = is_translated(cust_tab_name,"ACCT_TYPE_CD")
 SET translate_check = is_translated(cust_tab_name,"ACCT_ID")
 IF (translate_check=0)
  IF (type_trans_check=1)
   SET be_tab_string = dm2_get_rdds_tname(be_tab_name)
   SET at_tab_string = dm2_get_rdds_tname(at_tab_name)
   SELECT INTO "NL:"
    FROM (parser(be_tab_string) b),
     (parser(at_tab_string) a)
    PLAN (a
     WHERE a.acct_id=cust_acct_id)
     JOIN (b
     WHERE b.acct_templ_id=a.acct_templ_id)
    DETAIL
     ui_cnt = (ui_cnt+ 1), src_at_id = b.acct_templ_id, src_be_id = b.billing_entity_id
    WITH nocounter
   ;end select
   ...
   IF (ui_cnt > 1)
    SET dm2_ref_data_reply->error_ind = 1
    CALL merge_audit("FAILREASON","There were multiple relations in the AT_ACCT_RELTN table found.",1
     )
    IF (drdm_error_out_ind=1)
     ROLLBACK
     GO TO exit_3155
    ...
    GO TO exit_3155
   ENDIF
   SET tgt_at_id = select_merge_translate(cnvtstring(src_at_id),"ACCT_TEMPLATE")
   SET tgt_be_id = select_merge_translate(cnvtstring(src_be_id),"BILLING_ENTITY")
   IF (((tgt_be_id="No Trans") OR (tgt_at_id="No Trans")) )
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script,
     ": The UI columns have not been translated yet.")
    CALL merge_audit("FAILREASON","The UI columns have not been translated yet",2)
    IF (drdm_error_out_ind=1)
     ROLLBACK
     GO TO exit_3155
    ENDIF
   ...
   ENDIF
   SELECT INTO "NL:"
    FROM at_acct_reltn a
    WHERE a.acct_templ_id IN (
    (SELECT
     b.acct_templ_id
     FROM be_at_reltn b
     WHERE b.billing_entity_id=cnvtreal(tgt_be_id)
      AND b.acct_templ_id=cnvtreal(tgt_at_id)))
    HEAD REPORT
     ui_cnt = 0, stat = alterlist(cust_acct->qual,10)
    DETAIL
     ui_cnt = (ui_cnt+ 1)
     IF (mod(ui_cnt,10)=1
      AND ui_cnt != 1)
      stat = alterlist(cust_acct->qual,(ui_cnt+ 9))
     ENDIF
     cust_acct->qual[ui_cnt].acct_id = a.acct_id
    FOOT REPORT
     stat = alterlist(cust_acct->qual,ui_cnt)
    WITH nocounter
   ;end select
   IF (error(dm2_ref_data_reply->error_msg,0) > 0)
    SET dm2_ref_data_reply->error_ind = 1
   ...
   ENDIF
   IF (ui_cnt=0)
    SET val = get_seq(cust_tab_name,"ACCT_ID")
    IF (val > 0)
     CALL put_value(cust_tab_name,"ACCT_ID",cnvtstring(val))
     SET current_merges = (current_merges+ 1)
     SET child_merge_audit->num[current_merges].action = "NEWSEQ"
     SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  ACCT_ID")
    ELSE
     GO TO exit_3155
    ENDIF
   ELSE
    SET cust_acct_id = 0
    FOR (acct_cnt = 1 TO ui_cnt)
      SELECT INTO "nl:"
       FROM account a
       WHERE (a.acct_id=cust_acct->qual[acct_cnt].acct_id)
        AND a.acct_type_cd=acct_type
       DETAIL
        acct_rows = (acct_rows+ 1), cust_acct_id = a.acct_id
       WITH nocounter
      ;end select
    ENDFOR
    IF (acct_rows > 1)
     ...
     SET dm2_ref_data_reply->error_msg = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
      custom_script,": There where multiple rows returned from the UI")
     ...
     GO TO exit_3155
    ELSEIF (acct_rows=1)
     CALL put_value(cust_tab_name,"ACCT_ID",cnvtstring(cust_acct_id))
     SET current_merges = (current_merges+ 1)
   ...
   SET from_cd = get_value(cust_tab_name,"ACCT_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_3155
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(from_cd)
   ENDIF
   SET from_cd = get_value(cust_tab_name,"ACCT_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_3155
   ELSE
  ...
  ELSE
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script,
    ": The ACCT_TYPE_CD on the ACCOUNT table is not translated.")
   CALL merge_audit("FAILREASON","The ACCT_TYPE_CD on the ACCOUNT table is not translated.",2)
   IF (drdm_error_out_ind=1)
    ROLLBACK
    GO TO exit_3155
   ENDIF
 ...
 ENDIF
 IF (idcd_check=1)
  SET ins_upd = acct_insert_update(temp_tbl_cnt,perm_col_cnt)
 ...
 ELSE
  ...
  ENDIF
 ENDIF
 SUBROUTINE acct_insert_update(iur_temp_tbl_cnt,iur_perm_col_cnt)
   DECLARE first_where = i2
   DECLARE active_in = i2
   DECLARE drdm_col_name = vc
   DECLARE drdm_table_name = vc
   ...
   ELSE
    ...
    IF (curqual > 0)
     ...
     ELSE
      IF (non_key_ind=1)
       ...
       FOR (update_loop = 1 TO iur_perm_col_cnt)
         IF ((dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].col_qual[update_loop].pk_ind=0))
          IF ( NOT ((dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].col_qual[update_loop].column_name
           IN ("ACCT_BALANCE", "ADJUSTMENT_BALANCE", "APPLIED_PAYMENT_BALANCE", "BAD_DEBT_BALANCE",
          "CHARGE_BALANCE",
          "HI_ACCT_BALANCE", "PENDING_ACCT_BAL", "UNAPPLIED_PAYMENT_BALANCE"))))
           IF (drdm_parser_cnt > 2)
            SET drdm_parser->statement[drdm_parser_cnt].frag = ", "
            SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
           ENDIF
    ...
    ELSE
     ...
     FOR (insert_loop = 1 TO iur_perm_col_cnt)
       ...
       ELSEIF (drdm_col_name="UPDT_CNT")
        SET drdm_parser->statement[drdm_parser_cnt].frag = "dc.UPDT_CNT = 0"
       ELSEIF (drdm_col_name IN ("ACCT_BALANCE", "ADJUSTMENT_BALANCE", "APPLIED_PAYMENT_BALANCE",
       "BAD_DEBT_BALANCE", "CHARGE_BALANCE",
       "HI_ACCT_BALANCE", "PENDING_ACCT_BAL", "UNAPPLIED_PAYMENT_BALANCE"))
        SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" dc.",drdm_col_name," = 0")
       ELSE
        IF ((dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].col_qual[insert_loop].check_null=1)
         AND (dm2_ref_data_doc->tbl_qual[iur_temp_tbl_cnt].col_qual[insert_loop].nullable="Y"))
         SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" dc.",drdm_col_name," = null ")
...
#exit_3155
 ...
 FREE SET tgt_at_id
 FREE SET tgt_be_id
 FREE SET cust_acct_id
 FREE SET ui_cnt
 FREE SET val
 FREE SET ins_upd
 FREE SET idcd_check
 FREE SET cust_tab_name
 FREE SET at_tab_name
 FREE SET be_tab_name
 FREE SET translate_check
 FREE SET type_trans_check
 FREE SET at_tab_string
 FREE SET be_tab_string
 FREE SET acct_cnt
 FREE SET acct_rows
END GO
