CREATE PROGRAM dm2_mig_setup_source:dba
 ...
 SUBROUTINE dctx_restore_prev_contexts(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(dtde_tables_doc_list->list_filled,- (1))=- (1))
  AND (validate(dtde_tables_doc_list->list_filled,- (2))=- (2)))
  FREE RECORD dtde_tables_doc_list
  RECORD dtde_tables_doc_list(
    1 list_filled = i2
    1 cnt = i4
    1 qual[*]
      2 table_name = vc
 ...
 ENDIF
 DECLARE dm2_tables_doc_exists(dtde_table_name=vc,dtde_tables_doc_location=i4(ref),dtde_force_refresh
  =i2) = i2
 SUBROUTINE dm2_tables_doc_exists(dtde_table_name,dtde_tables_doc_location,dtde_force_refresh)
   IF (dtde_force_refresh=2
    AND (dtde_tables_doc_list->list_filled=1))
    RETURN(1)
   ENDIF
   IF (dtde_force_refresh=1)
    SET dtde_tables_doc_list->list_filled = 0
   ENDIF
   IF ((dtde_tables_doc_list->list_filled=0))
    ...
    SELECT DISTINCT INTO "nl:"
     ...
     HEAD REPORT
      dtde_tables_doc_list->cnt = 0, stat = alterlist(dtde_tables_doc_list->qual,0)
     DETAIL
      dtde_tables_doc_list->cnt = (dtde_tables_doc_list->cnt+ 1)
      IF (mod(dtde_tables_doc_list->cnt,250)=1)
       stat = alterlist(dtde_tables_doc_list->qual,(dtde_tables_doc_list->cnt+ 249))
      ENDIF
      dtde_tables_doc_list->qual[dtde_tables_doc_list->cnt].table_name = dtd.table_name,
      dtde_tables_doc_list->qual[dtde_tables_doc_list->cnt].table_suffix = dtd.table_suffix
     FOOT REPORT
      stat = alterlist(dtde_tables_doc_list->qual,dtde_tables_doc_list->cnt)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    SET dtde_tables_doc_list->list_filled = 1
   ENDIF
   IF (dtde_force_refresh=2)
    SET dtde_tables_doc_location = 0
   ELSE
    SET dtde_tables_doc_location = dm2_binary_search(dtde_table_name,"dtde_tables_doc_list->qual",
     "table_name")
   ENDIF
   RETURN(1)
 END ;Subroutine
 ...
 RECORD mig_data(
   ...
   1 qual[*]
     ...
     2 trig_new_ind = i2
     2 table_size = f8
     2 updt_task_exists_ind = i2
     2 col_bf_init_ind = i2
     2 num_rows_prior = f8
     2 incremental_space_ind = i2
 )
 FREE RECORD mig_user
 RECORD mig_user(
   1 user_cnt = i4
   1 qual[*]
     2 migusername = vc
 )
 FREE RECORD all_unique_index_list
 RECORD all_unique_index_list(
   1 tab_cnt = i4
   1 tabs[*]
     2 table_name = vc
     2 owner = vc
 ...
 RECORD tspace_rec(
   1 space_needed_ind = i2
   1 asm_space_needed_ind = i2
   1 cnt = i4
   1 tot_dg_cnt = i4
   1 tot_dg[*]
     2 dg_name = vc
   ...
   1 qual[*]
     2 tspace_name = vc
     2 tspace_exists_ind = i2
     2 initial_free_space = f8
     2 dm_info_row_exists = i2
     2 ext_mgmt = vc
     2 space_needed = f8
     2 max_init_ext = f8
     2 max_next_ext = f8
 ...
 FREE RECORD dmss_trig_needs
 RECORD dmss_trig_needs(
   1 cnt = i4
   1 qual[*]
     2 owner = vc
     2 table_name = vc
 )
 FREE RECORD dmss_pk_val
 RECORD dmss_pk_val(
   1 pk_cnt = i4
   1 pks[*]
     2 pk_name = vc
 ...
 DECLARE dmss_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmss_env_name = vc WITH protect, noconstant("")
 DECLARE dmss_handle = vc WITH protect, noconstant("")
 DECLARE dmss_run_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmss_schema_date = dq8 WITH protect, noconstant(0.0)
 DECLARE dmss_schema_date_offset = i2 WITH protect, noconstant(0)
 DECLARE dmss_schema_date_search_continue = i2 WITH protect, noconstant(0)
 DECLARE dmss_search = i4 WITH protect, noconstant(0)
 DECLARE dmss_search2 = i4 WITH protect, noconstant(0)
 DECLARE dmss_iter = i4 WITH protect, noconstant(0)
 DECLARE dmss_iter2 = i4 WITH protect, noconstant(0)
 DECLARE dmss_appl_status = vc WITH protect, noconstant("")
 DECLARE dmss_rpt_filename = vc WITH protect, noconstant("")
 DECLARE dmss_disp_rpt_ind = i2 WITH protect, noconstant(1)
 DECLARE dmss_tspace_maint_choice = vc WITH protect, noconstant("")
 DECLARE dmss_priv_give = i2 WITH protect, noconstant(0)
 DECLARE cons_64k = f8 WITH protect, constant(65536.0)
 DECLARE cons_1m = f8 WITH protect, constant(1048576.0)
 DECLARE cons_8m = f8 WITH protect, constant(8388608.0)
 DECLARE cons_64m = f8 WITH protect, constant(67108864.0)
 DECLARE cons_1g = f8 WITH protect, constant(1073741824.0)
 DECLARE cons_160k = f8 WITH protect, constant(163840.0)
 DECLARE dmss_op_type = vc WITH protect, noconstant("")
 DECLARE dmss_table_name = vc WITH protect, noconstant("")
 DECLARE dmss_tablespace_name = vc WITH protect, noconstant("")
 DECLARE dmss_row_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmss_obj_name = vc WITH protect, noconstant("")
 DECLARE dmss_priority = i4 WITH protect, noconstant(0)
 DECLARE dmss_operation = vc WITH protect, noconstant("")
 DECLARE dmss_v500_mig_user_found = i2 WITH protect, noconstant(0)
 DECLARE dmss_tspace_info_char = vc WITH protect, noconstant("")
 DECLARE dmss_use_alloc_free_space = i2 WITH protect, noconstant(0)
 DECLARE dmss_ndx = i4 WITH protect, noconstant(0)
 DECLARE dmss_owner = vc WITH protect, noconstant("")
 DECLARE dmss_managed_tables = vc WITH protect, noconstant("")
 DECLARE dmss_str = vc WITH protect, noconstant("")
 DECLARE dmss_tspace_needs_viewed = i2 WITH protect, noconstant(0)
 DECLARE dmss_dvm_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmss_bucket_limit = i4 WITH protect, noconstant(0)
 DECLARE dmss_parallel_thds = i4 WITH protect, noconstant(0)
 DECLARE dmss_limit_override = i4 WITH protect, noconstant(0)
 DECLARE dmss_parallel_override = i4 WITH protect, noconstant(0)
 DECLARE dmss_insert_ddl_op_row(null) = i2
 DECLARE dmss_calc_lmt_ext_size(total_size=f8) = f8
 DECLARE dmss_pop_current_size(null) = i2
 DECLARE dmss_manual_process_tspace(null) = i2
 DECLARE dmss_process_asm_tspace(null) = i2
 DECLARE dmss_calc_tspace_needs(null) = i2
 DECLARE dmss_disp_tspace_rpt(null) = i2
 DECLARE dmss_alloc_free_space_usage(dafsu_reserve_free_space=vc) = i2
 SET dm2_install_schema->process_option = "MIGRATION SETUP"
 IF (check_logfile("dm2_mig_setup_source",".log","dm2_mig_setup_source LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_setup_source"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 IF (validate(dm2_mig_reserve_free_space,"X") != "X")
  IF (dm2_mig_reserve_free_space="N")
   SET dmss_use_alloc_free_space = 1
  ENDIF
 ENDIF
 IF (dmss_use_alloc_free_space=1)
  IF ((dm_err->debug_flag > 1))
   CALL echo("dm2_mig_reserve_free_space set to N. Inserting dm_info rows and warning users")
  ENDIF
  SET dmss_tspace_info_char = "0"
  IF (dmss_alloc_free_space_usage(dm2_mig_reserve_free_space)=0)
   GO TO exit_script
  ENDIF
 ELSE
  IF ((dm_err->debug_flag > 1))
   CALL echo("dm2_mig_reserve_free_space not set. Updating DM2_MIG_TSPACE_SIZE dm_info rows if exist"
    )
  ENDIF
  SET dmss_tspace_info_char = "1"
  SET dm_err->eproc = "Updating DM_INFO row for Tablespace size"
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE "DM2_MIG_TSPACE_SIZE"=di.info_domain
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (curqual)
   UPDATE  FROM dm_info di
    SET di.info_char = dmss_tspace_info_char
    WHERE di.info_domain="DM2_MIG_TSPACE_SIZE"
    WITH nocounter
   ;end update
  ENDIF
  IF (check_error(dm_err->eproc)=1)
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Retrieving existing migration users from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_USER"
  HEAD REPORT
   mig_user->user_cnt = 0
  DETAIL
   IF (di.info_name="V500")
    dmss_v500_mig_user_found = 1
   ENDIF
   mig_user->user_cnt = (mig_user->user_cnt+ 1), stat = alterlist(mig_user->qual,mig_user->user_cnt),
   mig_user->qual[mig_user->user_cnt].migusername = di.info_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF ( NOT (curqual))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No migration users present."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSEIF ( NOT (dmss_v500_mig_user_found))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "V500 migration user not present."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining environment name from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE "DATA MANAGEMENT"=di.info_domain
   AND (("DM_ENV_NAME"=di.info_name) OR ("DM_ENV_ID"=di.info_name))
  DETAIL
   CASE (di.info_name)
    OF "DM_ENV_NAME":
     dmss_env_name = di.info_char
    OF "DM_ENV_ID":
     dmss_env_id = di.info_number
   ENDCASE
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking for DM2_LIMIT_OVERRIDE in dm_info"
 SET dmss_limit_override = 0
 SELECT INTO "nl:"
  d.info_number
  FROM dm_info d
  WHERE d.info_domain="DM2_MIG_SS"
   AND d.info_name="LIMIT_OVERRIDE"
  DETAIL
   dmss_limit_override = d.info_number
  WITH nocounter
 ;end select
 SET dm_err->eproc = "Checking for DM2_PARALLEL_OVERRIDE in dm_info"
 SET dmss_parallel_override = 0
 SELECT INTO "nl:"
  d.info_number
  FROM dm_info d
  WHERE d.info_domain="DM2_MIG_SS"
   AND d.info_name="PARALLEL_OVERRIDE"
  DETAIL
   dmss_parallel_override = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (0 < dmss_limit_override)
  SET dmss_bucket_limit = dmss_limit_override
 ELSE
  SET dmss_bucket_limit = 10000
 ENDIF
 SET message = window
 CALL clear(1,1)
 CALL box(1,1,24,131)
 CALL video(b)
 CALL text(2,2,"Warning!!")
 CALL video(n)
 IF ((dm2_sys_misc->cur_db_os != dm2_sys_misc->cur_os))
  CALL text(6,2,"Warning: You are not on the database node.")
 ENDIF
 CALL text(7,2,"No other Installation Tools activities should be performed during this process.")
 CALL text(8,2,"Please confirm that the following database represents the correct domain.")
 CALL text(10,2,concat("Environment ID: ",build(dmss_env_id)))
 CALL text(11,2,concat("Environment Name: ",dmss_env_name))
 CALL text(12,2,concat("Database Name: ",dmr_mig_data->src_db_name))
 CALL text(13,2,concat("Database Create Date: ",format(dmr_mig_data->src_created_date,";;Q")))
 CALL text(15,2,"Is this the correct domain? (Y/N):")
 CALL accept(15,37,"A;CU","N"
  WHERE curaccept IN ("Y", "N"))
 IF (curaccept="N")
  CALL clear(1,1)
  SET message = nowindow
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "User chose not to continue."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 CALL text(17,2,"Degrees of parallelism (1-99): ")
 IF (0=dmss_parallel_override)
  CALL accept(17,33,"99;H",1
   WHERE curaccept BETWEEN 1 AND 99)
  SET dmss_parallel_thds = curaccept
 ELSE
  CALL text(18,2,"*Note: Changing parallel will remove parallel_overide*")
  CALL accept(17,33,"99;H",dmss_parallel_override
   WHERE curaccept BETWEEN 1 AND 99)
  SET dmss_parallel_thds = curaccept
  IF (dmss_parallel_thds != dmss_parallel_override)
   SET dm_err->eproc = "Deleting  from dm_info where info_name = PARALLEL_OVERRIDE"
 ...
 CALL clear(1,1)
 SET message = nowindow
 SET dm_err->eproc = "Updating info_number where info_name = PARALLEL in DM_INFO"
 MERGE INTO dm_info d
 USING DUAL ON (d.info_domain="DM2_MIG_SS"
  AND d.info_name="PARALLEL")
 WHEN MATCHED THEN
 (UPDATE
  SET d.info_number = dmss_parallel_thds
  WHERE 1=1
 ;end update
 )
 WHEN NOT MATCHED THEN
 (INSERT  FROM (d.dm_info)
  (info_domain, info_name, info_number)
  VALUES("DM2_MIG_SS", "PARALLEL", dmss_parallel_thds)
  WITH nocounter
 ;end insert
 )
 IF (check_error(dm_err->eproc)=1)
 ...
 ELSE
  COMMIT
 ENDIF
 SET dm_err->eproc = "Updating info_number where info_name = LIMIT in DM_INFO"
 MERGE INTO dm_info d
 USING DUAL ON (d.info_domain="DM2_MIG_SS"
  AND d.info_name="LIMIT")
 WHEN MATCHED THEN
 (UPDATE
  SET d.info_number = dmss_bucket_limit
  WHERE 1=1
 ;end update
 )
 WHEN NOT MATCHED THEN
 (INSERT  FROM (d.dm_info)
  (info_domain, info_name, info_number)
  VALUES("DM2_MIG_SS", "LIMIT", dmss_bucket_limit)
  WITH nocounter
 ;end insert
 )
 IF (check_error(dm_err->eproc)=1)
 ...
 ELSE
  COMMIT
 ENDIF
 SET dm_err->eproc = "Exclude V500.DM_TXN_TRACKING table for adding a PK Column"
 MERGE INTO dm_info d
 USING DUAL ON (d.info_domain="DM2_MIG_NONGG_REP"
  AND d.info_name="V500.DM_TXN_TRACKING")
 WHEN MATCHED THEN
 (UPDATE
  SET d.updt_cnt = (d.updt_cnt+ 1), d.info_date = cnvtdatetime(curdate,curtime3)
  WHERE 1=1
 ;end update
 )
 WHEN NOT MATCHED THEN
 (INSERT  FROM (d.dm_info)
  ...
  VALUES("DM2_MIG_NONGG_REP", "V500.DM_TXN_TRACKING", "OWNER_NAME,TABLE_NAME,TXN_ID_TEXT,DEL_IND")
 ;end insert
 )
 SET dm_err->eproc = "Exclude V500.GG_HEARTBEAT table for adding a PK Colum"
 MERGE INTO dm_info d
 USING DUAL ON (d.info_domain="DM2_MIG_ALLOW_NOPK"
  AND d.info_name="V500.GG_HEARTBEAT")
 WHEN MATCHED THEN
 (UPDATE
  SET d.updt_cnt = (d.updt_cnt+ 1), d.info_date = cnvtdatetime(curdate,curtime3)
  WHERE 1=1
 ...
 (INSERT  FROM (d.dm_info)
  (info_domain, info_name, info_number)
  VALUES("DM2_MIG_ALLOW_NOPK", "V500.GG_HEARTBEAT", 0)
  WITH nocounter
 ;end insert
 )
 SET dm_err->eproc = "Exclude V500.GG_HEARTBEAT_HISTORY table for adding a PK Colum"
 MERGE INTO dm_info d
 USING DUAL ON (d.info_domain="DM2_MIG_ALLOW_NOPK"
  AND d.info_name="V500.GG_HEARTBEAT_HISTORY")
 WHEN MATCHED THEN
 (UPDATE
  SET d.updt_cnt = (d.updt_cnt+ 1), d.info_date = cnvtdatetime(curdate,curtime3)
  WHERE 1=1
 ...
 (INSERT  FROM (d.dm_info)
  (info_domain, info_name, info_number)
  VALUES("DM2_MIG_ALLOW_NOPK", "V500.GG_HEARTBEAT_HISTORY", 0)
  WITH nocounter
 ;end insert
 )
 SET dm_err->eproc = "Exclude V500.GG_HEARTBEAT_SEED table for adding a PK Colum"
 MERGE INTO dm_info d
 USING DUAL ON (d.info_domain="DM2_MIG_ALLOW_NOPK"
  AND d.info_name="V500.GG_HEARTBEAT_SEED ")
 WHEN MATCHED THEN
 (UPDATE
  SET d.updt_cnt = (d.updt_cnt+ 1), d.info_date = cnvtdatetime(curdate,curtime3)
  WHERE 1=1
 ...
 (INSERT  FROM (d.dm_info)
  (info_domain, info_name, info_number)
  VALUES("DM2_MIG_ALLOW_NOPK", "V500.GG_HEARTBEAT_SEED", 0)
  WITH nocounter
 ;end insert
 )
 IF (check_error(dm_err->eproc)=1)
 ...
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if Source is in archive log mode."
 SELECT INTO "nl:"
  FROM v$database vd
  WHERE vd.log_mode="ARCHIVELOG"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Source is not in archive log mode."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dmr_mig_data->src_node_cnt > 1))
  FOR (dmss_iter = 1 TO dmr_mig_data->src_node_cnt)
    IF ((dmr_mig_data->src_nodes[dmss_iter].instance_number > 1))
     SET dm2_install_schema->dbase_name = dmr_mig_data->src_nodes[dmss_iter].instance_name
     SET dm2_install_schema->u_name = "V500"
     EXECUTE dm2_connect_to_dbase "PC"
     IF ((dm_err->err_ind=1))
      GO TO exit_script
     ENDIF
     SET dmss_cstr->cstr_cnt = (dmss_cstr->cstr_cnt+ 1)
     SET stat = alterlist(dmss_cstr->qual,dmss_cstr->cstr_cnt)
     SET dmss_cstr->qual[dmss_cstr->cstr_cnt].cstr = dm2_install_schema->connect_str
     SET dmss_cstr->qual[dmss_cstr->cstr_cnt].inst_nbr = dmr_mig_data->src_nodes[dmss_iter].
     instance_number
     SET dmss_cstr->qual[dmss_cstr->cstr_cnt].link_name = concat("DM2MIGINST",trim(cnvtstring(
        dmr_mig_data->src_nodes[dmss_iter].instance_number)))
    ENDIF
  ENDFOR
  IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Determining if source preparation is currently running, from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE "DATABASE MIGRATION"=di.info_domain
   AND "PREPARING SOURCE"=di.info_name
  DETAIL
   dmss_handle = di.info_char
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (dmss_handle > "")
  IF (assign(dmss_appl_status,dm2_get_appl_status(dmss_handle))="E")
   GO TO exit_script
  ELSEIF (dmss_appl_status="A")
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Process is currently running.  You cannot start it again."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ELSE
   SET dm_err->eproc =
   "Updating row in dm_info to show that source preparation is currently running."
   UPDATE  FROM dm_info di
    SET di.info_char = value(trim(currdbhandle,3))
    WHERE "DATABASE MIGRATION"=di.info_domain
     AND "PREPARING SOURCE"=di.info_name
    WITH nocounter
   ;end update
 ...
 ELSE
  SET dm_err->eproc =
  "Inserting row into dm_info to show that source preparation is currently running."
  INSERT  FROM dm_info di
   SET di.info_domain = "DATABASE MIGRATION", di.info_name = "PREPARING SOURCE", di.info_char = value
    (trim(currdbhandle,3))
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
 ...
 IF (dm2_cleanup_stranded_appl(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if there are running operations in dm2_ddl_ops_log."
 SELECT INTO "nl:"
  FROM dm2_ddl_ops_log ddol
  WHERE (("RUNNING"=ddol.status) OR (ddol.status = null))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual > 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "There are currently running operations."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if dm2_ddl_ops row exists for this process."
 SELECT INTO "nl:"
  FROM dm2_ddl_ops ddo
  WHERE "DATABASE MIGRATION"=ddo.process_option
  DETAIL
   dmss_run_id = ddo.run_id, dmss_schema_date = ddo.schema_date
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Determining new run_id from dm2_ddl_ops."
  SELECT INTO "nl:"
   max_run_id = max(ddo.run_id)
   FROM dm2_ddl_ops ddo
   HEAD REPORT
    dmss_run_id = 0
   DETAIL
    dmss_run_id = max_run_id
   FOOT REPORT
    dmss_run_id = (dmss_run_id+ 1)
   WITH nocounter, nullreport
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dmss_schema_date_search_continue = 1
  SET dmss_schema_date_offset = 365
  WHILE (dmss_schema_date_search_continue=1
   AND dmss_schema_date_offset >= 335)
    SET dmss_schema_date_offset = (dmss_schema_date_offset - 1)
    SET dmss_schema_date = cnvtdatetime((curdate - dmss_schema_date_offset),0)
    SET dm_err->eproc = "Determining usable schema date from dm2_ddl_ops."
    SELECT INTO "nl:"
     FROM dm2_ddl_ops ddo
     WHERE ddo.schema_date=cnvtdatetime(dmss_schema_date)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    IF (curqual=0)
     SET dmss_schema_date_search_continue = 0
    ENDIF
  ENDWHILE
  IF (dmss_schema_date_search_continue=1)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Unable to find a usable schema date."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Inserting new run_id into dm2_ddl_ops."
  INSERT  FROM dm2_ddl_ops ddo
   SET ddo.run_id = dmss_run_id, ddo.gen_dt_tm = cnvtdatetime(curdate,curtime3), ddo.ocd = 0.0,
    ddo.schema_date = cnvtdatetime(dmss_schema_date), ddo.process_option = "DATABASE MIGRATION", ddo
  ...
  COMMIT
 ELSE
  SET dm_err->eproc = "Removing old DDL rows found for run_id from dm2_ddl_ops_log."
  DELETE  FROM dm2_ddl_ops_log ddol
   WHERE ddol.run_id=dmss_run_id
    AND (("COMPLETE" != ddol.status) OR (ddol.status = null))
    AND ddol.op_type != "EXECUTE*"
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Updating old DDL rows found for run_id in dm2_ddl_ops_log."
  UPDATE  FROM dm2_ddl_ops_log ddol
   SET ddol.status = null, ddol.appl_id = currdbhandle, ddol.error_msg = ""
   WHERE ddol.run_id=dmss_run_id
    AND (("COMPLETE" != ddol.status) OR (ddol.status = null))
    AND ddol.op_type="EXECUTE*"
   WITH nocounter
  ...
  COMMIT
 ENDIF
 IF ((dmss_cstr->cstr_cnt > 0))
  FOR (dmss_iter = 1 TO dmss_cstr->cstr_cnt)
    SET dm_err->eproc = concat("Determining if ",dmss_cstr->qual[dmss_iter].link_name,
     " database link already exists.")
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dba_db_links d
     WHERE d.db_link=patstring(concat(dmss_cstr->qual[dmss_iter].link_name,".*"))
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    IF (curqual > 0)
     SET dm_err->eproc = concat(dmss_cstr->qual[dmss_iter].link_name," database link already exists."
      )
     CALL disp_msg("",dm_err->logfile,0)
    ELSE
     SET dm_err->eproc = concat("Creating ",dmss_cstr->qual[dmss_iter].link_name," database link.")
     CALL disp_msg("",dm_err->logfile,0)
     IF (dm2_push_cmd(concat("rdb asis (^create public database link ",dmss_cstr->qual[dmss_iter].
       link_name,' connect to V500 identified by "',dmr_mig_data->src_v500_pwd,^" using '^,
       dmss_cstr->qual[dmss_iter].cstr,"'^) go"),1)=0)
      GO TO exit_script
     ENDIF
    ENDIF
    SET dm_err->eproc = concat("Validating ",dmss_cstr->qual[dmss_iter].link_name," database link")
    SELECT INTO "nl:"
     FROM (value(concat("V$DATABASE@",dmss_cstr->qual[dmss_iter].link_name)) a)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ELSEIF (curqual=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Error validating ",dmss_cstr->qual[dmss_iter].link_name,
      " database link.")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     GO TO exit_script
    ENDIF
  ENDFOR
 ENDIF
 SET dmss_managed_tables = ""
 IF (dmr_load_managed_tables(dmss_managed_tables)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if dm2_mig_seq sequence already exists from user_sequences."
 SELECT INTO "nl:"
  FROM user_sequences us
  WHERE "DM2_MIG_SEQ"=us.sequence_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET mig_data->initsetup = 1
 ENDIF
 SET dm_err->eproc = "Getting list of all tables with unique indexes."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT DISTINCT INTO "nl:"
  ui.table_name
  FROM dm_dba_indexes_actual_stats ui
  WHERE "UNIQUE"=ui.uniqueness
   AND "NORMAL"=ui.index_type
   AND ui.table_owner IN (
  (SELECT
   info_name
   FROM dm_info
   WHERE info_domain="DM2_MIG_USER"))
  ORDER BY ui.owner, ui.table_name
  HEAD REPORT
   all_unique_index_list->tab_cnt = 0, stat = alterlist(all_unique_index_list->tabs,0)
  HEAD ui.owner
   row + 0
  HEAD ui.table_name
   all_unique_index_list->tab_cnt = (all_unique_index_list->tab_cnt+ 1)
   IF (mod(all_unique_index_list->tab_cnt,500)=1)
    stat = alterlist(all_unique_index_list->tabs,(all_unique_index_list->tab_cnt+ 499))
   ENDIF
   all_unique_index_list->tabs[all_unique_index_list->tab_cnt].table_name = ui.table_name,
   all_unique_index_list->tabs[all_unique_index_list->tab_cnt].owner = ui.table_owner
  FOOT REPORT
   stat = alterlist(all_unique_index_list->tabs,all_unique_index_list->tab_cnt)
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Getting list of all tables needing migration triggers."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  dtc.owner, dtc.table_name
  FROM dba_tab_columns dtc
  WHERE dtc.column_name="DM2_MIG_SEQ_ID"
   AND  NOT ( EXISTS (
  (SELECT
   "x"
   FROM dba_triggers dt
   WHERE dt.table_name=dtc.table_name
    AND dt.table_owner=dtc.owner
    AND dt.trigger_name="TRG_DM2_MIG*")))
  ...
  HEAD dtc.table_name
   row + 0
  DETAIL
   dmss_trig_needs->cnt = (dmss_trig_needs->cnt+ 1), stat = alterlist(dmss_trig_needs->qual,
    dmss_trig_needs->cnt), dmss_trig_needs->qual[dmss_trig_needs->cnt].owner = dtc.owner,
   dmss_trig_needs->qual[dmss_trig_needs->cnt].table_name = dtc.table_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 7))
  CALL echorecord(dmss_trig_needs)
 ENDIF
 SET dm_err->eproc =
 "Getting list of tables from dba_tables/dba_tablespaces to see what doesn't have a unique index."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_objects d,
   dm_dba_tables_actual_stats ut,
   dba_tablespaces uts
  WHERE d.object_type="TABLE"
   AND d.object_name=ut.table_name
   AND d.owner=ut.owner
   AND parser(concat(" ut.table_name not in (",dmss_managed_tables,")"))
   AND ut.tablespace_name=uts.tablespace_name
   AND ut.temporary="N"
   AND  NOT ( EXISTS (
  (SELECT
   "x"
   FROM dm_info di
   WHERE di.info_domain IN ("DM2_MIG_EXCL", "DM2_MIG_NONGG_REP", "DM2_MIG_ALLOW_NOPK")
    AND di.info_name=concat(ut.owner,".",ut.table_name))))
   AND ut.owner IN (
  (SELECT
   info_name
   FROM dm_info
  ...
  HEAD REPORT
   mig_data->cnt = 0, stat = alterlist(mig_data->qual,0)
  HEAD ut.owner
   row + 0
  DETAIL
   IF ((( NOT (locateval(dmss_search,1,all_unique_index_list->tab_cnt,ut.table_name,
    all_unique_index_list->tabs[dmss_search].table_name,
    ut.owner,all_unique_index_list->tabs[dmss_search].owner))) OR (locateval(dmss_search,1,
    dmss_trig_needs->cnt,ut.table_name,dmss_trig_needs->qual[dmss_search].table_name,
    ut.owner,dmss_trig_needs->qual[dmss_search].owner))) )
    mig_data->cnt = (mig_data->cnt+ 1)
    IF (mod(mig_data->cnt,25)=1)
     stat = alterlist(mig_data->qual,(mig_data->cnt+ 24))
    ENDIF
    mig_data->qual[mig_data->cnt].table_name = ut.table_name, mig_data->qual[mig_data->cnt].owner =
    ut.owner, mig_data->qual[mig_data->cnt].tspace_name = ut.tablespace_name,
    mig_data->qual[mig_data->cnt].num_rows = ut.num_rows, mig_data->qual[mig_data->cnt].
    num_rows_prior = mig_data->qual[mig_data->cnt].num_rows, mig_data->qual[mig_data->cnt].ext_mgmt
     = substring(1,1,uts.extent_management),
    mig_data->qual[mig_data->cnt].tbl_cur_bytes = ((ut.blocks+ ut.empty_blocks) * 8192.0)
    CASE (mig_data->qual[mig_data->cnt].ext_mgmt)
     OF "L":
      mig_data->qual[mig_data->cnt].tbl_init_ext = 0.0,mig_data->qual[mig_data->cnt].tbl_next_ext =
      dmss_calc_lmt_ext_size(mig_data->qual[mig_data->cnt].tbl_cur_bytes),mig_data->qual[mig_data->
      cnt].ind_init_ext = 0.0,
      mig_data->qual[mig_data->cnt].ind_next_ext = cons_64k
     OF "D":
      mig_data->qual[mig_data->cnt].tbl_init_ext = ut.initial_extent,mig_data->qual[mig_data->cnt].
      tbl_next_ext = ut.next_extent,mig_data->qual[mig_data->cnt].ind_init_ext = cons_160k,
      mig_data->qual[mig_data->cnt].ind_next_ext = cons_160k
    ENDCASE
    mig_data->qual[mig_data->cnt].col_name = "DM2_MIG_SEQ_ID", mig_data->qual[mig_data->cnt].
    col_new_ind = 1, mig_data->qual[mig_data->cnt].ind_new_ind = 1,
    mig_data->qual[mig_data->cnt].ind_tspace = "I_DM2MIG", mig_data->qual[mig_data->cnt].trig_new_ind
     = 1, mig_data->qual[mig_data->cnt].table_suffix = trim(cnvtstring(d.object_id)),
    mig_data->qual[mig_data->cnt].ind_name = concat("XIEDM2_MIG_",mig_data->qual[mig_data->cnt].
     table_suffix), mig_data->qual[mig_data->cnt].trig_name = concat("TRG_DM2_MIG_",mig_data->qual[
     mig_data->cnt].table_suffix)
   ENDIF
  FOOT REPORT
   stat = alterlist(mig_data->qual,mig_data->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 FOR (dmss_iter = 1 TO mig_data->cnt)
  SET dm_err->eproc = concat(mig_data->qual[dmss_iter].owner,".",mig_data->qual[dmss_iter].table_name,
   " has no unique index.")
  CALL disp_msg("",dm_err->logfile,0)
 ENDFOR
 SET dm_err->eproc = "Querying dba_tab_columns to see if column already exists."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_tab_columns utc
  WHERE utc.column_name IN ("DM2_MIG_SEQ_ID", "UPDT_TASK")
   AND utc.owner IN (
  (SELECT
   info_name
   FROM dm_info
  ...
  HEAD utc.owner
   row + 0
  HEAD utc.table_name
   dmss_search = locateval(dmss_search,1,mig_data->cnt,utc.table_name,mig_data->qual[dmss_search].
    table_name,
    utc.owner,mig_data->qual[dmss_search].owner)
  DETAIL
   IF (dmss_search)
    IF (utc.column_name="DM2_MIG_SEQ_ID")
     mig_data->qual[dmss_search].col_new_ind = 0
    ELSE
     mig_data->qual[dmss_search].updt_task_exists_ind = 1
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Querying to see if index already exists."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_indexes ui,
   dba_ind_columns ic
  WHERE ui.table_owner IN (
  (SELECT
   info_name
   FROM dm_info
   WHERE info_domain="DM2_MIG_USER"))
   AND ui.owner=ic.index_owner
   AND ui.index_name=ic.index_name
   AND ic.column_position=1
  ORDER BY ui.table_owner, ui.table_name
  HEAD ui.table_owner
   row + 0
  HEAD ui.table_name
   dmss_search = locateval(dmss_search,1,mig_data->cnt,ui.table_name,mig_data->qual[dmss_search].
    table_name,
    ui.table_owner,mig_data->qual[dmss_search].owner)
  DETAIL
   IF (dmss_search)
    IF ((ic.column_name=mig_data->qual[dmss_search].col_name))
     mig_data->qual[dmss_search].ind_new_ind = 0
    ENDIF
    IF ((mig_data->qual[dmss_search].ind_new_ind=1))
     IF ((mig_data->qual[dmss_search].ext_mgmt="D"))
      mig_data->qual[dmss_search].ind_init_ext = greatest(mig_data->qual[dmss_search].ind_init_ext,ui
       .initial_extent), mig_data->qual[dmss_search].ind_next_ext = greatest(mig_data->qual[
       dmss_search].ind_next_ext,ui.next_extent)
     ENDIF
    ENDIF
   ENDIF
  WITH nocounter
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Querying to see if trigger already exists."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_triggers ut
  WHERE ut.status="ENABLED"
   AND ut.trigger_name="TRG_DM2_MIG*"
   AND ut.owner IN (
  (SELECT
   info_name
   FROM dm_info
  ...
  HEAD ut.owner
   row + 0
  DETAIL
   IF (assign(dmss_search,locateval(dmss_search,1,mig_data->cnt,ut.table_name,mig_data->qual[
     dmss_search].table_name,
     ut.owner,mig_data->qual[dmss_search].owner))
    AND ut.trigger_body="*DM2_MIG_SEQ_ID*")
    mig_data->qual[dmss_search].trig_new_ind = 0
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 7))
  CALL echorecord(mig_data)
 ENDIF
 FOR (dmss_iter = 1 TO mig_data->cnt)
   IF ((mig_data->initsetup=1)
    AND (mig_data->qual[dmss_iter].col_new_ind=0))
    SET dm_err->eproc = concat("Set priority 55 indicator row in dm_info for table: ",mig_data->qual[
     dmss_iter].table_name)
    UPDATE  FROM dm_info di
     SET di.info_date = cnvtdatetime(curdate,curtime3)
     WHERE di.info_domain="DM2_COLINIT_DBMIG"
      AND di.info_name=concat(mig_data->qual[dmss_iter].owner,".",mig_data->qual[dmss_iter].
      table_name)
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
    ...
    IF (curqual=0)
     INSERT  FROM dm_info di
      SET di.info_domain = "DM2_COLINIT_DBMIG", di.info_name = concat(mig_data->qual[dmss_iter].owner,
        ".",mig_data->qual[dmss_iter].table_name), di.info_date = cnvtdatetime(curdate,curtime3)
      WITH nocounter
     ;end insert
     IF (check_error(dm_err->eproc)=1)
      ROLLBACK
   ...
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Setting backfill indicator"
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di,
   (dummyt d  WITH seq = value(mig_data->cnt))
  PLAN (d)
   JOIN (di
   WHERE di.info_domain="DM2_COLINIT_DBMIG"
    AND di.info_name=concat(mig_data->qual[d.seq].owner,".",mig_data->qual[d.seq].table_name))
  DETAIL
   mig_data->qual[d.seq].col_bf_init_ind = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 FOR (dmss_iter = 1 TO mig_data->cnt)
   IF ((mig_data->qual[dmss_iter].col_new_ind=1))
    SET dmss_op_type = "ADD COLUMN"
    IF ((mig_data->qual[dmss_iter].owner != "V500"))
     SET dmss_obj_name = concat(mig_data->qual[dmss_iter].owner,".",mig_data->qual[dmss_iter].
      col_name)
    ELSE
     SET dmss_obj_name = mig_data->qual[dmss_iter].col_name
    ENDIF
    SET dmss_table_name = mig_data->qual[dmss_iter].table_name
    SET dmss_priority = 20
    SET dmss_tablespace_name = mig_data->qual[dmss_iter].tspace_name
    SET dmss_row_cnt = mig_data->qual[dmss_iter].num_rows
    SET dmss_operation = concat("RDB ASIS(^ ALTER TABLE ",mig_data->qual[dmss_iter].owner,".",
     mig_data->qual[dmss_iter].table_name," ADD ",
     mig_data->qual[dmss_iter].col_name," NUMBER NULL ^) GO")
    SET dmss_owner = mig_data->qual[dmss_iter].owner
    IF (dmss_insert_ddl_op_row(null)=0)
     GO TO exit_script
    ENDIF
    IF ((mig_data->qual[dmss_iter].owner="V500"))
     SET dmss_op_type = "EXECUTE ORAGEN3 (TABLE)"
     SET dmss_obj_name = mig_data->qual[dmss_iter].table_name
     SET dmss_table_name = mig_data->qual[dmss_iter].table_name
     SET dmss_priority = 700
     SET dmss_tablespace_name = "-1"
     SET dmss_row_cnt = 0
     SET dmss_owner = mig_data->qual[dmss_iter].owner
     SET dmss_operation = concat("EXECUTE ORAGEN3 '",mig_data->qual[dmss_iter].table_name,"' GO")
     IF (dmss_insert_ddl_op_row(null)=0)
      GO TO exit_script
     ENDIF
    ENDIF
   ENDIF
   IF ((((mig_data->qual[dmss_iter].col_new_ind=1)) OR ((((mig_data->qual[dmss_iter].ind_new_ind=1))
    OR ((mig_data->qual[dmss_iter].trig_new_ind=1))) )) )
    IF ((mig_data->qual[dmss_iter].col_bf_init_ind=1))
     SET dmss_op_type = "COLUMN BACKFILL"
     IF ((mig_data->qual[dmss_iter].owner != "V500"))
      SET dmss_obj_name = concat(mig_data->qual[dmss_iter].owner,".",mig_data->qual[dmss_iter].
       col_name)
     ELSE
     ...
     ENDIF
     SET dmss_table_name = mig_data->qual[dmss_iter].table_name
     SET dmss_priority = 55
     SET dmss_tablespace_name = mig_data->qual[dmss_iter].tspace_name
     SET dmss_row_cnt = mig_data->qual[dmss_iter].num_rows
     SET dmss_owner = mig_data->qual[dmss_iter].owner
     SET dmss_operation = concat(" RDB ASIS(^"," DECLARE"," tot_row_cnt NUMBER:=0;",
      " err_msg  VARCHAR2(150);"," snapshot_too_old  EXCEPTION;",
      " dm_op_id NUMBER := !op_id; "," PRAGMA exception_init(snapshot_too_old, -1555);",
      " parallel_var NUMBER:= ",trim(cnvtstring(dmss_parallel_thds)),";",
      " BEGIN","  for xrec in (select info_name,info_number from dm_info where ",
      "    info_name in('PARALLEL_OVERRIDE') and ","    info_domain= 'DM2_MIG_SS') ","    loop ",
      "        parallel_var := xrec.info_number; ","    end loop; ",
      " execute immediate 'Select sum(COLUMN_VALUE) FROM TABLE(V500.dm2_mig_seq_id_bf(",
      "'|| parallel_var ||'",",''",
      dmss_owner,"'',''",dmss_table_name,"''",", CURSOR(SELECT /",
      char(42),"+ PARALLEL(d  ","'|| parallel_var ||'",") ",char(42),
      "/ rowid FROM ",char(34),dmss_owner,char(34),".",
      char(34),dmss_table_name,char(34)," d WHERE d.dm2_mig_seq_id IS NOT NULL)",",",
      trim(cnvtstring(dmss_bucket_limit)),",","''null''))' into tot_row_cnt;"," BEGIN",
      " update dm2_ddl_ops_log ddol ",
      " set ddol.row_cnt = tot_row_cnt where ddol.op_id = dm_op_id;"," EXCEPTION"," WHEN OTHERS THEN",
      " ROLLBACK;"," err_msg:=SUBSTR(sqlerrm,1,150);",
      " raise_application_error(-20555, err_msg);"," END;"," BEGIN"," DELETE"," FROM DM_INFO",
      " WHERE INFO_DOMAIN = 'DM2_COLINIT_DBMIG'"," AND INFO_NAME     = '",dmss_owner,".",
      dmss_table_name,
      "';"," EXCEPTION"," WHEN OTHERS THEN"," ROLLBACK;"," err_msg:=SUBSTR(sqlerrm,1,150);",
      " raise_application_error(-20555, err_msg);"," END;"," commit;"," EXCEPTION",
      " WHEN OTHERS THEN",
      " ROLLBACK;"," err_msg:=SUBSTR(sqlerrm,1,150);"," raise_application_error(-20555, err_msg);",
      " END;^) GO")
     IF (dmss_insert_ddl_op_row(null)=0)
      GO TO exit_script
     ENDIF
    ...
    ENDIF
    SET dmss_table_name = mig_data->qual[dmss_iter].table_name
    SET dmss_priority = 60
    SET dmss_tablespace_name = mig_data->qual[dmss_iter].tspace_name
    SET dmss_row_cnt = mig_data->qual[dmss_iter].num_rows
    SET dmss_owner = mig_data->qual[dmss_iter].owner
    SET dmss_operation = concat("RDB ASIS(^ "," DECLARE "," tot_row_cnt NUMBER:=0;",
     " finished NUMBER:=0;"," err_msg VARCHAR2(150);",
     " dm_op_id NUMBER := !op_id; "," snapshot_too_old EXCEPTION;",
     " PRAGMA exception_init(snapshot_too_old, -1555);"," old_size NUMBER:=0;",
     " current_size NUMBER:=0;",
     " parallel_var NUMBER:= ",trim(cnvtstring(dmss_parallel_thds)),";"," BEGIN ",
     "  for xrec in (select info_name,info_number from dm_info where ",
     "    info_name in('PARALLEL_OVERRIDE') and ","    info_domain= 'DM2_MIG_SS') ","    loop ",
     "        parallel_var := xrec.info_number; ","    end loop; ",
     " BEGIN "," BEGIN SELECT info_number INTO old_size FROM dm_info",
     " WHERE info_domain = 'DM2_MIG_TSPACE_SIZE' "," and info_name = '",mig_data->qual[dmss_iter].
     tspace_name,
     "'"," and info_char = '1'; "," EXCEPTION WHEN NO_DATA_FOUND THEN NULL; "," END;",
     " SELECT sum(bytes) INTO current_size FROM dba_free_space",
     " WHERE tablespace_name = '",mig_data->qual[dmss_iter].tspace_name,"' GROUP BY tablespace_name;",
     " IF current_size <= (0.90 * old_size) THEN",
     " err_msg:= 'Overused allocated space'; raise_application_error(-20556, err_msg); ",
     " END IF;"," EXCEPTION WHEN OTHERS THEN",
     " err_msg:=substr(sqlerrm,1,150); raise_application_error(-20555, err_msg); "," END;",
     "execute immediate 'Select sum(COLUMN_VALUE) FROM TABLE(V500.dm2_mig_seq_id_bf(",
     "'|| parallel_var ||'",",''",dmss_owner,"'',''",dmss_table_name,
     "''",", CURSOR(SELECT /",char(42),"+ PARALLEL(d  ","'|| parallel_var ||'",
     ") ",char(42),"/ rowid FROM ",char(34),dmss_owner,
     char(34),".",char(34),dmss_table_name,char(34),
     " d WHERE (d.dm2_mig_seq_id IS NULL or d.dm2_mig_seq_id = 0))",",",trim(cnvtstring(
       dmss_bucket_limit)),",","''DM2_MIG_SEQ.NEXTVAL''))'  into tot_row_cnt;",
     " BEGIN"," update dm2_ddl_ops_log ddol ",
     " set ddol.row_cnt = tot_row_cnt where ddol.op_id = dm_op_id;"," EXCEPTION"," WHEN OTHERS THEN",
     " ROLLBACK;"," err_msg:=SUBSTR(sqlerrm,1,150);"," raise_application_error(-20555, err_msg);",
     " END;"," commit; ",
     " EXCEPTION"," WHEN OTHERS THEN"," ROLLBACK;"," err_msg:=SUBSTR(sqlerrm,1,150);",
    ...
    ENDIF
   ENDIF
   IF ((mig_data->qual[dmss_iter].ind_new_ind=1))
    SET dmss_op_type = "CREATE INDEX"
    IF ((mig_data->qual[dmss_iter].owner != "V500"))
     SET dmss_obj_name = concat(mig_data->qual[dmss_iter].owner,".",mig_data->qual[dmss_iter].
      ind_name)
    ELSE
     SET dmss_obj_name = mig_data->qual[dmss_iter].ind_name
    ENDIF
    SET dmss_table_name = mig_data->qual[dmss_iter].table_name
    SET dmss_priority = 320
    SET dmss_tablespace_name = mig_data->qual[dmss_iter].ind_tspace
    SET dmss_row_cnt = mig_data->qual[dmss_iter].num_rows
    SET dmss_owner = mig_data->qual[dmss_iter].owner
    SET dmss_operation = concat("RDB ASIS(^ CREATE UNIQUE INDEX ",mig_data->qual[dmss_iter].owner,".",
     mig_data->qual[dmss_iter].ind_name," ON ",
     mig_data->qual[dmss_iter].owner,".",mig_data->qual[dmss_iter].table_name," ( ",mig_data->qual[
     dmss_iter].col_name,
     " )")
    IF ((dm2_db_options->new_tspace_type="D"))
     SET dmss_operation = concat(dmss_operation," STORAGE ( INITIAL ",trim(cnvtstring(dm2ceil((
         mig_data->qual[dmss_iter].ind_init_ext/ 1024)))),"K NEXT ",trim(cnvtstring(dm2ceil((mig_data
         ->qual[dmss_iter].ind_next_ext/ 1024)))),
      "K)")
    ENDIF
    SET dmss_operation = concat(dmss_operation," LOGGING ONLINE TABLESPACE ",mig_data->qual[dmss_iter
     ].ind_tspace,"^) GO")
    IF (dmss_insert_ddl_op_row(null)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   IF ((mig_data->qual[dmss_iter].trig_new_ind=1))
    SET dmss_op_type = "CREATE SQL-TRIGGER"
    IF ((mig_data->qual[dmss_iter].owner != "V500"))
     SET dmss_obj_name = concat(mig_data->qual[dmss_iter].owner,".",mig_data->qual[dmss_iter].
      trig_name)
    ELSE
     SET dmss_obj_name = mig_data->qual[dmss_iter].trig_name
    ENDIF
    SET dmss_table_name = mig_data->qual[dmss_iter].table_name
    SET dmss_priority = 30
    SET dmss_tablespace_name = "-1"
    SET dmss_row_cnt = 0
    SET dmss_owner = mig_data->qual[dmss_iter].owner
    SET dmss_operation = concat("RDB ASIS(^CREATE OR REPLACE TRIGGER ",mig_data->qual[dmss_iter].
     owner,".",mig_data->qual[dmss_iter].trig_name," BEFORE INSERT ON ",
     mig_data->qual[dmss_iter].owner,".",mig_data->qual[dmss_iter].table_name,
     " FOR EACH ROW BEGIN SELECT DM2_MIG_SEQ.NEXTVAL INTO :new.DM2_MIG_SEQ_ID FROM dual; END; ^) GO")
    IF (dmss_insert_ddl_op_row(null)=0)
     GO TO exit_script
    ENDIF
   ENDIF
 ENDFOR
 IF ((validate(dm2_migration_bypass_validation,- (1))=- (1)))
  SET dm_err->eproc = "Load primary key constraints which are not validated."
  SELECT
   IF ((dm2_rdbms_version->level1 < 10))
    FROM dba_constraints dc
    WHERE dc.constraint_type="P"
     AND dc.validated="NOT VALIDATED"
     AND dc.owner IN (
    (SELECT
     info_name
     FROM dm_info
     WHERE info_domain="DM2_MIG_USER"))
     AND  NOT (concat(dc.owner,".",dc.table_name) IN (
    (SELECT
     info_name
     FROM dm_info
     WHERE info_domain IN ("DM2_MIG_EXCL", "DM2_MIG_NONGG_REP", "DM2_MIG_ALLOW_NOPK"))))
   ELSE
    FROM dba_constraints dc
    WHERE dc.constraint_type="P"
     AND dc.validated="NOT VALIDATED"
    ...
    (SELECT
     ...
     FROM dm_info
     WHERE info_domain IN ("DM2_MIG_EXCL", "DM2_MIG_NONGG_REP", "DM2_MIG_ALLOW_NOPK"))))
     AND  NOT (list(dc.owner,dc.table_name) IN (
    (SELECT
     owner, object_name
     FROM dba_recyclebin)))
   ENDIF
   INTO "nl:"
   DETAIL
    dmss_pk_val->pk_cnt = (dmss_pk_val->pk_cnt+ 1)
    IF (mod(dmss_pk_val->pk_cnt,100)=1)
     stat = alterlist(dmss_pk_val->pks,(dmss_pk_val->pk_cnt+ 99))
    ENDIF
    dmss_pk_val->pks[dmss_pk_val->pk_cnt].pk_name = dc.constraint_name, dmss_pk_val->pks[dmss_pk_val
    ->pk_cnt].owner = dc.owner, dmss_pk_val->pks[dmss_pk_val->pk_cnt].table_name = dc.table_name
   FOOT REPORT
    stat = alterlist(dmss_pk_val->pks,dmss_pk_val->pk_cnt)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  FOR (dmss_iter = 1 TO dmss_pk_val->pk_cnt)
    SET dmss_op_type = "VALIDATE PK CONSTRAINT"
    IF ((dmss_pk_val->pks[dmss_iter].owner != "V500"))
     SET dmss_obj_name = concat(dmss_pk_val->pks[dmss_iter].owner,".",dmss_pk_val->pks[dmss_iter].
      table_name)
    ELSE
     SET dmss_obj_name = dmss_pk_val->pks[dmss_iter].table_name
    ENDIF
    SET dmss_table_name = dmss_pk_val->pks[dmss_iter].table_name
    SET dmss_priority = 470
    SET dmss_tablespace_name = "-1"
    SET dmss_row_cnt = 0
    SET dmss_owner = dmss_pk_val->pks[dmss_iter].owner
    SET dmss_operation = concat("RDB ASIS(^ALTER TABLE ",dmss_obj_name," MODIFY CONSTRAINT ",
     dmss_pk_val->pks[dmss_iter].pk_name," VALIDATE ^) GO")
    IF (dmss_insert_ddl_op_row(null)=0)
     GO TO exit_script
    ENDIF
  ENDFOR
 ENDIF
 IF ((mig_data->initsetup=1))
  SET dm_err->eproc = "Creating dm2_mig_seq sequence."
  IF (dm2_push_cmd(concat("RDB CREATE SEQUENCE DM2_MIG_SEQ INCREMENT BY 1 NOMAXVALUE CACHE ",
    dm2_db_options->cursor_commit_cnt," GO"),1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Compiling PLSQL TYPE V500.DM2_MIG_VC100_T"
 IF (dm2_push_cmd(concat("RDB ASIS(^ ",
   "CREATE OR REPLACE TYPE V500.DM2_MIG_VC100_T AS TABLE OF VARCHAR2(100); "," ^) go"),1)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Compiling Bulk Parallel Piped function"
 IF (dm2_push_cmd(concat("RDB ASIS(^ ","create or replace FUNCTION V500.dm2_mig_seq_id_bf (",
   "  dm2_parallel in NUMBER, ","  dm2_table_owner in VARCHAR2, ","  dm2_table_name in VARCHAR2, ",
   "  dm2_cursor_bf IN SYS_REFCURSOR, ","  dm2_limit in NUMBER, ",
   "  dm2_mig_seq_id_val in VARCHAR2) ","  RETURN V500.DM2_MIG_VC100_T ",
   "  PARALLEL_ENABLE (PARTITION dm2_cursor_bf BY ANY) ",
   "  PIPELINED ","  IS ","    PRAGMA AUTONOMOUS_TRANSACTION; ",
   "    type ROWID_TT is table of rowid index by binary_integer; ","    rowid_tab ROWID_TT; ",
   "    dm2_row_count NUMBER := 0; ","    updt_task_str VARCHAR2(30) := ', UPDT_TASK = 15301'; ",
   "    updt_task_ind NUMBER := 0;  ","    err_msg VARCHAR2(150); ",
   "    parallel_var NUMBER := dm2_parallel; ",
   "    limit_var NUMBER := dm2_limit;","  BEGIN",
   "    for xrec in (select info_name,info_number from dm_info where ",
   "    info_name in('LIMIT_OVERRIDE','PARALLEL_OVERRIDE') and ","    info_domain= 'DM2_MIG_SS') ",
   "    loop ","      if xrec.info_name = 'PARALLEL_OVERRIDE' then ",
   "        parallel_var := xrec.info_number; ","      else ",
   "        limit_var := xrec.info_number; ",
   "      end if;  ","    end loop; ","  DBMS_APPLICATION_INFO.SET_MODULE(MODULE_NAME=>",
   " 'DM2MIG_BF('||parallel_var||','||limit_var||','||dm2_mig_seq_id_val||')',",
   " action_name=>dm2_table_owner||'.'||dm2_table_name); ",
   "    SELECT count(",char(42),") into updt_task_ind ","    from dba_tab_columns d",
   "    where d.table_name = dm2_table_name",
   "    and d.column_name = 'UPDT_TASK'","    and d.owner = dm2_table_owner;","    ",
   "    IF(0 = UPDT_TASK_IND ) THEN","      updt_task_str := ' ';",
   "    END IF;","    LOOP","      FETCH dm2_cursor_bf BULK COLLECT INTO rowid_tab LIMIT limit_var;",
   "      EXIT WHEN rowid_tab.COUNT() = 0;","      FORALL i IN rowid_tab.FIRST .. rowid_tab.LAST",
   "        execute immediate 'UPDATE /",char(42),"+ ",
   " PARALLEL('||dm2_table_owner||'.'||dm2_table_name||', '||parallel_var||') ",char(42),
   "/ '||dm2_table_owner||'.'||dm2_table_name||",
   "        ' SET    dm2_mig_seq_id = ' || dm2_mig_seq_id_val|| updt_task_str ||",
   "        ' WHERE  rowid = :1' using rowid_tab(i);","      commit;",
   "      dm2_row_count := dm2_row_count + rowid_tab.COUNT;",
   "    DBMS_APPLICATION_INFO.SET_CLIENT_INFO(client_info=>'Processed '||dm2_row_count||' rows'); ",
   "    END LOOP;","    CLOSE dm2_cursor_bf;",
   "    DBMS_APPLICATION_INFO.SET_MODULE(MODULE_NAME=>NULL,action_name=>NULL); ",
   "    DBMS_APPLICATION_INFO.SET_CLIENT_INFO(client_info=>NULL); ",
   "    pipe row(dm2_row_count);","    RETURN;","  EXCEPTION","    WHEN OTHERS THEN","    ROLLBACK;",
   "    DBMS_APPLICATION_INFO.SET_MODULE(MODULE_NAME=>NULL,action_name=>NULL); ",
   "    DBMS_APPLICATION_INFO.SET_CLIENT_INFO(client_info=>NULL); ",
   " err_msg:=SUBSTR(sqlerrm,1,150);"," raise_application_error(-20555, err_msg);","  END;",
   "  "," ^) go"),1)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if public synonym exists from dba_synonyms."
 SELECT INTO "nl:"
  FROM dba_synonyms ds
  WHERE ds.owner="PUBLIC"
   AND ds.synonym_name="DM2_MIG_SEQ"
   AND ds.table_name="DM2_MIG_SEQ"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ( NOT (curqual))
  IF (dm2_push_cmd(concat("RDB CREATE PUBLIC SYNONYM DM2_MIG_SEQ FOR DM2_MIG_SEQ GO"),1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (dm2_push_cmd(concat("RDB GRANT SELECT ON DM2_MIG_SEQ TO PUBLIC GO"),1)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Getting row count of dm_vdata_master"
 SELECT INTO "nl:"
  x = count(*)
 ...
 ENDIF
 IF (dmss_dvm_cnt > 0)
  SET dm_err->eproc = "Truncating dm_vdata_master* tables"
  IF (dm2_push_cmd(concat("RDB TRUNCATE TABLE DM_VDATA_MASTER GO"),1)=0)
   GO TO exit_script
  ENDIF
  IF (dm2_push_cmd(concat("RDB TRUNCATE TABLE DM_VDATA_MISMATCH GO"),1)=0)
   GO TO exit_script
  ENDIF
  IF (dm2_push_cmd(concat("RDB TRUNCATE TABLE DM_VDATA_EXCLUDE_DTL GO"),1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((((mig_data->cnt > 0)) OR ((dmss_pk_val->pk_cnt > 0))) )
  IF (get_unique_file("dmss_ddl_report",".rpt")=0)
   GO TO exit_script
  ELSE
   SET dm2rpting->report_name = dm_err->unique_fname
  ENDIF
  SET dm2rpting->run_id = dmss_run_id
  SET dm2rpting->prompt_mode = "NOPROMPT"
  SET dm2rpting->report_option = "DDL to be executed"
  SET dm2rpting->status_criteria = "ddol.status = null "
  SET dm2rpting->install_mode = "SCHEMA_DATE"
  SET dm2rpting->schema_date = format(cnvtdatetime(dmss_schema_date),"DD-MMM-YYYY;;D")
  ...
  CALL clear(1,1)
  CALL video(n)
  CALL text(2,1,"Continue SOURCE setup?")
  CALL text(4,1,">>> Enter 'C' to continue continue SOURCE setup or 'Q' to quit (C or Q):")
  CALL accept(4,79,"A;CU"," "
   WHERE curaccept IN ("C", "Q"))
  SET message = nowindow
  IF (curaccept="Q")
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Prompt user to continue with SOURCE setup."
   SET dm_err->emsg = "User chose to quit from SOURCE setup."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Calculating incremental space needs"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Get previous value of num rows for tables"
 SELECT INTO "nl:"
  FROM dm2_ddl_ops_log ddol
  WHERE (ddol.run_id=
  (SELECT
   ddo.run_id
   FROM dm2_ddl_ops ddo
   WHERE ddo.process_option="DATABASE MIGRATION"
    AND ddo.last_checkpoint="COMPLETE"))
   AND ddol.op_type="ADD COLUMN"
   AND ddol.obj_name=patstring("*DM2_MIG_SEQ_ID")
  DETAIL
   IF (findstring(".",ddol.obj_name) > 0)
    dmss_owner = substring(1,(findstring(".",ddol.obj_name) - 1),ddol.obj_name)
   ELSE
    dmss_owner = "V500"
   ENDIF
   dmss_ndx = locateval(dmss_iter,1,mig_data->cnt,dmss_owner,mig_data->qual[dmss_iter].owner,
    ddol.table_name,mig_data->qual[dmss_iter].table_name)
   IF (dmss_ndx > 0)
    mig_data->qual[dmss_ndx].num_rows_prior = ddol.row_cnt
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 FOR (dmss_iter = 1 TO mig_data->cnt)
   IF ((mig_data->qual[dmss_iter].col_new_ind=1))
    SET mig_data->qual[dmss_iter].table_size = ((10+ 3) * mig_data->qual[dmss_iter].num_rows)
    IF ((mig_data->qual[dmss_iter].ext_mgmt="D"))
     SET mig_data->qual[dmss_iter].table_size = (mig_data->qual[mig_data->cnt].tbl_next_ext * dm2ceil
     ((mig_data->qual[dmss_iter].table_size/ mig_data->qual[mig_data->cnt].tbl_next_ext)))
    ELSE
     SET mig_data->qual[dmss_iter].tbl_next_ext = dmss_calc_lmt_ext_size((mig_data->qual[dmss_iter].
      table_size+ mig_data->qual[dmss_iter].tbl_cur_bytes))
     SET mig_data->qual[dmss_iter].table_size = (mig_data->qual[dmss_iter].tbl_next_ext * dm2ceil((
      mig_data->qual[dmss_iter].table_size/ mig_data->qual[dmss_iter].tbl_next_ext)))
    ENDIF
   ENDIF
   IF ((mig_data->qual[dmss_iter].col_bf_init_ind=1)
    AND (mig_data->qual[dmss_iter].num_rows > mig_data->qual[dmss_iter].num_rows_prior))
    SET mig_data->qual[dmss_iter].table_size = ((10+ 3) * (mig_data->qual[dmss_iter].num_rows -
    mig_data->qual[dmss_iter].num_rows_prior))
    SET mig_data->qual[dmss_iter].incremental_space_ind = 1
    IF ((mig_data->qual[dmss_iter].ext_mgmt="D"))
     SET mig_data->qual[dmss_iter].table_size = (mig_data->qual[mig_data->cnt].tbl_next_ext * dm2ceil
     ((mig_data->qual[dmss_iter].table_size/ mig_data->qual[mig_data->cnt].tbl_next_ext)))
    ELSE
   ...
   ENDIF
   IF ((mig_data->qual[dmss_iter].ind_new_ind=1))
    SET mig_data->qual[dmss_iter].ind_size = (((10+ 9) * mig_data->qual[dmss_iter].num_rows) * 1.45)
    SET mig_data->qual[dmss_iter].ind_size = (mig_data->qual[dmss_iter].ind_size+ cons_64k)
    SET mig_data->qual[dmss_iter].tbl_next_ext = dmss_calc_lmt_ext_size(mig_data->qual[dmss_iter].
     ind_size)
    SET mig_data->qual[dmss_iter].ind_size = (mig_data->qual[dmss_iter].ind_next_ext * dm2ceil((
     mig_data->qual[dmss_iter].ind_size/ mig_data->qual[dmss_iter].ind_next_ext)))
   ENDIF
 ENDFOR
 IF ((dm_err->debug_flag > 7))
  CALL echorecord(mig_data)
 ENDIF
 SET dm_err->eproc = "Calculating tspace needs in record structure mig_data."
 SELECT INTO "nl:"
  tspace = substring(1,30,mig_data->qual[d.seq].tspace_name)
  FROM (dummyt d  WITH seq = value(mig_data->cnt))
  WHERE (mig_data->qual[d.seq].tspace_name > "")
  ORDER BY tspace
  HEAD REPORT
   tspace_rec->cnt = 0, stat = alterlist(tspace_rec->qual,0)
  HEAD tspace
   tspace_rec->cnt = (tspace_rec->cnt+ 1)
   IF (mod(tspace_rec->cnt,25)=1)
    stat = alterlist(tspace_rec->qual,(tspace_rec->cnt+ 24))
   ENDIF
   tspace_rec->qual[tspace_rec->cnt].tspace_name = mig_data->qual[d.seq].tspace_name, tspace_rec->
   qual[tspace_rec->cnt].ext_mgmt = mig_data->qual[d.seq].ext_mgmt
  DETAIL
   IF ((((mig_data->qual[d.seq].col_new_ind=1)) OR ((mig_data->qual[d.seq].incremental_space_ind=1)
   )) )
    tspace_rec->qual[tspace_rec->cnt].space_needed = (tspace_rec->qual[tspace_rec->cnt].space_needed
    + mig_data->qual[d.seq].table_size), tspace_rec->qual[tspace_rec->cnt].max_init_ext = greatest(
     tspace_rec->qual[tspace_rec->cnt].max_init_ext,mig_data->qual[d.seq].tbl_init_ext), tspace_rec->
    qual[tspace_rec->cnt].max_next_ext = greatest(tspace_rec->qual[tspace_rec->cnt].max_next_ext,
     mig_data->qual[d.seq].tbl_next_ext)
   ENDIF
  FOOT  tspace
   tspace_rec->qual[tspace_rec->cnt].chunks_needed = dm2ceil((tspace_rec->qual[tspace_rec->cnt].
    space_needed/ tspace_rec->qual[tspace_rec->cnt].max_next_ext)), tspace_rec->qual[tspace_rec->cnt]
   .bytes_to_add = ((tspace_rec->qual[tspace_rec->cnt].chunks_needed * tspace_rec->qual[tspace_rec->
   cnt].max_next_ext)+ cons_1m)
  FOOT REPORT
   stat = alterlist(tspace_rec->qual,tspace_rec->cnt)
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET tspace_rec->cnt = (tspace_rec->cnt+ 1)
 SET stat = alterlist(tspace_rec->qual,tspace_rec->cnt)
 SET tspace_rec->qual[tspace_rec->cnt].tspace_name = "I_DM2MIG"
 SET tspace_rec->qual[tspace_rec->cnt].ext_mgmt = "L"
 FOR (dmss_iter = 1 TO mig_data->cnt)
   IF ((mig_data->qual[dmss_iter].ind_new_ind=1))
    SET tspace_rec->qual[tspace_rec->cnt].space_needed = (tspace_rec->qual[tspace_rec->cnt].
    space_needed+ mig_data->qual[dmss_iter].ind_size)
    SET tspace_rec->qual[tspace_rec->cnt].max_init_ext = greatest(tspace_rec->qual[tspace_rec->cnt].
     max_init_ext,mig_data->qual[dmss_iter].ind_init_ext)
    SET tspace_rec->qual[tspace_rec->cnt].max_next_ext = greatest(tspace_rec->qual[tspace_rec->cnt].
     max_next_ext,mig_data->qual[dmss_iter].ind_next_ext)
    SET tspace_rec->qual[tspace_rec->cnt].chunks_needed = dm2ceil((tspace_rec->qual[tspace_rec->cnt].
     space_needed/ tspace_rec->qual[tspace_rec->cnt].max_next_ext))
    SET tspace_rec->qual[tspace_rec->cnt].bytes_to_add = ((tspace_rec->qual[tspace_rec->cnt].
    chunks_needed * tspace_rec->qual[tspace_rec->cnt].max_next_ext)+ cons_1m)
   ENDIF
 ENDFOR
 IF ((dmr_mig_data->src_storage_type != "ASM"))
  SET dm_err->eproc = "Determining initial free space from dba_free_space."
  SELECT INTO "nl:"
   dfs.tablespace_name, total_bytes = sum(dfs.bytes)
   FROM dba_free_space dfs
   GROUP BY dfs.tablespace_name
   DETAIL
    IF (assign(dmss_search,locateval(dmss_search,1,tspace_rec->cnt,dfs.tablespace_name,tspace_rec->
      qual[dmss_search].tspace_name)) > 0)
     tspace_rec->qual[dmss_search].initial_free_space = total_bytes
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Determining if free space row already exists."
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE "DM2_MIG_TSPACE_SIZE"=di.info_domain
   DETAIL
    IF (assign(dmss_search,locateval(dmss_search,1,tspace_rec->cnt,di.info_name,tspace_rec->qual[
      dmss_search].tspace_name)) > 0)
     tspace_rec->qual[dmss_search].dm_info_row_exists = 1, tspace_rec->qual[dmss_search].
     initial_free_space = di.info_number
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
  SET dm_err->eproc = "Inserting free space row into dm_info."
  INSERT  FROM dm_info di,
    (dummyt d  WITH seq = value(tspace_rec->cnt))
   SET di.info_domain = "DM2_MIG_TSPACE_SIZE", di.info_name = tspace_rec->qual[d.seq].tspace_name, di
    .info_number = tspace_rec->qual[d.seq].initial_free_space,
    di.info_char = dmss_tspace_info_char, tspace_rec->qual[d.seq].dm_info_row_exists = 1
   PLAN (d
    WHERE (tspace_rec->qual[d.seq].dm_info_row_exists=0)
     AND (tspace_rec->qual[d.seq].tspace_name > ""))
    JOIN (di)
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
  ...
  COMMIT
 ENDIF
 IF ((validate(dm2_migration_bypass_tspace,- (1))=- (1))
  AND (validate(dm2_migration_bypass_tspace,- (2))=- (2)))
  IF (dmss_calc_tspace_needs(null)=0)
   GO TO exit_script
  ENDIF
 ELSE
  SET dm_err->eproc = "User elected to bypass tablespace logic"
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 IF ((tspace_rec->space_needed_ind=1))
  IF ((( NOT ((dm2_sys_misc->cur_db_os IN ("AXP", "VMS")))) OR ((dm2_sys_misc->cur_db_os !=
  dm2_sys_misc->cur_os))) )
   SET dmss_tspace_maint_choice = "M"
  ELSE
   SET message = window
   CALL clear(1,1)
   CALL text(1,1,"About to begin Tablespace Maintenance.  ")
   CALL text(5,4,"Please enter M to view a report of tablespace needs and set of")
   CALL text(6,4,"sample commands to complete tablespace maintenance.")
   CALL text(8,4,"Please enter A to continue on to Cerner's tablespace maintenance utility.")
   CALL text(10,4,"(A)uto, (M)anual, (Q)uit:")
   CALL accept(10,31,"A;CU"," "
    WHERE curaccept IN ("A", "M", "Q"))
   CALL clear(1,1)
   SET message = nowindow
   SET dmss_tspace_maint_choice = curaccept
  ENDIF
  CASE (dmss_tspace_maint_choice)
   OF "Q":
    SET dm_err->emsg = "User Chose to Quit from Mode Prompt during Tablespace Assessment."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   OF "A":
    SET dm_err->eproc = "Writing tablespace needs to logfile."
    SELECT INTO dm_err->logfile
     FROM (dummyt d  WITH seq = value(rtspace->rtspace_cnt))
     HEAD REPORT
      col 1, "Tablespaces and size needed", row + 2,
      col 1, "TABLESPACE NAME", col 35,
      "BYTES NEEDED", row + 2
     DETAIL
      col 1, rtspace->qual[d.seq].tspace_name, col 35,
      rtspace->qual[d.seq].bytes_needed, row + 1
    ...
    ENDIF
   OF "M":
    IF (dmss_rpt_filename="")
     IF (get_unique_file("dmss_tspace_needs",".rpt")=0)
      GO TO exit_script
     ELSE
      SET dmss_rpt_filename = dm_err->unique_fname
     ENDIF
    ENDIF
    IF (dmss_pop_current_size(null)=0)
     GO TO exit_script
    ENDIF
    IF ((dmr_mig_data->src_storage_type="ASM"))
     WHILE (dmss_tspace_needs_viewed=0)
       IF (dmss_process_asm_tspace(null)=0)
        GO TO exit_script
       ENDIF
     ENDWHILE
    ELSE
     WHILE ((tspace_rec->space_needed_ind=1))
       IF (dmss_manual_process_tspace(null)=0)
        GO TO exit_script
       ENDIF
     ENDWHILE
    ENDIF
  ENDCASE
 ENDIF
 SET dm_err->eproc = "Verify that I_DM2MIG exists"
 SELECT INTO "nl:"
  FROM dba_tablespaces
  WHERE "I_DM2MIG"=tablespace_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(mig_user)
 ENDIF
 IF (curqual > 0)
  SET dmss_iter = 0
  FOR (dmss_iter = 1 TO mig_user->user_cnt)
    SET dmss_priv_give = 0
    SET dm_err->eproc = concat("Determine if ",mig_user->qual[dmss_iter].migusername,
     " has UNLIMITED TABLESPACE system privilege")
    SELECT INTO "nl:"
     FROM dba_sys_privs dsp
     WHERE (dsp.grantee=mig_user->qual[dmss_iter].migusername)
    ...
    IF (curqual=0)
     SET dm_err->eproc = concat("Determine if ",mig_user->qual[dmss_iter].migusername,
      "  has unlimited quota on I_DM2MIG")
     SELECT INTO "nl:"
      FROM dba_ts_quotas dtq
      WHERE (dtq.username=mig_user->qual[dmss_iter].migusername)
       AND dtq.tablespace_name="I_DM2MIG"
       AND dtq.max_blocks < 0
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     ENDIF
     IF (curqual=0)
      SET dmss_priv_give = 1
     ELSE
      SET dmss_priv_give = 0
     ENDIF
    ENDIF
    IF (dmss_priv_give=1)
     SET dm_err->eproc = concat("Adding I_DM2MIG privs for ",mig_user->qual[dmss_iter].migusername)
     IF (dm2_push_cmd(concat("rdb alter user ",mig_user->qual[dmss_iter].migusername,
       " quota unlimited on I_DM2MIG go"),1)=0)
      GO TO exit_script
     ENDIF
    ENDIF
  ENDFOR
 ENDIF
 IF ((validate(dm2_troubleshoot_migration,- (1)) != - (1)))
  SET message = window
  SET width = 132
  CALL clear(1,1)
  CALL video(n)
  CALL text(2,1,"Troubleshooting migration process: ")
  CALL text(4,1,">>> Enter 'C' to continue execution of ddl or 'Q' to quit (C or Q):")
  ...
  ENDIF
 ENDIF
 SET dm2_install_schema->run_id = dmss_run_id
 EXECUTE dm2_set_ready_to_run "ALL"
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if entire process is complete from dm2_ddl_ops_log."
 SELECT INTO "nl:"
  FROM dm2_ddl_ops_log ddol
  WHERE ddol.run_id=dmss_run_id
   AND (("COMPLETE" != ddol.status) OR (ddol.status = null))
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Deleting initial tspace size rows from dm_info."
  DELETE  FROM dm_info di
   WHERE "DM2_MIG_TSPACE_SIZE"=di.info_domain
   WITH nocounter
  ;end delete
 ...
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "There are still rows not marked as complete in dm2_ddl_ops_log for this run_id."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 FOR (dmss_iter = 1 TO mig_user->user_cnt)
   SET dm_err->eproc = concat("Turn on table monitoring for ",mig_user->qual[dmss_iter].migusername)
   EXECUTE dm2_setup_table_monitoring mig_user->qual[dmss_iter].migusername
   IF ((dm_err->err_ind > 0))
    SET dm_err->eproc = "Errors from DM2_SETUP_TABLE_MONITORING are ignorable."
    SET dm_err->err_ind = 0
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Determining if schema_setup_complete row already exists in dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_SETUP"
 ...
 ENDIF
 IF ( NOT (curqual))
  SET dm_err->eproc = "Inserting schema_setup_complete row into dm_info."
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2_MIG_SETUP", di.info_name = "SCHEMA_SETUP_COMPLETE"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 GO TO exit_script
 SUBROUTINE dmss_disp_tspace_rpt(null)
   SET dm_err->eproc = "Displaying Tablespace report."
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE ddt_type_disp = i2 WITH protect, noconstant(0)
   DECLARE ddt_ndx = i4 WITH protect, noconstant(0)
   DECLARE ddt_exist_needs_space = i2 WITH protect, noconstant(0)
   DECLARE ddt_bytes = f8 WITH protect, noconstant(0.0)
   IF ((dmr_mig_data->src_storage_type != "ASM"))
    IF (locateval(ddt_ndx,1,size(rtspace->qual,5),1,rtspace->qual[ddt_ndx].new_ind) > 0)
     SET ddt_type_disp = 1
    ENDIF
    IF (locateval(ddt_ndx,1,size(rtspace->qual,5),0,rtspace->qual[ddt_ndx].new_ind) > 0)
     IF (ddt_type_disp=1)
      SET ddt_type_disp = 3
     ELSE
      SET ddt_type_disp = 2
     ENDIF
    ENDIF
    SET dm_err->eproc = concat("Generating ",dmss_rpt_filename)
    SELECT INTO value(dmss_rpt_filename)
     FROM (dummyt d  WITH seq = size(rtspace->qual,5))
     PLAN (d
      WHERE d.seq > 0
       AND (rtspace->qual[d.seq].bytes_needed > 0))
     HEAD REPORT
      row + 1, col 0,
      "The following report contains the tablespace sizing needs that must be completed",
      row + 1, col 0,
      "before continuing.  Example SQL commands are located at the bottom of this report.",
      row + 2, col 0, "Tablespace Needs      ",
      row + 1, col 0,
      "                                Current           Space                Minimum ",
      row + 1, col 0,
      "                                Total Size        Needed               Chunk Size           Number of",
      row + 1, col 0,
      "Tablespace Name                 (MBYTES)          (MBYTES)             (MBYTES)             Chunks Needed",
      row + 1, col 0,
      "_______________                 ______________    ______________        __________          _____________",
      row + 1
     DETAIL
      col 0, rtspace->qual[d.seq].tspace_name
      IF ((rtspace->qual[d.seq].new_ind=1))
       col 32, "NEW"
      ELSE
       ddt_bytes = round(((rtspace->qual[d.seq].cur_bytes_allocated/ 1024.0)/ 1024.0),2), col 32,
       ddt_bytes
      ENDIF
      ddt_bytes = round(((rtspace->qual[d.seq].bytes_needed/ 1024.0)/ 1024.0),2), col 50, ddt_bytes,
      ddt_bytes = round(((rtspace->qual[d.seq].chunk_size/ 1024.0)/ 1024.0),2), col 68, ddt_bytes,
      col 94, rtspace->qual[d.seq].chunks_needed, row + 1
     FOOT REPORT
      row + 2, col 0,
      "What is a chunk?  A chunk is a block of contiguous space within a tablespace.  ",
      row + 2, col 0, "Tablespace Name = Contains list of tablespaces that need to be created   ",
      row + 1, col 0, "                  or existing tablespaces that require additional free space.",
      row + 2, col 0, "Current Total Size = Lists size of existing tablespaces in MBYTES or marks  ",
      row + 1, col 0, "                     tablespaces as NEW         ",
      row + 2, col 0, "Space Needed = Minimum amount of space in MBYTES that needs to be added to",
      row + 1, col 0, "               existing tablespaces or new tablespaces.     ",
      row + 2, col 0, "Minimum Chunk Size = The minimum data file size in MBYTES that can be added",
      row + 1, col 0,
      "                     to an existing or new tablespace or minimum size in MBYTES",
      row + 1, col 0, "                     that an existing data file can be extended.    ",
      row + 2, col 0,
      "Number of Chunks Needed = The minimum number of chunks need to satisfy tablespace needs. ",
      row + 2, col 0, "Tablespace Maintenance Examples",
      row + 2
      IF (ddt_type_disp IN (2, 3))
       IF (cursys="AIX")
        col 0, "Add Data File to Existing Tablespace    ", row + 2,
        col 0, "  Sql> alter tablespace D_A_SMALL add datafile", row + 1,
        col 0, "       '/dev/rdtest_0000099' size 15M;", row + 2,
        col 0, "Extend an Existing Tablespace   ", row + 2,
        col 0, "  Sql> alter database test datafile ", row + 1,
        col 0, "       '/dev/rdtest_0000099' resize 31M;", row + 2
       ELSE
        col 0, "Add Data File to Existing Tablespace    ", row + 2,
        col 0, "  Sql> alter tablespace D_A_SMALL add datafile", row + 1,
        col 0, "       'data01:[v500.db_test]d_a_small_002.dbs' size 16M;", row + 2,
        col 0, "Extend an Existing Tablespace   ", row + 2,
        col 0, "  Sql> alter database test datafile ", row + 1,
        col 0, "       'data01:[v500.db_test]d_a_small_001.dbs' resize 32M;", row + 2
       ENDIF
      ENDIF
      IF (ddt_type_disp IN (1, 3))
       IF ((dm2_db_options->new_tspace_type="L"))
        IF (cursys="AIX")
         col 0, "Create New Tablespace  ", row + 2,
         col 0, "  Sql> create tablespace D_A_SMALL datafile", row + 1,
         col 0, "       '/dev/rdtest_0000099' size 15M", row + 1,
         col 0, "       extent management local autoallocate segment space management auto;", row + 2
        ELSE
         col 0, "Create New Tablespace  ", row + 2,
         col 0, "  Sql> create tablespace D_A_SMALL datafile", row + 1,
         col 0, "       'data01:[v500.db_test]d_a_small_001.dbs' size 15M", row + 1,
         col 0, "        extent management local autoallocate segment space management auto;", row +
         2
        ENDIF
       ELSE
        IF (cursys="AIX")
         ...
         col 0, "  Sql> create tablespace D_A_SMALL datafile", row + 1,
         col 0, "       '/dev/rdtest_0000099' size 15M", row + 1,
         col 0, "       extent management dictionary;   ", row + 2
        ELSE
         col 0, "Create New Tablespace  ", row + 2,
         col 0, "  Sql> create tablespace D_A_SMALL datafile", row + 1,
         col 0, "       'data01:[v500.db_test]d_a_small_001.dbs' size 15M", row + 1,
   ...
   ELSE
    ...
    ENDIF
    SELECT INTO value(dmss_rpt_filename)
     FROM (dummyt d  WITH seq = value(tspace_rec->tot_dg_cnt))
     PLAN (d
      WHERE d.seq > 0)
     HEAD REPORT
      col 0, "Tablespace Maintenance Report for ASM", row + 2,
      col 0,
      "The following report displays the estimated amount of space that will be consumed by database migration setup.",
      row + 2
      IF ((tspace_rec->asm_space_needed_ind=1))
       col 0,
       "*************************************************************************************************",
       row + 1,
       col 0,
       "WARNING! One or more disk groups do not contain free space adequate for source setup.            ",
       row + 1,
       col 0,
       "*************************************************************************************************",
       row + 2
      ENDIF
      col 0, "Calculated Space Needs for each Disk Group :  ", row + 2,
      ...
      col 0, "  ___________________                           _______________   ____________ ", row
       + 2
     DETAIL
      IF ((tspace_rec->tot_dg[d.seq].bytes_to_add > 0))
       col 2, tspace_rec->tot_dg[d.seq].dg_name, col 49,
       CALL print(dm2ceil(((tspace_rec->tot_dg[d.seq].bytes_to_add/ 1024.0)/ 1024.0))), col 64,
       CALL print(dm2ceil(tspace_rec->tot_dg[d.seq].free_space_mb)),
       row + 1
      ENDIF
     FOOT REPORT
    ...
    ENDIF
   ENDIF
   IF (dm2_disp_file(dmss_rpt_filename,concat("Report File may be found in CCLUSERDIR:",
     dmss_rpt_filename))=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmss_calc_tspace_needs(null)
   DECLARE dctn_df_to_add = i2 WITH protect, noconstant(0)
   DECLARE dctn_cnt = i4 WITH protect, noconstant(0)
   DECLARE dctn_str = vc WITH protect, noconstant("")
   DECLARE dctn_dg = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Determining tablespaces already exist in dba_tablespaces."
   SELECT INTO "nl:"
    FROM dba_tablespaces dt
    DETAIL
     IF (assign(dmss_search,locateval(dmss_search,1,tspace_rec->cnt,dt.tablespace_name,tspace_rec->
       qual[dmss_search].tspace_name)) > 0)
      tspace_rec->qual[dmss_search].tspace_exists_ind = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   FOR (dmss_iter = 1 TO tspace_rec->cnt)
     IF ((tspace_rec->qual[dmss_iter].tspace_exists_ind=1))
      IF (dm2_push_cmd(concat("rdb alter tablespace ",tspace_rec->qual[dmss_iter].tspace_name,
        " coalesce go"),1)=0)
       RETURN(0)
      ENDIF
     ELSE
      IF ((dmr_mig_data->src_storage_type="ASM"))
       SET dctn_df_to_add = dm2ceil((tspace_rec->qual[dmss_iter].space_needed/ (((32.0 * 1024.0) *
        1024.0) * 1024.0)))
       ...
       WHILE (dctn_dg="")
         SET message = window
         CALL clear(1,1)
         SET help = pos(9,8,10,100)
         CALL box(1,1,8,100)
         CALL text(3,2,concat("Migration tablespace, ",tspace_rec->qual[dmss_iter].tspace_name,
           " , requires ",trim(cnvtstring(dm2ceil(((tspace_rec->qual[dmss_iter].space_needed/ 1024.0)
              / 1024.0))))," MB of Space."))
         CALL text(4,2,"Choose the disk group that should be used to create the tablespace.")
         CALL text(6,2,"Disk Group:")
         SET help =
         SELECT INTO "nl:"
          disk_group___________________ = trim(rdisk->qual[d.seq].disk_name), __block_size = trim(
           cnvtstring(rdisk->qual[d.seq].block_size_b)), allocation_unit = trim(cnvtstring(rdisk->
            qual[d.seq].alloc_unit_b)),
          ...
          FROM (dummyt d  WITH seq = value(rdisk->disk_cnt))
          WHERE d.seq > 0
          WITH nocounter
         ;end select
         CALL accept(6,14,"P(9);cuF")
         IF (curhelp > 0)
          SET dctn_dg = rdisk->qual[curhelp].disk_name
         ELSE
          IF (locateval(dmss_search,1,rdisk->disk_cnt,curaccept,rdisk->qual[dmss_search].disk_name)=0
          )
           SET dctn_dg = ""
           CALL text(7,2,concat(curaccept," is not a valid disk group. Choose a valid disk group."))
           CALL pause(5)
          ELSE
           SET dctn_dg = curaccept
          ENDIF
         ENDIF
         CALL clear(1,1)
         SET help = off
         SET message = nowindow
       ENDWHILE
       SET dctn_str = concat("rdb create tablespace ",tspace_rec->qual[dmss_iter].tspace_name,
        " DATAFILE ")
       FOR (dctn_cnt = 1 TO dctn_df_to_add)
        IF (dctn_cnt=1)
         SET dctn_str = concat(dctn_str," '+",dctn_dg,"' ")
        ELSE
         SET dctn_str = concat(dctn_str,",'+",dctn_dg,"' ")
        ENDIF
        SET dctn_str = concat(dctn_str," size 1M autoextend on next 64M ")
       ENDFOR
       SET dctn_str = concat(dctn_str," go")
       IF (dm2_push_cmd(dctn_str,1)=0)
        RETURN(0)
       ENDIF
      ENDIF
     ENDIF
   ...
   IF ((dmr_mig_data->src_storage_type="ASM"))
    FOR (dmss_iter = 1 TO tspace_rec->cnt)
     SET tspace_rec->qual[dmss_iter].dg_cnt = 0
     SET stat = alterlist(tspace_rec->qual[dmss_iter].dg,0)
    ENDFOR
    SET tspace_rec->tot_dg_cnt = 0
    SET stat = alterlist(tspace_rec->tot_dg,0)
    SET dm_err->eproc = "Check which disk groups the tablespaces are using."
    SELECT INTO "nl:"
     FROM (dummyt d  WITH seq = value(tspace_rec->cnt)),
      dba_data_files df
     PLAN (d
      WHERE d.seq > 0)
      JOIN (df
      WHERE (df.tablespace_name=tspace_rec->qual[d.seq].tspace_name))
     ORDER BY df.tablespace_name
     DETAIL
      dmss_str = substring(2,(findstring("/",df.file_name,2,0) - 2),df.file_name), dmss_search = 0
      IF ((tspace_rec->qual[d.seq].dg_cnt > 0))
       dmss_search = locateval(dmss_search,1,tspace_rec->qual[d.seq].dg_cnt,dmss_str,tspace_rec->
        qual[d.seq].dg[dmss_search].dg_name)
      ENDIF
      IF (dmss_search=0)
       tspace_rec->qual[d.seq].dg_cnt = (tspace_rec->qual[d.seq].dg_cnt+ 1), stat = alterlist(
        tspace_rec->qual[d.seq].dg,tspace_rec->qual[d.seq].dg_cnt), tspace_rec->qual[d.seq].dg[
       tspace_rec->qual[d.seq].dg_cnt].dg_name = dmss_str
      ENDIF
      dmss_search = 0
      IF ((tspace_rec->tot_dg_cnt > 0))
       dmss_search = locateval(dmss_search,1,tspace_rec->tot_dg_cnt,dmss_str,tspace_rec->tot_dg[
        dmss_search].dg_name)
      ENDIF
      IF (dmss_search=0)
       tspace_rec->tot_dg_cnt = (tspace_rec->tot_dg_cnt+ 1), stat = alterlist(tspace_rec->tot_dg,
        tspace_rec->tot_dg_cnt), tspace_rec->tot_dg[tspace_rec->tot_dg_cnt].dg_name = dmss_str
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
    FOR (dmss_iter = 1 TO tspace_rec->cnt)
      IF ((tspace_rec->qual[dmss_iter].space_needed > 0))
       FOR (dmss_iter2 = 1 TO tspace_rec->qual[dmss_iter].dg_cnt)
         SET dmss_search = 0
         SET dmss_search = locateval(dmss_search,1,tspace_rec->tot_dg_cnt,tspace_rec->qual[dmss_iter]
          .dg[dmss_iter2].dg_name,tspace_rec->tot_dg[dmss_search].dg_name)
         SET tspace_rec->tot_dg[dmss_search].bytes_to_add = (tspace_rec->tot_dg[dmss_search].
         bytes_to_add+ dm2ceil((tspace_rec->qual[dmss_iter].space_needed/ tspace_rec->qual[dmss_iter]
          .dg_cnt)))
       ENDFOR
      ENDIF
    ENDFOR
   ENDIF
   IF ((dmr_mig_data->src_storage_type != "ASM"))
    SET dm_err->eproc = "Determining free space from dba_free_space."
    SELECT INTO "nl:"
     FROM dba_free_space dfs
     ORDER BY dfs.tablespace_name
     HEAD REPORT
      free_chunks_available = 0, total_free_chunks_available = 0, chunks_to_add = 0
     HEAD dfs.tablespace_name
      total_free_chunks_available = 0, dmss_search = locateval(dmss_search,1,tspace_rec->cnt,dfs
       .tablespace_name,tspace_rec->qual[dmss_search].tspace_name)
     DETAIL
      IF (dmss_search > 0)
       free_chunks_available = dm2floor((dfs.bytes/ tspace_rec->qual[dmss_search].max_next_ext)),
       total_free_chunks_available = (total_free_chunks_available+ free_chunks_available)
      ENDIF
     FOOT  dfs.tablespace_name
      IF (dmss_search > 0)
       IF (validate(dm2_mig_reserve_free_space,"X")="X"
        AND validate(dm2_mig_reserve_free_space,"Y")="Y")
        total_free_chunks_available = (total_free_chunks_available - dm2ceil((tspace_rec->qual[
         dmss_search].initial_free_space/ tspace_rec->qual[dmss_search].max_next_ext)))
        IF ((dm_err->debug_flag > 1))
         CALL echo(concat("total_free_chunks_available = ",cnvtstring(total_free_chunks_available))),
         CALL echo(cnvtstring((tspace_rec->qual[dmss_search].initial_free_space/ tspace_rec->qual[
          dmss_search].max_next_ext)))
        ENDIF
       ENDIF
       chunks_to_add = (tspace_rec->qual[dmss_search].chunks_needed - total_free_chunks_available)
       IF (chunks_to_add > 0)
        tspace_rec->qual[dmss_search].bytes_to_add = ((chunks_to_add * tspace_rec->qual[dmss_search].
        max_next_ext)+ cons_1m)
       ELSE
        tspace_rec->qual[dmss_search].bytes_to_add = 0
       ENDIF
      ENDIF
     WITH nocounter
    ;end select
    ...
    ENDIF
   ELSE
    SET dm_err->eproc = "Determining free space from v$asm_diskgroup."
    SELECT INTO "nl:"
     FROM v$asm_diskgroup dg,
      (dummyt d  WITH seq = value(tspace_rec->tot_dg_cnt))
     PLAN (d
      WHERE d.seq > 0)
      JOIN (dg
      WHERE (tspace_rec->tot_dg[d.seq].dg_name=dg.name))
     ORDER BY dg.name
     DETAIL
      tspace_rec->tot_dg[d.seq].free_space_mb = dg.free_mb
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dmr_mig_data->src_storage_type != "ASM"))
    SET tspace_rec->space_needed_ind = 0
    FOR (dmss_iter = 1 TO tspace_rec->cnt)
      IF ((tspace_rec->qual[dmss_iter].bytes_to_add > 0))
       SET tspace_rec->space_needed_ind = 1
       SET dmss_iter = (tspace_rec->cnt+ 1)
      ENDIF
    ENDFOR
   ELSE
    SET tspace_rec->space_needed_ind = 0
    SET tspace_rec->asm_space_needed_ind = 0
    FOR (dmss_iter = 1 TO tspace_rec->tot_dg_cnt)
      IF ((dm2floor(((tspace_rec->tot_dg[dmss_iter].bytes_to_add/ 1024.0)/ 1024.0)) > tspace_rec->
      tot_dg[dmss_iter].free_space_mb))
       SET tspace_rec->asm_space_needed_ind = 1
       SET tspace_rec->space_needed_ind = 1
      ELSEIF (dm2floor(((tspace_rec->tot_dg[dmss_iter].bytes_to_add/ 1024.0)/ 1024.0)) > 0)
       SET tspace_rec->space_needed_ind = 1
      ENDIF
    ENDFOR
   ENDIF
   ...
   IF ((tspace_rec->space_needed_ind=1))
    ...
    FOR (dmss_iter = 1 TO tspace_rec->cnt)
      IF ((tspace_rec->qual[dmss_iter].bytes_to_add > 0))
       ...
       SET rtspace->qual[rtspace->rtspace_cnt].tspace_name = tspace_rec->qual[dmss_iter].tspace_name
       SET rtspace->qual[rtspace->rtspace_cnt].new_ind = (1 - tspace_rec->qual[dmss_iter].
       tspace_exists_ind)
       SET rtspace->qual[rtspace->rtspace_cnt].ext_mgmt = tspace_rec->qual[dmss_iter].ext_mgmt
       SET rtspace->qual[rtspace->rtspace_cnt].bytes_needed = tspace_rec->qual[dmss_iter].
       bytes_to_add
       SET rtspace->qual[rtspace->rtspace_cnt].chunk_size = tspace_rec->qual[dmss_iter].max_next_ext
    ...
    SET rtspace->install_type_value = format(cnvtdatetime(dmss_schema_date),"DD-MMM-YYYY;;D")
    SET dm_err->eproc = "Deleting rows from dm2_tspace_size."
    DELETE  FROM dm2_tspace_size dts,
      (dummyt d  WITH seq = value(rtspace->rtspace_cnt))
     SET dts.seq = 1
     PLAN (dts)
      JOIN (d
      WHERE (dts.install_type=rtspace->install_type)
       AND (dts.install_type_value=rtspace->install_type_value)
       AND (dts.tspace_name=rtspace->qual[d.seq].tspace_name))
     WITH nocounter
    ;end delete
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Inserting new rows into dm2_tspace_size."
    INSERT  FROM dm2_tspace_size dt,
      (dummyt d  WITH seq = value(rtspace->rtspace_cnt))
     SET dt.bytes_needed = rtspace->qual[d.seq].bytes_needed, dt.new_ind = rtspace->qual[d.seq].
      new_ind, dt.tspace_name = rtspace->qual[d.seq].tspace_name,
      dt.install_type = rtspace->install_type, dt.install_type_value = rtspace->install_type_value,
      dt.extent_size_bytes = evaluate(rtspace->qual[d.seq].ext_mgmt,"D",cons_160k,0),
      dt.updt_dt_tm = cnvtdatetime(curdate,curtime), dt.extent_management = rtspace->qual[d.seq].
      ext_mgmt, dt.chunk_size = rtspace->qual[d.seq].chunk_size
     PLAN (d)
      JOIN (dt)
     WITH nocounter
    ;end insert
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmss_manual_process_tspace(null)
   IF (dmss_disp_rpt_ind=1)
    IF (dmss_disp_tspace_rpt(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   SET message = window
   ...
   CALL text(3,1,"Tablespace maintenance must be completed before continuing!")
   ...
   CASE (curaccept)
    ...
    OF "V":
     IF (dm2_disp_file(dmss_rpt_filename,concat("Report File may be found in CCLUSERDIR:",
      ...
      RETURN(0)
     ENDIF
     SET dmss_disp_rpt_ind = 0
    OF "Q":
     SET dm_err->eproc = "Prompt user Tablespace Maintenance Manual."
     SET dm_err->emsg =
     "User Chose to Quit from Tablespace Needs Prompt during Tablespace Assessment."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
   ENDCASE
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmss_process_asm_tspace(null)
   IF (dmss_disp_tspace_rpt(null)=0)
    RETURN(0)
   ENDIF
   SET message = window
   CALL clear(1,1)
   CALL text(1,1,"Tablespace Maintenance for ASM")
   CALL text(3,1,"Tablespace maintenance must be completed before continuing!")
   CALL text(4,1,
    "Enter (V) to View tablespace needs. If satisfied with the amount of space available, enter (C) to Continue."
    )
   CALL text(7,1,"(V)iew Tablespace Needs, (C)ontinue, (Q)uit:")
   CALL accept(7,46,"P;CU","V"
    WHERE curaccept IN ("V", "C", "Q"))
   CALL clear(1,1)
   SET message = nowindow
   CASE (curaccept)
    OF "C":
     SET dmss_tspace_needs_viewed = 1
    OF "V":
     IF (dm2_disp_file(dmss_rpt_filename,concat("Report File may be found in CCLUSERDIR:",
       dmss_rpt_filename))=0)
      RETURN(0)
     ENDIF
    OF "Q":
     SET dm_err->eproc = "Prompt user Tablespace Maintenance for ASM."
     SET dm_err->emsg =
     "User Chose to Quit from Tablespace Needs Prompt during Tablespace Assessment."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmss_pop_current_size(null)
  IF ((rtspace->rtspace_cnt > 0))
   SET dm_err->eproc = "Getting total size for existing tablespaces..."
   SELECT INTO "nl:"
    dddf.tablespace_name, total_bytes = sum(dddf.bytes)
    FROM dba_data_files dddf
    GROUP BY dddf.tablespace_name
    ORDER BY dddf.tablespace_name
    HEAD dddf.tablespace_name
     dmss_search = locateval(dmss_search,1,rtspace->rtspace_cnt,dddf.tablespace_name,rtspace->qual[
      dmss_search].tspace_name)
    DETAIL
     IF (dmss_search > 0)
      rtspace->qual[dmss_search].cur_bytes_allocated = total_bytes
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
  ...
  RETURN(1)
 END ;Subroutine
 SUBROUTINE dmss_calc_lmt_ext_size(total_size)
   IF (total_size > 0
    AND total_size <= cons_1m)
    RETURN(cons_64k)
   ELSEIF (total_size > cons_1m
    AND total_size <= cons_64m)
    RETURN(cons_1m)
   ELSEIF (total_size > cons_64m
    AND total_size <= cons_1g)
    RETURN(cons_8m)
   ELSE
    RETURN(cons_64m)
   ENDIF
 END ;Subroutine
 SUBROUTINE dmss_insert_ddl_op_row(null)
   DECLARE didor_op_id = f8 WITH protect, noconstant(0.0)
   SET dm_err->eproc = "Deleting duplicate rows for this operation."
   DELETE  FROM dm2_ddl_ops_log ddol
    WHERE ddol.run_id=dmss_run_id
     AND ddol.op_type=dmss_op_type
     AND ddol.obj_name=dmss_obj_name
     AND ddol.priority=dmss_priority
     AND ddol.table_name=dmss_table_name
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Determining max op_id from dm2_ddl_ops_log."
   SELECT INTO "nl:"
    max_op_id = max(ddol.op_id)
    FROM dm2_ddl_ops_log ddol
    HEAD REPORT
     didor_op_id = 0
    DETAIL
     didor_op_id = max_op_id
    FOOT REPORT
     didor_op_id = (didor_op_id+ 1)
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Replacing !op_id in dmss_operation with didor_op_id"
   SET dmss_operation = replace(dmss_operation,"!op_id",trim(cnvtstring(didor_op_id)),0)
   SET dm_err->eproc = "Inserting dm2_ddl_ops_log row for operation."
   INSERT  FROM dm2_ddl_ops_log ddol
    SET ddol.op_id = didor_op_id, ddol.owner_name = dmss_owner, ddol.run_id = dmss_run_id,
     ddol.gen_dt_tm = cnvtdatetime(curdate,curtime3), ddol.gen_id = dmss_run_id, ddol.begin_dt_tm =
     cnvtdatetime(null),
     ddol.end_dt_tm = cnvtdatetime(null), ddol.est_duration = null, ddol.act_duration = null,
     ddol.op_type = dmss_op_type, ddol.table_name = dmss_table_name, ddol.tablespace_name =
     dmss_tablespace_name,
     ddol.row_cnt = dmss_row_cnt, ddol.obj_name = dmss_obj_name, ddol.priority = dmss_priority,
     ddol.status = null, ddol.error_msg = null, ddol.operation = dmss_operation,
     ddol.on_error_operation = null, ddol.schema_date = cnvtdatetime(dmss_schema_date), ddol
     .schema_instance = 0,
     ddol.ready_to_run_ind = 0, ddol.appl_id = currdbhandle
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmss_alloc_free_space_usage(dafsu_reserve_free_space)
   SET dm_err->eproc = "Confirming allocated free space usage during migration"
   CALL disp_msg("",dm_err->logfile,0)
   IF (dafsu_reserve_free_space="N")
    SET message = window
    CALL clear(1,1)
    CALL box(1,1,24,131)
    CALL video(b)
    ...
    CALL video(n)
    CALL text(6,2,
     "It has been identified that allocated free space can be used during the migration setup.")
    CALL text(8,2,
     "It is recommended that this setup script be executed first to reserve all allocated free space"
     )
    CALL text(9,2,
     "so that tablespace needs that would barely be resolved by allocated free space can be evaluated appropriately"
     )
    CALL text(11,2,"Press C to continue setup or E to exit (C,E):")
    CALL accept(11,48,"A;CU","E"
    ...
    IF (curaccept="E")
     ...
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Inserting DM_INFO row for allocated free space usage"
    SELECT INTO "nl:"
     FROM dm_info di
     WHERE di.info_domain="DM2_MIG_SETUP_SOURCE"
      AND di.info_name="USE_ALLOCATED_FREE_SPACE"
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (curqual)
     IF ((dm_err->debug_flag > 1))
      CALL echo("Updating USE_ALLOCATED_FREE_SPACE dm_info row")
     ENDIF
     UPDATE  FROM dm_info di
      SET di.info_number = (di.info_number+ 1), di.info_date = cnvtdatetime(curdate,curtime3)
      WHERE di.info_domain="DM2_MIG_SETUP_SOURCE"
       AND di.info_name="USE_ALLOCATED_FREE_SPACE"
      WITH nocounter
     ;end update
    ELSE
     IF ((dm_err->debug_flag > 1))
      CALL echo("Inserting USE_ALLOCATED_FREE_SPACE dm_info row")
     ENDIF
     INSERT  FROM dm_info di
      SET di.info_domain = "DM2_MIG_SETUP_SOURCE", di.info_name = "USE_ALLOCATED_FREE_SPACE", di
       .info_number = 1,
...
#exit_script
 ...
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_setup_source completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_setup_source")
END GO
