CREATE PROGRAM dm2_auto_success_schema:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dass_tgt_dms = vc WITH protect, noconstant(" ")
 DECLARE dass_logfile_prefix = vc WITH protect, constant("dm2autosuccschema")
 DECLARE dass_envid = f8 WITH protect, noconstant(0.0)
 DECLARE dass_lnksrv = vc WITH protect, noconstant(" ")
 IF (check_logfile(dass_logfile_prefix,".log","DM2_AUTO_SUCCESS_SCHEMA LOGFILE")=0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_auto_success_schema.prg"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (currdb != "SQLSRV")
  SET dm_err->eproc = "Validating RDBM script is running on."
  SET dm_err->emsg = "This script should only run on SQL Server Inhouse Domains"
  SET dm_err->err_ind = 1
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO end_program
 ENDIF
 SET dass_tgt_dms = concat("OUTCOMES - ",cnvtupper(currdbname))
 CALL echo(concat("Target Database - ",dass_tgt_dms))
 ...
 ELSEIF (curqual=0)
  ...
  GO TO end_program
 ENDIF
 SET modify openquery dass_lnksrv
 SET dm_err->eproc = "Performing auto-success updates on Millennium schema for SQL Server"
 CALL disp_msg(" ",dm_err->logfile,0)
 UPDATE  FROM dm_project_status_env dm
  SET dm.dm_status = "SUCCESS", dm.dm_status_msg = "AUTO-SUCCESSED", dm.dm_status_dt_tm =
   cnvtdatetime(curdate,curtime3)
  WHERE dm.environment_id=dass_envid
   AND dm.proj_type="SCHEMA"
   AND dm.dm_status = null
   AND dm.proj_name != "DM_INFO"
   AND  NOT (dm.proj_name IN (
  (SELECT
   dtd.table_name
   FROM dm_tables_doc dtd
   WHERE dtd.data_model_section=dass_tgt_dms)))
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ENDIF
#end_program
 SET dm_err->eproc = "Ending dm2_auto_success_schema.prg"
 CALL final_disp_msg(dass_logfile_prefix)
END GO
