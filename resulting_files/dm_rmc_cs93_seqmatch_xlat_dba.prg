CREATE PROGRAM dm_rmc_cs93_seqmatch_xlat:dba
 ...
 SET readme_data->message = "Readme Failed: Starting script dm_rmc_cs93_seqmatch_xlat.prg..."
 DECLARE drcsx_loop = i4 WITH protect, noconstant(0)
 DECLARE drcsx_cs93_cnt = i4 WITH protect, noconstant(0)
 DECLARE drcsx_xlat_ret = vc WITH protect, noconstant("")
 FREE RECORD drcsx_rec
 RECORD drcsx_rec(
   1 env_id = f8
   1 mock_id = f8
   1 ref_seq_cnt = i4
 ...
 IF (check_logfile("dm_cs93_seqmatch",".log","dm_rmc_cs93_seqmatch_xlat LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET drcsx_xlat_ret = cs93_create_seqmatch(drcsx_rec)
 IF (drcsx_xlat_ret="F")
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error creating seqmatch rows:",dm_err->emsg)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET drcsx_xlat_ret = " "
 FOR (drcsx_loop = 1 TO drcsx_rec->env_cnt)
   IF ((drcsx_rec->env_qual[drcsx_loop].done2_exist_ind=1))
    SET dm_err->eproc = "Backfilling translations for CODE_SET 93, for valid environment pairs"
    SELECT INTO "NL:"
     v.name
     FROM (parser(concat("V$DATABASE",drcsx_rec->env_qual[drcsx_loop].db_link)) v)
     DETAIL
      drcsx_rec->env_qual[drcsx_loop].valid_env_ind = 1
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     IF (((findstring("ORA-02019",dm_err->emsg) > 0) OR (((findstring("ORA-04052",dm_err->emsg) > 0)
      OR (findstring("ORA-01034",dm_err->emsg) > 0)) )) )
      SET dm_err->err_ind = 0
     ELSE
      SET readme_data->status = "F"
      SET readme_data->message = concat("Error gathering DONE2 rows:",dm_err->emsg)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_script
     ENDIF
    ENDIF
    IF ((drcsx_rec->env_qual[drcsx_loop].valid_env_ind=1))
     SET drcsx_xlat_ret = cs93_xlat_backfill(drcsx_rec->env_qual[drcsx_loop].parent_env_id)
     IF (drcsx_xlat_ret="F")
      SET readme_data->status = "F"
      SET readme_data->message = concat("Error completing xlat backfill:",dm_err->emsg)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_script
     ENDIF
    ENDIF
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Validating that all SEQMATCH rows were created successfully"
 SELECT INTO "NL:"
  cnt = count(*)
  FROM dm_info di
  WHERE di.info_name="CODESET93"
   AND di.info_domain=patstring(concat("MERGE*",trim(cnvtstring(drcsx_rec->mock_id,20)),"SEQMATCH"))
  DETAIL
   drcsx_cs93_cnt = cnt
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error validating SEQMATCH rows:",dm_err->emsg)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dm_err->err_ind=0)
  AND (drcsx_cs93_cnt=drcsx_rec->ref_seq_cnt))
  SET readme_data->status = "S"
  SET readme_data->message = "SUCCESS: CODE_SET 93 translations were backfilled correctly"
 ENDIF
#exit_script
