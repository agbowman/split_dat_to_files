CREATE PROGRAM cp_upd_node:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CP_UPD_NODE"
 DECLARE verifyinputs(null) = null WITH protect
 DECLARE evaluaterecs(null) = i4 WITH protect
 DECLARE lstat = i4 WITH protect, noconstant(0)
 DECLARE dqupddttm = dq8 WITH protect, constant(cnvtdatetime(sysdate))
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE nodelistsize = i4 WITH protect, constant(size(request->qual,5))
   DECLARE x = i4 WITH protect, noconstant(0)
   DECLARE codevaluevalid = i2 WITH protect, noconstant(0)
   DECLARE codevaluevalid = i2 WITH noconstant(0), protect
   DECLARE concept_cd_cs = i4 WITH protect, constant(4003132)
   DECLARE note_type_cd_cs = i4 WITH protect, constant(4003131)
   IF (nodelistsize <= 0)
    SET reply->status_data.status = "F"
    ...
    SET reply->status_data.subeventstatus.targetobjectvalue = "Node list must be nonzero in size."
    GO TO exit_script
   ENDIF
   FOR (x = 1 TO nodelistsize)
     IF ((request->qual[x].cp_node_id > 0))
      SET codevaluevalid = checknodeid(request->qual[x].cp_node_id)
      IF (codevaluevalid=0)
       SET reply->status_data.status = "F"
       SET reply->status_data.subeventstatus.operationname = "PERFORM"
       SET reply->status_data.subeventstatus.operationstatus = "F"
     ...
     ELSE
      ...
      SET reply->status_data.subeventstatus.operationstatus = "F"
      SET reply->status_data.subeventstatus.targetobjectname = "Validate"
      SET reply->status_data.subeventstatus.targetobjectvalue = build2("CP_NODE_ID for list seq ",
       trim(cnvtstring(x),3)," must be non-zero.")
      GO TO exit_script
     ENDIF
     IF ((request->qual[x].concept_cd > 0))
      SET codevaluevalid = checkcodevalue(request->qual[x].concept_cd,concept_cd_cs)
      IF (codevaluevalid != 1)
       SET reply->status_data.status = "F"
       SET reply->status_data.subeventstatus.operationname = "PERFORM"
       SET reply->status_data.subeventstatus.operationstatus = "F"
       SET reply->status_data.subeventstatus.targetobjectname = "Validate"
       SET reply->status_data.subeventstatus.targetobjectvalue = build2("Concept code value ",trim(
         cnvtstring(request->qual[x].concept_cd),3)," does not exist or is inactive.")
       GO TO exit_script
      ENDIF
     ENDIF
     IF (size(trim(request->qual[x].node_name,3),1)=0)
      SET reply->status_data.status = "F"
      SET reply->status_data.subeventstatus.operationname = "PERFORM"
      SET reply->status_data.subeventstatus.operationstatus = "F"
      SET reply->status_data.subeventstatus.targetobjectname = "Validate"
      SET reply->status_data.subeventstatus.targetobjectvalue = build2("Node name for list seq ",trim
       (cnvtstring(x),3)," must be valid string with length > 0.")
      GO TO exit_script
     ENDIF
     IF ((request->qual[x].node_type_cd > 0))
      SET codevaluevalid = checkcodevalue(request->qual[x].node_type_cd,note_type_cd_cs)
      IF (codevaluevalid != 1)
       SET reply->status_data.status = "F"
       SET reply->status_data.subeventstatus.operationname = "PERFORM"
       SET reply->status_data.subeventstatus.operationstatus = "F"
       SET reply->status_data.subeventstatus.targetobjectname = "Validate"
       SET reply->status_data.subeventstatus.targetobjectvalue = build2("Node Type CD ",trim(
         cnvtstring(request->qual[x].node_type_cd),3)," does not exist or is inactive.")
       GO TO exit_script
      ENDIF
     ELSE
      SET reply->status_data.status = "F"
      ...
      SET reply->status_data.subeventstatus.operationstatus = "F"
      SET reply->status_data.subeventstatus.targetobjectname = "Validate"
      SET reply->status_data.subeventstatus.targetobjectvalue = build2("Node Type CD for list seq ",
       trim(cnvtstring(x),3)," must be non-zero.")
      GO TO exit_script
     ENDIF
   ENDFOR
 ...
 SUBROUTINE (checknodeid(nid=f8) =i4 WITH protect)
   ...
   RETURN(curqual)
 END ;Subroutine
 SUBROUTINE evaluaterecs(null)
   CALL log_message("In evaluateRecs()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE xsize = i4 WITH protect, constant(size(request->qual,5))
   DECLARE x = i4 WITH protect, noconstant(0)
   DECLARE cnid = f8 WITH protect, noconstant(0)
   SET lstat = alterlist(reply->qual,xsize)
   IF (checkdic("CP_NODE","T",0) != 2)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus.operationname = "PERFORM"
    SET reply->status_data.subeventstatus.operationstatus = "F"
    SET reply->status_data.subeventstatus.targetobjectname = "INSERT"
    SET reply->status_data.subeventstatus.targetobjectvalue = build2("CP_NODE Table does not exist ",
     "or Permission is not granted to access.")
    GO TO exit_script
   ENDIF
   FOR (x = 1 TO xsize)
     IF ((request->qual[x].new_version_ind=1))
      SET lstat = versionrow(x)
      SET lstat = insertrow(x)
     ELSE
      SET lstat = updaterow(x)
     ENDIF
   ENDFOR
   CALL log_message(build("Exit evaluateRecs(), Elapsed time in seconds:",((curtime3 - begin_curtime3
     )/ 100.0)),log_level_debug)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (versionrow(idx=i4) =i4 WITH protect)
   CALL log_message("In versionRow()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH private, constant(curtime3)
   UPDATE  FROM cp_node cn
    SET cn.active_ind = 0, cn.end_effective_dt_tm = cnvtdatetime(dqupddttm)
    WHERE (cn.cp_node_id=request->qual[idx].cp_node_id)
    WITH nocounter
   ;end update
   SET lstat = error_and_zero_check_rec(1,"UPDATE","ERROR INACTIVATING ON CP_NODE",1,1,
    reply)
   CALL log_message(build("Exit versionRow(), Elapsed time in seconds:",((curtime3 - begin_curtime3)
     / 100.0)),log_level_debug)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (updaterow(idx=i4) =i4 WITH protect)
   CALL log_message("In updateRow()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH private, constant(curtime3)
   UPDATE  FROM cp_node cn
    SET cn.active_ind = 1, cn.concept_cd = request->qual[idx].concept_cd, cn.node_display = request->
     qual[idx].node_display,
     cn.node_type_cd = request->qual[idx].node_type_cd, cn.publish_ind = request->qual[idx].
     publish_ind, cn.version_text = request->qual[idx].version_text,
     cn.updt_id = reqinfo->updt_id, cn.updt_applctx = reqinfo->updt_applctx, cn.updt_task = reqinfo->
     updt_task,
     cn.updt_dt_tm = cnvtdatetime(sysdate), cn.updt_cnt = (cn.updt_cnt+ 1)
    WHERE (cn.cp_node_id=request->qual[idx].cp_node_id)
    WITH nocounter
   ;end update
   SET lstat = error_and_zero_check_rec(1,"UPDATE","ERROR UPDATING INTO CP_NODE",1,1,
    reply)
   CALL log_message(build("Exit updateRow(), Elapsed time in seconds:",((curtime3 - begin_curtime3)/
     100.0)),log_level_debug)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (insertrow(idx=i4) =i4 WITH protect)
   CALL log_message("In insertRow()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE cnid = f8 WITH protect, noconstant(0.0)
   IF (checkdic("CP_NODE","T",0) != 2)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus.operationname = "PERFORM"
    SET reply->status_data.subeventstatus.operationstatus = "F"
   ...
   IF (cnid=0.0)
    ...
    SET reply->status_data.subeventstatus.targetobjectvalue = build2(
     "Unable to generate cp_node_id from REFERENCE_SEQ.")
    ROLLBACK
    GO TO exit_script
   ENDIF
   INSERT  FROM cp_node cn
    SET cn.cp_node_id = cnid, cn.active_ind = 1, cn.create_dt_tm = cnvtdatetime(dqupddttm),
     cn.beg_effective_dt_tm = cnvtdatetime(dqupddttm), cn.end_effective_dt_tm = cnvtdatetime(
      "31-DEC-2100 23:59"), cn.concept_cd = request->qual[idx].concept_cd,
     cn.node_display = request->qual[idx].node_display, cn.node_name = request->qual[idx].node_name,
     cn.node_type_cd = request->qual[idx].node_type_cd,
     cn.publish_ind = request->qual[idx].publish_ind, cn.version_node_id =
     IF ((request->qual[idx].version_node_id > 0.0)) request->qual[idx].version_node_id
     ELSE cnid
     ENDIF
     , cn.version_text = request->qual[idx].version_text,
     cn.updt_id = reqinfo->updt_id, cn.updt_applctx = reqinfo->updt_applctx, cn.updt_task = reqinfo->
     updt_task,
     cn.updt_dt_tm = cnvtdatetime(sysdate), cn.updt_cnt = 0
    WITH nocounter
   ;end insert
   SET lstat = error_and_zero_check_rec(1,"INSERT","ERROR INSERTING INTO CP_NODE",1,1,
    reply)
   SET reply->qual[idx].cp_node_id = cnid
