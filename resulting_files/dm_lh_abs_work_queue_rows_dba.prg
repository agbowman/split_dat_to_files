CREATE PROGRAM dm_lh_abs_work_queue_rows:dba
 ...
 SET reply->table_name = "LH_ABS_WORK_QUEUE"
 SET reply->rows_between_commit = 100
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 DECLARE v_min_day = dq8 WITH protect, noconstant(cnvtdatetime("01-JAN-1800 00:00:00"))
 DECLARE v_max_day = dq8 WITH protect, noconstant(0)
 DECLARE v_cur_day = dq8 WITH protect, noconstant(datetimefind(cnvtdatetime(curdate,0),"D","B","B"))
 DECLARE v_years_to_keep = i4 WITH protect, noconstant(10)
 DECLARE v_token_idx = i4 WITH protect, noconstant(0)
 DECLARE v_days_str = vc WITH protect, noconstant("")
 ...
 FOR (v_token_idx = 1 TO size(request->tokens,5))
   IF ((request->tokens[v_token_idx].token_str="YEARSTOKEEP"))
    IF (cnvtreal(request->tokens[v_token_idx].value) > 0)
     SET v_years_to_keep = ceil(cnvtreal(request->tokens[v_token_idx].value))
    ENDIF
   ENDIF
 ENDFOR
 IF (v_years_to_keep < 7)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must keep at least 7 years' worth of data. You entered %1 year(s).","i",v_years_to_keep)
  GO TO exit_script
 ENDIF
 SET v_max_day = cnvtlookbehind(build(v_years_to_keep,",Y"),v_cur_day)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(lm.lh_abs_work_queue_id)
   FROM lh_abs_work_queue lm
   WHERE lm.lh_abs_work_queue_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(lm.lh_abs_work_queue_id)
  FROM lh_abs_work_queue lm
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    ...
    FROM lh_abs_work_queue lm,
     encounter e
    WHERE lm.lh_abs_work_queue_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND lm.encntr_id=e.encntr_id
     AND parser(sbr_getrowidnotexists(
      "e.disch_dt_tm between cnvtdatetime(v_min_day) and cnvtdatetime(v_max_day)","lm"))
     AND lm.lh_abs_work_queue_id != 0.0
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = lm.rowid
    WITH nocounter, maxqual(lm,value(v_rows_left))
   ;end select
