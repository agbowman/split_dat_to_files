CREATE PROGRAM dm_stat_alt_sel_cat:dba
 ...
 ENDIF
 DECLARE ms_snapshot_type = vc WITH protect, constant("ALT_SEL_CAT")
 DECLARE error_msg = vc WITH noconstant("")
 DECLARE ds_cnt = i4
 DECLARE ms_snapshot_time = dq8 WITH constant(cnvtdatetime((curdate - 1),0))
 DECLARE ms_info_domain = vc WITH constant("DM_STAT_ALT_SEL_CAT")
 ...
 ENDIF
 CALL sbr_debug_timer("START_TOTAL","ALT_SEL_CAT")
 CALL sbr_debug_timer("START","INSERTING ALT_SEL_CAT DATA")
 SELECT INTO "nl:"
  a.alt_sel_category_id, description = trim(a.long_description)
  FROM alt_sel_cat a
  WHERE a.child_cat_ind=1
   AND a.owner_id=0
  HEAD REPORT
  ...
  DETAIL
   ...
   ENDIF
   dsr->qual[1].qual[ds_cnt].stat_name = "ALT_SEL_CAT", dsr->qual[1].qual[ds_cnt].stat_type = 0, dsr
   ->qual[1].qual[ds_cnt].stat_seq = ds_cnt,
   dsr->qual[1].qual[ds_cnt].stat_number_val = a.alt_sel_category_id, dsr->qual[1].qual[ds_cnt].
   stat_clob_val = description
  FOOT REPORT
   IF (ds_cnt=0)
    stat = alterlist(dsr->qual[1].qual,1), dsr->qual[1].qual[1].stat_name = "NO_DATA"
   ELSE
 ...
 IF (error(error_msg,0) != 0)
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_debug_timer("END","INSERTING ALT_SEL_CAT DATA")
 ...
 END ;Subroutine
#exit_program
 CALL sbr_debug_timer("END_TOTAL","ALT_SEL_CAT")
END GO
