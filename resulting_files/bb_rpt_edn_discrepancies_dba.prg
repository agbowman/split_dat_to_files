CREATE PROGRAM bb_rpt_edn_discrepancies:dba
 ...
 SET captions->report_header = uar_i18ngetmessage(i18nhandle,"report_name",
  "Electronic Blood Order Discrepancy Report")
 ...
 SET captions->end_of_report = uar_i18ngetmessage(i18nhandle,"end_of_report",
  "* * * End of Report * * *")
 SET captions->page_nbr = uar_i18ngetmessage(i18nhandle,"page_nbr","Page:")
 DECLARE sorg_name = c112 WITH protect, noconstant(" ")
 DECLARE sproduct_type_disp = c25 WITH protect, noconstant(" ")
 DECLARE sovrd_reason_disp = c30 WITH protect, noconstant(" ")
 DECLARE stech_id = c11 WITH protect, noconstant(" ")
 DECLARE sfill_line = c130 WITH protect, constant(fillstring(130,"-"))
 DECLARE irpt_cnt = i2 WITH protect, noconstant(0)
 DECLARE exception_type_cs = i4 WITH protect, constant(14072)
 DECLARE edn_discrep_mean = c11 WITH protect, constant("EDN_DISCREP")
 DECLARE edn_discrep_cd = f8 WITH protect, noconstant(0.0)
 DECLARE begday = dq8 WITH protect, noconstant(0.0)
 DECLARE endday = dq8 WITH protect, noconstant(0.0)
 DECLARE ierrorcd = i4 WITH protect, noconstant(0)
 DECLARE serrormsg = c132 WITH protect, noconstant(" ")
 DECLARE istat = i4 WITH protect, noconstant(0)
 DECLARE iisqueue = i4 WITH protect, noconstant(0)
 ...
 ELSE
  ...
  SELECT
   IF ((request->bb_edn_id=0.0)
    ...
    PLAN (bb
     WHERE edn_discrep_cd=bb.exception_type_cd
      AND bb.active_status_dt_tm BETWEEN cnvtdatetime(request->beg_dt_tm) AND cnvtdatetime(request->
      end_dt_tm)
      AND ((bb.active_ind+ 0)=1))
     JOIN (bed
     WHERE bed.exception_id=bb.exception_id)
     JOIN (bea
     WHERE bea.bb_edn_admin_id=bed.bb_edn_admin_id
      AND ((bea.destination_loc_cd+ 0)=request->cur_owner_area_cd))
     JOIN (pr
     WHERE pr.person_id=bed.updt_id)
     JOIN (p
     WHERE (p.product_id= Outerjoin(bed.product_id)) )
   ...
   ELSE
    PLAN (bea
     WHERE (bea.bb_edn_admin_id=request->bb_edn_id)
      AND ((bea.destination_loc_cd+ 0)=request->cur_owner_area_cd))
     JOIN (bed
     WHERE bed.bb_edn_admin_id=bea.bb_edn_admin_id)
     JOIN (bb
     WHERE bed.exception_id=bb.exception_id
      AND bb.active_ind=1)
     JOIN (pr
   ...
   bea.admin_dt_tm, bea.order_nbr_ident, bed.edn_product_nbr_ident,
   ...
   HEAD PAGE
    ...
    IF (sub_get_location_name != "<<INFORMATION NOT FOUND>>")
     ...
     ENDIF
    ENDIF
    row 0, col 45, captions->report_header,
    col 113, captions->time, col 124,
    curtime"HH:MM;;M", row + 1, col 113,
    captions->as_of_date, col 124, curdate"DDMMMYY;;D",
    row + 1
    IF ((request->bb_edn_id=0.0))
     col 35, captions->beg_date, col 51,
     beg_dt_tm"DDMMMYY HH:MM;;D", col 71, captions->end_date,
     col 84, end_dt_tm"DDMMMYY HH:MM;;D"
    ENDIF
    row + 2, col 1, captions->bb_owner,
    col 19, sorg_name, row + 2,
    col 1, captions->order_dt_tm, row + 1,
    col 4, captions->order_number, col 20,
    captions->receive_date, col 39, captions->product_nbr,
    col 57, captions->product_type, col 85,
    captions->reason, col 118, captions->tech,
    row + 1, col 1, sfill_line
   HEAD bea.admin_dt_tm
    IF (row > 56)
     BREAK
    ENDIF
    row + 1, col 1, bea.admin_dt_tm"MM/DD/YY  HH:MM;;Q",
    row + 1
   ...
   DETAIL
    IF (row > 59)
     BREAK
    ENDIF
    row + 1, col 4, bea.order_nbr_ident
    IF (bed.product_id > 0.0)
     col 20, p.recv_dt_tm
    ENDIF
    col 39, bed.edn_product_nbr_ident, col 57,
    sproduct_type_disp, col 85, sovrd_reason_disp,
    col 118, pr.username"############", row + 1
   FOOT  bea.order_nbr_ident
    row + 0
   FOOT  bea.admin_dt_tm
    row + 0
   FOOT PAGE
    row 61, col 1, sfill_line,
    row + 1, col 1, cpm_cfn_info->file_name,
    col 122, captions->page_nbr, col 128,
    curpage"###"
   FOOT REPORT
    row 63, col 53, captions->end_of_report
  ;end select
 ENDIF
