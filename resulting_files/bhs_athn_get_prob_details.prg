CREATE PROGRAM bhs_athn_get_prob_details
 SELECT INTO  $1
  ...
  HEAD p.problem_id
   ...
   row + 1, probability = build("<Probability>",p.probability,"</Probability>"), col + 1,
   probability, row + 1, col 1,
   "<OnsetDateCd>", row + 1, onset_cd_v = build("<Value>",p_onset_dt_cd,"</Value>"),
   col + 1, onset_cd_v, row + 1,
   onset_cd_d = build("<Display>",p_onset_dt_disp,"</Display>"), col + 1, onset_cd_d,
   row + 1, onset_cd_m = build("<Meaning>",p_onset_dt_mean,"</Meaning>"), col + 1,
   onset_cd_m, row + 1, col 1,
   "</OnsetDateCd>", row + 1, lyfcdt_dt = build("<LifeCycleDateTime>",p_life_cycle_dt_tm,
    "</LifeCycleDateTime>"),
   col + 1, lyfcdt_dt, row + 1,
   lyfcdt_dt_flg = build("<LifeCycleDateTimeFlag>",p_life_cycle_dt_flag,"</LifeCycleDateTimeFlag>"),
   col + 1, lyfcdt_dt_flg,
   row + 1, col 1, "<LifeCycleDateCd>",
   row + 1, lyfcdt_cd_v = build("<Value>",p_life_cycle_dt_cd,"</Value>"), col + 1,
   lyfcdt_cd_v, row + 1, lyfcdt_cd_d = build("<Display>",p_life_cycle_dt_disp,"</Display>"),
   col + 1, lyfcdt_cd_d, row + 1,
   lyfcdt_cd_m = build("<Meaning>",p_life_cycle_dt_mean,"</Meaning>"), col + 1, lyfcdt_cd_m,
   row + 1, col 1, "</LifeCycleDateCd>",
   row + 1, col 1, "<QualifierCd>",
   row + 1, qual_cd_v = build("<Value>",p_qualifier_cd,"</Value>"), col + 1,
   qual_cd_v, row + 1, qual_cd_d = build("<Display>",p_qualifier_disp,"</Display>"),
   col + 1, qual_cd_d, row + 1,
   qual_cd_m = build("<Meaning>",p_qualifier_mean,"</Meaning>"), col + 1, qual_cd_m,
   row + 1, col 1, "</QualifierCd>",
   row + 1, col 1, "<SeverityCd>",
   row + 1, sevr_cd_v = build("<Value>",p_severity_cd,"</Value>"), col + 1,
   sevr_cd_v, row + 1, sevr_cd_d = build("<Display>",p_severity_disp,"</Display>"),
   col + 1, sevr_cd_d, row + 1,
   sevr_cd_m = build("<Meaning>",p_severity_mean,"</Meaning>"), col + 1, sevr_cd_m,
   row + 1, col 1, "</SeverityCd>",
   row + 1, sevr_ft = build("<SeverityFreeText>",p_severity_ftdesc,"</SeverityFreeText>"), col + 1,
   sevr_ft, row + 1, col 1,
   "<SeverityClassCd>", row + 1, sevr_cls_cd_v = build("<Value>",p_severity_class_cd,"</Value>"),
   col + 1, sevr_cls_cd_v, row + 1,
   sevr_cls_cd_d = build("<Display>",p_severity_class_disp,"</Display>"), col + 1, sevr_cls_cd_d,
   row + 1, sevr_cls_cd_m = build("<Meaning>",p_severity_class_mean,"</Meaning>"), col + 1,
   sevr_cls_cd_m, row + 1, col 1,
   "</SeverityClassCd>", row + 1
  FOOT REPORT
