CREATE PROGRAM dm2_add_default_rows:dba
 ...
 DECLARE dadr_tname = vc WITH protect, noconstant(" ")
 DECLARE dadr_idx = i4 WITH protect, noconstant(0)
 DECLARE ix1 = i4 WITH protect, noconstant(0)
 DECLARE dadr_logfile_name = vc WITH protect, noconstant("dm2_adddrrows")
 DECLARE dadr_error_cnt = i2 WITH protect, noconstant(0)
 DECLARE dadr_warning_cnt = i2 WITH protect, noconstant(0)
 DECLARE dadr_tcnt = i4 WITH protect, noconstant(0)
 DECLARE dadr_ccnt = i4 WITH protect, noconstant(0)
 DECLARE dadr_wildcard_found = i2 WITH protect, noconstant(0)
 DECLARE dadr_ignore_admin_ind = i2 WITH protect, noconstant(0)
 DECLARE dadr_sd_in_use_ind = i2 WITH protect, noconstant(0)
 DECLARE dadr_trg_drop_ind = i2 WITH protect, noconstant(0)
 DECLARE dadr_pkg_process = i4 WITH protect, noconstant(0)
 DECLARE dadr_pkg_num = i4 WITH protect, noconstant(0)
 DECLARE dadr_tab_cnt = i4 WITH protect, noconstant(0)
 DECLARE dadr_retry_ceiling = i4 WITH public
 DECLARE regexp_replace() = c300
 FREE RECORD dadr_tbl
 RECORD dadr_tbl(
   1 tcnt = i4
   1 tbl[*]
     2 tname = vc
     ...
     2 trig_version = i4
     2 col_cnt = i4
     2 trig_del_reg_exists = i2
     2 trig_del_reg_name = vc
     2 trig_del_c_exists = i2
     2 trig_del_c_name = vc
     2 trig_upd_reg_exists = i2
     2 trig_upd_reg_name = vc
     2 trig_upd_c_exists = i2
     2 trig_upd_c_name = vc
     2 col[*]
       3 col_name = vc
       3 data_type = vc
       3 pk_ind = i2
       3 data_default_null_ind = i2
       3 data_default = vc
       3 nullable = c1
 )
 RECORD dadr_err_tbl(
 ...
 IF (check_logfile(dadr_logfile_name,".log","DM2_ADD_DEFAULT_ROWS.PRG ...") != 1)
  GO TO exit_program
 ENDIF
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ...
 IF ((dm2_rdbms_version->level1 > 10))
  IF (dir_get_ddl_gen_retry(dadr_retry_ceiling)=0)
   GO TO exit_program
  ENDIF
 ELSE
  SET dadr_retry_ceiling = 0
 ENDIF
 SET dm_err->eproc = "BEGINNING DM2_ADD_DEFAULT_ROWS.PRG ..."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dsr_sd_in_use_check("DM2NOTSET",dadr_sd_in_use_ind)=0)
  GO TO exit_program
 ENDIF
 IF (dadr_sd_in_use_ind=1)
  IF (dsr_usage_prep("DM2NOTSET")=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (validate(dm2_etu->mode," ")="PACKAGE")
  SET dadr_pkg_process = 1
  SET dadr_pkg_num = dm2_etu->pkg_num
  SET dm_err->eproc = "Gathering list of tables on Package."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dm2_set_autocommit(1)=0)
   GO TO exit_program
  ENDIF
  SELECT INTO "NL:"
   FROM dm_tables_doc dtd,
    dm_afd_tables dat
   WHERE dat.alpha_feature_nbr=dadr_pkg_num
    AND dat.owner=currdbuser
    AND dat.table_name=dtd.table_name
    AND dtd.table_name=dtd.full_table_name
    AND dtd.default_row_ind=1
   DETAIL
    dadr_tab_cnt = (dadr_tab_cnt+ 1)
    IF (mod(dadr_tab_cnt,100)=1)
     stat = alterlist(dadr_afd_tab->afd,(dadr_tab_cnt+ 99))
    ENDIF
    dadr_afd_tab->afd[dadr_tab_cnt].afd_tname = dtd.table_name, dadr_afd_tab->afd[dadr_tab_cnt].
    afd_suffix = dtd.table_suffix
   FOOT REPORT
    stat = alterlist(dadr_afd_tab->afd,dadr_tab_cnt)
   WITH nocounter
  ;end select
  IF (check_error(concat("DM2_ADD_DEFAULT_ROWS.PRG: Error on select to identify tables on package "))
   != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (dm2_set_autocommit(0)=0)
   GO TO exit_program
  ENDIF
  IF (size(dadr_afd_tab->afd,5)=0)
   SET dm_err->eproc = concat(" ** No tables were found to need a default row for package (",
    cnvtstring(dadr_pkg_num),") **")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
 ELSE
  SET dadr_pkg_process = 0
 ENDIF
 SET dadr_tname = cnvtupper(trim( $1))
 IF (textlen(trim(dadr_tname))=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "A input parameter was not supplied.  Usage: DM2_ADD_DEFAULT_ROW '<TABLE NAME/NO_PK_TABLES>' GO"
  SET dm_err->eproc = "DM2_ADD_DEFAULT_ROWS"
  SET dm_err->user_action =
  "Please enter a table name and/or a wildcard or NO_PK_TABLES as an input parameter."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (findstring(char(42),dadr_tname) > 0)
  SET dadr_wildcard_found = 1
 ENDIF
 IF (dadr_tname="NO_PK_TABLES")
  SET dm_err->eproc = "Retrieving Non-PK tables list from dma_sql* tables"
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   ...
   PLAN (dsoi
    ...
    JOIN (dtd
    WHERE dsoi.object_name=dtd.table_name
     AND dtd.table_name=dtd.full_table_name)
    JOIN (utc
    WHERE dsoi.object_name=utc.table_name
     AND dsoia.attr_value_char=utc.column_name)
   ORDER BY utc.table_name, utc.column_name
   HEAD REPORT
    dadr_tcnt = 0
   HEAD utc.table_name
    dadr_tcnt = (dadr_tcnt+ 1), dadr_ccnt = 0, stat = alterlist(dadr_tbl->tbl,dadr_tcnt),
    dadr_tbl->tbl[dadr_tcnt].tname = utc.table_name
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("TABLE NAME:",utc.table_name))
    ENDIF
   DETAIL
    dadr_ccnt = (dadr_ccnt+ 1), stat = alterlist(dadr_tbl->tbl[dadr_tcnt].col,dadr_ccnt)
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("COLUMN NAME:",trim(utc.column_name)," adding..."))
    ENDIF
    dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].col_name = utc.column_name, dadr_tbl->tbl[dadr_tcnt].col[
    dadr_ccnt].data_type = utc.data_type, dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].nullable = "N",
    dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].data_default_null_ind = 1
    IF (nullind(utc.data_default)=0)
     dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].data_default_null_ind = 0, dadr_tbl->tbl[dadr_tcnt].col[
     dadr_ccnt].data_default = utc.data_default
    ENDIF
   FOOT  utc.table_name
    stat = alterlist(dadr_tbl->tbl[dadr_tcnt].col,dadr_ccnt), dadr_tbl->tbl[dadr_tcnt].col_cnt =
    dadr_ccnt
   FOOT REPORT
    stat = alterlist(dadr_tbl->tbl,dadr_tcnt), dadr_tbl->tcnt = dadr_tcnt
   WITH nocounter
  ;end select
  IF (check_error(concat(
    "DM2_ADD_DEFAULT_ROWS.PRG: Error on Select from user_tab_columns, dm_tables_doc and dma* tables "
    )) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF ((dm_err->debug_flag > 1))
   CALL echorecord(dadr_tbl)
  ENDIF
  FOR (ix1 = 1 TO dadr_tbl->tcnt)
    IF (checkdic(cnvtupper(dadr_tbl->tbl[ix1].tname),"T",0)=2)
     EXECUTE dm2_add_default_rows_c
    ENDIF
  ENDFOR
  GO TO exit_program
 ENDIF
 IF (validate(dazr_ignore_admin_ind,0)=1)
  SET dadr_ignore_admin_ind = 1
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(build("dadr_ignore_admin_ind= ",dadr_ignore_admin_ind))
 ENDIF
 IF (dadr_pkg_process=0)
  SET dm_err->eproc = concat(" ** Getting primary key information for candidate tables of : (",
   dadr_tname,") ....")
  CALL disp_msg(" ",dm_err->logfile,0)
 ELSE
  SET dm_err->eproc = concat(
   " ** Getting primary key information for candidate tables of package : (",dadr_tname,") ....")
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (dm2_set_autocommit(1)=0)
  GO TO exit_program
 ENDIF
 SELECT
  IF (dadr_pkg_process=1)INTO "NL:"
   pkg_var = dadr_afd_tab->afd[d.seq].afd_suffix
   FROM user_tab_columns dutc,
    user_constraints duc,
    user_cons_columns ducc,
    (dummyt d  WITH seq = size(dadr_afd_tab->afd,5))
   PLAN (d)
    JOIN (dutc
    WHERE (dadr_afd_tab->afd[d.seq].afd_tname=dutc.table_name))
    JOIN (duc
    WHERE dutc.table_name=duc.table_name
     AND duc.constraint_type="P")
    JOIN (ducc
    WHERE duc.table_name=ducc.table_name
     AND ducc.constraint_name=duc.constraint_name
     AND duc.owner=ducc.owner
     AND ducc.column_name=dutc.column_name)
  ELSEIF (dadr_ignore_admin_ind=0)INTO "NL:"
   pkg_var = dtd.table_suffix
   FROM dm_tables_doc dtd,
    user_tab_columns dutc,
    user_constraints duc,
    user_cons_columns ducc
   WHERE dtd.table_name=dutc.table_name
    AND dtd.table_name=dtd.full_table_name
    AND dtd.table_name=patstring(dadr_tname)
    AND dtd.default_row_ind=1
    AND dtd.table_name=dutc.table_name
    AND dtd.table_name=duc.table_name
    AND dtd.table_name=ducc.table_name
    AND duc.constraint_type="P"
    AND duc.constraint_name=ducc.constraint_name
    AND duc.owner=ducc.owner
    AND duc.table_name=dutc.table_name
    AND ducc.table_name=dutc.table_name
    AND ducc.column_name=dutc.column_name
  ELSEIF (dadr_ignore_admin_ind=1)INTO "NL:"
   pkg_var = ""
   FROM user_tab_columns dutc,
    user_constraints duc,
    user_cons_columns ducc
   WHERE dutc.table_name=patstring(dadr_tname)
    AND dutc.table_name=duc.table_name
    AND dutc.table_name=ducc.table_name
    AND dutc.column_name=ducc.column_name
    AND duc.constraint_type="P"
    AND duc.constraint_name=ducc.constraint_name
  ELSE INTO "NL:"
   pkg_var = validate(dtd.table_suffix," ")
   FROM dm2_user_tab_columns dutc,
    dm2_user_constraints duc,
    dm2_user_cons_columns ducc
   WHERE dutc.table_name=patstring(dadr_tname)
    AND dutc.table_name=duc.table_name
    AND dutc.table_name=ducc.table_name
    AND dutc.column_name=ducc.column_name
    ...
    AND duc.constraint_name=ducc.constraint_name
  ENDIF
  ORDER BY dutc.table_name, ducc.position
  HEAD REPORT
   dadr_tcnt = 0
  HEAD dutc.table_name
   dadr_tcnt = (dadr_tcnt+ 1), dadr_ccnt = 0, stat = alterlist(dadr_tbl->tbl,dadr_tcnt),
   dadr_tbl->tbl[dadr_tcnt].tname = dutc.table_name, dadr_tbl->tbl[dadr_tcnt].short_tname = pkg_var,
   dadr_tbl->tbl[dadr_tcnt].trig_del_reg_name = concat("TRG_",dadr_tbl->tbl[dadr_tcnt].short_tname,
    "_DR_UPDT_DEL"),
   dadr_tbl->tbl[dadr_tcnt].trig_version = - (1)
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("TABLE NAME:",dutc.table_name))
   ENDIF
  DETAIL
   dadr_ccnt = (dadr_ccnt+ 1), stat = alterlist(dadr_tbl->tbl[dadr_tcnt].col,dadr_ccnt)
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("COLUMN NAME:",trim(dutc.column_name)," adding..."))
   ENDIF
   dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].col_name = dutc.column_name, dadr_tbl->tbl[dadr_tcnt].col[
   dadr_ccnt].data_type = dutc.data_type
   IF (currdb="ORACLE")
    dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].nullable = "N"
   ELSE
    dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].nullable = dutc.nullable
   ENDIF
   IF (nullind(ducc.position)=0)
    dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].pk_ind = 1
   ELSE
    dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].pk_ind = 0
   ENDIF
   dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].data_default_null_ind = 1
   IF (nullind(dutc.data_default)=0)
    dadr_tbl->tbl[dadr_tcnt].col[dadr_ccnt].data_default_null_ind = 0, dadr_tbl->tbl[dadr_tcnt].col[
    dadr_ccnt].data_default = dutc.data_default
   ENDIF
 ...
 ;end select
 IF (check_error(concat(
   "DM2_ADD_DEFAULT_ROWS.PRG: Error on Main select to identify default row tables candidates ")) != 0
 )
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 IF (dadr_pkg_process=0)
  SET dm_err->eproc = concat(
   " ** Getting columns with null default values for candidate tables of : (",dadr_tname,") ....")
  CALL disp_msg(" ",dm_err->logfile,0)
 ELSE
  SET dm_err->eproc = concat(
   " ** Getting columns with null default values for candidate tables of package : (",dadr_tname,
   ") ....")
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT
  IF (dadr_pkg_process=1)
   FROM user_tab_columns utc,
    dm2_user_notnull_cols dunc
   PLAN (utc
    WHERE utc.data_default = null
    ...
    (SELECT
     ...
     WHERE dat.alpha_feature_nbr=dadr_pkg_num
      ...
      AND dtd.table_name=dtd.full_table_name)))
    JOIN (dunc
    WHERE utc.table_name=dunc.table_name
     AND utc.column_name=dunc.column_name)
  ELSEIF (dadr_ignore_admin_ind=0)
   FROM user_tab_columns utc,
    dm2_user_notnull_cols dunc
   PLAN (utc
    WHERE utc.data_default = null
    ...
    (SELECT
     dtd.table_name
     FROM dm_tables_doc dtd
     WHERE dtd.table_name=patstring(dadr_tname)
      AND dtd.default_row_ind=1
      AND dtd.table_name=dtd.full_table_name)))
    JOIN (dunc
    WHERE utc.table_name=dunc.table_name
     AND utc.column_name=dunc.column_name)
  ELSEIF (dadr_ignore_admin_ind=1)
   FROM user_tab_columns utc,
    dm2_user_notnull_cols dunc
   PLAN (utc
    WHERE utc.table_name=patstring(dadr_tname)
  ...
  HEAD utc.table_name
   dadr_idx = 0
   FOR (ix1 = 1 TO dadr_tbl->tcnt)
     IF ((dadr_tbl->tbl[ix1].tname=utc.table_name))
      dadr_idx = ix1, ix1 = dadr_tbl->tcnt
     ENDIF
   ENDFOR
  DETAIL
   IF (dadr_idx > 0)
    col_found = 0
    FOR (ix2 = 1 TO dadr_tbl->tbl[dadr_idx].col_cnt)
      IF ((dadr_tbl->tbl[dadr_idx].col[ix2].col_name=utc.column_name))
       col_found = 1, ix2 = dadr_tbl->tbl[dadr_idx].col_cnt
      ENDIF
    ENDFOR
    IF (col_found=0)
     dadr_tbl->tbl[dadr_idx].col_cnt = (dadr_tbl->tbl[dadr_idx].col_cnt+ 1), stat = alterlist(
      dadr_tbl->tbl[dadr_idx].col,dadr_tbl->tbl[dadr_idx].col_cnt), dadr_tbl->tbl[dadr_idx].col[
     dadr_tbl->tbl[dadr_idx].col_cnt].col_name = utc.column_name,
     dadr_tbl->tbl[dadr_idx].col[dadr_tbl->tbl[dadr_idx].col_cnt].data_type = utc.data_type, dadr_tbl
     ->tbl[dadr_idx].col[dadr_tbl->tbl[dadr_idx].col_cnt].nullable = "N", dadr_tbl->tbl[dadr_idx].
     col[dadr_tbl->tbl[dadr_idx].col_cnt].data_default_null_ind = 1
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat(" ** Table/Column (",trim(utc.table_name),"/",trim(utc.column_name),
       ") non-pk not null column added to dadr_tbl..."))
     ENDIF
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(concat(
   "DM2_ADD_DEFAULT_ROWS.PRG: Error on Select2 from user_tab_columns and dm_tables_doc ")) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 1))
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(build("dadr_tbl->tcnt=",dadr_tbl->tcnt))
 ENDIF
 IF (dm2_set_autocommit(0)=0)
  GO TO exit_program
 ENDIF
 IF ((dadr_tbl->tcnt > 0))
  IF (currdb != "SQLSRV"
   AND dadr_ignore_admin_ind=0)
   IF (dadr_pkg_process=0)
    SET dm_err->eproc = concat(" ** Matching qualifying default row tables with existing triggers ",
     " in the database for : (",dadr_tname,")... **")
    CALL disp_msg(" ",dm_err->logfile,0)
   ELSE
    SET dm_err->eproc = concat(" ** Matching qualifying default row tables with existing triggers ",
     " in the database for package : (",dadr_tname,")... **")
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT
    IF (dadr_pkg_process=1)
     ...
     WHERE ut.trigger_name="TRG*_DR_UPDT_DEL*"
    ...
    HEAD ut.table_name
     dadr_idx = 0
     FOR (ix1 = 1 TO dadr_tbl->tcnt)
       IF ((dadr_tbl->tbl[ix1].tname=ut.table_name)
        AND findstring("$",ut.table_name)=0)
        dadr_idx = ix1, ix1 = dadr_tbl->tcnt
       ENDIF
     ENDFOR
    DETAIL
     IF (dadr_idx > 0)
      dadr_tbl->tbl[dadr_idx].trig_version = cnvtint(tmp_version)
      CASE (currdb)
       OF "ORACLE":
        IF (substring((textlen(trim(ut.trigger_name)) - 1),2,ut.trigger_name)="$C")
         IF (substring((textlen(trim(ut.trigger_name)) - 12),11,ut.trigger_name)="DR_UPDT_DEL")
          dadr_tbl->tbl[dadr_idx].trig_del_c_exists = 1, dadr_tbl->tbl[dadr_idx].trig_del_c_name = ut
          .trigger_name
         ENDIF
        ELSE
         IF (substring((textlen(trim(ut.trigger_name)) - 10),11,ut.trigger_name)="DR_UPDT_DEL")
          dadr_tbl->tbl[dadr_idx].trig_del_reg_exists = 1, dadr_tbl->tbl[dadr_idx].trig_del_reg_name
           = ut.trigger_name
         ENDIF
        ENDIF
       OF "DB2UDB":
        IF (substring((textlen(trim(ut.trigger_name)) - 1),2,ut.trigger_name)="$C")
         IF (substring((textlen(trim(ut.trigger_name)) - 6),5,ut.trigger_name)="DRDEL")
          dadr_tbl->tbl[dadr_idx].trig_del_c_exists = 1, dadr_tbl->tbl[dadr_idx].trig_del_c_name = ut
          .trigger_name
         ELSEIF (substring((textlen(trim(ut.trigger_name)) - 6),5,ut.trigger_name)="DRUPD")
          dadr_tbl->tbl[dadr_idx].trig_upd_c_exists = 1, dadr_tbl->tbl[dadr_idx].trig_upd_c_name = ut
          .trigger_name
         ENDIF
        ELSE
         IF (substring((textlen(trim(ut.trigger_name)) - 4),5,ut.trigger_name)="DRDEL")
          dadr_tbl->tbl[dadr_idx].trig_del_reg_exists = 1, dadr_tbl->tbl[dadr_idx].trig_del_reg_name
           = ut.trigger_name
         ELSEIF (substring((textlen(trim(ut.trigger_name)) - 4),5,ut.trigger_name)="DRUPD")
          dadr_tbl->tbl[dadr_idx].trig_upd_reg_exists = 1, dadr_tbl->tbl[dadr_idx].trig_upd_reg_name
           = ut.trigger_name
         ENDIF
        ENDIF
      ENDCASE
   ...
   ;end select
   IF (check_error(concat(
     "DM2_ADD_DEFAULT_ROWS.PRG: Error on Select3 from dm2_user_triggers and dm_tables_doc ")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
  ENDIF
  ...
  FOR (ix1 = 1 TO dadr_tbl->tcnt)
    IF (checkdic(cnvtupper(dadr_tbl->tbl[ix1].tname),"T",0)=2)
     SET dadr_trg_drop_ind = 0
     IF (dadr_sd_in_use_ind=1)
      IF (dsr_check_object_state(currdbuser,"TRIGGER",dadr_tbl->tbl[ix1].trig_del_reg_name,"DROPPED",
       dadr_trg_drop_ind)=0)
       GO TO exit_program
      ENDIF
     ENDIF
     IF (dadr_trg_drop_ind=1)
      CALL echo(concat("Schema Deployment in use and trigger marked as dropped (",dadr_tbl->tbl[ix1].
        trig_del_reg_name,")"))
     ELSE
      EXECUTE dm2_add_default_rows_c
     ENDIF
    ELSE
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat(trim(dadr_tbl->tbl[ix1].tname)," ignored due to no ccl definition."))
     ENDIF
    ENDIF
  ENDFOR
 ELSE
  SET dm_err->eproc = concat(" ** No tables were found to need a default row for (",dadr_tname,") **"
   )
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (dm2_set_autocommit(1)=0)
 ...
 SELECT
  IF (dadr_pkg_process=1)
   ...
   (SELECT
    "X"
    FROM user_constraints duc
    WHERE dut.table_name=duc.table_name
     AND duc.constraint_type="P")))
  ELSEIF (dadr_ignore_admin_ind=0)
   FROM dm2_user_tables dut
   WHERE dut.table_name IN (
  ...
  ELSE
   FROM dm2_user_tables dut
   WHERE dut.table_name=patstring(dadr_tname)
    AND  NOT ( EXISTS (
   (SELECT
    "X"
    FROM user_constraints duc
  ...
  HEAD REPORT
   dadr_err_tbl->tcnt = 0
  DETAIL
   dadr_err_tbl->tcnt = (dadr_err_tbl->tcnt+ 1), stat = alterlist(dadr_err_tbl->tbl,dadr_err_tbl->
    tcnt), dadr_err_tbl->tbl[dadr_err_tbl->tcnt].tname = dut.table_name
  WITH nocounter
 ;end select
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dadr_err_tbl)
 ENDIF
 IF (check_error("DM2_ADD_DEFAULT_ROWS.PRG: Error on select4 not exists check.") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dm2_set_autocommit(0)=0)
  GO TO exit_program
 ENDIF
 IF ((dadr_err_tbl->tcnt > 0))
  SET dm_err->eproc = build("Warning:  DM2_ADD_DEFAULT_ROWS found (",dadr_err_tbl->tcnt,
   ") table(s) as needing a default row, ",
   " but no primary key constraint was found for the table(s) in the database.")
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dm_err->eproc = build("Following are the (",dadr_err_tbl->tcnt,
   ") default row tables without a primary key.")
  CALL disp_msg(" ",dm_err->logfile,0)
  FOR (ix1 = 1 TO dadr_err_tbl->tcnt)
   SET dm_err->eproc = build(ix1,". ",trim(dadr_err_tbl->tbl[ix1].tname))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDFOR
  SET dm_err->eproc = build("End of default row tables without a primary key.")
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dadr_warning_cnt = (dadr_warning_cnt+ dadr_err_tbl->tcnt)
 ENDIF
 SET dm_err->eproc = "DM2_ADD_DEFAULT_ROWS is complete."
 IF (dadr_error_cnt > 0)
  SET dm_err->eproc = concat(dm_err->eproc,"  There were ",trim(cnvtstring(dadr_error_cnt)),
   " errors encountered.")
 ENDIF
 IF (dadr_warning_cnt > 0)
  SET dm_err->eproc = concat(dm_err->eproc,"  There were ",trim(cnvtstring(dadr_warning_cnt)),
   " warnings encountered.")
 ENDIF
 CALL disp_msg(" ",dm_err->logfile,0)
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "ENDING DM2_ADD_DEFAULT_ROWS.PRG"
 CALL final_disp_msg(dadr_logfile_name)
 FREE RECORD dadr_table
 FREE RECORD dadr_ccldef
 FREE RECORD dadr_afd_tab
END GO
