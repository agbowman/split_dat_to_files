CREATE PROGRAM dm2_install_plan_menu_arch
 ...
 SUBROUTINE dipma_check_db_arch_mode(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dipma_inst_mon_stat = i2 WITH protect, noconstant( $1)
 DECLARE dipma_continue_loop = i2 WITH protect, noconstant(1)
 DECLARE dipma_archmon_status = i4 WITH protect, noconstant(0)
 DECLARE dipma_arch_dest = vc WITH protect, noconstant(" ")
 DECLARE dipma_arch_cur_spc_used = f8 WITH protect, noconstant(0.0)
 DECLARE dipma_arch_notify_threshold = f8 WITH protect, noconstant(0.0)
 DECLARE dipma_arch_pause_threshold = f8 WITH protect, noconstant(0.0)
 DECLARE dipma_arch_mon_not_set = i2 WITH protect, noconstant(1)
 DECLARE dipma_accept_validate = i2 WITH protect, noconstant(1)
 DECLARE dipma_qual_cnt = i4 WITH protect, noconstant(0)
 DECLARE dipma_temp_num = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm2_install_plan_ma",".log","DM2_INSTALL_PLAN_MA LogFile...") != 1)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 ...
 IF (dipma_check_db_arch_mode(null)=0)
  ...
  ENDIF
 ELSE
  IF (dipma_get_arch_thresholds(dipma_arch_pause_threshold,dipma_arch_notify_threshold)=0)
   IF ((dm_err->err_ind=1))
    CALL dipr_disp_error_msg(null)
    GO TO exit_program
   ENDIF
  ELSE
   SET dipma_archmon_status = 1
   SET dipma_arch_mon_not_set = 0
  ENDIF
 ENDIF
 IF (dipma_arch_mon_not_set=0)
  IF (dipma_get_arch_dest_space(dipma_arch_dest,dipma_arch_cur_spc_used)=0)
   CALL dipr_disp_error_msg(null)
   GO TO exit_program
  ENDIF
 ENDIF
 WHILE (dipma_continue_loop=1)
   SET message = window
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL text(2,2,"ARCHIVE LOG [MANAGE MONITORING]")
   ...
   IF (dipma_inst_mon_stat=0)
    CALL text(7,21,evaluate(dipma_inst_mon_stat,1,"ON","OFF"))
   ELSE
    CALL text(7,21,evaluate(dipma_archmon_status,1,"ON","OFF"))
   ENDIF
   CALL text(8,2,concat("Archive Log Destination: ",evaluate(dipma_arch_mon_not_set,1,"Not Set",
      dipma_arch_dest)))
   SET dipma_temp_num = dipma_get_two_dec(build(dipma_arch_cur_spc_used))
   CALL text(9,2,concat("Current Space Used: ",evaluate(dipma_arch_mon_not_set,1,"Unknown",concat(
       dipma_temp_num," Gigabytes"))))
   SET dipma_temp_num = dipma_get_two_dec(build(dipma_arch_notify_threshold))
   CALL text(10,2,concat("Warning emails occur when ArchiveLog space usage exceeds: ",dipma_temp_num,
     " Gigabytes. (0 means no Warning)"))
   SET dipma_temp_num = dipma_get_two_dec(build(dipma_arch_pause_threshold))
   CALL text(11,2,concat("Installation Pause occurs when ArchiveLog space usage exceeds: ",
     dipma_temp_num," Ggigabytes. (0 means no Pause)"))
   CALL text(13,2,"(M)odify, (S)ave, (B)ack: ")
   CALL accept(13,30,"p;cu","S"
    WHERE curaccept IN ("M", "S", "B"))
   IF (curaccept IN ("M", "S")
    AND dipma_inst_mon_stat=0)
    CALL text(24,2,concat("Current settings can not be ",evaluate(curaccept,"M","modified","saved"),
      " since Install Monitoring is OFF"))
    CALL pause(3)
   ELSE
    CASE (curaccept)
     OF "B":
      SET dipma_continue_loop = 0
     OF "M":
      SET help = pos(4,80,10,50)
      SET help = fix("ON,OFF")
      CALL clear(7,21,11)
      CALL accept(7,21,"A(3);CUF")
      IF (curhelp > 0)
       SET dipma_archmon_status = evaluate(curhelp,1,1,2,0)
      ELSE
       SET dipma_archmon_status = evaluate(trim(curaccept,3),"ON",1,0)
      ENDIF
      SET help = off
      SET message = window
      IF (dipma_archmon_status=1)
       IF (dipma_check_db_arch_mode(null)=0)
        IF ((dm_err->err_ind=1))
         CALL dipr_disp_error_msg(null)
         GO TO exit_program
        ENDIF
        CALL clear(24,1)
        SET dipma_archmon_status = 0
        CALL text(24,2,
         "Database Archive Logging needs to be enabled before Archive Log Monitoring can be turned ON"
         )
        CALL pause(5)
       ELSE
        IF (dipma_get_arch_dest_space(dipma_arch_dest,dipma_arch_cur_spc_used)=0)
        ...
        CALL text(8,2,concat("Archive Log Destination: ",evaluate(dipma_arch_mon_not_set,1,"Not Set",
           dipma_arch_dest)))
        SET dipma_temp_num = dipma_get_two_dec(build(dipma_arch_cur_spc_used))
        CALL text(9,2,concat("Current Space Used: ",evaluate(dipma_arch_mon_not_set,1,"Unknown",
           concat(dipma_temp_num," Gigabytes"))))
        SET dipma_accept_validate = 1
        WHILE (dipma_accept_validate=1)
          CALL clear(24,1)
          CALL clear(10,60,40)
          CALL text(10,71,"Gigabytes. (0 means no Warning)")
          CALL accept(10,61,"N(9);cu","0"
           WHERE cnvtreal(curaccept) >= 0)
          SET dipma_arch_notify_threshold = cnvtreal(dipma_get_two_dec(curaccept))
          CALL clear(11,65,40)
          CALL text(11,76,"Gigabytes. (0 means no Pause)")
          CALL accept(11,66,"N(9);cu","0"
           WHERE cnvtreal(curaccept) >= 0)
          SET dipma_arch_pause_threshold = cnvtreal(dipma_get_two_dec(curaccept))
          IF (dipma_archmon_status=1
           AND dipma_arch_notify_threshold=0
           AND dipma_arch_pause_threshold=0)
           CALL clear(24,1)
           CALL text(24,2,
            "One of the two values just entered needs to be greater than or equal to 0.01")
           CALL pause(3)
          ELSEIF (dipma_archmon_status=1
           AND dipma_arch_pause_threshold != 0
           AND dipma_arch_pause_threshold <= dipma_arch_notify_threshold)
           CALL clear(24,1)
           CALL text(24,2,
            "Value entered for Installation Pause needs to be greater than value for Warning e-mail")
           CALL pause(3)
          ELSE
           SET dipma_accept_validate = 0
          ENDIF
        ENDWHILE
       ENDIF
      ENDIF
     OF "S":
      SET dm_err->eproc = "Inserting or Deleting ArchiveLog Monitoring Settings"
      IF (dipma_archmon_status=0)
       DELETE  FROM dm_info di
        WHERE di.info_domain="DM2_INSTALL_MONITOR:ARCHLOG"
         AND di.info_name="SETTINGS"
        WITH nocounter
      ...
      ELSE
       UPDATE  FROM dm_info di
        SET di.info_number = dipma_arch_notify_threshold, di.info_long_id =
         dipma_arch_pause_threshold, di.updt_cnt = (di.updt_cnt+ 1),
       ...
       IF (curqual=0)
        INSERT  FROM dm_info di
         SET di.info_number = dipma_arch_notify_threshold, di.info_long_id =
          dipma_arch_pause_threshold, di.info_name = "SETTINGS",
          di.info_domain = "DM2_INSTALL_MONITOR:ARCHLOG", di.updt_cnt = 0, di.updt_task = reqinfo->
          updt_task,
      ...
      SET dm2_process_event_rs->begin_dt_tm = cnvtdatetime(curdate,curtime3)
      SET dm2_process_event_rs->end_dt_tm = cnvtdatetime(curdate,curtime3)
      CALL dm2_process_log_add_detail_text("STATUS",evaluate(dipma_archmon_status,1,"ON","OFF"))
      CALL dm2_process_log_add_detail_number("ARCH_WARNING",dipma_arch_notify_threshold)
      CALL dm2_process_log_add_detail_number("ARCH_PAUSE",dipma_arch_pause_threshold)
      CALL dm2_process_log_add_detail_text("USERNAME",curuser)
      CALL dm2_process_log_row(dpl_install_monitor,dpl_auditlog,dpl_no_prev_id,1)
      SET dipma_continue_loop = 0
    ENDCASE
   ENDIF
 ENDWHILE
