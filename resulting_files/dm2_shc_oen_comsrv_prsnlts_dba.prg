CREATE PROGRAM dm2_shc_oen_comsrv_prsnlts:dba
 ...
 END ;Subroutine
 DECLARE cnt = i4 WITH protect, noconstant(0)
 DECLARE dsocp_fatal_err_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_shc_oen",".log","DM2 SYSTEM HEALTH CHECK OEN COMSERVER LOG FILE")=0)
  SET dsocp_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 ...
 ELSE
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Checking the configuration of OEN Comserver Personalities."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dshc_add_check_to_rs("OEN COMSERVER PERSONALITIES","OEN Comserver Personalities",0,"R",
  dshc_ctf_millennium)=0)
  SET dsocp_fatal_err_ind = 1
 ENDIF
 SELECT INTO "nl:"
  oen.interfaceid, oen.name, oen.value
  FROM oen_personality oen
  WHERE ((oen.name="SLEEP"
   AND cnvtint(oen.value) < 10) OR (((oen.name="TRACE_LEVEL"
   AND cnvtint(oen.value) != 0) OR (((oen.name="LOWLEVELTRACE"
   AND cnvtint(oen.value) != 0) OR (((oen.name="EVENT_LOG"
   AND oen.value != "N") OR (oen.name="OEN_EVENT_LOG"
   AND oen.value != "N")) )) )) ))
  ORDER BY oen.name, oen.interfaceid
  HEAD REPORT
   dshc->check[dshc->chk_cnt].check_desc_text =
   "The following OEN Comserver Personalities do not meet Cerner's standard configuration."
  DETAIL
   cnt += 1, dshc->check[dshc->chk_cnt].desc_dtl_cnt = cnt, stat = alterlist(dshc->check[dshc->
    chk_cnt].desc_detail,dshc->check[dshc->chk_cnt].desc_dtl_cnt)
   IF (oen.name="SLEEP")
    dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text =
    "The following OEN Comservers have a SLEEP time of less than 10 seconds:"
   ELSEIF (oen.name="TRACE_LEVEL")
    dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text =
    "The following OEN Comservers have a TRACE_LEVEL property that is not set to 0:"
   ELSEIF (oen.name="LOWLEVELTRACE")
    dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text =
    "The following OEN Comservers have a LOWLEVELTRACE property that is not set to 0:"
   ELSEIF (oen.name="EVENT_LOG")
    dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text =
    "The following OEN Comservers have an EVENT_LOG property that is not set to N:"
   ELSEIF (oen.name="OEN_EVENT_LOG")
    dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text =
    "The following OEN Comservers have an OEN_EVENT_LOG property that is not set to N:"
   ENDIF
   dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "OEN Comserver Personalities", dshc->check[dshc
   ->chk_cnt].desc_detail[cnt].value_prefix = "NOT_SET", dshc->check[dshc->chk_cnt].desc_detail[cnt].
   type = 1,
   dshc->check[dshc->chk_cnt].desc_detail[cnt].nbr_value = oen.interfaceid
  WITH nocounter
 ;end select
 IF (check_error("Checking the configuration of OEN Comserver personalities.")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  SET dshc->check[dshc->chk_cnt].error_ind = 1
  SET dshc->check[dshc->chk_cnt].good_ind = 0
  GO TO exit_script
 ...
 ENDIF
#exit_script
 CALL final_disp_msg("dm2_shc_oen")
 IF (dsocp_fatal_err_ind=0)
  SET dm_err->err_ind = 0
 ENDIF
END GO
