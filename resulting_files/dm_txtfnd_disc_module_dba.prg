CREATE PROGRAM dm_txtfnd_disc_module:dba
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 DECLARE dtdm_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dtdm_emsg = vc WITH protect, noconstant(" ")
 DECLARE dtdm_det_name = vc WITH protect, noconstant(" ")
 DECLARE dtdm_flag = i4 WITH protect, noconstant(0)
 DECLARE dtdm_idx = i4 WITH protect, noconstant(0)
 DECLARE dtdm_alias = vc WITH protect, noconstant("")
 DECLARE dtdm_cnt = i4 WITH protect, noconstant(0)
 DECLARE dtdm_pos = i4 WITH protect, noconstant(0)
 FREE RECORD dtdm_prog
 RECORD dtdm_prog(
   1 cnt = i4
   1 qual[*]
     2 line_data = vc
 )
 FREE RECORD dtdm_rules
 RECORD dtdm_rules(
   1 cnt = i4
   1 qual[*]
     2 script_name = vc
 ...
 IF (check_logfile("dm_tf_dm",".log","DM_TXTFND_DISC_MODULE LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_TXTFND_DISC_MODULE..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET modify maxvarlen 268435456
 ...
 IF (dtla_detail_id=0.0)
  ...
  SELECT INTO "nl:"
   ...
   WHERE d.detail_type_flag=2
    AND d.active_ind=1
    AND d.detail_script_name="DM_TXTFND_DISC_MODULE"
   DETAIL
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "No active data found in DM_TEXT_FIND_DETAIL for script DM_TXTFND_DISC_MODULE."
   IF ((script_rec->dm_text_find_log_id > 0.0))
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
   ENDIF
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "NL:"
  FROM dm_text_find d,
   dm_text_find_detail d2
  WHERE d2.dm_text_find_detail_id=dtla_detail_id
   AND d2.dm_text_find_id=d.dm_text_find_id
  DETAIL
   dtdm_det_name = d.find_name, dtdm_flag = d2.detail_type_flag
  WITH nocounter
 ;end select
 IF (check_error("Getting DM_TEXT_FIND information") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for detail information:",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 IF (curqual=0)
  SET dm_err->err_ind = 1
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_TXTFND_ATTRIBUTE:EKS_MODULESTORAGE"
   AND  EXISTS (
  (SELECT
   "x"
   FROM user_tab_columns utc
   WHERE utc.table_name="EKS_MODULESTORAGE"
    AND utc.column_name=di.info_name))
  ORDER BY di.info_name
  DETAIL
   dtdm_attrib->cnt = (dtdm_attrib->cnt+ 1), stat = alterlist(dtdm_attrib->qual,dtdm_attrib->cnt),
   dtdm_attrib->qual[dtdm_attrib->cnt].table_name = "EKS_MODULESTORAGE",
   dtdm_attrib->qual[dtdm_attrib->cnt].column_name = di.info_name, dtdm_attrib->qual[dtdm_attrib->cnt
   ].column_label = di.info_char, dtdm_attrib->qual[dtdm_attrib->cnt].column_flg = di.info_number
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_TXTFND_ATTRIBUTE:EKS_MODULE"
   AND  EXISTS (
  (SELECT
   "x"
   FROM user_tab_columns utc
   WHERE utc.table_name="EKS_MODULE"
    AND utc.column_name=di.info_name))
  ORDER BY di.info_name
  DETAIL
   dtdm_attrib->cnt = (dtdm_attrib->cnt+ 1), stat = alterlist(dtdm_attrib->qual,dtdm_attrib->cnt),
   dtdm_attrib->qual[dtdm_attrib->cnt].table_name = "EKS_MODULE",
   dtdm_attrib->qual[dtdm_attrib->cnt].column_name = di.info_name, dtdm_attrib->qual[dtdm_attrib->cnt
   ].column_label = di.info_char, dtdm_attrib->qual[dtdm_attrib->cnt].column_flg = di.info_number
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_TXTFND_ATTRIBUTE:PRSNL"
   AND  EXISTS (
  (SELECT
   "x"
   FROM user_tab_columns utc
  ...
  DETAIL
   dtdm_attrib->cnt = (dtdm_attrib->cnt+ 1), stat = alterlist(dtdm_attrib->qual,dtdm_attrib->cnt),
   dtdm_attrib->qual[dtdm_attrib->cnt].table_name = "PRSNL",
   dtdm_attrib->qual[dtdm_attrib->cnt].column_name = di.info_name, dtdm_attrib->qual[dtdm_attrib->cnt
   ].column_label = di.info_char, dtdm_attrib->qual[dtdm_attrib->cnt].column_flg = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  ...
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for Attribute list:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ELSEIF ((dtdm_attrib->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No attribute data found for EKS_MODULE."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for Attribute list:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Gathering RULE Compile_dt_tm values"
 SET dtdm_rules->cnt = 0
 SET stat = alterlist(dtdm_rules->qual,0)
 SELECT INTO "NL:"
  FROM dprotect d
  WHERE d.object="E"
  DETAIL
   dtdm_rules->cnt = (dtdm_rules->cnt+ 1), stat = alterlist(dtdm_rules->qual,dtdm_rules->cnt),
   dtdm_rules->qual[dtdm_rules->cnt].script_name = d.object_name,
   dtdm_rules->qual[dtdm_rules->cnt].compile_dt_tm = cnvtdatetime(d.datestamp,cnvttime2(format(d
      .timestamp,"######;rp0"),"HHMMSS"))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for compiled rules:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Gathering Discern Modules"
 SET script_rec->long_cnt = 0
 SET stat = alterlist(script_rec->long_qual,script_rec->long_cnt)
 SET script_rec->script_cnt = 0
 SET stat = alterlist(script_rec->script_qual,script_rec->script_cnt)
 SET stat = initrec(dtg)
 CALL parser("select into 'nl:' from eks_module m, eks_modulestorage e, prsnl p ",0)
 CALL parser(
  'where m.active_flag = "A" and e.module_name = m.module_name and e.version = m.version and e.data_type in (7,8,9)',
  0)
 CALL parser(" and m.maint_validation not in ('EXAMPLE') ",0)
 CALL parser(
  "   and (NOT EXISTS (select 'x' from dm_text_find_data d where d.dm_text_find_detail_id = dtla_detail_id ",
  0)
 CALL parser(
  "   and d.script_name=trim(cnvtupper(e.MODULE_NAME)) and d.group_num = e.data_type and d.find_name = dtdm_det_name)",
  0)
 CALL parser(
  "   or EXISTS (select 'x' from dm_text_find_data dt where dt.dm_text_find_detail_id = dtla_detail_id ",
  0)
 CALL parser(
  "   and dt.script_name=trim(cnvtupper(e.MODULE_NAME)) and dt.find_name=dtdm_det_name and dt.group_num=e.data_type",
  0)
 CALL parser(
  " and dt.compile_dt_tm<nullval(m.maint_date,cnvtdatetime(curdate,curtime3)))) and p.person_id=outerjoin(m.updt_id)",
  0)
 CALL parser(" head report script_rec->dm_text_find_detail_id = dtla_detail_id dtdm_cnt = 0 ",0)
 CALL parser(
  "detail dtg->cnt = dtg->cnt + 1 stat = alterlist(dtg->qual, dtg->cnt) dtdm_pos = dtg->cnt ",0)
 CALL parser(" dtg->qual[dtdm_pos].data_source = m.maint_title",0)
 CALL parser(" dtg->qual[dtdm_pos].group_num = e.data_type",0)
 CALL parser(" dtg->qual[dtdm_pos].s_name = e.module_name",0)
 CALL parser(" dtg->qual[dtdm_pos].compile_dt_tm = cnvtdatetime(m.maint_date)",0)
 CALL parser("   dtg->qual[dtdm_pos].att_cnt = dtg->qual[dtdm_pos].att_cnt + 1",0)
 CALL parser("   stat = alterlist(dtg->qual[dtdm_pos].att_qual, dtg->qual[dtdm_pos].att_cnt)",0)
 CALL parser(
  "      dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].tab_name = 'EKS_MODULESTORAGE'",0)
 CALL parser("   dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_name = 'DATA_TYPE'",0)
 CALL parser(
  "   dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_txt = 'Module Section'",0)
 CALL parser(concat("   dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_val = ",
   "evaluate(e.data_type,7,'Evoke',8,'Logic',9,'Action')"),0)
 CALL parser("   dtg->qual[dtdm_pos].att_cnt = dtg->qual[dtdm_pos].att_cnt + 1",0)
 CALL parser("   stat = alterlist(dtg->qual[dtdm_pos].att_qual, dtg->qual[dtdm_pos].att_cnt)",0)
 CALL parser("      dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].tab_name = 'DPROTECT'",
  0)
 CALL parser(
  "   dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_name = 'COMPILE_DT_TM'",0)
 CALL parser(
  "   dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_txt = 'Compile Date/Time'",0)
 CALL parser(
  "   dtdm_idx = locateval(dtdm_idx, 1, dtdm_rules->cnt, m.module_name, dtdm_rules->qual[dtdm_idx].script_name)",
  0)
 CALL parser(
  "   if (dtdm_idx > 0) dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_val = ",0)
 CALL parser("   format(dtdm_rules->qual[dtdm_idx].compile_dt_Tm, 'YYYY-MMM-DD HH:MM;;Q') ",0)
 CALL parser(
  "   else dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_val = 'N/A' endif",0)
 FOR (dtdm_idx = 1 TO dtdm_attrib->cnt)
   CALL parser("   dtg->qual[dtdm_pos].att_cnt = dtg->qual[dtdm_pos].att_cnt + 1",0)
   CALL parser("   stat = alterlist(dtg->qual[dtdm_pos].att_qual, dtg->qual[dtdm_pos].att_cnt)",0)
   CALL parser(concat("      dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].tab_name = '",
     dtdm_attrib->qual[dtdm_idx].table_name,"'"),0)
   CALL parser(concat("   dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_name = '",
     dtdm_attrib->qual[dtdm_idx].column_name,"'"),0)
   CALL parser(concat(
     "   dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_txt = dtdm_attrib->qual[",trim
     (cnvtstring(dtdm_idx)),"].column_label"),0)
   IF ((dtdm_attrib->qual[dtdm_idx].table_name="PRSNL"))
    SET dtdm_alias = "p"
   ELSEIF ((dtdm_attrib->qual[dtdm_idx].table_name="EKS_MODULE"))
    SET dtdm_alias = "m"
   ELSE
    SET dtdm_alias = "e"
   ENDIF
   IF ((dtdm_attrib->qual[dtdm_idx].column_flg=3))
    CALL parser(concat(
      "   dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_val = format(",dtdm_alias,".",
      dtdm_attrib->qual[dtdm_idx].column_name,",';;q')"),0)
   ELSEIF ((dtdm_attrib->qual[dtdm_idx].column_flg=2))
    CALL parser(concat(
      "   dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_val = cnvtstring(",dtdm_alias,
      ".",dtdm_attrib->qual[dtdm_idx].column_name,",20)"),0)
   ELSE
    CALL parser(concat(
      "   dtg->qual[dtdm_pos].att_qual[dtg->qual[dtdm_pos].att_cnt].col_val = trim(replace(",
      dtdm_alias,".",dtdm_attrib->qual[dtdm_idx].column_name,",char(0),' ',0))"),0)
   ENDIF
 ENDFOR
 CALL parser("with nocounter go",1)
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for Discern modules:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 IF ((dtg->cnt > 0))
  ...
  ENDIF
 ENDIF
 FOR (dtdm_idx = 1 TO script_rec->script_cnt)
   SET stat = initrec(dtdm_prog)
   SET dm_err->eproc = "Gathering script body from EKS_MODULESTORAGE"
   SELECT INTO "NL:"
    bloblen = textlen(e.ekm_info)
    FROM eks_module m,
     eks_modulestorage e
    WHERE (m.module_name=script_rec->script_qual[dtdm_idx].script_name)
     AND m.active_flag="A"
     AND e.module_name=m.module_name
     AND e.version=m.version
     AND (e.data_type=script_rec->script_qual[dtdm_idx].group_num)
    HEAD REPORT
     outbuf = fillstring(32767," "), offset = 0, retlen = 0,
     dtdm_prog->cnt = 1, stat = alterlist(dtdm_prog->qual,1), dtdm_prog->qual[1].line_data = " "
    DETAIL
     retlen = blobget(outbuf,offset,e.ekm_info)
     WHILE (retlen > 0)
       IF ((dtdm_prog->qual[1].line_data=" "))
        dtdm_prog->qual[1].line_data = notrim(outbuf)
       ELSE
        dtdm_prog->qual[1].line_data = notrim(concat(notrim(dtdm_prog->qual[1].line_data),notrim(
           substring(1,retlen,outbuf))))
       ENDIF
       offset = (offset+ retlen), retlen = blobget(outbuf,offset,e.ekm_info)
     ENDWHILE
     dtdm_prog->qual[1].line_data = trim(dtdm_prog->qual[1].line_data)
    WITH nocounter, rdbarrayfetch = 1
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for script body:",
      dm_err->emsg),0.0,0.0)
    SET dtdm_err_ind = dm_err->err_ind
    SET dtdm_emsg = dm_err->emsg
    SET dm_err->err_ind = 0
   ELSE
    CALL dst_insert_data(dtdm_idx,script_rec,dtdm_det_name,dtdm_prog,dtdm_flag)
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dtdm_err_ind = dm_err->err_ind
     SET dtdm_emsg = dm_err->emsg
 ...
 ENDFOR
#exit_script
 SET dm_err->eproc = "...Ending DM_TXTFND_DISC_MODULE"
 CALL final_disp_msg("dm_tf_dm")
 IF (dtdm_err_ind=1
  AND (dm_err->err_ind=0))
  SET dm_err->err_ind = dtdm_err_ind
  SET dm_err->emsg = dtdm_emsg
 ENDIF
END GO
