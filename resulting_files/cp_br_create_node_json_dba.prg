CREATE PROGRAM cp_br_create_node_json:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Pathway Id" = 0.0,
  "Node Name	" = "",
  "Concept Cd" = 0.0,
  "Treatment Line Cd" = 0.0,
  "Intention Cd" = 0.0,
  "Publish Ind" = 1,
  "Sequence" = 0
  WITH outdev, pathwayid, nodename,
  conceptcd, treatmentcd, intentioncd,
  publishind, nodeseq
 FREE RECORD request
 RECORD request(
   1 cp_pathway_id = f8
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE verifyinputsandloadrequest(null) = null WITH protect
 DECLARE executescript(null) = null WITH protect
 DECLARE verified = i2 WITH noconstant(0), protect
 DECLARE pathwayid = f8 WITH noconstant(0.0), protect
 DECLARE nodename = vc WITH noconstant(""), protect
 DECLARE conceptcd = f8 WITH noconstant(0.0), protect
 DECLARE treatmentcd = f8 WITH noconstant(0.0), protect
 DECLARE intentioncd = f8 WITH noconstant(0.0), protect
 DECLARE publishind = i4 WITH noconstant(0), protect
 ...
 CALL verifyinputsandloadrequest(null)
 CALL executescript(null)
 SUBROUTINE verifyinputsandloadrequest(null)
   CALL log_message("In verifyInputsAndLoadRequest()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE x = i4 WITH noconstant(0), protect
   DECLARE verified = i2 WITH noconstant(0), protect
   SET nodename =  $NODENAME
   SET conceptcd =  $CONCEPTCD
   SET treatmentcd =  $TREATMENTCD
   SET intentioncd =  $INTENTIONCD
   SET pathwayid =  $PATHWAYID
   SET publishind =  $PUBLISHIND
   SET nodeseq =  $NODESEQ
   CALL echo(build("nodeName 		: ",nodename))
   CALL echo(build("conceptCd 		: ",conceptcd))
   CALL echo(build("treatmentCd 	: ",treatmentcd))
   CALL echo(build("intentionCd 	: ",intentioncd))
   CALL echo(build("pathwayId 		: ",pathwayid))
   SET stat = alterlist(request->node_list,1)
   SET request->cp_pathway_id = pathwayid
   SET request->node_list[1].concept_cd = conceptcd
   SET request->node_list[1].intention_cd = intentioncd
   SET request->node_list[1].node_name = nodename
   SET request->node_list[1].treatment_line_cd = treatmentcd
   SET request->node_list[1].publish_ind = publishind
   SET request->node_list[1].category_mean = ""
   SET request->node_list[1].node_seq = nodeseq
   CALL echorecord(request)
   CALL log_message(build("Exit verifyInputsAndLoadRequest(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE executescript(null)
   CALL log_message("In executeScript()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   EXECUTE cp_br_create_node
   CALL echorecord(reply)
   IF ((reply->status_data.status="S"))
    COMMIT
   ENDIF
   SET stat = moverec(reply->node_list,mpagereply->node_list)
   SET stat = moverec(reply->status_data,mpagereply->status_data)
   CALL log_message(build("Exit executeScript(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (validate(debug_ind,0)=1)
