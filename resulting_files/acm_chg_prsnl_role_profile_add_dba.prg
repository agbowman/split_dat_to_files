CREATE PROGRAM acm_chg_prsnl_role_profile_add:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->prsnl_role_profile_addn_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   prsnl_role_profile_addn p
  SET p.prsnl_role_profile_addn_id = acm_request->prsnl_role_profile_addn_qual[xref->chg[d.seq].idx].
   prsnl_role_profile_addn_id, p.prsnl_org_reltn_type_id =
   IF ((acm_request->prsnl_role_profile_addn_qual[xref->chg[d.seq].idx].prsnl_org_reltn_type_id > 0))
     acm_request->prsnl_role_profile_addn_qual[xref->chg[d.seq].idx].prsnl_org_reltn_type_id
   ELSE reply->prsnl_org_reltn_type_qual[acm_request->prsnl_role_profile_addn_qual[xref->chg[d.seq].
    idx].prsnl_org_reltn_type_idx].prsnl_org_reltn_type_id
   ENDIF
   , p.child_type_cd = acm_request->prsnl_role_profile_addn_qual[xref->chg[d.seq].idx].child_type_cd,
   p.type_entity_name = acm_request->prsnl_role_profile_addn_qual[xref->chg[d.seq].idx].
   type_entity_name, p.type_entity_id = acm_request->prsnl_role_profile_addn_qual[xref->chg[d.seq].
   idx].type_entity_id, p.beg_effective_dt_tm = cnvtdatetime(acm_request->
    prsnl_role_profile_addn_qual[xref->chg[d.seq].idx].beg_effective_dt_tm),
   p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_role_profile_addn_qual[xref->chg[d.seq].
    idx].end_effective_dt_tm), p.active_ind = acm_request->prsnl_role_profile_addn_qual[xref->chg[d
   .seq].idx].active_ind, p.active_status_cd = acm_request->prsnl_role_profile_addn_qual[xref->chg[d
   .seq].idx].active_status_cd,
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.prsnl_role_profile_addn_id=acm_request->prsnl_role_profile_addn_qual[xref->chg[d.seq].idx
   ].prsnl_role_profile_addn_id))
  WITH nocounter, status(reply->prsnl_role_profile_addn_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->prsnl_role_profile_addn_qual[xref->chg[index].idx].status != 1))
    SET failed = update_error
    SET table_name = "PRSNL_ROLE_PROFILE_ADDN"
    GO TO exit_script
   ENDIF
 ENDFOR
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM prsnl_role_profile_addn p
    WHERE expand(t1,f_val,t_val,p.prsnl_role_profile_addn_id,acm_request->
     prsnl_role_profile_addn_qual[xref->chg[t1].idx].prsnl_role_profile_addn_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.prsnl_role_profile_addn_id,acm_request->
      prsnl_role_profile_addn_qual[xref->chg[t1].idx].prsnl_role_profile_addn_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->prsnl_role_profile_addn_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->prsnl_role_profile_addn_qual[idx].status = - (1), reply->prsnl_role_profile_addn_qual[
      idx].prsnl_role_profile_addn_id = acm_request->prsnl_role_profile_addn_qual[idx].
      prsnl_role_profile_addn_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->prsnl_role_profile_addn_qual[idx].chg_str
     IF (findstring("PRSNL_ORG_RELTN_TYPE_ID,",chg_str)=0)
      acm_request->prsnl_role_profile_addn_qual[idx].prsnl_org_reltn_type_id = p
      .prsnl_org_reltn_type_id
     ENDIF
     IF (findstring("CHILD_TYPE_CD,",chg_str)=0)
      acm_request->prsnl_role_profile_addn_qual[idx].child_type_cd = p.child_type_cd
     ENDIF
     IF (findstring("TYPE_ENTITY_NAME,",chg_str)=0)
      acm_request->prsnl_role_profile_addn_qual[idx].type_entity_name = p.type_entity_name
     ENDIF
     IF (findstring("TYPE_ENTITY_ID,",chg_str)=0)
      acm_request->prsnl_role_profile_addn_qual[idx].type_entity_id = p.type_entity_id
     ENDIF
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_role_profile_addn_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_role_profile_addn_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->prsnl_role_profile_addn_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->prsnl_role_profile_addn_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_role_profile_addn_qual[xref->chg[index].idx].status=0))
