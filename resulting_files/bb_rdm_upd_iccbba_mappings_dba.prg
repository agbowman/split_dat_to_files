CREATE PROGRAM bb_rdm_upd_iccbba_mappings:dba
 FREE RECORD mapping_rec
 RECORD mapping_rec(
   1 mapping_list[*]
     2 version_nbr = i4
     2 table_type = c12
 ...
 SUBROUTINE insertreqproc(nrequest,spfmt,ntargetreq,ndeststep,sservice,nreprocess,naction,srefpfmt,
  nseq)
   ...
   ENDCASE
 END ;Subroutine
 SET sreadme_name = "bb_rdm_upd_iccbba_mappings"
#start_script
 CALL logscriptstart(sreadme_name)
 DECLARE lrowcnt = i4
 DECLARE nlistcnt = i2
 DECLARE lrowmax = i4
 SET nlistcnt = size(requestin->list_0,5)
 SET lrowmax = 33
 IF (nlistcnt=0)
  CALL logstatus(build(sreadme_name,"No records read from input file.  Nothing to process."),f)
  GO TO script_end
 ENDIF
 SELECT INTO "nl:"
  nnomatchrowind = evaluate(nullind(b.bb_iccbba_mapping_id),0,1,1,0)
  FROM bb_iccbba_mapping b,
   (dummyt d1  WITH seq = nlistcnt)
  PLAN (d1)
   JOIN (b
   WHERE b.version_nbr=outerjoin(cnvtint(requestin->list_0[d1.seq].version_nbr))
    AND b.table_type_desc=outerjoin(requestin->list_0[d1.seq].table_type)
    AND b.column_type_desc=outerjoin(requestin->list_0[d1.seq].column_type))
  HEAD REPORT
   ncnt = 0
  DETAIL
   IF (nnomatchrowind=0)
    ncnt = (ncnt+ 1)
    IF (ncnt > size(mapping_rec->mapping_list,5))
     stat = alterlist(mapping_rec->mapping_list,(ncnt+ 9))
    ENDIF
    stat = alterlist(mapping_rec->mapping_list,ncnt), mapping_rec->mapping_list[ncnt].column_name =
    requestin->list_0[d1.seq].column_name, mapping_rec->mapping_list[ncnt].column_type = requestin->
    list_0[d1.seq].column_type,
    mapping_rec->mapping_list[ncnt].table_name = requestin->list_0[d1.seq].table_name, mapping_rec->
    mapping_list[ncnt].table_type = requestin->list_0[d1.seq].table_type, mapping_rec->mapping_list[
    ncnt].version_nbr = cnvtint(requestin->list_0[d1.seq].version_nbr),
    mapping_rec->mapping_list[ncnt].unique_ver_ind = cnvtint(requestin->list_0[d1.seq].unique_ver_ind
     )
   ENDIF
  FOOT REPORT
   stat = alterlist(mapping_rec->mapping_list,ncnt)
  WITH nocounter, outerjoin = d1
 ;end select
 IF (size(mapping_rec->mapping_list,5) > 0)
  INSERT  FROM bb_iccbba_mapping b,
    (dummyt d2  WITH seq = value(size(mapping_rec->mapping_list,5)))
   SET b.bb_iccbba_mapping_id = cnvtreal(seq(reference_seq,nextval)), b.column_name = mapping_rec->
    mapping_list[d2.seq].column_name, b.column_type_desc = mapping_rec->mapping_list[d2.seq].
    column_type,
    b.table_name = mapping_rec->mapping_list[d2.seq].table_name, b.table_type_desc = mapping_rec->
    mapping_list[d2.seq].table_type, b.version_nbr = mapping_rec->mapping_list[d2.seq].version_nbr,
    b.unique_version_ind = mapping_rec->mapping_list[d2.seq].unique_ver_ind, b.updt_applctx =
    validate(reqinfo->updt_applctx,0), b.updt_cnt = 0,
    b.updt_dt_tm = cnvtdatetime(curdate,curtime3), b.updt_id = validate(reqinfo->updt_id,0.0), b
  ...
  ;end insert
 ELSE
  CALL logstatus(build(sreadme_name,"Success"),"S")
  GO TO script_end
 ENDIF
 SELECT INTO "nl:"
  lcnt = count(bb.bb_iccbba_mapping_id)
  FROM bb_iccbba_mapping bb
  DETAIL
   lrowcnt = lcnt
  WITH nocounter
 ;end select
 IF (lrowcnt=lrowmax)
  CALL logstatus(build(sreadme_name,"Success"),"S")
 ELSE
  CALL logstatus(build(sreadme_name,"Failed"),"F")
 ENDIF
#script_end
 FREE SET mapping_rec
