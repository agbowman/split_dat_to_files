CREATE PROGRAM acm_del_phone:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.phone_id=acm_request->phone_qual[xref->del[d.seq].idx].phone_id))
  WITH nocounter, status(reply->phone_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->phone_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    FROM phone p
    WHERE expand(t1,f_val,t_val,p.phone_id,acm_request->phone_qual[xref->del[t1].idx].phone_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.phone_id,acm_request->phone_qual[xref->del[t1].idx].phone_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->phone_qual[xref->del[index].idx].status=0))
