CREATE PROGRAM dm_pft_br_edit_failure_rows:dba
 ...
 SET reply->table_name = "PFT_BR_EDIT_FAILURE"
 ...
 DECLARE min_days_to_keep_limit = i4 WITH protect, constant(731)
 ...
 IF (v_days_to_keep < min_days_to_keep_limit)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"k1",
   "You must keep at least 731 days worth of data. You entered %1 days.","i",v_days_to_keep)
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(pbef.pft_br_edit_failure_id)
   FROM pft_br_edit_failure pbef
   WHERE pbef.pft_br_edit_failure_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(pbef.pft_br_edit_failure_id)
  FROM pft_br_edit_failure pbef
  WHERE pbef.pft_br_edit_failure_id > 0.0
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM pft_br_edit_failure pbef
    WHERE pbef.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND pbef.pft_br_edit_failure_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("pbef.pft_br_edit_failure_id != 0","pbef"))
    DETAIL
     row_cnt += 1
     IF (mod(row_cnt,100)=1)
      stat = alterlist(reply->rows,(row_cnt+ 99))
     ENDIF
     reply->rows[row_cnt].row_id = pbef.rowid
    WITH nocounter, maxqual(pbef,value(rowsleft))
   ;end select
