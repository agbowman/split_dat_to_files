CREATE PROGRAM dm_rmc_nsi_dup_check
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dupsize = i4 WITH protect
 DECLARE cnt = i4 WITH protect
 DECLARE dupcnt = i4 WITH protect
 IF (check_logfile("dm_rmc_nsi_dupchk",".log","DM_RMC_NSI_DUP_CHECK LOG FILE...") != 1)
  SET dm_err->err_ind = 1
 ...
 IF ((validate(request->source_env_id,- (1))=- (1)))
  ...
  SET dm_err->eproc = concat("Checking environment id prompt")
  IF (isnumeric( $1) > 0)
   SET request->source_env_id =  $1
  ELSE
   SET dm_err->emsg =
   "Source_env_id missing. Build request structure or enter source_env_id after program name."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 FREE RECORD nsi_dups
 RECORD nsi_dups(
   1 qual[*]
     2 target_nsi = f8
     2 r_nsi = f8
 )
 SET dm_err->eproc = concat("Beginning program dm_rmc_nsi_dup_check")
 CALL disp_msg("",dm_err->logfile,0)
 DELETE  FROM dm_refchg_warning
  WHERE (source_env_id=request->source_env_id)
   AND warning_type="NSI DUPLICATE"
  WITH nocounter
 ;end delete
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = concat("Verify $R Table exists")
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  ut.table_name
  FROM user_tables ut
  WHERE ut.table_name="NORMALIZED_STR0858$R"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Check for duplicates in Normalized_string_index")
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  t1.normalized_string_id, r1.normalized_string_id
  FROM normalized_str0858$r r1,
   normalized_string_index t1
  WHERE t1.normalized_string=r1.normalized_string
   AND t1.nomenclature_id=r1.nomenclature_id
   AND r1.rdds_status_flag < 9000
   AND r1.rdds_delete_ind=0
   AND t1.normalized_string_id != r1.normalized_string_id
   AND  NOT ( EXISTS (
  (SELECT
   "x"
   FROM normalized_str0858$r r2
   WHERE r2.normalized_string_id=t1.normalized_string_id
    AND r2.rdds_status_flag=0)))
  HEAD REPORT
   dupcnt = 0
  DETAIL
   dupcnt = (dupcnt+ 1)
   IF (mod(dupcnt,10)=1)
    stat = alterlist(nsi_dups->qual,(dupcnt+ 9))
   ENDIF
   nsi_dups->qual[dupcnt].target_nsi = t1.normalized_string_id, nsi_dups->qual[dupcnt].r_nsi = r1
   .normalized_string_id
  FOOT REPORT
   stat = alterlist(nsi_dups->qual,dupcnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  GO TO exit_program
 ENDIF
 SET dupsize = size(nsi_dups->qual,5)
 IF (dupsize > 0)
  SET dm_err->eproc = concat("Inserting dm_refchg_warning rows")
  CALL disp_msg("",dm_err->logfile,0)
  FOR (cnt = 1 TO dupsize)
    INSERT  FROM dm_refchg_warning drw
     SET drw.dm_refchg_warning_id = seq(dm_clinical_seq,nextval), drw.message = concat(
       "Normalized_String_ID duplicates. $R value: ",cnvtstring(nsi_dups->qual[cnt].r_nsi),
       ", Live value: ",cnvtstring(nsi_dups->qual[cnt].target_nsi),
       " violates index XAK1NORMALIZED_STRING_INDEX"), drw.source_env_id = request->source_env_id,
      drw.table_name = "NORMALIZED_STRING_INDEX", drw.updt_applctx = reqinfo->updt_applctx, drw
      .updt_cnt = 0,
      drw.updt_dt_tm = cnvtdatetime(curdate,curtime3), drw.updt_id = reqinfo->updt_id, drw.updt_task
       = reqinfo->updt_task,
      drw.warning_type = "NSI DUPLICATE"
    ;end insert
  ENDFOR
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
