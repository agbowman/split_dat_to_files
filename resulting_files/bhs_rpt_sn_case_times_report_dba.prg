CREATE PROGRAM bhs_rpt_sn_case_times_report:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  FOOT  scp.surg_case_proc_id
   pl_add_ind = 0
  WITH nocounter, outerjon = d1, outerjoin = d2
 ;end select
 CALL echorecord(m_case_time)
 SELECT INTO  $OUTDEV
  date_of_surgery = m_case_time->list[d.seq].f_sched_dttm"@SHORTDATE", day_of_week = trim(substring(1,
    20,m_case_time->list[d.seq].s_sched_day),3), sched_or_rm = trim(substring(1,50,m_case_time->list[
    d.seq].s_sched_rm),3),
  ...
  m_case_time->list[d.seq].f_sched_dttm"@SHORTDATE", sched_time_tm = m_case_time->list[d.seq].
  f_sched_dttm"@TIMENOSECONDS",
  cancel_flag = m_case_time->list[d.seq].s_cancel_ind, checkin_time = m_case_time->list[d.seq].
  f_checkin_dttm"@SHORTDATETIME", preop_in_date = m_case_time->list[d.seq].f_preopin_dttm"@SHORTDATE",
  preop_in_time = m_case_time->list[d.seq].f_preopin_dttm"@TIMENOSECONDS", preop_out_date =
  m_case_time->list[d.seq].f_preopout_dttm"@SHORTDATE", preop_out_time = m_case_time->list[d.seq].
  f_preopout_dttm"@TIMENOSECONDS",
  preop_totmins = datetimediff(cnvtdatetime(m_case_time->list[d.seq].f_preopout_dttm),cnvtdatetime(
    m_case_time->list[d.seq].f_preopin_dttm),4), pat_inrm_date = m_case_time->list[d.seq].
  f_patinrm_dttm"@SHORTDATE", pat_inrm_time = m_case_time->list[d.seq].f_patinrm_dttm"@TIMENOSECONDS",
  surgery_start_date = m_case_time->list[d.seq].s_surgstart_day, surgery_start_day = m_case_time->
  list[d.seq].f_surstart_dttm"@TIMENOSECONDS", surgery_stop_time = m_case_time->list[d.seq].
  f_surstop_dttm"@TIMENOSECONDS",
  pat_outrm_time = m_case_time->list[d.seq].f_patoutrm_dttm"@TIMENOSECONDS", orhold_start_time =
  m_case_time->list[d.seq].f_orholdin_dttm"@TIMENOSECONDS", orhold_end_time = m_case_time->list[d.seq
  ].f_orholdout_dttm"@TIMENOSECONDS",
  surgery_totalmins = datetimediff(cnvtdatetime(m_case_time->list[d.seq].f_surstop_dttm),cnvtdatetime
   (m_case_time->list[d.seq].f_surstart_dttm),4), orroom_totalmins = datetimediff(cnvtdatetime(
    m_case_time->list[d.seq].f_patoutrm_dttm),cnvtdatetime(m_case_time->list[d.seq].f_patinrm_dttm),4
   ), orhold_totalmins = datetimediff(cnvtdatetime(m_case_time->list[d.seq].f_orholdout_dttm),
   cnvtdatetime(m_case_time->list[d.seq].f_orholdin_dttm),4),
  pacu_in_date = m_case_time->list[d.seq].f_pacu1in_dttm"@SHORTDATE", pacu1_in_time = m_case_time->
  list[d.seq].f_pacu1in_dttm"@TIMENOSECONDS", pacu1_out_time = m_case_time->list[d.seq].
  f_pacu1out_dttm"@TIMENOSECONDS",
  pacu1_totmins = datetimediff(cnvtdatetime(m_case_time->list[d.seq].f_pacu1out_dttm),cnvtdatetime(
    m_case_time->list[d.seq].f_pacu1in_dttm),4), pacu2_in_time = m_case_time->list[d.seq].
  f_pacu2in_dttm"@TIMENOSECONDS", pacu2_out_time = m_case_time->list[d.seq].f_pacu2out_dttm
  "@TIMENOSECONDS",
  pacu2_totmins = datetimediff(cnvtdatetime(m_case_time->list[d.seq].f_pacu2out_dttm),cnvtdatetime(
    m_case_time->list[d.seq].f_pacu2in_dttm),4), pacu3_in_time = m_case_time->list[d.seq].
  f_pacu3in_dttm"@TIMENOSECONDS", pacu3_out_time = m_case_time->list[d.seq].f_pacu3out_dttm
  "@TIMENOSECONDS",
  pacu3_totmins = datetimediff(cnvtdatetime(m_case_time->list[d.seq].f_pacu3out_dttm),cnvtdatetime(
    m_case_time->list[d.seq].f_pacu3in_dttm),4), pacu_totmin = ((datetimediff(cnvtdatetime(
    m_case_time->list[d.seq].f_pacu1out_dttm),cnvtdatetime(m_case_time->list[d.seq].f_pacu1in_dttm),4
   )+ datetimediff(cnvtdatetime(m_case_time->list[d.seq].f_pacu2out_dttm),cnvtdatetime(m_case_time->
    list[d.seq].f_pacu2in_dttm),4))+ datetimediff(cnvtdatetime(m_case_time->list[d.seq].
    f_pacu3out_dttm),cnvtdatetime(m_case_time->list[d.seq].f_pacu3in_dttm),4)), sch_proc_code =
  m_case_time->list[d.seq].f_proccode,
  sch_proce_name = m_case_time->list[d.seq].s_schproc_name, sch_proc_type = m_case_time->list[d.seq].
  ...
  preform_proc_surgeon = m_case_time->list[d.seq].s_procsurgeon, proc_start = m_case_time->list[d.seq
  ].f_procstart_dttm"@TIMENOSECONDS", proc_end = m_case_time->list[d.seq].f_procend_dttm
  "@TIMENOSECONDS",
  procdur_mins = m_case_time->list[d.seq].l_procdur_mins, cancal_date = m_case_time->list[d.seq].
  f_cancel_dttm"@SHORTDATETIME", cancal_reason = m_case_time->list[d.seq].s_cancel_res
  FROM (dummyt d  WITH seq = m_case_time->l_cnt)
  ORDER BY surgical_area, date_of_surgery, sched_time_tm
