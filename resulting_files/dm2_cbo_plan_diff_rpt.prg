CREATE PROGRAM dm2_cbo_plan_diff_rpt
 ...
 SUBROUTINE dcr_get_instance_info(dgii_instance_name,dgii_instance_nbr)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_plndifrpt",".log","dm2_cbo_plan_diff_rpt")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating input parameters."
 DECLARE dcpdr_envid = f8 WITH protect, noconstant( $1)
 ...
 DECLARE dcpdr_instance_name = vc WITH protect, noconstant("")
 DECLARE dcpdr_instance_nbr = i4 WITH protect, noconstant(0)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->user_action = "Please verify correct number and format of input parameters."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting Dm2_cbo_plan_diff_rpt"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF ((dm2_rdbms_version->level1 < 11))
  SET dm_err->eproc = "Dm2_cbo_plan_diff_rpt used only for Oracle 11 and above. Exiting."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (dcpdr_fname="MINE")
  SET message = window
  CALL clear(1,1)
  CALL text(4,2,"Display plan data since:")
  CALL accept(4,28,"99DAAAD9999D99D99D99;CU",format(cnvtdatetime((curdate - 1),curtime3),
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Obtain Environment name."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_environment d
  WHERE d.environment_id=dcpdr_envid
  DETAIL
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = concat("Environment_ID (",trim(cnvtstring(dcpdr_envid)),
   ") is INVALID. No entry found in DM_ENVIRONMENT.")
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 ...
 CALL dcr_add_break_str(notrim(" "),0)
 SET dm_err->eproc = concat("Obtain Sql_Ids for reporting.")
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  ...
  (SELECT
   "x"
   FROM dm_sqlplan_hist i
   WHERE d.sql_id=i.sql_id
    AND d.environment_id=i.environment_id
    AND d.plan_hash_value != i.plan_hash_value
    AND i.last_load_dt_tm > cnvtdatetime(dcpdr_exclude_dt)))
   AND d.last_load_dt_tm > cnvtdatetime(dcpdr_exclude_dt)
   AND d.owner_name=trim(cnvtupper(currdbuser))
   AND d.environment_id=dcpdr_envid
  ORDER BY d.sql_id
  HEAD d.sql_id
   CALL dcr_add_sql_id(d.sql_id,dcpdr_envid,0),
   CALL dcr_add_plan_data(d.sql_id,dcpdr_envid,d.plan_hash_value,d.first_load_dt_tm,d.last_load_dt_tm,
   d.first_gets_exec,d.last_gets_exec,1), dcpdr_cur_plan_cnt = 0
  DETAIL
   CALL dcr_add_plan_data(d.sql_id,d.environment_id,d.plan_hash_value,d.first_load_dt_tm,d
   .last_load_dt_tm,d.first_gets_exec,d.last_gets_exec,0),
   CALL dcr_add_child_number(d.sql_id,d.plan_hash_value,- (1.0),dcpdr_envid), dcpdr_cur_plan_cnt = (
   dcpdr_cur_plan_cnt+ 1)
  FOOT  d.sql_id
 ...
 ENDIF
 SET dm_err->eproc = "Obtain SQL Text for Sql_Ids."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM v$sql v,
  ...
  HEAD v.sql_id
   ...
   CALL dcr_add_sql_txt("",dcpdr_envid,"",1),
   CALL dcr_split_text(null)
   FOR (dcpdr_piece_cnt = 1 TO dcrstrrec->orig_str[1].piece_cnt)
     CALL dcr_add_sql_txt(v.sql_id,dcpdr_envid,dcrstrrec->orig_str[1].piece[dcpdr_piece_cnt].str,0)
   ENDFOR
  HEAD v.plan_hash_value
   CALL dcr_add_child_number(v.sql_id,v.plan_hash_value,v.child_number,dcpdr_envid)
  WITH nocounter
 ;end select
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dcr_plan_data)
 ...
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dcr_plan_data)
 ENDIF
 IF (dcr_get_instance_info(dcpdr_instance_name,dcpdr_instance_nbr)=0)
  GO TO exit_program
 ENDIF
 SELECT INTO value(dcpdr_fname)
  ...
  HEAD REPORT
   ...
   CALL print(concat("Instance Name    : ",dcpdr_instance_name)),
   row + 1,
   CALL print(concat("Instance Number  : ",cnvtstring(dcpdr_instance_nbr))), row + 1,
   CALL print(concat("Dates After:",dcpdr_exclude_dt_str)), row + 1, row + 1
  DETAIL
   IF ((dcr_plan_data->cnt=0))
   ...
   FOR (dcpdr_sql_cnt = 1 TO dcr_plan_data->cnt)
     ...
     CALL print(concat("SQL_ID: ",dcr_plan_data->qual[dcpdr_sql_cnt].sql_id)),
     row + 1, row + 1
     IF ((dcr_plan_data->qual[dcpdr_sql_cnt].sql_text_cnt=0))
      CALL print("NO SQL TEXT AVAILABLE"), row + 1
     ENDIF
     ...
     ENDFOR
     row + 1, col 0,
     CALL print("PLAN HASH VALUE"),
     col 20,
     CALL print("FIRST_LOAD_TIME"), col 40,
     CALL print("FIRST GETS/EXEC"), col 60,
     CALL print("LAST_LOAD_TIME"),
     col 80,
     CALL print("LAST GETS/EXEC"), row + 1,
     col 0,
     CALL print("---------------"), col 20,
     CALL print("---------------"), col 40,
     CALL print("---------------"),
     ...
     FOR (dcpdr_plan_cnt = 1 TO dcr_plan_data->qual[dcpdr_sql_cnt].plan_cnt)
       col 0,
       CALL print(trim(cnvtstring(dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].
         plan_hash_value,17))), col 20,
       CALL print(format(cnvtdatetime(dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].
         first_load_dt_tm),"DD-MMM-YY HH:MM:SS;;D")), col 40,
       CALL print(trim(cnvtstring(dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].
         first_gets_exec,17))),
       col 60,
       CALL print(format(cnvtdatetime(dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].
         last_load_dt_tm),"DD-MMM-YY HH:MM:SS;;D")), col 80,
       CALL print(trim(cnvtstring(dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].
         last_gets_exec,17))), row + 1
     ENDFOR
     FOR (dcpdr_plan_cnt = 1 TO dcr_plan_data->qual[dcpdr_sql_cnt].plan_cnt)
       IF ((((dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].plan_text_cnt=0)) OR ((((
       dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].plan_text[1].txt=
       "NOTE: cannot fetch plan*")) OR ((dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].
       plan_text[1].txt="SQL_ID:*cannot be found*"))) )) )
        row + 1,
        CALL print(concat("PLAN HASH VALUE ",trim(cnvtstring(dcr_plan_data->qual[dcpdr_sql_cnt].
           plans[dcpdr_plan_cnt].plan_hash_value,17))," - NO PLAN AVAILABLE")), row + 1
       ELSE
        row + 1,
        CALL print(concat("PLAN HASH VALUE ",trim(cnvtstring(dcr_plan_data->qual[dcpdr_sql_cnt].
           plans[dcpdr_plan_cnt].plan_hash_value,17)))), row + 1
        FOR (dcpdr_plantxt_cnt = 1 TO dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].
        plan_text_cnt)
         CALL print(dcr_plan_data->qual[dcpdr_sql_cnt].plans[dcpdr_plan_cnt].plan_text[
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "Dm2_cbo_plan_diff_rpt completed"
 CALL final_disp_msg("dm2_plndifrpt")
END GO
