CREATE PROGRAM dm2_ref_data_mover_5836:dba
 ...
 DECLARE cust_temp_col_cnt = i4
 DECLARE cust_temp_entity_name = vc
 ...
 SET cust_temp_col_cnt = get_col_pos(cust_tab_name,"PARENT_ENTITY_ID")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_temp_col_cnt].translated = 0
 SET cust_temp_entity_name = get_value(cust_tab_name,"PARENT_ENTITY_NAME","FROM")
 SET cust_from_value = get_value(cust_tab_name,"PARENT_ENTITY_ID","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_5836
 ENDIF
 CASE (cust_temp_entity_name)
  OF "HEALTHPLAN":
   SET cust_item = "HEALTH_PLAN"
  OF patstring("CODESET*",0):
   SET cust_item = "CODE_VALUE"
  OF "DEFAULT":
   SET cust_item = "CODE_VALUE"
  OF "PAYER":
   SET cust_item = "ORGANIZATION"
  OF patstring("HCFA_*",0):
   SET cust_item = "CODE_VALUE"
  ELSE
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = concat("ERROR: the entity name of ",cust_temp_entity_name,
    " is not supported at this time")
   GO TO exit_5836
 ENDCASE
 IF (cnvtreal(cust_from_value) > 0)
  SET cust_new_value = select_merge_translate(cust_from_value,cust_item)
  IF (cust_new_value != "No Trans")
   CALL put_value(cust_tab_name,"PARENT_ENTITY_ID",cust_new_value)
  ENDIF
 ELSE
  CALL put_value(cust_tab_name,"PARENT_ENTITY_ID",cust_from_value)
 ENDIF
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_5836
 ENDIF
