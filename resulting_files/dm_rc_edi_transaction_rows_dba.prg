CREATE PROGRAM dm_rc_edi_transaction_rows:dba
 ...
 SET reply->table_name = "RC_EDI_TRANSACTION"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(rcet.rc_edi_transaction_id)
   FROM rc_edi_transaction rcet
   WHERE rcet.rc_edi_transaction_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(rcet.rc_edi_transaction_id)
  FROM rc_edi_transaction rcet
  WHERE rcet.rc_edi_transaction_id > 0.0
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM rc_edi_transaction rcet
    WHERE rcet.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND rcet.rc_edi_transaction_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("rcet.rc_edi_transaction_id != 0","rcet"))
    DETAIL
     row_cnt = (row_cnt+ 1)
     IF (mod(row_cnt,100)=1)
      stat = alterlist(reply->rows,(row_cnt+ 99))
     ENDIF
     reply->rows[row_cnt].row_id = rcet.rowid
    WITH nocounter, maxqual(rcet,value(rowsleft))
   ;end select
