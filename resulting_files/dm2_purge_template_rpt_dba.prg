CREATE PROGRAM dm2_purge_template_rpt:dba
 ...
 SUBROUTINE drr_chk_active_runners(dcar_dblink,dcar_count_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dptr_plan_id = f8 WITH protect, noconstant(0.0)
 DECLARE dptr_report_destination = vc WITH protect, noconstant("")
 DECLARE dptr_xnt_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_purge_temp_rpt",".log","DM2_PURGE_TEMPLATE_RPT LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning DM2_PURGE_TEMPLATE_RPT"
 CALL disp_msg("",dm_err->logfile,0)
 SET dptr_plan_id = cnvtreal( $1)
 IF ((dm2_install_schema->logfile_prefix != "NONE"))
  SET dptr_report_destination = build(dm2_install_schema->logfile_prefix,"pt.rpt")
 ELSE
  IF (dptr_plan_id < 0)
   SET dcvr_rpt_prefix = build("dm2ob",format(abs(dptr_plan_id),"######;p0"),"_")
  ELSE
   SET dcvr_rpt_prefix = build("dm2os",format(dptr_plan_id,"######;p0"),"_")
  ENDIF
  IF (get_unique_file(dcvr_rpt_prefix,"pt.rpt")=0)
   GO TO exit_script
  ENDIF
  SET dptr_report_destination = dm_err->unique_fname
 ENDIF
 SET dm_err->eproc = "Determining if there are XNT template job set up."
 SELECT INTO "nl:"
 ...
 IF (curqual > 0)
  SET dptr_xnt_ind = 1
  IF ((dm_err->debug_flag > 2))
   CALL echo(build("dptr_xnt_ind = ",dptr_xnt_ind))
  ENDIF
 ENDIF
 SET dm_err->eproc = "Determining if there are purge templates to report on from ocd_install_log."
 SELECT
  IF (dptr_xnt_ind=0)
   WHERE oil.ocd=dptr_plan_id
    AND oil.component_type="PURGE TEMPLATE"
    AND  NOT ( EXISTS (
   (SELECT
    "x"
    FROM dm_purge_template dpt
    WHERE oil.end_state=cnvtstring(dpt.template_nbr)
     AND dpt.program_str="XNT*")))
  ELSE
  ENDIF
  INTO "nl:"
  FROM ocd_install_log oil
  WHERE oil.ocd=dptr_plan_id
   AND oil.component_type="PURGE TEMPLATE"
  WITH nocounter, maxqual(oil,1)
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "No new purge templates to report."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc =
 "Selecting purge templates installed for given plan id from ocd_install_log for report."
 SELECT
  IF (dptr_xnt_ind=0)
   WHERE oil.ocd=dptr_plan_id
    AND oil.component_type="PURGE TEMPLATE"
   ...
   (SELECT
    ...
    WHERE oil.end_state=cnvtstring(dpt.template_nbr)
     AND dpt.program_str="XNT*")))
   ORDER BY oil.install_text
  ELSE
  ENDIF
  INTO build(dptr_report_destination)
  FROM ocd_install_log oil
  WHERE oil.ocd=dptr_plan_id
   AND oil.component_type="PURGE TEMPLATE"
  ORDER BY oil.install_text
  HEAD REPORT
   row + 0, col 0, "Purge Template Installation Report",
   row + 2, col 0,
   CALL print(concat("The following Purge Templates are NEW or CHANGED as part of installing",
    evaluate(dptr_plan_id,abs(dptr_plan_id)," Package "," Install Plan "),trim(format(abs(
       dptr_plan_id),";;I"),3),".")),
   row + 1, col 0,
   "Please use DM Purge Job Manager to review the settings for each of these Purge Templates.",
   row + 2, col 0, "NEW/CHANGED",
   col 15, "NAME", col 86,
   "TEMPLATE NUMBER", row + 1, col 0,
   CALL print(fillstring(101,"-"))
  DETAIL
   row + 1, col 0,
   CALL print(evaluate(oil.update_ind,0,"NEW",1,"CHANGED")),
   col 15,
   CALL print(substring(1,70,oil.install_text)), col 86,
   CALL print(build(oil.end_state))
  FOOT REPORT
   row + 3,
   CALL center("*** END OF REPORT ***",1,101)
  WITH nocounter, nullreport, maxcol = 132,
   format = variable, formfeed = none, maxrow = 3
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dir_ui_misc->dm_process_event_id > 0))
  IF (drr_assign_file_to_installs("RPT:PURGE TEMPLATES INSTALLED",dptr_report_destination,dir_ui_misc
   ->dm_process_event_id)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (dir_silmode_requested_ind=1)
  CALL dir_add_silmode_entry("Purge Template Report",dptr_report_destination)
  SET dm_err->eproc = concat("Silent Mode:  Purge Template Report was bypassed.  Report exixts",
   " in CCLUSERDIR under file name: ",dptr_report_destination)
  CALL disp_msg("",dm_err->logfile,0)
 ELSE
  IF (dm2_disp_file(dptr_report_destination,"")=0)
   GO TO exit_script
  ENDIF
 ENDIF
#exit_script
 SET dm_err->eproc = "Ending DM2_PURGE_TEMPLATE_RPT."
 CALL final_disp_msg("dm2_purge_temp_rpt")
END GO
