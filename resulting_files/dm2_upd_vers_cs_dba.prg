CREATE PROGRAM dm2_upd_vers_cs:dba
 ...
 IF (check_logfile("dm2_upd_verscs_",".log","dm2_upd_verscs LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_upd_vers_cs..."
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF (merge_check_cnt > 0)
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (dc
    WHERE (dc.cki=requestin->list_0[d.seq].cki)
     AND (((dc.cdf_meaning != requestin->list_0[d.seq].cdf_meaning)) OR (((dc.cdf_meaning = null
     AND (requestin->list_0[d.seq].cdf_meaning > " ")) OR (((dc.cdf_meaning IS NOT null
     AND trim(requestin->list_0[d.seq].cdf_meaning)="") OR ((((dc.definition != requestin->list_0[d
    .seq].definition)) OR (((dc.definition = null
     AND (requestin->list_0[d.seq].definition > " ")) OR (((dc.definition IS NOT null
     AND trim(requestin->list_0[d.seq].definition)="") OR (dc.active_ind != cnvtint(requestin->
     list_0[d.seq].active_ind))) )) )) )) )) )) )
   DETAIL
    dim_cnt = (dim_cnt+ 1), rs_exist->qual[d.seq].is_there = 1
   WITH nocounter
  ;end select
  SET dm_err->eproc = "Updating code_value..."
  CALL disp_msg("",dm_err->logfile,0)
  SET readme_data->status = "F"
  SET readme_data->message = "Updating code_value..."
  UPDATE  FROM code_value dc,
    (dummyt d  WITH seq = merge_check_cnt)
   SET dc.cdf_meaning = requestin->list_0[d.seq].cdf_meaning, dc.active_ind = cnvtint(requestin->
     list_0[d.seq].active_ind), dc.definition = requestin->list_0[d.seq].definition,
 ...
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading code_value with info in dm2_upd_vers_cs.csv."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  ...
  GO TO exit_script
 ELSE
  SET dm_err->eproc = "Finished update of code_value"
 ...
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = "Check failed. Rows in request != rows in local code_value."
 ENDIF
#exit_script
 IF ((readme_data->status="S"))
  SET readme_data->message = "code_value has been updated successfully."
  COMMIT
 ELSE
  ROLLBACK
 ENDIF
