CREATE PROGRAM bhs_rpt_lab_in_office:dba
 PROMPT
 ...
 SUBROUTINE req_orders_headabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_helvetica12b0)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("SPECIMEN COLLECTION",char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
 ...
 SUBROUTINE req_orders_providerabs(ncalc,offsetx,offsety)
   ...
   DECLARE __order_provider = vc WITH noconstant(build2(m_info->labs[ml_loop].s_ord_provider,char(0))
    ), protect
 ...
 SUBROUTINE foot_sectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
      "PLEASE BRING THIS FORM TO THE LAB.  SPECIMEN COLLECTION TESTING REQUIRES A PHYSICIANS ORDER AND SIGNATURE TO BE PERFORMED",
      char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_RPT_LAB_IN_OFFICE"
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   WHERE o.encntr_id=mf_encntr_id
    ...
    AND o.activity_type_cd IN (mf_lab_inoff_op_cd, mf_micro_op_cd, mf_combo_lab_op_cd,
   mf_pathology_op_cd)
    AND o.active_ind=1
  ...
  DETAIL
   ...
   IF (od.oe_field_id IN (mf_lab_req_cd, mf_op_lab_cd)
   ...
   ENDIF
  FOOT REPORT
   CALL echo(m_info->labs[pl_cnt].s_lab_det), stat = alterlist(m_info->labs,pl_cnt)
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1
   ...
   WHERE n.nomenclature_id=d.nomenclature_id
    AND n.active_ind=1)
  DETAIL
   CALL echo(n.concept_cki), m_info->labs[d1.seq].icd9s[d2.seq].s_cd = trim(n.source_identifier)
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   m_info->labs_by_time[pl_cnt].f_act_type_cd = m_info->labs[d.seq].f_act_type_cd
  WITH nocounter
 ;end select
 ...
 FOR (ml_loop = 1 TO size(m_info->labs_by_time,5))
  ...
  IF ((m_info->labs_by_time[ml_loop].f_act_type_cd=mf_lab_inoff_op_cd))
   IF (ml_loop=1)
   ...
   IF (trim(m_info->labs[ml_idx].s_req_type) != trim(ms_tmp)
    AND ml_loop != 1)
    CALL echo(build(ml_idx))
    CALL echo("break page1")
   ...
   ELSEIF (mf_rem_space < 0.25)
    ...
    SET d0 = req_orders_head(rpt_render)
    SET d0 = req_orders_type(rpt_render)
   ELSEIF ((mf_rem_space < (0.32+ req_orders_provider(rpt_calcheight))))
    SET d0 = foot_section(rpt_render)
    SET d0 = pagebreak(0)
    SET d0 = page_head_section(rpt_render)
    SET d0 = req_orders_head(rpt_render)
   ...
   WHILE (ml_cont_ind=1)
     SET d0 = foot_section(rpt_render)
     CALL echo("break page3")
     SET d0 = pagebreak(0)
     SET d0 = page_head_section(rpt_render)
     SET d0 = req_orders_head(rpt_render)
     SET d0 = req_orders_type(rpt_render)
     SET mf_rem_space = (mf_page_size - (_yoffset+ foot_section(rpt_calcheight)))
     CALL echo(mf_rem_space)
     SET d0 = req_orders_det(rpt_render,mf_rem_space,ml_cont_ind)
   ENDWHILE
   SET mf_rem_space = (mf_page_size - (_yoffset+ foot_section(rpt_calcheight)))
   IF (mf_rem_space < req_orders_provider(rpt_calcheight))
    SET d0 = foot_section(rpt_render)
    CALL echo("break page4")
    SET d0 = pagebreak(0)
    SET d0 = page_head_section(rpt_render)
    SET d0 = req_orders_head(rpt_render)
    SET d0 = req_orders_type(rpt_render)
   ...
   IF (ml_loop=size(m_info->labs,5))
    SET d0 = foot_section(rpt_render)
   ENDIF
  ELSEIF ((m_info->labs_by_time[ml_loop].f_act_type_cd IN (mf_micro_op_cd, mf_pathology_op_cd)))
   SET mf_rem_space = (mf_page_size - (_yoffset+ foot_section(rpt_calcheight)))
   IF (((mf_rem_space < req_orders_provider(rpt_calcheight)) OR (ml_loop > 1)) )
    CALL sbr_pagebreak(0)
   ENDIF
   SET d0 = req_orders_type(rpt_render)
   SET d0 = req_orders_det(rpt_render,mf_rem_space,ml_cont_ind)
   SET d0 = req_orders_provider(rpt_render)
  ELSEIF ((m_info->labs_by_time[ml_loop].f_act_type_cd=mf_combo_lab_op_cd))
   SET mf_rem_space = (mf_page_size - (_yoffset+ foot_section(rpt_calcheight)))
   SET d0 = req_orders_type(rpt_render)
   SET d0 = req_orders_det(rpt_render,mf_rem_space,ml_cont_ind)
   SET d0 = req_orders_provider(rpt_render)
   IF (mf_rem_space < 0.25)
    ...
    SET d0 = req_orders_head(rpt_render)
   ENDIF
   IF ((mf_rem_space < (0.32+ req_orders_provider(rpt_calcheight))))
    SET d0 = foot_section(rpt_render)
    SET d0 = pagebreak(0)
    SET d0 = page_head_section(rpt_render)
    SET d0 = req_orders_head(rpt_render)
