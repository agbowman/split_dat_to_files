CREATE PROGRAM dm_mmc_case_rows:dba
 ...
 FOR (tok_ndx = 1 TO size(request->tokens,5))
   IF (trim(cnvtupper(request->tokens[tok_ndx].token_str),3)="YEARSOFCASEDATA")
    SET v_years = cnvtreal(request->tokens[tok_ndx].value)
   ENDIF
 ENDFOR
 IF ((v_years=- (1)))
  SET reply->err_code = - (1)
  SET reply->err_msg = build("The token, YEARSOFCASEDATA was not found!")
  GO TO exit_script
 ELSEIF (v_years < 2)
  SET reply->err_code = - (1)
  SET reply->err_msg = build("You must keep a minimum of two years worth of data. You entered ",
   nullterm(cnvtstring(v_years),3)," years to keep.")
  GO TO exit_script
 ENDIF
 SET reply->table_name = "MMC_CASE"
