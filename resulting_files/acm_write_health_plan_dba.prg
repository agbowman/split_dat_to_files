CREATE PROGRAM acm_write_health_plan:dba
 ...
 DECLARE ubound = i4 WITH protect, noconstant(reply->health_plan_qual_cnt)
 SET stat = alterlist(reply->health_plan_qual,reply->health_plan_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->health_plan_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->health_plan_qual[index].health_plan_id = acm_request->health_plan_qual[index].
     health_plan_id
     SET reply->health_plan_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_health_plan
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->act_cnt > 0))
  EXECUTE acm_act_health_plan
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_health_plan
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_health_plan
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
