CREATE PROGRAM dm_rmc_mover_7828:dba
 ...
 DECLARE cust_from_group_id = vc
 DECLARE cust_to_group_id = f8
 DECLARE cust_from_action_id = f8
 DECLARE cust_to_action_id = vc
 DECLARE cust_par_tab_name = vc
 DECLARE cust_rpt_val = vc
 DECLARE cust_cnt = i4
 DECLARE cust_nomv_chk = vc WITH protect, noconstant("")
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET temp_col_cnt = get_col_pos(cust_tab_name,"SA_REF_GROUP_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  SET cust_from_group_id = get_value(cust_tab_name,"SA_REF_GROUP_ID","FROM")
  SET cust_nomv_chk = select_merge_translate(cnvtstring(cust_from_group_id),cust_tab_name)
  IF (cust_nomv_chk="NOMV21")
   ...
   GO TO exit_7828
  ENDIF
  SET cust_par_tab_name = dm2_get_rdds_tname("SA_REF_ACTION_GROUP_R")
  SELECT INTO "NL:"
   FROM (parser(cust_par_tab_name) s)
   WHERE s.sa_ref_group_id=cnvtreal(cust_from_group_id)
   DETAIL
    cust_from_action_id = s.sa_ref_action_id
   WITH nocounter
  ;end select
  SET cust_to_action_id = select_merge_translate(cnvtstring(cust_from_action_id),"SA_REF_ACTION")
  IF (isnumeric(cust_to_action_id))
   SET cust_cnt = 0
   SELECT INTO "NL:"
    FROM sa_ref_action_group_r sa,
     sa_ref_group sg
    PLAN (sg
     WHERE (sg.group_prompt=rs_7828->to_values.group_prompt)
      AND (sg.multi_select_ind=rs_7828->to_values.multi_select_ind)
      AND (sg.selection_required_ind=rs_7828->to_values.selection_required_ind))
     JOIN (sa
     WHERE sa.sa_ref_group_id=sg.sa_ref_group_id
      AND sa.sa_ref_action_id=cnvtreal(cust_to_action_id))
    DETAIL
     cust_cnt = (cust_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,cust_cnt), ui_query_eval_rec
     ->qual[cust_cnt].root_entity_attr = sa.sa_ref_group_id
    WITH nocounter
   ;end select
   SET cust_to_group_id = evaluate_exec_ui_query(cust_cnt,temp_tbl_cnt,perm_col_cnt)
   IF (cust_to_group_id < 0)
    IF ((cust_to_group_id=- (3)))
     SET cust_to_group_id = get_seq(cust_tab_name,"SA_REF_GROUP_ID")
     IF (cust_to_group_id > 0)
      CALL put_value(cust_tab_name,"SA_REF_GROUP_ID",cnvtstring(cust_to_group_id))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"SA_REF_GROUP_ID")
     ELSE
      GO TO exit_7828
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"SA_REF_GROUP_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_7828
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"SA_REF_GROUP_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_7828
   ELSE
  ...
  ELSE
   SET cust_rpt_val = report_missing("SA_REF_ACTION","SA_REF_ACTION_ID",cust_from_action_id)
   CALL evaluate_rpt_missing(cust_rpt_val)
  ENDIF
 ENDIF
