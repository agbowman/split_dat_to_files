CREATE PROGRAM dm2_capture_source:dba
 ...
 SUBROUTINE drr_del_preserved_ts(dcdl_tgt_db_name)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_capture_source",".log","dm2_capture_source")=false)
  GO TO exit_program
 ENDIF
 DECLARE dcs_schema_str = vc WITH protect, noconstant( $1)
 DECLARE dcs_data = vc WITH protect, noconstant(cnvtupper( $2))
 DECLARE dcs_pct = i4 WITH protect, noconstant( $3)
 DECLARE dsc_str = vc WITH protect, noconstant(" ")
 SET dm_err->eproc = "Validate schema date format"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (dm2_val_sch_date_str(dcs_schema_str)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validate data to move"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF ( NOT (dcs_data IN ("REF", "ALL")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Data to Move must be either REF or ALL"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validate percentage"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF ( NOT (dcs_pct BETWEEN 0 AND 100)
  AND (drr_clin_copy_data->process != "MIGRATION"))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Percent must be between 0 and 100"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dm2_install_schema->process_option="NONE"))
  SET dm2_install_schema->process_option = "CLIN COPY"
  SET dm2_install_schema->data_to_move = dcs_data
  SET dm2_install_schema->percent_tspace = evaluate(dcs_pct,0,10,dcs_pct)
 ELSE
  IF (validate(drrr_responsefile_in_use,- (1))=1)
   EXECUTE dm2_create_database_link "REPL_SOURCE", drrr_rf_data->src_db_link_cnct_desc, drrr_rf_data
   ->src_db_user,
  ...
  IF ((dm2_rdbms_version->level1 >= 11)
   AND (drr_clin_copy_data->process != "MIGRATION"))
   SET dm_err->eproc = "Exporting SOURCE Statistics to DM_STAT_TABLE."
   CALL disp_msg("",dm_err->logfile,0)
   IF ((validate(dm2_rep_no_stats,- (1))=- (1)))
    IF (dm2_push_cmd(concat(
      ^RDB ASIS("BEGIN dbms_stats.delete_schema_stats(ownname=>'V500',stattab=>'DM_STAT_TABLE',statid=>'^,
      trim(cnvtupper(currdbname)),^STATS',statown=>'V500',force=>TRUE); END;") go^),1)=0)
     GO TO exit_program
    ENDIF
    IF (dm2_push_cmd(concat(
      ^RDB ASIS("BEGIN dbms_stats.export_schema_stats(ownname=>'V500',stattab=>'DM_STAT_TABLE',statid=>'^,
      trim(cnvtupper(currdbname)),^STATS',statown=>'V500'); END;") go^),1)=0)
     GO TO exit_program
    ENDIF
   ENDIF
  ENDIF
 ...
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(dm2_install_schema)
 ENDIF
 EXECUTE dm2_capture_schema dcs_schema_str, "Capturing SOURCE schema for replicate", "*"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 IF ((drr_clin_copy_data->ads_chosen_ind=1))
  EXECUTE dm2_capture_tspace dcs_schema_str, "ADS"
 ELSE
  EXECUTE dm2_capture_tspace dcs_schema_str, dm2_install_schema->data_to_move
 ENDIF
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM2_CAPTURE_SOURCE completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_capture_source")
END GO
