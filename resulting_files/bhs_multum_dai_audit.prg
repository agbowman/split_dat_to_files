CREATE PROGRAM bhs_multum_dai_audit
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ede
   WHERE trim(ede.dlg_name)="MUL_MED!DRUGALLERGY"
    AND ede.updt_dt_tm BETWEEN cnvtdatetime(beg_date_qual,0) AND cnvtdatetime(end_date_qual,235959))
   JOIN (edea
  ...
  DETAIL
   ...
   IF (report_type="2")
    ...
    output_line = build(',"',eks->list[d.seq].trigger.order_mnemonic,'","',eks->list[d.seq].
     ...
     '","',facility_disp,'","',eks->list[d.seq].trigger.physician_name,'","',
 ...
 ;end select
 IF (email_ind=1)
  CALL echo(report_type)
  SET filename_in = concat(trim(output_dest),".dat")
  SET filename_out = concat(format(curdate,"MMDDYYYY;;D"),".csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog," - B M C Drug Allergy Interaction Rpt ",beg_date_disp," to ",
   end_date_disp)
  IF (report_type="1")
