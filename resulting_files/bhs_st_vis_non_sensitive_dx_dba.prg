CREATE PROGRAM bhs_st_vis_non_sensitive_dx:dba
 DECLARE mf_cs12033_sensitive = f8 WITH constant(uar_get_code_by("DISPLAYKEY",12033,"SENSITIVE")),
 protect
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dx
   WHERE (dx.encntr_id=request->visit[1].encntr_id)
    ...
    AND dx.active_ind=1
    AND dx.end_effective_dt_tm > sysdate
    AND dx.classification_cd != mf_cs12033_sensitive)
   JOIN (n
