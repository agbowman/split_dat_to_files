CREATE PROGRAM dm_oucb_patient_event_detail:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "dm_oucb_patient_event_detail"
 ...
 IF ((rchildren->qual1[det_cnt].combine_action_cd=del))
  IF ("PATIENT_EVENT"=trim(rchildren->qual1[det_cnt].attribute_name))
 ...
 SUBROUTINE cust_ucb_del_event(dummy)
   ...
   IF (parent_entity_id > 0)
    ...
    IF (add_event_hist(rchildren->qual1[det_cnt].entity_pk[1].data_number)=0)
     ...
     SET error_table = rchildren->qual1[det_cnt].entity_name
     SET request->error_message = substring(1,132,build(
       "Could not insert history for patient_event with pk val=",rchildren->qual1[det_cnt].entity_pk[
       1].data_number))
     RETURN(0)
    ENDIF
    RETURN(1)
 ...
 SUBROUTINE cust_ucb_del_detail(dummy)
   ...
   ;end select
   IF (parent_entity_id > 0)
    UPDATE  FROM patient_event_detail ped
    ...
    IF (add_detail_hist(rchildren->qual1[det_cnt].entity_pk[1].data_number)=0)
     ...
     SET error_table = rchildren->qual1[det_cnt].entity_name
     SET request->error_message = substring(1,132,build(
       "Could not insert history for patient_event_detail with pk val=",rchildren->qual1[det_cnt].
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0)
    ...
    SET dcipht_request->transaction_reason_txt = "DM_OUCB_PATIENT_EVENT_DETAIL"
