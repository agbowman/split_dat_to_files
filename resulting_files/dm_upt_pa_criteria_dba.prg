CREATE PROGRAM dm_upt_pa_criteria:dba
 ...
 IF ((request->search_process_id != 0))
  ...
  UPDATE  FROM pa_criteria
   SET description = trim(cnvtupper(request->description)), updt_cnt = (updt_cnt+ 1), updt_dt_tm =
   ...
   WHERE (search_process_id=request->search_process_id)
    AND key1=trim(cnvtupper(request->key1))
    AND key2=trim(cnvtupper(request->key2))
    AND key3=trim(cnvtupper(request->key3))
    AND key4=trim(cnvtupper(request->key4))
   WITH nocounter
  ;end update
  IF (curqual=0)
