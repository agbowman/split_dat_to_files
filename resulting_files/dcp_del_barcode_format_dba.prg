CREATE PROGRAM dcp_del_barcode_format:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 FOR (x = 1 TO value(numbertochange))
  DELETE  FROM org_barcode_format obf
   WHERE (obf.org_barcode_format_id=request->barcodeformat[x].org_barcode_format_id)
   WITH nocounter
  ;end delete
  IF (curqual=0)
   SET failed = "T"
