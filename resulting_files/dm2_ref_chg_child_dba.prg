CREATE PROGRAM dm2_ref_chg_child:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dgnp_numcnt = i4 WITH protect, noconstant(0)
 DECLARE num_actind = i2 WITH protect, noconstant(0)
 DECLARE num_size = i4 WITH protect, constant(30000)
 DECLARE dgnp_cmd_cur = i4 WITH protect, noconstant(0)
 DECLARE child_log_type = c6 WITH protect, constant("REFCHG")
 SET num_cnt =  $1
 FREE RECORD num_par
 RECORD num_par(
   1 list[*]
 ...
 CALL build_cmd("select if(dguc_act->dtd_hold[num_cnt].merge_active_ind = 1)")
 CALL build_cmd("  where td.active_ind = 1  ")
 CALL build_cmd(build("  and td.",dguc_act->dtd_hold[num_cnt].pk_hold[1].pk_name," > ",qry_pre))
 CALL drcc_special_table(num_cnt)
 CALL build_cmd("else ")
 CALL build_cmd(build("  where td.",dguc_act->dtd_hold[num_cnt].pk_hold[1].pk_name," > ",qry_pre))
 CALL drcc_special_table(num_cnt)
 CALL build_cmd("endif")
 IF ((dguc_act->dtd_hold[num_cnt].distinct_ind=0))
  CALL build_cmd(concat("into 'nl:' from ",dguc_act->dtd_hold[num_cnt].tbl_name," td"))
 ELSE
  CALL build_cmd(concat("distinct into 'nl:' td.",dguc_act->dtd_hold[num_cnt].pk_hold[1].pk_name))
  CALL build_cmd(concat("from ",dguc_act->dtd_hold[num_cnt].tbl_name," td"))
 ENDIF
 CALL build_cmd(concat("order by td.",dguc_act->dtd_hold[num_cnt].pk_hold[1].pk_name))
 CALL build_cmd("detail dgnp_numcnt = dgnp_numcnt+1")
 CALL build_cmd("if(mod(dgnp_numcnt, 1000) = 1)")
 CALL build_cmd("stat = alterlist(num_hold->list, dgnp_numcnt + 999) endif")
 CALL build_cmd("num_hold->list[dgnp_numcnt].pk_value = 'WHERE '")
 FOR (ucc = 1 TO dguc_act->dtd_hold[num_cnt].pk_cnt)
   CALL build_cmd(concat("if (nullind(td.",dguc_act->dtd_hold[num_cnt].pk_hold[ucc].pk_name,") = 1)")
    )
   CALL build_cmd(
    "num_hold->list[dgnp_numcnt].pk_value = concat(num_hold->list[dgnp_numcnt].pk_value,")
   IF (ucc > 1)
    CALL build_cmd(concat("' AND ",dguc_act->dtd_hold[num_cnt].pk_hold[ucc].pk_name," is NULL","')"))
   ELSE
    CALL build_cmd(concat("' ",dguc_act->dtd_hold[num_cnt].pk_hold[ucc].pk_name," is NULL","')"))
   ENDIF
   CALL build_cmd(
    "else num_hold->list[dgnp_numcnt].pk_value = build(num_hold->list[dgnp_numcnt].pk_value,")
   IF (ucc > 1)
    CALL build_cmd(concat(" ' AND ",dguc_act->dtd_hold[num_cnt].pk_hold[ucc].pk_name," = ',"))
    CALL build_cmd(concat("td.",dguc_act->dtd_hold[num_cnt].pk_hold[ucc].pk_name," ) endif "))
   ELSE
    CALL build_cmd(concat(" ' ",dguc_act->dtd_hold[num_cnt].pk_hold[ucc].pk_name," = ',"))
    CALL build_cmd(concat("td.",dguc_act->dtd_hold[num_cnt].pk_hold[ucc].pk_name," ) endif "))
   ENDIF
   IF ((dguc_act->dtd_hold[num_cnt].pk_hold[ucc].pk_name="ACTIVE_IND"))
    SET num_actind = 1
   ENDIF
 ENDFOR
 CALL build_cmd("if (dguc_act->dtd_hold[num_cnt].merge_active_ind = 1 and num_actind = 0)")
 CALL build_cmd("num_hold->list[dgnp_numcnt].pk_value = ")
 CALL build_cmd('concat(num_hold->list[dgnp_numcnt].pk_value," AND ACTIVE_IND = 1") endif')
 CALL build_cmd("foot report ")
 CALL build_cmd(concat("  num_hold->tbl_name = '",dguc_act->dtd_hold[num_cnt].tbl_name,"'"))
 CALL build_cmd(build("   qry_pre = td.",dguc_act->dtd_hold[num_cnt].pk_hold[1].pk_name))
 CALL build_cmd("   stat = alterlist(num_hold->list, dgnp_numcnt)")
 CALL build_cmd("   num_hold->pk_total = dgnp_numcnt ")
 CALL build_cmd(build("with nocounter, maxqual(td, ",num_size,") go"))
 CALL dgnp_exepar(num_cnt)
 CALL insert_dcl(null)
 IF ((num_hold->pk_total < num_size))
  SET sele_base = 1
 ENDIF
 FREE RECORD num_hold
 CALL echo(qry_pre)
 SUBROUTINE insert_dcl(null)
   DECLARE dgnp_finished = i4 WITH protect, noconstant(0)
   DECLARE dgnp_insert_cnt = i4 WITH protect, noconstant(0)
   DECLARE incre_size = i4 WITH protect, constant(3000)
   DECLARE dgnp_child_cnt = i4 WITH protect, noconstant(0)
   FOR (dgnp_env_cnt = 1 TO value(env_list->env_tol))
     SET dgnp_finished = 0
     SET dgnp_insert_cnt = 0
     WHILE (dgnp_finished < value(num_hold->pk_total))
       SET dgnp_finished = (dgnp_insert_cnt+ incre_size)
       INSERT  FROM dm_chg_log dcl,
         (dummyt d  WITH seq = value(dgnp_finished))
        SET dcl.log_id = seq(dm_clinical_seq,nextval), dcl.delete_ind = 0, dcl.target_env_id =
         env_list->list[dgnp_env_cnt].env_id,
         dcl.log_type = child_log_type, dcl.table_name = num_hold->tbl_name, dcl.pk_where = num_hold
         ->list[d.seq].pk_value,
         dcl.pk_where_value = 0
        PLAN (d
         WHERE d.seq > dgnp_insert_cnt
          AND (d.seq <= num_hold->pk_total))
         JOIN (dcl)
        WITH nocounter
       ;end insert
       SET dgnp_insert_cnt = dgnp_finished
       SET dgnp_child_cnt = curqual
       IF (check_error(concat("ERROR: inserting data from ",num_hold->tbl_name)) != 0)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ROLLBACK
        SET dgnp_finished = (num_hold->pk_total+ 1)
        SET dgnp_env_cnt = (1+ env_list->env_tol)
        GO TO exit_program
       ENDIF
       COMMIT
     ENDWHILE
   ENDFOR
 END ;Subroutine
 SUBROUTINE build_cmd(cmd_tx)
   SET dgnp_cmd_cur = (dgnp_cmd_cur+ 1)
   IF (mod(dgnp_cmd_cur,100)=1)
    SET stat = alterlist(num_par->list,(dgnp_cmd_cur+ 99))
   ENDIF
   SET num_par->list[dgnp_cmd_cur].st = cmd_tx
 END ;Subroutine
 SUBROUTINE dgnp_exepar(par_tbl_cnt)
   SET stat = alterlist(num_par->list,dgnp_cmd_cur)
   FOR (prc = 1 TO dgnp_cmd_cur)
     IF (findstring("#",num_par->list[prc].st))
      SET num_par->list[prc].st = replace(num_par->list[prc].st,"#","^",0)
     ENDIF
     IF (findstring("$",num_par->list[prc].st))
      SET num_par->list[prc].st = replace(num_par->list[prc].st,"$","'",0)
     ENDIF
     IF (prc < dgnp_cmd_cur)
      CALL parser(num_par->list[prc].st,0)
     ELSEIF (prc=dgnp_cmd_cur)
      CALL parser(num_par->list[prc].st,1)
     ENDIF
   ENDFOR
   IF (check_error(concat("ERROR: select data from ",dguc_act->dtd_hold[par_tbl_cnt].tbl_name)) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SET dgnp_cmd_cur = 0
   SET stat = alterlist(num_par->list,0)
 END ;Subroutine
 SUBROUTINE drcc_organization(null)
   CALL build_cmd("  and not exists (select 'x'")
   CALL build_cmd("  from code_value cv")
   CALL build_cmd("  where  CV.CODE_VALUE = td.ORG_CLASS_CD")
   CALL build_cmd("  AND CV.CDF_MEANING = 'FREETEXT'")
   CALL build_cmd("  and cv.CODE_SET = 396)")
 END ;Subroutine
 SUBROUTINE drcc_fre_sch(null)
   CALL build_cmd("   and td.PARENT_ENTITY != 'ORDERS'")
 END ;Subroutine
 SUBROUTINE drcc_event_code(null)
   CALL build_cmd("  and td.EVENT_CD_DISP !='Formbuilder Form'")
   CALL build_cmd("  and exists(")
   CALL build_cmd("     SELECT 'x' FROM CODE_VALUE c")
   CALL build_cmd("     WHERE c.CDF_MEANING = 'ACTIVE' AND c.CODE_SET = 48")
   CALL build_cmd("     and c.code_value = td.code_status_cd)")
   CALL build_cmd("  and EXISTS(")
   CALL build_cmd("  select 'x' from code_value v ")
   CALL build_cmd("  where v.code_value = td.event_cd) ")
 END ;Subroutine
 SUBROUTINE drcc_special_table(dst_tblcnt)
   IF ((dguc_act->dtd_hold[dst_tblcnt].tbl_name="ORGANIZATION"))
    CALL drcc_organization(null)
   ELSEIF ((dguc_act->dtd_hold[dst_tblcnt].tbl_name="FREQUENCY_SCHEDULE"))
    CALL drcc_fre_sch(null)
   ELSEIF ((dguc_act->dtd_hold[dst_tblcnt].tbl_name="V500_EVENT_CODE"))
    CALL drcc_event_code(null)
   ENDIF
 END ;Subroutine
#exit_program
END GO
