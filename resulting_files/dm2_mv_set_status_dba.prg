CREATE PROGRAM dm2_mv_set_status:dba
 ...
 SUBROUTINE dmvr_drop_mlog(ddm_bt_name)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmss_mv_name = vc WITH protect, noconstant(" ")
 DECLARE dmss_mv_status = vc WITH protect, noconstant(" ")
 DECLARE dmss_return_msg = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm2_mv_set_status",".log","dm2_mv_set_status LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating inputs for dm2_mv_set_status..."
 CALL disp_msg(" ",dm_err->logfile,dm_err->err_ind)
 SET dmss_mv_name = trim(cnvtupper( $1))
 SET dmss_mv_status = trim(cnvtupper( $2))
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg = concat("Parameter usage: dm2_mv_set_status '<Materialzied View Name>,",
   "<MVIEW/CVIEW/DISABLED>'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (trim(dmss_mv_name)="")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No Materialized View name supplied - ",
   "Usage: dm2_mv_set_status '<Materialzied View Name>, <MVIEW/CVIEW/DISABLED>'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ( NOT (dmss_mv_status IN ("MVIEW", "CVIEW", "DISABLED", "MVIEWBUILD")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("MVIEW/CVIEW/DISABLE are the valid status options - ",
   "Usage: dm2_mv_set_status '<Materialzied View Name>, <MVIEW/CVIEW/DISABLED>'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (dmvr_updt_mv_inst("U",dmss_mv_name,0,dmss_mv_status)=0)
  GO TO exit_program
 ENDIF
 IF (curqual=1)
  SET dmss_return_msg = concat(" successful. Update successful for: ",dmss_mv_name)
 ELSE
  SET dmss_return_msg = concat(" failed. Row not found for: ",dmss_mv_name)
 ENDIF
#exit_program
 IF ((dm_err->err_ind != 1))
  SET dm_err->eproc = concat("DM2_MV_SET_STATUS ",dmss_return_msg)
 ENDIF
 CALL final_disp_msg("DM2_MV_SET_STATUS")
END GO
