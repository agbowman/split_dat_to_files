CREATE PROGRAM dm_rmc_purge_admin_xlats:dba
 ...
 SUBROUTINE xta_dbl_hlv(i_gap)
   ...
   RETURN(xdh_gap)
 END ;Subroutine
 IF (check_logfile("DM_RMC_PAX",".log","DM_RMC_PAX LogFile")=0)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 ...
 ENDIF
 DECLARE pax_continue_ind = i2
 DECLARE pax_row_qual = i4
 DECLARE pax_logging_val = i4
 DECLARE pax_ret = vc
 SET pax_logging_val = 203
 SET pax_ret = dm_check_syn(null)
 IF (pax_ret="F")
 ...
 IF ((request->dm_rdds_chain_id=0))
  SET dm_err->emsg = "chain_id of 0 was passed in"
  SET dm_err->err_ind = 1
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Updating dm_rdds_chain row to state_flag = 3"
 CALL disp_msg(" ",dm_err->logfile,0)
 UPDATE  FROM dm_rdds_chain drc
  SET drc.state_flag = 3, drc.action_dt_tm = sysdate
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (di.info_number < 2000000000)
    pax_row_qual = di.info_number
   ELSE
    pax_row_qual = 2000000000
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO end_program
 ENDIF
 IF (((curqual=0) OR (pax_row_qual < 100)) )
  SET pax_row_qual = 100000
 ENDIF
 WHILE (pax_continue_ind=0)
   SET dm_err->eproc = concat("Cleaning up translations found in admin for chain_id: ",cnvtstring(
     request->dm_rdds_chain_id))
   CALL disp_msg(" ",dm_err->logfile,0)
   DELETE  FROM dm_rdds_dmt drd
    WHERE (drd.dm_rdds_chain_id=request->dm_rdds_chain_id)
    WITH maxqual(drd,value(pax_row_qual)), nocounter
   ;end delete
   SET pax_qual = curqual
   IF (check_error(dm_err->eproc)=1)
    SET pax_row_qual = xta_dbl_hlv(cnvtreal(pax_row_qual))
    IF (pax_row_qual <= 0)
     GO TO end_program
    ENDIF
   ELSE
    IF (pax_qual < pax_row_qual)
     SET pax_continue_ind = 1
    ENDIF
    COMMIT
   ENDIF
   SELECT INTO "nl:"
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO end_program
   ENDIF
   IF (pax_logging_val=303)
    GO TO end_program
   ENDIF
 ENDWHILE
 SET pax_logging_val = 103
#end_program
 UPDATE  FROM dm_rdds_chain drc
  SET drc.state_flag = pax_logging_val, drc.error_msg = substring(1,255,dm_err->emsg), drc
