CREATE PROGRAM dm_build_ea_triggers:dba
 ...
 SUBROUTINE dsr_chk_sd_in_process(dcsip_db_group,dcsip_owner,dcsip_sd_in_process)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE fill_trg(null) = i4 WITH protect
 DECLARE fill_trg_from_tbl(null) = i4 WITH protect
 DECLARE get_select_trg_name = null WITH protect
 DECLARE o_drop_triggers(null) = i4 WITH protect
 DECLARE o_create_triggers(null) = i4 WITH protect
 DECLARE d_drop_triggers(null) = i4 WITH protect
 DECLARE d_create_triggers(null) = i4 WITH protect
 DECLARE v_trigger_str = vc WITH public, noconstant(" ")
 DECLARE v_trigger_declare_str = vc WITH public, noconstant(" ")
 DECLARE v_pe_col_name_ind = i2 WITH public, noconstant(0)
 DECLARE v_updt_str = vc WITH public, noconstant(" ")
 DECLARE v_ins_str1 = vc WITH public, noconstant(" ")
 DECLARE v_ins_str2 = vc WITH public, noconstant(" ")
 DECLARE v_when_clause = vc WITH public, noconstant(" ")
 DECLARE v_drop_str = vc WITH public, noconstant(" ")
 DECLARE trg_cnt = i4 WITH public, noconstant(0)
 DECLARE row_cnt = i4 WITH public, noconstant(0)
 DECLARE regexp_replace() = c300
 DECLARE dbet_sd_in_use_ind = i2 WITH protect, noconstant(0)
 DECLARE dbet_trg_drop_ind = i2 WITH protect, noconstant(0)
 DECLARE v_global_when_clause = vc WITH protect, noconstant(" ")
 FREE RECORD trg
 RECORD trg(
   1 tab[*]
     2 trig_version = i4
 ...
 SET stat = check_logfile("dm_ea_trg",".log","DM_BUILD_EA_TRIGGERS Begin")
 SET v_global_when_clause = " (NVL(SYS_CONTEXT('CERNER','FIRE_EA_TRG'),'DM2NULLVAL')  != 'NO') "
 IF (dsr_sd_in_use_check("DM2NOTSET",dbet_sd_in_use_ind)=0)
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
  GO TO exit_script
 ENDIF
 IF (dbet_sd_in_use_ind=1)
 ...
 IF ( NOT (fill_trg(null)))
  SET readme_data->status = "F"
  SET readme_data->message = "No rows selected from dm_entity_activity_trigger table"
  GO TO exit_script
 ENDIF
 IF ( NOT (fill_trg_from_tbl(null)))
  SET readme_data->status = "F"
  SET readme_data->message = "Error retrieving triggers from dm2_user_triggers"
  GO TO exit_script
 ENDIF
 ...
 GO TO exit_script
 SUBROUTINE fill_trg_from_tbl(null)
  SELECT INTO "nl:"
   trig_body = replace(replace(replace(cnvtupper(ut.trigger_body),char(10)," ",0),char(13)," ",0),
    char(0)," ",0)
   FROM dm2_user_triggers ut
   WHERE ((ut.trigger_name="TRG*EA") OR (ut.trigger_name="TRG*$C"))
   ORDER BY ut.table_name
   HEAD REPORT
    tab_cnt = size(trg->tab,5), search_item = 0, found_pos = 0,
    found_pos2 = 0, row_cnt = 0, start_pos = 1,
    iter = 0
   HEAD ut.table_name
    FOR (iter = 1 TO 10)
      trig_body = replace(trig_body,"  "," ",0)
    ENDFOR
    IF (cnvtupper(trig_body)="*UPDATE DM_ENTITY_ACTIVITY*")
     IF (locateval(search_item,1,tab_cnt,ut.table_name,trg->tab[search_item].table_name)=0)
      tab_cnt = (tab_cnt+ 1), stat = alterlist(trg->tab,tab_cnt), trg->tab[tab_cnt].table_name =
      cnvtupper(ut.table_name),
      trg->tab[tab_cnt].inactive_ind = 0, row_cnt = 0, start_pos = 1,
      found_pos = findstring("IF (:NEW.",trig_body,start_pos), found_pos2 = findstring(
       "UPDATE DM_ENTITY_ACTIVITY EA",trig_body,start_pos)
      WHILE (((found_pos > 0) OR (found_pos2 > 0)) )
        row_cnt = (row_cnt+ 1), stat = alterlist(trg->tab[tab_cnt].data,row_cnt)
        IF (found_pos=0)
         start_pos = found_pos2
        ELSE
         start_pos = least(found_pos,found_pos2)
        ENDIF
        IF (start_pos=found_pos)
         trg->tab[tab_cnt].data[row_cnt].pe_col_name = substring((found_pos+ 9),(findstring(" ",
           trig_body,(found_pos+ 9)) - (found_pos+ 9)),trig_body), start_pos = found_pos2
        ENDIF
        found_pos = findstring("WHERE EA.PARENT_ENTITY_NAME = '",trig_body,start_pos), trg->tab[
        tab_cnt].data[row_cnt].parent_name = substring((found_pos+ 31),(findstring("'",trig_body,(
          found_pos+ 31)) - (found_pos+ 31)),trig_body), found_pos = findstring(
         "AND EA.PARENT_ENTITY_ID = :NEW.",trig_body,start_pos),
        trg->tab[tab_cnt].data[row_cnt].column_name = substring((found_pos+ 31),(findstring(" ",
          trig_body,(found_pos+ 31)) - (found_pos+ 31)),trig_body), found_pos = findstring(
         "AND EA.ENTITY_ACTIVITY_TYPE_CD = ",trig_body,start_pos), trg->tab[tab_cnt].data[row_cnt].
        entity_activity_type_cd = cnvtreal(substring((found_pos+ 33),(findstring(" ",trig_body,(
           found_pos+ 33)) - (found_pos+ 33)),trig_body)),
        trg->tab[tab_cnt].data[row_cnt].active_ind = 1, trg->tab[tab_cnt].data[row_cnt].comparator =
        build(trim(trg->tab[tab_cnt].data[row_cnt].pe_col_name,3),trim(trg->tab[tab_cnt].data[row_cnt
          ].parent_name,3),trim(trg->tab[tab_cnt].data[row_cnt].column_name,3),trg->tab[tab_cnt].
         data[row_cnt].entity_activity_type_cd)
        IF (row_cnt > 1
         AND locateval(search_item,1,(row_cnt - 1),trg->tab[tab_cnt].data[row_cnt].comparator,trg->
         tab[tab_cnt].data[search_item].comparator) > 0)
         row_cnt = (row_cnt - 1), stat = alterlist(trg->tab[tab_cnt].data,row_cnt)
        ENDIF
        start_pos = found_pos, found_pos = findstring("IF (:NEW.",trig_body,start_pos), found_pos2 =
        findstring("UPDATE DM_ENTITY_ACTIVITY EA",trig_body,start_pos)
      ENDWHILE
     ENDIF
    ENDIF
   WITH nocounter
 ...
 SUBROUTINE fill_trg(null)
  SELECT INTO "nl:"
   ea.table_name, ea.column_name, ea.pe_col_name,
   ea.parent_name, ea.active_ind, ea.entity_activity_type_cd
   FROM dm_entity_activity_trigger ea
   ORDER BY ea.table_name, ea.column_name, ea.parent_name
   HEAD REPORT
    tab_cnt = 0
   HEAD ea.table_name
    tab_cnt = (tab_cnt+ 1)
   ...
   HEAD ea.parent_name
    row_cnt = (row_cnt+ 1)
    IF (mod(row_cnt,10)=1)
     stat = alterlist(trg->tab[tab_cnt].data,(row_cnt+ 9))
    ENDIF
    trg->tab[tab_cnt].data[row_cnt].column_name = ea.column_name, trg->tab[tab_cnt].data[row_cnt].
    pe_col_name = ea.pe_col_name, trg->tab[tab_cnt].data[row_cnt].parent_name = cnvtupper(ea
     .parent_name),
    trg->tab[tab_cnt].data[row_cnt].active_ind = ea.active_ind
    IF (ea.active_ind=1)
     trg->tab[tab_cnt].inactive_ind = 0
    ENDIF
    trg->tab[tab_cnt].data[row_cnt].entity_activity_type_cd = ea.entity_activity_type_cd
   FOOT  ea.table_name
    stat = alterlist(trg->tab[tab_cnt].data,row_cnt)
   FOOT REPORT
 ...
 SUBROUTINE o_drop_triggers(null)
   FOR (x = 1 TO size(trg->tab,5))
    SET trg->tab[x].trig_version = - (1)
    SELECT INTO "nl:"
     tmp_version = regexp_replace(ut.description,"(.*/\*version_)(\d+)(\*/.*)","\2",1,0,
      "in")
     FROM dm2_user_triggers ut
     ...
     DETAIL
      trg->tab[x].trig_version = cnvtint(tmp_version), len = textlen(trim(ut.trigger_name,3))
      ...
      ELSE
       trg->tab[x].qual[cnt].trg_reg_exists = 1, trg->tab[x].qual[cnt].trg_reg_name = ut.trigger_name
      ENDIF
     WITH nocounter
    ;end select
   ENDFOR
   FOR (tcnt = 1 TO size(trg->tab,5))
     FOR (cnt = 1 TO size(trg->tab[tcnt].qual,5))
       IF ((trg->tab[tcnt].inactive_ind=1)
        AND (trg->tab[tcnt].trig_version <= 0))
        IF ((trg->tab[tcnt].qual[cnt].trg_c_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tcnt].qual[cnt].trg_c_name," go"),
          1)))
          RETURN(0)
         ENDIF
        ENDIF
        IF ((trg->tab[tcnt].qual[cnt].trg_reg_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tcnt].qual[cnt].trg_reg_name,
           " go"),1)))
          RETURN(0)
         ENDIF
        ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE o_create_triggers(null)
  FOR (tab_ndx = 1 TO size(trg->tab,5))
    SET v_trigger_str = ""
    SET v_updt_str = ""
    SET v_ins_str1 = ""
    SET v_ins_str2 = ""
    SET v_when_clause = ""
    IF ((trg->tab[tab_ndx].inactive_ind=0))
     SET trg->tab[tab_ndx].trigger_name = concat(trim(substring(1,27,concat("TRG",trim(trg->tab[
          tab_ndx].table_name,3))),3),"_EA")
     SET dbet_trg_drop_ind = 0
     IF (dbet_sd_in_use_ind=1
      AND (trg->tab[tab_ndx].trig_version <= 0))
      IF (dsr_check_object_state(currdbuser,"TRIGGER",trim(trg->tab[tab_ndx].trigger_name,3),
       "DROPPED",dbet_trg_drop_ind)=0)
       SET dm_sql_reply->msg = dm_err->emsg
       RETURN(0)
      ENDIF
     ENDIF
     IF ((((trg->tab[tab_ndx].trig_version > 0)) OR (dbet_trg_drop_ind=1)) )
      CALL echo(concat("Schema Deployment now managing trigger ",trim(trg->tab[tab_ndx].trigger_name,
         3)))
     ELSE
      SELECT INTO "nl:"
       ut.column_name
       FROM user_tab_columns ut
       WHERE ut.table_name=trim(trg->tab[tab_ndx].table_name,3)
        AND ut.column_name IN ("UPDT_ID", "UPDT_APPLCTX", "UPDT_TASK")
       DETAIL
        IF (ut.column_name="UPDT_ID")
         v_updt_str = concat(trim(v_updt_str,3)," ,EA.updt_id = :new.updt_id "), v_ins_str1 = concat(
          trim(v_ins_str1,3)," ,updt_id "), v_ins_str2 = concat(trim(v_ins_str2,3)," ,:new.updt_id ")
        ELSEIF (ut.column_name="UPDT_APPLCTX")
         v_updt_str = concat(trim(v_updt_str,3)," ,EA.updt_applctx = :new.updt_applctx "), v_ins_str1
          = concat(trim(v_ins_str1,3)," ,updt_applctx "), v_ins_str2 = concat(trim(v_ins_str2,3),
          " ,:new.updt_applctx ")
        ELSEIF (ut.column_name="UPDT_TASK")
         v_updt_str = concat(trim(v_updt_str,3)," ,EA.updt_task = :new.updt_task "), v_ins_str1 =
         concat(trim(v_ins_str1,3)," ,updt_task "), v_ins_str2 = concat(trim(v_ins_str2,3),
          " ,:new.updt_task ")
        ENDIF
       WITH nocounter
      ;end select
      FOR (row_ndx = 1 TO size(trg->tab[tab_ndx].data,5))
        IF ((trg->tab[tab_ndx].data[row_ndx].active_ind=1))
         SET v_pe_col_name_ind = 0
         IF (size(trim(trg->tab[tab_ndx].data[row_ndx].pe_col_name,3)) != 0)
          SET v_pe_col_name_ind = 1
          IF (size(trim(v_when_clause,3)) != 0)
           SET v_when_clause = concat(trim(v_when_clause,3)," OR ")
          ENDIF
          SET v_when_clause = concat(trim(v_when_clause,3)," new.",trim(trg->tab[tab_ndx].data[
            row_ndx].pe_col_name,3)," = '",trim(trg->tab[tab_ndx].data[row_ndx].parent_name,3),
           "'")
          SET v_trigger_str = concat(trim(v_trigger_str,3)," if (:new.",trim(trg->tab[tab_ndx].data[
            row_ndx].pe_col_name,3)," = '",trim(trg->tab[tab_ndx].data[row_ndx].parent_name,3),
           "') then")
         ENDIF
         SET v_trigger_declare_str = concat("DECLARE",char(10),"  temp number;",char(10),
          "  resource_busy EXCEPTION;",
          char(10),"  PRAGMA EXCEPTION_INIT(resource_busy, -54);",char(10))
         SET v_trigger_str = concat(trim(v_trigger_str,3),char(10),"  BEGIN",char(10),
          "   SELECT 0 into temp FROM DM_ENTITY_ACTIVITY EA",
          char(10),"    WHERE EA.parent_entity_name = '",trim(trg->tab[tab_ndx].data[row_ndx].
           parent_name,3),"' ",char(10),
          "      AND EA.parent_entity_id = :new.",trim(trg->tab[tab_ndx].data[row_ndx].column_name,3),
          char(10),"      AND EA.entity_activity_type_cd = ",cnvtstring(trg->tab[tab_ndx].data[
           row_ndx].entity_activity_type_cd),
          char(10),"    FOR UPDATE NOWAIT;",char(10),"   EXCEPTION",char(10),
          "     WHEN resource_busy THEN null;",char(10),"     WHEN TOO_MANY_ROWS THEN null;",char(10),
          "     WHEN NO_DATA_FOUND THEN",
          char(10),"        INSERT into DM_ENTITY_ACTIVITY ",char(10),
          "        (entity_activity_id, parent_entity_name, parent_entity_id, ",char(10),
          "         entity_activity_type_cd, updt_cnt, updt_dt_tm ",trim(v_ins_str1,3),")",char(10),
          "         VALUES(dm_clinical_seq.nextval, '",
          trim(trg->tab[tab_ndx].data[row_ndx].parent_name,3),"', ",char(10),"                :new.",
          trim(trg->tab[tab_ndx].data[row_ndx].column_name,3),
          ",",cnvtstring(trg->tab[tab_ndx].data[row_ndx].entity_activity_type_cd)," , 0, SYSDATE ",
          trim(v_ins_str2,3),");",
          char(10),"      WHEN OTHERS THEN raise;",char(10),"  END;",char(10),
          "  IF SQL%FOUND THEN",char(10),"    UPDATE DM_ENTITY_ACTIVITY EA ",char(10),
          "    SET EA.updt_cnt = updt_cnt + 1,",
          char(10),"      EA.updt_dt_tm = SYSDATE ",char(10),"      ",trim(v_updt_str,3),
          char(10),"    WHERE EA.parent_entity_name = '",trim(trg->tab[tab_ndx].data[row_ndx].
           parent_name,3),"' ",char(10),
          "   AND EA.parent_entity_id = :new.",trim(trg->tab[tab_ndx].data[row_ndx].column_name,3),
          char(10),"   AND EA.entity_activity_type_cd = ",cnvtstring(trg->tab[tab_ndx].data[row_ndx].
           entity_activity_type_cd),
          ";",char(10),"  END IF; ",char(10))
         IF (v_pe_col_name_ind=1)
          SET v_trigger_str = concat(trim(v_trigger_str,3),char(10)," END IF;")
         ENDIF
        ENDIF
      ENDFOR
      IF (size(trim(v_when_clause,3)) != 0)
       SET v_when_clause = concat(" WHEN (",v_global_when_clause," AND (",trim(v_when_clause,3),"))")
      ELSE
       SET v_when_clause = concat(" WHEN (",v_global_when_clause," )")
      ENDIF
      FOR (tcnt = 1 TO size(trg->tab[tab_ndx].qual,5))
        IF ((trg->tab[tab_ndx].qual[tcnt].trg_c_exists=1)
         AND (trg->tab[tab_ndx].qual[tcnt].trg_reg_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tab_ndx].qual[tcnt].trg_c_name,
           " go"),1)))
          RETURN(0)
         ENDIF
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tab_ndx].qual[tcnt].trg_reg_name,
           " go"),1)))
          RETURN(0)
         ENDIF
         SET trg->tab[tab_ndx].trigger_name = trg->tab[tab_ndx].qual[tcnt].trg_reg_name
        ELSEIF ((trg->tab[tab_ndx].qual[tcnt].trg_c_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tab_ndx].qual[tcnt].trg_c_name,
           " go"),1)))
          RETURN(0)
         ENDIF
         SET trg->tab[tab_ndx].trigger_name = trg->tab[tab_ndx].qual[tcnt].trg_c_name
        ELSEIF ((trg->tab[tab_ndx].qual[tcnt].trg_reg_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tab_ndx].qual[tcnt].trg_reg_name,
           " go"),1)))
          RETURN(0)
         ENDIF
        ...
        ENDIF
      ENDFOR
      SET v_trigger_str = concat("create or replace trigger ",trim(trg->tab[tab_ndx].trigger_name,3),
       char(10)," after insert or update on ",trim(trg->tab[tab_ndx].table_name,3),
       char(10)," for each row ",char(10)," ",trim(v_when_clause,3),
       char(10),trim(v_trigger_declare_str,3),char(10)," begin ",char(10),
       " ",trim(v_trigger_str,3),char(10)," end ",trim(trg->tab[tab_ndx].trigger_name,3),
       "; ")
      SET v_trigger_str = concat("rdb asis(^",trim(v_trigger_str,3),"^) end go")
      IF ( NOT (dm2_push_cmd(v_trigger_str,1)))
       RETURN(0)
      ENDIF
      EXECUTE dm_readme_include_sql_chk trg->tab[tab_ndx].trigger_name, "trigger"
 ...
 SUBROUTINE d_drop_triggers(null)
     ...
     FOR (cnt = 1 TO size(trg->tab[tcnt].qual,5))
       IF ((trg->tab[tcnt].inactive_ind=1))
        IF ((trg->tab[tcnt].qual[cnt].trg_eains_c_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tcnt].qual[cnt].trg_eains_c_name,
           " go"),1)))
          RETURN(0)
         ENDIF
        ENDIF
        IF ((trg->tab[tcnt].qual[cnt].trg_eaupd_c_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tcnt].qual[cnt].trg_eaupd_c_name,
           " go"),1)))
          RETURN(0)
         ENDIF
        ENDIF
        IF ((trg->tab[tcnt].qual[cnt].trg_eains_reg_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tcnt].qual[cnt].
           trg_eains_reg_name," go"),1)))
          RETURN(0)
         ENDIF
        ENDIF
        IF ((trg->tab[tcnt].qual[cnt].trg_eaupd_reg_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tcnt].qual[cnt].
           trg_eaupd_reg_name," go"),1)))
          RETURN(0)
         ENDIF
        ENDIF
       ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE d_create_triggers(null)
   DECLARE len = i4
   DECLARE trigger_name = vc
   FOR (tab_ndx = 1 TO size(trg->tab,5))
     SET v_trigger_str = ""
     SET v_updt_str = ""
     SET v_ins_str1 = ""
     ...
     SET v_drop_str = ""
     IF ((trg->tab[tab_ndx].inactive_ind=0))
      IF ((trg->tab[tab_ndx].eaupd_trigger_name=" "))
       SET len = textlen(trim(trg->tab[tab_ndx].table_name,3))
       SET trigger_name = concat("trg",trim(substring((len - 3),len,trg->tab[tab_ndx].table_name),3),
        "_EAUPD")
       SET trg->tab[tab_ndx].eaupd_trigger_name = trigger_name
      ENDIF
      SELECT INTO "nl:"
       ut.column_name
       FROM user_tab_columns ut
       ...
       DETAIL
        IF (ut.column_name="UPDT_ID")
         v_updt_str = concat(trim(v_updt_str,3)," ,EA.updt_id = n.updt_id "), v_ins_str1 = concat(
          trim(v_ins_str1,3)," ,updt_id "), v_ins_str2 = concat(trim(v_ins_str2,3)," ,n.updt_id ")
        ELSEIF (ut.column_name="UPDT_APPLCTX")
         v_updt_str = concat(trim(v_updt_str,3)," ,EA.updt_applctx = n.updt_applctx "), v_ins_str1 =
         concat(trim(v_ins_str1,3)," ,updt_applctx "), v_ins_str2 = concat(trim(v_ins_str2,3),
          " ,n.updt_applctx ")
        ELSEIF (ut.column_name="UPDT_TASK")
         v_updt_str = concat(trim(v_updt_str,3)," ,EA.updt_task = n.updt_task "), v_ins_str1 = concat
         (trim(v_ins_str1,3)," ,updt_task "), v_ins_str2 = concat(trim(v_ins_str2,3)," ,n.updt_task "
          )
        ENDIF
       WITH nocounter
      ;end select
        ...
        IF ((trg->tab[tab_ndx].data[row_ndx].active_ind=1))
         ...
         IF (size(trim(trg->tab[tab_ndx].data[row_ndx].pe_col_name,3)) != 0)
          ...
          IF (size(trim(v_when_clause,3)) != 0)
           SET v_when_clause = concat(trim(v_when_clause,3)," OR ")
          ENDIF
          SET v_when_clause = concat(trim(v_when_clause,3)," n.",trim(trg->tab[tab_ndx].data[row_ndx]
            .pe_col_name,3)," = '",trim(trg->tab[tab_ndx].data[row_ndx].parent_name,3),
           "'")
          SET v_trigger_str = concat(trim(v_trigger_str,3)," if (n.",trim(trg->tab[tab_ndx].data[
            row_ndx].pe_col_name,3)," = '",trim(trg->tab[tab_ndx].data[row_ndx].parent_name,3),
           "') then")
         ENDIF
         SET v_trigger_str = concat(trim(v_trigger_str,3)," update dm_entity_activity EA ",char(10),
          " set EA.updt_cnt = updt_cnt + 1,",char(10),
          " EA.updt_dt_tm = current timestamp ",char(10),trim(v_updt_str,3),char(10),
          " where EA.parent_entity_name = '",
          trim(trg->tab[tab_ndx].data[row_ndx].parent_name,3),"' ",char(10),
          " and EA.parent_entity_id = n.",trim(trg->tab[tab_ndx].data[row_ndx].column_name,3),
          char(10)," and EA.entity_activity_type_cd = ",cnvtstring(trg->tab[tab_ndx].data[row_ndx].
           entity_activity_type_cd),";",char(10),
          " get diagnostics rcount = ROW_COUNT;",char(10)," if (rcount = 0) then",char(10),
          " insert into dm_entity_activity ",
          char(10)," (entity_activity_id, parent_entity_name, parent_entity_id, ",char(10),
          "  entity_activity_type_cd, updt_cnt, updt_dt_tm ",char(10),
          trim(v_ins_str1,3),")",char(10)," values(nextval for dm_clinical_seq, '",trim(trg->tab[
           tab_ndx].data[row_ndx].parent_name,3),
          "', ",char(10)," n.",trim(trg->tab[tab_ndx].data[row_ndx].column_name,3),",",
          cnvtstring(trg->tab[tab_ndx].data[row_ndx].entity_activity_type_cd),
          " , 0, current timestamp ",char(10),trim(v_ins_str2,3),");",
          char(10)," end if; ")
         IF (v_pe_col_name_ind=1)
          SET v_trigger_str = concat(trim(v_trigger_str,3)," end if;")
         ENDIF
        ENDIF
      ENDFOR
      IF (size(trim(v_when_clause,3)) != 0)
       SET v_when_clause = concat(" when (",trim(v_when_clause,3),")")
      ENDIF
      FOR (tcnt = 1 TO size(trg->tab[tab_ndx].qual,5))
        IF ((trg->tab[tab_ndx].qual[tcnt].trg_eaupd_c_exists=1)
         AND (trg->tab[tab_ndx].qual[tcnt].trg_eaupd_reg_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tab_ndx].qual[tcnt].
           trg_eaupd_c_name," go"),1)))
          RETURN(0)
         ELSE
          COMMIT
         ENDIF
         ...
         ELSE
          COMMIT
         ENDIF
         SET trg->tab[tab_ndx].eains_trigger_name = trg->tab[tab_ndx].qual[tcnt].trg_eaupd_reg_name
        ELSEIF ((trg->tab[tab_ndx].qual[tcnt].trg_eaupd_c_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tab_ndx].qual[tcnt].
           trg_eaupd_c_name," go"),1)))
          RETURN(0)
         ELSE
          COMMIT
         ENDIF
         SET trg->tab[tab_ndx].eaupd_trigger_name = trg->tab[tab_ndx].qual[tcnt].trg_eaupd_c_name
        ELSEIF ((trg->tab[tab_ndx].qual[tcnt].trg_eaupd_reg_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tab_ndx].qual[tcnt].
           trg_eaupd_reg_name," go"),1)))
          RETURN(0)
         ELSE
          COMMIT
         ENDIF
         SET trg->tab[tab_ndx].eaupd_trigger_name = trg->tab[tab_ndx].qual[tcnt].trg_eaupd_reg_name
        ENDIF
      ENDFOR
      SET v_trigger_str = concat("create trigger ",trim(trg->tab[tab_ndx].eaupd_trigger_name,3),char(
        10)," after update on ",trim(trg->tab[tab_ndx].table_name,3),
       char(10)," referencing new as n",char(10)," for each row mode db2sql ",char(10),
       " ",trim(v_when_clause,3),char(10)," begin atomic ",char(10),
       " declare rcount integer; ",char(10)," ",trim(v_trigger_str,3),char(10),
       " end ")
      SET v_trigger_str = concat("rdb asis(^",trim(v_trigger_str,3),"^) end go")
      IF ( NOT (dm2_push_cmd(v_trigger_str,1)))
       RETURN(0)
      ELSE
       COMMIT
      ENDIF
      EXECUTE dm_readme_include_sql_chk trg->tab[tab_ndx].eaupd_trigger_name, "trigger"
      IF ((dm_sql_reply->status="F"))
       RETURN(0)
      ENDIF
      SET v_trigger_str = ""
      ...
      SET v_ins_str2 = ""
      SET v_when_clause = ""
      IF ((trg->tab[tab_ndx].eains_trigger_name=" "))
       SET len = textlen(trim(trg->tab[tab_ndx].table_name,3))
       SET trigger_name = concat("trg",trim(substring((len - 3),len,trg->tab[tab_ndx].table_name),3),
        "_EAINS")
       SET trg->tab[tab_ndx].eains_trigger_name = trigger_name
      ENDIF
      SELECT INTO "nl:"
       ut.column_name
       FROM user_tab_columns ut
      ...
      ENDIF
      FOR (tcnt = 1 TO size(trg->tab[tab_ndx].qual,5))
        IF ((trg->tab[tab_ndx].qual[tcnt].trg_eains_c_exists=1)
         AND (trg->tab[tab_ndx].qual[tcnt].trg_eains_reg_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tab_ndx].qual[tcnt].
           trg_eains_c_name," go"),1)))
          RETURN(0)
         ELSE
          COMMIT
         ENDIF
         ...
         ELSE
          COMMIT
         ENDIF
         SET trg->tab[tab_ndx].eains_trigger_name = trg->tab[tab_ndx].qual[tcnt].trg_eains_reg_name
        ELSEIF ((trg->tab[tab_ndx].qual[tcnt].trg_eains_c_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tab_ndx].qual[tcnt].
           trg_eains_c_name," go"),1)))
          RETURN(0)
         ELSE
          COMMIT
         ENDIF
         SET trg->tab[tab_ndx].eains_trigger_name = trg->tab[tab_ndx].qual[tcnt].trg_eains_c_name
        ELSEIF ((trg->tab[tab_ndx].qual[tcnt].trg_eains_reg_exists=1))
         IF ( NOT (dm2_push_cmd(concat("rdb drop trigger ",trg->tab[tab_ndx].qual[tcnt].
           trg_eains_reg_name," go"),1)))
          RETURN(0)
         ELSE
        ...
        ENDIF
      ENDFOR
      SET v_trigger_str = concat("create trigger ",trim(trg->tab[tab_ndx].eains_trigger_name,3),char(
        10)," after insert on ",trim(trg->tab[tab_ndx].table_name,3),
       char(10)," referencing new as n",char(10)," for each row mode db2sql ",char(10),
       " ",trim(v_when_clause,3),char(10)," begin atomic ",char(10),
       " declare rcount integer; ",char(10)," ",trim(v_trigger_str,3),char(10),
       " end ")
      ...
      ELSE
       COMMIT
      ENDIF
      EXECUTE dm_readme_include_sql_chk trg->tab[tab_ndx].eains_trigger_name, "trigger"
      IF ((dm_sql_reply->status="F"))
       RETURN(0)
      ENDIF
     ENDIF
...
#exit_script
 IF ((readme_data->status="S"))
  SET readme_data->message = "All triggers in DM_BUILD_EA_TRIGGERS were built successfully."
 ENDIF
