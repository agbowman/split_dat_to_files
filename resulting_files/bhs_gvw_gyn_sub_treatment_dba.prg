CREATE PROGRAM bhs_gvw_gyn_sub_treatment:dba
 FREE RECORD m_rec
 RECORD m_rec(
   1 l_cnt = i4
   1 qual[*]
     2 s_gyn_sub_treatment = vc
     2 f_gyn_sub_date = f8
     2 s_gyn_sub_date = vc
     2 f_gyn_sub_end_date = f8
     2 s_gyn_sub_end_date = vc
 ) WITH protect
 DECLARE mf_gynsubsequenttreatment_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "GYNSUBSEQUENTTREATMENT"))
 DECLARE mf_gynsubsequenttreatmentdate_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "GYNSUBSEQUENTTREATMENTDATE"))
 DECLARE mf_gynsubsequenttreatmentenddate_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",
   72,"GYNSUBSEQUENTTREATMENTENDDATE"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ce
   WHERE ce.person_id=mf_per_id
    AND ce.event_cd IN (mf_gynsubsequenttreatment_cd, mf_gynsubsequenttreatmentdate_cd,
   mf_gynsubsequenttreatmentenddate_cd)
  ...
  HEAD ce.event_cd
   CASE (ce.event_cd)
    OF mf_gynsubsequenttreatmentdate_cd:
     m_rec->qual[ml_cnt].s_gyn_sub_date = format(cdr.result_dt_tm,"mm/dd/yy;;D"),m_rec->qual[ml_cnt].
     f_gyn_sub_date = cdr.result_dt_tm
    OF mf_gynsubsequenttreatmentenddate_cd:
     m_rec->qual[ml_cnt].s_gyn_sub_end_date = format(cdr.result_dt_tm,"mm/dd/yy;;D")
    OF mf_gynsubsequenttreatment_cd:
     m_rec->qual[ml_cnt].s_gyn_sub_treatment = trim(ce.result_val)
   ENDCASE
  WITH nocounter
 ;end select
 IF (ml_cnt < 1)
 ...
 SET ms_text_temp = concat(ms_rh2b,"{Gyn Subsequent Treatment}",ms_reol)
 SET reply->text = concat(reply->text," ",ms_text_temp)
 IF ((reqinfo->updt_task=3202004))
  SET ms_text_temp = concat("\trowd\trgaph30\cellx4000\cellx6000\cellx8000\intbl",ms_wb,
   "{Gyn Subsequent Treatment} ","\cell ",ms_wb,
   "{Gyn Subsequent Treatment Date} ","\cell ",ms_wb,"{Gyn Subsequent Treatment End Date} ","\cell ",
   "\row ")
  SET reply->text = concat(reply->text," ",ms_text_temp)
 ENDIF
 SELECT INTO "nl:"
  gyn_sub_date = m_rec->qual[d.seq].f_gyn_sub_date
  FROM (dummyt d  WITH seq = m_rec->l_cnt)
  ORDER BY gyn_sub_date DESC
  DETAIL
   IF ((reqinfo->updt_task=3202004))
    ms_text_temp = concat("\trowd\trgaph30\cellx4000\cellx6000\cellx8000\intbl",ms_wr,"{",trim(m_rec
      ->qual[d.seq].s_gyn_sub_treatment),"} ",
     "\cell ",ms_wr,"{",trim(m_rec->qual[d.seq].s_gyn_sub_date),"} ",
     "\cell ",ms_wr,"{",trim(m_rec->qual[d.seq].s_gyn_sub_end_date),"} ",
     "\cell ","\row "), reply->text = concat(reply->text," ",ms_text_temp)
   ELSE
    ms_text_temp = concat(ms_wb,ms_hi,"{Gyn Subsequent Treatment: } ",ms_wr,"{",
     trim(m_rec->qual[d.seq].s_gyn_sub_treatment),"} ",ms_reol,ms_wb,
     "{Gyn Subsequent Treatment Date: } ",
     ms_wr,"{",trim(m_rec->qual[d.seq].s_gyn_sub_date),"} ",ms_reol,
     ms_wb,"{Gyn Subsequent Treatment End Date: } ",ms_wr,"{",trim(m_rec->qual[d.seq].
      s_gyn_sub_end_date),
     "} ",ms_reol,ms_reol), reply->text = concat(reply->text," ",ms_text_temp)
   ENDIF
  WITH nocounter
 ;end select
