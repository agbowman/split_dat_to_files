CREATE PROGRAM dm_omf_8
 ...
 UPDATE  FROM omf_outcome_indicator ooi
  SET ooi.indicator_name = "Mortality rate for PTCA"
  WHERE ooi.indicator_id=8
  WITH nocounter
 ;end update
 IF (curqual=0)
  INSERT  FROM omf_outcome_indicator ooi
   SET ooi.indicator_id = 8, ooi.indicator_name = "Mortality rate for PTCA"
   WITH nocounter
  ;end insert
 ENDIF
 ...
 IF (curqual=0)
  INSERT  FROM omf_outcome_rate oor
   SET oor.total_cases = data->num_patients, oor.numerator_value = numer, oor.denominator_value =
    ...
    oor.indicator_id = 8, oor.reporting_period = cnvtdatetime(startdatesstring), oor.client_id = data
    ->client_id
   WITH nocounter
  ;end insert
 ENDIF
