CREATE PROGRAM dm_load_session_param:dba
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dlsp_ins_cnt = i4 WITH protect, noconstant(0)
 DECLARE dlsp_rud_cnt = i4 WITH protect, noconstant(0)
 DECLARE dlsp_iter = i4 WITH protect, noconstant(0)
 DECLARE dlsp_rdbms_num_ver = i4 WITH protect, noconstant(0)
 DECLARE dlsp_parse_oracle_version(null) = i2
 FREE RECORD oracle_version
 RECORD oracle_version(
   ...
   1 level3 = i2
   1 level4 = i2
   1 numeric_version = i4
   1 comparator = vc
 )
 SUBROUTINE dlsp_parse_oracle_version(null)
   SET dm_err->eproc = "Getting oracle version."
   DECLARE dpov_level = i2 WITH protect, noconstant(0)
   DECLARE dpov_loc = i2 WITH protect, noconstant(0)
   DECLARE dpov_prev_loc = i2 WITH protect, noconstant(0)
   DECLARE dpov_loop = i2 WITH protect, noconstant(0)
   DECLARE dpov_len = i2 WITH protect, noconstant(0)
   WHILE (dpov_loop=0)
     SET dpov_level = (dpov_level+ 1)
     SET dpov_prev_loc = dpov_loc
     SET dpov_loc = 0
     SET dpov_loc = findstring(".",oracle_version->version,(dpov_prev_loc+ 1),0)
     IF (((dpov_loc > 0) OR (dpov_loc=0
      AND dpov_level > 1)) )
      IF (dpov_loc=0
       AND dpov_level > 1)
       SET dpov_len = (textlen(oracle_version->version) - dpov_prev_loc)
       SET dpov_loop = 1
      ELSE
       SET dpov_len = ((dpov_loc - dpov_prev_loc) - 1)
      ENDIF
      CASE (dpov_level)
       OF 1:
        SET oracle_version->level1 = cnvtint(substring(1,dpov_len,oracle_version->version))
       OF 2:
        SET oracle_version->level2 = cnvtint(substring((dpov_prev_loc+ 1),dpov_len,oracle_version->
          version))
       OF 3:
        SET oracle_version->level3 = cnvtint(substring((dpov_prev_loc+ 1),dpov_len,oracle_version->
          version))
       OF 4:
        SET oracle_version->level4 = cnvtint(substring((dpov_prev_loc+ 1),dpov_len,oracle_version->
          version))
       ELSE
        SET dpov_loop = 1
      ENDCASE
     ELSE
      IF (dpov_level=1)
       SET dm_err->err_ind = 1
       SET dm_err->emsg = "Oracle version not in expected format."
       CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
       RETURN(0)
      ENDIF
      SET dpov_loop = 1
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm_load_session_p",".log","DM_LOAD_SESSION_PARAM LOGFILE")=0)
  GO TO exit_script
 ENDIF
 ...
 IF (size(requestin->list_0,5) > 0)
  SET dm_err->eproc =
  "Deleting all DM_SET_SESSION_PARAMETERS and DM_SET_SYSTEM_PARAMETERS rows from dm_info."
  DELETE  FROM dm_info dm
   WHERE dm.info_domain IN ("DM_SET_SESSION_PARAMETERS", "DM_SET_SYSTEM_PARAMETERS")
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 FOR (dlsp_iter = 1 TO size(requestin->list_0,5))
  IF ((requestin->list_0[dlsp_iter].mode=optimizer_mode))
   IF ((((requestin->list_0[dlsp_iter].os=cursys)) OR ((requestin->list_0[dlsp_iter].os="ALL"))) )
    IF ((requestin->list_0[dlsp_iter].version_rule="NONE"))
     SET requestin->list_0[dlsp_iter].load_ind = "YES"
    ELSEIF (substring(1,1,requestin->list_0[dlsp_iter].version_rule) IN ("<", "=", ">"))
     SET oracle_version->comparator = substring(1,1,requestin->list_0[dlsp_iter].version_rule)
     SET requestin->list_0[dlsp_iter].version_rule = substring(2,(size(requestin->list_0[dlsp_iter].
       version_rule) - 1),requestin->list_0[dlsp_iter].version_rule)
     SET oracle_version->version = requestin->list_0[dlsp_iter].version_rule
     IF (dlsp_parse_oracle_version(null)=0)
      GO TO exit_script
     ELSE
      SET oracle_version->numeric_version = ((((oracle_version->level1 * 1000000)+ (oracle_version->
      level2 * 10000))+ (oracle_version->level3 * 100))+ oracle_version->level4)
      SET dlsp_rdbms_num_ver = ((((dm2_rdbms_version->level1 * 1000000)+ (dm2_rdbms_version->level2
       * 10000))+ (dm2_rdbms_version->level3 * 100))+ dm2_rdbms_version->level4)
      CASE (oracle_version->comparator)
       OF "<":
        IF ((dlsp_rdbms_num_ver < oracle_version->numeric_version))
         SET requestin->list_0[dlsp_iter].load_ind = "YES"
        ENDIF
       OF "=":
        IF ((dlsp_rdbms_num_ver=oracle_version->numeric_version))
  ...
  ENDIF
  IF ((dm_err->debug_flag >= 1))
   CALL echo(concat("dlsp_rdbms_num_ver: ",build(dlsp_rdbms_num_ver)))
   CALL echorecord(oracle_version)
  ENDIF
 ENDFOR
 IF (size(requestin->list_0,5) > 0)
  SET dm_err->eproc = "Inserting rows into dm_info"
  INSERT  FROM dm_info dm,
   ...
   SET dm.info_domain = requestin->list_0[d.seq].info_domain, dm.info_name = requestin->list_0[d.seq]
    ...
    dm.info_char = requestin->list_0[d.seq].info_char
   PLAN (d
    WHERE (requestin->list_0[d.seq].load_ind="YES"))
    JOIN (dm)
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  SET dlsp_ins_cnt = curqual
  SET dm_err->eproc = "Selecting from dm_info to get inserted rows."
  SELECT INTO "nl:"
   FROM dm_info dm,
    (dummyt d  WITH seq = value(size(requestin->list_0,5)))
   PLAN (d
    ...
    WHERE (dm.info_domain=requestin->list_0[d.seq].info_domain)
     ...
     AND (dm.info_char=requestin->list_0[d.seq].info_char))
   HEAD REPORT
    dlsp_rud_cnt = 0
   DETAIL
    dlsp_rud_cnt = (dlsp_rud_cnt+ 1)
   WITH nocounter, nullreport
  ;end select
  IF (dlsp_rud_cnt=dlsp_ins_cnt)
   COMMIT
  ELSE
   ROLLBACK
  ENDIF
...
#exit_script
 IF (dlsp_rud_cnt != dlsp_ins_cnt)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Not all dm_info rows loaded successfully."
 ENDIF
 SET dm_err->eproc = "Ending DM_LOAD_SESSION_PARAM."
 CALL final_disp_msg("dm_load_session_p")
END GO
