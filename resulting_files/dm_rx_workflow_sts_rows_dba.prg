CREATE PROGRAM dm_rx_workflow_sts_rows:dba
 ...
 SET reply->table_name = "RX_WORKFLOW_STS"
 ...
 ELSE
  ...
  SELECT INTO "nl:"
   rws.rx_workflow_sts_id
   FROM rx_workflow_sts rws
   WHERE parser(sbr_getrowidnotexists("rws.updt_dt_tm < cnvtdatetime(v_days_back)","rws"))
    AND ((rws.rx_workflow_sts_id+ 0) > 0)
   HEAD REPORT
    v_rows = 0
   DETAIL
    v_rows += 1
    ...
    ENDIF
    reply->rows[v_rows].row_id = rws.rowid
   FOOT REPORT
    stat = alterlist(reply->rows,v_rows)
   WITH nocounter, maxqual(rws,value(request->max_rows))
  ;end select
