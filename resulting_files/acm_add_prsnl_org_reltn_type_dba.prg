CREATE PROGRAM acm_add_prsnl_org_reltn_type:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->prsnl_org_reltn_type_qual[xref->add[index].idx].prsnl_org_reltn_type_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(prsnl_seq,nextval)
     FROM dual
     DETAIL
      acm_request->prsnl_org_reltn_type_qual[xref->add[index].idx].prsnl_org_reltn_type_id = cnvtreal
      (nextseqnum), reply->prsnl_org_reltn_type_qual[xref->add[index].idx].prsnl_org_reltn_type_id =
      cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->prsnl_org_reltn_type_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->prsnl_org_reltn_type_qual[xref->add[index].idx].prsnl_org_reltn_type_id = acm_request
    ->prsnl_org_reltn_type_qual[xref->add[index].idx].prsnl_org_reltn_type_id
   ENDIF
   SET reply->prsnl_org_reltn_type_qual[xref->add[index].idx].status = 0
   IF ((acm_request->prsnl_org_reltn_type_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->prsnl_org_reltn_type_qual[xref->add[index].idx].beg_effective_dt_tm =
    cnvtdatetime(curdate,curtime3)
   ENDIF
   IF ((acm_request->prsnl_org_reltn_type_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->prsnl_org_reltn_type_qual[xref->add[index].idx].end_effective_dt_tm =
    cnvtdatetime("31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   prsnl_org_reltn_type p
  SET p.prsnl_org_reltn_type_id = acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].
   prsnl_org_reltn_type_id, p.beg_effective_dt_tm = cnvtdatetime(acm_request->
    prsnl_org_reltn_type_qual[xref->add[d.seq].idx].beg_effective_dt_tm), p.contributor_system_cd =
   IF ((acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].
    end_effective_dt_tm), p.organization_id = acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq]
   .idx].organization_id, p.org_type_cd = acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx
   ].org_type_cd,
   p.position_cd = acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].position_cd, p
   .prsnl_id = acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].prsnl_id, p
   .prsnl_org_reltn_type_cd = acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].
   prsnl_org_reltn_type_cd,
   p.role_profile = acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].role_profile, p
   .job_role_cd = acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].job_role_cd, p
   .access_position_cd = acm_request->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].
   access_position_cd,
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->prsnl_org_reltn_type_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->prsnl_org_reltn_type_qual[xref->add[index].idx].status=0))
