CREATE PROGRAM bhs_gvw_ice_score:dba
 IF (validate(reply)=0)
  RECORD reply(
    1 status_data[1]
      2 status = c4
 ...
 IF (validate(request)=0)
  ...
  SET request->person[1].person_id = 26321458.00
  SET request->visit[1].encntr_id = 175726009.00
 ...
 DECLARE mf_cs72_icescore = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,"ICESCORE")), protect
 ...
 DECLARE ml_maxresult = i4 WITH protect, constant(5)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (e
   ...
   WHERE ce.encntr_id=e.encntr_id
    ...
    AND ce.event_cd=mf_cs72_icescore
  ...
  DETAIL
   m_rec->cnt += 1
   IF ((m_rec->cnt <= ml_maxresult))
    stat = alterlist(m_rec->list,m_rec->cnt), m_rec->list[m_rec->cnt].s_score_dt_tm = trim(format(ce
      .event_end_dt_tm,"dd-mmm-yyyy hh:mm;;d")), m_rec->list[m_rec->cnt].s_score_result = trim(ce
     .result_val)
   ENDIF
  WITH nocounter
 ;end select
 SET reply->text = concat(ms_rhead,ms_rh2bu,"ICE Score",ms_rtab,ms_rtab,
  "Date/Time",ms_reol)
 FOR (ml_cnt = 1 TO m_rec->cnt)
   SET reply->text = concat(reply->text,ms_rh2r,ms_rtab,m_rec->list[ml_cnt].s_score_result,ms_rtab,
    ms_rtab,m_rec->list[ml_cnt].s_score_dt_tm,ms_reol)
 ENDFOR
