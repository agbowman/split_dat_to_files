CREATE PROGRAM dm2_ins_upd_col_doc:dba
 ...
 ENDIF
 DECLARE iucdp_logfile_prefix = vc WITH protect, constant("dm2_ins_upd_col_doc")
 DECLARE iucdp_cnt = i4 WITH protect, noconstant(0)
 DECLARE iucdp_curqual = i4 WITH protect, noconstant(0)
 IF (check_logfile(iucdp_logfile_prefix,".log","DM2_INS_UPD_COL_DOC LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning maintenance on DM_COLUMNS_DOC...."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (validate(col_doc->qual[1].table_name,"@")="@")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Record structure required for this process is not established."
  SET dm_err->eproc = "Confirming record structure [col_doc] existence."
 ...
 IF (validate(col_doc_maint->qual[1].maint_type,"@")="@")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Record structure required for this process is not established."
  SET dm_err->eproc = "Confirming record structure [col_doc_maint] existence."
  SET dm_err->user_action = "Please establish required record structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 ...
 ELSEIF (curqual=1)
  ...
  GO TO exit_program
 ENDIF
 IF ((col_doc_maint->num_of_inserts > 0))
  IF (dm2_set_autocommit(1)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Performing insert maintenance on DM_COLUMNS_DOC."
  CALL disp_msg(" ",dm_err->logfile,0)
  CALL dm2_push_cmd("insert into dm_columns_doc  dcd, ",0)
  CALL dm2_push_cmd("  (dummyt d with seq = value(size(col_doc->qual,5))) ",0)
  CALL dm2_push_cmd(" set dcd.table_name = col_doc->qual[d.seq]->table_name ",0)
  CALL dm2_push_cmd("  ,dcd.column_name = col_doc->qual[d.seq]->column_name ",0)
  CALL dm2_push_cmd("  ,dcd.sequence_name = col_doc->qual[d.seq]->sequence_name ",0)
  CALL dm2_push_cmd("  ,dcd.code_set = col_doc->qual[d.seq]->code_set ",0)
  CALL dm2_push_cmd("  ,dcd.description = col_doc->qual[d.seq]->description ",0)
  CALL dm2_push_cmd("  ,dcd.definition = col_doc->qual[d.seq]->definition ",0)
  CALL dm2_push_cmd("  ,dcd.flag_ind = col_doc->qual[d.seq]->flag_ind ",0)
  CALL dm2_push_cmd("  ,dcd.updt_dt_tm = cnvtdatetime(curdate,curtime3) ",0)
  CALL dm2_push_cmd("  ,dcd.updt_cnt = col_doc->qual[d.seq]->updt_cnt ",0)
  CALL dm2_push_cmd("  ,dcd.updt_applctx = 0 ",0)
  CALL dm2_push_cmd("  ,dcd.updt_task = 0 ",0)
  CALL dm2_push_cmd("  ,dcd.updt_id = 0 ",0)
  CALL dm2_push_cmd("  ,dcd.unique_ident_ind = col_doc->qual[d.seq]->unique_ident_ind ",0)
  CALL dm2_push_cmd("  ,dcd.root_entity_name = col_doc->qual[d.seq]->root_entity_name ",0)
  CALL dm2_push_cmd("  ,dcd.root_entity_attr = col_doc->qual[d.seq]->root_entity_attr ",0)
  CALL dm2_push_cmd("  ,dcd.constant_value = col_doc->qual[d.seq]->constant_value ",0)
  CALL dm2_push_cmd("  ,dcd.parent_entity_col = col_doc->qual[d.seq]->parent_entity_col ",0)
  CALL dm2_push_cmd("  ,dcd.exception_flg = col_doc->qual[d.seq]->exception_flg ",0)
  CALL dm2_push_cmd("  ,dcd.defining_attribute_ind = col_doc->qual[d.seq]->defining_attribute_ind ",0
   )
  CALL dm2_push_cmd("  ,dcd.merge_updateable_ind = col_doc->qual[d.seq]->merge_updateable_ind ",0)
  CALL dm2_push_cmd("  ,dcd.nls_col_ind = col_doc->qual[d.seq]->nls_col_ind ",0)
  IF ((validate(col_doc->qual[1].absolute_date_ind,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dcd.absolute_date_ind = col_doc->qual[d.seq]->absolute_date_ind ",0)
  ENDIF
  IF ((validate(col_doc->qual[1].merge_delete_ind,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dcd.merge_delete_ind = col_doc->qual[d.seq]->merge_delete_ind ",0)
  ENDIF
  IF ((validate(col_doc->qual[1].tz_rule_flag,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dcd.tz_rule_flag = col_doc->qual[d.seq]->tz_rule_flag ",0)
  ENDIF
  CALL dm2_push_cmd(' plan d where col_doc_maint->qual[d.seq]->maint_type = "I" ',0)
  CALL dm2_push_cmd(" join dcd where dcd.table_name = col_doc->qual[d.seq]->table_name ",0)
  CALL dm2_push_cmd("            and dcd.column_name = col_doc->qual[d.seq]->column_name ",0)
  CALL dm2_push_cmd("with nocounter, maxcommit = 5000 go",1)
  IF ((dm_err->err_ind=0))
   COMMIT
   SET iucdp_curqual = curqual
  ELSE
   ROLLBACK
   GO TO exit_program
  ENDIF
  ...
  ENDIF
 ENDIF
 IF ((col_doc_maint->num_of_updates > 0))
  IF (currdb="DB2UDB")
  ...
  IF (dm2_set_autocommit(1)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Performing update maintenance on DM_COLUMNS_DOC."
  CALL disp_msg(" ",dm_err->logfile,0)
  CALL dm2_push_cmd("update into dm_columns_doc  dcd, ",0)
  CALL dm2_push_cmd("  (dummyt d with seq = value(size(col_doc->qual,5))) ",0)
  CALL dm2_push_cmd(" set dcd.sequence_name = col_doc->qual[d.seq]->sequence_name ",0)
  CALL dm2_push_cmd("  ,dcd.code_set = col_doc->qual[d.seq]->code_set ",0)
  CALL dm2_push_cmd("  ,dcd.description = col_doc->qual[d.seq]->description ",0)
  CALL dm2_push_cmd("  ,dcd.definition = col_doc->qual[d.seq]->definition ",0)
  CALL dm2_push_cmd("  ,dcd.flag_ind = col_doc->qual[d.seq]->flag_ind ",0)
  ...
  CALL dm2_push_cmd("  ,dcd.merge_updateable_ind = col_doc->qual[d.seq]->merge_updateable_ind ",0)
  CALL dm2_push_cmd("  ,dcd.nls_col_ind = col_doc->qual[d.seq]->nls_col_ind ",0)
  IF ((validate(col_doc->qual[1].drop_ind,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dcd.drop_ind = col_doc->qual[d.seq]->drop_ind ",0)
  ENDIF
  IF ((validate(col_doc->qual[1].merge_delete_ind,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dcd.merge_delete_ind = col_doc->qual[d.seq]->merge_delete_ind ",0)
  ENDIF
  IF ((validate(col_doc->qual[1].tz_rule_flag,- (1)) != - (1)))
   CALL dm2_push_cmd("  ,dcd.tz_rule_flag = col_doc->qual[d.seq]->tz_rule_flag ",0)
  ENDIF
  CALL dm2_push_cmd(' plan d where col_doc_maint->qual[d.seq]->maint_type = "U" ',0)
  CALL dm2_push_cmd(" join dcd where dcd.table_name = col_doc->qual[d.seq]->table_name ",0)
  CALL dm2_push_cmd("            and dcd.column_name = col_doc->qual[d.seq]->column_name ",0)
  CALL dm2_push_cmd("with nocounter, maxcommit = 5000 go",1)
  IF ((dm_err->err_ind=0))
 ...
 IF (iucdp_curqual > 0
  ...
  SET auto_ver_request->qual[1].event_reason = "DM_COLUMNS_DOC change by package install"
...
#exit_program
 ...
 SET dm_err->eproc = "Maintenance on DM_COLUMNS_DOC Complete"
 CALL final_disp_msg(iucdp_logfile_prefix)
END GO
