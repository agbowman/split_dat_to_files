CREATE PROGRAM bhs_req_04_layout:dba
 PROMPT
  "program_name" = ""
  WITH program_name
 ...
 DECLARE mf_cs6003_statuschange = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6003,"STATUSCHANGE")),
 protect
 ...
 DECLARE mf_cs6000_laboratory_cd = f8 WITH protect, constant(uar_get_code_by_cki("CKI.CODEVALUE!3081"
   ))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   WHERE expand(ml_ord_idx,1,size(request->order_qual,5),o.order_id,request->order_qual[ml_ord_idx].
    ...
    AND o.catalog_type_cd=mf_cs6000_laboratory_cd)
   JOIN (e
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   action_type_cd IN (activatestudentorder_cd, activate_cd, modify_cd, complete_cd,
   mf_cs6003_statuschange))
 ...
 FOR (x = 1 TO order_cnt)
   IF ((orders->qual[x].comment_ind=1))
    SELECT INTO "nl:"
     ...
     DETAIL
      orders->qual[x].comment = replace(replace(lt.long_text,char(013)," "),char(010)," ")
     WITH nocounter
    ;end select
    SET pt->line_cnt = 0
    SET max_length = 90
 ...
 IF ((orders->spoolout_ind=1))
  ...
  END ;Subroutine
  SUBROUTINE (sec_drawonly(ncalc=i2) =f8 WITH protect)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = sec_drawonlyabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE (sec_drawonlyabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
    ...
    IF (ncalc=rpt_render)
     ...
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
       "PLEASE DRAW & SEND TESTING TO BAYSTATE MEDICAL CENTER",char(0)))
  ...
  SUBROUTINE (print_header2abs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
   WITH protect)
    DECLARE sectionheight = f8 WITH noconstant(2.500000), private
    ...
    IF ( NOT (program1 IN ("BHS_REQ_OLO", "BHS_REQ_OCO")))
  ...
  SUBROUTINE (print_headerabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
   WITH protect)
    ...
    IF ( NOT ( NOT (program1 IN ("BHS_REQ_OLO", "BHS_REQ_RAD", "BHS_REQ_OCO"))))
     RETURN(0.0)
    ENDIF
    IF (bcontinue=0)
     SET _remallergy = 1
  ...
  SUBROUTINE (order_sectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
    ...
    ENDIF
    IF (program1 IN ("BHS_REQ_OLO", "BHS_REQ_RAD", "BHS_REQ_OCO"))
  ...
  SUBROUTINE (print_top_foot_blankabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
    ...
    IF ( NOT ( NOT (cnvtupper(program1) IN ("BHS_REQ_OLO", "BHS_REQ_RAD", "BHS_REQ_OCO"))))
     RETURN(0.0)
    ENDIF
    IF (ncalc=rpt_render)
     SET _yoffset = (offsety+ sectionheight)
  ...
  SUBROUTINE (patient_note_oloabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
    ...
    RETURN(sectionheight)
  END ;Subroutine
  SUBROUTINE (patient_note_oco(ncalc=i2) =f8 WITH protect)
    DECLARE a1 = f8 WITH noconstant(0.0), private
    SET a1 = patient_note_ocoabs(ncalc,_xoffset,_yoffset)
    RETURN(a1)
  END ;Subroutine
  SUBROUTINE (patient_note_ocoabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
    DECLARE sectionheight = f8 WITH noconstant(0.250000), private
    IF ( NOT (program1 IN ("BHS_REQ_OCO")))
     RETURN(0.0)
    ENDIF
    IF (ncalc=rpt_render)
     ...
     SET _oldfont = uar_rptsetfont(_hreport,_times12b0)
     SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
     IF (cnvtupper(program1) IN ("BHS_REQ_OCO"))
      SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
        "PLEASE BRING THIS FORM WITH YOU TO HAVE THIS TESTING DONE",char(0)))
     ENDIF
     SET _yoffset = (offsety+ sectionheight)
    ENDIF
    RETURN(sectionheight)
  ...
  SUBROUTINE initializereport(dummy)
    ...
    SET rptreport->m_reportname = "BHS_REQ_04_LAYOUT"
  ...
  IF (curqual > 0)
   ...
   FOR (prt_ord = 1 TO size(orders->qual,5))
     ...
     IF (ml_print_draw_only=1)
      SET d0 = sec_drawonly(rpt_render)
     ENDIF
