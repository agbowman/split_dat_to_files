CREATE PROGRAM dm2_readme_runner:dba
 ...
 SUBROUTINE drr_alert_killed_appl(daka_load_ind,daka_fmt_appl_id,daka_kill_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_readme_runner",".log","dm2_readme_runner")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting DM2_README_RUNNER."
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE drr_get_rdm_to_process(dgrp_got_one_out=i2(ref)) = i2
 DECLARE drr_validate_readme(dvr_valid_rdm_out=i2(ref)) = i2
 DECLARE drr_rr_file(drf_file=vc,drf_found_out=i2(ref)) = i2
 DECLARE drr_rr_script_exists(drse_script=vc,drse_script_ind_out=i2(ref)) = i2
 DECLARE drr_rr_inhouse_compile(drc_ran=i2,drc_compile_status_out=i2(ref),drc_msg_out=vc(ref)) = i2
 DECLARE drr_rr_status(s_status=vc,s_message=vc) = i2
 DECLARE drr_rr_post_run(null) = i2
 DECLARE drr_check_for_running_readmes(dcfrr_readmes_still_running=i2(ref)) = i2
 DECLARE drr_dgrp_ret = i2 WITH protect, noconstant(0)
 DECLARE drr_dvr_readme_valid_ret = i2 WITH protect, noconstant(0)
 DECLARE drr_rdm_to_process = i2 WITH protect, noconstant(0)
 DECLARE drr_drr_run_ret = i2 WITH protect, noconstant(0)
 DECLARE drr_drcs_chk_ret = i2 WITH protect, noconstant(0)
 DECLARE drr_msg_ret = vc WITH protect, noconstant(" ")
 DECLARE drr_env_hold = f8 WITH protect, noconstant(0.0)
 DECLARE drr_check_again = i2 WITH protect, noconstant(0)
 DECLARE drr_currdbhandle = vc WITH protect, noconstant(" ")
 DECLARE drr_cleanup_timestamp = f8 WITH protect, noconstant(0.0)
 DECLARE drr_runner_type = vc WITH protect, noconstant(" ")
 DECLARE drr_called_in_background = i2 WITH protect, noconstant(0)
 DECLARE drr_readmes_still_running = i2 WITH protect, noconstant(0)
 DECLARE drr_status = i2 WITH protect, noconstant(0)
 DECLARE drr_readme_execution = vc WITH protect, noconstant(" ")
 DECLARE drr_global_luts_context_ind = i2 WITH protect, noconstant(1)
 DECLARE drr_readme_luts_context_ind = i2 WITH protect, noconstant(1)
 DECLARE drr_admin_db_link = vc WITH protect, noconstant("")
 DECLARE drr_join_str = vc WITH protect, noconstant("")
 DECLARE drr_admin_currdbopt = i2 WITH protect, noconstant(1)
 DECLARE drr_fail_ind = i2 WITH protect, noconstant(0)
 ...
 IF ((dm2_rr_misc->env_id=0.0))
  IF (dm2_get_env_data(1,drr_env_hold)=0)
   GO TO exit_program
  ENDIF
  SET dm2_rr_misc->env_id = drr_env_hold
 ENDIF
 IF ((dm2_rr_misc->batch_dt_tm=cnvtdatetime("01-JAN-1800")))
  SET low_batch_dt_tm = cnvtdatetime("01-JAN-1800")
  SET high_batch_dt_tm = cnvtdatetime("31-DEC-2100")
 ELSE
  SET low_batch_dt_tm = dm2_rr_misc->batch_dt_tm
  SET high_batch_dt_tm = dm2_rr_misc->batch_dt_tm
 ENDIF
 SET drr_currdbhandle = currdbhandle
 SET drr_cleanup_timestamp = cnvtlookbehind("15,MIN",cnvtdatetimeutc(cnvtdatetime(curdate,curtime)))
 SET drr_runner_type = evaluate(dm2_rr_misc->batch_dt_tm,cnvtdatetime("01-JAN-1800"),
  "PARALLEL_RUNNER","MAIN_RUNNER")
 EXECUTE dm2_apply_session_overrides
 IF ((dm_err->err_ind > 0))
  GO TO exit_program
 ENDIF
 IF (drr_insert_runner_row("DM2_README_RUNNER",drr_currdbhandle,drr_runner_type,1,0.0)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining if readme runner initiated in the background..."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_BACKGROUND_RUNNER"
   AND di.info_name=drr_currdbhandle
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual > 0)
  SET drr_called_in_background = 1
 ENDIF
 ...
 IF (dm2_set_inhouse_domain(null)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Check for LUTS context row in dm_info"
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="FIRE_LUTS_TRG_README_CONTEXT"
   AND di.info_name="GLOBAL"
  DETAIL
   IF (di.info_char="NO")
    drr_global_luts_context_ind = 0
   ELSE
    drr_global_luts_context_ind = 1
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF (drr_global_luts_context_ind=0)
  IF (dctx_set_context("FIRE_LUTS_TRG","NO")=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dir_get_admin_db_link(0,drr_admin_db_link,drr_fail_ind)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determine admin optimizer mode"
 SELECT INTO "nl:"
  FROM (value(concat("V$PARAMETER@",drr_admin_db_link)) v)
  WHERE cnvtupper(v.name)="OPTIMIZER_MODE"
  DETAIL
   drr_admin_currdbopt = evaluate(cnvtupper(v.value),"RULE",1,8)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drr_rdm_to_process = 1
 WHILE (drr_rdm_to_process=1)
   SET drr_drcs_chk_ret = 1
   SET drr_drr_run_ret = 1
   SET drr_msg_ret = ""
   SET drr_check_again = 1
   SET drr_readme_luts_context_ind = - (1)
   WHILE (drr_check_again=1)
    IF (drr_get_rdm_to_process(drr_dgrp_ret)=0)
     GO TO exit_program
    ENDIF
    CASE (drr_dgrp_ret)
     OF 0:
      SET drr_check_again = 0
      IF (drr_called_in_background=1)
       SET drr_readmes_still_running = 0
       IF (drr_check_for_running_readmes(drr_readmes_still_running)=0)
        GO TO exit_program
       ENDIF
       IF (drr_readmes_still_running=1)
        SET drr_check_again = 1
       ENDIF
      ENDIF
      IF (drr_check_again=0)
       SET dm_err->eproc = "No available readme remains to be executed.  Exiting program."
       CALL disp_msg(" ",dm_err->logfile,0)
       GO TO exit_program
      ENDIF
     OF 1:
    ...
    ENDCASE
   ENDWHILE
   IF (drr_rr_status("","Starting from DM2_README_RUNNER.")=0)
    GO TO exit_program
   ENDIF
   IF (drr_validate_readme(drr_dvr_readme_valid_ret)=0)
    GO TO exit_program
   ENDIF
   IF (drr_dvr_readme_valid_ret=0)
    IF (drr_rr_post_run(null)=0)
     GO TO exit_program
    ENDIF
   ELSE
    SET dm_err->eproc = concat("Performing Readme Number: ",trim(cnvtstring(readme_data->readme_id),3
    ...
    IF ((dm_err->debug_flag > 1))
     ...
     CALL echo(fillstring(110,"*"))
    ENDIF
    SET dm_err->eproc = build("Check for LUTS context row in dm_info for"," readme(",trim(cnvtstring(
       readme_data->readme_id),3),"), instance(",trim(cnvtstring(readme_data->instance),3),
     ")")
    IF ((dm_err->debug_flag > 2))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
     FROM dm_info di
     WHERE di.info_domain="FIRE_LUTS_TRG_README_CONTEXT"
      AND di.info_name=patstring(concat(trim(cnvtstring(readme_data->readme_id),3),":*"))
     DETAIL
      IF ((cnvtint(substring((findstring(":",di.info_name,1,1)+ 1),(textlen(trim(di.info_name)) -
        findstring(":",di.info_name,1,1)),di.info_name)) <= readme_data->instance))
       IF (di.info_char="NO")
        drr_readme_luts_context_ind = 0
       ELSE
        drr_readme_luts_context_ind = 1
       ENDIF
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     ...
     GO TO exit_program
    ENDIF
    IF (drr_readme_luts_context_ind=0)
     EXECUTE dm2_set_context "FIRE_LUTS_TRG", "NO"
     IF ((dm_err->err_ind=1))
      GO TO exit_program
     ENDIF
    ELSEIF (drr_readme_luts_context_ind=1)
     EXECUTE dm2_set_context "FIRE_LUTS_TRG", "YES"
     IF ((dm_err->err_ind=1))
      GO TO exit_program
     ENDIF
    ENDIF
    IF ((inhouse_misc->inhouse_domain=1))
     IF (drr_rr_inhouse_compile(0,drr_drr_run_ret,drr_msg_ret)=0)
      GO TO exit_program
     ENDIF
     IF (drr_drr_run_ret=0)
      SET readme_data->message = drr_msg_ret
     ...
     CASE (readme_data->readme_type)
      ...
      OF dm2_rr_tbl_import:
       ...
       IF ((readme_data->status="F"))
        SET drr_drr_run_ret = 0
       ENDIF
      ELSE
       IF (drr_readme_execution != dm2_rr_timer)
        EXECUTE value(readme_data->script)
       ENDIF
       SET trace = norecpersist
     ENDCASE
    ENDIF
    IF (dctx_restore_prev_contexts(null)=0)
     GO TO exit_program
    ENDIF
    SET dm_err->eproc = concat("Completed Readme Number: ",trim(cnvtstring(readme_data->readme_id),3),
     ".  Description: ",readme_data->description,".")
    CALL disp_msg(" ",dm_err->logfile,0)
    IF (drr_drr_run_ret=0)
     IF (drr_rr_post_run(null)=0)
    ...
    ELSE
     IF (size(trim(readme_data->check_script,3)))
      SET drr_drcs_chk_ret = 1
      SET dm_err->eproc = concat("Starting Check script for Readme Number: ",trim(cnvtstring(
         readme_data->readme_id),3),".  Description: ",readme_data->description,".")
      CALL disp_msg(" ",dm_err->logfile,0)
      IF ((inhouse_misc->inhouse_domain=1))
       SET drr_msg_ret = ""
       IF (drr_rr_inhouse_compile(1,drr_drcs_chk_ret,drr_msg_ret)=0)
        GO TO exit_program
       ENDIF
       IF (drr_drcs_chk_ret=0)
        SET readme_data->message = drr_msg_ret
        SET dm_err->eproc = readme_data->message
        CALL disp_msg(" ",dm_err->logfile,0)
        SET readme_data->status = "F"
     ...
     ENDIF
    ENDIF
    IF (drr_drcs_chk_ret=1
     AND drr_drr_run_ret=1)
     IF (drr_rr_post_run(null)=0)
      GO TO exit_program
     ENDIF
    ENDIF
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE drr_rr_file(drf_file,drf_found_out)
   DECLARE drf_install_file = vc WITH protect, noconstant(" ")
   SET dm_err->eproc = concat("Checking if ",trim(drf_file)," exists in cer_install.")
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SET drf_found_out = 0
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET drf_install_file = build("cer_install:",drf_file)
   ELSE
    SET drf_install_file = build("cer_install/",drf_file)
   ENDIF
   IF ( NOT (findfile(drf_install_file)))
    SET drf_found_out = 0
    RETURN(1)
   ENDIF
   SET drf_found_out = 1
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drr_rr_script_exists(drse_script,drse_script_ind_out)
   SET dm_err->eproc = concat("Checking if ",trim(drse_script)," exists in dictionary.")
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    ...
    FROM dprotect p
    WHERE p.object="P"
     AND p.object_name=cnvtupper(trim(drse_script,3))
    DETAIL
     drse_script_ind_out = 1
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drr_rr_inhouse_compile(drc_ran,drc_compile_status_out,drc_msg_out)
   DECLARE drc_rrc_str = vc WITH protect, noconstant(" ")
   DECLARE drc_seq_num = f8 WITH protect, noconstant(0.0)
   DECLARE drc_skip = i2 WITH protect, noconstant(0)
   DECLARE drc_uniq_file = vc WITH private, noconstant(" ")
   DECLARE drc_uniq_output = vc WITH private, noconstant(" ")
   DECLARE drc_readme_run_logical = vc WITH private, noconstant(" ")
   DECLARE drc_rrc_exec_str = vc WITH protect, noconstant(" ")
   SET dm_err->eproc = "Get unique file."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SET drc_uniq_file = concat(trim(cnvtstring(readme_data->readme_id)),"_",trim(cnvtstring(
      readme_data->instance)),"_file")
   IF ( NOT (get_unique_file(drc_uniq_file,".txt")))
    RETURN(0)
   ELSE
    SET drc_uniq_file = concat(dm_err->unique_fname)
    SET drc_uniq_output = replace(drc_uniq_file,"_file","_output",1)
   ENDIF
   IF ((dm_err->debug_flag > 1))
    CALL echo(concat("EXECUTE FILE:",drc_uniq_file))
    CALL echo(concat("OUTPUT FILE:",drc_uniq_output))
   ENDIF
   SET dm_err->eproc = "Building execute file."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(drc_uniq_file)
    FROM dual
    HEAD REPORT
     drc_rrc_str = fillstring(150," ")
    DETAIL
     IF (drc_ran=0)
      drc_rrc_exec_str = trim(readme_data->script,3)
      CASE (readme_data->readme_type)
       OF dm2_rr_dbimport:
        drc_rrc_str = concat("execute dm_dbimport 'cer_install:",readme_data->data_file,"','",
         readme_data->script,"',",
         cnvtstring(readme_data->blocks)," go"),col 0,drc_rrc_str
       OF dm2_rr_oracle:
        drc_rrc_str = concat("execute dm_readme_oracle_import '",readme_data->data_file,"','",
         readme_data->par_file,"',0 go"),col 0,drc_rrc_str
       OF dm2_rr_oracle_ref:
        drc_rrc_str = concat("execute dm_readme_oracle_import '",readme_data->data_file,"','",
         readme_data->par_file,"',1 go"),col 0,drc_rrc_str
       OF dm2_rr_ccl_dbimport:
        drc_rrc_str = concat("execute dm_dbimport 'cer_install:",readme_data->data_file,"','",
         readme_data->script,"',",
         cnvtstring(readme_data->blocks)," go"),col 0,drc_rrc_str
       OF dm2_rr_tbl_import:
        IF (currdb="ORACLE")
         drc_rrc_str = concat("execute dm_readme_oracle_import2 '",readme_data->data_file,"','",
          readme_data->par_file,"','",
          readme_data->driver,"' go"), col 0, drc_rrc_str
        ELSEIF (currdb="DB2UDB")
         drc_rrc_str = concat("execute dm_readme_db2_import '",readme_data->data_file,"','",
          readme_data->driver,"',",
          cnvtstring(readme_data->blocks)," go"), col 0, drc_rrc_str
        ENDIF
       ELSE
        col 0,"execute ",col + 5,
        drc_rrc_exec_str,col + 1," go"
      ENDCASE
     ELSEIF (drc_ran=1)
      drc_rrc_exec_str = trim(readme_data->check_script,3), col 0, "execute ",
      col + 5, drc_rrc_exec_str, col + 1,
      " go"
     ENDIF
   ...
   ENDIF
   IF (drc_skip=1)
    SET drc_compile_status_out = 1
   ELSE
    SET dm_err->eproc = concat("Call compile for ",drc_uniq_file,". Output to ",drc_uniq_output)
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SET drc_rrc_str = concat("call compile('",drc_uniq_file,"' , '",drc_uniq_output,"') go")
    IF (dm2_push_cmd(drc_rrc_str,1)=0)
     SET dm_err->err_ind = 0
    ENDIF
    IF ((dm_err->debug_flag > 1))
     CALL echo(concat("EXECUTION STRING",drc_rrc_str))
     CALL echo(concat("PROGRAM EXECUTING",drc_rrc_exec_str))
    ENDIF
    SET drc_rrc_str = trim(drc_uniq_output,3)
    SET logical drc_readme_run_logical value(drc_rrc_str)
    FREE DEFINE rtl
    DEFINE rtl "drc_readme_run_logical"
    SET dm_err->eproc = "Looking for errors in output file."
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SELECT INTO noforms
    ...
    ENDIF
   ENDIF
   IF (drc_compile_status_out=1)
    IF ((readme_data->status="S"))
     SET stat = remove(value(drc_uniq_output))
    ENDIF
   ELSE
    IF (drc_ran=0)
     SET drc_msg_out = build(
      "FAIL: ccl errors found when readme script was ran. Check log for details:",drc_uniq_output)
    ELSE
     SET drc_msg_out = build(
    ...
    ENDIF
   ENDIF
   SET stat = remove(value(drc_uniq_file))
   IF ((dm_err->debug_flag > 1))
    CALL echo(concat("COMPILE MESSAGE",drc_msg_out))
    CALL echo(concat("COMPILE STATUS",build(drc_compile_status_out)))
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drr_rr_status(s_status,s_message)
   SET dm_err->eproc = concat(s_message,"  Number: ",trim(cnvtstring(readme_data->readme_id),3),
    ".  Description: ",readme_data->description,
    ".")
   CALL disp_msg(" ",dm_err->logfile,0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drr_get_rdm_to_process(dgrp_got_one_out)
   SET dgrp_got_one_out = 0
   ...
   IF ((dm_err->debug_flag > 1))
    CALL echo(concat("Current date and time = ",format(cnvtdatetime(curdate,curtime),
       "mm/dd/yyyy hh:mm:ss;;d")))
    CALL echo(concat("Last cleanup date and time = ",format(drr_cleanup_timestamp,
       "mm/dd/yyyy hh:mm:ss;;d")))
    CALL echo(concat("Number of minutes elapsed = ",cnvtstring(datetimediff(cnvtdatetime(curdate,
         curtime),cnvtdatetime(drr_cleanup_timestamp),4),4,3)))
   ENDIF
   IF (datetimediff(cnvtdatetimeutc(cnvtdatetime(curdate,curtime)),cnvtdatetimeutc(cnvtdatetime(
      drr_cleanup_timestamp)),4) > 5)
    SET drr_cleanup_timestamp = cnvtdatetimeutc(cnvtdatetime(curdate,curtime))
    ...
    ENDIF
    UPDATE  FROM dm_ocd_log l
     SET l.status = dm2_rr_failed, l.start_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)), l
     ...
     (SELECT
      ...
      WHERE r.readme_id=cnvtint(l.project_name)
       ...
       AND (x.environment_id=dm2_rr_misc->env_id)
    ...
    ENDIF
    COMMIT
    IF (drr_check_runner_status("DM2_README_RUNNER",drr_currdbhandle,drr_status)=0)
     RETURN(0)
    ENDIF
    IF (drr_status=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Readme runner has been marked to stop."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   SET drr_join_str = evaluate(drr_admin_currdbopt,1," cnvtreal(l.project_name) = d.readme_id",
    " l.project_name = trim(cnvtstring(d.readme_id),3)")
   SET dm_err->eproc = "Searching for next readme to execute."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    l.project_name, d.execution
    FROM dm_ocd_log l,
     dm_readme d
    WHERE (l.environment_id=dm2_rr_misc->env_id)
     AND l.project_type=dm2_rr_readme
    ...
    (SELECT
     ...
     FROM dm_readme p,
      dm_ocd_log o
     WHERE p.readme_id=d.parent_readme_id
      AND d.parent_readme_id > 0
      AND p.active_ind=1
      AND (o.environment_id=dm2_rr_misc->env_id)
      AND o.environment_id=l.environment_id
      AND o.project_type=dm2_rr_readme
      AND o.project_type=l.project_type
      AND o.project_name=trim(cnvtstring(p.readme_id),3)
      AND ((o.status != dm2_rr_done) OR (o.status=null))
      AND o.active_ind=1)))
    DETAIL
     readme_data->ocd = l.ocd, readme_data->readme_id = cnvtint(l.project_name), readme_data->
     instance = l.project_instance,
     readme_data->batch_dt_tm = l.batch_dt_tm, drr_readme_execution = d.execution
    WITH nocounter, maxqual(l,1)
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ENDIF
   IF (curqual > 0)
    SET dm_err->eproc = "Attempting to lock dm_ocd_log row."
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
    ...
    ENDIF
    IF (curqual > 0)
     SET dm_err->eproc = "Setting start_dt_tm in dm_ocd_log."
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     UPDATE  FROM dm_ocd_log dol
      SET dol.start_dt_tm = cnvtdatetime(curdate,curtime3), dol.appl_ident = trim(validate(
         currdbhandle," "))
     ...
     ENDIF
     COMMIT
     SET dgrp_got_one_out = 1
    ELSE
     SET dgrp_got_one_out = 2
     ROLLBACK
    ENDIF
    IF (dgrp_got_one_out=1)
     SET dm_err->eproc = concat("Gathering readme related information for readme ",build(readme_data
       ->readme_id))
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     SELECT INTO "NL:"
      ...
      DETAIL
       ...
       IF ((((readme_data->readme_type=dm2_rr_oracle)) OR ((((readme_data->readme_type=
       dm2_rr_oracle_ref)) OR ((readme_data->readme_type=dm2_rr_tbl_import))) )) )
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drr_validate_readme(dvr_valid_rdm_out)
   DECLARE drr_status_ret = i2 WITH protect, noconstant(0)
   SET dvr_valid_rdm_out = 0
   SET dm_err->eproc = concat("Validating Readme Number: ",trim(cnvtstring(readme_data->readme_id),3),
    ".  Description: ",readme_data->description,".")
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (size(trim(readme_data->script,3)))
    IF (drr_rr_script_exists(readme_data->script,drr_status_ret)=0)
     RETURN(0)
    ENDIF
    IF (drr_status_ret=0)
     IF (drr_rr_status("F",concat("Main script not found in CCL dictionary.  Script name: ",
       readme_data->script,"."))=0)
      RETURN(0)
     ENDIF
     SET dvr_valid_rdm_out = 0
     RETURN(1)
   ...
   ENDIF
   IF (size(trim(readme_data->check_script,3)))
    IF (drr_rr_script_exists(readme_data->check_script,drr_status_ret)=0)
     RETURN(0)
    ENDIF
    IF (drr_status_ret=0)
     IF (drr_rr_status("F",concat("Check script not found in CCL dictionary.  Script name: ",
       readme_data->check_script,"."))=0)
      RETURN(0)
     ENDIF
     SET dvr_valid_rdm_out = 0
     RETURN(1)
   ...
   IF (size(trim(readme_data->data_file,3)))
    IF (drr_rr_file(readme_data->data_file,drr_status_ret)=0)
     RETURN(0)
    ENDIF
    IF (drr_status_ret=0)
     SET dvr_valid_rdm_out = 0
     IF (drr_rr_status("F",concat("Data file not found in CER_INSTALL directory.  File: ",trim(
        readme_data->data_file,3),"."))=0)
      RETURN(0)
     ENDIF
     RETURN(1)
    ENDIF
   ...
   IF (size(trim(readme_data->par_file,3)))
    IF (drr_rr_file(readme_data->par_file,drr_status_ret)=0)
     RETURN(0)
    ENDIF
    IF (drr_status_ret=0)
     SET dvr_valid_rdm_out = 0
     IF (drr_rr_status("F",concat("Data file not found in CER_INSTALL directory.  File: ",trim(
        readme_data->par_file,3),"."))=0)
      RETURN(0)
     ENDIF
     RETURN(1)
    ENDIF
   ENDIF
   SET dvr_valid_rdm_out = 1
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drr_rr_post_run(null)
   IF ((dm_err->debug_flag > 1))
    CALL echo(
     "%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"
     )
   ...
   IF ((readme_data->status != "S")
    ...
    AND (readme_data->status != "R"))
    IF (drr_rr_status("F","Automatically failed because no overall status was logged.")=0)
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_readme_finished_ind = "T"
   IF (drr_rr_status(readme_data->status,readme_data->message)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drr_check_for_running_readmes(dcfrr_readmes_still_running)
   IF (drr_check_runner_status("DM2_README_RUNNER",drr_currdbhandle,drr_status)=0)
    RETURN(0)
   ENDIF
   IF (drr_status=0)
   ...
   ENDIF
   SET dm_err->eproc =
   "Looking for readmes that are currently running or have not been picked up yet..."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    l.project_name
    FROM dm_ocd_log l
   ...
   ELSE
    IF (curqual > 0)
     SET dcfrr_readmes_still_running = 1
     CALL echo("**************************************************")
     CALL echo(concat(format(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),"mm/dd/yyyy hh:mm:ss;;d"
        ),
       "  Waiting on other background runners to complete [parent] readmes before exiting script...")
      )
     CALL echo("**************************************************")
     CALL pause(5)
    ELSE
     SET dcfrr_readmes_still_running = 0
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_program
 EXECUTE dm2_apply_session_overrides
 CALL dctx_set_context("FIRE_LUTS_TRG","YES")
 SET dm_err->eproc = "Deleting DM_INFO runner row for DM2_README_RUNNER ..."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 DELETE  FROM dm_info di
 ...
 ;end delete
 IF ((dm_err->err_ind=0))
  IF (check_error("Removing DM_INFO runner row for DM2_README_RUNNER")=1)
   ROLLBACK
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ELSE
   COMMIT
 ...
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending DM2_README_RUNNER"
 ENDIF
 CALL final_disp_msg("dm2_readme_runner")
END GO
