CREATE PROGRAM ct_mp_get_access_mask:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_access_mask"
 ...
 SUBROUTINE getcurrentposition(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE getaccessmask(null) = null WITH protect
 DECLARE getpositioncd(null) = null WITH protect
 DECLARE getaccessmaskprotrole(null) = null WITH protect
 DECLARE getaccessmaskuserrights(null) = null WITH protect
 DECLARE checkpreference(null) = null WITH protect
 DECLARE stat = i4 WITH protect, noconstant(0)
 ...
 SET log_domain_exists_ind = checkdic("PROT_ROLE_ACCESS.LOGICAL_DOMAIN_ID","A",0)
 ...
 ENDIF
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_access_mask","Invalid JSON Request",
   PUBLIC::"reply")
  GO TO exit_script
 ENDIF
 SET stat = cnvtjsontorec( $JSONREQUEST)
 CALL getaccessmask(null)
 FREE RECORD request
 SET public::"reply"->status_data.status = "S"
 SUBROUTINE getaccessmask(null)
   CALL log_message("In GetProtActivationDate()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE accesscount = i4 WITH protect, noconstant(size(request->access_rights_list,5))
   DECLARE accessindex = i4 WITH protect, noconstant(0)
   DECLARE protocolviewsecurity = i2 WITH protect, noconstant(0)
   DECLARE patientlistsecurity = i2 WITH protect, noconstant(0)
   IF (accesscount > 0)
    SET stat = alterlist(public::"reply"->"access_rights_list",accesscount)
    SET ct_get_pref_request->pref_entry = "protocol_view_security"
    EXECUTE ct_get_pref  WITH replace("REQUEST_STRUCT","CT_GET_PREF_REQUEST"), replace("REPLY",
     "CT_GET_PREF_REPLY")
    SET protocolviewsecurity = ct_get_pref_reply->pref_value
    SET stat = initrec(ct_get_pref_request)
    SET stat = initrec(ct_get_pref_reply)
    SET ct_get_pref_request->pref_entry = "patient_list_security"
    EXECUTE ct_get_pref  WITH replace("REQUEST_STRUCT","CT_GET_PREF_REQUEST"), replace("REPLY",
     "CT_GET_PREF_REPLY")
    SET patientlistsecurity = ct_get_pref_reply->pref_value
    SET stat = initrec(ct_get_pref_request)
    SET stat = initrec(ct_get_pref_reply)
    SET ct_get_pref_request->pref_entry = "screener_pref"
    EXECUTE ct_get_pref  WITH replace("REQUEST_STRUCT","CT_GET_PREF_REQUEST"), replace("REPLY",
     "CT_GET_PREF_REPLY")
    SET public::"reply"->"screener_pref" = ct_get_pref_reply->pref_value
    FREE RECORD ct_get_pref_request
    FREE RECORD ct_get_pref_reply
    CALL getpositioncd(null)
    CALL getaccessmaskprotrole(null)
    CALL getaccessmaskuserrights(null)
    CALL checkpreference(null)
   ELSE
    CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_access_mask",
     "Did not receive any functionality",PUBLIC::"reply")
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit GetProtActivationDate(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getpositioncd(null)
   CALL log_message("In GetPositionCd()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   ...
   ELSE
    CALL echo(build("Default position lookup failed with status: ",sac_cur_pos_rep->status_data.
      status))
   ENDIF
   CALL log_message(build("Exit GetPositionCd(), Elapsed time in seconds:",datetimediff(cnvtdatetime(
       sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getaccessmaskprotrole(null)
   CALL log_message("In GetAccessMaskProtRole()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   SELECT
    FROM code_value cv,
     prot_role pr,
     prot_role_access pra,
     (dummyt d  WITH seq = value(accesscount))
    PLAN (d)
     JOIN (pr
     WHERE (pr.prot_amendment_id=request->amendment_id)
     ...
     JOIN (cv
     WHERE (cv.cdf_meaning=request->access_rights_list[d.seq].functionality)
      AND cv.code_set=17311)
     JOIN (pra
     WHERE pr.prot_role_cd=pra.prot_role_cd
      AND pra.functionality_cd=cv.code_value
      AND pra.end_effective_dt_tm=cnvtdatetime("31-dec-2100 00:00:00.00")
      AND parser(log_domain_where_str))
    DETAIL
     public::"reply"->access_rights_list[d.seq]."functionality" = request->access_rights_list[d.seq].
     functionality
     IF (findstring("R",pra.access_mask) > 0
      AND findstring("R",public::"reply"->access_rights_list[d.seq]."access_right")=0)
      public::"reply"->access_rights_list[d.seq]."access_right" = build(public::"reply"->
       access_rights_list[d.seq]."access_right","R")
     ENDIF
     IF (findstring("C",pra.access_mask) > 0
      AND findstring("C",public::"reply"->access_rights_list[d.seq]."access_right")=0)
      public::"reply"->access_rights_list[d.seq]."access_right" = build(public::"reply"->
       access_rights_list[d.seq]."access_right","C")
     ENDIF
     IF (findstring("U",pra.access_mask) > 0
      AND findstring("U",public::"reply"->access_rights_list[d.seq]."access_right")=0)
      public::"reply"->access_rights_list[d.seq]."access_right" = build(public::"reply"->
       access_rights_list[d.seq]."access_right","U")
     ENDIF
     IF (findstring("D",pra.access_mask) > 0
      AND findstring("D",public::"reply"->access_rights_list[d.seq]."access_right")=0)
      public::"reply"->access_rights_list[d.seq]."access_right" = build(public::"reply"->
       access_rights_list[d.seq]."access_right","D")
     ENDIF
     IF (findstring("E",pra.access_mask) > 0
      AND findstring("E",public::"reply"->access_rights_list[d.seq]."access_right")=0)
      public::"reply"->access_rights_list[d.seq]."access_right" = build(public::"reply"->
       access_rights_list[d.seq]."access_right","E")
     ENDIF
    WITH nocounter
   ;end select
   CALL log_message(build("Exit GetAccessMaskProtRole(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getaccessmaskuserrights(null)
   CALL log_message("In GetAccessMaskUserRights()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   CALL echo(protocolviewsecurity)
   SELECT
    ...
    PLAN (d)
     ...
     WHERE (cv.cdf_meaning=request->access_rights_list[d.seq].functionality)
      AND cv.code_set=17311)
     JOIN (ea
     WHERE (ea.prot_amendment_id=request->amendment_id)
      AND (ea.person_id=reqinfo->updt_id)
      AND ea.functionality_cd=cv.code_value
      AND ea.end_effective_dt_tm=cnvtdatetime("31-dec-2100 00:00:00.00"))
    DETAIL
     IF (findstring("R",ea.access_mask) > 0
      AND findstring("R",public::"reply"->access_rights_list[d.seq]."access_right")=0)
      public::"reply"->access_rights_list[d.seq]."access_right" = build(public::"reply"->
       access_rights_list[d.seq]."access_right","R")
     ELSEIF (findstring("R",ea.access_mask)=0)
      IF (findstring("R",public::"reply"->access_rights_list[d.seq]."access_right") > 0)
       public::"reply"->access_rights_list[d.seq]."access_right" = removechar(public::"reply"->
        access_rights_list[d.seq]."access_right","R")
      ENDIF
     ENDIF
     IF (findstring("C",ea.access_mask) > 0
      AND findstring("C",public::"reply"->access_rights_list[d.seq]."access_right")=0)
      public::"reply"->access_rights_list[d.seq]."access_right" = build(public::"reply"->
       access_rights_list[d.seq]."access_right","C")
     ELSEIF (findstring("C",ea.access_mask)=0)
      IF (findstring("C",public::"reply"->access_rights_list[d.seq]."access_right") > 0)
       public::"reply"->access_rights_list[d.seq]."access_right" = removechar(public::"reply"->
        access_rights_list[d.seq]."access_right","C")
      ENDIF
     ENDIF
     IF (findstring("U",ea.access_mask) > 0
      AND findstring("U",public::"reply"->access_rights_list[d.seq]."access_right")=0)
      public::"reply"->access_rights_list[d.seq]."access_right" = build(public::"reply"->
       access_rights_list[d.seq]."access_right","U")
     ELSEIF (findstring("U",ea.access_mask)=0)
      IF (findstring("U",public::"reply"->access_rights_list[d.seq]."access_right") > 0)
       public::"reply"->access_rights_list[d.seq]."access_right" = removechar(public::"reply"->
        access_rights_list[d.seq]."access_right","U")
      ENDIF
     ENDIF
     IF (findstring("D",ea.access_mask) > 0
      AND findstring("D",public::"reply"->access_rights_list[d.seq]."access_right")=0)
      public::"reply"->access_rights_list[d.seq]."access_right" = build(public::"reply"->
       access_rights_list[d.seq]."access_right","D")
     ELSEIF (findstring("D",ea.access_mask)=0)
      IF (findstring("D",public::"reply"->access_rights_list[d.seq]."access_right") > 0)
       public::"reply"->access_rights_list[d.seq]."access_right" = removechar(public::"reply"->
        access_rights_list[d.seq]."access_right","D")
      ENDIF
     ENDIF
     IF (findstring("E",ea.access_mask) > 0
      AND findstring("E",public::"reply"->access_rights_list[d.seq]."access_right")=0)
      public::"reply"->access_rights_list[d.seq]."access_right" = build(public::"reply"->
       access_rights_list[d.seq]."access_right","E")
     ELSEIF (findstring("E",ea.access_mask)=0)
      IF (findstring("E",public::"reply"->access_rights_list[d.seq]."access_right") > 0)
       public::"reply"->access_rights_list[d.seq]."access_right" = removechar(public::"reply"->
        access_rights_list[d.seq]."access_right","E")
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   CALL log_message(build("Exit GetAccessMaskUserRights(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
 SUBROUTINE checkpreference(null)
   FOR (accessindex = 1 TO accesscount)
    IF ((public::"reply"->access_rights_list[accessindex]."functionality"=""))
     SET public::"reply"->access_rights_list[accessindex]."functionality" = request->
     access_rights_list[accessindex].functionality
    ENDIF
    IF (findstring("R",public::"reply"->access_rights_list[accessindex]."access_right")=0)
     IF (((protocolviewsecurity=0
      AND (((request->access_rights_list[accessindex].functionality="ROLE")) OR ((request->
     access_rights_list[accessindex].functionality="PROTOCOL"))) ) OR (patientlistsecurity=0
      AND (((request->access_rights_list[accessindex].functionality="PATIENTENROL")) OR ((request->
     access_rights_list[accessindex].functionality="PRESCREEN"))) )) )
      SET public::"reply"->access_rights_list[accessindex]."access_right" = build(public::"reply"->
       access_rights_list[accessindex]."access_right","R")
     ENDIF
    ENDIF
   ENDFOR
 END ;Subroutine
 SUBROUTINE (removechar(parentstr=vc(val),char=c1(val)) =vc WITH protect)
   CALL log_message("In ReplaceChar()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   DECLARE parentstrlen = i2 WITH constant(textlen(parentstr)), private
   DECLARE childstr = vc WITH noconstant(""), private
   IF (parentstrlen > 0)
    DECLARE charpos = i2 WITH constant(findstring(char,parentstr)), private
    IF (charpos > 0)
     CALL echo(substring(1,(charpos - 1),parentstr))
     SET childstr = trim(build(substring(1,(charpos - 1),parentstr),substring((charpos+ 1),(
        parentstrlen - charpos),parentstr)))
    ENDIF
   ENDIF
   CALL log_message(build("Exit ReplaceChar(), Elapsed time in seconds:",datetimediff(cnvtdatetime(
       sysdate),begin_date_time,5)),log_level_debug)
   RETURN(trim(childstr))
 END ;Subroutine
#exit_script
 IF (g_debug_ind=1)
