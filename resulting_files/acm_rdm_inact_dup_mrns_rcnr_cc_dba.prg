CREATE PROGRAM acm_rdm_inact_dup_mrns_rcnr_cc:dba
 ...
 SET ms_child_c_info_domain_nm = "ACM_RDM_INACT_DUP_MRNS_RCNR2"
 SET ms_child_c_max_name = "MAX ENCNTR_ALIAS_ID EVALUATED"
 CALL sbr_get_min_max(mf_min_id,mf_max_id)
 CALL echo("Processing acm_rdm_inact_dup_mrns_rcnr_cc...")
 CALL parser("rdb asis(^    update encntr_alias ea1 ^)")
 CALL parser(concat("asis(^ set ea1.ACTIVE_IND = 0, ea1.ACTIVE_STATUS_CD = ",cnvtstring(dinactivecd),
   ", ^)"))
 CALL parser(concat("asis(^ ea1.updt_applctx = ",cnvtstring(reqinfo->updt_applctx),", ^)"))
 CALL parser("asis(^        ea1.updt_dt_tm = CURRENT_DATE, ^)")
 CALL parser("asis(^        ea1.updt_cnt = ea1.updt_cnt + 1, ^)")
 CALL parser(concat("asis(^ ea1.updt_id = ",cnvtstring(reqinfo->updt_id),", ^)"))
 CALL parser(concat("asis(^ ea1.updt_task = ",cnvtstring(reqinfo->updt_task)," ^)"))
 CALL parser(
  "asis(^        where ea1.encntr_alias_id in (select ea2.encntr_alias_id from encntr_alias ea2 ^)")
 CALL parser(concat("asis(^                               where ea2.encntr_alias_type_cd = ",
   cnvtstring(dmrncd)," ^)"))
 CALL parser("asis(^                                      and ea2.active_ind = 1 ^)")
 CALL parser(
  "asis(^                                      and ea2.encntr_id in (select ea3.encntr_id ^)")
 CALL parser(
  "asis(^                                                            from encntr_alias ea3 ^)")
 CALL parser(concat(
   "asis(^                                           where ea3.encntr_alias_type_cd = ",cnvtstring(
    dmrncd)," ^)"))
 CALL parser(
  "asis(^                                                              and ea3.active_ind = 1 ^)")
 CALL parser(
  "asis(^                                   and ea3.beg_effective_dt_tm <= CURRENT_DATE ^)")
 CALL parser("asis(^                                  and ea3.end_effective_dt_tm > CURRENT_DATE ^)")
 CALL parser(
  "asis(^                                   and ea3.updt_dt_tm >= TO_DATE('2014-07-01','YYYY-MM-DD') ^)"
  )
 CALL parser(
  "asis(^           and ea3.encntr_id in (select encntr_id from pm_hist_tracking where transaction_type_txt = 'RCNR') ^)"
  )
 CALL parser(
  "asis(^                                                        group by ea3.encntr_id, ea3.alias, ea3.alias_pool_cd ^)"
  )
 CALL parser(
  "asis(^                                                            having count(encntr_id) > 1)) ^)"
  )
 CALL parser("asis(^         and ea1.encntr_alias_id not in  ^)")
 CALL parser(
  "asis(^                                     (select min(ea2.encntr_alias_id) from encntr_alias ea2 ^)"
  )
 CALL parser(concat("asis(^                               where ea2.encntr_alias_type_cd = ",
   cnvtstring(dmrncd)," ^)"))
 CALL parser("asis(^                                      and ea2.active_ind = 1 ^)")
 CALL parser(
  "asis(^                                      and ea2.encntr_id in (select distinct ea3.encntr_id ^)"
  )
 CALL parser(
  "asis(^                                                            from encntr_alias ea3 ^)")
 CALL parser(concat(
 ...
 CALL parser(
  "asis(^                                                             and ea3.active_ind = 1 ^)")
 CALL parser("asis(^                    and ea3.beg_effective_dt_tm <= CURRENT_DATE ^)")
 CALL parser("asis(^                  and ea3.end_effective_dt_tm > CURRENT_DATE ^)")
 CALL parser("asis(^                   and ea3.updt_dt_tm >= TO_DATE('2014-07-01','YYYY-MM-DD') ^)")
 CALL parser(
  "asis(^           and ea3.encntr_id in (select encntr_id from pm_hist_tracking where transaction_type_txt = 'RCNR') ^)"
  )
 CALL parser(
 ...
 CALL parser("asis(^                                      group by ea2.encntr_id ) ^)")
 CALL parser(concat("asis(^   and ea1.encntr_alias_id between ",cnvtstring(mf_min_id)," and ",
   cnvtstring(mf_max_id)," ^)"))
 CALL parser("asis(^          go ^)")
