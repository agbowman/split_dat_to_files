CREATE PROGRAM cp_br_add_component_json:dba
 PROMPT
 ...
 FREE RECORD add_comp_request
 RECORD add_comp_request(
   1 cp_node_id = f8
 ...
 FREE RECORD add_comp_reply
 RECORD add_comp_reply(
   1 cp_node_id = f8
   1 br_wizard_name = vc
   1 component_list[*]
     2 comp_type_cd = f8
 ...
 DECLARE comptypecd = f8 WITH noconstant(0), protect
 DECLARE compseqtxt = vc WITH constant( $COMPSEQTXT), protect
 ...
 SUBROUTINE insertrows(null)
   ...
   DECLARE complistsize = i4 WITH protect, noconstant(size(component_json->component_type_code_list,5
     ))
   SET stat = alterlist(add_comp_request->component_list,complistsize)
   IF (iscomptypelistjson=1)
    FOR (x = 1 TO complistsize)
      SET comptypecd = uar_get_code_by("MEANING",comp_type_cd_codeset,component_json->
       component_type_code_list[x].type_code)
      SET add_comp_request->cp_node_id = nodeid
      SET add_comp_request->component_list[x].comp_type_cd = comptypecd
      SET add_comp_request->component_list[x].component_seq_txt = compseqtxt
    ENDFOR
    EXECUTE cp_br_add_component  WITH replace(request,add_comp_request), replace(reply,add_comp_reply
     )
   ENDIF
   IF (validate(debug_ind,0)=1)
    CALL echo(concat("compListSize: ",cnvtstring(complistsize)))
   ENDIF
   IF ((add_comp_reply->status_data.status="S"))
    COMMIT
    CALL alterlist(mpagereply->component_list,complistsize)
    SET mpagereply->cp_node_id = add_comp_reply->cp_node_id
    IF (size(add_comp_reply->component_list,5) > 0)
     FOR (y = 1 TO complistsize)
       SET mpagereply->component_list[y].comp_type_cd = add_comp_reply->component_list[y].
       comp_type_cd
       SET mpagereply->component_list[y].comp_type_disp = uar_get_code_display(add_comp_reply->
        component_list[y].comp_type_cd)
       SET mpagereply->component_list[y].comp_type_cd_meaning = uar_get_code_meaning(add_comp_reply->
        component_list[y].comp_type_cd)
       SET mpagereply->component_list[y].component_seq_txt = add_comp_reply->component_list[y].
       component_seq_txt
       SET mpagereply->component_list[y].cp_component_id = add_comp_reply->component_list[y].
       cp_component_id
       SET mpagereply->component_list[y].br_wizard_name = add_comp_reply->br_wizard_name
     ENDFOR
     SET stat = moverec(add_comp_reply->status_data,mpagereply->status_data)
    ENDIF
   ELSE
    SET mpagereply->status_data.status = add_comp_reply->status_data.status
    SET stat = moverec(add_comp_reply->status_data,mpagereply->status_data)
   ENDIF
   CALL log_message(build("Exit insertRows(), Elapsed time in seconds:",((curtime3 - begin_curtime3)
     / 100.0)),log_level_debug)
 END ;Subroutine
