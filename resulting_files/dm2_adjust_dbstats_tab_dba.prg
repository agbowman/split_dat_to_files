CREATE PROGRAM dm2_adjust_dbstats_tab:dba
 ...
 SUBROUTINE dm2_adb_check(dac_db_link,dac_adb_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dadt_inadjtype = vc WITH protect, noconstant(" ")
 DECLARE dadt_inowner = vc WITH protect, noconstant(" ")
 DECLARE dadt_intable = vc WITH protect, noconstant(" ")
 DECLARE dadt_instatid = vc WITH protect, noconstant(" ")
 DECLARE dadt_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dadt_cmd = vc WITH protect, noconstant(" ")
 DECLARE dadt_step_beg_dttm = dq8 WITH protect, noconstant(0.0)
 DECLARE dadt_step_end_dttm = dq8 WITH protect, noconstant(0.0)
 DECLARE dadt_hidate_active_ind = i2 WITH protect, noconstant(1)
 DECLARE dadt_hidate_qualifier_offset = i4 WITH protect, noconstant(3)
 DECLARE dadt_hidate_min_bump_days = i4 WITH protect, noconstant(5)
 DECLARE dadt_hidate_max_bump_days = i4 WITH protect, noconstant(50)
 DECLARE dadt_hidate_formula_element1 = f8 WITH protect, noconstant(0.1)
 DECLARE dadt_hilow_active_ind = i2 WITH protect, noconstant(1)
 DECLARE dadt_newhist_active_ind = i2 WITH protect, noconstant(1)
 DECLARE dadt_hilow_lowdate_min = vc WITH protect, noconstant("31-12-1899")
 DECLARE dadt_hilow_highdate_max = vc WITH protect, noconstant("01-01-2101")
 DECLARE dadt_minmax_active_ind = i2 WITH protect, noconstant(1)
 DECLARE dadt_cnt = i4 WITH protect, noconstant(0)
 DECLARE dadt_stat_tab = vc WITH protect, noconstant(" ")
 DECLARE dadt_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dadt_emsg = vc WITH protect, noconstant(" ")
 DECLARE dadt_cum_epn_delta = f8 WITH protect, noconstant(0.0)
 DECLARE dadt_prev_epn = f8 WITH protect, noconstant(0.0)
 DECLARE dadt_lock_available_ind = i2 WITH protect, noconstant(0)
 DECLARE dadt_lock_obtained_ind = i2 WITH protect, noconstant(0)
 DECLARE dadt_pub_val = vc WITH protect, noconstant("TRUE")
 DECLARE dadt_skip_ind = i2 WITH protect, noconstant(0)
 DECLARE dadt_retry_attempts = i2 WITH protect, noconstant(0)
 DECLARE dadt_retry_ceiling = i2 WITH protect, noconstant(0)
 DECLARE dadt_allow_publish = i2 WITH protect, noconstant(0)
 DECLARE dadt_blocking_str = vc WITH protect, noconstant("")
 DECLARE dadt_in_parse_ind = i2 WITH protect, noconstant(0)
 DECLARE dadt_wait_pref_ind = i2 WITH protect, noconstant(0)
 DECLARE dadt_object_id = f8 WITH protect, noconstant(0.0)
 DECLARE dadt_adb_ind = i2 WITH protect, noconstant(0)
 DECLARE delete_column_stats(ownname=vc,tabname=vc,colname=vc,stattab=vc,statid=vc,
  statown=vc,col_stat_type=vc) = null WITH sql = "SYS.DBMS_STATS.DELETE_COLUMN_STATS", parameter
 DECLARE boil_raw_to_date(i_val=vc) = null WITH sql = "V500.DM_DBMS_STATS.BOIL_RAW_TO_DATE"
 DECLARE get_column_stats(i_owner=vc,i_table_name=vc,i_column_name=vc,i_datatype=vc,i_stattab=vc,
  i_statid=vc,i_stattype=vc) = dq8 WITH sql = "V500.DM_DBMS_STATS.GET_COLUMN_STATS", parameter
 DECLARE bump_high_date_column_stats(i_stattab=vc,i_statid=vc,i_owner=vc,i_table_name=vc,
  i_column_name=vc,
  i_number_of_days=i4) = null WITH sql = "V500.DM_DBMS_STATS.BUMP_HIGH_DATE_COLUMN_STATS", parameter
 DECLARE adjust_high_low_column_stats(i_stattab=vc,i_statid=vc,i_owner=vc,i_table_name=vc,
  i_column_name=vc) = null WITH sql = "V500.DM_DBMS_STATS.ADJUST_HIGH_LOW_COLUMN_STATS", parameter
 DECLARE squeeze_minmax(i_stattab=vc,i_statid=vc,i_owner=vc,i_table_name=vc,i_column_name=vc) = null
 WITH sql = "V500.DM_DBMS_STATS.SQUEEZE_MINMAX", parameter
 DECLARE set_table_prefs(ownname=vc,tabname=vc,pname=vc,pvalue=vc) = null WITH sql =
 "SYS.DBMS_STATS.SET_TABLE_PREFS", parameter
 DECLARE resolve_histogram(arh_statown_in=vc,arh_stattab_in=vc,arh_statid_in=vc,arh_owner_in=vc,
  arh_table_name=vc,
  arh_column_name=vc,arh_data_type_in=vc) = null WITH sql = "V500.DM_DBMS_STATS.AH_RESOLVE_HISTOGRAM",
 parameter
 DECLARE get_hist_type(aght_statown_in=vc,aght_stattab_in=vc,aght_statid_in=vc,aght_owner_in=vc,
  aght_table_name_in=vc,
  aght_column_name_in=vc) = c255 WITH sql = "V500.DM_DBMS_STATS.AH_GET_HIST_TYPE", parameter
 FREE RECORD dadt_date
 RECORD dadt_date(
   1 cnt = i4
   1 cur_date_time = dq8
   1 qual[*]
     ...
     2 hidate_value = dq8
     2 lowdate_value = dq8
     2 hidate_value_char = vc
     2 lowdate_value_char = vc
     2 process_hidate_ind = i2
     2 process_hilow_ind = i2
     2 process_minmax_ind = i2
     2 days_to_bump = i4
 )
 FREE RECORD dadt_newhist
 RECORD dadt_newhist(
   1 cnt = i4
   1 qual[*]
     2 table_owner = vc
     2 table_name = vc
     2 column_name = vc
     2 process_newhist_ind = i2
     2 data_type = vc
 )
 FREE RECORD dadt_procs
 RECORD dadt_procs(
   1 cnt = i4
   1 qual[*]
     2 proc_name = vc
 ...
 IF (check_logfile("dm2_adbs_tab",".log","dm2_adjust_dbstats_tab LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_adjust_dbstats_tab"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dadt_inadjtype = trim(cnvtupper( $1),3)
 SET dadt_inowner = trim(cnvtupper( $2),3)
 SET dadt_intable = trim( $3,3)
 SET dadt_instatid = trim(cnvtupper( $4),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg =
  "Parameter usage: dm2_adjust_dbstats_tab  '<Adjust Type>','<table owner>','<table_name>','<STATID>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dgdt_prefs->context_lock_ind=- (1)))
 ...
 CALL dm2_process_log_add_detail_text("ADJUST TYPE",dadt_inadjtype)
 CALL dm2_process_log_add_detail_text("OWNER",dadt_inowner)
 CALL dm2_process_log_add_detail_text("TABLE",dadt_intable)
 CALL dm2_process_log_add_detail_text("TABLE_OWNER.TABLE_NAME",concat(dadt_inowner,".",dadt_intable))
 CALL dm2_process_log_add_detail_text("STATID",dadt_instatid)
 ...
 SET dadt_log_id = dm2_process_event_rs->dm_process_event_id
 IF (ddf_autosuccess_init(null)=0)
  GO TO exit_script
 ELSEIF ((dgdt_prefs->autosuccess_ind=1))
  GO TO exit_script
 ENDIF
 IF (dadt_inadjtype IN ("ALL", "HIDATE"))
  SET dm_err->eproc = "Retrieving ADJUSTHIDATE configurables from DM_INFO"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:ADJUSTHIDATE"
   DETAIL
    CASE (d.info_name)
     OF "STATUS":
      dadt_hidate_active_ind = d.info_number
     OF "QUALIFIER_OFFSET":
      dadt_hidate_qualifier_offset = d.info_number
     OF "MIN_BUMP_DAYS":
      dadt_hidate_min_bump_days = d.info_number
     OF "MAX_BUMP_DAYS":
      dadt_hidate_max_bump_days = d.info_number
     OF "FORMULA_ELEMENT1":
      dadt_hidate_formula_element1 = d.info_number
    ENDCASE
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
  ...
  ENDIF
 ELSE
  SET dadt_hidate_active_ind = 0
 ENDIF
 CALL echo(build("dadt_hidate_active_ind:",dadt_hidate_active_ind))
 CALL echo(build("dadt_hidate_qualifier_offset:",dadt_hidate_qualifier_offset))
 CALL echo(build("dadt_hidate_min_bump_days:",dadt_hidate_min_bump_days))
 CALL echo(build("dadt_hidate_max_bump_days:",dadt_hidate_max_bump_days))
 CALL echo(build("dadt_hidate_formula_element1:",dadt_hidate_formula_element1))
 IF (ddf_get_publish_retry(dadt_retry_ceiling)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_adb_check("",dadt_adb_ind)=0)
  GO TO exit_script
 ENDIF
 IF (ddf_check_wait_pref(dadt_wait_pref_ind,dadt_adb_ind)=0)
  GO TO exit_script
 ENDIF
 IF (dadt_inadjtype IN ("ALL", "MINMAX"))
  SET dm_err->eproc = "Retrieving SQUEEZEMINMAX configurables from DM_INFO"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:SQUEEZEMINMAX"
    AND d.info_name="STATUS"
   DETAIL
    dadt_minmax_active_ind = d.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ELSE
  SET dadt_minmax_active_ind = 0
 ENDIF
 IF (dadt_inadjtype IN ("ALL", "HILOW"))
  SET dm_err->eproc = "Retrieving ADJUSTHILOW configurables from DM_INFO"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:ADJUSTHILOW"
   DETAIL
    CASE (d.info_name)
     OF "STATUS":
      dadt_hilow_active_ind = d.info_number
     OF "HIGHDATE_MAX":
      dadt_hilow_highdate_max = d.info_char
     OF "LOWDATE_MIN":
      dadt_hilow_lowdate_min = d.info_char
    ENDCASE
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
  ...
  ENDIF
 ELSE
  SET dadt_hilow_active_ind = 0
 ENDIF
 IF (dadt_inadjtype IN ("ALL", "NEWHIST")
  AND dadt_instatid != "LIVE")
  SET dm_err->eproc = "Retrieving NEWHIST configurables from DM_INFO"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:NEWHIST"
   DETAIL
    CASE (d.info_name)
     OF "STATUS":
      dadt_newhist_active_ind = d.info_number
    ENDCASE
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
  ...
  ENDIF
 ELSE
  SET dadt_newhist_active_ind = 0
 ENDIF
 CALL echo(build("dadt_newhist_active_ind:",dadt_newhist_active_ind))
 CALL echo(build("dadt_minmax_active_ind:",dadt_minmax_active_ind))
 CALL echo(build("dadt_hilow_active_ind:",dadt_hilow_active_ind))
 CALL echo(build("dadt_hilow_lowdate_min:",dadt_hilow_lowdate_min))
 CALL echo(build("dadt_hilow_highdate_max:",dadt_hilow_highdate_max))
 SET dadt_date->cur_date_time = cnvtdatetime(curdate,curtime3)
 SET dadt_cmd = concat("RDB ASIS(^ begin SYS.DBMS_SESSION.RESET_PACKAGE; end; ^) go")
 IF (dm2_push_cmd(dadt_cmd,1)=0)
  ...
  ENDIF
 ENDIF
 IF (dadt_instatid="LIVE")
  SET dadt_stat_tab = "LIVE"
 ELSE
  SET dadt_stat_tab = "DM_STAT_TABLE"
 ENDIF
 IF (dadt_instatid="LIVE")
  IF (((dadt_hidate_active_ind=1) OR (((dadt_hilow_active_ind=1) OR (dadt_minmax_active_ind=1)) )) )
   SET dm_err->eproc = "Load DATE adjustment candidates into memory from LIVE dictionary"
   SELECT INTO "nl:"
    h_val = sqlpassthru(
     "to_char(dm_dbms_stats.boil_raw_to_date(dtc.high_value),'DD-MM-YYYY HH24:MI:SS')",50), l_val =
    sqlpassthru("to_char(dm_dbms_stats.boil_raw_to_date(dtc.low_value),'DD-MM-YYYY HH24:MI:SS')",50)
    FROM dba_tab_columns dtc,
     dba_tables dt
    WHERE  NOT (dtc.histogram IN ("FREQUENCY", "TOP-FREQUENCY"))
     AND dtc.data_type="DATE"
     AND dtc.high_value IS NOT null
     AND dtc.low_value IS NOT null
     AND dtc.owner=patstring(dadt_inowner)
     AND dtc.table_name=patstring(dadt_intable)
     AND dtc.owner=dt.owner
     AND dtc.table_name=dt.table_name
     AND dt.owner=patstring(dadt_inowner)
     AND dt.table_name=patstring(dadt_intable)
     AND  NOT ( EXISTS (
    (SELECT
     "x"
     FROM dm_info
     WHERE info_domain="DM2_ORACLE_USER"
      AND info_name=dtc.owner)))
    ORDER BY dtc.owner, dtc.table_name, dtc.column_name
    HEAD REPORT
     dadt_date->cnt = 0, stat = alterlist(dadt_date->qual,dadt_date->cnt)
    DETAIL
     dadt_date->cnt = (dadt_date->cnt+ 1), stat = alterlist(dadt_date->qual,dadt_date->cnt),
     dadt_date->qual[dadt_date->cnt].table_owner = dtc.owner,
     dadt_date->qual[dadt_date->cnt].table_name = dtc.table_name, dadt_date->qual[dadt_date->cnt].
     column_name = dtc.column_name, dadt_date->qual[dadt_date->cnt].last_analyzed = dt.last_analyzed,
     dadt_date->qual[dadt_date->cnt].hidate_value_char = h_val, dadt_date->qual[dadt_date->cnt].
     lowdate_value_char = l_val
     IF ((dadt_date->qual[dadt_date->cnt].hidate_value_char != patstring("00-*-0000 00:00:00")))
      dadt_date->qual[dadt_date->cnt].hidate_value = cnvtdatetimeutc(cnvtdatetime(cnvtdate2(dadt_date
         ->qual[dadt_date->cnt].hidate_value_char,"DD-MM-YYYY"),0),2), dadt_date->qual[dadt_date->cnt
      ].lowdate_value = cnvtdatetimeutc(cnvtdatetime(cnvtdate2(dadt_date->qual[dadt_date->cnt].
         lowdate_value_char,"DD-MM-YYYY"),0),2)
     ENDIF
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc) != 0)
 ...
 IF (dadt_instatid != "LIVE")
  IF (((dadt_hidate_active_ind=1) OR (((dadt_hilow_active_ind=1) OR (dadt_minmax_active_ind=1)) )) )
   SET dm_err->eproc = "Load Column adjustment candidates into memory from DM_STAT_TABLE"
   SELECT INTO "nl:"
    d.c5, d.c1, d.c4,
    d.d1, d.n10, h_val = sqlpassthru(
     "to_char(dm_dbms_stats.boil_raw_to_date(d.r2),'DD-MM-YYYY HH24:MI:SS')",50),
    l_val = sqlpassthru("to_char(dm_dbms_stats.boil_raw_to_date(d.r1),'DD-MM-YYYY HH24:MI:SS')",50)
    FROM dm_stat_table d
    WHERE d.statid=dadt_instatid
     AND d.c5=patstring(dadt_inowner)
     AND d.c1=patstring(dadt_intable)
     AND d.type="C"
     AND d.c4 IN (
    (SELECT
     c.column_name
     ...
     WHERE c.owner=d.c5
      AND c.table_name=d.c1
      AND c.data_type="DATE"
      AND c.owner=patstring(dadt_inowner)
      AND c.table_name=patstring(dadt_intable)))
     AND d.r2 IS NOT null
     AND d.r1 IS NOT null
    ORDER BY d.c5, d.c1, d.c4,
     d.n10
    HEAD REPORT
     dadt_date->cnt = 0, stat = alterlist(dadt_date->qual,dadt_date->cnt)
    HEAD d.c5
    ...
    HEAD d.c4
     dadt_date->cnt = (dadt_date->cnt+ 1), stat = alterlist(dadt_date->qual,dadt_date->cnt),
     dadt_date->qual[dadt_date->cnt].table_owner = d.c5,
     dadt_date->qual[dadt_date->cnt].table_name = d.c1, dadt_date->qual[dadt_date->cnt].column_name
      = d.c4, dadt_date->qual[dadt_date->cnt].last_analyzed = d.d1,
     dadt_date->qual[dadt_date->cnt].lowdate_value_char = l_val, dadt_date->qual[dadt_date->cnt].
     hidate_value_char = h_val
     IF ((dadt_date->qual[dadt_date->cnt].hidate_value_char != patstring("00-*-0000 00:00:00")))
      dadt_date->qual[dadt_date->cnt].hidate_value = cnvtdatetimeutc(cnvtdatetime(cnvtdate2(dadt_date
         ->qual[dadt_date->cnt].hidate_value_char,"DD-MM-YYYY"),0),2), dadt_date->qual[dadt_date->cnt
      ].lowdate_value = cnvtdatetimeutc(cnvtdatetime(cnvtdate2(dadt_date->qual[dadt_date->cnt].
     ...
     ENDIF
     IF ((dm_err->debug_flag > 0)
      AND (dadt_date->qual[dadt_date->cnt].hidate_value=0))
      CALL echo(build("HiValueForColumn (",d.c4,") is null!"))
     ENDIF
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc) != 0)
   ...
   ENDIF
  ENDIF
  IF (dadt_newhist_active_ind=1)
   SET dm_err->eproc = "Load NUMBER, FLOAT and DATE columns for NEWHIST consideration"
   SELECT INTO "nl:"
    FROM dm_stat_table d,
     dba_tab_columns c
    WHERE d.statid=dadt_instatid
     ...
     AND d.type="C"
     AND d.c5=c.owner
     AND d.c1=c.table_name
     AND d.c4=c.column_name
     AND c.data_type IN ("NUMBER", "FLOAT", "DATE")
    GROUP BY d.c5, d.c1, d.c4,
     c.data_type
    ORDER BY d.c5, d.c1, d.c4
    HEAD REPORT
     dadt_newhist->cnt = 0, stat = alterlist(dadt_newhist->qual,0)
    DETAIL
     dadt_newhist->cnt = (dadt_newhist->cnt+ 1), stat = alterlist(dadt_newhist->qual,dadt_newhist->
      cnt), dadt_newhist->qual[dadt_newhist->cnt].table_owner = d.c5,
     dadt_newhist->qual[dadt_newhist->cnt].table_name = d.c1, dadt_newhist->qual[dadt_newhist->cnt].
     column_name = d.c4, dadt_newhist->qual[dadt_newhist->cnt].process_newhist_ind = 0,
     dadt_newhist->qual[dadt_newhist->cnt].data_type = c.data_type
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   SET dadt_cnt = 0
   SET dm_err->eproc = "Checking ESTIMATE_PERCENT preference for NEWHIST consideration"
   SELECT INTO "nl:"
    FROM dba_tab_stat_prefs d
    WHERE d.owner=patstring(dadt_inowner)
     AND d.table_name=patstring(dadt_intable)
     AND d.preference_name="ESTIMATE_PERCENT"
    DETAIL
     IF (cnvtreal(d.preference_value) != 100)
      FOR (dadt_cnt = 1 TO dadt_newhist->cnt)
        IF ((dadt_newhist->qual[dadt_cnt].table_owner=d.owner)
         AND (dadt_newhist->qual[dadt_cnt].table_name=d.table_name))
         dadt_newhist->qual[dadt_cnt].process_newhist_ind = 1
        ENDIF
      ENDFOR
     ENDIF
    WITH nocounter
   ...
   SET dadt_cnt = 0
   FOR (dadt_cnt = 1 TO dadt_newhist->cnt)
     IF ((dadt_newhist->qual[dadt_cnt].process_newhist_ind=0))
      SELECT INTO "nl:"
       hbtype = get_hist_type("V500",dadt_stat_tab,dadt_instatid,dadt_newhist->qual[dadt_cnt].
        table_owner,dadt_newhist->qual[dadt_cnt].table_name,
        dadt_newhist->qual[dadt_cnt].column_name)
       FROM dual
       DETAIL
        IF (trim(hbtype)="HEIGHT BALANCED")
         IF ((dm_err->debug_flag > 0))
          CALL echo(concat(trim(hbtype)," histogram detected for ",trim(dadt_newhist->qual[dadt_cnt].
            table_owner),".",trim(dadt_newhist->qual[dadt_cnt].table_name),
           ".",trim(dadt_newhist->qual[dadt_cnt].column_name)))
         ENDIF
         dadt_newhist->qual[dadt_cnt].process_newhist_ind = 1
        ENDIF
       WITH nocounter
   ...
   ENDFOR
   SET dadt_cnt = 0
   SET dm_err->eproc = "Load NEWHIST exclusions"
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="DM2GDBS:NEWHIST:EXCLUDE*"
    ORDER BY d.info_name
    DETAIL
     IF (d.info_domain="DM2GDBS:NEWHIST:EXCLUDE_OWNER")
      FOR (dadt_cnt = 1 TO dadt_newhist->cnt)
        IF ((dadt_newhist->qual[dadt_cnt].table_owner=cnvtupper(d.info_name)))
         dadt_newhist->qual[dadt_cnt].process_newhist_ind = - (1)
        ENDIF
      ENDFOR
     ENDIF
     IF (d.info_domain="DM2GDBS:NEWHIST:EXCLUDE_TABLE")
      FOR (dadt_cnt = 1 TO dadt_newhist->cnt)
        IF (concat(dadt_newhist->qual[dadt_cnt].table_owner,".",dadt_newhist->qual[dadt_cnt].
         table_name)=d.info_name)
         dadt_newhist->qual[dadt_cnt].process_newhist_ind = - (2)
        ENDIF
      ENDFOR
     ENDIF
     IF (d.info_domain="DM2GDBS:NEWHIST:EXCLUDE_COLUMN")
      FOR (dadt_cnt = 1 TO dadt_newhist->cnt)
        IF (concat(dadt_newhist->qual[dadt_cnt].table_owner,".",dadt_newhist->qual[dadt_cnt].
         table_name,".",dadt_newhist->qual[dadt_cnt].column_name)=d.info_name)
         dadt_newhist->qual[dadt_cnt].process_newhist_ind = - (3)
        ENDIF
      ENDFOR
     ENDIF
    WITH nocounter
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dadt_newhist)
   ENDIF
  ENDIF
 ENDIF
 IF ((dadt_date->cnt > 0))
  IF (dadt_hidate_active_ind=1)
   SET dm_err->eproc = "Load dm_info HIDATE exclusions from DM_INFO"
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="DM2GDBS:ADJUSTHIDATE:EXCLUDE*"
    ORDER BY d.info_name
    DETAIL
     IF (d.info_domain="DM2GDBS:ADJUSTHIDATE:EXCLUDE_OWNER")
      FOR (dadt_cnt = 1 TO dadt_date->cnt)
        IF ((dadt_date->qual[dadt_cnt].table_owner=cnvtupper(d.info_name)))
         dadt_date->qual[dadt_cnt].process_hidate_ind = - (1)
        ENDIF
      ENDFOR
     ENDIF
     IF (d.info_domain="DM2GDBS:ADJUSTHIDATE:EXCLUDE_TABLE")
      FOR (dadt_cnt = 1 TO dadt_date->cnt)
        IF (concat(dadt_date->qual[dadt_cnt].table_owner,".",dadt_date->qual[dadt_cnt].table_name)=d
        .info_name)
         dadt_date->qual[dadt_cnt].process_hidate_ind = - (2)
        ENDIF
      ENDFOR
     ENDIF
     IF (d.info_domain="DM2GDBS:ADJUSTHIDATE:EXCLUDE_COLUMN")
      FOR (dadt_cnt = 1 TO dadt_date->cnt)
        IF (concat(dadt_date->qual[dadt_cnt].table_owner,".",dadt_date->qual[dadt_cnt].table_name,".",
         dadt_date->qual[dadt_cnt].column_name)=d.info_name)
         dadt_date->qual[dadt_cnt].process_hidate_ind = - (3)
        ENDIF
      ENDFOR
     ENDIF
    WITH nocounter
   ...
   ENDIF
  ENDIF
  IF (dadt_hilow_active_ind=1)
   SET dm_err->eproc = "Load dm_info HILOW exclusions from DM_INFO"
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="DM2GDBS:ADJUSTHILOW:EXCLUDE*"
    ORDER BY d.info_name
    DETAIL
     IF (d.info_domain="DM2GDBS:ADJUSTHILOW:EXCLUDE_OWNER")
      FOR (dadt_cnt = 1 TO dadt_date->cnt)
        IF ((dadt_date->qual[dadt_cnt].table_owner=cnvtupper(d.info_name)))
         dadt_date->qual[dadt_cnt].process_hilow_ind = - (1)
        ENDIF
      ENDFOR
     ENDIF
     IF (d.info_domain="DM2GDBS:ADJUSTHILOW:EXCLUDE_TABLE")
      FOR (dadt_cnt = 1 TO dadt_date->cnt)
        IF (concat(dadt_date->qual[dadt_cnt].table_owner,".",dadt_date->qual[dadt_cnt].table_name)=d
        .info_name)
         dadt_date->qual[dadt_cnt].process_hilow_ind = - (2)
        ENDIF
      ENDFOR
     ENDIF
     IF (d.info_domain="DM2GDBS:ADJUSTHILOW:EXCLUDE_COLUMN")
      FOR (dadt_cnt = 1 TO dadt_date->cnt)
        IF (concat(dadt_date->qual[dadt_cnt].table_owner,".",dadt_date->qual[dadt_cnt].table_name,".",
         dadt_date->qual[dadt_cnt].column_name)=d.info_name)
         dadt_date->qual[dadt_cnt].process_hilow_ind = - (3)
        ENDIF
      ENDFOR
     ENDIF
    WITH nocounter
   ...
   ENDIF
  ENDIF
  IF (dadt_minmax_active_ind=1)
   SET dm_err->eproc = "Exclude non-indexed DATE columns for minmax"
   SELECT INTO "nl:"
    FROM dba_ind_columns i
    WHERE i.table_owner=patstring(dadt_inowner)
     AND i.table_name=patstring(dadt_intable)
     AND  NOT (i.table_owner IN (
    (SELECT
     d.info_name
     FROM dm_info d
    ...
    HEAD REPORT
     idx = 0
    DETAIL
     idx = 0, idx = locateval(idx,1,dadt_date->cnt,i.table_owner,dadt_date->qual[idx].table_owner,
      i.table_name,dadt_date->qual[idx].table_name,i.column_name,dadt_date->qual[idx].column_name)
     IF (idx > 0)
      dadt_date->qual[idx].process_minmax_ind = 1, dadt_date->qual[idx].process_hilow_ind = - (4)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    ...
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = "Load dm_info MINMAX exclusions from DM_INFO"
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="DM2GDBS:SQUEEZEMINMAX:EXCLUDE*"
    ORDER BY d.info_name
    DETAIL
     IF (d.info_domain="DM2GDBS:SQUEEZEMINMAX:EXCLUDE_OWNER")
      FOR (dadt_cnt = 1 TO dadt_date->cnt)
        IF ((dadt_date->qual[dadt_cnt].table_owner=cnvtupper(d.info_name)))
         dadt_date->qual[dadt_cnt].process_minmax_ind = - (1)
        ENDIF
      ENDFOR
     ENDIF
     IF (d.info_domain="DM2GDBS:SQUEEZEMINMAX:EXCLUDE_TABLE")
      FOR (dadt_cnt = 1 TO dadt_date->cnt)
        IF (concat(dadt_date->qual[dadt_cnt].table_owner,".",dadt_date->qual[dadt_cnt].table_name)=d
        .info_name)
         dadt_date->qual[dadt_cnt].process_minmax_ind = - (2)
        ENDIF
      ENDFOR
     ENDIF
     IF (d.info_domain="DM2GDBS:SQUEEZEMINMAX:EXCLUDE_COLUMN")
      FOR (dadt_cnt = 1 TO dadt_date->cnt)
        IF (concat(dadt_date->qual[dadt_cnt].table_owner,".",dadt_date->qual[dadt_cnt].table_name,".",
         dadt_date->qual[dadt_cnt].column_name)=d.info_name)
         dadt_date->qual[dadt_cnt].process_minmax_ind = - (3)
        ENDIF
      ENDFOR
     ENDIF
    WITH nocounter
  ...
  IF ((dm_err->debug_flag > 3))
   CALL echorecord(dadt_date)
  ENDIF
  FOR (dadt_cnt = 1 TO dadt_date->cnt)
    SET dadt_skip_ind = 0
    SET dadt_lock_available_ind = 0
    SET dadt_lock_obtained_ind = 0
    IF (dadt_instatid="LIVE")
     IF ((dm_err->debug_flag > 0))
      SET dm_err->eproc = "Attempt to get a statistics lock on the table"
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     IF ((dgdt_prefs->context_lock_ind=1))
      SET dadt_object_id = 0.0
      IF (ddf_get_object_id(dadt_date->qual[dadt_cnt].table_owner,dadt_date->qual[dadt_cnt].
       table_name,dadt_object_id)=0)
       GO TO exit_script
      ENDIF
     ENDIF
     IF (ddf_context_lock_maint("CHECK","STAT",1,dadt_date->qual[dadt_cnt].table_owner,dadt_date->
      qual[dadt_cnt].table_name,
      dadt_object_id,dadt_lock_available_ind)=0)
      GO TO exit_script
     ENDIF
     IF (dadt_lock_available_ind=1)
      IF (ddf_context_lock_maint("GET","STAT",2,dadt_date->qual[dadt_cnt].table_owner,dadt_date->
       qual[dadt_cnt].table_name,
       dadt_object_id,dadt_lock_obtained_ind)=0)
       GO TO exit_script
      ENDIF
     ENDIF
     IF (dadt_lock_obtained_ind=0)
      SET dm_err->eproc = "Unable to obtain adjust stats lock. Skipping table."
      CALL disp_msg(" ",dm_err->logfile,0)
      CALL dm2_process_log_add_detail_text("TABLE SKIP REASON",concat(
        "Could not obtain ADJUST STATS LOCK for ",dadt_date->qual[dadt_cnt].table_name))
      IF (dm2_process_log_dtl_row(dadt_log_id,0)=0)
       GO TO exit_script
      ENDIF
      SET dadt_skip_ind = 1
     ENDIF
     IF (dadt_skip_ind=0)
      SET dm_err->eproc = "Obtain current publish status"
      SET dadt_pub_val = "TRUE"
      SELECT INTO "nl:"
       FROM dba_tab_stat_prefs d
       WHERE (d.owner=dadt_date->qual[dadt_cnt].table_owner)
        AND (d.table_name=dadt_date->qual[dadt_cnt].table_name)
        AND d.preference_name="PUBLISH"
       DETAIL
        dadt_pub_val = trim(cnvtupper(d.preference_value))
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc) != 0)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_script
      ENDIF
      IF (dadt_pub_val="FALSE")
       IF ((dm_err->debug_flag > 0))
        SET dm_err->eproc = "Call dbms_stats.SET_TABLE_PREFS to set PUBLISH to TRUE"
        CALL disp_msg(" ",dm_err->logfile,0)
       ENDIF
       CALL set_table_prefs(dadt_date->qual[dadt_cnt].table_owner,concat('"',dadt_date->qual[dadt_cnt
    ...
    IF (dadt_skip_ind=0)
     IF (dadt_hidate_active_ind=1
      AND (dadt_date->qual[dadt_cnt].process_hidate_ind >= 0)
      AND (dadt_date->qual[dadt_cnt].hidate_value > 0))
      IF (datetimediff(datetimeadd(dadt_date->cur_date_time,dadt_hidate_qualifier_offset),dadt_date->
       qual[dadt_cnt].hidate_value) >= 0)
       SET dadt_date->qual[dadt_cnt].process_hidate_ind = 1
       SET dadt_date->qual[dadt_cnt].days_to_bump = round((datetimediff(dadt_date->qual[dadt_cnt].
         hidate_value,dadt_date->qual[dadt_cnt].lowdate_value) * dadt_hidate_formula_element1),0)
       IF ((dm_err->debug_flag > 0))
        CALL echo(build("days_to_bump calculated[",dadt_cnt,"]:",dadt_date->qual[dadt_cnt].
          days_to_bump))
       ENDIF
       IF ((dadt_date->qual[dadt_cnt].days_to_bump > dadt_hidate_max_bump_days))
        SET dadt_date->qual[dadt_cnt].days_to_bump = dadt_hidate_max_bump_days
       ENDIF
       IF ((dadt_date->qual[dadt_cnt].days_to_bump < dadt_hidate_min_bump_days))
        SET dadt_date->qual[dadt_cnt].days_to_bump = dadt_hidate_min_bump_days
       ENDIF
      ELSE
       SET dadt_date->qual[dadt_cnt].process_hidate_ind = 0
       SET dadt_date->qual[dadt_cnt].days_to_bump = 0
      ENDIF
     ENDIF
     IF (dadt_hilow_active_ind=1
      AND (dadt_date->qual[dadt_cnt].process_hilow_ind >= 0)
      AND (dadt_date->qual[dadt_cnt].hidate_value > 0))
      IF ((dadt_date->qual[dadt_cnt].hidate_value > cnvtdatetimeutc(cnvtdatetime(cnvtdate2(
         dadt_hilow_highdate_max,"DD-MM-YYYY"),0),2)))
       SET dadt_date->qual[dadt_cnt].process_hilow_ind = 1
      ENDIF
      IF ((dadt_date->qual[dadt_cnt].lowdate_value < cnvtdatetimeutc(cnvtdatetime(cnvtdate2(
         dadt_hilow_lowdate_min,"DD-MM-YYYY"),0),2)))
       SET dadt_date->qual[dadt_cnt].process_hilow_ind = 1
      ENDIF
     ENDIF
     IF ((dadt_date->qual[dadt_cnt].process_minmax_ind=1)
      AND (dadt_date->qual[dadt_cnt].hidate_value > 0))
      SET dadt_retry_attempts = 0
      SET dadt_allow_publish = 0
      WHILE (dadt_allow_publish=0
       AND ((dadt_retry_attempts < dadt_retry_ceiling) OR ((dadt_retry_ceiling=- (1)))) )
        SET dadt_allow_publish = 1
        SET dadt_blocking_str = ""
        IF (dadt_instatid="LIVE")
         IF (dadt_wait_pref_ind=0)
          IF (ddf_check_in_parse(dadt_date->qual[dadt_cnt].table_owner,dadt_date->qual[dadt_cnt].
           table_name,dadt_in_parse_ind,dadt_blocking_str)=0)
           GO TO exit_script
          ENDIF
         ENDIF
         IF (dadt_in_parse_ind=1)
          SET dadt_allow_publish = 0
         ENDIF
         IF ((dadt_retry_ceiling=- (1))
          AND dadt_allow_publish=0)
          SET dadt_allow_publish = 1
          SET dm_err->eproc = "Parse detection overridden. Allowing Publish."
          CALL disp_msg(" ",dm_err->logfile,0)
         ENDIF
        ENDIF
        IF (dadt_allow_publish=1)
         SET dm_err->eproc = build("Calling DM_DBMS_STATS.SQUEEZE_MINMAX for (",dadt_date->qual[
          dadt_cnt].table_owner,".",dadt_date->qual[dadt_cnt].table_name,".",
          dadt_date->qual[dadt_cnt].column_name,") STATID (",dadt_instatid,")","MINMAXIND(",
          dadt_date->qual[dadt_cnt].process_minmax_ind,") ")
         CALL disp_msg(dm_err->eproc,dm_err->logfile,10)
         CALL squeeze_minmax(dadt_stat_tab,dadt_instatid,dadt_date->qual[dadt_cnt].table_owner,
          dadt_date->qual[dadt_cnt].table_name,dadt_date->qual[dadt_cnt].column_name)
         CALL check_error(dm_err->eproc)
         IF ((dm_err->err_ind > 0))
          IF (findstring("ORA-04022",dm_err->emsg) > 0)
           SET dadt_allow_publish = 0
           SET dm_err->err_ind = 0
          ELSE
           CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          ...
          ENDIF
         ELSE
          CALL dm2_process_log_add_detail_text(build("SM:",dadt_cnt),dm_err->eproc)
          IF (dm2_process_log_dtl_row(dadt_log_id,0)=0)
           GO TO exit_script
          ENDIF
         ENDIF
        ENDIF
        IF (dadt_allow_publish=0)
         SET dadt_retry_attempts = (dadt_retry_attempts+ 1)
         IF (dadt_retry_attempts < dadt_retry_ceiling)
          SET dm_err->eproc = concat(dadt_blocking_str," Attempt ",trim(cnvtstring(
             dadt_retry_attempts))," of ",trim(cnvtstring(dadt_retry_ceiling)),
           ".")
          CALL disp_msg(" ",dm_err->logfile,0)
          CALL dm2_process_log_add_detail_text("PARSE_EVENT:SMLOG",dm_err->eproc)
          IF (dm2_process_log_dtl_row(dadt_log_id,0)=0)
           GO TO exit_script
          ENDIF
          CALL pause(10)
         ELSE
          SET dm_err->err_ind = 1
          SET dm_err->emsg = concat(dadt_blocking_str," Attempt ",trim(cnvtstring(dadt_retry_attempts
             ))," of ",trim(cnvtstring(dadt_retry_ceiling)),
           ". Aborting Publish")
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          CALL dm2_process_log_add_detail_text("PARSE_EVENT:SMFAILURE",dm_err->eproc)
          IF (dm2_process_log_dtl_row(dadt_log_id,1)=0)
           GO TO exit_script
          ENDIF
          GO TO exit_script
         ENDIF
      ...
      ENDWHILE
     ENDIF
     IF ((dadt_date->qual[dadt_cnt].process_hidate_ind=1))
      SET dadt_retry_attempts = 0
      SET dadt_allow_publish = 0
      WHILE (dadt_allow_publish=0
       AND ((dadt_retry_attempts < dadt_retry_ceiling) OR ((dadt_retry_ceiling=- (1)))) )
        ...
        ENDIF
        IF (dadt_allow_publish=1)
         SET dm_err->eproc = build("Calling DM_DBMS_STATS.BUMP_HIGH_DATE_COLUMN_STATS for (",
          dadt_date->qual[dadt_cnt].table_owner,".",dadt_date->qual[dadt_cnt].table_name,".",
          dadt_date->qual[dadt_cnt].column_name,") STATID (",dadt_instatid,") DAYSTOBUMP (",dadt_date
          ->qual[dadt_cnt].days_to_bump,
          ")","HIDATEIND(",dadt_date->qual[dadt_cnt].process_hidate_ind,") ")
         CALL disp_msg(dm_err->eproc,dm_err->logfile,10)
         CALL bump_high_date_column_stats(dadt_stat_tab,dadt_instatid,dadt_date->qual[dadt_cnt].
          table_owner,dadt_date->qual[dadt_cnt].table_name,dadt_date->qual[dadt_cnt].column_name,
          dadt_date->qual[dadt_cnt].days_to_bump)
         CALL check_error(dm_err->eproc)
         IF ((dm_err->err_ind > 0))
          IF (findstring("ORA-04022",dm_err->emsg) > 0)
           SET dadt_allow_publish = 0
          ...
          ENDIF
         ELSE
          CALL dm2_process_log_add_detail_text(build("BHDCS:",dadt_cnt),dm_err->eproc)
          IF (dm2_process_log_dtl_row(dadt_log_id,0)=0)
           GO TO exit_script
          ENDIF
         ENDIF
        ...
        IF (dadt_allow_publish=0)
         ...
         IF (dadt_retry_attempts < dadt_retry_ceiling)
          SET dm_err->eproc = concat(dadt_blocking_str," Attempt ",trim(cnvtstring(
           ...
           ".")
          CALL disp_msg(" ",dm_err->logfile,0)
          CALL dm2_process_log_add_detail_text("PARSE_EVENT:BHDCSLOG",dm_err->eproc)
          IF (dm2_process_log_dtl_row(dadt_log_id,0)=0)
           GO TO exit_script
          ENDIF
          CALL pause(10)
         ELSE
          ...
          SET dm_err->emsg = concat(dadt_blocking_str," Attempt ",trim(cnvtstring(dadt_retry_attempts
           ...
           ". Aborting Publish")
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          CALL dm2_process_log_add_detail_text("PARSE_EVENT:BHDCSFAILURE",dm_err->eproc)
          IF (dm2_process_log_dtl_row(dadt_log_id,1)=0)
           GO TO exit_script
          ENDIF
          GO TO exit_script
      ...
      ENDWHILE
     ENDIF
     IF ((dadt_date->qual[dadt_cnt].process_hilow_ind=1))
      SET dadt_retry_attempts = 0
      SET dadt_allow_publish = 0
      WHILE (dadt_allow_publish=0
       AND ((dadt_retry_attempts < dadt_retry_ceiling) OR ((dadt_retry_ceiling=- (1)))) )
        ...
        ENDIF
        IF (dadt_allow_publish=1)
         SET dm_err->eproc = build("Calling DM_DBMS_STATS.ADJUST_HIGH_LOW_COLUMN_STATS for (",
          dadt_date->qual[dadt_cnt].table_owner,".",dadt_date->qual[dadt_cnt].table_name,".",
          dadt_date->qual[dadt_cnt].column_name,") STATID (",dadt_instatid,")","HILOWIND(",
          dadt_date->qual[dadt_cnt].process_hilow_ind,") ")
         CALL disp_msg(dm_err->eproc,dm_err->logfile,10)
         CALL adjust_high_low_column_stats(dadt_stat_tab,dadt_instatid,dadt_date->qual[dadt_cnt].
          table_owner,dadt_date->qual[dadt_cnt].table_name,dadt_date->qual[dadt_cnt].column_name)
         CALL check_error(dm_err->eproc)
         IF ((dm_err->err_ind > 0))
          IF (findstring("ORA-04022",dm_err->emsg) > 0)
          ...
          ENDIF
         ELSE
          CALL dm2_process_log_add_detail_text(build("AHLCS:",dadt_cnt),dm_err->eproc)
          IF (dm2_process_log_dtl_row(dadt_log_id,0)=0)
           GO TO exit_script
          ENDIF
         ENDIF
        ...
        IF (dadt_allow_publish=0)
         ...
         IF (dadt_retry_attempts < dadt_retry_ceiling)
          SET dm_err->eproc = concat(dadt_blocking_str," Attempt ",trim(cnvtstring(
           ...
           ".")
          CALL disp_msg(" ",dm_err->logfile,0)
          CALL dm2_process_log_add_detail_text("PARSE_EVENT:AHLCSLOG",dm_err->eproc)
          IF (dm2_process_log_dtl_row(dadt_log_id,0)=0)
           GO TO exit_script
          ENDIF
          CALL pause(10)
         ELSE
          ...
          SET dm_err->emsg = concat(dadt_blocking_str," Attempt ",trim(cnvtstring(dadt_retry_attempts
           ...
           ". Aborting Publish")
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          CALL dm2_process_log_add_detail_text("PARSE_EVENT:AHLCSFAILURE",dm_err->eproc)
          IF (dm2_process_log_dtl_row(dadt_log_id,1)=0)
           GO TO exit_script
          ENDIF
          GO TO exit_script
     ...
     IF (dadt_instatid="LIVE")
      IF (dadt_pub_val="FALSE")
       IF ((dm_err->debug_flag > 0))
        SET dm_err->eproc = "Call dbms_stats.SET_TABLE_PREFS to set PUBLISH to FALSE"
        CALL disp_msg(" ",dm_err->logfile,0)
       ENDIF
       CALL set_table_prefs(dadt_date->qual[dadt_cnt].table_owner,concat('"',dadt_date->qual[dadt_cnt
         ].table_name,'"'),"PUBLISH","FALSE")
      ENDIF
      IF (ddf_context_lock_maint("RELEASE","STAT",2,dadt_date->qual[dadt_cnt].table_owner,dadt_date->
       qual[dadt_cnt].table_name,
       dadt_object_id,dadt_lock_obtained_ind)=0)
       GO TO exit_script
      ENDIF
  ...
  ENDFOR
 ENDIF
 IF ((dadt_newhist->cnt > 0))
  SET dadt_cnt = 0
  FOR (dadt_cnt = 1 TO dadt_newhist->cnt)
    IF ((dadt_newhist->qual[dadt_cnt].process_newhist_ind=1))
     SET dm_err->eproc = build("Calling DM_DBMS_STATS.AH_RESOLVE_HISTOGRAM for (",dadt_newhist->qual[
      dadt_cnt].table_owner,".",dadt_newhist->qual[dadt_cnt].table_name,".",
      dadt_newhist->qual[dadt_cnt].column_name,") STATID (",dadt_instatid,")")
     CALL disp_msg(dm_err->eproc,dm_err->logfile,10)
     CALL resolve_histogram("V500",dadt_stat_tab,dadt_instatid,dadt_newhist->qual[dadt_cnt].
      table_owner,dadt_newhist->qual[dadt_cnt].table_name,
      dadt_newhist->qual[dadt_cnt].column_name,dadt_newhist->qual[dadt_cnt].data_type)
     IF (check_error(dm_err->eproc) != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_script
     ENDIF
  ...
  ENDFOR
 ENDIF
 SET dm_err->eproc = "Get DM2GDBS:CUSTADJUSTPROC rows from DM_INFO."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2GDBS:STAGEADJUSTPROC-TAB"
  DETAIL
   dadt_procs->cnt = (dadt_procs->cnt+ 1), stat = alterlist(dadt_procs->qual,dadt_procs->cnt),
   dadt_procs->qual[dadt_procs->cnt].proc_name = d.info_name,
   dadt_procs->qual[dadt_procs->cnt].proc_cmd = d.info_char
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dadt_procs->cnt > 0))
  FOR (x = 1 TO dadt_procs->cnt)
    SET dadt_cmd = dadt_procs->qual[x].proc_cmd
    SET dadt_cmd = replace(dadt_cmd,"<MODE>",dadt_inadjtype)
    SET dadt_cmd = replace(dadt_cmd,"<OBJECT_OWNER>",dadt_inowner)
    SET dadt_cmd = replace(dadt_cmd,"<OBJECT_NAME>",dadt_intable)
    SET dadt_cmd = replace(dadt_cmd,"<OBJECT_TYPE>","TABLE")
    SET dadt_cmd = replace(dadt_cmd,"<STATID>",dadt_instatid)
    SET dadt_cmd = concat("RDB ASIS(^ begin ",dadt_cmd,"; end; ^) go")
    IF (dm2_push_cmd(dadt_cmd,1)=0)
     CALL disp_msg(" ",dm_err->logfile,1)
     IF ((dm_err->err_ind=1))
      SET dm_err->err_ind = 0
...
#exit_script
 ...
 SET dadt_err_ind = dm_err->err_ind
 SET dadt_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_execution,dadt_log_id,1)
 SET dm_err->err_ind = dadt_err_ind
 SET dm_err->emsg = dadt_emsg
 SET dm_err->eproc = "Ending dm2_adjust_dbstats_tab"
 CALL final_disp_msg("dm2_adbs_tab")
END GO
