CREATE PROGRAM dm2_ads_dbg:dba
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dad_mode = vc WITH protect, noconstant("")
 DECLARE dad_table_name = vc WITH protect, noconstant("")
 DECLARE dad_table_extract_nbr = i4 WITH protect, noconstant(0)
 DECLARE dad_cnt = i4 WITH protect, noconstant(1)
 DECLARE dad_where_cond = vc WITH protect, noconstant("")
 DECLARE dad_iter = i4 WITH protect, noconstant(0)
 DECLARE dad_pos = i4 WITH protect, noconstant(0)
 DECLARE dad_ref_ind = i2 WITH protect, noconstant(0)
 DECLARE dad_table_name = vc WITH protect, noconstant("")
 IF ( NOT (check_logfile("dm2_ads_dbg",".log","dm2_ads_dbg LOGFILE")))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_ads_dbg"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dad_mode =  $1
 SET dad_table_name =  $2
 SET dad_table_extract_nbr =  $3
 SET dad_ref_ind =  $4
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = concat("Invalid Request")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating input parameters."
 IF ( NOT (((dad_mode="LOAD") OR (dad_mode="UNLOAD")) ))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Invalid Parameter values"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ( NOT (((dad_table_extract_nbr > 0) OR ((dad_table_extract_nbr=- (1)))) ))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Invalid Parameter Values"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ...
 IF (dad_mode="UNLOAD")
  IF (dad_ref_ind=1)
   SET dad_table_name = "dm_ads_extract_stg@ref_data_link"
  ELSE
   SET dad_table_name = "dm_ads_extract_stg"
  ENDIF
  EXECUTE oragen3 value(dad_table_name)
  IF ((dm_err->err_ind > 0))
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Deleting old data from dm_ads_extract_stg"
  CALL disp_msg("",dm_err->logfile,0)
  IF (dad_ref_ind=1)
   DELETE  FROM dm_ads_extract_stg@ref_data_link daeg
    WHERE dm_ads_extract_stg_id != 0
    WITH nocounter
   ;end delete
  ELSE
   DELETE  FROM dm_ads_extract_stg daeg
    WHERE dm_ads_extract_stg_id != 0
    WITH nocounter
   ;end delete
  ENDIF
  ...
  ENDIF
  COMMIT
  IF (dar_get_extract_list(0,"*",- (1),dad_ref_ind)=0)
   GO TO exit_program
  ENDIF
  IF (dar_scr_dads_list(0)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Inserting data into dm_ads_extract_stg"
  CALL disp_msg("",dm_err->logfile,0)
  IF (dad_ref_ind=1)
   INSERT  FROM dm_ads_extract_stg@ref_data_link daeg,
     (dummyt d  WITH seq = value(dads_list->extract_cnt))
    SET daeg.dm_ads_extract_stg_id = dads_list->extract_qual[d.seq].extract_id, daeg.owner_name =
     dads_list->extract_qual[d.seq].owner_name, daeg.table_name = dads_list->extract_qual[d.seq].
     table_name,
     daeg.table_extract_nbr = dads_list->extract_qual[d.seq].table_extract_nbr, daeg
     .table_extract_instance_nbr = dads_list->extract_qual[d.seq].table_extract_instance_nbr, daeg
     .active_ind = dads_list->extract_qual[d.seq].active_ind,
     daeg.driver_table_name = dads_list->extract_qual[d.seq].driver_table_name, daeg.data_class_type
      = dads_list->extract_qual[d.seq].data_class_type, daeg.extract_method = dads_list->
     extract_qual[d.seq].extract_method,
     daeg.driver_table_ind = dads_list->extract_qual[d.seq].driver_table_ind, daeg.driver_keycol_name
      = dads_list->extract_qual[d.seq].driver_keycol_name, daeg.driver_rankcol_name = dads_list->
     extract_qual[d.seq].driver_rankcol_name,
     daeg.apply_where_ind = dads_list->extract_qual[d.seq].apply_where_ind, daeg
     .expimp_parent_table_name = dads_list->extract_qual[d.seq].expimp_parent_table_name, daeg
     .expimp_level_nbr = dads_list->extract_qual[d.seq].expimp_level_nbr,
     daeg.where_clause = dads_list->extract_qual[d.seq].where_clause, daeg.table_comment = dads_list
     ->extract_qual[d.seq].table_comment, daeg.dupdel_skip_ind = dads_list->extract_qual[d.seq].
     dupdel_skip_ind,
     daeg.updt_cnt = 0, daeg.updt_task = 0, daeg.updt_id = 0,
     daeg.updt_applctx = 0, daeg.updt_dt_tm = cnvtdatetime(curdate,curtime3)
    PLAN (d)
     JOIN (daeg)
    WITH nocounter
   ;end insert
  ELSE
   INSERT  FROM dm_ads_extract_stg daeg,
     (dummyt d  WITH seq = value(dads_list->extract_cnt))
    SET daeg.dm_ads_extract_stg_id = dads_list->extract_qual[d.seq].extract_id, daeg.owner_name =
     dads_list->extract_qual[d.seq].owner_name, daeg.table_name = dads_list->extract_qual[d.seq].
     table_name,
  ...
  IF (dar_clear_dads_list(null)=0)
   GO TO exit_program
  ENDIF
 ELSEIF (dad_mode="LOAD")
  IF (validate(dad_load_allowed,1)=1
   AND validate(dad_load_allowed,2)=2)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Invalid Request"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ELSE
   IF (dad_table_extract_nbr > 0)
    SET dad_where_cond = concat("daeg.table_extract_nbr = ",build(dad_table_extract_nbr))
   ELSE
    SET dad_where_cond = "daeg.table_extract_nbr > 0"
   ENDIF
   IF (dar_clear_dads_list(null)=0)
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = "Retrieve data from dm_ads_extract_stg"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_ads_extract_stg daeg
    WHERE daeg.table_name=patstring(dad_table_name)
     AND daeg.dm_ads_extract_stg_id != 0
     AND parser(dad_where_cond)
    HEAD REPORT
     dad_cnt = 0, stat = alterlist(dads_list->extract_qual,dad_cnt)
    DETAIL
     dad_cnt = (dad_cnt+ 1)
     IF (mod(dad_cnt,10)=1)
      stat = alterlist(dads_list->extract_qual,(dad_cnt+ 9))
     ENDIF
     dads_list->extract_qual[dad_cnt].extract_id = daeg.dm_ads_extract_stg_id, dads_list->
     extract_qual[dad_cnt].owner_name = build(cnvtupper(daeg.owner_name)), dads_list->extract_qual[
     dad_cnt].table_name = build(cnvtupper(daeg.table_name)),
     dads_list->extract_qual[dad_cnt].table_extract_nbr = daeg.table_extract_nbr, dads_list->
     extract_qual[dad_cnt].table_extract_instance_nbr = daeg.table_extract_instance_nbr, dads_list->
     extract_qual[dad_cnt].active_ind = daeg.active_ind,
     dads_list->extract_qual[dad_cnt].driver_table_name = build(cnvtupper(daeg.driver_table_name)),
     dads_list->extract_qual[dad_cnt].data_class_type = build(cnvtupper(daeg.data_class_type)),
     dads_list->extract_qual[dad_cnt].extract_method = build(cnvtupper(daeg.extract_method)),
     dads_list->extract_qual[dad_cnt].driver_table_ind = daeg.driver_table_ind, dads_list->
     extract_qual[dad_cnt].driver_keycol_name = build(cnvtupper(daeg.driver_keycol_name)), dads_list
     ->extract_qual[dad_cnt].driver_rankcol_name = build(cnvtupper(daeg.driver_rankcol_name)),
     dads_list->extract_qual[dad_cnt].apply_where_ind = daeg.apply_where_ind, dads_list->
     extract_qual[dad_cnt].expimp_parent_table_name = build(cnvtupper(daeg.expimp_parent_table_name)),
     dads_list->extract_qual[dad_cnt].expimp_level_nbr = daeg.expimp_level_nbr,
     dads_list->extract_qual[dad_cnt].where_clause = daeg.where_clause, dads_list->extract_qual[
     dad_cnt].table_comment = daeg.table_comment, dads_list->extract_qual[dad_cnt].dupdel_skip_ind =
     daeg.dupdel_skip_ind,
     dads_list->extract_qual[dad_cnt].table_exists_ind = 0
    FOOT REPORT
     dads_list->extract_cnt = dad_cnt, stat = alterlist(dads_list->extract_qual,dads_list->
      extract_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   IF ((dads_list->extract_cnt=0))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Unable to load. No data found in dm_ads_extract_stg"
    SET dm_err->user_action = "Please unload data into dm_ads_extract_stg first."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (dar_scr_dads_list(1)=0)
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = "Determine if extract already exists in dm_ads_extract"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    ...
    PLAN (d
     WHERE d.seq > 0)
     JOIN (dae
     WHERE (dae.dm_ads_extract_id=dads_list->extract_qual[d.seq].extract_id))
    DETAIL
   ...
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dads_list)
   ENDIF
   SET dm_err->eproc = "Updating dm_ads_extract table with information from dm_ads_extract_stg"
   CALL disp_msg("",dm_err->logfile,0)
   UPDATE  FROM dm_ads_extract dae,
     (dummyt d  WITH seq = value(dads_list->extract_cnt))
    SET dae.table_extract_instance_nbr = dads_list->extract_qual[d.seq].table_extract_instance_nbr,
     dae.active_ind = dads_list->extract_qual[d.seq].active_ind, dae.driver_table_name = dads_list->
     extract_qual[d.seq].driver_table_name,
     dae.data_class_type = dads_list->extract_qual[d.seq].data_class_type, dae.extract_method =
     dads_list->extract_qual[d.seq].extract_method, dae.driver_table_ind = dads_list->extract_qual[d
     .seq].driver_table_ind,
     dae.driver_keycol_name = dads_list->extract_qual[d.seq].driver_keycol_name, dae
     .driver_rankcol_name = dads_list->extract_qual[d.seq].driver_rankcol_name, dae.apply_where_ind
      = dads_list->extract_qual[d.seq].apply_where_ind,
     dae.expimp_parent_table_name = dads_list->extract_qual[d.seq].expimp_parent_table_name, dae
     .expimp_level_nbr = dads_list->extract_qual[d.seq].expimp_level_nbr, dae.where_clause =
     dads_list->extract_qual[d.seq].where_clause,
     dae.table_comment = dads_list->extract_qual[d.seq].table_comment, dae.dupdel_skip_ind =
     dads_list->extract_qual[d.seq].dupdel_skip_ind, dae.updt_task = 1
    PLAN (d
     WHERE (dads_list->extract_qual[d.seq].table_exists_ind=1))
     JOIN (dae
     WHERE (dae.dm_ads_extract_id=dads_list->extract_qual[d.seq].extract_id))
    WITH nocounter
   ;end update
   ...
   ENDIF
   COMMIT
   SET dm_err->eproc = "Inserting into dm_ads_extract table with information from dm_ads_extract_stg"
   CALL disp_msg("",dm_err->logfile,0)
   INSERT  FROM dm_ads_extract dae,
     (dummyt d  WITH seq = value(dads_list->extract_cnt))
    SET dae.dm_ads_extract_id = dads_list->extract_qual[d.seq].extract_id, dae.table_name = dads_list
     ->extract_qual[d.seq].table_name, dae.table_extract_nbr = dads_list->extract_qual[d.seq].
     table_extract_nbr,
     dae.table_extract_instance_nbr = dads_list->extract_qual[d.seq].table_extract_instance_nbr, dae
     .active_ind = dads_list->extract_qual[d.seq].active_ind, dae.driver_table_name = dads_list->
     extract_qual[d.seq].driver_table_name,
     dae.data_class_type = dads_list->extract_qual[d.seq].data_class_type, dae.extract_method =
     dads_list->extract_qual[d.seq].extract_method, dae.driver_table_ind = dads_list->extract_qual[d
     .seq].driver_table_ind,
     ...
     dads_list->extract_qual[d.seq].where_clause,
     dae.table_comment = dads_list->extract_qual[d.seq].table_comment, dae.dupdel_skip_ind =
     dads_list->extract_qual[d.seq].dupdel_skip_ind, dae.updt_task = 2
    PLAN (d
     WHERE (dads_list->extract_qual[d.seq].table_exists_ind=0))
     JOIN (dae)
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
   ...
   IF (dar_clear_dads_list(null)=0)
    GO TO exit_program
   ENDIF
   FREE SET dad_load_allowed
  ENDIF
 ENDIF
#exit_program
 SET dm_err->eproc = "Ending dm2_ads_dbg."
 CALL final_disp_msg("dm2_ads_dbg")
END GO
