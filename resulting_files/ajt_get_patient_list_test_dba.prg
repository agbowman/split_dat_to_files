CREATE PROGRAM ajt_get_patient_list_test:dba
 PROMPT
 ...
 FOR (i = 1 TO num_arguments)
   CALL echo("attempt 1 to echo")
   CALL echo(dgapl_reply->patient_lists[selection].arguments[i].argument_name)
   ...
   IF ((request->arguments[i].argument_name="encntr_type"))
    ...
    SET dgapl_reply->patient_lists[selection].encntr_type_filters[my_cnt].encntr_type_cd =
    dgapl_reply->patient_lists[selection].arguments[i].parent_entity_id
   ENDIF
 ENDFOR
