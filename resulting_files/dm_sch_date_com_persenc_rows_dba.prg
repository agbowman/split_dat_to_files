CREATE PROGRAM dm_sch_date_com_persenc_rows:dba
 ...
 DECLARE g_rows_between_commit = f8 WITH protect, noconstant(100.0)
 ...
 IF (v_days_to_keep < g_minimum_keep_days)
  ...
  SET reply->status_data.status = "F"
 ELSE
  SET reply->table_name = "SCH_DATE_COMMENT"
  SET reply->rows_between_commit = g_rows_between_commit
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(sdc.date_comment_id)
    FROM sch_date_comment sdc
    WHERE sdc.date_comment_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   seqval = max(sdc.date_comment_id)
   FROM sch_date_comment sdc
   DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     a.rowid
     FROM sch_date_comment a
     WHERE a.date_comment_id BETWEEN curminid AND curmaxid
      AND ((a.parent_table="PERSON"
      AND a.text_type_meaning="PERSONCOMM") OR (a.parent_table="ENCOUNTER"
      AND a.text_type_meaning="ENCNTRCOMM"))
      AND parser(sbr_getrowidnotexists(
       "a.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep), curtime3)","a"))
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,100)=1)
