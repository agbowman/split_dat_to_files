CREATE PROGRAM dts_chk_dt_format_imprt:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "dts_chk_dt_format_imprt"
...
#exit_script
 IF (curqual=0)
  CALL logmsg("Populate of date/time on code_set 22669 failed.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
  CALL logstatus("Execution failed - see log message for more details.","F")
 ELSE
  IF (count < 600)
   CALL logmsg("Not all of date/time on code_set 22669 were updated.",((e_rdmstatus+ e_dispmsg)+
    e_logtofile))
   CALL logstatus("Execution failed - see log message for more details.","F")
  ELSE
   CALL logmsg("Populate of date/time on code_set 22669 successful.",((e_rdmstatus+ e_dispmsg)+
    e_logtofile))
