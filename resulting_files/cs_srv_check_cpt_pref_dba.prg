CREATE PROGRAM cs_srv_check_cpt_pref:dba
 CALL echo(build("Begin cs_srv_check_cpt_pref, version [",nullterm("488301.000"),"]"))
 ...
 IF ( NOT (validate(dm_info_name)))
  DECLARE dm_info_name = vc WITH protect, constant("PRIMARY CPT MODIFIER")
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   reply->cpt_preference = di.info_char
  WITH nocounter
 ;end select
