CREATE PROGRAM cl_clinic_signoff_specialty
 ...
 SELECT INTO "nl:"
  ...
  FROM sch_res_group srg,
   ...
   encounter enc
  PLAN (srg
   WHERE (srg.res_group_id=t_record->res_group_id)
    AND srg.active_ind=1)
   JOIN (srl
   WHERE srl.res_group_id=srg.res_group_id
    AND srl.active_ind=1
    AND srl.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
   JOIN (r
   WHERE r.resource_cd=srl.resource_cd
    AND r.active_ind=1)
   JOIN (a2
   WHERE a2.resource_cd=r.resource_cd
  ...
  HEAD a.schedule_id
   IF (uar_get_code_meaning(a2.appt_location_cd) != "ANCILSURG")
  ...
  FOOT  a.schedule_id
   IF (uar_get_code_meaning(a2.appt_location_cd) != "ANCILSURG")
    reply->query_qual[reply->query_qual_cnt].beg_dt_tm = cnvtdatetime(a.beg_dt_tm), reply->
    query_qual[reply->query_qual_cnt].duration = a.duration, reply->query_qual[reply->query_qual_cnt]
