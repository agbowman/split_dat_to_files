CREATE PROGRAM bed_get_related_items:dba
 ...
 ENDIF
 IF ( NOT (validate(relatedcodesetsrequest,0)))
  FREE RECORD relatedcodesetsrequest
  RECORD relatedcodesetsrequest(
    1 event_codes[*]
      2 code_value = f8
      2 concept_cki = vc
      2 display = vc
 ...
 DECLARE mill_name = vc WITH protect, constant(request->related_item.mill_name)
 DECLARE definition = vc WITH protect, constant(request->related_item.definition)
 DECLARE description = vc WITH protect, constant(request->related_item.description)
 DECLARE identifier = vc WITH protect, constant(request->related_item.identifier)
 DECLARE activity_type_cd = f8 WITH protect, constant(request->related_item.activity_type_cd)
 DECLARE code_value = f8 WITH protect, constant(request->related_item.code_value)
 DECLARE codesetindex = i4 WITH protect, noconstant(0)
 DECLARE addrelateditemtoreply(codeset=i4,millname=vc,definition=vc,description=vc,identifier=vc,
  activitytypecd=f8) = null
 DECLARE getrelatedeventcodesanddtas(dummyvar=i2) = null
 DECLARE getrelatedeventsetsanddtas(dummyvar=i2) = null
 DECLARE getrelatedeventcodesandeventsets(dummyvar=i2) = null
 DECLARE getrelatedordercatalogs(dummyvar=i2) = null
 DECLARE getrelatedorders(dummyvar=i2) = null
 FOR (codesetindex = 1 TO size(request->related_item.code_sets,5))
   IF ((request->related_item.code_sets[codesetindex].code_set=93))
    CALL getrelatedeventcodesanddtas(0)
   ELSEIF ((request->related_item.code_sets[codesetindex].code_set=72))
    CALL getrelatedeventsetsanddtas(0)
   ELSEIF ((request->related_item.code_sets[codesetindex].code_set=14003))
    CALL getrelatedeventcodesandeventsets(0)
   ELSEIF ((request->related_item.code_sets[codesetindex].code_set=200))
    CALL getrelatedordercatalogs(0)
   ELSEIF ((request->related_item.code_sets[codesetindex].code_set=- (1)))
    CALL getrelatedorders(0)
   ENDIF
 ENDFOR
#exit_script
 CALL bedexitscript(0)
 SUBROUTINE addrelateditemtoreply(codeset,millname,definition,description,identifier,activitytypecd,
  codevalue)
   CALL bedlogmessage("addRelatedItemToReply","Entering ...")
   ...
   ENDIF
   CALL logdebugmessage("Code Set:",codeset)
   CALL logdebugmessage("Mill Name:",millname)
   CALL logdebugmessage("Definition",definition)
   CALL logdebugmessage("Description:",description)
   CALL logdebugmessage("Identifier:",identifier)
   CALL logdebugmessage("Activity Type Cd:",activitytypecd)
   CALL logdebugmessage("Code Value:",codevalue)
   SET found = locateval(index,1,size(reply->related_items,5),codeset,reply->related_items[index].
    code_set,
    millname,reply->related_items[index].mill_name,definition,reply->related_items[index].definition,
    description,
    reply->related_items[index].description,identifier,reply->related_items[index].identifier,
    activitytypecd,reply->related_items[index].activity_type.code_value,
    codevalue,reply->related_items[index].code_value)
   CALL logdebugmessage("found:",found)
   IF (found=0)
    CALL logdebugmessage("Activity Type Display:",uar_get_code_display(activitytypecd))
    CALL logdebugmessage("Activity Type Meaning:",uar_get_code_meaning(activitytypecd))
    SET repcnt = (size(reply->related_items,5)+ 1)
    SET stat = alterlist(reply->related_items,repcnt)
    SET reply->related_items[repcnt].code_set = codeset
    SET reply->related_items[repcnt].mill_name = millname
    SET reply->related_items[repcnt].definition = definition
    SET reply->related_items[repcnt].description = description
    SET reply->related_items[repcnt].identifier = identifier
    SET reply->related_items[repcnt].activity_type.code_value = activitytypecd
    SET reply->related_items[repcnt].activity_type.display = uar_get_code_display(activitytypecd)
    SET reply->related_items[repcnt].activity_type.meaning = uar_get_code_meaning(activitytypecd)
    SET reply->related_items[repcnt].code_value = codevalue
   ENDIF
   IF (validate(debug,0)=1)
    CALL echorecord(reply)
   ENDIF
   CALL bedlogmessage("addRelatedItemToReply","Exiting ...")
 END ;Subroutine
 SUBROUTINE getrelatedeventcodesanddtas(dummyvar)
   CALL bedlogmessage("getRelatedEventCodesAndDTAs","Entering ...")
   DECLARE escnt = i4 WITH protect, noconstant(0)
   DECLARE esindex = i4 WITH protect, noconstant(0)
   DECLARE ecindex = i4 WITH protect, noconstant(0)
   DECLARE dtaindex = i4 WITH protect, noconstant(0)
   DECLARE eventsetcd = f8 WITH protect, noconstant(0.0)
   SET eventsetcd = geteventsetcodefromeventsetname(mill_name)
   CALL addrelateditemtoreply(93,mill_name,definition,description,identifier,
    0.0,eventsetcd)
   SET stat = initrec(relatedcodesetsrequest)
   SET stat = initrec(eventcodesreply)
   SET escnt = (size(relatedcodesetsrequest->event_sets,5)+ 1)
   SET stat = alterlist(relatedcodesetsrequest->event_sets,escnt)
   SET relatedcodesetsrequest->event_sets[escnt].event_set_name = mill_name
   SET relatedcodesetsrequest->event_sets[escnt].concept_cki = identifier
   IF (validate(debug,0)=1)
    CALL echorecord(relatedcodesetsrequest)
   ENDIF
   CALL geteventcodefromeventset(relatedcodesetsrequest,eventcodesreply)
   FOR (esindex = 1 TO size(eventcodesreply->eventcodereplystructure,5))
     FOR (ecindex = 1 TO size(eventcodesreply->eventcodereplystructure[esindex].event_codes,5))
       CALL addrelateditemtoreply(72,eventcodesreply->eventcodereplystructure[esindex].event_codes[
        ecindex].display,eventcodesreply->eventcodereplystructure[esindex].event_codes[ecindex].
        definition,eventcodesreply->eventcodereplystructure[esindex].event_codes[ecindex].description,
        getconceptckifromcodevalue(eventcodesreply->eventcodereplystructure[esindex].event_codes[
         ecindex].code_value),
        0.0,eventcodesreply->eventcodereplystructure[esindex].event_codes[ecindex].code_value)
     ENDFOR
   ENDFOR
   ...
   IF (validate(debug,0)=1)
    CALL echorecord(relatedcodesetsrequest)
   ENDIF
   CALL getdtafromeventcode(relatedcodesetsrequest,dtacodesreply)
   FOR (dtaindex = 1 TO size(dtacodesreply->dta_code_reltns,5))
     CALL addrelateditemtoreply(14003,dtacodesreply->dta_code_reltns[dtaindex].display,dtacodesreply
      ->dta_code_reltns[dtaindex].definition,getdescriptionfromcodevalue(dtacodesreply->
       dta_code_reltns[dtaindex].task_assay_cd),getconceptckifromcodevalue(dtacodesreply->
       dta_code_reltns[dtaindex].task_assay_cd),
      getactivitytypecodefromtaskassaycode(dtacodesreply->dta_code_reltns[dtaindex].task_assay_cd),
      dtacodesreply->dta_code_reltns[dtaindex].task_assay_cd)
   ENDFOR
   CALL bedlogmessage("getRelatedEventCodesAndDTAs","Exiting ...")
 END ;Subroutine
 SUBROUTINE getrelatedeventsetsanddtas(dummyvar)
   CALL bedlogmessage("getRelatedEventSetsAndDTAs","Entering ...")
   DECLARE eccnt = i4 WITH protect, noconstant(0)
   DECLARE esindex = i4 WITH protect, noconstant(0)
   DECLARE ecindex = i4 WITH protect, noconstant(0)
   DECLARE dtaindex = i4 WITH protect, noconstant(0)
   CALL addrelateditemtoreply(72,mill_name,definition,description,identifier,
    0.0,code_value)
   SET stat = initrec(relatedcodesetsrequest)
   SET stat = initrec(eventsetreply)
   SET eccnt = (size(relatedcodesetsrequest->event_codes,5)+ 1)
   SET stat = alterlist(relatedcodesetsrequest->event_codes,eccnt)
   SET relatedcodesetsrequest->event_codes[eccnt].code_value = code_value
   SET relatedcodesetsrequest->event_codes[eccnt].concept_cki = identifier
   SET relatedcodesetsrequest->event_codes[eccnt].display = mill_name
   IF (validate(debug,0)=1)
    CALL echorecord(relatedcodesetsrequest)
   ENDIF
   CALL geteventsetfromeventcode(relatedcodesetsrequest,eventsetreply)
   FOR (esindex = 1 TO size(eventsetreply->event_set_reltns,5))
     CALL addrelateditemtoreply(93,eventsetreply->event_set_reltns[esindex].display,eventsetreply->
      event_set_reltns[esindex].definition,getdescriptionfromcodevalue(eventsetreply->
       event_set_reltns[esindex].event_set_cd),getconceptckifromcodevalue(eventsetreply->
       event_set_reltns[esindex].event_set_cd),
      0.0,eventsetreply->event_set_reltns[esindex].event_set_cd)
   ENDFOR
   SET stat = initrec(dtacodesreply)
   IF (validate(debug,0)=1)
    CALL echorecord(relatedcodesetsrequest)
   ...
   FOR (dtaindex = 1 TO size(dtacodesreply->dta_code_reltns,5))
     CALL addrelateditemtoreply(14003,dtacodesreply->dta_code_reltns[dtaindex].display,dtacodesreply
      ...
      dtacodesreply->dta_code_reltns[dtaindex].task_assay_cd)
   ENDFOR
   CALL bedlogmessage("getRelatedEventSetsAndDTAs","Exiting ...")
 END ;Subroutine
 SUBROUTINE getrelatedeventcodesandeventsets(dummyvar)
   CALL bedlogmessage("getRelatedEventCodesAndEventSets","Entering ...")
   DECLARE dtacnt = i4 WITH protect, noconstant(0)
   DECLARE esindex = i4 WITH protect, noconstant(0)
   DECLARE ecindex = i4 WITH protect, noconstant(0)
   DECLARE dtaindex = i4 WITH protect, noconstant(0)
   DECLARE taskassaycd = f8 WITH protect, noconstant(0.0)
   SET taskassaycd = gettaskassaycdfrommnemonicandacvitytype(mill_name,activity_type_cd)
   CALL addrelateditemtoreply(14003,mill_name,definition,description,identifier,
    activity_type_cd,taskassaycd)
   SET stat = initrec(relatedcodesetsrequest)
   SET stat = initrec(eventcodesreplyfromdta)
   SET dtacnt = (size(relatedcodesetsrequest->dtas,5)+ 1)
   SET stat = alterlist(relatedcodesetsrequest->dtas,dtacnt)
   SET relatedcodesetsrequest->dtas[dtacnt].code_value = taskassaycd
   SET relatedcodesetsrequest->dtas[dtacnt].concept_cki = identifier
   SET relatedcodesetsrequest->dtas[dtacnt].display = mill_name
   IF (validate(debug,0)=1)
    CALL echorecord(relatedcodesetsrequest)
   ENDIF
   CALL geteventcodefromdta(relatedcodesetsrequest,eventcodesreplyfromdta)
   FOR (ecindex = 1 TO size(eventcodesreplyfromdta->event_codes,5))
     CALL addrelateditemtoreply(72,eventcodesreplyfromdta->event_codes[ecindex].display,
      eventcodesreplyfromdta->event_codes[ecindex].definition,getdescriptionfromcodevalue(
       eventcodesreplyfromdta->event_codes[ecindex].event_cd),getconceptckifromcodevalue(
       eventcodesreplyfromdta->event_codes[ecindex].event_cd),
      0.0,eventcodesreplyfromdta->event_codes[ecindex].event_cd)
   ENDFOR
   SET stat = initrec(eventsetreply)
   IF (validate(debug,0)=1)
    CALL echorecord(relatedcodesetsrequest)
   ...
   FOR (esindex = 1 TO size(eventsetreply->event_set_reltns,5))
     CALL addrelateditemtoreply(93,eventsetreply->event_set_reltns[esindex].display,eventsetreply->
      ...
      0.0,eventsetreply->event_set_reltns[esindex].event_set_cd)
   ENDFOR
   CALL bedlogmessage("getRelatedEventCodesAndEventSets","Exiting ...")
 END ;Subroutine
 SUBROUTINE getrelatedordercatalogs(dummyvar)
   CALL bedlogmessage("getRelatedOrderCatalogs","Entering ...")
   DECLARE ocnt = i4 WITH protect, noconstant(0)
   DECLARE oindex = i4 WITH protect, noconstant(0)
   DECLARE codevalue = f8 WITH protect, noconstant(0.0)
   SET codevalue = getcodevaluefromdisplayandcodeset(mill_name,200,definition)
   CALL addrelateditemtoreply(200,mill_name,definition,description,identifier,
    0.0,codevalue)
   SET stat = initrec(relatedcodesetsrequest)
   SET stat = initrec(ordersreply)
   SET ocnt = (size(relatedcodesetsrequest->order_codes,5)+ 1)
   SET stat = alterlist(relatedcodesetsrequest->order_codes,ocnt)
   SET relatedcodesetsrequest->order_codes[ocnt].code_value = codevalue
   SET relatedcodesetsrequest->order_codes[ocnt].concept_cki = identifier
   SET relatedcodesetsrequest->order_codes[ocnt].display = mill_name
   IF (validate(debug,0)=1)
    CALL echorecord(relatedcodesetsrequest)
   ENDIF
   CALL getordersfromorderscatalog(relatedcodesetsrequest,ordersreply)
   FOR (oindex = 1 TO size(ordersreply->orders,5))
     CALL addrelateditemtoreply(- (1),ordersreply->orders[oindex].display,getdefinitionfromcodevalue(
       ordersreply->orders[oindex].code_value),getdescriptionfromcodevalue(ordersreply->orders[oindex
       ].code_value),ordersreply->orders[oindex].identifier,
      0.0,ordersreply->orders[oindex].code_value)
   ENDFOR
   CALL bedlogmessage("getRelatedOrderCatalogs","Exiting ...")
 END ;Subroutine
 SUBROUTINE getrelatedorders(dummyvar)
   CALL bedlogmessage("getRelatedOrders","Entering ...")
   DECLARE ocnt = i4 WITH protect, noconstant(0)
   DECLARE oindex = i4 WITH protect, noconstant(0)
   DECLARE ordercatalogcd = f8 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM order_catalog oc
    PLAN (oc
     WHERE oc.primary_mnemonic=mill_name)
    DETAIL
     ordercatalogcd = oc.catalog_cd
    WITH nocounter
   ;end select
   CALL addrelateditemtoreply(- (1),mill_name,definition,description,identifier,
    0.0,ordercatalogcd)
   SET stat = initrec(relatedcodesetsrequest)
   SET stat = initrec(ordersreply)
   SET ocnt = (size(relatedcodesetsrequest->order_catalogs,5)+ 1)
   SET stat = alterlist(relatedcodesetsrequest->order_catalogs,ocnt)
   SET relatedcodesetsrequest->order_catalogs[ocnt].catalog_cd = ordercatalogcd
   SET relatedcodesetsrequest->order_catalogs[ocnt].concept_cki = identifier
   IF (validate(debug,0)=1)
    CALL echorecord(relatedcodesetsrequest)
   ENDIF
   CALL getordersfromcodeset200(relatedcodesetsrequest,ordersreply)
   FOR (oindex = 1 TO size(ordersreply->orders,5))
     CALL addrelateditemtoreply(200,ordersreply->orders[oindex].display,getdefinitionfromcodevalue(
       ordersreply->orders[oindex].code_value),getdescriptionfromcodevalue(ordersreply->orders[oindex
       ].code_value),ordersreply->orders[oindex].identifier,
      0.0,ordersreply->orders[oindex].code_value)
   ENDFOR
   CALL bedlogmessage("getRelatedOrders","Exiting ...")
 END ;Subroutine
END GO
