CREATE PROGRAM cv_chk_omf_cvaccg:dba
 SET error_msg = fillstring(200," ")
 SET failure = "F"
 SET v_count1 = 0
 SET v_cnt_indicator = 394
 SET v_cnt_pv_view = 3
 SET v_cnt_view_options = 370
 SET v_cnt_view_indicator = 735
 SET v_cnt_grid = 3
 SET v_cnt_grid_column = 39
 SET v_cnt_calc_indicator = 8
 ...
 IF (failure="T")
  ...
  IF (v_indicator_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_indicator_CVACCG")
  ENDIF
  IF (v_pv_view_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_pv_view_CVACCG")
  ENDIF
  IF (v_view_options_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_vo_type/omf_vo_type_display_CVACCG")
  ENDIF
  IF (v_view_indicator_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_view_indicator_CVACCG")
  ENDIF
  IF (v_grid_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_grid_CVACCG")
  ENDIF
  IF (v_grid_column_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_grid_column_CVACCG")
  ENDIF
  IF (v_calc_indicator_flag=1)
   SET error_msg = concat(trim(error_msg,3),",omf_calc_indicator_CVACCG")
  ENDIF
  CALL echo(error_msg)
 ELSE
  CALL echo("CVNet OMF ACC II Views successfully loaded!")
 ENDIF
END GO
