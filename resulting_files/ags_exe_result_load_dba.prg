CREATE PROGRAM ags_exe_result_load:dba
 CALL echo("<===== AGS_EXE_RESULT_LOAD Begin =====>")
 SET script_ver = "001 05/30/06"
 ...
 DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_exe_result_load_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> ags_exe_result_load"
...
#skip_parse
 ...
 FOR (exe_idx = 1 TO work->qual_knt)
   ...
   IF (current_task_id > 0)
    EXECUTE ags_result_load value(current_task_id), value(check_for_dups)
   ELSE
    SET failed = input_error
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> ags_exe_result_load"
 CALL handle_logging(sstatus_file_name,sstatus_email,ilog_status)
 CALL echo("<===== AGS_EXE_RESULT_LOAD End =====>")
 SET script_ver = "001 05/30/06"
END GO
