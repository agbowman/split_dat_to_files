CREATE PROGRAM dm2_ni_load_pkg_hist_wrapper
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dnlphw_loc = vc WITH protect, noconstant(concat(trim(logical("cer_install")),"/"))
 DECLARE dnlphw_csv_file = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dnlphw_idx = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_ni_load_hist_w",".log","dm2_ni_load_pkg_hist_wrapper")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_ni_load_pkg_hist_wrapper."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dehr_verify_admin_synonym(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Loading drr_env_hist_misc record structure manually."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET drr_env_hist_misc->cnt = 3
 SET stat = alterlist(drr_env_hist_misc->qual,drr_env_hist_misc->cnt)
 SET drr_env_hist_misc->qual[1].table_name = "DM_ALPHA_FEATURES_ENV"
 SET drr_env_hist_misc->qual[1].table_alias = "dafe"
 SET drr_env_hist_misc->qual[1].csv_file_name = concat("dm2_ni_load_hist_afe_",dm2_create_dom->dbtype,
  ".csv")
 SET drr_env_hist_misc->qual[2].table_name = "DM_RDDS_REQ_INSTALL_ENV"
 SET drr_env_hist_misc->qual[2].table_alias = "drrie"
 SET drr_env_hist_misc->qual[2].csv_file_name = concat("dm2_ni_load_hist_drrie_",dm2_create_dom->
  dbtype,".csv")
 SET drr_env_hist_misc->qual[3].table_name = "DM_OCD_LOG"
 SET drr_env_hist_misc->qual[3].table_alias = "dol"
 SET drr_env_hist_misc->qual[3].csv_file_name = concat("dm2_ni_load_hist_dol_",dm2_create_dom->dbtype,
  ".csv")
 FOR (dnlphw_idx = 1 TO drr_env_hist_misc->cnt)
   SET dm_err->eproc = concat("Verify file exists in cer_install for: ",drr_env_hist_misc->qual[
    dnlphw_idx].table_name)
   CALL disp_msg("",dm_err->logfile,0)
   IF (dm2_findfile(concat(dnlphw_loc,drr_env_hist_misc->qual[dnlphw_idx].csv_file_name))=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Package history file could not be found in cer_install."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
   EXECUTE dm2_dbimport concat(dnlphw_loc,drr_env_hist_misc->qual[dnlphw_idx].csv_file_name),
   "dm2_ni_load_pkg_hist", 1000
   IF ((dm_err->err_ind=1))
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_ni_load_pkg_hist_wrapper completed"
 ENDIF
 CALL final_disp_msg("dm2_ni_load_hist_w")
END GO
