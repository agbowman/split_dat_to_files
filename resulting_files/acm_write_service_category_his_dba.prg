CREATE PROGRAM acm_write_service_category_his:dba
 ...
 DECLARE ubound = i4 WITH protect, noconstant(reply->service_category_hist_qual_cnt)
 SET stat = alterlist(reply->service_category_hist_qual,reply->service_category_hist_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->service_category_hist_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->service_category_hist_qual[index].svc_cat_hist_id = acm_request->
     service_category_hist_qual[index].svc_cat_hist_id
     SET reply->service_category_hist_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_service_category_hist
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->act_cnt > 0))
  EXECUTE acm_act_service_category_hist
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_service_category_hist
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_service_category_hist
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
