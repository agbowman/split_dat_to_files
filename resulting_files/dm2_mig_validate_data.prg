CREATE PROGRAM dm2_mig_validate_data
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_mig_validata",".log","dm2_mig_validate_data LOGFILE")=0)
  GO TO exit_script
 ENDIF
 DECLARE dmvd_db_link = vc WITH protect, noconstant("")
 SET dmvd_db_link =  $1
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->emsg = "Parameter usage: dm2_mig_validate_data <source database link>"
  GO TO exit_script
 ENDIF
 ...
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 EXECUTE dm2_validate_data value(dmvd_db_link), 1000, "cer_install:dm2_mig_validate_tables.txt"
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 GO TO exit_script
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_validate_data completed successfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 CALL final_disp_msg("dm2_mig_validata")
END GO
