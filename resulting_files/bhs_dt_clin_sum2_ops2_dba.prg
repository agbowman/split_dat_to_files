CREATE PROGRAM bhs_dt_clin_sum2_ops2:dba
 SET unit_cd = 0
 ...
 SELECT INTO "nl:"
  ...
  .seq].room,
  bed = act_pat->qual[d.seq].bed, result_age = datetimediff(cnvtdatetime(sysdate),ce.event_end_dt_tm,
   3), visit_reason = trim(act_pat->qual[d.seq].visit_reason)
    ...
    SELECT INTO value(pat->qual[x].filename)
     ...
     DETAIL
      ...
      FOR (d1 = 1 TO size(pat->qual[x].nut,5))
        ...
        ENDIF
      ENDFOR
      ycol += 5, title = build("-","{b}Respiratory Therapy Orders:"),
      CALL print(calcpos(xcol,ycol)),
      "{b}", title, row + 1,
      ycol += 10
      FOR (r1 = 1 TO size(pat->qual[x].resp,5))
        ...
        ENDIF
      ENDFOR
      ycol += 5, title = build("-","{b}Assess/Monitor/Treat:"),
      CALL print(calcpos(xcol,ycol)),
      "{b}", title, row + 1,
      ycol += 10
      FOR (m1 = 1 TO size(pat->qual[x].monitor,5))
        ...
        ENDIF
      ENDFOR
      l += 1, ycol += 5, title = build(l,"-","Nurse Communication Orders:"),
      ...
      FOR (n1 = 1 TO size(pat->qual[x].ntnord,5))
        ...
        ENDIF
      ENDFOR
      l += 1, ycol += 5, title = build(l,"-","MD to RN Communication Orders:"),
      CALL print(calcpos(xcol,ycol)), "{b}", title,
      row + 1, ycol += 10
      FOR (n2 = 1 TO size(pat->qual[x].mdtrnord,5))
        ...
        ENDIF
      ENDFOR
      l += 1, ycol += 5, title = build(l,"-","{b}Invasive Lines/Tubes/Drains:"),
      CALL print(calcpos(xcol,ycol)), "{b}", title,
      row + 1, ycol += 10
