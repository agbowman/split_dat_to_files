CREATE PROGRAM ccps_pha_lbt_medsyr_1x35
 PROMPT
 ...
 WITH protect
 DECLARE fin_ord_syr_sec(ncalc=i2) = f8 WITH protect
 DECLARE fin_ord_syr_secabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 ...
 END ;Subroutine
 SUBROUTINE fin_ord_syr_sec(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = fin_ord_syr_secabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE fin_ord_syr_secabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.140000), private
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_default816777215)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET oldbackcolor = uar_rptsetbackcolor(_hreport,rpt_black)
 ...
 SUBROUTINE foot_secabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_aztec,(offsetx+ 3.106),(offsety+ 0.000))
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.273),(offsety+ 0.109),(offsetx+ 0.898),(offsety+
     0.109))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 1.314),(offsety+ 0.109),(offsetx+ 1.939),(offsety+
     0.109))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 2.429),(offsety+ 0.109),(offsetx+ 3.054),(offsety+
     0.109))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CCPS_PHA_LBT_MEDSYR_1X35"
   SET rptreport->m_pagewidth = 3.50
 ...
 WHILE (((_bcontdemographics_sec=1) OR (bfirsttime=1)) )
   ...
   SET bfirsttime = 0
 ENDWHILE
 SET _fdrawheight = fin_ord_syr_sec(rpt_calcheight)
 IF (((_yoffset+ _fdrawheight) > _fenddetail))
  CALL pagebreak(0)
 ENDIF
 SET dummy_val = fin_ord_syr_sec(rpt_render)
