CREATE PROGRAM dm2_ref_data_mover_6579:dba
 ...
 DECLARE cust_prem_flag = i4
 DECLARE cust_value1 = vc
 DECLARE cust_to_value1 = vc
 DECLARE cust_value1_pos = i4
 DECLARE cust_rpt_missing = vc
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_value1_pos = get_col_pos(cust_tab_name,"VALUE1")
 SET cust_prem_flag = cnvtint(get_value(cust_tab_name,"PREMISE_TYPE_FLAG","FROM"))
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_6579
 ENDIF
 IF (cust_prem_flag=2)
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_value1_pos].translated = 0
  SET cust_value1 = get_value(cust_tab_name,"VALUE1","FROM")
  IF (cnvtreal(cust_value1) > 0)
   SET cust_to_value1 = select_merge_translate(cust_value1,"CODE_VALUE")
   IF (cust_to_value1 != "No Trans")
    CALL put_value(cust_tab_name,"VALUE1",cust_to_value1)
   ELSE
    SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_to_value1))
    CALL evaluate_rpt_missing(cust_rpt_missing)
   ENDIF
  ELSE
   CALL put_value(cust_tab_name,"VALUE1","0")
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"DRC_PREMISE_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1)
   ...
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"DRC_PREMISE_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"DRC_PREMISE_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  DRC_PREMISE_ID")
     ELSE
      GO TO exit_6579
     ENDIF
    ELSE
   ...
   SET from_id = get_value(cust_tab_name,"DRC_PREMISE_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6579
   ELSE
 ...
 CALL echo("")
 CALL echo("")
 SET idcd_check = is_translated(cust_tab_name,"VALUE1")
 IF (idcd_check=1)
  SET idcd_check = 0
  SET idcd_ind = 1
  SET idcd_check = is_translated(cust_tab_name,"ALL")
 ...
 ELSE
  ...
  IF (cust_err_val=0)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "This log_id wasn't translated because of the VALUE1 column"
  ENDIF
 ENDIF
#exit_6579
 FREE SET idcd_check
