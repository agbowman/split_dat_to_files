CREATE PROGRAM cv_frpt_daily_vol_daily
 PROMPT
 ...
 SUBROUTINE get_reply_obj(dummy)
  ...
  SELECT
   ...
   cv_list_rpl_mth_yr = reply_obj->cv_list[d1.seq].rpl_mth_yr, cv_year = year(reply_obj->cv_list[d1
    .seq].rpl_mth_yr),
   cv_month = month(reply_obj->cv_list[d1.seq].rpl_mth_yr), cv_day = day(reply_obj->cv_list[d1.seq].
    rpl_mth_yr)
   FROM (dummyt d1  WITH seq = value(size(reply_obj->cv_list,5)))
   PLAN (d1)
   ORDER BY cv_year, cv_month, cv_day,
    cv_list_rpl_catalog_disp
   HEAD REPORT
    _d0 = cv_list_rpl_catalog_disp, _d1 = cv_list_rpl_mth_yr, sdate = format(cnvtdatetime(cnvtdate2(
    ...
    ndaylightvar = 0, stimezone = datetimezonebyindex(curtimezoneapp,noffsetvar,ndaylightvar,7,
     cnvtdatetime(curdate,curtime)), cnt_catalog = 0,
    total_cnt = 0, tgraph = 0, max_tgraph = 0,
    ...
    m_series[1].name = uar_i18ngetmessage(_hi18nhandle,"PROCEDURESERIESGPH","Procedures"),
   ...
   HEAD cv_day
    cur_year = format(year(cv_list_rpl_mth_yr),"####"), cur_month = format(cv_list_rpl_mth_yr,
     "MMMMMMMMM;;D"), _fdrawheight = headdaysection(rpt_calcheight)
   ...
   FOOT  cv_list_rpl_catalog_disp
    total_cnt = (total_cnt+ cnt_catalog), _fdrawheight = footcatalogdispsection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = footcatalogdispsection(rpt_render), cnt_catalog = 0
   FOOT  cv_day
    ...
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = footdaysection(rpt_render), total_cnt = 0, stat = alterlist(rptgraphrec->m_labels,
     _nfieldscnt),
    rptgraphrec->m_labels[_nfieldscnt].label = build2(format(cv_list_rpl_mth_yr,"@LONGDATE"),char(0)),
    stat = alterlist(rptgraphrec->m_series[1].y_values,_nfieldscnt), rptgraphrec->m_series[1].
    y_values[_nfieldscnt].y_f8 = tgraph,
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.375),(offsety+ 1.443),(offsetx+ 6.667),(offsety+
     1.443))
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(uar_i18ngetmessage(_hi18nhandle,
        "SIGNEDPROCEDURESBYDAY","Signed Procedures by Day")),char(0)))
    SET rptsd->m_flags = 0
 ...
 SUBROUTINE headdaysectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.188
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(format(cv_list_rpl_mth_yr,
        "@LONGDATE")),char(0)))
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
   ENDIF
   RETURN(sectionheight)
 ...
 SUBROUTINE headcatalogdispsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.188
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(cv_list_rpl_catalog_disp),char(0)
      ))
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
   ENDIF
   RETURN(sectionheight)
 ...
 SUBROUTINE footcatalogdispsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.188
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(cnvtstring(cnt_catalog)),char(0))
     )
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
 ...
 SUBROUTINE graphsectionabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(3.000000), private
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptgraphrec->m_fwidth = 5.927000
    SET rptgraphrec->m_fheight = 2.740000
    SET rptgraphrec->m_stitle = ""
    SET rptgraphrec->m_ssubtitle = ""
    SET rptgraphrec->m_sxtitle = ""
    SET rptgraphrec->m_sytitle = uar_i18ngetmessage(_hi18nhandle,"PROCSIGN","Procedures Signed")
 ...
 SUBROUTINE nodatasectionabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(1.360000), private
   IF ( NOT (size(reply_obj->cv_list,5) <= 0))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CV_FRPT_DAILY_VOL_DAILY"
