CREATE PROGRAM acm_chg_prsnl_prsnl_reltn:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->prsnl_prsnl_reltn_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   prsnl_prsnl_reltn p
  SET p.prsnl_prsnl_reltn_id = acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].
   prsnl_prsnl_reltn_id, p.beg_effective_dt_tm = cnvtdatetime(acm_request->prsnl_prsnl_reltn_qual[
    xref->chg[d.seq].idx].beg_effective_dt_tm), p.contributor_system_cd =
   IF ((acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.data_status_cd = acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].data_status_cd, p
   .data_status_prsnl_id = acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].
   data_status_prsnl_id, p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_prsnl_reltn_qual[
    xref->chg[d.seq].idx].end_effective_dt_tm),
   p.organization_id = acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].organization_id, p
   .person_id = acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].person_id, p
   .prsnl_prsnl_reltn_cd = acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].
   prsnl_prsnl_reltn_cd,
   p.related_person_id = acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].related_person_id,
   p.active_ind = acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].active_ind, p
   .active_status_cd = acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].active_status_cd,
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.prsnl_prsnl_reltn_id=acm_request->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].
   prsnl_prsnl_reltn_id))
  WITH nocounter, status(reply->prsnl_prsnl_reltn_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->prsnl_prsnl_reltn_qual[xref->chg[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,p.prsnl_prsnl_reltn_id,acm_request->prsnl_prsnl_reltn_qual[xref->chg[
     t1].idx].prsnl_prsnl_reltn_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.prsnl_prsnl_reltn_id,acm_request->prsnl_prsnl_reltn_qual[xref->
      chg[t1].idx].prsnl_prsnl_reltn_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->prsnl_prsnl_reltn_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->prsnl_prsnl_reltn_qual[idx].status = - (1), reply->prsnl_prsnl_reltn_qual[idx].
      prsnl_prsnl_reltn_id = acm_request->prsnl_prsnl_reltn_qual[idx].prsnl_prsnl_reltn_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->prsnl_prsnl_reltn_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].contributor_system_cd = p.contributor_system_cd
     ENDIF
     IF (findstring("DATA_STATUS_CD,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].data_status_cd = p.data_status_cd
     ENDIF
     IF (findstring("DATA_STATUS_PRSNL_ID,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].data_status_prsnl_id = p.data_status_prsnl_id
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("ORGANIZATION_ID,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].organization_id = p.organization_id
     ENDIF
     IF (findstring("PERSON_ID,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].person_id = p.person_id
     ENDIF
     IF (findstring("PRSNL_PRSNL_RELTN_CD,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].prsnl_prsnl_reltn_cd = p.prsnl_prsnl_reltn_cd
     ENDIF
     IF (findstring("RELATED_PERSON_ID,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].related_person_id = p.related_person_id
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->prsnl_prsnl_reltn_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_prsnl_reltn_qual[xref->chg[index].idx].status=0))
