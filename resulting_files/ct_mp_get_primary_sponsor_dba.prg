CREATE PROGRAM ct_mp_get_primary_sponsor:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_primary_sponsor"
 ...
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_primary_sponsor","Invalid JSON Request",
   PUBLIC::"reply")
  GO TO exit_script
 ENDIF
 ...
 SUBROUTINE getprimarysponsor(null)
   CALL log_message("In GetPrimarySponsor()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   ...
   IF (protcount > 0)
    ...
    SELECT
     ...
     PLAN (d)
      JOIN (pgs
      WHERE (pgs.prot_amendment_id=request->prot_list[d.seq].amendmentid))
      JOIN (org
     ...
     DETAIL
      ...
      "reply"->prot_list[d.seq]."prot_info"."primary_sponsor" = org.org_name
     WITH nocounter
    ;end select
   ELSE
    CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_primary_sponsor",
     "Did not receive any protocol ID",PUBLIC::"reply")
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit GetPrimarySponsor(), Elapsed time in seconds:",datetimediff(
      cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (g_debug_ind=1)
