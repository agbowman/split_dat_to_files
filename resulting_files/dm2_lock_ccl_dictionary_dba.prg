CREATE PROGRAM dm2_lock_ccl_dictionary:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dlld_os_cmd = vc WITH protect, noconstant("")
 DECLARE dlld_ccl_cmd = vc WITH protect, noconstant("")
 DECLARE dlld_push_ret = i2 WITH protect, noconstant(0)
 DECLARE dlld_ccldiraccess = vc WITH protect, noconstant("")
 IF (validate(reply->status_data.status,"ZZZ")="ZZZ")
  FREE RECORD reply
  RECORD reply(
    1 status_data
      2 status = c1
 ...
 IF (check_logfile("dm2_lock_ccl_dict_",".log","DM2_LOCK_LIVE_DICT LOG FILE...") != 1)
  SET reply->status_data.subeventstatus[1].targetobjectname = "Checking logfile"
  SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echo(concat("Session's CCLDIRACCESS before global lock op:",logical("CCLDIRACCESS")))
 ENDIF
 IF (cursys="AXP")
  SET dlld_os_cmd = "@cer_ocdtools:eucswapccl.com LOCK"
 ELSE
  SET dlld_os_cmd = "$cer_ocdtools/eucswapccl.ksh 'LOCK'"
 ENDIF
 SET dm_err->eproc = "Locking live CCL dictionary."
 CALL disp_msg("",dm_err->logfile,0)
 SET dlld_push_ret = dm2_push_dcl(dlld_os_cmd)
 IF ((dm_err->debug_flag > 1))
  CALL echo(concat("Session's CCLDIRACCESS after global lock op:",logical("CCLDIRACCESS")))
 ENDIF
 IF (dlld_push_ret=0)
  SET dm_err->eproc = "Locking live CCL dictionary."
  IF (cursys="AXP")
   SET dm_err->emsg = concat("Error occurred on call to eucswapccl.com 'LOCK', ",
    "please review cer_log:eucswapccl.log for the specific errors that occurred.")
  ELSE
   SET dm_err->emsg = concat("Error occurred on call to eucswapccl.ksh 'LOCK', ",
    "please review $cer_log/eucswapccl.log for the specific errors that occurred.")
  ENDIF
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  GO TO exit_program
 ENDIF
 SET dlld_ccldiraccess = logical("CCLDIRACCESS")
 SET dlld_ccldiraccess = replace(dlld_ccldiraccess,"READ","WRITE")
 SET dlld_ccl_cmd = concat('set logical CCLDIRACCESS "',dlld_ccldiraccess,'" go')
 IF (dm2_push_cmd(dlld_ccl_cmd,1)=0)
  SET reply->status_data.subeventstatus[1].targetobjectname = "Setting local logical"
  SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echo(concat("Session's CCLDIRACCESS after local logical op:",logical("CCLDIRACCESS")))
 ENDIF
...
#exit_program
 ...
 ENDIF
 SET dm_err->eproc = "...Ending DM2_LOCK_CCL_DICTIONARY"
 CALL final_disp_msg("dm2_lock_ccl_dict_")
END GO
