CREATE PROGRAM aps_get_diag_corr_groups:dba
...
#script
 ...
 SELECT INTO "nl:"
  cv.cdf_meaning, cv.code_value, pg.prsnl_group_name,
  pg_prsnl_group_id = pg.prsnl_group_id
  FROM code_value cv,
   prsnl_group pg
  PLAN (cv
  ...
  HEAD pg.prsnl_group_name
   IF (isresourceviewable(pg.service_resource_cd)=true)
    gcnt += 1
    IF (mod(gcnt,10)=1)
