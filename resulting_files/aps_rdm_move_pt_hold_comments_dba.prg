CREATE PROGRAM aps_rdm_move_pt_hold_comments:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_rdm_move_pt_hold_comments"
 ...
 SET sprogramfailed = "F"
 SET pt_commit_recs_whole = max_commit_recs_whole
 SET lt_commit_recs_whole = max_commit_recs_whole
 CALL logmsg("Getting ready to begin reading for processing_task hold_comment.",((e_dispmsg+
  e_rdmstatus)+ e_logtofile))
 SELECT INTO "nl:"
  FROM processing_task pt
  WHERE pt.processing_task_id > 0
   AND pt.hold_comment > " "
   AND pt.hold_comment_long_text_id IN (null, 0)
  DETAIL
   ...
   temp->qual[max_recs_to_commit].hold_comment = pt.hold_comment, temp->qual[max_recs_to_commit].
   updated_pt = "F", temp->qual[max_recs_to_commit].updated_lt = "F"
 ...
 IF (curqual=0)
  ...
  GO TO exit_script
 ENDIF
 CALL logmsg("Getting ready to begin updating long_text table.",((e_dispmsg+ e_rdmstatus)+
  e_logtofile))
 SET lt_cntr = max_recs_to_commit
 SET count = 0
 WHILE (lt_cntr >= 1)
   IF (lt_cntr >= lt_commit_recs_whole)
    SET lt_cntr = (lt_cntr - lt_commit_recs_whole)
   ELSE
    SET lt_commit_recs_whole = lt_cntr
    SET lt_cntr = 0
   ENDIF
   FOR (seq_retrieve = (count+ 1) TO (count+ lt_commit_recs_whole))
    SELECT INTO "nl:"
    ...
    IF (curqual=0)
     ROLLBACK
     CALL logmsg("Failed to obtain an accurate sequence number.",((e_dispmsg+ e_logtofile)+
      e_rdmstatus))
     SET sprogramfailed = "T"
     GO TO exit_script
    ENDIF
   ENDFOR
   INSERT  FROM long_text lt,
     (dummyt d1  WITH seq = value(lt_commit_recs_whole))
    SET lt.long_text_id = temp->qual[(count+ d1.seq)].hold_comment_long_text_id, lt.updt_cnt = 0, lt
     ...
     processing_task_id, lt.long_text = temp->qual[(count+ d1.seq)].hold_comment,
    ...
    WITH nocounter
   ;end insert
   IF (curqual != value(lt_commit_recs_whole))
    ROLLBACK
    CALL logmsg("Failed to update long_text table.",((e_dispmsg+ e_logtofile)+ e_rdmstatus))
    SET sprogramfailed = "T"
    GO TO exit_script
   ENDIF
   UPDATE  FROM processing_task pt,
     (dummyt d1  WITH seq = value(lt_commit_recs_whole))
    SET pt.hold_comment_long_text_id = temp->qual[(count+ d1.seq)].hold_comment_long_text_id, pt
     .updt_dt_tm = cnvtdatetime(curdate,curtime), pt.updt_cnt = (pt.updt_cnt+ 1),
   ...
   IF (curqual != value(lt_commit_recs_whole))
    ROLLBACK
    CALL logmsg("Failed to update processing_task table.",((e_dispmsg+ e_logtofile)+ e_rdmstatus))
    SET sprogramfailed = "T"
    GO TO exit_script
   ENDIF
   SET sprogramfailed = "F"
