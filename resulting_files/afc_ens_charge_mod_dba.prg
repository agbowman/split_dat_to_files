CREATE PROGRAM afc_ens_charge_mod:dba
 IF ("Z"=validate(afc_ens_charge_mod_vrsn,"Z"))
  DECLARE afc_ens_charge_mod_vrsn = vc WITH noconstant("675263.015")
 ENDIF
 SET afc_ens_charge_mod_vrsn = "675263.015"
 ...
 SET stat = alterlist(delchargeeventmodreq->objarray,1)
 ...
 IF ( NOT (validate(cs13019_bill_code)))
  DECLARE cs13019_bill_code = f8 WITH protect, noconstant(getcodevalue(13019,"BILL CODE",0))
 ENDIF
 FOR (ind = 1 TO request->charge_mod_qual)
   SET lockchargerequest->charge_item_id = request->charge_mod[ind].charge_item_id
   SET lockchargerequest->updt_cnt = validate(request->charge_mod[ind].updt_cnt,0)
   SET lockchargerequest->updt_cnt_ind = validate(request->charge_mod[ind].updt_cnt_ind,0)
   IF (validate(debug,- (1)) > 0)
 ...
 SET enscount = 0
 CALL echo("###### ENS_CHARGE_MOD")
 CALL echo("charge_mod_qual",0)
 CALL echo(cnvtstring(request->charge_mod_qual))
 IF ((request->charge_mod_qual > 0)
  AND (request->charge_mod[1].charge_item_id > 0.0))
  CALL getoldchargeitemdetails(request->charge_mod[1].charge_item_id)
 ENDIF
 FOR (enscount = 1 TO request->charge_mod_qual)
   CALL echo("ENS_CHARGE_MOD: ",0)
   CALL echo(request->charge_mod[enscount].action_type,0)
   CALL echo(" ensCount: ",0)
   CALL echo(cnvtstring(enscount))
   CASE (request->charge_mod[enscount].action_type)
    OF "ADD":
     SET action_begin = enscount
     SET action_end = enscount
     EXECUTE afc_add_charge_mod  WITH replace("REQUEST",request)
     IF (failed != false)
      GO TO check_error
     ENDIF
    OF "UPT":
     IF (checkdminfotoremoveparentcodes(0)
      AND ((uar_get_code_meaning(request->charge_mod[enscount].field1_id)="ICD9") OR (
     uar_get_code_meaning(request->charge_mod[enscount].field1_id)="MODIFIER")) )
      IF ( NOT (uar_get_code_meaning(request->charge_mod[enscount].field1_id)="MODIFIER"
       AND validate(request->charge_mod[enscount].charge_mod_source_cd,0.0) > 0)
       AND validate(request->charge_mod[enscount].charge_event_mod_id,0.0)=0.0)
       CALL writetocem(0)
      ENDIF
      DECLARE cmfield6 = vc WITH noconstant("")
      DECLARE billcodecdfmeaning = vc WITH noconstant("")
      SELECT INTO "nl:"
       FROM charge_mod cm
       PLAN (cm
        WHERE (cm.charge_mod_id=request->charge_mod[enscount].charge_mod_id)
         AND (cm.charge_item_id=request->charge_mod[enscount].charge_item_id))
       DETAIL
        cmfield6 = cm.field6, billcodecdfmeaning = uar_get_code_meaning(cm.field1_id)
       WITH nocounter
      ;end select
      IF (inactivateparentcodes(request->charge_mod[enscount].charge_item_id,cmfield6,
       billcodecdfmeaning))
     ...
     SET action_begin = enscount
     SET action_end = enscount
     EXECUTE afc_upt_charge_mod  WITH replace("REQUEST",request)
     IF (failed != false)
      GO TO check_error
     ENDIF
    OF "DEL":
     IF (checkdminfotoremoveparentcodes(0))
      ...
      SELECT INTO "nl:"
       ...
       WITH nocounter
      ;end select
      IF (((billcodecdfmeaning="ICD9") OR (billcodecdfmeaning="MODIFIER")) )
       DECLARE cmfield6 = vc WITH noconstant("")
       DECLARE billcodecdfmeaning = vc WITH noconstant("")
       SELECT INTO "nl:"
        FROM charge_mod cm
     ...
     SET action_begin = enscount
     SET action_end = enscount
     EXECUTE afc_del_charge_mod  WITH replace("REQUEST",request)
     IF (failed != false)
      GO TO check_error
     ENDIF
     DECLARE chargeeventid = f8 WITH protect, noconstant(0.0)
     DECLARE chargeeventmodid = f8 WITH protect, noconstant(0.0)
     DECLARE chargeeventupdtcnt = i4 WITH protect, noconstant(0)
     SELECT INTO "nl:"
      FROM charge c,
       charge_mod cm,
       charge_event_mod cem
      PLAN (c
       WHERE (c.charge_item_id=request->charge_mod[enscount].charge_item_id)
        AND c.active_ind=1)
       JOIN (cm
       WHERE cm.charge_item_id=c.charge_item_id
        AND (cm.charge_mod_id=request->charge_mod[enscount].charge_mod_id))
       JOIN (cem
      ...
      DETAIL
       chargeeventid = cem.charge_event_id, chargeeventmodid = cem.charge_event_mod_id,
       chargeeventupdtcnt = cem.updt_cnt
      WITH nocounter
     ;end select
     IF (chargeeventid > 0.0
      AND chargeeventmodid > 0.0)
      SET delchargeeventmodreq->objarray[1].action_type = "DEL"
      SET delchargeeventmodreq->objarray[1].charge_event_mod_id = chargeeventmodid
      SET delchargeeventmodreq->objarray[1].charge_event_id = chargeeventid
      SET delchargeeventmodreq->objarray[1].updt_cnt = chargeeventupdtcnt
     ...
     ENDIF
   ENDCASE
   DECLARE reprocessind = i2 WITH protect, noconstant(false)
   IF (uar_get_code_meaning(request->charge_mod[enscount].field1_id)="MODIFIER")
    SET reprocessind = true
   ELSEIF ((request->charge_mod[enscount].action_type="DEL"))
    DECLARE schedtype = f8 WITH protect, noconstant(0.0)
    SELECT INTO "nl:"
     FROM charge_mod cm
     WHERE (cm.charge_mod_id=request->charge_mod[enscount].charge_mod_id)
    ...
    ENDIF
   ENDIF
   IF (reprocessind=true
    AND checktierforcptmod(request->charge_mod[enscount].charge_item_id))
    IF (validate(debug,- (1)) > 0)
     CALL echo("Reprocess charge set to true, looking for an existing charge event")
    ENDIF
    DECLARE chargeeventid = f8 WITH protect, noconstant(0.0)
    SELECT INTO "nl:"
     FROM charge c
     WHERE (c.charge_item_id=request->charge_mod[enscount].charge_item_id)
     DETAIL
      chargeeventid = c.charge_event_id
     WITH nocounter
    ;end select
    ...
    DECLARE chargeitemfound = i2 WITH protect, noconstant(false)
    FOR (idx2 = 1 TO reprocesschargelist->charge_event_qual)
      IF ((reprocesschargelist->charge_event[idx2].charge_event_id=chargeeventid))
       IF (validate(debug,- (1)) > 0)
        CALL echo(build2("Existing charge event found in reprocess record, adding charge to event: ",
          reprocesschargelist->charge_event[idx2].charge_event_id))
       ENDIF
       SET chargeeventfound = true
       FOR (idx3 = 1 TO reprocesschargelist->charge_event[idx2].charge_qual)
         IF ((reprocesschargelist->charge_event[idx2].charge[idx3].charge_item_id=request->
         charge_mod[enscount].charge_item_id))
          SET chargeitemfound = true
          SET idx3 = reprocesschargelist->charge_event[idx2].charge_qual
          IF (validate(debug,- (1)) > 0)
           CALL echo("charge_item_id already exists in record, ignoring duplicate")
          ENDIF
         ENDIF
       ENDFOR
       IF (chargeitemfound=false)
        ...
        charge_qual].charge_item_id = request->charge_mod[enscount].charge_item_id
       ENDIF
    ...
    IF (chargeeventfound=false)
     ...
     SET reprocesschargelist->charge_event[reprocesschargelist->charge_event_qual].charge_qual += 1
     SET stat = alterlist(reprocesschargelist->charge_event[reprocesschargelist->charge_event_qual].
      charge,reprocesschargelist->charge_event[reprocesschargelist->charge_event_qual].charge_qual)
     ...
     charge_item_id = request->charge_mod[enscount].charge_item_id
     SET reprocesschargelist->charge_event[reprocesschargelist->charge_event_qual].charge_event_id =
     chargeeventid
    ENDIF
    UPDATE  FROM charge c
     ...
     WITH nocounter
    ;end update
    IF ( NOT (creditrelatedaddonsbeforereprocessing(request->charge_mod[enscount].charge_item_id,"",
     0.0)))
 ...
 IF ((request->charge_mod_qual > 0)
  AND (request->charge_mod[1].charge_item_id > 0.0))
  CALL savechargemodchangelogattributes(request)
 ENDIF
...
#check_error
 IF (failed=false)
  IF (validate(reply->charge_mod_qual,9999) != 9999)
   SET reply->charge_mod_qual = request->charge_mod_qual
  ENDIF
 ...
 SUBROUTINE (inactivateparentcodes(chargeitemid=f8,field6=vc,meaning=vc) =i2)
   ...
   DECLARE cemcnt = i4 WITH noconstant(0)
   SELECT INTO "nl:"
    FROM charge c,
     charge_event ce
    PLAN (c
     ...
     SELECT INTO "nl:"
      ...
      DETAIL
       IF (cem.charge_event_mod_id > 0
        ...
        CALL echo("in if"), parentchargeeventmods->cnt += 1, stat = alterlist(parentchargeeventmods->
         cemids,parentchargeeventmods->cnt),
        parentchargeeventmods->cemids[parentchargeeventmods->cnt].charge_event_mod_id = cem
        .charge_event_mod_id, parentchargeeventmods->cemids[parentchargeeventmods->cnt].field6 = cem
        .field6, parentchargeeventmods->cemids[parentchargeeventmods->cnt].charge_event_id = cem
        .charge_event_id,
        parentchargeeventmods->cemids[parentchargeeventmods->cnt].updt_cnt = cem.updt_cnt
       ENDIF
      WITH nocounter
     ;end select
     IF ( NOT (parentfound))
   ...
   FOR (cemcnt = 1 TO parentchargeeventmods->cnt)
     SET delchargeeventmodreq->objarray[1].action_type = "DEL"
     SET delchargeeventmodreq->objarray[1].charge_event_mod_id = parentchargeeventmods->cemids[cemcnt
     ].charge_event_mod_id
     SET delchargeeventmodreq->objarray[1].charge_event_id = parentchargeeventmods->cemids[cemcnt].
     charge_event_id
     SET delchargeeventmodreq->objarray[1].updt_cnt = parentchargeeventmods->cemids[cemcnt].updt_cnt
     EXECUTE afc_val_charge_event_mod  WITH replace("REQUEST",delchargeeventmodreq), replace("REPLY",
      delchargeeventmodrep)
     IF ((delchargeeventmodrep->status_data.status != "S"))
      SET failed = delete_error
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (writetocem(dummyvar=i2) =i2)
   CALL logmessage(curprog,"Entering writeToCem...",log_debug)
   DECLARE tempcharge_event_id = f8 WITH noconstant(0.0)
   DECLARE new_nbr = f8 WITH noconstant(0)
   DECLARE billcdcnt = i4 WITH protect, noconstant(0)
   ...
   IF ((request->charge_mod[enscount].charge_mod_type_cd=0))
    SET addchargeeventmodreq->objarray[billcdcnt].charge_event_mod_type_cd = cs13019_bill_code
   ELSE
    SET addchargeeventmodreq->objarray[billcdcnt].charge_event_mod_type_cd = request->charge_mod[
    enscount].charge_mod_type_cd
   ENDIF
   SET addchargeeventmodreq->objarray[billcdcnt].field1 = request->charge_mod[enscount].field1
   SET addchargeeventmodreq->objarray[billcdcnt].field2 = request->charge_mod[enscount].field2
   SET addchargeeventmodreq->objarray[billcdcnt].field3 = request->charge_mod[enscount].field3
   SET addchargeeventmodreq->objarray[billcdcnt].field4 = request->charge_mod[enscount].field4
   SET addchargeeventmodreq->objarray[billcdcnt].field5 = request->charge_mod[enscount].field5
   SET addchargeeventmodreq->objarray[billcdcnt].field6 = request->charge_mod[enscount].field6
   SET addchargeeventmodreq->objarray[billcdcnt].field7 = request->charge_mod[enscount].field7
   SET addchargeeventmodreq->objarray[billcdcnt].field8 = request->charge_mod[enscount].field8
   SET addchargeeventmodreq->objarray[billcdcnt].field9 = request->charge_mod[enscount].field9
   SET addchargeeventmodreq->objarray[billcdcnt].field10 = request->charge_mod[enscount].field10
   IF ((request->charge_mod[enscount].beg_effective_dt_tm <= 0))
   ...
   ELSE
    SET addchargeeventmodreq->objarray[billcdcnt].beg_effective_dt_tm = cnvtdatetime(request->
     charge_mod[enscount].beg_effective_dt_tm)
   ENDIF
   IF ((request->charge_mod[enscount].end_effective_dt_tm <= 0))
   ...
   ELSE
    SET addchargeeventmodreq->objarray[billcdcnt].end_effective_dt_tm = cnvtdatetime(request->
     charge_mod[enscount].end_effective_dt_tm)
   ENDIF
   IF ((request->charge_mod[enscount].active_ind_ind=false))
    SET addchargeeventmodreq->objarray[billcdcnt].active_ind = true
   ELSE
    SET addchargeeventmodreq->objarray[billcdcnt].active_ind = request->charge_mod[enscount].
    active_ind
   ENDIF
   IF ((request->charge_mod[enscount].active_status_cd=0))
    SET addchargeeventmodreq->objarray[billcdcnt].active_status_cd = active_code
   ELSE
    SET addchargeeventmodreq->objarray[billcdcnt].active_status_cd = request->charge_mod[enscount].
    active_status_cd
   ENDIF
   ...
   SET addchargeeventmodreq->objarray[billcdcnt].field1_id = request->charge_mod[enscount].field1_id
   SET addchargeeventmodreq->objarray[billcdcnt].field2_id = request->charge_mod[enscount].field2_id
   SET addchargeeventmodreq->objarray[billcdcnt].field3_id = request->charge_mod[enscount].field3_id
   SET addchargeeventmodreq->objarray[billcdcnt].field4_id = request->charge_mod[enscount].field4_id
   SET addchargeeventmodreq->objarray[billcdcnt].field5_id = request->charge_mod[enscount].field5_id
   SET addchargeeventmodreq->objarray[billcdcnt].nomen_id = request->charge_mod[enscount].nomen_id
   SET addchargeeventmodreq->objarray[billcdcnt].cm1_nbr = request->charge_mod[enscount].cm1_nbr
   SET addchargeeventmodreq->objarray[billcdcnt].code1_cd = validate(request->charge_mod[enscount].
    code1_cd,0.0)
   ...
   ELSE
    ...
    ENDIF
   ENDIF
   CALL logmessage(curprog,"Exiting writeToCem...",log_debug)
   RETURN(true)
 END ;Subroutine
#end_program
#exit_script
