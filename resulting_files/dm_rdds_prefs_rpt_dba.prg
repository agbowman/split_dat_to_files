CREATE PROGRAM dm_rdds_prefs_rpt:dba
 ...
 SUBROUTINE drmmi_backfill_mock_id(dbmi_env_id)
   ...
   RETURN(dbmi_mock_id)
 END ;Subroutine
 DECLARE accept_environment_id(null) = null
 DECLARE input_position_cd(null) = null
 DECLARE pos_reports(v_position=f8,v_source_position=f8) = null
 DECLARE pos_app_prefs(av_position=f8,av_source_position=f8) = null
 DECLARE pos_detail_prefs(dv_position=f8,dv_source_position=f8) = null
 DECLARE pos_view_comp_prefs(vcv_position=f8,vcv_source_position=f8) = null
 DECLARE pos_view_prefs(vv_position=f8,vv_source_position=f8) = null
 DECLARE display_report(rpt_rec=vc(ref)) = null
 DECLARE al_main_loop = i2
 DECLARE i_position = f8
 IF (check_logfile("dm_context_merge_adm",".log","DM_CONTEXT_MERGE_ADM")=0)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->emsg = "Starting dm_rdds_prefs_rpt"
 FREE RECORD al_env
 RECORD al_env(
   1 cr_env_id = f8
   1 cr_env_name = vc
   1 src_env_id = f8
   1 transl_trg_env_id = f8
   1 ref_seq = f8
   1 car_seq = f8
   1 rev_tran_ind = i2
 ) WITH protect
 SELECT INTO "nl:"
  ...
  DETAIL
   al_env->cr_env_id = b.environment_id, al_env->cr_env_name = b.environment_name
  WITH nocounter
 ;end select
 IF (curqual=0)
  ...
  GO TO exit_program
 ENDIF
 WHILE (al_main_loop=0)
   SET message = window
   CALL clear(1,1)
   SET width = 132
   CALL box(1,1,7,132)
   CALL text(3,46,"***  REPORT FOR PREF TABLES  ***")
   CALL text(4,30,"*** THIS PROGRAM SHOULD ONLY BE EXECUTED IN TARGET ENVIRONMENT ***")
   CALL text(6,75,"ENVIRONMENT ID:")
   CALL text(6,20,"ENVIRONMENT NAME:")
   CALL text(6,95,cnvtstring(al_env->cr_env_id))
   CALL text(6,40,al_env->cr_env_name)
   CALL text(9,3,"Please choose from the following options:")
   CALL text(11,3,"1 Input the source ENVIRONMENT_ID for the source environment")
   CALL text(12,3,"0 Exit this program")
   CALL accept(9,50,"99",0
    WHERE curaccept IN (1, 0))
   CASE (curaccept)
    OF 1:
     CALL accept_environment_id(null)
     IF ((al_env->src_env_id != 0))
      CALL input_position_cd(null)
     ENDIF
    OF 0:
     SET al_main_loop = 1
   ENDCASE
 ...
 CALL clear(1,1)
 SET width = 132
 SUBROUTINE accept_environment_id(null)
   DECLARE ae_insert_validate = i2 WITH protect, noconstant(0)
   DECLARE ae_insert_con = i2 WITH protect, noconstant(0)
   DECLARE temp_trans_id = f8
   DECLARE temp_info_domain = vc
   SET dm_err->eproc = "Obtaining a source_id"
   CALL disp_msg(" ",dm_err->logfile,0)
   CALL clear(1,1)
   SET width = 132
   CALL box(1,1,5,132)
   CALL text(3,45,"***  REPORT FOR PREF TABLES  ***")
   CALL text(4,44,"***  INSERT SOURCE ENVIRONMENT  ***")
   CALL text(7,3,"Please input source environment id: ")
   WHILE (ae_insert_validate=0)
     CALL clear(20,1)
     ...
     SELECT INTO "nl:"
      ...
      (SELECT
       ...
       WHERE (der.child_env_id=al_env->cr_env_id)
     ...
     ELSE
      SELECT INTO "nl:"
       ...
       (SELECT
        ...
        WHERE (der.child_env_id=al_env->cr_env_id)
         AND der.relationship_type="REFERENCE MERGE"))
        AND (de.environment_id != al_env->cr_env_id)
   ...
   CALL clear(20,1)
   IF (ae_insert_con=0)
    SET al_env->src_env_id = cnvtreal(curaccept)
   ENDIF
   SET al_env->transl_trg_env_id = drmmi_get_mock_id(al_env->cr_env_id)
   IF ((al_env->transl_trg_env_id < 0.0))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SET temp_info_domain = concat("MERGE",trim(cnvtstring(al_env->src_env_id)),trim(cnvtstring(al_env
      ->transl_trg_env_id)),"SEQMATCH")
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain=temp_info_domain
     AND di.info_name="REFERENCE_SEQ"
    DETAIL
     al_env->ref_seq = di.info_number
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg("Obtaining REFERENCE_SEQ",dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain=temp_info_domain
     AND di.info_name="CARENET_SEQ"
    DETAIL
     al_env->car_seq = di.info_number
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg("Obtaining CARENET_SEQ",dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SELECT INTO "nl"
    FROM dm_env_reltn der
    WHERE (der.parent_env_id=al_env->src_env_id)
     AND (der.child_env_id=al_env->cr_env_id)
     AND der.relationship_type="RDDS MOVER CHANGES NOT LOGGED"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg("Checking for reverse translations",dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (curqual != 0)
    SET al_env->rev_tran_ind = 1
   ENDIF
   SET help = off
 END ;Subroutine
 SUBROUTINE input_position_cd(null)
   DECLARE al_pos_loop = i2
   DECLARE i_position = f8
   DECLARE i_source_position = f8
   DECLARE continue_flag = i2
   DECLARE dm_merge = vc
   SET dm_merge = concat("dm_merge_translate@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET dm_err->eproc = "Obtaining a position_cd"
   CALL disp_msg(" ",dm_err->logfile,0)
   WHILE (al_pos_loop=0)
     SET message = window
     SET continue_flag = 0
     ...
     CALL box(1,1,7,132)
     CALL text(3,46,"***  REPORT FOR PREF TABLES  ***")
     CALL text(4,35,"*** INPUT POSITION CD FROM TARGET TO REPORT ON ***")
     CALL text(6,75,"ENVIRONMENT ID:")
     CALL text(6,20,"ENVIRONMENT NAME:")
     CALL text(6,95,cnvtstring(al_env->cr_env_id))
     CALL text(6,40,al_env->cr_env_name)
     CALL text(9,3,"Do you want to input a position_cd ? (Y/N-Exit) ")
     CALL accept(9,55,"P;CU","Y"
      WHERE curaccept IN ("Y", "N"))
     CASE (curaccept)
      OF "Y":
       CALL text(11,3,"Please enter a position_cd: ")
       CALL accept(11,35,"N(15);CU","0"
        WHERE cnvtreal(trim(curaccept)) >= 0)
       SELECT INTO "nl:"
        FROM code_value cv
        WHERE cv.code_set=88
         AND cv.code_value=cnvtreal(curaccept)
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg("Validating position_cd",dm_err->logfile,1)
        GO TO exit_program
       ENDIF
       IF (curqual != 0)
        SET i_position = cnvtreal(curaccept)
        IF ((i_position <= al_env->ref_seq))
         SET i_source_position = i_position
         SET continue_flag = 1
        ELSE
         SELECT INTO "nl:"
          FROM dm_merge_translate dmt
          WHERE dmt.to_value=i_position
           AND (dmt.env_source_id=al_env->src_env_id)
           AND (dmt.env_target_id=al_env->transl_trg_env_id)
           AND table_name="CODE_VALUE"
          DETAIL
           i_source_position = dmt.from_value
          WITH nocounter
         ;end select
         IF (check_error(dm_err->eproc)=1)
          CALL disp_msg("Obtaining source position_cd",dm_err->logfile,1)
          GO TO exit_program
         ENDIF
         IF (curqual=0)
          IF ((al_env->rev_tran_ind=1))
           SELECT INTO "nl:"
            FROM (parser(dm_merge) dmt)
            WHERE dmt.from_value=i_position
             AND (dmt.env_source_id=al_env->transl_trg_env_id)
             AND (dmt.env_target_id=al_env->src_env_id)
             AND table_name="CODE_VALUE"
            DETAIL
             i_source_position = dmt.to_value
            WITH nocounter
           ;end select
           IF (check_error(dm_err->eproc)=1)
            CALL disp_msg("Obtaining source position_cd",dm_err->logfile,1)
           ...
           IF (curqual != 0)
            SET continue_flag = 1
           ELSE
            CALL text(22,3,"The position_cd does not exist in source, please try another position_cd"
             )
            CALL pause(3)
           ENDIF
          ELSE
        ...
        ENDIF
        IF (continue_flag=1)
         CALL pos_reports(i_position,i_source_position)
        ENDIF
       ELSE
        CALL text(22,3,"Invalid position code")
        CALL pause(3)
       ENDIF
      OF "N":
       SET al_pos_loop = 1
   ...
   SET width = 132
 END ;Subroutine
 SUBROUTINE pos_reports(v_position,v_source_position)
   SET dm_err->eproc = "Choose a report with a given position_cd"
   CALL disp_msg(" ",dm_err->logfile,0)
   ...
   WHILE (s_sub1_cnt=0)
     CALL clear(1,1)
     CALL box(1,1,7,132)
     CALL text(3,50,"REPORTS FOR GIVEN POSITION_CD")
     CALL text(5,75,"ENVIRONMENT ID:")
     CALL text(5,20,"ENVIRONMENT NAME:")
     CALL text(5,95,cnvtstring(al_env->cr_env_id))
     CALL text(5,40,al_env->cr_env_name)
     CALL text(9,3,"Please choose from the following reports:")
     CALL text(11,3,"1 APP_PREFS")
     CALL text(12,3,"2 DETAIL_PREFS")
     CALL text(13,3,"3 VIEW_COMP_PREFS")
     CALL text(14,3,"4 VIEW_PREFS")
     CALL text(15,3,"0 Exit")
     CALL accept(9,60,"99",0
      WHERE curaccept IN (1, 2, 3, 4, 0))
     CASE (curaccept)
      OF 1:
       CALL pos_app_prefs(v_position,v_source_position)
      OF 2:
       CALL pos_detail_prefs(v_position,v_source_position)
      OF 3:
       CALL pos_view_comp_prefs(v_position,v_source_position)
      OF 4:
       CALL pos_view_prefs(v_position,v_source_position)
      OF 0:
       CALL clear(1,1)
       SET s_sub1_cnt = 1
     ENDCASE
   ENDWHILE
 END ;Subroutine
 SUBROUTINE pos_app_prefs(av_position,av_source_position)
   FREE RECORD ap_pos
   RECORD ap_pos(
     1 cr_pos_cd = f8
     1 src_pos_cd = f8
   ...
   DECLARE app_merge = vc
   DECLARE nvp_merge = vc
   DECLARE dm_merge = vc
   DECLARE idx = i4
   DECLARE idx2 = i4
   DECLARE cidx = i4
   SET app_merge = concat("app_prefs@merge",trim(cnvtstring(al_env->src_env_id)),trim(cnvtstring(
      al_env->cr_env_id)))
   SET nvp_merge = concat("name_value_prefs@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET dm_merge = concat("dm_merge_translate@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET ap_pos->cr_pos_cd = av_position
   SET ap_pos->src_pos_cd = av_source_position
   SELECT INTO "nl:"
    FROM (parser(app_merge) a)
    WHERE (a.position_cd=ap_pos->src_pos_cd)
    HEAD REPORT
     ap_pos->pi_cnt = 0
    DETAIL
     ap_pos->pi_cnt = (ap_pos->pi_cnt+ 1)
     IF (mod(ap_pos->pi_cnt,100)=1)
      stat = alterlist(ap_pos->parent_id,(ap_pos->pi_cnt+ 99))
     ENDIF
     ap_pos->parent_id[ap_pos->pi_cnt].src_pr_id = a.app_prefs_id
    FOOT REPORT
     stat = alterlist(ap_pos->parent_id,ap_pos->pi_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg("Obtaining source pref_ids",dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   FOR (loop_cnt = 1 TO ap_pos->pi_cnt)
     IF ((ap_pos->parent_id[loop_cnt].src_pr_id <= al_env->car_seq))
      SET ap_pos->parent_id[loop_cnt].cr_pr_id = ap_pos->parent_id[loop_cnt].src_pr_id
     ELSE
      SET ap_pos->parent_id[loop_cnt].cr_pr_id = - (1)
      SET ap_pos->parent_id[loop_cnt].type = 4
     ENDIF
   ENDFOR
   SET idx = 0
   SET idx2 = 0
   ...
   SELECT INTO "nl:"
    FROM dm_merge_translate dmt
    WHERE (dmt.env_source_id=al_env->src_env_id)
     AND (dmt.env_target_id=al_env->transl_trg_env_id)
     AND dmt.table_name="APP_PREFS"
     AND expand(idx,1,size(ap_pos->parent_id,5),dmt.from_value,ap_pos->parent_id[idx].src_pr_id)
    DETAIL
     cidx = locateval(idx2,1,size(ap_pos->parent_id,5),dmt.from_value,ap_pos->parent_id[idx2].
      src_pr_id)
     IF ((ap_pos->parent_id[cidx].src_pr_id > al_env->car_seq))
      ap_pos->parent_id[cidx].cr_pr_id = dmt.to_value, ap_pos->parent_id[cidx].type = 0
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg("Obtaining target pref_ids",dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF ((al_env->rev_tran_ind=1))
    SET idx = 0
    ...
    SET cidx = 0
    SELECT INTO "nl:"
     FROM (parser(dm_merge) dm)
     WHERE (dm.env_source_id=al_env->transl_trg_env_id)
      AND (dm.env_target_id=al_env->src_env_id)
      AND dm.table_name="APP_PREFS"
      AND expand(idx,1,size(ap_pos->parent_id,5),dm.to_value,ap_pos->parent_id[idx].src_pr_id)
     DETAIL
      cidx = locateval(idx2,1,size(ap_pos->parent_id,5),dm.to_value,ap_pos->parent_id[idx2].src_pr_id
       )
      IF ((ap_pos->parent_id[cidx].src_pr_id > al_env->car_seq)
       AND (ap_pos->parent_id[cidx].cr_pr_id=- (1)))
       ap_pos->parent_id[cidx].cr_pr_id = dm.from_value, ap_pos->parent_id[cidx].type = 0
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
   ENDIF
   FOR (loop_cnt1 = 1 TO ap_pos->pi_cnt)
     IF ((ap_pos->parent_id[loop_cnt1].cr_pr_id=- (1)))
      SET ap_pos->parent_id[loop_cnt1].cr_nvp_cnt = - (99)
      SET ap_pos->parent_id[loop_cnt1].cr_nvp_cnt_r = - (99)
      SET ap_pos->parent_id[loop_cnt1].src_nvp_cnt = - (99)
     ELSE
      SELECT INTO "nl:"
       cnt = count(*)
       FROM (parser(nvp_merge) nm)
       WHERE nm.parent_entity_name="APP_PREFS"
        AND (nm.parent_entity_id=ap_pos->parent_id[loop_cnt1].src_pr_id)
       DETAIL
        ap_pos->parent_id[loop_cnt1].src_nvp_cnt = cnt
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg("Obtaining source nvp row count",dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      SELECT INTO "nl:"
       cnt = count(*)
       ...
       WHERE nvp.parent_entity_name="APP_PREFS"
        AND (nvp.parent_entity_id=ap_pos->parent_id[loop_cnt1].cr_pr_id)
       DETAIL
        ap_pos->parent_id[loop_cnt1].cr_nvp_cnt = cnt
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg("Obtaining target nvp row count",dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      SELECT DISTINCT INTO "nl:"
       nvpr.name_value_prefs_id
       FROM name_value_pre1173$r nvpr
       WHERE nvpr.parent_entity_name="APP_PREFS"
        AND (nvpr.parent_entity_id=ap_pos->parent_id[loop_cnt1].cr_pr_id)
        AND nvpr.rdds_delete_ind=0
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg("Obtaining target $R nvp row count",dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      SET ap_pos->parent_id[loop_cnt1].cr_nvp_cnt_r = curqual
     ENDIF
   ENDFOR
   FOR (loop_cnt2 = 1 TO ap_pos->pi_cnt)
     IF ((ap_pos->parent_id[loop_cnt2].cr_pr_id != - (1)))
      IF ((ap_pos->parent_id[loop_cnt2].src_nvp_cnt=ap_pos->parent_id[loop_cnt2].cr_nvp_cnt_r))
       IF ((ap_pos->parent_id[loop_cnt2].cr_nvp_cnt_r != ap_pos->parent_id[loop_cnt2].cr_nvp_cnt))
        SET ap_pos->parent_id[loop_cnt2].type = 1
       ENDIF
      ELSE
       IF ((ap_pos->parent_id[loop_cnt2].cr_nvp_cnt_r=ap_pos->parent_id[loop_cnt2].cr_nvp_cnt))
        SET ap_pos->parent_id[loop_cnt2].type = 3
       ELSE
        SET ap_pos->parent_id[loop_cnt2].type = 2
       ENDIF
      ENDIF
     ENDIF
   ENDFOR
   CALL display_report(ap_pos)
 END ;Subroutine
 SUBROUTINE pos_detail_prefs(dv_position,dv_source_position)
   FREE RECORD dp_pos
   RECORD dp_pos(
     1 cr_pos_cd = f8
     1 src_pos_cd = f8
   ...
   DECLARE detail_merge = vc
   DECLARE nvp_merge = vc
   DECLARE dm_merge = vc
   DECLARE idx = i4
   DECLARE idx2 = i4
   DECLARE cidx = i4
   SET detail_merge = concat("detail_prefs@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET nvp_merge = concat("name_value_prefs@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET dm_merge = concat("dm_merge_translate@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET dp_pos->cr_pos_cd = dv_position
   SET dp_pos->src_pos_cd = dv_source_position
   SELECT INTO "nl:"
    FROM (parser(detail_merge) a)
    WHERE (a.position_cd=dp_pos->src_pos_cd)
    HEAD REPORT
     dp_pos->pi_cnt = 0
    DETAIL
     dp_pos->pi_cnt = (dp_pos->pi_cnt+ 1)
     IF (mod(dp_pos->pi_cnt,100)=1)
      stat = alterlist(dp_pos->parent_id,(dp_pos->pi_cnt+ 99))
     ENDIF
     dp_pos->parent_id[dp_pos->pi_cnt].src_pr_id = a.detail_prefs_id
    FOOT REPORT
     stat = alterlist(dp_pos->parent_id,dp_pos->pi_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg("Obtaining source pref_ids",dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   FOR (loop_cnt = 1 TO dp_pos->pi_cnt)
     IF ((dp_pos->parent_id[loop_cnt].src_pr_id <= al_env->car_seq))
      SET dp_pos->parent_id[loop_cnt].cr_pr_id = dp_pos->parent_id[loop_cnt].src_pr_id
     ELSE
      SET dp_pos->parent_id[loop_cnt].cr_pr_id = - (1)
      SET dp_pos->parent_id[loop_cnt].type = 4
     ENDIF
   ENDFOR
   SET idx = 0
   SET idx2 = 0
   ...
   SELECT INTO "nl:"
    ...
    WHERE (dmt.env_source_id=al_env->src_env_id)
     AND (dmt.env_target_id=al_env->transl_trg_env_id)
     AND dmt.table_name="DETAIL_PREFS"
     AND expand(idx,1,size(dp_pos->parent_id,5),dmt.from_value,dp_pos->parent_id[idx].src_pr_id)
    DETAIL
     cidx = locateval(idx2,1,size(dp_pos->parent_id,5),dmt.from_value,dp_pos->parent_id[idx2].
      src_pr_id)
     IF ((dp_pos->parent_id[cidx].src_pr_id > al_env->car_seq))
      dp_pos->parent_id[cidx].cr_pr_id = dmt.to_value, dp_pos->parent_id[cidx].type = 0
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   IF ((al_env->rev_tran_ind=1))
    ...
    SELECT INTO "nl:"
     ...
     WHERE (dm.env_source_id=al_env->transl_trg_env_id)
      AND (dm.env_target_id=al_env->src_env_id)
      AND dm.table_name="DETAIL_PREFS"
      AND expand(idx,1,size(dp_pos->parent_id,5),dm.to_value,dp_pos->parent_id[idx].src_pr_id)
     DETAIL
      cidx = locateval(idx2,1,size(dp_pos->parent_id,5),dm.to_value,dp_pos->parent_id[idx2].src_pr_id
       )
      IF ((dp_pos->parent_id[cidx].src_pr_id > al_env->car_seq)
       AND (dp_pos->parent_id[cidx].cr_pr_id=- (1)))
       dp_pos->parent_id[cidx].cr_pr_id = dm.from_value, dp_pos->parent_id[cidx].type = 0
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
   ENDIF
   FOR (loop_cnt1 = 1 TO dp_pos->pi_cnt)
     IF ((dp_pos->parent_id[loop_cnt1].cr_pr_id=- (1)))
      SET dp_pos->parent_id[loop_cnt1].cr_nvp_cnt = - (99)
      SET dp_pos->parent_id[loop_cnt1].cr_nvp_cnt_r = - (99)
      SET dp_pos->parent_id[loop_cnt1].src_nvp_cnt = - (99)
     ELSE
      SELECT INTO "nl:"
       cnt = count(*)
       FROM (parser(nvp_merge) nm)
       WHERE nm.parent_entity_name="DETAIL_PREFS"
        AND (nm.parent_entity_id=dp_pos->parent_id[loop_cnt1].src_pr_id)
       DETAIL
        dp_pos->parent_id[loop_cnt1].src_nvp_cnt = cnt
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg("Obtaining source nvp row count",dm_err->logfile,1)
      ...
      SELECT INTO "nl:"
       ...
       WHERE nvp.parent_entity_name="DETAIL_PREFS"
        AND (nvp.parent_entity_id=dp_pos->parent_id[loop_cnt1].cr_pr_id)
       DETAIL
        dp_pos->parent_id[loop_cnt1].cr_nvp_cnt = cnt
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg("Obtaining target nvp row count",dm_err->logfile,1)
      ...
      SELECT DISTINCT INTO "nl:"
       nvpr.name_value_prefs_id
       FROM name_value_pre1173$r nvpr
       WHERE nvpr.parent_entity_name="DETAIL_PREFS"
        AND (nvpr.parent_entity_id=dp_pos->parent_id[loop_cnt1].cr_pr_id)
        AND nvpr.rdds_delete_ind=0
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       ...
       GO TO exit_program
      ENDIF
      SET dp_pos->parent_id[loop_cnt1].cr_nvp_cnt_r = curqual
     ENDIF
   ENDFOR
   FOR (loop_cnt2 = 1 TO dp_pos->pi_cnt)
     IF ((dp_pos->parent_id[loop_cnt2].cr_pr_id != - (1)))
      IF ((dp_pos->parent_id[loop_cnt2].src_nvp_cnt=dp_pos->parent_id[loop_cnt2].cr_nvp_cnt_r))
       IF ((dp_pos->parent_id[loop_cnt2].cr_nvp_cnt_r != dp_pos->parent_id[loop_cnt2].cr_nvp_cnt))
        SET dp_pos->parent_id[loop_cnt2].type = 1
       ENDIF
      ELSE
       IF ((dp_pos->parent_id[loop_cnt2].cr_nvp_cnt_r=dp_pos->parent_id[loop_cnt2].cr_nvp_cnt))
        SET dp_pos->parent_id[loop_cnt2].type = 3
       ELSE
        SET dp_pos->parent_id[loop_cnt2].type = 2
       ENDIF
      ENDIF
     ENDIF
   ENDFOR
   CALL display_report(dp_pos)
 END ;Subroutine
 SUBROUTINE pos_view_comp_prefs(vcv_position,vcv_source_position)
   FREE RECORD vcp_pos
   RECORD vcp_pos(
     1 cr_pos_cd = f8
     1 src_pos_cd = f8
   ...
   DECLARE view_comp_merge = vc
   DECLARE nvp_merge = vc
   DECLARE dm_merge = vc
   DECLARE idx = i4
   DECLARE idx2 = i4
   DECLARE cidx = i4
   SET view_comp_merge = concat("view_comp_prefs@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET nvp_merge = concat("name_value_prefs@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET dm_merge = concat("dm_merge_translate@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET vcp_pos->cr_pos_cd = vcv_position
   SET vcp_pos->src_pos_cd = vcv_source_position
   SELECT INTO "nl:"
    FROM (parser(view_comp_merge) a)
    WHERE (a.position_cd=vcp_pos->src_pos_cd)
    HEAD REPORT
     vcp_pos->pi_cnt = 0
    DETAIL
     vcp_pos->pi_cnt = (vcp_pos->pi_cnt+ 1)
     IF (mod(vcp_pos->pi_cnt,100)=1)
      stat = alterlist(vcp_pos->parent_id,(vcp_pos->pi_cnt+ 99))
     ENDIF
     vcp_pos->parent_id[vcp_pos->pi_cnt].src_pr_id = a.view_comp_prefs_id
    FOOT REPORT
     stat = alterlist(vcp_pos->parent_id,vcp_pos->pi_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg("Obtaining source pref_ids",dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   FOR (loop_cnt = 1 TO vcp_pos->pi_cnt)
     IF ((vcp_pos->parent_id[loop_cnt].src_pr_id <= al_env->car_seq))
      SET vcp_pos->parent_id[loop_cnt].cr_pr_id = vcp_pos->parent_id[loop_cnt].src_pr_id
     ELSE
      SET vcp_pos->parent_id[loop_cnt].cr_pr_id = - (1)
      SET vcp_pos->parent_id[loop_cnt].type = 4
     ENDIF
   ENDFOR
   SET idx = 0
   SET idx2 = 0
   ...
   SELECT INTO "nl:"
    ...
    WHERE (dmt.env_source_id=al_env->src_env_id)
     AND (dmt.env_target_id=al_env->transl_trg_env_id)
     AND dmt.table_name="VIEW_COMP_PREFS"
     AND expand(idx,1,size(vcp_pos->parent_id,5),dmt.from_value,vcp_pos->parent_id[idx].src_pr_id)
    DETAIL
     cidx = locateval(idx2,1,size(vcp_pos->parent_id,5),dmt.from_value,vcp_pos->parent_id[idx2].
      src_pr_id)
     IF ((vcp_pos->parent_id[cidx].src_pr_id > al_env->car_seq))
      vcp_pos->parent_id[cidx].cr_pr_id = dmt.to_value, vcp_pos->parent_id[cidx].type = 0
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   IF ((al_env->rev_tran_ind=1))
    ...
    SELECT INTO "nl:"
     ...
     WHERE (dm.env_source_id=al_env->transl_trg_env_id)
      AND (dm.env_target_id=al_env->src_env_id)
      AND dm.table_name="VIEW_COMP_PREFS"
      AND expand(idx,1,size(vcp_pos->parent_id,5),dm.to_value,vcp_pos->parent_id[idx].src_pr_id)
     DETAIL
      cidx = locateval(idx2,1,size(vcp_pos->parent_id,5),dm.to_value,vcp_pos->parent_id[idx2].
       src_pr_id)
      IF ((vcp_pos->parent_id[cidx].src_pr_id > al_env->car_seq)
       AND (vcp_pos->parent_id[cidx].cr_pr_id=- (1)))
       vcp_pos->parent_id[cidx].cr_pr_id = dm.from_value, vcp_pos->parent_id[cidx].type = 0
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
   ENDIF
   FOR (loop_cnt1 = 1 TO vcp_pos->pi_cnt)
     IF ((vcp_pos->parent_id[loop_cnt1].cr_pr_id=- (1)))
      SET vcp_pos->parent_id[loop_cnt1].cr_nvp_cnt = - (99)
      SET vcp_pos->parent_id[loop_cnt1].cr_nvp_cnt_r = - (99)
      SET vcp_pos->parent_id[loop_cnt1].src_nvp_cnt = - (99)
     ELSE
      SELECT INTO "nl:"
       cnt = count(*)
       FROM (parser(nvp_merge) nm)
       WHERE nm.parent_entity_name="VIEW_COMP_PREFS"
        AND (nm.parent_entity_id=vcp_pos->parent_id[loop_cnt1].src_pr_id)
       DETAIL
        vcp_pos->parent_id[loop_cnt1].src_nvp_cnt = cnt
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg("Obtaining source nvp row count",dm_err->logfile,1)
      ...
      SELECT INTO "nl:"
       ...
       WHERE nvp.parent_entity_name="VIEW_COMP_PREFS"
        AND (nvp.parent_entity_id=vcp_pos->parent_id[loop_cnt1].cr_pr_id)
       DETAIL
        vcp_pos->parent_id[loop_cnt1].cr_nvp_cnt = cnt
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg("Obtaining target nvp row count",dm_err->logfile,1)
      ...
      SELECT DISTINCT INTO "nl:"
       nvpr.name_value_prefs_id
       FROM name_value_pre1173$r nvpr
       WHERE nvpr.parent_entity_name="VIEW_COMP_PREFS"
        AND (nvpr.parent_entity_id=vcp_pos->parent_id[loop_cnt1].cr_pr_id)
        AND nvpr.rdds_delete_ind=0
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       ...
       GO TO exit_program
      ENDIF
      SET vcp_pos->parent_id[loop_cnt1].cr_nvp_cnt_r = curqual
     ENDIF
   ENDFOR
   FOR (loop_cnt2 = 1 TO vcp_pos->pi_cnt)
     IF ((vcp_pos->parent_id[loop_cnt2].cr_pr_id != - (1)))
      IF ((vcp_pos->parent_id[loop_cnt2].src_nvp_cnt=vcp_pos->parent_id[loop_cnt2].cr_nvp_cnt_r))
       IF ((vcp_pos->parent_id[loop_cnt2].cr_nvp_cnt_r != vcp_pos->parent_id[loop_cnt2].cr_nvp_cnt))
        SET vcp_pos->parent_id[loop_cnt2].type = 1
       ENDIF
      ELSE
       IF ((vcp_pos->parent_id[loop_cnt2].cr_nvp_cnt_r=vcp_pos->parent_id[loop_cnt2].cr_nvp_cnt))
        SET vcp_pos->parent_id[loop_cnt2].type = 3
       ELSE
        SET vcp_pos->parent_id[loop_cnt2].type = 2
       ENDIF
      ENDIF
     ENDIF
   ENDFOR
   CALL display_report(vcp_pos)
 END ;Subroutine
 SUBROUTINE pos_view_prefs(vv_position,vv_source_position)
   FREE RECORD vp_pos
   RECORD vp_pos(
     1 cr_pos_cd = f8
     1 src_pos_cd = f8
   ...
   DECLARE view_merge = vc
   DECLARE nvp_merge = vc
   DECLARE dm_merge = vc
   DECLARE idx = i4
   DECLARE idx2 = i4
   DECLARE cidx = i4
   SET view_merge = concat("view_prefs@merge",trim(cnvtstring(al_env->src_env_id)),trim(cnvtstring(
      al_env->cr_env_id)))
   SET nvp_merge = concat("name_value_prefs@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET dm_merge = concat("dm_merge_translate@merge",trim(cnvtstring(al_env->src_env_id)),trim(
     cnvtstring(al_env->cr_env_id)))
   SET vp_pos->cr_pos_cd = vv_position
   SET vp_pos->src_pos_cd = vv_source_position
   SELECT INTO "nl:"
    FROM (parser(view_merge) a)
    WHERE (a.position_cd=vp_pos->src_pos_cd)
    HEAD REPORT
     vp_pos->pi_cnt = 0
    DETAIL
     vp_pos->pi_cnt = (vp_pos->pi_cnt+ 1)
     IF (mod(vp_pos->pi_cnt,100)=1)
      stat = alterlist(vp_pos->parent_id,(vp_pos->pi_cnt+ 99))
     ENDIF
     vp_pos->parent_id[vp_pos->pi_cnt].src_pr_id = a.view_prefs_id
    FOOT REPORT
     stat = alterlist(vp_pos->parent_id,vp_pos->pi_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg("Obtaining source pref_ids",dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   FOR (loop_cnt = 1 TO vp_pos->pi_cnt)
     IF ((vp_pos->parent_id[loop_cnt].src_pr_id <= al_env->car_seq))
      SET vp_pos->parent_id[loop_cnt].cr_pr_id = vp_pos->parent_id[loop_cnt].src_pr_id
     ELSE
      SET vp_pos->parent_id[loop_cnt].cr_pr_id = - (1)
      SET vp_pos->parent_id[loop_cnt].type = 4
     ENDIF
   ENDFOR
   SET idx = 0
   SET idx2 = 0
   ...
   SELECT INTO "nl:"
    ...
    WHERE (dmt.env_source_id=al_env->src_env_id)
     AND (dmt.env_target_id=al_env->transl_trg_env_id)
     AND dmt.table_name="VIEW_PREFS"
     AND expand(idx,1,size(vp_pos->parent_id,5),dmt.from_value,vp_pos->parent_id[idx].src_pr_id)
    DETAIL
     cidx = locateval(idx2,1,size(vp_pos->parent_id,5),dmt.from_value,vp_pos->parent_id[idx2].
      src_pr_id)
     IF ((vp_pos->parent_id[cidx].src_pr_id > al_env->car_seq))
      vp_pos->parent_id[cidx].cr_pr_id = dmt.to_value, vp_pos->parent_id[cidx].type = 0
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   IF ((al_env->rev_tran_ind=1))
    ...
    SELECT INTO "nl:"
     ...
     WHERE (dm.env_source_id=al_env->transl_trg_env_id)
      AND (dm.env_target_id=al_env->src_env_id)
      AND dm.table_name="VIEW_PREFS"
      AND expand(idx,1,size(vp_pos->parent_id,5),dm.to_value,vp_pos->parent_id[idx].src_pr_id)
     DETAIL
      cidx = locateval(idx2,1,size(vp_pos->parent_id,5),dm.to_value,vp_pos->parent_id[idx2].src_pr_id
       )
      IF ((vp_pos->parent_id[cidx].src_pr_id > al_env->car_seq)
       AND (vp_pos->parent_id[cidx].cr_pr_id=- (1)))
       vp_pos->parent_id[cidx].cr_pr_id = dm.from_value, vp_pos->parent_id[cidx].type = 0
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
   ENDIF
   FOR (loop_cnt1 = 1 TO vp_pos->pi_cnt)
     IF ((vp_pos->parent_id[loop_cnt1].cr_pr_id=- (1)))
      SET vp_pos->parent_id[loop_cnt1].cr_nvp_cnt = - (99)
      SET vp_pos->parent_id[loop_cnt1].cr_nvp_cnt_r = - (99)
      SET vp_pos->parent_id[loop_cnt1].src_nvp_cnt = - (99)
     ELSE
      SELECT INTO "nl:"
       cnt = count(*)
       FROM (parser(nvp_merge) nm)
       WHERE nm.parent_entity_name="VIEW_PREFS"
        AND (nm.parent_entity_id=vp_pos->parent_id[loop_cnt1].src_pr_id)
       DETAIL
        vp_pos->parent_id[loop_cnt1].src_nvp_cnt = cnt
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg("Obtaining source nvp row count",dm_err->logfile,1)
      ...
      SELECT INTO "nl:"
       ...
       WHERE nvp.parent_entity_name="VIEW_PREFS"
        AND (nvp.parent_entity_id=vp_pos->parent_id[loop_cnt1].cr_pr_id)
       DETAIL
        vp_pos->parent_id[loop_cnt1].cr_nvp_cnt = cnt
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg("Obtaining target nvp row count",dm_err->logfile,1)
      ...
      SELECT DISTINCT INTO "nl:"
       nvpr.name_value_prefs_id
       FROM name_value_pre1173$r nvpr
       WHERE nvpr.parent_entity_name="VIEW_PREFS"
        AND (nvpr.parent_entity_id=vp_pos->parent_id[loop_cnt1].cr_pr_id)
        AND nvpr.rdds_delete_ind=0
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       ...
       GO TO exit_program
      ENDIF
      SET vp_pos->parent_id[loop_cnt1].cr_nvp_cnt_r = curqual
     ENDIF
   ENDFOR
   FOR (loop_cnt2 = 1 TO vp_pos->pi_cnt)
     IF ((vp_pos->parent_id[loop_cnt2].cr_pr_id != - (1)))
      IF ((vp_pos->parent_id[loop_cnt2].src_nvp_cnt=vp_pos->parent_id[loop_cnt2].cr_nvp_cnt_r))
       IF ((vp_pos->parent_id[loop_cnt2].cr_nvp_cnt_r != vp_pos->parent_id[loop_cnt2].cr_nvp_cnt))
        SET vp_pos->parent_id[loop_cnt2].type = 1
       ENDIF
      ELSE
       IF ((vp_pos->parent_id[loop_cnt2].cr_nvp_cnt_r=vp_pos->parent_id[loop_cnt2].cr_nvp_cnt))
        SET vp_pos->parent_id[loop_cnt2].type = 3
       ELSE
        SET vp_pos->parent_id[loop_cnt2].type = 2
       ENDIF
      ENDIF
     ENDIF
   ENDFOR
   CALL display_report(vp_pos)
 END ;Subroutine
 SUBROUTINE display_report(rpt_rec)
   SET message = window
   SET help = off
   SET dm_err->eproc = "Choose a report type"
   CALL disp_msg(" ",dm_err->logfile,0)
   DECLARE dr_cnt = i4
   DECLARE rtp_loop = i4
   SET dr_cnt = 0
   WHILE (dr_cnt=0)
     CALL clear(1,1)
     CALL box(1,1,7,132)
     CALL text(3,50,"REPORT TYPES")
     CALL text(5,75,"ENVIRONMENT ID:")
     CALL text(5,20,"ENVIRONMENT NAME:")
     CALL text(5,95,cnvtstring(al_env->cr_env_id))
     CALL text(5,40,al_env->cr_env_name)
     CALL text(9,3,"Please choose from the following reports:")
     CALL text(11,3,"1 No Translations")
     CALL text(12,3,"2 Problems during cutover only")
     CALL text(13,3,"3 Problems during merge only (possible rows did not merge at all)")
     CALL text(14,3,"4 Problems in both merge and cutover")
     CALL text(15,3,"5 Display everything")
     ...
     CASE (curaccept)
      OF 1:
       ...
       SELECT
        *
        FROM (dummyt d  WITH seq = value(rpt_rec->pi_cnt))
        HEAD REPORT
         rpt_loop = 0, col 0, "SRC_POSITION_CD",
         col 18, "TRG_POSITION_CD", col 36,
         "SOURCE_PARENT_ID", col 54, "TARGET_PARENT_ID",
         col 72, "SOURCE_NVP_CNT", col 90,
         "TARGET_NVP_CNT", col 108, "TARGET_NPV$R_CNT"
        DETAIL
         rpt_loop = (rpt_loop+ 1)
         IF ((rpt_rec->parent_id[rpt_loop].type=4))
          row + 1, col 0, rpt_rec->src_pos_cd,
          col 18, rpt_rec->cr_pos_cd, col 36,
          rpt_rec->parent_id[rpt_loop].src_pr_id, col 54, rpt_rec->parent_id[rpt_loop].cr_pr_id,
          col 72, rpt_rec->parent_id[rpt_loop].src_nvp_cnt, col 90,
          rpt_rec->parent_id[rpt_loop].cr_nvp_cnt, col 108, rpt_rec->parent_id[rpt_loop].cr_nvp_cnt_r
         ENDIF
        WITH nocounter, formfeed = none
       ;end select
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg("Display : No Translations",dm_err->logfile,1)
        GO TO exit_program
       ENDIF
      OF 2:
       SET message = nowindow
        ...
        *
        ...
        DETAIL
         rpt_loop = (rpt_loop+ 1)
         IF ((rpt_rec->parent_id[rpt_loop].type=1))
          row + 1, col 0, rpt_rec->src_pos_cd,
          col 18, rpt_rec->cr_pos_cd, col 36,
          rpt_rec->parent_id[rpt_loop].src_pr_id, col 54, rpt_rec->parent_id[rpt_loop].cr_pr_id,
          col 72, rpt_rec->parent_id[rpt_loop].src_nvp_cnt, col 90,
       ...
       ;end select
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg("Display : Problems during cutover",dm_err->logfile,1)
        GO TO exit_program
       ENDIF
      OF 3:
       SET message = nowindow
        ...
        *
        ...
        DETAIL
         rpt_loop = (rpt_loop+ 1)
         IF ((rpt_rec->parent_id[rpt_loop].type=3))
          row + 1, col 0, rpt_rec->src_pos_cd,
          col 18, rpt_rec->cr_pos_cd, col 36,
          rpt_rec->parent_id[rpt_loop].src_pr_id, col 54, rpt_rec->parent_id[rpt_loop].cr_pr_id,
          col 72, rpt_rec->parent_id[rpt_loop].src_nvp_cnt, col 90,
       ...
       ;end select
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg("Display : Problems during merge only (possible rows did not merge at all)",
         dm_err->logfile,1)
        GO TO exit_program
       ENDIF
      OF 4:
        ...
        *
        ...
        DETAIL
         rpt_loop = (rpt_loop+ 1)
         IF ((rpt_rec->parent_id[rpt_loop].type=2))
          row + 1, col 0, rpt_rec->src_pos_cd,
          col 18, rpt_rec->cr_pos_cd, col 36,
          rpt_rec->parent_id[rpt_loop].src_pr_id, col 54, rpt_rec->parent_id[rpt_loop].cr_pr_id,
          col 72, rpt_rec->parent_id[rpt_loop].src_nvp_cnt, col 90,
       ...
       ;end select
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg("Display : Problems in both merge and cutover",dm_err->logfile,1)
        GO TO exit_program
       ENDIF
      OF 5:
       SET message = nowindow
        ...
        *
        ...
        HEAD REPORT
         ...
         "TARGET_NVP_CNT", col 108, "TARGET_NPV$R_CNT"
        DETAIL
         rpt_loop = (rpt_loop+ 1), row + 1, col 0,
         rpt_rec->src_pos_cd, col 18, rpt_rec->cr_pos_cd,
         col 36, rpt_rec->parent_id[rpt_loop].src_pr_id, col 54,
         rpt_rec->parent_id[rpt_loop].cr_pr_id, col 72, rpt_rec->parent_id[rpt_loop].src_nvp_cnt,
         col 90, rpt_rec->parent_id[rpt_loop].cr_nvp_cnt, col 108,
         rpt_rec->parent_id[rpt_loop].cr_nvp_cnt_r
        WITH nocounter, formfeed = none
       ;end select
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg("Display : everything",dm_err->logfile,1)
        GO TO exit_program
       ENDIF
      OF 0:
       CALL clear(1,1)
