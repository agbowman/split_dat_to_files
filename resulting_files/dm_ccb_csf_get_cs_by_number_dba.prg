CREATE PROGRAM dm_ccb_csf_get_cs_by_number:dba
 ...
 IF (size(request->filterlist,5) != 0)
  SET reply->message = "Non-code set number search criteria submitted."
  SET failed = "T"
  GO TO exit_script
 ENDIF
 IF ((request->code_set < 1))
  SET reply->message = "Must submit a non-zero code set number."
  SET failed = "T"
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   stat = alterlist(reply->code_set_list,1)
  DETAIL
   reply->code_set_list[1].code_set = cvs.code_set, reply->code_set_list[1].definition = cvs
   .definition, reply->code_set_list[1].description = cvs.description,
   reply->code_set_list[1].display = cvs.display
  WITH nocounter
 ;end select
 IF (size(reply->code_set_list,5) < 1)
  SELECT INTO "nl:"
   ...
   FOOT REPORT
    stat = alterlist(reply->code_set_list,code_set_count)
   WITH nocounter
  ;end select
 ENDIF
 IF (error(errmsg,0) != 0)
