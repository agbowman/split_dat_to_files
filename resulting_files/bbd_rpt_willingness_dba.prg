CREATE PROGRAM bbd_rpt_willingness:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 RECORD captions(
   ...
   1 willingness = vc
   1 rpt_as_of_date = vc
   1 willingness_disp = vc
   1 willingness_desc = vc
   1 willingness_type = vc
   1 effective = vc
   1 end_effective = vc
   1 active = vc
   1 yes = vc
 ...
 SET captions->rpt_title = uar_i18ngetmessage(i18nhandle,"rpt_title",
  "W I L L I N G N E S S   R E P O R T")
 SET captions->rpt_time = uar_i18ngetmessage(i18nhandle,"rpt_time","Time:")
 SET captions->willingness = uar_i18ngetmessage(i18nhandle,"willingness","(Willingness)")
 SET captions->rpt_as_of_date = uar_i18ngetmessage(i18nhandle,"rpt_as_of_date","As of Date:")
 SET captions->willingness_disp = uar_i18ngetmessage(i18nhandle,"willingness_disp",
  "Willingness Display")
 SET captions->willingness_desc = uar_i18ngetmessage(i18nhandle,"willingness_desc",
  "Willingness Description")
 SET captions->willingness_type = uar_i18ngetmessage(i18nhandle,"willingness_type","Willingness Type"
  )
 ...
 SET captions->rpt_id = uar_i18ngetmessage(i18nhandle,"rpt_id","Report ID: BBD_RPT_WILLINGNESS")
 ...
 SELECT INTO "cer_temp:bbdwlngness.txt"
  c.*, cs.*
  FROM code_value c,
   common_data_foundation cs
  PLAN (c
   WHERE c.code_set=14236
    AND c.code_value > 0)
   JOIN (cs
   WHERE cs.code_set=14236
  ...
  HEAD PAGE
   ...
   CALL center(captions->willingness,1,125),
   col 104, captions->rpt_as_of_date, col 118,
   curdate"@DATECONDENSED;;d", row + 2, col 3,
   captions->willingness_disp, col 27, captions->willingness_desc,
   col 55, captions->willingness_type, col 76,
   captions->effective, col 90, captions->end_effective,
   col 108, captions->active, row + 1,
   col 3, "-------------------", col 27,
   "-------------------------", col 55, "-------------------",
   col 76, "-------------", col 90,
   "---------------", col 108, "------",
  ...
  DETAIL
   ...
   cs.display"#################", col 76, c.begin_effective_dt_tm"@DATECONDENSED;;d",
   col 84, c.begin_effective_dt_tm"@TIMENOSECONDS;;M", col 90,
   c.end_effective_dt_tm"@DATECONDENSED;;d", col 99, c.end_effective_dt_tm"@TIMENOSECONDS;;M"
   IF (c.active_ind=1)
    col 109, captions->yes
   ELSE
    col 109, captions->no
   ENDIF
   row + 1
