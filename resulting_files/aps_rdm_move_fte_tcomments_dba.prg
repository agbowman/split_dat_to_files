CREATE PROGRAM aps_rdm_move_fte_tcomments:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_rdm_move_fte_tcomments"
 ...
 CALL logmsg("Getting ready to begin reading for ap_ft_event termination comment.",((e_dispmsg+
 ...
 WHILE (afe_cntr >= 1)
   ...
   IF (curqual != value(afe_commit_recs_whole))
    ROLLBACK
    CALL logmsg("Failed to update ap_ft_event table.",((e_dispmsg+ e_logtofile)+ e_rdmstatus))
    SET sprogramfailed = "T"
    GO TO exit_script
   ENDIF
   SET sprogramfailed = "F"
