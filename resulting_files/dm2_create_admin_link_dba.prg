CREATE PROGRAM dm2_create_admin_link:dba
 DECLARE dm2_mod = vc WITH private, constant("002")
 DECLARE cal_logfile_prefix = vc WITH protect, constant("dm2_cre_adm_link")
 DECLARE cal_admn_node = vc WITH protect, noconstant(" ")
 DECLARE cal_admn_port = vc WITH protect, noconstant(" ")
 DECLARE cal_admn_pword = vc WITH protect, noconstant(" ")
 DECLARE cal_username = vc WITH protect, noconstant(" ")
 DECLARE cal_host = vc WITH protect, noconstant(" ")
 DECLARE cal_str = vc WITH protect, noconstant(" ")
 DECLARE cal_admin_db_link_name = vc WITH protect, noconstant(" ")
 DECLARE cal_admin_db_link_cnt = i4 WITH protect, noconstant(0)
 DECLARE cal_dot_pos = i4 WITH protect, noconstant(0)
 IF (validate(dm2_create_admin_link_name," ")=" ")
  SET dm2_create_admin_link_name = "ADMIN1"
 ENDIF
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile(cal_logfile_prefix,".log","DM2_CREATE_ADMIN_LINK LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning Create Admin Link Process"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET cal_admn_node = build(trim( $1))
 IF (cal_admn_node=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Admin Node  ",cal_admn_node," is invalid.")
  SET dm_err->eproc = "Admin Node Validation"
  SET dm_err->user_action = concat("Please specify a valid admin node. ")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET cal_admn_port = build(trim( $2))
 IF (cal_admn_port=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Admin Port  ",cal_admn_port," is invalid.")
  SET dm_err->eproc = "Admin Port Validation"
  SET dm_err->user_action = concat("Please specify a valid admin port number. ")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET cal_admn_pword = build(trim( $3))
 IF (cal_admn_pword=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Admin Password  ",cal_admn_pword," is invalid.")
  SET dm_err->eproc = "Admin Password Validation"
  SET dm_err->user_action = concat("Please specify a valid admin password. ")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining if a database link for ADMIN exists in ALL_DB_LINKS"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  ...
  WHERE parser(build(" dl.db_link = '",dm2_create_admin_link_name,"*'"))
  DETAIL
   cal_dot_pos = findstring(".",dl.db_link,1)
   IF (cal_dot_pos > 0)
    cal_admin_db_link_name = substring(1,(cal_dot_pos - 1),dl.db_link)
   ELSE
    cal_admin_db_link_name = dl.db_link
   ENDIF
   IF (cnvtupper(cal_admin_db_link_name)=cnvtupper(dm2_create_admin_link_name))
    cal_admin_db_link_cnt = (cal_admin_db_link_cnt+ 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF (cal_admin_db_link_cnt=0)
  SET dm_err->eproc = concat("Creating database link for ",dm2_create_admin_link_name)
  CALL disp_msg(" ",dm_err->logfile,0)
  SET cal_str = concat("create public database link ",dm2_create_admin_link_name,
   ' connect to cdba identified by "',cal_admn_pword,^" using '^,
   cal_admn_node,"'")
  SET cal_str = concat("rdb asis(^ ",cal_str," ^) go")
  IF (dm2_push_cmd(cal_str,1)=0)
   GO TO exit_program
  ENDIF
 ELSEIF (cal_admin_db_link_cnt > 1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Multiple existing db links match the admin link entered."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Validating ",dm2_create_admin_link_name," database link")
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM (value(build("V$DATABASE@",dm2_create_admin_link_name)))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Error selecting against ",dm2_create_admin_link_name," - v$database")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
#exit_program
 SET dm_err->eproc = "Create Admin Link Process Completed!"
 CALL final_disp_msg(cal_logfile_prefix)
END GO
