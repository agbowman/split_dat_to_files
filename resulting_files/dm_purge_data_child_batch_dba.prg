CREATE PROGRAM dm_purge_data_child_batch:dba
 ...
 END ;Subroutine
 DECLARE dpdcb_loop = i4 WITH protect, noconstant(0)
 DECLARE dpdcb_rowidloop = i4 WITH protect, noconstant(0)
 DECLARE dpdcb_purgestart = dq8 WITH protect, noconstant(0.0)
 DECLARE dpdcb_purgeruntime = f8 WITH protect, noconstant(0.0)
 DECLARE dpdcb_rdbstmt = vc WITH protect, noconstant("")
 DECLARE dpdcb_insertcount = i4 WITH protect, noconstant(0)
 DECLARE dpbcb_insertresult = i4 WITH protect, noconstant(0)
 ...
 DECLARE tmp_call = vc WITH protect, noconstant("")
 CALL parser(concat("declare ",v_temp_proc_name,"((p_result = i4),(p_rowid = vc)) = null go"))
 WHILE ((dpdcb_loop < parser_child->max_parser_cnt)
  AND (parser_child->next_row_to_purge <= v_num_rows))
   IF ((jobs->data[job_ndx].purge_flag != c_audit))
    CALL parser(v_truncate_stmt)
   ENDIF
   SET dpdcb_rowidloop = 1
   SET dpdcb_insertcount = 0
   WHILE ((dpdcb_loop < parser_child->max_parser_cnt)
    AND (parser_child->next_row_to_purge <= v_num_rows)
    AND (dpdcb_rowidloop <= row_data->rows_between_commit))
     IF ((jobs->data[job_ndx].purge_flag != c_audit))
      SET tmp_call = concat("call ",v_temp_proc_name,"(dpbcb_insertResult, '",trim(row_data->rows[
        parser_child->next_row_to_purge].row_id,3),"') go")
      CALL echo(tmp_call)
      CALL parser(tmp_call)
      SET dpdcb_insertcount = (dpdcb_insertcount+ dpbcb_insertresult)
     ENDIF
     SET parser_child->next_row_to_purge = (parser_child->next_row_to_purge+ 1)
     SET dpdcb_loop = (dpdcb_loop+ 1)
     SET dpdcb_rowidloop = (dpdcb_rowidloop+ 1)
   ENDWHILE
   ...
   WHILE (v_tab_ndx >= 1)
     SET dpdcb_purgestart = cnvtdatetime(curdate,curtime3)
     SET dpdcb_rdbstmt = replace(trim(tab_list->tabs[v_tab_ndx].stmt,3),'"',"'",0)
     SET v_num_found = 0
     IF ((jobs->data[job_ndx].purge_flag=c_audit))
      IF (validate(request->debug_mode,"Z") != "Z")
       SET v_debug_str = concat("rdb asis(^ EXPLAIN PLAN SET STATEMENT_ID = '",trim(curuser,3),":",
        trim(cnvtstring(cnvtdatetime(curdate,curtime3)),3),"' INTO PLAN_TABLE FOR ",
        dpdcb_rdbstmt," ^) go")
       ...
       ;end delete
       CALL echo(v_debug_str)
       CALL parser(v_debug_str)
       CALL output_plan(v_statement_id,v_file,v_debug_str)
      ENDIF
      IF (v_tab_ndx=1)
       SET tab_list->tabs[v_tab_ndx].num_found = ((tab_list->tabs[v_tab_ndx].num_found+
       dpdcb_rowidloop) - 1)
      ELSE
       SET v_num_found = 0
      ENDIF
     ELSE
      IF (dpdcb_insertcount > 0)
       SET dpdcb_rdbstmt = concat("rdb asis(^ ",dpdcb_rdbstmt,"^) go")
       IF (validate(request->debug_mode,"Z") != "Z")
        CALL echo(concat("Generated query: ",dpdcb_rdbstmt))
       ENDIF
       CALL parser(dpdcb_rdbstmt)
       SET v_num_found = curqual
      ELSE
       IF (v_tab_ndx=1)
        SET v_num_found = (dpdcb_rowidloop - 1)
       ELSE
        SET v_num_found = 0
       ENDIF
      ENDIF
      SET dpdcb_purgeruntime = datetimediff(cnvtdatetime(curdate,curtime3),dpdcb_purgestart,4)
