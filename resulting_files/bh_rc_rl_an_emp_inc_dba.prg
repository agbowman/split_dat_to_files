CREATE PROGRAM bh_rc_rl_an_emp_inc:dba
 CALL echo("***** bh_rc_rl_an_emp_inc.prg - 677918 *****")
   ...
   FOR (lchgcnt = 1 TO size(request->integer_params,5))
     CASE (request->integer_params[lchgcnt].key_txt)
      OF "EMPLOYMENT_INCOME_ANNUAL":
       SET tmprec->employment_income_monthly = (request->integer_params[lchgcnt].integer_value/ 12)
     ENDCASE
   ENDFOR
 END ;Subroutine
 DECLARE createreply(null) = null
 SUBROUTINE createreply(null)
  CALL addtoreplyintegers("EMPLOYMENT_INCOME_MONTHLY",tmprec->employment_income_monthly)
