CREATE PROGRAM dm_mm_discrepancy_rows:dba
 ...
 SET reply->table_name = "MM_DISCREPANCY"
 SET reply->rows_between_commit = 50
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 DECLARE v_years_to_keep = i4 WITH protect, noconstant(- (1))
 DECLARE v_years_back = dq8 WITH protect, noconstant(0.0)
 ...
 IF (v_years_to_keep < 1)
  ...
  GO TO exit_script
 ENDIF
 SET v_years_back = cnvtlookbehind(build(v_years_to_keep,",Y"))
 SELECT INTO "nl:"
  FROM mm_discrepancy md
  WHERE ((md.mm_discrepancy_id+ 0) > 0)
   AND parser(sbr_getrowidnotexists("md.updt_dt_tm < cnvtdatetime(v_years_back)","md"))
  DETAIL
   v_rows = (v_rows+ 1)
   IF (mod(v_rows,100)=1)
    stat = alterlist(reply->rows,(v_rows+ 99))
   ENDIF
   reply->rows[v_rows].row_id = md.rowid
  WITH nocounter, maxqual(md,request->max_rows)
 ;end select
