CREATE PROGRAM dc_mp_get_sticky_notes:dba
 PROMPT
 ...
 IF ( NOT (validate(reply,0)))
  RECORD reply(
    ...
    1 sticky_note_type_cd_list[*]
      2 sticky_note_type_cd = f8
    1 sticky_note_type_mean = vc
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE sscript_name = vc WITH protect, constant("DC_MP_GET_STICKY_NOTES")
 DECLARE dpid = f8 WITH protect, constant(cnvtreal( $PID))
 DECLARE npind = i2 WITH protect, constant(cnvtint( $PUBIND))
 DECLARE dsncd = f8 WITH protect, noconstant(0.0)
 DECLARE sname = vc WITH protect, noconstant(" ")
 DECLARE lstat = i4 WITH protect, noconstant(0)
 DECLARE errmsg = c132 WITH protect, noconstant(fillstring(132," "))
 DECLARE error_check = i2 WITH protect, noconstant(error(errmsg,1))
 CALL echo(build2("dPID   --> ",dpid))
 CALL echo(build2("$SNCDF --> ", $SNCDF))
 CALL echo(build2("nPIND  --> ",npind))
 SET reply->status_data.status = "F"
 IF (( $SCD=0.0)
  AND ( $SNCDF > ""))
  SET dsncd = uar_get_code_by("MEANING",14122,nullterm( $SNCDF))
  SET temp_rec->cnt = 1
  SET lstat = alterlist(temp_rec->qual,1)
  SET temp_rec->qual[1].value = dsncd
 ELSE
 ...
 IF ((temp_rec->cnt <= 0))
  ...
  CALL errorhandler("F",concat("CDF passed in: [",trim( $SNCDF),"]"),concat(
    "uar_get_code_by(MEANING) returned: [",trim(cnvtstring(dsncd)),"]"))
 ENDIF
 DECLARE i = i4 WITH protect, noconstant(0)
 SET lstat = alterlist(reply->sticky_note_type_cd_list,temp_rec->cnt)
 FOR (i = 1 TO temp_rec->cnt)
   SET reply->sticky_note_type_cd_list[i].sticky_note_type_cd = temp_rec->qual[i].value
 ENDFOR
 ...
 SET reply->parent_entity_id = dpid
 SET reply->parent_entity_name = trim(sname)
 SET reply->sticky_note_type_cd = temp_rec->qual[1].value
 SET reply->sticky_note_type_mean = trim( $SNCDF)
 SET lstat = getstickynotes(dpid,trim(sname),npind)
 SUBROUTINE (getstickynotes(p=f8,n=vc,pi=i2) =i4)
   DECLARE i = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM sticky_note sn,
     long_text lt,
     prsnl pnl
    PLAN (sn
     WHERE expand(i,1,temp_rec->cnt,sn.sticky_note_type_cd,temp_rec->qual[i].value)
      AND sn.parent_entity_id=p
      AND sn.parent_entity_name=n
      AND sn.parent_entity_id != 0
      AND sn.public_ind >= pi)
     JOIN (lt
     WHERE (lt.long_text_id= Outerjoin(sn.long_text_id))
      AND (lt.active_ind= Outerjoin(1)) )
     JOIN (pnl
     WHERE (pnl.person_id= Outerjoin(sn.updt_id)) )
    ORDER BY sn.updt_dt_tm DESC
    HEAD REPORT
     i = 0
    DETAIL
     i += 1
     IF (i > size(reply->sticky_notelist,5))
      lstat = alterlist(reply->sticky_notelist,(i+ 5))
     ENDIF
     reply->sticky_notelist[i].sticky_note_id = sn.sticky_note_id, reply->sticky_notelist[i].
     sticky_note_type_cd = sn.sticky_note_type_cd, reply->sticky_notelist[i].vu_dt_tm = sn
     .end_effective_dt_tm
     IF (validate(reply->sticky_notelist[i].svu_dt_tm_utc,"-999") != "-999")
      reply->sticky_notelist[i].svu_dt_tm_utc = format(cnvtdatetimeutc(sn.end_effective_dt_tm,3),
       "YYYY-MM-DDTHH:MM:SSZ;;Q")
     ENDIF
     reply->sticky_notelist[i].svu_dt_tm = format(sn.end_effective_dt_tm,"@SHORTDATETIME"), reply->
     sticky_notelist[i].updt_id = sn.updt_id, reply->sticky_notelist[i].updt_name = trim(pnl
      .name_full_formatted),
     reply->sticky_notelist[i].updt_dt_tm = sn.updt_dt_tm
     IF (validate(reply->sticky_notelist[i].supdt_dt_tm_utc,"-999") != "-999")
      reply->sticky_notelist[i].supdt_dt_tm_utc = format(cnvtdatetimeutc(sn.updt_dt_tm,3),
       "YYYY-MM-DDTHH:MM:SSZ;;Q")
     ENDIF
     reply->sticky_notelist[i].supdt_dt_tm = format(sn.updt_dt_tm,"@SHORTDATETIME"), reply->
     sticky_notelist[i].public_ind = sn.public_ind
     IF (sn.long_text_id > 0)
      reply->sticky_notelist[i].sticky_note_text = lt.long_text
     ELSE
      reply->sticky_notelist[i].sticky_note_text = sn.sticky_note_text
     ENDIF
    FOOT REPORT
     lstat = alterlist(reply->sticky_notelist,i), reply->sticky_note_cnt = i
    WITH nocounter
   ;end select
   SET error_check = error(errmsg,0)
   IF (error_check != 0)
    CALL errorhandler("F",concat("Select GetStickyNotes(",trim(cnvtstring(p)),",",trim(cnvtstring(pi)
       )),errmsg)
   ENDIF
   RETURN(reply->sticky_note_cnt)
 END ;Subroutine
