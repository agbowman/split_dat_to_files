CREATE PROGRAM afc_ens_bill_item:dba
 ...
 IF ( NOT (validate(cs106_specific_addon)))
  DECLARE cs106_specific_addon = f8 WITH protect, constant(uar_get_code_by("MEANING",106,
    "AFC ADD SPEC"))
 ENDIF
 DECLARE logicaldomainid = f8 WITH protect, noconstant(0.0)
 DECLARE logicaldomaininuse = i2 WITH protect, noconstant(false)
 DECLARE msstat = f8 WITH protect, noconstant(0.0)
 SET hafc_ens_bill_item = 0
 SET istatus = 0
 ...
 IF ((request->bill_item_qual > 0))
  IF (arelogicaldomainsinuse(null))
   ...
   ENDIF
   CALL copyrequestinfo(0)
   SET ensurereq->logical_domain_id = logicaldomainid
   SET ensurereq->logical_domain_enabled_ind = true
   SET logicaldomaininuse = true
  ENDIF
  FOR (inx0 = 1 TO request->bill_item_qual)
    CASE (request->bill_item[inx0].action_type)
     OF "ADD":
      SET action_begin = inx0
      SET action_end = inx0
      SELECT INTO "nl:"
       b.bill_item_id, b.ext_parent_reference_id, b.ext_parent_contributor_cd,
       b.ext_child_reference_id, b.ext_child_contributor_cd
       FROM bill_item b
       WHERE (b.ext_parent_reference_id=request->bill_item[inx0].ext_parent_reference_id)
        AND (b.ext_parent_contributor_cd=request->bill_item[inx0].ext_parent_contributor_cd)
        AND (b.ext_child_reference_id=request->bill_item[inx0].ext_child_reference_id)
        AND (b.ext_child_contributor_cd=request->bill_item[inx0].ext_child_contributor_cd)
        AND b.logical_domain_id=logicaldomainid
       DETAIL
        request->bill_item[inx0].bill_item_id = b.bill_item_id, request->bill_item[inx0].action_type
         = "UPT"
       WITH nocounter
      ;end select
      IF (curqual != 0)
       EXECUTE afc_upt_bill_item
       SET stat = alterlist(reply->bill_item,inx0)
       SET reply->bill_item_qual = inx0
       SET reply->bill_item[inx0].bill_item_id = request->bill_item[inx0].bill_item_id
      ELSE
       IF ((request->bill_item[inx0].ext_owner_cd=cs106_specific_addon)
        AND logicaldomaininuse)
        EXECUTE afc_add_bill_item  WITH replace("REQUEST",ensurereq)
       ELSE
  ...
  ENDFOR
 ENDIF
 SUBROUTINE (copyrequestinfo(dummyvar=i2) =null)
  DECLARE cntbillitem = i4 WITH protect, noconstant(0)
  SELECT INTO "nl:"
   FROM (dummyt d1  WITH seq = value(size(request->bill_item,5)))
   HEAD REPORT
    ensurereq->bill_item_qual = request->bill_item_qual
   DETAIL
    cntbillitem += 1, stat = alterlist(ensurereq->bill_item,cntbillitem), ensurereq->bill_item[
    cntbillitem].active_ind = request->bill_item[cntbillitem].active_ind,
    ensurereq->bill_item[cntbillitem].active_ind_ind = request->bill_item[cntbillitem].active_ind_ind,
    ensurereq->bill_item[cntbillitem].active_status_cd = request->bill_item[cntbillitem].
    active_status_cd, ensurereq->bill_item[cntbillitem].active_status_dt_tm = request->bill_item[
    cntbillitem].active_status_dt_tm,
    ensurereq->bill_item[cntbillitem].active_status_prsnl_id = request->bill_item[cntbillitem].
    active_status_prsnl_id, ensurereq->bill_item[cntbillitem].beg_effective_dt_tm = request->
    bill_item[cntbillitem].beg_effective_dt_tm, ensurereq->bill_item[cntbillitem].bill_item_id =
    request->bill_item[cntbillitem].bill_item_id,
    ensurereq->bill_item[cntbillitem].charge_point_cd = request->bill_item[cntbillitem].
    charge_point_cd, ensurereq->bill_item[cntbillitem].child_seq = request->bill_item[cntbillitem].
    child_seq, ensurereq->bill_item[cntbillitem].end_effective_dt_tm = request->bill_item[cntbillitem
    ].end_effective_dt_tm,
    ensurereq->bill_item[cntbillitem].ext_child_contributor_cd = request->bill_item[cntbillitem].
    ext_child_contributor_cd, ensurereq->bill_item[cntbillitem].ext_child_reference_id = request->
    bill_item[cntbillitem].ext_child_reference_id, ensurereq->bill_item[cntbillitem].ext_description
     = request->bill_item[cntbillitem].ext_description,
    ensurereq->bill_item[cntbillitem].ext_owner_cd = request->bill_item[cntbillitem].ext_owner_cd,
    msstat = assign(ensurereq->bill_item[cntbillitem].ext_sub_owner_cd,validate(request->bill_item[
      cntbillitem].ext_sub_owner_cd,0.0)), ensurereq->bill_item[cntbillitem].
    ext_parent_contributor_cd = request->bill_item[cntbillitem].ext_parent_contributor_cd,
    ensurereq->bill_item[cntbillitem].ext_parent_reference_id = request->bill_item[cntbillitem].
    ext_parent_reference_id, ensurereq->bill_item[cntbillitem].ext_short_desc = request->bill_item[
    cntbillitem].ext_short_desc, ensurereq->bill_item[cntbillitem].late_chrg_excl_ind = request->
    bill_item[cntbillitem].late_chrg_excl_ind,
    ensurereq->bill_item[cntbillitem].parent_qual_ind = request->bill_item[cntbillitem].
    parent_qual_ind, ensurereq->bill_item[cntbillitem].workload_only_ind = request->bill_item[
    cntbillitem].workload_only_ind, ensurereq->bill_item[cntbillitem].updt_cnt = request->bill_item[
    cntbillitem].updt_cnt,
    ensurereq->bill_item[cntbillitem].misc_ind = request->bill_item[cntbillitem].misc_ind, ensurereq
    ->bill_item[cntbillitem].stats_only_ind = request->bill_item[cntbillitem].stats_only_ind
   WITH nocounter
  ;end select
 END ;Subroutine
#check_error
