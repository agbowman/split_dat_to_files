CREATE PROGRAM acm_mark_proc_as_reviewed:dba
 ...
 DECLARE proc_count = i4 WITH protect, constant(size(request->procedures,5))
 DECLARE proc_id = f8 WITH protect, noconstant(0)
 DECLARE start = i4 WITH protect, noconstant(1)
 DECLARE loop_cnt = i4 WITH protect, noconstant(0)
 DECLARE batch_size = i2 WITH protect, constant(20)
 DECLARE padded_proc_count = i4 WITH protect, noconstant(0)
 IF (proc_count=0)
  SET failed = attribute_error
  SET table_name = "No procedures to mark"
  GO TO exit_script
 ENDIF
 SET loop_cnt = ceil((cnvtreal(proc_count)/ batch_size))
 SET padded_proc_count = (batch_size * loop_cnt)
 IF (padded_proc_count > proc_count)
  SET stat = alterlist(request->procedures,padded_proc_count)
  FOR (idx = (proc_count+ 1) TO padded_proc_count)
    SET request->procedures[idx].procedure_id = request->procedures[proc_count].procedure_id
  ENDFOR
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   JOIN (p
   WHERE expand(idx,start,((start+ batch_size) - 1),p.procedure_id,request->procedures[idx].
    procedure_id))
  WITH nocounter
 ;end select
 SET stat = alterlist(request->procedures,proc_count)
 IF (curqual != proc_count)
  SET failed = attribute_error
  SET table_name = "Attempted to mark a non-existant procedure as reviewed"
  GO TO exit_script
 ENDIF
 SET stat = alterlist(add_procedure_acti_request->qual,proc_count)
 SET stat = alterlist(reply->procedures,proc_count)
 FOR (idx = 1 TO proc_count)
   IF ((request->procedures[idx].procedure_id <= 0))
    SET failed = attribute_error
    SET table_name = "Invalid procedure id"
    GO TO exit_script
   ELSE
    SET add_procedure_acti_request->qual[idx].procedure_id = request->procedures[idx].procedure_id
    SET add_procedure_acti_request->qual[idx].prsnl_id = reqinfo->updt_id
    SET add_procedure_acti_request->qual[idx].action_type_mean = "REVIEW"
    SET add_procedure_acti_request->qual[idx].action_dt_tm = cnvtdatetime(sysdate)
    SET reply->procedures[idx].procedure_id = add_procedure_acti_request->qual[idx].procedure_id
    SET reply->procedures[idx].last_action_dt_tm = add_procedure_acti_request->qual[idx].action_dt_tm
   ENDIF
 ENDFOR
