CREATE PROGRAM bed_get_xr_ops_prsnl:dba
 RECORD reply(
   1 provider_list[*]
     2 provider_id = f8
     2 provider_name = vc
 ...
 SUBROUTINE check_log_level(arg_log_level)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "BED_GET_XR_OPS_PRSNL"
 SET reply->status_data.status = "F"
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cop
   WHERE (cop.charting_operations_id=request->charting_operations_id)
    AND cop.charting_operations_prsnl_id != 0)
   JOIN (p
  ...
  DETAIL
   nbr = (nbr+ 1)
   IF (mod(nbr,10)=1)
    stat = alterlist(reply->provider_list,(nbr+ 9))
   ENDIF
   reply->provider_list[nbr].provider_id = p.person_id, reply->provider_list[nbr].provider_name = p
   .name_full_formatted
  FOOT REPORT
   stat = alterlist(reply->provider_list,nbr)
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET reply->status_data.status = "Z"
 ...
 ENDIF
 CALL log_message("End of script: bed_get_xr_ops_prsnl.",log_level_debug)
END GO
