CREATE PROGRAM dm2_mig_maintain_target:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD dmmt_ind_data
 RECORD dmmt_ind_data(
   1 cnt = i4
   1 qual[*]
     2 table_owner = vc
     2 table_name = vc
 ...
 DECLARE dmmt_run_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmmt_schema_date = dq8 WITH protect, noconstant(0.0)
 DECLARE dmmt_schema_date_offset = i2 WITH protect, noconstant(0)
 DECLARE dmmt_schema_date_search_continue = i2 WITH protect, noconstant(0)
 DECLARE dmmt_iter = i4 WITH protect, noconstant(0)
 DECLARE dmmt_op_type = vc WITH protect, noconstant("")
 DECLARE dmmt_table_name = vc WITH protect, noconstant("")
 DECLARE dmmt_tablespace_name = vc WITH protect, noconstant("")
 DECLARE dmmt_row_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmmt_obj_name = vc WITH protect, noconstant("")
 DECLARE dmmt_priority = i4 WITH protect, noconstant(0)
 DECLARE dmmt_operation = vc WITH protect, noconstant("")
 DECLARE dmmt_owner = vc WITH protect, noconstant("")
 DECLARE dmmt_insert_ddl_op_row(null) = i2
 SET dm2_install_schema->process_option = "MIGRATION MAINT"
 IF (check_logfile("dm2_mig_maintain_target",".log","dm2_mig_maintain_target LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_maintain_target"
 CALL disp_msg("",dm_err->logfile,0)
 IF (dmr_load_mig_data(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking connection data from mig file..."
 IF ((((dmr_mig_data->src_v500_pwd="DM2NOTSET")) OR ((((dmr_mig_data->src_v500_cnct_str="DM2NOTSET"))
  OR ((((dmr_mig_data->tgt_v500_pwd="DM2NOTSET")) OR ((dmr_mig_data->tgt_v500_cnct_str="DM2NOTSET")
 )) )) )) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Required connection data not found in migration config file, exiting..."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  WHERE (ddo.process_option=dm2_install_schema->process_option)
  DETAIL
   dmmt_run_id = ddo.run_id, dmmt_schema_date = ddo.schema_date
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  ...
  SELECT INTO "nl:"
   ...
   HEAD REPORT
    dmmt_run_id = 0
   DETAIL
    dmmt_run_id = max_run_id
   FOOT REPORT
    dmmt_run_id = (dmmt_run_id+ 1)
   WITH nocounter, nullreport
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dmmt_schema_date_search_continue = 1
  SET dmmt_schema_date_offset = 365
  WHILE (dmmt_schema_date_search_continue=1
   AND dmmt_schema_date_offset >= 335)
    SET dmmt_schema_date_offset = (dmmt_schema_date_offset - 1)
    SET dmmt_schema_date = cnvtdatetime((curdate - dmmt_schema_date_offset),0)
    SET dm_err->eproc = "Determining usable schema date from dm2_ddl_ops."
    SELECT INTO "nl:"
     FROM dm2_ddl_ops ddo
     WHERE ddo.schema_date=cnvtdatetime(dmmt_schema_date)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    IF (curqual=0)
     SET dmmt_schema_date_search_continue = 0
    ENDIF
  ENDWHILE
  IF (dmmt_schema_date_search_continue=1)
   SET dm_err->err_ind = 1
  ...
  INSERT  FROM dm2_ddl_ops ddo
   SET ddo.run_id = dmmt_run_id, ddo.gen_dt_tm = cnvtdatetime(curdate,curtime3), ddo.ocd = 0.0,
    ddo.schema_date = cnvtdatetime(dmmt_schema_date), ddo.process_option = dm2_install_schema->
    process_option, ddo.last_checkpoint = "MAINT"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSE
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Getting list indexes in a pending status."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_indexes i
  WHERE  NOT (i.index_type IN ("LOB", "IOT", "IOT - TOP", "CLUSTER"))
   AND i.index_name != "AQ$_QT$_OGG$*"
   AND i.table_name != "QT$_OGG$*"
   AND i.last_analyzed = null
  ...
  HEAD REPORT
   dmmt_ind_data->cnt = 0, stat = alterlist(dmmt_ind_data->qual,0)
  DETAIL
   dmmt_ind_data->cnt = (dmmt_ind_data->cnt+ 1)
   IF (mod(dmmt_ind_data->cnt,25)=1)
    stat = alterlist(dmmt_ind_data->qual,(dmmt_ind_data->cnt+ 24))
   ENDIF
   dmmt_ind_data->qual[dmmt_ind_data->cnt].index_owner = i.owner, dmmt_ind_data->qual[dmmt_ind_data->
   cnt].index_name = i.index_name, dmmt_ind_data->qual[dmmt_ind_data->cnt].table_owner = i
   .table_owner,
   dmmt_ind_data->qual[dmmt_ind_data->cnt].table_name = i.table_name, dmmt_ind_data->qual[
   dmmt_ind_data->cnt].index_tspace = i.tablespace_name
  FOOT REPORT
   stat = alterlist(dmmt_ind_data->qual,dmmt_ind_data->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 FOR (dmmt_iter = 1 TO dmmt_ind_data->cnt)
   SET dmmt_op_type = "EXECUTE DM2_FINISH_INDEX"
   SET dmmt_obj_name = dmmt_ind_data->qual[dmmt_iter].index_name
   SET dmmt_table_name = dmmt_ind_data->qual[dmmt_iter].table_name
   SET dmmt_priority = 342
   SET dmmt_tablespace_name = dmmt_ind_data->qual[dmmt_iter].index_tspace
   SET dmmt_owner = dmmt_ind_data->qual[dmmt_iter].index_owner
   SET dmmt_operation = concat('DM2_FINISH_INDEX "',trim(dmmt_ind_data->qual[dmmt_iter].table_owner),
    '","',trim(dmmt_table_name),'","',
    trim(dmmt_ind_data->qual[dmmt_iter].index_owner),'","',trim(dmmt_obj_name),'" GO')
   IF (dmmt_insert_ddl_op_row(null)=0)
    GO TO exit_script
   ENDIF
 ENDFOR
 SET dm2_install_schema->run_id = dmmt_run_id
 EXECUTE dm2_set_ready_to_run "ALL"
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if maintain process is complete from dm2_ddl_ops_log."
 SELECT INTO "nl:"
  FROM dm2_ddl_ops_log ddol
  WHERE ddol.run_id=dmmt_run_id
   AND (("COMPLETE" != ddol.status) OR (ddol.status = null))
 ...
 ENDIF
 GO TO exit_script
 SUBROUTINE dmmt_insert_ddl_op_row(null)
   DECLARE didor_op_id = f8 WITH protect, noconstant(0.0)
   SET dm_err->eproc = "Deleting duplicate rows for this operation."
   DELETE  FROM dm2_ddl_ops_log ddol
    WHERE ddol.run_id=dmmt_run_id
     AND ddol.op_type=dmmt_op_type
     AND ddol.obj_name=dmmt_obj_name
     AND ddol.priority=dmmt_priority
     AND ddol.table_name=dmmt_table_name
     AND ddol.owner_name=dmmt_owner
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Replacing !op_id in dmmt_operation with didor_op_id"
   SET dmmt_operation = replace(dmmt_operation,"!op_id",trim(cnvtstring(didor_op_id)),0)
   SET dm_err->eproc = "Inserting dm2_ddl_ops_log row for operation."
   INSERT  FROM dm2_ddl_ops_log ddol
    SET ddol.op_id = didor_op_id, ddol.owner_name = dmmt_owner, ddol.run_id = dmmt_run_id,
     ddol.gen_dt_tm = cnvtdatetime(curdate,curtime3), ddol.gen_id = dmmt_run_id, ddol.begin_dt_tm =
     cnvtdatetime(null),
     ddol.end_dt_tm = cnvtdatetime(null), ddol.est_duration = null, ddol.act_duration = null,
     ddol.op_type = dmmt_op_type, ddol.table_name = dmmt_table_name, ddol.tablespace_name =
     dmmt_tablespace_name,
     ddol.row_cnt = dmmt_row_cnt, ddol.obj_name = dmmt_obj_name, ddol.priority = dmmt_priority,
     ddol.status = null, ddol.error_msg = null, ddol.operation = dmmt_operation,
     ddol.on_error_operation = null, ddol.schema_date = cnvtdatetime(dmmt_schema_date), ddol
     .schema_instance = 0,
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_mig_maintain_target completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_maintain_target")
END GO
