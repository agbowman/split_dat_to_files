CREATE PROGRAM bed_aud_ap_orc_incmplt:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (oc
   WHERE oc.catalog_type_cd=genlab
    AND oc.activity_type_cd=apat_cd
    AND oc.active_ind=1)
  DETAIL
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   WHERE o.catalog_type_cd=genlab
    AND o.activity_type_cd=apat_cd
    AND o.active_ind=1)
   JOIN (cv1
 ...
 SET reply->collist[3].header_text = "No Subactivity Type"
 SET reply->collist[3].data_type = 1
 SET reply->collist[3].hide_ind = 0
 SET reply->collist[4].header_text = "No Assays"
 SET reply->collist[4].data_type = 1
 SET reply->collist[4].hide_ind = 0
 SET reply->collist[5].header_text = "No Work Routing"
 SET reply->collist[5].data_type = 1
 SET reply->collist[5].hide_ind = 0
 SET reply->collist[6].header_text = "Incomplete Collection Requirements"
 ...
 SET no_coll_req_cnt = 0
 FOR (x = 1 TO temp->o_cnt)
   IF ((((temp->olist[x].no_subact_ind=1)) OR ((((temp->olist[x].no_dta_ind=1)) OR ((((temp->olist[x]
   .no_routing_ind=1)) OR ((temp->olist[x].no_coll_req_ind=1))) )) )) )
 ...
 SET reply->statlist[1].statistic_meaning = "PATHAPORCNODTA"
 IF (no_dta_cnt > 0)
 ...
 SET reply->statlist[2].statistic_meaning = "PATHAPORCNOROUTING"
 IF (no_routing_cnt > 0)
 ...
 SET reply->statlist[3].statistic_meaning = "PATHAPORCNOCOLLREQ"
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("ap_orc_incmplt_audit.csv")
 ENDIF
