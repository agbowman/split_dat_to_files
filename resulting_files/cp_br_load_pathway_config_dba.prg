CREATE PROGRAM cp_br_load_pathway_config:dba
 PROMPT
  ...
  "Load Selected Nodes :" = 0,
  "Category Means :" = "",
  "Map Directory :" = "cer_install:"
  WITH outdev, inputpathwayid, inputconceptcd,
  inputpathwayinstanceid, inputunpublishedind, inputloadselnodes,
  inputcatmean, mapdir
 IF ( NOT (validate(reply)))
  RECORD reply(
    ...
    1 cp_concept_cd = f8
    1 isa_concept_group_cd = f8
    1 treatment_intention_cd = f8
    1 node_list[*]
      2 intention_cd = f8
 ...
 FREE RECORD cat_mean_list
 RECORD cat_mean_list(
   1 cnt = i4
   1 qual[*]
     2 value = vc
 )
 ...
 DECLARE readinputfileintorecord(null) = null WITH protect
 DECLARE gettriggeringsynonymsandsentences(null) = null WITH protect
 ...
 DECLARE treatment_intention_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003278,
   "TREATMENTS"))
 DECLARE isa_concept_group_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003133,"IS-A"))
 ...
 IF (( $INPUTPATHWAYID > 0))
  CALL gettriggeringcriteria(cnvtreal( $INPUTPATHWAYID))
  IF (( $INPUTLOADSELNODES=1))
   CALL getparametervalues(7,cat_mean_list)
   IF ((cat_mean_list->cnt > 0))
    IF (validate(debug_ind,0)=1)
     CALL echorecord(cat_mean_list)
    ENDIF
    CALL getselectedpathwayconfig(cnvtreal( $INPUTPATHWAYID))
   ELSE
    CALL reporterrorandexit("Initialization ","Node Category Meanings ","Empty")
   ENDIF
  ELSE
 ...
 SUBROUTINE readinputfileintorecord(null)
   ...
   IF (size(comp_req->components,5) > 0)
    SET stat = moverec(comp_req->components,reply->mappings)
   ELSE
    CALL echo("Components mapping is empty")
   ENDIF
 ...
 SUBROUTINE (getpathwayconfig(pathwayid=f8) =null WITH protect)
   ...
   DECLARE nodecnt = i4 WITH noconstant(0)
   SELECT INTO "NL:"
    FROM cp_pathway cp
    PLAN (cp
     WHERE cp.cp_pathway_id=pathwayid)
    HEAD REPORT
     dpathwayid = cp.cp_pathway_id, reply->cp_concept_cd = cp.cp_concept_cd, reply->pathway_id = cp
     .cp_pathway_id,
     ...
     uar_get_code_meaning(cp.pathway_status_cd), reply->concept_list[1].concept_cd = cp.cp_concept_cd,
     reply->pathway_diagram_url = cp.pathway_diagram_url
    WITH nocounter
   ;end select
   SET reply->treatment_intention_cd = treatment_intention_cd
   SET reply->isa_concept_group_cd = isa_concept_group_cd
   FREE RECORD request
   RECORD request(
     1 node_ids[*]
       2 cp_node_id = f8
   ...
   SELECT INTO "NL:"
    FROM cp_node cn
    PLAN (cn
     WHERE cn.cp_pathway_id=pathwayid
      AND cn.active_ind=1
    ...
    ORDER BY cn.version_node_id, cn.publish_ind
   ...
   CALL error_and_zero_check_rec(curqual,"getPathwayConfig","Get Pathway Nodes",1,1,
    reply)
   EXECUTE cp_br_get_nodes_config  WITH replace("REPLY","NODEREPLY")
   ...
   CALL log_message(build("Exit getPathwayConfig(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (getselectedpathwayconfig(pathwayid=f8) =null WITH protect)
   CALL log_message("In getPathwayConfig()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE vcpusblishparser = vc WITH noconstant("cn.publish_ind = 1")
   DECLARE cindx = i4 WITH protect, noconstant(0)
   DECLARE csize = i4 WITH protect, constant(cat_mean_list->cnt)
   FREE RECORD request
   RECORD request(
     1 node_ids[*]
       2 cp_node_id = f8
   ...
   SELECT INTO "NL:"
    ...
    PLAN (cp
     ...
     WHERE cn.cp_pathway_id=cp.cp_pathway_id
      ...
      AND cn.beg_effective_dt_tm < cnvtdatetime(sysdate)
      AND cn.end_effective_dt_tm > cnvtdatetime(sysdate)
      AND expand(cindx,1,csize,cn.category_mean,cat_mean_list->qual[cindx].value)
      AND parser(vcpusblishparser))
 ...
 SUBROUTINE (adddetailtocomp(nodepos=i4,comppos=i4,detailreplypos=i4,conceptgroupmean=vc,
  conceptgroupcd=f8) =i4 WITH protect)
   ...
   CALL log_message(build("Exit addDetailToComp(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (gettriggeringcriteria(pathwayid=f8) =null WITH protect)
   CALL log_message("Entering subroutine GetTriggeringCriteria()",log_level_debug)
   DECLARE is_care_pathway_build_tool = i2 WITH constant(1), protect
   EXECUTE cp_retr_triggering_criteria "NOFORMS", pathwayid, is_care_pathway_build_tool
   SET reply->cnt = triggering_criteria_reply->cnt
   SET stat = movereclist(triggering_criteria_reply->qual,reply->qual,1,0,triggering_criteria_reply->
    cnt,
    true)
   CALL gettriggeringsynonymsandsentences(null)
   CALL log_message("Exiting subroutine GetTriggeringCriteria()",log_level_debug)
 END ;Subroutine
 SUBROUTINE gettriggeringsynonymsandsentences(null)
   CALL log_message("Entering subroutine GetTriggeringSynonymsAndSentences()",log_level_debug)
   DECLARE ordersynonymindex = i4 WITH noconstant(0), protect
   DECLARE ordersynonymposition = i4 WITH noconstant(0), protect
   DECLARE ordersynonymsize = i4 WITH noconstant(0), protect
   DECLARE ordersentenceindex = i4 WITH noconstant(0), protect
   DECLARE ordersentenceposition = i4 WITH noconstant(0), protect
   DECLARE ordersentencesize = i4 WITH noconstant(0), protect
   SELECT INTO "nl:"
    ordersentenceisnull = nullind(os.order_sentence_id)
    FROM (dummyt d1  WITH seq = reply->cnt),
     order_catalog_synonym ocs,
     order_sentence os
    PLAN (d1
     WHERE (reply->qual[d1.seq].triggering_entity_name="ORDER_CATALOG_SYNONYM"))
     JOIN (ocs
     WHERE (ocs.synonym_id= Outerjoin(reply->qual[d1.seq].triggering_entity_id)) )
     JOIN (os
     WHERE (os.parent_entity_id= Outerjoin(ocs.synonym_id))
      AND (os.parent_entity_name= Outerjoin("ORDER_CATALOG_SYNONYM"))
      AND (os.parent_entity2_id= Outerjoin(0.0)) )
    ORDER BY d1.seq
    HEAD d1.seq
     ordersynonymsize = (size(reply->order_synonyms,5)+ 1), stat = alterlist(reply->order_synonyms,
      ordersynonymsize), reply->order_synonyms[ordersynonymsize].synonym_id = ocs.synonym_id,
     reply->order_synonyms[ordersynonymsize].synonym_mnemonic = ocs.mnemonic
    DETAIL
     IF (ordersentenceisnull=0)
      ordersentencesize = (size(reply->order_synonyms[ordersynonymsize].order_sentences,5)+ 1), stat
       = alterlist(reply->order_synonyms[ordersynonymsize].order_sentences,ordersentencesize), reply
      ->order_synonyms[ordersynonymsize].order_sentences[ordersentencesize].sentence_id = os
      .order_sentence_id,
      reply->order_synonyms[ordersynonymsize].order_sentences[ordersentencesize].sentence_display =
   ...
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = reply->cnt),
     order_sentence os,
     order_catalog_synonym os_ocs,
     order_sentence os_all
    PLAN (d1
     WHERE (reply->qual[d1.seq].triggering_entity_name="ORDER_SENTENCE"))
     JOIN (os
     WHERE (os.order_sentence_id= Outerjoin(reply->qual[d1.seq].triggering_entity_id)) )
     JOIN (os_ocs
     WHERE (os_ocs.synonym_id= Outerjoin(os.parent_entity_id)) )
     JOIN (os_all
     WHERE (os_all.parent_entity_id= Outerjoin(os_ocs.synonym_id))
      AND (os_all.parent_entity_name= Outerjoin("ORDER_CATALOG_SYNONYM"))
      AND (os_all.parent_entity2_id= Outerjoin(0.0)) )
    ORDER BY d1.seq, os_ocs.synonym_id, os_all.order_sentence_id
    HEAD d1.seq
     null
    HEAD os_ocs.synonym_id
     ordersynonymposition = locateval(ordersynonymindex,1,size(reply->order_synonyms,5),os_ocs
      .synonym_id,reply->order_synonyms[ordersynonymindex].synonym_id)
     IF (ordersynonymposition=0)
      ordersynonymsize = (size(reply->order_synonyms,5)+ 1), stat = alterlist(reply->order_synonyms,
       ordersynonymsize), reply->order_synonyms[ordersynonymsize].synonym_id = os_ocs.synonym_id,
      reply->order_synonyms[ordersynonymsize].synonym_mnemonic = os_ocs.mnemonic
     ENDIF
    DETAIL
     IF (ordersynonymposition=0)
      ordersentencesize = (size(reply->order_synonyms[ordersynonymsize].order_sentences,5)+ 1), stat
       = alterlist(reply->order_synonyms[ordersynonymsize].order_sentences,ordersentencesize), reply
      ->order_synonyms[ordersynonymsize].order_sentences[ordersentencesize].sentence_id = os_all
      .order_sentence_id,
      reply->order_synonyms[ordersynonymsize].order_sentences[ordersentencesize].sentence_display =
      os_all.order_sentence_display_line
     ENDIF
    WITH nocounter
   ;end select
   CALL log_message("Exiting subroutine GetTriggeringSynonymsAndSentences()",log_level_debug)
 END ;Subroutine
