CREATE PROGRAM dcp_get_tasks_by_result_sets:dba
 ...
 DECLARE result_set_count = i4 WITH constant(size(request->result_set_list,5)), protect
 DECLARE task_count = i4 WITH noconstant(0)
 SET reply->status_data.status = "F"
 IF (result_set_count > 0)
  SELECT INTO "nl:"
   FROM (dummyt dres  WITH seq = value(result_set_count)),
    task_activity ta,
    order_task ot,
    task_activity_assignment taa
   PLAN (dres)
    JOIN (ta
    WHERE (ta.result_set_id=request->result_set_list[dres.seq].result_set_id))
    JOIN (ot
