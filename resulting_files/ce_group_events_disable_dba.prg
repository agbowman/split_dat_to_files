CREATE PROGRAM ce_group_events_disable:dba
 ...
 SET readme_data->message = "Readme Failed: Starting ce_group_events_disable"
 ...
 IF (ce_job_cnt > 0)
  ...
  IF (((ce_cnt_idx > 0) OR (ce_state="RUNNING")) )
   ...
   ENDIF
  ENDIF
  SET dm_err->eproc = "Disable PROCESS_GROUPING_QUEUE job"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SET ce_job_cnt = 0
  SET ce_sql_string = concat(" rdb asis( ^ "," begin "," dbms_scheduler.disable("," NAME => '",concat
   (ce_owner,".",ce_job_name),
   "',"," FORCE => TRUE",");"," end;^) go")
  IF (dm2_push_cmd(ce_sql_string,1)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Set ENABLEGROUPEVENTS feature flag to N"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 ...
 SET readme_data->message = "Success: ce_group_events_disable completed with success"
 GO TO exit_program
#exit_program
 IF ((dm_err->err_ind=1))
 ...
 SET dm_err->eproc = "Ending ce_group_events_disable"
 CALL final_disp_msg("ce_group_events_disable")
END GO
