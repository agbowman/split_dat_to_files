CREATE PROGRAM ct_mp_get_prot_person_list:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_prot_person_list"
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getprotpersonlist(null) = null WITH protect
 DECLARE primarycd = f8 WITH protect, constant(uar_get_code_by("MEANING",17441,"PRIMARY"))
 DECLARE stat = i4 WITH protect, noconstant(0)
 DECLARE protpersonwherestring = vc WITH protect, noconstant("")
 DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
 DECLARE protcount = i4 WITH protect, noconstant(0)
 DECLARE max_date = dq8 WITH constant(cnvtdatetime("31-DEC-2100 00:00:00"))
 SET reply->status_data.status = "F"
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_person_list","Invalid JSON Request",
   reply)
  GO TO exit_script
 ENDIF
 SET stat = cnvtjsontorec( $JSONREQUEST)
 SET protcount = size(request->prot_list,5)
 IF (protcount <= 0)
  SET reply->status_data.status = "S"
  GO TO exit_script
 ENDIF
 IF ((request->context="PRINCIPAL_INVESTIGATOR"))
  SET protpersonwherestring = build("pr.prot_role_cd = ",primarycd)
 ELSEIF ((request->context="PROTOCOL_CONTACTS"))
  SET protpersonwherestring = "pr.primary_contact_ind = 1"
 ELSE
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_person_list","Invalid JSON Request",
   reply)
  GO TO exit_script
 ENDIF
 CALL getprotpersonlist(null)
 FREE RECORD request
 SET reply->status_data.status = "S"
 SUBROUTINE getprotpersonlist(null)
   CALL log_message("In GetProtPersonList()",log_level_debug)
   DECLARE protpersoncount = i4 WITH protect, noconstant(0)
   DECLARE amendmentitr = i4 WITH protect, noconstant(0)
   SELECT DISTINCT INTO "nl:"
    FROM prot_role pr,
     person p
    PLAN (pr
     WHERE expand(amendmentitr,1,protcount,pr.prot_amendment_id,cnvtreal(request->prot_list[
       amendmentitr].amendmentid))
      AND pr.end_effective_dt_tm=cnvtdatetime(max_date)
      AND parser(protpersonwherestring))
     JOIN (p
     WHERE p.person_id=pr.person_id)
    ORDER BY p.person_id
    HEAD p.person_id
     protpersoncount += 1
     IF (size(reply->prot_person_list,5) < protpersoncount)
      stat = alterlist(reply->prot_person_list,(protpersoncount+ 9))
     ENDIF
     reply->prot_person_list[protpersoncount].person_id = p.person_id, reply->prot_person_list[
     protpersoncount].person_name = p.name_full_formatted
    WITH nocounter
   ;end select
   SET stat = alterlist(reply->prot_person_list,protpersoncount)
 END ;Subroutine
#exit_script
 ...
 CALL log_message(build("ct_mp_get_prot_person_list ends here, Elapsed time in seconds:",datetimediff
   (cnvtdatetime(sysdate),begin_date_time,5)),log_level_debug)
END GO
