CREATE PROGRAM acm_chg_person_alias_hist:dba
 ...
 RECORD temp_record(
   1 person_alias_hist_qual[*]
     2 person_alias_hist_id = f8
 ...
 DECLARE tracking_bit = i4 WITH noconstant(127)
 ...
 ENDFOR
 SET stat = alterlist(temp_record->person_alias_hist_qual,xref->chg_cnt)
 SET pah_qual_cntr = 0
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(xref->chg_cnt))
  PLAN (d)
  DETAIL
   item_idx = xref->chg[d.seq].idx, chg_str = acm_request->person_alias_qual[item_idx].chg_str,
   chgbit = 0
   ...
   ENDIF
   IF (chgbit > 0)
    pah_qual_cntr = (pah_qual_cntr+ 1), temp_record->person_alias_hist_qual[pah_qual_cntr].change_bit
     = chgbit, temp_record->person_alias_hist_qual[pah_qual_cntr].item_idx = item_idx,
    temp_record->person_alias_hist_qual[pah_qual_cntr].pm_hist_tracking_id = acm_request->
    person_alias_qual[item_idx].pm_hist_tracking_id, temp_record->person_alias_hist_qual[
    pah_qual_cntr].person_id = acm_request->person_alias_qual[item_idx].person_id, temp_record->
    person_alias_hist_qual[pah_qual_cntr].end_effective_dt_tm = acm_request->person_alias_qual[
    item_idx].end_effective_dt_tm,
    temp_record->person_alias_hist_qual[pah_qual_cntr].beg_effective_dt_tm = acm_request->
    person_alias_qual[item_idx].beg_effective_dt_tm, temp_record->person_alias_hist_qual[
    pah_qual_cntr].transaction_dt_tm = acm_request->person_alias_qual[item_idx].transaction_dt_tm,
    temp_record->person_alias_hist_qual[pah_qual_cntr].person_alias_id = acm_request->
    person_alias_qual[item_idx].person_alias_id,
    temp_record->person_alias_hist_qual[pah_qual_cntr].person_alias_status_cd = acm_request->
    person_alias_qual[item_idx].person_alias_status_cd, temp_record->person_alias_hist_qual[
    pah_qual_cntr].health_card_ver_code = acm_request->person_alias_qual[item_idx].
    health_card_ver_code, temp_record->person_alias_hist_qual[pah_qual_cntr].health_card_province =
    acm_request->person_alias_qual[item_idx].health_card_province,
    temp_record->person_alias_hist_qual[pah_qual_cntr].health_card_type = acm_request->
    person_alias_qual[item_idx].health_card_type, temp_record->person_alias_hist_qual[pah_qual_cntr].
    health_card_expiry_dt_tm = acm_request->person_alias_qual[item_idx].health_card_expiry_dt_tm,
    temp_record->person_alias_hist_qual[pah_qual_cntr].health_card_issue_dt_tm = acm_request->
    person_alias_qual[item_idx].health_card_issue_dt_tm,
    temp_record->person_alias_hist_qual[pah_qual_cntr].alias = acm_request->person_alias_qual[
    item_idx].alias
   ELSE
    reply->person_alias_qual[item_idx].status = 1
   ENDIF
  WITH nocounter
 ;end select
 SET stat = alterlist(temp_record->person_alias_hist_qual,pah_qual_cntr)
 IF (pah_qual_cntr > 0)
  INSERT  FROM person_alias_hist pah,
    (dummyt d  WITH seq = value(pah_qual_cntr))
   SET pah.person_alias_hist_id = seq(person_seq,nextval), pah.pm_hist_tracking_id = temp_record->
    person_alias_hist_qual[d.seq].pm_hist_tracking_id, pah.person_id = temp_record->
    person_alias_hist_qual[d.seq].person_id,
    pah.end_effective_dt_tm = cnvtdatetime(temp_record->person_alias_hist_qual[d.seq].
     end_effective_dt_tm), pah.beg_effective_dt_tm = cnvtdatetime(temp_record->
     person_alias_hist_qual[d.seq].beg_effective_dt_tm), pah.transaction_dt_tm = cnvtdatetime(
     temp_record->person_alias_hist_qual[d.seq].transaction_dt_tm),
    pah.person_alias_id = temp_record->person_alias_hist_qual[d.seq].person_alias_id, pah
    .person_alias_status_cd = temp_record->person_alias_hist_qual[d.seq].person_alias_status_cd, pah
    .health_card_ver_code = temp_record->person_alias_hist_qual[d.seq].health_card_ver_code,
    pah.health_card_province = temp_record->person_alias_hist_qual[d.seq].health_card_province, pah
    .health_card_type = temp_record->person_alias_hist_qual[d.seq].health_card_type, pah
    .health_card_expiry_dt_tm = cnvtdatetime(temp_record->person_alias_hist_qual[d.seq].
     health_card_expiry_dt_tm),
    pah.health_card_issue_dt_tm = cnvtdatetime(temp_record->person_alias_hist_qual[d.seq].
     health_card_issue_dt_tm), pah.alias = temp_record->person_alias_hist_qual[d.seq].alias, pah
    .tracking_bit = tracking_bit,
    pah.change_bit = temp_record->person_alias_hist_qual[d.seq].change_bit, pah.active_ind = 1, pah
    .active_status_cd = reqdata->active_status_cd,
    pah.active_status_dt_tm = cnvtdatetime(curdate,curtime3), pah.active_status_prsnl_id = reqinfo->
    updt_id, pah.updt_dt_tm = cnvtdatetime(curdate,curtime3),
    pah.updt_id = reqinfo->updt_id, pah.updt_applctx = reqinfo->updt_applctx, pah.updt_task = reqinfo
    ->updt_task,
    pah.updt_cnt = updt_cnt
   PLAN (d)
    JOIN (pah)
   WITH nocounter, status(reply->person_alias_qual[temp_record->person_alias_hist_qual[d.seq].
    item_idx].status)
  ;end insert
 ENDIF
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->person_alias_qual[xref->chg[index].idx].status=0))
    ...
    SET table_name = "PERSON_ALIAS_HIST"
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
