CREATE PROGRAM acm_chg_encntr_prsnl_reltn:dba
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
  ...
  SET e.encntr_prsnl_reltn_id = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].
   encntr_prsnl_reltn_id, e.beg_effective_dt_tm = cnvtdatetime(acm_request->encntr_prsnl_reltn_qual[
    xref->chg[d.seq].idx].beg_effective_dt_tm), e.contributor_system_cd =
   IF ((acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   e.encntr_id =
   IF ((acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].encntr_id > 0)) acm_request->
    encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].encntr_idx].
    encntr_id
   ENDIF
   , e.encntr_prsnl_r_cd = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].
   encntr_prsnl_r_cd, e.end_effective_dt_tm = cnvtdatetime(acm_request->encntr_prsnl_reltn_qual[xref
    ->chg[d.seq].idx].end_effective_dt_tm),
   e.expiration_ind = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].expiration_ind, e
   .expire_dt_tm =
   IF ((acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].expire_dt_tm > 0)) cnvtdatetime(
     acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].expire_dt_tm)
   ELSE null
   ENDIF
   , e.free_text_cd = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].free_text_cd,
   e.ft_prsnl_name = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].ft_prsnl_name, e
   .internal_seq = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].internal_seq, e
   .manual_create_by_id = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].
   manual_create_by_id,
   e.manual_create_dt_tm =
   IF ((acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].manual_create_dt_tm > 0))
    cnvtdatetime(acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].manual_create_dt_tm)
   ELSE null
   ENDIF
   , e.manual_create_ind = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].
   manual_create_ind, e.manual_inact_by_id = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].
   idx].manual_inact_by_id,
   e.manual_inact_dt_tm =
   IF ((acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].manual_inact_dt_tm > 0))
    cnvtdatetime(acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].manual_inact_dt_tm)
   ELSE null
   ENDIF
   , e.manual_inact_ind = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].manual_inact_ind,
   e.notification_cd = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].notification_cd,
   e.priority_seq = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].priority_seq, e
   .prsnl_person_id = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].prsnl_person_id, e
   .transaction_dt_tm =
   IF ((acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].transaction_dt_tm > 0))
    cnvtdatetime(acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].transaction_dt_tm)
   ELSE null
   ENDIF
   ,
   e.active_ind = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].active_ind, e
   .active_status_cd = acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].active_status_cd, e
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_prsnl_reltn_id=acm_request->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].
   encntr_prsnl_reltn_id))
  WITH nocounter, status(reply->encntr_prsnl_reltn_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->encntr_prsnl_reltn_qual[xref->chg[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,e.encntr_prsnl_reltn_id,acm_request->encntr_prsnl_reltn_qual[xref->
     chg[t1].idx].encntr_prsnl_reltn_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,e.encntr_prsnl_reltn_id,acm_request->encntr_prsnl_reltn_qual[xref
      ->chg[t1].idx].encntr_prsnl_reltn_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((e.updt_cnt=acm_request->encntr_prsnl_reltn_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->encntr_prsnl_reltn_qual[idx].status = - (1), reply->encntr_prsnl_reltn_qual[idx].
      encntr_prsnl_reltn_id = acm_request->encntr_prsnl_reltn_qual[idx].encntr_prsnl_reltn_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->encntr_prsnl_reltn_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].beg_effective_dt_tm = e.beg_effective_dt_tm
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].contributor_system_cd = e.contributor_system_cd
     ENDIF
     IF (findstring("ENCNTR_ID,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].encntr_id = e.encntr_id
     ENDIF
     IF (findstring("ENCNTR_PRSNL_R_CD,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].encntr_prsnl_r_cd = e.encntr_prsnl_r_cd
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].end_effective_dt_tm = e.end_effective_dt_tm
     ENDIF
     IF (findstring("EXPIRATION_IND,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].expiration_ind = e.expiration_ind
     ENDIF
     IF (findstring("EXPIRE_DT_TM,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].expire_dt_tm = e.expire_dt_tm
     ENDIF
     IF (findstring("FREE_TEXT_CD,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].free_text_cd = e.free_text_cd
     ENDIF
     IF (findstring("FT_PRSNL_NAME,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].ft_prsnl_name = e.ft_prsnl_name
     ENDIF
     IF (findstring("INTERNAL_SEQ,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].internal_seq = e.internal_seq
     ENDIF
     IF (findstring("MANUAL_CREATE_BY_ID,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].manual_create_by_id = e.manual_create_by_id
     ENDIF
     IF (findstring("MANUAL_CREATE_DT_TM,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].manual_create_dt_tm = e.manual_create_dt_tm
     ENDIF
     IF (findstring("MANUAL_CREATE_IND,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].manual_create_ind = e.manual_create_ind
     ENDIF
     IF (findstring("MANUAL_INACT_BY_ID,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].manual_inact_by_id = e.manual_inact_by_id
     ENDIF
     IF (findstring("MANUAL_INACT_DT_TM,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].manual_inact_dt_tm = e.manual_inact_dt_tm
     ENDIF
     IF (findstring("MANUAL_INACT_IND,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].manual_inact_ind = e.manual_inact_ind
     ENDIF
     IF (findstring("NOTIFICATION_CD,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].notification_cd = e.notification_cd
     ENDIF
     IF (findstring("PRIORITY_SEQ,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].priority_seq = e.priority_seq
     ENDIF
     IF (findstring("PRSNL_PERSON_ID,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].prsnl_person_id = e.prsnl_person_id
     ENDIF
     IF (findstring("TRANSACTION_DT_TM,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].transaction_dt_tm = e.transaction_dt_tm
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].active_ind = e.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->encntr_prsnl_reltn_qual[idx].active_status_cd = e.active_status_cd
     ENDIF
