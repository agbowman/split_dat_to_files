CREATE PROGRAM bbt_rpt_crossover:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->autologous_directed = uar_i18ngetmessage(i18nhandle,"autologous_directed",
  "AUTOLOGOUS/DIRECTED CROSSOVER REASONS")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_crossover", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  cv.display, cv.description, cv.active_ind
  FROM code_value cv
  WHERE cv.code_set=1624
  HEAD REPORT
   select_ok_ind = 0
  HEAD PAGE
   ...
   col 41, captions->autologous_directed, row + 2,
