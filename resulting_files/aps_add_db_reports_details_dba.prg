CREATE PROGRAM aps_add_db_reports_details:dba
 RECORD reply(
   1 exception_data[1]
     2 add_cyto_report_control = c1
...
#script
 ...
 SET failed = "F"
 SET error_cnt = 0
 SET reply->exception_data[1].add_cyto_report_control = "S"
 ...
 INSERT  FROM cyto_report_control crc
  SET crc.catalog_cd = request->catalog_cd, crc.report_type_flag = request->report_type_flag, crc
   .endocerv_task_assay_cd = request->endo_task_assay_cd,
   crc.action_task_assay_cd = request->action_task_assay_cd, crc.diagnosis_task_assay_cd = request->
   diag_task_assay_cd, crc.adequacy_task_assay_cd = request->adeq_task_assay_cd,
   crc.clin_info_task_assay_cd = request->clin_info_task_assay_cd, crc.adeq_reason_task_assay_cd =
   request->adeq_reason_task_assay_cd, crc.updt_dt_tm = cnvtdatetime(curdate,curtime),
   crc.updt_id = reqinfo->updt_id, crc.updt_task = reqinfo->updt_task, crc.updt_cnt = 0,
   crc.updt_applctx = reqinfo->updt_applctx
  WITH nocounter
 ;end insert
 IF (curqual=0)
  CALL handle_errors("insert","f","table","cyto_report_control")
  SET reply->exception_data[1].add_cyto_report_control = "F"
 ELSE
  SET reply->exception_data[1].add_cyto_report_control = "S"
 ENDIF
 ...
 ENDIF
#end_of_plans
 IF ((reply->exception_data[1].add_cyto_report_control="S")
