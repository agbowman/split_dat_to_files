CREATE PROGRAM bed_aud_ap_ord_event_reltn:dba
 ...
 DECLARE aprpt = f8 WITH public, noconstant(0.0)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=106
    ...
    AND cv.active_ind=1)
  DETAIL
   ap = cv.code_value, act_type = cv.display
  WITH nocounter
 ;end select
 SELECT INTO "NL:"
  FROM code_value cv
 ...
 IF ((request->skip_volume_check_ind=0))
  SELECT INTO "nl:"
   ...
   PLAN (oc
    WHERE oc.activity_type_cd=ap
     AND oc.catalog_type_cd=lab
     AND oc.activity_subtype_cd=aprpt
     AND oc.active_ind=1)
    JOIN (cver
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("ap_ord_to_evntcode_evntset.csv")
 ENDIF
