CREATE PROGRAM acm_del_encntr_person_reltn:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_person_reltn_id=acm_request->encntr_person_reltn_qual[xref->del[d.seq].idx].
   encntr_person_reltn_id))
  WITH nocounter, status(reply->encntr_person_reltn_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->encntr_person_reltn_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->encntr_person_reltn_qual[xref->del[index].idx].status=0))
