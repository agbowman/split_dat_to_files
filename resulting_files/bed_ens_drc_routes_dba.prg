CREATE PROGRAM bed_ens_drc_routes:dba
 ...
 FREE SET tobekeptunchanged
 RECORD tobekeptunchanged(
   1 qual[*]
     2 code_value = f8
 )
 ...
 DECLARE dp = i2 WITH protect, constant(0)
 DECLARE dpl = i2 WITH protect, constant(1)
 DECLARE unchanged = i2 WITH protect, constant(0)
 DECLARE add = i2 WITH protect, constant(1)
 DECLARE del = i2 WITH protect, constant(2)
 DECLARE ridx = i4 WITH protect, noconstant(1)
 DECLARE pidx = i4 WITH protect, noconstant(1)
 DECLARE existedid = f8 WITH protect, noconstant(0.0)
 DECLARE routetobeadded = f8 WITH protect, noconstant(0.0)
 DECLARE keep_cnt = i2 WITH protect, noconstant(0)
 DECLARE add_cnt = i2 WITH protect, noconstant(0)
 DECLARE delete_cnt = i2 WITH protect, noconstant(0)
 DECLARE value1_string = vc WITH protect, noconstant("")
 DECLARE ensureroutestopremises(drc_premise_id=f8) = null
 DECLARE updatedrcpremise(drc_premise_id=f8,code_value=f8) = null
 DECLARE insertdrcpremise(code_value=f8) = f8
 DECLARE createblankpremiserow(dummyvar=i2) = f8
 DECLARE cleardrcpremisetable(drc_premise_id=f8) = null
 DECLARE insertintodrcpremiselist(drc_premise_id=f8,code_value=f8) = null
 DECLARE inactivatefromdrcpremiselist(drc_premise_id=f8,code_value=f8) = null
 DECLARE reactivatefromdrcpremiselist(drc_premise_list_id=f8) = null
 DECLARE checkifinactiverowexistsindpl(drc_premise_id=f8,code_value=f8) = f8
 DECLARE get_next_seq(dummyvar=i2) = f8
 DECLARE setdoserangecustomindafterpremiseupdate(premise_id=f8) = null
 FOR (ridx = 1 TO size(request->routes,5))
   CASE (request->routes[ridx].action_flag)
    OF unchanged:
     SET keep_cnt = (keep_cnt+ 1)
     SET stat = alterlist(tobekeptunchanged->qual,keep_cnt)
     SET tobekeptunchanged->qual[keep_cnt].code_value = request->routes[ridx].route_code_value
    OF add:
     SET add_cnt = (add_cnt+ 1)
     SET stat = alterlist(tobeadded->qual,add_cnt)
     SET tobeadded->qual[add_cnt].code_value = request->routes[ridx].route_code_value
    OF del:
     SET delete_cnt = (delete_cnt+ 1)
     SET stat = alterlist(tobedeleted->qual,delete_cnt)
     SET tobedeleted->qual[delete_cnt].code_value = request->routes[ridx].route_code_value
   ENDCASE
 ENDFOR
 FOR (pidx = 1 TO size(request->premises,5))
   CALL ensureroutestopremises(request->premises[pidx].drc_premise_id,request->premises[pidx].
    location_flag)
 ENDFOR
 SUBROUTINE ensureroutestopremises(drc_premise_id,location_flag)
  DECLARE blankpremiserow = f8
  IF (drc_premise_id=0.0)
   IF (add_cnt > 1)
    SET blankpremiserow = createblankpremiserow(0)
    CALL insert_version_row_drc_prem_ver_table(blankpremiserow)
    FOR (ridx = 1 TO add_cnt)
     CALL insertintodrcpremiselist(blankpremiserow,tobeadded->qual[ridx].code_value)
     CALL insert_version_row_drc_prem_list_ver_table(blankpremiserow,tobeadded->qual[ridx].code_value
      )
    ENDFOR
   ELSE
    SET newpremiserow = insertdrcpremise(tobeadded->qual[1].code_value)
    CALL insert_version_row_drc_prem_ver_table(newpremiserow)
   ENDIF
  ELSE
   CASE (location_flag)
    OF dp:
     IF (((keep_cnt+ add_cnt)=1))
      CALL updatedrcpremise(drc_premise_id,tobeadded->qual[1].code_value)
      CALL insert_version_row_drc_prem_ver_table(drc_premise_id)
     ELSE
      CALL cleardrcpremisetable(drc_premise_id)
      CALL insert_version_row_drc_prem_ver_table(drc_premise_id)
      FOR (ridx = 1 TO add_cnt)
       CALL insertintodrcpremiselist(drc_premise_id,tobeadded->qual[ridx].code_value)
       CALL insert_version_row_drc_prem_list_ver_table(drc_premise_id,tobeadded->qual[ridx].
        code_value)
      ENDFOR
      IF (keep_cnt=1)
       CALL insertintodrcpremiselist(drc_premise_id,tobekeptunchanged->qual[1].code_value)
       CALL insert_version_row_drc_prem_list_ver_table(drc_premise_id,tobekeptunchanged->qual[1].
        code_value)
      ENDIF
     ENDIF
    OF dpl:
     IF (delete_cnt > 0)
      FOR (ridx = 1 TO delete_cnt)
       CALL inactivatefromdrcpremiselist(drc_premise_id,tobedeleted->qual[ridx].code_value)
       CALL insert_version_row_drc_prem_list_ver_table(drc_premise_id,tobedeleted->qual[ridx].
        code_value)
      ENDFOR
     ENDIF
     IF (add_cnt > 0)
      FOR (ridx = 1 TO add_cnt)
        SET routetobeadded = tobeadded->qual[ridx].code_value
        SET existedid = checkifinactiverowexistsindpl(drc_premise_id,routetobeadded)
        IF (existedid > 0.0)
         CALL reactivatefromdrcpremiselist(existedid)
        ELSE
         CALL insertintodrcpremiselist(drc_premise_id,routetobeadded)
        ENDIF
        CALL insert_version_row_drc_prem_list_ver_table(drc_premise_id,routetobeadded)
      ENDFOR
     ENDIF
   ENDCASE
  ENDIF
 END ;Subroutine
 SUBROUTINE updatedrcpremise(drc_premise_id,code_value)
   SET value1_string = uar_get_code_display(code_value)
   UPDATE  FROM drc_premise dp
    SET dp.value_unit_cd = 0.0, dp.value1 = code_value, dp.value1_string = value1_string,
    ...
    WITH nocounter
   ;end update
   CALL bederrorcheck("Error 001: Error while updating row on drc_premise table.")
   CALL setdoserangecustomindafterpremiseupdate(drc_premise_id)
 END ;Subroutine
 SUBROUTINE insertdrcpremise(code_value)
   DECLARE newpremiseid = f8
   SET newpremiseid = get_next_seq(0)
   SET value1_string = uar_get_code_display(code_value)
   INSERT  FROM drc_premise dp
    SET dp.active_ind = 1, dp.dose_range_check_id = request->dose_range_check_id, dp
     .parent_premise_id = request->parent_premise_id,
     dp.drc_premise_id = newpremiseid, dp.premise_type_flag = 2, dp.value_unit_cd = 0.0,
     dp.value1 = code_value, dp.value1_string = value1_string, dp.value2 = 0.0,
     dp.value2_string = "0", dp.relational_operator_flag = 0, dp.value_type_flag = 3,
     dp.updt_applctx = reqinfo->updt_applctx, dp.updt_cnt = 0, dp.updt_dt_tm = cnvtdatetime(curdate,
      curtime3),
     dp.updt_id = reqinfo->updt_id, dp.updt_task = reqinfo->updt_task
    WITH nocounter
   ;end insert
   RETURN(newpremiseid)
   CALL bederrorcheck("Error 002: Error while updating row on drc_premise table.")
 END ;Subroutine
 SUBROUTINE createblankpremiserow(dummyvar)
   SET newpremiseid = get_next_seq(0)
   INSERT  FROM drc_premise dp
    SET dp.active_ind = 1, dp.drc_premise_id = newpremiseid, dp.dose_range_check_id = request->
     dose_range_check_id,
     dp.parent_premise_id = request->parent_premise_id, dp.premise_type_flag = 2, dp.value_unit_cd =
     0.0,
     dp.value1 = 0.0, dp.value1_string = "0", dp.value2 = 0.0,
     dp.value2_string = "0", dp.relational_operator_flag = 8, dp.value_type_flag = 4,
     dp.updt_applctx = reqinfo->updt_applctx, dp.updt_cnt = 0, dp.updt_dt_tm = cnvtdatetime(curdate,
      curtime3),
     dp.updt_id = reqinfo->updt_id, dp.updt_task = reqinfo->updt_task
    WITH nocounter
   ;end insert
   CALL bederrorcheck("Error 003: Error while inserting route row on drc_premise table.")
   RETURN(newpremiseid)
 END ;Subroutine
 SUBROUTINE cleardrcpremisetable(drc_premise_id)
   UPDATE  FROM drc_premise dp
    SET dp.value_unit_cd = 0.0, dp.value1 = 0.0, dp.value1_string = "0",
     dp.relational_operator_flag = 8, dp.value_type_flag = 4, dp.updt_applctx = reqinfo->updt_applctx,
    ...
    WITH nocounter
   ;end update
   CALL bederrorcheck("Error 004: Error while updating row on drc_premise table.")
   CALL setdoserangecustomindafterpremiseupdate(drc_premise_id)
 END ;Subroutine
 SUBROUTINE insertintodrcpremiselist(drc_premise_id,code_value)
   DECLARE next_seq = f8 WITH protect, noconstant(0.0)
   SET next_seq = get_next_seq(0)
   INSERT  FROM drc_premise_list dpl
    SET dpl.drc_premise_list_id = next_seq, dpl.drc_premise_id = drc_premise_id, dpl
     .parent_entity_name = "CODE_VALUE",
     dpl.parent_entity_id = code_value, dpl.active_ind = 1, dpl.updt_applctx = reqinfo->updt_applctx,
     dpl.updt_cnt = 0, dpl.updt_dt_tm = cnvtdatetime(curdate,curtime3), dpl.updt_id = reqinfo->
     updt_id,
     dpl.updt_task = reqinfo->updt_task
    WITH nocounter
   ;end insert
   CALL bederrorcheck("Error 005: Error while updating row on drc_premise_list table.")
 END ;Subroutine
 SUBROUTINE inactivatefromdrcpremiselist(drc_premise_id,code_value)
  UPDATE  FROM drc_premise_list dpl
   SET dpl.active_ind = 0, dpl.updt_applctx = reqinfo->updt_applctx, dpl.updt_cnt = (dpl.updt_cnt+ 1),
    dpl.updt_dt_tm = cnvtdatetime(curdate,curtime3), dpl.updt_id = reqinfo->updt_id, dpl.updt_task =
    reqinfo->updt_task
   WHERE dpl.active_ind=1
    AND dpl.parent_entity_name="CODE_VALUE"
    AND dpl.drc_premise_id=drc_premise_id
    AND dpl.parent_entity_id=code_value
   WITH nocounter
  ;end update
  CALL bederrorcheck("Error 006: Error when inactivating an entry in drc_premise_list table.")
 END ;Subroutine
 SUBROUTINE reactivatefromdrcpremiselist(existedid)
  UPDATE  FROM drc_premise_list dpl
   SET dpl.active_ind = 1, dpl.updt_applctx = reqinfo->updt_applctx, dpl.updt_cnt = (dpl.updt_cnt+ 1),
    dpl.updt_dt_tm = cnvtdatetime(curdate,curtime3), dpl.updt_id = reqinfo->updt_id, dpl.updt_task =
    reqinfo->updt_task
   WHERE dpl.drc_premise_list_id=existedid
   WITH nocounter
  ;end update
  CALL bederrorcheck("Error 007: Error when re-activating an entry in drc_premise_list table.")
 END ;Subroutine
 SUBROUTINE checkifinactiverowexistsindpl(drc_premise_id,code_value)
   DECLARE result = f8 WITH protect, noconstant(0.0)
   SELECT INTO "nl:"
    dpl.drc_premise_list_id
    FROM drc_premise_list dpl
    WHERE dpl.drc_premise_id=drc_premise_id
     AND dpl.parent_entity_id=code_value
     AND dpl.parent_entity_name="CODE_VALUE"
     AND dpl.active_ind=0
    DETAIL
     result = dpl.drc_premise_list_id
    WITH nocounter
   ;end select
   CALL bederrorcheck(
    "Error 008: Error when checking existance of drc_premise_id in drc_premise_list table.")
   RETURN(result)
 END ;Subroutine
 SUBROUTINE setdoserangecustomindafterpremiseupdate(premise_id)
   DECLARE drc_premise_id = f8 WITH protect, noconstant(0)
   SELECT
    dp.parent_premise_id
    FROM drc_premise dp
    WHERE dp.drc_premise_id=premise_id
    DETAIL
     drc_premise_id = dp.parent_premise_id
    WITH nocounter
   ;end select
   UPDATE  FROM drc_dose_range ddr
    ...
    WITH nocounter
   ;end update
   CALL bederrorcheck("Error 009: Error modifying dose range.")
   CALL insert_ver_row_dose_range_after_premise_update(drc_premise_id)
 END ;Subroutine
 SUBROUTINE get_next_seq(dummyvar)
   SET next_seq = 0.0
   SELECT INTO "nl:"
    number = seq(drc_seq,nextval)
    FROM dual
    DETAIL
     next_seq = cnvtreal(number)
    WITH counter
   ;end select
   CALL bederrorcheck("Error 011: Error getting a new seq")
   RETURN(next_seq)
 END ;Subroutine
#exit_script
