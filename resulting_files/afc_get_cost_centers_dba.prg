CREATE PROGRAM afc_get_cost_centers:dba
 DECLARE versionnbr = vc
 SET versionnbr = "000"
 CALL echo(build("AFC_GET_COST_CENTERS Version: ",versionnbr))
 RECORD reply(
   1 cost_center[*]
     2 cost_center_cd = f8
 ...
 IF (logicaldomainsinuse)
  ...
  ENDIF
 ENDIF
 IF ( NOT (logicaldomainsinuse))
  SET codesetrequest->objarray[1].codeset = 14058
  EXECUTE pft_fetch_code_set  WITH replace("REQUEST",codesetrequest), replace("REPLY",codesetreply)
  IF ((codesetreply->status_data.status="S"))
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(size(codesetreply->objarray,5)))
    ORDER BY codesetreply->objarray[d.seq].display
    DETAIL
     count1 += 1
     IF (mod(count1,10)=1)
      stat = alterlist(reply->cost_center,(count1+ 9))
     ENDIF
     reply->cost_center[count1].cost_center_cd = codesetreply->objarray[d.seq].code
    WITH nocounter
   ;end select
   SET stat = alterlist(reply->cost_center,count1)
  ELSE
   SET reply->status_data.status = codesetreply->status_data.status
   GO TO end_program
  ENDIF
 ELSE
  SELECT INTO "nl:"
   ...
   PLAN (c)
    ...
    WHERE o.organization_id=c.organization_id
     ...
     AND o.active_ind=1)
    JOIN (cv
    WHERE cv.code_value=c.cost_center_cd)
   ORDER BY cv.display
   HEAD cv.display
    count1 += 1
    IF (mod(count1,10)=1)
     stat = alterlist(reply->cost_center,(count1+ 9))
    ENDIF
    reply->cost_center[count1].cost_center_cd = cv.code_value
   DETAIL
    null
   WITH nocounter
  ;end select
 ...
 ENDIF
#end_program
 FREE SET codesetrequest
 FREE SET codesetreply
