CREATE PROGRAM acm_write_address:dba
 ...
 DECLARE ubound = i4 WITH protect, noconstant(reply->address_qual_cnt)
 SET stat = alterlist(reply->address_qual,reply->address_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->address_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->address_qual[index].address_id = acm_request->address_qual[index].address_id
     SET reply->address_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_address
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->act_cnt > 0))
  EXECUTE acm_act_address
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_address
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_address
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
