CREATE PROGRAM bh_rc_rl_an_casual:dba
 CALL echo("***** bh_rc_rl_an_casual.prg - 677918 *****")
 ...
 SUBROUTINE filltemprec(null)
   FOR (lchgcnt = 1 TO size(request->integer_params,5))
     CASE (request->integer_params[lchgcnt].key_txt)
      OF "MAJOR_CASUALTY_ANNUAL":
       SET tmprec->major_casualty_monthly = (request->integer_params[lchgcnt].integer_value/ 12)
     ENDCASE
   ENDFOR
 END ;Subroutine
 DECLARE createreply(null) = null
 SUBROUTINE createreply(null)
  CALL addtoreplyintegers("MAJOR_CASUALTY_MONTHLY",tmprec->major_casualty_monthly)
