CREATE PROGRAM dm2_fix_notnull_columns:dba
 ...
 ENDIF
 DECLARE dfnc_logfile_name = vc WITH protect, noconstant("dm2_fnc_")
 DECLARE dfnc_tbl_cnt = i4 WITH protect, noconstant(0)
 DECLARE dfnc_col_cnt = i4 WITH protect, noconstant(0)
 DECLARE dfnc_cons_cnt = i4 WITH protect, noconstant(0)
 DECLARE dfnc_tbl2_cnt = i4 WITH protect, noconstant(0)
 DECLARE dfnc_col2_cnt = i4 WITH protect, noconstant(0)
 DECLARE dfnc_cons2_cnt = i4 WITH protect, noconstant(0)
 DECLARE dfnc_tmp_str = vc WITH public, noconstant("NOT_SET")
 FREE RECORD dfnc_rs
 RECORD dfnc_rs(
   1 tbl_cnt = i4
   1 tbls[*]
 ...
 IF (check_logfile(dfnc_logfile_name,".log","DM2_FIX_NOTNULL_COLUMNS ...") != 1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Removing invalid NOT NULL check constraints."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  u.table_name, u.column_name, uc.constraint_name,
  uc.validated
  ...
  PLAN (u
   ...
   WHERE uc.table_name=ucc.table_name
    ...
    AND ucc.column_name=u.column_name
    AND ucc.position=null)
  ORDER BY u.table_name, u.column_name
  HEAD REPORT
   dfnc_tbl_cnt = 0
  HEAD u.table_name
   dfnc_tbl_cnt = (dfnc_tbl_cnt+ 1), dfnc_rs->tbl_cnt = dfnc_tbl_cnt
   IF (mod(dfnc_tbl_cnt,10)=1)
    stat = alterlist(dfnc_rs->tbls,(dfnc_tbl_cnt+ 9))
   ENDIF
   dfnc_rs->tbls[dfnc_tbl_cnt].table_name = u.table_name, dfnc_col_cnt = 0
  HEAD u.column_name
   dfnc_col_cnt = (dfnc_col_cnt+ 1)
   IF (mod(dfnc_col_cnt,10)=1)
    stat = alterlist(dfnc_rs->tbls[dfnc_tbl_cnt].cols,(dfnc_col_cnt+ 9))
   ENDIF
   dfnc_rs->tbls[dfnc_tbl_cnt].cols[dfnc_col_cnt].column_name = u.column_name, dfnc_cons_cnt = 0
  DETAIL
   IF (uc.search_condition="*NOT NULL*")
    dfnc_cons_cnt = (dfnc_cons_cnt+ 1), stat = alterlist(dfnc_rs->tbls[dfnc_tbl_cnt].cols[
     dfnc_col_cnt].cons,dfnc_cons_cnt), dfnc_rs->tbls[dfnc_tbl_cnt].cols[dfnc_col_cnt].cons[
    dfnc_cons_cnt].constraint_name = uc.constraint_name
   ENDIF
  FOOT  u.column_name
   dfnc_rs->tbls[dfnc_tbl_cnt].cols[dfnc_col_cnt].cons_cnt = dfnc_cons_cnt
  FOOT  u.table_name
   dfnc_rs->tbls[dfnc_tbl_cnt].col_cnt = dfnc_col_cnt, stat = alterlist(dfnc_rs->tbls[dfnc_tbl_cnt].
    cols,dfnc_rs->tbls[dfnc_tbl_cnt].col_cnt)
  FOOT REPORT
   dfnc_rs->tbl_cnt = dfnc_tbl_cnt, stat = alterlist(dfnc_rs->tbls,dfnc_rs->tbl_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(dfnc_rs)
 ENDIF
 IF (curqual=0)
  SET readme_data->status = "S"
  GO TO exit_program
 ENDIF
 FOR (dfnc_tbl2_cnt = 1 TO dfnc_rs->tbl_cnt)
   FOR (dfnc_col2_cnt = 1 TO dfnc_rs->tbls[dfnc_tbl2_cnt].col_cnt)
     IF ((dfnc_rs->tbls[dfnc_tbl2_cnt].cols[dfnc_col2_cnt].cons_cnt > 0))
      SET dfnc_tmp_str = concat("rdb asis (^alter table ",dfnc_rs->tbls[dfnc_tbl2_cnt].table_name,
       " MODIFY ( ",dfnc_rs->tbls[dfnc_tbl2_cnt].cols[dfnc_col2_cnt].column_name," NOT NULL ) ^) go")
      IF (dm2_push_cmd(dfnc_tmp_str,1)=0)
       GO TO exit_program
      ENDIF
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
       GO TO exit_program
      ENDIF
      FOR (dfnc_cons2_cnt = 1 TO dfnc_rs->tbls[dfnc_tbl2_cnt].cols[dfnc_col2_cnt].cons_cnt)
        SET dfnc_tmp_str = concat("rdb asis (^alter table ",dfnc_rs->tbls[dfnc_tbl2_cnt].table_name,
         " drop constraint ",dfnc_rs->tbls[dfnc_tbl2_cnt].cols[dfnc_col2_cnt].cons[dfnc_cons2_cnt].
         constraint_name,"^) go")
        IF (dm2_push_cmd(dfnc_tmp_str,1)=0)
         GO TO exit_program
        ENDIF
   ...
   ENDFOR
 ENDFOR
 SET dm_err->eproc = "Verifying that all not null columns have been corrected."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  u.table_name, u.column_name, uc.constraint_name,
  uc.validated
 ...
 IF (curqual > 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Readme failed: Invalid not null check constraints remain."
  GO TO exit_program
 ENDIF
#exit_program
 SET dm_err->eproc = "Finished removing invalid NOT NULL check constraints."
 IF ((dm_err->err_ind=1))
  SET readme_data->status = "F"
 ...
 ENDIF
 CALL final_disp_msg(dfnc_logfile_name)
END GO
