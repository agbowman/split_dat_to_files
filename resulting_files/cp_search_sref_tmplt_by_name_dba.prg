CREATE PROGRAM cp_search_sref_tmplt_by_name:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Template Category Prefix" = "",
  "Template Description" = ""
  WITH outdev, temp_cat, temp_desc
 ...
 RECORD record_data(
   ...
   1 qual[*]
     ...
     2 clin_ident = vc
     2 version_nbr = i4
     2 dd_sref_tmpl_instance_ident = vc
 ...
 SUBROUTINE main(null)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE pathwayname = vc WITH protect, noconstant(trim(piece( $TEMP_DESC," ",1,"Not Found")))
   CALL searchtemplate( $TEMP_DESC)
   IF ((record_data->cnt < 1))
    IF (pathwayname != "Not Found")
     CALL searchtemplate(pathwayname)
    ENDIF
   ENDIF
   CALL log_message(build("Exit main(), Elapsed time in seconds:",((curtime3 - begin_curtime3)/ 100.0
     )),log_level_debug)
 END ;Subroutine
 SUBROUTINE (searchtemplate(search_string=vc) =null WITH protect)
   DECLARE templatecatpat = vc WITH protect, constant(concat( $TEMP_CAT,"*"))
   DECLARE templatedescpat = vc WITH protect, constant(concat("*",cnvtupper(search_string),"*"))
   ...
   DECLARE tempcntr = i4 WITH protect, noconstant(0)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (dd
     WHERE dd.cln_ident=patstring(templatecatpat)
      AND cnvtupper(dd.display)=patstring(templatedescpat))
    ORDER BY dd.display, dd.dd_sref_tmpl_instance_ident
    HEAD dd.dd_sref_tmpl_instance_ident
     tloc = 0, tlsize = size(record_data->qual,5), tloc = locateval(tlindx,1,tlsize,dd.cln_ident,
      encodeinternationalcharacters(record_data->qual[tlindx].clin_ident)),
     ...
     clniduppertemplatesubstringpattern = cnvtupper(trim(substring(clnidposition,clnidlength,
        decodeinternationalcharacters(dd.cln_ident)))), templatedisplayupper = cnvtupper(trim(
       decodeinternationalcharacters(dd.display),8))
     ...
     ENDIF
     IF (tloc > 0
      AND (record_data->qual[tloc].version_nbr < dd.version_nbr))
      record_data->qual[tloc].clin_ident = decodeinternationalcharacters(dd.cln_ident), record_data->
      qual[tloc].dd_sref_tmpl_instance_ident = decodeinternationalcharacters(dd
       .dd_sref_tmpl_instance_ident), record_data->qual[tloc].display = decodeinternationalcharacters
      (dd.display),
      record_data->qual[tloc].version_nbr = dd.version_nbr, record_data->qual[tloc].update_date_time
       = cnvtdatetimeutc(dd.updt_dt_tm,1,curtimezonesys)
      CASE (templatedisplayupper)
       OF patstring(clniduppertemplatesubstringpattern,0):
        record_data->qual[tloc].doc_type = "IMPORTED_DOC"
       ELSE
        record_data->qual[tloc].doc_type = "ASSOCIATED_DOC"
      ENDCASE
     ELSEIF (tloc=0)
      tempcntr += 1
      IF (tempcntr > size(record_data->qual,5))
       stat = alterlist(record_data->qual,(tempcntr+ 10))
      ENDIF
      record_data->qual[tempcntr].clin_ident = decodeinternationalcharacters(dd.cln_ident),
      record_data->qual[tempcntr].dd_sref_tmpl_instance_ident = decodeinternationalcharacters(dd
       .dd_sref_tmpl_instance_ident), record_data->qual[tempcntr].display =
      decodeinternationalcharacters(dd.display),
      record_data->qual[tempcntr].version_nbr = dd.version_nbr, record_data->qual[tempcntr].
      update_date_time = cnvtdatetimeutc(dd.updt_dt_tm,1,curtimezonesys)
      CASE (templatedisplayupper)
       OF patstring(clniduppertemplatesubstringpattern,0):
        record_data->qual[tempcntr].doc_type = "IMPORTED_DOC"
       ELSE
        record_data->qual[tempcntr].doc_type = "ASSOCIATED_DOC"
      ENDCASE
     ENDIF
    FOOT REPORT
     record_data->cnt = tempcntr, stat = alterlist(record_data->qual,record_data->cnt)
    WITH nocounter
   ;end select
 END ;Subroutine
#exit_script
