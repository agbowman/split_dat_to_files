CREATE PROGRAM afc_ens_bill_item_groups:dba
 ...
 ENDIF
 SET hafc_ens_bill_item_groups = 0
 SET istatus = 0
 SET serrmsg = fillstring(132," ")
 SET ierrcode = error(serrmsg,1)
 SET reply->status_data.status = "F"
 SET table_name = "BILL_ITEM_GROUPS"
 IF ((request->bill_item_groups_qual > 0))
  FOR (inx0 = 1 TO request->bill_item_groups_qual)
    CASE (request->bill_item_groups[inx0].action_type)
     OF "ADD":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_add_bill_item_groups
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "UPT":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_upt_bill_item_groups
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "DEL":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_del_bill_item_groups
      IF (failed != false)
       GO TO check_error
      ENDIF
     ELSE
