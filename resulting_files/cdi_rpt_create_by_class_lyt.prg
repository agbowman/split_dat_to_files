CREATE PROGRAM cdi_rpt_create_by_class_lyt
 PROMPT
 ...
 EXECUTE cdi_rpt_create_by_class_drvr  $BEGINDATE,  $ENDDATE,  $BATCHCLASS
 ...
 DECLARE headbatch_lyt_batch_details_usernamesection(ncalc=i2) = f8 WITH protect
 ...
 DECLARE footbatch_lyt_batch_details_usernamesection(ncalc=i2) = f8 WITH protect
 ...
 SUBROUTINE batch_query(dummy)
   SELECT INTO "NL:"
    ...
    HEAD REPORT
     ...
     batch_lyt_batch_details_documentscreated,
     _d3 = batch_lyt_batch_details_documentsdeleted, _fenddetail = ((rptreport->m_pageheight -
     rptreport->m_marginbottom) - footpagesection(rpt_calcheight)), dummy_val = headreportsection(
      rpt_render),
     grand_total_created = 0, grand_total_deleted = 0, total_created_per_user = 0,
    ...
    DETAIL
     ...
     batch_lyt_batch_details_documentsdeleted), grand_total_created = (grand_total_created+
     batch_lyt_batch_details_documentscreated),
     total_deleted_per_user = (total_deleted_per_user+ batch_lyt_batch_details_documentsdeleted),
     total_created_per_user = (total_created_per_user+ batch_lyt_batch_details_documentscreated)
    FOOT  batch_lyt_batch_details_username
     _fdrawheight = footbatch_lyt_batch_details_usernamesection(rpt_calcheight)
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(concat(uar_i18ngetmessage(_hi18nhandle,
        ...
        "HeadReportSection_title1","Documents Created and Deleted for Batch Class")),char(0)))
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2('for Batch Class "',
        $BATCHCLASS,'"'),char(0)))
 ...
 SUBROUTINE headpagesectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "HeadPageSection_FieldName01","Documents Deleted"),char(0)))
    ENDIF
    SET rptsd->m_y = (offsety+ 0.000)
 ...
 SUBROUTINE footbatch_lyt_batch_details_usernamesectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.260
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(total_created_per_user,char(0)))
    ENDIF
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 5.500)
    SET rptsd->m_width = 1.750
 ...
 SUBROUTINE footreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.250
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(grand_total_created,char(0)))
    ENDIF
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CDI_RPT_CREATE_BY_CLASS_LYT"
