CREATE PROGRAM dm_rmc_code_value:dba
 ...
 DECLARE always_get_target_formula(i_cd_val=f8) = f8 WITH public
 DECLARE always_get_target_view_def(i_cd_val=f8) = f8 WITH public
 DECLARE dupind_get_target_mammo_ltr(dgtm_cd_val=f8) = f8 WITH public
 DECLARE get_target_event_set_name(gtesn_cv=f8) = f8 WITH public
 DECLARE sbr_err_val = i4
 DECLARE inc_prelink = vc
 DECLARE inc_postlink = vc
 DECLARE dmcv_parser_cmd = vc
 DECLARE get_parent_drdm_row = i2
 ...
 SUBROUTINE drcm_get_pk_str(dgps_rec,dgps_tab_name)
   ...
   RETURN(dgps_ret_str)
 END ;Subroutine
 IF (validate(request->sub_str,"-1")="=-1")
  FREE RECORD request
  RECORD request(
    1 sub_str = vc
  )
 ENDIF
 IF (validate(reply->return_msg,- (9999)) < 0)
  FREE RECORD reply
  RECORD reply(
    1 return_msg = f8
  )
 ENDIF
 DECLARE dmcv_ret = f8
 SET dmcv_parser_cmd = concat("set dmcv_ret = ",request->sub_str," go")
 CALL parser(dmcv_parser_cmd,1)
 SET reply->return_msg = dmcv_ret
 SUBROUTINE get_target_location_cd(loc_cd)
   ...
   DECLARE gtl_parent_drdm_row = i2 WITH protect, noconstant(0)
   DECLARE gtl_nomv06_msg = vc WITH protect, noconstant("")
   ...
   IF (mult_cnt=0)
    IF (unknown_ind=0)
     SET gtl_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
     CASE (rs_0619->from_values.cdf_meaning)
      OF "NURSEUNIT":
      OF "AMBULATORY":
       SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = concat("CODE_VALUE of ",trim(
         cnvtstring(rs_0619->from_values.code_value,20,1))," with a CDF_MEANING of ",rs_0619->
        from_values.cdf_meaning," in SOURCE was not present in the NURSE_UNIT table in source.")
      OF "ROOM":
       SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = concat("CODE_VALUE of ",trim(
         cnvtstring(rs_0619->from_values.code_value,20,1))," with a CDF_MEANING of ",rs_0619->
        from_values.cdf_meaning," in SOURCE was not present in the ROOM table in source.")
      OF "BED":
       SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = concat("CODE_VALUE of ",trim(
         cnvtstring(rs_0619->from_values.code_value,20,1))," with a CDF_MEANING of ",rs_0619->
        from_values.cdf_meaning," in SOURCE was not present in the BED table in source.")
      ELSE
       SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = concat("CODE_VALUE of ",trim(
         cnvtstring(rs_0619->from_values.code_value,20,1))," with a CDF_MEANING of ",rs_0619->
        from_values.cdf_meaning," in SOURCE did not have a parent CDF_MEANING of ",
        par_cdf)
       IF (alt_par_cdf > " ")
        SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = concat(drdm_chg->log[
         gtl_parent_drdm_row].chg_log_reason_txt," or ",alt_par_cdf)
       ENDIF
       SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = concat(drdm_chg->log[
        gtl_parent_drdm_row].chg_log_reason_txt,
        " in the CODE_VALUE table found by looking at the PARENT_LOC_CD column on the LOCATION_GROUP table."
        )
     ENDCASE
     RETURN(- (17))
    ELSEIF (unknown_ind=1)
     SET mult_cnt = 1
     SET stat = alterlist(mult_loc->qual,mult_cnt)
    ENDIF
   ENDIF
   FOR (mult_loop = 1 TO mult_cnt)
    IF ((mult_loc->qual[mult_loop].src_cd1 > 0))
     SET trans_ref = select_merge_translate(cnvtstring(mult_loc->qual[mult_loop].src_cd1,20,1),
      "CODE_VALUE")
     IF (isnumeric(trans_ref))
     ...
     ELSE
      ...
      IF (rpt_missing="ORPHAN")
       ...
       CASE (rs_0619->from_values.cdf_meaning)
        OF "NURSEUNIT":
        OF "AMBULATORY":
         SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt =
         "Column NURSE_UNIT.LOC_BUILDING_CD"
        OF "ROOM":
         SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = "Column ROOM.LOC_NURSE_UNIT_CD"
        OF "BED":
         SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = "Column BED.LOC_ROOM_CD"
        ELSE
         SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt =
         "Column LOCATION_GROUP.PARENT_LOC_CD"
       ENDCASE
       SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = concat(drdm_chg->log[
        gtl_parent_drdm_row].chg_log_reason_txt," with a value of ",trim(cnvtstring(mult_loc->qual[
          mult_loop].src_cd1,20,1))," does not exist in SOURCE on parent table CODE_VALUE.")
       RETURN(- (21))
      ENDIF
      IF (rpt_missing IN ("NOMV*", "CHLD*"))
       SET drdm_mini_loop_status = rpt_missing
       RETURN(- (22))
      ENDIF
     ENDIF
    ELSE
    ...
    IF ((mult_loc->qual[mult_loop].src_cd2 > 0))
     SET trans_ref = select_merge_translate(cnvtstring(mult_loc->qual[mult_loop].src_cd2,20,1),
      "CODE_VALUE")
     IF (isnumeric(trans_ref))
     ...
     ELSE
      ...
      IF (rpt_missing="ORPHAN")
       ...
       CASE (rs_0619->from_values.cdf_meaning)
        ...
        OF "AMBULATORY":
         SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt =
         "Column NURSE_UNIT.LOC_FACILITY_CD"
        ELSE
         SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt =
         "Column LOCATION_GROUP.ROOT_LOC_CD"
       ENDCASE
       SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = concat(drdm_chg->log[
        gtl_parent_drdm_row].chg_log_reason_txt," with a value of ",trim(cnvtstring(mult_loc->qual[
          mult_loop].src_cd1,20,1))," does not exist in SOURCE on parent table CODE_VALUE.")
     ...
     IF ((mult_loc->qual[mult_loop].trans_ind1=1)
      ...
      CASE (rs_0619->from_values.cdf_meaning)
       ...
       OF "AMBULATORY":
        ...
        SET gtl_nomv06_msg = concat(" CODE_SET = 220 and DISPLAY = ",rs_0619->to_values.display,
         " AND DISPLAY_KEY = ",rs_0619->to_values.display_key," AND CDF_MEANING = ",
         rs_0619->to_values.cdf_meaning,
         " AND CODE_VALUE is joined to the NURSE_UNIT table on LOCATION_CD to compare the ",
         "LOC_BUILDING_CD AND LOC_FACILITY_CD values between source and target.")
       OF "ROOM":
        SET gtl_spec_q_ind = 1
        ...
        SET gtl_nomv06_msg = concat(" CODE_SET = 220 and DISPLAY = ",rs_0619->to_values.display,
         " AND DISPLAY_KEY = ",rs_0619->to_values.display_key," AND CDF_MEANING = ",
         rs_0619->to_values.cdf_meaning," AND DESCRIPTION = ",rs_0619->to_values.description,
         " AND CODE_VALUE is joined to the ROOM table on LOCATION_CD to compare the ",
         "LOC_NURSE_UNIT_CD value between source and target.")
       OF "BED":
        SET gtl_spec_q_ind = 1
        ...
        SET gtl_nomv06_msg = concat(" CODE_SET = 220 and DISPLAY = ",rs_0619->to_values.display,
         " AND DISPLAY_KEY = ",rs_0619->to_values.display_key," AND CDF_MEANING = ",
         rs_0619->to_values.cdf_meaning,
         " AND CODE_VALUE is joined to the BED table on LOCATION_CD to compare the ",
         "LOC_ROOM_CD value between source and target.")
       OF "FACILITY":
       OF "CSTRACK":
       OF "CSLOGIN":
        SET gtl_spec_q_ind = 1
       ...
       ELSE
        ...
        ELSE
         ...
         SET gtl_nomv06_msg = concat(" CODE_SET = 220 and DISPLAY = ",rs_0619->to_values.display,
          " AND DISPLAY_KEY = ",rs_0619->to_values.display_key," AND CDF_MEANING = ",
          rs_0619->to_values.cdf_meaning,
          " AND CODE_VALUE is joined to the LOCATION_GROUP table on CHILD_LOC_CD to compare the ",
          "PARENT_LOC_CD value between source and target.")
         SET gtl_addl_cnt = 0
         SET ui_cnt = 0
         ...
         IF (((size(ui_query_eval_rec->qual,5) > 0) OR (mult_loop=mult_cnt)) )
          ...
          ELSEIF ((gtl_eval_ret=- (2)))
           IF (gtl_addl_cnt=size(target_query->addl_stmts,5))
            SET gtl_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
            SET drdm_chg->log[gtl_parent_drdm_row].chg_log_reason_txt = concat(
             "Multiple rows found in the TARGET where ",gtl_nomv06_msg)
            RETURN(- (19))
           ELSE
 ...
 SUBROUTINE get_target_catalog_cd(source_cat_code)
   ...
   ELSEIF ((ui_cat_cd=- (2)))
    ...
    IF ((ui_cat_cd=- (1)))
     RETURN(- (20))
    ELSEIF ((ui_cat_cd=- (2)))
     SET get_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
     SET drdm_chg->log[get_parent_drdm_row].chg_log_reason_txt = concat(
      "Multiple rows found in the TARGET ORDER_CATALOG table where PRIMARY_MNEMONIC is ",tcc_val,
      ". Found when processing CODE_VALUE ",trim(cnvtstring(source_cat_code,20,1)))
     RETURN(- (3))
    ELSEIF ((ui_cat_cd=- (3)))
     RETURN(0)
    ELSE
   ...
   ELSEIF ((ui_es_cd=- (2)))
    ...
    SELECT INTO "NL:"
     ...
     DETAIL
      ...
      tec_disp = es.event_cd_disp, tec_desc = es.event_cd_descr, tec_name = es.event_set_name
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ELSEIF ((ui_es_cd=- (2)))
     SET get_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
     SET drdm_chg->log[get_parent_drdm_row].chg_log_reason_txt = concat(
      "Multiple rows found in the TARGET where V500.EVENT_CODE table when processing CODE_VALUE ",
      trim(cnvtstring(tr_code,20,1)),", where EVENT_CD_DISP = ",tec_disp,", EVENT_CD_DESCR = ",
      tec_desc," and EVENT_SET_NAME = ",tec_name)
     RETURN(- (18))
    ELSEIF ((ui_es_cd=- (3)))
     RETURN(0)
    ELSE
 ...
 SUBROUTINE get_target_pchart_comp_cd(pchart_code)
   ...
   ELSEIF ((ui_cat_cd=- (2)))
    ...
    ELSEIF ((ui_cat_cd=- (2)))
     SET get_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
     SET drdm_chg->log[get_parent_drdm_row].chg_log_reason_txt = concat(
      "Multiple rows found in the TARGET where CODE_VALUE table with a DEFINITION of ",rs_0619->
      from_values.definition," and CDF_MEANING of ",rs_0619->from_values.cdf_meaning,".")
     RETURN(- (4))
    ELSEIF ((ui_cat_cd=- (3)))
     RETURN(0)
    ELSE
 ...
 SUBROUTINE get_target_resource_cd(rec_cd)
   ...
   DECLARE trc_parent_drdm_row = i2 WITH protect, noconstant(0)
   FREE RECORD res_cd
   RECORD res_cd(
     1 qual[*]
       2 from_val = f8
   ...
   IF (curqual > 0)
    ...
    FOR (res_rs_loop = 1 TO par_res_cnt)
     SET sbr_ret_value = select_merge_translate(cnvtstring(res_cd->qual[res_rs_loop].from_val,20,1),
      "CODE_VALUE")
     IF (isnumeric(sbr_ret_value))
     ...
     ELSE
      ...
      IF (rpt_missing="ORPHAN")
       SET trc_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
       SET drdm_chg->log[trc_parent_drdm_row].chg_log_reason_txt = concat("Column ",
        "RESOURCE_GROUP.PARENT_SERVICE_RESOURCE_CD with a value of ",trim(cnvtstring(res_cd->qual[
          res_rs_loop].from_val,20,1))," does not exist in SOURCE on parent table ","CODE_VALUE.")
       RETURN(- (21))
      ENDIF
      IF (rpt_missing IN ("NOMV*", "CHLD*"))
       SET drdm_mini_loop_status = rpt_missing
       ...
       IF ((res_cd->qual[res_rs_loop].trans_ind=1))
        ...
        ELSEIF ((ui_rec_cd=- (2)))
         ...
         ELSEIF ((ui_rec_cd=- (2))
          ...
          SET trc_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
          SET drdm_chg->log[trc_parent_drdm_row].chg_log_reason_txt = concat(
           "Multiple rows found in the TARGET CODE_VALUE table with a CODE_SET of 221, DISPLAY_KEY of ",
           rs_0619->from_values.display_key," and CDF_MEANING of ",rs_0619->from_values.cdf_meaning,
           " and the same PARENT_SERVICE_RESOURCE_CD relation in the RESOURCE_GROUP table.")
          RETURN(- (6))
         ELSEIF ((ui_rec_cd=- (2))
          AND res_rs_loop != par_res_cnt)
          SET res_mult_ind = 1
    ...
    ELSEIF ((ui_rec_cd=- (2)))
     ...
     ELSEIF ((ui_rec_cd=- (2)))
      ...
      SET drdm_chg->log[trc_parent_drdm_row].chg_log_reason_txt = concat(
       "Multiple rows found in the TARGET CODE_VALUE table with a CODE_SET of 221, DISPLAY_KEY of ",
       rs_0619->from_values.display_key," and CDF_MEANING of ",rs_0619->from_values.cdf_meaning,
       " and no PARENT_SERVICE_RESOURCE_CD relation in the RESOURCE_GROUP table.")
      RETURN(- (6))
     ELSEIF ((ui_rec_cd=- (3)))
      RETURN(0)
     ELSE
 ...
 SUBROUTINE get_target_image_class_cd(imc_cd)
   ...
   DECLARE vc_lib = vc
   DECLARE vc_par = vc
   DECLARE tic_parent_drdm_row = i2 WITH protect, noconstant(0)
   SET par_trans_ind = 0
   SET lib_trans_ind = 0
   SET no_par_ind = 0
   SET par_cd = 0
   ...
   IF (par_cd=imc_cd)
    ...
    SET par_trans_ind = 1
   ENDIF
   SET vc_lib = select_merge_translate(cnvtstring(lib_cd,20,1),"CODE_VALUE")
   IF (isnumeric(vc_lib))
   ...
   ELSE
    ...
    IF (rpt_missing="ORPHAN")
     SET tic_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
     SET drdm_chg->log[tic_parent_drdm_row].chg_log_reason_txt = concat("Column ",
      "IMAGE_CLASS_TYPE.LIB_GROUP_CD with a value of ",trim(cnvtstring(lib_cd,20,1)),
      " does not exist in SOURCE on parent table CODE_VALUE.")
     RETURN(- (21))
    ENDIF
    IF (rpt_missing IN ("NOMV*", "CHLD*"))
     SET drdm_mini_loop_status = rpt_missing
   ...
   ENDIF
   IF (no_par_ind=0)
    SET vc_par = select_merge_translate(cnvtstring(par_cd,20,1),"CODE_VALUE")
    IF (isnumeric(vc_par))
     SET to_par = cnvtreal(vc_par)
     SET par_trans_ind = 1
    ELSE
     SET rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",par_cd,20,1)
     IF (rpt_missing="ORPHAN")
      SET tic_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
      SET drdm_chg->log[tic_parent_drdm_row].chg_log_reason_txt = concat("Column ",
       "IMAGE_CLASS_TYPE.PARENT_IMAGE_CLASS_TYPE_CD with a value of ",trim(cnvtstring(par_cd,20,1)),
       " does not exist in SOURCE on parent table CODE_VALUE.")
      RETURN(- (21))
     ENDIF
     IF (rpt_missing IN ("NOMV*", "CHLD*"))
   ...
   ELSEIF ((ui_imc_cd=- (2)))
    ...
    IF ((ui_imc_cd=- (1)))
     RETURN(- (20))
    ELSEIF ((ui_imc_cd=- (2)))
     SET tic_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
     SET drdm_chg->log[tic_parent_drdm_row].chg_log_reason_txt = concat(
      "Multiple rows found in the TARGET CODE_VALUE table with a CODE_SET of 5503, DISPLAY_KEY of ",
      rs_0619->from_values.display_key," and DESCRIPTION of ",rs_0619->from_values.description,
      " and the same PARENT_IMAGE_CLASS_TYPE_CD ",
      "and LIB_GROUP_CD relations in the IMAGE_CLASS_TYPE table.")
     RETURN(- (9))
    ELSEIF ((ui_imc_cd=- (3)))
     RETURN(0)
    ELSE
 ...
 SUBROUTINE get_target_task_ref_cd(sbr_code)
   ...
   DECLARE gtt_parent_drdm_row = i2 WITH protect, noconstant(0)
   SET ui_tr_cnt = 0
   SET inc_prelink = dm2_ref_data_doc->pre_link_name
   SET inc_postlink = dm2_ref_data_doc->post_link_name
   SET gt_select = dm2_get_rdds_tname("CODE_VALUE")
   ...
   ENDIF
   IF (curqual=0)
    SET gtt_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
    SET drdm_chg->log[gtt_parent_drdm_row].chg_log_reason_txt = concat(
     "No CODE_VALUE with a DISPLAY of ",rs_0619->from_values.definition,
     " from CODE_SET 16730 could be found and it is necessary to translate a CODE_VALUE from CODE_SET 16589."
     )
    RETURN(- (28))
   ENDIF
   IF (ui_tr_cnt=1)
    SET sbr_ret_val = select_merge_translate(cnvtstring(s_tr_gr_cd,20,1),"CODE_VALUE")
    IF (isnumeric(sbr_ret_val))
     SET tr_gr_cd = cnvtreal(sbr_ret_val)
    ELSE
     SET rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",s_tr_gr_cd)
     IF (rpt_missing="ORPHAN")
      SET gtt_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
      SET drdm_chg->log[gtt_parent_drdm_row].chg_log_reason_txt = concat("Column ",
       "CODE_VALUE with a value of ",trim(cnvtstring(s_tr_gr_cd,20,1)),
       " does not exist in SOURCE on parent table CODE_VALUE.")
      RETURN(- (21))
     ENDIF
     IF (rpt_missing IN ("NOMV*", "CHLD*"))
   ...
   IF (ui_tr_cnt=0)
    SET gtt_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
    SET drdm_chg->log[gtt_parent_drdm_row].chg_log_reason_txt = concat("CODE_VALUE ",trim(cnvtstring(
       sbr_code,20,1)),
     " from CODE_SET 16730 is not translated and is necessary to translate a CODE_VALUE from CODE_SET 16589."
     )
    RETURN(- (10))
   ELSEIF (ui_tr_cnt=1)
    SET stat = alterlist(ui_query_eval_rec->qual,0)
    ...
    ELSEIF ((ui_tr_cd=- (2)))
     ...
     IF ((ui_tr_cd=- (1)))
      RETURN(- (20))
     ELSEIF ((ui_tr_cd=- (2)))
      SET gtt_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
      SET drdm_chg->log[gtt_parent_drdm_row].chg_log_reason_txt = concat(
       "Multiple ASSOC_CODE_VALUE values found in the TARGET TRACK_REFERENCE table when processing CODE_VALUE ",
       trim(cnvtstring(sbr_code,20,1))," where TRACKING_GROUP_CD is ",trim(cnvtstring(tr_gr_cd,20,1)),
       " and DESCRIPTION is",
       rs_0619->from_values.description,".")
      RETURN(- (12))
     ELSEIF ((ui_tr_cd=- (3)))
      RETURN(0)
     ELSE
   ...
   ELSE
    SET gtt_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
    SET drdm_chg->log[gtt_parent_drdm_row].chg_log_reason_txt = concat(
     "Multiple rows found in the SOURCE CODE_VALUE table when CODE_SET is 16370 and DISPLAY is ",
     rs_0619->from_values.definition,".")
    RETURN(- (11))
   ENDIF
 END ;Subroutine
 SUBROUTINE get_target_dta(source_dta_code)
   ...
   DECLARE sbr_ret_val = vc
   DECLARE gtd_parent_drdm_row = i2 WITH protect, noconstant(0)
   SET src_act_cnt = 0
   SET ui_cv_cnt = 0
   SET inc_prelink = dm2_ref_data_doc->pre_link_name
   SET inc_postlink = dm2_ref_data_doc->post_link_name
   ...
   ENDIF
   IF (src_act_cd > 0)
    SET sbr_ret_val = select_merge_translate(cnvtstring(src_act_cd,20,1),"CODE_VALUE")
    IF (isnumeric(sbr_ret_val))
    ...
    ELSE
     ...
     IF (rpt_missing="ORPHAN")
      SET gtd_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
      SET drdm_chg->log[gtd_parent_drdm_row].chg_log_reason_txt = concat("Column ",
       "DISCRETE_TASK_ASSAY.ACTIVITY_TYPE_CD with a value of ",trim(cnvtstring(src_act_cd,20,1)),
       " does not exist in SOURCE on parent table CODE_VALUE.")
      RETURN(- (21))
     ENDIF
     IF (rpt_missing IN ("NOMV*", "CHLD*"))
   ...
   IF (src_act_cnt > 0)
    ...
    ELSEIF ((ui_cv_cd=- (2)))
     ...
     IF ((ui_cv_cd=- (1)))
      RETURN(- (20))
     ELSEIF ((ui_cv_cd=- (2)))
      SET gtd_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
      SET drdm_chg->log[gtd_parent_drdm_row].chg_log_reason_txt = concat(
       "Multiple rows found in TARGET DISCRETE_TASK_ASSAY table with ACTIVITY_TYPE_CD of ",trim(
        cnvtstring(ui_dta_cd,20,1))," and the TASK_ASSAY_CD has DISPLAY of ",rs_0619->from_values.
       display," and CODE_SET of ",
       trim(cnvtstring(rs_0619->from_values.code_set,20))," and DISPLAY_KEY of ",rs_0619->from_values
       .display_key," in the CODE_VALUE table.")
      RETURN(- (14))
     ELSEIF ((ui_cv_cd=- (3)))
      RETURN(0)
     ELSE
 ...
 SUBROUTINE get_target_oefields(source_oe_code)
   ...
   DECLARE sbr_ret_val = vc
   DECLARE gto_parent_drdm_row = i2 WITH protect, noconstant(0)
   SET src_cat_cnt = 0
   SET ui_cv_cnt = 0
   SET inc_prelink = dm2_ref_data_doc->pre_link_name
   SET inc_postlink = dm2_ref_data_doc->post_link_name
   ...
   ENDIF
   IF (src_cat_cd > 0)
    SET sbr_ret_val = select_merge_translate(cnvtstring(src_cat_cd,20,1),"CODE_VALUE")
    IF (isnumeric(sbr_ret_val))
    ...
    ELSE
     ...
     IF (rpt_missing="ORPHAN")
      SET gto_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
      SET drdm_chg->log[gto_parent_drdm_row].chg_log_reason_txt = concat("Column ",
       "ORDER_ENTRY_FIELDS.CATALOG_TYPE_CD with a value of ",trim(cnvtstring(src_cat_cd,20,1)),
       " does not exist in SOURCE on parent table CODE_VALUE.")
      RETURN(- (21))
     ENDIF
     IF (rpt_missing IN ("NOMV*", "CHLD*"))
   ...
   IF (src_cat_cnt > 0)
    ...
    ELSEIF ((ui_cv_cd=- (2)))
     ...
     IF ((ui_cv_cd=- (1)))
      RETURN(- (20))
     ELSEIF ((ui_cv_cd=- (2)))
      SET gto_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
      SET drdm_chg->log[gto_parent_drdm_row].chg_log_reason_txt = concat(
       "Multiple rows found in the TARGET ORDER_ENTRY_FIELDS table with CATALOG_TYPE_CD of ",trim(
        cnvtstring(ui_oe_cd,20,1))," and the OE_FIELD_ID has CODE_SET of ",trim(cnvtstring(rs_0619->
         from_values.code_set,20))," and DISPLAY_KEY of ",
       rs_0619->from_values.display_key," in the CODE_VALUE table.")
      RETURN(- (16))
     ELSEIF ((ui_cv_cd=- (3)))
      RETURN(0)
     ELSE
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE always_get_target_formula(i_cd_val)
   DECLARE s_cv_tgt = vc
   DECLARE s_display_src = vc
   DECLARE s_display_tgt = vc
   DECLARE s_cdf_mean = vc
   DECLARE s_ret_val = vc
   DECLARE s_cd_val_tgt = f8
   DECLARE s_grid_cd_tgt = f8
   DECLARE s_grid_cd_src = f8
   DECLARE s_display_src_len = i4
   DECLARE s_pos = i4
   DECLARE s_next_pos = i4
   DECLARE s_display_src_len = i2
   DECLARE s_omf_form_tgt = vc
   DECLARE s_ui_cnt = i4
   DECLARE s_evaluate_id = f8
   DECLARE s_user_id_src = f8
   DECLARE s_user_id_tgt = f8
   DECLARE s_omf_name = vc
   DECLARE s_active_ind_src = i2
   DECLARE s_col_cnt = i4
   DECLARE s_ndx = i4
   DECLARE s_end_colon_ind = i2
   DECLARE s_parent_drdm_row = i2
   DECLARE s_parent_drdm_row = i2 WITH protect, noconstant(0)
   FREE RECORD s_tokens
   RECORD s_tokens(
     1 qual[*]
       2 str = vc
       2 val = f8
   ...
   SET s_display_src = get_value("CODE_VALUE","DISPLAY","FROM")
   SET s_display_src_len = size(s_display_src,1)
   SET s_pos = findstring(" :",s_display_src)
   IF (s_pos=0)
    SET s_pos = findstring(" ",s_display_src)
   ENDIF
   SET s_pos = (s_pos+ 1)
   SET s_next_pos = findstring(":",s_display_src,(s_pos+ 1))
   IF (s_next_pos=0)
    SET s_next_pos = (s_display_src_len+ 1)
   ELSE
    SET s_end_colon_ind = 1
   ENDIF
   WHILE (s_pos <= s_display_src_len)
     SET s_tokens->cnt = (s_tokens->cnt+ 1)
     SET stat = alterlist(s_tokens->qual,s_tokens->cnt)
     IF (substring(s_pos,3,s_display_src)=":CV")
      IF (s_end_colon_ind=0)
       SET drdm_mini_loop_status = "NOMV46"
       SET s_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
       SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat("DISPLAY ",s_display_src,
        " for CODE_VALUE ",trim(cnvtstring(i_cd_val,20,1)),
        " from CODE_SET 25633 in SOURCE contains unexpected data")
       RETURN(- (25))
      ELSE
       SET s_tokens->qual[s_tokens->cnt].str = substring((s_pos+ 3),((s_next_pos - s_pos) - 3),
        s_display_src)
       SET s_tokens->qual[s_tokens->cnt].val = cnvtreal(s_tokens->qual[s_tokens->cnt].str)
       SET s_pos = (s_next_pos+ 1)
      ENDIF
     ELSE
      SET s_tokens->qual[s_tokens->cnt].str = notrim(substring(s_pos,(s_next_pos - s_pos),
        s_display_src))
      SET s_pos = s_next_pos
     ENDIF
     SET s_next_pos = findstring(":",s_display_src,(s_pos+ 1))
     IF (s_next_pos=0)
      SET s_end_colon_ind = 0
      SET s_next_pos = (s_display_src_len+ 1)
     ELSE
      SET s_end_colon_ind = 1
     ENDIF
   ENDWHILE
   FOR (s_ndx = 1 TO s_tokens->cnt)
     IF ((s_tokens->qual[s_ndx].val > 0))
      SET s_ret_val = select_merge_translate(s_tokens->qual[s_ndx].str,"CODE_VALUE")
      IF (isnumeric(s_ret_val))
       SET s_tokens->qual[s_ndx].val_xlat = cnvtreal(s_ret_val)
      ELSE
       SET rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",s_tokens->qual[s_ndx].val)
       IF (rpt_missing="ORPHAN")
        SET s_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
        SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat("Column ","DISPLAY",
         " with a value of ",trim(cnvtstring(s_tokens->qual[s_ndx].val,20,1)),
         " does not exist in SOURCE on parent table CODE_VALUE.")
        RETURN(- (21))
       ELSEIF (rpt_missing IN ("NOMV*", "CHLD*"))
        SET drdm_mini_loop_status = rpt_missing
        RETURN(- (22))
       ELSE
        RETURN(- (24))
       ENDIF
      ENDIF
      SET s_display_tgt = concat(s_display_tgt,":CV",s_ret_val,":")
     ELSE
      SET s_display_tgt = notrim(concat(s_display_tgt,s_tokens->qual[s_ndx].str))
     ENDIF
   ENDFOR
   SET s_omf_form_tgt = dm2_get_rdds_tname("OMF_FORMULA")
   SELECT INTO "nl:"
    FROM (parser(s_omf_form_tgt) o)
    WHERE o.indicator_cd=i_cd_val
    DETAIL
     s_grid_cd_src = o.grid_cd, s_user_id_src = o.user_id, s_omf_name = o.name
    WITH nocounter
   ;end select
   IF (curqual=0)
    SET drdm_mini_loop_status = "NOMV46"
    SET s_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
    SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat("No relation for CODE_VALUE ",
     trim(cnvtstring(i_cd_val,20,1))," in SOURCE when matching on INDICATOR_CD in OMF_FORMULA.")
    RETURN(- (25))
   ENDIF
   SET s_ret_val = select_merge_translate(cnvtstring(s_grid_cd_src,20,1),"CODE_VALUE")
   IF (isnumeric(s_ret_val))
    SET s_grid_cd_tgt = cnvtreal(s_ret_val)
   ELSE
    SET rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",s_grid_cd_src)
    IF (rpt_missing="ORPHAN")
     SET s_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
     SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat("Column ",
      "OMF_FORMULA.GRID_CD with a value of ",trim(cnvtstring(s_grid_cd_src,20,1)),
      " does not exist in SOURCE on parent table CODE_VALUE.")
     RETURN(- (21))
    ELSEIF (rpt_missing IN ("NOMV*", "CHLD*"))
     SET drdm_mini_loop_status = rpt_missing
    ...
    ENDIF
   ENDIF
   IF (is_translated("CODE_VALUE","CODE_VALUE")=0)
    SET s_ret_val = select_merge_translate(cnvtstring(s_user_id_src,20,1),"PRSNL")
    IF (isnumeric(s_ret_val))
     SET s_user_id_tgt = cnvtreal(s_ret_val)
    ELSE
     SET rpt_missing = report_missing("PRSNL","PERSON_ID",s_user_id_src)
     IF (rpt_missing="ORPHAN")
      SET s_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
      SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat("Column ",
       "OMF_FORMULA.USER_ID with a value of ",trim(cnvtstring(s_user_id_src,20,1)),
       " does not exist in SOURCE on parent table PRSNL.")
      RETURN(- (21))
     ELSEIF (rpt_missing IN ("NOMV*", "CHLD*"))
      SET drdm_mini_loop_status = rpt_missing
      RETURN(- (22))
     ...
     ENDIF
    ENDIF
    SET s_cdf_mean = get_value("CODE_VALUE","CDF_MEANING","FROM")
    SELECT INTO "nl:"
     FROM code_value c
     WHERE c.cdf_meaning=s_cdf_mean
      AND c.display=patstring(concat("*",s_display_tgt))
      AND c.code_set=25633
      AND c.code_value IN (
     (SELECT
      o.indicator_cd
      FROM omf_formula o
      WHERE o.grid_cd=s_grid_cd_tgt
       AND o.user_id=s_user_id_tgt
       AND o.name=s_omf_name))
     HEAD REPORT
      s_ui_cnt = 0
     DETAIL
      s_ui_cnt = (s_ui_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,s_ui_cnt), ui_query_eval_rec
      ->qual[s_ui_cnt].root_entity_attr = c.code_value
     WITH nocounter
    ;end select
    SET s_evaluate_id = evaluate_exec_ui_query(s_ui_cnt,temp_tbl_cnt,perm_col_cnt)
    IF ((s_evaluate_id=- (2)))
     SET s_active_ind_src = cnvtint(get_value("CODE_VALUE","ACTIVE_IND","FROM"))
     SELECT INTO "nl:"
      FROM code_value c
      WHERE c.cdf_meaning=s_cdf_mean
       AND c.display=patstring(concat("*",s_display_tgt))
       AND c.code_set=25633.0
       AND c.active_ind=s_active_ind_src
       AND c.code_value IN (
      (SELECT
       o.indicator_cd
       FROM omf_formula o
      ...
      HEAD REPORT
       s_ui_cnt = 0
      DETAIL
       s_ui_cnt = (s_ui_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,s_ui_cnt),
       ui_query_eval_rec->qual[s_ui_cnt].root_entity_attr = c.code_value
      WITH nocounter
     ;end select
     SET s_evaluate_id = evaluate_exec_ui_query(s_ui_cnt,temp_tbl_cnt,perm_col_cnt)
     IF ((s_evaluate_id=- (2)))
      SET drdm_mini_loop_status = "NOMV06"
      SET s_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
      SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat(
       "Multiple rows found in the TARGET CODE_VALUE table with CODE_SET of 25633, DISPLAY of *",
       s_display_tgt," and OMF_FORMULA.GRID_CD of ",trim(cnvtstring(s_grid_cd_tgt,20,1)),
       " OMF_FORMULA.USER_ID of ",
       trim(cnvtstring(s_user_id_tgt,20,1))," and OMF_FORMULA.NAME of ",s_omf_name,
       " when joined on INDICATOR_CD.")
      RETURN(- (26))
     ELSEIF (s_evaluate_id <= 0
      AND (s_evaluate_id != - (3)))
      RETURN(- (20))
     ENDIF
    ELSEIF (s_evaluate_id <= 0
     ...
     RETURN(- (20))
    ENDIF
    SET s_cd_val_tgt = s_evaluate_id
   ELSE
    SET s_cd_val_tgt = cnvtreal(get_value("CODE_VALUE","CODE_VALUE","TO"))
   ENDIF
   FOR (s_ndx = 1 TO s_tokens->cnt)
     IF ((s_tokens->qual[s_ndx].val_xlat > 0))
      SELECT INTO "nl:"
       FROM omf_grid og,
        omf_grid_column ogc,
        omf_vo_type ovt,
        omf_vo_type_display ovtd,
        omf_vo_indicator_group ovig,
        omf_indicator oi1
       WHERE og.grid_cd=s_grid_cd_tgt
        AND og.active_ind=1
        AND ogc.grid_cd=og.grid_cd
        AND ovt.vo_type_cd=ogc.grid_column_cd
        AND ovtd.vo_type_cd=ovt.vo_type_cd
        AND ovig.parent_indicator_cd=ovt.vo_indicator_cd
        AND ovig.vo_display_seq=ovtd.vo_display_seq
        AND oi1.indicator_cd=ovig.child_indicator_cd
        AND oi1.fact_ind=1
        AND (oi1.indicator_cd=s_tokens->qual[s_ndx].val_xlat)
       WITH nocounter
      ;end select
      IF (curqual=0)
       SELECT INTO "nl:"
        ...
        WHERE og.grid_cd=s_grid_cd_tgt
         AND og.active_ind=1
         AND ogc.grid_cd=og.grid_cd
         AND oi1.indicator_cd=ogc.grid_column_cd
         AND oi1.fact_ind=1
         AND (oi1.indicator_cd=s_tokens->qual[s_ndx].val_xlat)
        WITH nocounter
       ;end select
       IF (curqual=0)
        SET drdm_mini_loop_status = "NOMV18"
        RETURN(- (23))
       ENDIF
      ENDIF
     ENDIF
   ENDFOR
   IF ((s_cd_val_tgt=- (3)))
    SET s_cd_val_tgt = get_seq("CODE_VALUE","CODE_VALUE")
   ENDIF
   SET s_display_tgt = concat(trim(cnvtstring(s_cd_val_tgt,20)),".00 ",s_display_tgt)
   SET rs_0619->to_values.description = s_display_tgt
   SET rs_0619->to_values.display = s_display_tgt
   SET rs_0619->to_values.display_key = trim(cnvtupper(cnvtalphanum(s_display_tgt)))
   RETURN(s_cd_val_tgt)
 END ;Subroutine
 SUBROUTINE always_get_target_view_def(i_cd_val)
   DECLARE s_display_src = vc
   DECLARE s_display_tgt = vc
   DECLARE s_cdf_mean = vc
   DECLARE s_ret_val = vc
   DECLARE s_display_tgt_cd_val = vc
   DECLARE s_display_src_cd_val = vc
   DECLARE s_cd_val_tgt = f8
   DECLARE s_pos = i4
   DECLARE s_ui_cnt = i4
   DECLARE s_evaluate_id = f8
   ...
   SET s_display_src = get_value("CODE_VALUE","DISPLAY","FROM")
   SET s_cdf_mean = get_value("CODE_VALUE","CDF_MEANING","FROM")
   IF (s_cdf_mean="INDICATOR"
    AND s_display_src=patstring("OMFABS PROFILE*"))
    SET s_pos = findstring(".",s_display_src)
    IF (s_pos > 0)
     SET s_display_src_cd_val = substring(1,(s_pos - 1),s_display_src)
     SET s_display_src_cd_val = cnvtalphanum(s_display_src_cd_val,1)
    ELSE
     SET s_display_src_cd_val = cnvtalphanum(replace(s_display_src,".00","",1),1)
    ENDIF
    SET s_ret_val = select_merge_translate(s_display_src_cd_val,"CODE_VALUE")
    IF (isnumeric(s_ret_val))
     SET s_display_tgt_cd_val = s_ret_val
    ELSE
     SET rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(s_display_src_cd_val))
     IF (rpt_missing="ORPHAN")
      SET s_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
      SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat("Column ",
       "DISPLAY with a value of ",trim(cnvtstring(s_display_src_cd_val,20,1)),
       " does not exist in SOURCE on parent table CODE_VALUE.")
      RETURN(- (21))
     ELSEIF (rpt_missing IN ("NOMV*", "CHLD*"))
      SET drdm_mini_loop_status = rpt_missing
     ...
     ENDIF
    ENDIF
    SET s_display_tgt = replace(s_display_src,s_display_src_cd_val,s_display_tgt_cd_val,1)
   ELSE
    SET s_display_tgt = s_display_src
   ENDIF
   IF (is_translated("CODE_VALUE","CODE_VALUE")=0)
    SELECT INTO "nl:"
     FROM code_value c
     WHERE c.cdf_meaning=s_cdf_mean
      AND c.display=s_display_tgt
      AND c.code_set=24509
     HEAD REPORT
      s_ui_cnt = 0
     DETAIL
      s_ui_cnt = (s_ui_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,s_ui_cnt), ui_query_eval_rec
    ...
    IF ((s_evaluate_id=- (2)))
     ...
     IF ((s_evaluate_id=- (2)))
      ...
      SET s_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
      SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat(
       "Multiple rows found in the TARGET CODE_VALUE table with CODE_SET of 24509, DISPLAY of ",
       s_display_tgt," and CDF_MEANING of ",s_cdf_mean,".")
      RETURN(- (27))
     ELSEIF (s_evaluate_id <= 0
      AND (s_evaluate_id != - (3)))
      RETURN(- (20))
   ...
   RETURN(s_cd_val_tgt)
 END ;Subroutine
 SUBROUTINE dupind_get_target_mammo_ltr(dgtm_cv_val)
   DECLARE dgtm_to_val = f8 WITH protect, noconstant(0.0)
   DECLARE dgtm_cdf_mean_src = vc WITH protect, noconstant(" ")
   DECLARE dgtm_display_src = vc WITH protect, noconstant(" ")
   DECLARE dgtm_display_key_src = vc WITH protect, noconstant(" ")
   DECLARE dgtm_ret_val = vc WITH protect, noconstant(" ")
   DECLARE dgtm_parent_drdm_row = f8 WITH protect, noconstant(0.0)
   DECLARE dgtm_mammo_pos = i4 WITH protect, noconstant(0)
   DECLARE magic = f8 WITH protect, constant(- (31.0))
   DECLARE leprechaun = f8 WITH protect, constant(- (30.0))
   DECLARE rainbow = f8 WITH protect, constant(- (21.0))
   DECLARE lucky_charms = f8 WITH protect, constant(- (22.0))
   DECLARE pot_o_gold = f8 WITH protect, constant(- (29.0))
   SET dgtm_display_src = get_value("CODE_VALUE","DISPLAY","FROM")
   SET dgtm_display_key_src = get_value("CODE_VALUE","DISPLAY_KEY","FROM")
   SET dgtm_cdf_mean_src = get_value("CODE_VALUE","CDF_MEANING","FROM")
   IF (isnumeric(dgtm_cdf_mean_src)=0
    AND isnumeric(dgtm_display_src)=0
    AND isnumeric(dgtm_display_key_src)=0)
    RETURN(magic)
   ELSEIF (dgtm_cdf_mean_src != dgtm_display_src)
    SET drdm_mini_loop_status = "NOMV1A"
    SET s_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
    SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat("Column ",
     "CODE_VALUE.CDF_MEANING with a value of ",dgtm_cdf_mean_src,
     " does not equal column CODE_VALUE.DISPLAY with a value of ",dgtm_display_src)
    RETURN(leprechaun)
   ELSEIF (dgtm_cdf_mean_src != dgtm_display_key_src)
    SET drdm_mini_loop_status = "NOMV1A"
    SET s_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
    SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat("Column ",
     "CODE_VALUE.CDF_MEANING with a value of ",dgtm_cdf_mean_src,
     " does not equal column CODE_VALUE.DISPLAY_KEY with a value of ",dgtm_display_key_src)
    RETURN(leprechaun)
   ENDIF
   SET dgtm_ret_val = select_merge_translate(dgtm_cdf_mean_src,"MAMMO_LETTER")
   IF (isnumeric(dgtm_ret_val))
    IF (findstring(".",dgtm_ret_val) > 0)
     SET dgtm_ret_val = substring(1,(findstring(".",dgtm_ret_val) - 1),dgtm_ret_val)
    ENDIF
    SET rs_0619->to_values.cdf_meaning = dgtm_ret_val
    SET rs_0619->to_values.display = dgtm_ret_val
    SET rs_0619->to_values.display_key = trim(cnvtupper(cnvtalphanum(dgtm_ret_val)))
   ELSE
    SET dgtm_mammo_pos = locateval(dgtm_mammo_pos,1,dm2_ref_data_doc->tbl_cnt,"MAMMO_LETTER",
     dm2_ref_data_doc->tbl_qual[dgtm_mammo_pos].table_name)
    IF ((dm2_ref_data_doc->tbl_qual[dgtm_mammo_pos].mergeable_ind=0))
     SET drdm_mini_loop_status = "NOMV04"
     SET s_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
     SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat("Column ",
      "CODE_VALUE.CDF_MEANING references value ",dgtm_cdf_mean_src,
      " which comes from non-mergeable table MAMMO_LETTER in source.")
     RETURN(lucky_charms)
    ELSE
     SET rpt_missing = report_missing("MAMMO_LETTER","LETTER_ID",cnvtreal(dgtm_cdf_mean_src))
     IF (rpt_missing="ORPHAN")
      SET s_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
      SET drdm_chg->log[s_parent_drdm_row].chg_log_reason_txt = concat("Column ",
       "CODE_VALUE.CDF_MEANING with a value of ",dgtm_cdf_mean_src,
       " does not exist in SOURCE on parent table MAMMO_LETTER.")
      RETURN(rainbow)
     ELSEIF (rpt_missing IN ("NOMV*", "CHLD*"))
      SET drdm_mini_loop_status = rpt_missing
      RETURN(lucky_charms)
   ...
   RETURN(magic)
 END ;Subroutine
 SUBROUTINE get_target_event_set_name(gtesn_cv)
   DECLARE gtesn_tab = vc WITH protect, noconstant(" ")
   DECLARE gtesn_cv_tab = vc WITH protect, noconstant(" ")
   DECLARE gtesn_ui_cnt = i4 WITH protect, noconstant(0)
   DECLARE gtesn_ui_cd = f8 WITH protect, noconstant(0.0)
   DECLARE gtesn_def = vc WITH protect, noconstant(" ")
   DECLARE gtesn_src_cnt = i4 WITH protect, noconstant(0)
   SET gtesn_tab = dm2_get_rdds_tname("V500_EVENT_SET_CODE")
   SET gtesn_cv_tab = dm2_get_rdds_tname("CODE_VALUE")
   SET stat = alterlist(ui_query_eval_rec->qual,0)
   SELECT INTO "NL:"
    cnt = count(*)
    FROM (parser(gtesn_tab) es1)
    WHERE es1.event_set_cd=gtesn_cv
    DETAIL
     gtesn_src_cnt = cnt
    WITH nocounter
   ;end select
   ...
   IF (gtesn_src_cnt=0)
    SELECT INTO "NL:"
     cv.code_value
     FROM code_value cv,
      (parser(gtesn_cv_tab) cv1)
     WHERE cv1.code_value=gtesn_cv
      AND ((cv.display_key=cv1.display_key) OR (cv1.display_key = null
      AND cv.display_key = null))
      AND ((cv.definition=cv1.definition) OR (cv1.definition = null
      AND cv.definition = null))
      AND  NOT ( EXISTS (
     (SELECT
      "x"
     ...
     DETAIL
      gtesn_ui_cnt = (gtesn_ui_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,gtesn_ui_cnt),
      ui_query_eval_rec->qual[gtesn_ui_cnt].root_entity_attr = cv.code_value
     WITH nocounter
    ;end select
   ELSE
    SELECT INTO "NL:"
     es.event_cd
     FROM v500_event_set_code es,
      (parser(gtesn_tab) es1)
     PLAN (es1
      WHERE es1.event_set_cd=gtesn_cv)
      JOIN (es
      WHERE ((es1.event_set_name=es.event_set_name) OR (es1.event_set_name=null
       AND es.event_set_name=null)) )
     DETAIL
      gtesn_ui_cnt = (gtesn_ui_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,gtesn_ui_cnt),
      ui_query_eval_rec->qual[gtesn_ui_cnt].root_entity_attr = es.event_set_cd
     WITH nocounter
    ;end select
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(- (20))
   ENDIF
   SET gtesn_ui_cd = evaluate_exec_ui_query(gtesn_ui_cnt,temp_tbl_cnt,perm_col_cnt)
   IF ((gtesn_ui_cd=- (1)))
    RETURN(- (20))
   ELSEIF ((gtesn_ui_cd=- (2)))
    SET gtesn_ui_cnt = 0
    SET stat = alterlist(ui_query_eval_rec->qual,0)
    IF (gtesn_src_cnt=0)
     SELECT INTO "NL:"
      ...
      (SELECT
       ...
       WHERE es.event_set_cd=cv.code_value)))
      DETAIL
       gtesn_def = cv.definition, gtesn_ui_cnt = (gtesn_ui_cnt+ 1), stat = alterlist(
        ui_query_eval_rec->qual,gtesn_ui_cnt),
       ui_query_eval_rec->qual[gtesn_ui_cnt].root_entity_attr = cv.code_value
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
    ...
    ELSEIF ((gtesn_ui_cd=- (2)))
     SET get_parent_drdm_row = find_original_log_row(drdm_chg,amx_log_loop)
     SET drdm_chg->log[get_parent_drdm_row].chg_log_reason_txt = concat(
      "Multiple rows found in the TARGET when processing a CODE_VALUE from CODE_SET 93 with a value of ",
      trim(cnvtstring(gtesn_cv,20,2)),", where DEFINITION is ",gtesn_def)
     RETURN(- (32))
    ELSEIF ((gtesn_ui_cd=- (3)))
     RETURN(0)
    ELSE
     RETURN(gtesn_ui_cd)
    ENDIF
