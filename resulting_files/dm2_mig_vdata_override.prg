CREATE PROGRAM dm2_mig_vdata_override
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmvo_nr = f8 WITH protect, noconstant(10000000.0)
 DECLARE dmvo_mode = vc WITH protect, noconstant( $1)
 DECLARE dmvo_onechunk = f8 WITH protect, noconstant(999999999.0)
 DECLARE dmvo_rpt_file = vc WITH protect, noconstant("")
 DECLARE dmvo_column = vc WITH protect, noconstant("")
 FREE RECORD dmvo_data
 RECORD dmvo_data(
   1 dd_cnt = i4
   1 qual[*]
     2 dd_owner = vc
     2 dd_tbl = vc
     2 dd_cycles_left = f8
     2 dd_modified_cycles_left = f8
     2 dd_cycles_completed = f8
     2 dd_rtc = f8
     2 dd_nr = f8
     2 dd_begkey = f8
     2 dd_endkey = f8
 ...
 IF (check_logfile("dm2_vdata_ovrde",".log","Dm2_mig_vdata_override LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validate mode of execution"
 IF ( NOT (dmvo_mode IN ("PREVIEW", "APPLY", "VIEW")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = 'Invalid mode of execution. Valid modes are "PREVIEW", "VIEW" and "APPLY".'
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dmvo_mode="VIEW")
  SET dmvo_column = "| Chunks Completed "
 ELSE
  SET dmvo_column = "| Modified Chunks Remaining "
 ENDIF
 IF ((validate(override_nr,- (1)) != - (1)))
  SET dmvo_nr = override_nr
 ENDIF
 ...
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Gather data compare information."
 SELECT INTO "nl:"
  FROM dm_vdata_master dvm,
   dm_dba_tables_actual_stats@ref_data_link dt
  WHERE  NOT (dvm.compare_status IN ("EXCLUDE", "COMPLETE"))
   AND dvm.owner_name=dt.owner
   AND dvm.table_name=dt.table_name
   AND dt.num_rows IS NOT null
   AND dvm.rows_to_compare < dt.num_rows
   AND dvm.range_start_keyid != dvm.max_src_keyid
  ORDER BY dvm.owner_name, dvm.table_name
  DETAIL
   dmvo_data->dd_cnt = (dmvo_data->dd_cnt+ 1), stat = alterlist(dmvo_data->qual,dmvo_data->dd_cnt),
   dmvo_data->qual[dmvo_data->dd_cnt].dd_owner = dvm.owner_name,
   dmvo_data->qual[dmvo_data->dd_cnt].dd_tbl = dvm.table_name, dmvo_data->qual[dmvo_data->dd_cnt].
   dd_begkey = dvm.range_beg_keyid, dmvo_data->qual[dmvo_data->dd_cnt].dd_endkey = dvm
   .range_end_keyid,
   dmvo_data->qual[dmvo_data->dd_cnt].dd_maxkey = dvm.max_src_keyid, dmvo_data->qual[dmvo_data->
   dd_cnt].dd_id = dvm.dm_vdata_master_id, dmvo_data->qual[dmvo_data->dd_cnt].dd_nr = dt.num_rows,
   dmvo_data->qual[dmvo_data->dd_cnt].dd_cycles_left = dm2ceil(((dvm.max_src_keyid - dvm
    .range_end_keyid)/ dvm.rows_to_compare)), dmvo_data->qual[dmvo_data->dd_cnt].dd_rtc = dvm
   .rows_to_compare
   IF (dvm.range_end_keyid < dvm.range_start_keyid)
    dmvo_data->qual[dmvo_data->dd_cnt].dd_cycles_completed = 0
   ELSE
    dmvo_data->qual[dmvo_data->dd_cnt].dd_cycles_completed = dm2ceil(((dvm.range_end_keyid - dvm
     .range_start_keyid)/ dvm.rows_to_compare))
   ENDIF
   dmvo_data->qual[dmvo_data->dd_cnt].dd_modified_cycles_left = dmvo_data->qual[dmvo_data->dd_cnt].
   dd_cycles_left
   IF (dmvo_mode != "VIEW")
    IF ((dmvo_data->qual[dmvo_data->dd_cnt].dd_nr < dmvo_nr))
     dmvo_data->qual[dmvo_data->dd_cnt].dd_rtc = dmvo_onechunk
    ENDIF
    IF ((dmvo_data->qual[dmvo_data->dd_cnt].dd_rtc > dmvo_data->qual[dmvo_data->dd_cnt].dd_nr))
     dmvo_data->qual[dmvo_data->dd_cnt].dd_modified_cycles_left = 1
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmvo_data)
 ENDIF
 IF ((dmvo_data->dd_cnt=0))
  SET dm_err->eproc = "No overrides available. Exiting script."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Generating Data Verification Override Report"
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 IF (get_unique_file("dm2_vdata_ovr",".rpt")=0)
  GO TO exit_program
 ELSE
  SET dmvo_rpt_file = dm_err->unique_fname
 ENDIF
 SELECT INTO value(dmvo_rpt_file)
  FROM (dummyt d  WITH seq = value(dmvo_data->dd_cnt))
  PLAN (d
   WHERE d.seq > 0)
  ORDER BY dmvo_data->qual[d.seq].dd_modified_cycles_left DESC
  HEAD REPORT
   CALL print("Database Migration Compare Override Report"), row + 2, col 1,
   "Owner.Table_Name", col 41, "| Chunks Remaining ",
   col 61, dmvo_column, col 91,
   "| Num Rows ", col 111, "| Current Begin Key ",
   col 131, "| Current End Key ", col 151,
   "| Max Key Id ", col 171, "| Rows To Compare ",
   row + 1,
   CALL print(fillstring(250,"-")), row + 2
  DETAIL
   col 1,
   CALL print(concat(dmvo_data->qual[d.seq].dd_owner,".",dmvo_data->qual[d.seq].dd_tbl))
   IF ((dmvo_data->qual[d.seq].dd_cycles_left > 9999999999999.0))
    col 41, "| ", "*************"
   ELSE
    col 41, "| ",
    CALL print(format(dmvo_data->qual[d.seq].dd_cycles_left,";L"))
   ENDIF
   IF (dmvo_mode="VIEW")
    IF ((dmvo_data->qual[d.seq].dd_cycles_completed > 9999999999999.0))
     col 61, "| ", "*************"
    ELSE
     col 61, "| ",
     CALL print(format(dmvo_data->qual[d.seq].dd_cycles_completed,";L"))
    ENDIF
   ELSE
    IF ((dmvo_data->qual[d.seq].dd_modified_cycles_left > 9999999999999.0))
     col 61, "| ", "*************"
    ELSE
     col 61, "| ",
     CALL print(format(dmvo_data->qual[d.seq].dd_modified_cycles_left,";L"))
    ENDIF
   ENDIF
   IF ((dmvo_data->qual[d.seq].dd_nr > 9999999999999.0))
    col 91, "| ", "*************"
   ELSE
    col 91, "| ",
    CALL print(format(dmvo_data->qual[d.seq].dd_nr,";L"))
   ENDIF
   IF ((dmvo_data->qual[d.seq].dd_begkey > 9999999999999.0))
    col 111, "| ", "*************"
   ELSE
    col 111, "| ",
    CALL print(format(dmvo_data->qual[d.seq].dd_begkey,";L"))
   ENDIF
   IF ((dmvo_data->qual[d.seq].dd_endkey > 9999999999999.0))
    col 131, "| ", "*************"
   ELSE
    col 131, "| ",
    CALL print(format(dmvo_data->qual[d.seq].dd_endkey,";L"))
   ENDIF
   IF ((dmvo_data->qual[d.seq].dd_maxkey > 9999999999999.0))
    col 151, "| ", "*************"
   ELSE
    col 151, "| ",
    CALL print(format(dmvo_data->qual[d.seq].dd_maxkey,";L"))
   ENDIF
   IF ((dmvo_data->qual[d.seq].dd_rtc > 9999999999999.0))
    col 171, "| ", "*************"
   ELSE
    col 171, "| ",
    CALL print(format(dmvo_data->qual[d.seq].dd_rtc,";L"))
   ENDIF
   row + 1
 ...
 IF (dm2_disp_file(dmvo_rpt_file,"Override Report")=0)
  GO TO exit_program
 ENDIF
 IF (dmvo_mode="APPLY")
  SET dm_err->eproc = "Apply changes to DM_VDATA_MASTER.ROWS_TO_COMPARE"
  UPDATE  FROM dm_vdata_master dvm,
    (dummyt d  WITH seq = value(dmvo_data->dd_cnt))
   SET dvm.rows_to_compare = dmvo_data->qual[d.seq].dd_rtc, dvm.compare_status = "READY"
   PLAN (d)
    JOIN (dvm
    WHERE (dmvo_data->qual[d.seq].dd_id=dvm.dm_vdata_master_id))
   WITH nocounter
  ;end update
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Dm2_mig_vdata_override completed"
 CALL final_disp_msg("dm2_vdata_ovrde")
END GO
