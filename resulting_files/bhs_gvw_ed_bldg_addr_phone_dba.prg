CREATE PROGRAM bhs_gvw_ed_bldg_addr_phone:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (e
   ...
   JOIN (a
   WHERE a.parent_entity_id=e.loc_building_cd
    AND a.parent_entity_name="LOCATION"
    AND a.address_type_cd=mf_business_add_cd
    AND a.active_ind=1
  ...
  HEAD REPORT
   ...
   ms_city_state_zip = concat(trim(a.city),", ",trim(evaluate(a.state_cd,0.0,a.state,
    ...
    "}",ms_reol)
   IF (p.phone_id > 0.00)
    reply->text = concat(reply->text,trim(cnvtphone(p.phone_num,p.phone_format_cd),3),ms_reol)
   ENDIF
