CREATE PROGRAM cp_generate_pathway_trail:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "String Input" = "",
  "Float Input" = 0.0
  WITH outdev, sinput, dinput
 ...
 DECLARE mysubroutine(null) = null WITH protect
 DECLARE dadvsr_status_inprogress = f8 WITH constant(uar_get_code_by("MEANING",4002229,"INPROGRESS"))
 DECLARE ncount = i2 WITH protect, noconstant(0)
 DECLARE lcount = i4 WITH protect, noconstant(0)
 DECLARE svar = vc WITH protect, noconstant(0)
 ...
 CALL mysubroutine(null)
 SET mpagereply->status_data.status = "S"
 SUBROUTINE mysubroutine(null)
   CALL log_message("Begin MySubroutine()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   CALL log_message(build("Exit MySubroutine(), Elapsed time in seconds:",((curtime3 - begin_curtime3
...
#exit_script
 ...
 ENDIF
 IF (( $OUTDEV != "NOFORMS"))
  CALL putstringtofile(cnvtrectojson(mpagereply))
 ENDIF
 FREE RECORD mpagereply
 CALL log_message(build("Exit Script inn_mp_ccl_unified_template, Elapsed time in seconds:",((
