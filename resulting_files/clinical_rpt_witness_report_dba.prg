CREATE PROGRAM clinical_rpt_witness_report:dba
 ...
 SET req->script_name = "clinical_rpt_witness_report"
 ...
 IF ((request->encntr_id > 0))
  ...
  SET req->encntr_list[1].encntr_id = request->encntr_id
 ENDIF
 SET no_of_encntrs = size(request->encntr_list,5)
 IF (no_of_encntrs > 0)
  SET stat = alterlist(req->encntr_list,no_of_encntrs)
  FOR (index = 1 TO no_of_encntrs)
    SET req->encntr_list[index].encntr_id = request->encntr_list[index].encntr_id
  ENDFOR
 ENDIF
 ...
 IF ((rep->status_data.status="S"))
  SET reply->output_file = req->output_device
 ENDIF
 IF (bglobaldebugflag=1)
