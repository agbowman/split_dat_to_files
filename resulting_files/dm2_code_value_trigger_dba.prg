CREATE PROGRAM dm2_code_value_trigger:dba
 ...
 ENDIF
 DECLARE dcvt_create_ora_trigger(dcot_trigger_name=vc) = i4
 DECLARE dcvt_create_db2_trigger(dcdt_trigger_name=vc,dcdt_type=vc) = i4
 DECLARE dcvt_find_trigger(dft_trigger_name=vc) = i4
 DECLARE dcvt_text = vc WITH public, noconstant(" ")
 DECLARE dcvt_trigger_count = i4 WITH public, noconstant(0)
 DECLARE dcvt_table_count = i4 WITH public, noconstant(0)
 DECLARE dcvt_table_name = vc WITH public, noconstant(" ")
 DECLARE dcvt_trigger_name = vc WITH public, noconstant(" ")
 DECLARE dcvt_trigger_tbl_name = vc WITH public, noconstant(" ")
 DECLARE dcvt_tname1 = vc WITH public, noconstant(" ")
 IF (check_logfile("dm2_code_val_trig",".log","DM2_CODE_VALUE_TRIGGER LOG ...") != 1)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "BEGINNING DM2_CODE_VALUE_TRIGGER.PRG"
 CALL disp_msg(" ",dm_err->logfile,0)
 CASE (currdb)
  OF "ORACLE":
   SET dcvt_tname1 = "CODE_VALUE"
  OF "DB2UDB":
   SET dcvt_tname1 = "CODE_VALUE0619"
 ENDCASE
 SET dm_err->eproc = concat("Verifying that the base table named ",dcvt_tname1," exists.")
 SELECT INTO "nl:"
  FROM dm2_user_tables a
  WHERE a.table_name=value(dcvt_tname1)
  DETAIL
   dcvt_table_count = (dcvt_table_count+ 1), dcvt_table_name = a.table_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 CASE (dcvt_table_count)
  OF 0:
   SET dm_err->emsg = concat("The required base table named ",dctv_tname1,
    " does not exist for triggers to be created. ")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dm_err->err_ind = 1
   GO TO exit_program
  OF 1:
   SET dcvt_table_count = 1
  ELSE
   SET dm_err->emsgc = concat(
    "Both CODE_VALUE or CODE_VALUE$C tables were found to exist.  Trigger process bypassed.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dm_err->err_ind = 1
   GO TO exit_program
 ENDCASE
 CASE (currdb)
  OF "ORACLE":
   IF (dcvt_find_trigger("TRG_CODE_VALUE_INS_UPDT*") != 1)
    SET dm_err->err_ind = 1
    GO TO exit_program
   ENDIF
   IF (dcvt_trigger_count > 1)
    SET dm_err->eproc = "Dropping TRG_CODE_VALUE_INS_UPDT$C trigger."
    SET dcvt_text = "RDB ASIS (^ DROP TRIGGER TRG_CODE_VALUE_INS_UPDT$C ^) GO"
    IF (dm2_push_cmd(dcvt_text,1) != 1)
     SET dm_err->err_ind = 1
     GO TO exit_program
    ENDIF
   ELSEIF (dcvt_trigger_count=0)
    SET dcvt_trigger_name = "TRG_CODE_VALUE_INS_UPDT"
   ENDIF
   IF (dcvt_create_ora_trigger(dcvt_trigger_name) != 1)
    SET dm_err->err_ind = 1
    GO TO exit_program
   ENDIF
  OF "DB2UDB":
   IF (dcvt_find_trigger("CODE_VALUE_INS*") != 1)
    SET dm_err->err_ind = 1
    GO TO exit_program
   ENDIF
   IF (dcvt_trigger_count > 1)
    SET dcvt_text = "RDB ASIS (^ DROP TRIGGER CODE_VALUE_INS$C ^) GO"
    IF (dm2_push_cmd(dcvt_text,1) != 1)
     SET dm_err->err_ind = 1
     ROLLBACK
     GO TO exit_program
    ENDIF
    SET dcvt_trigger_name = "CODE_VALUE_INS"
   ENDIF
   IF (dcvt_trigger_count > 0)
    SET dcvt_text = concat("RDB ASIS (^ DROP TRIGGER ",value(dcvt_trigger_name)," ^) GO")
    IF (dm2_push_cmd(dcvt_text,1) != 1)
     SET dm_err->err_ind = 1
     ROLLBACK
     GO TO exit_program
   ...
   ELSE
    SET dcvt_trigger_name = "CODE_VALUE_INS"
   ENDIF
   IF (dcvt_create_db2_trigger(dcvt_trigger_name,"INSERT")=1)
    COMMIT
   ELSE
    ROLLBACK
    SET dm_err->err_ind = 1
    GO TO exit_program
   ENDIF
   IF (dcvt_find_trigger("CODE_VALUE_UPD*") != 1)
    SET dm_err->err_ind = 1
    GO TO exit_program
   ENDIF
   IF (dcvt_trigger_count > 1)
    SET dcvt_text = "RDB ASIS (^ DROP TRIGGER CODE_VALUE_UPD$C ^) GO"
    IF (dm2_push_cmd(dcvt_text,1) != 1)
     SET dm_err->err_ind = 1
     ROLLBACK
     GO TO exit_program
    ENDIF
    SET dcvt_trigger_name = "CODE_VALUE_UPD"
   ENDIF
   IF (dcvt_trigger_count > 0)
    SET dcvt_text = concat("RDB ASIS (^ DROP TRIGGER ",value(dcvt_trigger_name)," ^) GO")
    IF (dm2_push_cmd(dcvt_text,1) != 1)
   ...
   ELSE
    SET dcvt_trigger_name = "CODE_VALUE_UPD"
   ENDIF
   IF (dcvt_create_db2_trigger(dcvt_trigger_name,"UPDATE")=1)
    COMMIT
   ELSE
    SET dm_err->err_ind = 1
    GO TO exit_program
   ENDIF
   IF (dcvt_find_trigger("CODE_VALUE_DEL*") != 1)
    SET dm_err->err_ind = 1
    GO TO exit_program
   ENDIF
   IF (dcvt_trigger_count > 1)
    SET dcvt_text = "RDB ASIS (^ DROP TRIGGER CODE_VALUE_DEL$C ^) GO"
    IF (dm2_push_cmd(dcvt_text,1) != 1)
     SET dm_err->err_ind = 1
     ROLLBACK
     GO TO exit_program
    ENDIF
    SET dcvt_trigger_name = "CODE_VALUE_DEL"
   ENDIF
   IF (dcvt_trigger_count > 0)
    SET dcvt_text = concat("RDB ASIS (^ DROP TRIGGER ",value(dcvt_trigger_name)," ^) GO")
    IF (dm2_push_cmd(dcvt_text,1) != 1)
   ...
   ELSE
    SET dcvt_trigger_name = "CODE_VALUE_DEL"
   ENDIF
   IF (dcvt_create_db2_trigger(dcvt_trigger_name,"DELETE")=1)
    COMMIT
   ELSE
    SET dm_err->err_ind = 1
    GO TO exit_program
 ...
 ENDCASE
 GO TO exit_program
 SUBROUTINE dcvt_create_ora_trigger(dcot_trigger_name)
   SET dcvt_text = " create or replace procedure dm_updt_inst(p1 in number) "
   SET dcvt_text = concat(dcvt_text,"   as begin ")
   SET dcvt_text = concat(dcvt_text,"      update dm_info ")
   SET dcvt_text = concat(dcvt_text,"      set info_date  = sysdate,")
   SET dcvt_text = concat(dcvt_text,"          updt_dt_tm = sysdate, ")
   SET dcvt_text = concat(dcvt_text,"          updt_cnt   = updt_cnt + 1 ")
   SET dcvt_text = concat(dcvt_text,"      where info_domain = 'CODE SET UPDATE' ")
   SET dcvt_text = concat(dcvt_text,"        and info_name   = to_char(p1); ")
   SET dcvt_text = concat(dcvt_text,"      if sql%notfound then ")
   SET dcvt_text = concat(dcvt_text,"          insert into dm_info ")
   SET dcvt_text = concat(dcvt_text,
    "              (info_domain, info_name, info_date, info_number, updt_dt_tm, updt_cnt) ")
   SET dcvt_text = concat(dcvt_text,"           values ")
   SET dcvt_text = concat(dcvt_text,
    "              ('CODE SET UPDATE', to_char(p1), sysdate, p1, sysdate, 0); ")
   SET dcvt_text = concat(dcvt_text,"      end if;")
   SET dcvt_text = concat(dcvt_text,"   end;")
   SET dcvt_text = concat("RDB ASIS (^ ",dcvt_text," ^) GO")
   IF (dm2_push_cmd(dcvt_text,1) != 1)
    SET dm_err->err_ind = 1
    RETURN(0)
   ENDIF
   SET dcvt_text = concat("create or replace trigger ",value(dcot_trigger_name)," before ")
   SET dcvt_text = concat(dcvt_text," update or insert on ",value(dcvt_trigger_tbl_name))
   SET dcvt_text = concat(dcvt_text," for each row  ")
   SET dcvt_text = concat(dcvt_text," begin  ")
   SET dcvt_text = concat(dcvt_text,"  if (UPDATING and :new.updt_dt_tm = :old.updt_dt_tm) then ")
   SET dcvt_text = concat(dcvt_text,"    :new.updt_dt_tm := sysdate;")
   SET dcvt_text = concat(dcvt_text,"  end if;")
   SET dcvt_text = concat(dcvt_text,"  dm_updt_inst(:new.code_set); ")
   SET dcvt_text = concat(dcvt_text," end;")
   SET dcvt_text = concat("RDB ASIS (^ ",dcvt_text," ^) GO")
   IF (dm2_push_cmd(dcvt_text,1) != 1)
    SET dm_err->err_ind = 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcvt_create_db2_trigger(dcdt_trigger_name,dcdt_type)
   SET dcvt_text = concat("create trigger ",value(dcdt_trigger_name))
   SET dcvt_text = concat(dcvt_text," after ",value(dcdt_type)," on ",value(dcvt_trigger_tbl_name))
   CASE (dcdt_type)
    OF "INSERT":
     SET dcvt_text = concat(dcvt_text," referencing new as new ")
    OF "UPDATE":
     SET dcvt_text = concat(dcvt_text," referencing new as new old as old ")
    OF "DELETE":
     SET dcvt_text = concat(dcvt_text," referencing old as old ")
   ENDCASE
   SET dcvt_text = concat(dcvt_text," for each row mode db2sql ")
   SET dcvt_text = concat(dcvt_text," begin atomic ")
   SET dcvt_text = concat(dcvt_text," declare rcount integer; ")
   SET dcvt_text = concat(dcvt_text," declare v_str varchar(70); ")
   IF (dcdt_type="UPDATE")
    SET dcvt_text = concat(dcvt_text," if (new.updt_dt_tm = old.updt_dt_tm) then ")
    SET dcvt_text = concat(dcvt_text,"  update code_value cv ")
    SET dcvt_text = concat(dcvt_text,"  set cv.updt_dt_tm = current timestamp ")
    SET dcvt_text = concat(dcvt_text,"  where cv.code_value = new.code_value; ")
    SET dcvt_text = concat(dcvt_text,"  get diagnostics rcount = ROW_COUNT; ")
    SET dcvt_text = concat(dcvt_text,"  if(rcount != 1) then ")
    SET dcvt_text = concat(dcvt_text,"    set v_str= 'Bad diagnostic row_count value('||")
    SET dcvt_text = concat(dcvt_text,"rtrim(char(rcount))||') for timestamp update command'; ")
    SET dcvt_text = concat(dcvt_text,"    signal sqlstate '70999' set message_text = v_str; ")
    SET dcvt_text = concat(dcvt_text,"  end if; ")
    SET dcvt_text = concat(dcvt_text," end if; ")
   ENDIF
   SET dcvt_text = concat(dcvt_text,"      update dm_info ")
   SET dcvt_text = concat(dcvt_text,"      set info_date = current timestamp, ")
   SET dcvt_text = concat(dcvt_text,"              updt_dt_tm = current timestamp, ")
   SET dcvt_text = concat(dcvt_text,"              updt_cnt = updt_cnt + 1 ")
   SET dcvt_text = concat(dcvt_text,"      where info_domain = 'CODE SET UPDATE' ")
   IF (dcdt_type="DELETE")
    SET dcvt_text = concat(dcvt_text," and info_name = char(old.code_set); ")
   ELSE
    SET dcvt_text = concat(dcvt_text," and info_name = char(new.code_set); ")
   ENDIF
   SET dcvt_text = concat(dcvt_text,"      get diagnostics rcount = ROW_COUNT; ")
   SET dcvt_text = concat(dcvt_text,"  if (rcount = 0) then ")
   SET dcvt_text = concat(dcvt_text,"        insert into dm_info ")
   SET dcvt_text = concat(dcvt_text,"              (info_domain, info_name, info_date,",
    "       info_number, updt_dt_tm, updt_cnt) ")
   SET dcvt_text = concat(dcvt_text,"        values ")
   IF (dcdt_type="DELETE")
    SET dcvt_text = concat(dcvt_text,"     ('CODE SET UPDATE', char(old.code_set),current timestamp,",
     "       old.code_set,current timestamp, 0 ); ")
   ELSE
    SET dcvt_text = concat(dcvt_text,"     ('CODE SET UPDATE', char(new.code_set),current timestamp,",
     "       new.code_set,current timestamp, 0 ); ")
   ENDIF
   SET dcvt_text = concat(dcvt_text,"         get diagnostics rcount = ROW_COUNT; ")
   SET dcvt_text = concat(dcvt_text,"         if(rcount != 1) then ")
   SET dcvt_text = concat(dcvt_text,"           set v_str= 'Bad diagnostic row_count value ('",
   ...
   SET dcvt_text = concat(dcvt_text,"           signal sqlstate '70999' set message_text = v_str; ")
   SET dcvt_text = concat(dcvt_text,"         end if; ")
   SET dcvt_text = concat(dcvt_text,"  elseif (rcount != 1) then ")
   SET dcvt_text = concat(dcvt_text,"        set v_str = 'Bad diagnostic row_count value ('||",
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcvt_find_trigger(dft_trigger_name)
   SET dcvt_trigger_count = 0
   SET dm_err->eproc = concat("Verifying which triggers exist for the ",dcvt_table_name," table.")
   SELECT INTO "nl:"
    FROM dm2_user_triggers a
    WHERE a.trigger_name=value(dft_trigger_name)
     AND a.table_name=dcvt_table_name
    ORDER BY trigger_name DESC
    DETAIL
     dcvt_trigger_name = a.trigger_name, dcvt_trigger_tbl_name = a.table_name, dcvt_trigger_count = (
     dcvt_trigger_count+ 1)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual != 1)
    SET dcvt_trigger_tbl_name = dcvt_table_name
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "ENDING DM2_CODE_VALUE_TRIGGER.PRG"
 CALL final_disp_msg("dm2_code_val_trig")
 IF ((dm_err->err_ind=1))
