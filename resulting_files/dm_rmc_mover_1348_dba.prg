CREATE PROGRAM dm_rmc_mover_1348:dba
 ...
 DECLARE aax_tab_name = vc
 DECLARE cust_tab_name_s = vc
 DECLARE aax_tab_name_s = vc
 DECLARE cust_chg_tab_name = vc
 DECLARE cust_aap_id = f8
 DECLARE cust_description = vc
 DECLARE cust_af_cd = f8
 DECLARE cust_af_cd_s = f8
 DECLARE af_cd_trans = vc
 DECLARE cust_sp_cd = f8
 DECLARE cust_sp_cd_s = f8
 DECLARE sp_cd_trans = vc
 DECLARE cust_aap_id = f8
 ...
 SET aax_tab_name = "ACCESSION_ASSIGN_XREF"
 SET cust_chg_tab_name = "DM_CHG_LOG_EXCEPTION"
 SET cust_excp_tab_name = dm2_get_rdds_tname(cust_chg_tab_name)
 SET cust_tab_name_s = dm2_get_rdds_tname(cust_tab_name)
 SET aax_tab_name_s = dm2_get_rdds_tname(aax_tab_name)
 SET cust_aap_id = cnvtreal(get_value(cust_tab_name,"ACCESSION_ASSIGNMENT_POOL_ID","FROM"))
 SET cust_description = get_value(cust_tab_name,"DESCRIPTION","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_1348
 ENDIF
 IF (is_translated(cust_tab_name,"ACCESSION_ASSIGNMENT_POOL_ID")=0)
  SET cust_nomv_chk = select_merge_translate(cnvtstring(cust_aap_id),cust_tab_name)
  ...
  ENDIF
  SELECT INTO "NL:"
   FROM (parser(aax_tab_name_s) c)
   WHERE c.accession_assignment_pool_id=cust_aap_id
   DETAIL
    cust_af_cd_s = c.accession_format_cd, cust_sp_cd_s = c.site_prefix_cd
   WITH nocounter
  ;end select
  ...
  IF (curqual=0)
   SELECT INTO "NL:"
    FROM (parser(cust_tab_name) c)
    WHERE c.description=cust_description
     AND  NOT (c.accession_assignment_pool_id IN (
    (SELECT
     a.accession_assignment_pool_id
     FROM accession_assign_xref a)))
    HEAD REPORT
     ui_cnt = 0
    DETAIL
     ui_cnt = (ui_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,ui_cnt), ui_query_eval_rec->qual[
     ui_cnt].root_entity_attr = c.accession_assignment_pool_id
    WITH nocounter
   ;end select
   IF (error(dm2_ref_data_reply->error_msg,0) > 0)
    SET dm2_ref_data_reply->error_ind = 1
   ...
   IF (evaluate_id < 0)
    IF ((evaluate_id=- (3)))
     SET cust_to_val = get_seq(cust_tab_name,"ACCESSION_ASSIGNMENT_POOL_ID")
     CALL put_value(cust_tab_name,"ACCESSION_ASSIGNMENT_POOL_ID",cnvtstring(cust_to_val))
    ...
    ENDIF
   ELSE
    SET cust_to_val = evaluate_id
    CALL put_value(cust_tab_name,"ACCESSION_ASSIGNMENT_POOL_ID",cnvtstring(cust_to_val))
   ENDIF
   SET stat = alterlist(dm2_ref_data_reply->qual,1)
   SET dm2_ref_data_reply->qual[1].table_name = cust_tab_name
   SET dm2_ref_data_reply->qual[1].from_value = cust_aap_id
   SET dm2_ref_data_reply->qual[1].to_value = cust_to_val
  ELSE
   SET af_cd_trans = select_merge_translate(cnvtstring(cust_af_cd_s),"CODE_VALUE")
   IF ( NOT (isnumeric(af_cd_trans)))
    SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cust_af_cd_s)
    CALL evaluate_rpt_missing(cust_rpt_missing)
    SET cust_err_val = get_err_val(null)
    IF (cust_err_val=0)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = concat(cust_tab_name,
      " cannot translate because of the ACCESSION_ASSIGN_XREF.ACCESSION_FORMAT_CD.")
    ENDIF
    GO TO exit_1348
   ELSE
    SET cust_af_cd = cnvtreal(af_cd_trans)
   ENDIF
   SET sp_cd_trans = select_merge_translate(cnvtstring(cust_sp_cd_s),"CODE_VALUE")
   IF ( NOT (isnumeric(sp_cd_trans)))
    SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cust_sp_cd_s)
    CALL evaluate_rpt_missing(cust_rpt_missing)
    SET cust_err_val = get_err_val(null)
    IF (cust_err_val=0)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = concat(cust_tab_name,
      " cannot translate because of the ACCESSION_ASSIGN_XREF.SITE_PREFIX_CD.")
    ENDIF
    GO TO exit_1348
   ELSE
    SET cust_sp_cd = cnvtreal(sp_cd_trans)
   ENDIF
   SET fr_temp = locateval(fr_return,1,dm2_ref_data_doc->tbl_cnt,aax_tab_name,dm2_ref_data_doc->
    tbl_qual[fr_return].table_name)
   IF (fr_temp=0)
    SET fr_temp = temp_tbl_cnt
    SET fr_return = fill_rs("TABLE",aax_tab_name)
   ...
   SELECT INTO "NL:"
    FROM (parser(cust_tab_name) c)
    WHERE c.description=cust_description
     AND c.accession_assignment_pool_id IN (
    (SELECT
     a.accession_assignment_pool_id
     FROM accession_assign_xref a
     WHERE a.accession_format_cd=cust_af_cd
      AND a.site_prefix_cd=cust_sp_cd
      AND  NOT (list(a.accession_format_cd,a.site_prefix_cd) IN (
     (SELECT
      b.accession_format_cd, b.site_prefix_cd
      FROM accession_assi5355$r b
      WHERE b.rdds_delete_ind=1
       AND b.rdds_status_flag < 9000
       AND (b.rdds_source_env_id=dm2_ref_data_doc->env_source_id))))))
    HEAD REPORT
     ui_cnt = 0
    DETAIL
     ui_cnt = (ui_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,ui_cnt), ui_query_eval_rec->qual[
