CREATE PROGRAM aps_rdm_move_rt_comments:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_rdm_move_rt_comments"
 ...
 CALL logmsg("Getting ready to begin reading for report_task comment.",((e_dispmsg+ e_rdmstatus)+
  e_logtofile))
 SELECT INTO "nl:"
  FROM report_task rt
 ...
 WHILE (rt_cntr >= 1)
   ...
   IF (curqual != value(rt_commit_recs_whole))
    ROLLBACK
    CALL logmsg("Failed to update report_task table.",((e_dispmsg+ e_logtofile)+ e_rdmstatus))
    SET sprogramfailed = "T"
    GO TO exit_script
   ENDIF
   SET sprogramfailed = "F"
