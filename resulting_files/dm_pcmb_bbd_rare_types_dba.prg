CREATE PROGRAM dm_pcmb_bbd_rare_types:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "BBD_RARE_TYPES"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_BBD_RARE_TYPES"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   .active_status_cd,
   rreclist->from_rec[v_cust_count1].rare_id = frm.rare_id, rreclist->from_rec[v_cust_count1].
   rare_type_cd = frm.rare_type_cd
  FOOT REPORT
 ...
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    rreclist->to_rec[v_cust_count2].rare_id = tu.rare_id, rreclist->to_rec[v_cust_count2].
    rare_type_cd = tu.rare_type_cd
   FOOT REPORT
  ...
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
   SELECT INTO "nl:"
    ...
    FROM bbd_rare_types tu
    WHERE (tu.person_id=request->xxx_combine[icombine].to_xxx_id)
     AND (tu.rare_type_cd=rreclist->from_rec[v_cust_loopcount].rare_type_cd)
     AND tu.active_ind=1
    WITH forupdate(tu)
   ;end select
   IF (curqual=0)
    CALL upt_from(rreclist->from_rec[v_cust_loopcount].rare_id,request->xxx_combine[icombine].
     to_xxx_id)
   ELSE
    CALL del_from(rreclist->from_rec[v_cust_loopcount].rare_id,request->xxx_combine[icombine].
 ...
 SUBROUTINE del_from(s_df_pk_id,s_df_to_fk_id,s_df_prev_act_ind,s_df_prev_act_status)
   UPDATE  FROM bbd_rare_types frm
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "BBD_RARE_TYPES"
