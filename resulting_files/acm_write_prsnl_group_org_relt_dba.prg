CREATE PROGRAM acm_write_prsnl_group_org_relt:dba
 ...
 DECLARE ubound = i4 WITH protect, noconstant(reply->prsnl_group_org_reltn_qual_cnt)
 SET table_name = "PRSNL_GROUP_ORG_RELTN"
 SET stat = alterlist(reply->prsnl_group_org_reltn_qual,reply->prsnl_group_org_reltn_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->prsnl_group_org_reltn_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->prsnl_group_org_reltn_qual[index].prsnl_group_org_reltn_id = acm_request->
     prsnl_group_org_reltn_qual[index].prsnl_group_org_reltn_id
     SET reply->prsnl_group_org_reltn_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_prsnl_group_org_reltn
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_prsnl_group_org_reltn
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_prsnl_group_org_reltn
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
