CREATE PROGRAM dm2_capture_desired_schema:dba
 ...
 ENDIF
 CALL check_logfile("dm2_cap_desired_sch",".log","DM2_CAPTURE_DESIRED_SCHEMA LOG FILE")
 SET dm_err->eproc = "Begin dm2_capture_desired_schema..."
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE check_date(null) = i4
 DECLARE get_new_schema(null) = i4
 DECLARE get_dropped_fk(null) = null
 DECLARE populate_schema(null) = i4
 DECLARE verify_dminfo_schemainstance(null) = i4
 DECLARE dcds_date_error(dde_err_ind=i4) = null
 SET dcds_schema_date = fillstring(11,"")
 DECLARE dcds_process_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dcds_tmp_err_ind = i2 WITH protect, noconstant(0)
 RECORD new_schema(
   1 qual[*]
     2 table_name = vc
     2 max_schema_dt = dq8
 ...
 IF ((validate(dm2_allow_cap_desired_schema_ind,- (1))=- (1)))
  CALL echo("***************************************")
  CALL echo("This script has been obsoleted.")
  CALL echo("***************************************")
  GO TO exit_script
 ENDIF
 ...
 ELSE
  ...
  GO TO exit_script
 ENDIF
 SET dcds_schema_date =  $1
 IF (check_date(null)=0)
  GO TO exit_script
 ENDIF
 ...
 IF ((inhouse_misc->inhouse_domain=0))
  IF (verify_dminfo_schemainstance(null)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 CALL dm2_process_log_add_detail_text("SCHEMA_DATE",dcds_schema_date)
 CALL dm2_process_log_add_detail_text("CURRRDBUSER",currdbuser)
 SET dm2_process_rs->process_name = dpl_cap_desired_schema
 SET dm2_process_event_rs->status = dpl_executing
 IF (dm2_process_log_row(dpl_cap_desired_schema,dpl_execution,dpl_no_prev_id,1)=0)
  GO TO exit_script
 ENDIF
 SET dcds_process_log_id = dm2_process_event_rs->dm_process_event_id
 IF ( NOT (get_new_schema(null)))
  GO TO exit_script
 ENDIF
 IF ( NOT (populate_schema(null)))
  GO TO exit_script
 ENDIF
 COMMIT
 SET validate_schema_initiated_script = 1
 EXECUTE dm2_capture_schema dcds_schema_date, "Validate Schema Process", "*"
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 GO TO exit_script
 SUBROUTINE check_date(null)
   SET dm_err->eproc = "Check schema date format..."
   DECLARE cd_sel_cnt = i4 WITH protect, noconstant(0)
   IF ( NOT (isnumeric(substring(1,2, $1))))
    CALL dcds_date_error(2)
    RETURN(0)
   ELSEIF (substring(3,1, $1) != "-")
    CALL dcds_date_error(2)
    RETURN(0)
   ELSEIF ( NOT (cnvtupper(substring(4,3, $1)) IN ("JAN", "FEB", "MAR", "APR", "MAY",
   "JUN", "JUL", "AUG", "SEP", "OCT",
   "NOV", "DEC")))
    CALL dcds_date_error(2)
    RETURN(0)
   ELSEIF ( NOT (isnumeric(substring(8,4, $1))))
    CALL dcds_date_error(2)
    RETURN(0)
   ELSEIF (substring(7,1, $1) != "-")
    CALL dcds_date_error(2)
    RETURN(0)
   ENDIF
   IF (day(cnvtdatetime(dcds_schema_date))=1)
    CALL dcds_date_error(0)
    RETURN(0)
   ELSEIF (month(cnvtdatetime(dcds_schema_date)) != month(datetimeadd(cnvtdatetime(dcds_schema_date),
     1)))
    CALL dcds_date_error(0)
    RETURN(0)
   ENDIF
   SELECT INTO "nl:"
    FROM dm_tables d
    WHERE d.schema_date=cnvtdatetimeutc(dcds_schema_date)
    WITH maxqual(d,1)
   ;end select
   SET cd_sel_cnt = curqual
   IF (check_error(dm_err->eproc) != 0)
   ...
   IF (cd_sel_cnt > 0)
    ...
    SET message = window
    CALL clear(1,1)
    CALL text(8,5,"This Schema Snapshot Date has previously been used.")
    CALL text(9,5,
     "If you choose to CONTINUE, then this process will delete the contents of previously captured")
    CALL text(10,5,"Schema Snapshot and store new content for this Schema Snapshot Date.")
    CALL text(11,5,"Continue (y/n)")
    CALL accept(11,21,"P;CU"
     WHERE curaccept IN ("Y", "N"))
    SET message = nowindow
    IF (curaccept="Y")
     DELETE  FROM dm_cons_columns dcc
      WHERE dcc.schema_date=cnvtdatetimeutc(dcds_schema_date)
      WITH nocounter
     ;end delete
     IF (check_error("Deleting old schema snapshot from dm_cons_columns for constraints columns") !=
     0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ELSE
      COMMIT
     ENDIF
     DELETE  FROM dm_constraints dcc
      WHERE dcc.schema_date=cnvtdatetimeutc(dcds_schema_date)
      WITH nocounter
     ;end delete
     IF (check_error("Deleting old schema snapshot from dm_constraints for constraints") != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ELSE
      COMMIT
     ENDIF
     DELETE  FROM dm_index_columns dcc
      WHERE dcc.schema_date=cnvtdatetimeutc(dcds_schema_date)
      WITH nocounter
     ;end delete
     IF (check_error("Deleting old schema snapshot from dm_index_columns for index columns") != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ELSE
      COMMIT
     ENDIF
     DELETE  FROM dm_indexes dcc
      WHERE dcc.schema_date=cnvtdatetimeutc(dcds_schema_date)
      WITH nocounter
     ;end delete
     IF (check_error("Deleting old schema snapshot from dm_indexes for indexes") != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ELSE
      COMMIT
     ENDIF
     DELETE  FROM dm_columns dcc
      WHERE dcc.schema_date=cnvtdatetimeutc(dcds_schema_date)
      WITH nocounter
     ;end delete
     IF (check_error("Deleting old schema snapshot from dm_columns for columns") != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ELSE
      COMMIT
     ENDIF
     DELETE  FROM dm_tables dcc
      WHERE dcc.schema_date=cnvtdatetimeutc(dcds_schema_date)
      WITH nocounter
     ;end delete
     IF (check_error("Deleting old schema snapshot from dm_tables for tables") != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ELSE
      COMMIT
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcds_date_error(dde_err_ind)
   SET dm_err->err_ind = 1
   CASE (dde_err_ind)
    OF 0:
     SET dm_err->emsg = concat("Date passed in is not valid.",
      " The first and last days of the month are reserved by Cerner")
    OF 2:
     SET dm_err->emsg = concat("Date passed in is not valid."," Please use the format dd-mmm-yyyy.")
   ENDCASE
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 END ;Subroutine
 SUBROUTINE get_new_schema(null)
   IF ((inhouse_misc->inhouse_domain=0))
    SET dm_err->eproc = "Getting new schema from afd tables"
   ELSE
    SET dm_err->eproc = "Getting new schema from adm tables"
   ENDIF
   IF ((inhouse_misc->inhouse_domain=0))
    SELECT INTO "nl:"
     dafd.table_name, dafd.schema_instance, dafd.schema_date,
     dafd.alpha_feature_nbr
     FROM dm_afd_tables dafd,
      dm_info di
     PLAN (di
      WHERE di.info_domain="DM2_SCHEMA_INSTANCE")
      JOIN (dafd
      WHERE dafd.table_name=di.info_name
       AND dafd.schema_instance=di.info_number
       AND dafd.alpha_feature_nbr > 0
       AND dafd.owner=currdbuser)
     ORDER BY dafd.table_name, dafd.alpha_feature_nbr
     HEAD REPORT
      cnt = 0, cnt2 = 0
     FOOT  dafd.table_name
      cnt = (cnt+ 1), stat = alterlist(new_schema->qual,cnt), new_schema->qual[cnt].table_name = trim
      (dafd.table_name,3),
      new_schema->qual[cnt].max_schema_dt = cnvtdatetimeutc(dafd.schema_date), new_schema->qual[cnt].
      alpha_feature_nbr = dafd.alpha_feature_nbr, new_schema->qual[cnt].schema_instance = dafd
      .schema_instance
     WITH nocounter
    ;end select
    IF (check_error("Getting new schema (table and FK) info.") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ELSE
    IF (currdbuser="V500")
     SELECT INTO "nl:"
      dpse.proj_name
      FROM dm_project_status_env dpse,
       dm_info di,
       dm_adm_tables dat
      PLAN (di
       WHERE di.info_domain="DATA MANAGEMENT"
        AND di.info_name="DM_ENV_ID")
       JOIN (dpse
       WHERE dpse.environment_id=di.info_number
        AND dpse.proj_type="SCHEMA"
        AND dpse.dm_status="SUCCESS")
       JOIN (dat
       WHERE dat.table_name=dpse.proj_name
        AND dat.schema_date=dpse.schema_date
        AND dat.owner=currdbuser)
      ORDER BY dpse.proj_name, dpse.schema_date DESC
      HEAD REPORT
       cnt = 0
      HEAD dpse.proj_name
       cnt = (cnt+ 1), stat = alterlist(new_schema->qual,cnt), new_schema->qual[cnt].table_name =
       trim(dpse.proj_name,3),
       new_schema->qual[cnt].max_schema_dt = dpse.schema_date, new_schema->qual[cnt].schema_instance
        = dat.schema_instance
      WITH nocounter
     ;end select
    ELSE
     SELECT INTO "nl:"
      ...
      PLAN (di
       ...
       WHERE dpse.environment_id=di.info_number
        AND dpse.proj_type="SCHEMA"
        AND dpse.dm_status="SUCCESS"
        AND sqlpassthru("trim(substr(dpse.proj_name,1,instr(dpse.proj_name,'.')-1))",50)=currdbuser)
       JOIN (dat
       WHERE dat.table_name=sqlpassthru("trim(substr(dpse.proj_name,instr(dpse.proj_name,'.')+1,30))",
        50)
        AND dat.schema_date=dpse.schema_date
        AND dat.owner=currdbuser)
      ORDER BY dpse.proj_name, dpse.schema_date DESC
      ...
      HEAD dpse.proj_name
       cnt = (cnt+ 1), stat = alterlist(new_schema->qual,cnt), new_schema->qual[cnt].table_name =
       trim(dat.table_name,3),
       new_schema->qual[cnt].max_schema_dt = dpse.schema_date, new_schema->qual[cnt].schema_instance
        = dat.schema_instance
      WITH nocounter
     ;end select
    ENDIF
    IF (check_error("Getting new schema - table for inhouse domain.") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(new_schema)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE populate_schema(null)
   SET dm_err->eproc = "Populate schema..."
   DECLARE insert_str = vc WITH protect, noconstant(" ")
   IF ((dm_err->debug_flag=1))
    CALL echo(build("new_schema size:",size(new_schema->qual,5)))
   ENDIF
   IF ((inhouse_misc->inhouse_domain=1))
    FOR (new_num = 1 TO size(new_schema->qual,5))
      IF ((dm_err->debug_flag > 0))
       CALL echo(build("Now populating table <",new_num,"> of <",size(new_schema->qual,5),">."))
      ENDIF
      SET dm_err->eproc = "Inserting schema snapshot rows to dm_tables"
      SET insert_str = " "
      SET insert_str = concat(
       "insert into dm_tables (table_name, schema_date, tablespace_name, pct_increase,",
       " pct_used, pct_free, updt_applctx, updt_dt_tm, updt_cnt, updt_id, updt_task)",
       " (select dat.table_name, cnvtdatetimeutc(dcds_schema_date),"," dat.tablespace_name,",
       " dat.pct_increase, dat.pct_used, dat.pct_free, dat.updt_applctx, dat.updt_dt_tm,",
       " dat.updt_cnt, dat.updt_id, dat.updt_task "," from dm_adm_tables dat ",
       " where dat.table_name = new_schema->qual[new_num].table_name ",
       " and dat.schema_date = cnvtdatetimeutc(new_schema->qual[new_num].max_schema_dt) ",
       " and dat.owner = currdbuser) ",
       " go")
      SET trace symbol mark
      IF ( NOT (dm2_push_cmd(insert_str,1)))
       RETURN(0)
      ELSE
       COMMIT
      ENDIF
      SET trace symbol release
      SET dm_err->eproc = "Inserting schema snapshot rows to dm_columns"
      SET insert_str = " "
      SET insert_str = concat("insert into dm_columns",
       " (table_name, column_name, schema_date, column_seq, data_type,",
       " data_length, data_precision, data_scale, nullable, updt_applctx,",
       " updt_dt_tm, updt_cnt, updt_id, updt_task, data_default, virtual_column)",
       " (select dac.table_name, dac.column_name, cnvtdatetimeutc(dcds_schema_date),",
       " dac.column_seq,",
       " dac.data_type, dac.data_length, dac.data_precision, dac.data_scale, dac.nullable,",
       " dac.updt_applctx, dac.updt_dt_tm, dac.updt_cnt, dac.updt_id,",
       " dac.updt_task, dac.data_default, dac.virtual_column"," from dm_adm_columns dac",
       " where dac.table_name = new_schema->qual[new_num].table_name",
       " and dac.schema_date = cnvtdatetimeutc(new_schema->qual[new_num].max_schema_dt)",
       " and dac.owner = currdbuser)"," go")
      SET trace symbol mark
      IF ( NOT (dm2_push_cmd(insert_str,1)))
       RETURN(0)
      ELSE
      ...
      ENDIF
      SET trace symbol release
      SET dm_err->eproc = "Inserting schema snapshot rows to dm_indexes"
      SET insert_str = " "
      SET insert_str = concat("insert into dm_indexes",
       " (index_name, schema_date, table_name, tablespace_name, pct_increase, pct_free,",
       " unique_ind, updt_applctx, updt_dt_tm, updt_cnt, updt_id, updt_task, index_type)",
       " (select dai.index_name, cnvtdatetimeutc(dcds_schema_date),",
       " dai.table_name, dai.tablespace_name,",
       " dai.pct_increase, dai.pct_free, dai.unique_ind, dai.updt_applctx,",
       " dai.updt_dt_tm, dai.updt_cnt, dai.updt_id, dai.updt_task, dai.index_type",
       " from dm_adm_indexes dai"," where dai.table_name = new_schema->qual[new_num].table_name",
       " and dai.schema_date = cnvtdatetimeutc(new_schema->qual[new_num].max_schema_dt)",
       " and dai.owner = currdbuser)"," go")
      SET trace symbol mark
      IF ( NOT (dm2_push_cmd(insert_str,1)))
       RETURN(0)
      ELSE
      ...
      ENDIF
      SET trace symbol release
      SET dm_err->eproc = "Inserting schema snapshot rows to dm_index_columns"
      SET insert_str = " "
      SET insert_str = concat("insert into dm_index_columns",
       " (index_name, table_name, column_name, schema_date, column_position,",
       " updt_applctx, updt_dt_tm, updt_cnt, updt_id, updt_task)",
       " (select dai.index_name, dai.table_name, dai.column_name,",
       " cnvtdatetimeutc(dcds_schema_date),",
       " dai.column_position, dai.updt_applctx, dai.updt_dt_tm, dai.updt_cnt,",
       " dai.updt_id, dai.updt_task"," from dm_adm_index_columns dai",
       " where dai.table_name = new_schema->qual[new_num].table_name",
       " and dai.schema_date = cnvtdatetimeutc(new_schema->qual[new_num].max_schema_dt)",
       " and dai.owner = currdbuser) go")
      SET trace symbol mark
      IF ( NOT (dm2_push_cmd(insert_str,1)))
      ...
      ENDIF
      SET trace symbol release
      SET dm_err->eproc = "Inserting schema snapshot rows to dm_constraints"
      SET insert_str = " "
      SET insert_str = concat("insert into dm_constraints",
       " (table_name, schema_date, constraint_name, constraint_type,",
       " parent_table_name, status_ind, updt_applctx, updt_dt_tm,",
       " updt_cnt, updt_id, updt_task, parent_table_columns,"," r_constraint_name, delete_rule)",
       " (select dai.table_name, cnvtdatetimeutc(dcds_schema_date),"," dai.constraint_name,",
       " dai.constraint_type, dai.parent_table_name, dai.status_ind,",
       " dai.updt_applctx, dai.updt_dt_tm, dai.updt_cnt, dai.updt_id,",
       " dai.updt_task, dai.parent_table_columns, dai.r_constraint_name, dai.delete_rule",
       " from dm_adm_constraints dai"," where dai.table_name = new_schema->qual[new_num].table_name",
       " and dai.schema_date = cnvtdatetimeutc(new_schema->qual[new_num].max_schema_dt)",
       " and dai.owner = currdbuser)"," go")
      SET trace symbol mark
      IF ( NOT (dm2_push_cmd(insert_str,1)))
      ...
      ENDIF
      SET trace symbol release
      SET dm_err->eproc = "Inserting schema snapshot rows to dm_cons_columns"
      SET insert_str = " "
      SET insert_str = concat("insert into dm_cons_columns",
       " (table_name, schema_date, constraint_name, column_name, position,",
       " updt_applctx, updt_dt_tm, updt_cnt, updt_id, updt_task)",
       " (select dai.table_name, cnvtdatetimeutc(dcds_schema_date), dai.constraint_name,",
       " dai.column_name, dai.position, dai.updt_applctx, dai.updt_dt_tm,",
       " dai.updt_cnt,dai.updt_id, dai.updt_task"," from dm_adm_cons_columns dai",
       " where dai.table_name = new_schema->qual[new_num].table_name",
       " and dai.schema_date = cnvtdatetimeutc(new_schema->qual[new_num].max_schema_dt)",
   ...
   ELSE
    FOR (new_num = 1 TO size(new_schema->qual,5))
      ...
      ENDIF
      SET insert_str = " "
      SET dm_err->eproc = "Inserting schema snapshot rows for dm_tables"
      SET insert_str = concat(
       "insert into dm_tables (table_name, schema_date, tablespace_name, pct_increase,",
       " pct_used, pct_free, updt_applctx, updt_dt_tm, updt_cnt, updt_id, updt_task)",
       " (select dat.table_name, cnvtdatetimeutc(dcds_schema_date),"," dat.tablespace_name,",
       " dat.pct_increase, dat.pct_used, dat.pct_free, dat.updt_applctx, dat.updt_dt_tm,",
       " dat.alpha_feature_nbr, dat.updt_id, dat.updt_task "," from dm_afd_tables dat ",
       " where dat.table_name = new_schema->qual[new_num].table_name ",
       " and dat.alpha_feature_nbr = new_schema->qual[new_num].alpha_feature_nbr ",
       " and dat.owner = currdbuser) ",
       " go")
      SET trace symbol mark
      IF ( NOT (dm2_push_cmd(insert_str,1)))
      ...
      SET trace symbol release
      SET insert_str = " "
      SET dm_err->eproc = "Inserting schema snapshot rows for dm_columns"
      SET insert_str = concat("insert into dm_columns",
       " (table_name, column_name, schema_date, column_seq, data_type,",
       " data_length, data_precision, data_scale, nullable, updt_applctx,",
       " updt_dt_tm, updt_cnt, updt_id, updt_task, data_default, virtual_column)",
       ...
       " dac.data_type, dac.data_length, dac.data_precision, dac.data_scale, dac.nullable,",
       " dac.updt_applctx, dac.updt_dt_tm, dac.updt_cnt, dac.updt_id,",
       " dac.updt_task, dac.data_default, dac.virtual_column"," from dm_afd_columns dac",
       " where dac.table_name = new_schema->qual[new_num].table_name",
       " and dac.alpha_feature_nbr = new_schema->qual[new_num].alpha_feature_nbr",
       " and dac.owner = currdbuser)"," go")
      SET trace symbol mark
      IF ( NOT (dm2_push_cmd(insert_str,1)))
       RETURN(0)
      ...
      SET trace symbol release
      SET insert_str = " "
      SET dm_err->eproc = "Inserting schema snapshot rows for dm_indexes"
      SET insert_str = concat("insert into dm_indexes",
       " (index_name, schema_date, table_name, tablespace_name, pct_increase, pct_free,",
       " unique_ind, updt_applctx, updt_dt_tm, updt_cnt, updt_id, updt_task, index_type)",
       " (select dai.index_name, cnvtdatetimeutc(dcds_schema_date),",
       ...
       " dai.pct_increase, dai.pct_free, dai.unique_ind, dai.updt_applctx,",
       " dai.updt_dt_tm, dai.updt_cnt, dai.updt_id, dai.updt_task, dai.index_type",
       " from dm_afd_indexes dai"," where dai.table_name = new_schema->qual[new_num].table_name",
       " and dai.alpha_feature_nbr = new_schema->qual[new_num].alpha_feature_nbr",
       " and dai.owner = currdbuser)"," go")
      SET trace symbol mark
      IF ( NOT (dm2_push_cmd(insert_str,1)))
       RETURN(0)
      ...
      SET trace symbol release
      SET insert_str = " "
      SET dm_err->eproc = "Inserting schema snapshot rows for dm_index_columns"
      SET insert_str = concat("insert into dm_index_columns",
       " (index_name, table_name, column_name, schema_date, column_position,",
       " updt_applctx, updt_dt_tm, updt_cnt, updt_id, updt_task)",
       " (select dai.index_name, dai.table_name, dai.column_name,",
       " cnvtdatetimeutc(dcds_schema_date),",
       " dai.column_position, dai.updt_applctx, dai.updt_dt_tm, dai.updt_cnt,",
       " dai.updt_id, dai.updt_task"," from dm_afd_index_columns dai",
       " where dai.table_name = new_schema->qual[new_num].table_name",
       " and dai.alpha_feature_nbr = new_schema->qual[new_num].alpha_feature_nbr",
       " and dai.owner = currdbuser)"," go")
      SET trace symbol mark
      ...
      SET trace symbol release
      SET insert_str = " "
      SET dm_err->eproc = "Inserting schema snapshot rows for dm_constraints"
      SET insert_str = concat("insert into dm_constraints",
       " (table_name, schema_date, constraint_name, constraint_type,",
       " parent_table_name, status_ind, updt_applctx, updt_dt_tm,",
       " updt_cnt, updt_id, updt_task, parent_table_columns,"," r_constraint_name, delete_rule)",
       ...
       " dai.constraint_type, dai.parent_table_name, dai.status_ind,",
       " dai.updt_applctx, dai.updt_dt_tm, dai.updt_cnt, dai.updt_id,",
       " dai.updt_task, dai.parent_table_columns, dai.r_constraint_name,",
       " dai.delete_rule"," from dm_afd_constraints dai",
       " where dai.table_name = new_schema->qual[new_num].table_name",
       " and dai.alpha_feature_nbr = new_schema->qual[new_num].alpha_feature_nbr",
       " and dai.owner = currdbuser)",
       " go")
      ...
      SET trace symbol release
      SET insert_str = " "
      SET dm_err->eproc = "Inserting schema snapshot rows for dm_cons_columns"
      SET insert_str = concat("insert into dm_cons_columns",
       " (table_name, schema_date, constraint_name, column_name, position,",
       " updt_applctx, updt_dt_tm, updt_cnt, updt_id, updt_task)",
       " (select dai.table_name, cnvtdatetimeutc(dcds_schema_date), dai.constraint_name,",
       " dai.column_name, dai.position, dai.updt_applctx, dai.updt_dt_tm,",
       " dai.updt_cnt,dai.updt_id, dai.updt_task"," from dm_afd_cons_columns dai",
       " where dai.table_name = new_schema->qual[new_num].table_name",
       " and dai.alpha_feature_nbr = new_schema->qual[new_num].alpha_feature_nbr",
       " and dai.owner = currdbuser)",
       " go")
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE verify_dminfo_schemainstance(null)
   SET dm_err->eproc = "Verify schema instance row on dm_info table exists."
   DECLARE dvsvds_invalid_list = vc WITH protect, noconstant(" ")
   DECLARE vds_sel_cnt = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    a.table_name
    ...
    PLAN (a)
     ...
     WHERE b.table_name=c.table_name
      ...
      AND c.alpha_feature_nbr > 0
      AND  NOT ( EXISTS (
     (SELECT
      "x"
      FROM dm_info d
      WHERE d.info_domain="DM2_SCHEMA_INSTANCE"
       AND d.info_name=c.table_name))))
     JOIN (e
     ...
     (SELECT
      ...
      WHERE i.info_domain="DATA MANAGEMENT"
       AND i.info_name="DM_ENV_ID"))
      AND c.alpha_feature_nbr=e.alpha_feature_nbr)
    ORDER BY a.table_name
    HEAD a.table_name
     dvsvds_invalid_list = concat(dvsvds_invalid_list,trim(a.table_name),",")
    FOOT REPORT
     dvsvds_invalid_list = replace(dvsvds_invalid_list,",","",2)
    WITH nocounter
   ;end select
   SET vds_sel_cnt = curqual
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (vds_sel_cnt > 0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Schema Instance rows were found missing for tables(",
     dvsvds_invalid_list,").","Please contact Cerner immediately to resolve this issue.",
     "Packages should not be installed into this environment until the missing schema instance rows have been resolved."
     )
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
...
#exit_script
 IF ((validate(dm2_allow_cap_desired_schema_ind,- (1)) != - (1)))
  ...
  IF (dcds_process_log_id > 0.0)
   ...
   SET dcds_tmp_err_ind = dm_err->err_ind
   SET dm_err->err_ind = 0
   CALL dm2_process_log_row(dpl_cap_desired_schema,dpl_execution,dcds_process_log_id,1)
   SET dm_err->err_ind = dcds_tmp_err_ind
  ENDIF
  IF ((dm_err->err_ind=0))
   SET dm_err->eproc = "DM2_CAPTURE_DESIRED_SCHEMA completed successfully"
   CALL final_disp_msg("dm2_capture_desired_schema")
  ENDIF
 ENDIF
 FREE RECORD new_schema
END GO
