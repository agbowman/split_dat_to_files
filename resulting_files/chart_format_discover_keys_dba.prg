CREATE PROGRAM chart_format_discover_keys:dba
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CHART_FORMAT_DISCOVER_KEYS"
 FREE RECORD reply
 RECORD reply(
   1 keys[*]
     2 key_id = vc
 ...
 CALL log_message("Entering chart_format_discover_keys script",log_level_debug)
 SET reply->status_data.status = "F"
 ...
 SUBROUTINE loadchartformats(null)
   ...
   SELECT INTO "nl:"
    ...
    WHERE cf.updt_dt_tm > cnvtdatetime(request->since)
 ...
 SUBROUTINE loadchartformsects(null)
   ...
   SELECT INTO "nl:"
    ...
    WHERE cfs.updt_dt_tm > cnvtdatetime(request->since)
    ...
    HEAD cfs.chart_format_id
     ...
     IF (format_loc=0)
      ...
      IF (ncount > size(reply->keys,5))
       stat = alterlist(reply->keys,(ncount+ 9))
      ENDIF
      reply->keys[ncount].changed = cfs.updt_dt_tm, reply->keys[ncount].key_id = string_format_id
     ENDIF
    DETAIL
     donothing = 0
    FOOT  cfs.chart_format_id
     donothing = 0
    FOOT REPORT
     stat = alterlist(reply->keys,ncount)
    WITH nocounter
 ...
 END ;Subroutine
#exit_script
 CALL log_message("Exiting chart_format_discover_keys script",log_level_debug)
