CREATE PROGRAM dm2_aload_application
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_app",".log","dm2_aload_application")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_application"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daa_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daa_file = vc WITH protect, noconstant( $2)
 DECLARE daa_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE daa_batch = i4 WITH protect, noconstant(0)
 DECLARE daa_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE daa_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daa_pkg,daa_file_loc_ret,daa_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daa_batch=0)
  SET daa_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_OCD_APPLICATION for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND daa_rows_returned=10000)
   DELETE  FROM dm_ocd_application d
    WHERE d.alpha_feature_nbr=daa_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET daa_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",daa_rows_returned))
   ENDIF
 ENDWHILE
 IF (daa_row_cnt > 0)
  EXECUTE dm2_dbimport concat(daa_file_loc_ret,daa_file), "dm2_aload_atr_app", daa_batch
  CALL echo(concat(daa_file_loc_ret,daa_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_application completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_application")
END GO
