CREATE PROGRAM bhs_rpt_surg_sched_mul_tst:dba
 PROMPT
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_RPT_SURG_SCHED_MUL"
 ...
 SELECT INTO "nl:"
  ...
  HEAD sc.surg_case_nbr_formatted
   m_case->l_cnt = (m_case->l_cnt+ 1), stat = alterlist(m_case->qual,m_case->l_cnt), m_case->qual[
   m_case->l_cnt].s_location = uar_get_code_display(sc.sched_surg_area_cd),
   ...
   m_case->qual[m_case->l_cnt].s_pat_name = p.name_full_formatted, m_case->qual[m_case->l_cnt].
   s_pat_type = uar_get_code_display(sc.sched_pat_type_cd), m_case->qual[m_case->l_cnt].s_cmrn = pa
   .alias,
   m_case->qual[m_case->l_cnt].s_mrn = trim(ea.alias), m_case->qual[m_case->l_cnt].s_pat_dob = format
   (p.birth_dt_tm,"DD-MMM-YYYY;;D"), m_case->qual[m_case->l_cnt].f_surg_case_id = sc.surg_case_id,
   m_case->qual[m_case->l_cnt].s_latex_allergy = trim(sed.oe_field_display_value,3)
 ...
 SELECT INTO "nl:"
  ...
  HEAD scp.surg_case_proc_id
   m_case->qual[ml_idx2].p_cnt = (m_case->qual[ml_idx2].p_cnt+ 1), stat = alterlist(m_case->qual[
    ml_idx2].proc,m_case->qual[ml_idx2].p_cnt), m_case->qual[ml_idx2].proc[m_case->qual[ml_idx2].
   p_cnt].s_proc_name = oc.primary_mnemonic,
   ...
   ml_idx2].l_tot_dur = (m_case->qual[ml_idx2].l_tot_dur+ scp.sched_dur)
  FOOT  scp.surg_case_proc_id
   null
  FOOT  scp.surg_case_id
   m_case->qual[ml_idx2].l_tot_dur = (m_case->qual[ml_idx2].l_tot_dur+ m_case->qual[ml_idx2].
   s_cleanup_tm)
  WITH nocounter
 ;end select
