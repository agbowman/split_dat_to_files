CREATE PROGRAM dm2_make_merge_trans_tbl:dba
 ...
 SET mrg_tbl_name = cnvtupper( $1)
 SET mrg_src_env_id =  $2
 SET mrg_tgt_env_id =  $3
 SET mrg_src_table = cnvtupper( $4)
 WHILE (con_flag=0)
  INSERT  FROM dm_merge_translate
   (env_source_id, env_target_id, table_name,
   from_value, to_value)(SELECT
    mrg_src_env_id, mrg_tgt_env_id, dmt1.table_name,
    dmt1.from_value, dmt1.to_value
    FROM dm_merge_translate dmt1
    WHERE dmt1.table_name=mrg_tbl_name
     AND dmt1.env_source_id=0
     AND dmt1.env_target_id=0
     AND  NOT ( EXISTS (
    (SELECT
     "x"
     FROM dm_merge_translate dmt3
     WHERE dmt3.table_name=dmt1.table_name
      AND dmt3.env_source_id=mrg_src_env_id
      AND dmt3.env_target_id=mrg_tgt_env_id
      AND dmt3.from_value=dmt1.from_value)))
     AND  EXISTS (
    (SELECT
     "x"
     FROM (value(mrg_src_table) dmt2)
     WHERE dmt2.table_name=dmt1.table_name
      AND dmt2.env_source_id=0
      AND dmt2.env_target_id=0
      AND dmt2.from_value=dmt1.from_value))
    WITH maxqual(dmt1,1000))
  ;end insert
 ...
 ENDWHILE
#exit_program
 FREE SET mrg_tbl_name
 FREE SET mrg_src_env_id
 FREE SET mrg_tgt_env_id
 FREE SET mrg_src_table
END GO
