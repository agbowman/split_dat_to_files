CREATE PROGRAM djh_ma_phys_sn_90_day_nologin
 PROMPT
 ...
 SELECT DISTINCT INTO value(output_dest)
  ...
  HEAD pr.name_last
   ...
   IF ( NOT (curendreport))
    row + 1
   ENDIF
  WITH format = variable, formfeed = none, maxrec = 20
 ;end select
 IF (email_ind=1)
  SET filename_in = trim(concat(output_dest,".dat"))
  SET filename_out = concat(format(curdate,"YYYYMMDD ;;D"),"-PHYS.csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog,"Vx - Baystate Health CIS PHYS Acnts Inactive 90 days")
