CREATE PROGRAM dm_rmc_bad_vers:dba
 ...
 SUBROUTINE drcm_get_pk_str(dgps_rec,dgps_tab_name)
   ...
   RETURN(dgps_ret_str)
 END ;Subroutine
 CALL check_logfile("dm_rmc_bad_vers",".log","DM_RMC_BAD_VERS")
 ...
 ENDIF
 SET reply->status_data.status = get_bad_pkw_vers(request->target_id,request->log_id,bad_pkw_rs)
 CALL echorecord(request)
 CALL echorecord(bad_pkw_rs)
