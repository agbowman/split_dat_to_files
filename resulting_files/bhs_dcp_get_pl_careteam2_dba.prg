CREATE PROGRAM bhs_dcp_get_pl_careteam2:dba
 RECORD orgs(
   1 qual[*]
     2 org_id = f8
     2 confid_level = i4
 ...
 SUBROUTINE performorgsecurity(x)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (d
     WHERE (dgp_reply->patients[d.seq].filter_ind=1))
    ORDER BY dgp_reply->patients[d.seq].organization_id
    HEAD REPORT
     orgid = 0.0, temp_cnt = 0
    DETAIL
     IF ((dgp_reply->patients[d.seq].organization_id != orgid))
      temp_cnt = (temp_cnt+ 1)
      IF (mod(temp_cnt,10)=1)
       stat = alterlist(orgs->temp,(temp_cnt+ 9))
      ENDIF
      orgs->temp[temp_cnt].org_id = dgp_reply->patients[d.seq].organization_id
     ENDIF
    WITH nocounter
   ;end select
   IF (temp_cnt > 0)
    SELECT INTO "nl:"
     FROM prsnl_org_reltn por
     WHERE (por.person_id=reqinfo->updt_id)
      AND expand(b,1,temp_cnt,por.organization_id,orgs->temp[b].org_id)
      AND por.active_ind=1
      AND por.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)
      AND por.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
     HEAD REPORT
     ...
     DETAIL
      ...
      orgs->qual[org_cnt].org_id = por.organization_id
      IF (confid_ind=1
       AND por.confid_level_cd > 0)
       orgs->qual[org_cnt].confid_level = maxval(0,uar_get_collation_seq(por.confid_level_cd))
      ELSE
       orgs->qual[org_cnt].confid_level = 0
      ENDIF
     WITH nocounter
   ...
   IF (org_cnt > 0)
    SELECT INTO "nl:"
     ...
     PLAN (d
      WHERE (dgp_reply->patients[d.seq].filter_ind=1))
      JOIN (d2
      WHERE (orgs->qual[d2.seq].org_id=dgp_reply->patients[d.seq].organization_id))
     DETAIL
      IF (((confid_ind=0) OR ((orgs->qual[d2.seq].confid_level >= dgp_reply->patients[d.seq].
      confid_level))) )
       dgp_reply->patients[d.seq].filter_ind = 0
      ENDIF
     WITH nocounter
    ;end select
   ENDIF
 ...
 SUBROUTINE performrelationshipoverrides(x)
   ...
   IF (reltncnt > 0)
    SELECT INTO "nl:"
     ...
     PLAN (d
      WHERE (dgp_reply->patients[d.seq].filter_ind=1))
     ORDER BY dgp_reply->patients[d.seq].person_id
     HEAD REPORT
      temp->entity_cnt = 0
     DETAIL
      IF ((((temp->entity_cnt=0)) OR ((temp->entities[temp->entity_cnt].entity_id != dgp_reply->
      patients[d.seq].person_id))) )
       temp->entity_cnt = (temp->entity_cnt+ 1)
       IF (mod(temp->entity_cnt,100)=1)
        stat = alterlist(temp->entities,(temp->entity_cnt+ 99))
       ENDIF
       temp->entities[temp->entity_cnt].entity_id = dgp_reply->patients[d.seq].person_id
      ENDIF
     WITH nocounter
    ;end select
     ...
     IF ((temp->person_cnt > 0))
      SELECT INTO "nl:"
       ...
       PLAN (d1
        ...
        JOIN (d2
        WHERE (dgp_reply->patients[d2.seq].person_id=temp->persons[d1.seq].person_id)
         AND (dgp_reply->patients[d2.seq].filter_ind=1))
       DETAIL
        IF ((((temp->persons[d1.seq].override_level=2)) OR ((dgp_reply->patients[d2.seq].confid_level
        =0))) )
         dgp_reply->patients[d2.seq].filter_ind = 0
        ENDIF
       WITH nocounter
      ;end select
     ENDIF
   ...
   SELECT INTO "nl:"
    ...
    PLAN (d
     WHERE (dgp_reply->patients[d.seq].filter_ind=1)
      AND (dgp_reply->patients[d.seq].encntr_id > 0.0))
    ORDER BY dgp_reply->patients[d.seq].encntr_id
    HEAD REPORT
     temp->entity_cnt = 0
    DETAIL
     temp->entity_cnt = (temp->entity_cnt+ 1)
     IF (mod(temp->entity_cnt,100)=1)
      stat = alterlist(temp->entities,(temp->entity_cnt+ 99))
     ENDIF
     temp->entities[temp->entity_cnt].entity_id = dgp_reply->patients[d.seq].encntr_id, temp->
     entities[temp->entity_cnt].entity_index = d.seq
    WITH nocounter
   ;end select
   IF ((temp->entity_cnt > 0))
    SET j = 1
    WHILE ((j <= temp->entity_cnt))
      ...
      SELECT INTO "nl:"
       ...
       HEAD epr.encntr_id
        ...
        IF (i > 0)
         dgp_reply->patients[temp->entities[i].entity_index].filter_ind = 0
        ENDIF
       WITH nocounter
      ;end select
      SET j = (j+ 200)
 ...
 SUBROUTINE removefilteredpatients(x)
   ...
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(patient_cnt))
    PLAN (d
     WHERE (dgp_reply->patients[d.seq].filter_ind=0))
    HEAD REPORT
     actual_cnt = 0
    DETAIL
     actual_cnt = (actual_cnt+ 1), dgp_reply->patients[actual_cnt].person_id = dgp_reply->patients[d
     .seq].person_id, dgp_reply->patients[actual_cnt].person_name = dgp_reply->patients[d.seq].
     person_name,
     dgp_reply->patients[actual_cnt].encntr_id = dgp_reply->patients[d.seq].encntr_id, dgp_reply->
     patients[actual_cnt].priority = dgp_reply->patients[d.seq].priority, dgp_reply->patients[
     actual_cnt].active_ind = dgp_reply->patients[d.seq].active_ind,
     dgp_reply->patients[actual_cnt].organization_id = dgp_reply->patients[d.seq].organization_id,
     dgp_reply->patients[actual_cnt].confid_level_cd = dgp_reply->patients[d.seq].confid_level_cd,
     dgp_reply->patients[actual_cnt].confid_level = dgp_reply->patients[d.seq].confid_level,
     dgp_reply->patients[actual_cnt].filter_ind = dgp_reply->patients[d.seq].filter_ind
    FOOT REPORT
     patient_cnt = actual_cnt, stat = alterlist(dgp_reply->patients,patient_cnt)
    WITH nocounter
   ;end select
   SET patient_cnt = actual_cnt
   SET stat = alterlist(dgp_reply->patients,patient_cnt)
 END ;Subroutine
 SUBROUTINE parseencounterbyencntr(condition,usecensuscritieria)
   ...
   IF ((criteria->admit_flag > 0))
    CALL parser(
     " and e.reg_dt_tm between cnvtdatetime(criteria->admit_dt_tm) and cnvtdatetime(curdate,curtime)"
     )
   ENDIF
   IF ((criteria->discharge_flag=1))
 ...
 SUBROUTINE parseencntrlochistbyencntr(alias)
   ...
   ELSEIF ((criteria->locations.unit_cnt > 0))
    ...
    CALL parser(" criteria->locations.units[locX].unit_cd)")
   ELSEIF ((critieria->locations.building_cnt > 0))
   ...
   IF ((request->patient_list_id > 0)
    ...
    SELECT INTO "nl:"
     ...
     PLAN (pr
      WHERE (pr.patient_list_id=request->patient_list_id)
       AND expand(m,1,patient_cnt,(pr.person_id+ 0),dgp_reply->patients[m].person_id))
     DETAIL
    ...
    FOR (j = 1 TO temp->person_cnt)
      FOR (i = 1 TO patient_cnt)
        IF ((dgp_reply->patients[i].person_id=temp->persons[j].person_id))
         SET dgp_reply->patients[i].priority = temp->persons[j].priority
        ENDIF
      ENDFOR
    ENDFOR
   ENDIF
 ...
 SUBROUTINE performbestencntr(n)
   ...
   DECLARE patcount = i4 WITH constant(size(dgp_reply->patients,5)), private
   ...
   FOR (x = 1 TO patcount)
     IF ((dgp_reply->patients[x].encntr_id=0))
      SET encntrrequest->persons[x].person_id = dgp_reply->patients[x].person_id
      SET encntrcnt = (encntrcnt+ 1)
     ENDIF
   ENDFOR
   IF (encntrcnt > 0)
    SET stat = alterlist(encntrrequest->persons,encntrcnt)
    EXECUTE pts_get_best_encntr_list  WITH replace(request,encntrrequest), replace(dgp_reply,
     encntrreply)
    SET encntr_cnt = size(encntrreply->encounters,5)
    FOR (x = 1 TO encntrcnt)
      FOR (y = 1 TO patcount)
        IF ((dgp_reply->patients[y].person_id=encntrreply->encounters[x].person_id)
         AND (dgp_reply->patients[y].encntr_id=0))
         SET dgp_reply->patients[y].encntr_id = encntrreply->encounters[x].encntr_id
         SET y = (patcount+ 1)
        ENDIF
      ENDFOR
    ENDFOR
 ...
 FREE SET criteria
 IF (patient_cnt=0)
  SET dgp_reply->status_data.status = "Z"
 ELSE
  SET dgp_reply->status_data.status = "S"
 ENDIF
 SUBROUTINE executeprimaryselect(x)
   ...
   CALL parser("stat = alterlist (DGP_REPLY->patients, patient_cnt + 49)")
   CALL parser("endif")
   CALL parser("DGP_REPLY->patients[patient_cnt].person_id = P.person_id")
   CALL parser("DGP_REPLY->patients[patient_cnt].person_name = P.name_full_formatted")
   CALL parser("DGP_REPLY->patients[patient_cnt].encntr_id = E.encntr_id")
   CALL parser("DGP_REPLY->patients[patient_cnt].priority = 0")
   CALL parser("DGP_REPLY->patients[patient_cnt].active_ind = 1")
   CALL parser("DGP_REPLY->patients[patient_cnt].organization_id = E.organization_id")
   CALL parser("DGP_REPLY->patients[patient_cnt].confid_level_cd = E.confid_level_cd")
   CALL parser(
    "DGP_REPLY->patients[patient_cnt].confid_level = uar_get_collation_seq(E.confid_level_cd)")
   IF ((dgp_reply->patients[patient_cnt].confid_level < 0))
    CALL parser("DGP_REPLY->patients[patient_cnt].confid_level = 0")
   ENDIF
   IF (encntr_org_sec_ind=0)
    CALL parser("DGP_REPLY->patients[patient_cnt].filter_ind = 0")
   ELSE
    CALL parser("DGP_REPLY->patients[patient_cnt].filter_ind = 1")
   ENDIF
