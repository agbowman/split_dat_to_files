CREATE PROGRAM dm_rrd_bridge_rows:dba
 ...
 SET reply->table_name = "RRDBRIDGE"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(b.rrdbridge_id)
   FROM rrdbridge b
   WHERE b.rrdbridge_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    b.rowid
    FROM rrdbridge b
    WHERE parser(sbr_getrowidnotexists("b.rrdbridge_id between curMinID and curMaxID","b"))
