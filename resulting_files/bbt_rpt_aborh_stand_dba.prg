CREATE PROGRAM bbt_rpt_aborh_stand:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->abo_rh = uar_i18ngetmessage(i18nhandle,"abo_rh","ABORh BUILDING TOOL")
 SET captions->standard_abo = uar_i18ngetmessage(i18nhandle,"standard_abo",
  "STANDARD ABO/Rh PARAMETERS")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_aborh_stand", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  cv.code_value, cv.display, cv.description,
  cv.collation_seq, cv.active_ind, ce.field_value"####################"
  FROM code_value cv,
  ...
  HEAD PAGE
   ...
   col 50, captions->abo_rh, row + 1,
   col 48, captions->standard_abo, row + 1,
   line = fillstring(122,"-"), line, row + 1,
   col 4, captions->display_name, col 30,
   captions->description, col 60, captions->barcode,
   col 85, captions->active, row + 1,
   line, row + 1
  DETAIL
   col 2, cv.display, col 20,
   cv.description, col 62, ce.field_value
   IF (cv.active_ind=1)
    col 87, captions->yes
