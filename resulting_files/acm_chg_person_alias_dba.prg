CREATE PROGRAM acm_chg_person_alias:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->person_alias_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   person_alias p
  SET p.person_alias_id = acm_request->person_alias_qual[xref->chg[d.seq].idx].person_alias_id, p
   .alias = cnvtupper(cnvtalphanum(acm_request->person_alias_qual[xref->chg[d.seq].idx].alias)), p
   .alias_pool_cd = acm_request->person_alias_qual[xref->chg[d.seq].idx].alias_pool_cd,
   p.assign_authority_sys_cd = acm_request->person_alias_qual[xref->chg[d.seq].idx].
   assign_authority_sys_cd, p.beg_effective_dt_tm = cnvtdatetime(acm_request->person_alias_qual[xref
    ->chg[d.seq].idx].beg_effective_dt_tm), p.check_digit = acm_request->person_alias_qual[xref->chg[
   d.seq].idx].check_digit,
   p.check_digit_method_cd = acm_request->person_alias_qual[xref->chg[d.seq].idx].
   check_digit_method_cd, p.contributor_system_cd =
   IF ((acm_request->person_alias_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->person_alias_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , p.end_effective_dt_tm = cnvtdatetime(acm_request->person_alias_qual[xref->chg[d.seq].idx].
    end_effective_dt_tm),
   p.health_card_expiry_dt_tm =
   IF ((acm_request->person_alias_qual[xref->chg[d.seq].idx].health_card_expiry_dt_tm > 0))
    cnvtdatetime(acm_request->person_alias_qual[xref->chg[d.seq].idx].health_card_expiry_dt_tm)
   ELSE null
   ENDIF
   , p.health_card_issue_dt_tm =
   IF ((acm_request->person_alias_qual[xref->chg[d.seq].idx].health_card_issue_dt_tm > 0))
    cnvtdatetime(acm_request->person_alias_qual[xref->chg[d.seq].idx].health_card_issue_dt_tm)
   ELSE null
   ENDIF
   , p.health_card_province = acm_request->person_alias_qual[xref->chg[d.seq].idx].
   health_card_province,
   p.health_card_type = acm_request->person_alias_qual[xref->chg[d.seq].idx].health_card_type, p
   .health_card_ver_code = acm_request->person_alias_qual[xref->chg[d.seq].idx].health_card_ver_code,
   p.person_alias_status_cd = acm_request->person_alias_qual[xref->chg[d.seq].idx].
   person_alias_status_cd,
   p.person_alias_sub_type_cd = acm_request->person_alias_qual[xref->chg[d.seq].idx].
   person_alias_sub_type_cd, p.person_alias_type_cd = acm_request->person_alias_qual[xref->chg[d.seq]
   .idx].person_alias_type_cd, p.person_id =
   IF ((acm_request->person_alias_qual[xref->chg[d.seq].idx].person_id > 0)) acm_request->
    person_alias_qual[xref->chg[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->person_alias_qual[xref->chg[d.seq].idx].person_idx].person_id
   ENDIF
   ,
   p.visit_seq_nbr = acm_request->person_alias_qual[xref->chg[d.seq].idx].visit_seq_nbr, p.active_ind
    = acm_request->person_alias_qual[xref->chg[d.seq].idx].active_ind, p.active_status_cd =
   acm_request->person_alias_qual[xref->chg[d.seq].idx].active_status_cd,
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.person_alias_id=acm_request->person_alias_qual[xref->chg[d.seq].idx].person_alias_id))
  WITH nocounter, status(reply->person_alias_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->person_alias_qual[xref->chg[index].idx].status != 1))
 ...
 ENDFOR
 IF (acm_hist_ind=1)
  EXECUTE acm_chg_person_alias_hist
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM person_alias p
    WHERE expand(t1,f_val,t_val,p.person_alias_id,acm_request->person_alias_qual[xref->chg[t1].idx].
     person_alias_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.person_alias_id,acm_request->person_alias_qual[xref->chg[t1].idx
      ].person_alias_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->person_alias_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->person_alias_qual[idx].status = - (1), reply->person_alias_qual[idx].person_alias_id =
      acm_request->person_alias_qual[idx].person_alias_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->person_alias_qual[idx].chg_str
     IF (findstring("ALIAS,",chg_str)=0)
      acm_request->person_alias_qual[idx].alias = p.alias
     ENDIF
     IF (findstring("ALIAS_POOL_CD,",chg_str)=0)
      acm_request->person_alias_qual[idx].alias_pool_cd = p.alias_pool_cd
     ENDIF
     IF (findstring("ASSIGN_AUTHORITY_SYS_CD,",chg_str)=0)
      acm_request->person_alias_qual[idx].assign_authority_sys_cd = p.assign_authority_sys_cd
     ENDIF
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->person_alias_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("CHECK_DIGIT,",chg_str)=0)
      acm_request->person_alias_qual[idx].check_digit = p.check_digit
     ENDIF
     IF (findstring("CHECK_DIGIT_METHOD_CD,",chg_str)=0)
      acm_request->person_alias_qual[idx].check_digit_method_cd = p.check_digit_method_cd
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->person_alias_qual[idx].contributor_system_cd = p.contributor_system_cd
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->person_alias_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("HEALTH_CARD_EXPIRY_DT_TM,",chg_str)=0)
      acm_request->person_alias_qual[idx].health_card_expiry_dt_tm = p.health_card_expiry_dt_tm
     ENDIF
     IF (findstring("HEALTH_CARD_ISSUE_DT_TM,",chg_str)=0)
      acm_request->person_alias_qual[idx].health_card_issue_dt_tm = p.health_card_issue_dt_tm
     ENDIF
     IF (findstring("HEALTH_CARD_PROVINCE,",chg_str)=0)
      acm_request->person_alias_qual[idx].health_card_province = p.health_card_province
     ENDIF
     IF (findstring("HEALTH_CARD_TYPE,",chg_str)=0)
      acm_request->person_alias_qual[idx].health_card_type = p.health_card_type
     ENDIF
     IF (findstring("HEALTH_CARD_VER_CODE,",chg_str)=0)
      acm_request->person_alias_qual[idx].health_card_ver_code = p.health_card_ver_code
     ENDIF
     IF (findstring("PERSON_ALIAS_STATUS_CD,",chg_str)=0)
      acm_request->person_alias_qual[idx].person_alias_status_cd = p.person_alias_status_cd
     ENDIF
     IF (findstring("PERSON_ALIAS_SUB_TYPE_CD,",chg_str)=0)
      acm_request->person_alias_qual[idx].person_alias_sub_type_cd = p.person_alias_sub_type_cd
     ENDIF
     IF (findstring("PERSON_ALIAS_TYPE_CD,",chg_str)=0)
      acm_request->person_alias_qual[idx].person_alias_type_cd = p.person_alias_type_cd
     ENDIF
     IF (findstring("PERSON_ID,",chg_str)=0)
      acm_request->person_alias_qual[idx].person_id = p.person_id
     ENDIF
     IF (findstring("VISIT_SEQ_NBR,",chg_str)=0)
      acm_request->person_alias_qual[idx].visit_seq_nbr = p.visit_seq_nbr
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->person_alias_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->person_alias_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->person_alias_qual[xref->chg[index].idx].status=0))
