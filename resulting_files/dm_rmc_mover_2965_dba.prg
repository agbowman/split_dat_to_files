CREATE PROGRAM dm_rmc_mover_2965:dba
 ...
 DECLARE cust_col_ndx = i4
 DECLARE cust_err_val = i4
 DECLARE cust_ref_type_cd = vc WITH protect, noconstant
 DECLARE cust_src_cv_tab = vc WITH protect, noconstant
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_col_ndx = get_col_pos(cust_tab_name,"ASSOC_CODE_VALUE")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_ndx].translated = 0
 SET from_cd = get_value(cust_tab_name,"ASSOC_CODE_VALUE","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_2965
 ENDIF
 IF (cnvtreal(from_cd) > 0)
  SET cust_ref_type_cd = get_value(cust_tab_name,"TRACKING_REF_TYPE_CD","FROM")
  SET cust_src_cv_tab = dm2_get_rdds_tname("CODE_VALUE")
  SET dm_err->eproc = "Determining if ASSOC_CODE_VALUE is CODE_VALUE or TRACK_EVENT_ID"
  SELECT INTO "NL:"
   FROM (parser(cust_src_cv_tab) cv)
   WHERE cv.code_value=cnvtreal(cust_ref_type_cd)
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg =
   "Could not determine if assoc_code_value is track_event_id or code_value."
   GO TO exit_2965
  ENDIF
  IF (curqual > 0)
   SET dm_err->eproc = "Translating ASSOC_CODE_VALUE as TRACK_EVENT_ID"
   SET new_cd = select_merge_translate(from_cd,"TRACK_EVENT")
   IF (isnumeric(new_cd))
    CALL put_value(cust_tab_name,"ASSOC_CODE_VALUE",new_cd)
    IF ((dm2_ref_data_reply->error_ind=1))
     GO TO exit_2965
    ENDIF
   ELSE
    SET cust_rpt_missing = report_missing("TRACK_EVENT","TRACK_EVENT_ID",cnvtreal(from_cd))
    CALL evaluate_rpt_missing(cust_rpt_missing)
   ENDIF
  ELSE
   SET dm_err->eproc = "Translating ASSOC_CODE_VALUE as CODE_VALUE"
  ...
  ENDIF
 ELSE
  CALL put_value(cust_tab_name,"ASSOC_CODE_VALUE",from_cd)
  IF ((dm2_ref_data_reply->error_ind=1))
   GO TO exit_2965
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"TRACKING_REF_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1
   AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_ndx].translated=1))
   SET val = query_target(temp_tbl_cnt,perm_col_cnt)
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"TRACKING_REF_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"TRACKING_REF_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  TRACKING_REF_ID")
     ELSE
      GO TO exit_2965
     ENDIF
    ELSE
   ...
   SET from_cd = get_value(cust_tab_name,"TRACKING_REF_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_2965
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(from_cd)
   ENDIF
   SET new_cd = get_value(cust_tab_name,"TRACKING_REF_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_2965
   ELSE
 ...
 ENDIF
 IF (idcd_check=1)
  SET idcd_check = is_translated(cust_tab_name,"ASSOC_CODE_VALUE")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the ASSOC_CODE_VALUE"
