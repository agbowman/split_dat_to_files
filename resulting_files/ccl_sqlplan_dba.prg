CREATE PROGRAM ccl_sqlplan:dba
 PROMPT
  "Enter output device (MINE/file/printer):" = "MINE",
  "Enter the number of queries to be analyzed:" = 25,
  "Enter in the name of the script to be analyzed:" = "*",
  "Enter sort criteria (1=Buffer Gets,2=Disk Reads,3=Buffer Ratio,4=Executes,5=Score):" = 0,
  "Enter Oracle Optimizer mode (C = Cost, R = Rule):" = "R"
 ...
 ELSE
  CALL parser("rdb alter session set optimizer_mode = choose go")
 ENDIF
 ...
 IF (currdb="ORACLE")
  ...
  DECLARE call_prog = c15
  DECLARE file_name = vc
  SET call_prog = "CCL_SQLPLAN"
  CALL echo("Begin CCL_SQLPLAN...")
  FREE SET sql_text
  RECORD sql_text(
    1 text_cnt = i4
    1 qual[*]
  ...
  IF (_scriptname > " ")
   SET script_name = build("*",cnvtupper(trim(_scriptname)),"*")
  ELSE
   SET script_name = "*"
  ENDIF
  SET load_time = ""
  ...
  SELECT INTO "nl:"
   ...
   WHERE a.buffer_gets > 0
    ...
    AND ((a.sql_text=patstring(script_name)) OR (((a.sql_text=patstring(cnvtupper(script_name))) OR (
   a.sql_text=patstring(cnvtlower(script_name)))) ))
   ...
   DETAIL
    ...
    IF (script_cnt <= script_num)
     sql_text->text_cnt += 1, stat = alterlist(sql_text->qual,sql_text->text_cnt), sql_text->qual[
     sql_text->text_cnt].piece_cnt = 0,
  ...
  IF (size(sql_text->qual,5) > 0)
   SELECT INTO "nl:"
    ...
    HEAD t.hash_value
     pcnt = 0
    DETAIL
     pcnt += 1, stat = alterlist(sql_text->qual[d.seq].qual,pcnt), sql_text->qual[d.seq].qual[pcnt].
     text = t.sql_text
    FOOT  t.hash_value
     FOR (z = 1 TO pcnt)
       need_a_space = 0
       ...
       ENDIF
       IF (need_a_space=1)
        sql_text->qual[d.seq].stmt = concat(sql_text->qual[d.seq].stmt," ",sql_text->qual[d.seq].
         qual[z].text)
       ELSE
...
#sqlplan_done
  ...
  FOR (cntx = 1 TO sql_text->text_cnt)
    SELECT INTO "nl:"
     stmt_nbr = seq(reference_seq,nextval)
     FROM dual
     HEAD REPORT
      cnts = 0
     DETAIL
      cnts += 1
      IF (cnts <= script_num)
       stmt->stmt_cnt += 1, stat = alterlist(stmt->qual,stmt->stmt_cnt), stmt->qual[stmt->stmt_cnt].
       stmt_id = trim(cnvtstring(stmt_nbr))
      ENDIF
      stmt_id_nbr = stmt_nbr
     WITH nocounter
    ;end select
    SET stmt_id_str = trim(cnvtstring(stmt_id_nbr))
    DELETE  FROM dm_sql_plan
     WHERE (statement_id=stmt->qual[stmt->stmt_cnt].stmt_id)
     WITH nocounter
    ;end delete
    IF ((sql_text->qual[cntx].stmt_flag != 1))
     ...
     IF (size(dgui_short_stmts->qual,5) > 0)
      ...
      ;end select
      CALL parser(concat('rdb read "',trim(file_name,3),'" end go'),1)
     ENDIF
     FREE SET errorcode
     ...
     SET errors->list[cntx].err_msg = errmsg
    ENDIF
  ENDFOR
  FREE SET ind_rec
  RECORD ind_rec(
  ...
  SET ind_rec->ind_cnt = 0
  SET tempstr2 = fillstring(255," ")
  SET id_cnt = 0
  SET i = 0
  SET num_ind = size(stmt->qual,5)
  SELECT DISTINCT INTO "nl:"
   ...
   PLAN (dsp
    WHERE dsp.operation="INDEX"
     AND dsp.index_columns=null)
    JOIN (ui
    WHERE ui.index_name=dsp.object_name
   ...
   HEAD dsp.object_name
    id_cnt += 1, stat = alterlist(ind_rec->qual,id_cnt), ind_rec->qual[id_cnt].index_name = dsp
    .object_name,
    ind_rec->qual[id_cnt].column_str = "", i = 0
   DETAIL
    i += 1
    IF (i > 1)
     ind_rec->qual[id_cnt].column_str = build(ind_rec->qual[id_cnt].column_str,", ","",uic
      .column_name)
    ELSE
     ind_rec->qual[id_cnt].column_str = uic.column_name
    ENDIF
   WITH nocounter
  ;end select
  SET num_upd = size(ind_rec->qual,5)
  IF (num_upd > 0)
   UPDATE  FROM dm_sql_plan dsp,
     (dummyt d2  WITH seq = value(num_upd))
  ...
  SET num_plan = size(ind_rec->qual,5)
  SET cnt_p = 0
  IF (num_id > 0)
   SELECT INTO  $1
    p.statement_id, p.operation, p.options,
    p.object_name, index_cols = substring(1,150,p.index_columns)
    FROM dm_sql_plan p,
    ...
    HEAD REPORT
     ...
     IF ((sql_text->qual[1].cpu_time > - (1)))
      line = fillstring(236,"="), line1 = fillstring(236,"-"), line2 = fillstring(236,"*")
     ELSE
      line = fillstring(220,"="), line1 = fillstring(220,"-"), line2 = fillstring(220,"*")
     ENDIF
     rpt_cnt = 0, plan_cnt = 0, rdate = format(curdate,"mm/dd/yyyy;;d"),
     rtime = format(curtime3,"hh:mm;;m"),
     CALL center("DM SQLPLAN OUTPUT",1,140), row + 2,
     col 56, "Report Time: ", col + 1,
     rdate, col + 1, rtime,
     pf_cpu_time_in_sec = 0.0
    HEAD d.seq
     IF ((((sql_text->qual[d.seq].flag=1)) OR ((sql_text->qual[d.seq].stmt_flag=1))) )
      ...
      100), row + 2, loops = ((textlen(sql_text->qual[d.seq].stmt)/ 125)+ 2),
      loops1 = cnvtint(loops)
      FOR (i = 1 TO loops1)
        tempstr = substring((((i - 1) * 125)+ 1),125,sql_text->qual[d.seq].stmt), tempstr, row + 1
      ENDFOR
      CALL center("Statistics",1,140), row + 1, col 0,
      line1, row + 1, col 0,
      "Buffer Gets: ", col 22, "Executions: ",
      col 45, "Buffer Ratio: ", col 65,
      "Disk Reads: ", col 83, "Disk Ratio: ",
      col 102, "First Load Time:", col 132,
      "Score: ", col 147, "Hash Value: ",
      col 167, "Sharable Mem: ", col 190,
      "Optimizer Mode: "
      IF ((sql_text->qual[d.seq].cpu_time > - (1)))
       col 214, "CPU Time(sec): "
      ENDIF
      row + 1, col 0, line1,
      row + 1, col 0, sql_text->qual[d.seq].buff,
      col 19, sql_text->qual[d.seq].exec, col 38,
      sql_text->qual[d.seq].rat, col 61, sql_text->qual[d.seq].disk,
      col 79, sql_text->qual[d.seq].drat, col 100,
      sql_text->qual[d.seq].first_time, col 123, sql_text->qual[d.seq].score,
      col 144, sql_text->qual[d.seq].hash_value, col 167,
      sql_text->qual[d.seq].sharable_mem, col 187, sql_text->qual[d.seq].optimizer_mode
      IF ((sql_text->qual[d.seq].cpu_time > - (1)))
       pf_cpu_time_in_sec = (sql_text->qual[d.seq].cpu_time/ 1000000), col 212, pf_cpu_time_in_sec
      ENDIF
      row + 1, row + 1, line1
     ...
     ELSE
      row + 1, col 0, line,
      row + 2, col 0, "Plan statement for: ",
      p.statement_id, row + 2, loops = ((textlen(sql_text->qual[d.seq].stmt)/ 125)+ 2),
      loops1 = cnvtint(loops)
      FOR (i = 1 TO loops1)
        tempstr = substring((((i - 1) * 125)+ 1),125,sql_text->qual[d.seq].stmt), tempstr, row + 1
      ENDFOR
      row + 1,
      CALL center("Statistics",1,140), row + 1,
      col 0, line1, row + 1,
      col 0, "Buffer Gets: ", col 22,
      "Executions: ", col 45, "Ratio: ",
      col 65, "Disk Reads: ", col 83,
      "Disk Ratio: ", col 102, "First Load Time:",
      col 132, "Score: ", col 147,
      "Hash Value: ", col 167, "Sharable Mem: ",
      col 190, "Optimizer Mode: "
      IF ((sql_text->qual[d.seq].cpu_time > - (1)))
       col 214, "CPU Time(sec): "
      ENDIF
      row + 1, col 0, line1,
      ...
      col 79, sql_text->qual[d.seq].drat, col 100,
      sql_text->qual[d.seq].first_time, col 123, sql_text->qual[d.seq].score,
      col 144, sql_text->qual[d.seq].hash_value, col 165,
      sql_text->qual[d.seq].sharable_mem, col 187, sql_text->qual[d.seq].optimizer_mode
      IF ((sql_text->qual[d.seq].cpu_time > - (1)))
       pf_cpu_time_in_sec = (sql_text->qual[d.seq].cpu_time/ 1000000), col 212, pf_cpu_time_in_sec
      ENDIF
    ...
    FOOT  d.seq
     row + 1
    WITH formfeed = none, outerjoin = d, maxcol = 245,
     landscape
   ;end select
  ENDIF
 ...
 END TABLE product_component_version
  ...
  IF (validate(dir_batch_queue,"X")="X"
   ...
   DECLARE dir_batch_queue = vc WITH public, constant(cnvtlower(build("INSTALL$",logical(
       "environment"))))
  ENDIF
    ...
    IF (cnvtupper(sbr_ccs_mode)="I")
     ...
     IF (curqual > 0)
      IF ((ccs_appl_id=dm2_install_schema->appl_id))
       SET dm_err->eproc =
       "Deleting concurrency row from dm_info - same application is restart mode."
       CALL disp_msg(" ",dm_err->logfile,0)
  ...
  SUBROUTINE (prompt_for_host(sbr_host_db=vc) =i2)
    ...
    CALL clear(1,1)
    CALL video(n)
    CALL text(2,1,concat("Creating a database connection to the ",cnvtupper(sbr_host_db),
      " database: "),w)
  ...
  SUBROUTINE (dm2_get_suffixed_tablename(tbl_name=vc) =i2)
    ...
    SET dm2_str = concat("select into 'nl:'"," from dm_tables_doc dtd ",
     ...
     " dm2_table->table_suffix = dtd.table_suffix",
     " dm2_table->full_table_name = dtd.full_table_name"," with nocounter",
     " go")
  ...
  CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
  DECLARE stmt_txt = vc WITH protect, noconstant(" ")
  DECLARE sql_file = vc WITH protect, noconstant(" ")
  DECLARE fmt_file = vc WITH protect, noconstant(" ")
  DECLARE qrytag = vc WITH protect, noconstant(" ")
  DECLARE push_str = vc WITH protect, noconstant(" ")
  DECLARE line_cnt = i4 WITH protect, noconstant(0)
  DECLARE u_name = vc WITH protect, noconstant(" ")
  DECLARE db_name = vc WITH protect, noconstant(" ")
  SET u_name = trim(cnvtlower(currdbuser))
  SET db_name = trim(cnvtlower(currdblink))
  FREE RECORD sql_text
  RECORD sql_text(
    1 text_cnt = i4
    1 qual[*]
      2 rows_read = f8
      ...
      2 row_ratio = f8
      2 stmt_sorts = f8
      2 total_exec_time = f8
      2 score = f8
      2 stmt_text = vc
      2 count[*]
        3 dat_line = vc
  )
  EXECUTE dm2_db2sqlplan value(prompt_val1), value(prompt_val2), value(prompt_val3)
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
  IF (get_unique_file("sql_tmp",".sql")=0)
   GO TO exit_script
  ENDIF
  SET sql_file = dm_err->unique_fname
  IF (get_unique_file("sqlplan",".dat")=0)
   GO TO exit_script
  ENDIF
  SET fmt_file = dm_err->unique_fname
  IF (get_unique_file("SQLPLAN","")=0)
   GO TO exit_script
  ENDIF
  SET qrytag = trim(cnvtupper(dm_err->unique_fname),3)
  SET dm_err->eproc = "Connecting to database"
  SET stmt_txt = concat("db2 connect to ",db_name," user ",u_name," using ",
   cnvtlower(ds_pword))
  IF (dm2_push_dcl(stmt_txt)=0)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Generating Plan statements"
  CALL disp_msg(" ",dm_err->logfile,0)
  FOR (exp_cnt = 1 TO sql_text->text_cnt)
    SET stmt_txt = concat("EXPLAIN PLAN SET QUERYTAG = '",qrytag,"' FOR ",sql_text->qual[exp_cnt].
     stmt_text,";")
    SELECT INTO value(sql_file)
     FROM (dummyt d  WITH seq = 1)
     DETAIL
      row 0, col 0, stmt_txt
     WITH nocounter, format = variable, formfeed = none,
      maxcol = 32000
    ;end select
    IF (dm2_push_dcl(concat("db2 -tvf ",sql_file))=0)
     GO TO exit_script
    ENDIF
    SET push_str = concat("db2exfmt -d ",db_name," -o ",fmt_file," -u ",
     u_name," ",trim(cnvtlower(ds_pword))," -w -1 -# -1")
    IF (dm2_push_dcl(push_str)=0)
     GO TO exit_script
    ENDIF
    FREE DEFINE rtl2
    DEFINE rtl2 value(fmt_file)
    SELECT INTO "nl:"
     FROM rtl2t t
     WHERE 1=1
     HEAD REPORT
      found_ind = 0, line_cnt = 0
     DETAIL
      IF (findstring("Access Plan:",t.line)=1)
       found_ind = 1
      ENDIF
      IF (found_ind > 0)
       found_ind += 1
       IF (found_ind > 3)
        IF (findstring("Objects Used in Access Plan:",t.line)=1)
         found_ind = 0, stat = alterlist(sql_text->qual[exp_cnt].count,line_cnt)
        ELSE
         line_cnt += 1
         IF (mod(line_cnt,50)=1)
          stat = alterlist(sql_text->qual[exp_cnt].count,(line_cnt+ 49))
         ENDIF
         sql_text->qual[exp_cnt].count[line_cnt].dat_line = trim(t.line)
        ENDIF
       ENDIF
      ENDIF
     WITH nocounter
    ;end select
    SET dm_err->eproc = concat("Parsing ",fmt_file," file for explain plan info.")
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
  ...
  IF ((sql_text->text_cnt > 0))
   SELECT INTO  $1
    d.seq
    FROM (dummyt d  WITH seq = value(sql_text->text_cnt))
    PLAN (d)
    ORDER BY d.seq
    HEAD REPORT
     line = fillstring(160,"="), line1 = fillstring(160,"-"), rdate = format(curdate,"mm/dd/yyyy;;d"),
     rtime = format(curtime3,"hh:mm;;m"),
     CALL center("CCL SQLPLAN OUTPUT",1,140), row + 2,
     col 56, "Report Time: ", col + 1,
     rdate, col + 1, rtime
    HEAD d.seq
     row + 1, col 0, line,
     row + 2, col 0, "Statement",
     row + 2, loops = ((textlen(sql_text->qual[d.seq].stmt_text)/ 125)+ 2), loops1 = cnvtint(loops)
     FOR (i = 1 TO loops1)
       tempstr = substring((((i - 1) * 125)+ 1),125,sql_text->qual[d.seq].stmt_text), tempstr, row +
       1
     ENDFOR
     row + 1,
     CALL center("Statistics",1,130), row + 1,
     col 0, line1, row + 1,
     col 0, "Rows Read: ", col 30,
     "Executions: ", col 60, "Ratio: ",
     col 87, "Sorts: ", col 110,
     "Execution Time: ", col 146, "Score: ",
     row + 1, col 0, line1,
     row + 1, col 0, sql_text->qual[d.seq].rows_read,
     col 25, sql_text->qual[d.seq].num_executions, col 55,
     sql_text->qual[d.seq].row_ratio, col 80, sql_text->qual[d.seq].stmt_sorts,
     col 109, sql_text->qual[d.seq].total_exec_time, col 142,
     sql_text->qual[d.seq].score, row + 1, row + 1,
     CALL center("SQL Plan Info",1,130), row + 1, col 0,
     line1, row + 2
     FOR (dsp_cnt = 1 TO size(sql_text->qual[d.seq].count,5))
       col 0, sql_text->qual[d.seq].count[dsp_cnt].dat_line, row + 1
     ENDFOR
    FOOT  d.seq
     row + 1
    WITH formfeed = none, outerjoin = d, maxcol = 2000
   ;end select
  ENDIF
  SET dm_err->eproc = "Displaying results."
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
...
#exit_script
 IF (currdb="ORACLE")
   ...
   IF (((((currev * 10000)+ (currevminor * 100))+ currevminor2) >= 80203))
    ...
    DELETE  FROM dm_sql_plan ds,
     ...
     PLAN (d
      ...
      JOIN (ds
      WHERE expand(ml_expand_num,ml_start_val,(ml_start_val+ (mn_batch_size - 1)),ds.statement_id,
       stmt->qual[ml_expand_num].stmt_id))
     WITH nocounter
    ;end delete
   ELSE
    DELETE  FROM dm_sql_plan ds,
  ...
  ENDIF
 ELSEIF (currdb="DB2UDB")
  DELETE  FROM explain_statement es
   WHERE es.querytag=qrytag
   WITH nocounter
  ;end delete
  SET dm_err->eproc = "cleaning up explain_statement table"
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ENDIF
  CALL dm2_push_dcl("db2 terminate")
  SET stat = remove(sql_file)
  SET stat = remove(fmt_file)
