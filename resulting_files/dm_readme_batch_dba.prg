CREATE PROGRAM dm_readme_batch:dba
 ...
 SUBROUTINE dctx_restore_prev_contexts(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drb_inhouse_ind = i2 WITH protect, noconstant(0)
 SUBROUTINE rb_table_exists(te_table)
   SET te_flag = 0
   SELECT INTO "nl:"
    a.table_name
    FROM dtableattr a
 ...
 EXECUTE FROM 2000_batch TO 2999_batch_exit
 GO TO 9999_exit_program
#1000_initialize
 SET rb_data->in_house = 0
 SET rb_data->batch_dt_tm = 0
 SET rb_data->env_id = 0
 SET rb_data->execution = ""
 SET rb_data->manual_readme_id = 0
 SET rb_data->low_proj_name = ""
 SET rb_data->high_proj_name = ""
 SET stat = alterlist(rb_data->readme,0)
 SET stat = alterlist(rb_data->inactive,0)
 ...
 IF ((dm2_rr_misc->dm2_toolset_usage="Y"))
  EXECUTE dm2_readme_batch  $1
  GO TO 9999_exit_program
 ENDIF
 IF (dm2_set_inhouse_domain(null)=1)
  SET drb_inhouse_ind = 1
  ...
  ENDIF
 ENDIF
 SET rb_data->batch_dt_tm = cnvtdatetime(curdate,curtime3)
 IF (size(trim(validate(rb_execution," "),3)))
  SET rb_data->execution = rb_execution
 ELSE
  SET rb_data->in_house = 1
 ENDIF
 IF (rb_data->in_house)
  SET rb_data->manual_readme_id =  $1
 ENDIF
 IF (rb_data->manual_readme_id)
  SET rb_data->low_proj_name = trim(cnvtstring(rb_data->manual_readme_id),3)
  SET rb_data->high_proj_name = rb_data->low_proj_name
 ELSE
  SET rb_data->low_proj_name = "0"
  SET rb_data->high_proj_name = "999999999"
 ENDIF
 IF (validate(rb_ocd,0)
  AND  NOT (validate(doc,0)))
  FREE SET doc
  RECORD doc(
    1 source_ocd_cnt = i4
    1 qual[1]
  ...
  SET doc->source_ocd_cnt = 1
  SET doc->qual[1].ocd_nbr = rb_ocd
  SET rr_batch_dt_tm = rb_data->batch_dt_tm
 ENDIF
...
#2000_batch
  ...
  IF (rb_data->manual_readme_id)
   ...
   UPDATE  FROM dm_project_status_env s
    SET s.dm_status = null, s.dm_status_dt_tm = null, s.dm_status_msg = null,
     s.processed_flag = 0
    WHERE s.environment_id=rm_env_id
     AND s.proj_type=rm_readme
     AND s.proj_name=trim(cnvtstring(rb_data->manual_readme_id),3)
     AND (s.source_set_instance=
    (SELECT
     max(r.source_set_instance)
     FROM dm_project_status_env r
     WHERE r.environment_id=rm_env_id
      AND r.proj_type=rm_readme
   ...
   IF (curqual)
    COMMIT
   ELSE
    CALL rm_log(rm_error,concat("No readme information found for readme ",rb_data->low_proj_name,
      ".  Make sure this readme has been built in Feature Tracker."))
   ENDIF
  ENDIF
  SET rb_i = 0
  SET rb_j = 0
  SELECT INTO "nl:"
   ...
   PLAN (s
    WHERE s.environment_id=rm_env_id
     AND s.proj_type=rm_readme
     AND cnvtint(s.proj_name) > 0
     AND s.proj_name BETWEEN rb_data->low_proj_name AND rb_data->high_proj_name
     AND s.dm_status = null)
    JOIN (r
    WHERE r.readme_id=cnvtint(s.proj_name)
     AND r.instance=s.source_set_instance)
   ...
   HEAD r.readme_id
    IF (r.active_ind)
     rb_i = (rb_i+ 1), stat = alterlist(rb_data->readme,rb_i), rb_data->readme[rb_i].id = r.readme_id,
     rb_data->readme[rb_i].instance = r.instance, rb_data->readme[rb_i].name = trim(cnvtstring(r
       .readme_id),3), rb_data->readme[rb_i].execution = trim(cnvtstring(r.execution),3),
     rb_data->readme[rb_i].description = trim(r.description,3), rb_data->readme[rb_i].driver_table =
     cnvtupper(trim(r.driver_table,3)), rb_data->readme[rb_i].estimated_time = r.execution_time
    ELSE
     rb_j = (rb_j+ 1), stat = alterlist(rb_data->inactive,rb_j), rb_data->inactive[rb_j].name = trim(
      cnvtstring(r.readme_id),3),
     rb_data->inactive[rb_j].instance = r.instance
    ENDIF
   WITH nocounter
  ;end select
  SET rb_i = size(rb_data->inactive,5)
  IF (rb_i)
   UPDATE  FROM dm_project_status_env s,
     (dummyt d  WITH seq = value(rb_i))
    SET s.dm_status = rm_done, s.dm_status_dt_tm = cnvtdatetime(curdate,curtime3), s.dm_status_msg =
     "This readme is inactive.  Automatically marked as successful."
    PLAN (d)
     JOIN (s
     WHERE s.proj_type=rm_readme
      AND (s.proj_name=rb_data->inactive[d.seq].name)
      AND s.environment_id=rm_env_id
      AND (s.source_set_instance <= rb_data->inactive[d.seq].instance))
    WITH nocounter
   ;end update
   DELETE  FROM dm_ocd_log l,
    ...
    PLAN (d)
     ...
     WHERE l.project_type=rm_readme
      AND (l.project_name=rb_data->inactive[d.seq].name)
      AND l.environment_id=rm_env_id)
    WITH nocounter
   ;end delete
   INSERT  FROM dm_ocd_log l,
     (dummyt d  WITH seq = value(rb_i))
    SET l.status = rm_done, l.start_dt_tm = cnvtdatetime(curdate,curtime3), l.end_dt_tm =
     cnvtdatetime(curdate,curtime3),
     l.updt_dt_tm = cnvtdatetime(curdate,curtime3), l.message =
     "This readme is inactive.  Automatically marked as successful.", l.project_type = rm_readme,
     l.project_name = rb_data->inactive[d.seq].name, l.environment_id = rm_env_id, l.project_instance
      = rb_data->inactive[d.seq].instance,
     l.ocd = 0, l.batch_dt_tm = cnvtdatetime(curdate,curtime3), l.active_ind = 1
    PLAN (d)
     JOIN (l)
    WITH nocounter
   ;end insert
   COMMIT
  ENDIF
  IF (size(rb_data->readme,5))
   UPDATE  FROM dm_project_status_env s,
     (dummyt d  WITH seq = value(size(rb_data->readme,5)))
    SET s.dm_status = rm_done, s.dm_status_dt_tm = cnvtdatetime(curdate,curtime3), s.dm_status_msg =
     "This readme is for tablespace check.  Automatically marked as successful."
    PLAN (d
     WHERE (rb_data->readme[d.seq].execution IN ("PRESPCHK", "POSTSPCHK")))
     JOIN (s
     WHERE s.proj_type=rm_readme
      AND (s.proj_name=rb_data->readme[d.seq].name)
      AND s.environment_id=rm_env_id
      AND (s.source_set_instance <= rb_data->readme[d.seq].instance))
    WITH nocounter
   ;end update
   COMMIT
   UPDATE  FROM dm_project_status_env s,
     (dummyt d  WITH seq = value(size(rb_data->readme,5)))
    SET s.dm_status = rm_done, s.dm_status_dt_tm = cnvtdatetime(curdate,curtime3), s.dm_status_msg =
     "This readme is for timing estimates.  Automatically marked as successful."
    PLAN (d
     WHERE (rb_data->readme[d.seq].execution IN ("RDMTIMER")))
     JOIN (s
     WHERE s.proj_type=rm_readme
      AND (s.proj_name=rb_data->readme[d.seq].name)
      AND s.environment_id=rm_env_id
 ...
 IF ( NOT (rb_data->in_house))
  ...
  SELECT INTO "nl:"
   ...
   HEAD r.readme_id
    IF (r.active_ind=1
     AND (r.execution=rb_data->execution))
     rb_i = (rb_i+ 1)
     IF (mod(rb_i,10)=1)
      stat = alterlist(rb_data->readme,(rb_i+ 9))
     ENDIF
     rb_data->readme[rb_i].id = r.readme_id, rb_data->readme[rb_i].instance = r.instance, rb_data->
     readme[rb_i].name = trim(cnvtstring(r.readme_id),3),
     rb_data->readme[rb_i].description = r.description, rb_data->readme[rb_i].ocd = o.ocd, rb_data->
     readme[rb_i].driver_table = cnvtupper(trim(r.driver_table,3)),
     rb_data->readme[rb_i].estimated_time = r.execution_time
    ENDIF
   FOOT REPORT
    stat = alterlist(rb_data->readme,rb_i)
   WITH nocounter
  ;end select
  IF ((doc->source_ocd_cnt=1))
   SELECT INTO "nl:"
    r.readme_id
    FROM dm_readme r,
     dm_ocd_readme o,
    ...
    PLAN (d)
     JOIN (r
     WHERE (r.readme_id=rb_data->readme[d.seq].id)
      AND (r.instance > rb_data->readme[d.seq].instance))
     JOIN (o
     WHERE o.readme_id=r.readme_id
      AND o.ocd > 0
      AND o.instance=r.instance)
    ...
    HEAD r.readme_id
     IF (r.active_ind=0)
      rb_data->readme[d.seq].skip = 1
     ELSE
      rb_data->readme[d.seq].instance = r.instance, rb_data->readme[d.seq].ocd = o.ocd, rb_data->
      readme[d.seq].description = r.description,
      rb_data->readme[d.seq].driver_table = cnvtupper(trim(r.driver_table,3)), rb_data->readme[d.seq]
      .estimated_time = r.execution_time
     ENDIF
    FOOT  r.readme_id
     row + 0
    WITH nocounter
   ...
   SELECT INTO "nl:"
    ...
    PLAN (d)
     ...
     WHERE l.environment_id=rm_env_id
      AND l.project_type=rm_readme
      AND l.project_name=trim(cnvtstring(rb_data->readme[d.seq].id),3)
      AND (l.ocd=rb_data->readme[d.seq].ocd)
      AND l.status=rm_done
      AND l.active_ind=1)
    DETAIL
     rb_data->readme[d.seq].skip = 1
 ...
 ENDIF
 IF ( NOT (rb_data->in_house))
  FOR (rb_i = 1 TO size(rb_data->readme,5))
    IF ( NOT (rb_data->readme[rb_i].skip))
     SET rb_j = 0
     SELECT INTO "nl:"
      l.project_name
      FROM dm_ocd_log l
      WHERE l.environment_id=rm_env_id
       AND l.project_type=rm_readme
       AND (l.project_name=rb_data->readme[rb_i].name)
       AND (l.project_instance=rb_data->readme[rb_i].instance)
       AND (l.ocd=rb_data->readme[rb_i].ocd)
       AND l.start_dt_tm IS NOT null
       AND l.end_dt_tm = null
       AND l.active_ind=1
      DETAIL
       rb_j = 1
      WITH nocounter
     ;end select
     IF (rb_j)
      CALL rm_log(rm_warning,concat(
        "It appears that the following readme didn't complete on an earlier run (",rb_data->readme[
        rb_i].name,"/",rb_data->readme[rb_i].description,")."))
     ENDIF
    ENDIF
  ENDFOR
 ENDIF
 FOR (rb_i = 1 TO size(rb_data->readme,5))
   IF ( NOT (rb_data->readme[rb_i].skip))
    IF (size(trim(rb_data->readme[rb_i].driver_table,3)))
     SET rb_data->readme[rb_i].driver_count = rm_row_count(rb_data->readme[rb_i].driver_table)
     SET rb_data->readme[rb_i].estimated_time = (rb_data->readme[rb_i].driver_count * rb_data->
     readme[rb_i].estimated_time)
    ENDIF
    DELETE  FROM dm_ocd_log l
     WHERE l.environment_id=rm_env_id
      AND l.project_type=rm_readme
    ...
    ;end delete
    INSERT  FROM dm_ocd_log l
     SET l.environment_id = rm_env_id, l.project_type = rm_readme, l.project_name = rb_data->readme[
      rb_i].name,
      l.project_instance = rb_data->readme[rb_i].instance, l.ocd = rb_data->readme[rb_i].ocd, l
      .batch_dt_tm = cnvtdatetime(rb_data->batch_dt_tm),
      l.driver_count = rb_data->readme[rb_i].driver_count, l.estimated_time = rb_data->readme[rb_i].
      estimated_time, l.active_ind = 1,
      l.updt_dt_tm = cnvtdatetime(rb_data->batch_dt_tm)
     WITH nocounter
    ;end insert
    IF (curqual <= 0)
     CALL rm_log(rm_error,"Unable to add new log row to batch.")
    ENDIF
   ENDIF
 ENDFOR
 COMMIT
 EXECUTE dm_readme_runner
 IF (rb_table_exists("DM_PROJECT_STATUS_ENV")
  AND (rb_data->in_house=1))
  UPDATE  FROM dm_project_status_env s
   SET s.dm_status = rm_failed, s.dm_status_dt_tm = cnvtdatetime(curdate,curtime3), s.dm_status_msg
     = "Automatically failed because of a parent readme failure."
   WHERE s.environment_id=rm_env_id
    AND s.proj_type=rm_readme
    AND s.dm_status = null
    AND  EXISTS (
   (SELECT
    ...
    (SELECT
     ...
     WHERE r.readme_id=cnvtint(l.project_name)
      ...
      AND x.environment_id=rm_env_id
      AND x.project_type=rm_readme
      AND x.project_name=trim(cnvtstring(r.parent_readme_id),3)
      AND x.status=rm_failed))))
   WITH nocounter
  ;end update
 ENDIF
 SET rdm_more = 1
 WHILE (rdm_more)
  UPDATE  FROM dm_ocd_log l
   SET l.status = rm_failed, l.start_dt_tm = cnvtdatetime(curdate,curtime3), l.end_dt_tm =
    cnvtdatetime(curdate,curtime3),
    l.message = "Automatically failed because of a parent readme failure."
   WHERE l.environment_id=rm_env_id
    AND l.project_type=rm_readme
    AND l.status = null
    AND  EXISTS (
   ...
   WITH nocounter
  ;end update
  SET rdm_more = curqual
 ENDWHILE
 COMMIT
#2999_batch_exit
#9999_exit_program
 IF (drb_inhouse_ind=1)
  IF ((dm_err->debug_flag < 1))
