CREATE PROGRAM bbd_chk_import_reg_conv:dba
 ...
 ENDIF
#start_script
 SET sreadme_name = "bbd_chk_import_reg_conv"
...
#script
 ...
 SELECT INTO "nl:"
  ...
  WHERE cnvtupper(p.description)="BLOOD BANK DONOR REGISTRATION"
  DETAIL
 ...
 IF (conv_id=0)
  ...
  SET readme_data->message = "BLOOD BANK DONOR REGISTRATION Conversation not added"
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  p.seq
 ...
 IF (count < 28)
  SET select_failed_flag = 1
  SET readme_data->message = "PROMPT rows for BLOOD BANK DONOR REGISTRATION not added"
  GO TO exit_script
 ENDIF
 SET count = 0
...
#exit_script
 ...
 IF (((lerrorcode != 0) OR (select_failed_flag=1)) )
  ...
  CALL logstatus("Insert into PM_FLX_CONVERSATION failed","F")
 ELSE
  COMMIT
  CALL logmsg("All PM_FLX_CONVERSATION rows inserted successfully.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
