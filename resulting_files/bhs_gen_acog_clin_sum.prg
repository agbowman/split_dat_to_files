CREATE PROGRAM bhs_gen_acog_clin_sum
 RECORD work(
   1 person_id = f8
   1 encntr_id = f8
   1 f_cnt = i4
 ...
 DECLARE cs14003_clinician_summary_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",14003,
   "CLINICIANSUMMARY"))
 DECLARE var_warning = vc WITH noconstant(build2(beg_bold,
   ...
   "Please use the flowsheet to view older ",trim(uar_get_code_display(cs14003_clinician_summary_cd)),
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    WHERE operator(ce.reference_nbr,"LIKE",patstring(work->forms[dseq].reference_nbr,1))
     AND ce.task_assay_cd=cs14003_clinician_summary_cd
     AND ce.view_level=1
   ...
   DETAIL
    ...
    IF ((work->forms[tmp_f].dcp_forms_activity_id=cnvtreal(substring(1,(findstring("!",ce
     ...
     work->entries[e_cnt].clinical_event_id = ce.event_id, work->entries[e_cnt].prsnl_name = trim(pr
      .name_full_formatted), work->entries[e_cnt].entry_dt_tm = ce.updt_dt_tm,
     work->entries[e_cnt].entry_title = trim(ce.event_title_text,3), work->entries[e_cnt].entry_text
      = replace(trim(blob_rtf,3),char(10)," \par ")
    ENDIF
   WITH nocounter
  ;end select
 ENDIF
