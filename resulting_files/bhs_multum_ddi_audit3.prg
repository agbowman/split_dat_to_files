CREATE PROGRAM bhs_multum_ddi_audit3
 PROMPT
 ...
 SELECT INTO value(output_dest)
  ...
  HEAD REPORT
   ...
   beg_date_disp, row + 1, col 1,
   "Ending Date: ", end_date_disp, row + 1
   IF (report_type="2")
    col 1, "Detail Report:", pat_type2,
    row + 1, output_line = build("Triggering Drug",",","Interacting Drug",",","Patient Name",
     ",","Facility",",","Ordering Physician",",",
     "Ordering Physician Position",",","FMRN",",","Order Date",
     ",","User Position",",","Order ID",",",
     "Override Reason",",","Free-text Override Reason",",","Severity",
     ",","Overridden?",",","Triggering Order Detail",",",
     "Interacting Order Detail"), col 1,
    output_line, row + 1
   ELSEIF (report_type="1")
    col 1, "Summary Report: ", pat_type2,
    row + 1, output_line = build("Triggering Drug",",","Interacting Drug",",","# Interact",
     ",","# Overridden",",","% Overridden",",",
     "# Not Overridden",",","% Not Overridden",",","# Total Orders",
     ",","Tot % Interact"), col 1,
    output_line, row + 1
   ENDIF
  HEAD trig_mnemonic
  ...
  DETAIL
   ...
   IF (report_type="2")
    ...
    ENDIF
    pat_name = replace(eks->list[d.seq].patient_name,","," ",0), phy_name = replace(eks->list[d.seq].
     trigger.physician_name,","," ",0), trig_det = replace(trgr.clinical_display_line,",",";",0),
    inter_det = replace(interact.clinical_display_line,",",";",0), output_line = build(trim(
      uar_get_code_display(eks->list[d.seq].trigger.catalog_cd)),",",trim(uar_get_code_display(eks->
       list[d.seq].interact.catalog_cd)),",",pat_name,
     ",",facility_disp,",",phy_name,",",
     trigger_ord_phys_pos,",",eks->list[d.seq].fmrn,",",order_date_disp,
     ",",user_position,",",eks->list[d.seq].trigger.order_id,",",
     override_reason,",",eks->list[d.seq].override_rsn_ft,",",eks->list[d.seq].severity,
     ",",override_yes_no,",",trig_det,",",
     inter_det), col 1,
    output_line, row + 1
   ENDIF
  FOOT  interact_mnemonic
   IF (report_type="1")
    ...
    build(trim(uar_get_code_display(eks->list[d.seq].trigger.catalog_cd)),",",trim(
      uar_get_code_display(eks->list[d.seq].interact.catalog_cd)),",",interact_count,
     ",",override_count,",",override_percent,",",
     not_override_count,",",not_override_percent,",",tot_cnt,
     ",",alert_percent),
