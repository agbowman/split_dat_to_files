CREATE PROGRAM bed_get_concept_manual_matches:dba
 ...
 ENDIF
 IF ( NOT (validate(eventsetcodesreply,0)))
  RECORD eventsetcodesreply(
    1 eventsetcodes[*]
      2 event_set_code = f8
      2 event_set_name = vc
      2 description = vc
 ...
 DECLARE search_string = vc WITH protect, constant(trim(cnvtupper(request->search_string)))
 DECLARE searchtxt = vc WITH protect, noconstant("")
 DECLARE parseidentifiertype = vc WITH protect, noconstant("")
 DECLARE populatemilldatafortabletypecode(codeset=i4) = null
 DECLARE populatemilldatafortabletypeorderable(dummyvar=i2) = null
 DECLARE populatemilldatafortabletypenomenclature(dummyvar=i2) = null
 DECLARE populatemilldatafor93codeset(dummyvar=i2) = null
 DECLARE populatemilldatafor14003codeset(dummyvar=i2) = null
 DECLARE populatemilldatafor72codeset(dummyvar=i2) = null
 IF (validate(debug,0)=1)
  CALL echorecord(request)
 ENDIF
 IF (search_string > "")
  IF ((request->search_type_flag="s"))
   SET searchtxt = concat(search_string,"*")
  ELSE
   SET searchtxt = concat("*",search_string,"*")
  ENDIF
 ENDIF
 IF (identifier_type="CONCEPTCKI")
  SET parseidentifiertype = "concept_cki"
 ELSEIF (identifier_type="CKI")
  SET parseidentifiertype = "cki"
 ELSEIF (identifier_type="CDFMEANING")
  SET parseidentifiertype = "cdf_meaning"
 ENDIF
 IF (table_type="CODE")
  CALL populatemilldatafortabletypecode(code_set)
 ELSEIF (table_type="ORDERABLE")
  CALL populatemilldatafortabletypeorderable(0)
 ELSEIF (table_type="NOMENCLATURE")
  CALL populatemilldatafortabletypenomenclature(0)
 ENDIF
 IF (validate(debug,0)=1)
...
#exit_script
 ...
 SUBROUTINE populatemilldatafortabletypecode(codeset)
   CALL bedlogmessage("populateMillDataForTableTypeCode","Entering ...")
   IF (codeset=93)
    CALL populatemilldatafor93codeset(0)
   ELSEIF (codeset=14003)
    CALL populatemilldatafor14003codeset(0)
   ELSEIF (codeset=72)
    CALL populatemilldatafor72codeset(0)
   ELSE
    DECLARE dispmatchedcnt = i4 WITH protect, noconstant(0)
    DECLARE identmatchedcnt = i4 WITH protect, noconstant(0)
    DECLARE matchedindex = i4 WITH protect, noconstant(0)
    DECLARE cv_parse = vc WITH protect, noconstant("")
    DECLARE index = i4 WITH protect, noconstant(0)
    DECLARE identifiermatchcnt = i4 WITH protect, noconstant(0)
    DECLARE displaymatchcnt = i4 WITH protect, noconstant(0)
    DECLARE codesetcnt = i4 WITH protect, noconstant(0)
    DECLARE codevaluecnt = i4 WITH protect, noconstant(0)
    SET cv_parse = "cv.code_value > 0 and cv.active_ind = 1 and cv.code_set = codeSet"
    IF ((request->search_by_flag="d"))
     SET cv_parse = concat(cv_parse,' and cnvtupper(cv.display) = "',searchtxt,'" ')
    ELSE
     SET cv_parse = concat(cv_parse,' and cnvtupper(parser(build2("cv.",parseIdentifierType))) = "',
      searchtxt,'" ')
    ENDIF
    SELECT INTO "nl:"
     FROM code_value cv
     ...
     DETAIL
      IF ((request->search_by_flag="d"))
       dispmatchedcnt = locateval(matchedindex,1,size(reply->mill_matches,5),cv.display,reply->
        mill_matches[matchedindex].display,
        cv.definition,reply->mill_matches[matchedindex].definition)
       IF (dispmatchedcnt > 0)
        identifiermatchcnt = locateval(index,1,size(reply->mill_matches,5),parser(build2("cv.",
           parseidentifiertype)),reply->mill_matches[index].identifier)
        IF (identifiermatchcnt > 0)
         codesetcnt = (size(reply->mill_matches[dispmatchedcnt].codesets,5)+ 1), stat = alterlist(
          reply->mill_matches[dispmatchedcnt].codesets,codesetcnt), reply->mill_matches[
         dispmatchedcnt].codesets[codesetcnt].code_set = cv.code_set
        ELSE
         codevaluecnt = (size(reply->mill_matches,5)+ 1), stat = alterlist(reply->mill_matches,
          codevaluecnt), reply->mill_matches[codevaluecnt].display = cv.display,
         reply->mill_matches[codevaluecnt].description = cv.description, reply->mill_matches[
         codevaluecnt].definition = cv.definition, reply->mill_matches[codevaluecnt].identifier =
         parser(build2("cv.",parseidentifiertype)),
         codesetcnt = (size(reply->mill_matches[codevaluecnt].codesets,5)+ 1), stat = alterlist(reply
          ->mill_matches[codevaluecnt].codesets,codesetcnt), reply->mill_matches[codevaluecnt].
         codesets[codesetcnt].code_set = cv.code_set
        ENDIF
       ELSE
        codevaluecnt = (size(reply->mill_matches,5)+ 1), stat = alterlist(reply->mill_matches,
         codevaluecnt), reply->mill_matches[codevaluecnt].display = cv.display,
       ...
       ENDIF
      ELSE
       identmatchedcnt = locateval(matchedindex,1,size(reply->mill_matches,5),parser(build2("cv.",
          parseidentifiertype)),reply->mill_matches[matchedindex].identifier)
       IF (identmatchedcnt > 0)
        displaymatchcnt = locateval(index,1,size(reply->mill_matches,5),cv.display,reply->
         mill_matches[index].display,
         cv.description,reply->mill_matches[index].description,cv.definition,reply->mill_matches[
         index].definition)
        IF (displaymatchcnt > 0)
         codesetcnt = (size(reply->mill_matches[displaymatchcnt].codesets,5)+ 1), stat = alterlist(
          reply->mill_matches[displaymatchcnt].codesets,codesetcnt), reply->mill_matches[
         displaymatchcnt].codesets[codesetcnt].code_set = cv.code_set
        ELSE
         codevaluecnt = (size(reply->mill_matches,5)+ 1), stat = alterlist(reply->mill_matches,
          codevaluecnt), reply->mill_matches[codevaluecnt].display = cv.display,
         reply->mill_matches[codevaluecnt].description = cv.description, reply->mill_matches[
    ...
    ;end select
   ENDIF
   CALL bedlogmessage("populateMillDataForTableTypeCode","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatemilldatafortabletypeorderable(dummyvar)
   CALL bedlogmessage("populateMillDataForTableTypeOrderable","Entering ...")
   DECLARE oc_parse = vc WITH protect, noconstant("")
   SET oc_parse = "oc.catalog_cd > 0 and oc.active_ind = 1"
   IF ((request->search_by_flag="d"))
    SET oc_parse = concat(oc_parse,' and cnvtupper(oc.primary_mnemonic) = "',searchtxt,'" ')
   ELSE
    SET oc_parse = concat(oc_parse,' and cnvtupper(parser(build2("oc.",parseIdentifierType))) = "',
     searchtxt,'" ')
   ENDIF
   SELECT INTO "nl:"
    FROM order_catalog oc
    ...
    DETAIL
     ocnt = (size(reply->mill_matches,5)+ 1), stat = alterlist(reply->mill_matches,ocnt), reply->
     mill_matches[ocnt].display = oc.primary_mnemonic,
     reply->mill_matches[ocnt].description = oc.description, reply->mill_matches[ocnt].identifier =
     parser(build2("oc.",parseidentifiertype)), codesetcnt = (size(reply->mill_matches[ocnt].codesets,
      5)+ 1),
     stat = alterlist(reply->mill_matches[ocnt].codesets,codesetcnt), reply->mill_matches[ocnt].
     codesets[codesetcnt].code_set = - (1)
    WITH nocounter
   ;end select
   CALL bedlogmessage("populateMillDataForTableTypeOrderable","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatemilldatafortabletypenomenclature(dummyvar)
   CALL bedlogmessage("populateMillDataForTableTypeNomenclature","Entering ...")
   DECLARE nomencnt = i4 WITH protect, noconstant(0)
   DECLARE n_parse = vc WITH protect, noconstant("")
   SET n_parse =
   "n.source_vocabulary_cd = PATIENT_CARE_CD and n.principle_type_cd = PRINCIPLE_TYPE_CD"
   IF ((request->search_by_flag="d"))
    SET n_parse = concat(n_parse,' and n.source_string_keycap = "',searchtxt,'" and n.active_ind = 1'
     )
   ELSE
    SET n_parse = concat(n_parse,' and cnvtupper(n.concept_cki) = "',searchtxt,
     '" and n.active_ind = 1')
   ENDIF
   SELECT INTO "nl:"
    FROM nomenclature n
    WHERE parser(n_parse)
    DETAIL
     nomencnt = (nomencnt+ 1), stat = alterlist(reply->mill_matches,nomencnt), reply->mill_matches[
     nomencnt].display = n.source_string,
     reply->mill_matches[nomencnt].description = n.mnemonic, reply->mill_matches[nomencnt].identifier
      = n.concept_cki
    WITH nocounter
   ;end select
   CALL bedlogmessage("populateMillDataForTableTypeNomenclature","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatemilldatafor93codeset(dummyvar)
   CALL bedlogmessage("populateMillDataFor93CodeSet","Entering ...")
   DECLARE ec = i4 WITH protect, noconstant(0)
   DECLARE eventsetnamematchcnt = i4 WITH protect, noconstant(0)
   DECLARE eventsetnamematchindex = i4 WITH protect, noconstant(0)
   DECLARE identifiermatchcnt = i4 WITH protect, noconstant(0)
   DECLARE identifiermatchindex = i4 WITH protect, noconstant(0)
   DECLARE codevaluecnt = i4 WITH protect, noconstant(0)
   DECLARE codesetcnt = i4 WITH protect, noconstant(0)
   DECLARE esc_parse = vc WITH protect, noconstant("")
   SET stat = initrec(eventsetcodesreply)
   IF ((request->search_by_flag="d"))
    SET esc_parse = "esc.event_set_cd > 0"
    SET esc_parse = concat(esc_parse,' and cnvtupper(esc.event_set_name) = "',searchtxt,'" ')
    CALL geteventsetcdfromeventsetname(esc_parse,identifier_type,eventsetcodesreply)
   ELSE
    SET esc_parse = "cv.code_value > 0"
    SET esc_parse = concat(esc_parse," and cv.code_set = 93")
    SET esc_parse = concat(esc_parse," and cv.active_ind = 1")
    SET esc_parse = concat(esc_parse,' and cnvtupper(parser(build2("cv.",parseIdentifierType))) = "',
     searchtxt,'" ')
    CALL geteventsetcdfromidentifier(esc_parse,identifier_type,eventsetcodesreply)
   ENDIF
   FOR (ec = 1 TO size(eventsetcodesreply->eventsetcodes,5))
    SET eventsetnamematchcnt = locateval(eventsetnamematchindex,1,size(reply->mill_matches,5),
     eventsetcodesreply->eventsetcodes[ec].event_set_name,reply->mill_matches[eventsetnamematchindex]
     .display,
     eventsetcodesreply->eventsetcodes[ec].definition,reply->mill_matches[eventsetnamematchindex].
     definition)
    IF (eventsetnamematchcnt > 0)
     SET identifiermatchcnt = locateval(identifiermatchindex,1,size(reply->mill_matches,5),
      eventsetcodesreply->eventsetcodes[ec].identifier,reply->mill_matches[identifiermatchindex].
      identifier)
     IF (identifiermatchcnt > 0)
      SET codesetcnt = (size(reply->mill_matches[eventsetnamematchcnt].codesets,5)+ 1)
      SET stat = alterlist(reply->mill_matches[eventsetnamematchcnt].codesets,codesetcnt)
      SET reply->mill_matches[eventsetnamematchcnt].codesets[codesetcnt].code_set =
      eventsetcodesreply->eventsetcodes[ec].code_set
     ELSE
      SET codevaluecnt = (size(reply->mill_matches,5)+ 1)
      SET stat = alterlist(reply->mill_matches,codevaluecnt)
      SET reply->mill_matches[codevaluecnt].display = eventsetcodesreply->eventsetcodes[ec].
      event_set_name
      SET reply->mill_matches[codevaluecnt].description = eventsetcodesreply->eventsetcodes[ec].
      description
      SET reply->mill_matches[codevaluecnt].definition = eventsetcodesreply->eventsetcodes[ec].
      definition
      SET reply->mill_matches[codevaluecnt].identifier = eventsetcodesreply->eventsetcodes[ec].
      identifier
      SET codesetcnt = (size(reply->mill_matches[codevaluecnt].codesets,5)+ 1)
      SET stat = alterlist(reply->mill_matches[codevaluecnt].codesets,codesetcnt)
      SET reply->mill_matches[codevaluecnt].codesets[codesetcnt].code_set = eventsetcodesreply->
      eventsetcodes[ec].code_set
     ENDIF
    ELSE
     SET codevaluecnt = (size(reply->mill_matches,5)+ 1)
     SET stat = alterlist(reply->mill_matches,codevaluecnt)
    ...
    ENDIF
   ENDFOR
   CALL bedlogmessage("populateMillDataFor93CodeSet","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatemilldatafor14003codeset(dummyvar)
   CALL bedlogmessage("populateMillDataFor14003CodeSet","Entering ...")
   DECLARE mnemonicmatchedcnt = i4 WITH protect, noconstant(0)
   DECLARE identmatchedcnt = i4 WITH protect, noconstant(0)
   DECLARE matchedindex = i4 WITH protect, noconstant(0)
   DECLARE dta_parse = vc WITH protect, noconstant("")
   DECLARE 14003matchcnt = i4 WITH protect, noconstant(0)
   DECLARE 14003indexcnt = i4 WITH protect, noconstant(0)
   DECLARE dt = i4 WITH protect, noconstant(0)
   DECLARE index = i4 WITH protect, noconstant(0)
   DECLARE identifiermatchcnt = i4 WITH protect, noconstant(0)
   DECLARE mnemonicmatchcnt = i4 WITH protect, noconstant(0)
   DECLARE codesetcnt = i4 WITH protect, noconstant(0)
   DECLARE codevaluecnt = i4 WITH protect, noconstant(0)
   SET stat = initrec(dtacodesreply)
   IF ((request->search_by_flag="d"))
    SET dta_parse = "dta.task_assay_cd > 0 and dta.active_ind = 1"
    SET dta_parse = concat(dta_parse,' and cnvtupper(dta.mnemonic) = "',searchtxt,'" ')
    CALL getdtacdfrommnemonic(dta_parse,identifier_type,dtacodesreply)
   ELSE
    SET dta_parse = "cv.code_value > 0"
    SET dta_parse = "cv.code_set = 14003"
    SET dta_parse = concat(dta_parse,' and cnvtupper(parser(build2("cv.",parseIdentifierType))) = "',
     searchtxt,'" ')
    CALL getdtacdfromidentifier(dta_parse,identifier_type,dtacodesreply)
   ENDIF
   FOR (dt = 1 TO size(dtacodesreply->dtacodes,5))
     IF ((request->search_by_flag="d"))
      SET mnemonicmatchedcnt = locateval(matchedindex,1,size(reply->mill_matches,5),dtacodesreply->
       dtacodes[dt].mnemonic,reply->mill_matches[matchedindex].display,
       dtacodesreply->dtacodes[dt].definition,reply->mill_matches[matchedindex].definition)
      IF (mnemonicmatchedcnt > 0)
       SET identifiermatchcnt = locateval(index,1,size(reply->mill_matches,5),dtacodesreply->
        dtacodes[dt].identifier,reply->mill_matches[index].identifier)
       IF (identifiermatchcnt > 0)
        SET 14003matchcnt = locateval(14003indexcnt,1,size(reply->mill_matches[mnemonicmatchedcnt].
          codesets,5),14003,reply->mill_matches[mnemonicmatchedcnt].codesets[14003indexcnt].code_set)
        IF (14003matchcnt > 0)
         IF ((reply->mill_matches[mnemonicmatchedcnt].activity_type.display=dtacodesreply->dtacodes[
         dt].activity_type_disp))
          SET codesetcnt = (size(reply->mill_matches[mnemonicmatchedcnt].codesets,5)+ 1)
          SET stat = alterlist(reply->mill_matches[mnemonicmatchedcnt].codesets,codesetcnt)
          SET reply->mill_matches[mnemonicmatchedcnt].codesets[codesetcnt].code_set = dtacodesreply->
          dtacodes[dt].code_set
         ELSE
          SET codevaluecnt = (size(reply->mill_matches,5)+ 1)
          SET stat = alterlist(reply->mill_matches,codevaluecnt)
          SET reply->mill_matches[codevaluecnt].display = dtacodesreply->dtacodes[dt].mnemonic
          SET reply->mill_matches[codevaluecnt].description = dtacodesreply->dtacodes[dt].description
          SET reply->mill_matches[codevaluecnt].definition = dtacodesreply->dtacodes[dt].definition
          SET reply->mill_matches[codevaluecnt].identifier = dtacodesreply->dtacodes[dt].identifier
          SET reply->mill_matches[codevaluecnt].activity_type.code_value = dtacodesreply->dtacodes[dt
          ].activity_type_cd
          SET reply->mill_matches[codevaluecnt].activity_type.display = dtacodesreply->dtacodes[dt].
          activity_type_disp
          SET reply->mill_matches[codevaluecnt].activity_type.meaning = dtacodesreply->dtacodes[dt].
          activity_type_def
          SET codesetcnt = (size(reply->mill_matches[codevaluecnt].codesets,5)+ 1)
          SET stat = alterlist(reply->mill_matches[codevaluecnt].codesets,codesetcnt)
          SET reply->mill_matches[codevaluecnt].codesets[codesetcnt].code_set = dtacodesreply->
          dtacodes[dt].code_set
         ENDIF
        ELSE
         SET codesetcnt = (size(reply->mill_matches[mnemonicmatchedcnt].codesets,5)+ 1)
         ...
         SET reply->mill_matches[mnemonicmatchedcnt].codesets[codesetcnt].code_set = dtacodesreply->
         dtacodes[dt].code_set
         SET reply->mill_matches[mnemonicmatchedcnt].activity_type.code_value = dtacodesreply->
         dtacodes[dt].activity_type_cd
         SET reply->mill_matches[mnemonicmatchedcnt].activity_type.display = dtacodesreply->dtacodes[
         dt].activity_type_disp
         SET reply->mill_matches[mnemonicmatchedcnt].activity_type.meaning = dtacodesreply->dtacodes[
         dt].activity_type_def
        ENDIF
       ELSE
        SET codevaluecnt = (size(reply->mill_matches,5)+ 1)
        SET stat = alterlist(reply->mill_matches,codevaluecnt)
      ...
      ELSE
       ...
       SET codesetcnt = (size(reply->mill_matches[codevaluecnt].codesets,5)+ 1)
       SET stat = alterlist(reply->mill_matches[codevaluecnt].codesets,codesetcnt)
       SET reply->mill_matches[codevaluecnt].codesets[codesetcnt].code_set = dtacodesreply->dtacodes[
       dt].code_set
      ENDIF
     ELSE
      SET identmatchedcnt = locateval(matchedindex,1,size(reply->mill_matches,5),dtacodesreply->
       dtacodes[dt].identifier,reply->mill_matches[matchedindex].identifier)
      IF (identmatchedcnt > 0)
       SET mnemonicmatchcnt = locateval(index,1,size(reply->mill_matches,5),dtacodesreply->dtacodes[
        dt].mnemonic,reply->mill_matches[index].display,
        dtacodesreply->dtacodes[dt].mnemonic,reply->mill_matches[index].description,dtacodesreply->
        dtacodes[dt].definition,reply->mill_matches[index].definition)
       IF (mnemonicmatchcnt > 0)
        SET 14003matchcnt = locateval(14003indexcnt,1,size(reply->mill_matches[mnemonicmatchcnt].
          codesets,5),14003,reply->mill_matches[mnemonicmatchcnt].codesets[14003indexcnt].code_set)
        IF (14003matchcnt > 0)
         IF ((reply->mill_matches[mnemonicmatchcnt].activity_type.display=dtacodesreply->dtacodes[dt]
         .activity_type_disp))
          SET codesetcnt = (size(reply->mill_matches[mnemonicmatchcnt].codesets,5)+ 1)
          SET stat = alterlist(reply->mill_matches[mnemonicmatchcnt].codesets,codesetcnt)
          SET reply->mill_matches[mnemonicmatchcnt].codesets[codesetcnt].code_set = dtacodesreply->
          dtacodes[dt].code_set
         ELSE
          SET codevaluecnt = (size(reply->mill_matches,5)+ 1)
          SET stat = alterlist(reply->mill_matches,codevaluecnt)
        ...
        ELSE
         ...
         SET reply->mill_matches[mnemonicmatchcnt].codesets[codesetcnt].code_set = dtacodesreply->
         dtacodes[dt].code_set
         SET reply->mill_matches[mnemonicmatchcnt].activity_type.code_value = dtacodesreply->
         dtacodes[dt].activity_type_cd
         SET reply->mill_matches[mnemonicmatchcnt].activity_type.display = dtacodesreply->dtacodes[dt
         ].activity_type_disp
         SET reply->mill_matches[mnemonicmatchcnt].activity_type.meaning = dtacodesreply->dtacodes[dt
         ].activity_type_def
        ENDIF
       ELSE
        SET codevaluecnt = (size(reply->mill_matches,5)+ 1)
     ...
     ENDIF
   ENDFOR
   CALL bedlogmessage("populateMillDataFor14003CodeSet","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatemilldatafor72codeset(dummyvar)
   CALL bedlogmessage("populateMillDataFor72CodeSet","Entering ...")
   DECLARE cv_parse = vc WITH protect, noconstant("")
   DECLARE repcnt = i4 WITH protect, noconstant(0)
   DECLARE codesetcnt = i4 WITH protect, noconstant(0)
   SET cv_parse = "cv.code_value > 0 and cv.active_ind = 1 and cv.code_set = 72"
   IF ((request->search_by_flag="d"))
    SET cv_parse = concat(cv_parse,' and cnvtupper(cv.display) = "',searchtxt,'" ')
   ELSE
    SET cv_parse = concat(cv_parse,' and cnvtupper(parser(build2("cv.",parseIdentifierType))) = "',
   ...
   SELECT INTO "nl:"
    ...
    ORDER BY cv.display
    HEAD cv.code_value
     repcnt = (size(reply->mill_matches,5)+ 1), stat = alterlist(reply->mill_matches,repcnt), reply->
     mill_matches[repcnt].display = cv.display,
     reply->mill_matches[repcnt].description = cv.description, reply->mill_matches[repcnt].definition
      = cv.definition, reply->mill_matches[repcnt].identifier = parser(build2("cv.",
       parseidentifiertype)),
     reply->mill_matches[repcnt].code_value = cv.code_value, codesetcnt = (size(reply->mill_matches[
      repcnt].codesets,5)+ 1), stat = alterlist(reply->mill_matches[repcnt].codesets,codesetcnt),
     reply->mill_matches[repcnt].codesets[codesetcnt].code_set = cv.code_set
    WITH nocounter
   ;end select
   CALL bedlogmessage("populateMillDataFor72CodeSet","Exiting ...")
 END ;Subroutine
END GO
