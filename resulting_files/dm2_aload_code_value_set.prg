CREATE PROGRAM dm2_aload_code_value_set
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_cvs",".log","dm2_aload_code_value_set")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_code_value_set"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dacvs_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dacvs_file = vc WITH protect, noconstant( $2)
 DECLARE dacvs_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dacvs_batch = i4 WITH protect, noconstant(0)
 DECLARE dacvs_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE dacvs_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dacvs_pkg,dacvs_file_loc_ret,dacvs_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dacvs_batch=0)
  SET dacvs_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_AFD_CODE_VALUE_SET for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND dacvs_rows_returned=10000)
   DELETE  FROM dm_afd_code_value_set d
    WHERE d.alpha_feature_nbr=dacvs_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET dacvs_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",dacvs_rows_returned))
   ENDIF
 ENDWHILE
 IF (dacvs_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dacvs_file_loc_ret,dacvs_file), "dm2_aload_cs_cvs", dacvs_batch
  CALL echo(concat(dacvs_file_loc_ret,dacvs_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_code_value_set completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_code_value_set")
END GO
