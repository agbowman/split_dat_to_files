CREATE PROGRAM dcpreqgen08:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   format(e.reg_dt_tm,"@SHORTDATE"), orders->dischg_dt = format(e.disch_dt_tm,"@SHORTDATE4YR")
   ...
   orders->bed = uar_get_code_display(e.loc_bed_cd), orders->location = concat(trim(orders->
     nurse_unit),"/",trim(orders->room),"/",trim(orders->bed)), orders->admit_diagnosis = trim(e
    .reason_for_visit,3),
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (c.event_cd=height_cd)
    ...
    orders->height_dt_tm = format(trim(concat(trim(datetimezoneformat(c.updt_dt_tm,c.performed_tz,
         "@SHORTDATE4YR"))," ",trim(datetimezoneformat(c.updt_dt_tm,c.performed_tz,"@TIMENOSECONDS"))
       )))
   ELSEIF (c.event_cd=weight_cd)
    orders->weight = concat(trim(c.event_tag)," ",trim(uar_get_code_display(c.result_units_cd))),
    orders->weight_dt_tm = format(trim(concat(trim(datetimezoneformat(c.updt_dt_tm,c.performed_tz,
         "@SHORTDATE4YR"))," ",trim(datetimezoneformat(c.updt_dt_tm,c.performed_tz,"@TIMENOSECONDS"))
       )))
   ENDIF
  WITH nocounter
  ...
  HEAD o.order_id
   ...
   IF (curutc > 0)
    orders->qual[ord_cnt].order_dt = concat(format(datetimezone(oa.order_dt_tm,oa.order_tz),
      "@SHORTDATETIMENOSEC")," ",datetimezonebyindex(oa.order_tz,offset,daylight,7,oa.order_dt_tm))
   ELSE
   ...
   orders->qual[ord_cnt].signed_dt = format(datetimezone(o.orig_order_dt_tm,o.orig_order_tz),
    "@SHORTDATE"), orders->qual[ord_cnt].comment_ind = o.order_comment_ind, orders->qual[ord_cnt].
   last_action_seq = o.last_action_sequence,
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   HEAD PAGE
    "{cpi/14}{f/12}", row + 1, "{pos/50/60}{box/98/5}",
    row + 1, "{pos/50/135}{box/98/18}", row + 1,
    "{pos/50/345}{box/98/21}", row + 1, "{pos/50/590}{box/98/6}",
    row + 1, "{pos/50/680}{box/98/2}", row + 1,
    "{cpi/11}{f/12}", row + 1, "{pos/1/70}{b}",
    CALL center(orders->facility,1,190), "{endb}", row + 1,
    ...
    "{pos/280/280}{f/6}Admit Diagnosis: "
    ...
    ENDIF
    IF (saved_pos > 0)
     "{pos/295/304}{f/6}Ordering MD: ", "{f/4}", orders->qual[saved_pos].order_dr,
     row + 1, "{pos/60/360}{f/6}Ordered Procedure: ", "{f/4}",
     "{B}", orders->qual[saved_pos].mnemonic, "{ENDB}",
     "{pos/60/372}{f/6}Order ID #: ", "{f/4}", orders->qual[saved_pos].order_id"#########",
     row + 1
     IF ((orders->qual[saved_pos].action != "Order"))
      CALL print(calcpos(200,372)), "*****", orders->qual[saved_pos].action,
      "*****", row + 1
     ENDIF
     saved_pos = 0
    ENDIF
   DETAIL
    FOR (vv = 1 TO value(ord_cnt))
      IF ((orders->qual[vv].display_ind=1))
       ...
       IF (first_page="N")
        BREAK
       ENDIF
       first_page = "N", "{pos/295/304}{f/6}Ordering MD: ", "{f/4}",
       orders->qual[vv].order_dr, row + 1, "{pos/60/360}{f/6}Ordered Procedure: ",
       "{f/4}", "{B}", orders->qual[vv].mnemonic,
       "{ENDB}", "{pos/60/372}{f/6}Order ID #: ", "{f/4}",
       orders->qual[vv].order_id"#########", row + 1
       IF ((orders->qual[vv].action != "Order"))
        CALL print(calcpos(200,372)), "*****", orders->qual[vv].action,
        "*****", row + 1
       ENDIF
       xcol = 60, ycol = 384
       FOR (fsub = 1 TO 31)
         FOR (x = 1 TO orders->qual[vv].d_cnt)
          IF ((((orders->qual[vv].d_qual[x].group_seq=fsub)) OR (fsub=31))
           ...
           orders->qual[vv].d_qual[x].print_ind = 1
           IF (textlen(trim(orders->qual[vv].d_qual[x].value,3)) > 0)
            "{cpi/14}",
            CALL print(calcpos(xcol,ycol)),
            CALL print(orders->qual[vv].d_qual[x].label_text),
            "  ", row + 1, xcol = 200
            IF (textlen(orders->qual[vv].d_qual[x].label_text) > 39)
             ycol = (ycol+ 12)
            ENDIF
            IF ((orders->qual[vv].d_qual[x].value_cnt > 1))
             FOR (dsub = 1 TO orders->qual[vv].d_qual[x].value_cnt)
               CALL print(calcpos(xcol,ycol)), "{b}", orders->qual[vv].d_qual[x].value_qual[dsub].
               value_line,
               "{endb}", row + 1, ycol = (ycol+ 12)
               IF (ycol > 540
                AND (dsub < orders->qual[vv].d_qual[x].value_cnt))
                CALL print(calcpos(xcol,ycol)), "**Continued on next page**", saved_pos = vv,
                BREAK, xcol = 60, ycol = 384,
                "{CPI/14}",
                CALL print(calcpos(xcol,ycol)),
                CALL print(concat(orders->qual[vv].d_qual[x].label_text," cont. ")),
                xcol = 200
                IF (textlen(orders->qual[vv].d_qual[x].label_text) > 39)
                 ycol = (ycol+ 12)
                ENDIF
             ...
             ENDFOR
            ELSE
             CALL print(calcpos(xcol,ycol)), "{b}", orders->qual[vv].d_qual[x].value,
             "{endb}", row + 1, ycol = (ycol+ 12)
            ENDIF
           ENDIF
           xcol = 60
    ...
    ENDFOR
   FOOT PAGE
    xcol = 60, ycol = 600, "{f/6}{b}",
    CALL print(calcpos(xcol,ycol)), "Notes: ", "{endb}{f/4}",
    row + 1, "{pos/60/695}{f/6}Entered By: ", "{f/4}",
    orders->qual[1].enter_by, row + 1, "{pos/350/695}{f/6}Date/Time: ",
    "{f/4}", orders->qual[1].signed_dt
