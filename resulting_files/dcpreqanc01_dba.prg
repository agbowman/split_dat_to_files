CREATE PROGRAM dcpreqanc01:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  p1.person_id, p2.person_id, o.org_name
  FROM person p,
   encounter e,
   person_alias pa,
   encntr_alias ea,
  ...
  WITH outerjoin = d1, dontcare = pa, outerjoin = d2,
   dontcare = ea, outerjoin = d3, outerjoin = d5,
   nocounter
 ;end select
   ...
   DETAIL
    IF (c.event_cd IN (62610, 62611, 62589, 62612))
     IF (all_last_ind=0)
      ...
      all_ind = 1
     ENDIF
    ELSEIF (c.event_cd IN (71041, 71050, 62616))
     IF (prec_last_ind=0)
 ...
 SELECT INTO "nl:"
  ...
  ph.phone_id, o.current_start_dt_tm
  FROM orders o,
   order_action oa,
   prsnl p,
   prsnl p1,
  ...
  SELECT INTO value(request->printer_name)
   ...
   DETAIL
    FOR (vv = 1 TO value(num_of_orders))
      ...
      CALL print(cnvtint(request->order_qual[1].encntr_id)), row + 1, "{CPI/12}{POS/35/180}",
      "Admit Dr:  ", "{CPI/12}{POS/110/180}", orders->admitting_dr_num,
      " ", orders->admitting_dr, row + 1,
      "{CPI/12}{POS/260/180}", "Ord Dr:  ",
      CALL print(trim(provider)),
      " ", orders->qual[vv].order_provider_name, row + 1,
      "{CPI/12}{POS/460/180}", "Ht:  ",
      CALL print(trim(pt_height)),
      " ", ht_date, row + 1,
      "{CPI/12}{POS/460/193}", "Wt:  ",
      CALL print(trim(pt_weight)),
      " ", wt_date, row + 1,
      "{CPI/12}{POS/35/205}", "Reason for Visit:  ", "{CPI/12}{POS/120/205}",
      orders->admit_diagnosis, row + 1, "{CPI/12}{POS/35/230}",
      "Order: ", row + 1, "{CPI/10}{POS/120/230}",
      "{b}", orders->qual[vv].order_mnemonic, "{endb}",
      row + 1, xcol = 120, ycol = 245
      FOR (fsub = 1 TO 31)
        FOR (ww = 1 TO orders->qual[vv].detail_cnt)
          IF ((((orders->qual[vv].detail_qual[ww].group_seq=fsub)) OR (fsub=31
           ...
           IF ((orders->qual[vv].detail_qual[ww].field_id IN (670046, 396515)))
            xcol = xcol
           ELSE
            orders->qual[vv].detail_qual[ww].print_ind = 1, "{CPI/12}",
            CALL print(calcpos(xcol,ycol)),
            CALL print(orders->qual[vv].detail_qual[ww].label_text), ":  ", xcol = 355,
            "{CPI/12}",
            CALL print(calcpos(xcol,ycol)),
            CALL print(orders->qual[vv].detail_qual[ww].display_value),
            row + 1, xcol = 120, ycol = (ycol+ 15)
           ENDIF
          ENDIF
        ENDFOR
      ENDFOR
      ...
      CALL print(trim(provider)),
      " ", orders->qual[vv].order_provider_name, row + 1,
      "{CPI/13}{POS/135/700}", orders->age2, "  ",
      CALL print(substring(1,1,orders->sex)), "  ", orders->dob,
      row + 1, "{CPI/13}{POS/135/710}", orders->qual[vv].order_mnemonic,
      row + 1, "{CPI/13}{POS/315/670}{b}", orders->name,
      row + 1, "{CPI/13}{POS/315/680}{b}", orders->fnbr,
      row + 1, "{CPI/13}{POS/365/680}", "Pt Loc: ",
      orders->location, row + 1, "{CPI/13}{POS/315/690}",
      ...
      orders->dob, row + 1, "{CPI/13}{POS/315/710}",
      orders->qual[vv].order_mnemonic, row + 1, "{CPI/13}{POS/35/740}",
      "Printed Dt/Tm: ", curdate"MM/DD/YYYY;;D", " ",
      curtime"HH:MM;;M", row + 1, "{CPI/13}{POS/185/740}",
      "Ordered Dt/Tm: ", signed, row + 1,
      "{CPI/13}{POS/340/740}", "Entered by: ", orders->qual[vv].action_prsnl_name,
      row + 1, "{CPI/13}{POS/520/740}", "Page: ",
      curpage"###", row + 1
    ENDFOR
   FOOT PAGE
