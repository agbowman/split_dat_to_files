CREATE PROGRAM bed_aud_firstnet_tabs:dba
 ...
 SELECT INTO "nl:"
  FROM application a
  PLAN (a
   WHERE a.application_number=4250111
    AND a.active_ind=1)
  HEAD REPORT
   positioncnt = 0
  DETAIL
  ...
  SELECT INTO "nl"
   ...
   PLAN (d1
    ...
    WHERE dp.prsnl_id=0
     ...
     AND dp.view_name IN ("FLOWSHEET", "TRACK_SHELL")
     AND (dp.view_seq=temp_reply->positions[d1.seq].views[d2.seq].table_view_seq))
    JOIN (nvp
    WHERE nvp.parent_entity_id=dp.detail_prefs_id
     AND nvp.parent_entity_name="DETAIL_PREFS"
     AND trim(nvp.pvc_name) IN ("R_EVENT_SET_NAME", "TRACKING_VIEW_TYPE"))
 ...
 FOR (x = 1 TO positioncnt)
   IF (size(temp_reply->positions[x].views,5) > 0)
    FOR (y = 1 TO size(temp_reply->positions[x].views,5))
     IF ((temp_reply->positions[x].views[y].view_name="TRACK_SHELL"))
      SET stat = add_rep(temp_reply->positions[x].position_txt,temp_reply->positions[x].views[y].
       level,temp_reply->positions[x].views[y].view_caption,cnvtstring(temp_reply->positions[x].
        views[y].view_seq),temp_reply->positions[x].views[y].view_name,
       temp_reply->positions[x].views[y].view_level,"","",temp_reply->positions[x].views[y].
       pref_level)
      SET pref_suffix = get_pref_suffix(temp_reply->positions[x].views[y].pref_value)
      SET stat = add_rep(temp_reply->positions[x].position_txt,temp_reply->positions[x].views[y].
       view_caption,concat(temp_reply->positions[x].views[y].pref_value," - ",pref_suffix),concat(
        trim(cnvtstring(temp_reply->positions[x].views[y].view_seq)),"--","1"),temp_reply->positions[
       x].views[y].view_name,
...
#exit_script
 ...
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("br_firstnet_tab.csv")
 ENDIF
