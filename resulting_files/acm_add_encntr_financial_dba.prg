CREATE PROGRAM acm_add_encntr_financial:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->encntr_financial_qual[xref->add[index].idx].encntr_financial_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(encounter_only_seq,nextval)
     FROM dual
     DETAIL
      acm_request->encntr_financial_qual[xref->add[index].idx].encntr_financial_id = cnvtreal(
       nextseqnum), reply->encntr_financial_qual[xref->add[index].idx].encntr_financial_id = cnvtreal
      (nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->encntr_financial_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->encntr_financial_qual[xref->add[index].idx].encntr_financial_id = acm_request->
    encntr_financial_qual[xref->add[index].idx].encntr_financial_id
   ENDIF
   SET reply->encntr_financial_qual[xref->add[index].idx].status = 0
   IF ((acm_request->encntr_financial_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->encntr_financial_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->encntr_financial_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->encntr_financial_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   encntr_financial e
  SET e.encntr_financial_id = acm_request->encntr_financial_qual[xref->add[d.seq].idx].
   encntr_financial_id, e.bill_type_cd = acm_request->encntr_financial_qual[xref->add[d.seq].idx].
   bill_type_cd, e.contributor_system_cd =
   IF ((acm_request->encntr_financial_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->encntr_financial_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   e.person_id =
   IF ((acm_request->encntr_financial_qual[xref->add[d.seq].idx].person_id > 0)) acm_request->
    encntr_financial_qual[xref->add[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->encntr_financial_qual[xref->add[d.seq].idx].person_idx].
    person_id
   ENDIF
   , e.research_account = acm_request->encntr_financial_qual[xref->add[d.seq].idx].research_account,
   e.beg_effective_dt_tm = cnvtdatetime(acm_request->encntr_financial_qual[xref->add[d.seq].idx].
    beg_effective_dt_tm),
   e.end_effective_dt_tm = cnvtdatetime(acm_request->encntr_financial_qual[xref->add[d.seq].idx].
    end_effective_dt_tm), e.data_status_cd =
   IF ((acm_request->encntr_financial_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request->
    encntr_financial_qual[xref->add[d.seq].idx].data_status_cd
   ...
   IF ((((acm_request->encntr_financial_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((
   acm_request->encntr_financial_qual[xref->add[d.seq].idx].active_status_cd=reqdata->
   active_status_cd))) ) 1
   ELSE 0
   ENDIF
   , e.active_status_cd =
   IF ((acm_request->encntr_financial_qual[xref->add[d.seq].idx].active_status_cd > 0.0)) acm_request
    ->encntr_financial_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (e)
  WITH nocounter, status(reply->encntr_financial_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->encntr_financial_qual[xref->add[index].idx].status=0))
    SET failed = insert_error
    SET table_name = "ENCNTR_FINANCIAL"
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
