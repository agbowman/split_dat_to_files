CREATE PROGRAM afc_rca_save_modify_mult_chrgs:dba
 ...
 RECORD reqbillcodedetails(
   1 charge_event_mod_qual = i4
   1 charge_event_mod[*]
     2 charge_mod_id = f8
     2 charge_item_id = f8
     2 charge_event_mod_id = f8
 ...
 RECORD chargeeventmoddetails(
   1 chargeid[*]
     ...
     2 charge_event_mod[*]
       ...
       3 code1cd = f8
       3 nomen_id = f8
     2 charge_event_mod_icd_count = i4
     2 charge_event_mod_modfr_count = i4
 ) WITH protect
 RECORD chargemoddetails(
   1 charge_mod[*]
     2 charge_mod_id = f8
     2 charge_item_id = f8
     2 charge_event_mod_id = f8
 ...
 RECORD reqbillcodesmod(
   ...
   1 bill_code_details[*]
     ...
     2 code1cd = f8
     2 cm1nbr = f8
     2 chargemodsourcecd = f8
 ) WITH protect
 RECORD reqbillcodesmodnodup(
   1 bill_code_qual = i4
   1 bill_code_details[*]
     2 field1id = f8
     2 field2id = f8
 ...
 RECORD duplicateaddonlist(
   1 charges[*]
     2 chargeitemid = f8
 ) WITH protect
 CALL beginservice("CHARGSRV-14332.011")
 DECLARE deactivatechargemoddetails(dummyvar=i2) = i2
 ...
 ENDIF
 IF ( NOT (validate(cs13019_bill_cd)))
  DECLARE cs13019_bill_cd = f8 WITH protect, constant(getcodevalue(13019,"BILL CODE",0))
 ENDIF
 ...
 DECLARE posted_status = i4 WITH protect, constant(100)
 DECLARE action_begin = i4 WITH protect, noconstant(0)
 DECLARE action_end = i4 WITH protect, noconstant(0)
 DECLARE proc_code = vc WITH protect, constant("PROCCODE")
 DECLARE icd_dx_code = vc WITH protect, constant("ICD9")
 DECLARE postedflag = i4 WITH protect, constant(100)
 DECLARE num = i4 WITH protect, noconstant(0)
 DECLARE reprocessmodind = i2 WITH protect, noconstant(false)
 ...
 ENDIF
 IF ( NOT (evaluatechargesformodification(0)))
  CALL exitservicefailure("Failed to evaluate charges for modification",true)
 ENDIF
 CALL populatebillcoderecs14002(0)
 IF ( NOT (findcharges(0)))
  CALL exitservicefailure("Failed to find charge Details",true)
 ENDIF
 CALL loadchargedetails(0)
 CALL getintervalcharges(0)
 SET reprocessmodind = iscptandphyupdate(0)
 IF ( NOT (createnewcreditcharges(0)))
  CALL exitservicefailure("Failed to create the credit charge",true)
 ENDIF
 IF (reprocessmodind)
  CALL removeduplicateaddonsfromchargedetails(0)
 ENDIF
 IF ( NOT (createnewdebitcharges(0)))
  CALL exitservicefailure("Failed to create the debit charge",true)
 ENDIF
 IF ( NOT (postcharges(afcprofitrequest,afcprofitreply)))
...
#exit_script
 ...
 ENDIF
 FREE RECORD chargefindrequest
 FREE RECORD chargefindreply
 FREE RECORD addchargedetailsrequest
 FREE RECORD addchargedetailsreply
 FREE RECORD creditchargerequest
 FREE RECORD creditchargereply
 FREE RECORD afcprofitrequest
 FREE RECORD afcprofitreply
 FREE RECORD modifychargerequest
 FREE RECORD modifychargereply
 FREE RECORD reqbillcodesicd
 FREE RECORD reqbillcodesmod
 FREE RECORD reqbillcodesmodnodup
 FREE RECORD reqbillcodesproc
 FREE RECORD chargemodrequest
 FREE RECORD chargemodreply
 FREE RECORD chargeeventidlist
 FREE RECORD chargemoddetails
 FREE RECORD reqbillcodedetails
 FREE RECORD syncrelease_reply
 FREE RECORD reqchargeeventmod
 FREE RECORD chargeeventmoddetails
 FREE RECORD recmodifier14002
 FREE RECORD reprocrequest
 FREE RECORD publishupdatedchargerequest
 SUBROUTINE (evaluatechargesformodification(dummyvar=i2) =i2)
   CALL logmessage(curprog,"Entering...evaluateChargesForModification",log_debug)
   DECLARE evalidx = i4 WITH protect, noconstant(0)
   DECLARE statusind = i2 WITH protect, noconstant(false)
   SELECT INTO "nl:"
    FROM charge c
    WHERE expand(evalidx,1,size(request->chargelist,5),c.charge_item_id,request->chargelist[evalidx].
     chargeid)
    ...
    WITH expand = 1
   ;end select
   IF (curqual != size(request->chargelist,5))
    CALL logmessage(curprog,"evaluateChargeForModification - Failed to validate all the charges",
     log_debug)
   ELSE
    SET statusind = true
   ENDIF
   CALL logmessage(curprog,"evaluateChargesForModification Exiting...",log_debug)
   RETURN(statusind)
 END ;Subroutine
 SUBROUTINE (populatebillcoderecs14002(dummyvar=i2) =null)
   CALL logmessage(curprog,"Executing populateBillCodeRecs14002",log_debug)
   DECLARE countmod = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM code_value cv
    WHERE cv.cdf_meaning="MODIFIER"
     AND cv.code_set=14002
     AND cv.active_ind=1
    DETAIL
     countmod += 1, stat = alterlist(recmodifier14002->listmodifier,countmod), recmodifier14002->
     listmodifier[countmod].codevalue = cv.code_value
    WITH nocounter
   ;end select
   CALL logmessage(curprog,"Exiting populateBillCodeRecs14002",log_debug)
 END ;Subroutine
 SUBROUTINE (findcharges(dummyvar=i2) =i2)
   CALL logmessage(curprog,"Executing findCharges",log_debug)
   DECLARE loopcnt = i4 WITH protect, noconstant(0)
   SET stat = alterlist(chargefindrequest->charge_items,size(request->chargelist,5))
   FOR (loopcnt = 1 TO size(request->chargelist,5))
     SET chargefindrequest->charge_items[loopcnt].charge_item_id = request->chargelist[loopcnt].
     chargeid
   ENDFOR
   EXECUTE afc_charge_find  WITH replace("REQUEST",chargefindrequest), replace("REPLY",
    chargefindreply)
   IF ((chargefindreply->status_data.status != "S"))
    RETURN(false)
   ENDIF
   CALL logmessage(curprog,"Exiting findCharges",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (loadchargedetails(dummyvar=i2) =null)
   CALL logmessage(curprog,"loadChargeDetails - Entering...",log_debug)
   DECLARE chargeidx = i4 WITH protect, noconstant(0)
   SET stat = alterlist(addchargedetailsrequest->objarray,size(chargefindreply->charge_items,5))
   SET stat = alterlist(creditchargerequest->charge,size(chargefindreply->charge_items,5))
   FOR (chargeidx = 1 TO size(chargefindreply->charge_items,5))
     SET addchargedetailsrequest->objarray[chargeidx].charge_item_id = chargefindreply->charge_items[
     chargeidx].charge_item_id
     SET addchargedetailsrequest->objarray[chargeidx].parent_charge_item_id = chargefindreply->
     charge_items[chargeidx].parent_charge_item_id
     SET addchargedetailsrequest->objarray[chargeidx].charge_event_act_id = chargefindreply->
     charge_items[chargeidx].charge_event_act_id
     SET addchargedetailsrequest->objarray[chargeidx].charge_event_id = chargefindreply->
     charge_items[chargeidx].charge_event_id
     SET addchargedetailsrequest->objarray[chargeidx].bill_item_id = chargefindreply->charge_items[
     chargeidx].bill_item_id
     SET addchargedetailsrequest->objarray[chargeidx].order_id = chargefindreply->charge_items[
     chargeidx].order_id
     SET addchargedetailsrequest->objarray[chargeidx].encntr_id = chargefindreply->charge_items[
     chargeidx].encntr_id
     SET addchargedetailsrequest->objarray[chargeidx].person_id = chargefindreply->charge_items[
     chargeidx].person_id
     SET addchargedetailsrequest->objarray[chargeidx].payor_id = chargefindreply->charge_items[
     chargeidx].payor_id
     SET addchargedetailsrequest->objarray[chargeidx].ord_loc_cd = chargefindreply->charge_items[
     chargeidx].ord_loc_cd
     SET addchargedetailsrequest->objarray[chargeidx].perf_phys_id = chargefindreply->charge_items[
     chargeidx].perf_phys_id
     SET addchargedetailsrequest->objarray[chargeidx].perf_loc_cd = evaluate2(
      IF ((request->delperforminglocind=1)
       AND (request->performinglocationcd=0.00)) 0.00
      ELSEIF ((request->delperforminglocind=0)
       AND (request->performinglocationcd > 0.00)) request->performinglocationcd
      ELSE chargefindreply->charge_items[chargeidx].perf_loc_cd
      ENDIF
      )
     SET addchargedetailsrequest->objarray[chargeidx].ord_phys_id = evaluate2(
      IF ((request->delorderingphysind=1)
       AND (request->orderingphysicianid=0.00)) 0.00
      ELSEIF ((request->delorderingphysind=0)
       AND (request->orderingphysicianid > 0.00)) request->orderingphysicianid
      ELSE chargefindreply->charge_items[chargeidx].ord_phys_id
      ENDIF
      )
     SET addchargedetailsrequest->objarray[chargeidx].verify_phys_id = evaluate2(
      IF ((request->delrenderingphysind=1)
       AND (request->renderingphysicianid=0.00)) 0.00
      ELSEIF ((request->delrenderingphysind=0)
       AND (request->renderingphysicianid > 0.00)) request->renderingphysicianid
      ELSE chargefindreply->charge_items[chargeidx].verify_phys_id
      ENDIF
      )
     SET addchargedetailsrequest->objarray[chargeidx].charge_description = chargefindreply->
     charge_items[chargeidx].charge_description
     SET addchargedetailsrequest->objarray[chargeidx].price_sched_id = chargefindreply->charge_items[
     chargeidx].price_sched_id
     SET addchargedetailsrequest->objarray[chargeidx].item_quantity = chargefindreply->charge_items[
     chargeidx].item_quantity
     SET addchargedetailsrequest->objarray[chargeidx].item_price = chargefindreply->charge_items[
     chargeidx].item_price
     SET addchargedetailsrequest->objarray[chargeidx].item_extended_price = chargefindreply->
     charge_items[chargeidx].item_extended_price
     SET addchargedetailsrequest->objarray[chargeidx].item_allowable = chargefindreply->charge_items[
     chargeidx].item_allowable
     SET addchargedetailsrequest->objarray[chargeidx].item_copay = chargefindreply->charge_items[
     chargeidx].item_copay
     SET addchargedetailsrequest->objarray[chargeidx].charge_type_cd = chargefindreply->charge_items[
     chargeidx].charge_type_cd
     SET addchargedetailsrequest->objarray[chargeidx].research_acct_id = chargefindreply->
     charge_items[chargeidx].research_acct_id
     SET addchargedetailsrequest->objarray[chargeidx].suspense_rsn_cd = evaluate(cs4001989_modify_cd,
      0.0,request->suspensersncd,0.0)
     SET addchargedetailsrequest->objarray[chargeidx].reason_comment = evaluate(cs4001989_modify_cd,
      0.0,request->reasoncomment,"")
     SET addchargedetailsrequest->objarray[chargeidx].posted_cd = chargefindreply->charge_items[
     chargeidx].posted_cd
     SET addchargedetailsrequest->objarray[chargeidx].posted_dt_tm = cnvtdatetime(chargefindreply->
      charge_items[chargeidx].posted_dt_tm)
     SET addchargedetailsrequest->objarray[chargeidx].process_flg = pending_status
     SET addchargedetailsrequest->objarray[chargeidx].service_dt_tm = cnvtdatetime(chargefindreply->
      charge_items[chargeidx].service_dt_tm)
     SET addchargedetailsrequest->objarray[chargeidx].activity_dt_tm = cnvtdatetime(chargefindreply->
      charge_items[chargeidx].activity_dt_tm)
     SET addchargedetailsrequest->objarray[chargeidx].updt_cnt = 0
     SET addchargedetailsrequest->objarray[chargeidx].updt_dt_tm = cnvtdatetime(sysdate)
     SET addchargedetailsrequest->objarray[chargeidx].updt_id = reqinfo->updt_id
     SET addchargedetailsrequest->objarray[chargeidx].updt_task = reqinfo->updt_task
     SET addchargedetailsrequest->objarray[chargeidx].updt_applctx = reqinfo->updt_applctx
     SET addchargedetailsrequest->objarray[chargeidx].active_ind = chargefindreply->charge_items[
     chargeidx].active_ind
     SET addchargedetailsrequest->objarray[chargeidx].active_status_cd = chargefindreply->
     charge_items[chargeidx].active_status_cd
     SET addchargedetailsrequest->objarray[chargeidx].active_status_dt_tm = cnvtdatetime(
      chargefindreply->charge_items[chargeidx].active_status_dt_tm)
     SET addchargedetailsrequest->objarray[chargeidx].active_status_prsnl_id = chargefindreply->
     charge_items[chargeidx].active_status_prsnl_id
     SET addchargedetailsrequest->objarray[chargeidx].beg_effective_dt_tm = cnvtdatetime(
      chargefindreply->charge_items[chargeidx].beg_effective_dt_tm)
     SET addchargedetailsrequest->objarray[chargeidx].end_effective_dt_tm = cnvtdatetime(
      chargefindreply->charge_items[chargeidx].end_effective_dt_tm)
     SET addchargedetailsrequest->objarray[chargeidx].credited_dt_tm = cnvtdatetime(sysdate)
     SET addchargedetailsrequest->objarray[chargeidx].adjusted_dt_tm = cnvtdatetime(chargefindreply->
      charge_items[chargeidx].adjusted_dt_tm)
     SET addchargedetailsrequest->objarray[chargeidx].interface_file_id = chargefindreply->
     charge_items[chargeidx].interface_file_id
     SET addchargedetailsrequest->objarray[chargeidx].tier_group_cd = chargefindreply->charge_items[
     chargeidx].tier_group_cd
     SET addchargedetailsrequest->objarray[chargeidx].def_bill_item_id = chargefindreply->
     charge_items[chargeidx].def_bill_item_id
     SET addchargedetailsrequest->objarray[chargeidx].gross_price = chargefindreply->charge_items[
     chargeidx].gross_price
     SET addchargedetailsrequest->objarray[chargeidx].discount_amount = chargefindreply->
     charge_items[chargeidx].discount_amount
     SET addchargedetailsrequest->objarray[chargeidx].manual_ind = chargefindreply->charge_items[
     chargeidx].manual_ind
     SET addchargedetailsrequest->objarray[chargeidx].combine_ind = chargefindreply->charge_items[
     chargeidx].combine_ind
     SET addchargedetailsrequest->objarray[chargeidx].activity_type_cd = chargefindreply->
     charge_items[chargeidx].activity_type_cd
     SET addchargedetailsrequest->objarray[chargeidx].provider_specialty_cd = chargefindreply->
     charge_items[chargeidx].provider_specialty_cd
     SET addchargedetailsrequest->objarray[chargeidx].activity_dt_tm = cnvtdatetime(chargefindreply->
      charge_items[chargeidx].activity_dt_tm)
     SET addchargedetailsrequest->objarray[chargeidx].admit_type_cd = chargefindreply->charge_items[
     chargeidx].admit_type_cd
     SET addchargedetailsrequest->objarray[chargeidx].bundle_id = chargefindreply->charge_items[
     chargeidx].bundle_id
     SET addchargedetailsrequest->objarray[chargeidx].department_cd = chargefindreply->charge_items[
     chargeidx].department_cd
     SET addchargedetailsrequest->objarray[chargeidx].institution_cd = chargefindreply->charge_items[
     chargeidx].institution_cd
     SET addchargedetailsrequest->objarray[chargeidx].level5_cd = chargefindreply->charge_items[
     chargeidx].level5_cd
     SET addchargedetailsrequest->objarray[chargeidx].med_service_cd = chargefindreply->charge_items[
     chargeidx].med_service_cd
     SET addchargedetailsrequest->objarray[chargeidx].section_cd = chargefindreply->charge_items[
     chargeidx].section_cd
     SET addchargedetailsrequest->objarray[chargeidx].subsection_cd = chargefindreply->charge_items[
     chargeidx].subsection_cd
     SET addchargedetailsrequest->objarray[chargeidx].abn_status_cd = chargefindreply->charge_items[
     chargeidx].abn_status_cd
     SET addchargedetailsrequest->objarray[chargeidx].cost_center_cd = chargefindreply->charge_items[
     chargeidx].cost_center_cd
     SET addchargedetailsrequest->objarray[chargeidx].inst_fin_nbr = chargefindreply->charge_items[
     chargeidx].inst_fin_nbr
     SET addchargedetailsrequest->objarray[chargeidx].fin_class_cd = chargefindreply->charge_items[
     chargeidx].fin_class_cd
     SET addchargedetailsrequest->objarray[chargeidx].health_plan_id = chargefindreply->charge_items[
     chargeidx].health_plan_id
     SET addchargedetailsrequest->objarray[chargeidx].item_interval_id = chargefindreply->
     charge_items[chargeidx].item_interval_id
     SET addchargedetailsrequest->objarray[chargeidx].item_list_price = chargefindreply->
     charge_items[chargeidx].item_list_price
     SET addchargedetailsrequest->objarray[chargeidx].item_reimbursement = chargefindreply->
     charge_items[chargeidx].item_reimbursement
     SET addchargedetailsrequest->objarray[chargeidx].list_price_sched_id = chargefindreply->
     charge_items[chargeidx].list_price_sched_id
     SET addchargedetailsrequest->objarray[chargeidx].payor_type_cd = chargefindreply->charge_items[
     chargeidx].payor_type_cd
     SET addchargedetailsrequest->objarray[chargeidx].epsdt_ind = chargefindreply->charge_items[
     chargeidx].epsdt_ind
     SET addchargedetailsrequest->objarray[chargeidx].ref_phys_id = chargefindreply->charge_items[
     chargeidx].ref_phys_id
     SET addchargedetailsrequest->objarray[chargeidx].start_dt_tm = cnvtdatetime(chargefindreply->
      charge_items[chargeidx].start_dt_tm)
     SET addchargedetailsrequest->objarray[chargeidx].stop_dt_tm = cnvtdatetime(chargefindreply->
      charge_items[chargeidx].stop_dt_tm)
     SET addchargedetailsrequest->objarray[chargeidx].server_process_flag = evaluate2(
      IF ((((request->delorderingphysind=1)) OR ((((request->orderingphysicianid > 0.00)) OR ((((
      request->delrenderingphysind=1)) OR ((request->renderingphysicianid > 0.00))) )) )) ) 0
      ELSE chargefindreply->charge_items[chargeidx].server_process_flag
      ENDIF
      )
     SET addchargedetailsrequest->objarray[chargeidx].item_deductible_amt = chargefindreply->
     charge_items[chargeidx].item_deductible_amt
     SET addchargedetailsrequest->objarray[chargeidx].patient_responsibility_flag = chargefindreply->
     charge_items[chargeidx].patient_responsibility_flag
     SET addchargedetailsrequest->objarray[chargeidx].offset_charge_item_id = 0.0
     IF ((addchargedetailsrequest->objarray[chargeidx].item_interval_id <= 0))
      SELECT INTO "nl:"
       FROM price_sched_items psi
       WHERE (psi.bill_item_id=chargefindreply->charge_items[chargeidx].bill_item_id)
        AND (psi.price_sched_id=chargefindreply->charge_items[chargeidx].price_sched_id)
        AND psi.beg_effective_dt_tm <= cnvtdatetime(chargefindreply->charge_items[chargeidx].
        service_dt_tm)
        AND psi.end_effective_dt_tm >= cnvtdatetime(chargefindreply->charge_items[chargeidx].
        service_dt_tm)
        AND psi.active_ind=1
       DETAIL
        addchargedetailsrequest->objarray[chargeidx].interval_template_cd = psi.interval_template_cd
       WITH nocounter
      ;end select
     ELSE
      SET addchargedetailsrequest->objarray[chargeidx].interval_template_cd = 0.00
     ENDIF
     SET stat = moverec(chargefindreply->charge_items[chargeidx].charge_mods,addchargedetailsrequest
      ->objarray[chargeidx].charge_mod)
     SET creditchargerequest->charge[chargeidx].charge_item_id = chargefindreply->charge_items[
     chargeidx].charge_item_id
     SET creditchargerequest->charge[chargeidx].suspense_rsn_cd = request->suspensersncd
     SET creditchargerequest->charge[chargeidx].reason_comment = request->reasoncomment
   ENDFOR
   CALL logmessage(curprog,"loadChargeDetails - Exiting...",log_debug)
 END ;Subroutine
 SUBROUTINE getintervalcharges(dummyvar)
   CALL logmessage(curprog,"Entering...getIntervalCharges",log_debug)
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = value(size(addchargedetailsrequest->objarray,5)))
    PLAN (d1)
    DETAIL
     IF ((((addchargedetailsrequest->objarray[d1.seq].item_interval_id > 0)) OR ((
     addchargedetailsrequest->objarray[d1.seq].interval_template_cd > 0))) )
      CALL get_all_related_charges(addchargedetailsrequest->objarray[d1.seq].item_interval_id,
      addchargedetailsrequest->objarray[d1.seq].charge_event_act_id,addchargedetailsrequest->
      objarray[d1.seq].tier_group_cd)
     ENDIF
    WITH nocounter
   ;end select
   CALL logmessage(curprog,"getIntervalCharges Exiting...",log_debug)
 END ;Subroutine
 SUBROUTINE (get_all_related_charges(item_interval_id=f8,charge_event_act_id=f8,tier_group_cd=f8) =
  null)
   ...
   DECLARE chgcount = i4 WITH protect, noconstant(0)
   DECLARE chrgmodcnt = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM item_interval_table i
    WHERE i.item_interval_id=item_interval_id
   ...
   SET chgcount = size(addchargedetailsrequest->objarray,5)
   SELECT INTO "nl:"
    ...
    PLAN (iit
     ...
     WHERE c.item_interval_id=iit.item_interval_id
      AND c.charge_event_act_id=charge_event_act_id
    ...
    HEAD c.charge_item_id
     chrgmodcnt = 0, chgcount += 1, stat = alterlist(addchargedetailsrequest->objarray,chgcount),
     stat = alterlist(creditchargerequest->charge,chgcount), addchargedetailsrequest->objarray[
     chgcount].charge_item_id = c.charge_item_id, addchargedetailsrequest->objarray[chgcount].
     parent_charge_item_id = c.parent_charge_item_id,
     addchargedetailsrequest->objarray[chgcount].charge_event_act_id = c.charge_event_act_id,
     addchargedetailsrequest->objarray[chgcount].charge_event_id = c.charge_event_id,
     addchargedetailsrequest->objarray[chgcount].bill_item_id = c.bill_item_id,
     addchargedetailsrequest->objarray[chgcount].order_id = c.order_id, addchargedetailsrequest->
     objarray[chgcount].encntr_id = c.encntr_id, addchargedetailsrequest->objarray[chgcount].
     person_id = c.person_id,
     addchargedetailsrequest->objarray[chgcount].payor_id = c.payor_id, addchargedetailsrequest->
     objarray[chgcount].ord_loc_cd = c.ord_loc_cd, addchargedetailsrequest->objarray[chgcount].
     perf_phys_id = c.perf_phys_id,
     addchargedetailsrequest->objarray[chargeidx].perf_loc_cd = evaluate2(
      IF ((request->delperforminglocind=1)
       AND (request->performinglocationcd=0.00)) 0.00
      ELSEIF ((request->delperforminglocind=0)
       AND (request->performinglocationcd > 0.00)) request->performinglocationcd
      ELSE c.perf_loc_cd
      ENDIF
      ), addchargedetailsrequest->objarray[chargeidx].ord_phys_id = evaluate2(
      IF ((request->delorderingphysind=1)
       AND (request->orderingphysicianid=0.00)) 0.00
      ELSEIF ((request->delorderingphysind=0)
       AND (request->orderingphysicianid > 0.00)) request->orderingphysicianid
      ELSE c.ord_phys_id
      ENDIF
      ), addchargedetailsrequest->objarray[chargeidx].verify_phys_id = evaluate2(
      IF ((request->delrenderingphysind=1)
       AND (request->renderingphysicianid=0.00)) 0.00
      ELSEIF ((request->delrenderingphysind=0)
       AND (request->renderingphysicianid > 0.00)) request->renderingphysicianid
     ...
     addchargedetailsrequest->objarray[chgcount].charge_description = c.charge_description,
     addchargedetailsrequest->objarray[chgcount].price_sched_id = c.price_sched_id,
     addchargedetailsrequest->objarray[chgcount].item_quantity = c.item_quantity,
     addchargedetailsrequest->objarray[chgcount].item_price = c.item_price, addchargedetailsrequest->
     objarray[chgcount].item_extended_price = c.item_extended_price, addchargedetailsrequest->
     objarray[chgcount].item_allowable = c.item_allowable,
     addchargedetailsrequest->objarray[chgcount].item_copay = c.item_copay, addchargedetailsrequest->
     objarray[chgcount].charge_type_cd = c.charge_type_cd, addchargedetailsrequest->objarray[chgcount
     ].research_acct_id = c.research_acct_id,
     addchargedetailsrequest->objarray[chgcount].suspense_rsn_cd = c.suspense_rsn_cd,
     addchargedetailsrequest->objarray[chgcount].reason_comment = c.reason_comment
     IF (cs13019_mod_rsn_cd <= 0.0)
      addchargedetailsrequest->objarray[chgcount].suspense_rsn_cd = request->suspensersncd,
      addchargedetailsrequest->objarray[chgcount].reason_comment = request->reasoncomment
     ENDIF
     addchargedetailsrequest->objarray[chgcount].posted_cd = c.posted_cd, addchargedetailsrequest->
     objarray[chgcount].posted_dt_tm = c.posted_dt_tm, addchargedetailsrequest->objarray[chgcount].
     process_flg = c.process_flg,
     addchargedetailsrequest->objarray[chgcount].service_dt_tm = c.service_dt_tm,
     addchargedetailsrequest->objarray[chgcount].activity_dt_tm = c.activity_dt_tm,
     addchargedetailsrequest->objarray[chgcount].updt_cnt = c.updt_cnt,
     addchargedetailsrequest->objarray[chgcount].updt_dt_tm = c.updt_dt_tm, addchargedetailsrequest->
     objarray[chgcount].updt_id = c.updt_id, addchargedetailsrequest->objarray[chgcount].updt_task =
     c.updt_task,
     addchargedetailsrequest->objarray[chgcount].updt_applctx = c.updt_applctx,
     addchargedetailsrequest->objarray[chgcount].active_status_dt_tm = c.active_status_dt_tm,
     addchargedetailsrequest->objarray[chgcount].active_status_prsnl_id = c.active_status_prsnl_id,
     addchargedetailsrequest->objarray[chgcount].active_ind = c.active_ind, addchargedetailsrequest->
     objarray[chgcount].active_status_cd = c.active_status_cd, addchargedetailsrequest->objarray[
     chgcount].beg_effective_dt_tm = c.beg_effective_dt_tm,
     addchargedetailsrequest->objarray[chgcount].end_effective_dt_tm = c.end_effective_dt_tm,
     addchargedetailsrequest->objarray[chgcount].credited_dt_tm = c.credited_dt_tm,
     addchargedetailsrequest->objarray[chgcount].adjusted_dt_tm = c.adjusted_dt_tm,
     addchargedetailsrequest->objarray[chgcount].interface_file_id = c.interface_file_id,
     addchargedetailsrequest->objarray[chgcount].tier_group_cd = c.tier_group_cd,
     addchargedetailsrequest->objarray[chgcount].def_bill_item_id = c.def_bill_item_id,
     addchargedetailsrequest->objarray[chgcount].gross_price = c.gross_price, addchargedetailsrequest
     ->objarray[chgcount].discount_amount = c.discount_amount, addchargedetailsrequest->objarray[
     chgcount].manual_ind = c.manual_ind,
     addchargedetailsrequest->objarray[chgcount].combine_ind = c.combine_ind, addchargedetailsrequest
     ->objarray[chgcount].activity_type_cd = c.activity_type_cd, addchargedetailsrequest->objarray[
     chgcount].activity_sub_type_cd = c.activity_sub_type_cd,
     addchargedetailsrequest->objarray[chgcount].provider_specialty_cd = c.provider_specialty_cd,
     addchargedetailsrequest->objarray[chgcount].admit_type_cd = c.admit_type_cd,
     addchargedetailsrequest->objarray[chgcount].bundle_id = c.bundle_id,
     addchargedetailsrequest->objarray[chgcount].department_cd = c.department_cd,
     addchargedetailsrequest->objarray[chgcount].institution_cd = c.institution_cd,
     addchargedetailsrequest->objarray[chgcount].level5_cd = c.level5_cd,
     addchargedetailsrequest->objarray[chgcount].med_service_cd = c.med_service_cd,
     addchargedetailsrequest->objarray[chgcount].section_cd = c.section_cd, addchargedetailsrequest->
     objarray[chgcount].subsection_cd = c.subsection_cd,
     addchargedetailsrequest->objarray[chgcount].abn_status_cd = c.abn_status_cd,
     addchargedetailsrequest->objarray[chgcount].cost_center_cd = c.cost_center_cd,
     addchargedetailsrequest->objarray[chgcount].inst_fin_nbr = c.inst_fin_nbr,
     addchargedetailsrequest->objarray[chgcount].fin_class_cd = c.fin_class_cd,
     addchargedetailsrequest->objarray[chgcount].health_plan_id = c.health_plan_id,
     addchargedetailsrequest->objarray[chgcount].item_interval_id = c.item_interval_id,
     addchargedetailsrequest->objarray[chgcount].item_list_price = c.item_list_price,
     addchargedetailsrequest->objarray[chgcount].item_reimbursement = c.item_reimbursement,
     addchargedetailsrequest->objarray[chgcount].list_price_sched_id = c.list_price_sched_id,
     addchargedetailsrequest->objarray[chgcount].payor_type_cd = c.payor_type_cd,
     addchargedetailsrequest->objarray[chgcount].epsdt_ind = c.epsdt_ind, addchargedetailsrequest->
     objarray[chgcount].ref_phys_id = c.ref_phys_id,
     addchargedetailsrequest->objarray[chgcount].start_dt_tm = c.start_dt_tm, addchargedetailsrequest
     ->objarray[chgcount].stop_dt_tm = c.stop_dt_tm, addchargedetailsrequest->objarray[chgcount].
     alpha_nomen_id = c.alpha_nomen_id,
     addchargedetailsrequest->objarray[chgcount].server_process_flag = evaluate2(
      IF ((((request->delorderingphysind=1)) OR ((((request->orderingphysicianid > 0.00)) OR ((((
      request->delrenderingphysind=1)) OR ((request->renderingphysicianid > 0.00))) )) )) ) 0
      ELSE c.server_process_flag
      ENDIF
      ), addchargedetailsrequest->objarray[chgcount].offset_charge_item_id = c.offset_charge_item_id,
     addchargedetailsrequest->objarray[chgcount].item_deductible_amt = c.item_deductible_amt,
     addchargedetailsrequest->objarray[chgcount].patient_responsibility_flag = c
     .patient_responsibility_flag, creditchargerequest->charge[chgcount].charge_item_id = c
     .charge_item_id, creditchargerequest->charge[chgcount].suspense_rsn_cd = request->suspensersncd,
     creditchargerequest->charge[chgcount].reason_comment = request->reasoncomment
    DETAIL
     chrgemodcnt = (chrgmodcnt+ 1), stat = alterlist(addchargedetailsrequest->objarray[chgcount].
      charge_mod,chrgmodcnt), addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].
     charge_mod_id = cm.charge_mod_id,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].charge_mod_type_cd = cm
     .charge_mod_type_cd, addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field1
      = cm.field1, addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field2 = cm
     .field3,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field4 = cm.field4,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field5 = cm.field5,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field6 = cm.field6,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field7 = cm.field7,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field8 = cm.field8,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field9 = cm.field9,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field10 = cm.field10,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field1_id = cm.field1_id,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field2_id = cm.field2_id,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field3_id = cm.field3_id,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field4_id = cm.field4_id,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].field5_id = cm.field5_id,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].nomen_id = cm.nomen_id,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].cm1_nbr = cm.cm1_nbr,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].activity_dt_tm = cm
     .activity_dt_tm,
     addchargedetailsrequest->objarray[chgcount].charge_mod[chrgmodcnt].active_ind = cm.active_ind
    WITH nocounter
   ;end select
   CALL logmessage(curprog,"Exiting subroutine get_all_related_charges",log_debug)
 END ;Subroutine
 SUBROUTINE (createnewcreditcharges(dummyvar=i2) =i2)
   CALL logmessage(curprog,"createNewCreditCharges - Entering...",log_debug)
   DECLARE pftcnt = i4 WITH noconstant(0)
   ...
   IF ((creditchargereply->status_data.status != "S"))
    RETURN(false)
   ENDIF
   SET pftcnt = size(afcprofitrequest->charges,5)
   SELECT INTO "nl:"
    ...
    DETAIL
     IF (i.profit_type_cd > 0
      ...
      pftcnt += 1, stat = alterlist(afcprofitrequest->charges,pftcnt), afcprofitrequest->charges[
      pftcnt].charge_item_id = creditchargereply->charge[d1.seq].charge_item_id
     ENDIF
    WITH nocounter
   ;end select
   CALL logmessage(curprog,"createNewCreditCharges Exiting...",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (getduplicateaddonforcredit(dummy=i2) =i2)
   ...
   DECLARE chargeidx = i4 WITH protect, noconstant(0)
   DECLARE isgenericaddonfound = i2 WITH protect, noconstant(false)
   DECLARE isspecificaddonfound = i2 WITH protect, noconstant(false)
   DECLARE isdefaultaddonfound = i2 WITH protect, noconstant(false)
   DECLARE creditchargepos = i4 WITH protect, noconstant(0)
   DECLARE chargeeventpos = i4 WITH protect, noconstant(0)
   DECLARE duplicateaddoncnt = i4 WITH protect, noconstant(0)
   ...
   FOR (chargeidx = 1 TO size(request->chargelist,5))
     SET isgenericaddonfound = false
     SET isspecificaddonfound = false
     SET isdefaultaddonfound = false
     SET chargeeventpos = locateval(num,1,size(chargefindreply->charge_items,5),request->chargelist[
      chargeidx].chargeid,chargefindreply->charge_items[num].charge_item_id)
     IF (chargeeventpos=0)
      RETURN(false)
     ENDIF
     IF (chargeeventpos > 0)
      SELECT INTO "nl:"
       FROM charge c,
        bill_item b
       PLAN (c
        WHERE (c.charge_item_id=request->chargelist[chargeidx].chargeid)
         AND c.active_ind=true)
        JOIN (b
      ...
      ELSE
       SELECT INTO "nl:"
        FROM charge c,
         ...
         charge c2
        PLAN (c
         WHERE (c.charge_event_id=chargefindreply->charge_items[chargeeventpos].charge_event_id))
         JOIN (b
         WHERE b.bill_item_id=c.bill_item_id)
         JOIN (bim
         ...
         WHERE c2.bill_item_id=b.bill_item_id
          AND (c2.charge_item_id=request->chargelist[chargeidx].chargeid))
        WITH nocounter
       ;end select
       IF (curqual > 0)
        IF (validate(debug,- (1)) > 0)
       ...
       ENDIF
      ENDIF
      IF ( NOT (isgenericaddonfound)
       AND  NOT (isspecificaddonfound)
       AND  NOT (isdefaultaddonfound))
       SELECT INTO "nl:"
        FROM charge c,
         bill_item b
        PLAN (c
         WHERE (c.charge_item_id != request->chargelist[chargeidx].chargeid)
          AND (c.charge_event_id=chargefindreply->charge_items[chargeeventpos].charge_event_id)
          AND (c.tier_group_cd=chargefindreply->charge_items[chargeeventpos].tier_group_cd)
          AND c.active_ind=1
          AND c.charge_type_cd=cs13028_debit_cd
        ...
        DETAIL
         creditchargepos = locateval(num,1,size(addchargedetailsrequest->objarray,5),c.charge_item_id,
          addchargedetailsrequest->objarray[num].charge_item_id)
         IF (creditchargepos=0)
          crchrgcount += 1, stat = alterlist(creditchargerequest->charge,crchrgcount),
          creditchargerequest->charge[crchrgcount].charge_item_id = c.charge_item_id,
          creditchargerequest->charge[crchrgcount].late_charge_processing_ind = 0.0,
          creditchargerequest->charge[crchrgcount].reason_comment = request->reasoncomment,
          creditchargerequest->charge[crchrgcount].suspense_rsn_cd = request->suspensersncd
         ELSE
          duplicateaddoncnt += 1, stat = alterlist(duplicateaddonlist->charges,duplicateaddoncnt),
          duplicateaddonlist->charges[duplicateaddoncnt].chargeitemid = c.charge_item_id
         ENDIF
        WITH nocounter
       ;end select
       SELECT INTO "nl:"
   ...
   ENDFOR
 END ;Subroutine
 SUBROUTINE (removeduplicateaddonsfromchargedetails(dummyvar=i2) =null)
   CALL logmessage(curprog,"removeDuplicateAddOnsFromChargeDetails Entering...",log_debug)
   DECLARE duplicateaddonchargepos = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = value(size(duplicateaddonlist->charges,5)))
    PLAN (d1)
    DETAIL
     duplicateaddonchargepos = locateval(num,1,size(addchargedetailsrequest->objarray,5),
      duplicateaddonlist->charges[d1.seq].chargeitemid,addchargedetailsrequest->objarray[num].
      charge_item_id)
     IF (duplicateaddonchargepos > 0)
      stat = alterlist(addchargedetailsrequest->objarray,(size(addchargedetailsrequest->objarray,5)
        - 1),(duplicateaddonchargepos - 1))
     ENDIF
    WITH nocounter
   ;end select
   CALL logmessage(curprog,"removeDuplicateAddOnsFromChargeDetails - Exiting...",log_debug)
 END ;Subroutine
 SUBROUTINE (iscptandphyupdate(dummyvar=i2) =i2)
   CALL logmessage(curprog,"cptAndPhyUpdate Entering...",log_debug)
   DECLARE locatemodifier = i4 WITH protect, noconstant(0)
   DECLARE modindex = i4 WITH protect, noconstant(0)
   DECLARE chargemodcount = i4 WITH protect, noconstant(0)
   DECLARE hascptmodifier = i2 WITH protect, noconstant(false)
   DECLARE physicianchange = i2 WITH protect, noconstant(false)
   DECLARE cptmodifierqualifierind = i2 WITH protect, noconstant(false)
   DECLARE ordphysqualifierind = i2 WITH protect, noconstant(false)
   DECLARE rendphysqualifierind = i2 WITH protect, noconstant(false)
   FOR (chargemodcount = 1 TO size(request->billcodesmodel,5))
    SET locatemodifier = locateval(modindex,1,size(recmodifier14002->listmodifier,5),request->
     billcodesmodel[chargemodcount].field1id,recmodifier14002->listmodifier[modindex].codevalue)
    IF (locatemodifier > 0)
     SET hascptmodifier = true
    ENDIF
   ENDFOR
   IF (hascptmodifier)
    FOR (x = 1 TO size(request->chargelist,5))
      IF (checktierforcptmod(request->chargelist[x].chargeid))
       SET cptmodifierqualifierind = true
      ENDIF
    ENDFOR
   ENDIF
   IF ((((request->orderingphysicianid > 0.00)) OR (request->delorderingphysind)) )
    SET physicianchange = true
    FOR (x = 1 TO size(request->chargelist,5))
      IF (checktierforordphys(request->chargelist[x].chargeid))
       SET ordphysqualifierind = true
      ENDIF
    ENDFOR
   ENDIF
   IF ((((request->renderingphysicianid > 0.00)) OR (request->delrenderingphysind)) )
    SET physicianchange = true
    FOR (x = 1 TO size(request->chargelist,5))
      IF (checktierforrendphys(request->chargelist[x].chargeid))
       SET rendphysqualifierind = true
      ENDIF
    ENDFOR
   ENDIF
   IF (((((physicianchange) OR (((ordphysqualifierind) OR (rendphysqualifierind)) )) ) OR (
   hascptmodifier
    AND cptmodifierqualifierind)) )
    RETURN(true)
   ENDIF
 END ;Subroutine
 SUBROUTINE (createnewdebitcharges(dummyvar=i2) =i2)
   CALL logmessage(curprog,"createNewDebitCharges Entering...",log_debug)
   DECLARE chargecount = i4 WITH protect, noconstant(0)
   DECLARE pftcnt = i4 WITH protect, noconstant(0)
   DECLARE chrgevntcount = i4 WITH protect, noconstant(0)
   DECLARE chargeitemcount = i4 WITH protect, noconstant(0)
   DECLARE chgevntidx = i4 WITH protect, noconstant(0)
   DECLARE locchgeventid = i4 WITH protect, noconstant(0)
   DECLARE new_cem_id = f8 WITH protect, noconstant(0)
   DECLARE modcnt = i4 WITH protect, noconstant(0)
   DECLARE locmod = i4 WITH protect, noconstant(0)
   DECLARE cmcount = i4 WITH protect, noconstant(0)
   DECLARE locidx = i4 WITH protect, noconstant(0)
   DECLARE num = i4 WITH protect, noconstant(0)
   DECLARE num1 = i4 WITH protect, noconstant(0)
   DECLARE countmod = i4 WITH protect, noconstant(0)
   DECLARE countevtmod = i4 WITH protect, noconstant(0)
   DECLARE indx = i4 WITH protect, noconstant(0)
   DECLARE indx1 = i4 WITH protect, noconstant(0)
   DECLARE idxmod = i4 WITH protect, noconstant(0)
   DECLARE idxmod1 = i4 WITH protect, noconstant(0)
   DECLARE updidx = i4 WITH protect, noconstant(0)
   DECLARE locatemodifier = i4 WITH protect, noconstant(0)
   DECLARE modindex = i4 WITH protect, noconstant(0)
   DECLARE sizeaddchargedetailsreply = i4 WITH protect, noconstant(0)
   DECLARE chargemodcount = i4 WITH protect, noconstant(0)
   ...
   DECLARE ordphysqualifierind = i2 WITH protect, noconstant(false)
   DECLARE rendphysqualifierind = i2 WITH protect, noconstant(false)
   DECLARE manualmodifierind = i2 WITH protect, noconstant(false)
   DECLARE counter1 = i4 WITH protect, noconstant(0)
   DECLARE chargecnt = i4 WITH protect, noconstant(0)
   DECLARE chargepos = i4 WITH protect, noconstant(0)
   DECLARE flexloc = i4 WITH protect, noconstant(0)
   DECLARE idxmod2 = i4 WITH protect, noconstant(0)
   FOR (chargecount = 1 TO size(addchargedetailsrequest->objarray,5))
    SET addchargedetailsrequest->objarray[chargecount].parent_charge_item_id =
    addchargedetailsrequest->objarray[chargecount].charge_item_id
    SET addchargedetailsrequest->objarray[chargecount].charge_item_id = 0.0
   ENDFOR
   ...
   IF ((addchargedetailsreply->status_data.status != "S"))
    ...
    RETURN(false)
   ENDIF
   SET sizeaddchargedetailsreply = size(addchargedetailsreply->mod_objs[1].mod_recs,5)
   SET chrgevntcount = 0
   IF (sizeaddchargedetailsreply > 0)
    SELECT INTO "nl:"
     FROM (dummyt d1  WITH seq = value(size(addchargedetailsreply->mod_objs[1].mod_recs,5))),
      charge c
     PLAN (d1)
      JOIN (c
      WHERE c.charge_item_id=cnvtreal(addchargedetailsreply->mod_objs[1].mod_recs[d1.seq].pk_values))
     ORDER BY c.charge_event_id, c.charge_item_id
     HEAD c.charge_event_id
      chrgevntcount += 1, stat = alterlist(chargeeventidlist->charge_event,chrgevntcount),
      chargeeventidlist->charge_event[chrgevntcount].charge_event_id = c.charge_event_id,
      chargeitemcount = 0
     HEAD c.charge_item_id
      chargeitemcount += 1, stat = alterlist(chargeeventidlist->charge_event[chrgevntcount].
       charge_item,chargeitemcount), chargeeventidlist->charge_event[chrgevntcount].charge_item[
      chargeitemcount].charge_item_id = c.charge_item_id,
      chargeeventidlist->charge_event[chrgevntcount].charge_item[chargeitemcount].
      parent_charge_item_id = c.parent_charge_item_id
     WITH nocounter
    ;end select
   ENDIF
   CALL loadbillcodes(0)
   IF (isscsaenabled)
    IF (validate(request->flexfieldsmodel))
     IF ( NOT (loadflexfielddataforchargerequest(0)))
      CALL logmessage("loadFlexFieldDataForChargeRequest",build(
        "Failed to load Flex field details for charges : "),log_debug)
     ENDIF
    ENDIF
   ENDIF
   CALL loadchargemodifierwithbillencntr(0)
   FOR (chargemodcount = 1 TO size(request->billcodesmodel,5))
    SET locatemodifier = locateval(modindex,1,size(recmodifier14002->listmodifier,5),request->
     billcodesmodel[chargemodcount].field1id,recmodifier14002->listmodifier[modindex].codevalue)
    IF (locatemodifier > 0)
   ...
   SELECT INTO "nl"
    ...
    WHERE expand(updidx,1,size(chargeeventidlist->charge_event,5),cem.charge_event_id,
     chargeeventidlist->charge_event[updidx].charge_event_id)
    HEAD REPORT
     chargeeventmodcount = 0
    DETAIL
     chargeeventmodcount += 1
     IF (mod(chargeeventmodcount,10)=1)
      stat = alterlist(delchargeeventmodreq->objarray,(chargeeventmodcount+ 9))
     ENDIF
     delchargeeventmodreq->objarray[chargeeventmodcount].action_type = "DEL", delchargeeventmodreq->
     objarray[chargeeventmodcount].charge_event_mod_id = cem.charge_event_mod_id,
     delchargeeventmodreq->objarray[chargeeventmodcount].charge_event_id = cem.charge_event_id,
     delchargeeventmodreq->objarray[chargeeventmodcount].active_status_cd = 0.00,
     delchargeeventmodreq->objarray[chargeeventmodcount].updt_cnt = cem.updt_cnt
    FOOT REPORT
     stat = alterlist(delchargeeventmodreq->objarray,chargeeventmodcount)
    WITH nocounter
   ;end select
   IF (size(delchargeeventmodreq->objarray,5) > 0)
    ...
    IF ((delchargeeventmodrep->status_data.status != "S"))
     CALL logmessage(curprog,"afc_val_charge_event_mod did not return success during delete",
      log_debug)
     IF (validate(debug,- (1)) > 0)
    ...
    ENDIF
   ENDIF
   IF (size(reqbillcodedetails->charge_event_mod,5) > 0)
    SELECT INTO "nl:"
     FROM (dummyt d1  WITH seq = value(size(reqbillcodedetails->charge_event_mod,5))),
      charge c
     PLAN (d1)
      JOIN (c
      WHERE (c.charge_item_id=reqbillcodedetails->charge_event_mod[d1.seq].charge_item_id))
     HEAD REPORT
      stat = alterlist(reqchargeeventmod->charge_event_mod,size(reqbillcodedetails->charge_event_mod,
        5))
     DETAIL
      indx = locateval(idxmod,1,size(reqchargeeventmod->charge_event_mod,5),c.charge_event_id,
       reqchargeeventmod->charge_event_mod[idxmod].charge_event_id,
       reqbillcodedetails->charge_event_mod[d1.seq].field1_id,reqchargeeventmod->charge_event_mod[
       idxmod].field1_id,reqbillcodedetails->charge_event_mod[d1.seq].field6,reqchargeeventmod->
       charge_event_mod[idxmod].field6,reqbillcodedetails->charge_event_mod[d1.seq].field2_id,
       reqchargeeventmod->charge_event_mod[idxmod].field2_id), indx1 = locateval(idxmod1,1,size(
        request->billcodesmodel,5),reqbillcodedetails->charge_event_mod[d1.seq].field6,request->
       billcodesmodel[idxmod1].field6,
       "MODIFIER",uar_get_code_meaning(request->billcodesmodel[idxmod1].field1id)), flexloc =
      locateval(idxmod2,1,size(reqchargeeventmod->charge_event_mod,5),c.charge_event_id,
       reqchargeeventmod->charge_event_mod[idxmod].charge_event_id,
       reqbillcodedetails->charge_event_mod[d1.seq].field1_id,reqchargeeventmod->charge_event_mod[
       idxmod].field1_id)
      IF (((indx=0
       AND indx1=0) OR ((reqbillcodedetails->charge_event_mod[d1.seq].charge_mod_type_cd=
      cs13019_flex_cd)
       AND flexloc=0)) )
       countmod += 1, reqchargeeventmod->charge_event_mod[countmod].charge_event_id = c
       .charge_event_id, reqchargeeventmod->charge_event_mod[countmod].charge_mod_type_cd =
       reqbillcodedetails->charge_event_mod[d1.seq].charge_mod_type_cd,
       reqchargeeventmod->charge_event_mod[countmod].field6 = reqbillcodedetails->charge_event_mod[d1
       .seq].field6, reqchargeeventmod->charge_event_mod[countmod].field7 = reqbillcodedetails->
       charge_event_mod[d1.seq].field7, reqchargeeventmod->charge_event_mod[countmod].nomen_id =
       reqbillcodedetails->charge_event_mod[d1.seq].nomen_id,
       reqchargeeventmod->charge_event_mod[countmod].field1_id = reqbillcodedetails->
       charge_event_mod[d1.seq].field1_id, reqchargeeventmod->charge_event_mod[countmod].field2_id =
       reqbillcodedetails->charge_event_mod[d1.seq].field2_id, reqchargeeventmod->charge_event_mod[
       countmod].field3_id = reqbillcodedetails->charge_event_mod[d1.seq].field3_id,
       reqchargeeventmod->charge_event_mod[countmod].field4_id = reqbillcodedetails->
       charge_event_mod[d1.seq].field4_id, reqchargeeventmod->charge_event_mod[countmod].field5_id =
       reqbillcodedetails->charge_event_mod[d1.seq].field5_id, reqchargeeventmod->charge_event_mod[
       countmod].field1 = reqbillcodedetails->charge_event_mod[d1.seq].field1,
       reqchargeeventmod->charge_event_mod[countmod].field2 = reqbillcodedetails->charge_event_mod[d1
       .seq].field2, reqchargeeventmod->charge_event_mod[countmod].cm1_nbr = reqbillcodedetails->
       charge_event_mod[d1.seq].cm1_nbr, reqchargeeventmod->charge_event_mod[countmod].code1_cd =
       reqbillcodedetails->charge_event_mod[d1.seq].code1_cd
      ENDIF
     FOOT REPORT
      IF ((reqbillcodedetails->charge_event_mod_qual != countmod))
       stat = alterlist(reqchargeeventmod->charge_event_mod,countmod)
      ENDIF
     WITH nocounter
    ;end select
   ENDIF
   SET reqchargeeventmod->charge_event_mod_qual = countmod
   IF ((reqchargeeventmod->charge_event_mod_qual > 0))
    SET stat = alterlist(addchargeeventmodreq->objarray,reqchargeeventmod->charge_event_mod_qual)
    FOR (i = 1 TO reqchargeeventmod->charge_event_mod_qual)
      SET addchargeeventmodreq->objarray[i].action_type = "ADD"
      SET addchargeeventmodreq->objarray[i].charge_event_mod_id = 0.0
      SET addchargeeventmodreq->objarray[i].charge_event_id = reqchargeeventmod->charge_event_mod[i].
      charge_event_id
      SET addchargeeventmodreq->objarray[i].charge_event_mod_type_cd = reqchargeeventmod->
      charge_event_mod[i].charge_mod_type_cd
      IF (validate(reqchargeeventmod->charge_event_mod[i].field1,char(128)) != char(128))
       SET addchargeeventmodreq->objarray[i].field1 = reqchargeeventmod->charge_event_mod[i].field1
      ELSE
       SET addchargeeventmodreq->objarray[i].field1 = null
      ENDIF
      IF (validate(reqchargeeventmod->charge_event_mod[i].field2,char(128)) != char(128))
       SET addchargeeventmodreq->objarray[i].field2 = reqchargeeventmod->charge_event_mod[i].field2
      ELSE
       SET addchargeeventmodreq->objarray[i].field2 = null
      ENDIF
      SET addchargeeventmodreq->objarray[i].field6 = reqchargeeventmod->charge_event_mod[i].field6
      SET addchargeeventmodreq->objarray[i].field7 = reqchargeeventmod->charge_event_mod[i].field7
      SET addchargeeventmodreq->objarray[i].updt_cnt = 0
      SET addchargeeventmodreq->objarray[i].active_ind = 1
      SET addchargeeventmodreq->objarray[i].active_status_dt_tm = cnvtdatetime(sysdate)
      SET addchargeeventmodreq->objarray[i].beg_effective_dt_tm = cnvtdatetime(sysdate)
      SET addchargeeventmodreq->objarray[i].end_effective_dt_tm = cnvtdatetime(
       "31-DEC-2100 23:59:59.99")
      IF ((validate(reqchargeeventmod->charge_event_mod[i].code1_cd,- (0.00001)) != - (0.00001)))
       SET addchargeeventmodreq->objarray[i].code1_cd = reqchargeeventmod->charge_event_mod[i].
       code1_cd
      ELSE
       SET addchargeeventmodreq->objarray[i].code1_cd = null
      ENDIF
      SET addchargeeventmodreq->objarray[i].nomen_id = reqchargeeventmod->charge_event_mod[i].
      nomen_id
      SET addchargeeventmodreq->objarray[i].field1_id = reqchargeeventmod->charge_event_mod[i].
      field1_id
      SET addchargeeventmodreq->objarray[i].field2_id = reqchargeeventmod->charge_event_mod[i].
      field2_id
      SET addchargeeventmodreq->objarray[i].field3_id = reqchargeeventmod->charge_event_mod[i].
      field3_id
      SET addchargeeventmodreq->objarray[i].field4_id = reqchargeeventmod->charge_event_mod[i].
      field4_id
      SET addchargeeventmodreq->objarray[i].field5_id = reqchargeeventmod->charge_event_mod[i].
      field5_id
      IF ((validate(reqchargeeventmod->charge_event_mod[i].cm1_nbr,- (0.00001)) != - (0.00001)))
       SET addchargeeventmodreq->objarray[i].cm1_nbr = reqchargeeventmod->charge_event_mod[i].cm1_nbr
      ELSE
       SET addchargeeventmodreq->objarray[i].cm1_nbr = null
      ENDIF
      IF (validate(reqchargeeventmod->charge_event_mod[i].activity_dt_tm,0.0) > 0.0)
       SET addchargeeventmodreq->objarray[i].activity_dt_tm = cnvtdatetime(reqchargeeventmod->
        charge_event_mod[i].activity_dt_tm)
      ELSE
       SET addchargeeventmodreq->objarray[i].activity_dt_tm = null
      ENDIF
    ENDFOR
   ...
   IF ((modifychargereply->status_data.status != "S"))
    CALL logmessage(curprog,"afc_add_charge_mod returned with failure",log_debug)
    RETURN(false)
   ENDIF
   IF (physicianchange
    AND  NOT (updatephysicians(0)))
    CALL exitservicefailure("Failed to update the charge_event_act_prsnl table",true)
   ENDIF
   IF (((((physicianchange) OR (((ordphysqualifierind) OR (rendphysqualifierind)) )) ) OR (
   hascptmodifier
    AND cptmodifierqualifierind)) )
    CALL synchrelease(0)
    SET stat = initrec(chargemoddetails)
    SET num = 0
    SET num1 = 0
    SET locatemodifier = 0
    SET modindex = 0
    IF ( NOT (hascptmodifier))
     IF (sizeaddchargedetailsreply > 0)
      SELECT INTO "nl:"
       FROM (dummyt d1  WITH seq = value(size(addchargedetailsreply->mod_objs[1].mod_recs,5))),
        charge_mod cm
       PLAN (d1)
        JOIN (cm
        WHERE cm.charge_item_id=cnvtreal(addchargedetailsreply->mod_objs[1].mod_recs[d1.seq].
         pk_values))
       DETAIL
        locatemodifier = locateval(modindex,1,size(recmodifier14002->listmodifier,5),cm.field1_id,
         recmodifier14002->listmodifier[modindex].codevalue)
        IF (locatemodifier > 0)
         locidx = locateval(num,1,size(chargemoddetails->charge_mod,5),cm.charge_item_id,
          chargemoddetails->charge_mod[num].charge_item_id,
          cm.field1_id,chargemoddetails->charge_mod[num].field1_id,cm.field6,chargemoddetails->
          charge_mod[num].field6)
         IF (locidx=0)
          locmod = locateval(num1,1,size(modifychargerequest->charge_mod,5),cm.charge_item_id,
           modifychargerequest->charge_mod[num1].charge_item_id,
           cm.field1_id,modifychargerequest->charge_mod[num1].field1_id,cm.field6,modifychargerequest
           ->charge_mod[num1].field6)
          IF (locmod > 0)
           cmcount += 1, stat = alterlist(chargemoddetails->charge_mod,cmcount), chargemoddetails->
           charge_mod[cmcount].charge_item_id = cm.charge_item_id,
           chargemoddetails->charge_mod[cmcount].field1_id = cm.field1_id, chargemoddetails->
           charge_mod[cmcount].field2_id = modifychargerequest->charge_mod[num1].field2_id,
           chargemoddetails->charge_mod[cmcount].field6 = cm.field6
          ENDIF
         ENDIF
        ENDIF
       WITH nocounter
      ;end select
     ENDIF
     IF (size(chargemoddetails->charge_mod,5) > 0)
      UPDATE  FROM charge_mod cm,
        (dummyt d1  WITH seq = value(size(chargemoddetails->charge_mod,5)))
       SET cm.active_ind = 0.00, cm.active_status_cd = 0.00, cm.active_status_prsnl_id = reqinfo->
        updt_id,
        cm.active_status_dt_tm = cnvtdatetime(curdate,curtime), cm.updt_cnt = (cm.updt_cnt+ 1), cm
        .updt_dt_tm = cnvtdatetime(curdate,curtime),
        cm.updt_id = reqinfo->updt_id, cm.updt_applctx = reqinfo->updt_applctx, cm.updt_task =
        reqinfo->updt_task,
        cm.end_effective_dt_tm = cnvtdatetime(sysdate)
       WHERE (cm.charge_item_id=chargemoddetails->charge_mod[d1.seq].charge_item_id)
        AND (cm.field1_id=chargemoddetails->charge_mod[d1.seq].field1_id)
        AND (cm.field6=chargemoddetails->charge_mod[d1.seq].field6)
        AND (cm.field2_id != chargemoddetails->charge_mod[d1.seq].field2_id)
      ;end update
     ENDIF
    ENDIF
   ENDIF
   SET stat = initrec(chargemodrequest)
   SET stat = initrec(chargemodreply)
   CALL loadreasoncodeintochargemodifier(0)
   IF ((chargemodrequest->charge_mod_qual > 0))
    SET action_begin = 1
    SET action_end = chargemodrequest->charge_mod_qual
    EXECUTE afc_add_charge_mod  WITH replace("REQUEST",chargemodrequest), replace("REPLY",
     chargemodreply)
   ...
   IF (cnvtreal(addchargedetailsreply->mod_objs[1].mod_recs[1].pk_values) > 0.0)
    SET pftcnt = size(afcprofitrequest->charges,5)
    FOR (chargecount = 1 TO size(addchargedetailsreply->mod_objs[1].mod_recs,5))
      SET pftcnt += 1
      SET stat = alterlist(publishupdatedchargerequest->charges,chargecount)
      SET stat = alterlist(afcprofitrequest->charges,pftcnt)
      SET afcprofitrequest->charges[pftcnt].charge_item_id = cnvtreal(addchargedetailsreply->
       mod_objs[1].mod_recs[chargecount].pk_values)
      SET publishupdatedchargerequest->charges[chargecount].charge_item_id = cnvtreal(
       addchargedetailsreply->mod_objs[1].mod_recs[chargecount].pk_values)
    ENDFOR
    IF (size(syncrelease_reply->charge_items,5) > 0)
     FOR (counter1 = 1 TO size(syncrelease_reply->charge_items,5))
      SET chargepos = locateval(chargecnt,1,size(afcprofitrequest->charges,5),syncrelease_reply->
       charge_items[counter1].charge_item_id,afcprofitrequest->charges[chargecnt].charge_item_id)
      IF (chargepos=0)
       SET pftcnt += 1
       SET stat = alterlist(afcprofitrequest->charges,pftcnt)
       SET afcprofitrequest->charges[pftcnt].charge_item_id = syncrelease_reply->charge_items[
   ...
   ELSE
    ...
    RETURN(false)
   ENDIF
   CALL logmessage(curprog,"createNewDebitCharges - Exiting...",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (loadbillcodes(dummyvar=i2) =null)
   CALL logmessage(curprog,"loadBillCodes - Entering...",log_debug)
   DECLARE bcount = i4 WITH protect, noconstant(0)
   DECLARE bill_code_qual_icd = i4 WITH protect, noconstant(0)
   DECLARE bill_code_qual_mod = i4 WITH protect, noconstant(0)
   DECLARE bill_code_qual_proc = i4 WITH protect, noconstant(0)
   DECLARE existingicdcodecnt = i4 WITH protect, noconstant(0)
   DECLARE existingmodcodecnt = i4 WITH protect, noconstant(0)
   DECLARE hascpthcpcs = i2 WITH protect, noconstant(false)
   DECLARE chrgmoddetailcnt = i4 WITH protect, noconstant(0)
   DECLARE chrgmodcnt = i4 WITH protect, noconstant(size(modifychargerequest->charge_mod,5))
   DECLARE pos = i4 WITH protect, noconstant(0)
   DECLARE loc = i4 WITH protect, noconstant(0)
   DECLARE num = i4 WITH protect, noconstant(0)
   DECLARE num1 = i4 WITH protect, noconstant(0)
   DECLARE cnt = i4 WITH protect, noconstant(0)
   DECLARE prioritycnt = i4 WITH protect, noconstant(0)
   DECLARE priorityidx = i4 WITH protect, noconstant(0)
   DECLARE codecount = i4 WITH protect, noconstant(0)
   DECLARE cemcount = i4 WITH protect, noconstant(0)
   DECLARE hcpcsidx = i4 WITH protect, noconstant(0)
   DECLARE cptidx = i4 WITH protect, noconstant(0)
   DECLARE locatecid = i4 WITH protect, noconstant(0)
   DECLARE locatebillcode = i4 WITH protect, noconstant(0)
   DECLARE cidcount = i4 WITH protect, noconstant(0)
   DECLARE nondupmodpriority = i4 WITH protect, noconstant(0)
   DECLARE locatemodinrequest = i4 WITH protect, noconstant(0)
   DECLARE ignoredmodifiers = i4 WITH protect, noconstant(0)
   SET locatemodifier = 0
   SET modindex = 0
   FOR (i = 1 TO size(request->billcodesmodel,5))
     SET bcount += 1
     SET locatemodifier = locateval(modindex,1,size(recmodifier14002->listmodifier,5),request->
      billcodesmodel[bcount].field1id,recmodifier14002->listmodifier[modindex].codevalue)
     IF (trim(uar_get_code_meaning(request->billcodesmodel[bcount].field1id))=icd_dx_code)
      SET bill_code_qual_icd += 1
      SET stat = alterlist(reqbillcodesicd->bill_code_details,bill_code_qual_icd)
      SET reqbillcodesicd->bill_code_details[bill_code_qual_icd].field1id = request->billcodesmodel[
      bcount].field1id
      SET reqbillcodesicd->bill_code_details[bill_code_qual_icd].field2id = request->billcodesmodel[
      bcount].field2id
      SET reqbillcodesicd->bill_code_details[bill_code_qual_icd].field3id = request->billcodesmodel[
      bcount].field3id
      SET reqbillcodesicd->bill_code_details[bill_code_qual_icd].field4id = request->billcodesmodel[
      bcount].field4id
      SET reqbillcodesicd->bill_code_details[bill_code_qual_icd].field5id = request->billcodesmodel[
      bcount].field5id
      SET reqbillcodesicd->bill_code_details[bill_code_qual_icd].field6 = request->billcodesmodel[
      bcount].field6
      SET reqbillcodesicd->bill_code_details[bill_code_qual_icd].field7 = request->billcodesmodel[
      bcount].field7
      SET reqbillcodesicd->bill_code_details[bill_code_qual_icd].nomenid = request->billcodesmodel[
      bcount].nomenid
      SET reqbillcodesicd->bill_code_details[bill_code_qual_icd].code1cd = request->billcodesmodel[
      bcount].code1cd
      SET reqbillcodesicd->bill_code_details[bill_code_qual_icd].cm1nbr = request->billcodesmodel[
      bcount].cm1nbr
     ENDIF
     IF (locatemodifier > 0)
      SET bill_code_qual_mod += 1
      SET stat = alterlist(reqbillcodesmod->bill_code_details,bill_code_qual_mod)
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].field1id = request->billcodesmodel[
      bcount].field1id
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].field2id = request->billcodesmodel[
      bcount].field2id
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].field3id = request->billcodesmodel[
      bcount].field3id
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].field4id = request->billcodesmodel[
      bcount].field4id
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].field5id = request->billcodesmodel[
      bcount].field5id
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].field6 = request->billcodesmodel[
      bcount].field6
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].field7 = request->billcodesmodel[
      bcount].field7
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].nomenid = request->billcodesmodel[
      bcount].nomenid
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].code1cd = request->billcodesmodel[
      bcount].code1cd
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].cm1nbr = request->billcodesmodel[
      bcount].cm1nbr
      SET reqbillcodesmod->bill_code_details[bill_code_qual_mod].chargemodsourcecd =
      cs4518006_manually_add_cd
     ENDIF
     IF (trim(uar_get_code_meaning(request->billcodesmodel[bcount].field1id))=proc_code)
      SET bill_code_qual_proc += 1
      SET stat = alterlist(reqbillcodesproc->bill_code_details,bill_code_qual_proc)
      SET reqbillcodesproc->bill_code_details[bill_code_qual_proc].field1id = request->
      billcodesmodel[bcount].field1id
      SET reqbillcodesproc->bill_code_details[bill_code_qual_proc].field2id = request->
      billcodesmodel[bcount].field2id
      SET reqbillcodesproc->bill_code_details[bill_code_qual_proc].field3id = request->
      billcodesmodel[bcount].field3id
      SET reqbillcodesproc->bill_code_details[bill_code_qual_proc].field4id = request->
      billcodesmodel[bcount].field4id
      SET reqbillcodesproc->bill_code_details[bill_code_qual_proc].field5id = request->
      billcodesmodel[bcount].field5id
      SET reqbillcodesproc->bill_code_details[bill_code_qual_proc].field6 = request->billcodesmodel[
      bcount].field6
      SET reqbillcodesproc->bill_code_details[bill_code_qual_proc].field7 = request->billcodesmodel[
      bcount].field7
      SET reqbillcodesproc->bill_code_details[bill_code_qual_proc].nomenid = request->billcodesmodel[
      bcount].nomenid
      SET reqbillcodesproc->bill_code_details[bill_code_qual_proc].code1cd = request->billcodesmodel[
      bcount].code1cd
      SET reqbillcodesproc->bill_code_details[bill_code_qual_proc].cm1nbr = request->billcodesmodel[
      bcount].cm1nbr
     ENDIF
   ENDFOR
   IF (bill_code_qual_icd > 0)
    SET reqbillcodesicd->bill_code_qual = bill_code_qual_icd
   ENDIF
   IF (bill_code_qual_mod > 0)
    SET reqbillcodesmod->bill_code_qual = bill_code_qual_mod
   ENDIF
   IF (bill_code_qual_proc > 0)
    SET reqbillcodesproc->bill_code_qual = bill_code_qual_proc
   ENDIF
   SET locatemodifier = 0
   SET modindex = 0
   IF (sizeaddchargedetailsreply > 0)
    SELECT INTO "nl:"
     ...
     ORDER BY c.charge_item_id, cem.field2_id
     HEAD REPORT
      stat = alterlist(chargeeventmoddetails->chargeid,size(addchargedetailsreply->mod_objs[1].
        mod_recs,5))
     HEAD c.charge_item_id
      cidcount += 1, chargeeventmoddetails->chargeid[cidcount].charge_item_id = c.charge_item_id,
      locatecid = locateval(num,1,size(addchargedetailsrequest->objarray,5),c.charge_item_id,
       addchargedetailsrequest->objarray[num].charge_item_id),
      cemcount = 0
     DETAIL
      cemcount += 1
      IF (mod(cemcount,10)=1)
       stat = alterlist(chargeeventmoddetails->chargeid[cidcount].charge_event_mod,(cemcount+ 9))
      ENDIF
      locatemodifier = locateval(modindex,1,size(recmodifier14002->listmodifier,5),cem.field1_id,
       recmodifier14002->listmodifier[modindex].codevalue)
      IF (locatemodifier > 0)
       IF (locatecid > 0)
        locatebillcode = locateval(num1,1,size(addchargedetailsrequest->objarray[locatecid].
          charge_mod,5),cem.field1_id,addchargedetailsrequest->objarray[locatecid].charge_mod[num1].
         field1_id,
         cem.field6,addchargedetailsrequest->objarray[locatecid].charge_mod[num1].field6)
        IF (locatebillcode > 0)
         chargeeventmoddetails->chargeid[cidcount].charge_event_mod_modfr_count += 1
        ENDIF
        chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field1_id = cem
        .field1_id, chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field2_id =
        cem.field2_id, chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field3_id
         = cem.field3_id,
        chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field4_id = cem
        .field4_id, chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field5_id =
        cem.field5_id, chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field6 =
        cem.field6,
        chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field7 = cem.field7,
        chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].cm1nbr = cem.cm1_nbr,
        chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].code1cd = cem.code1_cd,
        chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].nomen_id = cem.nomen_id
       ENDIF
      ELSE
       IF (trim(uar_get_code_meaning(cem.field1_id))=icd_dx_code)
        chargeeventmoddetails->chargeid[cidcount].charge_event_mod_icd_count += 1
       ENDIF
       chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field1_id = cem.field1_id,
       chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field2_id = cem.field2_id,
       chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field3_id = cem.field3_id,
       chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field4_id = cem.field4_id,
       chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field5_id = cem.field5_id,
       chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field6 = cem.field6,
       chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].field7 = cem.field7,
       chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].cm1nbr = cem.cm1_nbr,
       chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].code1cd = cem.code1_cd,
       chargeeventmoddetails->chargeid[cidcount].charge_event_mod[cemcount].nomen_id = cem.nomen_id
     ...
     FOOT  c.charge_item_id
      stat = alterlist(chargeeventmoddetails->chargeid[cidcount].charge_event_mod,cemcount)
     FOOT REPORT
      stat = alterlist(chargeeventmoddetails->chargeid,cidcount)
     WITH nocounter
    ;end select
   ENDIF
   IF (sizeaddchargedetailsreply > 0)
    SELECT INTO "nl:"
     ...
     PLAN (d1)
      ...
      WHERE c.charge_item_id=cnvtreal(addchargedetailsreply->mod_objs[1].mod_recs[d1.seq].pk_values))
      JOIN (c1
      WHERE c1.charge_item_id=c.parent_charge_item_id)
      JOIN (cm
      WHERE cm.charge_item_id=c1.charge_item_id
       AND cm.charge_mod_type_cd != cs13019_flex_cd
       AND cm.active_ind=1)
     ORDER BY c.charge_item_id, cm.field2_id
     HEAD c.charge_item_id
      hascpthcpcs = false, existingicdcodecnt = 0, existingmodcodecnt = 0,
      chrgmoddetailcnt = 0, cnt = 0, priorityidx = 0,
      prioritycnt = 0, pos = 0, loc = 0,
      num = 0, locatemodifier = 0, modindex = 0,
      ignoredmodifiers = 0, stat = initrec(reqbillcodesmodnodup), stat = initrec(chargemoddetails)
     DETAIL
      IF (((trim(uar_get_code_meaning(cm.field1_id))="CPT4") OR (trim(uar_get_code_meaning(cm
        .field1_id))="HCPCS")) )
       hascpthcpcs = true
      ENDIF
      locatemodifier = locateval(modindex,1,size(recmodifier14002->listmodifier,5),cm.field1_id,
       recmodifier14002->listmodifier[modindex].codevalue)
      IF (trim(uar_get_code_meaning(cm.field1_id))=icd_dx_code)
       existingicdcodecnt += 1
       IF (request->addatbegicdind)
        CALL copyexistingchargemods(reqbillcodesicd->bill_code_qual,icd_dx_code)
       ELSE
        CALL copyexistingchargemods(0,icd_dx_code)
       ENDIF
      ELSEIF (locatemodifier > 0)
       pos = locateval(num,1,reqbillcodesmod->bill_code_qual,cm.field6,reqbillcodesmod->
        bill_code_details[num].field6)
       IF ((request->addatbegmodind=false))
        IF (pos > 0)
         ignoredmodifiers += 1
        ELSE
         chrgmoddetailcnt += 1, stat = alterlist(chargemoddetails->charge_mod,chrgmoddetailcnt),
         chargemoddetails->charge_mod[chrgmoddetailcnt].charge_item_id = c.charge_item_id,
         chargemoddetails->charge_mod[chrgmoddetailcnt].charge_event_mod_id = 0.0, chargemoddetails->
         charge_mod[chrgmoddetailcnt].charge_mod_type_cd = cs13019_bill_cd, chargemoddetails->
         charge_mod[chrgmoddetailcnt].cm1_nbr = cm.cm1_nbr,
         chargemoddetails->charge_mod[chrgmoddetailcnt].code1_cd = cm.code1_cd, chargemoddetails->
         charge_mod[chrgmoddetailcnt].field1_id = cm.field1_id, chargemoddetails->charge_mod[
         chrgmoddetailcnt].field2_id = (cm.field2_id - ignoredmodifiers),
         chargemoddetails->charge_mod[chrgmoddetailcnt].field3_id = cm.field3_id, chargemoddetails->
         charge_mod[chrgmoddetailcnt].field4_id = cm.field4_id, chargemoddetails->charge_mod[
         chrgmoddetailcnt].field5_id = cm.field5_id,
         chargemoddetails->charge_mod[chrgmoddetailcnt].field6 = cm.field6, chargemoddetails->
         charge_mod[chrgmoddetailcnt].field7 = cm.field7, chargemoddetails->charge_mod[
         chrgmoddetailcnt].nomen_id = cm.nomen_id,
         chargemoddetails->charge_mod[chrgmoddetailcnt].action_type = "ADD", chargemoddetails->
         charge_mod[chrgmoddetailcnt].charge_mod_source_cd = cm.charge_mod_source_cd
        ENDIF
       ELSE
        IF (pos > 0)
         priorityidx += 1
        ELSE
         ...
         chargemoddetails->charge_mod[chrgmoddetailcnt].code1_cd = cm.code1_cd, chargemoddetails->
         charge_mod[chrgmoddetailcnt].field1_id = cm.field1_id, chargemoddetails->charge_mod[
         chrgmoddetailcnt].field2_id = (cm.field2_id - priorityidx),
         chargemoddetails->charge_mod[chrgmoddetailcnt].field3_id = cm.field3_id, chargemoddetails->
         charge_mod[chrgmoddetailcnt].field4_id = cm.field4_id, chargemoddetails->charge_mod[
         chrgmoddetailcnt].field5_id = cm.field5_id,
         chargemoddetails->charge_mod[chrgmoddetailcnt].field6 = cm.field6, chargemoddetails->
        ...
        ENDIF
       ENDIF
      ELSEIF (trim(uar_get_code_meaning(cm.field1_id))=proc_code)
       IF (size(reqbillcodesproc->bill_code_details,5)=0)
        CALL copyexistingchargemods(reqbillcodesproc->bill_code_qual,proc_code)
       ENDIF
      ELSE
       chrgmodcnt = size(modifychargerequest->charge_mod,5), chrgmodcnt += 1, stat = alterlist(
        modifychargerequest->charge_mod,chrgmodcnt),
       modifychargerequest->charge_mod[chrgmodcnt].charge_item_id = c.charge_item_id,
       modifychargerequest->charge_mod[chrgmodcnt].charge_event_mod_id = 0.0, modifychargerequest->
       charge_mod[chrgmodcnt].charge_mod_type_cd = cs13019_bill_cd,
       ...
       modifychargerequest->charge_mod[chrgmodcnt].field2_id = cm.field2_id, modifychargerequest->
       charge_mod[chrgmodcnt].field3_id = cm.field3_id, modifychargerequest->charge_mod[chrgmodcnt].
       field4_id = cm.field4_id,
       modifychargerequest->charge_mod[chrgmodcnt].field5_id = cm.field5_id, modifychargerequest->
       charge_mod[chrgmodcnt].field6 = cm.field6, modifychargerequest->charge_mod[chrgmodcnt].field7
        = cm.field7,
       modifychargerequest->charge_mod[chrgmodcnt].nomen_id = cm.nomen_id, modifychargerequest->
       charge_mod[chrgmodcnt].action_type = "ADD"
      ENDIF
     FOOT  c.charge_item_id
      chrgmodcnt = size(modifychargerequest->charge_mod,5)
      IF (size(chargemoddetails->charge_mod,5) > 0)
       existingmodcodecnt = size(chargemoddetails->charge_mod,5)
       FOR (i = 1 TO size(chargemoddetails->charge_mod,5))
         chrgmodcnt += 1, stat = alterlist(modifychargerequest->charge_mod,chrgmodcnt),
         modifychargerequest->charge_mod[chrgmodcnt].charge_item_id = chargemoddetails->charge_mod[i]
         .charge_item_id,
         modifychargerequest->charge_mod[chrgmodcnt].charge_event_mod_id = chargemoddetails->
         charge_mod[i].charge_event_mod_id, modifychargerequest->charge_mod[chrgmodcnt].
         charge_mod_type_cd = chargemoddetails->charge_mod[i].charge_mod_type_cd, modifychargerequest
         ->charge_mod[chrgmodcnt].cm1_nbr = chargemoddetails->charge_mod[i].cm1_nbr,
         modifychargerequest->charge_mod[chrgmodcnt].code1_cd = chargemoddetails->charge_mod[i].
         code1_cd, modifychargerequest->charge_mod[chrgmodcnt].field1_id = chargemoddetails->
         charge_mod[i].field1_id
         IF (hascpthcpcs
          AND request->addatbegmodind)
          modifychargerequest->charge_mod[chrgmodcnt].field2_id = (chargemoddetails->charge_mod[i].
          field2_id+ reqbillcodesmod->bill_code_qual)
         ELSE
          modifychargerequest->charge_mod[chrgmodcnt].field2_id = chargemoddetails->charge_mod[i].
          field2_id
         ENDIF
         modifychargerequest->charge_mod[chrgmodcnt].field3_id = chargemoddetails->charge_mod[i].
         field3_id, modifychargerequest->charge_mod[chrgmodcnt].field4_id = chargemoddetails->
         charge_mod[i].field4_id, modifychargerequest->charge_mod[chrgmodcnt].field5_id =
         chargemoddetails->charge_mod[i].field5_id,
         modifychargerequest->charge_mod[chrgmodcnt].field6 = chargemoddetails->charge_mod[i].field6,
         modifychargerequest->charge_mod[chrgmodcnt].field7 = chargemoddetails->charge_mod[i].field7,
         modifychargerequest->charge_mod[chrgmodcnt].nomen_id = chargemoddetails->charge_mod[i].
         nomen_id,
         modifychargerequest->charge_mod[chrgmodcnt].action_type = chargemoddetails->charge_mod[i].
         action_type, modifychargerequest->charge_mod[chrgmodcnt].charge_mod_source_cd =
         chargemoddetails->charge_mod[i].charge_mod_source_cd
       ENDFOR
      ENDIF
      IF ((reqbillcodesicd->bill_code_qual > 0))
       IF ((request->addatbegicdind=true))
        CALL copynewchargemods(c.charge_item_id,reqbillcodesicd,0,0)
       ELSE
        CALL copynewchargemods(c.charge_item_id,reqbillcodesicd,existingicdcodecnt,0)
       ENDIF
      ENDIF
      IF ((reqbillcodesmod->bill_code_qual > 0))
       IF (hascpthcpcs)
        FOR (i = 1 TO size(reqbillcodesmod->bill_code_details,5))
         loc = locateval(num,1,size(chargemoddetails->charge_mod,5),reqbillcodesmod->
          bill_code_details[i].field6,chargemoddetails->charge_mod[num].field6),
         IF (loc > 0)
          prioritycnt += 1
         ELSE
          cnt += 1, stat = alterlist(reqbillcodesmodnodup->bill_code_details,cnt),
          reqbillcodesmodnodup->bill_code_details[cnt].field1id = reqbillcodesmod->bill_code_details[
          i].field1id,
          reqbillcodesmodnodup->bill_code_details[cnt].field2id = reqbillcodesmod->bill_code_details[
          i].field2id, reqbillcodesmodnodup->bill_code_details[cnt].field3id = reqbillcodesmod->
          bill_code_details[i].field3id, reqbillcodesmodnodup->bill_code_details[cnt].field4id =
          reqbillcodesmod->bill_code_details[i].field4id,
          reqbillcodesmodnodup->bill_code_details[cnt].field5id = reqbillcodesmod->bill_code_details[
          i].field5id, reqbillcodesmodnodup->bill_code_details[cnt].field6 = reqbillcodesmod->
          bill_code_details[i].field6, reqbillcodesmodnodup->bill_code_details[cnt].field7 =
          reqbillcodesmod->bill_code_details[i].field7,
          reqbillcodesmodnodup->bill_code_details[cnt].nomenid = reqbillcodesmod->bill_code_details[i
          ].nomenid, reqbillcodesmodnodup->bill_code_details[cnt].code1cd = reqbillcodesmod->
          bill_code_details[i].code1cd, reqbillcodesmodnodup->bill_code_details[cnt].cm1nbr =
          reqbillcodesmod->bill_code_details[i].cm1nbr,
          reqbillcodesmodnodup->bill_code_details[cnt].chargemodsourcecd = reqbillcodesmod->
          bill_code_details[i].chargemodsourcecd
         ENDIF
        ENDFOR
        reqbillcodesmodnodup->bill_code_qual = cnt
        IF (request->addatbegmodind)
         CALL copynewchargemods(c.charge_item_id,reqbillcodesmodnodup,0,0)
        ELSE
         CALL copynewchargemods(c.charge_item_id,reqbillcodesmodnodup,existingmodcodecnt,
         ignoredmodifiers)
        ENDIF
       ENDIF
      ENDIF
      IF (size(reqbillcodesproc->bill_code_details,5) > 0)
       CALL copynewchargemods(c.charge_item_id,reqbillcodesproc,0,0)
      ENDIF
     FOOT REPORT
   ...
   ENDIF
   SET num = 0
   IF (size(chargeeventmoddetails->chargeid,5) > 0)
    FOR (i = 1 TO size(chargeeventmoddetails->chargeid,5))
      SET ignoredmodifiers = 0
      SET nondupmodpriority = bill_code_qual_mod
      FOR (x = 1 TO size(chargeeventmoddetails->chargeid[i].charge_event_mod,5))
        SET num = 0
        SET loc = 0
        SET loc = locateval(num,1,size(reqbillcodesmod->bill_code_details,5),trim(
          chargeeventmoddetails->chargeid[i].charge_event_mod[x].field6),trim(reqbillcodesmod->
          bill_code_details[num].field6))
        IF (loc > 0)
         SET ignoredmodifiers += 1
        ELSE
         SET codecount += 1
         SET stat = alterlist(reqbillcodedetails->charge_event_mod,codecount)
         SET reqbillcodedetails->charge_event_mod[codecount].charge_item_id = chargeeventmoddetails->
         chargeid[i].charge_item_id
         SET reqbillcodedetails->charge_event_mod[codecount].charge_event_mod_id = 0.00
         SET reqbillcodedetails->charge_event_mod[codecount].charge_mod_type_cd = cs13019_bill_cd
         SET reqbillcodedetails->charge_event_mod[codecount].cm1_nbr = chargeeventmoddetails->
         chargeid[i].charge_event_mod[x].cm1nbr
         SET reqbillcodedetails->charge_event_mod[codecount].code1_cd = chargeeventmoddetails->
         chargeid[i].charge_event_mod[x].code1cd
         SET reqbillcodedetails->charge_event_mod[codecount].field1_id = chargeeventmoddetails->
         chargeid[i].charge_event_mod[x].field1_id
         SET num = 0
         SET num1 = 0
         SET locatecid = 0
         SET locatebillcode = 0
         SET locatecid = locateval(num,1,size(addchargedetailsrequest->objarray,5),
          chargeeventmoddetails->chargeid[i].charge_item_id,addchargedetailsrequest->objarray[num].
          charge_item_id)
         IF (locatecid > 0)
          SET locatebillcode = locateval(num1,1,size(addchargedetailsrequest->objarray[locatecid].
            charge_mod,5),chargeeventmoddetails->chargeid[i].charge_event_mod[x].field1_id,
           addchargedetailsrequest->objarray[locatecid].charge_mod[num1].field1_id,
           chargeeventmoddetails->chargeid[i].charge_event_mod[x].field6,addchargedetailsrequest->
           objarray[locatecid].charge_mod[num1].field6)
          IF (locatebillcode=0)
           SET reqbillcodedetails->charge_event_mod[codecount].field2_id = chargeeventmoddetails->
           chargeid[i].charge_event_mod[x].field2_id
          ELSE
           SET locatemodifier = 0
           SET modindex = 0
           SET locatemodinrequest = 0
           SET locatemodifier = locateval(modindex,1,size(recmodifier14002->listmodifier,5),
            chargeeventmoddetails->chargeid[i].charge_event_mod[x].field1_id,recmodifier14002->
            listmodifier[modindex].codevalue)
           SET modindex = 0
           SET locatemodinrequest = locateval(modindex,1,size(request->billcodesmodel,5),
            chargeeventmoddetails->chargeid[i].charge_event_mod[x].field6,request->billcodesmodel[
            modindex].field6)
           IF (trim(uar_get_code_meaning(chargeeventmoddetails->chargeid[i].charge_event_mod[x].
             field1_id))=icd_dx_code
            AND (request->addatbegicdind=true)
            AND (reqbillcodesicd->bill_code_qual > 0))
            SET reqbillcodedetails->charge_event_mod[codecount].field2_id = (chargeeventmoddetails->
            chargeid[i].charge_event_mod[x].field2_id+ reqbillcodesicd->bill_code_qual)
           ELSEIF (locatemodifier > 0
            AND locatemodinrequest > 0
            AND (request->addatbegmodind=true)
            AND (reqbillcodesmod->bill_code_qual > 0))
            SET reqbillcodedetails->charge_event_mod[codecount].field2_id = request->billcodesmodel[
            locatemodinrequest].field2id
           ELSEIF (locatemodifier > 0
            AND (request->addatbegmodind=true)
            AND (reqbillcodesmod->bill_code_qual > 0))
            SET nondupmodpriority += 1
            SET reqbillcodedetails->charge_event_mod[codecount].field2_id = nondupmodpriority
           ELSE
            SET reqbillcodedetails->charge_event_mod[codecount].field2_id = (chargeeventmoddetails->
            chargeid[i].charge_event_mod[x].field2_id - ignoredmodifiers)
           ENDIF
          ENDIF
         ENDIF
         SET reqbillcodedetails->charge_event_mod[codecount].field3_id = chargeeventmoddetails->
         chargeid[i].charge_event_mod[x].field3_id
         SET reqbillcodedetails->charge_event_mod[codecount].field4_id = chargeeventmoddetails->
         chargeid[i].charge_event_mod[x].field4_id
         SET reqbillcodedetails->charge_event_mod[codecount].field5_id = chargeeventmoddetails->
         chargeid[i].charge_event_mod[x].field5_id
         SET reqbillcodedetails->charge_event_mod[codecount].field6 = chargeeventmoddetails->
         chargeid[i].charge_event_mod[x].field6
         SET reqbillcodedetails->charge_event_mod[codecount].field7 = chargeeventmoddetails->
         chargeid[i].charge_event_mod[x].field7
         SET reqbillcodedetails->charge_event_mod[codecount].nomen_id = chargeeventmoddetails->
         chargeid[i].charge_event_mod[x].nomen_id
         SET reqbillcodedetails->charge_event_mod[codecount].action_type = "ADD"
        ENDIF
      ENDFOR
    ENDFOR
   ENDIF
   SET reqbillcodedetails->charge_event_mod_qual = codecount
   CALL logmessage(curprog,"loadBillCodes - Exiting...",log_debug)
 END ;Subroutine
 SUBROUTINE (copynewchargemods(chargeitemid=f8,reqbillcodes=vc(ref),noofchargemod=i4,ignoredcnt=i4) =
  null)
   CALL logmessage(curprog,"copyNewChargeMods - Entering...",log_debug)
   DECLARE locidx = i4 WITH protect, noconstant(0)
   DECLARE loccid = i4 WITH protect, noconstant(0)
   SET num = 0
   SET locatemodifier = 0
   SET modindex = 0
   FOR (i = 1 TO size(reqbillcodes->bill_code_details,5))
     SET locatemodifier = locateval(modindex,1,size(recmodifier14002->listmodifier,5),reqbillcodes->
      bill_code_details[i].field1id,recmodifier14002->listmodifier[modindex].codevalue)
     SET chrgmodcnt = size(modifychargerequest->charge_mod,5)
     SET chrgmodcnt += 1
     SET stat = alterlist(modifychargerequest->charge_mod,chrgmodcnt)
     SET modifychargerequest->charge_mod[chrgmodcnt].charge_item_id = chargeitemid
     SET modifychargerequest->charge_mod[chrgmodcnt].charge_event_mod_id = 0.0
     SET modifychargerequest->charge_mod[chrgmodcnt].charge_mod_type_cd = cs13019_bill_cd
     SET modifychargerequest->charge_mod[chrgmodcnt].cm1_nbr = reqbillcodes->bill_code_details[i].
     cm1nbr
     SET modifychargerequest->charge_mod[chrgmodcnt].code1_cd = reqbillcodes->bill_code_details[i].
     code1cd
     SET modifychargerequest->charge_mod[chrgmodcnt].field1_id = reqbillcodes->bill_code_details[i].
     field1id
     SET modifychargerequest->charge_mod[chrgmodcnt].field2_id = (reqbillcodes->bill_code_details[i].
     field2id+ noofchargemod)
     SET modifychargerequest->charge_mod[chrgmodcnt].field3_id = reqbillcodes->bill_code_details[i].
     field3id
     SET modifychargerequest->charge_mod[chrgmodcnt].field4_id = reqbillcodes->bill_code_details[i].
     field4id
     SET modifychargerequest->charge_mod[chrgmodcnt].field5_id = reqbillcodes->bill_code_details[i].
     field5id
     SET modifychargerequest->charge_mod[chrgmodcnt].field6 = reqbillcodes->bill_code_details[i].
     field6
     SET modifychargerequest->charge_mod[chrgmodcnt].field7 = reqbillcodes->bill_code_details[i].
     field7
     SET modifychargerequest->charge_mod[chrgmodcnt].nomen_id = reqbillcodes->bill_code_details[i].
     nomenid
     SET modifychargerequest->charge_mod[chrgmodcnt].action_type = "ADD"
     SET modifychargerequest->charge_mod[chrgmodcnt].charge_mod_source_cd = validate(reqbillcodes->
      bill_code_details[i].chargemodsourcecd,0.0)
     IF (size(chargeeventmoddetails->chargeid,5) > 0)
      SET loccid = locateval(num,1,size(chargeeventmoddetails->chargeid,5),chargeitemid,
       chargeeventmoddetails->chargeid[num].charge_item_id)
     ENDIF
     IF (trim(uar_get_code_meaning(reqbillcodes->bill_code_details[i].field1id))=icd_dx_code)
      SET codecount += 1
      SET num = 0
      SET num1 = 0
      SET stat = alterlist(reqbillcodedetails->charge_event_mod,codecount)
      SET reqbillcodedetails->charge_event_mod[codecount].charge_item_id = chargeitemid
      SET reqbillcodedetails->charge_event_mod[codecount].charge_event_mod_id = 0.0
      SET reqbillcodedetails->charge_event_mod[codecount].charge_mod_type_cd = cs13019_bill_cd
      SET reqbillcodedetails->charge_event_mod[codecount].cm1_nbr = reqbillcodes->bill_code_details[i
      ].cm1nbr
      SET reqbillcodedetails->charge_event_mod[codecount].code1_cd = reqbillcodes->bill_code_details[
      i].code1cd
      SET reqbillcodedetails->charge_event_mod[codecount].field1_id = reqbillcodes->
      bill_code_details[i].field1id
      IF (request->addatbegicdind)
       SET reqbillcodedetails->charge_event_mod[codecount].field2_id = reqbillcodes->
       bill_code_details[i].field2id
      ELSE
       IF (loccid > 0)
        SET reqbillcodedetails->charge_event_mod[codecount].field2_id = (reqbillcodes->
        bill_code_details[i].field2id+ chargeeventmoddetails->chargeid[loccid].
        charge_event_mod_icd_count)
       ELSE
        SET reqbillcodedetails->charge_event_mod[codecount].field2_id = reqbillcodes->
        bill_code_details[i].field2id
       ENDIF
      ENDIF
      SET reqbillcodedetails->charge_event_mod[codecount].field3_id = reqbillcodes->
      bill_code_details[i].field3id
      SET reqbillcodedetails->charge_event_mod[codecount].field4_id = reqbillcodes->
      bill_code_details[i].field4id
      SET reqbillcodedetails->charge_event_mod[codecount].field5_id = reqbillcodes->
      bill_code_details[i].field5id
      SET reqbillcodedetails->charge_event_mod[codecount].field6 = reqbillcodes->bill_code_details[i]
      .field6
      SET reqbillcodedetails->charge_event_mod[codecount].field7 = reqbillcodes->bill_code_details[i]
      .field7
      SET reqbillcodedetails->charge_event_mod[codecount].nomen_id = reqbillcodes->bill_code_details[
      i].nomenid
      SET reqbillcodedetails->charge_event_mod[codecount].action_type = "ADD"
     ELSEIF (locatemodifier > 0)
      SET num = 0
      ...
      ELSE
       IF (loccid > 0)
        SET reqbillcodedetails->charge_event_mod[codecount].field2_id = ((reqbillcodes->
        bill_code_details[i].field2id+ chargeeventmoddetails->chargeid[loccid].
        charge_event_mod_modfr_count) - ignoredcnt)
       ELSE
        SET reqbillcodedetails->charge_event_mod[codecount].field2_id = reqbillcodes->
        bill_code_details[i].field2id
       ENDIF
     ...
     ENDIF
   ENDFOR
   CALL logmessage(curprog,"copyNewChargeMods - Exiting...",log_debug)
 END ;Subroutine
 SUBROUTINE (copyexistingchargemods(noofchargemod=i4,codetype=vc) =null)
   CALL logmessage(curprog,"copyExistingChargeMods - Enteringing...",log_debug)
   SET chrgmodcnt = size(modifychargerequest->charge_mod,5)
   SET chrgmodcnt += 1
   SET stat = alterlist(modifychargerequest->charge_mod,chrgmodcnt)
   IF (trim(uar_get_code_meaning(cm.field1_id))=codetype)
    SET modifychargerequest->charge_mod[chrgmodcnt].charge_item_id = c.charge_item_id
    SET modifychargerequest->charge_mod[chrgmodcnt].charge_event_mod_id = 0.0
    SET modifychargerequest->charge_mod[chrgmodcnt].charge_mod_type_cd = cs13019_bill_cd
    SET modifychargerequest->charge_mod[chrgmodcnt].cm1_nbr = cm.cm1_nbr
    SET modifychargerequest->charge_mod[chrgmodcnt].code1_cd = cm.code1_cd
    SET modifychargerequest->charge_mod[chrgmodcnt].field1_id = cm.field1_id
    SET modifychargerequest->charge_mod[chrgmodcnt].field2_id = (cm.field2_id+ noofchargemod)
    SET modifychargerequest->charge_mod[chrgmodcnt].field3_id = cm.field3_id
    SET modifychargerequest->charge_mod[chrgmodcnt].field4_id = cm.field4_id
    SET modifychargerequest->charge_mod[chrgmodcnt].field5_id = cm.field5_id
    SET modifychargerequest->charge_mod[chrgmodcnt].field6 = cm.field6
    SET modifychargerequest->charge_mod[chrgmodcnt].field7 = cm.field7
    SET modifychargerequest->charge_mod[chrgmodcnt].nomen_id = cm.nomen_id
    SET modifychargerequest->charge_mod[chrgmodcnt].action_type = "ADD"
    SET modifychargerequest->charge_mod[chrgmodcnt].charge_mod_source_cd = cm.charge_mod_source_cd
   ENDIF
   CALL logmessage(curprog,"copyExistingChargeMods - Exiting...",log_debug)
 END ;Subroutine
 SUBROUTINE (synchrelease(dummyvar=i2) =null)
   CALL logmessage(curprog,"synchRelease - Entering...",log_debug)
   DECLARE nreprocessrequest = i4 WITH protect, noconstant(0)
   ...
   DECLARE litemcount = i4 WITH protect, noconstant(0)
   DECLARE lloopcount = i4 WITH protect, noconstant(0)
   DECLARE chargepos = i4 WITH protect, noconstant(0)
   DECLARE num = i4 WITH protect, noconstant(0)
   DECLARE idx = i4 WITH protect, noconstant(0)
   DECLARE appid = i4 WITH protect, noconstant(0)
   DECLARE taskid = i4 WITH protect, noconstant(0)
   DECLARE reqid = i4 WITH protect, noconstant(0)
   DECLARE happ = i4 WITH protect, noconstant(0)
   DECLARE htask = i4 WITH protect, noconstant(0)
   DECLARE hreq = i4 WITH protect, noconstant(0)
   DECLARE hrequest = i4 WITH protect, noconstant(0)
   DECLARE hlist = i4 WITH protect, noconstant(0)
   DECLARE hlist1 = i4 WITH protect, noconstant(0)
   ...
   SET hcharge = 0
   SET crmstatus = 0
   SET stat = alterlist(reprocrequest->process_event,size(chargeeventidlist->charge_event,5))
   FOR (nreprocessrequest = 1 TO size(chargeeventidlist->charge_event,5))
     SET reprocrequest->process_event[nreprocessrequest].charge_event_id = chargeeventidlist->
     charge_event[nreprocessrequest].charge_event_id
     SET stat = alterlist(reprocrequest->process_event[nreprocessrequest].charge_item,size(
       chargeeventidlist->charge_event[nreprocessrequest].charge_item,5))
     FOR (chargeitemloop = 1 TO size(chargeeventidlist->charge_event[nreprocessrequest].charge_item,5
      ))
       SET reprocrequest->process_event[nreprocessrequest].charge_item[chargeitemloop].charge_item_id
        = chargeeventidlist->charge_event[nreprocessrequest].charge_item[chargeitemloop].
       charge_item_id
     ENDFOR
   ENDFOR
   SET reprocrequest->charge_event_qual = size(reprocrequest->process_event,5)
   IF (size(reprocrequest->process_event,5) > 0)
     ...
     IF (crmstatus=0)
      ...
      IF (crmstatus=0)
       ...
       ELSE
        ...
        ELSE
         SET stat = uar_srvsetshort(hrequest,"charge_event_qual",reprocrequest->charge_event_qual)
         SET reprocrequest->process_type_cd = cs13029_released_cd
         SET stat = uar_srvsetdouble(hrequest,"process_type_cd",reprocrequest->process_type_cd)
         FOR (nreprocessloop = 1 TO size(reprocrequest->process_event,5))
           SET hlist1 = uar_srvadditem(hrequest,"process_event")
           SET stat = uar_srvsetdouble(hlist1,"charge_event_id",reprocrequest->process_event[
            nreprocessloop].charge_event_id)
           SET reprocrequest->process_event[nreprocessloop].charge_item_qual = size(reprocrequest->
            process_event[nreprocessloop].charge_item,5)
           SET stat = uar_srvsetshort(hlist1,"charge_item_qual",reprocrequest->process_event[
            nreprocessloop].charge_item_qual)
           FOR (nreprocessloop2 = 1 TO size(reprocrequest->process_event[nreprocessloop].charge_item,
            5))
            SET hlist2 = uar_srvadditem(hlist1,"charge_item")
            SET stat = uar_srvsetdouble(hlist2,"charge_item_id",reprocrequest->process_event[
             nreprocessloop].charge_item[nreprocessloop2].charge_item_id)
           ENDFOR
           SET stat = uar_srvsetshort(hlist1,"ignored_event_mod_qual",reprocrequest->process_event[
            nreprocessloop].ignored_event_mod_qual)
           FOR (nreprocessloop3 = 1 TO size(reprocrequest->process_event[nreprocessloop].
            ignored_event_mod,5))
            SET hlist3 = uar_srvadditem(hlist1,"ignored_event_mod")
            SET stat = uar_srvsetstring(hlist3,"field6",reprocrequest->process_event[nreprocessloop].
             ignored_event_mod[nreprocessloop3].field6)
           ENDFOR
         ENDFOR
        ENDIF
       ENDIF
       ...
       IF (crmstatus=0)
        ...
        IF (hreply > 0)
         ...
         FOR (lloopcount = 1 TO litemcount)
          ...
          IF (validate(syncrelease_reply->charge_items[lloopcount].charge_item_id,null_f8) != null_f8
          )
 ...
 END ;Subroutine
 SUBROUTINE (updatephysicians(dummyvar=i2) =i2)
   CALL logmessage(curprog,"updatePhysicians - Entering...",log_debug)
   DECLARE temp_charge_act_id = f8 WITH protect, noconstant(0.0)
   DECLARE status_ind = i2 WITH protect, noconstant(false)
   DECLARE temp_prsnl_id = f8 WITH protect, noconstant(0.0)
   DECLARE temp_active_ind = i2 WITH protect, noconstant(false)
   IF (sizeaddchargedetailsreply > 0)
    FOR (count = 1 TO size(addchargedetailsreply->mod_objs[1].mod_recs,5))
      SELECT INTO "nl:"
       FROM charge c
       WHERE c.charge_item_id=cnvtreal(addchargedetailsreply->mod_objs[1].mod_recs[count].pk_values)
       DETAIL
        temp_charge_act_id = c.charge_event_act_id
       WITH nocounter
      ;end select
      SET temp_prsnl_id = 0.0
      SET temp_active_ind = false
      SELECT INTO "nl:"
       ...
       DETAIL
        temp_prsnl_id = ceap.prsnl_id, temp_active_ind = ceap.active_ind
       WITH nocounter
      ;end select
      IF (curqual > 0)
       UPDATE  FROM charge_event_act_prsnl ceap
        SET ceap.active_ind = evaluate2(
          IF ((request->orderingphysicianid > 0.0)) 1
          ELSEIF ((request->delorderingphysind=1)) 0
          ELSE temp_active_ind
          ENDIF
          ), ceap.prsnl_id = evaluate2(
          IF ((request->orderingphysicianid > 0.00)) request->orderingphysicianid
          ELSEIF ((request->delorderingphysind=1)) 0.0
          ELSE temp_prsnl_id
          ENDIF
          ), ceap.updt_cnt = (ceap.updt_cnt+ 1),
      ...
      ELSE
       INSERT  FROM charge_event_act_prsnl ceap
        SET ceap.active_ind = evaluate2(
          IF ((request->orderingphysicianid > 0.0)) 1
          ELSEIF ((((request->delorderingphysind=1)) OR ((request->orderingphysicianid=0.0))) ) 0
          ENDIF
          ), ceap.prsnl_id = evaluate2(
          IF ((request->orderingphysicianid > 0.00)) request->orderingphysicianid
          ELSEIF ((((request->delorderingphysind=1)) OR ((request->orderingphysicianid=0.0))) ) 0.0
          ENDIF
          ), ceap.updt_dt_tm = cnvtdatetime(sysdate),
         ceap.updt_id = reqinfo->updt_id, ceap.updt_task = reqinfo->updt_task, ceap.updt_applctx =
         reqinfo->updt_applctx,
         ceap.charge_event_act_id = temp_charge_act_id, ceap.prsnl_type_cd = cs13029_ordered_cd
        WITH nocounter
       ;end insert
       IF (curqual < 0)
        SET reply->status_data.status = "F"
      ...
      IF (curqual > 0)
       UPDATE  FROM charge_event_act_prsnl ceap
        SET ceap.active_ind = evaluate2(
          IF ((request->renderingphysicianid > 0.0)) 1
          ELSEIF ((request->delrenderingphysind=1)) 0
          ELSE temp_active_ind
          ENDIF
          ), ceap.prsnl_id = evaluate2(
          IF ((request->renderingphysicianid > 0.00)) request->renderingphysicianid
          ELSEIF ((request->delrenderingphysind=1)) 0.0
          ELSE temp_prsnl_id
          ENDIF
          ), ceap.updt_cnt = (ceap.updt_cnt+ 1),
         ceap.updt_dt_tm = cnvtdatetime(sysdate), ceap.updt_id = reqinfo->updt_id, ceap.updt_task =
      ...
      ELSE
       INSERT  FROM charge_event_act_prsnl ceap
        SET ceap.active_ind = evaluate2(
          IF ((request->renderingphysicianid > 0.0)) 1
          ELSEIF ((((request->delrenderingphysind=1)) OR ((request->renderingphysicianid=0.0))) ) 0
          ENDIF
          ), ceap.prsnl_id = evaluate2(
          IF ((request->renderingphysicianid > 0.00)) request->renderingphysicianid
          ELSEIF ((((request->delrenderingphysind=1)) OR ((request->renderingphysicianid=0.0))) ) 0.0
          ENDIF
          ), ceap.updt_dt_tm = cnvtdatetime(sysdate),
         ceap.updt_id = reqinfo->updt_id, ceap.updt_task = reqinfo->updt_task, ceap.updt_applctx =
         reqinfo->updt_applctx,
         ceap.charge_event_act_id = temp_charge_act_id, ceap.prsnl_type_cd = cs13029_verified_cd
        WITH nocounter
       ;end insert
       IF (curqual < 0)
        SET reply->status_data.status = "F"
    ...
    ENDFOR
   ENDIF
   CALL logmessage(curprog,"updatePhysicians - Exiting...",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (loadreasoncodeintochargemodifier(dummyvar=i2) =null)
   CALL logmessage(curprog,"loadReasonCodeintoChargeModifier - Entering...",log_debug)
   IF ((((request->suspensersncd > 0)) OR (trim(request->reasoncomment) != ""))
    AND cs4001989_modify_cd > 0)
    SET chargemodrequest->charge_mod_qual = sizeaddchargedetailsreply
    SET stat = alterlist(chargemodrequest->charge_mod,sizeaddchargedetailsreply)
    FOR (i = 1 TO sizeaddchargedetailsreply)
      SET chargemodrequest->charge_mod[i].charge_item_id = cnvtreal(addchargedetailsreply->mod_objs[1
       ].mod_recs[i].pk_values)
      SET chargemodrequest->charge_mod[i].charge_mod_type_cd = cs13019_mod_rsn_cd
      SET chargemodrequest->charge_mod[i].field1_id = cs4001989_modify_cd
      SET chargemodrequest->charge_mod[i].field6 = "The charge was modified"
      SET chargemodrequest->charge_mod[i].field7 = request->reasoncomment
      SET chargemodrequest->charge_mod[i].field2_id = request->suspensersncd
      SET chargemodrequest->charge_mod[i].activity_dt_tm = cnvtdatetime(sysdate)
      SET chargemodrequest->charge_mod[i].updt_cnt = 0
      SET chargemodrequest->charge_mod[i].updt_dt_tm = cnvtdatetime(sysdate)
      SET chargemodrequest->charge_mod[i].updt_id = reqinfo->updt_id
      SET chargemodrequest->charge_mod[i].updt_task = reqinfo->updt_task
      SET chargemodrequest->charge_mod[i].updt_applctx = reqinfo->updt_applctx
      SET chargemodrequest->charge_mod[i].active_ind = 1
      SET chargemodrequest->charge_mod[i].active_status_cd = cs48_active_cd
      SET chargemodrequest->charge_mod[i].active_status_dt_tm = cnvtdatetime(sysdate)
      SET chargemodrequest->charge_mod[i].active_status_prsnl_id = reqinfo->updt_id
      SET chargemodrequest->charge_mod[i].beg_effective_dt_tm = cnvtdatetime(sysdate)
      SET chargemodrequest->charge_mod[i].end_effective_dt_tm = cnvtdatetime(
       "31-DEC-2100 23:59:59.99")
      SET chargemodrequest->charge_mod[i].action_type = "ADD"
    ENDFOR
   ENDIF
   CALL logmessage(curprog,"loadReasonCodeintoChargeModifier - Exiting...",log_debug)
 END ;Subroutine
 SUBROUTINE (loadchargemodifierwithbillencntr(dummyvar=i2) =null)
   CALL logmessage(curprog,"loadChargeModifierWithBillEncntr - Entering...",log_debug)
   DECLARE chargeidx = i4 WITH protect, noconstant(0)
   FOR (chargeidx = 1 TO size(request->chargelist,5))
     SELECT INTO "nl:"
      FROM (dummyt d1  WITH seq = value(size(addchargedetailsreply->mod_objs[1].mod_recs,5))),
      ...
      PLAN (d1)
       JOIN (cm
       WHERE (cm.charge_item_id=request->chargelist[chargeidx].chargeid)
      ...
      DETAIL
       ...
       modifychargerequest->charge_mod[chrgmodcnt].charge_item_id = cnvtreal(addchargedetailsreply->
        mod_objs[1].mod_recs[d1.seq].pk_values),
       ...
       charge_mod[chrgmodcnt].field2 = cm.field2, modifychargerequest->charge_mod[chrgmodcnt].
       field2_id = cm.field2_id,
       ...
       modifychargerequest->charge_mod[chrgmodcnt].field6 = cm.field6, modifychargerequest->
       charge_mod[chrgmodcnt].field7 = cm.field7, modifychargerequest->charge_mod[chrgmodcnt].
     ...
     ;end select
   ENDFOR
   CALL logmessage(curprog,"loadChargeModifierWithBillEncntr - Exiting...",log_debug)
 END ;Subroutine
 SUBROUTINE (postcharges(prafcprofitrequest=vc(ref),prafcprofitreply=vc(ref)) =i2)
   CALL logmessage(curprog,"postCharges - Entering...",log_debug)
   CALL echorecord(prafcprofitrequest)
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (loadflexfielddataforchargerequest(dummyvar=i2) =i2)
   ...
   DECLARE countevtmod = i4 WITH protect, noconstant(0)
   DECLARE chrgmodcnt = i4 WITH protect, noconstant(0)
   IF (size(addchargedetailsreply->mod_objs[1].mod_recs,5) > 0)
    FOR (chargeidx = 1 TO size(addchargedetailsreply->mod_objs[1].mod_recs,5))
      IF (size(request->flexfieldsmodel,5) > 0)
       SELECT INTO "nl:"
        ...
        PLAN (d1
         ...
         JOIN (d2)
        HEAD REPORT
         chrgmodcnt = size(modifychargerequest->charge_mod,5), countevtmod = size(reqbillcodedetails
          ->charge_event_mod,5)
        DETAIL
         chrgmodcnt += 1, countevtmod += 1, stat = alterlist(modifychargerequest->charge_mod,
          chrgmodcnt),
         stat = alterlist(reqbillcodedetails->charge_event_mod,countevtmod), modifychargerequest->
         charge_mod[chrgmodcnt].charge_item_id = cnvtreal(addchargedetailsreply->mod_objs[1].
          mod_recs[chargeidx].pk_values), modifychargerequest->charge_mod[chrgmodcnt].
         charge_event_mod_id = request->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].
         chargeeventmodid,
         modifychargerequest->charge_mod[chrgmodcnt].action_type = "ADD", modifychargerequest->
         charge_mod[chrgmodcnt].charge_mod_type_cd = cs13019_flex_cd, modifychargerequest->
         charge_mod[chrgmodcnt].active_ind = 1,
         modifychargerequest->charge_mod[chrgmodcnt].field1_id = request->flexfieldsmodel[d1.seq].
         chargemodifiermodel[d2.seq].modifiercd, modifychargerequest->charge_mod[chrgmodcnt].field7
          = uar_get_code_display(request->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].
          modifiercd), reqbillcodedetails->charge_event_mod[countevtmod].charge_item_id = cnvtreal(
          addchargedetailsreply->mod_objs[1].mod_recs[chargeidx].pk_values),
         reqbillcodedetails->charge_event_mod[countevtmod].charge_event_mod_id = request->
         flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].chargeeventmodid, reqbillcodedetails->
         charge_event_mod[countevtmod].charge_mod_type_cd = cs13019_flex_cd, reqbillcodedetails->
         charge_event_mod[countevtmod].field7 = uar_get_code_display(request->flexfieldsmodel[d1.seq]
          .chargemodifiermodel[d2.seq].modifiercd),
         reqbillcodedetails->charge_event_mod[countevtmod].field1_id = request->flexfieldsmodel[d1
         .seq].chargemodifiermodel[d2.seq].modifiercd, fielddatatype = request->flexfieldsmodel[d1
         .seq].chargemodifiermodel[d2.seq].fielddatatype, modifychargerequest->charge_mod[chrgmodcnt]
         .field2 = fielddatatype,
         reqbillcodedetails->charge_event_mod[countevtmod].field2 = fielddatatype
         IF (fielddatatype="INDICATOR")
          modifychargerequest->charge_mod[chrgmodcnt].field2_id = request->flexfieldsmodel[d1.seq].
          chargemodifiermodel[d2.seq].fieldvaluenumber, reqbillcodedetails->charge_event_mod[
          countevtmod].field2_id = request->flexfieldsmodel[d1.seq].chargemodifiermodel[d2.seq].
          fieldvaluenumber
         ENDIF
        FOOT REPORT
         modifychargerequest->charge_mod_qual = chrgmodcnt, reqbillcodedetails->charge_event_mod_qual
          = countevtmod
        WITH nocounter
       ;end select
      ENDIF
    ENDFOR
   ELSE
    CALL logmessage("loadFlexFieldDataForChargeRequest","addChargeDetailsReply record is empty",
     log_error)
    RETURN(false)
   ENDIF
