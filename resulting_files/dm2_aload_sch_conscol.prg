CREATE PROGRAM dm2_aload_sch_conscol
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_sch_conscol",".log","dm2_aload_sch_conscol")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_sch_conscol"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dascc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dascc_str = vc WITH protect, noconstant("")
 DECLARE dascc_req_size = i4 WITH protect, noconstant(0)
 DECLARE dascc_ndx = i4 WITH protect, noconstant(0)
 FREE RECORD dascc_maint
 RECORD dascc_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET dascc_maint->num_of_inserts = 0
 SET dascc_maint->num_of_updates = 0
 SET dascc_maint->num_of_none = 0
 SET dascc_req_size = size(requestin->list_0,5)
 SET stat = alterlist(dascc_maint->qual,dascc_req_size)
 IF (dac_pop_coldic_rec("DM_AFD_CONS_COLUMNS")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  SET dm_err->eproc = "Determining maintenance type for DM_AFD_CONS_COLUMNS."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dascc_req_size))
  DETAIL
   dascc_maint->qual[d.seq].maint_type = "I", dascc_maint->num_of_inserts = (dascc_maint->
   num_of_inserts+ 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 SELECT INTO "nl:"
  FROM dm_afd_tables dat,
   dm_afd_cons_columns dai,
   (dummyt d  WITH seq = value(dascc_req_size))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (dat
   ...
   JOIN (dai
   WHERE dai.owner=dat.owner
    AND dai.alpha_feature_nbr=cnvtint(requestin->list_0[d.seq].alpha_feature_nbr)
    AND (dai.table_name=requestin->list_0[d.seq].table_name)
    AND (dai.constraint_name=requestin->list_0[d.seq].constraint_name)
    AND (dai.column_name=requestin->list_0[d.seq].column_name))
  ORDER BY dai.table_name, dai.constraint_name, dai.column_name
  DETAIL
   IF (cnvtint(requestin->list_0[d.seq].schema_instance) < dat.schema_instance)
    dascc_maint->qual[d.seq].maint_type = "N", dascc_maint->num_of_none = (dascc_maint->num_of_none+
    1), dascc_maint->num_of_inserts = (dascc_maint->num_of_inserts - 1)
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm)=cnvtdatetime(dai.updt_dt_tm))
    dascc_maint->qual[d.seq].maint_type = "N", dascc_maint->num_of_none = (dascc_maint->num_of_none+
    1), dascc_maint->num_of_inserts = (dascc_maint->num_of_inserts - 1)
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm) != cnvtdatetime(dai.updt_dt_tm))
    dascc_maint->qual[d.seq].maint_type = "U", dascc_maint->num_of_updates = (dascc_maint->
    num_of_updates+ 1), dascc_maint->num_of_inserts = (dascc_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dascc_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((dascc_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(dascc_req_size) )"),0)
  ...
  COMMIT
 ENDIF
 IF ((dascc_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where dascc_maint->qual[d.seq].maint_type = 'U' ",0)
  ...
  CALL dm2_push_cmd(" and requestin->list_0[d.seq].constraint_name = dcv.constraint_name ",0)
  CALL dm2_push_cmd(" and requestin->list_0[d.seq].column_name = dcv.column_name ",0)
  IF (dm2_push_cmd(
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_sch_conscol completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_sch_conscol")
END GO
