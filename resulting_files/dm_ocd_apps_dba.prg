CREATE PROGRAM dm_ocd_apps:dba
 ...
 SUBROUTINE drr_chk_active_runners(dcar_dblink,dcar_count_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE app_report_name = vc WITH protect, constant("Applications Installed Report")
 ...
 END ;Subroutine
#1000_initialize
 DECLARE doa_envid = f8 WITH protect, noconstant(0.0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="DM_ENV_ID"
  DETAIL
   doa_envid = di.info_number
  WITH nocounter
 ;end select
 IF (((curqual=0) OR (doa_envid=0)) )
  SET dm_err->err_ind = 1
 ...
 SET oa_data->low_date = cnvtdatetime("01-JAN-1800")
 SET oa_data->high_date = cnvtdatetime("31-DEC-2100")
 SET oa_data->high_ocd = 999999
 IF (validate(ocd_number,0))
  SET oa_data->low_ocd = ocd_number
  SET oa_data->high_ocd = ocd_number
  SET oa_data->called = 1
  SET oa_data->mode = ocd_apps_mode
  SET oa_data->filter_style = "O"
  IF (dir_silmode_requested_ind=0)
   SET message = nowindow
  ENDIF
 ELSE
...
#2000_process
 ...
 CALL clear(1,1)
 CALL box(1,1,3,132)
 CALL text(2,3,"O C D   A P P L I C A T I O N   L O G   M A N A G E R")
 CALL text(5,3,
  "Preview applications on OCD, Report application logs, Cleanup application logs, or Quit (P/R/C/Q)?"
  )
 CALL accept(5,102,"P;CU","R"
  WHERE curaccept IN ("P", "R", "C", "Q"))
 CASE (curaccept)
  OF oa_report:
   SET oa_data->mode = oa_report
  OF oa_clear:
   SET oa_data->mode = oa_clear
  OF oa_preview:
   SET oa_data->mode = oa_preview
   SET oa_data->filter_style = "O"
 ...
 SET oa_data->filter_style = curaccept
#2050_filter_prompt
 SET oa_data->low_date = cnvtdatetime("01-JAN-1800")
 SET oa_data->high_date = cnvtdatetime("31-DEC-2100")
 SET oa_data->high_ocd = 999999
 CASE (oa_data->filter_style)
  OF "D":
   CALL text(9,3,concat("Enter date range:             to ",format(cnvtdatetime(curdate,curtime3),
      "DD-MMM-YYYY;;D")))
   SET oa_data->low_date = 0.0
   SET oa_data->high_date = 0.0
   WHILE ( NOT (oa_data->low_date))
    CALL accept(9,21,"NNDCCCDNNNN;CU",format(cnvtdatetime((curdate - 30),curtime3),"DD-MMM-YYYY;;D"))
    SET oa_data->low_date = cnvtdatetime(concat(curaccept," 00:00:00.00"))
   ENDWHILE
   WHILE ((oa_data->high_date < oa_data->low_date))
    CALL accept(9,36,"NNDCCCDNNNN;CU",format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY;;D"))
    SET oa_data->high_date = cnvtdatetime(concat(curaccept," 23:59:59.00"))
   ENDWHILE
  OF "O":
   CALL text(9,3,"Enter an OCD number:")
   CALL accept(9,24,"9(6)")
   SET oa_data->low_ocd = curaccept
   SET oa_data->high_ocd = curaccept
 ENDCASE
#2100_perform
 DECLARE doa_txt_file = vc WITH protect
 IF ((oa_data->mode=oa_report))
  CASE (oa_data->filter_style)
   OF "D":
    SET doa_txt_file = build("ocd",format(oa_data->low_date,"mmddyy;;D"),"_app.txt")
   OF "O":
    SET doa_txt_file = build("ocd",oa_data->low_ocd,"_app.txt")
   OF "M":
    SET doa_txt_file = "ocd_mass_app.txt"
   OF "A":
    SET doa_txt_file = "ocd_all_app.txt"
  ENDCASE
 ENDIF
 IF ((oa_data->mode=oa_preview))
  SELECT INTO "nl:"
   z = count(*)
   FROM dm_ocd_application d
   WHERE d.alpha_feature_nbr BETWEEN oa_data->low_ocd AND oa_data->high_ocd
    AND  NOT ( EXISTS (
   (SELECT
    t.application_number
    FROM application t
    WHERE t.application_number=d.application_number)))
   DETAIL
    oa_data->rows = z
   WITH nocounter
  ;end select
 ELSEIF ((oa_data->filter_style="M"))
  SELECT INTO "nl:"
   z = count(*)
   FROM ocd_install_log l,
    dm_alpha_features_env d
   WHERE d.environment_id=doa_envid
    AND d.curr_migration_ind=1
    AND l.component_type=oa_app
 ...
 ELSE
  SELECT INTO "nl:"
   z = count(*)
   FROM ocd_install_log l
   WHERE l.component_type=oa_app
    AND l.install_dt_tm BETWEEN cnvtdatetime(oa_data->low_date) AND cnvtdatetime(oa_data->high_date)
    AND l.ocd BETWEEN oa_data->low_ocd AND oa_data->high_ocd
   DETAIL
    oa_data->rows = z
   WITH nocounter
  ;end select
 ...
 IF ( NOT (oa_data->rows))
  IF (oa_data->called)
   IF ((oa_data->mode=oa_preview))
    CALL echo("No new applications found on this OCD.  OCD application report will be skipped.")
    GO TO 2999_process_exit
   ELSE
    IF ((oa_data->mode=oa_report))
     SELECT INTO value(doa_txt_file)
      d.seq
      FROM (dummyt d  WITH seq = 1)
      DETAIL
       row + 1,
       "No new applications were installed by this OCD.  OCD application report will be skipped."
      WITH nocounter
     ;end select
    ENDIF
    CALL echo(
  ...
  ELSE
   IF ((oa_data->mode=oa_preview))
    CALL message("No new applications found on this OCD.  Press <RETURN> to continue...")
    GO TO 2999_process_exit
   ELSE
    IF ((oa_data->mode=oa_report))
     SELECT INTO value(doa_txt_file)
      FROM (dummyt d  WITH seq = 1)
      DETAIL
       row + 1, "No new application logs found.  OCD application report will be skipped."
      WITH nocounter
     ;end select
    ENDIF
    CALL message("No application logs found.  Press <RETURN> to continue...")
    GO TO 2900_continue
   ENDIF
  ENDIF
 ENDIF
 IF ((oa_data->mode=oa_preview))
  SELECT
   d.alpha_feature_nbr, d.application_number
   FROM dm_ocd_application d
   WHERE d.alpha_feature_nbr BETWEEN oa_data->low_ocd AND oa_data->high_ocd
    AND  NOT ( EXISTS (
   (SELECT
    ...
    FROM application t
    WHERE t.application_number=d.application_number)))
   ORDER BY d.alpha_feature_nbr, d.application_number
   HEAD PAGE
    CALL print("OCD APPLICATION Preview"), row + 2,
    CALL print("APPLICATIONS                                                             OCD"),
    row + 1,
    CALL print("----                                                                     ---"), row
     + 1
   DETAIL
    col 0,
    CALL print(concat(trim(cnvtstring(d.application_number),3)," - ",substring(1,60,d.description))),
    col 73,
 ...
 IF ((oa_data->mode=oa_report))
   ...
   SELECT INTO value(doa_txt_file)
    ...
    PLAN (d
     ...
     WHERE l.component_type=oa_app
      AND l.ocd=d.alpha_feature_nbr)
     JOIN (a
     WHERE a.application_number=cnvtint(l.end_state))
    ORDER BY a.application_number
    HEAD PAGE
     CALL print("OCD Application Log Report"), row + 2,
     CALL print(
     "APPLICATION                                                              Installed           OCD"
     ),
     row + 1,
     CALL print(
     "-----------                                                              ---------           ---"
   ...
   SELECT INTO value(doa_txt_file)
    ...
    PLAN (l
     WHERE l.component_type=oa_app
      ...
      AND l.ocd BETWEEN oa_data->low_ocd AND oa_data->high_ocd)
     JOIN (t
     WHERE t.application_number=cnvtint(l.end_state))
    ORDER BY l.install_dt_tm DESC, l.end_state, l.ocd
    HEAD PAGE
     CALL print(app_report_name), row + 2,
     CALL print(
     "APPLICATIONS                                                             Installed           Package"
     ),
     row + 1,
     CALL print(
     "------------                                                             ---------           -------"
    ...
    DETAIL
     col 0,
     CALL print(concat(trim(cnvtstring(t.application_number),3)," - ",substring(1,60,t.description))),
     col 73,
     CALL print(format(l.install_dt_tm,"DD-MMM-YYYY HH:mm;;D")), col 93,
     CALL print(trim(cnvtstring(l.ocd),3)),
     row + 1
  ...
  ENDIF
  IF ((dir_ui_misc->dm_process_event_id > 0))
   IF (drr_assign_file_to_installs("RPT:APPLICATIONS INSTALLED",doa_txt_file,dir_ui_misc->
    dm_process_event_id)=0)
    GO TO 9999_exit_program
   ENDIF
  ENDIF
  IF (dir_silmode_requested_ind=1)
   CALL dir_add_silmode_entry(app_report_name,doa_txt_file)
   SET dm_err->eproc = concat("Silent Mode:  ",app_report_name," was bypassed.  Report exits",
    " in CCLUSERDIR under file name: ",doa_txt_file)
   CALL disp_msg("",dm_err->logfile,0)
  ELSE
   IF (dm2_disp_file(doa_txt_file,app_report_name)=0)
    SET x = x
   ENDIF
  ENDIF
 ENDIF
 IF ((oa_data->mode=oa_clear))
  SET oa_data->text = concat(trim(cnvtstring(oa_data->rows),3),
   " application logs will be removed.  Continue (Y/N)?")
  CALL text(24,2,oa_data->text)
  CALL accept(24,(size(oa_data->text)+ 3),"P;CU","Y"
  ...
  IF (curaccept="Y")
   ...
   CALL message(concat(trim(cnvtstring(curqual),3),
     " application logs removed.  Press <RETURN> to continue..."))
  ENDIF
 ENDIF
#2900_continue
 IF ( NOT (oa_data->called))
  GO TO 2000_process
 ENDIF
#2999_process_exit
#9999_exit_program
