CREATE PROGRAM bhs_wet_read_report_new:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "prompt1" = 0
  WITH outdev, prompt1
 DECLARE fmc = f8 WITH constant(uar_get_code_by("DESCRIPTION",221,"Franklin Medical Center")),
 protect
 DECLARE mlhospital = f8 WITH constant(uar_get_code_by("DESCRIPTION",221,"Mary Lane Hospital")),
 protect
 DECLARE bmc = f8 WITH constant(uar_get_code_by("DESCRIPTION",221,"BayState Medical Center")),
 protect
 DECLARE bwh = f8 WITH constant(uar_get_code_by("DESCRIPTION",221,"Baystate Wing Hospital")), protect
 DECLARE bnh = f8 WITH constant(uar_get_code_by("DESCRIPTION",221,"Baystate Noble Hospital")),
 protect
 ...
 ENDIF
 SELECT INTO  $OUTDEV
  patient_name = per.name_full_formatted, accession = oros.accession_nbr, study_description =
  ...
  FROM rad_init_read r,
   ...
   person per
  PLAN (oros
   WHERE oros.order_dt_tm BETWEEN cnvtdatetime((curdate - 1),0) AND cnvtdatetime((curdate - 1),235959
    )
    AND (oros.perf_inst_cd= $PROMPT1))
   JOIN (r
   WHERE r.order_id=oros.order_id)
   JOIN (p
   WHERE p.person_id=r.updt_id)
   JOIN (per
   WHERE oros.patient_id=per.person_id)
  ORDER BY patient_name, accession, r.updt_dt_tm
  HEAD REPORT
   m_numlines = 0,
  ...
  HEAD patient_name
   ...
   CALL print(calcpos(72,(y_pos+ 11))), patient_name, row + 1,
   y_pos = (y_pos+ 12)
  HEAD accession
   IF (((y_pos+ 111) >= 612))
    y_pos = 0, BREAK
   ...
   CALL print(calcpos(126,(y_pos+ 11))), accession,
   CALL print(calcpos(288,(y_pos+ 11))),
   study_description, row + 1, y_pos = (y_pos+ 12)
  HEAD r.updt_dt_tm
   y_pos = (y_pos+ 0)
  DETAIL
   IF (((y_pos+ 97) >= 612))
