CREATE PROGRAM dm_remove_text_oen:dba
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 IF (check_logfile("dm_rmv_text",".log","DM_REMOVE_TEXT_OEN LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_REMOVE_TEXT_OEN..."
 ...
 IF (((reflect(parameter(1,0)) != "I*"
  ...
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "Expected syntax:  dm_remove_text_oen <DM_TEXT_FIND_DETAIL_ID>, <DM_TEXT_FIND_LOG_ID> go"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_oen
 ENDIF
 SET drto_det_id =  $1
 ...
 SELECT INTO "NL:"
  ...
  (SELECT
   "x"
   FROM oen_script o
   WHERE trim(cnvtupper(o.script_name))=d.script_name)))
  HEAD REPORT
...
#exit_oen
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending DM_REMOVE_TEXT_OEN"
  CALL final_disp_msg("dm_remove_text_oen")
 ENDIF
END GO
