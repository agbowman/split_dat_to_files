CREATE PROGRAM bhs_pharm_admin_hist_rpt2:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY o.order_id
  HEAD o.order_id
   t_record->order_cnt = (t_record->order_cnt+ 1)
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    WHERE oa.order_id=o.order_id
     AND oa.action_type_cd=complete_cd
   ...
   HEAD oa.clinical_display_line
    null
   HEAD o.order_id
    t_record->admn_cnt = (t_record->admn_cnt+ 1)
    IF (mod(t_record->admn_cnt,100)=1)
     stat = alterlist(t_record->admn_qual,(t_record->admn_cnt+ 99))
    ENDIF
    idx = t_record->admn_cnt, t_record->admn_qual[idx].name = p.name_full_formatted, t_record->
    admn_qual[idx].fin = trim(ea.alias),
    t_record->admn_qual[idx].age = trim(cnvtage(p.birth_dt_tm)), t_record->admn_qual[idx].unit =
    uar_get_code_display(elh.loc_nurse_unit_cd), t_record->admn_qual[idx].facility =
    uar_get_code_display(elh.loc_facility_cd),
    t_record->admn_qual[idx].mnemonic = o.order_mnemonic, t_record->admn_qual[idx].display = trim(oa
     .clinical_display_line), t_record->admn_qual[idx].provider = trim(p2.name_full_formatted),
    t_record->admn_qual[idx].admn_dt_tm = cnvtdatetime(oa.action_dt_tm)
   FOOT REPORT
 ...
 ELSE
  SELECT INTO "nl:"
   ...
   HEAD o.order_id
    ...
    ENDIF
    idx = t_record->admn_cnt, t_record->admn_qual[idx].name = p.name_full_formatted, t_record->
    admn_qual[idx].age = trim(cnvtage(p.birth_dt_tm)),
    t_record->admn_qual[idx].unit = uar_get_code_display(elh.loc_nurse_unit_cd), t_record->admn_qual[
    idx].facility = uar_get_code_display(elh.loc_facility_cd), t_record->admn_qual[idx].mnemonic = o
    .order_mnemonic,
    t_record->admn_qual[idx].display = trim(oa.clinical_display_line), t_record->admn_qual[idx].
 ...
 IF (email_ind=0)
  SELECT INTO  $OUTDEV
   patient = trim(substring(1,100,t_record->admn_qual[d.seq].name)), fin = trim(substring(1,12,
     t_record->admn_qual[d.seq].fin)), age = trim(substring(1,10,t_record->admn_qual[d.seq].age)),
   nurse_unit = trim(substring(1,100,t_record->admn_qual[d.seq].unit)), facility = trim(substring(1,
     100,t_record->admn_qual[d.seq].facility)), clinical_display_line = trim(substring(1,200,concat(
 ...
 ELSE
  SELECT INTO "pharm_admin_hist.xls"
   ...
   admn_qual[d.seq].age,
   clinical_display_line = concat(t_record->admn_qual[d.seq].mnemonic," ",t_record->admn_qual[d.seq].
    display), administration_date_time = format(t_record->admn_qual[d.seq].admn_dt_tm,
    "DD-MMM-YYYY HH:MM;;Q")
   FROM (dummyt d  WITH seq = t_record->admn_cnt)
   PLAN (d)
   ORDER BY patient, clinical_display_line, administration_date_time
