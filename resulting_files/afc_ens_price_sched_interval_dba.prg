CREATE PROGRAM afc_ens_price_sched_interval:dba
 ...
 IF ((request->item_interval_qual > 0))
  SET reply->item_interval_qual = request->item_interval_qual
  SET stat = alterlist(reply->item_interval,request->item_interval_qual)
  FOR (inx0 = 1 TO request->item_interval_qual)
    CASE (request->item_interval[inx0].action_type)
     OF "ADD":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_add_price_sched_interval
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "UPT":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_upt_price_sched_interval
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "DEL":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_del_price_sched_interval
      IF (failed != false)
       GO TO check_error
      ENDIF
     ELSE
