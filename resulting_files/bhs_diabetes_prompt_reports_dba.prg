CREATE PROGRAM bhs_diabetes_prompt_reports:dba
 PROMPT
 ...
 SET t_record->one_year_date = cnvtdatetime(datetimeadd(cnvtdatetime(curdate,curtime3),- (365)))
 IF (( $TYPE=0))
  SELECT INTO  $OUTDEV
 ...
 SELECT INTO "nl:"
  ...
  WITH maxcol = 1000
 ;end select
 CALL echo("get last scheduled appt")
 SELECT INTO "nl:"
  ...
  HEAD sa.person_id
   t_record->pat_qual[d.seq].last_visit_dt_tm = cnvtdatetime(sa.beg_dt_tm), t_record->pat_qual[d.seq]
   .last_visit_encntr_id = sa.encntr_id
  WITH nocounter
 ;end select
 CALL echo("get last scheduled appt end")
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = nbuckets),
   clinical_event ce
  PLAN (d
   ...
   WHERE expand(indx,nstart,(nstart+ (nbucketsize - 1)),ce.person_id,t_record->pat_qual[indx].
    ...
    AND ce.clinsig_updt_dt_tm >= cnvtdatetime(t_record->one_year_date))
  ...
  SELECT INTO "diabetes_registry.xls"
   ...
   HEAD REPORT
    ...
    t_line, row + 1, t_line = concat("Patient Name",char(9),"Medical Record #",char(9),
     ...
     "Med",char(9),"Last office visit",char(9),"Date",
