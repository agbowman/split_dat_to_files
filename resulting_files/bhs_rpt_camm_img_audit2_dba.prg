CREATE PROGRAM bhs_rpt_camm_img_audit2:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  HEAD ce1.parent_event_id
   ...
   CALL echo(substring(1,findstring("}",ce1.reference_nbr,1),ce1.reference_nbr))
  HEAD ce1.event_id
   ml_idx = 0
   ...
   IF (((ms_report_by="IMAGE") OR (ml_idx=0)) )
    ...
    .person_id, m_rec->note[ml_row_cnt].s_ref_nbr = substring(1,findstring("}",ce1.reference_nbr,1),
     ce1.reference_nbr),
    m_rec->note[ml_row_cnt].s_unit = trim(uar_get_code_display(e.loc_nurse_unit_cd),3), m_rec->note[
    ml_row_cnt].s_med_svc = trim(uar_get_code_display(e.med_service_cd),3), m_rec->note[ml_row_cnt].
    f_attach_event_id = ce1.event_id,
    ...
    ml_attach_cnt, m_rec->note[ml_row_cnt].f_attach_prsnl_id = pr2.person_id,
    m_rec->note[ml_row_cnt].s_attach_prsnl_userid = trim(pr2.username,3), m_rec->note[ml_row_cnt].
    s_attach_prsnl_name = trim(pr2.name_full_formatted,3), m_rec->note[ml_row_cnt].s_attach_prsnl_pos
     = trim(uar_get_code_display(pr2.position_cd),3)
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM dms_media_identifier dmid,
   ...
   prsnl pr
  PLAN (dmid
   WHERE expand(ml_exp,1,size(m_rec->note,5),dmid.media_object_identifier,m_rec->note[ml_exp].
    s_ref_nbr))
   JOIN (dmi
  ...
  DETAIL
   ml_idx = locateval(ml_loc,1,size(m_rec->note,5),dmid.media_object_identifier,m_rec->note[ml_loc].
    s_ref_nbr), m_rec->note[ml_idx].f_prsnl_id = dmi.created_by_id, m_rec->note[ml_idx].
   s_content_type = trim(dct.content_type_key),
   m_rec->note[ml_idx].s_prsnl_name = trim(pr.name_full_formatted,3), m_rec->note[ml_idx].
   s_prsnl_userid = trim(pr.username,3), m_rec->note[ml_idx].s_prsnl_pos = trim(uar_get_code_display(
     pr.position_cd),3)
 ...
 IF (ml_row_cnt > 0)
  IF (textlen(ms_email) > 0
   ...
   SELECT INTO value(ms_email_file)
    ...
    WHERE size(trim(m_rec->note[d.seq].s_ref_nbr)) > 0
     AND (m_rec->note[d.seq].s_content_type IN ("BHRIMG", "GNRIMG", "MEDIMG"))
    ORDER BY d.seq
    HEAD REPORT
     ms_tmp = concat(
    ...
    DETAIL
     ms_tmp = concat('"',trim(cnvtstring(m_rec->note[d.seq].f_encntr_id)),'",','"',trim(cnvtstring(
      ...
      '"',m_rec->note[d.seq].s_note_event_end_dt_tm,'",','"',trim(cnvtstring(m_rec->note[d.seq].
        l_attach_cnt)),
      '",','"',m_rec->note[ml_row_cnt].s_attach_prsnl_userid,'",','"',
      m_rec->note[ml_row_cnt].s_attach_prsnl_name,'",','"',m_rec->note[ml_row_cnt].s_attach_prsnl_pos,
      '",',
      '"',m_rec->note[ml_row_cnt].s_prsnl_userid,'",','"',m_rec->note[ml_row_cnt].s_prsnl_name,
      '",','"',m_rec->note[ml_row_cnt].s_prsnl_pos,'"')
     IF (ms_report_by="IMAGE")
