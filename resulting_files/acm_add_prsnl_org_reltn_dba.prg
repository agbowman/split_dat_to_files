CREATE PROGRAM acm_add_prsnl_org_reltn:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->prsnl_org_reltn_qual[xref->add[index].idx].prsnl_org_reltn_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(prsnl_seq,nextval)
     FROM dual
     DETAIL
      acm_request->prsnl_org_reltn_qual[xref->add[index].idx].prsnl_org_reltn_id = cnvtreal(
       nextseqnum), reply->prsnl_org_reltn_qual[xref->add[index].idx].prsnl_org_reltn_id = cnvtreal(
       nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->prsnl_org_reltn_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->prsnl_org_reltn_qual[xref->add[index].idx].prsnl_org_reltn_id = acm_request->
    prsnl_org_reltn_qual[xref->add[index].idx].prsnl_org_reltn_id
   ENDIF
   SET reply->prsnl_org_reltn_qual[xref->add[index].idx].status = 0
   IF ((acm_request->prsnl_org_reltn_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->prsnl_org_reltn_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->prsnl_org_reltn_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->prsnl_org_reltn_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   prsnl_org_reltn p
  SET p.prsnl_org_reltn_id = acm_request->prsnl_org_reltn_qual[xref->add[d.seq].idx].
   prsnl_org_reltn_id, p.person_id = acm_request->prsnl_org_reltn_qual[xref->add[d.seq].idx].
   person_id, p.organization_id = acm_request->prsnl_org_reltn_qual[xref->add[d.seq].idx].
   organization_id,
   p.confid_level_cd = acm_request->prsnl_org_reltn_qual[xref->add[d.seq].idx].confid_level_cd, p
   .beg_effective_dt_tm = cnvtdatetime(acm_request->prsnl_org_reltn_qual[xref->add[d.seq].idx].
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->prsnl_org_reltn_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->prsnl_org_reltn_qual[xref->add[index].idx].status=0))
