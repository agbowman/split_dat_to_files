CREATE PROGRAM dm2_mig_utc_compare_triggers:dba
 ...
 SUBROUTINE dm2_get_dbase_name(dgdn_name_out)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmuctgr_prep(null) = i2
 DECLARE dmuctgr_dbtype = vc WITH protect, noconstant("")
 DECLARE dmuctgr_rpt_file = vc WITH protect, noconstant("dm2_mig_invalid_tables.rpt")
 DECLARE dmuctgr_locndx = i4 WITH protect, noconstant(0)
 DECLARE dmuctgr_tbl_fnd = i4 WITH protect, noconstant(0)
 DECLARE dmuctgr_col_fnd = i4 WITH protect, noconstant(0)
 DECLARE dmuctgr_loop_var = i4 WITH protect, noconstant(0)
 DECLARE dmuctgr_col_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmuctgr_col_cnvt = i2 WITH protect, noconstant(0)
 DECLARE dmuctgr_dbase_name = vc WITH protect, noconstant(" ")
 DECLARE dmuctgr_db_link = vc WITH protect, noconstant("")
 DECLARE dmuctgr_tab_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmuctgr_tab_col_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmuctgr_standalone = i2 WITH protect, noconstant(0)
 DECLARE dmuctgr_ndx = i4 WITH protect, noconstant(0)
 DECLARE dmuctgr_mngd_tables = vc WITH protect, noconstant("")
 DECLARE dmuctgr_td_idx = i4 WITH protect, noconstant(0)
 FREE RECORD dmuctgr_tgr_rec
 RECORD dmuctgr_tgr_rec(
   1 cnt = i4
   1 qual[*]
     2 tbl_name = vc
     2 tgr_name = vc
 ...
 SET dmuctgr_tgr_rec->cnt = 0
 FREE RECORD dmuctgr_table_data
 RECORD dmuctgr_table_data(
   1 cnt = i4
   1 qual[*]
     2 tbl_name = vc
     2 tgr_not_fnd = i4
     2 orig_col_not_fnd = i4
     2 utc_col_not_fnd = i4
     2 ccnt = i4
     2 col[*]
       3 col_id = i4
       3 col_name = vc
 )
 SET dmuctgr_table_data->cnt = 0
 FREE RECORD dmuctgr_td_tables
 RECORD dmuctgr_td_tables(
   1 cnt = i4
   1 tbl[*]
     2 table_name = vc
 )
 SET dmuctgr_td_tables->cnt = 0
 IF ((dm2_install_schema->curprog="NONE"))
  SET dmuctgr_standalone = 1
 ENDIF
 IF (check_logfile("dm2_mig_utc_cmp_tgr",".log","DM2_MIG_UTC_CMP_TGR LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning DM2_MIG_UTC_COMPARE_TRIGGERS"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dmuctgr_dbtype = trim(cnvtupper( $1),3)
 IF (dmuctgr_dbtype="SOURCE")
  SET dmuctgr_db_link = "MIG_TARGET"
 ELSE
  SET dmuctgr_db_link = "REF_DATA_LINK"
 ENDIF
 IF (dmuctgr_prep(null)=0)
  GO TO exit_script
 ENDIF
 IF (dmuctgr_dbtype="SOURCE")
  IF (dm2_get_remote_dbase_name(dmuctgr_db_link,dmuctgr_dbase_name)=0)
   RETURN(0)
  ENDIF
 ELSE
  IF (dm2_get_dbase_name(dmuctgr_dbase_name)=0)
   RETURN(0)
  ENDIF
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  WHERE td.owner=currdbuser
   AND td.table_name=td.full_table_name
  HEAD REPORT
   stat = alterlist(dmuctgr_td_tables->tbl,10)
  DETAIL
   dmuctgr_td_tables->cnt = (dmuctgr_td_tables->cnt+ 1)
   IF (mod(dmuctgr_td_tables->cnt,10)=1
    AND (dmuctgr_td_tables->cnt != 1))
    stat = alterlist(dmuctgr_td_tables->tbl,(dmuctgr_td_tables->cnt+ 9))
   ENDIF
   dmuctgr_td_tables->tbl[dmuctgr_td_tables->cnt].table_name = td.table_name
  FOOT REPORT
   stat = alterlist(dmuctgr_td_tables->tbl,dmuctgr_td_tables->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dus_load_spec_cols(dmuctgr_dbase_name)=0)
  GO TO exit_script
 ENDIF
 ...
 ENDIF
 SET dum_utc_invalid_tables->cnt = 0
 SET dm_err->eproc = "Loading UTC Triggers into Record Strcuture."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT
  IF (dmuctgr_dbtype="SOURCE")
   FROM user_source@mig_target us,
    user_triggers@mig_target ut
   WHERE us.type="TRIGGER"
    AND us.name="UTC_CONVERT*"
    AND us.name != "UTC_CONVERT_TZ*"
    AND us.name=ut.trigger_name
   ORDER BY ut.table_name, us.line
  ELSE
   FROM user_source us,
    user_triggers ut
   WHERE us.type="TRIGGER"
  ...
  INTO "nl:"
  DETAIL
   dmuctgr_ndx = 0, dmuctgr_ndx = locateval(dmuctgr_ndx,1,dmuctgr_tgr_rec->cnt,ut.table_name,
    dmuctgr_tgr_rec->qual[dmuctgr_ndx].tbl_name)
   IF (dmuctgr_ndx=0)
    dmuctgr_tgr_rec->cnt = (dmuctgr_tgr_rec->cnt+ 1), stat = alterlist(dmuctgr_tgr_rec->qual,
     dmuctgr_tgr_rec->cnt), dmuctgr_tgr_rec->qual[dmuctgr_tgr_rec->cnt].tbl_name = ut.table_name,
    dmuctgr_tgr_rec->qual[dmuctgr_tgr_rec->cnt].tgr_name = ut.trigger_name, dmuctgr_tgr_rec->qual[
    dmuctgr_tgr_rec->cnt].tgr_text = us.text
   ELSE
    dmuctgr_tgr_rec->qual[dmuctgr_ndx].tgr_text = concat(dmuctgr_tgr_rec->qual[dmuctgr_ndx].tgr_text,
     us.text)
   ENDIF
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(dmuctgr_tgr_rec)
 ENDIF
 IF ((dmuctgr_tgr_rec->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Validating UTC_CONVERT_* Triggers on Target database."
  SET dm_err->emsg = "UTC Triggers does not exist on Target."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dmuctgr_mngd_tables = ""
 IF (dmr_load_managed_tables(dmuctgr_mngd_tables)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Loading Tables with Date Columns into Record structure."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT
  IF (dmuctgr_dbtype="SOURCE")
   FROM dba_tab_cols@mig_target utc,
    dba_tables@mig_target t
   WHERE  NOT (expand(dmuctgr_ndx,1,dm2_sch_except->tcnt,utc.table_name,dm2_sch_except->tbl[
    dmuctgr_ndx].tbl_name))
    AND t.table_name != "MVJ*"
    AND t.temporary="N"
    AND t.owner=currdbuser
    AND parser(concat(" t.table_name not in (",dmuctgr_mngd_tables,")"))
  ...
  utc.*
  HEAD REPORT
   dmuctgr_tab_cnt = 0, dmuctgr_tab_col_cnt = 0
  HEAD utc.table_name
   dmuctgr_td_idx = 0, dmuctgr_td_idx = locateval(dmuctgr_td_idx,1,dmuctgr_td_tables->cnt,utc
    .table_name,dmuctgr_td_tables->tbl[dmuctgr_td_idx].table_name)
   IF (dmuctgr_td_idx > 0)
    dmuctgr_tbl_fnd = 0, dmuctgr_tbl_fnd = locateval(dmuctgr_tbl_fnd,1,dus_std_convert_list->tbl_cnt,
     utc.table_name,dus_std_convert_list->tbl[dmuctgr_tbl_fnd].table_name), dmuctgr_tab_cnt = (
    dmuctgr_tab_cnt+ 1),
    stat = alterlist(dmuctgr_table_data->qual,dmuctgr_tab_cnt), dmuctgr_table_data->qual[
    dmuctgr_tab_cnt].tbl_name = utc.table_name
   ENDIF
  DETAIL
   IF (dmuctgr_td_idx > 0)
    dmuctgr_col_fnd = 0, dmuctgr_col_cnvt = 0
    IF (dmuctgr_tbl_fnd > 0)
     dmuctgr_col_fnd = locateval(dmuctgr_col_fnd,1,dus_std_convert_list->tbl[dmuctgr_tbl_fnd].col_cnt,
      utc.column_name,dus_std_convert_list->tbl[dmuctgr_tbl_fnd].col[dmuctgr_col_fnd].column_name)
     IF (dmuctgr_col_fnd > 0
      AND (dus_std_convert_list->tbl[dmuctgr_tbl_fnd].col[dmuctgr_col_fnd].no_convert_ind=1))
      dmuctgr_col_cnvt = 1
     ENDIF
    ENDIF
    IF (dmuctgr_col_cnvt=0)
     dmuctgr_tab_col_cnt = (dmuctgr_tab_col_cnt+ 1), stat = alterlist(dmuctgr_table_data->qual[
      dmuctgr_tab_cnt].col,dmuctgr_tab_col_cnt), dmuctgr_table_data->qual[dmuctgr_tab_cnt].col[
     dmuctgr_tab_col_cnt].col_id = utc.column_id,
     dmuctgr_table_data->qual[dmuctgr_tab_cnt].col[dmuctgr_tab_col_cnt].col_name = utc.column_name
    ENDIF
   ENDIF
  FOOT  utc.table_name
   IF (dmuctgr_td_idx > 0)
    stat = alterlist(dmuctgr_table_data->qual[dmuctgr_tab_cnt].col,dmuctgr_tab_col_cnt),
    dmuctgr_table_data->qual[dmuctgr_tab_cnt].ccnt = dmuctgr_tab_col_cnt, dmuctgr_tab_col_cnt = 0
   ENDIF
  FOOT REPORT
   stat = alterlist(dmuctgr_table_data->qual,dmuctgr_tab_cnt), dmuctgr_table_data->cnt =
   dmuctgr_tab_cnt
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(dmuctgr_table_data)
 ENDIF
 IF ((dmuctgr_table_data->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Validating Tables/Columns on Target database ."
  SET dm_err->emsg = "UTC Triggers does not exist on Target."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Check for the existence of Date column and UTC Column in the Triggers."
 CALL disp_msg(" ",dm_err->logfile,0)
 FOR (dmuctgr_loop_var = 1 TO dmuctgr_table_data->cnt)
   IF ((dmuctgr_table_data->qual[dmuctgr_loop_var].ccnt > 0))
    SET dmuctgr_tbl_fnd = locateval(dmuctgr_tbl_fnd,1,dmuctgr_tgr_rec->cnt,dmuctgr_table_data->qual[
     dmuctgr_loop_var].tbl_name,dmuctgr_tgr_rec->qual[dmuctgr_tbl_fnd].tbl_name)
    IF (dmuctgr_tbl_fnd=0)
     SET dmuctgr_table_data->qual[dmuctgr_loop_var].tgr_not_fnd = 1
     ...
     SET dum_utc_invalid_tables->qual[dum_utc_invalid_tables->cnt].table_name = dmuctgr_table_data->
     qual[dmuctgr_loop_var].tbl_name
    ELSE
     FOR (dmuctgr_col_cnt = 1 TO dmuctgr_table_data->qual[dmuctgr_loop_var].ccnt)
       IF (findstring(dmuctgr_table_data->qual[dmuctgr_loop_var].col[dmuctgr_col_cnt].col_name,
        dmuctgr_tgr_rec->qual[dmuctgr_tbl_fnd].tgr_text,1,0) > 0)
        IF ( NOT (findstring(build("UTC_TMP1_",dmuctgr_table_data->qual[dmuctgr_loop_var].col[
          dmuctgr_col_cnt].col_id),dmuctgr_tgr_rec->qual[dmuctgr_tbl_fnd].tgr_text,1,0) > 0))
         SET dmuctgr_table_data->qual[dmuctgr_loop_var].utc_col_not_fnd = 1
         SET dmuctgr_tab_cnt = 0
         SET dmuctgr_tab_cnt = locateval(dmuctgr_tab_cnt,1,dum_utc_invalid_tables->cnt,
          dmuctgr_table_data->qual[dmuctgr_loop_var].tbl_name,dum_utc_invalid_tables->qual[
          dmuctgr_tab_cnt].table_name)
         IF (dmuctgr_tab_cnt=0)
          SET dum_utc_invalid_tables->cnt = (dum_utc_invalid_tables->cnt+ 1)
          SET stat = alterlist(dum_utc_invalid_tables->qual,dum_utc_invalid_tables->cnt)
          SET dum_utc_invalid_tables->qual[dum_utc_invalid_tables->cnt].table_name =
          dmuctgr_table_data->qual[dmuctgr_loop_var].tbl_name
         ENDIF
        ENDIF
       ELSE
        SET dmuctgr_table_data->qual[dmuctgr_loop_var].orig_col_not_fnd = 1
        SET dmuctgr_tab_cnt = 0
        SET dmuctgr_tab_cnt = locateval(dmuctgr_tab_cnt,1,dum_utc_invalid_tables->cnt,
         dmuctgr_table_data->qual[dmuctgr_loop_var].tbl_name,dum_utc_invalid_tables->qual[
         dmuctgr_tab_cnt].table_name)
    ...
    ENDIF
    IF (dmuctgr_tbl_fnd > 0
     AND (dmuctgr_table_data->qual[dmuctgr_loop_var].orig_col_not_fnd != 1)
     AND (dmuctgr_table_data->qual[dmuctgr_loop_var].utc_col_not_fnd != 1)
     AND (dm_err->debug_flag > 1))
     SET dm_err->eproc = concat("UTC Trigger in sync for table [",trim(dmuctgr_tgr_rec->qual[
       dmuctgr_tbl_fnd].tbl_name),"] and trigger [",trim(dmuctgr_tgr_rec->qual[dmuctgr_tbl_fnd].
       tgr_name),"].")
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
   ENDIF
 ...
 IF ((dum_utc_invalid_tables->cnt > 0))
  IF (get_unique_file("dmuctgr_rpt_file",".rpt")=0)
   GO TO exit_script
  ENDIF
  SET dmuctgr_rpt_file = dm_err->unique_fname
  SELECT INTO value(dmuctgr_rpt_file)
   FROM (dummyt d  WITH d.seq = 1)
   HEAD REPORT
    col 40, "Triggers Out of on Target.", row + 1,
    col 40, "-------------------------", row + 1,
    row + 2, col 2, "Table Name",
    col 35, "Reason"
   DETAIL
    FOR (dmuctgr_locndx = 1 TO dmuctgr_table_data->cnt)
      IF ((dmuctgr_table_data->qual[dmuctgr_locndx].tgr_not_fnd=1))
       row + 2, col 2, dmuctgr_table_data->qual[dmuctgr_locndx].tbl_name,
       col 35, "Trigger Not found"
      ELSEIF ((dmuctgr_table_data->qual[dmuctgr_locndx].orig_col_not_fnd=1))
       row + 2, col 2, dmuctgr_table_data->qual[dmuctgr_locndx].tbl_name,
       col 35, "Date column not found in the Table Trigger"
      ELSEIF ((dmuctgr_table_data->qual[dmuctgr_locndx].utc_col_not_fnd=1))
       row + 2, col 2, dmuctgr_table_data->qual[dmuctgr_locndx].tbl_name,
       col 35, "Date column doesnt have counter part UTC Column in the Table Trigger"
      ENDIF
    ENDFOR
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  IF (((dmuctgr_standalone=1) OR (dmuctgr_dbtype="TARGET")) )
   IF (dm2_disp_file(dmuctgr_rpt_file," ")=0)
    GO TO exit_script
   ENDIF
  ENDIF
 ELSE
  SET dm_err->eproc = "Triggers in Sync on Target."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(dum_utc_invalid_tables)
 ENDIF
 GO TO exit_script
 SUBROUTINE dmuctgr_prep(null)
   IF ( NOT (dmuctgr_dbtype IN ("SOURCE", "TARGET")))
    SET dm_err->eproc = "Validating <db_type> input parameter."
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat(dmuctgr_dbtype,"is invalid. Valid values are 'SOURCE','TARGET.'")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = concat("Querying from ALL_DB_LINKS to see if ",dmuctgr_db_link,
    " dblink exist.")
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM all_db_links adl
    WHERE parser(build(" adl.db_link = '",dmuctgr_db_link,".*'"))
    DETAIL
     dmuctgr_db_link = adl.db_link
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (curqual=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat(dmuctgr_db_link," dblink does not exist. Cannot progress further.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ELSE
    SET dm_err->eproc = concat("Validating ",dmuctgr_db_link," database link")
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM (value(build("V$DATABASE@",dmuctgr_db_link)))
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ELSEIF (curqual=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Error selecting against ",dmuctgr_db_link," - v$database")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
   ENDIF
 ...
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "Ending DM2_MIG_UTC_CMP_TGR"
 CALL final_disp_msg("dm2_mig_utc_cmp_tgr")
END GO
