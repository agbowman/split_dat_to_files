CREATE PROGRAM acm_rdm_newborn_mother_swap_p:dba
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_RDM_NEWBORN_MOTHER_SWAP",concat(
   "create global temporary table TEMP_RDM_NEWBORN_MOTHER_SWAP(encntr_encntr_reltn_id number,",
   "mother_encntr_id number, child_encntr_id number)"," on commit preserve rows"),
  "Creating temporary table")
 CALL sbr_add_sql_stmt("INDEX","TEMPEERINDEX",
  "create unique index TEMPEERINDEX ON TEMP_RDM_NEWBORN_MOTHER_SWAP(encntr_encntr_reltn_id)",
  "Creating index")
