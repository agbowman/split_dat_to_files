CREATE PROGRAM dm_ins_upd_col_doc:dba
 ...
 ENDIF
 DECLARE iucd_logfile_prefix = vc WITH protect, constant("dm_ins_upd_col_doc")
 DECLARE iucd_pkg_num = i4 WITH protect, noconstant(0)
 IF (check_logfile(iucd_logfile_prefix,".log","DM_INS_UPD_COL_DOC LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning Insert/Update maintenance on DM_COLUMN_DOC...."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (validate(dm2_db_options->dm2_toolset_usage,"@")="Y")
  SET dm_err->eproc =
  "Maintenance on DM_COLUMNS_DOC/DM_TS_PRECEDENCE will be performed later in the process...."
  CALL disp_msg(" ",dm_err->logfile,0)
  FREE RECORD ts_prec
  FREE RECORD col_doc
  GO TO exit_script
 ENDIF
 IF ((validate(ocd_number,- (1))=- (1)))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Package Number could not be obtained."
  SET dm_err->eproc = "Retrieving package number."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ELSEIF (ocd_number <= 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Package Number invalid - must be greater then zero."
  SET dm_err->eproc = "Retrieving package number."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ELSE
  SET iucd_pkg_num = ocd_number
 ENDIF
 EXECUTE dm2_chk_col_doc iucd_pkg_num
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 ...
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 IF ( NOT (trim(ts_prec->qual[1].table_name,3) > " "))
  IF ((dm_err->debug_flag > 0))
   SET dm_err->eproc = concat("No maintenance to DM_TS_PRECEDENDE will be initiated - ",
    "ts_prec record structure not valued with data")
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning Insert/Update maintenance on DM_TS_PRECEDENCE...."
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_chk_ts_prec iucd_pkg_num
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 ...
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Insert/Update maintenance on DM_TS_PRECEDENCE Complete"
 CALL disp_msg(" ",dm_err->logfile,0)
 GO TO exit_script
#exit_script
 SET dm_err->eproc = "Insert/Update maintenance on DM_COLUMNS_DOC Complete"
 CALL final_disp_msg(iucd_logfile_prefix)
END GO
