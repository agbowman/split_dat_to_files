CREATE PROGRAM dm_rmc_mover_0455:dba
 ...
 DECLARE cust_column_str = vc
 DECLARE cust_group_str = vc
 DECLARE cust_column_str_pos = i4
 DECLARE cust_group_str_pos = i4
 DECLARE cust_pos1 = i4
 DECLARE cust_pos2 = i4
 DECLARE cust_str1 = vc
 DECLARE cust_new_str = vc
 DECLARE cust_ins_upd = i2
 DECLARE idcd_check = i2
 DECLARE cust_process_tab = vc
 DECLARE cust_rpt_missing = vc
 DECLARE cust_err_val = i4
 DECLARE cust_process_col = vc
 DECLARE cust_done_ind = i4
 DECLARE cust_cv_trans_cnt = i4 WITH noconstant(0)
 DECLARE cust_cv_parse_cnt = i4 WITH noconstant(0)
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_column_str = get_value(cust_tab_name,"COLUMN_STR","FROM")
 SET cust_group_str = get_value(cust_tab_name,"GROUP_BY_STR","FROM")
 SET cust_column_str_pos = get_col_pos(cust_tab_name,"COLUMN_STR")
 SET cust_group_str_pos = get_col_pos(cust_tab_name,"GROUP_BY_STR")
 ...
 IF (cust_column_str > " ")
  ...
  SET cust_pos2 = 0
  SET cust_pos1 = 0
  WHILE (cust_done_ind=0)
   SET cust_pos1 = findstring("omf_get_abstr_desc(",cnvtlower(cust_column_str),(cust_pos2+ 1),0)
   IF (cust_pos1 > 0)
    SET cust_pos2 = findstring(",",cnvtlower(cust_column_str),cust_pos1,0)
    IF (cust_pos2 > 0)
     SET cust_str1 = substring((cust_pos1+ 19),(cust_pos2 - (cust_pos1+ 19)),cust_column_str)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_column_str_pos].translated = 0
     SET cust_to_str = select_merge_translate(cust_str1,"CODE_VALUE")
     SET cust_to_str = concat(cust_to_str,".00")
     IF (isnumeric(cust_to_str))
      SET cust_new_str = replace(cust_column_str,cust_str1,cust_to_str,0)
      CALL put_value(cust_tab_name,"COLUMN_STR",cust_new_str)
     ELSE
      SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_str1))
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET cust_done_ind = 1
     ENDIF
    ELSE
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV56"
     SET cust_done_ind = 1
     GO TO exit_0455
    ENDIF
   ELSE
  ...
  SET cust_pos1 = 0
  WHILE (cust_done_ind=0)
   SET cust_pos1 = findstring("omf_get_abstr_date(",cnvtlower(cust_column_str),(cust_pos2+ 1),0)
   IF (cust_pos1 > 0)
    SET cust_pos2 = findstring(",",cnvtlower(cust_column_str),cust_pos1,0)
    IF (cust_pos2 > 0)
     SET cust_str1 = substring((cust_pos1+ 19),(cust_pos2 - (cust_pos1+ 19)),cust_column_str)
  ...
  SET cust_pos1 = 0
  WHILE (cust_done_ind=0)
   SET cust_pos1 = findstring("omf_get_abstr_cd(",cnvtlower(cust_column_str),(cust_pos2+ 1),0)
   IF (cust_pos1 > 0)
    SET cust_pos2 = findstring(",",cnvtlower(cust_column_str),cust_pos1,0)
    IF (cust_pos2 > 0)
     SET cust_str1 = substring((cust_pos1+ 17),(cust_pos2 - (cust_pos1+ 17)),cust_column_str)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_column_str_pos].translated = 0
     SET cust_to_str = select_merge_translate(cust_str1,"CODE_VALUE")
     SET cust_to_str = concat(cust_to_str,".00")
     IF (isnumeric(cust_to_str))
  ...
  SET cust_pos2 = 0
  SET cust_pos1 = 0
  SET cust_column_str = get_value(cust_tab_name,"COLUMN_STR","TO")
  SET cust_new_str = cust_column_str
  WHILE (cust_done_ind=0)
   SET cust_pos1 = findstring(":cv",cnvtlower(cust_column_str),(cust_pos2+ 1),0)
   IF (cust_pos1 > 0)
    SET cust_pos2 = findstring(":",cnvtlower(cust_column_str),(cust_pos1+ 3),0)
    IF (cust_pos2 > 0)
     SET cust_cv_parse_cnt = (cust_cv_parse_cnt+ 1)
     SET cust_str1 = substring((cust_pos1+ 3),(cust_pos2 - (cust_pos1+ 3)),cust_column_str)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_column_str_pos].translated = 0
     SET cust_to_str = select_merge_translate(cust_str1,"CODE_VALUE")
     IF (isnumeric(cust_to_str))
      SET cust_cv_trans_cnt = (cust_cv_trans_cnt+ 1)
      SET cust_to_str = concat(cust_to_str,".00")
      SET cust_new_str = replace(cust_new_str,cust_str1,cust_to_str,0)
     ELSE
      SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_str1))
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET cust_done_ind = 1
   ...
   ELSE
    SET cust_done_ind = 1
    IF (cust_cv_trans_cnt=cust_cv_parse_cnt)
     CALL put_value(cust_tab_name,"COLUMN_STR",cust_new_str)
    ENDIF
   ENDIF
  ENDWHILE
 ...
 IF (cust_group_str > " ")
  ...
  SET cust_pos1 = 0
  WHILE (cust_done_ind=0)
   SET cust_pos1 = findstring("omf_get_abstr_desc(",cnvtlower(cust_group_str),(cust_pos2+ 1),0)
   IF (cust_pos1 > 0)
    SET cust_pos2 = findstring(")",cnvtlower(cust_group_str),cust_pos1,0)
    IF (cust_pos2 > 0)
     SET cust_str1 = substring((cust_pos1+ 19),(cust_pos2 - (cust_pos1+ 19)),cust_group_str)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_group_str_pos].translated = 0
     SET cust_to_str = select_merge_translate(cust_str1,"CODE_VALUE")
     SET cust_to_str = concat(cust_to_str,".00")
     IF (isnumeric(cust_to_str))
      SET cust_new_str = replace(cust_group_str,cust_str1,cust_to_str,0)
      CALL put_value(cust_tab_name,"GROUP_BY_STR",cust_new_str)
     ELSE
      SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_str1))
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET cust_done_ind = 1
  ...
  SET cust_pos1 = 0
  WHILE (cust_done_ind=0)
   SET cust_pos1 = findstring("omf_get_abstr_date(",cnvtlower(cust_group_str),(cust_pos2+ 1),0)
   IF (cust_pos1 > 0)
    SET cust_pos2 = findstring(",",cnvtlower(cust_group_str),cust_pos1,0)
    IF (cust_pos2 > 0)
     SET cust_str1 = substring((cust_pos1+ 19),(cust_pos2 - (cust_pos1+ 19)),cust_group_str)
  ...
  SET cust_pos1 = 0
  WHILE (cust_done_ind=0)
   SET cust_pos1 = findstring("omf_get_abstr_cd(",cnvtlower(cust_group_str),(cust_pos2+ 1),0)
   IF (cust_pos1 > 0)
    SET cust_pos2 = findstring(",",cnvtlower(cust_group_str),cust_pos1,0)
    IF (cust_pos2 > 0)
     SET cust_str1 = substring((cust_pos1+ 17),(cust_pos2 - (cust_pos1+ 17)),cust_group_str)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_group_str_pos].translated = 0
     SET cust_to_str = select_merge_translate(cust_str1,"CODE_VALUE")
     SET cust_to_str = concat(cust_to_str,".00")
     IF (isnumeric(cust_to_str))
 ...
 SET idcd_check = is_translated(cust_tab_name,"COLUMN_STR")
 IF (idcd_check=1)
  SET idcd_check = 0
  SET idcd_check = is_translated(cust_tab_name,"GROUP_BY_STR")
  IF (idcd_check=1)
   SET idcd_check = 0
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of a GROUP_BY_STR column"
   ENDIF
  ENDIF
 ELSE
  SET cust_err_val = get_err_val(null)
  IF (cust_err_val=0)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of a COLUMN_STR column"
  ENDIF
 ENDIF
#exit_0455
END GO
