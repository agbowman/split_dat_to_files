CREATE PROGRAM bhs_eks_diag_to_hl7_adt:dba
 ...
 SET ms_trans_name = concat(trim(cnvtstring(trigger_personid,20),3),"_",trim(cnvtstring(
    trigger_encntrid,20),3),"_",format(cnvtdatetime(sysdate),"YYYYMMDDHHMMSS;;q"))
 SET ms_file_name = concat("diag_hl7_",trim(cnvtstring(rand(0),20),3),"_",ms_trans_name,".txt")
 SELECT INTO "nl:"
  FROM diagnosis d,
   nomenclature n
 ...
 IF (curqual=0)
  ...
  SET ms_tmp_message = build(ms_tmp_message,
   " Encounter does not have diagnosis associated with ICD9 or ICD10")
  GO TO exit_program
 ENDIF
 SET prsn_info->msh_hl7_slice = build("MSH|^~\&","|CERNER","|CERNER","|ADT DIAG","|PATIENTKEEPER",
  "|",format(cnvtdatetime(sysdate),"YYYYMMDDHHMMSS;;q"),"|","|ADT^A08","|",
 ...
 SELECT INTO "nl:"
  ...
  HEAD sea.alias
   IF (size(trim(sea.alias,3)) > 0)
    prsn_info->sch_cnt += 1, stat = alterlist(prsn_info->sch,prsn_info->sch_cnt), prsn_info->sch[
    prsn_info->sch_cnt].alias = sea.alias
   ENDIF
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   IF (size(trim(d.diagnosis_display,3)) > 0)
    prsn_info->diag[prsn_info->cnt].diag_display_as = trim(d.diagnosis_display,3)
   ELSE
    prsn_info->diag[prsn_info->cnt].diag_display_as = trim(n.source_string,3)
   ENDIF
  WITH nocounter
 ;end select
 IF ((prsn_info->cnt > 0))
  FOR (ml_loop = 1 TO prsn_info->cnt)
    SET prsn_info->diag[ml_loop].diag_hl7_slice = build("DG1","|",ml_loop,"|",prsn_info->diag[ml_loop
     ...
     ml_loop].diag_date,
     "|",prsn_info->diag[ml_loop].diag_type,"|",prsn_info->diag[ml_loop].diag_display_as,"|",
     "|","|","|","|","|",
