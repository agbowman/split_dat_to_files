CREATE PROGRAM dms_updt_delete_que_xref:dba
 CALL echo("<=================== Entering DMS_UPDT_DELETE_QUE_XREF Script ===================>")
 ...
 SELECT DISTINCT INTO "nl:"
  FROM dms_media_xref d,
   dms_media_identifier dm
  PLAN (d
   WHERE d.parent_entity_name="PERSON"
    AND (((d.parent_entity_id=request->person_id)) OR (d.parent_entity_name="ENCOUNTER"
    AND d.parent_entity_id IN (
   (SELECT
    e.encntr_id
    FROM encounter e
    WHERE (e.person_id=request->person_id))))) )
   JOIN (dm
   WHERE d.dms_media_identifier_id=dm.dms_media_identifier_id)
  DETAIL
   countmedia += 1, stat = alterlist(mediaobjectids->qual,countmedia), mediaobjectids->qual[
   countmedia].media_object_identifier = dm.media_object_identifier
  WITH nocounter
 ;end select
