CREATE PROGRAM bhs_athn_med_order_ls_v2
 ...
 SELECT DISTINCT INTO  $1
  ...
  ENDIF
  , o.prn_ind,
  order_cki = trim(replace(replace(replace(replace(replace(o.cki,"&","&amp;",0),"<","&lt;",0),">",
      "&gt;",0),"'","&apos;",0),'"',"&quot;",0),3), med_order_type_meaning = trim(replace(replace(
     replace(replace(replace(uar_get_code_meaning(o.med_order_type_cd),"&","&amp;",0),"<","&lt;",0),
      ">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3), o.med_order_type_cd,
  ...
  .display_description, op_proposal_status_disp = uar_get_code_display(op.proposal_status_cd),
  oc.dc_display_days, oc.stop_duration, oc.stop_duration_unit_cd,
  stop_duration_unit = uar_get_code_display(oc.stop_duration_unit_cd), t.reference_task_id, od1
  .action_sequence,
  dx_string = trim(replace(replace(replace(replace(replace(order_list->qual[d1.seq].dx_string,"&",
        "&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3), oc
  .disable_order_comment_ind, o_protocol_order_id = trim(replace(cnvtstring(o.protocol_order_id),
    ".0*","",0),3),
  oc.ref_text_mask
  FROM (dummyt d1  WITH seq = size(order_list->qual,5)),
   orders o,
  ...
  HEAD o.order_id
   ...
   row + 1, v1091 = build("<DisableOrderCommentFlag>",cnvtint(oc.disable_order_comment_ind),
    "</DisableOrderCommentFlag>"), col + 1,
   v1091, row + 1, v1092 = build("<ProtocolOrderId>",o_protocol_order_id,"</ProtocolOrderId>"),
   col + 1, v1092, row + 1,
   v1093 = build("<RefTextMask>",
   ...
   v16, row + 1, col + 1,
   v17, row + 1, foot_grp = build("</","StandardMedOrder",">"),
