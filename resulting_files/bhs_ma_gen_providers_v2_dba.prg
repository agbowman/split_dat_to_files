CREATE PROGRAM bhs_ma_gen_providers_v2:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (epr
   WHERE epr.encntr_id=eid
    AND epr.encntr_prsnl_r_cd IN (attendoc, consultdoc)
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY sa.end_effective_dt_tm DESC, p
  HEAD sa.end_effective_dt_tm
   IF (sa.active_ind=1
    AND sa.purge_ind=0
    AND p="BHS RN")
    providers->rn = trim(pr.name_full_formatted)
   ENDIF
  WITH format(date,";;q")
 ;end select
 SET x = 1
