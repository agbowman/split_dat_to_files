CREATE PROGRAM bb_rpt_qc_reagent_rels:dba
 RECORD reply(
   1 file_name = vc
   1 node = vc
   1 status_data
 ...
 RECORD params(
   1 qual[*]
     2 relationship_name = vc
     2 reagent_name = vc
     2 active_ind = i2
     2 details[*]
       3 control_material = vc
       3 testing_phase = vc
       3 enhancement_media = vc
       3 active_ind = i2
       3 expected_results[*]
         4 result_name = vc
 )
 ...
 DECLARE ni18nhandle = i4 WITH protect, noconstant(0)
 ...
 DECLARE stwentyfiveline = vc WITH protect, noconstant("")
 ...
 SET nstatus = uar_i18nlocalizationinit(ni18nhandle,curprog,"",curcclrev)
 RECORD captions(
   1 stitle = vc
   1 susername = vc
   1 sdomain = vc
 ...
 SET captions->stitle = uar_i18ngetmessage(ni18nhandle,"TITLE",
  "PathNet Blood Bank: QC Reagent Relationships Report")
 SET captions->susername = uar_i18ngetmessage(ni18nhandle,"NAME","Name:")
 SET captions->sdomain = uar_i18ngetmessage(ni18nhandle,"DOMAIN","Domain:")
 SET captions->sdate = uar_i18ngetmessage(ni18nhandle,"DATE","Date:")
 SET captions->stime = uar_i18ngetmessage(ni18nhandle,"TIME","Time:")
 SET captions->spage = uar_i18ngetmessage(ni18nhandle,"PAGE","Page:")
 SET captions->sactive = uar_i18ngetmessage(ni18nhandle,"ACTIVE","Active")
 SET captions->srelationship = uar_i18ngetmessage(ni18nhandle,"RELATIONSHIP","Relationship: ")
 SET captions->sreagent = uar_i18ngetmessage(ni18nhandle,"REAGENT","Reagent:      ")
 SET captions->scontrolmaterial = uar_i18ngetmessage(ni18nhandle,"CONTROL","Control Material")
 SET captions->stestingphase = uar_i18ngetmessage(ni18nhandle,"PHASE","Testing Phase")
 SET captions->senhancementmedia = uar_i18ngetmessage(ni18nhandle,"ENHANCEMENT","Enhancement Media")
 SET captions->sexpectedresults = uar_i18ngetmessage(ni18nhandle,"RESULTS","Expected Results")
 SET captions->sendofreport = uar_i18ngetmessage(ni18nhandle,"ENDOFREPORT",
  "* * * End of Report * * *")
 SELECT INTO "nl:"
  sreagentcode = uar_get_code_display(bbqcrr.reagent_cd), scontrolcode = uar_get_code_display(bbqcrrd
   .control_cd), sphasecode = uar_get_code_display(bbqcrrd.phase_cd),
  senhancementcode = uar_get_code_display(bbqcrrd.enhancement_cd)
  FROM bb_qc_rel_reagent bbqcrr,
   bb_qc_rel_reagent_detail bbqcrrd,
   bb_qc_expected_result_r bbqcerr,
   nomenclature n
  PLAN (bbqcrr
   WHERE (((request->related_reagent_id=0)) OR ((bbqcrr.related_reagent_id=request->
   related_reagent_id))) )
   JOIN (bbqcrrd
   WHERE bbqcrrd.related_reagent_id=bbqcrr.related_reagent_id
    AND bbqcrrd.active_ind=1
    AND bbqcrrd.related_reagent_detail_id=bbqcrrd.prev_related_reagent_detail_id)
   JOIN (bbqcerr
   WHERE bbqcerr.related_reagent_detail_id=bbqcrrd.related_reagent_detail_id
    AND bbqcerr.active_ind=1
    AND bbqcerr.expected_result_id=bbqcerr.prev_expected_result_id)
   JOIN (n
   WHERE n.nomenclature_id=bbqcerr.nomenclature_id)
  ORDER BY bbqcrr.related_reagent_name_key, scontrolcode, sphasecode,
   senhancementcode, n.mnemonic, bbqcrr.related_reagent_id,
   bbqcrrd.related_reagent_detail_id
  HEAD REPORT
   ncount = 0
  HEAD bbqcrr.related_reagent_id
   ncount += 1
   IF (ncount > size(params->qual,5))
    nstatus = alterlist(params->qual,(ncount+ 9))
   ENDIF
   params->qual[ncount].relationship_name = bbqcrr.related_reagent_name, params->qual[ncount].
   reagent_name = sreagentcode, params->qual[ncount].active_ind = bbqcrr.active_ind,
   ncount2 = 0
  HEAD bbqcrrd.related_reagent_detail_id
   ncount2 += 1
   IF (ncount2 > size(params->qual[ncount].details,5))
    nstatus = alterlist(params->qual[ncount].details,(ncount2+ 9))
   ENDIF
   params->qual[ncount].details[ncount2].control_material = scontrolcode, params->qual[ncount].
   details[ncount2].testing_phase = sphasecode, params->qual[ncount].details[ncount2].
   enhancement_media = senhancementcode,
   params->qual[ncount].details[ncount2].active_ind = bbqcrrd.active_ind, ncount3 = 0
  DETAIL
   ncount3 += 1
   IF (ncount3 > size(params->qual[ncount].details[ncount2].expected_results,5))
    nstatus = alterlist(params->qual[ncount].details[ncount2].expected_results,(ncount3+ 9))
   ENDIF
   params->qual[ncount].details[ncount2].expected_results[ncount3].result_name = n.mnemonic
  FOOT  bbqcrrd.related_reagent_detail_id
   nstatus = alterlist(params->qual[ncount].details[ncount2].expected_results,ncount3)
  FOOT  bbqcrr.related_reagent_id
   nstatus = alterlist(params->qual[ncount].details,ncount2)
  FOOT REPORT
   nstatus = alterlist(params->qual,ncount)
  WITH nocounter
 ;end select
 IF (error(serror,0) > 0)
  CALL subevent_add("REPORT","F","bb_rpt_qc_reagent_rels",serror)
  GO TO exit_script
 ENDIF
 EXECUTE cpm_create_file_name "bb_qcrr", "txt"
 IF ((cpm_cfn_info->status_data.status != "S"))
  CALL subevent_add("REPORT","F","bb_rpt_qc_reagent_rels","Failed to create a file name")
  GO TO exit_script
 ENDIF
 IF (size(params->qual,5)=0)
  SELECT INTO cpm_cfn_info->file_name_path
   ...
   PLAN (d)
   HEAD REPORT
    stwentyfiveline = fillstring(25,"-"), sfortythreeline = fillstring(43,"-"), sequalsline =
    fillstring(124,"="),
    sdashesline = fillstring(124,"-")
   DETAIL
    row 1,
    CALL center(captions->stitle,1,130), row + 1,
    col 1, captions->susername, col 9,
    request->username, col 111, captions->sdate,
    col 117, curdate"@SHORTDATE", row + 1,
    col 1, captions->sdomain, col 9,
    request->domain, col 111, captions->stime,
    col 117, curtime3"@TIMEWITHSECONDS"
   FOOT PAGE
  ...
  SELECT INTO cpm_cfn_info->file_name_path
   ...
   HEAD PAGE
    IF (((d.seq < value(size(params->qual,5))) OR (((nbreak=1) OR (value(size(params->qual,5))=1))
     ...
     IF (((value(size(params->qual,5))=1
      ...
      col 1, captions->sdomain, col 9,
      request->domain, col 111, captions->stime,
      col 117, curtime3"@TIMEWITHSECONDS", nfirsttime = 0
      IF (nbreak=1)
       row + 2
      ENDIF
     ENDIF
   ...
   DETAIL
    ...
    ELSE
     nbreak = 0, row + 2
    ENDIF
    col 1, captions->srelationship, col 15,
    params->qual[d.seq].relationship_name, row + 1, col 1,
    captions->sreagent, col 15, params->qual[d.seq].reagent_name,
    row + 2, col 1, captions->scontrolmaterial,
    col 28, captions->stestingphase, col 55,
    captions->senhancementmedia, col 82, captions->sexpectedresults,
    row + 1, col 1, stwentyfiveline,
    col 28, stwentyfiveline, col 55,
    stwentyfiveline, col 82, sfortythreeline,
    row + 1
    FOR (i = 1 TO size(params->qual[d.seq].details,5))
      IF ((row >= (nlinesperpage - 3)))
       nbreak = 1, BREAK
      ELSE
       nbreak = 0
      ENDIF
      col 1, params->qual[d.seq].details[i].control_material, col 28,
      params->qual[d.seq].details[i].testing_phase, col 55, params->qual[d.seq].details[i].
      enhancement_media,
      ssegment = ""
      FOR (j = 1 TO size(params->qual[d.seq].details[i].expected_results,5))
        IF (j > 1)
         ssegment = build(ssegment,",",params->qual[d.seq].details[i].expected_results[j].result_name
          )
        ELSE
         ssegment = params->qual[d.seq].details[i].expected_results[j].result_name
        ENDIF
      ENDFOR
      tblobin = " ", tblobin = trim(ssegment),
      CALL rtf_to_text(trim(tblobin),1,nlinelength)
      FOR (z = 1 TO size(tmptext->qual,5))
        col 82, tmptext->qual[z].text, row + 1
        IF ((row >= (nlinesperpage - 3)))
         nbreak = 1, BREAK
        ELSE
         nbreak = 0
