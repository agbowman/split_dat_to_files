CREATE PROGRAM ams_add_ref_txt
 PROMPT
  ...
  HEAD r.line
   ...
   IF (size(trim(line1),1) > 0)
    ...
    IF (count > 1)
     ...
     ENDIF
     temp->qual[row_count].text_type_cd = piece(line1,",",1,"Not Found"), temp->qual[row_count].text
      = piece(line1,",",2,"Not Found"), temp->qual[row_count].comments = piece(line1,",",3,
      "Not Found"),
     temp->qual[row_count].facility = piece(line1,",",4,"Not Found"), temp->qual[row_count].
     order_name = piece(line1,",",5,"Not Found")
    ENDIF
   ENDIF
  FOOT REPORT
 ...
 FOR (i = 1 TO value(size(temp->qual,5)))
   ...
   SET request->text_type_list[rcnt].variation_list[rcnt].version_list[rcnt].end_dt_tm = cnvtdatetime
   ("12/30/2100 23:59:59")
   SET request->text_type_list[rcnt].variation_list[rcnt].version_list[rcnt].auto_invoke_prep_ind = 0
   ...
   SET rcnt = 0
 ENDFOR
 SET last_mod = " 000 03/25/2016 KP035208   Initial Release"
END GO
