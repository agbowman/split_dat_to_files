CREATE PROGRAM dm2_show_plan_control
 ...
 END ;Subroutine
 DECLARE dspc_logfile_prefix = vc WITH protect, constant("dm2_show_plan_ctrl")
 DECLARE dspc_node = vc WITH protect, noconstant("")
 DECLARE dspc_num = i4 WITH protect, noconstant(0)
 DECLARE dspc_ndx = i4 WITH protect, noconstant(0)
 DECLARE dspc_plan_id = f8 WITH protect, noconstant(0.0)
 DECLARE dspc_dp_id = f8 WITH protect, noconstant(0.0)
 DECLARE dspc_dpe_id = f8 WITH protect, noconstant(0.0)
 DECLARE dspc_dped_id = f8 WITH protect, noconstant(0.0)
 DECLARE dspc_mode = vc WITH protect, noconstant("")
 DECLARE dspc_filter = vc WITH protect, noconstant("")
 DECLARE dspc_script = vc WITH protect, noconstant("")
 DECLARE dspc_str = vc WITH protect, noconstant("")
 DECLARE dspc_pos = i4 WITH protect, noconstant(0)
 DECLARE dspc_run_nbr = i4 WITH protect, noconstant(0)
 IF ((validate(dspc_dped->cnt,- (1))=- (1))
  AND (validate(dspc_dped->cnt,- (2))=- (2)))
  FREE RECORD dspc_dped
  RECORD dspc_dped(
    1 cnt = i4
    1 qual[*]
      2 dped_id = f8
 ...
 ENDIF
 IF ((validate(dspc_data->cnt,- (1))=- (1))
  AND (validate(dspc_data->cnt,- (2))=- (2)))
  FREE RECORD dspc_data
  RECORD dspc_data(
    1 cnt = i4
    1 plan_id = f8
    1 plan_dt_tm = dq8
 ...
 ENDIF
 DECLARE dspc_display_report(ddr_script=vc,ddr_mode=vc) = i2
 DECLARE dspc_get_curr_cclgrants(null) = i2
 DECLARE dspc_get_curr_baselines(null) = i2
 SET width = 132
 IF (check_logfile(dspc_logfile_prefix,".log","dm2_show_plan_control")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_show_plan_control"
 CALL disp_msg("",dm_err->logfile,0)
 SET dspc_node = trim(curnode)
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Node: ",cnvtupper(dspc_node)))
 ENDIF
 SET dm_err->eproc = "Validating input parameters"
 SET dspc_mode = cnvtupper( $1)
 IF ( NOT (dspc_mode IN ("SCRIPT", "PLAN")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Invalid first (mode) parameter - ",dspc_mode,
   ". It should be set to 'SCRIPT' or 'PLAN'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dspc_filter = cnvtupper( $2)
 IF ( NOT (dspc_filter IN ("RECENT", "ALL")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Invalid second (filter) parameter - ",dspc_filter,
   ". It should be set to 'RECENT' or 'ALL'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dspc_script = cnvtupper(trim( $3))
 SET dspc_plan_id = cnvtreal( $4)
 SET dm_err->eproc = concat("Retrive list of ids from dm_process_event")
 CALL disp_msg("",dm_err->logfile,0)
 IF (dspc_mode="SCRIPT")
  SELECT
   IF (dspc_filter="RECENT")
    WHERE dped.dm_process_event_id IN (
    (SELECT
     ...
     AND dped.detail_type=dpl_script
     AND trim(dped.detail_text)=patstring(dspc_script)
    ORDER BY dped.updt_dt_tm DESC
    WITH nocounter, maxrec = 1
   ELSEIF (dspc_filter="ALL")
    WHERE dped.dm_process_event_id IN (
    (SELECT
     dpe.dm_process_event_id
     FROM dm_process_event dpe
   ...
   DETAIL
    dspc_dped->cnt = (dspc_dped->cnt+ 1), stat = alterlist(dspc_dped->qual,dspc_dped->cnt), dspc_dped
    ->qual[dspc_dped->cnt].dped_id = dped.dm_process_event_id
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   ...
   GO TO exit_program
  ENDIF
 ELSEIF (dspc_mode="PLAN")
  IF (dspc_filter="RECENT")
   SET dm_err->eproc = concat("Retrieve run number for most recent execution of current plan id")
   SELECT INTO "nl:"
    FROM dm_process_event dpe
    WHERE dpe.dm_process_id IN (
    (SELECT
     ...
     FROM dm_process dp
     WHERE dp.process_name=dpl_plan_control))
     AND dpe.install_plan_id=dspc_plan_id
    HEAD REPORT
     dspc_run_nbr = 0
    DETAIL
     dspc_pos = findstring("|",trim(dpe.message_txt),13,0), dspc_str = substring((dspc_pos+ 1),(
      findstring("|",trim(dpe.message_txt),(dspc_pos+ 1),0) - (dspc_pos+ 1)),trim(dpe.message_txt))
     IF (cnvtint(dspc_str) > dspc_run_nbr)
      dspc_run_nbr = cnvtint(dspc_str)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
  ...
  SELECT
   IF (dspc_filter="RECENT")
    ...
    (SELECT
     ...
     WHERE dp.process_name=dpl_plan_control))
     AND dpe.install_plan_id=dspc_plan_id
     AND dpe.message_txt=patstring(concat("RUN_IDENT: |",trim(cnvtstring(dspc_plan_id)),"|",trim(
       cnvtstring(dspc_run_nbr)),"*"))
    ORDER BY dpe.updt_dt_tm DESC
   ELSEIF (dspc_filter="ALL")
    WHERE dpe.dm_process_id IN (
    (SELECT
     dp.dm_process_id
   ...
   DETAIL
    dspc_dped->cnt = (dspc_dped->cnt+ 1), stat = alterlist(dspc_dped->qual,dspc_dped->cnt), dspc_dped
    ->qual[dspc_dped->cnt].dped_id = dpe.dm_process_event_id,
    dspc_data->plan_id = dpe.install_plan_id
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dspc_dped)
 ENDIF
 IF ((dspc_dped->cnt > 0))
  SELECT INTO "nl:"
   FROM dm_process_event dpe,
    dm_process_event_dtl dped,
    (dummyt d  WITH seq = value(dspc_dped->cnt))
   PLAN (d
    WHERE d.seq > 0)
    JOIN (dpe
    WHERE (dpe.dm_process_event_id=dspc_dped->qual[d.seq].dped_id))
    JOIN (dped
    WHERE dped.dm_process_event_id=dpe.dm_process_event_id)
   ORDER BY dped.dm_process_event_id
   HEAD dped.dm_process_event_id
    dspc_data->cnt = (dspc_data->cnt+ 1), stat = alterlist(dspc_data->qual,dspc_data->cnt), dspc_data
    ->qual[dspc_data->cnt].plan_id = dpe.install_plan_id,
    dspc_data->qual[dspc_data->cnt].plan_dt_tm = cnvtdatetime(dpe.begin_dt_tm)
   DETAIL
    CASE (dped.detail_type)
     OF dpl_script:
      dspc_data->qual[dspc_data->cnt].script_name = dped.detail_text,dspc_data->qual[dspc_data->cnt].
      b_s_name = dped.detail_text
     OF dpl_grant_exists:
      dspc_data->qual[dspc_data->cnt].g_grant_exists_ind = evaluate(dped.detail_text,"YES",1,0)
     OF dpl_bl_exists:
      dspc_data->qual[dspc_data->cnt].b_baseline_exists_ind = evaluate(dped.detail_text,"YES",1,0)
     OF dpl_grant_str:
      dspc_data->qual[dspc_data->cnt].g_outstring = dped.detail_text
     OF dpl_grant_cmd:
      dspc_data->qual[dspc_data->cnt].g_grant_cmd_full = dped.detail_text
     OF dpl_bl_query_nbr:
      dspc_data->qual[dspc_data->cnt].b_q_pos = dped.detail_text
     OF dpl_bl_sql_handle:
      dspc_data->qual[dspc_data->cnt].b_sql_handle = dped.detail_text
     OF dpl_bl_sql_text:
      dspc_data->qual[dspc_data->cnt].b_sql_text = substring(1,500,dped.detail_text)
     OF dpl_bl_creator:
      dspc_data->qual[dspc_data->cnt].b_creator = dped.detail_text
     OF dpl_bl_desc:
      dspc_data->qual[dspc_data->cnt].b_description = dped.detail_text
     OF dpl_bl_enabled:
      dspc_data->qual[dspc_data->cnt].b_enabled = dped.detail_text
     OF dpl_bl_accepted:
      dspc_data->qual[dspc_data->cnt].b_accepted = dped.detail_text
     OF dpl_bl_plan_name:
      dspc_data->qual[dspc_data->cnt].b_plan_name = dped.detail_text
     OF dpl_bl_created:
      dspc_data->qual[dspc_data->cnt].b_created = dped.detail_text
     OF dpl_bl_last_mod:
      dspc_data->qual[dspc_data->cnt].b_last_modified = dped.detail_text
     OF dpl_bl_last_exec:
      dspc_data->qual[dspc_data->cnt].b_last_executed = dped.detail_text
    ENDCASE
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dspc_data)
 ENDIF
 IF (dspc_display_report(dspc_mode,dspc_filter)=0)
  RETURN(0)
 ENDIF
 GO TO exit_program
 SUBROUTINE dspc_display_report(ddr_mode,ddr_filter)
   DECLARE ddr_rpt_file = vc WITH protect, noconstant("")
   DECLARE ddr_cnt = i4 WITH protect, noconstant(0)
   DECLARE ddr_cnt2 = i4 WITH protect, noconstant(0)
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("MODE: ",ddr_mode))
    CALL echo(concat("FILTER: ",ddr_filter))
   ENDIF
   SET dm_err->eproc = "Generating Plan Control Summary Report"
   CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
   IF (get_unique_file("dm2_plan_ctrl_rpt",".dat")=0)
    GO TO exit_program
   ELSE
    SET ddr_rpt_file = dm_err->unique_fname
   ENDIF
   SELECT INTO value(ddr_rpt_file)
    FROM dummyt d
    HEAD REPORT
     CALL print(fillstring(250,"-")), row + 1, col 0,
     CALL print("PLAN CONTROL REPORT [SUMMARY REPORT]"), col 80,
     CALL print(build("REPORT DATE/TIME:",format(cnvtdatetime(curdate,curtime3),
       "DD-MMM-YYYY HH:MM:SS;;D"))),
     row + 1,
     CALL print(build("File Name: CCLUSERDIR:",ddr_rpt_file)), row + 1,
     CALL print(fillstring(250,"-")), row + 1
    DETAIL
     col 2, "REPORT MODE:  ", col 30,
     CALL print(ddr_mode), row + 1
     IF (ddr_mode="SCRIPT")
      col 2, "FILTER CRITERIA:  ", col 30,
      CALL print(ddr_filter), row + 1
     ENDIF
     IF (ddr_mode="PLAN")
      col 2, "PLAN ID:  ", col 30,
      CALL print(build(dspc_data->plan_id)), row + 1
     ENDIF
     CALL print(fillstring(250,"-")), row + 2, ddr_cnt = 0
     IF ((dspc_data->cnt=0))
      col 2,
      CALL print("No data was found matching the input criteria"), row + 1
     ELSE
      FOR (ddr_cnt = 1 TO dspc_data->cnt)
        col 2,
        CALL print(concat(build(dspc_data->qual[ddr_cnt].script_name)," [PLAN ID: ",build(cnvtint(
           dspc_data->qual[ddr_cnt].plan_id)),"  PLAN DT/TM: ",format(cnvtdatetime(dspc_data->qual[
           ddr_cnt].plan_dt_tm),"DD-MMM-YYYY HH:MM:SS;;D"),
         "]")), row + 2,
        col 10, "ATTRIBUTE ", col 40,
        "VALUE", row + 1, col 10,
        CALL print(fillstring(20,"-")), col 40,
        CALL print(fillstring(20,"-")),
        row + 1, col 10,
        CALL print(dpl_grant_exists),
        col 40,
        CALL print(evaluate(dspc_data->qual[ddr_cnt].g_grant_exists_ind,1,"YES","NO")), row + 1,
        col 10,
        CALL print(dpl_bl_exists), col 40,
        CALL print(evaluate(dspc_data->qual[ddr_cnt].b_baseline_exists_ind,1,"YES","NO")), row + 1
        IF ((dspc_data->qual[ddr_cnt].g_grant_exists_ind=1))
         col 10,
         CALL print(dpl_grant_str), col 40,
         CALL print(dspc_data->qual[ddr_cnt].g_outstring), row + 1, col 10,
         CALL print(dpl_grant_cmd), col 40,
         CALL print(dspc_data->qual[ddr_cnt].g_grant_cmd_full),
         row + 1
        ENDIF
        IF ((dspc_data->qual[ddr_cnt].b_baseline_exists_ind=1))
         col 10,
         CALL print(dpl_bl_query_nbr), col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_q_pos), row + 1, col 10,
         CALL print(dpl_bl_sql_handle), col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_sql_handle),
         row + 1, col 10,
         CALL print(dpl_bl_sql_text),
         col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_sql_text), row + 1,
         col 10,
         CALL print(dpl_bl_creator), col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_creator), row + 1, col 10,
         CALL print(dpl_bl_desc), col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_description),
         row + 1, col 10,
         CALL print(dpl_bl_enabled),
         col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_enabled), row + 1,
         col 10,
         CALL print(dpl_bl_accepted), col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_accepted), row + 1, col 10,
         CALL print(dpl_bl_plan_name), col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_plan_name),
         row + 1, col 10,
         CALL print(dpl_bl_created),
         col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_created), row + 1,
         col 10,
         CALL print(dpl_bl_last_mod), col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_last_modified), row + 1, col 10,
         CALL print(dpl_bl_last_exec), col 40,
         CALL print(dspc_data->qual[ddr_cnt].b_last_executed),
         row + 1
        ENDIF
   ...
   IF (check_error(dm_err->eproc))
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Displaying Plan Control Summary"
   IF (dm2_disp_file(ddr_rpt_file,"Plan Control Summary Report")=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "Ending dm2_show_plan_control"
 CALL final_disp_msg(dspc_logfile_prefix)
END GO
