CREATE PROGRAM bed_aud_hla_work_rtg:dba
 ...
 DECLARE hla_cd = f8 WITH protect, noconstant(0.0)
  ...
  SELECT INTO "nl:"
   ...
   PLAN (oc
    WHERE oc.catalog_type_cd=genlab
     AND oc.activity_type_cd=hla_cd
     AND oc.active_ind=1)
   DETAIL
 ...
 FOR (x = 1 TO temp->o_cnt)
   ...
   ELSE
    FOR (y = 1 TO temp->olist[x].r_cnt)
      ...
      SET reply->rowlist[row_nbr].celllist[5].string_value = cnvtstring(temp->olist[x].rlist[y].
       sequence)
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("hla_work_rtg_audit.csv")
 ENDIF
