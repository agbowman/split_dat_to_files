CREATE PROGRAM dynamic_label_witness_report
 ...
 SET req->output_device = trim(request->output_device,3)
 SET req->script_name = request->script_name
 IF (size(request->person,5)=1)
  SET req->person_id = request->person[1].person_id
 ENDIF
 SET req->scope_flag = 0
 IF ((request->visit_cnt=1))
  SET stat = alterlist(req->encntr_list,1)
  SET req->encntr_list[1].encntr_id = request->visit[1].encntr_id
 ENDIF
 SET req->param_list[1].param_name = request->nv[1].pvc_name
 SET req->param_list[1].param_value = request->nv[1].pvc_value
 SET req->param_list[2].param_name = request->nv[2].pvc_name
 SET req->param_list[2].param_value = request->nv[2].pvc_value
 ...
 IF (bglobaldebugflag=1)
  CALL echo(build("output_device->",req->output_device))
  CALL echo(build("script_name->",req->script_name))
  CALL echo(build("person_id->",req->person_id))
  CALL echo(build("scope_flag->",req->scope_flag))
  CALL echo(build("beg_dt_tm->",format(req->beg_dt_tm,";;q")))
  CALL echo(build("end_dt_tm->",format(req->end_dt_tm,";;q")))
 ENDIF
