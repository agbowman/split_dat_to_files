CREATE PROGRAM dm_cs_interface_charge_rows:dba
 DECLARE dm_cs_interface_charge_rows = vc WITH private, noconstant("206564.FT.002")
 ...
 SET reply->table_name = "INTERFACE_CHARGE"
  ...
  WHILE (curmindate <= maxdate
   AND rowsleft > 0)
    SELECT INTO "nl:"
     ...
     WHERE ic.updt_dt_tm >= cnvtdatetime(curmindate)
      AND ic.updt_dt_tm <= cnvtdatetime(curmaxdate)
      AND parser(sbr_getrowidnotexists("ic.interface_charge_id+0 > 0","ic"))
      AND ((ic.process_flg+ 0) != 0)
     DETAIL
      v_rows = (v_rows+ 1)
      IF (mod(v_rows,10)=1)
       stat = alterlist(reply->rows,(v_rows+ 9))
      ENDIF
      reply->rows[v_rows].row_id = ic.rowid
     WITH nocounter, maxqual(ic,value(rowsleft))
    ;end select
 ...
 SUBROUTINE sbr_get_min_date(null)
   ...
   DECLARE mindate = dq8
   SELECT INTO "nl:"
    seqval = min(ic.updt_dt_tm)
    FROM interface_charge ic
    WHERE ic.interface_charge_id=0.0
    DETAIL
     zerorowdate = seqval
    WITH nocounter
   ;end select
   ...
   UPDATE  FROM interface_charge ic
    SET ic.updt_dt_tm = cnvtdatetime(curdate,curtime3)
    WHERE ic.interface_charge_id=0.0
    WITH nocounter
   ;end update
   SET v_err_code2 = error(v_errmsg2,0)
   IF (v_err_code2 != 0)
    ...
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"UPDATEERROR1",
     "Failed to set ic.updt_dt_tm to curdate:%1","s",nullterm(v_errmsg2))
    ROLLBACK
    GO TO exit_script
   ELSE
    COMMIT
   ...
   ENDIF
   UPDATE  FROM interface_charge ic
    SET ic.updt_dt_tm = cnvtdatetime(zerorowdate)
    WHERE ic.interface_charge_id=0.0
    WITH nocounter
   ;end update
   SET v_err_code2 = error(v_errmsg2,0)
   IF (v_err_code2 != 0)
    ...
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"UPDATEERROR2",
     "Failed to set ic.updt_dt_tm to original date:%1","s",nullterm(v_errmsg2))
    ROLLBACK
    GO TO exit_script
   ELSE
    COMMIT
