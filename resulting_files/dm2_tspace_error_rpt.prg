CREATE PROGRAM dm2_tspace_error_rpt
 DECLARE dter_inst_type_in = vc WITH public, noconstant("")
 DECLARE dter_inst_val_in = vc WITH public, noconstant("")
 DECLARE dter_perform_check = i2 WITH public, noconstant(0)
 DECLARE dter_ts_size_exists = i2 WITH public, noconstant(0)
 DECLARE dter_run = i2 WITH public, noconstant(0)
 DECLARE dter_string = vc WITH public, noconstant("")
 DECLARE dter_rows_returned = i2 WITH public, noconstant(0)
 DECLARE dter_rpt_file = vc WITH private, noconstant("")
 DECLARE dter_err_msg = vc WITH private, noconstant("")
 ...
 SUBROUTINE dtr_load_clin_tspaces(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_tspace_err",".log","DM2_TSPACE_ERROR_RPT.PRG ...") != 1)
  GO TO exit_program
 ENDIF
 SET dter_inst_type_in = cnvtupper( $1)
 SET dter_inst_val_in =  $2
 IF (validate(drrr_responsefile_in_use,0)=1)
  IF (get_unique_file("dm2_tspace_errors",".rpt")=0)
   GO TO exit_program
  ENDIF
  SET dter_rpt_file = dm_err->unique_fname
  SET dter_rpt_file = build(drrr_misc_data->active_dir,dter_rpt_file)
  SET dm_err->eproc = "Generating report of errors encountered creating tablespaces."
  SELECT INTO value(cnvtlower(dter_rpt_file))
   FROM (dummyt d  WITH seq = size(rtspace->qual,5))
   PLAN (d
    WHERE (rtspace->qual[d.seq].ct_err_ind=1))
   HEAD REPORT
    CALL center("Create Tablespaces Error Report",1,132), row + 2, col 1,
    "Errors occurred when attempting to create one or more tablespaces.", row + 2, col 1,
    "Tablespace Name", col 33, "Error Message",
    dter_string = fillstring(132,"-"), row + 1, col 1,
    dter_string, row + 1
   DETAIL
    col 1, rtspace->qual[d.seq].tspace_name
    IF ((rtspace->qual[d.seq].ct_err_msg=""))
     col 33, "No Errors Reported"
    ELSE
     dter_err_msg = substring(1,250,rtspace->qual[d.seq].ct_err_msg)
     IF ((dter_err_msg=rtspace->qual[d.seq].ct_err_msg))
      col 33, dter_err_msg
     ELSE
      col 33, dter_err_msg, "..."
     ENDIF
    ENDIF
    row + 1, dter_rows_returned = 1
   FOOT REPORT
    IF (dter_rows_returned=0)
     col 1, "No errors have occurred in DM2_CREATE_TSPACE. All tablespaces created successfully.",
     row + 1
    ENDIF
   WITH nocounter, maxcol = 500, nullreport
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = concat("Create Tablespaces Error Report can be viewed at ",trim(dter_rpt_file,3
    ),".")
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF ( NOT (dter_inst_type_in IN ("PACKAGE", "SCHEMA_DATE", "MANUAL")))
  SET dm_err->emsg = "Invalid Install Type."
  SET dm_err->eproc = "Validate Install Type Parameter."
  SET dm_err->user_action = "Valid Install Types are PACKAGE or SCHEMA_DATE."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (checkdic("DM2_TSPACE_SIZE","T",0)=0)
  SET dm_err->emsg = "DM2_TSPACE_SIZE definition does not exist."
  SET dm_err->eproc = "Check DM2_TSPACE_SIZE ccl definition"
  SET dm_err->user_action = "No action needed."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ELSE
  SELECT INTO "nl:"
   FROM dm2_user_tab_columns u
   WHERE u.table_name="DM2_TSPACE_SIZE1"
   WITH nocounter
  ;end select
  IF (curqual=0)
   SET dm_err->emsg = "DM2_TSPACE_SIZE table does not exist."
   SET dm_err->eproc = "Check DM2_TSPACE_SIZE table existance"
   SET dm_err->user_action = "No action needed."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  ELSE
   SET dter_ts_size_exists = 1
  ENDIF
 ENDIF
 IF (dter_ts_size_exists=0)
  IF (size(rtspace->qual,5) > 0)
   SET dter_run = 1
  ENDIF
 ENDIF
 SET dm_err->eproc = "Validate information source"
 IF (dter_ts_size_exists=0)
  SET dm_err->emsg = " Required DM2_TSPACE_SIZE table and/or ccl definition is missing."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (dter_run=1)
  SELECT INTO mine
   ...
   PLAN (d
    WHERE (rtspace->qual[d.seq].ct_err_ind=1))
   HEAD REPORT
    CALL center("Errors Adding/Extending Tablespaces",1,132), row + 2, dter_string = concat(
     "Errors occurred with attempting to add/extend one or more tablespaces.",
     "Tablespaces with errors and the error messages"),
    col 1, dter_string, row + 1,
    dter_string = concat("received are shown below. After viewing this report you will ",
     "be able to work on these tablespaces again in dm2_tspace_menu."), col 1, dter_string,
    row + 2, col 1, "Tablespace Name",
    col 33, "Error Message", dter_string = fillstring(132,"-"),
    row + 1, col 1, dter_string,
    row + 1
   DETAIL
    col 1, rtspace->qual[d.seq].tspace_name
    IF ((rtspace->qual[d.seq].ct_err_msg=""))
     col 33, "No Errors Reported"
    ELSE
     col 33, rtspace->qual[d.seq].ct_err_msg
    ENDIF
    row + 1, dter_rows_returned = 1
   FOOT REPORT
    IF (dter_rows_returned=0)
  ...
  ;end select
 ELSEIF (dter_ts_size_exists=1)
  SELECT INTO mine
   FROM dm2_tspace_size d
   WHERE d.install_type=dter_inst_type_in
    AND d.install_type_value=dter_inst_val_in
    AND d.install_status="ERROR"
   HEAD REPORT
    CALL center("Errors Adding/Extending Tablespaces",1,132), row + 2, dter_string = concat(
     "Errors occurred when attempting to add/extend one or more tablespaces.",
     "Tablespaces with errors and the error messages"),
    col 1, dter_string, row + 1,
    dter_string = concat("received are shown below. After viewing this report you will ",
     "be able to work on these tablespaces again in dm2_tspace_menu."), col 1, dter_string,
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "Ending DM2_TSPACE_ERROR_RPT"
 CALL final_disp_msg(" ")
END GO
