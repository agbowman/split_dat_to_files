CREATE PROGRAM bed_del_dmart_filters_by_cat:dba
 ...
 SUBROUTINE delreportfilterreltns(null)
   CALL bedlogmessage("delReportFilterReltns","Entering ...")
   DELETE  FROM br_datamart_report_filter_r b,
    ...
    PLAN (d)
     ...
     (SELECT
      b1.br_datamart_filter_id
      FROM br_datamart_filter b1
      WHERE (b1.br_datamart_category_id=request->views[d.seq].br_datamart_category_id))))
    WITH nocounter
   ;end delete
   CALL bedlogmessage("delReportFilterReltns","Exiting ...")
 END ;Subroutine
 SUBROUTINE delfilterdefaults(null)
   CALL bedlogmessage("delFilterDefaults","Entering ...")
   DELETE  FROM br_datamart_default_detail b,
     (dummyt d  WITH seq = value(req_size))
    SET b.seq = 1
    PLAN (d)
     JOIN (b
     WHERE b.br_datamart_default_id IN (
     (SELECT
      b1.br_datamart_default_id
      FROM br_datamart_default b1
      WHERE b1.br_datamart_filter_id IN (
      (SELECT
       b2.br_datamart_filter_id
       FROM br_datamart_filter b2
       WHERE (b2.br_datamart_category_id=request->views[d.seq].br_datamart_category_id))))))
    WITH nocounter
   ;end delete
   ...
   CALL bedlogmessage("delFilterDefaults","Exiting ...")
 END ;Subroutine
 SUBROUTINE delsavedvalues(null)
   CALL bedlogmessage("delSavedValues","Entering ...")
   ...
   CALL bedlogmessage("delSavedValues","Exiting ...")
 END ;Subroutine
 SUBROUTINE delfiltertext(null)
   CALL bedlogmessage("delFilterText","Entering ...")
   ...
   CALL bedlogmessage("delFilterText","Exiting ...")
 END ;Subroutine
 SUBROUTINE delfilterdetails(null)
   CALL bedlogmessage("delFilterDetails","Entering ...")
   DELETE  FROM br_datamart_filter_detail b,
    ...
    WITH nocounter
   ;end delete
   CALL bederrorcheck("Error deleting from br_datamart_filter_detail table")
   CALL bedlogmessage("delFilterDetails","Exiting ...")
 END ;Subroutine
