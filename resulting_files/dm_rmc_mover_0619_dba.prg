CREATE PROGRAM dm_rmc_mover_0619:dba
 ...
 SUBROUTINE drcm_get_pk_str(dgps_rec,dgps_tab_name)
   ...
   RETURN(dgps_ret_str)
 END ;Subroutine
 DECLARE substitute_all(in_code=vc,origin=vc) = i2 WITH public
 DECLARE check_cki(sbr_cki=vc) = i2
 DECLARE check_substitute_val(cv_eval_return=f8) = i2
 ...
 DECLARE rpt_from = f8
 DECLARE orphan_ind = i2
 DECLARE chg_log_smry_name = vc
 DECLARE drdm_dmt_tab = vc
 DECLARE space_size = i4
 DECLARE cv_imt_check = i2
 DECLARE smt_ret_val = vc
 DECLARE cv_idcd_skip = i2
 DECLARE new_seq_ind = i2
 DECLARE cv_eval_return = f8
 DECLARE orig_dup_string = vc
 DECLARE cv_query_cnt = i4
 DECLARE cv_cur_dt_tm = f8
 DECLARE cv_effective_ind = i2
 DECLARE cv_done_ind = i2
 DECLARE cv_valid_effective_ind = i2
 DECLARE cki_num = i4
 DECLARE drdm_table_suffix = vc
 DECLARE code_vc = vc
 DECLARE cv_sub_val = vc
 DECLARE temp_substr = vc
 DECLARE cs_num = i4
 DECLARE cv_parent_drdm_row = i2 WITH protect, noconstant(0)
 DECLARE cv_nomv06_msg = vc WITH protect, noconstant("")
 DECLARE cv_prev_ctxt = vc WITH protect, noconstant("")
 SET current_merges = 0
 SET fail_merges = 0
 SET global_mover_rec->ccl_stmt = " "
 SET perm_cs_cnt = size(dm2_ref_data_doc->cs_qual,5)
 SET perm_col_cnt = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt
 SET cv_cur_dt_tm = cnvtdatetime(curdate,curtime3)
 SET drdm_table_suffix = "t0619"
 ...
 FOR (zero_rec = 1 TO perm_col_cnt)
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[zero_rec].check_null = 0
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[zero_rec].translated = 0
 ENDFOR
 ...
 SELECT INTO "NL:"
  ...
  DETAIL
   drdm_col_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, passed_var
    = concat("var",cnvtstring(d.seq))
   IF (drdm_parser_cnt > 2)
    drdm_parser->statement[drdm_parser_cnt].frag = " , ", drdm_parser_cnt = (drdm_parser_cnt+ 1)
   ENDIF
   drdm_parser->statement[drdm_parser_cnt].frag = concat(passed_var," = nullind(",drdm_table_suffix,
    ".",drdm_col_name,
 ...
 SELECT INTO "NL:"
  ...
  DETAIL
   drdm_col_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[d.seq].column_name, passed_var
    = concat("var",cnvtstring(d.seq)), drdm_parser_cnt = (drdm_parser_cnt+ 1),
   drdm_parser->statement[drdm_parser_cnt].frag = concat(
    " dm2_ref_Data_doc->tbl_qual[temp_tbl_cnt].col_qual[",trim(cnvtstring(d.seq)),"].check_null = ",
    passed_var), drdm_parser_cnt = (drdm_parser_cnt+ 1), drdm_parser->statement[drdm_parser_cnt].frag
    = concat(" RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->from_values.",drdm_col_name,
    " = ",
    drdm_table_suffix,".",drdm_col_name),
   drdm_parser_cnt = (drdm_parser_cnt+ 1), drdm_parser->statement[drdm_parser_cnt].frag = concat(
    " dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[",trim(cnvtstring(d.seq)),"].check_space = 0"
    )
  WITH nocounter
 ;end select
 SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
 ...
 IF (curqual=0)
  ...
  SET dm_err->emsg = concat("A CODE_VALUE row ",trim(drdm_chg->log[drdm_log_loop].pk_where),
   " wasn't found in the source.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->emsg = concat("A ",drdm_chg->log[drdm_log_loop].table_name," row ",trim(drdm_chg->log[
    drdm_log_loop].pk_where)," was deleted from the dm_chg_log.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_0619
 ENDIF
 IF ((rs_0619->from_values.begin_effective_dt_tm <= cv_cur_dt_tm)
  AND (rs_0619->from_values.end_effective_dt_tm > cv_cur_dt_tm))
  SET cv_effective_ind = 1
 ENDIF
 SET cv_valid_effective_ind = 1
 SET col_num = locateval(index_var,1,perm_col_cnt,"END_EFFECTIVE_DT_TM",dm2_ref_data_doc->tbl_qual[
  temp_tbl_cnt].col_qual[index_var].column_name)
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null=1))
  SET cv_valid_effective_ind = 0
 ENDIF
 SET col_num = locateval(index_var,1,perm_col_cnt,"BEGIN_EFFECTIVE_DT_TM",dm2_ref_data_doc->tbl_qual[
  temp_tbl_cnt].col_qual[index_var].column_name)
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null=1))
  SET cv_valid_effective_ind = 0
 ENDIF
 FOR (col_num = 1 TO perm_col_cnt)
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].data_type IN ("VC", "C*")))
   CALL parser(concat("set space_size = size(RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
     "->from_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].column_name,") go"),
    1)
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_null=0)
    AND space_size=0)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_space = 1
   ELSE
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_space = 0
   ENDIF
  ELSE
   SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].check_space = 0
  ENDIF
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].idcd_ind=0))
   CALL parser(concat("set rs_0619->to_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     col_num].column_name," = rs_0619->from_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
     col_qual[col_num].column_name," go"),1)
  ENDIF
 ENDFOR
 CALL echo("")
 CALL echo("")
 ...
 CALL echo("")
 CALL echo("")
 SET cki_num = locateval(index_var,1,perm_col_cnt,"CKI",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
  col_qual[index_var].column_name)
 SET col_num = locateval(index_var,1,perm_col_cnt,"UPDT_ID",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
  col_qual[index_var].column_name)
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
 SET rs_0619->to_values.updt_id = 0
 SET current_merges = (current_merges+ 1)
 SET child_merge_audit->num[current_merges].action = "CONSTANT"
 SET child_merge_audit->num[current_merges].text = "CODE_VALUE  UPDT_ID"
 SET col_num = locateval(index_var,1,perm_col_cnt,"ACTIVE_STATUS_PRSNL_ID",dm2_ref_data_doc->
  tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
 SET rs_0619->to_values.active_status_prsnl_id = 0
 SET current_merges = (current_merges+ 1)
 SET child_merge_audit->num[current_merges].action = "CONSTANT"
 SET child_merge_audit->num[current_merges].text = "CODE_VALUE  ACITVE_STATUS_PRSNL_ID"
 SET col_num = locateval(index_var,1,perm_col_cnt,"DATA_STATUS_PRSNL_ID",dm2_ref_data_doc->tbl_qual[
  temp_tbl_cnt].col_qual[index_var].column_name)
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
 SET rs_0619->to_values.data_status_prsnl_id = 0
 SET current_merges = (current_merges+ 1)
 SET child_merge_audit->num[current_merges].action = "CONSTANT"
 SET child_merge_audit->num[current_merges].text = "CODE_VALUE  DATA_STATUS_PRSNL_ID"
 SET rs_0619->log_id = drdm_chg->log[drdm_log_loop].log_id
 SET rs_0619->from_table_name = "CODE_VALUE"
 SET rs_0619->to_table_name = "CODE_VALUE"
 SET no_insert_update = 0
 IF ((rs_0619->from_values.active_type_cd != 0))
  SET found_rec = 0
  SET smt_ret_val = ""
  SET smt_ret_val = select_merge_translate(cnvtstring(rs_0619->from_values.active_type_cd,20,1),
   "CODE_VALUE")
  IF (isnumeric(smt_ret_val))
   SET rs_0619->to_values.active_type_cd = cnvtreal(smt_ret_val)
   SET found_rec = 1
  ENDIF
 ELSE
  SET found_rec = 1
  SET rs_0619->to_values.active_type_cd = 0
 ENDIF
 IF (found_rec=1)
  SET col_num = locateval(index_var,1,perm_col_cnt,"ACTIVE_TYPE_CD",dm2_ref_data_doc->tbl_qual[
   temp_tbl_cnt].col_qual[index_var].column_name)
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
  SET current_merges = (current_merges+ 1)
  SET child_merge_audit->num[current_merges].action = "LOOKUPS"
  SET child_merge_audit->num[current_merges].text = "CODE_VALUE  ACTIVE_TYPE_CD"
 ENDIF
 SET found_rec = 0
 IF ((rs_0619->from_values.data_status_cd != 0))
  SET smt_ret_val = ""
  SET smt_ret_val = select_merge_translate(cnvtstring(rs_0619->from_values.data_status_cd,20,1),
   "CODE_VALUE")
  IF (isnumeric(smt_ret_val))
   SET rs_0619->to_values.data_status_cd = cnvtreal(smt_ret_val)
   SET found_rec = 1
  ENDIF
 ELSE
  SET found_rec = 1
  SET rs_0619->to_values.data_status_cd = 0
 ENDIF
 IF (found_rec=1)
  SET col_num = locateval(index_var,1,perm_col_cnt,"DATA_STATUS_CD",dm2_ref_data_doc->tbl_qual[
   temp_tbl_cnt].col_qual[index_var].column_name)
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
  SET current_merges = (current_merges+ 1)
  SET child_merge_audit->num[current_merges].action = "LOOKUPS"
  SET child_merge_audit->num[current_merges].text = "CODE_VALUE  DATA_STATUS_CD"
 ENDIF
 SET found_rec = 0
 SET col_num = locateval(index_var,1,perm_col_cnt,"CODE_VALUE",dm2_ref_data_doc->tbl_qual[
  temp_tbl_cnt].col_qual[index_var].column_name)
 SET smt_ret_val = ""
 SET cv_prev_ctxt = select_merge_translate_rec->from_opt_ctxt
 SET select_merge_translate_rec->from_opt_ctxt = ""
 SET smt_ret_val = select_merge_translate(cnvtstring(rs_0619->from_values.code_value,20,1),
  "CODE_VALUE")
 SET select_merge_translate_rec->from_opt_ctxt = cv_prev_ctxt
 IF (smt_ret_val="NOMV21")
  ...
  GO TO exit_0619
 ELSEIF (isnumeric(smt_ret_val))
  IF ((global_mover_rec->cbc_ind=1))
   IF (add_xlat_ctxt_r("CODE_VALUE",rs_0619->from_values.code_value,cnvtreal(smt_ret_val),drdm_chg->
    log[drdm_log_loop].context_name)=1)
    SET fail_merges = (fail_merges+ 1)
    SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
    SET fail_merge_audit->num[fail_merges].text = dm_err->emsg
    CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges].
     text,1)
    IF (drdm_error_out_ind=1)
     ROLLBACK
    ENDIF
    SET drdm_mini_loop_status = "NOMV99"
    CALL orphan_child_tab(dm2_ref_data_doc->tbl_qual[lt_temp_tbl_cnt].table_name,"NOMV99","",0.0)
    SET lt_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
    SET drdm_chg->log[lt_parent_drdm_row].chg_log_reason_txt = concat(
     "An errored occurred while inserting data to DM_REFCHG_XLAT_CTXT_R: ",dm_err->emsg)
   ENDIF
  ENDIF
  SET rs_0619->to_values.code_value = cnvtreal(smt_ret_val)
  SET found_rec = 1
 ENDIF
 IF (found_rec=1)
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
  SET current_merges = (current_merges+ 1)
  SET child_merge_audit->num[current_merges].action = "LOOKUPS"
  SET child_merge_audit->num[current_merges].text = "CODE_VALUE  CODE_VALUE"
 ENDIF
 CALL echo(build("Post Lookup = ",curmem))
 SET cs_num = locateval(index_var,1,size(dm2_ref_data_doc->cs_qual,5),rs_0619->from_values.code_set,
  dm2_ref_data_doc->cs_qual[index_var].code_set)
 IF (cs_num=0)
  SET code_vc = trim(cnvtstring(rs_0619->from_values.code_set,20,0))
  SET temp_cs_cnt = fill_rs("CODE",code_vc)
  IF (temp_cs_cnt=0)
   SET temp_cs_cnt = (size(dm2_ref_data_doc->cs_qual,5)+ 1)
   SET stat = alterlist(dm2_ref_data_doc->cs_qual,temp_cs_cnt)
   SET dm2_ref_data_doc->cs_qual[temp_cs_cnt].code_set = rs_0619->from_values.code_set
   SET dm2_ref_data_doc->cs_qual[temp_cs_cnt].display_dup_ind = 1
   SET dm2_ref_data_doc->cs_qual[temp_cs_cnt].display_key_dup_ind = 1
   SET dm2_ref_data_doc->cs_qual[temp_cs_cnt].cdf_meaning_dup_ind = 1
   SET dm2_ref_data_doc->cs_qual[temp_cs_cnt].merge_ui_query_ni = 1
  ENDIF
 ENDIF
 IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].merge_ui_query > " "))
  SET temp_substr = substring(1,6,dm2_ref_data_doc->cs_qual[temp_cs_cnt].merge_ui_query)
  IF (((temp_substr="ALWAYS") OR (temp_substr="DUPIND")) )
   SET cv_sub_val = trim(cnvtstring(rs_0619->from_values.code_value,20,1))
   SET cv_eval_return = substitute_all(cv_sub_val,"VALUE")
   SET dm_err->eproc = "Translate Value"
   CALL check_substitute_val(cv_eval_return)
  ENDIF
 ENDIF
 IF (drdm_error_out_ind=1)
  GO TO exit_0619
 ENDIF
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated=0))
  DECLARE code_set_loop = i2
  DECLARE trans_val_loop = i4
  DECLARE code_vc = vc
  DECLARE value_status = f8
  DECLARE value_hold = vc
  DECLARE code_value_cnt = i4
  DECLARE disp_key_num = i4
  DECLARE disp_num = i4
  DECLARE cdf_mean_num = i4
  DECLARE active_ind_num = i4
  DECLARE index_var = i4
  DECLARE dyn_w_clause = vc
  DECLARE found_val = i2
  DECLARE drdm_table_name = vc
  DECLARE cki_val = vc
  DECLARE active_ind_val = i2
  DECLARE display_val = vc
  DECLARE dipslay_key_val = vc
  DECLARE cdf_meaning_val = vc
  SET found_val = 0
  ...
  CALL echo("")
  CALL echo("")
  CALL echo("**********************TRANSLATE VALUE**********************")
  CALL echo("")
  CALL echo("")
  FOR (trans_val_loop = 1 TO dm2_ref_data_doc->cs_cnt)
    IF ((dm2_ref_data_doc->cs_qual[trans_val_loop].code_set=rs_0619->from_values.code_set))
     SET temp_cs_cnt = trans_val_loop
    ENDIF
  ENDFOR
  SET no_code_set = 0
  IF (temp_cs_cnt=0)
   ...
   IF (temp_cs_cnt=0)
    ...
    SET dm2_ref_data_doc->cs_qual[temp_cs_cnt].merge_ui_query_ni = 1
   ENDIF
   SET dm_err->eproc = "Translating the Code Value"
  ENDIF
  IF (no_insert_update != 1)
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cki_num].check_null=0)
    AND (rs_0619->from_values.cki != "")
    AND (rs_0619->from_values.cki != " "))
    SET cki_valid_ind = check_cki(rs_0619->from_values.cki)
    IF (cki_valid_ind=1)
     SET found_val = 0
     SET cki_cnt = 0
     ...
     SELECT INTO "NL:"
      FROM code_value c
      WHERE (c.cki=rs_0619->from_values.cki)
      DETAIL
       cki_cnt = (cki_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,cki_cnt), ui_query_eval_rec->
       qual[cki_cnt].root_entity_attr = c.code_value
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     ENDIF
     IF (cki_cnt=1)
      SET value_status = evaluate_exec_ui_query(cki_cnt,temp_tbl_cnt,perm_col_cnt)
      IF (value_status > 0)
       SET found_val = 1
       SET cv_imt_check = insert_merge_translate(cnvtreal(rs_0619->from_values.code_value),
        value_status,"CODE_VALUE")
       IF (drmm_ddl_rollback_ind=1)
        GO TO exit_0619
       ENDIF
       IF (cv_imt_check=1)
        IF (drdm_error_out_ind=1)
         ROLLBACK
         GO TO exit_0619
        ELSEIF ((global_mover_rec->one_pass_ind=0))
         COMMIT
        ENDIF
        GO TO exit_0619
       ENDIF
       SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
       CALL rdds_del_except("CODE_VALUE",cnvtreal(rs_0619->from_values.code_value))
       SET rs_0619->to_values.code_value = value_status
       SET code_vc = concat("CODE VALUE  ",cnvtstring(rs_0619->from_values.code_value,20,1))
       SET current_merges = (current_merges+ 1)
       SET child_merge_audit->num[current_merges].action = "CKIEXIST"
       SET child_merge_audit->num[current_merges].text = code_vc
      ELSE
       SET value_status = 0
      ENDIF
     ELSE
      SET value_status = 0
     ENDIF
    ELSEIF ((cki_valid_ind=- (1)))
     SET no_insert_update = 1
     SET fail_merges = (fail_merges+ 1)
     SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
     SET fail_merge_audit->num[fail_merges].text = "No CKI pattern match data was found."
     CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges].
      text,1)
     GO TO exit_0619
    ENDIF
   ENDIF
   IF (found_val=0)
    IF ((((dm2_ref_data_doc->cs_qual[temp_cs_cnt].merge_ui_query_ni=1)) OR (temp_substr="DUPIND")) )
     SET update_error_check_ind = 0
     IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].display_dup_ind=0)
      AND (dm2_ref_data_doc->cs_qual[temp_cs_cnt].active_ind_dup_ind=0)
      AND (dm2_ref_data_doc->cs_qual[temp_cs_cnt].cdf_meaning_dup_ind=0)
      AND (dm2_ref_data_doc->cs_qual[temp_cs_cnt].display_key_dup_ind=0)
      AND (dm2_ref_data_doc->cs_qual[temp_cs_cnt].definition_dup_ind=0))
      SET dm2_ref_data_doc->cs_qual[temp_cs_cnt].display_dup_ind = 1
      SET dm2_ref_data_doc->cs_qual[temp_cs_cnt].display_key_dup_ind = 1
      SET dm2_ref_data_doc->cs_qual[temp_cs_cnt].cdf_meaning_dup_ind = 1
      CALL echo("Using Default Dup Indicators")
     ENDIF
     IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].display_dup_ind=1))
      SET disp_num = locateval(index_var,1,perm_col_cnt,"DISPLAY",dm2_ref_data_doc->tbl_qual[
       temp_tbl_cnt].col_qual[index_var].column_name)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[disp_num].check_null=0))
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[disp_num].check_space=1))
        SET dyn_w_clause = " cv.display = ' ' and "
        SET cv_nomv06_msg = "DISPLAY = ' ' AND "
       ELSE
        SET dyn_w_clause = concat(" cv.display = rs_0619->to_values.display and ")
        SET cv_nomv06_msg = concat("DISPLAY = ",rs_0619->to_values.display," AND ")
       ENDIF
      ELSE
       SET insert_update_reason = "DISPLAY DUP IS NULL"
       SET no_insert_update = 1
       SET fail_merges = (fail_merges+ 1)
       SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
       ...
       ENDIF
       SET drdm_mini_loop_status = "NOMV07"
       SET drdm_no_trans_ind = 1
       SET cv_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
       SET drdm_chg->log[cv_parent_drdm_row].chg_log_reason_txt = concat(
        "The DISPLAY column on the CODE_VALUE table for CODE_VALUE ",trim(cnvtstring(rs_0619->
          from_values.code_value,20,1))," and CODE_SET ",trim(cnvtstring(dm2_ref_data_doc->cs_qual[
          temp_cs_cnt].code_set,20,0))," is NULL in SOURCE.")
       CALL orphan_child_tab("CODE_VALUE","NOMV07","CODE_VALUE",rs_0619->from_values.code_value)
       GO TO exit_0619
      ENDIF
     ENDIF
     IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].cdf_meaning_dup_ind=1))
      SET cdf_mean_num = locateval(index_var,1,perm_col_cnt,"CDF_MEANING",dm2_ref_data_doc->tbl_qual[
       temp_tbl_cnt].col_qual[index_var].column_name)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cdf_mean_num].check_null=0))
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cdf_mean_num].check_space=1))
        SET dyn_w_clause = concat(dyn_w_clause,' cv.cdf_meaning = " " and ')
        SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
        SET cv_nomv06_msg = concat(cv_nomv06_msg,"CDF_MEANING = ' ' AND ")
       ELSE
        SET dyn_w_clause = concat(dyn_w_clause,
         " cv.cdf_meaning = rs_0619->to_values.cdf_meaning and ")
        SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
        SET cv_nomv06_msg = concat(cv_nomv06_msg,"CDF_MEANING = ",rs_0619->to_values.cdf_meaning,
         " AND ")
       ENDIF
      ELSE
       SET dyn_w_clause = concat(dyn_w_clause," cv.cdf_meaning = NULL"," and ")
       SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
      ENDIF
     ENDIF
     IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].display_key_dup_ind=1))
      SET disp_key_num = locateval(index_var,1,perm_col_cnt,"DISPLAY_KEY",dm2_ref_data_doc->tbl_qual[
       temp_tbl_cnt].col_qual[index_var].column_name)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[disp_key_num].check_null=0))
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[disp_key_num].check_space=1))
        SET dyn_w_clause = concat(dyn_w_clause,' cv.display_key = " " and ')
        SET cv_nomv06_msg = concat(cv_nomv06_msg,"DISPLAY_KEY = ' ' AND ")
       ELSE
        SET dyn_w_clause = concat(dyn_w_clause,
         " cv.display_key = rs_0619->to_values.display_key and ")
        SET cv_nomv06_msg = concat(cv_nomv06_msg,"DISPLAY_KEY = ",rs_0619->to_values.display_key,
         " AND ")
       ENDIF
       SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
      ELSE
       IF (trim(cnvtupper(cnvtalphanum(rs_0619->from_values.display)))="")
        IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].display_dup_ind=0))
         SET disp_num = locateval(index_var,1,perm_col_cnt,"DISPLAY",dm2_ref_data_doc->tbl_qual[
          temp_tbl_cnt].col_qual[index_var].column_name)
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[disp_num].check_null=0))
          IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[disp_num].check_space=1))
           SET dyn_w_clause = concat(dyn_w_clause," cv.display = ' ' and ")
           SET cv_nomv06_msg = concat(cv_nomv06_msg,"DISPLAY = ' ' AND ")
          ELSE
           CALL echo("vh015225: Inside else clause.")
           SET dyn_w_clause = concat(dyn_w_clause," cv.display = rs_0619->from_values.display and ")
           SET cv_nomv06_msg = concat(cv_nomv06_msg,"DISPLAY = ",rs_0619->to_values.display," AND ")
          ENDIF
         ELSE
          SET insert_update_reason = "DISPLAY DUP IS NULL"
          SET no_insert_update = 1
          ...
          SET cv_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
          SET drdm_chg->log[cv_parent_drdm_row].chg_log_reason_txt = concat(
           "The DISPLAY_KEY column on the CODE_VALUE table for CODE_VALUE ",trim(cnvtstring(rs_0619->
             from_values.code_value,20,1))," and CODE_SET ",trim(cnvtstring(dm2_ref_data_doc->
             cs_qual[temp_cs_cnt].code_set,20,0))," is NULL in SOURCE.")
          CALL orphan_child_tab("CODE_VALUE","NOMV07","CODE_VALUE",rs_0619->from_values.code_value)
          GO TO exit_0619
         ENDIF
        ENDIF
       ELSE
        SET insert_update_reason = "DISPLAY_KEY DUP IS NULL"
        SET no_insert_update = 1
        SET fail_merges = (fail_merges+ 1)
        SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
        SET fail_merge_audit->num[fail_merges].text = insert_update_reason
      ...
      ENDIF
     ENDIF
     IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].active_ind_dup_ind=1))
      SET active_ind_num = locateval(index_var,1,perm_col_cnt,"ACTIVE_IND",dm2_ref_data_doc->
       tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[active_ind_num].check_null=0))
       SET dyn_w_clause = concat(dyn_w_clause," cv.active_ind = rs_0619->to_values.active_ind and ")
       SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
       SET cv_nomv06_msg = concat(cv_nomv06_msg,"ACTIVE_IND = ",trim(cnvtstring(rs_0619->to_values.
          active_ind))," AND ")
      ELSE
       SET insert_update_reason = "ACTIVE_IND DUP IS NULL"
       SET no_insert_update = 1
       SET fail_merges = (fail_merges+ 1)
       SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
       SET fail_merge_audit->num[fail_merges].text = insert_update_reason
       ...
       SET cv_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
       SET drdm_chg->log[cv_parent_drdm_row].chg_log_reason_txt = concat(
        "The ACTIVE_IND column on the CODE_VALUE table for CODE_VALUE ",trim(cnvtstring(rs_0619->
          from_values.code_value,20,1))," and CODE_SET ",trim(cnvtstring(dm2_ref_data_doc->cs_qual[
          temp_cs_cnt].code_set,20,0))," is NULL in SOURCE.")
       CALL orphan_child_tab("CODE_VALUE","NOMV07","CODE_VALUE",rs_0619->from_values.code_value)
       GO TO exit_0619
      ENDIF
     ENDIF
     IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].definition_dup_ind=1))
      SET disp_num = locateval(index_var,1,perm_col_cnt,"DEFINITION",dm2_ref_data_doc->tbl_qual[
       temp_tbl_cnt].col_qual[index_var].column_name)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[disp_num].check_null=0))
       IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[disp_num].check_space=1))
        SET dyn_w_clause = concat(dyn_w_clause,' cv.definition = " " and ')
        SET cv_nomv06_msg = concat(cv_nomv06_msg,"DEFINITION = ' ' AND ")
       ELSE
        SET dyn_w_clause = concat(dyn_w_clause," cv.definition = rs_0619->to_values.definition and ")
        SET cv_nomv06_msg = concat(cv_nomv06_msg,"DEFINITION = ",rs_0619->to_values.definition,
         " AND ")
       ENDIF
      ELSE
       SET insert_update_reason = "DEFINITION DUP IS NULL"
       SET no_insert_update = 1
       SET fail_merges = (fail_merges+ 1)
       SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
       SET fail_merge_audit->num[fail_merges].text = insert_update_reason
       ...
       SET cv_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
       SET drdm_chg->log[cv_parent_drdm_row].chg_log_reason_txt = concat(
        "The DEFINITION column on the CODE_VALUE table for CODE_VALUE ",trim(cnvtstring(rs_0619->
          from_values.code_value,20,1))," and CODE_SET ",trim(cnvtstring(dm2_ref_data_doc->cs_qual[
          temp_cs_cnt].code_set,20,0))," is NULL in SOURCE.")
       CALL orphan_child_tab("CODE_VALUE","NOMV07","CODE_VALUE",rs_0619->from_values.code_value)
       GO TO exit_0619
      ENDIF
     ENDIF
     SET dyn_w_clause = concat(dyn_w_clause," cv.code_set = ",trim(cnvtstring(rs_0619->to_values.
        code_set,20,0)))
     SET cv_nomv06_msg = concat(cv_nomv06_msg,"CODE_SET = ",trim(cnvtstring(rs_0619->to_values.
        code_set,20,0)))
     SET orig_dup_string = dyn_w_clause
     IF (no_insert_update=0)
      SET cv_query_cnt = 1
      IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].active_ind_dup_ind=1))
       SET cv_query_cnt = (cv_query_cnt+ 1)
      ENDIF
      WHILE (cv_done_ind=0)
        SET code_value_cnt = 0
        SET stat = alterlist(ui_query_eval_rec->qual,0)
        SELECT INTO "NL:"
         FROM code_value cv
         WHERE parser(dyn_w_clause)
         DETAIL
          code_value_cnt = (code_value_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,
           code_value_cnt), ui_query_eval_rec->qual[code_value_cnt].root_entity_attr = cv.code_value
         WITH nocounter
        ;end select
        IF (check_error(dm_err->eproc)=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         ...
         SET dm_err->err_ind = 0
         SET no_insert_update = 1
         SET cv_eval_return = - (1)
        ELSE
         SET cv_eval_return = evaluate_exec_ui_query(code_value_cnt,temp_tbl_cnt,perm_col_cnt)
        ENDIF
        IF ((((cv_eval_return=- (3))) OR (cv_eval_return >= 0)) )
         SET cv_done_ind = 1
        ELSE
         SET cv_query_cnt = (cv_query_cnt+ 1)
         IF (cv_query_cnt=2)
          SET dyn_w_clause = concat(orig_dup_string,
           " and cv.active_ind = rs_0619->to_values.active_ind ")
         ELSEIF (cv_query_cnt=3)
          IF (cv_valid_effective_ind=1)
           IF (cv_effective_ind=1)
            SET dyn_w_clause = concat(orig_dup_string,
             " and cv.begin_effective_dt_tm <= cnvtdatetime(cv_cur_dt_Tm) and cv.end_effective_dt_tm > ",
             " cnvtdatetime(cv_cur_dt_tm) ")
           ELSE
            SET dyn_w_clause = concat(orig_dup_string,
             " and (cv.begin_effective_dt_tm > cnvtdatetime(cv_cur_dt_Tm) or cv.end_effective_dt_tm < ",
             " cnvtdatetime(cv_cur_dt_tm)) ")
           ENDIF
          ELSE
           SET cv_done_ind = 1
          ENDIF
         ELSEIF (cv_query_cnt=4)
          IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].active_ind_dup_ind=1))
           SET cv_done_ind = 1
          ELSE
           IF (cv_effective_ind=1)
            SET dyn_w_clause = concat(orig_dup_string,
             " and cv.begin_effective_dt_tm <= cnvtdatetime(cv_cur_dt_Tm) and cv.end_effective_dt_tm > ",
             " cnvtdatetime(cv_cur_dt_tm) and cv.active_ind = rs_0619->to_values.active_ind")
           ELSE
            SET dyn_w_clause = concat(orig_dup_string,
             " and (cv.begin_effective_dt_tm > cnvtdatetime(cv_cur_dt_Tm) or cv.end_effective_dt_tm < ",
             " cnvtdatetime(cv_cur_dt_tm)) and cv.active_ind = rs_0619->to_values.active_ind")
        ...
        ENDIF
      ENDWHILE
      SET short_string = "CODE_VALUE"
     ENDIF
     IF ((cv_eval_return=- (2)))
      SET insert_update_reason = "Multiple CODE_VALUES returned using the UI query"
      SET no_insert_update = 1
      SET fail_merges = (fail_merges+ 1)
      SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
      SET fail_merge_audit->num[fail_merges].text = insert_update_reason
      ...
      SET cv_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
      SET drdm_chg->log[cv_parent_drdm_row].chg_log_reason_txt = concat(
       "Multiple rows found in TARGET Where ",cv_nomv06_msg)
      IF (drdm_error_out_ind=1)
       ROLLBACK
       SET final_check_loop = perm_col_cnt
      ENDIF
      ...
      SET drdm_no_trans_ind = 1
      CALL orphan_child_tab("CODE_VALUE","NOMV06","CODE_VALUE",rs_0619->from_values.code_value)
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 0
      GO TO exit_0619
     ENDIF
    ELSEIF (temp_substr != "ALWAYS")
     SET cv_sub_val = trim(cnvtstring(rs_0619->from_values.code_value,20,1))
     SET cv_eval_return = substitute_all(cv_sub_val,"VALUE")
     SET dm_err->eproc = "Translate Value"
    ENDIF
  ...
  ENDIF
 ENDIF
 FREE SET code_set_loop
 FREE SET trans_val_loop
 FREE SET code_vc
 FREE SET value_status
 FREE SET value_hold
 FREE SET code_value_cnt
 FREE SET disp_key_num
 FREE SET disp_num
 FREE SET cdf_mean_num
 FREE SET active_ind_num
 FREE SET found_val
 FREE SET drdm_table_name
 FREE SET cki_val
 FREE SET display_val
 FREE SET dipslay_key_val
 FREE SET cdf_meaning_val
 FREE SET active_ind_val
 CALL echo(build("P Code_value = ",curmem))
 SET idcd_check = 0
 CALL echo("")
 CALL echo("")
 ...
 FOR (final_check_loop = 1 TO perm_col_cnt)
   IF (value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].idcd_ind)=1)
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].translated=0))
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].root_entity_name != "")
      AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].column_name !=
     "CODE_VALUE"))
      ...
      root_entity_attr
      CALL parser(concat("set rpt_from = rs_0619->from_values.",dm2_ref_data_doc->tbl_qual[
        temp_tbl_cnt].col_qual[final_check_loop].column_name," go"),1)
     ENDIF
     SET cv_idcd_skip = 0
     IF (rpt_table != ""
      AND rpt_from != 0)
      SET drdm_no_trans_ind = 1
      SET except_log_type = report_missing(rpt_table,rpt_column,rpt_from)
      IF (except_log_type="ORPHAN")
       IF ((((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].unique_ident_ind=1)
       ) OR ((((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].pk_ind=1)) OR ((
       dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].defining_attribute_ind=1)
       )) )) )
        SET fail_merges = (fail_merges+ 1)
        SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
        SET fail_merge_audit->num[fail_merges].text = concat("ORPHAN - ",dm2_ref_data_doc->tbl_qual[
         temp_tbl_cnt].col_qual[final_check_loop].column_name)
        CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges]
         .text,2)
        IF (drdm_error_out_ind=1)
         ROLLBACK
       ...
       ELSE
        ...
        SET cv_idcd_skip = 1
        CALL parser(concat("set rs_0619->to_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
          col_qual[final_check_loop].column_name," = 0 go"),1)
        SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].translated = 1
       ENDIF
      ELSEIF (except_log_type IN ("NOMV*", "BADLOG", "CHLD*"))
       SET fail_merges = (fail_merges+ 1)
       SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
       SET fail_merge_audit->num[fail_merges].text = concat(except_log_type," - ",dm2_ref_data_doc->
        tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].column_name)
       CALL orphan_child_tab("CODE_VALUE",except_log_type,"CODE_VALUE",rs_0619->from_values.
        code_value)
       CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges].
        text,2)
       IF (drdm_error_out_ind=1)
     ...
     IF (cv_idcd_skip=0)
      SET idcd_check = 1
      SET dm_err->eproc = concat("This log_id ","wasn't translated because of the ",dm2_ref_data_doc
       ->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].column_name," column.")
      CALL echo("")
      CALL echo("")
      CALL echo(dm_err->eproc)
      CALL echo("")
 ...
 IF (no_insert_update=1)
  SET insert_update_reason = concat(insert_update_reason," ",cnvtstring(drdm_chg->log[drdm_log_loop].
    log_id,20,1))
  CALL echo("")
  CALL echo("")
  ...
  SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
  SET fail_merge_audit->num[fail_merges].text = insert_update_reason
  CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges].text,
   2)
  IF (drdm_error_out_ind=1)
   ROLLBACK
   GO TO exit_0619
 ...
 IF (idcd_check=0)
  ...
  IF (nodelete_ind=0)
   ...
   ENDIF
   SET current_merges = 0
   CALL echo(build("p Delete = ",curmem))
  ELSE
 ...
 ELSE
  IF (orphan_ind=1)
   ...
   IF ((global_mover_rec->one_pass_ind=0))
    ROLLBACK
   ENDIF
   CALL orphan_child_tab("CODE_VALUE","ORPHAN","CODE_VALUE",rs_0619->from_values.code_value)
   IF (nodelete_ind=0)
    SET drdmm_srch_str = dm2_get_rdds_tname("DM_CHG_LOG")
    SELECT INTO "nl:"
     FROM (parser(drdmm_srch_str) dc)
     WHERE (dc.log_id=drdm_chg->log[drdm_log_loop].log_id)
     DETAIL
      update_cnt = dc.updt_cnt
     WITH nocounter
    ;end select
  ...
  ENDIF
 ENDIF
 SUBROUTINE substitute_all(in_code,origin)
   DECLARE point_hold = i4
   DECLARE substitute_return = f8
   DECLARE ui_strip = vc
   FREE RECORD subs_all_request
   RECORD subs_all_request(
     1 sub_str = vc
   )
   FREE RECORD subs_all_reply
   RECORD subs_all_reply(
     1 return_msg = f8
   )
   SET dm_err->eproc = "Substitute All"
   CALL echo("")
   CALL echo("")
   CALL echo("**********************SUBSTITUTE ALL**********************")
   CALL echo("")
   CALL echo("")
   IF (origin="VALUE")
    SET ui_strip = dm2_ref_data_doc->cs_qual[temp_cs_cnt].merge_ui_query
   ELSE
    SET ui_strip = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].merge_ui_query
   ENDIF
   SET point_hold = findstring("<",ui_strip)
   SET ui_strip = substring(1,(point_hold - 1),ui_strip)
   SET ui_strip = concat(ui_strip,"cnvtreal('",trim(in_code),"'))")
   CALL echo(ui_strip)
   SET subs_all_request->sub_str = ui_strip
   EXECUTE dm_rmc_code_value  WITH replace("REQUEST","SUBS_ALL_REQUEST"), replace("REPLY",
    "SUBS_ALL_REPLY")
   CASE (subs_all_reply->return_msg)
    OF - (1):
     SET insert_update_reason = concat("The PARENT_LOC_CD column is prohibiting the merge of ",
      "a row from code_set 220 having log_id of ")
     SET no_insert_update = 1
     SET substitute_return = - (1)
    OF - (2):
     SET insert_update_reason = "Multiple code_values returned for code_set 220 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (3):
     SET insert_update_reason = "Multiple code_values returned for code_set 200 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (4):
     SET insert_update_reason = "Multiple code_values returned for code_set 6020 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (5):
     SET insert_update_reason = concat(
      "The PARENT_SERVICE_RESOURCE_CD column is prohibiting the merge of ",
      "a row from code_set 221 having log_id of ")
     SET no_insert_update = 1
     SET substitute_return = - (1)
    OF - (6):
     SET insert_update_reason = "Multiple code_values returned for code_set 221 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (7):
     SET insert_update_reason = concat(
      "The PARENT_IMAGE_CLASS_TYPE_CD column is prohibiting the merge of ",
      "a row from code_set 5503 having log_id of ")
     SET no_insert_update = 1
     SET substitute_return = - (1)
    OF - (8):
     SET insert_update_reason = concat("The LIB_GROUP_CD column is prohibiting the merge of ",
      "a row from code_set 5503 having log_id of ")
     SET no_insert_update = 1
     SET substitute_return = - (1)
    OF - (9):
     SET insert_update_reason = "Multiple code_values returned for code_set 5503 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (10):
     SET insert_update_reason = concat("Code_Value from code_set 16370 is not translated.  ",
      "Not being translated is a row with log id of ")
     SET no_insert_update = 1
     SET substitute_return = - (1)
    OF - (11):
     SET insert_update_reason =
     "Multiple translations returned for code_set 16370 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (12):
     SET insert_update_reason =
     "Multiple code_values returned for code_set 16589 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (13):
     SET insert_update_reason =
     "There was no translation for DISCRETE_TASK_ASSAY.ACTIVITY_TYPE_CD for log_id "
     SET no_insert_update = 1
     SET substitute_return = - (1)
    OF - (14):
     SET insert_update_reason =
     "Multiple code_values returned for code_set 14003 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (15):
     SET insert_update_reason =
     "There was no translation for ORDER_ENTRY_FIELDS.CATALOG_TYPE_CD for log_id "
     SET no_insert_update = 1
     SET substitute_return = - (1)
    OF - (16):
     SET insert_update_reason =
     "Multiple code_values returned for code_set 16449 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (17):
     SET insert_update_reason =
     "An unexpected parent/child combinations was found for a location code w/ log_id of "
     SET no_insert_update = 1
     SET substitute_return = - (8)
    OF - (18):
     SET insert_update_reason = "Multiple code_values returned for code_set 72 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (19):
     SET insert_update_reason = concat("Multiples returned for initial query, none on second query",
      " for code_set 220 having a log id of ")
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (20):
     SET insert_update_reason =
     "Syntax error occurred with the UI query for a code_value having a log_id of "
     SET no_insert_update = 1
     SET substitute_return = - (5)
    OF - (21):
     SET insert_update_reason = "A parent row needed for the translation is an ORPHAN"
     SET no_insert_update = 1
     SET substitute_return = - (4)
    OF - (22):
     SET insert_update_reason = "A parent row needed for the translation has a status of NOMV*"
     SET no_insert_update = 1
     SET substitute_return = - (6)
    OF - (23):
     SET insert_update_reason = "Necessary OMF package content not in target for log_id "
     SET no_insert_update = 1
     SET substitute_return = - (6)
    OF - (24):
     SET insert_update_reason = "There was no translation for the embedded code_value for log_id "
     SET no_insert_update = 1
     SET substitute_return = - (1)
    OF - (25):
     SET insert_update_reason =
     "Unexpected data found for a code_value in codeset 25633  having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (7)
    OF - (26):
     SET insert_update_reason =
     "Multiple code_values returned for code_set 25633 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (27):
     SET insert_update_reason =
     "Multiple code_values returned for code_set 24509 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF - (28):
     SET insert_update_reason = concat("Code_Value from code_set 16370 was not found.  ",
      "Not being translated is a row with log id of ")
     SET no_insert_update = 1
     SET substitute_return = - (7)
    OF - (29):
     SET insert_update_reason =
     "There was no translation for the embedded MAMMO_LETTER.LETTER_ID for log_id "
     SET no_insert_update = 1
     SET substitute_return = - (1)
    OF - (30):
     SET insert_update_reason =
     "Unexpected data found for a code_value in codeset 413575  having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (6)
    OF - (31):
     SET substitute_return = - (9)
    OF - (32):
     SET insert_update_reason = "Multiple code_values returned for code_set 93 having a log id of "
     SET no_insert_update = 1
     SET substitute_return = - (2)
    OF 0:
     SET substitute_return = - (3)
    ELSE
     SET substitute_return = subs_all_reply->return_msg
   ENDCASE
   RETURN(substitute_return)
   FREE SET point_hold
   FREE SET substitute_return
   FREE SET ui_strip
 END ;Subroutine
 SUBROUTINE check_cki(sbr_cki)
   DECLARE good_cki_ind = i2
   DECLARE cki_loop = i4
   DECLARE cki_size = i4
   SET good_cki_ind = 0
   SET cki_size = size(global_mover_rec->qual,5)
   IF (cki_size=0)
    RETURN(- (1))
   ENDIF
   FOR (cki_loop = 1 TO cki_size)
     IF (sbr_cki=patstring(global_mover_rec->qual[cki_loop].pattern_cki))
      SET good_cki_ind = 1
     ENDIF
   ENDFOR
   RETURN(good_cki_ind)
 END ;Subroutine
 SUBROUTINE check_substitute_val(cv_eval_return)
   IF (no_insert_update=0)
    IF ((cv_eval_return=- (1)))
     SET fail_merges = (fail_merges+ 1)
     SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
     ...
     IF (drdm_error_out_ind=1)
      ...
      SET final_check_loop = perm_col_cnt
     ENDIF
    ELSEIF ((cv_eval_return=- (2)))
     CALL orphan_child_tab("CODE_VALUE","NOMV06","CODE_VALUE",rs_0619->from_values.code_value)
     SET drdm_mini_loop_status = "NOMV06"
     SET drdm_no_trans_ind = 1
    ELSEIF ((cv_eval_return=- (4)))
     CALL orphan_child_tab("CODE_VALUE","ORPHAN","CODE_VALUE",rs_0619->from_values.code_value)
     SET drdm_mini_loop_status = "ORPHAN"
     SET drdm_no_trans_ind = 1
    ELSEIF ((cv_eval_return=- (5)))
     CALL orphan_child_tab("CODE_VALUE","NOMV99","CODE_VALUE",rs_0619->from_values.code_value)
     SET drdm_mini_loop_status = "NOMV99"
     SET drdm_no_trans_ind = 1
    ELSEIF ((cv_eval_return=- (3)))
     SELECT INTO "NL:"
     ...
     SET child_merge_audit->num[current_merges].text = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt
      ].table_name,"  ",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].column_name)
    ELSEIF ((cv_eval_return=- (6)))
     CALL orphan_child_tab("CODE_VALUE",drdm_mini_loop_status,"CODE_VALUE",rs_0619->from_values.
      code_value)
     SET drdm_no_trans_ind = 1
    ELSEIF ((cv_eval_return=- (7)))
     CALL orphan_child_tab("CODE_VALUE","NOMV46","CODE_VALUE",rs_0619->from_values.code_value)
     SET drdm_mini_loop_status = "NOMV46"
     SET drdm_no_trans_ind = 1
    ELSEIF ((cv_eval_return=- (8)))
     CALL orphan_child_tab("CODE_VALUE","NOMV36","CODE_VALUE",rs_0619->from_values.code_value)
     SET drdm_mini_loop_status = "NOMV36"
     SET drdm_no_trans_ind = 1
    ELSEIF ((cv_eval_return=- (9)))
     RETURN(0)
    ELSE
     SET current_merges = (current_merges+ 1)
    ...
    ENDIF
    CALL rdds_del_except("CODE_VALUE",cnvtreal(rs_0619->from_values.code_value))
    SET drdm_from_con = cnvtstring(rs_0619->from_values.code_value,20,1)
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated=0))
     SET cv_imt_check = insert_merge_translate(cnvtreal(rs_0619->from_values.code_value),
      cv_eval_return,"CODE_VALUE")
     IF (drmm_ddl_rollback_ind=1)
      GO TO exit_0619
     ENDIF
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
    ...
    IF (cv_imt_check=1)
     ...
     GO TO exit_0619
    ENDIF
    SET rs_0619->to_values.code_value = cv_eval_return
   ELSE
    SET drdm_no_trans_ind = 1
    IF ((cv_eval_return=- (1)))
     SET fail_merges = (fail_merges+ 1)
...
#exit_0619
 IF (drmm_ddl_rollback_ind=1
  AND (drdm_chg->log[drdm_log_loop].table_name > ""))
  SET drdm_log_loop = redirect_to_start_row(drdm_chg,drdm_log_loop,drdm_next_row_pos,drdm_max_rs_size
   )
 ENDIF
