CREATE PROGRAM dm2_aload_sch_col
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_sch_col",".log","dm2_aload_sch_col")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_sch_col"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dacol_cnt = i4 WITH protect, noconstant(0)
 DECLARE dacol_str = vc WITH protect, noconstant("")
 DECLARE dacol_req_size = i4 WITH protect, noconstant(0)
 DECLARE dacol_ndx = i4 WITH protect, noconstant(0)
 DECLARE dacol_ndx2 = i4 WITH protect, noconstant(0)
 FREE RECORD dacol_maint
 RECORD dacol_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET dacol_maint->num_of_inserts = 0
 SET dacol_maint->num_of_updates = 0
 SET dacol_maint->num_of_none = 0
 SET dacol_req_size = size(requestin->list_0,5)
 SET stat = alterlist(dacol_maint->qual,dacol_req_size)
 IF (dac_pop_coldic_rec("DM_AFD_COLUMNS")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Initialize maintenance for DM_AFD_COLUMNS."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dacol_req_size))
  DETAIL
   dacol_maint->qual[d.seq].maint_type = "I", dacol_maint->num_of_inserts = (dacol_maint->
   num_of_inserts+ 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining maintenance type for DM_AFD_COLUMNS."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_afd_tables dat,
   dm_afd_columns dac,
   (dummyt d  WITH seq = value(dacol_req_size))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (dat
   ...
   JOIN (dac
   WHERE dac.alpha_feature_nbr=cnvtint(requestin->list_0[d.seq].alpha_feature_nbr)
    AND dac.owner=dat.owner
    AND (dac.table_name=requestin->list_0[d.seq].table_name)
    AND (dac.column_name=requestin->list_0[d.seq].column_name)
    AND dac.table_name=dat.table_name)
  ORDER BY dac.table_name, dac.column_name
  DETAIL
   IF (cnvtint(requestin->list_0[d.seq].schema_instance) < dat.schema_instance)
    dacol_maint->qual[d.seq].maint_type = "N", dacol_maint->num_of_none = (dacol_maint->num_of_none+
    1), dacol_maint->num_of_inserts = (dacol_maint->num_of_inserts - 1)
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm)=cnvtdatetime(dac.updt_dt_tm))
    dacol_maint->qual[d.seq].maint_type = "N", dacol_maint->num_of_none = (dacol_maint->num_of_none+
    1), dacol_maint->num_of_inserts = (dacol_maint->num_of_inserts - 1)
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm) != cnvtdatetime(dac.updt_dt_tm))
    dacol_maint->qual[d.seq].maint_type = "U", dacol_maint->num_of_updates = (dacol_maint->
    num_of_updates+ 1), dacol_maint->num_of_inserts = (dacol_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(dacol_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((dacol_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(dacol_req_size) )"),0)
  ...
  COMMIT
 ENDIF
 IF ((dacol_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where dacol_maint->qual[d.seq].maint_type = 'U' ",0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_sch_col completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_sch_col")
END GO
