CREATE PROGRAM dcp_get_pl_schedule:dba
 ...
 CALL populateschedulelocations(1)
 IF ((criteria->schedule_location_cnt > 0)
  AND (criteria->resource_cnt > 0))
  CALL executebothselect(1)
 ELSEIF ((criteria->schedule_location_cnt > 0))
  CALL executelocationselect(1)
 ELSEIF ((criteria->resource_cnt > 0))
  CALL executeresourceselect(1)
 ENDIF
 SET stat = alterlist(reply->patients,patient_cnt)
 IF (patient_cnt > 0)
 ...
 ENDIF
 FREE SET criteria
 SUBROUTINE (executelocationselect(x=i4) =null)
  DECLARE locx = i4 WITH noconstant(0)
  SELECT INTO "nl:"
   FROM sch_appt a,
    person p,
    encounter e
   PLAN (a
    WHERE ((a.beg_dt_tm+ 0) >= cnvtdatetime(datetimezone(datetimeadd(criteria->beg_effective_dt_tm,
       - ((max_appt_time/ 24.0))),criteria->begin_tz,1)))
     AND ((a.beg_dt_tm+ 0) <= cnvtdatetime(datetimezone(criteria->end_effective_dt_tm,criteria->
      end_tz,1)))
     AND a.end_dt_tm >= cnvtdatetime(datetimezone(criteria->beg_effective_dt_tm,criteria->begin_tz,1)
     )
     AND a.end_dt_tm <= cnvtdatetime(datetimezone(datetimeadd(criteria->end_effective_dt_tm,(
       max_appt_time/ 24.0)),criteria->end_tz,1))
     AND expand(locx,1,criteria->schedule_location_cnt,a.appt_location_cd,criteria->
     schedule_locations[locx].loc_cd)
   ...
   DETAIL
    patient_cnt += 1
    IF (mod(patient_cnt,50)=1)
     stat = alterlist(reply->patients,(patient_cnt+ 49))
    ENDIF
    reply->patients[patient_cnt].person_id = p.person_id, reply->patients[patient_cnt].person_name =
    p.name_full_formatted, reply->patients[patient_cnt].encntr_id = e.encntr_id,
    reply->patients[patient_cnt].organization_id = e.organization_id, reply->patients[patient_cnt].
    confid_level_cd = e.confid_level_cd
    IF (((encntr_org_sec_ind=0) OR (e.encntr_id=0)) )
     reply->patients[patient_cnt].filter_ind = 0
    ELSE
     reply->patients[patient_cnt].filter_ind = 1
     IF (confid_ind=1)
      reply->patients[patient_cnt].confid_level = maxval(0,uar_get_collation_seq(e.confid_level_cd))
     ENDIF
    ENDIF
    reply->patients[patient_cnt].active_ind = e.active_ind
   FOOT REPORT
    stat = alterlist(reply->patients,patient_cnt)
   WITH nocounter
  ;end select
 END ;Subroutine
 SUBROUTINE (executeresourceselect(x=i4) =null)
  DECLARE resx = i4 WITH noconstant(0)
  SELECT INTO "nl:"
   FROM sch_appt a,
   ...
   PLAN (a
    WHERE ((a.beg_dt_tm+ 0) >= cnvtdatetime(datetimezone(datetimeadd(criteria->beg_effective_dt_tm,
     ...
     AND a.end_dt_tm <= cnvtdatetime(datetimezone(datetimeadd(criteria->end_effective_dt_tm,(
       max_appt_time/ 24.0)),criteria->end_tz,1))
     AND expand(resx,1,criteria->resource_cnt,a.resource_cd,criteria->resources[resx].resource_cd)
     AND a.role_meaning != "PATIENT"
     AND a.state_meaning IN ("CONFIRMED", "CHECKED IN", "CHECKED OUT")
     AND a.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
    JOIN (sep
    WHERE sep.sch_event_id=a.sch_event_id
   ...
   FOOT REPORT
    stat = alterlist(reply->patients,patient_cnt)
   WITH nocounter, orahintcbo("INDEX(A XIE17SCH_APPT)")
  ;end select
 END ;Subroutine
 SUBROUTINE (executebothselect(x=i4) =null)
   DECLARE locx = i4 WITH noconstant(0)
   DECLARE resx = i4 WITH noconstant(0)
   SELECT INTO "nl:"
    FROM sch_appt a,
    ...
    PLAN (a
     WHERE ((a.beg_dt_tm+ 0) >= cnvtdatetime(datetimezone(datetimeadd(criteria->beg_effective_dt_tm,
      ...
      AND a.state_meaning IN ("CONFIRMED", "CHECKED IN", "CHECKED OUT")
      AND a.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00")
      AND expand(locx,1,criteria->schedule_location_cnt,(a.appt_location_cd+ 0),criteria->
      schedule_locations[locx].loc_cd))
     JOIN (sep
     WHERE sep.sch_event_id=a.sch_event_id
      AND sep.active_ind=1
