CREATE PROGRAM dm2_aload_afd_indexes
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_ind",".log","dm2_aload_afd_indexes")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_afd_indexes"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daai_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daai_file = vc WITH protect, noconstant( $2)
 DECLARE daai_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daai_batch = i4 WITH protect, noconstant(0)
 DECLARE daai_rows_returned = i4 WITH protect, noconstant(1)
 DECLARE daai_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daai_pkg,daai_file_loc_ret,daai_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daai_batch=0)
  SET daai_batch = 5000
 ENDIF
 IF (daai_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daai_file_loc_ret,daai_file), "dm2_aload_sch_ind", daai_batch
  CALL echo(concat(daai_file_loc_ret,daai_file))
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Remove extraneous DM_AFD_INDEXES rows"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dm_afd_indexes a
   WHERE a.alpha_feature_nbr=daai_pkg
    AND a.owner=currdbuser
    AND  NOT ( EXISTS (
   (SELECT
    "x"
    FROM dm_afd_tables b
    WHERE b.alpha_feature_nbr=a.alpha_feature_nbr
     AND b.owner=a.owner
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_afd_indexes completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_afd_indexes")
END GO
