CREATE PROGRAM dc_mp_get_hf
 ...
 SUBROUTINE iqrversioncontrolstring(category_topic_mean,beg_time)
   ...
   RETURN(version_number)
 END ;Subroutine
 DECLARE ver_num = f8 WITH protect, constant(iqrversioncontrol("HEART_FAILURE",cnvtdatetime(curdate,
    curtime3)))
 IF (( $2 != null))
  SET jrec = cnvtjsontorec( $2)
  IF (validate(debug_dc_mp_get_hf_ind,0)=1)
...
#exit_script
 IF (validate(debug_dc_mp_get_hf_ind,0)=1)
  CALL echo("SCRIPT VERSION IS 006 10/03/2011 Allison Wynn Performance Updates 3.3")
  CALL echorecord(filter)
  CALL echorecord(hfeval)
  CALL echorecord(hfreply)
 ENDIF
 IF (validate(_memory_reply_string)=1)
  SET _memory_reply_string = cnvtrectojson(hfreply)
 ELSE
  CALL echojson(hfreply, $1)
  SELECT INTO "nl:"
   DETAIL
    row + 0
   WITH nocounter
