CREATE PROGRAM bhs_rpt_admin_audit_all:dba
 PROMPT
  "output:" = "MINE",
 ...
 DECLARE mf_bmc_facility_cd = f8 WITH constant(uar_get_code_by("DESCRIPTION",220,
   "BAYSTATE MEDICAL CENTER"))
 DECLARE mf_bfmc_facility_cd = f8 WITH constant(uar_get_code_by("DESCRIPTION",220,
   "BAYSTATE FRANKLIN MEDICAL CENTER"))
 DECLARE mf_bwh_facility_cd = f8 WITH constant(uar_get_code_by("DESCRIPTION",220,
   "BAYSTATE WING HOSPITAL"))
 DECLARE mf_bmlh_facility_cd = f8 WITH constant(uar_get_code_by("DESCRIPTION",220,
   "BAYSTATE MARY LANE HOSPITAL"))
 DECLARE mf_bnh_facility_cd = f8 WITH constant(uar_get_code_by("DESCRIPTION",220,
 ...
 IF (( $RUNTYPE=2))
  SET work->beg_dt_tm = cnvtdatetime((curdate - 1),210000)
  SET work->end_dt_tm = cnvtdatetime(curdate,205900)
 ELSEIF (( $RUNTYPE=1))
  SET work->beg_dt_tm = cnvtdatetime( $BEGDATE, $BEGTIME)
  SET work->end_dt_tm = cnvtdatetime( $ENDDATE, $ENDTIME)
 ELSE
  SET work->beg_dt_tm = cnvtdatetime(concat( $BEGDATE," ", $BEGTIME))
  SET work->end_dt_tm = cnvtdatetime(concat( $BEGDATE," ", $ENDTIME))
 ENDIF
 CALL echo("TIMES:")
 ...
 IF (( $UNIT_CD_DISP_KEY > 0)
  AND ( $ALLUNITS=0))
  CALL echo("one location")
  ...
  SELECT INTO "nl:"
   ...
   HEAD p.person_id
    ...
    work->encntrs[work->e_cnt].firstname = trim(p.name_first,3), work->encntrs[work->e_cnt].
    middlename = trim(p.name_middle,3), work->encntrs[work->e_cnt].lastname = trim(p.name_last,3),
    work->encntrs[work->e_cnt].dob = trim(format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p
        .birth_tz),1),"MM-DD-YYYY;;q"),3), work->encntrs[work->e_cnt].fin = format(trim(ea.alias,3),
    ...
    encntrs[work->e_cnt].fac = trim(uar_get_code_display(ed.loc_facility_cd),3), work->encntrs[work->
    e_cnt].room = trim(uar_get_code_display(ed.loc_room_cd),3)
   WITH nocounter
  ;end select
 ELSE
  ...
  SELECT INTO "nl:"
   ...
   PLAN (ce
    WHERE ce.updt_dt_tm BETWEEN cnvtdatetime(work->beg_dt_tm) AND cnvtdatetime(work->end_dt_tm)
     AND ((ce.valid_until_dt_tm+ 0) >= cnvtdatetime(curdate,curtime))
     AND ((ce.view_level+ 0)=1)
     AND ((ce.event_class_cd+ 0) IN (232, 236, 228))
     AND ((ce.event_reltn_cd+ 0)=132.00)
     AND ce.result_status_cd IN (34.00, 35, 25.00))
    JOIN (cem
    WHERE cem.event_id=ce.event_id)
    JOIN (e
    WHERE e.encntr_id=ce.encntr_id
     AND e.loc_facility_cd IN (mf_bmc_facility_cd, mf_bfmc_facility_cd, mf_bwh_facility_cd,
    mf_bmlh_facility_cd, mf_bnh_facility_cd))
    JOIN (ea
    WHERE ea.encntr_id=e.encntr_id
     AND ea.encntr_alias_type_cd=cs319_fin_cd
     AND ea.active_ind=1)
   ...
   HEAD e.encntr_id
    ...
    middlename = trim(p.name_middle,3), work->encntrs[work->e_cnt].lastname = trim(p.name_last,3),
    work->encntrs[work->e_cnt].dob = trim(format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p
        .birth_tz),1),"MM-DD-YYYY;;d"),3), work->encntrs[work->e_cnt].fin = format(trim(ea.alias,3),
     "############;P0"), work->encntrs[work->e_cnt].mrn = format(trim(ea1.alias,3),"#######;P0"),
   ...
   HEAD o.order_id
    ...
    ].catalog_cd = o.catalog_cd, work->encntrs[work->e_cnt].meds[m_cnt].catprimary =
    uar_get_code_display(ce.event_cd),
    work->encntrs[work->e_cnt].meds[m_cnt].desc = build2(trim(o.hna_order_mnemonic,3)," (",trim(o
      .order_mnemonic,3),")"), work->encntrs[work->e_cnt].meds[m_cnt].freq = trim(
     uar_get_code_display(f.frequency_cd),3), work->encntrs[work->e_cnt].meds[m_cnt].comments_ind = o
    .order_comment_ind,
    work->encntrs[work->e_cnt].meds[m_cnt].order_id =
    IF (o.template_order_id > 0) o.template_order_id
    ELSE o.order_id
    ENDIF
   ...
   HEAD cem.event_id
    ...
    uar_get_code_display(cem.dosage_unit_cd), work->encntrs[work->e_cnt].meds[m_cnt].ordevent[
    eventcnt].droute = uar_get_code_display(cem.admin_route_cd),
    work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].admin_datetime = format(cnvtdatetime(ce
      .event_end_dt_tm),"YYYYMMDDHHMMSS;;q"), work->encntrs[work->e_cnt].meds[m_cnt].ordevent[
    eventcnt].bag = cem.substance_lot_number, work->encntrs[work->e_cnt].meds[m_cnt].ordevent[
    eventcnt].eventid = ce.event_id,
   ...
   FOOT  cem.event_id
    ...
    ELSE
     ...
     ENDIF
    ENDIF
    work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].diluent = cnvtstring(cem
     .diluent_type_cd), work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].eventcd =
    uar_get_code_display(ce.event_cd), work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].
    dilsyn = cnvtstring(cem.synonym_id),
    work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].ordercatprimary = uar_get_code_display(
     ce.event_cd), work->encntrs[work->e_cnt].meds[m_cnt].ordevent[eventcnt].parenteventid = ce
    .parent_event_id
   WITH nocounter
  ;end select
 ...
 ;end select
 IF (( $RUNTYPE > 0))
  SET var_output = build(logical("bhscust"),"/ftp/bhs_rpt_admin_audit_all/adminreport.",trim(format(
     cnvtdatetime(sysdate),"YYYYMMDD;;q"),3))
  SET sep = "|"
 ELSE
 ...
 IF (( $RUNTYPE=0))
  SELECT INTO value(var_output)
   ...
   eventcd = substring(1,100,work->encntrs[d.seq].meds[d1.seq].ordevent[d2.seq].eventcd),
   dilsyn = substring(1,100,work->encntrs[d.seq].meds[d1.seq].ordevent[d2.seq].dilsyn),
   ...
   admin_datetime = substring(1,20,work->encntrs[d.seq].meds[d1.seq].ordevent[d2.seq].admin_datetime),
   drug_cost = 0, status =
 ...
 ELSE
  ...
  SELECT INTO value(var_output)
   ...
   route = substring(1,50,work->encntrs[d.seq].meds[d1.seq].ordevent[d2.seq].droute), schedule_code
    = "n/a", admin_datetime = substring(1,20,work->encntrs[d.seq].meds[d1.seq].ordevent[d2.seq].
    admin_datetime),
   drug_cost = 0, status =
   IF ((work->encntrs[d.seq].meds[d1.seq].ordevent[d2.seq].dispense_amount=0)
    AND (work->encntrs[d.seq].meds[d1.seq].med_type=4)) "BeginBag"
