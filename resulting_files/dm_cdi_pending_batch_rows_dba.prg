CREATE PROGRAM dm_cdi_pending_batch_rows:dba
 ...
 SET reply->table_name = "CDI_PENDING_BATCH"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(b.cdi_pending_batch_id)
    FROM cdi_pending_batch b
    WHERE b.cdi_pending_batch_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   seqval = max(b.cdi_pending_batch_id)
   FROM cdi_pending_batch b
   DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     b.rowid
     FROM cdi_pending_batch b
     WHERE parser(sbr_getrowidnotexists("b.cdi_pending_batch_id between curMinID and curMaxID","b"))
      AND b.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
      AND  NOT ( EXISTS (
     (SELECT
      d.cdi_pending_document_id
      FROM cdi_pending_document d
      WHERE d.cdi_pending_batch_id=b.cdi_pending_batch_id
       AND d.active_ind=1
       AND ((d.process_location_flag != 0
       AND d.process_location_flag != 4) OR (d.updt_dt_tm >= cnvtdatetime((curdate - v_days_to_keep),
       curtime3))) )))
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,50)=1)
      ...
      ENDIF
      reply->rows[rows].row_id = b.rowid
     WITH nocounter, maxqual(b,value(rowsleft))
    ;end select
