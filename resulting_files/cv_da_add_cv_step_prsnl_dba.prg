CREATE PROGRAM cv_da_add_cv_step_prsnl:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_add_cv_step_prsnl_vrsn = vc WITH private, constant("20050914")
 ...
 WHILE (i <= size(request->objarray,5))
   ...
   IF (validate(request->objarray[i].cv_step_prsnl_id,- (0.00001)) <= 0.0)
    ...
    SET request->objarray[i].cv_step_prsnl_id = did
   ELSE
    SET did = request->objarray[i].cv_step_prsnl_id
   ENDIF
   IF (validate(reply->insert_ids)=1)
    SET reply->insert_ids[i].id = did
   ENDIF
   INSERT  FROM cv_step_prsnl c
    SET c.cv_step_prsnl_id = did, c.cv_step_id =
     ...
     ENDIF
     , c.action_type_cd =
     IF ((validate(request->objarray[i].action_type_cd,- (0.00001)) != - (0.00001))) validate(request
       ->objarray[i].action_type_cd,- (0.00001))
     ELSE 0.0
     ENDIF
     , c.action_dt_tm =
     ...
     ,
     c.action_sequence =
     IF ((validate(request->objarray[i].action_sequence,- (1)) != - (1))) validate(request->objarray[
       i].action_sequence,- (1))
     ELSE 0
     ENDIF
