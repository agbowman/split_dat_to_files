CREATE PROGRAM dcpreqgen01:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   orders->admit_dt = format(e.reg_dt_tm,"@SHORTDATE"), orders->facility = uar_get_code_description(e
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   orders->qual[ord_cnt].enter_for = pl2.name_full_formatted, orders->qual[ord_cnt].type =
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   HEAD REPORT
    ...
    reg3_dio = "{LPI/8}{CPI/12}{FONT/4}", first_page = "Y", ord_dr = orders->qual[1].enter_for,
    saved_pos = 0
   HEAD PAGE
    reg_dio, "  ", row + 1,
    line1 = fillstring(35,"_"), line2 = fillstring(10,"_"), allg_line1 = "                        ",
    allg_line2 = "                        ", "{CPI/7}{POS/150/67}", orders->facility,
    row + 1, "{CPI/10}{POS/45/140}{BOX/70/14}", row + 1,
    "{CPI/10}{POS/58/155}", "{B}", "CLINICAL INFORMATION: ",
    "{ENDB}", row + 1, "{CPI/12}{POS/58/176}",
    "{b}", "Name:  ", "{endb}",
    row + 1, "{CPI/12}{POS/175/176}",
    CALL print(trim(orders->name,3)),
    row + 1, "{CPI/12}{POS/58/187}", "{b}",
    "Location:  ", "{endb}", row + 1,
    "{CPI/12}{POS/175/187}", orders->location, row + 1,
    "{CPI/12}{POS/58/198}", "{b}", "Birthdate:  ",
    "{endb}", row + 1, "{CPI/12}{POS/175/198}",
    orders->dob, row + 1, "{CPI/12}{POS/58/209}",
    "{b}", "Age:  ", "{endb}",
    orders->age, row + 1, "{CPI/12}{POS/140/209}",
    "{b}", "Sex:  ", "{endb}",
    row + 1, "{CPI/12}{POS/175/209}", orders->sex,
    row + 1, "{CPI/12}{POS/58/231}", "{b}",
    "Admitting Diagnosis:  ", "{endb}", row + 1
    ...
    ENDIF
    "{CPI/12}{POS/58/242}", "{b}", "Allergies: ",
    "{endb}", row + 1
    IF ((allergy->line_cnt > 0))
     "{CPI/12}{POS/175/242}", allergy->line_qual[1].line, row + 1
    ENDIF
    IF ((allergy->line_cnt > 1))
     "{CPI/12}{POS/175/253}", allergy->line_qual[2].line, row + 1
    ENDIF
    "{CPI/10}{POS/315/155}", "{B}", "MR#: ",
    orders->mrn, "{ENDB}", row + 1,
    "{CPI/12}{POS/315/176}", "{b}", "FIN:  ",
    "{endb}", row + 1, "{CPI/12}{POS/415/176}",
    orders->fnbr, row + 1, "{CPI/12}{POS/315/187}",
    "{b}", "Admit Date:  ", "{endb}",
    row + 1, "{CPI/12}{POS/415/187}", orders->admit_dt,
    row + 1, "{CPI/12}{POS/315/198}", "{b}",
    "Attending MD:  ", "{endb}", row + 1,
    "{CPI/12}{POS/415/198}", orders->attending, row + 1,
    "{CPI/12}{POS/315/209}", "{b}", "Ordering MD:  ",
    "{endb}", row + 1, "{CPI/10}{POS/45/305}{BOX/70/35}",
    row + 1, "{CPI/10}{POS/55/320}", "{B}",
    "ORDERING INFORMATION: ", "{ENDB}", row + 1,
    "{CPI/10}{POS/45/660}{BOX/70/6}", row + 1, "{CPI/12}{POS/270/675}",
    "Location:  ", orders->order_location, row + 1,
    "{CPI/12}{POS/380/675}", "Time:  ", curtime,
    row + 1, "{CPI/12}{POS/460/675}", "Printer:  ",
    request->printer_name, row + 1
    IF (saved_pos > 0)
     "{B}{CPI/10}{POS/1/87}",
     CALL center(build(cnvtupper(orders->qual[saved_pos].catalog)," REQUISITION"),1,190), "{endb}",
     "{CPI/10}{POS/100/127}{b}", orders->qual[saved_pos].mnemonic, "{endb}",
     row + 1, "{CPI/10}{POS/1/107}",
     CALL center(concat("*******************",trim(uar_get_code_display(orders->qual[saved_pos].
        action_type_cd)),"*******************"),1,190),
     row + 1, "{CPI/12}{POS/315/320}{B}", "Order ID: ",
     "{ENDB}", "{CPI/12}{POS/370/320}", orders->qual[saved_pos].order_id"#########",
     row + 1, "{CPI/12}{POS/415/209}", orders->qual[saved_pos].enter_for,
     row + 1, "{CPI/12}{POS/60/675}", "Ordering Person:  ",
     orders->qual[saved_pos].enter_by, row + 1, saved_pos = 0
    ENDIF
   DETAIL
    FOR (vv = 1 TO value(ord_cnt))
      IF ((orders->qual[vv].display_ind=1))
       IF (first_page="N")
        BREAK
       ENDIF
       first_page = "N", "{B}{CPI/10}{POS/1/87}",
       CALL center(build(cnvtupper(orders->qual[vv].catalog)," REQUISITION"),1,190),
       "{endb}", "{CPI/10}{POS/100/127}{b}", orders->qual[vv].mnemonic,
       "{endb}", row + 1, "{CPI/10}{POS/1/107}",
       CALL center(concat("*******************",trim(uar_get_code_display(orders->qual[vv].
          action_type_cd)),"*******************"),1,190), row + 1, "{CPI/12}{POS/315/320}{B}",
       "Order ID: ", "{ENDB}", "{CPI/12}{POS/370/320}",
       orders->qual[vv].order_id"#########", row + 1, "{CPI/12}{POS/415/209}",
       orders->qual[vv].enter_for, row + 1, "{CPI/12}{POS/60/675}",
       "Ordering Person:  ", orders->qual[vv].enter_by, row + 1,
       xcol = 58, ycol = 335
       FOR (fsub = 1 TO 31)
         FOR (ww = 1 TO orders->qual[vv].d_cnt)
           IF ((((orders->qual[vv].d_qual[ww].group_seq=fsub)) OR (fsub=31
            ...
            IF ((orders->qual[vv].action_type_cd=cancel_cd))
             IF ((orders->qual[vv].d_qual[ww].label_text="Cancel Reason"))
              orders->qual[vv].d_qual[ww].print_ind = 1, xcol = 45, ycol = (ycol+ 10),
              "{CPI/12}",
              ...
              "{endb}", row + 1
             ENDIF
            ELSEIF ((orders->qual[vv].d_qual[ww].label_text="Comments")
             AND ycol > 324)
             xcol = 58, ycol = (ycol+ 12), "{CPI/12}",
            ...
            ELSE
             ...
             IF (textlen(trim(orders->qual[vv].d_qual[ww].value,3)) > 0)
              ycol = (ycol+ 12), "{CPI/12}",
              CALL print(calcpos(xcol,ycol)),
              CALL print(orders->qual[vv].d_qual[ww].label_text), ":  ", xcol = 212
            ...
            ENDIF
            IF (xcol > 314)
             xcol = 58, ycol = (ycol+ 12)
            ELSE
             xcol = 315
            ENDIF
            IF (xcol=315
             AND ycol > 562)
             xcol = 65, ycol = 495, saved_pos = vv,
             BREAK, xcol = 58, ycol = 335
            ENDIF
           ENDIF
         ENDFOR
       ENDFOR
       xcol = 71, ycol = (ycol+ 12)
       IF (ycol > 420)
        saved_pos = vv, BREAK, xcol = 71,
        ycol = 347
       ELSE
        ycol = (ycol+ 24)
       ENDIF
       "{cpi/12}",
       CALL print(calcpos(xcol,ycol)), "Order Comments: ",
       row + 1, ycol = (ycol+ 24)
