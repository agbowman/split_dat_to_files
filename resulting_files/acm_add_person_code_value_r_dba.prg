CREATE PROGRAM acm_add_person_code_value_r:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->person_code_value_r_qual[xref->add[index].idx].person_code_value_r_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(person_seq,nextval)
     FROM dual
     DETAIL
      acm_request->person_code_value_r_qual[xref->add[index].idx].person_code_value_r_id = cnvtreal(
       nextseqnum), reply->person_code_value_r_qual[xref->add[index].idx].person_code_value_r_id =
      cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->person_code_value_r_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->person_code_value_r_qual[xref->add[index].idx].person_code_value_r_id = acm_request->
    person_code_value_r_qual[xref->add[index].idx].person_code_value_r_id
   ENDIF
   SET reply->person_code_value_r_qual[xref->add[index].idx].status = 0
   IF ((acm_request->person_code_value_r_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->person_code_value_r_qual[xref->add[index].idx].beg_effective_dt_tm =
    cnvtdatetime(curdate,curtime3)
   ENDIF
   IF ((acm_request->person_code_value_r_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->person_code_value_r_qual[xref->add[index].idx].end_effective_dt_tm =
    cnvtdatetime("31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   person_code_value_r p
  SET p.person_code_value_r_id = acm_request->person_code_value_r_qual[xref->add[d.seq].idx].
   person_code_value_r_id, p.beg_effective_dt_tm = cnvtdatetime(acm_request->
    person_code_value_r_qual[xref->add[d.seq].idx].beg_effective_dt_tm), p.end_effective_dt_tm =
   cnvtdatetime(acm_request->person_code_value_r_qual[xref->add[d.seq].idx].end_effective_dt_tm),
   p.code_set = acm_request->person_code_value_r_qual[xref->add[d.seq].idx].code_set, p.code_value =
   acm_request->person_code_value_r_qual[xref->add[d.seq].idx].code_value, p.contributor_system_cd =
   IF ((acm_request->person_code_value_r_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->person_code_value_r_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.person_id =
   IF ((acm_request->person_code_value_r_qual[xref->add[d.seq].idx].person_id > 0)) acm_request->
    person_code_value_r_qual[xref->add[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->person_code_value_r_qual[xref->add[d.seq].idx].person_idx].
    person_id
   ENDIF
   , p.active_ind =
   IF ((((acm_request->person_code_value_r_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((
   acm_request->person_code_value_r_qual[xref->add[d.seq].idx].active_status_cd=reqdata->
   active_status_cd))) ) 1
   ELSE 0
   ENDIF
   , p.active_status_cd =
   IF ((acm_request->person_code_value_r_qual[xref->add[d.seq].idx].active_status_cd > 0.0))
    acm_request->person_code_value_r_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->person_code_value_r_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->person_code_value_r_qual[xref->add[index].idx].status=0))
 ...
 ENDFOR
 IF (acm_hist_ind=1)
  EXECUTE acm_add_person_cd_val_r_hist
