CREATE PROGRAM bhs_sfe_outcomes_summary24:dba
 PROMPT
 ...
 DECLARE expandreccnt = i4 WITH protect, constant(100)
 ...
 DECLARE expand_stop = i4 WITH noconstant(expandreccnt), protect
 CALL echo("2")
 ...
 SELECT INTO "NL:"
  ...
  HEAD REPORT
   stat = alterlist(formactlisttemp->qual,100), countl = 0
  HEAD dfa.encntr_id
   countl = (countl+ 1)
   IF (mod(countl,100)=1
    AND countl != 1)
    stat = alterlist(formactlisttemp->qual,(countl+ 99))
   ENDIF
 ...
 IF (( $LOCATION > 1))
  ...
  IF (mod(actual_size,expandreccnt) != 0)
   SET expand_total = (actual_size+ (expandreccnt - mod(actual_size,expandreccnt)))
   ...
   FOR (idx = (actual_size+ 1) TO expand_total)
    ...
    SET formactlisttemp->qual[countl].person_id = formactlisttemp->qual[countl].person_id
   ENDFOR
  ELSE
  ...
  SELECT INTO "NL:"
   FROM encounter e,
    encntr_loc_hist el,
    (dummyt d  WITH seq = value((expand_total/ expandreccnt)))
   PLAN (d
    WHERE assign(expand_start,evaluate(d.seq,1,1,(expand_start+ expandreccnt)))
     AND assign(expand_stop,(expand_start+ (expandreccnt - 1))))
    JOIN (e
   ...
   HEAD e.encntr_id
    IF (e.encntr_id > 0)
     countl = (countl+ 1), formactlisttemp->qual[countl].encntr_id = e.encntr_id, formactlist->qual[
     countl].person_id = e.person_id
    ENDIF
   WITH counter
  ;end select
 ...
 IF (mod(actual_size,expandreccnt) != 0)
  ...
  FOR (idx = (actual_size+ 1) TO expand_total)
   ...
   SET formactlist->qual[idx].person_id = formactlist->qual[countl].person_id
  ENDFOR
 ELSE
  SET expand_total = actual_size
 ENDIF
 ...
 IF (mod(actual_size,expandreccnt) != 0)
  ...
  FOR (idx = (actual_size+ 1) TO expand_total)
   SET sfeinfo->detailqual[idx].person_id = sfeinfo->detailqual[count].person_id
   SET sfeinfo->detailqual[idx].encntr_id = sfeinfo->detailqual[count].encntr_id
  ENDFOR
 ELSE
  SET expand_total = actual_size
 ENDIF
 SELECT INTO "NL:"
  ...
  HEAD o.encntr_id
   nrtgiven = 0
  HEAD o.catalog_cd
   pos = 0, locnum = 0, pos = locateval(locnum,1,size(formlist->formqual,5),o.encntr_id,sfeinfo->
