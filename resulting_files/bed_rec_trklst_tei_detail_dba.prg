CREATE PROGRAM bed_rec_trklst_tei_detail:dba
 ...
 SET reply->collist[3].header_text = "Total Icons"
 ...
 FOR (x = 1 TO plsize)
   IF ((request->paramlist[x].meaning="EDEPTTRLSTICON"))
    SET short_desc = ""
    SET resolution_txt = ""
    SELECT INTO "nl:"
     FROM br_rec b,
      br_long_text bl2
     PLAN (b
      WHERE b.rec_mean="EDEPTTRLSTICON")
      JOIN (bl2
    ...
    ;end select
    SELECT INTO "nl:"
     te.tracking_group_cd, icon_cnt = count(te.track_event_id)
     FROM track_event te
     PLAN (te
     ...
     DETAIL
      ...
      string_value = short_desc, reply->rowlist[tcnt].celllist[2].string_value = uar_get_code_display
      (te.tracking_group_cd),
      reply->rowlist[tcnt].celllist[3].nbr_value = icon_cnt, reply->rowlist[tcnt].celllist[4].
      string_value = "< 25", reply->rowlist[tcnt].celllist[5].string_value = resolution_txt
     FOOT REPORT
