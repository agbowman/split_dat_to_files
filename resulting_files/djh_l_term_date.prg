CREATE PROGRAM djh_l_term_date
 PROMPT
 ...
 ENDIF
 SELECT INTO  $OUTDEV
  actcd = evaluate(p.active_ind,1," ",2,"*"), p.username, p.active_ind,
  ...
  .beg_effective_dt_tm,
  p.updt_dt_tm"@SHORTDATETIME", p.end_effective_dt_tm, newdt = format(p.updt_dt_tm,"yyyy-mm-dd"),
  p.updt_id"#########", p.updt_task, p.create_prsnl_id"#########",
  p1.name_full_formatted, newdays = format( $DYBK,"##")
  FROM prsnl p,
   prsnl p1
  PLAN (p
  ...
  ORDER BY p.username, newdt, p.name_full_formatted
  HEAD REPORT
  ...
  HEAD PAGE
   ...
   CALL print(calcpos(264,(y_pos+ 0))), "List all TERM and/or Suppended Nursing Student IDs", row + 1,
   ...
   CALL print(calcpos(447,(y_pos+ 27))), "Start",
   CALL print(calcpos(450,(y_pos+ 36))),
   "Date",
   CALL print(calcpos(486,(y_pos+ 36))), "|  Date / Time  |",
   CALL print(calcpos(510,(y_pos+ 27))), "Change", row + 1,
   CALL print(calcpos(583,(y_pos+ 36))), "Created / Changed by", row + 1,
   y_pos = (y_pos+ 60)
  DETAIL
   IF (((y_pos+ 98) >= 612))
    y_pos = 0, BREAK
   ...
   username1 = substring(1,12,p.username), name_full_formatted1 = substring(1,30,p
    .name_full_formatted), name_full_formatted2 = substring(1,30,p1.name_full_formatted),
   row + 1, "{F/0}{CPI/14}", row + 1,
   CALL print(calcpos(22,(y_pos+ 0))), actcd, statcd = evaluate(p.active_status_cd,188.00,"",192.00,
    "*",
    194.00,"S"),
   row + 1,
   CALL print(calcpos(26,(y_pos+ 0))), statcd,
   row + 1,
   CALL print(calcpos(41,(y_pos+ 0))), username1,
   CALL print(calcpos(108,(y_pos+ 0))), name_full_formatted1,
   CALL print(calcpos(440,(y_pos+ 0))),
   p.beg_effective_dt_tm,
   CALL print(calcpos(486,(y_pos+ 0))), p.updt_dt_tm,
   CALL print(calcpos(578,(y_pos+ 0))), name_full_formatted2, y_pos = (y_pos+ 14)
  FOOT PAGE
