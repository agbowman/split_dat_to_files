CREATE PROGRAM dm2_freetext_chg_child:dba
 ...
 DECLARE dfcc_cnt = i4
 SET dfcc_cnt =  $1
 IF ((del_holder->qual[dfcc_cnt].table_name IN ("ADDRESS", "PHONE")))
  CALL parser(concat('select into "NL:" from ',del_holder->qual[dfcc_cnt].table_name," d"),0)
  CALL parser(concat(del_holder->qual[dfcc_cnt].pk_where,' and parent_entity_name = "ORGANIZATION"'),
   0)
  CALL parser(concat("detail del_holder->qual[dfcc_cnt].o_id = d.parent_entity_id ",
    " del_holder->qual[dfcc_cnt].o_ind = 1 with nocounter go"),1)
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Error selecting ",del_holder->qual[dfcc_cnt].table_name," rows")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
  ...
  ELSE
   IF ((del_holder->qual[dfcc_cnt].o_ind=1))
    SELECT INTO "NL:"
     FROM organization o
     WHERE (o.organization_id=del_holder->qual[dfcc_cnt].o_id)
      AND o.org_class_cd=ddcr_free_text
     DETAIL
      del_holder->qual[dfcc_cnt].d_ind = 1
     WITH nocounter
    ;end select
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Error selecting free text organization rows"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ROLLBACK
    SET readme_data->status = "F"
    SET readme_data->message = dm_err->emsg
    SET ddcr_error_ind = 1
   ELSE
    IF ((del_holder->qual[dfcc_cnt].d_ind=1))
     DELETE  FROM dm_chg_log d
      WHERE (d.log_id=del_holder->qual[dfcc_cnt].log_id)
      WITH nocounter
     ;end delete
     IF (check_error(dm_err->eproc)=1)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = concat("Error deleting free text ",del_holder->qual[dfcc_cnt].table_name,
       " rows")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ROLLBACK
      SET readme_data->status = "F"
   ...
   ENDIF
  ENDIF
 ELSEIF ((del_holder->qual[dfcc_cnt].table_name="ORGANIZATION"))
  CALL parser('select into "NL:" from organization o',0)
  CALL parser(concat(del_holder->qual[ddcr_del_loop].pk_where," and o.org_class_cd = ddcr_free_text"),
   0)
  CALL parser(" detail del_holder->qual[ddcr_del_loop].d_ind = 1 with nocounter go",1)
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Error selecting organization rows"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   SET readme_data->status = "F"
   SET readme_data->message = dm_err->emsg
   SET ddcr_error_ind = 1
  ELSE
   IF ((del_holder->qual[ddcr_del_loop].d_ind=1))
    DELETE  FROM dm_chg_log d
     WHERE (d.log_id=del_holder->qual[ddcr_del_loop].log_id)
     WITH nocounter
    ;end delete
    IF (check_error(dm_err->eproc)=1)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Error deleting free text organization rows"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ROLLBACK
     SET readme_data->status = "F"
     SET readme_data->message = dm_err->emsg
