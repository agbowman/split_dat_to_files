CREATE PROGRAM dm_rmc_refresh_r:dba
 ...
 SUBROUTINE get_lock(i_info_domain,i_info_name,i_retry_limit,io_reply)
   ...
   ENDIF
 END ;Subroutine
 DECLARE drrr_background_ind = i2 WITH protect, noconstant(0)
 DECLARE drrr_loop = i2 WITH protect, noconstant(0)
 DECLARE drrr_db_name = vc WITH protect, noconstant("")
 DECLARE drrr_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drrr_continue_cnt = i4 WITH protect, noconstant(0)
 DECLARE drrr_status = c1 WITH protect, noconstant("F")
 FREE RECORD drrr_r_tables
 RECORD drrr_r_tables(
   1 size = i4
   1 qual[*]
     2 r_tab_name = vc
     2 table_name = vc
 )
 FREE RECORD drrr_request
 RECORD drrr_request(
   1 table_name = vc
   1 trig_ind = i2
   1 database_name = vc
 )
 FREE RECORD drrr_reply
 RECORD drrr_reply(
   1 status_data
     2 status = c1
     2 logfile = vc
 )
 IF (validate(reply->status,"Z")="Z")
  FREE RECORD reply
  RECORD reply(
    1 status = c1
    1 message = vc
 ...
 ENDIF
 SET dm_err->eproc = "Starting dm_rmc_refresh_r"
 IF (check_logfile("dm_rmc_refresh_r",".log","DM_RMC_REFRESH_R LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Checking input parameters"
 IF (reflect(parameter(1,0)) != "F*"
  AND reflect(parameter(1,0)) != "I*")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Expected syntax:  dm_rmc_refresh_r <environment_id>")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drrr_env_id =  $1
 SET dm_err->eproc = "Querying for database name."
 SELECT INTO "nl:"
  FROM v$database v
  DETAIL
   drrr_db_name = v.name
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Gathering list of $R tables currently in the domain."
 SELECT INTO "nl:"
  FROM user_tables ut
  WHERE ut.table_name="*$R"
  DETAIL
   drrr_r_tables->size = (drrr_r_tables->size+ 1)
   IF (mod(drrr_r_tables->size,25)=1)
    stat = alterlist(drrr_r_tables->qual,(drrr_r_tables->size+ 24))
   ENDIF
   drrr_r_tables->qual[drrr_r_tables->size].r_tab_name = ut.table_name
  FOOT REPORT
   stat = alterlist(drrr_r_tables->qual,drrr_r_tables->size)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ((drrr_r_tables->size > 0))
  SET stat = initrec(task_info)
  SET task_info->environment_id = drrr_env_id
  SET task_info->process_name = drtw_process_name
  SET task_info->total = drrr_r_tables->size
  SET stat = alterlist(task_info->qual,drrr_r_tables->size)
  FOR (drrr_loop = 1 TO drrr_r_tables->size)
    SET drrr_r_tables->qual[drrr_loop].table_name = get_reg_tab_name(drrr_r_tables->qual[drrr_loop].
     r_tab_name,"")
    SET dm_err->eproc = "Inserting list of $R refreshes into the queue."
    SET task_info->qual[drrr_loop].task_desc = concat("Refreshing $R table: ",drrr_r_tables->qual[
     drrr_loop].r_tab_name)
    SET task_info->qual[drrr_loop].task_level = (drtw_task_level+ 50)
    SET task_info->qual[drrr_loop].task_name =
    'execute DM_RMC_CREATE_R_TABLE with replace("REPLY","DRR_REPLY"),replace("REQUEST","DRR_REQUEST") go'
    SET task_info->qual[drrr_loop].task_reply = concat(
     "<REC_DEF>record DRR_REPLY (1 status_data 2 status = c1 2 logfile = vc) go</REC_DEF>",
     "<ERR_IND>DRR_REPLY->status_data.status</ERR_IND><ERR_MSG> </ERR_MSG><NO_ERR_RESULT>S</NO_ERR_RESULT>",
     "<ERR_TYPE>VC</ERR_TYPE>")
    SET task_info->qual[drrr_loop].task_request = concat(
     "<REC_DEF>record DRR_REQUEST( 1 database_name = vc 1 table_name = vc 1 trig_ind = i2) go</REC_DEF>",
     "<VAR_DEF>declare drcrt_precreate_ind = i2 go</VAR_DEF>",
     "<VAL_SET>set drcrt_precreate_ind = 1 go</VAL_SET>",
     "<VAL_SET>set DRR_REQUEST->database_name = '",drrr_db_name,
     "' go</VAL_SET>","<VAL_SET>set DRR_REQUEST->table_name = '",drrr_r_tables->qual[drrr_loop].
     table_name,"' go</VAL_SET>","<VAL_SET>set DRR_REQUEST->trig_ind = 1 go</VAL_SET>")
  ENDFOR
  SET task_info->total = (task_info->total+ 1)
  SET stat = alterlist(task_info->qual,task_info->total)
  SET task_info->qual[task_info->total].task_desc = "Refreshing all $Rs"
  SET task_info->qual[task_info->total].task_level = (drtw_task_level+ 51)
  SET task_info->qual[task_info->total].task_name = "execute ORAGEN3 '*$R' go"
  SET dm_err->eproc = "Inserting refresh r table tasks."
  SET drrr_status = drtq_insert_task_process(task_info,0)
  IF (drrr_status="F")
 ...
 ENDIF
#exit_program
 CALL final_disp_msg("dm_rmc_refresh_r")
END GO
