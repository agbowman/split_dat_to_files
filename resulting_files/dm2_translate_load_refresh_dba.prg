CREATE PROGRAM dm2_translate_load_refresh:dba
 ...
 DECLARE get_header_info(null) = i2
 DECLARE populate_tables(null) = null
 DECLARE populate_tbl_columns(null) = null
 DECLARE display_tgt_rec_struct(null) = null
 IF (check_logfile("dm2_trn_load_refresh",".log","DM2_TRANSLATE_LOAD_REFRESH")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Entering Translate Load REFRESH Process...."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (initial_validation(null)=0)
  GO TO exit_program
 ENDIF
 CALL populate_tables(null)
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 ...
 SUBROUTINE initial_validation(null)
   SET dm_err->eproc = "Performing Initial Validation for Translate/Load REFRESH Process"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((tgtsch->tbl_cnt=0))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "TGTSCH record is empty."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    RETURN(0)
   ENDIF
   IF ((drr_clin_copy_data->temp_location=" "))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Schema files location not set."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    RETURN(0)
   ENDIF
   IF ((dm2_sys_misc->cur_os="AXP"))
   ...
   IF (open_sch_files(0)=0)
    RETURN(0)
   ENDIF
   IF (get_header_info(null)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE get_header_info(null)
   DECLARE ghi_admin_load = i2 WITH protect, noconstant(0)
   DECLARE ghi_source_rdbms = vc WITH protect, noconstant(" ")
   ...
   SELECT INTO "nl:"
    ...
    FROM dmheader h
    DETAIL
     ghi_admin_load = h.admin_load_ind, ghi_source_rdbms = h.source_rdbms
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSE
    ...
    ENDIF
   ENDIF
   IF ((ghi_source_rdbms != tgtsch->source_rdbms))
    SET dm_err->err_ind = 1
   ...
   IF (ghi_admin_load=1)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Admin_load_ind should not be 1 for schema files captured during refresh"
    SET dm_err->eproc = "Validating Schema Header Information"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE populate_tables(null)
   IF (dm2_set_autocommit(1)=0)
    RETURN(0)
   ENDIF
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     pres_idx = 0, pres_idx = locateval(pres_idx,1,drr_preserved_tables_data->cnt,t.table_name,
      drr_preserved_tables_data->tbl[pres_idx].table_name)
     IF (pres_idx > 0
      AND (drr_preserved_tables_data->tbl[pres_idx].refresh_ind=1))
      tbl_ndx = 0, tbl_ndx = locateval(tbl_ndx,1,tgtsch->tbl_cnt,t.table_name,tgtsch->tbl[tbl_ndx].
       tbl_name)
      IF (tbl_ndx=0
       AND (drr_preserved_tables_data->tbl[pres_idx].group="AUTOTESTER"))
       dm_err->err_ind = 1, dm_err->emsg = concat("Table ",t.table_name," is not found in Tgtsch."),
       CALL cancel(1)
      ENDIF
      drr_preserved_tables_data->tbl[pres_idx].tgtsch_idx = tbl_ndx
      IF ((drr_preserved_tables_data->tbl[pres_idx].tgtsch_idx > 0)
       AND validate(t.long_tspace," ") != "N/A")
       tgtsch->tbl[tbl_ndx].long_tspace = validate(t.long_tspace," ")
      ENDIF
      IF (tbl_ndx > 0
       AND (drr_preserved_tables_data->tbl[pres_idx].group="AUTOTESTER"))
       IF ((tgtsch->tbl[tbl_ndx].tbl_col_cnt=0))
        tgtsch->tbl[tbl_ndx].schema_date = t.schema_date, tgtsch->tbl[tbl_ndx].schema_instance = t
        .schema_instance, tgtsch->tbl[tbl_ndx].pct_increase = t.pct_increase,
        tgtsch->tbl[tbl_ndx].pct_used = t.pct_used, tgtsch->tbl[tbl_ndx].pct_free = t.pct_free,
        tgtsch->tbl[tbl_ndx].bytes_allocated = validate(t.bytes_allocated,0.0),
        tgtsch->tbl[tbl_ndx].bytes_used = validate(t.bytes_used,0.0), tgtsch->tbl[tbl_ndx].
        tspace_name = t.tablespace_name, tgtsch->tbl[tbl_ndx].ind_tspace = validate(t.index_tspace,
         " "),
        tgtsch->tbl[tbl_ndx].long_tspace = validate(t.long_tspace," "), tgtsch->tbl[tbl_ndx].init_ext
         = validate(t.init_ext,0.0), tgtsch->tbl[tbl_ndx].next_ext = validate(t.next_ext,0.0)
        IF ((tgtsch->tbl[tbl_ndx].pct_free > 60)
         AND (tgtsch->tbl[tbl_ndx].pct_used=0.0))
         tgtsch->tbl[tbl_ndx].pct_used = 1.0
        ENDIF
        tgtsch->tbl[tbl_ndx].dext_mgmt = validate(t.tspace_type," "), tgtsch->tbl[tbl_ndx].lext_mgmt
         = validate(t.long_tspace_type," "), tgtsch->tbl[tbl_ndx].max_ext = validate(t.max_ext,0.0),
        tgtsch->tbl[tbl_ndx].alpha_feature_nbr = tgtsch->alpha_feature_nbr, tgtsch->tbl[tbl_ndx].
        feature_number = t.feature_number, tgtsch->tbl[tbl_ndx].updt_dt_tm = t.updt_dt_tm,
        tgtsch->tbl[tbl_ndx].afd_schema_instance = 0, tgtsch->tbl[tbl_ndx].pull_from_afd = 0, tgtsch
        ->tbl[tbl_ndx].rowid_col_fnd = 0,
        tgtsch->tbl[tbl_ndx].xrid_ind_fnd = 0, tgtsch->tbl[tbl_ndx].full_tbl_name = validate(td
         .full_table_name," "), tgtsch->tbl[tbl_ndx].suff_tbl_name = validate(td.suffixed_table_name,
         " "),
        tgtsch->tbl[tbl_ndx].reference_ind = validate(td.reference_ind,0), tgtsch->tbl[tbl_ndx].
        table_suffix = validate(td.table_suffix," ")
       ENDIF
      ENDIF
     ENDIF
    WITH nocounter
   ...
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ELSEIF (curqual=0)
    SET dm_err->eproc = "No Preserved Table Need to be refreshed"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   IF (dm2_set_autocommit(0)=0)
    RETURN(0)
 ...
 SUBROUTINE populate_tbl_columns(null)
   ...
   DECLARE ptc_def_ni_idx = i4 WITH protect, noconstant(0)
   DECLARE pres_col_idx = i4 WITH protect, noconstant(0)
   ...
   SELECT INTO "nl:"
    ...
    HEAD REPORT
     MACRO (data_default_cleanup)
      ...
      ENDIF
     ENDMACRO
     , tbl_ndx = 0, pres_idx = 0
    HEAD tc.table_name
     tgt_col = 0, tbl_ndx = 0, pres_idx = 0,
     pres_idx = locateval(pres_idx,1,drr_preserved_tables_data->cnt,tc.table_name,
      drr_preserved_tables_data->tbl[pres_idx].table_name)
    DETAIL
     IF ( NOT (validate(tc.virtual_column,"NO")="YES"
      ...
      IF (pres_idx > 0)
       IF ((dm_err->debug_flag > 2))
        CALL echo(concat("Populate column for preserve table ",tc.table_name," column ",tc
         .column_name))
       ENDIF
       ...
       col_cnt+ 1), stat = alterlist(drr_preserved_tables_data->tbl[pres_idx].col,
        drr_preserved_tables_data->tbl[pres_idx].col_cnt), pres_col_idx = drr_preserved_tables_data->
       tbl[pres_idx].col_cnt,
       drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].col_name = tc.column_name,
       drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].data_type = tc.data_type
       IF ((((drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].data_type="LONG")) OR ((
       drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].data_type="LONG RAW"))) )
        drr_preserved_tables_data->tbl[pres_idx].long_cols_exist = 1
       ENDIF
       drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].data_length = tc.data_length,
       drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].data_default_ni = validate(tc
        .data_default_ni,1)
       IF ((drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].data_default_ni=0))
        data_default_cleanup, drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].
        data_default_ni = ptc_def_ni_idx
       ENDIF
       IF ((drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].data_default_ni=0))
        drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].data_default = tc.data_default
       ENDIF
       drr_preserved_tables_data->tbl[pres_idx].col[pres_col_idx].nullable = tc.nullable
      ENDIF
      IF ((drr_preserved_tables_data->tbl[pres_idx].refresh_ind=1)
       AND (drr_preserved_tables_data->tbl[pres_idx].tgtsch_idx > 0))
       tbl_ndx = drr_preserved_tables_data->tbl[pres_idx].tgtsch_idx, tgt_col = 0, tgt_col =
       locateval(tgt_col,1,tgtsch->tbl[tbl_ndx].tbl_col_cnt,tc.column_name,tgtsch->tbl[tbl_ndx].
        tbl_col[tgt_col].col_name)
       IF (tgt_col=0)
        IF ((dm_err->debug_flag > 2))
         CALL echo(concat("Populate column for preserve table ",tc.table_name," column ",tc
          .column_name))
        ENDIF
        tgtsch->tbl[tbl_ndx].tbl_col_cnt = (tgtsch->tbl[tbl_ndx].tbl_col_cnt+ 1), stat = alterlist(
         tgtsch->tbl[tbl_ndx].tbl_col,tgtsch->tbl[tbl_ndx].tbl_col_cnt), tgtsch->tbl[tbl_ndx].
        tbl_col[tgtsch->tbl[tbl_ndx].tbl_col_cnt].col_name = tc.column_name,
        tgtsch->tbl[tbl_ndx].tbl_col[tgtsch->tbl[tbl_ndx].tbl_col_cnt].col_seq = tc.column_seq,
        tgtsch->tbl[tbl_ndx].tbl_col[tgtsch->tbl[tbl_ndx].tbl_col_cnt].nullable = tc.nullable, tgtsch
        ->tbl[tbl_ndx].tbl_col[tgtsch->tbl[tbl_ndx].tbl_col_cnt].data_length = tc.data_length,
        tgtsch->tbl[tbl_ndx].tbl_col[tgtsch->tbl[tbl_ndx].tbl_col_cnt].data_type = tc.data_type,
        tgtsch->tbl[tbl_ndx].tbl_col[tgtsch->tbl[tbl_ndx].tbl_col_cnt].data_default_ni = validate(tc
         .data_default_ni,1)
        IF ((tgtsch->tbl[tbl_ndx].tbl_col[tgtsch->tbl[tbl_ndx].tbl_col_cnt].data_default_ni=0))
         data_default_cleanup, tgtsch->tbl[tbl_ndx].tbl_col[tgtsch->tbl[tbl_ndx].tbl_col_cnt].
         data_default_ni = ptc_def_ni_idx
        ENDIF
        IF ((tgtsch->tbl[tbl_ndx].tbl_col[tgtsch->tbl[tbl_ndx].tbl_col_cnt].data_default_ni=0))
         tgtsch->tbl[tbl_ndx].tbl_col[tgtsch->tbl[tbl_ndx].tbl_col_cnt].data_default = tc
         .data_default
        ENDIF
        drr_preserved_tables_data->tbl[pres_idx].extra_pre_cols = 1
       ELSE
        IF (drr_restore_col_checks("T",tbl_ndx,tgt_col,pres_idx,pres_col_idx,
         tbl_ndx,tgt_col)=0)
         dm_err->eproc = "Performing restore preserved table column checks", dm_err->err_ind = 1,
         CALL cancel(1)
        ENDIF
       ENDIF
      ENDIF
    ...
    SELECT INTO value(dtl_log_file1)
     ...
     DETAIL
      ...
      IF ((tgtsch->tbl[d.seq].tbl_col_cnt > 0))
       row + 1, "COLUMNS"
       FOR (tc_cnt = 1 TO tgtsch->tbl[d.seq].tbl_col_cnt)
         row + 1, col 3, "------------------",
         row + 1, col 3, "column name = ",
         tgtsch->tbl[d.seq].tbl_col[tc_cnt].col_name, row + 1, col 3,
         "data type = ", tgtsch->tbl[d.seq].tbl_col[tc_cnt].data_type, row + 1,
         col 3, "data length = ", tgtsch->tbl[d.seq].tbl_col[tc_cnt].data_length,
         dd = substring(1,70,tgtsch->tbl[d.seq].tbl_col[tc_cnt].data_default), row + 1, col 3,
         "data default = ", dd, row + 1,
         col 3, "data default ni = ", tgtsch->tbl[d.seq].tbl_col[tc_cnt].data_default_ni,
         row + 1, col 3, "nullable = ",
         tgtsch->tbl[d.seq].tbl_col[tc_cnt].nullable, row + 1, col 3,
         "column seq = ", tgtsch->tbl[d.seq].tbl_col[tc_cnt].col_seq, row + 1,
         col 3, "virtual column = ", tgtsch->tbl[d.seq].tbl_col[tc_cnt].virtual_column
       ENDFOR
      ENDIF
      IF ((tgtsch->tbl[d.seq].ind_cnt > 0))
       row + 2, "INDEXES"
       FOR (icnt = 1 TO tgtsch->tbl[d.seq].ind_cnt)
         row + 1, col 3, "------------------",
         row + 1, col 3, "index name = ",
         tgtsch->tbl[d.seq].ind[icnt].ind_name, row + 1, col 3,
         "full index name = ", tgtsch->tbl[d.seq].ind[icnt].full_ind_name, row + 1,
         col 3, "index type = ", tgtsch->tbl[d.seq].ind[icnt].index_type,
         row + 1, col 3, "unique_ind = ",
         tgtsch->tbl[d.seq].ind[icnt].unique_ind, row + 1, col 3,
         "index column count = ", tgtsch->tbl[d.seq].ind[icnt].ind_col_cnt, row + 1,
         col 3, "pct increase = ", tgtsch->tbl[d.seq].ind[icnt].pct_increase,
         row + 1, col 3, "pct_free = ",
         tgtsch->tbl[d.seq].ind[icnt].pct_free, row + 1, col 3,
         "init ext = ", tgtsch->tbl[d.seq].ind[icnt].init_ext, row + 1,
         col 3, "next ext = ", tgtsch->tbl[d.seq].ind[icnt].next_ext,
         row + 1, col 3, "unique ind = ",
         tgtsch->tbl[d.seq].ind[icnt].unique_ind, row + 1, col 3,
         "bytes allocated = ", tgtsch->tbl[d.seq].ind[icnt].bytes_allocated, row + 1,
         col 3, "bytes used = ", tgtsch->tbl[d.seq].ind[icnt].bytes_used
         FOR (ic_cnt = 1 TO tgtsch->tbl[d.seq].ind[icnt].ind_col_cnt)
           row + 1, col 6, "column name = ",
           tgtsch->tbl[d.seq].ind[icnt].ind_col[ic_cnt].col_name, row + 1, col 6,
           "column position = ", tgtsch->tbl[d.seq].ind[icnt].ind_col[ic_cnt].col_position
         ENDFOR
       ENDFOR
      ENDIF
      IF ((tgtsch->tbl[d.seq].cons_cnt > 0))
       row + 2, "CONSTRAINTS"
       FOR (ccnt = 1 TO tgtsch->tbl[d.seq].cons_cnt)
         row + 1, col 3, "------------------",
         row + 1, col 3, "constraint name = ",
         tgtsch->tbl[d.seq].cons[ccnt].cons_name, row + 1, col 3,
         "full constraint name = ", tgtsch->tbl[d.seq].cons[ccnt].full_cons_name, row + 1,
         col 3, "constraint type = ", tgtsch->tbl[d.seq].cons[ccnt].cons_type,
         row + 1, col 3, "parent table = ",
         tgtsch->tbl[d.seq].cons[ccnt].parent_table, row + 1, col 3,
         "status_ind = ", tgtsch->tbl[d.seq].cons[ccnt].status_ind, row + 1,
         col 3, "r_constraint_name = ", tgtsch->tbl[d.seq].cons[ccnt].r_constraint_name,
         row + 1, col 3, "parent table columns = ",
         tgtsch->tbl[d.seq].cons[ccnt].parent_table_columns, row + 1, col 3,
         "constraint column count = ", tgtsch->tbl[d.seq].cons[ccnt].cons_col_cnt, row + 1,
         col 3, "delete_rule = ", tgtsch->tbl[d.seq].cons[ccnt].delete_rule
         FOR (cc_cnt = 1 TO tgtsch->tbl[d.seq].cons[ccnt].cons_col_cnt)
           row + 1, col 6, "column name = ",
           tgtsch->tbl[d.seq].cons[ccnt].cons_col[cc_cnt].col_name, row + 1, col 6,
           "column position = ", tgtsch->tbl[d.seq].cons[ccnt].cons_col[cc_cnt].col_position
         ENDFOR
       ENDFOR
      ENDIF
     WITH nocounter, format = variable, formfeed = none,
...
#exit_program
 ...
 ENDIF
 SET dm_err->eproc = "Exiting Target Record Structure Load REFRESH Process"
 CALL final_disp_msg("dm2_trn_load_refresh")
END GO
