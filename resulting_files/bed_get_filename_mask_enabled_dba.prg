CREATE PROGRAM bed_get_filename_mask_enabled:dba
 RECORD reply(
   1 error_code = i2
   1 status_data
     2 status = c1
 ...
 SUBROUTINE check_log_level(arg_log_level)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "BED_GET_FILENAME_MASK_ENABLED"
 DECLARE new_schema = i2 WITH noconstant(0)
 DECLARE filename_setup_script_status = i2 WITH noconstant(0)
 DECLARE multi_tenant = i2 WITH noconstant(0)
 SET reply->status_data.status = "F"
 CALL echo(build2("default_ind",default_ind))
 SELECT INTO "nl:"
  dm.info_number
  FROM dm_info dm
  ...
  DETAIL
   filename_setup_script_status = dm.info_number
  WITH nocounter
 ;end select
 IF (checkdic("CR_MASK.DEFAULT_IND","A",0)=2
  AND checkdic("CR_MASK.PUBLISH_IND","A",0)=2
  AND checkdic("CR_MASK.LOGICAL_DOMAIN_ID","A",0)=2)
  SET new_schema = 1
 ENDIF
 CALL echo(build("schema: ",new_schema))
 SELECT INTO "nl:"
 ...
 IF (curqual > 1)
  SET multi_tenant = 1
 ENDIF
 CALL echo(build2("multi_ten: ",multi_tenant))
 SET reply->error_code = 0
 IF (new_schema=1)
  IF (multi_tenant=0)
   SET reply->status_data.status = "S"
  ELSE
   IF (filename_setup_script_status=1)
    SET reply->status_data.status = "S"
   ELSEIF (filename_setup_script_status=0)
    SET reply->error_code = 1
   ELSE
    SET reply->error_code = 2
   ENDIF
  ENDIF
 ELSE
  SET reply->error_code = 3
 ENDIF
 SET reply->status_data.status = "S"
...
#exit_script
 IF (validate(debug_ind,0))
  CALL echorecord(reply)
 ENDIF
 CALL log_message("End of script: bed_get_filename_mask_enabled.",log_level_debug)
END GO
