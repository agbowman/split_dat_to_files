CREATE PROGRAM cp_restore_chart_format:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 UPDATE  FROM chart_format cf
  SET cf.active_ind = 1, cf.active_status_cd = reqdata->active_status_cd, cf.active_status_prsnl_id
    = reqinfo->updt_id,
 ...
 IF (found=1)
  UPDATE  FROM format_org_reltn f
   SET f.active_ind = 1, f.active_status_cd = reqdata->active_status_cd, f.active_status_prsnl_id =
    reqinfo->updt_id,
