CREATE PROGRAM ce_blob_summary_upd:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 SET error_code = 0
 UPDATE  FROM ce_blob_summary t,
   (dummyt d  WITH seq = value(reply->array_size))
  SET t.ce_blob_summary_id = evaluate2(
    IF ((request->lst[d.seq].ce_blob_summary_id=- (1))) 0
    ELSE request->lst[d.seq].ce_blob_summary_id
    ENDIF
    ), t.blob_summary_id = evaluate2(
    IF ((request->lst[d.seq].blob_summary_id=- (1))) 0
    ELSE request->lst[d.seq].blob_summary_id
    ENDIF
  ...
  PLAN (d)
   JOIN (t
   WHERE (t.ce_blob_summary_id=request->lst[d.seq].ce_blob_summary_id))
