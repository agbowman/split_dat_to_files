CREATE PROGRAM djh_phys_detail_rpt_v2x:dba
 PROMPT
 ...
 SELECT INTO  $1
  ...
  HEAD REPORT
   ...
   position = fillstring(40," "), phys_flg = fillstring(3," ")
  HEAD PAGE
   col 5, line, row + 1,
  ...
  DETAIL
   ...
   "Aliases: ", col 70, "Start DT  End DT",
   row + 1
   ...
   IF (size(md_list->qual[d.seq].address,5) > 0)
    FOR (y = 1 TO size(md_list->qual[d.seq].address,5))
      ...
      ENDIF
      , col 4, actaddr,
      col 10, md_list->qual[d.seq].address[y].address_type_disp, actaddr,
      col 30, md_list->qual[d.seq].address[y].street1, row + 1
      IF ((md_list->qual[d.seq].address[y].street2 > " "))
       col 30, md_list->qual[d.seq].address[y].street2, row + 1
      ENDIF
