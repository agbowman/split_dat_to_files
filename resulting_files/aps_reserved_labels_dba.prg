CREATE PROGRAM aps_reserved_labels:dba
 RECORD reply(
   1 node = vc
   1 print_status_data
...
#script
 ...
 SET x = 1
 SET stat = alterlist(data->qual,(request->label_cnt * request->case_cnt))
 SET stat = alterlist(data->resrc[1].label,(request->label_cnt * request->case_cnt))
 SELECT INTO "nl:"
  p.name_full_formatted
  FROM (dummyt d  WITH seq = value(request->case_cnt)),
   person p
  PLAN (d)
   JOIN (p
   WHERE (p.person_id=request->qual[d.seq].person_id))
  DETAIL
   FOR (cnt = 1 TO request->label_cnt)
     data->qual[x].accession_nbr = request->qual[d.seq].case_nbr, data->resrc[1].label[x].
     accession_nbr = data->qual[x].accession_nbr
     IF ((request->qual[d.seq].person_id=0))
      data->qual[x].person_name = "NOT DESIGNATED"
     ELSE
      data->qual[x].person_name = p.name_full_formatted
     ENDIF
     data->resrc[1].label[x].name_full_formatted = data->qual[x].person_name, data->resrc[1].label[x]
     .fmt_accession_nbr = uar_fmt_accession(data->qual[x].accession_nbr,size(data->qual[x].
       accession_nbr)), x += 1
   ENDFOR
  WITH nocounter, outerjoin = d
 ;end select
 SET data->current_dt_tm_string = format(cnvtdatetime(sysdate),"@SHORTDATE;;D")
 SELECT INTO "nl:"
