CREATE PROGRAM dm_ocmb_pregnancy_instance:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  IF (debug_ind=1)
   CALL echo("********DM_OCMB_PREGNANCY_INSTANCE - maintaining exception")
  ENDIF
  SET stat = alterlist(dcem_request->qual,1)
  SET dcem_request->qual[1].parent_entity = "ORGANIZATION"
  SET dcem_request->qual[1].child_entity = "PREGNANCY_INSTANCE"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_OCMB_PREGNANCY_INSTANCE"
 ...
 ENDIF
 IF (debug_ind=1)
  CALL echo(build("********DM_OCMB_PREGNANCY_INSTANCE - from_xxx_id = ",request->xxx_combine[icombine
    ].from_xxx_id))
  CALL echo(build("********DM_OCMB_PREGNANCY_INSTANCE - to_xxx_id = ",request->xxx_combine[icombine].
    to_xxx_id))
 ENDIF
 SELECT INTO "nl:"
  frm.*
 ...
 SUBROUTINE updt_from(s_uf_pk_id)
   ...
   IF (curqual=0)
    ...
    SET request->error_message = build(
     "Could not update PREGNANCY_INSTANCE record with pregnancy_instance_id = ",s_uf_pk_id)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
