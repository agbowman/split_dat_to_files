CREATE PROGRAM bhs_athn_cancel_lab_rad_ord_v2
 DECLARE c_order_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",6003,"ORDER"))
 DECLARE c_cancel_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",6003,"CANCEL"))
 DECLARE c_discontinue_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",6003,"DISCONTINUE"))
 DECLARE c_void_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",6003,"DELETE"))
 DECLARE callupdorderprogram(null) = i2
 DECLARE callmanageorders(null) = i2
 DECLARE is_recurring_parent_ind = f8 WITH protect, noconstant(0)
 SELECT INTO "NL:"
  FROM orders o
  PLAN (o
   WHERE (o.protocol_order_id= $2))
  HEAD o.order_id
   is_recurring_parent_ind = 1
  WITH nocounter, time = 30
 ;end select
 IF (is_recurring_parent_ind=1)
 ...
 SUBROUTINE callmanageorders(null)
   ...
   DECLARE action_type_cd = f8 WITH protect, constant(cnvtint( $4))
   DECLARE orig_order_tz = i4 WITH protect, noconstant(0)
   DECLARE order_updt_cnt = i4 WITH protect, noconstant(0)
   DECLARE communication_type_cd = f8 WITH protect, noconstant(0)
   DECLARE report_status = vc WITH protect, noconstant("F")
   DECLARE report_error_str = vc WITH protect, noconstant("")
   DECLARE html_tag = vc WITH protect, noconstant("")
   ...
   SELECT INTO "NL:"
    ...
    HEAD o.order_id
     orig_order_tz = o.orig_order_tz, order_updt_cnt = o.updt_cnt
    HEAD oa.order_id
   ...
   SET i_request->prsnl_id = cnvtint( $3)
   ...
   RECORD req680217(
     ...
     1 action_group[*]
       ...
       2 cancel_actions[*]
         3 order_id = f8
         3 version = i4
         3 action_initiated_dt_tm = dq8
         3 communication_type_cd = f8
         3 responsible_provider_id = f8
         3 implicitly_reviewed_by_rx_ind = i2
         3 modified_details[*]
           4 order_entry_field_id = f8
           4 boolean_value[*]
             5 boolean_ind = i2
             5 text = vc
           4 date_values[*]
   ...
   DECLARE cancelreasonoefieldid = f8 WITH protect, noconstant(0.0)
   DECLARE dcreasonoefieldid = f8 WITH protect, noconstant(0.0)
   DECLARE voidreasonoefieldid = f8 WITH protect, noconstant(0.0)
   SELECT INTO "NL:"
    FROM oe_field_meaning ofm,
     order_entry_fields oef
    PLAN (ofm
     WHERE ofm.oe_field_meaning IN ("CANCELREASON", "DCREASON"))
     JOIN (oef
     WHERE oef.oe_field_meaning_id=ofm.oe_field_meaning_id)
    HEAD oef.oe_field_id
     IF (ofm.oe_field_meaning="CANCELREASON")
      cancelreasonoefieldid = oef.oe_field_id
     ELSEIF (ofm.oe_field_meaning="DCREASON")
      dcreasonoefieldid = oef.oe_field_id
     ENDIF
    WITH nocounter, time = 30
   ;end select
   SELECT INTO "NL:"
    ...
    PLAN (oef
     WHERE oef.description="Void Reason")
    HEAD oef.oe_field_id
     IF (oef.description="Void Reason")
      voidreasonoefieldid = oef.oe_field_id
     ENDIF
    WITH nocounter, time = 30
   ;end select
   SET req680217->acting_personnel.personnel_id = i_request->prsnl_id
   SET req680217->acting_personnel.personnel_tz = orig_order_tz
   SET stat = alterlist(req680217->action_group,1)
   IF (action_type_cd=c_cancel_cd)
    SET stat = alterlist(req680217->action_group[1].cancel_actions,1)
    SET req680217->action_group[1].cancel_actions[1].order_id =  $2
    SET req680217->action_group[1].cancel_actions[1].version = order_updt_cnt
    SET req680217->action_group[1].cancel_actions[1].action_initiated_dt_tm = cnvtdatetime( $6)
    SET req680217->action_group[1].cancel_actions[1].communication_type_cd = communication_type_cd
    SET req680217->action_group[1].cancel_actions[1].responsible_provider_id = i_request->prsnl_id
    SET stat = alterlist(req680217->action_group[1].cancel_actions[1].modified_details,1)
    SET req680217->action_group[1].cancel_actions[1].modified_details[1].order_entry_field_id =
    cancelreasonoefieldid
    SET stat = alterlist(req680217->action_group[1].cancel_actions[1].modified_details[1].
     numeric_values,1)
    SET req680217->action_group[1].cancel_actions[1].modified_details[1].numeric_values[1].
    numeric_value = trim(cnvtstring(cnvtint( $5)),3)
    SET req680217->action_group[1].cancel_actions[1].modified_details[1].numeric_values[1].text =
    uar_get_code_display(cnvtreal( $5))
   ELSEIF (action_type_cd=c_discontinue_cd)
    SET stat = alterlist(req680217->action_group[1].discontinue_actions,1)
    SET req680217->action_group[1].discontinue_actions[1].order_id =  $2
    SET req680217->action_group[1].discontinue_actions[1].version = order_updt_cnt
    SET req680217->action_group[1].discontinue_actions[1].action_initiated_dt_tm = cnvtdatetime( $6)
    SET req680217->action_group[1].discontinue_actions[1].communication_type_cd =
    communication_type_cd
    SET req680217->action_group[1].discontinue_actions[1].responsible_provider_id = i_request->
    prsnl_id
    SET stat = alterlist(req680217->action_group[1].discontinue_actions[1].modified_details,1)
    SET req680217->action_group[1].discontinue_actions[1].modified_details[1].order_entry_field_id =
    dcreasonoefieldid
    SET stat = alterlist(req680217->action_group[1].discontinue_actions[1].modified_details[1].
     numeric_values,1)
    SET req680217->action_group[1].discontinue_actions[1].modified_details[1].numeric_values[1].
    numeric_value = trim(cnvtstring(cnvtint( $5)),3)
    SET req680217->action_group[1].discontinue_actions[1].modified_details[1].numeric_values[1].text
     = uar_get_code_display(cnvtreal( $5))
   ELSEIF (action_type_cd=c_void_cd)
    SET stat = alterlist(req680217->action_group[1].void_actions,1)
    SET req680217->action_group[1].void_actions[1].order_id =  $2
    SET req680217->action_group[1].void_actions[1].version = order_updt_cnt
    SET req680217->action_group[1].void_actions[1].action_initiated_dt_tm = cnvtdatetime( $6)
    SET req680217->action_group[1].void_actions[1].responsible_provider_id = i_request->prsnl_id
    SET stat = alterlist(req680217->action_group[1].void_actions[1].modified_details,1)
    SET req680217->action_group[1].void_actions[1].modified_details[1].order_entry_field_id =
    voidreasonoefieldid
    SET stat = alterlist(req680217->action_group[1].void_actions[1].modified_details[1].
     numeric_values,1)
    SET req680217->action_group[1].void_actions[1].modified_details[1].numeric_values[1].
    numeric_value = trim(cnvtstring(cnvtint( $5)),3)
    SET req680217->action_group[1].void_actions[1].modified_details[1].numeric_values[1].text =
    uar_get_code_display(cnvtreal( $5))
   ELSE
    CALL echo("INVALID ACTION_TYPE_CD....EXITING")
    GO TO exit_script
   ENDIF
   ...
   IF ((rep680217->transaction_status.success_ind=1))
    IF (size(rep680217->action_group,5) > 0)
     IF (action_type_cd=c_cancel_cd
      AND size(rep680217->action_group[1].cancel_actions,5) > 0)
      IF (size(rep680217->action_group[1].cancel_actions[1].failures,5) > 0)
       SET report_status = "F"
       SET report_error_str = rep680217->action_group[1].cancel_actions[1].failures[1].
       debug_error_message
      ELSE
       SET report_status = "S"
      ENDIF
     ELSEIF (action_type_cd=c_discontinue_cd
      AND size(rep680217->action_group[1].discontinue_actions,5) > 0)
      IF (size(rep680217->action_group[1].discontinue_actions[1].failures,5) > 0)
       SET report_status = "F"
       SET report_error_str = rep680217->action_group[1].discontinue_actions[1].failures[1].
       debug_error_message
      ELSE
       SET report_status = "S"
      ENDIF
     ELSEIF (action_type_cd=c_void_cd
      AND size(rep680217->action_group[1].void_actions,5) > 0)
      IF (size(rep680217->action_group[1].void_actions[1].failures,5) > 0)
       SET report_status = "F"
       SET report_error_str = rep680217->action_group[1].void_actions[1].failures[1].
       debug_error_message
      ELSE
       SET report_status = "S"
      ENDIF
   ...
   SELECT INTO  $1
    ...
    HEAD REPORT
     ...
     row + 1
    DETAIL
     v1 = concat("<Status>",report_status,"</Status>"), col + 1, v1,
     row + 1, v2 = concat("<ErrorMessage>",substring(1,439,report_error_str),"</ErrorMessage>"), col
      + 1,
     v2, row + 1
    FOOT REPORT
 ...
 SUBROUTINE callupdorderprogram(null)
   ...
   DECLARE contributor_sys_cd = f8 WITH protect, constant(maxval(0.0,uar_get_code_by("DISPLAY_KEY",89,
      "PRAXIFY")))
   SELECT INTO "NL:"
    FROM orders o,
     encounter e,
    ...
    DETAIL
     ...
     requestin->locbedcd = e.loc_bed_cd, ord_dt_tm = cnvtdatetime(o.orig_order_dt_tm),
     cur_start_dt_tm = cnvtdatetime(o.current_start_dt_tm),
     oe_format_id = o.oe_format_id, catalog_type_cd = o.catalog_type_cd, bill_only_ind = oc
     .bill_only_ind,
     last_updt_cnt = o.updt_cnt, last_updt_action_seq = o.last_action_sequence, valid_dose_dt_tm =
     cnvtdatetime(o.valid_dose_dt_tm)
    WITH time = 10
   ;end select
   SELECT INTO "NL:"
   ...
   IF ((requestin->orderlist[1].actiontypecd != c_void_cd))
   ...
   SELECT DISTINCT INTO "NL:"
    ...
    DETAIL
     ...
     IF (oef3.field_type_flag=5)
      ...
      requestin->orderlist[1].detaillist[i].valuerequiredind = oef2.accept_flag, requestin->
      orderlist[1].detaillist[i].groupseq = oef2.group_seq, requestin->orderlist[1].detaillist[i].
      fieldseq = oef2.field_seq,
   ...
   SELECT INTO "NL:"
    ...
    DETAIL
     ...
     requestin->orderlist[1].stoptypecd = o.stop_type_cd, requestin->orderlist[1].
     needsintervalcalcind = 0, requestin->orderlist[1].templateorderflag = o.template_order_flag,
     ...
     requestin->orderlist[1].discontinuetypecd = o.discontinue_type_cd, requestin->orderlist[1].
     rxmask = o.rx_mask, requestin->orderlist[1].scheventid = 0.00,
     ...
     requestin->orderlist[1].locroomcd = e.loc_room_cd, requestin->orderlist[1].locbedcd = e
     .loc_bed_cd, requestin->orderlist[1].medordertypecd = o.med_order_type_cd,
   ...
   SELECT INTO "NL:"
    ...
    DETAIL
     ...
     orderlist[1].aliaslist[i].orderaliassubtypecd = oa.order_alias_sub_type_cd, requestin->
     orderlist[1].aliaslist[i].aliaspoolcd = oa.alias_pool_cd,
     ...
     requestin->orderlist[1].aliaslist[i].activeind = oa.active_ind, requestin->orderlist[1].
     aliaslist[i].billordnbrind = oa.bill_ord_nbr_ind, requestin->orderlist[1].aliaslist[i].
     primarydisplayind = oa.primary_display_ind
    WITH time = 10, format
   ;end select
   ...
   SET requestin->orderlist[1].validdosedttm = valid_dose_dt_tm
   ...
   SELECT INTO  $1
    ...
    HEAD REPORT
     ...
     row + 1
    DETAIL
     v1 = concat("<Status>",replyout->status_data.status,"</Status>"), col + 1, v1,
     row + 1, v2 = concat("<ErrorMessage>",substring(1,439,replyout->orderlist[d1.seq].
       specificerrorstr),"</ErrorMessage>"), col + 1,
     v2, row + 1
    FOOT REPORT
     col + 1, "</ReplyMessage>", row + 1
    WITH maxcol = 32000, nocounter, nullreport,
