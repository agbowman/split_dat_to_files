CREATE PROGRAM cp_add_triggering_criteria:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Pathway ID" = 0.0,
  "Triggering Criteria JSON" = "",
  "Delete Existing Criteria Indicator" = 0
  WITH outdev, pathway_id, triggering_criteria,
  del_existing_ind
 ...
 DECLARE json_str = vc WITH noconstant(trim( $TRIGGERING_CRITERIA,3)), protect
 DECLARE main(null) = null
 DECLARE verifypathwayid(null) = null
 DECLARE addtriggeringcriteria(null) = null
 RECORD triggering_criteria(
   1 cnt = i4
   1 qual[*]
 ...
 IF (textlen(trim(json_str,3)) > 0)
  SET stat = cnvtjsontorec(json_str)
 ENDIF
 SUBROUTINE main(null)
   ...
   IF (table_access=2)
    IF (valid_pathway_id=1)
     IF (( $DEL_EXISTING_IND=1))
      EXECUTE cp_del_triggering_criteria "NOFORMS",  $PATHWAY_ID
     ENDIF
     CALL addtriggeringcriteria(null)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE addtriggeringcriteria(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   DECLARE cp_triggering_criteria_id = f8 WITH noconstant(0.0), protect
   DECLARE x = i4 WITH noconstant(0), protect
   CALL log_message("In addTriggeringCriteria()",log_level_debug)
   FOR (x = 1 TO triggering_criteria->cnt)
     SET cp_triggering_criteria_id = 0.0
     SELECT INTO "nl:"
      nextseqnum = seq(reference_seq,nextval)
      FROM dual
      DETAIL
       cp_triggering_criteria_id = nextseqnum
      WITH nocounter
     ;end select
     IF (cp_triggering_criteria_id > 0.0)
      INSERT  FROM cp_triggering_criteria ctc
       SET ctc.cp_pathway_id = cp_pathway_id, ctc.cp_triggering_criteria_id =
        cp_triggering_criteria_id, ctc.criteria_type_mean = triggering_criteria->qual[x].
        criteria_type_mean,
        ctc.triggering_criteria_ident = triggering_criteria->qual[x].triggering_criteria_ident, ctc
        .triggering_entity_id = cnvtreal(triggering_criteria->qual[x].triggering_entity_id), ctc
        .triggering_entity_name = triggering_criteria->qual[x].triggering_entity_name,
        ctc.triggering_criteria_group_mean = triggering_criteria->qual[x].
        triggering_criteria_group_mean, ctc.updt_applctx = reqinfo->updt_applctx, ctc.updt_cnt = 0,
        ctc.updt_dt_tm = cnvtdatetime(sysdate), ctc.updt_id = reqinfo->updt_id, ctc.updt_task =
        reqinfo->updt_task
       WITH nocounter
      ;end insert
      IF (curqual=0)
       CALL error_and_zero_check_rec(curqual,"PERFORM INSERT",
        "Unable to insert row into cp_triggering_criteria",0,0,
        triggering_criteria_reply)
       ROLLBACK
       GO TO exit_script
      ENDIF
     ELSE
      SET triggering_criteria_reply->status_data.status = "F"
      SET triggering_criteria_reply->status_data.subeventstatus.operationname = "PERFORM"
      SET triggering_criteria_reply->status_data.subeventstatus.operationstatus = "F"
      SET triggering_criteria_reply->status_data.subeventstatus.targetobjectname = "INSERT"
      SET triggering_criteria_reply->status_data.subeventstatus.targetobjectvalue = build2(
       "Unable to generate id from REFERENCE_SEQ.")
      ROLLBACK
      GO TO exit_script
     ENDIF
   ENDFOR
   CALL log_message(build("Exit addTriggeringCriteria(), Elapsed time in seconds:",((curtime3 -
...
#exit_script
 IF ((triggering_criteria_reply->status_data.status="S"))
