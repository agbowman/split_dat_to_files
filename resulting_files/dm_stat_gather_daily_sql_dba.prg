CREATE PROGRAM dm_stat_gather_daily_sql:dba
 ...
 SUBROUTINE sbr_populate_object_name(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ms_logfile_prefix = vc WITH protect, constant("dm_daily_sql")
 ...
 DECLARE mn_daily_run_ind = i2 WITH protect, noconstant(0)
 DECLARE md_last_run_dt_tm = q8
 ...
 DECLARE sbr_gather_daily_sql(ms_mode=vc) = null
 FREE RECORD m_sql_entries
 RECORD m_sql_entries(
   1 sqls[*]
     2 sql_id = f8
 ) WITH protect
 IF (check_logfile(ms_logfile_prefix,".log","DM_DAILY_SQL")=0)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Error creating log file"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning DM_STAT_GATHER_DAILY_SQL"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 SELECT INTO "nl:"
  ...
  WHERE di.info_domain="DM_STAT_GATHER_TOP_SQL"
   AND di.info_name IN ("REPORT_CRITERIA", "LAST_DAILY_RUN", "CAPTURE_INTERACTIVE_QUERIES")
  DETAIL
   ...
   ELSEIF (di.info_name="LAST_DAILY_RUN")
    mn_daily_run_ind = 1, md_last_run_dt_tm = cnvtdatetime(di.info_date)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF (curqual < 3)
  IF (mn_daily_run_ind != 1)
   INSERT  FROM dm_info di
    SET di.info_domain = "DM_STAT_GATHER_TOP_SQL", di.info_name = "LAST_DAILY_RUN", di.info_date =
     cnvtdatetime(md_capture_dt_tm)
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Error: inserting DM_INFO data"
    GO TO exit_program
   ENDIF
  ENDIF
  SET dm_err->eproc = "Error: Certain DM_INFO rows do not exist. Continuing with defaults"
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 CALL sbr_gather_daily_sql("V500_HOURLY_TOP_SQL")
 CALL sbr_gather_daily_sql("V500_HOURLY_SHARE_MEM_SQL")
 IF (mn_allusers_ind=1)
  CALL sbr_gather_daily_sql("OTHER_HOURLY_TOP_SQL")
  CALL sbr_gather_daily_sql("OTHER_HOURLY_SHARE_MEM_SQL")
 ENDIF
 SET cur_list_size = size(m_sql_entries->sqls,5)
 SET loop_cnt = ceil((cnvtreal(cur_list_size)/ batch_size))
 SET new_list_size = (loop_cnt * batch_size)
 SET stat = alterlist(m_sql_entries->sqls,new_list_size)
 SET nstart = 1
 FOR (ml_idx = (cur_list_size+ 1) TO new_list_size)
   SET m_sql_entries->sqls[ml_idx].sql_id = m_sql_entries->sqls[cur_list_size].sql_id
 ENDFOR
 SELECT INTO "nl:"
  FROM (dummyt d1  WITH seq = value(loop_cnt)),
   dm_stat_snaps_sql dsss
  PLAN (d1
   WHERE initarray(nstart,evaluate(d1.seq,1,1,(nstart+ batch_size))))
   JOIN (dsss
   WHERE expand(ml_idx,nstart,(nstart+ (batch_size - 1)),dsss.sql_ident,m_sql_entries->sqls[ml_idx].
    sql_id))
  HEAD REPORT
   ml_dsr_cnt = (ml_dsr_cnt+ 1), ml_idx = 0, stat = alterlist(dsr->qual,ml_dsr_cnt),
   dsr->qual[ml_dsr_cnt].snapshot_type = "SQL_TEXT", dsr->qual[ml_dsr_cnt].stat_snap_dt_tm =
   cnvtdatetime(md_capture_dt_tm)
  DETAIL
   ml_idx = (ml_idx+ 1)
   IF (ml_idx > size(dsr->qual[ml_dsr_cnt].qual,5))
    stat = alterlist(dsr->qual[ml_dsr_cnt].qual,(ml_idx+ 9))
   ENDIF
   dsr->qual[ml_dsr_cnt].qual[ml_idx].stat_name = cnvtstring(dsss.sql_ident), dsr->qual[ml_dsr_cnt].
   qual[ml_idx].stat_clob_val = dsss.sql_text
  FOOT REPORT
   stat = alterlist(dsr->qual[ml_dsr_cnt].qual,ml_idx)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Error: gathering SQL_TEXT data"
  GO TO exit_program
 ENDIF
 ...
 UPDATE  FROM dm_info di
  SET di.info_date = cnvtdatetime(md_capture_dt_tm)
  WHERE di.info_domain="DM_STAT_GATHER_TOP_SQL"
   AND di.info_name="LAST_DAILY_RUN"
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Error: updating DM_INFO data"
  GO TO exit_program
 ENDIF
 SUBROUTINE sbr_gather_daily_sql(ms_mode)
  SELECT INTO "nl:"
   sql_id = dssv.stat_name, clob_metrics = dssv.stat_clob_val, num_metrics = dssv.stat_number_val
   FROM dm_stat_snaps dss,
    dm_stat_snaps_values dssv
   PLAN (dss
    WHERE dss.snapshot_type=ms_mode
     AND dss.stat_snap_dt_tm BETWEEN cnvtdatetime(md_last_run_dt_tm) AND cnvtdatetime(
     md_capture_dt_tm))
    JOIN (dssv
    WHERE dss.dm_stat_snap_id=dssv.dm_stat_snap_id)
   ORDER BY dssv.stat_number_val DESC, dssv.stat_name, dss.stat_snap_dt_tm DESC
   HEAD REPORT
    ml_idx = 0, ml_idx2 = 0, ml_dsr_cnt = (ml_dsr_cnt+ 1),
    stat = alterlist(dsr->qual,ml_dsr_cnt)
    CASE (ms_mode)
     OF "V500_HOURLY_SHARE_MEM_SQL":
      dsr->qual[ml_dsr_cnt].snapshot_type = "V500_SHARE_MEM_SQL"
     OF "OTHER_HOURLY_TOP_SQL":
      dsr->qual[ml_dsr_cnt].snapshot_type = "OTHER_TOP_SQL"
     OF "OTHER_HOURLY_SHARE_MEM_SQL":
      dsr->qual[ml_dsr_cnt].snapshot_type = "OTHER_SHARE_MEM_SQL"
     ELSE
      dsr->qual[ml_dsr_cnt].snapshot_type = "V500_TOP_SQL"
    ENDCASE
    dsr->qual[ml_dsr_cnt].stat_snap_dt_tm = cnvtdatetime(md_capture_dt_tm)
   DETAIL
    IF (ml_idx < ml_num_entries)
     ml_idx2 = locateval(ml_cnt,1,size(dsr->qual[ml_dsr_cnt].qual,5),dssv.stat_name,dsr->qual[
      ml_dsr_cnt].qual[ml_cnt].stat_name)
     IF (ml_idx2 <= 0)
      ml_idx = (ml_idx+ 1)
      IF (ml_idx > size(dsr->qual[ml_dsr_cnt].qual,5))
       stat = alterlist(dsr->qual[ml_dsr_cnt].qual,(ml_idx+ 9))
      ENDIF
      IF (ml_idx3 > size(m_sql_entries->sqls,5))
       stat = alterlist(m_sql_entries->sqls,(ml_idx3+ 9))
      ENDIF
      dsr->qual[ml_dsr_cnt].qual[ml_idx].stat_name = dssv.stat_name, dsr->qual[ml_dsr_cnt].qual[
      ml_idx].stat_number_val = dssv.stat_number_val, dsr->qual[ml_dsr_cnt].qual[ml_idx].
      stat_clob_val = dssv.stat_clob_val,
      ml_idx2 = locateval(ml_cnt,1,size(m_sql_entries->sqls,5),cnvtreal(dssv.stat_name),m_sql_entries
       ->sqls[ml_cnt].sql_id)
      IF (ml_idx2 <= 0)
       ml_idx3 = (ml_idx3+ 1)
       IF (ml_idx3 > size(m_sql_entries->sqls,5))
        stat = alterlist(m_sql_entries->sqls,(ml_idx3+ 9))
       ENDIF
       m_sql_entries->sqls[ml_idx3].sql_id = cnvtreal(dssv.stat_name)
      ENDIF
     ENDIF
    ELSE
     CALL cancel(1)
    ENDIF
   FOOT REPORT
    stat = alterlist(dsr->qual[ml_dsr_cnt].qual,ml_idx), stat = alterlist(m_sql_entries->sqls,ml_idx3
     )
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Error: gathering Hourly snapshot data"
   GO TO exit_program
  ENDIF
 END ;Subroutine
#exit_program
 ...
 IF ((dm_err->err_ind=0))
  COMMIT
  SET dm_err->eproc = "Ending DM_STAT_GATHER_DAILY_SQL.  Process completed successfully."
