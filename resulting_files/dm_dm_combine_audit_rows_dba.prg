CREATE PROGRAM dm_dm_combine_audit_rows:dba
 ...
 SET reply->table_name = "DM_COMBINE_AUDIT"
 ...
 DECLARE v_min_days_to_keep = i4 WITH protect, constant(3)
 ...
 FOR (v_ndx = 1 TO v_cnt)
   ...
   ENDIF
 ENDFOR
 SET v_days_to_keep = maxval(v_days_to_keep,v_min_days_to_keep)
 SET strunit = build(v_days_to_keep,"D")
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(d.dm_combine_audit_id)
   FROM dm_combine_audit d
   WHERE d.dm_combine_audit_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(d.dm_combine_audit_id)
  FROM dm_combine_audit d
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    d.rowid
    FROM dm_combine_audit d
    WHERE d.dm_combine_audit_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "d.updt_dt_tm < cnvtlookbehind(strUnit,cnvtdatetime(curdate,curtime3))","d"))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
    ...
    WITH nocounter, maxqual(d,value(v_rows_left))
   ;end select
