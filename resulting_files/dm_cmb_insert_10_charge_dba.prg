CREATE PROGRAM dm_cmb_insert_10_charge:dba
 ...
 CALL parser(build("where FRM.charge_item_id = ",rreclist->from_rec[pcc2_request->sbr_loopcount].
   from_id,")"))
 CALL parser("go")
 CALL error_logger(curqual,"Couldn't insert offset charge on the from person",insert_error)
 SET icombinedet += 1
 SET stat = alterlist(request->xxx_combine_det,icombinedet)
 SET request->xxx_combine_det[icombinedet].combine_action_cd = add
 IF ((rreclist->from_rec[pcc2_request->sbr_loopcount].charge_type_cd=credit_cd))
