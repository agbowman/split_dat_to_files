CREATE PROGRAM cust_concept_trigger_r
 PROMPT
 ...
 DECLARE stablename = vc WITH protect, constant("CUST_CONCEPT_TRIGGER_R")
 ...
 IF (checkdic(stablename,"T",0)=0)
  SELECT INTO TABLE cust_concept_trigger_r
   cust_concept_trigger_r_id = type("f8"), trigger_entity_id = type("f8"), trigger_entity = type("vc"
    ),
   trigger_name = type("vc"), trigger_name_key = type("vc"), updt_id = type("f8"),
   updt_dt_tm = type("dq8"), updt_cnt = type("i4"), updt_applctx = type("f8")
   FROM dummyt d1
   WITH indexunique(cust_concept_trigger_r_id), owner = value(stableowner), index(trigger_entity_id),
    index(trigger_name_key), index(updt_dt_tm), synonym = "CUST_CONCEPT_TRIGGER_R",
    organization = p
  ;end select
  DROP TABLE cust_concept_trigger_r
  EXECUTE oragen3 "V500_BHS.CUST_CONCEPT_TRIGGER_R"
