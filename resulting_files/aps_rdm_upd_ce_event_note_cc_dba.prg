CREATE PROGRAM aps_rdm_upd_ce_event_note_cc:dba
 ...
 DECLARE codevalue_chart = f8 WITH noconstant(0.0), protect
 DECLARE codevalue_nonchart = f8 WITH noconstant(0.0), protect
 DECLARE codevalue_ah = f8 WITH noconstant(0.0), protect
 DECLARE codevalue_rtf = f8 WITH noconstant(0.0), protect
 DECLARE parserstring = vc WITH public, noconstant(" ")
 SET parserstring = xntr_replace_where("cen.ce_event_note_id between mf_min_id and mf_max_id",
  "CE_EVENT_NOTE","cen")
 SET ms_child_c_info_domain_nm = "aps_rdm_ce_event_note_update"
 SET ms_child_c_max_name = "MAX ce_event_note_id EVALUATED"
 SET mf_min_id = xntr_alt_min_id(null)
 SET mf_max_id = xntr_alt_max_id(null)
 IF (((mf_min_id=xntr_default_id) OR (mf_max_id=xntr_default_id)) )
 ...
 SELECT INTO "nl:"
  ...
  FROM code_value c
  WHERE c.code_set=14
   AND c.cdf_meaning IN ("APIMGCHART", "APNOIMGCHART")
   AND c.active_ind=1
  DETAIL
   IF (c.cdf_meaning="APIMGCHART")
    codevalue_chart = c.code_value
   ELSE
    codevalue_nonchart = c.code_value
   ENDIF
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->message = concat("Failed to select code_value for note_type_cd: ",serrormsg)
  SET readme_data->status = "F"
  GO TO exit_program
 ENDIF
 IF (curqual=0)
  SET readme_data->status = "F"
  SET readme_data->message = "Code value not found for desire note_type."
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  c.code_value, c.cdf_meaning
  FROM code_value c
  WHERE c.code_set=23
   AND c.cdf_meaning IN ("AH", "RTF")
   AND c.active_ind=1
  DETAIL
   IF (c.cdf_meaning="AH")
    codevalue_ah = c.code_value
   ELSE
    codevalue_rtf = c.code_value
   ENDIF
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->message = concat("Failed to select code_value for note_format_cd: ",serrormsg)
  SET readme_data->status = "F"
  GO TO exit_program
 ENDIF
 IF (curqual=0)
  SET readme_data->status = "F"
  SET readme_data->message = "Code value not found for desire format_cd."
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  cen.ce_event_note_id, cen.event_id, cen.note_format_cd
  FROM ce_event_note cen
  WHERE parser(parserstring)
   AND cen.note_type_cd IN (codevalue_chart, codevalue_nonchart)
   AND cen.note_format_cd=codevalue_rtf
  WITH nocounter, forupdate(cen)
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->message = concat("Failed to select on table ce_event_note: ",serrormsg)
  SET readme_data->status = "F"
  GO TO exit_program
 ENDIF
 IF (curqual=0)
  ...
  SET readme_data->message = "No rows found to update,Data is already updated."
  GO TO exit_program
 ENDIF
 UPDATE  FROM ce_event_note cen
  SET cen.note_format_cd = codevalue_ah, cen.updt_applctx = reqinfo->updt_applctx, cen.updt_task =
   reqinfo->updt_task,
   cen.updt_cnt = (cen.updt_cnt+ 1), cen.updt_id = reqinfo->updt_id, cen.updt_dt_tm = cnvtdatetime(
    sysdate)
  WHERE parser(parserstring)
   AND cen.note_type_cd IN (codevalue_chart, codevalue_nonchart)
   AND cen.note_format_cd=codevalue_rtf
 ;end update
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->message = concat("Failed during update table: ",serrormsg)
  SET readme_data->status = "F"
  ROLLBACK
  GO TO exit_program
 ELSE
  COMMIT
  SET readme_data->message = "Readme successful: Table updated successfully"
  SET readme_data->status = "S"
 ENDIF
