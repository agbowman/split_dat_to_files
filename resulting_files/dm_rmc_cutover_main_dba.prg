CREATE PROGRAM dm_rmc_cutover_main:dba
 ...
 SUBROUTINE drcd_drp_del(drp_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE drcm_run(i_hreq=i4,i_source_env_id=f8,i_cutover_number=i4) = i4
 DECLARE v_happ = i4
 DECLARE v_htask = i4
 DECLARE v_hreq = i4
 DECLARE v_ret = i4
 DECLARE drcm_env_id = f8
 DECLARE drcm_cur_process = vc WITH protect, noconstant("")
 DECLARE drcm_del_ind = i4
 IF ((validate(dm_auto_cut_ind,- (1))=- (1)))
  DECLARE dm_auto_cut_ind = i2 WITH protect, noconstant(0)
 ENDIF
 CALL check_logfile("DM_RMC_CUTOVER_MAIN",".log","DM_RMC_CUTOVER_MAIN")
 SET dm_err->eproc = "Beginning DM2_RMC_CUTOVER_MAIN"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ENDIF
 SET drcm_del_ind = drcd_drp_del("CUTOVER PROCESS*")
 IF (drcm_del_ind=0)
  COMMIT
 ELSE
  ROLLBACK
 ...
 SELECT INTO "nl:"
  ...
  WHERE drel.rdds_event="Begin Reference Data Sync"
   ...
   AND drel.cur_environment_id=drcm_env_id
  ...
  (SELECT
   ...
   WHERE cur_environment_id=drcm_env_id
 ...
 IF (curqual=0)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Deleting info from previous run"
 CALL disp_msg(" ",dm_err->logfile,0)
 DELETE  FROM dm_info di
  WHERE di.info_domain=concat("RDDS CUTOVER::",trim(cnvtstring(request->source_env_id,20)))
   AND (di.info_number=- (1))
  WITH nocounter
 ;end delete
 COMMIT
 SET dm_err->eproc = "Opening Auto-Cutover Event"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm_auto_cut_ind=1)
  SET stat = alterlist(auto_ver_request->qual,1)
  SET auto_ver_request->qual[1].rdds_event = "Auto-Cutover Started"
  SET auto_ver_request->qual[1].cur_environment_id = drcm_env_id
  ...
  SET auto_ver_request->qual[1].detail_qual[1].event_detail2_txt = cnvtstring(drcm_cur_process)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Checking to see if all of the cut-over processes are running"
 CALL disp_msg(" ",dm_err->logfile,0)
 FOR (co_ndx = 1 TO request->num_cutover_processes)
   SELECT INTO "nl:"
    "x"
    FROM dm_refchg_process
    WHERE (env_source_id=request->source_env_id)
     AND rdbhandle_value IN (
    (SELECT
     audsid
     FROM gv$session))
     AND refchg_type=concat("CUTOVER PROCESS",trim(cnvtstring(co_ndx)))
   ...
   ENDIF
   IF (curqual=0)
    SET v_ret = dm2_asynch_pre(5000,5000,4320006,v_happ,v_htask,
     v_hreq)
    SET v_ret = drcm_run(v_hreq,request->source_env_id,co_ndx)
    SET v_ret = dm2_asynch_post(v_happ,v_htask,v_hreq)
   ENDIF
 ENDFOR
 SUBROUTINE drcm_run(i_hreq,i_source_env_id,i_cutover_number)
   SET dm_err->eproc = "Starting a new cut-over process"
   CALL disp_msg(" ",dm_err->logfile,0)
   DECLARE s_hreqstruct = i4 WITH protect
   DECLARE s_srvstat = f8 WITH protect
   DECLARE s_iret = i4 WITH protect
   DECLARE s_hreply = i4 WITH protect
   DECLARE s_hlist = i4 WITH protect
   DECLARE s_hstatusdata = i4 WITH protect
   DECLARE s_hitem = i4 WITH protect
   SET s_hreqstruct = uar_crmgetrequest(i_hreq)
   CALL uar_srvsetdouble(s_hreqstruct,"source_env_id",i_source_env_id)
   CALL uar_srvsetlong(s_hreqstruct,"cutover_number",i_cutover_number)
...
#exit_program
 ...
 SET dm_err->eproc = "End dm_rmc_cutover_main"
 CALL final_disp_msg("drcm")
END GO
