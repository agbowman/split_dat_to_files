CREATE PROGRAM bhs_ma_genview_pt_data:dba
 ...
 DECLARE spiritsac_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
 ...
 DECLARE favorite_color_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,"FAVORITECOLOR"
   ))
 DECLARE childs_preferred_name_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "CHILDSPREFERREDNAME"))
 DECLARE parents_preferred_name_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PARENTSPREFERREDNAME"))
 DECLARE favorite_activities_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "FAVORITEACTIVITIES"))
 DECLARE naps_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,"NAPS"))
 DECLARE bedtime_rituals_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "BEDTIMERITUALS"))
 DECLARE coping_measures_used_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "COPINGMEASURES"))
 DECLARE medications_administered_by_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "MEDICATIONSADMINISTEREDBY"))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (c
   ...
   homephonenumber_cd, relationshiptopatient_cd, contactperson_cd, spiritsac_cd, languagespoken_cd,
   ...
   birthweight_cd, code_cd, deliveredby_cd, placeofbirth_cd, favorite_color_cd,
   childs_preferred_name_cd, parents_preferred_name_cd, favorite_activities_cd, naps_cd,
   bedtime_rituals_cd,
   coping_measures_used_cd, medications_administered_by_cd)
  ...
  DETAIL
   ...
   CASE (c.event_cd)
    OF pcpofmother_cd:
     dlrec->seq[cnt].category = 7,dlrec->seq[cnt].result_sort = 14
    OF organdonor_cd:
    ...
    OF placeofbirth_cd:
     dlrec->seq[cnt].category = 8,dlrec->seq[cnt].result_sort = 15
    OF favorite_color_cd:
     dlrec->seq[cnt].category = 8,dlrec->seq[cnt].result_sort = 16
    OF childs_preferred_name_cd:
     dlrec->seq[cnt].category = 8,dlrec->seq[cnt].result_sort = 17
    OF parents_preferred_name_cd:
     dlrec->seq[cnt].category = 8,dlrec->seq[cnt].result_sort = 18
    OF favorite_activities_cd:
     dlrec->seq[cnt].category = 8,dlrec->seq[cnt].result_sort = 19
    OF naps_cd:
     dlrec->seq[cnt].category = 8,dlrec->seq[cnt].result_sort = 20
    OF bedtime_rituals_cd:
     dlrec->seq[cnt].category = 8,dlrec->seq[cnt].result_sort = 21
    OF coping_measures_used_cd:
     dlrec->seq[cnt].category = 8,dlrec->seq[cnt].result_sort = 22
    OF medications_administered_by_cd:
     dlrec->seq[cnt].category = 8,dlrec->seq[cnt].result_sort = 23
   ENDCASE
  FOOT REPORT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (c1
   WHERE (c1.person_id=dlrec->person_id)
    ...
    AND c1.event_tag > " ")
  ORDER BY cnvtdatetime(c1.event_end_dt_tm)
  HEAD REPORT
  ...
  DETAIL
   ...
   IF (c1.event_cd IN (birthweight_cd, gestationalage_cd, weeksofgestationatbirth_cd, code_cd))
    dlrec->seq[cnt].result = concat(trim(c1.result_val,3)," ",uar_get_code_display(c1.result_units_cd
      ))
   ENDIF
  FOOT REPORT
   stat = alterlist(dlrec->seq,cnt), dlrec->encntr_total = cnt
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  DETAIL
   IF (diag_cnt=0)
    ...
    dlrec->seq[cnt].category = 5, dlrec->seq[cnt].result = trim(c.result_val), dlrec->seq[cnt].
    result_date = substring(1,14,format(c.event_end_dt_tm,"@SHORTDATETIME;;Q")),
