CREATE PROGRAM ams_pft_get_crit_enctype_class:dba
 ...
 IF (size(pft_reply->encountertypeclasses,5) > 0)
  ...
  FOR (loopcnt = 1 TO size(pft_reply->encountertypeclasses,5))
    SET recordpos = getnextrecord(0)
    SET stat = setstringfield(recordpos,disppos,substring(1,100,pft_reply->encountertypeclasses[
      loopcnt].encntrtypeclassdisplay))
    SET stat = setrealfield(recordpos,valuepos,pft_reply->encountertypeclasses[loopcnt].
     encntrtypeclasscd)
  ENDFOR
