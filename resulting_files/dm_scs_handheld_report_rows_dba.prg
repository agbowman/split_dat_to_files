CREATE PROGRAM dm_scs_handheld_report_rows:dba
 ...
 SET reply->table_name = "HANDHELD_REPORT"
 ...
 SELECT INTO "nl:"
  hr.rowid
  FROM handheld_report hr
  WHERE parser(sbr_getrowidnotexists("hr.upload_dt_tm+0 <= cnvtdatetime(purge_dt_tm)","hr"))
   AND ((hr.report_id+ 0) > 0)
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = (cnt+ 1)
   ...
   ENDIF
   reply->rows[cnt].row_id = hr.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,cnt)
  WITH nocounter, maxqual(hr,value(request->max_rows))
 ;end select
