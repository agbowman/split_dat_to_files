CREATE PROGRAM dm2_flexible_schedule_menu
 ...
 ENDIF
 DECLARE dfsm_logfile_prefix = vc WITH protect, constant("dm2_flex_sched_main")
 DECLARE dfsm_menu_option = i2 WITH protect, noconstant(0)
 DECLARE dfsm_dm_info_fnd_ind = i2 WITH protect, noconstant(0)
 DECLARE dfsm_continue = i2 WITH protect, noconstant(1)
 IF (validate(dfsm_sched->cnt,8)=8
  AND validate(dfsm_sched->cnt,9)=9)
  FREE RECORD dfsm_sched
  RECORD dfsm_sched(
    1 readme_cnt = i2
    1 schema_cnt = i2
    1 readme_sched[*]
      2 time_period = i2
      2 start_time_hhmm = f8
      2 acc_start_time_hhmm = f8
      2 start_time_hh = i2
      2 start_time_am_pm = c2
      2 end_time_hhmm = f8
      2 acc_end_time_hhmm = f8
      2 end_time_hh = i2
      2 end_time_am_pm = c2
      2 num_of_runners = i2
      2 acc_num_of_runners = i2
    1 schema_sched[*]
      2 time_period = i2
      2 start_time_hhmm = f8
      2 acc_start_time_hhmm = f8
      ...
      2 num_of_runners = i2
      2 acc_num_of_runners = i2
    1 hour24_crossed_idx = i2
  )
  SET dfsm_sched->readme_cnt = 0
  SET dfsm_sched->schema_cnt = 0
  SET dfsm_sched->hour24_crossed_idx = 0
 ENDIF
 IF (validate(dfsm_runners->cnt,8)=8
  AND validate(dfsm_runners->cnt,9)=9)
  FREE RECORD dfsm_runners
  RECORD dfsm_runners(
    1 cnt = i2
    1 qual[*]
      2 process_id = vc
      ...
      2 active_ind = f8
      2 status = vc
      2 current_last_op = vc
      2 num_completed = i4
      2 bkgrnd_runner_ind = i2
      2 working_on = vc
      2 num_w_applid = i2
      2 host_name = vc
      2 instance_name = vc
  ...
  SET dfsm_runners->cnt = 0
  SET dfsm_runners->file_name = " "
 ENDIF
 DECLARE dfsm_display_main_menu(ddmm_menu_opt=i2(ref)) = i2
 DECLARE dfsm_display_maint_menu(null) = i2
 DECLARE dfsm_display_monitor_screen(null) = i2
 DECLARE dfsm_load_sched_to_screen(dpms_strt_line=i2,dpms_line_ctr=i2,dpms_disp=vc) = null
 DECLARE dfsm_validate_maint_input(dvmi_time_period=i2,dvmi_done=i2(ref),dvmi_valid_input=i2(ref),
  dvmi_ret_msg=vc(ref),dm_process=vc) = null
 DECLARE dfsm_display_maint_flex_sched(null) = i2
 DECLARE dfsm_save_flex_schedule(null) = i2
 DECLARE dfsm_load_flex_schedule(null) = i2
 DECLARE dfsm_load_runners_to_screen(dlrts_start_line=i2,dlrts_ret_msg=vc(ref)) = i2
 DECLARE dfsm_toggle_schedule(dts_ret_msg=vc(ref)) = i2
 DECLARE dfsm_stop_pkg_installs(null) = i2
 DECLARE dfsm_write_runners_to_file(null) = i2
 DECLARE dfsm_disp_error_msg(null) = i2
 SET modify curaliasreuse 1
 SET message = window
 SET width = 132
 IF (check_logfile(dfsm_logfile_prefix,".log","dm2_flexible_schedule_menu")=0)
  CALL dfsm_disp_error_msg(null)
  GO TO exit_program
 ENDIF
 IF (currdb != "ORACLE")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("The Installation Scheduler is not supported on ",trim(currdb,3),".")
  SET dm_err->eproc = "Validating script usage."
  CALL dfsm_disp_error_msg(null)
  GO TO exit_program
 ENDIF
 IF (dm2_table_and_ccldef_exists("DM_INFO",dfsm_dm_info_fnd_ind)=0)
  CALL dfsm_disp_error_msg(null)
  GO TO exit_program
 ELSEIF (dfsm_dm_info_fnd_ind=0)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Validating script usage."
  SET dm_err->emsg = "The Installation Scheduler cannot be used without the DM_INFO table"
  CALL dfsm_disp_error_msg(null)
  GO TO exit_program
 ENDIF
 IF (dm2_rr_toolset_usage(null)=0)
 ...
 IF ((dm2_rr_misc->dm2_toolset_usage="N"))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Validating script usage."
  SET dm_err->emsg = "The Installation Scheduler is not supported when dm readme tools are in use"
  CALL dfsm_disp_error_msg(null)
  GO TO exit_program
 ENDIF
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 WHILE (dfsm_continue=1)
   IF (dfsm_display_main_menu(dfsm_menu_option)=0)
    SET dfsm_continue = 0
   ELSE
    CASE (dfsm_menu_option)
     OF 0:
      SET dfsm_continue = 0
     OF 1:
      IF (dfsm_display_maint_menu(null)=0)
       GO TO exit_program
      ENDIF
     OF 2:
      SET message = nowindow
      EXECUTE dm2_install_plan_processes
      IF ((dm_err->err_ind > 0))
       GO TO exit_program
      ENDIF
      SET message = window
     OF 3:
      IF (dfsm_stop_pkg_installs(null)=0)
       GO TO exit_program
      ENDIF
    ENDCASE
   ENDIF
 ENDWHILE
 GO TO exit_program
 SUBROUTINE dfsm_display_main_menu(ddmm_menu_opt)
   DECLARE ddmm_continue = i2 WITH protect, noconstant(1)
   IF (drr_get_flexible_schedule(null)=0)
    CALL dfsm_disp_error_msg(null)
    RETURN(0)
   ENDIF
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL text(2,50,"***  INSTALLATION SCHEDULER ***")
   CALL text(5,2,concat("Status:  ",evaluate(drr_flex_sched->status,0,"Off","On")))
   CALL text(7,8,"Options are:")
   CALL text(9,8,"1. Create or Modify the Schedule for Background Install Processes")
   CALL text(11,8,"2. Monitor Install Processes")
   CALL text(13,8,"3. Stop All Package Installations Using the Installation Scheduler")
   CALL text(16,8,"Please enter one of the options described above or 0 to exit : ")
   WHILE (ddmm_continue=1)
     CALL accept(16,72,"9",0
      WHERE curaccept IN (0, 1, 2, 3))
     SET ddmm_menu_opt = curaccept
     IF ( NOT (ddmm_menu_opt IN (0, 1, 2, 3)))
      CALL clear(18,8,124)
      CALL text(18,8,concat("Invalid option.  Please enter a valid option."))
     ELSEIF (ddmm_menu_opt IN (2, 3)
      AND (drr_flex_sched->status=0))
      CALL clear(18,8,124)
      CALL text(18,8,concat(
        "Installation Schedule not set up or active - invalid option.  Enter another option."))
     ELSE
      SET ddmm_continue = 0
     ENDIF
   ENDWHILE
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_display_monitor_screen(null)
   DECLARE ddms_continue_loop = i2 WITH protect, noconstant(1)
   DECLARE ddms_option = vc WITH protect, noconstant(" ")
   DECLARE ddms_ret_msg = vc WITH protect, noconstant(" ")
   WHILE (ddms_continue_loop=1)
     IF (dfsm_load_flex_schedule(null)=0)
      RETURN(0)
     ENDIF
     CALL clear(1,1)
     CALL box(1,1,3,132)
     CALL text(2,50,"***  INSTALLATION SCHEDULER ***")
     CALL text(5,2,"Time Period")
     CALL text(5,16,"Start Time")
     CALL text(5,29,"End Time")
     CALL text(5,39,"Num of Runners")
     CALL text(6,1,"(R)")
     CALL text(5,56,"Time Period")
     CALL text(5,70,"Start Time")
     CALL text(5,83,"End Time")
     CALL text(5,93,"Num of Runners")
     CALL text(6,56,"(S)")
     CALL dfsm_load_sched_to_screen(6,1,"Y")
     IF (dfsm_load_runners_to_screen(11,ddms_ret_msg)=0)
      RETURN(0)
     ENDIF
     CALL text(23,2,
      "(R)efresh Screen, (V)iew Suspended Sessions, (Q)uit :           * Screen will automatically refresh every 2 minutes"
      )
     IF (ddms_ret_msg > " ")
      CALL text(24,1,ddms_ret_msg)
      SET ddms_ret_msg = " "
     ELSE
      CALL clear(24,1,130)
     ENDIF
     SET accept = time(120)
     CALL accept(23,58,"p;cu","R"
      WHERE curaccept IN ("R", "Q", "r", "q", "V",
      "v"))
     SET accept = time(0)
     SET ddms_option = cnvtupper(curaccept)
     CASE (ddms_option)
      OF "Q":
       SET ddms_continue_loop = 0
      OF "R":
       SET ddms_continue_loop = 1
      OF "V":
       IF ((dm2_rdbms_version->level1 >= 9))
        ...
        ENDIF
       ELSE
        CALL text(24,1,"Option available on Oracle Version 9 or greater")
        CALL pause(2)
       ENDIF
       SET ddms_continue_loop = 1
     ENDCASE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_display_maint_menu(null)
   DECLARE ddmm_continue_loop = i2 WITH protect, noconstant(1)
   DECLARE ddmm_ret_msg = vc WITH protect, noconstant(" ")
   DECLARE ddmm_option = vc WITH protect, noconstant(" ")
   WHILE (ddmm_continue_loop=1)
     IF (dfsm_load_flex_schedule(null)=0)
      RETURN(0)
     ENDIF
     CALL clear(1,1)
     ...
     CALL text(2,50,"***  INSTALLATION SCHEDULER ***")
     CALL text(5,2,concat("Status:  ",evaluate(drr_flex_sched->status,0,"Off","On")))
     CALL text(6,2,"README(R) SCHEMA(S)")
     CALL text(7,2,"Time Period")
     CALL text(7,16,"Start Time")
     CALL text(7,29,"End Time")
     CALL text(7,39,"Num of Runners")
     CALL text(9,1,"(R)")
     CALL text(13,1,"(S)")
     CALL dfsm_load_sched_to_screen(9,1,"N")
     CALL text(23,2,"(M)odify Schedule,  (T)oggle On/Off, (B)ack : ")
     IF (ddmm_ret_msg > " ")
      CALL text(24,1,ddmm_ret_msg)
      SET ddmm_ret_msg = " "
     ELSE
      CALL clear(24,1,130)
     ENDIF
     CALL accept(23,50,"p;cu","B"
      WHERE curaccept IN ("M", "T", "B"))
     SET ddmm_option = curaccept
     CASE (ddmm_option)
      OF "B":
       SET ddmm_continue_loop = 0
      OF "M":
       SET dfsm_sched->hour24_crossed_idx = 0
       IF (dfsm_display_maint_flex_sched(null)=0)
        RETURN(0)
       ENDIF
      OF "T":
       IF (dfsm_toggle_schedule(ddmm_ret_msg)=0)
        RETURN(0)
       ENDIF
     ENDCASE
   ENDWHILE
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_display_maint_flex_sched(null)
   DECLARE ddmfs_tp = i2 WITH protect, noconstant(0)
   DECLARE ddmfs_done = i2 WITH protect, noconstant(0)
   DECLARE ddmfs_valid_input = i2 WITH protect, noconstant(1)
   DECLARE ddmfs_continue_loop = i2 WITH protect, noconstant(1)
   DECLARE ddmfs_ret_msg = vc WITH protect, noconstant(" ")
   DECLARE ddmfs_row = i2
   DECLARE ddmfs_col = i2
   DECLARE ddmfs_option = vc WITH protect, noconstant(" ")
   DECLARE ddmfs_sc_tp = i4 WITH protect, noconstant(0)
   DECLARE ddmfs_sc_done = i2 WITH protect, noconstant(0)
   DECLARE ddmfs_sc_valid_input = i2 WITH protect, noconstant(1)
   DECLARE ddmfs_sc_continue_loop = i2 WITH protect, noconstant(1)
   DECLARE ddmfs_sc_ret_msg = vc WITH protect, noconstant(" ")
   DECLARE ddmfs_sc_option = vc WITH protect, noconstant(" ")
   DECLARE ddmfs_sc_row = i2
   DECLARE ddmfs_sc_col = i2
   CALL text(2,50,"***  INSTALLATION SCHEDULER ***")
   CALL text(6,2,"README(R) SCHEMA(S)")
   CALL text(7,2,"Time Period")
   CALL text(7,16,"Start Time")
   ...
   SET ddmfs_col = 18
   SET ddmfs_tp = 0
   WHILE (ddmfs_continue_loop=1)
     SET ddmfs_tp = (ddmfs_tp+ 1)
     CALL dfsm_load_sched_to_screen(9,1,"N")
     IF (ddmfs_valid_input=0)
      CALL text(19,1,ddmfs_ret_msg)
      SET ddmfs_ret_msg = " "
     ELSE
      CALL clear(19,1,130)
     ENDIF
     IF (ddmfs_tp=1)
      SET help = "Enter a standard hour from 1 - 12."
      CALL accept(ddmfs_row,ddmfs_col,"99",dfsm_sched->readme_sched[ddmfs_tp].start_time_hh
       WHERE curaccept >= 1
        AND curaccept <= 12)
      SET dfsm_sched->readme_sched[ddmfs_tp].start_time_hh = curaccept
      SET help = off
     ENDIF
     SET ddmfs_col = (ddmfs_col+ 3)
     CALL clear(24,1,130)
     IF (ddmfs_tp=1)
      SET help = "Enter AM or PM."
      CALL accept(ddmfs_row,ddmfs_col,"P(2);cu",dfsm_sched->readme_sched[ddmfs_tp].start_time_am_pm
       WHERE curaccept IN ("AM", "PM"))
      SET dfsm_sched->readme_sched[ddmfs_tp].start_time_am_pm = curaccept
      SET help = off
     ENDIF
     SET ddmfs_col = (ddmfs_col+ 10)
     CALL clear(24,1,130)
     IF (ddmfs_tp != 4)
      SET help = "Enter a standard hour from 1 - 12."
      CALL accept(ddmfs_row,ddmfs_col,"99",dfsm_sched->readme_sched[ddmfs_tp].end_time_hh
       WHERE curaccept >= 1
        AND curaccept <= 12)
      SET dfsm_sched->readme_sched[ddmfs_tp].end_time_hh = curaccept
      SET help = off
     ENDIF
     SET ddmfs_col = (ddmfs_col+ 3)
     CALL clear(24,1,130)
     IF (ddmfs_tp != 4)
      SET help = "Enter AM or PM."
      CALL accept(ddmfs_row,ddmfs_col,"P(2);cu",dfsm_sched->readme_sched[ddmfs_tp].end_time_am_pm
       WHERE curaccept IN ("AM", "PM"))
      SET dfsm_sched->readme_sched[ddmfs_tp].end_time_am_pm = curaccept
      SET help = off
     ENDIF
     SET ddmfs_col = (ddmfs_col+ 10)
     CALL clear(24,1,130)
     SET help = "Enter 0 - 10."
     CALL accept(ddmfs_row,ddmfs_col,"99",dfsm_sched->readme_sched[ddmfs_tp].num_of_runners
      WHERE curaccept >= 0
       AND (curaccept <= drr_flex_sched->max_runners))
     SET dfsm_sched->readme_sched[ddmfs_tp].acc_num_of_runners = curaccept
     SET help = off
     CALL clear(24,1,130)
     SET ddmfs_done = 0
     SET ddmfs_valid_input = 0
     SET ddmfs_ret_msg = " "
     CALL dfsm_validate_maint_input(ddmfs_tp,ddmfs_done,ddmfs_valid_input,ddmfs_ret_msg,"README")
     IF (ddmfs_valid_input=0
      AND ddmfs_done=1)
      SET ddmfs_tp = 0
      SET ddmfs_col = 18
      SET ddmfs_row = 9
     ELSEIF (ddmfs_valid_input=0)
      SET ddmfs_tp = (ddmfs_tp - 1)
      SET ddmfs_col = 18
     ELSE
      SET ddmfs_row = (ddmfs_row+ 1)
      SET ddmfs_col = 18
     ENDIF
     IF (ddmfs_valid_input=1
      AND ddmfs_done=1)
      SET ddmfs_continue_loop = 0
     ENDIF
   ENDWHILE
   SET dfsm_sched->hour24_crossed_idx = 0
   SET ddmfs_sc_row = 13
   SET ddmfs_sc_col = 18
   SET ddmfs_sc_continue_loop = 1
   WHILE (ddmfs_sc_continue_loop=1)
     SET ddmfs_sc_tp = (ddmfs_sc_tp+ 1)
     CALL dfsm_load_sched_to_screen(9,1,"N")
     IF (ddmfs_sc_tp=1)
      CALL accept(ddmfs_sc_row,ddmfs_sc_col,"99",dfsm_sched->schema_sched[ddmfs_sc_tp].start_time_hh
       WHERE curaccept >= 1
        AND curaccept <= 12)
      SET dfsm_sched->schema_sched[ddmfs_sc_tp].start_time_hh = curaccept
     ENDIF
     SET ddmfs_sc_col = (ddmfs_sc_col+ 3)
     CALL clear(24,1,130)
     IF (ddmfs_sc_tp=1)
      CALL accept(ddmfs_sc_row,ddmfs_sc_col,"P(2);cu",dfsm_sched->schema_sched[ddmfs_sc_tp].
       start_time_am_pm
       WHERE curaccept IN ("AM", "PM"))
      SET dfsm_sched->schema_sched[ddmfs_sc_tp].start_time_am_pm = curaccept
     ENDIF
     SET ddmfs_sc_col = (ddmfs_sc_col+ 10)
     CALL clear(24,1,130)
     IF (ddmfs_sc_tp != 4)
      CALL accept(ddmfs_sc_row,ddmfs_sc_col,"99",dfsm_sched->schema_sched[ddmfs_sc_tp].end_time_hh
       WHERE curaccept >= 1
        AND curaccept <= 12)
      SET dfsm_sched->schema_sched[ddmfs_sc_tp].end_time_hh = curaccept
     ENDIF
     SET ddmfs_sc_col = (ddmfs_sc_col+ 3)
     CALL clear(24,1,130)
     IF (ddmfs_sc_tp != 4)
      CALL accept(ddmfs_sc_row,ddmfs_sc_col,"P(2);cu",dfsm_sched->schema_sched[ddmfs_sc_tp].
       end_time_am_pm
       WHERE curaccept IN ("AM", "PM"))
      SET dfsm_sched->schema_sched[ddmfs_sc_tp].end_time_am_pm = curaccept
     ENDIF
     SET ddmfs_sc_col = (ddmfs_sc_col+ 10)
     CALL clear(24,1,130)
     CALL accept(ddmfs_sc_row,ddmfs_sc_col,"99",dfsm_sched->schema_sched[ddmfs_sc_tp].num_of_runners
      WHERE curaccept >= 0
       AND (curaccept <= drr_flex_sched->max_runners))
     SET dfsm_sched->schema_sched[ddmfs_sc_tp].acc_num_of_runners = curaccept
     CALL clear(24,1,130)
     SET ddmfs_sc_done = 0
     SET ddmfs_sc_valid_input = 0
     SET ddmfs_sc_ret_msg = " "
     CALL dfsm_validate_maint_input(ddmfs_sc_tp,ddmfs_sc_done,ddmfs_sc_valid_input,ddmfs_sc_ret_msg,
      "SCHEMA")
     IF (ddmfs_sc_valid_input=0
      AND ddmfs_sc_done=1)
      SET ddmfs_sc_tp = 0
      SET ddmfs_sc_col = 18
      SET ddmfs_sc_row = 9
     ELSEIF (ddmfs_sc_valid_input=0)
      SET ddmfs_sc_tp = (ddmfs_sc_tp - 1)
      SET ddmfs_sc_col = 18
     ELSE
      SET ddmfs_sc_row = (ddmfs_sc_row+ 1)
      SET ddmfs_sc_col = 18
     ENDIF
     IF (ddmfs_sc_valid_input=1
      AND ddmfs_sc_done=1)
      SET ddmfs_sc_continue_loop = 0
     ENDIF
   ENDWHILE
   CALL dfsm_load_sched_to_screen(9,1,"N")
   CALL clear(19,1,130)
   CALL text(23,2,'Please enter "S" to save schedule or "Q" to quit without saving schedule : ')
   CALL accept(23,77,"p;cu"," "
    WHERE curaccept IN ("S", "Q"))
   SET ddmfs_option = curaccept
   IF (ddmfs_option="Q")
    RETURN(1)
   ELSE
    IF (dfsm_save_flex_schedule(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_load_flex_schedule(null)
   DECLARE dlfs_cnt = i2 WITH protect, noconstant(0)
   IF (drr_get_flexible_schedule(null)=0)
    CALL dfsm_disp_error_msg(null)
    RETURN(0)
   ENDIF
   SET dfsm_sched->readme_cnt = 4
   SET stat = alterlist(dfsm_sched->readme_sched,4)
   SET dfsm_sched->schema_cnt = 4
   SET stat = alterlist(dfsm_sched->schema_sched,4)
   FOR (dlfs_cnt = 1 TO drr_flex_sched->readme_time_periods)
     SET dfsm_sched->readme_sched[dlfs_cnt].time_period = dlfs_cnt
     SET dfsm_sched->readme_sched[dlfs_cnt].start_time_hhmm = drr_flex_sched->readme_schedule[
     dlfs_cnt].start_time
     SET ddmm_hold_time = cnvtupper(format(dfsm_sched->readme_sched[dlfs_cnt].start_time_hhmm,"HH;;s"
       ))
     SET dfsm_sched->readme_sched[dlfs_cnt].start_time_hh = cnvtint(trim(replace(replace(
         ddmm_hold_time,"AM","",0),"PM","",0),3))
     IF ((dfsm_sched->readme_sched[dlfs_cnt].start_time_hh=0))
      SET dfsm_sched->readme_sched[dlfs_cnt].start_time_hh = 12
     ENDIF
     IF (findstring("AM",ddmm_hold_time) > 0)
      SET dfsm_sched->readme_sched[dlfs_cnt].start_time_am_pm = "AM"
     ELSE
      SET dfsm_sched->readme_sched[dlfs_cnt].start_time_am_pm = "PM"
     ENDIF
     SET dfsm_sched->readme_sched[dlfs_cnt].end_time_hhmm = drr_flex_sched->readme_schedule[dlfs_cnt]
     .end_time
     SET ddmm_hold_time = cnvtupper(format(dfsm_sched->readme_sched[dlfs_cnt].end_time_hhmm,"HH;;s"))
     SET dfsm_sched->readme_sched[dlfs_cnt].end_time_hh = cnvtint(trim(replace(replace(ddmm_hold_time,
         "AM","",0),"PM","",0),3))
     IF ((dfsm_sched->readme_sched[dlfs_cnt].end_time_hh=0))
      SET dfsm_sched->readme_sched[dlfs_cnt].end_time_hh = 12
     ENDIF
     IF (findstring("AM",ddmm_hold_time) > 0)
      SET dfsm_sched->readme_sched[dlfs_cnt].end_time_am_pm = "AM"
     ELSE
      SET dfsm_sched->readme_sched[dlfs_cnt].end_time_am_pm = "PM"
     ENDIF
     SET dfsm_sched->readme_sched[dlfs_cnt].num_of_runners = drr_flex_sched->readme_schedule[dlfs_cnt
     ].num_of_runners
   ENDFOR
   IF ((drr_flex_sched->readme_time_periods < 4))
    SET dlfs_cnt = (drr_flex_sched->readme_time_periods+ 1)
    FOR (dlfs_cnt = dlfs_cnt TO dfsm_sched->readme_cnt)
      SET dfsm_sched->readme_sched[dlfs_cnt].time_period = dlfs_cnt
      SET dfsm_sched->readme_sched[dlfs_cnt].start_time_hhmm = 0
      SET dfsm_sched->readme_sched[dlfs_cnt].start_time_hh = 0
      SET dfsm_sched->readme_sched[dlfs_cnt].start_time_am_pm = "  "
      SET dfsm_sched->readme_sched[dlfs_cnt].end_time_hhmm = 0
      SET dfsm_sched->readme_sched[dlfs_cnt].end_time_hh = 0
      SET dfsm_sched->readme_sched[dlfs_cnt].end_time_am_pm = "  "
      SET dfsm_sched->readme_sched[dlfs_cnt].num_of_runners = 0
    ENDFOR
   ENDIF
   FOR (dlfs_cnt = 1 TO drr_flex_sched->schema_time_periods)
     SET dfsm_sched->schema_sched[dlfs_cnt].time_period = dlfs_cnt
     SET dfsm_sched->schema_sched[dlfs_cnt].start_time_hhmm = drr_flex_sched->schema_schedule[
     dlfs_cnt].start_time
     SET ddmm_hold_time = cnvtupper(format(dfsm_sched->schema_sched[dlfs_cnt].start_time_hhmm,"HH;;s"
       ))
     SET dfsm_sched->schema_sched[dlfs_cnt].start_time_hh = cnvtint(trim(replace(replace(
         ddmm_hold_time,"AM","",0),"PM","",0),3))
     IF ((dfsm_sched->schema_sched[dlfs_cnt].start_time_hh=0))
      SET dfsm_sched->schema_sched[dlfs_cnt].start_time_hh = 12
     ENDIF
     IF (findstring("AM",ddmm_hold_time) > 0)
      SET dfsm_sched->schema_sched[dlfs_cnt].start_time_am_pm = "AM"
     ELSE
      SET dfsm_sched->schema_sched[dlfs_cnt].start_time_am_pm = "PM"
     ENDIF
     SET dfsm_sched->schema_sched[dlfs_cnt].end_time_hhmm = drr_flex_sched->schema_schedule[dlfs_cnt]
     .end_time
     SET ddmm_hold_time = cnvtupper(format(dfsm_sched->schema_sched[dlfs_cnt].end_time_hhmm,"HH;;s"))
     SET dfsm_sched->schema_sched[dlfs_cnt].end_time_hh = cnvtint(trim(replace(replace(ddmm_hold_time,
         "AM","",0),"PM","",0),3))
     IF ((dfsm_sched->schema_sched[dlfs_cnt].end_time_hh=0))
      SET dfsm_sched->schema_sched[dlfs_cnt].end_time_hh = 12
     ENDIF
     IF (findstring("AM",ddmm_hold_time) > 0)
      SET dfsm_sched->schema_sched[dlfs_cnt].end_time_am_pm = "AM"
     ELSE
      SET dfsm_sched->schema_sched[dlfs_cnt].end_time_am_pm = "PM"
     ENDIF
     SET dfsm_sched->schema_sched[dlfs_cnt].num_of_runners = drr_flex_sched->schema_schedule[dlfs_cnt
     ].num_of_runners
   ENDFOR
   IF ((drr_flex_sched->schema_time_periods < 4))
    SET dlfs_cnt = (drr_flex_sched->schema_time_periods+ 1)
    FOR (dlfs_cnt = dlfs_cnt TO dfsm_sched->schema_cnt)
      SET dfsm_sched->schema_sched[dlfs_cnt].time_period = dlfs_cnt
      SET dfsm_sched->schema_sched[dlfs_cnt].start_time_hhmm = 0
      SET dfsm_sched->schema_sched[dlfs_cnt].start_time_hh = 0
      SET dfsm_sched->schema_sched[dlfs_cnt].start_time_am_pm = "  "
      SET dfsm_sched->schema_sched[dlfs_cnt].end_time_hhmm = 0
      SET dfsm_sched->schema_sched[dlfs_cnt].end_time_hh = 0
      SET dfsm_sched->schema_sched[dlfs_cnt].end_time_am_pm = "  "
      SET dfsm_sched->schema_sched[dlfs_cnt].num_of_runners = 0
    ENDFOR
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_load_sched_to_screen(dpms_strt_line,dpms_line_ctr,dpms_disp)
   DECLARE dlsts_cnt = i4 WITH protect, noconstant(0)
   DECLARE dlsts_row = i2
   DECLARE dlsts_col = i2
   SET dlsts_row = dpms_strt_line
   SET dlsts_col = 7
   FOR (dlsts_cnt = 1 TO dfsm_sched->readme_cnt)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dlsts_cnt))
     SET dlsts_col = (dlsts_col+ 11)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dfsm_sched->readme_sched[dlsts_cnt].start_time_hh))
     SET dlsts_col = (dlsts_col+ 3)
     CALL text(dlsts_row,dlsts_col,dfsm_sched->readme_sched[dlsts_cnt].start_time_am_pm)
     SET dlsts_col = (dlsts_col+ 10)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dfsm_sched->readme_sched[dlsts_cnt].end_time_hh))
     SET dlsts_col = (dlsts_col+ 3)
     CALL text(dlsts_row,dlsts_col,dfsm_sched->readme_sched[dlsts_cnt].end_time_am_pm)
     SET dlsts_col = (dlsts_col+ 10)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dfsm_sched->readme_sched[dlsts_cnt].num_of_runners))
     SET dlsts_row = (dlsts_row+ dpms_line_ctr)
     SET dlsts_col = 7
   ENDFOR
   IF (dpms_disp="Y")
    SET dlsts_row = dpms_strt_line
    SET dlsts_col = 61
   ELSE
    SET dlsts_row = (dpms_strt_line+ value(dfsm_sched->readme_cnt))
    SET dlsts_col = 7
   ENDIF
   FOR (dlsts_cnt = 1 TO dfsm_sched->schema_cnt)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dlsts_cnt))
     SET dlsts_col = (dlsts_col+ 11)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dfsm_sched->schema_sched[dlsts_cnt].start_time_hh))
     SET dlsts_col = (dlsts_col+ 3)
     CALL text(dlsts_row,dlsts_col,dfsm_sched->schema_sched[dlsts_cnt].start_time_am_pm)
     SET dlsts_col = (dlsts_col+ 10)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dfsm_sched->schema_sched[dlsts_cnt].end_time_hh))
     SET dlsts_col = (dlsts_col+ 3)
     CALL text(dlsts_row,dlsts_col,dfsm_sched->schema_sched[dlsts_cnt].end_time_am_pm)
     SET dlsts_col = (dlsts_col+ 10)
     CALL text(dlsts_row,dlsts_col,cnvtstring(dfsm_sched->schema_sched[dlsts_cnt].num_of_runners))
     SET dlsts_row = (dlsts_row+ dpms_line_ctr)
     IF (dpms_disp="Y")
      SET dlsts_col = 61
     ELSE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_load_runners_to_screen(dlrts_start_line,dlrts_ret_msg)
   DECLARE dlrts_idx = i2 WITH protect, noconstant(0)
   DECLARE dlrts_cnt = i4 WITH protect, noconstant(0)
   DECLARE dlrts_row = i2
   DECLARE dlrts_col = i2
   DECLARE dlrts_env_hold = f8 WITH protect, noconstant(0.0)
   DECLARE dlrts_max = i2 WITH protect, noconstant(0)
   DECLARE dlrts_file_name = vc WITH protect, noconstant(" ")
   DECLARE dlrts_len = i4 WITH protect, noconstant(0)
   IF (drr_cleanup_dm_info_runners(null)=0)
    CALL dfsm_disp_error_msg(null)
    RETURN(0)
   ENDIF
   SET dlrts_row = dlrts_start_line
   CALL clear(dlrts_row,1)
   CALL text(dlrts_row,2,"Runner")
   CALL text(dlrts_row,9,"Process Id")
   CALL text(dlrts_row,23,"HostName")
   CALL text(dlrts_row,43,"InstanceName")
   CALL text(dlrts_row,56,"Application Id")
   CALL text(dlrts_row,83,"Status")
   CALL text(dlrts_row,96,"Last Operation")
   CALL text(dlrts_row,121,"# Completed")
   SET dm_err->eproc = concat("Getting runner statuses...")
   IF ((dm_err->debug_flag > 622))
    SET message = nowindow
    CALL disp_msg(" ",dm_err->logfile,0)
    SET message = window
   ENDIF
   SET dfsm_runners->cnt = 0
   SET stat = alterlist(dfsm_runners->qual,0)
   SELECT INTO "nl:"
    FROM dm_info di,
     gv$session s,
     gv$instance i,
     gv$px_session p
    PLAN (di
     WHERE di.info_domain IN ("DM2_BACKGROUND_RUNNER", "DM2_SCHEMA_RUNNER", "DM2_README_RUNNER"))
     JOIN (s
     WHERE s.audsid=cnvtint(trim(di.info_name,3)))
     JOIN (i
     WHERE s.inst_id=i.inst_id)
     JOIN (p
     WHERE outerjoin(s.sid)=p.sid)
    ORDER BY di.info_name
    HEAD REPORT
     dfsm_runners->cnt = 0
    DETAIL
     IF (((p.qcsid=0) OR (p.sid=p.qcsid)) )
      dlrts_idx = 0
      IF ((dfsm_runners->cnt > 0))
       dlrts_idx = locateval(dlrts_idx,1,dfsm_runners->cnt,trim(di.info_name,3),dfsm_runners->qual[
        dlrts_idx].appl_id)
      ENDIF
      IF (dlrts_idx=0)
       dfsm_runners->cnt = (dfsm_runners->cnt+ 1)
       IF (mod(dfsm_runners->cnt,10)=1)
        stat = alterlist(dfsm_runners->qual,(dfsm_runners->cnt+ 9))
       ENDIF
       dfsm_runners->qual[dfsm_runners->cnt].appl_id = trim(di.info_name,3), dfsm_runners->qual[
       dfsm_runners->cnt].bkgrnd_runner_ind = 0, dfsm_runners->qual[dfsm_runners->cnt].working_on =
       " ",
       dfsm_runners->qual[dfsm_runners->cnt].active_ind = di.info_number, dfsm_runners->qual[
       dfsm_runners->cnt].num_w_applid = 1, dfsm_runners->qual[dfsm_runners->cnt].process_id = s
       .process,
       dfsm_runners->qual[dfsm_runners->cnt].host_name = i.host_name, dfsm_runners->qual[dfsm_runners
       ->cnt].instance_name = i.instance_name, dlrts_idx = dfsm_runners->cnt
      ELSE
       dfsm_runners->qual[dlrts_idx].num_w_applid = (dfsm_runners->qual[dlrts_idx].num_w_applid+ 1)
      ENDIF
      IF (di.info_domain="DM2_BACKGROUND_RUNNER")
       dfsm_runners->qual[dlrts_idx].bkgrnd_runner_ind = 1
      ENDIF
      IF (di.info_domain="DM2_SCHEMA_RUNNER")
       dfsm_runners->qual[dlrts_idx].working_on = "SCHEMA"
      ELSEIF (di.info_domain="DM2_README_RUNNER")
       dfsm_runners->qual[dlrts_idx].working_on = "READMES"
      ENDIF
     ENDIF
    FOOT REPORT
     stat = alterlist(dfsm_runners->qual,dfsm_runners->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL dfsm_disp_error_msg(null)
    RETURN(0)
   ENDIF
   FOR (dlrts_cnt = 1 TO dfsm_runners->cnt)
     IF ((dfsm_runners->qual[dlrts_cnt].active_ind=0))
      SET dfsm_runners->qual[dlrts_cnt].status = "Stopping"
     ELSE
      IF ((dfsm_runners->qual[dlrts_cnt].working_on=" ")
       AND (dfsm_runners->qual[dlrts_cnt].bkgrnd_runner_ind=1))
       SET dfsm_runners->qual[dlrts_cnt].status = "Idle"
      ELSE
       IF ((dfsm_runners->qual[dlrts_cnt].working_on="SCHEMA"))
        SELECT INTO "nl:"
         FROM dm2_ddl_ops_log ddol
         WHERE ddol.status IN ("RUNNING", "COMPLETE")
          AND (ddol.appl_id=dfsm_runners->qual[dlrts_cnt].appl_id)
         ORDER BY ddol.begin_dt_tm
         HEAD REPORT
          dfsm_runners->qual[dlrts_cnt].num_completed = 0
         DETAIL
          dfsm_runners->qual[dlrts_cnt].num_completed = (dfsm_runners->qual[dlrts_cnt].num_completed
          + 1), dfsm_runners->qual[dlrts_cnt].status = evaluate(ddol.status,"RUNNING","Running",
           "Idle"), dfsm_runners->qual[dlrts_cnt].current_last_op = ddol.op_type
         WITH nocounter
        ;end select
        IF (check_error("Determining schema status for application id")=1)
         CALL dfsm_disp_error_msg(null)
         RETURN(0)
        ENDIF
        IF (curqual=0)
         SET dfsm_runners->qual[dlrts_cnt].status = "Idle"
         SET dfsm_runners->qual[dlrts_cnt].current_last_op = " "
         SET dfsm_runners->qual[dlrts_cnt].num_completed = 0
        ELSE
         IF ((dfsm_runners->qual[dlrts_cnt].status="Running"))
          SET dfsm_runners->qual[dlrts_cnt].num_completed = (dfsm_runners->qual[dlrts_cnt].
          num_completed - 1)
         ENDIF
         SET dlrts_len = findstring("-",dfsm_runners->qual[dlrts_cnt].current_last_op)
         IF (dlrts_len > 0)
          SET dfsm_runners->qual[dlrts_cnt].current_last_op = trim(substring(1,(dlrts_len - 1),
            dfsm_runners->qual[dlrts_cnt].current_last_op))
         ENDIF
         SET dlrts_len = findstring("(",dfsm_runners->qual[dlrts_cnt].current_last_op)
         IF (dlrts_len > 0)
          SET dfsm_runners->qual[dlrts_cnt].current_last_op = trim(substring(1,(dlrts_len - 1),
       ...
       ELSE
        IF ((dm2_rr_misc->env_id=0.0))
         IF (dm2_get_env_data(1,dlrts_env_hold)=0)
          CALL dfsm_disp_error_msg(null)
          RETURN(0)
         ENDIF
         SET dm2_rr_misc->env_id = dlrts_env_hold
        ENDIF
        SELECT INTO "nl:"
         FROM dm_ocd_log d
         WHERE (d.environment_id=dm2_rr_misc->env_id)
          AND d.project_type=dm2_rr_readme
          AND d.status IN (dm2_rr_running, dm2_rr_done)
          AND (d.appl_ident=dfsm_runners->qual[dlrts_cnt].appl_id)
         ORDER BY d.start_dt_tm
         HEAD REPORT
          dfsm_runners->qual[dlrts_cnt].num_completed = 0
         DETAIL
          dfsm_runners->qual[dlrts_cnt].num_completed = (dfsm_runners->qual[dlrts_cnt].num_completed
          + 1), dfsm_runners->qual[dlrts_cnt].status = evaluate(d.status,dm2_rr_running,"Running",
           "Idle"), dfsm_runners->qual[dlrts_cnt].current_last_op = concat("Readme ",trim(d
            .project_name,3))
         WITH nocounter
        ;end select
        IF (check_error("Determining readme status for application id")=1)
         CALL dfsm_disp_error_msg(null)
         RETURN(0)
        ENDIF
        IF (curqual=0)
   ...
   IF ((dm2_rdbms_version->level1 >= 9))
    SELECT INTO "nl:"
     FROM user_resumable u,
      v$session v
     WHERE u.session_id=v.sid
      AND u.status="SUSPENDED"
     DETAIL
      dlrts_idx = locateval(dlrts_idx,1,size(dfsm_runners->qual,5),trim(cnvtstring(v.audsid,0)),
       dfsm_runners->qual[dlrts_idx].appl_id)
      IF (dlrts_idx > 0)
       dfsm_runners->qual[dlrts_idx].status = u.status
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error("Find SUSPENDED sessions")=1)
     CALL dfsm_disp_error_msg(null)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((dm_err->debug_flag > 622))
    SET message = nowindow
    CALL echorecord(dfsm_runners)
    SET message = window
   ENDIF
   SET dlrts_row = (dlrts_row+ 1)
   IF ((dfsm_runners->cnt > 10))
    IF (dfsm_write_runners_to_file(null)=0)
     RETURN(0)
    ENDIF
    SET dlrts_max = 10
    SET dlrts_ret_msg = concat("Total runners currently processing exceeded display limit.  See ",
     trim(dfsm_runners->file_name,3)," for complete list.")
   ELSE
    IF ((dfsm_runners->file_name > " "))
     IF (remove(dfsm_runners->file_name)=0)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = concat("Could not remove ",trim(dfsm_runners->file_name,3),
       " from ccluserdir.")
      SET dm_err->eproc = "Removing installation schedule runners report."
      CALL dfsm_disp_error_msg(null)
      RETURN(0)
     ENDIF
     SET dfsm_runners->file_name = " "
    ENDIF
    SET dlrts_max = dfsm_runners->cnt
    SET dlrts_ret_msg = " "
   ENDIF
   FOR (dlrts_cnt = 1 TO dlrts_max)
     CALL text(dlrts_row,2,cnvtstring(dlrts_cnt))
     CALL text(dlrts_row,9,substring(1,13,dfsm_runners->qual[dlrts_cnt].process_id))
     CALL text(dlrts_row,23,substring(1,19,dfsm_runners->qual[dlrts_cnt].host_name))
     CALL text(dlrts_row,43,substring(1,12,dfsm_runners->qual[dlrts_cnt].instance_name))
     CALL text(dlrts_row,56,substring(1,26,dfsm_runners->qual[dlrts_cnt].appl_id))
     CALL text(dlrts_row,83,substring(1,12,dfsm_runners->qual[dlrts_cnt].status))
     CALL text(dlrts_row,96,substring(1,34,dfsm_runners->qual[dlrts_cnt].current_last_op))
     CALL text(dlrts_row,121,substring(1,10,cnvtstring(dfsm_runners->qual[dlrts_cnt].num_completed)))
     SET dlrts_row = (dlrts_row+ 1)
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_validate_maint_input(dvmi_time_period,dvmi_done,dvmi_valid_input,dvmi_ret_msg,
  dm_process)
   DECLARE dvmi_time_period_changed = i2 WITH protect, noconstant(0)
   DECLARE dvmi_tp = i2 WITH protect, noconstant(0)
   DECLARE dvmi_num_runners_set = i2 WITH protect, noconstant(0)
   DECLARE dvmi_cnt = i2 WITH protect, noconstant(0)
   DECLARE dvmi_max = i2 WITH protect, noconstant(0)
   DECLARE dvmi_rd_sc_cnt = i2 WITH protect, noconstant(0)
   SET dvmi_done = 0
   SET dvmi_valid_input = 1
   SET dvmi_ret_msg = " "
   IF (dm_process="README")
    SET curalias valid_sched dfsm_sched->readme_sched[dvmi_time_period]
    SET curalias first_sched dfsm_sched->readme_sched[1]
    SET curalias last_sched dfsm_sched->readme_sched[4]
    SET dvmi_rd_sc_cnt = dfsm_sched->readme_cnt
   ELSEIF (dm_process="SCHEMA")
    SET curalias valid_sched dfsm_sched->schema_sched[dvmi_time_period]
    SET curalias first_sched dfsm_sched->schema_sched[1]
    SET curalias last_sched dfsm_sched->schema_sched[4]
    SET dvmi_rd_sc_cnt = dfsm_sched->schema_cnt
   ENDIF
   IF ((valid_sched->start_time_am_pm="PM")
    AND (valid_sched->start_time_hh != 12))
    SET valid_sched->acc_start_time_hhmm = cnvtint(concat(cnvtstring((valid_sched->start_time_hh+ 12)
       ),"00"))
    CALL echo(build("SR_if condition start",valid_sched->acc_start_time_hhmm))
   ELSEIF ((valid_sched->start_time_am_pm="AM")
    AND (valid_sched->start_time_hh=12))
    SET valid_sched->acc_start_time_hhmm = 0000
    CALL echo(build("SR_else if condition start",valid_sched->acc_start_time_hhmm))
   ELSE
    CALL echo("SR_start time hit")
    SET valid_sched->acc_start_time_hhmm = cnvtint(concat(cnvtstring(valid_sched->start_time_hh),"00"
      ))
    CALL echo(build("SR_else  condition start",valid_sched->acc_start_time_hhmm))
   ENDIF
   IF ((valid_sched->end_time_am_pm="PM")
    AND (valid_sched->end_time_hh != 12))
    SET valid_sched->acc_end_time_hhmm = cnvtint(concat(cnvtstring((valid_sched->end_time_hh+ 12)),
      "00"))
    CALL echo(build("SR_if condition start",valid_sched->acc_end_time_hhmm))
   ELSEIF ((valid_sched->end_time_am_pm="AM")
    AND (valid_sched->end_time_hh=12))
    SET valid_sched->acc_end_time_hhmm = 0000
    CALL echo(build("SR_else if condition start",valid_sched->acc_end_time_hhmm))
   ELSE
    SET valid_sched->acc_end_time_hhmm = cnvtint(concat(cnvtstring(valid_sched->end_time_hh),"00"))
    CALL echo(build("SR_else  condition start",valid_sched->acc_end_time_hhmm))
   ENDIF
   IF ((((valid_sched->acc_start_time_hhmm != valid_sched->start_time_hhmm)) OR ((valid_sched->
   acc_end_time_hhmm != valid_sched->end_time_hhmm))) )
    SET dvmi_time_period_changed = 1
   ENDIF
   IF ((valid_sched->acc_num_of_runners != valid_sched->num_of_runners))
    SET valid_sched->num_of_runners = valid_sched->acc_num_of_runners
   ENDIF
   IF ((first_sched->acc_start_time_hhmm=valid_sched->acc_end_time_hhmm))
    SET dvmi_done = 1
   ENDIF
   IF (dvmi_done=0)
    IF ((valid_sched->acc_start_time_hhmm=valid_sched->acc_end_time_hhmm))
     IF (dvmi_time_period != 1)
      SET dvmi_valid_input = 0
      SET dvmi_ret_msg = "Invalid input.  Please reenter data."
      RETURN(null)
     ENDIF
    ELSEIF ((valid_sched->acc_start_time_hhmm < valid_sched->acc_end_time_hhmm))
     IF (dvmi_time_period != 1)
      SET dvmi_max = (dvmi_time_period - 1)
      FOR (dvmi_cnt = 1 TO dvmi_max)
       IF (dm_process="README")
        SET curalias dvmi_sched dfsm_sched->readme_sched[dvmi_cnt]
       ELSEIF (dm_process="SCHEMA")
        SET curalias dvmi_sched dfsm_sched->schema_sched[dvmi_cnt]
       ENDIF
       IF ((dvmi_cnt=dfsm_sched->hour24_crossed_idx))
        IF ((((valid_sched->acc_end_time_hhmm >= dvmi_sched->start_time_hhmm)
         AND (valid_sched->acc_end_time_hhmm < 2400)) OR ((valid_sched->acc_end_time_hhmm > 0000)
         AND (valid_sched->acc_end_time_hhmm < dvmi_sched->end_time_hhmm))) )
         SET dvmi_valid_input = 0
         SET dvmi_ret_msg = "Invalid input.  Please reenter data."
         RETURN(null)
        ENDIF
     ...
     ENDIF
    ELSE
     IF ((dfsm_sched->hour24_crossed_idx > 0))
      SET dvmi_valid_input = 0
      SET dvmi_ret_msg = "Invalid input.  Please reenter data."
      RETURN(null)
     ELSEIF ((valid_sched->acc_end_time_hhmm > first_sched->start_time_hhmm)
      AND dvmi_time_period != 1)
      SET dvmi_valid_input = 0
      SET dvmi_ret_msg = "Invalid input.  Please reenter data."
      RETURN(null)
     ELSE
      SET dfsm_sched->hour24_crossed_idx = dvmi_time_period
     ENDIF
    ENDIF
   ENDIF
   IF (dvmi_valid_input=1)
    SET valid_sched->start_time_hhmm = valid_sched->acc_start_time_hhmm
    SET valid_sched->end_time_hhmm = valid_sched->acc_end_time_hhmm
   ENDIF
   IF (dm_process="README")
    SET curalias dv_sched dfsm_sched->readme_sched[dvmi_tp]
   ELSEIF (dm_process="SCHEMA")
    SET curalias dv_sched dfsm_sched->schema_sched[dvmi_tp]
   ENDIF
   IF (dvmi_done=0
    AND dvmi_time_period < 3
    AND dvmi_time_period_changed=1)
    CALL echo(build("SR_Incremental set is configured",dvmi_time_period))
    CALL echo(build("SR_Configure next row start time",valid_sched->end_time_hhmm,valid_sched->
      end_time_hh,valid_sched->end_time_am_pm))
    SET dvmi_tp = (dvmi_time_period+ 1)
    SET dv_sched->start_time_hhmm = valid_sched->end_time_hhmm
    SET dv_sched->start_time_hh = valid_sched->end_time_hh
    SET dv_sched->start_time_am_pm = valid_sched->end_time_am_pm
    SET dv_sched->end_time_hhmm = first_sched->start_time_hhmm
    SET dv_sched->end_time_hh = first_sched->start_time_hh
    SET dv_sched->end_time_am_pm = first_sched->start_time_am_pm
    SET dv_sched->num_of_runners = 0
    SET dvmi_tp = (dvmi_tp+ 1)
    FOR (dvmi_tp = dvmi_tp TO dvmi_rd_sc_cnt)
      SET dv_sched->start_time_hhmm = 0
      SET dv_sched->start_time_hh = 0
      SET dv_sched->start_time_am_pm = "  "
      SET dv_sched->end_time_hhmm = 0
      SET dv_sched->end_time_hh = 0
      SET dv_sched->end_time_am_pm = " "
      SET dv_sched->num_of_runners = 0
    ENDFOR
   ENDIF
   IF (dvmi_done=0
    AND dvmi_time_period=3)
    SET last_sched->start_time_hhmm = valid_sched->end_time_hhmm
    SET last_sched->start_time_hh = valid_sched->end_time_hh
    SET last_sched->start_time_am_pm = valid_sched->end_time_am_pm
    SET last_sched->end_time_hhmm = first_sched->start_time_hhmm
    SET last_sched->end_time_hh = first_sched->start_time_hh
    SET last_sched->end_time_am_pm = first_sched->start_time_am_pm
   ENDIF
   IF (dvmi_done=1
    AND dvmi_time_period < 4)
    SET dvmi_tp = (dvmi_time_period+ 1)
    FOR (dvmi_tp = dvmi_tp TO dvmi_rd_sc_cnt)
      SET dv_sched->start_time_hhmm = 0
      SET dv_sched->start_time_hh = 0
   ...
   ENDIF
   IF (dvmi_done=1)
    SET dvmi_num_runners_set = 0
    SET dvmi_tp = 1
    FOR (dvmi_tp = dvmi_tp TO dvmi_rd_sc_cnt)
      IF ((dv_sched->num_of_runners > 0))
       SET dvmi_num_runners_set = 1
       SET dvmi_tp = dvmi_rd_sc_cnt
      ENDIF
    ENDFOR
    IF (dvmi_num_runners_set=0)
     SET dvmi_valid_input = 0
     SET dvmi_ret_msg =
     "At least one time period must have <Num of Runners> valued.  Please reenter data."
     RETURN(null)
    ELSE
     SET dvmi_valid_input = 1
    ENDIF
   ENDIF
   RETURN(null)
   SET curalias valid_sched off
   SET curalias first_sched off
   SET curalias last_sched off
   SET curalias dv_sched off
   SET curalias dvmi_sched off
 END ;Subroutine
 SUBROUTINE dfsm_save_flex_schedule(null)
   DECLARE dsfs_tp = i4 WITH protect, noconstant
   SET dm_err->eproc = "Removing existing installation schedule."
   IF ((dm_err->debug_flag > 622))
    SET message = nowindow
    CALL disp_msg(" ",dm_err->logfile,0)
    SET message = window
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Inserting new installation schedule."
   IF ((dm_err->debug_flag > 622))
    SET message = nowindow
    CALL disp_msg(" ",dm_err->logfile,0)
    SET message = window
   ENDIF
   FOR (dsfs_tp = 1 TO dfsm_sched->readme_cnt)
     IF ((((((dfsm_sched->readme_sched[dsfs_tp].start_time_hhmm > 0)) OR ((dfsm_sched->readme_sched[
     dsfs_tp].end_time_hhmm > 0))) ) OR ((dfsm_sched->readme_sched[dsfs_tp].start_time_hhmm=0)
      AND (dfsm_sched->readme_sched[dsfs_tp].end_time_hhmm=0)
      AND (dfsm_sched->readme_sched[dsfs_tp].num_of_runners > 0))) )
      SET dsfs_info_name = concat("TIME PERIOD ",trim(cnvtstring(dsfs_tp),3)," - START")
      INSERT  FROM dm_info di
       SET di.info_domain = "DM2_FLEXIBLE_SCHEDULE_README", di.info_name = dsfs_info_name, di
        .info_date = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.info_char = " ", di.info_number = dfsm_sched->readme_sched[dsfs_tp].start_time_hhmm, di
        .updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.updt_applctx = 0, di.updt_cnt = dsfs_tp, di.updt_id = 0,
        di.updt_task = reqinfo->updt_task
       WITH nocounter
      ;end insert
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET dsfs_info_name = concat("TIME PERIOD ",trim(cnvtstring(dsfs_tp),3)," - END")
      INSERT  FROM dm_info di
       SET di.info_domain = "DM2_FLEXIBLE_SCHEDULE_README", di.info_name = dsfs_info_name, di
        .info_date = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.info_char = " ", di.info_number = dfsm_sched->readme_sched[dsfs_tp].end_time_hhmm, di
        .updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.updt_applctx = 0, di.updt_cnt = dsfs_tp, di.updt_id = 0,
        di.updt_task = reqinfo->updt_task
       WITH nocounter
      ...
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET dsfs_info_name = concat("TIME PERIOD ",trim(cnvtstring(dsfs_tp),3)," - NUM OF RUNNERS")
      INSERT  FROM dm_info di
       SET di.info_domain = "DM2_FLEXIBLE_SCHEDULE_README", di.info_name = dsfs_info_name, di
        .info_date = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.info_char = " ", di.info_number = dfsm_sched->readme_sched[dsfs_tp].num_of_runners, di
        .updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.updt_applctx = 0, di.updt_cnt = dsfs_tp, di.updt_id = 0,
        di.updt_task = reqinfo->updt_task
       WITH nocounter
     ...
     ENDIF
   ENDFOR
   FOR (dsfs_tp = 1 TO dfsm_sched->schema_cnt)
     SET dm_err->eproc = "Save schema hit"
     CALL disp_msg(" ",dm_err->logfile,0)
     IF ((((((dfsm_sched->schema_sched[dsfs_tp].start_time_hhmm > 0)) OR ((dfsm_sched->schema_sched[
     dsfs_tp].end_time_hhmm > 0))) ) OR ((dfsm_sched->schema_sched[dsfs_tp].start_time_hhmm=0)
      AND (dfsm_sched->schema_sched[dsfs_tp].end_time_hhmm=0)
      AND (dfsm_sched->schema_sched[dsfs_tp].num_of_runners > 0))) )
      SET dsfs_info_name = concat("TIME PERIOD ",trim(cnvtstring(dsfs_tp),3)," - START")
      INSERT  FROM dm_info di
       SET di.info_domain = "DM2_FLEXIBLE_SCHEDULE_SCHEMA", di.info_name = dsfs_info_name, di
        .info_date = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.info_char = " ", di.info_number = dfsm_sched->schema_sched[dsfs_tp].start_time_hhmm, di
        .updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.updt_applctx = 0, di.updt_cnt = dsfs_tp, di.updt_id = 0,
        di.updt_task = reqinfo->updt_task
       WITH nocounter
      ...
      INSERT  FROM dm_info di
       SET di.info_domain = "DM2_FLEXIBLE_SCHEDULE_SCHEMA", di.info_name = dsfs_info_name, di
        .info_date = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.info_char = " ", di.info_number = dfsm_sched->schema_sched[dsfs_tp].end_time_hhmm, di
        .updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.updt_applctx = 0, di.updt_cnt = dsfs_tp, di.updt_id = 0,
        di.updt_task = reqinfo->updt_task
       WITH nocounter
      ...
      INSERT  FROM dm_info di
       SET di.info_domain = "DM2_FLEXIBLE_SCHEDULE_SCHEMA", di.info_name = dsfs_info_name, di
        .info_date = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.info_char = " ", di.info_number = dfsm_sched->schema_sched[dsfs_tp].num_of_runners, di
        .updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
        di.updt_applctx = 0, di.updt_cnt = dsfs_tp, di.updt_id = 0,
        di.updt_task = reqinfo->updt_task
       WITH nocounter
     ...
     ENDIF
   ENDFOR
   SET dm_err->eproc = "Determine if installation schedule status exists."
   IF ((dm_err->debug_flag > 622))
    SET message = nowindow
    CALL disp_msg(" ",dm_err->logfile,0)
    SET message = window
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ELSEIF (curqual=0)
    SET dm_err->eproc = 'Inserting installation schedule status row - defaulting to "Off".'
    IF ((dm_err->debug_flag > 622))
     SET message = nowindow
     CALL disp_msg(" ",dm_err->logfile,0)
     SET message = window
    ENDIF
    INSERT  FROM dm_info di
     SET di.info_domain = "DM2_FLEXIBLE_SCHEDULE", di.info_name = "STATUS", di.info_date =
      cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
      di.info_char = "OFF", di.info_number = 1, di.updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,
        curtime3)),
      di.updt_applctx = 0, di.updt_cnt = 0, di.updt_id = 0,
      di.updt_task = reqinfo->updt_task
     WITH nocounter
    ...
    ENDIF
   ENDIF
   SET dm_err->eproc = "Determine if installation schedule max runner limit row exists."
   IF ((dm_err->debug_flag > 622))
    SET message = nowindow
    CALL disp_msg(" ",dm_err->logfile,0)
    SET message = window
   ...
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="DM2_FLEXIBLE_SCHEDULE"
     AND di.info_name="RUNNER TIME LIMIT"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
   ...
   ELSEIF (curqual=0)
    SET dm_err->eproc =
    "Inserting installation schedule max runner limit row - defaulting to 10 hours."
    IF ((dm_err->debug_flag > 622))
     SET message = nowindow
     CALL disp_msg(" ",dm_err->logfile,0)
     SET message = window
    ENDIF
    INSERT  FROM dm_info di
     SET di.info_domain = "DM2_FLEXIBLE_SCHEDULE", di.info_name = "RUNNER TIME LIMIT", di.info_date
       = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
      di.info_char = " ", di.info_number = 10, di.updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,
        curtime3)),
      di.updt_applctx = 0, di.updt_cnt = 0, di.updt_id = 0,
      di.updt_task = reqinfo->updt_task
     WITH nocounter
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_toggle_schedule(dts_ret_msg)
   SET dts_ret_msg = " "
   IF ((((drr_flex_sched->readme_time_periods=0)) OR ((drr_flex_sched->sched_set_up=0))) )
    SET dts_ret_msg =
    "Installation schedule not set up or is incomplete.  Confirm schedule before toggling on/off."
    RETURN(1)
   ENDIF
   IF ((((drr_flex_sched->schema_time_periods=0)) OR ((drr_flex_sched->sched_set_up=0))) )
    SET dts_ret_msg =
    "Installation schedule not set up or is incomplete.  Confirm schedule before toggling on/off."
    RETURN(1)
   ENDIF
   IF ((drr_flex_sched->status=0))
    ...
    SELECT INTO "nl:"
     FROM dm_info di
     WHERE di.info_domain IN ("DM2_INSTALL_RUNNER", "DM2_SCHEMA_RUNNER", "DM2_README_RUNNER")
      AND di.info_number=1
     WITH nocounter, maxqual(ddol,1)
    ;end select
    IF (check_error("Determining if interactive runners are actively executing.")=1)
     CALL dfsm_disp_error_msg(null)
     RETURN(0)
    ENDIF
    IF (curqual > 0)
     SET dts_ret_msg =
     "Cannot toggle installation schedule on until all interactive runners have completed."
     RETURN(1)
    ENDIF
   ENDIF
   IF ((drr_flex_sched->status=1))
    ...
    SELECT INTO "nl:"
     ...
     WITH nocounter, maxqual(ddol,1)
    ;end select
    IF (check_error("Determining if packages using the installation schedule are actively running.")=
    1)
     CALL dfsm_disp_error_msg(null)
     RETURN(0)
    ENDIF
    IF (curqual > 0)
     SET dts_ret_msg =
     "Cannot toggle installation schedule off until all packages using the schedule have completed."
     RETURN(1)
    ENDIF
   ENDIF
   SET dm_err->eproc = concat("Updating installation schedule status to ",evaluate(drr_flex_sched->
     status,0,"ON","OFF"),".")
   IF ((dm_err->debug_flag > 622))
    SET message = nowindow
    CALL disp_msg(" ",dm_err->logfile,0)
   ...
   ENDIF
   UPDATE  FROM dm_info di
    SET di.info_char = evaluate(drr_flex_sched->status,0,"ON","OFF"), di.updt_dt_tm = cnvtdatetimeutc
     (cnvtdatetime(curdate,curtime3))
    WHERE di.info_domain="DM2_FLEXIBLE_SCHEDULE"
     AND di.info_name="STATUS"
    WITH nocounter
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ELSE
    SET dts_ret_msg = concat("Installation schedule has been toggled ",evaluate(drr_flex_sched->
      status,0,'"ON"','"OFF"'),".")
    COMMIT
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_stop_pkg_installs(null)
   DECLARE dspi_continue_loop = i2 WITH protect, noconstant(1)
   DECLARE dspi_ret_msg = vc WITH protect, noconstant(" ")
   DECLARE dspi_option = vc WITH protect, noconstant(" ")
   WHILE (dspi_continue_loop=1)
     CALL clear(1,1)
     CALL box(1,1,3,132)
     CALL text(2,3,"Stopping all package installation processing...")
     CALL text(5,3,
      "Please confirm action to stop all package installations using the installation schedule.")
     CALL text(7,3,'Press "C" to confirm action, or "B" to go back without taking action. ')
     CALL text(9,3,"(C)onfirm Action,  (B)ack  : ")
     CALL accept(9,30,"p;cu"," "
      WHERE curaccept IN ("C", "B"))
     SET dspi_option = curaccept
     CASE (dspi_option)
      OF "B":
       SET dspi_continue_loop = 0
      OF "C":
       IF (drr_stop_runners("ALL",0)=0)
        CALL dfsm_disp_error_msg(null)
        RETURN(0)
       ENDIF
       IF (drr_stop_installs_using_flex_sched(null)=0)
        CALL dfsm_disp_error_msg(null)
        RETURN(0)
       ENDIF
       SET dspi_continue_loop = 0
       IF (dfsm_display_monitor_screen(null)=0)
        RETURN(0)
       ENDIF
     ENDCASE
   ENDWHILE
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_write_runners_to_file(null)
   SET dwrtf_timestamp = format(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),";;q")
   IF ((dfsm_runners->file_name=" "))
    SET dfsm_runners->file_name = build("dm2_flex_sched_runners_",trim(currdbhandle,3),".rpt")
   ENDIF
   SET dm_err->eproc = "Creating Installation Schedule Runners Report"
   SELECT INTO value(dfsm_runners->file_name)
    FROM dual
    HEAD REPORT
     proc_id = fillstring(13," "), host_name = fillstring(19," "), instance_name = fillstring(12," "),
     appl_id = fillstring(26," ")
    DETAIL
     col 10, "Installation Schedule Runners Report", row + 2,
     col 10, "  as of ", dwrtf_timestamp,
     row + 1, col 10, "--------------------------------",
     row + 2, col 2, "Runner",
     col 9, "Process Id", col 23,
     "HostName", col 43, "InstanceName",
     col 56, "Application Id", col 83,
     "Status", row + 2
     FOR (dlrts_cnt = 1 TO dfsm_runners->cnt)
       col 2, dlrts_cnt"##", proc_id = substring(1,13,dfsm_runners->qual[dlrts_cnt].process_id),
       col 9, proc_id, host_name = substring(1,19,dfsm_runners->qual[dlrts_cnt].host_name),
       col 23, host_name, instance_name = substring(1,12,dfsm_runners->qual[dlrts_cnt].instance_name),
       col 43, instance_name, appl_id = substring(1,26,dfsm_runners->qual[dlrts_cnt].appl_id),
       col 56, appl_id, col 83,
       dfsm_runners->qual[dlrts_cnt].status, row + 1
     ENDFOR
    FOOT REPORT
     row + 0
    WITH nocounter, format = variable, formfeed = none,
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfsm_disp_error_msg(null)
   SET message = nowindow
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   RETURN(null)
 END ;Subroutine
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "LEAVING DM2_FLEXIBLE_SCHEDULE_MENU"
 CALL final_disp_msg(dfsm_logfile_prefix)
END GO
