CREATE PROGRAM dm2_aload_adoc_dcd
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_adoc_dcd",".log","dm2_aload_adoc_dcd")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_adoc_dcd"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dacd_cnt = i4 WITH protect, noconstant(0)
 DECLARE dacd_str = vc WITH protect, noconstant("")
 DECLARE dacd_req_size = i4 WITH protect, noconstant(0)
 FREE RECORD dacd_maint
 RECORD dacd_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET dacd_maint->num_of_inserts = 0
 SET dacd_maint->num_of_updates = 0
 SET dacd_maint->num_of_none = 0
 SET dacd_req_size = size(requestin->list_0,5)
 SET stat = alterlist(dacd_maint->qual,dacd_req_size)
 IF (dac_pop_coldic_rec("DM_COLUMNS_DOC")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Initializing maintenance structure."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dacd_req_size))
  DETAIL
   dacd_maint->qual[d.seq].maint_type = "I", dacd_maint->num_of_inserts = (dacd_maint->num_of_inserts
   + 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining maintenance type for DM_COLUMNS_DOC."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_columns_doc dat,
   (dummyt d  WITH seq = value(dacd_req_size))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (dat
   WHERE dat.owner=currdbuser
    AND (dat.table_name=requestin->list_0[d.seq].table_name)
    AND (dat.column_name=requestin->list_0[d.seq].column_name))
  ORDER BY dat.table_name, dat.column_name
  DETAIL
   IF (cnvtint(requestin->list_0[d.seq].updt_cnt) >= dat.updt_cnt)
    dacd_maint->qual[d.seq].maint_type = "U", dacd_maint->num_of_updates = (dacd_maint->
    num_of_updates+ 1), dacd_maint->num_of_inserts = (dacd_maint->num_of_inserts - 1)
   ELSE
    dacd_maint->qual[d.seq].maint_type = "N", dacd_maint->num_of_none = (dacd_maint->num_of_none+ 1),
    dacd_maint->num_of_inserts = (dacd_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dacd_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((dacd_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(dacd_req_size) )"),0)
  ...
  CALL dm2_push_cmd("plan d where dacd_maint->qual[d.seq].maint_type = 'I' ",0)
  CALL dm2_push_cmd("join dcv  ",0)
  IF (dm2_push_cmd("with nocounter go",1)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  SET dadcd_curqual = (dadcd_curqual+ curqual)
  IF ((curqual != dacd_maint->num_of_inserts))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Inserted:",curqual,". Does not match expected rows inserted:",
    dacd_maint->num_of_inserts)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
  COMMIT
 ENDIF
 IF ((dacd_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where dacd_maint->qual[d.seq].maint_type = 'U' ",0)
  ...
  IF (dm2_push_cmd(" and requestin->list_0[d.seq].column_name = dcv.column_name  with nocounter go",1
   )=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  SET dadcd_curqual = (dadcd_curqual+ curqual)
  IF ((curqual != dacd_maint->num_of_updates))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Updated:",curqual,". Does not match expected rows updated:",
    dacd_maint->num_of_updates)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_adoc_dcd completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_adoc_dcd")
END GO
