CREATE PROGRAM afc_get_locations:dba
 DECLARE versionnbr = vc
 SET versionnbr = "000"
 CALL echo(build("AFC_GET_LOCATIONS Version: ",versionnbr))
 RECORD reply(
   1 location[*]
     2 location_cd = f8
     2 location_disp = c40
 ...
 DECLARE code_set = i4 WITH protect, constant(220)
 ...
 IF ( NOT (getlogicaldomain(ld_concept_organization,curlogicaldomainid)))
  GO TO end_program
 ENDIF
 SET codesetrequest->objarray[1].codeset = 220
 SET codesetrequest->objarray[1].meaning = request->cdf_meaning
 ...
 IF ((codesetreply->status_data.status != "S"))
  ...
  GO TO end_program
 ENDIF
 IF (size(codesetreply->objarray,5) > 0)
  SELECT INTO "nl:"
   FROM location l,
    organization o
   PLAN (l
    WHERE expand(num,1,size(codesetreply->objarray,5),l.location_cd,codesetreply->objarray[num].code)
    )
    JOIN (o
   ...
   DETAIL
    count2 += 1
    IF (mod(count2,10)=1)
     stat = alterlist(reply->location,(count2+ 9))
    ENDIF
    reply->location[count2].location_cd = l.location_cd
   WITH nocounter, expand = 1
  ;end select
  SET stat = alterlist(reply->location,count2)
  IF (curqual > 0)
