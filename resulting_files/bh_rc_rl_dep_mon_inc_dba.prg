CREATE PROGRAM bh_rc_rl_dep_mon_inc:dba
 CALL echo("***** bh_rc_rl_dep_mon_inc.prg - 677918 *****")
 ...
 SUBROUTINE filltemprec(null)
   FOR (lchgcnt = 1 TO size(request->integer_params,5))
     CASE (cnvtupper(trim(request->integer_params[lchgcnt].key_txt,3)))
      OF "DEP_MONTH":
       SET tmprec->dep_annual = (request->integer_params[lchgcnt].integer_value * 12)
     ENDCASE
   ENDFOR
 END ;Subroutine
 DECLARE createreply(null) = null
 SUBROUTINE createreply(null)
  CALL addtoreplyintegers("DEP_ANNUAL",tmprec->dep_annual)
