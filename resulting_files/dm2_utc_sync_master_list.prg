CREATE PROGRAM dm2_utc_sync_master_list
 ...
 SUBROUTINE dm2_get_remote_dbase_name(dgrdn_db_link,dgrdn_name_out)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dusml_dbase_name = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_utc_sync_master_list",".log","dm2_utc_sync_master_list")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_utc_sync_master_list"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Querying from ALL_DB_LINKS to see if MIG_TARGET dblink exist."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM all_db_links adl
  WHERE adl.db_link="MIG_TARGET.*"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "MIG_TARGET dblink does not exist. Cannot progress further."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Query for Target Database name."
 CALL disp_msg("",dm_err->logfile,0)
 IF (dm2_get_remote_dbase_name("MIG_TARGET",dusml_dbase_name)=0)
  GO TO exit_program
 ENDIF
 IF (dum_load_date_columns("U",dusml_dbase_name)=0)
  GO TO exit_program
 ENDIF
 IF (dum_load_date_columns("C",dusml_dbase_name)=0)
  GO TO exit_program
 ENDIF
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Ending dm2_utc_sync_master_list"
 CALL final_disp_msg("dm2_mig_utc_src_sync")
END GO
