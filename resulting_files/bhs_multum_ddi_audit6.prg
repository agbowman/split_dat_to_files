CREATE PROGRAM bhs_multum_ddi_audit6
 PROMPT
  ...
  *
  FROM (dummyt d  WITH seq = value(size(eks->list,5))),
   ...
   dummyt d2
  PLAN (d
   WHERE (eks->list[d.seq].severity >= 0))
   JOIN (oc
    ...
    DETAIL
     ...
     IF (report_type="2")
      ...
      output_line = build(',"',trim(uar_get_code_display(eks->list[d.seq].trigger.catalog_cd)),'","',
       ...
       eks->list[d.seq].patient_name,'","',facility_disp,'","',eks->list[d.seq].trigger.
       physician_name,
