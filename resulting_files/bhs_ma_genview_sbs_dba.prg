CREATE PROGRAM bhs_ma_genview_sbs:dba
 SET rhead =
 ...
 SET labordflag = 0
 SET rtab = "\tab "
 DECLARE sbs_result = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "STATEBEHAVIORALSCALESBSPEDIATRIC"))
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  HEAD REPORT
   cnt = 0, temp_disp = "State Behavioral Scale (SBS)", drec->line_qual[lidx].disp_line = concat(
    rhead,rh2bu," ",rh2bu3,trim(temp_disp),
    reol,reol,wr),
  ...
  DETAIL
   ...
   IF (cnt > 0)
    ...
    concat("  ",date,"  ",trim(ce_event_disp),"  ",
