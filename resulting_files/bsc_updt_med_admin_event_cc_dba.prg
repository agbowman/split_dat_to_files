CREATE PROGRAM bsc_updt_med_admin_event_cc:dba
 ...
 SET ms_child_c_info_domain_nm = "UPDATE ADMIN EVENT - EVENT_ID"
 SET ms_child_c_max_name = "MAX ADMIN EVENT_ID EVALUATED"
 ...
 CALL echo("Updating the MED_ADMIN_EVENT table")
 CALL parser(
  "rdb asis(^update /*+  CERNER_README<BSC_UPDT_MED_ADMIN_EVENT_C:Q1> */ med_admin_event mae^)")
 CALL parser("asis(^ set (mae.template_order_id, mae.needs_verify_flag, mae.verified_prsnl_id,^)")
 CALL parser("asis(^ mae.verification_dt_tm,mae.verification_tz, mae.updt_task, mae.updt_dt_tm ) =^)"
  )
 CALL parser("asis(^ (select m.template_order_id,^)")
 CALL parser(
  "asis(^ nvl(decode(oa.needs_verify_ind, 2, 1, 1, oa.needs_verify_ind, 0, oa.needs_verify_ind, ^)")
 CALL parser(
  "asis(^ med_admin_event_orv_val_2(m.template_order_id, m.action_seq,m.event_dt_tm,oa.needs_verify_ind, 'FLAG')),0.0),^)"
  )
 CALL parser("asis(^ nvl(decode(oa.needs_verify_ind, 0, oa.action_personnel_id, 1, 0, 4, 0,^)")
 CALL parser(
  "asis(^ med_admin_event_orv_val_2(m.template_order_id, m.action_seq,m.event_dt_tm, oa.needs_verify_ind, 'PRSNL')),0.0),^)"
  )
 CALL parser("asis(^ nvl(decode(oa.needs_verify_ind, 0, oa.action_dt_tm, 1, NULL, 4, NULL,^)")
 CALL parser(
  "asis(^ med_admin_event_orv_date_2(m.template_order_id, m.action_seq,m.event_dt_tm,oa.needs_verify_ind, 'DATE')),NULL),^)"
  )
 CALL parser("asis(^ nvl(decode(oa.needs_verify_ind, 0, oa.action_tz, 1, 0, 4, 0,^)")
 CALL parser(
  "asis(^ med_admin_event_orv_val_2(m.template_order_id, m.action_seq,m.event_dt_tm,oa.needs_verify_ind, 'TZONE')),0.0),^)"
  )
 CALL parser(concat("asis(^  ",cnvtstring(reqinfo->updt_task),",^)"))
 CALL parser("asis(^        sysdate^)")
 CALL parser("asis(^ from order_action oa, ^)")
 CALL parser("asis(^ ((select mae.med_admin_event_id as med_admin_event_id,^)")
 CALL parser(
  "asis(^ decode(o.template_order_id,0.0,o.order_id,o.template_order_id) as template_order_id,^)")
 CALL parser("asis(^ mae.documentation_action_sequence as action_seq,^)")
 CALL parser("asis(^ mae.beg_dt_tm as event_dt_tm ^)")
 CALL parser("asis(^ from med_admin_event mae, orders o^)")
 CALL parser("asis(^ where mae.template_order_id = 0.0^)")
 CALL parser("asis(^ and mae.order_id > 0.0^)")
 CALL parser("asis(^ and o.order_id = mae.order_id) )m^)")
 CALL parser("asis(^ where oa.order_id(+) = m.template_order_id^)")
 CALL parser("asis(^ and oa.action_sequence(+) = m.action_seq^)")
 CALL parser("asis(^ and m.med_admin_event_id = mae.med_admin_event_id)^)")
 CALL parser("asis(^ where mae.med_admin_event_id in (select m2.med_admin_event_id ^)")
 CALL parser("asis(^ from order_action oa2, ^)")
 CALL parser("asis(^ ((select mae2.med_admin_event_id as med_admin_event_id,^)")
 CALL parser("asis(^ decode(o2.template_order_id,0.0,o2.order_id, ^)")
 CALL parser("asis(^ o2.template_order_id) as template_order_id,^)")
 CALL parser("asis(^ mae2.documentation_action_sequence as action_seq^)")
 CALL parser("asis(^ from med_admin_event mae2, orders o2^)")
 CALL parser(concat("asis(^ where mae2.med_admin_event_id >=",cnvtstring(mf_min_id),"^)"))
 CALL parser(concat("asis(^ and mae2.med_admin_event_id <=",cnvtstring(mf_max_id),"^)"))
 CALL parser("asis(^ and mae2.template_order_id = 0.0^)")
 CALL parser("asis(^	and mae2.order_id != 0.0^)")
 CALL parser("asis(^ and o2.order_id = mae2.order_id) )m2^)")
