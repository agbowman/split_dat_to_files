CREATE PROGRAM bhs_ma_attd_signed_disch_sum
 PROMPT
 ...
 SELECT DISTINCT INTO value(var_output)
  ...
  physician = substring(1,40,trim(p.name_full_formatted)), date = format(cep.updt_dt_tm,
  ...
  lookbackend = format(cnvtdatetime(lbdateend),";;q")
  FROM scd_story s,
   ...
   encntr_alias ea1
  PLAN (srp
   WHERE trim(srp.display_key)=value("PHYSICIANDISCHARGESUMMARY*"))
   JOIN (ssp
