CREATE PROGRAM dm_rmc_drop_all_r:dba
 ...
 FREE RECORD drdar_r_tbl_defs
 RECORD drdar_r_tbl_defs(
   1 list[*]
     2 table_name = vc
 )
 FREE RECORD drdar_parse
 RECORD drdar_parse(
   1 stmt[*]
     2 str = vc
 )
 ...
 DECLARE s_drp_def = vc
 DECLARE v_valid_ind = i2
 DECLARE drdar_num = i4
 DECLARE drdar_def_cnt = i4 WITH protect, noconstant(0)
 DECLARE drdar_loop = i4 WITH protect, noconstant(0)
 SET v_valid_ind = 0
 IF (check_logfile("dm_rmc_drop_r",".log","DM_RMC_DROP_ALL_R LOG")=0)
  SET dm_err->err_ind = 1
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (mod(v_tab_cnt,10)=1)
    stat = alterlist(all_tables->list,(v_tab_cnt+ 9))
   ENDIF
   all_tables->list[v_tab_cnt].r_tab_name = ut.table_name, all_tables->list[v_tab_cnt].tab_suffix =
   substring((size(all_tables->list[v_tab_cnt].r_tab_name,1) - 5),4,all_tables->list[v_tab_cnt].
    r_tab_name)
  FOOT REPORT
   stat = alterlist(all_tables->list,v_tab_cnt)
  WITH nocounter
 ;end select
 IF (check_error("Gathering list of $R tables to drop") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dar
 ENDIF
 SELECT INTO "nl:"
  ...
  WHERE dtd.table_name=dtd.full_table_name
   AND expand(drdar_num,1,v_tab_cnt,dtd.table_suffix,all_tables->list[drdar_num].tab_suffix)
  DETAIL
   drdar_num = locateval(drdar_num,1,v_tab_cnt,dtd.table_suffix,all_tables->list[drdar_num].
    tab_suffix), all_tables->list[drdar_num].tab_name = dtd.table_name
  WITH nocounter, expand = 1
 ;end select
 IF (check_error("Gathering live table information") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dar
 ENDIF
 FOR (for_cnt = 1 TO v_tab_cnt)
  CALL drop_$r_tab(all_tables->list[for_cnt].tab_name)
  IF (check_error("Gathering list of $R tables to drop") != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dm_err_msg = dm_err->emsg
   SET dm_err->err_ind = 0
  ENDIF
 ENDFOR
 IF (dm_err_msg > " ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = dm_err_msg
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dar
 ENDIF
 SELECT INTO "nl:"
  FROM dtable d
  WHERE d.table_name="*$R"
  DETAIL
   drdar_def_cnt = (drdar_def_cnt+ 1), stat = alterlist(drdar_r_tbl_defs->list,drdar_def_cnt),
   drdar_r_tbl_defs->list[drdar_def_cnt].table_name = d.table_name
  WITH nocounter
 ;end select
 IF (check_error("Gathering list of $R table CCL defs to drop") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dar
 ENDIF
 IF (drdar_def_cnt > 0)
  FOR (drdar_loop = 1 TO drdar_def_cnt)
    SET stat = initrec(drdar_parse)
    SET stat = alterlist(drdar_parse->stmt,1)
    SET drdar_parse->stmt[1].str = concat("drop table ",drdar_r_tbl_defs->list[drdar_loop].table_name,
     " go")
    EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DRDAR_PARSE")
    IF (check_error("Performing drop CCL table def command") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_dar
    ENDIF
  ENDFOR
 ENDIF
 SET dm_err->emsg = "Checking for remaining REG MC triggers"
 SELECT INTO "nl:"
  FROM user_triggers ut
  WHERE ut.trigger_name="REFCHG*REG*MC*"
  WITH nocounter
 ;end select
 IF (check_error("Checking for remaining REG MC triggers") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dar
 ELSEIF (curqual != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "NOT ALL REG_* TRIGGERS WERE DROPPED"
  SET v_valid_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dar
 ENDIF
 SET dm_err->emsg = "Checking for remaining $R tables"
 SELECT INTO "nl:"
  FROM user_tables ut
  WHERE ut.table_name="*$R"
  WITH nocounter
 ;end select
 IF (check_error("Checking for remaining $R tables") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dar
 ELSEIF (curqual != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "NOT ALL $R OBJECTS WERE DROPPED"
  SET v_valid_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dar
 ENDIF
 SET dm_err->emsg = "Checking for remaining $R definitions"
 SELECT INTO "nl:"
  FROM dtable dt
  WHERE dt.table_name="*$R"
  WITH nocounter
 ;end select
 IF (check_error("Checking for remaining $R definitions") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dar
 ELSEIF (curqual != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "NOT ALL $R DEFINITIONS WERE DROPPED"
  SET v_valid_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dar
 ENDIF
#exit_dar
 IF (((v_valid_ind=1) OR ((dm_err->err_ind=1))) )
  CALL echo("DM_RMC_DROP_ALL_R DID NOT FINISH WITH SUCCESS")
 ELSE
  CALL echo("DM_RMC_DROP_ALL_R FINISHED WITH SUCCESS")
 ENDIF
END GO
