CREATE PROGRAM ams_pft_get_crit_health_plans:dba
 ...
 IF (size(pft_reply->healthplans,5) > 0)
  ...
  FOR (loopcnt = 1 TO size(pft_reply->healthplans,5))
    SET recordpos = getnextrecord(0)
    SET stat = setstringfield(recordpos,disppos,substring(1,100,pft_reply->healthplans[loopcnt].
      healthplanname))
    SET stat = setrealfield(recordpos,valuepos,pft_reply->healthplans[loopcnt].healthplanid)
  ENDFOR
