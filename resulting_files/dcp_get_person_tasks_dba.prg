CREATE PROGRAM dcp_get_person_tasks:dba
 SET count1 = 0
 SET count2 = 0
 ...
 SELECT
  ...
  ELSEIF (types_to_get > 0
   ...
   AND (request->end_dt_tm > 0)
   AND encntrs_to_get > 0)
   FROM (dummyt d2  WITH seq = value(types_to_get)),
   ...
   PLAN (d2)
    JOIN (d4)
    JOIN (ta
    WHERE (ta.task_type_cd=request->type_list[d2.seq].task_type_cd)
  ...
  ELSEIF (types_to_get=0
   ...
   AND (request->end_dt_tm > 0)
   AND encntrs_to_get > 0)
   FROM (dummyt d2  WITH seq = value(encntrs_to_get)),
    task_activity ta,
    (dummyt d3  WITH seq = 1),
    task_activity_assignment taa
   PLAN (d2)
    JOIN (ta
    WHERE (ta.encntr_id=request->encntr_list[d2.seq].encntr_id)
     AND ta.active_ind=1
     AND ta.task_status_cd != dropped
     AND ta.task_status_cd != inerror
     AND ((ta.task_dt_tm BETWEEN cnvtdatetime(request->beg_dt_tm) AND cnvtdatetime(request->end_dt_tm
