CREATE PROGRAM dm_ap_report_task_rows:dba
 ...
 DECLARE months_to_keep = i4 WITH protect, noconstant(0)
 DECLARE minimum_months_to_keep = i4 WITH protect, constant(12)
 ...
 DECLARE batchsize = i4 WITH protect, noconstant(50000)
 ...
 FOR (tok_ndx = 1 TO size(request->tokens,5))
   IF ((request->tokens[tok_ndx].token_str="MONTHSTOKEEP"))
    SET months_to_keep = ceil(cnvtreal(request->tokens[tok_ndx].value))
   ENDIF
 ENDFOR
 IF (months_to_keep < minimum_months_to_keep)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"TOKENOUTOFRANGE",
   "You must keep at least %1 months worth of data.  You entered %2 months or did not enter any value.",
   "ii",minimum_months_to_keep,
   months_to_keep)
  GO TO exit_script
 ENDIF
 SET reply->table_name = "REPORT_TASK"
 SET reply->rows_between_commit = 50
 SET purge_dt_tm = cnvtagedatetime(0,months_to_keep,0,0)
 SET stat = uar_get_meaning_by_codeset(1305,"CANCEL",1,cancel_status_cd)
 IF (cancel_status_cd=0.0)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"CANCEL",
   "Unable to find code value in code set 1305 with CDF meaning 'CANCEL'.")
  GO TO exit_script
 ENDIF
 ...
 IF (verified_status_cd=0.0)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"VERIFIED",
   "Unable to find code value in code set 1305 with CDF meaning 'VERIFIED'.")
  GO TO exit_script
 ENDIF
 ...
 IF (corrected_status_cd=0.0)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"CORRECTED",
   "Unable to find code value in code set 1305 with CDF meaning 'CORRECTED'.")
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(rt.report_id)
   FROM report_task rt
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    rt.rowid
    FROM report_task rt
    WHERE parser(sbr_getrowidnotexists("rt.report_id between curMinID and curMaxID","rt"))
     AND  EXISTS (
    (SELECT
     cr.report_id
     FROM case_report cr
     WHERE cr.report_id=rt.report_id
      AND cr.status_cd IN (cancel_status_cd, verified_status_cd, corrected_status_cd)
      AND cr.status_dt_tm <= cnvtdatetime(purge_dt_tm)))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,100)=1)
      stat = alterlist(reply->rows,(rows+ 99))
     ENDIF
     reply->rows[rows].row_id = rt.rowid
    WITH nocounter, maxqual(rt,value(rowsleft))
   ;end select
