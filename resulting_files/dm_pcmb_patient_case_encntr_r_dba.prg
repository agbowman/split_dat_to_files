CREATE PROGRAM dm_pcmb_patient_case_encntr_r:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "PATIENT_CASE_ENCNTR_R"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_PATIENT_CASE_ENCNTR_R"
 ...
 SELECT INTO "nl:"
  frm.*
  FROM patient_case_encntr_r frm
  WHERE (frm.encntr_id=request->xxx_combine[icombine].encntr_id)
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count].from_id = frm.patient_case_encntr_r_id, rreclist->from_rec[
   v_cust_count].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count].active_status_cd = frm
   .active_status_cd
  WITH forupdatewait(frm)
 ;end select
 IF (v_cust_count > 0)
  FOR (v_cust_loopcount = 1 TO v_cust_count)
    CALL del_from(rreclist->from_rec[v_cust_loopcount].from_id,rreclist->from_rec[v_cust_loopcount].
     active_ind,rreclist->from_rec[v_cust_loopcount].active_status_cd)
  ENDFOR
 ENDIF
 SUBROUTINE del_from(s_df_pk_id,s_df_prev_act_ind,s_df_prev_act_status)
   UPDATE  FROM patient_case_encntr_r frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.active_status_dt_tm =
     ...
     cnvtdatetime(sysdate)
    WHERE frm.patient_case_encntr_r_id=s_df_pk_id
    WITH nocounter
   ;end update
   IF (curqual=0)
