CREATE PROGRAM dm2_mrg_load_codeset_dup
 ...
 DECLARE inhouse_ind = i4
 FREE RECORD rs_exist
 RECORD rs_exist(
   1 qual[*]
     2 is_there = i2
     2 mrg_codeset = i4
     2 mrg_display_key_dup = i2
     2 mrg_display_dup = i2
     2 mrg_cdf_meaning_dup = i2
     2 mrg_active_ind_dup = i2
 )
 SET dim_filename = fillstring(132," ")
 SET dim_filename = "cer_install:dm2_mrg_codeset_dup.csv"
 SET readme_data->status = "F"
 SET readme_data->message = "Starting Readme..."
 IF (check_logfile("dm2_mrg_cs_dup",".log","dm2_mrg_cs_dup LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_mrg_load_codeset_dup..."
 CALL disp_msg("",dm_err->logfile,0)
 IF (findfile(dim_filename)=0)
  SET readme_data->status = "F"
  SET readme_data->message = "Cannot find dm2_mrg_codeset_dup.csv in CER_INSTALL."
  SET dm_err->emsg = "Cannot find dm2_mrg_codeset_dup.csv in CER_INSTALL."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF (merge_check_cnt > 0)
  ...
  IF (inhouse_ind=1)
   ...
   SET dm_err->eproc = "Not executed since we are in inhouse domain."
   CALL disp_msg("",dm_err->logfile,0)
   GO TO exit_script
  ENDIF
  SELECT INTO "nl:"
   ...
   DETAIL
    rs_exist->qual[d.seq].is_there = 1, rs_exist->qual[d.seq].mrg_codeset = cnvtint(requestin->
     list_0[d.seq].code_set), rs_exist->qual[d.seq].mrg_display_key_dup = cnvtint(requestin->list_0[d
     .seq].display_key_dup_ind),
    rs_exist->qual[d.seq].mrg_display_dup = cnvtint(requestin->list_0[d.seq].display_dup_ind),
    rs_exist->qual[d.seq].mrg_cdf_meaning_dup = cnvtint(requestin->list_0[d.seq].cdf_meaning_dup_ind),
    rs_exist->qual[d.seq].mrg_active_ind_dup = cnvtint(requestin->list_0[d.seq].active_ind_dup_ind)
   WITH nocounter
  ;end select
  SET dm_err->eproc = "Updating code_value_set..."
  CALL disp_msg("",dm_err->logfile,0)
  SET readme_data->status = "F"
  SET readme_data->message = "Updating code_value_set.."
  UPDATE  FROM code_value_set dc,
    (dummyt d  WITH seq = merge_check_cnt)
   SET dc.display_key_dup_ind = rs_exist->qual[d.seq].mrg_display_key_dup, dc.active_ind_dup_ind =
    rs_exist->qual[d.seq].mrg_active_ind_dup, dc.cdf_meaning_dup_ind = rs_exist->qual[d.seq].
    mrg_cdf_meaning_dup,
    dc.display_dup_ind = rs_exist->qual[d.seq].mrg_display_dup
   PLAN (d
    WHERE d.seq > 0)
    JOIN (dc
    WHERE dc.code_set=cnvtint(rs_exist->qual[d.seq].mrg_codeset))
   WITH nocounter
  ;end update
 ...
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading code_value_set with info in dm2_mrg_codeset_dup.csv."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = "Error loading code_value_set with info in dm2_mrg_codeset_dup.csv."
  GO TO exit_script
 ELSE
  SET dm_err->eproc = "Finished update of code_value_set"
  SET readme_data->status = "S"
 ENDIF
 SET dim_cnt = 0
 SELECT INTO "nl:"
  FROM code_value_set dc,
   (dummyt d  WITH seq = size(rs_exist->qual,5))
  PLAN (d)
   JOIN (dc
   WHERE (dc.code_set=rs_exist->qual[d.seq].mrg_codeset)
    AND (dc.display_key_dup_ind=rs_exist->qual[d.seq].mrg_display_key_dup)
    AND (dc.active_ind_dup_ind=rs_exist->qual[d.seq].mrg_active_ind_dup)
    AND (dc.cdf_meaning_dup_ind=rs_exist->qual[d.seq].mrg_cdf_meaning_dup)
    AND (dc.display_dup_ind=rs_exist->qual[d.seq].mrg_display_dup))
  DETAIL
   dim_cnt = (dim_cnt+ 1)
  WITH nocounter
 ;end select
 ...
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = "Check failed. Rows in request != rows in local code_value_set."
 ENDIF
#exit_script
 IF ((readme_data->status="S"))
  IF (inhouse_ind=1)
   SET readme_data->message = "Not executed since we are in inhouse domain."
  ELSE
   SET readme_data->message = "CODE_VALUE_SET has been updated successfully."
  ENDIF
 ELSE
  ROLLBACK
 ENDIF
