CREATE PROGRAM ch_get_physicians:dba
 RECORD reply(
   1 qual[10]
     2 person_id = f8
 ...
 SELECT INTO "nl:"
  FROM prsnl p
  WHERE p.name_last_key=patstring(searchstring)
 ...
 SUBROUTINE logicaldomainlookup(null)
   ...
   IF (lgetldstatus != ld_success)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus[1].operationname = "ch_get_physicians"
