CREATE PROGRAM ajt_dietary_report
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Please enter a facility:",
  "Nursing Unit",
  "Only patients with TPN orders?" = 4185.000,
  "Order Statuses" = value(2543.000,2550.000)
  WITH outdev, facility, nurse_unit,
  tpn_only, order_status
 IF (( $FACILITY=0)
  AND ( $NURSE_UNIT=0))
  SELECT INTO  $OUTDEV
   FROM dummyt d
 ...
 SET outpatient_enc_type_cd = uar_get_code_by("DISPLAY",71,"Outpatient")
 ...
 SET pharm_act_type_cd = uar_get_code_by("MEANING",106,"PHARMACY")
 ...
 SELECT
  IF (( $NURSE_UNIT=0))
   PLAN (ed
    ...
    WHERE e.encntr_id=ed.encntr_id
     AND e.encntr_type_cd IN (inpatient_enc_type_cd, outpatient_enc_type_cd, daystay_enc_type_cd)
     AND e.active_ind=1
  ...
  ELSE
   PLAN (ed
    WHERE ed.active_ind=1
     AND (ed.loc_nurse_unit_cd= $NURSE_UNIT)
     AND ed.loc_room_cd != 0
     AND ed.loc_bed_cd != 0)
    JOIN (e
    WHERE e.encntr_id=ed.encntr_id
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   (SELECT
    max(oii.action_sequence)
    FROM order_ingredient oii
 ...
 SELECT INTO  $OUTDEV
  ...
  FROM (dummyt d  WITH seq = value(diet->cnt))
  PLAN (d
   WHERE ((uar_get_code_display( $TPN_ONLY)="Yes"
    AND (diet->qual[d.seq].has_tpn_order=1)) OR (uar_get_code_display( $TPN_ONLY)="No")) )
  ...
  HEAD PAGE
   ...
   "Run Time: ", curtime, row + 1,
   ycol = (ycol+ 10),
   CALL print(calcpos(xcol,ycol)), "Facility: ",
   loc_facility, row + 1, ycol = (ycol+ 10),
   CALL print(calcpos(xcol,ycol)), "Nurse Unit: ", loc_nurse_unit,
   row + 1, ycol = (ycol+ 10)
  HEAD loc_nurse_unit
   row + 0
  HEAD loc_bed
   ...
   IF ((diet->qual[d.seq].order_cnt > 0))
    FOR (i = 1 TO diet->qual[d.seq].order_cnt)
      ...
      tempstring = substring(1,500,trim(concat(trim(diet->qual[d.seq].orders[i].order_mnemonic)," ",
         trim(diet->qual[d.seq].orders[i].clinical_display_line)))), xcol = 75, wrapcol = 95,
