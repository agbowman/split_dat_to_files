CREATE PROGRAM cp_get_pathway_activity:dba
 PROMPT
 ...
 SUBROUTINE main(null)
   ...
   DECLARE person_id = f8 WITH constant(cnvtreal( $PID)), protect
   DECLARE encntr_id = f8 WITH constant(cnvtreal( $EID)), protect
   ...
   DECLARE vcencntrparser = vc WITH noconstant("1=1"), protect
   DECLARE cnt = i4 WITH noconstant(0), protect
   DECLARE x = i4 WITH noconstant(0), protect
   CALL log_message("In main()",log_level_debug)
   IF (encntr_id > 0.0)
    SET vcencntrparser = build2("cpa.encntr_id = ",cnvtstring(encntr_id,17,3))
   ENDIF
   IF (person_id > 0.0)
    SELECT
     pathway_name = substring(1,200,cnvtupper(cp.pathway_name))
     FROM cp_pathway_activity cpa,
      cp_pathway cp,
      prsnl p
     PLAN (cpa
      WHERE cpa.person_id=person_id
       AND parser(vcencntrparser)
       AND cpa.pathway_activity_status_cd != pathway_status_proposed_cd)
      JOIN (cp
      WHERE cp.cp_pathway_id=cpa.cp_pathway_id
       AND ((cp.pathway_type_cd+ 0) IN (pathway_type_oncology, pathway_type_cpm,
      pathway_type_legacydebug)))
      JOIN (p
      WHERE (cpa.prsnl_id= Outerjoin(p.person_id)) )
     ORDER BY pathway_name, cpa.cp_pathway_id, cpa.pathway_instance_id,
      cpa.beg_effective_dt_tm DESC
     DETAIL
      cnt += 1
      IF ((cnt > pathway_activity_reply->activity_cnt))
       pathway_activity_reply->activity_cnt += 10, stat = alterlist(pathway_activity_reply->
        activity_qual,pathway_activity_reply->activity_cnt)
      ENDIF
      pathway_activity_reply->activity_qual[cnt].pathway_name = cp.pathway_name,
      pathway_activity_reply->activity_qual[cnt].auto_added_ind = cpa.auto_added_ind,
      pathway_activity_reply->activity_qual[cnt].beg_effective_dt_tm = getutcdttm(cpa
       .beg_effective_dt_tm),
      pathway_activity_reply->activity_qual[cnt].comment_txt = cpa.comment_txt,
      pathway_activity_reply->activity_qual[cnt].cp_pathway_activity_id = cpa.cp_pathway_activity_id,
      pathway_activity_reply->activity_qual[cnt].cp_pathway_id = cpa.cp_pathway_id,
      pathway_activity_reply->activity_qual[cnt].pathway_instance_id = cpa.pathway_instance_id,
      pathway_activity_reply->activity_qual[cnt].encntr_id = cpa.encntr_id, pathway_activity_reply->
      activity_qual[cnt].end_effective_dt_tm = getutcdttm(cpa.end_effective_dt_tm),
      pathway_activity_reply->activity_qual[cnt].pathway_activity_status_cd = cpa
      .pathway_activity_status_cd, pathway_activity_reply->activity_qual[cnt].
      pathway_activity_status_mean = uar_get_code_meaning(cpa.pathway_activity_status_cd),
      pathway_activity_reply->activity_qual[cnt].person_id = cpa.person_id,
      pathway_activity_reply->activity_qual[cnt].rowid = cpa.rowid, pathway_activity_reply->
      activity_qual[cnt].updt_id = cpa.prsnl_id, pathway_activity_reply->activity_qual[cnt].
      off_pathway_ind = cpa.off_pathway_ind,
      pathway_activity_reply->activity_qual[cnt].clinical_trial_ind = cpa.clinical_trial_ind,
      pathway_activity_reply->activity_qual[cnt].updt_name = p.name_full_formatted
     FOOT REPORT
      pathway_activity_reply->activity_cnt = cnt, stat = alterlist(pathway_activity_reply->
       activity_qual,cnt)
     WITH nocounter
    ;end select
    DECLARE idx = i4 WITH noconstant(0), protect
    FOR (idx = 1 TO pathway_activity_reply->activity_cnt)
      IF ((pathway_activity_reply->activity_qual[idx].pathway_activity_status_mean="ACTIVE"))
       SET pathway_activity_reply->activity_qual[idx].diagnosis_nomen_id = getisadiagnomenid(cnvtreal
        (pathway_activity_reply->activity_qual[idx].cp_pathway_id),person_id,pathway_activity_reply->
        activity_qual[idx].encntr_id)
      ENDIF
    ENDFOR
   ENDIF
