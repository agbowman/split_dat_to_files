CREATE PROGRAM bhs_ops_fax_prodtest:dba
 PROMPT
 ...
 SUBROUTINE (coversheet_sectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __pcp_name = vc WITH noconstant(build2(m_rec->fax[ml_cnt].recip[ml_recip_cnt].s_phys_name,
     char(0))), protect
   DECLARE __patient_name = vc WITH noconstant(build2(m_rec->fax[ml_cnt].s_patient,char(0))), protect
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(concat("Confidentiality Note:",_crlf,
       ...
       "e received this communication in error, please notify us immediately by telephone at 413-794-7002. Thank you."
       ),char(0)))
 ...
 SUBROUTINE (head_sectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __pcp_name = vc WITH noconstant(build2(m_rec->fax[ml_cnt].recip[ml_recip_cnt].s_phys_name,
     char(0))), protect
   DECLARE __patient_name = vc WITH noconstant(build2(m_rec->fax[ml_cnt].s_patient,char(0))), protect
   DECLARE __mrn = vc WITH noconstant(build2(m_rec->fax[ml_cnt].s_mrn_nbr,char(0))), protect
   DECLARE __fin = vc WITH noconstant(build2(m_rec->fax[ml_cnt].s_acct_nbr,char(0))), protect
   DECLARE __admit_date = vc WITH noconstant(build2(m_rec->fax[ml_cnt].s_admit_dt,char(0))), protect
   DECLARE __disch_date = vc WITH noconstant(build2(m_rec->fax[ml_cnt].s_disch_dt,char(0))), protect
   DECLARE __location = vc WITH noconstant(build2(concat(m_rec->fax[ml_cnt].s_nurse_unit," ",m_rec->
      fax[ml_cnt].s_facility),char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 64
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_OPS_FAX_PRODTEST"
 ...
 DECLARE ms_env = vc WITH protect, noconstant(" ")
 ...
 ELSEIF (mc_csv_ind="Y"
  ...
  ENDIF
 ENDIF
 SET ms_csv_filename = concat("bhs_ops_fax_",cnvtlower(ms_mode),trim(format(sysdate,"mmddyyhhmmss;;d"
    )),".csv")
 SELECT INTO "nl:"
  FROM v$database v
  DETAIL
   ms_env = trim(cnvtupper(v.name))
   IF (trim(cnvtupper(v.name)) != "PROD")
    mn_test = 1, ms_ftp_path = "ciscore\NOTES_FAXING\NONPROD"
   ELSE
    ms_ftp_path = "ciscore\NOTES_FAXING\PROD"
   ENDIF
  WITH nocounter
 ;end select
 IF (ms_mode="PCP")
 ...
 ELSEIF (ms_mode="APC")
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   PLAN (ed
    ...
    WHERE s.encounter_id=e.encntr_id
     AND s.story_completion_status_cd=mf_story_sign_cd
     AND s.active_status_dt_tm BETWEEN cnvtdatetime(ms_apc_beg_dt_tm) AND cnvtdatetime(
     ms_apc_end_dt_tm))
    JOIN (ssp
    WHERE ssp.scd_story_id=s.scd_story_id
     AND ssp.scr_pattern_id IN (
 ...
 IF (size(m_rec->fax,5)=0)
  ...
  IF (ms_recipients="FTP")
   SELECT INTO value(ms_csv_filename)
    ...
    WITH nocounter
   ;end select
   IF (ms_env IN ("READ", "PROD"))
    SET ms_dclcom_str = concat("$cust_script/bhs_ftp_file.ksh ",ms_csv_filename,
     " transfer.baystatehealth.org 'bhs\cisftp' C!sftp01 '",'"',ms_ftp_path,
     '"',"'")
   ELSE
    SET ms_dclcom_str = concat("$bhscust/bhs_ftp_file.ksh ",ms_csv_filename,
     " transfer.baystatehealth.org 'bhs\cisftp' C!sftp01 '",'"',ms_ftp_path,
     '"',"'")
   ENDIF
   SET len = textlen(trim(ms_dclcom_str))
 ...
 FOR (ml_cnt = 1 TO size(m_rec->fax,5))
   ...
   SELECT INTO "nl:"
    ...
    HEAD REPORT
     ...
     pl_mid = 0, pl_obrac = 0, pl_obrac_cnt = 0,
     pl_loop = 0
    HEAD cb.event_id
     pl_num = 0
     IF (textlen(trim(cb.blob_contents)) > 0)
      ...
      IF (ms_mode="APC"
       ...
       ENDIF
       WHILE (pl_beg > 0)
         ml_loop_cnt += 1, pl_mid = findstring(",",ms_blob,pl_beg), pl_end = findstring(".",ms_blob,
          pl_beg),
         ...
         IF (locateval(pl_num,1,size(m_rec->fax[ml_idx].recip,5),ms_tmp,m_rec->fax[ml_idx].recip[
          ...
          pl_cnt += 1, stat = alterlist(m_rec->fax[ml_idx].recip,pl_cnt), ml_log_grp_cnt += 1,
      ...
      ELSEIF (ms_mode="APC"
       ...
       ENDIF
       WHILE (pl_beg > 0)
         ml_loop_cnt += 1, pl_end = findstring(concat(char(92),"par"),ms_cc_rtf,pl_beg)
         IF (pl_end > 0)
      ...
      IF (size(m_rec->fax[ml_idx].recip,5) > 0)
       ...
       pl_obrac = 0, pl_beg = findstring("{\deleted\",ms_blob), x = 0
       WHILE (pl_beg > 0)
         pl_obrac_cnt = 0, pl_obrac = findstring("{",ms_blob,(pl_beg+ 1)), pl_end = findstring("}",
          ms_blob,pl_beg)
         WHILE (pl_obrac < pl_end
          AND pl_obrac > 0)
           pl_obrac = findstring("{",ms_blob,(pl_obrac+ 1)), pl_obrac_cnt += 1
           IF (pl_obrac_cnt=1000)
            pl_obrac = 0
           ENDIF
         ENDWHILE
         FOR (pl_loop = 1 TO pl_obrac_cnt)
           pl_end = findstring("}",ms_blob,(pl_end+ 1))
         ENDFOR
         ms_tmp = substring(pl_beg,((pl_end - pl_beg)+ 1),ms_blob),
         CALL echo("******* delete *******"),
         CALL echo(ms_tmp),
         CALL echo("******* delete *******"), ms_blob = replace(ms_blob,ms_tmp,"",0), pl_beg =
         findstring("{\deleted\",ms_blob,pl_beg),
         x += 1
         IF (x=1000)
          pl_beg = 0
         ENDIF
