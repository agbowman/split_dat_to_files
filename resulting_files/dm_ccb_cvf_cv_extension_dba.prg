CREATE PROGRAM dm_ccb_cvf_cv_extension:dba
 ...
 ENDIF
 DECLARE dcccve_filterstring = vc WITH protect, noconstant(" ")
 DECLARE code_value_count = i4 WITH protect, noconstant(0)
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (cve
   WHERE (cve.code_set=request->code_set)
    AND parser(filterstring_builder(dcccve_filterstring,"cve",request)))
   JOIN (cv
  ...
  HEAD REPORT
   code_value_count = 0
  DETAIL
   code_value_count = (code_value_count+ 1)
   IF (mod(code_value_count,10)=1)
    stat = alterlist(reply->cd_value_list,(code_value_count+ 9))
   ENDIF
   reply->cd_value_list[code_value_count].code_value = cv.code_value, reply->cd_value_list[
   code_value_count].display = cv.display, reply->cd_value_list[code_value_count].cdf_meaning = cv
   .cdf_meaning
  FOOT REPORT
   stat = alterlist(reply->cd_value_list,code_value_count)
  WITH nocounter
 ;end select
 IF (error(errmsg,0) != 0)
  SET failed = "T"
