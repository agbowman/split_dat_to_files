CREATE PROGRAM dm_transaction_admit_noti_rows
 ...
 SET reply->table_name = "TRANSACTION_ADMIT_NOTIFY"
 ...
 FOR (t_idx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 180)
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "You must keep at least 180 days worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
  GO TO exit_script
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    minval = min(ta.transaction_admit_notify_id)
    FROM transaction_admit_notify ta
    WHERE ta.transaction_admit_notify_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   maxval = max(ta.transaction_admit_notify_id)
   FROM transaction_admit_notify ta
   DETAIL
  ...
  WHILE (v_cur_min_id <= v_max_id
   AND v_rows_left > 0)
    SELECT INTO "nl:"
     ta.rowid
     FROM transaction_admit_notify ta
     WHERE parser(sbr_getrowidnotexists("ta.sent_dt_tm < cnvtdatetime(v_days_back)","ta"))
      AND ta.transaction_admit_notify_id BETWEEN v_cur_min_id AND v_cur_max_id
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,100)=1)
       stat = alterlist(reply->rows,(rows+ 99))
      ENDIF
      reply->rows[rows].row_id = ta.rowid
     WITH nocounter, maxqual(ta,value(v_rows_left))
    ;end select
