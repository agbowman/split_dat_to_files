...
#1000_initialize
 ...
 ENDIF
 CALL echo(concat("*** CS_ESO_MSH  START CTRL_ID1 = ",concat("Q:",trim(cnvtstring(get_esoinfo_double(
       "queue_id")))," T:",trim(cnvtstring(get_esoinfo_double("trigger_id"))))))
 SET stat = 0
...
#2000_main
 ...
 SET context->control_group[1].msh[1].sending_application.name_id = "CS_BATCH_CHARGE"
 ...
 SET context->control_group[1].msh[1].message_time_stamp = hl7_format_datetime(cnvtdatetime(sysdate),
  hl7_dt_tm)
 SET context->control_group[1].msh[1].message_type.messg_type = "DFT"
 IF ((request->cqminfo.subtype != "DETAIL"))
  SET context->control_group[1].msh[1].message_type.messg_trigger = substring(1,3,request->cqminfo.
   subtype)
  SET length = 0
  SET length = findstring("~",request->cqminfo.contribrefnum)
  IF (length=0)
   SET length = size(request->cqminfo.contribrefnum)
  ELSE
   SET length -= 1
  ENDIF
  SET reply->interface_file_id = cnvtreal(substring(1,length,request->cqminfo.contribrefnum))
  IF ((reply->interface_file_id > 0))
   SELECT INTO "nl:"
    i_f.file_name, i_f.description
    FROM interface_file i_f
    WHERE (i_f.interface_file_id=reply->interface_file_id)
   ...
   ;end select
  ENDIF
  SET stat = set_esoinfo_double("Interface file id",reply->interface_file_id)
 ...
 ELSE
  SET context->control_group[1].msh[1].message_type.messg_trigger = "P03"
 ENDIF
...
#9999_end
 ...
 CALL echo(concat("*** CS_ESO_MSH  END CTRL_ID1 = ",concat("Q:",trim(cnvtstring(get_esoinfo_double(
       "queue_id")))," T:",trim(cnvtstring(get_esoinfo_double("trigger_id"))))))
END GO
