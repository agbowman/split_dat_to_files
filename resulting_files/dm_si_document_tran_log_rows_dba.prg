CREATE PROGRAM dm_si_document_tran_log_rows:dba
 CALL echo("<===== DM_SI_DOCUMENT_TRAN_LOG_ROWS BEGIN =====>")
 ...
 SET reply->table_name = "SI_DOCUMENT_TRANSACTION_LOG"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(sdtl.si_document_transaction_log_id)
   FROM si_document_transaction_log sdtl
   WHERE sdtl.si_document_transaction_log_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(sdtl.si_document_transaction_log_id)
  FROM si_document_transaction_log sdtl
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    sdtl.rowid
    FROM si_document_transaction_log sdtl
    WHERE sdtl.si_document_transaction_log_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists(
      "sdtl.transaction_dt_tm < cnvtdatetime((curdate - days_to_keep), 0)","sdtl"))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,500)=1)
      stat = alterlist(reply->rows,(rows+ 499))
     ENDIF
     reply->rows[rows].row_id = sdtl.rowid
    WITH nocounter, maxqual(sdtl,value(rowsleft))
   ;end select
...
#exit_script
 CALL echo("<===== DM_SI_DOCUMENT_TRAN_LOG_ROWS END =====>")
END GO
