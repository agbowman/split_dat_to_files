CREATE PROGRAM bbd_chg_confidential_note:dba
 RECORD reply(
   1 confidential_note_id = f8
 ...
 DECLARE confidential_note_id = f8 WITH protect, noconstant(0.0)
 ...
 IF ((request->new_indicator=1))
  ...
  ;end select
  SET confidential_note_id = new_pathnet_seq
  SELECT INTO "nl:"
  ...
  INSERT  FROM long_text lt
   SET lt.long_text_id = text_id, lt.parent_entity_name = "BBD_CONFIDENTIAL_NOTE", lt
    .parent_entity_id = confidential_note_id,
  ...
  IF (curqual=0)
   ...
   SET reply->status_data.subeventstatus[1].sourceobjectvalue = "bbd_chg_confidential_note"
   ...
   GO TO exit_script
  ENDIF
  INSERT  FROM bbd_confidential_note b
   SET b.confidential_id = confidential_note_id, b.person_id = request->person_id, b.long_text_id =
    text_id,
  ...
  IF (curqual=0)
   ...
   SET reply->status_data.subeventstatus[1].operationname = "insert"
   SET reply->status_data.subeventstatus[1].operationstatus = "F"
   SET reply->status_data.subeventstatus[1].targetobjectname = "bbd_confidential_note"
   SET reply->status_data.subeventstatus[1].targetobjectvalue = "confidential note"
   SET reply->status_data.subeventstatus[1].sourceobjectqual = 1
   SET reply->status_data.subeventstatus[1].sub_event_dt_tm = cnvtdatetime(curdate,curtime)
   GO TO exit_script
  ELSE
   SET reply->confidential_note_id = confidential_note_id
   SET reply->confidential_updt_cnt = 0
 ...
 ELSE
  ...
  SELECT INTO "nl:"
   ...
   WHERE (b.confidential_id=request->confidential_id)
    AND (b.updt_cnt=request->confidential_updt_cnt)
  ...
  IF (curqual=0)
   ...
   SET reply->status_data.subeventstatus[1].operationstatus = "F"
   SET reply->status_data.subeventstatus[1].targetobjectname = "bbd_confidential_note"
   SET reply->status_data.subeventstatus[1].targetobjectvalue = "confidential note lock"
   SET reply->status_data.subeventstatus[1].sourceobjectqual = 1
   SET reply->status_data.subeventstatus[1].sub_event_dt_tm = cnvtdatetime(curdate,curtime)
   GO TO exit_script
  ENDIF
  UPDATE  FROM bbd_confidential_note b
  ...
  IF (curqual=0)
   ...
   SET reply->status_data.subeventstatus[1].operationstatus = "F"
   SET reply->status_data.subeventstatus[1].targetobjectname = "confidential note"
   SET reply->status_data.subeventstatus[1].targetobjectvalue = "confidential note inactivate"
   SET reply->status_data.subeventstatus[1].sourceobjectqual = 1
   SET reply->status_data.subeventstatus[1].sub_event_dt_tm = cnvtdatetime(curdate,curtime)
   GO TO exit_script
  ENDIF
