CREATE PROGRAM bhs_phys_cmplt_info_csv:dba
 PROMPT
 ...
 SELECT INTO value(output_dest)
  ...
  HEAD REPORT
   ...
   FOR (y = 1 TO size(md_alias->qual[d.seq],5))
     IF ((md_alias->qual[y].dea_nbr != null)
      AND (md_alias->qual[y].npi_nbr != null)
      AND (((md_alias->qual[y].b_addr_1 != null)) OR ((md_alias->qual[y].b_addr_2 != null)))
      AND (md_alias->qual[y].b_phone != null)
      AND (md_alias->qual[y].b_fax != null)
      AND (((md_alias->qual[y].ez_addr_1 != null)) OR ((md_alias->qual[y].ez_addr_2 != null)))
      AND (md_alias->qual[y].ez_phone != null))
 ...
 IF (email_ind=1)
  ...
  SET filename_out = concat(format(curdate,"YYYY-MM-DD;;D"),"_PHYS_cmplt_info.csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog,"-V1.x - PHYS Complete info ",curnode)
