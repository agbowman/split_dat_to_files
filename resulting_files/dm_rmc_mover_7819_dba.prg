CREATE PROGRAM dm_rmc_mover_7819:dba
 ...
 DECLARE cust_from_item_id = vc
 DECLARE cust_par_tab_name = vc
 DECLARE cust_from_group_id = f8
 DECLARE cust_to_group_id = vc
 DECLARE cust_to_item_id = f8
 DECLARE cust_loop = i4
 ...
 SET cust_type_flag = cnvtint(get_value(cust_tab_name,"VALUE_TYPE_FLAG","FROM"))
 IF (cust_type_flag=3)
  SET cust_pe_name = "SA_REF_ITEM"
  SET cust_pe_col = "SA_REF_ITEM_ID"
 ELSEIF (cust_type_flag=4)
  SET cust_pe_name = "PRSNL"
  SET cust_pe_col = "PERSON_ID"
 ENDIF
 IF (cust_pe_name != "")
  SET cust_default_value = get_value(cust_tab_name,"DEFAULT_VALUE","FROM")
  IF (isnumeric(cust_default_value) > 0)
   SET cust_value_pos = get_col_pos(cust_tab_name,"DEFAULT_VALUE")
   SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_value_pos].translated = 0
   SET cust_new_value = select_merge_translate(cust_default_value,cust_pe_name)
   IF (isnumeric(cust_new_value))
    CALL put_value(cust_tab_name,"DEFAULT_VALUE",cust_new_value)
   ELSE
    SET cust_rpt_missing = report_missing(cust_pe_name,cust_pe_col,cnvtreal(cust_default_value))
    CALL evaluate_rpt_missing(cust_rpt_missing)
   ENDIF
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"SA_REF_ACTION_ITEM_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   SET cust_from_item_id = get_value(cust_tab_name,"SA_REF_ACTION_ITEM_ID","FROM")
   SET cust_nomv_chk = select_merge_translate(cnvtstring(cust_from_item_id),cust_tab_name)
   IF (cust_nomv_chk="NOMV21")
    ...
    GO TO exit_7819
   ENDIF
   SET cust_par_tab_name = dm2_get_rdds_tname("SA_REF_GROUP_ACTION_ITEM_R")
   SELECT INTO "NL:"
    FROM (parser(cust_par_tab_name) s)
    WHERE s.sa_ref_action_item_id=cnvtreal(cust_from_item_id)
    DETAIL
     cust_from_group_id = s.sa_ref_group_id
    WITH nocounter
   ;end select
   SET cust_to_group_id = select_merge_translate(cnvtstring(cust_from_group_id),"SA_REF_GROUP")
   IF (isnumeric(cust_to_group_id))
    SET cust_cnt = 0
    SELECT INTO "NL:"
     FROM sa_ref_group_action_item_r sa,
      sa_ref_action_item si
     PLAN (si
      WHERE (si.action_item_name_key=rs_7819->to_values.action_item_name_key)
       AND (si.child_selection_req_ind=rs_7819->to_values.child_selection_req_ind)
       AND (si.value_required_ind=rs_7819->to_values.value_required_ind)
       AND (si.value_type_flag=rs_7819->to_values.value_type_flag))
      JOIN (sa
      WHERE sa.sa_ref_action_item_id=si.sa_ref_action_item_id
       AND sa.sa_ref_group_id=cnvtreal(cust_to_group_id))
     DETAIL
      cust_cnt = (cust_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,cust_cnt), ui_query_eval_rec
      ->qual[cust_cnt].root_entity_attr = sa.sa_ref_action_item_id
     WITH nocounter
    ;end select
    SET cust_to_item_id = evaluate_exec_ui_query(cust_cnt,temp_tbl_cnt,perm_col_cnt)
    IF (cust_to_item_id < 0)
     IF ((cust_to_item_id=- (3)))
      SET cust_to_item_id = get_seq(cust_tab_name,"SA_REF_ACTION_ITEM_ID")
      IF (cust_to_item_id > 0)
       CALL put_value(cust_tab_name,"SA_REF_ACTION_ITEM_ID",cnvtstring(cust_to_item_id))
       SET current_merges = (current_merges+ 1)
       SET child_merge_audit->num[current_merges].action = "NEWSEQ"
       SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,
        "  SA_REF_ACTION_ITEM_ID")
      ELSE
       GO TO exit_7819
      ENDIF
     ELSE
    ...
    SET cust_from_cd = get_value(cust_tab_name,"SA_REF_ACTION_ITEM_ID","FROM")
    IF ((dm2_ref_data_reply->error_ind=1))
     GO TO exit_7819
    ELSE
     SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
    ENDIF
    SET cust_new_cd = get_value(cust_tab_name,"SA_REF_ACTION_ITEM_ID","TO")
    IF ((dm2_ref_data_reply->error_ind=1))
     GO TO exit_7819
    ELSE
   ...
   ELSE
    SET cust_rpt_missing = report_missing("SA_REF_GROUP","SA_REF_GROUP_ID",cust_from_group_id)
    CALL evaluate_rpt_missing(cust_rpt_missing)
   ENDIF
  ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"DEFAULT_VALUE")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the DEFAULT_VALUE"
