CREATE PROGRAM aps_get_db_dc_reports:dba
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (isresourceviewable(service_resource_cd)=true)
    x += 1, stat = alterlist(reply->qual,x), reply->qual[x].study_id = ads.study_id,
    reply->qual[x].description = ads.description, reply->qual[x].across_case_ind = ads
    .across_case_ind, reply->qual[x].active_ind = ads.active_ind,
    reply->qual[x].slide_counts_prompt_ind = ads.slide_counts_prompt_ind, reply->qual[x].
    include_cytotechs_ind = ads.include_cytotechs_ind, reply->qual[x].default_to_group_ind = ads
    .default_to_group_ind,
    reply->qual[x].updt_cnt = ads.updt_cnt
   ENDIF
  WITH nocounter
 ;end select
 IF (curqual=0)
