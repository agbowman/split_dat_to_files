CREATE PROGRAM acm_write_encntr_domain:dba
 ...
 DECLARE ubound = i4 WITH protect, noconstant(reply->encntr_domain_qual_cnt)
 SET stat = alterlist(reply->encntr_domain_qual,reply->encntr_domain_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->encntr_domain_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->encntr_domain_qual[index].encntr_domain_id = acm_request->encntr_domain_qual[index].
     encntr_domain_id
     SET reply->encntr_domain_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_encntr_domain
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->act_cnt > 0))
  EXECUTE acm_act_encntr_domain
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_encntr_domain
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_encntr_domain
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
