CREATE PROGRAM afc_get_fav_folder_cat:dba
 DECLARE afc_get_fav_folder_cat_ver = vc
 SET afc_get_fav_folder_cat_ver = "002"
 RECORD reply(
 ...
 ENDIF
 SELECT INTO "nl:"
  c.fav_folder_cat_id, c.priority_seq
  FROM fav_folder_cat c,
   fav_folder_list l,
   dummyt d1
  PLAN (c
   WHERE c.logical_domain_id=curlogicaldomainid
    AND c.active_ind=1)
   JOIN (d1)
   JOIN (l
   WHERE l.child_fav_folder_cat_id=c.fav_folder_cat_id
    AND l.list_type=1
    AND l.child_fav_folder_cat_id > 0
    AND l.active_ind=1)
  ORDER BY c.fav_folder_cat_id
  DETAIL
   count1 += 1, stat = alterlist(reply->fav_folder_cat,count1), reply->fav_folder_cat[count1].
   ...
   CALL echo(build("Parent level folders: ",c.description))
 ...
 IF (curqual=0)
  ...
  SET reply->status_data.subeventstatus[1].targetobjectvalue = "FAV_FOLDER_CAT"
