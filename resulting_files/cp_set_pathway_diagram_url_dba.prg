CREATE PROGRAM cp_set_pathway_diagram_url:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "CP Pathway ID" = 0.0,
  "URL" = ""
  WITH outdev, dinputpathwayid, sinputurl
 ...
 DECLARE updatepathwayurl(surl=vc) = null WITH protect
 ...
 CALL updatepathwayurl( $DINPUTPATHWAYID, $SINPUTURL)
 SET mpagereply->status_data.status = "S"
 SUBROUTINE updatepathwayurl(dpathwayid,surl)
   CALL log_message("Begin UpdatePathwayUrl()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   UPDATE  FROM cp_pathway cp
    SET cp.pathway_diagram_url = surl, cp.updt_applctx = reqinfo->updt_applctx, cp.updt_cnt = (cp
     .updt_cnt+ 1),
    ...
    WITH nocounter
   ;end update
   CALL error_and_zero_check_rec(curqual,"CP_PATHWAY","UpdatePathwayUrl",1,1,
    mpagereply)
   CALL log_message(build("Exit UpdatePathwayUrl(), Elapsed time in seconds:",((curtime3 -
