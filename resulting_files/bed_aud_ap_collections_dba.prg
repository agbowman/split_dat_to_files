CREATE PROGRAM bed_aud_ap_collections:dba
 ...
 DECLARE ap = f8 WITH public, noconstant(uar_get_code_by("MEANING",106,"AP"))
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   temp->qual[cnt].spec_handling = cv7.display, temp->qual[cnt].activity_type = cv8.display, temp->
   qual[cnt].activity_subtype = cv9.display
  WITH nocounter
 ;end select
 ...
 IF (cnt > 0)
  ...
  IF ((request->skip_volume_check_ind=0))
   ...
   IF ((reply->high_volume_flag IN (1, 2)))
    SET reply->output_filename = build("ap_collections_audit.csv")
