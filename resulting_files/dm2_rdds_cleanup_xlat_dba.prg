CREATE PROGRAM dm2_rdds_cleanup_xlat:dba
 ...
 SUBROUTINE dm2_rdds_init_display(null)
   ...
   CALL clear(1,1)
 END ;Subroutine
 IF (validate(dtc_table_names_rec->qual[1].tname,"NONE")="NONE")
  FREE RECORD dtc_tables_names_rec
  RECORD dtc_table_names_rec(
    1 qual[*]
      2 tname = vc
  )
 ENDIF
 DECLARE dtc_source_env_id = f8
 DECLARE dtc_target_env_id = f8
 DECLARE dtc_source_dblink = vc
 DECLARE dtc_table_name = vc
 DECLARE dtc_done_flg = i2
 DECLARE dtc_cur_date = f8
 DECLARE dtc_info_domain = vc
 DECLARE dtc_standalone_flg = i2
 DECLARE dtc_base_xlat_flg = i2
 DECLARE dtc_count = i4
 DECLARE dtc_loop = i4
 DECLARE dtc_info_domain_strt_fin = vc
 DECLARE dtc_dm_merge_translate = vc
 DECLARE dtc_dm_info = vc
 DECLARE dtc_sample_tname = vc
 DECLARE dtc_tname_type = c1
 SET dtc_table_name = ""
 SET dtc_done_flg = 0
 SET dtc_info_domain = ""
 SET dtc_standalone_flg = 0
 SET dtc_base_xlat_flg = 0
 SET dtc_count = 0
 SET dtc_loop = 0
 SET dtc_source_env_id =  $1
 SET dtc_target_env_id =  $2
 SET dtc_source_dblink = cnvtupper( $3)
 SET dtc_info_domain_strt_fin = ""
 SET dtc_dm_merge_translate = ""
 SET dtc_dm_info = ""
 SET dtc_sample_tname = ""
 SET dtc_tname_type = "N"
 IF ((dm2_install_schema->curprog="NONE"))
  SET dtc_standalone_flg = 1
  IF (dtc_source_env_id=0
   AND dtc_target_env_id=0)
   SET dtc_base_xlat_flg = 1
  ENDIF
 ENDIF
 IF (check_logfile("dm2_rdds_clean_xlat",".log","DM2_CLEANUP_XLAT LOG")=0)
  GO TO exit_dtc
 ENDIF
 IF (trim(dtc_source_dblink)="")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "INVALID DATABASE LINK FOR THE GIVEN SOURCE/TARGET RELATIONSHIP"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dtc
 ENDIF
 SET dtc_info_domain = dm2_write_info_domain("MERGE",dtc_source_env_id,dtc_target_env_id,"SEQMATCH")
 SET dtc_info_domain_strt_fin = dm2_write_info_domain("MERGE",dtc_source_env_id,dtc_target_env_id," "
  )
 IF (dtc_source_dblink="DM2NOLINK")
  SET dtc_dm_info = "DM_INFO"
  SET dtc_dm_merge_translate = "DM_MERGE_TRANSLATE"
 ELSE
  SET dtc_dm_info = concat("DM_INFO",dtc_source_dblink)
  SET dtc_dm_merge_translate = concat("DM_MERGE_TRANSLATE",trim(dtc_source_dblink))
 ENDIF
 SET dm_err->eproc = "STARTING CLEANUP PROCESS"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (((dtc_standalone_flg=0) OR (dtc_standalone_flg=1
  AND dtc_base_xlat_flg=1)) )
  DELETE  FROM (parser(dtc_dm_info) d)
   WHERE d.info_name IN ("XLAT_CLEANUP_START", "XLAT_CLEANUP_FINISH")
    AND d.info_domain=dtc_info_domain_strt_fin
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_dtc
  ENDIF
  INSERT  FROM (parser(dtc_dm_info) d)
   SET d.info_domain = dtc_info_domain_strt_fin, d.info_name = "XLAT_CLEANUP_START", d.info_date =
    cnvtdatetime(curdate,curtime3)
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
  ...
  ELSE
   COMMIT
  ENDIF
  SET dm_err->eproc = "PERFORM INITIAL DELETE"
  CALL disp_msg(" ",dm_err->logfile,0)
  DELETE  FROM (parser(dtc_dm_merge_translate) d)
   WHERE d.env_source_id=dtc_source_env_id
    AND d.env_target_id=dtc_target_env_id
    AND d.table_name > " "
    AND d.from_value > 0
   WITH maxqual(d,100)
  ;end delete
  ...
  COMMIT
  IF (curqual < 100)
   SET dm_err->eproc = build("TRANSLATIONS FOUND=",curqual,". INITIAL DELETE SUFFICIENT.")
   CALL disp_msg(" ",dm_err->logfile,0)
  ELSE
   SET dm_err->eproc = "BUILDING RECORD STRUCTURE"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    d.*
    FROM (parser(dtc_dm_merge_translate) d)
    WHERE d.table_name > " "
    DETAIL
     dtc_sample_tname = cnvtupper(d.table_name)
    WITH maxqual(d,1)
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   SELECT INTO "nl:"
    ...
    WHERE d.table_name=dtc_sample_tname
    DETAIL
     IF (d.table_name=d.suffixed_table_name)
      dtc_tname_type = "S"
     ELSEIF (d.table_name=d.full_table_name)
      dtc_tname_type = "F"
     ELSE
      dtc_tname_type = "N"
     ENDIF
   ...
   SELECT
    IF (dtc_tname_type="S")
     WHERE d.table_name=d.suffixed_table_name
    ELSEIF (dtc_tname_type="F")
    ...
    HEAD REPORT
     stat = alterlist(dtc_table_names_rec->qual,10)
    DETAIL
     dtc_count = (dtc_count+ 1)
     IF (mod(dtc_count,10)=1
      AND dtc_count != 1)
      stat = alterlist(dtc_table_names_rec->qual,(dtc_count+ 9))
     ENDIF
     dtc_table_names_rec->qual[dtc_count].tname = cnvtupper(d.table_name)
    FOOT REPORT
     stat = alterlist(dtc_table_names_rec->qual,dtc_count)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_dtc
   ENDIF
   SET dm_err->eproc = "DELETING TRANSLATIONS"
   CALL disp_msg(" ",dm_err->logfile,0)
   FOR (dtc_loop = 1 TO dtc_count)
    SET dtc_done_flg = 0
    WHILE (dtc_done_flg=0)
      SET dm_err->eproc = concat("Checking for translations for table ",dtc_table_names_rec->qual[
       dtc_loop].tname)
      CALL disp_msg(" ",dm_err->logfile,0)
      DELETE  FROM (parser(dtc_dm_merge_translate) d)
       WHERE (d.table_name=dtc_table_names_rec->qual[dtc_loop].tname)
        AND d.env_target_id=dtc_target_env_id
        AND d.env_source_id=dtc_source_env_id
    ...
    ENDWHILE
   ENDFOR
   SET dm_err->eproc = "Insuring no translations left. May take several minutes."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dtc_done_flg = 0
   WHILE (dtc_done_flg=0)
     DELETE  FROM (parser(dtc_dm_merge_translate) d)
   ...
   ENDWHILE
  ENDIF
  IF (dtc_base_xlat_flg=0)
   SET dm_err->eproc = "DELETING SEQMATCH ROWS"
   CALL disp_msg(" ",dm_err->logfile,0)
   DELETE  FROM (parser(dtc_dm_info) d)
    WHERE d.info_domain=dtc_info_domain
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
  ENDIF
  SET dm_err->eproc = "RECORDING CLEANUP FINISH IN DM_INFO"
  INSERT  FROM (parser(dtc_dm_info) d)
   SET d.info_domain = dtc_info_domain_strt_fin, d.info_name = "XLAT_CLEANUP_FINISH", d.info_date =
    cnvtdatetime(curdate,curtime3)
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "This progam can only be run standalone to clean up base translations"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_dtc
 ENDIF
 SET dm_err->eproc = "Exiting Translation Cleanup Process: CLEANUP SUCCESSFUL"
#exit_dtc
 CALL final_disp_msg("dm2_rdds_clean_xlat")
 IF ((dm_err->err_ind=1))
  ROLLBACK
 ENDIF
END GO
