CREATE PROGRAM dm_him_abstracting_rows:dba
 ...
 SET reply->table_name = "ABSTRACTING"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(a.abstracting_id)
   FROM abstracting a
   WHERE a.abstracting_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    a.rowid
    FROM abstracting a
    WHERE parser(sbr_getrowidnotexists("a.abstracting_id between curMinID and curMaxID","a"))
     AND a.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
