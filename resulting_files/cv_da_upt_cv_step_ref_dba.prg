CREATE PROGRAM cv_da_upt_cv_step_ref:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_upt_cv_step_ref_vrsn = vc WITH private, constant("20060905")
 ...
 WHILE (i <= size(request->objarray,5))
   ...
   SELECT INTO "nl:"
    c.*
    FROM cv_step_ref c
    WHERE (c.task_assay_cd=request->objarray[i].task_assay_cd)
    DETAIL
     IF ((validate(request->objarray[i].step_level_flag,c.step_level_flag) != - (1))
      AND validate(request->objarray[i].step_level_flag,c.step_level_flag) != c.step_level_flag)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].doc_type_cd,c.doc_type_cd) != - (0.00001))
      AND validate(request->objarray[i].doc_type_cd,c.doc_type_cd) != c.doc_type_cd)
      alteredind = 1
     ENDIF
     IF (validate(request->objarray[i].doc_id_str,c.doc_id_str) != char(128)
      AND validate(request->objarray[i].doc_id_str,c.doc_id_str) != c.doc_id_str)
      alteredind = 1
     ENDIF
     ...
     .activity_subtype_cd)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].schedule_ind,c.schedule_ind) != - (1))
      AND validate(request->objarray[i].schedule_ind,c.schedule_ind) != c.schedule_ind)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].proc_status_cd,c.proc_status_cd) != - (0.00001))
      ...
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].study_reltn_flag,c.study_reltn_flag) != - (1))
      AND validate(request->objarray[i].study_reltn_flag,c.study_reltn_flag) != c.study_reltn_flag)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].step_type_cd,c.step_type_cd) != - (0.00001))
      AND validate(request->objarray[i].step_type_cd,c.step_type_cd) != c.step_type_cd)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].doc_template_id,c.doc_template_id) != - (0.00001))
      AND validate(request->objarray[i].doc_template_id,c.doc_template_id) != c.doc_template_id)
      alteredind = 1
     ENDIF
   ...
   ENDIF
   IF (alteredind > 0)
    UPDATE  FROM cv_step_ref c
     SET c.step_level_flag =
      IF ((validate(request->objarray[i].step_level_flag,- (1)) != - (1))) validate(request->
        objarray[i].step_level_flag,- (1))
      ELSE c.step_level_flag
      ENDIF
      , c.doc_type_cd =
      IF ((validate(request->objarray[i].doc_type_cd,- (0.00001)) != - (0.00001))) validate(request->
        objarray[i].doc_type_cd,- (0.00001))
      ELSE c.doc_type_cd
      ENDIF
      , c.doc_id_str =
      IF (validate(request->objarray[i].doc_id_str,char(128)) != char(128)) validate(request->
        objarray[i].doc_id_str,char(128))
      ELSE c.doc_id_str
      ENDIF
      ,
      ...
      ENDIF
      , c.schedule_ind =
      IF ((validate(request->objarray[i].schedule_ind,- (1)) != - (1))) validate(request->objarray[i]
        .schedule_ind,- (1))
      ELSE c.schedule_ind
      ENDIF
      , c.proc_status_cd =
      ...
      ,
      c.study_reltn_flag =
      IF ((validate(request->objarray[i].study_reltn_flag,- (1)) != - (1))) validate(request->
        objarray[i].study_reltn_flag,- (1))
      ELSE c.study_reltn_flag
      ENDIF
      , c.step_type_cd =
      IF ((validate(request->objarray[i].step_type_cd,- (0.00001)) != - (0.00001))) validate(request
        ->objarray[i].step_type_cd,- (0.00001))
      ELSE c.step_type_cd
      ENDIF
      , c.doc_template_id =
      IF ((validate(request->objarray[i].doc_template_id,- (0.00001)) != - (0.00001))) validate(
        request->objarray[i].doc_template_id,- (0.00001))
      ELSE c.doc_template_id
      ENDIF
      ,
