CREATE PROGRAM cps_ens_health_plan:dba
 RECORD reply(
 ...
 SET cps_ens_health_plan = 0
 SET istatus = 0
 SET serrmsg = fillstring(132," ")
 SET ierrcode = error(serrmsg,1)
 SET reply->status_data.status = "F"
 SET table_name = "health_plan"
 FOR (inx0 = 1 TO request->health_plan_qual)
   CASE (request->health_plan[inx0].action_type)
    OF "ADD":
     SET action_begin = inx0
     SET action_end = inx0
     EXECUTE cps_add_health_plan
     IF (failed != false)
      GO TO check_error
     ENDIF
    OF "UPT":
     SET action_begin = inx0
     SET action_end = inx0
     IF ((request->health_plan[inx0].health_plan_id=0))
     ...
     SELECT INTO "nl:"
      h.*
      FROM health_plan h
      WHERE (h.health_plan_id=request->health_plan[inx0].health_plan_id)
       AND h.active_ind=true
      DETAIL
       request->health_plan[inx0].health_plan_id = h.health_plan_id
      WITH nocounter
     ;end select
     IF (curqual=0)
      SET failed = update_error
