CREATE PROGRAM dm2_mig_dt_bdm_reset
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ddbr_cmd = vc WITH protect, noconstant("")
 DECLARE ddbr_dir = vc WITH protect, noconstant("")
 DECLARE ddbr_cnt = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_migdtbdmrst",".log","dm2_mig_dt_bdm_reset")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_dt_bdm_reset"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Verify that DT BDM reset may be run."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Reset not necessary since Downtime BDM has not been completed."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 FOR (ddbr_cnt = 1 TO 3)
   CASE (ddbr_cnt)
    OF 1:
     SET ddbr_dir = evaluate(dm2_sys_misc->cur_os,"AXP","ggdelivery:[dirtmp.log]",
      "/ggdelivery/dirtmp/log/")
    OF 2:
     SET ddbr_dir = evaluate(dm2_sys_misc->cur_os,"AXP","ggdelivery:[dirtmp.exec]",
      "/ggdelivery/dirtmp/exec/")
    OF 3:
     SET ddbr_dir = evaluate(dm2_sys_misc->cur_os,"AXP","ggdelivery:[dirtmp]","/ggdelivery/dirtmp/")
   ENDCASE
   SET ddbr_cmd = concat(evaluate(dm2_sys_misc->cur_os,"AXP",concat("delete ",ddbr_dir),concat(
      "rm -f ",ddbr_dir)),"dm2_*_ei_d*.*")
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET ddbr_cmd = concat(ddbr_cmd,";*")
   ENDIF
   SET dm_err->disp_dcl_err_ind = 0
   IF (dm2_push_dcl(ddbr_cmd) != 1)
    IF ((dm_err->err_ind=1))
     GO TO exit_program
   ...
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Clean DT specific metadata."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 DELETE  FROM dm_info d
  WHERE d.info_domain="DM2_MIGRATION"
   AND d.info_name IN ("DM2_MIG_DT_EXPIMP_DRIVER_STATUS", "DM2_MIG_DT_EXPIMP_DATA")
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 COMMIT
#exit_program
 SET dm_err->eproc = "Dm2_mig_dt_bdm_reset completed"
 CALL final_disp_msg("dm2_migdtbdmrst")
END GO
