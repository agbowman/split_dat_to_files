CREATE PROGRAM dm2_shc_nocach_cd_val:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsn_err_status = c1 WITH protect, noconstant(" ")
 FREE RECORD dsn_code
 RECORD dsn_code(
   1 qual[*]
     2 codevalue = f8
   1 codevalue_cnt = i4
 )
 IF (check_logfile("DM2_SHC_NOCACH_CV",".log","DM2_SHC_NOCACH_CD_VAL LOG FILE")=0)
  SET dsn_err_status = "F"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "System Health Check working on Non-Cached Code Values"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 SELECT INTO "nl:"
  ...
  c.collation_seq
  FROM code_value c
  WHERE ((c.collation_seq=null) OR (c.begin_effective_dt_tm=null))
  DETAIL
   dsn_code->codevalue_cnt += 1, stat = alterlist(dsn_code->qual,dsn_code->codevalue_cnt), dsn_code->
   qual[dsn_code->codevalue_cnt].codevalue = c.code_value
  WITH nocounter
 ;end select
 IF (check_error("Obtain non-cached code values.") > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 IF ((dm_err->err_ind=1))
  SET dsn_err_status = "E"
 ELSEIF (curqual=0)
  SET dsn_err_status = "G"
 ELSE
  SET dsn_err_status = "R"
 ENDIF
 IF (dshc_add_check_to_rs("NON-CACHED CODE VALUES","Non-Cached Code Values",0,dsn_err_status,
  dshc_ctf_millennium)=0)
  SET dsn_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dsn_err_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dsn_code->codevalue_cnt))
  DETAIL
   ...
   dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Non-Cached Code Value", dshc->check[dshc->
   chk_cnt].desc_detail[cnt].value_prefix = "NOT_SET", dshc->check[dshc->chk_cnt].desc_detail[cnt].
   dtl_desc_text = "NOT_SET",
   dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 1, dshc->check[dshc->chk_cnt].desc_detail[cnt].
   nbr_value = dsn_code->qual[d.seq].codevalue
  WITH nocounter
 ;end select
 IF (check_error("Write out description details for non-cached code values.") > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dsn_err_status = "F"
 ENDIF
 SET dshc->check[dshc->chk_cnt].check_desc_text =
 "The following code values are not being cached by the CPM Code Cache Manager:"
#exit_script
 ...
 IF (dsn_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_NOCACH_CD_VAL WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_NOCACH_CD_VAL WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_NOCACH_CD_VAL COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_NOCACH_CV")
 FREE RECORD dsn_code
END GO
