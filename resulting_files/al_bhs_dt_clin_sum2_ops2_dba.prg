CREATE PROGRAM al_bhs_dt_clin_sum2_ops2:dba
 SET unit_cd = 0
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (((e.reg_dt_tm >= cnvtlookbehind("36,H",sysdate)
    AND unit_cd_disp_key IN ("EDPEDI", "EDMAIN", "EDA", "EDGTA", "ESA",
   "ESB", "ESC", "ESD", "ESP", "ESX",
   "ESW")) OR ( NOT (unit_cd_disp_key IN ("EDPEDI", "EDMAIN", "EDA", "EDGTA", "ESA",
   "ESB", "ESC", "ESD", "ESP", "ESX",
   "ESW")))) )
    ...
    act_pat->qual[cnt].filename = build(trim(substring(1,5,trim(cnvtlower(cnvtalphanum(p
   ...
   IF ((pat->qual[x].filename > " "))
    SELECT INTO value(pat->qual[x].filename)
     ...
     DETAIL
      ...
      temp2 = pat->qual[x].ms_wearing_bpv_band, tempstring = concat(build(temp2)), ycol = (ycol+ 10),
      wrapcol = 122, line_wrap2, temp1 = fillstring(500,"")
      IF (ycol > 710)
       xcol = 100, ycol = (ycol+ 10),
       CALL print(calcpos(xcol,ycol)),
