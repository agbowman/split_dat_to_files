CREATE PROGRAM dm2_verify_date_columns
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dvdc_loop_var = i4 WITH protect, noconstant(0)
 DECLARE dvdc_dcuc_fname = vc WITH protect, noconstant(" ")
 DECLARE dvdc_idx = i4 WITH protect, noconstant(0)
 DECLARE dvdc_var = i4 WITH protect, noconstant(0)
 DECLARE dvdc_ndx = i4 WITH protect, noconstant(0)
 DECLARE dvdc_skip_table = i4 WITH protect, noconstant(0)
 DECLARE dvdc_mngd_tables = vc WITH protect, noconstant("")
 DECLARE dvdc_doc_idx = i4 WITH protect, noconstant(0)
 DECLARE dvdc_standalone = i2 WITH protect, noconstant(0)
 FREE RECORD dvdc_curr_dt_cols
 RECORD dvdc_curr_dt_cols(
   1 cnt = i4
   1 qual[*]
     2 tbl_name = vc
     2 col_name = vc
 )
 SET dvdc_curr_dt_cols->cnt = 0
 FREE RECORD dvdc_doc
 RECORD dvdc_doc(
   1 cnt = i4
   1 qual[*]
     2 tbl_name = vc
 ...
 SET dvdc_doc->cnt = 0
 FREE RECORD dvdc_invalid_dt_cols
 RECORD dvdc_invalid_dt_cols(
   1 cnt = i4
   1 qual[*]
     2 tbl_name = vc
     2 col_name = vc
 )
 SET dvdc_invalid_dt_cols->cnt = 0
 IF ((dm2_install_schema->curprog="NONE"))
  SET dvdc_standalone = 1
 ENDIF
 IF (check_logfile("dm2_verify_date_columns",".log","dm2_verify_date_columns")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_verify_date_columns"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (validate(dm2_bypass_verify_dt_cols,- (1))=1)
  SET dm_err->eproc = "User requested to bypass dm2_verify_date_columns.  Exiting script."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dvdc_mngd_tables = ""
 IF (dmr_load_managed_tables(dvdc_mngd_tables)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_fill_sch_except("LOCAL")=0)
  GO TO exit_script
 ENDIF
 IF (dum_load_date_columns("V","")=0)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 5))
  CALL echorecord(dus_date_cols)
 ENDIF
 SET dm_err->eproc = "Loading Documented Tables into Record structure."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  dtd.table_name
  FROM dm_tables_doc dtd
  WHERE  NOT (expand(dvdc_ndx,1,dm2_sch_except->tcnt,dtd.table_name,dm2_sch_except->tbl[dvdc_ndx].
   tbl_name))
   AND dtd.owner="V500"
   AND dtd.drop_ind=0
  ORDER BY dtd.owner, dtd.table_name
  DETAIL
   dvdc_doc->cnt = (dvdc_doc->cnt+ 1), stat = alterlist(dvdc_doc->qual,dvdc_doc->cnt), dvdc_doc->
   qual[dvdc_doc->cnt].tbl_name = dtd.table_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 IF ((dvdc_doc->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No documented tables loaded."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 5))
  CALL echorecord(dvdc_doc)
 ENDIF
 SET dm_err->eproc = "Loading Current Date columns into Record structure."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  dtc.table_name, dtc.column_name
  FROM dba_tab_cols dtc,
   dba_tables dt
  WHERE dt.owner="V500"
   AND dt.table_name != "MVJ*"
   AND  NOT (dt.table_name IN ("CCL_SYNCH_CMP", "CCL_SYNCH_DATA", "TEMP_OCI", "TEMP_OCS"))
   AND parser(concat(" dt.table_name not in (",dvdc_mngd_tables,")"))
   AND dt.temporary="N"
   AND dt.owner=dtc.owner
   AND dt.table_name=dtc.table_name
   AND dtc.data_type="DATE"
   AND dtc.column_name != "UTC_TMP*"
   AND  NOT (dtc.hidden_column="YES"
   AND dtc.virtual_column="NO")
  ORDER BY dtc.owner, dtc.table_name, dtc.column_name
  HEAD dtc.table_name
   dvdc_skip_table = 0, dvdc_doc_idx = 0
   IF (substring(1,4,dtc.table_name)="CQM_"
    AND ((substring((size(trim(dtc.table_name)) - 3),4,dtc.table_name)="_QUE") OR (((substring((size(
     trim(dtc.table_name)) - 4),5,dtc.table_name)="_TR_1") OR (substring((size(trim(dtc.table_name))
     - 4),5,dtc.table_name)="_TR_2")) )) )
    dvdc_skip_table = 1
   ENDIF
   IF (dvdc_skip_table=0)
    dvdc_doc_idx = locateval(dvdc_doc_idx,1,dvdc_doc->cnt,dtc.table_name,dvdc_doc->qual[dvdc_doc_idx]
     .tbl_name)
   ENDIF
  DETAIL
   IF (dvdc_doc_idx > 0)
    dvdc_curr_dt_cols->cnt = (dvdc_curr_dt_cols->cnt+ 1), stat = alterlist(dvdc_curr_dt_cols->qual,
     dvdc_curr_dt_cols->cnt), dvdc_curr_dt_cols->qual[dvdc_curr_dt_cols->cnt].tbl_name = dtc
    .table_name,
    dvdc_curr_dt_cols->qual[dvdc_curr_dt_cols->cnt].col_name = dtc.column_name
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 5))
  CALL echorecord(dvdc_curr_dt_cols)
 ENDIF
 FOR (dvdc_loop_var = 1 TO dvdc_curr_dt_cols->cnt)
   SET dvdc_idx = 0
   SET dvdc_idx = locateval(dvdc_idx,1,dus_date_cols->cnt,dvdc_curr_dt_cols->qual[dvdc_loop_var].
    tbl_name,dus_date_cols->qual[dvdc_idx].tbl_name,
    dvdc_curr_dt_cols->qual[dvdc_loop_var].col_name,dus_date_cols->qual[dvdc_idx].col_name)
   IF (dvdc_idx=0)
    SET dvdc_invalid_dt_cols->cnt = (dvdc_invalid_dt_cols->cnt+ 1)
    SET stat = alterlist(dvdc_invalid_dt_cols->qual,dvdc_invalid_dt_cols->cnt)
    SET dvdc_invalid_dt_cols->qual[dvdc_invalid_dt_cols->cnt].tbl_name = dvdc_curr_dt_cols->qual[
    dvdc_loop_var].tbl_name
    SET dvdc_invalid_dt_cols->qual[dvdc_invalid_dt_cols->cnt].col_name = dvdc_curr_dt_cols->qual[
    dvdc_loop_var].col_name
   ENDIF
 ENDFOR
 IF ((dm_err->debug_flag > 5))
  CALL echorecord(dvdc_invalid_dt_cols)
 ENDIF
 IF ((dvdc_invalid_dt_cols->cnt > 0))
  SET dm_err->eproc = "Create file for Date Columns not found."
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dvdc_dcuc_fname = "dm2_migutc_missing_dates"
  IF (get_unique_file(dvdc_dcuc_fname,".rpt")=0)
   GO TO exit_script
  ENDIF
  SET dvdc_dcuc_fname = dm_err->unique_fname
  SELECT INTO value(dvdc_dcuc_fname)
   FROM (dummyt d  WITH d.seq = 1)
   HEAD REPORT
    col 40, "Missing Date Columns from Master List", row + 1,
    col 40, "--------------------------------------------", row + 1,
    row + 1, col 2, "Table Name",
    col 40, "Column Name"
   DETAIL
    FOR (dvdc_idx = 1 TO dvdc_invalid_dt_cols->cnt)
      row + 2, col 2, dvdc_invalid_dt_cols->qual[dvdc_idx].tbl_name,
      col 40, dvdc_invalid_dt_cols->qual[dvdc_idx].col_name
    ENDFOR
   WITH nocounter, format = variable, formfeed = none,
    nullreport
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  IF (dvdc_standalone=1)
   IF (dm2_disp_file(dvdc_dcuc_fname," ")=0)
    GO TO exit_script
   ENDIF
  ENDIF
  SET dm_err->eproc = "Check database for Missing Tables/Columns from Master date column List."
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat(
   "Found missing table/columns. Review the following report in CCLUSERDIR for further details: ",
   dvdc_dcuc_fname)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSE
  SET dm_err->eproc = "All clinical dates accounted for in master list."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 GO TO exit_script
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_verify_date_columns completed successfully"
 ENDIF
 CALL final_disp_msg("dm2_verify_date_columns")
END GO
