CREATE PROGRAM bhs_rpt_tran_tagtst:dba
 PROMPT
  ...
  "Print Unit Totals:" = 1
  WITH outdev, nurseunit, bdate,
  edate, emailadd, formtype,
  printtotals
 ...
 DECLARE sbp15minautotransfusion = f8 WITH constant(260368074.00)
 ...
 DECLARE incomplience = vc WITH noconstant(" ")
 ...
 IF (reflect( $NURSEUNIT)="F8")
  SELECT INTO "nl:"
   ...
   FROM encntr_loc_hist elh,
    ...
    person p
   PLAN (elh
    WHERE elh.beg_effective_dt_tm <= cnvtdatetime(end_date_qual)
     AND elh.end_effective_dt_tm >= cnvtdatetime(beg_date_qual)
     AND elh.loc_nurse_unit_cd=cnvtreal( $NURSEUNIT))
    JOIN (e
    ...
    WHERE e.person_id=p.person_id
     AND p.active_ind=1)
   ORDER BY elh.loc_nurse_unit_cd, p.name_full_formatted
   HEAD REPORT
    i_nurseunit_cnt = 0, i_patient_cnt = 0, ms_facility_name = uar_get_code_display(elh
     .loc_facility_cd),
    ms_nursingunit_name = uar_get_code_display(elh.loc_nurse_unit_cd)
   HEAD elh.loc_nurse_unit_cd
    i_patient_cnt = 0, i_nurseunit_cnt = (i_nurseunit_cnt+ 1), stat = alterlist(forms->nurseunit,
     i_nurseunit_cnt),
    forms->nurseunit[i_nurseunit_cnt].s_nurseunitname = trim(uar_get_code_display(elh
      .loc_nurse_unit_cd),3), forms->nurseunit[i_nurseunit_cnt].f_nurseunit_cd = elh
    .loc_nurse_unit_cd
   HEAD elh.encntr_id
    ...
    ENDIF
    forms->nurseunit[i_nurseunit_cnt].patientslist[i_patient_cnt].f_encntr_id = elh.encntr_id, forms
 ...
 ELSE
  SELECT INTO "NL:"
   FROM dcp_forms_ref dfr,
    ...
    encntr_loc_hist elh
   PLAN (dfr
    WHERE dfr.beg_effective_dt_tm <= cnvtdatetime(cnvtdatetime(curdate,curtime))
     AND dfr.end_effective_dt_tm >= cnvtdatetime(cnvtdatetime(curdate,curtime))
 ...
 SELECT INTO "nl:"
  nurseunit = uar_get_code_display(forms->nurseunit[d.seq].f_nurseunit_cd), name = forms->nurseunit[d
  .seq].patientslist[d1.seq].s_person_name, ce.parent_event_id,
  ce2.event_cd
  ...
  PLAN (d
   ...
   mf_nursestatementofattestation))
   JOIN (elh
   WHERE elh.encntr_id=ce.encntr_id
    AND ce.performed_dt_tm BETWEEN elh.beg_effective_dt_tm AND elh.end_effective_dt_tm)
   JOIN (pr
  ...
  ORDER BY nurseunit, name, ce.parent_event_id,
   ce2.event_cd DESC
  HEAD REPORT
  ...
  HEAD ce.parent_event_id
   ...
   i_infused_cnt = 0, i_reaction_cnt = 0, forms->nurseunit[d.seq].s_nurseunitname =
   uar_get_code_display(elh.loc_nurse_unit_cd),
   i_instance_cnt = (i_instance_cnt+ 1), stat = alterlist(forms->nurseunit[d.seq].patientslist[d1.seq
    ].forminstanceslist[i_instance_cnt],i_instance_cnt), forms->nurseunit[d.seq].patientslist[d1.seq]
   .forminstanceslist[i_instance_cnt].f_parent_event_id = ce.parent_event_id,
   ...
   i_instance_cnt].s_performed_on = build(format(ce.performed_dt_tm,";;q"),"_",ce.parent_event_id)
  HEAD ce2.event_cd
   ...
   ELSEIF (ce2.event_cd=mf_nursestatementofattestation)
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    s_nursestatementofattestation = s_tran_result
   ENDIF
  FOOT  ce2.event_cd
   stat = 0
  FOOT  ce.parent_event_id
   IF (i_consent_cnt=1
    ...
    forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_incomplience =
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (i_first=0)
    ...
    forminstanceslist[d2.seq].s_originalnurse = trim(p.name_full_formatted)
   ENDIF
   IF (i_first=1)
 ...
 SELECT INTO value(trim(var_output))
  ...
  ENDIF
  , finnumber = substring(1,20,trim(forms->nurseunit[d.seq].patientslist[d1.seq].s_finnbr)),
  ...
  rhimmuneglobulin = substring(1,25,trim(forms->nurseunit[d.seq].patientslist[d1.seq].
    ...
    s_bloodproductamountinfused)), transfusionreactiondescription = substring(1,25,trim(forms->
    nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[d2.seq].s_transfusionreactiondescription)
   ),
  incomplience = substring(1,25,trim(forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[
    d2.seq].s_incomplience)), originalnurse = substring(1,25,trim(forms->nurseunit[d.seq].
    patientslist[d1.seq].forminstanceslist[d2.seq].s_originalnurse)), nursestatementofattestation =
  substring(1,50,trim(forms->nurseunit[d.seq].patientslist[d1.seq].forminstanceslist[d2.seq].
    s_nursestatementofattestation))
