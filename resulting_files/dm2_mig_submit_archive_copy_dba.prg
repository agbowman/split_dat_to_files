CREATE PROGRAM dm2_mig_submit_archive_copy:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmsac_iter = i4 WITH protect, noconstant(0)
 DECLARE dmsac_row_exists = i2 WITH protect, noconstant(0)
 DECLARE dmsac_continue = i2 WITH protect, noconstant(1)
 DECLARE dmsac_file_loc = vc WITH protect, noconstant(logical("CCLUSERDIR"))
 DECLARE dmsac_submit_time = i4 WITH protect, noconstant(5)
 DECLARE dmsac_retain_time = i4 WITH protect, noconstant(120)
 DECLARE dmsac_last_timestamp = dq8 WITH protect, noconstant(cnvtdatetime(curdate,curtime3))
 DECLARE dmsac_temp_line = vc WITH protect, noconstant("")
 DECLARE dmsac_queue_fnd_ret = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_submit_arch",".log","dm2_mig_submit_archive_copy LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_submit_archive_copy"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Determining current OS."
 IF (cursys != "AXP")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "dm2_mig_submit_archive_copy can only be used from a VMS node."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Check for archive copy first execution row in DM_INFO."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_MIG_DATA"
   AND d.info_name="ARCHIVE_COPY_FIRST_EXEC"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Insert archive copy first execution row into DM_INFO."
  INSERT  FROM dm_info d
   SET d.info_domain = "DM2_MIG_DATA", d.info_name = "ARCHIVE_COPY_FIRST_EXEC", d.info_date =
    cnvtdatetime(curdate,curtime3)
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Prompt for miscellaneous information to get archive copy job submitted."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET width = 132
 SET message = window
 CALL clear(1,1)
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name="MISC_FILE_LOCATION"
  DETAIL
   dmsac_file_loc = di.info_char
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dmsac_row_exists = evaluate(curqual,0,0,1)
 WHILE (dmsac_continue=1)
   CALL text(2,2,"Please provide a location where miscellaneous log files will be stored:")
   CALL accept(2,74,"P(38);CU",dmsac_file_loc)
   SET dmsac_file_loc = curaccept
   IF (dmr_check_directory(dmsac_file_loc)=0)
    SET dm_err->err_ind = 0
    CALL text(23,2,"The location you have provided is not valid.  Please try again.")
   ELSE
    SET dmsac_continue = 0
    CALL clear(23,1,131)
    CALL box(1,1,24,131)
   ENDIF
 ENDWHILE
 IF (dmsac_row_exists=1)
  SET dm_err->eproc = "Attempting to update existing file location row in dm_info."
  UPDATE  FROM dm_info di
   SET di.info_char = dmsac_file_loc
   WHERE di.info_domain="DM2_MIG_DATA"
    AND di.info_name="MISC_FILE_LOCATION"
   WITH nocounter
  ;end update
 ...
 ELSE
  ...
  INSERT  FROM dm_info di
   SET di.info_char = dmsac_file_loc, di.info_domain = "DM2_MIG_DATA", di.info_name =
    "MISC_FILE_LOCATION"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name="ARCH_SUBMIT_TIME"
  DETAIL
   dmsac_submit_time = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 SET dmsac_row_exists = evaluate(curqual,0,0,1)
 CALL text(4,2,"How often would you like the archive log copy job to be submitted (1-59 minutes):")
 CALL accept(4,84,"99",dmsac_submit_time
  WHERE curaccept BETWEEN 1 AND 59)
 SET dmsac_submit_time = curaccept
 IF (dmsac_row_exists=1)
  SET dm_err->eproc = "Attempting to update existing submit time row in dm_info."
  UPDATE  FROM dm_info di
   SET di.info_number = dmsac_submit_time
   WHERE di.info_domain="DM2_MIG_DATA"
    AND di.info_name="ARCH_SUBMIT_TIME"
   WITH nocounter
  ;end update
 ...
 ELSE
  ...
  INSERT  FROM dm_info di
   SET di.info_number = dmsac_submit_time, di.info_domain = "DM2_MIG_DATA", di.info_name =
    "ARCH_SUBMIT_TIME"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Determining existing retain time from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name="ARCH_RETAIN_TIME"
  DETAIL
   dmsac_retain_time = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 SET dmsac_row_exists = evaluate(curqual,0,0,1)
 CALL text(6,2,"How many hours of archive logs should be retained (4-168 hours):")
 CALL accept(6,66,"999",dmsac_retain_time
  WHERE curaccept BETWEEN 4 AND 168)
 SET dmsac_retain_time = curaccept
 IF (dmsac_row_exists=1)
  SET dm_err->eproc = "Attempting to update existing retain time row in dm_info."
  UPDATE  FROM dm_info di
   SET di.info_number = dmsac_retain_time
   WHERE di.info_domain="DM2_MIG_DATA"
    AND di.info_name="ARCH_RETAIN_TIME"
   WITH nocounter
  ;end update
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Inserting retain time row into dm_info."
  INSERT  FROM dm_info di
   SET di.info_number = dmsac_retain_time, di.info_domain = "DM2_MIG_DATA", di.info_name =
    "ARCH_RETAIN_TIME"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Determining existing last timestamp from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_DATA"
   AND di.info_name="ARCH_LAST_TIMESTAMP"
  DETAIL
   dmsac_last_timestamp = di.info_date
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 SET dmsac_row_exists = evaluate(curqual,0,0,1)
 CALL text(8,2,"Please provide the last timestamp when archive logs were copied:")
 CALL accept(8,67,"99DAAAD9999D99D99D99;CU",format(dmsac_last_timestamp,"DD-MMM-YYYY:HH:MM:SS;;Q")
  WHERE curaccept=format(cnvtdatetime(curaccept),"DD-MMM-YYYY:HH:MM:SS;;Q"))
 SET dmsac_last_timestamp = cnvtdatetime(curaccept)
 IF (dmsac_row_exists=1)
  SET dm_err->eproc = "Attempting to update existing last timestamp row in dm_info."
  UPDATE  FROM dm_info di
   SET di.info_date = cnvtdatetime(dmsac_last_timestamp)
   WHERE di.info_domain="DM2_MIG_DATA"
    AND di.info_name="ARCH_LAST_TIMESTAMP"
   WITH nocounter
  ;end update
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Inserting last timestamp row into dm_info."
  INSERT  FROM dm_info di
   SET di.info_date = cnvtdatetime(dmsac_last_timestamp), di.info_domain = "DM2_MIG_DATA", di
    .info_name = "ARCH_LAST_TIMESTAMP"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
 ...
 IF (curqual=0)
  ...
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dmr_check_directory(build("NFSARCH:[V500.DB_",dmr_mig_data->src_db_name,".ARCH]"))=0)
   SET dm_err->emsg = build("NFSARCH:[V500.DB_",dmr_mig_data->src_db_name,
    ".ARCH] is not available or does not have write privs.")
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Check that migration batch queue exists."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dmr_check_batch_queue(dmr_batch_queue,dmsac_queue_fnd_ret)=0)
  GO TO exit_script
 ELSEIF (dmsac_queue_fnd_ret=0)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = concat("Check if migration batch queue, ",dmr_batch_queue," is properly setup."
   )
 ...
 IF (dmr_get_queue_contents(dmr_batch_queue)=0)
  GO TO exit_script
 ENDIF
 FOR (dmsac_iter = 1 TO dmr_queue->cnt)
   IF ((dmr_queue->qual[dmsac_iter].jobname="DM2_MIG_COPY_ARCHIVELOGS"))
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Determining if Migration Archive Copy Job is already in job queue."
    SET dm_err->emsg = "The job is already in the queue.  Please stop the job via dm2_mig_menu."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Writing contents to dm2_mig_copy_archivelogs.com."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO value(build(dmsac_file_loc,"dm2_mig_copy_archivelogs.com"))
  FROM (dummyt d  WITH seq = 1)
  DETAIL
   CALL print("$!"), row + 1,
   CALL print("$! Executing Copy Archivelogs Script..."),
   row + 1,
   CALL print("!"), row + 1,
   ...
   CALL print(concat("define oraclesystem 'V500/",dmr_mig_data->src_v500_pwd,"@",dmr_mig_data->
    src_v500_cnct_str,"' go")), row + 1,
   CALL print("dm2_mig_copy_archivelogs go"),
   row + 1,
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 IF (dm2_push_dcl(concat("SET FILE/PROT =(s:RWED, o:RWED, g:RWED, w:RWED) ",dmsac_file_loc,
   "dm2_mig_copy_archivelogs.com"))=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Submitting dm2_mig_copy_archivelogs.com job to migration batch queue."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_push_dcl(concat("SUBMIT /QUEUE=",dmr_batch_queue," /NAME=dm2_mig_copy_archivelogs ",
   dmsac_file_loc,"dm2_mig_copy_archivelogs.com /LOG_FILE=",
   dmsac_file_loc,"dm2_mig_copy_archivelogs.log"))=0)
  GO TO exit_script
 ENDIF
#exit_script
 SET message = nowindow
 ...
 ELSE
  ...
  CALL disp_msg("",dm_err->logfile,0)
  SET dm_err->eproc = "Dm2_mig_submit_archive_copy completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_submit_arch")
END GO
