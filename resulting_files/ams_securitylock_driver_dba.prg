CREATE PROGRAM ams_securitylock_driver:dba
 PROMPT
 ...
 RECORD file_content(
   1 qual[*]
     2 security_key_name = vc
     2 security_key_lock = vc
 )
 SELECT
  r.line
  FROM rtl2t r
  ...
  HEAD r.line
   ...
   IF (size(trim(line1),1) > 0)
    ...
    IF (count > 1)
     ...
     ENDIF
     file_content->qual[row_count].security_key_name = piece(line1,",",1,"Not Found"), file_content->
     qual[row_count].security_key_lock = piece(line1,",",2,"Not Found")
    ENDIF
   ENDIF
  FOOT REPORT
 ...
 IF (( $AUDITCOMMIT="Audit"))
  SELECT INTO  $OUTDEV
   security_key_name = substring(1,30,file_content->qual[d1.seq].security_key_name),
   security_key_lock = substring(1,30,file_content->qual[d1.seq].security_key_lock)
   FROM (dummyt d1  WITH seq = value(size(file_content->qual,5)))
   PLAN (d1)
   WITH nocounter, separator = " ", format
  ;end select
 ELSE
  EXECUTE ams_security_lock_to_key_chain:dba
