CREATE PROGRAM dm2_mig_setup_suppl_log:dba
 ...
 END ;Subroutine
 FREE RECORD dmssl_sup_logging_table_list
 RECORD dmssl_sup_logging_table_list(
   1 cnt = i4
   1 qual[*]
     2 table_name = vc
     2 owner = vc
     ...
     2 del_trig_ind = i2
     2 upd_trig_ind = i2
     2 upd_trig_col_list = vc
     2 object_id = f8
     2 pk_ind = i2
     2 when_clause = vc
 )
 FREE RECORD dmssl_unique_indexes
 RECORD dmssl_unique_indexes(
   1 cnt = i4
   1 qual[*]
     2 index_name = vc
     2 owner = vc
 )
 DECLARE dmssl_temp_col_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmssl_temp_col_list = vc WITH protect, noconstant("")
 DECLARE dmssl_sup_act_ind = i2 WITH protect, noconstant(0)
 DECLARE dmssl_run_id = i4 WITH protect, noconstant(0)
 DECLARE dmssl_search = i4 WITH protect, noconstant(0)
 DECLARE dmssl_search2 = i4 WITH protect, noconstant(0)
 DECLARE dmssl_schema_date = dq8 WITH protect, noconstant(0.0)
 DECLARE dmssl_max_op_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmssl_op_type = vc WITH protect, noconstant("")
 DECLARE dmssl_obj_name = vc WITH protect, noconstant("")
 DECLARE dmssl_table_name = vc WITH protect, noconstant("")
 DECLARE dmssl_priority = i4 WITH protect, noconstant(0)
 DECLARE dmssl_tablespace_name = vc WITH protect, noconstant("")
 DECLARE dmssl_row_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmssl_operation = vc WITH protect, noconstant("")
 DECLARE dmssl_desc = vc WITH protect, noconstant("")
 DECLARE dmssl_temp = vc WITH protect, noconstant("")
 DECLARE dmssl_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmssl_pk_trig_needed = i2 WITH protect, noconstant(0)
 SET dm2_install_schema->process_option = "MIGRATION SETUP"
 IF (check_logfile("dm2_mig_setup_suppl",".log","DM2_MIG_SETUP_SUPPL_LOG LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_setup_suppl_log"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Determining max op_id from dm2_ddl_ops_log for target."
 SELECT INTO "nl:"
  max_op_id = max(ddol.op_id)
  FROM dm2_ddl_ops_log ddol
  DETAIL
   dmssl_max_op_id = max_op_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSEIF ((dmr_user_list->cnt=0))
  ...
  GO TO exit_script
 ENDIF
 FOR (dmssl_cnt = 1 TO dmr_user_list->cnt)
   IF ( NOT ((dmr_user_list->qual[dmssl_cnt].user IN ("V500", "V500_EVENT", "V500_REF", "V500_READ"))
   ))
    SET dm_err->eproc = concat("Grant privs on DM_INFO to ",dmr_user_list->qual[dmssl_cnt].user)
    IF (dm2_push_cmd(concat("rdb grant select,insert,update on V500.DM_INFO to ",dmr_user_list->qual[
      dmssl_cnt].user," go"),1)=0)
     GO TO exit_script
    ENDIF
   ENDIF
 ENDFOR
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dmssl_run_id = ddo.run_id, dmssl_schema_date = ddo.schema_date
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSE
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Determining max op_id from dm2_ddl_ops_log for source."
 SELECT INTO "nl:"
  max_op_id = max(ddol.op_id)
  FROM dm2_ddl_ops_log ddol
  DETAIL
   dmssl_max_op_id = greatest(max_op_id,(dmssl_max_op_id+ 100))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 SET dm_err->eproc =
 "Determining list of tables that will be moved as part of database migration from dba_tables."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_tables dt,
   dba_objects do
  ...
  HEAD REPORT
   dmssl_sup_logging_table_list->cnt = 0, stat = alterlist(dmssl_sup_logging_table_list->qual,0)
  DETAIL
   dmssl_sup_logging_table_list->cnt = (dmssl_sup_logging_table_list->cnt+ 1)
   IF (mod(dmssl_sup_logging_table_list->cnt,100)=1)
    stat = alterlist(dmssl_sup_logging_table_list->qual,(dmssl_sup_logging_table_list->cnt+ 99))
   ENDIF
   dmssl_sup_logging_table_list->qual[dmssl_sup_logging_table_list->cnt].owner = dt.owner,
   dmssl_sup_logging_table_list->qual[dmssl_sup_logging_table_list->cnt].table_name = dt.table_name,
   dmssl_sup_logging_table_list->qual[dmssl_sup_logging_table_list->cnt].object_id = do.object_id,
   dmssl_sup_logging_table_list->qual[dmssl_sup_logging_table_list->cnt].sup_log_name = build("GGS_",
    substring(1,15,dt.table_name),"_",cnvtint(do.object_id)), dmssl_sup_logging_table_list->qual[
   dmssl_sup_logging_table_list->cnt].del_trig_name = build("TRG_DM2_MIG_DEL_",cnvtint(do.object_id)),
   dmssl_sup_logging_table_list->qual[dmssl_sup_logging_table_list->cnt].upd_trig_name = build(
    "TRG_DM2_MIG_UPD_",cnvtint(do.object_id))
  FOOT REPORT
   stat = alterlist(dmssl_sup_logging_table_list->qual,dmssl_sup_logging_table_list->cnt)
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 SET dm_err->eproc =
 "Determining if supplemental logging has already been enabled for the database migration from dba_log_groups."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_log_groups dlg,
   dba_log_group_columns dlgc
  WHERE dlg.owner IN (
  (SELECT
   info_name
   FROM dm_info
   WHERE info_domain="DM2_MIG_USER"))
   AND dlg.owner=dlgc.owner
   AND dlg.log_group_name=dlgc.log_group_name
   AND dlg.table_name=dlgc.table_name
  ORDER BY dlg.owner, dlg.table_name, dlg.log_group_name,
   dlgc.position
  HEAD dlg.owner
   row + 0
  HEAD dlg.table_name
   row + 0
  HEAD dlg.log_group_name
   dmssl_search = 0
   IF (assign(dmssl_search,locateval(dmssl_search,1,dmssl_sup_logging_table_list->cnt,dlg
     .log_group_name,dmssl_sup_logging_table_list->qual[dmssl_search].sup_log_name)))
    dmssl_sup_logging_table_list->qual[dmssl_search].sup_log_ind = 1
   ENDIF
  DETAIL
   IF (dmssl_search > 0)
    dmssl_sup_logging_table_list->qual[dmssl_search].sup_col_list = build(
     dmssl_sup_logging_table_list->qual[dmssl_search].sup_col_list,dlgc.column_name,",")
   ENDIF
  FOOT  dlg.table_name
   IF (dmssl_search > 0)
    dmssl_sup_logging_table_list->qual[dmssl_search].sup_col_list = substring(1,(size(trim(
       dmssl_sup_logging_table_list->qual[dmssl_search].sup_col_list)) - 1),
     dmssl_sup_logging_table_list->qual[dmssl_search].sup_col_list)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 IF (dmssl_pk_trig_needed=1)
  SET dm_err->eproc =
  "Determining if Migration key column update and delete triggers have already been created."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   ...
   (SELECT
    ...
    FROM dm_info
    WHERE info_domain="DM2_MIG_USER"))
    AND dt.trigger_name IN ("TRG_DM2_MIG_DEL*", "TRG_DM2_MIG_UPD*")
    AND dt.status="ENABLED"
   ORDER BY dt.trigger_name
   DETAIL
    dmssl_search = 0, dmssl_search2 = 0
    IF (assign(dmssl_search,locateval(dmssl_search,1,dmssl_sup_logging_table_list->cnt,dt
      .trigger_name,dmssl_sup_logging_table_list->qual[dmssl_search].del_trig_name)))
     dmssl_sup_logging_table_list->qual[dmssl_search].del_trig_ind = 1
    ELSEIF (assign(dmssl_search2,locateval(dmssl_search2,1,dmssl_sup_logging_table_list->cnt,dt
      .trigger_name,dmssl_sup_logging_table_list->qual[dmssl_search2].upd_trig_name)))
     dmssl_sup_logging_table_list->qual[dmssl_search2].upd_trig_ind = 1, dmssl_desc = cnvtupper(trim(
       dt.description)), dmssl_temp = concat("ON ",dmssl_sup_logging_table_list->qual[dmssl_search2].
      owner,".",dmssl_sup_logging_table_list->qual[dmssl_search2].table_name),
     dmssl_sup_logging_table_list->qual[dmssl_search2].upd_trig_col_list = trim(substring((findstring
       ("BEFORE UPDATE OF",dmssl_desc,1)+ 17),(findstring(dmssl_temp,dmssl_desc) - (findstring(
        "BEFORE UPDATE OF",dmssl_desc,1)+ 17)),dmssl_desc))
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Determining indexes that support a primary key from dba_constraints."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  ...
  (SELECT
   ...
   WHERE info_domain="DM2_MIG_USER"))
  HEAD REPORT
   dmssl_unique_indexes->cnt = 0, stat = alterlist(dmssl_unique_indexes->qual,0)
  DETAIL
   dmssl_unique_indexes->cnt = (dmssl_unique_indexes->cnt+ 1)
   IF (mod(dmssl_unique_indexes->cnt,100)=1)
    stat = alterlist(dmssl_unique_indexes->qual,(dmssl_unique_indexes->cnt+ 99))
   ENDIF
   dmssl_unique_indexes->qual[dmssl_unique_indexes->cnt].index_name = dc.index_name,
   dmssl_unique_indexes->qual[dmssl_unique_indexes->cnt].owner = dc.owner
  FOOT REPORT
   stat = alterlist(dmssl_unique_indexes->qual,dmssl_unique_indexes->cnt)
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(dmssl_unique_indexes)
 ENDIF
 SET dm_err->eproc = "Determining columns for primary key indexes from dba_ind_columns."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_ind_columns dic
  WHERE dic.table_owner IN (
  (SELECT
   info_name
   FROM dm_info
   WHERE info_domain="DM2_MIG_USER"))
   AND dic.index_owner=dic.table_owner
  ORDER BY dic.table_owner, dic.table_name, dic.index_name,
   dic.column_position
  HEAD dic.table_owner
   row + 0
  HEAD dic.table_name
   dmssl_search = 0, dmssl_search = locateval(dmssl_search,1,dmssl_sup_logging_table_list->cnt,dic
    .table_name,dmssl_sup_logging_table_list->qual[dmssl_search].table_name,
    dic.table_owner,dmssl_sup_logging_table_list->qual[dmssl_search].owner)
  HEAD dic.index_name
   IF (dmssl_search)
    dmssl_search2 = locateval(dmssl_search2,1,dmssl_unique_indexes->cnt,dic.index_name,
     dmssl_unique_indexes->qual[dmssl_search2].index_name,
     dic.index_owner,dmssl_unique_indexes->qual[dmssl_search2].owner)
   ENDIF
  DETAIL
   IF (dmssl_search
    AND dmssl_search2)
    dmssl_sup_logging_table_list->qual[dmssl_search].index_col_cnt = (dmssl_sup_logging_table_list->
    qual[dmssl_search].index_col_cnt+ 1)
    IF ((dmssl_sup_logging_table_list->qual[dmssl_search].index_col_cnt=1))
     dmssl_sup_logging_table_list->qual[dmssl_search].index_col_list = dic.column_name,
     dmssl_sup_logging_table_list->qual[dmssl_search].when_clause = concat("OLD.",trim(dic
       .column_name)," <> NEW.",trim(dic.column_name))
    ELSE
     dmssl_sup_logging_table_list->qual[dmssl_search].index_col_list = concat(
      dmssl_sup_logging_table_list->qual[dmssl_search].index_col_list,",",dic.column_name),
     dmssl_sup_logging_table_list->qual[dmssl_search].when_clause = concat(
      dmssl_sup_logging_table_list->qual[dmssl_search].when_clause," OR OLD.",trim(dic.column_name),
      " <> NEW.",trim(dic.column_name))
    ENDIF
   ENDIF
  FOOT  dic.index_name
   IF (dmssl_search
    AND dmssl_search2)
    dmssl_sup_logging_table_list->qual[dmssl_search].pk_ind = 1
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining columns for unique indexes from dba_ind_columns."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT DISTINCT INTO "nl:"
  dic.table_name, dic.column_name
  FROM dba_ind_columns dic
  WHERE dic.index_owner=dic.table_owner
   AND  EXISTS (
  (SELECT
   "x"
   FROM dba_indexes di
  ...
  ORDER BY dic.table_owner, dic.table_name, dic.column_name
  HEAD dic.table_owner
   row + 0
  HEAD dic.table_name
   IF (assign(dmssl_search,locateval(dmssl_search,1,dmssl_sup_logging_table_list->cnt,dic.table_name,
     dmssl_sup_logging_table_list->qual[dmssl_search].table_name,
     dic.table_owner,dmssl_sup_logging_table_list->qual[dmssl_search].owner))
    AND dmssl_sup_logging_table_list->qual[dmssl_search].pk_ind)
    dmssl_search = 0
   ENDIF
 ...
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(dmssl_sup_logging_table_list)
 ENDIF
 SET dm_err->eproc = "Determining if supplemental logging activation row already exists in dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_SETUP"
   AND di.info_name="SUPPLEMENTAL LOGGING ACTIVATED"
  DETAIL
   dmssl_sup_act_ind = 1
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF ( NOT (curqual))
  SET dm_err->eproc = "Inserting supplemental logging activation row into dm_info."
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2_MIG_SETUP", di.info_name = "SUPPLEMENTAL LOGGING ACTIVATED"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Determining if supplemental logging is activated from v$database."
 SELECT INTO "nl:"
  FROM v$database vd
  WHERE vd.supplemental_log_data_min IN ("YES", "IMPLICIT")
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ( NOT (curqual))
  SET dmssl_sup_act_ind = 1
  IF (dm2_push_cmd(concat("RDB ALTER DATABASE ADD SUPPLEMENTAL LOG DATA GO"),1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 FOR (dmssl_iter = 1 TO dmssl_sup_logging_table_list->cnt)
   IF ((dmssl_sup_logging_table_list->qual[dmssl_iter].owner="V500")
    AND (dmssl_sup_logging_table_list->qual[dmssl_iter].table_name="DM_INFO"))
    SET dmssl_sup_logging_table_list->qual[dmssl_iter].del_trig_ind = 1
    SET dmssl_sup_logging_table_list->qual[dmssl_iter].upd_trig_ind = 1
    SET dmssl_sup_logging_table_list->qual[dmssl_iter].upd_trig_col_list =
    dmssl_sup_logging_table_list->qual[dmssl_iter].index_col_list
   ENDIF
   IF (dmssl_sup_logging_table_list->qual[dmssl_iter].sup_log_ind
    AND (dmssl_sup_logging_table_list->qual[dmssl_iter].sup_col_list != dmssl_sup_logging_table_list
   ->qual[dmssl_iter].index_col_list))
    SET dmssl_sup_act_ind = 1
    SET dmssl_op_type = "DROP SUPPLEMENTAL LOG GROUP"
    SET dmssl_obj_name = dmssl_sup_logging_table_list->qual[dmssl_iter].sup_log_name
    SET dmssl_table_name = dmssl_sup_logging_table_list->qual[dmssl_iter].table_name
    SET dmssl_priority = 730
    SET dmssl_tablespace_name = ""
    SET dmssl_row_cnt = 0
    SET dmssl_operation = concat("RDB ASIS(^ ALTER TABLE ",dmssl_sup_logging_table_list->qual[
     dmssl_iter].owner,".",dmssl_sup_logging_table_list->qual[dmssl_iter].table_name,
     " DROP SUPPLEMENTAL LOG GROUP ",
     dmssl_obj_name," ^) GO")
    IF (dmssl_insert_ddl_op_row(null)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   IF ((((dmssl_sup_logging_table_list->qual[dmssl_iter].sup_log_ind=0)) OR ((
   dmssl_sup_logging_table_list->qual[dmssl_iter].sup_col_list != dmssl_sup_logging_table_list->qual[
   dmssl_iter].index_col_list))) )
    SET dmssl_sup_act_ind = 1
    SET dmssl_op_type = "ADD SUPPLEMENTAL LOG GROUP"
    SET dmssl_obj_name = dmssl_sup_logging_table_list->qual[dmssl_iter].sup_log_name
    SET dmssl_table_name = dmssl_sup_logging_table_list->qual[dmssl_iter].table_name
    SET dmssl_priority = 740
    SET dmssl_tablespace_name = ""
    SET dmssl_row_cnt = 0
    SET dmssl_operation = concat("RDB ASIS(^ ALTER TABLE ",dmssl_sup_logging_table_list->qual[
     dmssl_iter].owner,".",dmssl_sup_logging_table_list->qual[dmssl_iter].table_name,
     " ADD SUPPLEMENTAL LOG GROUP ",
     dmssl_obj_name," (",dmssl_sup_logging_table_list->qual[dmssl_iter].index_col_list,
     ") ALWAYS ^) GO")
    IF (dmssl_insert_ddl_op_row(null)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   IF (dmssl_pk_trig_needed=1)
    IF ((((dmssl_sup_logging_table_list->qual[dmssl_iter].upd_trig_ind=0)) OR ((
    dmssl_sup_logging_table_list->qual[dmssl_iter].upd_trig_col_list != dmssl_sup_logging_table_list
    ->qual[dmssl_iter].index_col_list))) )
     SET dmssl_op_type = "CREATE SQL-TRIGGER"
     SET dmssl_obj_name = dmssl_sup_logging_table_list->qual[dmssl_iter].upd_trig_name
     SET dmssl_table_name = dmssl_sup_logging_table_list->qual[dmssl_iter].table_name
     SET dmssl_priority = 750
     SET dmssl_tablespace_name = ""
     SET dmssl_row_cnt = 0
     SET dmssl_operation = concat("RDB ASIS(^ CREATE OR REPLACE TRIGGER ",
      dmssl_sup_logging_table_list->qual[dmssl_iter].owner,".",dmssl_obj_name," BEFORE UPDATE OF ",
      dmssl_sup_logging_table_list->qual[dmssl_iter].index_col_list," ON ",
      dmssl_sup_logging_table_list->qual[dmssl_iter].owner,".",dmssl_sup_logging_table_list->qual[
      dmssl_iter].table_name,
      " REFERENCING NEW AS NEW OLD AS OLD ","FOR EACH ROW WHEN (",dmssl_sup_logging_table_list->qual[
      dmssl_iter].when_clause,")",
      " DECLARE A NUMBER; BEGIN SELECT COUNT(1) INTO A FROM V500.DM_INFO WHERE ",
      "INFO_DOMAIN = 'DM2_MIG_UPD_TRIG' AND INFO_NAME = '",dmssl_sup_logging_table_list->qual[
      dmssl_iter].owner,".",dmssl_sup_logging_table_list->qual[dmssl_iter].table_name,"'; ",
      "IF A = 0 THEN INSERT INTO V500.DM_INFO (INFO_DOMAIN, INFO_NAME, UPDT_DT_TM, INFO_DATE) VALUES ",
      "('DM2_MIG_UPD_TRIG', '",dmssl_sup_logging_table_list->qual[dmssl_iter].owner,".",
      dmssl_sup_logging_table_list->qual[dmssl_iter].table_name,
      "', SYSDATE, SYSDATE); END IF; ","EXCEPTION WHEN DUP_VAL_ON_INDEX THEN NULL; END; ^) GO")
     IF (dmssl_insert_ddl_op_row(null)=0)
      GO TO exit_script
     ENDIF
    ENDIF
    IF ((dmssl_sup_logging_table_list->qual[dmssl_iter].del_trig_ind=0))
     SET dmssl_op_type = "CREATE SQL-TRIGGER"
     SET dmssl_obj_name = dmssl_sup_logging_table_list->qual[dmssl_iter].del_trig_name
     SET dmssl_table_name = dmssl_sup_logging_table_list->qual[dmssl_iter].table_name
     SET dmssl_priority = 755
     SET dmssl_tablespace_name = ""
     SET dmssl_row_cnt = 0
     SET dmssl_operation = concat("RDB ASIS(^ CREATE OR REPLACE TRIGGER ",
      dmssl_sup_logging_table_list->qual[dmssl_iter].owner,".",dmssl_obj_name," AFTER DELETE ON ",
      dmssl_sup_logging_table_list->qual[dmssl_iter].owner,".",dmssl_sup_logging_table_list->qual[
      dmssl_iter].table_name,
      " DECLARE A NUMBER; BEGIN SELECT COUNT(1) INTO A FROM V500.DM_INFO WHERE ",
      "INFO_DOMAIN = 'DM2_MIG_DEL_TRIG' AND INFO_NAME = '",
      dmssl_sup_logging_table_list->qual[dmssl_iter].owner,".",dmssl_sup_logging_table_list->qual[
      dmssl_iter].table_name,"'; ",
      "IF A = 0 THEN INSERT INTO V500.DM_INFO (INFO_DOMAIN, INFO_NAME, UPDT_DT_TM, INFO_DATE) VALUES ",
      "('DM2_MIG_DEL_TRIG', '",dmssl_sup_logging_table_list->qual[dmssl_iter].owner,".",
      dmssl_sup_logging_table_list->qual[dmssl_iter].table_name,"', SYSDATE, SYSDATE); END IF; ",
      "EXCEPTION WHEN DUP_VAL_ON_INDEX THEN NULL; END; ^) GO ")
     IF (dmssl_insert_ddl_op_row(null)=0)
      GO TO exit_script
     ENDIF
    ENDIF
 ...
 ENDFOR
 IF (dmssl_op_type > " ")
  IF (get_unique_file("dmssl_ddl_report",".rpt")=0)
   GO TO exit_script
  ELSE
   SET dm2rpting->report_name = dm_err->unique_fname
  ENDIF
  SET dm2rpting->run_id = dmssl_run_id
  ...
  SET dm2rpting->schema_date = format(cnvtdatetime(dmssl_schema_date),"DD-MMM-YYYY;;D")
  ...
  CALL clear(1,1)
  CALL video(n)
  CALL text(2,1,"Continue supplemental logging setup?")
  CALL text(4,1,">>> Enter 'C' to continue continue setup or 'Q' to quit (C or Q):")
  CALL accept(4,72,"A;CU"," "
   WHERE curaccept IN ("C", "Q"))
  SET message = nowindow
  IF (curaccept="Q")
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Prompt user to continue with supplemental logging setup."
   SET dm_err->emsg = "User chose to quit from supplemental logging setup."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((validate(dm2_troubleshoot_migration,- (1)) != - (1)))
  ...
  ENDIF
 ENDIF
 SET dm2_install_schema->run_id = dmssl_run_id
 ...
 IF (curqual)
  ...
  GO TO exit_script
 ENDIF
 IF ( NOT (dmssl_sup_act_ind))
  SET dm_err->eproc = "Deleting supplemental logging activation row from dm_info."
  DELETE  FROM dm_info di
   WHERE di.info_domain="DM2_MIG_SETUP"
    AND di.info_name="SUPPLEMENTAL LOGGING ACTIVATED"
   WITH nocounter
 ...
 ENDIF
 GO TO exit_script
 SUBROUTINE dmssl_insert_ddl_op_row(null)
   SET dmssl_max_op_id = (dmssl_max_op_id+ 1)
   SET dm_err->eproc = "Deleting duplicate rows for this operation."
   DELETE  FROM dm2_ddl_ops_log ddol
    WHERE ddol.run_id=dmssl_run_id
     AND ddol.op_type=dmssl_op_type
     AND ddol.obj_name=dmssl_obj_name
     AND ddol.priority=dmssl_priority
     AND ddol.table_name=dmssl_table_name
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   INSERT  FROM dm2_ddl_ops_log ddol
    SET ddol.op_id = dmssl_max_op_id, ddol.run_id = dmssl_run_id, ddol.gen_dt_tm = cnvtdatetime(
      curdate,curtime3),
     ddol.gen_id = dmssl_run_id, ddol.begin_dt_tm = cnvtdatetime(null), ddol.end_dt_tm = cnvtdatetime
     (null),
     ddol.est_duration = null, ddol.act_duration = null, ddol.op_type = dmssl_op_type,
     ddol.table_name = dmssl_table_name, ddol.tablespace_name = dmssl_tablespace_name, ddol.row_cnt
      = dmssl_row_cnt,
     ddol.obj_name = dmssl_obj_name, ddol.priority = dmssl_priority, ddol.status = null,
     ddol.error_msg = null, ddol.operation = dmssl_operation, ddol.on_error_operation = null,
     ddol.schema_date = cnvtdatetime(dmssl_schema_date), ddol.schema_instance = 0, ddol
     .ready_to_run_ind = 0,
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_setup_suppl_log completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_setup_suppl")
END GO
