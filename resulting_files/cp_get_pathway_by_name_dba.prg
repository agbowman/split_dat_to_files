CREATE PROGRAM cp_get_pathway_by_name:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Pathway Name" = "",
  "Pathway Type Meaning" = ""
  WITH outdev, path_name, path_type_mean
 RECORD record_data(
   1 pathway_list[*]
     2 cp_pathway_id = f8
     2 cp_concept_mean = vc
     2 pathway_name = vc
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE retrievepathways(null) = null WITH protect
 DECLARE pathway_name_str = vc WITH protect, noconstant(cnvtupper(build("*", $PATH_NAME,"*")))
 DECLARE pathway_type_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003197,trim(
     $PATH_TYPE_MEAN,3)))
 DECLARE active_pathway_status_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003198,
   "ACTIVE"))
 DECLARE user_logic_domain_id = f8 WITH protect, constant(getuserlogicaldomain(reqinfo->updt_id))
 IF (pathway_type_cd <= 0)
  SET record_data->status_data.status = "F"
  SET record_data->status_data.subeventstatus.operationname = "Invalid Pathway Type Specified"
  SET record_data->status_data.subeventstatus.operationstatus = "F"
 ELSE
  CALL retrievepathways(null)
  SET record_data->status_data.status = "S"
 ENDIF
 CALL echojson(record_data)
 CALL ajaxreply(cnvtrectojson(record_data))
 SUBROUTINE retrievepathways(null)
   CALL log_message("In retrievePathways()",log_level_debug)
   DECLARE record_data_cntr = i4 WITH protect, noconstant(0)
   IF (validate(debug_ind,0))
    CALL echo(build(" pathway_type_cd -- > ",pathway_type_cd))
    CALL echo(build(" pathway_name_str -- > ",pathway_name_str))
    CALL echo(build(" user_logic_domain_id --> ",user_logic_domain_id))
   ENDIF
   SELECT INTO "nl:"
    FROM cp_pathway c
    PLAN (c
     WHERE cnvtupper(c.pathway_name)=patstring(pathway_name_str)
      AND c.pathway_type_cd=pathway_type_cd
      AND ((c.pathway_status_cd+ 0)=active_pathway_status_cd)
      AND c.logical_domain_id=user_logic_domain_id)
    ORDER BY cnvtlower(c.pathway_name)
    HEAD c.cp_pathway_id
     record_data_cntr += 1, stat = alterlist(record_data->pathway_list,record_data_cntr), record_data
     ->pathway_list[record_data_cntr].cp_concept_mean = uar_get_code_meaning(c.cp_concept_cd),
     record_data->pathway_list[record_data_cntr].cp_pathway_id = c.cp_pathway_id, record_data->
     pathway_list[record_data_cntr].pathway_name = c.pathway_name
    WITH nocounter
   ;end select
   CALL log_message("Exit retrievePathways()",log_level_debug)
 END ;Subroutine
END GO
