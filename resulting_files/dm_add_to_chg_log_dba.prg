CREATE PROGRAM dm_add_to_chg_log:dba
 ...
 SUBROUTINE call_ins_dcl(i_table_name,i_pk_where,i_log_type,i_delete_ind,i_updt_id,i_updt_task,
  i_updt_applctx,i_ctx_str,i_envid,i_spass_log_id,i_dblink,i_pk_hash,i_ptam_hash)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dat_filter(i_table_name=vc,i_rowid=vc) = i2
 DECLARE dat_pk_where(i_table_name=vc,i_rowid=vc) = vc
 DECLARE dat_proc_refchg_ins_log(i_table_name=vc,i_pk_where=vc,i_log_type=vc,i_updt_id=vc,i_updt_task
  =vc,
  i_updt_applctx=vc) = null
 DECLARE dat_proc_refchg_nm(i_table_name=vc,i_child_id=f8,i_parent_name=vc,i_parent_id=f8,i_updt_id=
  vc,
  i_updt_task=vc,i_updt_applctx=vc,i_select_str=vc) = null
 DECLARE dat_main(i_table_name=vc,i_rowid=vc,i_db_link=vc,i_context_name=vc) = null
 DECLARE dat_delete(i_table_name=vc,i_db_link=vc,i_context_name=vc) = null
 DECLARE dat_replace_delim(drd_str=vc) = vc
 IF ( NOT (validate(dm_log,0)))
  FREE RECORD dm_log
  RECORD dm_log(
    1 total = i4
    1 qual[*]
 ...
 ENDIF
 SUBROUTINE dat_filter(i_table_name,i_rowid)
   DECLARE dat_filter_name = vc
   DECLARE dat_filter_declare = vc
   DECLARE dat_return = i2
   DECLARE dat_suffix = vc
   DECLARE dat_suffix = vc
   DECLARE dat_filter_temp = vc
   SET dat_return = 0
   SET dm_err->eproc = "Checking for filter"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dat_filter_name = " "
   SELECT INTO "nl:"
    FROM dm_tables_doc dtd
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   SET dat_filter_temp = concat("REFCHG_FILTER_",trim(dat_suffix),"_*")
   SELECT INTO "nl:"
    FROM user_objects uo
    WHERE uo.object_name=patstring(dat_filter_temp)
     AND uo.object_type="FUNCTION"
     AND uo.status="VALID"
    DETAIL
     dat_filter_name = uo.object_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (trim(dat_filter_name) > " ")
    SET dm_err->eproc = "Setting up filter logic."
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dat_filter_declare = concat("declare ",dat_filter_name,"()=i2 go")
    CALL parser(dat_filter_declare)
    SELECT INTO "nl:"
     dr.column_name
     FROM dm_refchg_filter_parm dr
     WHERE dr.table_name=i_table_name
      AND dr.active_ind=1
     ORDER BY dr.parm_nbr
     HEAD REPORT
      dat_filter_name = concat(dat_filter_name,"('UPD'")
     DETAIL
      dat_filter_name = concat(dat_filter_name,",s.",trim(dr.column_name),",s.",trim(dr.column_name))
     FOOT REPORT
      dat_filter_name = concat(dat_filter_name,")")
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    SET dm_err->eproc = "Checking if row is filtered."
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dat_return = 0
    SELECT INTO "nl:"
     FROM (parser(i_table_name) s)
     WHERE s.rowid=i_rowid
      AND parser(dat_filter_name)=1
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ELSE
    SET dm_err->eproc = "Validating rowid."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM (parser(i_table_name) s)
     WHERE s.rowid=i_rowid
    ...
    IF (curqual=0)
     CALL echo("******************************************")
     CALL echo("No rows qualified using the rowid provided")
     CALL echo("******************************************")
     GO TO exit_program
    ENDIF
    SET dat_return = 1
   ...
   RETURN(dat_return)
 END ;Subroutine
 SUBROUTINE dat_pk_where(i_table_name,i_rowid)
   DECLARE dat_pk_function = vc
   DECLARE dat_pk_fun = vc
   DECLARE dat_pk_result = vc
   DECLARE dat_suffix = vc
   DECLARE dat_pk_temp = vc
   SET dat_pk_function = " "
   SET dm_err->eproc = "Obtaining refchg_pk_where function"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_tables_doc dtd
    WHERE dtd.table_name=i_table_name
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   SET dat_pk_temp = concat("REFCHG_PK_WHERE_",trim(dat_suffix),"_*")
   SELECT INTO "nl:"
    FROM user_objects uo
    WHERE uo.object_name=patstring(dat_pk_temp)
     AND uo.object_type="FUNCTION"
     AND uo.status="VALID"
    DETAIL
     dat_pk_function = uo.object_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (trim(dat_pk_function) > " ")
    SET dm_err->eproc = "Building refchg_pk_where function"
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dat_pk_fun = concat("declare ",dat_pk_function,"() = c2000 go")
    CALL parser(dat_pk_fun)
    SELECT INTO "nl:"
     drpp.column_name
     FROM dm_refchg_pkw_parm drpp
     WHERE drpp.table_name=i_table_name
     ORDER BY drpp.parm_nbr
     HEAD REPORT
      dat_pk_function = concat(dat_pk_function,"('INS/UPD'")
     DETAIL
      dat_pk_function = concat(dat_pk_function,",s.",trim(drpp.column_name))
     FOOT REPORT
      dat_pk_function = concat(dat_pk_function,")")
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ELSE
    CALL echo("REFCHG_PK_WHERE function not found for table passed in")
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = "Obtaining result for the refchg_pk_where function"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    result1 = parser(dat_pk_function)
    FROM (parser(i_table_name) s)
    WHERE s.rowid=i_rowid
    DETAIL
     dat_pk_result = result1
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   RETURN(dat_pk_result)
 END ;Subroutine
 SUBROUTINE dat_proc_refchg_ins_log(i_table_name,i_pk_where,i_log_type,i_updt_id,i_updt_task,
  i_updt_applctx)
   FREE RECORD dat_stmt
   RECORD dat_stmt(
     1 stmt[3]
   ...
   SET dat_stmt->stmt[1].str = concat("RDB ASIS(^ BEGIN proc_refchg_ins_log","('",i_table_name,"',^)"
    )
   SET dat_stmt->stmt[2].str = concat("ASIS(^ ",i_pk_where,",^)")
   SET dat_stmt->stmt[3].str = concat("ASIS(^ dbms_utility.get_hash_value(",i_pk_where,
    ",0,1073741824.0)",",'",i_log_type,
    "',0,",i_updt_id,",",i_updt_task,",",
    i_updt_applctx,"); END; ^) go")
   CALL echo(dat_stmt->stmt[1].str)
   CALL echo(dat_stmt->stmt[2].str)
   CALL echo(dat_stmt->stmt[3].str)
   CALL parser(dat_stmt->stmt[1].str,0)
   CALL parser(dat_stmt->stmt[2].str,0)
   CALL parser(dat_stmt->stmt[3].str,1)
 END ;Subroutine
 SUBROUTINE dat_proc_refchg_nm(i_table_name,i_child_id,i_parent_name,i_parent_id,i_updt_id,
  i_updt_task,i_updt_applctx,i_select_str)
   DECLARE dat_proc_function = vc
   DECLARE dat_suffix = vc
   DECLARE dat_proc_temp = vc
   DECLARE dat_par_id = vc
   IF (i_parent_id=0)
    SET dat_par_id = "null"
   ELSE
    SET dat_par_id = trim(cnvtstring(i_parent_id))
   ENDIF
   FREE RECORD dat_stmt
   RECORD dat_stmt(
     1 stmt[3]
   ...
   SET dm_err->eproc = "Obtaining proc_refchg_<suffix>_1/2 function"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_tables_doc dtd
    WHERE dtd.table_name=i_table_name
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   SET dat_proc_temp = concat("PROC_REFCHG",trim(dat_suffix),"_*")
   SELECT INTO "nl:"
    FROM user_objects uo
    WHERE uo.object_name=patstring(dat_proc_temp)
     AND uo.object_type="PROCEDURE"
     AND uo.status="VALID"
    DETAIL
     dat_proc_function = uo.object_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual != 0)
    SET dat_stmt->stmt[1].str = concat("RDB ASIS(^ BEGIN ",dat_proc_function,"(",trim(cnvtstring(
       i_child_id)),",^)")
    SET dat_stmt->stmt[2].str = concat("ASIS(^ ",dat_par_id,",'",i_parent_name,"'",
     ",^)")
    SET dat_stmt->stmt[3].str = concat("ASIS(^ ",i_updt_id,",",i_updt_task,",",
     i_updt_applctx,",'",i_select_str,"'); END; ^) go")
   ELSE
    CALL echo("PROC_REFCHG procedure was not found for current table")
    GO TO exit_program
   ENDIF
   CALL echo(dat_stmt->stmt[1].str)
   CALL echo(dat_stmt->stmt[2].str)
   ...
   CALL parser(dat_stmt->stmt[3].str,1)
 END ;Subroutine
 SUBROUTINE dat_main(i_table_name,i_rowid,i_db_link,i_context_name)
   DECLARE pk_where = vc
   DECLARE table_suffix = vc
   DECLARE table_name = vc
   DECLARE dcl_name = vc WITH protect, noconstant("")
   DECLARE dm_ptam_result = f8
   DECLARE cid_return = vc
   DECLARE pk_where_hash = f8
   DECLARE ptam_match_hash = f8
   DECLARE dm_pkw_value = f8 WITH protect, noconstant(0.0)
   DECLARE dm_total = i4 WITH protect, noconstant(0)
   DECLARE dm_idx = i4 WITH protect, noconstant(0)
   SET table_name = i_table_name
   SELECT INTO "nl:"
    FROM dm_tables_doc dtd
    WHERE dtd.full_table_name=table_name
    DETAIL
     table_suffix = dtd.table_suffix, pk_where = concat("WHERE t",trim(dtd.table_suffix),".ROWID = '",
      i_rowid,"'")
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   SELECT INTO "nl:"
    ...
    WHERE dpw.table_name=table_name
     AND dpw.delete_ind=0
    DETAIL
     pk_where_hash = dpw.pk_where_hash, ptam_match_hash = dpw.ptam_match_hash
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN
   ENDIF
   CALL drmm_get_col_string(pk_where,0.0,table_name,table_suffix,i_db_link)
   CALL drmm_get_ptam_match_query(0.0,table_name,table_suffix,i_db_link)
   SET dm_ptam_result = drmm_get_ptam_match_result(0.0,0.0,0.0,i_db_link,1,
    "FROM")
   SET pk_where = drmm_get_pk_where(table_name,table_suffix,0,i_db_link)
   SET cid_return = call_ins_dcl(table_name,pk_where,"REFCHG",0,reqinfo->updt_id,
    reqinfo->updt_task,reqinfo->updt_applctx,i_context_name,0.0,0.0,
    i_db_link,pk_where_hash,ptam_match_hash)
   SET pk_where = dat_replace_delim(pk_where)
   DECLARE dm_get_hash_value() = f8
   SELECT INTO "nl:"
    pkw_value = dm_get_hash_value(pk_where,0,1073741824.0)
    FROM dual
    DETAIL
     dm_pkw_value = pkw_value
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   SET dcl_name = concat("DM_CHG_LOG",trim(i_db_link))
   SELECT INTO "nl:"
    d.target_env_id, d.log_id, d.log_type,
    d.context_name, ctx_nullind = nullind(d.context_name)
    FROM (parser(dcl_name) d)
    WHERE d.table_name=table_name
     AND d.delete_ind=0
     AND d.pk_where=pk_where
     AND d.log_type IN ("NORDDS", "REFCHG")
     AND d.pk_where_value=dm_pkw_value
    ORDER BY d.target_env_id
    HEAD REPORT
     dm_total = 0
    DETAIL
     dm_total = (dm_total+ 1), stat = alterlist(dm_log->qual,dm_total), dm_log->qual[dm_total].
     target_env_id = d.target_env_id,
     dm_log->qual[dm_total].log_id = d.log_id, dm_log->qual[dm_total].log_type = d.log_type
     IF (ctx_nullind=1)
      dm_log->qual[dm_total].context_name = "NULL"
     ELSE
      dm_log->qual[dm_total].context_name = d.context_name
     ENDIF
    FOOT REPORT
     dm_log->total = dm_total
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ENDIF
   SELECT INTO noforms
    FROM (dummyt d  WITH seq = value(dm_log->total))
    HEAD REPORT
     stars = fillstring(70,"*"), col 5, stars,
     row + 1, col 8, "TARGET_ENV_ID",
     col 28, "LOG_ID", col 45,
     "LOG_TYPE", col 60, "CONTEXT_NAME",
     row + 1
    DETAIL
     target = format(dm_log->qual[d.seq].target_env_id,"#############.##"), col 5, target,
     log_id = format(dm_log->qual[d.seq].log_id,"#############.##"), col 25, log_id,
     col 45, dm_log->qual[d.seq].log_type, col 60,
     dm_log->qual[d.seq].context_name, row + 1
    FOOT REPORT
     col 5, stars
    WITH nocounter, maxrow = 1, formfeed = none,
     maxcol = 100, format = variable
   ;end select
 END ;Subroutine
 SUBROUTINE dat_delete(i_table_name,i_db_link,i_context_name)
   DECLARE table_suffix = vc
   DECLARE table_name = vc
   DECLARE dcl_name = vc WITH protect, noconstant("")
   DECLARE dm_ptam_result = f8
   ...
   DECLARE dm_total = i4 WITH protect, noconstant(0)
   DECLARE dm_idx = i4 WITH protect, noconstant(0)
   DECLARE dd_str = vc WITH protect, noconstant(" ")
   DECLARE sys_context() = c4000
   SET table_name = cnvtupper(i_table_name)
   SELECT INTO "nl:"
    dd_ctx_str = sys_context("CERNER","RDDS_COL_STRING",4000)
    FROM dual
    DETAIL
     dd_str = dd_ctx_str
    WITH nocounter
   ...
   IF (dd_str <= " ")
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Invalid setup.  COL_STRING context must be set to add a delete row."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN
   ENDIF
   SELECT INTO "nl:"
   ...
   SET dm_ptam_result = drmm_get_ptam_match_result(0.0,0.0,0.0,i_db_link,1,
    "FROM")
   SET pk_where = drmm_get_pk_where(table_name,table_suffix,1,i_db_link)
   SET cid_return = call_ins_dcl(table_name,pk_where,"REFCHG",1,reqinfo->updt_id,
    reqinfo->updt_task,reqinfo->updt_applctx,i_context_name,0.0,0.0,
    i_db_link,pk_where_hash,ptam_match_hash)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ;end select
 END ;Subroutine
 SUBROUTINE dat_replace_delim(drd_string)
   DECLARE drd_start_idx = i4
   DECLARE drd_pos = i4
   DECLARE drd_return = vc
   DECLARE drd_temp_val = vc
   DECLARE drd_concat = vc
   SET drd_return = replace(drd_string,"'",'"',0)
   RETURN(drd_return)
 END ;Subroutine
 IF (check_logfile("dm_add_to_chg_log",".log","dm_add_to_chg_log LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (validate(request->table_name,"-+")="-+")
  ...
  SET request->table_name = trim(cnvtupper( $1))
  SET request->rowid =  $2
  SET request->db_link = " "
  SET request->delete_ind = 0
  SET request->context_name = " "
 ENDIF
 IF ((request->delete_ind=0))
  CALL dat_main(request->table_name,request->rowid,request->db_link,request->context_name)
 ELSE
  CALL dat_delete(request->table_name,request->db_link,request->context_name)
 ENDIF
 COMMIT
#exit_program
END GO
