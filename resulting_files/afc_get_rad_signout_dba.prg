CREATE PROGRAM afc_get_rad_signout:dba
 DECLARE codeset = i4
 ...
 CALL echo("CALL GET MISSING RAD SIGNOUT")
 EXECUTE afc_get_missing_rad_signout
 CALL echo("AFTER GET MISSING RAD SIGNOUT")
 CALL echo(build("ce_signout: ",ce_signout))
 CALL echo(build(size(results->results,5)," events"))
 SET codeset = 13016
 ...
 IF (size(results->results,5) > 0)
  ...
  SELECT INTO "NL:"
   ...
   DETAIL
    ...
    count1].order_mnemonic = results->results[d1.seq].order_mnem, reply->charge_event[count1].
    order_id = results->results[d1.seq].order_id,
    ...
    cea_type_cd = ce_signout,
    reply->charge_event[count1].charge_event_act[1].cea_prsnl_id = results->results[d1.seq].prsnl_id,
    ...
    results[d1.seq].signout_dt_tm,
    reply->charge_event[count1].charge_event_act[1].charge_type_cd = 0, reply->charge_event[count1].
    charge_event_act[1].quantity = 0, reply->charge_event[count1].charge_event_act_qual = 1
   WITH nocounter
  ;end select
 ENDIF
