CREATE PROGRAM bhs_multum_ddi_audit
 PROMPT
 ...
 IF (findstring("@", $OUTDEV) > 0)
  SET email_ind = 1
  SET output_dest = concat(trim(cnvtlower(curprog)))
 ELSE
  SET email_ind = 0
 ...
 ELSEIF (cnvtupper( $BEGDATE) IN ("BEGOFWEEKLY", "BOW"))
  SET beg_date_qual = datetimefind(cnvtdatetime((curdate - 10),0000),"W","B","B")
 ELSE
  SET beg_date_qual = cnvtdatetime(cnvtdate( $BEGDATE),0000)
 ENDIF
 IF (cnvtupper( $ENDDATE) IN ("ENDOFPREVMONTH", "EOM"))
  SET end_date_qual = datetimefind(cnvtdatetime((curdate - 10),235959),"M","E","E")
 ELSEIF (cnvtupper( $ENDDATE) IN ("ENDOFWEEKLY", "EOW"))
  SET end_date_qual = datetimefind(cnvtdatetime((curdate - 10),235959),"W","E","E")
 ELSE
 ...
 RECORD eks(
   1 list[*]
     ...
     2 severity = i4
     2 ml_major_contraindicated_ind = i4
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   row + 1, stat = alterlist(eks->list,10)
  ...
  DETAIL
   ...
   ELSEIF (trim(edea.attr_name)="MAJOR_CONTRAINDICATED_IND")
    eks->list[cnt].ml_major_contraindicated_ind = edea.attr_id
   ENDIF
  FOOT REPORT
 ...
 ;end select
 SET eks_cnt = size(eks->list,5)
 CALL echo(build("eks_count:",eks_cnt))
 ...
 ELSE
  SET num_reports = 1
 ENDIF
 FOR (ii = 1 TO num_reports)
   CALL echo(build("I:",ii))
   IF (validate(request->batch_selection))
    IF (ii=1)
     SET report_type = "1"
    ELSE
     SET report_type = "2"
    ENDIF
    CALL echo(build("Report Type:",report_type))
   ELSE
    ...
    HEAD REPORT
     ...
     IF (report_type="2")
      ...
      '",', row + 1, output_line = build(',"',"Triggering Drug",'","',"Interacting Drug",'","',
       "Facility",'","',"Ordering Physician",'","',"Ordering Physician Position",
       '","',"FMRN",'","',"Order Date",'","',
       "User Position",'","',"Order ID",'","',"Override Reason",
       '","',"Free-text Override Reason",'","',"Severity",'","',
       "Major Contraindicated",'","',"Overridden?",'","',"Triggering Order Detail",
    ...
    DETAIL
     ...
     IF (report_type="2")
      ...
      interact_ord_phys_pos = substring(1,40,uar_get_code_display(eks->list[d.seq].interact.
        physician_position_cd)), trigger_ord_phys_pos = substring(1,40,uar_get_code_display(eks->
      ...
      output_line = build(',"',trim(uar_get_code_display(eks->list[d.seq].trigger.catalog_cd)),'","',
       ...
       facility_disp,'","',eks->list[d.seq].trigger.physician_name,'","',trigger_ord_phys_pos,
       '","',eks->list[d.seq].fmrn,'","',order_date_disp,'","',
       user_position,'","',eks->list[d.seq].trigger.order_id,'","',override_reason,
       '","',eks->list[d.seq].override_rsn_ft,'","',eks->list[d.seq].severity,'","',
       eks->list[d.seq].ml_major_contraindicated_ind,'","',override_yes_no,'","',trgr
