CREATE PROGRAM dm2_aload_adoc_dtp
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_adoc_dtp",".log","dm2_aload_adoc_dtp")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_adoc_dtp"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE datp_cnt = i4 WITH protect, noconstant(0)
 DECLARE datp_str = vc WITH protect, noconstant("")
 DECLARE datp_req_size = i4 WITH protect, noconstant(0)
 DECLARE datp_ndx = i4 WITH protect, noconstant(0)
 FREE RECORD datp_maint
 RECORD datp_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET datp_maint->num_of_inserts = 0
 SET datp_maint->num_of_updates = 0
 SET datp_maint->num_of_none = 0
 SET datp_req_size = size(requestin->list_0,5)
 SET stat = alterlist(datp_maint->qual,datp_req_size)
 IF (dac_pop_coldic_rec("DM_TS_PRECEDENCE")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  SET dm_err->eproc = "Determining maintenance type for DM_TS_PRECEDENCE."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(datp_req_size))
  DETAIL
   datp_maint->qual[d.seq].maint_type = "I", datp_maint->num_of_inserts = (datp_maint->num_of_inserts
   + 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 SELECT INTO "nl:"
  FROM dm_ts_precedence dat,
   (dummyt d  WITH seq = value(datp_req_size))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (dat
   WHERE dat.owner=currdbuser
    AND (dat.table_name=requestin->list_0[d.seq].table_name)
    AND dat.precedence=cnvtint(requestin->list_0[d.seq].precedence))
  ORDER BY dat.table_name
  DETAIL
   IF (cnvtint(requestin->list_0[d.seq].updt_cnt) >= dat.updt_cnt)
    datp_maint->qual[d.seq].maint_type = "U", datp_maint->num_of_updates = (datp_maint->
    num_of_updates+ 1), datp_maint->num_of_inserts = (datp_maint->num_of_inserts - 1)
   ELSE
    datp_maint->qual[d.seq].maint_type = "N", datp_maint->num_of_none = (datp_maint->num_of_none+ 1),
    datp_maint->num_of_inserts = (datp_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(datp_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((datp_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(datp_req_size) )"),0)
  ...
  COMMIT
 ENDIF
 IF ((datp_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where datp_maint->qual[d.seq].maint_type = 'U' ",0)
  ...
  IF (dm2_push_cmd(
   " and cnvtint(requestin->list_0[d.seq].precedence) = dcv.precedence  with nocounter go",1)=0)
   ROLLBACK
   GO TO exit_program
  ENDIF
  IF ((curqual != datp_maint->num_of_updates))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = build("Rows Updated:",curqual,". Does not match expected rows updated:",
    datp_maint->num_of_updates)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_adoc_dtp completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_adoc_dtp")
END GO
