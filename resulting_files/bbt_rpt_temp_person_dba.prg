CREATE PROGRAM bbt_rpt_temp_person:dba
 ...
 SET captions->rpt_title = uar_i18ngetmessage(i18nhandle,"rpt_title",
  "AUTOLOGOUS/DIRECTED PERSONS NOT COMBINED")
 ...
 SET captions->expected = uar_i18ngetmessage(i18nhandle,"expected","EXPECTED")
 ...
 SET captions->usage = uar_i18ngetmessage(i18nhandle,"usage","USAGE")
 SET captions->rpt_id = uar_i18ngetmessage(i18nhandle,"rpt_id","Report ID: BBT_RPT_TEMP_PERSON.PRG")
 ...
 SET stat = uar_get_meaning_by_codeset(alias_type_code_set,nullterm(ssn_alias_cdf_meaning),1,
  ssn_alias_type_cd)
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  pcd_ind = decode(pcd.seq,"Y","N"), ad.active_status_dt_tm";;f", ad.person_id,
  ad.product_id, ad.encntr_id, ad.expected_usage_dt_tm,
  pcd.person_combine_id, per.person_id, per.name_full_formatted,
  per.birth_dt_tm, p.product_id, p.product_nbr,
  p.cur_expire_dt_tm, p.product_nbr, p.product_sub_nbr,
  p_display = uar_get_code_display(p.product_cd), pa.person_alias_id, pa_ind = decode(pa.seq,"Y","N"),
  ...
  PLAN (ad
   ...
   WHERE d_e.seq=1)
   JOIN (e
   WHERE e.person_id=ad.person_id)
   JOIN (d_pcd
   WHERE d_pcd.seq=1)
   JOIN (pcd
   WHERE pcd.entity_name="AUTO_DIRECTED"
   ...
   WHERE pa.person_id=per.person_id
    ...
    AND pa.active_ind=1)
  ORDER BY per.name_full_formatted, per.person_id, p.product_nbr,
   p.product_id
  HEAD REPORT
   product_cnt = 0, select_ok_ind = 0
  HEAD PAGE
   new_page = "Y",
   ...
   col 118, curdate"@DATECONDENSED;;d", row + 2,
   col 104, captions->page_no, col 118,
   curpage"##", inc_i18nhandle = 0, inc_h = uar_i18nlocalizationinit(inc_i18nhandle,curprog,"",
    curcclrev),
   row 0
   ...
   col 1, captions->inventory_area, col 19,
   cur_inv_area_disp, row + 2, line = fillstring(125,"-"),
   line2 = fillstring(125,"-"), row + 1, col 001,
   captions->person_name, col 118, captions->expected,
   row + 1, col 004, captions->ssn,
   col 021, captions->birth_date, col 033,
   captions->product_number, col 061, captions->type,
   col 103, captions->expiration, col 120,
   captions->usage, row + 1, line_ssn = fillstring(18,"-"),
   line_birthdate = fillstring(10,"-"), line_product = fillstring(26,"-"), line_type = fillstring(40,
    "-"),
   line_expire = fillstring(12,"-"), line_usage = fillstring(8,"-"), col 001,
   line_ssn, col 021, line_birthdate,
   col 031, line_product, col 061,
   line_type, col 103, line_expire,
   col 118, line_usage
  HEAD per.person_id
   new_person = "Y", temp_person->person_id = per.person_id, temp_person->name_full_formatted = per
   .name_full_formatted
   IF (pa_ind="Y")
   ...
   ENDIF
   temp_person->birth_dt_tm = per.birth_dt_tm, temp_person->birth_tz = validate(per.birth_tz,0),
   product_cnt = 0,
   stat = alterlist(temp_person->productlist,0), stat = alterlist(temp_person->productlist,5)
  HEAD p.product_id
   IF (pcd_ind != "Y"
    AND e_ind="0.0")
    product_cnt += 1
    ...
    ENDIF
    temp_person->productlist[product_cnt].product_id = p.product_id, temp_person->productlist[
    product_cnt].product_cd = p.product_cd, temp_person->productlist[product_cnt].product_disp =
    p_display,
    temp_person->productlist[product_cnt].cur_expire_dt_tm = p.cur_expire_dt_tm, temp_person->
    productlist[product_cnt].product_nbr = concat(trim(p.product_nbr)," ",trim(p.product_sub_nbr)),
  ...
  FOOT  per.person_id
   IF (product_cnt > 0)
    ...
    ELSE
     row + 1
    ENDIF
    row + 1, col 001, temp_person->name_full_formatted,
    prod = 1
    WHILE (((prod=1) OR (prod <= product_cnt)) )
      ...
      IF (prod=1)
       ...
       IF (curutc=1)
        temp1 = format(datetimezone(temp_person->birth_dt_tm,temp_person->birth_tz),
         "@DATECONDENSED;4;q")
       ELSE
      ...
      ENDIF
      IF (prod <= product_cnt)
       col 032, temp_person->productlist[prod].product_nbr, col 061,
       temp_person->productlist[prod].product_disp, col 103, temp_person->productlist[prod].
       cur_expire_dt_tm"@DATETIMECONDENSED;;d",
  ...
  FOOT PAGE
   ...
   col 60, captions->rpt_page, col 067,
   curpage"###", col 100, captions->printed,
   col 110, curdate"@DATECONDENSED;;d", col 120,
   curtime"@TIMENOSECONDS;;M", col 80, captions->printed_by,
   col 85, reportbyusername"#############"
  FOOT REPORT
   row + 1, col 053, captions->end_of_report,
   report_complete_ind = "Y", select_ok_ind = 1
  WITH nocounter, nullreport, outerjoin(d_e),
   dontcare(e), outerjoin(d_pcd), dontcare(pcd),
   outerjoin(d_pa), dontcare(pa), maxrow = 61,
