CREATE PROGRAM cp_upd_pathway_trail_section
 PROMPT
 ...
 SET stat = cnvtjsontorec( $UPDATES)
 DECLARE main(null) = null
 IF ( NOT (validate(trail_data)))
  RECORD trail_data(
    1 cnt = i4
    1 qual[*]
 ...
 SUBROUTINE main(null)
   ...
   DECLARE mean_cd = f8 WITH noconstant(0), protect
   DECLARE type_cd = f8 WITH noconstant(0), protect
   DECLARE x = i4 WITH noconstant(0), protect
   IF ((trail_data->cnt <= 0))
    GO TO exit_script
   ELSE
    FOR (x = 1 TO trail_data->cnt)
      SET mean_cd = uar_get_code_by("MEANING",4010001,trail_data->qual[x].action_detail_status_mean)
      SET type_cd = uar_get_code_by("MEANING",4003199,trail_data->qual[x].action_detail_type_mean)
      UPDATE  FROM cp_pathway_action_detail cpad
       SET cpad.action_detail_entity_id = cnvtreal(trail_data->qual[x].action_detail_entity_id), cpad
        .action_detail_entity_name = trail_data->qual[x].action_detail_entity_name, cpad
        .action_detail_status_cd = mean_cd,
        cpad.cp_action_detail_type_cd = type_cd, cpad.cp_pathway_action_id = cnvtreal(trail_data->
         qual[x].action_id), cpad.updt_cnt = (cpad.updt_cnt+ 1),
        cpad.updt_dt_tm = cnvtdatetime(sysdate), cpad.updt_id = reqinfo->updt_id
       PLAN (cpad
        WHERE cpad.cp_pathway_action_detail_id=cnvtreal(trail_data->qual[x].action_detail_id))
      ;end update
    ENDFOR
   ENDIF
   CALL log_message(build("Exit main(), Elapsed time in seconds:",((curtime3 - start_curtime3)/ 100.0
     )),log_level_debug)
 END ;Subroutine
 SET reply->status_data.status = "F"
 CALL main(null)
