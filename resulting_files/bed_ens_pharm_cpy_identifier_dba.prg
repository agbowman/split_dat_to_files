CREATE PROGRAM bed_ens_pharm_cpy_identifier:dba
 ...
 FOR (x = 1 TO size(request->items,5))
   ...
   SET stat = alterlist(request_main->items[x].identifiers,size(request->items[x].identifiers,5))
   FOR (y = 1 TO size(request->items[x].identifiers,5))
    SET request_main->items[x].identifiers[y].ident_type_code_value = request->items[x].identifiers[y
    ].ident_type_code_value
    SET request_main->items[x].identifiers[y].copy_from_type = request->items[x].identifiers[y].
    copy_from_type
   ENDFOR
 ENDFOR
 FREE SET request
        ...
        IF ((request->meddefqual[x].meddefflexqual[y].flex_type_cd=system_code_value))
         ...
         FOR (w = 1 TO size(request_main->items[main_i].identifiers,5))
           IF ((request_main->items[main_i].identifiers[w].ident_type_code_value != unique_code_value
            ...
            IF (size(request->meddefqual[x].meddefflexqual[y].medidentifierqual,5) > 0)
             SET copy_from_id = 0
             DECLARE copy_from_value = vc
             SELECT INTO "nl:"
              FROM (dummyt d  WITH seq = size(request->meddefqual[x].meddefflexqual[y].
                medidentifierqual,5))
              PLAN (d
               WHERE (request->meddefqual[x].meddefflexqual[y].medidentifierqual[d.seq].
               med_identifier_type_cd=request_main->items[main_i].identifiers[w].copy_from_type)
                AND (request->meddefqual[x].meddefflexqual[y].medidentifierqual[d.seq].primary_ind=1)
                AND (request->meddefqual[x].meddefflexqual[y].medidentifierqual[d.seq].active_ind=1))
              ORDER BY d.seq
              DETAIL
               copy_from_id = d.seq, copy_from_value = request->meddefqual[x].meddefflexqual[y].
               medidentifierqual[d.seq].value
              WITH nocounter
             ;end select
             IF (copy_from_id > 0)
              SET dup_ind = 0
              ...
              IF (dup_ind=0)
               ...
               short_desc_code_value))
                SELECT INTO "nl:"
                 FROM (dummyt d  WITH seq = size(request->meddefqual[x].ordcat[1].qual_mnemonic,5))
                 DETAIL
                  request->meddefqual[x].ordcat[1].qual_mnemonic[d.seq].mnemonic = copy_from_value
                 WITH nocounter
                ;end select
                SET upd_unique_id = replace(upd_unique_id,"/s",copy_from_value)
                SET upd_short_ind = 1
               ENDIF
               IF ((request_main->items[main_i].identifiers[w].ident_type_code_value=desc_code_value)
               )
                SET upd_unique_id = replace(upd_unique_id,"/d",copy_from_value)
                SET upd_desc_ind = 1
               ENDIF
              ENDIF
              SELECT INTO "nl:"
              ...
              SELECT INTO "nl:"
               ...
               DETAIL
                IF (dup_upd_ind=0)
                 request->meddefqual[x].meddefflexqual[y].medidentifierqual[d.seq].primary_ind = 1,
                 request->meddefqual[x].meddefflexqual[y].medidentifierqual[d.seq].active_ind = 1,
                 request->meddefqual[x].meddefflexqual[y].medidentifierqual[d.seq].db_rec_status = 1,
                 request->meddefqual[x].meddefflexqual[y].medidentifierqual[d.seq].sequence = 1,
                 dup_upd_ind = 1
                ENDIF
               WITH nocounter
              ;end select
              IF (dup_upd_ind=0
               ...
               salable_by_vendor_ind = request->meddefqual[x].meddefflexqual[y].medidentifierqual[
               copy_from_id].salable_by_vendor_ind
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               salable_by_mfr_ind = request->meddefqual[x].meddefflexqual[y].medidentifierqual[
               copy_from_id].salable_by_mfr_ind
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].id_type_cd
                = request->meddefqual[x].meddefflexqual[y].medidentifierqual[copy_from_id].id_type_cd
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               db_rec_status = 3
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               package_type_id = request->meddefqual[x].meddefflexqual[y].medidentifierqual[
               copy_from_id].package_type_id
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               med_def_flex_id = request->meddefqual[x].meddefflexqual[y].medidentifierqual[
               copy_from_id].med_def_flex_id
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               flex_sort_flag = request->meddefqual[x].meddefflexqual[y].medidentifierqual[
               copy_from_id].flex_sort_flag
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               med_identifier_id = 0
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].order_set_id
                = request->meddefqual[x].meddefflexqual[y].medidentifierqual[copy_from_id].
               order_set_id
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].item_id =
               request->meddefqual[x].meddefflexqual[y].medidentifierqual[copy_from_id].item_id
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               med_product_id = request->meddefqual[x].meddefflexqual[y].medidentifierqual[
               copy_from_id].med_product_id
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].sequence = 1
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               pharmacy_type_cd = request->meddefqual[x].meddefflexqual[y].medidentifierqual[
               copy_from_id].pharmacy_type_cd
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               parent_entity_id = request->meddefqual[x].meddefflexqual[y].medidentifierqual[
               copy_from_id].parent_entity_id
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               parent_entity = request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               parent_entity
               ...
               request->meddefqual[x].meddefflexqual[y].medidentifierqual[copy_from_id].value
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].value_key =
               request->meddefqual[x].meddefflexqual[y].medidentifierqual[copy_from_id].value_key
               SET request->meddefqual[x].meddefflexqual[y].medidentifierqual[ident_cnt].
               med_type_flag = request->meddefqual[x].meddefflexqual[y].medidentifierqual[
               copy_from_id].med_type_flag
         ...
         IF (unique_code_value > 0)
          ...
          IF (dup_check_ind=1)
           ...
           WHILE (duplicate_ind=1)
             ...
             IF (duplicate_ind=1)
              SET rx_update_ind = 1
              IF (check_rx_unique_id=cur_unique_id
