CREATE PROGRAM bhs_rpt_chf_prompts:dba
 PROMPT
 ...
 IF (( $TYPE=1))
  SELECT INTO "chf_registry.xls"
   ...
   ORDER BY name, id
   HEAD REPORT
    t_line = concat("CHF Registry for ",t_record->name), col 0, t_line,
    row + 1, t_line = concat("Patient Name",char(9),"Medical Record #",char(9),"Date of Birth",
     char(9),"Last office visit with PCP",char(9),"CHF on Problem List",char(9),
     "IV Meds",char(9),"Spiro and Coreg",char(9),"Beta Blocker Therapy",
     char(9),"ACEI/ARB Prescribed",char(9),"Warfarin for Atrial Fib or Cont",char(9),
     "LV Assessment Date Time",char(9),"LV Assessment",char(9),"LDL Date Time",
     char(9),"LDL Value",char(9),"Creatinine Date Time",char(9),
     "Creatinine Value",char(9),"Last Hospitalization for HF dx",char(9),"ICD/ Biventricular ICD",
     char(9),"Patient Education",char(9),"Smoking Status",char(9),
     "Pneumo-coccal Vaccine",char(9),"Influenza Vaccine",char(9),"Street Address",
     char(9),"City",char(9),"State",char(9),
     "Zip Code",char(9)), col 0,
    t_line, row + 1
   HEAD name
    null
   HEAD id
    ...
    address = trim(concat(t_record->pat_qual[d.seq].street1," ",t_record->pat_qual[d.seq].street2," ",
     ...
     char(9),format(t_record->pat_qual[d.seq].last_visit_dt_tm,"mm/dd/yyyy;;q"),char(9),problem,char(
      9),
     iv_meds,char(9),spiro,char(9),beta_blocker,
     char(9),ace_arb,char(9),warfarin,char(9),
     format(t_record->pat_qual[d.seq].lv_dt_tm,"mm/dd/yyyy;;q"),char(9),t_record->pat_qual[d.seq].
     lv_value,char(9),format(t_record->pat_qual[d.seq].ldl_dt_tm,"mm/dd/yyyy;;q"),
     char(9),t_record->pat_qual[d.seq].ldl_val,char(9),format(t_record->pat_qual[d.seq].
      creatinine_dt_tm,"mm/dd/yyyy;;q"),char(9),
     t_record->pat_qual[d.seq].creatinine_val,char(9),format(t_record->pat_qual[d.seq].hosp_dt_tm,
      "mm/dd/yyyy;;q"),char(9),icd,
     char(9),t_record->pat_qual[d.seq].education,char(9),t_record->pat_qual[d.seq].smoking,char(9),
     format(t_record->pat_qual[d.seq].pneu_vac_dt_tm,"mm/dd/yyyy;;q"),char(9),format(t_record->
      pat_qual[d.seq].flu_vac_dt_tm,"mm/dd/yyyy;;q"),char(9),address,
  ...
  SELECT INTO "chf_registry.xls"
   ...
   HEAD id
    ...
    address = trim(concat(t_record->pat_qual[d.seq].street1," ",t_record->pat_qual[d.seq].street2," ",
     ...
     char(9),format(t_record->pat_qual[d.seq].ldl_dt_tm,"mm/dd/yyyy;;q"),char(9),t_record->pat_qual[d
     .seq].ldl_val,char(9),
