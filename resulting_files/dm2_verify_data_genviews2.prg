CREATE PROGRAM dm2_verify_data_genviews2
 ...
 DECLARE dgvv_col_cnt = i4 WITH protect, noconstant(0)
 DECLARE dgvv_unique_str = vc WITH protect, noconstant(" ")
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating inputs for dm2_verify_data_genviews2"
 ...
 CALL dm2_push_cmd(concat("asis(^SELECT v.",dmvd->tbls[dgvv_cnt].keyid_col_name," AS UNIQUE_ID ^)"),0
  )
 IF ((dmvd->tbls[dgvv_cnt].scui_ind != 1))
  SET dgvv_unique_str = "'<#>'"
  FOR (dgvv_col_cnt = 1 TO dmvd->tbls[dgvv_cnt].col_cnt)
   IF ((dmvd->tbls[dgvv_cnt].cols[dgvv_col_cnt].data_type="DATE"))
    SET dgvv_unique_str = concat(dgvv_unique_str,"||to_char(",dmvd->tbls[dgvv_cnt].cols[dgvv_col_cnt]
     .col_name,",'DD-MON-YYYY HH24:MI:SS')||")
   ELSE
    SET dgvv_unique_str = concat(dgvv_unique_str,"||",dmvd->tbls[dgvv_cnt].cols[dgvv_col_cnt].
     col_name,"||")
   ENDIF
   SET dgvv_unique_str = concat(dgvv_unique_str,"'<#>'")
  ENDFOR
  CALL dm2_push_cmd(concat("asis(^ , ",dgvv_unique_str," as UNIQUE_STR ^)"),0)
 ENDIF
 ...
 CALL dm2_push_cmd(concat("asis(^SELECT ",dmvd->tbls[dgvv_cnt].col_list," FROM ",dgvv_full_tname,"@",
   dmvd->db_link," ",dmvd->tbls[dgvv_cnt].compare_where,"^)"),0)
 CALL dm2_push_cmd("asis(^MINUS^)",0)
 CALL dm2_push_cmd(concat("asis(^SELECT ",dmvd->tbls[dgvv_cnt].col_list," FROM ",dgvv_full_tname,"^)"
   ),0)
 CALL dm2_push_cmd(asis(") V ORDER BY UNIQUE_ID "),0)
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_verify_data_genviews2 completed succesfully."
 ELSE
