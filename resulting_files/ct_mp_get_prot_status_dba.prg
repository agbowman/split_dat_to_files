CREATE PROGRAM ct_mp_get_prot_status:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_prot_status"
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getprotstatus(null) = null WITH protect
 DECLARE stat = i4 WITH protect, noconstant(0)
 SET public::"reply"->status_data.status = "F"
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_status","Invalid JSON Request",
   PUBLIC::"reply")
  GO TO exit_script
 ENDIF
 SET stat = cnvtjsontorec( $JSONREQUEST)
 CALL getprotstatus(null)
 FREE RECORD request
 SET public::"reply"->status_data.status = "S"
 SUBROUTINE getprotstatus(null)
   CALL log_message("In GetProtStatus()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   ...
   IF (protcount > 0)
    ...
    SELECT
     ...
     DETAIL
      ...
      "reply"->prot_list[d.seq]."prot_info"."prot_status_cd" = pm.prot_status_cd,
      public::"reply"->prot_list[d.seq]."prot_info"."prot_status_disp" = uar_get_code_display(pm
       .prot_status_cd)
     WITH nocounter
    ;end select
   ELSE
    CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_status",
     "Did not receive any protocol ID",PUBLIC::"reply")
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit GetProtStatus(), Elapsed time in seconds:",datetimediff(cnvtdatetime(
