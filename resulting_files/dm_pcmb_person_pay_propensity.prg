CREATE PROGRAM dm_pcmb_person_pay_propensity
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "PERSON_PAYMENT_PROPENSITY"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_PERSON_PAY_PROPENSITY"
 ...
 SELECT INTO "nl:"
  frm.*
  FROM person_payment_propensity frm
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count_from].from_id = frm.person_payment_propensity_id, rreclist->
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM person_payment_propensity frm
    ...
    WHERE frm.person_payment_propensity_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "PERSON_PAYMENT_PROPENSITY"
