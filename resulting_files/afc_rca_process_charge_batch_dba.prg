CREATE PROGRAM afc_rca_process_charge_batch:dba
 ...
 SUBROUTINE getcodevalue(code_set,cdf_meaning,option_flag)
   ...
   RETURN(s_code_value)
 END ;Subroutine
 FREE RECORD pendingchargebatchlist
 RECORD pendingchargebatchlist(
   1 chargebatchids[*]
     2 chargebatchid = f8
   1 dohardcommit = i2
 )
 ...
 ENDIF
 DECLARE cntpendingbatch = i4 WITH protect, noconstant(0)
 CALL beginservice("192772.001")
 CALL logmessage("Main","Begining main processing",log_debug)
 SELECT INTO "nl:"
  FROM charge_batch cb,
   charge_batch_detail cbd
  PLAN (cb
   WHERE cb.charge_batch_id > 0.0
    AND cb.status_cd=cs4002322_pending_cd
    AND ((cb.active_ind+ 0)=1))
   JOIN (cbd
   WHERE cbd.charge_batch_id=cb.charge_batch_id
    AND cbd.status_cd=cs4002322_pending_cd
    AND ((cbd.active_ind+ 0)=1)
    AND  NOT ( EXISTS (
   (SELECT
    cbd1.charge_batch_id
    FROM charge_batch_detail cbd1
    WHERE cbd1.charge_batch_id=cb.charge_batch_id
     AND cbd1.updt_dt_tm >= cnvtdatetime(curdate,0)))))
  DETAIL
   cntpendingbatch = (cntpendingbatch+ 1)
   IF (mod(cntpendingbatch,10)=1)
    stat = alterlist(pendingchargebatchlist->chargebatchids,(cntpendingbatch+ 9))
   ENDIF
   pendingchargebatchlist->chargebatchids[cntpendingbatch].chargebatchid = cb.charge_batch_id
  WITH nocounter
 ;end select
 SET stat = alterlist(pendingchargebatchlist->chargebatchids,cntpendingbatch)
 SET pendingchargebatchlist->dohardcommit = true
 IF (size(pendingchargebatchlist->chargebatchids,5) > 0)
  EXECUTE afc_rca_submit_charge_batches  WITH replace("REQUEST",pendingchargebatchlist), replace(
   "REPLY",chrgbatchrep)
 ELSE
  CALL logmessage("Main","No batch charge in pending  state",log_debug)
 ENDIF
