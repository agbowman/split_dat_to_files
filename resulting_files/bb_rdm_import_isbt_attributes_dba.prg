CREATE PROGRAM bb_rdm_import_isbt_attributes:dba
 RECORD requestin(
   1 list_0[*]
     2 standard_display = vc
     2 label_display = vc
     2 attribute_group = vc
 )
 ...
 SUBROUTINE insertreqproc(nrequest,spfmt,ntargetreq,ndeststep,sservice,nreprocess,naction,srefpfmt,
  nseq)
   ...
   ENDCASE
 END ;Subroutine
 DECLARE nnumberofrows = i2
 DECLARE nfailedind = i2
 DECLARE smsg = vc
 DECLARE nskiprow = i2
 DECLARE row_cnt = i2
 DECLARE new_pathnet_seq = f8 WITH protect, noconstant(0.0)
#start_script
 SET sreadme_name = "bb_rdm_import_isbt_attributes"
 CALL logscriptstart(sreadme_name)
 COMMIT
 SET nnumberofrows = size(requestin->list_0,5)
 CALL logmsg("Inserting rows to bb_isbt_attribute table...",(e_dispmsg+ e_logtofile))
 FOR (i = 1 TO nnumberofrows)
    ...
    *
    ...
    WHERE (bia.standard_display=requestin->list_0[i].standard_display)
    DETAIL
     nskiprow = 1
    WITH nocounter
   ;end select
   IF (nskiprow=1)
    SET smsg = build("Attribute->",requestin->list_0[i].standard_display,
     " already exists on the table.  Skipping attribute->",requestin->list_0[i].standard_display)
   ...
   ELSE
    ...
    ;end select
    SET smsg = build("Inserting attribute->",requestin->list_0[i].standard_display)
    CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    INSERT  FROM bb_isbt_attribute
     SET bb_isbt_attribute_id = new_pathnet_seq, standard_display = requestin->list_0[i].
      standard_display, label_display = requestin->list_0[i].label_display,
      attribute_group = requestin->list_0[i].attribute_group, active_ind = 1, active_status_dt_tm =
      cnvtdatetime(curdate,curtime3),
      active_status_prsnl_id = 1, updt_cnt = 0, updt_dt_tm = cnvtdatetime(curdate,curtime3),
      updt_id = 1, updt_task = 1, updt_applctx = 1
     WITH nocounter
    ;end insert
    SET nerrorstatus = error(serrormsg,0)
    IF (nerrorstatus=0)
     CALL logmsg("Row added successfully",(e_dispmsg+ e_logtofile))
    ELSE
     CALL logmsg("Error adding row - script failed",(e_dispmsg+ e_logtofile))
     GO TO script_fail
    ENDIF
   ENDIF
 ENDFOR
 ...
 ;end select
 IF (row_cnt=0)
  CALL logmsg("No rows added to the bb_isbt_attribute table - script failed",(e_dispmsg+ e_logtofile)
   )
  ROLLBACK
  SET smsg = "Execution failed.  Check of successful inserts failed."
...
#script_fail
 ROLLBACK
 SET smsg = build("Execution failed.  Unable to add row for attribute->",requestin->list_0[i].
  standard_display)
 CALL logstatus(smsg,"F")
#exit_script
