CREATE PROGRAM dm2_capture_schema_csv:dba
 ...
 DECLARE dcsc_ttspace = vc WITH protect, noconstant("")
 DECLARE dcsc_itspace = vc WITH protect, noconstant("")
 DECLARE dcsc_ltspace = vc WITH protect, noconstant("")
 DECLARE dcsc_source_rdbms = vc WITH protect, noconstant("")
 DECLARE dcsc_sf_ver = i4 WITH protect, constant(1)
 DECLARE dcsc_cerinstall = vc WITH protect, constant("cer_install")
 DECLARE dcsc_sch_date_str = vc WITH protect
 DECLARE dcsc_sch_date_fmt = f8 WITH protect
 DECLARE dcsc_table_prefix = vc WITH protect
 DECLARE dcsc_sch_date_type = vc WITH protect
 DECLARE dcsc_select_dir = vc WITH protect
 DECLARE dcsc_sf_desc = vc WITH protect
 DECLARE dcsc_loc = vc WITH protect, noconstant("")
 DECLARE dcsc_file_prefix = vc WITH protect, noconstant("")
 DECLARE dcsc_filename = vc WITH protect, noconstant("")
 DECLARE dcsc_cmd = vc WITH protect, noconstant("")
 DECLARE dcsc_str = vc WITH protect, noconstant("")
 DECLARE dcsc_alpha_feature_nbr = i4 WITH protect, noconstant(0)
 DECLARE dcsc_schema_instance = i4 WITH protect, noconstant(0)
 DECLARE dcsc_max_tc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_max_i_cnt = i2 WITH protect, noconstant(0)
 DECLARE dcsc_max_ic_cnt = i2 WITH protect, noconstant(0)
 DECLARE dcsc_max_c_cnt = i2 WITH protect, noconstant(0)
 DECLARE dcsc_max_cc_cnt = i2 WITH protect, noconstant(0)
 DECLARE dcsc_csv_h_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_csv_t_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_csv_tc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_csv_i_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_csv_ic_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_csv_c_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_csv_cc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_csv_sq_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_cursch_t_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_cursch_tc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_cursch_i_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_cursch_ic_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_cursch_c_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_cursch_cc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcsc_cursch_sq_cnt = i4 WITH protect, noconstant(0)
 DECLARE val_write_privs(vwp_dir=vc,vwp_full_dir=vc) = i4
 IF (check_logfile("dm2_capture_sch_csv",".log","DM2_CAPTURE_SCHEMA_CSV LOG FILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Begin dm2_capture_schema_csv"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (cnvtupper(currdbuser) != "CDBA")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat(trim(currdbuser)," is an invalid user.")
  SET dm_err->eproc = "Validating current RDBMS user."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 IF (cnvtupper(currdb) != "ORACLE")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat(trim(currdb)," is an invalid RDBMS.")
  SET dm_err->eproc = "Validating current RDBMS."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 SET dcsc_sch_date_str =  $1
 SET dcsc_sf_desc = cnvtupper(substring(1,30, $2))
 SET dcsc_table_prefix =  $3
 SET dcsc_select_dir = dcsc_cerinstall
 SET dm_err->eproc = "Validating input"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dcsc_sf_desc=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("CSV Schema File Description is invalid.")
  SET dm_err->eproc = "CSV Schema File Description Validation"
  SET dm_err->user_action = concat("Please specify a valid CSV Schema File Description.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating user privileges"
 ...
 IF (val_user_privs(1)=0)
  GO TO exit_script
 ENDIF
 IF (val_write_privs(dcsc_select_dir,dm2_install_schema->cer_install)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_val_sch_date_str(dcsc_sch_date_str)=0)
  GO TO exit_script
 ENDIF
 SET dcsc_source_rdbms = currdb
 SET dcsc_sch_date_fmt = cnvtdate2(dcsc_sch_date_str,"DD-MMM-YYYY")
 SET dcsc_sch_date_type = "dm2a"
 SET dcsc_file_prefix = build(dcsc_sch_date_type,cnvtalphanum(format(dcsc_sch_date_fmt,
    "MM/DD/YYYY;;D")))
 CALL echo("*************************************************")
 CALL echo(build("Schema Date(Parm 1):",dcsc_sch_date_str))
 CALL echo(build("Capture Description (Parm 2):",dcsc_sf_desc))
 CALL echo(build("Table Prefix(Parm 3):",dcsc_table_prefix))
 CALL echo(build("Schema Date Prefix:",dcsc_sch_date_type))
 CALL echo(build("Format for select Directory:",dcsc_select_dir))
 CALL echo(build("Capture Source RDBMS:",dcsc_source_rdbms))
 CALL echo("*************************************************")
 SET dm_err->eproc = "LOAD SNAPSHOT OF DATABASE SCHEMA INTO MEMORY"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm2_install_schema->schema_prefix = dcsc_sch_date_type
 SET dm2_install_schema->file_prefix = cnvtalphanum(format(dcsc_sch_date_fmt,"MM/DD/YYYY;;D"))
 SET dcsc_loc = concat(trim(logical(dcsc_cerinstall)),"/")
 SET dcsc_alpha_feature_nbr = - ((1 * cnvtint(format(cnvtdate2(dm2_install_schema->file_prefix,
    "MMDDYYYY"),"YYYYMMDD;;D"))))
 SET dcsc_schema_instance = abs(dcsc_alpha_feature_nbr)
 EXECUTE dm2_get_cursch value(dcsc_table_prefix)
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 IF (dm2_fill_sch_except("LOCAL")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "POPULATE HEADER CSV FILE WITH HEADER INFO"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dcsc_filename = build(dcsc_loc,dcsc_file_prefix,dcfr_csv_file->qual[1].file_suffix,".csv")
 IF (dm2_findfile(dcsc_filename) > 0)
  SET dcsc_cmd = concat("rm ",dcsc_filename)
  IF (dm2_push_dcl(dcsc_cmd)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SELECT INTO value(dcsc_filename)
  FROM dual
  DETAIL
   dcsc_csv_h_cnt = (dcsc_csv_h_cnt+ 1), dcsc_str = concat("DESCRIPTION",",SOURCE_RDBMS",
    ",ADMIN_LOAD_IND",",SF_VERSION"), col 0,
   dcsc_str, row + 1, dcsc_str = build(concat('"""',trim(check(dcsc_sf_desc," ")),'"""'),",",concat(
     '"""',trim(check(dcsc_source_rdbms," ")),'"""'),",",0,
    ",",build(dcsc_sf_ver)),
   col 0, dcsc_str, row + 1
  WITH nocounter, maxcol = 32000, format = variable,
 ...
 IF ((cur_sch->tbl_cnt <= 0))
  ...
  SET dm_err->eproc = "Validating tables in schema snapshot."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "POPULATE TABLE CSV FILE WITH TABLE INFO"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dcsc_ttspace = "D_TOOLKIT"
 SET dcsc_itspace = "I_TOOLKIT"
 SET dcsc_ltspace = "D_TOOLKIT"
 SELECT INTO "nl:"
  d.seq
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dcsc_filename = build(dcsc_loc,dcsc_file_prefix,dcfr_csv_file->qual[2].file_suffix,".csv")
 IF (dm2_findfile(dcsc_filename) > 0)
  SET dcsc_cmd = concat("rm ",dcsc_filename)
  IF (dm2_push_dcl(dcsc_cmd)=0)
   GO TO exit_script
 ...
 SELECT INTO value(dcsc_filename)
  ...
  HEAD REPORT
   dcsc_str = concat("TABLE_NAME",",TABLESPACE_NAME",",PCT_INCREASE",",PCT_USED",",PCT_FREE",
    ",SCHEMA_DATE",",ALPHA_FEATURE_NBR",",FEATURE_NUMBER",",SCHEMA_INSTANCE"), col 0, dcsc_str,
   row + 1
  DETAIL
   dcsc_csv_t_cnt = (dcsc_csv_t_cnt+ 1), dcsc_str = build(evaluate(cur_sch->tbl[d.seq].tbl_name,"",
     "::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch->tbl[d.seq].tbl_name)," ")),
      '"""')),",","::DM2NULLVALUE::",",",build(cur_sch->tbl[d.seq].pct_increase),
    ",",build(cur_sch->tbl[d.seq].pct_used),",",build(cur_sch->tbl[d.seq].pct_free),",",
    format(cnvtdatetimeutc(cur_sch->tbl[d.seq].schema_date,1),"dd-mmm-yyyy hh:mm:ss;;d"),",",build(
     dcsc_alpha_feature_nbr),",",0,
    ",",build(dcsc_schema_instance)), col 0,
   dcsc_str, row + 1
  WITH nocounter, maxcol = 32000, format = variable,
   formfeed = none, maxrow = 1
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
  DETAIL
   IF ((cur_sch->tbl[d.seq].tbl_col_cnt > dcsc_max_tc_cnt))
    dcsc_max_tc_cnt = cur_sch->tbl[d.seq].tbl_col_cnt
   ENDIF
  WITH nocounter
 ;end select
 IF (dcsc_max_tc_cnt > 0)
  SET dcsc_filename = build(dcsc_loc,dcsc_file_prefix,dcfr_csv_file->qual[3].file_suffix,".csv")
  IF (dm2_findfile(dcsc_filename) > 0)
   SET dcsc_cmd = concat("rm ",dcsc_filename)
   IF (dm2_push_dcl(dcsc_cmd)=0)
    GO TO exit_script
  ...
  SELECT INTO value(dcsc_filename)
   FROM (dummyt d  WITH seq = value(cur_sch->tbl_cnt)),
    (dummyt d2  WITH seq = value(dcsc_max_tc_cnt))
   PLAN (d
    WHERE (cur_sch->tbl[d.seq].capture_ind=1))
    JOIN (d2
    WHERE (d2.seq <= cur_sch->tbl[d.seq].tbl_col_cnt))
   HEAD REPORT
    dcsc_str = concat("TABLE_NAME",",COLUMN_NAME",",COLUMN_SEQ",",DATA_TYPE",",DATA_LENGTH",
     ",NULLABLE",",DATA_DEFAULT",",ALPHA_FEATURE_NBR"), col 0, dcsc_str,
    row + 1
   DETAIL
    dcsc_csv_tc_cnt = (dcsc_csv_tc_cnt+ 1), dcsc_str = build(evaluate(cur_sch->tbl[d.seq].tbl_name,"",
      "::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch->tbl[d.seq].tbl_name)," ")),
       '"""')),",",evaluate(cur_sch->tbl[d.seq].tbl_col[d2.seq].col_name,"","::DM2NULLVALUE::",concat
      ('"""',trim(check(substring(1,30,cur_sch->tbl[d.seq].tbl_col[d2.seq].col_name)," ")),'"""')),
     ",",build(cur_sch->tbl[d.seq].tbl_col[d2.seq].col_seq),
     ",",evaluate(cur_sch->tbl[d.seq].tbl_col[d2.seq].data_type,"","::DM2NULLVALUE::",concat('"""',
       trim(check(substring(1,100,cur_sch->tbl[d.seq].tbl_col[d2.seq].data_type)," ")),'"""')),",",
     build(cur_sch->tbl[d.seq].tbl_col[d2.seq].data_length),",",
     evaluate(cur_sch->tbl[d.seq].tbl_col[d2.seq].nullable,"","::DM2NULLVALUE::",concat('"""',trim(
        check(substring(1,1,cur_sch->tbl[d.seq].tbl_col[d2.seq].nullable)," ")),'"""')),",",evaluate(
      cur_sch->tbl[d.seq].tbl_col[d2.seq].data_default,"","::DM2NULLVALUE::",concat('"""',trim(check(
         substring(1,254,cur_sch->tbl[d.seq].tbl_col[d2.seq].data_default)," ")),'"""')),",",build(
      dcsc_alpha_feature_nbr)), col 0,
    dcsc_str, row + 1
   WITH nocounter, maxcol = 32000, format = variable,
    formfeed = none, maxrow = 1
  ;end select
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
  DETAIL
   IF ((cur_sch->tbl[d.seq].ind_cnt > dcsc_max_i_cnt))
    dcsc_max_i_cnt = cur_sch->tbl[d.seq].ind_cnt
   ENDIF
  WITH nocounter
 ;end select
 IF (dcsc_max_i_cnt > 0)
  SET dcsc_filename = build(dcsc_loc,dcsc_file_prefix,dcfr_csv_file->qual[4].file_suffix,".csv")
  IF (dm2_findfile(dcsc_filename) > 0)
   SET dcsc_cmd = concat("rm ",dcsc_filename)
   IF (dm2_push_dcl(dcsc_cmd)=0)
    GO TO exit_script
  ...
  SELECT INTO value(dcsc_filename)
   FROM (dummyt d  WITH seq = value(cur_sch->tbl_cnt)),
    (dummyt d2  WITH seq = value(dcsc_max_i_cnt))
   PLAN (d
    WHERE (cur_sch->tbl[d.seq].capture_ind=1))
    JOIN (d2
    WHERE (d2.seq <= cur_sch->tbl[d.seq].ind_cnt))
   HEAD REPORT
    dcsc_str = concat("TABLE_NAME",",INDEX_NAME",",FULL_IND_NAME",",PCT_INCREASE",",PCT_FREE",
     ",UNIQUE_IND",",TABLESPACE_NAME",",ALPHA_FEATURE_NBR"), col 0, dcsc_str,
    row + 1
   DETAIL
    dcsc_csv_i_cnt = (dcsc_csv_i_cnt+ 1), dcsc_str = build(evaluate(cur_sch->tbl[d.seq].tbl_name,"",
      "::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch->tbl[d.seq].tbl_name)," ")),
       '"""')),",",evaluate(cur_sch->tbl[d.seq].ind[d2.seq].ind_name,"","::DM2NULLVALUE::",concat(
       '"""',trim(check(substring(1,30,cur_sch->tbl[d.seq].ind[d2.seq].ind_name)," ")),'"""')),",",
     evaluate(cur_sch->tbl[d.seq].ind[d2.seq].full_ind_name,"","::DM2NULLVALUE::",concat('"""',trim(
        check(substring(1,30,cur_sch->tbl[d.seq].ind[d2.seq].full_ind_name)," ")),'"""')),
     ",",build(cur_sch->tbl[d.seq].ind[d2.seq].pct_increase),",",build(cur_sch->tbl[d.seq].ind[d2.seq
      ].pct_free),",",
     build(cur_sch->tbl[d.seq].ind[d2.seq].unique_ind),",","::DM2NULLVALUE::",",",build(
      dcsc_alpha_feature_nbr)), col 0,
    dcsc_str, row + 1
   WITH nocounter, maxcol = 32000, format = variable,
    formfeed = none, maxrow = 1
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d)
    ...
    WHERE (d2.seq <= cur_sch->tbl[d.seq].ind_cnt))
   DETAIL
    IF ((cur_sch->tbl[d.seq].ind[d2.seq].ind_col_cnt > dcsc_max_ic_cnt))
     dcsc_max_ic_cnt = cur_sch->tbl[d.seq].ind[d2.seq].ind_col_cnt
    ENDIF
   WITH nocounter
  ;end select
  IF (dcsc_max_ic_cnt > 0)
   SET dcsc_filename = build(dcsc_loc,dcsc_file_prefix,dcfr_csv_file->qual[5].file_suffix,".csv")
   IF (dm2_findfile(dcsc_filename) > 0)
    SET dcsc_cmd = concat("rm ",dcsc_filename)
    IF (dm2_push_dcl(dcsc_cmd)=0)
     GO TO exit_script
   ...
   SELECT INTO value(dcsc_filename)
    FROM (dummyt d  WITH seq = value(cur_sch->tbl_cnt)),
     (dummyt d2  WITH seq = value(dcsc_max_i_cnt)),
     (dummyt d3  WITH seq = value(dcsc_max_ic_cnt))
    PLAN (d
     WHERE (cur_sch->tbl[d.seq].capture_ind=1))
     JOIN (d2
     WHERE (d2.seq <= cur_sch->tbl[d.seq].ind_cnt))
    ...
    HEAD REPORT
     dcsc_str = concat("INDEX_NAME",",COLUMN_NAME",",TABLE_NAME",",COLUMN_POSITION",
      ",ALPHA_FEATURE_NBR"), col 0, dcsc_str,
     row + 1
    DETAIL
     dcsc_csv_ic_cnt = (dcsc_csv_ic_cnt+ 1), dcsc_str = build(evaluate(cur_sch->tbl[d.seq].ind[d2.seq
       ].ind_name,"","::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch->tbl[d.seq].
           ind[d2.seq].ind_name)," ")),'"""')),",",evaluate(cur_sch->tbl[d.seq].ind[d2.seq].ind_col[
       d3.seq].col_name,"","::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch->tbl[d
           .seq].ind[d2.seq].ind_col[d3.seq].col_name)," ")),'"""')),",",evaluate(cur_sch->tbl[d.seq]
       .tbl_name,"","::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch->tbl[d.seq].
           tbl_name)," ")),'"""')),
      ",",build(cur_sch->tbl[d.seq].ind[d2.seq].ind_col[d3.seq].col_position),",",build(
       dcsc_alpha_feature_nbr)), col 0,
     dcsc_str, row + 1
    WITH nocounter, maxcol = 32000, format = variable,
     formfeed = none, maxrow = 1
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
  DETAIL
   IF ((cur_sch->tbl[d.seq].cons_cnt > dcsc_max_c_cnt))
    dcsc_max_c_cnt = cur_sch->tbl[d.seq].cons_cnt
   ENDIF
  WITH nocounter
 ;end select
 IF (dcsc_max_c_cnt > 0)
  SET dcsc_filename = build(dcsc_loc,dcsc_file_prefix,dcfr_csv_file->qual[6].file_suffix,".csv")
  IF (dm2_findfile(dcsc_filename) > 0)
   SET dcsc_cmd = concat("rm ",dcsc_filename)
   IF (dm2_push_dcl(dcsc_cmd)=0)
    GO TO exit_script
  ...
  SELECT INTO value(dcsc_filename)
   FROM (dummyt d  WITH seq = value(cur_sch->tbl_cnt)),
    (dummyt d2  WITH seq = value(dcsc_max_c_cnt))
   PLAN (d
    WHERE (cur_sch->tbl[d.seq].capture_ind=1))
    JOIN (d2
    WHERE (d2.seq <= cur_sch->tbl[d.seq].cons_cnt))
   HEAD REPORT
    dcsc_str = concat("TABLE_NAME",",CONSTRAINT_NAME",",FULL_CONS_NAME",",CONSTRAINT_TYPE",
     ",STATUS_IND",
     ",R_CONSTRAINT_NAME",",PARENT_TABLE_NAME",",PARENT_TABLE_COLUMNS",",ALPHA_FEATURE_NBR"), col 0,
    dcsc_str,
    row + 1
   DETAIL
    dcsc_csv_c_cnt = (dcsc_csv_c_cnt+ 1), dcsc_str = build(evaluate(cur_sch->tbl[d.seq].tbl_name,"",
      "::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch->tbl[d.seq].tbl_name)," ")),
       '"""')),",",evaluate(cur_sch->tbl[d.seq].cons[d2.seq].cons_name,"","::DM2NULLVALUE::",concat(
       '"""',trim(check(substring(1,30,cur_sch->tbl[d.seq].cons[d2.seq].cons_name)," ")),'"""')),",",
     evaluate(cur_sch->tbl[d.seq].cons[d2.seq].full_cons_name,"","::DM2NULLVALUE::",concat('"""',trim
       (check(substring(1,30,cur_sch->tbl[d.seq].cons[d2.seq].full_cons_name)," ")),'"""')),
     ",",evaluate(cur_sch->tbl[d.seq].cons[d2.seq].cons_type,"","::DM2NULLVALUE::",concat('"""',trim(
        check(substring(1,1,cur_sch->tbl[d.seq].cons[d2.seq].cons_type)," ")),'"""')),",",build(
      cur_sch->tbl[d.seq].cons[d2.seq].status_ind),",",
     evaluate(cur_sch->tbl[d.seq].cons[d2.seq].r_constraint_name,"","::DM2NULLVALUE::",concat('"""',
       trim(check(substring(1,30,cur_sch->tbl[d.seq].cons[d2.seq].r_constraint_name)," ")),'"""')),
     ",",evaluate(cur_sch->tbl[d.seq].cons[d2.seq].parent_table,"","::DM2NULLVALUE::",concat('"""',
       trim(check(substring(1,30,cur_sch->tbl[d.seq].cons[d2.seq].parent_table)," ")),'"""')),",",
     evaluate(cur_sch->tbl[d.seq].cons[d2.seq].parent_table_columns,"","::DM2NULLVALUE::",concat(
       '"""',trim(check(substring(1,255,cur_sch->tbl[d.seq].cons[d2.seq].parent_table_columns)," ")),
       '"""')),
     ",",build(dcsc_alpha_feature_nbr)), col 0,
    dcsc_str, row + 1
   WITH nocounter, maxcol = 32000, format = variable,
    formfeed = none, maxrow = 1
  ;end select
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d)
    ...
    WHERE (d2.seq <= cur_sch->tbl[d.seq].cons_cnt))
   DETAIL
    IF ((cur_sch->tbl[d.seq].cons[d2.seq].cons_col_cnt > dcsc_max_cc_cnt))
     dcsc_max_cc_cnt = cur_sch->tbl[d.seq].cons[d2.seq].cons_col_cnt
    ENDIF
   WITH nocounter
  ;end select
  IF (dcsc_max_cc_cnt > 0)
   SET dcsc_filename = build(dcsc_loc,dcsc_file_prefix,dcfr_csv_file->qual[7].file_suffix,".csv")
   IF (dm2_findfile(dcsc_filename) > 0)
    SET dcsc_cmd = concat("rm ",dcsc_filename)
    IF (dm2_push_dcl(dcsc_cmd)=0)
     GO TO exit_script
   ...
   SELECT INTO value(dcsc_filename)
    FROM (dummyt d  WITH seq = value(cur_sch->tbl_cnt)),
     (dummyt d2  WITH seq = value(dcsc_max_c_cnt)),
     (dummyt d3  WITH seq = value(dcsc_max_cc_cnt))
    PLAN (d
     WHERE (cur_sch->tbl[d.seq].capture_ind=1))
     JOIN (d2
     WHERE (d2.seq <= cur_sch->tbl[d.seq].cons_cnt))
    ...
    HEAD REPORT
     dcsc_str = concat("CONSTRAINT_NAME",",COLUMN_NAME",",TABLE_NAME",",POSITION",
      ",ALPHA_FEATURE_NBR"), col 0, dcsc_str,
     row + 1
    DETAIL
     dcsc_csv_cc_cnt = (dcsc_csv_cc_cnt+ 1), dcsc_str = build(evaluate(cur_sch->tbl[d.seq].cons[d2
       .seq].cons_name,"","::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch->tbl[d
           .seq].cons[d2.seq].cons_name)," ")),'"""')),",",evaluate(cur_sch->tbl[d.seq].cons[d2.seq].
       cons_col[d3.seq].col_name,"","::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch
           ->tbl[d.seq].cons[d2.seq].cons_col[d3.seq].col_name)," ")),'"""')),",",evaluate(cur_sch->
       tbl[d.seq].tbl_name,"","::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch->tbl[
           d.seq].tbl_name)," ")),'"""')),
      ",",build(cur_sch->tbl[d.seq].cons[d2.seq].cons_col[d3.seq].col_position),",",build(
       dcsc_alpha_feature_nbr)), col 0,
     dcsc_str, row + 1
    WITH nocounter, maxcol = 32000, format = variable,
     formfeed = none, maxrow = 1
 ...
 IF ((cur_sch->sequence_cnt > 0))
  SELECT INTO "nl:"
   ...
   WITH nocounter, outerjoin = d, dontexist
  ;end select
  SET dcsc_filename = build(dcsc_loc,dcsc_file_prefix,dcfr_csv_file->qual[8].file_suffix,".csv")
  IF (dm2_findfile(dcsc_filename) > 0)
   SET dcsc_cmd = concat("rm ",dcsc_filename)
   IF (dm2_push_dcl(dcsc_cmd)=0)
    GO TO exit_script
  ...
  SELECT INTO value(dcsc_filename)
   ...
   HEAD REPORT
    dcsc_str = concat("SEQUENCE_NAME",",MIN_VALUE",",MAX_VALUE",",INCREMENT_BY",",CYCLE_FLAG",
     ",LAST_NUMBER",",ALPHA_FEATURE_NBR",",FEATURE_NUMBER",",UPDT_DT_TM"), col 0, dcsc_str,
    row + 1
   DETAIL
    dcsc_csv_sq_cnt = (dcsc_csv_sq_cnt+ 1), dcsc_str = build(evaluate(cur_sch->sequence[d.seq].
      seq_name,"","::DM2NULLVALUE::",concat('"""',trim(check(substring(1,30,cur_sch->sequence[d.seq].
          seq_name)," ")),'"""')),",",build(cur_sch->sequence[d.seq].min_val),",",build(cur_sch->
      sequence[d.seq].max_val),
     ",",build(cur_sch->sequence[d.seq].increment_by),",",evaluate(cur_sch->sequence[d.seq].
      cycle_flag,"","::DM2NULLVALUE::",concat('"""',trim(check(substring(1,1,cur_sch->sequence[d.seq]
          .cycle_flag)," ")),'"""')),",",
     build(cur_sch->sequence[d.seq].last_number),",",0,",",build(cur_sch->sequence[d.seq].
      increment_by),
     ",",format(cnvtdatetime(curdate,curtime3),"dd-mmm-yyyy hh:mm:ss;;d")), col 0,
    dcsc_str, row + 1
   WITH nocounter, maxcol = 32000, format = variable,
    formfeed = none, maxrow = 1
  ;end select
 ...
 IF ((dm_err->debug_flag > 0)
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    WHERE (cur_sch->tbl[d.seq].capture_ind=1))
   DETAIL
    dcsc_cursch_t_cnt = (dcsc_cursch_t_cnt+ 1), dcsc_cursch_tc_cnt = (dcsc_cursch_tc_cnt+ cur_sch->
    tbl[d.seq].tbl_col_cnt), dcsc_cursch_i_cnt = (dcsc_cursch_i_cnt+ cur_sch->tbl[d.seq].ind_cnt)
    FOR (x = 1 TO cur_sch->tbl[d.seq].ind_cnt)
      dcsc_cursch_ic_cnt = (dcsc_cursch_ic_cnt+ cur_sch->tbl[d.seq].ind[x].ind_col_cnt)
    ENDFOR
    dcsc_cursch_c_cnt = (dcsc_cursch_c_cnt+ cur_sch->tbl[d.seq].cons_cnt)
    FOR (x = 1 TO cur_sch->tbl[d.seq].cons_cnt)
      dcsc_cursch_cc_cnt = (dcsc_cursch_cc_cnt+ cur_sch->tbl[d.seq].cons[x].cons_col_cnt)
    ENDFOR
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   ...
   PLAN (d
    WHERE (cur_sch->sequence[d.seq].capture_ind=1))
   DETAIL
    dcsc_cursch_sq_cnt = (dcsc_cursch_sq_cnt+ 1)
   WITH nocounter
  ;end select
  ...
  SELECT INTO value(dm_err->logfile)
   ...
   FROM (dummyt d  WITH seq = value(cur_sch->sequence_cnt))
   PLAN (d
    WHERE (cur_sch->sequence[d.seq].capture_ind=0))
   HEAD REPORT
    row + 2, col 30, "Sequences with capture_ind = 0",
    row + 1
   DETAIL
    row + 1, cur_sch->sequence[d.seq].seq_name
   WITH nocounter, append
  ;end select
  SELECT INTO value(dm_err->logfile)
   d.seq
   ...
   PLAN (d)
   HEAD REPORT
    row + 2, col 18, "Counts of rows in csv schema files vs. Cur_Sch record where capture_ind = 1",
    row + 1, row + 1, "HEADER:",
    col 20, dcsc_csv_h_cnt, row + 1,
    "TABLE:", col 20, dcsc_csv_t_cnt,
    col 35, "Cur_Sch table cnt:", col 70,
    dcsc_cursch_t_cnt, row + 1, "COLUMN:",
    col 20, dcsc_csv_tc_cnt, col 35,
    "Cur_Sch column cnt:", col 70, dcsc_cursch_tc_cnt,
    row + 1, "INDEX:", col 20,
    dcsc_csv_i_cnt, col 35, "Cur_Sch index cnt:",
    col 70, dcsc_cursch_i_cnt, row + 1,
    "INDCOL:", col 20, dcsc_csv_ic_cnt,
    col 35, "Cur_Sch index column cnt:", col 70,
    dcsc_cursch_ic_cnt, row + 1, "CONS:",
    col 20, dcsc_csv_c_cnt, col 35,
    "Cur_Sch constraint cnt:", col 70, dcsc_cursch_c_cnt,
    row + 1, "CONSCOL:", col 20,
    dcsc_csv_cc_cnt, col 35, "Cur_Sch cons column cnt:",
    col 70, dcsc_cursch_cc_cnt, row + 1,
    "SEQ:", col 20, dcsc_csv_sq_cnt,
    col 35, "Cur_Sch sequence cnt:", col 70,
    dcsc_cursch_sq_cnt, row + 2, " "
   WITH nocounter, append
  ;end select
 ENDIF
 GO TO exit_script
 ...
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "CSV FILES CREATED SUCCESSFULLY"
 CALL final_disp_msg("dm2_capture_sch_csv")
END GO
