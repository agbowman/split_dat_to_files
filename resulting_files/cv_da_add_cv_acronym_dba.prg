CREATE PROGRAM cv_da_add_cv_acronym:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_add_cv_acronym_vrsn = vc WITH private, constant("20081029")
 ...
 SET stat = alterlist(id_list->item,size(size(request->objarray,5)))
 ...
 WHILE (i <= size(request->objarray,5))
   SET did = 0.0
   IF (validate(request->objarray[i].cv_acronym_id,- (0.00001)) <= 0.0)
    SET did = id_list->item[i].id
    SET request->objarray[i].cv_acronym_id = did
   ELSE
    SET did = request->objarray[i].cv_acronym_id
   ENDIF
   IF (validate(reply->insert_ids)=1)
    SET reply->insert_ids[i].id = did
   ENDIF
   INSERT  FROM cv_acronym c
    SET c.cv_acronym_id = did, c.provider_id =
