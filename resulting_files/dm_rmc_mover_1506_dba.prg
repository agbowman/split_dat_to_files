CREATE PROGRAM dm_rmc_mover_1506:dba
 ...
 SET cust_req_id = get_value(cust_tab_name,"REQUESTER_ID","FROM")
 IF (cust_pe_name="REQUESTER"
  AND cust_pe_id=cust_req_id)
  SET cust_to_val = select_merge_translate(cust_req_id,cust_tab_name)
  IF (cust_to_val="NOMV21")
   ...
   GO TO exit_1506
  ELSEIF ( NOT (isnumeric(cust_to_val)))
   SET cust_seq_new = get_seq(cust_tab_name,"REQUESTER_ID")
   CALL put_value(cust_tab_name,"REQUESTER_ID",cnvtstring(cust_seq_new))
   CALL put_value(cust_tab_name,"PARENT_ENTITY_ID",cnvtstring(cust_seq_new))
   SET stat = alterlist(dm2_ref_data_reply->qual,1)
   SET dm2_ref_data_reply->qual[1].table_name = cust_tab_name
   SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_req_id)
   SET dm2_ref_data_reply->qual[1].to_value = cust_seq_new
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"REQUESTER_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"REQUESTER_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"REQUESTER_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"REQUESTER_ID")
     ELSE
      GO TO exit_1506
     ENDIF
    ELSE
   ...
   SET stat = alterlist(dm2_ref_data_reply->qual,1)
   SET dm2_ref_data_reply->qual[1].table_name = cust_tab_name
   SET cust_from_cd = get_value(cust_tab_name,"REQUESTER_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1506
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"REQUESTER_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1506
   ELSE
