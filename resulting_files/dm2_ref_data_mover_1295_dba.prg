CREATE PROGRAM dm2_ref_data_mover_1295:dba
 ...
 SUBROUTINE dm2_rdds_get_tgt_id(s_gmti_tgt_rs)
   ...
   RETURN(drgmti_return_val)
 END ;Subroutine
 DECLARE bed_from_cd = vc
 DECLARE bed_new_cd = vc
 DECLARE bed_no_query = i2
 DECLARE bed_val = f8
 DECLARE bed_idcd_check = i2
 DECLARE bed_tab_name = vc
 DECLARE bed_tname_fc = vc
 DECLARE bed_field_value = vc
 DECLARE bed_new_value = vc
 DECLARE bed_status_default = f8 WITH noconstant(0.0)
 DECLARE bed_insert_update(biu_temp_tbl_cnt=i4,biu_perm_col_cnt=i4) = null
 SET dm_err->eproc = "Starting DM2_REF_DATA_MOVER_1295"
 SET bed_status_default = uar_get_code_by("MEANING",291,"AVAILABLE")
 IF (bed_status_default < 0)
  SELECT INTO "nl:"
   c1.code_value
   FROM code_value c,
    code_value_extension c1
   PLAN (c
    WHERE c.code_set=291
     AND c.cdf_meaning="AVAILABLE"
     AND c.active_ind=1)
    JOIN (c1
    WHERE c1.code_set=c.code_set
     AND c1.code_value=c.code_value
     AND c1.field_name="REPAIR AVAILABLE")
   DETAIL
    IF (c1.field_value="1")
     bed_status_default = c1.code_value
    ENDIF
   FOOT REPORT
    IF (bed_status_default < 0)
     bed_status_default = c1.code_value
  ...
  ;end select
  IF (bed_status_default < 0)
   SET dm_err->emsg = "Can not find qualified default value for bed status"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dm2_ref_data_reply->error_ind = 1
  ...
  ENDIF
 ENDIF
 SET bed_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET bed_idcd_check = 0
 CALL echo("")
 CALL echo("")
 ...
 SET bed_idcd_check = is_translated(bed_tab_name,"ALL")
 IF (drdm_error_out_ind=1)
  GO TO exit_1295
 ENDIF
 IF (bed_idcd_check=1)
  SET ins_upd = bed_insert_update(temp_tbl_cnt,perm_col_cnt)
 ENDIF
 SUBROUTINE bed_insert_update(biu_temp_tbl_cnt,biu_perm_col_cnt)
   DECLARE biu_first_where = i2
   DECLARE biu_active_in = i2
   DECLARE biu_col_name = vc
   DECLARE biu_table_name = vc
   DECLARE biu_tab_ind = i2
   DECLARE biu_data_type = vc
   DECLARE biu_no_update_ind = i2
   DECLARE biu_non_key_ind = i2
   DECLARE biu_pk_cnt = i4
   DECLARE biu_parser_cnt = i4
   DECLARE biu_pk_where = vc
   SET biu_pk_where = " where "
   SET biu_table_name = concat("RS_",dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].suffix)
   SET dm_err->eproc = concat("Inserting or Updating Row ",cnvtstring(drdm_chg->log[drdm_log_loop].
     log_id))
   CALL disp_msg("",dm_err->logfile,0)
   SET biu_tab_ind = 0
   SET biu_first_where = 0
   SET biu_no_update_ind = 0
   SET drdm_parser->statement[1].frag = concat("select into 'NL:' from ",value(dm2_ref_data_doc->
     tbl_qual[biu_temp_tbl_cnt].table_name)," dc where ")
   SET biu_parser_cnt = 2
   SET biu_pk_cnt = 0
   FOR (ins_upd_loop = 1 TO biu_perm_col_cnt)
     IF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[ins_upd_loop].pk_ind=1))
      SET biu_pk_cnt = (biu_pk_cnt+ 1)
      SET biu_data_type = dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[ins_upd_loop].
      data_type
      SET biu_col_name = value(dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[ins_upd_loop].
       column_name)
      SET drdm_from_con = concat(biu_table_name,"->To_values.",biu_col_name)
      IF (biu_parser_cnt > 2)
       SET drdm_parser->statement[biu_parser_cnt].frag = " and "
       SET biu_parser_cnt = (biu_parser_cnt+ 1)
       SET biu_pk_where = concat(biu_pk_where," and ")
      ENDIF
      IF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[ins_upd_loop].check_null=1)
       AND (dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[ins_upd_loop].nullable="Y"))
       SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name," = null")
       SET biu_pk_where = concat(biu_pk_where," dc.",biu_col_name," = null")
      ELSEIF (biu_data_type="DQ8")
       SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name,
        " = cnvtdatetime(",drdm_from_con,")")
       SET biu_pk_where = concat(biu_pk_where," dc.",biu_col_name," = cnvtdatetime(",drdm_from_con,
        ")")
      ELSEIF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[ins_upd_loop].check_space=1))
       SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name," = ' '")
       SET biu_pk_where = concat(biu_pk_where," dc.",biu_col_name," = ' '")
      ELSE
       SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name," = ",
        drdm_from_con)
       SET biu_pk_where = concat(biu_pk_where," dc.",biu_col_name," = ",drdm_from_con)
      ENDIF
      SET biu_parser_cnt = (biu_parser_cnt+ 1)
     ELSE
      SET biu_non_key_ind = 1
     ENDIF
   ENDFOR
   IF (biu_pk_cnt=0)
    SET no_delete = 1
    SET dm_err->emsg = "The table has no primary_key information, check to see if it is mergeable."
   ELSE
    SET drdm_parser->statement[biu_parser_cnt].frag = " go"
    CALL parse_statements(biu_parser_cnt)
    IF (curqual > 0)
     IF (new_seq_ind=1
      AND drdm_override_ind=0)
      SET biu_no_update_ind = 1
      CALL merge_audit("FAILREASON",
       "A new sequence was created for the table, but the sequence value already exists in the target table",
       1)
     ELSE
      IF (biu_non_key_ind=1)
       SET drdm_parser->statement[1].frag = concat("update into ",dm2_ref_data_doc->tbl_qual[
        biu_temp_tbl_cnt].table_name,"  dc set ")
       SET biu_parser_cnt = 2
       FOR (update_loop = 1 TO biu_perm_col_cnt)
         IF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[update_loop].pk_ind=0))
          IF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[update_loop].column_name !=
          "BED_STATUS_CD"))
           IF (biu_parser_cnt > 2)
            SET drdm_parser->statement[biu_parser_cnt].frag = ", "
            SET biu_parser_cnt = (biu_parser_cnt+ 1)
           ENDIF
           SET biu_col_name = dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[update_loop].
           column_name
           SET drdm_from_con = concat(biu_table_name,"->to_values.",biu_col_name)
           IF (biu_col_name="ACTIVE_IND")
            IF (drdm_active_ind_merge=0)
             CALL parser(concat("set biu_active_in = ",biu_table_name,"->from_values.active_ind go"),
              1)
             IF (((biu_active_in=0) OR ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[
             update_loop].exception_flg=8))) )
              IF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[update_loop].check_null=1)
               AND (dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[update_loop].nullable="Y"))
               SET drdm_parser->statement[biu_parser_cnt].frag = " dc.ACTIVE_IND = null"
              ELSE
               SET drdm_parser->statement[biu_parser_cnt].frag = " dc.ACTIVE_IND = biu_active_in"
              ENDIF
             ELSE
              IF (((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_cnt - biu_pk_cnt)=1))
               SET biu_no_update_ind = 1
              ELSE
               IF (biu_parser_cnt=2)
                SET biu_parser_cnt = (biu_parser_cnt - 1)
               ELSE
                SET biu_parser_cnt = (biu_parser_cnt - 2)
               ENDIF
              ENDIF
             ENDIF
            ELSE
             IF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[update_loop].check_null=1)
              ...
              SET drdm_parser->statement[biu_parser_cnt].frag = " dc.ACTIVE_IND = null"
             ELSE
              SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.ACTIVE_IND = ",
               drdm_from_con)
             ENDIF
            ENDIF
           ELSEIF (biu_col_name="UPDT_TASK")
            SET drdm_parser->statement[biu_parser_cnt].frag = " dc.UPDT_TASK = 4310001"
           ELSEIF (biu_col_name="UPDT_DT_TM")
            SET drdm_parser->statement[biu_parser_cnt].frag =
            " dc.UPDT_DT_TM = cnvtdatetime(curdate, curtime3)"
           ELSEIF (biu_col_name="UPDT_CNT")
            SET drdm_parser->statement[biu_parser_cnt].frag = "dc.UPDT_CNT = dc.UPDT_CNT + 1"
           ELSE
            IF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[update_loop].check_null=1)
             AND (dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[update_loop].nullable="Y"))
             SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name," = null")
            ELSEIF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[update_loop].data_type=
            "DQ8"))
             SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name,
              " = cnvtdatetime(",drdm_from_con,")")
            ELSEIF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[update_loop].check_space=1
            ))
             SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name," = ' '")
            ELSE
             SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name," = ",
         ...
         ENDIF
       ENDFOR
       IF (biu_no_update_ind=0)
        SET drdm_parser->statement[biu_parser_cnt].frag = biu_pk_where
        SET biu_parser_cnt = (biu_parser_cnt+ 1)
       ENDIF
       SET current_merges = (current_merges+ 1)
       SET child_merge_audit->num[current_merges].action = "UPDATE"
       SET child_merge_audit->num[current_merges].text = dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt]
       .table_name
       SET ins_ind = 0
      ELSE
       SET biu_tab_ind = 1
    ...
    ELSE
     ...
     SET drdm_parser->statement[1].frag = concat("insert into ",dm2_ref_data_doc->tbl_qual[
      biu_temp_tbl_cnt].table_name," dc set ")
     SET biu_parser_cnt = 2
     FOR (insert_loop = 1 TO biu_perm_col_cnt)
       SET biu_col_name = dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[insert_loop].
       column_name
       SET drdm_from_con = concat(biu_table_name,"->to_values.",biu_col_name)
       IF (biu_parser_cnt > 2)
        SET drdm_parser->statement[biu_parser_cnt].frag = ", "
        SET biu_parser_cnt = (biu_parser_cnt+ 1)
       ENDIF
       IF (biu_col_name="UPDT_TASK")
        SET drdm_parser->statement[biu_parser_cnt].frag = " dc.UPDT_TASK = 4310001"
       ELSEIF (biu_col_name="UPDT_DT_TM")
        SET drdm_parser->statement[biu_parser_cnt].frag =
        " dc.UPDT_DT_TM = cnvtdatetime(curdate, curtime3)"
       ELSEIF (biu_col_name="UPDT_CNT")
        SET drdm_parser->statement[biu_parser_cnt].frag = "dc.UPDT_CNT = 0"
       ELSEIF (biu_col_name="BED_STATUS_CD")
        SET drdm_parser->statement[biu_parser_cnt].frag = concat(
         " dc.BED_STATUS_CD = bed_status_default")
       ELSE
        IF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[insert_loop].check_null=1)
         AND (dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[insert_loop].nullable="Y"))
         SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name," = null ")
        ELSEIF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[insert_loop].data_type="DQ8"))
         SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name,
          " = cnvtdatetime(",drdm_from_con,")")
        ELSEIF ((dm2_ref_data_doc->tbl_qual[biu_temp_tbl_cnt].col_qual[insert_loop].check_space=1))
         SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name," = ' '")
        ELSE
         SET drdm_parser->statement[biu_parser_cnt].frag = concat(" dc.",biu_col_name," = ",
          drdm_from_con)
    ...
    SET drdm_parser->statement[biu_parser_cnt].frag = " go"
    IF (biu_tab_ind=0
     AND biu_no_update_ind=0)
     IF (ins_ind=0
      AND biu_non_key_ind=0)
      CALL echo("No update will be done on this table because there are no non-key columns")
     ELSE
      CALL parse_statements(biu_parser_cnt)
     ENDIF
    ENDIF
   ENDIF
   FREE SET biu_first_where
   FREE SET biu_tab_ind
   FREE SET biu_active_in
   FREE SET biu_table_name
 END ;Subroutine
#exit_1295
END GO
