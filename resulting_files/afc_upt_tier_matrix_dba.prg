CREATE PROGRAM afc_upt_tier_matrix:dba
 ...
 CALL upt_tier_matrix(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE upt_tier_matrix(upt_begin,upt_end)
   FOR (x = upt_begin TO upt_end)
     IF ((request->tier_matrix[x].end_effective_dt_tm > cnvtdatetime("31-DEC-2100 00:00:00.00")))
      SET request->tier_matrix[x].end_effective_dt_tm = cnvtdatetime("31-DEC-2100 00:00:00.00")
     ENDIF
     UPDATE  FROM tier_matrix t
      SET t.seq = 1, t.tier_group_cd = nullcheck(t.tier_group_cd,request->tier_matrix[x].
        tier_group_cd,
        IF ((request->tier_matrix[x].tier_group_cd=0)) 0
        ELSE 1
        ENDIF
        ), t.tier_col_num = nullcheck(t.tier_col_num,request->tier_matrix[x].tier_col_num,
        IF ((request->tier_matrix[x].tier_col_num=0)) 0
        ELSE 1
        ENDIF
        ),
       t.tier_row_num = nullcheck(t.tier_row_num,request->tier_matrix[x].tier_row_num,
        IF ((request->tier_matrix[x].tier_row_num=0)) 0
        ELSE 1
        ENDIF
        ), t.tier_cell_type_cd = nullcheck(t.tier_cell_type_cd,request->tier_matrix[x].
        tier_cell_type_cd,
       ...
       ENDIF
       , t.tier_cell_string = nullcheck(t.tier_cell_string,request->tier_matrix[x].tier_cell_string,
        IF (trim(request->tier_matrix[x].tier_cell_string)="") 0
        ELSE 1
        ENDIF
       ...
       ENDIF
       , t.active_status_cd = nullcheck(t.active_status_cd,request->tier_matrix[x].active_status_cd,
        IF ((request->tier_matrix[x].active_status_cd=t.active_status_cd)) 0
        ELSE 1
        ENDIF
        ), t.active_status_prsnl_id = nullcheck(t.active_status_prsnl_id,reqinfo->updt_id,
        IF ((request->tier_matrix[x].active_status_cd=t.active_status_cd)) 0
        ELSE 1
        ENDIF
        ),
       t.active_status_dt_tm = nullcheck(t.active_status_dt_tm,cnvtdatetime(sysdate),
        IF ((request->tier_matrix[x].active_status_cd=t.active_status_cd)) 0
        ELSE 1
        ENDIF
       ...
       updt_task,
       t.tier_cell_entity_name = t.tier_cell_entity_name
