CREATE PROGRAM aps_rdm_move_prompt_tests:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "APS_RDM_MOVE_PROMPT_TESTS"
 ...
 DECLARE lcounter = i4 WITH protect, noconstant(1)
 DECLARE lgennum = i4 WITH protect, noconstant(0)
 DECLARE lgencorrect = i4 WITH protect, noconstant(0)
 DECLARE lindex = i4 WITH protect, noconstant(1)
 DECLARE generatesequencevalues1(lnbrtoadd=i4) = i4
 DECLARE generatesequencevalues2(lnbrtoadd=i4) = i4
 DECLARE lerrorcode = i4 WITH protect, noconstant(0)
 DECLARE serrormessage = vc WITH protect, noconstant(" ")
 DECLARE dactivitysubtype = f8 WITH protect, noconstant(0.0)
 DECLARE dactivestatuscd = f8 WITH protect, noconstant(0.0)
 DECLARE dactivitytype = f8 WITH protect, noconstant(0.0)
 DECLARE dcatalogtypecd = f8 WITH protect, noconstant(0.0)
 DECLARE ncatalogtypeactiveind = i4 WITH protect, noconstant(0)
 DECLARE nactivitytypeactiveind = i4 WITH protect, noconstant(0)
 DECLARE nactivitysubtypeactiveind = i4 WITH protect, noconstant(0)
 CALL logmsg("Retrieving Code Values...",((e_dispmsg+ e_rdmstatus)+ e_logtofile))
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
  ...
  DETAIL
   dcatalogtypecd = cv.code_value, ncatalogtypeactiveind = cv.active_ind
  WITH nocounter
 ;end select
 IF (dcatalogtypecd <= 0)
  SET readme_data->message = "Code Value, 6000, GENERAL LAB"
  GO TO exit_script
 ELSEIF (ncatalogtypeactiveind=0)
 ...
 SELECT INTO "nl:"
  ...
  WHERE cv.code_set=106
   ...
   AND cnvtdatetime(curdate,curtime3) BETWEEN cv.begin_effective_dt_tm AND cv.end_effective_dt_tm
  DETAIL
   dactivitytype = cv.code_value, nactivitytypeactiveind = cv.active_ind
  WITH nocounter
 ;end select
 IF (dactivitytype <= 0)
  SET readme_data->message = "Code Value, 106, AP"
  GO TO exit_script
 ELSEIF (nactivitytypeactiveind=0)
  SET readme_data->status = "S"
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  ...
  WHERE cv.code_set=5801
   ...
   AND cnvtdatetime(curdate,curtime3) BETWEEN cv.begin_effective_dt_tm AND cv.end_effective_dt_tm
  DETAIL
   dactivitysubtype = cv.code_value, nactivitysubtypeactiveind = cv.active_ind
  WITH nocounter
 ;end select
 IF (dactivitysubtype <= 0)
  SET readme_data->message = "Code Value, 5801, APSPECIMEN"
  GO TO exit_script
 ELSEIF (nactivitysubtypeactiveind=0)
  SET readme_data->status = "S"
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 IF (dactivestatuscd <= 0)
  SET readme_data->message = "Code Value, 48, ACTIVE"
  GO TO exit_script
 ENDIF
 CALL logmsg("Retrieving prompts...",((e_dispmsg+ e_rdmstatus)+ e_logtofile))
 SELECT INTO "nl:"
  FROM order_catalog oc,
   profile_task_r ptr,
   long_text lt
  PLAN (oc
   WHERE oc.activity_type_cd=dactivitytype
    AND oc.activity_subtype_cd=dactivitysubtype
    AND oc.active_ind=1
   ...
   JOIN (lt
   WHERE ptr.prompt_long_text_id=lt.long_text_id
    AND lt.long_text_id != 0
    AND  NOT ( EXISTS (
   (SELECT
    ap.task_assay_cd
    FROM ap_prompt ap
    WHERE ap.task_assay_cd=ptr.task_assay_cd
     AND ap.catalog_cd=ptr.catalog_cd))))
  DETAIL
   lcnt = (lcnt+ 1)
   IF (mod(lcnt,10)=1)
    stat = alterlist(prompttest->prompt_test_qual,(lcnt+ 9))
   ENDIF
   prompttest->prompt_test_qual[lcnt].task_assay_cd = ptr.task_assay_cd, prompttest->
   prompt_test_qual[lcnt].specimen_catalog_cd = ptr.catalog_cd, prompttest->prompt_test_qual[lcnt].
   text = lt.long_text
  WITH nocounter
 ;end select
 SET stat = alterlist(prompttest->prompt_test_qual,lcnt)
 IF (curqual=0)
  CALL logmsg(
   "ReadMe has previously been executed or there are no prompts to move. Exiting ReadMe with Success.",
   ((e_dispmsg+ e_rdmstatus)+ e_logtofile))
  SET readme_data->status = "S"
  GO TO exit_script
 ENDIF
 CALL logmsg("Generating Sequence Values...",((e_dispmsg+ e_rdmstatus)+ e_logtofile))
 SET lgennum = generatesequencevalues1(lcnt)
 IF (lgennum != lcnt)
  CALL logmsg("Generate Sequence Values1 Error. Readme Failed.",((e_dispmsg+ e_rdmstatus)+
   e_logtofile))
  SET readme_data->message = "Generate Sequence Values1 Error"
  GO TO exit_script
 ENDIF
 CALL logmsg("Generating Sequence Values...",((e_dispmsg+ e_rdmstatus)+ e_logtofile))
 SET lgennum = generatesequencevalues2(lcnt)
 IF (lgennum != lcnt)
  CALL logmsg("Generate Sequence Values2 Error. Readme Failed.",((e_dispmsg+ e_rdmstatus)+
   e_logtofile))
  SET readme_data->message = "Generate Sequence Values2 Error"
  GO TO exit_script
 ENDIF
 CALL logmsg("Updating long_text_reference table...",((e_dispmsg+ e_rdmstatus)+ e_logtofile))
 INSERT  FROM long_text_reference ltr,
   (dummyt d  WITH seq = value(lcnt))
  SET ltr.parent_entity_id = prompttest->prompt_test_qual[d.seq].prompt_id, ltr.long_text_id =
   prompttest->prompt_test_qual[d.seq].long_text_id, ltr.parent_entity_name = "AP_PROMPT",
   ltr.long_text = prompttest->prompt_test_qual[d.seq].text, ltr.updt_cnt = 0, ltr.updt_id = reqinfo
   ->updt_id,
   ltr.updt_task = reqinfo->updt_task, ltr.updt_applctx = reqinfo->updt_applctx, ltr.active_ind = 1,
   ltr.active_status_cd = dactivestatuscd, ltr.active_status_prsnl_id = reqinfo->updt_id
  PLAN (d)
   JOIN (ltr)
  WITH nocounter
 ;end insert
 ...
 IF (lerrorcode != 0)
  CALL logmsg("Insert into long_text_reference failed.",((e_dispmsg+ e_rdmstatus)+ e_logtofile))
  SET readme_data->message = "Insert into long_text_reference failed"
  GO TO exit_script
 ENDIF
 CALL logmsg("Updating ap_prompt table...",((e_dispmsg+ e_rdmstatus)+ e_logtofile))
 INSERT  FROM ap_prompt ap,
   (dummyt d  WITH seq = value(lcnt))
  SET ap.ap_prompt_id = prompttest->prompt_test_qual[d.seq].prompt_id, ap.task_assay_cd = prompttest
   ->prompt_test_qual[d.seq].task_assay_cd, ap.catalog_cd = prompttest->prompt_test_qual[d.seq].
   specimen_catalog_cd,
   ap.long_text_id = prompttest->prompt_test_qual[d.seq].long_text_id, ap.action_flag = 0, ap
   .updt_cnt = 0,
 ...
 SET lerrorcode = error(serrormessage,0)
 IF (lerrorcode != 0)
  CALL logmsg("Insert into ap_prompt failed.",((e_dispmsg+ e_rdmstatus)+ e_logtofile))
  SET readme_data->message = "Insert into ap_prompt failed"
  GO TO exit_script
 ENDIF
 FREE SET prompttest
 SET lerrorcode = error(serrormessage,0)
 IF (lerrorcode != 0)
  ROLLBACK
  CALL logmsg("Error executing readme.",((e_rdmstatus+ e_dispmsg)+ e_logtofile))
 ...
 SET readme_data->status = "S"
 GO TO exit_script
 SUBROUTINE generatesequencevalues1(lnbrtoadd)
   RECORD m_dm2_seq_stat(
     1 n_status = i4
     1 s_error_msg = vc
   )
   EXECUTE dm2_install_get_bulk_seq "promptTest->prompt_test_qual", lnbrtoadd, "prompt_id",
   1, "reference_seq"
   ...
   RETURN(lnbrtoadd)
 END ;Subroutine
 SUBROUTINE generatesequencevalues2(lnbrtoadd)
   RECORD m_dm2_seq_stat(
     1 n_status = i4
     1 s_error_msg = vc
   )
   EXECUTE dm2_install_get_bulk_seq "promptTest->prompt_test_qual", lnbrtoadd, "long_text_id",
   1, "long_data_seq"
   IF ((m_dm2_seq_stat->n_status=0))
    SET lnbrtoadd = 0
   ENDIF
...
#exit_script
 ...
 ELSE
  ROLLBACK
  CALL logstatus("Execution failed.  ReadMe unsuccessful.","F")
  COMMIT
 ENDIF
 EXECUTE dm_readme_status
 CALL echorecord(readme_data)
