CREATE PROGRAM bhs_rpt_enc_aud_acct_day:dba
 PROMPT
 ...
 DECLARE ms_loc_dir = vc WITH protect, constant(build(logical("bhscust"),
   "/ftp/bhs_rpt_enc_aud_acct_day/"))
 DECLARE ms_filename = vc WITH protect, constant(build(ms_loc_dir,"cisencounter_accounts.dat"))
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 CALL echo(build2("cnt: ",size(m_rec->accts,5)))
...
#exit_script
 IF (size(m_rec->accts,5)=0)
  IF (size(m_rec->accts,5)=0)
   SET dclcom = concat('"no data" | mailx -s "NO DATA FOUND - Enc Aud Acct Daily ',trim(format(
      sysdate,"dd-mmm-yyyy hh:mm;;d")),'" ',"ciscore@bhs.org, justin.choiniere@baystatehealth.org")
