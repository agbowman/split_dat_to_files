CREATE PROGRAM dm_prucb_cdi_rule_criteria:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "dm_prucb_cdi_rule_criteria"
  ...
  GO TO exit_sub
 ENDIF
 IF ((rchildren->qual1[det_cnt].attribute_name="VALUE_ENTITY_ID"))
 ...
 SUBROUTINE entity_ucb_upt(dummy)
   UPDATE  FROM cdi_rule_criteria
    SET updt_id = reqinfo->updt_id, updt_dt_tm = cnvtdatetime(trans_dt_tm), updt_applctx = reqinfo->
     updt_applctx,
     updt_cnt = (updt_cnt+ 1), updt_task = reqinfo->updt_task, value_entity_id = request->
     xxx_uncombine[ucb_cnt].to_xxx_id
    WHERE (cdi_rule_criteria_id=rchildren->qual1[det_cnt].entity_id)
    WITH nocounter
   ;end update
   IF (curqual=0)
