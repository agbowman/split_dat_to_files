CREATE PROGRAM acm_chg_prsnl_org_reltn_type:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->prsnl_org_reltn_type_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
  ...
  SET p.prsnl_org_reltn_type_id = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].
   prsnl_org_reltn_type_id, p.beg_effective_dt_tm = cnvtdatetime(acm_request->
    prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].beg_effective_dt_tm), p.contributor_system_cd =
   IF ((acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].
    end_effective_dt_tm), p.organization_id = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq]
   .idx].organization_id, p.org_type_cd = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx
   ].org_type_cd,
   p.position_cd = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].position_cd, p
   .prsnl_id = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].prsnl_id, p
   .prsnl_org_reltn_type_cd = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].
   prsnl_org_reltn_type_cd,
   p.role_profile = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].role_profile, p
   .job_role_cd = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].job_role_cd, p
   .access_position_cd = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].
   access_position_cd,
   p.active_ind = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].active_ind, p
   .active_status_cd = acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].active_status_cd,
   p.active_status_prsnl_id = active_status_prsnl_id,
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.prsnl_org_reltn_type_id=acm_request->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].
   prsnl_org_reltn_type_id))
  WITH nocounter, status(reply->prsnl_org_reltn_type_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->prsnl_org_reltn_type_qual[xref->chg[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     ENDIF
     chg_str = acm_request->prsnl_org_reltn_type_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].contributor_system_cd = p.contributor_system_cd
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("ORGANIZATION_ID,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].organization_id = p.organization_id
     ENDIF
     IF (findstring("ORG_TYPE_CD,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].org_type_cd = p.org_type_cd
     ENDIF
     IF (findstring("POSITION_CD,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].position_cd = p.position_cd
     ENDIF
     IF (findstring("PRSNL_ID,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].prsnl_id = p.prsnl_id
     ENDIF
     IF (findstring("PRSNL_ORG_RELTN_TYPE_CD,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].prsnl_org_reltn_type_cd = p.prsnl_org_reltn_type_cd
     ENDIF
     IF (findstring("ROLE_PROFILE,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].role_profile = p.role_profile
     ENDIF
     IF (findstring("JOB_ROLE_CD,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].job_role_cd = p.job_role_cd
     ENDIF
     IF (findstring("ACCESS_POSITION_CD,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].access_position_cd = p.access_position_cd
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->prsnl_org_reltn_type_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_org_reltn_type_qual[xref->chg[index].idx].status=0))
