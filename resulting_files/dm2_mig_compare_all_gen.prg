CREATE PROGRAM dm2_mig_compare_all_gen
 ...
 IF (check_logfile("dm2_mig_compare_allg",".log","dm2_mig_compare_all_gen LOGFILE")=0)
  GO TO exit_script
 ENDIF
 ...
 ELSEIF ((dmr_user_list->cnt=0))
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Creating ccluserdir:dm2migcmpall.dat"
 CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
 SELECT INTO "ccluserdir:dm2migcmpall.dat"
  FROM dba_tab_columns dtc,
   dba_ind_columns dic
  WHERE dtc.table_name=dic.table_name
   AND dtc.column_name=dic.column_name
   ...
   AND dtc.data_type="DATE"
   AND dic.column_position=1
   AND ((dtc.column_name="*UPDT*DT_TM") OR (((dtc.column_name="*UPDATE*DT_TM") OR (((dtc.column_name=
  "*STATUS*DT_TM") OR (((dtc.column_name="*ACTIVITY*DT_TM") OR (dtc.column_name="LAST*DT_TM")) )) ))
  ))
   AND list(dic.table_name,dic.table_owner) IN (
  (SELECT
   dic2.table_name, dic2.table_owner
   FROM dba_ind_columns dic2,
    dba_indexes di
   WHERE dic2.index_name=di.index_name
   ...
   (SELECT
    1
    FROM dba_tab_columns dtc2
    WHERE dtc2.column_name=dic2.column_name
     AND dtc2.table_name=dic2.table_name
     AND trim(dtc2.owner)=dic2.table_owner
     AND  NOT (dtc2.data_type IN ("CHAR", "DATE", "FLOAT", "NUMBER", "VARCHAR2")))))))
  ORDER BY dtc.owner, dtc.table_name, dic.column_name DESC,
   dic.column_position
  HEAD REPORT
   row_ind = 0, tmp_col_name = "                              ", output_string = fillstring(60," ")
  HEAD dtc.owner
   row + 0
  HEAD dtc.table_name
   IF (row_ind=0)
   ...
   tmp_col_name = "x"
  HEAD dic.index_name
   IF (tmp_col_name != "*UPDT*DT_TM")
    tmp_col_name = dtc.column_name
   ENDIF
  FOOT  dtc.table_name
   output_string = concat(trim(dtc.owner),",",trim(dtc.table_name),",",trim(tmp_col_name)),
   output_string
  WITH nocounter, noheading, formfeed = none,
   maxrow = 1
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  GO TO exit_script
 ENDIF
 IF (curqual=0)
  SET dm_err->emsg = "Error retrieving qualifying tables"
  SET dm_err->err_ind = 1
  GO TO exit_script
 ENDIF
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_compare_all_gen completed successfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 CALL final_disp_msg("dm2_mig_compare_allg")
END GO
