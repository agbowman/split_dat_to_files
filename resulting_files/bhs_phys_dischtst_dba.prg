CREATE PROGRAM bhs_phys_dischtst:dba
 ...
 FOR (pds = 1 TO patient_cnt)
   ...
   SET home_ph_cd = uar_get_code_by("MEANING",43,"HOME")
   SET home_add_cd = uar_get_code_by("MEANING",212,"HOME")
   SET attenddoc = uar_get_code_by("MEANING",333,"ATTENDDOC")
   SET referdoc = uar_get_code_by("MEANING",333,"REFERDOC")
   SET pcpdoc = uar_get_code_by("MEANING",331,"PCP")
   SET dischtype = uar_get_code_by("displaykey",72,"DISCHARGETRANSFERNOTEHOSPITAL")
   FREE RECORD pat
   RECORD pat(
     1 person_id = f8
     1 event_qual = i4
   ...
   FOR (agc = 1 TO pat->event_qual)
     SELECT INTO value(printer_name)
      ...
      admit_dt = format(e.reg_dt_tm,"mm/dd/yy;;d"),
      admitdoc = substring(1,30,pl.name_full_formatted), attenddoc = substring(1,30,pl2
       .name_full_formatted), referdoc = substring(1,30,pl3.name_full_formatted),
      pcpdoc = substring(1,30,pl4.name_full_formatted), location = substring(1,20,
       uar_get_code_display(e.location_cd)), facility = uar_get_code_description(e.loc_facility_cd),
      ...
      age = build(trim(cnvtage(cnvtdate(p.birth_dt_tm),curdate))), sex = build(trim(
        ...
        .event_cd)),
      event_title_text = substring(1,110,ce.event_title_text), event_id = ce.event_id, event_tag =
      substring(1,50,ce.event_tag),
      parent_event_id = ce.parent_event_id, event_date = substring(1,20,format(ce.event_end_dt_tm,
        "mm/dd/yyyy hh:mm;;d")), author = substring(1,40,pl5.name_full_formatted),
      ...
      phone_nbr = ph.phone_num, add_street1 = a.street_addr, add_street2 = a.street_addr2,
      add_city = a.city, add_state = a.state, add_zip = a.zipcode
      FROM clinical_event ce,
       ce_blob ceb,
       ce_event_prsnl cep,
       prsnl pl5,
      ...
      PLAN (ce
       ...
       JOIN (pl5
       WHERE outerjoin(cep.action_prsnl_id)=pl5.person_id
        AND pl5.active_ind=outerjoin(1))
       JOIN (e
       ...
       JOIN (epr2
       WHERE outerjoin(e.encntr_id)=epr2.encntr_id
        AND epr2.encntr_prsnl_r_cd=outerjoin(attenddoc))
       JOIN (pl2
       WHERE outerjoin(epr2.prsnl_person_id)=pl2.person_id)
       JOIN (epr3
       WHERE outerjoin(e.encntr_id)=epr3.encntr_id
        AND epr3.encntr_prsnl_r_cd=outerjoin(referdoc))
       JOIN (pl3
       WHERE outerjoin(epr3.prsnl_person_id)=pl3.person_id)
       JOIN (ppr
       WHERE outerjoin(p.person_id)=ppr.person_id
        AND ppr.person_prsnl_r_cd=outerjoin(pcpdoc))
       JOIN (pl4
       WHERE outerjoin(ppr.prsnl_person_id)=pl4.person_id)
       JOIN (a
       ...
       WHERE outerjoin(p.person_id)=ph.parent_entity_id
        ...
        AND ph.phone_type_cd=outerjoin(home_ph_cd)
      ...
      ORDER BY ce.event_end_dt_tm, ceb.event_id, epr.active_status_dt_tm DESC,
       epr2.active_status_dt_tm DESC, epr3.active_status_dt_tm DESC, ppr.active_status_dt_tm DESC
      HEAD REPORT
       ...
       addr_line = fillstring(80," "), addr_line2 = fillstring(80," "), phone_line = fillstring(30,
        " "),
       ycol = 100, xcol = 55,
       CALL print(calcpos(xcol,ycol)),
       "{f/8}{cpi/10}", ycol = (ycol+ 15)
       IF (add_street2 > " ")
        addr_line = concat(trim(add_street1),char(44),char(32),char(32),trim(add_street2)),
        addr_line2 = concat(trim(add_city),char(44),char(32),char(32),trim(add_state),
         char(44),char(32),char(32),trim(add_zip))
       ELSE
        addr_line = concat(trim(add_street1)), addr_line2 = concat(trim(add_city),char(44),char(32),
         char(32),trim(add_state),
         char(44),char(32),char(32),trim(add_zip))
       ENDIF
       phone_line = phone_nbr,
       CALL print(calcpos(xcol,ycol)), "Admitting Physician:",
       xcol = 200,
       CALL print(calcpos(xcol,ycol)), "{b}",
       admitdoc, "{endb}", row + 1,
       ycol = (ycol+ 15), xcol = 55,
       CALL print(calcpos(xcol,ycol)),
       "Attending Physician:", xcol = 200,
       CALL print(calcpos(xcol,ycol)),
       "{b}", attenddoc, "{endb}",
       row + 1, xcol = 55, ycol = (ycol+ 15),
       CALL print(calcpos(xcol,ycol)), "Referring Physician:", xcol = 200,
       CALL print(calcpos(xcol,ycol)), "{b}", referdoc,
       "{endb}", row + 1, xcol = 55,
       ycol = (ycol+ 15),
       CALL print(calcpos(xcol,ycol)), "Primary Care Physician:",
       xcol = 200,
       CALL print(calcpos(xcol,ycol)), "{b}",
       pcpdoc, "{endb}", row + 1,
       xcol = 55, ycol = (ycol+ 15),
       CALL print(calcpos(xcol,ycol)),
       "Patient Address:", xcol = 200,
       CALL print(calcpos(xcol,ycol)),
       "{b}", addr_line, "{endb}",
       row + 1, ycol = (ycol+ 15),
       CALL print(calcpos(xcol,ycol)),
       ...
       CALL print(calcpos(xcol,ycol)), "Patient Phone:", xcol = 200,
       CALL print(calcpos(xcol,ycol)), "{b}", phone_line,
       "{endb}", row + 1, xcol = 55,
       ycol = (ycol+ 15),
      ...
      FOOT PAGE
       ...
       "{cpi/10}{pos/55/708}{b}", "Physician Discharge Summary", "{f/8}{cpi/15}{lpi/10}{pos/55/720}",
       "BHS_PHYS_DISCH", "{f/4}{cpi/12}{pos/55/732}{b/8}Printed: ", curdate,
       "  ", curtime, "{endb}",
       row + 1
      FOOT REPORT
       "{f/4}{cpi/12}{pos/239/674}", "{b}", "****  END OF REPORT  ****",
       "{endb}"
