CREATE PROGRAM bhs_rx_easyscripttest:dba
 FREE RECORD request
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
 ...
 SET request->person_id = 1820606.00
 SET request->print_prsnl_id = 0
 SET request->order_qual[1].order_id = 578584603.00
 SET request->order_qual[1].encntr_id = 49698475.00
 SET request->printer_name = "bmc361south2x"
 FREE RECORD reply
 RECORD reply(
   1 status_data
     2 status = c1
 ...
 DECLARE ezscriptphone_var = f8 WITH constant(uar_get_code_by("DISPLAYKEY",43,"EZSCRIPTPHONE")),
 protect
 DECLARE ezscriptaddress_var = f8 WITH constant(uar_get_code_by("DISPLAYKEY",212,"EZSCRIPTADDRESS")),
 protect
 DECLARE provider_id = f8 WITH public, noconstant
 ...
 SET position_cd1 = uar_get_code_by("displaykey",88,"BHSASSOCIATEPROFESSIONAL")
 SET position_cd2 = uar_get_code_by("displaykey",88,"BHSPCOASSOCIATEPROFESSIONAL")
 SET position_cd3 = uar_get_code_by("displaykey",88,"BHSMIDWIFE")
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   WHERE (o.order_id=request->order_qual[d.seq].order_id)
    ...
    AND ((o.projected_stop_dt_tm = null) OR (o.projected_stop_dt_tm > cnvtlookahead("5,MIN")))
  ...
  HEAD o.order_id
   ...
   temp_req->qual[knt].oe_format_id = o.oe_format_id, temp_req->qual[knt].eprsnl_id = oa
   .action_personnel_id, temp_req->qual[knt].ord_det_disp = o.order_detail_display_line
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   .street_addr4, temp_req->qual[d.seq].phys_city = concat(trim(a.city),", ",trim(
     uar_get_code_display(a.state_cd)),"  ",trim(a.zipcode)),
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   replace(ph_phone,"-","",0),
   cnt = (cnt+ 1), stat = alterlist(phy_phone_list->qual,cnt), phy_phone_list->qual[cnt].phone =
   ph_phone
  FOOT REPORT
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     IF (odflag=true)
      ...
      ELSEIF (od.oe_field_meaning="REQREFILLDATE")
       temp_req->qual[x].req_refill_date = od.oe_field_dt_tm_value
      ELSEIF (trim(od.oe_field_meaning)=trim("REQSTARTDTTM"))
       temp_req->qual[x].req_start_date = od.oe_field_dt_tm_value
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (new_job=true)
    ...
    jknt].org_name = temp_req->qual[d.seq].org_name, tprint_req->job[jknt].location_cd = temp_req->
    qual[d.seq].location_cd,
    tprint_req->job[jknt].loc_addr = temp_req->qual[d.seq].loc_addr, tprint_req->job[jknt].loc_addr2
     = temp_req->qual[d.seq].loc_addr2, tprint_req->job[jknt].loc_city = temp_req->qual[d.seq].
    loc_city,
    tprint_req->job[jknt].loc_state = temp_req->qual[d.seq].loc_state, tprint_req->job[jknt].loc_zip
     = temp_req->qual[d.seq].loc_zip, tprint_req->job[jknt].loc_ph = temp_req->qual[d.seq].loc_ph,
    tprint_req->job[jknt].eprsnl_dea = temp_req->qual[d.seq].eprsnl_dea
 ...
 FOR (i = 1 TO tprint_req->job_knt)
   FOR (ii = 1 TO tprint_req->job[i].req_knt)
     ...
     IF ((tprint_req->job[i].output_dest_cd < 1))
      IF (trim(tprint_req->job[i].print_loc) IN ("bmc34rxpain1", "bmc34rxpain2", "bmc361rxtst1"))
       SELECT INTO value(tprint_req->job[i].print_loc)
        ...
        HEAD REPORT
         ...
         sig_1st = 1, assoc_1 = concat("{b}","Supervising Physician","{endb}"), txt_line1 =
         "INTERCHANGE MANDATED UNLESS PRACTITIONER WRITES THE WORDS",
         txt_line2 = '"NO SUBSTITUTION" AND "BRAND NAME MEDICALLY NECESSARY" BELOW.', txt_line3 = "",
         yvar = 0,
         yvar1 = 0,
         MACRO (strip)
         ...
         FOR (x = 1 TO tprint_req->job[i].req[ii].med_knt)
           ...
           CALL print(calcpos(xpos,ypos)), rx = build(cnvtupper(tprint_req->job[i].req[ii].med[x].
             disp)),
           rx, row + 1, ypos = (ypos+ (y_jump * 2))
         ENDFOR
         xpos = lt_xpos,
         CALL print(calcpos(xpos,ypos)), "{b}SIG: "
         ...
         IF ((tprint_req->job[i].req[ii].special_knt > 0)
          ...
          xpos = lt_xpos, ypos = (ypos+ (y_jump * 2)),
          CALL print(calcpos(xpos,ypos)),
          "{b}Spe. Inst.: ", xpos = 65
          FOR (x = 1 TO tprint_req->job[i].req[ii].special_knt)
            CALL print(calcpos(xpos,ypos)), spe_inst = cnvtupper(tprint_req->job[i].req[ii].special[x
          ...
          ypos = (ypos - (y_jump * 2))
         ENDIF
         row + 1, xpos = lt_xpos, ypos = (ypos+ (y_jump * 2))
         FOR (x = 1 TO tprint_req->job[i].req[ii].indic_knt)
           ...
           ENDIF
         ENDFOR
         xpos = lt_xpos, ypos = (ypos+ (y_jump * 2)), ypos = (ypos+ (y_jump * 2)),
         CALL print(calcpos(xpos,ypos)), "{b}Comment : ", xpos = 65
         ...
         strip, exp2, row + 1,
         xpos = ct_xpos,
         CALL print(calcpos(xpos,ypos)), "{b}Refill : "
         IF (trim(tprint_req->job[i].req[ii].total_refill)="0")
          refill = "NO"
         ELSE
          refill = trim(tprint_req->job[i].req[ii].total_refill)
         ENDIF
         refill, row + 1, ypos = 250
         IF ((tprint_req->job[i].position_cd IN (position_cd1, position_cd2, position_cd3)))
          xpos = lt_xpos, ypos = (ypos+ (y_jump * 2)),
          CALL print(calcpos(xpos,ypos)),
          "{b}DX: ", line1, row + 1
          IF (np_ind=true)
           ...
           line2, row + 1, ypos = (ypos+ y_jump),
           xoffset = cnvtint(((size(trim(tprint_req->job[i].eprsnl_name)) * 2)/ 2)), xpos = (ct_xpos
            - xoffset),
           CALL print(calcpos(xpos,ypos)),
           tprint_req->job[i].eprsnl_name, row + 1, xpos = lt_xpos,
           ypos = (ypos+ (y_jump * 1.5)),
           CALL print(calcpos(xpos,ypos)), "{b}RNP DEA NUMBER: {endb}",
           tprint_req->job[i].eprsnl_dea, row + 1, xpos = lt_xpos,
           ypos = (ypos+ (y_jump * 2)),
           ...
           xpos = lt_xpos, ypos = (ypos+ (y_jump * 1.5)),
           CALL print(calcpos(xpos,ypos)),
           "{b}PHY DEA NUMBER: {endb}", tprint_req->job[i].phys_dea, row + 1,
           ypos = (ypos+ (y_jump * 2)),
           CALL print(calcpos(xpos,ypos)), txt_line1,
           row + 1, ypos = (ypos+ y_jump),
           CALL print(calcpos(xpos,ypos)),
           txt_line2, row + 1, ypos = (ypos+ (y_jump * 4)),
           CALL print(calcpos(xpos,ypos)), txt_line3, row + 1,
           row + 1, ypos = (ypos+ (y_jump * 2))
          ELSE
           ypos = (ypos+ (y_jump * 2)),
           CALL print(calcpos(xpos,ypos)), "{b}SIGNATURE: {endb}",
           line2, row + 1, ypos = (ypos+ y_jump),
           ...
           xoffset),
           CALL print(calcpos(xpos,ypos)),
           tprint_req->job[i].phys_bname, row + 1, ypos = (ypos+ (y_jump * 2)),
           CALL print(calcpos(xpos,ypos)), xpos = lt_xpos,
           CALL print(calcpos(xpos,ypos)),
           "{b}RNP DEA NUMBER: {endb}", tprint_req->job[i].phys_dea, row + 1,
           ypos = (ypos+ (y_jump * 2)),
           CALL print(calcpos(xpos,ypos)), "Supervising Physician: Alfred L. McKee Jr, MD",
           row + 1, xpos = lt_xpos, ypos = (ypos+ (y_jump * 2)),
           CALL print(calcpos(xpos,ypos)), txt_line1, row + 1,
           ypos = (ypos+ y_jump),
           CALL print(calcpos(xpos,ypos)), txt_line2,
           row + 1, ypos = (ypos+ (y_jump * 2)),
           CALL print(calcpos(xpos,ypos)),
           line3, row + 1
          ENDIF
         ELSE
          ...
          ypos = (ypos+ (y_jump * 1.5)),
          CALL print(calcpos(xpos,ypos)), "{b}PHY DEA NUMBER: {endb}",
          tprint_req->job[i].phys_dea, row + 1, ypos = (ypos+ (y_jump * 2)),
          CALL print(calcpos(xpos,ypos)), txt_line1, row + 1,
          ypos = (ypos+ y_jump),
          CALL print(calcpos(xpos,ypos)), txt_line2,
          row + 1
         ENDIF
        FOOT REPORT
         orderid = build("[",oid,"]"), ypos = (ypos+ y_jump),
         CALL print(calcpos(xpos,ypos)),
         orderid, row + 1
      ...
      ELSE
       ...
       SELECT INTO value(tprint_req->job[i].print_loc)
        ...
        HEAD REPORT
         ...
         "INTERCHANGE MANDATED UNLESS PRACTITIONER WRITES THE WORDS",
         txt_line2 = '"NO SUBSTITUTION" AND "BRAND NAME MEDICALLY NECESSARY" BELOW.', txt_line3 =
         "For Schedule II drugs - Valid only if raised seal present OR printed on tamper-proof paper",
         txt_line4 = "** Autofaxed Transmission **",
         yvar = 0, yvar1 = 0,
         MACRO (strip)
          x = 0, len = 0, x = findstring(">",exp),
          len = size(trim(exp)), x2 = (len - 2), exp2 = substring(2,x2,exp)
         ...
         IF ((temp_req->qual[i].phys_addr_id=0))
          ypos = (ypos+ y_jump), xoffset = cnvtint(((size(trim(org)) * 4)/ 2)), xpos = (ct_xpos -
          xoffset),
          org_name = trim(org),
          CALL print(calcpos(xpos,ypos)), org_name,
          row + 1, ypos = (ypos+ y_jump), xoffset = cnvtint(((size(trim(street)) * 4)/ 2)),
          xpos = (ct_xpos - xoffset), str = trim(street),
          CALL print(calcpos(xpos,ypos)),
          street, row + 1, ypos = (ypos+ y_jump),
          ...
          ypos = (ypos+ y_jump), p_phone = trim(phone), xoffset = cnvtint(((size(trim(concat(
              "Phone: ",p_phone))) * 4)/ 2)),
          xpos = (ct_xpos - xoffset),
          CALL print(calcpos(xpos,ypos)), "{b}Phone: {endb}",
          p_phone, row + 1
         ELSE
          ypos = (ypos+ y_jump), street2 = trim(temp_req->qual[i].phys_addr1), street3 = trim(
           temp_req->qual[i].phys_addr2)
          IF (trim(street2) > "")
           xoffset = cnvtint(((size(trim(street2)) * 4)/ 2)), xpos = (ct_xpos - xoffset),
           CALL print(calcpos(xpos,ypos)),
           street2, row + 1, ypos = (ypos+ y_jump),
          ...
          ELSE
           ...
           street3, row + 1
          ENDIF
          ypos = (ypos+ y_jump), csz = trim(temp_req->qual[i].phys_city), xoffset = cnvtint(((size(
            trim(csz)) * 4)/ 2)),
          xpos = (ct_xpos - xoffset),
          CALL print(calcpos(xpos,ypos)), csz,
          row + 1
          FOR (ph = 1 TO phy_phone_list->cnt)
            ypos = (ypos+ y_jump), p_phone = format(phy_phone_list->qual[ph].phone,"(###) ###-####"),
            xoffset = cnvtint(((size(trim(concat("Phone: ",p_phone))) * 4)/ 2)),
            xpos = (ct_xpos - xoffset),
            CALL print(calcpos(xpos,ypos)), "{b}Phone: {endb}",
            p_phone, row + 1
          ENDFOR
         ...
         ENDFOR
         xpos = lt_xpos, ypos = (ypos+ (y_jump * 2)), ypos = (ypos+ (y_jump * 2)),
         CALL print(calcpos(xpos,ypos)), "{b}Comments: {endb}", xpos = (65+ lt_xpos)
         FOR (x = 1 TO tprint_req->job[i].req[ii].comment_knt)
           CALL print(calcpos(xpos,ypos)), comment = cnvtupper(tprint_req->job[i].req[ii].comment[x].
            disp), comment,
           row + 1, ypos = (ypos+ (y_jump * 2))
         ...
         IF ((tprint_req->job[i].position_cd IN (position_cd1, position_cd2, position_cd3)))
          ...
          ELSE
           ...
           xpos = lt_xpos, ypos = (ypos+ (y_jump * 2)),
           CALL print(calcpos(xpos,ypos)),
           txt_line1, row + 1, ypos = (ypos+ y_jump),
           CALL print(calcpos(xpos,ypos)), txt_line2, row + 1,
           ypos = (ypos+ (y_jump * 4)),
           CALL print(calcpos(xpos,ypos)), txt_line3,
           row + 1
          ENDIF
         ELSE
          xpos = lt_xpos, ypos = (ypos+ (y_jump * 2)),
     ...
     ELSE
      ...
      SELECT INTO "nl:"
       ...
       HEAD REPORT
        ...
        txt_line1 = "INTERCHANGE MANDATED UNLESS PRACTITIONER WRITES THE WORDS", txt_line2 =
        '"NO SUBSTITUTION" AND "BRAND NAME MEDICALLY NECESSARY" BELOW.', txt_line3 =
        "For Schedule II drugs - Valid only if raised seal present OR printed on tamper-proof paper",
        ...
        concat(wb,center," ","BAYSTATE HEALTH",reol),
        row + 1
        IF ((temp_req->qual[i].phys_addr_id=0))
         nur_unit = trim(org), lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
         drec->line_qual[lidx].disp_line = concat(wb,center," ",nur_unit,reol), row + 1, str = trim(
          street),
         lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line =
         concat(wb,center," ",str,reol),
         row + 1, csz = concat(build(city),", ",build(st),"  ",build(zip)), lidx = (lidx+ 1),
         stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line = concat(center,wb,
          " ",csz,reol), row + 1,
         p_phone = concat("Phone : ",trim(phone)), lidx = (lidx+ 1), stat = alterlist(drec->line_qual,
          lidx),
        ...
        ELSE
         ...
         IF (trim(str1) > "")
          lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line
           = concat(wb,center," ",str1,reol),
          row + 1, lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
          drec->line_qual[lidx].disp_line = concat(wb,center," ",str2,reol), row + 1
         ELSE
          lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line
           = concat(wb,center," ",str2,reol),
          row + 1
         ENDIF
         csz = trim(temp_req->qual[i].phys_city), lidx = (lidx+ 1), stat = alterlist(drec->line_qual,
          lidx),
         drec->line_qual[lidx].disp_line = concat(center,wb," ",csz,reol), row + 1
         FOR (ph = 1 TO phy_phone_list->cnt)
           phone = format(phy_phone_list->qual[ph].phone,"(###) ###-####"), p_phone = concat(
            "Phone : ",trim(phone)), lidx = (lidx+ 1),
           stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line = concat("\ql",wb,
            " ","Phone: ",phone,
            reol,wr), row + 1
         ENDFOR
        ENDIF
        lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line =
        concat(" ",wb,"Date : ",tprint_req->job[i].req[ii].start_dt,reol),
        row + 1, name = trim(demo_info->pat_name), row + 1,
        lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line =
        concat(" ","Patient Name : ",name," ",reol,
         wr),
        lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line =
        concat(wb," ","Date of Birth: ",demo_info->pat_bday,reol),
        row + 1, lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
        ...
        FOR (x = 1 TO tprint_req->job[i].req[ii].med_knt)
         rx = cnvtupper(tprint_req->job[i].req[ii].med[x].disp),
         IF (x=1)
          lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line
           = concat(wb," ","RX : ",rx,reol)
         ELSE
          lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line
           = concat(wb," ",rx," ",reol)
         ENDIF
        ...
        IF ((tprint_req->job[i].req[ii].sig_knt > 0))
         FOR (x = 1 TO tprint_req->job[i].req[ii].sig_knt)
          sig_disp = build(tprint_req->job[i].req[ii].sig[x].disp),
          IF (x=1)
           lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line
            = concat(wb," ","SIG : ",sig_disp," ",
            reol)
          ELSE
           lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line
            = concat(wb," ",sig_disp," ",reol)
          ENDIF
         ENDFOR
        ELSE
         FOR (x = 1 TO tprint_req->job[i].req[ii].special_knt)
           IF (x=1)
            spe_inst = cnvtupper(tprint_req->job[i].req[ii].special[x].disp), lidx = (lidx+ 1), stat
             = alterlist(drec->line_qual,lidx),
            drec->line_qual[lidx].disp_line = concat(wb," ","SIG : ",spe_inst," ",
             reol)
           ELSE
            spe_inst = cnvtupper(tprint_req->job[i].req[ii].special[x].disp), lidx = (lidx+ 1), stat
             = alterlist(drec->line_qual,lidx),
        ...
        ENDFOR
        IF (trim(tprint_req->job[i].req[ii].total_refill)="0")
         refill = "NO", lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
         drec->line_qual[lidx].disp_line = concat(wb," ","Refill : No"," ",reol)
        ELSE
         lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line =
         concat(wb," ","Refill: ",tprint_req->job[i].req[ii].total_refill," ",
          reol)
        ENDIF
        row + 1
        IF ((tprint_req->job[i].req[ii].comment_knt > 0))
         FOR (x = 1 TO tprint_req->job[i].req[ii].comment_knt)
           IF (x=1)
            comment = cnvtupper(substring(1,60,trim(tprint_req->job[i].req[ii].comment[x].disp))),
            lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
            drec->line_qual[lidx].disp_line = concat(wb," ","Comment : ",comment," ",
             reol), row + 1
           ELSE
            comment = cnvtupper(substring(1,60,trim(tprint_req->job[i].req[ii].comment[x].disp))),
            lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
            drec->line_qual[lidx].disp_line = concat(wb," ",comment," ",reol), row + 1
           ENDIF
         ENDFOR
        ENDIF
        ...
        row + 1
        IF ((tprint_req->job[i].position_cd IN (position_cd1, position_cd2, position_cd3)))
         line1 = "    ", lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
         drec->line_qual[lidx].disp_line = concat(wb," ",line1," ",reol), row + 1
         IF (np_ind=true)
          phys_name = trim(tprint_req->job[i].phys_bname), lidx = (lidx+ 1), stat = alterlist(drec->
           line_qual,lidx),
          drec->line_qual[lidx].disp_line = concat(rtab,rtab,wb,"Supervising Physician: "," ",
           phys_name," ",reol), row + 1, lidx = (lidx+ 1),
          stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line = concat(wb," ",
           "PHY DEA NUMBER: ",tprint_req->job[i].phys_dea,reol), row + 1,
          lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line
           = concat(wb," ","___________________________________"," ",reol),
          row + 1, lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
          drec->line_qual[lidx].disp_line = concat(wb,"Ordered By: ",tprint_req->job[i].eprsnl_name,
           " ",reol), row + 1, lidx = (lidx+ 1),
          stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line = concat(wb," ",
           "RNP DEA NUMBER: ",tprint_req->job[i].eprsnl_dea,reol)
         ELSE
          lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line
           = concat(wb,"Ordered By: ",tprint_req->job[i].phys_bname," ",reol),
          row + 1, lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
          drec->line_qual[lidx].disp_line = concat(wb," ","RNP DEA NUMBER: ",tprint_req->job[i].
           phys_dea,reol)
         ENDIF
         lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line =
         concat(" ",txt_line1," ",reol),
         row + 1, lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
         drec->line_qual[lidx].disp_line = concat(wb," ",txt_line2," ",reol), row + 1, lidx = (lidx+
         1),
         stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line = concat(wb," ",
          "___________________________________"," ",reol), row + 1
        ELSE
         line1 = "   ", lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
         drec->line_qual[lidx].disp_line = concat(wb," ",line1," ",reol), row + 1, phys_name = trim(
          tprint_req->job[i].phys_bname),
         lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line =
         concat(rtab,wb28,"Autofaxed By: "," ",phys_name,
          " ",reol),
         row + 1, lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
         drec->line_qual[lidx].disp_line = concat(" ","PHY DEA NUMBER : ",tprint_req->job[i].phys_dea,
          " ",reol), row + 1, lidx = (lidx+ 1),
         stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line = concat(wb," ",
          txt_line1," ",reol), row + 1,
         lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line =
         concat(wb," ",txt_line2," ",reol),
         row + 1, lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
         drec->line_qual[lidx].disp_line = concat(wb," ",txt_line3," ",reol), row + 1, lidx = (lidx+
         1),
         stat = alterlist(drec->line_qual,lidx), lidx = (lidx+ 1), stat = alterlist(drec->line_qual,
          lidx),
         drec->line_qual[lidx].disp_line = concat(wb," ",t1_line," ",reol), row + 1, lidx = (lidx+ 1),
         stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line = concat(wb," ",
          t2_line," ",reol), row + 1,
         lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line =
         concat(wb," ",t3_line," ",reol),
         row + 1, lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
         drec->line_qual[lidx].disp_line = concat(wb," ",t4_line," ",reol), row + 1, lidx = (lidx+ 1),
         stat = alterlist(drec->line_qual,lidx), drec->line_qual[lidx].disp_line = concat(wb," ",
          t5_line," ",reol), row + 1
        ENDIF
       FOOT REPORT
        orderid = build("[",oid,"]"), lidx = (lidx+ 1), stat = alterlist(drec->line_qual,lidx),
