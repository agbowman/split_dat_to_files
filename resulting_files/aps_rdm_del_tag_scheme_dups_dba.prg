CREATE PROGRAM aps_rdm_del_tag_scheme_dups:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "APS_RDM_DEL_TAG_SCHEME_DUPS"
 ...
 DECLARE dtempid = f8 WITH protect, noconstant(0.0)
 DECLARE stempdisplay = c7 WITH protect, noconstant("       ")
 DECLARE ltempseq = i4 WITH protect, noconstant(0)
 DECLARE nduplicatefound = i2 WITH protect, noconstant(0)
 DECLARE lseqcount = i4 WITH protect, noconstant(0)
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY t.tag_group_id, t.tag_display, t.tag_sequence
  HEAD t.tag_group_id
   stempdisplay = " ", dtempid = t.tag_group_id, ltempseq = 250,
   nduplicatefound = 0
  DETAIL
   IF (t.tag_group_id=dtempid
    AND t.tag_display=stempdisplay)
    IF (nduplicatefound=0)
     nduplicatefound = 1, lseqcount = (lseqcount+ 1), stat = alterlist(sequences->qual,lseqcount)
    ENDIF
    IF (t.tag_sequence < ltempseq)
     ltempseq = t.tag_sequence, stempdisplay = t.tag_display, sequences->qual[lseqcount].tag_group_id
      = dtempid,
     sequences->qual[lseqcount].tag_sequence = t.tag_sequence
    ENDIF
   ELSE
    stempdisplay = t.tag_display
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM tag_foundation t,
   (dummyt d  WITH seq = value(lseqcount))
  PLAN (d)
   JOIN (t
   WHERE (t.tag_group_id=sequences->qual[d.seq].tag_group_id)
    AND (t.tag_sequence >= sequences->qual[d.seq].tag_sequence))
  WITH forupdate(t)
 ;end select
 SET lerrorcode = error(serrormessage,0)
 IF (lerrorcode > 0)
  ROLLBACK
  CALL logmsg("Error locking rows on tag_foundation.",((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  CALL logmsg(serrormessage,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  SET readme_data->status = "F"
  SET readme_data->message = serrormessage
  GO TO exit_script
 ENDIF
 DELETE  FROM tag_foundation t,
   (dummyt d  WITH seq = value(lseqcount))
  SET t.seq = 1
  PLAN (d)
   JOIN (t
 ...
 IF (lerrorcode > 0)
  ROLLBACK
  CALL logmsg("Error deleting rows from tag_foundation.",((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  CALL logmsg(serrormessage,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  SET readme_data->status = "F"
  SET readme_data->message = serrormessage
  GO TO exit_script
...
#exit_script
 IF (nfailed=0)
  ...
  COMMIT
 ELSE
  CALL logstatus("Execution failed.  Delete unsuccessful.","F")
  COMMIT
 ENDIF
 EXECUTE dm_readme_status
 CALL echorecord(readme_data)
