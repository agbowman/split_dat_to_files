CREATE PROGRAM ccps_pha_lbt_pass_1x35
 PROMPT
 ...
 SUBROUTINE footer_secabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.273),(offsety+ 0.234),(offsetx+ 0.992),(offsety+
     0.234))
   ENDIF
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 1.450),(offsety+ 0.234),(offsetx+ 2.169),(offsety+
     0.234))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_y = (offsety+ 0.000)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.273),(offsety+ 0.109),(offsetx+ 0.992),(offsety+
     0.109))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_y = (offsety+ 0.000)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 1.450),(offsety+ 0.109),(offsetx+ 2.169),(offsety+
     0.109))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_y = (offsety+ 0.000)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 2.606),(offsety+ 0.109),(offsetx+ 3.325),(offsety+
     0.109))
   ENDIF
   SET rptsd->m_flags = 1024
   SET rptsd->m_y = (offsety+ 0.125)
   SET rptsd->m_x = (offsetx+ 3.023)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CCPS_PHA_LBT_PASS_1X35"
 ...
 SET _bsubreport = 1
 EXECUTE ccps_pha_lbt_header_addr_1x35  $1
 SET _bsubreport = 0
