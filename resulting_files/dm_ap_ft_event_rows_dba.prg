CREATE PROGRAM dm_ap_ft_event_rows:dba
 ...
 DECLARE months_to_keep = i4 WITH noconstant(0)
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(afe.followup_event_id)
  FROM ap_ft_event afe
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    afe.rowid
    FROM ap_ft_event afe
    WHERE parser(sbr_getrowidnotexists("afe.followup_event_id between curMinID and curMaxID","afe"))
     AND afe.term_dt_tm <= cnvtdatetime(purge_dt_tm)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,100)=1)
      stat = alterlist(reply->rows,(rows+ 99))
     ENDIF
     reply->rows[rows].row_id = afe.rowid
    WITH nocounter, maxqual(afe,value(rowsleft))
   ;end select
