CREATE PROGRAM dm2_dpq_report:dba
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE ddr_proc_type = vc WITH protect, noconstant("")
 DECLARE ddr_status = vc WITH protect, noconstant("")
 DECLARE ddr_sort_field = vc WITH protect, noconstant("")
 DECLARE ddr_sort_value = vc WITH protect, noconstant("")
 DECLARE ddr_continue = i2 WITH protect, noconstant(1)
 DECLARE ddr_where_clause = vc WITH protect, noconstant("")
 DECLARE ddr_report_destination = vc WITH protect, noconstant("")
 DECLARE ddr_ndx = i4 WITH protect, noconstant(0)
 DECLARE ddr_pos = i4 WITH protect, noconstant(0)
 IF ((validate(ddr_list->cnt,- (1))=- (1))
  AND (validate(ddr_list->cnt,- (2))=- (2)))
  FREE RECORD ddr_list
  RECORD ddr_list(
    1 cnt = i4
    1 qual[*]
      2 dpq_id = f8
 ...
 ENDIF
 IF ((validate(ddr_statuses->cnt,- (1))=- (1))
  AND (validate(ddr_statuses->cnt,- (2))=- (2)))
  FREE RECORD ddr_statuses
  RECORD ddr_statuses(
    1 cnt = i4
    1 qual[*]
      2 status = vc
 ...
 ENDIF
 IF (check_logfile("dm2_dpq_rpt",".log","dm2_dpq_report LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_dpq_report"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET ddr_proc_type = cnvtupper(trim( $1))
 SET ddr_status = cnvtupper(trim( $2))
 IF (((check_error(dm_err->eproc)=1) OR ((( NOT (ddr_proc_type > " ")) OR ( NOT (ddr_status > " ")))
 )) )
  SET dm_err->emsg = "Parameter usage: dm2_dpq_report '<PROCESS_TYPE>/ALL', '<PROCESS_STATUS>/ALL'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Obtaining sort criteria from user"
 CALL disp_msg("",dm_err->logfile,0)
 WHILE (ddr_continue=1)
   SET message = window
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL text(2,2,"DM_PROCESS_QUEUE REPORT [SORT CRITERIA]")
   ...
   SET help = fix("PROCESS TYPE,OWNER.OBJECT,OBJECT TYPE,PROCESS STATUS,PRIORITY,BEGIN TIME,END TIME"
    )
   CALL accept(5,18,"P(20);CUF",ddr_sort_field
    WHERE curaccept IN ("PROCESS TYPE", "OWNER.OBJECT", "OBJECT TYPE", "PROCESS STATUS", "PRIORITY",
    "BEGIN TIME", "END TIME"))
   SET help = off
   SET ddr_sort_field = curaccept
   CALL text(6,2,"Sort Value: ")
   SET help = pos(6,60,10,60)
   SET help = fix("ASC,DESC")
   CALL accept(6,18,"P(15);CUF",ddr_sort_value
    WHERE curaccept IN ("ASC", "DESC"))
   SET help = off
   SET ddr_sort_value = curaccept
   CALL text(10,2,"(C)ontinue, (M)odify, (Q)uit :")
   CALL accept(10,34,"p;cu","C"
 ...
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ENDIF
 IF (ddr_proc_type="ALL")
  IF (ddr_status="ALL")
   SET ddr_where_clause = "dpq.process_type = patstring('*') and dpq.process_status = patstring('*')"
  ELSE
   SET ddr_where_clause = concat("dpq.process_type = patstring('*') and dpq.process_status = '",
    ddr_status,"'")
  ENDIF
 ELSE
  IF (ddr_status="ALL")
   SET ddr_where_clause = concat("dpq.process_type = patstring('",ddr_proc_type,
    "') and dpq.process_status = patstring('*')")
  ELSE
   SET ddr_where_clause = concat("dpq.process_type = patstring('",ddr_proc_type,
    "') and dpq.process_status = '",ddr_status,"'")
  ENDIF
 ENDIF
 SET dm_err->eproc = "Loading rows from DM_PROCESS_QUEUE"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_process_queue dpq
  WHERE parser(ddr_where_clause)
  DETAIL
   ddr_list->cnt = (ddr_list->cnt+ 1)
   IF (mod(ddr_list->cnt,50)=1)
    stat = alterlist(ddr_list->qual,(ddr_list->cnt+ 49))
   ENDIF
   ddr_list->qual[ddr_list->cnt].dpq_id = dpq.dm_process_queue_id, ddr_list->qual[ddr_list->cnt].
   process_type = dpq.process_type, ddr_list->qual[ddr_list->cnt].owner_name = dpq.owner_name,
   ddr_list->qual[ddr_list->cnt].object_type = dpq.object_type, ddr_list->qual[ddr_list->cnt].
   object_name = dpq.object_name, ddr_list->qual[ddr_list->cnt].owner_object = build(ddr_list->qual[
    ddr_list->cnt].owner_name,".",ddr_list->qual[ddr_list->cnt].object_name),
   ddr_list->qual[ddr_list->cnt].operation_txt = dpq.operation_txt, ddr_list->qual[ddr_list->cnt].
   process_status = dpq.process_status, ddr_list->qual[ddr_list->cnt].priority = dpq.priority,
   ddr_list->qual[ddr_list->cnt].message_txt = dpq.message_txt, ddr_list->qual[ddr_list->cnt].
   begin_dt_tm = dpq.begin_dt_tm, ddr_list->qual[ddr_list->cnt].end_dt_tm = dpq.end_dt_tm
   IF (ddr_proc_type != "ALL"
    AND ddr_status="ALL")
    ddr_pos = locateval(ddr_ndx,1,ddr_statuses->cnt,ddr_list->qual[ddr_list->cnt].process_status,
     ddr_statuses->qual[ddr_ndx].status)
    IF (ddr_pos=0)
     ddr_statuses->cnt = (ddr_statuses->cnt+ 1), stat = alterlist(ddr_statuses->qual,ddr_statuses->
      cnt), ddr_statuses->qual[ddr_statuses->cnt].status = ddr_list->qual[ddr_list->cnt].
     process_status,
     ddr_statuses->qual[ddr_statuses->cnt].status_cnt = (ddr_statuses->qual[ddr_statuses->cnt].
     status_cnt+ 1)
    ELSE
     ddr_statuses->qual[ddr_pos].status_cnt = (ddr_statuses->qual[ddr_pos].status_cnt+ 1)
    ENDIF
   ENDIF
  FOOT REPORT
   stat = alterlist(ddr_list->qual,ddr_list->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(ddr_list)
 ENDIF
 SET ddr_report_destination = concat("dm2_dpq_rpt_",format(cnvtdatetime(curdate,curtime3),
   "MMDDYYHHMM;;Q"))
 SET dm_err->eproc = "Generating DM_PROCESS_QUEUE report data to file from record structure."
 SELECT
  IF (ddr_sort_field="PROCESS TYPE"
   AND ddr_sort_value="ASC")
   WHERE 1=1
   ORDER BY process_type
  ELSEIF (ddr_sort_field="PROCESS TYPE"
   AND ddr_sort_value="DESC")
   WHERE 1=1
   ORDER BY process_type DESC
  ELSEIF (ddr_sort_field="OWNER.OBJECT"
   AND ddr_sort_value="ASC")
   WHERE 1=1
   ORDER BY owner_object
  ELSEIF (ddr_sort_field="OWNER.OBJECT"
   AND ddr_sort_value="DESC")
   WHERE 1=1
   ORDER BY owner_object DESC
  ELSEIF (ddr_sort_field="OBJECT TYPE"
   AND ddr_sort_value="ASC")
   WHERE 1=1
  ...
  ELSEIF (ddr_sort_field="PROCESS STATUS"
   AND ddr_sort_value="ASC")
   WHERE 1=1
  ...
  ELSEIF (ddr_sort_field="PRIORITY"
   AND ddr_sort_value="ASC")
   WHERE 1=1
   ORDER BY priority
  ELSEIF (ddr_sort_field="PRIORITY"
  ...
  ELSEIF (ddr_sort_field="BEGIN TIME"
   AND ddr_sort_value="ASC")
   WHERE 1=1
   ORDER BY begin_dt_tm
  ELSEIF (ddr_sort_field="BEGIN TIME"
   AND ddr_sort_value="DESC")
   WHERE 1=1
   ORDER BY begin_dt_tm DESC
  ELSEIF (ddr_sort_field="END TIME"
   AND ddr_sort_value="ASC")
   WHERE 1=1
   ORDER BY end_dt_tm
  ELSEIF (ddr_sort_field="END TIME"
   AND ddr_sort_value="DESC")
   WHERE 1=1
   ORDER BY end_dt_tm DESC
  ELSE
  ENDIF
  INTO build(ddr_report_destination)
  process_type = ddr_list->qual[d.seq].process_type, owner_object = ddr_list->qual[d.seq].
  owner_object, object_type = ddr_list->qual[d.seq].object_type,
  process_status = ddr_list->qual[d.seq].process_status, priority = ddr_list->qual[d.seq].priority,
  begin_dt_tm = ddr_list->qual[d.seq].begin_dt_tm,
  end_dt_tm = ddr_list->qual[d.seq].end_dt_tm
  FROM (dummyt d  WITH seq = value(ddr_list->cnt))
  WHERE 1=1
  HEAD REPORT
   row + 0, col 0, "***************** DM_PROCESS_QUEUE REPORT ***********************",
   row + 1, col 0, "-- Criteria specified --",
   row + 1, col 0, "PROCESS TYPE:",
   col 25, ddr_proc_type, row + 1,
   col 0, "PROCESS STATUS:", col 25,
   ddr_status, row + 1
   IF (ddr_proc_type != "ALL")
    row + 1, col 0, "-- Status Summary --",
    row + 1, col 0, "Status",
    col 25, "Count"
    FOR (ddr_pos = 1 TO ddr_statuses->cnt)
      row + 1, col 0,
      CALL print(ddr_statuses->qual[ddr_pos].status),
      col 25,
      CALL print(ddr_statuses->qual[ddr_pos].status_cnt)
    ENDFOR
   ENDIF
   row + 2, row + 0, col 0,
   "************* DM_PROCESS_QUEUE LIST ***********************", row + 2, col 0,
   "PROCESS_TYPE", col 30, "OWNER.OBJECT_NAME",
   col 80, "STATUS", col 95,
   "TYPE", col 110, "PRIORITY",
   col 120, "BEGIN TIME", col 145,
   "END TIME", col 170, "OPERATION TXT",
   col 230, "MESSAGE TXT", row + 1,
   col 0,
   CALL print(fillstring(25,"-")), col 30,
   CALL print(fillstring(45,"-")), col 80,
   CALL print(fillstring(10,"-")),
   col 95,
   CALL print(fillstring(5,"-")), col 110,
   CALL print(fillstring(10,"-")), col 120,
   CALL print(fillstring(20,"-")),
   col 145,
   CALL print(fillstring(20,"-")), col 170,
   CALL print(fillstring(10,"-")), col 230,
   CALL print(fillstring(10,"-")),
   row + 1
  DETAIL
   IF ((ddr_list->cnt > 0))
    row + 1, col 0,
    CALL print(ddr_list->qual[d.seq].process_type),
    col 30,
    CALL print(ddr_list->qual[d.seq].owner_object), col 80,
    CALL print(ddr_list->qual[d.seq].process_status), col 95,
    CALL print(ddr_list->qual[d.seq].object_type),
    col 110,
    CALL print(cnvtstring(ddr_list->qual[d.seq].priority)), col 120,
    CALL print(format(ddr_list->qual[d.seq].begin_dt_tm,"DD-MMM-YYYY HH:MM:SS;;D")), col 145,
    CALL print(format(ddr_list->qual[d.seq].end_dt_tm,"DD-MMM-YYYY HH:MM:SS;;D")),
    col 170,
    CALL print(substring(1,50,ddr_list->qual[d.seq].operation_txt)), col 230,
    CALL print(substring(1,100,ddr_list->qual[d.seq].message_txt))
   ELSE
  ...
  WITH nocounter, nullreport, maxcol = 512,
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  RETURN(0)
 ENDIF
 IF (dm2_disp_file(ddr_report_destination,"Dm_process_queue Report")=0)
  GO TO exit_script
 ENDIF
 GO TO exit_script
#exit_script
 SET dm_err->eproc = "Ending dm2_dpq_report"
 CALL final_disp_msg("dm2_dpq_rpt")
END GO
