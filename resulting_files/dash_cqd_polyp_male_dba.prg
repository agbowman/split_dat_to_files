CREATE PROGRAM dash_cqd_polyp_male:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (st
   WHERE expand(idx,1,procedures->qual_cnt,st.scd_story_id,procedures->qual[idx].scd_story_id,
    cs57male,procedures->qual[idx].sex_cd)
  ...
  DETAIL
   ...
   num_polyp+ 1)
  FOOT REPORT
   json_data->pct_polyp = cnvtreal(((json_data->num_polyp/ cnvtreal(procedures->male_qual_cnt)) * 100
    ))
  WITH nocounter, expand = 1
 ;end select
#exit_script
 IF ((procedures->male_qual_cnt > 0))
