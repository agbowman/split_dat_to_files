CREATE PROGRAM cv_chk_sts_dataset:dba
 ...
 SELECT INTO "nl:"
  ...
  WHERE cx.dataset_id=cd.dataset_id
   AND cnvtupper(trim(cx.xref_internal_name))="STS*"
  DETAIL
 ...
 SELECT INTO "nl:"
  ...
  WHERE cd.dataset_id=cx.dataset_id
   ...
   AND cnvtupper(trim(cr.response_internal_name))="STS*"
  DETAIL
 ...
 IF (failure="T")
  ...
  SET error_msg = " Incorrect STS Dataset file readme counts found in the following tables:"
  IF (dataset_flag=1)
 ...
 ELSE
  ...
  CALL echo("CVNet STS_Dataset has been successfully loaded!")
 ENDIF
