CREATE PROGRAM bhs_gen_disch_dt_tm_cond:dba
 SET trace = nocost
   ...
   ELSE
    ...
    FOR (i = 1 TO encntr_count)
      UPDATE  FROM encounter e
       SET e.disch_dt_tm = cnvtdatetime(encntrs->encntr[i].reg_dt_tm), e.depart_dt_tm = cnvtdatetime(
         encntrs->encntr[i].reg_dt_tm), e.updt_task = 3366,
        e.updt_cnt = (e.updt_cnt+ 1), e.updt_dt_tm = cnvtdatetime(curdate,curtime3), e.updt_id = 3366,
        e.encntr_status_cd = 856
