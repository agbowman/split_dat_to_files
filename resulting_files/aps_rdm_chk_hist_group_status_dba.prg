CREATE PROGRAM aps_rdm_chk_hist_group_status:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_rdm_chk_hist_grp"
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   ...
   (SELECT
    rhg.grouping_cd
    FROM report_history_grouping_r rhg
    WHERE rhg.grouping_cd=cv.code_value))))
  DETAIL
   ncnt = (ncnt+ 1)
   IF (mod(ncnt,10)=1)
    stat = alterlist(upd_codeset_req->qual,(ncnt+ 9))
   ENDIF
   upd_codeset_req->qual[ncnt].code_value = cv.code_value, upd_codeset_req->qual[ncnt].code_set = cv
   .code_set, upd_codeset_req->qual[ncnt].cdf_meaning = cv.cdf_meaning,
   upd_codeset_req->qual[ncnt].display = cv.display, upd_codeset_req->qual[ncnt].description = cv
   .description, upd_codeset_req->qual[ncnt].definition = cv.definition,
   upd_codeset_req->qual[ncnt].collation_seq = cv.collation_seq, upd_codeset_req->qual[ncnt].
   active_ind = 0
  FOOT REPORT
   stat = alterlist(upd_codeset_req->qual,ncnt)
  WITH nocounter
 ;end select
 ...
 ENDIF
#exit_script
 IF (((size(upd_codeset_req->qual,5)=0) OR ((upd_codeset_rep->status_data.status="S"))) )
  CALL logstatus("Code_value table updated successfully.","S")
 ELSE
  CALL logstatus("Failed to update code_value table.","F")
 ENDIF
