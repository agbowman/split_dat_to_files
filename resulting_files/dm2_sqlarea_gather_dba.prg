CREATE PROGRAM dm2_sqlarea_gather:dba
 PROMPT
  "Output File Name (default = displayer): " = "MINE"
 DECLARE ms_logfile_prefix = vc WITH protect, constant("dm2_sqlarea_gather")
 ...
 SUBROUTINE sbr_gather_database_sql(ps_output_file_name,pn_gather_mode)
   ...
   RETURN(1)
 END ;Subroutine
 SET ms_output_file_name = trim(cnvtlower( $1),3)
 ...
 ENDIF
 IF (check_logfile(ms_logfile_prefix,".log","DM2_SQLAREA_GATHER LOGFILE")=0)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Error creating log file"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning DM2_SQLAREA_GATHER"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (ms_output_file_name="*.csv")
  CALL sbr_gather_database_sql(ms_output_file_name,mn_csv_output)
 ELSE
  CALL sbr_gather_database_sql(ms_output_file_name,mn_dat_output)
 ENDIF
...
#exit_program
 FREE RECORD m_files_delta
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending DM2_SQLAREA_GATHER.  Process completed successfully."
