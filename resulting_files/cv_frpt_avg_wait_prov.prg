CREATE PROGRAM cv_frpt_avg_wait_prov
 PROMPT
  ...
  SELECT
   ...
   HEAD REPORT
    ...
    ndaylightvar = 0, stimezone = datetimezonebyindex(curtimezoneapp,noffsetvar,ndaylightvar,7,
     cnvtdatetime(curdate,curtime)), gtotal_avg = 0,
    total_cnt = 0, total_avg = 0, p_curr = 0,
    p_max = 0, proc_performed = 0, tproc_performed = 0,
    tprov_avg = 0, tgraph = 0, max_tgraph = 0,
    graph_max = 0, tsub_avg = 0, sub_avg = 0,
    sub_cnt = 0, _fenddetail = (rptreport->m_pageheight - rptreport->m_marginbottom), _fenddetail = (
   ...
   HEAD cv_list_rpl_catalog_disp
    tsub_avg = 0, prov_cnt = 0, prov_avg = 0,
   ...
   DETAIL
    total_cnt = (total_cnt+ 1), total_avg = (total_avg+ cv_list_rpl_wait_minutes), prov_cnt = (
    prov_cnt+ 1),
    prov_avg = (prov_avg+ cv_list_rpl_wait_minutes), proc_performed = (proc_performed+ 1),
   ...
   FOOT  cv_list_rpl_full_name
    tprov_avg = avg(cv_list_rpl_wait_minutes), tproc_performed = (tproc_performed+ proc_performed),
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.188),(offsety+ 1.255),(offsetx+ 7.251),(offsety+
     1.255))
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(uar_i18ngetmessage(_hi18nhandle,
        "AVERAGE PATIENT WAIT TIME BY PROVIDER","Average Patient Wait Time by Provider")),char(0)))
 ...
 SUBROUTINE headfullnamesectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
      IF (trim(cv_list_rpl_full_name)=null) uar_i18ngetmessage(_hi18nhandle,"NOPROVIDER",
        "No Provider Found")
      ENDIF
      ,char(0)))
    SET rptsd->m_y = (offsety+ 0.000)
 ...
 SUBROUTINE footcatalogsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 2.500),(offsety+ 0.032),(offsetx+ 7.250),(offsety+
     0.032))
    SET rptsd->m_flags = 32
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 4.500)
    ...
    SET rptsd->m_height = 0.188
    SET _dummyfont = uar_rptsetfont(_hreport,_times108388608)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "TOTAL","Total"),char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
 ...
 SUBROUTINE graphsectionabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(3.650000), private
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptgraphrec->m_fwidth = 6.500000
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CV_FRPT_AVG_WAIT_PROV"
