CREATE PROGRAM dm2_aload_afd_part_subpart_tsp
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_afd_part_subpart_tsp",".log","dm2_aload_afd_part_subpart_tsp")=0)
  GO TO exit_program
 ENDIF
 DECLARE daapst_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daapst_file = vc WITH protect, noconstant( $2)
 DECLARE daapst_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daapst_batch = i4 WITH protect, noconstant(0)
 DECLARE daapst_rows_returned = i4 WITH protect, noconstant(1)
 DECLARE daapst_file_loc_ret = vc WITH protect, noconstant("")
 SET dm_err->eproc = build("Starting dm2_aload_afd_part_subpart_tsp",daapst_pkg,daapst_file,
  daapst_expected_rows)
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dac_prelim(daapst_pkg,daapst_file_loc_ret,daapst_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daapst_batch=0)
  SET daapst_batch = 5000
 ENDIF
 IF (daapst_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daapst_file_loc_ret,daapst_file), "dm2_aload_sch_part_subpart_tsp",
  daapst_batch
  CALL echo(concat(daapst_file_loc_ret,daapst_file))
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Remove extraneous DM2_ALOAD_AFD_PART_SUBPART_TSPACES rows"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dm_afd_part_subpart_tspaces dapst
   WHERE dapst.alpha_feature_nbr=daapst_pkg
    AND dapst.owner=currdbuser
    AND  NOT ( EXISTS (
   (SELECT
    "x"
    FROM dm_afd_tables dat
    WHERE dat.alpha_feature_nbr=dapst.alpha_feature_nbr
     AND dat.owner=dapst.owner
     AND dat.table_name=dapst.table_name
     AND dat.updt_dt_tm=dapst.updt_dt_tm)))
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_afd_part_subpart_tsp completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_afd_part_subpart_tsp")
END GO
