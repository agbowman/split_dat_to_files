CREATE PROGRAM dm_rmc_mover_3203:dba
 ...
 DECLARE dsr_imt_check = i2
 DECLARE cust_beg_eff_ind = i2
 DECLARE r_tab_name_dir = vc
 DECLARE r_tab_name_nvp = vc
 DECLARE r_tab_name_dsr = vc
 DECLARE drm3203_context_name = vc
 DECLARE mxlat_loop = i4
 DECLARE drm_context = vc
 DECLARE drm_inst_id = f8
 DECLARE drm_section_oct = i4
 DECLARE drs_sc_parent_ind = i2
 DECLARE drs_parent_drdm_row = i2
 DECLARE cust_nomv_chk = vc WITH protect, noconstant("")
 DECLARE drm_ptam_result = f8
 DECLARE del_$r_sub(sbr_tab_name_dir=vc,sbr_tab_name_dsr=vc,sbr_tab_name_nvp=vc) = null
 FREE RECORD input_ref
 RECORD input_ref(
   1 qual[*]
     2 ref_id = f8
 )
 IF ((global_mover_rec->ptam_ind=1))
  FREE RECORD dcpsr_drpb_request
  RECORD dcpsr_drpb_request(
    1 table_name = vc
    1 table_suffix = vc
    1 match_str_tspace_cnt = i4
    1 src_match_result = f8
  ...
  FREE RECORD dcpsr_drpb_reply
  RECORD dcpsr_drpb_reply(
    1 nomv_type = vc
    1 blocking_log_id = f8
    1 error_ind = i2
    1 reason_text = vc
  )
  SET dcpsr_drpb_request->table_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
  SET dcpsr_drpb_request->table_suffix = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix
  SET dcpsr_drpb_request->src_env_id = dm2_ref_data_doc->env_source_id
  SET dcpsr_drpb_request->trg_env_id = dm2_ref_data_doc->env_target_id
  SET dcpsr_drpb_request->src_match_result = drdm_chg->log[drdm_log_loop].ptam_match_result
 ENDIF
 SET r_tab_name_dir = cutover_tab_name("DCP_INPUT_REF","")
 SET r_tab_name_nvp = cutover_tab_name("NAME_VALUE_PREFS","")
 SET r_tab_name_dsr = cutover_tab_name("DCP_SECTION_REF","")
 ...
 SET ref_vc = get_value(cust_tab_name,"DCP_SECTION_REF_ID","FROM")
 SET inst_vc = get_value(cust_tab_name,"DCP_SECTION_INSTANCE_ID","FROM")
 ...
 IF (active_ind=0)
  SELECT INTO "NL:"
   ...
   WHERE d.dcp_section_ref_id=cnvtreal(ref_vc)
    AND d.active_ind=1
   DETAIL
    inactivate_ind = 0, drm_inst_id = d.dcp_section_instance_id
   WITH nocounter
  ;end select
 ...
 SET ref_num = locateval(index_var,1,perm_col_cnt,"DCP_SECTION_REF_ID",dm2_ref_data_doc->tbl_qual[
  temp_tbl_cnt].col_qual[index_var].column_name)
 SET inst_num = locateval(index_var,1,perm_col_cnt,"DCP_SECTION_INSTANCE_ID",dm2_ref_data_doc->
 ...
 CALL echo("")
 CALL echo("")
 SET dm_err->eproc = "Checking if dcp_section_ref is a single commit parent"
 SET drs_sc_parent_ind = 0
 SELECT INTO "NL:"
  FROM dm_refchg_tab_r r
  WHERE r.parent_table="DCP_SECTION_REF"
  DETAIL
   drs_sc_parent_ind = 1
  WITH nocounter
 ;end select
 ...
 IF (inactivate_ind=0
  AND active_ind=1)
  SET inst_to_vc = select_merge_translate(inst_vc,"DCP_SECTION_REF")
  IF (inst_to_vc="No Trans")
   SET inst_val = get_seq(cust_tab_name,"DCP_SECTION_INSTANCE_ID")
   IF (inst_val > 0)
    CALL put_value(cust_tab_name,"DCP_SECTION_INSTANCE_ID",cnvtstring(inst_val))
    SET current_merges = (current_merges+ 1)
    SET child_merge_audit->num[current_merges].action = "NEWSEQ"
    SET child_merge_audit->num[current_merges].text = "DCP_SECTION_REF   DCP_SECTION_INSTANCE_ID"
   ELSE
    SET stat = alterlist(dm2_ref_data_reply->qual,0)
    GO TO exit_3203
   ENDIF
   SET from_id = cnvtreal(get_value(cust_tab_name,"DCP_SECTION_INSTANCE_ID","FROM"))
   IF ((dm2_ref_data_reply->error_ind=1))
    SET stat = alterlist(dm2_ref_data_reply->qual,0)
    GO TO exit_3203
   ELSE
    SET dsr_imt_check = insert_merge_translate(from_id,inst_val,"DCP_SECTION_REF")
    IF (dsr_imt_check=1)
     GO TO exit_3203
    ENDIF
   ENDIF
  ELSEIF (inst_to_vc="NOMV21")
 ...
 ELSEIF (inactivate_ind=0
  ...
  IF (drs_sc_parent_ind=1)
   CALL trigger_proc_call(cust_tab_name,"",drdm_chg->log[drdm_log_loop].src_cntxt_name,
    "DCP_SECTION_INSTANCE_ID",drm_inst_id)
  ENDIF
  FOR (mxlat_loop = 1 TO size(missing_xlats->qual,5))
    IF ((missing_xlats->qual[mxlat_loop].table_name="DCP_SECTION_REF")
     AND (missing_xlats->qual[mxlat_loop].missing_value=drm_inst_id))
     SET missing_xlats->qual[mxlat_loop].processed_ind = 1
    ENDIF
  ENDFOR
  SET drm_section_oct = orphan_child_tab(cust_tab_name,"OLDVER","",cnvtreal(rs_3203->from_values.
    dcp_section_instance_id))
 ELSE
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[inst_num].translated = 1
 ENDIF
 IF (((active_ind=1) OR (inactivate_ind=1)) )
  SET temp_col_cnt = get_col_pos(cust_tab_name,"DCP_SECTION_INSTANCE_ID")
  IF ((dm2_ref_data_reply->error_ind=1))
   SET stat = alterlist(dm2_ref_data_reply->qual,0)
   GO TO exit_3203
  ENDIF
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[ref_num].translated=0))
   SET cust_nomv_chk = select_merge_translate(ref_vc,cust_tab_name)
   ...
   SELECT INTO "NL:"
    ...
    WHERE (d.description=rs_3203->to_values.description)
     AND (d.definition=rs_3203->to_values.definition)
     AND d.active_ind=1
    DETAIL
     ref_ui_cnt = (ref_ui_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,ref_ui_cnt),
     ui_query_eval_rec->qual[ref_ui_cnt].root_entity_attr = d.dcp_section_ref_id
    WITH nocounter
   ;end select
   SET ref_val = evaluate_exec_ui_query(ref_ui_cnt,temp_tbl_cnt,perm_col_cnt)
   IF ((ref_val=- (3)))
    IF (inactivate_ind=1)
     CALL echo("dm_rmc_mover_3203:The inactive section doesn't exist in target")
     CALL merge_audit("FAILREASON","The inactive section doesn't exist in target",2)
     SET drdm_no_trans_ind = 1
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "OLDVER"
     FOR (mxlat_loop = 1 TO size(missing_xlats->qual,5))
       IF ((missing_xlats->qual[mxlat_loop].table_name="DCP_SECTION_REF")
        AND (missing_xlats->qual[mxlat_loop].missing_value IN (rs_3203->from_values.
       dcp_section_ref_id, rs_3203->from_values.dcp_section_instance_id)))
        SET missing_xlats->qual[mxlat_loop].processed_ind = 1
       ENDIF
     ENDFOR
     IF (drdm_error_out_ind=1)
     ...
     GO TO exit_3203
    ELSE
     SET ref_val = get_seq(cust_tab_name,"DCP_SECTION_REF_ID")
     IF (ref_val > 0)
      CALL put_value(cust_tab_name,"DCP_SECTION_REF_ID",cnvtstring(ref_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = "DCP_SECTION_REF   DCP_SECTION_REF_ID"
     ELSE
      SET stat = alterlist(dm2_ref_data_reply->qual,0)
      GO TO exit_3203
     ENDIF
   ...
   ELSE
    CALL put_value(cust_tab_name,"DCP_SECTION_REF_ID",cnvtstring(ref_val))
   ENDIF
   SET from_id = cnvtreal(get_value(cust_tab_name,"DCP_SECTION_REF_ID","FROM"))
   IF ((dm2_ref_data_reply->error_ind=1))
    SET stat = alterlist(dm2_ref_data_reply->qual,0)
    GO TO exit_3203
   ELSE
    SET dsr_imt_check = insert_merge_translate(from_id,ref_val,"DCP_SECTION_REF")
    IF (dsr_imt_check=1)
     GO TO exit_3203
    ENDIF
   ENDIF
  ...
  SET drm_ptam_result = drmm_get_ptam_match_result(0.0,dm2_ref_data_doc->env_source_id,
   dm2_ref_data_doc->env_target_id,dm2_ref_data_doc->post_link_name,0,
   "FROM")
  IF ((global_mover_rec->ptam_ind=1))
   SET dcpsr_drpb_request->trg_match_result = drm_ptam_result
   EXECUTE dm_rmc_ptam_block  WITH replace("REQUEST","DCPSR_DRPB_REQUEST"), replace("REPLY",
    "DCPSR_DRPB_REPLY")
   IF (check_error(dm_err->eproc)=1)
    SET dm2_ref_data_reply->error_msg = dm_err->emsg
    SET dm2_ref_data_reply->error_ind = 1
    GO TO exit_3203
   ENDIF
   IF ((dcpsr_drpb_reply->blocking_log_id > 0))
    SET drdm_chg->log[drdm_log_loop].blocking_log_id = dcpsr_drpb_reply->blocking_log_id
    SET drs_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
    SET drdm_chg->log[drs_parent_drdm_row].chg_log_reason_txt = dcpsr_drpb_reply->reason_text
    SET dm2_ref_data_reply->error_ind = 1
    SET drdm_mini_loop_status = dcpsr_drpb_reply->nomv_type
    SET drdm_no_trans_ind = 1
    SET dm2_ref_data_reply->error_msg = dcpsr_drpb_reply->nomv_type
    CALL orphan_child_tab(cust_tab_name,dcpsr_drpb_reply->nomv_type,"DCP_SECTION_INSTANCE_ID",
     cnvtreal(rs_3203->from_values.dcp_section_instance_id))
    GO TO exit_3203
   ENDIF
  ENDIF
  IF (cust_beg_eff_ind=1)
   SELECT INTO "NL:"
    ...
    WHERE dsr.dcp_section_instance_id=inst_val
     AND dsr.active_ind=1
    DETAIL
     cur_dt_tm = cnvtdatetime(dsr.beg_effective_dt_tm)
    WITH nocounter
   ;end select
   IF (curqual=0)
    SELECT INTO "NL:"
     FROM dcp_section_re3203$r dsr
     WHERE dsr.dcp_section_instance_id=inst_val
      AND dsr.active_ind=1
      AND dsr.rdds_status_flag < 9000
     WITH nocounter
    ;end select
    IF (curqual=0)
     SELECT INTO "NL:"
      ...
      FROM dcp_section_ref dsr
      WHERE ((dsr.dcp_section_instance_id=inst_val) UNION (
      (SELECT
       dcp_section_instance_id
       FROM dcp_section_re3203$r
       WHERE dcp_section_instance_id=inst_val
        AND rdds_status_flag < 9000)))
      WITH nocounter
     ;end select
     IF (curqual != 0)
      SET dm2_ref_data_reply->error_ind = 1
      SET dm2_ref_data_reply->error_msg = "NOMV29"
      SET stat = alterlist(dm2_ref_data_reply->qual,0)
      CALL merge_audit("FAILREASON",
       "A dual maintenance change to a DCP_SECTION_REF row was not merged into SOURCE",1)
      GO TO exit_3203
     ENDIF
    ENDIF
   ENDIF
  ENDIF
  SET rs_3203->to_values.beg_effective_dt_tm = cur_dt_tm
  IF ((dm2_ref_data_reply->error_ind=1))
   SET stat = alterlist(dm2_ref_data_reply->qual,0)
   GO TO exit_3203
  ENDIF
  ...
  CALL echo("")
  IF (inactivate_ind=1)
   CALL put_value(cust_tab_name,"DCP_SECTION_INSTANCE_ID","0")
  ENDIF
  SET idcd_check = is_translated("DCP_SECTION_REF","ALL")
  IF (drdm_error_out_ind=1)
   SET stat = alterlist(dm2_ref_data_reply->qual,0)
   GO TO exit_3203
  ENDIF
  IF (idcd_check=1)
   IF ((dm2_ref_data_reply->error_ind=1))
    ...
    GO TO exit_3203
   ENDIF
   SET from_id = cnvtreal(get_value(cust_tab_name,"DCP_SECTION_REF_ID","TO"))
   IF ((global_mover_rec->context_ind=0))
    SET drm3203_context_name = drdm_chg->log[drdm_log_loop].context_name
   ELSE
    SET drm3203_context_name = drdm_oracle_context
   ENDIF
   IF (cust_beg_eff_ind != 1)
    CALL del_$r_sub(r_tab_name_dir,r_tab_name_dsr,r_tab_name_nvp)
    SELECT INTO "NL:"
     FROM (parser(r_tab_name_dsr) dsr)
     WHERE (dsr.dcp_section_ref_id=rs_3203->to_values.dcp_section_ref_id)
      AND dsr.active_ind=0
      AND (dsr.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
      AND dsr.rdds_status_flag < 9000
     DETAIL
      drm_context = dsr.rdds_context_name
     WITH nocounter
    ;end select
    IF (curqual > 0)
     SET drm3203_context_name = evaluate_context_name(drm_context,drm3203_context_name)
     UPDATE  FROM (parser(r_tab_name_dsr) dsr)
      SET rdds_context_name = drm3203_context_name, rdds_dt_tm = cnvtdatetime(curdate,curtime3),
       rdds_log_id = drdm_chg->log[drdm_log_loop].log_id,
       dsr.rdds_ptam_match_result = drm_ptam_result
      WHERE (dsr.dcp_section_ref_id=rs_3203->to_values.dcp_section_ref_id)
       AND dsr.active_ind=0
       AND (dsr.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
       AND dsr.rdds_status_flag < 9000
     ...
     ENDIF
    ELSE
     INSERT  FROM (parser(r_tab_name_dsr) dsr)
      SET dsr.active_ind = 0, dsr.end_effective_dt_tm = cnvtdatetime(cur_dt_tm), dsr
       .dcp_section_ref_id = rs_3203->to_values.dcp_section_ref_id,
       rdds_status_flag = 0, rdds_dt_tm = cnvtdatetime(curdate,curtime3), rdds_log_id = drdm_chg->
       log[drdm_log_loop].log_id,
       rdds_context_name = drm3203_context_name, rdds_source_env_id = dm2_ref_data_doc->env_source_id,
       rdds_delete_ind = 0,
       dsr.rdds_ptam_match_result = drm_ptam_result
      WITH nocounter
     ;end insert
  ...
  ENDIF
 ENDIF
 SUBROUTINE del_$r_sub(sbr_tab_name_dir,sbr_tab_name_dsr,sbr_tab_name_nvp)
   DECLARE v_context = vc WITH protect
   DECLARE fr_return = i4
   DECLARE fr_temp = i4
   IF (drs_sc_parent_ind=1)
    SET fr_temp = locateval(fr_return,1,dm2_ref_data_doc->tbl_cnt,"NAME_VALUE_PREFS",dm2_ref_data_doc
     ->tbl_qual[fr_return].table_name)
    IF (fr_temp=0)
     SET fr_temp = temp_tbl_cnt
     SET fr_return = fill_rs("TABLE","NAME_VALUE_PREFS")
     SET temp_tbl_cnt = fr_temp
     IF ((fr_return=- (2)))
      SET drdm_log_loop = redirect_to_start_row(drdm_chg,drdm_log_loop,drdm_next_row_pos,
       drdm_max_rs_size)
      GO TO exit_3203
   ...
   ENDIF
   SELECT INTO "NL:"
    FROM (parser(sbr_tab_name_dsr) dsr)
    WHERE (dsr.dcp_section_ref_id=rs_3203->to_values.dcp_section_ref_id)
     AND dsr.active_ind=1
     AND (dsr.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
     AND dsr.rdds_status_flag < 9000
    DETAIL
     v_context = dsr.rdds_context_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    GO TO exit_3203
   ENDIF
   SET drdm_chg->log[drdm_log_loop].context_name = evaluate_context_name(v_context,drdm_chg->log[
    drdm_log_loop].context_name)
   IF (drs_sc_parent_ind=1)
    SET dm_err->eproc = "Deleting children from $R tables"
    DELETE  FROM (parser(sbr_tab_name_nvp) nvp)
     WHERE nvp.parent_entity_name="DCP_INPUT_REF"
      AND nvp.rdds_status_flag < 9000
      AND (nvp.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
      AND nvp.parent_entity_id IN (
     (SELECT
      dir.dcp_input_ref_id
      FROM (parser(sbr_tab_name_dir) dir)
      WHERE dir.rdds_status_flag < 9000
       AND (dir.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
       AND dir.dcp_section_instance_id IN (
      (SELECT
       dsr.dcp_section_instance_id
       FROM (parser(sbr_tab_name_dsr) dsr)
       WHERE (dsr.dcp_section_ref_id=rs_3203->to_values.dcp_section_ref_id)
    ...
    IF (check_error(dm_err->eproc)=1)
     ...
     GO TO exit_3203
    ENDIF
    DELETE  FROM (parser(sbr_tab_name_dir) dir)
     WHERE dir.dcp_section_instance_id IN (
     (SELECT
      dsr.dcp_section_instance_id
      FROM (parser(sbr_tab_name_dsr) dsr)
      WHERE (dsr.dcp_section_ref_id=rs_3203->to_values.dcp_section_ref_id)
    ...
    ENDIF
   ENDIF
   DELETE  FROM (parser(sbr_tab_name_dsr) dsr)
    WHERE (dsr.dcp_section_ref_id=rs_3203->to_values.dcp_section_ref_id)
     AND dsr.active_ind=1
     AND (dsr.rdds_source_env_id=dm2_ref_data_doc->env_source_id)
     AND dsr.rdds_status_flag < 9000
