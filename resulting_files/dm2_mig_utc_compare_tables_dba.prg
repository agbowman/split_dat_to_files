CREATE PROGRAM dm2_mig_utc_compare_tables:dba
 ...
 SUBROUTINE dm2_get_dbase_name(dgdn_name_out)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmuct_dbtype = vc WITH protect, noconstant("")
 DECLARE dmuct_rpt_file = vc WITH protect, noconstant("dm2_mig_invalid_tables")
 DECLARE dmuct_locndx = i4 WITH protect, noconstant(0)
 DECLARE dmuct_db_link = vc WITH protect, noconstant("")
 DECLARE dmuct_standalone = i2 WITH protect, noconstant(0)
 DECLARE dmuct_dbase_name = vc WITH protect, noconstant(" ")
 DECLARE dmuct_ndx = i2 WITH protect, noconstant(0)
 FREE RECORD dmuct_nomove
 RECORD dmuct_nomove(
   1 cnt = i4
   1 qual[*]
     2 tbl = vc
 )
 DECLARE dmuct_prep(null) = i2
 DECLARE dmuct_cmp_tbl(null) = i2
 IF ((dm2_install_schema->curprog="NONE"))
  SET dmuct_standalone = 1
 ENDIF
 IF (check_logfile("dm2_mig_utc_cmp_tbl",".log","DM2_MIG_UTC_CMP_TBL LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning DM2_MIG_UTC_COMPARE_TABLES"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dmuct_dbtype = trim(cnvtupper( $1),3)
 IF (dmuct_dbtype="SOURCE")
  SET dmuct_db_link = "MIG_TARGET"
 ELSE
  SET dmuct_db_link = "REF_DATA_LINK"
 ENDIF
 IF (dmuct_prep(null)=0)
  GO TO exit_script
 ENDIF
 IF (dmuct_dbtype="SOURCE")
  IF (dm2_get_remote_dbase_name(dmuct_db_link,dmuct_dbase_name)=0)
   RETURN(0)
  ENDIF
 ELSE
  IF (dm2_get_dbase_name(dmuct_dbase_name)=0)
   RETURN(0)
  ENDIF
 ENDIF
 IF (dus_load_spec_cols(dmuct_dbase_name)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_fill_sch_except("LOCAL")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Load NOMOVE tables."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT
  IF (dmuct_dbtype="SOURCE")
   FROM dm_info d
  ...
  WHERE d.info_domain="DM2_MIG_EXCL"
   AND d.info_char="0"
  DETAIL
   IF (cnvtupper(substring(1,(findstring(".",d.info_name) - 1),d.info_name))="V500")
    dmuct_nomove->cnt = (dmuct_nomove->cnt+ 1), stat = alterlist(dmuct_nomove->qual,dmuct_nomove->cnt
     ), dmuct_nomove->qual[dmuct_nomove->cnt].tbl = substring((findstring(".",d.info_name)+ 1),size(d
 ...
 ENDIF
 SET dum_utc_invalid_tables->cnt = 0
 IF (dmuct_cmp_tbl(null)=0)
  GO TO exit_script
 ENDIF
 IF ((dum_utc_invalid_tables->cnt > 0))
  IF (get_unique_file("dmuct_rpt_file",".rpt")=0)
   GO TO exit_script
  ENDIF
  SET dmuct_rpt_file = dm_err->unique_fname
  SELECT INTO value(dmuct_rpt_file)
   FROM (dummyt d  WITH d.seq = 1)
   HEAD REPORT
    col 20, "Tables Out of Sync Between Source and Target.", row + 1,
    col 20, "---------------------------------------------", row + 1,
    row + 2, col 2, "Table Name"
   DETAIL
    FOR (dmuct_locndx = 1 TO dum_utc_invalid_tables->cnt)
      row + 2, col 2, dum_utc_invalid_tables->qual[dmuct_locndx].table_name
    ENDFOR
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  IF (((dmuct_standalone=1) OR (dmuct_dbtype="TARGET")) )
   IF (dm2_disp_file(dmuct_rpt_file," ")=0)
    GO TO exit_script
   ENDIF
  ENDIF
 ELSE
  SET dm_err->eproc = "Tables/Columns are in Sync Between Source and Target."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF ((dm_err->debug_flag > 2))
 ...
 ENDIF
 GO TO exit_script
 SUBROUTINE dmuct_prep(null)
   IF ( NOT (dmuct_dbtype IN ("SOURCE", "TARGET")))
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Validating <db_type> input parameter."
    SET dm_err->emsg = concat(dmuct_dbtype,"is invalid. Valid values are 'SOURCE','TARGET.'")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = concat("Querying from ALL_DB_LINKS to see if ",dmuct_db_link," dblink exist.")
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM all_db_links adl
    WHERE parser(build(" adl.db_link = '",dmuct_db_link,".*'"))
    DETAIL
     dmuct_db_link = adl.db_link
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (curqual=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat(dmuct_db_link," dblink does not exist. Cannot progress further.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ELSE
    SET dm_err->eproc = concat("Validating ",dmuct_db_link," database link")
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM (value(build("V$DATABASE@",dmuct_db_link)))
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ELSEIF (curqual=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Error selecting against ",dmuct_db_link," - v$database")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmuct_cmp_tbl(null)
   DECLARE dct_oracle_table1 = vc WITH protect, noconstant("")
   DECLARE dct_oracle_table2 = vc WITH protect, noconstant("")
   DECLARE dct_table1 = vc WITH protect, noconstant("")
   DECLARE dct_table2 = vc WITH protect, noconstant("")
   DECLARE dct_pos = i4 WITH protect, noconstant(0)
   DECLARE dct_col_cnvt = i2 WITH protect, noconstant(0)
   DECLARE dct_mngd_tables = vc WITH protect, noconstant("")
   DECLARE dct_loop_var = i2 WITH protect, noconstant(0)
   DECLARE dct_nomove_pos = i4 WITH protect, noconstant(0)
   SET dct_mngd_tables = ""
   IF (dmr_load_managed_tables(dct_mngd_tables)=0)
    RETURN(0)
   ENDIF
   FOR (dct_loop_var = 1 TO 2)
     IF (dct_loop_var=1)
      SET dct_oracle_table1 = "DBA_TAB_COLUMNS"
      SET dct_oracle_table2 = build("DBA_TAB_COLUMNS@",dmuct_db_link)
      SET dct_table1 = "DBA_TABLES"
      SET dct_table2 = build("DBA_TABLES@",dmuct_db_link)
     ELSE
      SET dct_oracle_table1 = build("DBA_TAB_COLUMNS@",dmuct_db_link)
      SET dct_oracle_table2 = "DBA_TAB_COLUMNS"
      SET dct_table1 = build("DBA_TABLES@",dmuct_db_link)
      SET dct_table2 = "DBA_TABLES"
     ENDIF
     SET dm_err->eproc = concat("Loading invalid tables when comparing ",trim(dct_table2)," to ",trim
      (dct_table1),".")
     CALL disp_msg(" ",dm_err->logfile,0)
     SELECT INTO "nl:"
      dlta.table_name, dlta.column_name, dlta.column_id
      FROM (
       (
       (SELECT
        table_name = utc.table_name, column_id = utc.column_id, column_name = utc.column_name
        FROM (value(dct_oracle_table2) utc),
         (value(dct_table2) t)
        WHERE  NOT (expand(dmuct_ndx,1,dm2_sch_except->tcnt,utc.table_name,dm2_sch_except->tbl[
         dmuct_ndx].tbl_name))
         AND utc.owner=currdbuser
         AND utc.owner=t.owner
         AND utc.table_name=t.table_name
         AND t.temporary="N"
         AND ((parser(concat(" t.table_name not in (",dct_mngd_tables,")"))) MINUS (
        (SELECT
         table_name = utc.table_name, column_id = utc.column_id, column_name = utc.column_name
         FROM (value(dct_oracle_table1) utc),
          (value(dct_table1) t)
         WHERE utc.owner=currdbuser
          AND utc.owner=t.owner
          AND utc.table_name=t.table_name
          AND t.temporary="N"
          AND parser(concat(" t.table_name not in (",dct_mngd_tables,")"))
         WITH sqltype("C30","I4","C30")))) ))
       dlta),
       dm_tables_doc dtd
      WHERE dtd.owner="V500"
       AND dlta.table_name=dtd.table_name
      ORDER BY 1, 2, 3
      HEAD dlta.table_name
       dmuct_tbl_fnd = 0, dmuct_tbl_fnd = locateval(dmuct_tbl_fnd,1,dus_std_convert_list->tbl_cnt,
        dlta.table_name,dus_std_convert_list->tbl[dmuct_tbl_fnd].table_name)
      DETAIL
       dmuct_col_fnd = 0, dct_col_cnvt = 0
       IF (dmuct_tbl_fnd > 0)
        dmuct_col_fnd = locateval(dmuct_col_fnd,1,dus_std_convert_list->tbl[dmuct_tbl_fnd].col_cnt,
         dlta.column_name,dus_std_convert_list->tbl[dmuct_tbl_fnd].col[dmuct_col_fnd].column_name)
        IF (dmuct_col_fnd > 0
         AND (dus_std_convert_list->tbl[dmuct_tbl_fnd].col[dmuct_col_fnd].no_convert_ind=1))
         dct_col_cnvt = 1
        ENDIF
       ENDIF
       IF (dct_col_cnvt=0)
        dct_nomove_pos = 0, dct_nomove_pos = locateval(dct_nomove_pos,1,dmuct_nomove->cnt,dlta
         .table_name,dmuct_nomove->qual[dct_nomove_pos].tbl)
        IF (dct_nomove_pos=0)
         dct_pos = 0, dct_pos = locateval(dct_pos,1,dum_utc_invalid_tables->cnt,dlta.table_name,
          dum_utc_invalid_tables->qual[dct_pos].table_name)
         IF (dct_pos=0)
          dum_utc_invalid_tables->cnt = (dum_utc_invalid_tables->cnt+ 1), stat = alterlist(
           dum_utc_invalid_tables->qual,dum_utc_invalid_tables->cnt), dum_utc_invalid_tables->qual[
          dum_utc_invalid_tables->cnt].table_name = dlta.table_name
         ENDIF
        ENDIF
       ENDIF
      WITH format, separator = " ", rdbunion
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
 ...
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "Ending DM2_MIG_UTC_CMP_TBL"
 CALL final_disp_msg("dm2_mig_utc_cmp_tbl")
END GO
