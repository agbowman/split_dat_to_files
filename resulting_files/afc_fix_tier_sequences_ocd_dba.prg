CREATE PROGRAM afc_fix_tier_sequences_ocd:dba
 ...
 FOR (count1 = 1 TO tiergroup_cv->tier_num)
  CASE (requestforscript->qual[count1].cdf_meaning)
   ...
   OF "PROCCODE":
    SET requestforscript->qual[count1].collation_seq = 21
   OF "REVENUE":
 ...
 SUBROUTINE update_codeset(i)
   SET readme_data->message = build("meaning: ",requestforscript->qual[i].cdf_meaning,
    " collatiion_seq: ",requestforscript->qual[i].collation_seq)
   ...
   IF (curqual > 0)
    ...
    SET readme_data->message = "Sequences properly updated."
   ELSE
    SET readme_data->status = "F"
    SET readme_data->message = "No sequences updated."
   ENDIF
 END ;Subroutine
