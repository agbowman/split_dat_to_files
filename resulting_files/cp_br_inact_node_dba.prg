CREATE PROGRAM cp_br_inact_node:dba
 ...
 DECLARE inactivatenoderows(null) = i2 WITH protect
 ...
 END ;Subroutine
 SUBROUTINE inactivatenoderows(null)
   ...
   DECLARE inactivename = vc WITH noconstant(""), protect
   ...
   FOR (x = 1 TO size(request->node_list,5))
     ...
     SELECT INTO "nl:"
      ...
      PLAN (cn
       WHERE (cn.cp_node_id=request->node_list[x].cp_node_id)
        AND cn.category_mean != "")
       JOIN (bdc
       WHERE bdc.category_mean=cn.category_mean)
      HEAD cn.cp_node_id
       IF (cn.category_mean != ""
        AND bdc.br_datamart_category_id != 0)
        cp_br_category_cnt += 1, stat = alterlist(del_bed_view_req->views,cp_br_category_cnt),
        del_bed_view_req->views[cp_br_category_cnt].br_datamart_category_id = bdc
        .br_datamart_category_id
       ENDIF
      WITH nocounter
     ;end select
     SET inactivename = concat(trim(trim(cnvtstring(request->node_list[x].cp_node_id),4),1),"_",trim(
       trim(request->node_list[x].cp_node_name,4),1))
     IF (size(inactivename,1) > 100)
      SET inactivename = substring(1,99,inactivename)
     ENDIF
     IF (inactivename="")
      SET inactivename = trim(trim(cnvtstring(request->node_list[x].cp_node_id),4),1)
     ENDIF
     IF (validate(debug_ind,0)=1)
      CALL echo(build("Renamed to ---> ",inactivename))
     ENDIF
     UPDATE  FROM cp_node cn
      SET cn.active_ind = 0, cn.node_name = inactivename, cn.end_effective_dt_tm = cnvtdatetime(
        sysdate),
       cn.category_mean = "", cn.updt_cnt = (cn.updt_cnt+ 1), cn.updt_dt_tm = cnvtdatetime(sysdate),
       cn.updt_task = reqinfo->updt_task, cn.updt_applctx = reqinfo->updt_applctx
      WHERE (cn.cp_node_id=request->node_list[x].cp_node_id)
     ;end update
     IF (curqual=0)
      CALL reporterror("PERFORM","Inactivate Update","Unable to updated row from CP_NODE table.")
     ...
     ENDIF
   ENDFOR
   IF (size(del_bed_view_req->views,5) > 0)
    ...
    ELSE
     ...
     SET stat = moverec(reply->status_data.subeventstatus,del_bed_view_rep->status_data.
      subeventstatus)
     GO TO exit_script
    ENDIF
   ENDIF
