CREATE PROGRAM bhs_behav_id_upd_data:dba
 PROMPT
  "behav id" = 0.0
  WITH f_behav_id
 ...
 SELECT INTO "nl:"
  ...
  HEAD b.bhs_pat_behav_ident_id
   ...
   ENDIF
   m_rec->s_rationale = b.rationale, m_rec->s_clinical_contact = b.clinical_contact
   ...
   ENDIF
   m_rec->s_intervention = b.intervention, m_rec->s_goal = b.goal, m_rec->s_approval = b.approval
   IF (findstring("OTHER",b.approval))
    ml_pos = findstring("OTHER-",b.approval), ms_tmp = substring((ml_pos+ 6),textlen(trim(b.approval)
      ),b.approval), m_rec->s_approval_ft = ms_tmp
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ps_beg_dt_tm = m_rec->s_beg_dt_tm, ps_review_dt_tm = m_rec->s_review_dt_tm, ps_reason = m_rec->
  s_reason,
  ps_reason_ft = m_rec->s_reason_ft, ps_rationale = m_rec->s_rationale, ps_clin_contact = m_rec->
  s_clinical_contact,
  ps_clin_contact_ft = m_rec->s_clin_contact_ft, ps_phys_name = m_rec->s_clin_phys_name, ps_pcp_name
   = m_rec->s_clin_pcp_name,
  ps_intervention = m_rec->s_intervention, ps_goal = m_rec->s_goal, ps_approval = m_rec->s_approval,
  ps_approval_ft = m_rec->s_approval_ft
  FROM dummyt d
  HEAD REPORT
   stat = makedataset(10)
  DETAIL
