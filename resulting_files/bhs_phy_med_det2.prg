CREATE PROGRAM bhs_phy_med_det2
 PROMPT
 ...
 SELECT DISTINCT INTO  $OUTDEV
  ...
  PLAN (d1)
   JOIN (e
   WHERE e.disch_dt_tm BETWEEN cnvtdatetime((curdate - 12),0000) AND cnvtdatetime(curdate,235959)
   ...
   WHERE o1.encntr_id=outerjoin(e.encntr_id)
    AND o1.catalog_cd=outerjoin(48115919)
    AND o1.order_status_cd=outerjoin(2543))
   JOIN (o2
  ...
  FOOT  phys
   ...
   "{F/5}{CPI/14}",
   CALL print(calcpos(198,(y_pos+ 18))), "Patients Reported:",
   ...
   CALL print(calcpos(180,(y_pos+ 36))), "Admit Recon Complete:", row + 1,
   CALL print(calcpos(306,(y_pos+ 36))), count_med_recon, perc_med_recon = concat(cnvtstring(((
     count_med_recon/ pat_count) * 100)),"%"),
   row + 1,
   CALL print(calcpos(414,(y_pos+ 36))), perc_med_recon,
   row + 1, row + 1,
   CALL print(calcpos(180,(y_pos+ 54))),
   "Home Meds Updated:", row + 1,
   CALL print(calcpos(306,(y_pos+ 54))),
   count_med_updt2, perc_med_updt = concat(cnvtstring(((count_med_updt2/ pat_count) * 100)),"%"),
   row + 1,
   CALL print(calcpos(414,(y_pos+ 54))), perc_med_updt, y_pos = (y_pos+ 77)
  FOOT REPORT
   IF (((y_pos+ 170) >= 612))
    y_pos = 0, BREAK
   ELSE
   ...
   ENDIF
   row + 1, "{F/5}{CPI/11}",
   CALL print(calcpos(353,(y_pos+ 0))),
   "Report Summary", row + 1, "{F/0}{CPI/16}",
   pat_count = count(acct_nbr), pat_count2 = count(acct_nbr), count_med_recon = count(o1.order_id
    WHERE o1.orig_order_dt_tm != null),
   count_med_updt = count(o2.order_id
    WHERE o2.orig_order_dt_tm != null), count_med_updt2 = count(o3.order_id
    WHERE diff_disch_ord="Yes"), row + 1,
   "{F/5}{CPI/14}",
   CALL print(calcpos(198,(y_pos+ 72))), "Patients Reported:",
   row + 1,
   CALL print(calcpos(306,(y_pos+ 72))), pat_count2,
   row + 1, row + 1,
   CALL print(calcpos(180,(y_pos+ 90))),
   "Admit Recon Complete:", row + 1,
   CALL print(calcpos(306,(y_pos+ 90))),
   count_med_recon, perc_med_recon = concat(cnvtstring(((count_med_recon/ pat_count) * 100)),"%"),
   row + 1,
   CALL print(calcpos(414,(y_pos+ 90))), perc_med_recon, row + 1,
   row + 1,
   CALL print(calcpos(180,(y_pos+ 108))), "Home Meds Updated:",
   row + 1,
   CALL print(calcpos(306,(y_pos+ 108))), count_med_updt2,
   perc_med_updt = concat(cnvtstring(((count_med_updt2/ pat_count) * 100)),"%"), row + 1,
   CALL print(calcpos(414,(y_pos+ 108))),
   perc_med_updt
