CREATE PROGRAM bbt_rdm_move_long_text:dba
...
#start_script
 ...
 DECLARE error_ind = i2
 DECLARE serrormsg = vc
 DECLARE nerrorstatus = i4
 DECLARE row_cnt = i4
 SET sreadme_name = "bbt_rdm_move_long_text"
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ir
   ...
   JOIN (lt
   WHERE lt.long_text_id=ir.long_text_id
    AND lt.active_ind=1
 ...
 IF (curqual > 0)
  SELECT INTO "nl:"
   ...
   PLAN (d
    WHERE d.seq <= row_cnt)
    JOIN (ltr
    WHERE (ltr.long_text_id=longtexttable->rowlist[d.seq].long_text_id))
   WITH nocounter
  ;end select
  IF (curqual=0)
   SET row_cnt = 0
   ...
   FOR (idx = 1 TO row_cnt)
    INSERT  FROM long_text_reference ltr
     SET ltr.long_text_id = longtexttable->rowlist[idx].long_text_id, ltr.long_text = longtexttable->
      rowlist[idx].long_text, ltr.parent_entity_name = longtexttable->rowlist[idx].parent_entity_name,
      ltr.parent_entity_id = longtexttable->rowlist[idx].parent_entity_id, ltr.active_ind =
      longtexttable->rowlist[idx].active_ind, ltr.active_status_cd = longtexttable->rowlist[idx].
      active_status_cd,
      ltr.active_status_dt_tm = cnvtdatetime(longtexttable->rowlist[idx].active_status_dt_tm), ltr
      .active_status_prsnl_id = longtexttable->rowlist[idx].active_status_prsnl_id, ltr.updt_cnt =
      longtexttable->rowlist[idx].updt_cnt,
      ltr.updt_dt_tm = cnvtdatetime(longtexttable->rowlist[idx].updt_dt_tm), ltr.updt_id =
      longtexttable->rowlist[idx].updt_id, ltr.updt_task = longtexttable->rowlist[idx].updt_task,
...
#exit_script
 IF (error_ind=1)
  ROLLBACK
  CALL logstatus("No long_text rows moved to long_text_reference.","F")
  CALL logmsg("No long_text rows moved to long_text_reference.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
 ELSEIF (error_ind=2)
  ROLLBACK
  CALL logstatus("Error getting lock on rows to update.  Please try again.","F")
  CALL logmsg("Error getting lock on rows to update.  Please try again.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
 ELSEIF (error_ind=99)
  CALL logstatus("No long_text rows found to move to long_text_reference.","S")
  CALL logmsg("No long_text rows found to move to long_text_reference.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
 ELSE
  COMMIT
  CALL logstatus("Long_text rows moved to long_text_reference.","S")
  CALL logmsg("Long_text rows moved to long_text_reference.",((e_rdmstatus+ e_dispmsg)+ e_logtofile))
 ENDIF
 FREE RECORD longtexttable
