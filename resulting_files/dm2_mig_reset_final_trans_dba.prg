CREATE PROGRAM dm2_mig_reset_final_trans:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmsft_db_name = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_migresetfnl_trns",".log","dm2_mig_reset_final_trans")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_reset_final_trans"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Delete downtime mock data generated by dm2_mig_mock_final_trans"
 CALL disp_msg(" ",dm_err->logfile,0)
 DELETE  FROM dm2_admin_dm_info dd
  WHERE dd.info_domain IN ("DM2MIG_DELDATA", "DM2MIG_FINAL_TRANS_CHECKPOINT", "DM2MIG_CAPDATA")
  WITH nocounter
 ;end delete
 DELETE  FROM dm_info d
  WHERE d.info_domain="DATABASE MIGRATION"
   AND d.info_name="FINAL T*"
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Alter archive log"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF ((dmr_mig_data->src_ora_level1 > 11))
  SELECT INTO "nl:"
   FROM v$database v
   DETAIL
    dmsft_db_name = trim(cnvtupper(v.name))
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET dm2_install_schema->dbase_name = dmsft_db_name
  SET dm2_install_schema->u_name = "SYS"
  SET dm2_force_connect_string = 1
  SET dm_err->eproc = "Prompting for SOURCE container connection."
  CALL disp_msg("",dm_err->logfile,0)
 ...
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Execute stop compare module"
 ...
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Reset compare status"
 UPDATE  FROM dm_vdata_master d
  SET d.compare_status = "MATCH"
  WHERE d.compare_status="COMPLETE"
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "To reset downtime bulk data move"
 EXECUTE dm2_mig_dt_bdm_reset
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
#exit_program
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "dm2_mig_reset_final_trans completed"
 ENDIF
 CALL final_disp_msg("dm2_migresetfnl_trns")
END GO
