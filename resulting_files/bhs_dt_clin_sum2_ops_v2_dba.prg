CREATE PROGRAM bhs_dt_clin_sum2_ops_v2:dba
 SET unit_cd = 0
 SET printer_name = trim( $1)
 SELECT INTO "nl:"
  FROM code_value cv
  WHERE cv.code_set=220
   AND cv.cdf_meaning != "ROOM"
   AND trim(cv.display)=trim( $2)
   AND cv.active_ind=1
  DETAIL
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   pat->qual[d.seq].dob = cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1)
      ...
      FOR (z1 = 1 TO size(scheduled_orders->qual,5))
        ...
        FOR (z2 = 1 TO size(scheduled_orders_disp->scheduled_orders,5))
          ...
          template_order_id))
           ...
           ycol += 8, tempstring = concat("Order Detail :",scheduled_orders_disp->scheduled_orders[z2
            ].core_actions[1].clinical_display_line), xcol = 50,
