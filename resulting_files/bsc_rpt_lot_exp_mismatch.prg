CREATE PROGRAM bsc_rpt_lot_exp_mismatch
 PROMPT
 ...
 DECLARE i18n_stitle = vc WITH protect, constant(trim(uar_i18ngetmessage(i18nhandle,"i18n_TITLE",
    "Immunization Lot Expiration Mismatch Report"),3))
 ...
 DECLARE i18n_sexpecteddt = vc WITH protect, constant(trim(uar_i18ngetmessage(i18nhandle,
    "i18n_EXPECTEDDT","Expected"),3))
 DECLARE i18n_sdocumenteddt = vc WITH protect, constant(trim(uar_i18ngetmessage(i18nhandle,
    "i18n_DOCUMENTEDDT","Documented"),3))
 ...
 DECLARE i18n_snone = vc WITH protect, constant(trim(uar_i18ngetmessage(i18nhandle,"i18n_NONE","None"
    ),3))
 ...
 SUBROUTINE (generatereport(null) =null)
   SELECT
    IF (( $DISPLAY_TYPE=display_per_day))
     sort_ord = ce.event_end_dt_tm, sort_ord2 = prsnl.name_full_formatted, sort_ord3 =
     uar_get_code_display(e.loc_nurse_unit_cd),
     datetime = ce.event_end_dt_tm, patient_name = person.name_full_formatted, prsnl_name = prsnl
     .name_full_formatted,
     hna_order_mnemonic = o.hna_order_mnemonic, ordered_as_mnemonic = o.ordered_as_mnemonic,
     order_mnemonic = o.order_mnemonic,
     nurse_unit_cd = e.loc_nurse_unit_cd, lot_number = cmr.substance_lot_number, event_id = ce
     .event_id,
     expected_exp_dt = lni.exp_dt_tm, documented_exp_dt = cmr.substance_exp_dt_tm
    ELSEIF (( $DISPLAY_TYPE=display_per_user))
     sort_ord = prsnl.name_full_formatted, sort_ord2 = ce.event_end_dt_tm, sort_ord3 =
     uar_get_code_display(e.loc_nurse_unit_cd),
     datetime = ce.event_end_dt_tm, orderid = ce.order_id, nurse_unit_cd = e.loc_nurse_unit_cd,
     patient_name = person.name_full_formatted, prsnl_name = prsnl.name_full_formatted,
     hna_order_mnemonic = o.hna_order_mnemonic,
     ordered_as_mnemonic = o.ordered_as_mnemonic, order_mnemonic = o.order_mnemonic, lot_number = cmr
     .substance_lot_number,
     expected_exp_dt = lni.exp_dt_tm, documented_exp_dt = cmr.substance_exp_dt_tm
    ELSEIF (( $DISPLAY_TYPE=display_per_nurse_unit))
     sort_ord = uar_get_code_display(e.loc_nurse_unit_cd), sort_ord2 = ce.event_end_dt_tm, sort_ord3
    ...
    PLAN (e
     ...
     WHERE o.order_id=ce.order_id
      AND o.active_ind=1)
     JOIN (lni
     WHERE cnvtupper(cmr.substance_lot_number)=lni.lot_number_txt
      AND ((lni.exp_dt_tm = null) OR (datetimecmputc(lni.exp_dt_tm,cmr.substance_exp_dt_tm,2) != 0))
      AND lni.active_ind=1)
    ORDER BY sort_ord, sort_ord2, sort_ord3
    HEAD PAGE
     IF ( NOT (( $1 IN ("MINE"))))
     ...
     sdisplay, sdisplay = formatlabelbylength(i18n_sexpecteddt,9), col 112,
     sdisplay, sdisplay = formatlabelbylength(i18n_sdocumenteddt,10), col 121,
    ...
    DETAIL
     ...
     sdisplay = format(expected_exp_dt,"@SHORTDATE")
     IF (sdisplay="")
      sdisplay = i18n_snone
     ENDIF
     col 112, sdisplay, sdisplay = format(documented_exp_dt,"@SHORTDATE"),
