CREATE PROGRAM agcrxreqgen01:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 FOR (i = 1 TO tprint_req->job_knt)
   SELECT INTO value(tprint_req->job[i].print_loc)
    org = substring(1,30,o.org_name), location = substring(1,30,uar_get_code_display(tprint_req->job[
      i].location_cd)), street = substring(1,30,concat(trim(tprint_req->job[i].loc_addr),char(32),
      trim(tprint_req->job[i].loc_addr2))),
    csz = substring(1,30,concat(trim(tprint_req->job[i].loc_city),char(32),trim(tprint_req->job[i].
       loc_state),char(32),trim(tprint_req->job[i].loc_zip))), phone = substring(1,30,concat(
      "PHONE: ",char(32),tprint_req->job[i].loc_ph))
    ...
    HEAD REPORT
     ...
     size_fac = 0, x_pos = top_left_x, y_pos = top_left_y,
     row + 4, col 0,
     CALL center(org,0,66),
     row + 1, col 0,
     CALL center(location,0,66),
     row + 1, col 0,
     CALL center(street,0,66),
     row + 1, col 0,
     CALL center(csz,0,66),
     row + 1, col 0,
     CALL center(phone,0,66),
     row + 2, col 5, "DATE: ",
     tprint_req->job[i].req[1].start_dt, row + 1, col 5,
     demo_info->pat_name, row + 1, col 5,
     "DATE OF BIRTH: ", demo_info->pat_bday, col 5,
     demo_info->pat_addr, row + 1, col 5,
     demo_info->pat_city, row + 2, col 5,
     "RX:", row + 1, col 5,
     tprint_req->job[i].req[1].med[1].disp, row + 1, blob_out2 = tprint_req->job[i].req[1].
     ord_det_disp,
     eol = size(trim(blob_out2),1), bseg = 1, eseg = 1,
     temp = 0
     WHILE (eseg <= eol)
       IF (((row+ 4) >= maxrow))
        BREAK
       ENDIF
       temp = (temp+ 1), bseg = eseg, eseg = (eseg+ 50)
       IF (findstring(" ",substring(bseg,(eseg - bseg),blob_out2)) > 0)
        WHILE (substring((eseg - 1),1,blob_out2) != " "
         AND eseg != bseg)
          eseg = (eseg - 1)
        ENDWHILE
        segment = substring(1,eseg,substring(bseg,(eseg - bseg),blob_out2))
       ELSE
        segment = substring(1,eseg,substring(bseg,(eseg - bseg),blob_out2))
       ENDIF
       col 5, segment, row + 1
     ENDWHILE
     col 5, "REFILL: ", tprint_req->job[i].req[1].refill[1].disp,
     row + 5, col 5, "DX: ______________________________________",
     row + 2, col 5, "SIGNATURE:",
     row + 10, col 5, "DEA NUMBER: ______________________________",
     row + 1, col 5, "-*-"
    WITH dio = 8
   ;end select
 ENDFOR
