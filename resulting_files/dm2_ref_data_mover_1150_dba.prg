CREATE PROGRAM dm2_ref_data_mover_1150:dba
 ...
 SET dmt_cnt = 0
 SET null_check2 = get_nullind(cust_tab_name,"KEY2_ENTITY_NAME")
 SET null_check3 = get_nullind(cust_tab_name,"KEY3_ENTITY_NAME")
 SET null_check4 = get_nullind(cust_tab_name,"KEY4_ENTITY_NAME")
 SET null_check5 = get_nullind(cust_tab_name,"KEY5_ENTITY_NAME")
 SET id2_vc = get_value(cust_tab_name,"KEY2_ID","FROM")
 SET id3_vc = get_value(cust_tab_name,"KEY3_ID","FROM")
 SET id4_vc = get_value(cust_tab_name,"KEY4_ID","FROM")
 SET id5_vc = get_value(cust_tab_name,"KEY5_ID","FROM")
 ...
 ENDIF
 IF (null_check2=0
  AND (rs_1150->from_values.key2_entity_name=""))
  SET null_check2 = 1
 ENDIF
 IF (null_check3=0
  AND (rs_1150->from_values.key3_entity_name=""))
  SET null_check3 = 1
 ENDIF
 IF (null_check4=0
  AND (rs_1150->from_values.key4_entity_name=""))
  SET null_check4 = 1
 ENDIF
 IF (null_check5=0
  AND (rs_1150->from_values.key5_entity_name=""))
  SET null_check5 = 1
 ENDIF
 IF (null_check2=1
  AND cnvtreal(id2_vc) > 0)
  SET to_id = drdm_smt(rdds_exception,"KEY2_ID","CODE_VALUE","CODE_VALUE",id2_vc,
   "CODE_VALUE")
  IF (to_id != "No Trans")
   CALL put_value(cust_tab_name,"KEY2_ID",to_id)
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1150
   ENDIF
  ELSE
   SET cust_rpt_missing = report_missing("CODE_VALUE",rdds_exception->qual[size(rdds_exception->qual,
     5)].tru_col_name,cnvtreal(id2_vc))
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ENDIF
 ENDIF
 IF (type_mean="INTERVALCODE")
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[k2pos].translated = 0
  SET to_id = drdm_smt(rdds_exception,"KEY2_ID","ITEM_INTERVAL_TABLE","ITEM_INTERVAL_ID",id2_vc,
   "ITEM_INTERVAL_TABLE")
  IF (to_id != "No Trans")
   CALL put_value(cust_tab_name,"KEY2_ID",to_id)
   IF ((dm2_ref_data_reply->error_ind=1))
   ...
   ENDIF
  ELSE
   SET cust_rpt_missing = report_missing("ITEM_INTERVAL_TABLE",rdds_exception->qual[size(
     rdds_exception->qual,5)].tru_col_name,cnvtreal(id2_vc))
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ENDIF
 ENDIF
 SET dmt_cnt = 0
 IF (null_check3=1
  AND cnvtreal(id3_vc) > 0)
  SET to_id = drdm_smt(rdds_exception,"KEY3_ID","NOMENCLATURE","NOMENCLATURE_ID",id3_vc,
   "NOMENCLATURE")
  IF (to_id != "No Trans")
   CALL put_value(cust_tab_name,"KEY3_ID",to_id)
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1150
   ENDIF
  ELSE
   SET to_id = select_merge_translate(id3_vc,"WORKLOAD_CODE")
   IF (to_id != "No Trans")
    CALL put_value(cust_tab_name,"KEY3_ID",to_id)
    IF ((dm2_ref_data_reply->error_ind=1))
     GO TO exit_1150
    ENDIF
   ELSE
    SET cust_rpt_missing = report_missing("NOMENCLATURE",rdds_exception->qual[size(rdds_exception->
      qual,5)].tru_col_name,cnvtreal(id3_vc))
    CALL evaluate_rpt_missing(cust_rpt_missing)
   ENDIF
  ENDIF
 ENDIF
 SET dmt_cnt = 0
 IF (null_check4=1
  AND cnvtreal(id4_vc) > 0)
  SET to_id = drdm_smt(rdds_exception,"KEY4_ID","CODE_VALUE","CODE_VALUE",id4_vc,
   "CODE_VALUE")
  IF (to_id != "No Trans")
   CALL put_value(cust_tab_name,"KEY4_ID",to_id)
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1150
   ENDIF
  ELSE
   SET cust_rpt_missing = report_missing("CODE_VALUE",rdds_exception->qual[size(rdds_exception->qual,
     5)].tru_col_name,cnvtreal(id4_vc))
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ENDIF
 ENDIF
 SET dmt_cnt = 0
 IF (null_check5=1
  AND cnvtreal(id5_vc) > 0)
  SET to_id = drdm_smt(rdds_exception,"KEY5_ID","CODE_VALUE","CODE_VALUE",id5_vc,
   "CODE_VALUE")
  IF (to_id != "No Trans")
   CALL put_value(cust_tab_name,"KEY5_ID",to_id)
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1150
   ENDIF
  ELSE
   SET cust_rpt_missing = report_missing("CODE_VALUE",rdds_exception->qual[size(rdds_exception->qual,
     5)].tru_col_name,cnvtreal(id5_vc))
   CALL evaluate_rpt_missing(cust_rpt_missing)
  ENDIF
 ENDIF
