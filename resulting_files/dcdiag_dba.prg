CREATE PROGRAM dcdiag:dba
 ...
 DECLARE ml_working_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",17,"WORKING"))
 DECLARE ml_discharge_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",17,"DISCHARGE"))
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (dx
   WHERE (dx.person_id=request->person_id)
    ...
    AND dx.diag_type_cd IN (ml_working_cd, ml_discharge_cd)
    AND dx.active_ind=1
