CREATE PROGRAM dm2_create_system_defs:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_system_defs",".log","DM2_CREATE_SYSTEM_DEFS LOGFILE")=0)
  GO TO exit_views
 ENDIF
 DECLARE current_user = vc WITH protect, constant(cnvtupper(currdbuser))
 DECLARE dba_version = vc WITH protect, noconstant(" ")
 DECLARE view_command = vc WITH protect, noconstant(" ")
 DECLARE dm2_max_reclen = i4 WITH public, noconstant(0)
 DECLARE dcsd_dm_info_exists = i2 WITH protect, noconstant(0)
 DECLARE from_clause1 = vc
 DECLARE from_clause2 = vc
 DECLARE from_clause3 = vc
 DECLARE dsd_parse_str = vc WITH protect, noconstant(" ")
 SET dm_err->eproc = "Beginning DM2_CREATE_SYSTEM_DEFS"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_table_and_ccldef_exists("DM_INFO",dcsd_dm_info_exists)=0)
  GO TO exit_views
 ENDIF
 IF (dcsd_dm_info_exists=1)
  SET dm_err->eproc = "Check for dm_info row"
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="DM2_CREATE_SYSTEM_DEFS_PROCESS"
    AND di.info_name="SKIP_EXECUTION"
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF (curqual=1)
   SET dm_err->eproc = "Skipping DM2_CREATE_SYSTEM_DEFS process due to existence of bypass row."
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_views
  ENDIF
 ENDIF
 IF ((validate(oragen_sys_ind,- (1))=- (1)))
  DECLARE oragen_sys_ind = i2 WITH public, noconstant(1)
 ELSE
  SET oragen_sys_ind = 1
 ENDIF
 IF ((validate(dm2_oragen_system_defs,- (1))=- (1))
  AND (validate(dm2_oragen_system_defs,- (2))=- (2)))
  DECLARE dm2_oragen_system_defs = i2 WITH protect, constant(0)
 ENDIF
 IF ( NOT (dm2_oragen_system_defs IN (0, 1)))
  SET dm2_oragen_system_defs = 0
 ENDIF
 DECLARE create_oracle_sys_defs(null) = i4
 DECLARE oragen_oracle_sys_defs(null) = i4
 DECLARE dcsd_chk_drop_view(vname=vc) = i2
 DECLARE dcsd_chk_drop_syn(sname=vc) = i2
 SET dm2_install_schema->oragen3_ignore_dm_columns_doc = 1
 SELECT INTO "nl:"
  f.max_reclen
  FROM dfile f
  WHERE cnvtupper(f.file_name)="V500"
  DETAIL
   dm2_max_reclen = cnvtint(f.max_reclen)
  WITH nocounter
 ;end select
 IF (dm2_max_reclen < 55000)
  UPDATE  FROM dfile f
   SET f.max_reclen = 55000
   WHERE cnvtupper(f.file_name)="V500"
   WITH nocounter
  ;end update
 ENDIF
 IF (currdb="ORACLE")
  IF (dm2_oragen_system_defs=0)
   IF (create_oracle_sys_defs(null)=0)
    GO TO exit_views
   ENDIF
  ENDIF
  IF (oragen_oracle_sys_defs(null)=0)
   GO TO exit_views
  ENDIF
 ENDIF
 GO TO exit_views
 SUBROUTINE create_oracle_sys_defs(null)
   SET dm_err->eproc = "ORACLE DATABASE DETECTED.  CREATING TABLE DEFS."
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_system_defs_init(1)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "CREATING DM2_* TABLE DEFS"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_get_rdbms_version(null)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_TABLES VIEW"
   RDB create or replace view dm2_dba_tables as select * from dba_tables
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_TABLES VIEW"
   CALL parser("rdb create or replace view dm2_user_tables")
   CALL parser("as select * from dm2_dba_tables")
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_TAB_COLUMNS VIEW"
   CALL parser("rdb create or replace view dm2_user_tab_columns")
   CALL parser("as select * from dm2_dba_tab_columns")
   CALL parser(concat("where owner = '",current_user,"'"))
   CALL parser("go")
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_TAB_COLS VIEW"
   CALL parser("rdb create or replace view dm2_user_tab_cols")
   CALL parser("as select * from dm2_dba_tab_cols")
   CALL parser(concat("where owner = '",current_user,"'"))
   CALL parser("go")
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_INDEXES VIEW"
   RDB create or replace view dm2_dba_indexes as select * from dba_indexes
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_INDEXES VIEW"
   IF ((dm2_rdbms_version->level1=9))
    RDB create or replace view dm2_user_indexes ( index_name , index_type , table_owner , table_name
    , table_type , uniqueness , compression , prefix_length , tablespace_name , ini_trans , max_trans
     , initial_extent , next_extent , min_extents , max_extents , pct_increase , pct_threshold ,
    include_column , freelists , freelist_groups , pct_free , logging , blevel , leaf_blocks ,
    distinct_keys , avg_leaf_blocks_per_key , avg_data_blocks_per_key , clustering_factor , status ,
    num_rows , sample_size , last_analyzed , degree , instances , partitioned , temporary , generated
     ...
     parameters , global_stats , domidx_status , domidx_opstatus , funcidx_status , join_index ,
    visibility ) as select t . index_name , t . index_type , t . table_owner , t . table_name , t .
    table_type , t . uniqueness , t . compression , t . prefix_length , t . tablespace_name , t .
    ini_trans , t . max_trans , t . initial_extent , t . next_extent , t . min_extents , t .
    max_extents , t . pct_increase , t . pct_threshold , t . include_column , t . freelists , t .
    freelist_groups , t . pct_free , t . logging , t . blevel , t . leaf_blocks , t . distinct_keys ,
     t . avg_leaf_blocks_per_key , t . avg_data_blocks_per_key , t . clustering_factor , t . status ,
     t . num_rows , t . sample_size , t . last_analyzed , t . degree , t . instances , t .
    partitioned , t . temporary , t . generated , t . secondary , t . buffer_pool , t . user_stats ,
    t . duration , t . pct_direct_access , t . ityp_owner , t . ityp_name , t . parameters , t .
    global_stats , t . domidx_status , t . domidx_opstatus , t . funcidx_status , t . join_index ,
    cast ( "VISIBLE" as varchar2 ( 9 ) ) from user_indexes t
    END ;Rdb
   ELSEIF ((dm2_rdbms_version->level1=10))
    RDB create or replace view dm2_user_indexes ( index_name , index_type , table_owner , table_name
    , table_type , uniqueness , compression , prefix_length , tablespace_name , ini_trans , max_trans
     , initial_extent , next_extent , min_extents , max_extents , pct_increase , pct_threshold ,
    include_column , freelists , freelist_groups , pct_free , logging , blevel , leaf_blocks ,
    ...
    num_rows , sample_size , last_analyzed , degree , instances , partitioned , temporary , generated
     , secondary , buffer_pool , user_stats , duration , pct_direct_access , ityp_owner , ityp_name ,
     parameters , global_stats , domidx_status , domidx_opstatus , funcidx_status , join_index ,
    iot_redundant_pkey_elim , dropped , visibility ) as select t . index_name , t . index_type , t .
    table_owner , t . table_name , t . table_type , t . uniqueness , t . compression , t .
    prefix_length , t . tablespace_name , t . ini_trans , t . max_trans , t . initial_extent , t .
    next_extent , t . min_extents , t . max_extents , t . pct_increase , t . pct_threshold , t .
    include_column , t . freelists , t . freelist_groups , t . pct_free , t . logging , t . blevel ,
    t . leaf_blocks , t . distinct_keys , t . avg_leaf_blocks_per_key , t . avg_data_blocks_per_key ,
     t . clustering_factor , t . status , t . num_rows , t . sample_size , t . last_analyzed , t .
    degree , t . instances , t . partitioned , t . temporary , t . generated , t . secondary , t .
    buffer_pool , t . user_stats , t . duration , t . pct_direct_access , t . ityp_owner , t .
    ityp_name , t . parameters , t . global_stats , t . domidx_status , t . domidx_opstatus , t .
    funcidx_status , t . join_index , t . iot_redundant_pkey_elim , t . dropped , cast ( "VISIBLE" as
     varchar2 ( 9 ) ) from user_indexes t
    END ;Rdb
   ELSE
    RDB create or replace view dm2_user_indexes as select * from user_indexes
    END ;Rdb
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_IND_COLUMNS VIEW"
   RDB create or replace view dm2_dba_ind_columns as select * from dba_ind_columns
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "CREATE & DROP DM2_USER_IND_COLUMNS VIEW"
   CALL parser("rdb create or replace view dm2_user_ind_columns")
   CALL parser("as select * from dm2_dba_ind_columns")
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_CONSTRAINTS VIEW"
   RDB create or replace view dm2_dba_constraints as select * from dba_constraints
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_CONSTRAINTS VIEW"
   CALL parser("rdb create or replace view dm2_user_constraints")
   CALL parser("as select * from dm2_dba_constraints")
   CALL parser(concat("where owner = '",current_user,"'"))
   CALL parser("go")
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_CONS_COLUMNS VIEW"
   RDB create or replace view dm2_dba_cons_columns as select * from dba_cons_columns
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_CONS_COLUMNS VIEW"
   CALL parser("rdb create or replace view dm2_user_cons_columns")
   CALL parser("as select * from dm2_dba_cons_columns")
   CALL parser(concat("where owner = '",current_user,"'"))
   CALL parser("go")
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_SEQUENCES VIEW"
   RDB create or replace view dm2_dba_sequences as select * from dba_sequences
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_SEQUENCES VIEW"
   CALL parser("rdb create or replace view dm2_user_sequences")
   CALL parser("as select * from dm2_dba_sequences")
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_VIEWS VIEW"
   RDB create or replace view dm2_dba_views as select * from dba_views
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_VIEWS VIEW"
   CALL parser("rdb create or replace view dm2_user_views")
   CALL parser("as select * from dm2_dba_views")
   CALL parser(concat("where owner = '",current_user,"'"))
   CALL parser("go")
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_SYNONYMS VIEW"
   RDB create or replace view dm2_dba_synonyms as select * from dba_synonyms
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_SYNONYMS VIEW"
   CALL parser("rdb create or replace view dm2_user_synonyms")
   CALL parser("as select * from dm2_dba_synonyms")
   CALL parser(concat("where owner = '",current_user,"'"))
   CALL parser("go")
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_TABLESPACES VIEW"
   IF ((dm2_rdbms_version->level1=9))
    RDB create or replace view dm2_dba_tablespaces ( tablespace_name , initial_extent , next_extent ,
     min_extents , max_extents , pct_increase , status , contents , tbspace_type , pagesize , ngname
    , bpname , min_extlen , logging , extent_management , allocation_type , segment_space_management
    , bigfile ) as select t . tablespace_name , t . initial_extent , t . next_extent , t .
    min_extents , t . max_extents , t . pct_increase , t . status , t . contents , "N/A" , 0 , cast (
     "N/A" as varchar2 ( 18 ) ) , cast ( "N/A" as varchar2 ( 18 ) ) , t . min_extlen , t . logging ,
    t . extent_management , t . allocation_type , t . segment_space_management , cast ( "NO" as
    varchar2 ( 3 ) ) from dba_tablespaces t
    END ;Rdb
   ELSE
    RDB create or replace view dm2_dba_tablespaces as select * from dba_tablespaces
    END ;Rdb
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_TABLESPACES VIEW"
   RDB create or replace view dm2_user_tablespaces as select * from dm2_dba_tablespaces
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_TRIGGERS VIEW"
   RDB create or replace view dm2_dba_triggers as select * from dba_triggers
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_TRIGGERS VIEW"
   CALL parser("rdb create or replace view dm2_user_triggers")
   CALL parser("as select * from dm2_dba_triggers")
   CALL parser(concat("where owner = '",current_user,"'"))
   CALL parser("go")
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_OBJECTS VIEW"
   RDB create or replace view dm2_dba_objects as select * from dba_objects
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "CREATING DM2_USER_OBJECTS VIEW"
   CALL parser("rdb create or replace view dm2_user_objects")
   CALL parser("as select * from dm2_dba_objects")
   CALL parser(concat("where owner = '",current_user,"'"))
   CALL parser("go")
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_FREE_SPACE VIEW"
   RDB create or replace view dm2_dba_free_space as select * from dba_free_space
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_DATA_FILES VIEW"
   RDB create or replace view dm2_dba_data_files as select * from dba_data_files
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ...
   IF ((dm2_rdbms_version->level1 < 12))
    SET dm_err->eproc = "CREATE OR REPLACE DM2_GET_COND FUNCTION"
    SET dsd_parse_str = "rdb asis (^CREATE OR REPLACE FUNCTION dm2_get_cond ^)"
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^  ( ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^    con_name     VARCHAR2,  ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^    owner_name   VARCHAR2   ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^  )  ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^  RETURN VARCHAR2 AS  ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^  ser_cond VARCHAR2(2000);  ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^  BEGIN ^)")
    IF ((dm2_rdbms_version->level1 >= 11))
     SET dsd_parse_str = concat(dsd_parse_str,"    asis (^    SELECT  /*+ ALL_ROWS */  ^)")
    ELSE
     SET dsd_parse_str = concat(dsd_parse_str,"    asis (^    SELECT  ^)")
    ENDIF
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^     search_condition INTO ser_cond  ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^    FROM dba_constraints ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^    WHERE  ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^      constraint_name=con_name ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^      AND owner=owner_name; ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^    RETURN ser_cond; ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^  EXCEPTION ^)")
    SET dsd_parse_str = concat(dsd_parse_str,
     "    asis (^    when VALUE_ERROR then return('DM2_VALUE_ERROR'); ^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^ END; ^) go")
    CALL parser(dsd_parse_str,1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Checking the validity of the DM2_GET_COND function."
    SELECT INTO "nl:"
     uo.status
     FROM user_objects uo
     WHERE uo.object_name="DM2_GET_COND"
    ...
    IF (curqual)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "The DM2_GET_COND function is INVALID."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_DBA_NOTNULL_COLS VIEW"
   IF ((dm2_rdbms_version->level1 >= 12))
    SET dsd_parse_str =
    "rdb asis (^create or replace view dm2_dba_notnull_cols (OWNER, TABLE_NAME, COLUMN_NAME) ^)"
    SET dsd_parse_str = concat(dsd_parse_str,"asis (^ as select dc.owner, dc.table_name, ^)")
    SET dsd_parse_str = concat(dsd_parse_str,
     "asis (^replace(replace(translate(upper(dc.search_condition_vc),")
    SET dsd_parse_str = concat(dsd_parse_str,^'()"',' '),' '),'ISNOTNULL')^)
    SET dsd_parse_str = concat(dsd_parse_str,"^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^from dba_constraints dc ^)")
    SET dsd_parse_str = concat(dsd_parse_str,
     "    asis (^where dc.constraint_type = 'C' and dc.status = 'ENABLED' ^)")
    SET dsd_parse_str = concat(dsd_parse_str,
     "asis (^  and upper(replace(dc.search_condition_vc,' ')) like '%ISNOTNULL%' ^) go")
   ELSE
    SET dsd_parse_str =
    "rdb asis (^create or replace view dm2_dba_notnull_cols (OWNER, TABLE_NAME, COLUMN_NAME) ^)"
    SET dsd_parse_str = concat(dsd_parse_str,"asis (^ as select dc.owner, dc.table_name, ^)")
    SET dsd_parse_str = concat(dsd_parse_str,
     "asis (^replace(replace(translate(upper(dm2_get_cond(dc.constraint_name,dc.owner)),")
    SET dsd_parse_str = concat(dsd_parse_str,^'()"',' '),' '),'ISNOTNULL')^)
    SET dsd_parse_str = concat(dsd_parse_str,"^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^from dba_constraints dc ^)")
    SET dsd_parse_str = concat(dsd_parse_str,
     "    asis (^where dc.constraint_type = 'C' and dc.status = 'ENABLED' ^)")
    SET dsd_parse_str = concat(dsd_parse_str,
     "asis (^  and upper(replace(dm2_get_cond(dc.constraint_name,dc.owner),' ')) like '%ISNOTNULL%' ^) go"
     )
   ENDIF
   CALL parser(dsd_parse_str,1)
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_USER_NOTNULL_COLS VIEW"
   IF ((dm2_rdbms_version->level1 >= 12))
    SET dsd_parse_str =
    "rdb asis (^create or replace view dm2_user_notnull_cols (OWNER, TABLE_NAME, COLUMN_NAME) ^)"
    SET dsd_parse_str = concat(dsd_parse_str,"asis (^ as select dc.owner, dc.table_name, ^)")
    SET dsd_parse_str = concat(dsd_parse_str,
     "asis (^ replace(replace(translate(upper(dc.search_condition_vc),")
    SET dsd_parse_str = concat(dsd_parse_str,^'()"',' '),' '),^)
    SET dsd_parse_str = concat(dsd_parse_str,"'ISNOTNULL')^)")
    SET dsd_parse_str = concat(dsd_parse_str,"    asis (^from user_constraints dc ^)")
    SET dsd_parse_str = concat(dsd_parse_str,
     "    asis (^where dc.constraint_type = 'C' and dc.status = 'ENABLED' ^)")
    SET dsd_parse_str = concat(dsd_parse_str,
     "asis (^  and upper(replace(dc.search_condition_vc,' ')) like '%ISNOTNULL%' ^) go")
   ...
   IF ((dm2_rdbms_version->level1 > 10))
    SET dsd_parse_str = ""
    IF ((dm2_rdbms_version->level1 >= 12))
     SET dsd_parse_str = concat("create or replace view dm2_objects_in_parse as",
      " with sql_parsing as (",
      " select a.inst_id,a.session_id,a.session_serial#,a.sql_id,a.sql_child_number,a.con_id,",
      "    (max(cast(a.sample_time as date)) - min(cast(a.sample_time as date))) * 24 * 60 * 60 as seconds_diff",
      "  from",
      "      gv$active_session_history a, v$pdbs p","   where p.con_id = a.con_id",
      "      and a.in_hard_parse = 'Y'","      and a.sample_time > sysdate - (1/1440)",
      "      and a.sql_id is not null",
      " group by a.inst_id,a.session_id,a.session_serial#,a.sql_id,a.sql_child_number,a.con_id)",
      " select /*+leading(s)*/ d.inst_id,s.con_id,s.session_id,s.session_serial#,d.to_owner,",
      " d.to_name,s.sql_id,s.seconds_diff as last_call_et",
      " from sql_parsing s, gv$session v, gv$object_dependency d"," where s.sql_id = v.sql_id",
      " and s.con_id = v.con_id"," and s.inst_id = v.inst_id",
      " and s.sql_child_number = v.sql_child_number"," and s.session_serial#=v.serial#",
      " and d.from_hash = v.sql_hash_value",
      " and d.to_owner is not null"," and d.inst_id = v.inst_id"," and d.con_id = v.con_id")
    ELSE
     SET dsd_parse_str = concat("create or replace view dm2_objects_in_parse as",
      " with sql_parsing as ("," select inst_id,session_id,session_serial#,sql_id,sql_child_number, ",
      "    (max(cast(sample_time as date)) - min(cast(sample_time as date))) * 24 * 60 * 60 as seconds_diff",
      "  from",
      "      gv$active_session_history","   where in_hard_parse = 'Y'",
      "      and sample_time > sysdate - (1/1440)","      and sql_id is not null",
      " group by inst_id,session_id,session_serial#,sql_id,sql_child_number)",
      " select /*+leading(s)*/ d.inst_id,s.session_id,s.session_serial#,d.to_owner,",
      " d.to_name,s.sql_id,s.seconds_diff as last_call_et",
      " from sql_parsing s, gv$session v, gv$object_dependency d"," where s.sql_id = v.sql_id",
      " and s.inst_id = v.inst_id",
      " and s.sql_child_number = v.sql_child_number"," and s.session_serial#=v.serial#",
      " and d.from_hash = v.sql_hash_value"," and d.to_owner is not null",
      " and d.inst_id = v.inst_id")
    ENDIF
    CALL parser(concat("rdb asis(^",dsd_parse_str,"^) go"),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
    EXECUTE oragen3 "DM2_OBJECTS_IN_PARSE"
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   RDB create or replace public synonym dm_user_tables_actual_stats for sys . user_tables
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_USER_TABLES_ACTUAL_STATS")=0)
    RETURN(0)
   ENDIF
   RDB create or replace public synonym dm_user_columns_actual_stats for sys . user_tab_columns
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_USER_COLUMNS_ACTUAL_STATS")=0)
    RETURN(0)
   ENDIF
   RDB create or replace public synonym dm_user_indexes_actual_stats for sys . user_indexes
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_USER_INDEXES_ACTUAL_STATS")=0)
    RETURN(0)
   ENDIF
   RDB create or replace public synonym dm_dba_tables_actual_stats for sys . dba_tables
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_DBA_TABLES_ACTUAL_STATS")=0)
    RETURN(0)
   ENDIF
   RDB create or replace public synonym dm_dba_columns_actual_stats for sys . dba_tab_columns
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_DBA_COLUMNS_ACTUAL_STATS")=0)
    RETURN(0)
   ENDIF
   RDB create or replace public synonym dm_dba_indexes_actual_stats for sys . dba_indexes
   END ;Rdb
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_DBA_INDEXES_ACTUAL_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_USER_TABLES_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_USER_COLUMNS_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_USER_INDEXES_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_DBA_TABLES_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_DBA_COLUMNS_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_view("DM_DBA_INDEXES_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_syn("DM_USER_TABLES_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_syn("DM_USER_COLUMNS_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_syn("DM_USER_INDEXES_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_syn("DM_DBA_TABLES_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_syn("DM_DBA_COLUMNS_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   IF (dcsd_chk_drop_syn("DM_DBA_INDEXES_GOLDAVG_STATS")=0)
    RETURN(0)
   ENDIF
   EXECUTE oragen3 "SYS.DM_*_ACTUAL_STATS"
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "DROP & CREATE DM2_INDEX_USAGE VIEW"
   IF ((dm2_rdbms_version->level1=11))
    RDB create or replace view dm2_index_usage ( owner , index_name , table_name , monitoring , used
    , start_monitoring , end_monitoring ) as select u . name , io . name , t . name , decode ( bitand
     ( i . flags , 65536 ) , 0 , "NO" , "YES" ) , decode ( bitand ( ou . flags , 1 ) , 0 , "NO" ,
    "YES" ) , ou . start_monitoring , ou . end_monitoring from sys . user$ u , sys . obj$ io , sys .
    obj$ t , sys . ind$ i , sys . object_usage ou where i . obj# = ou . obj# and io . obj# = ou .
    obj# and t . obj# = i . bo# and u . user# = io . owner#
    END ;Rdb
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    EXECUTE oragen3 "DM2_INDEX_USAGE"
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE oragen_oracle_sys_defs(null)
   EXECUTE oragen3 "DM2_DBA*"
   IF ((dm_err->err_ind=1))
    RETURN(0)
   ENDIF
   EXECUTE oragen3 "DM2_USER*"
   IF ((dm_err->err_ind=1))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcsd_chk_drop_view(vname)
   SET dm_err->eproc = "Checking and dropping view if it exists."
   SELECT INTO "nl:"
    FROM user_views uv
    WHERE uv.view_name=vname
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (curqual=1)
    ...
    IF (check_error(concat("Dropping view: ",vname))=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcsd_chk_drop_syn(sname)
   SET dm_err->eproc = "Checking and dropping synonym if it exists."
   SELECT INTO "nl:"
    FROM dba_synonyms dv
    WHERE dv.synonym_name=sname
     AND dv.table_owner="V500"
     AND dv.owner="PUBLIC"
     AND dv.table_name=sname
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=1)
    CALL parser(concat("rdb drop public synonym ",sname," go"))
    IF (check_error(concat("Dropping synonym: ",sname))=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
...
#exit_views
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "ENDING DM2_CREATE_SYSTEM_DEFS"
 ENDIF
 CALL final_disp_msg("dm2_system_defs")
END GO
