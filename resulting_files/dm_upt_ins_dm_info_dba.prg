CREATE PROGRAM dm_upt_ins_dm_info:dba
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "DM_UPT_INS_DM_INFO"
 FREE RECORD reply
 RECORD reply(
   1 status_data
     2 status = c1
 ...
 SET reply->status_data.status = "F"
 CALL log_message("Starting script: dm_upt_ins_dm_info",log_level_debug)
 ...
 IF (size(trim(request->info_name)) > 0)
  CALL buildwhereclause(build("di.info_name = '",request->info_name,"'"))
 ENDIF
 IF (size(trim(request->info_domain)) > 0)
  CALL buildwhereclause(build("di.info_domain = '",request->info_domain,"'"))
 ENDIF
 IF (validate(request->logical_domain_ind,0)=1)
  SELECT INTO "nl:"
   ...
   WITH nocounter
  ;end select
  CALL buildwhereclause(build("di.info_domain_id = ",personnel_logical_domain_id))
 ENDIF
 ...
 IF (curqual=0)
  INSERT  FROM dm_info di
   SET di.info_date = cnvtdatetime(request->info_date), di.info_char = request->info_char, di
    .info_number = request->info_number,
    di.info_long_id = request->info_long_id, di.updt_cnt = 0, di.updt_dt_tm = cnvtdatetime(sysdate),
    di.updt_applctx = reqinfo->updt_applctx, di.updt_id = reqinfo->updt_id, di.updt_task = reqinfo->
    updt_task,
    di.info_domain = request->info_domain, di.info_name = request->info_name, di.info_domain_id =
 ...
 ENDIF
 IF (curqual=0)
  CALL log_message("Zero dm_info rows update/inserted.",log_level_error)
  SET reply->status_data.status = "F"
 ELSE
  CALL log_message("Dm_info rows successfully update/inserted.",log_level_debug)
 ...
 END ;Subroutine
#exit_script
 CALL log_message("End of script: dm_upt_ins_dm_info",log_level_debug)
 IF ((request->debug_ind=1))
