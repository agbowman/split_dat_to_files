CREATE PROGRAM ct_mp_get_prot_pi:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_prot_pi"
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getprotprincipalinvestigator(null) = null WITH protect
 DECLARE primarycd = f8 WITH protect, noconstant(0.0)
 DECLARE stat = i4 WITH protect, noconstant(0)
 SET public::"reply"->status_data.status = "F"
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_pi","Invalid JSON Request",PUBLIC::
   "reply")
  GO TO exit_script
 ENDIF
 SET stat = cnvtjsontorec( $JSONREQUEST)
 CALL getprotprincipalinvestigator(null)
 FREE RECORD request
 SET public::"reply"->status_data.status = "S"
 SUBROUTINE getprotprincipalinvestigator(null)
   CALL log_message("In GetProtPrincipalInvestigator()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   ...
   IF (protcount > 0)
    ...
    SELECT
     ...
     DETAIL
      ...
      "reply"->prot_list[d.seq]."key" = request->prot_list[d.seq].key,
      public::"reply"->prot_list[d.seq]."prot_info"."principal_investigator" = p.name_full_formatted
     WITH nocounter
    ;end select
   ELSE
    CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_pi",
     "Did not receive any protocol ID",PUBLIC::"reply")
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit GetProtPrincipalInvestigator(), Elapsed time in seconds:",
