CREATE PROGRAM bhs_find_phy_by_provider_group:dba
 PROMPT
 ...
 SELECT INTO "NL:"
  ...
  pid = person->qual[d.seq].person_id, otherlocations = substring(1,100,person->qual[d.seq].extralocs
   ), morethenonelocation =
 ...
 ;end select
 SELECT INTO "NL:"
  location = substring(1,50,person->qual[d.seq].group_full_name), o.catalog_cd, oa.order_id,
  ce.event_id, name = substring(1,50,person->qual[d.seq].name), oa.order_detail_display_line,
  cat_cd = uar_get_code_display(o.catalog_cd)
  FROM order_action oa,
   orders o,
   encounter e,
   clinical_event ce,
   (dummyt d  WITH seq = size(person->qual,5))
  PLAN (d)
   JOIN (oa
   WHERE ((oa.order_provider_id+ 0)=person->qual[d.seq].person_id)
    AND oa.action_type_cd=2534.00
    AND oa.action_dt_tm BETWEEN cnvtdatetime("01-JAN-2009 00:00:00") AND cnvtdatetime(
    "01-JAN-2009 23:59:59"))
   JOIN (o
   WHERE o.order_id=oa.order_id
    AND o.catalog_type_cd IN (2513.00, 2517.00)
    AND  NOT (o.order_status_cd IN (2542.00, 2544.00, 2545.00)))
   JOIN (e
   WHERE e.encntr_id=o.encntr_id
    AND e.encntr_type_cd IN (679653.00, 309309.00, 241211989.00))
   JOIN (ce
   WHERE ce.order_id=oa.order_id
    AND ce.result_status_cd IN (34.00, 25.00, 614349.00, 27.00, 35.00,
   38.00, 703418.00)
    AND ce.valid_until_dt_tm >= cnvtdatetime("31-DEC-2100 00:00:00"))
  ORDER BY location, o.catalog_type_cd, o.order_id,
   ce.event_id
  HEAD location
   stat = 0, headlocpos = d.seq
  HEAD o.catalog_cd
   stat = 0
  HEAD o.order_id
   IF (o.catalog_type_cd=2517)
    person->qual[d.seq].radordcnt = (person->qual[d.seq].radordcnt+ 1), person->qual[headlocpos].
    totradordcnt = (person->qual[headlocpos].totradordcnt+ 1)
   ELSEIF (o.catalog_type_cd=2513)
    person->qual[d.seq].labordcnt = (person->qual[d.seq].labordcnt+ 1), person->qual[headlocpos].
    totlabordcnt = (person->qual[headlocpos].totlabordcnt+ 1)
   ENDIF
  HEAD ce.event_id
   IF (o.catalog_type_cd=2517)
    person->qual[d.seq].radeventcnt = (person->qual[d.seq].radeventcnt+ 1), person->qual[headlocpos].
    totradeventcnt = (person->qual[headlocpos].totradeventcnt+ 1)
   ELSEIF (o.catalog_type_cd=2513)
    person->qual[d.seq].labeventcnt = (person->qual[d.seq].labeventcnt+ 1), person->qual[headlocpos].
    totlabeventcnt = (person->qual[headlocpos].totlabeventcnt+ 1)
   ENDIF
  WITH format, separator = " ", outerjoin = d1
 ;end select
 SELECT INTO  $OUTDEV
  location = substring(1,50,person->qual[d.seq].group_full_name), name = substring(1,50,person->qual[
   d.seq].name), radorders = person->qual[d.seq].radordcnt,
  rads = person->qual[d.seq].radeventcnt, laborders = person->qual[d.seq].labordcnt, labs = person->
  qual[d.seq].labeventcnt,
  totradorders = person->qual[d.seq].totradordcnt, totrads = person->qual[d.seq].totradeventcnt,
  totlaborders = person->qual[d.seq].totlabordcnt,
  totlabs = person->qual[d.seq].totlabeventcnt
  FROM (dummyt d  WITH seq = size(person->qual,5))
  PLAN (d)
  ORDER BY location, totrads DESC, totlaborders DESC
  WITH format, separator = " "
 ;end select
END GO
