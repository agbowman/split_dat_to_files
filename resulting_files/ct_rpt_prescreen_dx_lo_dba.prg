CREATE PROGRAM ct_rpt_prescreen_dx_lo:dba
 PROMPT
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CT_RPT_PRESCREEN_DX_LO"
  ...
  IF ( NOT (eksctrequest->opsind)
   ...
   IF ((eksctreply->ctfndind=1))
    IF (size(eksctreply->qual,5) > 0)
     ...
     IF (( $ORDERBY=1))
      SET _bsubreport = 1
      EXECUTE ct_sub_rpt_prescreen_dx_prsn:dba  $OUTDEV,  $EXECMODE,  $STARTDATE,
       $ENDDATE,  $ENCNTRTYPECD,  $FACILITYCD,
       $TRIGGERNAME,  $ORDERBY,  $GENDER,
       $QUALIFIER,  $AGE1,  $AGE2,
       $RACE,  $ETHNICITY,  $TERMINOLOGY,
