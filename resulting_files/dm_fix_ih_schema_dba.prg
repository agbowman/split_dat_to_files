CREATE PROGRAM dm_fix_ih_schema:dba
 ...
 SUBROUTINE dir_get_storage_type(dgst_db_link)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dlr_get_lock(dgl_lock_holder=vc,dgl_lock_obtained_ind=i2(ref)) = i2
 DECLARE dlr_release_lock(drl_lock_holder=vc) = i2
 SUBROUTINE dlr_get_lock(dgl_lock_holder,dgl_lock_obtained_ind)
   DECLARE dgl_audsid = vc WITH protect, noconstant("")
   SET dgl_lock_obtained_ind = 0
   SET dm_err->eproc = "Checking for already active processes"
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain=cnvtupper(dgl_lock_holder)
     AND di.info_name="RUN_LOCK"
    DETAIL
     dgl_audsid = di.info_char
   ...
   ENDIF
   IF (curqual > 0)
    IF (dm2_get_appl_status(dgl_audsid)="A")
     RETURN(1)
    ELSE
     SET dm_err->eproc = "Update lock row in dm_info with current audsid"
    ...
    ENDIF
   ELSE
    SET dm_err->eproc = "Insert lock row in dm_info with current audsid"
    INSERT  FROM dm_info di
     SET di.info_domain = cnvtupper(dgl_lock_holder), di.info_name = "RUN_LOCK", di.info_char =
      currdbhandle
     WITH nocounter
    ;end insert
    IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dlr_release_lock(drl_lock_holder)
   SET dm_err->eproc = "Remove locking row for current process"
   DELETE  FROM dm_info di
    WHERE di.info_domain=cnvtupper(drl_lock_holder)
     AND di.info_name="RUN_LOCK"
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dfi_get_mm_tables(dfi_sbr_status=vc) = i2
 DECLARE dfi_status = vc
 DECLARE dfi_logfile_prefix = vc WITH protect, constant("dm_fix_ih_schema")
 DECLARE dfi_lnksrv = vc WITH protect, noconstant(" ")
 DECLARE dfis_push_str = vc WITH protect, noconstant(" ")
 DECLARE dfis_tbl_name = vc WITH protect, noconstant(" ")
 DECLARE dfis_cnct_str = vc WITH protect, noconstant(" ")
 DECLARE dfis_dbname = vc WITH protect, noconstant(" ")
 DECLARE dfis_str = vc WITH protect, noconstant(" ")
 DECLARE dfis_start_pos = i4 WITH protect, noconstant(0)
 DECLARE dfis_end_pos = i4 WITH protect, noconstant(0)
 DECLARE dfis_pswd = vc WITH protect, noconstant(" ")
 DECLARE dfis_sch_dt_str = vc WITH protect, noconstant(" ")
 DECLARE dfis_sch_dt_fmt = f8 WITH protect, noconstant(0.00)
 DECLARE dfis_sysdefs_prot_date = dq8 WITH protect, noconstant(0.00)
 DECLARE dfis_sysdefs_dminfo_date = dq8 WITH protect, noconstant(0.00)
 DECLARE dfis_cnt = i4 WITH protect, noconstant(0)
 DECLARE dfis_warn_msg = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dfis_lock_ind = i2 WITH protect, noconstant(0)
 FREE RECORD dfis_admin_tbl
 RECORD dfis_admin_tbl(
   1 admin_max_ddl = dq8
   1 last_admin_max_ddl = dq8
   1 admin_dblink_name = vc
   1 tbl_cnt = i4
   1 qual[*]
     2 table_name = vc
     2 exists_ind = i2
 ...
 IF (check_logfile(dfi_logfile_prefix,".log","DM_FIX_IH_SCHEMA LOGFILE")=0)
  GO TO end_program
 ENDIF
 ...
 IF ((dm_err->debug_flag > 1))
  ...
  ;end select
 ENDIF
 SET dm_err->eproc = "Starting dm_fix_ih_schema.prg"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dlr_get_lock("DM_FIX_IH_SCHEMA",dfis_lock_ind)=0)
  GO TO end_program
 ENDIF
 IF (dfis_lock_ind=0)
  SET dm_err->eproc = "A DM_FIX_IH_SCHEMA process is already running. Exiting."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO end_program
 ENDIF
 ...
 ELSEIF ((inhouse_misc->inhouse_domain=0))
  ...
  SET dm_err->eproc = "Determining if inhouse process should continue."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF (dm2_set_autocommit(1)=0)
 ...
 SELECT INTO "nl:"
  ...
  WHERE a.info_name="DM_ENV_ID"
   ...
   AND a.info_number=b.environment_id
  DETAIL
   envid = b.environment_id, dfis_str = b.v500_connect_string
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  FROM dprotect dp
  WHERE dp.object="P"
   AND dp.object_name="DM2_CREATE_SYSTEM_DEFS"
  DETAIL
   dfis_sysdefs_prot_date = cnvtdatetime(dp.datestamp,dp.timestamp)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Get last run timestamp from DM_INFO for DM2_CREATE_SYTEM_DEFS."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2 TOOLSET PREREQS COMPLETE"
   AND di.info_name IN ("DM2_CREATE_SYSTEM_DEFS_DATE", "DM2_CREATE_ADMIN_DDL_DATE")
  HEAD REPORT
   dfis_sysdefs_dminfo_date = 0.0, dfis_admin_tbl->last_admin_max_ddl = cnvtdatetime("01-JAN-1900")
  DETAIL
   IF (di.info_name="DM2_CREATE_SYSTEM_DEFS_DATE")
    dfis_sysdefs_dminfo_date = cnvtdatetime(di.info_char)
   ELSE
    dfis_admin_tbl->last_admin_max_ddl = cnvtdatetime(di.info_char)
   ENDIF
  WITH nocounter, nullreport
 ;end select
 SET dm_err->eproc = "Get ADMIN database link name."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  ...
  DETAIL
   dfis_admin_tbl->admin_dblink_name = substring(1,(findstring(".",ds.db_link,1,0) - 1),ds.db_link)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Get highest DDL time for tables in ADMIN."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (value(concat("DBA_OBJECTS@",dfis_admin_tbl->admin_dblink_name)) d)
  WHERE d.owner="CDBA"
   AND d.object_type="TABLE"
   AND substring(1,3,d.object_name) != "DM2"
   AND  NOT (d.object_name IN ("DM_INFO", "DM_SEGMENTS", "DM_TABLE_LIST", "DM_USER_CONSTRAINTS",
  "DM_USER_CONS_COLUMNS",
  "DM_USER_IND_COLUMNS", "DM_USER_TAB_COLS", "EXPLAIN_ARGUMENT", "EXPLAIN_INSTANCE", "EXPLAIN_OBJECT",
  "EXPLAIN_OPERATOR", "EXPLAIN_PREDICATE", "EXPLAIN_STATEMENT", "EXPLAIN_STREAM"))
   AND d.last_ddl_time > cnvtdatetime(dfis_admin_tbl->last_admin_max_ddl)
  ORDER BY d.last_ddl_time DESC
  HEAD REPORT
   dfis_admin_tbl->admin_max_ddl = cnvtdatetime(d.last_ddl_time)
  DETAIL
   dfis_admin_tbl->tbl_cnt = (dfis_admin_tbl->tbl_cnt+ 1), stat = alterlist(dfis_admin_tbl->qual,
    dfis_admin_tbl->tbl_cnt), dfis_admin_tbl->qual[dfis_admin_tbl->tbl_cnt].table_name = d
   .object_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO end_program
 ENDIF
 IF ((dfis_admin_tbl->tbl_cnt > 0))
  SET dm_err->eproc = "Query for DM2_ADMIN_TABLE rows in admin DM_INFO."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM (value(concat("DM_INFO@",dfis_admin_tbl->admin_dblink_name)) di),
    (dummyt d  WITH seq = value(dfis_admin_tbl->tbl_cnt))
   PLAN (d
    WHERE d.seq > 0)
    JOIN (di
    WHERE di.info_domain="DM2_ADMIN_TABLE"
     AND (di.info_name=dfis_admin_tbl->qual[d.seq].table_name))
   DETAIL
    dfis_admin_tbl->qual[d.seq].exists_ind = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF ((dm_err->debug_flag > 0))
   SET dm_err->eproc = "Insert DM2_ADMIN_TABLE rows into admin DM_INFO."
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  INSERT  FROM (value(concat("DM_INFO@",dfis_admin_tbl->admin_dblink_name)) di),
    (dummyt d  WITH seq = value(dfis_admin_tbl->tbl_cnt))
   SET di.info_domain = "DM2_ADMIN_TABLE", di.info_name = dfis_admin_tbl->qual[d.seq].table_name
   PLAN (d
    WHERE (dfis_admin_tbl->qual[d.seq].exists_ind=0))
    JOIN (di
    WHERE di.info_domain="DM2_ADMIN_TABLE"
     AND (di.info_name=dfis_admin_tbl->qual[d.seq].table_name))
   WITH nocounter
  ...
  ENDIF
  COMMIT
  IF ((dfis_admin_tbl->last_admin_max_ddl=cnvtdatetime("01-JAN-1900")))
   EXECUTE dm2_create_admin_nicknames "*", "CDBA"
   IF ((dm_err->err_ind > 0))
    GO TO end_program
   ENDIF
  ELSE
   FOR (dfis_cnt = 1 TO dfis_admin_tbl->tbl_cnt)
    EXECUTE dm2_create_admin_nicknames value(dfis_admin_tbl->qual[dfis_cnt].table_name), "CDBA"
    IF ((dm_err->err_ind > 0))
     GO TO end_program
    ENDIF
   ENDFOR
  ENDIF
  IF ((dm_err->debug_flag > 0))
   SET dm_err->eproc = "Log new max admin ddl row in DM_INFO."
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dm_info di
   WHERE di.info_domain="DM2 TOOLSET PREREQS COMPLETE"
    AND di.info_name="DM2_CREATE_ADMIN_DDL_DATE"
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2 TOOLSET PREREQS COMPLETE", di.info_name = "DM2_CREATE_ADMIN_DDL_DATE",
    di.info_char = format(dfis_admin_tbl->admin_max_ddl,"DD-MMM-YYYY HH:MM:SS;;q"),
  ...
  COMMIT
 ENDIF
 IF (dfis_sysdefs_prot_date > dfis_sysdefs_dminfo_date)
  EXECUTE dm2_create_system_defs
  IF ((dm_err->err_ind > 0))
   GO TO end_program
  ENDIF
  SET dm_err->eproc = "Deleting dm_info rows."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  DELETE  FROM dm_info di
   WHERE di.info_domain="DM2 TOOLSET PREREQS COMPLETE"
    AND di.info_name="DM2_CREATE_SYSTEM_DEFS_DATE"
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   GO TO end_program
  ENDIF
  SET dm_err->eproc = "Inserting CCLPROT date row into dm_info."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2 TOOLSET PREREQS COMPLETE", di.info_name = "DM2_CREATE_SYSTEM_DEFS_DATE",
    di.info_char = format(dfis_sysdefs_prot_date,"DD-MMM-YYYY HH:MM:SS;;q"),
    di.updt_dt_tm = cnvtdatetime(curdate,curtime3), di.updt_applctx = 0, di.updt_cnt = 0,
    di.updt_id = 0, di.updt_task = reqinfo->updt_task
   WITH nocounter
  ;end insert
  ...
  COMMIT
 ENDIF
 IF (((findstring("/",dfis_str)=0) OR (findstring("@",dfis_str) > 0
  AND currdb="DB2UDB")) )
  SET dm_err->err_ind = 1
  ...
  GO TO end_program
 ENDIF
 SET dfis_start_pos = (findstring("/",dfis_str)+ 1)
 IF (currdb="ORACLE")
  SET dfis_end_pos = findstring("@",dfis_str,dfis_start_pos)
  IF (dfis_end_pos=0)
   SET dfis_end_pos = textlen(dfis_str)
  ELSE
   SET dfis_end_pos = (dfis_end_pos - 1)
  ENDIF
 ELSEIF (currdb="SQLSRV")
  SET dfis_end_pos = findstring("/",dfis_str,dfis_start_pos)
  IF (dfis_end_pos=0)
  ...
  ENDIF
 ELSE
  SET dfis_end_pos = (findstring("/",dfis_str,dfis_start_pos) - 1)
 ENDIF
 SET dfis_pswd = substring(dfis_start_pos,((dfis_end_pos - dfis_start_pos)+ 1),dfis_str)
 IF (currdb="SQLSRV")
 ...
 IF (currdb="SQLSRV")
  EXECUTE dm2_auto_success_schema
  IF ((dm_err->err_ind=1))
   GO TO end_program
  ENDIF
  IF (dm2_set_autocommit(1)=0)
   GO TO end_program
  ENDIF
  SET dm_err->eproc = "Selecting schema changes to be installed from DM_PROJECT_STATUS_ENV"
  CALL disp_msg(" ",dm_err->logfile,0)
  SET modify openquery dfi_lnksrv
  SELECT INTO "nl:"
   FROM dm_project_status_env dm
   WHERE dm.environment_id=envid
    AND dm.proj_type="SCHEMA"
    AND dm.dm_status = null
    AND  NOT ( EXISTS (
   (SELECT
    "X"
   ...
   HEAD dm.proj_name
    ...
    ENDIF
    list->qual[list->count].table_name = dm.proj_name
   DETAIL
    x = 1
   WITH nocounter
  ;end select
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO end_program
 ELSEIF (curqual=0)
  EXECUTE dm2_sync_new_stats
  IF ((dm_err->err_ind > 0))
   GO TO end_program
  ENDIF
  SET dm_err->eproc = "No schema changes to be installed"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO end_program
 ENDIF
 SET stat = alterlist(list->qual,list->count)
 IF (currdb="SQLSRV")
  SET modify = noopenquery
 ENDIF
 IF (currdb="SQLSRV")
  SET modify openquery dfi_lnksrv
  FOR (ix1 = 1 TO list->count)
    UPDATE  FROM dm_project_status_env dm
     SET dm.dm_status = "RUNNING", dm.dm_status_dt_tm = cnvtdatetime(curdate,curtime3)
     WHERE (dm.proj_name=list->qual[ix1].table_name)
      AND dm.proj_type="SCHEMA"
      AND dm.environment_id=envid
      AND dm.dm_status = null
     WITH nocounter
    ;end update
    IF (check_error("Updating schema changes found to a RUNNING status on DM_PROJECT_STATUS_ENV")=1)
     ROLLBACK
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO end_program
    ENDIF
  ...
  SET modify = noopenquery
 ELSE
  SET dm_err->eproc = "Updating schema changes found to a RUNNING status on DM_PROJECT_STATUS_ENV"
  CALL disp_msg(" ",dm_err->logfile,0)
  UPDATE  FROM dm_project_status_env dm,
   ...
   PLAN (d)
    JOIN (dm
    WHERE (dm.proj_name=list->qual[d.seq].table_name)
     AND dm.proj_type="SCHEMA"
     AND dm.environment_id=envid
     AND dm.dm_status = null)
   WITH nocounter
 ...
 ENDIF
 SET sch_cnt = 0
 FOR (sch_cnt = 1 TO list->count)
   IF (dm2_set_autocommit(1)=0)
    GO TO end_program
   ENDIF
   SET dm_err->err_ind = 0
   SET dfis_warn_msg = "DM2NOTSET"
   ...
   SET inhouse_reply->table_name = list->qual[sch_cnt].table_name
   ...
   IF (currdb="SQLSRV")
    SET modify openquery dfi_lnksrv
   ENDIF
   SET dfis_tbl_name = list->qual[sch_cnt].table_name
   SELECT INTO "NL:"
    dcf.schema_date
    FROM dm_project_status_env dcf
    WHERE dcf.proj_name=dfis_tbl_name
     AND dcf.proj_type="SCHEMA"
    ...
    WITH nocounter
   ;end select
   IF (check_error("Retrieving highest schema date for a particular tables schema")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 0
    CALL dfi_update_status("FAILED")
    SET dm_err->err_ind = 1
   ENDIF
   IF ((dm_err->err_ind=0))
    IF (dm2_set_autocommit(0)=0)
   ...
   ENDIF
   IF ((dm_err->err_ind=0))
    IF (checkprg("DM_DAF_IH_PRE_SCHEMA") > 0)
     EXECUTE dm_daf_ih_pre_schema list->qual[sch_cnt].table_name
     IF ((dm_err->err_ind=1))
      SET dm_err->err_ind = 0
      CALL dfi_update_status("FAILED")
      SET dm_err->err_ind = 1
   ...
   ENDIF
   IF ((dm_err->err_ind=0))
    EXECUTE dm2_create_adm_schema_files list->qual[sch_cnt].table_name, cnvtdatetime(r1->rdate)
    IF ((dm_err->err_ind=1))
     SET dm_err->err_ind = 0
     CALL dfi_update_status("FAILED")
     SET dm_err->err_ind = 1
    ELSE
     UPDATE  FROM dm_project_status_env
      SET dm_status_msg = concat("Schema Date:",trim(dm2_install_schema->file_prefix))
      WHERE (proj_name=list->qual[sch_cnt].table_name)
       AND proj_type="SCHEMA"
       AND schema_date=cnvtdatetime(r1->rdate)
       AND environment_id=envid
      WITH nocounter
     ;end update
     IF (check_error("Recording schema date in dm_project_status_env")=1)
      ROLLBACK
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dm_err->err_ind = 0
      CALL dfi_update_status("FAILED")
     ...
     COMMIT
     IF (currdb="ORACLE")
      SET dfis_cnct_str = currdblink
     ELSE
      SET dfis_cnct_str = currdbname
     ENDIF
     SET dfis_dbname = validate(currdbname,"TEST")
     EXECUTE dm2_install_schema "INHOUSE", dm2_install_schema->file_prefix, dfis_dbname,
     "NONE", "NONE", dfis_pswd,
     dfis_cnct_str
     IF ((dm_err->err_ind=0))
      ...
      CALL disp_msg(" ",dm_err->logfile,0)
      IF (cursys="AXP")
       SET dfis_push_str = concat("delete ",dm2_install_schema->ccluserdir,"dm2c",dm2_install_schema
        ->file_prefix,"*.*;*")
      ELSEIF (cursys="WIN")
       SET dfis_push_str = concat("del ",dm2_install_schema->ccluserdir,"dm2c",dm2_install_schema->
        file_prefix,"*")
      ELSE
       SET dfis_push_str = concat("rm -f ",dm2_install_schema->ccluserdir,"dm2c",dm2_install_schema->
        file_prefix,"*")
      ENDIF
      IF (dm2_push_dcl(dfis_push_str)=0)
       SET dm_err->err_ind = 0
       SET dm_err->emsg = concat("WARNING: ",dm_err->emsg)
       SET dfis_warn_msg = dm_err->emsg
      ENDIF
      SET dfi_status = "SUCCESS"
      IF (checkprg("DM_DAF_IH_POST_SCHEMA") > 0)
       EXECUTE dm_daf_ih_post_schema list->qual[sch_cnt].table_name
       IF ((dm_err->err_ind=1))
        SET dm_err->err_ind = 0
        SET dfi_status = "FAILED"
       ENDIF
      ENDIF
      IF (dfis_warn_msg != "DM2NOTSET"
       AND dfi_status="SUCCESS")
       SET dm_err->emsg = dfis_warn_msg
      ENDIF
     ELSE
      SET dm_err->err_ind = 0
      SET dfi_status = "FAILED"
     ENDIF
     IF (dfi_update_status(dfi_status)=0)
      SET dm_err->err_ind = 0
     ENDIF
    ENDIF
   ENDIF
 ...
 IF ((dm_err->err_ind > 0))
  GO TO end_program
 ENDIF
 SUBROUTINE dfi_update_status(dfi_sbr_status)
   IF ((dm_err->debug_flag > 2))
    CALL echorecord(dm2_install_schema)
   ENDIF
   SET inhouse_reply->status = dfi_sbr_status
   SET inhouse_reply->err_code = dm_err->ecode
   SET inhouse_reply->status_msg = substring(1,200,dm_err->emsg)
   SET inhouse_reply->logfile = dm_err->logfile
   IF (currdb="SQLSRV")
    IF (dm2_set_autocommit(1)=0)
     RETURN(0)
    ENDIF
   ...
   IF (currdb="SQLSRV")
    SET modify openquery dfi_lnksrv
   ENDIF
   IF (dfi_sbr_status="SUCCESS")
    UPDATE  FROM dm_project_status_env
     SET dm_status = inhouse_reply->status, dm_status_msg = inhouse_reply->status_msg,
      dm_status_dt_tm = cnvtdatetime(curdate,curtime3),
      log_file_name = inhouse_reply->logfile
     WHERE (proj_name=list->qual[sch_cnt].table_name)
      AND proj_type="SCHEMA"
      AND schema_date <= cnvtdatetime(r1->rdate)
      AND dm_status IN ("RUNNING", "FAILED")
      AND environment_id=envid
     WITH nocounter
    ;end update
    IF (check_error("Updating status to SUCCESS on dm_project_status_env")=1)
     ROLLBACK
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    COMMIT
    UPDATE  FROM dm_project_status_env
     SET dm_status = null, parent_proj_name = " ", err_code = 0,
      processed_flag = 0, dm_status_dt_tm = cnvtdatetime(curdate,curtime3)
     WHERE (parent_proj_name=list->qual[sch_cnt].table_name)
      AND err_code=94
      AND dm_status="FAILED"
      AND environment_id=envid
     WITH nocounter
    ;end update
    IF (check_error("Updating status to NULL on dm_project_status_env")=1)
     ROLLBACK
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ...
   ELSE
    UPDATE  FROM dm_project_status_env
     SET dm_status = inhouse_reply->status, err_code = inhouse_reply->err_code, dm_status_dt_tm =
      cnvtdatetime(curdate,curtime3),
      dm_status_msg = inhouse_reply->status_msg, log_file_name = inhouse_reply->logfile,
      parent_proj_name = inhouse_reply->parent_proj_name
     WHERE (proj_name=list->qual[sch_cnt].table_name)
      AND proj_type="SCHEMA"
      AND dm_status="RUNNING"
      AND schema_date <= cnvtdatetime(r1->rdate)
      AND environment_id=envid
     WITH nocounter
    ;end update
    IF (check_error("Updating status to FAILED on dm_project_status_env")=1)
     ROLLBACK
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
 ...
 IF ((dm_err->debug_flag > 1))
  ...
  ;end select
 ENDIF
 CALL dlr_release_lock("DM_FIX_IH_SCHEMA")
 SET dm_err->eproc = "Ending dm_fix_ih_schema.prg"
 CALL final_disp_msg(dfi_logfile_prefix)
END GO
