CREATE PROGRAM dm2_mig_replicate_runner
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmr_from_mig_replicate = i4 WITH protect, noconstant(1)
 IF (check_logfile("dm2_mig_rep_run",".log","DM2_MIG_REPLICATE_RUNNER LOG FILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "BEGIN DM2_MIG_REPLICATE_RUNNER"
 CALL disp_msg(" ",dm_err->logfile,0)
...
#exit_program
 SET dm_err->eproc = "DM2_MIG_REPLICATE_RUNNER completes successfully"
 CALL final_disp_msg("dm2_mig_rep_run")
END GO
