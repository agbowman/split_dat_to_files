CREATE PROGRAM dm2_repl_data_cleanup
 ...
 SUBROUTINE dctx_restore_prev_contexts(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drdc_mode = vc WITH protect, noconstant("")
 DECLARE drdc_info_domain = vc WITH protect, noconstant("")
 DECLARE drdc_tbl_cnt = i4 WITH protect, noconstant(0)
 DECLARE drdc_col_cnt = i4 WITH protect, noconstant(0)
 DECLARE drdc_tbl_idx = i4 WITH protect, noconstant(0)
 DECLARE drdc_col_idx = i4 WITH protect, noconstant(0)
 DECLARE drdc_tbl_name = vc WITH protect, noconstant("")
 DECLARE drdc_col_name = vc WITH protect, noconstant("")
 DECLARE drdc_col_value = vc WITH protect, noconstant("")
 DECLARE drdc_rows_updated = i4 WITH protect, noconstant(0)
 DECLARE drdc_commit_cnt = i4 WITH protect, noconstant(0)
 DECLARE drdc_warn_cnt = i4 WITH protect, noconstant(0)
 DECLARE drdc_col_oradef_ind = i2 WITH protect, noconstant(0)
 DECLARE drdc_col_ccldef_ind = i2 WITH protect, noconstant(0)
 DECLARE drdc_data_type = vc WITH protect, noconstant("")
 DECLARE drdc_drr_cleanup = vc WITH protect, noconstant("")
 DECLARE drdc_parallel_cnt = i4 WITH protect, noconstant(0)
 DECLARE drdc_create_clnup_objects(null) = i2
 DECLARE drdc_repl_cleanup(drc_owner=vc,drc_tbl_name=vc,drc_updt_str=vc,drc_where_str=vc,
  drc_parallel_cnt=i4) = i2
 DECLARE drdc_set_parallel_degree(dspd_o_parallel_cnt=i4(ref)) = i2
 FREE RECORD drdc_cleanup
 RECORD drdc_cleanup(
   1 tbl_cnt = i4
   1 tbl[*]
     2 table_name = vc
 ...
 SET drdc_cleanup->tbl_cnt = 0
 IF ((dm2_install_schema->curprog="NONE"))
  IF (validate(dm2_clean_standalone,1)=1
   AND validate(dm2_clean_standalone,2)=2)
   SET dm_err->logfile = "DM2NOTSET"
   SET dm_err->eproc = "Validate standalone status of replicate cleanup script."
   SET dm_err->emsg = "DM2_REPL_DATA_CLEANUP may not be run Standalone"
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (check_logfile("dm2_repl_data_cln",".log","dm2_repl_data_cleanup")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_repl_data_cleanup"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (validate(dm2_repl_data_cleanup_skip,- (1))=1)
  SET dm_err->eproc =
  "Replicate data cleanup on V500 tables has been bypassed - no updates will be performed to 'initialize' specific table data"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (dctx_set_context("FIRE_LUTS_TRG","NO")=0)
  GO TO exit_program
 ENDIF
 SET drdc_mode = cnvtupper( $1)
 IF ( NOT (drdc_mode IN ("ALL", "REF")))
  SET dm_err->eproc = "Validate mode of execution."
  SET dm_err->emsg = concat("Invalid mode of execution,",drdc_mode,". Valid modes are REF and ALL")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drdc_drr_cleanup = trim(cnvtupper( $2),3)
 IF ( NOT (drdc_drr_cleanup IN ("ALL", "RR_ALL", "NONE")))
  SET dm_err->eproc = "Validate DRR cleanup parameter."
  SET dm_err->emsg = concat("Invalid DRR cleanup parameter,",drdc_drr_cleanup,
   ". Valid modes are ALL, RR_ALL and NONE")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echo(build("drdc_mode =",drdc_mode))
  CALL echo(build("drdc_drr_cleanup =",drdc_drr_cleanup))
 ENDIF
 SET drdc_info_domain = evaluate(drdc_mode,"ALL","DM2_REPLICATE_CLEANUP-ALL",
  "DM2_REPLICATE_CLEANUP-REF")
 SET dm_err->eproc = "Pull in cleanup critera"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain=drdc_info_domain
  ORDER BY d.info_name
  HEAD REPORT
   drdc_tbl_cnt = 0
  DETAIL
   drdc_tbl_name = substring(1,(findstring("-",d.info_name,1,0) - 1),d.info_name), drdc_col_name =
   substring((findstring("-",d.info_name,1,0)+ 1),(textlen(d.info_name) - findstring("-",d.info_name,
     1,0)),d.info_name), drdc_col_value = d.info_char
   IF ((dm_err->debug_flag > 1))
    CALL echo(drdc_col_name),
    CALL echo(drdc_tbl_name),
    CALL echo(drdc_col_value)
   ENDIF
   drdc_tbl_idx = 0
   IF (size(drdc_cleanup->tbl,5)=0)
    drdc_col_cnt = 0, drdc_tbl_cnt = (drdc_tbl_cnt+ 1), stat = alterlist(drdc_cleanup->tbl,
     drdc_tbl_cnt),
    drdc_cleanup->tbl[drdc_tbl_cnt].table_name = drdc_tbl_name
   ELSE
    drdc_tbl_idx = locateval(drdc_tbl_idx,1,size(drdc_cleanup->tbl,5),drdc_tbl_name,drdc_cleanup->
     tbl[drdc_tbl_idx].table_name)
    IF (drdc_tbl_idx=0)
     drdc_col_cnt = 0, drdc_tbl_cnt = (drdc_tbl_cnt+ 1), stat = alterlist(drdc_cleanup->tbl,
      drdc_tbl_cnt),
     drdc_cleanup->tbl[drdc_tbl_cnt].table_name = drdc_tbl_name
    ENDIF
   ENDIF
   IF (drdc_col_name="WHERECLAUSE")
    drdc_cleanup->tbl[drdc_tbl_cnt].where_clause = drdc_col_value
   ELSEIF (drdc_col_name="DELETEWHERECLAUSE")
    drdc_cleanup->tbl[drdc_tbl_cnt].delete_ind = 1, drdc_cleanup->tbl[drdc_tbl_cnt].
    delete_where_clause = drdc_col_value
   ELSE
    drdc_col_cnt = (drdc_col_cnt+ 1), stat = alterlist(drdc_cleanup->tbl[drdc_tbl_cnt].col,
     drdc_col_cnt), drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].column_name = drdc_col_name,
    drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].updt_value = drdc_col_value, drdc_cleanup->tbl[
    drdc_tbl_cnt].col_cnt = drdc_col_cnt
   ENDIF
  FOOT REPORT
   drdc_cleanup->tbl_cnt = size(drdc_cleanup->tbl,5)
  WITH nocounter
 ;end select
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(drdc_cleanup)
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No cleanup information obtained. Process FAILED"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Check columns to be updated against USER_TAB_COLUMNS"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM user_tab_columns u,
   (dummyt d  WITH seq = value(drdc_cleanup->tbl_cnt))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (u
   WHERE (u.table_name=drdc_cleanup->tbl[d.seq].table_name))
  DETAIL
   drdc_cleanup->tbl[d.seq].exists_ind = 1, drdc_col_idx = 0, drdc_col_idx = locateval(drdc_col_idx,1,
    drdc_cleanup->tbl[d.seq].col_cnt,u.column_name,drdc_cleanup->tbl[d.seq].col[drdc_col_idx].
    column_name)
   IF (drdc_col_idx > 0)
    drdc_cleanup->tbl[d.seq].col[drdc_col_idx].exists_ind = 1
   ENDIF
  WITH nocounter
 ;end select
 IF ((dm_err->debug_flag > 2))
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Check if ANY columns to be updated exist on table"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 FOR (drdc_tbl_cnt = 1 TO value(drdc_cleanup->tbl_cnt))
   IF ((drdc_cleanup->tbl[drdc_tbl_cnt].exists_ind=1))
    IF ((drdc_cleanup->tbl[drdc_tbl_cnt].delete_ind=1))
     SET drdc_col_idx = 0
     SET drdc_col_idx = locateval(drdc_col_idx,1,drdc_cleanup->tbl[drdc_tbl_cnt].col_cnt,0,
      drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_idx].exists_ind)
     IF (drdc_col_idx=1)
      SET dm_err->eproc = concat("Ignoring ",drdc_cleanup->tbl[drdc_tbl_cnt].table_name,
       " since ALL columns in delete where clause do not exist.")
      CALL disp_msg("",dm_err->logfile,0)
      SET drdc_cleanup->tbl[drdc_tbl_cnt].exists_ind = 0
     ENDIF
    ELSE
     SET drdc_col_idx = 0
     SET drdc_col_idx = locateval(drdc_col_idx,1,drdc_cleanup->tbl[drdc_tbl_cnt].col_cnt,1,
      drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_idx].exists_ind)
     IF (drdc_col_idx=0)
      SET dm_err->eproc = concat("Ignoring ",drdc_cleanup->tbl[drdc_tbl_cnt].table_name,
       " since no columns exist for update.")
      CALL disp_msg("",dm_err->logfile,0)
      SET drdc_cleanup->tbl[drdc_tbl_cnt].exists_ind = 0
     ENDIF
    ENDIF
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Check columns to be updated against the dictionary"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 FOR (drdc_tbl_cnt = 1 TO value(drdc_cleanup->tbl_cnt))
   IF ((drdc_cleanup->tbl[drdc_tbl_cnt].exists_ind=1)
    AND checkdic(drdc_cleanup->tbl[drdc_tbl_cnt].table_name,"T",0)=2)
    IF ((drdc_cleanup->tbl[drdc_tbl_cnt].exists_ind != 0))
     SET drdc_cleanup->tbl[drdc_tbl_cnt].exists_ind = 2
    ENDIF
    FOR (drdc_col_idx = 1 TO value(drdc_cleanup->tbl[drdc_tbl_cnt].col_cnt))
      SET drdc_col_oradef_ind = 0
      SET drdc_col_ccldef_ind = 0
      SET drdc_data_type = ""
      IF (dm2_table_column_exists("",drdc_cleanup->tbl[drdc_tbl_cnt].table_name,drdc_cleanup->tbl[
       drdc_tbl_cnt].col[drdc_col_idx].column_name,0,1,
       1,drdc_col_oradef_ind,drdc_col_ccldef_ind,drdc_data_type)=0)
       GO TO exit_program
      ENDIF
      IF (drdc_col_ccldef_ind=1)
       IF ((drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_idx].exists_ind != 0))
        SET drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_idx].exists_ind = 2
       ENDIF
      ENDIF
    ENDFOR
   ENDIF
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validate table/columns to be updated are in the CCLDEF for the table"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 FOR (drdc_tbl_cnt = 1 TO value(drdc_cleanup->tbl_cnt))
   IF ((drdc_cleanup->tbl[drdc_tbl_cnt].exists_ind=1))
    SET dm_err->emsg = concat("CCLDEF does not exist for table:",drdc_cleanup->tbl[drdc_tbl_cnt].
     table_name)
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ELSE
    FOR (drdc_col_cnt = 1 TO value(drdc_cleanup->tbl[drdc_tbl_cnt].col_cnt))
      IF ((drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].exists_ind=1))
       SET dm_err->emsg = concat("CCLDEF not in synch for table:",drdc_cleanup->tbl[drdc_tbl_cnt].
        table_name,". One or more columns from table missing from CCLDEF.")
       SET dm_err->err_ind = 1
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ENDIF
 ...
 IF ((dm2_db_options->load_ind=0))
  ...
  ENDIF
 ENDIF
 SET drdc_commit_cnt = cnvtint(dm2_db_options->cursor_commit_cnt)
 SET drdc_rows_updated = drdc_commit_cnt
 SET dm_err->eproc = "Format update statements to initialize tables."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 FOR (drdc_tbl_cnt = 1 TO value(drdc_cleanup->tbl_cnt))
   IF ((drdc_cleanup->tbl[drdc_tbl_cnt].exists_ind=2))
    SET drdc_rows_updated = drdc_commit_cnt
    SET dm_err->eproc = concat("Updating rows in ",drdc_cleanup->tbl[drdc_tbl_cnt].table_name)
    WHILE (drdc_rows_updated >= drdc_commit_cnt
     AND (dm_err->err_ind=0))
      IF ((drdc_cleanup->tbl[drdc_tbl_cnt].delete_ind=1))
       CALL dm2_push_cmd(concat("delete from ",drdc_cleanup->tbl[drdc_tbl_cnt].table_name," t "),0)
      ELSE
       CALL dm2_push_cmd(concat("update into ",drdc_cleanup->tbl[drdc_tbl_cnt].table_name," t set "),
        0)
       SET drdc_col_idx = 0
       FOR (drdc_col_cnt = 1 TO value(drdc_cleanup->tbl[drdc_tbl_cnt].col_cnt))
         IF ((drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].exists_ind=2))
          SET drdc_col_idx = (drdc_col_idx+ 1)
          IF (drdc_col_idx > 1)
           CALL dm2_push_cmd(" , ",0)
          ENDIF
          CALL dm2_push_cmd(concat(" t.",drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].
            column_name," = ",drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].updt_value),0)
         ENDIF
       ENDFOR
      ENDIF
      SET drdc_col_idx = 0
      IF ((drdc_cleanup->tbl[drdc_tbl_cnt].where_clause > " "))
       CALL dm2_push_cmd(concat(" where ",drdc_cleanup->tbl[drdc_tbl_cnt].where_clause," and ("),0)
       SET drdc_col_idx = (drdc_col_idx+ 1)
      ELSEIF ((drdc_cleanup->tbl[drdc_tbl_cnt].delete_where_clause > " "))
       CALL dm2_push_cmd(concat(" where ",drdc_cleanup->tbl[drdc_tbl_cnt].delete_where_clause),0)
       SET drdc_col_idx = (drdc_col_idx+ 1)
      ENDIF
      IF ( NOT ((drdc_cleanup->tbl[drdc_tbl_cnt].delete_where_clause > " ")))
       FOR (drdc_col_cnt = 1 TO value(drdc_cleanup->tbl[drdc_tbl_cnt].col_cnt))
         IF ((drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].exists_ind=2))
          SET drdc_col_idx = (drdc_col_idx+ 1)
          IF (drdc_col_idx=1)
           CALL dm2_push_cmd(concat(" where t.",drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].
             column_name," != ",drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].updt_value),0)
          ELSE
           IF ((drdc_cleanup->tbl[drdc_tbl_cnt].where_clause > " ")
            AND drdc_col_idx=2)
            CALL dm2_push_cmd(concat(" t.",drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].
              column_name," != ",drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].updt_value),0)
           ELSE
            CALL dm2_push_cmd(concat(" or t.",drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].
              column_name," != ",drdc_cleanup->tbl[drdc_tbl_cnt].col[drdc_col_cnt].updt_value),0)
           ENDIF
          ENDIF
         ENDIF
      ...
      IF ((drdc_cleanup->tbl[drdc_tbl_cnt].where_clause > " "))
       CALL dm2_push_cmd(")",0)
      ENDIF
      CALL dm2_push_cmd(concat("with nocounter, maxqual(t, ",build(drdc_commit_cnt),") go "),1)
      SET drdc_rows_updated = curqual
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("rows updated:",drdc_rows_updated))
      ENDIF
      IF ((dm_err->err_ind=1))
       ROLLBACK
       IF (((findstring("CCL-E-26-",dm_err->emsg,1,1) > 0) OR (findstring("ORA-00904",dm_err->emsg,1,
        1) > 0)) )
        SET dm_err->err_ind = 0
        SET drdc_rows_updated = 0
        SET drdc_warn_cnt = (drdc_warn_cnt+ 1)
        SET stat = alterlist(drr_cleanup_warnings->qual,drdc_warn_cnt)
        SET drr_cleanup_warnings->cnt = drdc_warn_cnt
        SET drr_cleanup_warnings->qual[drdc_warn_cnt].table_name = drdc_cleanup->tbl[drdc_tbl_cnt].
        table_name
        SET drr_cleanup_warnings->qual[drdc_warn_cnt].message =
        "Invalid column in WHERE CLAUSE of update"
        IF ((dm_err->debug_flag > 2))
         CALL echorecord(drr_cleanup_warnings)
        ENDIF
   ...
   ENDIF
 ENDFOR
 IF (drdc_set_parallel_degree(drdc_parallel_cnt)=0)
  GO TO exit_program
 ENDIF
 IF (drdc_mode="ALL")
  IF (drdc_create_clnup_objects(null)=0)
   GO TO exit_program
  ENDIF
  IF (validate(dm2_bypass_cleanup_person,- (1))=1)
   SET dm_err->eproc = "User elected to skip update against PERSON table."
   CALL disp_msg("",dm_err->logfile,0)
  ELSE
   IF (drdc_repl_cleanup("V500","PERSON","'UPDATE PERSON set vip_cd = 0 where rowid = :1'",
    "d.vip_cd != 0",drdc_parallel_cnt)=0)
    GO TO exit_program
   ENDIF
  ENDIF
 ENDIF
 IF (drdc_drr_cleanup != "NONE")
  EXECUTE dm2_drr_copy_cleanup value(drdc_drr_cleanup)
  IF ((dm_err->err_ind > 0))
   GO TO exit_program
  ENDIF
 ENDIF
 GO TO exit_program
 SUBROUTINE drdc_create_clnup_objects(null)
   DECLARE dcco_create_cmd = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Compiling PLSQL TYPE V500.DM2_REPL_NUMBER_TABLE"
   CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
   IF (dm2_push_cmd(concat("RDB ASIS(^ ",
     "CREATE OR REPLACE TYPE V500.DM2_REPL_NUMBER_TABLE AS TABLE OF NUMBER "," ^) go"),1)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Compiling Bulk Parallel Piped function"
   CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
   SET dcco_create_cmd = concat("RDB ASIS(^ ","create or replace FUNCTION dm2_repl_data_cleanup (",
    "    rdc_cur IN SYS_REFCURSOR, ","    rdc_updt_str IN clob, ","    rdc_limit in number) ",
    "  RETURN V500.DM2_REPL_NUMBER_TABLE ","  PARALLEL_ENABLE (PARTITION rdc_cur BY ANY) ",
    "  PIPELINED ","  IS ","    PRAGMA AUTONOMOUS_TRANSACTION; ",
    "    TYPE rowid_t is table of rowid index by binary_integer; ","    rowid_tab rowid_t; ",
    "    rdc_cnt NUMBER := 0; ","    err_msg VARCHAR2(150); ","  BEGIN ",
    "    LOOP ","      FETCH rdc_cur BULK COLLECT INTO rowid_tab LIMIT rdc_limit; ",
    "      EXIT WHEN rowid_tab.COUNT() = 0; ","      FORALL i IN rowid_tab.FIRST .. rowid_tab.LAST ",
    "        execute immediate rdc_updt_str using rowid_tab(i); ",
    "      commit; ","      rdc_cnt := rdc_cnt + rowid_tab.COUNT; ","    END LOOP; ",
    "    CLOSE rdc_cur; ","    PIPE ROW(rdc_cnt); ",
    "    RETURN; ","  EXCEPTION ","  when others then ","    rollback; ",
    "    err_msg:=SUBSTR(sqlerrm,1,150);",
    "    raise_application_error(-20555, err_msg);","END; ","^) go")
   IF (dm2_push_cmd(dcco_create_cmd,1)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drdc_repl_cleanup(drc_owner,drc_tbl_name,drc_updt_str,drc_where_str,drc_parallel_cnt)
   DECLARE drc_tbl_exists = i4 WITH protect, noconstant(0)
   DECLARE drc_create_cmd = vc WITH protect, noconstant("")
   IF (dm2_table_and_ccldef_exists(drc_tbl_name,drc_tbl_exists)=0)
    RETURN(0)
   ENDIF
   IF (drc_tbl_exists=0)
    SET dm_err->eproc = concat("Table ",drc_tbl_name,
     " does not exist, skipping replicate data cleanup.")
    CALL disp_msg("",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   SET drc_create_cmd = concat("RDB ASIS(^ ","declare ","  update_cnt number := 0; ",
    "  commit_cnt number := ",trim(cnvtstring(drdc_commit_cnt)),
    "; ","begin ","select sum(column_value) into update_cnt ",
    "  from table(dm2_repl_data_cleanup(cursor(select /",char(42),
    "+ PARALLEL(d,",trim(cnvtstring(drc_parallel_cnt)),") ",char(42),"/ d.rowid from  ",
    "      ",drc_owner,".",drc_tbl_name," d  ",
    "    where ",drc_where_str,"),",drc_updt_str,", commit_cnt)); ",
    "dbms_output.put_line('CNT='||update_cnt); ","exception  ","when others then  ","  raise;  ",
    "end; ^) go")
   IF (dm2_push_cmd(drc_create_cmd,1)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drdc_set_parallel_degree(dspd_o_parallel_cnt)
   DECLARE dspd_parallel_override = i4 WITH protect, noconstant(0)
   DECLARE dspd_cpu_count = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Checking for PARALLEL OVERRIDE in dm_info"
   SET dspd_parallel_override = 0
   SELECT INTO "nl:"
    d.info_number
    FROM dm_info d
    WHERE d.info_domain="DM2_REPL_CLEANUP"
     AND d.info_name="PARALLEL_DEGREE"
    DETAIL
     dspd_parallel_override = d.info_number
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (dspd_parallel_override > 0)
    IF (dspd_parallel_override > 6)
     SET dspd_o_parallel_cnt = 6
    ELSE
     SET dspd_o_parallel_cnt = dspd_parallel_override
    ENDIF
   ELSE
    SET dm_err->eproc = "Set parallel degree default value to CPU_COUNT"
    SET dspd_o_parallel_cnt = 0
    SELECT INTO "nl:"
     v.value
     FROM v$parameter v
     WHERE v.name="cpu_count"
     DETAIL
      dspd_cpu_count = cnvtint(v.value)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (dspd_cpu_count > 0)
     IF (dspd_cpu_count > 2)
      SET dspd_o_parallel_cnt = 2
     ELSE
      SET dspd_o_parallel_cnt = dspd_cpu_count
     ENDIF
    ELSE
     SET dspd_o_parallel_cnt = 2
    ENDIF
...
#exit_program
 ...
 SET dm_err->eproc = "dm2_repl_data_cleanup completed"
 CALL final_disp_msg("dm2_repl_data_cleanup")
END GO
