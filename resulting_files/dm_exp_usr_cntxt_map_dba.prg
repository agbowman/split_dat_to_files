CREATE PROGRAM dm_exp_usr_cntxt_map:dba
 ...
 END ;Subroutine
 DECLARE dducm_filename = vc WITH protect, noconstant(" ")
 DECLARE dducm_cnt = i4 WITH protect, noconstant(0)
 ...
 SET dm_err->eproc = "Starting dm_exp_usr_cntxt_map"
 IF (check_logfile("dm_exp_usr_cntxt_map",".log","DM_EXP_USR_CNTXT_MAP LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_mon
 ENDIF
 SET dducm_filename = dmda_get_file_name(dducm_request->env_id,dducm_request->env_name,
  "USER-CONTEXT MAPPING EXPORT","rdds_user_cntxt_map",".csv",
  "export")
 IF (dducm_filename="-1")
  SET dm_err->eproc = "User chose to exit USER-CONTEXT MAPPING EXPORT"
  CALL disp_msg("User exit USER-CONTEXT MAPPING EXPORT",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SELECT INTO value(dducm_filename)
  p.username, r.context_name, delete_ind = "0"
  FROM dm_refchg_prsnl_ctx_r r,
   prsnl p
  WHERE r.active_ind=1
   ...
   AND p.person_id=r.prsnl_id
  ORDER BY r.context_name, p.username
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dducm_cnt = curqual
 SET drcm_files->cnt = 1
 SET stat = alterlist(drcm_files->qual,drcm_files->cnt)
 SET drcm_files->qual[drcm_files->cnt].file_name = dducm_filename
#exit_program
 IF (trim(dducm_filename) != "-1")
  CALL drcm_file_complete("USER-CONTEXT MAPPING EXPORT",drcm_files,"Export",dducm_cnt,dm_err->err_ind,
   dducm_request->env_name,dducm_request->env_id)
 ENDIF
END GO
