CREATE PROGRAM dm_dm2_tspace_size_rows:dba
 ...
 DECLARE v_tablename = vc WITH noconstant("DM2_TSPACE_SIZE1")
 ...
 ENDIF
 DECLARE ddts_max_rows = i4 WITH protect, noconstant(0)
 DECLARE ddts_max_out = i2 WITH protect, noconstant(0)
 SET reply->status_data.status = "F"
 SET reply->table_name = v_tablename
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 IF (check_logfile("DM_DM2_TSPACE_SIZE",".log","DM_DM2_TSPACE_SIZE_ROWS log file")=0)
  SET reply->status_data.status = "F"
  SET reply->err_code = dm_err->ecode
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"LOGERROR",
   "Failed in initialization of log file: %1","s",nullterm(dm_err->emsg))
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Begin DM_DM2_TSPACE_SIZE_ROWS"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  SET reply->status_data.status = "F"
  SET reply->err_code = dm_err->ecode
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"ORATBLERR",
   "Failed to check for Oracle table existence: %1","s",nullterm(dm_err->emsg))
  GO TO end_program
 ENDIF
 ...
 SET dm_err->eproc = build("Find rows older than ",v_days_to_keep," days to purge")
 CALL disp_msg("",dm_err->logfile,0)
 IF (v_archi_version >= 2.0)
  SET v_parser_stmt = sbr_getrowidnotexists(
   "d.updt_dt_tm < cnvtdatetime(curdate - v_days_to_keep, curtime3)","d")
 ELSE
  SET v_parser_stmt = "d.updt_dt_tm < cnvtdatetime(curdate - v_days_to_keep, curtime3)"
 ENDIF
 SELECT INTO "nl:"
  d.rowid
  FROM dm2_tspace_size1 d
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERR",
   "Failed to collect rows for purging: %1","s",nullterm(dm_err->emsg))
  GO TO end_program
 ENDIF
 SET reply->status_data.status = "S"
 SET reply->err_code = 0
 ...
 ENDIF
 SET dm_err->eproc = "Ending DM_DM2_TSPACE_SIZE_ROWS"
 CALL final_disp_msg("DM_DM2_TSPACE_SIZE")
END GO
