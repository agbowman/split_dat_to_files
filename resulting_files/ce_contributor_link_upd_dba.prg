CREATE PROGRAM ce_contributor_link_upd:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 SET error_code = 0
 UPDATE  FROM ce_contributor_link t,
  ...
  SET t.event_id = evaluate2(
    ...
    ENDIF
    ), t.contributor_event_id = evaluate2(
    IF ((request->lst[d.seq].contributor_event_id=- (1))) 0
    ELSE request->lst[d.seq].contributor_event_id
    ENDIF
    ), t.ce_valid_from_dt_tm = cnvtdatetimeutc(request->lst[d.seq].ce_valid_from_dt_tm),
   t.type_flag = evaluate2(
    IF ((request->lst[d.seq].type_flag=- (1))) 0
    ELSE request->lst[d.seq].type_flag
    ENDIF
  ...
  PLAN (d)
   ...
   WHERE (t.event_id=request->lst[d.seq].event_id)
    AND (t.contributor_event_id=request->lst[d.seq].contributor_event_id)
