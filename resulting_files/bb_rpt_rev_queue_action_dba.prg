CREATE PROGRAM bb_rpt_rev_queue_action:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 IF (validate(i18nuar_def,999)=999)
  ...
  persist
 ENDIF
 DECLARE i18nhandle = i2 WITH noconstant(0)
 DECLARE h = i2 WITH noconstant(0)
 DECLARE select_ok_ind = i2 WITH noconstant(0)
 DECLARE rpt_cnt = i2 WITH noconstant(0)
 DECLARE rpt_filename = c25
 DECLARE queue_action = i4 WITH constant(27942)
 ...
 SET captions->review_queue_action = uar_i18ngetmessage(i18nhandle,"review_queue_action",
  "BB REVIEW QUEUE ACTION")
 ...
 SET select_ok_ind = 0
 EXECUTE cpm_create_file_name_logical "bb_rev_q_action", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  cv.display, cv.description, cv.active_ind
  FROM code_value cv
  WHERE code_set=queue_action
  HEAD REPORT
   select_ok_ind = 0
  HEAD PAGE
   col 1, captions->as_of_date, col 14,
   curdate"@DATECONDENSED;;d",
   CALL center(captions->database_audit,1,125), col 108,
   captions->page_no, col 120, curpage"##",
   row + 1, col 7, captions->time,
   col 14, curtime"@TIMENOSECONDS;;M",
   CALL center(captions->reason_tool,1,125),
   row + 1,
   CALL center(captions->review_queue_action,1,125), row + 2,
   line = fillstring(122,"-"), line, row + 1,
   col 4, captions->display_name, col 50,
   captions->description, col 96, captions->active,
   row + 1, line, row + 2
  DETAIL
   col 4, cv.display, col 50,
   cv.description
   IF (cv.active_ind=1)
    col 96, captions->yes
