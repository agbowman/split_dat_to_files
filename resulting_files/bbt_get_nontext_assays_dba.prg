 ...
 SELECT INTO "nl:"
  ...
  PLAN (apr
   ...
   WHERE dta.task_assay_cd=apr.task_assay_cd
    AND dta.activity_type_cd=act_cd
  ...
  DETAIL
   IF (dta.task_assay_cd != hold_task_assay)
    ...
    ENDIF
    task_count = (task_count+ 1), hold_task_assay = dta.task_assay_cd, reply->qual[count1].task[
    task_count].task_cd = dta.task_assay_cd,
    reply->qual[count1].task[task_count].task_disp = dta.mnemonic, reply->qual[count1].task[
    task_count].meaning = uar_get_code_meaning(apr.default_result_type_cd)
   ENDIF
  WITH counter
 ;end select
