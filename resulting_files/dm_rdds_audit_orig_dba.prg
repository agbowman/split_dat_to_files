CREATE PROGRAM dm_rdds_audit_orig:dba
 PROMPT
 ...
 DECLARE icustomtable = i4 WITH protect, noconstant(0)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   sdblinkmock = concat("@MERGE",trim(cnvtstring(isrcenvid),3),trim(cnvtstring(di.info_number),3)),
   itgtenvidmock = di.info_number
  WITH nocounter
 ;end select
 IF (itgtenvidmock > 0.0)
  SET sseqmatchstr = concat(trim(replace(sdblinkmock,"@",""),3),"SEQMATCH")
 ELSE
  SET sseqmatchstr = concat(trim(replace(sdblink,"@",""),3),"SEQMATCH")
  SET itgtenvidmock = itgtenvid
 ENDIF
 ...
 FOR (itablecnt = 1 TO itables)
   ...
   IF (((iidx > 0) OR (icontextauditind=0)) )
    ...
    ENDIF
    IF (imergeable != 1)
     CALL echo(concat("Table is not flagged as mergeable.  ",dguc_reply->dtd_hold[itablecnt].tbl_name,
       " will be skipped."))
    ELSEIF (icustomtable > 0)
    ...
    ELSE
     ...
     IF (curqual > 0)
      ...
      SELECT INTO "NL:"
       ...
       ) OR (((o.object_name=concat("REFCHG_PK_WHERE_",table_info->tables[itableidx].table_suffix,
        "_1")) OR (o.object_name=concat("REFCHG_PK_WHERE_",table_info->tables[itableidx].table_suffix,
        "_2"))) )) ))
       DETAIL
        IF (o.object_name="*REFCHG_PK_WHERE*")
      ...
      SET cnt = 0
      SELECT INTO "NL:"
       FROM (parser(concat("user_tab_columns",sdblink)) uc),
        (parser(concat("dm_refchg_pkw_parm",sdblink)) d)
       WHERE (d.table_name=table_info->tables[itableidx].table_name)
        AND d.table_name=uc.table_name
        AND d.column_name=uc.column_name
       ORDER BY d.parm_nbr
       DETAIL
        cnt = (cnt+ 1), table_info->tables[itableidx].src_pkwhere_signature = trim(concat(table_info
          ->tables[itableidx].src_pkwhere_signature,",::",trim(d.column_name,3)," ",trim(uc.data_type,
      ...
      SELECT INTO "NL:"
       ...
       DETAIL
        cnt = (cnt+ 1), table_info->tables[itableidx].tgt_pkwhere_signature = trim(concat(table_info
          ->tables[itableidx].tgt_pkwhere_signature,",::",trim(d.column_name,3)," ",trim(uc.data_type,
      ...
      SET cnt = 0
      SELECT INTO "NL:"
       d.column_name, uc.data_type
       FROM user_tab_columns uc,
        dm_refchg_pkw_parm d
       WHERE (d.table_name=table_info->tables[itableidx].table_name)
        AND d.table_name=uc.table_name
        AND ((d.column_name=uc.column_name) UNION (
       (SELECT
        d.column_name, uc.data_type
        FROM (parser(concat("user_tab_columns",sdblink)) uc),
         (parser(concat("dm_refchg_pkw_parm",sdblink)) d)
       ...
       DETAIL
        cnt = (cnt+ 1), table_info->tables[itableidx].cmp_pkwhere_signature = trim(concat(table_info
          ->tables[itableidx].cmp_pkwhere_signature,",::",trim(d.column_name,3)," ",trim(uc.data_type,
      ...
      SELECT INTO "NL:"
       ...
       DETAIL
        ...
        ELSE
         iidx = locateval(inum,1,size(exception_columns->columns,5),dc.column_name,exception_columns
          ->columns[inum].column_name)
         IF (iidx > 0)
          table_info->tables[itableidx].columns[cnt].translation_ind = 1
         ENDIF
        ENDIF
        ...
        ELSE
         ...
         IF ((table_info->tables[itableidx].columns[icolumnidx].parent_entity_col != ""))
          ...
          ELSE
           ...
           SET stat = alterlist(parser_buffer->qual,19)
           SET parser_buffer->qual[1].line = concat("select distinct into ",'"nl:"')
           SET parser_buffer->qual[2].line = concat("trim(t.",table_info->tables[itableidx].columns[
            icolumnidx].parent_entity_col,",3)")
           ...
           SET parser_buffer->qual[4].line = concat("where trim(t.",table_info->tables[itableidx].
            columns[icolumnidx].parent_entity_col,",3) > ''")
           SET parser_buffer->qual[5].line = concat("order ",table_info->tables[itableidx].columns[
            icolumnidx].parent_entity_col)
           SET parser_buffer->qual[6].line = "head report"
           SET parser_buffer->qual[7].line = "  iNum = 0"
           SET parser_buffer->qual[8].line = "detail"
           SET parser_buffer->qual[9].line =
           "nPESize = size(table_info->tables[iTableIdx].columns[iColumnIdx].parent_entity_vals, 5)"
           SET parser_buffer->qual[10].line = concat("iIdx = locateval(iNum, 1, nPESize,","trim(t.",
            table_info->tables[itableidx].columns[icolumnidx].parent_entity_col,",3),",
            "table_info->tables[iTableIdx].columns[iColumnIdx].parent_entity_vals[iNum].pe_name)")
           SET parser_buffer->qual[11].line = "if (iIdx = 0)"
           SET parser_buffer->qual[12].line = "cnt = cnt + 1"
           SET parser_buffer->qual[13].line =
           "stat = alterlist(table_info->tables[iTableIdx].columns[iColumnIdx].parent_entity_vals,cnt)"
           SET parser_buffer->qual[14].line = concat(
            "table_info->tables[iTableIdx].columns[iColumnIdx].parent_entity_vals[cnt].pe_name = trim(t.",
            table_info->tables[itableidx].columns[icolumnidx].parent_entity_col,",3)")
           ...
           SET parser_buffer->qual[16].line = "endif"
           SET parser_buffer->qual[17].line = "foot report"
           SET parser_buffer->qual[18].line = "  iNum = iNum"
           SET parser_buffer->qual[19].line = "with nocounter go"
           SET stat = execute_ccl(cnt)
           IF (((check_error(dm_err->eproc)=1) OR (stat=0)) )
            SET dm_err->emsg = concat("Target parent entity check failed for ",table_info->tables[
           ...
           SET stat = execute_ccl(cnt)
           IF (((check_error(dm_err->eproc)=1) OR (stat=0)) )
            SET dm_err->emsg = concat("Source parent entity check failed for ",table_info->tables[
             itableidx].table_name)
            CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
            SET dm_err->err_ind = 0
           ENDIF
           ...
           FOR (i = 1 TO size(table_info->tables[itableidx].columns[icolumnidx].parent_entity_vals,5)
             ...
             SET ssubstr = ""
             SET dm_err->eproc =
             "Check for table level parent entity overrides for the current table"
             SELECT INTO "nl:"
              FROM dm_info d
              WHERE info_domain=concat("RDDS_PE_ABBREV:",trim(table_info->tables[itableidx].
                table_name,3))
               AND info_name=patstring(concat("*",trim(table_info->tables[itableidx].columns[
                 icolumnidx].parent_entity_vals[i].pe_name,3)))
              DETAIL
               table_info->tables[itableidx].columns[icolumnidx].parent_entity_vals[i].pe_name = d
               .info_char
              WITH nocounter
             ;end select
             IF (check_error(dm_err->eproc)=1)
              SET dm_err->emsg = "Table specific parent entity override check failed"
              CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
              SET dm_err->err_ind = 0
             ENDIF
             IF (curqual=0)
              SET dm_err->eproc = "Check for any generic parent entity overrides"
              SELECT INTO "nl:"
               FROM dm_info d
               WHERE d.info_domain="RDDS_PE_ABBREVIATIONS"
                AND (d.info_name=table_info->tables[itableidx].columns[icolumnidx].
             ...
             IF (iidx=0)
              ...
              SET dguc_request->req_special[10].sp_tbl = "ACCOUNT"
              SET dguc_request->req_special[11].sp_tbl = "AT_ACCT_RELTN"
              SET icnt = locateval(inum,1,size(dguc_request->req_special,5),dguc_request->what_tables,
               dguc_request->req_special[inum].sp_tbl)
              IF (icnt=0)
               SET ireferenceind = 0
              ELSE
               SET ireferenceind = 1
             ...
             SET dm_err->eproc = "Check for the root entity information for the parent entity column"
             SET parser_buffer->qual[1].line = concat("select into ",'"nl:"')
             ...
             SET parser_buffer->qual[7].line =
             "table_info->tables[iTableIdx].columns[iColumnIdx].parent_entity_vals[i].root_entity_attr = t.root_entity_attr"
             SET parser_buffer->qual[8].line =
             "table_info->tables[iTableIdx].columns[iColumnIdx].parent_entity_vals[i].root_entity_name = t.root_entity_name"
             SET parser_buffer->qual[9].line =
      ...
      CALL parser(concat("declare dm_rdds_log_type_src(",substring(2,size(replace(table_info->tables[
          ...
          src_pkwhere_parameters,"::","in_")),",in_iContextInd) = c30 go"))
      CALL parser(concat("declare dm_rdds_log_type_tgt(",substring(2,size(replace(table_info->tables[
          ...
          tgt_pkwhere_parameters,"::","in_")),",in_iContextInd) = c30 go"))
       ...
       IF ((table_info->tables[itableidx].columns[x].root_entity_name > ""))
        IF ((table_info->tables[itableidx].columns[x].no_backfill_ind != 1))
         SELECT INTO "NL:"
          ...
          WHERE cv.code_set=255351
           AND cv.active_ind=1
           AND cv.cdf_meaning IN ("ALG1", "ALG2")
      ...
      EXECUTE dm_rdds_compare_table_orig
 ...
 SUBROUTINE create_functions(dummy)
   ...
   SET ssql = concat("create or replace function rdds_diff_translate_src (",char(10),
    ...
    "   if (seq_match >= iFromValue or iFromValue=0 or iExceptionFlg = 1) then       ",char(10),
    "       return (iFromValue); 				",char(10),"   end if;									",
    char(10),"   if (root_table_name = 'PRSNL') then	 	",char(10),
    "	    v_alt_root_table := 'PERSON';		",char(10),
    "	elsif (root_table_name = 'PERSON')then 	",char(10),"       v_alt_root_table := 'PRSNL';		",char
    (10),"	else									",
    char(10),"       v_alt_root_table := root_table_name;",char(10),"   end if;									",char(10),
    "   begin 									",char(10),
    "   select nvl(max(dmt.to_value),-1) into v_to_value from dm_merge_translate dmt ",char(10),
    "     where dmt.env_source_id = ",
    cnvtstring(isrcenvid),char(10),"         and dmt.env_target_id = ",cnvtstring(itgtenvidmock),char
    (10),
    "         and dmt.table_name in (v_alt_root_table, root_table_name) ",char(10),
    "         and dmt.from_value = iFromValue;",char(10),
    "    if (v_to_value > 0) then return (v_to_value); end if;",
    char(10),"   end;",char(10),"begin ",char(10),
    "select nvl(max(dmt.from_value),-1) into v_to_value from dm_merge_translate",sdblink," dmt ",char
    (10),"where dmt.env_source_id = ",
    cnvtstring(itgtenvidmock),"  ",char(10),"      and dmt.env_target_id = ",cnvtstring(isrcenvid),
    "  ",char(10),"      and dmt.table_name in (v_alt_root_table, root_table_name) ",char(10),
    "      and dmt.to_value = iFromValue; ",
    char(10),"   if (iMergeDelete = 1 and v_to_value = -1) then ",char(10),
    "      return(iFromValue);		   ",char(10),
    "   elsif (iDefAttInd = 0 and v_to_value = -1) then ",char(10),"      return(0);		   ",char(10),
    "	  else						   ",
    char(10),"       return(v_to_value);                ",char(10),"   end if;						   ",char(10),
    "end; ",char(10),"end;")
   ...
   SET ssql = concat("create or replace function rdds_diff_translate_tgt(",char(10),
    ...
    char(10),"if (root_table_name = 'PRSNL') then        ",char(10),
    "    v_alt_root_table := 'PERSON';		    ",char(10),
    "elsif (root_table_name = 'PERSON') then    ",char(10),"       v_alt_root_table := 'PRSNL';		",
    char(10),"else									    ",
    char(10),"       v_alt_root_table := root_table_name;",char(10),"end if;									",char(10),
    "begin ",char(10),
    "select nvl(max(dmt.from_value),-1) into v_to_value from dm_merge_translate dmt  ",char(10),
    "where dmt.env_source_id = ",
    cnvtstring(isrcenvid),"  ",char(10),"  and dmt.env_target_id = ",cnvtstring(itgtenvidmock),
    "  ",char(10),"      and dmt.table_name in (v_alt_root_table, root_table_name) ",char(10),
    "      and dmt.to_value = iFromValue; ",
    char(10)," if (v_to_value > 0) then return (v_to_value); end if; ",char(10),"end; ",char(10),
    "begin ",char(10),"select nvl(max(dmt.to_value),-1) into v_to_value from dm_merge_translate",
    sdblink," dmt ",
    char(10),"where dmt.env_source_id = ",cnvtstring(itgtenvidmock),"  ",char(10),
    "  and dmt.env_target_id = ",cnvtstring(isrcenvid),"  ",char(10),
    "      and dmt.table_name = root_table_name ",
    char(10),"      and dmt.from_value = iFromValue; ",char(10),
    "   if (iMergeDelete = 1 and v_to_value = -1) then     ",char(10),
    "      return(iFromValue); 	   ",char(10),"   elsif (iDefAttInd = 0 and v_to_value = -1) then ",
    char(10),"      return(0);		   ",
    char(10),"	  else						   ",char(10),"       return(v_to_value);         ",char(10),
 ...
 SUBROUTINE create_table_functions(dummy)
   ...
   SET ssql = concat("rdb asis(^create or replace function ::FUNCTIONNAME(",char(10),"::PKSIG,",char(
    ...
    char(10),") return varchar is ",char(10),"v_rtn varchar2(30);",char(10),
    "v_pk_where varchar2(500);",char(10),"v_hash number;",char(10),"begin ",
    char(10),"select ::PKWHERE::DBLINK",char(10),"('INS/UPD'::PKPARM",char(10),
   ...
   SET ssql2 = concat("rdb asis(^create or replace function ::FUNCTIONNAME(",char(10),"::PKSIG,",char
    (10),"iContextInd number",
    char(10),") return varchar is ",char(10),"v_rtn varchar2(30);",char(10),
    "v_pk_where_src varchar2(500);",char(10),"v_hash_src number;",char(10),"v_rtn_src varchar2(30);",
    char(10),"v_src_id number;",char(10),"v_rtn_tgt varchar2(30);",char(10),
    "v_tgt_id number;",char(10),"v_pk_where_tgt varchar2(500);",char(10),"v_hash_tgt number;",
    char(10),"begin ",char(10),"select ::PKWHERESRC::DBLINK",char(10),
    "('INS/UPD'::PKPARMSRC",char(10),") into v_pk_where_src from dual;",char(10),
    ...
    char(10),"('INS/UPD'::PKPARMTGT",char(10),") into v_pk_where_tgt from dual;",char(10),
   ...
   SET ssqlcmpfunction = replace(ssqlcmpfunction,"::PKWHERETGT",table_info->tables[itableidx].
    tgt_pkwhere_function)
   SET ssqlsrcfunction = replace(ssqlsrcfunction,"::PKPARM",replace(table_info->tables[itableidx].
     src_pkwhere_parameters,"::",""))
   SET ssqltgtfunction = replace(ssqltgtfunction,"::PKPARM",replace(table_info->tables[itableidx].
     tgt_pkwhere_parameters,"::",""))
   SET ssqlcmpfunction = replace(ssqlcmpfunction,"::PKPARMSRC",replace(table_info->tables[itableidx].
     src_pkwhere_parameters,"::",""))
   SET ssqlcmpfunction = replace(ssqlcmpfunction,"::PKPARMTGT",replace(table_info->tables[itableidx].
     tgt_pkwhere_parameters,"::",""))
   ...
   SELECT INTO value(sfile)
    ...
    DETAIL
     ...
     FOR (iaudittypeidx = 1 TO size(audit_info->audit_types,5))
       ...
       ELSE
        CALL print(concat("<row_count>",trim(audit_info->audit_types[iaudittypeidx].audit_msg,3),
         "</row_count>")), row + 1
       ENDIF
