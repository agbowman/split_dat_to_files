CREATE PROGRAM cp_get_mpage_settings:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Bedrock Category Mean:" = "",
  "Personnel ID:" = 0.00,
  "Provider Position Code:" = 0.00,
  "Output Record Indicator: " = 0,
  "Ignore Cache Ind: " = 0,
  "Encounter ID" = 0.0,
  "Pathway ID" = 0.0,
  "Concept CD" = 0.0,
  "Instance ID" = 0.0,
  "Load Unpublished Indicator:" = 0,
  "Don't Load CP Configuration Indicator:" = 0,
  "Application Number:" = 0
  WITH outdev, category_mean, prsnl_id,
  position_cd, output_record_ind, nocache_ind,
  encntr_id, pw_pathway_id, pw_concept_cd,
  pw_instance_id, pw_unpublished_ind, no_load_cp_config_ind,
  app_number
 ...
 DECLARE addorgsettings(null) = null
 DECLARE lappnbr = i4 WITH protect, noconstant(0)
 RECORD reply(
   1 mpage[*]
     2 settings
       3 c_id = f8
 ...
 SUBROUTINE main(null)
   EXECUTE mp_get_mpage_settings "NOFORMS",  $CATEGORY_MEAN,  $PRSNL_ID,
    $POSITION_CD,  $OUTPUT_RECORD_IND,  $NOCACHE_IND,
    $ENCNTR_ID, lappnbr
   SET stat = cnvtjsontorec(_memory_reply_string)
   SET stat = moverec(mp_settings->mpage,reply->mpage)
   CALL addorgsettings(null)
   IF (( $NO_LOAD_CP_CONFIG_IND=0))
    EXECUTE cp_br_load_pathway_config "NOFORMS",  $PW_PATHWAY_ID,  $PW_CONCEPT_CD,
     $PW_INSTANCE_ID,  $PW_UNPUBLISHED_IND
   ENDIF
 END ;Subroutine
 SUBROUTINE addorgsettings(null)
   CALL log_message("Begin addOrgSettings()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE orglevelcatname = vc WITH protect, constant("Care Pathway Organization Level Settings")
   DECLARE treatmentoptionbr = vc WITH protect, constant("PW_TREATMENT_OPTION")
   DECLARE guidedtreatmentbr = vc WITH protect, constant("PW_GUIDED_TREATMENT")
   DECLARE cporgcategorymean = c30 WITH protect, noconstant("")
   DECLARE tlsettingspos = i2 WITH protect, noconstant(0)
   DECLARE topos = i2 WITH protect, noconstant(0)
   DECLARE x = i2 WITH protect, noconstant(0)
   DECLARE y = i2 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM br_datamart_category bdc
    WHERE bdc.category_name=orglevelcatname
    HEAD REPORT
     cporgcategorymean = bdc.category_mean
    WITH nocounter
   ;end select
   SET stat = initrec(mp_settings)
   EXECUTE mp_get_mpage_settings "NOFORMS", cporgcategorymean,  $PRSNL_ID,
    $POSITION_CD,  $OUTPUT_RECORD_IND,  $NOCACHE_IND,
    $ENCNTR_ID, lappnbr
   SET stat = cnvtjsontorec(_memory_reply_string)
   IF ((mp_settings->mpage[1].settings.br_set.cs[1].r_mn IN (treatmentoptionbr, guidedtreatmentbr)))
    SET topos = locateval(x,1,size(reply->mpage[1].settings.br_set.cs,5),guidedtreatmentbr,reply->
     mpage[1].settings.br_set.cs[x].r_mn)
    IF (topos=0)
     SET topos = locateval(x,1,size(reply->mpage[1].settings.br_set.cs,5),treatmentoptionbr,reply->
      mpage[1].settings.br_set.cs[x].r_mn)
    ENDIF
    IF (topos > 0)
     FOR (i = 1 TO size(mp_settings->mpage[1].settings.br_set.cs[1].filt,5))
      SET tlsettingspos = locateval(y,1,size(reply->mpage[1].settings.br_set.cs[topos].filt,5),
       mp_settings->mpage[1].settings.br_set.cs[1].filt[i].f_mn,reply->mpage[1].settings.br_set.cs[
       topos].filt[y].f_mn)
      IF (tlsettingspos=0)
       SET stat = movereclist(mp_settings->mpage[1].settings.br_set.cs[1].filt,reply->mpage[1].
        settings.br_set.cs[topos].filt,i,0,1,
        true)
      ENDIF
     ENDFOR
    ENDIF
   ENDIF
   CALL log_message(build("Exit addOrgSettings(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 IF (( $APP_NUMBER > 0))
  SET lappnbr =  $APP_NUMBER
 ELSE
  SET lappnbr = reqinfo->updt_app
 ENDIF
 CALL main(null)
