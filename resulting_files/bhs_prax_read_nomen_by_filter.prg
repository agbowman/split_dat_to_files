CREATE PROGRAM bhs_prax_read_nomen_by_filter
 FREE RECORD result
 RECORD result(
   1 items[*]
 ...
 DECLARE sortresults(mode=i2) = i4
...
#exit_script
 ...
 SUBROUTINE callallergysearch(null)
   ...
   SET req963000->compare_dt_tm = cnvtdatetime(curdate,curtime3)
 ...
 SUBROUTINE sortresults(mode)
   ...
   IF (size(nomenclatures->items,5) > 0)
    ...
    SELECT INTO "NL:"
     ...
     ORDER BY sortkey
     DETAIL
      resultcnt = (resultcnt+ 1), result->items[resultcnt].nomenclature_id = nomenclatures->items[d
      .seq].nomenclature_id, result->items[resultcnt].source_string = nomenclatures->items[d.seq].
      source_string,
