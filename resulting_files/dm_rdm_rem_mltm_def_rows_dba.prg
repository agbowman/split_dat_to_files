CREATE PROGRAM dm_rdm_rem_mltm_def_rows:dba
 ...
 SET readme_data->message = "Readme Failed: Starting script dm_rdm_rem_mltm_def_rows.prg..."
 FREE RECORD tbl_names
 RECORD tbl_names(
   1 list[*]
     2 tbl_name = vc
 )
 SET stat = alterlist(tbl_names->list,15)
 SET tbl_names->list[1].tbl_name = "MLTM_COMBINATION_DRUG"
 SET tbl_names->list[2].tbl_name = "MLTM_DOSE_FORM"
 SET tbl_names->list[3].tbl_name = "MLTM_DRUG_CATEGORIES"
 SET tbl_names->list[4].tbl_name = "MLTM_DRUG_NAME"
 SET tbl_names->list[5].tbl_name = "MLTM_LABEL"
 SET tbl_names->list[6].tbl_name = "MLTM_NDC_ACT_INGRED_LIST"
 SET tbl_names->list[7].tbl_name = "MLTM_NDC_ACTIVE_INGRED"
 SET tbl_names->list[8].tbl_name = "MLTM_NDC_BRAND_NAME"
 SET tbl_names->list[9].tbl_name = "MLTM_NDC_INGRED_STRENGTH"
 SET tbl_names->list[10].tbl_name = "MLTM_NDC_MAIN_DRUG_CODE"
 SET tbl_names->list[11].tbl_name = "MLTM_NDC_ORANGE_BOOK"
 SET tbl_names->list[12].tbl_name = "MLTM_NDC_SOURCE"
 SET tbl_names->list[13].tbl_name = "MLTM_PRODUCT_ROUTE"
 SET tbl_names->list[14].tbl_name = "MLTM_PRODUCT_STRENGTH"
 SET tbl_names->list[15].tbl_name = "MLTM_UNITS"
 ...
 SELECT INTO "nl:"
  FROM dm2_user_triggers dut,
   (dummyt d  WITH seq = value(size(tbl_names->list,5)))
  PLAN (d)
   JOIN (dut
   WHERE (dut.table_name=tbl_names->list[d.seq].tbl_name)
 ...
 IF (trg_cnt > 0)
  ...
  ENDFOR
 ENDIF
 FOR (mltm_i = 1 TO size(tbl_names->list,5))
   CALL echo(concat("Dropping Default Row for Table ",tbl_names->list[mltm_i].tbl_name,"..."))
   EXECUTE dm2_undo_default_rows tbl_names->list[mltm_i].tbl_name
   IF ((dm_err->err_ind=1))
    SET readme_data->message = concat("Readme Failed: Unable to Remove Default Row for ",tbl_names->
     list[mltm_i].tbl_name)
    GO TO exit_script
   ENDIF
 ENDFOR
 ...
 IF (currdb="ORACLE")
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (dtd
    WHERE (dtd.table_name=tbl_names->list[d.seq].tbl_name))
    JOIN (ut
 ...
 ENDIF
 IF (trg_remaining > 0)
  SET readme_data->message = "Readme Failed: Not all Triggers could be removed."
 ELSE
  SET readme_data->status = "S"
  SET readme_data->message = "Readme Success: All Default Rows Removed and Triggers Dropped"
 ENDIF
#exit_script
 FREE RECORD trg_names
 FREE RECORD tbl_names
