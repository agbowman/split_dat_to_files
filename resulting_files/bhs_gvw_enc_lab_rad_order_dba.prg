CREATE PROGRAM bhs_gvw_enc_lab_rad_order:dba
 ...
 DECLARE mf_cs14281_collected_cd = f8 WITH protect, constant(uar_get_code_by_cki(
   "CKI.CODEVALUE!12587"))
 DECLARE mf_cs14281_inprocess_cd = f8 WITH protect, constant(uar_get_code_by_cki(
   "CKI.CODEVALUE!12589"))
 ...
 SELECT INTO "nl:"
  ...
  WHERE (o.person_id=request->person[1].person_id)
   ...
   AND o.active_ind=1
   AND o.order_status_cd IN (mf_ordered_cd, mf_inprocess_cd, mf_pending_cd, mf_completed_cd)
   AND o.catalog_type_cd IN (mf_laboratory_cd, mf_radiology_cd)
 ...
 IF ((prsn_orders->n_cnt > 0))
  ...
  ENDFOR
 ELSE
  SET s_text = concat(s_text," No tests performed during this visit. \par ")
 ENDIF
