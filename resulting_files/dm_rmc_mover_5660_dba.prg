CREATE PROGRAM dm_rmc_mover_5660:dba
 ...
 SUBROUTINE drmm_breakup_string(sbr_rs_reply)
   ...
   RETURN("S")
 END ;Subroutine
 DECLARE cust_alg_type = vc
 DECLARE cust_tab_name = vc
 DECLARE cust_comp_name_pos = i4
 DECLARE cust_comp_pref_pos = i4
 DECLARE cust_comp_type_cd = f8
 DECLARE cust_cdf_meaning = vc
 DECLARE cust_comp_name_unq = vc
 DECLARE cust_comp_pref = vc
 DECLARE cust_track_pref_id = vc
 DECLARE cust_delim_cnt = i4
 DECLARE cust_delim_vc = vc
 DECLARE cust_bad_delim_ind = i2
 DECLARE cust_val = f8
 DECLARE cust_no_query = i2
 DECLARE cust_code_value_tab = vc
 DECLARE cust_from_id = f8
 DECLARE cust_new_id = f8
 DECLARE cust_idcd_check = i2
 DECLARE cust_bad_ui_query = i2
 DECLARE cust_parent_pref = vc
 DECLARE cust_parent_pref_to = vc
 DECLARE cust_comp_type_to = vc
 DECLARE cust_ui_cnt = i4
 DECLARE cust_comp_name_unq_check = i2
 DECLARE cust_comp_pref_check = i2
 DECLARE cust_ins_upd = i2
 DECLARE cust_delim_list = vc
 DECLARE cust_rpt_missing = vc
 DECLARE cust_tgt_track_pref = f8
 DECLARE cust_tgt_comp_name = vc
 DECLARE cust_query_tgt_ind = i2
 DECLARE cust_nomv_chk = vc WITH protect, noconstant("")
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_bad_delim_ind = 0
 SET cust_comp_name_pos = get_col_pos(cust_tab_name,"COMP_NAME_UNQ")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_comp_name_pos].translated = 0
 SET cust_comp_pref_pos = get_col_pos(cust_tab_name,"COMP_PREF")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_comp_pref_pos].translated = 0
 SET cust_comp_type_cd = cnvtreal(get_value(cust_tab_name,"COMP_TYPE_CD","FROM"))
 SET cust_comp_name_unq = get_value(cust_tab_name,"COMP_NAME_UNQ","FROM")
 SET cust_comp_pref = get_value(cust_tab_name,"COMP_PREF","FROM")
 SET temp_col_cnt = get_col_pos(cust_tab_name,"TRACK_PREF_ID")
 SET dm_err->eproc = "Beginning dm_rmc_mover_5660"
 IF (cust_comp_type_cd >= 0)
  SET cust_code_value_tab = dm2_get_rdds_tname("CODE_VALUE")
  SET dm_err->eproc = "Querying for COMP_TYPE_CD cdf_meaning"
  SELECT INTO "NL:"
   FROM (parser(cust_code_value_tab) c)
   WHERE c.code_set IN (20500, 20314, 20316)
    AND c.code_value=cust_comp_type_cd
   DETAIL
    cust_cdf_meaning = c.cdf_meaning
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (curqual=0)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV52"
   CALL disp_msg(concat("ERROR: the value of COMP_TYPE_CD is not a valid value."),dm_err->logfile,1)
   SET dm_err->err_ind = 0
   GO TO exit_5660
  ELSE
   SET dm_err->eproc = "Looking up custom ALG_TYPE in code_value"
   SELECT INTO "NL:"
    FROM code_value c
    WHERE c.code_set=4002166
     AND c.display=cust_cdf_meaning
     AND c.active_ind=1
    DETAIL
     cust_alg_type = c.cdf_meaning
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (curqual=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV52"
    CALL disp_msg(concat("ERROR: No ALG type found for the COMP_TYPE_CD."),dm_err->logfile,1)
    SET dm_err->err_ind = 0
    GO TO exit_5660
   ENDIF
  ENDIF
 ELSE
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV52"
  CALL disp_msg(concat("ERROR: the value of COMP_TYPE_CD is not an expected value"),dm_err->logfile,1
   )
  SET dm_err->err_ind = 0
  GO TO exit_5660
 ENDIF
 SET dm_err->eproc = "Performing custom translations on COMP_NAME_UNQ and COMP_PREF columns"
 CASE (cust_alg_type)
  OF "ALG0":
   CALL put_value(cust_tab_name,"COMP_NAME_UNQ",cust_comp_name_unq)
   CALL put_value(cust_tab_name,"COMP_PREF",cust_comp_pref)
  OF "ALG1":
   CALL delim_string(cust_comp_name_unq,";")
   IF (size(delim_rs->qual,5)=0)
    SET stat = alterlist(delim_rs->qual,1)
    SET delim_rs->qual[1].val = cust_comp_name_unq
   ENDIF
   SET cust_delim_list = " "
   FOR (cust_delim_cnt = 1 TO size(delim_rs->qual,5))
     IF (isnumeric(delim_rs->qual[cust_delim_cnt].val) IN (1, 2))
      SET cust_delim_vc = select_merge_translate(delim_rs->qual[cust_delim_cnt].val,"CODE_VALUE")
      IF (isnumeric(cust_delim_vc))
       SET delim_rs->qual[cust_delim_cnt].val = replace(delim_rs->qual[cust_delim_cnt].val,trim(
         cnvtstring(delim_rs->qual[cust_delim_cnt].val,20)),trim(cnvtstring(cust_delim_vc,20)),1)
      ELSE
       ...
       SET cust_delim_cnt = size(delim_rs->qual,5)
       SET cust_bad_delim_ind = 1
      ENDIF
      IF (cust_delim_cnt=1)
       SET cust_delim_list = delim_rs->qual[cust_delim_cnt].val
      ELSE
       SET cust_delim_list = concat(cust_delim_list,";",delim_rs->qual[cust_delim_cnt].val)
      ENDIF
     ELSE
      SET dm2_ref_data_reply->error_ind = 1
      SET dm2_ref_data_reply->error_msg = "NOMV52"
      CALL disp_msg(concat("ERROR: the value of COMP_NAME_UNQ contains an unexpected value"),dm_err->
       logfile,1)
      SET dm_err->err_ind = 0
      GO TO exit_5660
     ENDIF
   ENDFOR
   IF (cust_bad_delim_ind=0)
    CALL put_value(cust_tab_name,"COMP_NAME_UNQ",cust_delim_list)
   ENDIF
   CALL put_value(cust_tab_name,"COMP_PREF",cust_comp_pref)
  OF "ALG2":
   CALL delim_string(cust_comp_name_unq,";")
   IF (size(delim_rs->qual,5) != 2)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV52"
    CALL disp_msg(concat("ERROR: the value of COMP_NAME_UNQ contains an unexpected value"),dm_err->
     logfile,1)
   ...
   ENDIF
   SET cust_delim_list = " "
   SET cust_delim_vc = select_merge_translate(delim_rs->qual[1].val,"CODE_VALUE")
   IF (isnumeric(cust_delim_vc))
    SET delim_rs->qual[1].val = replace(delim_rs->qual[1].val,trim(cnvtstring(delim_rs->qual[1].val,
       20)),trim(cnvtstring(cust_delim_vc,20)),1)
   ELSE
    SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[1].val))
    CALL evaluate_rpt_missing(cust_rpt_missing)
    SET cust_bad_delim_ind = 1
   ENDIF
   SET cust_delim_vc = select_merge_translate(delim_rs->qual[2].val,"TRACK_REFERENCE")
   IF (isnumeric(cust_delim_vc))
    SET delim_rs->qual[2].val = replace(delim_rs->qual[2].val,trim(cnvtstring(delim_rs->qual[2].val,
       20)),trim(cnvtstring(cust_delim_vc,20)),1)
   ELSE
   ...
   ENDIF
   IF (cust_bad_delim_ind=0)
    SET cust_delim_list = concat(delim_rs->qual[1].val,";",delim_rs->qual[2].val)
    CALL put_value(cust_tab_name,"COMP_NAME_UNQ",cust_delim_list)
   ENDIF
   CALL put_value(cust_tab_name,"COMP_PREF",cust_comp_pref)
  OF "ALG3":
   ...
   IF (cust_bad_delim_ind=0)
    CALL put_value(cust_tab_name,"COMP_NAME_UNQ",cust_delim_list)
   ENDIF
   CALL delim_string(cust_comp_pref,"|")
   IF (size(delim_rs->qual,5)=0)
    SET stat = alterlist(delim_rs->qual,1)
    SET delim_rs->qual[1].val = cust_comp_pref
   ENDIF
   SET cust_delim_list = " "
   FOR (cust_delim_cnt = 1 TO size(delim_rs->qual,5))
     IF (isnumeric(delim_rs->qual[cust_delim_cnt].val) IN (1, 2))
      SET cust_delim_vc = select_merge_translate(delim_rs->qual[cust_delim_cnt].val,"NOMEN_CATEGORY")
      IF (isnumeric(cust_delim_vc))
       SET delim_rs->qual[cust_delim_cnt].val = replace(delim_rs->qual[cust_delim_cnt].val,trim(
         cnvtstring(delim_rs->qual[cust_delim_cnt].val,20)),trim(cnvtstring(cust_delim_vc,20)),1)
      ELSE
       SET cust_rpt_missing = report_missing("NOMEN_CATEGORY","NOMEN_CATEGORY_ID",cnvtreal(delim_rs->
         qual[cust_delim_cnt].val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET cust_delim_cnt = size(delim_rs->qual,5)
       SET cust_bad_delim_ind = 1
      ENDIF
     ...
     ELSE
      SET dm2_ref_data_reply->error_ind = 1
      SET dm2_ref_data_reply->error_msg = "NOMV52"
      CALL disp_msg(concat("ERROR: the value of COMP_PREF contains an unexpected value"),dm_err->
       logfile,1)
      SET dm_err->err_ind = 0
      GO TO exit_5660
     ENDIF
   ENDFOR
   IF (cust_bad_delim_ind=0)
    CALL put_value(cust_tab_name,"COMP_PREF",cust_delim_list)
   ENDIF
  OF "ALG4":
   CALL delim_string(cust_comp_name_unq,";")
   IF (size(delim_rs->qual,5)=0)
  ...
  OF "ALG5":
   ...
   IF (size(delim_rs->qual,5)=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV52"
    CALL disp_msg(concat("ERROR: Invalid number of Code_values in COMP_NAME_UNQ"),dm_err->logfile,1)
    SET dm_err->err_ind = 0
    GO TO exit_5660
   ENDIF
   SET cust_delim_list = " "
   ...
   ENDIF
  OF "ALG6":
   SET cust_track_pref_id = get_value(cust_tab_name,"TRACK_PREF_ID","FROM")
   IF (cnvtreal(cust_track_pref_id) != cnvtreal(cust_comp_name_unq))
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV52"
    CALL disp_msg(concat("ERROR: the value of COMP_NAME_UNQ does not match the TRACK_PREF_ID"),dm_err
     ->logfile,1)
    SET dm_err->err_ind = 0
    GO TO exit_5660
   ELSEIF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=1))
    SET cust_delim_list = get_value(cust_tab_name,"TRACK_PREF_ID","TO")
    SET cust_delim_list = replace(cust_comp_name_unq,trim(cnvtstring(cust_comp_name_unq,20)),trim(
      cnvtstring(cust_delim_list,20)),1)
    CALL put_value(cust_tab_name,"COMP_NAME_UNQ",cust_delim_list)
   ENDIF
   CALL put_value(cust_tab_name,"COMP_PREF",cust_comp_pref)
  OF "ALG7":
   SET cust_delim_list = " "
   IF (substring(1,1,cust_comp_name_unq)="^")
    SET cust_delim_list = substring(2,(size(cust_comp_name_unq,1) - 1),cust_comp_name_unq)
    SET cust_delim_vc = select_merge_translate(cust_delim_list,"CODE_VALUE")
    IF (isnumeric(cust_delim_vc))
     SET cust_delim_list = replace(cust_comp_name_unq,trim(cnvtstring(cust_delim_list,20)),trim(
       cnvtstring(cust_delim_vc,20)),1)
    ELSE
     SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_delim_list))
     CALL evaluate_rpt_missing(cust_rpt_missing)
     SET cust_bad_delim_ind = 1
    ENDIF
    IF (cust_bad_delim_ind=0)
   ...
   ELSE
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV52"
    CALL disp_msg(concat("ERROR: the value of COMP_NAME_UNQ is missing a ^"),dm_err->logfile,1)
    SET dm_err->err_ind = 0
    GO TO exit_5660
   ENDIF
   SET cust_delim_list = " "
   IF (isnumeric(cust_comp_pref) IN (1, 2))
    SET cust_delim_vc = select_merge_translate(cust_comp_pref,"CODE_VALUE")
    IF (isnumeric(cust_delim_vc))
     SET cust_delim_list = replace(cust_comp_pref,trim(cnvtstring(cust_comp_pref,20)),trim(cnvtstring
       (cust_delim_vc,20)),1)
    ELSE
     SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_comp_pref))
     CALL evaluate_rpt_missing(cust_rpt_missing)
     SET cust_bad_delim_ind = 1
    ENDIF
    IF (cust_bad_delim_ind=0)
   ...
   ELSE
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV52"
    CALL disp_msg(concat("ERROR: the COMP_PREF column contains an unexpected value"),dm_err->logfile,
     1)
    SET dm_err->err_ind = 0
    GO TO exit_5660
   ENDIF
  OF "ALG8":
   ...
   FOR (cust_delim_cnt = 1 TO size(delim_rs->qual,5))
     IF (isnumeric(delim_rs->qual[cust_delim_cnt].val) IN (1, 2))
      SET cust_delim_vc = select_merge_translate(delim_rs->qual[cust_delim_cnt].val,"TRACK_REFERENCE"
       )
      IF (isnumeric(cust_delim_vc))
       SET delim_rs->qual[cust_delim_cnt].val = replace(delim_rs->qual[cust_delim_cnt].val,trim(
         cnvtstring(delim_rs->qual[cust_delim_cnt].val,20)),trim(cnvtstring(cust_delim_vc,20)),1)
  ...
  OF "ALG9":
   ...
   SET cust_delim_list = " "
   IF (isnumeric(cust_comp_pref) IN (1, 2))
    SET cust_delim_vc = select_merge_translate(cust_comp_pref,"APPLICATION_TASK")
    IF (isnumeric(cust_delim_vc))
     SET cust_delim_list = replace(cust_comp_pref,trim(cnvtstring(cust_comp_pref,20)),trim(cnvtstring
       (cust_delim_vc,20)),1)
    ELSE
     SET cust_rpt_missing = report_missing("APPLICATION_TASK","TASK_NUMBER",cnvtreal(cust_comp_pref))
     CALL evaluate_rpt_missing(cust_rpt_missing)
     SET cust_bad_delim_ind = 1
    ENDIF
   ELSE
  ...
  OF "ALG10":
   ...
   ENDIF
   SET cust_delim_list = " "
   IF (isnumeric(delim_rs->qual[1].val) IN (1, 2))
    SET cust_delim_vc = select_merge_translate(delim_rs->qual[1].val,"CODE_VALUE")
    IF (isnumeric(cust_delim_vc))
     SET cust_delim_list = replace(cust_comp_name_unq,trim(cnvtstring(delim_rs->qual[1].val,20)),trim
      (cnvtstring(cust_delim_vc,20)),1)
    ELSE
     SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[1].val))
     CALL evaluate_rpt_missing(cust_rpt_missing)
  ...
  ELSE
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV52"
   CALL disp_msg(concat("ERROR: the ALG_TYPE is unsupported"),dm_err->logfile,1)
   SET dm_err->err_ind = 0
   GO TO exit_5660
 ENDCASE
 SET dm_err->eproc = "Translating the primary_key value"
 SET cust_query_tgt_ind = 1
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  IF (cust_alg_type != "ALG6")
   SET cust_no_query = is_translated(cust_tab_name,"COMP_NAME_UNQ")
   IF (cust_no_query=1)
    SET cust_val = query_target(temp_tbl_cnt,perm_col_cnt)
   ELSE
    SET cust_query_tgt_ind = 0
   ENDIF
  ELSE
   SET cust_delim_list = " "
   SET cust_idcd_check = 0
   SET cust_bad_ui_query = 0
   SET cust_comp_name = get_value(cust_tab_name,"COMP_NAME","TO")
   SET cust_idcd_check = is_translated(cust_tab_name,"COMP_TYPE_CD")
   IF (cust_idcd_check=0)
    SET cust_query_tgt_ind = 0
   ENDIF
   SET cust_idcd_check = is_translated(cust_tab_name,"PARENT_PREF_ID")
   IF (cust_idcd_check=0)
    SET cust_query_tgt_ind = 0
   ENDIF
   IF (cust_query_tgt_ind=1)
    SET cust_parent_pref_to = get_value(cust_tab_name,"PARENT_PREF_ID","TO")
    SET cust_comp_type_to = get_value(cust_tab_name,"COMP_TYPE_CD","TO")
    SET cust_ui_cnt = 0
    ...
    CALL echo("")
    CALL echo("")
    SET cust_nomv_chk = select_merge_translate(cnvtstring(cust_track_pref_id),cust_tab_name)
    IF (cust_nomv_chk="NOMV21")
    ...
    ENDIF
    SELECT INTO "nl:"
     FROM (parser(cust_tab_name) tp)
     WHERE tp.comp_type_cd=cnvtreal(cust_comp_type_to)
      AND tp.parent_pref_id=cnvtreal(cust_parent_pref_to)
      AND tp.comp_name=cust_comp_name
     DETAIL
      cust_tgt_track_pref = tp.track_pref_id, cust_tgt_comp_name = tp.comp_name_unq
      IF (cust_tgt_track_pref=cnvtreal(cust_tgt_comp_name))
       cust_ui_cnt = (cust_ui_cnt+ 1)
       IF (mod(cust_ui_cnt,10)=1)
        stat = alterlist(ui_query_eval_rec->qual,(cust_ui_cnt+ 9))
       ENDIF
       ui_query_eval_rec->qual[cust_ui_cnt].root_entity_attr = tp.track_pref_id
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     ...
     GO TO exit_5660
    ENDIF
    SET cust_val = evaluate_exec_ui_query(cust_ui_cnt,temp_tbl_cnt,perm_col_cnt)
    IF ((cust_val=- (2)))
     ...
     ROLLBACK
     CALL orphan_child_tab(sbr_table_name,"NOMV06","",0.0)
     COMMIT
    ENDIF
   ENDIF
  ENDIF
  IF (cust_query_tgt_ind=1)
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"TRACK_PREF_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"TRACK_PREF_ID",cnvtstring(cust_val))
      IF (cust_alg_type="ALG6")
       SET cust_comp_name_unq = replace(cust_comp_name_unq,trim(cnvtstring(cust_comp_name_unq,20)),
        trim(cnvtstring(cust_val,20)),1)
       CALL put_value(cust_tab_name,"COMP_NAME_UNQ",cust_comp_name_unq)
      ENDIF
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  TRACK_PREF_ID")
     ELSE
      GO TO exit_5660
     ENDIF
    ELSE
   ...
   ELSE
    ...
    CALL put_value(cust_tab_name,"TRACK_PREF_ID",cnvtstring(cust_val,20))
    IF (cust_alg_type="ALG6")
     SET cust_comp_name_unq = replace(cust_comp_name_unq,trim(cnvtstring(cust_comp_name_unq,20)),trim
      (cnvtstring(cust_val,20)),1)
     CALL put_value(cust_tab_name,"COMP_NAME_UNQ",cust_comp_name_unq)
    ENDIF
   ENDIF
   SET cust_from_id = cnvtreal(get_value(cust_tab_name,"TRACK_PREF_ID","FROM"))
   ...
   ELSE
    ...
    SET dm2_ref_data_reply->qual[1].from_value = cust_from_id
   ENDIF
   SET cust_new_id = cnvtreal(get_value(cust_tab_name,"TRACK_PREF_ID","TO"))
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5660
   ELSE
    SET dm2_ref_data_reply->qual[1].to_value = cust_new_id
   ENDIF
  ENDIF
 ENDIF
 SET cust_idcd_check = 0
 ...
 IF (drdm_error_out_ind=1)
  GO TO exit_5660
 ENDIF
 SET dm_err->eproc = "Checking status of custom translations"
 IF (cust_idcd_check=1)
  SET cust_comp_name_unq_check = is_translated(cust_tab_name,"COMP_NAME_UNQ")
  SET cust_comp_pref_check = is_translated(cust_tab_name,"COMP_PREF")
  SET dm_err->eproc = "Status of translation"
  IF (cust_idcd_check=1
   AND cust_comp_name_unq_check=1
   AND cust_comp_pref_check=1)
   SET cust_ins_upd = insert_update_row(temp_tbl_cnt,perm_col_cnt)
  ELSE
   SET cust_err_val = get_err_val(null)
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    IF (cust_comp_name_unq_check=0)
     SET dm2_ref_data_reply->error_msg =
     "This row wasn't translated because of the COMP_NAME_UNQ column"
    ELSE
     SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the COMP_PREF column"
    ENDIF
 ...
 ENDIF
#exit_5660
 FREE SET cust_alg_type
 FREE SET cust_tab_name
 FREE SET cust_delim_cnt
 FREE SET cust_delim_vc
 FREE SET cust_bad_delim_ind
 FREE SET cust_no_query
 FREE SET cust_val
 FREE SET cust_from_id
 FREE SET cust_new_id
 FREE SET cust_idcd_check
 FREE SET cust_comp_name_unq_check
 FREE SET cust_comp_pref_check
 FREE SET cust_ins_upd
END GO
