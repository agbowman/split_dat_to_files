CREATE PROGRAM dm_pcmb_person_code_value_r_hi:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PCMB_PERSON_CODE_VALUE_R_HI"
 ...
 SELECT INTO "nl:"
  frm.*
  FROM person_code_value_r_hist frm
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].from_pk_id = frm.person_code_value_r_h_id, rreclist->from_rec[
   v_cust_count1].prev_active_ind = frm.active_ind, rreclist->from_rec[v_cust_count1].
 ...
 IF (v_cust_count1 > 0)
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
    IF (del_from(rreclist->from_rec[v_cust_loopcount].from_pk_id,v_cust_loopcount)=0)
     GO TO end_prog
    ENDIF
  ENDFOR
 ENDIF
 SUBROUTINE del_from(s_pcvrh_id,s_frm_cnt)
   UPDATE  FROM person_code_value_r_hist p
    SET p.person_id = request->xxx_combine[icombine].to_xxx_id, p.active_ind = false, p
     .active_status_cd = combinedaway,
    ...
    WHERE p.person_code_value_r_h_id=s_pcvrh_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
   SET request->xxx_combine_det[icombinedet].combine_action_cd = del
   SET request->xxx_combine_det[icombinedet].entity_id = s_pcvrh_id
   ...
   IF (curqual=0)
    ...
    SET request->error_message = build(
     "DEL_FROM: No values found on the person_code_value_r_hist table with person_code_value_r_h_id = ",
     s_pcvrh_id)
    RETURN(0)
   ENDIF
   RETURN(1)
