CREATE PROGRAM bhs_dt_clin_sum2_ops3:dba
 SET unit_cd = 0
 DECLARE unit_cd_disp_key = vc WITH protect, noconstant("")
 SET unit_cd_disp_key = trim( $2)
 SELECT INTO "nl:"
  FROM code_value cv
  PLAN (cv
   WHERE cv.display_key=trim( $2)
    AND cv.code_set=220
    AND cv.cdf_meaning IN ("NURSEUNIT", "AMBULATORY")
    AND cv.active_ind=1)
  DETAIL
   unit_cd = cv.code_value
  WITH nocounter
 ;end select
 IF (unit_cd <= 0.00)
 ...
 SELECT INTO "nl:"
  ...
  FROM encntr_domain ed,
   ...
   person p
  PLAN (ed
   WHERE ed.loc_nurse_unit_cd=unit_cd
   ...
   WHERE p.person_id=e.person_id
    AND p.name_last_key != "ZZZZ")
  ORDER BY unit, room, bed
  HEAD REPORT
   cnt = 0
  DETAIL
   IF (((e.reg_dt_tm >= cnvtlookbehind("36,H",sysdate)
    AND unit_cd_disp_key IN ("EDPEDI", "EDMAIN", "ESHLD", "EDA", "EDGTA",
   "ESA", "ESB", "ESC", "ESD", "ESE",
   "ESP", "ESX", "ESW")) OR ( NOT (unit_cd_disp_key IN ("EDPEDI", "EDMAIN", "ESHLD", "EDA", "EDGTA",
   "ESA", "ESB", "ESC", "ESD", "ESE",
   "ESP", "ESX", "ESW")))) )
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY unit, room, bed,
   e.encntr_id, sec.section_seq, grpr.grouper_seq,
  ...
  HEAD e.encntr_id
   IF (((dta.max_lookback_hours=0) OR (result_age <= dta.max_lookback_hours)) )
    ...
    pat->qual[cnt].eid = e.encntr_id, pat->qual[cnt].unit_room_bed = build(unit,"/",room,"-",bed),
   ...
   IF ((pat->qual[x].filename > " "))
    SELECT INTO value(pat->qual[x].filename)
     ...
     HEAD PAGE
      ...
      row + 1, ycol = (ycol+ 10), breakflag = 0,
      xcol = 30
      IF (curpage=1)
    ...
    ;end select
    SET filenames->cnt = (filenames->cnt+ 1)
    SET stat = alterlist(filenames->qual,filenames->cnt)
    SET filenames->qual[filenames->cnt].name = pat->qual[x].filename
   ENDIF
 ENDFOR
 SELECT INTO "downtimefilenames"
  exp = substring(1,50,filenames->qual[d.seq].name)
  FROM (dummyt d  WITH seq = value(filenames->cnt))
  WITH nocounter
 ;end select
#exit_script
END GO
