CREATE PROGRAM bed_aud_pref_maint:dba
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   nvpcnt = (nvpcnt+ 1), stat = alterlist(nvps->prefs,nvpcnt), nvps->prefs[nvpcnt].nvp_id = nvp
   .name_value_prefs_id,
   nvps->prefs[nvpcnt].parent_entity_id = nvp.parent_entity_id, nvps->prefs[nvpcnt].
   parent_entity_name = nvp.parent_entity_name, nvps->prefs[nvpcnt].pvc_name = nvp.pvc_name,
   nvps->prefs[nvpcnt].pvc_value = nvp.pvc_value, nvps->prefs[nvpcnt].sequence = nvp.sequence
  WITH nocounter
 ;end select
 IF (nvpcnt=0)
  GO TO exit_script
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   tcnt = (tcnt+ 1), stat = alterlist(temp->pref_list,tcnt), temp->pref_list[tcnt].nvp_id = nvps->
   prefs[d.seq].nvp_id,
   temp->pref_list[tcnt].pref_id = p.app_prefs_id, temp->pref_list[tcnt].pvc_value = nvps->prefs[d
   .seq].pvc_value, temp->pref_list[tcnt].application_number = p.application_number,
   temp->pref_list[tcnt].position_cd = p.position_cd, temp->pref_list[tcnt].person_id = p.prsnl_id,
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   tcnt = (tcnt+ 1), stat = alterlist(temp->pref_list,tcnt), temp->pref_list[tcnt].nvp_id = nvps->
   prefs[d.seq].nvp_id,
   temp->pref_list[tcnt].pref_id = p.view_prefs_id, temp->pref_list[tcnt].pvc_value = nvps->prefs[d
   .seq].pvc_value, temp->pref_list[tcnt].application_number = p.application_number,
   temp->pref_list[tcnt].position_cd = p.position_cd, temp->pref_list[tcnt].person_id = p.prsnl_id,
   temp->pref_list[tcnt].pvc_name = nvps->prefs[d.seq].pvc_name,
   temp->pref_list[tcnt].level = build2(trim(p.view_name)," | ",trim(nvp.pvc_value)), parent_string
    = p.frame_type
   IF (trim(nvp2.pvc_value) > " ")
    parent_string = build2(trim(p.frame_type)," | ",trim(nvp2.pvc_value))
   ENDIF
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   tcnt = (tcnt+ 1), stat = alterlist(temp->pref_list,tcnt), temp->pref_list[tcnt].nvp_id = nvps->
   prefs[d.seq].nvp_id,
   temp->pref_list[tcnt].pref_id = p.view_comp_prefs_id, temp->pref_list[tcnt].pvc_value = nvps->
   prefs[d.seq].pvc_value, temp->pref_list[tcnt].application_number = p.application_number,
   temp->pref_list[tcnt].position_cd = p.position_cd, temp->pref_list[tcnt].person_id = p.prsnl_id,
   temp->pref_list[tcnt].pvc_name = nvps->prefs[d.seq].pvc_name,
   temp->pref_list[tcnt].level = p.comp_name, temp->pref_list[tcnt].parent = build2(trim(p.view_name),
    " | ",trim(nvp.pvc_value))
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  ORDER BY d.seq, p.position_cd, p.prsnl_id
  HEAD d.seq
   tcnt = (tcnt+ 1), stat = alterlist(temp->pref_list,tcnt), temp->pref_list[tcnt].nvp_id = nvps->
   prefs[d.seq].nvp_id,
   temp->pref_list[tcnt].pref_id = p.detail_prefs_id, temp->pref_list[tcnt].pvc_name = nvps->prefs[d
   .seq].pvc_name, temp->pref_list[tcnt].pvc_value = nvps->prefs[d.seq].pvc_value,
   temp->pref_list[tcnt].application_number = p.application_number
   IF (prsnl.person_id > 0)
    temp->pref_list[tcnt].position_cd = prsnl.position_cd
   ELSE
   ...
   pref_list[tcnt].parent = p.view_name,
   temp->pref_list[tcnt].search_detail_pref = 1
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (trim(nvp.pvc_value) > " ")
    temp->pref_list[d.seq].parent = build2(trim(prefs.view_name)," | ",trim(nvp.pvc_value))
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   temp->pref_list[d.seq].position_name = position.display, temp->pref_list[d.seq].position_active =
   position.active_ind, temp->pref_list[d.seq].person_name = person.name_full_formatted,
 ...
 SELECT INTO "nl:"
  ...
  HEAD d.seq
   ...
   IF ((temp->pref_list[d.seq].level=""))
    temp->pref_list[d.seq].level = b.br_value
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  HEAD d.seq
   ...
   IF ((temp->pref_list[d.seq].level=""))
    temp->pref_list[d.seq].level = a.description
   ENDIF
 ...
 SET rowcnt = 0
 SELECT INTO "nl:"
  an = substring(0,32,temp->pref_list[d.seq].application_name), posn = substring(0,32,temp->
   pref_list[d.seq].position_name), pern = substring(0,32,temp->pref_list[d.seq].person_name),
  value = substring(0,128,temp->pref_list[d.seq].person_name), level = substring(0,64,temp->
   pref_list[d.seq].person_name), parent = substring(0,64,temp->pref_list[d.seq].person_name)
  FROM (dummyt d  WITH seq = tcnt)
  ORDER BY an, posn, pern,
   value, level, parent
  DETAIL
   IF ((((temp->pref_list[d.seq].position_cd IN (0, null))) OR ((temp->pref_list[d.seq].
   position_active=1))) )
    IF ((((temp->pref_list[d.seq].person_id IN (0, null))) OR ((temp->pref_list[d.seq].person_active=
    1))) )
     rowcnt = (rowcnt+ 1), stat = alterlist(reply->rowlist,rowcnt), stat = alterlist(reply->rowlist[
      rowcnt].celllist,7),
     reply->rowlist[rowcnt].celllist[1].string_value = temp->pref_list[d.seq].application_name, reply
     ->rowlist[rowcnt].celllist[2].string_value = temp->pref_list[d.seq].position_name, reply->
     rowlist[rowcnt].celllist[3].string_value = temp->pref_list[d.seq].person_name,
     reply->rowlist[rowcnt].celllist[4].string_value = temp->pref_list[d.seq].pvc_name, reply->
     rowlist[rowcnt].celllist[5].string_value = temp->pref_list[d.seq].pvc_value, reply->rowlist[
     rowcnt].celllist[6].string_value = temp->pref_list[d.seq].level,
     reply->rowlist[rowcnt].celllist[7].string_value = temp->pref_list[d.seq].parent
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
