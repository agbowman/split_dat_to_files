CREATE PROGRAM dab_pt_transfer:dba
 PROMPT
 ...
 RECORD t_rec(
   ...
   1 event[*]
     ...
     2 clindoc_result_value = gvc
     2 clindoc_result_size = i4
     2 compressed_blob = vc
     2 compressed_blob_ind = i2
 )
 ...
 DECLARE matt_device = vc WITH protect
 DECLARE mattressdevicechg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,"MATTRESSDEVICECHG")),
 protect
 DECLARE completed_var = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6004,"COMPLETED")), protect
 DECLARE transfusegranulocytes = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSEGRANULOCYTES")), protect
 DECLARE transfuseplatelets = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,"TRANSFUSEPLATELETS")
  ), protect
 DECLARE transfusecryoprecipitate = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSECRYOPRECIPITATE")), protect
 DECLARE transfuserbcsforsurgery = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSERBCSFORSURGERY")), protect
 DECLARE transfuserbcs = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,"TRANSFUSERBCS")), protect
 DECLARE transfuseplateletsneonate = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSEPLATELETSNEONATE")), protect
 DECLARE transfuseffpneonate = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSEFFPNEONATE")), protect
 DECLARE transfuserbcsneonate = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSERBCSNEONATE")), protect
 DECLARE transfuserbcs1 = f8 WITH constant(uar_get_code_by("DESCRIPTION",200,"Transfuse RBCs")),
 protect
 DECLARE transfuserbcsprn = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,"TRANSFUSERBCSPRN")),
 protect
 DECLARE transfuserbcsautologous = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSERBCSAUTOLOGOUS")), protect
 DECLARE transfusecryoprecipitateneonate = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSECRYOPRECIPITATENEONATE")), protect
 DECLARE transfusegranulocytesneonate = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSEGRANULOCYTESNEONATE")), protect
 DECLARE transfuserbcspedi3059kg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSERBCSPEDI3059KG")), protect
 DECLARE transfuserbcsautologouspedi = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSERBCSAUTOLOGOUSPEDI")), protect
 DECLARE transfuserbcsautologouspedi30kg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSERBCSAUTOLOGOUSPEDI30KG")), protect
 DECLARE transfusecryoprecipitatepedi3059kg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSECRYOPRECIPITATEPEDI3059KG")), protect
 DECLARE transfusecryoprecipitatepedi30kg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSECRYOPRECIPITATEPEDI30KG")), protect
 DECLARE transfuseffppedi3059kg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSEFFPPEDI3059KG")), protect
 DECLARE transfuseplateletspedi30kg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSEPLATELETSPEDI30KG")), protect
 DECLARE transfuserbcspedi30kg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSERBCSPEDI30KG")), protect
 DECLARE transfuseplateletspedi3059kg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSEPLATELETSPEDI3059KG")), protect
 DECLARE transfuserbcsautologouspedi3059kg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSERBCSAUTOLOGOUSPEDI3059KG")), protect
 DECLARE transfuserbcspedi = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,"TRANSFUSERBCSPEDI")),
 protect
 DECLARE transfuseffppedi30kg = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSEFFPPEDI30KG")), protect
 DECLARE transfuseffppedi = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,"TRANSFUSEFFPPEDI")),
 protect
 DECLARE transfuseplateletspedi = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSEPLATELETSPEDI")), protect
 DECLARE transfusecryoprecipitatepedi = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "TRANSFUSECRYOPRECIPITATEPEDI")), protect
 ...
 DECLARE woundsuperficialculturewgramsmear = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "WOUNDSUPERFICIALCULTUREWGRAMSMEAR")), protect
 DECLARE woundassessment = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,"WOUNDASSESSMENT")),
 protect
 DECLARE wounddeepculturewgramsmear = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,
   "WOUNDDEEPCULTUREWGRAMSMEAR")), protect
 DECLARE woundcare = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,"WOUNDCARE")), protect
 DECLARE woundvac = f8 WITH constant(uar_get_code_by("DISPLAYKEY",200,"WOUNDVAC")), protect
 DECLARE bradenassessment = f8 WITH constant(uar_get_code_by("DISPLAYKEY",93,"BRADENASSESSMENT")),
 protect
 DECLARE woundassessments = f8 WITH constant(uar_get_code_by("DISPLAYKEY",93,"WOUNDASSESSMENTS")),
 protect
 DECLARE pressureulcerassessments = f8 WITH constant(uar_get_code_by("DISPLAYKEY",93,
   "PRESSUREULCERASSESSMENTS")), protect
 DECLARE primaryeventid = f8 WITH constant(uar_get_code_by("DISPLAYKEY",18189,"PRIMARYEVENTID")),
 protect
 DECLARE laboratory_hdr = f8 WITH constant(uar_get_code_by("DISPLAYKEY",93,"LABORATORY")), protect
 DECLARE search_hold2 = vc WITH protect
 DECLARE search_hold = vc WITH protect
 DECLARE cnt_found = i4 WITH protect
 DECLARE search_string = vc WITH protect
 SET modify maxvarlen 5000000
 DECLARE bblobnortf = vc WITH protect
 DECLARE bbsize = i4 WITH protect
 DECLARE remove_sec = vc WITH protect
 DECLARE find_pict = i4 WITH protect
 DECLARE keep_pnote_rec_size = vc WITH protect
 ...
 DECLARE encounterpathway = f8 WITH constant(uar_get_code_by("MEANING",14409,"EP")), protect
 DECLARE band_name = vc WITH protect
 DECLARE datetimeoflastdressingchangewnd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,
   "DATETIMEOFLASTDRESSINGCHANGEWND")), protect
 DECLARE mn_cardio_ind = i2 WITH protect, noconstant(0)
 ...
 FREE RECORD doc_rx_meds_disp
 RECORD doc_rx_meds_disp(
   1 doc_rx_meds_disp[*]
     2 order_id = f8
     2 orig_order_dt_tm = dq8
     2 mnemonic = vc
 ...
 DECLARE os_inproc_cd = f8
 SET stat = uar_get_meaning_by_codeset(6004,"INPROCESS",1,os_inproc_cd)
 ...
 DECLARE sacrament_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "SPIRITUALSACRAMENTALRESOURCES"))
 DECLARE rn_rn_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",106,"RNTORN"))
 DECLARE md_rn1_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",106,"MDTORNTXPROCEDURES"))
 DECLARE md_rn2_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",106,"MDTORNCONSULTS"))
 DECLARE nursing_care_plan_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "NURSINGCAREPLANINITIATEDUPDATED"))
 DECLARE ulceri_nurse_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PRESSUREULCERIWOUNDNURSECONSULT"))
 DECLARE ulcerii_nurse_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PRESSUREULCERIIWOUNDNURSECONSULT"))
 DECLARE ulceriii_nurse_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PRESSUREULCERIIIWOUNDNURSECONSULT"))
 DECLARE ulceriv_nurse_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PRESSUREULCERIVWOUNDNURSECONSULT"))
 DECLARE ulcerv_nurse_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PRESSUREULCERVWOUNDNURSECONSULT"))
 DECLARE ulceri_md_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PRESSUREULCERIMDNOTIFIED"))
 DECLARE ulcerii_md_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PRESSUREULCERIIMDNOTIFIED"))
 DECLARE ulceriii_md_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PRESSUREULCERIIIMDNOTIFIED"))
 DECLARE ulceriv_md_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PRESSUREULCERIVMDNOTIFIED"))
 DECLARE ulcerv_md_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "PRESSUREULCERVMDNOTIFIED"))
 DECLARE woundi_nurse_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "WOUNDIWOUNDCARENURSECONSULT"))
 DECLARE woundii_nurse_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "WOUNDIIWOUNDCARENURSECONSULT"))
 DECLARE woundiii_nurse_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "WOUNDIIIWOUNDCARENURSECONSULT"))
 DECLARE woundiv_nurse_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "WOUNDIVWOUNDCARENURSECONSULT"))
 DECLARE woundv_nurse_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "WOUNDVWOUNDCARENURSECONSULT"))
 DECLARE woundi_md_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,"WOUNDIMDNOTIFIED"))
 DECLARE woundii_md_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,"WOUNDIIMDNOTIFIED"
   ))
 DECLARE woundiii_md_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,
   "WOUNDIIIMDNOTIFIED"))
 DECLARE woundiv_md_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,"WOUNDIVMDNOTIFIED"
   ))
 DECLARE woundv_md_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",72,"WOUNDVMDNOTIFIED"))
 SET code_value = 0.0
 SET code_set = 0.0
 ...
 DECLARE page_string = vc
 DECLARE discharge_found = i2
 IF (test_ind=1)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (e
   ...
   WHERE (epr1.encntr_id= Outerjoin(e.encntr_id))
    ...
    AND (epr1.end_effective_dt_tm> Outerjoin(cnvtdatetime(sysdate))) )
   JOIN (pr1
  ...
  DETAIL
   ...
   pt_transfer->dob = format(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p.birth_tz),1),"mm/dd/yy;;d"),
   pt_transfer->mrn = trim(ea.alias), pt_transfer->fin = trim(ea1.alias),
   pt_transfer->admit_reason = trim(e.reason_for_visit), pt_transfer->reg_date = format(e.reg_dt_tm,
    "mm/dd/yy hh:mm;;d"), pt_transfer->disch_date = e.disch_dt_tm,
   pt_transfer->facility = trim(uar_get_code_description(e.loc_facility_cd)), pt_transfer->attending
    = substring(1,20,pr1.name_full_formatted), pt_transfer->pcp = substring(1,20,pr
    .name_full_formatted),
   pt_transfer->facility = trim(uar_get_code_display(e.loc_facility_cd))
   IF (ph.phone_id > 0
   ...
   ELSEIF ((pt_transfer->facility IN ("MLH", "BMLH")))
    ...
    ENDIF
   ELSEIF ((pt_transfer->facility IN ("BWH", "BNH")))
    IF (phloc.parent_entity_id > 0)
     pt_transfer->facility_phone = cnvtphone(phloc.phone_num,phloc.phone_format_cd,2)
    ELSE
     pt_transfer->facility_phone = ""
    ENDIF
   ENDIF
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ce
   WHERE (ce.person_id=pt_transfer->person_id)
    AND (ce.encntr_id=pt_transfer->encntr_id)
    AND ce.event_cd IN (mf_bloodpressurevenipuncture, mf_wearing_bpv_band)
    AND ce.valid_until_dt_tm > sysdate
    AND ce.result_status_cd=authverified)
  ORDER BY ce.event_cd, ce.updt_dt_tm DESC
  HEAD ce.event_cd
   IF (ce.event_cd=mf_bloodpressurevenipuncture)
    pt_transfer->ms_bloodpressurevenipuncture = ce.result_val
   ELSEIF (ce.event_cd=mf_wearing_bpv_band)
    pt_transfer->ms_wearing_bpv_band = ce.result_val
   ENDIF
  WITH nocounter
 ;end select
 IF ((pt_transfer->ms_bloodpressurevenipuncture=null))
  SET pt_transfer->ms_bloodpressurevenipuncture = "No result found"
 ENDIF
 IF (test_ind=1)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ce
   ...
   ht_cd, wt_cd)))
    ...
    AND ce.view_level=1
    AND ce.result_status_cd != inerror_cd)
  ORDER BY sort_item, ce.event_cd, ce.event_end_dt_tm DESC
  HEAD REPORT
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY ce.order_id, event_name, ce.event_end_dt_tm DESC,
   ce.event_id
  HEAD REPORT
  ...
  HEAD ce.event_id
   IF (o.activity_type_cd IN (micro_cd))
    ...
    IF (ceb.event_id IN (null, 0))
     IF (lb.long_blob_id > 0)
      pt_transfer->micro_results[micro_cnt].event_id = cen.event_id, pt_transfer->micro_results[
      micro_cnt].blob_contents = lb.long_blob, pt_transfer->micro_results[micro_cnt].comp_cd = cen
      .compression_cd,
      pt_transfer->micro_results[micro_cnt].blobseq = 0, pt_transfer->micro_results[micro_cnt].
      comment_comp_cd = 0, pt_transfer->micro_results[micro_cnt].result_comments = trim("")
     ENDIF
    ENDIF
   ELSEIF (o.activity_type_cd IN (gen_lab_cd)
    AND ((ce.event_end_dt_tm >= cnvtdatetime(datetimeadd(pt_transfer->disch_date,- (1.0)))) OR (ce
   .event_end_dt_tm >= cnvtdatetime((curdate - 1),curtime)))
    AND ce.result_val > " ")
    lab_cnt += 1
    IF (mod(lab_cnt,10)=1)
 ...
 SELECT INTO "nl:"
  event_name = cnvtupper(uar_get_code_display(ce.event_cd)), clinsig_updt_dt_tm = format(ce
   .clinsig_updt_dt_tm,"mm/dd/yyyy hh:mm;;D"), collect_dt_tm = format(ce.event_end_dt_tm,
   "mm/dd/yyyy hh:mm;;d"),
  result_status = uar_get_code_display(ce.result_status_cd), ese_event_set_disp =
  uar_get_code_display(ese.event_set_cd), ves1_parent_event_set_disp = uar_get_code_display(ves1
   .parent_event_set_cd)
  FROM orders o,
   clinical_event ce,
   ce_event_note cen,
  ...
  PLAN (o
   ...
   WHERE ce.order_id=o.order_id
    ...
    AND ce.publish_flag=1
    AND ce.valid_until_dt_tm > cnvtdatetime(curdate,curtime)
    AND ce.event_end_dt_tm >= cnvtdatetime(datetimeadd(pt_transfer->disch_date,- (3.0)))
    AND ce.result_status_cd IN (final_cd, modified_cd)
    AND ce.result_val > " ")
   JOIN (ec
   WHERE ec.event_cd=ce.event_cd)
   JOIN (ese
   WHERE ese.event_cd=ec.event_cd
    AND ese.event_set_level=1)
   JOIN (ves
   WHERE ves.event_set_cd=ese.event_set_cd)
   JOIN (ves1
   WHERE ves.parent_event_set_cd=ves1.event_set_cd
    AND ves1.parent_event_set_cd=laboratory_hdr)
   JOIN (cen
   WHERE (cen.event_id= Outerjoin(ce.event_id))
    AND (cen.valid_until_dt_tm> Outerjoin(cnvtdatetime(curdate,curtime))) )
   JOIN (lb
   WHERE (lb.parent_entity_id= Outerjoin(cen.ce_event_note_id)) )
  ORDER BY ese_event_set_disp, ce.event_cd, ce.event_end_dt_tm DESC
  HEAD REPORT
   stat = alterlist(pt_transfer->special_labs,5), spec_cnt = 0
  HEAD ese_event_set_disp
   spec_cnt += 1
   IF (mod(spec_cnt,5)=1)
   ...
   pt_transfer->special_labs[spec_cnt].order_name = trim(uar_get_code_display(o.catalog_cd)),
   pt_transfer->special_labs[spec_cnt].header2 = ese_event_set_disp,
   res_cnt = 0, stat = alterlist(pt_transfer->special_labs[spec_cnt].result_qual,10)
  HEAD ce.event_cd
   IF ( NOT (trim(uar_get_code_display(ce.event_cd)) IN ("NAFGFR", "GFREST")))
    ...
    ENDIF
    pt_transfer->special_labs[spec_cnt].result_qual[res_cnt].header = ves1_parent_event_set_disp,
    ...
    pt_transfer->special_labs[spec_cnt].result_qual[res_cnt].result_val = trim(ce.result_val,3),
    ...
    trim(uar_get_code_display(ce.normalcy_cd),3),
    pt_transfer->special_labs[spec_cnt].result_qual[res_cnt].comment_comp_cd = cen.compression_cd,
 ...
 DECLARE suspend_ord = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6003,"SUSPEND")), protect
 DECLARE cancel_ord = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6003,"CANCEL")), protect
 DECLARE complete_ord = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6003,"COMPLETE")), protect
 DECLARE discontinue_ord = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6003,"DISCONTINUE")),
 protect
 DECLARE modify_ord = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6003,"MODIFY")), protect
 DECLARE order_ord = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6003,"ORDER")), protect
 DECLARE canceldc_ord = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6003,"CANCELDC")), protect
 DECLARE resume_ord = f8 WITH constant(uar_get_code_by("DISPLAYKEY",6003,"RESUME")), protect
 SELECT INTO "nl:"
  FROM orders o,
   order_action oa
  PLAN (o
   WHERE (o.person_id=pt_transfer->person_id)
    AND o.catalog_cd IN (woundassessment, woundcare, wounddeepculturewgramsmear, woundvac,
   woundsuperficialculturewgramsmear)
    AND o.template_order_id=0)
   JOIN (oa
   WHERE oa.order_id=o.order_id
    AND oa.action_type_cd IN (suspend_ord, cancel_ord, complete_ord, discontinue_ord, modify_ord,
   order_ord, canceldc_ord, resume_ord))
  ORDER BY o.person_id, o.order_id, oa.action_dt_tm DESC
  HEAD REPORT
   stat = alterlist(wounds->wound_ord,10), cnt_wnd = 0
  HEAD o.person_id
   IF ((((pt_transfer->disch_date=null)) OR ((pt_transfer->disch_date=0))) )
    wounds->pat_status = "Orders"
   ELSE
    wounds->pat_status = "Orders On Discharge"
   ENDIF
   CALL echo(build("o.ordered_as_mnemonic = ",o.hna_order_mnemonic))
  HEAD o.order_id
   CALL echo(build("o.ordered_as_mnemonic = ",o.ordered_as_mnemonic))
   IF (trim(wounds->pat_status,3)=trim("Orders",3)
    AND oa.action_type_cd IN (modify_ord, order_ord, resume_ord))
    cnt_wnd += 1
    IF (mod(cnt_wnd,10)=1
     AND cnt_wnd > 1)
     stat = alterlist(wounds->wound_ord,(cnt_wnd+ 9))
    ENDIF
    wounds->wound_ord[cnt_wnd].name = o.hna_order_mnemonic
   ELSEIF ((oa.action_dt_tm < pt_transfer->disch_date))
    cnt_wnd += 1
    IF (mod(cnt_wnd,10)=1
     AND cnt_wnd > 1)
     stat = alterlist(wounds->wound_ord,(cnt_wnd+ 9))
    ...
    wounds->wound_ord[cnt_wnd].name = o.hna_order_mnemonic
   ENDIF
   CALL echo(build("wound count = ",cnt_wnd))
  FOOT  o.person_id
   stat = alterlist(wounds->wound_ord,cnt_wnd), cnt_wnd = 0
  FOOT REPORT
   null
  WITH nocounter
 ;end select
 CALL echorecord(wounds)
 CALL echo("Get mattress device")
 SET matt_device = "XXXX"
 SELECT INTO "nl:"
  FROM clinical_event ce
  PLAN (ce
   WHERE (ce.encntr_id=pt_transfer->encntr_id)
    AND ce.event_cd=mattressdevicechg)
  ORDER BY ce.event_end_dt_tm DESC
  HEAD ce.event_end_dt_tm
   matt_device = trim(ce.result_val,3)
  WITH nocounter
 ;end select
 IF (test_ind=1)
   ...
   DETAIL
    IF (actioncnt=1)
     admincnt += 1, stat = alterlist(scheduled_orders_disp->scheduled_orders[schedordercnt].admins,
      admincnt), scheduled_orders_disp->scheduled_orders[schedordercnt].admins[admincnt].order_id = o
     .order_id,
 ...
 IF (prn_med_cnt > 0)
  SELECT DISTINCT INTO "nl:"
   ...
   HEAD REPORT
    ...
    stat = alterlist(prn_orders_disp->prn_orders,10)
   HEAD o.order_id
    ordercnt += 1,
    CALL echo(build("ORDERCNT&&&& = ",ordercnt))
   ...
   HEAD oa.action_sequence
    actioncnt += 1,
    CALL echo(build("ACTIONCNT&&&& = ",actioncnt))
 ...
 IF (cont_med_cnt > 0)
  SELECT DISTINCT INTO "nl:"
   ...
   HEAD REPORT
    ordercnt = 0, actioncnt = 0, admincnt = 0,
    stat = alterlist(continuous_orders_disp->continuous_orders,10)
   HEAD o.order_id
    ordercnt += 1
    IF (mod(ordercnt,10)=1)
    ...
    ENDIF
    stat = alterlist(continuous_orders_disp->continuous_orders[ordercnt].core_actions,5)
   HEAD oa.action_sequence
    actioncnt += 1
    IF (mod(actioncnt,5)=1)
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    WHILE (b_cc)
      ...
      IF (b_e)
       scheduled_orders->qual[xx].comm_cnt += 1, tmp_var = scheduled_orders->qual[xx].comm_cnt, stat
        = alterlist(scheduled_orders->qual[xx].comment,tmp_var),
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ce
   ...
   "Cardiovascular", "Cardiovascular (new)", "CARDIOVASCULAR TEST", "History and Physical - Dictated",
   "RADIOLOGY",
   "Consultation Note - Dictated", "History and Physical Hospital",
   "Discharge/Transfer Note Hospital", "History and Physical Hospital HIM",
  ...
  HEAD ce.parent_event_id
   IF (vesc.event_set_name="Nursing Discharge Status Report")
    IF (first_nurse_blob_ind1=0)
     first_nurse_blob_ind1 = 1, cnt_blob += 1
     IF (mod(cnt_blob,5)=1)
  ...
  HEAD ce.event_id
   IF (vesc.event_set_name="Nursing Discharge Status Report")
    IF (first_nurse_blob_ind2=0)
     first_nurse_blob_ind2 = 1, cnt_blobs += 1
   ...
   "History and Physical Hospital") OR (((vesc.event_set_name="Discharge Summary - Dictated") OR (((
   vesc.event_set_name="Discharge/Transfer Note Hospital HIM") OR (vesc.event_set_name=
   "History and Physical Hospital HIM")) )) )) )) )) )) )) )
    discharge_found = 1
   ENDIF
 ...
 SET stat = alterlist(powernote->qual,3)
 SET powernote->qual[1].display_key = "NURSINGPROGRESSNOTE*"
 SET powernote->qual[1].sortseq = 1
 SET powernote->qual[2].display_key = "WOUNDCAREPROGRESSNOTE*"
 ...
 SELECT INTO "NL:"
  ...
  PLAN (d)
   ...
   WHERE operator(srp.display_key,"like",patstring(powernote->qual[d.seq].display_key,1))
    AND srp.pattern_type_cd=encounterpathway)
   JOIN (ssp
   ...
   WHERE s.scd_story_id=ssp.scd_story_id
    ...
    AND (s.encounter_id=pt_transfer->encntr_id))
   JOIN (ce
   WHERE ce.event_id=s.event_id
    AND ce.valid_until_dt_tm >= cnvtdatetime(sysdate)
    AND ce.result_status_cd IN (altered, modified, authverified))
  ORDER BY sort, ce.event_end_dt_tm DESC
  HEAD REPORT
   cnt = 0, num_nurse_note = 0, nursingprogressnotebhs_cnt = 0,
   woundcareprogressnote_cnt = 0, stat = alterlist(info->powernote,6)
  DETAIL
   IF (trim(srp.display_key) IN (value("NURSINGPROGRESSNOTEBHS*"))
    AND nursingprogressnotebhs_cnt < 3)
    cnt += 1, info->powernote[cnt].title = trim(srp.display,3), info->powernote[cnt].sortseq = 1,
    info->powernote[cnt].event_id = ce.event_id, nursingprogressnotebhs_cnt += 1
   ELSEIF (trim(srp.display_key) IN (value("WOUNDCAREPROGRESSNOTE*"))
    AND woundcareprogressnote_cnt < 3)
    cnt += 1, info->powernote[cnt].title = trim(srp.display,3), info->powernote[cnt].sortseq = 2,
    info->powernote[cnt].event_id = ce.event_id, woundcareprogressnote_cnt += 1
   ENDIF
  FOOT REPORT
   stat = alterlist(info->powernote,(woundcareprogressnote_cnt+ nursingprogressnotebhs_cnt)),
   CALL echo(build("form cnt = ",cnt)),
   CALL echo(build("WOUNDCAREPROGRESSNOTE_cnt = ",woundcareprogressnote_cnt)),
   cnt = 0,
   CALL echo("echo 2")
  WITH nocounter
 ;end select
 CALL echo("Section to handle very large blobs")
 ...
 DECLARE blobout_big = vc WITH public
 DECLARE blob_in = vc WITH public
 DECLARE blob_in_size = i4 WITH public
 ...
 FOR (x = 1 TO pt_transfer->blob_cnt)
  FOR (y = 1 TO pt_transfer->blob_qual[x].blobs_cnt)
    ...
    EXECUTE bhs_multi_record_blob value(pt_transfer->blob_qual[x].blobs_qual[y].event_id)
    SET stat = uar_ocf_uncompress(good_blob,size(good_blob),blobout_big,size(blobout_big),
     blob_return_len)
    SET search_string = blobout_big
    SET find_pict1 = findstring("{\pict",search_string,1,0)
    SET cnt_found = 0
    WHILE (findstring("{\pict",search_string,1,0) > 0
     AND cnt_found < 100)
      SET cnt_found += 1
      SET search_hold1 = substring(1,(findstring("{\pict",search_string,1,0) - 1),search_string)
      SET search_hold2 = substring(findstring("{\pict",search_string,1,0),size(search_string),
       search_string)
      SET search_hold2 = substring((findstring("}",search_hold2,1,0)+ 1),size(search_hold2),
       search_hold2)
      SET search_string = concat(trim(search_hold1,3)," <<Unable to Print Picture>> ",trim(
        search_hold2,3))
    ENDWHILE
    SET blobout_big = search_string
    SET stat = uar_rtf2(blobout_big,size(blobout_big),blob_out2,size(blob_out2),bsize,
     bflag)
  ...
  SET good_blob = ""
 ENDFOR
 CALL echo("Large blob2")
 FOR (xnote = 1 TO size(info->powernote,5))
   SET pt->line_cnt = 0
   SET blob_out = fillstring(32000,"")
   SET blob_out2 = fillstring(32000,"")
   SET good_blob = " "
   SET blobout_big = " "
   EXECUTE bhs_multi_record_blob value(info->powernote[xnote].event_id)
   SET stat = uar_ocf_uncompress(good_blob,size(good_blob),blobout_big,size(blobout_big),
    blob_return_len)
   SET search_string = blobout_big
   SET find_pict1 = findstring("{\pict",search_string,1,0)
   ...
   SET stat = uar_rtf2(blobout_big,size(blobout_big),blob_out2,size(blob_out2),bsize,
    bflag)
   EXECUTE dcp_parse_text value(blob_out2), value(max_length)
   SET stat = alterlist(info->powernote[xnote].line,pt->line_cnt)
   FOR (ynote = 1 TO pt->line_cnt)
     SET info->powernote[xnote].line[ynote].disp = trim(pt->lns[ynote].line)
   ENDFOR
   SET good_blob = ""
 ENDFOR
 IF (test_ind=1)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   WHERE (o.encntr_id=pt_transfer->encntr_id)
    AND ((o.order_status_cd+ 0) IN (os_ord_cd, os_inproc_cd))
    AND o.template_order_flag IN (0, 1)
    AND o.orig_ord_as_flag=0
    AND ((((((o.catalog_cd+ 0) IN (full_code_cd, limited_code_cd, no_code_cd, fullop_code_cd,
   limitedop_code_cd,
   ...
   noop_code_cd, op_code_cd, isolation_cd)))
    AND o.order_status_cd != os_canc_cd)) )) ))
    AND ((((o.activity_type_cd+ 0) != md_rn1_cd)) OR (((((o.activity_type_cd+ 0) != rn_rn_cd)) OR (((
   o.activity_type_cd+ 0) != md_rn2_cd))) ))
 ...
 ENDIF
 SELECT INTO "nl:"
  status_date = cnvtdatetime(pt_transfer->consult_qual[d1.seq].status_date)
  FROM (dummyt d1  WITH seq = value(size(pt_transfer->consult_qual,5)))
 ...
 SELECT
  ese_event_set_disp = uar_get_code_display(ese.event_set_cd), ese_event_disp = uar_get_code_display(
   ese.event_cd), result_units = uar_get_code_display(ce.result_units_cd)
  FROM v500_event_code ec,
   v500_event_set_explode ese,
   v500_event_set_canon esc,
   clinical_event ce
  PLAN (esc
   WHERE esc.event_set_cd IN (woundassessments, pressureulcerassessments))
   JOIN (ese
   ...
   JOIN (ce
   WHERE (ce.encntr_id=pt_transfer->encntr_id)
    AND ce.event_cd=ec.event_cd)
  ORDER BY esc.event_set_collating_seq, ese_event_disp, ce.event_cd,
   ce.event_end_dt_tm DESC
  HEAD REPORT
   cnt_wound = 0, stat = alterlist(wound_band->wound_section,10)
  HEAD esc.event_set_collating_seq
   cnt_wound += 1
   IF (mod(cnt_wound,10)=1)
    stat = alterlist(wound_band->wound_section,(cnt_wound+ 9))
   ENDIF
   wound_band->wound_section[cnt_wound].section_name = trim(ese_event_set_disp,3), cnt_wndres = 0,
   stat = alterlist(wound_band->wound_section[cnt_wound].result,10)
  HEAD ce.event_cd
   cnt_wndres += 1
   IF (mod(cnt_wndres,10)=1)
    stat = alterlist(wound_band->wound_section[cnt_wound].result,(cnt_wndres+ 9))
   ENDIF
   IF (ce.event_cd != datetimeoflastdressingchangewnd)
    wound_band->wound_section[cnt_wound].result[cnt_wndres].wound_result = concat(trim(ese_event_disp,
      3),": ",trim(ce.result_val,3),trim(result_units,3)," ",
     trim(format(ce.event_end_dt_tm,"@SHORTDATETIME"),3))
   ELSE
    dress_date = format(cnvtdatetime(cnvtdate2(substring(3,8,ce.result_val),"yyyymmdd"),cnvttime2(
       substring(11,6,ce.result_val),"hhmmss")),"dd-mmm-yyyy hh:mm:ss;;d"), wound_band->
    wound_section[cnt_wound].result[cnt_wndres].wound_result = concat(trim(ese_event_disp,3),": ",
     trim(dress_date,3),trim(result_units,3)," ",
     trim(format(ce.event_end_dt_tm,"@SHORTDATETIME"),3))
   ENDIF
  FOOT  esc.event_set_collating_seq
   stat = alterlist(wound_band->wound_section[cnt_wound].result,cnt_wndres), cnt_wndres = 0
  FOOT REPORT
   stat = alterlist(wound_band->wound_section,cnt_wound), cnt_wound = 0
  WITH nocounter, time = 60
 ;end select
 CALL echorecord(wound_band)
 IF (test_ind=1)
  CALL echo("Get form dta..")
 ENDIF
 DECLARE number_of_forms = i2
 SET number_of_forms = 36
 ...
 SET form_data->qual[1].max_cnt = 100
 ...
 SET form_data->qual[17].dcp_forms_ref_id = 2079431.00
 SET form_data->qual[17].type = "PULM Eval2"
 SET form_data->qual[17].max_cnt = 1
 SET form_data->qual[17].cur_cnt = 0
 SET form_data->qual[18].dcp_forms_ref_id = 90718331.00
 SET form_data->qual[18].type = "Braden Newborn"
 SET form_data->qual[18].max_cnt = 1
 SET form_data->qual[18].cur_cnt = 0
 SET form_data->qual[19].dcp_forms_ref_id = 90720693.00
 SET form_data->qual[19].type = "Braden Pedi"
 SET form_data->qual[19].max_cnt = 1
 SET form_data->qual[19].cur_cnt = 0
 SET form_data->qual[20].dcp_forms_ref_id = 90854766.00
 SET form_data->qual[20].type = "OT Eval Pedi"
 SET form_data->qual[20].max_cnt = 1
 SET form_data->qual[20].cur_cnt = 0
 SET form_data->qual[21].dcp_forms_ref_id = 90853490.00
 SET form_data->qual[21].type = "OT Feeding Eval Pedi"
 SET form_data->qual[21].max_cnt = 1
 SET form_data->qual[21].cur_cnt = 0
 SET form_data->qual[22].dcp_forms_ref_id = 90850179.00
 SET form_data->qual[22].type = "OT Swallow Pedi"
 SET form_data->qual[22].max_cnt = 1
 SET form_data->qual[22].cur_cnt = 0
 SET form_data->qual[23].dcp_forms_ref_id = 90848762.00
 SET form_data->qual[23].type = "OT Treatment Pedi"
 SET form_data->qual[23].max_cnt = 2
 SET form_data->qual[23].cur_cnt = 0
 SET form_data->qual[24].dcp_forms_ref_id = 90848198.00
 SET form_data->qual[24].type = "PT Eval Pedi"
 SET form_data->qual[24].max_cnt = 1
 SET form_data->qual[24].cur_cnt = 0
 SET form_data->qual[25].dcp_forms_ref_id = 90843017.00
 SET form_data->qual[25].type = "PT Treat Pedi"
 SET form_data->qual[25].max_cnt = 2
 SET form_data->qual[25].cur_cnt = 0
 SET form_data->qual[26].dcp_forms_ref_id = 90839162.00
 SET form_data->qual[26].type = "ST Barium Swallow Pedi"
 SET form_data->qual[26].max_cnt = 1
 SET form_data->qual[26].cur_cnt = 0
 SET form_data->qual[27].dcp_forms_ref_id = 90835564.00
 SET form_data->qual[27].type = "ST Speech Pedi"
 SET form_data->qual[27].max_cnt = 1
 SET form_data->qual[27].cur_cnt = 0
 SET form_data->qual[28].dcp_forms_ref_id = 90831090.00
 SET form_data->qual[28].type = "ST Treatment Pedi"
 SET form_data->qual[28].max_cnt = 2
 SET form_data->qual[28].cur_cnt = 0
 SET form_data->qual[29].dcp_forms_ref_id = 90841023.00
 SET form_data->qual[29].type = "ST BS Swallow Pedi"
 SET form_data->qual[29].max_cnt = 1
 SET form_data->qual[29].cur_cnt = 0
 SET form_data->qual[30].dcp_forms_ref_id = 131490413.00
 SET form_data->qual[30].type = "Social Work Consult"
 SET form_data->qual[30].max_cnt = 20
 SET form_data->qual[30].cur_cnt = 0
 SET form_data->qual[31].dcp_forms_ref_id = 131648154.00
 SET form_data->qual[31].type = "Social Work Follow Up Note"
 SET form_data->qual[31].max_cnt = 3
 SET form_data->qual[31].cur_cnt = 0
 SET form_data->qual[32].dcp_forms_ref_id = 257719192.00
 SET form_data->qual[32].type = "PT Eval & Tx Pedi:Ortho/Trauma"
 SET form_data->qual[32].max_cnt = 1
 SET form_data->qual[32].cur_cnt = 0
 SET form_data->qual[33].dcp_forms_ref_id = 274954119.00
 SET form_data->qual[33].type = "OT Eval & Tx:Infant/PreSchool"
 SET form_data->qual[33].max_cnt = 1
 SET form_data->qual[33].cur_cnt = 0
 SET form_data->qual[34].dcp_forms_ref_id = 274860083.00
 SET form_data->qual[34].type = "OT Eval & Tx:Newborn/Infant"
 SET form_data->qual[34].max_cnt = 1
 SET form_data->qual[34].cur_cnt = 0
 SET form_data->qual[35].dcp_forms_ref_id = 275212807.00
 SET form_data->qual[35].type = "OT Feeding Eval & Tx:Newborn/Infant"
 SET form_data->qual[35].max_cnt = 1
 SET form_data->qual[35].cur_cnt = 0
 SET form_data->qual[36].dcp_forms_ref_id = 762731
 SET form_data->qual[36].type = "Pul Rehab Nurse Eval Treat"
 SET form_data->qual[36].max_cnt = 1
 SET form_data->qual[36].cur_cnt = 0
 CALL echo("Get From Data")
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE dcpc.dcp_forms_activity_id=dcp.dcp_forms_activity_id
    ...
    AND ((dcpc.component_cd+ 0)=primaryeventid))
  ...
  DETAIL
   IF ((form_data->qual[d.seq].cur_cnt < form_data->qual[d.seq].max_cnt))
    ...
    form_qual[cnt_f].form_event_id = dcpc.parent_entity_id, form_results->form_qual[cnt_f].comp_cd =
    dcpc.component_cd
   ENDIF
   form_data->qual[d.seq].cur_cnt += 1
  FOOT REPORT
 ...
 CALL echo("Get From Data")
 CALL echo("Find Forms")
 SELECT INTO "nl:"
  sort_form =
  IF ((form_results->form_qual[d.seq].form_type IN ("*Social Work Follow Up Note*"))) 1
  ELSEIF ((form_results->form_qual[d.seq].form_type IN ("*Social Work Follow Up Note*"))) 2
  ELSEIF ((form_results->form_qual[d.seq].form_type="Nutrition Eval1")) 3
  ELSEIF ((form_results->form_qual[d.seq].form_type="Nutrition Eval2")) 4
  ELSEIF ((form_results->form_qual[d.seq].form_type IN ("PT Eval1", "PT Eval2", "PT Eval Pedi"))) 5
  ELSEIF ((form_results->form_qual[d.seq].form_type="PT Eval3")) 6
  ELSEIF ((form_results->form_qual[d.seq].form_type IN ("PT Treat1", "PT Treat Pedi"))) 7
  ELSEIF ((form_results->form_qual[d.seq].form_type="PT Eval & Tx Pedi:Ortho/Trauma")) 8
  ELSEIF ((form_results->form_qual[d.seq].form_type IN ("OT Eval1", "OT Eval Pedi",
  "OT Feeding Eval Pedi", "OT Swallow Pedi", "OT Eval & Tx:Infant/PreSchool",
  "OT Eval & Tx:Newborn/Infant", "OT Feeding Eval & Tx:Newborn/Infant"))) 9
  ELSEIF ((form_results->form_qual[d.seq].form_type IN ("OT Treat1", "OT Treatment Pedi"))) 10
  ELSEIF ((form_results->form_qual[d.seq].form_type IN ("ST Eval1", "ST Eval2", "ST Eval3",
  "ST Eval4", "ST Barium Swallow Pedi",
  "ST Speech Pedi", "ST BS Swallow Pedi"))) 11
  ELSEIF ((form_results->form_qual[d.seq].form_type="ST Treat1")) 12
  ELSEIF ((form_results->form_qual[d.seq].form_type="RT Eval1")) 13
  ELSEIF ((form_results->form_qual[d.seq].form_type="RT Treat1")) 14
  ELSEIF ((form_results->form_qual[d.seq].form_type IN ("PULM Eval1", "PULM Eval2",
  "Pul Rehab Nurse Eval Treat"))) 15
  ELSEIF ((form_results->form_qual[d.seq].form_type IN ("Braden", "Braden Newborn", "Braden Pedi")))
   16
  ENDIF
  ...
  PLAN (d)
   ...
   WHERE (ce.event_id=form_results->form_qual[d.seq].form_event_id)
    ...
    AND ce.result_status_cd != inerror_cd)
   JOIN (ce1
   WHERE (ce1.parent_event_id= Outerjoin(ce.event_id))
    AND (ce1.valid_until_dt_tm>= Outerjoin(cnvtdatetime(curdate,curtime)))
    AND (ce1.event_cd!= Outerjoin(ce.event_cd))
    AND (ce1.result_status_cd!= Outerjoin(inerror_cd)) )
   JOIN (ce2
   WHERE (ce2.parent_event_id= Outerjoin(ce1.event_id))
    AND (ce2.valid_until_dt_tm>= Outerjoin(cnvtdatetime(curdate,curtime)))
    AND (ce2.event_cd!= Outerjoin(nursing_care_plan_cd))
    AND (ce2.event_cd!= Outerjoin(ulceri_nurse_cd))
    AND (ce2.event_cd!= Outerjoin(ulcerii_nurse_cd))
    AND (ce2.event_cd!= Outerjoin(ulceriii_nurse_cd))
    AND (ce2.event_cd!= Outerjoin(ulceriv_nurse_cd))
    AND (ce2.event_cd!= Outerjoin(ulcerv_nurse_cd))
    AND (ce2.event_cd!= Outerjoin(ulceri_md_cd))
    AND (ce2.event_cd!= Outerjoin(ulcerii_md_cd))
    AND (ce2.event_cd!= Outerjoin(ulceriii_md_cd))
    AND (ce2.event_cd!= Outerjoin(ulceriv_md_cd))
    AND (ce2.event_cd!= Outerjoin(ulcerv_md_cd))
    AND (ce2.event_cd!= Outerjoin(woundi_nurse_cd))
    AND (ce2.event_cd!= Outerjoin(woundii_nurse_cd))
    AND (ce2.event_cd!= Outerjoin(woundiii_nurse_cd))
    AND (ce2.event_cd!= Outerjoin(woundiv_nurse_cd))
    AND (ce2.event_cd!= Outerjoin(woundv_nurse_cd))
    AND (ce2.event_cd!= Outerjoin(woundi_md_cd))
    AND (ce2.event_cd!= Outerjoin(woundii_md_cd))
    AND (ce2.event_cd!= Outerjoin(woundiii_md_cd))
    AND (ce2.event_cd!= Outerjoin(woundiv_md_cd))
    AND (ce2.event_cd!= Outerjoin(woundv_md_cd)) )
   JOIN (cen
 ...
 ;end select
 CALL echo("sacraments")
 SELECT INTO "nl:"
  FROM clinical_event ce
  PLAN (ce
   WHERE (ce.encntr_id=pt_transfer->encntr_id)
    AND ce.event_cd=sacrament_cd)
  DETAIL
   pt_transfer->sacrament = ce.result_val
  WITH nocounter
 ;end select
 CALL echo("sacraments2")
 CALL echo("Get home meds")
 SELECT INTO "nl:"
  FROM orders o,
   encounter e
  PLAN (e
  ...
  HEAD e.encntr_id
   cnt_home_med = 0, stat = alterlist(doc_rx_meds_disp->doc_rx_meds_disp,10)
  DETAIL
   cnt_home_med += 1
   IF (mod(cnt_home_med,10)=1
    AND cnt_home_med > 1)
    stat = alterlist(doc_rx_meds_disp->doc_rx_meds_disp,(cnt_home_med+ 9))
   ENDIF
   IF (o.ordered_as_mnemonic=o.hna_order_mnemonic)
    doc_rx_meds_disp->doc_rx_meds_disp[cnt_home_med].mnemonic = trim(o.hna_order_mnemonic)
   ELSE
    doc_rx_meds_disp->doc_rx_meds_disp[cnt_home_med].mnemonic = concat(trim(o.ordered_as_mnemonic,3),
     " (",trim(o.hna_order_mnemonic,3),")")
   ENDIF
   doc_rx_meds_disp->doc_rx_meds_disp[cnt_home_med].clinical_display_line = o.clinical_display_line,
   doc_rx_meds_disp->doc_rx_meds_disp[cnt_home_med].orig_order_dt_tm = o.orig_order_dt_tm
  FOOT  e.encntr_id
   stat = alterlist(doc_rx_meds_disp->doc_rx_meds_disp,cnt_home_med)
  WITH nocounter
 ;end select
 CALL echorecord(doc_rx_meds_disp)
 CALL echo("Start tranfusion Orders")
 SELECT INTO "nl:"
  FROM orders o,
   encounter e
  PLAN (e
   ...
   WHERE o.person_id=e.person_id
    ...
    AND ((o.order_status_cd+ 0) IN (ordered_var, completed_var))
    AND o.catalog_cd IN (transfusegranulocytes, transfuseplatelets, transfusecryoprecipitate,
   transfuserbcsforsurgery, transfuserbcs,
   transfuseplateletsneonate, transfuseffpneonate, transfuserbcsneonate, transfuserbcs,
   transfuserbcsprn,
   transfuserbcsautologous, transfusecryoprecipitateneonate, transfusegranulocytesneonate,
   transfuserbcspedi3059kg, transfuserbcsautologouspedi,
   transfuserbcsautologouspedi30kg, transfusecryoprecipitatepedi3059kg,
   transfusecryoprecipitatepedi30kg, transfuseffppedi3059kg, transfuseplateletspedi30kg,
   transfuserbcspedi30kg, transfuseplateletspedi3059kg, transfuserbcsautologouspedi3059kg,
   transfuserbcspedi, transfuseffppedi30kg,
   transfuseffppedi, transfuseplateletspedi, transfusecryoprecipitatepedi)
    AND o.template_order_flag <= 1
    AND o.orig_order_dt_tm >= cnvtdatetime(datetimeadd(pt_transfer->disch_date,- (14.0))))
  ORDER BY e.encntr_id, o.order_id
  HEAD e.encntr_id
   cnt_trans = 0, stat = alterlist(transfusion->txfuse_ord,10)
  DETAIL
   cnt_trans += 1
   IF (mod(cnt_trans,10)=1
    AND cnt_trans > 1)
    stat = alterlist(transfusion->txfuse_ord,(cnt_trans+ 9))
   ENDIF
   IF (o.ordered_as_mnemonic=o.hna_order_mnemonic)
    transfusion->txfuse_ord[cnt_trans].mnemonic = trim(o.hna_order_mnemonic)
   ELSE
    transfusion->txfuse_ord[cnt_trans].mnemonic = concat(trim(o.ordered_as_mnemonic,3)," (",trim(o
      .hna_order_mnemonic,3),")")
   ENDIF
   transfusion->txfuse_ord[cnt_trans].clinical_display_line = o.clinical_display_line, transfusion->
   txfuse_ord[cnt_trans].orig_order_dt_tm = o.orig_order_dt_tm
  FOOT  e.encntr_id
   stat = alterlist(transfusion->txfuse_ord,cnt_trans)
  WITH nocounter
 ;end select
 CALL echo("End tranfusion Orders")
 CALL echorecord(transfusion)
 DECLARE total_pages = i4
 CALL echo("Print Dummy report to Memory for total page count")
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   MACRO (print_blob)
    ...
    FOR (y = 1 TO pt_transfer->blob_qual[x].blobs_cnt)
      IF (substring(1,11,pt_transfer->blob_qual[x].blobs_qual[y].event_title_text)="Addendum by")
       ...
       "{f/0}{cpi/14}{b}", pt_transfer->blob_qual[x].blobs_qual[y].event_title_text, ycol += 12,
       row + 1
      ELSE
       ycol += 12, row + 1, xcol = 15,
       row + 1,
       ...
       pt_transfer->blob_qual[x].blob_event_cd, ycol += 12, row + 1,
       xcol = 15, row + 1,
       CALL print(calcpos(xcol,ycol)),
       "Note Date: ", pt_transfer->blob_qual[x].blob_date, ycol += 12,
       row + 1
      ENDIF
      ...
      FOR (z = 1 TO pt_transfer->blob_qual[x].action_cnt)
        IF (textlen(trim(pt_transfer->blob_qual[x].action_qual[z].type,3)) > 0
         ...
         CALL print(calcpos(xcol,ycol)), output_display, ycol += 12,
         row + 1
        ENDIF
      ENDFOR
    ENDFOR
   ENDMACRO
   ,
   CALL echo("Print Dummy report to Memory for total page count2"),
   MACRO (print_blob_comment_report)
    IF (compression_cd=compress_cd)
     ...
     IF (findstring("\rtf",blobout2,1,0) > 0)
      IF (mn_cardio_ind=1)
       CALL uar_rtf2(trim(blobout2),size(blobout2),blobout2,size(blobout2),bsize,1)
      ELSE
    ...
    IF (blob_type="report")
     ...
     IF (trim(new_blob_contents,3) > " ")
      ...
      xcol = 50, row + 1,
      CALL print(calcpos(xcol,ycol)),
      "{f/0}{cpi/14}Report: ", ycol += 12, row + 1
      IF (findstring("|",new_blob_contents,1,0) > 0)
       ...
       WHILE (size(trim(parse_string,3)) > 0)
        ...
        IF (size(trim(parse_string,3)) > 0)
         ...
         CALL print(calcpos(xcol,ycol)), parse_string, ycol += 12,
      ...
      ELSE
       ...
       CALL print(calcpos(xcol,ycol)), output_display1, ycol += 12
      ENDIF
     ENDIF
    ENDIF
   ENDMACRO
  ...
  HEAD PAGE
   ...
   pt_transfer->facility_phone, ycol += 12, row + 1,
   xcol = 30, row + 1,
   CALL print(calcpos(xcol,ycol)),
   line, ycol += 24, row + 1
  DETAIL
   ...
   xcol = 30, row + 1,
   CALL print(calcpos(xcol,ycol)),
   "{b}{u}Patient Demographics:{endb}{endu}", ycol += 12, row + 1,
   xcol = 250, row + 1,
   CALL print(calcpos(xcol,ycol)),
   "{b}Facility:{endb}", xcol = 310, row + 1,
   CALL print(calcpos(xcol,ycol)), pt_transfer->facility, xcol = 440,
   row + 1,
   CALL print(calcpos(xcol,ycol)), "{b}Religion:{endb}",
   xcol = 500, row + 1,
   CALL print(calcpos(xcol,ycol)),
   pt_transfer->religion, ycol += 12, row + 1,
   xcol = 30, row + 1,
   CALL print(calcpos(xcol,ycol)),
   "{b}Attending:{endb}", xcol = 100, row + 1,
   CALL print(calcpos(xcol,ycol)), pt_transfer->attending, xcol = 250,
   row + 1,
   CALL print(calcpos(xcol,ycol)), "{b}PCP:{endb}",
   xcol = 310, row + 1,
   CALL print(calcpos(xcol,ycol)),
   pt_transfer->pcp, ycol += 12, row + 1,
   xcol = 30, row + 1,
   CALL print(calcpos(xcol,ycol)),
   "{b}Admit Reason:{endb}"
   ...
   xcol = 100, row + 1,
   CALL print(calcpos(xcol,ycol)),
   pt_transfer->address_line, ycol += 12, row + 1,
   xcol = 100, row + 1,
   CALL print(calcpos(xcol,ycol)),
   pt_transfer->city_state_zip, ycol += 24, row + 1,
   output_display = ""
   IF (ycol > 725)
    BREAK
   ENDIF
   IF (size(pt_transfer->code_status_qual,5) > 0)
    output_display = concat("{b}{u}Code Status before discharge: ",pt_transfer->code_status_qual[1].
     order_name," ",pt_transfer->code_status_qual[1].order_status," on ",
   ...
   IF (ycol > 725)
    BREAK
   ENDIF
   IF ((pt_transfer->sacrament > " "))
    output_display = concat("{b}{u}Sacraments of the Sick: ",pt_transfer->sacrament,"{endb}{endu}")
    IF (size(trim(output_display,3)) > 120)
     tempstring = trim(output_display), xcol = 30, wrapcol = 120,
     line_wrap
    ELSEIF (size(trim(output_display,3)) > 0)
    ...
    ycol += 12, row + 1
   ELSE
    output_display = concat("{b}{u}No Sacraments of the Sick Found{endb}{endu}"), xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)), output_display, ycol += 12,
    row + 1
   ENDIF
   IF (ycol > 725)
   ...
   IF ((pt_transfer->adv_dir > " "))
    ...
    CALL print(calcpos(xcol,ycol)), pt_transfer->adv_dir, ycol += 12,
    row + 1
   ELSE
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{b}{u}Advance Directives:{endb}{endu}", xcol = 140, row + 1,
    CALL print(calcpos(xcol,ycol)), "No result found", ycol += 12,
    row + 1
   ENDIF
   IF ((pt_transfer->adv_date > " "))
    ...
    CALL print(calcpos(xcol,ycol)),
    "Date:", xcol = 140, row + 1,
    CALL print(calcpos(xcol,ycol)), pt_transfer->adv_date, ycol += 12,
    row + 1
   ENDIF
   IF ((pt_transfer->adv_proxy > " "))
    ...
    CALL print(calcpos(xcol,ycol)), pt_transfer->adv_proxy, ycol += 12,
    row + 1
   ENDIF
   IF ((pt_transfer->adv_phone > " "))
    ...
    CALL print(calcpos(xcol,ycol)), pt_transfer->adv_phone, ycol += 12,
    row + 1
   ENDIF
   IF ((pt_transfer->adv_type > " "))
    ...
    CALL print(calcpos(xcol,ycol)),
    "Type:", xcol = 140, row + 1,
    CALL print(calcpos(xcol,ycol)), pt_transfer->adv_type, ycol += 12,
    row + 1
   ELSE
    xcol = 100, row + 1,
    CALL print(calcpos(xcol,ycol)),
   ...
   IF ((pt_transfer->adv_copy > " "))
    ...
    CALL print(calcpos(xcol,ycol)), pt_transfer->adv_copy, ycol += 12,
    row + 1
   ENDIF
   IF ((pt_transfer->adv_loc > " "))
    ...
    CALL print(calcpos(xcol,ycol)), pt_transfer->adv_loc, ycol += 12,
    row + 1
   ENDIF
   IF (ycol > 725)
    BREAK
   ENDIF
   xcol = 30, ycol += 12, row + 1,
   ...
   IF ((pt_transfer->disch_date > 0))
    xcol = 30, ycol += 12, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}{u}Pneumococcal and Influenza Immunizations before discharge:{endb}{endu}"
   ELSE
    xcol = 30, ycol += 12, row + 1,
    CALL print(calcpos(xcol,ycol)),
   ...
   ENDIF
   xcol = 30, ycol += 12, row + 1,
   CALL print(calcpos(xcol,ycol)), "{f/0}{cpi/14}{b}{u}Blood Pressure/Venipuncture:{endb}{endu}",
   ycol += 12,
   row + 1
   IF (size(trim(pt_transfer->ms_bloodpressurevenipuncture,3)) > 100)
    tempstring = pt_transfer->ms_bloodpressurevenipuncture, xcol = 50, wrapcol = 100,
    line_wrap
   ELSEIF (size(trim(pt_transfer->ms_bloodpressurevenipuncture,3),3) > 0)
    xcol = 50,
    CALL print(calcpos(xcol,ycol)), pt_transfer->ms_bloodpressurevenipuncture,
    ycol += 12, row + 1
   ENDIF
   xcol = 30, ycol += 12, row + 1,
   CALL print(calcpos(xcol,ycol)),
   "{f/0}{cpi/14}{b}{u}Wearing BP/Venipuncture Restriction Band:{endb}{endu}"
   IF (size(trim(pt_transfer->ms_wearing_bpv_band,3)) > 100)
    ycol += 12, row + 1, tempstring = pt_transfer->ms_wearing_bpv_band,
    xcol = 50, wrapcol = 100, line_wrap
   ELSEIF (size(trim(pt_transfer->ms_wearing_bpv_band,3),3) > 0)
    xcol = 245,
    CALL print(calcpos(xcol,ycol)), pt_transfer->ms_wearing_bpv_band,
    ycol += 12, row + 1
   ENDIF
   output_display = ""
   IF (ycol > 725)
   ...
   IF ((pt_transfer->disch_date > 0))
    xcol = 30, ycol += 12, row + 1,
    CALL print(calcpos(xcol,ycol)), "{b}{u}Infection Control before discharge:{endb}{endu}"
   ELSE
    xcol = 30, ycol += 12, row + 1,
   ...
   ELSE
    ...
    ELSE
     xcol = 130
    ENDIF
    CALL print(calcpos(xcol,ycol)), "No Isolation Orders found.", ycol += 12,
    row + 1
   ENDIF
   IF (ycol > 725)
    BREAK
   ...
   IF ((pt_transfer->ins_cnt > 0))
    FOR (x = 1 TO pt_transfer->ins_cnt)
      ...
      xcol = 100, row + 1,
      CALL print(calcpos(xcol,ycol)),
      "Name: ", pt_transfer->ins_qual[x].name, ycol += 12,
      ...
      IF ((pt_transfer->ins_qual[x].group_nbr > " "))
       xcol = 100, row + 1,
       CALL print(calcpos(xcol,ycol)),
       "Group #: ", pt_transfer->ins_qual[x].group_nbr, ycol += 12,
       row + 1
      ENDIF
      IF ((pt_transfer->ins_qual[x].member_nbr > " "))
       xcol = 100, row + 1,
       CALL print(calcpos(xcol,ycol)),
       "Member #: ", pt_transfer->ins_qual[x].member_nbr, ycol += 12,
       row + 1
      ENDIF
    ENDFOR
   ELSE
    xcol = 100, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "No insurance information found on visit.", ycol += 12, row + 1
   ENDIF
   IF (ycol > 725)
    BREAK
   ENDIF
   ...
   IF ((pt_transfer->cont_cnt > 0))
    FOR (x = 1 TO pt_transfer->cont_cnt)
      ...
      xcol = 100, row + 1,
      CALL print(calcpos(xcol,ycol)),
      "Name: ", pt_transfer->cont_qual[x].name, ycol += 12,
      row + 1
      FOR (y = 1 TO pt_transfer->cont_qual[x].phone_cnt)
       ...
       IF ((pt_transfer->cont_qual[x].phone_qual[y].home_phone > " "))
        xcol = 100, row + 1,
        CALL print(calcpos(xcol,ycol)),
        "Home Phone: ", pt_transfer->cont_qual[x].phone_qual[y].home_phone, ycol += 12,
        row + 1
       ELSEIF ((pt_transfer->cont_qual[x].phone_qual[y].bus_phone > " "))
        xcol = 100, row + 1,
        CALL print(calcpos(xcol,ycol)),
        "Bus Phone: ", pt_transfer->cont_qual[x].phone_qual[y].bus_phone, ycol += 12,
        row + 1
   ...
   ELSE
    xcol = 100, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "No Contact information found on visit.", ycol += 12, row + 1
   ENDIF
   BREAK, output_display = "", output_display1 = "",
   output_display2 = "", found = 0
   FOR (x = 1 TO pt_transfer->blob_cnt)
     FOR (y = 1 TO pt_transfer->blob_qual[x].blobs_cnt)
   ...
   ENDFOR
   IF (found=0
    AND phys_summ_found=0)
    ycol += 12, row + 1, xcol = 30,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}No Physician Discharge Summary or Dictated documents found on visit.{endb}",
    ycol += 12,
    row + 1
   ENDIF
   IF (ycol > 725)
   ...
   FOR (x = 1 TO pt_transfer->blob_cnt)
     FOR (y = 1 TO pt_transfer->blob_qual[x].blobs_cnt)
       ...
       IF (test_ind=1)
        CALL echo("line 4767")
       ENDIF
       IF ((pt_transfer->blob_qual[x].blob_set_name="Discharge/Transfer Note Hospital"))
        blob_set_name = pt_transfer->blob_qual[x].blob_set_name
        IF (found=0)
         ycol += 12, row + 1, xcol = 30,
         row + 1,
        ...
        ENDIF
       ENDIF
       IF ((pt_transfer->blob_qual[x].blob_set_name="Discharge/Transfer Note Hospital HIM"))
        blob_set_name = pt_transfer->blob_qual[x].blob_set_name
        IF (found=0)
         ycol += 12, row + 1, xcol = 30,
         row + 1,
   ...
   IF (found=0)
    ycol += 12, row + 1, xcol = 30,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}No Discharge/Transfer Note found on visit.{endb}", ycol += 12,
    row + 1
   ENDIF
   found = 0
   IF (test_ind=1)
   ...
   ENDFOR
   xcol = 30, ycol += 12, row + 1,
   CALL print(calcpos(xcol,ycol)), "{b}{REPEAT/110/_/}{endb}", ycol += 12,
   row + 1
   IF (ycol > 725)
    BREAK
   ENDIF
   ...
   FOR (x = 1 TO pt_transfer->blob_cnt)
     FOR (y = 1 TO pt_transfer->blob_qual[x].blobs_cnt)
       IF ((pt_transfer->blob_qual[x].blob_set_name="History and Physical - Dictated"))
        ...
        ENDIF
       ENDIF
       IF ((pt_transfer->blob_qual[x].blob_set_name="History and Physical Hospital"))
        blob_set_name = "History and Physical Hospital"
        IF (found=0)
         ycol += 12, row + 1, xcol = 30,
         row + 1,
         CALL print(calcpos(xcol,ycol)), "{f/0}{cpi/14}{b}{u}",
        ...
        ENDIF
       ENDIF
       IF ((pt_transfer->blob_qual[x].blob_set_name="History and Physical Hospital HIM"))
        blob_set_name = "History and Physical Hospital HIM"
        IF (found=0)
         ycol += 12, row + 1, xcol = 30,
         row + 1,
         CALL print(calcpos(xcol,ycol)), "{f/0}{cpi/14}{b}{u}",
   ...
   xcol = 30, row + 1,
   CALL print(calcpos(xcol,ycol)),
   "{f/0}{cpi/14}{b}{u}Current Medication List{endu}{endb}", ycol += 12, row + 1
   IF (ycol > 725)
    BREAK
   ENDIF
   ...
   IF (size(continuous_orders_disp->continuous_orders,5) > 0)
    FOR (z1 = 1 TO size(continuous_orders_disp->continuous_orders,5))
      IF ((continuous_orders_disp->continuous_orders[z1].action_type="Ordered"))
       ...
       meds_found = 1
       IF (print_title=0)
        xcol = 30, ycol += 12, title = build("{b}{u}IV Infusions before Discharge:{endb}{endu}"),
        CALL print(calcpos(xcol,ycol)), title, row + 1,
        ycol += 12, print_title = 1, xcol = 50
       ENDIF
   ...
   IF (meds_found=0)
    IF (ycol > 725)
     BREAK
    ENDIF
    xcol = 30, ycol += 12, title = build(
     "{b}{u}IV Infusions before Discharge:{endb}{endu} None found on visit."),
    CALL print(calcpos(xcol,ycol)), title, row + 1,
    ycol += 12
   ENDIF
   IF (ycol > 725)
    ...
    FOR (z1 = 1 TO size(scheduled_orders->qual,5))
      ...
      FOR (z2 = 1 TO size(scheduled_orders_disp->scheduled_orders,5))
        ...
        template_order_id)
         ...
         IF (print_title=0)
          xcol = 30, ycol += 12, row + 1,
          title = build("{b}{u}Active Meds before Discharge:{endb}{endu}"),
          CALL print(calcpos(xcol,ycol)), "{b}",
          title, row + 1, ycol += 12,
          print_title = 1, xcol = 50
         ENDIF
         IF (ycol > 725)
          BREAK
         ENDIF
         ...
         IF (size(scheduled_orders->qual[z1].child_ord,5) > 0)
   ...
   IF (meds_found=0)
    ...
    ENDIF
    xcol = 30, ycol += 12, title = build(
     "{b}{u}Active Meds before Discharge:{endb}{endu} None found on visit."),
    CALL print(calcpos(xcol,ycol)), title, row + 1,
    ycol += 12
   ENDIF
   IF (ycol > 725)
   ...
   IF (size(prn_orders_disp->prn_orders,5) > 0)
    FOR (z1 = 1 TO size(prn_orders_disp->prn_orders,5))
      IF ((prn_orders_disp->prn_orders[z1].action_type="Ordered"))
       ...
       ENDIF
       IF (print_title=0)
        xcol = 30, ycol += 12, title = build("{b}{u}PRN Meds before Discharge:{endb}{endu}"),
        CALL print(calcpos(xcol,ycol)), "{b}", title,
        row + 1, ycol += 12, print_title = 1,
        xcol = 50
       ENDIF
   ...
   IF (meds_found=0)
    ...
    ENDIF
    xcol = 30, ycol += 12, title = build(
     "{b}{u}PRN Meds before Discharge:{endb}{endu} None found on visit."),
    CALL print(calcpos(xcol,ycol)), title, row + 1,
    ycol += 12
   ENDIF
   xcol = 30, ycol += 12, row + 1,
      ...
      FOR (z2 = 1 TO size(scheduled_orders_disp->scheduled_orders,5))
        ...
        template_order_id)
         ...
         IF (print_title=0)
          ...
          IF ((pt_transfer->disch_date > 0))
           title = build("{b}{u}",
            "Discontinued or Completed Scheduled Meds in last 7 days before Discharge:{endb}{endu}")
          ELSE
    ...
    FOR (sch = 1 TO size(scheduled_orders->qual,5))
      ...
      IF ((scheduled_orders->qual[sch].print_ind=0)
       ...
       IF (print_title=0)
        ...
        IF ((pt_transfer->disch_date > 0))
         title = build(
          "{b}{u}Discontinued or Completed Scheduled Meds in last 7 days before Discharge:{endb}{endu}"
          )
        ELSE
         title = build("{b}{u}Discontinued or Completed Scheduled Meds in last 7 days:{endb}{endu}")
        ENDIF
   ...
   IF (meds_found=0)
    ...
    ycol += 12
   ENDIF
   CALL echo("Print Home and Rx Meds")
   IF (size(doc_rx_meds_disp->doc_rx_meds_disp,5) > 0)
    IF (ycol > 725)
     BREAK
    ENDIF
    print_title = 0
    FOR (sch = 1 TO size(doc_rx_meds_disp->doc_rx_meds_disp,5))
      IF (ycol > 725)
       BREAK, print_title = 10
      ENDIF
      xcol = 50
      IF (((print_title=0) OR (print_title=10)) )
       xcol = 30, ycol += 12, row + 1
       IF (print_title=10)
        title = build(
         "{b}{u}Patient Stated Medication List at time of Admission (continued){endb}{endu}")
       ELSE
        title = build("{b}{u}Patient Stated Medication List at time of Admission{endb}{endu}")
       ENDIF
       CALL print(calcpos(xcol,ycol)), title, row + 1,
       ycol += 12, print_title = 1, xcol = 50
      ENDIF
      tempstring = concat("{b}",doc_rx_meds_disp->doc_rx_meds_disp[sch].mnemonic,"{endb}: ",
       doc_rx_meds_disp->doc_rx_meds_disp[sch].clinical_display_line," ",
       format(doc_rx_meds_disp->doc_rx_meds_disp[sch].orig_order_dt_tm,"mm/dd/yy hh:mm;;q")), wrapcol
       = 100, line_wrap,
      CALL print(calcpos(xcol,ycol)), tempstring, row + 1,
      ycol += 12
    ENDFOR
   ELSE
    IF (ycol > 725)
    ...
    ENDIF
    xcol = 30, ycol += 12, title = build(
     "{b}{u}Patient Stated Medication List:{endb}{endu} None found on visit."),
    CALL print(calcpos(xcol,ycol)), title, row + 1,
    ycol += 12
   ENDIF
   BREAK,
   CALL echo("PRINTS WOUND INFO 2")
   IF (((ycol+ 10) > 725))
    BREAK
   ENDIF
   IF (size(wound_band->wound_section,5) > 0
    AND size(wounds->wound_ord,5) > 0)
    ycol += 24, row + 1, xcol = 30,
    ycol += 12, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}{u}Wound Care: Orders at Discharge and Last Dcoumentation{endu}{endb}"
    IF (matt_device != "XXXX")
     xcol = 30, ycol += 12, row + 1,
     CALL print(calcpos(xcol,ycol)), "{b} Mattress Device: {endb}", matt_device,
     ycol += 12, row + 1
    ENDIF
    FOR (wndord_prt = 1 TO size(wounds->wound_ord,5))
      xcol = 30, ycol += 12, row + 1,
      CALL print(calcpos(xcol,ycol)), wounds->wound_ord[wndord_prt].name, ycol += 12,
      row + 1
    ENDFOR
    FOR (wndband_prt = 1 TO size(wound_band->wound_section,5))
      xcol = 30, ycol += 12, row + 1,
      CALL print(calcpos(xcol,ycol)), "{b}{u}", wound_band->band_name,
      "{endu}{endb}", ycol += 12, row + 1
      FOR (wndres_prt = 1 TO size(wound_band->wound_section[wndband_prt].result,5))
       IF (wndres_prt=1)
        IF (ycol > 713)
         BREAK, xcol = 30, ycol += 12,
         row + 1,
         CALL print(calcpos(xcol,ycol)), "{b}",
         wound_band->band_name, "{endb}", ycol += 12,
         row + 1, xcol = 30, ycol += 12,
         row + 1,
         CALL print(calcpos(xcol,ycol)), "{b}",
         wound_band->wound_section[wndband_prt].section_name, "{endb}"
        ELSE
         xcol = 30, ycol += 12, row + 1,
         CALL print(calcpos(xcol,ycol)), "{b}", wound_band->wound_section[wndband_prt].section_name,
         "{endb}"
        ENDIF
       ENDIF
       ,
       IF (ycol > 725)
        ...
        row + 1,
        CALL print(calcpos(xcol,ycol)), "{b}",
        wound_band->wound_section[wndband_prt].section_name, "{endb}", ycol += 12,
        row + 1
       ELSE
        xcol = 30, ycol += 10, row + 1
        IF ((wound_band->wound_section[wndband_prt].result[wndres_prt].wound_result > ""))
         tempstring = trim(wound_band->wound_section[wndband_prt].result[wndres_prt].wound_result,3),
         line_wrap2
        ENDIF
       ENDIF
      ENDFOR
   ...
   ELSE
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{b}{u}No Wound Results{endu}{endb}", ycol += 12, row + 1
   ENDIF
   xcol = 30, ycol += 12, row + 1,
   CALL print(calcpos(xcol,ycol)), "{b}{REPEAT/110/_/}{endb}", ycol += 12,
   row + 1, output_display = "", output_display1 = "",
   output_display2 = ""
   IF (ycol > 725)
    BREAK
   ENDIF
   ...
   ELSE
    xcol = 140, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "No Consult Orders found on visit.", ycol += 12, row + 1
   ENDIF
   IF (ycol > 725)
    BREAK
   ENDIF
   found = 0,
   CALL echo("print consults"), BREAK,
   ycol += 12, row + 1, xcol = 30,
   CALL print(calcpos(xcol,ycol)), "{f/0}{cpi/14}{b}{u}Consultation Notes - All{endu}{endb}"
   FOR (x = 1 TO pt_transfer->blob_cnt)
     FOR (y = 1 TO pt_transfer->blob_qual[x].blobs_cnt)
   ...
   IF ((pt_transfer->disch_date > 0))
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}{u}Vitals in the last 24 hours before discharge:{endb}{endu}"
   ELSE
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
   ...
   ELSE
    ...
    ycol += 12, row + 1
   ENDIF
   CALL echo("Transfusion Orders")
   IF (size(transfusion->txfuse_ord,5) > 0)
    IF (ycol > 725)
     BREAK
    ENDIF
    print_title = 0
    FOR (sch = 1 TO size(transfusion->txfuse_ord,5))
      IF (ycol > 725)
       BREAK, print_title = 10
      ENDIF
      xcol = 50
      IF (((print_title=0) OR (print_title=10)) )
       xcol = 30, ycol += 12, row + 1
       IF (print_title=10)
        title = build("{b}{u}Transfusion Orders for last 14 days (continued){endb}{endu}")
       ELSE
        title = build("{b}{u}Transfusion Orders for last 14 days{endb}{endu}")
       ENDIF
       CALL print(calcpos(xcol,ycol)), title, row + 1,
       ycol += 12, print_title = 1, xcol = 50
      ENDIF
      tempstring = concat("{b}",transfusion->txfuse_ord[sch].mnemonic,"{endb}: ",transfusion->
       txfuse_ord[sch].clinical_display_line," ",
       format(transfusion->txfuse_ord[sch].orig_order_dt_tm,"mm/dd/yy hh:mm;;q")), wrapcol = 100,
      line_wrap,
      CALL print(calcpos(xcol,ycol)), tempstring, row + 1,
      ycol += 12
    ENDFOR
   ELSE
    ...
    ENDIF
    xcol = 30, ycol += 12, title = build(
     "{b}{u}Transfusion Orders for last 14 days:{endb}{endu} None found on visit."),
    CALL print(calcpos(xcol,ycol)), title, row + 1,
    ycol += 12
   ENDIF
   IF (ycol > 725)
   ...
   ELSE
    xcol = 270, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "No BSA, BMI, Height, or Weight found on visit.", ycol += 12, row + 1
   ENDIF
   output_display = "", output_display1 = "", output_display2 = ""
   IF (ycol > 725)
    BREAK
   ...
   IF ((pt_transfer->special_labs_cnt > 0))
    FOR (x = 1 TO size(pt_transfer->special_labs,5))
      ycol += 12, row + 1, xcol = 30,
      row + 1,
      CALL print(calcpos(xcol,ycol)), "{f/0}{cpi/12}{b}{u}",
      pt_transfer->special_labs[x].header2, "{endb}{endu}", ycol += 12,
      row + 1
   ...
   ELSE
    ...
    xcol = 270, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "No CBC, Lytes, BUN, Creat, or INR results found on visit.", ycol += 12, row + 1
   ENDIF
   ...
   IF ((pt_transfer->disch_date > 0))
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}{u}Lab results in the last 24 hours before discharge:{endb}{endu}"
   ELSE
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}{u}Lab results in the last 3 days:{endb}{endu}"
   ENDIF
   ...
   ELSE
    ...
    IF ((pt_transfer->disch_date > 0))
     xcol = 360, row + 1,
     CALL print(calcpos(xcol,ycol)),
     "No Lab results found in the last 7 days on visit."
    ELSE
     xcol = 200, row + 1,
     CALL print(calcpos(xcol,ycol)),
     "No Lab results found in the last 24 hrs. on visit."
    ENDIF
    ycol += 12, row + 1
   ENDIF
   critical_value = 0, output_display = "", output_display1 = "",
   ...
   IF ((pt_transfer->disch_date > 0))
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}{u}Micro results in the last 14 days before discharge:{endb}{endu}"
   ELSE
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
   ...
   IF ((pt_transfer->micro_cnt > 0))
    FOR (x = 1 TO pt_transfer->micro_cnt)
      ...
      ELSE
       ...
       xcol = 50, row + 1,
       CALL print(calcpos(xcol,ycol)),
       "No micro document or result available for test.", ycol += 12
      ENDIF
   ...
   FOR (x = 1 TO pt_transfer->blob_cnt)
     FOR (y = 1 TO pt_transfer->blob_qual[x].blobs_cnt)
      ...
      "CARDIOVASCULAR TEST")))
       mn_cardio_ind = 1, blob_set_name = pt_transfer->blob_qual[x].blob_set_name
       IF (found=0)
        ycol += 12, row + 1, xcol = 30,
        row + 1,
        CALL print(calcpos(xcol,ycol)), "{f/0}{cpi/14}{b}{u}",
   ...
   IF (found=0)
    ycol += 12, row + 1, xcol = 30,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}No Cardiovascular documents found on visit .{endb}", ycol += 12,
    row + 1
   ENDIF
   mn_cardio_ind = 0, output_display = " ", output_display1 = " ",
   output_display2 = " "
   IF ((pt_transfer->form_cnt > 0))
    ...
    xcol = 30, ycol += 12, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{b}{u}Ancillary Documention - Initial Assessment and Last two Treatment Notes:{endu}{endb}",
    ycol += 12,
    row + 1
    FOR (l = 1 TO pt_transfer->form_cnt)
      ...
      "PT Eval Pedi")))
       ...
       CALL print(calcpos(xcol,ycol)), output_display, ycol += 12,
       row + 1
      ELSEIF ((pt_transfer->form_qual[l].form_type IN ("PT Treat1", "PT Treat Pedi")))
       output_display = concat("{f/0}{cpi/14}{b}{u}Physical Therapy Treatment:{endb}{endu}"), xcol =
       30, row + 1,
       CALL print(calcpos(xcol,ycol)), output_display, ycol += 12,
       row + 1
      ELSEIF ((pt_transfer->form_qual[l].form_type IN ("OT Eval1", "OT Eval Pedi",
      "OT Feeding Eval Pedi", "OT Swallow Pedi")))
       output_display = concat("{f/0}{cpi/14}{b}{u}Occupational Therapy Assessment:{endb}{endu}"),
       xcol = 30, row + 1,
       CALL print(calcpos(xcol,ycol)), output_display, ycol += 12,
       row + 1
      ELSEIF ((pt_transfer->form_qual[l].form_type IN ("OT Treat1", "OT Treatment Pedi")))
      ...
      "ST Barium Swallow Pedi",
      "ST Speech Pedi", "ST BS Swallow Pedi")))
       output_display = concat("{f/0}{cpi/14}{b}{u}Speech Therapy Assessment:{endb}{endu}"), xcol =
       30, row + 1,
       CALL print(calcpos(xcol,ycol)), output_display, ycol += 12,
       row + 1
      ELSEIF ((pt_transfer->form_qual[l].form_type IN ("ST Treat1", "ST Treatment Pedi")))
      ...
      ELSEIF ((pt_transfer->form_qual[l].form_type IN ("PULM Eval1", "PULM Eval2")))
       ...
       CALL print(calcpos(xcol,ycol)), output_display, ycol += 12,
       row + 1
      ELSEIF ((pt_transfer->form_qual[l].form_type IN ("Braden", "Braden Newborn", "Braden Pedi")))
       output_display = concat("{f/0}{cpi/14}{b}{u}Braden Assessment:{endb}{endu}"), xcol = 30, row
        + 1,
       CALL print(calcpos(xcol,ycol)), output_display, ycol += 12,
       row + 1
      ENDIF
   ...
   ELSE
    null
   ENDIF
   CALL echo("Print 1 for Powernotes"),
   CALL echo("Large blob2"), BREAK
   IF (size(info->powernote,5) > 0)
    xcol = 30, ycol += 12, row + 1,
    CALL print(calcpos(xcol,ycol)), "{b}{u}Last Three Progress Notes:{endu}{endb}", ycol += 12,
    row + 1
    FOR (xn = 1 TO size(info->powernote,5))
      bbsize = 0, bflag = 0, xcol = 30,
      ycol += 12, row + 1,
      CALL print(calcpos(xcol,ycol)),
      "{b}{u}", info->powernote[xn].title, "{endu}{endb}"
      FOR (yprt = 1 TO size(info->powernote[xn].line,5))
        xcol = 30, ycol += 12, row + 1,
        CALL print(calcpos(xcol,ycol)), info->powernote[xn].line[yprt].disp
        IF (ycol > 725)
         BREAK
        ENDIF
      ENDFOR
   ...
   ELSE
    xcol = 30, ycol += 12, row + 1,
    CALL print(calcpos(xcol,ycol)), "{b}{u}No Progress Notes Found:{endu}{endb}", ycol += 12,
    row + 1
   ENDIF
   BREAK, print_title = 0, meds_found = 0
   IF (ycol > 725)
    BREAK
   ENDIF
   IF (size(prn_orders_disp->prn_orders,5) > 0)
    FOR (z1 = 1 TO size(prn_orders_disp->prn_orders,5))
      IF (trim(prn_orders_disp->prn_orders[z1].action_type) IN ("Completed", "Discontinued"))
       ...
       IF (print_title=0)
        ...
        IF ((pt_transfer->disch_date > 0))
         title = build(
          "{b}{u}Discontinued or Completed PRN Meds in last 7 days before Discharge:{endb}{endu}")
        ELSE
   ...
   IF (meds_found=0)
    ...
    IF ((pt_transfer->disch_date > 0))
     title = build(
      "{b}{u}Discontinued or Completed PRN Meds in last 7 days before Discharge:{endb}{endu}"), title
      = build(title," None found in last 7 days.")
    ELSE
     title = build("{b}{u}Discontinued or Completed PRN Meds in last 7 days:{endb}{endu}"), title =
     build(title," None found in last 7 days.")
    ENDIF
    CALL print(calcpos(xcol,ycol)), title, ycol += 12,
    row + 1
   ENDIF
   print_title = 0, meds_found = 0
   IF (ycol > 725)
    ...
    FOR (z1 = 1 TO size(continuous_orders_disp->continuous_orders,5))
      IF ((continuous_orders_disp->continuous_orders[z1].action_type IN ("Completed", "Discontinued")
       ...
       IF (print_title=0)
        ...
        IF ((pt_transfer->disch_date > 0))
         title = build(
          "{f/0}{cpi/14}{b}{u}Discontinued or Completed IV Infusions in last 7 days before Discharge:{endb}{endu}"
          )
        ELSE
         title = build(
   ...
   IF ((pt_transfer->disch_date > 0))
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}{u}", "Clinical Summary - Last 7 days of Orders before discharge:{endb}{endu}",
    ycol += 12,
    row + 1, xcol = 30, row + 1,
    ...
    FOR (x = 1 TO pt_transfer->catalog_cnt)
      ...
      FOR (y = 1 TO pt_transfer->catalog_qual[x].orders_cnt)
        ...
        ELSE
         xcol = 30, row + 1,
         CALL print(calcpos(xcol,ycol)),
         output_display1, ycol += 12, row + 1
        ENDIF
        output_display2 =
        "_______________________________________________________________________________________________",
        xcol = 105, row + 1,
        CALL print(calcpos(xcol,ycol)), output_display2, ycol += 12,
        row + 1
      ENDFOR
      ycol += 6, row + 1
    ENDFOR
   ELSE
    ...
    IF ((pt_transfer->disch_date > 0))
     xcol = 50, row + 1,
     CALL print(calcpos(xcol,ycol)),
     "No Orders found in the last 7 days on visit.", ycol += 12, row + 1
    ELSE
     xcol = 50, row + 1,
     CALL print(calcpos(xcol,ycol)),
     "No Orders found in the last 7 days on visit.", ycol += 12, row + 1
   ...
   ENDIF
  FOOT PAGE
   page_string = concat("Page: ",trim(cnvtstring(curpage))," of ",trim(cnvtstring(total_pages))),
   ycol = 24, xcol = 306,
   row + 1, "{f/0}{cpi/14}{endb}", "{ps/",
   xcol, ycol, " moveto (",
   page_string, ") dup stringwidth pop 2 div neg 0 rmoveto show/}"
  FOOT REPORT
   total_pages = curpage
  WITH maxrow = 800, maxcol = 800, check
 ;end select
 IF (test_ind=1)
 ...
 SELECT INTO value(printer_disp)
  ...
  HEAD REPORT
   ...
   ,
   MACRO (line_wrap3)
    b3_linefeed = concat(char(10)), b3_cc = 0, b3_s = 1,
    b3_len = 0, b3_e = 0, b3_tmp_comment = fillstring(90,""),
    a3_tmp_string = fillstring(90,""), tmp_var = 0, b3_cc = 1,
    b3_s = 1
    WHILE (b3_cc)
      IF (ycol > 725)
       BREAK
      ENDIF
      b3_tmp_comment = substring(b3_s,90,tempstring), b3_e = findstring(b3_linefeed,b3_tmp_comment,1)
      IF (b3_e)
       a3_tmp_string = substring(1,b3_e,b3_tmp_comment),
       CALL print(calcpos(xcol,ycol)), a3_tmp_string,
       row + 1, b3_s += b3_e
      ELSE
       IF (b3_tmp_comment > " ")
        CALL print(calcpos(xcol,ycol)), b3_tmp_comment, row + 1,
        ycol += 8, b3_s += 90
       ELSE
        b3_cc = 0
       ENDIF
   ...
   ENDMACRO
   , line = fillstring(200,"-"),
   line_short = fillstring(125,"-"), xcol = 0, ycol = 0
  HEAD PAGE
   output_display = "", "{f/0}{cpi/14}", ycol = 30,
   xcol = 30,
   CALL print(calcpos(xcol,ycol)), "Baystate Health Post Acute Care Discharge Report",
  ...
  DETAIL
   ...
   FOR (x = 1 TO pt_transfer->blob_cnt)
     FOR (y = 1 TO pt_transfer->blob_qual[x].blobs_cnt)
       IF ((pt_transfer->blob_qual[x].blob_set_name="Discharge/Transfer Note"))
        blob_set_name = "Physician Discharge Summary", found = 1
        IF (found=0)
         ycol += 12, row + 1, xcol = 30,
         row + 1,
         CALL print(calcpos(xcol,ycol)), "{f/0}{cpi/14}{b}{u}",
   ...
   IF (found=0
    ...
    ycol += 12, row + 1, xcol = 30,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}No Physcian Discharge Summary or Dictated documents found on visit.{endb}", ycol
     += 12,
    row + 1
   ENDIF
   IF (ycol > 725)
   ...
   IF (ycol > 725)
    BREAK
   ENDIF
   CALL echo("Line 9464"), print_title = 0, meds_found = 0
   IF (size(scheduled_orders->qual,5) > 0)
    FOR (z1 = 1 TO size(scheduled_orders->qual,5))
      IF (ycol > 725)
       BREAK
   ...
   IF (size(wound_band->wound_section,5) > 0
    ...
    FOR (wndord_prt = 1 TO size(wounds->wound_ord,5))
      IF (wndord_prt=1)
       xcol = 30, ycol += 12, row + 1,
       CALL print(calcpos(xcol,ycol)), "{b}Wound Orders{endb}"
      ENDIF
      xcol = 30, ycol += 12, row + 1,
      CALL print(calcpos(xcol,ycol)), wounds->wound_ord[wndord_prt].name
    ENDFOR
    FOR (wndband_prt = 1 TO size(wound_band->wound_section,5))
      FOR (wndres_prt = 1 TO size(wound_band->wound_section[wndband_prt].result,5))
       IF (wndres_prt=1)
   ...
   row + 1,
   CALL print(calcpos(xcol,ycol)),
   "{f/0}{cpi/14}{b}{u}Last results for CBC, Lytes, BUN, Creat, INR within last 3 days:{endb}{endu}"
   IF (ycol > 725)
    BREAK, xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}{u}Last results for CBC, Lytes, BUN, Creat, INR within last 3 days:{endb}{endu}"
   ...
   ELSE
    IF (ycol > 725)
     BREAK
    ENDIF
    ycol += 12, row + 1, xcol = 200,
    row + 1,
    CALL print(calcpos(xcol,ycol)),
    "No CBC, Lytes, BUN, Creat, or INR results found on visitwithin last 3 days.",
    ycol += 12, row + 1
   ENDIF
   critical_value = 0, output_display = "", output_display1 = "",
   output_display2 = ""
   ...
   ELSE
    xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)),
    "{f/0}{cpi/14}{b}{u}Lab results in the last 24 hours:{endb}{endu}"
   ENDIF
   IF ((pt_transfer->lab_results_cnt > 0))
    FOR (x = 1 TO pt_transfer->lab_results_cnt)
      critical_value = 0
   ...
   ELSE
    ...
    IF ((pt_transfer->disch_date > 0))
     xcol = 360, row + 1,
     CALL print(calcpos(xcol,ycol)),
     "No Lab results found in the last 24 hours on visit."
    ELSE
     xcol = 200, row + 1,
     CALL print(calcpos(xcol,ycol)),
     "No Lab results found in the last 24 hours on visit."
   ...
   ELSE
    null, xcol = 30, row + 1,
    CALL print(calcpos(xcol,ycol)), "{b}No Ancillary Documenation.{endb}", ycol += 12,
    row + 1
   ENDIF
   CALL echo("Print 2 for Powernotes"),
   CALL echo("Large blob2"), BREAK
   IF (size(info->powernote,5) > 0)
    xcol = 30, ycol += 12, row + 1,
    CALL print(calcpos(xcol,ycol)), "{b}{u}Last Three Progress Notes: {endu}{endb}", ycol += 12,
   ...
   ENDIF
  FOOT PAGE
   page_string = concat("Patient Name: ",pt_transfer->name," MRN#: ",pt_transfer->mrn,
    "        Page: ",
    trim(cnvtstring(curpage))," of ",trim(cnvtstring(total_pages))), ycol = 24, xcol = 306,
   row + 1, "{f/0}{cpi/14}{endb}", "{ps/",
   xcol, ycol, " moveto (",
   page_string, ") dup stringwidth pop 2 div neg 0 rmoveto show/}"
  FOOT REPORT
   "{f/0}{cpi/14}{endb}", page_string = "END OF REPORT", ycol = 12,
   xcol = 306, row + 1, "{ps/",
   xcol, ycol, " moveto (",
   page_string, ") dup stringwidth pop 2 div neg 0 rmoveto show/}"
...
#exit_script
 CALL echorecord(pt_transfer)
 CALL echorecord(scheduled_orders_disp)
