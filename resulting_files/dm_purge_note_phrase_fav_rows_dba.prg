CREATE PROGRAM dm_purge_note_phrase_fav_rows:dba
 ...
 DECLARE v_errcode2 = i4 WITH noconstant(0)
 DECLARE i18nhandle = w8 WITH protect, noconstant(0)
 DECLARE v_errmsg2 = vc WITH protect, noconstant("")
 SET reply->status_data.status = "F"
 SET reply->table_name = "NOTE_PHRASE_FAVORITE"
 SET reply->rows_between_commit = 100
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 SELECT INTO "nl:"
  npf.rowid
  FROM note_phrase_favorite npf
  WHERE parser(sbr_getrowidnotexists("npf.prsnl_id > 0","npf"))
   AND npf.prsnl_id IN (
  (SELECT
   p.person_id
   FROM prsnl p
   WHERE p.active_ind=0))
   AND  NOT (npf.note_phrase_favorite_id IN (
  (SELECT
   npf2.parent_note_phrase_favorite_id
   FROM note_phrase_favorite npf2
   WHERE npf2.note_phrase_favorite_id != npf.note_phrase_favorite_id)))
  HEAD REPORT
   row_cnt = 0
  DETAIL
   row_cnt += 1
   ...
   ENDIF
   reply->rows[row_cnt].row_id = npf.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,row_cnt)
  WITH nocounter, maxqual(npf,value(request->max_rows))
 ;end select
 SET v_errcode2 = error(v_errmsg2,1)
 IF (v_errcode2 != 0)
  SET reply->err_code = v_errcode2
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s2","Error occured during processing %1","s",
   nullterm(v_errmsg2))
  GO TO exit_script
 ENDIF
 SET reply->status_data.status = "S"
 SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s2","%1 rows qualified","i",row_cnt)
