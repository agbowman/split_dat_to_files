CREATE PROGRAM dm_pcmb_onc_ii_auto_activity:dba
 ...
 RECORD rreclist(
   1 from_rec[*]
     ...
     2 from_encounter_id = f8
     2 from_parent_event_id = f8
     2 from_order_id = f8
   1 to_rec[*]
     2 to_id = f8
     2 active_ind = i4
     2 active_status_cd = f8
     2 to_encounter_id = f8
     2 to_parent_event_id = f8
     2 to_order_id = f8
 )
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "ONC_II_AUTO_ACTIVITY"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_ONC_II_AUTO_ACTIVITY"
 ...
 IF ((request->xxx_combine[icombine].encntr_id=0))
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ENDIF
    rreclist->from_rec[v_cust_count1].from_id = frm.onc_ii_auto_activity_id, rreclist->from_rec[
    v_cust_count1].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count1].active_status_cd =
    frm.active_status_cd,
    rreclist->from_rec[v_cust_count1].from_encounter_id = frm.encntr_id, rreclist->from_rec[
    v_cust_count1].from_parent_event_id = frm.parent_event_id, rreclist->from_rec[v_cust_count1].
    from_order_id = frm.order_id
   FOOT REPORT
 ...
 ELSE
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    v_cust_count1].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count1].active_status_cd =
    frm.active_status_cd,
    rreclist->from_rec[v_cust_count1].from_parent_event_id = frm.parent_event_id, rreclist->from_rec[
    v_cust_count1].from_order_id = frm.order_id
   FOOT REPORT
    stat = alterlist(rreclist->from_rec,v_cust_count1)
   WITH forupdatewait(frm)
  ;end select
 ...
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count2].to_id = tu.onc_ii_auto_activity_id, rreclist->to_rec[
    v_cust_count2].active_ind = tu.active_ind, rreclist->to_rec[v_cust_count2].active_status_cd = tu
    .active_status_cd,
    rreclist->to_rec[v_cust_count2].to_encounter_id = tu.encntr_id, rreclist->to_rec[v_cust_count2].
    to_parent_event_id = tu.parent_event_id, rreclist->to_rec[v_cust_count2].to_order_id = tu
    .order_id
   FOOT REPORT
  ...
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
    ...
    FOR (v_cust_loopcount2 = 1 TO v_cust_count2)
      IF ((rreclist->from_rec[v_cust_loopcount].from_encounter_id=rreclist->to_rec[v_cust_loopcount2]
      .to_encounter_id)
       AND (rreclist->from_rec[v_cust_loopcount].from_order_id=rreclist->to_rec[v_cust_loopcount2].
      to_order_id)
       AND (rreclist->from_rec[v_cust_loopcount].from_parent_event_id=rreclist->to_rec[
      v_cust_loopcount2].to_parent_event_id))
       SET to_id_present = 1
      ENDIF
    ENDFOR
    IF (to_id_present=1)
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM onc_ii_auto_activity frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.onc_ii_auto_activity_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "ONC_II_AUTO_ACTIVITY"
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_to_fk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =
  i4)
   UPDATE  FROM onc_ii_auto_activity frm
    ...
    WHERE frm.onc_ii_auto_activity_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
   ...
   IF (curqual=0)
    ...
    SET request->error_message = substring(1,132,build("Could not delete pk val=",s_df_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
