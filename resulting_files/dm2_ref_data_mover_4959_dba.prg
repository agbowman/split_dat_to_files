CREATE PROGRAM dm2_ref_data_mover_4959:dba
 ...
 DECLARE cust_pref_value = vc
 DECLARE cust_pref_name = vc
 DECLARE cust_new_value = vc
 DECLARE cust_pref_value_pos = i4
 ...
 SET cust_pref_value = get_value(cust_tab_name,"PREF_VALUE","FROM")
 SET cust_pref_name = get_value(cust_tab_name,"PREF_NAME","FROM")
 SET cust_pref_value_pos = get_col_pos(cust_tab_name,"PREF_VALUE")
 SET cust_pe_name = get_value(cust_tab_name,"PARENT_ENTITY_NAME","FROM")
 IF ( NOT (cust_pe_name IN ("SN_DOC_REF", "AREA_PREFS")))
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV11"
  GO TO exit_4959
 ELSE
  IF (cust_pref_name IN ("OPS_PRINT_CS_QUE", "OPS_PRINT_QUE", "AUTO_PICK_LIST_PRINT_QUEUE"))
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV11"
   GO TO exit_4959
  ENDIF
 ENDIF
 IF (cust_pref_name IN ("COST_TYPE_CD", "EVENT_CD", "ROOT_LOC_CD", "REPORT_CLASS_INSTANCE_CD"))
  SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_pref_value_pos].translated = 0
  SET cust_new_value = select_merge_translate(cust_pref_value,"CODE_VALUE")
  IF (cust_new_value != "No Trans")
   CALL put_value(cust_tab_name,"PREF_VALUE",cust_new_value)
  ELSE
   SET cust_rpt_value = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(cust_pref_value))
 ...
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"NAME_VALUE_PREFS_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"NAME_VALUE_PREFS_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
   ...
   SET cust_from_cd = get_value(cust_tab_name,"NAME_VALUE_PREFS_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_4959
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"NAME_VALUE_PREFS_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_4959
   ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"PREF_VALUE")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the PREF_VALUE"
