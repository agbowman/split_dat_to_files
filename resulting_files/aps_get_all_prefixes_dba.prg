CREATE PROGRAM aps_get_all_prefixes:dba
...
#script
 ...
 ELSEIF ((request->skip_resource_security_ind=0))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (p
   WHERE parser(prefixes_where)
    AND p.prefix_id != 0.0)
  DETAIL
   service_resource_cd = p.service_resource_cd
   ...
   ENDIF
  FOOT REPORT
   stat = alter(reply->prefix_qual,prefix_cnt)
  WITH nocounter
 ;end select
