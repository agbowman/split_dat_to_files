CREATE PROGRAM dm2_aload_prgtpl_tab
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_prg_tab",".log","dm2_aload_prgtpl_tab")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_prgtpl_tab"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daptab_cnt = i4 WITH protect, noconstant(0)
 DECLARE daptab_str = vc WITH protect, noconstant("")
 DECLARE daptab_req_size = i4 WITH protect, noconstant(0)
 SET daptab_req_size = size(requestin->list_0,5)
 IF (dac_pop_coldic_rec("DM_ADM_PURGE_TABLE")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
 ...
 CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
   " dcv ,(dummyt d with seq = value(daptab_req_size) )"),0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_prgtpl_tab completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_prgtpl_tab")
END GO
