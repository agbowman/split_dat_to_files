CREATE PROGRAM ct_pt_mp_get_prot_contact_info:dba
 PROMPT
 ...
 SUBROUTINE (populatereplywitherrormessageifany(reply=vc(ref)) =vc)
   ...
   RETURN(errormessage)
 END ;Subroutine
 SET log_program_name = "ct_pt_mp_get_prot_contact_info"
 FREE RECORD request
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_pt_mp_get_prot_contact_info",
 ...
 DECLARE alphapager_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",212,"ALPHAPAGER"))
 DECLARE email_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",212,"EMAIL"))
 DECLARE bus_phone_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",43,"BUSINESS"))
 DECLARE alpha_phone_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",43,"PAGER BUS"))
 SET reply->status_data.status = "F"
  ...
  FOR (index = 1 TO prot_size)
    ...
    SELECT INTO "nl:"
     FROM prot_role pr,
      ...
      prsnl p
     PLAN (pr
      WHERE (pr.prot_amendment_id=request->protocols[index].prot_amendment_id)
     ...
     DETAIL
      contactcount += 1
      IF (contactcount > size(reply->primary_contacts[index].primary_contact_info,5))
      ...
      rank_nbr = pr.primary_contact_rank_nbr,
      reply->primary_contacts[index].primary_contact_info[contactcount].role_name =
      uar_get_code_display(pr.prot_role_cd), reply->primary_contacts[index].primary_contact_info[
      contactcount].organization_name = o.org_name
      IF (email_source=0)
    ...
    SELECT INTO "nl:"
     ...
     HEAD REPORT
      stat = alterlist(reply->primary_contacts[index].primary_contact_info,(contactcount+ 10))
     DETAIL
      contactcount += 1
      IF (contactcount > size(reply->primary_contacts[index].primary_contact_info,5))
       stat = alterlist(reply->primary_contacts[index].primary_contact_info,(contactcount+ 9))
     ...
     FOR (count = 1 TO contactcount)
       ...
       SELECT INTO "nl:"
        ...
        PLAN (a
         ...
         person_id)
          AND a.address_type_cd=alphapager_cd)
        DETAIL
    ...
    ENDIF
  ENDFOR
  IF (size(reply->primary_contacts[0].primary_contact_info,5)=0)
