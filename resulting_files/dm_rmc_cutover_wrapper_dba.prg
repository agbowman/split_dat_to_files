CREATE PROGRAM dm_rmc_cutover_wrapper:dba
 ...
 SUBROUTINE dm2_get_program_stack(null)
   ...
   ENDIF
 END ;Subroutine
 DECLARE check_table_errors(cte_env_id=f8) = i4
 SUBROUTINE check_table_errors(cte_env_id)
   SELECT INTO "NL:"
    FROM dm_info d
    WHERE d.info_domain=concat("RDDS CUTOVER::",trim(cnvtstring(cte_env_id,20)))
     AND (d.info_number=- (1))
     AND (( NOT (d.updt_applctx IN (
    (SELECT
     audsid
     FROM gv$session)))) OR (d.updt_applctx=cnvtreal(currdbhandle)))
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SUBROUTINE check_open_event(cur_env_id,paired_env_id)
   ...
   RETURN(coe_event_flg)
 END ;Subroutine
 DECLARE drcw_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drcw_auto_cut_ind = i2 WITH protect, noconstant(0)
 DECLARE drcw_oe_check = i4 WITH protect, noconstant(0)
 DECLARE drc_work_ind = i4 WITH protect, noconstant(0)
 ...
 ENDIF
 IF (validate(drcw_reply->status,"D")="D")
  FREE RECORD drcw_reply
  RECORD drcw_reply(
    1 status = c1
    1 status_msg = vc
  ) WITH protect
 ENDIF
 IF (check_logfile("dm_rmc_cut_wrp",".log","DM_RMC_CUTOVER_WRAPPER LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_wrapper
 ENDIF
 SET drcw_reply->status = "F"
 SET drcw_reply->status_msg = "Starting Script"
 SET dm_err->eproc = "Starting DM_RMC_CUOTVER..."
 EXECUTE dm_rmc_cutover
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->err_ind = 1
  GO TO exit_wrapper
 ENDIF
 SET dm_err->eproc = "Checking to see if any tables were in error"
 SET drcw_oe_check = check_table_errors(request->source_env_id)
 IF (drcw_oe_check=1)
  SET drcw_reply->status = "F"
  SET drcw_reply->status_msg = "One or more table(s) failed to cutover correctly."
 ELSEIF ((drcw_oe_check=- (1)))
  SET drcw_reply->status = "F"
  SET drcw_reply->status_msg = dm_err->err_ind
 ELSE
  IF ((request->table_name <= " ")
   ...
   IF (curqual=0)
    ...
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET drcw_reply->status = "F"
    SET drcw_reply->status_msg = dm_err->emsg
    GO TO exit_wrapper
   ENDIF
   SELECT INTO "NL:"
    FROM dm_env_reltn d
    WHERE d.child_env_id=drcw_env_id
     AND (d.parent_env_id=request->source_env_id)
     AND d.relationship_type="AUTO CUTOVER"
    DETAIL
     drcw_auto_cut_ind = 1
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_wrapper
   ENDIF
   IF (drcw_auto_cut_ind=1)
    SELECT INTO "NL:"
     FROM dm_info d
     WHERE d.info_domain=concat("RDDS CUTOVER::",trim(cnvtstring(request->source_env_id,20)))
      AND d.info_number != 2
      AND d.updt_applctx IN (
     (SELECT
      audsid
      FROM gv$session))
     WITH nocounter
    ...
    ENDIF
    IF (curqual > 0)
     SET drcw_reply->status = "S"
    ELSE
     SET drcw_oe_check = check_open_event(drcw_env_id,request->source_env_id)
     IF (drcw_oe_check IN (0, 2))
      SET drcw_reply->status = "S"
     ELSE
      SET drcw_reply->status = "F"
      SET drcw_reply->status_msg =
      "No cutover sessions are running, but the event is not closed and it is auto-cutover environment"
     ENDIF
    ENDIF
   ELSE
    SET drcw_reply->status = "S"
