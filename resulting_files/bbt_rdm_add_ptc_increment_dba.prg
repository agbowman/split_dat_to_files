CREATE PROGRAM bbt_rdm_add_ptc_increment:dba
 ...
 IF (curqual=0)
  INSERT  FROM dm_info dm
   SET dm.info_domain = pathnet_bbt, dm.info_name = ptc_loop_increment, dm.info_date = cnvtdatetime(
     curdate,curtime),
    dm.info_char = export_increment, dm.updt_dt_tm = cnvtdatetime(curdate,curtime)
  ;end insert
  IF (curqual=0)
 ...
 IF (curqual=0)
  INSERT  FROM dm_info dm
   SET dm.info_domain = pathnet_bbt, dm.info_name = ptc_report_loop_increment, dm.info_date =
    cnvtdatetime(curdate,curtime),
    dm.info_char = report_increment, dm.updt_dt_tm = cnvtdatetime(curdate,curtime)
  ;end insert
  IF (curqual=0)
   SET smsg = build("Unable to insert DM_INFO Row.")
   CALL logmsg(smsg,e_dispmsg)
