CREATE PROGRAM da2_import_reports
 ...
 SET readme_data->message = "Readme failure.  Starting da2_import_reports script."
 DECLARE app = i4 WITH noconstant(5000)
 DECLARE task = i4 WITH noconstant(3202004)
 DECLARE req = i4 WITH noconstant(5009202)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (mod(rdm_rptcnt,10)=1)
    rdm_stat = alterlist(reports->list,(rdm_rptcnt+ 9))
   ENDIF
   ...
   reports->list[rdm_rptcnt].filename = build(path,file)
  WITH nocounter, maxcol = 2100
 ;end select
 SET rdm_stat = alterlist(reports->list,rdm_rptcnt)
 CALL echorecord(reports)
 ...
 ELSE
  ...
  SET querycount = size(reports->list,5)
  FOR (queryindex = 1 TO querycount)
    SET hitem = uar_srvadditem(hrequest,"reports")
    SET stat = uar_srvsetstring(hitem,"filename",nullterm(reports->list[querycount].filename))
    IF (stat=0)
