CREATE PROGRAM bhs_prax_past_medical_hx
 ...
 SELECT INTO "NL:"
  ...
  uar_get_code_display(p.life_cycle_status_cd), pc.problem_comment,
  comment_added_by = pr1.name_full_formatted, comment_dt_tm = format(pc.comment_dt_tm,
   "MM/DD/YYYY HH:MM"), n.source_identifier,
  ...
  HEAD p.problem_id
   ...
   problem_hx[vcnt].problem_id = p.problem_id,
   ...
   problem_hx[vcnt].onset_dt_tm = onset_dt_tm,
   problem_procedure_hx->problem_hx[vcnt].annotated_display = p.annotated_display,
   problem_procedure_hx->problem_hx[vcnt].problem_name = n.source_string, problem_procedure_hx->
   problem_hx[vcnt].code = n.source_identifier,
   problem_procedure_hx->problem_hx[vcnt].vocabulary = n_source_vocabulary_disp, ccnt = 0
  DETAIL
