CREATE PROGRAM dm2_oragen_spec_cols:dba
 ...
 ENDIF
 DECLARE dosc_logfile_name = vc WITH protect, noconstant("dm2_osc_")
 SET readme_data->status = "F"
 IF (check_logfile(dosc_logfile_name,".log","DM2_ORAGEN_SPEC_COLS ...") != 1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking for tables with APPLCTX columns to Oragen."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  u.table_name, tl.attr_name, tl.type,
  tl.len
 ...
 ENDIF
#exit_script
 SET dm_err->eproc = "Finished updating table definitions."
 IF ((dm_err->err_ind=1))
  SET readme_data->status = "F"
 ...
 ENDIF
 CALL final_disp_msg(dosc_logfile_name)
END GO
