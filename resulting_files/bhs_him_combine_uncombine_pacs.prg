CREATE PROGRAM bhs_him_combine_uncombine_pacs
 PROMPT
 ...
 SELECT INTO "NL:"
  ...
  FROM person_combine pc,
   ...
   person p2
  PLAN (pc
   WHERE pc.updt_dt_tm BETWEEN cnvtdatetime((curdate - 1),curtime3) AND cnvtdatetime(curdate,curtime3
    ))
   JOIN (pr
   WHERE pc.updt_id=pr.person_id)
   JOIN (ea
   WHERE outerjoin(pc.encntr_id)=ea.encntr_id
 ...
 SELECT INTO "NL:"
  FROM encntr_combine ec,
   ...
   encntr_alias ea2
  PLAN (ec
   WHERE ec.updt_dt_tm BETWEEN cnvtdatetime((curdate - 1),curtime3) AND cnvtdatetime(curdate,curtime3
    ))
   JOIN (pr
   ...
   JOIN (ea1
   WHERE outerjoin(ecd.entity_id)=ea1.encntr_alias_id
    AND ea1.encntr_alias_type_cd=outerjoin(1077.00))
   JOIN (ea2
   WHERE outerjoin(ec.to_encntr_id)=ea2.encntr_id
 ...
 SELECT INTO "NL:"
  ...
  PLAN (d1
   ...
   WHERE (cmb->combines[d1.seq].to_person_id=pa.person_id)
    ...
    AND ((pa.active_ind=1) OR (pa.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3))) )
  HEAD REPORT
   ta_cnt = 0
  DETAIL
 ...
 SELECT INTO value(var_output)
  ...
  HEAD REPORT
   ...
   CALL print(build("RAD_IND",char(9)))
  DETAIL
   FOR (c = 1 TO cmb->c_cnt)
    row + 1,
    IF ((cmb->combines[c].rad_order_ind=1))
     col + 0,
     ...
     ENDIF
     col + 0,
     CALL print(build(cmb->combines[c].rad_order_ind))
    ELSE
     col + 0,
     CALL print("NO RADNET ORDER COMBINE/UNCOMBINE")
    ENDIF
   ENDFOR
