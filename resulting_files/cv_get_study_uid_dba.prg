CREATE PROGRAM cv_get_study_uid:dba
 IF (validate(stat)=0)
 ...
 SET stat = alterlist(request->cv_proc,proc_pad)
 FOR (proc_idx = (proc_cnt+ 1) TO proc_pad)
   SET request->cv_proc[proc_idx].cv_proc_id = request->cv_proc[proc_cnt].cv_proc_id
 ENDFOR
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   JOIN (imspr
   WHERE expand(proc_idx,nstart,((nstart+ c_block_size) - 1),imspr.parent_entity_id,request->cv_proc[
    proc_idx].cv_proc_id)
    AND imspr.parent_entity_name="CV_PROC"
    AND (request->cv_proc[proc_idx].study_state_cd=0))
   JOIN (ims
   WHERE ims.im_study_id=imspr.im_study_id)
  ORDER BY ims.im_study_id
  HEAD REPORT
   stat = alterlist(request->cv_proc,proc_cnt)
  ...
  DETAIL
   proc_idx = locateval(proc_idx,(1+ ((d.seq - 1) * c_block_size)),proc_cnt,imspr.parent_entity_id,
    request->cv_proc[proc_idx].cv_proc_id), request->cv_proc[proc_idx].created_study_uid = ims
   .created_study_uid, request->cv_proc[proc_idx].study_uid = ims.study_uid
   IF ((request->cv_proc[proc_idx].study_state_cd=0))
    request->cv_proc[proc_idx].study_state_cd = ims.study_state_cd
   ENDIF
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   DETAIL
    ...
    FOR (l_matched_p_idx = 1 TO l_matched_p_cnt)
      proc_idx = matched->qual[match_idx].procs[l_matched_p_idx].proc_idx, request->cv_proc[proc_idx]
      .device_name = imd.device_name, request->cv_proc[proc_idx].ip_address = imd.ip_address,
      request->cv_proc[proc_idx].station_name = imas.station_name, request->cv_proc[proc_idx].
      im_device_id = imd.im_device_id
    ENDFOR
   WITH nocounter
  ;end select
 ELSE
 ...
 ENDIF
 FREE RECORD matched
 SET stat = alterlist(request->cv_proc,proc_cnt)
 SET reply->status_data.status = "S"
#exit_script
 CALL cv_log_msg_post("007 10/27/23   AS043139")
END GO
