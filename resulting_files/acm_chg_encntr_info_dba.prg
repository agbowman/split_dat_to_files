CREATE PROGRAM acm_chg_encntr_info:dba
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   encntr_info e
  SET e.encntr_info_id = acm_request->encntr_info_qual[xref->chg[d.seq].idx].encntr_info_id, e
   .beg_effective_dt_tm = cnvtdatetime(acm_request->encntr_info_qual[xref->chg[d.seq].idx].
    beg_effective_dt_tm), e.chartable_ind = acm_request->encntr_info_qual[xref->chg[d.seq].idx].
   chartable_ind,
   e.contributor_system_cd =
   IF ((acm_request->encntr_info_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0)) acm_request
    ->encntr_info_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , e.encntr_id =
   IF ((acm_request->encntr_info_qual[xref->chg[d.seq].idx].encntr_id > 0)) acm_request->
    encntr_info_qual[xref->chg[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->encntr_info_qual[xref->chg[d.seq].idx].encntr_idx].
    encntr_id
   ENDIF
   , e.end_effective_dt_tm = cnvtdatetime(acm_request->encntr_info_qual[xref->chg[d.seq].idx].
    end_effective_dt_tm),
   e.info_sub_type_cd = acm_request->encntr_info_qual[xref->chg[d.seq].idx].info_sub_type_cd, e
   .info_type_cd = acm_request->encntr_info_qual[xref->chg[d.seq].idx].info_type_cd, e.internal_seq
    = acm_request->encntr_info_qual[xref->chg[d.seq].idx].internal_seq,
   e.long_text_id = acm_request->encntr_info_qual[xref->chg[d.seq].idx].long_text_id, e.priority_seq
    = acm_request->encntr_info_qual[xref->chg[d.seq].idx].priority_seq, e.value_cd = acm_request->
   encntr_info_qual[xref->chg[d.seq].idx].value_cd,
   e.value_dt_tm =
   IF ((acm_request->encntr_info_qual[xref->chg[d.seq].idx].value_dt_tm > 0)) cnvtdatetime(
     acm_request->encntr_info_qual[xref->chg[d.seq].idx].value_dt_tm)
   ELSE null
   ENDIF
   , e.value_numeric = acm_request->encntr_info_qual[xref->chg[d.seq].idx].value_numeric, e
   .active_ind = acm_request->encntr_info_qual[xref->chg[d.seq].idx].active_ind,
   e.active_status_cd = acm_request->encntr_info_qual[xref->chg[d.seq].idx].active_status_cd, e
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_info_id=acm_request->encntr_info_qual[xref->chg[d.seq].idx].encntr_info_id))
  WITH nocounter, status(reply->encntr_info_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->encntr_info_qual[xref->chg[index].idx].status != 1))
    SET failed = update_error
    SET table_name = "ENCNTR_INFO"
    GO TO exit_script
   ENDIF
 ENDFOR
 IF (acm_hist_ind=1)
  EXECUTE acm_chg_encntr_info_hist
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM encntr_info e
    WHERE expand(t1,f_val,t_val,e.encntr_info_id,acm_request->encntr_info_qual[xref->chg[t1].idx].
     encntr_info_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,e.encntr_info_id,acm_request->encntr_info_qual[xref->chg[t1].idx].
      encntr_info_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((e.updt_cnt=acm_request->encntr_info_qual[idx].updt_cnt)) OR ((acm_request->force_updt_ind
     =1))) )
      reply->encntr_info_qual[idx].status = - (1), reply->encntr_info_qual[idx].encntr_info_id =
      acm_request->encntr_info_qual[idx].encntr_info_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->encntr_info_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_info_qual[idx].beg_effective_dt_tm = e.beg_effective_dt_tm
     ENDIF
     IF (findstring("CHARTABLE_IND,",chg_str)=0)
      acm_request->encntr_info_qual[idx].chartable_ind = e.chartable_ind
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->encntr_info_qual[idx].contributor_system_cd = e.contributor_system_cd
     ENDIF
     IF (findstring("ENCNTR_ID,",chg_str)=0)
      acm_request->encntr_info_qual[idx].encntr_id = e.encntr_id
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_info_qual[idx].end_effective_dt_tm = e.end_effective_dt_tm
     ENDIF
     IF (findstring("INFO_SUB_TYPE_CD,",chg_str)=0)
      acm_request->encntr_info_qual[idx].info_sub_type_cd = e.info_sub_type_cd
     ENDIF
     IF (findstring("INFO_TYPE_CD,",chg_str)=0)
      acm_request->encntr_info_qual[idx].info_type_cd = e.info_type_cd
     ENDIF
     IF (findstring("INTERNAL_SEQ,",chg_str)=0)
      acm_request->encntr_info_qual[idx].internal_seq = e.internal_seq
     ENDIF
     IF (findstring("LONG_TEXT_ID,",chg_str)=0)
      acm_request->encntr_info_qual[idx].long_text_id = e.long_text_id
     ENDIF
     IF (findstring("PRIORITY_SEQ,",chg_str)=0)
      acm_request->encntr_info_qual[idx].priority_seq = e.priority_seq
     ENDIF
     IF (findstring("VALUE_CD,",chg_str)=0)
      acm_request->encntr_info_qual[idx].value_cd = e.value_cd
     ENDIF
     IF (findstring("VALUE_DT_TM,",chg_str)=0)
      acm_request->encntr_info_qual[idx].value_dt_tm = e.value_dt_tm
     ENDIF
     IF (findstring("VALUE_NUMERIC,",chg_str)=0)
      acm_request->encntr_info_qual[idx].value_numeric = e.value_numeric
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->encntr_info_qual[idx].active_ind = e.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->encntr_info_qual[idx].active_status_cd = e.active_status_cd
     ENDIF
