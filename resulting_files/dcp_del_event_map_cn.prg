CREATE PROGRAM dcp_del_event_map_cn
 ...
 DECLARE scripttocall = vc WITH constant("DCP_DEL_RESULT_TO_EVENT_MAP")
 RECORD req_to_fill(
...
#main_app
 ...
 SUBROUTINE prompt_success(null)
   ...
   CALL clear(1,1)
   CALL video(n)
   CALL text(5,1,"Row(s) added for deletion: ",w)
   CALL text(10,1,">>> Enter 'Q' to quit, 'C' to add another row or 'D' to delete:")
   CALL accept(10,70,"A;cu","Q"
    WHERE curaccept IN ("Q", "C", "D"))
 ...
 SUBROUTINE prompt_failed(null)
   ...
   CALL clear(1,1)
   CALL video(n)
   CALL text(5,1,"FAILED to delete Row from table: ",w)
   CALL text(10,1,">>> Enter 'Q' to quit or 'C' to add another row for deletion:")
 ...
 SUBROUTINE prompt_error_msg(null)
   ...
   CALL clear(1,1)
   CALL video(n)
   CALL text(5,1,"No matching row found for deletion: ",w)
   CALL text(10,1,">>> Enter 'Q' to quit or 'C' to add nother row for deletion:")
   CALL accept(10,70,"A;cu","Q"
    WHERE curaccept IN ("Q", "C"))
   SET choice = curaccept
   SET message = nowindow
 ...
 SUBROUTINE prompt_user_for_corrtype(null)
   ...
   CALL clear(1,1)
   CALL video(n)
   CALL text(2,1,"Delete result from the event map: ",w)
