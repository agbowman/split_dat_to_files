CREATE PROGRAM dm2_create_purge_ddl:dba
 ...
 SUBROUTINE dacr_get_range(i_min,i_max,i_batch_size,i_range_num,io_low,io_high)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dcpd_insert_ops_log_row(io_run_id=f8,io_table_name=vc,io_priority=f8,io_where_clause=vc) =
 i2
 DECLARE dcpd_process_batch(i_table_name=vc,i_driver_table=vc,i_where_clause=vc,i_default_batch_size=
  i4,i_run_id=f8,
  i_priority=f8) = i2
 DECLARE dcpd_gen_default_exclusion_str(i_table_name=vc) = vc
 DECLARE dcpd_run_id = f8 WITH protect, noconstant(0.0)
 DECLARE dcpd_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcpd_ndx = i4 WITH protect, noconstant(0)
 DECLARE dcpd_ndx2 = i4 WITH protect, noconstant(0)
 DECLARE dcpd_where_clause = vc WITH protect, noconstant("")
 DECLARE dcpd_priority = f8 WITH protect, noconstant(0.0)
 DECLARE dcpd_default_batch_size = i4 WITH protect, noconstant(10000)
 DECLARE dcpd_op_id = f8 WITH protect, noconstant(0.0)
 DECLARE dcpd_excl_str = vc WITH protect, noconstant("")
 DECLARE dcpd_extract_loc = i4 WITH protect, noconstant(0)
 DECLARE dcpd_person_table_loc = i4 WITH protect, noconstant(0)
 DECLARE dcpd_idx = i4 WITH protect, noconstant(0)
 IF (check_logfile("create_purge_ddl",".log","dm2_create_purge_ddl")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Creating new dm2_ddl_ops row"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  max_run_id = max(d.run_id)
  FROM dm2_ddl_ops d
  HEAD REPORT
   dcpd_run_id = 0
  DETAIL
   dcpd_run_id = max_run_id
  FOOT REPORT
   dcpd_run_id = (dcpd_run_id+ 1)
  WITH nocounter, nullreport
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 INSERT  FROM dm2_ddl_ops d
  SET d.run_id = dcpd_run_id, d.gen_dt_tm = cnvtdatetime(curdate,curtime3), d.process_option =
   "PURGE SOURCE",
   d.last_checkpoint = "INITIAL"
  WITH nocounter
 ;end insert
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Querying for default Purge batch size"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="ADS DEFAULT BATCH SIZE"
  DETAIL
   dcpd_default_batch_size = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Loop through all tables and generate delete DDL statments"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dcpd_where_clause = concat(
  "create public database link PURGE_TGT connect to v500 identified by ",request->tgt_p_word,
  " using '",request->tgt_connect_str,"' ")
 SELECT INTO "nl:"
  dcd_max_op_id = max(d.op_id)
  FROM dm2_ddl_ops_log d
  HEAD REPORT
   dcpd_op_id = 0
  DETAIL
   dcpd_op_id = dcd_max_op_id
  FOOT REPORT
   dcpd_op_id = (dcpd_op_id+ 1)
  WITH nocounter, nullreport
 ;end select
 IF (check_error("Selecting max op_id from dm2_ddl_ops_log.")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 INSERT  FROM dm2_ddl_ops_log d
  SET d.op_id = dcpd_op_id, d.run_id = dcpd_run_id, d.begin_dt_tm = cnvtdatetime(null),
   d.end_dt_tm = cnvtdatetime(null), d.gen_dt_tm = cnvtdatetime(curdate,curtime3), d.gen_id = request
   ->config_id,
   d.op_type = "PURGE ACTIVITY DATA", d.priority = 1000, d.status = "INCOMPLETE",
   d.error_msg = null, d.obj_name = "DB LINK", d.table_name = "DB LINK",
   d.operation = dcpd_where_clause, d.ready_to_run_ind = 0, d.updt_dt_tm = cnvtdatetime(curdate,
    curtime3)
  WITH nocounter
 ;end insert
 IF (check_error("Inserting into dm2_ddl_ops_log.")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dcpd_person_table_loc = locateval(dcpd_idx,1,request->tbl_cnt,"PERSON",request->tbl[dcpd_idx].
  table_name)
 SET dcpd_extract_loc = locateval(dcpd_idx,1,request->tbl[dcpd_person_table_loc].extract_cnt,1,
  request->tbl[dcpd_person_table_loc].ext[dcpd_idx].driver_table_ind)
 SET dcpd_where_clause = concat("delete from person<<TGTDBLINK>> where person_id > 0 ",
  " and (name_last_key not in ('SYSTEM', 'SYSTEMOE','CERNER') OR name_last_key is NULL) ",
  " and person_id not in (select driver_key_id from dm_ads_config_driver where ",
  " dm_ads_extract_id = ",trim(cnvtstring(request->tbl[dcpd_person_table_loc].ext[dcpd_extract_loc].
    extract_id)),
  " and dm_ads_config_id = ",trim(cnvtstring(request->config_id)),") ")
 SET dcpd_where_clause = concat(dcpd_where_clause,
  " AND EXISTS (select 1 from dm2_ddl_ops<<TGTDBLINK>> WHERE ",
  "process_option =  'PURGE TARGET' and last_checkpoint = '",trim(cnvtstring(request->config_id)),
  "') ")
 SELECT INTO "nl:"
  dcd_max_op_id = max(d.op_id)
  FROM dm2_ddl_ops_log d
 ...
 INSERT  FROM dm2_ddl_ops_log d
  SET d.op_id = dcpd_op_id, d.run_id = dcpd_run_id, d.begin_dt_tm = cnvtdatetime(null),
   d.end_dt_tm = cnvtdatetime(null), d.gen_dt_tm = cnvtdatetime(curdate,curtime3), d.gen_id = request
   ->config_id,
   d.op_type = "PURGE ACTIVITY DATA", d.priority = 1001, d.status = "INCOMPLETE",
   d.error_msg = null, d.obj_name = "TARGET PRSNL PRE-DELETE", d.table_name = "PERSON",
   d.operation = dcpd_where_clause, d.ready_to_run_ind = 0, d.updt_dt_tm = cnvtdatetime(curdate,
    curtime3)
  WITH nocounter
 ;end insert
 IF (check_error("Inserting into dm2_ddl_ops_log.")=1)
  ...
  GO TO exit_program
 ENDIF
 FOR (dcpd_cnt = 1 TO request->tbl_cnt)
   IF ((request->tbl[dcpd_cnt].move_all=0)
    AND (request->tbl[dcpd_cnt].nomove=0)
    AND locateval(dcpd_ndx,1,request->tbl[dcpd_cnt].extract_cnt,"BYCONFIG",request->tbl[dcpd_cnt].
    ext[dcpd_ndx].extract_method)=1)
    IF (locateval(dcpd_ndx2,1,request->tbl[dcpd_cnt].extract_cnt,1,request->tbl[dcpd_cnt].ext[
     dcpd_ndx2].driver_table_ind) > 0)
     SET dcpd_priority = 2
    ELSE
     SET dcpd_priority = 1
    ENDIF
    SET dcpd_excl_str = " "
    SET dcpd_excl_str = dcpd_gen_default_exclusion_str(request->tbl[dcpd_cnt].table_name)
    IF (check_error(concat("Generating default row exclusion string for table ",request->tbl[dcpd_cnt
      ].table_name,"."))=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    IF ((request->tbl[dcpd_cnt].pk_col_cnt=1))
     IF ((request->tbl[dcpd_cnt].extract_cnt=1))
      IF (findstring("<<BATCH",request->tbl[dcpd_cnt].ext[1].purge_where_clause) > 0)
       SET dcpd_where_clause = cnvtupper(concat("delete from ",request->tbl[dcpd_cnt].table_name,
         " where ",request->tbl[dcpd_cnt].ext[1].purge_where_clause," "))
       IF (dcpd_process_batch(request->tbl[dcpd_cnt].table_name,request->tbl[dcpd_cnt].ext[1].
        driver_table_name,dcpd_where_clause,dcpd_default_batch_size,dcpd_run_id,
        dcpd_priority)=0)
        GO TO exit_program
       ENDIF
      ELSE
       SET dcpd_where_clause = concat("delete from ",request->tbl[dcpd_cnt].table_name," where ",
        request->tbl[dcpd_cnt].pk_col[1].col_name," in (select ",
        request->tbl[dcpd_cnt].pk_col[1].col_name," from ",request->tbl[dcpd_cnt].table_name,
        "<<TGTDBLINK>> where 1=1 ",dcpd_excl_str,
        ")")
       IF (dcpd_insert_ops_log_row(dcpd_run_id,request->tbl[dcpd_cnt].table_name,dcpd_priority,
        dcpd_where_clause)=0)
        GO TO exit_program
       ENDIF
      ENDIF
     ELSE
      FOR (dcpd_ndx = 1 TO request->tbl[dcpd_cnt].extract_cnt)
        IF (findstring("<<BATCH",request->tbl[dcpd_cnt].ext[dcpd_ndx].purge_where_clause) > 0)
         SET dcpd_priority = (dcpd_priority+ 1)
         SET dcpd_where_clause = cnvtupper(concat("delete from ",request->tbl[dcpd_cnt].table_name,
           " where ",request->tbl[dcpd_cnt].ext[dcpd_ndx].purge_where_clause," "))
         IF (dcpd_process_batch(request->tbl[dcpd_cnt].table_name,request->tbl[dcpd_cnt].ext[dcpd_ndx
          ].driver_table_name,dcpd_where_clause,dcpd_default_batch_size,dcpd_run_id,
          dcpd_priority)=0)
          GO TO exit_program
         ENDIF
      ...
      SET dcpd_where_clause = concat("delete from ",request->tbl[dcpd_cnt].table_name," where ",
       ...
       "<<TGTDBLINK>> where 1=1",dcpd_excl_str,
       ")")
      FOR (dcpd_ndx2 = 1 TO request->tbl[dcpd_cnt].extract_cnt)
        IF ((request->tbl[dcpd_cnt].ext[dcpd_ndx2].data_class_type="REFERENCE-MIXED"))
         SET dcpd_where_clause = concat(dcpd_where_clause," and not (",request->tbl[dcpd_cnt].ext[
          dcpd_ndx2].purge_where_clause,")")
        ENDIF
      ENDFOR
      SET dcpd_where_clause = cnvtupper(concat(dcpd_where_clause," "))
      IF (dcpd_insert_ops_log_row(dcpd_run_id,request->tbl[dcpd_cnt].table_name,dcpd_priority,
       dcpd_where_clause)=0)
       GO TO exit_program
      ENDIF
    ...
    ELSE
     FOR (dcpd_ndx = 1 TO request->tbl[dcpd_cnt].extract_cnt)
       IF (cnvtupper(request->tbl[dcpd_cnt].ext[dcpd_ndx].data_class_type)=patstring("ACTIVITY*"))
        IF (findstring("<<BATCH",request->tbl[dcpd_cnt].ext[dcpd_ndx].purge_where_clause) > 0)
         SET dcpd_where_clause = cnvtupper(concat("delete from ",request->tbl[dcpd_cnt].table_name,
           " where ",request->tbl[dcpd_cnt].ext[dcpd_ndx].purge_where_clause," "))
         IF (dcpd_process_batch(request->tbl[dcpd_cnt].table_name,request->tbl[dcpd_cnt].ext[dcpd_ndx
   ...
   ENDIF
 ENDFOR
 SUBROUTINE dcpd_insert_ops_log_row(io_run_id,io_table_name,io_priority,io_where_clause)
   DECLARE dgnoi_op_id = f8 WITH protect, noconstant(0.0)
   SELECT INTO "nl:"
    dcd_max_op_id = max(d.op_id)
    FROM dm2_ddl_ops_log d
    HEAD REPORT
     dgnoi_op_id = 0
    DETAIL
     dgnoi_op_id = dcd_max_op_id
    FOOT REPORT
     dgnoi_op_id = (dgnoi_op_id+ 1)
    WITH nocounter, nullreport
   ;end select
   IF (check_error("Selecting max op_id from dm2_ddl_ops_log.")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET io_where_clause = replace(io_where_clause,"@REPL_TGT","<<TGTDBLINK>>")
   SET io_where_clause = concat(io_where_clause,
    " AND EXISTS (select 1 from dm2_ddl_ops<<TGTDBLINK>> WHERE ",
    "process_option =  'PURGE TARGET' and last_checkpoint = '",trim(cnvtstring(request->config_id)),
    "') ")
   INSERT  FROM dm2_ddl_ops_log d
    SET d.op_id = dgnoi_op_id, d.run_id = io_run_id, d.begin_dt_tm = cnvtdatetime(null),
     d.end_dt_tm = cnvtdatetime(null), d.gen_dt_tm = cnvtdatetime(curdate,curtime3), d.gen_id =
     request->config_id,
     d.op_type = "PURGE ACTIVITY DATA", d.table_name = io_table_name, d.obj_name = io_table_name,
     d.priority = (io_priority+ 1001), d.status = "INCOMPLETE", d.error_msg = null,
     d.operation = io_where_clause, d.ready_to_run_ind = 0, d.updt_dt_tm = cnvtdatetime(curdate,
      curtime3)
    WITH nocounter
   ;end insert
   IF (check_error("Inserting into dm2_ddl_ops_log.")=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcpd_process_batch(i_table_name,i_driver_table,i_where_clause,i_default_batch_size,
  i_run_id,i_priority)
   DECLARE dpb_start_tag = i2 WITH protect, noconstant(0)
   DECLARE dpb_end_tag = i2 WITH protect, noconstant(0)
   DECLARE dpb_replace_batch_str = vc WITH protect, noconstant("")
   DECLARE dpb_driver_tbl_idx = i2 WITH protect, noconstant(0)
   DECLARE dpb_batch_size = i4 WITH protect, noconstant(0)
   DECLARE dpb_batch_where = vc WITH protect, noconstant("")
   DECLARE dpb_batch_cnt = i4 WITH protect, noconstant(0)
   SET dpb_start_tag = findstring("<<BATCH<>",i_where_clause)
   IF (dpb_start_tag > 0)
    SET dpb_end_tag = findstring(">>",i_where_clause,dpb_start_tag)
    SET dpb_replace_batch_str = substring(dpb_start_tag,((dpb_end_tag+ 2) - dpb_start_tag),
     i_where_clause)
    SET dpb_start_tag = (dpb_start_tag+ 9)
    SET dpb_batch_size = cnvtint(substring(dpb_start_tag,(dpb_end_tag - dpb_start_tag),i_where_clause
      ))
   ELSE
    SET dpb_batch_size = i_default_batch_size
    SET dpb_replace_batch_str = "<<BATCH>>"
   ENDIF
   SET dpb_driver_tbl_idx = dacr_get_driver_table_ranges(i_driver_table,request->config_id,
    dpb_batch_size)
   IF ((dpb_driver_tbl_idx=- (1)))
    SET dm_err->err_ind = 1
    RETURN(0)
   ENDIF
   FOR (dpb_batch_cnt = 1 TO dacr_driver_info->driver[dpb_driver_tbl_idx].range_cnt)
    SET dpb_batch_where = replace(i_where_clause,dpb_replace_batch_str,concat(
      "SELECT DRIVER_KEY_ID FROM DM_ADS_CONFIG_DRIVER WHERE ","DM_ADS_CONFIG_ID = ",build(request->
       config_id)," AND DM_ADS_EXTRACT_ID = ",build(dacr_driver_info->driver[dpb_driver_tbl_idx].
       extract_id),
      " AND DM_ADS_CONFIG_DRIVER_ID BETWEEN ",build(dacr_driver_info->driver[dpb_driver_tbl_idx].
       ranges[dpb_batch_cnt].lower_bound)," and ",build(dacr_driver_info->driver[dpb_driver_tbl_idx].
       ranges[dpb_batch_cnt].upper_bound)))
    IF (dcpd_insert_ops_log_row(i_run_id,i_table_name,i_priority,dpb_batch_where)=0)
     RETURN(0)
    ENDIF
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcpd_gen_default_exclusion_str(i_table_name)
   DECLARE dgdes_excl_str = vc WITH protect, noconstant(" ")
   DECLARE dgdes_temp_str = vc WITH protect, noconstant(" ")
   SELECT INTO "nl:"
    FROM user_triggers u
    WHERE u.table_name=i_table_name
     AND u.trigger_name="*DR_UPDT_DEL*"
    DETAIL
     dgdes_temp_str = replace(cnvtupper(u.when_clause),"OLD.","",0), dgdes_excl_str = concat(
      " and not (",dgdes_temp_str,") ")
    WITH nocounter
   ;end select
   RETURN(dgdes_excl_str)
 END ;Subroutine
#exit_program
END GO
