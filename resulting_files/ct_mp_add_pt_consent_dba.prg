CREATE PROGRAM ct_mp_add_pt_consent:dba
 PROMPT
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "ct_mp_add_pt_consent"
 ...
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_add_pt_consent","Invalid JSON Request",
   "reply")
  GO TO exit_script
 ENDIF
 ...
 ;end select
 IF (time_field_ind != 2)
  CALL echo("insert into the pt_consent table without time ind")
  CALL echo(request->amendment_id)
  INSERT  FROM pt_consent pc
   SET pc.pt_consent_id = conid, pc.consent_id = conid, pc.ct_document_version_id =
    ...
    ENDIF
    , pc.reason_for_consent_cd = request->reason_for_consent_cd, pc.person_id = request->person_id,
    pc.prot_amendment_id = request->amendment_id, pc.not_returned_reason_cd = request->
    not_returned_reason_cd, pc.updt_dt_tm = cnvtdatetime(sysdate),
    pc.beg_effective_dt_tm = cnvtdatetime(sysdate), pc.end_effective_dt_tm = cnvtdatetime(
     "31-Dec-2100 00:00:00.00"), pc.updt_id = reqinfo->updt_id,
    pc.updt_task = reqinfo->updt_task, pc.updt_applctx = reqinfo->updt_applctx, pc.updt_cnt = 0
   WITH nocounter
  ;end insert
 ELSE
  CALL echo("insert into the pt_consent table with time ind")
  CALL echo(request->amendment_id)
  INSERT  FROM pt_consent pc
   SET pc.pt_consent_id = conid, pc.consent_id = conid, pc.ct_document_version_id =
    IF ((request->ct_document_version_id=0)) 0
    ...
    ELSE cnvtdatetime(request->not_returned_dt_tm)
    ENDIF
    , pc.not_returned_tm_ind = request->not_returned_tm_ind, pc.consent_released_tm_ind = request->
    consent_released_tm_ind,
    pc.consent_signed_tm_ind = request->consent_signed_tm_ind, pc.consent_received_tm_ind = request->
    consent_received_tm_ind, pc.reason_for_consent_cd = request->reason_for_consent_cd,
 ...
 IF (time_field_ind=2)
  IF ((request->consent_released_dt_tm != "0"))
   SET qual_count += 1
   SET stat = alterlist(audit_events->qual,qual_count)
   SET audit_events->qual[qual_count].eventname = "Consent_Release_Date-Time"
   SET audit_events->qual[qual_count].eventtype = "Add"
  ENDIF
  IF ((request->consent_signed_dt_tm != "0"))
   SET qual_count += 1
   SET stat = alterlist(audit_events->qual,qual_count)
   SET audit_events->qual[qual_count].eventname = "Consent_Signed_Date-Time"
   SET audit_events->qual[qual_count].eventtype = "Add"
  ENDIF
  IF ((request->consent_received_dt_tm != "0"))
   SET qual_count += 1
   SET stat = alterlist(audit_events->qual,qual_count)
   SET audit_events->qual[qual_count].eventname = "Consent_Returned_Date-Time"
   SET audit_events->qual[qual_count].eventtype = "Add"
  ENDIF
  IF ((request->not_returned_dt_tm != "0"))
   SET qual_count += 1
   SET stat = alterlist(audit_events->qual,qual_count)
   SET audit_events->qual[qual_count].eventname = "Con_Not_Returned_Dt-Tm"
   SET audit_events->qual[qual_count].eventtype = "Add"
  ENDIF
 ENDIF
 IF (curqual=1)
 ...
 IF (doinsert=1)
  ...
  FOR (x = 1 TO qual_count)
    EXECUTE cclaudit audit_mode, audit_events->qual[x].eventname, audit_events->qual[x].eventtype,
    "Person", "Patient", "Patient",
    "Origination", request->person_id, ""
  ENDFOR
  COMMIT
