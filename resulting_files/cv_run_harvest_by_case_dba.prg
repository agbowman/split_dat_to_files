CREATE PROGRAM cv_run_harvest_by_case:dba
 ...
 ENDIF
 DECLARE rec_cnt = i4 WITH public, noconstant(0)
 SELECT INTO "NL:"
  ...
  HEAD REPORT
   rec_nct = 0
  DETAIL
   rec_cnt = (rec_cnt+ 1), stat = alterlist(cv_hrv_rec->harvest_rec,rec_cnt), cv_hrv_rec->
   harvest_rec[rec_cnt].dataset_id = ccdr.dataset_id,
   cv_hrv_rec->harvest_rec[rec_cnt].case_id = ccdr.cv_case_id
  WITH nocounter
 ;end select
 IF (curqual=0)
  CALL echo("Failed in getting dataset_id!")
 ENDIF
 IF (size(cv_hrv_rec->harvest_rec,5) > 0)
  UPDATE  FROM cv_case_dataset_r ccdr,
    (dummyt d  WITH seq = value(size(cv_hrv_rec->harvest_rec,5)))
   SET ccdr.status_cd = 0
   PLAN (d)
    JOIN (ccdr
    WHERE (ccdr.cv_case_id=cv_hrv_rec->harvest_rec[d.seq].case_id))
   WITH nocounter
  ;end update
  IF (curqual=0)
   CALL echo("Failed in updating status_cd in cv_case_dataset_r!")
  ENDIF
 ENDIF
 CALL echo(build("Dataset_ID: ",cv_hrv_rec->harvest_rec[1].dataset_id))
 CALL echo(build("CASE_ID: ",cv_hrv_rec->harvest_rec[1].case_id))
