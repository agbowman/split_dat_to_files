CREATE PROGRAM dm2_auto_install_status:dba
 ...
 DECLARE dais_validate_request(null) = i2
 DECLARE dais_get_cur_install_mode(null) = i2
 DECLARE dais_get_plan_event_status(null) = i2
 DECLARE dais_get_cur_itinerary(null) = i2
 DECLARE dais_get_event_status(null) = i2
 DECLARE dais_retrieve_dblink(null) = i2
 DECLARE dais_populate_subeventstatus(dps_operationname=vc,dps_targetobjectvalue=vc,
  dps_targetobjectname=vc) = null
 DECLARE dais_plan_id = f8 WITH protect, constant(request->plan_id)
 DECLARE dais_install_mode = vc WITH protect, constant(cnvtupper(trim(request->install_mode)))
 DECLARE dais_process_event_id = f8 WITH protect, noconstant(0.0)
 DECLARE dais_event_status = vc WITH protect, noconstant("")
 DECLARE dais_msg_text = vc WITH protect, noconstant("")
 DECLARE dais_itinerary_event = vc WITH protect, noconstant("")
 DECLARE dais_targetobjectname = vc WITH protect, noconstant("")
 DECLARE dais_operationname = vc WITH protect, noconstant("")
 DECLARE dais_dblink = vc WITH protect, noconstant("")
 DECLARE dais_inactive_ind = i2 WITH protect, noconstant(0)
 SET reply->status_data.status = "F"
 IF (check_logfile("dm2_auto_install_status",".log","dm2_auto_install_status")=0)
  SET dais_targetobjectname = "check_logfile"
  SET dais_operationname = "LOG FILE CREATION"
  CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_auto_install_status"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET reply->install_status = "DM2NOTSET"
 IF (dais_validate_request(null)=0)
  GO TO exit_program
 ENDIF
 IF (dais_retrieve_dblink(null)=0)
  GO TO exit_program
 ENDIF
 IF (drr_cleanup_dm_info_runners(null)=0)
  SET dais_targetobjectname = "drr_cleanup_dm_info_runners"
  SET dais_operationname = "Cleaning invalid runners from dm_info"
  CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
  GO TO exit_program
 ENDIF
 IF (dais_get_cur_install_mode(null)=0)
  GO TO exit_program
 ELSE
  IF ((reply->install_status="DM2NOTSET"))
   IF (dais_get_plan_event_status(null)=0)
    GO TO exit_program
   ELSE
    IF ((reply->install_status="DM2NOTSET"))
     IF (dais_get_cur_itinerary(null)=0)
      GO TO exit_program
     ELSE
      IF ((reply->install_status="DM2NOTSET"))
       IF (dais_set_event_status(null)=0)
        GO TO exit_program
       ENDIF
      ENDIF
     ENDIF
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE dais_validate_request(null)
   SET dm_err->eproc = "Validating user input"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dais_plan_id <= 0)
    SET dm_err->eproc = "Validating the passed Plan ID"
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Invalid Plan ID has been encountered"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dais_targetobjectname = "dais_validate_request"
    SET dais_operationname = "Validating Plan_ID"
    CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Validating plan_id"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_install_plan dip
    WHERE dip.install_plan_id=dais_plan_id
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dais_operationname = "Querying dm_install_plan"
    SET dais_targetobjectname = "dais_validate_request"
    CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
    RETURN(0)
   ENDIF
   IF (curqual=0)
    SET dm_err->eproc = "Checking the existence of passed install plan"
    SET dm_err->err_ind = 1
    SET dm_err->emsg = build2("Install Plan ID ",dais_plan_id," not found on DM_INSTALL_PLAN.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dais_targetobjectname = "dais_validate_request"
    SET dais_operationname = "Check Plan_ID exist"
    CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Validating install_mode"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ( NOT (dais_install_mode IN ("BATCHUP", "BATCHDOWN", "BATCHPOST", "BATCHPRECYCLE", "CURRENT")))
    SET dm_err->eproc = "Validating the install mode"
    SET dm_err->err_ind = 1
    SET dm_err->emsg = build2(dais_install_mode," is invalid install mode.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dais_targetobjectname = "dais_validate_request"
    SET dais_operationname = "Validating install_mode"
    CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dais_retrieve_dblink(null)
   DECLARE drd_fail_ind = i2 WITH protect, noconstant(0)
   IF (dir_get_admin_db_link(1,dais_dblink,drd_fail_ind)=0)
    SET dm_err->emsg = "Invalid admin DB link"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dais_targetobjectname = "dais_retrieve_dblink"
    SET dais_operationname = "Validating admin DB link"
    CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
    RETURN(0)
   ENDIF
   IF (drd_fail_ind=1)
    SET dm_err->eproc = "To check if the admin DBLink is unknown in dm2_auto_install_status"
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Unknown admin DBLink"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dais_targetobjectname = "dais_retrieve_dblink"
    SET dais_operationname = "Validating admin DBLink"
    CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dais_get_cur_install_mode(null)
   DECLARE dgcim_cur_dpe_set = i2 WITH protect, noconstant(0)
   SET dipm_misc_data->cur_mode = "DM2NOTSET"
   SET dm_err->eproc = "Get current install mode"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_process dp,
     dm_process_event dpe,
     dm_process_event_dtl dped
    PLAN (dpe
     WHERE dpe.install_plan_id=dais_plan_id)
     JOIN (dp
     WHERE dpe.dm_process_id=dp.dm_process_id
      AND dp.process_name=value(dpl_package_install)
      AND dp.action_type=value(dpl_execution)
    ...
    HEAD REPORT
     dgcim_cur_dpe_set = 0
    DETAIL
     IF (dgcim_cur_dpe_set=0)
      IF (cnvtupper(trim(dped.detail_text)) != "BATCHPREVIEW")
       dgcim_cur_dpe_set = 1
      ENDIF
      IF (cnvtupper(dipm_misc_data->cur_mode) != cnvtupper(trim(dped.detail_text)))
       dipm_misc_data->cur_dpe_id = dpe.dm_process_event_id, dipm_misc_data->cur_mode = cnvtupper(
        trim(dped.detail_text))
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF (curqual=0)
    SET reply->message = build2("No package install has been initiated for the",dais_plan_id)
    SET reply->install_status = "NONE"
   ELSE
    IF (dais_install_mode != "CURRENT"
     AND (dais_install_mode != dipm_misc_data->cur_mode))
     SET dm_err->eproc = "Get latest instance of passed install mode"
     CALL disp_msg(" ",dm_err->logfile,0)
     SELECT INTO "nl:"
      FROM dm_process dp,
       dm_process_event dpe,
      ...
      PLAN (dpe
       ...
       WHERE dpe.dm_process_event_id=dped.dm_process_event_id
        AND dped.detail_type=value(dpl_install_mode)
        AND cnvtupper(trim(dped.detail_text))=dais_install_mode)
      ORDER BY dpe.begin_dt_tm DESC
      DETAIL
       dais_process_event_id = dpe.dm_process_event_id
      WITH nocounter, maxrec = 1
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      SET dais_operationname = "Querying dm_process"
      SET dais_targetobjectname = "dais_get_cur_install_mode"
      CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
      RETURN(0)
     ENDIF
     IF (curqual=0)
      SET reply->message = build2("No event information found for ",dais_install_mode,
       " install mode and plan id",dais_plan_id)
      SET reply->install_status = "NONE"
     ENDIF
    ELSE
     SET dais_process_event_id = dipm_misc_data->cur_dpe_id
    ENDIF
   ENDIF
   IF (dais_install_mode="CURRENT")
    SET reply->install_mode_ret = dipm_misc_data->cur_mode
   ELSE
    SET reply->install_mode_ret = dais_install_mode
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dais_get_plan_event_status(null)
   SET dm_err->eproc = "Retrieve the current plan event status"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_process_event dpe,
     dm_process_event_dtl dped
    PLAN (dpe
     WHERE dpe.dm_process_event_id=dais_process_event_id)
     JOIN (dped
     WHERE dpe.dm_process_event_id=dped.dm_process_event_id)
    DETAIL
     dais_event_status = dpe.event_status
     IF (cnvtupper(dpe.message_txt)="*ACTIVE STATUS FOUND WITHOUT ACTIVE EVENT PROCESS*")
      dais_inactive_ind = 1
     ENDIF
     IF (dped.detail_type="LAST_STATUS_MESSAGE")
      dais_msg_text = dped.detail_text
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dais_operationname = "Querying dm_process_event"
    SET dais_targetobjectname = "dais_get_plan_event_status"
    CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
    RETURN(0)
   ENDIF
   IF (size(dais_msg_text)=0)
    SET dais_msg_text = "Initializing..."
   ENDIF
   IF (curqual=0)
    SET reply->message = build2("No event status information found for ",reply->install_mode_ret,
     " install mode and plan id",dais_plan_id)
    SET reply->install_status = "NONE"
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dais_get_cur_itinerary(null)
   SET dm_err->eproc = "Retrieve the current itinenary step"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_process_event dpe2,
     ...
     dm_process_event_dtl dped2
    PLAN (dpe2
     WHERE dpe2.install_plan_id=dais_plan_id)
     JOIN (dp2
    ...
    DETAIL
     dais_itinerary_event = dped2.detail_text
    WITH nocounter, maxrec = 1
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dais_operationname = "Querying dm_process_event"
    SET dais_targetobjectname = "dais_get_cur_itinerary"
    CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
    RETURN(0)
   ENDIF
   IF (curqual=0)
    SET reply->message = build2("No itinerary information found for ",reply->install_mode_ret,
     " install mode and plan id",dais_plan_id)
    SET reply->install_status = "NONE"
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dais_set_event_status(null)
   DECLARE dses_status = vc WITH protect, noconstant("")
   DECLARE dses_runner_ind = i2 WITH protect, noconstant(0)
   SET dses_status = dais_event_status
   SET dm_err->eproc = "Set the status of the install plan"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dses_status="FAILED")
    IF (dais_inactive_ind=1)
     IF (dais_install_mode != "CURRENT"
      AND (dais_install_mode != dipm_misc_data->cur_mode))
      SET dses_status = "FAILED"
     ELSE
      IF (dais_itinerary_event IN ("Setup", "Pre-Schema Readmes", "Post-Schema Readmes", "Readmes",
      "Schema"))
       IF (drr_chk_active_runners(dais_dblink,dses_runner_ind)=0)
        SET dais_targetobjectname = "dais_set_event_status"
        SET dais_operationname = "Check for active runners"
        CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
        RETURN(0)
       ELSE
        IF (dses_runner_ind=1)
         SET dses_status = "EXECUTING"
         SET dais_msg_text = build2("Main install abnormally ended, but ",dais_itinerary_event,
          " background process still executing")
        ELSE
         SET dses_status = "RESTART"
         SET dais_msg_text = "Need to restart package installation due to abrupt ending"
        ENDIF
       ENDIF
      ELSE
       SET dses_status = "RESTART"
     ...
     ENDIF
    ENDIF
   ELSEIF ( NOT (dses_status IN ("EXECUTING", "COMPLETE")))
    SET dm_err->eproc = "Validating the auto install event status"
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Invalid installation event status"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dais_operationname = "Error while getting the auto install status."
    SET dais_targetobjectname = "dais_set_event_status"
    CALL dais_populate_subeventstatus(dais_operationname,dm_err->emsg,dais_targetobjectname)
    RETURN(0)
   ENDIF
   SET reply->event = dais_itinerary_event
   SET reply->message = dais_msg_text
   SET reply->install_status = dses_status
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dais_populate_subeventstatus(dps_operationname,dps_targetobjectvalue,dps_targetobjectname
  )
   SET reply->status_data.subeventstatus[1].operationname = dps_operationname
   SET reply->status_data.subeventstatus[1].operationstatus = "F"
   SET reply->status_data.subeventstatus[1].targetobjectname = dps_targetobjectname
   SET reply->status_data.subeventstatus[1].targetobjectvalue = dps_targetobjectvalue
 END ;Subroutine
#exit_program
 IF ((dm_err->debug_flag >= 2))
 ...
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = concat("AUTO_INSTALL_STATUS_RESULTS: ","Plan: ",trim(cnvtstring(dais_plan_id)),
   "; Input Mode: ",dais_install_mode,
   "; Return Mode: ",reply->install_mode_ret,"; Return Event: ",reply->event,"; Return Status: ",
   reply->install_status,"; Message: ",reply->message)
  CALL disp_msg(" ",dm_err->logfile,0)
  SET reply->status_data.status = "S"
  SET dm_err->eproc = "dm2_auto_install_status completed"
 ENDIF
 CALL final_disp_msg("dm2_auto_install_status")
END GO
