CREATE PROGRAM cva_chk_for_dup_outbound:dba
 RECORD reply(
   1 qual[*]
     2 alias = vc
 ...
 SELECT INTO "nl:"
  ...
  FROM (dummyt d  WITH seq = value(num_als)),
   code_value_outbound a
  PLAN (d)
   JOIN (a
   WHERE (request->contributor_source_cd=a.contributor_source_cd)
    AND (request->qual[d.seq].code_value=a.code_value))
  DETAIL
