CREATE PROGRAM bhs_ma_nut_stats_v2:dba
 PROMPT
  ...
  fname, nunit, email
 IF (datetimediff(cnvtdatetime(cnvtdate( $EDATE),0000),cnvtdatetime(cnvtdate( $BDATE),2359),1) > 365)
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
  ...
  ;end select
  GO TO exit_prg
 ELSEIF ((datetimediff(cnvtdatetime(cnvtdate( $EDATE),0000),cnvtdatetime(cnvtdate( $BDATE),2359),1)
  < - (1)))
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
 ...
 IF (findstring("@", $EMAIL) > 0)
  SET email_ind = 1
  SET var_output = "bhs_diet_stats.csv"
 ELSE
  SET email_ind = 0
 ENDIF
 ...
 SELECT
  IF (any_status_ind="C")
   PLAN (oa
    WHERE oa.action_dt_tm BETWEEN cnvtdatetime(cnvtdate( $BDATE),0) AND cnvtdatetime(cnvtdate( $EDATE
      ),235959)
     AND ((oa.action_type_cd+ 0)=order_var))
    JOIN (o
    WHERE o.order_id=oa.order_id
     AND ((o.catalog_cd+ 0) IN (nsconsult_var, parnut_var, tubefeed_var, oralsupp_var, assessadd_var,
  ...
  o.hna_order_mnemonic, sort_order = evaluate(o.catalog_cd,1845632.00,1.0,1289389.00,2.0,
   ...
   5.0,792551.00,7.0,71524750.00,9.0,
   10.0), od.oe_field_display_value,
  ...
  ORDER BY o.catalog_cd, o.order_id, od.order_id,
   od.action_sequence, od.detail_sequence, 0
  HEAD REPORT
  ...
  WITH nocounter, outerjoin = d3, time = 360
 ;end select
 SELECT
  IF (any_status_ind="C")
   PLAN (ce
    WHERE ce.valid_from_dt_tm BETWEEN cnvtdatetime(cnvtdate( $BDATE),0) AND cnvtdatetime(cnvtdate(
       $EDATE),235959)
     AND ce.clinsig_updt_dt_tm BETWEEN cnvtdatetime(cnvtdate( $BDATE),0) AND cnvtdatetime(cnvtdate(
       $EDATE),235959)
     AND ce.catalog_cd IN (906086.00)
 ...
 IF (email_ind=1)
  ...
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
    msg1 = concat(trim("bhs_diet_stats_"),format(curdate,"MMDDYYYY;;D"),".csv will be sent to -"),
