CREATE PROGRAM ccps_cs_susp_chrg_org:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Organization Name" = 0,
  "Activity Type" = 0
  WITH outdev, org_name, activity_type
 ...
 DECLARE posting_chrg_mod_type_cd = f8 WITH constant(uar_get_code_by("MEANING",13030,"POSTING")),
 protect
 DECLARE unknown_reason_cd = f8 WITH constant(uar_get_code_by("MEANING",4001910,"UNKNOWN")), protect
 ...
 ELSE
  SET activity_type_parser = getpromptlist(3,"c.activity_type_cd")
 ENDIF
 FREE RECORD suspended_charges
 ...
 SELECT INTO "nl:"
  suspense_reason_sort =
  IF (cm.field1_id=posting_chrg_mod_type_cd) trim(substring(1,200,uar_get_code_display(cm.field3_id)),
    3)
  ELSE trim(substring(1,200,uar_get_code_display(cm.field1_id)),3)
  ENDIF
  FROM charge c,
   charge_mod cm,
   encounter e,
  ...
  PLAN (c
   WHERE c.process_flg=1
    AND c.active_ind=1
    AND c.interface_file_id >= 0.00
    AND c.beg_effective_dt_tm != null
  ...
  ORDER BY o.organization_id, suspense_reason_sort, c.encntr_id,
   c.charge_item_id, eafin.end_effective_dt_tm DESC, eafin2.end_effective_dt_tm DESC
  HEAD REPORT
   sc_cnt = 0
  HEAD o.organization_id
   sc_cnt = (sc_cnt+ 1)
   IF (mod(sc_cnt,10)=1)
    stat = alterlist(suspended_charges->sc_detail,(sc_cnt+ 9))
   ENDIF
   suspended_charges->sc_detail[sc_cnt].org_name = o.org_name, suspended_charges->sc_detail[sc_cnt].
   org_id = o.organization_id, sr_cnt = 0
  HEAD suspense_reason_sort
   sr_cnt = (sr_cnt+ 1)
   IF (mod(sr_cnt,10)=1)
    stat = alterlist(suspended_charges->sc_detail[sc_cnt].sr_detail,(sr_cnt+ 9))
   ENDIF
   IF (cm.field1_id=posting_chrg_mod_type_cd)
    IF (cm.field3_id=unknown_reason_cd
     AND textlen(trim(cm.field3,3)) > 0)
     suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].susp_reason = cm.field3,
     suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].susp_reason_cd = cm.field3_id
    ELSE
     suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].susp_reason = uar_get_code_display(cm
      .field3_id), suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].susp_reason_cd = cm
     .field3_id
    ENDIF
   ELSE
    suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].susp_reason = uar_get_code_display(cm
     .field1_id), suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].susp_reason_cd = cm
    .field1_id
   ENDIF
   ei_cnt = 0
  HEAD c.encntr_id
   ei_cnt = (ei_cnt+ 1)
   IF (mod(ei_cnt,10)=1)
    stat = alterlist(suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail,(ei_cnt+ 9))
   ENDIF
   suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].patient_name = p
   .name_full_formatted
   IF (textlen(trim(eafin.alias,3)) > 0)
    suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].fin = cnvtalias(eafin
     .alias,eafin.alias_pool_cd)
   ELSE
    suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].fin = cnvtalias(eafin2
     .alias,eafin2.alias_pool_cd)
   ENDIF
   suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].name_last_key = p
   .name_last_key, suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].
   name_first_key = p.name_first_key, suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].
   ei_detail[ei_cnt].encounter_id = c.encntr_id,
   suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].person_id = c.person_id,
   ci_cnt = 0
  HEAD c.charge_item_id
   ci_cnt = (ci_cnt+ 1)
   IF (mod(ci_cnt,200)=1)
    stat = alterlist(suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].
     ci_detail,(ci_cnt+ 199))
   ENDIF
   suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].ci_detail[ci_cnt].
   activity_type = uar_get_code_display(c.activity_type_cd), suspended_charges->sc_detail[sc_cnt].
   sr_detail[sr_cnt].ei_detail[ei_cnt].ci_detail[ci_cnt].charge_desc = c.charge_description,
   suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].ci_detail[ci_cnt].
   service_d_t = build2(format(c.service_dt_tm,"@SHORTDATE")," ",format(c.service_dt_tm,
     "@TIMENOSECONDS")),
   suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].ci_detail[ci_cnt].
   activity_d_t = build2(format(c.activity_dt_tm,"@SHORTDATE")," ",format(c.activity_dt_tm,
     "@TIMENOSECONDS")), suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].
   ci_detail[ci_cnt].update_d_t = build2(format(c.updt_dt_tm,"@SHORTDATE")," ",format(c.updt_dt_tm,
     "@TIMENOSECONDS")), suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].
   ci_detail[ci_cnt].charge_item_id = c.charge_item_id,
   suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].ci_detail[ci_cnt].
   bill_item_id = c.bill_item_id, suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].
   count_per_susp_reason = (suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].
   count_per_susp_reason+ 1), suspended_charges->sc_detail[sc_cnt].count_per_org = (suspended_charges
   ->sc_detail[sc_cnt].count_per_org+ 1)
  FOOT  c.charge_item_id
   null
  FOOT  c.encntr_id
   stat = alterlist(suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].
    ci_detail,ci_cnt), suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail[ei_cnt].
   ci_cnt = ci_cnt
  FOOT  suspense_reason_sort
   stat = alterlist(suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_detail,ei_cnt),
   suspended_charges->sc_detail[sc_cnt].sr_detail[sr_cnt].ei_cnt = ei_cnt
  FOOT  o.organization_id
   stat = alterlist(suspended_charges->sc_detail[sc_cnt].sr_detail,sr_cnt), suspended_charges->
   sc_detail[sc_cnt].sr_cnt = sr_cnt
  FOOT REPORT
   stat = alterlist(suspended_charges->sc_detail,sc_cnt), suspended_charges->sc_cnt = sc_cnt
  WITH nocounter
 ;end select
 EXECUTE reportrtl
 ...
 DECLARE headpagesection0(ncalc=i2) = f8 WITH protect
 DECLARE headpagesection0abs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE headorgsection0(ncalc=i2) = f8 WITH protect
 DECLARE headorgsection0abs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE headsuspreasonsection0(ncalc=i2) = f8 WITH protect
 DECLARE headsuspreasonsection0abs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE beforedetailspacingsection0(ncalc=i2) = f8 WITH protect
 DECLARE beforedetailspacingsection0abs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE detailsection0(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE detailsection0abs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8 WITH
 protect
 ...
 DECLARE _bcontdetailsection0 = i2 WITH noconstant(0), protect
 ...
 END ;Subroutine
 SUBROUTINE headpagesection0(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headpagesection0abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headpagesection0abs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.880000), private
   DECLARE __fieldname0 = vc WITH noconstant(build2(trim(suspended_charges->sc_detail[i].org_name),
     char(0))), protect
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_rotationangle = 0
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 9.281)
    SET rptsd->m_width = 1.188
    SET rptsd->m_height = 0.260
    SET _oldfont = uar_rptsetfont(_hreport,_times80)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(rpt_pageofpage,char(0)))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.822),(offsetx+ 10.500),(offsety
     + 0.822))
    ...
    SET rptsd->m_height = 0.260
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("SUSPENSE REASON",char(0)))
    SET rptsd->m_y = (offsety+ 0.552)
    ...
    SET rptsd->m_y = (offsety+ 0.573)
    SET rptsd->m_x = (offsetx+ 8.958)
    SET rptsd->m_width = 0.740
    SET rptsd->m_height = 0.260
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("ACTIVITY DATE/TIME",char(0)))
    SET rptsd->m_y = (offsety+ 0.573)
    ...
    SET rptsd->m_width = 5.010
    SET rptsd->m_height = 0.260
    SET _dummyfont = uar_rptsetfont(_hreport,_times12b0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Suspended Charge Report",char(0)))
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE headorgsection0(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headorgsection0abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headorgsection0abs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.920000), private
   DECLARE __fieldname2 = vc WITH noconstant(build2(concat("Summary for ",trim(suspended_charges->
       sc_detail[i].org_name)),char(0))), protect
   DECLARE __fieldname1 = vc WITH noconstant(build2(concat("Total Suspended ",trim(cnvtstring(
        suspended_charges->sc_detail[i].count_per_org))),char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 16
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET rptsd->m_flags = 4
    SET rptsd->m_y = (offsety+ 0.760)
    SET rptsd->m_x = (offsetx+ 2.750)
    SET rptsd->m_width = 1.479
    SET rptsd->m_height = 0.156
    SET _oldfont = uar_rptsetfont(_hreport,_times8u0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Suspense Reason",char(0)))
    SET rptsd->m_flags = 68
    SET rptsd->m_y = (offsety+ 0.760)
    SET rptsd->m_x = (offsetx+ 6.271)
    SET rptsd->m_width = 1.479
    SET rptsd->m_height = 0.156
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Suspense Reason Count",char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE headsuspreasonsection0(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headsuspreasonsection0abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headsuspreasonsection0abs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.170000), private
   DECLARE __fieldname0 = vc WITH noconstant(build2(suspended_charges->sc_detail[i].sr_detail[aa].
     susp_reason,char(0))), protect
   DECLARE __fieldname1 = vc WITH noconstant(build2(suspended_charges->sc_detail[i].sr_detail[aa].
     count_per_susp_reason,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE beforedetailspacingsection0(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = beforedetailspacingsection0abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE beforedetailspacingsection0abs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 2.750)
    SET rptsd->m_width = 5.021
    SET rptsd->m_height = 0.167
    SET _oldfont = uar_rptsetfont(_hreport,_times80)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("< End Summary >",char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE detailsection0(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = detailsection0abs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE detailsection0abs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   DECLARE __fieldname4 = vc WITH noconstant(build2(suspended_charges->sc_detail[i].sr_detail[ii].
     ei_detail[iii].ci_detail[iiii].charge_desc,char(0))), protect
   DECLARE __fieldname7 = vc WITH noconstant(build2(suspended_charges->sc_detail[i].sr_detail[ii].
     ei_detail[iii].ci_detail[iiii].activity_type,char(0))), protect
   DECLARE __fieldname0 = vc WITH noconstant(build2(suspended_charges->sc_detail[i].sr_detail[ii].
     ei_detail[iii].ci_detail[iiii].service_d_t,char(0))), protect
   DECLARE __fieldname1 = vc WITH noconstant(build2(suspended_charges->sc_detail[i].sr_detail[ii].
     ei_detail[iii].ci_detail[iiii].activity_d_t,char(0))), protect
   DECLARE __fieldname2 = vc WITH noconstant(build2(suspended_charges->sc_detail[i].sr_detail[ii].
     ei_detail[iii].ci_detail[iiii].update_d_t,char(0))), protect
   DECLARE __fieldname3 = vc WITH noconstant(build2(suspended_charges->sc_detail[i].sr_detail[ii].
     ei_detail[iii].patient_name,char(0))), protect
   DECLARE __fieldname6 = vc WITH noconstant(build2(suspended_charges->sc_detail[i].sr_detail[ii].
     ei_detail[iii].fin,char(0))), protect
   DECLARE __fieldname5 = vc WITH noconstant(build2(suspended_charges->sc_detail[i].sr_detail[ii].
     susp_reason,char(0))), protect
   IF (bcontinue=0)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CCPS_CS_SUSP_CHRG_ORG"
 ...
 IF ((suspended_charges->sc_cnt > 0))
  SELECT INTO  $OUTDEV
   org_name_sort = cnvtupper(suspended_charges->sc_detail[d.seq].org_name), susp_reason_sort =
   cnvtupper(suspended_charges->sc_detail[d.seq].sr_detail[d1.seq].susp_reason), patient_name_sort =
   concat(suspended_charges->sc_detail[d.seq].sr_detail[d1.seq].ei_detail[d2.seq].name_last_key," ",
    suspended_charges->sc_detail[d.seq].sr_detail[d1.seq].ei_detail[d2.seq].name_first_key," ",trim(
     cnvtstring(suspended_charges->sc_detail[d.seq].sr_detail[d1.seq].ei_detail[d2.seq].person_id))),
   charge_desc_sort = cnvtupper(suspended_charges->sc_detail[d.seq].sr_detail[d1.seq].ei_detail[d2
    .seq].ci_detail[d3.seq].charge_desc)
   FROM (dummyt d  WITH seq = value(suspended_charges->sc_cnt)),
    (dummyt d1  WITH seq = 1),
    (dummyt d2  WITH seq = 1),
    (dummyt d3  WITH seq = 1)
   PLAN (d
    WHERE maxrec(d1,suspended_charges->sc_detail[d.seq].sr_cnt))
    JOIN (d1
    WHERE maxrec(d2,suspended_charges->sc_detail[d.seq].sr_detail[d1.seq].ei_cnt))
    JOIN (d2
    WHERE maxrec(d3,suspended_charges->sc_detail[d.seq].sr_detail[d1.seq].ei_detail[d2.seq].ci_cnt))
    JOIN (d3)
   ORDER BY org_name_sort, susp_reason_sort, patient_name_sort,
    charge_desc_sort
   HEAD REPORT
    first_org = 1
   HEAD org_name_sort
    ...
    ENDIF
    i = d.seq
    IF (((_yoffset+ headpagesection0(rpt_calcheight)) > 7.8))
     d0 = pagebreak(0)
    ENDIF
    d0 = headpagesection0(rpt_render)
    IF (((_yoffset+ headorgsection0(rpt_calcheight)) > 7.8))
     d0 = pagebreak(0), d0 = headpagesection0(rpt_render)
    ENDIF
    d0 = headorgsection0(rpt_render)
    FOR (aa = 1 TO suspended_charges->sc_detail[d.seq].sr_cnt)
     IF (((_yoffset+ headsuspreasonsection0(rpt_calcheight)) > 7.8))
      d0 = pagebreak(0), d0 = headpagesection0(rpt_render)
     ENDIF
     ,d0 = headsuspreasonsection0(rpt_render)
    ENDFOR
    IF (((_yoffset+ beforedetailspacingsection0(rpt_calcheight)) > 7.8))
     d0 = pagebreak(0), d0 = headpagesection0(rpt_render)
    ENDIF
    d0 = beforedetailspacingsection0(rpt_render)
   HEAD susp_reason_sort
    null
   HEAD patient_name_sort
    null
   HEAD charge_desc_sort
    null
   DETAIL
    ii = d1.seq, iii = d2.seq, iiii = d3.seq
    IF (((_yoffset+ detailsection0(rpt_calcheight,2.0,_bcontdetailsection0)) > 7.8))
     d0 = pagebreak(0), d0 = headpagesection0(rpt_render)
    ENDIF
    d0 = detailsection0(rpt_render,2.0,_bcontdetailsection0)
   FOOT  charge_desc_sort
    null
   FOOT  patient_name_sort
    null
   FOOT  susp_reason_sort
    null
   FOOT  org_name_sort
    null
   FOOT REPORT
 ...
 ELSE
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
    display_message = "There are no suspended charges for the selected organization(s).", d0 =
    nodatasection0(rpt_render)
  ;end select
 ENDIF
