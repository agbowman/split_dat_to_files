CREATE PROGRAM bed_ens_concept_name_match:dba
 ...
 DECLARE parsecodesetstring = vc WITH protect, noconstant("")
 DECLARE matchescnt = i4 WITH protect, noconstant(0)
 IF (size(request->matches,5) > 0)
  CALL bedlogmessage("identsToSave","Entering ...")
  FOR (matchescnt = 1 TO size(request->matches,5))
    SET stat = alterlist(identstosave->ident,matchescnt)
    IF (identifier_type="CKI")
     SET identstosave->ident[matchescnt].parseidentifierstring = "cki"
    ELSEIF (identifier_type="CDFMEANING")
     SET identstosave->ident[matchescnt].parseidentifierstring = "cdf_meaning"
    ELSEIF (identifier_type="CONCEPTCKI")
     SET identstosave->ident[matchescnt].parseidentifierstring = "concept_cki"
    ENDIF
    SET identstosave->ident[matchescnt].display = request->matches[matchescnt].display
    SET identstosave->ident[matchescnt].identifier = request->matches[matchescnt].identifier
    SET identstosave->ident[matchescnt].code_set = code_set
  ENDFOR
  CALL bedlogmessage("identsToSave","Exiting ...")
 ENDIF
 FOR (matchescnt = 1 TO size(identstosave->ident,5))
   SET identtosave->code_set = identstosave->ident[matchescnt].code_set
   SET identtosave->display = identstosave->ident[matchescnt].display
   SET identtosave->identifier = identstosave->ident[matchescnt].identifier
   SET identtosave->parseidentifierstring = identstosave->ident[matchescnt].parseidentifierstring
   IF (table_type="CODE")
