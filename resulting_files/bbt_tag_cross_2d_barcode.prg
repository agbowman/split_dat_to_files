CREATE PROGRAM bbt_tag_cross_2d_barcode
 EXECUTE reportrtl
 ...
 DECLARE _remlbldeclaration1 = i4 WITH noconstant(1), protect
 ...
 SUBROUTINE (tablerowabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 1.313),offsety,(offsetx+ 1.313),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 2.439),offsety,(offsetx+ 2.439),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 3.565),offsety,(offsetx+ 3.565),(offsety+
     sectionheight))
 ...
 SUBROUTINE (detailsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
  WITH protect)
   ...
   DECLARE drawheight_lbldeclaration1 = f8 WITH noconstant(0.0), private
   ...
   DECLARE __txttagbbid = vc WITH noconstant(build2(
     ...
     ENDIF
     ,char(0))), protect
   DECLARE __txttagproductvolume = vc WITH noconstant(build2(
     IF (tag_per_event_derivative_ind=0) build(concat("Product Volume: ",tag_per_event_prod_volume))
     ELSE tag_per_event_qty_iu
     ENDIF
     ,char(0))), protect
   ...
   DECLARE __txtbbid = vc WITH noconstant(build2(
     ...
     ENDIF
     ,char(0))), protect
   DECLARE __lbldeclaration1 = vc WITH noconstant(build2(uar_i18ngetmessage(_hi18nhandle,
      "DetailSection_lblDeclaration1",build2(concat(
        ...
        "abel and the unit identification.  Transfusionist______________________  Witness______________________",
        _crlf,
   ...
   SET rptsd->m_width = 7.820
   SET rptsd->m_height = ((offsety+ maxheight) - rptsd->m_y)
   SET _oldfont = uar_rptsetfont(_hreport,_helvetica90)
   SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
   SET _holdremlbldeclaration1 = _remlbldeclaration1
   IF (_remlbldeclaration1 > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_remlbldeclaration1,((size
       (__lbldeclaration1) - _remlbldeclaration1)+ 1),__lbldeclaration1)))
    SET drawheight_lbldeclaration1 = rptsd->m_height
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _remlbldeclaration1 = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remlbldeclaration1,((size(
        __lbldeclaration1) - _remlbldeclaration1)+ 1),__lbldeclaration1)))))
     SET _remlbldeclaration1 += rptsd->m_drawlength
    ELSE
     SET _remlbldeclaration1 = 0
    ENDIF
    SET growsum += _remlbldeclaration1
   ENDIF
   SET _dummypen = uar_rptsetpen(_hreport,_pen14s0c8)
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 2.626),(offsety+ 7.819),3.376,2.944,
     rpt_nofill,rpt_black)
   ENDIF
   SET _dummypen = uar_rptsetpen(_hreport,_pen14s0c0)
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 4.320),(offsety+ 3.694),3.751,1.500,
     rpt_nofill,rpt_black)
   ENDIF
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 0.320),(offsety+ 3.694),3.695,1.500,
     rpt_nofill,rpt_black)
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_lblHeadTag",build2("CROSSMATCH TRANSFUSION TAG",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 12
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 2.626)
   SET rptsd->m_x = (offsetx+ 0.320)
   SET rptsd->m_width = 0.570
   SET rptsd->m_height = 0.202
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_lblTagPhysician",build2("Ordering Physician:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 3.056)
   SET rptsd->m_x = (offsetx+ 4.244)
   SET rptsd->m_width = 1.612
   SET rptsd->m_height = 0.202
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_lblTagCrossmatchVerifiedBy",build2("Crossmatch Verified By:",char(0))),char(0))
     )
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_lblCrossmatchDateTime",build2("Crossmatch Date/Time:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 3.376)
   SET rptsd->m_x = (offsetx+ 1.570)
   SET rptsd->m_width = 2.376
   SET rptsd->m_height = 0.202
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(tag_per_event_ordering_physician_name,
      char(0)))
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_borders = rpt_sdnoborders
   ...
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 2.556)
   SET rptsd->m_x = (offsetx+ 4.250)
   SET rptsd->m_width = 2.501
   SET rptsd->m_height = 0.202
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__txttagproductvolume)
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(tag_per_event_crossmatch_interp,char(0
       )))
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_borders = rpt_sdnoborders
   ...
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 3.056)
   SET rptsd->m_x = (offsetx+ 5.744)
   SET rptsd->m_width = 2.251
   SET rptsd->m_height = 0.202
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(tag_per_event_crossmatch_verified_by,
      char(0)))
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_borders = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(tag_per_event_crossmatch_dttm,char(0))
     )
   ENDIF
   SET rptsd->m_flags = 12
   SET rptsd->m_borders = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "DetailSection_lblAccession",build2("Accession #:",char(0))),char(0)))
   ENDIF
   SET rptsd->m_flags = 0
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(tag_per_event_accession_nbr,char(0)))
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_y = (offsety+ 9.876)
   SET rptsd->m_x = (offsetx+ 2.751)
   ...
   SET rptsd->m_x = (offsetx+ 0.320)
   SET rptsd->m_width = 7.820
   SET rptsd->m_height = drawheight_lbldeclaration1
   IF (ncalc=rpt_render
    AND _holdremlbldeclaration1 > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdremlbldeclaration1,((
       size(__lbldeclaration1) - _holdremlbldeclaration1)+ 1),__lbldeclaration1)))
   ELSE
    SET _remlbldeclaration1 = _holdremlbldeclaration1
   ENDIF
   SET _yoffset = (offsety+ 6.250)
   ...
   SET rptsd->m_paddingwidth = 0.000
   SET rptsd->m_linespacing = rpt_oneandahalf
   SET rptsd->m_y = (offsety+ 7.320)
   SET rptsd->m_x = (offsetx+ 0.278)
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 2.626),(offsety+ 7.444),0.112,0.090,
     rpt_nofill,rpt_black)
   ENDIF
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 3.070),(offsety+ 7.444),0.112,0.090,
     rpt_nofill,rpt_black)
   ENDIF
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 4.820),(offsety+ 7.569),0.112,0.090,
     rpt_nofill,rpt_black)
   ENDIF
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 5.195),(offsety+ 7.569),0.112,0.090,
     rpt_nofill,rpt_black)
   ENDIF
   SET _dummypen = uar_rptsetpen(_hreport,_pen13s0c0)
   IF (ncalc=rpt_render
    ...
    IF (tag_per_event_alias_mrn_raw != " ")
     SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_code39,(offsetx+ 0.320),(offsety+ 1.819))
     SET rptbce->m_recsize = 88
   ...
   IF (ncalc=rpt_render
    ...
    IF (tag_per_event_alias_fin_raw != " ")
     SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_code39,(offsetx+ 0.320),(offsety+ 3.069))
     SET rptbce->m_recsize = 88
     SET rptbce->m_width = 1.75
     SET rptbce->m_height = 0.25
     SET rptbce->m_rotation = 0
   ...
   IF (ncalc=rpt_render
    ...
    IF (tag_per_event_product_type_barcode != " ")
     SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_code128,(offsetx+ 4.278),(offsety+ 2.000))
     SET rptbce->m_recsize = 88
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BBT_TAG_CROSS_2D_BARCODE"
 ...
 END ;Subroutine
 DECLARE selectxmtag(dummy) = null WITH protect
 SUBROUTINE selectxmtag(ncalc)
   ...
   SELECT
    ...
    tag_per_event_dob = tag_from_request->tag_per_event[d1.seq].dob, tag_per_event_sex =
    tag_from_request->tag_per_event[d1.seq].sex,
    tag_per_event_dispense_physician_name = substring(1,40,tag_from_request->tag_per_event[d1.seq].
     dispense_physician_name), tag_per_event_prod_nbr = substring(1,40,tag_from_request->
     tag_per_event[d1.seq].prod_nbr), tag_per_event_prod_desc = substring(1,40,tag_from_request->
     tag_per_event[d1.seq].prod_desc),
    ...
    tag_per_event_unit_count = tag_from_request->tag_per_event[d1.seq].unit_count,
    ...
    tag_per_event_patient_trans_req = substring(1,250,tag_from_request->tag_per_event[d1.seq].
     patient_trans_req), tag_per_event_patient_antibodies = substring(1,250,tag_from_request->
     tag_per_event[d1.seq].patient_antibodies), tag_per_event_product_special_testing = substring(1,
     250,tag_from_request->tag_per_event[d1.seq].product_special_testing),
    tag_per_event_product_antigen = substring(1,250,tag_from_request->tag_per_event[d1.seq].
     product_antigen), tag_per_event_owninvarea_disp = substring(1,50,tag_from_request->
     tag_per_event[d1.seq].owninvarea_disp), tag_per_event_crossmatch_interp = substring(1,30,
     tag_from_request->tag_per_event[d1.seq].crossmatch_interp),
    tag_per_event_crossmatch_verified_by = substring(1,30,tag_from_request->tag_per_event[d1.seq].
     crossmatch_verified_by), tag_per_event_crossmatch_dttm = tag_from_request->tag_per_event[d1.seq]
    .crossmatch_dttm, tag_per_event_accession_nbr = substring(1,30,tag_from_request->tag_per_event[d1
     .seq].accession_nbr),
    tag_per_event_ordering_physician_name = substring(1,30,tag_from_request->tag_per_event[d1.seq].
     ordering_physician_name), tag_per_event_qty_iu = substring(1,60,tag_from_request->tag_per_event[
     d1.seq].qty_iu), tag_per_event_derivative_ind = tag_from_request->tag_per_event[d1.seq].
    derivative_ind,
    address_enc_facility_address1 = substring(1,50,tag_from_request->tag_per_event[d1.seq].address[d2
     .seq].enc_facility_address1), address_enc_facility_address2 = substring(1,50,tag_from_request->
     tag_per_event[d1.seq].address[d2.seq].enc_facility_address2), address_enc_facility_address3 =
    substring(1,50,tag_from_request->tag_per_event[d1.seq].address[d2.seq].enc_facility_address3),
    address_enc_facility_address4 = substring(1,50,tag_from_request->tag_per_event[d1.seq].address[d2
     .seq].enc_facility_address4), address_enc_facility_citystatezip = substring(1,50,
     tag_from_request->tag_per_event[d1.seq].address[d2.seq].enc_facility_citystatezip),
    address_enc_facility_country = substring(1,50,tag_from_request->tag_per_event[d1.seq].address[d2
     .seq].enc_facility_country),
    address_facility_disp = substring(1,50,tag_from_request->tag_per_event[d1.seq].address[d2.seq].
    ...
    PLAN (d1
     WHERE maxrec(d2,size(tag_from_request->tag_per_event[d1.seq].address,5)))
     JOIN (d2)
    HEAD REPORT
     _d0 = tag_per_event_patient_name, _d1 = tag_per_event_patient_location, _d2 =
     tag_per_event_patient_aborh_disp,
     _d3 = tag_per_event_bb_id, _d4 = tag_per_event_mrn, _d5 = tag_per_event_fin,
     _d6 = tag_per_event_dob, _d7 = tag_per_event_sex, _d8 = tag_per_event_prod_nbr,
     _d9 = tag_per_event_prod_desc, _d10 = tag_per_event_prod_aborh_disp, _d11 =
     tag_per_event_prod_expiration,
     _d12 = tag_per_event_prod_volume, _d13 = tag_per_event_patient_trans_req, _d14 =
     tag_per_event_patient_antibodies,
     _d15 = tag_per_event_product_special_testing, _d16 = tag_per_event_product_antigen, _d17 =
     tag_per_event_owninvarea_disp,
     _d18 = tag_per_event_crossmatch_interp, _d19 = tag_per_event_crossmatch_verified_by, _d20 =
     tag_per_event_crossmatch_dttm,
     _d21 = tag_per_event_accession_nbr, _d22 = tag_per_event_ordering_physician_name, _d23 =
     tag_per_event_qty_iu,
     _d24 = tag_per_event_derivative_ind, _d25 = address_enc_facility_address1, _d26 =
    ...
    DETAIL
     _bholdcontinue = 0, bfirsttime = 1
     WHILE (((_bholdcontinue=1) OR (bfirsttime=1)) )
       IF (y >= _nrows)
        x = 0, y = 0,
        CALL pagebreak(0)
       ENDIF
       _xoffset = ((_flmargin+ (_flabelwidth * x))+ (_fcolgutter * x)), _yoffset = ((_ftmargin+ (
       _flabelheight * y))+ (_frowgutter * y)), dummy_val = detailsection(rpt_render,_flabelheight,
        _bholdcontinue),
       x += 1
       IF (x >= _ncols)
        x = 0, y += 1
   ...
   ;end select
 END ;Subroutine
 EXECUTE cpm_create_file_name_logical "bbt_tag_cross_2D", "dat", "x"
