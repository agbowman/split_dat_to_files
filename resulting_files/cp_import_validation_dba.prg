CREATE PROGRAM cp_import_validation:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE"
  WITH outdev
 DECLARE nodecount = i2 WITH protect, noconstant(0)
 DECLARE brcount = i2 WITH protect, noconstant(0)
 DECLARE scatname = vc WITH protect
 DECLARE dpathwayactivecd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003198,"ACTIVE"))
 ...
 DECLARE pathwayname = vc WITH protect, noconstant("")
 ...
 ENDIF
 IF (validate(dup_fin_del_rec) != 1)
  FREE RECORD dup_fin_del_rec
  RECORD dup_fin_del_rec(
    1 delete_pw_name = vc
  ) WITH protect
 ENDIF
 IF ( NOT (validate(pathway_valid_rep)))
  RECORD pathway_valid_rep(
    1 incomplete_sdoc_ind = i2
    1 incomplete_bedrock_ind = i2
    1 existing_pathway_ind = i2
    1 fin_list[*]
      2 fin_nbr = vc
      2 name = vc
    1 status_data
 ...
 SUBROUTINE (setauditinitreplystatusbyprogramreplystatus(reply=vc(ref)) =null WITH protect)
  ...
  ENDIF
 END ;Subroutine
 DECLARE findduplicates(null) = null WITH protect
 DECLARE isstructdocbuilt(null) = null WITH protect
 DECLARE isbedrockbuilt(null) = null WITH protect
 IF (validate(request->blob_in))
  IF ((request->blob_in > " "))
   CALL logblobindata("CP_IMPORT_VALIDATION",request->blob_in, $OUTDEV)
   IF (validate(debug_ind,0)=1)
   ...
   IF (validate(debug_ind,0)=1)
    ...
    ELSE
     CALL echorecord(dup_fin_del_rec)
    ENDIF
   ENDIF
   FREE RECORD request
  ENDIF
 ENDIF
 SET pathway_valid_rep->status_data.status = "F"
 IF (size(pw_rec->qual,5) > 0)
  SET pathwayname = pw_rec->qual[1].pathway_name
  CALL findduplicates(null)
  CALL isstructdocbuilt(null)
  CALL isbedrockbuilt(null)
 ELSE
  SET pathwayname = dup_fin_del_rec->delete_pw_name
  CALL findduplicates(null)
 ENDIF
 SET pathway_valid_rep->status_data.status = "S"
 SUBROUTINE findduplicates(null)
   CALL log_message("Begin FindDuplicates()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE dfin = f8 WITH constant(uar_get_code_by("MEANING",319,"FIN NBR")), protect
   DECLARE dactivepw = f8 WITH constant(uar_get_code_by("MEANING",4003352,"ACTIVE")), protect
   DECLARE spathwaynamesearch = vc WITH protect
   DECLARE ifincount = i2 WITH noconstant(0), protect
   SET pathway_valid_rep->existing_pathway_ind = 0
   SELECT INTO "nl:"
    FROM cp_pathway cp,
     prsnl p
    PLAN (p
     ...
     JOIN (cp
     WHERE cp.pathway_name=pathwayname
      AND cp.pathway_status_cd=dpathwayactivecd
      AND cp.logical_domain_id=p.logical_domain_id)
    WITH nocounter
   ;end select
   IF (curqual > 0)
    SET pathway_valid_rep->existing_pathway_ind = 1
    SET pathway_valid_rep->status_data.status = "S"
    SELECT INTO "nl:"
     ...
     PLAN (cp
      WHERE cp.pathway_name=pathwayname)
      JOIN (cpa
      WHERE cpa.cp_pathway_id=cp.cp_pathway_id)
      JOIN (e
      WHERE e.encntr_id=cpa.encntr_id)
      JOIN (ea
      WHERE ea.encntr_id=e.encntr_id
       AND ea.encntr_alias_type_cd=dfin)
      JOIN (p
      WHERE p.person_id=e.person_id
       AND p.logical_domain_id=cp.logical_domain_id)
     ORDER BY cpa.updt_dt_tm DESC
     HEAD REPORT
      ifincount = 0
     HEAD cpa.pathway_instance_id
      IF (cpa.pathway_activity_status_cd=dactivepw
       AND cpa.end_effective_dt_tm > cnvtdatetime(sysdate))
       pathway_valid_rep->existing_pathway_ind = 1, ifincount += 1
       IF (ifincount > size(pathway_valid_rep->fin_list,5))
        stat = alterlist(pathway_valid_rep->fin_list,(ifincount+ 10))
       ENDIF
       pathway_valid_rep->fin_list[ifincount].fin_nbr = ea.alias, pathway_valid_rep->fin_list[
       ifincount].name = p.name_full_formatted
      ENDIF
     FOOT REPORT
      stat = alterlist(pathway_valid_rep->fin_list,ifincount), pathway_valid_rep->status_data.status
       = "S"
     WITH nocounter
    ;end select
   ENDIF
   CALL log_message(build("Exit FindDuplicates(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE isstructdocbuilt(null)
   CALL log_message("Begin IsStructDocBuilt()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SET pathway_valid_rep->incomplete_sdoc_ind = 1
   SET sdoccnt = 0
   SET sdoccompcnt = 0
   FOR (i = 1 TO size(pw_rec->qual[1].node_list,5))
     FOR (j = 1 TO size(pw_rec->qual[1].node_list[i].comp_list,5))
       IF ((pw_rec->qual[1].node_list[i].comp_list[j].comp_type_mean="PATHWAY_DOC"))
        SET sdoccompcnt += 1
        FOR (k = 1 TO size(pw_rec->qual[1].node_list[i].comp_list[j].comp_dtl_list,5))
          IF ((pw_rec->qual[1].node_list[i].comp_list[j].comp_dtl_list[k].component_ident > " "))
           SELECT INTO "nl:"
            FROM dd_sref_template dst
            WHERE (dst.dd_sref_tmpl_instance_ident=pw_rec->qual[1].node_list[i].comp_list[j].
            comp_dtl_list[k].component_ident)
            DETAIL
             sdoccnt += 1
            WITH nocounter
           ;end select
     ...
     ENDFOR
   ENDFOR
   IF (sdoccnt=size(pw_rec->qual[1].node_list,5))
    SET pathway_valid_rep->incomplete_sdoc_ind = 0
   ELSEIF (sdoccompcnt=0)
    SET pathway_valid_rep->incomplete_sdoc_ind = 2
   ENDIF
   CALL log_message(build("Exit IsStructDocBuilt(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE isbedrockbuilt(null)
   CALL log_message("Begin IsBedrockBuilt()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SET pathway_valid_rep->incomplete_bedrock_ind = 1
   SET brcount = 0
   SET compcount = 0
   SET idx = 0
   SET nodecount = pw_rec->qual[1].node_cnt
   FOR (z = 1 TO nodecount)
     SET compcount += pw_rec->qual[1].node_list[z].comp_cnt
   ENDFOR
   IF (((nodecount=0) OR (nodecount > 0
    AND compcount=0)) )
    SET pathway_valid_rep->incomplete_bedrock_ind = 2
   ELSE
    SELECT INTO "nl:"
     FROM br_datamart_category bdc
     WHERE expand(idx,1,pw_rec->qual[1].node_cnt,bdc.category_mean,pw_rec->qual[1].node_list[idx].
      category_mean)
     DETAIL
      brcount += 1
     WITH nocounter
    ;end select
    IF (brcount=nodecount)
     SET pathway_valid_rep->incomplete_bedrock_ind = 0
    ENDIF
   ENDIF
   CALL log_message(build("Exit IsBedrockBuilt(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
#exit_script
 CALL setauditinitreplystatusbyprogramreplystatus(pathway_valid_rep)
 CALL putjsonrecordtofile(pathway_valid_rep)
 FREE RECORD pathway_valid_rep
END GO
