CREATE PROGRAM afc_ens_bill_item_internal:dba
 ...
 IF ( NOT (validate(c106_generic_addon_cd)))
  DECLARE c106_generic_addon_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",106,
    "AFC ADD GEN"))
 ENDIF
 DECLARE logicaldomainid = f8 WITH protect, noconstant(0.0)
 DECLARE logicaldomaininuse = i2 WITH protect, noconstant(false)
 SET hafc_ens_bill_item_internal = 0
 SET istatus = 0
  ...
  FOR (inx0 = 1 TO request->bill_item_qual)
    CASE (request->bill_item[inx0].action_type)
     OF "ADD":
      ...
      SET action_end = inx0
      IF ((request->bill_item[inx0].ext_owner_cd=c106_generic_addon_cd)
       AND logicaldomaininuse)
       EXECUTE afc_add_bill_item_internal  WITH replace("REQUEST",ensurereq)
      ELSE
       EXECUTE afc_add_bill_item_internal
      ENDIF
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "UPT":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_upt_bill_item_internal
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "DEL":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_del_bill_item_internal
      IF (failed != false)
       GO TO check_error
      ENDIF
     ELSE
 ...
 SUBROUTINE (copyrequestinfo(dummyvar=i2) =null)
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ensurereq->bill_item[cntbillitem].calc_type_cd = request->bill_item[cntbillitem].calc_type_cd,
    ensurereq->bill_item[cntbillitem].charge_point_cd = request->bill_item[cntbillitem].
    charge_point_cd, ensurereq->bill_item[cntbillitem].end_effective_dt_tm = request->bill_item[
    cntbillitem].end_effective_dt_tm,
    ...
    ensurereq->bill_item[cntbillitem].ext_parent_contributor_cd = request->bill_item[cntbillitem].
    ext_parent_contributor_cd, ensurereq->bill_item[cntbillitem].ext_parent_reference_id = request->
    bill_item[cntbillitem].ext_parent_reference_id,
    ensurereq->bill_item[cntbillitem].parent_qual_cd = request->bill_item[cntbillitem].parent_qual_cd,
    ensurereq->bill_item[cntbillitem].physician_qual_cd = request->bill_item[cntbillitem].
    physician_qual_cd
   WITH nocounter
  ;end select
 END ;Subroutine
