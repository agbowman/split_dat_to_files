CREATE PROGRAM dm2_aload_code_value_ext
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_cve",".log","dm2_aload_code_value_ext")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_code_value_ext"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dacve_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dacve_file = vc WITH protect, noconstant( $2)
 DECLARE dacve_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dacve_batch = i4 WITH protect, noconstant(0)
 DECLARE dacve_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE dacve_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dacve_pkg,dacve_file_loc_ret,dacve_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dacve_batch=0)
  SET dacve_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_AFD_CODE_VALUE_EXTENSION for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND dacve_rows_returned=10000)
   DELETE  FROM dm_afd_code_value_extension d
    WHERE d.alpha_feature_nbr=dacve_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET dacve_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",dacve_rows_returned))
   ENDIF
 ENDWHILE
 IF (dacve_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dacve_file_loc_ret,dacve_file), "dm2_aload_cs_cve", dacve_batch
  CALL echo(concat(dacve_file_loc_ret,dacve_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_code_value_ext completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_code_value_ext")
END GO
