CREATE PROGRAM dm_cutover_rollback:dba
 ...
 DECLARE acquire_successful_check(null) = null
 DECLARE clean_translations(null) = null
 DECLARE clean_chg_log(null) = null
 DECLARE clean_exceptions(null) = null
 DECLARE confirm_rollback(null) = null
 DECLARE clean_r_tables(null) = null
 DECLARE wait_to_rollback(null) = null
 DECLARE finished_rollback(null) = null
 SET message = window
 DECLARE dcr_roll_con = i2 WITH protect, noconstant(0)
 CALL check_logfile("DM_CUTOVER_ROLLBACK",".log","DM_CUTOVER_ROLLBACK")
 SET dm_err->eproc = "Beginning DM_CUTOVER_ROLLBACK"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET logfile_name = dm_err->logfile
 FREE RECORD dcr_roll_tables
 RECORD dcr_roll_tables(
   1 r_cnt = i4
   1 r_table[*]
     2 r_t_name = vc
 ) WITH protect
 FREE RECORD dcr_st_env
 RECORD dcr_st_env(
   1 env_id = f8
   1 env_name = vc
   1 parent_env_id = f8
 ...
 IF (curqual=0)
  SET message = nowindow
  CALL echo(
   "The current domain is not enabled for Merge and Cut. Please update the correct DM_INFO and execute rollback again"
   )
  CALL pause(10)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  ...
  DETAIL
   dcr_st_env->env_id = b.environment_id, dcr_st_env->env_name = b.environment_name
  WITH nocounter
 ;end select
 IF (curqual=0)
  ...
  GO TO exit_program
 ENDIF
 WHILE (dcr_roll_con=0)
   SET message = window
   CALL clear(1,1)
   SET width = 132
   CALL box(1,1,7,132)
   CALL text(3,46,"***  ROLLBACK FOR $R TABLES  ***")
   ...
   CALL text(6,95,cnvtstring(dcr_st_env->env_id))
   CALL text(6,40,dcr_st_env->env_name)
   CALL text(9,3,"Please choose from the following options:")
   CALL text(11,3,"1 Input the source ENVIRONMENT_ID for the source you want to rollback")
   ...
   CASE (curaccept)
    OF 1:
     ...
     CALL acquire_successful_check(null)
     CALL confirm_rollback(null)
     CALL wait_to_rollback(null)
     CALL clean_translations(null)
     CALL clean_chg_log(null)
     CALL clean_exceptions(null)
     CALL clean_r_tables(null)
     CALL finished_rollback(null)
    OF 0:
     SET dcr_roll_con = 1
   ENDCASE
 ENDWHILE
 ...
 SUBROUTINE accept_environment_id(null)
   ...
   WHILE (ae_insert_validate=0)
     ...
     SELECT INTO "nl:"
      ...
      (SELECT
       ...
       WHERE (child_env_id=dcr_st_env->env_id)))
     ...
     ELSE
      SELECT INTO "nl:"
       ...
       (SELECT
        ...
        FROM dm_env_reltn der
        WHERE (child_env_id=dcr_st_env->env_id)))
        AND (de.environment_id != dcr_st_env->env_id)
   ...
   CALL clear(20,1)
   IF (ae_insert_con=0)
    SET dcr_st_env->parent_env_id = cnvtreal(curaccept)
    SET help = off
   ENDIF
 END ;Subroutine
 SUBROUTINE acquire_successful_check(null)
   SET dm_err->eproc = "Obtaining last successful check time"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="RDDS CONFIGURATION"
     AND d.info_name="LAST SUCCESSFUL CUT-OVER*"
     AND (d.info_number=dcr_st_env->parent_env_id)
    DETAIL
     dcr_st_env->succ_check_dt = d.info_date
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (curqual=0)
    CALL clear(20,1)
    CALL text(20,3,"Valid rollback date could not be obtained. Please contact the IRC")
    CALL pause(5)
    GO TO exit_program
   ENDIF
 END ;Subroutine
 SUBROUTINE clean_translations(null)
   DECLARE ct_loop = i2 WITH protect, noconstant(0)
   SET dm_err->eproc = "Removing rows from dm_merge translate table"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET ct_loop = 0
   WHILE (ct_loop=0)
     DELETE  FROM dm_merge_translate d
      WHERE (d.env_source_id=dcr_st_env->parent_env_id)
       AND (d.env_target_id=dcr_st_env->env_id)
       AND d.updt_dt_tm >= cnvtdatetime(dcr_st_env->succ_check_dt)
      WITH maxqual(d,5000)
     ;end delete
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDWHILE
 END ;Subroutine
 SUBROUTINE clean_chg_log(null)
   DECLARE ccl_loop = i2 WITH protect, noconstant(0)
   DECLARE ccl_temp_dlink = vc WITH protect
   SET dm_err->eproc = "Updating rows from the dm_chg_log table"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET ccl_loop = 0
   SET ccl_temp_dlink = concat("dm_chg_log","@merge",trim(cnvtstring(dcr_st_env->parent_env_id)),trim
    (cnvtstring(dcr_st_env->env_id)))
   WHILE (ccl_loop=0)
     UPDATE  FROM (parser(ccl_temp_dlink) d)
      SET d.log_type = "REFCHG"
      WHERE (d.target_env_id=dcr_st_env->env_id)
       AND ((d.log_type IN ("MERGED", "ORPHAN", "OLDVER", "BADLOG", "BADTRN",
      "PROCES", "REPORT")) OR (d.log_type="NOMV*"))
       AND (d.target_env_id=dcr_st_env->env_id)
       AND d.updt_dt_tm >= cnvtdatetime(dcr_st_env->succ_check_dt)
      WITH nocounter, maxqual(d,5000)
     ;end update
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ROLLBACK
   ...
   ENDWHILE
 END ;Subroutine
 SUBROUTINE clean_exceptions(null)
   DECLARE ce_temp_dlink = vc WITH protect
   DECLARE ce_loop = i2 WITH protect
   SET dm_err->eproc = "Removing rows from the dm_chg_log_exception table"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET ce_temp_dlink = concat("dm_chg_log_exception","@merge",trim(cnvtstring(dcr_st_env->
      parent_env_id)),trim(cnvtstring(dcr_st_env->env_id)))
   SET ce_loop = 0
   WHILE (ce_loop=0)
     DELETE  FROM (parser(ce_temp_dlink) d)
      WHERE (d.target_env_id=dcr_st_env->env_id)
       AND d.updt_dt_tm >= cnvtdatetime(dcr_st_env->succ_check_dt)
      WITH nocounter, maxqual(d,5000)
     ;end delete
   ...
   ENDWHILE
 END ;Subroutine
 SUBROUTINE confirm_rollback(null)
   SET message = window
   CALL clear(1,1)
   SET width = 132
   CALL box(1,1,5,132)
   CALL text(3,5,"You are about to start the ROLLBACK process for environment: ")
   CALL text(3,67,cnvtstring(dcr_st_env->parent_env_id))
   CALL text(4,5,"It will remove all RDDS actions since: ")
   CALL text(4,44,format(cnvtdatetime(dcr_st_env->succ_check_dt),";;q"))
   CALL text(7,5,"Are you sure this is the action you want to take (Y/N)?")
   CALL accept(7,70,"P;CU","N"
    WHERE curaccept IN ("Y", "N"))
   IF (curaccept="N")
    GO TO exit_program
   ELSE
    CALL add_tracking_row(dcr_st_env->parent_env_id,"ROLLBACK RDDS","WORKING")
   ENDIF
 END ;Subroutine
 SUBROUTINE clean_r_tables(null)
   DECLARE crt_loop_cnt = i4 WITH protect
   DECLARE crt_table_name = vc WITH protect
   DECLARE crt_del_loop_cnt = i2 WITH protect
   SET dm_err->eproc = "Filling in record with $R table names"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT DISTINCT INTO "nl:"
    FROM user_tables t
    WHERE t.table_name="*$R"
    HEAD REPORT
     dcr_roll_tables->r_cnt = 0
    DETAIL
     dcr_roll_tables->r_cnt = (dcr_roll_tables->r_cnt+ 1)
     IF (mod(dcr_roll_tables->r_cnt,100)=1)
      stat = alterlist(dcr_roll_tables->r_table,(dcr_roll_tables->r_cnt+ 99))
     ENDIF
     dcr_roll_tables->r_table[dcr_roll_tables->r_cnt].r_t_name = t.table_name
    FOOT REPORT
     stat = alterlist(dcr_roll_tables->r_table,dcr_roll_tables->r_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   SET crt_loop_cnt = 1
   SET dm_err->eproc = "Cleaning the $R tables"
   CALL disp_msg(" ",dm_err->logfile,0)
   WHILE ((crt_loop_cnt <= dcr_roll_tables->r_cnt))
     SET crt_del_loop_cnt = 0
     SET crt_table_name = concat(dcr_roll_tables->r_table[crt_loop_cnt].r_t_name)
     WHILE (crt_del_loop_cnt=0)
       DELETE  FROM (parser(crt_table_name) t)
        WHERE t.rdds_dt_tm >= cnvtdatetime(dcr_st_env->succ_check_dt)
         AND t.rdds_status_flag < 9000
         AND (t.rdds_source_env_id=dcr_st_env->parent_env_id)
        WITH nocounter, maxqual(t,5000)
       ;end delete
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ROLLBACK
       ...
       ENDIF
       IF (curqual=0)
        SET crt_del_loop_cnt = 1
       ENDIF
     ENDWHILE
     SET crt_loop_cnt = (crt_loop_cnt+ 1)
   ENDWHILE
 END ;Subroutine
 SUBROUTINE wait_to_rollback(null)
   SET message = window
   CALL clear(1,1)
   SET width = 132
   CALL box(1,1,5,132)
   CALL text(3,5,"The ROLLBACK process has been started. Please wait for it to complete. Thank you. "
    )
   CALL pause(2)
 END ;Subroutine
 SUBROUTINE finished_rollback(null)
   SET message = window
   CALL clear(1,1)
   SET width = 132
   CALL box(1,1,5,132)
   CALL text(3,5,"The ROLLBACK process has finished. Thank you for your patience. ")
   CALL pause(2)
 END ;Subroutine
#exit_program
 ROLLBACK
 ...
 ELSE
  CALL disp_msg("Rollback on $R tables has completed sucessfully",dm_err->logfile,0)
 ENDIF
