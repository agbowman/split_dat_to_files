CREATE PROGRAM cds_batch_rerun_run:dba
 PROMPT
...
#ukr_error_subroutines_exit
 SET last_mod = "464886"
 IF (validate(ukr_common_dates_subroutines) != 0)
  GO TO ukr_common_dates_subroutines_exit
 ENDIF
 DECLARE ukr_common_dates_subroutines = i2 WITH public, constant(1)
 SUBROUTINE checkdatewithperiod(s_date_prompt,use_time_ind,end_time_ind)
   DECLARE d_return_date = dq8 WITH private, noconstant(0.0)
   DECLARE i_time = i4 WITH private, noconstant(0)
   DECLARE new_date = dq8 WITH private, noconstant(0)
   DECLARE temp_dt_tm = c20 WITH private, noconstant
   DECLARE period = vc WITH private, noconstant("")
   DECLARE sysdate_ind = i4 WITH private, noconstant(0)
   DECLARE minus_plus_ind = c1 WITH private, noconstant("")
   SET s_date_prompt = cnvtupper(trim(s_date_prompt,3))
   IF (use_time_ind > 0)
    IF (end_time_ind=2)
     SET i_time = cnvttime(cnvtdatetime(sysdate))
    ELSE
   ...
   IF (textlen(trim(s_date_prompt,3))=0)
    RETURN(0)
   ENDIF
   IF (s_date_prompt IN ("CURDATE", "SYSDATE"))
    SET d_return_date = cnvtdatetime(curdate,i_time)
    RETURN(d_return_date)
   ENDIF
   IF (s_date_prompt="*SYSDATE*")
    SET sysdate_ind = 1
    SET s_date_prompt = replace(s_date_prompt,"SYSDATE","CURDATE",0)
   ENDIF
   IF (s_date_prompt="*CURDATE*")
    SET temp_dt_tm = trim(replace(s_date_prompt,"CURDATE"," "),3)
    IF (substring(1,1,temp_dt_tm) != "-")
     RETURN(0)
    ENDIF
    SET temp_dt_tm = substring(2,(textlen(temp_dt_tm) - 1),temp_dt_tm)
    IF (isnumeric(temp_dt_tm))
     SET period = "D"
    ELSE
     SET period = cnvtalphanum(temp_dt_tm,2)
     IF (sysdate_ind=0
      AND  NOT (period IN ("Y", "M", "W", "D", "H",
     "MIN", "S")))
      RETURN(0)
     ENDIF
     IF (sysdate_ind=1
      AND  NOT (period IN ("Y", "M", "W", "D")))
      RETURN(0)
     ENDIF
    ENDIF
    IF (sysdate_ind=1)
     SET new_date = datetimefind(cnvtdatetime(curdate,i_time),period,"B","P")
    ELSE
     SET new_date = cnvtdatetime(curdate,i_time)
    ENDIF
    SET d_return_date = cnvtlookbehind(concat(cnvtalphanum(temp_dt_tm,1),",",period),new_date)
    IF (end_time_ind=1)
     SET d_return_date = cnvtlookbehind("1,D",d_return_date)
    ENDIF
   ELSEIF (s_date_prompt="*CURMONTH*")
    IF (s_date_prompt="CURMONTH")
     SET period = "M"
     SET temp_dt_tm = "0"
    ELSE
     SET temp_dt_tm = trim(replace(s_date_prompt,"CURMONTH"," "),3)
     SET minus_plus_ind = substring(1,1,temp_dt_tm)
     IF (minus_plus_ind != "-")
      RETURN(0)
     ENDIF
     SET temp_dt_tm = substring(2,(textlen(temp_dt_tm) - 1),temp_dt_tm)
     ...
     ELSE
      SET period = cnvtalphanum(temp_dt_tm,2)
      IF ( NOT (period IN ("Y", "M", "W", "D")))
       RETURN(0)
      ENDIF
     ENDIF
    ENDIF
    IF (end_time_ind=0)
     SET new_date = datetimefind(cnvtdatetime(curdate,i_time),period,"B","B")
     SET d_return_date = cnvtlookbehind(concat(cnvtalphanum(temp_dt_tm,1),",",period),new_date)
    ELSE
     SET new_date = datetimefind(cnvtdatetime(curdate,i_time),period,"E","E")
     SET new_date = cnvtlookbehind(concat(cnvtalphanum(temp_dt_tm,1),",",period),new_date)
     SET d_return_date = datetimefind(cnvtdatetime(new_date),period,"E","E")
    ENDIF
   ELSE
   ...
   RETURN(d_return_date)
 END ;Subroutine
#ukr_common_dates_subroutines_exit
 DECLARE rept_start_dt_tm = vc WITH protect, constant(format(checkdatewithperiod( $STARTDATE,0,0),
   "DD-MMM-YYYY;;D"))
 DECLARE rept_end_dt_tm = vc WITH protect, constant(format(checkdatewithperiod( $ENDDATE,0,1),
   "DD-MMM-YYYY;;D"))
 EXECUTE ukr_cds_create_extracts  $OUTDEV, rept_start_dt_tm, rept_end_dt_tm,
  $BATCH_TYPE,  $ANON,  $ORG,
  $VERSION, 0, "CDS_BATCH_RERUN_BY_RUN_DATE",
  $MARS_REPORT_ID
#exit_script
