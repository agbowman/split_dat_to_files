CREATE PROGRAM dcp_arpt_24_icu_readmit_rev:dba
 RECORD rpt_params(
   1 output_device = vc
   1 beg_dt_tm = dq8
   1 end_dt_tm = dq8
 ...
 SELECT INTO "nl:"
  icu_los_act_curr = datetimediff(cnvtdatetime(curdate,curtime3),pat_record->pat_data[d.seq].
   ...
   pat_record->pat_data[d.seq].r_icu_admit_dt_tm,1)
  FROM (dummyt d  WITH seq = pat_record->cnt),
   risk_adjustment_outcomes rao1
  PLAN (d
   WHERE (pat_record->pat_data[d.seq].r_readmit_ind=1))
   JOIN (rao1
   WHERE (rao1.risk_adjustment_day_id=pat_record->pat_data[d.seq].ra_risk_adjustment_day_id)
    AND rao1.active_ind=1
    AND rao1.equation_name="NTL_1ST_TISS")
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = (cnt+ 1), stat = alterlist(readmit->data,cnt), readmit->data[cnt].last_name = pat_record->
   pat_data[d.seq].e_name_last,
   readmit->data[cnt].person_id = pat_record->pat_data[d.seq].r_person_id, readmit->data[cnt].
   encntr_id = pat_record->pat_data[d.seq].e_encntr_id, readmit->data[cnt].doc_id = pat_record->
   pat_data[d.seq].r_adm_doc_id,
   readmit->data[cnt].doc_srv = pat_record->pat_data[d.seq].e_med_service_cd, readmit->data[cnt].age
    = pat_record->pat_data[d.seq].r_age, readmit->data[cnt].admit_dx = pat_record->pat_data[d.seq].
   r_admitdiagnosis,
   readmit->data[cnt].icu_admit_dt = pat_record->pat_data[d.seq].r_icu_admit_dt_tm
   IF (cnvtdatetime(pat_record->pat_data[d.seq].r_icu_disch_dt_tm)=cnvtdatetime(null_date))
    readmit->data[cnt].icu_los_act = icu_los_act_curr
   ELSE
    readmit->data[cnt].icu_los_act = icu_los_act
   ENDIF
   IF (cnvtdatetime(pat_record->pat_data[d.seq].r_icu_disch_dt_tm) != cnvtdatetime(null_date))
    IF ((pat_record->pat_data[d.seq].r_diedinicu_ind=1))
     readmit->data[cnt].icu_disch_stat = "D"
    ELSE
     readmit->data[cnt].icu_disch_stat = "A"
    ENDIF
   ELSE
    readmit->data[cnt].icu_disch_stat = ""
   ENDIF
 ...
 ;end select
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = readmit->cnt),
   encntr_alias ea
  PLAN (d)
   JOIN (ea
   WHERE (ea.encntr_id=readmit->data[d.seq].encntr_id)
    AND ea.encntr_alias_type_cd=mrn_cd
    AND ea.active_ind=1)
  DETAIL
   readmit->data[d.seq].mrn = cnvtalias(ea.alias,ea.alias_pool_cd)
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = pat_record->cnt),
  ...
  PLAN (d)
   ...
   WHERE (rao.risk_adjustment_day_id=pat_record->pat_data[d.seq].ra_risk_adjustment_day_id)
    AND rao.equation_name="NTL_ICU_DEATH"
 ...
 SELECT INTO rpt_params->output_device
  ...
  HEAD PAGE
   ...
   x_pos = 0, break_pos = 520, date_full_disp = fillstring(70," "),
   CALL print(calcpos(15,y_pos)), rpt_params->gen_on,
   CALL print(calcpos(575,y_pos)),
   "By Module: dcp_arpt_24_icu_reamdit_rev",
   CALL print(calcpos(352,y_pos)), "APACHE For ICU",
   row + 1, y_pos = (y_pos+ 10),
   CALL print(calcpos(358,y_pos)),
   "FOCUS Report", y_pos = (y_pos+ 10),
   CALL print(calcpos(15,y_pos)),
   font80c, row + 1, y_pos = (y_pos+ 10),
   CALL print(calcpos(310,y_pos)), "ICU Readmission Review", row + 1,
   CALL print(calcpos(15,y_pos)), font110c, row + 1,
   y_pos = (y_pos+ 15), len = size(rpt_params->date_type_range_disp,1), x_pos = (400 - (len * 2.5)),
   CALL print(calcpos(x_pos,y_pos)), rpt_params->date_type_range_disp, row + 1,
   y_pos = (y_pos+ 15), line = fillstring(100,"-"),
   CALL print(calcpos(155,y_pos)),
   line, row + 1, y_pos = (y_pos+ 15),
   len = size(rpt_params->org_name,1), x_pos = (400 - (len * 2.5)),
   CALL print(calcpos(x_pos,y_pos)),
   rpt_params->org_name, row + 1, y_pos = (y_pos+ 10),
   len = size(rpt_params->unit_disp,1), x_pos = (400 - (len * 2.5)),
   CALL print(calcpos(x_pos,y_pos)),
   rpt_params->unit_disp, y_pos = (y_pos+ 30), row + 1,
   line2 = fillstring(300,"-"), row + 1, y_pos = (y_pos+ 10),
   CALL print(calcpos(15,y_pos)), "Patient",
   CALL print(calcpos(90,y_pos)),
   "Patient",
   CALL print(calcpos(410,y_pos)), "Chronic",
   CALL print(calcpos(490,y_pos)), "ICU Admit",
   CALL print(calcpos(590,y_pos)),
   "ICU Admit",
   CALL print(calcpos(670,y_pos)), "ICU",
   CALL print(calcpos(700,y_pos)), "Dischg. Status", row + 1,
   y_pos = (y_pos+ 10),
   CALL print(calcpos(15,y_pos)), "Name",
   CALL print(calcpos(90,y_pos)), "ID",
   CALL print(calcpos(190,y_pos)),
   "Physician",
   CALL print(calcpos(260,y_pos)), "Service",
   CALL print(calcpos(380,y_pos)), "Age",
   CALL print(calcpos(410,y_pos)),
   "Health",
   CALL print(calcpos(490,y_pos)), "Diagnosis",
   CALL print(calcpos(605,y_pos)), "Date",
   CALL print(calcpos(670,y_pos)),
   "LOS",
   CALL print(calcpos(705,y_pos)), "ICU",
   CALL print(calcpos(735,y_pos)), "Hosp", row + 1,
   y_pos = (y_pos+ 10),
   CALL print(calcpos(15,y_pos)), line2
  DETAIL
   FOR (x = 1 TO readmit->cnt)
     row + 1, y_pos = (y_pos+ 10)
     IF (y_pos > break_pos
      AND (x < readmit->cnt))
      BREAK
     ENDIF
     doc_id_disp = format(readmit->data[x].doc_id,"###########;l"), age_disp = format(readmit->data[x
      ].age,"###;l"), icu_admit_dt_disp = format(readmit->data[x].icu_admit_dt,"dd-mmm-yyyy ;;d"),
     icu_los_act = format(readmit->data[x].icu_los_act,"###.#;r"), srv = substring(1,20,readmit->
      data[x].doc_srv), name = substring(1,15,readmit->data[x].last_name),
     mrn = substring(1,15,readmit->data[x].mrn),
     CALL print(calcpos(15,y_pos)), name,
     CALL print(calcpos(90,y_pos)), mrn,
     CALL print(calcpos(190,y_pos)),
     doc_id_disp,
     CALL print(calcpos(260,y_pos)), srv,
     CALL print(calcpos(380,y_pos)), age_disp,
     CALL print(calcpos(410,y_pos)),
     readmit->data[x].chi,
     CALL print(calcpos(490,y_pos)), readmit->data[x].admit_dx,
     CALL print(calcpos(580,y_pos)), icu_admit_dt_disp,
     CALL print(calcpos(660,y_pos)),
     icu_los_act,
     CALL print(calcpos(710,y_pos)), readmit->data[x].icu_disch_stat,
     CALL print(calcpos(740,y_pos)), readmit->data[x].hosp_disch_stat
   ENDFOR
   row + 3, y_pos = (y_pos+ 30)
   IF (((y_pos+ 40) > break_pos))
    BREAK
   ENDIF
   CALL print(calcpos(15,y_pos)), "Flagged patients..................:", surg_cnt = format(readmit->
    cnt,"#######;l"),
   col + 1, surg_cnt, row + 1,
   y_pos = (y_pos+ 10),
   CALL print(calcpos(15,y_pos)), "Total number of patients(see note):",
   pat_cnt = format(pat_record->cnt,"#######;l"), col + 1, pat_cnt,
   row + 1, y_pos = (y_pos+ 10), col 5,
   "Note: Includes all ICU patients during this time period.", row + 1, y_pos = (y_pos+ 10),
   CALL print(calcpos(15,y_pos)), "Percentage........................:", ftot_admit = 0.0,
   ftot_admit = pat_record->cnt, pct_ratio = ((readmit->cnt/ ftot_admit) * 100), pct_disp = format(
    pct_ratio,"###.##%;l"),
   col + 1, pct_disp, row + 3,
   y_pos = (y_pos+ 30)
  FOOT PAGE
   row + 1, curr_page = format(curpage,"###"), page_disp = concat("------  Page ",trim(curr_page),
    "  ------"),
   CALL print(calcpos(330,540)), page_disp
  WITH dio = postscript, maxcol = 700, maxrow = 80
 ;end select
END GO
