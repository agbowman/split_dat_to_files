CREATE PROGRAM bhs_rpt_prenatal_summary:dba
 PROMPT
 ...
 CALL echo(build2("mf_REQ_ORDER_CD: ",mf_req_order_cd))
 CALL echo(build2("mf_CARESET_cd: ",mf_careset_cd))
 CALL echo(build2("mf_tlab_cd: ",mf_tlab_cd))
 ...
 CALL echo(build2("mf_LAB_CATTYP_CD: ",mf_lab_cattyp_cd))
 ...
 WHILE (ml_cnt1 < 100
  AND ml_cnt1 != 100)
   ...
   IF (ms_tmp_str="1"
    AND ml_cnt1=1)
    SET stat = alterlist(m_rec->pat,ml_cnt1)
    SET m_rec->pat[ml_cnt1].f_person_id = cnvtreal(trim(ms_temp_plist,3))
   ELSEIF (textlen(ms_tmp_str) > 1
    AND ((ms_tmp_str != "1") OR (ms_tmp_str="1"
    AND ml_cnt1=1)) )
    SET stat = alterlist(m_rec->pat,ml_cnt1)
    SET m_rec->pat[ml_cnt1].f_person_id = cnvtreal(ms_tmp_str)
   ELSE
    SET ml_cnt1 = 101
   ENDIF
 ENDWHILE
 ...
 SELECT DISTINCT INTO "nl:"
  pi.person_id
  FROM (dummyt d  WITH seq = size(m_rec->pat,5)),
   pregnancy_instance pi,
   pregnancy_estimate pe
  PLAN (d)
   JOIN (pi
   WHERE (pi.person_id=m_rec->pat[d.seq].f_person_id)
    AND pi.active_ind=1
 ...
 SELECT INTO "nl:"
  ...
  HEAD dfr.definition
   pl_cnt = (pl_cnt+ 1),
   CALL alterlist(m_rec->dfr,pl_cnt), m_rec->dfr[pl_cnt].f_form_id = dfr.dcp_forms_ref_id,
   m_rec->dfr[pl_cnt].s_form_def = trim(dfr.definition,3), m_rec->dfr[pl_cnt].s_form_desc = trim(dfr
    .description,3)
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (dfa
   WHERE (dfa.person_id=m_rec->pat[d.seq].f_person_id)
    AND expand(ml_exp,1,size(m_rec->dfr,5),dfa.dcp_forms_ref_id,m_rec->dfr[ml_exp].f_form_id)
    AND dfa.flags > 0
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (d)
   JOIN (d1
   WHERE (prentl_sum->patient[d1.seq].mf_person_id=m_rec->pat[d.seq].f_person_id))
   JOIN (ce
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (d)
   JOIN (a
   WHERE (a.person_id=m_rec->pat[d.seq].f_person_id)
 ...
 CALL echo(build2("mf_careset_cd: ",mf_careset_cd))
 CALL echo(build2("mf_tlab_cd: ",mf_tlab_cd))
 CALL echo(format(cnvtdatetime(md_start_date),"mm/dd/yy;;d"))
 CALL echo(format(cnvtdatetime(md_end_dt_tm),"mm/dd/yy;;d"))
 SELECT DISTINCT INTO "nl:"
  o.catalog_cd
  FROM (dummyt d  WITH seq = size(m_rec->pat,5)),
   orders o,
  ...
  PLAN (d)
   ...
   WHERE ce.order_id=o.order_id
    AND ce.view_level=1
    AND ce.event_reltn_cd=132)
  ORDER BY d.seq, o.catalog_cd, ce.event_end_dt_tm DESC
  HEAD REPORT
   pl_p_cnt = 0
  HEAD d.seq
   pl_p_cnt = 0
  HEAD o.catalog_cd
   ...
   CALL echo(build2("lab: ",prentl_sum->patient[d.seq].lab_data[pl_p_cnt].ms_lab_name)), prentl_sum->
   patient[d.seq].lab_data[pl_p_cnt].ms_lab_rslt = substring(1,80,trim(ce.result_val)), prentl_sum->
   patient[d.seq].lab_data[pl_p_cnt].ms_lab_dt = substring(1,80,trim(format(o.orig_order_dt_tm,
      "dd-mmm-yyyy hh:mm;;d")))
  WITH nocounter
 ;end select
 ...
 SELECT DISTINCT INTO "nl"
  ...
  FROM problem p,
   nomenclature n,
   (dummyt d  WITH seq = size(m_rec->pat,5))
  PLAN (d)
   JOIN (p
   WHERE (p.person_id=m_rec->pat[d.seq].f_person_id)
    AND p.active_ind=1
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  FROM (dummyt d  WITH value(m_rec->pat,5)),
   (dummyt d1  WITH seq = size(prentl_sum->patient,5)),
   clinical_event ce
  PLAN (d)
   JOIN (ce
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   mf_edema_cd)
    ...
    AND ce.valid_until_dt_tm >= cnvtdatetime(curdate,curtime3)
    AND ce.result_status_cd IN (mf_auth_cd, mf_altered_cd, mf_modified_cd))
  ORDER BY d.seq, ce.clinical_event_id, ce.event_end_dt_tm_os DESC
  HEAD REPORT
   pl_ga_cnt = 0, pl_height_cnt = 0, pl_pres_cnt = 0,
   pl_fhr_cnt = 0, pl_cerv_ex_cnt = 0, pl_sbp_cnt = 0,
   pl_dbp_cnt = 0, pl_weight_cnt = 0, pl_edema_cnt = 0,
   pl_add = 0
  HEAD d.seq
   pl_ga_cnt = 0, pl_height_cnt = 0, pl_pres_cnt = 0,
   pl_fhr_cnt = 0, pl_cerv_ex_cnt = 0, pl_sbp_cnt = 0,
   pl_dbp_cnt = 0, pl_weight_cnt = 0, pl_edema_cnt = 0,
   pl_add = 0, pl_h_cnt = 1, md_hold_dt = ce.event_end_dt_tm_os
  HEAD ce.clinical_event_id
   null
  HEAD ce.event_end_dt_tm_os
   pl_add = 0, stat = alterlist(prentl_sum->patient[d.seq].visit_summary,pl_h_cnt)
   CASE (ce.task_assay_cd)
    OF mf_gasage_cd:
     pl_ga_cnt = (pl_ga_cnt+ 1),
     IF (pl_ga_cnt < 5)
      pl_add = 1, prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_gest_age = trim(ce.result_val
       )
     ENDIF
    OF mf_fundalhg_cd:
     pl_height_cnt = (pl_height_cnt+ 1),
     IF (pl_height_cnt < 5)
      pl_add = 1, prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_fndal_hght = trim(ce
       .result_val)
     ENDIF
    OF mf_presentation_cd:
     pl_pres_cnt = (pl_pres_cnt+ 1),
     IF (pl_pres_cnt < 5)
      pl_add = 1, prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_presentation = trim(ce
       .result_val)
     ENDIF
    OF mf_fhr_cd:
     pl_fhr_cnt = (pl_fhr_cnt+ 1),
     IF (pl_fhr_cnt < 5)
      pl_add = 1, prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_fhr = trim(ce.result_val)
     ENDIF
    OF mf_cervdilation_cd:
    OF mf_cerveffacent_cd:
    OF mf_cervstation_cd:
     pl_cerv_ex_cnt = (pl_cerv_ex_cnt+ 1),
     IF (pl_cerv_ex_cnt < 5)
      pl_add = 1, ms_cerv_data = concat(ms_cerv_data,trim(ce.result_val)), prentl_sum->patient[d.seq]
      .visit_summary[pl_h_cnt].ms_cervical_exam = ms_cerv_data
     ENDIF
    OF mf_systolic_cd:
     pl_sbp_cnt = (pl_sbp_cnt+ 1),
     IF (pl_sbp_cnt < 5)
      pl_add = 1, prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_systolic_bp = trim(ce
       .result_val)
     ENDIF
    OF mf_diastolic_cd:
     pl_dbp_cnt = (pl_dbp_cnt+ 1),
     IF (pl_dbp_cnt < 5)
      pl_add = 1, prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_diastolic_bp = trim(ce
       .result_val)
     ENDIF
    OF mf_weight_cd:
     pl_weight_cnt = (pl_weight_cnt+ 1),
     IF (pl_weight_cnt < 5)
      pl_add = 1, prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_weight_kg = trim(ce
       .result_val)
     ENDIF
    OF mf_edema_cd:
     pl_edema_cnt = (pl_edema_cnt+ 1),
     IF (pl_edema_cnt < 5)
      pl_add = 1, prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_edema = trim(ce.result_val)
     ENDIF
   ENDCASE
   CALL echo(build2("ga: ",trim(cnvtstring(pl_ga_cnt),3)," ","ht: ",trim(cnvtstring(pl_height_cnt),3),
    " ","pr: ",trim(cnvtstring(pl_pres_cnt),3)," ","fr: ",
    trim(cnvtstring(pl_fhr_cnt),3)," ","cv: ",trim(cnvtstring(pl_cerv_ex_cnt),3)," ",
    "sp: ",trim(cnvtstring(pl_sbp_cnt),3)," ","dp: ",trim(cnvtstring(pl_dbp_cnt),3),
    " ","wt: ",trim(cnvtstring(pl_weight_cnt),3)," ","ed: ",
    trim(cnvtstring(pl_edema_cnt),3)," ","add:",trim(cnvtstring(pl_add),3)))
   IF (((pl_ga_cnt < 5) OR (((pl_height_cnt < 5) OR (((pl_pres_cnt < 5) OR (((pl_fhr_cnt < 5) OR (((
   pl_cerv_ex_cnt < 5) OR (((pl_sbp_cnt < 5) OR (((pl_dbp_cnt < 5) OR (((pl_weight_cnt < 5) OR (
   pl_edema_cnt < 5)) )) )) )) )) )) )) ))
    AND pl_add=1)
    prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_vdate = format(ce.event_end_dt_tm,
     "MM/DD/YYYY;;d")
    IF (md_hold_dt != ce.event_end_dt_tm)
     md_hold_dt = ce.event_end_dt_tm, pl_h_cnt = (pl_h_cnt+ 1)
    ENDIF
   ENDIF
  FOOT REPORT
   IF (textlen(trim(prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_gest_age,3))=0
    AND textlen(trim(prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_fndal_hght,3))=0
    AND textlen(trim(prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_presentation,3))=0
    AND textlen(trim(prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_fhr,3))=0
    AND textlen(trim(prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_cervical_exam,3))=0
    AND textlen(trim(prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_systolic_bp,3))=0
    AND textlen(trim(prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_diastolic_bp,3))=0
    AND textlen(trim(prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_weight_kg,3))=0
    AND textlen(trim(prentl_sum->patient[d.seq].visit_summary[pl_h_cnt].ms_edema,3))=0)
    CALL alterlist(prentl_sum->patient[d.seq].visit_summary,(pl_h_cnt - 1))
   ENDIF
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (d1
   WHERE (work_form->forms[d1.seq].mf_person_id=m_rec->pat[d.seq].f_person_id))
   JOIN (ce
 ...
 SUBROUTINE tablerow1abs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   IF (ncalc=rpt_render)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.063),offsety,(offsetx+ 0.063),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.885),offsety,(offsetx+ 0.885),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 1.615),offsety,(offsetx+ 1.615),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 2.344),offsety,(offsetx+ 2.344),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 3.073),offsety,(offsetx+ 3.073),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 3.802),offsety,(offsetx+ 3.802),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 4.531),offsety,(offsetx+ 4.531),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 5.260),offsety,(offsetx+ 5.260),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 5.990),offsety,(offsetx+ 5.990),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 6.719),offsety,(offsetx+ 6.719),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 7.438),offsety,(offsetx+ 7.438),(offsety+
     sectionheight))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.063),(offsety+ 0.000),(offsetx+ 7.438),(offsety+
     0.000))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.063),(offsety+ sectionheight),(offsetx+ 7.438),(
     offsety+ sectionheight))
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
