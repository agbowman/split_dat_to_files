CREATE PROGRAM dm_rc_f_mthly_census_smry_rows:dba
 IF ( NOT (validate(script_version)))
  DECLARE script_version = vc WITH constant("326978.002"), private
 ENDIF
 ...
 SET reply->table_name = "RC_F_MONTHLY_CENSUS_SMRY"
 IF ( NOT (getlogicaldomain(ld_concept_organization,logicaldomainid)))
  CALL echo("Could not retrieve logical domain")
  GO TO exit_script
 ENDIF
 ...
 DECLARE activitymonth = i4 WITH noconstant(0)
 DECLARE activityyear = i4 WITH noconstant(0)
 DECLARE backdays = i4 WITH noconstant(0)
 DECLARE i18nhandle = i4 WITH noconstant(0)
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 SET v_err_code2 = error(v_errmsg2,0)
 IF (v_err_code2 != 0)
  SET reply->err_code = v_err_code2
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"SELECTERROR","Failed during select:%1","s",
   nullterm(v_errmsg2))
  GO TO exit_script
 ENDIF
 ...
 FOR (tok_ndx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 370)
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"DAYSTOKEEP",
   "You must keep at least 370 Days' worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
 ELSE
  SET backdays = (curdate - v_days_to_keep)
  SET activitymonth = month(backdays)
  SET activityyear = year(backdays)
  SELECT INTO "nl:"
   FROM rc_f_monthly_census_smry r
   WHERE r.shr_d_month_id < activitymonth
    AND r.activity_year < activityyear
    AND ((r.rc_f_monthly_census_smry_id+ 0) != 0)
    AND r.mill_logical_domain_id=logicaldomainid
   HEAD REPORT
    row_cnt = 0
   DETAIL
    row_cnt += 1
