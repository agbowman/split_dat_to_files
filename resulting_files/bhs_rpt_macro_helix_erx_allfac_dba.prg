CREATE PROGRAM bhs_rpt_macro_helix_erx_allfac:dba
 PROMPT
  "Facility:" = 0,
  "Begin Date" = "CURDATE",
  "End Date" = "CURDATE"
 ...
 ELSE
  SET ms_file_name = concat("_eprescribe_",trim(format(sysdate,"YYYYMMDD;;d")),".txt")
  ...
  ENDIF
 ENDIF
 SET ms_file_name = "je_test_allfac.txt"
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   m_rec->erx[pl_cnt].s_pat_name_last = trim(p.name_last_key,3), m_rec->erx[pl_cnt].s_pat_name_first
    = trim(p.name_first_key,3), m_rec->erx[pl_cnt].s_pat_dob = format(cnvtdatetimeutc(datetimezone(p
      .birth_dt_tm,p.birth_tz),1),"mm/dd/yy;;d"),
 ...
 IF (size(m_rec->erx,5) > 0)
  ...
  SET frec->file_buf = concat("Patient Last Name|Patient First Name|DOB|Zip Code|MRN|","Facility|",
   ...
   "Diagnosis Code|Designation for ICD9 or ICD10|Primary Diagnosis","|Location",char(10))
  SET stat = cclio("WRITE",frec)
  FOR (ml_loop = 1 TO size(m_rec->erx,5))
    IF ((m_rec->erx[ml_loop].n_skip=0))
     IF (textlen(trim(m_rec->erx[ml_loop].s_written_loc,3)) > 0)
      SET ms_tmp = concat(m_rec->erx[ml_loop].s_facility," ",m_rec->erx[ml_loop].s_written_loc)
     ELSEIF (textlen(trim(m_rec->erx[ml_loop].s_written_loc,3))=0
      AND textlen(trim(m_rec->erx[ml_loop].s_written_loc_desc,3)) > 0)
      SET ms_tmp = concat(m_rec->erx[ml_loop].s_facility," ",m_rec->erx[ml_loop].s_written_loc_desc)
     ELSE
      SET ms_tmp = m_rec->erx[ml_loop].s_facility
     ENDIF
     SET ms_line = concat(m_rec->erx[ml_loop].s_pat_name_last,"|",m_rec->erx[ml_loop].
      ...
      m_rec->erx[ml_loop].s_facility,"|",m_rec->erx[ml_loop].s_pharm_npi,"|",m_rec->erx[ml_loop].
      s_pharm_ncpdp,
      "|",m_rec->erx[ml_loop].s_prov_npi,"|",m_rec->erx[ml_loop].s_prov_dea,"|",
      m_rec->erx[ml_loop].s_pres_name_last,"|",m_rec->erx[ml_loop].s_pres_name_first,"|",m_rec->erx[
      ml_loop].s_written_loc,
      "|",m_rec->erx[ml_loop].s_written_loc_desc,"|",m_rec->erx[ml_loop].s_written_dt,"|",
      m_rec->erx[ml_loop].s_written_tm,"|",trim(cnvtstring(m_rec->erx[ml_loop].f_order_id),3),"|",
      m_rec->erx[ml_loop].s_order_status,
      "|",m_rec->erx[ml_loop].s_ndc_desc,"|",m_rec->erx[ml_loop].s_drug_desc,"|",
      m_rec->erx[ml_loop].s_drug_qty,"|",m_rec->erx[ml_loop].s_drug_ndc,"|",m_rec->erx[ml_loop].s_fin,
      "|",m_rec->erx[ml_loop].s_diag_cd,"|",m_rec->erx[ml_loop].s_diag_src,"|",
      m_rec->erx[ml_loop].s_diag_prim,"|",ms_tmp,char(10))
