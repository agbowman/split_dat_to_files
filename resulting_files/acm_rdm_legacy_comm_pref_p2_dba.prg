CREATE PROGRAM acm_rdm_legacy_comm_pref_p2:dba
 ...
 SET ms_info_domain_nm = "ACM_RDM_MIGRATE_PERSON_PATIENT_TO_PERSON_PREF_COMM"
 SET ms_range = "ACM_RDM_MIGRATE_PERSON_PATIENT_TO_PERSON_PREF_COMM_RANGE"
 SET ms_max_name_p = "MAX PERSON_ID EVALUATED"
 SET ms_child_script = "acm_rdm_legacy_comm_pref_cc"
 SET mn_num_children = 5.0
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
 ...
 ENDIF
 SELECT INTO "nl:"
  max_val = max(p.person_id)
  FROM person_patient p
  WHERE p.person_id > 0
   AND p.active_ind=1
   AND p.active_status_cd=dactivecd
  DETAIL
...
#exit_program
 ...
 ELSE
  COMMIT
  CALL sbr_add_dropped_object("INDEX","TEMPPPCINDEX","Dropping index")
  CALL sbr_add_dropped_object("TABLE","TEMP_RDM_MIGRATE_PPC","Dropping table")
