CREATE PROGRAM dcp_mu_ip_pat_list_data_set:dba
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    (SELECT
     cv.code_value
     FROM code_value cv
     WHERE cv.cki="CKI.CODEVALUE!4977"
      AND cv.active_ind=1)))
    JOIN (l
    ...
    (SELECT
     ...
     AND l.active_ind=1)
   ORDER BY uar_get_code_description(l.location_cd)
   HEAD REPORT
    org_cnt = 0, stat = makedataset(10), location_cd = addrealfield("LOCATION_CD","LOCATION_CD",1),
    disp = addstringfield("DISP","DISP",1,100)
   HEAD l.location_cd
    org_cnt = getnextrecord(0)
   DETAIL
    stat = setrealfield(org_cnt,location_cd,l.location_cd), stat = setstringfield(org_cnt,disp,
     uar_get_code_description(l.location_cd))
   FOOT REPORT
    stat = closedataset(0)
   WITH check
  ;end select
