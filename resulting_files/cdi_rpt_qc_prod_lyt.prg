CREATE PROGRAM cdi_rpt_qc_prod_lyt
 PROMPT
 ...
 RECORD batch_lyt(
   1 batch_details[*]
     ...
     2 processeddocs = i4
     2 processedpages = i4
   1 totalprocesseddocs = i4
   1 totalprocessedpages = i4
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 EXECUTE cdi_rpt_qc_prod_drvr  $BEGINDATE,  $ENDDATE
 ...
 SUBROUTINE batch_query(dummy)
   ...
   SELECT INTO "NL:"
    ...
    batch_lyt_batch_details_processeddocs = batch_lyt->batch_details[dtrs1.seq].processeddocs,
    batch_lyt_batch_details_processedpages = batch_lyt->batch_details[dtrs1.seq].processedpages,
    batch_lyt_batch_details_totalprocesseddocs = batch_lyt->totalprocesseddocs,
    batch_lyt_batch_details_totalprocessedpages = batch_lyt->totalprocessedpages
    FROM (dummyt dtrs1  WITH seq = value(size(batch_lyt->batch_details,5)))
    ORDER BY batch_lyt_batch_details_username
    HEAD REPORT
     _d0 = batch_lyt_batch_details_username, _d1 = batch_lyt_batch_details_processeddocs, _d2 =
     batch_lyt_batch_details_processedpages,
     _fenddetail = ((rptreport->m_pageheight - rptreport->m_marginbottom) - footpagesection(
      rpt_calcheight))
     IF (((_yoffset+ headreportsection(rpt_calcheight)) > (rptreport->m_pageheight - rptreport->
     m_marginbottom)))
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "HeadReportSection_title","ProVision Document Imaging"),char(0)))
   ENDIF
   ...
   IF (ncalc=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "HeadReportSection_title1","Quality Control Productivity"),char(0)))
   ENDIF
 ...
 SUBROUTINE headpagesectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "HeadPageSection_FieldName01","Pages Processed"),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "HeadPageSection_FieldName02","Documents Processed"),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_y = (offsety+ 0.00)
 ...
 SUBROUTINE detailsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(
        batch_lyt_batch_details_processeddocs),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(
        batch_lyt_batch_details_processedpages),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_flags = 4
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   SET rptsd->m_flags = 36
   SET rptsd->m_y = (offsety+ 0.00)
   SET rptsd->m_x = (offsetx+ 3.00)
   SET rptsd->m_width = 0.75
   SET rptsd->m_height = 0.25
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build(format(((100.00 *
         batch_lyt_batch_details_processeddocs)/ batch_lyt->totalprocesseddocs),"####.##;I"),"%"),
       char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   SET rptsd->m_x = (offsetx+ 5.75)
   SET rptsd->m_width = 0.75
   SET rptsd->m_height = 0.25
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build(format(((100.00 *
         batch_lyt_batch_details_processedpages)/ batch_lyt->totalprocessedpages),"####.##;I"),"%"),
       char(0)))
    ENDIF
   ENDIF
   SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
 ...
 SUBROUTINE footreportsectionabs(ncalc,offsetx,offsety)
   ...
   SET rptsd->m_y = (offsety+ 0.00)
   SET rptsd->m_x = (offsetx+ 5.25)
   SET rptsd->m_width = 2.25
   SET rptsd->m_height = 0.25
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(batch_lyt->totalprocessedpages,
        " Pages"),char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   SET rptsd->m_x = (offsetx+ 2.25)
   SET rptsd->m_width = 2.75
   SET rptsd->m_height = 0.25
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(batch_lyt->totalprocesseddocs,
        " Documents"),char(0)))
    ENDIF
   ENDIF
   SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CDI_RPT_QC_PROD_LYT"
