CREATE PROGRAM acm_chg_service_category_hist:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->service_category_hist_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
  ...
  SET s.svc_cat_hist_id = acm_request->service_category_hist_qual[xref->chg[d.seq].idx].
   svc_cat_hist_id, s.attend_prsnl_id = acm_request->service_category_hist_qual[xref->chg[d.seq].idx]
   .attend_prsnl_id, s.beg_effective_dt_tm = cnvtdatetime(acm_request->service_category_hist_qual[
    xref->chg[d.seq].idx].beg_effective_dt_tm),
   s.encntr_id =
   IF ((acm_request->service_category_hist_qual[xref->chg[d.seq].idx].encntr_id > 0)) acm_request->
    service_category_hist_qual[xref->chg[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->service_category_hist_qual[xref->chg[d.seq].idx].
    encntr_idx].encntr_id
   ENDIF
   , s.end_effective_dt_tm = cnvtdatetime(acm_request->service_category_hist_qual[xref->chg[d.seq].
    idx].end_effective_dt_tm), s.med_service_cd = acm_request->service_category_hist_qual[xref->chg[d
   .seq].idx].med_service_cd,
   s.service_category_cd = acm_request->service_category_hist_qual[xref->chg[d.seq].idx].
   service_category_cd, s.transaction_dt_tm =
   IF ((acm_request->service_category_hist_qual[xref->chg[d.seq].idx].transaction_dt_tm > 0))
    cnvtdatetime(acm_request->service_category_hist_qual[xref->chg[d.seq].idx].transaction_dt_tm)
   ELSE null
   ENDIF
   , s.active_ind = acm_request->service_category_hist_qual[xref->chg[d.seq].idx].active_ind,
   s.active_status_cd = acm_request->service_category_hist_qual[xref->chg[d.seq].idx].
   active_status_cd, s.active_status_prsnl_id = active_status_prsnl_id, s.active_status_dt_tm =
   cnvtdatetime(active_status_dt_tm),
   s.updt_cnt = (s.updt_cnt+ 1), s.updt_dt_tm = cnvtdatetime(sysdate), s.updt_id = reqinfo->updt_id,
   s.updt_applctx = reqinfo->updt_applctx, s.updt_task = reqinfo->updt_task
  PLAN (d)
   JOIN (s
   WHERE (s.svc_cat_hist_id=acm_request->service_category_hist_qual[xref->chg[d.seq].idx].
   svc_cat_hist_id))
  WITH nocounter, status(reply->service_category_hist_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->service_category_hist_qual[xref->chg[index].idx].status != 1))
    SET failed = update_error
    SET table_name = "SERVICE_CATEGORY_HIST"
    GO TO exit_script
   ENDIF
 ENDFOR
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,s.svc_cat_hist_id,acm_request->service_category_hist_qual[xref->chg[
     t1].idx].svc_cat_hist_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,s.svc_cat_hist_id,acm_request->service_category_hist_qual[xref->
      chg[t1].idx].svc_cat_hist_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((s.updt_cnt=acm_request->service_category_hist_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->service_category_hist_qual[idx].status = - (1), reply->service_category_hist_qual[idx].
      svc_cat_hist_id = acm_request->service_category_hist_qual[idx].svc_cat_hist_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->service_category_hist_qual[idx].chg_str
     IF (findstring("ATTEND_PRSNL_ID,",chg_str)=0)
      acm_request->service_category_hist_qual[idx].attend_prsnl_id = s.attend_prsnl_id
     ENDIF
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->service_category_hist_qual[idx].beg_effective_dt_tm = s.beg_effective_dt_tm
     ENDIF
     IF (findstring("ENCNTR_ID,",chg_str)=0)
      acm_request->service_category_hist_qual[idx].encntr_id = s.encntr_id
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->service_category_hist_qual[idx].end_effective_dt_tm = s.end_effective_dt_tm
     ENDIF
     IF (findstring("MED_SERVICE_CD,",chg_str)=0)
      acm_request->service_category_hist_qual[idx].med_service_cd = s.med_service_cd
     ENDIF
     IF (findstring("SERVICE_CATEGORY_CD,",chg_str)=0)
      acm_request->service_category_hist_qual[idx].service_category_cd = s.service_category_cd
     ENDIF
     IF (findstring("TRANSACTION_DT_TM,",chg_str)=0)
      acm_request->service_category_hist_qual[idx].transaction_dt_tm = s.transaction_dt_tm
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->service_category_hist_qual[idx].active_ind = s.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->service_category_hist_qual[idx].active_status_cd = s.active_status_cd
     ENDIF
     ...
     ELSE
      active_status_prsnl_id = s.active_status_prsnl_id, active_status_dt_tm = cnvtdatetime(s
       .active_status_dt_tm)
     ENDIF
    WITH nocounter, forupdatewait(s), time = 5
   ;end select
   IF (failed)
    SET table_name = "SERVICE_CATEGORY_HIST"
    GO TO exit_script
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->service_category_hist_qual[xref->chg[index].idx].status=0))
