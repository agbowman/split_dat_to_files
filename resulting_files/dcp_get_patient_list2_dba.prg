CREATE PROGRAM dcp_get_patient_list2:dba
 RECORD reply(
 ...
 SET remove_option = request->rmv_pl_rows_flag
 CASE (listtype)
  OF "CUSTOM":
 ...
 SUBROUTINE assignremoveindicator(x)
   SELECT INTO "nl:"
    FROM dcp_pl_prioritization p,
     (dummyt d1  WITH seq = value(size(reply->patients,5)))
    PLAN (d1)
     JOIN (p
     WHERE (p.patient_list_id=request->patient_list_id)
      AND (p.person_id=reply->patients[d1.seq].person_id)
      AND (p.encntr_id=reply->patients[d1.seq].encntr_id)
      AND p.remove_ind=1)
    DETAIL
     reply->patients[d1.seq].remove_ind = p.remove_ind, reply->patients[d1.seq].remove_dt_tm =
     cnvtdatetime(p.remove_dt_tm)
    WITH nocounter
   ;end select
 END ;Subroutine
 EXECUTE cclaudit 0, "Query List", "Patient",
 "Other", "Query", "Patient List",
 "Access/Use", request->patient_list_id, request->patient_list_name
 ...
 SUBROUTINE logstatistics(seconds)
   ...
   ENDIF
 END ;Subroutine
 SET script_version = "MOD 006 09/15/09 ms5566"
#error
END GO
