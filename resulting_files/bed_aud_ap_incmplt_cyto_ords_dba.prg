CREATE PROGRAM bed_aud_ap_incmplt_cyto_ords:dba
  ...
  SELECT DISTINCT INTO "NL:"
   ...
   PLAN (oc
    ...
    WHERE dta.task_assay_cd=ptr.task_assay_cd
     AND dta.default_result_type_cd IN (interp_type_cd, alpha_type_cd)
     AND dta.active_ind=1)
    JOIN (crc
    WHERE crc.catalog_cd=outerjoin(oc.catalog_cd))
 ...
 SELECT DISTINCT INTO "NL:"
  ...
  HEAD oc.catalog_cd
   ...
   IF (crc.catalog_cd=0)
    tcnt = (tcnt+ 1), stat = alterlist(temp->tqual,tcnt), temp->tqual[tcnt].primary_synonym = oc
    .primary_mnemonic
   ENDIF
  WITH nocounter
 ;end select
 ...
 SET reply->statlist[1].statistic_meaning = "APRPTINCMPLTCYTO"
...
#exit_script
 ...
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("ap_reports_missing_cyto_params.csv")
 ENDIF
