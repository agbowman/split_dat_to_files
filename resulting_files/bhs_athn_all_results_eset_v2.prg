CREATE PROGRAM bhs_athn_all_results_eset_v2
 RECORD orequest(
   1 query_mode = i4
   1 query_mode_ind = i2
   1 event_set_cd = f8
 ...
 SELECT INTO "nl:"
  ...
  PLAN (vesc
   WHERE (vesc.event_set_name= $6))
  HEAD REPORT
   orequest->event_set_cd = vesc.event_set_cd
  WITH nocounter, time = 30
 ;end select
 ...
 FOR (i = 1 TO size(out_rec->children,5))
   FOR (j = 1 TO size(out_rec->children[i].event_codes,5))
     IF ((size(out_rec->children[i].event_codes[j].event_list,5) >  $9)
      AND ( $9 > 0))
      SET stat = alterlist(out_rec->children[i].event_codes[j].event_list, $9)
     ENDIF
   ENDFOR
 ENDFOR
#end_prog
