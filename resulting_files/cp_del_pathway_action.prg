CREATE PROGRAM cp_del_pathway_action
 IF (validate(action_del_request) != 1)
  RECORD action_del_request(
    1 qual[*]
 ...
 RECORD action_detail_list(
   1 qual[*]
     2 cp_pathway_action_id = f8
     2 cp_pathway_action_detail_id = f8
 )
 ...
 SET action_del_reply->status_data.status = "F"
 CALL verifyinputs(null)
 CALL deletedetailrows(null)
 CALL deleterows(null)
 SET action_del_reply->status_data.status = "S"
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE actionlistsize = i4 WITH protect, noconstant(size(action_del_request->qual,5))
   IF (actionlistsize <= 0)
    CALL reporterrorandexit("PERFORM","Validate","Component list must be nonzero in size.")
   ENDIF
   FOR (x = 1 TO actionlistsize)
    SELECT INTO "NL:"
     FROM cp_pathway_action cpa
     WHERE (cpa.cp_pathway_action_id=action_del_request->qual[x].cp_pathway_action_id)
     WITH nocounter
    ;end select
    IF (curqual=0)
     CALL reporterrorandexit("PERFORM","Validate","cp_pathway_action_id is invalid.")
    ENDIF
   ENDFOR
 ...
 SUBROUTINE deleterows(null)
   ...
   ENDIF
   SET stat = alterlist(action_del_reply->component_list,size(action_del_request->qual,5))
   FOR (x = 1 TO size(action_del_request->qual,5))
     SELECT INTO "nl:"
      FROM cp_pathway_action cc
      WHERE (cc.cp_pathway_action_id=action_del_request->qual[x].cp_pathway_action_id)
      WITH nocounter
     ;end select
     IF (curqual=0)
      CALL reporterror("PERFORM","RETRIEVE",
       " Unable to find cp_pathway_action_ID on cp_pathway_action table.")
      ROLLBACK
      GO TO exit_script
     ENDIF
     DELETE  FROM cp_pathway_action cpa
      WHERE (cpa.cp_pathway_action_id=action_del_request->qual[x].cp_pathway_action_id)
      WITH nocounter
     ;end delete
     IF (curqual=0)
      CALL reporterror("PERFORM","DELETE","Unable to delete row from cp_pathway_action table.")
      ROLLBACK
      GO TO exit_script
     ELSE
      SET action_del_reply->status_data.status = "S"
 ...
 SUBROUTINE deletedetailrows(null)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE x = i4 WITH noconstant(0), protect
   IF (checkdic("CP_PATHWAY_ACTION_DETAIL","T",0) != 2)
    CALL reporterrorandexit("PERFORM","INSERT",
     " CP_PATHWAY_ACTION_DETAIL does not exist or permission is not granted to access it.")
   ENDIF
   SET stat = alterlist(action_del_reply->component_list,size(action_del_request->qual,5))
   FOR (x = 1 TO size(action_del_request->qual,5))
     DELETE  FROM cp_pathway_action_detail cpa
      WHERE (cpa.cp_pathway_action_id=action_del_request->qual[x].cp_pathway_action_id)
      WITH nocounter
     ;end delete
   ENDFOR
 ...
 SUBROUTINE (reporterror(operationname=vc,targetobjectname=vc,targetobjectvalue=vc) =null WITH
  protect)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SET action_del_reply->status_data.status = "F"
   SET action_del_reply->status_data.subeventstatus.operationname = operationname
   SET action_del_reply->status_data.subeventstatus.operationstatus = "F"
   SET action_del_reply->status_data.subeventstatus.targetobjectname = targetobjectname
   SET action_del_reply->status_data.subeventstatus.targetobjectvalue = targetobjectvalue
