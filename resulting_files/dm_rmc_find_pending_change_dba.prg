CREATE PROGRAM dm_rmc_find_pending_change:dba
 ...
 SUBROUTINE drcm_get_pk_str(dgps_rec,dgps_tab_name)
   ...
   RETURN(dgps_ret_str)
 END ;Subroutine
 DECLARE pwi_ret = c1 WITH protect, noconstant("")
 DECLARE drfpc_r_tab_src = vc WITH protect, noconstant("")
 DECLARE drfpc_ut_src = vc WITH protect, noconstant("")
 DECLARE drfpc_temp_idx = i2 WITH protect, noconstant(0)
 DECLARE drfpc_doc_idx = i4 WITH protect, noconstant(0)
 DECLARE drfpc_r_tab = vc WITH protect, noconstant("")
 DECLARE check_blocking(i_target_env_id=f8,i_source_env_id=f8,i_data=vc(ref),i_r_tab_name=vc,
  i_table_suffix=vc,
  i_pk_where=vc,i_del_check=i2,i_ins_check=i2) = f8
 DECLARE check_null_space(i_pkw_info=vc(ref),i_col_data_rec_name=vc,i_ref_data_doc=vc(ref),i_tbl_cnt=
  i4,i_del_ind=i2) = vc
 DECLARE changed_ui(cu_rs_string=vc,cui_tab_idx=vc,cui_del_ind=i2) = vc
 DECLARE check_iu_pkwhere(cip_iu_string=vc,cip_tab_name=vc,cip_data=vc(ref)) = i2
 FREE RECORD pwi_reply
 RECORD pwi_reply(
   1 data[*]
     2 table_name = vc
     2 pkw_function = vc
 ...
 IF ((request->table_name="SEG_GRP_SEQ_R"))
  SELECT INTO "nl:"
   FROM dm_chg_log d
   WHERE ((d.target_env_id+ 0)=request->source_env_id)
    AND (d.table_name=request->table_name)
    AND d.log_type IN ("REFCHG", "NORDDS", "PROCES")
   DETAIL
    reply->blocking_log_id = d.log_id, reply->nomv_type = "NOMV19"
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_drfpc
  ENDIF
  SET reply->reason_text = concat("A row was detected in TARGET with a LOG_ID of ",trim(cnvtstring(
     reply->blocking_log_id,20,1))," which represents a change. Row will be blocked from merging.")
  IF (curqual=0)
   SET drfpc_r_tab_src = cutover_tab_name(request->table_name,request->table_suffix)
   SET drfpc_ut_src = dm2_get_rdds_tname("USER_TABLES")
   SELECT INTO "nl:"
    FROM (parser(drfpc_ut_src) u)
    WHERE u.table_name=drfpc_r_tab_src
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual > 0)
    SET drfpc_r_tab_src = dm2_get_rdds_tname(drfpc_r_tab_src)
    SELECT INTO "nl:"
     FROM (parser(drfpc_r_tab_src) d)
     WHERE d.rdds_status_flag < 9000
      AND (d.rdds_source_env_id=request->target_env_id)
     DETAIL
      reply->blocking_log_id = d.rdds_log_id, reply->nomv_type = "NOMV20"
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_drfpc
    ENDIF
    SET drfpc_r_tab = cutover_tab_name(request->table_name,request->table_suffix)
    SET reply->reason_text = concat("A row in the ",drfpc_r_tab,
     " table in SOURCE with a RDDS_LOG_ID of ",trim(cnvtstring(reply->blocking_log_id,20,1)),
     ", represents a blocking change.")
   ENDIF
  ENDIF
 ELSEIF ((request->delete_ind=0))
  SET pwi_ret = pk_where_info(request->table_name,request->table_suffix,concat("RS_",request->
    table_suffix,"->TO_VALUES"),"",pwi_reply)
  IF (pwi_ret="F")
   SET reply->nomv_type = "NOMV99"
   SET reply->reason_text = dm_err->emsg
   GO TO exit_drfpc
  ENDIF
  SET pwi_reply->data[1].pkw_iu_str = check_null_space(pwi_reply,concat("RS_",request->table_suffix,
    "->TO_VALUES"),dm2_ref_data_doc,request->tbl_cnt,0)
  SET pwi_reply->data[1].pkw_del_str = check_null_space(pwi_reply,concat("RS_",request->table_suffix,
    "->TO_VALUES"),dm2_ref_data_doc,request->tbl_cnt,1)
  SET reply->blocking_log_id = check_blocking(request->target_env_id,request->source_env_id,pwi_reply,
   "","",
   "",0,0)
  IF ((reply->blocking_log_id > 0))
   SET reply->nomv_type = "NOMV19"
   SET reply->reason_text = concat("A row was detected in TARGET with a LOG_ID of ",trim(cnvtstring(
      reply->blocking_log_id,20,1))," which represents a change. Row will be blocked from merging.")
   GO TO exit_drfpc
  ELSE
   ...
   IF (curqual > 0)
    SET drfpc_r_tab_src = dm2_get_rdds_tname(drfpc_r_tab_src)
    SET pwi_ret = pk_where_info(request->table_name,request->table_suffix,concat("RS_",request->
      table_suffix,"->FROM_VALUES"),"",pwi_reply)
    IF (pwi_ret="F")
     SET reply->nomv_type = "NOMV99"
     SET reply->reason_text = dm_err->emsg
     GO TO exit_drfpc
    ENDIF
    SET pwi_reply->data[1].pkw_iu_str = check_null_space(pwi_reply,concat("RS_",request->table_suffix,
      "->FROM_VALUES"),dm2_ref_data_doc,request->tbl_cnt,0)
    SET pwi_reply->data[1].pkw_del_str = check_null_space(pwi_reply,concat("RS_",request->
      table_suffix,"->FROM_VALUES"),dm2_ref_data_doc,request->tbl_cnt,1)
    SET reply->blocking_log_id = check_blocking(request->target_env_id,request->source_env_id,
     pwi_reply,drfpc_r_tab_src,request->table_suffix,
     "",0,0)
    IF ((reply->blocking_log_id > 0))
     SET reply->nomv_type = "NOMV20"
     SET drfpc_r_tab = cutover_tab_name(request->table_name,request->table_suffix)
     SET reply->reason_text = concat("A row in the ",drfpc_r_tab,
      " table in SOURCE with a RDDS_LOG_ID of ",trim(cnvtstring(reply->blocking_log_id,20,1)),
      ", represents a blocking change.")
   ...
   ENDIF
   IF ((reply->blocking_log_id=0))
    IF ((((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].version_ind=1)
     AND (dm2_ref_data_doc->tbl_qual[request->tbl_cnt].version_type IN ("ALG1", "ALG3", "ALG3D",
    "ALG6"))) OR ((((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].merge_delete_ind=1)) OR ((
    dm2_ref_data_doc->tbl_qual[request->tbl_cnt].table_name IN ("DCP_SECTION_REF", "DCP_FORMS_REF"))
    )) )) )
     SET reply->nomv_type = changed_ui(concat("RS_",request->table_suffix,"->to_values."),request->
      tbl_cnt,0)
    ENDIF
   ENDIF
  ENDIF
 ELSE
  FREE RECORD drfpc_pkw
  CALL parser("record drfpc_pkw( 1 cols ",0)
  FOR (i = 1 TO parsed_pkw->total)
    CALL parser(concat(" 2 ",parsed_pkw->tokens[i].col_name," = ",parsed_pkw->tokens[i].data_type),0)
  ENDFOR
  CALL parser(" ) go",1)
  FOR (i = 1 TO parsed_pkw->total)
    IF ((parsed_pkw->tokens[i].data_type="*C*"))
     CALL parser(concat("set drfpc_pkw->cols.",parsed_pkw->tokens[i].col_name,
       " = parsed_pkw->tokens[i].value go"))
    ELSE
     CALL parser(concat("set drfpc_pkw->cols.",parsed_pkw->tokens[i].col_name," = ",parsed_pkw->
       tokens[i].value," go"))
    ENDIF
  ENDFOR
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_drfpc
  ENDIF
  SET pwi_ret = pk_where_info(request->table_name,request->table_suffix,"drfpc_pkw->cols","",
   pwi_reply)
  IF (pwi_ret="F")
   SET reply->nomv_type = "NOMV99"
   SET reply->reason_text = dm_err->emsg
   GO TO exit_drfpc
  ENDIF
  SET pwi_reply->data[1].pkw_iu_str = check_null_space(pwi_reply,"drfpc_pkw->cols",idr_ref_data_doc,1,
   0)
  SET pwi_reply->data[1].pkw_del_str = check_null_space(pwi_reply,"drfpc_pkw->cols",idr_ref_data_doc,
   1,1)
  IF ((((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].version_ind=0)) OR ((dm2_ref_data_doc->
  tbl_qual[request->tbl_cnt].version_ind=1)
   AND (dm2_ref_data_doc->tbl_qual[request->tbl_cnt].version_type="ALG2")))
   AND  NOT ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].table_name IN ("DCP_SECTION_REF",
  "DCP_FORMS_REF"))))
   IF ((parsed_pkw->total != size(pwi_reply->data[1].pkw,5)))
    SET reply->nomv_type = "NOMV99"
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Delete pk_where did not match expected pk_where column list"
    SET reply->reason_text = dm_err->emsg
    GO TO exit_drfpc
   ELSE
    FOR (i = 1 TO parsed_pkw->total)
      IF (locateval(drfpc_temp_idx,1,size(pwi_reply->data[1].pkw,5),parsed_pkw->tokens[i].col_name,
       pwi_reply->data[1].pkw[drfpc_temp_idx].parm_name)=0)
       SET reply->nomv_type = "NOMV99"
       SET dm_err->emsg = "Delete pk_where did not match expected pk_where column list"
       SET reply->reason_text = dm_err->emsg
       GO TO exit_drfpc
  ...
  IF ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].version_ind=1)
   AND (((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].version_type IN ("ALG1", "ALG3", "ALG3D")))
   OR ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].table_name IN ("DCP_SECTION_REF",
  "DCP_FORMS_REF")))) )
   SET reply->blocking_log_id = check_blocking(request->target_env_id,request->source_env_id,
    pwi_reply,"","",
    "",0,1)
  ...
  ELSE
   ...
   IF ((reply->blocking_log_id=0))
    IF ((((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].version_ind=1)
     ...
     OR ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].table_name IN ("DCP_SECTION_REF",
    "DCP_FORMS_REF")))) )
     SET reply->nomv_type = changed_ui("drfpc_pkw->cols.",request->tbl_cnt,1)
    ENDIF
   ENDIF
  ENDIF
 ENDIF
 SUBROUTINE check_blocking(i_target_env_id,i_source_env_id,i_data,i_r_tab_name,i_table_suffix,
  i_pk_where,i_del_check,i_ins_check)
   DECLARE drfpc_iu_str = vc WITH protect, noconstant("")
   DECLARE drfpc_del_str = vc WITH protect, noconstant("")
   DECLARE drfpc_iu_pkw_val = f8 WITH protect, noconstant(0.0)
   DECLARE drfpc_del_pkw_val = f8 WITH protect, noconstant(0.0)
   DECLARE s_blocking_log_id = f8 WITH protect, noconstant(0.0)
   DECLARE drfpc_loop = i4 WITH protect, noconstant(0)
   DECLARE drfpc_rec_idx = i4 WITH protect, noconstant(0)
   DECLARE drfpc_loop2 = i4 WITH protect, noconstant(0)
   DECLARE drfpc_idx2 = i4 WITH protect, noconstant(0)
   DECLARE drfpc_rs_name = vc WITH protect, noconstant("")
   DECLARE drfpc_tspc_flag = i4 WITH protect, noconstant(0)
   FREE RECORD pkhold
   RECORD pkhold(
     1 pkcnt = i4
     1 qual[*]
   ...
   IF (trim(i_pk_where)="")
    CALL parser(i_data->data[1].pkw_declare,1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_drfpc
    ENDIF
    IF (i_ins_check=0)
     CALL parser("select into 'nl:' pkw = ",0)
     CALL parser(i_data->data[1].pkw_iu_str,0)
     CALL parser(" from DUAL detail drfpc_iu_str = pkw with nocounter go ",1)
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_drfpc
     ENDIF
    ENDIF
   ELSE
    SET drfpc_iu_str = i_pk_where
   ENDIF
   IF (i_r_tab_name="")
    DECLARE dm_get_hash_value() = f8
    IF (i_del_check=0)
     CALL parser("select into 'nl:' pkw = ",0)
     CALL parser(i_data->data[1].pkw_del_str,0)
     CALL parser(" from DUAL detail drfpc_del_str = pkw with nocounter go ",1)
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_drfpc
     ENDIF
    ENDIF
    SELECT INTO "NL:"
     iu = dm_get_hash_value(drfpc_iu_str,0,1073741824.0), del = dm_get_hash_value(drfpc_del_str,0,
      1073741824.0)
     FROM dual
     DETAIL
      drfpc_iu_pkw_val = iu, drfpc_del_pkw_val = del
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (i_ins_check=0)
     SELECT INTO "nl:"
      FROM dm_chg_log d
      WHERE ((d.target_env_id+ 0)=i_source_env_id)
       AND (d.table_name=request->table_name)
       AND d.log_type IN ("REFCHG", "NORDDS", "PROCES")
       AND d.pk_where=drfpc_iu_str
       AND d.pk_where_value=drfpc_iu_pkw_val
       AND d.delete_ind=0
      DETAIL
       s_blocking_log_id = d.log_id
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (s_blocking_log_id=0
     ...
     SELECT INTO "nl:"
      ...
      WHERE ((d.target_env_id+ 0)=i_source_env_id)
       AND (d.table_name=request->table_name)
       AND d.log_type IN ("REFCHG", "NORDDS", "PROCES")
       AND d.pk_where=drfpc_del_str
       AND d.pk_where_value=drfpc_del_pkw_val
       AND d.delete_ind=1
      DETAIL
       s_blocking_log_id = d.log_id
      WITH nocounter
   ...
   ELSE
    ...
    CALL parser(concat(i_r_tab_name," t",i_table_suffix," "),0)
    CALL parser(drfpc_iu_str,0)
    CALL parser(concat(" and rdds_source_env_id = ",cnvtstring(i_target_env_id,20,1)),0)
    CALL parser(" and rdds_status_flag < 9000 ",0)
    CALL parser(concat(" detail s_blocking_log_id = t",i_table_suffix,
      ".rdds_log_id with nocounter go"),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_drfpc
    ENDIF
    FOR (drfpc_loop = 1 TO dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_cnt)
      IF ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_loop].pk_ind=1))
       SET pkhold->pkcnt = (pkhold->pkcnt+ 1)
       SET stat = alterlist(pkhold->qual,pkhold->pkcnt)
       SET pkhold->qual[pkhold->pkcnt].column_name = dm2_ref_data_doc->tbl_qual[request->tbl_cnt].
       col_qual[drfpc_loop].column_name
       SET pkhold->qual[pkhold->pkcnt].data_type = dm2_ref_data_doc->tbl_qual[request->tbl_cnt].
       col_qual[drfpc_loop].data_type
      ENDIF
    ENDFOR
    IF (check_iu_pkwhere(drfpc_iu_str,request->table_name,pkhold)=0)
     FOR (drfpc_loop = 1 TO pkhold->pkcnt)
       SET drfpc_idx2 = locateval(drfpc_loop2,1,dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_cnt,
        pkhold->qual[drfpc_loop].column_name,dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[
        drfpc_loop2].column_name)
       SET drfpc_rs_name = concat("RS_",i_table_suffix,"->from_values.",pkhold->qual[drfpc_loop].
        column_name)
       IF ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].trailing_space_cnt > 0)
        AND (dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].data_type="VC")
        AND (dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].check_space=0))
        CALL parser(concat("free set iur_nospc",trim(cnvtstring(drfpc_loop))," go"),1)
        CALL parser(concat("declare iur_nospc",trim(cnvtstring(drfpc_loop))," =  vc with notrim go"),
         1)
        CALL parser(concat("set iur_nospc",trim(cnvtstring(drfpc_loop))," = notrim(concat(",
          drfpc_rs_name,", fillstring(value(",
          trim(cnvtstring(dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].
            trailing_space_cnt)),"), ' '))) go"))
        SET drfpc_tspc_flag = 1
       ENDIF
     ENDFOR
     CALL parser("select into 'nl:' from ",0)
     CALL parser(concat(i_r_tab_name," t where "),0)
     SET drfpc_loop = 0
     FOR (drfpc_loop = 1 TO pkhold->pkcnt)
       SET drfpc_idx2 = locateval(drfpc_loop2,1,dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_cnt,
        pkhold->qual[drfpc_loop].column_name,dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[
        drfpc_loop2].column_name)
       SET drfpc_rs_name = concat("RS_",i_table_suffix,"->from_values.",dm2_ref_data_doc->tbl_qual[
        request->tbl_cnt].col_qual[drfpc_idx2].column_name)
       IF (drfpc_loop > 1)
        CALL parser(" and ",0)
       ENDIF
       IF ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].check_null=1)
        AND (dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].nullable="Y"))
        IF ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].data_type IN ("DQ8",
        "I4", "F8", "I2")))
         CALL parser(concat(" t.",dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].
           column_name," = null"),0)
        ELSE
         CALL parser(concat(" (t.",dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].
           column_name," = null or t.",dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[
           drfpc_idx2].column_name," = ' ')"),0)
        ENDIF
       ELSEIF ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].data_type="DQ8"))
        CALL parser(concat(" t.",dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].
          column_name," = cnvtdatetime(",drfpc_rs_name,")"),0)
       ELSEIF ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].check_space=1))
        CALL parser(concat(" (t.",dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].
          column_name," = null or t.",dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[
          drfpc_idx2].column_name," = ' ')"),0)
       ELSEIF ((dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].trailing_space_cnt
        > 0)
        AND (dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].data_type="VC"))
        CALL parser(concat("t.",dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].
          column_name," = iur_nospc",trim(cnvtstring(drfpc_loop))),0)
       ELSE
        CALL parser(concat(" t.",dm2_ref_data_doc->tbl_qual[request->tbl_cnt].col_qual[drfpc_idx2].
          column_name," = ",drfpc_rs_name),0)
       ENDIF
     ENDFOR
     CALL parser(concat(" and t.rdds_source_env_id = ",trim(cnvtstring(i_target_env_id,20)),".0"),0)
     CALL parser(" and t.rdds_status_flag < 9000 ",0)
     CALL parser(" detail s_blocking_log_id = t.rdds_log_id",0)
     IF (drfpc_tspc_flag=1)
      CALL parser(" with notrim go",1)
     ELSE
      CALL parser(" go",1)
     ENDIF
     IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
   ENDIF
   RETURN(s_blocking_log_id)
 END ;Subroutine
 SUBROUTINE check_null_space(i_pkw_info,i_col_data_rec_name,i_ref_data_doc,i_tbl_cnt,i_del_ind)
   DECLARE v_parm_cnt = i2 WITH protect, noconstant(0)
   DECLARE v_col_pos = i2 WITH protect, noconstant(0)
   DECLARE v_col_cnt = i2 WITH protect, noconstant(0)
   DECLARE v_new_pkw = vc WITH protect, noconstant("")
   DECLARE v_temp_ndx = i4 WITH protect, noconstant(0)
   DECLARE v_temp_str = vc WITH protect, noconstant("")
   DECLARE v_temp2_str = vc WITH protect, noconstant("")
   DECLARE v_tbl_cnt = i2 WITH protect, noconstant(0)
   DECLARE v_col_pos2 = i2 WITH protect, noconstant(0)
   IF (i_del_ind=0)
    SET v_new_pkw = i_pkw_info->data[1].pkw_iu_str
   ELSE
    SET v_new_pkw = i_pkw_info->data[1].pkw_del_str
   ENDIF
   SET v_parm_cnt = size(i_pkw_info->data[1].pkw,5)
   SET v_col_cnt = i_ref_data_doc->tbl_qual[i_tbl_cnt].col_cnt
   SET v_tbl_cnt = locateval(v_temp_ndx,1,dm2_ref_data_doc->tbl_cnt,i_ref_data_doc->tbl_qual[
    i_tbl_cnt].table_name,dm2_ref_data_doc->tbl_qual[v_temp_ndx].table_name)
   IF ((dm2_ref_data_doc->tbl_qual[v_tbl_cnt].version_ind=1)
    AND (((dm2_ref_data_doc->tbl_qual[v_tbl_cnt].version_type IN ("ALG1", "ALG3", "ALG3D", "ALG4")))
    OR ((dm2_ref_data_doc->tbl_qual[v_tbl_cnt].table_name IN ("DCP_SECTION_REF", "DCP_FORMS_REF"))))
   )
    IF (i_del_ind=1)
     FOR (v_temp_ndx = 1 TO dm2_ref_data_doc->tbl_qual[v_tbl_cnt].col_cnt)
       IF ((dm2_ref_data_doc->tbl_qual[v_tbl_cnt].col_qual[v_temp_ndx].pk_ind=0)
        AND (dm2_ref_data_doc->tbl_qual[v_tbl_cnt].col_qual[v_temp_ndx].unique_ident_ind=1))
        SET v_temp_str = concat(i_col_data_rec_name,".",dm2_ref_data_doc->tbl_qual[v_tbl_cnt].
         col_qual[v_temp_ndx].column_name,",")
        SET v_temp2_str = concat(i_col_data_rec_name,".",dm2_ref_data_doc->tbl_qual[v_tbl_cnt].
         col_qual[v_temp_ndx].column_name,")")
        IF ((dm2_ref_data_doc->tbl_qual[v_tbl_cnt].col_qual[v_temp_ndx].data_type="*C*"))
         CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp_str,'","^junk^,") go'),1)
         CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp2_str,'","^junk^)") go'),1)
        ELSE
         CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp_str,'","0,") go'),1)
         CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp2_str,'","0)") go'),1)
        ENDIF
       ENDIF
     ENDFOR
    ENDIF
   ENDIF
   FOR (i = 1 TO v_parm_cnt)
     SET v_col_pos = locateval(v_temp_ndx,1,v_col_cnt,i_pkw_info->data[1].pkw[i].parm_name,
      i_ref_data_doc->tbl_qual[i_tbl_cnt].col_qual[v_temp_ndx].column_name)
     SET v_col_pos2 = locateval(v_temp_ndx,1,dm2_ref_data_doc->tbl_qual[v_tbl_cnt].col_cnt,i_pkw_info
      ->data[1].pkw[i].parm_name,dm2_ref_data_doc->tbl_qual[v_tbl_cnt].col_qual[v_temp_ndx].
      column_name)
     SET v_temp_str = concat(i_col_data_rec_name,".",i_pkw_info->data[1].pkw[i].parm_name,",")
     SET v_temp2_str = concat(i_col_data_rec_name,".",i_pkw_info->data[1].pkw[i].parm_name,")")
     IF ((i_ref_data_doc->tbl_qual[i_tbl_cnt].col_qual[v_col_pos].check_null=1))
      CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp_str,'","NULL,") go'),1)
      CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp2_str,'","NULL)") go'),1)
     ELSEIF ((i_ref_data_doc->tbl_qual[i_tbl_cnt].col_qual[v_col_pos].check_space=1))
      CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp_str,'","char(32),") go'),1)
      CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp2_str,'","char(32))") go'),1)
     ELSEIF ((i_ref_data_doc->tbl_qual[i_tbl_cnt].col_qual[v_col_pos].trailing_space_cnt > 0))
      CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp_str,'",concat(char(39),value(',
        substring(1,(size(v_temp_str) - 1),v_temp_str),"),fillstring(",
        cnvtstring(i_ref_data_doc->tbl_qual[i_tbl_cnt].col_qual[v_col_pos].trailing_space_cnt),
        '," "),char(39),",")) go'),1)
      CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp2_str,'",concat(char(39),value(',
        substring(1,(size(v_temp_str) - 1),v_temp_str),"),fillstring(",
        cnvtstring(i_ref_data_doc->tbl_qual[i_tbl_cnt].col_qual[v_col_pos].trailing_space_cnt),
        '," "),char(39),")")) go'),1)
     ELSEIF ((dm2_ref_data_doc->tbl_qual[v_tbl_cnt].col_qual[v_col_pos2].data_type="DQ8"))
      CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp_str,'","cnvtdatetime(',
        v_temp_str,'),") go'),1)
      CALL parser(concat('set v_new_pkw = replace(v_new_pkw,"',v_temp2_str,'","cnvtdatetime(',
        v_temp2_str,')") go'),1)
     ENDIF
   ENDFOR
   RETURN(v_new_pkw)
 END ;Subroutine
 SUBROUTINE changed_ui(cu_rs_string,cui_tab_idx,cui_del_ind)
   DECLARE cui_loop = i4 WITH protect, noconstant(0)
   DECLARE cui_top_level_col = vc WITH protect, noconstant("")
   DECLARE cui_temp_cnt = i4 WITH protect, noconstant(0)
   DECLARE cui_ret = c1 WITH protect, noconstant("")
   DECLARE cui_col_size = i4 WITH protect, noconstant(0)
   DECLARE cui_qual_cnt = i4 WITH protect, noconstant(0)
   FREE RECORD cui_ref_data_doc
   RECORD cui_ref_data_doc(
     1 tbl_qual[1]
       2 col_cnt = i2
       2 col_qual[*]
         3 column_name = vc
   ...
   SET cui_ref_data_doc->tbl_qual[1].table_name = dm2_ref_data_doc->tbl_qual[cui_tab_idx].table_name
   FOR (cui_loop = 1 TO dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_cnt)
     IF ((dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].pk_ind=1)
      AND (dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].root_entity_name=request->
     table_name)
      AND (dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].root_entity_attr=
     dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].column_name))
      SET cui_top_level_col = dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].column_name
     ENDIF
   ENDFOR
   IF (cui_top_level_col > "")
    SET cui_temp_cnt = 0
    FREE RECORD cui_row
    CALL parser("record cui_row (",0)
    CALL parser(" 1 qual[*]",0)
    FOR (cui_loop = 1 TO dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_cnt)
      IF ((((((dm2_ref_data_doc->tbl_qual[cui_tab_idx].version_ind=1)) OR ((dm2_ref_data_doc->
      tbl_qual[cui_tab_idx].table_name IN ("DCP_SECTION_REF", "DCP_FORMS_REF"))))
       AND (((dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].unique_ident_ind=1)) OR ((
      dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].pk_ind=1))) ) OR ((dm2_ref_data_doc
      ->tbl_qual[cui_tab_idx].merge_delete_ind=1)
       AND (dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].merge_delete_ind=1))) )
       SET cui_temp_cnt = (cui_temp_cnt+ 1)
       SET stat = alterlist(cui_ref_data_doc->tbl_qual[1].col_qual,cui_temp_cnt)
       SET cui_ref_data_doc->tbl_qual[1].col_cnt = cui_temp_cnt
       SET cui_ref_data_doc->tbl_qual[1].col_qual[cui_temp_cnt].column_name = dm2_ref_data_doc->
       tbl_qual[cui_tab_idx].col_qual[cui_loop].column_name
       CALL parser(concat(" 2 ",dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].
         column_name," = ",dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].data_type),0)
      ENDIF
    ENDFOR
    CALL parser(") go",1)
    IF (check_error(dm_err->eproc)=1)
    ...
    FOR (cui_loop = 1 TO dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_cnt)
      ...
      ->tbl_qual[cui_tab_idx].merge_delete_ind=1)
       ...
       ENDIF
       SET cui_temp_cnt = (cui_temp_cnt+ 1)
       CALL parser(concat("var",cnvtstring(cui_loop)," = nullind(","d.",dm2_ref_data_doc->tbl_qual[
         cui_tab_idx].col_qual[cui_loop].column_name,
         ")"),0)
       IF ((dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].data_type="VC"))
        CALL parser(concat(",ts",cnvtstring(cui_loop)," = length(","d.",dm2_ref_data_doc->tbl_qual[
          cui_tab_idx].col_qual[cui_loop].column_name,
          ")"),0)
       ENDIF
      ENDIF
    ENDFOR
    CALL parser(concat(" from ",request->table_name," d where "),0)
    CALL parser(concat("d.",cui_top_level_col," = ",cu_rs_string,cui_top_level_col),0)
    IF (cui_del_ind=0)
     CALL parser(" and (",0)
     SET cui_temp_cnt = 0
     FOR (cui_loop = 1 TO dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_cnt)
       ...
       ->tbl_qual[cui_tab_idx].merge_delete_ind=1)
        ...
        ENDIF
        SET cui_temp_cnt = (cui_temp_cnt+ 1)
        CALL parser(concat(" d.",dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].
          column_name," != rs_",request->table_suffix,"->to_values.",
          dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].column_name),0)
       ENDIF
     ENDFOR
     CALL parser(")",0)
    ENDIF
    CALL parser(
     " detail cui_qual_cnt = cui_qual_cnt + 1 stat = alterlist(cui_row->qual, cui_qual_cnt) ",0)
    SET cui_temp_cnt = 0
    FOR (cui_loop = 1 TO dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_cnt)
      IF ((((((dm2_ref_data_doc->tbl_qual[cui_tab_idx].version_ind=1)) OR ((dm2_ref_data_doc->
      tbl_qual[cui_tab_idx].table_name IN ("DCP_SECTION_REF", "DCP_FORMS_REF"))))
      ...
      ->tbl_qual[cui_tab_idx].merge_delete_ind=1)
       AND (dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].merge_delete_ind=1))) )
       SET cui_temp_cnt = (cui_temp_cnt+ 1)
       CALL parser(concat("cui_row->qual[cui_qual_cnt].",dm2_ref_data_doc->tbl_qual[cui_tab_idx].
         col_qual[cui_loop].column_name," = ","d.",dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[
         cui_loop].column_name),0)
       CALL parser(concat("cui_ref_data_doc->tbl_qual[1].col_qual[",trim(cnvtstring(cui_temp_cnt)),
         "].check_null = var",trim(cnvtstring(cui_loop))),0)
       IF ((dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].data_type="VC"))
        CALL parser(concat("cui_ref_data_doc->tbl_qual[1].col_qual[",trim(cnvtstring(cui_temp_cnt)),
          "].trailing_space_cnt = ts",trim(cnvtstring(cui_loop)),
          " - size(cui_row->qual[cui_qual_cnt].",
          dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].column_name,")"),0)
       ENDIF
      ENDIF
    ENDFOR
    ...
    FOR (cui_loop = 1 TO dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_cnt)
      ...
      ->tbl_qual[cui_tab_idx].merge_delete_ind=1)
       ...
       SET cui_temp_cnt = (cui_temp_cnt+ 1)
       IF ((dm2_ref_data_doc->tbl_qual[cui_tab_idx].col_qual[cui_loop].data_type="VC"))
        CALL parser(concat("set cui_col_size = size(cui_row->qual[1].",dm2_ref_data_doc->tbl_qual[
          cui_tab_idx].col_qual[cui_loop].column_name,") go"),1)
        IF ((cui_ref_data_doc->tbl_qual[1].col_qual[cui_temp_cnt].check_null=0)
         AND cui_col_size=0)
         SET cui_ref_data_doc->tbl_qual[1].col_qual[cui_temp_cnt].check_space = 1
        ENDIF
       ENDIF
      ENDIF
    ENDFOR
    IF (curqual=1)
     SET cui_ret = pk_where_info(request->table_name,request->table_suffix,concat("CUI_ROW->QUAL[1]"),
      "",pwi_reply)
     IF (pwi_ret="F")
      SET reply->reason_text = dm_err->emsg
      RETURN("NOMV99")
     ENDIF
     SET pwi_reply->data[1].pkw_iu_str = check_null_space(pwi_reply,"CUI_ROW->QUAL[1]",
      cui_ref_data_doc,1,0)
     SET reply->blocking_log_id = check_blocking(request->target_env_id,request->source_env_id,
      pwi_reply,"","",
      "",1,0)
     IF ((reply->blocking_log_id > 0))
      SET reply->reason_text = concat("A row was detected in TARGET with a LOG_ID of ",trim(
        cnvtstring(reply->blocking_log_id,20,1)),
       " which represents a change. Row will be blocked from merging.")
      RETURN("NOMV19")
     ENDIF
    ENDIF
   ENDIF
   RETURN("")
 END ;Subroutine
 SUBROUTINE check_iu_pkwhere(cip_iu_string,cip_tab_name,cip_data)
   DECLARE cip_rec_idx = i4 WITH protect, noconstant(0)
   DECLARE cip_rec_loop = i4 WITH protect, noconstant(0)
   DECLARE cip_col_cnt = i4 WITH protect, noconstant(0)
   DECLARE cip_and_cnt = i4 WITH protect, noconstant(0)
   DECLARE cip_and_pos = i4 WITH protect, noconstant(0)
   DECLARE cip_end_ind = i4 WITH protect, noconstant(0)
   DECLARE cip_delim1 = i4 WITH protect, noconstant(0)
   DECLARE cip_delim2 = i4 WITH protect, noconstant(0)
   DECLARE cip_delim3 = i4 WITH protect, noconstant(0)
   DECLARE cip_delim_val = vc WITH protect, noconstant("")
   DECLARE cip_delim_start = i4 WITH protect, noconstant(0)
   DECLARE cip_delim_stop = i4 WITH protect, noconstant(0)
   DECLARE cip_col_value = vc WITH protect, noconstant("")
   IF ((cip_data->pkcnt=0))
    CALL disp_msg(concat("There were no PK columns defined for the current table: ",cip_tab_name),
     dm_err->logfile,1)
    RETURN(0)
   ENDIF
   FOR (cip_rec_loop = 1 TO cip_data->pkcnt)
     IF (((findstring(concat(cip_data->qual[cip_rec_loop].column_name," "),cip_iu_string,1,0) > 0)
      OR (findstring(concat(cip_data->qual[cip_rec_loop].column_name,"="),cip_iu_string,1,0) > 0)) )
      SET cip_col_cnt = (cip_col_cnt+ 1)
     ENDIF
   ENDFOR
   IF ((cip_col_cnt=cip_data->pkcnt))
    WHILE (cip_end_ind=0)
      SET cip_rec_loop = 0
      SET cip_rec_loop = findstring(" AND ",cip_iu_string,cip_and_pos,0)
      IF (cip_rec_loop=0)
       SET cip_end_ind = 1
      ELSE
       SET cip_and_cnt = (cip_and_cnt+ 1)
       SET cip_and_pos = (cip_rec_loop+ 3)
      ENDIF
    ENDWHILE
    IF (cip_and_cnt > 0)
     SET cip_and_pos = 0
     SET cip_rec_loop = 1
     WHILE (cip_rec_loop > 0)
      SET cip_rec_loop = findstring("=",cip_iu_string,cip_and_pos,0)
      IF (cip_rec_loop > 0)
       SET cip_delim1 = findstring("'",cip_iu_string,cip_rec_loop,0)
       SET cip_delim2 = findstring('"',cip_iu_string,cip_rec_loop,0)
       SET cip_delim3 = findstring("^",cip_iu_string,cip_rec_loop,0)
       IF (cip_delim1=0)
        SET cip_delim1 = size(cip_iu_string)
       ENDIF
       IF (cip_delim2=0)
        SET cip_delim2 = size(cip_iu_string)
       ENDIF
       IF (cip_delim3=0)
        SET cip_delim3 = size(cip_iu_string)
       ENDIF
       IF (cip_delim1 < cip_delim2)
        IF (cip_delim1 < cip_delim3)
         SET cip_delim_val = "'"
         SET cip_delim_start = cip_delim1
        ELSE
         SET cip_delim_val = "^"
         SET cip_delim_start = cip_delim3
        ENDIF
       ELSE
        IF (cip_delim2 < cip_delim3)
         SET cip_delim_val = '"'
         SET cip_delim_start = cip_delim2
        ELSE
         SET cip_delim_val = "^"
         SET cip_delim_start = cip_delim3
        ENDIF
       ENDIF
       IF (cip_delim_start < size(cip_iu_string))
        SET cip_delim_stop = findstring(cip_delim_val,cip_iu_string,(cip_delim_start+ 1),0)
        IF (cip_delim_stop > 0)
         SET cip_col_value = substring((cip_delim_start+ 1),(cip_delim_stop - (cip_delim_start+ 1)),
          cip_iu_string)
         IF (findstring(" AND ",cip_col_value,0,0) > 0)
          SET cip_and_cnt = (cip_and_cnt - 1)
         ENDIF
         SET cip_and_pos = (cip_delim_stop+ 1)
        ELSE
         RETURN(0)
        ENDIF
       ELSE
        SET cip_and_pos = cip_delim_start
       ENDIF
      ENDIF
     ENDWHILE
    ENDIF
    IF ((cip_and_cnt != (cip_col_cnt - 1)))
     RETURN(0)
    ENDIF
   ELSE
    RETURN(0)
