CREATE PROGRAM acm_rdm_svc_provider_org_cc:dba
 ...
 SET ms_child_c_info_domain_nm = "ACM_RDM_SVC_PROVIDER_ORG"
 SET ms_child_c_max_name = "MAX ENCNTR_ID EVALUATED"
 CALL sbr_get_min_max(mf_min_id,mf_max_id)
 CALL echo("Processing acm_rdm_svc_provider_org_cc...")
 SET cmd = concat(" rdb update encounter e "," set e.service_provider_org_id = "," case ",
  ...
  " e.service_provider_org_id <= 0 or e.service_provider_org_id is null ",
  " and e.encntr_id >= ",cnvtstring(mf_min_id)," and e.encntr_id <= ",cnvtstring(mf_max_id)," go ")
 CALL parser(cmd)
 IF (sbr_handle_cc_error(null)=0)
  GO TO exit_program
 ENDIF
 CALL echo("Processing acm_rdm_svc_provider_org_cc completed")
