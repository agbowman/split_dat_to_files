CREATE PROGRAM aps_prt_db_fixatives:dba
 ...
 SET captions->rptaps = uar_i18ngetmessage(i18nhandle,"rptaps","REPORT: APS_PRT_DB_FIXATIVES.PRG")
 ...
 SET captions->dbft = uar_i18ngetmessage(i18nhandle,"dbft","DB FIXATIVES TOOL")
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY cv.active_ind DESC, cv.display_key
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = (cnt+ 1), stat = alterlist(temp->qual,cnt), temp->qual[cnt].display = cv.display,
   temp->qual[cnt].display_key = cv.display_key, temp->qual[cnt].description = cv.description, temp->
   qual[cnt].active_ind = cv.active_ind
  WITH nocounter
 ;end select
#report_maker
 EXECUTE cpm_create_file_name_logical "apsDbFixatives", "dat", "x"
 ...
 SELECT INTO value(reply->print_status_data.print_filename)
  temp->qual[d.seq].display, temp->qual[d.seq].active_ind
  FROM (dummyt d  WITH seq = value(size(temp->qual,5)))
  PLAN (d)
  ORDER BY temp->qual[d.seq].active_ind DESC, temp->qual[d.seq].display_key
  HEAD REPORT
  ...
  HEAD PAGE
   ...
   CALL center(captions->dbft,0,132), col 110, captions->ppage,
   ":", col 117, curpage"###",
   row + 2, row + 1, col 0,
   captions->status, col 10, captions->code,
   col 27, captions->desc, row + 1,
   col 0, "--------", col 10,
   "---------------", col 27, "---------------------------------------"
  DETAIL
   row + 1, col 0
   ...
   ELSE
    captions->inactive
   ENDIF
   col 10, temp->qual[d.seq].display, col 27,
   temp->qual[d.seq].description
   IF (((row+ 7) > maxrow))
    BREAK
   ENDIF
  FOOT PAGE
