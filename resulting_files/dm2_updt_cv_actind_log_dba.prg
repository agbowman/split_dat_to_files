CREATE PROGRAM dm2_updt_cv_actind_log:dba
 ...
 SUBROUTINE dilc_check_oil_column(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ducal_pkg_nbr = i4 WITH protect, noconstant(0)
 DECLARE ducal_err_ind = i2 WITH protect, noconstant(0)
 DECLARE ducal_tmp_cs = f8 WITH protect, noconstant(0.0)
 DECLARE ducal_tmp_cv = f8 WITH protect, noconstant(0.0)
 DECLARE ducal_tmp_desc = vc WITH protect, noconstant("NOT_SET")
 DECLARE ducal_tmp_cdfm = vc WITH protect, noconstant("NOT_SET")
 DECLARE ducal_tmp_display = vc WITH protect, noconstant("NOT_SET")
 DECLARE ducal_tmp_actind = i2 WITH protect, noconstant(0)
 FREE RECORD ducal_cv_list
 RECORD ducal_cv_list(
   1 cv_cnt = i4
   1 cv[*]
     2 code_set = f8
     2 code_value = f8
 ...
 SET ducal_cv_list->cv_cnt = 0
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO exit_program
 ENDIF
 SET ducal_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 IF (( $1 != 0))
  SET ducal_pkg_nbr =  $1
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Validating package number or plan ID."
  SET dm_err->emsg = "Invalid package or plan ID."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 ...
 IF (dilc_check_oil_column(null)=0)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting list of new code values."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  o.component_type, o.end_state, o.update_ind,
  o.install_text, cv.code_set, cv.code_value
  FROM ocd_install_log o,
   code_value cv
  PLAN (o
   WHERE o.ocd=ducal_pkg_nbr
    AND o.component_type=dilc_code_value)
   JOIN (cv
   WHERE cv.code_value=cnvtreal(o.end_state))
  DETAIL
   ducal_tmp_cs = 0.00, ducal_tmp_cv = 0.00, ducal_tmp_desc = "",
   ducal_tmp_cdfm = "", ducal_tmp_display = "", ducal_tmp_actind = 0,
   stat = dilc_get_install_text(ducal_tmp_cs,ducal_tmp_cv,ducal_tmp_desc,ducal_tmp_cdfm,
    ducal_tmp_display,
    ducal_tmp_actind,o.install_text)
   IF (((cv.description != ducal_tmp_desc) OR (((cv.cdf_meaning != ducal_tmp_cdfm) OR (((cv.display
    != ducal_tmp_display) OR (cv.active_ind != ducal_tmp_actind)) )) )) )
    ducal_cv_list->cv_cnt = (ducal_cv_list->cv_cnt+ 1)
    IF (mod(ducal_cv_list->cv_cnt,100)=1)
     stat = alterlist(ducal_cv_list->cv,(ducal_cv_list->cv_cnt+ 99))
    ENDIF
    ducal_cv_list->cv[ducal_cv_list->cv_cnt].code_set = ducal_tmp_cs, ducal_cv_list->cv[ducal_cv_list
    ->cv_cnt].code_value = ducal_tmp_cv, ducal_cv_list->cv[ducal_cv_list->cv_cnt].cv_description = cv
    .description,
    ducal_cv_list->cv[ducal_cv_list->cv_cnt].cdf_meaning = cv.cdf_meaning, ducal_cv_list->cv[
    ducal_cv_list->cv_cnt].display = cv.display, ducal_cv_list->cv[ducal_cv_list->cv_cnt].active_ind
     = cv.active_ind,
    ducal_cv_list->cv[ducal_cv_list->cv_cnt].install_text = dilc_set_install_text(ducal_tmp_cs,
     ducal_tmp_cv,cv.description,cv.cdf_meaning,cv.display,
     cv.active_ind)
   ENDIF
  FOOT REPORT
   stat = alterlist(ducal_cv_list->cv,ducal_cv_list->cv_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 ENDIF
 IF ((dm_err->debug_flag >= 2))
  CALL echorecord(ducal_cv_list)
 ENDIF
 IF ((ducal_cv_list->cv_cnt > 0))
  SET dm_err->eproc = "Updating code value log after post-up readmes."
  CALL disp_msg("",dm_err->logfile,0)
  UPDATE  FROM ocd_install_log o,
    (dummyt d  WITH seq = ducal_cv_list->cv_cnt)
   SET o.install_text = ducal_cv_list->cv[d.seq].install_text
   PLAN (d)
    JOIN (o
    WHERE o.ocd=ducal_pkg_nbr
     AND o.component_type=dilc_code_value
     AND o.end_state=trim(format(ducal_cv_list->cv[d.seq].code_value,";;I"),3))
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "No code value log rows to update."
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
#exit_program
 CALL dm2_prg_maint("END")
 SET dm_err->err_ind = ducal_err_ind
END GO
