CREATE PROGRAM cp_get_dta_info:dba
 PROMPT
  ...
  "Date of Birth:" = "",
  "Gender:" = 0.0,
  "Gestational days:" = 0.0,
  "List of Task Assay Codes:" = ""
  WITH outdev, person_id, inputproviderid,
  encntr_id, inputppr, dob,
  gender, gestational_days, task_assay_cds
 FREE RECORD request
 ...
 DECLARE task_rec = i4 WITH protect
 DECLARE task_cnt = i4 WITH protect, noconstant(0)
 DECLARE jsonstr = vc WITH protect, noconstant(trim( $TASK_ASSAY_CDS))
 IF (jsonstr > " ")
  CALL echo(jsonstr)
  SET task_rec = cnvtjsontorec(trim(jsonstr,3))
  IF (validate(debug_ind,0)=1)
   CALL echorecord(task_assay_cds_list)
  ENDIF
 ENDIF
 FOR (task_cnt = 1 TO size(task_assay_cds_list->qual,5))
  SET stat = alterlist(request->task_assay_cds,task_cnt)
  SET request->task_assay_cds[task_cnt].task_assay_cd = task_assay_cds_list->qual[task_cnt].
  task_assay_cd
 ENDFOR
 SET request->person_id =  $PERSON_ID
 SET request->encounter_id =  $ENCNTR_ID
 SET request->prsnl_id =  $INPUTPROVIDERID
 SET request->ppr_cd =  $INPUTPPR
 SET request->dob =  $DOB
 SET request->person_id =  $PERSON_ID
 SET request->gender = uar_get_code_meaning( $GENDER)
 SET request->gestational_age_days =  $GESTATIONAL_DAYS
 EXECUTE mp_preg_get_dta_info
#exit_script
 IF (validate(debug_ind,0)=1)
