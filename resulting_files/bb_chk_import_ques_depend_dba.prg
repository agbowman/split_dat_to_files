CREATE PROGRAM bb_chk_import_ques_depend:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bb_chk_import_ques_depend"
 CALL logscriptstart(sreadme_name)
 DECLARE success_ind = i2 WITH protect, noconstant(1)
 DECLARE dispense_notice_ques_cd = f8 WITH protect, noconstant(0.0)
 DECLARE override_aborh_ques_cd = f8 WITH protect, noconstant(0.0)
 DECLARE override_absc_ques_cd = f8 WITH protect, noconstant(0.0)
 DECLARE question_code_set = i4 WITH protect, constant(1661)
 DECLARE dispense_notice_cdf_meaning = c12 WITH protect, constant("PRODDSNOTICE")
 DECLARE override_aborh_cdf_meaning = c12 WITH protect, constant("OVERDSPABORH")
 DECLARE override_absc_cdf_meaning = c12 WITH protect, constant("OVERDSPABSC")
 SELECT INTO "nl:"
  c.code_value
  FROM code_value c
  PLAN (c
   WHERE c.code_set=question_code_set
    AND c.cdf_meaning IN (dispense_notice_cdf_meaning, override_aborh_cdf_meaning,
   override_absc_cdf_meaning)
    AND c.active_ind=1)
  DETAIL
   IF (c.cdf_meaning=dispense_notice_cdf_meaning)
    dispense_notice_ques_cd = c.code_value
   ELSEIF (c.cdf_meaning=override_aborh_cdf_meaning)
    override_aborh_ques_cd = c.code_value
   ELSEIF (c.cdf_meaning=override_absc_cdf_meaning)
    override_absc_ques_cd = c.code_value
   ENDIF
  WITH nocounter
 ;end select
 IF (dispense_notice_ques_cd=0.0)
  SET success_ind = 0
  SET smsg = concat("product dispense notice code lookup failed->","PRODDSNOTICE")
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 IF (override_aborh_ques_cd=0.0)
  SET success_ind = 0
  SET smsg = concat("product dispense notice code lookup failed->","OVERDSPABORH")
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 IF (override_absc_ques_cd=0.0)
  SET success_ind = 0
  SET smsg = concat("product dispense notice code lookup failed->","OVERDSPABSC")
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM question_dependency q
  WHERE q.question_cd=dispense_notice_ques_cd
  WITH nocounter
 ;end select
 IF (curqual != 1)
  SET success_ind = 0
  SET smsg = "Can't find question for dispense notice question."
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 ENDIF
 SELECT INTO "nl:"
  FROM dependency d
  WHERE d.question_cd=dispense_notice_ques_cd
   AND d.depend_quest_cd IN (override_aborh_ques_cd, override_absc_ques_cd)
  WITH nocounter
 ;end select
 IF (curqual != 2)
  SET success_ind = 0
  SET smsg = "Can't find dependencies for dispense notice question."
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 ENDIF
#exit_script
 IF (success_ind=0)
  ROLLBACK
  CALL logstatus("Failed to insert all dependecy rows successfully.","F")
  CALL logmsg("Failed to insert all dependecy rows successfully.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
 ELSE
  CALL logstatus("Inserted all dependecy rows successfully.","S")
  CALL logmsg("Inserted all dependecy rows successfully.",((e_rdmstatus+ e_dispmsg)+ e_logtofile))
 ENDIF
