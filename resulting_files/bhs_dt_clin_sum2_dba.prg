CREATE PROGRAM bhs_dt_clin_sum2:dba
 ...
 SET eid1 = request->visit[1].encntr_id
 FREE RECORD sw
 RECORD sw(
   1 consultstatus = vc
   1 planoutcome = vc
 ...
 FOR (x = 1 TO pat_cnt)
   ...
   SELECT INTO request->output_device
    ...
    DETAIL
     ...
     FOR (y = 1 TO size(pat->qual[x].sec,5))
       IF ((pat->qual[x].sec[y].sec_disp IN ("Patient Data", "Additional Patient Data")))
        IF (ycol > 740)
         ...
         "**  Continue on Next Page **", row + 1, BREAK
        ENDIF
        l += 1, temp1 = build(cnvtstring(y),"-",pat->qual[x].sec[y].sec_disp,":"),
        CALL print(calcpos(xcol,ycol)),
        "{b}", temp1, row + 1,
        ycol += 8
     ...
     temp2 = pat->qual[x].ms_bloodpressurevenipuncture, tempstring = concat(build(temp2)), ycol += 10,
     ...
     IF (size(pat->qual[x].diet,5) > 0)
      l += 1, title = build(l,"-","Patient Care Orders :"),
      CALL print(calcpos(xcol,ycol)),
      "{b}", title, row + 1,
      ycol += 10, title = build("-","{b}Diet Orders:"),
      CALL print(calcpos(xcol,ycol)),
      "{b}", title, row + 1,
      ycol += 10
