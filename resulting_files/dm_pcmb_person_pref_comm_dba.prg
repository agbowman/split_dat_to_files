CREATE PROGRAM dm_pcmb_person_pref_comm:dba
 ...
 DECLARE v_cust_loopcount_to = i4 WITH protect, noconstant(0)
 DECLARE move_pref_ind = i2 WITH protect, noconstant(0)
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PCMB_PERSON_PREF_COMM"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count_from].from_id = frm.person_pref_comm_id, rreclist->from_rec[
   v_cust_count_from].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count_from].
   active_status_cd = frm.active_status_cd,
   rreclist->from_rec[v_cust_count_from].communication_type_cd = frm.communication_type_cd
 ...
 IF (v_cust_count_from > 0)
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count_to].to_id = tu.person_pref_comm_id, rreclist->to_rec[
    v_cust_count_to].active_ind = tu.active_ind, rreclist->to_rec[v_cust_count_to].active_status_cd
     = tu.active_status_cd,
    rreclist->to_rec[v_cust_count_to].communication_type_cd = tu.communication_type_cd
  ...
  FOR (v_cust_loopcount_from = 1 TO v_cust_count_from)
   IF ((rreclist->from_rec[v_cust_loopcount_from].active_ind=1))
    ...
    FOR (v_cust_loopcount_to = 1 TO v_cust_count_to)
      IF ((rreclist->from_rec[v_cust_loopcount_from].communication_type_cd=rreclist->to_rec[
      v_cust_loopcount_to].communication_type_cd)
       AND (rreclist->to_rec[v_cust_loopcount_to].active_ind=1))
       SET move_pref_ind = 0
       IF ((rev_cmb_request->reverse_ind=1))
        IF (del_to(rreclist->to_rec[v_cust_loopcount_to].to_id,request->xxx_combine[icombine].
         to_xxx_id,rreclist->to_rec[v_cust_loopcount_to].active_ind,rreclist->to_rec[
         v_cust_loopcount_to].active_status_cd)=0)
         GO TO exit_sub
        ENDIF
       ENDIF
      ENDIF
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_to_fk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =
  i4)
   UPDATE  FROM person_pref_comm frm
    ...
    WHERE frm.person_pref_comm_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "PERSON_PREF_COMM"
   ...
   IF (add_hist(s_df_pk_id)=0)
    ...
    SET request->error_message = substring(1,132,build(
      "del_from Could not insert history for person_pref_comm with pk=",s_df_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (del_to(s_dt_pk_id=f8,s_dt_from_fk_id=f8,s_dt_prev_act_ind=i2,s_dt_prev_act_status=f8) =
  i4)
   UPDATE  FROM person_pref_comm tu
    SET tu.active_ind = false, tu.active_status_cd = combinedaway, tu.active_status_dt_tm =
     ...
     cnvtdatetime(sysdate),
     tu.person_id = s_dt_from_fk_id
    WHERE tu.person_pref_comm_id=s_dt_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
   ...
   IF (curqual=0)
    ...
    RETURN(0)
   ENDIF
   IF (add_hist(s_dt_pk_id)=0)
    SET failed = insert_error
    SET request->error_message = substring(1,132,build(
      "del_to Could not insert history for person_pref_comm with pk=",s_dt_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE upt_from(s_uf_pk_id,s_uf_to_fk_id)
   UPDATE  FROM person_pref_comm frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.person_pref_comm_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
   ...
   IF (curqual=0)
    ...
    RETURN(0)
   ENDIF
   IF (add_hist(s_uf_pk_id)=0)
    SET failed = insert_error
    SET request->error_message = substring(1,132,build(
      "Could not insert history for person_pref_comm with pk=",s_uf_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 ...
 SUBROUTINE initialize_hist_id(null)
  IF (v_hist_init_ind=0)
   ...
   SET dcipht_request->transaction_reason_txt = "DM_PCMB_PERSON_PREF_COMM"
