CREATE PROGRAM dm2_apply_sizing:dba
 ...
 SUBROUTINE drr_chk_active_runners(dcar_dblink,dcar_count_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_apply_sizing",".log","DM2_APPLY_SIZING LOG FILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "BEGIN DM2_APPLY_SIZING"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE das_ind_header = i4 WITH protect, constant(9)
 DECLARE das_tbl_header = i4 WITH protect, constant(3)
 DECLARE das_o_fudge = f8 WITH protect, constant(1.45)
 DECLARE das_factor = f8 WITH protect, constant(2.0)
 DECLARE m_long_val = f8 WITH protect, constant(163840.0)
 DECLARE lmt_min_ext_size = f8 WITH protect, constant(65536.0)
 DECLARE das_date_len = i4 WITH protect, constant(8)
 DECLARE das_timestamp_len = i4 WITH protect, constant(12)
 DECLARE das_rowid_len = i2 WITH protect, constant(10)
 DECLARE das_avg_sum = f8 WITH protect, noconstant(0.0)
 DECLARE ind_factor = f8 WITH protect, constant(1.45)
 DECLARE das_a_sum = f8 WITH protect, noconstant(0.0)
 DECLARE das_rat = f8 WITH protect, noconstant(0.0)
 DECLARE das_tbl_ndx = i4 WITH protect, noconstant(0)
 DECLARE das_col_ndx = i4 WITH protect, noconstant(0)
 DECLARE ceil_factor = i4 WITH protect, noconstant(0)
 DECLARE max_ind_seq = i4 WITH protect, noconstant(0)
 DECLARE max_tblcol_seq = i4 WITH protect, noconstant(0)
 DECLARE max_indcol_seq = i4 WITH protect, noconstant(0)
 DECLARE tsp_cnt = i4 WITH protect, noconstant(0)
 DECLARE nidx_cnt = i4 WITH protect, noconstant(0)
 DECLARE idx_cnt = i4 WITH protect, noconstant(0)
 DECLARE idx_colcnt = i4 WITH protect, noconstant(0)
 DECLARE tblc_cnt = i4 WITH protect, noconstant(0)
 DECLARE t_cnt = i4 WITH protect, noconstant(0)
 DECLARE dms_idx = i4 WITH protect, noconstant(0)
 DECLARE lmt_64k = f8 WITH protect, constant(65536.0)
 DECLARE lmt_1m = f8 WITH protect, constant(1048576.0)
 DECLARE lmt_8m = f8 WITH protect, constant(8388608.0)
 DECLARE lmt_64m = f8 WITH protect, constant(67108864.0)
 DECLARE lmt_1g = f8 WITH protect, constant(1073741824.0)
 DECLARE das_ttgtidx = i4 WITH protect, noconstant(0)
 DECLARE das_tcuridx = i4 WITH protect, noconstant(0)
 DECLARE das_t = i4 WITH protect, noconstant(0)
 DECLARE das_icuridx = i4 WITH protect, noconstant(0)
 DECLARE das_i = i4 WITH protect, noconstant(0)
 DECLARE das_tot_size = f8 WITH protect, noconstant(0.0)
 DECLARE das_last_analyzed_days = i4 WITH protect, noconstant(30)
 DECLARE das_mv_indrow_size = i4 WITH protect, noconstant(0)
 DECLARE das_mv_indcol_size = i4 WITH protect, noconstant(0)
 DECLARE das_avgcol_func_valid = i2 WITH protect, noconstant(0)
 DECLARE das_nbrull_func_valid = i2 WITH protect, noconstant(0)
 DECLARE das_start_pos = i4 WITH protect, noconstant(0)
 DECLARE das_end_pos = i4 WITH protect, noconstant(0)
 DECLARE das_length = i4 WITH protect, noconstant(0)
 DECLARE das_string_idx = i4 WITH protect, noconstant(0)
 DECLARE das_comma_delimited_columns = vc WITH protect, noconstant(" ")
 DECLARE cal_round_size(crs_size=f8,crs_nextext=f8) = f8
 DECLARE get_data_length(dtl_tbl_seq=i4,dtl_tc_seq=i4) = f8
 DECLARE das_get_max_seq(null) = null
 DECLARE das_update_size(null) = null
 DECLARE sub_calc_new_ind_new_tbl(nint_tblcnt=i4,nint_idxcnt=i4) = null
 DECLARE sub_calc_new_tbl(tbl_seq=i4) = null
 DECLARE sub_tot_new_idx_exist_tbl(ets_tblcnt=i4,ets_indcnt=i4) = null
 DECLARE sub_calc_idx_null_tonot_null(intn_tblcnt=i4,intn_colcnt=i4,intn_idxcnt=i4,intn_curidx=i4) =
 null
 DECLARE sub_calc_null_to_not_null(das_tbl_seq=i4,das_col_seq=i4) = null
 DECLARE sub_calc_new_not_null(scn_tbl_seq=i4,scn_col_seq=i4) = i2
 DECLARE sub_calc_tot_size_not_null(tsnn_tblcnt=i4,tsnn_curidx=i4) = null
 DECLARE sub_add_lob_size(als_tbl_cnt=i4) = null
 DECLARE sub_big_nonew_notfixed(nn_tblcnt=i4,nn_idxcnt=i4,nn_tblcol=i4) = null
 DECLARE sub_assign_ext_size(null) = null
 DECLARE sub_clinical_ext_size(ces_type=c1,ces_seq=i4,ces_ind_seq=i4) = null
 DECLARE sub_new_idx_exist_tbl(tsp_cnt=i4,dms_idx=i4,idx_cnt=i4) = null
 DECLARE das_get_col_size(dgc_rec=c1,dgc_tidx=i4,dgc_cidx=i4,dgc_lrg_dload=i2) = f8
 DECLARE das_get_col_size2(dgc2_rec=c1,dgc2_tidx=i4,dgc2_cidx=i4,dgc2_lrg_dload=i2) = f8
 DECLARE das_ins_tspace_obj_size(dit_rtype=vc,dit_otype=vc,dit_tidx=i4,dit_iidx=i4,dit_cidx=i4) = i2
 DECLARE das_load_obj_space_needs(null) = i2
 DECLARE das_cal_rr_ins_size(dcr_ttgtidx=i4,dcr_tcuridx=i4,dcr_t=i4) = null
 DECLARE das_cal_rr_updt_size(dcru_ttgtidx=i4,dcru_tcuridx=i4,dcru_t=i4) = null
 DECLARE das_get_mview_sizing(dgmsd_tblcnt=i4) = i2
 DECLARE das_verify_function(dvf_function_name=vc) = i2
 DECLARE das_comma_delimited_string(dcds_text=vc) = null
 DECLARE cal_lmt_ext_size(cle_currsize=f8) = f8
 FREE RECORD das_columns
 RECORD das_columns(
   1 tbl_cnt = i4
   1 tbl[*]
     2 tbl_name = vc
 ...
 FREE RECORD das_separated_columns
 RECORD das_separated_columns(
   1 list_cnt = i4
   1 list[*]
     2 val = vc
 )
 IF ((validate(dm2_analyzed_days,- (1)) != - (1)))
  SET das_last_analyzed_days = dm2_analyzed_days
 ENDIF
 IF ((dm2_install_schema->run_id > 0))
  SET dm_err->eproc = "Gathering ADD COLUMN op_types."
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   ...
   HEAD REPORT
    das_columns->tbl_cnt = 0
   HEAD d.table_name
    das_columns->tbl_cnt = (das_columns->tbl_cnt+ 1)
    IF (mod(das_columns->tbl_cnt,10)=1)
     stat = alterlist(das_columns->tbl,(das_columns->tbl_cnt+ 9))
    ENDIF
    das_columns->tbl[das_columns->tbl_cnt].tbl_name = d.table_name
   DETAIL
    IF (findstring(",",d.obj_name,1,1) > 0
     AND findstring("<more>",d.obj_name,1,1)=0)
     CALL das_comma_delimited_string(d.obj_name)
     FOR (das_string_idx = 1 TO das_separated_columns->list_cnt)
       das_columns->tbl[das_columns->tbl_cnt].col_cnt = (das_columns->tbl[das_columns->tbl_cnt].
       col_cnt+ 1), stat = alterlist(das_columns->tbl[das_columns->tbl_cnt].col,(das_columns->tbl[
        das_columns->tbl_cnt].col_cnt+ 1)), das_columns->tbl[das_columns->tbl_cnt].col[das_columns->
       tbl[das_columns->tbl_cnt].col_cnt].col_name = das_separated_columns->list[das_string_idx].val
     ENDFOR
    ELSEIF (findstring(",",d.obj_name,1,1) > 0
     AND findstring("<more>",d.obj_name,1,1) > 0)
     das_start_pos = findstring("column_list:",d.operation,1,0), das_end_pos = findstring("*/",d
      .operation,1,0), das_length = (das_end_pos - das_start_pos),
     das_comma_delimited_columns = substring((das_start_pos+ 13),(das_length - 13),d.operation),
     CALL das_comma_delimited_string(das_comma_delimited_columns)
     FOR (das_string_idx = 1 TO das_separated_columns->list_cnt)
       das_columns->tbl[das_columns->tbl_cnt].col_cnt = (das_columns->tbl[das_columns->tbl_cnt].
       col_cnt+ 1), stat = alterlist(das_columns->tbl[das_columns->tbl_cnt].col,(das_columns->tbl[
        das_columns->tbl_cnt].col_cnt+ 1)), das_columns->tbl[das_columns->tbl_cnt].col[das_columns->
     ...
     ENDFOR
    ELSE
     das_columns->tbl[das_columns->tbl_cnt].col_cnt = (das_columns->tbl[das_columns->tbl_cnt].col_cnt
     + 1), stat = alterlist(das_columns->tbl[das_columns->tbl_cnt].col,(das_columns->tbl[das_columns
      ->tbl_cnt].col_cnt+ 1)), das_columns->tbl[das_columns->tbl_cnt].col[das_columns->tbl[
     das_columns->tbl_cnt].col_cnt].col_name = d.obj_name
    ENDIF
   FOOT REPORT
    stat = alterlist(das_columns->tbl,das_columns->tbl_cnt)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF ((dm2_rr_spc_needs->space_needed=1))
  IF ((tgtsch->tbl_cnt > 0)
   AND (cur_sch->tbl_cnt > 0))
   EXECUTE dm2_get_space_used "B"
  ELSEIF ((cur_sch->tbl_cnt > 0))
   EXECUTE dm2_get_space_used "C"
  ENDIF
 ELSEIF ((tgtsch->tbl_cnt > 0))
  EXECUTE dm2_get_space_used "T"
 ENDIF
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 CALL sub_assign_ext_size(null)
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 CALL das_get_max_seq(null)
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 IF ((tgtsch->diff_ind=1))
  FOR (tsp_cnt = 1 TO tgtsch->tbl_cnt)
   IF ((tgtsch->tbl[tsp_cnt].mview_flag=1)
    AND (tgtsch->tbl[tsp_cnt].mview_build_ind=1)
    AND (((tgtsch->tbl[tsp_cnt].new_ind=1)) OR ((tgtsch->tbl[tsp_cnt].diff_ind=1))) )
    IF (das_get_mview_sizing(tsp_cnt)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   IF ((tgtsch->tbl[tsp_cnt].new_ind=1)
    AND (tgtsch->tbl[tsp_cnt].mview_build_ind=0))
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("Processing new table ",tgtsch->tbl[tsp_cnt].tbl_name))
    ENDIF
    CALL sub_calc_new_tbl(tsp_cnt)
    IF ((dm_err->err_ind=1))
     GO TO exit_script
    ENDIF
    FOR (nidx_cnt = 1 TO tgtsch->tbl[tsp_cnt].ind_cnt)
      IF ((tgtsch->tbl[tsp_cnt].ind[nidx_cnt].build_ind=1))
       CALL sub_calc_new_ind_new_tbl(tsp_cnt,nidx_cnt)
       IF ((dm_err->err_ind=1))
        GO TO exit_script
       ENDIF
      ENDIF
    ENDFOR
   ELSEIF ((tgtsch->tbl[tsp_cnt].diff_ind=1)
    AND (tgtsch->tbl[tsp_cnt].mview_build_ind=0))
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("Processing existing table ",tgtsch->tbl[tsp_cnt].tbl_name))
    ENDIF
    FOR (idx_cnt = 1 TO tgtsch->tbl[tsp_cnt].ind_cnt)
      IF ((tgtsch->tbl[tsp_cnt].ind[idx_cnt].build_ind=1))
       FOR (idx_colcnt = 1 TO tgtsch->tbl[tsp_cnt].ind[idx_cnt].ind_col_cnt)
         SET dms_idx = 0
         SET dms_idx = locateval(dms_idx,1,tgtsch->tbl[tsp_cnt].tbl_col_cnt,tgtsch->tbl[tsp_cnt].ind[
          idx_cnt].ind_col[idx_colcnt].col_name,tgtsch->tbl[tsp_cnt].tbl_col[dms_idx].col_name)
         IF (dms_idx > 0)
          CALL sub_new_idx_exist_tbl(tsp_cnt,dms_idx,idx_cnt)
          IF ((dm_err->err_ind=1))
           GO TO exit_script
          ENDIF
         ELSE
          SET dm_err->err_ind = 1
          SET dm_err->emsg = concat("Can not find index column ",tgtsch->tbl[tsp_cnt].ind[idx_cnt].
           ind_col[idx_colcnt].col_name," on table ",tgtsch->tbl[tsp_cnt].tbl_name)
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          GO TO exit_script
         ENDIF
       ENDFOR
       CALL sub_tot_new_idx_exist_tbl(tsp_cnt,idx_cnt)
       IF ((dm_err->err_ind=1))
        GO TO exit_script
       ENDIF
      ENDIF
    ENDFOR
    FOR (tblc_cnt = 1 TO tgtsch->tbl[tsp_cnt].tbl_col_cnt)
     IF ((tgtsch->tbl[tsp_cnt].tbl_col[tblc_cnt].nullable="N")
      AND (tgtsch->tbl[tsp_cnt].tbl_col[tblc_cnt].new_ind=1))
      IF (sub_calc_new_not_null(tsp_cnt,tblc_cnt)=0)
       GO TO exit_script
      ENDIF
     ELSEIF ((tgtsch->tbl[tsp_cnt].tbl_col[tblc_cnt].null_to_notnull_ind=1))
      SET dm_err->eproc = "Sizing for column changes from null to not null"
      SET das_tbl_ndx = 0
      SET das_col_ndx = 0
      SET das_tbl_ndx = locateval(das_tbl_ndx,1,das_columns->tbl_cnt,tgtsch->tbl[tsp_cnt].tbl_name,
       das_columns->tbl[das_tbl_ndx].tbl_name)
      IF (das_tbl_ndx > 0)
       SET das_col_ndx = locateval(das_col_ndx,1,das_columns->tbl[das_tbl_ndx].col_cnt,tgtsch->tbl[
        tsp_cnt].tbl_col[tblc_cnt].col_name,das_columns->tbl[das_tbl_ndx].col[das_col_ndx].col_name)
      ENDIF
      IF (das_col_ndx > 0
       AND das_tbl_ndx > 0)
       IF ((dm_err->debug_flag > 0))
        CALL echo("***")
        CALL echo("Revert to new column sizing since column added during previous execution.")
        CALL echo(build("tbl=",tgtsch->tbl[tsp_cnt].tbl_name))
        CALL echo(build("col=",tgtsch->tbl[tsp_cnt].tbl_col[tblc_cnt].col_name))
        CALL echo("***")
       ENDIF
       IF (sub_calc_new_not_null(tsp_cnt,tblc_cnt)=0)
        GO TO exit_script
       ENDIF
      ELSE
       IF ((tgtsch->tbl[tsp_cnt].row_cnt < 10000))
        SET dm_err->eproc = concat("Obtain sizing for column on table < 10000 rows:",tgtsch->tbl[
         tsp_cnt].tbl_col[tblc_cnt].col_name)
        IF ((dm_err->debug_flag > 0))
         CALL disp_msg("",dm_err->logfile,0)
        ENDIF
        SET tgtsch->tbl[tsp_cnt].size = (tgtsch->tbl[tsp_cnt].size+ get_data_length(tsp_cnt,tblc_cnt)
        )
        IF (check_error(dm_err->eproc)=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         GO TO exit_script
        ENDIF
       ELSEIF (datetimediff(cnvtdatetime(curdate,curtime3),tgtsch->tbl[tsp_cnt].last_analyzed_dt_tm)
        <= das_last_analyzed_days)
        SET dm_err->eproc = concat("Obtain AVG_COL_LEN for null to not null column:",tgtsch->tbl[
         tsp_cnt].tbl_col[tblc_cnt].col_name)
        IF ((dm_err->debug_flag > 0))
         CALL disp_msg("",dm_err->logfile,0)
        ENDIF
        SELECT INTO "nl:"
         FROM dm_user_columns_actual_stats u
         WHERE (u.table_name=tgtsch->tbl[tsp_cnt].tbl_name)
          AND (u.column_name=tgtsch->tbl[tsp_cnt].tbl_col[tblc_cnt].col_name)
         DETAIL
          tgtsch->tbl[tsp_cnt].tbl_col[tblc_cnt].size = u.avg_col_len
         WITH nocounter
        ;end select
        IF (check_error(dm_err->eproc)=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         GO TO exit_script
        ENDIF
        SET tgtsch->tbl[tsp_cnt].size = (tgtsch->tbl[tsp_cnt].size+ tgtsch->tbl[tsp_cnt].tbl_col[
        tblc_cnt].size)
        IF ((dm_err->debug_flag > 0))
         CALL echo("***")
         CALL echo("Null to not Null from AVG_COL_LEN")
         CALL echo(build("tbl=",tgtsch->tbl[tsp_cnt].tbl_name))
         CALL echo(build("col=",tgtsch->tbl[tsp_cnt].tbl_col[tblc_cnt].col_name))
         CALL echo(build("tbl_size=",tgtsch->tbl[tsp_cnt].size))
         CALL echo("***")
        ENDIF
       ELSE
        CALL sub_calc_null_to_not_null(tsp_cnt,tblc_cnt)
        IF ((dm_err->err_ind=1))
         GO TO exit_script
        ENDIF
       ENDIF
       FOR (nnidx_cnt = 1 TO tgtsch->tbl[tsp_cnt].ind_cnt)
         IF ((tgtsch->tbl[tsp_cnt].ind[nnidx_cnt].build_ind=0))
          CALL sub_calc_idx_null_tonot_null(tsp_cnt,tblc_cnt,nnidx_cnt,tgtsch->tbl[tsp_cnt].cur_idx)
          IF ((dm_err->err_ind=1))
           GO TO exit_script
          ENDIF
         ENDIF
      ...
      ENDIF
     ENDIF
     IF ((tgtsch->tbl[tsp_cnt].tbl_col[tblc_cnt].new_ind=1)
      AND (tgtsch->tbl[tsp_cnt].tbl_col[tblc_cnt].data_type IN ("CLOB", "BLOB")))
      SET dm_err->eproc = "Size for new LOB column on existing tables"
      CALL sub_add_lob_size(tsp_cnt)
      IF ((dm_err->err_ind=1))
       GO TO exit_script
      ENDIF
     ENDIF
    ENDFOR
    IF ((tgtsch->tbl[tsp_cnt].size > 0))
     SET dm_err->eproc = "Size for column from Nullable to not NULL and new not NULL"
     CALL sub_calc_tot_size_not_null(tsp_cnt,tgtsch->tbl[tsp_cnt].cur_idx)
     IF ((dm_err->err_ind=1))
      GO TO exit_script
     ENDIF
    ENDIF
   ENDIF
  ENDFOR
  CALL das_update_size(null)
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL debug_disp_tgt_size(null)
  ENDIF
 ENDIF
 IF ((dm2_rr_spc_needs->space_needed=1))
  FOR (das_t = 1 TO dm2_rr_spc_needs->tbl_cnt)
    IF ((dm2_rr_spc_needs->tbl[das_t].skip_ind=0))
     SET das_ttgtidx = dm2_rr_spc_needs->tbl[das_t].tgt_idx
     SET das_tcuridx = dm2_rr_spc_needs->tbl[das_t].cur_idx
     IF ((dm2_rr_spc_needs->tbl[das_t].insert_row_cnt > 0))
      CALL das_cal_rr_ins_size(das_ttgtidx,das_tcuridx,das_t)
     ELSEIF ((dm2_rr_spc_needs->tbl[das_t].col_updt_cnt > 0))
      CALL das_cal_rr_updt_size(das_ttgtidx,das_tcuridx,das_t)
     ENDIF
     IF ((cur_sch->tbl[das_tcuridx].ext_mgmt="D"))
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("spc_needs = ",dm2_rr_spc_needs->tbl[das_t].space_needed))
      ENDIF
      SET dm2_rr_spc_needs->tbl[das_t].space_needed = cal_round_size(dm2_rr_spc_needs->tbl[das_t].
       space_needed,cur_sch->tbl[das_tcuridx].next_ext)
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("tbl_next_ext =",cur_sch->tbl[das_tcuridx].next_ext))
       CALL echo(build("spc_round_to = ",dm2_rr_spc_needs->tbl[das_t].space_needed))
      ENDIF
     ELSEIF ((cur_sch->tbl[das_tcuridx].ext_mgmt="L"))
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("spc_needs = ",dm2_rr_spc_needs->tbl[das_t].space_needed))
      ENDIF
      SET das_tot_size = 0.0
      SET das_tot_size = (dm2_rr_spc_needs->tbl[das_t].space_needed+ cur_sch->tbl[das_tcuridx].
      bytes_allocated)
      SET cur_sch->tbl[das_tcuridx].next_ext = cal_lmt_ext_size(das_tot_size)
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("size = ",dm2_rr_spc_needs->tbl[das_t].space_needed))
       CALL echo(build("bytes_allocated =",cur_sch->tbl[das_tcuridx].bytes_allocated))
       CALL echo(build("cur table next_ext =",cur_sch->tbl[das_tcuridx].next_ext))
      ENDIF
      SET dm2_rr_spc_needs->tbl[das_t].space_needed = cal_round_size(dm2_rr_spc_needs->tbl[das_t].
       space_needed,cur_sch->tbl[das_tcuridx].next_ext)
      IF ((dm_err->debug_flag > 2))
      ...
      ENDIF
     ENDIF
     FOR (das_i = 1 TO dm2_rr_spc_needs->tbl[das_t].ind_cnt)
      SET das_icuridx = dm2_rr_spc_needs->tbl[das_t].ind[das_i].cur_idx
      IF ((cur_sch->tbl[das_tcuridx].ind[das_icuridx].ext_mgmt="D"))
       IF ((dm_err->debug_flag > 2))
        CALL echo(build("spc_needs = ",dm2_rr_spc_needs->tbl[das_t].ind[das_i].space_needed))
       ENDIF
       SET dm2_rr_spc_needs->tbl[das_t].ind[das_i].space_needed = cal_round_size(dm2_rr_spc_needs->
        tbl[das_t].ind[das_i].space_needed,cur_sch->tbl[das_tcuridx].ind[das_icuridx].next_ext)
       IF ((dm_err->debug_flag > 2))
        CALL echo(build("ind_next_ext =",cur_sch->tbl[das_tcuridx].ind[das_icuridx].next_ext))
        CALL echo(build("spc_round_to = ",dm2_rr_spc_needs->tbl[das_t].ind[das_i].space_needed))
       ENDIF
      ELSEIF ((cur_sch->tbl[das_tcuridx].ind[das_icuridx].ext_mgmt="L"))
       IF ((dm_err->debug_flag > 2))
        CALL echo(build("spc_needs = ",dm2_rr_spc_needs->tbl[das_t].ind[das_i].space_needed))
       ENDIF
       SET das_tot_size = 0.0
       SET das_tot_size = (dm2_rr_spc_needs->tbl[das_t].ind[das_i].space_needed+ cur_sch->tbl[
       das_tcuridx].ind[das_icuridx].bytes_allocated)
       SET cur_sch->tbl[das_tcuridx].ind[das_icuridx].next_ext = cal_lmt_ext_size(das_tot_size)
       IF ((dm_err->debug_flag > 2))
        CALL echo(build("space_needed =",dm2_rr_spc_needs->tbl[das_t].ind[das_i].space_needed))
        CALL echo(build("bytes_allocated =",cur_sch->tbl[das_tcuridx].ind[das_icuridx].
          bytes_allocated))
        CALL echo(build("cur index next_ext =",cur_sch->tbl[das_tcuridx].ind[das_icuridx].next_ext))
       ENDIF
       SET dm2_rr_spc_needs->tbl[das_t].ind[das_i].space_needed = cal_round_size(dm2_rr_spc_needs->
        tbl[das_t].ind[das_i].space_needed,cur_sch->tbl[das_tcuridx].ind[das_icuridx].next_ext)
       IF ((dm_err->debug_flag > 2))
  ...
  ENDFOR
 ENDIF
 IF (das_load_obj_space_needs(null)=0)
  GO TO exit_script
 ENDIF
 GO TO exit_script
 SUBROUTINE cal_round_size(crs_size,crs_nextent)
   DECLARE crs_ret_size = f8 WITH protect, noconstant(0.0)
   IF (crs_nextent=0)
   ...
   ENDIF
   SET ceil_factor = 0
   SET ceil_factor = dm2ceil((crs_size/ crs_nextent))
   SET crs_ret_size = (crs_nextent * ceil_factor)
   RETURN(crs_ret_size)
 END ;Subroutine
 SUBROUTINE cal_lmt_ext_size(cle_currsize)
   DECLARE cle_next_ext = f8 WITH protect, noconstant(0.0)
   IF (cle_currsize > 0
    AND cle_currsize <= lmt_1m)
    SET cle_next_ext = lmt_64k
   ELSEIF (cle_currsize > lmt_1m
    AND cle_currsize <= lmt_64m)
    SET cle_next_ext = lmt_1m
   ELSEIF (cle_currsize > lmt_64m
    AND cle_currsize <= lmt_1g)
    SET cle_next_ext = lmt_8m
   ELSE
    SET cle_next_ext = lmt_64m
   ENDIF
   RETURN(cle_next_ext)
 END ;Subroutine
 SUBROUTINE das_get_max_seq(null)
   SET dm_err->eproc = "get max sequence"
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(tgtsch->tbl_cnt))
    DETAIL
     IF ((max_ind_seq < tgtsch->tbl[d.seq].ind_cnt))
      max_ind_seq = tgtsch->tbl[d.seq].ind_cnt
     ENDIF
     IF ((max_tblcol_seq < tgtsch->tbl[d.seq].tbl_col_cnt))
      max_tblcol_seq = tgtsch->tbl[d.seq].tbl_col_cnt
     ENDIF
     FOR (dgms_lp = 1 TO tgtsch->tbl[d.seq].ind_cnt)
       IF ((max_indcol_seq < tgtsch->tbl[d.seq].ind[dgms_lp].ind_col_cnt))
        max_indcol_seq = tgtsch->tbl[d.seq].ind[dgms_lp].ind_col_cnt
       ENDIF
     ENDFOR
    WITH nocounter
   ;end select
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE get_data_length(dtl_tbl_seq,dtl_tc_seq)
   DECLARE gdl_data_type = vc WITH protect
   DECLARE gdl_colsum = f8 WITH protect, noconstant(0.0)
   SET gdl_data_type = tgtsch->tbl[dtl_tbl_seq].tbl_col[dtl_tc_seq].data_type
   IF (gdl_data_type="TIMESTAMP*")
    SET gdl_colsum = das_timestamp_len
   ELSEIF (gdl_data_type="CHAR")
    SET gdl_colsum = (tgtsch->tbl[dtl_tbl_seq].tbl_col[dtl_tc_seq].data_length+ 1)
   ELSEIF (gdl_data_type="DATE")
    SET gdl_colsum = das_date_len
   ELSEIF (gdl_data_type="ROWID")
    SET gdl_colsum = das_rowid_len
   ELSEIF ((tgtsch->tbl[dtl_tbl_seq].tbl_col[dtl_tc_seq].virtual_column="YES"))
    SET gdl_colsum = 1
   ELSE
    SET gdl_colsum = (size(trim(tgtsch->tbl[dtl_tbl_seq].tbl_col[dtl_tc_seq].data_default))+ 1)
    IF ((dm_err->debug_flag > 0))
     CALL echo(build("data default=",tgtsch->tbl[dtl_tbl_seq].tbl_col[dtl_tc_seq].data_default,
       "/size =",gdl_colsum,":",
       tgtsch->tbl[dtl_tbl_seq].tbl_name,"/",tgtsch->tbl[dtl_tbl_seq].tbl_col[dtl_tc_seq].col_name))
    ENDIF
   ENDIF
   RETURN(gdl_colsum)
 END ;Subroutine
 SUBROUTINE das_update_size(null)
   DECLARE dus_sz = f8 WITH protect, noconstant(0.0)
   SET dm_err->eproc = "update ind_size to the total size"
   IF ((dm_err->debug_flag > 0))
    CALL echo(dm_err->eproc)
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt dt  WITH seq = value(tgtsch->tbl_cnt)),
     (dummyt di  WITH seq = max_ind_seq)
    PLAN (dt)
     JOIN (di
     WHERE (di.seq <= tgtsch->tbl[dt.seq].ind_cnt))
    ORDER BY dt.seq, di.seq
    HEAD dt.seq
     dus_sz = 0
    DETAIL
     IF ((tgtsch->tbl[dt.seq].ind[di.seq].build_ind=1))
      dus_sz = (dus_sz+ tgtsch->tbl[dt.seq].ind[di.seq].size)
     ENDIF
    FOOT  dt.seq
     tgtsch->tbl[dt.seq].ind_size = dus_sz
     IF ((dm_err->debug_flag > 1))
      CALL echo(build("Total index size for table(",tgtsch->tbl[dt.seq].tbl_name,")=",dus_sz))
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE sub_calc_new_tbl(tbl_seq)
   SET dm_err->eproc = "Sizing for new tables"
   IF ((dm_err->debug_flag > 0))
    CALL echo(dm_err->eproc)
   ENDIF
   IF ((tgtsch->tbl[tbl_seq].dext_mgmt="D"))
    SET tgtsch->tbl[tbl_seq].size = tgtsch->tbl[tbl_seq].init_ext
   ELSE
    SET tgtsch->tbl[tbl_seq].size = lmt_min_ext_size
   ENDIF
   FOR (t_cnt = 1 TO tgtsch->tbl[tbl_seq].tbl_col_cnt)
     IF ((tgtsch->tbl[tbl_seq].tbl_col[t_cnt].data_type="*LOB*"))
      SET tgtsch->tbl[tbl_seq].long_size = (tgtsch->tbl[tbl_seq].long_size+ m_long_val)
     ENDIF
   ENDFOR
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
 END ;Subroutine
 SUBROUTINE sub_calc_new_ind_new_tbl(nint_tblcnt,nint_idxcnt)
   SET dm_err->eproc = "Sizing for new indexes on new tables"
   IF ((dm_err->debug_flag > 0))
    CALL echo(dm_err->eproc)
   ENDIF
   IF ((tgtsch->tbl[nint_tblcnt].iext_mgmt="D"))
    SET tgtsch->tbl[nint_tblcnt].ind[nint_idxcnt].size = (tgtsch->tbl[nint_tblcnt].ind[nint_idxcnt].
    init_ext+ dmt_min_ext_size)
   ELSE
    SET tgtsch->tbl[nint_tblcnt].ind[nint_idxcnt].size = (lmt_min_ext_size * 2)
   ENDIF
   SET tgtsch->tbl[nint_tblcnt].ind[nint_idxcnt].size = cal_round_size(tgtsch->tbl[nint_tblcnt].ind[
    nint_idxcnt].size,tgtsch->tbl[nint_tblcnt].ind[nint_idxcnt].next_ext)
   IF ((dm_err->debug_flag > 1))
    CALL echo(build(tgtsch->tbl[nint_tblcnt].ind[nint_idxcnt].ind_name,": size=",tgtsch->tbl[
      nint_tblcnt].ind[nint_idxcnt].size))
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
 END ;Subroutine
 SUBROUTINE sub_big_nonew_notfixed(nn_tblcnt,nn_idxcnt,nn_tblcol)
   SET dm_err->eproc = "Execute sub_big_nonew_notfixed..."
   IF ((dm_err->debug_flag > 0))
    CALL echo(dm_err->eproc)
   ENDIF
   DECLARE das_size_stmt = vc WITH protect
   DECLARE das_tbl_name1 = vc WITH protect
   DECLARE das_col_name1 = vc WITH protect
   DECLARE das_ind_size1 = f8 WITH protect, noconstant(0.0)
   DECLARE das_final_avg_col_len = f8 WITH protect, noconstant(- (1.0))
   DECLARE das_col_fnd_ind1 = i2 WITH protect, noconstant(0)
   DECLARE das_coldef_fnd_ind1 = i2 WITH protect, noconstant(0)
   DECLARE das_data_type1 = vc WITH protect, noconstant("")
   SET das_avg_sum = 0.0
   SET das_tbl_name1 = tgtsch->tbl[nn_tblcnt].tbl_name
   SET das_col_name1 = tgtsch->tbl[nn_tblcnt].tbl_col[nn_tblcol].col_name
   SET das_ind_size1 = tgtsch->tbl[nn_tblcnt].ind[nn_idxcnt].size
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("tbl name:",das_tbl_name1))
    CALL echo(concat("col name:",das_col_name1))
   ENDIF
   SET das_size_stmt = build("vsize(d.",das_col_name1,")")
   SET dm_err->eproc = concat("Check for stats on ",das_tbl_name1,".",das_col_name1)
   IF ((dm_err->debug_flag > 0))
    CALL echo(dm_err->eproc)
   ENDIF
   SELECT INTO "nl:"
    FROM dm_user_columns_actual_stats utc
    WHERE utc.table_name=das_tbl_name1
     AND utc.column_name=das_col_name1
     AND utc.last_analyzed IS NOT null
    DETAIL
     das_final_avg_col_len = utc.avg_col_len
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN
   ENDIF
   IF ((das_final_avg_col_len=- (1)))
    CASE (trim(tgtsch->tbl[nn_tblcnt].tbl_col[nn_tblcol].data_type,3))
     OF "NUMBER":
      SET das_final_avg_col_len = 3.2
     OF "DATE":
      SET das_final_avg_col_len = 5.9
     OF "FLOAT":
      SET das_final_avg_col_len = 2.2
    ENDCASE
    SET dm_err->eproc = "Check for avg_col_len overrides"
    IF ((dm_err->debug_flag > 0))
     CALL echo(dm_err->eproc)
    ENDIF
    SELECT INTO "nl:"
     FROM dm_info di
     WHERE di.info_domain="DM_AVG_COL_LEN"
      AND di.info_name=trim(tgtsch->tbl[nn_tblcnt].tbl_col[nn_tblcol].data_type,3)
     DETAIL
      das_final_avg_col_len = di.info_number
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   SET das_avg_sum = 0.0
   IF ((das_final_avg_col_len > - (1.0)))
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat(das_tbl_name1,".",das_col_name1,":",trim(tgtsch->tbl[nn_tblcnt].tbl_col[
        nn_tblcol].data_type,3),
       ":",build(das_final_avg_col_len)))
    ENDIF
    SET das_avg_sum = das_final_avg_col_len
   ELSE
    SET dm_err->eproc = concat("Check if table/column ",das_tbl_name1,"/",das_col_name1,
     " exists in CCL defintion.")
    IF ((dm_err->debug_flag > 0))
     CALL echo(dm_err->eproc)
    ENDIF
    IF (dm2_table_column_exists("",das_tbl_name1,das_col_name1,0,1,
     1,das_col_fnd_ind1,das_coldef_fnd_ind1,das_data_type1)=0)
     RETURN
    ENDIF
    IF (das_coldef_fnd_ind1=0)
     IF (das_verify_function("DM2_GET_AVG_COL_SIZE")=0)
      RETURN
     ENDIF
     SET dm_err->eproc = concat("Get average column size for table/column ",das_tbl_name1,"/",
      das_col_name1,".")
     IF ((dm_err->debug_flag > 0))
      CALL echo(dm_err->eproc)
     ENDIF
     SELECT INTO "nl:"
      x = sqlpassthru(concat("dm2_get_avg_col_size('",das_tbl_name1,"','",das_col_name1,"')"),0)
      FROM dual
      DETAIL
       das_avg_sum = x
      WITH nocounter
     ...
     ENDIF
    ELSE
     EXECUTE dm2_apply_sizing_parser 1, das_size_stmt, das_tbl_name1,
     das_col_name1
     IF ((dm_err->err_ind=1))
      RETURN
     ENDIF
    ENDIF
   ENDIF
   SET tgtsch->tbl[nn_tblcnt].ind[nn_idxcnt].size = (das_ind_size1+ (das_avg_sum+ 1))
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("average size for (",das_tbl_name1,"/",das_col_name1,")=",
      das_avg_sum))
    CALL echo(build("case1=",tgtsch->tbl[nn_tblcnt].ind[nn_idxcnt].size))
   ENDIF
 END ;Subroutine
 SUBROUTINE das_comma_delimited_string(dcds_text)
   DECLARE dcds_prev_pos = i4 WITH protect, noconstant(0)
   DECLARE dcds_pos = i4 WITH protect, noconstant(0)
   DECLARE dcds_idx = i4 WITH protect, noconstant(0)
   DECLARE dcds_delim_length = i4 WITH protect, constant(1)
   DECLARE dcds_delimiter = vc WITH protect, constant(",")
   SET dcds_pos = findstring(dcds_delimiter,dcds_text,(dcds_prev_pos+ 1),0)
   IF (dcds_pos=0)
    SET das_separated_columns->list_cnt = 1
    SET stat = alterlist(das_separated_columns->list,1)
    SET das_separated_columns->list[1].val = dcds_text
   ELSE
    WHILE (dcds_pos > 0)
      SET dcds_idx = (dcds_idx+ 1)
      IF ((dcds_idx > das_separated_columns->list_cnt))
       SET das_separated_columns->list_cnt = (dcds_idx+ 1)
       SET stat = alterlist(das_separated_columns->list,das_separated_columns->list_cnt)
      ENDIF
      SET das_separated_columns->list[dcds_idx].val = substring((dcds_prev_pos+ dcds_delim_length),((
       dcds_pos - dcds_prev_pos) - dcds_delim_length),dcds_text)
      SET dcds_prev_pos = dcds_pos
      SET dcds_pos = findstring(dcds_delimiter,dcds_text,(dcds_prev_pos+ dcds_delim_length),0)
    ENDWHILE
    SET dcds_idx = (dcds_idx+ 1)
    SET das_separated_columns->list_cnt = dcds_idx
    SET stat = alterlist(das_separated_columns->list,das_separated_columns->list_cnt)
    SET das_separated_columns->list[dcds_idx].val = substring((dcds_prev_pos+ dcds_delim_length),
     textlen(trim(dcds_text)),dcds_text)
   ENDIF
 END ;Subroutine
 SUBROUTINE sub_tot_new_idx_exist_tbl(ets_tblcnt,ets_indcnt)
   SET dm_err->eproc = "estimate total size for each index"
   IF ((dm_err->debug_flag > 0))
    CALL echo(dm_err->eproc)
   ENDIF
   DECLARE sub_tmp_row_cnt = f8 WITH protect, noconstant(0.0)
   IF ((tgtsch->tbl[ets_tblcnt].row_cnt=0))
    SET sub_tmp_row_cnt = 1
   ELSE
    SET sub_tmp_row_cnt = tgtsch->tbl[ets_tblcnt].row_cnt
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("sub_tmp_row_cnt = ",sub_tmp_row_cnt))
   ENDIF
   SET tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size = (((tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size
   + das_ind_header) * sub_tmp_row_cnt) * das_o_fudge)
   IF ((tgtsch->tbl[ets_tblcnt].iext_mgmt="D"))
    IF ((tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size < tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].
    init_ext))
     SET tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size = tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].
     init_ext
    ENDIF
    SET tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size = (tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size
    + dmt_min_ext_size)
   ELSE
    SET tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size = (tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size
    + lmt_min_ext_size)
    SET tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].next_ext = cal_lmt_ext_size(tgtsch->tbl[ets_tblcnt].
     ind[ets_indcnt].size)
    IF ((dm_err->debug_flag > 0))
     CALL echo("***")
     CALL echo(build("index:",tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].ind_name))
     CALL echo(build("size=",tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size))
     CALL echo(build("cur_bytes_allocated =",tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].
       cur_bytes_allocated))
     CALL echo(build("index_next_ext=",tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].next_ext))
     CALL echo("***")
    ENDIF
   ENDIF
   SET tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size = cal_round_size(tgtsch->tbl[ets_tblcnt].ind[
    ets_indcnt].size,tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].next_ext)
   IF ((dm_err->debug_flag > 0))
    CALL echo("***")
    CALL echo(build("index:",tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].ind_name))
    CALL echo(build("rounded_size=",tgtsch->tbl[ets_tblcnt].ind[ets_indcnt].size))
    CALL echo("***")
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
 END ;Subroutine
 SUBROUTINE sub_calc_new_not_null(scn_tbl_seq,scn_col_seq)
   SET dm_err->eproc = "Sizing for new not null column"
   SET tgtsch->tbl[scn_tbl_seq].tbl_col[scn_col_seq].size = get_data_length(scn_tbl_seq,scn_col_seq)
   IF ((tgtsch->tbl[scn_tbl_seq].tbl_col[scn_col_seq].diff_backfill=1))
    SET tgtsch->tbl[scn_tbl_seq].size = (tgtsch->tbl[scn_tbl_seq].size+ tgtsch->tbl[scn_tbl_seq].
    tbl_col[scn_col_seq].size)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo("***")
    CALL echo("New not Null")
    CALL echo(build("tbl=",tgtsch->tbl[scn_tbl_seq].tbl_name))
    CALL echo(build("col=",tgtsch->tbl[scn_tbl_seq].tbl_col[scn_col_seq].col_name))
    CALL echo(build("tbl_size=",tgtsch->tbl[scn_tbl_seq].size))
    CALL echo("***")
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sub_calc_null_to_not_null(das_tbl_seq,das_col_seq)
   SET dm_err->eproc = "execute sub_calc_null_to_not_null"
   IF ((dm_err->debug_flag > 0))
    CALL echo(dm_err->eproc)
   ENDIF
   DECLARE das_tbl_name2 = vc WITH protect
   DECLARE das_col_name2 = vc WITH protect
   DECLARE das_tbl_size = f8 WITH protect, noconstant(0.0)
   DECLARE das_col_fnd_ind2 = i2 WITH protect, noconstant(0)
   DECLARE das_coldef_fnd_ind2 = i2 WITH protect, noconstant(0)
   DECLARE das_data_type2 = vc WITH protect, noconstant("")
   SET das_tbl_name2 = tgtsch->tbl[das_tbl_seq].tbl_name
   SET das_col_name2 = tgtsch->tbl[das_tbl_seq].tbl_col[das_col_seq].col_name
   SET das_tbl_size = tgtsch->tbl[das_tbl_seq].size
   IF ((dm_err->debug_flag > 0))
    CALL echo("***")
    CALL echo(build("tbl=",tgtsch->tbl[das_tbl_seq].tbl_name))
    CALL echo(build("col=",tgtsch->tbl[das_tbl_seq].tbl_col[das_col_seq].col_name))
    CALL echo("***")
   ENDIF
   IF (dm2_table_column_exists("",das_tbl_name2,das_col_name2,0,1,
    1,das_col_fnd_ind2,das_coldef_fnd_ind2,das_data_type2)=0)
    RETURN
   ENDIF
   SET das_a_sum = 0.0
   IF (das_coldef_fnd_ind2=0)
    IF (das_verify_function("DM2_GET_NBR_NULL_VALUES")=0)
     RETURN
    ENDIF
    SET dm_err->eproc = concat("Get number of null values for table/column ",das_tbl_name2,"/",
     das_col_name2,".")
    IF ((dm_err->debug_flag > 0))
     CALL echo(dm_err->eproc)
    ENDIF
    SELECT INTO "nl:"
     x = sqlpassthru(concat("dm2_get_nbr_null_values('",das_tbl_name2,"','",das_col_name2,"')"),0)
     FROM dual
     DETAIL
      das_a_sum = x
      IF ((dm_err->debug_flag > 0))
       CALL echo(concat(das_tbl_name2,".",das_col_name2,":"," das_a_sum = ",
        build(das_a_sum)))
      ENDIF
     WITH nocounter
    ;end select
    ...
    ENDIF
   ELSE
    EXECUTE dm2_apply_sizing_parser 2, "", das_tbl_name2,
    das_col_name2
    IF ((dm_err->err_ind=1))
     RETURN
    ENDIF
   ENDIF
   SET das_rat = (das_a_sum/ 10000)
   IF ((dm_err->debug_flag > 0))
    CALL echo("***")
    CALL echo(build("a_sum=",das_a_sum))
    CALL echo(build("ratio=",das_rat))
    CALL echo("***")
   ENDIF
   SET tgtsch->tbl[das_tbl_seq].size = (das_tbl_size+ (get_data_length(das_tbl_seq,das_col_seq) *
   das_rat))
   IF ((tgtsch->tbl[das_tbl_seq].size=0.0))
    SET tgtsch->tbl[das_tbl_seq].size = 1
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo("***")
    CALL echo("null to not null")
    CALL echo(build("tbl=",tgtsch->tbl[das_tbl_seq].tbl_name))
    CALL echo(tgtsch->tbl[das_tbl_seq].tbl_col[das_col_seq].col_name)
    CALL echo(build("tbl_size=",tgtsch->tbl[das_tbl_seq].size))
    CALL echo("***")
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
 END ;Subroutine
 SUBROUTINE sub_calc_tot_size_not_null(tsnn_tblcnt,tsnn_curidx)
   SET dm_err->eproc = "execute sub_calc_tot_size_not_null"
   IF ((dm_err->debug_flag > 0))
    CALL echo(dm_err->eproc)
   ENDIF
   SET tgtsch->tbl[tsnn_tblcnt].size = ((tgtsch->tbl[tsnn_tblcnt].size+ das_tbl_header) * tgtsch->
   tbl[tsnn_tblcnt].row_cnt)
   IF ((tgtsch->tbl[tsnn_tblcnt].dext_mgmt="D"))
    SET tgtsch->tbl[tsnn_tblcnt].size = cal_round_size(tgtsch->tbl[tsnn_tblcnt].size,cur_sch->tbl[
     tsnn_curidx].next_ext)
   ELSE
    SET tgtsch->tbl[tsnn_tblcnt].next_ext = cal_lmt_ext_size((tgtsch->tbl[tsnn_tblcnt].size+ tgtsch->
    ...
    IF ((dm_err->debug_flag > 0))
     ...
     CALL echo(build("cur_bytes_allocated=",tgtsch->tbl[tsnn_tblcnt].cur_bytes_allocated))
     CALL echo(build("table next_extent=",tgtsch->tbl[tsnn_tblcnt].next_ext))
     CALL echo("***")
    ENDIF
    SET tgtsch->tbl[tsnn_tblcnt].size = cal_round_size(tgtsch->tbl[tsnn_tblcnt].size,tgtsch->tbl[
     tsnn_tblcnt].next_ext)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo("***")
    CALL echo(build("Table:",tgtsch->tbl[tsnn_tblcnt].tbl_name))
    CALL echo(build("rounded_size=",tgtsch->tbl[tsnn_tblcnt].size))
    CALL echo("***")
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
 END ;Subroutine
 SUBROUTINE sub_calc_idx_null_tonot_null(intn_tblcnt,intn_colcnt,intn_idxcnt,intn_curidx)
   DECLARE intn_idx = i4 WITH protect, noconstant(0)
   DECLARE cur_ind_pos = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "execute sub_calc_idx_null_tonot_null"
   IF ((dm_err->debug_flag > 0))
    CALL echo("*******")
    CALL echo(dm_err->eproc)
   ENDIF
   SET intn_idx = locateval(intn_idx,1,tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].ind_col_cnt,tgtsch->
    tbl[intn_tblcnt].tbl_col[intn_colcnt].col_name,tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].ind_col[
    intn_idx].col_name)
   IF (intn_idx > 0)
    SET tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].size = (tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].
    size+ (((size(trim(tgtsch->tbl[intn_tblcnt].tbl_col[intn_colcnt].data_default))+ 1) * tgtsch->
    tbl[intn_tblcnt].row_cnt) * ind_factor))
    IF ((dm_err->debug_flag > 0))
     CALL echo("***")
     CALL echo(build("col_name in index:",tgtsch->tbl[intn_tblcnt].tbl_col[intn_colcnt].col_name))
     CALL echo(build("index_name:",tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].ind_name))
     CALL echo(build("index size=",tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].size))
     CALL echo("***")
    ENDIF
    SET cur_ind_pos = tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].cur_idx
    IF ((cur_sch->tbl[intn_curidx].ind[cur_ind_pos].ext_mgmt="D"))
     IF ((dm_err->debug_flag > 1))
      CALL echo(tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].ind_name)
      CALL echo(tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].size)
      CALL echo(cur_sch->tbl[intn_curidx].ind[cur_ind_pos].next_ext)
      CALL echo(cur_ind_pos)
     ENDIF
     SET tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].size = cal_round_size(tgtsch->tbl[intn_tblcnt].
      ind[intn_idxcnt].size,cur_sch->tbl[intn_curidx].ind[cur_ind_pos].next_ext)
    ELSE
     SET das_tot_size = 0.0
     SET das_tot_size = (tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].size+ tgtsch->tbl[intn_tblcnt].
     ind[intn_idxcnt].cur_bytes_allocated)
     SET tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].next_ext = cal_lmt_ext_size(das_tot_size)
     IF ((dm_err->debug_flag > 0))
      CALL echo("***")
      CALL echo(build("col_name in index:",tgtsch->tbl[intn_tblcnt].tbl_col[intn_colcnt].col_name))
      CALL echo(build("index_name:",tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].ind_name))
      CALL echo(build("index next extent=",tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].next_ext))
      CALL echo(build("index size=",tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].size))
      CALL echo(build("cur_bytes_allocated=",tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].
        cur_bytes_allocated))
      CALL echo("***")
     ENDIF
     SET tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].size = cal_round_size(tgtsch->tbl[intn_tblcnt].
      ind[intn_idxcnt].size,tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].next_ext)
     IF ((dm_err->debug_flag > 0))
      CALL echo("***")
      CALL echo(build("col_name in index:",tgtsch->tbl[intn_tblcnt].tbl_col[intn_colcnt].col_name))
      CALL echo(build("index_name:",tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].ind_name))
      CALL echo(build("rounded index size=",tgtsch->tbl[intn_tblcnt].ind[intn_idxcnt].size))
      CALL echo("***")
     ENDIF
    ENDIF
   ENDIF
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE sub_add_lob_size(als_tbl_cnt)
  IF ((dm_err->debug_flag > 0))
   CALL echo(concat(dm_err->eproc,"(",tgtsch->tbl[als_tbl_cnt].tbl_name,")"))
  ENDIF
  SET tgtsch->tbl[als_tbl_cnt].long_size = (tgtsch->tbl[als_tbl_cnt].long_size+ m_long_val)
 END ;Subroutine
 SUBROUTINE sub_new_idx_exist_tbl(niet_tblcnt,niet_colcnt,niet_idxcnt)
   SET dm_err->eproc = "Executing sub_new_idx_exist_tbl"
   DECLARE niet_tbl_ndx = i2 WITH protect, noconstant(0)
   DECLARE niet_col_ndx = i2 WITH protect, noconstant(0)
   IF ((das_columns->tbl_cnt > 0))
    IF ((dm_err->debug_flag > 0))
     CALL echo("Searching for columns added during this install and treat them as new columns.")
    ENDIF
    SET niet_tbl_ndx = 0
    SET niet_col_ndx = 0
    SET niet_tbl_ndx = locateval(niet_tbl_ndx,1,das_columns->tbl_cnt,tgtsch->tbl[niet_tblcnt].
     tbl_name,das_columns->tbl[niet_tbl_ndx].tbl_name)
    IF (niet_tbl_ndx > 0)
     SET niet_col_ndx = locateval(niet_col_ndx,1,das_columns->tbl[niet_tbl_ndx].col_cnt,tgtsch->tbl[
      niet_tblcnt].tbl_col[niet_colcnt].col_name,das_columns->tbl[niet_tbl_ndx].col[niet_col_ndx].
      col_name)
    ENDIF
    IF (niet_col_ndx > 0
     AND niet_tbl_ndx > 0)
     IF ((dm_err->debug_flag > 0))
      CALL echo("***")
      CALL echo("Revert to new column sizing since column added during previous execution.")
      CALL echo(build("tbl=",tgtsch->tbl[niet_tblcnt].tbl_name))
      CALL echo(build("col=",tgtsch->tbl[niet_tblcnt].tbl_col[niet_colcnt].col_name))
      CALL echo("***")
     ENDIF
    ENDIF
   ENDIF
   IF ((tgtsch->tbl[niet_tblcnt].tbl_col[niet_colcnt].new_ind=0)
    AND (tgtsch->tbl[niet_tblcnt].row_cnt >= 10000)
    AND  NOT ((tgtsch->tbl[niet_tblcnt].tbl_col[niet_colcnt].data_type IN ("CHAR", "DATE",
   "TIMESTAMP*", "ROWID")))
    AND niet_col_ndx=0)
    CALL sub_big_nonew_notfixed(niet_tblcnt,niet_idxcnt,niet_colcnt)
    IF ((dm_err->err_ind=1))
     GO TO exit_script
    ENDIF
   ELSEIF ((tgtsch->tbl[niet_tblcnt].tbl_col[niet_colcnt].data_type IN ("CHAR", "DATE", "TIMESTAMP*",
   "ROWID")))
    SET tgtsch->tbl[niet_tblcnt].ind[niet_idxcnt].size = (tgtsch->tbl[niet_tblcnt].ind[niet_idxcnt].
    size+ get_data_length(niet_tblcnt,niet_colcnt))
    IF ((dm_err->debug_flag > 0))
     CALL echo(build(tgtsch->tbl[niet_tblcnt].tbl_name,"/",tgtsch->tbl[niet_tblcnt].tbl_col[
       niet_colcnt].col_name,"=",tgtsch->tbl[niet_tblcnt].ind[niet_idxcnt].size))
    ENDIF
    IF ((dm_err->debug_flag > 1))
     CALL echo(build("case2=",tgtsch->tbl[niet_tblcnt].ind[niet_idxcnt].size))
    ENDIF
   ELSEIF ((((((tgtsch->tbl[niet_tblcnt].tbl_col[niet_colcnt].new_ind=1)) OR (niet_col_ndx > 0))
    AND (tgtsch->tbl[niet_tblcnt].tbl_col[niet_colcnt].data_default_ni=0)) OR ((tgtsch->tbl[
   niet_tblcnt].tbl_col[niet_colcnt].new_ind=0)
    AND niet_col_ndx=0
    AND (tgtsch->tbl[niet_tblcnt].tbl_col[niet_colcnt].data_default_ni=0)
    AND (tgtsch->tbl[niet_tblcnt].row_cnt < 10000))) )
    SET tgtsch->tbl[niet_tblcnt].ind[niet_idxcnt].size = (tgtsch->tbl[niet_tblcnt].ind[niet_idxcnt].
    size+ get_data_length(niet_tblcnt,niet_colcnt))
    IF ((dm_err->debug_flag > 1))
     CALL echo(build("case3=",tgtsch->tbl[niet_tblcnt].ind[niet_idxcnt].size))
    ENDIF
   ELSEIF ((((((tgtsch->tbl[niet_tblcnt].tbl_col[niet_colcnt].new_ind=1)) OR (niet_col_ndx > 0))
    AND (tgtsch->tbl[niet_tblcnt].tbl_col[niet_colcnt].data_default_ni=1)) OR ( NOT ((tgtsch->tbl[
   niet_tblcnt].tbl_col[niet_colcnt].data_type IN ("CHAR", "DATE", "TIMESTAMP*", "ROWID")))
    AND (tgtsch->tbl[niet_tblcnt].row_cnt < 10000)
    AND (tgtsch->tbl[niet_tblcnt].tbl_col[niet_colcnt].data_default_ni=1))) )
    SET tgtsch->tbl[niet_tblcnt].ind[niet_idxcnt].size = (tgtsch->tbl[niet_tblcnt].ind[niet_idxcnt].
    size+ 1)
    IF ((dm_err->debug_flag > 1))
     CALL echo(build("case4=",tgtsch->tbl[niet_tblcnt].ind[niet_idxcnt].size))
    ENDIF
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
 END ;Subroutine
 SUBROUTINE sub_clinical_ext_size(ces_type,ces_seq,ces_ind_seq)
   SET dm_err->eproc = "assign extent size for clinical tables"
   DECLARE cies_cur_icnt = i4 WITH protect, noconstant(0)
   DECLARE cies_idx = i4 WITH protect, noconstant(0)
   DECLARE cies_tsp_idx = i4 WITH protect, noconstant(0)
   DECLARE cies_max_init_ext = f8 WITH protect, noconstant(0.0)
   DECLARE cies_max_next_ext = f8 WITH protect, noconstant(0.0)
   SET cies_idx = locateval(cies_idx,1,das_dtp->dtp_cnt,tgtsch->tbl[ces_seq].tbl_name,das_dtp->qual[
    cies_idx].tname)
   IF (cies_idx=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Can not find table ",tgtsch->tbl[ces_seq].tbl_name,
     " in table DM_TS_PRECEDENCE")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN
   ENDIF
   IF (ces_type="T")
    IF ((dm_err->debug_flag > 1))
     CALL echo("****************************************")
     CALL echo(concat("table::",tgtsch->tbl[ces_seq].tbl_name))
     CALL echo(concat("d assignment choice::",tgtsch->tbl[ces_seq].ttspace_assignment_choice))
     CALL echo(concat("i assignment choice::",tgtsch->tbl[ces_seq].itspace_assignment_choice))
     CALL echo("****************************************")
    ENDIF
    IF ((tgtsch->tbl[ces_seq].ttspace_assignment_choice="CUSTOM TABLESPACE MAPPING"))
     SET cies_tsp_idx = 1
    ELSE
     SET cies_tsp_idx = locateval(cies_tsp_idx,1,das_dtp->qual[cies_idx].prec_cnt,tgtsch->tbl[ces_seq
      ].tspace_name,das_dtp->qual[cies_idx].prec[cies_tsp_idx].data_tspace)
    ENDIF
    IF (cies_tsp_idx=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Can not find tablespace ",tgtsch->tbl[ces_seq].tspace_name,
      " in table DM_TS_PRECEDENCE")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN
    ENDIF
    SET tgtsch->tbl[ces_seq].init_ext = das_dtp->qual[cies_idx].prec[cies_tsp_idx].data_extent_size
    SET tgtsch->tbl[ces_seq].next_ext = das_dtp->qual[cies_idx].prec[cies_tsp_idx].data_extent_size
   ELSEIF (ces_type="I")
    IF ((tgtsch->tbl[ces_seq].cur_idx > 0))
     IF ((cur_sch->tbl[tgtsch->tbl[ces_seq].cur_idx].ind_cnt > 0))
      FOR (cies_cur_icnt = 1 TO cur_sch->tbl[tgtsch->tbl[ces_seq].cur_idx].ind_cnt)
       IF ((cur_sch->tbl[tgtsch->tbl[ces_seq].cur_idx].ind[cies_cur_icnt].init_ext >
       cies_max_init_ext))
        SET cies_max_init_ext = cur_sch->tbl[tgtsch->tbl[ces_seq].cur_idx].ind[cies_cur_icnt].
        init_ext
       ENDIF
       IF ((cur_sch->tbl[tgtsch->tbl[ces_seq].cur_idx].ind[cies_cur_icnt].next_ext >
       cies_max_next_ext))
        SET cies_max_next_ext = cur_sch->tbl[tgtsch->tbl[ces_seq].cur_idx].ind[cies_cur_icnt].
        next_ext
       ENDIF
      ENDFOR
     ENDIF
    ENDIF
    IF ((tgtsch->tbl[ces_seq].itspace_assignment_choice="CUSTOM TABLESPACE MAPPING"))
     SET cies_tsp_idx = 1
    ELSE
     SET cies_tsp_idx = locateval(cies_tsp_idx,1,das_dtp->qual[cies_idx].prec_cnt,tgtsch->tbl[ces_seq
      ].ind_tspace,das_dtp->qual[cies_idx].prec[cies_tsp_idx].ind_tspace)
    ENDIF
    IF (cies_tsp_idx=0)
     CALL echo(concat("**tgtsch->tbl[ces_seq]->ind_tspace: *",tgtsch->tbl[ces_seq].ind_tspace))
     SET dm_err->eproc = concat("Tablespace ",tgtsch->tbl[ces_seq].ind_tspace,
      " not found in DM_TS_PRECEDENCE for table ",das_dtp->qual[cies_idx].tname,
      " The default extent size will be assigned")
     CALL disp_msg(" ",dm_err->logfile,0)
     SET tgtsch->tbl[ces_seq].ind[ces_ind_seq].init_ext = dmt_min_ext_size
     SET tgtsch->tbl[ces_seq].ind[ces_ind_seq].next_ext = dmt_min_ext_size
    ELSE
     SET tgtsch->tbl[ces_seq].ind[ces_ind_seq].init_ext = das_dtp->qual[cies_idx].prec[cies_tsp_idx].
     index_extent_size
     SET tgtsch->tbl[ces_seq].ind[ces_ind_seq].next_ext = das_dtp->qual[cies_idx].prec[cies_tsp_idx].
     index_extent_size
    ENDIF
    IF ((tgtsch->tbl[ces_seq].ind[ces_ind_seq].init_ext < cies_max_init_ext))
     SET tgtsch->tbl[ces_seq].ind[ces_ind_seq].init_ext = cies_max_init_ext
    ENDIF
    IF ((tgtsch->tbl[ces_seq].ind[ces_ind_seq].next_ext < cies_max_next_ext))
     SET tgtsch->tbl[ces_seq].ind[ces_ind_seq].next_ext = cies_max_next_ext
    ENDIF
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
 END ;Subroutine
 SUBROUTINE sub_assign_ext_size(null)
   SET dm_err->eproc = "Assign extent size for tables in target schema"
   CALL disp_msg(" ",dm_err->logfile,0)
   DECLARE saes_t_curidx = i4 WITH protect, noconstant(0)
   IF ((tgtsch->diff_ind=1))
    FOR (scces_cnt = 1 TO tgtsch->tbl_cnt)
      IF ((((tgtsch->tbl[scces_cnt].new_ind=1)) OR ((tgtsch->tbl[scces_cnt].diff_ind=1))) )
       IF ((tgtsch->tbl[scces_cnt].dext_mgmt="D"))
        IF ((tgtsch->tbl[scces_cnt].cur_idx > 0))
         SET saes_t_curidx = tgtsch->tbl[scces_cnt].cur_idx
         SET tgtsch->tbl[scces_cnt].init_ext = cur_sch->tbl[saes_t_curidx].init_ext
         SET tgtsch->tbl[scces_cnt].next_ext = cur_sch->tbl[saes_t_curidx].next_ext
        ELSE
         IF ((dm2_install_schema->dbase_type="ADMIN"))
          SET tgtsch->tbl[scces_cnt].init_ext = dmt_min_ext_size
          SET tgtsch->tbl[scces_cnt].next_ext = dmt_min_ext_size
         ELSEIF ((dm2_install_schema->dbase_type="CLINICAL"))
          CALL sub_clinical_ext_size("T",scces_cnt,0)
          IF ((dm_err->err_ind > 0))
           RETURN
          ENDIF
         ENDIF
        ENDIF
       ELSEIF ((tgtsch->tbl[scces_cnt].dext_mgmt="L"))
        IF ((tgtsch->tbl[scces_cnt].new_ind=1))
         SET tgtsch->tbl[scces_cnt].init_ext = 0
         SET tgtsch->tbl[scces_cnt].next_ext = lmt_min_ext_size
        ELSE
         SET tgtsch->tbl[scces_cnt].init_ext = 0
         SET tgtsch->tbl[scces_cnt].next_ext = cal_lmt_ext_size(tgtsch->tbl[scces_cnt].
          cur_bytes_allocated)
         IF ((dm_err->debug_flag > 1))
          CALL echo(build("table_name =",tgtsch->tbl[scces_cnt].tbl_name))
          CALL echo(build("cur_bytes_allocated = ",tgtsch->tbl[scces_cnt].cur_bytes_allocated))
          CALL echo(build("table next_ext = ",tgtsch->tbl[scces_cnt].next_ext))
         ENDIF
        ENDIF
       ENDIF
       IF ((tgtsch->tbl[scces_cnt].iext_mgmt="D"))
        FOR (saes_lp = 1 TO tgtsch->tbl[scces_cnt].ind_cnt)
          SET saes_t_curidx = 0
          SET saes_i_curidx = 0
          SET saes_t_curidx = tgtsch->tbl[scces_cnt].cur_idx
          IF ((tgtsch->tbl[scces_cnt].ind[saes_lp].cur_idx > 0))
           SET saes_i_curidx = tgtsch->tbl[scces_cnt].ind[saes_lp].cur_idx
           SET tgtsch->tbl[scces_cnt].ind[saes_lp].init_ext = cur_sch->tbl[saes_t_curidx].ind[
           saes_i_curidx].init_ext
           SET tgtsch->tbl[scces_cnt].ind[saes_lp].next_ext = cur_sch->tbl[saes_t_curidx].ind[
           saes_i_curidx].next_ext
          ELSE
           IF ((dm2_install_schema->dbase_type="ADMIN"))
            SET tgtsch->tbl[scces_cnt].ind[saes_lp].init_ext = dmt_min_ext_size
            SET tgtsch->tbl[scces_cnt].ind[saes_lp].next_ext = dmt_min_ext_size
           ELSEIF ((dm2_install_schema->dbase_type="CLINICAL"))
            CALL sub_clinical_ext_size("I",scces_cnt,saes_lp)
            IF ((dm_err->err_ind > 0))
             RETURN
            ENDIF
           ENDIF
          ENDIF
        ENDFOR
       ELSEIF ((tgtsch->tbl[scces_cnt].iext_mgmt="L"))
        FOR (saesi_lp = 1 TO tgtsch->tbl[scces_cnt].ind_cnt)
          IF ((tgtsch->tbl[scces_cnt].ind[saesi_lp].new_ind=1))
           SET tgtsch->tbl[scces_cnt].ind[saesi_lp].init_ext = 0
           SET tgtsch->tbl[scces_cnt].ind[saesi_lp].next_ext = lmt_min_ext_size
          ELSE
           SET tgtsch->tbl[scces_cnt].ind[saesi_lp].init_ext = 0
           SET tgtsch->tbl[scces_cnt].ind[saesi_lp].next_ext = cal_lmt_ext_size(tgtsch->tbl[scces_cnt
            ].ind[saesi_lp].cur_bytes_allocated)
           IF ((dm_err->debug_flag > 1))
            CALL echo(build("table_name =",tgtsch->tbl[scces_cnt].tbl_name))
            CALL echo(build("index_name =",tgtsch->tbl[scces_cnt].ind[saesi_lp].ind_name))
            CALL echo(build("cur_bytes_allocated = ",tgtsch->tbl[scces_cnt].ind[saesi_lp].
              cur_bytes_allocated))
            CALL echo(build("index next_ext = ",tgtsch->tbl[scces_cnt].ind[saesi_lp].next_ext))
           ENDIF
          ENDIF
        ENDFOR
       ENDIF
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE das_get_col_size(dgc_rec,dgc_tidx,dgc_cidx,dgc_lrg_dload)
   DECLARE dgc_data_type = vc WITH protect, noconstant(" ")
   DECLARE dgc_column_name = vc WITH protect, noconstant(" ")
   DECLARE dgc_data_length = i4 WITH protect, noconstant(0)
   IF (dgc_rec="T")
    SET dgc_column_name = tgtsch->tbl[dgc_tidx].tbl_col[dgc_cidx].col_name
    SET dgc_data_type = tgtsch->tbl[dgc_tidx].tbl_col[dgc_cidx].data_type
    SET dgc_data_length = tgtsch->tbl[dgc_tidx].tbl_col[dgc_cidx].data_length
   ELSEIF (dgc_rec="C")
    SET dgc_column_name = cur_sch->tbl[dgc_tidx].tbl_col[dgc_cidx].col_name
    SET dgc_data_type = cur_sch->tbl[dgc_tidx].tbl_col[dgc_cidx].data_type
    SET dgc_data_length = cur_sch->tbl[dgc_tidx].tbl_col[dgc_cidx].data_length
   ENDIF
   SET dm_err->eproc = concat("Finding column length for column ",dgc_column_name)
   IF ((dm_err->debug_flag > 2))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   CASE (dgc_data_type)
    OF "CHAR":
    OF "RAW":
     RETURN(dgc_data_length)
    OF "VARCHAR":
    OF "VARCHAR2":
     IF (dgc_data_length > 50)
      RETURN(50)
     ELSE
      RETURN(dgc_data_length)
     ENDIF
    OF "DATE":
     RETURN(8)
    OF patstring("TIMESTAMP*",0):
     RETURN(das_timestamp_len)
    OF "FLOAT":
     RETURN(10)
    OF "NUMBER":
     CASE (dgc_column_name)
      OF patstring("*_FLAG",0):
       RETURN(2)
      OF patstring("*_IND",0):
       RETURN(2)
      OF patstring("*_COMPL",0):
       RETURN(10)
      OF patstring("*_VALUE",0):
       RETURN(10)
      OF patstring("*_CD",0):
       RETURN(10)
      OF patstring("*_ID",0):
       RETURN(10)
      OF patstring("*APPLCTX",0):
       RETURN(10)
      ELSE
       RETURN(6)
     ENDCASE
   ...
   ENDCASE
 END ;Subroutine
 SUBROUTINE das_get_col_size2(dgc2_rec,dgc2_tidx,dgc2_cidx,dgc2_lrg_dload)
   DECLARE dgc2_data_type = vc WITH protect, noconstant(" ")
   DECLARE dgc2_column_name = vc WITH protect, noconstant(" ")
   DECLARE dgc2_data_length = i4 WITH protect, noconstant(0)
   IF (dgc2_rec="T")
    SET dgc2_column_name = tgtsch->tbl[dgc2_tidx].tbl_col[dgc2_cidx].col_name
    SET dgc2_data_type = tgtsch->tbl[dgc2_tidx].tbl_col[dgc2_cidx].data_type
    SET dgc2_data_length = tgtsch->tbl[dgc2_tidx].tbl_col[dgc2_cidx].data_length
   ELSEIF (dgc2_rec="C")
    SET dgc2_column_name = cur_sch->tbl[dgc2_tidx].tbl_col[dgc2_cidx].col_name
    SET dgc2_data_type = cur_sch->tbl[dgc2_tidx].tbl_col[dgc2_cidx].data_type
    SET dgc2_data_length = cur_sch->tbl[dgc2_tidx].tbl_col[dgc2_cidx].data_length
   ENDIF
   SET dm_err->eproc = concat("Finding column length for column ",dgc2_column_name)
   IF ((dm_err->debug_flag > 2))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   CASE (dgc2_data_type)
    OF "CHAR":
    OF "RAW":
     RETURN((dgc2_data_length+ 1))
    OF "VARCHAR":
    OF "VARCHAR2":
     IF (dgc2_data_length > 127)
      RETURN((dgc2_data_length+ 2))
     ELSE
      RETURN((dgc2_data_length+ 1))
     ENDIF
    OF "DATE":
     RETURN((8+ 1))
    OF patstring("TIMESTAMP*",0):
     RETURN((das_timestamp_len+ 1))
    OF "FLOAT":
     RETURN((10+ 1))
    OF "NUMBER":
     CASE (dgc2_column_name)
      OF patstring("*_FLAG",0):
       RETURN((2+ 1))
      OF patstring("*_IND",0):
       RETURN((2+ 1))
   ...
   ENDCASE
 END ;Subroutine
 SUBROUTINE das_load_obj_space_needs(null)
   SET dm_err->eproc = "Load object space needs to DM2_TSPACE_OBJ_SIZE table."
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE dlosn_install_type = vc WITH protect, noconstant(" ")
   DECLARE dlosn_install_value = vc WITH protect, noconstant(" ")
   DECLARE dlosn_gen_id = f8 WITH protect, noconstant(0.0)
   IF ((dm_err->debug_flag > 2))
    CALL echo(build("dm2_install_schema->process_option =",dm2_install_schema->process_option))
   ENDIF
   IF ((dm2_install_schema->process_option IN ("CLIN UPGRADE", "CLIN UPGRADE NR", "PREVIEW",
   "INHOUSE")))
    IF ((dm2_install_schema->schema_prefix="dm2o"))
     SET dlosn_install_type = "PACKAGE"
     SET dlosn_install_value = dm2_install_schema->file_prefix
    ELSE
     SET dlosn_install_type = "SCHEMA_DATE"
     SET dlosn_install_value = format(cnvtdate2(dm2_install_schema->file_prefix,"MMDDYYYY"),
      "DD-MMM-YYYY;;D")
    ENDIF
    SET dm_err->eproc = "Determine gen_id needed to insert into dm2_tspace_obj_size table."
    SELECT INTO "nl:"
     FROM dm2_tspace_obj_size s
     WHERE s.install_type=dlosn_install_type
      AND s.install_type_value=cnvtupper(dlosn_install_value)
     ORDER BY s.gen_id
     FOOT REPORT
      dlosn_gen_id = (s.gen_id+ 1)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF ((tgtsch->diff_ind > 0))
     FOR (das_t = 1 TO tgtsch->tbl_cnt)
       IF ((tgtsch->tbl[das_t].size > 0))
        IF (das_ins_tspace_obj_size("SCHEMA","TABLE",das_t,0,0)=0)
         RETURN(0)
        ENDIF
       ENDIF
       FOR (das_i = 1 TO tgtsch->tbl[das_t].ind_cnt)
         IF ((tgtsch->tbl[das_t].ind[das_i].size > 0))
          IF (das_ins_tspace_obj_size("SCHEMA","INDEX",das_t,das_i,0)=0)
           RETURN(0)
          ENDIF
         ENDIF
       ENDFOR
       IF ((tgtsch->tbl[das_t].long_size > 0))
        FOR (das_c = 1 TO tgtsch->tbl[das_t].tbl_col_cnt)
          IF ((tgtsch->tbl[das_t].tbl_col[das_c].data_type IN ("CLOB", "BLOB")))
           IF (das_ins_tspace_obj_size("SCHEMA","LOB",das_t,0,das_c)=0)
            RETURN(0)
           ENDIF
          ENDIF
        ENDFOR
    ...
    IF ((dm2_rr_spc_needs->space_needed > 0))
     FOR (das_t = 1 TO dm2_rr_spc_needs->tbl_cnt)
      IF ((dm2_rr_spc_needs->tbl[das_t].space_needed > 0))
       IF (das_ins_tspace_obj_size("README","TABLE",das_t,0,0)=0)
        RETURN(0)
       ENDIF
      ENDIF
      FOR (das_i = 1 TO dm2_rr_spc_needs->tbl[das_t].ind_cnt)
        IF ((dm2_rr_spc_needs->tbl[das_t].ind[das_i].space_needed > 0))
         IF (das_ins_tspace_obj_size("README","INDEX",das_t,das_i,0)=0)
          RETURN(0)
         ENDIF
        ENDIF
      ENDFOR
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE das_ins_tspace_obj_size(dit_rtype,dit_otype,dit_tidx,dit_iidx,dit_cidx)
   DECLARE dit_tbl_name = vc WITH protect, noconstant(" ")
   DECLARE dit_tspace_name = vc WITH protect, noconstant(" ")
   DECLARE dit_obj_name = vc WITH protect, noconstant(" ")
   DECLARE dit_bytes_needed = f8 WITH protect, noconstant(0.0)
   IF (dit_rtype="SCHEMA")
    SET dit_tbl_name = tgtsch->tbl[dit_tidx].tbl_name
    CASE (dit_otype)
     OF "TABLE":
      SET dit_obj_name = tgtsch->tbl[dit_tidx].tbl_name
      SET dit_tspace_name = tgtsch->tbl[dit_tidx].tspace_name
      SET dit_bytes_needed = tgtsch->tbl[dit_tidx].size
     OF "INDEX":
      SET dit_obj_name = tgtsch->tbl[dit_tidx].ind[dit_iidx].ind_name
      SET dit_tspace_name = tgtsch->tbl[dit_tidx].ind[dit_iidx].tspace_name
      SET dit_bytes_needed = tgtsch->tbl[dit_tidx].ind[dit_iidx].size
     OF "LOB":
      SET dit_obj_name = tgtsch->tbl[dit_tidx].tbl_col[dit_cidx].col_name
      SET dit_tspace_name = tgtsch->tbl[dit_tidx].long_tspace
      SET dit_bytes_needed = tgtsch->tbl[dit_tidx].long_size
     ELSE
      SET dm_err->err_ind = 1
      SET dm_err->emsg = concat("Request type ",rtype," is not valid.")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
    ENDCASE
   ELSEIF (dit_rtype="README")
    SET dit_tbl_name = dm2_rr_spc_needs->tbl[dit_tidx].table_name
    CASE (dit_otype)
     OF "TABLE":
      SET dit_obj_name = dm2_rr_spc_needs->tbl[dit_tidx].table_name
      SET dit_tspace_name = cur_sch->tbl[dm2_rr_spc_needs->tbl[dit_tidx].cur_idx].tspace_name
      SET dit_bytes_needed = dm2_rr_spc_needs->tbl[dit_tidx].space_needed
     OF "INDEX":
      SET dit_obj_name = dm2_rr_spc_needs->tbl[dit_tidx].ind[dit_iidx].ind_name
      SET dit_tspace_name = cur_sch->tbl[dm2_rr_spc_needs->tbl[dit_tidx].cur_idx].ind[
      dm2_rr_spc_needs->tbl[dit_tidx].ind[dit_iidx].cur_idx].tspace_name
      SET dit_bytes_needed = dm2_rr_spc_needs->tbl[dit_tidx].ind[dit_iidx].space_needed
     ELSE
      SET dm_err->err_ind = 1
      SET dm_err->emsg = concat("Request type ",dit_rtype," is not valid.")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
    ENDCASE
   ENDIF
   INSERT  FROM dm2_tspace_obj_size s
    SET s.install_type = dlosn_install_type, s.install_type_value = dlosn_install_value, s.gen_id =
     dlosn_gen_id,
     s.request_type = dit_rtype, s.table_name = dit_tbl_name, s.obj_type = dit_otype,
     s.obj_name = dit_obj_name, s.tspace_name = dit_tspace_name, s.bytes_needed = dit_bytes_needed,
     s.install_status =
     IF ((dm2_install_schema->process_option="INHOUSE")) "SUCCESS"
     ELSE null
     ENDIF
     , s.updt_dt_time = cnvtdatetime(curdate,curtime3)
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
   ...
   ELSE
    COMMIT
   ENDIF
   SET dtr_tspace_misc->gen_id = dlosn_gen_id
   RETURN(1)
 END ;Subroutine
 SUBROUTINE das_cal_rr_ins_size(dcr_ttgtidx,dcr_tcuridx,dcr_t)
   SET dm_err->eproc = "Calculate readme tspace needs for inserts."
   IF ((dm_err->debug_flag > 1))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   DECLARE dcr_col_size = f8 WITH protect, noconstant(0.0)
   DECLARE dcr_row_size = f8 WITH protect, noconstant(0.0)
   DECLARE dcr_updt_col_size = f8 WITH protect, noconstant(0.0)
   DECLARE dcr_updt_ind_size = f8 WITH protect, noconstant(0.0)
   DECLARE dcr_ind_size = f8 WITH protect, noconstant(0.0)
   DECLARE dcr_ind_col_size = f8 WITH protect, noconstant(0.0)
   DECLARE dcr_cidx = i4 WITH protect, noconstant(0)
   DECLARE dcr_c = i4 WITH protect, noconstant(0)
   DECLARE dcr_icuridx = i4 WITH protect, noconstant(0)
   DECLARE dcr_ic = i4 WITH protect, noconstant(0)
   DECLARE dcr_ccuridx = i4 WITH protect, noconstant(0)
   IF ((dm_err->debug_flag > 2))
    CALL echo(build("init_updt_col_size =",dcr_updt_col_size))
   ENDIF
   IF (dcr_ttgtidx > 0)
    FOR (dcr_c = 1 TO tgtsch->tbl[dcr_ttgtidx].tbl_col_cnt)
      SET dcr_col_size = das_get_col_size("T",dcr_ttgtidx,dcr_c,dm2_rr_spc_needs->tbl[dcr_t].
       large_data_loaded)
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("col_size =",dcr_col_size))
      ENDIF
      SET dcr_row_size = (dcr_row_size+ dcr_col_size)
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("row_size =",dcr_row_size))
      ENDIF
      IF ((dm2_rr_spc_needs->tbl[dcr_t].col_updt_cnt > 0))
       SET dcr_cidx = 0
       SET dcr_cidx = locateval(dcr_cidx,1,dm2_rr_spc_needs->tbl[dcr_t].col_updt_cnt,tgtsch->tbl[
        dcr_ttgtidx].tbl_col[dcr_c].col_name,dm2_rr_spc_needs->tbl[dcr_t].col_updt[dcr_cidx].
        column_name)
       IF (dcr_cidx > 0)
        SET dcr_updt_col_size = (dcr_updt_col_size+ (dcr_col_size * dm2_rr_spc_needs->tbl[dcr_t].
        col_updt[dcr_cidx].update_row_cnt))
        IF ((dm_err->debug_flag > 2))
         CALL echo(build("updt_row_cnt =",dm2_rr_spc_needs->tbl[dcr_t].col_updt[dcr_cidx].
           update_row_cnt))
         CALL echo(build("updt_col_size =",dcr_updt_col_size))
        ENDIF
       ENDIF
      ENDIF
    ENDFOR
   ELSEIF (dcr_tcuridx > 0)
    FOR (dcr_c = 1 TO cur_sch->tbl[dcr_tcuridx].tbl_col_cnt)
      SET dcr_col_size = das_get_col_size("C",dcr_tcuridx,dcr_c,dm2_rr_spc_needs->tbl[dcr_t].
       large_data_loaded)
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("col_size =",dcr_col_size))
      ENDIF
      ...
      IF ((dm2_rr_spc_needs->tbl[dcr_t].col_updt_cnt > 0))
       SET dcr_cidx = 0
       SET dcr_cidx = locateval(dcr_cidx,1,dm2_rr_spc_needs->tbl[dcr_t].col_updt_cnt,cur_sch->tbl[
        dcr_tcuridx].tbl_col[dcr_c].col_name,dm2_rr_spc_needs->tbl[dcr_t].col_updt[dcr_cidx].
        column_name)
       IF (dcr_cidx > 0)
        SET dcr_updt_col_size = (dcr_updt_col_size+ (dcr_col_size * dm2_rr_spc_needs->tbl[dcr_t].
        col_updt[dcr_cidx].update_row_cnt))
    ...
    ENDFOR
   ENDIF
   SET dm2_rr_spc_needs->tbl[dcr_t].space_needed = ((dcr_row_size * dm2_rr_spc_needs->tbl[dcr_t].
   insert_row_cnt)+ dcr_updt_col_size)
   IF ((dm_err->debug_flag > 2))
    CALL echo(build("insert_row_cnt =",dm2_rr_spc_needs->tbl[dcr_t].insert_row_cnt))
    CALL echo(build("tbl_spc_need =",dm2_rr_spc_needs->tbl[dcr_t].space_needed))
   ENDIF
   SET stat = alterlist(dm2_rr_spc_needs->tbl[dcr_t].ind,cur_sch->tbl[dcr_tcuridx].ind_cnt)
   SET dm2_rr_spc_needs->tbl[dcr_t].ind_cnt = cur_sch->tbl[dcr_tcuridx].ind_cnt
   FOR (dcr_i = 1 TO cur_sch->tbl[dcr_tcuridx].ind_cnt)
     SET dm2_rr_spc_needs->tbl[dcr_t].ind[dcr_i].cur_idx = dcr_i
     SET dm2_rr_spc_needs->tbl[dcr_t].ind[dcr_i].ind_name = cur_sch->tbl[dcr_tcuridx].ind[dcr_i].
     ind_name
     SET dcr_ind_size = 0.0
     SET dcr_updt_ind_size = 0.0
     SET dcr_icuridx = dcr_i
     FOR (dcr_ic = 1 TO cur_sch->tbl[dcr_tcuridx].ind[dcr_icuridx].ind_col_cnt)
       SET dcr_ind_col_size = 0.0
       SET dcr_ccuridx = locateval(dcr_ccuridx,1,cur_sch->tbl[dcr_tcuridx].tbl_col_cnt,cur_sch->tbl[
        dcr_tcuridx].ind[dcr_icuridx].ind_col[dcr_ic].col_name,cur_sch->tbl[dcr_tcuridx].tbl_col[
        dcr_ccuridx].col_name)
       IF (dcr_ccuridx > 0)
        SET dcr_ind_col_size = das_get_col_size("C",dcr_tcuridx,dcr_ccuridx,dm2_rr_spc_needs->tbl[
         dcr_t].large_data_loaded)
        SET dcr_ind_size = (dcr_ind_size+ dcr_ind_col_size)
        IF ((dm_err->debug_flag > 2))
         CALL echo(build("dcr_ind_col_size =",dcr_ind_col_size))
         CALL echo(build("dcr_ind_size =",dcr_ind_size))
        ENDIF
        IF ((dm2_rr_spc_needs->tbl[dcr_t].col_updt_cnt > 0))
         SET dcr_cidx = 0
         SET dcr_cidx = locateval(dcr_cidx,1,dm2_rr_spc_needs->tbl[dcr_t].col_updt_cnt,cur_sch->tbl[
          dcr_tcuridx].ind[dcr_icuridx].ind_col[dcr_ic].col_name,dm2_rr_spc_needs->tbl[dcr_t].
          col_updt[dcr_cidx].column_name)
         IF (dcr_cidx > 0)
          SET dcr_updt_ind_size = (dcr_updt_ind_size+ (dcr_ind_col_size * dm2_rr_spc_needs->tbl[dcr_t
          ].col_updt[dcr_cidx].update_row_cnt))
          IF ((dm_err->debug_flag > 2))
           CALL echo(build("updt_row_cnt =",dm2_rr_spc_needs->tbl[dcr_t].col_updt[dcr_cidx].
             update_row_cnt))
           CALL echo(build("dcr_updt_ind_size =",dcr_updt_ind_size))
          ENDIF
         ENDIF
        ENDIF
       ENDIF
     ENDFOR
     SET dm2_rr_spc_needs->tbl[dcr_t].ind[dcr_i].space_needed = ((dcr_ind_size * dm2_rr_spc_needs->
     tbl[dcr_t].insert_row_cnt)+ dcr_updt_ind_size)
     IF ((dm_err->debug_flag > 2))
      CALL echo(build("insert_row_cnt =",dm2_rr_spc_needs->tbl[dcr_t].insert_row_cnt))
      CALL echo(build("ind_spc_need =",dm2_rr_spc_needs->tbl[dcr_t].ind[dcr_i].space_needed))
     ENDIF
   ENDFOR
   RETURN(null)
 END ;Subroutine
 SUBROUTINE das_cal_rr_updt_size(dcru_ttgtidx,dcru_tcuridx,dcru_t)
   SET dm_err->eproc = "Calculate readme tspace needs for updates."
   IF ((dm_err->debug_flag > 1))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   DECLARE dcru_c = i4 WITH protect, noconstant(0)
   DECLARE dcru_ctgtidx = i4 WITH protect, noconstant(0)
   DECLARE dcru_ccuridx = i4 WITH protect, noconstant(0)
   DECLARE dcru_icidx = i4 WITH protect, noconstant(0)
   DECLARE dcru_icuridx = i4 WITH protect, noconstant(0)
   DECLARE dcru_col_size = f8 WITH protect, noconstant(0.0)
   DECLARE dcru_updt_col_size = f8 WITH protect, noconstant(0.0)
   FOR (dcru_c = 1 TO dm2_rr_spc_needs->tbl[dcru_t].col_updt_cnt)
     SET dcru_ctgtidx = 0
     SET dcru_ccuridx = 0
     SET dcru_col_size = 0.0
     SET dcru_updt_col_size = 0.0
     IF (dcru_ttgtidx > 0)
      SET dcru_ctgtidx = locateval(dcru_ctgtidx,1,tgtsch->tbl[dcru_ttgtidx].tbl_col_cnt,
       dm2_rr_spc_needs->tbl[dcru_t].col_updt[dcru_c].column_name,tgtsch->tbl[dcru_ttgtidx].tbl_col[
       dcru_ctgtidx].col_name)
      SET dcru_col_size = das_get_col_size("T",dcru_ttgtidx,dcru_ctgtidx,dm2_rr_spc_needs->tbl[dcru_t
       ].large_data_loaded)
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("col_size =",dcru_col_size))
      ENDIF
      SET dcru_updt_col_size = (dcru_col_size * dm2_rr_spc_needs->tbl[dcru_t].col_updt[dcru_c].
      update_row_cnt)
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("update_row_cnt =",dm2_rr_spc_needs->tbl[dcru_t].col_updt[dcru_c].
         update_row_cnt))
       CALL echo(build("dcru_updt_col_size =",dcru_updt_col_size))
      ENDIF
     ELSEIF (dcru_tcuridx > 0)
      SET das_ccuridx = locateval(dcru_ccuridx,1,cur_sch->tbl[dcru_tcuridx].tbl_col_cnt,
       dm2_rr_spc_needs->tbl[dcru_t].col_updt[dcru_c].column_name,cur_sch->tbl[dcru_tcuridx].tbl_col[
       dcru_ccuridx].col_name)
      SET dcru_col_size = das_get_col_size("C",dcru_tcuridx,dcru_ccuridx,dm2_rr_spc_needs->tbl[dcru_t
       ].large_data_loaded)
      IF ((dm_err->debug_flag > 2))
       CALL echo(build("col_size =",dcru_col_size))
      ENDIF
      ...
      ENDIF
     ENDIF
     SET dm2_rr_spc_needs->tbl[dcru_t].space_needed = (dm2_rr_spc_needs->tbl[dcru_t].space_needed+
     dcru_updt_col_size)
     FOR (dcru_i = 1 TO cur_sch->tbl[dcru_tcuridx].ind_cnt)
       SET dcru_icidx = 0
       SET dcru_icidx = locateval(dcru_icidx,1,cur_sch->tbl[dcru_tcuridx].ind[dcru_i].ind_col_cnt,
        dm2_rr_spc_needs->tbl[dcru_t].col_updt[dcru_c].column_name,cur_sch->tbl[dcru_tcuridx].ind[
        dcru_i].ind_col[dcru_icidx].col_name)
       IF (dcru_icidx > 0)
        SET dcru_icuridx = 0
        SET dcru_icuridx = locateval(dcru_icuridx,1,dm2_rr_spc_needs->tbl[dcru_t].ind_cnt,cur_sch->
         tbl[dcru_tcuridx].ind[dcru_i].ind_name,dm2_rr_spc_needs->tbl[dcru_t].ind[dcru_icuridx].
         ind_name)
        IF (dcru_icuridx=0)
         SET dm2_rr_spc_needs->tbl[dcru_t].ind_cnt = (dm2_rr_spc_needs->tbl[dcru_t].ind_cnt+ 1)
         SET stat = alterlist(dm2_rr_spc_needs->tbl[dcru_t].ind,dm2_rr_spc_needs->tbl[dcru_t].ind_cnt
          )
         SET dm2_rr_spc_needs->tbl[dcru_t].ind[dm2_rr_spc_needs->tbl[dcru_t].ind_cnt].cur_idx =
         dcru_i
         SET dm2_rr_spc_needs->tbl[dcru_t].ind[dm2_rr_spc_needs->tbl[dcru_t].ind_cnt].ind_name =
         cur_sch->tbl[dcru_tcuridx].ind[dcru_i].ind_name
         SET dm2_rr_spc_needs->tbl[dcru_t].ind[dm2_rr_spc_needs->tbl[dcru_t].ind_cnt].space_needed =
         dcru_updt_col_size
         IF ((dm_err->debug_flag > 2))
          CALL echo(build("ind_spc_need =",dm2_rr_spc_needs->tbl[dcru_t].ind[dm2_rr_spc_needs->tbl[
            dcru_t].ind_cnt].space_needed))
         ENDIF
        ELSE
         SET dm2_rr_spc_needs->tbl[dcru_t].ind[dcru_icuridx].space_needed = (dm2_rr_spc_needs->tbl[
         dcru_t].ind[dcru_icuridx].space_needed+ dcru_updt_col_size)
         IF ((dm_err->debug_flag > 2))
          CALL echo(build("ind_spc_need =",dm2_rr_spc_needs->tbl[dcru_t].ind[dcru_icuridx].
            space_needed))
         ENDIF
        ENDIF
       ENDIF
   ...
   ENDFOR
   IF ((dm_err->debug_flag > 2))
    CALL echo(build("tbl_spc_need =",dm2_rr_spc_needs->tbl[dcru_t].space_needed))
   ENDIF
   RETURN(null)
 END ;Subroutine
 SUBROUTINE das_get_mview_sizing(dgmsd_tblcnt)
   SET dm_err->eproc = "Get Materialized View space requirements."
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE dgmsd_iter = i4 WITH protect, noconstant(0)
   DECLARE dgmsd_iter2 = i4 WITH protect, noconstant(0)
   DECLARE dgmsd_indcol_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgmsd_locval = i4 WITH protect, noconstant(0)
   DECLARE dgmsd_mview_rowcnt = f8 WITH protect, noconstant(0.0)
   DECLARE dgmsd_mview_rowsize = i4 WITH protect, noconstant(0)
   DECLARE dgmsd_cmd = vc WITH protect, noconstant(" ")
   DECLARE dgmsd_bytes_alloc = f8 WITH protect, noconstant(0.0)
   DECLARE dgmsd_indrowsize = i4 WITH protect, noconstant(0)
   DECLARE dgmsd_indbytes_alloc = f8 WITH protect, noconstant(0.0)
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("TgtSch->tbl[dgmsd_tblcnt]->tbl_name:",tgtsch->tbl[dgmsd_tblcnt].tbl_name))
    CALL echo(build("TgtSch->tbl[dgmsd_tblcnt]->schema_instance:",tgtsch->tbl[dgmsd_tblcnt].
      schema_instance))
   ENDIF
   SET dm_err->eproc = "Retreiving Metadata for MVIEW"
   IF (dmvr_get_mv_metadata(tgtsch->tbl[dgmsd_tblcnt].tbl_name,tgtsch->tbl[dgmsd_tblcnt].
    schema_instance)=0)
    RETURN(0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(mv)
   ENDIF
   SET dm_err->eproc = concat("Getting rowcount estimate for MVIEW: ",tgtsch->tbl[dgmsd_tblcnt].
    tbl_name)
   SET dgmsd_cmd = concat("select into 'nl:' trowcnt= count(*) ",mv->sq_query,
    " detail dgmsd_mview_rowcnt=trowcnt with nocounter go")
   IF ( NOT (dm2_push_cmd(dgmsd_cmd,1)))
    RETURN(0)
   ENDIF
   FOR (dgmsd_iter = 1 TO tgtsch->tbl[dgmsd_tblcnt].tbl_col_cnt)
     SET dgmsd_mview_rowsize = (dgmsd_mview_rowsize+ das_get_col_size2("T",dgmsd_tblcnt,dgmsd_iter,0)
     )
   ENDFOR
   SET tgtsch->tbl[dgmsd_tblcnt].row_cnt = dgmsd_mview_rowcnt
   SET tgtsch->tbl[dgmsd_tblcnt].size = ((dgmsd_mview_rowcnt * dgmsd_mview_rowsize) * 1.15)
   SET dm_err->eproc = concat("Get Space Allocated for MVIEW: ",tgtsch->tbl[dgmsd_tblcnt].tbl_name)
   SET dgmsd_bytes_alloc = 0
   SELECT INTO "nl:"
    t_sum_bytes = sum(ds.bytes)
    FROM user_segments ds
    WHERE (ds.segment_name=tgtsch->tbl[dgmsd_tblcnt].tbl_name)
    FOOT REPORT
     dgmsd_bytes_alloc = t_sum_bytes
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("DAS: MVIEW_NAME:",tgtsch->tbl[dgmsd_tblcnt].tbl_name))
    CALL echo(build("DAS: dgmsd_mview_rowcnt:",dgmsd_mview_rowcnt))
    CALL echo(build("DAS: dgmsd_mview_rowsize:",dgmsd_mview_rowsize))
    CALL echo(build("DAS: dgmsd_bytes_alloc:",dgmsd_bytes_alloc))
    CALL echo(build("DAS: SIZE1:",tgtsch->tbl[dgmsd_tblcnt].size))
   ENDIF
   IF ((tgtsch->tbl[dgmsd_tblcnt].dext_mgmt="D"))
    SET tgtsch->tbl[dgmsd_tblcnt].init_ext = dmt_min_ext_size
   ELSEIF ((tgtsch->tbl[dgmsd_tblcnt].dext_mgmt="L"))
    SET tgtsch->tbl[dgmsd_tblcnt].next_ext = lmt_min_ext_size
   ENDIF
   SET tgtsch->tbl[dgmsd_tblcnt].cur_bytes_allocated = dgmsd_bytes_alloc
   IF ((dgmsd_bytes_alloc >= tgtsch->tbl[dgmsd_tblcnt].size))
    SET tgtsch->tbl[dgmsd_tblcnt].size = 0
   ELSE
    SET tgtsch->tbl[dgmsd_tblcnt].size = (tgtsch->tbl[dgmsd_tblcnt].size - dgmsd_bytes_alloc)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("DAS: SIZE2:",tgtsch->tbl[dgmsd_tblcnt].size))
    CALL echo(build("DAS: TBL nextext=",tgtsch->tbl[dgmsd_tblcnt].next_ext))
   ENDIF
   FOR (dgmsd_iter = 1 TO tgtsch->tbl[dgmsd_tblcnt].ind_cnt)
     SET dgmsd_indrowsize = 0
     SET dgmsd_indcol_cnt = tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter].ind_col_cnt
     FOR (dgmsd_iter2 = 1 TO dgmsd_indcol_cnt)
       SET dgmsd_locval = 0
       SET dgmsd_locval = locateval(dgmsd_locval,1,tgtsch->tbl[dgmsd_tblcnt].tbl_col_cnt,tgtsch->tbl[
        dgmsd_tblcnt].ind[dgmsd_iter].ind_col[dgmsd_iter2].col_name,tgtsch->tbl[dgmsd_tblcnt].
        tbl_col[dgmsd_locval].col_name)
       IF (dgmsd_locval > 0)
        SET dgmsd_indrowsize = (dgmsd_indrowsize+ das_get_col_size2("T",dgmsd_tblcnt,dgmsd_locval,0))
       ELSE
        SET dm_err->err_ind = 1
        SET dm_err->emsg = concat("Can not find index column ",tgtsch->tbl[dgmsd_tblcnt].ind[
         dgmsd_iter].ind_col[dgmsd_iter2].col_name," on table ",tgtsch->tbl[dgmsd_tblcnt].tbl_name)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        GO TO exit_script
       ENDIF
     ENDFOR
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("DAS: dgmsd_indrowsize1=",dgmsd_indrowsize))
     ENDIF
     SET dgmsd_indrowsize = ((dgmsd_indrowsize+ 1) * 1.1)
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("DAS: dgmsd_indrowsize2=",dgmsd_indrowsize))
     ENDIF
     SET tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter].size = dgmsd_indrowsize
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("DAS: indsize1=",tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter].size))
     ENDIF
     CALL sub_tot_new_idx_exist_tbl(dgmsd_tblcnt,dgmsd_iter)
     IF ((dm_err->err_ind=1))
      GO TO exit_script
     ENDIF
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("DAS: indsize2=",tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter].size))
     ENDIF
     SET dm_err->eproc = concat("Get Space Allocated for MVIEW index: ",tgtsch->tbl[dgmsd_tblcnt].
      ind[dgmsd_iter].ind_name)
     SET dgmsd_indbytes_alloc = 0
     SELECT INTO "nl:"
      t_sum_bytes = sum(us.bytes)
      FROM user_segments us
      WHERE (us.segment_name=tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter].ind_name)
      FOOT REPORT
       dgmsd_indbytes_alloc = t_sum_bytes
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
     SET tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter].cur_bytes_allocated = dgmsd_bytes_alloc
     IF ((dgmsd_indbytes_alloc >= tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter].size))
      SET tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter].size = 0
     ELSE
      SET tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter].size = (tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter]
      .size - dgmsd_indbytes_alloc)
     ENDIF
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("DAS: dgmsd_indbytes_alloc:",dgmsd_indbytes_alloc))
      CALL echo(build("DAS: indsize3:",tgtsch->tbl[dgmsd_tblcnt].ind[dgmsd_iter].size))
     ENDIF
   ENDFOR
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("DAS: tbl nextext=",tgtsch->tbl[dgmsd_tblcnt].next_ext))
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE das_verify_function(dvf_function_name)
   IF ( NOT (dvf_function_name IN ("DM2_GET_AVG_COL_SIZE", "DM2_GET_NBR_NULL_VALUES")))
    SET dm_err->eproc = "Validate function name passed into routine is valid."
    SET dm_err->emsg = concat("Function name ",dvf_function_name," is not valid.")
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = concat("Checking the validity/existence of function ",dvf_function_name,".")
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   IF (das_avgcol_func_valid=1
    AND dvf_function_name="DM2_GET_AVG_COL_SIZE")
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("Function ",dvf_function_name," already validated."))
    ENDIF
    RETURN(1)
   ENDIF
   IF (das_nbrull_func_valid=1
    AND dvf_function_name="DM2_GET_NBR_NULL_VALUES")
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("Function ",dvf_function_name," already validated."))
    ENDIF
    RETURN(1)
   ...
   SELECT INTO "nl:"
    ...
    WHERE uo.object_name=dvf_function_name
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->eproc = concat("Creating function ",dvf_function_name,".")
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
    IF (dvf_function_name="DM2_GET_AVG_COL_SIZE")
     CALL dm2_push_cmd(concat("rdb asis(^CREATE OR REPLACE FUNCTION DM2_GET_AVG_COL_SIZE^)"),0)
     CALL dm2_push_cmd(concat("asis(^ (table_name VARCHAR2,column_name VARCHAR2)",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ RETURN NUMBER IS",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ Query_str VARCHAR2(1000);",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ avg_return NUMBER;",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ BEGIN",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ Query_str := 'SELECT avg(nvl(vsize(' || column_name^)"),0)
     CALL dm2_push_cmd(concat("asis(^ || '),0)) FROM ' || table_name ^)"),0)
     CALL dm2_push_cmd(concat(
       "asis(^ || ' where not ' || column_name || ' is NULL and rownum <= 10000'; ",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ EXECUTE IMMEDIATE query_str INTO avg_return;",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ RETURN avg_return;",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ end; ",char(10),"^)"),0)
     CALL dm2_push_cmd(" end go ",1)
    ELSE
     CALL dm2_push_cmd(concat("rdb asis(^CREATE OR REPLACE FUNCTION DM2_GET_NBR_NULL_VALUES^)"),0)
     CALL dm2_push_cmd(concat("asis(^ (table_name VARCHAR2,column_name VARCHAR2)",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ RETURN NUMBER IS",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ Query_str VARCHAR2(1000);",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ sum_return NUMBER;",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ BEGIN",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ Query_str := 'SELECT sum(nvl2(' || column_name^)"),0)
     CALL dm2_push_cmd(concat("asis(^ || ',0,1)) FROM ' || table_name ^)"),0)
     CALL dm2_push_cmd(concat("asis(^ || ' where rownum <= 10000'; ",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ EXECUTE IMMEDIATE query_str INTO sum_return;",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ RETURN sum_return;",char(10),"^)"),0)
     CALL dm2_push_cmd(concat("asis(^ end; ",char(10),"^)"),0)
     CALL dm2_push_cmd(" end go ",1)
    ENDIF
    IF (check_error(dm_err->eproc)=1)
     ...
     RETURN(0)
    ENDIF
    SET dm_err->eproc = concat("Check if function ",dvf_function_name," is valid.")
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
    ...
    ENDIF
    IF (curqual=0)
     SET dm_err->emsg = concat("Function ",dvf_function_name," is invalid.")
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
   ENDIF
   IF (dvf_function_name="DM2_GET_AVG_COL_SIZE")
    SET das_avgcol_func_valid = 1
   ELSE
    SET das_nbrull_func_valid = 1
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE debug_disp_tgt_size(null)
   DECLARE das_log = vc
   SET das_log = build(dm2_install_schema->schema_prefix,dm2_install_schema->file_prefix,
    "_apply_s.log")
   SELECT INTO value(das_log)
    d.seq
    FROM (dummyt d  WITH seq = value(tgtsch->tbl_cnt))
    WHERE (((tgtsch->tbl[d.seq].diff_ind=1)) OR ((tgtsch->tbl[d.seq].new_ind=1)))
    HEAD REPORT
     "table count = ", tgtsch->tbl_cnt, row + 1,
     "Current Database = ", currdb, row + 1,
     "TgtSch"
    DETAIL
     row + 1, tgt_sd = format(tgtsch->tbl[d.seq].schema_date,"dd-mmm-yyyy hh:mm:ss;;d"), row + 1,
     row + 1, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", row + 1,
     "table number = ", d.seq, row + 1,
     "table_name = ", tgtsch->tbl[d.seq].tbl_name, row + 1,
     "table new ind=", tgtsch->tbl[d.seq].new_ind, row + 1,
     "table diff ind = ", tgtsch->tbl[d.seq].diff_ind, row + 1,
     "data tablespace = ", tgtsch->tbl[d.seq].tspace_name, row + 1,
     "index tablespace = ", tgtsch->tbl[d.seq].ind_tspace, row + 1,
     "long tablespace = ", tgtsch->tbl[d.seq].long_tspace, row + 1,
     "data tablespace management = ", tgtsch->tbl[d.seq].dext_mgmt, row + 1,
     "ind tablespace management = ", tgtsch->tbl[d.seq].iext_mgmt, row + 1,
     "long tablespace management = ", tgtsch->tbl[d.seq].lext_mgmt, row + 1,
     "data tablespace new ind = ", tgtsch->tbl[d.seq].ttspace_new_ind, row + 1,
     "ind tablespace new ind = ", tgtsch->tbl[d.seq].itspace_new_ind, row + 1,
     "long tablespace new ind = ", tgtsch->tbl[d.seq].ltspace_new_ind, row + 1,
     "table size = ", tgtsch->tbl[d.seq].size, row + 1,
     "index size = ", tgtsch->tbl[d.seq].ind_size, row + 1,
     "long size = ", tgtsch->tbl[d.seq].long_size, row + 1,
     "table init extent = ", tgtsch->tbl[d.seq].init_ext, row + 1,
     "table next extent = ", tgtsch->tbl[d.seq].next_ext, row + 1,
     "reference ind = ", tgtsch->tbl[d.seq].reference_ind
    WITH nocounter
   ;end select
   IF ((dir_ui_misc->dm_process_event_id > 0))
    CALL drr_assign_file_to_installs("RPT:APPLYSIZING",das_log,dir_ui_misc->dm_process_event_id)
   ENDIF
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "DM2_APPLY_SIZING COMPLETED"
 CALL final_disp_msg("dm2_apply_sizing")
END GO
