CREATE PROGRAM acm_add_prsnl_role_profile_add:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->prsnl_role_profile_addn_qual[xref->add[index].idx].prsnl_role_profile_addn_id
    <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(prsnl_seq,nextval)
     FROM dual
     DETAIL
      acm_request->prsnl_role_profile_addn_qual[xref->add[index].idx].prsnl_role_profile_addn_id =
      cnvtreal(nextseqnum), reply->prsnl_role_profile_addn_qual[xref->add[index].idx].
      prsnl_role_profile_addn_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->prsnl_role_profile_addn_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->prsnl_role_profile_addn_qual[xref->add[index].idx].prsnl_role_profile_addn_id =
    acm_request->prsnl_role_profile_addn_qual[xref->add[index].idx].prsnl_role_profile_addn_id
   ENDIF
   SET reply->prsnl_role_profile_addn_qual[xref->add[index].idx].status = 0
   IF ((acm_request->prsnl_role_profile_addn_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->prsnl_role_profile_addn_qual[xref->add[index].idx].beg_effective_dt_tm =
    cnvtdatetime(curdate,curtime3)
   ENDIF
   IF ((acm_request->prsnl_role_profile_addn_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->prsnl_role_profile_addn_qual[xref->add[index].idx].end_effective_dt_tm =
    cnvtdatetime("31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   prsnl_role_profile_addn p
  SET p.prsnl_role_profile_addn_id = acm_request->prsnl_role_profile_addn_qual[xref->add[d.seq].idx].
   prsnl_role_profile_addn_id, p.prsnl_org_reltn_type_id =
   IF ((acm_request->prsnl_role_profile_addn_qual[xref->add[d.seq].idx].prsnl_org_reltn_type_id > 0))
     acm_request->prsnl_role_profile_addn_qual[xref->add[d.seq].idx].prsnl_org_reltn_type_id
   ELSE reply->prsnl_org_reltn_type_qual[acm_request->prsnl_role_profile_addn_qual[xref->add[d.seq].
    idx].prsnl_org_reltn_type_idx].prsnl_org_reltn_type_id
   ENDIF
   , p.child_type_cd = acm_request->prsnl_role_profile_addn_qual[xref->add[d.seq].idx].child_type_cd,
   p.type_entity_name = acm_request->prsnl_role_profile_addn_qual[xref->add[d.seq].idx].
   type_entity_name, p.type_entity_id = acm_request->prsnl_role_profile_addn_qual[xref->add[d.seq].
   idx].type_entity_id, p.beg_effective_dt_tm = cnvtdatetime(acm_request->
    prsnl_role_profile_addn_qual[xref->add[d.seq].idx].beg_effective_dt_tm),
   p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_role_profile_addn_qual[xref->add[d.seq].
    idx].end_effective_dt_tm), p.active_ind = 1, p.active_status_cd = reqdata->active_status_cd,
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->prsnl_role_profile_addn_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->prsnl_role_profile_addn_qual[xref->add[index].idx].status=0))
