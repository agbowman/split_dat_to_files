CREATE PROGRAM afc_del_interval:dba
 ...
 CALL del_interval_table(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE del_interval_table(del_begin,del_end)
   FOR (x = del_begin TO del_end)
     ...
     UPDATE  FROM interval_table i
      SET i.active_ind = false, i.active_status_cd = nullcheck(active_code,request->interval_table[x]
        .active_status_cd,
        IF ((request->interval_table[x].active_status_cd=0)) 0
        ELSE 1
        ENDIF
        ), i.active_status_prsnl_id = reqinfo->updt_id,
       i.active_status_dt_tm = cnvtdatetime(sysdate), i.updt_cnt = (i.updt_cnt+ 1), i.updt_dt_tm =
