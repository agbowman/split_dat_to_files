CREATE PROGRAM bhs_cnvt_powernotes_v2:dba
 ...
 CALL sbr_log(concat("event_title_text: ",trim(request->clin_detail_list[1].event_title_text)))
 ...
 IF (trim(request->clin_detail_list[1].event_title_text) != ms_title_text
  AND trim(request->clin_detail_list[1].event_title_text) != ms_title_text2)
 ...
 IF (textlen(trim(ms_fin_nbr)) > 0)
  ...
  ELSEIF (trim(request->clin_detail_list[1].event_title_text)=ms_title_text2)
 ...
 IF (log_message="")
  ...
  SET log_message = concat("Success: created file ",ms_file_name)
  CALL sbr_log(log_message)
 ENDIF
 IF (retval=0)
  SET retval = 100
