CREATE PROGRAM ccps_pha_lbt_footer_nobc
 PROMPT
 ...
 SUBROUTINE check_secabs(ncalc,offsetx,offsety)
   ...
   DECLARE __exp_label = vc WITH noconstant(build2(
     IF ((((params->label_per != "DOSE")) OR (((com_msg=concat("execute ",trim(params->
       order_header_object)," '",print_file,"',1 go")) OR (print_file=build(logical("cer_temp"),
      dir_char,"phadrvr_",pdq,"_",
   ...
   IF (ncalc=rpt_render)
    ...
    IF ((params->label_per="DOSE")
     ...
     )) OR (print_file=build(logical("cer_temp"),dir_char,"phadrvr_",pdq,"_",
     cnvtint(data->run_id),"_",label_rec_ndx,"_foot.dat"))) ))
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CCPS_PHA_LBT_FOOTER_NOBC"
