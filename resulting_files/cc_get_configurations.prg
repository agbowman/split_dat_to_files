CREATE PROGRAM cc_get_configurations
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Prsnl Id" = 0.0,
  "Position Cd" = 0.0
  WITH outdev, prsnlid, positioncd
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CC_GET_CONFIGURATIONS"
 ...
 SUBROUTINE (validatesrvreply(nsrvstat=i4,recorddata=vc(ref),zeroforceexit=i2) =i4 WITH protect)
   ...
   ENDIF
 END ;Subroutine
 DECLARE request_number = i4 WITH protect, constant(3208641)
 DECLARE srv_request = vc WITH protect, constant("QueryConfigurations")
 ...
 SUBROUTINE (getdata(hreply=i4(ref)) =null WITH protect)
   ...
   SET putrequest->source_dir =  $OUTDEV
   SET putrequest->isblob = "1"
   SET pos = uar_srvgetasissize(hreply,"content_json")
   SET putrequest->document = substring(1,pos,uar_srvgetasisptr(hreply,"content_json"))
   SET putrequest->document_size = size(putrequest->document)
   CALL exit_servicerequest(happ,htask,hstep)
