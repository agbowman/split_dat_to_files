CREATE PROGRAM bhs_athn_get_pat_diagnosis
 IF (( $2=0))
 ...
 SELECT INTO  $1
  ...
  d.originating_nomenclature_id, d.person_id, d_present_on_admit_disp = uar_get_code_display(d
   .present_on_admit_cd),
  d.probability, d_ranking_disp = uar_get_code_display(d.ranking_cd), d_severity_disp =
  uar_get_code_display(d.severity_cd),
  d_severity_class_disp = uar_get_code_display(d.severity_class_cd), d_severity_free_text = trim(
   replace(replace(replace(replace(replace(substring(1,255,d.severity_ftdesc),"&","&amp;",0),"<",
       "&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3), d.updt_dt_tm,
  d.updt_id, n.source_identifier, n_source_vocabulary_disp = uar_get_code_display(n
   .source_vocabulary_cd),
  n_source_string = trim(replace(replace(replace(replace(replace(substring(1,255,n.source_string),"&",
        "&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3), l_diag_comment =
  IF (l.long_blob_id != 0) trim(replace(replace(replace(replace(replace(substring(1,30000,l.long_blob
          ),"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3)
  ELSE " "
  ENDIF
  ...
  HEAD d.diagnosis_group
   ...
   col + 1, v13, row + 1,
   v131 = build("<OriginatingNomenclatureId>",cnvtint(d.originating_nomenclature_id),
    "</OriginatingNomenclatureId>"), col + 1, v131,
   row + 1, v14 = build("<SourceIdentifier>",n.source_identifier,"</SourceIdentifier>"), col + 1,
   v14, row + 1, v15 = build("<SourceString>",n_source_string,"</SourceString>"),
   col + 1, v15, row + 1,
   v16 = build("<SourceVacabulary>",n_source_vocabulary_disp,"</SourceVacabulary>"), col + 1, v16,
   row + 1, v17 = build("<DiagnosisComment>",l_diag_comment,"</DiagnosisComment>"), col + 1,
   v17, row + 1, v18 = build("<DiagnosisStatus>",d_active_status_disp,"</DiagnosisStatus>"),
   col + 1, v18, row + 1,
   v181 = build("<DiagnosisStatusCd>",cnvtint(d.active_status_cd),"</DiagnosisStatusCd>"), col + 1,
   v181,
   row + 1, v19 = build("<DiagnosisType>",d_diag_type_disp,"</DiagnosisType>"), col + 1,
   v19, row + 1, v191 = build("<DiagnosisTypeCd>",cnvtint(d.diag_type_cd),"</DiagnosisTypeCd>"),
   col + 1, v191, row + 1,
   v20 = build("<ConfidentialityLevel>",d_confid_level_disp,"</ConfidentialityLevel>"), col + 1, v20,
   row + 1, v201 = build("<ConfidentialityLevelCd>",cnvtint(d.confid_level_cd),
    "</ConfidentialityLevelCd>"), col + 1,
   v201, row + 1, v21 = build("<DiagnosisCategory>",d_diagnostic_category_disp,"</DiagnosisCategory>"
    ),
   col + 1, v21, row + 1,
   v211 = build("<DiagnosisCategoryCd>",cnvtint(d.diagnostic_category_cd),"</DiagnosisCategoryCd>"),
   col + 1, v211,
   row + 1, v22 = build("<Probability>",d.probability,"</Probability>"), col + 1,
   v22, row + 1, v23 = build("<PresentOnAdmission>",d_present_on_admit_disp,"</PresentOnAdmission>"),
   col + 1, v23, row + 1,
   v231 = build("<PresentOnAdmissionCd>",cnvtint(d.present_on_admit_cd),"</PresentOnAdmissionCd>"),
   col + 1, v231,
   row + 1, v24 = build("<Severity>",d_severity_disp,"</Severity>"), col + 1,
   v24, row + 1, v241 = build("<SeverityCd>",cnvtint(d.severity_cd),"</SeverityCd>"),
   col + 1, v241, row + 1,
   v25 = build("<Laterality>",d_laterality_disp,"</Laterality>"), col + 1, v25,
   row + 1, v251 = build("<LateralityCd>",cnvtint(d.laterality_cd),"</LateralityCd>"), col + 1,
   v251, row + 1, v26 = build("<ActiveIndicator>",cnvtint(d.active_ind),"</ActiveIndicator>"),
   col + 1, v26, row + 1,
   v27 = build("<EndEffectiveDate>",format(d.end_effective_dt_tm,"MM/DD/YYYY"),"</EndEffectiveDate>"),
   col + 1, v27,
   row + 1, v28 = build("<Certainity>",d_certainty_disp,"</Certainity>"), col + 1,
   v28, row + 1, v29 = build("<CertainityCd>",cnvtint(d.certainty_cd),"</CertainityCd>"),
   col + 1, v29, row + 1,
   v30 = build("<SeverityClass>",d_severity_class_disp,"</SeverityClass>"), col + 1, v30,
   row + 1, v31 = build("<SeverityClassCd>",cnvtint(d.severity_class_cd),"</SeverityClassCd>"), col
    + 1,
   v31, row + 1, v32 = build("<SeverityFreeText>",d_severity_free_text,"</SeverityFreeText>"),
   col + 1, v32, row + 1,
   v33 = build("<ConditionalQualifier>",d_conditional_qual_disp,"</ConditionalQualifier>"), col + 1,
   v33,
   row + 1, v34 = build("<ConditionalQualifierCd>",cnvtint(d.conditional_qual_cd),
    "</ConditionalQualifierCd>"), col + 1,
   v34, row + 1, v35 = build("<DiagnosisClass>",d_diag_class_disp,"</DiagnosisClass>"),
   col + 1, v35, row + 1,
   v36 = build("<DiagnosisClassCd>",cnvtint(d.diag_class_cd),"</DiagnosisClassCd>"), col + 1, v36,
   row + 1
