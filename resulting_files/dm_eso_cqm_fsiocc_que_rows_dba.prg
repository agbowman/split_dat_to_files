CREATE PROGRAM dm_eso_cqm_fsiocc_que_rows:dba
 CALL echo("<===== DM_ESO_CQM_FSIOCC_QUE_ROWS  BEGIN =====>")
 ...
 DECLARE tkn_idx = i4 WITH protect, noconstant(0)
 ...
 SET reply->table_name = "CQM_FSIOCC_QUE"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(c.queue_id)
   FROM cqm_fsiocc_que c
   WHERE c.queue_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   ...
   IF (errcd > 0)
    ...
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s2","Failed in row collection: %1","d",
     nullterm(errmsg))
    SET reply->status_data.status = "F"
    GO TO exit_script
   ENDIF
...
#exit_script
 CALL echo("<===== DM_ESO_CQM_FSIOCC_QUE_ROWS END =====>")
END GO
