CREATE PROGRAM dm_ce_draft_rows:dba
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(cd.ce_draft_id)
   FROM ce_draft cd
   WHERE cd.ce_draft_id > 0
   DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    cd.rowid
    FROM ce_draft cd
    WHERE cd.ce_draft_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists("cd.last_modified_dt_tm < cnvtdatetime(target_dt_tm)","cd"))
     AND  NOT ( EXISTS (
    (SELECT
     1
     FROM clinical_event ce,
      ce_draft_reltn dr
     WHERE cd.ce_draft_id=dr.ce_draft_id
      AND dr.event_id=ce.event_id
      AND ce.record_status_cd IN (
     (SELECT
      cv.code_value
      FROM code_value cv
      WHERE cv.code_set=48
       AND cv.cdf_meaning IN ("ACTIVE", "DELETED"))))))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = cd.rowid
    WITH nocounter, maxqual(cd,value(v_rows_left))
   ;end select
