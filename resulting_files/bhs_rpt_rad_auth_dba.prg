CREATE PROGRAM bhs_rpt_rad_auth:dba
 PROMPT
 ...
 SUBROUTINE (sec_titleabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_x = (offsetx+ 1.760)
    SET rptsd->m_width = 3.552
    SET rptsd->m_height = 0.208
    SET _oldfont = uar_rptsetfont(_hreport,_times120)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Radiology Authorization Report",char(
       0)))
 ...
 SUBROUTINE (sec_appt_headerabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.021),(offsety+ 0.313),(offsetx+ 7.500),(offsety+
     0.313))
    SET rptsd->m_y = (offsety+ 0.063)
 ...
 SUBROUTINE (sec_appt_detailabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   protect
   DECLARE __sad_mrn7 = vc WITH noconstant(build2(s_data->qual[ml_idx].s_fin,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.031),(offsety+ 0.282),(offsetx+ 7.500),(offsety+
     0.282))
    SET rptsd->m_y = (offsety+ 0.031)
    ...
    SET rptsd->m_width = 0.563
    SET rptsd->m_height = 0.260
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__sad_mrn7)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (sec_auth_head1(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = sec_auth_head1abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (sec_auth_head1abs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_times9b0)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Authorization:",char(0)))
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (sec_auth_head2(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = sec_auth_head2abs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (sec_auth_head2abs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.220000), private
   DECLARE __sad_auth_num = vc WITH noconstant(build2(s_data->qual[ml_idx].aqual[ml_idx1].s_auth_nbr,
     char(0))), protect
   DECLARE __sad_auth_ins_name = vc WITH noconstant(build2(s_data->qual[ml_idx].aqual[ml_idx1].
     s_ins_name,char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__sad_auth_num)
    SET rptsd->m_y = (offsety+ 0.031)
    ...
    SET rptsd->m_height = 0.188
    SET _dummyfont = uar_rptsetfont(_hreport,_times90)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__sad_auth_ins_name)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (sec_auth_detail(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = sec_auth_detailabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (sec_auth_detailabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.220000), private
   DECLARE __sad_auth_num = vc WITH noconstant(build2(s_data->qual[ml_idx].aqual[ml_idx1].s_auth_nbr,
     char(0))), protect
   DECLARE __sad_auth_ins_name = vc WITH noconstant(build2(s_data->qual[ml_idx].aqual[ml_idx1].
 ...
 SUBROUTINE (sec_order_headerabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.438),(offsety+ 0.322),(offsetx+ 7.480),(offsety+
     0.322))
    SET rptsd->m_y = (offsety+ 0.063)
 ...
 SUBROUTINE (sec_order_detailabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _dummypen = uar_rptsetpen(_hreport,_pen14s3c0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.438),(offsety+ 0.240),(offsetx+ 7.532),(offsety+
     0.240))
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (sec_proc_header(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = sec_proc_headerabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (sec_proc_headerabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.380000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    ...
    SET rptsd->m_width = 1.063
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Completed Date",char(0)))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.438),(offsety+ 0.322),(offsetx+ 7.480),(offsety+
     0.322))
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 5.813)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (sec_proc_detail(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = sec_proc_detailabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (sec_proc_detailabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.290000), private
   DECLARE __sad_proc_name = vc WITH noconstant(build2(s_data->qual[ml_idx].pqual[ml_idx1].
     s_order_disp,char(0))), protect
   DECLARE __accession = vc WITH noconstant(build2(s_data->qual[ml_idx].pqual[ml_idx1].s_accession,
     char(0))), protect
   DECLARE __sad_proc_status = vc WITH noconstant(build2(s_data->qual[ml_idx].pqual[ml_idx1].
     s_order_status,char(0))), protect
   DECLARE __sad_proc_comp_date = vc WITH noconstant(build2(s_data->qual[ml_idx].pqual[ml_idx1].
     s_complete_dt,char(0))), protect
   DECLARE __sad_priority = vc WITH noconstant(build2(s_data->qual[ml_idx].pqual[ml_idx1].s_priority,
     char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_times90)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__sad_proc_name)
    SET rptsd->m_y = (offsety+ 0.031)
    ...
    SET rptsd->m_width = 0.625
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__sad_proc_status)
    SET rptsd->m_y = (offsety+ 0.031)
    SET rptsd->m_x = (offsetx+ 6.500)
    SET rptsd->m_width = 0.938
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__sad_proc_comp_date)
    SET _dummypen = uar_rptsetpen(_hreport,_pen14s3c0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.438),(offsety+ 0.271),(offsetx+ 7.532),(offsety+
     0.271))
    SET rptsd->m_y = (offsety+ 0.031)
    SET rptsd->m_x = (offsetx+ 4.875)
    ...
    SET rptsd->m_height = 0.188
    SET _dummypen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__sad_priority)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_RPT_RAD_AUTH"
 ...
 SELECT INTO "nl:"
  ...
  HEAD sa.sch_event_id
   ...
   sa.person_id, s_data->qual[s_data->l_cnt].f_sch_event_id = sa.sch_event_id,
   s_data->qual[s_data->l_cnt].s_appt_loc = uar_get_code_display(sa.appt_location_cd), s_data->qual[
   s_data->l_cnt].s_appt_type = uar_get_code_display(se.appt_type_cd), s_data->qual[s_data->l_cnt].
   s_fin = trim(ea2.alias,3),
   s_data->qual[s_data->l_cnt].s_mrn = trim(ea1.alias,3), s_data->qual[s_data->l_cnt].s_appt_status
    = uar_get_code_display(se.sch_state_cd), s_data->qual[s_data->l_cnt].s_pat_name = p
   .name_full_formatted,
   s_data->qual[s_data->l_cnt].f_appt_date = sa.beg_dt_tm, s_data->qual[s_data->l_cnt].l_ocnt = 0
  HEAD ord.order_id
   ...
   s_data->qual[s_data->l_cnt].oqual[s_data->qual[s_data->l_cnt].l_ocnt].s_reason_for_visit = trim(e
    .reason_for_visit,3)
  ...
  FOR (ml_idx = 1 TO s_data->l_cnt)
   SELECT INTO "nl:"
    FROM authorization a,
     health_plan hp
    PLAN (a
     WHERE (a.encntr_id=s_data->qual[ml_idx].f_encntr_id)
      AND a.active_ind=1
      AND a.end_effective_dt_tm > sysdate)
     JOIN (hp
     WHERE hp.health_plan_id=a.health_plan_id
      AND hp.health_plan_id != 0)
    ORDER BY a.auth_nbr
    HEAD REPORT
     s_data->qual[ml_idx].l_acnt = 0
    HEAD a.auth_nbr
     s_data->qual[ml_idx].l_acnt += 1, stat = alterlist(s_data->qual[ml_idx].aqual,s_data->qual[
      ml_idx].l_acnt), s_data->qual[ml_idx].aqual[s_data->qual[ml_idx].l_acnt].s_auth_nbr = a
     .auth_nbr,
     s_data->qual[ml_idx].aqual[s_data->qual[ml_idx].l_acnt].s_ins_name = hp.plan_name
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    FROM order_radiology ord
    WHERE (ord.encntr_id=s_data->qual[ml_idx].f_encntr_id)
    HEAD REPORT
     s_data->qual[ml_idx].l_pcnt = 0
    DETAIL
     s_data->qual[ml_idx].l_pcnt += 1, stat = alterlist(s_data->qual[ml_idx].pqual,s_data->qual[
      ml_idx].l_pcnt), s_data->qual[ml_idx].pqual[s_data->qual[ml_idx].l_pcnt].f_order_id = ord
     .order_id,
     s_data->qual[ml_idx].pqual[s_data->qual[ml_idx].l_pcnt].f_catalog_cd = ord.catalog_cd, s_data->
     qual[ml_idx].pqual[s_data->qual[ml_idx].l_pcnt].s_order_disp = uar_get_code_display(ord
      .catalog_cd), s_data->qual[ml_idx].pqual[s_data->qual[ml_idx].l_pcnt].s_order_status =
     uar_get_code_display(ord.exam_status_cd),
     s_data->qual[ml_idx].pqual[s_data->qual[ml_idx].l_pcnt].s_accession = trim(ord.accession,3),
     s_data->qual[ml_idx].pqual[s_data->qual[ml_idx].l_pcnt].s_complete_dt = format(ord
      .complete_dt_tm,"MM/DD/YYYY;;q"), s_data->qual[ml_idx].pqual[s_data->qual[ml_idx].l_pcnt].
     s_priority = uar_get_code_display(ord.priority_cd)
    WITH nocounter
   ;end select
  ENDFOR
 ENDIF
 ...
 IF ((s_data->l_cnt > 0))
  FOR (ml_idx = 1 TO s_data->l_cnt)
    ...
    IF ((s_data->qual[ml_idx].l_acnt=0))
     SET mf_rem_space = (mf_page_size - (_yoffset+ sec_auth_head1(rpt_calcheight)))
     IF (mf_rem_space <= 0.25)
      SET _yoffset = 10.18
      SET d0 = pagebreak(0)
     ENDIF
     SET d0 = sec_auth_head1(rpt_render)
    ELSE
     FOR (ml_idx1 = 1 TO s_data->qual[ml_idx].l_acnt)
       IF (ml_idx1=1)
        SET mf_rem_space = (mf_page_size - (_yoffset+ sec_auth_head2(rpt_calcheight)))
        IF (mf_rem_space <= 0.25)
         SET _yoffset = 10.18
         SET d0 = pagebreak(0)
        ENDIF
        SET d0 = sec_auth_head2(rpt_render)
       ELSE
        SET mf_rem_space = (mf_page_size - (_yoffset+ sec_auth_detail(rpt_calcheight)))
        IF (mf_rem_space <= 0.25)
         SET _yoffset = 10.18
         SET d0 = pagebreak(0)
        ENDIF
        SET d0 = sec_auth_detail(rpt_render)
       ENDIF
     ENDFOR
    ENDIF
    ...
    IF ((s_data->qual[ml_idx].l_ocnt > 0))
     ...
     ENDFOR
    ENDIF
    SET mf_rem_space = (mf_page_size - (_yoffset+ sec_proc_header(rpt_calcheight)))
    IF (mf_rem_space <= 0.25)
     SET _yoffset = 10.18
     SET d0 = pagebreak(0)
    ENDIF
    SET d0 = sec_proc_header(rpt_render)
    IF ((s_data->qual[ml_idx].l_pcnt > 0))
     FOR (ml_idx1 = 1 TO s_data->qual[ml_idx].l_pcnt)
       SET mf_rem_space = (mf_page_size - (_yoffset+ sec_proc_detail(rpt_calcheight)))
       IF (mf_rem_space <= 0.25)
        SET _yoffset = 10.18
        SET d0 = pagebreak(0)
       ENDIF
       SET d0 = sec_proc_detail(rpt_render)
     ENDFOR
    ENDIF
  ENDFOR
 ENDIF
