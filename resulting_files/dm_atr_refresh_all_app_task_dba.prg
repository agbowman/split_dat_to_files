CREATE PROGRAM dm_atr_refresh_all_app_task:dba
 SET inhouse_ind = 0
 IF (trim(ref)="C")
  SET inhouse_ind = 1
 ENDIF
 ...
 SET at_list->count = 0
 SET filename = "DM_ATR_REFRESH_APP_TASK.DAT"
 ...
 ENDIF
 CALL echo("Generating list of latest Application-Task relations...")
 SELECT INTO value(filename)
  *
  FROM dm_application_task_r d
  WHERE sqlpassthru(concat("(d.application_number,d.task_number,d.schema_date) in ",
    "(select dm.application_number, dm.task_number, max(dm.schema_date) from dm_application_task_r dm, dm_features df where 1=1"
    ))
   AND sqlpassthru(concat(" UPPER(df.feature_status) in ",trim(feature_status)," "))
   AND sqlpassthru(
   " dm.feature_number = df.feature_number group by dm.application_number, dm.task_number )")
  ORDER BY d.feature_number, d.application_number, d.task_number
  HEAD REPORT
   IF (readme_ind > 0.0)
    ...
    row + 1
   ELSE
    at_list->count = 0, stat = alterlist(at_list->at,0)
   ENDIF
   ...
   "    2 task_number = i4", row + 1, "    2 deleted_ind = i2",
  ...
  DETAIL
   ...
   row + 1, "set request->atr_list[request->atr_count]->task_number = ", d.task_number,
   " go", row + 1, "set request->atr_list[request->atr_count]->deleted_ind = ",
   d.deleted_ind, " go", row + 1
   IF (readme_ind=0.0)
    at_list->count = (at_list->count+ 1), stat = alterlist(at_list->at,at_list->count), at_list->at[
    at_list->count].a_num = d.application_number,
    at_list->at[at_list->count].t_num = d.task_number, at_list->at[at_list->count].f_num = d
    .feature_number, at_list->at[at_list->count].s_dt = d.schema_date
   ENDIF
   atr_cnt = (atr_cnt+ 1)
   IF (atr_cnt=500)
    row + 1, row + 1, "/* Call the import script every 500th ATR */",
    row + 1, "execute dm_atr_app_task_import go", row + 1,
  ...
  FOOT REPORT
   row + 1, row + 1
   IF (atr_cnt > 0)
    "/* Now call the import script */", row + 1, "execute dm_atr_app_task_import go",
 ...
 ;end select
 IF (readme_ind=0.0)
  CALL echo("Updating ATR status for Application-Task Relations...")
  UPDATE  FROM dm_feature_app_task_env dm,
    (dummyt d  WITH seq = value(size(at_list->at,5)))
   SET dm.status = "1"
   PLAN (d)
    JOIN (dm
    WHERE dm.environment=env
     AND (dm.application_number=at_list->at[d.seq].a_num)
     AND (dm.task_number=at_list->at[d.seq].t_num)
     AND (((dm.feature_number=at_list->at[d.seq].f_num)) OR (datetimecmp(dm.schema_dt_tm,cnvtdatetime
     (format(at_list->at[d.seq].s_dt,"DD-MMM-YYYY;;D"))) <= 0)) )
  ;end update
  COMMIT
 ENDIF
END GO
