CREATE PROGRAM bh_rc_rl_an_spou_inc:dba
 CALL echo("***** bh_rc_rl_an_spou_inc.prg - 677918 *****")
 ...
 SUBROUTINE filltemprec(null)
   FOR (lchgcnt = 1 TO size(request->integer_params,5))
     CASE (request->integer_params[lchgcnt].key_txt)
      OF "SPOUSE_ANNUAL":
       SET tmprec->spouse_monthly = (request->integer_params[lchgcnt].integer_value/ 12)
     ENDCASE
   ENDFOR
 END ;Subroutine
 DECLARE createreply(null) = null
 SUBROUTINE createreply(null)
  CALL addtoreplyintegers("SPOUSE_MONTHLY",tmprec->spouse_monthly)
