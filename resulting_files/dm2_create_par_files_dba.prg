CREATE PROGRAM dm2_create_par_files:dba
 ...
 SUBROUTINE drr_del_preserved_ts(dcdl_tgt_db_name)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(dcpf_text->line_cnt,- (1))=- (1))
  AND (validate(dcpf_text->line_cnt,- (2))=- (2)))
  FREE RECORD dcpf_text
  RECORD dcpf_text(
    1 line_cnt = i4
    1 line[*]
      2 text = vc
  )
  SET dcpf_text->line_cnt = 0
 ENDIF
 DECLARE dcpf_mode = vc WITH protect, noconstant("NOT SET")
 DECLARE dcpf_loc = vc WITH protect, noconstant("")
 DECLARE dcpf_session_resumable = i2 WITH protect, noconstant(0)
 DECLARE dcpf_session_timeout = i4 WITH protect, noconstant(0)
 DECLARE dcpf_resumable_time = vc WITH protect, noconstant("NOT SET")
 DECLARE dcpf_statistics_imp = i2 WITH protect, noconstant(0)
 DECLARE dcpf_parfile_table_cnt = i4 WITH protect, noconstant(400)
 DECLARE dcpf_build_reference(null) = i2
 DECLARE dcpf_build_mixed(null) = i2
 DECLARE dcpf_build_indmixed_ref(null) = i2
 DECLARE dcpf_build_indmixed_import_parfile(null) = i2
 DECLARE dcpf_build_allv500(null) = i2
 DECLARE dcpf_build_pts(null) = i2
 DECLARE dcpf_build_rdds(null) = i2
 IF (check_logfile("dm2_create_parfiles",".log","DM2_CREATE_PAR_FILES LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dcpf_mode = cnvtupper( $1)
 SET dcpf_loc = trim( $2)
 IF (currdb="ORACLE")
  IF (dm2_get_rdbms_version(null)=0)
   GO TO exit_program
  ENDIF
  IF ((dm2_rdbms_version->level1 >= 9))
   SET dcpf_session_resumable = 1
   SET dcpf_statistics_imp = 1
   SET dcpf_session_timeout = 28800
   SET dcpf_resumable_time = build("resumable_timeout=",dcpf_session_timeout)
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echo(build("oracle_version_base =",dm2_rdbms_version->level1))
   CALL echo(build("session_resumable =",dcpf_session_resumable))
   CALL echo(build("session_timeout =",dcpf_session_timeout))
  ENDIF
 ENDIF
 IF ((dm2_sys_misc->cur_os != "AXP"))
  IF (findstring("/",trim(dcpf_loc),1,1) != size(trim(dcpf_loc)))
   SET dcpf_loc = concat(trim(dcpf_loc),"/")
  ENDIF
 ENDIF
 SET dm_err->eproc = concat("Validating input parameter, ",dcpf_loc)
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (findfile(dcpf_loc)=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Invalid parameter. ",dcpf_loc," does not exist ")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ((drr_clin_copy_data->temp_location="DM2NOTSET"))
  SET drr_clin_copy_data->temp_location = dcpf_loc
 ENDIF
 SET dm_err->eproc = "Check for Parfile count override row in DM_INFO"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="PARFILE TABLE COUNT"
   AND d.info_number BETWEEN 1 AND dcpf_parfile_table_cnt
  DETAIL
   dcpf_parfile_table_cnt = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Validating input parameter, ",dcpf_mode)
 CALL disp_msg(" ",dm_err->logfile,0)
 CASE (dcpf_mode)
  OF "REFERENCE":
   IF (dcpf_build_reference(null)=0)
    GO TO exit_program
   ENDIF
  OF "MIXED":
   IF (dcpf_build_mixed(null)=0)
    GO TO exit_program
   ENDIF
  OF "INDMIXED-REFERENCE":
   IF (dcpf_build_indmixed_ref(null)=0)
    GO TO exit_program
   ENDIF
   IF (dcpf_build_indmixed_import_parfile(null)=0)
    GO TO exit_program
   ENDIF
  OF "ALLV500":
   IF (dcpf_build_allv500(null)=0)
    GO TO exit_program
   ENDIF
  OF "PTS":
   IF (dcpf_build_pts(null)=0)
    GO TO exit_program
   ENDIF
  OF "RDDS":
   IF (dcpf_build_rdds(null)=0)
    GO TO exit_program
   ENDIF
  ELSE
   SET dm_err->err_ind = 1
   SET dm_err->emsg =
   "Invalid parameter.  First parameter must be either INDMIXED-REFERENCE, REFERENCE, or MIXED."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
 ENDCASE
 GO TO exit_program
 SUBROUTINE dcpf_build_allv500(null)
   DECLARE dba_par_file = vc WITH protect, noconstant("")
   DECLARE dba_dat_file = vc WITH protect, noconstant("")
   DECLARE dba_imp_par_file = vc WITH protect, noconstant("")
   DECLARE dba_resumable_name = vc WITH protect, noconstant("")
   DECLARE dba_tmp_cnt = i4 WITH protect, noconstant(0)
   DECLARE dba_include_drr_copy = i2 WITH protect, noconstant(0)
   SET dm_err->eproc = "Obtaining list of tables from USER_TABLES"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF (validate(drrr_responsefile_in_use,0)=1)
    IF ((drrr_rf_data->tgt_expimp_drr_shadow_tables="YES"))
     SET dba_include_drr_copy = 1
    ENDIF
   ENDIF
   SELECT
    IF (dba_include_drr_copy=1)
     FROM user_tables u
     WHERE  NOT ( EXISTS (
     (SELECT
      "x"
      FROM user_mviews um
      WHERE um.mview_name=u.table_name)))
      AND u.table_name != "MLOG$*"
      AND u.table_name != "*$R"
    ELSE
     FROM user_tables u
     WHERE  NOT ( EXISTS (
    ...
    ORDER BY u.table_name
    HEAD REPORT
     drr_all_tables_data->par_file_cnt = 0, drr_all_tables_data->cnt = 0
    DETAIL
     drr_all_tables_data->cnt = (drr_all_tables_data->cnt+ 1)
     IF (((mod(drr_all_tables_data->cnt,dcpf_parfile_table_cnt)=1) OR (dcpf_parfile_table_cnt=1)) )
      drr_all_tables_data->par_file_cnt = (drr_all_tables_data->par_file_cnt+ 1)
     ENDIF
     IF (mod(drr_all_tables_data->cnt,2000)=1)
      stat = alterlist(drr_all_tables_data->tbl,(drr_all_tables_data->cnt+ 1999))
     ENDIF
     drr_all_tables_data->tbl[drr_all_tables_data->cnt].table_name = u.table_name,
     drr_all_tables_data->tbl[drr_all_tables_data->cnt].par_group = drr_all_tables_data->par_file_cnt
    FOOT REPORT
     stat = alterlist(drr_all_tables_data->tbl,drr_all_tables_data->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(drr_all_tables_data)
   ENDIF
   IF ((dm2_sys_misc->cur_db_os="AXP"))
    SET dm_err->eproc = "Building par files for ALL V500 tables."
    CALL disp_msg(" ",dm_err->logfile,0)
    FOR (dba_tmp_cnt = 1 TO drr_all_tables_data->par_file_cnt)
      SET dba_imp_par_file = build(drr_clin_copy_data->temp_location,drr_clin_copy_data->
       imp_all_prefix,dba_tmp_cnt,".par")
      SET dba_par_file = build(drr_clin_copy_data->temp_location,drr_clin_copy_data->exp_all_prefix,
       dba_tmp_cnt,".par")
      SET dba_dat_file = build(drr_clin_copy_data->temp_location,drr_clin_copy_data->exp_all_prefix,
       dba_tmp_cnt,".dat")
      IF ((dm_err->debug_flag > 0))
       CALL echo(dba_imp_par_file)
       CALL echo(dba_par_file)
       CALL echo(dba_dat_file)
      ENDIF
      SET dm_err->eproc = concat("Now creating the par file: <",dba_par_file,">")
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SELECT INTO value(dba_par_file)
       drr_all_tables_data->tbl[d.seq].table_name
       FROM (dummyt d  WITH seq = drr_all_tables_data->cnt)
       WHERE (drr_all_tables_data->tbl[d.seq].par_group=dba_tmp_cnt)
       HEAD REPORT
        col 0, "TABLES=(", char_cnt = 8
       DETAIL
        char_cnt = ((char_cnt+ size(drr_all_tables_data->tbl[d.seq].table_name,1))+ 3)
        IF (char_cnt >= 512)
         row + 1, char_cnt = (size(drr_all_tables_data->tbl[d.seq].table_name,1)+ 3)
        ENDIF
        col + 0, '"', drr_all_tables_data->tbl[d.seq].table_name,
        '"', ","
       FOOT REPORT
        col- (1), ")", row + 1,
      ...
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET dm_err->eproc = concat("Now creating the dat file: <",dba_dat_file,">")
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SELECT INTO value(dba_dat_file)
       FROM (dummyt d  WITH seq = drr_all_tables_data->cnt)
       WHERE (drr_all_tables_data->tbl[d.seq].par_group=dba_tmp_cnt)
       DETAIL
        col 0, drr_all_tables_data->tbl[d.seq].table_name, row + 1
       WITH nocounter, format = variable, formfeed = none,
        maxrow = 1, maxcol = 512
      ;end select
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET dm_err->eproc = concat("Creating import parfile: ",dba_imp_par_file)
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SELECT INTO value(dba_imp_par_file)
       FROM (dummyt d  WITH seq = 1)
       DETAIL
        col 0, "buffer=100000", row + 1,
        col 0, "ignore=y ", row + 1,
        col 0, "commit=y", row + 1,
        col 0, "fromuser=v500", row + 1,
        col 0, "touser=v500"
        IF (dcpf_session_resumable=1)
         col 0, "resumable=y", row + 1,
         dba_resumable_name = build("resumable_name=",cnvtupper(drr_clin_copy_data->imp_all_prefix),
          dba_tmp_cnt), col 0, dba_resumable_name,
         row + 1, col 0, dcpf_resumable_time,
         row + 1
        ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcpf_build_pts(null)
   DECLARE dbp_par_file = vc WITH protect, noconstant("")
   DECLARE dbp_imp_par_file = vc WITH protect, noconstant("")
   SET dbp_imp_par_file = concat(drr_clin_copy_data->temp_location,drr_clin_copy_data->imp_pts_prefix,
    ".par")
   SET dbp_par_file = concat(drr_clin_copy_data->temp_location,drr_clin_copy_data->exp_pts_prefix,
    ".par")
   SET dm_err->eproc = "Building export par file for PTS."
   IF ((dm_err->err_ind > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Building import par file for PTS tables."
   IF ((dm_err->err_ind > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(dbp_imp_par_file)
    ...
    HEAD REPORT
     col 0, "fromuser=v500", row + 1,
     col 0, "touser= v500", row + 1,
     col 0, "buffer=1000000", row + 1,
     col 0, "ignore=y", row + 1,
     col 0, "commit=y", row + 1
     IF (dcpf_statistics_imp=1)
      col 0, "statistics=none", row + 1
     ENDIF
    WITH nocounter, format = variable, formfeed = none,
     maxrow = 1, maxcol = 512
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcpf_build_reference(null)
   DECLARE dbr_tmp_cnt = i4 WITH protect, noconstant(0)
   DECLARE dbr_parfile_name = vc WITH protect, noconstant("NOT SET")
   DECLARE dbr_tmp_par_group = i4 WITH protect, noconstant(1)
   DECLARE dbr_tbl_cnt = i4 WITH protect, noconstant(0)
   DECLARE dbr_datfile_name = vc WITH protect, noconstant("")
   DECLARE dbr_resumable_name = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Loading reference data tables."
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (drr_load_ref_table_data(0)=0)
    RETURN(0)
   ENDIF
   SET drr_clin_copy_data->ref_par_file_cnt = ceil((drr_ref_tables_data->cnt/ cnvtreal(
     dcpf_parfile_table_cnt)))
   FOR (dbr_tbl_cnt = 1 TO drr_ref_tables_data->cnt)
    IF (dbr_tmp_cnt=dcpf_parfile_table_cnt)
     SET dbr_tmp_cnt = 1
     SET dbr_tmp_par_group = (dbr_tmp_par_group+ 1)
    ELSE
     SET dbr_tmp_cnt = (dbr_tmp_cnt+ 1)
    ENDIF
    SET drr_ref_tables_data->tbl[dbr_tbl_cnt].par_group = dbr_tmp_par_group
   ENDFOR
   SET dbr_tmp_cnt = 0
   IF ((dm2_sys_misc->cur_db_os="AXP"))
    SET dm_err->eproc = "Building par files for reference tables."
    CALL disp_msg(" ",dm_err->logfile,0)
    FOR (dbr_tmp_cnt = 1 TO drr_clin_copy_data->ref_par_file_cnt)
      SET dbr_parfile_name = build(drr_clin_copy_data->temp_location,drr_clin_copy_data->
       exp_ref_prefix,dbr_tmp_cnt,".par")
      SET dbr_datfile_name = build(drr_clin_copy_data->temp_location,drr_clin_copy_data->
       exp_ref_prefix,dbr_tmp_cnt,".dat")
      SET dm_err->eproc = concat("Now creating the par file: <",dbr_parfile_name,">")
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SELECT INTO value(dbr_parfile_name)
       drr_ref_tables_data->tbl[d.seq].table_name
       FROM (dummyt d  WITH seq = drr_ref_tables_data->cnt)
       WHERE (drr_ref_tables_data->tbl[d.seq].par_group=dbr_tmp_cnt)
       HEAD REPORT
        col 0, "TABLES=(", char_cnt = 8
       DETAIL
        char_cnt = ((char_cnt+ size(drr_ref_tables_data->tbl[d.seq].table_name,1))+ 1)
        IF (char_cnt >= 512)
         row + 1, char_cnt = (size(drr_ref_tables_data->tbl[d.seq].table_name,1)+ 1)
        ENDIF
        col + 0, drr_ref_tables_data->tbl[d.seq].table_name, ","
       FOOT REPORT
        col- (1), ")", row + 1,
        col 0, "constraints=n", row + 1,
        col 0, "indexes=n", row + 1,
      ...
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET dm_err->eproc = concat("Now creating the dat file: <",dbr_datfile_name,">")
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SELECT INTO value(dbr_datfile_name)
       FROM (dummyt d  WITH seq = drr_ref_tables_data->cnt)
       WHERE (drr_ref_tables_data->tbl[d.seq].par_group=dbr_tmp_cnt)
       DETAIL
        col 0, drr_ref_tables_data->tbl[d.seq].table_name, row + 1
       WITH nocounter, format = variable, formfeed = none,
        maxrow = 1, maxcol = 512
      ;end select
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET dbr_parfile_name = ""
      SET dbr_parfile_name = build(drr_clin_copy_data->temp_location,drr_clin_copy_data->
       imp_ref_prefix,dbr_tmp_cnt,".par")
      SET dm_err->eproc = concat("Creating import parfile: ",dbr_parfile_name)
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SELECT INTO value(dbr_parfile_name)
       ...
       DETAIL
        ...
        IF (dcpf_session_resumable=1)
         col 0, "resumable=y", row + 1,
         dbr_resumable_name = build("resumable_name=",cnvtupper(drr_clin_copy_data->imp_ref_prefix),
          dbr_tmp_cnt), col 0, dbr_resumable_name,
         row + 1, col 0, dcpf_resumable_time,
         row + 1
        ENDIF
       WITH nocounter, format = stream, maxrow = 1,
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcpf_build_mixed(null)
   DECLARE dbm_par_file = vc WITH protect, noconstant("")
   SET dbm_par_file = concat(drr_clin_copy_data->temp_location,drr_clin_copy_data->
    mixed_tables_parfile_name)
   SET dm_err->eproc = concat("Loading data for mixed tables.")
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (drr_load_mixed_table_data(0)=0)
    RETURN(0)
   ENDIF
   IF ((dm2_sys_misc->cur_db_os="AXP"))
    SELECT INTO value(dbm_par_file)
     drr_mixed_tables_data->tbl[d.seq].table_name
     FROM (dummyt d  WITH seq = drr_mixed_tables_data->cnt)
     HEAD REPORT
      col 0, "TABLES=(", char_cnt = 8
     DETAIL
      char_cnt = ((char_cnt+ size(drr_mixed_tables_data->tbl[d.seq].table_name,1))+ 1)
      IF (char_cnt >= 512)
       row + 1, char_cnt = (size(drr_mixed_tables_data->tbl[d.seq].table_name,1)+ 1)
      ENDIF
      col + 0, drr_mixed_tables_data->tbl[d.seq].table_name, ","
     FOOT REPORT
      col- (1), ")"
     WITH nocounter, format = variable, formfeed = none,
      maxrow = 1, maxcol = 512
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcpf_build_indmixed_ref(null)
   DECLARE dbir_tbl_cnt = i4 WITH protect, noconstant(0)
   DECLARE dbir_where_cnt = i4 WITH protect, noconstant(0)
   DECLARE dbir_working_text = vc WITH protect, noconstant("NOT SET")
   DECLARE dbir_wclause_size = i4 WITH protect, noconstant(0)
   DECLARE dbir_parfile_name = vc WITH protect, noconstant("NOT SET")
   DECLARE dbir_export_ind = i2 WITH protect, noconstant(0)
   DECLARE dbir_text_pos = i4 WITH protect, noconstant(0)
   DECLARE dbir_prev_pos = i4 WITH protect, noconstant(0)
   DECLARE dbir_nextsp_pos = i4 WITH protect, noconstant(0)
   DECLARE dbir_lastsp_pos = i4 WITH protect, noconstant(0)
   DECLARE dbir_break_pos = i4 WITH protect, noconstant(0)
   DECLARE dbir_lastsp_found = i2 WITH protect, noconstant(0)
   IF (drr_load_mixed_table_data(0)=0)
    RETURN(0)
   ENDIF
   IF ((dm2_sys_misc->cur_db_os="AXP"))
    FOR (dbir_tbl_cnt = 1 TO drr_mixed_tables_data->cnt)
      SET dbir_export_ind = 0
      SET dcpf_text->line_cnt = 1
      SET stat = alterlist(dcpf_text->line,dcpf_text->line_cnt)
      SET dcpf_text->line[dcpf_text->line_cnt].text = "NOT_SET"
      SET dcpf_text->line[dcpf_text->line_cnt].text = concat("TABLES=",drr_mixed_tables_data->tbl[
       dbir_tbl_cnt].table_name)
      FOR (dbir_where_cnt = 1 TO drr_mixed_tables_data->tbl[dbir_tbl_cnt].where_clause_cnt)
        IF ((drr_mixed_tables_data->tbl[dbir_tbl_cnt].qual[dbir_where_cnt].process_type="EXPORT"))
         SET dbir_export_ind = 1
         SET dbir_working_text = concat("query=",trim(drr_mixed_tables_data->tbl[dbir_tbl_cnt].qual[
           dbir_where_cnt].where_clause,1))
         SET dbir_wclause_size = size(dbir_working_text,1)
         IF ((dm_err->debug_flag > 0))
          CALL echo(dm_err->asterisk_line)
          CALL echo(build("where clause size = <",dbir_wclause_size,">"))
          CALL echo(dm_err->asterisk_line)
          CALL echo(dbir_working_text)
         ENDIF
         IF (dbir_wclause_size > 512)
          SET dbir_text_pos = 0
          SET dbir_prev_pos = 0
          SET dbir_nextsp_pos = 0
          SET dbir_lastsp_pos = 0
          SET dbir_break_pos = 0
          WHILE (dbir_text_pos < dbir_wclause_size)
            SET dcpf_text->line_cnt = (dcpf_text->line_cnt+ 1)
            SET stat = alterlist(dcpf_text->line,dcpf_text->line_cnt)
            SET dbir_prev_pos = dbir_text_pos
            IF ((dbir_wclause_size > (dbir_prev_pos+ 511)))
             SET dbir_text_pos = (dbir_prev_pos+ 511)
             IF (substring(dbir_text_pos,1,dbir_working_text)=" ")
              SET dbir_break_pos = (dbir_text_pos - 1)
              SET dbir_text_pos = (dbir_text_pos+ 1)
             ELSEIF (substring((dbir_text_pos+ 1),1,dbir_working_text)=" ")
              SET dbir_break_pos = dbir_text_pos
              SET dbir_text_pos = (dbir_text_pos+ 1)
             ELSE
              SET dbir_lastsp_found = 0
              WHILE (dbir_lastsp_found=0)
                IF ((dm_err->debug_flag >= 1))
                 CALL echo(build("  dbir_nextsp_pos=<",dbir_nextsp_pos,">"))
                 CALL echo(build("  dbir_text_pos=<",dbir_text_pos,">"))
                ENDIF
                SET dbir_lastsp_pos = dbir_nextsp_pos
                SET dbir_nextsp_pos = findstring(" ",dbir_working_text,(dbir_nextsp_pos+ 1),0)
                IF (((dbir_lastsp_pos > dbir_nextsp_pos) OR (dbir_nextsp_pos >= dbir_text_pos)) )
                 SET dbir_lastsp_found = 1
                ENDIF
              ENDWHILE
              SET dbir_break_pos = (dbir_lastsp_pos - 1)
              SET dbir_text_pos = (dbir_lastsp_pos+ 1)
             ENDIF
             SET dcpf_text->line[dcpf_text->line_cnt].text = substring(dbir_prev_pos,(dbir_break_pos
               - dbir_prev_pos),dbir_working_text)
            ELSE
             SET dcpf_text->line[dcpf_text->line_cnt].text = substring(dbir_prev_pos,((
              dbir_wclause_size - dbir_prev_pos)+ 1),dbir_working_text)
             SET dbir_text_pos = dbir_wclause_size
            ENDIF
          ENDWHILE
         ELSE
          SET dcpf_text->line_cnt = 2
          SET stat = alterlist(dcpf_text->line,dcpf_text->line_cnt)
          SET dcpf_text->line[2].text = dbir_working_text
         ENDIF
        ENDIF
      ENDFOR
      IF (dbir_export_ind=0)
       SET dm_err->eproc = concat("Searching for an EXPORT process type for table: ",
        drr_mixed_tables_data->tbl[dbir_tbl_cnt].table_name)
       SET dm_err->emsg = "No EXPORT process type found.  Cannot create par file for table."
       SET dm_err->err_ind = 1
       RETURN(0)
      ENDIF
      SET dbir_parfile_name = cnvtlower(build(drr_clin_copy_data->temp_location,drr_mixed_tables_data
        ->tbl[dbir_tbl_cnt].prefix,"_exp.par"))
      SET dm_err->eproc = concat("Writing to file: ",dbir_parfile_name)
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SELECT INTO value(dbir_parfile_name)
       d.*
       FROM (dummyt d  WITH seq = dcpf_text->line_cnt)
       HEAD REPORT
        col 0, "constraints=n", row + 1,
        col 0, "indexes=n", row + 1,
        col 0, "triggers=n", row + 1,
        ...
        col 0, "statistics=none", row + 1
       DETAIL
        col 0, dcpf_text->line[d.seq].text
        IF ((d.seq < dcpf_text->line_cnt))
         row + 1
        ENDIF
       WITH nocounter, format = stream, maxrow = 1,
        maxcol = 512, formfeed = none
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcpf_build_indmixed_import_parfile(null)
   DECLARE dbip_parfile_name = vc WITH protect, noconstant("NOT SET")
   DECLARE dbip_tbl_cnt = i4 WITH protect, noconstant(0)
   DECLARE dbip_resumable_name = vc WITH protect, noconstant("NOT SET")
   IF ((dm2_sys_misc->cur_db_os="AXP"))
    FOR (dbip_tbl_cnt = 1 TO drr_mixed_tables_data->cnt)
      SET dbip_parfile_name = cnvtlower(build(drr_clin_copy_data->temp_location,drr_mixed_tables_data
        ->tbl[dbip_tbl_cnt].prefix,"_imp.par"))
      SET dm_err->eproc = concat("Creating import parfile: ",dbip_parfile_name)
      CALL disp_msg(" ",dm_err->logfile,0)
      SELECT INTO value(dbip_parfile_name)
       d.*
       FROM (dummyt d  WITH seq = 1)
       DETAIL
        col 0, "buffer=100000", row + 1,
        ...
        IF (dcpf_session_resumable=1)
         col 0, "resumable=y", row + 1,
         dbip_resumable_name = build("resumable_name=",cnvtupper(drr_mixed_tables_data->tbl[
           dbip_tbl_cnt].prefix),"_IMP_",cnvtupper(drr_mixed_tables_data->tbl[dbip_tbl_cnt].
           table_name)), col 0, dbip_resumable_name,
         row + 1, col 0, dcpf_resumable_time,
         row + 1
        ENDIF
       WITH nocounter, format = stream, maxrow = 1,
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcpf_build_rdds(null)
   DECLARE dbr_par_file = vc WITH protect, noconstant("")
   DECLARE dbr_dat_file = vc WITH protect, noconstant("")
   DECLARE dbr_imp_par_file = vc WITH protect, noconstant("")
   DECLARE dbr_resumable_name = vc WITH protect, noconstant("")
   DECLARE dbr_tmp_cnt = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Obtaining list of $R tables from USER_TABLES"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    ...
    ORDER BY u.table_name
    HEAD REPORT
     drr_rdds_tables_data->par_file_cnt = 0, drr_rdds_tables_data->cnt = 0
    DETAIL
     drr_rdds_tables_data->cnt = (drr_rdds_tables_data->cnt+ 1)
     IF (((mod(drr_rdds_tables_data->cnt,dcpf_parfile_table_cnt)=1) OR (dcpf_parfile_table_cnt=1)) )
      drr_rdds_tables_data->par_file_cnt = (drr_rdds_tables_data->par_file_cnt+ 1)
     ENDIF
     IF (mod(drr_rdds_tables_data->cnt,2000)=1)
      stat = alterlist(drr_rdds_tables_data->tbl,(drr_rdds_tables_data->cnt+ 1999))
     ENDIF
     drr_rdds_tables_data->tbl[drr_rdds_tables_data->cnt].table_name = u.table_name,
     drr_rdds_tables_data->tbl[drr_rdds_tables_data->cnt].par_group = drr_rdds_tables_data->
     par_file_cnt
    FOOT REPORT
     stat = alterlist(drr_rdds_tables_data->tbl,drr_rdds_tables_data->cnt)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->eproc = "No RDDS tables found in source domain"
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(drr_rdds_tables_data)
   ENDIF
   IF ((dm2_sys_misc->cur_db_os="AXP"))
    SET dm_err->eproc = "Building par files for $R tables."
    CALL disp_msg(" ",dm_err->logfile,0)
    FOR (dbr_tmp_cnt = 1 TO drr_rdds_tables_data->par_file_cnt)
      SET dbr_imp_par_file = build(drr_clin_copy_data->temp_location,drr_clin_copy_data->
       imp_rdds_prefix,dbr_tmp_cnt,".par")
      SET dbr_par_file = build(drr_clin_copy_data->temp_location,drr_clin_copy_data->exp_rdds_prefix,
       dbr_tmp_cnt,".par")
      SET dbr_dat_file = build(drr_clin_copy_data->temp_location,drr_clin_copy_data->exp_rdds_prefix,
       dbr_tmp_cnt,".dat")
      IF ((dm_err->debug_flag > 0))
       CALL echo(dbr_imp_par_file)
       CALL echo(dbr_par_file)
       CALL echo(dbr_dat_file)
      ENDIF
      SET dm_err->eproc = concat("Now creating the par file: <",dbr_par_file,">")
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SELECT INTO value(dbr_par_file)
       drr_rdds_tables_data->tbl[d.seq].table_name
       FROM (dummyt d  WITH seq = drr_rdds_tables_data->cnt)
       WHERE (drr_rdds_tables_data->tbl[d.seq].par_group=dbr_tmp_cnt)
       HEAD REPORT
        col 0, "TABLES=(", char_cnt = 8
       DETAIL
        char_cnt = ((char_cnt+ size(drr_rdds_tables_data->tbl[d.seq].table_name,1))+ 3)
        IF (char_cnt >= 512)
         row + 1, char_cnt = (size(drr_rdds_tables_data->tbl[d.seq].table_name,1)+ 3)
        ENDIF
        col + 0, '"', drr_rdds_tables_data->tbl[d.seq].table_name,
        '"', ","
       FOOT REPORT
        col- (1), ")", row + 1,
        col 0, "constraints=y", row + 1,
        col 0, "indexes=y", row + 1,
        col 0, "triggers=n", row + 1,
        col 0, "direct=y", row + 1,
        col 0, "compress=n", row + 1
       WITH nocounter, format = variable, formfeed = none,
      ...
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET dm_err->eproc = concat("Now creating the dat file: <",dbr_dat_file,">")
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SELECT INTO value(dbr_dat_file)
       FROM (dummyt d  WITH seq = drr_rdds_tables_data->cnt)
       WHERE (drr_rdds_tables_data->tbl[d.seq].par_group=dbr_tmp_cnt)
       DETAIL
        col 0, drr_rdds_tables_data->tbl[d.seq].table_name, row + 1
       WITH nocounter, format = variable, formfeed = none,
        maxrow = 1, maxcol = 512
      ;end select
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET dm_err->eproc = concat("Creating import parfile: ",dbr_imp_par_file)
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SELECT INTO value(dbr_imp_par_file)
       FROM (dummyt d  WITH seq = 1)
       DETAIL
        col 0, "buffer=100000", row + 1,
        col 0, "ignore=y ", row + 1,
        ...
        IF (dcpf_session_resumable=1)
         col 0, "resumable=y", row + 1,
         dbr_resumable_name = build("resumable_name=",cnvtupper(drr_clin_copy_data->imp_rdds_prefix),
          dbr_tmp_cnt), col 0, dbr_resumable_name,
         row + 1, col 0, dcpf_resumable_time,
         row + 1
        ENDIF
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "DM2_CREATE_PAR_FILES Completed."
 CALL final_disp_msg("dm2_create_parfiles")
END GO
