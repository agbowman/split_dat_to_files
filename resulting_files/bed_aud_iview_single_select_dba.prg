CREATE PROGRAM bed_aud_iview_single_select:dba
 ...
 DECLARE multi = f8 WITH public, noconstant(0.0)
 DECLARE multialphaandfreetext = f8 WITH public, noconstant(0.0)
 SELECT INTO "NL:"
  FROM code_value cv
  PLAN (cv
   WHERE cv.code_set=289
    AND cv.display_key IN ("MULTI", "MULTIALPHAANDFREETEXT")
    AND cv.active_ind=1)
  DETAIL
   IF (cv.display_key="MULTI")
    multi = cv.code_value
   ELSEIF (cv.display_key="MULTIALPHAANDFREETEXT")
    multialphaandfreetext = cv.code_value
   ENDIF
  WITH nocounter
 ;end select
 ...
 IF ((request->skip_volume_check_ind=0))
  SELECT INTO "nl:"
   ...
   PLAN (dta
    WHERE dta.default_result_type_cd IN (multi, multialphaandfreetext)
     AND dta.single_select_ind=0
    ...
    WHERE ar.reference_range_factor_id=rrf.reference_range_factor_id
     AND cnvtupper(ar.description) IN ("NONE", "NO COMPLICATIONS")
     AND ar.active_ind=1)
    JOIN (ese
 ...
 SELECT INTO "NL:"
  ...
  HEAD dta.task_assay_cd
   ...
   IF (dta.single_select_ind=0)
    tcnt = (tcnt+ 1), stat = alterlist(temp->tqual,tcnt), temp->tqual[tcnt].assay_display = dta
    .mnemonic,
    temp->tqual[tcnt].assay_code_value = dta.task_assay_cd, temp->tqual[tcnt].alpha_response = ar
    .description, temp->tqual[tcnt].result_type = cv1.display,
 ...
 SET reply->collist[2].header_text = "Assay Code Value"
 ...
 FOR (x = 1 TO tcnt)
   ...
   SET reply->rowlist[row_nbr].celllist[2].double_value = temp->tqual[x].assay_code_value
   SET reply->rowlist[row_nbr].celllist[3].string_value = temp->tqual[x].alpha_response
 ...
 SET reply->statlist[1].statistic_meaning = "IVIEWSINGLESELECT"
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("assays_missing_single_select_setting.csv")
 ENDIF
