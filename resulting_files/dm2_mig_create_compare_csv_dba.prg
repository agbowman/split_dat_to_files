CREATE PROGRAM dm2_mig_create_compare_csv:dba
 ...
 SUBROUTINE dvdr_get_verify2(null)
  ...
  RETURN(1)
 END ;Subroutine
 DECLARE dmccc_filename = vc WITH protect, noconstant("")
 DECLARE dmccc_plan_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccc_cnt = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_create_compare_csv",".log","dm2_mig_create_compare_csv")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_create_compare_csv"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dmccc_plan_id =  $1
 SET dmccc_filename =  $2
 SET dm_err->eproc = "Verifying plan_id."
 IF (((isnumeric(dmccc_plan_id)=0) OR (dmccc_plan_id <= 0)) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Incorrrect plan_id. Plan_id should be integer and > 0")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Verifying Csv filename."
 IF (((cnvtalphanum(dmccc_filename) != replace(replace(dmccc_filename,"_","",0),".","",2)) OR (((size
 (dmccc_filename) > 30) OR (((cnvtlower(dmccc_filename) != dmccc_filename) OR (dmccc_filename !=
 "dm2*.csv")) )) )) )
  SET dm_err->emsg = concat(
   "Incorrect csv filename. CSV file name must be alphanumeric allowing underscores;",
   " cannot exceed 30; must be lowercase; starts with dm2; ends in .csv")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dm2_findfile(dmccc_filename) > 0)
  SET dm_err->eproc = "Check that csv file does not exist in CCLUSERDIR"
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat(" The ",trim(dmccc_filename)," file exists in the CCLUSERDIR directory.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validate plan id"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  temp_cnt = count(*)
  FROM dm_install_plan di
  WHERE di.install_plan_id=dmccc_plan_id
  DETAIL
   dmccc_cnt = temp_cnt
  WITH nocounter
 ;end select
 ...
 IF (dmccc_cnt=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = build("No package was installed with the plan id :",dmccc_plan_id)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Create CSV file"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT DISTINCT INTO value(dmccc_filename)
  FROM dm2_ddl_ops_log dl
  WHERE dl.op_type IN ("CREATE TABLE", "CREATE INDEX - FOR TWO PHASED CHANGE (TEMP)")
   AND dl.run_id IN (
  (SELECT
   run_id
   FROM dm2_ddl_ops d
   WHERE (d.ocd=- ((1 * dmccc_plan_id)))))
   AND dl.status="COMPLETE"
  ORDER BY dl.table_name
  HEAD REPORT
   col 0, "OWNER,TABLE NAME", row + 1
  DETAIL
   CALL print(concat(trim("V500"),",",trim(dl.table_name))), row + 1
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No content was loaded into csv file")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 GO TO exit_script
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_mig_create_compare_csv completed succesfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_create_compare_csv")
END GO
