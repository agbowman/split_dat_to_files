CREATE PROGRAM dm2_manage_index
 ...
 SUBROUTINE ddf_context_lock_maint(dclm_mode,dclm_process,dclm_process_code,dclm_owner,
  dclm_table_name,dclm_object_id,dclm_lock_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_mi",".log","dm2_manage_index LOGFILE")=0)
  GO TO exit_program
 ENDIF
 DECLARE dmi_create_cmd = vc WITH protect, noconstant("")
 DECLARE dmi_orig_name = vc WITH protect, noconstant("")
 DECLARE dmi_new_name = vc WITH protect, noconstant("")
 DECLARE dmi_table_name = vc WITH protect, noconstant("")
 DECLARE dmi_ddl_cmd = vc WITH protect, noconstant("")
 DECLARE dmi_mode = vc WITH protect, noconstant("")
 DECLARE dmi_interm_name = vc WITH protect, noconstant("")
 DECLARE dmi_ind = i2 WITH protect, noconstant(0)
 DECLARE dmi_con_ind_name = vc WITH protect, noconstant("")
 DECLARE dmi_partitioned = vc WITH protect, noconstant("")
 DECLARE dmi_visibility = vc WITH protect, noconstant("")
 DECLARE dmi_cons_name = vc WITH protect, noconstant("")
 DECLARE dmi_interm_ind = i2 WITH protect, noconstant(0)
 DECLARE dmi_new_ind = i2 WITH protect, noconstant(0)
 DECLARE dmi_degree_of_parallel = i4 WITH protect, noconstant(0)
 DECLARE dmi_idx = i2 WITH protect, noconstant(0)
 DECLARE dmi_end_idx = i2 WITH protect, noconstant(0)
 DECLARE dmi_cmd_using_parallel_ind = i2 WITH protect, noconstant(0)
 DECLARE dmi_ind_exists(die_owner=vc,die_index_name=vc,die_exists_ind=i2(ref),die_partitioned=vc(ref),
  die_visibility=vc(ref)) = i2
 DECLARE dmi_con_check(dcc_cons_name=vc,dcc_index_name=vc(ref)) = i2
 DECLARE dmi_exec_ddl(ded_ddl_str=vc,ded_table_name=vc) = i2
 SET dmi_create_cmd = trim( $1)
 SET dmi_orig_name = trim( $2)
 SET dmi_new_name = trim( $3)
 SET dmi_table_name = trim( $4)
 SET dmi_mode = trim( $5)
 SET dmi_interm_name = trim( $6)
 SET dmi_cons_name = trim( $7)
 SET dm_err->eproc = "Validating input parameters"
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Beginning dm2_manage_index for ",dmi_orig_name,":",dmi_new_name)
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (findstring("PARALLEL (DEGREE",cnvtupper(dmi_create_cmd)) > 0)
  SET dmi_cmd_using_parallel_ind = 1
 ENDIF
 IF ((dm2_db_options->load_ind=1)
  AND dmi_cmd_using_parallel_ind=1)
  SET dm2_db_options->load_ind = 0
 ENDIF
 ...
 IF ((dm2_db_options->degree_of_parallel="NOT_SET"))
  SET dmi_degree_of_parallel = 0
 ELSE
  SET dmi_degree_of_parallel = cnvtint(dm2_db_options->degree_of_parallel)
 ENDIF
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 IF (dmi_degree_of_parallel < 2
  AND dmi_cmd_using_parallel_ind=1)
  SET dmi_idx = findstring("PARALLEL",cnvtupper(dmi_create_cmd),1,1)
  SET dmi_end_idx = (findstring(")",cnvtupper(dmi_create_cmd),dmi_idx,0)+ 1)
  SET dmi_create_cmd = replace(dmi_create_cmd,substring(dmi_idx,(dmi_end_idx - dmi_idx),
    dmi_create_cmd)," ",1)
  SET dmi_create_cmd = trim(dmi_create_cmd)
 ENDIF
 IF (dmi_mode="NEW")
  SET dmi_ind = 0
  IF (dmi_ind_exists(trim(currdbuser),dmi_new_name,dmi_ind,dmi_partitioned,dmi_visibility)=0)
   GO TO exit_program
  ENDIF
  IF (dmi_ind=0)
   IF (dmi_exec_ddl(dmi_create_cmd,dmi_table_name)=0)
    GO TO exit_program
   ENDIF
  ENDIF
  IF ((dm2_rdbms_version->level1 >= 11))
   EXECUTE dm2_finish_index trim(currdbuser), trim(dmi_table_name), trim(currdbuser),
   dmi_new_name
   IF ((dm_err->err_ind > 0))
    GO TO exit_program
   ENDIF
  ENDIF
 ENDIF
 IF (((dmi_mode="REPLACE") OR (dmi_mode="RENAME")) )
  IF (dmi_mode="REPLACE")
   SET dmi_ind = 0
   IF (dmi_ind_exists(trim(currdbuser),dmi_new_name,dmi_ind,dmi_partitioned,dmi_visibility)=0)
    GO TO exit_program
   ENDIF
   IF (dmi_ind=0)
    SET dmi_create_cmd = replace(dmi_create_cmd,"<!DMI_IND_NAME!>",dmi_new_name,1)
    IF (dmi_exec_ddl(dmi_create_cmd,dmi_table_name)=0)
     GO TO exit_program
    ENDIF
   ENDIF
  ...
  ENDIF
  SET dmi_ind = 0
  IF (dmi_ind_exists(trim(currdbuser),dmi_orig_name,dmi_ind,dmi_partitioned,dmi_visibility)=0)
   GO TO exit_program
  ENDIF
  IF (dmi_ind=1)
   SET dmi_ddl_cmd = concat("rdb asis(^drop index ",dmi_orig_name,"^) go")
   IF (dmi_exec_ddl(dmi_ddl_cmd,dmi_table_name)=0)
    GO TO exit_program
   ENDIF
  ENDIF
  SET dmi_ddl_cmd = concat("rdb asis(^alter index ",dmi_new_name," rename to ",dmi_orig_name,"^) go")
  IF (dmi_exec_ddl(dmi_ddl_cmd,dmi_table_name)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmi_mode="REBUILD")
  IF (dmi_con_check(dmi_cons_name,dmi_con_ind_name)=0)
   GO TO exit_program
  ENDIF
  IF ((dm_err->debug_flag >= 2))
   CALL echo(build("dmi_con_ind_name = ",dmi_con_ind_name))
  ENDIF
  IF (dmi_ind_exists(trim(currdbuser),dmi_new_name,dmi_new_ind,dmi_partitioned,dmi_visibility)=0)
   GO TO exit_program
  ENDIF
  IF ((dm_err->debug_flag >= 2))
   CALL echo(build("dmi_new_name = ",dmi_new_name))
  ENDIF
  IF (dmi_con_ind_name=dmi_new_name)
   IF (dmi_partitioned="YES"
    AND dmi_new_ind=1)
    GO TO exit_program
   ENDIF
  ENDIF
  IF (dmi_orig_name != "DM2NOTSET")
   IF (dmi_ind_exists(trim(currdbuser),dmi_orig_name,dmi_ind,dmi_partitioned,dmi_visibility)=0)
    GO TO exit_program
   ENDIF
   IF (dmi_ind=1)
    IF (dmi_con_ind_name != dmi_orig_name)
     IF (dmi_ind_exists(trim(currdbuser),dmi_interm_name,dmi_interm_ind,dmi_partitioned,
      dmi_visibility)=0)
      GO TO exit_program
     ENDIF
     IF (dmi_interm_ind=0)
      SET dmi_ddl_cmd = concat("rdb asis(^alter table ",dmi_table_name," modify constraint ",
       dmi_cons_name," using index ",
       dmi_orig_name,"^) go")
      IF (dmi_exec_ddl(dmi_ddl_cmd,dmi_table_name)=0)
       GO TO exit_program
      ELSE
       SET dmi_con_ind_name = dmi_orig_name
      ENDIF
     ENDIF
    ENDIF
    IF (dmi_new_ind=1)
     SET dmi_ddl_cmd = concat("rdb asis(^drop index ",dmi_new_name," online ^) go")
     IF (dmi_exec_ddl(dmi_ddl_cmd,dmi_table_name)=0)
      GO TO exit_program
     ENDIF
    ENDIF
    SET dmi_ddl_cmd = concat("rdb asis(^alter index ",dmi_orig_name," rename to ",dmi_new_name,
     "^) go")
    IF (dmi_exec_ddl(dmi_ddl_cmd,dmi_table_name)=0)
     GO TO exit_program
    ELSE
     SET dmi_new_ind = 1
     SET dmi_con_ind_name = dmi_new_name
    ENDIF
   ENDIF
  ENDIF
  IF (dmi_ind_exists(trim(currdbuser),dmi_interm_name,dmi_interm_ind,dmi_partitioned,dmi_visibility)=
  0)
   GO TO exit_program
  ENDIF
  IF (dmi_interm_ind=0)
   SET dmi_create_cmd = replace(dmi_create_cmd,"<!DMI_IND_NAME!>",dmi_interm_name,1)
   IF (dmi_exec_ddl(dmi_create_cmd,dmi_table_name)=0)
    GO TO exit_program
   ELSE
    SET dmi_interm_ind = 1
    SET dmi_visibility = "INVISIBLE"
   ENDIF
  ENDIF
  IF (dmi_visibility="INVISIBLE")
   EXECUTE dm2_finish_index trim(currdbuser), trim(dmi_table_name), trim(currdbuser),
   dmi_interm_name
   IF ((dm_err->err_ind > 0))
    GO TO exit_program
   ELSE
    SET dmi_visibility = "VISIBLE"
   ENDIF
  ENDIF
  IF (dmi_con_ind_name != dmi_interm_name)
   IF (dmi_interm_ind=1)
    SET dmi_ddl_cmd = concat("rdb asis(^alter table ",dmi_table_name," modify constraint ",
     dmi_cons_name," using index ",
     dmi_interm_name,"^) go")
  ...
  ENDIF
  IF (dmi_new_ind=1)
   CALL echo(build("dmi_interm_ind = ",dmi_interm_ind))
   IF (dmi_interm_ind=1)
    SET dmi_ddl_cmd = concat("rdb asis(^drop index ",dmi_new_name," online ^) go")
    IF (dmi_exec_ddl(dmi_ddl_cmd,dmi_table_name)=0)
     GO TO exit_program
  ...
  ENDIF
  IF (dmi_interm_ind=1)
   SET dmi_ddl_cmd = concat("rdb asis(^alter index ",dmi_interm_name," rename to ",dmi_new_name,
    "^) go")
   IF (dmi_exec_ddl(dmi_ddl_cmd,dmi_table_name)=0)
    GO TO exit_program
   ENDIF
  ENDIF
 ENDIF
 SUBROUTINE dmi_ind_exists(die_owner,die_index_name,die_exists_ind,die_partitioned,die_visibility)
   SET die_exists_ind = 0
   SET die_partitioned = ""
   SET die_visibility = ""
   SET dm_err->eproc = concat("Check if index ",die_index_name," exists")
   SELECT INTO "nl:"
    FROM dba_indexes d
    WHERE d.owner=die_owner
     AND d.index_name=die_index_name
    DETAIL
     die_exists_ind = 1, die_partitioned = d.partitioned, die_visibility = d.visibility
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmi_con_check(dcc_cons_name,dcc_index_name)
   SET dcc_index_name = ""
   SET dm_err->eproc = concat("Check if constraints index ",dcc_cons_name," exists")
   SELECT INTO "nl:"
    uc.index_name
    FROM user_constraints uc
    WHERE constraint_name=dcc_cons_name
    DETAIL
     dcc_index_name = uc.index_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmi_exec_ddl(ded_ddl_str,ded_table_name)
   DECLARE ded_blocking_str = vc WITH protect, noconstant("")
   DECLARE ded_in_parse_ind = i2 WITH protect, noconstant(0)
   DECLARE ded_cmd_rscbsy_retry_cnt = i4 WITH protect, noconstant(0)
   DECLARE ded_resource_busy_max_retry = i2 WITH protect, noconstant(cnvtint(dm2_db_options->
     resource_busy_maxretry))
   DECLARE ded_cmd_rscbsy_error = i2 WITH protect, noconstant(0)
   DECLARE ded_loop_var = i2 WITH protect, noconstant(0)
   SET ded_in_parse_ind = 1
   WHILE (ded_in_parse_ind=1
    AND ded_loop_var < ded_resource_busy_max_retry)
     IF ((dm2_rdbms_version->level1 >= 11))
      IF (ddf_check_in_parse(trim(currdbuser),ded_table_name,ded_in_parse_ind,ded_blocking_str)=0)
       RETURN(0)
      ENDIF
     ELSE
      SET ded_in_parse_ind = 0
     ENDIF
     SET ded_loop_var = (ded_loop_var+ 1)
     IF (ded_in_parse_ind=1)
      SET dm_err->eproc = concat("Pausing 30 seconds due to hard parse event detected on ",trim(
        currdbuser),".",ded_table_name)
      CALL disp_msg(" ",dm_err->logfile,0)
      SET dm_err->eproc = ded_blocking_str
      CALL disp_msg(" ",dm_err->logfile,0)
      CALL pause(30)
     ENDIF
     IF (ded_loop_var=ded_resource_busy_max_retry
      AND ded_in_parse_ind=1)
      SET dm_err->eproc = concat("Validating whether the table (",ded_table_name,
       ") is part of a long parsing query.")
      SET dm_err->emsg = concat("Parse retry count of ",build(ded_resource_busy_max_retry),
       " exceeded for ",ded_table_name)
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
   ENDWHILE
   SET ded_cmd_rscbsy_retry_cnt = 0
   SET ded_cmd_rscbsy_error = 1
   WHILE (ded_cmd_rscbsy_error=1
    AND ded_cmd_rscbsy_retry_cnt < ded_resource_busy_max_retry)
     SET dm_err->err_ind = 0
     SET ded_cmd_rscbsy_error = 0
     CALL dm2_push_cmd(ded_ddl_str,1)
     IF ((dm_err->err_ind > 0))
      IF (((findstring("ORA-00054",dm_err->emsg) > 0) OR (((findstring("ORA-14450",dm_err->emsg) > 0)
      ...
      ENDIF
     ENDIF
     IF (ded_cmd_rscbsy_error=1)
      SET ded_cmd_rscbsy_retry_cnt = (ded_cmd_rscbsy_retry_cnt+ 1)
      CALL echo(concat("Resource busy attempt ",build(ded_cmd_rscbsy_retry_cnt)," of ",build(
         ded_resource_busy_max_retry)))
      CALL echo("Pausing before retrying DDL command that failed due to resource busy.")
      CALL pause(3)
     ENDIF
   ENDWHILE
   IF (ded_cmd_rscbsy_error=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    CALL echo(concat("Reached resource busy retry limit of ",build(ded_resource_busy_max_retry)))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm2_manage_index"
 CALL final_disp_msg("dm2_mi")
END GO
