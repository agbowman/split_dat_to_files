CREATE PROGRAM bb_rdm_import_ques_depend:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bb_rdm_import_ques_depend"
 ...
 DECLARE cdf_meaning = c12
 DECLARE code_set = i4
 DECLARE success_flag = i2
 DECLARE smsg = vc
 DECLARE success_cnt = i2
 DECLARE module_cd = f8
 DECLARE process_cd = f8
 DECLARE question_cd = f8
 DECLARE depend_quest_cd = f8
 DECLARE response_cd = f8
 DECLARE response_code_set = f8
 DECLARE quest_mean = c12
 DECLARE last_quest_mean = c12
 DECLARE question_code_set = i4 WITH constant(1661)
 DECLARE module_code_set = i4 WITH constant(1660)
 DECLARE process_code_set = i4 WITH constant(1662)
 SET num_questions = size(requestin->list_0,5)
 IF (num_questions=0)
  SET smsg = "No records read from input file."
 ...
 WHILE (q_cnt <= num_questions)
   ...
   SET success_flag = 1
   SET quest_mean = requestin->list_0[q_cnt].question_mean
   IF (trim(quest_mean)="")
    SET success_flag = 0
    SET smsg = build("Question meaning blank, can't import->",q_cnt)
    CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
   ENDIF
   IF (quest_mean > ""
    AND quest_mean != trim(last_quest_mean))
    SET last_quest_mean = quest_mean
    SET code_set = 0
    SET code_set = question_code_set
    SET question_cd = 0.0
    SET cdf_meaning = requestin->list_0[q_cnt].question_mean
    SET question_cd = getcodevaluefromcdf(code_set,cdf_meaning)
    IF (question_cd=0)
     SET success_flag = 0
     SET smsg = build("Question lookup failed->",cdf_meaning)
     CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    ENDIF
    SET code_set = 0
    SET code_set = module_code_set
    SET module_cd = 0.0
    SET cdf_meaning = substring(1,12,requestin->list_0[q_cnt].module_mean)
    SET module_cd = getcodevaluefromcdf(code_set,cdf_meaning)
    IF (module_cd=0)
     SET success_flag = 0
     SET smsg = build("Module lookup failed->",cdf_meaning)
     CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    ENDIF
    SET code_set = 0
    SET code_set = process_code_set
    SET process_cd = 0.0
    SET cdf_meaning = requestin->list_0[q_cnt].process_mean
    SET process_cd = getcodevaluefromcdf(code_set,cdf_meaning)
    IF (process_cd=0)
     SET smsg = build("Process lookup failed->",cdf_meaning)
     CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
     SET success_flag = 0
    ENDIF
    SELECT INTO "NL:"
    ...
    IF (curqual > 0
     AND success_flag=1)
     SET smsg = build("Found Question_Dependency->",question_cd)
     CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    ELSEIF (curqual=0
     AND success_flag=1)
     INSERT  FROM question_dependency q
      SET q.question_cd = question_cd, q.updt_dt_tm = cnvtdatetime(curdate,curtime3), q.updt_id = 1,
       q.updt_task = 1, q.updt_applctx = 1, q.updt_cnt = 0
      WITH nocounter
     ;end insert
     IF (curqual=0)
      SET success_flag = 0
      SET smsg = build("Question_Dependency insert failed->",question_cd)
      CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
     ENDIF
    ENDIF
   ENDIF
   IF (success_flag=1)
    SET code_set = 0
    SET code_set = 1661
    SET depend_quest_cd = 0.0
    SET cdf_meaning = requestin->list_0[q_cnt].depend_quest_mean
    SET depend_quest_cd = getcodevaluefromcdf(code_set,cdf_meaning)
    IF (depend_quest_cd=0)
     SET success_flag = 0
     SET smsg = build("Depend_quest lookup failed->",cdf_meaning)
     CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    ENDIF
    SET response_code_set = cnvtreal(trim(requestin->list_0[q_cnt].response_code_set))
    SET response_cd = 0.0
    SET cdf_meaning = requestin->list_0[q_cnt].response_mean
    SET response_cd = getcodevaluefromcdf(response_code_set,cdf_meaning)
    IF (response_cd=0)
     SET success_flag = 0
     SET smsg = build("Response lookup failed->",cdf_meaning)
     CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    ENDIF
    SELECT INTO "NL:"
     d.question_cd
     FROM dependency d
     WHERE d.question_cd=question_cd
      AND d.depend_quest_cd=depend_quest_cd
      AND d.response_cd=response_cd
     DETAIL
      row + 0
     WITH nocounter
    ;end select
   ...
   IF (curqual > 0
    AND success_flag=1)
    SET smsg = build("Found Dependency->",depend_quest_cd)
    CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    SET success_flag = 2
   ELSEIF (curqual=0
    AND success_flag=1)
    INSERT  FROM dependency d
     SET d.module_cd = module_cd, d.process_cd = process_cd, d.question_cd = question_cd,
      d.response_cd = response_cd, d.depend_quest_cd = depend_quest_cd, d.updt_dt_tm = cnvtdatetime(
       curdate,curtime3),
      d.updt_id = 1, d.updt_task = 1, d.updt_applctx = 1,
      d.updt_cnt = 0
     WITH nocounter
    ;end insert
    IF (curqual=0)
     SET success_flag = 0
     SET smsg = build("Dependency insert failed for depend->",depend_quest_cd)
     CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    ENDIF
   ENDIF
   IF (success_flag=0)
   ...
   ELSEIF (success_flag=2)
    SET smsg = build("No update necessary for question number->",q_cnt)
    CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
