CREATE PROGRAM dm2_fix_cs_comp_child:dba
 ...
 DECLARE dfcc_min = f8
 DECLARE dfc_cnt = i4
 DECLARE dfc_action = vc
 DECLARE dfc_extra_cnt = i4
 DECLARE dfcc_table_cnt = i4
 DECLARE check_trig(sbr_suffix=vc) = i2
 FREE RECORD cs_dels
 RECORD cs_dels(
   1 qual[*]
     2 log_id = f8
 ...
 DECLARE fix_pk_where(sbr_table_name=vc,sbr_pk_where=vc) = vc
 SET dfcc_min =  $1
 CALL parser("SELECT INTO 'NL:' from dm_chg_log d where d.table_name in (",0)
 FOR (dfc_table_loop = 1 TO dfc_table_cnt)
  CALL parser(concat("'",cs_tables->qual[dfc_table_loop].table_name,"'"),0)
  IF (dfc_table_loop < dfc_table_cnt)
   CALL parser(",",0)
  ELSE
   CALL parser(")",0)
  ENDIF
 ENDFOR
 CALL parser(" and d.log_id >= dfcc_min and d.log_id < dfcc_min + dfc_batch",0)
 CALL parser(" and d.log_type in ('REFCHG', 'NORDDS', 'MERGED', 'ORPHAN')",0)
 CALL parser(" order by d.table_name head report dfc_cnt = 0 stat = alterlist(cs_dels->qual, 100)",0)
 CALL parser(" detail dfc_cnt = dfc_cnt + 1 if (mod(dfc_cnt,100) = 1 and dfc_cnt != 1)",0)
 CALL parser(
  " stat = alterlist(cs_dels->qual, dfc_cnt + 99) endif cs_dels->qual[dfc_cnt].log_id = d.log_id ",0)
 CALL parser(
  " cs_dels->qual[dfc_cnt].pk_where = d.pk_where cs_dels->qual[dfc_cnt].del_ind = d.delete_ind",0)
 CALL parser(
  " cs_dels->qual[dfc_cnt].table_name = d.table_name foot report stat = alterlist(cs_dels->qual,dfc_cnt)",
  0)
 CALL parser(" with nocounter go",1)
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "Error loading merge delete rows from the DM_CHG_LOG table into record structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
 ...
 ENDIF
 SET dfc_error_ind = 0
 FOR (dfc_loop = 1 TO dfc_cnt)
   IF ((cs_dels->qual[dfc_loop].del_ind=1))
    SET cs_dels->qual[dfc_loop].new_row_ind = 0
   ELSE
    SET dfc_action = fix_pk_where(cs_dels->qual[dfc_loop].table_name,cs_dels->qual[dfc_loop].pk_where
     )
    IF (dfc_action="BAD")
     SET cs_dels->qual[dfc_loop].new_row_ind = 1
    ENDIF
   ENDIF
 ENDFOR
 FOR (dfc_loop = 1 TO dfc_cnt)
  IF ((cs_dels->qual[dfc_loop].new_row_ind=1))
   UPDATE  FROM dm_chg_log d
    SET d.log_type = "BADLOG", d.updt_dt_tm = cnvtdatetime(curdate,curtime3)
    WHERE (d.log_id=cs_dels->qual[dfc_loop].log_id)
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc) != 0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Error changing log_type in dm_chg_log."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ROLLBACK
    SET readme_data->status = "F"
    SET readme_data->message = dm_err->emsg
    SET dfc_error_ind = 1
    SET dfc_loop = dfc_cnt
   ENDIF
   IF ((cs_dels->qual[dfc_loop].del_ind=0)
    AND dfc_error_ind=0)
    SET dfcc_table_cnt = locateval(dfc_extra_cnt,1,size(cs_tables->qual,5),cs_dels->qual[dfc_loop].
     table_name,cs_tables->qual[dfc_extra_cnt].table_name)
    IF ((cs_tables->qual[dfcc_table_cnt].trig_ind=0))
     SET cs_tables->qual[dfcc_table_cnt].trig_ind = check_trig(cs_tables->qual[dfcc_table_cnt].suffix
      )
     IF (check_error(dm_err->eproc) != 0)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Error selecting from user_triggers."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ROLLBACK
      SET readme_data->status = "F"
      SET readme_data->message = dm_err->emsg
     ...
     ENDIF
     IF ((cs_tables->qual[dfcc_table_cnt].trig_ind=0))
      SET cs_tables->qual[dfcc_table_cnt].add_trig_ind = 1
      EXECUTE dm2_add_chg_log_triggers cs_tables->qual[dfcc_table_cnt].table_name, "REFCHG"
      SET cs_tables->qual[dfcc_table_cnt].trig_ind = check_trig(cs_tables->qual[dfcc_table_cnt].
       suffix)
      IF (((check_error(dm_err->eproc) != 0) OR ((cs_tables->qual[dfcc_table_cnt].trig_ind=0))) )
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat("Trigger could not be added for the ",cs_tables->qual[dfcc_table_cnt
        ].table_name," table.")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       ROLLBACK
       SET readme_data->status = "F"
     ...
     ENDIF
    ENDIF
    CALL parser(concat("update into ",cs_dels->qual[dfc_loop].table_name," t",cs_tables->qual[
      dfcc_table_cnt].suffix),0)
    CALL parser(concat(" set t",cs_tables->qual[dfcc_table_cnt].suffix,".",cs_tables->qual[
      dfcc_table_cnt].qual[1].column_name," = t",
      cs_tables->qual[dfcc_table_cnt].suffix,".",cs_tables->qual[dfcc_table_cnt].qual[1].column_name),
     0)
    CALL parser(concat(cs_dels->qual[dfc_loop].pk_where," go"),1)
    IF (check_error(dm_err->eproc) != 0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Error performing update."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ROLLBACK
     SET readme_data->status = "F"
     SET readme_data->message = dm_err->emsg
  ...
  ENDIF
 ENDFOR
 SUBROUTINE check_trig(sbr_suffix)
   ...
   SELECT INTO "NL:"
    FROM dm2_user_objects d1,
     dm2_user_triggers d
    PLAN (d
     WHERE d.trigger_name=patstring(concat("REFCHG",sbr_suffix,"UPD*")))
     JOIN (d1
     WHERE d1.object_name=d.trigger_name
      AND d1.status="VALID")
    DETAIL
     sbr_return = 1
    WITH nocounter
   ...
   RETURN(sbr_return)
 END ;Subroutine
 SUBROUTINE fix_pk_where(sbr_table_name,sbr_pk_where)
   DECLARE end_dfc_cnt = i4
   DECLARE ret_pk_where = vc
   DECLARE fpw_col_cnt = i4
   DECLARE fpw_col_loop = i4
   DECLARE fpw_pk_cols = i4
   DECLARE fpw_table_cnt = i4
   DECLARE fpw_dummy_loop = i4
   DECLARE fpw_dummy_cnt = i4
   DECLARE fpw_and_cnt = i4
   DECLARE end_and_ind = i2
   SET fpw_table_cnt = locateval(fpw_dummy_loop,1,dfc_table_cnt,sbr_table_name,cs_tables->qual[
    fpw_dummy_loop].table_name)
   SET fpw_col_cnt = size(cs_tables->qual[fpw_table_cnt].qual,5)
   SET fpw_pk_cols = 0
   FOR (fpw_col_loop = 1 TO fpw_col_cnt)
     SET fpw_dummy_cnt = 0
     SET fpw_dummy_cnt = findstring(cs_tables->qual[fpw_table_cnt].qual[fpw_col_loop].column_name,
      sbr_pk_where,1,0)
     IF (fpw_dummy_cnt > 0)
      SET fpw_pk_cols = (fpw_pk_cols+ 1)
     ENDIF
   ENDFOR
   SET fpw_and_cnt = 0
   IF (fpw_col_cnt=fpw_pk_cols)
    SET fpw_dummy_loop = 1
    SET end_and_ind = 0
    WHILE (end_and_ind=0)
      SET fpw_dummy_cnt = 0
      SET fpw_dummy_cnt = findstring(" AND ",sbr_pk_where,fpw_dummy_loop,0)
      IF (fpw_dummy_cnt=0)
       SET end_and_ind = 1
      ELSE
       SET fpw_and_cnt = (fpw_and_cnt+ 1)
       SET fpw_dummy_loop = (fpw_dummy_cnt+ 3)
      ENDIF
    ENDWHILE
   ELSE
    RETURN("BAD")
   ENDIF
   IF ((fpw_and_cnt=(fpw_col_cnt - 1)))
    RETURN("GOOD")
   ELSE
    RETURN("BAD")
   ENDIF
