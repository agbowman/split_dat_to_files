CREATE PROGRAM dm_rmc_vers_tab:dba
 ...
 SUBROUTINE drcm_get_pk_str(dgps_rec,dgps_tab_name)
   ...
   RETURN(dgps_ret_str)
 END ;Subroutine
 CALL check_logfile("dm_rmc_vers_tab",".log","DM_RMC_VERS_TAB")
 SET dm_err->eproc = "Beginning dm_rmc_vers_tab"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (validate(request->table_name,"-99")="-99")
  FREE RECORD request
  RECORD request(
    1 table_name = vc
    1 table_suffix = vc
  )
  SET request->table_name = cnvtupper( $1)
  SET request->table_suffix = cnvtupper( $2)
 ENDIF
 ...
 ENDIF
 SET reply->status_data.status = upd_pkw_vers_tab(request->table_name,request->table_suffix)
