CREATE PROGRAM dm_extern_serv_orph_rows:dba
 ...
 SET reply->table_name = "SI_EXTERNAL_SERVICE"
 ...
 DECLARE ext_srv_cd = i4 WITH protect, noconstant(0)
 DECLARE v_errcode2 = i4 WITH protect, noconstant(0)
 DECLARE v_errmsg2 = vc WITH protect, noconstant("")
 SET ext_srv_cd = uar_get_code_by("MEANING",4002691,"OMIMSGVAL")
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(es.si_external_service_id)
   FROM si_external_service es
   WHERE es.external_service_type_cd=ext_srv_cd
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(es.si_external_service_id)
  FROM si_external_service es
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    es.rowid
    FROM si_external_service es
    WHERE es.si_external_service_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND es.external_service_type_cd=ext_srv_cd
     AND  NOT ( EXISTS (
    (SELECT
     1
     FROM si_service_property sp
     WHERE sp.prop_name="RULESET_SERVICE_NAMES"
      AND sp.prop_value=es.service_name)))
    DETAIL
     v_rows += 1
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = es.rowid
    WITH nocounter, maxqual(es,value(v_rows_left))
   ;end select
