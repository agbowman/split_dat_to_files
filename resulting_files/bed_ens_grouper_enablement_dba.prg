CREATE PROGRAM bed_ens_grouper_enablement:dba
 ...
 DECLARE checkname = vc WITH protect, noconstant("")
 DECLARE buildflag = i2 WITH protect, noconstant(0)
 DECLARE contentruleident = i4 WITH protect, noconstant(0)
 DECLARE drc_form_reltn_id = f8 WITH protect, noconstant(0.0)
 SELECT INTO "nl:"
  FROM dose_range_check drc
  PLAN (drc
   WHERE (drc.dose_range_check_id=request->dose_range_check_id)
    AND (drc.active_ind != request->drc_active_ind))
  DETAIL
   checkname = drc.dose_range_check_name, buildflag = drc.build_flag, contentruleident = drc
   .content_rule_identifier
  WITH nocounter
 ;end select
 IF (curqual=0)
  CALL bederror("Error001: Nothing to update dose_range_check.")
 ENDIF
 UPDATE  FROM dose_range_check drc
  SET drc.active_ind = request->drc_active_ind, drc.updt_applctx = reqinfo->updt_applctx, drc
   .updt_cnt = (drc.updt_cnt+ 1),
   drc.updt_dt_tm = cnvtdatetime(curdate,curtime3), drc.updt_id = reqinfo->updt_id, drc.updt_task =
   reqinfo->updt_task
  WHERE (drc.dose_range_check_id=request->dose_range_check_id)
  WITH nocounter
 ;end update
 CALL bederrorcheck("Error002: Failed to update dose_range_check.")
 CALL insert_version_row_dose_range_check_ver_table(request->dose_range_check_id)
 SELECT INTO "NL:"
  FROM drc_form_reltn dfr
  WHERE (dfr.dose_range_check_id=request->dose_range_check_id)
  DETAIL
   drc_form_reltn_id = dfr.drc_form_reltn_id
  WITH nocounter
 ;end select
 UPDATE  FROM drc_form_reltn dfr
  SET dfr.active_ind = request->drc_active_ind, dfr.updt_applctx = reqinfo->updt_applctx, dfr
   .updt_cnt = (dfr.updt_cnt+ 1),
   dfr.updt_dt_tm = cnvtdatetime(curdate,curtime3), dfr.updt_id = reqinfo->updt_id, dfr.updt_task =
   reqinfo->updt_task
  WHERE (dfr.dose_range_check_id=request->dose_range_check_id)
   AND dfr.drc_form_reltn_id=drc_form_reltn_id
  WITH nocounter
 ;end update
 CALL bederrorcheck("Error003: Failed to update drc_form_reltn.")
 CALL insert_version_row_drc_form_reltn_ver_table(drc_form_reltn_id)
#exit_script
