CREATE PROGRAM br_datamart_rpt_filter_config:dba
 ...
 SET readme_data->message = "Readme Failed: Starting <br_datamart_rpt_filter_config.prg> script"
 ...
 DECLARE topic_mean_exists = i2 WITH protect, noconstant(0)
 ...
 RECORD to_delete(
   1 list_0[*]
     2 report_filter_r_id = f8
 ) WITH protect
 SET stat = alterlist(br_existsinfo->list_0,cnt)
 IF (validate(requestin->list_0.topic_mean)=1)
  FOR (x = 1 TO cnt)
    IF ((requestin->list_0[x].topic_mean > ""))
     SET topic_mean_exists = 1
    ENDIF
  ENDFOR
 ENDIF
 IF (topic_mean_exists=1)
  SELECT INTO "nl:"
  ...
  IF (catcnt > 1)
   SET readme_data->status = "F"
   SET readme_data->message = concat("Multiple category ids found for topic mean ",requestin->list_0[
    d.seq].topic_mean)
   GO TO exit_script
  ENDIF
 ENDIF
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE bdr.report_mean=cnvtupper(requestin->list_0[d.seq].report_mean)
    AND ((bdr.br_datamart_category_id=cat_id) OR (topic_mean_exists=0)) )
   JOIN (bdf
 ...
 ELSE
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    JOIN (b
    WHERE (b.br_datamart_filter_id=br_existsinfo->list_0[d.seq].filterid)
     AND (b.br_datamart_report_id != br_existsinfo->list_0[d.seq].reportid))
    JOIN (r
    WHERE r.br_datamart_report_id=b.br_datamart_report_id)
    JOIN (f
    WHERE (f.br_datamart_filter_id=br_existsinfo->list_0[d.seq].filterid)
     AND f.br_datamart_category_id=r.br_datamart_category_id)
   DETAIL
    delcnt += 1, stat = alterlist(to_delete->list_0,delcnt), to_delete->list_0[delcnt].
    report_filter_r_id = b.br_datamart_report_filter_r_id
   WITH nocounter
  ;end select
  DELETE  FROM br_datamart_report_filter_r b,
   ...
   PLAN (d)
    JOIN (b
    WHERE (b.br_datamart_report_filter_r_id=to_delete->list_0[d.seq].report_filter_r_id))
   WITH nocounter
  ;end delete
 ENDIF
 INSERT  FROM br_datamart_report_filter_r b,
  ...
  PLAN (d
   WHERE (((br_existsinfo->list_0[d.seq].r_existsind=0)) OR (topic_mean_exists=1)) )
   JOIN (b)
  WITH nocounter
 ;end insert
 IF (error(errmsg,0) > 0)
