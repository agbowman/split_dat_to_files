CREATE PROGRAM dm2_mig_create_db_linksyn
 ...
 END ;Subroutine
 SET width = 132
 DECLARE dmcdl_ndx = i4 WITH protect, noconstant(0)
 DECLARE dmcdl_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmcdl_eval_exist(null) = i2
 DECLARE dmcdl_get_link_list(null) = i2
 DECLARE dmcdl_get_link_owners(null) = i2
 DECLARE dmcdl_get_link_ownerids(null) = i2
 DECLARE dmcdl_build_dblink_stmt(null) = i2
 DECLARE dmcdl_gendisp_dblink_rpt(null) = i2
 FREE RECORD dmcdl_link
 RECORD dmcdl_link(
   1 link_cnt = i4
   1 link[*]
     2 link_owner = vc
 ...
 IF (check_logfile("dm2_migcrtlnk",".log","dm2_mig_create_db_linksyn")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_create_db_linksyn"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dmr_prompt_retrieve_mig_data(0,1,1,1)=0)
  GO TO exit_program
 ENDIF
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 IF (dmcdl_get_link_list(null)=0)
  GO TO exit_program
 ENDIF
 IF ((dmcdl_link->link_cnt=0))
  SET dm_err->eproc = "No database link creation required."
  CALL disp_msg("",dm_err->logfile,0)
 ELSE
  IF (dmcdl_get_link_owners(null)=0)
   GO TO exit_program
  ENDIF
  IF (dmcdl_get_link_ownerids(null)=0)
   GO TO exit_program
  ENDIF
  IF (dmcdl_eval_exist(null)=0)
   GO TO exit_program
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echorecord(dmcdl_link)
  ENDIF
  SET dmcdl_ndx = locateval(dmcdl_ndx,1,dmcdl_link->link_cnt,0,dmcdl_link->link[dmcdl_ndx].
   link_in_tgt)
  IF (dmcdl_ndx=0)
   SET dm_err->eproc = "No database link creation required."
   CALL disp_msg("",dm_err->logfile,0)
  ELSE
   IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "SYS")) )
    IF (dmr_prompt_connect_data("SOURCE","SYS","CO")=0)
     GO TO exit_program
    ENDIF
   ENDIF
   IF (dmcdl_build_dblink_stmt(null)=0)
    GO TO exit_program
   ENDIF
   IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
    IF (dmr_prompt_connect_data("TARGET","V500","CO")=0)
    ...
    ENDIF
   ENDIF
   IF (dmcdl_gendisp_dblink_rpt(null)=0)
    GO TO exit_program
   ENDIF
  ENDIF
 ENDIF
 EXECUTE dm2_mig_create_synonyms
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 GO TO exit_program
 SUBROUTINE dmcdl_gendisp_dblink_rpt(null)
   DECLARE dgdr_rpt_file = vc WITH protect, noconstant(" ")
   DECLARE dgdr_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgdr_ndx = i4 WITH protect, noconstant(0)
   DECLARE dgdr_disp_header = i2 WITH protect, noconstant(0)
   IF (get_unique_file("dm2_mig_dblink",".rpt")=0)
    RETURN(0)
   ELSE
    SET dgdr_rpt_file = dm_err->unique_fname
   ENDIF
   SET dm_err->eproc =
   "Create report for user to see create statements for database links missing from TARGET."
   SELECT INTO value(dgdr_rpt_file)
    lowner = dmcdl_link->link[d.seq].link_owner
    FROM (dummyt d  WITH seq = value(dmcdl_link->link_cnt))
    PLAN (d
     WHERE (dmcdl_link->link[d.seq].link_in_tgt=0))
    ORDER BY lowner
    HEAD REPORT
     row + 2,
     CALL print(concat(
      "The database link creation statement(s) listed in this report need to be run against the ",
      dmr_mig_data->tgt_db_name," database on node:",dmr_mig_data->tgt_nodes[1].node_name,".")), row
      + 2,
     CALL print(concat(
      "In order for the database links to work correctly, please verify TNSNAMES.ORA",
      " entries exist for the following ORACLE_SID(s):")), row + 1,
     CALL print(
     "---------------------------------------------------------------------------------------------------"
     ),
     row + 2
     FOR (dgdr_cnt = 1 TO dmcdl_link->link_cnt)
       IF ((dmcdl_link->link[dgdr_cnt].link_in_tgt=0))
        col 2,
        CALL print(concat("'",dmcdl_link->link[dgdr_cnt].link_host,"' for ",dmcdl_link->link[dgdr_cnt
         ].link_owner,".",
         dmcdl_link->link[dgdr_cnt].link_name)), row + 1
       ENDIF
     ENDFOR
     row + 2,
     CALL print(
     "Once TNSNAMES.ORA entries have been verified, use the statements below to create the database links."
     ), row + 1
     IF ((((dm2_rdbms_version->level1=11)
      AND (dm2_rdbms_version->level2 >= 2)) OR ((dm2_rdbms_version->level1 > 11))) )
      CALL print(
      "Replace <user_provided_password> token before execute create database link statement."), row
       + 1
     ENDIF
     CALL print(
     "---------------------------------------------------------------------------------------------------"
    ...
    HEAD lowner
     IF (lowner="PUBLIC")
      CALL print(concat("Execute connected as user with privileges to create public database links:")
      ), row + 2
     ELSE
      CALL print(concat("Execute connected as ",dmcdl_link->link[d.seq].link_owner,":")), row + 2
     ENDIF
    DETAIL
     col 2,
     CALL print(dmcdl_link->link[d.seq].link_stmt), row + 1
    FOOT  lowner
     row + 1
    FOOT REPORT
     row + 2
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   IF (dm2_disp_file(dgdr_rpt_file,"Database Migration: Database Link Creation Request")=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmcdl_build_dblink_stmt(null)
   DECLARE dbds_ndx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Query for database link creation information in SOURCE."
   CALL disp_msg("",dm_err->logfile,0)
   IF (dm2_get_rdbms_version(null)=0)
    RETURN(0)
   ENDIF
   SELECT
    IF ((((dm2_rdbms_version->level1=10)
     AND (dm2_rdbms_version->level2 >= 2)) OR ((dm2_rdbms_version->level1 > 10))) )
     FROM (
      (
      (SELECT
       pwdstmt = sqlpassthru("rawtohex(l.passwordx)"), host = l.host, lname = l.name,
       uname = u.name, owner# = l.owner#, userid = l.userid
       FROM link$ l,
        user$ u
       WHERE l.owner#=u.user#
       WITH sqltype("C32000","C2000","C128","C128","I4",
         "C30")))
      a)
    ELSE
     FROM (
      (
      (SELECT
       pwdstmt = l.password, host = l.host, lname = l.name,
       uname = u.name, owner# = l.owner#, userid = l.userid
       FROM link$ l,
        user$ u
       WHERE l.owner#=u.user#
    ...
    INTO "nl:"
    DETAIL
     dbds_ndx = locateval(dbds_ndx,1,dmcdl_link->link_cnt,a.lname,dmcdl_link->link[dbds_ndx].
      link_name,
      a.owner#,dmcdl_link->link[dbds_ndx].link_owner_id)
     IF (dbds_ndx > 0)
      IF ((((dm2_rdbms_version->level1=11)
       AND (dm2_rdbms_version->level2 >= 2)) OR ((dm2_rdbms_version->level1 > 11))) )
       dmcdl_link->link[dbds_ndx].link_stmt = concat("create",evaluate(trim(a.uname),"PUBLIC",
         " public "," "),"database link ",trim(replace(dmcdl_link->link[dbds_ndx].link_name,".WORLD",
          " ",0),3)," connect to ",
        trim(a.userid),' identified by "<user_provided_password>"'," using '",trim(replace(a.host,
          ".WORLD"," ",0),3),"';")
      ELSEIF ((((dm2_rdbms_version->level1=10)
       ...
       dmcdl_link->link[dbds_ndx].link_stmt = concat("create",evaluate(trim(a.uname),"PUBLIC",
         " public "," "),"database link ",trim(replace(dmcdl_link->link[dbds_ndx].link_name,".WORLD",
          " ",0),3)," connect to ",
        trim(a.userid)," identified by values '",trim(a.pwdstmt),"' using '",trim(replace(a.host,
          ".WORLD"," ",0),3),
        "';")
      ELSE
       dmcdl_link->link[dbds_ndx].link_stmt = concat("create",evaluate(trim(a.uname),"PUBLIC",
         " public "," "),"database link ",trim(replace(dmcdl_link->link[dbds_ndx].link_name,".WORLD",
          " ",0),3)," connect to ",
        trim(a.userid)," identified by ",trim(a.pwdstmt)," using '",trim(replace(a.host,".WORLD"," ",
          0),3),
        "';")
      ENDIF
      dmcdl_link->link[dbds_ndx].link_host = a.host
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmcdl_get_link_ownerids(null)
   SET dm_err->eproc = "Obtain owner_ids for link owners."
   CALL disp_msg("",dm_err->logfile,0)
   IF ((dmcdl_link->link_cnt > 0))
    SELECT INTO "nl:"
     FROM dba_users@ref_data_link du,
      (dummyt d  WITH seq = value(dmcdl_link->link_cnt))
     PLAN (d)
      JOIN (du
      WHERE (du.username=dmcdl_link->link[d.seq].link_owner))
     DETAIL
      dmcdl_link->link[d.seq].link_owner_id = du.user_id
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmcdl_get_link_list(null)
   SET dm_err->eproc = "Gather list of database links missing from TARGET."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_db_links@ref_data_link a
    WHERE  NOT ( EXISTS (
    (SELECT
     "x"
     FROM dba_db_links d
     WHERE d.db_link=a.db_link)))
    DETAIL
     dmcdl_link->link_cnt = (dmcdl_link->link_cnt+ 1), stat = alterlist(dmcdl_link->link,dmcdl_link->
      link_cnt)
     IF (findstring(".WORLD",a.db_link,1,1)=0)
      dmcdl_link->link[dmcdl_link->link_cnt].link_owner = substring(1,(findstring(".",a.db_link,1,0)
        - 1),a.db_link), dmcdl_link->link[dmcdl_link->link_cnt].link_name = concat(substring((
        findstring(".",a.db_link,1,0)+ 1),(textlen(a.db_link) - findstring(".",a.db_link,1,0)),a
        .db_link),".WORLD")
     ELSE
      dmcdl_link->link[dmcdl_link->link_cnt].link_name = a.db_link
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmcdl_get_link_owners(null)
   DECLARE dglo_cnt = i4 WITH protect, noconstant(0)
   DECLARE dglo_ndx = i4 WITH protect, noconstant(0)
   DECLARE dglo_link = vc WITH protect, noconstant("")
   IF ((dm_err->debug_flag > 10))
    CALL echorecord(dmcdl_link)
   ENDIF
   SET dm_err->eproc = "Find database link owners."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_db_links@ref_data_link a
    DETAIL
     dglo_link = trim(a.db_link)
     FOR (dglo_cnt = 1 TO dmcdl_link->link_cnt)
       IF (locateval(dglo_ndx,1,dmcdl_link->link_cnt,dglo_link,dmcdl_link->link[dglo_ndx].link_name,
        a.owner,dmcdl_link->link[dglo_ndx].link_owner)=0)
        IF ((dmcdl_link->link[dglo_cnt].link_name=a.db_link)
         AND (dmcdl_link->link[dglo_cnt].link_owner=""))
         dmcdl_link->link[dglo_cnt].link_owner = a.owner
         IF (a.owner="PUBLIC")
          dmcdl_link->link[dglo_cnt].link_owner_id = 1
         ENDIF
        ENDIF
       ENDIF
     ENDFOR
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmcdl_eval_exist(null)
   SET dm_err->eproc = "Determine if database links are missing from TARGET."
   CALL disp_msg("",dm_err->logfile,0)
   IF ((dmcdl_link->link_cnt > 0))
    SELECT INTO "nl:"
     FROM dba_db_links dl,
      (dummyt d  WITH seq = value(dmcdl_link->link_cnt))
     PLAN (d
      WHERE (dmcdl_link->link[d.seq].link_name > " "))
      JOIN (dl
      WHERE (dl.db_link=dmcdl_link->link[d.seq].link_name)
       AND (dl.owner=dmcdl_link->link[d.seq].link_owner))
     DETAIL
      dmcdl_link->link[d.seq].link_in_tgt = 1
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_program
 IF ((dmr_mig_data->cur_db_type="SOURCE"))
  SET dm_err->eproc = "Terminating connection to database"
  CALL disp_msg(" ",dm_err->logfile,0)
  CALL dm2_push_cmd("free define oraclesystem go",1)
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_mig_create_db_linksyn completed."
 ENDIF
 CALL final_disp_msg("dm2_migcrtlnk")
END GO
