CREATE PROGRAM cps_get_all_alt_sel_item:dba
 FREE SET reply
 RECORD reply(
   1 synonym_count = i4
   1 synonym[*]
 ...
 SET loop_count = ceil((cnvtreal(request->cat_list_qual)/ batch_size))
 SET new_list_size = (loop_count * batch_size)
 SET stat = alterlist(request->cat_list,new_list_size)
 FOR (idx = (request->cat_list_qual+ 1) TO new_list_size)
   SET request->cat_list[idx].alt_sel_cat_id = request->cat_list[request->cat_list_qual].
   alt_sel_cat_id
 ENDFOR
 ...
 SELECT
  IF (ifacilityind=1
   AND (request->cat_flag=1))
   PLAN (d1
    WHERE initarray(nstart,evaluate(d1.seq,1,1,(nstart+ batch_size))))
    JOIN (al
    WHERE expand(inum,nstart,(nstart+ (batch_size - 1)),al.alt_sel_category_id,request->cat_list[inum
     ].alt_sel_cat_id,
     200)
     AND al.synonym_id > 0
     AND al.list_type=2)
  ...
  ELSEIF (ifacilityind=1
   AND (request->cat_flag=0))
   PLAN (d1
    WHERE initarray(nstart,evaluate(d1.seq,1,1,(nstart+ batch_size))))
    JOIN (al
    WHERE expand(inum,nstart,(nstart+ (batch_size - 1)),al.alt_sel_category_id,request->cat_list[inum
...
#exit_script
 IF (failed=false)
  ...
  ENDIF
 ENDIF
 SET script_version = "021 05/26/09 JT018805"
END GO
