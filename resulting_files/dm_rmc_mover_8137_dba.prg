CREATE PROGRAM dm_rmc_mover_8137:dba
 ...
 DECLARE cror_tab_name = vc
 DECLARE cust_tab_name_s = vc
 DECLARE cror_tab_name_s = vc
 DECLARE cro_tab_name = vc
 DECLARE cust_chg_tab_name = vc
 DECLARE cust_sect_id = f8
 DECLARE cust_object_id = f8
 DECLARE cust_object_id_s = f8
 DECLARE object_trans = vc
 DECLARE cust_sect_name = vc
 ...
 SET cror_tab_name = "CCL_REPORT_OBJECT_R"
 SET cro_tab_name = "CCL_REPORT_OBJECT"
 ...
 SET cror_tab_name_s = dm2_get_rdds_tname(cror_tab_name)
 SET cust_sect_id = cnvtreal(get_value(cust_tab_name,"SECTION_ID","FROM"))
 SET cust_sect_name = get_value(cust_tab_name,"SECTION_NAME","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_8137
 ENDIF
 IF (is_translated(cust_tab_name,"SECTION_ID")=0)
  SET cust_nomv_chk = select_merge_translate(cnvtstring(cust_sect_id),cust_tab_name)
  ...
  ENDIF
  SELECT INTO "NL:"
   FROM (parser(cror_tab_name_s) c)
   WHERE c.section_id=cust_sect_id
   DETAIL
    cust_object_id_s = c.object_id
   WITH nocounter
  ;end select
  ...
  IF (curqual=0)
   ...
   GO TO exit_8137
  ENDIF
  SET object_trans = select_merge_translate(cnvtstring(cust_object_id_s),cro_tab_name)
  IF ( NOT (isnumeric(object_trans)))
   SET cust_rpt_missing = report_missing(cro_tab_name,"OBJECT_ID",cust_object_id_s)
   ...
   IF (cust_err_val=0)
    ...
    SET dm2_ref_data_reply->error_msg = concat(cust_tab_name,
     " cannot translate because of the CCL_REPORT_OBJECT.OBJECT_ID.")
   ENDIF
   GO TO exit_8137
  ELSE
   SET cust_object_id = cnvtreal(object_trans)
  ENDIF
  SET fr_temp = locateval(fr_return,1,dm2_ref_data_doc->tbl_cnt,cror_tab_name,dm2_ref_data_doc->
   tbl_qual[fr_return].table_name)
  IF (fr_temp=0)
   SET fr_temp = temp_tbl_cnt
   SET fr_return = fill_rs("TABLE",cror_tab_name)
  ...
  SELECT INTO "NL:"
   ...
   WHERE c.section_name=cust_sect_name
    AND c.section_id IN (
   (SELECT
    r.section_id
    FROM ccl_report_object_r r
    WHERE r.object_id=cust_object_id
     AND  NOT (r.rep_obj_reltn_id IN (
    (SELECT
     r2.rep_obj_reltn_id
     FROM ccl_report_obj8139$r r2
     WHERE r2.rdds_delete_ind=1
      AND r2.rdds_status_flag < 9000
      AND (r2.rdds_source_env_id=dm2_ref_data_doc->env_source_id))))))
   HEAD REPORT
    ui_cnt = 0
   DETAIL
    ui_cnt = (ui_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,ui_cnt), ui_query_eval_rec->qual[
    ui_cnt].root_entity_attr = c.section_id
   WITH nocounter
  ;end select
  IF (error(dm2_ref_data_reply->error_msg,0) > 0)
   SET dm2_ref_data_reply->error_ind = 1
  ...
  IF (evaluate_id < 0)
   IF ((evaluate_id=- (3)))
    SET cust_to_val = get_seq(cust_tab_name,"SECTION_ID")
    CALL put_value(cust_tab_name,"SECTION_ID",cnvtstring(cust_to_val))
