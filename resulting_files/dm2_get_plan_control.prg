CREATE PROGRAM dm2_get_plan_control
 ...
 END ;Subroutine
 DECLARE dgpc_logfile_prefix = vc WITH protect, constant("dm2_get_plan_control")
 DECLARE dgpc_node = vc WITH protect, noconstant("")
 DECLARE dgpc_num = i4 WITH protect, noconstant(0)
 DECLARE dgpc_pos = i4 WITH protect, noconstant(0)
 DECLARE dgpc_tmp1 = i4 WITH protect, noconstant(0)
 DECLARE dgpc_tmp2 = i4 WITH protect, noconstant(0)
 DECLARE dgpc_str = vc WITH protect, noconstant("")
 DECLARE dgpc_plan_id = f8 WITH protect, noconstant(0.0)
 DECLARE dgpc_dpe_id = f8 WITH protect, noconstant(0.0)
 DECLARE dgpc_run_ident = vc WITH protect, noconstant("")
 DECLARE dgpc_run_nbr = i4 WITH protect, noconstant(0)
 DECLARE dgpc_ccl_ver = i4 WITH protect, constant((((cnvtint(currev) * 10000)+ (cnvtint(currevminor)
   * 100))+ cnvtint(currevminor2)))
 IF ((validate(request->object_list_cnt,- (1))=- (1))
  AND (validate(request->object_list_cnt,- (2))=- (2)))
  FREE RECORD request
  RECORD request(
    1 node = vc
    1 plan_id = f8
    1 object_list_cnt = i4
    1 object_list[*]
      2 object_name = vc
  )
 ENDIF
 IF ((validate(reply->object_list_cnt,- (1))=- (1))
  AND (validate(reply->object_list_cnt,- (2))=- (2)))
  RECORD reply(
    1 status_data
      2 status = c1
      2 subeventstatus[1]
 ...
 ENDIF
 IF ((validate(dgpc_data->cnt,- (1))=- (1))
  AND (validate(dgpc_data->cnt,- (2))=- (2)))
  FREE RECORD dgpc_data
  RECORD dgpc_data(
    1 cnt = i4
    1 qual[*]
      2 script_name = vc
      2 g_grant_exists_ind = i2
      2 g_rdboptdefault = i2
      2 g_dyn_query_rdbopt = i2
      2 g_outstring = vc
      2 g_all_equal_ind = i2
      2 g_all_grant_value = i4
      2 g_query_grant_value[50] = i4
      2 g_grant_cmd_full = vc
      2 b_baseline_exists_ind = i2
      2 b_sql_handle = vc
      2 b_sql_text = vc
      2 b_creator = vc
      2 b_description = vc
      2 b_created = vc
      2 b_last_modified = vc
      2 b_last_executed = vc
      2 b_enabled = vc
      2 b_accepted = vc
      2 b_plan_name = vc
      2 b_seq = i4
 ...
 ENDIF
 DECLARE dgpc_get_curr_cclgrants(null) = i2
 DECLARE dgpc_get_curr_baselines(null) = i2
 SET width = 132
 SET reply->status_data.status = "F"
 SET reply->object_list_cnt = 0
 IF (check_logfile(dgpc_logfile_prefix,".log","dm2_get_plan_control")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_get_plan_control"
 CALL disp_msg("",dm_err->logfile,0)
 SET dgpc_node = cnvtupper(trim(curnode))
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Node: ",cnvtupper(dgpc_node)))
 ENDIF
 ...
 IF ((dm2_rdbms_version->level1 < 11))
  ...
  SET dm_err->eproc = "Bypassing dm2_get_plan_control on oracle version < 11"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (dgpc_ccl_ver < 80604)
  SET reply->status_data.status = "S"
  SET dm_err->eproc = "Bypassing dm2_get_plan_control on CCL version < 8.6.4"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Check for bypass dm_info row"
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="BYPASS_EM_GET_PLAN_CONTROL"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
 ...
 IF (curqual > 0)
  SET reply->status_data.status = "S"
  SET dm_err->eproc = "Bypassing dm2_get_plan_control due to presence of dm_info row"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating request structure"
 IF ((request->plan_id=0.0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Plan ID must be set in the request structure"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dgpc_plan_id = request->plan_id
 IF ((((request->object_list_cnt=0)) OR (size(request->object_list,5)=0)) )
  SET dm_err->eproc =
  "No objects exist in the request structure. No plan control information will be retrieved."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (dgpc_get_curr_cclgrants(null)=0)
  GO TO exit_program
 ENDIF
 IF (dgpc_get_curr_baselines(null)=0)
  GO TO exit_program
 ENDIF
 IF ((dgpc_data->cnt > 0))
  SET dm_err->eproc = "Retrieve run identifier for current exeuction of plan id"
  SELECT INTO "nl:"
   FROM dm_process_event dpe
   WHERE dpe.dm_process_id IN (
   (SELECT
    dp.dm_process_id
    FROM dm_process dp
    WHERE dp.process_name=dpl_plan_control))
    AND dpe.message_txt=patstring(concat("RUN_IDENT: |",trim(cnvtstring(request->plan_id)),"|*|*"))
   HEAD REPORT
    dgpc_run_nbr = 0
   DETAIL
    dgpc_pos = findstring("|",trim(dpe.message_txt),13,0), dgpc_str = substring((dgpc_pos+ 1),(
     findstring("|",trim(dpe.message_txt),(dgpc_pos+ 1),0) - (dgpc_pos+ 1)),trim(dpe.message_txt))
    IF (cnvtint(dgpc_str) > dgpc_run_nbr)
     dgpc_run_nbr = cnvtint(dgpc_str)
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
  IF (curqual > 0)
   SET dgpc_run_ident = concat("RUN_IDENT: |",trim(cnvtstring(request->plan_id)),"|",trim(cnvtstring(
      (dgpc_run_nbr+ 1))),"|")
  ELSE
   SET dgpc_run_ident = concat("RUN_IDENT: |",trim(cnvtstring(request->plan_id)),"|1|")
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echo(dgpc_pos)
   CALL echo(dgpc_str)
   CALL echo(build(dgpc_run_nbr))
   CALL echo(dgpc_run_ident)
  ENDIF
  SET dm_err->eproc = "Insert plan control information into dm_process* tables"
  CALL disp_msg("",dm_err->logfile,0)
  FOR (dgpc_num = 1 TO dgpc_data->cnt)
    IF ((((dgpc_data->qual[dgpc_num].g_grant_exists_ind=1)) OR ((dgpc_data->qual[dgpc_num].
    b_baseline_exists_ind=1))) )
     SET dm_err->eproc = concat("Adding plan control information for ",dgpc_data->qual[dgpc_num].
      script_name)
     SET dm2_process_rs->process_name = dpl_plan_control
     SET dm2_process_event_rs->status = dpl_complete
     SET dm2_process_event_rs->install_plan_id = request->plan_id
     SET dm2_process_event_rs->message = concat(dgpc_run_ident,
      "Adding plan control information for: ",dgpc_data->qual[dgpc_num].script_name)
     IF (dm2_process_log_row(dpl_plan_control,dpl_snapshot,dpl_no_prev_id,1)=0)
      GO TO exit_program
     ENDIF
     SET dgpc_dpe_id = dm2_process_event_rs->dm_process_event_id
     CALL dm2_process_log_add_detail_text(dpl_script,dgpc_data->qual[dgpc_num].script_name)
     CALL dm2_process_log_add_detail_text(dpl_grant_exists,evaluate(dgpc_data->qual[dgpc_num].
       g_grant_exists_ind,1,"YES","NO"))
     CALL dm2_process_log_add_detail_text(dpl_bl_exists,evaluate(dgpc_data->qual[dgpc_num].
       b_baseline_exists_ind,1,"YES","NO"))
     IF ((dgpc_data->qual[dgpc_num].g_grant_exists_ind=1))
      CALL dm2_process_log_add_detail_text(dpl_grant_str,dgpc_data->qual[dgpc_num].g_outstring)
      CALL dm2_process_log_add_detail_text(dpl_grant_cmd,dgpc_data->qual[dgpc_num].g_grant_cmd_full)
     ENDIF
     IF ((dgpc_data->qual[dgpc_num].b_baseline_exists_ind=1))
      CALL dm2_process_log_add_detail_text(dpl_bl_query_nbr,dgpc_data->qual[dgpc_num].b_q_pos)
      CALL dm2_process_log_add_detail_text(dpl_bl_sql_handle,dgpc_data->qual[dgpc_num].b_sql_handle)
      CALL dm2_process_log_add_detail_text(dpl_bl_creator,dgpc_data->qual[dgpc_num].b_creator)
      CALL dm2_process_log_add_detail_text(dpl_bl_desc,dgpc_data->qual[dgpc_num].b_description)
      CALL dm2_process_log_add_detail_text(dpl_bl_enabled,dgpc_data->qual[dgpc_num].b_enabled)
      CALL dm2_process_log_add_detail_text(dpl_bl_accepted,dgpc_data->qual[dgpc_num].b_accepted)
      CALL dm2_process_log_add_detail_text(dpl_bl_plan_name,dgpc_data->qual[dgpc_num].b_plan_name)
      CALL dm2_process_log_add_detail_text(dpl_bl_created,dgpc_data->qual[dgpc_num].b_created)
      CALL dm2_process_log_add_detail_text(dpl_bl_last_mod,dgpc_data->qual[dgpc_num].b_last_modified)
      CALL dm2_process_log_add_detail_text(dpl_bl_last_exec,dgpc_data->qual[dgpc_num].b_last_executed
       )
      CALL dm2_process_log_add_detail_text(dpl_bl_sql_text,trim(substring(1,2000,dgpc_data->qual[
         dgpc_num].b_sql_text)))
      SET dgpc_tmp1 = size(dgpc_data->qual[dgpc_num].b_sql_text,1)
      SET dgpc_tmp2 = 1
      SET dgpc_pos = 1
      SET dgpc_str = ""
      WHILE (dgpc_tmp2 <= dgpc_tmp1)
        SET dgpc_str = trim(substring(dgpc_tmp2,1000,dgpc_data->qual[dgpc_num].b_sql_text))
        CALL dm2_process_log_add_detail_text(build(dpl_bl_sql_text,"_",dgpc_pos),dgpc_str)
        SET dgpc_pos = (dgpc_pos+ 1)
        SET dgpc_tmp2 = (dgpc_tmp2+ 1000)
      ENDWHILE
     ENDIF
     CALL dm2_process_log_dtl_row(dgpc_dpe_id,0)
     SET dm_err->eproc = concat("Adding object ",dgpc_data->qual[dgpc_num].script_name,
      "to the reply structure")
     SET reply->object_list_cnt = (reply->object_list_cnt+ 1)
     SET stat = alterlist(reply->object_list,reply->object_list_cnt)
     SET reply->object_list[reply->object_list_cnt].object_name = dgpc_data->qual[dgpc_num].
     script_name
     IF ((dgpc_data->qual[dgpc_num].g_grant_exists_ind=1))
      SET reply->object_list[reply->object_list_cnt].grant_ind = 1
     ENDIF
     IF ((dgpc_data->qual[dgpc_num].b_baseline_exists_ind=1))
      SET reply->object_list[reply->object_list_cnt].bl_ind = 1
     ENDIF
    ENDIF
  ENDFOR
 ENDIF
 ...
 SET reply->status_data.status = "S"
 GO TO exit_program
 SUBROUTINE dgpc_get_curr_cclgrants(null)
   ...
   DECLARE dgcc_ndx = i4 WITH protect, noconstant(0)
   DECLARE dgcc_cmd = vc WITH protect, noconstant("")
   ...
   SELECT INTO "nl:"
    object_name = substring(2,30,g.rest), rdboptval = evaluate(substring(274,6,g.data),"<sec1>",ichar
     (substring(248,1,g.data)),0), rdboptqry = evaluate(substring(274,6,g.data),"<sec1>",substring(
      249,25,g.data),fillstring(25,char(0)))
    ...
    WHERE g.platform="H0000"
     ...
     BETWEEN 0 AND 8
    HEAD REPORT
     dgpc_data->cnt = 0, stat = alterlist(dgpc_data->qual,dgpc_data->cnt), dgcc_val1 = 0,
     dgcc_val2 = 0, dgcc_val = 0, dgcc_num = 0,
     dgcc_num1 = 0, dgcc_num2 = 0, dgcc_tmp = - (1)
    DETAIL
     IF (locateval(dgcc_ndx,1,request->object_list_cnt,trim(object_name),cnvtupper(trim(request->
        object_list[dgcc_ndx].object_name))) > 0)
      dgcc_tmp = - (1), dgpc_data->cnt = (dgpc_data->cnt+ 1)
      IF (mod(dgpc_data->cnt,1000)=1)
       stat = alterlist(dgpc_data->qual,(dgpc_data->cnt+ 999))
      ENDIF
      dgpc_data->qual[dgpc_data->cnt].script_name = trim(object_name), dgcc_val1 = (band(rdboptval,
       240)/ 16), dgcc_val2 = band(rdboptval,15)
      IF ((( NOT (dgcc_val1 IN (0, 8))) OR ( NOT (dgcc_val2 IN (0, 8)))) )
       dgpc_data->qual[dgpc_data->cnt].g_grant_exists_ind = 1
      ENDIF
      dgpc_data->qual[dgpc_data->cnt].g_all_equal_ind = 1, dgpc_data->qual[dgpc_data->cnt].
      g_all_grant_value = 0, dgpc_data->qual[dgpc_data->cnt].g_rdboptdefault = dgcc_val2,
      dgpc_data->qual[dgpc_data->cnt].g_dyn_query_rdbopt = dgcc_val1, dgpc_data->qual[dgpc_data->cnt]
      .g_outstring = concat(dgpc_data->qual[dgpc_data->cnt].script_name,",",format(dgcc_val2,"#"),",",
       format(dgcc_val1,"#"),
       ",")
      FOR (dgcc_num = 1 TO 25)
        ...
        IF (((dgcc_val1 != dgcc_tmp) OR (dgcc_val2 != dgcc_tmp)) )
         dgpc_data->qual[dgpc_data->cnt].g_all_equal_ind = 0
        ENDIF
        dgpc_data->qual[dgpc_data->cnt].g_query_grant_value[dgcc_num1] = dgcc_val1, dgpc_data->qual[
        dgpc_data->cnt].g_query_grant_value[dgcc_num2] = dgcc_val2, dgpc_data->qual[dgpc_data->cnt].
        g_outstring = concat(dgpc_data->qual[dgpc_data->cnt].g_outstring,format(dgcc_val1,"#"),",",
         format(dgcc_val2,"#"))
        IF ((( NOT (dgcc_val1 IN (0, 8))) OR ( NOT (dgcc_val2 IN (0, 8)))) )
         dgpc_data->qual[dgpc_data->cnt].g_grant_exists_ind = 1
        ENDIF
        IF (dgcc_num != 25)
         dgpc_data->qual[dgpc_data->cnt].g_outstring = concat(dgpc_data->qual[dgpc_data->cnt].
          g_outstring,",")
        ENDIF
      ENDFOR
      IF ((dgpc_data->qual[dgpc_data->cnt].g_all_equal_ind=1))
       dgpc_data->qual[dgpc_data->cnt].g_all_grant_value = dgcc_val1
      ENDIF
      IF ((dgpc_data->qual[dgpc_data->cnt].g_grant_exists_ind=1))
       dgcc_cmd = concat("grant execute on ",dgpc_data->qual[dgpc_data->cnt].script_name,
        " to all with add,")
       IF ( NOT ((dgpc_data->qual[dgpc_data->cnt].g_rdboptdefault IN (0, 8))))
        dgcc_cmd = concat(dgcc_cmd," rdbopt=",build(dgpc_data->qual[dgpc_data->cnt].g_rdboptdefault))
       ENDIF
       IF ( NOT ((dgpc_data->qual[dgpc_data->cnt].g_dyn_query_rdbopt IN (0, 8))))
        dgcc_cmd = concat(dgcc_cmd,", rdbopt(0,",build(dgpc_data->qual[dgpc_data->cnt].
          g_dyn_query_rdbopt),")")
       ENDIF
       IF ((dgpc_data->qual[dgpc_data->cnt].g_all_equal_ind=1)
        AND  NOT ((dgpc_data->qual[dgpc_data->cnt].g_all_grant_value IN (0, 8))))
        dgcc_cmd = concat(dgcc_cmd,", rdbopt(1,",build(dgpc_data->qual[dgpc_data->cnt].
          g_all_grant_value),",50)")
       ELSE
        FOR (dgcc_num = 1 TO 50)
          IF ( NOT ((dgpc_data->qual[dgpc_data->cnt].g_query_grant_value[dgcc_num] IN (0, 8))))
           dgcc_cmd = concat(dgcc_cmd,", rdbopt(",build(dgcc_num),",",build(dgpc_data->qual[dgpc_data
             ->cnt].g_query_grant_value[dgcc_num]),
            ")")
          ENDIF
        ENDFOR
       ENDIF
       dgcc_cmd = concat(dgcc_cmd," go")
      ENDIF
      dgpc_data->qual[dgpc_data->cnt].g_grant_cmd_full = dgcc_cmd
     ENDIF
    FOOT REPORT
     stat = alterlist(dgpc_data->qual,dgpc_data->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   IF ((dm_err->debug_flag > 10))
    CALL echorecord(dgpc_data)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dgpc_get_curr_baselines(null)
   DECLARE dgcb_num = i4 WITH protect, noconstant(0)
   DECLARE dgcb_ndx = i4 WITH protect, noconstant(0)
   DECLARE dgcb_ndx2 = i4 WITH protect, noconstant(0)
   DECLARE dgcb_pos = i4 WITH protect, noconstant(0)
   DECLARE dgcb_pos2 = i4 WITH protect, noconstant(0)
   DECLARE dgcb_s_str = vc WITH protect, noconstant("")
   DECLARE dgcb_q_str = vc WITH protect, noconstant("")
   DECLARE dgcb_t_str = vc WITH protect, noconstant("")
   DECLARE dgcn_str = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Retrieving list of applied baselines from dba_sql_plan_baselines"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    ...
    HEAD dp.sql_handle
     dgcb_num = 0, dgcb_pos = 0
    DETAIL
     dgcn_str = trim(substring(1,200,dp.sql_text)), dgcb_pos = findstring("CCL<",dgcn_str,1,0),
     dgcb_s_str = "NONE",
     dgcb_q_str = "NONE"
     IF (dgcb_pos > 0)
      dgcb_t_str = substring((dgcb_pos+ 4),size(dgcn_str),dgcn_str), dgcb_pos = findstring(":",
       dgcb_t_str,1,0), dgcb_pos2 = findstring("> ",dgcb_t_str,1,1)
      IF (dgcb_pos > 0
       AND dgcb_pos2 > 0)
       dgcb_t_str = substring(1,(dgcb_pos2 - 1),dgcb_t_str), dgcb_pos2 = findstring(":q",cnvtlower(
         dgcb_t_str),1,1)
       IF (dgcb_pos2 > 0)
        dgcb_s_str = substring(1,(dgcb_pos - 1),dgcb_t_str), dgcb_q_str = trim(substring((dgcb_pos2+
          2),size(dgcb_t_str),dgcb_t_str))
       ENDIF
      ENDIF
     ENDIF
     IF (locateval(dgcb_ndx,1,request->object_list_cnt,dgcb_s_str,cnvtupper(trim(request->
        object_list[dgcb_ndx].object_name))) > 0)
      dgcb_num = locateval(dgcb_ndx2,1,dgpc_data->cnt,dgcb_s_str,dgpc_data->qual[dgcb_ndx2].
       script_name)
      IF (dgcb_num=0)
       dgpc_data->cnt = (dgpc_data->cnt+ 1), stat = alterlist(dgpc_data->qual,dgpc_data->cnt),
       dgcb_num = dgpc_data->cnt
      ENDIF
      dgpc_data->qual[dgcb_num].b_s_name = dgcb_s_str, dgpc_data->qual[dgcb_num].b_q_pos = dgcb_q_str,
      dgpc_data->qual[dgcb_num].b_baseline_exists_ind = 1,
      dgpc_data->qual[dgcb_num].b_accepted = trim(dp.accepted), dgpc_data->qual[dgcb_num].b_created
       = format(validate(dp.created,cnvtdate(01011900)),";;q"), dgpc_data->qual[dgcb_num].b_creator
       = trim(dp.creator),
      dgpc_data->qual[dgcb_num].b_description = evaluate(size(trim(dp.description)),0,"NONE",trim(dp
        .description)), dgpc_data->qual[dgcb_num].b_enabled = trim(dp.enabled), dgpc_data->qual[
      dgcb_num].b_last_executed = format(validate(dp.last_executed,cnvtdate(01011900)),";;q"),
      dgpc_data->qual[dgcb_num].b_last_modified = format(validate(dp.last_modified,cnvtdate(01011900)
        ),";;q"), dgpc_data->qual[dgcb_num].b_sql_handle = trim(dp.sql_handle), dgpc_data->qual[
      dgcb_num].b_sql_text = trim(dp.sql_text),
      dgpc_data->qual[dgcb_num].b_plan_name = dp.plan_name
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
...
#exit_program
 IF ((reply->status_data.status="S"))
  SET reply->status_data.subeventstatus[1].operationname = "dm2_get_plan_control"
  SET reply->status_data.subeventstatus[1].operationstatus = "S"
  SET reply->status_data.subeventstatus[1].targetobjectname = "Retrieve plan control information"
  SET reply->status_data.subeventstatus[1].targetobjectvalue =
  "dm2_get_plan_control completed successfully"
 ELSE
  SET reply->status_data.subeventstatus[1].operationname = "dm2_get_plan_control"
  SET reply->status_data.subeventstatus[1].operationstatus = "F"
  SET reply->status_data.subeventstatus[1].targetobjectname = dm_err->eproc
  SET reply->status_data.subeventstatus[1].targetobjectvalue = dm_err->emsg
 ENDIF
 SET dm_err->eproc = "Ending dm2_get_plan_control"
 CALL final_disp_msg(dgpc_logfile_prefix)
END GO
