CREATE PROGRAM dm2_frequent_gather_dbstats:dba
 ...
 SUBROUTINE dmai_get_cur_mod_act(dgcma_mod_out,dmgca_act_out)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dfgd_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dfgd_size_threshold = f8 WITH protect, noconstant(10)
 DECLARE dfgd_stale_threshold = i4 WITH protect, noconstant(30)
 DECLARE dfgd_last_mod_threshold = i4 WITH protect, noconstant(1)
 DECLARE dfgd_mode = vc WITH protect, noconstant("PREVIEW")
 DECLARE dfgd_str_pos = i4 WITH protect, noconstant(0)
 DECLARE dfgd_tab_idx = i4 WITH protect, noconstant(0)
 DECLARE dfgd_ind_idx = i4 WITH protect, noconstant(0)
 DECLARE dfgd_lock_available_ind = i2 WITH protect, noconstant(0)
 DECLARE dfgd_lock_obtained_ind = i2 WITH protect, noconstant(0)
 DECLARE dfgd_freq_gather_ind = i2 WITH protect, noconstant(1)
 DECLARE locndx = i4 WITH protect, noconstant(0)
 DECLARE forndx = i4 WITH protect, noconstant(0)
 DECLARE dfgd_module_name = vc WITH protect, constant("DM_STATS_PROCESS")
 DECLARE dfgd_action_name = vc WITH protect, constant("FREQ_DBSTATS")
 DECLARE dfgd_original_module = vc WITH protect, noconstant("")
 DECLARE dfgd_original_action = vc WITH protect, noconstant("")
 DECLARE dfgd_ins_del_pct_threshold = i4 WITH protect, noconstant(50)
 IF ((validate(dfgd_list->cnt,- (1))=- (1))
  AND (validate(dfgd_list->cnt,- (2))=- (2)))
  FREE RECORD dfgd_list
  RECORD dfgd_list(
    1 cnt = i4
    1 qual[*]
      2 dpq_id = f8
      ...
      2 tab_created = dq8
      2 tab_last_ddl = dq8
      2 tab_no_stats_ind = i2
      2 tab_new_col_ind = i2
      2 tab_num_rows = f8
      2 tab_stale_pct = i4
      2 freq_gather_ind = i2
      2 manual_inc_ind = i2
      2 tab_gather_operation = vc
      2 tab_ins_del_pct = i4
      2 ind_cnt = i4
      2 ind[*]
        3 dpq_id = f8
        3 index_name = vc
 ...
 ENDIF
 IF ((validate(dfgd_excl->cnt,- (1))=- (1))
  AND (validate(dfgd_excl->cnt,- (2))=- (2)))
  FREE RECORD dfgd_excl
  RECORD dfgd_excl(
    1 cnt = i4
    1 qual[*]
      2 tab_name = vc
 ...
 ENDIF
 IF ((validate(dfgd_incl->cnt,- (1))=- (1))
  AND (validate(dfgd_incl->cnt,- (2))=- (2)))
  FREE RECORD dfgd_incl
  RECORD dfgd_incl(
    1 cnt = i4
    1 qual[*]
      2 tab_name = vc
 ...
 ENDIF
 DECLARE dfgd_load_queued_list(null) = i2
 DECLARE dfgd_preview_rpt(null) = i2
 IF (check_logfile("dm2_fgd",".log","dm2_frequent_gather_dbstats LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_frequent_gather_dbstats"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dfgd_freq_gather_ind = 1
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dfgd_mode = cnvtupper(trim( $1))
 IF (((check_error(dm_err->eproc)=1) OR ( NOT (dfgd_mode IN ("PREVIEW", "GATHER")))) )
  SET dm_err->emsg = "Parameter usage: dm2_frequent_gather_dbstats <PREVIEW/GATHER>"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dmai_get_cur_mod_act(dfgd_original_module,dfgd_original_action)=0)
  GO TO exit_script
 ENDIF
 CALL dmai_set_mod_act(dfgd_module_name,dfgd_action_name)
 SET dm_err->eproc = "Check if dm2_frequent_gather_dbstats is already running"
 IF (ddf_check_lock("QUEUE DBSTATS LOCK","X",1,dfgd_lock_available_ind)=0)
  GO TO exit_script
 ENDIF
 IF (dfgd_lock_available_ind=1)
  IF (ddf_check_lock("FREQUENT STATS GATHER LOCK","X",1,dfgd_lock_available_ind)=0)
   GO TO exit_script
  ENDIF
  IF (dfgd_lock_available_ind=1)
   IF (ddf_get_lock("FREQUENT STATS GATHER LOCK","X",1,dfgd_lock_obtained_ind)=0)
    GO TO exit_script
   ENDIF
  ENDIF
 ENDIF
 IF (dfgd_lock_obtained_ind=0)
  SET dm_err->eproc =
  "Unable to obtain lock to run dm2_frequent_gather_dbstats. Process will be run next cycle. Exiting Script."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Load DM_INFO frequent gathering thresholds"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2GDBS:FREQUENT_GATHERING_THRESHOLD"
  ORDER BY di.info_domain, di.info_name
  DETAIL
   IF (di.info_name="TABLE_SIZE")
    dfgd_size_threshold = di.info_number
   ELSEIF (di.info_name="LAST_MOD")
    dfgd_last_mod_threshold = di.info_number
   ELSEIF (di.info_name="STALE_PCT")
    dfgd_stale_threshold = di.info_number
   ELSEIF (di.info_name="INS_DEL_PCT")
    dfgd_ins_del_pct_threshold = di.info_number
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
 ...
 CALL dm2_process_log_add_detail_text("TABLE_SIZE_THRESHOLD",cnvtstring(dfgd_size_threshold))
 CALL dm2_process_log_add_detail_text("LAST_MOD_THRESHOLD",cnvtstring(dfgd_last_mod_threshold))
 CALL dm2_process_log_add_detail_text("STALE_PCT_THRESHOLD",cnvtstring(dfgd_stale_threshold))
 CALL dm2_process_log_add_detail_text("INS_DEL_PCT_THRESHOLD",cnvtstring(dfgd_ins_del_pct_threshold))
 ...
 SET dfgd_log_id = dm2_process_event_rs->dm_process_event_id
 IF (ddf_autosuccess_init(null)=0)
  GO TO exit_script
 ELSEIF ((dgdt_prefs->autosuccess_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Load DM_INFO frequent stat gathering inclusions"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2GDBS:FREQUENT_GATHERING_INCLUDE"
  ORDER BY di.info_domain, di.info_name
  DETAIL
   dfgd_str_pos = 0, dfgd_str_pos = findstring(".",di.info_name,1,0)
   IF (dfgd_str_pos > 0)
    dfgd_incl->cnt = (dfgd_incl->cnt+ 1), stat = alterlist(dfgd_incl->qual,dfgd_incl->cnt), dfgd_incl
    ->qual[dfgd_incl->cnt].owner = cnvtupper(substring(1,(dfgd_str_pos - 1),di.info_name)),
    dfgd_incl->qual[dfgd_incl->cnt].tab_name = cnvtupper(trim(substring((dfgd_str_pos+ 1),size(di
 ...
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Load DM_INFO frequent stat gathering exclusions"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2GDBS:FREQUENT_GATHERING_EXCLUDE"
  ORDER BY di.info_domain, di.info_name
  DETAIL
   dfgd_str_pos = 0, dfgd_str_pos = findstring(".",di.info_name,1,0)
   IF (dfgd_str_pos > 0)
    dfgd_excl->cnt = (dfgd_excl->cnt+ 1), stat = alterlist(dfgd_excl->qual,dfgd_excl->cnt), dfgd_excl
    ->qual[dfgd_excl->cnt].owner = cnvtupper(substring(1,(dfgd_str_pos - 1),di.info_name)),
    dfgd_excl->qual[dfgd_excl->cnt].tab_name = cnvtupper(trim(substring((dfgd_str_pos+ 1),size(di
        .info_name),di.info_name)))
   ENDIF
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dfgd_excl)
  CALL echorecord(dfgd_incl)
 ENDIF
 SET dm_err->eproc = "Deleting dm_process_queue rows for frequent gathering candidate objects"
 CALL disp_msg(" ",dm_err->logfile,0)
 DELETE  FROM dm_process_queue dpq
 ...
 ENDIF
 COMMIT
 EXECUTE dm2_queue_dbstats "ALL", "*", "*",
 "*", "*", "QUEUE_STALE"
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 CALL dmai_set_mod_act(dfgd_module_name,dfgd_action_name)
 IF (dfgd_load_queued_list(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Selecting candidates for frequent stats gathering."
 CALL disp_msg(" ",dm_err->logfile,0)
 FOR (dfgd_tab_idx = 1 TO dfgd_list->cnt)
   IF ((dfgd_list->qual[dfgd_tab_idx].tab_size < dfgd_size_threshold)
    AND (((((dfgd_list->qual[dfgd_tab_idx].tab_stale_pct >= dfgd_stale_threshold)
    AND (dfgd_list->qual[dfgd_tab_idx].tab_ins_del_pct < dfgd_ins_del_pct_threshold)) OR ((dfgd_list
   ->qual[dfgd_tab_idx].tab_no_stats_ind=1))) ) OR ((dfgd_list->qual[dfgd_tab_idx].tab_new_col_ind=1)
   )) )
    SET dfgd_list->qual[dfgd_tab_idx].freq_gather_ind = 1
    CALL echo(concat(dfgd_list->qual[dfgd_tab_idx].tab_owner,".",dfgd_list->qual[dfgd_tab_idx].
      tab_name," has been identified as a candidate for frequent stats gathering"))
   ENDIF
   SET dm_err->eproc = "Identifying candidates that have been included manually."
   IF (locateval(locndx,1,dfgd_incl->cnt,dfgd_list->qual[dfgd_tab_idx].tab_owner,dfgd_incl->qual[
    locndx].owner,
    dfgd_list->qual[dfgd_tab_idx].tab_name,dfgd_incl->qual[locndx].tab_name) > 0)
    SET dfgd_list->qual[dfgd_tab_idx].freq_gather_ind = 1
    SET dfgd_list->qual[dfgd_tab_idx].manual_inc_ind = 1
    CALL echo(concat(dfgd_list->qual[dfgd_tab_idx].tab_owner,".",dfgd_list->qual[dfgd_tab_idx].
      tab_name," has been manually included for frequent stats gathering"))
   ENDIF
   IF ((dfgd_list->qual[dfgd_tab_idx].freq_gather_ind=1))
    SET dm_err->eproc = concat("Identifying indexes for table: ",dfgd_list->qual[dfgd_tab_idx].
     tab_name)
    SELECT INTO "nl:"
     FROM dba_indexes di
     WHERE (di.table_owner=dfgd_list->qual[dfgd_tab_idx].tab_owner)
      AND (di.table_name=dfgd_list->qual[dfgd_tab_idx].tab_name)
      AND di.index_type IN ("NORMAL", "FUNCTION-BASED NORMAL")
     ORDER BY di.table_owner, di.table_name, di.index_name
     HEAD REPORT
      icnt = 0
     DETAIL
      icnt = (icnt+ 1), stat = alterlist(dfgd_list->qual[dfgd_tab_idx].ind,icnt), dfgd_list->qual[
      dfgd_tab_idx].ind[icnt].index_name = trim(di.index_name),
      dfgd_list->qual[dfgd_tab_idx].ind[icnt].index_owner = trim(di.owner), dfgd_list->qual[
      dfgd_tab_idx].ind_cnt = icnt
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ENDIF
    IF ((dfgd_list->qual[dfgd_tab_idx].ind_cnt > 0))
     SET dm_err->eproc = concat("Loading queued indexes for table: ",dfgd_list->qual[dfgd_tab_idx].
      tab_name)
     CALL disp_msg(" ",dm_err->logfile,0)
     SELECT INTO "nl:"
      FROM dm_process_queue dpq,
       (dummyt d  WITH seq = value(dfgd_list->qual[dfgd_tab_idx].ind_cnt))
      PLAN (d
       WHERE d.seq > 0)
       JOIN (dpq
       WHERE dpq.process_type=dpq_statistics
        AND dpq.object_type="INDEX"
        AND (dpq.owner_name=dfgd_list->qual[dfgd_tab_idx].ind[d.seq].index_owner)
        AND (dpq.object_name=dfgd_list->qual[dfgd_tab_idx].ind[d.seq].index_name))
      ORDER BY dpq.priority DESC, dpq.gen_dt_tm
      DETAIL
       dfgd_list->qual[dfgd_tab_idx].ind[d.seq].dpq_id = dpq.dm_process_queue_id
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc) != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    IF (dfgd_mode="GATHER")
     SET dm_err->eproc = "Updating process_type for frequent gathering candidate tables"
     CALL disp_msg(" ",dm_err->logfile,0)
     UPDATE  FROM dm_process_queue dpq
      SET dpq.process_type = dpq_freq_statistics
      WHERE dpq.process_type=dpq_statistics
       AND (dpq.dm_process_queue_id=dfgd_list->qual[dfgd_tab_idx].dpq_id)
       AND dpq.process_status="QUEUED"
      WITH nocounter
     ;end update
     IF (check_error(dm_err->eproc) != 0)
      ROLLBACK
     ...
     ENDIF
     IF (curqual > 0
      AND (dfgd_list->qual[dfgd_tab_idx].ind_cnt > 0))
      SET dm_err->eproc = "Updating process_type for frequent gathering candidate indexes"
      UPDATE  FROM dm_process_queue dpq,
        (dummyt d  WITH seq = value(dfgd_list->qual[dfgd_tab_idx].ind_cnt))
       SET dpq.process_type = dpq_freq_statistics
       PLAN (d
        ...
        JOIN (dpq
        WHERE dpq.process_type=dpq_statistics
         AND (dpq.dm_process_queue_id=dfgd_list->qual[dfgd_tab_idx].ind[d.seq].dpq_id)
         AND dpq.process_status="QUEUED")
       WITH nocounter
      ;end update
      IF (check_error(dm_err->eproc) != 0)
 ...
 ENDFOR
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(dfgd_list)
 ENDIF
 IF (dfgd_mode="PREVIEW")
  IF (dfgd_preview_rpt(null)=0)
   GO TO exit_script
  ENDIF
 ELSE
  IF ((dgdt_prefs->context_lock_ind=- (1)))
   ...
   ENDIF
  ENDIF
  EXECUTE dm2_process_queue_runner dpq_freq_statistics
  IF ((dm_err->err_ind=1))
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  CALL dmai_set_mod_act(dfgd_module_name,dfgd_action_name)
  FOR (forndx = 1 TO dfgd_list->cnt)
    IF ((dfgd_list->qual[forndx].freq_gather_ind=1))
     EXECUTE dm2_publish_dbstats dfgd_list->qual[forndx].tab_owner, dfgd_list->qual[forndx].tab_name,
     "PUBLISH"
     IF ((dm_err->err_ind=1))
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_script
 ...
 ENDIF
 GO TO exit_script
 SUBROUTINE dfgd_load_queued_list(null)
   DECLARE locndx = i4 WITH protect, noconstant(0)
   DECLARE dlgl_mode_where = vc WITH protect, noconstant(" 1=1 ")
   DECLARE dlgl_num = i4 WITH protect, noconstant(0)
   DECLARE dlgl_tmp_size = f8 WITH protect, noconstant(0.0)
   DECLARE dlgl_tot_mod = i4 WITH protect, noconstant(0)
   DECLARE dlgl_stale_pct = f8 WITH protect, noconstant(0.0)
   DECLARE dlgl_total_size = f8 WITH protect, noconstant(0.0)
   SET dm_err->eproc = "Loading queued tables from DM_PROCESS_QUEUE"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_process_queue dpq
    WHERE dpq.process_status=dpq_queued
     AND dpq.process_type=dpq_statistics
     AND dpq.object_type="TABLE"
    ORDER BY dpq.priority DESC, dpq.gen_dt_tm
    DETAIL
     locndx = 0, locndx = locateval(locndx,1,dfgd_excl->cnt,dpq.owner_name,dfgd_excl->qual[locndx].
      owner,
      dpq.object_name,dfgd_excl->qual[locndx].tab_name)
     IF (locndx=0)
      dfgd_list->cnt = (dfgd_list->cnt+ 1)
      IF (mod(dfgd_list->cnt,50)=1)
       stat = alterlist(dfgd_list->qual,(dfgd_list->cnt+ 49))
      ENDIF
      dfgd_list->qual[dfgd_list->cnt].dpq_id = dpq.dm_process_queue_id, dfgd_list->qual[dfgd_list->
      cnt].tab_name = dpq.object_name, dfgd_list->qual[dfgd_list->cnt].tab_owner = dpq.owner_name,
      dfgd_list->qual[dfgd_list->cnt].tab_size = 0.0
     ENDIF
    FOOT REPORT
     stat = alterlist(dfgd_list->qual,dfgd_list->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dfgd_list->cnt > 0))
    SET dm_err->eproc = "Idenitfying size of tables"
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dba_segments ds,
      (dummyt d  WITH seq = value(dfgd_list->cnt))
     PLAN (d
      WHERE d.seq > 0)
      JOIN (ds
      WHERE ds.segment_type IN ("TABLE", "TABLE PARTITION", "TABLE SUBPARTITION")
       AND (ds.owner=dfgd_list->qual[d.seq].tab_owner)
       AND (ds.segment_name=dfgd_list->qual[d.seq].tab_name))
     ORDER BY ds.owner, ds.segment_name
     HEAD ds.segment_name
      dlgl_tmp_size = 0.0, dfgd_list->qual[d.seq].tab_size = 0.0, dlgl_total_size = 0.0
     DETAIL
      dlgl_total_size = (cnvtreal(dlgl_total_size)+ cnvtreal(ds.bytes))
     FOOT  ds.segment_name
      dlgl_tmp_size = (((cnvtreal(dlgl_total_size)/ 1024.0)/ 1024.0)/ 1024.0), dfgd_list->qual[d.seq]
      .tab_size = cnvtreal(dlgl_tmp_size)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Identifying if stats exist for tables"
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     analyze_tab_ind = nullind(dts.last_analyzed)
     FROM dba_tab_statistics dts,
      (dummyt d  WITH seq = value(dfgd_list->cnt))
     PLAN (d
      WHERE d.seq > 0)
      JOIN (dts
      WHERE (dts.owner=dfgd_list->qual[d.seq].tab_owner)
       AND (dts.table_name=dfgd_list->qual[d.seq].tab_name))
     ORDER BY dts.owner, dts.table_name
     DETAIL
      IF (analyze_tab_ind=1)
       dfgd_list->qual[d.seq].tab_no_stats_ind = 1
      ELSE
       dfgd_list->qual[d.seq].tab_num_rows = dts.num_rows
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc) != 0)
     ...
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Identifying staleness of tables"
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM all_tab_modifications atm,
      (dummyt d  WITH seq = value(dfgd_list->cnt))
     PLAN (d
      WHERE (dfgd_list->qual[d.seq].tab_no_stats_ind=0))
      JOIN (atm
      WHERE (atm.table_owner=dfgd_list->qual[d.seq].tab_owner)
       AND (atm.table_name=dfgd_list->qual[d.seq].tab_name))
     ORDER BY atm.table_owner, atm.table_name
     HEAD atm.table_name
      dlgl_tot_mod = 0, dlgl_stale_pct = 0.0
     DETAIL
      dlgl_tot_mod = nullval((atm.inserts - atm.deletes),0.0)
      IF ((dfgd_list->qual[d.seq].tab_num_rows=0.0)
       AND dlgl_tot_mod > 0)
       dlgl_stale_pct = 100
      ELSEIF ((dfgd_list->qual[d.seq].tab_num_rows=0.0)
       AND dlgl_tot_mod=0)
       dlgl_stale_pct = 0
      ELSE
       dlgl_stale_pct = least(100,round((((atm.inserts - atm.deletes)/ dfgd_list->qual[d.seq].
         tab_num_rows) * 100),0))
      ENDIF
      dfgd_list->qual[d.seq].tab_stale_pct = dlgl_stale_pct, dfgd_list->qual[d.seq].tab_ins_del_pct
       = 0
      IF (atm.inserts > 1
       AND atm.deletes > 1)
       dfgd_list->qual[d.seq].tab_ins_del_pct = round(((minval(atm.inserts,atm.deletes)/ cnvtreal(
         maxval(atm.inserts,atm.deletes))) * 100),0)
      ENDIF
      IF ((dm_err->debug_flag > 0))
       CALL echo(dfgd_list->qual[d.seq].tab_name),
       CALL echo(build("dlgl_tot_mod =",cnvtstring(dlgl_tot_mod))),
       CALL echo(build("tab_num_rows =",cnvtstring(dfgd_list->qual[d.seq].tab_num_rows))),
       CALL echo(build("dlgl_stale_pct =",cnvtstring(dlgl_stale_pct))),
       CALL echo(build("tab_ins_del_pct =",cnvtstring(dfgd_list->qual[d.seq].tab_ins_del_pct)))
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc) != 0)
     ...
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Idenitfying any new columns without stats"
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     ...
     PLAN (d
      WHERE d.seq > 0)
      JOIN (dtc
      WHERE (dtc.owner=dfgd_list->qual[d.seq].tab_owner)
       AND (dtc.table_name=dfgd_list->qual[d.seq].tab_name)
       AND  NOT ( EXISTS (
      (SELECT
       "x"
       ...
       WHERE dtcs.owner=dtc.owner
        ...
        AND dtcs.column_name=dtc.column_name))))
     ORDER BY dtc.owner, dtc.table_name
     DETAIL
      dfgd_list->qual[d.seq].tab_new_col_ind = 1
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dfgd_preview_rpt(null)
   DECLARE locndx = i4 WITH protect, noconstant(0)
   DECLARE dlsr_tab_idx = i4 WITH protect, noconstant(0)
   DECLARE dlsr_ind_idx = i4 WITH protect, noconstant(0)
   DECLARE dlsr_report_destination = vc WITH protect, noconstant("")
   SET dlsr_report_destination = concat("dm2_freqgthr_",format(cnvtdatetime(curdate,curtime3),
     "MMDDYYHHMM;;Q"))
   SET dm_err->eproc =
   "Generating FREQUENT GATHER preview report data to file from record structure."
   SELECT INTO build(dlsr_report_destination)
    FROM (dummyt d  WITH seq = value(dfgd_list->cnt))
    ORDER BY dfgd_list->qual[d.seq].tab_owner, dfgd_list->qual[d.seq].tab_name
    HEAD REPORT
     row + 0, col 0,
     "****** DATABASE STATISTICS FREQUENT GATHER PREVIEW REPORT ***********************",
     row + 2, col 0, "-- Criteria specified --",
     row + 1, col 0, "Table Size Threshold: ",
     col 25,
     CALL print(concat("Less than ",build(dfgd_size_threshold)," GB")), row + 1,
     col 0, "Staleness Threshold: ", col 25,
     CALL print(concat("Greater than ",build(dfgd_stale_threshold)," percent stale")), row + 2, col 0,
     "-- Details --", row + 1, col 0,
     "T/I", col 5, "OWNER.OBJECT_NAME",
     col 55, "STALE%", col 65,
     "SIZE(#GB)", col 80, "NO_STATS",
     col 90, "MANUAL INC.", row + 1,
     col 0, "---", col 5,
     CALL print(fillstring(45,"-")), col 55, "------",
     col 65,
     CALL print(fillstring(10,"-")), col 80,
     CALL print(fillstring(8,"-")), col 90, "---------"
    DETAIL
     IF ((dfgd_list->cnt > 0))
      IF ((dfgd_list->qual[d.seq].freq_gather_ind=1))
       row + 1, col 1, "T",
       col 5,
       CALL print(substring(1,50,concat(trim(dfgd_list->qual[d.seq].tab_owner),".",trim(dfgd_list->
          qual[d.seq].tab_name)))), col 55,
       CALL print(cnvtstring(dfgd_list->qual[d.seq].tab_stale_pct)), col 65,
       CALL print(format(dfgd_list->qual[d.seq].tab_size,"#####.####;;F"))
       IF ((((dfgd_list->qual[d.seq].tab_no_stats_ind=1)) OR ((dfgd_list->qual[d.seq].tab_new_col_ind
       =1))) )
        col 80,
        CALL print("YES")
       ELSE
       ...
       ENDIF
       col 90,
       CALL print(evaluate(dfgd_list->qual[d.seq].manual_inc_ind,1,"YES","NO"))
       IF ((dfgd_list->qual[d.seq].ind_cnt > 0))
        FOR (locndx = 1 TO dfgd_list->qual[d.seq].ind_cnt)
          row + 1, col 1, "I",
          col 5,
          CALL print(substring(1,50,concat(trim(dfgd_list->qual[d.seq].ind[locndx].index_owner),".",
            trim(dfgd_list->qual[d.seq].ind[locndx].index_name))))
        ENDFOR
       ENDIF
      ENDIF
     ELSE
    ...
    WITH nocounter, nullreport, maxcol = 150,
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   IF (dm2_disp_file(dlsr_report_destination,"Frequent Gather Candidate Report")=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_script
 ...
 SET dfgd_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_execution,dfgd_log_id,1)
 IF (dfgd_lock_obtained_ind=1)
  CALL ddf_release_lock("FREQUENT STATS GATHER LOCK","X",1)
 ENDIF
 SET dm_err->err_ind = dfgd_err_ind
 CALL dmai_set_mod_act(dfgd_original_module,dfgd_original_action)
 SET dm_err->eproc = "Ending dm2_frequent_gather_dbstats"
 CALL final_disp_msg("dm2_fgd")
END GO
