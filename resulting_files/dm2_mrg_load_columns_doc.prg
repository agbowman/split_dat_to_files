CREATE PROGRAM dm2_mrg_load_columns_doc
 ...
 SET dim_filename = "cer_install:dm2_mrg_columns_doc.csv"
 SET readme_data->status = "F"
 SET readme_data->message = "Starting Readme..."
 IF (check_logfile("dm2_load_mrg_tbls_",".log","dm2_mrg_load_columns_doc LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_mrg_load_columns_doc..."
 CALL disp_msg("",dm_err->logfile,0)
 IF (findfile(dim_filename)=0)
  SET readme_data->status = "F"
  SET readme_data->message = "Cannot find dm2_mrg_columns_doc.csv in CER_INSTALL."
  SET dm_err->emsg = "Cannot find dm2_mrg_columns_doc.csv in CER_INSTALL."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF (merge_check_cnt > 0)
  ...
  IF (inhouse_ind=1)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Updating dm_columns_doc..."
  CALL disp_msg("",dm_err->logfile,0)
  SET readme_data->status = "F"
  SET readme_data->message = "Updating dm_columns_doc.."
  SELECT INTO "nl:"
   FROM dm_columns_doc doc,
    (dummyt d  WITH seq = merge_check_cnt)
   PLAN (d
    WHERE d.seq > 0)
    JOIN (doc
    WHERE (requestin->list_0[d.seq].table_name=doc.table_name)
     AND (requestin->list_0[d.seq].column_name=doc.column_name))
   ORDER BY doc.table_name
   DETAIL
  ...
  UPDATE  FROM dm_columns_doc dcd,
    (dummyt d  WITH seq = merge_check_cnt)
   SET dcd.table_name = trim(requestin->list_0[d.seq].table_name), dcd.column_name = trim(requestin->
     list_0[d.seq].column_name), dcd.root_entity_name = trim(requestin->list_0[d.seq].
     root_entity_name),
    dcd.root_entity_attr = trim(requestin->list_0[d.seq].root_entity_attr), dcd.code_set = cnvtint(
     requestin->list_0[d.seq].code_set), dcd.merge_delete_ind = cnvtint(requestin->list_0[d.seq].
     merge_delete_ind),
    dcd.sequence_name = trim(requestin->list_0[d.seq].sequence_name), dcd.parent_entity_col = trim(
     requestin->list_0[d.seq].parent_entity_col), dcd.constant_value = evaluate(trim(requestin->
      list_0[d.seq].constant_value),"",null,trim(requestin->list_0[d.seq].constant_value)),
    dcd.exception_flg = cnvtint(requestin->list_0[d.seq].exception_flg), dcd.unique_ident_ind =
    cnvtint(requestin->list_0[d.seq].unique_ident_ind), dcd.updt_cnt = cnvtint(trim(requestin->
      list_0[d.seq].updt_cnt)),
    dcd.defining_attribute_ind = cnvtint(requestin->list_0[d.seq].defining_attribute_ind)
   PLAN (d
    WHERE (rs_exist->qual[d.seq].is_there=1))
    JOIN (dcd
    WHERE (dcd.table_name=requestin->list_0[d.seq].table_name)
     AND (dcd.column_name=requestin->list_0[d.seq].column_name)
     AND dcd.updt_cnt <= cnvtint(requestin->list_0[d.seq].updt_cnt))
   WITH nocounter
  ;end update
  SET ddcd_curqual = (ddcd_curqual+ curqual)
  SET after_cnt = curqual
  SET readme_data->message = "Inserting into dm_columns_doc.."
 ...
 ELSE
  ...
  SET readme_data->message = concat("Readme Failed.  The request structure has not been ",
   "populated with dm2_mrg_columns_doc.csv information.")
 ...
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading dm_columns_doc with info in dm2_mrg_columns_doc.csv."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = "Error loading dm_columns_doc with info in dm2_mrg_columns_doc.csv."
  GO TO exit_script
 ELSE
  SET readme_data->status = "S"
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (dcd
   WHERE dcd.table_name=trim(requestin->list_0[d.seq].table_name)
    AND dcd.column_name=trim(requestin->list_0[d.seq].column_name)
    AND dcd.exception_flg=cnvtint(requestin->list_0[d.seq].exception_flg)
    AND dcd.unique_ident_ind=cnvtint(requestin->list_0[d.seq].unique_ident_ind)
    AND dcd.merge_delete_ind=cnvtint(requestin->list_0[d.seq].merge_delete_ind)
    AND dcd.updt_cnt=cnvtint(trim(requestin->list_0[d.seq].updt_cnt)))
  DETAIL
   dim_cnt = (dim_cnt+ 1)
  WITH nocounter
 ;end select
 ...
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = "Check failed. Rows in request != rows in local dm_columns_doc."
 ENDIF
#exit_script
 IF ((readme_data->status="S"))
  IF (inhouse_ind=1)
   SET readme_data->message = "Not executed since we are in inhouse domain."
  ELSE
   SET readme_data->message = "DM_columns_DOC has been updated successfully."
  ENDIF
 ELSE
  ROLLBACK
 ENDIF
 SET dm_err->eproc = "...Ending dm2_mrg_load_columns_doc"
