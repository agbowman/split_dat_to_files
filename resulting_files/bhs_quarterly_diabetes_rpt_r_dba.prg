CREATE PROGRAM bhs_quarterly_diabetes_rpt_r:dba
   ...
   SELECT INTO "nl:"
    ...
    PLAN (d)
     ...
     WHERE ce.person_id=d.pid
      ...
      AND ce.clinsig_updt_dt_tm >= datetimeadd(cnvtdatetime(curdate,curtime3),- (730))
      AND ce.view_level=1)
    ...
    HEAD ce.person_id
     count = 0, seven_ind = 0, ce_id = 0,
     count = (count+ 1), t_record->prac_qual[i].pat_with_a1c_cnt = (t_record->prac_qual[i].
     pat_with_a1c_cnt+ 1)
 ...
 SELECT INTO "quarterly_diabetes_report.xls"
  ...
  HEAD loc_name
   ...
   pat_cnt)),
   p10 = (100 * (cnvtreal(t_record->prac_qual[d.seq].hb_val_cnt)/ t_record->prac_qual[d.seq].
   pat_with_a1c_cnt)), p11 = (100 * (cnvtreal(t_record->prac_qual[d.seq].ldl_val_cnt)/ t_record->
   prac_qual[d.seq].pat_cnt)), p12 = (100 * (cnvtreal(t_record->prac_qual[d.seq].blood_pres_val_cnt)
   / t_record->prac_qual[d.seq].pat_cnt)),
