CREATE PROGRAM ch_eks_module_action:dba
 PROMPT
 ...
 RECORD discern_rule_action(
   1 action_data[*]
     2 module_name = vc
     2 version = vc
     2 action_operator = vc
     2 action_id = vc
     2 action_template = vc
 ...
 SUBROUTINE (parsetemplate(txt=vc) =null)
   ...
   DECLARE record_cnt = i4 WITH protect, noconstant(size(discern_rule_action->action_data,5))
   ...
   WHILE (size(remainder) > 0
    AND limit < upper_bound)
     ...
     SET token_cnt += 1
     SET record_cnt += 1
     SET stat = alterlist(discern_rule_action->action_data,record_cnt)
     ...
     IF ( NOT (isoperator(token)))
      SET template_cnt += 1
      CALL echo(build("+++",token,"=>","A",template_cnt))
      ...
      ELSE
       SET remainder = null
      ENDIF
      SET discern_rule_action->action_data[record_cnt].action_id = build("A",template_cnt)
      SET discern_rule_action->action_data[record_cnt].action_template = token
     ELSE
      SET discern_rule_action->action_data[record_cnt].action_operator = token
     ENDIF
     SET discern_rule_action->action_data[record_cnt].module_name = module_name
     SET discern_rule_action->action_data[record_cnt].version = version
     SET discern_rule_action->action_data[record_cnt].action_sequence = token_cnt
   ENDWHILE
   SET tokens->cnt = token_cnt
   SET stat = alterlist(tokens->list,token_cnt)
   SET stat = alterlist(discern_rule_action->action_data,record_cnt)
   IF (limit >= upper_bound)
    CALL cclexception(999,"E",build("Infinite loop in parseTemplate(A)"))
   ENDIF
 END ;Subroutine
 ...
 FOR (i = 1 TO size(eksmod_reply->qual,5))
   ...
   ENDIF
 ENDFOR
 CALL echo(size(discern_rule_action->action_data,5))
 FOR (i = 1 TO size(discern_rule_action->action_data,5))
   SET manualxml = ""
   IF (i > 1)
    SET manualxml = char(10)
   ENDIF
   SET manualxml = concat(manualxml,"<MODULE_NAME>",trim(discern_rule_action->action_data[i].
     module_name,3),"</MODULE_NAME>","<MODULE_VERSION>",
    trim(discern_rule_action->action_data[i].version,3),"</MODULE_VERSION>","<ACTION_OPERATOR>",trim(
     discern_rule_action->action_data[i].action_operator,3),"</ACTION_OPERATOR>",
    "<ACTION_ID>",trim(discern_rule_action->action_data[i].action_id,3),"</ACTION_ID>",
    "<ACTION_TEMPLATE>",trim(discern_rule_action->action_data[i].action_template,3),
    "</ACTION_TEMPLATE>","<ACTION_SEQUENCE>",trim(cnvtstring(discern_rule_action->action_data[i].
      action_sequence),3),"</ACTION_SEQUENCE>")
   SET stat = writetofile("ccluserdir:mill_rules_discern_rule_action_2023_07_05_00_03_14.dat",
    manualxml)
   IF (mod(i,100)=0)
