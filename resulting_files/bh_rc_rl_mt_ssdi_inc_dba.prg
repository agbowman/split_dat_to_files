CREATE PROGRAM bh_rc_rl_mt_ssdi_inc:dba
 CALL echo("***** bh_rc_rl_mt_ssdi_inc.prg - 677918 *****")
 ...
 SUBROUTINE filltemprec(null)
   FOR (lchgcnt = 1 TO size(request->integer_params,5))
     CASE (request->integer_params[lchgcnt].key_txt)
      OF "SSDI_INCOME_MONTHLY":
       SET tmprec->ssdi_income_annual = (request->integer_params[lchgcnt].integer_value * 12)
     ENDCASE
   ENDFOR
 END ;Subroutine
 DECLARE createreply(null) = null
 SUBROUTINE createreply(null)
  CALL addtoreplyintegers("SSDI_INCOME_ANNUAL",tmprec->ssdi_income_annual)
  CALL echorecord(tmprec)
 END ;Subroutine
 FREE RECORD tmprec
 RECORD tmprec(
   1 ssdi_income_annual = i4
 )
