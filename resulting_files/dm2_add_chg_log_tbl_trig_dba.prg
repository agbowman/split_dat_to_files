CREATE PROGRAM dm2_add_chg_log_tbl_trig:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dacltt_build(db_text) = null
 DECLARE dacltt_exe(null) = null
 DECLARE db_trig_cnt = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_log_tbl_trig",".log","dm2_add_chg_log_tbl_trig LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_add_chg_log_tbl_trig..."
 CALL disp_msg("",dm_err->logfile,0)
 FREE RECORD trig_stmt
 RECORD trig_stmt(
   1 stmt_cnt = i4
   1 list[*]
     2 stmt = vc
 )
 CALL dacltt_build('rdb asis("create or replace trigger trg_dm_chg_log_del after")')
 CALL dacltt_build('asis ("delete on dm_chg_log")')
 CALL dacltt_build('asis ("for each row")')
 CALL dacltt_build('asis ("begin")')
 CALL dacltt_build('asis ("   update dm_chg_log_smry set")')
 CALL dacltt_build('asis ("     row_count=row_count-1,")')
 CALL dacltt_build('asis ("     updt_dt_tm = sysdate,")')
 CALL dacltt_build('asis ("     updt_cnt = updt_cnt + 1")')
 CALL dacltt_build('asis ("   where table_name = :old.table_name")')
 CALL dacltt_build('asis ("     and target_env_id = :old.target_env_id")')
 CALL dacltt_build('asis ("     and log_type = :old.log_type;")')
 CALL dacltt_build('asis ("end;")')
 CALL dacltt_build(" go")
 CALL dacltt_exe(null)
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 CALL dacltt_build('rdb asis ("create or replace trigger trg_dm_chg_log_ins after")')
 CALL dacltt_build('asis ("insert on dm_chg_log")')
 CALL dacltt_build('asis ("for each row")')
 CALL dacltt_build('asis ("begin")')
 CALL dacltt_build('asis ("   update dm_chg_log_smry set")')
 CALL dacltt_build('asis ("     row_count=row_count+1,")')
 CALL dacltt_build('asis ("     updt_dt_tm = sysdate,")')
 CALL dacltt_build('asis ("     updt_cnt = updt_cnt + 1")')
 CALL dacltt_build('asis ("   where table_name = :new.table_name")')
 CALL dacltt_build('asis ("     and target_env_id = :new.target_env_id")')
 CALL dacltt_build('asis ("     and log_type = :new.log_type;")')
 CALL dacltt_build('asis ("     if sql%notfound then")')
 CALL dacltt_build('asis ("        insert into dm_chg_log_smry")')
 CALL dacltt_build(
  'asis ("       (table_name, target_env_id, log_type, updt_dt_tm, updt_cnt, row_count)")')
 CALL dacltt_build('asis ("        values ")')
 CALL dacltt_build(
  'asis ("       (:new.table_name, :new.target_env_id, :new.log_type, sysdate, 0, 1);")')
 CALL dacltt_build('asis ("      end if;")')
 CALL dacltt_build('asis ("end;")')
 CALL dacltt_build(" go")
 CALL dacltt_exe(null)
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 CALL dacltt_build('rdb asis ("create or replace trigger trg_dm_chg_log_upd after")')
 CALL dacltt_build('asis ("update on dm_chg_log")')
 CALL dacltt_build('asis ("for each row")')
 CALL dacltt_build('asis ("begin")')
 CALL dacltt_build('asis ("   update dm_chg_log_smry set")')
 CALL dacltt_build('asis ("     row_count=row_count+1,")')
 ...
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SUBROUTINE dacltt_build(db_stmt)
   SET db_trig_cnt = (db_trig_cnt+ 1)
   IF (mod(db_trig_cnt,100)=1)
    SET stat = alterlist(trig_stmt->list,(db_trig_cnt+ 99))
   ENDIF
   SET trig_stmt->list[db_trig_cnt].stmt = db_stmt
   SET trig_stmt->stmt_cnt = db_trig_cnt
 END ;Subroutine
 SUBROUTINE dacltt_exe(null)
   FOR (de_lp = 1 TO trig_stmt->stmt_cnt)
     CALL dm2_push_cmd(trig_stmt->list[de_lp].stmt,1)
   ENDFOR
   SET trig_stmt->stmt_cnt = 0
   SET stat = alterlist(trig_stmt->list,0)
   SET db_trig_cnt = 0
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "...Ending dm2_add_chg_log_tbl_trig"
 CALL final_disp_msg("dm2_log_tbl_trig")
END GO
