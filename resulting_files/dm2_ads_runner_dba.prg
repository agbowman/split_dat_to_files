CREATE PROGRAM dm2_ads_runner:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dar_iter = i4 WITH protect, noconstant(0)
 DECLARE dar_continue_ind = i2 WITH protect, noconstant(0)
 DECLARE dar_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dar_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dar_running = i2 WITH protect, noconstant(0)
 DECLARE dar_running_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_ads_runner",".log","DM2_ADS_RUNNER LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning DM2_ADS_RUNNER"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_row(dpl_sample,dpl_execution,dpl_no_prev_id,1)
 SET dar_log_id = dm2_process_event_rs->dm_process_event_id
 CALL drr_insert_runner_row(dpl_ads_runner,cnvtstring(currdbhandle),"SAMPLE RUNNER",1,0.0)
 SET dscg_dpe_id = dm2_process_event_rs->dm_process_event_id
 IF (dpq_populate_queue_rows(dpl_sample)=0)
  GO TO exit_program
 ENDIF
 IF (dar_check_ads_execution(dar_running)=0)
  GO TO exit_program
 ENDIF
 IF (dar_running=1
  AND (dpq_proc_list->proc_cnt > 0))
  FOR (dar_iter = 1 TO dpq_proc_list->proc_cnt)
    IF (drr_check_runner_status(dpl_ads_runner,cnvtstring(currdbhandle),dar_running_ind)=0)
     GO TO exit_program
    ENDIF
    IF (dar_running_ind=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Runner has been marked to be stopped in dm_info. Exiting..."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    SET dpq_process_queue->dm_process_queue_id = dpq_proc_list->qual[dar_iter].dm_process_queue_id
    SET dpq_process_queue->operation_txt = dpq_proc_list->qual[dar_iter].operation_txt
    SET dpq_process_queue->op_method = dpq_proc_list->qual[dar_iter].op_method
    IF ( NOT (dpq_lock_queue_row(dar_continue_ind)))
     GO TO exit_program
    ELSEIF (dar_continue_ind)
     IF (((dpq_execute_command(dar_continue_ind)=0) OR (dar_continue_ind=0)) )
      SET dm_err->err_ind = 0
     ENDIF
     IF (dpq_update_queue_row(dar_continue_ind)=0)
      GO TO exit_program
     ENDIF
    ENDIF
  ENDFOR
...
#exit_program
 ...
 SET dar_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_sample,dpl_execution,dar_log_id,1)
 CALL drr_remove_runner_row(dpl_ads_runner,cnvtstring(currdbhandle))
 SET dm_err->err_ind = dar_err_ind
 CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 SET dm_err->eproc = "Ending DM2_ADS_RUNNER"
 CALL final_disp_msg("dm2_ads_runner")
