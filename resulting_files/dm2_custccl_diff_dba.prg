CREATE PROGRAM dm2_custccl_diff:dba
 ...
 SUBROUTINE dcr_rpt_plan_file_list(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dcd_file1_dir = vc WITH protect, noconstant( $1)
 DECLARE dcd_file1_file = vc WITH protect, noconstant( $2)
 DECLARE dcd_file2_dir = vc WITH protect, noconstant( $3)
 DECLARE dcd_file2_file = vc WITH protect, noconstant( $4)
 DECLARE dcd_cur_script_name = vc WITH protect, noconstant("DM2_NOT_SET")
 DECLARE dcd_cur_query_number = i4 WITH protect, noconstant(0)
 DECLARE dcd_prev_script_name = vc WITH protect, noconstant("DM2_NOT_SET")
 DECLARE dcd_prev_query_number = i4 WITH protect, noconstant(0)
 DECLARE dcd_path_separator = vc WITH protect, noconstant("/")
 DECLARE dcd_script_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcd_query_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcd_line_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcd_new_script_ind = i2 WITH protect, noconstant(0)
 RECORD dcd_file1(
   1 script_cnt = i4
   1 scripts[*]
     2 script_name = vc
 ...
 IF (check_logfile("dm2_custccldiff",".log","DM2_CUSTCCL_DIFF LOG FILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = build("Logging at debug level:",dm_err->debug_flag)
 CALL disp_msg("",dm_err->logfile,0)
 IF ((dm2_sys_misc->cur_os="AXP"))
  SET dcd_path_separator = ":"
 ENDIF
 SET dm_err->eproc = concat("Validating <",dcd_file1_dir,dcd_path_separator,dcd_file1_file,">.")
 IF (dm2_findfile(concat(dcd_file1_dir,dcd_path_separator,dcd_file1_file))=0)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Validating <",dcd_file2_dir,dcd_path_separator,dcd_file2_file,">.")
 IF (dm2_findfile(concat(dcd_file2_dir,dcd_path_separator,dcd_file2_file))=0)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->eproc,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Loading <",dcd_file1_dir,dcd_path_separator,dcd_file1_file,">.")
 CALL disp_msg("",dm_err->logfile,0)
 FREE DEFINE rtl2
 FREE SET file_loc
 SET logical file_loc value(dcd_file1_file)
 DEFINE rtl2 "file_loc"
 SELECT INTO "nl:"
  r.line
  FROM rtl2t r
  HEAD REPORT
   rpt_name_length = 0, rpt_qnum_start = 0, rpt_qnum_length = 0,
   rpt_line_cnt = 0
  DETAIL
   rpt_name_length = (findstring(",",r.line,0,0) - 1)
   IF (rpt_name_length > 0)
    rpt_qnum_start = (rpt_name_length+ 2), rpt_qnum_length = (findstring(",",r.line,rpt_qnum_start,0)
     - rpt_qnum_start), dcd_cur_script_name = substring(1,rpt_name_length,r.line),
    dcd_cur_query_number = cnvtint(substring(rpt_qnum_start,rpt_qnum_length,r.line))
    IF (dcd_cur_script_name != dcd_prev_script_name)
     dcd_prev_script_name = dcd_cur_script_name, dcd_file1->script_cnt = (dcd_file1->script_cnt+ 1),
     stat = alterlist(dcd_file1->scripts,dcd_file1->script_cnt),
     dcd_file1->scripts[dcd_file1->script_cnt].script_name = dcd_cur_script_name, dcd_file1->scripts[
     dcd_file1->script_cnt].query_cnt = 1, stat = alterlist(dcd_file1->scripts[dcd_file1->script_cnt]
      .queries,dcd_file1->scripts[dcd_file1->script_cnt].query_cnt),
     dcd_file1->scripts[dcd_file1->script_cnt].queries[dcd_file1->scripts[dcd_file1->script_cnt].
     query_cnt].query_number = dcd_cur_query_number, dcd_file1->scripts[dcd_file1->script_cnt].
     queries[dcd_file1->scripts[dcd_file1->script_cnt].query_cnt].line_cnt = 0, rpt_line_cnt = 0,
     dcd_prev_query_number = dcd_cur_query_number
    ELSEIF (dcd_cur_query_number != dcd_prev_query_number)
     dcd_file1->scripts[dcd_file1->script_cnt].query_cnt = (dcd_file1->scripts[dcd_file1->script_cnt]
     .query_cnt+ 1), stat = alterlist(dcd_file1->scripts[dcd_file1->script_cnt].queries,dcd_file1->
      scripts[dcd_file1->script_cnt].query_cnt), dcd_file1->scripts[dcd_file1->script_cnt].queries[
     dcd_file1->scripts[dcd_file1->script_cnt].query_cnt].query_number = dcd_cur_query_number,
     dcd_file1->scripts[dcd_file1->script_cnt].queries[dcd_file1->scripts[dcd_file1->script_cnt].
     query_cnt].line_cnt = 0, rpt_line_cnt = 0, dcd_prev_query_number = dcd_cur_query_number
    ENDIF
    dcd_file1->scripts[dcd_file1->script_cnt].queries[dcd_file1->scripts[dcd_file1->script_cnt].
    query_cnt].line_cnt = (dcd_file1->scripts[dcd_file1->script_cnt].queries[dcd_file1->scripts[
    dcd_file1->script_cnt].query_cnt].line_cnt+ 1), rpt_line_cnt = dcd_file1->scripts[dcd_file1->
    script_cnt].queries[dcd_file1->scripts[dcd_file1->script_cnt].query_cnt].line_cnt, stat =
    alterlist(dcd_file1->scripts[dcd_file1->script_cnt].queries[dcd_file1->scripts[dcd_file1->
     script_cnt].query_cnt].lines,dcd_file1->scripts[dcd_file1->script_cnt].queries[dcd_file1->
     scripts[dcd_file1->script_cnt].query_cnt].line_cnt),
    dcd_file1->scripts[dcd_file1->script_cnt].queries[dcd_file1->scripts[dcd_file1->script_cnt].
    query_cnt].lines[rpt_line_cnt].line = r.line
   ENDIF
  WITH nocounter, maxcol = 255
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dcd_prev_script_name = "DM2_NOT_SET"
 SET dcd_prev_query_number = 0
 SET dm_err->eproc = concat("Loading <",dcd_file2_dir,dcd_path_separator,dcd_file2_file,">.")
 FREE DEFINE rtl2
 FREE SET file_loc
 SET logical file_loc value(dcd_file2_file)
 DEFINE rtl2 "file_loc"
 SELECT INTO "nl:"
  r.line
  FROM rtl2t r
  HEAD REPORT
   rpt_name_length = 0, rpt_qnum_start = 0, rpt_qnum_length = 0,
   rpt_line_cnt = 0, rpt_f1idx = 0, rpt_f1qidx = 0
  DETAIL
   rpt_name_length = (findstring(",",r.line,1,0) - 1)
   IF (rpt_name_length > 0)
    rpt_qnum_start = (rpt_name_length+ 2), rpt_qnum_length = (findstring(",",r.line,rpt_qnum_start,0)
     - rpt_qnum_start), dcd_cur_script_name = substring(1,rpt_name_length,r.line),
    dcd_cur_query_number = cnvtint(substring(rpt_qnum_start,rpt_qnum_length,r.line)), rpt_f1idx =
    locateval(rpt_f1idx,1,dcd_file1->script_cnt,dcd_cur_script_name,dcd_file1->scripts[rpt_f1idx].
     script_name)
    IF (dcd_cur_script_name != dcd_prev_script_name)
     dcd_prev_script_name = dcd_cur_script_name, dcd_file2->script_cnt = (dcd_file2->script_cnt+ 1),
     stat = alterlist(dcd_file2->scripts,dcd_file2->script_cnt),
     dcd_file2->scripts[dcd_file2->script_cnt].script_name = dcd_cur_script_name, dcd_file2->scripts[
     dcd_file2->script_cnt].query_cnt = 1, stat = alterlist(dcd_file2->scripts[dcd_file2->script_cnt]
      .queries,dcd_file2->scripts[dcd_file2->script_cnt].query_cnt),
     dcd_file2->scripts[dcd_file2->script_cnt].queries[dcd_file2->scripts[dcd_file2->script_cnt].
     query_cnt].query_number = dcd_cur_query_number, dcd_file2->scripts[dcd_file2->script_cnt].
     queries[dcd_file2->scripts[dcd_file2->script_cnt].query_cnt].line_cnt = 0, rpt_line_cnt = 0,
     dcd_prev_query_number = dcd_cur_query_number
    ELSEIF (dcd_cur_query_number != dcd_prev_query_number)
     dcd_file2->scripts[dcd_file2->script_cnt].query_cnt = (dcd_file2->scripts[dcd_file2->script_cnt]
     .query_cnt+ 1), stat = alterlist(dcd_file2->scripts[dcd_file2->script_cnt].queries,dcd_file2->
      scripts[dcd_file2->script_cnt].query_cnt), dcd_file2->scripts[dcd_file2->script_cnt].queries[
     dcd_file2->scripts[dcd_file2->script_cnt].query_cnt].query_number = dcd_cur_query_number,
     dcd_file2->scripts[dcd_file2->script_cnt].queries[dcd_file2->scripts[dcd_file2->script_cnt].
     query_cnt].line_cnt = 0, rpt_line_cnt = 0, dcd_prev_query_number = dcd_cur_query_number
    ENDIF
    IF (rpt_f1idx > 0)
     dcd_file2->scripts[dcd_file2->script_cnt].f1_idx = rpt_f1idx, dcd_file1->scripts[rpt_f1idx].
     f2_idx = dcd_file2->script_cnt, rpt_f1qidx = locateval(rpt_f1qidx,1,dcd_file1->scripts[dcd_file2
      ->scripts[dcd_file2->script_cnt].f1_idx].query_cnt,dcd_cur_query_number,dcd_file1->scripts[
      rpt_f1idx].queries[rpt_f1qidx].query_number)
     IF (rpt_f1qidx > 0)
      dcd_file2->scripts[dcd_file2->script_cnt].queries[dcd_file2->scripts[dcd_file2->script_cnt].
      query_cnt].f1_idx = rpt_f1qidx, dcd_file1->scripts[rpt_f1idx].queries[rpt_f1qidx].f2_idx =
      dcd_file2->scripts[dcd_file2->script_cnt].query_cnt
     ENDIF
    ENDIF
    dcd_file2->scripts[dcd_file2->script_cnt].queries[dcd_file2->scripts[dcd_file2->script_cnt].
    query_cnt].line_cnt = (dcd_file2->scripts[dcd_file2->script_cnt].queries[dcd_file2->scripts[
    dcd_file2->script_cnt].query_cnt].line_cnt+ 1), rpt_line_cnt = dcd_file2->scripts[dcd_file2->
    script_cnt].queries[dcd_file2->scripts[dcd_file2->script_cnt].query_cnt].line_cnt, stat =
    alterlist(dcd_file2->scripts[dcd_file2->script_cnt].queries[dcd_file2->scripts[dcd_file2->
     script_cnt].query_cnt].lines,dcd_file2->scripts[dcd_file2->script_cnt].queries[dcd_file2->
     scripts[dcd_file2->script_cnt].query_cnt].line_cnt),
    dcd_file2->scripts[dcd_file2->script_cnt].queries[dcd_file2->scripts[dcd_file2->script_cnt].
    query_cnt].lines[rpt_line_cnt].line = r.line
   ENDIF
  WITH nocounter, maxcol = 255
 ...
 ENDIF
 IF ((dm_err->debug_flag > 9))
  CALL echorecord(dcd_file1)
  CALL echorecord(dcd_file2)
 ENDIF
 FOR (dcd_script_cnt = 1 TO dcd_file1->script_cnt)
   IF ((dcd_file1->scripts[dcd_script_cnt].f2_idx=0))
    SET dcd_new_script_ind = 1
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Searching for script plan differences and generating report."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "mine"
  FROM (dummyt d  WITH seq = dcd_file1->script_cnt)
  HEAD REPORT
   rpt_line_diff_ind = 0, rpt_f2idx = 0, rpt_f2qidx = 0,
   col 0, "Script Plan Difference Report", row + 1,
   col 0, "******************************", row + 2,
   col 0, "File 1: ", dcd_file1_dir,
   dcd_path_separator, dcd_file1_file, row + 1,
   col 0, "File 2: ", dcd_file2_dir,
   dcd_path_separator, dcd_file2_file, row + 2,
   col 0,
   "NOTE:  Query numbers for a reported script may not represent the same query from one plan to the next.",
   row + 1,
   col 0, "       For any plan differences reported below, verify that the queries actually match.",
   row + 2,
   col 0, "       To display a program's SQL by query number, enter the following command in CCL:",
   row + 1,
   col 0, "          CCL> translate <CCL SCRIPT NAME> with query go", row + 2
   IF (dcd_new_script_ind=1)
    col 0,
    "The following scripts were identified in File 1 but not found in File 2.  These scripts should be reviewed",
    row + 1,
    col 0, "for query plan performance.", row + 2,
    col 0, "CCL Script Name               ", row + 1,
    col 0, "******************************", row + 1
    FOR (dcd_script_cnt = 1 TO dcd_file1->script_cnt)
      IF ((dcd_file1->scripts[dcd_script_cnt].f2_idx=0))
       col 0, dcd_file1->scripts[dcd_script_cnt].script_name, row + 1
      ENDIF
    ENDFOR
    row + 2
   ENDIF
   col 0,
   "The following scripts contain queries whose plans had changed between File 1 and File 2.  These scripts should be",
   row + 1,
   col 0, "reviewed for query plan performance.", row + 2
  DETAIL
   IF ((dcd_file1->scripts[d.seq].f2_idx > 0))
    rpt_f2idx = dcd_file1->scripts[d.seq].f2_idx
    FOR (dcd_query_cnt = 1 TO dcd_file1->scripts[d.seq].query_cnt)
      rpt_line_diff_ind = 0, rpt_f2qidx = 0
      IF ((dcd_file1->scripts[d.seq].queries[dcd_query_cnt].f2_idx > 0))
       rpt_f2qidx = dcd_file1->scripts[d.seq].queries[dcd_query_cnt].f2_idx
       IF ((dcd_file1->scripts[d.seq].queries[dcd_query_cnt].line_cnt=dcd_file2->scripts[rpt_f2idx].
       queries[rpt_f2qidx].line_cnt))
        FOR (dcd_line_cnt = 1 TO dcd_file1->scripts[d.seq].queries[dcd_query_cnt].line_cnt)
          IF ((dcd_file1->scripts[d.seq].queries[dcd_query_cnt].lines[dcd_line_cnt].line != dcd_file2
          ->scripts[rpt_f2idx].queries[rpt_f2qidx].lines[dcd_line_cnt].line))
           rpt_line_diff_ind = 1, dcd_line_cnt = dcd_file1->scripts[d.seq].queries[dcd_query_cnt].
           line_cnt
          ENDIF
        ENDFOR
       ELSE
        rpt_line_diff_ind = 1
       ENDIF
       IF (rpt_line_diff_ind=1)
        row + 2, col 0, "Script Name:  ",
        dcd_file1->scripts[d.seq].script_name, row + 1, col 0,
        "Query Number: ",
        CALL print(build(dcd_file1->scripts[d.seq].queries[dcd_query_cnt].query_number)), row + 2,
        col 3, "File 1 Query Plan", row + 1,
        col 3, "-----------------", row + 1
        FOR (dcd_line_cnt = 1 TO dcd_file1->scripts[d.seq].queries[dcd_query_cnt].line_cnt)
          col 3, dcd_file1->scripts[d.seq].queries[dcd_query_cnt].lines[dcd_line_cnt].line, row + 1
        ENDFOR
        row + 1, col 3, "File 2 Query Plan",
        row + 1, col 3, "-----------------",
        row + 1
        FOR (dcd_line_cnt = 1 TO dcd_file2->scripts[rpt_f2idx].queries[rpt_f2qidx].line_cnt)
          col 3, dcd_file2->scripts[rpt_f2idx].queries[rpt_f2qidx].lines[dcd_line_cnt].line, row + 1
        ENDFOR
       ENDIF
      ELSE
       row + 2, col 0, "Script Name:  ",
       ...
       FOR (dcd_line_cnt = 1 TO dcd_file1->scripts[d.seq].queries[dcd_query_cnt].line_cnt)
         col 3, dcd_file1->scripts[d.seq].queries[dcd_query_cnt].lines[dcd_line_cnt].line, row + 1
       ENDFOR
       row + 2, col 3, "File 2 Query Plan",
       row + 1, col 3, "-----------------",
       row + 1, col 3, "No data found in File 2 for this query number.",
       row + 2
      ENDIF
    ENDFOR
   ENDIF
  FOOT REPORT
   row + 1, col 0, "********* END REPORT *********"
  WITH nocounter, format = variable, maxcol = 255
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "Finished comparing plan reports."
 CALL final_disp_msg("dm2_custccldiff")
END GO
