CREATE PROGRAM dm2_ref_data_mover_6020:dba
 ...
 SET cust_column_cnt = get_col_pos(cust_tab_name,"HOLD_CRITERIA")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_column_cnt].translated = 0
 IF (is_translated(cust_tab_name,"HOLD_CRITERIA_CD")=0)
  CALL is_translated(cust_tab_name,"ALL")
  GO TO exit_6020
 ENDIF
 SET cust_temp_cd = cnvtreal(get_value(cust_tab_name,"HOLD_CRITERIA_CD","TO"))
 SELECT INTO "nl:"
  FROM code_value cv
 ...
 IF (cust_temp_cdf_mean <= " ")
  ...
  CALL disp_msg("ERROR: invalid cdf_meaning retuned by uar call",dm_err->logfile,0)
  GO TO exit_6020
 ENDIF
 SET cust_from_value = get_value(cust_tab_name,"HOLD_CRITERIA","FROM")
 ...
 CASE (cust_temp_cdf_mean)
  ...
  OF "DAYSSINCE":
  OF "EXCEPDOLLAR":
   CALL put_value(cust_tab_name,"HOLD_CRITERIA",cust_from_value)
  OF "HEALTHPLAN":
  OF "SECHEALTHPLN":
  ...
  ELSE
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV99"
   CALL disp_msg(concat("ERROR: the cdf_meaning of ",cust_temp_cdf_mean,
 ...
 IF (cust_process_tab > " ")
  IF (cnvtreal(cust_from_value) > 0)
   ...
   IF (cust_new_value != "No Trans")
    CALL put_value(cust_tab_name,"HOLD_CRITERIA",cust_new_value)
   ELSE
 ...
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_6020
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"PFT_HOLD_CRITERIA_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"PFT_HOLD_CRITERIA_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"PFT_HOLD_CRITERIA_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  PFT_HOLD_CRITERIA_ID"
       )
     ELSE
      GO TO exit_6020
     ENDIF
   ...
   SET cust_from_value = get_value(cust_tab_name,"PFT_HOLD_CRITERIA_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6020
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_value)
   ENDIF
   SET cust_new_value = get_value(cust_tab_name,"PFT_HOLD_CRITERIA_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6020
   ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"HOLD_CRITERIA")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    ...
    "This row wasn't translated because of the HOLD_CRITERIA column"
