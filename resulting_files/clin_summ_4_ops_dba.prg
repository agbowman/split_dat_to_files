CREATE PROGRAM clin_summ_4_ops:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  HEAD e.person_id
   p = (p+ 1)
   IF (mod(p,10)=1)
    stat = alterlist(request->person,(p+ 9))
   ENDIF
   request->person[p].person_id = e.person_id, request->person_cnt = p
  HEAD e.encntr_id
   e = (e+ 1)
   IF (mod(e,10)=1)
    stat = alterlist(request->visit,(e+ 9))
   ENDIF
   request->visit[e].encntr_id = e.encntr_id, request->visit_cnt = e
  WITH nocounter
 ;end select
  ...
  SELECT INTO "nl:"
   ...
   HEAD e.encntr_id
    IF (e.encntr_id > 0)
     ...
     dlrec->seq[cnt].org_name = org.org_name
    ENDIF
   FOOT  e.encntr_id
    stat = alterlist(dlrec->seq[cnt].diagnosis,1), dlrec->seq[cnt].diagnosis[1].diag_type_desc =
    "Reason For Visit", dlrec->seq[cnt].diagnosis[1].source_string = e.reason_for_visit,
    dlrec->seq[cnt].diagnosis[1].diag_dt_tm = substring(1,14,format(e.reg_dt_tm,"@SHORTDATETIME;;Q")),
    dlrec->seq[cnt].total_diag = 1
   FOOT  pt_loc
    col + 0
   FOOT REPORT
    IF (cnt > 0)
    ...
    FOR (i = 1 TO dlrec->encntr_total)
      ...
      IF ((dlrec->seq[i].problem_total > 0))
       ...
       FOR (a = 1 TO dlrec->seq[i].problem_total)
        tempstring = concat(dlrec->seq[i].problem[a].full_text," "),line_wrap
       ENDFOR
      ENDIF
      xcolvar = xcol1
      FOR (a = 1 TO dlrec->seq[i].section1_total)
       tempstring = concat("{b}",dlrec->seq[i].section1_events[a].event_cd_disp,"{endb} ",dlrec->seq[
        i].section1_events[a].result),line_wrap
      ENDFOR
      ...
      IF ((dlrec->seq[i].total_diag > 0))
       ...
       FOR (a = 1 TO dlrec->seq[i].total_diag)
        tempstring = concat(trim(dlrec->seq[i].diagnosis[a].source_string,3)," (",trim(dlrec->seq[i].
          diagnosis[a].diag_type_desc,3),")   "),line_wrap
       ENDFOR
      ENDIF
      wrapcol = 120, xcolvar = xcol1
      FOR (a = 1 TO dlrec->seq[i].section2_total)
       tempstring = concat("{b}",dlrec->seq[i].section2_events[a].event_cd_disp,"{endb} ",dlrec->seq[
        i].section2_events[a].result),line_wrap
      ENDFOR
      ...
      FOR (a = 1 TO dlrec->seq[i].section3_total)
       tempstring = concat("{b}",dlrec->seq[i].section3_events[a].event_cd_disp,"{endb} ",dlrec->seq[
        i].section3_events[a].result),line_wrap
      ENDFOR
      xcolvar = xcol1
      FOR (a = 1 TO dlrec->seq[i].orders35_total)
        IF ((dlrec->seq[i].orders35[a].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].orders35[a].order_mnemonic,"{endb} ",dlrec->seq[i].
          orders35[a].clinical_display_line,"  --  COMMENT --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].orders35[a].order_mnemonic,"{endb} ",dlrec->seq[i].
          orders35[a].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      title_string = "Discharge Communication:", title_print, last_title = title_string,
      rowplusone, xcolvar = xcol1
      FOR (a = 1 TO dlrec->seq[i].section4_total)
       tempstring = concat("{b}",dlrec->seq[i].section4_events[a].event_cd_disp,"{endb} ",dlrec->seq[
        i].section4_events[a].result),line_wrap
      ENDFOR
      ...
      FOR (a = 1 TO dlrec->seq[i].diet_total)
        IF ((dlrec->seq[i].diet[a].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].diet[a].order_mnemonic,"{endb} ",dlrec->seq[i].diet[
          a].clinical_display_line,"  -- COMMENT --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].diet[a].order_mnemonic,"{endb} ",dlrec->seq[i].diet[
          a].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      ...
      FOR (a = 1 TO dlrec->seq[i].resp_total)
        IF ((dlrec->seq[i].resp[a].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].resp[a].order_mnemonic,"{endb} ",dlrec->seq[i].resp[
          a].clinical_display_line,"  -- COMMENT --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].resp[a].order_mnemonic,"{endb} ",dlrec->seq[i].resp[
          a].clinical_display_line), line_wrap
      ...
      FOR (a = 1 TO dlrec->seq[i].asmt_total)
        IF ((dlrec->seq[i].asmt[a].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].asmt[a].order_mnemonic,"{endb} ",dlrec->seq[i].asmt[
          a].clinical_display_line,"  --  COMMENT  --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].asmt[a].order_mnemonic,"{endb} ",dlrec->seq[i].asmt[
          a].clinical_display_line), line_wrap
      ...
      FOR (a = 1 TO dlrec->seq[i].ltd_total)
        IF ((dlrec->seq[i].ltd[a].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].ltd[a].order_mnemonic,"{endb} ",dlrec->seq[i].ltd[a]
          .clinical_display_line,"  --  COMMENT  --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].ltd[a].order_mnemonic,"{endb} ",dlrec->seq[i].ltd[a]
          .clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      ...
      FOR (a = 1 TO dlrec->seq[i].number_of_meds)
        ...
        "{endb}", "   ", dlrec->seq[i].meds[a].need_rx_verify_str
        IF ((dlrec->seq[i].meds[a].order_comment_ind=1))
         CALL print("  --  COMMENT  --  ")
        ENDIF
        breakflag = 0, rowplusone2, tempstring = dlrec->seq[i].meds[a].display_line,
      ...
      rowplusone, xcolvar = xcol1
      FOR (a = 1 TO dlrec->seq[i].rorders_total)
        IF ((dlrec->seq[i].rorders[a].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].rorders[a].order_mnemonic,"{endb} ",dlrec->seq[i].
          rorders[a].clinical_display_line,"  --  COMMENT  --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].rorders[a].order_mnemonic,"{endb} ",dlrec->seq[i].
          rorders[a].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      rowplusone,
      ...
      FOR (a = 1 TO dlrec->seq[i].cporders_total)
        IF ((dlrec->seq[i].cporders[a].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].cporders[a].order_mnemonic,"{endb} ",dlrec->seq[i].
          cporders[a].clinical_display_line,"  --  COMMENT  --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].cporders[a].order_mnemonic,"{endb} ",dlrec->seq[i].
          cporders[a].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      ...
      FOR (a = 1 TO dlrec->seq[i].d0events_total)
        IF ((dlrec->seq[i].d0events[a].header1_txt != last_header1_txt))
         last_header1_txt = dlrec->seq[i].d0events[a].header1_txt, title_string = dlrec->seq[i].
         d0events[a].header1_txt, last_title = title_string,
         title_print, rowplusone
        ENDIF
        IF ((dlrec->seq[i].d0events[a].header2_txt != last_header2_txt))
         last_header2_txt = dlrec->seq[i].d0events[a].header2_txt, rowplusone,
         CALL print(calcpos(xcol1,yrow1)),
         "{b}{u} ", last_header2_txt, " {endu}{endb}",
         last_title = concat("Documentation: ",last_header2_txt), rowplusone
        ENDIF
        IF ((dlrec->seq[i].d0events[a].parent_event > " "))
         IF ((dlrec->seq[i].d0events[a].parent_event != last_parent_event))
          last_parent_event = dlrec->seq[i].d0events[a].parent_event
          IF ((dlrec->seq[i].d0events[a].parent_event != dlrec->seq[i].d0events[a].event_cd_disp))
          ...
          ENDIF
         ENDIF
         tempstring = concat("{b}   ",dlrec->seq[i].d0events[a].event_cd_disp,"{endb} ",dlrec->seq[i]
          .d0events[a].result,"   (",
          dlrec->seq[i].d0events[a].event_dt_tm,")"), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].d0events[a].event_cd_disp,"{endb} ",dlrec->seq[i].
          d0events[a].result,"   (",
          dlrec->seq[i].d0events[a].event_dt_tm,")"), line_wrap
