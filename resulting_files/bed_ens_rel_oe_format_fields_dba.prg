CREATE PROGRAM bed_ens_rel_oe_format_fields:dba
 FREE SET reply
 RECORD reply(
   1 error_msg = vc
   1 status_data
 ...
 FOR (o = 1 TO oef_cnt)
  ...
  FOR (x = 1 TO flist_cnt)
    ...
    FOR (f = 1 TO fld_cnt)
     IF ((((request->oef_list[o].flist[x].fld_list[f].field_type=6)) OR ((((request->oef_list[o].
     flist[x].fld_list[f].field_type=9)) OR ((request->oef_list[o].flist[x].fld_list[f].field_type=12
     ))) )) )
     ...
     INSERT  FROM accept_format_flexing aff
      SET aff.oe_format_id = request->oef_list[o].oe_format_id, aff.oe_field_id = request->oef_list[o
       ...
       flex_accept_flag,
       aff.default_value = default_value, aff.updt_cnt = 0, aff.updt_dt_tm = cnvtdatetime(curdate,
        curtime),
       aff.updt_id = reqinfo->updt_id, aff.updt_task = reqinfo->updt_task, aff.updt_applctx = reqinfo
       ->updt_applctx,
       aff.default_parent_entity_name = default_parent_entity_name, aff.default_parent_entity_id =
       default_parent_entity_id, aff.flex_parent_entity_name = " ",
