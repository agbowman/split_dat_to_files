CREATE PROGRAM bmdi_add_acquired_results:dba
 RECORD reply(
   1 result_list[*]
     2 result_id = f8
     2 device_id = f8
 ...
 INSERT  FROM bmdi_acquired_results bar,
  ...
  SET bar.result_id = request->result_list[d.seq].result_id, bar.device_id = request->result_list[d
   ...
   result_list[d.seq].result_format_cd, bar.result_val = request->result_list[d.seq].result_val,
...
#get_data_partialsuccess
 IF (failure="P")
  ...
  SET reply->status_data.subeventstatus[1].targetobjectname = "bmdi_add_acquired_results"
  ...
  IF (ierrcode > 0)
   ...
   SET reply->status_data.subeventstatus[2].targetobjectname = "bmdi_add_acquired_results"
