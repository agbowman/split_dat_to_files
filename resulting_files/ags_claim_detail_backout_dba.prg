CREATE PROGRAM ags_claim_detail_backout:dba
 PROMPT
  "Backout Type (J, I, P) =" = "J",
  "AGS_JOB_ID (0.0) = " = 0.0
  WITH cbackout_type, djob_id
 CALL echo("***")
 CALL echo("***   BEG :: AGS_CLAIM_DETAIL_BACKOUT")
 CALL echo("***")
 ...
 IF ( NOT (validate(log,0)))
  ...
  DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_claim_detail_backout_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_CLAIM_DETAIL_BACKOUT"
 ...
 FREE RECORD claim_detail_di
 RECORD claim_detail_di(
   1 qual_knt = i4
   1 qual[*]
     2 item_id = f8
 )
 ...
 IF (the_type="P"
  ...
  IF (size(person_rec->qual,5) < 1)
   ...
   GO TO exit_script
  ENDIF
  SET stat = initrec(claim_detail_di)
  ...
  SELECT INTO "nl:"
   ...
   HEAD REPORT
    sknt = 0, stat = alterlist(claim_detail_di->qual,10)
   DETAIL
    sknt = (sknt+ 1)
    IF (mod(sknt,10)=1
     AND sknt != 1)
     stat = alterlist(claim_detail_di->qual,(sknt+ 9))
    ENDIF
    claim_detail_di->qual[sknt].item_id = a.hea_claim_visit_detail_id
   FOOT REPORT
    claim_detail_di->qual_knt = sknt, stat = alterlist(claim_detail_di->qual,sknt)
   WITH nocounter
  ;end select
  SET ierrcode = error(serrmsg,1)
  IF (ierrcode > 0)
   ...
   SET log->qual[log->qual_knt].smsgtype = "ERROR"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("SELECT ERROR HEA_CLAIM_VISIT_DETAIL >> ",trim(serrmsg)
    )
   GO TO exit_script
  ENDIF
  IF ((claim_detail_di->qual_knt > 0))
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   DELETE  FROM hea_claim_visit_detail h,
     (dummyt d  WITH seq = value(claim_detail_di->qual_knt))
    SET h.seq = 1
    PLAN (d
     WHERE d.seq > 0
      AND (claim_detail_di->qual[d.seq].item_id > 0))
     JOIN (h
     WHERE (h.hea_claim_visit_detail_id=claim_detail_di->qual[d.seq].item_id))
  ...
  UPDATE  FROM ags_claim_detail_data a,
   ...
   SET a.status = "BACK OUT", a.status_dt_tm = cnvtdatetime(curdate,curtime3), a
    ...
    a.hea_claim_visit_id = 0.0, a.person_id = 0.0
   PLAN (d
    WHERE d.seq > 0
     AND (person_rec->qual[d.seq].validated_ind > 0)
     AND (person_rec->qual[d.seq].person_id > 0))
  ...
  IF (ierrcode > 0)
   ...
   SET log->qual[log->qual_knt].smsgtype = "ERROR"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_CLAIM_DETAIL_DATA >> ",trim(serrmsg))
   GO TO exit_script
  ENDIF
 ELSEIF (the_type="J"
  AND the_job_id > 0.0)
  ...
  DELETE  FROM hea_claim_visit_detail h
   SET h.seq = 1
   PLAN (h
    WHERE (h.hea_claim_visit_detail_id=
    (SELECT
     hea_claim_visit_detail_id
     FROM ags_claim_detail_data
     WHERE ags_job_id=the_job_id
      AND hea_claim_visit_detail_id > 0))
     AND h.hea_claim_visit_detail_id > 0)
   WITH nocounter, maxcommit = 5000
  ;end delete
  SET ierrcode = error(serrmsg,1)
  IF (ierrcode > 0)
 ...
 ELSEIF (the_type="I")
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    JOIN (a
    WHERE (a.ags_claim_detail_data_id=item_rec->qual[d.seq].data_id)
     AND a.hea_claim_visit_detail_id > 0)
   HEAD REPORT
    sknt = 0, stat = alterlist(claim_detail_di->qual,10)
   DETAIL
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_CLAIM_DETAIL_BACKOUT"
 IF (define_logging_sub=true)
  CALL echo("***")
 ...
 ENDIF
 CALL echo("***")
 CALL echo("***   END :: AGS_CLAIM_DETAIL_BACKOUT")
 CALL echo("***")
