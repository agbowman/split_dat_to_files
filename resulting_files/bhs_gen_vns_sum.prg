CREATE PROGRAM bhs_gen_vns_sum
 RECORD work(
   1 person_id = f8
   1 encntr_id = f8
   1 f_cnt = i4
 ...
 DECLARE cs14003_vns_summary_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",14003,"VNSSUMMARY"))
 DECLARE var_warning = vc WITH noconstant(build2(beg_bold,
   ...
   "Please use the flowsheet to view older ",trim(uar_get_code_display(cs14003_vns_summary_cd)),
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dfa
   WHERE (work->person_id=dfa.person_id)
    AND dfa.description="Vagus Nerve Stimulation Flow Sheet"
    AND dfa.flags > 0
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    WHERE operator(ce.reference_nbr,"LIKE",patstring(work->forms[dseq].reference_nbr,1))
     AND ce.task_assay_cd=cs14003_vns_summary_cd
     AND ce.view_level=1
