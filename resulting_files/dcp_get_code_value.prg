CREATE PROGRAM dcp_get_code_value
 RECORD reply(
   1 code_cnt = i2
   1 code_list[*]
     2 code_value = f8
 ...
 SET code_cnt = 0
 SET code_nbr = cnvtint(size(request->code_list,5))
 ...
 IF ((request->mode_flag=1))
  SELECT INTO "nl:"
   ...
   FROM code_value c
   PLAN (c
    WHERE expand(indx,1,size(request->code_list,5),c.code_value,request->code_list[indx].code_value))
   DETAIL
   ...
   WITH counter
  ;end select
 ELSEIF ((request->mode_flag=3))
  SELECT INTO "nl:"
   c.*
   FROM code_value c
   PLAN (c
    WHERE expand(indx,1,size(request->code_list,5),c.code_set,request->code_list[indx].code_set)
     AND expand(indx,1,size(request->code_list,5),c.display,request->code_list[indx].code_display))
   DETAIL
    code_cnt = (code_cnt+ 1), stat = alterlist(reply->code_list,code_cnt), reply->code_list[code_cnt]
    .code_value = c.code_value,
    reply->code_list[code_cnt].code_set = c.code_set, reply->code_list[code_cnt].display = c.display,
 ...
 ELSE
  SELECT INTO "nl:"
   ...
   PLAN (c
    WHERE expand(indx,1,size(request->code_list,5),c.code_set,request->code_list[indx].code_set)
     AND expand(indx,1,size(request->code_list,5),c.cdf_meaning,request->code_list[indx].code_meaning
     ))
   DETAIL
    code_cnt = (code_cnt+ 1), stat = alterlist(reply->code_list,code_cnt), reply->code_list[code_cnt]
    .code_value = c.code_value,
