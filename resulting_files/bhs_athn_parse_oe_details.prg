CREATE PROGRAM bhs_athn_parse_oe_details
 ...
 FOR (idx = 1 TO oeblockcnt)
   ...
   IF (((idx=1) OR (oefieldcntvalidind=1)) )
      ...
      IF (startpos < endpos)
       ...
       IF (size(param) > 0)
        ...
        ELSEIF (oefieldcnt > 3)
         ...
         CALL echo(
          "CHECK THAT OEFIELDS CONTAINING RESERVED CHARACTERS USE APPROPRIATE ESCAPE SEQUENCE")
         GO TO exit_script
        ENDIF
       ENDIF
      ENDIF
    ...
    IF (size(block)=0
     AND oefieldcnt=2)
     SET reply->detaillist[idx].oefielddisplayvalue = ""
     SET oefieldcntvalidind = 1
    ENDIF
   ENDIF
 ENDFOR
