CREATE PROGRAM bhs_athn_inbox_msg_save_v3
 FREE RECORD result
 RECORD result(
   1 modify_ind = i2
   1 msg_complete_ind = i2
   1 reschedule_ind = i2
   1 message = vc
   1 subject = vc
   1 from_line = vc
 ...
 RECORD req967501(
   1 mod_list[*]
     ...
     2 reminder_dt_tm = dq8
     2 due_dt_tm = dq8
     2 proxy_action_prsnl_id = f8
     2 inactivate_ind = i2
     2 redirect_ind = i2
     2 redirect_receiver_list[*]
       3 receiver_id = f8
       3 assign_prsnl_id = f8
       3 assign_pool_id = f8
     2 encntr_id = f8
 ...
 DECLARE callupdnotification(null) = i4
 ...
 DECLARE buildmessageblob(null) = i4
 ...
 DECLARE c_msg_type_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",6026,trim( $25,3)))
 ...
 SET result->modify_ind = evaluate( $23,0.0,0,1)
 SET result->msg_complete_ind =  $28
 SET result->reschedule_ind =  $29
 IF (textlen(trim( $4,3)) > 0
  AND cnvtdatetime( $4) > 0.0)
 ...
 IF (( $3 <= 0.0))
  ...
  GO TO exit_script
 ELSEIF (((textlen(trim( $25,3)) <= 0) OR (c_msg_type_cd <= 0.0)) )
  CALL echo("INVALID MESSAGE TYPE PARAMETER...EXITING")
  GO TO exit_script
 ENDIF
 SET result->pool_ind = evaluate( $32,0.0,0,1)
 ...
 SET result->notify.completed_ind = cnvtint( $27)
 IF ((result->modify_ind=0))
  SET result->task_type_cd = c_msg_type_cd
  SET result->event_cd =  $24
 ELSE
  SELECT INTO "NL:"
   ...
   HEAD ta.task_id
    IF (( $24 <= 0.0))
     result->event_cd = ta.event_cd
    ELSE
     result->event_cd =  $24
    ENDIF
    result->task_type_cd = ta.task_type_cd
 ...
 SET result->task_uid = concat("urn:cerner:mid:object.task:",trim(cnvtlower(curdomain),3),":taskId=",
  trim(replace(cnvtstring( $23),".00","",0),3),",ownerId=",
  evaluate(result->pool_ind,1,trim(replace(cnvtstring( $32),".00","",0),3),trim(replace(cnvtstring(
       $3),".00","",0),3)),",enum=",evaluate(result->task_type_cd,c_reminder_msg_cd,"-11",
   c_consult_msg_cd,"-10",
   "5"),",poolInd=",trim(cnvtstring(result->pool_ind),3))
 ...
 IF (textlen(trim( $5,3)))
  ...
  EXECUTE bhs_athn_format_str_param  WITH replace("REQUEST","REQ_FORMAT_STR"), replace("REPLY",
   "REP_FORMAT_STR")
  SET result->message = rep_format_str->param
 ENDIF
 IF (textlen(trim( $9,3)))
 ...
 ENDWHILE
 DECLARE topoolidparam = vc WITH protect, noconstant("")
 DECLARE topoolcnt = i4 WITH protect, noconstant(0)
 SET startpos = 1
 SET topoolidparam = trim( $30,3)
 CALL echo(build2("TOPOOLIDPARAM IS: ",topoolidparam))
 WHILE (size(topoolidparam) > 0)
   SET endpos = (findstring(";",topoolidparam,1) - 1)
   IF (endpos <= 0)
    SET endpos = size(topoolidparam)
   ENDIF
   CALL echo(build("ENDPOS:",endpos))
   IF (startpos < endpos)
    SET param = substring(1,endpos,topoolidparam)
    CALL echo(build("PARAM:",param))
    SET topoolcnt = (topoolcnt+ 1)
    SET stat = alterlist(result->to_pool,topoolcnt)
    SET result->to_pool[topoolcnt].pool_id = cnvtreal(param)
   ENDIF
   SET topoolidparam = substring((endpos+ 2),(size(topoolidparam) - endpos),topoolidparam)
   CALL echo(build("TOPOOLIDPARAM:",topoolidparam))
   CALL echo(build("SIZE(TOPOOLIDPARAM):",size(topoolidparam)))
 ENDWHILE
 DECLARE ccpoolidparam = vc WITH protect, noconstant("")
 DECLARE ccpoolcnt = i4 WITH protect, noconstant(0)
 SET startpos = 1
 SET ccpoolidparam = trim( $31,3)
 CALL echo(build2("CCPOOLIDPARAM IS: ",ccpoolidparam))
 WHILE (size(ccpoolidparam) > 0)
   SET endpos = (findstring(";",ccpoolidparam,1) - 1)
   IF (endpos <= 0)
    SET endpos = size(ccpoolidparam)
   ENDIF
   CALL echo(build("ENDPOS:",endpos))
   IF (startpos < endpos)
    SET param = substring(1,endpos,ccpoolidparam)
    CALL echo(build("PARAM:",param))
    SET ccpoolcnt = (ccpoolcnt+ 1)
    SET stat = alterlist(result->cc_pool,ccpoolcnt)
    SET result->cc_pool[ccpoolcnt].pool_id = cnvtreal(param)
   ENDIF
   SET ccpoolidparam = substring((endpos+ 2),(size(ccpoolidparam) - endpos),ccpoolidparam)
   CALL echo(build("CCPOOLIDPARAM:",ccpoolidparam))
   CALL echo(build("SIZE(CCPOOLIDPARAM):",size(ccpoolidparam)))
 ENDWHILE
 ...
 IF ((result->modify_ind=1))
  ...
  IF (stat=fail)
   GO TO exit_script
  ENDIF
  IF ((result->msg_complete_ind=0))
   SET stat = buildmessageblob(null)
   IF (stat=fail)
    GO TO exit_script
   ENDIF
  ENDIF
  SET stat = callupdnotification(null)
  IF (stat=fail)
   GO TO exit_script
  ENDIF
 ELSE
...
#exit_script
 ...
 SUBROUTINE calladdnotification(null)
   ...
   DECLARE selcnt = i4 WITH protect, noconstant(0)
   FREE RECORD i_request
   RECORD i_request(
     1 prsnl_id = f8
   ) WITH protect
   ...
   FOR (idx = 1 TO size(result->to_prsnl,5))
     ...
     SET req967503->message_list[1].assign_prsnl_list[apcnt].selection_nbr = selcnt
   ENDFOR
   ...
   ENDFOR
   SET apcnt = 0
   SET stat = alterlist(req967503->message_list[1].assign_pool_list,(size(result->to_pool,5)+ size(
     result->cc_pool,5)))
   FOR (idx = 1 TO size(result->to_pool,5))
     SET apcnt = (apcnt+ 1)
     SET selcnt = (selcnt+ 1)
     SET req967503->message_list[1].assign_pool_list[apcnt].assign_pool_id = result->to_pool[idx].
     pool_id
     SET req967503->message_list[1].assign_pool_list[apcnt].cc_ind = 0
     SET req967503->message_list[1].assign_pool_list[apcnt].selection_nbr = selcnt
   ENDFOR
   FOR (idx = 1 TO size(result->cc_pool,5))
     SET apcnt = (apcnt+ 1)
     SET selcnt = (selcnt+ 1)
     SET req967503->message_list[1].assign_pool_list[apcnt].assign_pool_id = result->cc_pool[idx].
     pool_id
     SET req967503->message_list[1].assign_pool_list[apcnt].cc_ind = 1
     SET req967503->message_list[1].assign_pool_list[apcnt].selection_nbr = selcnt
   ENDFOR
   ...
   SET req967503->message_list[1].msg_sender_pool_id =  $32
   IF ((result->task_type_cd != c_consult_msg_cd))
    SET req967503->message_list[1].responsible_prsnl_id =  $3
   ENDIF
   ...
   ELSE
    SET req967503->message_list[1].msg_text = result->message_blob
   ENDIF
   SET req967503->message_list[1].order_id = result->order_id
   SET req967503->message_list[1].event_cd = result->event_cd
   SET req967503->message_list[1].original_task_uid = evaluate(result->modify_ind,1,result->task_uid,
    "")
   ...
   RETURN(fail)
 END ;Subroutine
 SUBROUTINE callupdnotification(null)
   DECLARE applicationid = i4 WITH constant(600005)
   DECLARE taskid = i4 WITH constant(967100)
   DECLARE requestid = i4 WITH constant(967501)
   DECLARE errmsg = vc WITH protect, noconstant("")
   FREE RECORD i_request
   RECORD i_request(
     1 prsnl_id = f8
   ...
   IF ((i_reply->status_data.status != "S"))
    ...
    RETURN(fail)
   ENDIF
   SET stat = alterlist(req967501->mod_list,1)
   SET req967501->mod_list[1].notification_uid = result->task_uid
   SET req967501->mod_list[1].save_to_chart_ind = result->save_to_chart_ind
   SET req967501->mod_list[1].task_updt_cnt = result->task_updt_cnt
   SET req967501->mod_list[1].assign_updt_cnt = result->assign_updt_cnt
   IF ((result->msg_complete_ind=1))
    SET req967501->mod_list[1].task_status_cd = c_complete_cd
    SET req967501->mod_list[1].action_type.update_type = 1
   ELSE
    SET req967501->mod_list[1].event_cd = result->event_cd
    SET req967501->mod_list[1].task_status_cd = result->task_status_cd
    SET req967501->mod_list[1].msg_text = result->message_blob
    IF ((result->reschedule_ind=1))
     SET req967501->mod_list[1].task_status_cd = c_pending_cd
     SET req967501->mod_list[1].action_type.update_type = 0
    ENDIF
   ENDIF
   IF (textlen(trim( $11,3)) > 0
    AND cnvtdatetime( $11) > 0.0)
    SET req967501->mod_list[1].reminder_dt_tm = cnvtdatetime( $11)
   ENDIF
   IF (textlen(trim( $12,3)) > 0
    AND cnvtdatetime( $12) > 0.0)
    SET req967501->mod_list[1].due_dt_tm = cnvtdatetime( $12)
   ENDIF
   SET req967501->action_dt_tm = result->performed_dt_tm
   SET req967501->action_tz = app_tz
   CALL echorecord(req967501)
   SET stat = tdbexecute(applicationid,taskid,requestid,"REC",req967501,
    "REC",rep967501,1)
   IF (stat > 0)
    SET errcode = error(errmsg,1)
    CALL echo(build("TDBEXECUTE ",requestid," - ERROR CODE: ",errcode," - ERROR MESSAGE: ",
    ...
    RETURN(fail)
   ENDIF
   CALL echorecord(rep967501)
   IF ((rep967501->status_data.status="S"))
    RETURN(success)
   ENDIF
   RETURN(fail)
 END ;Subroutine
 SUBROUTINE callgetmessagetext(null)
   ...
   IF ((rep967511->status_data.status="S"))
    IF (size(rep967511->get_list_item,5) > 0)
     ...
     SET result->task_status_cd = rep967511->get_list_item[1].status_cd
     SET result->task_updt_cnt = rep967511->get_list_item[1].updt_cnt
     SET result->assign_updt_cnt = rep967511->get_list_item[1].owner_updt_cnt
     SET result->parent_event.event_id = rep967511->get_list_item[1].event_id
     IF ((result->parent_event.event_id=0))
      SET result->save_to_chart_ind =  $22
     ENDIF
     IF ((result->modify_ind=1)
      AND (result->parent_event.event_id=0))
      CALL echo(build("REP967511->GET_LIST_ITEM[1].TEXT:",rep967511->get_list_item[1].text))
      SET startpos = findstring("\plain\f1\fs20\cf1",rep967511->get_list_item[1].text,1)
      SET endpos = findstring("\par\pard\itap",rep967511->get_list_item[1].text,startpos)
      IF (((startpos <= 0) OR (endpos <= 0)) )
       SET startpos = findstring("---------------------",rep967511->get_list_item[1].text,1)
       SET endpos = findstring("\par \par}",rep967511->get_list_item[1].text,startpos)
      ENDIF
      IF (startpos > 0
       AND endpos > 0)
      ...
      ELSE
       CALL echo("ORIG_MSG_BODY NOT FOUND IN 967511...EXITING")
       RETURN(fail)
      ENDIF
     ENDIF
    ENDIF
 ...
 SUBROUTINE calleventensureserver(null)
   ...
   SET iret = uar_srvsetdouble(hblobresultitem,"FORMAT_CD",c_rtf_cd)
   SET hblobitem = uar_srvadditem(hblobresultitem,"BLOB")
   SET iret = uar_srvsetasis(hblobitem,"BLOB_CONTENTS",result->message_blob,size(result->message_blob,
     1))
   SET iret = uar_srvsetlong(hblobitem,"BLOB_LENGTH",size(result->message_blob,1))
   SET hprsnllistitem = uar_srvadditem(hchildlistitem,"EVENT_PRSNL_LIST")
 ...
 END ;Subroutine
 SUBROUTINE buildmessageblob(null)
   ...
   DECLARE new_event_title_text_temp = vc WITH protect, noconstant("")
   IF ((result->parent_event.event_id=0))
    SET new_event_title_text_temp = "Entered by"
   ELSE
    SET new_event_title_text_temp = "Addendum by"
   ENDIF
   SET result->new_event_title_text = concat(new_event_title_text_temp," ",result->
    performed_prsnl_name_last,"  , ",result->performed_prsnl_name_first,
    " on ",performed_date_str)
   ...
   IF ((result->modify_ind=0))
    ...
    IF (size(result->to_prsnl,5) > 0)
     SELECT INTO "NL:"
      ...
      ORDER BY p.person_id
      HEAD p.person_id
       pos = locateval(locidx,1,size(result->to_prsnl,5),p.person_id,result->to_prsnl[locidx].
        prsnl_id)
       IF (pos > 0)
     ...
     ;end select
    ENDIF
    IF (size(result->to_pool,5) > 0)
     SELECT INTO "NL:"
      FROM prsnl_group pg
      PLAN (pg
       WHERE expand(idx,1,size(result->to_pool,5),pg.prsnl_group_id,result->to_pool[idx].pool_id)
        AND pg.active_ind=1
        AND pg.beg_effective_dt_tm < sysdate
        AND pg.end_effective_dt_tm > sysdate)
      ORDER BY pg.prsnl_group_id
      HEAD pg.prsnl_group_id
       pos = locateval(locidx,1,size(result->to_pool,5),pg.prsnl_group_id,result->to_pool[locidx].
        pool_id)
       IF (pos > 0)
        result->to_pool[pos].name = pg.prsnl_group_name
       ENDIF
      WITH nocounter, expand = 1, time = 30
     ;end select
    ENDIF
    IF (((size(result->to_prsnl,5) > 0) OR (size(result->to_pool,5) > 0)) )
     SET result->to_line = " To:"
     FOR (idx = 1 TO size(result->to_prsnl,5))
       SET result->to_line = concat(result->to_line," ",trim(result->to_prsnl[idx].name,3),";")
     ENDFOR
     FOR (idx = 1 TO size(result->to_pool,5))
       SET result->to_line = concat(result->to_line," ",trim(result->to_pool[idx].name,3),";")
     ENDFOR
    ...
    IF (size(result->cc_prsnl,5) > 0)
     SELECT INTO "NL:"
      ...
      ORDER BY p.person_id
      HEAD p.person_id
       pos = locateval(locidx,1,size(result->cc_prsnl,5),p.person_id,result->cc_prsnl[locidx].
        prsnl_id)
       IF (pos > 0)
     ...
     ;end select
    ENDIF
    IF (size(result->cc_pool,5) > 0)
     SELECT INTO "NL:"
      FROM prsnl_group pg
      PLAN (pg
       WHERE expand(idx,1,size(result->cc_pool,5),pg.prsnl_group_id,result->cc_pool[idx].pool_id)
        AND pg.active_ind=1
        AND pg.beg_effective_dt_tm < sysdate
        AND pg.end_effective_dt_tm > sysdate)
      ORDER BY pg.prsnl_group_id
      HEAD pg.prsnl_group_id
       pos = locateval(locidx,1,size(result->cc_pool,5),pg.prsnl_group_id,result->cc_pool[locidx].
        pool_id)
       IF (pos > 0)
        result->cc_pool[pos].name = pg.prsnl_group_name
       ENDIF
      WITH nocounter, expand = 1, time = 30
     ;end select
    ENDIF
    IF (((size(result->cc_prsnl,5) > 0) OR (size(result->cc_pool,5) > 0)) )
     SET result->cc_line = " Cc:"
     FOR (idx = 1 TO size(result->cc_prsnl,5))
       SET result->cc_line = concat(result->cc_line," ",trim(result->cc_prsnl[idx].name,3),";")
     ENDFOR
     FOR (idx = 1 TO size(result->cc_pool,5))
       SET result->cc_line = concat(result->cc_line," ",trim(result->cc_pool[idx].name,3),";")
     ENDFOR
    ...
    ENDIF
    SET result->sent_line = concat(" Sent: ",trim(formatnotedate(result->performed_dt_tm,1),3),
     evaluate( $6,c_stat_cd," !\par","\par"))
    IF (size(result->actions,5) > 0)
     SET result->action_line = " \b Actions:"
     FOR (idx = 1 TO size(result->actions,5))
       SET result->action_line = concat(result->action_line,evaluate(idx,1," ",", "),result->actions[
        idx].action_disp)
     ENDFOR
    ...
    ENDIF
    SET result->message_blob = nullterm(concat(
      ...
      result->due_line,"\pard\itap0\nowidctlpar\plain\f1\fs20\cf1\par ",result->message,"\par \par}")
     )
   ELSE
    IF ((result->parent_event.event_id=0)
     AND textlen(trim(result->orig_msg_body,3)) > 0)
     SET result->message_blob = nullterm(concat(
       "{\rtf1\ansi\ansicpg1252\uc1\deff0{\fonttbl  {\f0\fnil\fcharset0\fprq2 Arial;}  {\",
       "f1\fswiss\fcharset0\fprq2 Arial;}  {\f2\froman\fcharset2\fprq2 Symbol;}}  {\colo",
       "rtbl;\red0\green0\blue0;}  {\stylesheet{\s0\itap0\nowidctlpar\f0\fs24 [Normal];}",
       ...
       "pard\itap0\nowidctlpar\tx720\tx1440\tx2160\tx2880\tx3600\tx4320\tx5040\tx5760\tx",
       "6480\tx7200\tx7920\tx8640\tx9360\tx10080\plain\f1\fs20 \par\par ",result->
       new_event_title_text,
       " \par ",result->message,"\par\par \par}"))
     SET result->message_blob = nullterm(concat(substring(1,(size(result->message_blob) - 1),result->
        message_blob),result->orig_msg_body))
    ELSE
     SET result->message_blob = nullterm(concat(
       "{\rtf1\ansi\ansicpg1252\uc1\deff0{\fonttbl  {\f0\fnil\fcharset0\fprq2 Arial;}  {\",
       "f1\fswiss\fcharset0\fprq2 Arial;}  {\f2\froman\fcharset2\fprq2 Symbol;}}  {\colo",
       ...
       "000\pghsxn15840\marglsxn1440\margtsxn1440\margrsxn1440\margbsxn1440\pgbrdropt32\",
       "pard\itap0\nowidctlpar\tx720\tx1440\tx2160\tx2880\tx3600\tx4320\tx5040\tx5760\tx",
       "6480\tx7200\tx7920\tx8640\tx9360\tx10080\plain\f1\fs20 \par ",result->message,
       "\par\par \par}"))
    ENDIF
   ENDIF
   CALL echo(build("RESULT->MESSAGE_BLOB:",result->message_blob))
   RETURN(success)
 END ;Subroutine
 SUBROUTINE calladdreminder(null)
   ...
   SET req967731->action_pool_id =  $32
   ...
   ELSE
    SET req967731->reminders[1].text = result->message_blob
   ENDIF
   ...
   SET stat = alterlist(req967731->reminders[1].recipients,(((size(result->to_prsnl,5)+ size(result->
     cc_prsnl,5))+ size(result->to_pool,5))+ size(result->cc_pool,5)))
   FOR (idx = 1 TO size(result->to_prsnl,5))
     SET apcnt = (apcnt+ 1)
   ...
   FOR (idx = 1 TO size(result->to_pool,5))
     SET apcnt = (apcnt+ 1)
     SET req967731->reminders[1].recipients[apcnt].pool_id = result->to_pool[idx].pool_id
     SET req967731->reminders[1].recipients[apcnt].cc_ind = 0
     SET req967731->reminders[1].recipients[apcnt].selection_nbr = apcnt
   ENDFOR
   FOR (idx = 1 TO size(result->cc_pool,5))
     SET apcnt = (apcnt+ 1)
     SET req967731->reminders[1].recipients[apcnt].pool_id = result->cc_pool[idx].pool_id
     SET req967731->reminders[1].recipients[apcnt].cc_ind = 1
     SET req967731->reminders[1].recipients[apcnt].selection_nbr = apcnt
   ENDFOR
   ...
   FOR (idx = 1 TO size(result->actions,5))
     ...
     action_cd
   ENDFOR
   SET req967731->reminders[1].original_task_uid = evaluate(result->modify_ind,1,result->task_uid,"")
   SET req967731->reminders[1].responsible_prsnl_id =  $3
