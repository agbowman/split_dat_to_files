CREATE PROGRAM dm2_add_default_rows_backfill:dba
 ...
 DECLARE dadrb_idx = i4 WITH protect, noconstant(0)
 DECLARE dadrb_warning_cnt = i4 WITH protect, noconstant(0)
 DECLARE ix1 = i4 WITH protect, noconstant(0)
 DECLARE dadrb_text = vc WITH protect, noconstant(" ")
 DECLARE dadrb_logfile_name = vc WITH protect, noconstant("dm2_adddefrows_bf")
 DECLARE dadrb_linkname = vc WITH protect, noconstant(" ")
 DECLARE dadrb_oragen3_name = vc WITH protect, noconstant(" ")
 FREE RECORD dadrb_tbl
 RECORD dadrb_tbl(
   1 tcnt = i4
   1 tbl[*]
     2 tname = vc
 )
 IF (check_logfile(dadrb_logfile_name,".log","DM2_ADD_DEFAULT_ROWS_BACKFILL.PRG ...") != 1)
  GO TO exit_program
 ENDIF
 CALL echo("** Searching the database for default row table candidates.... **")
 SELECT
  IF (currdb="ORACLE")
   FROM user_tab_columns utc,
    user_constraints uc,
   ...
   (SELECT
    ...
    WHERE utc.table_name=dtc.table_name
     AND dtc.default_row_ind=1)))
  ELSEIF (currdb="DB2UDB")
   FROM dm2_user_tab_columns utc,
    dm2_user_constraints uc,
    dm2_user_cons_columns ucc
   WHERE utc.table_name=uc.table_name
    AND utc.table_name=ucc.table_name
    AND utc.column_name=ucc.column_name
    AND uc.constraint_name=ucc.constraint_name
  ...
  ENDIF
  INTO "nl:"
  ORDER BY utc.table_name, ucc.position DESC
  HEAD REPORT
   tcnt = 0, ccnt = 0
  HEAD utc.table_name
   IF (findstring("$",utc.table_name)=0)
    IF (ucc.position=1
     AND utc.data_type IN ("NUMBER", "FLOAT", "BIGINT", "SMALLINT", "INTEGER",
    "DOUBLE"))
     tcnt = (tcnt+ 1), stat = alterlist(dadrb_tbl->tbl,tcnt), dadrb_tbl->tbl[tcnt].tname = utc
     .table_name
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat(utc.table_name," found with single numeric PK.  Storing for later update."))
     ENDIF
    ELSE
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat("BYPASSING table: ",utc.table_name))
     ENDIF
    ENDIF
   ENDIF
  DETAIL
   ccnt = (ccnt+ 1)
  FOOT REPORT
   stat = alterlist(dadrb_tbl->tbl,tcnt), dadrb_tbl->tcnt = tcnt
  WITH nocounter
 ;end select
 IF (check_error(concat(
   "DM2_ADD_DEFAULT_ROWS_BACKFILL.PRG:  ** Error on Main select to identify default",
   " row table candidates ")) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dadrb_tbl->tcnt > 0))
  FOR (ix1 = 1 TO dadrb_tbl->tcnt)
    CALL echo(concat("Updating : ",dadrb_tbl->tbl[ix1].tname))
    UPDATE  FROM dm_tables_doc_tab dtd
     SET dtd.default_row_ind = 1, dtd.updt_cnt = (dtd.updt_cnt+ 1), dtd.updt_dt_tm = cnvtdatetime(
       curdate,curtime3)
     WHERE (dtd.table_name=dadrb_tbl->tbl[ix1].tname)
     WITH nocounter
    ;end update
    IF (check_error(concat("** DM2_ADD_DEFAULT_ROWS_BACKFILL.PRG:   Error Updating DM_TABLES_DOC ",
      "from dadr_tbl record structure "))=0)
     IF (curqual=0)
      SET dm_err->eproc = concat(" Warning:  ",trim(dadrb_tbl->tbl[ix1].tname),
       " was found with a single",
       " numeric primary key, but was not on dm_tables_doc to update.  Bypassing it....")
      CALL disp_msg("",dm_err->logfile,0)
      SET dadrb_warning_cnt = (dadrb_warning_cnt+ 1)
     ENDIF
     COMMIT
    ELSE
     ROLLBACK
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET ix1 = dadrb_tbl->tcnt
     GO TO exit_program
    ENDIF
  ENDFOR
 ELSE
  SET dm_err->eproc = "** NO exception list tables found for Update on dm_tables_doc. "
  CALL disp_msg("",dm_err->logfile,0)
  SET dadrb_warning_cnt = (dadrb_warning_cnt+ 1)
 ENDIF
#exit_program
 IF (dadrb_warning_cnt > 0)
  SET dm_err->eproc = concat("...ENDING DM2_ADD_DEFAULT_ROWS_BACKFILL.PRG.  ",
   " Warnings were encountered during this program's execution, please check logfile for details.")
 ELSE
  SET dm_err->eproc = "...ENDING DM2_ADD_DEFAULT_ROWS_BACKFILL.PRG"
 ENDIF
 CALL final_disp_msg(dadrb_logfile_name)
 FREE RECORD dadrb_table
 FREE RECORD dadrb_except
END GO
