CREATE PROGRAM cps_del_person_ntwk_prvdr:dba
 ...
 CALL del_person_ntwk_prvdr(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE del_person_ntwk_prvdr(del_begin,del_end)
   FOR (x = del_begin TO del_end)
     ...
     UPDATE  FROM person_ntwk_prvdr p
      SET p.active_ind = false, p.active_status_cd = nullcheck(active_code,request->
        person_ntwk_prvdr[x].active_status_cd,
        IF ((request->person_ntwk_prvdr[x].active_status_cd=0)) 0
        ELSE 1
        ENDIF
