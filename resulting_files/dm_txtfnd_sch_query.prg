CREATE PROGRAM dm_txtfnd_sch_query
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 DECLARE dtsq_cnt = i4 WITH protect, noconstant(0)
 DECLARE dtsq_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dtsq_emsg = vc WITH protect, noconstant(" ")
 DECLARE dtsq_idx = i4 WITH protect, noconstant(0)
 DECLARE dtsq_pos = i4 WITH protect, noconstant(0)
 DECLARE dtsq_num = i4 WITH protect, noconstant(0)
 FREE RECORD dtsq_attrib
 RECORD dtsq_attrib(
   1 cnt = i4
   1 qual[*]
     2 column_name = vc
 ...
 IF (check_logfile("dm_tf_sch_qry",".log","DM_TXTFND_SCH_QUERY LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_TXTFND_SCH_QUERY..."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (dtla_detail_id=0.0)
  ...
  SELECT INTO "nl:"
   ...
   WHERE d.detail_type_flag=2
    AND d.active_ind=1
    AND d.detail_script_name="DM_TXTFND_SCH_QUERY"
   DETAIL
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "No active data found in DM_TEXT_FIND_DETAIL for script DM_TXTFND_SCH_QUERY."
   IF ((script_rec->dm_text_find_log_id > 0.0))
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
   ENDIF
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_TXTFND_ATTRIBUTE:SCH_QUERY"
   AND  EXISTS (
  (SELECT
   "x"
   FROM user_tab_columns utc
   WHERE utc.table_name="SCH_QUERY"
    AND utc.column_name=di.info_name))
  ORDER BY di.info_name
  DETAIL
   dtsq_attrib->cnt = (dtsq_attrib->cnt+ 1), stat = alterlist(dtsq_attrib->qual,dtsq_attrib->cnt),
   dtsq_attrib->qual[dtsq_attrib->cnt].column_name = di.info_name,
   dtsq_attrib->qual[dtsq_attrib->cnt].column_label = di.info_char, dtsq_attrib->qual[dtsq_attrib->
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No attribute data found for SCH_QUERY."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for Attribute list:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ...
 SET dm_err->eproc = "Gathering Scheduling Inquiry scripts"
 SELECT DISTINCT INTO "nl:"
  sq.program_name
  FROM sch_query sq
  WHERE sq.program_name IS NOT null
   AND sq.program_name > " "
  HEAD REPORT
   script_rec->dm_text_find_detail_id = dtla_detail_id, dtsq_cnt = 0
  DETAIL
   dtsq_cnt = (dtsq_cnt+ 1)
   IF (mod(dtsq_cnt,10)=1)
    stat = alterlist(dtg->qual,(dtsq_cnt+ 9))
   ENDIF
   dtg->qual[dtsq_cnt].s_name = sq.program_name
  FOOT REPORT
   dtg->cnt = dtsq_cnt, stat = alterlist(dtg->qual,dtg->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for Scheduling Inquiry scripts:",dm_err->emsg),0.0,0.0)
  SET dtsq_err_ind = dm_err->err_ind
  SET dtsq_emsg = dm_err->emsg
  SET dm_err->err_ind = 0
 ENDIF
 IF ((dtg->cnt > 0)
  AND dtsq_err_ind=0)
  SET dm_err->eproc = "Gathering attribute information from SCH_QUERY"
  CALL parser("select into 'nl:'",0)
  CALL parser("  from sch_query sq",0)
  CALL parser(" where expand(dtsq_num,1,dtg->cnt,sq.program_name,dtg->qual[dtsq_num].s_name)",0)
  CALL parser(" order by sq.program_name, sq.updt_dt_tm desc")
  CALL parser("head sq.program_name",0)
  CALL parser(
   "   dtsq_pos = locateval(dtsq_num,1,dtg->cnt,sq.program_name,dtg->qual[dtsq_num].s_name) ",0)
  CALL parser("detail",0)
  CALL parser("  if (dtg->qual[dtsq_pos].data_source <= ' ')",0)
  CALL parser("   dtg->qual[dtsq_pos].data_source = sq.mnemonic",0)
  FOR (dtsq_idx = 1 TO dtsq_attrib->cnt)
    CALL parser("   dtg->qual[dtsq_pos].att_cnt = dtg->qual[dtsq_pos].att_cnt + 1",0)
    CALL parser("   stat = alterlist(dtg->qual[dtsq_pos].att_qual, dtg->qual[dtsq_pos].att_cnt)",0)
    CALL parser(concat(
      "      dtg->qual[dtsq_pos].att_qual[dtg->qual[dtsq_pos].att_cnt].tab_name = 'SCH_QUERY'"),0)
    CALL parser(concat(
      "   dtg->qual[dtsq_pos].att_qual[dtg->qual[dtsq_pos].att_cnt].col_name = dtsq_attrib->qual[",
      trim(cnvtstring(dtsq_idx)),"].column_name"),0)
    CALL parser(concat(
      "   dtg->qual[dtsq_pos].att_qual[dtg->qual[dtsq_pos].att_cnt].col_txt = dtsq_attrib->qual[",
      trim(cnvtstring(dtsq_idx)),"].column_label"),0)
    IF ((dtsq_attrib->qual[dtsq_idx].column_flg=3))
     CALL parser(concat(
       "   dtg->qual[dtsq_pos].att_qual[dtg->qual[dtsq_pos].att_cnt].col_val = format(sq.",
       dtsq_attrib->qual[dtsq_idx].column_name,",';;q')"),0)
    ELSEIF ((dtsq_attrib->qual[dtsq_idx].column_flg=2))
     CALL parser(concat(
       "   dtg->qual[dtsq_pos].att_qual[dtg->qual[dtsq_pos].att_cnt].col_val = cnvtstring(sq.",
       dtsq_attrib->qual[dtsq_idx].column_name,",20)"),0)
    ELSE
     CALL parser(concat("   dtg->qual[dtsq_pos].att_qual[dtg->qual[dtsq_pos].att_cnt].col_val = sq.",
       dtsq_attrib->qual[dtsq_idx].column_name),0)
    ENDIF
  ENDFOR
 ...
 SET stat = alterlist(script_rec->script_qual,0)
 SET stat = initrec(dtg)
 SET dm_err->eproc = "Gathering Scheduling Inquiry FKs to LONG_TEXT_REFERENCE"
 SELECT DISTINCT INTO "nl:"
  sq.text_id
  FROM sch_query sq
  WHERE sq.text_id > 0.0
   AND  EXISTS (
  (SELECT
   "x"
   FROM long_text_reference l
   WHERE l.long_text_id=sq.text_id))
  HEAD REPORT
   script_rec->dm_text_find_detail_id = dtla_detail_id, dtsq_cnt = 0
  DETAIL
   dtsq_cnt = (dtsq_cnt+ 1)
   IF (mod(dtsq_cnt,10)=1)
    stat = alterlist(dtg->qual,(dtsq_cnt+ 9))
   ENDIF
   dtg->qual[dtsq_cnt].pe_name = "LONG_TEXT_REFERENCE", dtg->qual[dtsq_cnt].pe_id = sq.text_id, dtg->
   qual[dtsq_cnt].pe_col = "LONG_TEXT_ID",
   dtg->qual[dtsq_cnt].srch_col = "LONG_TEXT"
  FOOT REPORT
   dtg->cnt = dtsq_cnt, stat = alterlist(dtg->qual,dtg->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for Scheduling LONG_TEXT_REFERENCE FKs:",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 IF ((dtg->cnt > 0))
  SET dm_err->eproc = "Gathering attribute information from SCH_QUERY"
  CALL parser("select into 'nl:'",0)
  CALL parser("  from sch_query sq",0)
  CALL parser(" where expand(dtsq_num, 1, dtg->cnt,sq.text_id,dtg->qual[dtsq_num].pe_id)",0)
  CALL parser(" order by sq.text_id, sq.updt_dt_tm desc ",0)
  CALL parser("head sq.text_id",0)
  CALL parser("   dtsq_pos = locateval(dtsq_num, 1, dtg->cnt,sq.text_id,dtg->qual[dtsq_num].pe_id) ",
   0)
  CALL parser("detail",0)
  CALL parser("  if (dtg->qual[dtsq_pos].data_source <= ' ')",0)
  CALL parser("   dtg->qual[dtsq_pos].data_source = sq.mnemonic",0)
 ...
 ENDIF
#exit_script
 SET dm_err->eproc = "...Ending DM_TXTFND_SCH_QUERY"
 CALL final_disp_msg("dm_tf_sch_qry")
 IF (dtsq_err_ind=1
  AND (dm_err->err_ind=0))
  SET dm_err->err_ind = dtsq_err_ind
  SET dm_err->emsg = dtsq_emsg
 ENDIF
END GO
