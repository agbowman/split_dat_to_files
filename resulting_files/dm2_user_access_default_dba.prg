CREATE PROGRAM dm2_user_access_default:dba
 ...
 SUBROUTINE duar_validate_secuser(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_user_access_d",".log","dm2_user_access_default LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_user_access_default"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF (duar_validate_secuser(null)=0)
  GO TO exit_script
 ENDIF
 IF ((duar_seclogininfo->valid_username_ind=0))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Updating SYSTEM user as ADMINISTRATOR in dm_info."
 UPDATE  FROM dm_info di
  SET di.updt_id = 1, di.info_char = duar_seclogininfo->username, di.updt_cnt = (di.updt_cnt+ 1),
   di.updt_dt_tm = cnvtdatetime(curdate,curtime3)
  WHERE "DM_INSTALL USER ADMINISTRATOR"=di.info_domain
   AND "SYSTEM"=di.info_name
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inserting SYSTEM user as ADMINISTRATOR into dm_info."
  INSERT  FROM dm_info di
   SET di.info_domain = "DM_INSTALL USER ADMINISTRATOR", di.info_name = "SYSTEM", di.info_char =
    duar_seclogininfo->username,
    di.updt_id = 1
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
...
#exit_script
 IF ((dm_err->err_ind=0))
  ...
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SET dm_err->eproc = "Ending dm2_user_access_default."
 CALL final_disp_msg("dm2_user_access_m")
END GO
