CREATE PROGRAM afc_del_charge:dba
 ...
 CALL del_charge(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE del_charge(del_begin,del_end)
   FOR (x = del_begin TO del_end)
     SET active_code = 0.0
     UPDATE  FROM charge c
      SET c.end_effective_dt_tm =
       IF ((request->charge[x].end_effective_dt_tm=0)) null
       ELSE cnvtdatetime(curdate,curtime)
       ENDIF
       , c.active_ind = false, c.active_status_cd = nullcheck(active_code,request->charge[x].
        active_status_cd,
        IF ((request->charge[x].active_status_cd=0)) 0
        ELSE 1
        ENDIF
        ),
       c.active_status_prsnl_id = reqinfo->updt_id, c.active_status_dt_tm = cnvtdatetime(curdate,
        curtime), c.updt_cnt = (c.updt_cnt+ 1),
     ...
     IF (curqual=0)
      ...
      RETURN
     ELSE
      SET reply->charge[x].charge_item_id = request->charge[x].charge_item_id
     ENDIF
   ENDFOR
 END ;Subroutine
#end_program
