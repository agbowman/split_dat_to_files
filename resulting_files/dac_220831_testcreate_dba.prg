CREATE PROGRAM dac_220831_testcreate:dba
 ...
 SET dac_test_reply->message = "Test failed: starting test dac_220831_testCreate..."
 CALL parser("rdb ALTER SESSION SET NLS_LANGUAGE= 'FRENCH' go")
 IF (error(dt_errmsg,0) != 0)
  SET dac_test_reply->status = "F"
  SET dac_test_reply->message = concat("Failed to set a_nls functionality for session: ",dt_errmsg)
  GO TO exit_test
 ENDIF
 SELECT INTO "nl:"
 ...
 IF ((readme_data->status != "S"))
  SET dac_test_reply->status = "F"
  SET dac_test_reply->message = concat("Readme failed: ",readme_data->message)
  GO TO exit_test
 ENDIF
 SELECT INTO "nl:"
  FROM all_synonyms
  WHERE synonym_name="CERN_NLS_SORT_PATMATCH"
   AND owner="PUBLIC"
   AND table_owner="V500"
  WITH nocounter
 ;end select
 IF (error(dt_errmsg,0) != 0)
  SET dac_test_reply->status = "F"
  SET dac_test_reply->message = concat("Failed to insert row: ",dt_errmsg)
  GO TO exit_test
 ELSEIF (curqual=0)
  SET dac_test_reply->status = "F"
  SET dac_test_reply->message = "Failed to find synonym"
  GO TO exit_test
 ENDIF
