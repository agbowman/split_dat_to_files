CREATE PROGRAM bed_ens_xr_server_config:dba
 ...
 SUBROUTINE check_log_level(arg_log_level)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "BED_ENS_XR_SERVER_CONFIG"
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 FOR (requestenvironmententryindex = 1 TO size(request->environmententry,5))
  SELECT INTO "nl:"
   sc.type_flag
   FROM cr_server_configuration sc
   WHERE (sc.type_flag=request->environmententry[requestenvironmententryindex].type_flag)
   WITH nocounter
  ;end select
  IF (curqual=0)
   CALL insertserverconfigurationentry(requestenvironmententryindex)
  ELSE
   CALL updateserverconfigurationentry(requestenvironmententryindex)
  ENDIF
 ENDFOR
 SET reqinfo->commit_ind = 1
 SET reply->status_data.status = "S"
 SUBROUTINE insertserverconfigurationentry(requestenvironmententryindex)
   INSERT  FROM cr_server_configuration sc
    SET sc.cr_server_configuration_id = seq(reference_seq,nextval), sc.cr_server_value_txt = request
     ->environmententry[requestenvironmententryindex].type_value_txt, sc.type_flag = request->
     environmententry[requestenvironmententryindex].type_flag,
     sc.updt_cnt = 0, sc.updt_dt_tm = cnvtdatetime(curdate,curtime3), sc.updt_id = reqinfo->updt_id,
     sc.updt_applctx = reqinfo->updt_applctx, sc.updt_task = reqinfo->updt_task
    WITH nocounter
   ;end insert
 END ;Subroutine
 SUBROUTINE updateserverconfigurationentry(requestenvironmententryindex)
   UPDATE  FROM cr_server_configuration sc
    SET sc.cr_server_value_txt = request->environmententry[requestenvironmententryindex].
     type_value_txt, sc.updt_cnt = (updt_cnt+ 1), sc.updt_dt_tm = cnvtdatetime(curdate,curtime3),
...
#exit_script
 ...
 ENDIF
 CALL log_message("End of script: bed_ens_xr_server_config.",log_level_debug)
END GO
