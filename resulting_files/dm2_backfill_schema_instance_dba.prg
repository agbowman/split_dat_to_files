CREATE PROGRAM dm2_backfill_schema_instance:dba
 DECLARE dbsi_envid = f8 WITH protect, noconstant(0.0)
 DECLARE dbsi_cnt = i4 WITH protect, noconstant(0)
 DECLARE dbsi_chk_cnt = i4 WITH protect, noconstant(0)
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_bkfil_sch_inst",".log","dm2_backfill_schema_instance")=0)
  GO TO exit_program
 ENDIF
 FREE RECORD dbsi_table_info
 RECORD dbsi_table_info(
   1 cnt = i4
   1 qual[*]
     2 table_name = vc
     2 sch_instance = i4
 ...
 SET dm_err->eproc = "Determine if DM_AFD_TABLES contains SCHEMA_INSTANCE"
 CALL disp_msg("",dm_err->logfile,0)
 RANGE OF a IS dm_afd_tables
 IF ((validate(a.schema_instance,- (999999))=- (999999)))
  FREE RANGE a
  GO TO exit_program
 ENDIF
 FREE RANGE a
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Get Environment_ID from DM_INFO"
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  b.environment_id
  FROM dm_info a,
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "DM_ENV_ID row not in DM_INFO"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determine if INHOUSE domain."
 CALL disp_msg("",dm_err->logfile,0)
 IF (dm2_set_inhouse_domain(null)=0)
  GO TO exit_program
 ENDIF
 IF ((inhouse_misc->inhouse_domain=1))
  SET dm_err->eproc = "Determine highest SCHEMA_INSTANCE for tables processed for INHOUSE domain."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dm_project_status_env dpse,
    ...
    dm_adm_tables a
   PLAN (dpse
    WHERE dpse.environment_id=dbsi_envid
     AND dpse.proj_type="SCHEMA"
     AND dpse.dm_status="SUCCESS")
    JOIN (dtd
    WHERE dtd.factory_name=dpse.proj_name)
    JOIN (a
    WHERE dpse.schema_date=a.schema_date
     AND a.owner=dtd.owner
     AND a.table_name=dtd.table_name)
   ORDER BY a.table_name, dpse.schema_date DESC
   HEAD REPORT
    dbsi_cnt = 0
   HEAD a.table_name
    dbsi_cnt = (dbsi_cnt+ 1)
    IF (mod(dbsi_cnt,10)=1)
     stat = alterlist(dbsi_table_info->qual,(dbsi_cnt+ 9))
    ENDIF
    dbsi_table_info->qual[dbsi_cnt].table_name = a.table_name, dbsi_table_info->qual[dbsi_cnt].
    sch_instance = a.schema_instance, dbsi_table_info->qual[dbsi_cnt].insert_ind = 1
   FOOT REPORT
    dbsi_table_info->cnt = dbsi_cnt, stat = alterlist(dbsi_table_info->qual,dbsi_cnt)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Determine highest SCHEMA_INSTANCE for tables processed."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   a.table_name, max_instance = max(a.schema_instance)
   FROM dm_alpha_features_env dafe,
    dm_afd_tables a
   PLAN (dafe
    WHERE dafe.environment_id=dbsi_envid)
    JOIN (a
    WHERE dafe.alpha_feature_nbr=a.alpha_feature_nbr
   ...
   HEAD a.table_name
    ...
    ENDIF
    dbsi_table_info->qual[dbsi_cnt].table_name = a.table_name, dbsi_table_info->qual[dbsi_cnt].
    sch_instance = max_instance, dbsi_table_info->qual[dbsi_cnt].insert_ind = 1
   FOOT REPORT
    stat = alterlist(dbsi_table_info->qual,dbsi_cnt), dbsi_table_info->cnt = dbsi_cnt
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 SET dbsi_cnt = 0
 IF ((dbsi_table_info->cnt > 0))
  SET dm_err->eproc = "Determine whether to insert into DM_INFO."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(dbsi_table_info->cnt)),
    dm_info di
   PLAN (d)
    JOIN (di
    WHERE di.info_domain="DM2_SCHEMA_INSTANCE"
     AND (di.info_name=dbsi_table_info->qual[d.seq].table_name))
   DETAIL
    dbsi_cnt = (dbsi_cnt+ 1), dbsi_table_info->qual[d.seq].insert_ind = 0
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 IF ((dbsi_cnt != dbsi_table_info->cnt))
  SET dm_err->eproc = "Insert into DM_INFO."
  CALL disp_msg("",dm_err->logfile,0)
  INSERT  FROM dm_info di,
    (dummyt d  WITH seq = value(dbsi_table_info->cnt))
   SET di.info_domain = "DM2_SCHEMA_INSTANCE", di.info_name = dbsi_table_info->qual[d.seq].table_name,
    di.info_number = dbsi_table_info->qual[d.seq].sch_instance,
    di.updt_dt_tm = cnvtdatetime(curdate,curtime3)
   PLAN (d
    WHERE (dbsi_table_info->qual[d.seq].insert_ind=1))
    JOIN (di
    WHERE (dbsi_table_info->qual[d.seq].table_name=di.info_name))
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Verify that DM_INFO contains Schema Instance Rows."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  dbsi_all_cnt = count(*)
  FROM dm_info di
 ...
 IF ((dm_err->debug_flag > 0))
  CALL echo("**********************************")
  CALL echo(build("Check_Cnt:",dbsi_chk_cnt))
  CALL echo(build("Tbl_cnt:",dbsi_table_info->cnt))
  CALL echo("**********************************")
 ENDIF
 IF ((dbsi_chk_cnt < dbsi_table_info->cnt))
  SET dm_err->emsg = build("Number of Rows existing in DM_INFO:",dbsi_table_info->cnt,
   ", is less than check count of rows in DM_INFO:",dbsi_chk_cnt,".")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ELSE
  SET dm_err->eproc = build("Number of Rows existing in DM_INFO:",dbsi_table_info->cnt,
   ", is greater than or equal to count of rows in DM_INFO:",dbsi_chk_cnt,". Success.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  SET dm_err->eproc = "Logging Backfill Complete in DM_INFO."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  UPDATE  FROM dm_info d
   SET d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_cnt = (d.updt_cnt+ 1)
   WHERE d.info_domain="DATA MANAGEMENT"
    AND d.info_name="DM2 SCHEMA INSTANCE BACKFILL COMPLETE"
   WITH nocounter
  ;end update
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->emsg = "Error inserting Backfill Complete row into DM_INFO."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   GO TO exit_program
  ENDIF
  IF (curqual=0)
   INSERT  FROM dm_info d
    SET d.info_domain = "DATA MANAGEMENT", d.info_name = "DM2 SCHEMA INSTANCE BACKFILL COMPLETE"
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->emsg = "Error updating DM_INFO for Backfill Complete row."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    GO TO exit_program
   ENDIF
  ENDIF
...
#exit_program
 IF ((dm_err->err_ind > 0))
  ROLLBACK
 ELSE
  SET dm_err->eproc = "Deleting DM2_BACKFILL_SCHEMA_INSTANCE row from DM_INFO."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  DELETE  FROM dm_info
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dbsi_table_info)
 ENDIF
 SET dm_err->eproc = "Exiting DM2_BACKFILL_SCHEMA_INSTANCE"
 CALL final_disp_msg("dm2_bkfil_sch_inst")
END GO
