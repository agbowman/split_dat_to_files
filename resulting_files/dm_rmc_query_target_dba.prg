CREATE PROGRAM dm_rmc_query_target:dba
 ...
 END ;Subroutine
 DECLARE drqt_query_target(qt_temp_tbl_cnt=i4,qt_perm_col_cnt=i4) = f8 WITH public
 DECLARE drqt_exec_ui_query(exec_tbl_cnt=i4,exec_perm_col_cnt=i4) = f8 WITH public
 DECLARE drqt_evaluate_exec_ui_query(sbr_current_qual=i4,eval_tbl_cnt=i4,eval_perm_col_cnt=i4) = f8
 WITH public
 DECLARE drqt_sub_return = f8
 IF ((validate(request->sub_call_flag,- (99))=- (99)))
  FREE RECORD request
  RECORD request(
    1 sub_call_flag = i4
    1 temp_tbl_cnt = i4
 ...
 ENDIF
 IF ((request->sub_call_flag=1))
  SET drqt_sub_return = drqt_query_target(request->temp_tbl_cnt,request->perm_col_cnt)
 ELSEIF ((request->sub_call_flag=2))
  SET drqt_sub_return = drqt_exec_ui_query(request->temp_tbl_cnt,request->perm_col_cnt)
 ELSEIF ((request->sub_call_flag=3))
  SET drqt_sub_return = drqt_evaluate_exec_ui_query(request->eval_rec_size,request->temp_tbl_cnt,
   request->perm_col_cnt)
 ENDIF
 SET reply->drqt_return = drqt_sub_return
 SUBROUTINE drqt_query_target(qt_temp_tbl_cnt,qt_perm_col_cnt)
   ...
   SET sbr_pk_col_val = get_value(sbr_table_name,dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
    temp_col_cnt].column_name,"FROM")
   SET cust_nomv_chk = select_merge_translate(sbr_pk_col_val,sbr_table_name)
   ...
   ELSE
    ...
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind=1))
     ...
     ENDIF
    ENDIF
    SET sbr_returned_value = drqt_exec_ui_query(qt_temp_tbl_cnt,qt_perm_col_cnt)
    SET stat = alterlist(ui_query_rec->qual[sbr_rec_size].other_pk_col,0)
    RETURN(sbr_returned_value)
   ENDIF
 END ;Subroutine
 SUBROUTINE drqt_exec_ui_query(exec_tbl_cnt,exec_perm_col_cnt)
   ...
   DECLARE euq_tspc_flag = i2 WITH noconstant(0)
   DECLARE euq_nomv_msg = vc WITH noconstant("")
   DECLARE euq_nomv_num = f8
   DECLARE euq_nomv_vc = vc WITH noconstant("")
   DECLARE euq_nomv_name = vc WITH noconstant("")
   DECLARE euq_parent_drdm_row = i2 WITH protect, noconstant(0)
    ...
    AND sbr_done_select=0)
     ...
     IF ((dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].merge_ui_query_ni=1))
      ...
      FOR (drdm_loop_cnt = 1 TO exec_perm_col_cnt)
        IF ((dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].col_qual[drdm_loop_cnt].unique_ident_ind=1)
         ...
         IF ((dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].col_qual[drdm_loop_cnt].check_null=1))
          ...
          IF (sbr_loop=1)
           SET euq_nomv_msg = concat(euq_nomv_msg," ",drdm_col_name," = NULL")
          ENDIF
          SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
         ELSEIF ((dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].col_qual[drdm_loop_cnt].check_space=1))
          ...
          SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
         ELSEIF ((dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].col_qual[drdm_loop_cnt].trailing_space_cnt
          > 0)
          AND (dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].col_qual[drdm_loop_cnt].data_type="VC"))
          CALL parser(concat("free set euq_nospc",trim(cnvtstring(drdm_loop_cnt))," go"))
          CALL parser(concat("declare euq_nospc",trim(cnvtstring(drdm_loop_cnt)),
            " =  vc with notrim go"),1)
          CALL parser(concat("set euq_nospc",trim(cnvtstring(drdm_loop_cnt))," = notrim(concat(",
            drdm_from_con,", fillstring(value(",
            trim(cnvtstring(dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].col_qual[drdm_loop_cnt].
              trailing_space_cnt)),"), ' '))) go"),1)
          SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" dc.",drdm_col_name,
           " = euq_nospc",trim(cnvtstring(drdm_loop_cnt)))
          SET euq_nomv_name = concat("euq_nospc",trim(cnvtstring(drdm_loop_cnt)))
          CALL parser(concat("set euq_nomv_vc = concat(^'^,",euq_nomv_name,",^'^) go"),1)
          IF (sbr_loop=1)
           SET euq_nomv_msg = concat(euq_nomv_msg," ",drdm_col_name," = cnvtdatetime(",trim(
             euq_nomv_vc),
            ")")
          ENDIF
          SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
         ...
         ELSE
          CASE (dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].col_qual[drdm_loop_cnt].data_type)
           OF "DQ8":
            ...
            CALL parser(concat("set euq_nomv_num = ",drdm_from_con," go"),1)
            IF (sbr_loop=1)
             SET euq_nomv_msg = concat(euq_nomv_msg," ",drdm_col_name," = cnvtdatetime(",trim(
               cnvtstring(euq_nomv_num,20,1)),
              ")")
            ENDIF
            SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
           ELSE
            SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" dc.",drdm_col_name," = ",
             drdm_from_con)
            IF ((dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].col_qual[drdm_loop_cnt].data_type IN ("VC",
            "C*")))
             CALL parser(concat("set euq_nomv_vc = ",drdm_from_con," go"),1)
            ELSE
             CALL parser(concat("set euq_nomv_vc = trim(cnvtstring(",drdm_from_con,",20,1)) go"),1)
            ENDIF
            IF (sbr_loop=1)
             SET euq_nomv_msg = concat(euq_nomv_msg," ",drdm_col_name," = ",euq_nomv_vc)
            ENDIF
            SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
          ENDCASE
         ENDIF
        ELSEIF ((ui_query_rec->qual[sbr_loop].qtype="*GROUPER*")
         AND (dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].col_qual[drdm_loop_cnt].exception_flg=12))
         SET no_unique_ident = 1
         IF (drdm_parser_cnt > 3)
          SET drdm_parser->statement[drdm_parser_cnt].frag = " and "
          IF (sbr_loop=1)
        ...
        ENDIF
      ENDFOR
      IF ((ui_query_rec->dom="$R"))
       SET drdm_parser->statement[drdm_parser_cnt].frag =
       " and dc.rdds_status_flag = 0 and dc.rdds_delete_ind = 0 "
       SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
       SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" and dc.rdds_source_env_id = ",
        "dm2_ref_data_doc->env_source_id")
       SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
      ENDIF
      IF (no_unique_ident=0)
       IF ((ui_query_rec->qual[sbr_loop].qtype="*GROUPER*"))
        SET insert_update_reason = "There were no grouper columns for log_id "
       ELSE
       ...
       IF ((ui_query_rec->qual[sbr_loop].qtype="*GROUPER*"))
        SET dm2_ref_data_reply->error_msg = concat(dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].
         custom_script,": There were no grouper columns")
       ELSE
        SET dm2_ref_data_reply->error_msg = concat(dm2_ref_data_doc->tbl_qual[exec_tbl_cnt].
      ...
      CASE (ui_query_rec->qual[sbr_loop].qtype)
       ...
       OF "END_EFFECTIVE":
        SET ui_query_rec->qual[sbr_loop].where_clause = concat(" AND dc.",dm2_ref_data_doc->tbl_qual[
         temp_tbl_cnt].end_col_name,"<= cnvtdatetime(sbr_cur_date)")
       OF "COMBO":
        ...
        ENDFOR
       OF "GROUPER-EFFECTIVE":
        SET ui_query_rec->qual[sbr_loop].where_clause = concat(" AND dc.",dm2_ref_data_doc->tbl_qual[
         temp_tbl_cnt].beg_col_name,"<=  cnvtdatetime(sbr_cur_date) AND dc.",dm2_ref_data_doc->
         tbl_qual[temp_tbl_cnt].end_col_name,">= cnvtdatetime(sbr_cur_date)")
       OF "GROUPER-END_EFFECTIVE":
        SET ui_query_rec->qual[sbr_loop].where_clause = concat(" AND dc.",dm2_ref_data_doc->tbl_qual[
         temp_tbl_cnt].beg_col_name,">=  cnvtdatetime(sbr_cur_date) OR dc.",dm2_ref_data_doc->
         tbl_qual[temp_tbl_cnt].end_col_name,"<= cnvtdatetime(sbr_cur_date)")
       OF "FUTURE_EFFECTIVE":
        SET ui_query_rec->qual[sbr_loop].where_clause = concat(" AND dc.",dm2_ref_data_doc->tbl_qual[
         temp_tbl_cnt].beg_col_name,">=  cnvtdatetime(sbr_cur_date)")
      ENDCASE
      IF ((ui_query_rec->qual[sbr_loop].additional_clause > ""))
       SET ui_query_rec->qual[sbr_loop].where_clause = concat(ui_query_rec->qual[sbr_loop].
        where_clause," ",replace(ui_query_rec->qual[sbr_loop].additional_clause,"<SUFFIX>","dc",0))
       SET ui_query_rec->qual[sbr_loop].additional_clause = ""
      ENDIF
      ...
      IF ((ui_query_rec->usage="VERSION")
       AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].filter_string > " "))
       SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" and ",replace(replace(
          dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].filter_string,"<SUFFIX>","dc",0),"<MERGE LINK>",
      ...
      IF (euq_tspc_flag=1)
       SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" foot report",
        " stat = alterlist(ui_query_eval_rec->qual, query_cnt)"," with nocounter, notrim go")
      ELSE
       SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" foot report",
      ...
      ELSEIF ((query_return != - (1)))
       SET query_return = drqt_evaluate_exec_ui_query(query_cnt,exec_tbl_cnt,exec_perm_col_cnt)
      ENDIF
   ...
   IF ((query_return=- (2))
    ...
    SET euq_parent_drdm_row = find_original_log_row(drdm_chg,drdm_log_loop)
    SET drdm_chg->log[euq_parent_drdm_row].chg_log_reason_txt = concat(
     "Multiple rows found in the TARGET where ",euq_nomv_msg)
    ROLLBACK
   ...
   RETURN(query_return)
 END ;Subroutine
 SUBROUTINE drqt_evaluate_exec_ui_query(sbr_current_qual,eval_tbl_cnt,eval_perm_col_cnt)
   ...
   ELSEIF (sbr_current_qual=1)
    IF ((((ui_query_rec->usage="VERSION")
     ...
     SET select_merge_translate_rec->type = "TO"
     SET select_merge_translate_rec->to_opt_ind = 1
     SET sbr_trans_val = select_merge_translate(cnvtstring(sbr_temp_pk_value,20,1),sbr_table_name)
     SET select_merge_translate_rec->to_opt_ind = 0
     SET select_merge_translate_rec->type = "FROM"
     IF ( NOT (isnumeric(sbr_trans_val)))
   ...
   ELSE
    ...
    ELSE
     ...
     ELSEIF (sbr_not_translated_count=1)
      ...
      RETURN(current_qual)
     ELSE
      IF ((dm2_ref_data_doc->tbl_qual[eval_tbl_cnt].overrule_nomv06_ind=1))
       SET sbr_root_entity_attr_val = value(ui_query_eval_rec->qual[sbr_val_pos].root_entity_attr)
       RETURN(sbr_root_entity_attr_val)
      ELSE
       RETURN(- (2))
      ENDIF
