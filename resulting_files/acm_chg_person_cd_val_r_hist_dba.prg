CREATE PROGRAM acm_chg_person_cd_val_r_hist:dba
 ...
 RECORD temp_record(
   1 person_cd_val_r_hist_qual[*]
     2 person_code_value_r_h_id = f8
     2 person_id = f8
 ...
 ENDFOR
 SET stat = alterlist(temp_record->person_cd_val_r_hist_qual,xref->chg_cnt)
 SET pcvrh_qual_cntr = 0
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(xref->chg_cnt))
  PLAN (d)
  DETAIL
   item_idx = xref->chg[d.seq].idx, chg_str = acm_request->person_code_value_r_qual[item_idx].chg_str,
   chgbit = 0
   ...
   ENDIF
   IF (chgbit > 0)
    pcvrh_qual_cntr = (pcvrh_qual_cntr+ 1), temp_record->person_cd_val_r_hist_qual[pcvrh_qual_cntr].
    change_bit = chgbit, temp_record->person_cd_val_r_hist_qual[pcvrh_qual_cntr].item_idx = item_idx,
    temp_record->person_cd_val_r_hist_qual[pcvrh_qual_cntr].person_id = acm_request->
    person_code_value_r_qual[item_idx].person_id, temp_record->person_cd_val_r_hist_qual[
    pcvrh_qual_cntr].pm_hist_tracking_id = acm_request->person_code_value_r_qual[item_idx].
    pm_hist_tracking_id, temp_record->person_cd_val_r_hist_qual[pcvrh_qual_cntr].transaction_dt_tm =
    acm_request->person_code_value_r_qual[item_idx].transaction_dt_tm,
    temp_record->person_cd_val_r_hist_qual[pcvrh_qual_cntr].code_set = acm_request->
    person_code_value_r_qual[item_idx].code_set, temp_record->person_cd_val_r_hist_qual[
    pcvrh_qual_cntr].code_value = acm_request->person_code_value_r_qual[item_idx].code_value
   ELSE
    reply->person_code_value_r_qual[item_idx].status = 1
   ENDIF
  WITH nocounter
 ;end select
 SET stat = alterlist(temp_record->person_cd_val_r_hist_qual,pcvrh_qual_cntr)
 IF (pcvrh_qual_cntr > 0)
  INSERT  FROM person_code_value_r_hist pcvrh,
    (dummyt d  WITH seq = value(pcvrh_qual_cntr))
   SET pcvrh.person_code_value_r_h_id = seq(person_seq,nextval), pcvrh.person_id = temp_record->
    person_cd_val_r_hist_qual[d.seq].person_id, pcvrh.pm_hist_tracking_id = temp_record->
    person_cd_val_r_hist_qual[d.seq].pm_hist_tracking_id,
    pcvrh.transaction_dt_tm = cnvtdatetime(temp_record->person_cd_val_r_hist_qual[d.seq].
     transaction_dt_tm), pcvrh.code_set = temp_record->person_cd_val_r_hist_qual[d.seq].code_set,
    pcvrh.code_value = temp_record->person_cd_val_r_hist_qual[d.seq].code_value,
    pcvrh.tracking_bit = tracking_bit, pcvrh.change_bit = temp_record->person_cd_val_r_hist_qual[d
    .seq].change_bit, pcvrh.updt_dt_tm = cnvtdatetime(curdate,curtime3),
    pcvrh.updt_id = reqinfo->updt_id, pcvrh.updt_applctx = reqinfo->updt_applctx, pcvrh.updt_task =
    reqinfo->updt_task,
    pcvrh.updt_cnt = updt_cnt
   PLAN (d)
    JOIN (pcvrh)
   WITH nocounter, status(reply->person_code_value_r_qual[temp_record->person_cd_val_r_hist_qual[d
    .seq].item_idx].status)
  ;end insert
 ENDIF
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->person_code_value_r_qual[xref->chg[index].idx].status=0))
    ...
    SET table_name = "PERSON_CODE_VALUE_R_HIST"
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
