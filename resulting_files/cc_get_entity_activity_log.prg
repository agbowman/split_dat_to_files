CREATE PROGRAM cc_get_entity_activity_log
 PROMPT
  ...
  "ResultVersions" = 0.0,
  "OrderVersions" = 0.0,
  "OrderNurseReview" = 0.0,
  "ResultUpdtDtTm" = 0.0,
  "OrderUpdtDtTm" = 0.0,
  "Prsnl Id" = 0.0,
  "Primary PPR Cd" = 0.0
  WITH outdev, resultids, orderids,
  resultversions, orderversions, ordernursereview,
  resultupdtdttm, orderupdtdttm, prsnlid,
  primarypprcd
 ...
 FREE RECORD resultids_rec
 RECORD resultids_rec(
   1 cnt = i4
   1 qual[*]
     2 value = f8
 )
 FREE RECORD orderids_rec
 RECORD orderids_rec(
   1 cnt = i4
   1 qual[*]
     2 value = f8
 )
 FREE RECORD resultversions_rec
 RECORD resultversions_rec(
   1 cnt = i4
   1 qual[*]
     2 value = f8
 )
 FREE RECORD orderversions_rec
 RECORD orderversions_rec(
   1 cnt = i4
   1 qual[*]
     2 value = f8
 )
 FREE RECORD ordernursereview_rec
 RECORD ordernursereview_rec(
   1 cnt = i4
   1 qual[*]
     2 value = f8
 )
 FREE RECORD resultupdtdttm_rec
 RECORD resultupdtdttm_rec(
   1 cnt = i4
   1 qual[*]
     2 value = f8
 )
 FREE RECORD orderupdtdttm_rec
 RECORD orderupdtdttm_rec(
   1 cnt = i4
   1 qual[*]
     2 value = f8
 )
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CC_GET_ENTITY_ACTIVITY_LOG"
 ...
 SUBROUTINE (validatesrvreply(nsrvstat=i4,recorddata=vc(ref),zeroforceexit=i2) =i4 WITH protect)
   ...
   ENDIF
 END ;Subroutine
 DECLARE request_number = i4 WITH protect, constant(3208639)
 DECLARE srv_request = vc WITH protect, constant("QueryEntityActivityLog")
 DECLARE hresults = i4 WITH noconstant(0), protect
 DECLARE horders = i4 WITH noconstant(0), protect
 SET reply_data->status_data.status = "F"
 CALL getparametervalues(2,resultids_rec)
 CALL getparametervalues(3,orderids_rec)
 CALL getparametervalues(4,resultversions_rec)
 CALL getparametervalues(5,orderversions_rec)
 CALL getparametervalues(6,ordernursereview_rec)
 CALL getparametervalues(7,resultupdtdttm_rec)
 CALL getparametervalues(8,orderupdtdttm_rec)
 CALL initializerequest(reply_data,request_number)
 SET nsrvstat = uar_srvsetdouble(hreq,"prsnl_id", $PRSNLID)
 SET nsrvstat = uar_srvsetdouble(hreq,"primary_ppr_cd", $PRIMARYPPRCD)
 IF ((resultids_rec->cnt > 0)
  AND (resultversions_rec->cnt > 0))
  DECLARE idx = i4 WITH noconstant(0), protect
  FOR (idx = 1 TO resultids_rec->cnt)
    SET hresults = uar_srvadditem(hreq,"results")
    SET nsrvstat = uar_srvsetdouble(hresults,"entity_id",resultids_rec->qual[idx].value)
    SET nsrvstat = uar_srvsetdouble(hresults,"version",resultversions_rec->qual[idx].value)
    SET nsrvstat = uar_srvsetdouble(hresults,"updtDtTmMilliSeconds",resultupdtdttm_rec->qual[idx].
     value)
  ENDFOR
 ENDIF
 IF ((orderids_rec->cnt > 0)
  AND (orderversions_rec->cnt > 0))
  DECLARE idx = i4 WITH noconstant(0), protect
  FOR (idx = 1 TO orderids_rec->cnt)
    SET horders = uar_srvadditem(hreq,"orders")
    SET nsrvstat = uar_srvsetdouble(horders,"entity_id",orderids_rec->qual[idx].value)
    SET nsrvstat = uar_srvsetdouble(horders,"version",orderversions_rec->qual[idx].value)
    SET nsrvstat = uar_srvsetdouble(horders,"nurseReview",ordernursereview_rec->qual[idx].value)
    SET nsrvstat = uar_srvsetdouble(horders,"updtDtTmMilliSeconds",orderupdtdttm_rec->qual[idx].value
     )
  ENDFOR
 ENDIF
