CREATE PROGRAM dm_rmc_mover_2403:dba
 ...
 DECLARE cfs_tab_name = vc
 DECLARE cf_tab_name = vc
 DECLARE cust_tab_name_s = vc
 DECLARE cfs_tab_name_s = vc
 DECLARE cf_tab_name_s = vc
 DECLARE cust_sect_id = f8
 DECLARE cs_seq_num = i4
 DECLARE cust_format_id_s = f8
 DECLARE format_trans = vc
 DECLARE cust_format_id = f8
 DECLARE cust_to_val = f8
 DECLARE chart_desc = vc
 DECLARE cust_sect_loop = i4
 DECLARE cust_sect_matches = i4
 DECLARE cust_rpt_missing = vc
 DECLARE cust_err_val = i4
 FREE RECORD cust_sect
 RECORD cust_sect(
   1 qual[*]
     2 to_val_id = f8
     2 updt_ind = i2
 ...
 SET cfs_tab_name = "CHART_FORM_SECTS"
 SET cf_tab_name = "CHART_FORMAT"
 ...
 SET cfs_tab_name_s = dm2_get_rdds_tname(cfs_tab_name)
 SET cf_tab_name_s = dm2_get_rdds_tname(cf_tab_name)
 SET cust_sect_id = cnvtreal(get_value(cust_tab_name,"CHART_SECTION_ID","FROM"))
 SET chart_desc = get_value(cust_tab_name,"CHART_SECTION_DESC","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_2403
 ENDIF
 IF (is_translated(cust_tab_name,"CHART_SECTION_ID")=0)
  SET ui_cnt = 0
  SELECT INTO "NL:"
   FROM (parser(cfs_tab_name_s) c)
   WHERE c.chart_section_id=cust_sect_id
   DETAIL
    ui_cnt = (ui_cnt+ 1), cust_format_id_s = c.chart_format_id, cs_seq_num = c.cs_sequence_num
   WITH nocounter
  ;end select
  ...
  IF (ui_cnt > 1)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV42"
   CALL merge_audit("FAILREASON",concat(cust_tab_name,
     "There are multiple relations to the same CHART_SECTION_ID on the CHART_FORM_SECTS table."),1)
   GO TO exit_2403
  ELSEIF (ui_cnt=0)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg =
   "ORPHAN: The CHART_SECTION_ID on the CHART_FORM_SECTS table, is orphaned from the CHART_FORMAT_ID"
   GO TO exit_2403
  ELSE
   SELECT INTO "NL:"
    FROM (parser(cf_tab_name_s) c)
    WHERE c.chart_format_id=cust_format_id_s
    WITH nocounter
   ;end select
   IF (error(dm2_ref_data_reply->error_msg,0) > 0)
    SET dm2_ref_data_reply->error_ind = 1
   ...
   ENDIF
   IF (curqual=0)
    SET cust_rpt_missing = report_missing("CHART_FORMAT","CHART_FORMAT_ID",cust_format_id_s)
    CALL evaluate_rpt_missing(cust_rpt_missing)
    GO TO exit_2403
   ENDIF
   SET format_trans = select_merge_translate(cnvtstring(cust_format_id_s),cf_tab_name)
   IF ( NOT (isnumeric(format_trans)))
    SET cust_rpt_missing = report_missing("CHART_FORMAT","CHART_FORMAT_ID",cust_format_id_s)
    CALL evaluate_rpt_missing(cust_rpt_missing)
    ...
    IF (cust_err_val=0)
     ...
     SET dm2_ref_data_reply->error_msg = concat(cust_tab_name,
      " cannot translate because of the CHART_FORMAT_ID.")
    ENDIF
    GO TO exit_2403
   ELSE
    SET cust_format_id = cnvtreal(format_trans)
   ENDIF
   SET ui_cnt = 0
   SELECT INTO "NL:"
    FROM (parser(cfs_tab_name) c)
    WHERE c.chart_format_id=cust_format_id
     AND c.cs_sequence_num=cs_seq_num
    DETAIL
     ui_cnt = (ui_cnt+ 1), stat = alterlist(cust_sect->qual,ui_cnt), cust_sect->qual[ui_cnt].
     to_val_id = c.chart_section_id
    WITH nocounter
   ;end select
   IF (error(dm2_ref_data_reply->error_msg,0) > 0)
    SET dm2_ref_data_reply->error_ind = 1
   ...
   ENDIF
   IF (ui_cnt > 0)
    SET cust_sect_matches = 0
    FOR (cust_sect_loop = 1 TO ui_cnt)
      SELECT INTO "NL:"
       FROM (parser(cust_tab_name) c)
       WHERE (c.chart_section_id=cust_sect->qual[cust_sect_loop].to_val_id)
        AND c.chart_section_desc=chart_desc
       DETAIL
        cust_sect->qual[cust_sect_loop].updt_ind = 1, cust_sect_matches = (cust_sect_matches+ 1)
       WITH nocounter
      ;end select
    ENDFOR
    IF (cust_sect_matches > 1)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV06"
     CALL merge_audit("FAILREASON",concat(cust_tab_name,
       " Multiple translations were found for the CHART_SECTION_ID."),1)
     GO TO exit_2403
    ELSEIF (cust_sect_matches=1)
     FOR (cust_sect_loop = 1 TO ui_cnt)
       IF ((cust_sect->qual[cust_sect_loop].updt_ind=1))
        SET cust_to_val = cust_sect->qual[cust_sect_loop].to_val_id
       ENDIF
     ENDFOR
     CALL put_value(cust_tab_name,"CHART_SECTION_ID",cnvtstring(cust_to_val))
     SET current_merges = (current_merges+ 1)
     SET child_merge_audit->num[current_merges].action = "UIEXIST"
     SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  CHART_SECTION_ID")
    ELSE
     SET cust_to_val = get_seq(cust_tab_name,"CHART_SECTION_ID")
     IF (cust_to_val > 0)
      CALL put_value(cust_tab_name,"CHART_SECTION_ID",cnvtstring(cust_to_val))
      SET current_merges = (current_merges+ 1)
   ...
   SET dm2_ref_data_reply->qual[1].from_value = cust_sect_id
