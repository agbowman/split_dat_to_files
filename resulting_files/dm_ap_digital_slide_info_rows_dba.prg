CREATE PROGRAM dm_ap_digital_slide_info_rows:dba
 ...
 SET reply->table_name = "AP_DIGITAL_SLIDE_INFO"
 ...
 IF (months_to_keep < minimum_months_to_keep)
  SET months_to_keep = minimum_months_to_keep
 ENDIF
 SET purge_dt_tm = cnvtagedatetime(0,months_to_keep,0,0)
 SELECT INTO "nl:"
  adsi.rowid
  FROM ap_digital_slide_info adsi
  WHERE parser(sbr_getrowidnotexists("adsi.active_ind = 0","adsi"))
   AND adsi.end_effective_dt_tm <= cnvtdatetime(purge_dt_tm)
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt += 1
   IF (mod(cnt,100)=1)
    stat = alterlist(reply->rows,(cnt+ 99))
   ENDIF
   reply->rows[cnt].row_id = adsi.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,cnt)
  WITH nocounter, maxqual(adsi,value(request->max_rows))
 ;end select
