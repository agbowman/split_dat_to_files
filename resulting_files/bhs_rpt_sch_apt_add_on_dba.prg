CREATE PROGRAM bhs_rpt_sch_apt_add_on:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Appointment Date Start:" = "CURDATE",
  "Appointment Date End:" = "CURDATE",
  "Resource Group:" = 0,
  "Scheduled Date Start:" = "CURDATE",
  "Scheduled Date End:" = "CURDATE"
  WITH outdev, ms_start_date, ms_end_date,
  mf_res_grp, ms_sched_start_date, ms_sched_end_date
 ...
 DECLARE mf_end_dt_tm = f8 WITH protect, constant(cnvtdatetime(cnvtdate2( $MS_END_DATE,"DD-MMM-YYYY"),
   235959))
 DECLARE mf_sched_begin_dt_tm = f8 WITH protect, constant(cnvtdatetime(cnvtdate2(
     $MS_SCHED_START_DATE,"DD-MMM-YYYY"),0))
 DECLARE mf_sched_end_dt_tm = f8 WITH protect, constant(cnvtdatetime(cnvtdate2( $MS_SCHED_END_DATE,
    "DD-MMM-YYYY"),235959))
 ...
 SELECT INTO "nl:"
  ...
  PLAN (sa
   ...
   WHERE sea.sch_event_id=se.sch_event_id
    AND sea.action_meaning="SCHEDULE"
    AND sea.action_dt_tm BETWEEN cnvtdatetime(mf_sched_begin_dt_tm) AND cnvtdatetime(
    mf_sched_end_dt_tm))
   JOIN (sep
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   .patient_qual[d2.seq].appt_qual[t_appt].beg_dt_tm = a.beg_dt_tm,
   t_rec->qual[d.seq].patient_qual[d2.seq].appt_qual[t_appt].end_dt_tm = a.end_dt_tm, t_rec->qual[d
   .seq].patient_qual[d2.seq].appt_qual[t_appt].sch_state_disp = sch_state_disp, t_rec->qual[d.seq].
   patient_qual[d2.seq].appt_qual[t_appt].appt_synonym_free = e.appt_synonym_free,
 ...
 SELECT INTO  $1
  ...
  HEAD PAGE
   ...
   "{POS/145/70}{B}Resource Appointment Summary (add-on Appointments)", row + 1,
   "{F/4}{CPI/11}{LPI/6}",
  ...
  DETAIL
   FOR (i = 1 TO t_rec->qual_cnt)
     ...
     FOR (j = 1 TO t_rec->qual[i].detail_qual_cnt)
       IF ((t_rec->qual[i].detail_qual[j].oe_field_meaning="SCHORDPHYS"))
     ...
     ENDFOR
     FOR (j = 1 TO t_rec->qual[i].detail_qual_cnt)
       IF ((t_rec->qual[i].detail_qual[j].oe_field_id != 663838.00))
...
#exit_script
 SET last_mod =
 "001 04/17/17 C14616       Added data fields  Reason for exam,previous imaging and comments"
END GO
