CREATE PROGRAM dm2_mig_verify_data
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmvd_stop = i2 WITH protect, noconstant(0)
 DECLARE dmvd_table = vc WITH protect, noconstant("")
 DECLARE dmvd_owner = vc WITH protect, noconstant("")
 DECLARE dmvd_masterid = f8 WITH protect, noconstant(0.0)
 DECLARE dmvd_begin_dt_tm = dq8 WITH protect, noconstant(0.0)
 IF (check_logfile("dm2_mig_verify_data",".log","dm2_mig_verify_data LOGFILE")=0)
  GO TO exit_program
 ENDIF
 IF (dmr_load_mig_data(null)=0)
  GO TO exit_program
 ...
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Retrieve database link information for compare."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_VDATA_INFO"
   AND d.info_name="DM2_VDATA_DBLINK"
  DETAIL
 ...
 WHILE (dmvd_stop=0)
   IF (dvdr_cleanup_stranded(null)=0)
    GO TO exit_program
   ENDIF
   IF (dvdr_check_stop(dmvd_stop)=0)
    GO TO exit_program
   ENDIF
   IF (dmvd_stop=1)
    ...
    GO TO exit_program
   ENDIF
   IF (dvdr_get_table(dmvd_masterid)=0)
    GO TO exit_program
   ENDIF
   IF (dmvd_masterid=0)
    SET dmvd_stop = 1
    SET dm_err->eproc = "No tables to compare"
    CALL disp_msg(" ",dm_err->logfile,0)
    GO TO exit_program
   ENDIF
   IF (dvdr_load_master(dmvd_masterid)=0)
    GO TO exit_program
   ENDIF
   EXECUTE dm2_verify_data
   IF ((dm_err->err_ind > 0))
    GO TO exit_program
   ENDIF
   IF ((datetimediff(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),cnvtdatetimeutc(cnvtdatetime(
    ...
    SET dm_err->eproc = concat(build(dmvd->timeout),
     " minute run time expired, ending DM2_VERIFY_DATA:",dmvd->tbls[1].owner,".",dmvd->tbls[1].
     table_name)
    CALL disp_msg("",dm_err->logfile,0)
    SET dmvd_stop = 1
   ENDIF
   SET dmvd->tbl_cnt = 0
   SET stat = alterlist(dmvd->tbls,0)
 ENDWHILE
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "DM2_MIG_VERIFY_DATA completed"
 CALL final_disp_msg("DM2_MIG_VERIFY_DATA")
END GO
