CREATE PROGRAM cv_da_update_cv_step:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_update_cv_step_vrsn = vc WITH private, constant("20230430")
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     .cv_doc_template_id)
      alteredind = 1
     ENDIF
     IF (validate(request->objarray[i].study_identifier,c.study_identifier) != char(128)
      AND validate(request->objarray[i].study_identifier,c.study_identifier) != c.study_identifier)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].modality_cd,c.modality_cd) != - (0.00001))
      AND validate(request->objarray[i].modality_cd,c.modality_cd) != c.modality_cd)
      alteredind = 1
     ENDIF
     IF (((validate(request->objarray[i].study_dt_tm,c.study_dt_tm) != 0.0
      AND validate(request->objarray[i].study_dt_tm,c.study_dt_tm) != c.study_dt_tm) OR (validate(
      request->objarray[i].study_dt_tm_null,0)=1)) )
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].vendor_cd,c.vendor_cd) != - (0.00001))
      AND validate(request->objarray[i].vendor_cd,c.vendor_cd) != c.vendor_cd)
      alteredind = 1
     ENDIF
     IF (validate(request->objarray[i].pdf_doc_identifier,c.pdf_doc_identifier) != char(128)
      AND validate(request->objarray[i].pdf_doc_identifier,c.pdf_doc_identifier) != c
     .pdf_doc_identifier)
      alteredind = 1
     ENDIF
    WITH forupdate(c)
   ...
   IF (alteredind > 0)
    UPDATE  FROM cv_step c
     SET c.cv_proc_id =
      ...
      ENDIF
      , c.study_identifier =
      IF (validate(request->objarray[i].study_identifier,char(128)) != char(128)) validate(request->
        objarray[i].study_identifier,char(128))
      ELSE c.study_identifier
      ENDIF
      , c.modality_cd =
      IF ((validate(request->objarray[i].modality_cd,- (0.00001)) != - (0.00001))) validate(request->
        objarray[i].modality_cd,- (0.00001))
      ELSE c.modality_cd
      ENDIF
      ,
      c.study_dt_tm =
      IF (validate(request->objarray[i].study_dt_tm,0.0) > 0.0) cnvtdatetime(validate(request->
         objarray[i].study_dt_tm,0.0))
      ELSEIF (validate(request->objarray[i].study_dt_tm_null,0)=1) null
      ELSE c.study_dt_tm
      ENDIF
      , c.vendor_cd =
      IF ((validate(request->objarray[i].vendor_cd,- (0.00001)) != - (0.00001))) validate(request->
        objarray[i].vendor_cd,- (0.00001))
      ELSE c.vendor_cd
      ENDIF
      , c.pdf_doc_identifier =
      IF (validate(request->objarray[i].pdf_doc_identifier,char(128)) != char(128)) validate(request
        ->objarray[i].pdf_doc_identifier,char(128))
      ELSE c.pdf_doc_identifier
      ENDIF
     WHERE (c.cv_step_id=request->objarray[i].cv_step_id)
     WITH nocounter
    ;end update
