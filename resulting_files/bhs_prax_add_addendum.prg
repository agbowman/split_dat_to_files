CREATE PROGRAM bhs_prax_add_addendum
 FREE RECORD result
 RECORD result(
   1 addendum = vc
   1 new_blob = vc
 ...
 IF ((result->event_class_cd != c_mdoc_cd))
  ...
  SELECT INTO "NL:"
   FROM ce_blob cb
   PLAN (cb
    WHERE (cb.event_id=result->parent_event_id)
     AND ((cb.valid_until_dt_tm+ 0) > cnvtdatetime(curdate,curtime3)))
   ...
   HEAD cb.event_id
    ...
    FOR (idx = 1 TO fill_loops)
      result->prev_blob = notrim(concat(notrim(result->prev_blob),notrim(fillstring(32768," "))))
    ENDFOR
    finlen = mod(cb.blob_length,segmentlen), result->prev_blob = notrim(concat(notrim(result->
       prev_blob),notrim(substring(1,finlen,fillstring(32768," ")))))
   DETAIL
    retlen = 1, offset = 0
    WHILE (retlen > 0)
   ...
   FOOT  cb.event_id
    ...
    uar_ocf_uncompress(blobrec->good_blob,size(blobrec->good_blob),result->prev_blob,size(result->
      prev_blob),newsize),
    CALL echo(build("RESULT->PREV_BLOB SIZE: ",size(result->prev_blob))),
    CALL echo(build("RESULT->PREV_BLOB:",result->prev_blob))
 ...
 SELECT INTO "NL:"
  ...
  WITH nocounter, time = 30
 ;end select
 SET result->new_blob = nullterm(concat(
"{\rtf1\ansi\ansicpg1252\uc1\deff0{\fonttbl  {\f0\fnil\fcharset0\fprq2 Arial;}  {\f1\fswiss\fcharset0\fprq2 Arial;}  {\f2\f\
roman\f\
",
"charset2\fprq2 Symbol;}}  {\colortbl;\red0\green0\blue0;}  {\stylesheet{\s0\itap0\nowidctlpar\f0\fs24 [Normal];}{\*\cs10\a\
dditive \
",
"Default Paragraph Font;}}  {\*\generator TX_RTF32 18.0.541.501;}  \paperw15000\paperh15840\margl1440\margt1440\margr1440\m\
argb14\
",
"40\deftab1134\widowctrl\lytexcttp\formshade  {\*\background{\shp{\*\shpinst\shpleft0\shptop0\shpright0\shpbottom0\shpfhdr0\
\shpbx\
",
"margin\shpbxignore\shpbymargin\shpbyignore\shpwr0\shpwrk0\shpfblwtxt1\shplid1025{\sp{\sn shapeType}{\sv 1}}{\sp{\sn fFlipH\
}{\sv \
",
"0}}{\sp{\sn fFlipV}{\sv 0}}{\sp{\sn fillColor}{\sv 16777215}}{\sp{\sn fFilled}{\sv 1}}{\sp{\sn lineWidth}{\sv 0}}{\sp{\sn \
fLine}{\
",
"\sv 0}}{\sp{\sn fBackground}{\sv 1}}{\sp{\sn fLayoutInCell}{\sv 1}}}}}\sectd  \headery720\footery720\pgwsxn15000\pghsxn158\
40\mar\
",
   "glsxn1440\margtsxn1440\margrsxn1440\margbsxn1440\pgbrdropt32\pard\itap0\nowidctlpar\plain\f1\fs20\cf1 ",
   " \par ",result->addendum,
   "\par }"))
...
#exit_script
 ...
 IF (size(trim(moutputdevice,3)) > 0)
  SELECT INTO value(moutputdevice)
   ...
   HEAD REPORT
    ...
    v1, row + 1, v2 = build("<EventID>",cnvtint(result->new_event_id),"</EventID>"),
    col + 1, v2, row + 1,
 ...
 SUBROUTINE calleventensureserver(null)
    ...
    SELECT INTO "NL:"
     ...
     PLAN (cen
      WHERE (cen.event_id= $2)
       AND ((cen.valid_until_dt_tm+ 0) > cnvtdatetime(curdate,curtime3)))
      JOIN (lb
   ...
   SET iret = uar_srvsetstring(hcestruct,"COLLATING_SEQ",evaluate(result->event_class_cd,c_mdoc_cd,"",
     "1"))
   SET iret = uar_srvsetlong(hcestruct,"UPDT_CNT",result->updt_cnt)
   SET iret = uar_srvsetdouble(hcestruct,"ENTRY_MODE_CD",evaluate(result->event_class_cd,c_mdoc_cd,
     c_undefined_cd,c_clin_notes_cd))
   ...
   IF (size(result->prev_blob,1) > 0
    AND (result->event_class_cd != c_mdoc_cd))
    SET hblobresultitem = uar_srvadditem(hcestruct,"BLOB_RESULT")
    SET iret = uar_srvsetdouble(hblobresultitem,"SUCCESSION_TYPE_CD",c_final_cd)
    SET iret = uar_srvsetdouble(hblobresultitem,"STORAGE_CD",c_long_blob_cd)
    SET iret = uar_srvsetdouble(hblobresultitem,"FORMAT_CD",c_rtf_cd)
    SET iret = uar_srvsetdouble(hblobresultitem,"EVENT_ID",cnvtreal( $2))
    ...
    SET iret = uar_srvsetasis(hblobitem,"BLOB_CONTENTS",result->prev_blob,size(result->prev_blob,1))
    SET iret = uar_srvsetdouble(hblobitem,"EVENT_ID",cnvtreal( $2))
    SET iret = uar_srvsetshort(hblobitem,"VALID_FROM_DT_TM_IND",1)
    SET iret = uar_srvsetshort(hblobitem,"VALID_UNTIL_DT_TM_IND",1)
    SET iret = uar_srvsetlong(hblobitem,"BLOB_LENGTH",size(result->prev_blob,1))
   ENDIF
   ...
   FOR (idx = 1 TO event_prsnl_cnt)
     ...
     SET iret = uar_srvsetdouble(hprsnllistitem,"ACTION_TYPE_CD",result->event_prsnl[idx].
      action_type_cd)
     SET iret = uar_srvsetdate(hprsnllistitem,"ACTION_DT_TM",result->event_prsnl[idx].action_dt_tm)
     SET iret = uar_srvsetdouble(hprsnllistitem,"ACTION_PRSNL_ID",result->event_prsnl[idx].
      action_prsnl_id)
     SET iret = uar_srvsetdouble(hprsnllistitem,"ACTION_STATUS_CD",result->event_prsnl[idx].
      action_status_cd)
