CREATE PROGRAM dm_eucb_mp_primed_pop:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_EUCB_MP_PRIMED_POP"
  ...
  GO TO exit_script
 ENDIF
 DECLARE cust_ucb_upt(dummy=i2) = null
 DECLARE cust_ucb_dummy = i2
 DECLARE d_person_id = f8
 ...
 SUBROUTINE cust_ucb_upt(dummy)
   SELECT INTO "NL:"
    FROM encounter e
    WHERE e.encntr_id IN (request->xxx_uncombine[ucb_cnt].from_xxx_id, request->xxx_uncombine[ucb_cnt
    ].to_xxx_id)
    DETAIL
     d_person_id = e.person_id
    WITH nocounter
   ;end select
   SELECT INTO "NL:"
    FROM mp_primed_view_pop mdp
    WHERE mdp.person_id=d_person_id
   ...
   DELETE  FROM mp_primed_view_act mda
    WHERE mda.person_id=d_person_id
    WITH nocounter
   ;end delete
   ...
   IF (curqual=0)
    ...
    GO TO exit_script
   ENDIF
   SET activity_updt_cnt = (activity_updt_cnt+ curqual)
 END ;Subroutine
#exit_script
END GO
