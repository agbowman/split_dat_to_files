CREATE PROGRAM bhs_cls_physstats:dba
 PROMPT
 ...
 ELSE
  SET beg_date_qual = cnvtdate( $PROMPT2)
 ENDIF
 IF (( $PROMPT3="ENDOFPREVMONTH"))
  ...
  SET end_date_qual = (cnvtdate(concat(format(current_month,"##"),"01",format(current_year,"####")))
   - 1)
 ELSE
  SET end_date_qual = cnvtdate( $PROMPT3)
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  PLAN (oa
   ...
   WHERE p.person_id=outerjoin(oa.order_provider_id)
    AND  NOT (oa.order_provider_id IN (null, 1934566, 1196828, 589879, 589827,
 ...
 SELECT INTO value(output_dest)
  ...
  HEAD REPORT
   ...
   IF (( $PROMPT6="1"))
    ...
    ENDIF
   ELSE
    row + 1, output_string = concat(',"EndDate","DocName","Specialty","Facility","Facility_Group"',
     ',"MD Ord","Prot Ord","PhVb Ord","SID Ord","Writ Ord",','"Other Ord","Total Ord","CPOE_COUNT"'),
    col 1,
    output_string, row + 1
   ENDIF
  ...
  DETAIL
   ...
   ELSE
    ...
    ELSEIF (loc_facility_display IN ("BWH", "BWH INPT PSYCH"))
     loc_facility_group = "BWH "
    ELSEIF (loc_facility_display IN ("BNH", "BNH INPT PSYCH"))
     loc_facility_group = "BNH "
    ...
    ENDIF
    cpoe_count = (phys->list[d.seq].mdcount+ phys->list[d.seq].wrcount), output_string = build(",",
     end_date_disp,',"',docname,'","',
     phys_type,'","',loc_facility_display,'","',loc_facility_group,
     '",',phys->list[d.seq].mdcount,",",phys->list[d.seq].prcount,",",
     phys->list[d.seq].pvcount,",",phys->list[d.seq].sidcount,",",phys->list[d.seq].wrcount,
     ",",phys->list[d.seq].otcount,",",phys->list[d.seq].totcount,",",
     cpoe_count), col 1,
    output_string, row + 1
   ENDIF
