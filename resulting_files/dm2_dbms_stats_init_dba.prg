CREATE PROGRAM dm2_dbms_stats_init:dba
 ...
 DECLARE ddsi_owner = vc WITH protect, noconstant("")
 DECLARE ddsi_table = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_dbms_stats_init",".log","DM2_DBMS_STATS_INIT LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dr_err_ind = dm_err->err_ind
 SET dr_err_emsg = dm_err->emsg
 SET dr_err_eproc = dm_err->eproc
 SET dm_err->eproc = "Beginning DM2_DBMS_STATS_INIT"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying input parameters."
 SET ddsi_owner = trim(cnvtupper( $1),3)
 SET ddsi_table = trim(cnvtupper( $2),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = "Parameter usage: dm2_dbms_stats_init '<owner>', '<table_name>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dcr_get_optimizer_settings(null)=0)
  GO TO exit_script
 ENDIF
 SET dcr_optvalues->implementer_opt_mode = "COST"
 SET dcr_optvalues->force_ind = 1
 EXECUTE dm2_runstats value(ddsi_owner), value(ddsi_table), "GATHER",
 0
 SET dm_err->err_ind = 0
#exit_script
 SET dm_err->eproc = "Ending DM2_DBMS_STATS_INIT."
 CALL final_disp_msg("dm2_dbms_stats_init")
