CREATE PROGRAM dm_ecmb_encntr_ambulance:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "ENCNTR_AMBULANCE"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_ECMB_ENCNTR_AMBULANCE"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count_from].from_id = frm.encntr_ambulance_id, rreclist->from_rec[
   v_cust_count_from].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count_from].
   active_status_cd = frm.active_status_cd,
   rreclist->from_rec[v_cust_count_from].company_name = frm.company_name, rreclist->from_rec[
   v_cust_count_from].service_number_ident = frm.service_number_ident
 ...
 IF (v_cust_count_from > 0)
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count_to].company_name = tu.company_name, rreclist->to_rec[
    v_cust_count_to].service_number_ident = tu.service_number_ident
  ...
  IF (has_active_rows_ind=1)
   FOR (v_cust_loopcount_from = 1 TO v_cust_count_from)
     IF ((rreclist->from_rec[v_cust_loopcount_from].active_ind=1)
      AND del_from(rreclist->from_rec[v_cust_loopcount_from].from_id,rreclist->from_rec[
      v_cust_loopcount_from].active_ind,rreclist->from_rec[v_cust_loopcount_from].active_status_cd)=0
     )
      GO TO exit_sub
     ENDIF
     SET pos = locateval(v_cust_searchindex,1,size(rreclist->to_rec,5),rreclist->from_rec[
      v_cust_loopcount_from].company_name,rreclist->to_rec[v_cust_searchindex].company_name,
      rreclist->from_rec[v_cust_loopcount_from].service_number_ident,rreclist->to_rec[
      v_cust_searchindex].service_number_ident)
     IF (pos=0)
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =i4)
   UPDATE  FROM encntr_ambulance frm
    SET frm.active_ind = false, frm.active_status_cd = combinedaway, frm.active_status_dt_tm =
     ...
     cnvtdatetime(trans_dt_tm)
    WHERE frm.encntr_ambulance_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "ENCNTR_AMBULANCE"
   ...
   IF (add_hist(s_df_pk_id)=0)
    ...
    SET request->error_message = substring(1,132,build(
      "Could not insert history for encntr_ambulance with pk=",s_df_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM encntr_ambulance frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ->updt_applctx,
     frm.updt_task = reqinfo->updt_task, frm.updt_dt_tm = cnvtdatetime(trans_dt_tm), frm.encntr_id =
     s_uf_to_fk_id
    WHERE frm.encntr_ambulance_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
 ...
 SUBROUTINE (add_hist(s_at_to_parent_pk_id=f8) =i4)
   ...
   SET dm_cmb_cust_cols2->add_col_val[1].col_name = "ENCNTR_AMBULANCE_HIST_ID"
   ...
   SET dm_cmb_cust_cols2->where_col_val[1].col_name = "ENCNTR_AMBULANCE_ID"
   SET dm_cmb_cust_cols2->where_col_val[1].col_value = build(s_at_to_parent_pk_id)
   IF (size(dm_cmb_cust_cols2->col,5)=0)
    SET dm_cmb_cust_cols2->tbl_name = "ENCNTR_AMBULANCE_HIST"
    SET dm_cmb_cust_cols2->sub_select_from_tbl = "ENCNTR_AMBULANCE"
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0)
    ...
    SET dcipht_request->transaction_reason_txt = "DM_ECMB_ENCNTR_AMBULANCE"
