CREATE PROGRAM cp_triggering_criteria_readme:dba
 ...
 SET readme_data->message = "Readme Failed:  Starting script cp_triggering_criteria_readme"
 ...
 DECLARE getcodevaluebymeaning(i4,vc) = f8
 DECLARE getnextsequence(null) = f8
 DECLARE main(null) = null
 DECLARE handleerror(vc) = null
 DECLARE migratelegacytriggeringcriteria(null) = null
 DECLARE populatelegacytriggeringcriteria(null) = null
 RECORD legacy_triggering_criteria(
   1 pathway_cnt = i4
   1 pathway_qual[*]
     2 cp_pathway_id = f8
     2 triggering_criteria_cnt = i4
     2 triggering_criteria_qual[*]
 ...
 SUBROUTINE main(null)
   ...
   CALL populatelegacytriggeringcriteria(null)
   CALL migratelegacytriggeringcriteria(null)
   CALL log_message(build("Exit main(), Elapsed time in seconds:",((curtime3 - begin_curtime3)/ 100.0
     )),log_level_debug)
 END ;Subroutine
 SUBROUTINE populatelegacytriggeringcriteria(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   CALL log_message("Begin populateLegacyTriggeringCriteria()",log_level_debug)
   DECLARE is_a_cd = f8 WITH constant(getcodevaluebymeaning(4003133,"IS-A")), protect
   DECLARE pathway_type_cd = f8 WITH constant(getcodevaluebymeaning(4003197,"CPM")), protect
   DECLARE active_pathway_cd = f8 WITH constant(getcodevaluebymeaning(4003198,"ACTIVE")), protect
   DECLARE oncology_type_cd = f8 WITH constant(getcodevaluebymeaning(4003197,"ONCOLOGY")), protect
   DECLARE pathway_cnt = i4 WITH noconstant(0), protect
   DECLARE criteria_cnt = i4 WITH noconstant(0), protect
   SELECT INTO "nl:"
    FROM cp_pathway cp,
     ...
     nomenclature n
    PLAN (cp
     WHERE cp.pathway_status_cd=active_pathway_cd
      AND cp.pathway_type_cd IN (pathway_type_cd, oncology_type_cd))
     JOIN (ccg
     WHERE ccg.concept_cd=cp.cp_concept_cd
      AND ccg.concept_group_cd=is_a_cd)
     JOIN (ccgd
     WHERE ccgd.cp_concept_group_id=ccg.cp_concept_group_id
      AND ccgd.concept_entity_name="NOMENCLATURE")
     JOIN (n
     WHERE n.nomenclature_id=ccgd.concept_entity_id)
    ORDER BY cp.cp_pathway_id
    HEAD cp.cp_pathway_id
     criteria_cnt = 0, pathway_cnt = (pathway_cnt+ 1)
     IF (pathway_cnt > size(legacy_triggering_criteria->pathway_qual,5))
      stat = alterlist(legacy_triggering_criteria->pathway_qual,(pathway_cnt+ 10))
     ENDIF
     legacy_triggering_criteria->pathway_qual[pathway_cnt].cp_pathway_id = cp.cp_pathway_id
    HEAD ccgd.cp_concept_group_dtl_id
     criteria_cnt = (criteria_cnt+ 1)
     IF (criteria_cnt > size(legacy_triggering_criteria->pathway_qual[pathway_cnt].
      triggering_criteria_qual,5))
      stat = alterlist(legacy_triggering_criteria->pathway_qual[pathway_cnt].triggering_criteria_qual,
       (criteria_cnt+ 10))
     ENDIF
     legacy_triggering_criteria->pathway_qual[pathway_cnt].triggering_criteria_qual[criteria_cnt].
     criteria_type_mean = "THIS_VISIT_PROBLEM", legacy_triggering_criteria->pathway_qual[pathway_cnt]
     .triggering_criteria_qual[criteria_cnt].triggering_criteria_ident = n.concept_cki
    FOOT  cp.cp_pathway_id
     legacy_triggering_criteria->pathway_qual[pathway_cnt].triggering_criteria_cnt = criteria_cnt,
     stat = alterlist(legacy_triggering_criteria->pathway_qual[pathway_cnt].triggering_criteria_qual,
      criteria_cnt)
    FOOT REPORT
     legacy_triggering_criteria->pathway_cnt = pathway_cnt, stat = alterlist(
      legacy_triggering_criteria->pathway_qual,pathway_cnt)
    WITH nocounter
   ;end select
   SET err_code = error(err_msg,0)
   IF (err_code > 0)
    CALL handleerror(concat("Error in populateLegacyTriggeringCriteria: ",err_msg))
   ENDIF
   CALL log_message(build("Exit populateLegacyTriggeringCriteria(), Elapsed time in seconds:",((
     curtime3 - begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE migratelegacytriggeringcriteria(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   CALL log_message("Begin migrateLegacyTriggeringCriteria()",log_level_debug)
   DECLARE x = i4 WITH noconstant(0), protect
   DECLARE y = i4 WITH noconstant(0), protect
   DECLARE insert_ind = i2 WITH noconstant(0), protect
   DECLARE cp_triggering_criteria_id = f8 WITH noconstant(0.0), protect
   FOR (x = 1 TO legacy_triggering_criteria->pathway_cnt)
     FOR (y = 1 TO legacy_triggering_criteria->pathway_qual[x].triggering_criteria_cnt)
       SET insert_ind = 1
       SELECT INTO "nl:"
        FROM cp_triggering_criteria ctc
        PLAN (ctc
         WHERE ctc.cp_pathway_id=cnvtreal(legacy_triggering_criteria->pathway_qual[x].cp_pathway_id)
          AND ctc.triggering_entity_id=cnvtreal(legacy_triggering_criteria->pathway_qual[x].
          triggering_criteria_qual[y].triggering_entity_id)
          AND (ctc.triggering_entity_name=legacy_triggering_criteria->pathway_qual[x].
         triggering_criteria_qual[y].triggering_entity_name)
          AND (ctc.triggering_criteria_ident=legacy_triggering_criteria->pathway_qual[x].
         triggering_criteria_qual[y].triggering_criteria_ident)
          AND (ctc.criteria_type_mean=legacy_triggering_criteria->pathway_qual[x].
         triggering_criteria_qual[y].criteria_type_mean))
        HEAD REPORT
         insert_ind = 0
        WITH nocounter
       ;end select
       SET err_code = error(err_msg,0)
       IF (err_code > 0)
        CALL handleerror(concat("Error in migrateLegacyTriggeringCriteria search: ",err_msg))
       ENDIF
       IF (insert_ind=1)
        SET cp_triggering_criteria_id = getnextsequence(null)
        IF (cp_triggering_criteria_id > 0)
         INSERT  FROM cp_triggering_criteria ctc
          SET ctc.cp_triggering_criteria_id = cp_triggering_criteria_id, ctc.cp_pathway_id =
           legacy_triggering_criteria->pathway_qual[x].cp_pathway_id, ctc.criteria_type_mean =
           legacy_triggering_criteria->pathway_qual[x].triggering_criteria_qual[y].criteria_type_mean,
           ctc.triggering_criteria_ident = legacy_triggering_criteria->pathway_qual[x].
           triggering_criteria_qual[y].triggering_criteria_ident, ctc.triggering_entity_id = cnvtreal
           (legacy_triggering_criteria->pathway_qual[x].triggering_criteria_qual[y].
            triggering_entity_id), ctc.triggering_entity_name = legacy_triggering_criteria->
           pathway_qual[x].triggering_criteria_qual[y].triggering_entity_name,
           ctc.updt_applctx = reqinfo->updt_applctx, ctc.updt_cnt = 0, ctc.updt_dt_tm = cnvtdatetime(
            curdate,curtime3),
           ctc.updt_id = reqinfo->updt_id, ctc.updt_task = reqinfo->updt_task
          WITH nocounter
         ;end insert
         SET err_code = error(err_msg,0)
         IF (err_code > 0)
          CALL handleerror(concat("Error in migrateLegacyTriggeringCriteria insert: ",err_msg))
         ENDIF
        ELSE
         CALL handleerror("Error in migrateLegacyTriggeringCriteria: invalid ID.")
        ENDIF
       ENDIF
     ENDFOR
   ENDFOR
   CALL log_message(build("Exit migrateLegacyTriggeringCriteria(), Elapsed time in seconds:",((
     curtime3 - begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getcodevaluebymeaning(codeset,meaning)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE ret_val = f8 WITH noconstant(0.0), protect
   CALL log_message("Begin getCodeValueByMeaning()",log_level_debug)
   IF (codeset > 0
    AND textlen(trim(meaning,3)) > 0)
    SELECT INTO "nl:"
     FROM code_value cv
     PLAN (cv
     ...
     HEAD REPORT
      ret_val = cv.code_value
     WITH nocounter
    ;end select
    SET err_code = error(err_msg,0)
    IF (err_code > 0)
     CALL handleerror(concat("Error in getCodeValueByMeaning: ",err_msg))
    ENDIF
   ELSE
    CALL handleerror("Error in getCodeValueByMeaning: bad parameters.")
   ENDIF
   CALL log_message(build("Exit getCodeValueByMeaning(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(ret_val)
 END ;Subroutine
 SUBROUTINE getnextsequence(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE ret_val = f8 WITH noconstant(0.0), protect
   CALL log_message("Begin getNextSequence()",log_level_debug)
   SELECT INTO "nl:"
   ...
   SET err_code = error(err_msg,0)
   IF (err_code > 0)
    CALL handleerror(concat("Error in getNextSequence: ",err_msg))
   ENDIF
   CALL log_message(build("Exit getNextSequence(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(ret_val)
 END ;Subroutine
 SUBROUTINE handleerror(error_message)
   ROLLBACK
   CALL echo(error_message)
   SET readme_data->message = error_message
   SET readme_data->status = "F"
   GO TO exit_script
 END ;Subroutine
 CALL log_message(build("Starting Script:",log_program_name),log_level_debug)
 CALL main(null)
 SET readme_data->message = "Successfully migrated Care Pathways Triggering Criteria."
 SET readme_data->status = "S"
 COMMIT
#exit_script
 FREE RECORD legacy_triggering_criteria
