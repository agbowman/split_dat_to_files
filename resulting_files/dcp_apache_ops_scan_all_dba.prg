CREATE PROGRAM dcp_apache_ops_scan_all:dba
 RECORD reply(
   1 ops_event = vc
   1 status_data
     2 status = c1
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (rar.accept_tiss_acttx_if_ind=1)
    CALL echo("TISS ACT = 1"), accept_tiss_acttx_if_ind = 1
   ENDIF
   IF (rar.accept_tiss_nonacttx_if_ind=1)
    CALL echo("TISS NON ACT = 1"), accept_tiss_nonacttx_if_ind = 1
   ENDIF
   IF (rar.auto_calc_intubated_ind=1)
    auto_calc_intubated_ind = 1
   ENDIF
  WITH nocounter
 ;end select
 ...
 IF (accept_worst_vitals_ind >= 0)
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d)
    ...
    heartrate3_cd, systolic_cd, diastolic_cd, systolic3_cd, diastolic3_cd,
    diastolic4_cd, systolic2_cd, diastolic2_cd, eyes_cd, motor_cd,
    verbal_cd, meds_cd))
     AND ce.event_cd > 0)
    JOIN (rad
    WHERE (rad.risk_adjustment_id=temp->clist[d.seq].risk_adjustment_id)
   ...
   DETAIL
    ...
    IF ((((rad.vent_ind=- (1))) OR (rad.vent_ind=0
     ...
     CALL echo(build("setting unset_vent_ind = 1")), temp->clist[d.seq].dlist[dcnt].unset_vent_ind =
     1, new_possible_worst = "Y"
    ELSE
    ...
    ELSEIF (ce.event_cd=eyes_cd)
     temp->clist[d.seq].dlist[dcnt].old_worst = rad.worst_gcs_eye_score, worst_eyes = rad
     .worst_gcs_eye_score
    ELSEIF (ce.event_cd=motor_cd)
     temp->clist[d.seq].dlist[dcnt].old_worst = rad.worst_gcs_motor_score, worst_motor = rad
     .worst_gcs_motor_score
    ELSEIF (ce.event_cd=verbal_cd)
     temp->clist[d.seq].dlist[dcnt].old_worst = rad.worst_gcs_verbal_score, worst_verbal = rad
     .worst_gcs_verbal_score
    ELSEIF (ce.event_cd=meds_cd)
     temp->clist[d.seq].dlist[dcnt].old_worst = rad.meds_ind, worst_meds = rad.meds_ind
    ENDIF
 ...
 SELECT INTO "nl:"
  ...
  HEAD rad.cc_day
   tiss_found = 0
  DETAIL
   CALL echo("IN VENT DETAIL SECTION - YEAH!!!"), dcnt = (dcnt+ 1), stat = alterlist(temp->clist[d
    .seq].dlist,dcnt),
   temp->clist[d.seq].dlist[dcnt].clinical_event_id = 0.0, temp->clist[d.seq].dlist[dcnt].
   risk_adjustment_day_id = rad.risk_adjustment_day_id, temp->clist[d.seq].dlist[dcnt].cc_day = rad
   .cc_day,
   temp->clist[d.seq].dlist[dcnt].cc_beg_dt_tm = cnvtdatetime(rad.cc_beg_dt_tm), temp->clist[d.seq].
   dlist[dcnt].cc_end_dt_tm = cnvtdatetime(rad.cc_end_dt_tm), temp->clist[d.seq].dlist[dcnt].event_cd
    = 0.0,
   temp->clist[d.seq].dlist[dcnt].lab_ind = 0, temp->clist[d.seq].dlist[dcnt].vital_ind = 0, temp->
   clist[d.seq].dlist[dcnt].tiss_ce_ind = 0,
   temp->clist[d.seq].dlist[dcnt].unset_vent_ind = 1, temp->clist[d.seq].dlist[dcnt].temp_ind = 0,
   temp->clist[d.seq].dlist[dcnt].urine_ind = 0,
   temp->clist[d.seq].dlist[dcnt].worst_resolved_ind = 0, temp->clist[d.seq].dlist[dcnt].old_worst =
   0.0, new_possible_worst = "Y"
  FOOT  d.seq
   temp->clist[d.seq].dcnt = dcnt
  WITH nocounter
 ;end select
 IF (new_possible_worst="N"
  ...
  "dcp_apache_ops_scan_all successful completion (no changes made)"
  SET reply->ops_event = "The scanner has run successfully."
  GO TO exit_program
 ENDIF
 IF (new_possible_worst="Y")
  CALL echo("going to look for new worsts")
  SET one_new_worst = "N"
 ...
 FOR (x = 1 TO ccnt)
   ...
   ELSEIF ((((temp->clist[x].sex_chg_ind=1)) OR ((((temp->clist[x].age_chg_ind=1)) OR ((((temp->
   clist[x].hosp_admit_dt_tm_chg_ind=1)) OR ((temp->clist[x].vent_flag_issue_ind=1))) )) )) )
    EXECUTE FROM update_ra TO update_ra_exit
   ENDIF
   CALL echo(build("temp->clist[x].dcnt=",temp->clist[x].dcnt))
   IF ((temp->clist[x].dcnt > 0))
    IF ((temp->clist[x].tiss_changed_ind=1))
     SET one_new_worst = "Y"
    ENDIF
    CALL echo("BEFORE NEW_WORST_CHECK")
 ...
 ELSE
  SET reply->status_data.status = "S"
  SET reply->subeventstatus[1].targetobjectvalue = "dcp_apache_ops_scan_all successful completion"
  SET reply->ops_event = "The scanner has run successfully."
  COMMIT
 ENDIF
 GO TO exit_program
...
#update_ra
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   temp3->admitsource_flag = ra.admitsource_flag, temp3->discharge_location_cd = ra
   .discharge_location_cd, temp3->hrs_at_source = ra.hrs_at_source,
   ...
   temp3->hosp_admit_dt_tm = cnvtdatetime(ra.hosp_admit_dt_tm), temp3->gender = ra.gender_flag, temp3
   ->teach_type_flag = ra.teach_type_flag,
   ...
   temp3->adm_doc_id = ra.adm_doc_id, temp3->disease_category_cd = ra.disease_category_cd, temp3->
   med_service_cd = ra.med_service_cd,
  ...
  ELSE
   ...
   INSERT  FROM risk_adjustment ra
    SET ra.risk_adjustment_id = ra_id, ra.person_id = temp3->person_id, ra.encntr_id = temp3->
     ...
     ra.admitsource_flag = temp3->admitsource_flag, ra.discharge_location_cd = temp3->
     discharge_location_cd, ra.hrs_at_source = temp3->hrs_at_source,
 ...
 IF (error_flag="N")
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    temp4->dlist[cnt].meds_ind = rad.meds_ind, temp4->dlist[cnt].urine = rad.urine_output, temp4->
    dlist[cnt].urine_24hr = rad.urine_24hr_output,
    temp4->dlist[cnt].wbc = rad.worst_wbc_result, temp4->dlist[cnt].wbc_ce_id = rad.wbc_ce_id, temp4
    ->dlist[cnt].temp = rad.worst_temp,
    temp4->dlist[cnt].temp_ce_id = rad.temp_ce_id, temp4->dlist[cnt].resp = rad.worst_resp_result,
    temp4->dlist[cnt].resp_ce_id = rad.resp_ce_id,
    temp4->dlist[cnt].vent_ind = rad.vent_ind, temp4->dlist[cnt].sodium = rad.worst_sodium_result,
    temp4->dlist[cnt].sodium_ce_id = rad.sodium_ce_id,
    temp4->dlist[cnt].heartrate = rad.worst_heart_rate, temp4->dlist[cnt].heartrate_ce_id = rad
    .heartrate_ce_id, temp4->dlist[cnt].meanbp = rad.mean_blood_pressure,
    temp4->dlist[cnt].ph = rad.worst_ph_result, temp4->dlist[cnt].ph_ce_id = rad.ph_ce_id, temp4->
    dlist[cnt].hematocrit = rad.worst_hematocrit,
    temp4->dlist[cnt].hematocrit_ce_id = rad.hematocrit_ce_id, temp4->dlist[cnt].creatinine = rad
    .worst_creatinine_result, temp4->dlist[cnt].creatinine_ce_id = rad.creatinine_ce_id,
    temp4->dlist[cnt].albumin = rad.worst_albumin_result, temp4->dlist[cnt].albumin_ce_id = rad
    .albumin_ce_id, temp4->dlist[cnt].pao2 = rad.worst_pao2_result,
    temp4->dlist[cnt].pao2_ce_id = rad.pao2_ce_id, temp4->dlist[cnt].pco2 = rad.worst_pco2_result,
    temp4->dlist[cnt].pco2_ce_id = rad.pco2_ce_id,
    temp4->dlist[cnt].bun = rad.worst_bun_result, temp4->dlist[cnt].bun_ce_id = rad.bun_ce_id, temp4
    ->dlist[cnt].glucose = rad.worst_glucose_result,
    temp4->dlist[cnt].glucose_ce_id = rad.glucose_ce_id, temp4->dlist[cnt].bilirubin = rad
    .worst_bilirubin_result, temp4->dlist[cnt].bilirubin_ce_id = rad.bilirubin_ce_id,
    temp4->dlist[cnt].potassium = rad.worst_potassium_result, temp4->dlist[cnt].potassium_ce_id = rad
    .potassium_ce_id, temp4->dlist[cnt].fio2 = rad.worst_fio2_result,
    temp4->dlist[cnt].fio2_ce_id = rad.fio2_ce_id, temp4->dlist[cnt].aps_score = rad.aps_score, temp4
    ->dlist[cnt].aps_day1 = rad.aps_day1,
    temp4->dlist[cnt].aps_yesterday = rad.aps_yesterday, temp4->dlist[cnt].activetx_ind = rad
    .activetx_ind, temp4->dlist[cnt].vent_today_ind = rad.vent_today_ind,
    temp4->dlist[cnt].pa_line_today_ind = rad.pa_line_today_ind, temp4->dlist[cnt].outcome_status =
    rad.outcome_status, temp4->dlist[cnt].apache_iii_score = rad.apache_iii_score,
    temp4->dlist[cnt].phys_res_pts = rad.phys_res_pts
   FOOT REPORT
    day_cnt = cnt
   WITH nocounter
  ;end select
...
#outcomes
 ...
 IF ((temp3->admit_source="CHPAIN_CTR"))
...
#create_rad
 ...
 ELSE
  ...
  INSERT  FROM risk_adjustment_day rad
   SET rad.risk_adjustment_day_id = rad_id, rad.risk_adjustment_id = ra_id, rad.cc_day = temp4->
    ...
    .urine_output = temp4->dlist[y].urine,
    rad.urine_24hr_output = temp4->dlist[y].urine_24hr, rad.worst_wbc_result = temp4->dlist[y].wbc,
    rad.wbc_ce_id = temp4->dlist[y].wbc_ce_id,
    rad.worst_temp = temp4->dlist[y].temp, rad.temp_ce_id = temp4->dlist[y].temp_ce_id, rad
    .worst_resp_result = temp4->dlist[y].resp,
    rad.resp_ce_id = temp4->dlist[y].resp_ce_id, rad.worst_sodium_result = temp4->dlist[y].sodium,
    rad.sodium_ce_id = temp4->dlist[y].sodium_ce_id,
    rad.worst_heart_rate = temp4->dlist[y].heartrate, rad.heartrate_ce_id = temp4->dlist[y].
    heartrate_ce_id, rad.mean_blood_pressure = temp4->dlist[y].meanbp,
    rad.worst_ph_result = temp4->dlist[y].ph, rad.ph_ce_id = temp4->dlist[y].ph_ce_id, rad
    .worst_hematocrit = temp4->dlist[y].hematocrit,
    rad.hematocrit_ce_id = temp4->dlist[y].hematocrit_ce_id, rad.worst_creatinine_result = temp4->
    dlist[y].creatinine, rad.creatinine_ce_id = temp4->dlist[y].creatinine_ce_id,
    rad.worst_albumin_result = temp4->dlist[y].albumin, rad.albumin_ce_id = temp4->dlist[y].
    albumin_ce_id, rad.worst_pao2_result = temp4->dlist[y].pao2,
    rad.pao2_ce_id = temp4->dlist[y].pao2_ce_id, rad.worst_pco2_result = temp4->dlist[y].pco2, rad
    .pco2_ce_id = temp4->dlist[y].pco2_ce_id,
    rad.worst_bun_result = temp4->dlist[y].bun, rad.bun_ce_id = temp4->dlist[y].bun_ce_id, rad
    .worst_glucose_result = temp4->dlist[y].glucose,
    rad.glucose_ce_id = temp4->dlist[y].glucose_ce_id, rad.worst_bilirubin_result = temp4->dlist[y].
    bilirubin, rad.bilirubin_ce_id = temp4->dlist[y].bilirubin_ce_id,
    rad.worst_potassium_result = temp4->dlist[y].potassium, rad.potassium_ce_id = temp4->dlist[y].
    potassium_ce_id, rad.worst_fio2_result = temp4->dlist[y].fio2,
    rad.fio2_ce_id = temp4->dlist[y].fio2_ce_id, rad.aps_score = temp4->dlist[y].aps_score, rad
    .aps_day1 = temp4->dlist[y].aps_day1,
    rad.aps_yesterday = temp4->dlist[y].aps_yesterday, rad.activetx_ind = temp4->dlist[y].
    activetx_ind, rad.vent_today_ind = temp4->dlist[y].vent_today_ind,
    rad.pa_line_today_ind = temp4->dlist[y].pa_line_today_ind, rad.outcome_status = temp4->dlist[y].
    outcome_status, rad.apache_iii_score = ap3_score,
    rad.phys_res_pts = value(temp4->dlist[y].phys_res_pts), rad.active_ind = 1, rad
    .active_status_dt_tm = cnvtdatetime(curdate,curtime3),
   ...
   IF ((temp4->dlist[y].outcome_status > 0))
    ...
    FOR (num = 1 TO 100)
      IF ((aps_outcome->qual[num].szequationname > " "))
       ...
       ;end select
       IF (rao_id=0.0)
        CALL echo("get_new_rao_id error")
        SET failed_ind = "Y"
 ...
 IF ((temp4->dlist[y].cc_day=1)
  AND error_flag="N")
  SET therapy_level = - (1)
  IF ((((temp4->dlist[y].outcome_status=- (23100))) OR ((((temp4->dlist[y].outcome_status=- (23103)))
   OR ((temp4->dlist[y].outcome_status=- (23117)))) )) )
...
#create_rad_exit
#new_worst_check
 CALL echo("TOP OF NEW WORST CHECK")
 SET ce_id = 0.0
 ...
 FOR (y = 1 TO temp->clist[x].dcnt)
   SET abg_flag = 0
   SET gcs_flag = 0
   IF ((temp->clist[x].dlist[y].unset_vent_ind=1))
    SET one_new_worst = "Y"
   ENDIF
   IF ((temp->clist[x].dlist[y].worst_resolved_ind=0))
    ...
    ENDIF
   ENDIF
   IF ((temp->clist[x].dlist[y].worst_resolved_ind=1)
 ...
 IF (((one_new_worst="Y") OR (found_tiss_change=1)) )
  ...
  SET cc_day = 0
  FOR (y = 1 TO temp->clist[x].dcnt)
    IF ((((temp->clist[x].dlist[y].worst_resolved_ind=2)) OR ((((temp->clist[x].dlist[y].tiss_ce_ind=
    1)) OR ((temp->clist[x].dlist[y].unset_vent_ind=1))) )) )
     SET cc_day = temp->clist[x].dlist[y].cc_day
     SET y = temp->clist[x].dcnt
    ENDIF
  ENDFOR
  IF (cc_day=0)
   CALL echo("CC_DAY STILL ZERO")
   CALL echo(build("temp->clist[x].risk_adjustment_id=",temp->clist[x].risk_adjustment_id))
   SET cc_day = 1
  ENDIF
  SET day_cnt = 0
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    temp4->dlist[cnt].resp_ce_id = rad.resp_ce_id, temp4->dlist[cnt].sodium = rad.worst_sodium_result,
    temp4->dlist[cnt].sodium_ce_id = rad.sodium_ce_id,
    temp4->dlist[cnt].heartrate = rad.worst_heart_rate, temp4->dlist[cnt].heartrate_ce_id = rad
    .heartrate_ce_id, temp4->dlist[cnt].meanbp = rad.mean_blood_pressure,
    temp4->dlist[cnt].ph = rad.worst_ph_result, temp4->dlist[cnt].ph_ce_id = rad.ph_ce_id, temp4->
       ...
       IF ((temp->clist[x].dlist[z].cc_day=temp4->dlist[y].cc_day))
        IF ((temp->clist[x].dlist[z].worst_resolved_ind=2))
         ...
         ELSEIF ((temp->clist[x].dlist[z].event_cd=meds_cd))
          ...
          SET temp4->dlist[y].meds_ind_ce_id = temp->clist[x].dlist[z].new_worst_ce_id
 ...
 IF (eyes_cd > 0.0
  ...
  SELECT INTO "nl:"
   ...
   HEAD REPORT
    IF (rad.meds_ind=1)
     rad_gcs_total = 15,
     CALL echo("setting last gcs_rad_total = 15")
    ELSEIF (rad.worst_gcs_eye_score > 0
     ...
     .worst_gcs_verbal_score),
     CALL echo(build("setting rad_gcs_total=",rad_gcs_total))
    ELSE
     CALL echo("can't set rad_gcs_total - leave =16")
    ENDIF
   ...
   DETAIL
    ...
    IF (ce.event_cd=eyes_cd)
     hold_eyes_ce_id = ce.clinical_event_id
     ...
     ENDIF
    ELSEIF (ce.event_cd=motor_cd)
     hold_motor_ce_id = ce.clinical_event_id
     ...
     ENDIF
    ELSEIF (ce.event_cd=verbal_cd)
     hold_verbal_ce_id = ce.clinical_event_id
     ...
     ENDIF
    ELSEIF (ce.event_cd=meds_cd)
     hold_meds_ce_id = ce.clinical_event_id
     IF (cnvtupper(ce.event_tag) IN ("ANESTHESIA", "SEDATION", "PARALYTICS"))
      temp_meds = 1
     ELSE
      temp_meds = 0
     ENDIF
   ...
   FOOT  ce.event_end_dt_tm
    IF (((temp_eyes > 0
     ...
     IF (((temp_total < gcs_total) OR (gcs_total=0)) )
      ...
      hold_verbal = temp_verbal, hold_meds = temp_meds
     ENDIF
    ENDIF
   WITH nocounter
  ;end select
  ...
  IF (((gcs_total < 1) OR (gcs_total >= rad_gcs_total)) )
   ...
   CALL echo("new GCS not worse")
  ELSE
   CALL echo("new GCS worst")
   SET one_new_worst = "Y"
   SET temp->clist[x].dlist[y].worst_resolved_ind = 2
...
#worst_resp
 ...
 IF (resp_cd > 0.0)
  ...
  SELECT INTO "nl:"
   ...
   FOOT  ce.event_end_dt_tm
    IF (temp_resp > 0)
     ...
     ENDIF
     IF (temp_diff > hold_diff)
      hold_diff = temp_diff, hold_resp = temp_resp
      IF (vent_cd > 0
       AND temp_vent < 0)
       temp_vent = 0.0
      ENDIF
      hold_vent = temp_vent, event_tag_num = temp_resp, ce_id = temp_ce_id
  ...
  IF ((hold_resp=temp->clist[x].dlist[y].old_worst)
   AND (hold_vent=temp->clist[x].dlist[y].old_vent_ind))
   SET temp->clist[x].dlist[y].worst_resolved_ind = 1
  ELSE
 ...
 IF (((systolic_cd > 0.0
  ...
  SELECT INTO "nl:"
   ...
   HEAD REPORT
    ...
    temp_sys3 = 0.0, temp_dia1 = 0.0, temp_dia2 = 0.0,
    temp_dia3 = 0.0, temp_dia4 = 0.0, temp_diff = - (1.0),
    hold_diff = - (1.0), isnum = 0
    IF (event_tag_num > 0)
...
#worst_abg
 ...
 IF (pco2_cd > 0.0
  ...
  AND fio2_cd > 0.0
  AND ph_cd > 0.0
  AND ((intub_cd > 0.0) OR (((intub2_cd > 0.0) OR (auto_calc_intubated_ind=1)) )) )
  SELECT INTO "nl:"
   FROM clinical_event ce,
    risk_adjustment ra,
    risk_adjustment_day rad
   ...
   HEAD REPORT
    IF (rad.worst_pao2_result > 0
     ...
     IF (((rad.intubated_ind=0) OR (rad.worst_fio2_result < 50.00)) )
   ...
   DETAIL
    ...
    IF (isnum > 0)
     ...
     ELSEIF (ce.event_cd=intub_cd)
      ...
      ELSE
       IF (temp_intub < 0)
        temp_intub = 0, temp_intub_ce_id = 0
       ENDIF
      ENDIF
     ENDIF
    ENDIF
...
#load_tiss_items_to_arrays
 ...
 IF (accept_tiss_nonacttx_if_ind=1)
  ...
  SET scan_tiss_list->list[67].ce_cd = uar_get_code_by_cki(nullterm("CKI.EC!8190"))
