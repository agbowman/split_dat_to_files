CREATE PROGRAM cp_get_triggering_criteria:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Pathway Id:" = 0.0,
  "User id:" = 0.0,
  "Position Cd:" = 0.0
  WITH outdev, inputpathwayid, user_id,
  position_cd
 RECORD report_data(
   1 target_vocab = f8
   1 dx_type = f8
   1 dx_confirmation = f8
   1 dx_classification = f8
   1 cnt = i4
   1 qual[*]
 ...
 DECLARE gettriggeringnomenclature = null WITH protect
 DECLARE gettargetvocab(null) = null WITH protect
 DECLARE index = i4 WITH noconstant(0), protect
 DECLARE pathway_id = f8 WITH protect, constant(cnvtreal( $INPUTPATHWAYID))
 DECLARE user_id = f8 WITH protect, constant(cnvtreal( $USER_ID))
 DECLARE position_cd = f8 WITH protect, constant(cnvtreal( $POSITION_CD))
 DECLARE application_num = i4 WITH protect, constant(reqinfo->updt_app)
 DECLARE comp_name = vc WITH protect, constant("ClinicalDx")
 ...
 CALL gettargetvocab(null)
 CALL gettriggeringnomenclature(null)
 SET report_data->status_data.status = "S"
 SUBROUTINE gettriggeringnomenclature(null)
   CALL log_message("In getTriggeringNomenclature()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   EXECUTE cp_retr_triggering_criteria "NOFORMS", value(pathway_id), 1 WITH replace(
    "TRIGGERING_CRITERIA_REPLY","REPORT_DATA")
   CALL log_message(build("Exit getTriggeringNomenclature(), Elapsed time in seconds:",((curtime3 -
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   IF (pathway_id=0)
    CALL reporterrorandexit("PERFORM","Validate","Pathway ID not provided.")
   ELSEIF (user_id=0)
    CALL reporterrorandexit("PERFORM","Validate","User ID not provided.")
   ELSEIF (position_cd=0)
    CALL reporterrorandexit("PERFORM","Validate","Position CD not provided.")
   ENDIF
 ...
 SUBROUTINE (reporterror(operationname=vc,targetobjectname=vc,targetobjectvalue=vc) =null WITH
  protect)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SET report_data->status_data.status = "F"
   SET report_data->status_data.subeventstatus.operationname = operationname
   SET report_data->status_data.subeventstatus.operationstatus = "F"
   SET report_data->status_data.subeventstatus.targetobjectname = targetobjectname
   SET report_data->status_data.subeventstatus.targetobjectvalue = targetobjectvalue
   CALL log_message(build("Exit reportError(), Elapsed time in seconds:",((curtime3 - begin_curtime3)
     / 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE gettargetvocab(null)
   CALL log_message("In getTargetVocab()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE default_vocab = f8 WITH noconstant(0), private
   SELECT INTO "nl:"
    FROM detail_prefs dp,
     name_value_prefs nv
    PLAN (dp
     WHERE dp.prsnl_id IN (0.0, user_id)
      AND dp.position_cd IN (0.0, position_cd)
      AND dp.application_number=application_num
      AND dp.comp_name=comp_name
      AND dp.view_name=comp_name
      AND dp.view_seq=0
      AND dp.comp_seq=0)
     JOIN (nv
     WHERE nv.parent_entity_id=dp.detail_prefs_id
      AND nv.parent_entity_name="DETAIL_PREFS"
      AND nv.pvc_name IN ("DX_TARGET_VOCABULARY", "DX_DEFAULT_TYPE", "DX_DEFAULT_CONFIRM",
     "DX_DEFAULT_CLASSIFICATION")
      AND nv.active_ind=1
      AND nv.merge_id > 0)
    ORDER BY nv.pvc_name, dp.prsnl_id DESC, dp.position_cd DESC
    HEAD nv.pvc_name
     CASE (cnvtupper(nv.pvc_name))
      OF "DX_TARGET_VOCABULARY":
       report_data->target_vocab = nv.merge_id
      OF "DX_DEFAULT_TYPE":
       report_data->dx_type = nv.merge_id
      OF "DX_DEFAULT_CONFIRM":
       report_data->dx_confirmation = nv.merge_id
      OF "DX_DEFAULT_CLASSIFICATION":
       report_data->dx_classification = nv.merge_id
     ENDCASE
    WITH nocounter
   ;end select
   IF ((report_data->target_vocab=0))
    CALL reporterrorandexit("PERFORM","Validate","TargetVocab not found")
   ENDIF
   CALL log_message(build("Exit getTargetVocab(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (validate(debug_ind,0)=1)
