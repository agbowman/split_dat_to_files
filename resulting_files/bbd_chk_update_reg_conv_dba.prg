CREATE PROGRAM bbd_chk_update_reg_conv:dba
 ...
 ENDIF
#start_script
 SET sreadme_name = "bbd_chk_update_reg_conv"
 CALL logscriptstart(sreadme_name)
 SET readme_data->status = "F"
#script
 DECLARE lerrorcode = i4 WITH noconstant(0)
 DECLARE serrormessage = c132 WITH noconstant(fillstring(132," "))
 DECLARE srdm_error = vc
 SET conv_id = 0
 SET select_failed_flag = 0
 SELECT INTO "nl:"
  p.seq
 ...
 IF (lerrorcode != 0)
  SET srdm_error = "Select failed"
 ENDIF
 IF (conv_id=0)
  SET select_failed_flag = 2
  SET readme_data->message = "BLOOD BANK DONOR REGISTRATION Conversation does not exist"
 ...
 SELECT INTO "nl:"
  ...
  WHERE p.prompt_id > 0
   AND p.parent_entity_id=conv_id
   AND p.parent_entity_name="PM_FLX_CONVERSATION"
   AND p.field="PERSON.MAIDEN_NAME.NAME_LAST"
   AND p.description="Maiden Name"
   AND p.label="Maiden Name:"
  WITH nocounter
 ;end select
 SET lerrorcode = error(serrormessage,0)
 IF (lerrorcode != 0)
 ...
 ENDIF
 IF (curqual=0)
  SET select_failed_flag = 1
  SET readme_data->message = "PM_FLX_PROMPT row for BLOOD BANK DONOR REGISTRATION not updated"
  GO TO exit_script
 ENDIF
 SET readme_data->status = "S"
#exit_script
 EXECUTE dm_readme_status
 IF (((lerrorcode != 0) OR (select_failed_flag=1)) )
  CALL logmsg(srdm_error,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  CALL logmsg(serrormessage,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  CALL logstatus("Update into PM_FLX_PROMPT failed","F")
 ELSEIF (select_failed_flag=2)
  CALL logmsg("Blood Bank Donor Registration Conv. does not exist. Readme Successful.",((e_rdmstatus
   + e_dispmsg)+ e_logtofile))
  CALL logstatus("No row found to update.  Readme Successful.","S")
 ELSE
  COMMIT
  CALL logmsg("PM_FLX_PROMPT row updated successfully.",((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  CALL logstatus("update Successful.","S")
 ENDIF
 CALL echorecord(readme_data)
