CREATE PROGRAM dm_bb_worklist_rows:dba
 ...
 SET reply->table_name = "BB_WORKLIST"
 SET reply->rows_between_commit = 50
 FOR (ltokenidx = 1 TO size(request->tokens,5))
   IF ((request->tokens[ltokenidx].token_str="RETENTIONDAYS"))
 ...
 IF (ldaystokeep < 0)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"INVALIDDAYS",
   "You must enter a value of 0 or greater for the number of days to retain data.  You entered %1 or did not enter any value.",
   "i",ldaystokeep)
  GO TO exit_script
 ENDIF
 SET dpurgedttm = cnvtlookbehind(build(ldaystokeep,"D"),cnvtdatetime(curdate,curtime3))
 SELECT INTO "nl:"
  FROM bb_worklist bw
  WHERE parser(sbr_getrowidnotexists("bw.create_dt_tm < cnvtdatetime(dPurgeDtTm)","bw"))
   AND ((bw.worklist_id+ 0) > 0)
  HEAD REPORT
   row_cnt = 0
  DETAIL
   row_cnt = (row_cnt+ 1)
   ...
   ENDIF
   reply->rows[row_cnt].row_id = bw.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,row_cnt)
  WITH nocounter, maxqual(bw,value(request->max_rows))
 ;end select
