CREATE PROGRAM dpo_process_event_rows:dba
 ...
 SET dpo_reply->table_name = "DM_PROCESS_EVENT"
 ...
 IF (validate(dper_lookbehind_secs,- (1)) > 1)
  SET dper_date_char = format(cnvtlookbehind(build(dper_lookbehind_secs,"S",sysdate)),"YYYY-MM-DD;;D"
   )
 ENDIF
 IF (validate(request->debug_mode,"Z") != "Z")
  CALL echo(build("dper_date_char:",dper_date_char))
 ENDIF
 SET dper_cursor_query = build("select rowid from V500.DM_PROCESS_EVENT WHERE UPDT_DT_TM <",
