CREATE PROGRAM bed_ens_mltm_drc_grouper:dba
 ...
 FREE SET active_grouper
 RECORD active_grouper(
   1 groupers[*]
     2 grouper_id = f8
 )
 FREE SET inactive_grouper
 RECORD inactive_grouper(
   1 groupers[*]
     2 grouper_id = f8
 )
 FREE SET reply
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   WHERE (request->groupers[d.seq].active_ind=1))
  ORDER BY d.seq
  HEAD REPORT
   act_cnt = 0, act_tot_cnt = 0, stat = alterlist(active_grouper->groupers,100)
  DETAIL
   act_cnt = (act_cnt+ 1), act_tot_cnt = (act_tot_cnt+ 1)
   IF (act_tot_cnt > 100)
    stat = alterlist(active_grouper->groupers,(act_cnt+ 100)), act_tot_cnt = 1
   ENDIF
   active_grouper->groupers[act_cnt].grouper_id = request->groupers[d.seq].id
  FOOT REPORT
   stat = alterlist(active_grouper->groupers,act_cnt)
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = cnt)
  PLAN (d
   WHERE (request->groupers[d.seq].active_ind=0))
  ORDER BY d.seq
  HEAD REPORT
   inact_cnt = 0, inact_tot_cnt = 0, stat = alterlist(inactive_grouper->groupers,100)
  DETAIL
   inact_cnt = (inact_cnt+ 1), inact_tot_cnt = (inact_tot_cnt+ 1)
   IF (inact_tot_cnt > 100)
    stat = alterlist(inactive_grouper->groupers,(inact_cnt+ 100)), inact_tot_cnt = 1
   ENDIF
   inactive_grouper->groupers[inact_cnt].grouper_id = request->groupers[d.seq].id
  FOOT REPORT
   stat = alterlist(inactive_grouper->groupers,inact_cnt)
  WITH nocounter
 ;end select
 SET cnt = size(active_grouper->groupers,5)
 IF (cnt > 0)
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (m
    WHERE (m.grouper_id=active_grouper->groupers[d.seq].grouper_id)
  ...
  ENDFOR
 ENDIF
 SET cnt = size(inactive_grouper->groupers,5)
 SET stat = alterlist(temp_kia->drc_obj_list,0)
 SET stat = alterlist(temp_kia->drc_obj_list,cnt)
 IF (cnt > 0)
  SET temp_kia->import_method_flag = 1
  SET temp_kia->domain_flag = 0
  SELECT INTO "nl:"
   ...
   PLAN (d)
    JOIN (m
    WHERE (m.grouper_id=inactive_grouper->groupers[d.seq].grouper_id)
     AND m.dose_range_type_id != 5
     AND m.dose_unit_cki > " ")
    JOIN (der
    WHERE der.entity_reltn_mean="DRC/ROUTE"
