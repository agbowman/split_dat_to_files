CREATE PROGRAM dm2_exec_ddlops:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(ddta_ddlops_xrec->ddlx_cnt,- (1))=- (1)))
  FREE RECORD ddta_ddlops_xrec
  RECORD ddta_ddlops_xrec(
    1 ddlx_cnt = i2
    1 ddlxrec_idx[*]
      2 ddlx_db = vc
      2 ddlx_ndx = f8
      2 ddlx_pre_condition = vc
      2 ddlx_pre_condition_met = i2
      2 ddlx_command_to_execute = vc
      2 ddlx_on_error = vc
  )
 ENDIF
 FREE RECORD ddt_ddlops
 RECORD ddt_ddlops(
   1 pre_condition_met = i2
   1 success_ind = i2
 )
 FREE RECORD ddt_ddlops_etu
 RECORD ddt_ddlops_etu(
   1 cnt = i4
   1 qual[*]
     2 obj_name = vc
 )
 DECLARE pc_table_exists(par_table_name=vc) = i4
 DECLARE pc_table_not_exists(par_table_name=vc) = i4
 DECLARE pc_view_not_exists(par_view_name=vc) = i4
 DECLARE pc_index_not_exists(par_table_name=vc,par_index_name=vc) = i4
 DECLARE pc_constraint_not_exists(par_table_name=vc,par_constraint_name=vc) = i4
 DECLARE pc_sequence_not_exists(par_sequence_name=vc) = i4
 DECLARE sc_table_exists(par_table_name=vc) = i4
 DECLARE sc_table_not_exists(par_table_name=vc) = i4
 DECLARE sc_row_count_compare(par_table_name1=vc,par_table_name2=vc) = i4
 DECLARE table_copy(par_table_name1=vc,par_table_name2=vc) = i4
 DECLARE create_view(par_view=vc,par_table=vc) = i4
 DECLARE ddt_update_tgtsch(par_update_type=i4,par_object=vc,par_new_ind=i4,par_diff_ind=i4) = i4
 DECLARE ddt_par_value1 = vc WITH public, noconstant(" ")
 DECLARE ddt_par_value2 = vc WITH public, noconstant(" ")
 DECLARE ddt_dyncmd = vc WITH public, noconstant(" ")
 SUBROUTINE pc_table_exists(par_table_name)
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->pre_condition_met = 0
   SET ddt_ddlops->success_ind = 1
   SET ddt_par_value1 = trim(cnvtupper(par_table_name))
   SELECT INTO "nl:"
    FROM dm2_user_tables u
    WHERE u.table_name=ddt_par_value1
    WITH nocounter
   ;end select
   IF (check_error("pc_table_exists..select from dm2_user_tables") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->eproc = "*> pc_table_exists...Error selecting from dm2_user_tables "
    CALL ded_progress_message(0)
    SET ddt_ddlops->success_ind = 0
    RETURN(0)
   ELSE
    IF (curqual > 0)
     SET ddt_ddlops->pre_condition_met = 1
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE pc_table_not_exists(par_table_name)
   DECLARE pt_idx = i2 WITH protect, noconstant(0)
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->pre_condition_met = 0
   SET ddt_ddlops->success_ind = 1
   SET ddt_par_value1 = trim(cnvtupper(par_table_name))
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   IF (check_error("dm2_exec_ddlops.pc_table_not_exists") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->eproc = "*> pc_table_not_exists...Error selecting from dm2_user_tables "
    CALL ded_progress_message(0)
    SET ddt_ddlops->success_ind = 0
    RETURN(0)
   ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE pc_view_not_exists(par_view_name)
   DECLARE pv_idx = i2 WITH protect, noconstant(0)
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->pre_condition_met = 0
   SET ddt_ddlops->success_ind = 1
   SET ddt_par_value1 = trim(cnvtupper(par_view_name))
   SELECT INTO "nl:"
    FROM dm2_user_views u
    WHERE u.view_name=ddt_par_value1
    WITH nocounter
   ;end select
   IF (check_error("dm2_exec_ddlops.pc_view_not_exists") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->eproc = "*> pc_view_not_exists...Error selecting from dm2_user_views "
    CALL ded_progress_message(0)
    SET ddt_ddlops->success_ind = 0
    RETURN(0)
   ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE pc_index_not_exists(par_table_name,par_index_name)
   DECLARE pine_ind_name2 = vc WITH protect, noconstant(concat(trim(substring(1,28,cnvtupper(
        par_index_name))),"$C"))
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->pre_condition_met = 0
   SET ddt_ddlops->success_ind = 1
   SELECT INTO "nl:"
    FROM dm2_user_indexes u
    WHERE u.table_name=trim(cnvtupper(par_table_name))
     AND u.index_name IN (trim(cnvtupper(par_index_name)), pine_ind_name2)
    WITH nocounter
   ;end select
   IF (check_error("dm2_exec_ddlops.pc_index_not_exists") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->eproc = "*> pc_index_not_exists...Error selecting from dm2_user_indexes "
    CALL ded_progress_message(0)
    SET ddt_ddlops->success_ind = 0
    RETURN(0)
   ELSE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE pc_constraint_not_exists(par_table_name,par_constraint_name)
   DECLARE pcne_ind_name2 = vc WITH protect, noconstant(concat(trim(substring(1,28,cnvtupper(
        par_constraint_name))),"$C"))
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->pre_condition_met = 0
   SET ddt_ddlops->success_ind = 1
   SELECT INTO "nl:"
    FROM dm2_user_constraints u
    WHERE u.table_name=trim(cnvtupper(par_table_name))
     AND u.constraint_name IN (trim(cnvtupper(par_constraint_name)), pcne_ind_name2)
    WITH nocounter
   ;end select
   IF (check_error("dm2_exec_ddlops.pc_constraint_not_exists") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->eproc = "*> pc_constraint_not_exists...Error selecting from dm2_user_constraints "
    CALL ded_progress_message(0)
    SET ddt_ddlops->success_ind = 0
    RETURN(0)
   ELSE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE pc_sequence_not_exists(par_sequence_name)
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->pre_condition_met = 0
   SET ddt_ddlops->success_ind = 1
   SET ddt_par_value1 = trim(cnvtupper(par_sequence_name))
   SELECT INTO "nl:"
    FROM dm2_user_sequences u
    WHERE u.sequence_name=ddt_par_value1
    WITH nocounter
   ;end select
   IF (check_error("dm2_exec_ddlops.pc_sequence_not_exists") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->eproc = "*> pc_sequence_not_exists...Error selecting from dm2_user_sequences "
    CALL ded_progress_message(0)
    SET ddt_ddlops->success_ind = 0
    RETURN(0)
   ELSE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sc_table_exists(par_table_name)
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->success_ind = 0
   SET ddt_par_value1 = trim(cnvtupper(par_table_name))
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   IF (check_error("sc_table_exists...Select from dm2_user_tables") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->eproc = "*> sc_table_exists...Error selecting from dm2_user_tables "
    CALL ded_progress_message(0)
    RETURN(0)
   ELSE
    IF (curqual > 0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sc_table_not_exists(par_table_name)
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->success_ind = 0
   SET ddt_par_value1 = trim(cnvtupper(par_table_name))
   SELECT INTO "nl:"
    ...
    WITH nocounter
   ;end select
   IF (check_error("sc_table_not_exists..Select from dm2_user_tables") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->eproc = "*> sc_table_exists...Error selecting from dm2_user_tables "
    CALL ded_progress_message(0)
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sc_row_count_compare(par_table_name1,par_table_name2)
   DECLARE table1_cnt = i4 WITH public, noconstant(0)
   DECLARE table2_cnt = i4 WITH public, noconstant(0)
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->success_ind = 0
   SET ddt_par_value1 = trim(cnvtupper(par_table_name1))
   SET ddt_par_value2 = trim(cnvtupper(par_table_name2))
   SET ddt_dyncmd = concat("select into ",'"',"nl:",'"')
   SET ddt_dyncmd = concat(ddt_dyncmd," rowcnt = count(*) ")
   SET ddt_dyncmd = concat(ddt_dyncmd," from ",ddt_par_value1)
   SET ddt_dyncmd = concat(ddt_dyncmd," detail ")
   SET ddt_dyncmd = concat(ddt_dyncmd," table1_cnt = rowcnt ")
   SET ddt_dyncmd = concat(ddt_dyncmd," with nocounter  go")
   SET dm_err->eproc = build("*> Executing.... ",ddt_dyncmd)
   CALL ded_progress_message(0)
   SET dm_err->eproc = "sc_row_count_compare...Select on table1"
   IF (dm2_push_cmd(ddt_dyncmd,1)=0)
    RETURN(0)
   ENDIF
   SET ddt_dyncmd = concat("select into ",'"',"nl:",'"')
   SET ddt_dyncmd = concat(ddt_dyncmd," rowcnt = count(*) ")
   SET ddt_dyncmd = concat(ddt_dyncmd," from ",ddt_par_value2)
   SET ddt_dyncmd = concat(ddt_dyncmd," detail ")
   SET ddt_dyncmd = concat(ddt_dyncmd," table2_cnt = rowcnt ")
   SET ddt_dyncmd = concat(ddt_dyncmd," with nocounter  go")
   SET dm_err->eproc = build("*> Executing.... ",ddt_dyncmd)
   CALL ded_progress_message(0)
   SET dm_err->eproc = "sc_row_count_compare...Select on table2"
   IF (dm2_push_cmd(ddt_dyncmd,1)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = build("Table 1 Count= ",table1_cnt," Table 2 Count= ",table2_cnt)
   CALL ded_progress_message(0)
   IF (table1_cnt=table2_cnt)
    SET ddt_ddlops->success_ind = 1
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE table_copy(par_table_name1,par_table_name2)
   RECORD utcrec(
     1 utc_cnt = i2
     1 utc_columns[*]
       2 utc_column_name = vc
   )
   DECLARE dyn_sel_def = vc WITH private, noconstant(" ")
   DECLARE dyn_ins_def = vc WITH private, noconstant(" ")
   DECLARE forloopnum = i4 WITH private, noconstant(0)
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->success_ind = 0
   SET utcrec->utc_cnt = 0
   SET ddt_par_value1 = trim(cnvtupper(par_table_name1))
   SET ddt_par_value2 = trim(cnvtupper(par_table_name2))
   SELECT INTO "nl:"
    FROM dm2_user_tab_columns a
    WHERE a.table_name=ddt_par_value1
     AND a.column_name IN (
    (SELECT
     b.column_name
     FROM dm2_user_tab_columns b
     WHERE b.table_name=ddt_par_value2))
    ORDER BY a.column_name
    HEAD REPORT
     stat = alterlist(utcrec->utc_columns,10)
    DETAIL
     utcrec->utc_cnt = (utcrec->utc_cnt+ 1)
     IF (mod(utcrec->utc_cnt,10)=1
      AND (utcrec->utc_cnt != 1))
      stat = alterlist(utcrec->utc_columns,(utcrec->utc_cnt+ 9))
     ENDIF
     utcrec->utc_columns[utcrec->utc_cnt].utc_column_name = a.column_name
    FOOT REPORT
     stat = alterlist(utcrec->utc_columns,utcrec->utc_cnt)
    WITH nocounter
   ;end select
   IF (check_error("table_copy - SELECT from DM2_USER_TABLES") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((utcrec->utc_cnt > 0))
    FOR (forloopnum = 1 TO utcrec->utc_cnt)
     SET dyn_sel_def = concat(dyn_sel_def,"a.",utcrec->utc_columns[forloopnum].utc_column_name)
     IF ((forloopnum != utcrec->utc_cnt))
      SET dyn_sel_def = concat(dyn_sel_def,", ")
     ENDIF
    ENDFOR
    SET dyn_ins_def = replace(dyn_sel_def,"a.","b.",0)
    SET ddt_dyncmd = concat(" insert into ",ddt_par_value2," b ")
    SET ddt_dyncmd = concat(ddt_dyncmd," ( ",dyn_ins_def," )")
    SET ddt_dyncmd = concat(ddt_dyncmd," (  ")
    SET ddt_dyncmd = concat(ddt_dyncmd,"  select ",dyn_sel_def)
    SET ddt_dyncmd = concat(ddt_dyncmd," from ",ddt_par_value1," a ")
    SET ddt_dyncmd = concat(ddt_dyncmd," ) go ")
    SET dm_err->eproc = "subroutine table copy...insert"
    IF (dm2_push_cmd(ddt_dyncmd,1)=0)
     ROLLBACK
     RETURN(0)
    ELSE
    ...
    ENDIF
   ELSE
    SET dm_err->eproc = build("*> table_copy error. NO common columns between (",ddt_par_value1,
     ") and (",ddt_par_value2,")")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE create_view(par_view_name,par_table_name)
   SET ded_subroutine_executed = 1
   SET ddt_ddlops->success_ind = 0
   SET ddt_par_value1 = trim(cnvtupper(par_view_name))
   SET ddt_par_value2 = trim(cnvtupper(par_table_name))
   SET dm_err->eproc = build("create_view subroutine : view_name(",ddt_par_value1,")  table name : (",
    trim(ddt_par_value2),")")
   CALL ded_progress_message(0)
   SET ddt_dyncmd = concat(" rdb create or replace  view ",ddt_par_value1," as ")
   SET ddt_dyncmd = concat(ddt_dyncmd," ( select * from ",ddt_par_value2," ) go")
   SET dm_err->eproc = build("*> dm2_ddlops_table->create_view executing: ",ddt_dyncmd)
   CALL ded_progress_message(0)
   IF (dm2_push_cmd(ddt_dyncmd,1)=0)
    RETURN(0)
   ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddt_update_tgtsch(par_update_type,par_object,par_new_ind,par_diff_ind)
   SET dm_err->eproc = build("Executing Subroutine dm2_ddlops_tables.update_tgtsch ")
   CALL ded_progress_message(0)
   CASE (par_update_type)
    OF 1:
     SELECT INTO "nl:"
      d.seq
      FROM (dummyt d  WITH seq = value(tgtsch->tbl_cnt))
      DETAIL
       IF ((tgtsch->tbl[d.seq].tbl_name=par_object))
        tgtsch->tbl[d.seq].new_ind = par_new_ind, tgtsch->tbl[d.seq].diff_ind = par_diff_ind
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error("ddt_update_tgtsch..select from dummyt tgtsch->tbl.  Error on select") != 0)
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dm_err->eproc = "*> ddt_update_tgtsch..select from dummyt tgtsch->tbl.  Error on select"
      CALL ded_progress_message(0)
      RETURN(0)
     ENDIF
    OF 2:
     SELECT INTO "nl:"
      ...
      DETAIL
       IF ((tgtsch->sequence[d.seq].seq_name=par_object))
        tgtsch->sequence[d.seq].new_ind = par_new_ind, tgtsch->sequence[d.seq].diff_ind = par_new_ind
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error("ddt_update_tgtsch..select from dummyt tgtsch->sequence.  Error on select") != 0
     )
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dm_err->eproc =
      "*> ddt_update_tgtsch..select from dummyt tgtsch->sequence.  Error on select"
      CALL ded_progress_message(0)
      RETURN(0)
     ENDIF
    ELSE
     SET dm_err->eproc = build("*> ddt_update_tgtsch: invalid update_type (",par_update_type,
 ...
 SUBROUTINE dm2_adb_check(dac_db_link,dac_adb_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ded_process_pre_conditions(null) = i4
 DECLARE ded_process_commands_to_execute(null) = i4
 DECLARE ded_token_check(null) = i4
 DECLARE ded_replace_tokens_with_values(null) = i4
 DECLARE ded_exit_program(exit_status=i4) = i4
 DECLARE ded_progress_message(write_to_log=i4) = i4
 DECLARE ded_eval_ora_tbl_cnts(deotc_table_name=vc,deotc_vc_tot_len=i2(ref),deotc_table_column_cnt=i2
  (ref)) = i2
 DECLARE ded_check_def(dc_object_name=vc) = i2
 DECLARE ded_eval_ccl_tbl_cnts(dectc_table_name=vc,dectc_ded_tbl_len=i2(ref),dectc_ded_tbl_cnt=i2(ref
   )) = i2
 DECLARE ded_pre_condition_met = i4 WITH public, noconstant(0)
 DECLARE ded_subroutine_executed = i4 WITH public, noconstant(0)
 DECLARE ded_file_name = vc WITH public, noconstant(" ")
 DECLARE forloopnum = i4 WITH protect, noconstant(0)
 DECLARE ded_vc_tot_len = i4 WITH protect, noconstant(0)
 DECLARE ded_table_column_cnt = i4 WITH protect, noconstant(0)
 DECLARE ded_tbl_len = i4 WITH protect, noconstant(0)
 DECLARE ded_tbl_cnt = i4 WITH protect, noconstant(0)
 DECLARE pt_idx = i2 WITH protect, noconstant(0)
 DECLARE ded_adb_ind = i2 WITH protect, noconstant(0)
 IF ((dm2_install_schema->curprog="NONE"))
  SET dm_err->logfile = "DM2_LOGFILE_NOTSET"
 ENDIF
 IF (dm2_prg_maint("BEGIN") != 1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = build("* BEGINNING DM2_EXEC_DDLOPS.PRG...")
 CALL ded_progress_message(1)
 EXECUTE dm2_create_ddlops_file
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  SET dm_err->eproc = build("**> Checking for tokens...")
  CALL ded_progress_message(0)
 ENDIF
 IF (dm2_adb_check("",ded_adb_ind)=0)
  GO TO exit_program
 ENDIF
 IF (ded_adb_ind=0)
  IF (ded_token_check(null)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = build("* Determining which database commands to execute...")
 CALL ded_progress_message(1)
 FOR (forloopnum = 1 TO ddta_ddlops_xrec->ddlx_cnt)
  SET ded_pre_condition_met = ded_process_pre_conditions(null)
  IF (ded_pre_condition_met > 0)
   SET dm_err->eproc = build("* Executing database commands...")
   CALL ded_progress_message(1)
   IF (ded_process_commands_to_execute(null)=0)
    GO TO exit_program
   ENDIF
  ELSEIF ((ded_pre_condition_met=- (1)))
   SET forloopnum = ddta_ddlops_xrec->ddlx_cnt
  ENDIF
 ENDFOR
 IF (ded_check_def("DM2_DDL_OPS1")=0)
  GO TO exit_program
 ENDIF
 IF (ded_check_def("DM2_DDL_OPS")=0)
  GO TO exit_program
 ENDIF
 IF (ded_check_def("DM2_TSPACE_SIZE1")=0)
  GO TO exit_program
 ENDIF
 IF (ded_check_def("DM2_TSPACE_SIZE")=0)
  GO TO exit_program
 ENDIF
 IF (ded_check_def("DM2_TSPACE_OBJ_SIZE1")=0)
  GO TO exit_program
 ENDIF
 IF (ded_check_def("DM2_TSPACE_OBJ_SIZE")=0)
  GO TO exit_program
 ENDIF
 IF (ded_check_def("DM2_DDL_OPS_LOG1")=0)
  GO TO exit_program
 ENDIF
 IF (ded_check_def("DM2_DDL_OPS_LOG")=0)
  GO TO exit_program
 ENDIF
 IF (locateval(pt_idx,1,ddt_ddlops_etu->cnt,"DM2_DDL_OPS_LOG1",ddt_ddlops_etu->qual[pt_idx].obj_name)
 =0)
  IF (ded_eval_ora_tbl_cnts("DM2_DDL_OPS_LOG1",ded_vc_tot_len,ded_table_column_cnt)=0)
   GO TO exit_program
  ENDIF
  IF (ded_eval_ccl_tbl_cnts("DM2_DDL_OPS_LOG1",ded_tbl_len,ded_tbl_cnt)=0)
   GO TO exit_program
  ENDIF
  IF (((ded_tbl_cnt != ded_table_column_cnt) OR (ded_vc_tot_len != ded_tbl_len)) )
   SET ddt_ddlops_etu->cnt = (ddt_ddlops_etu->cnt+ 1)
   SET stat = alterlist(ddt_ddlops_etu->qual,ddt_ddlops_etu->cnt)
   SET ddt_ddlops_etu->qual[ddt_ddlops_etu->cnt].obj_name = "DM2_DDL_OPS_LOG1"
  ENDIF
 ENDIF
 SET pt_idx = 0
 SET ded_tbl_len = 0
 SET ded_tbl_cnt = 0
 IF (locateval(pt_idx,1,ddt_ddlops_etu->cnt,"DM2_DDL_OPS_LOG",ddt_ddlops_etu->qual[pt_idx].obj_name)=
 0)
  IF (ded_eval_ccl_tbl_cnts("DM2_DDL_OPS_LOG",ded_tbl_len,ded_tbl_cnt)=0)
   GO TO exit_program
  ENDIF
  IF (((ded_tbl_cnt != ded_table_column_cnt) OR (ded_vc_tot_len != ded_tbl_len)) )
   SET ddt_ddlops_etu->cnt = (ddt_ddlops_etu->cnt+ 1)
   SET stat = alterlist(ddt_ddlops_etu->qual,ddt_ddlops_etu->cnt)
   SET ddt_ddlops_etu->qual[ddt_ddlops_etu->cnt].obj_name = "DM2_DDL_OPS_LOG"
  ENDIF
 ENDIF
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(ddta_ddlops_xrec)
 ENDIF
 SET dm_err->eproc = build("* Executing table utilities...")
 CALL ded_progress_message(1)
 FOR (forloopnum = 1 TO ddt_ddlops_etu->cnt)
   IF (cnvtupper(currdbuser)="V500")
    EXECUTE dm2_create_views ddt_ddlops_etu->qual[forloopnum].obj_name
    IF ((dm_err->err_ind=1))
     GO TO exit_program
    ENDIF
   ENDIF
   EXECUTE oragen3 ddt_ddlops_etu->qual[forloopnum].obj_name
   IF ((dm_err->err_ind=1))
    GO TO exit_program
   ENDIF
 ENDFOR
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE ded_process_pre_conditions(null)
  IF (textlen(trim(ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_pre_condition)) > 0)
   IF (forloopnum > 1
    AND (ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_pre_condition=ddta_ddlops_xrec->ddlxrec_idx[(
   forloopnum - 1)].ddlx_pre_condition))
    SET ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_pre_condition_met = ddta_ddlops_xrec->
    ddlxrec_idx[(forloopnum - 1)].ddlx_pre_condition_met
    SET dm_err->eproc = " Pre_condition = previous, setting pre_condition_met = previous value"
    CALL ded_progress_message(0)
   ELSE
    IF ((dm_err->debug_flag > 0))
     SET dm_err->eproc = build(" Executing precondition: ",ddta_ddlops_xrec->ddlxrec_idx[forloopnum].
      ddlx_pre_condition)
     CALL ded_progress_message(0)
    ENDIF
    CALL parser(ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_pre_condition,1)
    IF ((dm_err->err_ind=1))
     RETURN(- (1))
    ELSE
     SET ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_pre_condition_met = ddt_ddlops->
     pre_condition_met
    ENDIF
   ENDIF
  ELSE
   SET ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_pre_condition_met = 1
  ENDIF
  RETURN(ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_pre_condition_met)
 END ;Subroutine
 SUBROUTINE ded_process_commands_to_execute(null)
  IF ((ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_pre_condition_met=1)
   AND textlen(trim(ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_command_to_execute)) != 0)
   IF (ded_replace_tokens_with_values(null)=1)
    SET ded_subroutine_executed = 0
    IF ((dm_err->debug_flag > 0))
     SET dm_err->eproc = build("**> NDX=",ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_ndx,
      "  Executing Command...",ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_command_to_execute)
     CALL ded_progress_message(0)
    ENDIF
    CALL parser(ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_command_to_execute,1)
    IF ((dm_err->err_ind=1))
     SET ded_subroutine_executed = 1
     SET ddt_ddlops->success_ind = 0
    ENDIF
    IF (ded_subroutine_executed=0)
     IF (check_error(build("Command_to_execute: ",ddta_ddlops_xrec->ddlxrec_idx[forloopnum].
       ddlx_command_to_execute))=0)
      SET ddt_ddlops->success_ind = 1
      COMMIT
     ELSE
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     ENDIF
    ENDIF
    SET dm_err->eproc = build("**> Success_ind = ",ddt_ddlops->success_ind)
    CALL ded_progress_message(0)
    IF ((ddt_ddlops->success_ind != 1))
     SET dm_err->eproc = build("** DDLOPS.COMMAND_TO_EXECUTE ERROR ON DDLOPS.NDX = ",ddta_ddlops_xrec
      ->ddlxrec_idx[forloopnum].ddlx_ndx,"...DDLOPS.COMMAND_TO_EXECUTE = ",ddta_ddlops_xrec->
      ddlxrec_idx[forloopnum].ddlx_command_to_execute)
     CALL ded_progress_message(0)
     IF (textlen(ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_on_error) > 0)
      IF ((dm_err->debug_flag > 0))
       CALL echo(build("Executing on_error command : ",ddta_ddlops_xrec->ddlxrec_idx[forloopnum].
         ddlx_on_error))
      ENDIF
      SET dm_err->err_ind = 0
      CALL parser(ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_on_error,1)
      IF (check_error("dm2_exec_ddlops.on_error")=0)
       COMMIT
      ELSE
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dm_err->eproc = build("**> Error executing> ",ddta_ddlops_xrec->ddlxrec_idx[forloopnum].
        ddlx_on_error)
       CALL ded_progress_message(0)
       ROLLBACK
      ENDIF
   ...
   ENDIF
  ELSE
   IF (textlen(trim(ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_command_to_execute))=0)
    SET dm_err->eproc = build(
     "Command_to_execute error: pre_condition met, but command_to_execute empty for ndx= ",
     ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_ndx," on dm2_ddlops table")
    CALL disp_msg(dm_err->eproc,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    RETURN(0)
   ENDIF
   SET dm_err->eproc = build("PRE_CONDITION NOT MET FOR NDX: ",ddta_ddlops_xrec->ddlxrec_idx[
    forloopnum].ddlx_ndx)
   CALL ded_progress_message(0)
  ENDIF
  RETURN(1)
 END ;Subroutine
 SUBROUTINE ded_token_check(null)
   SET dm_err->eproc = "Determining tablespaces for dm2_ddl_ops tables."
   CALL ded_progress_message(0)
   IF ((dir_tools_tspaces->data_tspace="NONE"))
    SET dm_err->eproc = "ded_token_check - Tablespace not found for data tablespace."
    CALL disp_msg("",dm_err->logfile,0)
    IF (dis_ded_exec_nbr=1)
     SET dis_ded_rerun_ind = 1
    ELSE
     SET dm_err->err_ind = 1
    ENDIF
    RETURN(0)
   ENDIF
   IF ((dir_tools_tspaces->index_tspace="NONE"))
    SET dm_err->eproc = "ded_token_check - Tablespace not found for index tablespace."
    CALL disp_msg("",dm_err->logfile,0)
    IF (dis_ded_exec_nbr=1)
     SET dis_ded_rerun_ind = 1
    ELSE
   ...
   ENDIF
   IF ((dir_tools_tspaces->lob_tspace="NONE"))
    SET dm_err->eproc = "ded_token_check - Tablespace not found for lob tablespace."
    CALL disp_msg("",dm_err->logfile,0)
    IF (dis_ded_exec_nbr=1)
     SET dis_ded_rerun_ind = 1
    ELSE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ded_replace_tokens_with_values(null)
   SET ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_command_to_execute = replace(ddta_ddlops_xrec->
    ddlxrec_idx[forloopnum].ddlx_command_to_execute,"%TABLETSPACE%",dir_tools_tspaces->data_tspace,
    0)
   SET ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_command_to_execute = replace(ddta_ddlops_xrec->
    ddlxrec_idx[forloopnum].ddlx_command_to_execute,"%INDEXTSPACE%",dir_tools_tspaces->index_tspace,
    0)
   SET ddta_ddlops_xrec->ddlxrec_idx[forloopnum].ddlx_command_to_execute = replace(ddta_ddlops_xrec->
    ddlxrec_idx[forloopnum].ddlx_command_to_execute,"%LONGTSPACE%",dir_tools_tspaces->lob_tspace,0)
 END ;Subroutine
 SUBROUTINE ded_check_def(dc_object_name)
  IF (checkdic(dc_object_name,"T",0) != 2)
   SET ddt_ddlops_etu->cnt = (ddt_ddlops_etu->cnt+ 1)
   SET stat = alterlist(ddt_ddlops_etu->qual,ddt_ddlops_etu->cnt)
   SET ddt_ddlops_etu->qual[ddt_ddlops_etu->cnt].obj_name = dc_object_name
   IF (check_error(dm_err->eproc)=1)
    IF (findstring("ORA-00942",dm_err->emsg,1,1) > 0)
     SET dm_err->err_ind = 0
  ...
  RETURN(1)
 END ;Subroutine
 SUBROUTINE ded_eval_ora_tbl_cnts(deotc_table_name,deotc_vc_tot_len,deotc_table_column_cnt)
   SET dm_err->eproc =
   "Obtain the data_length sum of all the VARCHAR2 columns for tables and total number of columns"
   CALL ded_progress_message(1)
   SET deotc_vc_tot_len = 0
   SET deotc_table_column_cnt = 0
   SELECT INTO "nl:"
    FROM user_tab_columns u
    WHERE u.table_name=deotc_table_name
     AND  NOT (u.column_name IN ("LAST_UTC_TS", "TXN_ID_TEXT", "INST_ID"))
    DETAIL
     deotc_table_column_cnt = (deotc_table_column_cnt+ 1)
     IF (u.data_type="VARCHAR2")
      deotc_vc_tot_len = (deotc_vc_tot_len+ u.data_length)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ded_eval_ccl_tbl_cnts(dectc_table_name,dectc_ded_tbl_len,dectc_ded_tbl_cnt)
   SET dm_err->eproc =
   "Obtain the length sum of all the character columns for both the table and view’s CCL table definitions "
   CALL ded_progress_message(1)
   SET dectc_ded_tbl_len = 0
   SET dectc_ded_tbl_cnt = 0
   SELECT INTO "nl:"
    t.table_name, tl.*
    FROM dtable t,
     dtableattr ta,
     dtableattrl tl
    WHERE t.table_name=dectc_table_name
     AND t.table_name=ta.table_name
     AND tl.structtype="F"
     AND btest(tl.stat,11)=0
     AND  NOT (tl.attr_name IN ("LAST_UTC_TS", "TXN_ID_TEXT", "INST_ID"))
    DETAIL
     IF (tl.type="C"
      AND tl.attr_name != "OPERATION")
      dectc_ded_tbl_len = (dectc_ded_tbl_len+ tl.len)
     ENDIF
     dectc_ded_tbl_cnt = (dectc_ded_tbl_cnt+ 1)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ded_progress_message(write_to_log)
   IF (write_to_log=1)
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "* EXITING DM2_EXEC_DDLOPS.PRG (ALL COMMANDS SUCCESSFUL)."
  CALL ded_progress_message(1)
 ENDIF
 IF (dm2_prg_maint("END") != 1)
  SET dm_err->eproc = "Error calling dm2_prg_maint"
 ENDIF
END GO
