CREATE PROGRAM bhs_ma_error_signing_rpt2:dba
 PROMPT
 ...
 IF (( $2 > " "))
  SET from_date =  $2
 ELSE
  SET from_date = format((curdate - 1),"MMDDYY;;d")
 ENDIF
 IF (( $3 > " "))
  SET to_date =  $3
 ELSE
  SET to_date = format(curdate,"MMDDYY;;d")
 ENDIF
 RECORD dlrec(
   1 ord_total = i4
   1 seq[*]
 ...
 SELECT INTO "nl:"
  ...
  PLAN (orv
   ...
   WHERE orv.order_id=orn.order_id
    ...
    AND orn.updt_dt_tm BETWEEN cnvtdatetime(cnvtdate2(from_date,"MMDDYY"),0) AND cnvtdatetime(
    cnvtdate2(to_date,"MMDDYY"),235959))
   JOIN (pl
  ...
  DETAIL
   ...
   IF (a=1)
    ...
    dlrec->seq[c].to_prsnl_name = to_prsnl_name, col 5, pt_name,
    row + 1, col 7, "ORDER ENTERED BY: ",
    order_person, row + 1, col 7,
    "ORDER: ", o.order_id"##########", " ",
    o.order_mnemonic, row + 1, col 10,
    o.clinical_display_line, row + 2
   ENDIF
  FOOT  orv.order_id
   col + 0
  FOOT  p.person_id
