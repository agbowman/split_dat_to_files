CREATE PROGRAM dm2_aload_sch_cons
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_sch_cons",".log","dm2_aload_sch_cons")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_sch_cons"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dasc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dasc_str = vc WITH protect, noconstant("")
 DECLARE dasc_req_size = i4 WITH protect, noconstant(0)
 FREE RECORD dasc_maint
 RECORD dasc_maint(
   1 qual[*]
     2 maint_type = c1
   1 num_of_inserts = i4
 ...
 SET dasc_maint->num_of_inserts = 0
 SET dasc_maint->num_of_updates = 0
 SET dasc_maint->num_of_none = 0
 SET dasc_req_size = size(requestin->list_0,5)
 SET stat = alterlist(dasc_maint->qual,dasc_req_size)
 IF (dac_pop_coldic_rec("DM_AFD_CONSTRAINTS")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  SET dm_err->eproc = "Determining maintenance type for DM_AFD_CONSTRAINTS."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dasc_req_size))
  DETAIL
   dasc_maint->qual[d.seq].maint_type = "I", dasc_maint->num_of_inserts = (dasc_maint->num_of_inserts
   + 1)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 SELECT INTO "nl:"
  FROM dm_afd_tables dat,
   dm_afd_constraints dac,
   (dummyt d  WITH seq = value(dasc_req_size))
  PLAN (d
   WHERE d.seq > 0)
   JOIN (dat
   ...
   JOIN (dac
   WHERE dac.owner=dat.owner
    AND dac.alpha_feature_nbr=cnvtint(requestin->list_0[d.seq].alpha_feature_nbr)
    AND (dac.table_name=requestin->list_0[d.seq].table_name)
    AND (dac.constraint_name=requestin->list_0[d.seq].constraint_name)
  ...
  DETAIL
   IF (cnvtint(requestin->list_0[d.seq].schema_instance) < dat.schema_instance)
    dasc_maint->qual[d.seq].maint_type = "N", dasc_maint->num_of_none = (dasc_maint->num_of_none+ 1),
    dasc_maint->num_of_inserts = (dasc_maint->num_of_inserts - 1)
   ...
   ELSEIF (cnvtint(requestin->list_0[d.seq].schema_instance)=dat.schema_instance
    AND cnvtdatetime(requestin->list_0[d.seq].updt_dt_tm) != cnvtdatetime(dac.updt_dt_tm))
    dasc_maint->qual[d.seq].maint_type = "U", dasc_maint->num_of_updates = (dasc_maint->
    num_of_updates+ 1), dasc_maint->num_of_inserts = (dasc_maint->num_of_inserts - 1)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dasc_maint)
  CALL echorecord(requestin)
 ENDIF
 IF ((dasc_maint->num_of_inserts > 0))
  ...
  CALL dm2_push_cmd(concat("insert into ",dac_col_list->tbl,
    " dcv ,(dummyt d with seq = value(dasc_req_size) )"),0)
  ...
  COMMIT
 ENDIF
 IF ((dasc_maint->num_of_updates > 0))
  ...
  IF (dac_build_col_list(null)=0)
   ...
   GO TO exit_program
  ENDIF
  CALL dm2_push_cmd("plan d where dasc_maint->qual[d.seq].maint_type = 'U' ",0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_sch_cons completed"
 ENDIF
 CALL final_disp_msg("dm2_aload_sch_cons")
END GO
