CREATE PROGRAM dm_chg_log_summary:dba
 ...
 DECLARE tgt_select = f8
 DECLARE tabl_name = vc
 DECLARE tbl_cnt = i4
 IF (check_logfile("dm_chg_log_smry",".log","DM_CHG_LOG_SMRY LOGFILE")=0)
  SET dm_err->err_ind = 1
...
#redo
 ...
 CALL text(7,10,"Please select target environment: ")
 ...
 SELECT INTO "NL:"
  ...
  (SELECT DISTINCT
   target_env_id
   FROM dm_chg_log_smry))
  WITH nocounter
 ;end select
 CALL accept(7,44,"9(11)",0)
 SET tgt_select = curaccept
 SET valid_input_ind = 0
 SELECT INTO "NL:"
  FROM dm_chg_log_smry d
  WHERE d.target_env_id=tgt_select
  DETAIL
   valid_input_ind = 1
  WITH nocounter, maxqual(d,2)
 ;end select
 IF (valid_input_ind=0)
  CALL text(21,2,"Invalid target selected.")
  CALL pause(5)
  SET tgt_select = 0
  GO TO redo
 ENDIF
 ...
 CALL echo("********************************************************************************")
 CALL echo("*")
 CALL echo("Gathering Information . . .")
 CALL echo("*")
 CALL echo("********************************************************************************")
 CALL echo("*")
 CALL echo("*")
 ...
 SET tbl_cnt = 0
 SELECT INTO "NL:"
  d.table_name, d.log_type, d.row_count
  FROM dm_chg_log_smry d
  WHERE d.target_env_id=tgt_select
  ORDER BY d.table_name, d.log_type
  HEAD d.table_name
   tbl_cnt = (tbl_cnt+ 1), stat = alterlist(smry->qual,tbl_cnt), smry->qual[tbl_cnt].table_name = d
   .table_name
  DETAIL
   IF (d.log_type="REFCHG")
    smry->qual[tbl_cnt].row_cnt = d.row_count
   ELSEIF (d.log_type="MERGED")
    smry->qual[tbl_cnt].mrg_cnt = d.row_count
   ELSEIF (d.log_type="ORPHAN")
    smry->qual[tbl_cnt].orf_cnt = d.row_count
   ELSEIF (d.log_type="NORDDS")
    smry->qual[tbl_cnt].nrd_cnt = d.row_count
   ELSEIF (d.log_type="NO SRC")
    smry->qual[tbl_cnt].nsrc_cnt = d.row_count
   ELSEIF (d.log_type="OLDVER")
    smry->qual[tbl_cnt].ver_cnt = d.row_count
   ELSE
    smry->qual[tbl_cnt].err_cnt = d.row_count
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO mine
  FROM (dummyt d  WITH seq = value(size(smry->qual,5)))
  HEAD REPORT
   col 0, "TABLE NAME", col 37,
   "ROW COUNT", col 52, "MERGED",
   col 64, "ORPHAN", col 76,
   "NORDDS", col 88, "NO SRC",
   col 101, "OLDVER", col 114,
   "ERROR"
  DETAIL
   row + 1, col 0, smry->qual[d.seq].table_name,
   col 35, smry->qual[d.seq].row_cnt, col 47,
   smry->qual[d.seq].mrg_cnt, col 59, smry->qual[d.seq].orf_cnt,
   col 71, smry->qual[d.seq].nrd_cnt, col 83,
   smry->qual[d.seq].nsrc_cnt, col 96, smry->qual[d.seq].ver_cnt,
   col 108, smry->qual[d.seq].err_cnt
