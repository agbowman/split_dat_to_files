CREATE PROGRAM dm_dm2_tspace_obj_size_rows:dba
 ...
 DECLARE v_days_to_keep_min = i4 WITH noconstant(90)
 DECLARE v_tablename = vc WITH noconstant("DM2_TSPACE_OBJ_SIZE1")
 ...
 IF (check_logfile("DM_DM2_TSPACE_OBJ",".log","DM_DM2_TSPACE_OBJ_SIZE_ROWS log file")=0)
  SET reply->status_data.status = "F"
  ...
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Begin DM_DM2_TSPACE_OBJ_SIZE_ROWS"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = concat("Check if table ",v_tablename," exists on USER_TABLES.")
 SELECT INTO "nl:"
  u.table_name
  FROM user_tables u
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  SET reply->status_data.status = "F"
  SET reply->err_code = dm_err->ecode
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"ORATABLEERR",
   "Failed to check for existence of table in Oracle: %1","s",nullterm(dm_err->emsg))
  GO TO end_program
 ENDIF
 IF (v_table_exists_flag=v_user_tables)
  SET v_table_exists_flag = false
  SET v_table_exists_flag = checkdic(v_tablename,"T",0)
 ENDIF
 IF (v_table_exists_flag=false)
  SET reply->err_code = 0
  SET v_errmsg2 = uar_i18nbuildmessage(i18nhandle,"INF_TABLEEXISTANCE",
 ...
 FOR (v_ndx = 1 TO v_cnt)
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < v_days_to_keep_min)
  SET dm_err->err_ind = 1
  SET dm_err->ecode = - (1)
  SET dm_err->emsg = uar_i18nbuildmessage(i18nhandle,"ERR_DAYSTOKEEPMIN",
 ...
 SET dm_err->eproc = concat("Find rows older than ",trim(cnvtstring(v_days_to_keep),3),
  " days to purge")
 CALL disp_msg("",dm_err->logfile,0)
 IF (v_archi_version >= 2.0)
  SET v_parser_stmt = sbr_getrowidnotexists(
   "d.updt_dt_time < cnvtdatetime(curdate - v_days_to_keep, curtime3)","d")
 ELSE
  SET v_parser_stmt = "d.updt_dt_time < cnvtdatetime(curdate - v_days_to_keep, curtime3)"
 ENDIF
 SELECT INTO "nl:"
  d.rowid
  FROM dm2_tspace_obj_size1 d
  ...
  DETAIL
   ...
   ENDIF
   reply->rows[v_rows].row_id = d.rowid
  FOOT REPORT
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERR",
   "Failed in collection of purgeable data: %1","s",nullterm(dm_err->emsg))
  GO TO end_program
 ENDIF
 SET reply->status_data.status = "S"
 SET reply->err_code = 0
 ...
 ENDIF
 SET dm_err->eproc = "Ending DM_DM2_TSPACE_OBJ_SIZE_ROWS"
 CALL final_disp_msg("DM_DM2_TSPACE_OBJ")
END GO
