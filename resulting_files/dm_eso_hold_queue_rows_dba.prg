CREATE PROGRAM dm_eso_hold_queue_rows:dba
 CALL echo("<===== DM_ESO_HOLD_QUEUE_ROWS  BEGIN =====>")
 ...
 SET reply->table_name = "HOLD_QUEUE"
 SET reply->rows_between_commit = 100
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(h.hold_queue_id)
   FROM hold_queue h
   WHERE h.hold_queue_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    h.rowid
    FROM hold_queue h
    WHERE parser(sbr_getrowidnotexists("h.hold_queue_id between curMinID and curMaxID","h"))
     AND h.queue_id > 0
     AND  NOT ( EXISTS (
    (SELECT
     e.queue_id
     FROM cqm_fsieso_que e
     WHERE e.queue_id=h.queue_id)))
    DETAIL
     rows += 1
     IF (mod(rows,1000)=1)
      stat = alterlist(reply->rows,(rows+ 999))
     ENDIF
     reply->rows[rows].row_id = h.rowid
    WITH nocounter, maxqual(h,value(rowsleft))
   ;end select
   SET errcd = error(errmsg,1)
   IF (errcd > 0)
    SET reply->err_code = errcd
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERROR",
     "Failed in row collection: %1","s",nullterm(errmsg))
    SET reply->status_data.status = "F"
    GO TO exit_script
   ENDIF
...
#exit_script
 CALL echo("<===== DM_ESO_HOLD_QUEUE_ROWS END =====>")
END GO
