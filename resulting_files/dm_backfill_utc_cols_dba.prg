CREATE PROGRAM dm_backfill_utc_cols:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 CALL check_logfile("dm_backfill_utc_cols",".log","DM_BACKFILL_UTC_COLS LOGFILE")
 SET dm_err->eproc = "Beginning dm_backfill_utc_cols"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (currdb="DB2UDB")
  SET dm_err->eproc = "Current database is DB2. No update of _TZ columns needed"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 DECLARE dbuc_tblname = vc WITH protect
 DECLARE dbuc_colname = vc WITH protect
 DECLARE dm_time_zone = i4 WITH protect, noconstant(0)
 DECLARE check_result = i2 WITH protect, noconstant(0)
 DECLARE dbuc_prmname = vc WITH protect
 DECLARE dbuc_exe(s_tblnm=vc,s_colnm=vc,s_prmnm=vc) = null
 DECLARE dbuc_check(c_tblnm=vc,c_colnm=vc,c_prmnm=vc) = null
 DECLARE check_session(null) = null
 DECLARE push_tx(dbuc_tx=vc) = null
 DECLARE clean_dminfo(null) = null
 SET dm_time_zone = curtimezonesys
 IF (reflect(parameter(1,0)) > " ")
  SET dbuc_tblname = parameter(1,0)
  SET dbuc_tblname = cnvtupper(dbuc_tblname)
  IF (reflect(parameter(2,0)) > " ")
   SET dbuc_colname = parameter(2,0)
   SET dbuc_colname = cnvtupper(dbuc_colname)
   IF (reflect(parameter(3,0)) > " ")
    SET dbuc_prmname = parameter(3,0)
    SET dbuc_prmname = cnvtupper(dbuc_prmname)
    CALL dbuc_check(dbuc_tblname,dbuc_colname,dbuc_prmname)
    IF (check_result=0)
     CALL check_session(null)
     CALL dbuc_exe(dbuc_tblname,dbuc_colname,dbuc_prmname)
    ENDIF
   ELSE
    SET dm_err->eproc = "ERROR: Wrong parameter number. Need one more parameter"
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
  ELSE
   SET dm_err->eproc = "ERROR: Wrong parameter number. Need two more parameters"
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
 ELSE
  SET dm_err->eproc = "There is no parameter passed in. Check for request structure"
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (validate(requestin->list_0[1].table_name," ")=" ")
   SET dm_err->eproc = "FAILED: The request structure doesn't exist."
   CALL disp_msg(" ",dm_err->logfile,0)
  ELSE
   CALL check_session(null)
   FOR (dm_executions = 1 TO size(requestin->list_0,5))
    CALL dbuc_check(cnvtupper(requestin->list_0[dm_executions].table_name),cnvtupper(requestin->
      list_0[dm_executions].column_name),cnvtupper(requestin->list_0[dm_executions].primary_col))
    IF (check_result=0)
     CALL dbuc_exe(cnvtupper(requestin->list_0[dm_executions].table_name),cnvtupper(requestin->
       list_0[dm_executions].column_name),cnvtupper(requestin->list_0[dm_executions].primary_col))
    ENDIF
   ENDFOR
  ENDIF
 ENDIF
 SUBROUTINE dbuc_check(c_tblnm,c_colnm,c_prmnm)
   DECLARE coln_flag = i2 WITH protect, noconstant(0)
   DECLARE prm_flag = i2 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM dm2_user_tab_columns utc
    WHERE utc.table_name=c_tblnm
    HEAD REPORT
     coln_flag = 0
    DETAIL
     IF (utc.column_name=c_colnm)
      coln_flag = 1
     ENDIF
     IF (utc.column_name=c_prmnm)
      prm_flag = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (curqual=0)
    SET dm_err->eproc = concat("WARNING: Oracle definition for table ",c_tblnm," doesn't exist")
    CALL disp_msg(" ",dm_err->logfile,0)
    SET check_result = 1
    RETURN
   ELSEIF (coln_flag=0)
    SET dm_err->eproc = concat("WARNING: column ",c_colnm," doesn't exist in table ",c_tblnm)
    CALL disp_msg(" ",dm_err->logfile,0)
    SET check_result = 1
    RETURN
   ELSEIF (prm_flag=0)
    SET dm_err->eproc = concat("WARNING: column ",c_prmnm," doesn't exist in table ",c_tblnm)
    CALL disp_msg(" ",dm_err->logfile,0)
    SET check_result = 1
    RETURN
   ENDIF
   SELECT INTO "nl:"
    ...
    FROM dtableattr d,
     dtableattrl l
    WHERE d.table_name=c_tblnm
    HEAD REPORT
     coln_flag = 0, prm_flag = 0
    DETAIL
     IF (l.attr_name=c_colnm)
      coln_flag = 1
     ENDIF
     IF (l.attr_name=c_prmnm)
      prm_flag = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (curqual=0)
    SET dm_err->eproc = concat("WARNING: CCL definition for table ",c_tblnm," doesn't exist")
    CALL disp_msg(" ",dm_err->logfile,0)
    SET check_result = 1
    RETURN
   ELSEIF (coln_flag=0)
   ...
   SET check_result = 0
 END ;Subroutine
 SUBROUTINE dbuc_exe(s_tblnm,s_colnm,s_prmnm)
   DECLARE dbuc_cmd_cnt = i4 WITH protect, constant(5)
   DECLARE pkid_cnt = i4 WITH protect, noconstant(0)
   DECLARE tbl_flag = i2 WITH protect, noconstant(0)
   DECLARE ph_txt_cnt = i4 WITH protect, noconstant(0)
   FREE RECORD dm_pk_ids
   RECORD dm_pk_ids(
     1 qual[*]
       2 pkid = f8
   )
   FREE RECORD dbuc_cmd
   RECORD dbuc_cmd(
     1 list[*]
       2 cmd_tx = vc
   )
   SET stat = alterlist(dbuc_cmd->list,dbuc_cmd_cnt)
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="DM_BACKFILL_UTC_COLS"
     AND di.info_name=s_tblnm
    WITH nocounter
   ;end select
   IF (curqual > 0)
    SET dm_err->eproc = concat("Table ",s_tblnm," is busy. ")
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN
   ELSE
    INSERT  FROM dm_info di
     SET di.info_domain = "DM_BACKFILL_UTC_COLS", di.info_name = s_tblnm, di.info_char = s_colnm
     WITH nocounter
    ;end insert
    COMMIT
   ENDIF
   SET dm_err->eproc = concat("Working on table ",s_tblnm," Column ",s_colnm)
   CALL disp_msg(" ",dm_err->logfile,0)
   FREE RECORD dbuc_ph
   RECORD dbuc_ph(
     1 list[*]
   ...
   CALL push_tx(concat("select into 'nl:' from ",s_tblnm," tt"))
   CALL push_tx(concat("where tt.",s_colnm," is NULL or tt.",s_colnm," = 0"))
   CALL push_tx(concat("order by tt.",s_prmnm))
   CALL push_tx("head report ")
   CALL push_tx("dm_counter = 1 pkid_cnt = 1")
   CALL push_tx("stat = alterlist(dm_pk_ids->qual,pkid_cnt)")
   CALL push_tx(build("dm_pk_ids->qual[pkid_cnt]->pkid = tt.",s_prmnm))
   CALL push_tx("detail if(dm_counter= 5000) pkid_cnt = pkid_cnt + 1")
   CALL push_tx("stat = alterlist(dm_pk_ids->qual,pkid_cnt)")
   CALL push_tx(build("dm_pk_ids->qual[pkid_cnt]->pkid = tt.",s_prmnm))
   CALL push_tx("dm_counter = 1 endif ")
   CALL push_tx("dm_counter = dm_counter+ 1")
   CALL push_tx("foot report pkid_cnt = pkid_cnt + 1")
   CALL push_tx("stat = alterlist(dm_pk_ids->qual,pkid_cnt)")
   CALL push_tx(build("dm_pk_ids->qual[pkid_cnt]->pkid = tt.",s_prmnm))
   CALL push_tx("with nocounter go")
   SET stat = alterlist(dbuc_ph->list,ph_txt_cnt)
   FOR (tcnt = 1 TO ph_txt_cnt)
     IF (tcnt < ph_txt_cnt)
      CALL dm2_push_cmd(dbuc_ph->list[tcnt].text,0)
     ELSEIF (tcnt=ph_txt_cnt)
      CALL dm2_push_cmd(dbuc_ph->list[tcnt].text,1)
     ENDIF
   ENDFOR
   IF (check_error(concat("ERROR on table ",s_tblnm)) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    CALL clean_dminfo(null)
    GO TO exit_program
   ENDIF
   FOR (updt_lp = 1 TO (pkid_cnt - 1))
     SET dbuc_cmd->list[1].cmd_tx = concat("update into ",value(s_tblnm)," dd")
     SET dbuc_cmd->list[2].cmd_tx = build("set dd.",s_colnm,"=",dm_time_zone)
     SET dbuc_cmd->list[3].cmd_tx = concat("where dd.",s_prmnm," between cnvtreal(",trim(cnvtstring(
        dm_pk_ids->qual[updt_lp].pkid)),") and cnvtreal(",
      trim(cnvtstring(dm_pk_ids->qual[(updt_lp+ 1)].pkid)),")")
     SET dbuc_cmd->list[4].cmd_tx = concat("and (dd.",s_colnm," is null or dd.",s_colnm," = 0)")
     SET dbuc_cmd->list[5].cmd_tx = "with nocounter go"
     FOR (prc = 1 TO dbuc_cmd_cnt)
       IF (prc < dbuc_cmd_cnt)
        CALL dm2_push_cmd(dbuc_cmd->list[prc].cmd_tx,0)
       ELSEIF (prc=dbuc_cmd_cnt)
        CALL dm2_push_cmd(dbuc_cmd->list[prc].cmd_tx,1)
       ENDIF
     ENDFOR
     IF (check_error(concat("ERROR on table ",s_tblnm)) != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET updt_lp = pkid_cnt
      CALL clean_dminfo(null)
      GO TO exit_program
     ENDIF
     COMMIT
   ENDFOR
   SELECT INTO "nl:"
    FROM (value(s_tblnm) tt)
    WHERE parser(concat("tt.",s_colnm," is NULL or tt.",s_colnm," = 0"))
    WITH nocounter
   ;end select
   IF (curqual > 0)
    SET dm_err->eproc = concat("Failed: ",s_colnm," on table ",s_tblnm," still has NULL or 0 values")
    CALL disp_msg(" ",dm_err->logfile,0)
   ELSE
    SET dm_err->eproc = concat("Success: finish checking ",s_colnm," on table ",s_tblnm)
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   CALL clean_dminfo(null)
 END ;Subroutine
 SUBROUTINE check_session(null)
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="DM_BACKFILL_UTC_COLS"
   WITH nocounter
  ...
  IF (curqual > 0)
   ...
   CALL clear(1,1)
   CALL box(1,2,5,78)
   CALL text(3,30,"CHECK SESSION")
   CALL text(7,3,"Is there another session running this program?(Y/N)")
   CALL accept(7,65,"P;CU","Y"
  ...
  ENDIF
 END ;Subroutine
 SUBROUTINE push_tx(dbuc_tx)
   SET ph_txt_cnt = (ph_txt_cnt+ 1)
   IF (mod(ph_txt_cnt,50)=1)
    SET stat = alterlist(dbuc_ph->list,(ph_txt_cnt+ 49))
   ENDIF
   SET dbuc_ph->list[ph_txt_cnt].text = dbuc_tx
 END ;Subroutine
 SUBROUTINE clean_dminfo(null)
  DELETE  FROM dm_info di
   WHERE di.info_domain="DM_BACKFILL_UTC_COLS"
    AND di.info_name=s_tblnm
    AND di.info_char=s_colnm
   WITH nocounter
  ;end delete
  COMMIT
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm_backfill_utc_cols"
 CALL final_disp_msg("dm_backfill_utc_cols")
END GO
