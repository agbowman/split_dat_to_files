CREATE PROGRAM afc_get_tier_groups:dba
 DECLARE versionnbr = vc
 SET versionnbr = "001"
 CALL echo(build("AFC_GET_TIER_GROUPS Version: ",versionnbr))
 RECORD reply(
   1 tier_group[*]
     2 tier_group_cd = f8
 ...
 DECLARE cntcount = i4 WITH protect, noconstant(0)
 ...
 IF ( NOT (logicaldomainsinuse))
  SET codesetrequest->objarray[1].codeset = 13035
  ...
  IF ((codesetreply->status_data.status="S"))
   SELECT INTO "nl:"
    ...
    PLAN (d)
     JOIN (c
     WHERE (c.code_value=codesetreply->objarray[d.seq].code))
    ORDER BY codesetreply->objarray[d.seq].display
    DETAIL
     count1 += 1
     IF (mod(count1,10)=1)
      stat = alterlist(reply->tier_group,(count1+ 9))
     ENDIF
     reply->tier_group[count1].tier_group_cd = codesetreply->objarray[d.seq].code, reply->tier_group[
     count1].active_ind = c.active_ind, reply->tier_group[count1].updt_cnt = c.updt_cnt
    WITH nocounter
   ;end select
   SET stat = alterlist(reply->tier_group,count1)
  ELSE
   SET reply->status_data.status = codesetreply->status_data.status
   GO TO end_program
  ENDIF
 ELSE
  SET cvextrequest->code_set = 13035
  SET cvextrequest->cv_mode = 1
  EXECUTE dm_get_cv_extensions  WITH replace("REQUEST",cvextrequest), replace("REPLY",cvextreply)
  IF ((cvextreply->status_data.status="S"))
   FOR (count1 = 1 TO size(cvextreply->qual,5))
     FOR (count2 = 1 TO size(cvextreply->qual[count1].ext,5))
       IF ((((cvextreply->qual[count1].ext[count2].field_name="Logical Domain")) OR ((cvextreply->
       qual[count1].ext[count2].field_name=""))) )
        IF (cnvtreal(cvextreply->qual[count1].ext[count2].field_value)=curlogicaldomainid)
         SET repcount += 1
         SET stat = alterlist(temprep->tier_group,repcount)
         SET temprep->tier_group[repcount].tier_group_cd = cvextreply->qual[count1].code_value
        ENDIF
       ENDIF
     ENDFOR
   ENDFOR
   IF (size(reply->tier_group,5)=0)
    SET reply->status_data.status = "Z"
   ENDIF
  ELSE
   SET reply->status_data.status = cvextreply->status_data.status
   GO TO end_program
  ENDIF
  IF (validate(debug,- (1)) > 0)
   CALL echorecord(cvextreply)
  ENDIF
  SELECT INTO "nl:"
   FROM code_value c,
    (dummyt d1  WITH seq = value(size(temprep->tier_group,5)))
   PLAN (d1)
    JOIN (c
    WHERE (c.code_value=temprep->tier_group[d1.seq].tier_group_cd))
   ORDER BY c.display
   HEAD c.display_key
    cntcount += 1
    IF (mod(cntcount,10) > 0)
     stat = alterlist(reply->tier_group,(cntcount+ 9))
    ENDIF
    reply->tier_group[cntcount].tier_group_cd = c.code_value, reply->tier_group[cntcount].
    tier_group_desc = c.description, reply->tier_group[cntcount].tier_group_disp = c.display,
    reply->tier_group[cntcount].tier_group_mean = c.cdf_meaning, reply->tier_group[cntcount].
    active_ind = c.active_ind, reply->tier_group[cntcount].updt_cnt = c.updt_cnt
   DETAIL
    null
   WITH nocounter
  ;end select
  SET stat = alterlist(reply->tier_group,cntcount)
 ENDIF
 IF (curqual > 0)
