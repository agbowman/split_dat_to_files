CREATE PROGRAM bed_get_mdro_cat_org_lookback:dba
 ...
 SUBROUTINE loadlookbacksettings(dummy_var)
   ...
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = size(request->mdro_items,5)),
     br_mdro_cat_organism cat_org,
     code_value cv
    PLAN (d)
     JOIN (cat_org
     WHERE (((cat_org.br_mdro_id=request->mdro_items[d.seq].br_mdro_id)) OR ((request->mdro_items[d
     .seq].br_mdro_id=0.0)))
      AND (((cat_org.br_mdro_cat_id=request->mdro_items[d.seq].br_mdro_cat_id)) OR ((request->
     mdro_items[d.seq].br_mdro_cat_id=0.0)))
      AND (((cat_org.organism_cd=request->mdro_items[d.seq].organism_cd)) OR ((request->mdro_items[d
     .seq].organism_cd=0.0)))
      AND (((cat_org.location_cd=request->mdro_items[d.seq].location_cd)) OR ((request->mdro_items[d
     .seq].location_cd=0.0))) )
     JOIN (cv
     WHERE cv.code_value=outerjoin(cat_org.lookback_time_span_unit_cd))
    ORDER BY d.seq, cat_org.br_mdro_cat_organism_id
    HEAD REPORT
     mdro_return_item_count = 0
    DETAIL
     IF (cat_org.lookback_time_span_unit_cd > 0.0)
      ...
      .br_mdro_id = cat_org.br_mdro_id, reply->mdro_items[mdro_return_item_count].br_mdro_cat_id =
      cat_org.br_mdro_cat_id,
      reply->mdro_items[mdro_return_item_count].organism_cd = cat_org.organism_cd, reply->mdro_items[
      mdro_return_item_count].location_cd = cat_org.location_cd, reply->mdro_items[
      mdro_return_item_count].lookback_time_span_nbr = cat_org.lookback_time_span_nbr,
