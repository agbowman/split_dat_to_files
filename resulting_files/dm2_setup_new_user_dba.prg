CREATE PROGRAM dm2_setup_new_user:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsnu_di_found = i2 WITH protect, noconstant(0)
 DECLARE dsnu_dii_found = i2 WITH protect, noconstant(0)
 DECLARE dsnu_stmt = vc WITH protect, noconstant("")
 DECLARE dsnu_dbname = vc
 DECLARE dsnu_orig_user = vc WITH protect, noconstant("")
 DECLARE dsnu_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dsnu_env_name = vc WITH protect, noconstant("")
 DECLARE dsnu_env_desc = vc WITH protect, noconstant("")
 DECLARE dsnu_env_exists = i2 WITH protect, noconstant(0)
 DECLARE dsnu_env_established = i2 WITH protect, noconstant(0)
 DECLARE dsnu_dtsp_fnd = i2 WITH protect, noconstant(0)
 DECLARE dsnu_itsp_fnd = i2 WITH protect, noconstant(0)
 DECLARE dsnu_d_tspace = vc WITH protect, noconstant("D_R_M002")
 DECLARE dsnu_i_tspace = vc WITH protect, noconstant("I_R_M002")
 DECLARE dsnu_free_define = i2 WITH protect, noconstant(0)
 DECLARE dsnu_disk_group = vc WITH protect, noconstant("")
 DECLARE dsnu_full_cnnct_str = vc WITH protect, noconstant("")
 DECLARE dsnu_loop = i2 WITH protect, noconstant(0)
 DECLARE dsnu_emsg = vc WITH protect, noconstant("")
 DECLARE dsnu_action = vc WITH protect, noconstant("")
 DECLARE dsnu_curuser = vc WITH protect, noconstant("")
 DECLARE dsnu_cnnct_str = vc WITH protect, noconstant(" ")
 DECLARE dsnu_done = i2 WITH protect, noconstant(0)
 DECLARE dsnu_fini = i2 WITH protect, noconstant(0)
 DECLARE dsnu_choice = vc WITH protect, noconstant("")
 DECLARE dsnu_sys_user = vc WITH protect, noconstant("")
 DECLARE dsnu_sys_pwd = vc WITH protect, noconstant("")
 DECLARE dsnu_cnt = i4 WITH protect, noconstant(0)
 DECLARE dsnu_loc = i4 WITH protect, noconstant(0)
 DECLARE dsnu_file = vc WITH protect, noconstant("")
 DECLARE dsnu_cmd = vc WITH protect, noconstant("")
 DECLARE dsnu_uspdef_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE dsnu_do_tspace = vc WITH protect, noconstant(" ")
 DECLARE dsnu_io_tspace = vc WITH protect, noconstant(" ")
 DECLARE dsnu_set_cmd1 = vc WITH protect, noconstant("set trace")
 DECLARE dsnu_set_cmd2 = vc WITH protect, noconstant("backdoor")
 DECLARE dsnu_dba_user = vc WITH protect, noconstant("P30INS")
 DECLARE dm2_create_admin_link_name = vc WITH public, noconstant("ADMIN1")
 DECLARE dsnu_curaccept = vc WITH protect, noconstant("")
 DECLARE dsnu_hold_eproc = vc WITH protect, noconstant("")
 DECLARE dsnu_hold_emsg = vc WITH protect, noconstant("")
 DECLARE dsnu_hold_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dsnu_hold_user_action = vc WITH protect, noconstant("")
 FREE RECORD dsnu_privs
 RECORD dsnu_privs(
   1 cnt = i4
   1 privs_needed_ind = i2
   1 privs_granted_ind = i2
   1 qual[*]
     2 priv_str = vc
     2 already_granted_ind = i2
 )
 SET dsnu_privs->cnt = 0
 SET dsnu_privs->privs_needed_ind = 0
 SET dsnu_privs->privs_granted_ind = 0
 IF (check_logfile("dm2_setup_new_user",".log","DM2_SETUP_NEW_USER LOG FILE")=0)
  GO TO exit_program
 ENDIF
 SET width = 132
 SET dm_err->eproc = "Begin dm2_setup_new_user..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Validating User Privileges"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dsnu_curuser = curuser
 IF (dsnu_curuser="P30INS")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Validate current o/s user."
  SET dm_err->emsg = "Current o/s user is invalid."
  SET dm_err->user_action = "Please exit ccl and re-enter to establish proper o/s user."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ELSEIF (val_user_privs(1)=0)
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "The above error is ignorable."
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dsnu_cmd = concat(dsnu_set_cmd1," ",dsnu_set_cmd2," ",dsnu_dba_user,
   " go")
  IF (dm2_push_cmd(dsnu_cmd,1)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Check whether current ccl user exists."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM duaf d
   WHERE d.user_name=dsnu_curuser
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF (curqual > 0)
   IF (dm2_push_cmd(concat("drop ccluser ",dsnu_curuser," go"),1)=0)
    GO TO exit_program
   ENDIF
  ENDIF
  IF (dm2_push_cmd(concat("create ccluser ",dsnu_curuser," to dba go"),1)=0)
   GO TO exit_program
  ENDIF
  SET dsnu_cmd = concat(dsnu_set_cmd1," ",dsnu_set_cmd2," ",dsnu_curuser,
   " go")
  IF (dm2_push_cmd(concat(dsnu_cmd),1)=0)
   GO TO exit_program
  ENDIF
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Check whether oraclesystem database object exists in dictionary."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dprotect d
  WHERE d.object="D"
   AND d.object_name="ORACLESYSTEM"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dsnu_stmt = concat("create database oraclesystem organization(oracle) ",
   "format(undefined) size(8000) type(s) schema(oraclesystem) go")
  IF (dm2_push_cmd(dsnu_stmt,1)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 ...
 SET width = 132
 CALL clear(1,1)
 SET dm2_install_schema->u_name = " "
 SET dm2_install_schema->p_word = " "
 SET dm2_install_schema->connect_str = " "
 CALL video(n)
 CALL clear(5,1,60)
 CALL text(5,1,"Connecting to database with CLINICAL user information provided below.")
 CALL video(n)
 SET dsnu_done = 0
 WHILE (dsnu_done=0)
   CALL text(8,1,"Please enter the user: ")
   CALL text(9,1,"Please enter the password (case-sensitive): ")
   CALL text(10,1,"Please enter the connect string: ")
   CALL text(11,1,"Enter 'C' to continue or 'Q' to continue later. (C or Q): ")
   CALL accept(8,60,"P(30);c",dm2_install_schema->u_name
    WHERE  NOT (curaccept=" "))
   SET dm2_install_schema->u_name = build(curaccept)
   CALL accept(9,60,"P(30);c",dm2_install_schema->p_word)
   SET dm2_install_schema->p_word = build(curaccept)
   CALL accept(10,60,"P(30);c",dm2_install_schema->connect_str
    WHERE  NOT (curaccept=" "))
   SET dm2_install_schema->connect_str = build(curaccept)
   SET dsnu_fini = 0
   WHILE (dsnu_fini=0)
     CALL accept(11,60,"A;cu","C")
     SET dsnu_choice = curaccept
     IF (((dsnu_choice="Q") OR (dsnu_choice="C")) )
      SET dsnu_fini = 1
     ENDIF
   ENDWHILE
   IF (dsnu_choice="Q")
    ...
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "User quit process.  "
    SET dm_err->eproc = "Prompting for database connect information."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ENDIF
   SET message = nowindow
   CALL parser("free define oraclesystem go",1)
   SET dsnu_cnnct_str = build(dm2_install_schema->u_name,"/",dm2_install_schema->p_word,"@",
    dm2_install_schema->connect_str)
   CALL parser(concat("define oraclesystem '",trim(dsnu_cnnct_str),"' go"),1)
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 0
    SET message = window
    SET width = 132
    CALL clear(1,1)
    CALL video(r)
    CALL text(5,1,concat("Unsuccessful connection to ",dm2_install_schema->dbase_name,
      " database for user ",dm2_install_schema->u_name,".  Please retry."))
    CALL video(n)
   ELSE
    SET dm2_sys_misc->cur_db_os = validate(currdbsys,cursys)
    IF (size(dm2_sys_misc->cur_db_os) != 3)
   ...
   ENDIF
 ENDWHILE
 SET dsnu_orig_user = dm2_install_schema->u_name
 ...
 IF (validate(dm2_bypass_priv_checks,- (1))=1)
  SET dm_err->eproc = concat("Bypassing logic for granting/revoking privileges. ",cnvtlower(
    dsnu_orig_user)," user should already be granted the required privs.")
  CALL disp_msg(" ",dm_err->logfile,0)
 ELSE
  SET dsnu_privs->cnt = 11
  SET stat = alterlist(dsnu_privs->qual,dsnu_privs->cnt)
  SET dsnu_privs->qual[1].priv_str = "SELECT ANY DICTIONARY"
  SET dsnu_privs->qual[1].already_granted_ind = 0
  SET dsnu_privs->qual[2].priv_str = "SELECT ANY TABLE"
  SET dsnu_privs->qual[2].already_granted_ind = 0
  SET dsnu_privs->qual[3].priv_str = "CREATE PUBLIC SYNONYM"
  SET dsnu_privs->qual[3].already_granted_ind = 0
  SET dsnu_privs->qual[4].priv_str = "CREATE PUBLIC DATABASE LINK"
  SET dsnu_privs->qual[4].already_granted_ind = 0
  SET dsnu_privs->qual[5].priv_str = "DROP PUBLIC SYNONYM"
  SET dsnu_privs->qual[5].already_granted_ind = 0
  SET dsnu_privs->qual[6].priv_str = "DROP PUBLIC DATABASE LINK"
  SET dsnu_privs->qual[6].already_granted_ind = 0
  SET dsnu_privs->qual[7].priv_str = "CREATE ANY CONTEXT"
  SET dsnu_privs->qual[7].already_granted_ind = 0
  SET dsnu_privs->qual[8].priv_str = "ALTER TABLESPACE"
  SET dsnu_privs->qual[8].already_granted_ind = 0
  SET dsnu_privs->qual[9].priv_str = "ALTER SESSION"
  SET dsnu_privs->qual[9].already_granted_ind = 0
  SET dsnu_privs->qual[10].priv_str = "RESUMABLE"
  SET dsnu_privs->qual[10].already_granted_ind = 0
  SET dsnu_privs->qual[11].priv_str = "UNLIMITED TABLESPACE"
  SET dsnu_privs->qual[11].already_granted_ind = 0
  IF ((dm_err->debug_flag > 0))
   CALL echorecord(dsnu_privs)
  ENDIF
  SET dsnu_file = concat(trim(logical("cer_install")),"\dm2_",cnvtlower(dsnu_orig_user),"_privs.txt")
  IF (findfile(dsnu_file) > 0)
   SET dm_err->eproc = concat("Read file:",dsnu_file)
   CALL disp_msg("",dm_err->logfile,0)
   FREE DEFINE rtl2
   SET logical file_name value(dsnu_file)
   ...
   SELECT INTO "nl:"
    ...
    WHERE t.line > " "
    DETAIL
     dsnu_loc = locateval(dsnu_cnt,1,dsnu_privs->cnt,trim(cnvtupper(t.line)),dsnu_privs->qual[
      dsnu_cnt].priv_str)
     IF (dsnu_loc > 0)
      dsnu_privs->qual[dsnu_loc].already_granted_ind = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    ...
    GO TO exit_program
   ENDIF
   FOR (dsnu_cnt = 1 TO dsnu_privs->cnt)
     IF ((dsnu_privs->qual[dsnu_cnt].already_granted_ind=0))
      SET dsnu_privs->privs_needed_ind = 1
     ENDIF
   ENDFOR
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dsnu_privs)
  ...
  ELSE
   SELECT INTO "nl:"
    FROM dtable d
    WHERE d.table_name="USER_SYS_PRIVS"
    DETAIL
     dsnu_uspdef_exists_ind = 1
    WITH nocounter
   ;end select
   IF (check_error("Verifying that table definitions exist for USER_SYS_PRIVS.")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (dsnu_uspdef_exists_ind=0)
    DROP TABLE user_sys_privs
    DROP DDLRECORD user_sys_privs FROM DATABASE v500 WITH deps_deleted
    CREATE DDLRECORD user_sys_privs FROM DATABASE v500
 TABLE user_sys_privs
  1 username  = vc30 CCL(username)
  1 privilege  = vc40 CCL(privilege)
  1 admin_option  = vc3 CCL(admin_option)
  1 rowid CCL(rowid)
    2 rowid_fld  = c18
 END TABLE user_sys_privs
   ENDIF
   SET dm_err->eproc = concat("Identify if user ",dsnu_orig_user," is already a high privilege user")
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM user_sys_privs p,
     (dummyt d  WITH seq = dsnu_privs->cnt)
    PLAN (d
     WHERE d.seq > 0)
     JOIN (p
     WHERE (p.privilege=dsnu_privs->qual[d.seq].priv_str))
    DETAIL
     dsnu_privs->qual[d.seq].already_granted_ind = 1
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   FOR (dsnu_cnt = 1 TO dsnu_privs->cnt)
     ...
     ENDIF
   ENDFOR
   IF ((dsnu_privs->privs_needed_ind=1))
    SET dm_err->eproc = concat("Creating the user privs file: <",dsnu_file,">")
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO value(dsnu_file)
     FROM (dummyt d  WITH seq = dsnu_privs->cnt)
     WHERE (dsnu_privs->qual[d.seq].already_granted_ind=1)
     DETAIL
      col 0, dsnu_privs->qual[d.seq].priv_str, row + 1
     WITH nocounter, format = variable, formfeed = none,
      maxrow = 1, maxcol = 512
    ;end select
    IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
  IF ((dsnu_privs->privs_needed_ind=1))
   SET dm_err->eproc = "Prompt for HIGH_PRIV (DBA) database connect string and validate"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dm2_install_schema->dbase_name = "CLINICAL"
   SET message = window
   SET width = 132
   ...
   CALL video(n)
   CALL clear(5,1,60)
   CALL text(5,1,concat("Connecting to database with HIGH-PRIV (DBA) user."))
   CALL video(n)
   SET dsnu_done = 0
   WHILE (dsnu_done=0)
     CALL text(7,1,"Please enter a high priv user:")
     CALL text(8,1,"Please enter the database password (case-sensitive): ")
     CALL text(9,1,"Please enter the database connect string: ")
     CALL text(10,1,"Enter 'C' to continue or 'Q' to continue later. (C or Q): ")
     CALL accept(7,60,"P(30);c",dm2_install_schema->u_name
      WHERE  NOT (curaccept=" "))
     SET dm2_install_schema->u_name = build(curaccept)
     CALL accept(8,60,"P(30);c",dm2_install_schema->p_word
      WHERE  NOT (curaccept=" "))
     SET dm2_install_schema->p_word = build(curaccept)
     CALL accept(9,60,"P(30);c",dm2_install_schema->connect_str
      WHERE  NOT (curaccept=" "))
     SET dm2_install_schema->connect_str = build(curaccept)
     SET dsnu_fini = 0
     WHILE (dsnu_fini=0)
       CALL accept(10,60,"A;cu","C")
       SET dsnu_choice = curaccept
       IF (((dsnu_choice="Q") OR (dsnu_choice="C")) )
        SET dsnu_fini = 1
       ENDIF
     ...
     ENDIF
   ENDWHILE
   SET dsnu_sys_user = dm2_install_schema->u_name
   SET dsnu_sys_pwd = dm2_install_schema->p_word
   SET dm_err->eproc = concat("Issuing priviliges to user: ",dsnu_orig_user)
   CALL disp_msg(" ",dm_err->logfile,0)
   FOR (dsnu_cnt = 1 TO dsnu_privs->cnt)
     SET dsnu_cmd = concat("rdb grant ",dsnu_privs->qual[dsnu_cnt].priv_str," to ",dsnu_orig_user,
      " go")
     IF (dm2_push_cmd(dsnu_cmd,1)=0)
      GO TO exit_program
     ENDIF
     SET dsnu_privs->privs_granted_ind = 1
   ENDFOR
  ENDIF
 ENDIF
 SET dm_err->eproc = "Prompt for admin database connect string and validate..."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 CALL video(n)
 CALL clear(5,1,60)
 CALL text(5,1,concat("Connecting to ",dm2_install_schema->dbase_name,
   " database with user information provided below."))
 CALL video(n)
 SET dsnu_done = 0
 WHILE (dsnu_done=0)
   CALL text(7,1,"Entering database connect information for user:")
   CALL text(7,60,dm2_install_schema->u_name)
   CALL text(8,1,"Please enter the database password (case-sensitive): ")
   CALL text(9,1,"Please enter the database connect string: ")
   CALL text(10,1,"Enter 'C' to continue or 'Q' to continue later. (C or Q): ")
   CALL accept(8,60,"P(30);c",dm2_install_schema->p_word
 ...
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm2_install_schema->dbase_name = '"CLINICAL"'
 SET dm2_install_schema->u_name = dsnu_orig_user
 ...
 CALL parser("free define oraclesystem go",1)
 SET dsnu_cnnct_str = cnvtlower(build(dm2_install_schema->u_name,"/",dm2_install_schema->p_word,"@",
   dm2_install_schema->connect_str))
 CALL parser(concat("define oraclesystem '",trim(dsnu_cnnct_str),"' go"),1)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
 ...
 IF (currdb != "ORACLE")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Must be connected to Oracle database.")
  SET dm_err->eproc = "RDBMS Validation"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ((validate(dm2_bypass_sys_oragen,- (1))=- (1)))
  ...
  CALL box(1,1,5,132)
  SET dsnu_row = 3
  CALL text(dsnu_row,48,"***  ORAGEN SYSTEM OBJECTS PROMPT  ***")
  SET dsnu_row = (dsnu_row+ 4)
  CALL text(dsnu_row,2,
   "WARNING:  Oragen of all system objects will take several hours.  Press enter to continue... ")
  SET dsnu_row = (dsnu_row+ 2)
  CALL text(dsnu_row,2,
   "Note:  If oragen has already been completed for this environemnt, you can skip oragen work. ")
  SET dsnu_row = (dsnu_row+ 3)
  CALL text(dsnu_row,2,
   "Enter 'C' to continue, 'S' to skip oragen because already completed or 'Q' to quit:  ")
  CALL accept(dsnu_row,86,"A;CU"," "
   WHERE curaccept IN ("C", "S", "Q"))
  SET dsnu_curaccept = curaccept
  CALL clear(1,1)
  SET message = nowindow
  IF (dsnu_curaccept="Q")
   SET dm_err->emsg = "User elected to quit from oragen system objects prompt."
   SET dm_err->eproc = "Issuing oragen system objects prompt."
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ELSEIF (dsnu_curaccept="S")
   SET dm_err->eproc =
   "Oragen system objects skipped.  User elected to skip as already completed previously."
   CALL disp_msg(" ",dm_err->logfile,0)
  ELSE
   EXECUTE oragen3 "sys.*"
   IF ((dm_err->err_ind=1))
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Bypassing oragen of system objects."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SET dm2_install_schema->curprog = "DM2_SETUP_NEW_USER"
 EXECUTE dm2_create_system_defs
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 IF (dm2_get_dbase_name(dsnu_dbname)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("database name = ",dsnu_dbname))
 ENDIF
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 EXECUTE dm2_create_admin_link value(dm2_install_schema->cdba_connect_str), "none", value(
  dm2_install_schema->cdba_p_word)
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Check if DM required tablespace exists."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_tablespaces dt
  WHERE dt.tablespace_name IN (dsnu_d_tspace, dsnu_i_tspace)
  DETAIL
   IF (dt.tablespace_name=dsnu_d_tspace)
    dsnu_dtsp_fnd = 1
   ELSE
    dsnu_itsp_fnd = 1
   ENDIF
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Check if DM_INFO table exists."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM user_tables ut
 ...
 ENDIF
 IF (dsnu_di_found=1)
  SET dm_err->eproc = "Check if DM_INFO primary key index exists."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM user_indexes ui
   WHERE ui.index_name="XPKDM_INFO"
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ELSEIF (curqual > 0)
   SET dsnu_dii_found = 1
  ENDIF
 ENDIF
 IF (((dsnu_di_found=0) OR (dsnu_dii_found=0)) )
  SET dsnu_loop = 1
  WHILE (dsnu_loop=1)
    SET dsnu_do_tspace = ""
    SET dsnu_io_tspace = ""
    ...
    CALL clear(1,1)
    CALL box(1,1,5,132)
    CALL text(3,45,"***  Tablespace Override for DM_INFO Table/Index  ***")
    CALL text(6,2,concat(
      "Note:  Tablespace(s) must exist to override defaults.  Current defaults are ",trim(
       dsnu_d_tspace)," and ",trim(dsnu_i_tspace),". "))
    CALL text(8,2,"Override tablespace defaults (Y/N): ")
    CALL text(10,2,"Tablespace to be used to create DM_INFO table: ")
    CALL text(11,2,"Tablespace to be used to create DM_INFO index: ")
    IF (dsnu_emsg > "")
     CALL text(13,2,dsnu_emsg)
     SET dsnu_emsg = ""
    ENDIF
    IF (dsnu_action > "")
     CALL text(15,2,dsnu_action)
     SET dsnu_action = ""
    ENDIF
    CALL accept(8,55,"A;CU"," "
     WHERE curaccept IN ("Y", "N"))
    IF (curaccept="Y")
     CALL text(13,2,fillstring(130," "))
     CALL text(15,2,fillstring(130," "))
     CALL accept(10,55,"P(30);CU",dsnu_do_tspace
      WHERE build(curaccept) > "")
     SET dsnu_do_tspace = curaccept
     CALL accept(11,55,"P(30);CU",dsnu_io_tspace
      WHERE build(curaccept) > "")
     SET dsnu_io_tspace = curaccept
     SET dsnu_cnt = 0
     SELECT INTO "nl:"
      FROM dba_tablespaces dt
      WHERE dt.tablespace_name IN (dsnu_do_tspace, dsnu_io_tspace)
      DETAIL
       IF (dt.tablespace_name=dsnu_do_tspace)
        dsnu_cnt = (dsnu_cnt+ 1)
       ENDIF
       IF (dt.tablespace_name=dsnu_io_tspace)
        dsnu_cnt = (dsnu_cnt+ 1)
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error("Check that Tablespace exists.")=1)
      CALL clear(1,1)
      SET message = nowindow
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ELSEIF (dsnu_cnt != 2)
      SET dsnu_emsg = concat("ERROR:  Tablespace ",trim(dsnu_do_tspace)," and/or ",trim(
        dsnu_io_tspace)," does not exist.")
      SET dsnu_action = concat("ACTION: Please enter existing tablespaces.")
     ELSE
      SET dsnu_d_tspace = dsnu_do_tspace
      SET dsnu_i_tspace = dsnu_io_tspace
      SET dsnu_dtsp_fnd = 1
      SET dsnu_itsp_fnd = 1
      SET dsnu_loop = 0
      CALL clear(1,1)
 ...
 ENDIF
 IF (((dsnu_di_found=0) OR (dsnu_dii_found=0))
  AND ((dsnu_dtsp_fnd=0) OR (dsnu_itsp_fnd=0)) )
  IF ((dir_storage_misc->cur_storage_type="ASM"))
   SET dsnu_loop = 1
   WHILE (dsnu_loop=1)
     SET dsnu_disk_group = ""
     IF ((dm_err->debug_flag=722))
     ...
     CALL box(1,1,5,132)
     SET dsnu_row = 3
     CALL text(dsnu_row,48,"***  ASM DISK GROUP PROMPT  ***")
     SET dsnu_row = (dsnu_row+ 3)
     CALL text(dsnu_row,2,concat("Database Name : ",trim(dsnu_dbname)))
     SET dsnu_row = (dsnu_row+ 3)
     CALL text(dsnu_row,2,"Disk group to be used to create DM required tablespaces: ")
     IF (dsnu_emsg > "")
      CALL text((dsnu_row+ 3),2,dsnu_emsg)
      SET dsnu_emsg = ""
     ENDIF
     SET help =
     SELECT
      ...
      WHERE a.state IN ("CONNECTED", "MOUNTED")
      ORDER BY a.name
      WITH nocounter
     ;end select
     CALL accept(dsnu_row,59,"P(30);CUF",dsnu_disk_group
      WHERE build(curaccept) > "")
     SET help = off
     SET dsnu_disk_group = curaccept
     SET dm_err->eproc = "Check that disk group exists."
     CALL disp_msg(" ",dm_err->logfile,0)
     SELECT INTO "nl:"
      FROM v$asm_diskgroup a
      WHERE cnvtupper(a.name)=dsnu_disk_group
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL clear(1,1)
      ...
      GO TO exit_program
     ELSEIF (curqual=0)
      SET dsnu_emsg = "ERROR: Disk group not found.  Please enter a valid disk group."
     ELSE
      CALL clear((dsnu_row+ 3),2,132)
      SET dsnu_row = (dsnu_row+ 5)
      CALL text(dsnu_row,2,concat("Please confirm disk group to be used is: ",dsnu_disk_group))
      SET dsnu_row = (dsnu_row+ 3)
      CALL text(dsnu_row,2,
       "Enter 'C' to continue, 'R' to reenter disk group or 'Q' to quit (C/R/Q): ")
      CALL accept(dsnu_row,74,"A;CU"," "
       WHERE curaccept IN ("C", "R", "Q"))
      IF (curaccept="Q")
       CALL clear(1,1)
       SET message = nowindow
       SET dm_err->eproc = "Displaying ASM Disk Group Prompt."
       SET dm_err->emsg = "User chose to quit from ASM Disk Group Prompt."
       SET dm_err->err_ind = 1
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ELSEIF (curaccept="C")
   ...
   SET message = nowindow
   IF (dsnu_dtsp_fnd=0)
    SET dm_err->eproc = concat("Create ",dsnu_d_tspace," tablespace.")
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dsnu_stmt = concat("RDB ASIS (^ create tablespace ",trim(dsnu_d_tspace,3)," datafile '+",trim
     (dsnu_disk_group,3),"' size 1M autoextend ",
     "on next 64M ^) GO ")
    IF (dm2_push_cmd(dsnu_stmt,1)=0)
     GO TO exit_program
    ENDIF
   ENDIF
   IF (dsnu_itsp_fnd=0)
    SET dm_err->eproc = concat("Create ",dsnu_i_tspace," tablespace.")
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dsnu_stmt = concat("RDB ASIS (^ create tablespace ",trim(dsnu_i_tspace,3)," datafile '+",trim
     (dsnu_disk_group,3),"' size 1M autoextend ",
     "on next 64M ^) GO ")
    IF (dm2_push_cmd(dsnu_stmt,1)=0)
     GO TO exit_program
  ...
  ELSE
   ...
   WHILE (dsnu_loop=1)
     ...
     CALL box(1,1,5,132)
     SET dsnu_row = 3
     CALL text(dsnu_row,48,"***  TABLESPACE CREATION PROMPT  ***")
     SET dsnu_row = (dsnu_row+ 3)
     CALL text(dsnu_row,2,concat("Database Name : ",trim(dsnu_dbname)))
     SET dsnu_row = (dsnu_row+ 3)
     CALL text(dsnu_row,2,"Create the following tablespace(s) before continuing... ")
     SET dsnu_row = (dsnu_row+ 1)
     IF (dsnu_dtsp_fnd=0)
      SET dsnu_row = (dsnu_row+ 1)
      CALL text(dsnu_row,7,concat(trim(dsnu_d_tspace)," (64K)"))
     ENDIF
     IF (dsnu_itsp_fnd=0)
      SET dsnu_row = (dsnu_row+ 1)
      CALL text(dsnu_row,7,concat(trim(dsnu_i_tspace)," (64K)"))
     ENDIF
     SET dsnu_row = (dsnu_row+ 3)
     CALL text(dsnu_row,2,"Enter 'C' to continue or 'Q' to quit:  ")
     CALL clear((dsnu_row+ 3),2,132)
     IF (dsnu_emsg > "")
      CALL text((dsnu_row+ 3),2,dsnu_emsg)
      SET dsnu_emsg = ""
     ENDIF
     CALL accept(dsnu_row,41,"A;CU"," "
      WHERE curaccept IN ("C", "Q"))
     IF (curaccept="Q")
      CALL clear(1,1)
      SET message = nowindow
      SET dm_err->emsg = "User elected to quit from tablespace creation prompt."
      SET dm_err->eproc = "Issuing tablespace creation prompt."
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
     ...
     IF (check_error(dm_err->eproc)=1)
      ...
      GO TO exit_program
     ENDIF
     IF (((dsnu_dtsp_fnd=0) OR (dsnu_itsp_fnd=0)) )
      SET dsnu_emsg = concat("ERROR: Required tablespace(s) do not exist.",
       "  Please create aforementioned tablespace(s) before continuing.")
     ELSE
      SET dsnu_loop = 0
     ENDIF
   ENDWHILE
 ...
 ENDIF
 IF (dsnu_di_found=0)
  SET dm_err->eproc = "Create DM_INFO table."
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dsnu_stmt = concat("RDB ASIS (^ CREATE TABLE DM_INFO (INFO_DOMAIN VARCHAR2(80) DEFAULT ",
   "'DATA MANAGEMENT' NOT NULL, INFO_NAME VARCHAR2(255) DEFAULT ' ' NOT NULL, INFO_DATE DATE NULL, ",
   "INFO_CHAR VARCHAR2(4000) NULL, INFO_NUMBER FLOAT NULL, INFO_LONG_ID NUMBER DEFAULT 0 ",
   "NOT NULL, UPDT_APPLCTX NUMBER DEFAULT 0 NOT NULL, UPDT_DT_TM DATE DEFAULT SYSDATE NOT NULL, ",
   "UPDT_CNT NUMBER DEFAULT 0 NOT NULL, UPDT_ID NUMBER DEFAULT 0 NOT NULL, UPDT_TASK NUMBER ",
   "DEFAULT 0 NOT NULL, INFO_DOMAIN_ID NUMBER DEFAULT 0 NOT NULL) PCTFREE 10 STORAGE ",
   "(INITIAL 65536 NEXT 65536) TABLESPACE ",trim(dsnu_d_tspace,3),"^) GO ")
  IF (dm2_push_cmd(dsnu_stmt,1)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dsnu_dii_found=0)
  SET dm_err->eproc = "Create DM_INFO primary key index."
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dsnu_stmt = concat(
   "RDB ASIS (^CREATE UNIQUE INDEX XPKDM_INFO ON DM_INFO (INFO_NAME,INFO_DOMAIN) ",
   "STORAGE (INITIAL 65536 NEXT 65536) LOGGING TABLESPACE ",trim(dsnu_i_tspace,3),"^) GO ")
  IF (dm2_push_cmd(dsnu_stmt,1)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 EXECUTE oragen3 "DM_INFO"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 IF (validate(dm2_bypass_create_admin_syn,- (1))=1)
  SET dm_err->eproc = concat("Bypassing creation of clinical public synonyms on Admin tables.  ",
   "Assumed all created from previous execution.")
  CALL disp_msg(" ",dm_err->logfile,0)
 ELSE
  SET dm_err->eproc = "Checking for existence of DM_ENVIRONEMNT synonym."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_program
  ELSEIF (curqual=1)
   SET dm_err->eproc = "Dropping DM_ENVIRONMENT synonym so all Admin synonyms re-created."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dsnu_stmt = concat("rdb drop public synonym DM_ENVIRONMENT go")
   IF (dm2_push_cmd(dsnu_stmt,1)=0)
    GO TO exit_program
   ENDIF
  ENDIF
  SET dm_err->eproc = "Create Admin public synonyms for all Admin tables and select sequences."
  CALL disp_msg("",dm_err->logfile,0)
  EXECUTE dm2_create_admin_nicknames "*", value(dm2_install_schema->cdba_p_word)
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Selecting environment data from dm_info/dm_environment."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  d1.info_number, d2.info_char
  FROM dm_info d1,
   dm_info d2,
   dm_environment de
  WHERE d1.info_name="DM_ENV_ID"
   AND d1.info_domain="DATA MANAGEMENT"
   AND de.environment_id=d1.info_number
   AND d2.info_name="DM_ENV_NAME"
   AND d2.info_domain="DATA MANAGEMENT"
   AND de.environment_name=d2.info_char
  DETAIL
   dsnu_env_id = de.environment_id, dsnu_env_name = de.environment_name, dsnu_env_desc = de
   .description,
   dsnu_env_established = 1, dsnu_env_exists = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dsnu_env_established=0)
  SET dsnu_loop = 1
  WHILE (dsnu_loop=1)
    SET dsnu_env_name = ""
    SET dsnu_env_desc = ""
    SET dsnu_env_id = 0.0
    SET dsnu_env_exists = 0
    IF ((dm_err->debug_flag=722))
     SET message = nowindow
    ELSE
     SET message = window
    ...
    CALL box(1,1,5,132)
    SET dsnu_row = 3
    CALL text(dsnu_row,48,"***  SETUP NEW ENVIRONMENT  ***")
    SET dsnu_row = (dsnu_row+ 3)
    CALL text(dsnu_row,2,concat("Database Name : ",trim(dsnu_dbname)))
    SET dsnu_row = (dsnu_row+ 3)
    CALL text(dsnu_row,2,"Environment Name: ")
    IF ((dm2_sys_misc->cur_os="WIN"))
     CALL text((dsnu_row+ 1),2,
      "  * Enter a new environment name or press <F9> to select an existing environment name.")
    ELSE
     CALL text((dsnu_row+ 1),2,
      "  * Enter a new environment name or press <Shift><F5> to select an existing environment name."
      )
    ENDIF
    SET help =
    SELECT
     b.environment_name, b.database_name
     FROM dm_environment b
     WHERE (b.target_operating_system=dm2_sys_misc->cur_db_os)
      AND substring(1,5,b.environment_name) != "OLD <"
     ORDER BY b.environment_name
     WITH nocounter
    ;end select
    CALL accept(dsnu_row,21,"P(20);CU",dsnu_env_name
     WHERE build(curaccept) > "")
    SET help = off
    SET dsnu_env_name = curaccept
    SET dm_err->eproc = "Selecting environment description from dm_environment."
    SELECT INTO "nl:"
     FROM dm_environment de
     WHERE cnvtupper(de.environment_name)=dsnu_env_name
     DETAIL
      dsnu_env_id = de.environment_id, dsnu_env_desc = de.description, dsnu_env_exists = 1
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL clear(1,1)
     ...
     GO TO exit_program
    ENDIF
    IF (dsnu_env_exists=0)
     SET dsnu_row = (dsnu_row+ 4)
     CALL text(dsnu_row,2,"Environment Description: ")
     CALL accept(dsnu_row,27,"P(60);CU",dsnu_env_desc
      WHERE build(curaccept) > "")
     SET dsnu_env_desc = curaccept
    ENDIF
    SET dsnu_row = (dsnu_row+ 5)
    CALL text(dsnu_row,2,concat("Please confirm the following: "))
    SET dsnu_row = (dsnu_row+ 2)
    CALL text(dsnu_row,2,concat("   Environment Name:         ",trim(dsnu_env_name)))
    SET dsnu_row = (dsnu_row+ 1)
    CALL text(dsnu_row,2,concat("   Environment Description:  ",trim(dsnu_env_desc)))
    SET dsnu_row = (dsnu_row+ 3)
    CALL text(dsnu_row,2,
     "Enter 'C' to continue, 'R' to reenter environment info or 'Q' to quit (C/R/Q): ")
    CALL accept(dsnu_row,81,"A;CU"," "
     WHERE curaccept IN ("C", "R", "Q"))
    IF (curaccept="Q")
     CALL clear(1,1)
     SET message = nowindow
     SET dm_err->eproc = "Displaying Setup New Environment Prompt."
     SET dm_err->emsg = "User chose to quit from Setup New Environment Prompt."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ELSEIF (curaccept="C")
  ...
  SET message = nowindow
 ELSE
  SET dm_err->eproc = "Environment data already established in dm_info/dm_environment."
  CALL disp_msg("",dm_err->logfile,0)
  IF ((dm_err->debug_flag > 0))
   CALL echo(build("Environment Id   = ",dsnu_env_id))
   CALL echo(concat("Environment Name = ",dsnu_env_name))
   CALL echo(concat("Environment Desc = ",dsnu_env_desc))
  ENDIF
 ENDIF
 IF (dsnu_env_exists=0)
  ...
  CALL disp_msg("",dm_err->logfile,0)
  SET dsnu_full_cnnct_str = build(dsnu_orig_user,"/",dm2_install_schema->v500_p_word,"@",
   dm2_install_schema->v500_connect_str)
  INSERT  FROM dm_environment de
   SET de.environment_id = dsnu_env_id, de.description = dsnu_env_desc, de.environment_name =
    dsnu_env_name,
    de.database_name = dsnu_dbname, de.target_operating_system = dm2_sys_misc->cur_db_os, de
    .admin_dbase_link_name = "ADMIN1",
    de.db_version = 0, de.v500_connect_string = dsnu_full_cnnct_str, de.v500ref_connect_string = " ",
    de.cerner_fs_mtpt = "", de.ora_pri_fs_mtpt = "", de.ora_sec_fs_mtpt = "",
    de.oracle_version = dm2_rdbms_version->version, de.max_file_size = 0, de.envset_string = "",
    de.root_dir_name = " ", de.volume_group = " "
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SET message = nowindow
 IF (dsnu_env_established=0)
  SET dm_err->eproc = "Delete existing environment_id from DM_INFO."
  CALL disp_msg("",dm_err->logfile,0)
  DELETE  FROM dm_info d
   WHERE d.info_domain="DATA MANAGEMENT"
    AND d.info_name IN ("DM_ENV_ID", "DM_ENV_NAME")
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Insert new environment_id and environment_name into DM_INFO."
  CALL disp_msg("",dm_err->logfile,0)
  INSERT  FROM dm_info d
   SET d.info_domain = "DATA MANAGEMENT", d.info_name = "DM_ENV_ID", d.info_number = dsnu_env_id
   WITH nocounter
  ;end insert
  INSERT  FROM dm_info d
   SET d.info_domain = "DATA MANAGEMENT", d.info_name = "DM_ENV_NAME", d.info_char = dsnu_env_name
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Create login_default.ccl file in CCLUSERDIR..."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ELSE
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Check for DDL tables."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM user_tables u
 ...
 ELSE
  ...
  ENDIF
 ENDIF
 SET dsnu_free_define = 0
 GO TO exit_program
#exit_program
 CALL clear(1,1)
 SET message = nowindow
 SET dsnu_hold_eproc = dm_err->eproc
 SET dsnu_hold_emsg = dm_err->emsg
 SET dsnu_hold_err_ind = dm_err->err_ind
 SET dsnu_hold_user_action = dm_err->user_action
 SET dm_err->err_ind = 0
 IF (validate(dm2_bypass_priv_checks,- (1)) != 1)
  IF ((dsnu_privs->privs_needed_ind=1)
   AND (dsnu_privs->privs_granted_ind=1))
   SET dm_err->eproc = "Connect to clinical database as High Priv User..."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dm2_install_schema->dbase_name = '"CLINICAL"'
   SET dm2_install_schema->u_name = dsnu_sys_user
   SET dm2_install_schema->connect_str = dm2_install_schema->v500_connect_str
   SET dm2_install_schema->p_word = dsnu_sys_pwd
   CALL parser("free define oraclesystem go",1)
   SET dsnu_cnnct_str = cnvtlower(build(dm2_install_schema->u_name,"/",dm2_install_schema->p_word,"@",
     dm2_install_schema->connect_str))
   CALL parser(concat("define oraclesystem '",trim(dsnu_cnnct_str),"' go"),1)
   ...
   SET dm_err->eproc = concat(
    "Revoke privileges that were temporarily granted for this script for user: ",dsnu_orig_user)
   CALL disp_msg(" ",dm_err->logfile,0)
   FOR (dsnu_cnt = 1 TO dsnu_privs->cnt)
     IF ((dsnu_privs->qual[dsnu_cnt].already_granted_ind=0))
      SET dsnu_cmd = concat("rdb revoke ",dsnu_privs->qual[dsnu_cnt].priv_str," from ",dsnu_orig_user,
       " go")
      CALL dm2_push_cmd(dsnu_cmd,1)
     ENDIF
   ENDFOR
   IF (findfile(dsnu_file) > 0)
    CALL dm2_push_dcl(concat("del ",dsnu_file))
   ENDIF
  ENDIF
 ENDIF
 IF (dsnu_free_define=1)
  IF ((dm_err->debug_flag < 1))
  ...
  CALL echo("*")
  CALL echo("***************************************")
  CALL echo("* DATABASE CONNECTION REMOVED.        *")
  CALL echo("***************************************")
  CALL echo("*")
 ENDIF
 SET dm_err->eproc = dsnu_hold_eproc
 SET dm_err->emsg = dsnu_hold_emsg
 SET dm_err->err_ind = dsnu_hold_err_ind
 SET dm_err->user_action = dsnu_hold_user_action
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM2_SETUP_NEW_USER Completed."
 ENDIF
 CALL final_disp_msg("dm2_setup_new_user")
END GO
