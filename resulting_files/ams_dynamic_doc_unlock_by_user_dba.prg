CREATE PROGRAM ams_dynamic_doc_unlock_by_user:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Personnel" = 0,
  "Documents" = 0
  WITH outdev, author, ddsessionid
 ...
 DECLARE prog_name = vc WITH constant("ams_dynamic_doc_unlock_by_user")
 ...
 IF (run_ind=1
  ...
  CALL unlockops(0)
  SET reply->status_data[stat_cnt].status = "S"
  SET reply->status_data[stat_cnt].subeventstatus[1].operationstatus = "S"
  SET reply->status_data[stat_cnt].subeventstatus[1].operationname = prog_name
 ...
 SELECT INTO  $OUTDEV
  ...
  HEAD REPORT
   IF ((reply->status_data[stat_cnt].status="S"))
    ...
    row 7, tempstring = concat(cnvtstring( $DDSESSIONID)," DELETED FOR USER ",trim(provider_name)),
    col 10,
    tempstring
   ELSE
    reply->status_data[stat_cnt].status = "F", reply->status_data[stat_cnt].subeventstatus[1].
    operationstatus = "F", reply->status_data[stat_cnt].subeventstatus[1].operationname = prog_name,
    reply->status_data[stat_cnt].subeventstatus[1].targetobjectname = "err_msg", reply->status_data[
    stat_cnt].subeventstatus[1].targetobjectvalue = err_msg, row 5,
  ...
  WITH nocounter
 ;end select
 SET last_mod = "001 07/04/16 CD047457 Unlock now calls EJS 389"
END GO
