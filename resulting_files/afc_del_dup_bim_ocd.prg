CREATE PROGRAM afc_del_dup_bim_ocd
 ...
 SELECT INTO "NL:"
  ...
  WHERE utc.table_name="BILL_ITEM_MODIFIER"
  DETAIL
   table_exists = "T"
  WITH nocounter
 ;end select
 ...
 SET readme_data->message = "Update possible NULL fields to 0..."
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 IF (value(size(dup_keys->dups,5)) > 0)
  SET count1 = 0
  SET dup_qual = 0
  SELECT INTO "nl:"
  ...
  ;end select
 ENDIF
 SET readme_data->message = build("number of dups: ",size(dup_rows->dups,5))
