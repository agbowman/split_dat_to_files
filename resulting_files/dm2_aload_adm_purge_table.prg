CREATE PROGRAM dm2_aload_adm_purge_table
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_tbl",".log","dm2_aload_adm_purge_table")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_adm_purge_table"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daaptbl_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daaptbl_file = vc WITH protect, noconstant( $2)
 DECLARE daaptbl_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE daaptbl_batch = i4 WITH protect, noconstant(0)
 DECLARE daaptbl_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daaptbl_pkg,daaptbl_file_loc_ret,daaptbl_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daaptbl_batch=0)
  SET daaptbl_batch = 5000
 ENDIF
 IF (daaptbl_expected_rows > 0)
  EXECUTE dm2_dbimport concat(daaptbl_file_loc_ret,daaptbl_file), "dm2_aload_prgtpl_tab",
  daaptbl_batch
  CALL echo(concat(daaptbl_file_loc_ret,daaptbl_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_adm_purge_table completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_adm_purge_table")
END GO
