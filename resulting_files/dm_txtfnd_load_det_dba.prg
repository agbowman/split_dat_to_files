CREATE PROGRAM dm_txtfnd_load_det:dba
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 IF (check_logfile("dm_tgtfnd_load",".log","DM_TXTFND_LOAD_DET LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_load
 ENDIF
 SET dm_err->eproc = "Starting DM_TXTFND_LOAD_DET..."
 FREE RECORD txtfnd_scripts
 RECORD txtfnd_scripts(
   1 cnt = i4
   1 success_cnt = i4
   1 qual[*]
     2 script_name = vc
 ...
 DECLARE dtla_loop = i4 WITH protect, noconstant(0)
 DECLARE dtla_detail_id = f8 WITH protect, noconstant(0.0)
 DECLARE dtla_start_dt_tm = f8 WITH protect, noconstant(0.0)
 DECLARE dtla_end_dt_tm = f8 WITH protect, noconstant(0.0)
 DECLARE dtla_error_ind = i2 WITH protect, noconstant(0)
 DECLARE dtla_error_msg = vc WITH protect, noconstant("")
 IF ((daf_icd9_request->parent_log_id=0.0))
  ...
  GO TO exit_load
 ELSE
  SET script_rec->dm_text_find_log_id = daf_icd9_request->parent_log_id
 ENDIF
 IF ((daf_icd9_request->text_find_detail_id > 0.0))
  SELECT INTO "NL:"
   ...
   DETAIL
    txtfnd_scripts->cnt = (txtfnd_scripts->cnt+ 1), stat = alterlist(txtfnd_scripts->qual,
     txtfnd_scripts->cnt), txtfnd_scripts->qual[txtfnd_scripts->cnt].det_id = c
    .dm_text_find_detail_id,
    txtfnd_scripts->qual[txtfnd_scripts->cnt].script_name = cnvtupper(c.detail_script_name)
   WITH nocounter
  ;end select
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_load
 ENDIF
 IF ((txtfnd_scripts->cnt=0))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No rows found in DM_TEXT_FIND_DETAIL for for data in REQUEST structure."
  CALL dst_add_log_det(daf_icd9_request->parent_log_id,"ERROR",concat(
    "Getting DM_TEXT_FIND_DETAIL info: ",dm_err->emsg),0.0,0.0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_load
 ENDIF
 FOR (dtla_loop = 1 TO txtfnd_scripts->cnt)
   SET dtla_detail_id = txtfnd_scripts->qual[dtla_loop].det_id
   ...
   SET dtg->cnt = 0
   SET stat = alterlist(dtg->qual,0)
   SET stat = alterlist(script_rec->long_qual,0)
   IF (checkprg(txtfnd_scripts->qual[dtla_loop].script_name) > 0)
    SET dtla_start_dt_tm = cnvtdatetime(curdate,curtime3)
    CALL parser(concat("execute ",txtfnd_scripts->qual[dtla_loop].script_name," go"),1)
    SET dtla_end_dt_tm = cnvtdatetime(curdate,curtime3)
    IF (check_error(dm_err->eproc)=1)
     SET dtla_error_ind = 1
     SET dtla_error_msg = dm_err->emsg
     CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,dtla_start_dt_tm,
      dtla_end_dt_tm)
     SET dm_err->err_ind = 0
    ELSE
     CALL dst_add_log_det(script_rec->dm_text_find_log_id,"SUCCESS",concat("The ",txtfnd_scripts->
       qual[dtla_loop].script_name," script run for DM_TEXT_FIND_DETAIL_ID of ",trim(cnvtstring(
         dtla_detail_id,20,1))," was completed successfully."),dtla_start_dt_tm,dtla_end_dt_tm)
    ENDIF
   ELSE
    SET dtla_error_ind = 1
    SET dtla_error_msg = concat("The ",txtfnd_scripts->qual[dtla_loop].script_name,
     " script does not exist in the domain.")
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dtla_error_msg,dtla_start_dt_tm,
     dtla_end_dt_tm)
   ENDIF
 ENDFOR
#exit_load
 IF (dtla_error_ind=1)
  SET dm_err->err_ind = dtla_error_ind
  SET dm_err->emsg = dtla_error_msg
 ENDIF
 IF ((dm_err->err_ind=1))
 ...
 ELSE
  ...
  SET daf_icd9_reply->message = "DM_TXTFND_LOAD_DET completed without error."
 ENDIF
 SET dm_err->eproc = "Ending DM_TXTFND_LOAD_DET"
 CALL final_disp_msg("dm_txtfnd_load_det")
END GO
