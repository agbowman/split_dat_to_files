CREATE PROGRAM dcp_solcap_kia_prefs:dba
 SET modify = predeclare
 DECLARE addpreference(capability_identifier=vc,section=vc,section_id=vc,subgroup_path=vc,pref_entry=
  vc,
  restrict_default=i1,restrict_facility=i1,restrict_position=i1) = i1
 DECLARE addbatchloadpath(section=vc,section_id=vc) = i4
 DECLARE batchloadpreferencesforcontext(contextcd=f8,contextname=vc) = i1
 DECLARE retrievepreference(preference_idx=i4) = vc
 DECLARE getpreferencevalue(hgroup=i4,entry=vc) = vc
 DECLARE getsubgroupbyname(hgroup=i4,subgroupname=vc) = i4
 DECLARE interpretpreferencevalue(pref_entry=vc,pref_value=vc,value_num=i4(ref),value_str=vc(ref)) =
 i1
 DECLARE stat = i2 WITH protect, noconstant(0)
 DECLARE return_status = i1 WITH private, noconstant(0)
 DECLARE num_prefs = i4 WITH private, constant(1)
 ...
 DECLARE shx_pref_name = vc WITH protect, constant("document consolidated form")
 SET return_status = addpreference("2010.2.00098.3","component","Social History","",shx_pref_name,
  1,1,0)
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   batch_idx = 0, pref_idx = 0, pref_value = fillstring(255," "),
   value_num = 0, value_str = fillstring(255," ")
  HEAD cv.code_value
   return_status = batchloadpreferencesforcontext(cv.code_value,"facility"), res_count = (res_count+
   1)
  DETAIL
   IF (return_status=1)
    ...
    ENDIF
    pref_value = retrievepreference(d.seq)
    IF (interpretpreferencevalue(pref_list->list[d.seq].pref_entry,pref_value,value_num,value_str))
     pref_list->list[d.seq].active_cnt = (pref_list->list[d.seq].active_cnt+ 1)
    ENDIF
    reply->solcap[d.seq].facility[res_count].display = cv.display, reply->solcap[d.seq].facility[
    res_count].value_num = value_num, reply->solcap[d.seq].facility[res_count].value_str = value_str
   ENDIF
  FOOT  cv.code_value
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   value_num = 0, value_str = fillstring(255," ")
  HEAD cv.code_value
   return_status = batchloadpreferencesforcontext(cv.code_value,"position"), res_count = (res_count+
   1)
  DETAIL
   IF (return_status=1)
    IF (mod(res_count,10)=1)
    ...
    res_count].value_num = value_num, reply->solcap[d.seq].position[res_count].value_str = value_str
   ENDIF
  FOOT  cv.code_value
   FOR (batch_idx = 1 TO batch_list_cnt)
     IF ((batch_load_path_list->list[batch_idx].pref_handle != 0))
 ...
 FOR (pref_idx = 1 TO pref_cnt)
   ...
   ENDIF
 ENDFOR
 SUBROUTINE addpreference(capability_identifier,section,section_id,subgroup_path,pref_entry,
  restrict_default,restrict_facility,restrict_position)
   ...
   WHILE (search_idx != 0)
    ...
    IF (search_idx > 0)
     SET subgroup_cnt = (subgroup_cnt+ 1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE addbatchloadpath(section,section_id)
   ...
   FOR (idx_cnt = 1 TO path_list_size)
     ...
     ENDIF
   ENDFOR
   SET batch_list_cnt = (batch_list_cnt+ 1)
 ...
 END ;Subroutine
 SUBROUTINE batchloadpreferencesforcontext(contextcd,contextname)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE retrievepreference(preference_idx)
 ...
 SUBROUTINE getpreferencevalue(hgroup,entry)
   ...
   RETURN("Unknown")
 END ;Subroutine
 SUBROUTINE getsubgroupbyname(hgroup,subgrouptofind)
   DECLARE entrycnt = i4 WITH private, noconstant(0)
   ...
   RETURN(0)
 END ;Subroutine
 SUBROUTINE interpretpreferencevalue(pref_entry,pref_value,value_num,value_str)
   SET value_num = 0
   SET value_str = fillstring(255," ")
   IF (pref_entry=shx_pref_name)
    IF (pref_value="1")
     SET value_num = 1
     SET value_str = "ON"
     RETURN(1)
 ...
 SET script_version = "MOD 000 AY015871 12/03/10"
END GO
