CREATE PROGRAM acm_act_encntr_code_value_r:dba
 ...
 FOR (index = 1 TO xref->act_cnt)
   SET reply->encntr_code_value_r_qual[xref->act[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->act_cnt)),
  ...
  SET e.active_ind = xref->act[d.seq].active_ind, e.active_status_cd =
   ...
   e.active_status_dt_tm = cnvtdatetime(sysdate), e.updt_cnt = (e.updt_cnt+ 1), e.updt_dt_tm =
   cnvtdatetime(sysdate),
   e.updt_id = reqinfo->updt_id, e.updt_applctx = reqinfo->updt_applctx, e.updt_task = reqinfo->
   updt_task
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_code_value_r_id=acm_request->encntr_code_value_r_qual[xref->act[d.seq].idx].
   encntr_code_value_r_id))
  WITH nocounter, status(reply->encntr_code_value_r_qual[xref->act[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->act_cnt)
   IF ((reply->encntr_code_value_r_qual[xref->act[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->encntr_code_value_r_qual[xref->act[index].idx].status=0))
