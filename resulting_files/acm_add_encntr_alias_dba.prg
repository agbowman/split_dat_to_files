CREATE PROGRAM acm_add_encntr_alias:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->encntr_alias_qual[xref->add[index].idx].encntr_alias_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(encounter_seq,nextval)
     FROM dual
     DETAIL
      acm_request->encntr_alias_qual[xref->add[index].idx].encntr_alias_id = cnvtreal(nextseqnum),
      reply->encntr_alias_qual[xref->add[index].idx].encntr_alias_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->encntr_alias_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->encntr_alias_qual[xref->add[index].idx].encntr_alias_id = acm_request->
    encntr_alias_qual[xref->add[index].idx].encntr_alias_id
   ENDIF
   SET reply->encntr_alias_qual[xref->add[index].idx].status = 0
   IF ((acm_request->encntr_alias_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->encntr_alias_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->encntr_alias_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->encntr_alias_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   encntr_alias e
  SET e.encntr_alias_id = acm_request->encntr_alias_qual[xref->add[d.seq].idx].encntr_alias_id, e
   .alias = cnvtupper(cnvtalphanum(acm_request->encntr_alias_qual[xref->add[d.seq].idx].alias)), e
   .alias_pool_cd = acm_request->encntr_alias_qual[xref->add[d.seq].idx].alias_pool_cd,
   e.beg_effective_dt_tm = cnvtdatetime(acm_request->encntr_alias_qual[xref->add[d.seq].idx].
    beg_effective_dt_tm), e.check_digit = acm_request->encntr_alias_qual[xref->add[d.seq].idx].
   check_digit, e.check_digit_method_cd = acm_request->encntr_alias_qual[xref->add[d.seq].idx].
   check_digit_method_cd,
   e.contributor_system_cd =
   IF ((acm_request->encntr_alias_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->encntr_alias_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , e.encntr_alias_sub_type_cd = acm_request->encntr_alias_qual[xref->add[d.seq].idx].
   encntr_alias_sub_type_cd, e.encntr_alias_type_cd = acm_request->encntr_alias_qual[xref->add[d.seq]
   .idx].encntr_alias_type_cd,
   e.encntr_id = acm_request->encntr_alias_qual[xref->add[d.seq].idx].encntr_id, e
   .end_effective_dt_tm = cnvtdatetime(acm_request->encntr_alias_qual[xref->add[d.seq].idx].
    end_effective_dt_tm), e.data_status_cd =
   IF ((acm_request->encntr_alias_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request->
    encntr_alias_qual[xref->add[d.seq].idx].data_status_cd
   ELSE reqdata->data_status_cd
   ENDIF
   ,
   e.data_status_dt_tm = cnvtdatetime(curdate,curtime3), e.data_status_prsnl_id = reqinfo->updt_id, e
   .active_ind =
   IF ((((acm_request->encntr_alias_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((
   acm_request->encntr_alias_qual[xref->add[d.seq].idx].active_status_cd=reqdata->active_status_cd)
   )) ) 1
   ELSE 0
   ENDIF
   ,
   e.active_status_cd =
   IF ((acm_request->encntr_alias_qual[xref->add[d.seq].idx].active_status_cd > 0.0)) acm_request->
    encntr_alias_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (e)
  WITH nocounter, status(reply->encntr_alias_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->encntr_alias_qual[xref->add[index].idx].status=0))
 ...
 ENDFOR
 IF (acm_hist_ind=1)
  EXECUTE acm_add_encntr_alias_hist
