CREATE PROGRAM acm_add_prsnl_reltn_child:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->prsnl_reltn_child_qual[xref->add[index].idx].prsnl_reltn_child_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(person_only_seq,nextval)
     FROM dual
     DETAIL
      acm_request->prsnl_reltn_child_qual[xref->add[index].idx].prsnl_reltn_child_id = cnvtreal(
       nextseqnum), reply->prsnl_reltn_child_qual[xref->add[index].idx].prsnl_reltn_child_id =
      cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->prsnl_reltn_child_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->prsnl_reltn_child_qual[xref->add[index].idx].prsnl_reltn_child_id = acm_request->
    prsnl_reltn_child_qual[xref->add[index].idx].prsnl_reltn_child_id
   ENDIF
   SET reply->prsnl_reltn_child_qual[xref->add[index].idx].status = 0
   IF ((acm_request->prsnl_reltn_child_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->prsnl_reltn_child_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->prsnl_reltn_child_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->prsnl_reltn_child_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   prsnl_reltn_child p
  SET p.prsnl_reltn_child_id = acm_request->prsnl_reltn_child_qual[xref->add[d.seq].idx].
   prsnl_reltn_child_id, p.prsnl_reltn_id =
   IF ((acm_request->prsnl_reltn_child_qual[xref->add[d.seq].idx].prsnl_reltn_id > 0)) acm_request->
    prsnl_reltn_child_qual[xref->add[d.seq].idx].prsnl_reltn_id
   ELSE reply->prsnl_reltn_qual[acm_request->prsnl_reltn_child_qual[xref->add[d.seq].idx].
    prsnl_reltn_idx].prsnl_reltn_id
   ENDIF
   , p.parent_entity_id = acm_request->prsnl_reltn_child_qual[xref->add[d.seq].idx].parent_entity_id,
   p.parent_entity_name = acm_request->prsnl_reltn_child_qual[xref->add[d.seq].idx].
   parent_entity_name, p.display_seq = acm_request->prsnl_reltn_child_qual[xref->add[d.seq].idx].
   display_seq, p.beg_effective_dt_tm = cnvtdatetime(acm_request->prsnl_reltn_child_qual[xref->add[d
    .seq].idx].beg_effective_dt_tm),
   p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_reltn_child_qual[xref->add[d.seq].idx].
    end_effective_dt_tm), p.updt_cnt = 0, p.updt_dt_tm = cnvtdatetime(curdate,curtime3),
   p.updt_id = reqinfo->updt_id, p.updt_applctx = reqinfo->updt_applctx, p.updt_task = reqinfo->
   updt_task
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->prsnl_reltn_child_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->prsnl_reltn_child_qual[xref->add[index].idx].status=0))
