CREATE PROGRAM bhs_ma_daily_diabetes_1:dba
 ...
 DECLARE start_id = f8 WITH protect, noconstant(484904849.00)
 DECLARE last_id = f8
 SET last_id = (start_id+ increment)
 DECLARE end_id = f8 WITH protect, noconstant(493296443.00)
 DECLARE done = i2
 WHILE (done=0)
   CALL echo(start_id)
   CALL echo(last_id)
     ...
     IF (result > 200)
      ...
      ELSE
       idx1 = locateval(indx,1,t_record->person_cnt,ce.person_id,t_record->person_qual[indx].
        person_id)
       IF (idx1=0)
        t_record->person_cnt = (t_record->person_cnt+ 1), idx = t_record->person_cnt, stat =
        alterlist(t_record->person_qual,idx),
        t_record->person_qual[idx].person_id = ce.person_id
       ENDIF
      ENDIF
     ENDIF
