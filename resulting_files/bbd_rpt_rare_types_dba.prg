CREATE PROGRAM bbd_rpt_rare_types:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 RECORD captions(
   ...
   1 rare_types = vc
   1 rpt_as_of_date = vc
   1 rare_types_disp = vc
   1 rare_types_desc = vc
   1 effective = vc
   1 end_effective = vc
   1 active = vc
   1 yes = vc
 ...
 SET captions->rpt_title = uar_i18ngetmessage(i18nhandle,"rpt_title",
  "R A R E   D O N O R   T Y P E S   R E P O R T")
 SET captions->rpt_time = uar_i18ngetmessage(i18nhandle,"rpt_time","Time:")
 SET captions->rare_types = uar_i18ngetmessage(i18nhandle,"rare_types","(Rare Types)")
 SET captions->rpt_as_of_date = uar_i18ngetmessage(i18nhandle,"rpt_as_of_date","As of Date:")
 SET captions->rare_types_disp = uar_i18ngetmessage(i18nhandle,"rare_types_disp","Rare Types Display"
  )
 SET captions->rare_types_desc = uar_i18ngetmessage(i18nhandle,"rare_types_desc",
  "Rare Types Description")
 ...
 SET captions->rpt_id = uar_i18ngetmessage(i18nhandle,"rpt_id","Report ID: BBD_RPT_RARE_TYPES")
 ...
 SELECT INTO "cer_temp:bbdraretypes.txt"
  c.*
  FROM code_value c
  PLAN (c
   WHERE c.code_set=14235
    AND c.code_value > 0)
  ...
  HEAD PAGE
   ...
   CALL center(captions->rare_types,1,125),
   col 104, captions->rpt_as_of_date, col 118,
   curdate"@DATECONDENSED;;d", row + 2, col 3,
   captions->rare_types_disp, col 27, captions->rare_types_desc,
