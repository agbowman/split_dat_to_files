CREATE PROGRAM dcp_get_filtered_org:dba
 RECORD reply(
   1 organization[*]
 ...
 DECLARE encntr_org_sec_ind = i2 WITH noconstant(0), public
 DECLARE flag = i2 WITH noconstant(0)
 DECLARE ncnt = i2 WITH noconstant(0), public
 ...
 END ;Subroutine
 CALL examineorgsecurity(encntr_org_sec_ind)
 CALL echo(build("encntr_org_sec_ind = ",encntr_org_sec_ind))
 IF (encntr_org_sec_ind=0)
  SELECT INTO "nl:"
   o.organization_id, o.org_name
   FROM organization o
   WHERE  NOT (o.organization_id=0)
    AND o.active_ind=1
   ORDER BY o.org_name
   HEAD REPORT
    ncnt = 0
   DETAIL
    CALL processdetail(o.organization_id)
   FOOT REPORT
 ...
 ELSE
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    JOIN (o
    WHERE expand(num,expand_start,expand_stop,o.organization_id,sac_org->organizations[num].
     organization_id)
     AND  NOT (o.organization_id=0)
     AND o.active_ind=1)
 ...
 ENDIF
 GO TO exit_script
 SUBROUTINE processdetail(organization_id)
   SET ncnt += 1
   IF (ncnt > size(reply->organization,5))
    SET stat = alterlist(reply->organization,(ncnt+ 10))
   ENDIF
   SET reply->organization[ncnt].organization_id = o.organization_id
   SET reply->organization[ncnt].organization_name = o.org_name
 END ;Subroutine
#exit_script
 SET reply->status_data.subeventstatus.operationname = "Get organizations"
 SET reply->status_data.subeventstatus.targetobjectname = "Table: ORGANIZATION"
 SET reply->status_data.subeventstatus.targetobjectvalue = "dcp_get_filtered_org.prg"
