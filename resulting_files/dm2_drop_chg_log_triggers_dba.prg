CREATE PROGRAM dm2_drop_chg_log_triggers:dba
 ...
 ENDIF
 DECLARE dcl_table_name = vc
 DECLARE dcl_log_type = vc
 DECLARE d_tbl_cnt = i4
 DECLARE drop_text = vc
 DECLARE dcl_cnt = i4
 DECLARE trig_func = vc
 DECLARE hold_trig = vc
 DECLARE hold_point = i4
 DECLARE dc_temp_envid = i4
 DECLARE dc_trig_cnt = i4
 DECLARE dc_tab_cnt = i4
 DECLARE drop_trig_date = f8
 DECLARE trig_log_type = vc
 DECLARE dcl_tab_cnt = i4
 DECLARE dcl_src_id = f8
 DECLARE dcl_temp_tab = vc
 DECLARE dcl_num = i4
 DECLARE dcl_r_event = vc
 DECLARE dcl_ev_reason = vc
 DECLARE dcl_pos = i4
 DECLARE dcl_loop = i4
 DECLARE dcl_filter_func = vc
 DECLARE dcl_old_pkw_func = vc
 DECLARE dcl_pkw_func = vc
 DECLARE dcl_del_pkw_func = vc
 DECLARE dcl_colstring_func = vc
 DECLARE dcl_colstring_gen_func = vc
 DECLARE dcl_ptam_match_func = vc
 DECLARE dcl_ptam_query_func = vc
 DECLARE dcl_func_cnt = i4
 DECLARE dcl_parse_loop = i4 WITH protect, noconstant(0)
 DECLARE dcl_parse_func_loop = i4 WITH protect, noconstant(0)
 DECLARE dcl_retry_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcl_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dcl_err_msg = vc WITH protect, noconstant("")
 DECLARE dcl_table_suffix = vc WITH protect, noconstant("")
 DECLARE dcl_table_loc = i4 WITH protect, noconstant(0)
 DECLARE dcl_ndx = i4 WITH protect, noconstant(0)
 DECLARE dcl_ndx2 = i4 WITH protect, noconstant(0)
 DECLARE hold_point_end = i4 WITH protect, noconstant(0)
 DECLARE dcl_saved_err = i2 WITH protect, noconstant(0)
 DECLARE dcl_retry_max = i2 WITH protect, constant(5)
 DECLARE dcl_table_suffix_len = i2 WITH protect, constant(4)
 DECLARE dcl_parent_table = vc WITH protect, noconstant(" ")
 DECLARE dcl_child_table = vc WITH protect, noconstant(" ")
 DECLARE dcl_temp_str = vc WITH protect, noconstant(" ")
 DECLARE dcl_child_loc = i4 WITH protect, noconstant(0)
 SET dc_tab_cnt = 0
 SET drop_trig_date = 0
 SET dc_trig_cnt = 0
 SET drop_trig_date = cnvtdatetime(curdate,curtime3)
 IF (validate(trg_drop_reason,"X")="X")
  DECLARE trg_drop_reason = vc
  SET trg_drop_reason = "Manual regeneration via DROP script"
 ENDIF
 FREE RECORD drop_stmt
 RECORD drop_stmt(
   1 qual[*]
     2 stmt = vc
     2 d_tab_name = vc
 )
 FREE RECORD dcl_trig_info
 RECORD dcl_trig_info(
   1 qual[*]
     2 table_name = vc
     2 trigger_count = i4
     2 table_suffix = vc
 )
 FREE RECORD drop_obj
 RECORD drop_obj(
   1 stmt[*]
     2 str = vc
 )
 FREE RECORD dcl_drop_func
 RECORD dcl_drop_func(
   1 table_cnt = i4
   1 table_qual[*]
     2 table_suffix = vc
     2 table_name = vc
 ...
 FREE RECORD dcl_cur_state
 RECORD dcl_cur_state(
   1 child_cnt = i4
   1 qual[*]
     2 child_table = vc
     2 parent_cnt = i4
 ...
 SET stat = alterlist(drop_stmt->qual,0)
 SET stat = alterlist(dcl_trig_info->qual,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="DM_ENV_ID"
  DETAIL
   dcl_src_id = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (check_logfile("dm2_refdrop_trig_",".log","DM2CHG_DROP LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (cnvtupper( $2) != "REFCHG")
  SET dm_err->emsg = concat("******TRIGGER PREFIX MUST BE 'REFCHG'******")
  SET dm_err->eproc = concat("ATTEMPTED TRIGGER PREFIX WAS:", $2)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSE
  SET dcl_log_type = cnvtupper( $2)
  SET dcl_table_name = cnvtupper( $1)
 ENDIF
 SET dm_err->eproc = "Starting dm2_drop_chg_log_triggers..."
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Building list of triggers to be dropped..."
 CALL disp_msg("",dm_err->logfile,0)
 SET trig_log_type = dcl_log_type
 SET dcl_log_type = concat(dcl_log_type,"*")
 SELECT INTO "nl:"
  FROM dm2_user_triggers a
  WHERE a.trigger_name=patstring(dcl_log_type)
   AND a.trigger_name != patstring(concat(dcl_log_type,"MC"))
   AND a.table_name=patstring(dcl_table_name)
  ORDER BY a.table_name
  HEAD REPORT
   drop_cnt = 0, stat = alterlist(drop_stmt->qual,100)
  HEAD a.table_name
   dc_tab_cnt = (dc_tab_cnt+ 1)
  DETAIL
   drop_cnt = (drop_cnt+ 1)
   IF (mod(drop_cnt,100)=1)
    stat = alterlist(drop_stmt->qual,(drop_cnt+ 99))
   ENDIF
   drop_stmt->qual[drop_cnt].stmt = concat("rdb drop trigger ",a.trigger_name," go"), drop_stmt->
   qual[drop_cnt].d_tab_name = a.table_name
  FOOT REPORT
   stat = alterlist(drop_stmt->qual,drop_cnt)
  WITH nocounter
 ;end select
 IF (check_error(concat("Error building list of triggers...")) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Executing trigger statements..."
 CALL disp_msg("",dm_err->logfile,0)
 FOR (pcnt = 1 TO value(size(drop_stmt->qual,5)))
   SET hold_point = findstring("REFCHG",drop_stmt->qual[pcnt].stmt)
   SET hold_point_end = findstring("GO",cnvtupper(drop_stmt->qual[pcnt].stmt))
   SET hold_trig = substring(hold_point,(hold_point_end - hold_point),drop_stmt->qual[pcnt].stmt)
   CALL echo(concat("Dropping trigger....",hold_trig))
   SET stat = initrec(drop_obj)
   SET stat = alterlist(drop_obj->stmt,1)
   SET drop_obj->stmt[1].str = drop_stmt->qual[pcnt].stmt
   SET dcl_retry_cnt = 0
   SET dm_err->err_ind = 0
   WHILE (dcl_retry_cnt < dcl_retry_max)
    EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DROP_OBJ")
    IF (check_error(concat(" Trigger_Build: Error in dropping trigger ",hold_trig)) != 0)
     IF (findstring("ORA-00054",dm_err->emsg))
      SET dm_err->user_action =
      "Retrying drop trigger command due to resource busy, error may be ignored."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dm_err->user_action = ""
      SET dcl_retry_cnt = (dcl_retry_cnt+ 1)
     ELSE
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dm_err->err_ind = 1
      SET dcl_retry_cnt = dcl_retry_max
     ENDIF
     SET dcl_err_ind = dm_err->err_ind
     SET (dm_err->err - ind) = 0
     SET dcl_err_msg = dm_err->emsg
    ELSE
     SET dcl_retry_cnt = dcl_retry_max
     SET dcl_temp_tab = drop_stmt->qual[pcnt].d_tab_name
     SET dcl_tab_cnt = size(dcl_trig_info->qual,5)
     SET dcl_tab_found = locateval(dcl_num,1,dcl_tab_cnt,dcl_temp_tab,dcl_trig_info->qual[dcl_num].
      table_name)
     IF (dcl_tab_found > 0)
      SET dcl_trig_info->qual[dcl_tab_found].trigger_count = (dcl_trig_info->qual[dcl_tab_found].
      trigger_count+ 1)
     ELSE
      SET dcl_tab_cnt = (size(dcl_trig_info->qual,5)+ 1)
      SET stat = alterlist(dcl_trig_info->qual,dcl_tab_cnt)
      SET dcl_trig_info->qual[dcl_tab_cnt].table_name = dcl_temp_tab
      SET dcl_trig_info->qual[dcl_tab_cnt].trigger_count = 1
     ENDIF
    ENDIF
   ENDWHILE
 ENDFOR
 SET dm_err->eproc = "Building list of functions to drop..."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM user_objects uo
  WHERE uo.object_type="FUNCTION"
   AND ((uo.object_name="REFCHG_FILTER*") OR (((uo.object_name="REFCHG_PK_WHERE_*") OR (((uo
  .object_name="REFCHG_PKW*") OR (((uo.object_name="REFCHG_DEL_PKW*") OR (((uo.object_name=
  "REFCHG_COLSTRING*") OR (((uo.object_name="REFCHG_GEN_PK_PTAM*") OR (((uo.object_name=
  "REFCHG_PTAM_MATCH*") OR (uo.object_name="PTAM_MATCH_QUERY*")) )) )) )) )) )) ))
   AND uo.object_name != "REFCHG_COLSTRING_GET_COL"
  DETAIL
   dcl_table_suffix = ""
   IF (findstring("REFCHG_FILTER",uo.object_name) > 0)
    dcl_table_suffix = substring((size("REFCHG_FILTER_")+ 1),dcl_table_suffix_len,uo.object_name)
   ELSEIF (findstring("REFCHG_PK_WHERE",uo.object_name) > 0)
    dcl_table_suffix = substring((size("REFCHG_PK_WHERE_")+ 1),dcl_table_suffix_len,uo.object_name)
   ELSEIF (findstring("REFCHG_PKW",uo.object_name) > 0)
    dcl_table_suffix = substring((size("REFCHG_PKW_")+ 1),dcl_table_suffix_len,uo.object_name)
   ELSEIF (findstring("REFCHG_DEL_PKW",uo.object_name) > 0)
    dcl_table_suffix = substring((size("REFCHG_DEL_PKW_")+ 1),dcl_table_suffix_len,uo.object_name)
   ELSEIF (findstring("REFCHG_COLSTRING",uo.object_name) > 0)
    dcl_table_suffix = substring((size("REFCHG_COLSTRING_")+ 1),dcl_table_suffix_len,uo.object_name)
   ELSEIF (findstring("REFCHG_GEN_PK_PTAM",uo.object_name) > 0)
    dcl_table_suffix = substring((size("REFCHG_GEN_PK_PTAM_")+ 1),dcl_table_suffix_len,uo.object_name
     )
   ELSEIF (findstring("REFCHG_PTAM_MATCH",uo.object_name) > 0)
    dcl_table_suffix = substring((size("REFCHG_PTAM_MATCH_")+ 1),dcl_table_suffix_len,uo.object_name)
   ELSEIF (findstring("PTAM_MATCH_QUERY",uo.object_name) > 0)
    dcl_table_suffix = substring((size("PTAM_MATCH_QUERY_")+ 1),dcl_table_suffix_len,uo.object_name)
   ENDIF
   dcl_table_loc = locateval(dcl_ndx,1,size(dcl_drop_func->table_qual,5),dcl_table_suffix,
    dcl_drop_func->table_qual[dcl_ndx].table_suffix)
   IF (dcl_table_loc=0)
    dcl_drop_func->table_cnt = (dcl_drop_func->table_cnt+ 1), dcl_table_loc = dcl_drop_func->
    table_cnt
    IF (mod(dcl_drop_func->table_cnt,10)=1)
     stat = alterlist(dcl_drop_func->table_qual,(dcl_drop_func->table_cnt+ 9))
    ENDIF
    dcl_drop_func->table_qual[dcl_drop_func->table_cnt].table_suffix = dcl_table_suffix
   ENDIF
   dcl_drop_func->table_qual[dcl_table_loc].func_cnt = (dcl_drop_func->table_qual[dcl_table_loc].
   func_cnt+ 1), stat = alterlist(dcl_drop_func->table_qual[dcl_table_loc].func_qual,dcl_drop_func->
    table_qual[dcl_table_loc].func_cnt), dcl_drop_func->table_qual[dcl_table_loc].func_qual[
   dcl_drop_func->table_qual[dcl_table_loc].func_cnt].func_name = uo.object_name
  WITH nocounter
 ;end select
 SET stat = alterlist(dcl_drop_func->table_qual,dcl_drop_func->table_cnt)
 IF (check_error(concat("Error building list of functions to drop...")) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Retrieving table names..."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_tables_doc_local dtd
  WHERE dtd.full_table_name=dtd.table_name
   AND expand(dcl_ndx,1,dcl_drop_func->table_cnt,dtd.table_suffix,dcl_drop_func->table_qual[dcl_ndx].
   table_suffix)
  DETAIL
   dcl_table_loc = locateval(dcl_ndx,1,size(dcl_drop_func->table_qual,5),dtd.table_suffix,
    dcl_drop_func->table_qual[dcl_ndx].table_suffix)
   IF (dcl_table_loc > 0)
    dcl_drop_func->table_qual[dcl_table_loc].table_name = dtd.table_name
   ENDIF
  WITH nocounter, expand = 1
 ;end select
 IF (check_error(concat("Error retrieving table_names...")) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="RDDS CURRENT STATE TABLES"
  DETAIL
   dcl_temp_str = trim(di.info_name), dcl_pos = findstring(":",dcl_temp_str), dcl_child_table =
   substring(1,(dcl_pos - 1),dcl_temp_str),
   dcl_ndx = locateval(dcl_ndx,1,dcl_cur_state->child_cnt,dcl_child_table,dcl_cur_state->qual[dcl_ndx
    ].child_table)
   IF (dcl_ndx=0)
    dcl_cur_state->child_cnt = (dcl_cur_state->child_cnt+ 1), stat = alterlist(dcl_cur_state->qual,
     dcl_cur_state->child_cnt), dcl_cur_state->qual[dcl_cur_state->child_cnt].child_table =
    dcl_child_table,
    dcl_ndx = dcl_cur_state->child_cnt
   ENDIF
   dcl_pos = findstring(":",dcl_temp_str,1,1), dcl_parent_table = substring((dcl_pos+ 1),(size(
     dcl_temp_str,1) - dcl_pos),dcl_temp_str), dcl_cur_state->qual[dcl_ndx].parent_cnt = (
   dcl_cur_state->qual[dcl_ndx].parent_cnt+ 1),
   stat = alterlist(dcl_cur_state->qual[dcl_ndx].parent_qual,dcl_cur_state->qual[dcl_ndx].parent_cnt),
   dcl_cur_state->qual[dcl_ndx].parent_qual[dcl_cur_state->qual[dcl_ndx].parent_cnt].parent_table =
   dcl_parent_table
  WITH nocounter
 ;end select
 SET dm_err->eproc = "Validating REFCHG triggers exist for a given table..."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM user_triggers ut
  WHERE ut.trigger_name=patstring(dcl_log_type)
   AND ut.trigger_name != patstring(concat(dcl_log_type,"MC"))
   AND expand(dcl_ndx,1,size(dcl_drop_func->table_qual,5),ut.table_name,dcl_drop_func->table_qual[
   dcl_ndx].table_name)
  ORDER BY ut.table_name
  DETAIL
   dcl_table_loc = locateval(dcl_ndx,1,size(dcl_drop_func->table_qual,5),ut.table_name,dcl_drop_func
    ->table_qual[dcl_ndx].table_name)
   IF (dcl_table_loc > 0)
    dcl_drop_func->table_qual[dcl_table_loc].trig_ind = 1
   ENDIF
   dcl_child_loc = locateval(dcl_ndx,1,dcl_cur_state->child_cnt,ut.table_name,dcl_cur_state->qual[
    dcl_ndx].child_table)
   IF (dcl_child_loc > 0)
    FOR (dcl_ndx2 = 1 TO dcl_cur_state->qual[dcl_child_loc].parent_cnt)
     dcl_table_loc = locateval(dcl_ndx,1,size(dcl_drop_func->table_qual,5),dcl_cur_state->qual[
      dcl_child_loc].parent_qual[dcl_ndx2].parent_table,dcl_drop_func->table_qual[dcl_ndx].table_name
      ),
     IF (dcl_table_loc > 0)
      dcl_drop_func->table_qual[dcl_table_loc].trig_ind = 1
     ENDIF
  ...
  WITH nocounter, expand = 1
 ;end select
 IF (check_error(concat("Error validating REFCHG triggers exist for a given table...")) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 FOR (dcl_parse_loop = 1 TO dcl_drop_func->table_cnt)
   IF ((dcl_drop_func->table_qual[dcl_parse_loop].trig_ind=0))
    FOR (dcl_parse_func_loop = 1 TO dcl_drop_func->table_qual[dcl_parse_loop].func_cnt)
      SET stat = initrec(drop_obj)
      SET stat = alterlist(drop_obj->stmt,1)
      SET drop_obj->stmt[1].str = concat("rdb drop function ",dcl_drop_func->table_qual[
       dcl_parse_loop].func_qual[dcl_parse_func_loop].func_name," go")
      EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DROP_OBJ")
      IF (check_error(concat("Build: Error in dropping FUNCTION ",dcl_drop_func->table_qual[
        dcl_parse_loop].func_qual[dcl_parse_func_loop].func_name)))
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dcl_err_ind = dm_err->err_ind
       SET dm_err->err_ind = 0
       SET dcl_err_msg = dm_err->emsg
...
#exit_program
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dc_temp_envid = di.info_number
  WITH nocounter
 ;end select
 SET drop_trig_cnt = 0
 SET dc_trig_cnt = size(drop_stmt->qual,5)
 INSERT  FROM dm_chg_log d
  SET d.log_id = seq(dm_clinical_seq,nextval), d.target_env_id = dc_temp_envid, d.log_type = "DRPTRG",
   d.table_name = trig_log_type, d.chg_dt_tm = cnvtdatetime(drop_trig_date), d.updt_task = dc_tab_cnt,
   d.updt_cnt = dc_trig_cnt
  WITH nocounter
 ;end insert
 IF (size(derg_reply->child_env_list,5)=0)
  SET derg_request->env_id = dc_temp_envid
  SET derg_request->relationship_type = "REFERENCE MERGE"
  EXECUTE dm_get_env_reltn
 ENDIF
 IF (((size(trim(dcl_table_name),1) > 1) OR (ichar(dcl_table_name) != 42)) )
  SET dcl_r_event = "Subset of Triggers Dropped"
  SET dcl_ev_reason = "Manual drop of subset"
 ELSE
  SET dcl_r_event = "Drop Environment Triggers"
  SET dcl_ev_reason = trg_drop_reason
 ENDIF
 SET stat = alterlist(auto_ver_request->qual,size(derg_reply->child_env_list,5))
 FOR (i = 1 TO size(derg_reply->child_env_list,5))
   SET auto_ver_request->qual[i].rdds_event = dcl_r_event
   SET auto_ver_request->qual[i].cur_environment_id = dcl_src_id
   SET auto_ver_request->qual[i].paired_environment_id = derg_reply->child_env_list[i].env_id
   SET auto_ver_request->qual[i].event_reason = dcl_ev_reason
   SET stat = alterlist(auto_ver_request->qual[i].detail_qual,size(dcl_trig_info->qual,5))
   FOR (j = 1 TO size(dcl_trig_info->qual,5))
    SET auto_ver_request->qual[i].detail_qual[j].event_detail1_txt = dcl_trig_info->qual[j].
    table_name
    SET auto_ver_request->qual[i].detail_qual[j].event_value = dcl_trig_info->qual[j].trigger_count
   ENDFOR
 ENDFOR
 ...
 ELSE
  COMMIT
  SET dm_err->eproc = "...Ending dm2_drop_chg_log_triggers"
  CALL final_disp_msg("dm2_refchg_trig_")
 ENDIF
 IF (dcl_err_ind=1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = dcl_err_msg
 ENDIF
