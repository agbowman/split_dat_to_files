CREATE PROGRAM dm2_execute_schema:dba
 ...
 SET des_rs->schema_date = 0
 DECLARE des_sch_date_str = vc WITH protect, noconstant(" ")
 DECLARE des_priority_grp = vc WITH protect, noconstant(" ")
 DECLARE des_idx = i4 WITH protect, noconstant(0)
 DECLARE des_priority_from = i4 WITH protect, noconstant(0)
 DECLARE des_priority_to = i4 WITH protect, noconstant(0)
 DECLARE des_src_dbase_name = vc WITH protect, noconstant("")
 DECLARE des_dbase_name = vc WITH protect, noconstant("")
 DECLARE des_grp_cnt = i4 WITH protect, noconstant(0)
 DECLARE des_ckpt_ret = vc WITH protect, noconstant("")
 DECLARE des_standalone = i4 WITH protect, noconstant(0)
 DECLARE des_continue = i2 WITH protect, noconstant(1)
 DECLARE des_check_restart(null) = i2
 DECLARE des_checkpoint_prompt(dc_ckpt_type=vc,dc_choice_ret=vc(ref)) = i2
 IF ((dm2_install_schema->curprog="NONE"))
  IF (validate(dm2_troubleshoot_replicate,1)=1
   AND validate(dm2_troubleshoot_replicate,2)=2)
   SET dm_err->logfile = "DM2NOTSET"
   SET dm_err->eproc = "Validate standalone status."
   SET dm_err->emsg = "DM2_EXECUTE_SCHEMA may not be run Standalone"
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ELSE
   SET des_standalone = 1
  ENDIF
 ENDIF
 IF (check_logfile("dm2_exec_schema",".log","DM2_EXECUTE_SCHEMA LOGFILE")=0)
  GO TO exit_program
 ENDIF
 IF (des_standalone=1)
  SET drr_clin_copy_data->checkpoint_ind = 1
 ...
 CALL disp_msg("",dm_err->logfile,0)
 SET des_sch_date_str = trim( $1)
 IF (format(cnvtdate(cnvtalphanum(des_sch_date_str)),"MM-DD-YYYY;;D") != des_sch_date_str)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Invalid schema date entered - ",des_sch_date_str)
  SET dm_err->eproc = "Schema Date Validation"
  SET dm_err->user_action = "Enter schema date in MM-DD-YYYY format"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET des_rs->schema_date = cnvtdate(cnvtint(build(substring(1,8,cnvtalphanum(des_sch_date_str)))))
 SET des_priority_grp = cnvtupper(trim( $2))
 SET des_idx = locateval(des_idx,1,drr_priority_group_matrix->cnt,des_priority_grp,
  drr_priority_group_matrix->priority_group[des_idx].group_name)
 IF (des_idx=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Priority group ",des_priority_grp," is invalid.")
  SET dm_err->eproc = "Priority Group Validation"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ELSE
  SET des_priority_from = drr_priority_group_matrix->priority_group[des_idx].priority_from_range
  SET des_priority_to = drr_priority_group_matrix->priority_group[des_idx].priority_to_range
 ENDIF
 IF (des_standalone=1)
  IF (trim(currdbhandle,3) > " ")
   IF (cnvtupper(currdbuser)="V500")
    IF (dm2_get_dbase_name(des_dbase_name)=0)
     RETURN(0)
    ELSEIF (des_dbase_name=" ")
    ...
    ELSE
     ...
     SET dm2_install_schema->src_v500_connect_str = dm2_install_schema->connect_str
    ENDIF
    IF (dm2_get_dbase_name(des_src_dbase_name)=0)
     GO TO exit_program
    ELSEIF (des_src_dbase_name=" ")
     SET dm_err->emsg = "Source database name could not be obtained"
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    SET dm2_install_schema->dbase_name = des_dbase_name
    SET dm2_install_schema->u_name = "V500"
    SET dm2_force_connect_string = 1
    EXECUTE dm2_connect_to_dbase "PC"
    SET dm2_force_connect_string = 0
    ...
    ELSEIF (des_dbase_name=" ")
     ...
     RETURN(0)
    ENDIF
    IF (des_src_dbase_name=des_dbase_name
     AND (validate(dm2_allow_same_db_name,- (1))=- (1)))
     SET dm_err->emsg = concat("Source:",des_src_dbase_name," and Target:",des_dbase_name,
      " databases may not be the same.")
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     GO TO exit_program
    ELSE
     SET dm_err->eproc = concat("Source:",des_src_dbase_name," and Target:",des_dbase_name)
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
   ...
   ELSE
    ...
    SET dm_err->user_action = concat("Please connect to database as V500. ")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    GO TO exit_program
   ENDIF
  ELSE
   ...
   SET dm_err->user_action = concat("Please connect to database V500. ")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
 ...
 IF ((dm_err->debug_flag > 0))
  CALL echo(dm_err->asterisk_line)
  CALL echo(build("des::reqinfo->updt_task=",reqinfo->updt_task))
  CALL echo(dm_err->asterisk_line)
 ENDIF
 SET dm_err->eproc = "Starting DM2_EXECUTE_SCHEMA process..."
 CALL disp_msg("",dm_err->logfile,0)
 IF (des_standalone=1)
  SET dm_err->eproc = "Get highest CLIN COPY run_id for schema date passed in"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM dm2_ddl_ops d
   WHERE d.schema_date=cnvtdate(cnvtalphanum(des_sch_date_str))
   DETAIL
  ...
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ELSEIF (((curqual=0) OR ((dm2_install_schema->run_id=0))) )
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("No run information found for schema date ",des_sch_date_str)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc =
 "Checking success of DDL operations that need to be complete prior to this run..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm2_ddl_ops_log ddol
  WHERE (ddol.run_id=dm2_install_schema->run_id)
   AND ddol.priority < des_priority_from
   AND ((ddol.status = null) OR (ddol.status IN ("RUNNING", "ERROR")))
 ...
 ELSEIF (curqual > 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Cannot proceed with this run until prior runs have completed successfully."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Check if process is in restart mode and perform ddl cleanup if necessary."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (des_check_restart(null)=0)
  GO TO exit_program
 ENDIF
 IF (((des_standalone=0) OR (des_standalone=1
  AND des_priority_grp != "ALL DDL")) )
  WHILE (des_continue=1)
    EXECUTE dm2_set_ready_to_run des_priority_grp
    IF ((dm_err->err_ind=1))
     GO TO exit_program
    ENDIF
    IF ((dir_install_misc->ddl_failed_ind=1))
     SET dm_err->eproc = "Check for failed import/export operations"
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     SELECT INTO "nl:"
      FROM dm2_ddl_ops_log d
      WHERE (d.run_id=dm2_install_schema->run_id)
       AND d.priority >= des_priority_from
       AND d.priority <= des_priority_to
       AND d.op_type IN ("IMPORT*", "EXPORT*")
       AND ((d.status="ERROR"
       AND d.error_msg = null) OR (d.status="ERROR"
       AND substring(1,14,d.error_msg) != "Application Id"))
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     ENDIF
     IF (curqual > 0)
      EXECUTE dm2_retry_imp_exp des_priority_grp
      IF ((dm_err->err_ind=1))
       GO TO exit_program
      ENDIF
     ELSE
      SET dm_err->err_ind = 1
      SET dm_err->eproc = "Execute_DDL_Command"
      SET dm_err->emsg = "One or more errors were encountered executing DDL"
      IF ((drr_clin_copy_data->process="MIGRATION"))
       SET dm_err->user_action =
       "To view all errors, please execute DM2_MIG_REPLICATE_MONITOR->View all Failed Schema Operations. "
      ELSE
       SET dm_err->user_action = concat(
        "To view all errors, please execute DM2_DOMAIN_MAINT->Replicate/Refresh ",
        "a Domain->Monitor Copy of Clinical Database->View all Failed Schema Operations. ")
      ENDIF
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
  ...
  ENDWHILE
 ELSE
  FOR (des_grp_cnt = 1 TO drr_priority_group_matrix->cnt)
    IF ((drr_priority_group_matrix->priority_group[des_grp_cnt].group_name != "ALL DDL"))
     SET dm_err->eproc = concat("Check for operations for priorities from ",build(
       drr_priority_group_matrix->priority_group[des_grp_cnt].priority_from_range)," to ",build(
       drr_priority_group_matrix->priority_group[des_grp_cnt].priority_to_range))
     CALL disp_msg("",dm_err->logfile,0)
     SELECT INTO "nl:"
      FROM dm2_ddl_ops_log d
      WHERE (d.run_id=dm2_install_schema->run_id)
       AND (d.priority >= drr_priority_group_matrix->priority_group[des_grp_cnt].priority_from_range)
       AND (d.priority <= drr_priority_group_matrix->priority_group[des_grp_cnt].priority_to_range)
       AND d.status = null
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
     ...
     ENDIF
     IF (curqual > 0)
      SET dm_err->eproc = "Checkpoint Prompt"
      IF (des_checkpoint_prompt(drr_priority_group_matrix->priority_group[des_grp_cnt].group_name,
       des_ckpt_ret)=0)
       GO TO exit_program
      ENDIF
      SET des_continue = 1
      IF (des_ckpt_ret="C")
       WHILE (des_continue=1)
         EXECUTE dm2_set_ready_to_run drr_priority_group_matrix->priority_group[des_grp_cnt].
         group_name
         IF ((dm_err->err_ind=1))
          GO TO exit_program
         ENDIF
         IF ((dir_install_misc->ddl_failed_ind=1))
          IF ((drr_priority_group_matrix->priority_group[des_grp_cnt].group_name IN ("IMPORT*",
          "EXPORT*")))
           EXECUTE dm2_retry_imp_exp drr_priority_group_matrix->priority_group[des_grp_cnt].
           group_name
           IF ((dm_err->err_ind=1))
            GO TO exit_program
           ENDIF
      ...
      ELSE
       SET dm_err->err_ind = 1
       SET dm_err->emsg = "User Chose to quit from Checkpoint screen."
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ENDIF
     ELSE
      SET dm_err->eproc = concat("No operations for priorities from ",build(drr_priority_group_matrix
        ->priority_group[des_grp_cnt].priority_from_range)," to ",build(drr_priority_group_matrix->
        priority_group[des_grp_cnt].priority_to_range))
      CALL disp_msg("",dm_err->logfile,0)
     ENDIF
    ENDIF
  ENDFOR
 ENDIF
 GO TO exit_program
 SUBROUTINE des_check_restart(null)
   DECLARE dcr_exp_loc = vc WITH protect, noconstant("")
   IF (des_standalone=1)
    SET dm_err->eproc = "Checking if restart."
    IF (drr_clin_copy_restart_chk(null)=0)
     RETURN(0)
    ENDIF
    IF ((drr_clin_copy_data->starting_point="FROM_BEGINNING"))
     SET dm_err->emsg = "User cannot execute schema at this time"
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ELSE
     IF ((drr_clin_copy_data->create_truncate_cmds=1))
      IF (des_standalone=1)
       IF (drr_get_exp_dmp_loc(dcr_exp_loc)=0)
        RETURN(0)
       ENDIF
       SET drr_clin_copy_data->temp_location = dcr_exp_loc
      ENDIF
    ...
    ENDIF
   ENDIF
   SET dm_err->eproc = "Updating any old DDL commands found for run id..."
   CALL disp_msg(" ",dm_err->logfile,0)
   UPDATE  FROM dm2_ddl_ops_log a
    SET a.status = null, a.appl_id = dm2_install_schema->appl_id, a.error_msg = " "
    WHERE (a.run_id=dm2_install_schema->run_id)
     AND a.op_type != "*REMOTE*"
     AND ((a.status="ERROR") OR (a.status = null))
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSE
    COMMIT
    SET dm_err->eproc = build("Restart Cleanup complete - updated (",curqual,") rows. ")
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE des_checkpoint_prompt(dc_ckpt_type,dc_choice_ret)
   DECLARE dc_choice = vc WITH protect, noconstant("")
   SET message = window
   SET width = 132
   CALL clear(1,1)
   CALL box(6,6,10,100)
   CALL text(7,7,"Clinical Copy Checkpoint:")
   CALL text(9,7,concat('"',dc_ckpt_type,'"  Ready to Execute. '))
   CALL text(12,7,"Enter 'C' to Continue or 'Q' to Quit (C or Q):")
   CALL accept(12,55,"A;cu","Q"
    WHERE curaccept IN ("Q", "C"))
   SET dc_choice = curaccept
   SET dc_choice_ret = dc_choice
   SET message = nowindow
   RETURN(1)
 END ;Subroutine
#exit_program
 IF (trim(currdbhandle,3) > " "
  AND des_standalone=1)
  IF ((dm_err->debug_flag < 1))
 ...
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM2_EXECUTE_SCHEMA COMPLETED"
 ENDIF
 CALL final_disp_msg("dm2_exec_schema")
END GO
