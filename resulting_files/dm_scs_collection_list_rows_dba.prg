CREATE PROGRAM dm_scs_collection_list_rows:dba
 ...
 SET reply->table_name = "COLLECTION_LIST"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(cl.collection_list_id)
   FROM collection_list cl
   WHERE cl.collection_list_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(cl.collection_list_id)
  FROM collection_list cl
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    ...
    WHERE parser(sbr_getrowidnotexists("col.collection_list_id between curMinID and curMaxID","col"))
     AND col.list_type_flag=1
     AND ((col.updt_dt_tm+ 0) <= cnvtdatetime(purge_dt_tm))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = col.rowid
    WITH nocounter, maxqual(col,value(rowsleft))
   ;end select
