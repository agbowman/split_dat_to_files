CREATE PROGRAM cv_da_upt_cv_order_category:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_upt_cv_order_category_vrsn = vc WITH private, constant("20080311")
 ...
 SET msdatablename = "CV_ORDER_CATEGORY"
 ...
 WHILE (i <= size(request->objarray,5))
   ...
   SELECT INTO "nl:"
    ...
    WHERE (c.cv_order_category_id=request->objarray[i].cv_order_category_id)
    DETAIL
     IF (validate(request->objarray[i].category_name,c.category_name) != char(128)
      AND validate(request->objarray[i].category_name,c.category_name) != c.category_name)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].category_limit,c.category_limit) != - (1))
      AND validate(request->objarray[i].category_limit,c.category_limit) != c.category_limit)
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].collation_seq,c.collation_seq) != - (1))
      ...
      alteredind = 1
     ENDIF
     IF ((validate(request->objarray[i].section_enum,c.section_enum) != - (1))
      AND validate(request->objarray[i].section_enum,c.section_enum) != c.section_enum)
      alteredind = 1
     ENDIF
   ...
   ENDIF
   IF (alteredind > 0)
    UPDATE  FROM cv_order_category c
     SET c.category_name =
      IF (validate(request->objarray[i].category_name,char(128)) != char(128)) validate(request->
        objarray[i].category_name,char(128))
      ELSE c.category_name
      ENDIF
      , c.category_limit =
      IF ((validate(request->objarray[i].category_limit,- (1)) != - (1))) validate(request->objarray[
        i].category_limit,- (1))
      ELSE c.category_limit
      ENDIF
      , c.collation_seq =
      ...
      ,
      c.section_enum =
      IF ((validate(request->objarray[i].section_enum,- (1)) != - (1))) validate(request->objarray[i]
        .section_enum,- (1))
      ELSE c.section_enum
      ENDIF
