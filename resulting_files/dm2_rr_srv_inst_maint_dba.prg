CREATE PROGRAM dm2_rr_srv_inst_maint:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(dsir_srv_sets->set_cnt,- (1))=- (1))
  AND validate(dsir_srv_sets->set_cnt,1)=1)
  FREE RECORD dsir_srv_sets
  RECORD dsir_srv_sets(
    1 set_cnt = i4
  ...
  SET dsir_srv_sets->set_cnt = 0
  SET dsir_srv_sets->current_set_name = "DM2NOTSET"
 ENDIF
 IF ((validate(dsir_set_detail->server_cnt,- (1))=- (1))
  AND validate(dsir_set_detail->server_cnt,1)=1)
  FREE RECORD dsir_set_detail
  RECORD dsir_set_detail(
    1 set_name = vc
    1 set_desc = vc
    1 date_modified = dq8
    1 server_cnt = i4
  ...
  SET dsir_set_detail->set_name = "DM2NOTSET"
  SET dsir_set_detail->set_desc = "DM2NOTSET"
  SET dsir_set_detail->server_cnt = 0
 ENDIF
 IF ((validate(dsir_curenv_set_detail->server_cnt,- (1))=- (1))
  AND validate(dsir_curenv_set_detail->server_cnt,1)=1)
  FREE RECORD dsir_curenv_set_detail
  RECORD dsir_curenv_set_detail(
    1 set_name = vc
    1 set_desc = vc
    1 server_cnt = i4
    1 servers[*]
  ...
  SET dsir_curenv_set_detail->set_name = "DM2NOTSET"
  SET dsir_curenv_set_detail->set_desc = "DM2NOTSET"
  SET dsir_curenv_set_detail->server_cnt = 0
 ENDIF
 IF ((validate(dsir_rpt_detail->server_cnt,- (1))=- (1))
  AND validate(dsir_rpt_detail->server_cnt,1)=1)
  FREE RECORD dsir_rpt_detail
  RECORD dsir_rpt_detail(
    1 set_cnt = i4
    1 sets[*]
      2 set_name = vc
      2 set_seq = i4
    ...
    1 servers[*]
      2 server_name = vc
      2 server_nbr = i4
      2 set1_num_instances = i4
      2 set2_num_instances = i4
      2 set3_num_instances = i4
      2 curenv_num_instances = i4
  )
  SET dsir_rpt_detail->server_cnt = 0
  SET dsir_rpt_detail->set_cnt = 0
 ENDIF
 DECLARE dsir_get_set_list(dgsl_show_default=i2) = i2
 DECLARE dsir_get_server_set(dgss_show_default=i2) = i2
 DECLARE dsir_clear_dsir_set_detail(null) = i2
 DECLARE dsir_clear_dsir_srv_sets(null) = i2
 DECLARE dsir_load_srv_set_details(dlsd_set_name=vc,dlsd_reset_rs_ind=i2) = i2
 DECLARE dsir_load_cur_env_set_details(dlcesd_domain=vc) = i2
 DECLARE dsir_get_lreg_value(dglv_parm_req=vc,dglv_parm_ret=vc(ref)) = i2
 DECLARE dsir_copy_srv_details(null) = i2
 DECLARE dsir_report_sets(null) = i2
 DECLARE dsir_get_cur_domain(dgcd_cur_domain=vc(ref)) = i2
 SUBROUTINE dsir_clear_dsir_srv_sets(null)
   SET dm_err->eproc = "Clearing up the dsir_srv_sets record structure"
   CALL disp_msg("",dm_err->logfile,0)
   SET dsir_srv_sets->set_cnt = 0
   SET dsir_srv_sets->current_set_name = ""
   SET stat = alterlist(dsir_srv_sets->sets,0)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsir_clear_dsir_set_detail(null)
   SET dm_err->eproc = "Clearing up the dsir_set_detail record structure"
   CALL disp_msg("",dm_err->logfile,0)
   SET dsir_set_detail->server_cnt = 0
   SET dsir_set_detail->set_name = ""
   SET dsir_set_detail->set_desc = ""
   SET stat = alterlist(dsir_set_detail->servers,0)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsir_get_set_list(dgsl_show_default)
   SET dm_err->eproc =
   "Populating the dsir_srv_sets record structure with list of server sets from admin dm_info"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT
    IF (dgsl_show_default=0)
     WHERE di.info_domain=patstring("DM2_SRV_INST_CONFIG-*")
      AND di.info_domain != "DM2_SRV_INST_CONFIG-CERNER_SHIPPED_DEFAULT"
    ELSE
     WHERE di.info_domain=patstring("DM2_SRV_INST_CONFIG-*")
    ENDIF
    DISTINCT INTO "nl:"
    ...
    HEAD REPORT
     dsir_srv_sets->set_cnt = 0, stat = alterlist(dsir_srv_sets->sets,dsir_srv_sets->set_cnt)
    HEAD di.info_domain
     dsir_srv_sets->set_cnt = (dsir_srv_sets->set_cnt+ 1)
     IF (mod(dsir_srv_sets->set_cnt,10)=1)
      stat = alterlist(dsir_srv_sets->sets,(dsir_srv_sets->set_cnt+ 9))
     ENDIF
     dsir_srv_sets->sets[dsir_srv_sets->set_cnt].set_name = replace(di.info_domain,
      "DM2_SRV_INST_CONFIG-","",1), dsir_srv_sets->sets[dsir_srv_sets->set_cnt].server_cnt = 0
    DETAIL
     IF (di.info_name="SERVER_SET_DESCRIPTION")
      dsir_srv_sets->sets[dsir_srv_sets->set_cnt].set_desc = trim(di.info_char), dsir_srv_sets->sets[
      dsir_srv_sets->set_cnt].date_modified = di.info_date
     ELSE
      dsir_srv_sets->sets[dsir_srv_sets->set_cnt].server_cnt = (dsir_srv_sets->sets[dsir_srv_sets->
      set_cnt].server_cnt+ 1)
     ENDIF
    FOOT REPORT
     stat = alterlist(dsir_srv_sets->sets,dsir_srv_sets->set_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dsir_srv_sets)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsir_get_server_set(dgss_show_default)
   DECLARE dgss_invalid = i2 WITH protect, noconstant(0)
   DECLARE dgss_set_name = vc WITH protect, noconstant("")
   DECLARE dgss_set_idx = i4 WITH protect, noconstant(0)
   DECLARE dgss_continue = i2 WITH protect, noconstant(0)
   DECLARE dgss_line = i4 WITH protect, noconstant(0)
   DECLARE dgss_cur_domain = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Obtaining Server Set"
   CALL disp_msg("",dm_err->logfile,0)
   SET dsir_srv_sets->current_set_name = "DM2NOTSET"
   IF (dsir_clear_dsir_set_detail(0)=0)
    RETURN(0)
   ENDIF
   IF (dsir_get_cur_domain(dgss_cur_domain)=0)
    RETURN(0)
   ENDIF
   SET dgss_continue = 1
   SET dm_err->eproc = "Verifying at least one Server set exists"
   IF (dsir_get_set_list(dgss_show_default)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Obtaining Server Set from user"
   WHILE (dgss_continue=1)
     SET message = window
     CALL clear(1,1)
     CALL box(1,1,3,132)
     CALL text(2,2,"SERVER INSTANCE CONFIGURATION [SERVER SET SELECTION]")
     CALL text(2,70,"DOMAIN: ")
     CALL text(2,80,cnvtupper(dgss_cur_domain))
     IF ((dsir_srv_sets->set_cnt=0))
      CALL text(5,2,"**No Server Sets currently exist. Press <Enter> to Continue")
      CALL accept(5,90,"p;cu"," "
       WHERE curaccept IN (" "))
      SET dgss_continue = 0
     ELSE
      CALL text(7,2,"Server Set Name: ")
      SET help = pos(9,2,10,128)
      SET help =
      SELECT INTO "nl:"
       server_set = substring(1,30,dsir_srv_sets->sets[t.seq].set_name), server_cnt = cnvtstring(
        dsir_srv_sets->sets[t.seq].server_cnt)
       FROM (dummyt t  WITH seq = value(dsir_srv_sets->set_cnt))
       ORDER BY dsir_srv_sets->sets[t.seq].set_name
       WITH nocounter
      ;end select
      CALL accept(7,30,"P(30);CUF")
      SET help = off
      SET dgss_set_name = trim(curaccept)
      SET dm_err->eproc = "Verifying that Server Set exists"
      IF (locateval(dgss_set_idx,1,dsir_srv_sets->set_cnt,dgss_set_name,dsir_srv_sets->sets[
       dgss_set_idx].set_name) > 0)
       SET dsir_srv_sets->current_set_name = dgss_set_name
       SET dgss_continue = 0
       CALL clear(5,2,100)
      ELSE
       CALL text(5,2,build("**",dgss_set_name,
         " does not exist. Please provide another Server Set name."))
       SET dgss_continue = 1
      ENDIF
     ENDIF
   ENDWHILE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsir_load_srv_set_details(dlsd_set_name,dlsd_reset_rs_ind)
   DECLARE dlssd_cnt = i4 WITH protect, noconstant(0)
   DECLARE dlssd_loc = i4 WITH protect, noconstant(0)
   DECLARE dlssd_idx = i4 WITH protect, noconstant(0)
   DECLARE dlssd_cur_domain = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Clearing up the dsir_set_detail record structure"
   CALL disp_msg("",dm_err->logfile,0)
   IF (dlsd_reset_rs_ind=1)
    IF (dsir_clear_dsir_set_detail(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (dsir_get_cur_domain(dlssd_cur_domain)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Retrieving server set details from dm2_admin_dm_info"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_admin_dm_info di
    WHERE di.info_domain=concat("DM2_SRV_INST_CONFIG-",dlsd_set_name)
    ORDER BY di.info_name
    HEAD REPORT
     dlssd_cnt = 0, stat = alterlist(dsir_set_detail->servers,dlssd_cnt)
    DETAIL
     dsir_set_detail->set_name = dlsd_set_name
     IF (di.info_name="SERVER_SET_DESCRIPTION")
      dsir_set_detail->set_desc = di.info_char, dsir_set_detail->date_modified = di.info_date
     ELSE
      dlssd_cnt = (dlssd_cnt+ 1)
      IF (mod(dlssd_cnt,10)=1)
       stat = alterlist(dsir_set_detail->servers,(dlssd_cnt+ 9))
      ENDIF
      dsir_set_detail->servers[dlssd_cnt].server_name = di.info_name, dsir_set_detail->servers[
      dlssd_cnt].num_instances = di.info_number
     ENDIF
    FOOT REPORT
     dsir_set_detail->server_cnt = dlssd_cnt, stat = alterlist(dsir_set_detail->servers,
      dsir_set_detail->server_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
    ...
    RETURN(0)
   ENDIF
   IF ((dsir_curenv_set_detail->server_cnt=0))
    IF (dsir_load_cur_env_set_details(dlssd_cur_domain)=0)
     RETURN(0)
    ENDIF
   ENDIF
   FOR (dlssd_cnt = 1 TO dsir_set_detail->server_cnt)
    SET dlssd_loc = locateval(dlssd_idx,1,dsir_curenv_set_detail->server_cnt,dsir_set_detail->
     servers[dlssd_cnt].server_name,dsir_curenv_set_detail->servers[dlssd_idx].server_name)
    IF (dlssd_loc > 0)
     SET dsir_set_detail->servers[dlssd_cnt].server_nbr = dsir_curenv_set_detail->servers[dlssd_loc].
     server_nbr
    ELSE
     SET dsir_set_detail->servers[dlssd_cnt].server_nbr = - (1)
    ENDIF
   ENDFOR
   IF ((dm_err->debug_flag=722))
    CALL echorecord(dsir_set_detail)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsir_copy_srv_details(null)
   DECLARE dcsd_ndx = i4 WITH protect, noconstant(0)
   DECLARE dcsd_cnt = i4 WITH protect, noconstant(0)
   DECLARE dcsd_from_cnt = i4 WITH protect, noconstant(0)
   DECLARE dcsd_cmd = vc WITH protect, noconstant("")
   SET dcsd_cnt = 0
   SET stat = alterlist(dsir_set_detail->servers,dcsd_cnt)
   FOR (dcsd_ndx = 1 TO dsir_curenv_set_detail->server_cnt)
     SET dcsd_cnt = (dcsd_cnt+ 1)
     SET stat = alterlist(dsir_set_detail->servers,dcsd_cnt)
     SET dsir_set_detail->servers[dcsd_cnt].server_name = dsir_curenv_set_detail->servers[dcsd_ndx].
     server_name
     SET dsir_set_detail->servers[dcsd_cnt].num_instances = dsir_curenv_set_detail->servers[dcsd_ndx]
     .num_instances
     SET dsir_set_detail->servers[dcsd_cnt].server_nbr = dsir_curenv_set_detail->servers[dcsd_ndx].
     server_nbr
   ENDFOR
   SET dsir_set_detail->server_cnt = dcsd_cnt
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsir_load_cur_env_set_details(dlcesd_domain)
   DECLARE dlcesd_cnt = i4 WITH protect, noconstant(0)
   DECLARE dlcesd_cmd = vc WITH protect, noconstant("")
   DECLARE dlcesd_cmd_file = vc WITH protect, noconstant("")
   DECLARE dlcesd_node = vc WITH protect, noconstant(cnvtlower(trim(curnode)))
   DECLARE dlcesd_lreg_path = vc WITH protect, noconstant("")
   DECLARE dlcesd_srv_name = vc WITH protect, noconstant("")
   DECLARE dlcesd_srv_num = i4 WITH protect, noconstant(- (1))
   DECLARE dlcesd_srv_inst = i4 WITH protect, noconstant(- (1))
   IF (get_unique_file("get_lreg_tree",evaluate(dm2_sys_misc->cur_os,"AXP",".com",".ksh"))=0)
    RETURN(0)
   ENDIF
   SET dlcesd_cmd_file = dm_err->unique_fname
   SET dm_err->eproc = concat("Create file ",dlcesd_cmd_file," to find all servers for domain ",
    dlcesd_domain)
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dlcesd_lreg_path = evaluate(dm2_sys_misc->cur_os,"AXP",concat("\node\",dlcesd_node,"\domain\",
     dlcesd_domain,"\servers\"),concat("\\node\\",dlcesd_node,"\\domain\\",dlcesd_domain,
     "\\servers\\"))
   SELECT INTO value(dlcesd_cmd_file)
    DETAIL
     IF ((dm2_sys_misc->cur_os="AXP"))
      CALL print(concat("$!",dlcesd_cmd_file)), row + 1,
      CALL print("$!"),
      row + 1,
      CALL print("$define sys$output dlcesd_server_tree.dat"), row + 1,
      CALL print("$mcr cer_exe:lregview"), row + 1,
      CALL print(concat("cd ",dlcesd_lreg_path)),
      row + 1,
      CALL print(concat("tree ")), row + 1,
      CALL print("exit"), row + 1,
      CALL print("$deassign sys$output"),
      row + 1,
      CALL print(concat("$open/read SERVER_TREE ","dlcesd_server_tree.dat")), row + 1,
      CALL print("$READ_SERVER_TREE:"), row + 1,
      CALL print("$   read/end_of_file=END_READ_SERVER_TREE SERVER_TREE record"),
      row + 1,
      CALL print('$   record = f$edit(record, "lowercase")'), row + 1,
      CALL print("$   length = f$length(record)"), row + 1,
      CALL print('$   pos = f$locate(": key not found", record)'),
      row + 1,
      CALL print("$   if (pos .gt. 0) .and. (pos .ne. length)"), row + 1,
      CALL print("$   then"), row + 1,
      CALL print("$      not_valid = 1"),
      row + 1,
      CALL print("$   endif"), row + 1,
      CALL print("$   goto READ_SERVER_TREE "), row + 1,
      CALL print("$END_READ_SERVER_TREE: "),
      row + 1,
      CALL print("$   close SERVER_TREE  "), row + 1,
      CALL print("$if (not_valid .eq. 1)"), row + 1,
      CALL print("$then"),
      row + 1,
      CALL print('$   write sys$output "Unable to find Registry Keys for given node/domain"'), row +
      1,
     ...
     ELSE
      ...
      CALL print(concat("# ",dlcesd_cmd_file)), row + 1,
      CALL print("#"), row + 1,
      CALL print("$cer_exe/lregview <<!>dlcesd_server_tree.dat"),
      row + 1,
      CALL print(concat("cd ",dlcesd_lreg_path)), row + 1,
      CALL print(concat("tree ")), row + 1,
      CALL print("exit"),
      row + 1,
      CALL print("!"), row + 1,
      CALL print(concat("tr '[:upper:]' '[:lower:]' < ",
       'dlcesd_server_tree.dat |grep ": key not found" ')), row + 1,
      CALL print("if [[ $? -eq 0 ]]"),
      row + 1,
      CALL print("then"), row + 1,
      CALL print('   echo "Unable to find Registry Keys for given node/domain"'), row + 1,
      CALL print("   exit 1"),
      row + 1,
      CALL print("fi"), row + 1
     ENDIF
   ...
   IF (check_error(dm_err->eproc) != 0)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = concat("Execute file: ",dlcesd_cmd_file)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET dlcesd_cmd = concat("@",dlcesd_cmd_file)
   ELSE
    SET dlcesd_cmd = concat(". $CCLUSERDIR/",dlcesd_cmd_file)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dm_err)
   ENDIF
   SET dm_err->disp_dcl_err_ind = 0
   IF (dm2_push_dcl(dlcesd_cmd)=0)
    IF (findstring("Unable to find Registry Keys for given node/domain",dm_err->errtext,1,1) > 0)
     SET dm_err->err_ind = 1
     SET dm_err->eproc = "Verify that appropriate registry keys exist for given node/domain"
     SET dm_err->emsg = concat("Unable to find Registry Keys for :",dlcesd_lreg_path)
     SET dm_err->user_action = concat("Verify that registry keys exist for domain ","(",dlcesd_domain,
      ") on current node (",dlcesd_node,
      ")")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ELSE
   ...
   ENDIF
   SET dm_err->eproc =
   "Populating the dsir_set_detail record structure from current environment registry"
   CALL disp_msg("",dm_err->logfile,0)
   FREE DEFINE rtl2
   FREE SET file_loc
   SET logical file_loc value("dlcesd_server_tree.dat")
   DEFINE rtl2 "file_loc"
   SELECT INTO "nl:"
    r.line
    FROM rtl2t r
    WHERE r.line > " "
    HEAD REPORT
     dlcesd_cnt = 0, stat = alterlist(dsir_curenv_set_detail->servers,dlcesd_cnt), beg_pos = 0,
     end_pos = 0, beg_pos2 = 0, end_pos2 = 0,
     beg_pos3 = 0, end_pos3 = 0, tmp_num = 0,
     dlcesd_srv_name = "DM2NOTSET", dlcesd_srv_num = - (1), dlcesd_srv_inst = - (1)
    DETAIL
     IF ((dm_err->debug_flag > 1))
      CALL echo(concat("LINE = ",r.line))
     ENDIF
     IF ((dlcesd_srv_num > - (1)))
      IF (findstring("servername",cnvtlower(r.line),1,0) > 0)
       beg_pos2 = findstring("=",r.line,1,0), end_pos2 = findstring(" ",r.line,(beg_pos2+ 2),1)
       IF ((dm_err->debug_flag > 0))
        CALL echo(build("beg_pos2=",beg_pos2," ; end_pos2=",end_pos2))
       ENDIF
       IF (beg_pos > 0
        AND end_pos > 0)
        dlcesd_srv_name = substring((beg_pos2+ 2),((end_pos2 - beg_pos2) - 1),r.line)
        IF ((dm_err->debug_flag > 0))
         CALL echo(build("SERVER_NAME=",dlcesd_srv_name))
        ENDIF
       ENDIF
      ENDIF
      IF (findstring("numinstances",cnvtlower(r.line),1,0) > 0)
       beg_pos3 = findstring("=",r.line,1,0), end_pos3 = findstring(" ",r.line,(beg_pos3+ 2),0)
       IF ((dm_err->debug_flag > 0))
        CALL echo(build("beg_pos3=",beg_pos3," ; end_pos3=",end_pos3))
       ENDIF
       IF (beg_pos3 > 0
        AND end_pos3 > 0)
        dlcesd_srv_inst = cnvtint(substring((beg_pos3+ 2),((end_pos3 - beg_pos3) - 1),r.line))
        IF ((dm_err->debug_flag > 0))
         CALL echo(build("NUMINSTANCES=",dlcesd_srv_inst))
        ENDIF
       ENDIF
      ENDIF
      IF ((dlcesd_srv_inst > - (1))
       AND dlcesd_srv_name != "DM2NOTSET")
       dlcesd_cnt = (dlcesd_cnt+ 1)
       IF (mod(dlcesd_cnt,10)=1)
        stat = alterlist(dsir_curenv_set_detail->servers,(dlcesd_cnt+ 9))
       ENDIF
       dsir_curenv_set_detail->servers[dlcesd_cnt].server_name = cnvtupper(dlcesd_srv_name),
       dsir_curenv_set_detail->servers[dlcesd_cnt].server_nbr = dlcesd_srv_num,
       dsir_curenv_set_detail->servers[dlcesd_cnt].num_instances = dlcesd_srv_inst,
       dlcesd_srv_num = - (1), dlcesd_srv_inst = - (1), dlcesd_srv_name = "DM2NOTSET"
      ENDIF
     ELSE
      beg_pos = findstring("\",cnvtlower(r.line),1,0)
      IF (beg_pos > 0)
       tmp_num = ichar(substring((beg_pos+ 1),1,cnvtlower(r.line)))
       IF (tmp_num > 48
        AND tmp_num < 58)
        end_pos = findstring(" ",r.line,beg_pos,0)
        IF ((dm_err->debug_flag > 0))
         CALL echo(build("beg_pos=",beg_pos," ; end_pos=",end_pos))
        ENDIF
        dlcesd_srv_num = cnvtint(substring((beg_pos+ 1),((end_pos - beg_pos) - 1),r.line))
        IF ((dm_err->debug_flag > 0))
         CALL echo(build("SERVER_NUMBER=",dlcesd_srv_num))
        ENDIF
       ENDIF
      ENDIF
     ENDIF
    FOOT REPORT
     dsir_curenv_set_detail->server_cnt = dlcesd_cnt, stat = alterlist(dsir_curenv_set_detail->
      servers,dsir_curenv_set_detail->server_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   ENDIF
   IF ((dm_err->debug_flag=722))
    CALL echorecord(dsir_curenv_set_detail)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsir_insert_set_details(null)
   SET dm_err->eproc = concat("Deleting Server Set details for ",dsir_set_detail->set_name,
    " from dm2_admin_dm_info")
   CALL disp_msg("",dm_err->logfile,0)
   DELETE  FROM dm2_admin_dm_info di
    WHERE di.info_domain=concat("DM2_SRV_INST_CONFIG-",dsir_srv_sets->current_set_name)
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = concat("Inserting Server Set details for ",dsir_set_detail->set_name,
    " into dm2_admin_dm_info")
   CALL disp_msg("",dm_err->logfile,0)
   INSERT  FROM dm2_admin_dm_info di
    SET di.info_domain = concat("DM2_SRV_INST_CONFIG-",dsir_set_detail->set_name), di.info_name =
     "SERVER_SET_DESCRIPTION", di.info_char = dsir_set_detail->set_desc,
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   IF ((dsir_set_detail->server_cnt > 0))
    SET dm_err->eproc = concat("Inserting Individual Server details for ",dsir_set_detail->set_name,
     " into dm2_admin_dm_info")
    INSERT  FROM dm2_admin_dm_info di,
      (dummyt d  WITH seq = value(dsir_set_detail->server_cnt))
     SET di.info_domain = concat("DM2_SRV_INST_CONFIG-",dsir_set_detail->set_name), di.info_name =
      dsir_set_detail->servers[d.seq].server_name, di.info_number = dsir_set_detail->servers[d.seq].
      num_instances
     PLAN (d
      WHERE d.seq > 0
       AND (dsir_set_detail->servers[d.seq].remove_ind=0))
      JOIN (di)
     WITH nocounter
    ;end insert
    IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsir_report_sets(null)
   DECLARE drs_continue = i2 WITH protect, noconstant(1)
   DECLARE drs_continue2 = i2 WITH protect, noconstant(1)
   DECLARE drs_set_name = vc WITH protect, noconstant("")
   DECLARE drs_seq = i4 WITH protect, noconstant(1)
   DECLARE drs_index = i4 WITH protect, noconstant(1)
   DECLARE drs_index2 = i4 WITH protect, noconstant(1)
   DECLARE drs_idx = i4 WITH protect, noconstant(0)
   DECLARE drs_loc = i4 WITH protect, noconstant(0)
   DECLARE drs_rpt_set_cnt = i4 WITH protect, noconstant(0)
   DECLARE drs_location = i4 WITH protect, noconstant(0)
   DECLARE drs_cur_domain = vc WITH protect, noconstant("")
   SET dsir_rpt_detail->set_cnt = 0
   SET stat = alterlist(dsir_rpt_detail->sets,dsir_rpt_detail->set_cnt)
   SET dsir_rpt_detail->server_cnt = 0
   SET stat = alterlist(dsir_rpt_detail->servers,dsir_rpt_detail->server_cnt)
   IF (dsir_get_cur_domain(drs_cur_domain)=0)
    RETURN(0)
   ENDIF
   IF ((dsir_curenv_set_detail->server_cnt=0))
    IF (dsir_load_cur_env_set_details(drs_cur_domain)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (dsir_get_set_list(1)=0)
    RETURN(0)
   ENDIF
   WHILE (drs_continue=1)
     SET dm_err->eproc = "Report Server Sets"
     SET message = window
     CALL clear(1,1)
     CALL box(1,1,3,132)
     CALL box(1,1,24,132)
     CALL text(2,2,"SERVER INSTANCE CONFIGURATION [SERVER SET REPORT]")
     CALL text(2,70,"DOMAIN: ")
     CALL text(2,80,cnvtupper(drs_cur_domain))
     IF ((dsir_srv_sets->set_cnt=0))
      CALL text(5,2,"**No Server Sets currently exist. Press <Enter> to Continue")
      CALL accept(5,90,"p;cu"," "
       WHERE curaccept IN (" "))
      SET drs_continue = 0
     ELSE
      CALL text(7,2,"Choose a Server Set to add to the Comparison Report.")
      CALL text(8,2,"You can add up to 3 server sets for comparison.")
      SET drs_seq = 10
      SET drs_continue2 = 1
      WHILE (drs_continue2=1)
        CALL text((drs_seq+ drs_rpt_set_cnt),2,concat("Server Set ",evaluate((drs_rpt_set_cnt+ 1),1,
           "I",2,"II",
           3,"III"),": "))
        SET help = pos(9,2,10,128)
        SET help =
        SELECT INTO "nl:"
         server_set = substring(1,30,dsir_srv_sets->sets[t.seq].set_name), server_cnt = cnvtstring(
          dsir_srv_sets->sets[t.seq].server_cnt)
         FROM (dummyt t  WITH seq = value(dsir_srv_sets->set_cnt))
         WHERE (dsir_srv_sets->sets[t.seq].rpt_chosen_ind=0)
         ORDER BY dsir_srv_sets->sets[t.seq].set_name
         WITH nocounter
        ;end select
        CALL accept((drs_seq+ drs_rpt_set_cnt),30,"P(30);CUF")
        SET help = off
        SET drs_set_name = trim(curaccept)
        SET drs_index = locateval(drs_idx,1,dsir_srv_sets->set_cnt,drs_set_name,dsir_srv_sets->sets[
         drs_idx].set_name)
        IF (drs_index > 0)
         SET dsir_srv_sets->sets[drs_index].rpt_chosen_ind = 1
         SET drs_index2 = locateval(drs_idx,1,dsir_rpt_detail->set_cnt,drs_set_name,dsir_rpt_detail->
          sets[drs_idx].set_name)
         IF (drs_index2=0)
          SET drs_rpt_set_cnt = (drs_rpt_set_cnt+ 1)
          SET dsir_rpt_detail->set_cnt = (dsir_rpt_detail->set_cnt+ 1)
          SET stat = alterlist(dsir_rpt_detail->sets,dsir_rpt_detail->set_cnt)
          SET dsir_rpt_detail->sets[dsir_rpt_detail->set_cnt].set_name = drs_set_name
          SET dsir_rpt_detail->sets[dsir_rpt_detail->set_cnt].set_seq = drs_rpt_set_cnt
          CALL clear(5,2,100)
         ENDIF
        ELSE
         CALL text(5,2,build("**",drs_set_name,
           " does not exist. Please provide another Server Set name."))
         SET drs_continue2 = 1
        ENDIF
        IF (drs_rpt_set_cnt=3)
         CALL text(15,2,
          "Three server sets have been added to the report. Please choose an option below: ")
         SET drs_continue2 = 0
        ELSE
         CALL text(15,2,"Add another server set to the report? (Y/N): ")
         CALL accept(15,50,"A;CU","N"
        ...
        ENDIF
      ENDWHILE
      CALL text(23,2,"(C)reate Report, (B)ack: ")
      CALL accept(23,30,"A;CUS"," "
       WHERE curaccept IN ("M", "C", "B"))
      CASE (curaccept)
       OF "B":
        SET drs_continue = 0
       OF "C":
        FOR (drs_index = 1 TO dsir_rpt_detail->set_cnt)
         IF (dsir_load_srv_set_details(dsir_rpt_detail->sets[drs_index].set_name,1)=0)
          RETURN(0)
         ENDIF
         FOR (drs_index2 = 1 TO dsir_set_detail->server_cnt)
          SET drs_location = locateval(drs_idx,1,dsir_rpt_detail->server_cnt,dsir_set_detail->
           servers[drs_index2].server_name,dsir_rpt_detail->servers[drs_idx].server_name)
          IF (drs_location=0)
           SET dsir_rpt_detail->server_cnt = (dsir_rpt_detail->server_cnt+ 1)
           SET stat = alterlist(dsir_rpt_detail->servers,dsir_rpt_detail->server_cnt)
           SET dsir_rpt_detail->servers[dsir_rpt_detail->server_cnt].server_name = dsir_set_detail->
           servers[drs_index2].server_name
           SET dsir_rpt_detail->servers[dsir_rpt_detail->server_cnt].server_nbr = dsir_set_detail->
           servers[drs_index2].server_nbr
           SET dsir_rpt_detail->servers[dsir_rpt_detail->server_cnt].set1_num_instances = - (1)
           SET dsir_rpt_detail->servers[dsir_rpt_detail->server_cnt].set2_num_instances = - (1)
           SET dsir_rpt_detail->servers[dsir_rpt_detail->server_cnt].set3_num_instances = - (1)
           SET dsir_rpt_detail->servers[dsir_rpt_detail->server_cnt].curenv_num_instances = - (1)
           CASE (drs_index)
            OF 1:
             SET dsir_rpt_detail->servers[dsir_rpt_detail->server_cnt].set1_num_instances =
             dsir_set_detail->servers[drs_index2].num_instances
            OF 2:
             SET dsir_rpt_detail->servers[dsir_rpt_detail->server_cnt].set2_num_instances =
             dsir_set_detail->servers[drs_index2].num_instances
            OF 3:
             SET dsir_rpt_detail->servers[dsir_rpt_detail->server_cnt].set3_num_instances =
             dsir_set_detail->servers[drs_index2].num_instances
           ENDCASE
           SET drs_loc = locateval(drs_idx,1,dsir_curenv_set_detail->server_cnt,dsir_set_detail->
            servers[drs_index2].server_name,dsir_curenv_set_detail->servers[drs_idx].server_name)
           IF (drs_loc > 0)
            SET dsir_rpt_detail->servers[dsir_rpt_detail->server_cnt].curenv_num_instances =
            dsir_curenv_set_detail->servers[drs_loc].num_instances
           ENDIF
          ELSE
           CASE (drs_index)
            OF 1:
             SET dsir_rpt_detail->servers[drs_location].set1_num_instances = dsir_set_detail->
             servers[drs_index2].num_instances
            OF 2:
             SET dsir_rpt_detail->servers[drs_location].set2_num_instances = dsir_set_detail->
             servers[drs_index2].num_instances
            OF 3:
             SET dsir_rpt_detail->servers[drs_location].set3_num_instances = dsir_set_detail->
             servers[drs_index2].num_instances
           ENDCASE
          ENDIF
         ENDFOR
        ENDFOR
        IF (dsir_build_report(null)=0)
         RETURN(0)
        ENDIF
        SET drs_continue = 0
      ENDCASE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsir_build_report(null)
   DECLARE dbr_file_name = vc WITH protect, noconstant("")
   IF (get_unique_file("dm2_srv_set",".rpt")=0)
    RETURN(0)
   ENDIF
   SET dbr_file_name = cnvtlower(dm_err->unique_fname)
   IF (validate(drrr_responsefile_in_use,- (1))=1)
    SET dbr_file_name = build(drrr_misc_data->active_dir,cnvtlower(dm_err->unique_fname))
   ENDIF
   SET dm_err->eproc = concat("Creating Server Set Summary Report: ",dbr_file_name)
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO value(dbr_file_name)
    server_name = dsir_rpt_detail->servers[d.seq].server_name, curdomain_num_instances =
    dsir_rpt_detail->servers[d.seq].curenv_num_instances, set1_num_instances = dsir_rpt_detail->
    servers[d.seq].set1_num_instances,
    set2_num_instances = dsir_rpt_detail->servers[d.seq].set2_num_instances, set3_num_instances =
    dsir_rpt_detail->servers[d.seq].set3_num_instances
    FROM (dummyt d  WITH seq = value(dsir_rpt_detail->server_cnt))
    ORDER BY server_name
    HEAD REPORT
     row + 1, "Report Location:  ", dbr_file_name,
     row + 1, row + 1,
     "The following report shows the NumInstances values for all the servers present in the selected 'Server Sets'",
     row + 1,
     "If a server does not exist in a particular set, its NumInstances value is shown as N/A.", row
      + 2,
     "The following Server Sets are represented in the report: ", row + 1, col 2,
     "CUR_DOMAIN ", col 30, " - Server Instance Configuration for Current Domain",
     row + 1
     FOR (idx = 1 TO dsir_rpt_detail->set_cnt)
       col 2,
       CALL print(concat("SET ",evaluate(idx,1,"I",2,"II",
         3,"III")," ")), col 30,
       CALL print(concat(" - ",dsir_rpt_detail->sets[idx].set_name)), row + 1
     ENDFOR
    HEAD PAGE
     row + 2, col 80, "NumInstances",
     row + 1, col 40,
     CALL print(fillstring(80,"-")),
     row + 1, col 2, "SERVER NAME(SERVER NUMBER)",
     col 50, "CUR_DOMAIN", col 70,
     "SET I"
     IF ((dsir_rpt_detail->set_cnt >= 2))
      col 90, "SET II"
     ENDIF
     IF ((dsir_rpt_detail->set_cnt >= 3))
      col 110, "SET III"
     ENDIF
     row + 1, col 2,
     CALL print(fillstring(120,"-")),
     row + 1
    DETAIL
     col 2,
     CALL print(build(dsir_rpt_detail->servers[d.seq].server_name,"(",evaluate(dsir_rpt_detail->
       servers[d.seq].server_nbr,- (1),"N/A",cnvtstring(dsir_rpt_detail->servers[d.seq].server_nbr)),
      ")")), col 50,
     CALL print(evaluate(dsir_rpt_detail->servers[d.seq].curenv_num_instances,- (1),"N/A",cnvtstring(
       dsir_rpt_detail->servers[d.seq].curenv_num_instances))), col 70,
     CALL print(evaluate(dsir_rpt_detail->servers[d.seq].set1_num_instances,- (1),"N/A",cnvtstring(
       dsir_rpt_detail->servers[d.seq].set1_num_instances)))
     IF ((dsir_rpt_detail->set_cnt >= 2))
      col 90,
      CALL print(evaluate(dsir_rpt_detail->servers[d.seq].set2_num_instances,- (1),"N/A",cnvtstring(
        dsir_rpt_detail->servers[d.seq].set2_num_instances)))
     ENDIF
     IF ((dsir_rpt_detail->set_cnt >= 3))
      col 110,
      CALL print(evaluate(dsir_rpt_detail->servers[d.seq].set3_num_instances,- (1),"N/A",cnvtstring(
        dsir_rpt_detail->servers[d.seq].set3_num_instances)))
     ENDIF
     row + 1
    WITH nocounter, format = variable, formfeed = none,
   ...
   IF (validate(drrr_responsefile_in_use,0)=0)
    IF (dm2_disp_file(dbr_file_name,"Server Set Summary Report")=0)
     RETURN(0)
    ENDIF
   ENDIF
 END ;Subroutine
 SUBROUTINE dsir_get_lreg_value(dglv_parm_req,dglv_parm_ret)
   DECLARE dglv_cmd = vc WITH protect, noconstant("")
   DECLARE dglv_file = vc WITH protect, noconstant("")
   DECLARE dglv_no_error = i2 WITH protect, noconstant(0)
   IF (get_unique_file("dsir_get_reg",evaluate(dm2_sys_misc->cur_os,"AXP",".com",".ksh"))=0)
    RETURN(0)
   ELSE
    SET dglv_file = dm_err->unique_fname
   ENDIF
   SET dm_err->eproc = concat("Create file to obtain registry info:",dglv_file)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO value(dglv_file)
    DETAIL
     IF ((dm2_sys_misc->cur_os="AXP"))
      CALL print(concat("$ mcr cer_exe:lreg -getp ",dglv_parm_req)), row + 1,
      CALL print("$ write sys$output lreg_result"),
      row + 1
     ELSE
      CALL print(concat("$cer_exe/lreg -getp ",dglv_parm_req)), row + 1
     ENDIF
    WITH nocounter, maxcol = 500, format = variable,
     maxrow = 1
   ;end select
   ...
   ENDIF
   IF ((dm2_sys_misc->cur_os="AXP"))
    SET dglv_cmd = concat("@",dglv_file)
   ELSE
    SET dglv_cmd = concat(". $CCLUSERDIR/",dglv_file)
   ENDIF
   SET dm_err->disp_dcl_err_ind = 0
   SET dglv_no_error = dm2_push_dcl(dglv_cmd)
   IF (dglv_no_error=0)
    IF ((dm_err->err_ind=1))
     RETURN(0)
    ENDIF
   ...
   IF (((findstring("unable",dm_err->errtext,1,1)) OR ((((dm_err->errtext="")) OR (((findstring(
    ...
    SET dglv_no_error = 1
    SET dglv_parm_ret = "NOPARMRETURNED"
   ELSE
    SET dglv_parm_ret = dm_err->errtext
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("parm_value:",dglv_parm_ret))
   ENDIF
   IF (dglv_no_error=0)
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsir_get_cur_domain(dgcd_cur_domain)
   DECLARE dgcd_env = vc WITH protect, noconstant(trim(cnvtlower(logical("environment"))))
   DECLARE dgcd_str = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Retrieve current domain name from registry"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SET dgcd_str = concat("\\environment\\",dgcd_env," Domain")
   IF (dsir_get_lreg_value(dgcd_str,dgcd_cur_domain)=0)
    RETURN(0)
   ENDIF
   IF (dgcd_cur_domain="NOPARMRETURNED")
    SET dm_err->emsg = concat("Unable to retrieve domain name property for ",dgcd_env)
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 DECLARE dsim_cur_domain = vc WITH protect, noconstant("")
 DECLARE dsim_create_srv_set(null) = i2
 DECLARE dsim_pre_load_servers(dpls_set_name=vc,dpls_preload_choice=i4,dpls_preload_set=vc) = i2
 DECLARE dsim_modify_srv_set(null) = i2
 DECLARE dsim_remove_srv_set(null) = i2
 IF (check_logfile("dm2_srv_inst_maint",".log","dm2_srv_inst_maint")=0)
  GO TO exit_program
 ENDIF
 IF (dsir_get_cur_domain(dsim_cur_domain)=0)
  GO TO exit_program
 ENDIF
 IF (dsir_load_cur_env_set_details(dsim_cur_domain)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag=722))
  SET message = nowindow
 ...
 WHILE (true)
   ...
   CALL box(1,1,3,132)
   CALL box(1,1,24,132)
   CALL text(2,2,"SERVER INSTANCE CONFIGURATION [MAIN]")
   CALL text(2,70,"DOMAIN: ")
   CALL text(2,80,cnvtupper(dsim_cur_domain))
   CALL text(7,10,"1. Create Server Set")
   CALL text(9,10,"2. Modify Server Set")
   CALL text(11,10,"3. Remove Server Set")
   CALL text(13,10,"4. Report Server Sets")
   ...
   CASE (curaccept)
    OF 0:
     GO TO exit_program
    OF 1:
     IF (dsim_create_srv_set(null)=0)
      GO TO exit_program
     ENDIF
    OF 2:
     IF (dsim_modify_srv_set(null)=0)
      GO TO exit_program
     ENDIF
    OF 3:
     IF (dsim_remove_srv_set(null)=0)
      GO TO exit_program
     ENDIF
    OF 4:
     IF (dsir_report_sets(null)=0)
      GO TO exit_program
     ENDIF
   ENDCASE
 ENDWHILE
 GO TO exit_program
 SUBROUTINE dsim_create_srv_set(null)
   DECLARE dcss_set_exists = i2 WITH protect, noconstant(0)
   DECLARE dcss_preload_invalid = i2 WITH protect, noconstant(0)
   DECLARE dcss_set_name = vc WITH protect, noconstant("")
   DECLARE dcss_set_desc = vc WITH protect, noconstant("")
   DECLARE dcss_preload_set = vc WITH protect, noconstant("DM2NOTSET")
   DECLARE dcss_preload_choice = i4 WITH protect, noconstant(0)
   DECLARE dcss_continue = i2 WITH protect, noconstant(1)
   DECLARE dcss_continue2 = i2 WITH protect, noconstant(1)
   DECLARE dcss_set_idx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Creating new Server Set"
   CALL disp_msg("",dm_err->logfile,0)
   IF (dsir_get_set_list(0)=0)
    RETURN(0)
   ENDIF
   SET dsir_srv_sets->current_set_name = "DM2NOTSET"
   IF (dsir_clear_dsir_set_detail(null)=0)
   ...
   ENDIF
   SET dm_err->eproc = "Obtaining Server Set from user"
   WHILE (dcss_continue=1)
     SET message = window
     CALL clear(1,1)
     CALL box(1,1,3,132)
     CALL box(1,1,24,132)
     CALL text(2,2,"SERVER INSTANCE CONFIGURATION [CREATE NEW SERVER SET]")
     CALL text(2,70,"DOMAIN: ")
     CALL text(2,80,cnvtupper(dsim_cur_domain))
     SET dcss_continue2 = 1
     WHILE (dcss_continue2=1)
       CALL text(7,2,"Server Set Name: ")
       CALL accept(7,30,"P(30);CU",dcss_set_name
        WHERE curaccept > " ")
       SET dcss_set_name = trim(curaccept)
       IF (locateval(dcss_set_idx,1,dsir_srv_sets->set_cnt,dcss_set_name,dsir_srv_sets->sets[
        dcss_set_idx].set_name) > 0)
        CALL text(5,2,concat("**",dcss_set_name," is already in use. Please use another name."))
        SET dcss_continue2 = 1
       ELSE
        CALL clear(5,2,100)
        SET dcss_continue2 = 0
       ENDIF
     ENDWHILE
     CALL text(9,2,"Server Set Description: ")
     CALL accept(9,30,"P(70);C",dcss_set_desc)
     SET dcss_set_desc = trim(curaccept)
     SET dcss_continue2 = 1
     WHILE (dcss_continue2=1)
       CALL text(11,2,concat("Pre-Load Options: ",
         "(0)No Pre-load, (1)Cerner Default Set, (2)Existing Server Set, (3)Current Environment"))
       CALL text(12,2,"Your Selection: ")
       CALL accept(12,30,"9;",dcss_preload_choice
        WHERE curaccept IN (0, 1, 2, 3))
       SET dcss_preload_choice = curaccept
       IF (dcss_preload_choice=2)
        IF ((dsir_srv_sets->set_cnt=0))
         CALL text(5,2,concat("**",
           "There are no existing server sets. Please choose a different option."))
         SET dcss_continue2 = 1
        ELSE
         CALL clear(5,2,100)
         SET dcss_continue2 = 0
     ...
     SET dcss_continue2 = 1
     WHILE (dcss_continue2=1
      AND dcss_preload_choice=2)
       CALL text(14,2,"Server Set to Pre-Load: ")
       SET help = pos(9,2,10,128)
       SET help =
       SELECT INTO "nl:"
        server_set_name = substring(1,30,dsir_srv_sets->sets[t.seq].set_name), server_cnt =
        cnvtstring(dsir_srv_sets->sets[t.seq].server_cnt)
        FROM (dummyt t  WITH seq = value(dsir_srv_sets->set_cnt))
        ORDER BY dsir_srv_sets->sets[t.seq].set_name
        WITH nocounter
       ;end select
       CALL accept(14,30,"P(30);CUF",dcss_preload_set
        WHERE curaccept > " ")
       SET help = off
       SET dcss_preload_set = trim(curaccept)
       IF (locateval(dcss_set_idx,1,dsir_srv_sets->set_cnt,dcss_preload_set,dsir_srv_sets->sets[
        dcss_set_idx].set_name)=0)
        CALL text(5,2,concat("**",dcss_preload_set,
          " does not exist. Please provide an existing server set to pre-load."))
        SET dcss_continue2 = 1
       ELSE
        CALL clear(5,2,100)
        SET dcss_continue2 = 0
     ...
     CASE (curaccept)
      ...
      OF "M":
       SET dcss_continue = 1
      OF "C":
       IF (dcss_preload_choice > 0)
        IF (dsim_pre_load_servers(dcss_set_name,dcss_preload_choice,dcss_preload_set)=0)
         RETURN(0)
        ENDIF
       ENDIF
       SET dsir_srv_sets->current_set_name = dcss_set_name
       SET dsir_set_detail->set_name = dcss_set_name
       SET dsir_set_detail->set_desc = dcss_set_desc
       SET dsir_set_detail->date_modified = cnvtdatetime(curdate,curtime3)
       IF (dsir_insert_set_details(null)=0)
        RETURN(0)
       ENDIF
       SET dcss_continue = 0
     ENDCASE
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   IF ((dsir_srv_sets->current_set_name != "DM2NOTSET"))
    IF (dsim_modify_srv_set(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsim_pre_load_servers(dpls_set_name,dpls_preload_choice,dpls_preload_set)
   CASE (dpls_preload_choice)
    OF 1:
     IF (dsir_load_srv_set_details("CERNER_SHIPPED_DEFAULT",1)=0)
      RETURN(0)
     ENDIF
    OF 2:
     IF (dsir_load_srv_set_details(dpls_preload_set,1)=0)
      RETURN(0)
     ENDIF
    OF 3:
     IF (dsir_load_cur_env_set_details(dsim_cur_domain)=0)
      RETURN(0)
     ENDIF
     IF (dsir_copy_srv_details(null)=0)
      RETURN(0)
     ENDIF
   ENDCASE
 END ;Subroutine
 SUBROUTINE dsim_modify_srv_set(null)
   DECLARE dmss_server_name = vc WITH protect, noconstant("")
   DECLARE dmss_set_desc = vc WITH protect, noconstant("")
   DECLARE dmss_num_instances = i4 WITH protect, noconstant(0)
   DECLARE dmss_server_nbr = i4 WITH protect, noconstant(0)
   DECLARE dmss_continue = i2 WITH protect, noconstant(1)
   DECLARE dmss_continue2 = i2 WITH protect, noconstant(1)
   DECLARE dmss_server_idx = i4 WITH protect, noconstant(0)
   DECLARE dmss_seq = i4 WITH protect, noconstant(1)
   DECLARE dmss_index = i4 WITH protect, noconstant(1)
   DECLARE dmss_start = i4 WITH protect, noconstant(1)
   DECLARE dmss_end = i4 WITH protect, noconstant(1)
   DECLARE dmss_location = i4 WITH protect, noconstant(0)
   DECLARE dmss_exists_ind = i2 WITH protect, noconstant(0)
   IF ((dsir_srv_sets->current_set_name="DM2NOTSET"))
    IF (dsir_get_server_set(0)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((dsir_srv_sets->current_set_name="DM2NOTSET"))
    RETURN(1)
   ENDIF
   IF (dsir_load_srv_set_details(dsir_srv_sets->current_set_name,1)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Modify Server Set"
   WHILE (dmss_continue=1)
     SET message = window
     CALL clear(1,1)
     CALL box(1,1,3,132)
     CALL box(1,1,24,132)
     CALL text(2,2,"SERVER INSTANCE CONFIGURATION [VIEW/MODIFY SERVER SET]")
     CALL text(2,70,"DOMAIN: ")
     CALL text(2,80,cnvtupper(dsim_cur_domain))
     CALL text(4,2,"Server Set Name: ")
     CALL text(4,30,dsir_set_detail->set_name)
     CALL text(4,70,"Last Modified:")
     CALL text(4,85,format(cnvtdatetime(dsir_set_detail->date_modified),"DD-MMM-YYYY HH:MM:SS;;D"))
     CALL text(5,2,"Server Set Description: ")
     CALL text(5,30,dsir_set_detail->set_desc)
     CALL text(7,2,"LINE_NBR")
     CALL text(7,12,"SERVER_NAME")
     CALL text(7,60,"NUM_INSTANCES")
     CALL text(7,75,"REMOVE_IND.")
     CALL text(8,2,fillstring(90,"-"))
     SET dmss_seq = 1
     FOR (dmss_index = dmss_start TO least((dmss_start+ 9),dsir_set_detail->server_cnt))
       CALL text((8+ dmss_seq),2,concat(build(format(dmss_index,"###;P0")),") "))
       CALL text((8+ dmss_seq),12,dsir_set_detail->servers[dmss_index].server_name)
       CALL text((8+ dmss_seq),65,cnvtstring(dsir_set_detail->servers[dmss_index].num_instances))
       IF ((dsir_set_detail->servers[dmss_index].remove_ind=1))
        CALL text((8+ dmss_seq),75,"*")
       ENDIF
       SET dmss_end = (dmss_start+ dmss_seq)
       SET dmss_seq = (dmss_seq+ 1)
     ENDFOR
     IF ((dsir_set_detail->server_cnt > 10))
      IF (dmss_start=1)
       CALL text(20,2,"(Beginning of List, use <Down> key to scroll)")
      ELSEIF ((dsir_set_detail->server_cnt >= dmss_end))
     ...
     IF ((dsir_set_detail->server_cnt > 0))
      CALL text(23,2,
       "(M)odify Description, (A)dd Server, (U)pdate Server, (R)emove Server, (C)onfirm changes, (B)ack: "
       )
      CALL accept(23,100,"A;CUS"," "
       WHERE curaccept IN ("M", "A", "U", "R", "C",
       "B"))
     ELSE
      CALL text(23,2,"(M)odify Description, (A)dd Server, (C)onfirm changes, (B)ack: ")
      CALL accept(23,70,"A;CUS"," "
       WHERE curaccept IN ("M", "A", "R", "C", "B"))
     ENDIF
     CASE (curscroll)
      OF 0:
       CASE (curaccept)
        OF "M":
         ...
         CALL box(1,1,3,132)
         CALL box(1,1,24,132)
         CALL text(2,2,"SERVER INSTANCE CONFIGURATION [MODIFY SERVER SET DESCRIPTION]")
         CALL text(2,70,"DOMAIN: ")
         CALL text(2,80,cnvtupper(dsim_cur_domain))
         CALL text(5,2,"Server Set Name: ")
         CALL text(5,30,dsir_set_detail->set_name)
         CALL text(5,70,"Last Modified:")
         CALL text(5,85,format(cnvtdatetime(dsir_set_detail->date_modified),"DD-MMM-YYYY HH:MM:SS;;D"
           ))
         CALL text(6,2,"Server Set Description: ")
         CALL text(6,30,dsir_set_detail->set_desc)
         SET dmss_set_desc = dsir_set_detail->set_desc
         CALL text(9,2,"New Server Set Description: ")
         CALL accept(9,30,"P(70);C",dmss_set_desc)
         SET dmss_set_desc = curaccept
         CALL text(16,2,"Continue? (Y/N):")
         CALL accept(16,35,"p;cu","Y"
          WHERE curaccept IN ("Y", "N"))
         CASE (curaccept)
          OF "N":
           SET dmss_continue = 1
          OF "Y":
           SET dmss_continue = 1
           SET dsir_set_detail->set_desc = dmss_set_desc
         ENDCASE
        OF "A":
         SET message = window
         CALL clear(1,1)
         CALL box(1,1,3,132)
         CALL box(1,1,24,132)
         CALL text(2,2,"SERVER INSTANCE CONFIGURATION [ADD SERVER TO SERVER SET]")
         CALL text(2,70,"DOMAIN: ")
         CALL text(2,80,cnvtupper(dsim_cur_domain))
         CALL text(5,2,"Server Set Name: ")
         CALL text(5,30,dsir_set_detail->set_name)
         ...
         CALL text(6,2,"Server Set Description: ")
         CALL text(6,30,dsir_set_detail->set_desc)
         SET dmss_exists_ind = 0
         SET dmss_continue2 = 1
         WHILE (dmss_continue2=1)
           CALL text(10,2,"Server to Add:")
           CALL text(18,2,"Press <Shift><F5> for a list of servers in current environment")
           SET help =
           SELECT
            server_name = substring(1,50,dsir_curenv_set_detail->servers[t.seq].server_name),
            server_nbr = cnvtstring(dsir_curenv_set_detail->servers[t.seq].server_nbr)
            FROM (dummyt t  WITH seq = value(dsir_curenv_set_detail->server_cnt))
            ORDER BY dsir_curenv_set_detail->servers[t.seq].server_name
            WITH nocounter
           ;end select
           IF (check_error(dm_err->eproc)=1)
            CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
            RETURN(0)
           ENDIF
           CALL accept(10,40,"P(50);CU"," "
            WHERE curaccept > " ")
           SET help = off
           CALL clear(18,2,130)
           SET dmss_server_name = curaccept
           SET dmss_server_idx = locateval(dmss_index,1,dsir_curenv_set_detail->server_cnt,
            dmss_server_name,dsir_curenv_set_detail->servers[dmss_index].server_name)
           CALL clear(8,2,100)
           IF (locateval(dmss_index,1,dsir_set_detail->server_cnt,dmss_server_name,dsir_set_detail->
            servers[dmss_index].server_name) > 0)
            CALL text(8,2,concat("**",dmss_server_name,
              " has already been added to the current server set. ","Press <Enter> to continue..."))
            CALL accept(8,110,"p;cu"," "
             WHERE curaccept IN (" "))
            SET dmss_continue2 = 0
            SET dmss_exists_ind = 1
           ELSE
            SET dmss_continue2 = 0
           ENDIF
         ENDWHILE
         IF (dmss_exists_ind=0)
          IF (dmss_server_idx > 0)
           SET dmss_num_instances = dsir_curenv_set_detail->servers[dmss_server_idx].num_instances
           SET dmss_server_nbr = dsir_curenv_set_detail->servers[dmss_server_idx].server_nbr
           CALL text(11,2,"NumInstances value in current domain: ")
           CALL text(11,40,build(dsir_curenv_set_detail->servers[dmss_server_idx].num_instances))
          ELSE
           SET dmss_num_instances = 0
          ENDIF
          CALL text(12,2,"Desired NumInstances value: ")
          CALL accept(12,40,"9(2);",dmss_num_instances
           WHERE curaccept >= 0)
          SET dmss_num_instances = curaccept
          CALL text(16,2,"Continue? (Y/N):")
          CALL accept(16,35,"p;cu","Y"
           WHERE curaccept IN ("Y", "N"))
          CASE (curaccept)
           ...
           OF "Y":
            SET dmss_continue = 1
            SET dmss_server_idx = locateval(dmss_index,1,dsir_set_detail->server_cnt,dmss_server_name,
             dsir_set_detail->servers[dmss_index].server_name)
            IF (dmss_server_idx=0)
             SET dsir_set_detail->server_cnt = (dsir_set_detail->server_cnt+ 1)
             SET stat = alterlist(dsir_set_detail->servers,dsir_set_detail->server_cnt)
             SET dsir_set_detail->servers[dsir_set_detail->server_cnt].server_name = dmss_server_name
             SET dsir_set_detail->servers[dsir_set_detail->server_cnt].server_nbr = dmss_server_nbr
             SET dsir_set_detail->servers[dsir_set_detail->server_cnt].num_instances =
             dmss_num_instances
            ELSE
             SET dsir_set_detail->servers[dmss_server_idx].num_instances = dmss_num_instances
             SET dsir_set_detail->servers[dmss_server_idx].remove_ind = 0
            ENDIF
          ENDCASE
         ENDIF
        OF "U":
         CALL clear(20,2,129)
         CALL clear(21,2,129)
         CALL text(20,2,"Server to Update, by line number (O to go back): ")
         CALL accept(20,52,"999;H",0
          WHERE curaccept BETWEEN 0 AND dsir_set_detail->server_cnt)
         CALL clear(20,2,129)
         SET dmss_location = curaccept
         IF (dmss_location > 0)
          SET message = window
          CALL clear(1,1)
          CALL box(1,1,3,132)
          CALL box(1,1,24,132)
          CALL text(2,2,"SERVER INSTANCE CONFIGURATION [UPDATE SERVER]")
          CALL text(2,70,"DOMAIN: ")
          CALL text(2,80,cnvtupper(dsim_cur_domain))
          CALL text(5,2,"Server Set Name: ")
          CALL text(5,30,dsir_set_detail->set_name)
          CALL text(5,70,"Last Modified:")
          CALL text(5,85,format(cnvtdatetime(dsir_set_detail->date_modified),
            "DD-MMM-YYYY HH:MM:SS;;D"))
          CALL text(6,2,"Server Set Description: ")
          CALL text(6,30,dsir_set_detail->set_desc)
          SET dmss_num_instances = dsir_set_detail->servers[dmss_location].num_instances
          CALL text(10,2,"Server Name:")
          CALL text(10,40,dsir_set_detail->servers[dmss_location].server_name)
          CALL text(11,2,"Current NumInstances value: ")
          CALL text(11,40,build(dsir_set_detail->servers[dmss_location].num_instances))
          CALL text(12,2,"Desired NumInstances value: ")
          CALL accept(12,40,"9(2);",dmss_num_instances
           WHERE curaccept >= 0)
          SET dmss_num_instances = curaccept
          ...
          CASE (curaccept)
           ...
           OF "Y":
            SET dmss_continue = 1
            SET dsir_set_detail->servers[dmss_location].num_instances = dmss_num_instances
          ENDCASE
         ENDIF
        OF "R":
         CALL clear(20,2,129)
         CALL clear(21,2,129)
         CALL text(20,2,"Server to Remove, by line number (O to go back): ")
         CALL accept(20,52,"999;H",0
          WHERE curaccept BETWEEN 0 AND dsir_set_detail->server_cnt)
         CALL clear(20,2,129)
         SET dmss_location = curaccept
         IF (dmss_location > 0)
          IF ((dsir_set_detail->servers[dmss_location].remove_ind=1))
           CALL text(20,2,concat("**Server has already been marked for removal."))
          ELSE
           CALL text(20,2,concat("**Server ",dsir_set_detail->servers[dmss_location].server_name,
             " will be removed from this list."))
           CALL text(21,2,"Continue? (Y/N): ")
           CALL accept(21,20,"A;CU","N"
            WHERE curaccept IN ("Y", "N"))
           IF (curaccept="Y")
            SET dsir_set_detail->servers[dmss_location].remove_ind = 1
           ENDIF
          ENDIF
         ENDIF
        OF "C":
       ...
       ENDCASE
      OF 1:
       SET dmss_start = greatest(least((dmss_start+ 10),(dsir_set_detail->server_cnt - 9)),1)
      OF 2:
       SET dmss_start = greatest((dmss_start - 10),1)
      OF 5:
       SET dmss_start = greatest((dmss_start - 10),1)
      OF 6:
       SET dmss_start = greatest(least((dmss_start+ 10),(dsir_set_detail->server_cnt - 9)),1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsim_remove_srv_set(null)
   IF (dsir_get_server_set(0)=0)
    RETURN(0)
   ENDIF
   IF ((dsir_srv_sets->current_set_name != "DM2NOTSET"))
    IF (dsir_load_srv_set_details(dsir_srv_sets->current_set_name,1)=0)
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Remove Server Set"
    SET message = window
    CALL clear(1,1)
    CALL box(1,1,3,132)
    CALL box(1,1,24,132)
    CALL text(2,2,"SERVER INSTANCE CONFIGURATION [REMOVE SERVER SET]")
    CALL text(2,70,"DOMAIN: ")
    CALL text(2,80,cnvtupper(dsim_cur_domain))
    CALL text(4,2,"Server Set Name: ")
    CALL text(4,30,dsir_set_detail->set_name)
    ...
    CALL text(5,2,"Server Set Description: ")
    CALL text(5,30,dsir_set_detail->set_desc)
    CALL text(7,2,"The above Server Set will be completely removed. ")
    CALL text(9,2,"Are you sure you want to continue? (Y/N):")
    CALL accept(9,45,"A;CU","N"
     WHERE curaccept IN ("Y", "N"))
    IF (curaccept="Y")
     DELETE  FROM dm2_admin_dm_info di
      WHERE di.info_domain=concat("DM2_SRV_INST_CONFIG-",dsir_srv_sets->current_set_name)
    ...
    IF (dsir_clear_dsir_set_detail(null)=0)
     RETURN(0)
    ENDIF
    IF (dsir_clear_dsir_srv_sets(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
...
#exit_program
 ...
 SET message = nowindow
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_rr_srv_inst_maint has completed"
 ENDIF
 CALL final_disp_msg("dm2_srv_inst_maint")
END GO
