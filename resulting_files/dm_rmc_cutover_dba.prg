CREATE PROGRAM dm_rmc_cutover:dba
 ...
 SUBROUTINE drrm_check_freeze(dcf_env_id)
   ...
   RETURN(dcf_ret_val)
 END ;Subroutine
 DECLARE gen_tabs_to_cutover(i_source_env_id=f8,io_tab_name=vc(ref),p_r_tables=vc(ref),i_parent_rs=vc
  (ref)) = null
 DECLARE get_tab_to_cutover(i_source_env_id=f8,i_info_domain=vc,i_parent_rs=vc(ref)) = vc
 DECLARE single_tab_cutover(i_info_domain=vc,i_table_name=vc,i_source_id=f8,i_parent_rs=vc(ref),
  i_target_id=f8) = null
 DECLARE get_table_defs(v_table_name=vc) = null
 DECLARE drc_process_check(dpc_table_name=vc,dpc_info_domain=vc,dpc_source_env_id=f8,dpc_parent_rs=vc
  (ref),dpc_cbc_rs=vc(ref)) = i2
 DECLARE drc_rows_left_check(i_src_env_id=f8) = i2
 DECLARE drc_v_continue_ind = i2 WITH protect, noconstant(0)
 DECLARE drc_v_tab_name = vc WITH protect
 DECLARE drc_v_status = vc WITH protect
 DECLARE drc_info_domain = vc WITH protect
 DECLARE drc_p_t_name = vc WITH protect
 DECLARE drc_oracle_context = vc WITH protect
 DECLARE drc_target_id = f8 WITH protect, noconstant(0.0)
 DECLARE drc_cut_proc_cnt = i4 WITH protect, noconstant(0)
 DECLARE avr_dq = i4
 DECLARE drc_auto_cut_ind = i2 WITH protect, noconstant(0)
 DECLARE drc_open_event_reason = vc WITH protect, noconstant("")
 DECLARE drc_cur_process = vc WITH protect, noconstant("")
 IF ((validate(drc_work_ind,- (1))=- (1)))
  DECLARE drc_work_ind = i2 WITH protect, noconstant(0)
 ENDIF
 DECLARE drc_client_ident = vc WITH protect, noconstant("")
 DECLARE drc_drwcc_status = c1 WITH protect, noconstant("")
 DECLARE drc_cbc_ind = i2 WITH protect, noconstant(0)
 DECLARE drc_mock_ind = i2 WITH protect, noconstant(0)
 DECLARE drc_circ_reltn = i2 WITH protect, noconstant(0)
 DECLARE drc_mult_reltn = i2 WITH protect, noconstant(0)
 DECLARE drc_orig_err_ind = i4 WITH protect, noconstant(0)
 DECLARE drc_orig_emsg = vc WITH protect, noconstant("")
 DECLARE evaluate_pe_name() = c255
 FREE RECORD drc_contexts_to_cutover
 RECORD drc_contexts_to_cutover(
   1 values[*]
     2 str = vc
   1 len = i4
 ) WITH protect
 FREE RECORD drc_tabs
 RECORD drc_tabs(
   1 cnt = i4
   1 qual[*]
     2 table_name = vc
 )
 FREE RECORD drc_cbc_ctxts
 RECORD drc_cbc_ctxts(
   1 cur_ctxt_cnt = i4
   1 all_ind = i2
   1 cur_ctxt_qual[*]
     2 context_name = vc
   1 context_qual_str = vc
 )
 CALL check_logfile("DM_RMC_CUTOVER",".log","DM_RMC_CUTOVER")
 SET dm_err->eproc = "Beginning DM_RMC_CUTOVER"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET logfile_name = dm_err->logfile
 SET modify maxvarlen 268435456
 SET drc_client_ident = dci_get_rdds_identifier(null)
 CALL dci_set_rdds_identifier("RDDS CUTOVER")
 EXECUTE dm2_set_context "FIRE_RMC_TRG", "NO"
 EXECUTE dm2_set_context "FIRE_EA_TRG", "NO"
 IF ((validate(request->source_env_id,- (99))=- (99)))
  FREE RECORD request
  RECORD request(
    1 source_env_id = f8
    1 cutover_number = i4
  ...
  SET request->source_env_id = cnvtreal( $1)
  SET request->cutover_number = cnvtreal( $2)
  SET request->table_name =  $3
  DECLARE dm_cutover_run = vc
  SET dm_cutover_run = "Interactive Cutover"
 ELSE
  DECLARE dm_cutover_run = vc
  SET dm_cutover_run = "Full Cutover From Main"
 ENDIF
 EXECUTE dm2_set_context "FIRE_REFCHG_TRG_MVR", trim(cnvtstring(request->source_env_id,30))
 SELECT INTO "nl:"
  FROM gv$session g
  WHERE g.audsid=cnvtreal(currdbhandle)
  DETAIL
   drc_cur_process = g.process
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   drc_target_id = b.environment_id
  WITH nocounter
 ;end select
 IF (curqual=0)
 ...
 IF (drrm_check_mig(1,"") > 0)
  ...
  SET dm_err->emsg = concat("It has been detected that a database migration is in progress.  ",
   "There is an RDDS project strategy that allows for cutover to run during a migration, but it requires an acknowledgement ",
   "of the strategy through DM_MERGE_DOMAIN_ADM.  Please use that to start cutover.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining if we should turn off FIRE_REFCHG_TRG context."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_env_reltn d
  WHERE d.parent_env_id=drc_target_id
   AND d.relationship_type IN ("RDDS MOVER CHANGES NOT LOGGED", "REFERENCE MERGE")
  DETAIL
   IF ((d.child_env_id=request->source_env_id))
    IF (d.relationship_type="RDDS MOVER CHANGES NOT LOGGED")
     drc_circ_reltn = (drc_circ_reltn+ 1)
    ENDIF
   ELSE
    IF (d.relationship_type="REFERENCE MERGE")
     drc_mult_reltn = (drc_mult_reltn+ 1)
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  WHERE drel.rdds_event="Begin Reference Data Sync"
   ...
   AND drel.cur_environment_id=drc_target_id
   AND (drel.paired_environment_id != request->source_env_id)
  ...
  (SELECT
   ...
   WHERE cur_environment_id=drc_target_id
    AND (paired_environment_id != request->source_env_id)
 ...
 IF (curqual > 0)
  SET dm_err->emsg =
  "There are currently open events for alternate source environments, exiting process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->emsg = "There are no current open events for this source environment, exiting process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FREE RECORD drc_ready
 RECORD drc_ready(
   1 status = vc
   1 err_msg = vc
 ...
 EXECUTE dm_rmc_check_unprocessed  WITH replace("REPLY","DRC_READY")
 IF ((drc_ready->status="F"))
  SET dm_err->emsg = drc_ready->err_msg
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((drc_ready->cutover_ready_ind=0))
  SET dm_err->eproc = "Checking for unacknowledged dual build."
  SET dm_err->emsg = concat(
   "There are unacknowledged dual build changes detected which are preventing Cutover.  ",
   "Access the RDDS menu option 'Acknowledge Database Integrity Concerns for Cutover' to accept dual build risks to ",
   "proceed with Cutover.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Querying for Cutover By Context settings from DM_INFO."
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="RDDS CONFIGURATION"
   AND d.info_name="CUTOVER BY CONTEXT"
 ...
 IF (curqual > 0)
  SELECT INTO "nl:"
   ...
   HEAD REPORT
    cnt = 0
   DETAIL
    cnt = (cnt+ 1), stat = alterlist(drc_contexts_to_cutover->values,cnt), drc_contexts_to_cutover->
    values[cnt].str = d.info_name
   FOOT REPORT
    drc_contexts_to_cutover->len = cnt
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Checking for contexts to cutover"
   SET dm_err->emsg =
   "Cutover By Context functionality was enabled but there were no contexts to cutover set."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 SET drc_cbc_ind = dcs_check_cbc(null)
 IF (drc_cbc_ind < 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSEIF (drc_cbc_ind=0)
  SET drc_cbc_ctxts->context_qual_str = " 1=1 "
 ELSEIF (drc_cbc_ind=1)
  CALL dcs_gather_cbc_ctxt(drc_cbc_ctxts)
  IF ((drc_cbc_ctxts->cur_ctxt_cnt=0))
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Checking for contexts to cutover"
   SET dm_err->emsg =
   "Cutover By Context functionality was enabled but there were no contexts to cutover set."
   ...
   GO TO exit_program
  ENDIF
  IF ((drc_cbc_ctxts->all_ind=0))
   FOR (drc_iterator = 1 TO drc_cbc_ctxts->cur_ctxt_cnt)
    IF (drc_iterator > 1)
     SET drc_cbc_ctxts->context_qual_str = concat(drc_cbc_ctxts->context_qual_str," OR ")
    ENDIF
    SET drc_cbc_ctxts->context_qual_str = concat(drc_cbc_ctxts->context_qual_str,
     " concat('::',rdds_context_name,'::')"," = '*::",drc_cbc_ctxts->cur_ctxt_qual[drc_iterator].
     context_name,"::*' ")
   ENDFOR
  ELSE
   SET drc_cbc_ctxts->context_qual_str = " 1=1 "
 ...
 SET dm_err->eproc = "Querying for Cutover Continue Errors from DM_INFO."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="RDDS CUTOVER CONTINUE ERRORS"
   AND info_number >= 9000
  HEAD REPORT
   drc_cnt = 0
  DETAIL
   drc_cnt = (drc_cnt+ 1)
   IF (mod(drc_cnt,10)=1)
    stat = alterlist(drc_continue->qual,(drc_cnt+ 9))
   ENDIF
   drc_continue->qual[drc_cnt].err_msg = di.info_name, drc_continue->qual[drc_cnt].status_flag = di
   .info_number
  FOOT REPORT
   drc_continue->total = drc_cnt, stat = alterlist(drc_continue->qual,drc_continue->total)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   drc_oracle_context = d.info_char
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (trim(drc_oracle_context) > " ")
  CALL parser(concat("RDB ASIS(^ BEGIN DM2_CONTEXT_CONTROL('RDDS_CONTEXT','",drc_oracle_context,
    "'); END; ^) GO"),1)
 ELSE
  CALL parser(concat("RDB ASIS(^ BEGIN DM2_CONTEXT_CONTROL('RDDS_CONTEXT',''); END; ^) GO"),1)
 ENDIF
 FREE RECORD drc_parent
 RECORD drc_parent(
   1 cnt = i4
   1 list[*]
     2 parent_table = vc
 ...
 SELECT INTO "nl"
  FROM dm_refchg_version_r drvr
  DETAIL
   drc_parent->cnt = (drc_parent->cnt+ 1)
   IF (mod(drc_parent->cnt,10)=1)
    stat = alterlist(drc_parent->list,(drc_parent->cnt+ 9))
   ENDIF
   drc_parent->list[drc_parent->cnt].parent_table = drvr.parent_table, drc_parent->list[drc_parent->
   cnt].child_table = drvr.child_table
  WITH nocounter
 ;end select
 IF (check_error("Obtaining rows from dm_refchg_version_r table")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl"
  FROM dm_info di
  WHERE info_domain="RDDS CHILDREN CUTOVER"
  DETAIL
   drc_parent->cnt = (drc_parent->cnt+ 1)
   IF (mod(drc_parent->cnt,10)=1)
    stat = alterlist(drc_parent->list,(drc_parent->cnt+ 9))
   ENDIF
   drc_parent->list[drc_parent->cnt].parent_table = trim(substring(1,(findstring("::",di.info_name)
      - 1),di.info_name)), drc_parent->list[drc_parent->cnt].child_table = trim(substring((findstring
     ("::",di.info_name)+ 2),((size(di.info_name,1) - findstring("::",di.info_name))+ 2),di.info_name
     ))
  WITH nocounter
 ;end select
 IF (check_error("Obtaining parent child rows from dm_info table")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET stat = alterlist(drc_parent->list,drc_parent->cnt)
 FREE RECORD drc_r_tables
 RECORD drc_r_tables(
   1 r_cnt = i4
   1 r_table[*]
     2 r_t_name = vc
 ) WITH protect
 SET dm_err->eproc = "Processing $R tables"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET drc_info_domain = concat("RDDS CUTOVER::",trim(cnvtstring(request->source_env_id,20)))
 CALL add_tracking_row(request->source_env_id,concat("CUTOVER PROCESS",trim(cnvtstring(request->
     cutover_number))),"IN PROGRESS")
 SET stat = alterlist(auto_ver_request->qual,1)
 SET auto_ver_request->qual[1].rdds_event = "Cutover Process Started"
 SET auto_ver_request->qual[1].cur_environment_id = drc_target_id
 SET auto_ver_request->qual[1].paired_environment_id = request->source_env_id
 SET auto_ver_request->qual[1].event_reason = dm_cutover_run
 ...
 SET auto_ver_request->qual[1].detail_qual[1].event_detail2_txt = cnvtstring(drc_cur_process)
 SET stat = alterlist(auto_ver_request->qual[1].detail_qual,(1+ drc_contexts_to_cutover->len))
 FOR (i = 1 TO drc_contexts_to_cutover->len)
  SET auto_ver_request->qual[1].detail_qual[(i+ 1)].event_detail1_txt = "CONTEXT"
  SET auto_ver_request->qual[1].detail_qual[(i+ 1)].event_detail2_txt = drc_contexts_to_cutover->
  values[i].str
 ENDFOR
 EXECUTE dm_rmc_auto_verify_setup
 IF ((auto_ver_reply->status="F"))
 ...
 SELECT INTO "nl:"
  FROM dm_env_reltn d
  WHERE (d.parent_env_id=request->source_env_id)
   AND d.child_env_id=drc_target_id
   AND relationship_type="AUTO CUTOVER"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drc_auto_cut_ind = curqual
 IF (drc_auto_cut_ind=1)
  DELETE  FROM dm_info di
   WHERE di.info_domain=concat("RDDS CUTOVER::",cnvtstring(request->source_env_id,20))
    AND (di.info_number=- (1))
   WITH nocounter
  ;end delete
  IF (check_error("Clearing out any error $R rows in DM_INFO")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 COMMIT
 IF (drmmi_get_mock_id(drc_target_id)=drc_target_id)
  SET drc_mock_ind = 0
 ELSE
  SET drc_mock_ind = 1
 ENDIF
 SET drl_reply->status = ""
 SET drl_reply->status_msg = ""
 CALL get_lock("RDDS INS_SET CUTOVER","RDDS INS_SET CUTOVER",0,drl_reply)
 IF ((drl_reply->status="F"))
  CALL disp_msg(drl_reply->status_msg,dm_err->logfile,1)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ELSEIF ((drl_reply->status="Z"))
  WHILE ((drl_reply->status="Z"))
    SET drl_reply->status = ""
    SET drl_reply->status_msg = ""
    CALL pause(10)
    CALL check_lock("RDDS INS_SET CUTOVER","RDDS INS_SET CUTOVER",drl_reply)
    IF ((drl_reply->status="F"))
     CALL disp_msg(drl_reply->status_msg,dm_err->logfile,1)
     SET dm_err->err_ind = 1
     GO TO exit_program
 ...
 ELSE
  ...
  SET dm_get_request->table_name = "<NO_TABLE_LISTED>"
  SET stat = alterlist(dm_get_request->run_qual,dm_get_request->run_cnt)
  SET dm_get_request->run_qual[1].run_time_flag = 3
  EXECUTE dm_rmc_get_ins_set
  IF ((dm_get_reply->status="F"))
   SET dm_err->err_ind = 0
   SET drl_reply->status = ""
   SET drl_reply->status_msg = ""
   CALL remove_lock("RDDS INS_SET CUTOVER","RDDS INS_SET CUTOVER",currdbhandle,drl_reply)
   SET dm_err->emsg = dm_get_reply->status_msg
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ELSE
  ...
  ENDIF
 ENDIF
 IF (validate(request->table_name,"-9999") != "-9999"
  AND (request->table_name > " "))
  IF ((request->table_name="EA_USER"))
   EXECUTE dm_rmc_was_cutover_check  WITH replace("CTC_REC","DRC_CONTEXTS_TO_CUTOVER"), replace(
    "DRWCC_STATUS","DRC_DRWCC_STATUS")
   IF (drc_drwcc_status="F")
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
  ...
  ENDIF
  SET dm_get_request->run_cnt = 1
  SET dm_get_request->table_name = request->table_name
  SET stat = alterlist(dm_get_request->run_qual,dm_get_request->run_cnt)
  SET dm_get_request->run_qual[1].run_time_flag = 3
  EXECUTE dm_rmc_get_ins_set
  IF ((dm_get_reply->status="F"))
   ...
   GO TO exit_program
  ENDIF
  CALL single_tab_cutover(drc_info_domain,request->table_name,request->source_env_id,drc_parent,
   drc_target_id)
  SET drc_tabs->cnt = (drc_tabs->cnt+ 1)
  SET stat = alterlist(drc_tabs->qual,drc_tabs->cnt)
  SET drc_tabs->qual[drc_tabs->cnt].table_name = request->table_name
  SET dm_get_request->run_cnt = 1
  SET dm_get_request->table_name = request->table_name
  SET stat = alterlist(dm_get_request->run_qual,dm_get_request->run_cnt)
  SET dm_get_request->run_qual[1].run_time_flag = 4
  EXECUTE dm_rmc_get_ins_set
  IF ((dm_get_reply->status="F"))
   SET dm_err->emsg = dm_get_reply->status_msg
   SET dm_err->err_ind = 1
  ...
  ENDIF
 ELSE
  SET drc_v_continue_ind = 1
  WHILE (drc_v_continue_ind=1)
    SET drc_v_tab_name = ""
    SET drc_v_tab_name = get_tab_to_cutover(request->source_env_id,drc_info_domain,drc_parent)
    IF (drc_v_tab_name <= " ")
     CALL gen_tabs_to_cutover(request->source_env_id,drc_v_tab_name,drc_r_tables,drc_parent)
    ENDIF
    IF (drc_v_tab_name <= " ")
     SET drc_v_continue_ind = 0
    ELSE
     SET drc_p_t_name = get_reg_tab_name(drc_v_tab_name,"")
     IF (drc_p_t_name="EA_USER")
      EXECUTE dm_rmc_was_cutover_check  WITH replace("CTC_REC","DRC_CONTEXTS_TO_CUTOVER"), replace(
       "DRWCC_STATUS","DRC_DRWCC_STATUS")
      IF (drc_drwcc_status="F")
       ROLLBACK
     ...
     ENDIF
     SET dm_get_request->run_cnt = 1
     SET dm_get_request->table_name = drc_p_t_name
     SET stat = alterlist(dm_get_request->run_qual,dm_get_request->run_cnt)
     SET dm_get_request->run_qual[1].run_time_flag = 3
     EXECUTE dm_rmc_get_ins_set
     IF ((dm_get_reply->status="F"))
     ...
     ENDIF
     UPDATE  FROM dm_info
      SET info_date = sysdate, updt_applctx = cnvtreal(currdbhandle), updt_id = reqinfo->updt_id,
       updt_dt_tm = sysdate, updt_task = reqinfo->updt_task, updt_cnt = (updt_cnt+ 1)
      WHERE info_domain=drc_info_domain
       AND info_name=drc_v_tab_name
     ;end update
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ROLLBACK
      GO TO exit_program
     ENDIF
     DELETE  FROM dm_refchg_warning d
      WHERE d.table_name=drc_v_tab_name
       AND (d.source_env_id=request->source_env_id)
     ...
     SET drc_tabs->cnt = (drc_tabs->cnt+ 1)
     SET stat = alterlist(drc_tabs->qual,drc_tabs->cnt)
     SET drc_tabs->qual[drc_tabs->cnt].table_name = drc_p_t_name
     SET drc_work_ind = 1
     SET drc_v_status = cutover_get_and_run_dml(drc_p_t_name,request->source_env_id,1,1,drc_target_id
      )
     IF (drc_v_status != "F")
      DELETE  FROM dm_refchg_warning drw
       WHERE drw.table_name=drc_v_tab_name
        AND (drw.source_env_id=request->source_env_id)
      ...
      ENDIF
      UPDATE  FROM dm_info
       SET info_number = 2, updt_applctx = cnvtreal(currdbhandle), updt_id = reqinfo->updt_id,
        updt_dt_tm = sysdate, updt_task = reqinfo->updt_task, updt_cnt = (updt_cnt+ 1)
       WHERE info_domain=drc_info_domain
        AND info_name=drc_v_tab_name
      ;end update
     ...
     ELSE
      INSERT  FROM dm_refchg_warning
       SET dm_refchg_warning_id = seq(dm_clinical_seq,nextval), source_env_id = request->
        source_env_id, table_name = drc_v_tab_name,
        updt_applctx = cnvtreal(currdbhandle), updt_cnt = 0, updt_dt_tm = sysdate,
        updt_id = reqinfo->updt_id, updt_task = reqinfo->updt_task, warning_type =
      ...
      ENDIF
      UPDATE  FROM dm_info
       SET info_number = - (1), updt_applctx = cnvtreal(currdbhandle), updt_id = reqinfo->updt_id,
        updt_dt_tm = cnvtdatetime(curdate,curtime2), updt_task = reqinfo->updt_task, updt_cnt = (
        updt_cnt+ 1)
       WHERE info_domain=drc_info_domain
        AND info_name=drc_v_tab_name
      ;end update
    ...
    ENDIF
  ENDWHILE
  IF (drc_rows_left_check(request->source_env_id)=0)
   DELETE  FROM dm_refchg_warning drw
    WHERE (drw.source_env_id=request->source_env_id)
     AND drw.warning_type="TABLE PROCESSING ERROR"
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   UPDATE  FROM dm_info
    ...
    WHERE info_domain="RDDS CONFIGURATION"
     AND info_name=concat("LAST SUCCESSFUL CUT-OVER:",cnvtstring(request->source_env_id,20))
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ROLLBACK
   ...
   IF (curqual=0)
    INSERT  FROM dm_info
     SET info_domain = "RDDS CONFIGURATION", info_name = concat("LAST SUCCESSFUL CUT-OVER:",
       cnvtstring(request->source_env_id,20)), info_number = request->source_env_id,
      info_date = sysdate, updt_applctx = cnvtreal(currdbhandle), updt_id = reqinfo->updt_id,
      updt_dt_tm = sysdate, updt_task = reqinfo->updt_task, updt_cnt = 0
    ;end insert
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (drc_work_ind=1)
    SET stat = alterlist(auto_ver_request->qual,1)
    IF (drc_auto_cut_ind=0)
     SET auto_ver_request->qual[1].rdds_event = "Cutover Finished"
    ELSE
     SET auto_ver_request->qual[1].rdds_event = "Auto-Cutover Finished"
    ENDIF
    SET auto_ver_request->qual[1].cur_environment_id = drc_target_id
    SET auto_ver_request->qual[1].paired_environment_id = request->source_env_id
    SET stat = alterlist(auto_ver_request->qual[1].detail_qual,1)
    ...
    IF (drc_auto_cut_ind > 0
     ...
     SET drl_reply->status = ""
     SET drl_reply->status_msg = ""
     CALL get_lock("CUTOVER CLOSE_EVENT","CUTOVER CLOSE_EVENT",0,drl_reply)
     IF ((drl_reply->status="F"))
      CALL disp_msg(drl_reply->status_msg,dm_err->logfile,1)
      SET dm_err->err_ind = 1
      GO TO exit_program
     ELSEIF ((drl_reply->status="Z"))
      CALL echo("Lock already obtained, to close the event.  This session exiting.")
     ELSE
      SET drc_cut_proc_cnt = 1
      WHILE (drc_cut_proc_cnt != 0
       AND drc_cut_proc_cnt < 21)
        SELECT INTO "NL:"
         ...
         (SELECT
          ...
          AND d.rdbhandle_value != cnvtreal(currdbhandle)
         WITH nocounter
        ;end select
        ...
        IF (curqual > 0)
         CALL pause(30)
         SET drc_cut_proc_cnt = (drc_cut_proc_cnt+ 1)
        ELSE
         SET drc_cut_proc_cnt = curqual
        ENDIF
      ENDWHILE
      IF (drc_cut_proc_cnt > 0)
       SET dm_err->err_ind = 1
       SET dm_err->emsg =
       "Evidence of hanging cutover session detected.  Exiting auto-cutover session without closing event."
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      SET dm_err->eproc = "Determining open events for current source environment."
      SELECT INTO "nl:"
       ...
       (SELECT
        ...
        WHERE cur_environment_id=drc_target_id
         ...
         AND rdds_event_key="ENDREFERENCEDATASYNC")))
       DETAIL
        drc_open_event_reason = drel.event_reason
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ...
      ENDIF
      IF (curqual != 0)
       SET dm_err->eproc = "Adding End Bookmark event."
       CALL disp_msg(" ",dm_err->logfile,0)
       CALL add_tracking_row(request->source_env_id,"CLOSE OPEN EVENT","IN PROGRESS")
       EXECUTE dm_rmc_bookmark_end request->source_env_id, drc_target_id, drc_open_event_reason
       IF (check_error(dm_err->eproc)=1)
        SET dm_err->err_ind = 0
        SET drl_reply->status = ""
        SET drl_reply->status_msg = ""
        CALL remove_lock("CUTOVER CLOSE_EVENT","CUTOVER CLOSE_EVENT",currdbhandle,drl_reply)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        GO TO exit_program
       ELSE
        SET drl_reply->status = ""
  ...
  ENDIF
 ENDIF
 SUBROUTINE gen_tabs_to_cutover(i_source_env_id,io_tab_name,p_r_tables,i_parent_rs)
   DECLARE gttc_s_loop_cnt = f8 WITH protect
   DECLARE gttc_s_continue_ind = i2 WITH protect
   DECLARE gttc_r_loop_cnt = i4 WITH protect
   DECLARE gttc_cbc_ind = i2 WITH protect, noconstant(0)
   DECLARE gttc_rtp_ind = i2 WITH protect, noconstant(0)
   SET gttc_s_loop_cnt = 0
   SET gttc_s_continue_ind = 1
   SET gttc_r_loop_cnt = 1
   SET dm_err->eproc = "Generating DM_INFO rows"
   CALL disp_msg(" ",dm_err->logfile,0)
   WHILE (gttc_s_continue_ind=1
    AND gttc_s_loop_cnt <= 150)
     SET gttc_s_loop_cnt = (gttc_s_loop_cnt+ 1)
     SELECT INTO "nl:"
      "x"
      FROM dm_info d
      WHERE d.info_domain="RDDS CUTOVER"
       AND d.info_name="GENERATE DM_INFO TAB ROWS"
      WITH forupdate(d)
     ;end select
     IF (check_error(dm_err->eproc)=1)
      SET dm_err->err_ind = 0
     ...
     IF (curqual=0)
      SELECT INTO "nl:"
       FROM dm_info di
       WHERE di.info_domain="RDDS CUTOVER"
        AND di.info_name="GENERATE DM_INFO TAB ROWS"
       WITH nocounter
      ;end select
      IF (curqual=0)
       INSERT  FROM dm_info d
        SET d.info_domain = "RDDS CUTOVER", d.info_name = "GENERATE DM_INFO TAB ROWS"
       ;end insert
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ROLLBACK
      ...
      ELSE
       CALL pause(2)
       SET io_tab_name = get_tab_to_cutover(i_source_env_id,drc_info_domain,i_parent_rs)
      ENDIF
      IF (io_tab_name > " ")
       SET gttc_s_continue_ind = 0
      ENDIF
     ELSE
      SELECT INTO "nl:"
       ut.table_name
       ...
       (SELECT
        ...
        FROM dm_info
        WHERE info_domain=drc_info_domain
         AND info_name=ut.table_name
         AND ((info_number=0) OR ((((info_number=- (1))) OR (info_number=1
         AND updt_applctx IN (
        (SELECT
         audsid
         FROM gv$session)))) )) )))
       HEAD REPORT
        drc_r_tables->r_cnt = 0
       DETAIL
        drc_r_tables->r_cnt = (drc_r_tables->r_cnt+ 1)
        IF (mod(drc_r_tables->r_cnt,100)=1)
         stat = alterlist(drc_r_tables->r_table,(drc_r_tables->r_cnt+ 99))
        ENDIF
        drc_r_tables->r_table[drc_r_tables->r_cnt].r_t_name = ut.table_name
       FOOT REPORT
        stat = alterlist(drc_r_tables->r_table,drc_r_tables->r_cnt)
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      SET gttc_cbc_ind = dcs_check_cbc(null)
      IF (gttc_cbc_ind < 0)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      CALL dcs_gather_cbc_ctxt(drc_cbc_ctxts)
      WHILE ((gttc_r_loop_cnt <= drc_r_tables->r_cnt))
        CALL get_table_defs(drc_r_tables->r_table[gttc_r_loop_cnt].r_t_name)
        SET gttc_rtp_ind = 0
        IF (gttc_cbc_ind=1
         AND (drc_cbc_ctxts->all_ind=0))
         SELECT DISTINCT INTO "nl:"
          rt.rdds_context_name
          FROM (parser(drc_r_tables->r_table[gttc_r_loop_cnt].r_t_name) rt)
          WHERE rt.rdds_status_flag < 9000
           AND (rt.rdds_source_env_id=request->source_env_id)
          DETAIL
           FOR (i = 1 TO drc_cbc_ctxts->cur_ctxt_cnt)
             gttc_rtp_ind = (gttc_rtp_ind+ findstring(concat("::",trim(drc_cbc_ctxts->cur_ctxt_qual[i
                ].context_name),"::"),concat("::",trim(rt.rdds_context_name),"::")))
           ENDFOR
          WITH nocounter
         ;end select
        ELSE
        ...
        IF (gttc_rtp_ind > 0)
         UPDATE  FROM dm_info
          SET info_number = 0, updt_applctx = cnvtreal(currdbhandle), updt_id = reqinfo->updt_id,
           updt_dt_tm = cnvtdatetime(curdate,curtime3), updt_task = reqinfo->updt_task, updt_cnt = (
           updt_cnt+ 1)
          WHERE info_domain=drc_info_domain
           AND (info_name=drc_r_tables->r_table[gttc_r_loop_cnt].r_t_name)
         ;end update
         IF (check_error(dm_err->eproc)=1)
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          ROLLBACK
         ...
         IF (curqual=0)
          INSERT  FROM dm_info
           SET info_number = 0, updt_applctx = cnvtreal(currdbhandle), updt_id = reqinfo->updt_id,
            updt_dt_tm = cnvtdatetime(curdate,curtime3), updt_task = reqinfo->updt_task, updt_cnt = 0,
            info_domain = drc_info_domain, info_name = drc_r_tables->r_table[gttc_r_loop_cnt].
            r_t_name
          ;end insert
          IF (check_error(dm_err->eproc)=1)
           CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         ...
         ENDIF
        ENDIF
        SET gttc_r_loop_cnt = (gttc_r_loop_cnt+ 1)
      ENDWHILE
      COMMIT
      SET io_tab_name = get_tab_to_cutover(i_source_env_id,drc_info_domain,i_parent_rs)
      SET gttc_s_continue_ind = 0
   ...
   ENDWHILE
 END ;Subroutine
 SUBROUTINE get_tab_to_cutover(i_source_env_id,i_info_domain,i_parent_rs)
   DECLARE gttc_s_table_name = vc WITH protect
   DECLARE gttc_indx = i4
   DECLARE gttc_con_ind = i2
   SET dm_err->eproc = "Obtaining tables to cutover"
   CALL disp_msg(" ",dm_err->logfile,0)
   FREE RECORD gttc_exclude
   RECORD gttc_exclude(
     1 cnt = i4
     1 list[*]
       2 table_name = vc
   )
   SET gttc_exclude->cnt = 0
   SET gttc_con_ind = 1
   WHILE (gttc_con_ind=1)
     SET gttc_s_table_name = ""
     SELECT
      IF (size(gttc_exclude->list,5)=0)
       WHERE di.info_domain=i_info_domain
        AND ((di.info_number=0) OR (di.info_number=1
        AND  NOT (di.updt_applctx IN (
       (SELECT
        audsid
        FROM gv$session)))))
      ELSE
       ...
       (SELECT
        audsid
        FROM gv$session)))))
        AND  NOT (di.info_name IN (
       (SELECT
        d.info_name
        FROM dm_info d
        WHERE expand(gttc_indx,1,size(gttc_exclude->list,5),d.info_name,gttc_exclude->list[gttc_indx]
         .table_name))))
      ENDIF
      INTO "nl"
      FROM dm_info di
      DETAIL
       gttc_s_table_name = di.info_name
      WITH nocounter, forupdatewait(di), maxqual(di,1)
     ;end select
     IF (check_error("Obtaining table to cutover")=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ROLLBACK
      GO TO exit_program
     ENDIF
     IF (curqual > 0)
      IF (drc_process_check(gttc_s_table_name,i_info_domain,i_source_env_id,i_parent_rs,drc_cbc_ctxts
       )=0)
       SET gttc_exclude->cnt = (gttc_exclude->cnt+ 1)
       SET stat = alterlist(gttc_exclude->list,gttc_exclude->cnt)
       SET gttc_exclude->list[gttc_exclude->cnt].table_name = gttc_s_table_name
      ELSE
       SET gttc_con_ind = 0
      ENDIF
     ELSE
     ...
     ENDIF
   ENDWHILE
   IF (gttc_s_table_name > " ")
    UPDATE  FROM dm_info
     SET info_number = 1, updt_applctx = cnvtreal(currdbhandle)
     WHERE info_domain=i_info_domain
      AND info_name=gttc_s_table_name
    ;end update
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
   ...
   ENDIF
   COMMIT
   RETURN(gttc_s_table_name)
 END ;Subroutine
 SUBROUTINE single_tab_cutover(i_info_domain,i_table_name,i_source_id,i_parent_rs,i_target_id)
   DECLARE v_num_tabs = i4 WITH protect
   DECLARE v_table_name = vc WITH protect
   DECLARE v_tabs_counter = i4 WITH protect
   DECLARE v_proc_order = i4 WITH protect
   DECLARE stc_status = vc WITH protect
   DECLARE temp_v_table_name = vc WITH protect
   DECLARE stc_loop = i4 WITH protect
   DECLARE stc_r_tab_name_parent = vc WITH protect
   RECORD run_tabs(
     1 data[*]
       2 table_name = vc
   )
   CALL echo(build("single tab: ",i_table_name))
   SET v_table_name = i_table_name
   SET stat = alterlist(run_tabs->data,1)
   SET run_tabs->data[1].table_name = v_table_name
   SET v_num_tabs = 1
   SET dm_err->eproc = "Performing Cut-Over on a single table"
   CALL disp_msg(" ",dm_err->logfile,0)
   FOR (stc_loop = 1 TO i_parent_rs->cnt)
     IF ((i_parent_rs->list[stc_loop].child_table=v_table_name))
      SET stc_r_tab_name_parent = cutover_tab_name(i_parent_rs->list[stc_loop].parent_table,"")
      SELECT INTO "nl:"
       di.info_name
       FROM dm_info di
       WHERE info_domain=i_info_domain
        AND info_name=stc_r_tab_name_parent
        AND (info_number=- (1))
       WITH nocounter
      ;end select
      IF (check_error("Checking to see if there is a parent table that has had a processing error")=1
      )
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      IF (curqual != 0)
       CALL disp_msg(concat("There is a parent table that had a processing error: ",
         stc_r_tab_name_parent),dm_err->logfile,1)
       GO TO exit_program
      ENDIF
     ENDIF
   ENDFOR
   SELECT INTO "nl:"
    drvr.parent_table
    FROM dm_refchg_version_r drvr
    WHERE drvr.child_table=v_table_name
     AND drvr.child_table != "NAME_VALUE_PREFS"
    DETAIL
     v_num_tabs = (v_num_tabs+ 1), stat = alterlist(run_tabs->data,v_num_tabs), run_tabs->data[
     v_num_tabs].table_name = drvr.parent_table
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SET v_tabs_counter = v_num_tabs
   WHILE (v_num_tabs > 0)
     SET v_t_name = cutover_tab_name(run_tabs->data[v_num_tabs].table_name,"")
     SELECT INTO "nl:"
      info_number
      FROM dm_info
      WHERE info_domain=i_info_domain
       AND info_name=value(v_t_name)
       AND info_number=1
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
     ...
     IF (curqual != 0)
      ...
      CALL echo(concat("The table: ",run_tabs->data[v_num_tabs].table_name,"$R",
        " is currently being processed"))
      CALL echo("This process will quit without cutting it over.")
      CALL echo(
       "***********************************************************************************************"
       )
      CALL pause(5)
     ...
     ELSE
      UPDATE  FROM dm_info
       SET info_number = 1, updt_applctx = cnvtreal(currdbhandle), info_date = sysdate,
        updt_id = reqinfo->updt_id, updt_dt_tm = sysdate, updt_task = reqinfo->updt_task,
        updt_cnt = (updt_cnt+ 1)
       WHERE info_domain=i_info_domain
        AND info_name=value(v_t_name)
      ;end update
      ...
      ENDIF
      IF (curqual=0)
       SET v_proc_order = ((v_tabs_counter - v_num_tabs)+ 1)
       INSERT  FROM dm_info
        SET info_number = 1, info_char = cnvtstring(v_proc_order), info_date = sysdate,
         updt_applctx = cnvtreal(currdbhandle), updt_id = reqinfo->updt_id, updt_dt_tm = sysdate,
         updt_task = reqinfo->updt_task, updt_cnt = 0, info_domain = i_info_domain,
         info_name = v_t_name
       ;end insert
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ...
      ENDIF
      DELETE  FROM dm_refchg_warning d
       WHERE d.table_name=v_t_name
        AND d.source_env_id=i_source_id
        AND d.warning_type="TABLE PROCESSING ERROR"
       WITH nocounter
      ;end delete
      IF (check_error(dm_err->eproc)=1)
      ...
      ELSE
       COMMIT
      ENDIF
      SET temp_v_table_name = run_tabs->data[v_num_tabs].table_name
      CALL echo(build("About to run DML:",temp_v_table_name))
      SET stc_status = cutover_get_and_run_dml(temp_v_table_name,i_source_id,1,1,i_target_id)
      CALL echo(build("Result from DML:",stc_status))
      IF (stc_status != "F")
       DELETE  FROM dm_refchg_warning drw
        WHERE drw.table_name=v_t_name
         AND (drw.source_env_id=request->source_env_id)
         AND drw.warning_type="TABLE PROCESSING ERROR"
        WITH nocounter
       ;end delete
      ...
      ELSE
       INSERT  FROM dm_refchg_warning
        SET dm_refchg_warning_id = seq(dm_clinical_seq,nextval), source_env_id = i_source_id,
         table_name = v_t_name,
         updt_applctx = cnvtreal(currdbhandle), updt_cnt = 0, updt_dt_tm = sysdate,
         updt_id = reqinfo->updt_id, updt_task = reqinfo->updt_task, warning_type =
         "TABLE PROCESSING ERROR",
      ...
      ENDIF
     ENDIF
     SET v_num_tabs = (v_num_tabs - 1)
   ENDWHILE
 END ;Subroutine
 SUBROUTINE get_table_defs(v_table_name)
   SELECT INTO "nl:"
    FROM dtable dt
    WHERE dt.table_name=v_table_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    EXECUTE oragen3 value(v_table_name)
   ENDIF
 END ;Subroutine
 SUBROUTINE drc_process_check(dpc_table_name,dpc_info_domain,dpc_source_env_id,dpc_parent_rs,
  dpc_cbc_rs)
   DECLARE drc_loop = i4
   DECLARE drc_reg_tab_name_child = vc
   DECLARE drc_r_tab_name_parent = vc
   DECLARE drc_return = i2
   DECLARE drc_info_num = i4 WITH protect, noconstant(0)
   SET drc_reg_tab_name_child = get_reg_tab_name(dpc_table_name,"")
   SET dm_err->eproc = "Checking if it is ok to cutover the table"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET drc_return = 1
   FOR (drc_loop = 1 TO dpc_parent_rs->cnt)
     IF ((dpc_parent_rs->list[drc_loop].child_table=drc_reg_tab_name_child))
      SET drc_r_tab_name_parent = cutover_tab_name(dpc_parent_rs->list[drc_loop].parent_table,"")
      SELECT INTO "nl:"
       ut.table_name
       FROM user_tables ut
       WHERE ut.table_name=drc_r_tab_name_parent
       WITH nocounter
      ;end select
      IF (check_error("Checking to see if table exists")=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      IF (curqual != 0)
       SELECT INTO "nl:"
        di.info_name
        FROM dm_info di
        WHERE info_domain=dpc_info_domain
         AND info_name=drc_r_tab_name_parent
         AND info_number != 2
        DETAIL
         drc_info_num = di.info_number
        WITH nocounter
       ;end select
       IF (check_error("Checking to see if there is a parent table that has not been processed")=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        GO TO exit_program
       ENDIF
       IF (curqual != 0)
        SELECT INTO "nl:"
         ...
         WHERE table_name=dpc_table_name
          AND source_env_id=dpc_source_env_id
          AND warning_type="TABLE PROCESSING ERROR"
         WITH nocounter
        ;end select
        IF (curqual=0)
         INSERT  FROM dm_refchg_warning
          SET dm_refchg_warning_id = seq(dm_clinical_seq,nextval), source_env_id = dpc_source_env_id,
           table_name = dpc_table_name,
           updt_applctx = cnvtreal(currdbhandle), updt_cnt = 0, updt_dt_tm = sysdate,
           updt_id = reqinfo->updt_id, updt_task = reqinfo->updt_task, warning_type =
           "TABLE PROCESSING ERROR",
           message = concat("Table ",dpc_table_name," is unable to cutover since its parent table ",
            drc_r_tab_name_parent," has not completed cutover.")
          WITH nocounter
         ;end insert
         IF (check_error(dm_err->eproc)=1)
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       ...
       ELSE
        SELECT INTO "nl:"
         FROM (parser(drc_r_tab_name_parent) drt)
         WHERE drt.rdds_status_flag < 9000
          AND drt.rdds_source_env_id=dpc_source_env_id
          AND parser(dpc_cbc_rs->context_qual_str)
         WITH maxqual(drt,1)
        ;end select
        IF (check_error("Checking to see if there are any uncut rows in the $R table")=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         GO TO exit_program
        ENDIF
        IF (curqual > 0)
   ...
   RETURN(drc_return)
 END ;Subroutine
 SUBROUTINE drc_rows_left_check(i_src_env)
   DECLARE v_ret = i2 WITH protect, noconstant(0)
   DECLARE v_ndx = i2 WITH protect, noconstant(0)
   DECLARE v_temp_eproc = vc
   DECLARE drlc_uncut_cnt = i4 WITH protect, noconstant(0)
   RECORD tab(
     1 data[*]
       2 tab_name = vc
   )
   SET v_temp_eproc = dm_err->eproc
   SET dm_err->eproc = "drc_rows_left_check subroutine: Gathering list of $R tables"
   SELECT INTO "nl:"
    FROM user_tables ut
    WHERE table_name="*$R"
    ...
    HEAD REPORT
     t_cnt = 0
    DETAIL
     t_cnt = (t_cnt+ 1), stat = alterlist(tab->data,t_cnt), tab->data[t_cnt].tab_name = ut.table_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = "drc_rows_left_check subroutine: Checking $R tables for uncutover rows"
   FOR (v_ndx = 1 TO size(tab->data,5))
     SELECT INTO "nl:"
      cnt = count(*)
      FROM (parser(tab->data[v_ndx].tab_name) t)
      WHERE rdds_status_flag < 9000
       AND rdds_source_env_id=i_src_env
      DETAIL
       drlc_uncut_cnt = cnt
      WITH nocounter
     ;end select
     ...
     IF (drlc_uncut_cnt > 0)
      SET v_ret = 1
      SET v_ndx = (size(tab->data,5)+ 1)
     ENDIF
   ENDFOR
   SET dm_err->eproc = v_temp_eproc
   RETURN(v_ret)
 END ;Subroutine
#exit_program
 SET drc_orig_err_ind = dm_err->err_ind
 SET drc_orig_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 IF (drc_target_id > 0.0
  AND (request->source_env_id > 0.0)
  AND dm_cutover_run > " ")
  SET stat = alterlist(auto_ver_request->qual,1)
  SET auto_ver_request->qual[1].rdds_event = "Cutover Process Stopped"
  SET auto_ver_request->qual[1].cur_environment_id = drc_target_id
  SET auto_ver_request->qual[1].paired_environment_id = request->source_env_id
  SET auto_ver_request->qual[1].event_reason = dm_cutover_run
  SET stat = alterlist(auto_ver_request->qual[1].detail_qual,(drc_tabs->cnt+ 1))
  SET auto_ver_request->qual[1].detail_qual[1].event_detail1_txt = "PROCESS"
  SET auto_ver_request->qual[1].detail_qual[1].event_detail2_txt = cnvtstring(drc_cur_process)
  FOR (avr_dq = 1 TO drc_tabs->cnt)
    SET auto_ver_request->qual[1].detail_qual[(avr_dq+ 1)].event_detail1_txt = drc_tabs->qual[avr_dq]
    .table_name
  ENDFOR
  EXECUTE dm_rmc_auto_verify_setup
  IF ((auto_ver_reply->status="F"))
 ...
 IF (drc_circ_reltn=1
  AND drc_mult_reltn=0)
  EXECUTE dm2_set_context "FIRE_REFCHG_TRG", "YES"
 ENDIF
 IF (drc_client_ident > " ")
  CALL dci_set_rdds_identifier(drc_client_ident)
 ENDIF
 SET dm_err->err_ind = drc_orig_err_ind
 SET dm_err->emsg = drc_orig_emsg
 SET dm_err->eproc = "End dm_rmc_cutover"
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 CALL final_disp_msg("DM_RMC_CUTOVER")
END GO
