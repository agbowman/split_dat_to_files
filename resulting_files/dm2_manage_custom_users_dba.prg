CREATE PROGRAM dm2_manage_custom_users:dba
 ...
 END ;Subroutine
 IF ((validate(dmcu_user_list->cnt,- (1))=- (1))
  AND validate(dmcu_user_list->cnt,1)=1)
  FREE RECORD dmcu_user_list
  RECORD dmcu_user_list(
    1 change_ind = i2
    1 cnt = i4
    1 qual[*]
      2 user = vc
 ...
 ENDIF
 DECLARE dmcu_user_name = vc WITH protect, noconstant("")
 DECLARE dmcu_iter = i4 WITH protect, noconstant(0)
 DECLARE dmcu_index = i4 WITH protect, noconstant(0)
 DECLARE dmcu_start = i4 WITH protect, noconstant(1)
 DECLARE dmcu_end = i4 WITH protect, noconstant(1)
 DECLARE dmcu_row_seq = i4 WITH protect, noconstant(1)
 DECLARE dmcu_remove_location = i4 WITH protect, noconstant(0)
 DECLARE dmcu_cont = i2 WITH protect, noconstant(1)
 DECLARE dmcu_get_user_list(null) = i2
 IF (check_logfile("dm2_mng_cust_users",".log","dm2_manage_custom_users LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_manage_custom_users"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Determining if V500 user row exists in dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_VALID_CERNER_USER"
 ...
 ENDIF
 IF (curqual)
  SET dm_err->eproc = "Deleting V500 user row from dm_info."
 ...
 WHILE (dmcu_cont)
   ...
   CALL clear(1,1)
   CALL box(1,1,24,131)
   CALL text(2,56,"MANAGE CUSTOM USERS")
   CALL text(5,2,"List of Current Custom Users: ")
   SET dmcu_row_seq = 1
   FOR (dmcu_index = dmcu_start TO least((dmcu_start+ 9),dmcu_user_list->cnt))
     CALL text((6+ dmcu_row_seq),4,concat(build(format(dmcu_index,"###;P0")),") ",dmcu_user_list->
       qual[dmcu_index].user))
     SET dmcu_end = (dmcu_start+ dmcu_row_seq)
     SET dmcu_row_seq = (dmcu_row_seq+ 1)
   ENDFOR
   IF ((dmcu_user_list->cnt > 10))
    IF (dmcu_start=1)
     CALL text(18,2,"(Beginning of List, use <Down> key to scroll)")
    ELSEIF ((dmcu_user_list->cnt >= dmcu_end))
     CALL text(18,2,"(More available, use <Up>/<Down> keys to scroll)")
    ELSEIF ((dmcu_user_list->cnt < dmcu_end))
     CALL text(18,2,"(End of List, use <Up> key to scroll)")
    ENDIF
   ENDIF
   CALL text(23,2,"(A)dd user, (R)emove user, (E)xit: ")
   CALL accept(23,38,"A;CUS"," "
    WHERE curaccept IN ("A", "R", "E"))
   CASE (curscroll)
    OF 0:
     CASE (curaccept)
      OF "A":
       CALL text(20,2,"User to Add:")
       SET help =
       SELECT
        user = substring(1,30,au.username)
        FROM all_users au
        WHERE  NOT (au.username IN ("CDBA", "V500"))
         AND  NOT (au.username IN (
        (SELECT
         di.info_name
         FROM dm_info di
         WHERE di.info_domain IN ("DM2_CERNER_USER", "DM2_ORACLE_USER")
          AND di.info_number=1
         WITH nordbbindcons)))
         AND  NOT (expand(dmcu_index,1,dmcu_user_list->cnt,au.username,dmcu_user_list->qual[
         dmcu_index].user))
       ...
       IF (check_error(dm_err->eproc)=1)
        ...
        GO TO exit_script
       ENDIF
       CALL accept(20,34,"P(30);CUFH","NONE")
       CALL clear(20,2,129)
       SET dmcu_user_name = curaccept
       IF ( NOT (dmcu_user_name IN ("NONE", "V500", "CDBA"))
        AND  NOT (locateval(dmcu_index,1,dmcu_user_list->cnt,dmcu_user_name,dmcu_user_list->qual[
        dmcu_index].user)))
        SET dm_err->eproc = "Verifying selected user is a legitimate user from all_users."
        SELECT INTO "nl:"
         FROM all_users au
         WHERE au.username=dmcu_user_name
         WITH nocounter
        ;end select
        IF (check_error(dm_err->eproc)=1)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ...
        IF (curqual)
         SELECT INTO "nl:"
          ...
          WHERE di.info_domain IN ("DM2_CERNER_USER", "DM2_ORACLE_USER")
           AND di.info_number=1
           AND di.info_name=dmcu_user_name
          WITH nocounter
         ;end select
         IF (check_error(dm_err->eproc)=1)
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         ...
         ENDIF
         IF ( NOT (curqual))
          CALL text(20,2,concat("Ensure ",dmcu_user_name," user, when replicated to Target, ",
            "will not have data pointing back to Source (i.e. PROD)"))
          ...
          IF (curaccept="Y")
           ...
           INSERT  FROM dm_info di
            SET di.info_domain = "DM2_CUSTOM_USER", di.info_name = dmcu_user_name, di.info_date =
           ...
           IF (check_error(dm_err->eproc)=1)
            ...
            GO TO exit_script
           ENDIF
           SET dmcu_user_list->change_ind = 1
           SET dm2_process_event_rs->status = dpl_success
           SET dm2_process_event_rs->message = concat("User ",dmcu_user_name,
            " added to custom user list.")
           IF (dm2_process_log_row(dpl_custom_user_mgmt,dpl_auditlog,dpl_no_prev_id,1)=0)
            GO TO exit_script
           ENDIF
           COMMIT
          ENDIF
         ELSE
          CALL text(20,2,concat("User ",dmcu_user_name,
            " is a Cerner-shipped user. Press <Enter> to continue"))
          CALL accept(20,75,"P;CU"," "
           WHERE curaccept IN (" "))
         ENDIF
        ELSE
         CALL text(20,2,concat("User ",dmcu_user_name,
           " is not a valid database user. Press <Enter> to continue"))
         CALL accept(20,75,"P;CU"," "
          WHERE curaccept IN (" "))
        ENDIF
       ENDIF
      OF "R":
       CALL text(20,2,"User to Remove, by number (O to go back): ")
       CALL accept(20,45,"999;H",0
        WHERE curaccept BETWEEN 0 AND dmcu_user_list->cnt)
       CALL clear(20,2,129)
       SET dmcu_remove_location = curaccept
       IF (dmcu_remove_location > 0)
        CALL text(20,2,concat("User ",dmcu_user_list->qual[dmcu_remove_location].user,
          " will be removed from this list."))
        CALL text(21,2,"Continue? (Y/N): ")
        CALL accept(21,20,"A;CU","N"
         WHERE curaccept IN ("Y", "N"))
        IF (curaccept="Y")
         SET dm_err->eproc = "Deleting marked user rows from dm_info."
         DELETE  FROM dm_info di
          WHERE di.info_domain="DM2_CUSTOM_USER"
           AND (di.info_name=dmcu_user_list->qual[dmcu_remove_location].user)
          WITH nocounter
         ;end delete
         IF (check_error(dm_err->eproc)=1)
          ROLLBACK
         ...
         SET dmcu_user_list->change_ind = 1
         SET dm2_process_event_rs->status = dpl_success
         SET dm2_process_event_rs->message = concat("User ",dmcu_user_list->qual[dmcu_remove_location
          ].user," removed from custom user list.")
         IF (dm2_process_log_row(dpl_custom_user_mgmt,dpl_auditlog,dpl_no_prev_id,1)=0)
          GO TO exit_script
         ENDIF
         COMMIT
     ...
     ENDCASE
    OF 1:
     SET dmcu_start = greatest(least((dmcu_start+ 10),(dmcu_user_list->cnt - 9)),1)
    OF 2:
     SET dmcu_start = greatest((dmcu_start - 10),1)
    OF 5:
     SET dmcu_start = greatest((dmcu_start - 10),1)
    OF 6:
     SET dmcu_start = greatest(least((dmcu_start+ 10),(dmcu_user_list->cnt - 9)),1)
   ENDCASE
   IF ((dmcu_user_list->change_ind=1))
    IF (dmcu_get_user_list(null)=0)
     GO TO exit_script
    ENDIF
   ENDIF
 ENDWHILE
 GO TO exit_script
 SUBROUTINE dmcu_get_user_list(null)
   SET dm_err->eproc = "Querying for list of custom users from dm_info."
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="DM2_CUSTOM_USER"
    ORDER BY di.info_name
    HEAD REPORT
     dmcu_user_list->change_ind = 0, dmcu_user_list->cnt = 0, stat = alterlist(dmcu_user_list->qual,0
      )
    DETAIL
     dmcu_user_list->cnt = (dmcu_user_list->cnt+ 1), stat = alterlist(dmcu_user_list->qual,
      dmcu_user_list->cnt), dmcu_user_list->qual[dmcu_user_list->cnt].user = di.info_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dmcu_user_list)
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_script
 ...
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "dm2_manage_custom_users completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mng_cust_users")
END GO
