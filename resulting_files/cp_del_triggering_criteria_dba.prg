CREATE PROGRAM cp_del_triggering_criteria:dba
 PROMPT
 ...
 DECLARE deletetriggeringcriteria(null) = null
 RECORD del_triggering_criteria_reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 SUBROUTINE main(null)
   ...
   IF (table_access=2)
    IF (valid_pathway_id=1)
     CALL deletetriggeringcriteria(null)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE deletetriggeringcriteria(null)
   DECLARE sub_start_curtime3 = dq8 WITH private, constant(curtime3)
   CALL log_message("In deleteTriggeringCriteria()",log_level_debug)
   DELETE  FROM cp_triggering_criteria ctc
    WHERE ctc.cp_pathway_id=cp_pathway_id
   ;end delete
   CALL log_message(build("Exit deleteTriggeringCriteria(), Elapsed time in seconds:",((curtime3 -
     sub_start_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 CALL log_message(build("Starting Script:",log_program_name),log_level_debug)
 SET reqinfo->commit_ind = 0
 SET del_triggering_criteria_reply->status_data.status = "F"
 CALL main(null)
 SET del_triggering_criteria_reply->status_data.status = "S"
#exit_script
 IF ((del_triggering_criteria_reply->status_data.status="S"))
 ...
 CALL log_message(build("Total time in seconds:",((curtime3 - script_start_curtime3)/ 100.0)),
  log_level_debug)
 IF (( $OUTDEV != "NOFORMS"))
  CALL putjsonrecordtofile(del_triggering_criteria_reply)
 ENDIF
END GO
