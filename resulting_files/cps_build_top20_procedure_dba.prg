CREATE PROGRAM cps_build_top20_procedure:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ap
   ...
   WHERE nvp.parent_entity_id=ap.app_prefs_id
    ...
    AND nvp.pvc_name="TOP20_PROCEDURE_COUNT")
  HEAD REPORT
 ...
 SET ierrcode = 0
 SELECT INTO "nl:"
  item_knt = count(p.procedure_id), p.updt_id, p.nomenclature_id
  FROM procedure p
  PLAN (p
   WHERE p.updt_id > 0
  ...
  HAVING count(p.procedure_id) >= low_knt
 ...
 SET code_value = 0.0
 SET code_set = 25321
 SET cdf_meaning = "PROCEDURE"
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,1,procedure_cd)
 IF (stat > 0)
  SET failed = select_error
   ...
   SELECT INTO "nl:"
    ...
    PLAN (nc
     WHERE nc.parent_entity_name="PRSNL"
      ...
      AND nc.category_type_cd=procedure_cd
      AND nc.category_name="TOP_20_PROCEDURE")
    DETAIL
   ...
   IF (curqual < 1)
    ...
    INSERT  FROM nomen_category nc
     SET nc.nomen_category_id = hold->qual[i].nomen_category_id, nc.category_name =
      "TOP_20_PROCEDURE", nc.category_type_cd = procedure_cd,
