CREATE PROGRAM dm2_drop_nls_indexes:dba
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ddni_valid_nls = i2 WITH protect, noconstant(0)
 DECLARE ddni_idx = i4 WITH protect, noconstant(0)
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: Starting dm2_drop_nls_indexes"
 IF (check_logfile("dm2_drop_nls_indexes",".log","dm2_drop_nls_indexes LogFile...") != 1)
  GO TO exit_program
 ENDIF
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 SET ddni_valid_nls = 1
 SET ddni_valid_nls = sbr_check_for_a_nls(null)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 IF (ddni_valid_nls=0)
  SET dm_err->eproc = "check and load if there are NLS indexes that need to be dropped"
  CALL disp_msg("",dm_err->logfile,0)
  SELECT DISTINCT INTO "nl:"
   di.index_owner, di.index_name, di.table_owner,
   di.table_name
   FROM dba_ind_columns di
   WHERE operator(di.column_name,"ESCAPELIKE",patstring("*\_NLS"))
    AND di.table_name != "BIN$*"
   ...
   DETAIL
    ...
    ddr_ddl_req->ddl_cnt].ddl_object_owner = di.index_owner, ddr_ddl_req->ddls[ddr_ddl_req->ddl_cnt].
    ddl_table_owner = di.table_owner,
    ddr_ddl_req->ddls[ddr_ddl_req->ddl_cnt].ddl_table_name = di.table_name
    IF ((dm2_rdbms_version->level1 > 11))
     ddr_ddl_req->ddls[ddr_ddl_req->ddl_cnt].ddl_cmd = concat(" rdb DROP INDEX ",trim(build(
        ddr_ddl_req->ddls[ddr_ddl_req->ddl_cnt].ddl_object_owner,".",ddr_ddl_req->ddls[ddr_ddl_req->
        ddl_cnt].ddl_object_name))," online go")
    ELSE
     ddr_ddl_req->ddls[ddr_ddl_req->ddl_cnt].ddl_cmd = concat("rdb DROP INDEX ",trim(build(
        ddr_ddl_req->ddls[ddr_ddl_req->ddl_cnt].ddl_object_owner,".",ddr_ddl_req->ddls[ddr_ddl_req->
        ddl_cnt].ddl_object_name))," go")
    ENDIF
   WITH nocounter
  ;end select
 ...
 ELSE
  ...
  SET readme_data->message = "AutoSuccess: Valid NLS site"
  GO TO exit_program
 ENDIF
 SET readme_data->status = "S"
 SET readme_data->message = "Success: dm2_drop_nls_indexes completed with success"
#exit_program
 IF ((dm_err->err_ind=1))
 ...
 SET dm_err->eproc = "Ending dm2_drop_nls_indexes"
 CALL final_disp_msg("dm2_drop_nls_indexes")
END GO
