CREATE PROGRAM bhs_ma_error_signing_rpt3:dba
 PROMPT
  "Output to File/Printer/MINE " = "MINE",
  "From Date (MMDDYY HHMM) Default=Yesterday " = " ",
  "To Date (MMDDYY HHMM) Default=Today " = " "
 DECLARE from_date = vc WITH public, noconstant(" ")
 DECLARE to_date = vc WITH public, noconstant(" ")
 IF (( $2 > " "))
  SET from_date =  $2
 ELSE
  SET from_date = format(cnvtdatetime((curdate - 1),curtime),"mmddyy HHMM;;d")
 ENDIF
 IF (( $3 > " "))
  SET to_date =  $3
 ELSE
  SET to_date = format(cnvtdatetime(curdate,curtime),"mmddyy HHMM;;d")
 ENDIF
 RECORD dlrec(
   1 ord_total = i4
   1 seq[*]
 ...
 SET notif_status[6] = "No Longer Needed"
 SET notif_status[5] = "Admin-Cleared"
 SET notif_status[4] = "Forwarded"
 SET notif_status[3] = "Refused"
 SET notif_status[2] = "Completed"
 SET notif_status[1] = "Pending"
 ...
 SELECT INTO "nl:"
  ...
  , action_type = substring(1,30,uar_get_code_display(oa.action_type_cd)), pt_name = substring(1,30,p
   .name_full_formatted),
  sign_doctor = substring(1,30,pl.name_full_formatted), order_doctor = substring(1,30,pl3
   .name_full_formatted), order_person = substring(1,30,pl2.name_full_formatted),
  to_prsnl_name = substring(1,30,pl4.name_full_formatted), reason = uar_get_code_display(orn
   .notification_reason_cd), notif_comment = orn.notification_comment
  FROM order_review orv,
   order_notification orn,
   orders o,
   encounter e,
   organization org,
   order_action oa,
   ...
   prsnl pl4
  PLAN (orv
   WHERE orv.review_type_flag > 1
    AND orv.provider_id > 0)
   JOIN (orn
   WHERE orv.order_id=orn.order_id
    AND orv.action_sequence=orn.action_sequence
    AND orn.caused_by_flag > 0
    AND orn.notification_dt_tm BETWEEN cnvtdatetime(cnvtdate2(from_date,"MMDDYY"),cnvtint(substring(8,
      4,from_date))) AND cnvtdatetime(cnvtdate2(to_date,"MMDDYY"),cnvtint(substring(8,4,to_date))))
   JOIN (pl
   ...
   JOIN (pl3
   WHERE oa2.order_provider_id=pl3.person_id)
   JOIN (pl4
   WHERE outerjoin(orn.to_prsnl_id)=pl4.person_id)
   JOIN (p
  ...
  DETAIL
   ...
   IF (a=1)
    ...
    dlrec->seq[c].order_doctor = order_doctor, dlrec->seq[c].pt_name = pt_name, dlrec->seq[c].
    order_id = o.order_id,
    dlrec->seq[c].order_mnemonic = substring(1,80,o.order_mnemonic), dlrec->seq[c].order_comment_ind
     = comment_flag, dlrec->seq[c].clinical_display_line = o.clinical_display_line,
    dlrec->seq[c].reason = reason, dlrec->seq[c].notif_status = notif_status[orn
    .notification_status_flag], dlrec->seq[c].notif_comment = notif_comment,
    dlrec->seq[c].to_prsnl_name = to_prsnl_name, dlrec->seq[c].notification_dt_tm = format(orn
     .notification_dt_tm,"mm/dd/yy hh:mm;;d"), col 5,
    pt_name, row + 1, col 7,
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   dlrec->seq[d.seq].encntr_type = uar_get_code_display(e.encntr_type_cd)
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(dlrec->ord_total)),
   ...
   long_text l
  PLAN (d
   WHERE (dlrec->seq[d.seq].order_comment_ind=1))
   JOIN (oc
   WHERE (oc.order_id=dlrec->seq[d.seq].order_id)
    AND oc.comment_type_cd=ord_comment_cd)
   JOIN (l
   WHERE l.long_text_id=oc.long_text_id)
  ORDER BY d.seq, oc.order_id, oc.action_sequence DESC
  HEAD oc.order_id
   seq_flag = 0
  DETAIL
   IF (seq_flag=0)
    dlrec->seq[d.seq].order_comment = l.long_text, seq_flag = 1
   ENDIF
  FOOT  oc.order_id
   col + 0
  WITH nocounter
 ...
 SELECT INTO  $1
  HEAD REPORT
   ...
   xcol5 = 228, xcol6 = 273, xcol7 = 338,
   xcol8 = 388, xcol9 = 428, xcol10 = 433,
   xcol11 = 477, xcol12 = 520,
   MACRO (rowplusone)
   ...
   ,
   MACRO (check_break)
    lns = 50, lns = (lns+ (10 * (size(trim(dlrec->seq[c].clinical_display_line))/ 110))), lns = (lns
    + (10 * (size(trim(dlrec->seq[c].order_comment))/ 100)))
    IF (((yrow1+ lns) > 720))
     yrow1 = 5, BREAK
    ENDIF
   ENDMACRO
   ...
   MACRO (line_wrap)
    hlimit = 0
    IF (findstring(char(10),tempstring) > 0)
     hold_tempstring = tempstring
     WHILE (findstring(char(10),hold_tempstring) > 0
      AND hlimit < 1000)
       hlimit = (hlimit+ 1), xx = (findstring(char(10),hold_tempstring) - 1), ll = (size(
        hold_tempstring) - xx),
       tempstring = substring(1,xx,hold_tempstring), hold_tempstring = substring((xx+ 2),ll,
        hold_tempstring), limit = 0
       IF (maxlen=0)
        maxlen = 20
       ENDIF
       WHILE (tempstring > " "
  ...
  HEAD PAGE
   ...
   CALL print(calcpos(xcol2,yrow1)), "{b}ERROR SIGNING REPORT  {endb}", rowplusone2,
   CALL print(calcpos(xcol2,yrow1)), "Report Range: ", from_date,
   " to ", to_date, rowplusone2,
   ...
   "{b/5}Order",
   CALL print(calcpos(xcol6,yrow1)), "{b/3}MRN",
   CALL print(calcpos(xcol7,yrow1)), "{b/5}ACCT#",
   CALL print(calcpos(xcol10,yrow1)),
   "{b/8}Location",
   CALL print(calcpos(xcol11,yrow1)), "{b/4}Room",
   CALL print(calcpos(xcol12,yrow1)), "{b/4}Type", yrow1 = (yrow1+ 4),
   CALL print(calcpos(xcol1,yrow1)), line1, rowplusone2,
   CALL print(calcpos(xcol1,yrow1)), "{B}The Following Were Signed In Error by: ", dlrec->seq[c].
   sign_doctor,
   "{endb}", rowplusone2, rowplusone2
  DETAIL
   FOR (c = 1 TO dlrec->ord_total)
     ...
     CALL print(calcpos(xcol7,yrow1)), dlrec->seq[c].account_nbr,
     CALL print(calcpos(xcol10,yrow1)),
     dlrec->seq[c].nurse_unit,
     CALL print(calcpos(xcol11,yrow1)), dlrec->seq[c].room_bed,
     CALL print(calcpos(xcol12,yrow1)), dlrec->seq[c].encntr_type, rowplusone,
     CALL print(calcpos(xcol2,yrow1)), "Order Entered By: ", dlrec->seq[c].order_person,
     rowplusone,
     CALL print(calcpos(xcol2,yrow1)), "Entered For: ",
     dlrec->seq[c].order_doctor, rowplusone,
     CALL print(calcpos(xcol2,yrow1)),
     "Order: ", dlrec->seq[c].order_id"########",
     CALL print(calcpos(xcol4,yrow1)),
     dlrec->seq[c].order_mnemonic, rowplusone,
     CALL print(calcpos(xcol2,yrow1)),
     "Order Detail: ", xcolvar = xcol3, maxlen = 110,
     tempstring = dlrec->seq[c].clinical_display_line, line_wrap
     IF ((dlrec->seq[c].clinical_display_line <= " "))
      rowplusone
     ENDIF
     CALL print(calcpos(xcol2,yrow1)), "Order Comment: ", xcolvar = (xcol3+ 10),
     maxlen = 100, tempstring = dlrec->seq[c].order_comment, line_wrap
     IF ((dlrec->seq[c].order_comment <= " "))
      rowplusone
     ENDIF
     CALL print(calcpos(xcol2,yrow1)), "Notification to : ", dlrec->seq[c].to_prsnl_name,
     rowplusone,
     CALL print(calcpos(xcol2,yrow1)), "Notification Reason : ",
     dlrec->seq[c].reason, rowplusone,
     CALL print(calcpos(xcol2,yrow1)),
     "Notification Status : ", dlrec->seq[c].notif_status, rowplusone,
     CALL print(calcpos(xcol2,yrow1)), "Notification Comment: ", xcolvar = (xcol3+ 30),
     maxlen = 960, tempstring = dlrec->seq[c].notif_comment, line_wrap
     IF ((dlrec->seq[c].notif_comment <= " "))
      rowplusone
     ENDIF
     CALL print(calcpos(xcol2,yrow1)), "Notification dttm : ", dlrec->seq[c].notification_dt_tm,
     rowplusone, rowplusone
     IF ((dlrec->seq[c].break_flag=1))
      CALL print(calcpos(260,(yrow1+ 20))), "**** End of Report ****", row + 1,
      yrow1 = 5, BREAK
     ENDIF
   ENDFOR
  FOOT PAGE
