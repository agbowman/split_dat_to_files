CREATE PROGRAM cdi_rpt_create_del_by_usr_lyt
 PROMPT
 ...
 EXECUTE cdi_rpt_create_del_by_usr_drvr  $BEGINDATE,  $ENDDATE
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(concat(uar_i18ngetmessage(_hi18nhandle,
        ...
        "HeadReportSection_title1","Documents Created and Deleted by User and Date")),char(0)))
   ENDIF
   SET rptsd->m_flags = 16
   SET rptsd->m_y = (offsety+ 0.56)
   SET rptsd->m_x = (offsetx+ 0.75)
   SET rptsd->m_width = 5.94
 ...
 SUBROUTINE footbatch_lyt_batch_details_usernamesectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(total_created_per_user),
       char(0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(total_deleted_per_user),
       char(0)))
    ENDIF
   ENDIF
   SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
 ...
 SUBROUTINE footreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(grand_total_deleted),char(
        0)))
    ENDIF
   ENDIF
   SET rptsd->m_y = (offsety+ 0.00)
   ...
   IF (ncalc=0)
    IF (size(trim(batch_lyt_batch_details_username)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(cnvtstring(grand_total_created),char(
        0)))
    ENDIF
   ENDIF
   SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CDI_RPT_CREATE_DEL_BY_USR_LYT"
