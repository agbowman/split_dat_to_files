CREATE PROGRAM cv_da_upt_cv_order_category_r:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_upt_cv_order_category_r_vrsn = vc WITH private, constant("20080311")
 ...
 SET msdatablename = "CV_ORDER_CATEGORY_R"
 ...
 WHILE (i <= size(request->objarray,5))
   ...
   SELECT INTO "nl:"
    c.*
    FROM cv_order_category_r c
    WHERE (c.cv_order_category_r_id=request->objarray[i].cv_order_category_r_id)
    DETAIL
     IF ((validate(request->objarray[i].cv_order_category_id,c.cv_order_category_id) != - (0.00001))
      AND validate(request->objarray[i].cv_order_category_id,c.cv_order_category_id) != c
     ...
     IF ((validate(request->objarray[i].catalog_cd,c.catalog_cd) != - (0.00001))
      ...
      alteredind = 1
     ENDIF
     IF (validate(request->objarray[i].detail_txt,c.detail_txt) != char(128)
      AND validate(request->objarray[i].detail_txt,c.detail_txt) != c.detail_txt)
      alteredind = 1
     ENDIF
   ...
   ENDIF
   IF (alteredind > 0)
    UPDATE  FROM cv_order_category_r c
     SET c.cv_order_category_id =
      IF ((validate(request->objarray[i].cv_order_category_id,- (0.00001)) != - (0.00001))) validate(
        request->objarray[i].cv_order_category_id,- (0.00001))
      ELSE c.cv_order_category_id
      ENDIF
      , c.catalog_cd =
      ...
      ENDIF
      , c.detail_txt =
      IF (validate(request->objarray[i].detail_txt,char(128)) != char(128)) validate(request->
        objarray[i].detail_txt,char(128))
      ELSE c.detail_txt
      ENDIF
      ,
