CREATE PROGRAM ct_trial_prescreen_test:dba
 PROMPT
 ...
 DECLARE rpt_title = vc WITH constant(uar_i18ngetmessage(i18nhandle,"TEST_SCREENING_PRESCREEN_RPT",
   "Test Pre-Screening"))
 ...
 CALL echo(concat("$encntrTypeCd = ",build(parameter(5,0))))
 CALL echo(concat("$facilityCd = ",build(parameter(6,0))))
 CALL echo(concat("$triggerName = ",build(parameter(7,0))))
 CALL echo(concat("Execution Mode = ", $EXECMODE))
 CALL echo(concat("Gender = ",cnvtstring(cnvtreal( $GENDER))))
 CALL echo(concat("Qualifier = ",cnvtstring(cnvtreal( $QUALIFIER))))
 CALL echo(concat("Age1 = ",cnvtstring(cnvtint( $AGE1))))
 CALL echo(concat("Age2 = ",cnvtstring(cnvtint( $AGE2))))
 ...
 CALL echo(concat("eParse for protocol paramater= ",eparse))
 SET bset = 0
 IF (eparse="C1")
  ...
  EXECUTE ct_get_protocol_access 0 WITH replace("PROTLIST","CALLING_REPLY")
  SET bset = 1
 ...
 IF (bset=0)
  ...
  FOR (indx = 1 TO paramlists->protcnt)
   ...
   CALL echo(concat("Protocol to check:  ",eksctrequest->checkct[indx].primary_mnemonic))
  ENDFOR
  SELECT INTO "nl:"
   pm.prot_master_id
   FROM prot_master pm,
 ...
 SET cnt = 0
 CALL echo("before big query")
 SELECT
  IF (( $EVALBY < 2))DISTINCT INTO value(toutputdev)
  ...
  ELSEIF (( $EVALBY=2))DISTINCT INTO value(toutputdev)
   e.person_id, e.encntr_id, p.sex_cd,
   p.birth_dt_tm
   FROM encounter e,
    person p,
  ...
  HEAD REPORT
   cnt = 0,
   CALL echo("person_id encntr_id"), personage = (datetimediff(cnvtdatetime(sysdate),p.birth_dt_tm,1)
   / 365.25)
  HEAD p.person_id
   bfound = 0
   IF (age_qual != "1=1")
   ...
   ENDIF
   IF (bfound=1)
    CALL echo(concat("p_id:  ",build(p.person_id),"  ",substring(1,25,p.name_full_formatted),"  ",
     trim(uar_get_code_display(e.loc_facility_cd))," has loc_facility_cd of ",build(e.loc_facility_cd
      )," age: ",build(personage))), interest = " ", cnt += 1
    IF (mod(cnt,100)=1)
    ...
    race_cd = p.race_cd,
    CALL echo(concat("EncntrTypeCd = ",build(e.encntr_type_cd),"   ",uar_get_code_display(e
      .encntr_type_cd)))
   ENDIF
  DETAIL
   CALL echo(concat("   ",build(p.person_id),"  ",build(e.encntr_id)))
  FOOT REPORT
 ...
 FOR (chunk_index = 1 TO chunks)
   ...
   IF (cnt
    ...
    ENDIF
    EXECUTE ct_run_prescreen  WITH replace("REQUEST","EKSCTREQUEST"), replace("JOBDETAILREQUEST",
     "RUNPSJOBREQUEST")
       ...
       FOR (chunk_reply_index = 1 TO chunk_reply_index_size)
         ...
         IF (chunked_reply_qual_ctcnt)
          ...
          FOR (chunk_reply_ctqual_index = 1 TO chunked_reply_qual_ctcnt)
            ...
            pt_prot_prescreen_id = eksctreply->qual[chunk_reply_index].ctqual[
            chunk_reply_ctqual_index].pt_prot_prescreen_id
            SET consolidated_chunks_reply->qual[reply_index].ctqual[chunk_reply_ctqual_index].
            ...
            ELSE
             ...
             FOR (k = 1 TO prot_cnt)
              IF ((protlist->protqual[k].prot_master_id=eksctreply->qual[i].ctqual[j].prot_master_id)
               ...
               person_id
               SET protlist->protqual[k].personqual[person_cnt].comment = eksctreply->qual[i].ctqual[
               j].comment
