CREATE PROGRAM bhs_athn_scratchpad_alert_v4
 FREE RECORD result
 RECORD result(
   1 person_id = f8
   1 encntr_id = f8
 ...
 IF (( $24 > 0.0))
  SET result->person_id =  $22
  SET result->birth_dt_tm =  $23
  SET result->sex_cd =  $24
 ELSE
  SELECT INTO "NL:"
   FROM person p
   PLAN (p
    WHERE (p.person_id= $22)
     AND p.active_ind=1
 ...
 ENDIF
 IF (( $26 > 0.0))
  SET result->catalog_type_cd =  $26
  SET result->activity_type_cd =  $27
  SET result->activity_subtype_cd =  $28
 ELSE
  SELECT INTO "NL:"
 ...
 ENDIF
 IF (size(trim( $30,3)) > 0)
  SET detaillistcnt = (size(rep_oeparse->detaillist,5)+ 1)
  CALL echo(build("DETAILLISTCNT:",detaillistcnt))
  SET stat = alterlist(rep_oeparse->detaillist,detaillistcnt)
  SET rep_oeparse->detaillist[detaillistcnt].oefieldid = 0.0
  SET rep_oeparse->detaillist[detaillistcnt].oefieldvalue = 1
  SET rep_oeparse->detaillist[detaillistcnt].oefielddisplayvalue = "Do Check Continuous"
  SET rep_oeparse->detaillist[detaillistcnt].oefielddttmvalue = 0.0
  SET rep_oeparse->detaillist[detaillistcnt].oefieldmeaning = "CHECKCONTINUOUS"
 ENDIF
 DECLARE pharmacy_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",6000,"PHARMACY"))
 IF ((result->catalog_type_cd=pharmacy_cd))
  SELECT INTO "NL:"
   FROM order_catalog_synonym ocs
   PLAN (ocs
    WHERE (ocs.catalog_cd= $9)
     AND (ocs.synonym_id= $8)
     AND ocs.active_ind=1)
   HEAD ocs.synonym_id
    result->rx_mask = ocs.rx_mask, result->intermittent_ind = ocs.intermittent_ind
   WITH nocounter, time = 30
  ;end select
  SET pos = locateval(locidx,1,size(rep_oeparse->detaillist,5),"PHARMORDERTYPE",rep_oeparse->
   detaillist[locidx].oefieldmeaning)
  IF (pos=0)
   SET detaillistcnt = (size(rep_oeparse->detaillist,5)+ 1)
   CALL echo(build("DETAILLISTCNT:",detaillistcnt))
   SET stat = alterlist(rep_oeparse->detaillist,detaillistcnt)
   SET pos = detaillistcnt
   SET rep_oeparse->detaillist[pos].oefieldid = 12771
   SET rep_oeparse->detaillist[pos].oefielddttmvalue = 0.0
   SET rep_oeparse->detaillist[pos].oefieldmeaning = "PHARMORDERTYPE"
  ENDIF
  IF (band(result->rx_mask,1) > 0)
   SET rep_oeparse->detaillist[pos].oefieldvalue = 2
   SET rep_oeparse->detaillist[pos].oefielddisplayvalue = "2"
  ELSEIF ((result->intermittent_ind=1))
   SET rep_oeparse->detaillist[pos].oefieldvalue = 3
   SET rep_oeparse->detaillist[pos].oefielddisplayvalue = "3"
  ELSE
   SET rep_oeparse->detaillist[pos].oefieldvalue = 1
   SET rep_oeparse->detaillist[pos].oefielddisplayvalue = "1"
  ENDIF
 ENDIF
 IF (size(trim( $30,3)) > 0)
...
#exit_script
 ...
 SUBROUTINE calldiscerndialogueserver(null)
   ...
   SET dialoguerequest->orderlist[1].actiontypecd = evaluate( $29,1,0.0,c_order_cd)
   ...
   IF (size(result->subcomponents,5) > 0)
    SET stat = alterlist(dialoguerequest->orderlist[1].ingredientlist,size(result->subcomponents,5))
    FOR (idx = 1 TO size(result->subcomponents,5))
      SET dialoguerequest->orderlist[1].ingredientlist[idx].ivseq = idx
      SET dialoguerequest->orderlist[1].ingredientlist[idx].catalogcd = result->subcomponents[idx].
      catalog_cd
      SET dialoguerequest->orderlist[1].ingredientlist[idx].synonymid = result->subcomponents[idx].
      synonym_id
      IF (size(trim(result->subcomponents[idx].free_text_dose,3))=0)
       IF ((result->subcomponents[idx].str_dose > 0))
        SET dialoguerequest->orderlist[1].ingredientlist[idx].strengthdose = result->subcomponents[
        idx].str_dose
        SET dialoguerequest->orderlist[1].ingredientlist[idx].strengthunit = result->subcomponents[
        idx].str_dose_unit_cd
       ENDIF
       IF ((result->subcomponents[idx].vol_dose > 0))
        SET dialoguerequest->orderlist[1].ingredientlist[idx].volumedose = result->subcomponents[idx]
        .vol_dose
        SET dialoguerequest->orderlist[1].ingredientlist[idx].volumeunit = result->subcomponents[idx]
        .vol_dose_unit_cd
       ENDIF
      ELSE
       SET dialoguerequest->orderlist[1].ingredientlist[idx].freetextdose = result->subcomponents[idx
       ].free_text_dose
      ENDIF
      SET dialoguerequest->orderlist[1].ingredientlist[idx].bag_frequency_cd = result->subcomponents[
      idx].frequency_cd
      SET dialoguerequest->orderlist[1].ingredientlist[idx].normalized_rate = result->subcomponents[
      idx].normalized_rate
      SET dialoguerequest->orderlist[1].ingredientlist[idx].normalized_rate_unit = result->
   ...
   IF ((dialoguereply->status_data.status != "F"))
    ...
    FOR (idx = 1 TO alertcnt)
      ...
      SET result->alerts[idx].text = trim(replace(replace(replace(replace(dialoguereply->alerts[idx].
           text,"’","'",0),"‘","'",0),"â€˜","'",0),"â€™","'",0),3)
