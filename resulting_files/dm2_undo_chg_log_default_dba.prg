CREATE PROGRAM dm2_undo_chg_log_default:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_undo_chglog_",".log","dm2_undo_chglog LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Removing the default row from DM_CHG_LOG"
 CALL disp_msg("",dm_err->logfile,0)
 IF (checkprg("DM2_UNDO_DEFAULT_ROWS") != 0)
  EXECUTE dm2_undo_default_rows "DM_CHG_LOG"
 ELSE
 ...
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error removing default row from DM_CHG_LOG"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
 ...
 ELSE
  COMMIT
  SET readme_data->message = "Finished removal of default row in DM_CHG_LOG"
