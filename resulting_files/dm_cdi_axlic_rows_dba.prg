CREATE PROGRAM dm_cdi_axlic_rows:dba
 ...
 SET reply->table_name = "CDI_AXLIC_USAGE"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(cau.cdi_axlic_usage_id)
   FROM cdi_axlic_usage cau
   WHERE cau.cdi_axlic_usage_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(cau.cdi_axlic_usage_id)
  FROM cdi_axlic_usage cau
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    cau.rowid
    FROM cdi_axlic_usage cau
    WHERE cau.cdi_axlic_usage_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "cau.license_dt_tm < cnvtdatetime(curdate - v_days_to_keep, curtime3)","cau"))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = cau.rowid
