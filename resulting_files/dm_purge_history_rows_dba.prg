CREATE PROGRAM dm_purge_history_rows:dba
 ...
 SET reply->table_name = "DM_PURGE_HISTORY"
 ...
 ELSE
  SELECT INTO "nl:"
   ...
   WHERE parser(sbr_getrowidnotexists(
     "dph.updt_dt_tm < cnvtdatetime(curdate - v_days_to_keep, curtime3)","dph"))
    AND ((dph.dm_purge_history_id+ 0) > 0.0)
   HEAD REPORT
    v_rows = 0
   DETAIL
    v_rows = (v_rows+ 1)
    ...
    ENDIF
    reply->rows[v_rows].row_id = dph.rowid
   FOOT REPORT
    stat = alterlist(reply->rows,v_rows)
   WITH nocounter, maxqual(ts,value(request->max_rows))
  ;end select
