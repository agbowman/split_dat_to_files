CREATE PROGRAM bhs_rpt_ed_work:dba
 PROMPT
 ...
 RECORD m_info(
   ...
   1 s_work_time = vc
   1 s_injury = vc
   1 n_work_related_ind = i2
   1 s_work_status = vc
   1 s_recommendations = vc
   1 s_followup = vc
   1 addendums[*]
     2 s_title = vc
     2 s_text = vc
 ...
 DECLARE work_note_header(ncalc=i2) = f8 WITH protect
 DECLARE work_note_headerabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE work_note_detail(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE work_note_detailabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8
 WITH protect
 DECLARE section_space(ncalc=i2) = f8 WITH protect
 DECLARE section_spaceabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE work_status_header(ncalc=i2) = f8 WITH protect
 DECLARE work_status_headerabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE work_status_detail(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE work_status_detailabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8
 WITH protect
 DECLARE light_duty_header(ncalc=i2) = f8 WITH protect
 DECLARE light_duty_headerabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE light_duty_detail(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE light_duty_detailabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8
 WITH protect
 DECLARE followup_header(ncalc=i2) = f8 WITH protect
 DECLARE followup_headerabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE followup_detail(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE followup_detailabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8 WITH
 protect
 ...
 DECLARE _remnotes = i4 WITH noconstant(1), protect
 DECLARE _bholdcontinue = i2 WITH noconstant(0), protect
 DECLARE _bcontwork_note_detail = i2 WITH noconstant(0), protect
 DECLARE _rems_work_status = i4 WITH noconstant(1), protect
 DECLARE _bcontwork_status_detail = i2 WITH noconstant(0), protect
 DECLARE _rems_recommendations = i4 WITH noconstant(1), protect
 DECLARE _bcontlight_duty_detail = i2 WITH noconstant(0), protect
 DECLARE _rems_followup = i4 WITH noconstant(1), protect
 DECLARE _bcontfollowup_detail = i2 WITH noconstant(0), protect
 DECLARE _rems_text = i4 WITH noconstant(1), protect
 ...
 SUBROUTINE report_headabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Emergency Department Work Note",char(
       0)))
 ...
 SUBROUTINE section_divider2abs(ncalc,offsetx,offsety)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE work_note_header(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = work_note_headerabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE work_note_headerabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_helvetica12b0)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Work Note",char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE work_note_detail(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = work_note_detailabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE work_note_detailabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_notes = f8 WITH noconstant(0.0), private
   DECLARE __notes = vc WITH noconstant(build2(m_info->s_work_note,char(0))), protect
   IF (bcontinue=0)
    SET _remnotes = 1
   ENDIF
   SET rptsd->m_flags = 5
 ...
 SUBROUTINE section_spaceabs(ncalc,offsetx,offsety)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE work_status_header(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = work_status_headerabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE work_status_headerabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_helvetica12b0)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Work Status",char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE work_status_detail(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = work_status_detailabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE work_status_detailabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_s_work_status = f8 WITH noconstant(0.0), private
   DECLARE __s_work_status = vc WITH noconstant(build2(m_info->s_work_status,char(0))), protect
   IF (bcontinue=0)
    SET _rems_work_status = 1
   ENDIF
   SET rptsd->m_flags = 5
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   SET _oldfont = uar_rptsetfont(_hreport,_helvetica12b0)
   SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
   SET _holdrems_work_status = _rems_work_status
   IF (_rems_work_status > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_rems_work_status,((size(
        __s_work_status) - _rems_work_status)+ 1),__s_work_status)))
    SET drawheight_s_work_status = rptsd->m_height
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _rems_work_status = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_rems_work_status,((size(__s_work_status)
        - _rems_work_status)+ 1),__s_work_status)))))
     SET _rems_work_status = (_rems_work_status+ rptsd->m_drawlength)
    ELSE
     SET _rems_work_status = 0
    ENDIF
    SET growsum = (growsum+ _rems_work_status)
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   SET rptsd->m_x = (offsetx+ 0.250)
   SET rptsd->m_width = 7.250
   SET rptsd->m_height = drawheight_s_work_status
   IF (ncalc=rpt_render
    AND _holdrems_work_status > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdrems_work_status,((
       size(__s_work_status) - _holdrems_work_status)+ 1),__s_work_status)))
   ELSE
    SET _rems_work_status = _holdrems_work_status
   ENDIF
   SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
   SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
   IF (ncalc=rpt_render)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE light_duty_header(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = light_duty_headerabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE light_duty_headerabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_helvetica12b0)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Recommendations",char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE light_duty_detail(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = light_duty_detailabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE light_duty_detailabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_s_recommendations = f8 WITH noconstant(0.0), private
   DECLARE __s_recommendations = vc WITH noconstant(build2(m_info->s_recommendations,char(0))),
   protect
   IF (bcontinue=0)
    SET _rems_recommendations = 1
   ENDIF
   SET rptsd->m_flags = 5
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   SET _oldfont = uar_rptsetfont(_hreport,_helvetica12b0)
   SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
   SET _holdrems_recommendations = _rems_recommendations
   IF (_rems_recommendations > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_rems_recommendations,((
       size(__s_recommendations) - _rems_recommendations)+ 1),__s_recommendations)))
    SET drawheight_s_recommendations = rptsd->m_height
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _rems_recommendations = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_rems_recommendations,((size(
        __s_recommendations) - _rems_recommendations)+ 1),__s_recommendations)))))
     SET _rems_recommendations = (_rems_recommendations+ rptsd->m_drawlength)
    ELSE
     SET _rems_recommendations = 0
    ENDIF
    SET growsum = (growsum+ _rems_recommendations)
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   SET rptsd->m_x = (offsetx+ 0.250)
   SET rptsd->m_width = 7.250
   SET rptsd->m_height = drawheight_s_recommendations
   IF (ncalc=rpt_render
    AND _holdrems_recommendations > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdrems_recommendations,
       ((size(__s_recommendations) - _holdrems_recommendations)+ 1),__s_recommendations)))
   ELSE
    SET _rems_recommendations = _holdrems_recommendations
   ENDIF
   SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
   SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
   IF (ncalc=rpt_render)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE followup_header(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = followup_headerabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE followup_headerabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_helvetica12b0)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Follow Up Care",char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE followup_detail(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = followup_detailabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE followup_detailabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   DECLARE drawheight_s_followup = f8 WITH noconstant(0.0), private
   DECLARE __s_followup = vc WITH noconstant(build2(m_info->s_followup,char(0))), protect
   IF (bcontinue=0)
    SET _rems_followup = 1
   ENDIF
   SET rptsd->m_flags = 5
   SET rptsd->m_borders = rpt_sdnoborders
   SET rptsd->m_padding = rpt_sdnoborders
   ...
   SET _oldfont = uar_rptsetfont(_hreport,_helvetica12b0)
   SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
   SET _holdrems_followup = _rems_followup
   IF (_rems_followup > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_rems_followup,((size(
        __s_followup) - _rems_followup)+ 1),__s_followup)))
    SET drawheight_s_followup = rptsd->m_height
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _rems_followup = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_rems_followup,((size(__s_followup) -
       _rems_followup)+ 1),__s_followup)))))
     SET _rems_followup = (_rems_followup+ rptsd->m_drawlength)
    ELSE
     SET _rems_followup = 0
    ENDIF
    SET growsum = (growsum+ _rems_followup)
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   SET rptsd->m_x = (offsetx+ 0.250)
   SET rptsd->m_width = 7.250
   SET rptsd->m_height = drawheight_s_followup
   IF (ncalc=rpt_render
    AND _holdrems_followup > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdrems_followup,((size(
        __s_followup) - _holdrems_followup)+ 1),__s_followup)))
   ELSE
    SET _rems_followup = _holdrems_followup
   ENDIF
   SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
   SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
   IF (ncalc=rpt_render)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_RPT_ED_WORK"
 ...
 SELECT
  ...
  PLAN (ss
   ...
   WHERE ssp.scr_pattern_id=sp.scr_pattern_id
    ...
    AND sp.cki_identifier="EP BHS ED WORK NOTE")
   JOIN (e
  ...
  DETAIL
   ...
   IF (pn_det_cnt=1)
    IF (textlen(trim(cb.blob_contents)) > 0)
     ms_blob = sbr_process_blob(cb.blob_contents,cb.compression_cd), pl_beg_pos = findstring(
      "WORK NOTE",cnvtupper(ms_blob))
     IF (pl_beg_pos > 0)
      pl_beg_pos = (pl_beg_pos+ 10), pl_end_pos = findstring("WORK RELATED",cnvtupper(ms_blob))
      IF (pl_end_pos=0)
       pl_end_pos = findstring("WORK STATUS",cnvtupper(ms_blob))
       IF (pl_end_pos=0)
        pl_end_pos = findstring("RECOMMENDATIONS",cnvtupper(ms_blob))
        IF (pl_end_pos=0)
         pl_end_pos = textlen(trim(ms_blob))
        ENDIF
       ENDIF
      ENDIF
      ms_string = trim(substring(pl_beg_pos,(pl_end_pos - pl_beg_pos),ms_blob),3), ms_string =
      replace(ms_string,char(13)," ")
      IF (substring(1,1,ms_string)=" ")
       ms_string = substring(2,(textlen(ms_string) - 1),ms_string)
      ENDIF
      pl_beg_pos = (findstring(":",ms_string)+ 5), pl_end_pos = findstring(" ",ms_string,(pl_beg_pos
       + 2)), m_info->s_work_date = substring(pl_beg_pos,(pl_end_pos - pl_beg_pos),ms_string),
      pl_beg_pos = (pl_end_pos+ 1), pl_end_pos = findstring(" ",ms_string,pl_beg_pos)
      IF (pl_end_pos=0
       AND findstring("for",ms_string,pl_beg_pos)=0)
       pl_end_pos = (findstring(".",ms_string,pl_beg_pos)+ 1)
      ENDIF
      m_info->s_work_time = substring(pl_beg_pos,((pl_end_pos - pl_beg_pos) - 1),ms_string),
      pl_beg_pos = (findstring("for",ms_string,pl_end_pos)+ 4)
      IF (pl_beg_pos=0)
       pl_beg_pos = (pl_end_pos+ 1)
      ENDIF
      pl_end_pos = textlen(trim(ms_string)), m_info->s_injury = substring(pl_beg_pos,((pl_end_pos -
       pl_beg_pos) - 1),ms_string)
     ENDIF
     pl_beg_pos = findstring("WORK RELATED",cnvtupper(ms_blob))
     IF (pl_beg_pos > 0)
      ms_string = trim(cnvtupper(substring(pl_beg_pos,30,ms_blob)),3)
      IF (findstring("YES",ms_string) > 0)
       m_info->n_work_related_ind = 1
      ELSEIF (findstring("NO",ms_string) > 0)
       m_info->n_work_related_ind = 0
      ELSEIF (findstring("UNDETERMINED",ms_string) > 0)
       m_info->n_work_related_ind = 2
      ENDIF
     ENDIF
     pl_beg_pos = findstring("WORK STATUS",cnvtupper(ms_blob))
     IF (pl_beg_pos > 0)
      pl_end_pos = findstring("RECOMMENDATIONS",cnvtupper(ms_blob))
      IF (pl_end_pos=0)
       pl_end_pos = (textlen(trim(ms_blob))+ 1)
      ENDIF
      pl_beg_pos = (pl_beg_pos+ 12), ms_string = trim(substring(pl_beg_pos,(pl_end_pos - pl_beg_pos),
        ms_blob),3), ms_string = replace(ms_string,char(13)," ")
      IF (((substring(1,1,ms_string)=" ") OR (substring(1,1,ms_string)=char(10))) )
       ms_string = substring(2,pl_end_pos,ms_string)
      ENDIF
      m_info->s_work_status = ms_string
     ENDIF
     pl_beg_pos = findstring("RECOMMENDATIONS",cnvtupper(ms_blob))
     IF (pl_beg_pos > 0)
      pl_beg_pos = (pl_beg_pos+ 15), pl_end_pos = textlen(ms_blob), ms_string = trim(substring(
        pl_beg_pos,((pl_end_pos - pl_beg_pos)+ 1),ms_blob),3),
      ms_string = replace(ms_string,char(13)," ")
      IF (((substring(1,1,ms_string)=" ") OR (substring(1,1,ms_string)=char(10))) )
       ms_string = substring(2,textlen(ms_string),ms_string)
      ENDIF
      m_info->s_recommendations = ms_string
     ENDIF
    ENDIF
 ...
 ENDIF
 SET m_info->s_work_note = concat(
  "The above patient was seen at the Baystate Medical Center Emergency Department on ",m_info->
  s_work_date," at ",m_info->s_work_time,".")
 IF ((m_info->n_work_related_ind=0))
  SET m_info->s_followup =
  "The patient has been advised on their discharge instructions regarding appropriate follow-up."
 ELSEIF ((m_info->n_work_related_ind=1))
  SET m_info->s_work_note = concat(m_info->s_work_note,char(10),char(10),
   "Based on the available information, the patient had the following work related injury/illness: ",
   m_info->s_injury)
  SET m_info->s_followup = concat(
   "The patient was advised on their discharge instructions regarding follow-up ",
   "recommendations for a possible work related injury/illness.  You (the employer) ",
   "may wish to have the patient evaluated by your occupational medicine provider regarding ",
   "additional care.  If you do not have an occupational provider, Concentra ",
   "746-4006 may be called as an occupational health and rehabilitation option. ",
   char(10),char(10),
   "If this was a Baystate Health work-related injury, then the patient should follow up ",
   "at Employee Health Services on the next business day.  Please call 794-3254 ",
   "for an appointment.",
   char(10),char(10),
   "If any additional information is required, please contact the Baystate Medical Center ",
   "Emergency Department Follow-Up Nurse at: (413)-794-4774.")
 ELSEIF ((m_info->n_work_related_ind=2))
  SET m_info->s_work_note = concat(m_info->s_work_note,char(10),char(10),
   "Based upon the information available, we were unable to determine if the patient ",
 ...
 IF (textlen(trim(m_info->s_work_note)))
  SET d0 = work_note_header(rpt_render)
  SET mf_rem_space = (mf_page_size - _yoffset)
  SET d0 = work_note_detail(rpt_render,mf_rem_space,ml_cont_ind)
  WHILE (ml_cont_ind=1)
   CALL sbr_pagebreak(0)
   SET d0 = work_note_detail(rpt_render,mf_rem_space,ml_cont_ind)
  ENDWHILE
 ENDIF
 IF (textlen(trim(m_info->s_work_status)))
  SET d0 = section_space(rpt_render)
  SET mf_rem_space = (mf_page_size - _yoffset)
  IF (mf_rem_space < 0.22)
   CALL sbr_pagebreak(0)
  ENDIF
  SET d0 = work_status_header(rpt_render)
  SET d0 = work_status_detail(rpt_render,mf_rem_space,ml_cont_ind)
  WHILE (ml_cont_ind=1)
   CALL sbr_pagebreak(0)
   SET d0 = work_status_detail(rpt_render,mf_rem_space,ml_cont_ind)
  ENDWHILE
 ENDIF
 IF (textlen(trim(m_info->s_recommendations)))
  SET d0 = section_space(rpt_render)
  SET mf_rem_space = (mf_page_size - _yoffset)
  IF (mf_rem_space < 0.22)
   CALL sbr_pagebreak(0)
  ENDIF
  SET d0 = light_duty_header(rpt_render)
  SET d0 = light_duty_detail(rpt_render,mf_rem_space,ml_cont_ind)
  WHILE (ml_cont_ind=1)
   CALL sbr_pagebreak(0)
   SET d0 = light_duty_detail(rpt_render,mf_rem_space,ml_cont_ind)
  ENDWHILE
 ENDIF
 IF (textlen(trim(m_info->s_followup)))
  SET d0 = section_space(rpt_render)
  SET mf_rem_space = (mf_page_size - _yoffset)
  IF (mf_rem_space < 0.22)
   CALL sbr_pagebreak(0)
  ENDIF
  SET d0 = followup_header(rpt_render)
  SET d0 = followup_detail(rpt_render,mf_rem_space,ml_cont_ind)
  WHILE (ml_cont_ind=1)
   CALL sbr_pagebreak(0)
   SET d0 = followup_detail(rpt_render,mf_rem_space,ml_cont_ind)
  ENDWHILE
 ...
 SUBROUTINE sbr_process_blob(s_blob_in,f_comp_cd)
   ...
   ELSEIF (f_comp_cd=mf_no_comp_cd)
    ...
    ENDIF
   ENDIF
   SET ms_blob_out = trim(ms_blob_out,3)
