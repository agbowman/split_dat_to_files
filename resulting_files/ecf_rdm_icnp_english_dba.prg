CREATE PROGRAM ecf_rdm_icnp_english:dba
 ...
 DECLARE kia_nomen_csv_filename = vc WITH public, constant("icnp_english_nomen.csv")
 DECLARE kia_concept_filename = vc WITH public, constant("icnp_english_concept.csv")
 DECLARE kia_reltn_filename = vc WITH public, constant("icnp_english_reltn.csv")
 SET kia_tbl_desc = "ICNP English Content Import"
 SET kia_short_desc = "ICNP English"
 SET kia_rdm_log_name = "ecf_rdm_icnp_english.log"
 SET kia_rdm_nbr = 11572
 SET kia_rdm_vocab_mean = "ICNP_ENGLISH"
 SET readme_data->status = "F"
 ...
 EXECUTE kia_dm_dbimport "cer_install:icnp_english_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("ecf_rdm_icnp_english Begin> ",format(cnvtdatetime(curdate,curtime3),
 ...
 CALL kia_chk_status_info("LOAD ICNP BASELINE DATA","check")
 IF (kia_obj_suc_ind=0)
  CALL echo("DELETING EXISTING CONTENT FROM NOMENCLATURE_LOAD_NS TABLE...")
  ...
  CALL kia_set_status_success("LOAD ICNP BASELINE DATA")
 ENDIF
 ...
 ENDIF
 CALL kia_chk_status_info("LOAD ICNP CONCEPT BASELINE DATA","check")
 IF (kia_obj_suc_ind=0)
  CALL echo("DELETING CMT_CONCEPT_LOAD_NS ROWS...")
  DELETE  FROM cmt_concept_load_ns c
   WHERE ((c.concept_cki="ICNP_ENGLISH*") OR (c.concept_cki="SNOMED*"))
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_concept_filename,"kia_imp_concept_load_ns","kia_imp_concept_load_ns.log")
  CALL kia_set_status_success("LOAD ICNP CONCEPT BASELINE DATA")
 ENDIF
