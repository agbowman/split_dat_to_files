CREATE PROGRAM dm_rmc_get_trg_data:dba
 ...
 ENDIF
 DECLARE drgtd_all_tables = i2 WITH protect, noconstant(0)
 DECLARE v_where_str = vc WITH protect, noconstant(" ")
 DECLARE v_tab_str = vc WITH protect, noconstant(" ")
 DECLARE v_where_cnt = i4 WITH protect, noconstant(0)
 DECLARE drgtd_cnt = i4 WITH protect, noconstant(0)
 DECLARE drgtd_num = i4 WITH protect, noconstant(0)
 DECLARE drgtd_pos = i4 WITH protect, noconstant(0)
 DECLARE drgtd_loop = i4 WITH protect, noconstant(0)
 DECLARE drgtd_tbl_name_find = vc WITH protect, noconstant(" ")
 DECLARE drgtd_tab_cnt = i4 WITH protect, noconstant(0)
 DECLARE drg_non_eng_ind = i2 WITH protect, noconstant(0)
 FREE RECORD trg_table_data
 RECORD trg_table_data(
   1 err_ind = i2
   1 err_msg = vc
   1 table_cnt = i4
   1 qual[*]
     ...
     2 non_delete_data
       3 template_id = f8
       3 pk_where = vc
       3 pk_where_template = vc
       3 pk_where_where_query = vc
       3 pk_where_parm_query = vc
       3 pk_where_function_ind = i2
       3 ptam_match_template = vc
       3 ptam_match_query = vc
       3 ptam_match_where_query = vc
       3 ptam_match_parm_query = vc
       3 ptam_match_function_ind = i2
       3 updt_ind = i2
       3 ins_ind = i2
     2 delete_data
       3 template_id = f8
 ...
 IF ((dm_err->debug_flag > 0))
  FREE RECORD miss_trg_data
  RECORD miss_trg_data(
    1 total = i4
    1 qual[*]
      2 table_name = vc
  )
 ENDIF
 IF (check_logfile("dm_rmc_trg_data",".log","dm_rmc_get_trg_data LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm_rmc_get_trg_data..."
 CALL disp_msg("",dm_err->logfile,dm_err->err_ind)
 SET dm_err->eproc = "Determining nls settings."
 SET drg_non_eng_ind = 1
 SELECT INTO "nl:"
  FROM v$nls_parameters n
  WHERE cnvtupper(n.parameter)="NLS_LANGUAGE"
   AND cnvtupper(n.value) IN (
  (SELECT
   di.info_name
   FROM dm_info di
   WHERE di.info_domain="RDDS STANDARD NLS"))
  DETAIL
   drg_non_eng_ind = 0
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  SET drgtd_reply->err_ind = 1
  SET drgtd_reply->err_msg = dm_err->emsg
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting list of tables to gather meta data information"
 CALL disp_msg("",dm_err->logfile,dm_err->err_ind)
 SET drgtd_tbl_name_find = drgtd_request->what_tables
 IF (drgtd_tbl_name_find != char(42))
  IF (size(drgtd_request->req_special,5) > 0)
   SET v_where_str = concat(v_where_str,"((")
  ENDIF
  SET v_where_str = concat(v_where_str," dtd.table_name = patstring(drgtd_tbl_name_find) ")
  IF (size(drgtd_request->req_special,5) > 0)
   SET v_where_str = concat(v_where_str,") or  dtd.table_name in (")
   FOR (my_cnt = 1 TO size(drgtd_request->req_special,5))
    IF (my_cnt != 1)
     SET v_where_str = concat(v_where_str,",")
    ENDIF
    SET v_where_str = concat(v_where_str,'"',drgtd_request->req_special[my_cnt].sp_tbl,'"')
   ENDFOR
   SET v_where_str = concat(v_where_str,"))")
  ENDIF
 ELSE
 ...
 SELECT INTO "nl:"
  ...
  WHERE ((dtd.reference_ind=1
   AND dtd.table_name=dtd.full_table_name) OR ( EXISTS (
  (SELECT
   "x"
   FROM dm_rdds_refmrg_tables drrt
   WHERE drrt.table_name=dtd.table_name))))
  ...
  HEAD REPORT
   drgtd_cnt = 0
  DETAIL
   drgtd_cnt = (drgtd_cnt+ 1)
   IF (mod(drgtd_cnt,100)=1)
    stat = alterlist(trg_table_data->qual,(drgtd_cnt+ 99))
   ENDIF
   trg_table_data->qual[drgtd_cnt].table_name = dtd.table_name, trg_table_data->qual[drgtd_cnt].
   table_suffix = dtd.table_suffix, trg_table_data->qual[drgtd_cnt].mergeable_ind = dtd.mergeable_ind,
   trg_table_data->qual[drgtd_cnt].reference_ind = 1, trg_table_data->qual[drgtd_cnt].
   merge_delete_ind = dtd.merge_delete_ind
  FOOT REPORT
   stat = alterlist(trg_table_data->qual,drgtd_cnt), trg_table_data->table_cnt = drgtd_cnt
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  ...
  GO TO exit_program
 ENDIF
 IF ((trg_table_data->table_cnt=0))
  SET drgtd_reply->err_ind = 1
  SET drgtd_reply->err_msg = "No tables qualified."
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Gathering table versioning information."
 CALL disp_msg("",dm_err->logfile,dm_err->err_ind)
 SELECT INTO "nl:"
  FROM code_value cv
  WHERE expand(drgtd_num,1,trg_table_data->table_cnt,cv.display,trg_table_data->qual[drgtd_num].
   table_name)
   AND cv.active_ind=1
   AND cv.cdf_meaning != "NONE"
   AND cv.code_set=255351
  DETAIL
   drgtd_pos = locateval(drgtd_num,1,trg_table_data->table_cnt,cv.display,trg_table_data->qual[
    drgtd_num].table_name), trg_table_data->qual[drgtd_pos].version_ind = 1, trg_table_data->qual[
   drgtd_pos].version_alg = cv.cdf_meaning
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Gathering table template information."
 CALL disp_msg("",dm_err->logfile,dm_err->err_ind)
 SELECT INTO "nl:"
  FROM dm_pk_where_local d
  WHERE expand(drgtd_num,1,trg_table_data->table_cnt,d.table_name,trg_table_data->qual[drgtd_num].
   table_name)
  ORDER BY d.table_name, d.template_id
  HEAD d.table_name
   drgtd_pos = locateval(drgtd_num,1,trg_table_data->table_cnt,d.table_name,trg_table_data->qual[
    drgtd_num].table_name)
  DETAIL
   IF (d.delete_ind=0)
    trg_table_data->qual[drgtd_pos].non_delete_data.template_id = d.template_id, trg_table_data->
    qual[drgtd_pos].non_delete_data.pk_where_template = d.pk_where_template, trg_table_data->qual[
    drgtd_pos].non_delete_data.pk_where_where_query = d.pk_where_where_query,
    trg_table_data->qual[drgtd_pos].non_delete_data.pk_where_parm_query = d.pk_where_parm_query,
    trg_table_data->qual[drgtd_pos].non_delete_data.pk_where_function_ind = d.pk_where_function_ind,
    trg_table_data->qual[drgtd_pos].non_delete_data.ptam_match_template = d.ptam_match_template,
    trg_table_data->qual[drgtd_pos].non_delete_data.ptam_match_where_query = d.ptam_match_where_query,
    trg_table_data->qual[drgtd_pos].non_delete_data.ptam_match_function_ind = d
    .ptam_match_function_ind, trg_table_data->qual[drgtd_pos].non_delete_data.ptam_match_parm_query
     = d.ptam_match_parm_query
   ELSE
    trg_table_data->qual[drgtd_pos].delete_data.template_id = d.template_id, trg_table_data->qual[
    drgtd_pos].delete_data.pk_where_template = d.pk_where_template, trg_table_data->qual[drgtd_pos].
    delete_data.pk_where_where_query = d.pk_where_where_query,
    trg_table_data->qual[drgtd_pos].delete_data.pk_where_parm_query = d.pk_where_parm_query,
    trg_table_data->qual[drgtd_pos].delete_data.pk_where_function_ind = d.pk_where_function_ind,
    trg_table_data->qual[drgtd_pos].delete_data.ptam_match_template = d.ptam_match_template,
    trg_table_data->qual[drgtd_pos].delete_data.ptam_match_where_query = d.ptam_match_where_query,
    trg_table_data->qual[drgtd_pos].delete_data.ptam_match_function_ind = d.ptam_match_function_ind,
    trg_table_data->qual[drgtd_pos].delete_data.ptam_match_parm_query = d.ptam_match_parm_query
   ENDIF
  WITH nocounter, expand = 2
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 SELECT INTO "nl:"
  FROM dm_pk_where_local d
  WHERE d.table_name="DEFAULT"
  DETAIL
   FOR (drgtd_pos = 1 TO trg_table_data->table_cnt)
     IF (((d.version_ind=1
      AND (trg_table_data->qual[drgtd_pos].version_ind=1)
      AND (d.version_alg=trg_table_data->qual[drgtd_pos].version_alg)) OR (d.version_ind=0
      AND (trg_table_data->qual[drgtd_pos].version_ind=0)
      AND (d.merge_delete_ind=trg_table_data->qual[drgtd_pos].merge_delete_ind))) )
      IF (d.delete_ind=0
       AND (trg_table_data->qual[drgtd_pos].non_delete_data.template_id=0))
       trg_table_data->qual[drgtd_pos].non_delete_data.template_id = d.template_id, trg_table_data->
       qual[drgtd_pos].non_delete_data.pk_where_template = d.pk_where_template, trg_table_data->qual[
       drgtd_pos].non_delete_data.pk_where_where_query = d.pk_where_where_query,
       trg_table_data->qual[drgtd_pos].non_delete_data.pk_where_parm_query = d.pk_where_parm_query,
       trg_table_data->qual[drgtd_pos].non_delete_data.pk_where_function_ind = d
       .pk_where_function_ind, trg_table_data->qual[drgtd_pos].non_delete_data.ptam_match_template =
       d.ptam_match_template,
       trg_table_data->qual[drgtd_pos].non_delete_data.ptam_match_where_query = d
       .ptam_match_where_query, trg_table_data->qual[drgtd_pos].non_delete_data.
       ptam_match_function_ind = d.ptam_match_function_ind, trg_table_data->qual[drgtd_pos].
       non_delete_data.ptam_match_parm_query = d.ptam_match_parm_query
      ELSEIF ((trg_table_data->qual[drgtd_pos].delete_data.template_id=0))
       trg_table_data->qual[drgtd_pos].delete_data.template_id = d.template_id, trg_table_data->qual[
       drgtd_pos].delete_data.pk_where_template = d.pk_where_template, trg_table_data->qual[drgtd_pos
       ].delete_data.pk_where_where_query = d.pk_where_where_query,
       trg_table_data->qual[drgtd_pos].delete_data.pk_where_parm_query = d.pk_where_parm_query,
       ...
       trg_table_data->qual[drgtd_pos].delete_data.ptam_match_template = d.ptam_match_template,
       trg_table_data->qual[drgtd_pos].delete_data.ptam_match_where_query = d.ptam_match_where_query,
       trg_table_data->qual[drgtd_pos].delete_data.ptam_match_function_ind = d
       .ptam_match_function_ind, trg_table_data->qual[drgtd_pos].delete_data.ptam_match_parm_query =
       d.ptam_match_parm_query
      ENDIF
     ENDIF
   ENDFOR
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET stat = alterlist(drgtd_reply->qual,trg_table_data->table_cnt)
 FOR (drgtd_loop = 1 TO trg_table_data->table_cnt)
   SET stat = alterlist(md_list->template,0)
   SET md_list->tmpl_cnt = 0
   SET dm_err->eproc = concat("Gathering templates for table ",trg_table_data->qual[drgtd_loop].
    table_name,".")
   CALL disp_msg("",dm_err->logfile,dm_err->err_ind)
   SET trg_table_data->qual[drgtd_loop].non_delete_data.pk_where = replace(trg_table_data->qual[
    drgtd_loop].non_delete_data.pk_where_template,"<!TABLE_SUFFIX!>",trg_table_data->qual[drgtd_loop]
    .table_suffix,0)
   IF ((trg_table_data->qual[drgtd_loop].non_delete_data.pk_where_where_query > ""))
    SET refchg_where_query->pkw_template = ""
    SET refchg_where_query->err_ind = 0
    SET refchg_where_query->err_msg = ""
    CALL parser(concat("execute ",trg_table_data->qual[drgtd_loop].non_delete_data.
      pk_where_where_query,' "',trg_table_data->qual[drgtd_loop].table_name,'","',
      trg_table_data->qual[drgtd_loop].table_suffix,
      '" with replace("DRGQT_REPLY", "REFCHG_WHERE_QUERY") go'),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     SET drgtd_reply->err_ind = 1
     SET drgtd_reply->err_msg = dm_err->emsg
     GO TO exit_program
    ELSEIF ((refchg_where_query->err_ind=1))
     CALL disp_msg(refchg_where_query->err_msg,dm_err->logfile,dm_err->err_ind)
     SET drgtd_reply->err_ind = 1
     SET drgtd_reply->err_msg = refchg_where_query->err_msg
     GO TO exit_program
    ENDIF
    SET trg_table_data->qual[drgtd_loop].non_delete_data.pk_where = replace(trg_table_data->qual[
     drgtd_loop].non_delete_data.pk_where,"<!PK_WHERE_WHERE_QUERY!>",refchg_where_query->pkw_template,
     0)
   ENDIF
   SET trg_table_data->qual[drgtd_loop].delete_data.pk_where = replace(trg_table_data->qual[
    drgtd_loop].delete_data.pk_where_template,"<!TABLE_SUFFIX!>",trg_table_data->qual[drgtd_loop].
    table_suffix,0)
   IF ((trg_table_data->qual[drgtd_loop].delete_data.pk_where_where_query > ""))
    SET refchg_where_query->pkw_template = ""
    SET refchg_where_query->err_ind = 0
    SET refchg_where_query->err_msg = ""
    CALL parser(concat("execute ",trg_table_data->qual[drgtd_loop].delete_data.pk_where_where_query,
      ' "',trg_table_data->qual[drgtd_loop].table_name,'","',
      trg_table_data->qual[drgtd_loop].table_suffix,
      '" with replace("DRGQT_REPLY", "REFCHG_WHERE_QUERY") go'),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    ...
    ENDIF
    SET trg_table_data->qual[drgtd_loop].delete_data.pk_where = replace(trg_table_data->qual[
     drgtd_loop].delete_data.pk_where,"<!PK_WHERE_WHERE_QUERY!>",refchg_where_query->pkw_template,0)
   ENDIF
   SET trg_table_data->qual[drgtd_loop].non_delete_data.ptam_match_query = replace(trg_table_data->
    qual[drgtd_loop].non_delete_data.ptam_match_template,"<!TABLE_SUFFIX!>",trg_table_data->qual[
    drgtd_loop].table_suffix,0)
   IF ((trg_table_data->qual[drgtd_loop].non_delete_data.ptam_match_where_query > ""))
    SET refchg_where_query->pkw_template = ""
    SET refchg_where_query->err_ind = 0
    SET refchg_where_query->err_msg = ""
    CALL parser(concat("execute ",trg_table_data->qual[drgtd_loop].non_delete_data.
      ptam_match_where_query,'"',trg_table_data->qual[drgtd_loop].table_name,'","',
      trg_table_data->qual[drgtd_loop].table_suffix,
      '" with replace("DRGQT_REPLY", "REFCHG_WHERE_QUERY") go'),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    ...
    ENDIF
    SET trg_table_data->qual[drgtd_loop].non_delete_data.ptam_match_query = replace(trg_table_data->
     qual[drgtd_loop].non_delete_data.ptam_match_query,"<!PTAM_MATCH_WHERE_QUERY!>",
     refchg_where_query->pkw_template,0)
   ENDIF
   SET trg_table_data->qual[drgtd_loop].delete_data.ptam_match_query = replace(trg_table_data->qual[
    drgtd_loop].delete_data.ptam_match_template,"<!TABLE_SUFFIX!>",trg_table_data->qual[drgtd_loop].
    table_suffix,0)
   IF ((trg_table_data->qual[drgtd_loop].delete_data.ptam_match_where_query > ""))
    SET refchg_where_query->pkw_template = ""
    SET refchg_where_query->err_ind = 0
    SET refchg_where_query->err_msg = ""
    CALL parser(concat("execute ",trg_table_data->qual[drgtd_loop].delete_data.ptam_match_where_query,
      ' "',trg_table_data->qual[drgtd_loop].table_name,'","',
      trg_table_data->qual[drgtd_loop].table_suffix,
      '" with replace("DRGQT_REPLY", "REFCHG_WHERE_QUERY") go'),1)
    IF (check_error(dm_err->eproc)=1)
    ...
    ENDIF
    SET trg_table_data->qual[drgtd_loop].delete_data.ptam_match_query = replace(trg_table_data->qual[
     drgtd_loop].delete_data.ptam_match_query,"<!PTAM_MATCH_WHERE_QUERY!>",refchg_where_query->
     pkw_template,0)
   ENDIF
   DECLARE dm_get_hash_value() = f8
   SET dm_err->eproc = "Populating DM_PK_WHERE table."
   IF ((trg_table_data->qual[drgtd_loop].non_delete_data.pk_where > "")
    AND (trg_table_data->qual[drgtd_loop].delete_data.pk_where > "")
    AND (trg_table_data->qual[drgtd_loop].non_delete_data.ptam_match_query > "")
    AND (trg_table_data->qual[drgtd_loop].delete_data.ptam_match_query > ""))
    SELECT INTO "nl:"
     FROM dm_pk_where d
     WHERE (d.table_name=trg_table_data->qual[drgtd_loop].table_name)
     DETAIL
      IF (d.delete_ind=0)
       IF ((((d.merge_delete_ind != trg_table_data->qual[drgtd_loop].merge_delete_ind)) OR ((((d
       .version_ind != trg_table_data->qual[drgtd_loop].version_ind)) OR ((((d.version_alg !=
       trg_table_data->qual[drgtd_loop].version_alg)) OR ((((d.template_id != trg_table_data->qual[
       drgtd_loop].non_delete_data.template_id)) OR ((((d.pk_where != trg_table_data->qual[drgtd_loop
       ].non_delete_data.pk_where)) OR ((((d.ptam_match_query != trg_table_data->qual[drgtd_loop].
       non_delete_data.ptam_match_query)) OR ((((d.ptam_match_function_ind != trg_table_data->qual[
       drgtd_loop].non_delete_data.ptam_match_function_ind)) OR ((d.pk_where_function_ind !=
       trg_table_data->qual[drgtd_loop].non_delete_data.pk_where_function_ind))) )) )) )) )) )) )) )
        trg_table_data->qual[drgtd_loop].non_delete_data.updt_ind = 1
       ENDIF
      ELSE
       IF ((((d.merge_delete_ind != trg_table_data->qual[drgtd_loop].merge_delete_ind)) OR ((((d
       .version_ind != trg_table_data->qual[drgtd_loop].version_ind)) OR ((((d.version_alg !=
       trg_table_data->qual[drgtd_loop].version_alg)) OR ((((d.template_id != trg_table_data->qual[
       drgtd_loop].delete_data.template_id)) OR ((((d.pk_where != trg_table_data->qual[drgtd_loop].
       delete_data.pk_where)) OR ((((d.ptam_match_query != trg_table_data->qual[drgtd_loop].
       delete_data.ptam_match_query)) OR ((((d.ptam_match_function_ind != trg_table_data->qual[
       drgtd_loop].delete_data.ptam_match_function_ind)) OR ((d.pk_where_function_ind !=
       trg_table_data->qual[drgtd_loop].delete_data.pk_where_function_ind))) )) )) )) )) )) )) )
        trg_table_data->qual[drgtd_loop].delete_data.updt_ind = 1
       ENDIF
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     ...
     GO TO exit_program
    ELSEIF (curqual=0)
     SET trg_table_data->qual[drgtd_loop].non_delete_data.ins_ind = 1
     SET trg_table_data->qual[drgtd_loop].delete_data.ins_ind = 1
    ENDIF
    IF ((trg_table_data->qual[drgtd_loop].non_delete_data.updt_ind=1))
     UPDATE  FROM dm_pk_where d
      SET d.merge_delete_ind = trg_table_data->qual[drgtd_loop].merge_delete_ind, d.version_ind =
       trg_table_data->qual[drgtd_loop].version_ind, d.version_alg = trg_table_data->qual[drgtd_loop]
       .version_alg,
       d.template_id = trg_table_data->qual[drgtd_loop].non_delete_data.template_id, d.pk_where =
       trg_table_data->qual[drgtd_loop].non_delete_data.pk_where, d.pk_where_hash = dm_get_hash_value
       (trg_table_data->qual[drgtd_loop].non_delete_data.pk_where,0,1073741824.0),
       d.ptam_match_query = trg_table_data->qual[drgtd_loop].non_delete_data.ptam_match_query, d
       .ptam_match_hash = dm_get_hash_value(trg_table_data->qual[drgtd_loop].non_delete_data.
        ptam_match_query,0,1073741824.0), d.ptam_match_function_ind = trg_table_data->qual[drgtd_loop
       ].non_delete_data.ptam_match_function_ind,
       d.pk_where_function_ind = trg_table_data->qual[drgtd_loop].non_delete_data.
       pk_where_function_ind, d.updt_task = reqinfo->updt_task, d.updt_applctx = reqinfo->
       updt_applctx,
       d.updt_id = reqinfo->updt_id, d.updt_dt_tm = cnvtdatetime(curdate,curtime3)
      WHERE (d.table_name=trg_table_data->qual[drgtd_loop].table_name)
       AND d.delete_ind=0
      WITH nocounter
     ;end update
     IF (check_error(dm_err->eproc)=1)
      ...
      GO TO exit_program
     ENDIF
    ELSEIF ((trg_table_data->qual[drgtd_loop].non_delete_data.ins_ind=1))
     INSERT  FROM dm_pk_where d
      SET d.table_name = trg_table_data->qual[drgtd_loop].table_name, d.delete_ind = 0, d
       .merge_delete_ind = trg_table_data->qual[drgtd_loop].merge_delete_ind,
       d.version_ind = trg_table_data->qual[drgtd_loop].version_ind, d.version_alg = trg_table_data->
       qual[drgtd_loop].version_alg, d.template_id = trg_table_data->qual[drgtd_loop].non_delete_data
       .template_id,
       d.pk_where = trg_table_data->qual[drgtd_loop].non_delete_data.pk_where, d.pk_where_hash =
       dm_get_hash_value(trg_table_data->qual[drgtd_loop].non_delete_data.pk_where,0,1073741824.0), d
       .ptam_match_query = trg_table_data->qual[drgtd_loop].non_delete_data.ptam_match_query,
       d.ptam_match_hash = dm_get_hash_value(trg_table_data->qual[drgtd_loop].non_delete_data.
        ptam_match_query,0,1073741824.0), d.ptam_match_function_ind = trg_table_data->qual[drgtd_loop
       ].non_delete_data.ptam_match_function_ind, d.pk_where_function_ind = trg_table_data->qual[
       drgtd_loop].non_delete_data.pk_where_function_ind,
       d.updt_task = reqinfo->updt_task, d.updt_applctx = reqinfo->updt_applctx, d.updt_id = reqinfo
       ->updt_id,
     ...
     ENDIF
    ENDIF
    IF ((trg_table_data->qual[drgtd_loop].delete_data.updt_ind=1))
     UPDATE  FROM dm_pk_where d
      SET d.merge_delete_ind = trg_table_data->qual[drgtd_loop].merge_delete_ind, d.version_ind =
       trg_table_data->qual[drgtd_loop].version_ind, d.version_alg = trg_table_data->qual[drgtd_loop]
       .version_alg,
       d.template_id = trg_table_data->qual[drgtd_loop].delete_data.template_id, d.pk_where =
       trg_table_data->qual[drgtd_loop].delete_data.pk_where, d.pk_where_hash = dm_get_hash_value(
        trg_table_data->qual[drgtd_loop].delete_data.pk_where,0,1073741824.0),
       d.ptam_match_query = trg_table_data->qual[drgtd_loop].delete_data.ptam_match_query, d
       .ptam_match_hash = dm_get_hash_value(trg_table_data->qual[drgtd_loop].delete_data.
        ptam_match_query,0,1073741824.0), d.ptam_match_function_ind = trg_table_data->qual[drgtd_loop
       ].delete_data.ptam_match_function_ind,
       d.pk_where_function_ind = trg_table_data->qual[drgtd_loop].delete_data.pk_where_function_ind,
     ...
     IF (check_error(dm_err->eproc)=1)
      ...
      GO TO exit_program
     ENDIF
    ELSEIF ((trg_table_data->qual[drgtd_loop].delete_data.ins_ind=1))
     INSERT  FROM dm_pk_where d
      SET d.table_name = trg_table_data->qual[drgtd_loop].table_name, d.delete_ind = 1, d
       .merge_delete_ind = trg_table_data->qual[drgtd_loop].merge_delete_ind,
       d.version_ind = trg_table_data->qual[drgtd_loop].version_ind, d.version_alg = trg_table_data->
       qual[drgtd_loop].version_alg, d.template_id = trg_table_data->qual[drgtd_loop].delete_data.
       template_id,
       d.pk_where = trg_table_data->qual[drgtd_loop].delete_data.pk_where, d.pk_where_hash =
       dm_get_hash_value(trg_table_data->qual[drgtd_loop].delete_data.pk_where,0,1073741824.0), d
       .ptam_match_query = trg_table_data->qual[drgtd_loop].delete_data.ptam_match_query,
       d.ptam_match_hash = dm_get_hash_value(trg_table_data->qual[drgtd_loop].delete_data.
        ptam_match_query,0,1073741824.0), d.ptam_match_function_ind = trg_table_data->qual[drgtd_loop
       ].delete_data.ptam_match_function_ind, d.pk_where_function_ind = trg_table_data->qual[
       drgtd_loop].delete_data.pk_where_function_ind,
       d.updt_task = reqinfo->updt_task, d.updt_applctx = reqinfo->updt_applctx, d.updt_id = reqinfo
       ->updt_id,
       d.updt_dt_tm = cnvtdatetime(curdate,curtime3)
      WITH nocounter
     ...
     ENDIF
    ENDIF
    DELETE  FROM dm_pk_where_parm d
     WHERE (d.table_name=trg_table_data->qual[drgtd_loop].table_name)
     WITH nocounter
    ;end delete
    IF (check_error(dm_err->eproc)=1)
     ...
     GO TO exit_program
    ENDIF
    SET dm_err->eproc = "Populating DM_PK_WHERE_PARM table."
    SET stat = alterlist(refchg_collist_query->qual,0)
    SET refchg_collist_query->err_ind = 0
    SET refchg_collist_query->err_msg = ""
    CALL parser(concat("execute ",trg_table_data->qual[drgtd_loop].non_delete_data.
      pk_where_parm_query,' "',trg_table_data->qual[drgtd_loop].table_name,'" go'),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     SET drgtd_reply->err_ind = 1
     SET drgtd_reply->err_msg = dm_err->emsg
     GO TO exit_program
    ELSEIF ((refchg_collist_query->err_ind=1))
     CALL disp_msg(refchg_collist_query->err_msg,dm_err->logfile,dm_err->err_ind)
     SET drgtd_reply->err_ind = 1
     SET drgtd_reply->err_msg = refchg_collist_query->err_msg
     GO TO exit_program
    ENDIF
    IF (size(refchg_collist_query->qual,5) > 0)
     INSERT  FROM dm_pk_where_parm d,
       (dummyt d2  WITH seq = size(refchg_collist_query->qual,5))
      SET d.table_name = trg_table_data->qual[drgtd_loop].table_name, d.function_type = "PK_WHERE", d
       .delete_ind = 0,
       d.parm_nbr = d2.seq, d.column_name = refchg_collist_query->qual[d2.seq].column_name, d
       .data_type = refchg_collist_query->qual[d2.seq].column_datatype,
       d.trans_ind = refchg_collist_query->qual[d2.seq].trans_ind, d.exist_ind = refchg_collist_query
       ->qual[d2.seq].exist_ind, d.updt_dt_tm = cnvtdatetime(curdate,curtime3)
      PLAN (d2)
       JOIN (d)
      WITH nocounter
     ;end insert
    ...
    SET refchg_collist_query->err_ind = 0
    SET refchg_collist_query->err_msg = ""
    CALL parser(concat("execute ",trg_table_data->qual[drgtd_loop].delete_data.pk_where_parm_query,
      ' "',trg_table_data->qual[drgtd_loop].table_name,'" go'),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     SET drgtd_reply->err_ind = 1
     SET drgtd_reply->err_msg = dm_err->emsg
    ...
    SET refchg_collist_query->err_msg = ""
    CALL parser(concat("execute ",trg_table_data->qual[drgtd_loop].non_delete_data.
      ptam_match_parm_query,' "',trg_table_data->qual[drgtd_loop].table_name,'" go'),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     SET drgtd_reply->err_ind = 1
     SET drgtd_reply->err_msg = dm_err->emsg
    ...
    IF (size(refchg_collist_query->qual,5) > 0)
     INSERT  FROM dm_pk_where_parm d,
       (dummyt d2  WITH seq = size(refchg_collist_query->qual,5))
      SET d.table_name = trg_table_data->qual[drgtd_loop].table_name, d.function_type = "PTAM_MATCH",
       d.delete_ind = 0,
       d.parm_nbr = d2.seq, d.column_name = refchg_collist_query->qual[d2.seq].column_name, d
       .data_type = refchg_collist_query->qual[d2.seq].column_datatype,
       d.trans_ind = refchg_collist_query->qual[d2.seq].trans_ind, d.exist_ind = refchg_collist_query
    ...
    SET refchg_collist_query->err_ind = 0
    SET refchg_collist_query->err_msg = ""
    CALL parser(concat("execute ",trg_table_data->qual[drgtd_loop].delete_data.ptam_match_parm_query,
      ' "',trg_table_data->qual[drgtd_loop].table_name,'" go'),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     SET drgtd_reply->err_ind = 1
    ...
    IF (size(refchg_collist_query->qual,5) > 0)
     ...
     ENDIF
    ENDIF
    SET drgtd_tab_cnt = (drgtd_tab_cnt+ 1)
    SET drgtd_reply->qual[drgtd_tab_cnt].table_name = trg_table_data->qual[drgtd_loop].table_name
    SET drgtd_reply->qual[drgtd_tab_cnt].table_suffix = trg_table_data->qual[drgtd_loop].table_suffix
    SET drgtd_reply->qual[drgtd_tab_cnt].mergeable_ind = trg_table_data->qual[drgtd_loop].
    mergeable_ind
    SET drgtd_reply->qual[drgtd_tab_cnt].reference_ind = trg_table_data->qual[drgtd_loop].
    reference_ind
    IF ((((trg_table_data->qual[drgtd_loop].non_delete_data.updt_ind=1)) OR ((((trg_table_data->qual[
    drgtd_loop].non_delete_data.ins_ind=1)) OR ((((trg_table_data->qual[drgtd_loop].delete_data.
    updt_ind=1)) OR ((trg_table_data->qual[drgtd_loop].delete_data.ins_ind=1))) )) )) )
     SET drgtd_reply->qual[drgtd_tab_cnt].metadata_change_ind = 1
    ENDIF
   ELSE
    IF ((dm_err->debug_flag > 0))
     SET miss_trg_data->total = (miss_trg_data->total+ 1)
     SET stat = alterlist(miss_trg_data->qual,miss_trg_data->total)
     SET miss_trg_data->qual[miss_trg_data->total].table_name = trg_table_data->qual[drgtd_loop].
     table_name
    ENDIF
   ENDIF
 ENDFOR
 SET stat = alterlist(drgtd_reply->qual,drgtd_tab_cnt)
 IF ((dm_err->debug_flag > 0))
  CALL echo(
   "The following tables have meta-data that prevents the creation of REFCHG triggers on them.")
  FOR (drgtd_loop = 1 TO miss_trg_data->total)
    CALL echo(concat("    ",miss_trg_data->qual[drgtd_loop].table_name))
  ENDFOR
 ENDIF
#exit_program
 ...
 ENDIF
 FREE RECORD trg_table_data
 SET dm_err->eproc = "...Ending dm_rmc_get_trg_data"
 CALL final_disp_msg("dm_rmc_trg_data_")
END GO
