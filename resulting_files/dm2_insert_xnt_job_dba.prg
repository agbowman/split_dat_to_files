CREATE PROGRAM dm2_insert_xnt_job:dba
 ...
 IF ((request->template_nbr <= 0))
  SET reply->status_data.status = "F"
  SET reply->status_data.subeventstatus.operationname = "validating request data"
  ...
  SET reply->status_data.subeventstatus.targetobjectvalue = uar_i18nbuildmessage(dcpj_i18nhandle,
   "BADTEMP","Invalid template number: %1","i",request->template_nbr)
  GO TO exit_script
 ENDIF
 ...
 INSERT  FROM dm_purge_job dpj
  SET dpj.job_id = reply->job_id, dpj.template_nbr = request->template_nbr, dpj.max_rows = 0,
   dpj.purge_flag = 0, dpj.active_flag = request->active_flag, dpj.updt_id = reqinfo->updt_id,
 ...
 IF (error(dcpj_errmsg,0) > 0)
  ROLLBACK
  SET reply->status_data.status = "F"
  SET reply->status_data.subeventstatus.operationname = "inserting XNT job"
  SET reply->status_data.subeventstatus.operationstatus = "F"
  SET reply->status_data.subeventstatus.targetobjectname = curprog
  SET reply->status_data.subeventstatus.targetobjectvalue = uar_i18nbuildmessage(dcpj_i18nhandle,
 ...
 IF (size(request->tokens,5) > 0)
  ...
  IF (error(dcpj_errmsg,0) > 0)
   ROLLBACK
   SET reply->status_data.status = "F"
   SET reply->status_data.subeventstatus.operationname = "inserting XNT job tokens"
   SET reply->status_data.subeventstatus.operationstatus = "F"
   SET reply->status_data.subeventstatus.targetobjectname = curprog
   SET reply->status_data.subeventstatus.targetobjectvalue = uar_i18nbuildmessage(dcpj_i18nhandle,
  ...
  ENDIF
 ENDIF
 IF (textlen(trim(request->job_name)) > 0)
  INSERT  FROM dm_purge_job_token dpjt
   SET dpjt.job_id = reply->job_id, dpjt.token_str = "JOBNAME", dpjt.value = request->job_name,
    dpjt.updt_id = reqinfo->updt_id, dpjt.updt_dt_tm = cnvtdatetime(curdate,curtime3), dpjt.updt_task
     = reqinfo->updt_task,
    dpjt.updt_cnt = 0, dpjt.updt_applctx = reqinfo->updt_applctx
   PLAN (dpjt)
  ...
  IF (error(dcpj_errmsg,0) > 0)
   ROLLBACK
   SET reply->status_data.status = "F"
   SET reply->status_data.subeventstatus.operationname = "inserting XNT job name"
   SET reply->status_data.subeventstatus.operationstatus = "F"
   SET reply->status_data.subeventstatus.targetobjectname = curprog
   SET reply->status_data.subeventstatus.targetobjectvalue = uar_i18nbuildmessage(dcpj_i18nhandle,
    "INSNAME","Failed to insert job name: %1","s",nullterm(dcpj_errmsg))
   GO TO exit_script
  ENDIF
 ENDIF
