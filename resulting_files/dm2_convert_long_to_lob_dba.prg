CREATE PROGRAM dm2_convert_long_to_lob:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("long_to_lob",".log","long_to_lob")=0)
  GO TO exit_program
 ENDIF
 DECLARE dcltl_table_name = vc WITH protect, constant(cnvtupper( $1))
 DECLARE dcltl_mode = vc WITH protect, constant(cnvtupper( $2))
 DECLARE dcltl_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcltl_t_ndx = i4 WITH protect, noconstant(0)
 DECLARE dcltl_icnt = i4 WITH protect, noconstant(0)
 DECLARE dcltl_error = i2 WITH protect, noconstant(0)
 DECLARE dcltl_max_ind_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcltl_load_columns(null) = i2
 DECLARE dcltl_get_space_total(dsc_table_in=vc) = i2
 DECLARE dcltl_verify_free_space(dvfs_tspace_in=vc,dvfs_bytes_in=f8) = f8
 DECLARE dcltl_load_indexes(null) = i2
 DECLARE dcltl_alter_indexes(dai_index_in=vc) = i2
 DECLARE dcltl_coalesce_tablespace(dct_tspace_in=vc) = i2
 DECLARE dcltl_convert_column(dcc_table_in=vc,dcc_col_in=vc,dcc_type_in=vc) = i2
 DECLARE dcltl_display_convert_report(null) = null
 FREE RECORD dcltl_tspchk
 RECORD dcltl_tspchk(
   1 dcltltsp_cnt = i4
   1 dcltl_need_cnt = i4
   1 qual[*]
     2 tablespace_name = vc
     2 max_table_bytes = f8
     2 bytes_needed = f8
     2 table_name = vc
 )
 FREE RECORD dcltl_driver
 RECORD dcltl_driver(
   1 dcltl_cnt = i4
   1 qual[*]
     2 table_name = vc
 ...
 FREE RECORD dcltl_indlist
 RECORD dcltl_indlist(
   1 dcltl_icnt = i4
   1 tab[*]
     2 table_name = vc
     2 ind_cnt = i4
 ...
 CALL dcltl_get_space_total(dcltl_table_name)
 FOR (dcltl_cnt = 1 TO dcltl_tspchk->dcltltsp_cnt)
  SET dcltl_tspchk->qual[dcltl_cnt].bytes_needed = dcltl_verify_free_space(dcltl_tspchk->qual[
   dcltl_cnt].tablespace_name,dcltl_tspchk->qual[dcltl_cnt].max_table_bytes)
  IF ((dcltl_tspchk->qual[dcltl_cnt].bytes_needed > 0))
   SET dcltl_tspchk->dcltl_need_cnt = (dcltl_tspchk->dcltl_need_cnt+ 1)
  ENDIF
 ENDFOR
 IF (dcltl_table_name="DM2_DDL_OPS_LOG*")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Validate Table Name"
  SET dm_err->emsg = "Invalid Table Name"
  SET dm_err->user_action = "Please select another table name that is not DM2_DDL_OPS_LOG*."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 CALL dcltl_tspace_report(null)
 IF (dcltl_mode="PREVIEW")
  SET dm_err->eproc = "Loading Columns."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  CALL dcltl_load_columns(null)
  SET dm_err->eproc = "Loading Indexes."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  CALL dcltl_load_indexes(null)
  CALL dcltl_display_convert_report(null)
  GO TO exit_program
 ELSEIF (dcltl_mode="CONVERT")
  IF ((dcltl_tspchk->dcltl_need_cnt > 0))
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Validate Tablespace Needs"
   SET dm_err->emsg = "Tablespace(s) need more space before convert may take place."
   SET dm_err->user_action = "Add more space to tablespace(s)."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ELSE
   SET dm_err->eproc = "Loading Columns."
   ...
   CALL dcltl_display_convert_report(null)
   SET dcltl_cnt = 0
   FOR (dcltl_cnt = 1 TO dcltl_driver->dcltl_cnt)
     SET dm_err->eproc = concat("Performing operations for ",dcltl_driver->qual[dcltl_cnt].table_name
      )
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     SET dm_err->eproc = concat("Altering column ",dcltl_driver->qual[dcltl_cnt].column_name,"to ",
      dcltl_driver->qual[dcltl_cnt].lob_data_type)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     IF (dcltl_convert_column(dcltl_driver->qual[dcltl_cnt].table_name,dcltl_driver->qual[dcltl_cnt].
      column_name,dcltl_driver->qual[dcltl_cnt].lob_data_type)=1)
      SET dm_err->eproc = concat("Coalesce:",dcltl_tspchk->qual[dcltl_t_ndx].tablespace_name)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      SET dcltl_t_ndx = 0
      SET dcltl_t_ndx = locateval(dcltl_t_ndx,1,dcltl_tspchk->dcltltsp_cnt,dcltl_driver->qual[
       dcltl_cnt].table_name,dcltl_tspchk->qual[dcltl_t_ndx].table_name)
      IF (dcltl_coalesce_tablespace(dcltl_tspchk->qual[dcltl_t_ndx].tablespace_name)=1)
       SET dcltl_t_ndx = 0
       SET dcltl_t_ndx = locateval(dcltl_t_ndx,1,size(dcltl_indlist->tab,5),dcltl_driver->qual[
        dcltl_cnt].table_name,dcltl_indlist->tab[dcltl_t_ndx].table_name)
       SET dcltl_icnt = 0
       FOR (dcltl_icnt = 1 TO size(dcltl_indlist->tab[dcltl_t_ndx].ind,5))
         SET dm_err->eproc = concat("Rebuilding index:",dcltl_indlist->tab[dcltl_t_ndx].ind[
          dcltl_icnt].index_name)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
         IF (dcltl_alter_indexes(dcltl_indlist->tab[dcltl_t_ndx].ind[dcltl_icnt].index_name)=1)
          SET dm_err->eproc = concat("Coalesce:",dcltl_indlist->tab[dcltl_t_ndx].ind[dcltl_icnt].
           tablespace_name)
          CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
          CALL dcltl_coalesce_tablespace(dcltl_indlist->tab[dcltl_t_ndx].ind[dcltl_icnt].
           tablespace_name)
         ENDIF
       ENDFOR
       IF ((dm_err->err_ind=0))
        ...
        SET dm_err->eproc = concat("Coalesce:",dcltl_tspchk->qual[dcltl_t_ndx].tablespace_name)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
        CALL dcltl_coalesce_tablespace(dcltl_tspchk->qual[dcltl_t_ndx].tablespace_name)
       ENDIF
      ENDIF
     ENDIF
     SET dm_err->err_ind = 0
   ENDFOR
   CALL dcltl_validate_indexes(null)
  ENDIF
 ENDIF
 SUBROUTINE dcltl_get_space_total(dsc_table_in)
   DECLARE dsc_ndx = i4 WITH protect, noconstant(0)
   DECLARE dsc_cnt = i4 WITH protect, noconstant(0)
   DECLARE dsc_sum = f8 WITH protect, noconstant(0.0)
   SELECT INTO "nl:"
    FROM user_tab_columns a,
     user_segments b,
     user_tables c
    WHERE a.table_name=patstring(dsc_table_in)
     AND a.table_name=b.segment_name
     AND a.table_name=c.table_name
     AND a.data_type=patstring("*LONG*")
     AND a.table_name != patstring("DM2_DDL_OPS_LOG*")
    ORDER BY a.table_name
    HEAD a.table_name
     dsc_sum = 0.0
    DETAIL
     dsc_sum = (dsc_sum+ b.bytes)
    FOOT  a.table_name
     dsc_ndx = 0
     IF (locateval(dsc_ndx,1,dcltl_tspchk->dcltltsp_cnt,c.tablespace_name,dcltl_tspchk->qual[dsc_ndx]
      .tablespace_name)=0)
      dsc_cnt = (dsc_cnt+ 1), stat = alterlist(dcltl_tspchk->qual,dsc_cnt), dcltl_tspchk->
      dcltltsp_cnt = (dcltl_tspchk->dcltltsp_cnt+ 1),
      dcltl_tspchk->qual[dsc_cnt].tablespace_name = c.tablespace_name, dcltl_tspchk->qual[dsc_cnt].
      max_table_bytes = dsc_sum, dcltl_tspchk->qual[dsc_cnt].table_name = c.table_name
     ELSE
      IF ((dsc_sum > dcltl_tspchk->qual[dsc_ndx].max_table_bytes))
       dcltl_tspchk->qual[dsc_cnt].max_table_bytes = dsc_sum
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
 END ;Subroutine
 SUBROUTINE dcltl_validate_indexes(null)
   DECLARE dvi_txt = vc WITH protect, noconstant("")
   DECLARE dvi_enter_ind = i2 WITH protect, noconstant(0)
   SELECT INTO mine
    FROM (dummyt d1  WITH seq = size(dcltl_indlist->tab,5)),
     user_indexes u,
     (dummyt d2  WITH seq = dcltl_max_ind_cnt)
    PLAN (d1)
     JOIN (d2
     WHERE d2.seq <= size(dcltl_indlist->tab[d1.seq].ind,5))
     JOIN (u
     WHERE (dcltl_indlist->tab[d1.seq].table_name=u.table_name)
      AND (dcltl_indlist->tab[d1.seq].ind[d2.seq].index_name=u.index_name)
      AND u.status="UNUSABLE")
    HEAD REPORT
     CALL center("*********** Status of Indexes Report ***********",0,132), row + 2
    DETAIL
     dvi_txt = "", dvi_txt = concat("Index: ",trim(u.index_name)," on Table: ",trim(u.table_name),
      " is ",
      trim(u.status),". "), col 1,
     dvi_txt, row + 1, dvi_enter_ind = 1
    FOOT REPORT
     IF (dvi_enter_ind=0)
      col 0, "All affected indexes are USABLE", row + 1
     ENDIF
    WITH nocounter, nullreport, maxcol = 300
   ;end select
 END ;Subroutine
 SUBROUTINE dcltl_verify_free_space(dvfs_tspace_in,dvfs_bytes_in)
   DECLARE dvfs_bytes_needed = f8 WITH protect, noconstant(0.0)
   SELECT INTO "nl:"
    tsp_sum_bytes = sum(a.bytes)
    FROM dba_free_space a
    WHERE a.tablespace_name=dvfs_tspace_in
    GROUP BY a.tablespace_name
    DETAIL
     IF ((tsp_sum_bytes <= (dvfs_bytes_in * 2)))
      dvfs_bytes_needed = ((dvfs_bytes_in * 2) - tsp_sum_bytes)
     ELSE
      dvfs_bytes_needed = 0
     ENDIF
    WITH nocounter
   ;end select
   RETURN(dvfs_bytes_needed)
 END ;Subroutine
 SUBROUTINE dcltl_tspace_report(null)
   DECLARE dtr_txt = vc WITH protect, noconstant("")
   DECLARE dtr_dash_line = vc WITH protect, constant(fillstring(132,"_"))
   DECLARE dtr_cnt = i4 WITH protect, noconstant(0)
   SELECT INTO mine
    FROM (dummyt d  WITH seq = dcltl_tspchk->dcltltsp_cnt)
    PLAN (d
     WHERE (dcltl_tspchk->qual[d.seq].bytes_needed > 0))
    HEAD REPORT
     CALL center("*********** Tablespace Report ***********",0,132), row + 1
     IF ((dcltl_tspchk->dcltl_need_cnt > 0))
      dtr_txt = concat("****You have ",trim(cnvtstring(dcltl_tspchk->dcltl_need_cnt)),
       " Tablespace(s) with needs. Exit out and add space to the Tablespace(s) below.***"),
      CALL center(dtr_txt,0,132)
     ELSE
      CALL center("****Tablespace is sufficient to perform long to lob conversion.***",0,132)
     ENDIF
     row + 2, col 0, "          Tablespace                    Bytes Needed(MB)",
     row + 1, col 0, dtr_dash_line,
     row + 2
     IF ((dcltl_tspchk->dcltl_need_cnt=0))
      col 0, "NO additional space is needed, you may convert your datatypes.", row + 1
     ENDIF
     dtr_cnt = 0
    DETAIL
     dtr_cnt = (dtr_cnt+ 1), dtr_txt = "", dtr_txt = build(dtr_cnt,"."),
     col 6, dtr_txt, col 10,
     dcltl_tspchk->qual[d.seq].tablespace_name, dtr_txt = "", dtr_txt = trim(cnvtstring(dm2ceil(((
        dcltl_tspchk->qual[d.seq].bytes_needed/ 1024)/ 1024)))),
     col 41, dtr_txt, row + 1
    WITH nocounter, nullreport, maxcol = 300
   ;end select
 END ;Subroutine
 SUBROUTINE dcltl_display_convert_report(null)
   DECLARE ddcr_txt = vc WITH protect, noconstant("")
   DECLARE ddcr_dash_line = vc WITH protect, constant(fillstring(132,"_"))
   DECLARE ddcr_cnt = i4 WITH protect, noconstant(0)
   DECLARE ddcr_ind_cnt = i4 WITH protect, noconstant(0)
   DECLARE ddcr_tab_cnt = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Displaying Conversion Report."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   SELECT INTO mine
    FROM (dummyt d  WITH seq = dcltl_driver->dcltl_cnt)
    PLAN (d
     WHERE d.seq > 0)
    HEAD REPORT
     CALL center("*********** Column Conversion Report ***********",0,132), row + 2, col 6,
     "Table                         Column                        From Type           To Type ", row
      + 1, col 0,
     ddcr_dash_line, row + 2
    DETAIL
     ddcr_txt = "", ddcr_txt = build(d.seq,"."), col 0,
     ddcr_txt, col 6, dcltl_driver->qual[d.seq].table_name,
     ddcr_txt = "", ddcr_txt = trim(dcltl_driver->qual[d.seq].column_name), col 36,
     ddcr_txt, col 66, dcltl_driver->qual[d.seq].long_data_type,
     col 86, dcltl_driver->qual[d.seq].lob_data_type, row + 1
    FOOT REPORT
     row + 2, col 0, " ",
     row + 2, col 0, "The following indexes will be Rebuilt.",
     row + 2, col 0, "Table Name                      Index Name",
     row + 1, col 0,
     "-------------------------------------------------------------------------------",
     row + 1
     FOR (ddcr_tab_cnt = 1 TO size(dcltl_indlist->tab,5))
       FOR (ddcr_ind_cnt = 1 TO size(dcltl_indlist->tab[ddcr_tab_cnt].ind,5))
         col 0, dcltl_indlist->tab[ddcr_tab_cnt].table_name, col 32,
         dcltl_indlist->tab[ddcr_tab_cnt].ind[ddcr_ind_cnt].index_name, row + 1
       ENDFOR
     ENDFOR
    WITH nocounter, nullreport, maxcol = 300
   ;end select
 END ;Subroutine
 SUBROUTINE dcltl_load_columns(null)
  DECLARE dlc_cnt = i4 WITH protect, noconstant(0)
  SELECT INTO "nl:"
   FROM user_tab_columns c,
    user_tables u
   WHERE u.table_name=patstring(dcltl_table_name)
    AND c.table_name=u.table_name
    AND c.data_type IN ("LONG", "LONG RAW")
    AND c.table_name != patstring("DM2_DDL_OPS_LOG*")
   ORDER BY c.table_name
   DETAIL
    dlc_cnt = (dlc_cnt+ 1), stat = alterlist(dcltl_driver->qual,dlc_cnt), dcltl_driver->qual[dlc_cnt]
    .column_name = c.column_name,
    dcltl_driver->qual[dlc_cnt].table_name = c.table_name, dcltl_driver->qual[dlc_cnt].long_data_type
     = c.data_type, dcltl_driver->qual[dlc_cnt].tablespace_name = u.tablespace_name
    CASE (c.data_type)
     OF "LONG":
      dcltl_driver->qual[dlc_cnt].lob_data_type = "CLOB"
     OF "LONG RAW":
      dcltl_driver->qual[dlc_cnt].lob_data_type = "BLOB"
    ENDCASE
   FOOT REPORT
    dcltl_driver->dcltl_cnt = dlc_cnt
   WITH nocounter
  ;end select
 END ;Subroutine
 SUBROUTINE dcltl_load_indexes(null)
   DECLARE dli_cnt = i4 WITH protect, noconstant(0)
   DECLARE dli_tab_cnt = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM user_indexes u,
     (dummyt d  WITH seq = dcltl_driver->dcltl_cnt)
    PLAN (d
     WHERE d.seq > 0)
     JOIN (u
     WHERE (u.table_name=dcltl_driver->qual[d.seq].table_name))
    ORDER BY u.table_name
    HEAD u.table_name
     dli_tab_cnt = (dli_tab_cnt+ 1)
     IF (mod(dli_tab_cnt,10)=1)
      stat = alterlist(dcltl_indlist->tab,(dli_tab_cnt+ 9))
     ENDIF
     dcltl_indlist->tab[dli_tab_cnt].table_name = u.table_name, dli_cnt = 0
    DETAIL
     dli_cnt = (dli_cnt+ 1)
     IF (mod(dli_cnt,10)=1)
      stat = alterlist(dcltl_indlist->tab[dli_tab_cnt].ind,(dli_cnt+ 9))
     ENDIF
     dcltl_indlist->tab[dli_tab_cnt].ind[dli_cnt].index_name = u.index_name, dcltl_indlist->tab[
     dli_tab_cnt].ind[dli_cnt].tablespace_name = u.tablespace_name
    FOOT  u.table_name
     IF (dli_cnt > dcltl_max_ind_cnt)
      dcltl_max_ind_cnt = dli_cnt
     ENDIF
     dcltl_indlist->tab[dli_tab_cnt].ind_cnt = dli_cnt, stat = alterlist(dcltl_indlist->tab[
      dli_tab_cnt].ind,dli_cnt)
    FOOT REPORT
     stat = alterlist(dcltl_indlist->tab,dli_tab_cnt), dcltl_indlist->dcltl_icnt = dli_tab_cnt
    WITH nocounter
   ;end select
 END ;Subroutine
 SUBROUTINE dcltl_alter_indexes(dai_index_in)
   CALL echo(concat("rdb alter index ",dai_index_in," rebuild go"))
   IF ((dm_err->debug_flag != 722))
    CALL dm2_push_cmd(concat("rdb alter index ",dai_index_in," rebuild go"),1)
   ENDIF
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dcltl_error = 1
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dcltl_convert_column(dcc_table_in,dcc_col_in,dcc_type_in)
   CALL echo(concat("rdb alter table ",dcc_table_in," modify( ",dcc_col_in," ",
     dcc_type_in," ) go"))
   IF ((dm_err->debug_flag != 722))
    CALL dm2_push_cmd(concat("rdb alter table ",dcc_table_in," modify( ",dcc_col_in," ",
      dcc_type_in," ) go"),1)
   ENDIF
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dcltl_coalesce_tablespace(dct_tspace_in)
   CALL echo(concat("rdb alter tablespace ",dct_tspace_in," coalesce go"))
   IF ((dm_err->debug_flag != 722))
    CALL dm2_push_cmd(concat("rdb alter tablespace ",dct_tspace_in," coalesce go"),1)
   ENDIF
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dcltl_error = 1
...
#exit_program
 IF (dcltl_error=1)
  SET dm_err->eproc = "Error occurred.Please view logfile."
 ELSE
  SET dm_err->eproc = "Operations completed successfully."
 ENDIF
 CALL final_disp_msg("long_to_lob")
END GO
