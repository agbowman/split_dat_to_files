CREATE PROGRAM dm_ce_med_admin_ident_rows:dba
 ...
 SET reply->table_name = "CE_MED_ADMIN_IDENT"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(cma.ce_med_admin_ident_id)
   FROM ce_med_admin_ident cma
   WHERE cma.ce_med_admin_ident_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(cma.ce_med_admin_ident_id)
  FROM ce_med_admin_ident cma
  DETAIL
   v_max_id = cnvtreal(maxval)
  WITH nocounter
 ;end select
 SET where_clause = concat("not exists (select 1 from ce_med_admin_ident_reltn cmar ",
  "where cma.ce_med_admin_ident_id = cmar.ce_med_admin_ident_id)")
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    cma.rowid
    FROM ce_med_admin_ident cma
    WHERE cma.ce_med_admin_ident_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(where_clause,"cma"))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = cma.rowid
    WITH nocounter, maxqual(cma,value(v_rows_left))
   ;end select
