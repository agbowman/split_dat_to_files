CREATE PROGRAM dm_ecmb_workflow_task_queue:dba
 ...
 DECLARE isencountercombinequeueable = i2 WITH protect, noconstant(true)
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_WORKFLOW_TASK_QUEUE"
  ...
  GO TO exit_sub
 ENDIF
 CALL processencntrcombtowtp(0)
 SUBROUTINE (processencntrcombtowtp(dummy=i2) =null)
   IF ((request->parent_table="ENCOUNTER"))
    SET wtptaskrequest->smode = "ENCOUNTER_COMBINE"
   ...
   IF ((wtptaskrequest->cmb_qual[1].from_xxx_id != 0))
    SELECT INTO "nl:"
     ...
     PLAN (ecd
      WHERE (ecd.entity_id=wtptaskrequest->cmb_qual[1].from_xxx_id)
       AND ecd.entity_name="WORKFLOW_TASK_QUEUE"
       AND ecd.active_ind=true)
    ;end select
    IF (curqual)
     SET isencountercombinequeueable = false
    ENDIF
   ENDIF
   IF (usewtpframework)
    IF (isencountercombinequeueable)
     SET icombinedet += 1
