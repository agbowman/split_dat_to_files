CREATE PROGRAM dm2_upd_schema_instance:dba
 ...
 SUBROUTINE dsr_chk_sd_in_process(dcsip_db_group,dcsip_owner,dcsip_sd_in_process)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dusi_logfile = vc WITH public, constant("dm2updschinst")
 DECLARE dusi_in_schema_instance = i4 WITH public, noconstant(0)
 DECLARE dusi_in_table_name = vc WITH public, noconstant(" ")
 DECLARE dusi_dminfo_schema_instance = i4 WITH public, noconstant(0)
 DECLARE dusi_commit_ind = i2 WITH protect, noconstant(1)
 DECLARE dusi_sql_cmd = vc WITH protect, noconstant("")
 DECLARE dusi_sd_in_use_ind = i2 WITH protect, noconstant(- (1))
 DECLARE dusi_info_domain = vc WITH protect, noconstant("DM2_SCHEMA_INSTANCE")
 IF (currdbuser="CDBA")
  SET dusi_info_domain = "DM2_ADMIN_TABLE"
  SET dusi_commit_ind = 0
 ENDIF
 IF (check_logfile(dusi_logfile,".log","DM2_UPD_SCHEMA_INSTANCE LOG FILE BEGIN ")=0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_upd_schema_instance"
 IF ((dm_err->debug_flag >= 5))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SET dusi_sd_in_use_ind = validate(dis_sd_in_use_ind,- (1))
 IF ((dusi_sd_in_use_ind=- (1)))
  IF (dsr_sd_in_use_check("DM2NOTSET",dusi_sd_in_use_ind)=0)
   GO TO end_program
  ENDIF
 ENDIF
 SET dusi_in_table_name = cnvtupper(trim( $1))
 SET dusi_in_schema_instance =  $2
 IF ((dm_err->debug_flag >= 5))
  CALL echo(build("Input Parameter Table:", $1))
  CALL echo(build("Input Parameter Schema Instance :", $2))
 ENDIF
 IF (dusi_sd_in_use_ind=0)
  SET dm_err->eproc = concat("Determine if DM_INFO row exists for table ",dusi_in_table_name)
  IF ((dm_err->debug_flag >= 5))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   i.info_number
   FROM dm_info i
   WHERE i.info_domain=value(dusi_info_domain)
    AND i.info_name=dusi_in_table_name
   DETAIL
    dusi_dminfo_schema_instance = i.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) > 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF (curqual > 0)
   IF (dusi_dminfo_schema_instance < dusi_in_schema_instance)
    SET dm_err->eproc = "Update Schema Instance row on DM_INFO"
    IF ((dm_err->debug_flag >= 5))
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
    UPDATE  FROM dm_info i
     SET i.info_number = dusi_in_schema_instance, i.updt_dt_tm = cnvtdatetime(curdate,curtime3), i
    ...
    ENDIF
    IF ((dm_err->debug_flag >= 5))
     CALL echo(build("Update Schema Instance Row Curqual:",curqual))
    ENDIF
    IF (dusi_commit_ind=1)
     COMMIT
    ENDIF
   ENDIF
  ELSE
   ...
   INSERT  FROM dm_info i
    SET i.info_domain = value(dusi_info_domain), i.info_name = dusi_in_table_name, i.info_date = null,
     i.info_char = "", i.info_number = dusi_in_schema_instance, i.info_long_id = 0,
   ...
   ENDIF
   IF ((dm_err->debug_flag >= 5))
    CALL echo(build("Insert of Schema Instance Rows Curqual :",curqual))
   ENDIF
   IF (dusi_commit_ind=1)
    COMMIT
   ENDIF
  ENDIF
 ELSE
  SET dm_err->eproc = "Update version via SYNC_TABLE_VERSION "
  IF ((dm_err->debug_flag >= 5))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SET dusi_sql_cmd = trim(build(" rdb asis( ^  "," begin  ",
    "   CERADM.SD_OBJECT_VERSION_PKG.SYNC_TABLE_VERSION( ","   '",currdbuser,
    "', ","   '",dusi_in_table_name,"', ","   ",
    dusi_in_schema_instance,", ","   1,","   ",dusi_commit_ind,
    ");"," end; "," ^ ) go"))
  IF (dm2_push_cmd(dusi_sql_cmd,1)=0)
   ROLLBACK
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO end_program
  ENDIF
 ...
 GO TO end_program
#end_program
 SET dm_err->eproc = "Exiting dm2_upd_schema_instance"
 CALL final_disp_msg(dusi_logfile)
END GO
