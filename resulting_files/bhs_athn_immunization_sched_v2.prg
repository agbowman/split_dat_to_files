CREATE PROGRAM bhs_athn_immunization_sched_v2
 ...
 SET hm_request->eval_start_dt_tm = cnvtdatetime(sysdate)
 SET hm_request->eval_end_dt_tm = cnvtdatetime(sysdate)
 ...
 ELSE
  ...
  SELECT INTO "NL:"
   ...
   HEAD he.expect_id
    ecnt += 1, stat = alterlist(immunizations->series,ecnt), immunizations->series[ecnt].
   ...
   HEAD hep.expect_step_id
    scnt += 1, stat = alterlist(immunizations->series[ecnt].steps,scnt), immunizations->series[ecnt].
    steps[scnt].expect_step_id = hep.expect_step_id,
    ...
    ->series[ecnt].steps[scnt].audience_flag = hep.audience_flag
   FOOT  he.expect_id
    row + 0
  ...
  FOR (idx = 1 TO size(immunizations->series,5))
    FOR (jdx = 1 TO size(immunizations->series[idx].steps,5))
     ...
     IF (pos > 0)
      SET immunizations->series[idx].steps[jdx].recommend_due_dt_tm = datetimezoneformat(reply->
       person[1].reminder[pos].recommend_due_dt_tm,curtimezonesys,"MM/dd/yyyy HH:mm:ss",
       curtimezonedef)
     ENDIF
    ENDFOR
  ENDFOR
 ...
 SELECT INTO "NL:"
  ...
  PLAN (es
   ...
   WHERE (cen.event_id= Outerjoin(ce.event_id))
    AND (cen.valid_until_dt_tm> Outerjoin(cnvtdatetime(sysdate)))
    AND (cen.note_type_cd= Outerjoin(c_res_comment_cd)) )
  ORDER BY he.expect_id, ce.event_end_dt_tm, cen.event_id DESC
  HEAD he.expect_id
   ecnt += 1, stat = alterlist(events->series,ecnt), events->series[ecnt].expect_id = he.expect_id,
  ...
  HEAD ce.clinical_event_id
   IF (cen.event_id=0)
    acnt += 1, stat = alterlist(events->series[ecnt].admins,acnt), events->series[ecnt].admins[acnt].
    clinical_event_id = ce.clinical_event_id,
    events->series[ecnt].admins[acnt].event_id = ce.event_id, events->series[ecnt].admins[acnt].
    admin_dt_tm = datetimezoneformat(ce.event_end_dt_tm,curtimezonesys,"MM/dd/yyyy HH:mm:ss",
     curtimezonedef), events->series[ecnt].admins[acnt].performed_prsnl_id = ce.performed_prsnl_id,
