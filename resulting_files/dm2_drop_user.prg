CREATE PROGRAM dm2_drop_user
 ...
 SUBROUTINE drr_del_preserved_ts(dcdl_tgt_db_name)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ddu_get_tables_mviews(dgtm_user=vc) = i2
 DECLARE ddu_insert_ddl_op_row(null) = i2
 DECLARE ddu_check_user_exists(dcue_user=vc,dcue_user_exists_ind=i2(ref)) = i2
 DECLARE ddu_determine_method(ddm_restricted_method_ind=i2(ref)) = i2
 DECLARE ddu_check_for_connections(dcfc_user=vc,dcfc_connect_fnd_ind=i2(ref)) = i2
 DECLARE ddu_drop_objects_serially(null) = i2
 DECLARE ddu_drop_objects_in_parallel(null) = i2
 DECLARE ddu_prep_for_drop(null) = i2
 DECLARE ddu_standalone = i2 WITH protect, noconstant(0)
 DECLARE ddu_dbname = vc WITH protect, noconstant("")
 DECLARE ddu_user = vc WITH protect, noconstant("")
 DECLARE ddu_ses_attempts = i4 WITH protect, noconstant(0)
 DECLARE ddu_user_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE ddu_connect_fnd_ind = i2 WITH protect, noconstant(0)
 DECLARE ddu_restricted_method_ind = i2 WITH protect, noconstant(0)
 DECLARE ddu_run_id = f8 WITH protect, noconstant(0.0)
 DECLARE ddu_op_id = f8 WITH protect, noconstant(0.0)
 DECLARE ddu_schema_date = dq8 WITH protect, noconstant(0.0)
 DECLARE ddu_op_type = vc WITH protect, noconstant("")
 DECLARE ddu_table_name = vc WITH protect, noconstant("")
 DECLARE ddu_obj_name = vc WITH protect, noconstant("")
 DECLARE ddu_priority = i4 WITH protect, noconstant(0)
 DECLARE ddu_operation = vc WITH protect, noconstant("")
 FREE RECORD ddu_tab
 RECORD ddu_tab(
   1 attempt = i2
   1 qual[*]
 ...
 FREE RECORD ddu_excl_tbl
 RECORD ddu_excl_tbl(
   1 qual[*]
     2 tbl_name = vc
 )
 IF ((dm2_install_schema->curprog="NONE"))
  SET ddu_standalone = 1
 ENDIF
 IF (check_logfile("dm2_drop_user",".log","dm2_drop_user")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_drop_user"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET ddu_user = cnvtupper( $1)
 IF (ddu_user="SYS")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Cannot drop the SYS user."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (ddu_determine_method(ddu_restricted_method_ind)=0)
  GO TO exit_program
 ENDIF
 IF (ddu_check_user_exists(ddu_user,ddu_user_exists_ind)=0)
  GO TO exit_program
 ENDIF
 IF (ddu_user_exists_ind=0)
  SET dm_err->eproc = concat("Database User ",ddu_user," does not exist in DBA_USERS.")
  CALL disp_msg(" ",dm_err->logfile,0)
 ELSE
  SET dm_err->eproc = concat("Database User ",ddu_user," exists. Preparing to drop.")
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (ddu_prep_for_drop(null)=0)
   GO TO exit_program
  ENDIF
  IF (((validate(dm2_bypass_drop_obj_parallel,0)=1) OR (validate(drrr_responsefile_in_use,0)=0)) )
   IF (ddu_drop_objects_serially(null)=0)
    GO TO exit_program
   ENDIF
  ELSE
   IF (ddu_drop_objects_in_parallel(null)=0)
    GO TO exit_program
   ENDIF
  ENDIF
  IF (ddu_check_for_connections(ddu_user,ddu_connect_fnd_ind)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (ddu_connect_fnd_ind=1)
  IF (ddu_restricted_method_ind=1
   AND (validate(dm2_rrr_skip_refresh_drop_restrict,- (1))=- (1)))
   SET dm_err->eproc = concat(ddu_user," user is currently connected or unable to drop the user.",
    "Restarting the database in restrict mode and dropping the user ",ddu_user,".")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   SET ddu_ses_attempts = 1
   WHILE (ddu_ses_attempts <= 5)
     IF (drr_refresh_drop_restrict("I",0)=0)
      GO TO exit_program
     ENDIF
     IF (ddu_check_user_exists(ddu_user,ddu_user_exists_ind)=0)
      GO TO exit_program
     ENDIF
     IF (ddu_user_exists_ind=1)
      SET ddu_ses_attempts = (ddu_ses_attempts+ 1)
     ELSE
      SET ddu_ses_attempts = 6
     ENDIF
   ENDWHILE
  ELSE
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat(ddu_user," user is currently connected.")
   SET dm_err->user_action = concat("Disconnect any sessions currently connected as ",ddu_user,
    " for ",ddu_dbname," database or check the logfile for the errors before continuing.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (ddu_check_user_exists(ddu_user,ddu_user_exists_ind)=0)
 ...
 ENDIF
 IF (ddu_user_exists_ind=1)
  SET dm_err->eproc = concat("Dropping User:",ddu_user)
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dm2_push_cmd(concat("rdb asis(^ drop user ",ddu_user," cascade ^) go"),1)=0)
   IF (ddu_restricted_method_ind=1
    AND findstring("ORA-01940",cnvtupper(dm_err->emsg),1,0) > 0
    AND (validate(dm2_rrr_skip_refresh_drop_restrict,- (1))=- (1)))
    SET dm_err->eproc = concat(ddu_user," user is currently connected or unable to drop the user.",
     "Restarting the database in restrict mode and dropping the user ",ddu_user,".")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 ...
 ENDIF
 IF (ddu_restricted_method_ind=1)
  SET dm_err->eproc = "Deleting the Admin dm_info row for refresh drop restrict."
  CALL disp_msg("",dm_err->logfile,0)
  IF (drr_refresh_drop_restrict("D",0)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 GO TO exit_program
 SUBROUTINE ddu_get_tables_mviews(dgtm_user)
   DECLARE dgtm_pos = i4 WITH protect, noconstant(0)
   DECLARE dgtm_mv_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgtm_tbl_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgtm_cons_cnt = i4 WITH protect, noconstant(0)
   SET ddu_mv->mv_cnt = 0
   SET stat = alterlist(ddu_mv->qual,ddu_mv->mv_cnt)
   SET dm_err->eproc = concat("Get list of Materialized Views for user ",dgtm_user)
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_mviews dm
    WHERE dm.owner=dgtm_user
    HEAD REPORT
     dgtm_mv_cnt = 0
    DETAIL
     dgtm_mv_cnt = (dgtm_mv_cnt+ 1)
     IF (mod(dgtm_mv_cnt,50)=1)
      stat = alterlist(ddu_mv->qual,(dgtm_mv_cnt+ 49))
     ENDIF
     ddu_mv->qual[dgtm_mv_cnt].mv_name = dm.mview_name
    FOOT REPORT
     stat = alterlist(ddu_mv->qual,dgtm_mv_cnt), ddu_mv->mv_cnt = dgtm_mv_cnt
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 2))
    CALL echorecord(ddu_mv)
   ENDIF
   SET dm_err->eproc = concat("Get list of tables for user ",dgtm_user)
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_tables d
    WHERE d.owner=dgtm_user
     AND d.table_name != "MLOG*"
    HEAD REPORT
     dgtm_tbl_cnt = 0
    DETAIL
     IF (locateval(dgtm_pos,1,size(ddu_excl_tbl->qual,5),d.table_name,ddu_excl_tbl->qual[dgtm_pos].
      tbl_name)=0)
      dgtm_tbl_cnt = (dgtm_tbl_cnt+ 1)
      IF (mod(dgtm_tbl_cnt,500)=1)
       stat = alterlist(ddu_tab->qual,(dgtm_tbl_cnt+ 499))
      ENDIF
      ddu_tab->qual[dgtm_tbl_cnt].tbl_name = d.table_name
     ENDIF
    FOOT REPORT
     stat = alterlist(ddu_tab->qual,dgtm_tbl_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 2))
    CALL echorecord(ddu_tab)
   ENDIF
   IF (ddu_restricted_method_ind=1)
    SET ddu_fk_cons->constraint_cnt = 0
    SET stat = alterlist(ddu_fk_cons->qual,ddu_fk_cons->constraint_cnt)
    SET dm_err->eproc = concat("Get list of foreign key constraints for user ",dgtm_user)
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dgtm_pos = 0
    SELECT INTO "nl:"
     ...
     DETAIL
      IF (locateval(dgtm_pos,1,size(ddu_excl_tbl->qual,5),d.table_name,ddu_excl_tbl->qual[dgtm_pos].
       tbl_name)=0)
       dgtm_cons_cnt = (dgtm_cons_cnt+ 1)
       IF (mod(dgtm_cons_cnt,500)=1)
        stat = alterlist(ddu_fk_cons->qual,(dgtm_cons_cnt+ 499))
       ENDIF
       ddu_fk_cons->qual[dgtm_cons_cnt].constraint_name = d.constraint_name, ddu_fk_cons->qual[
       dgtm_cons_cnt].table_name = d.table_name
      ENDIF
     FOOT REPORT
      stat = alterlist(ddu_fk_cons->qual,dgtm_cons_cnt), ddu_fk_cons->constraint_cnt = dgtm_cons_cnt
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    IF ((dm_err->debug_flag > 2))
     CALL echorecord(ddu_fk_cons)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddu_insert_ddl_op_row(null)
   SET dm_err->eproc = "Inserting dm2_ddl_ops_log row for operation."
   INSERT  FROM dm2_ddl_ops_log ddol
    SET ddol.op_id = ddu_op_id, ddol.owner_name = ddu_user, ddol.run_id = ddu_run_id,
     ddol.gen_dt_tm = cnvtdatetime(curdate,curtime3), ddol.gen_id = 1, ddol.begin_dt_tm =
     cnvtdatetime(null),
     ddol.end_dt_tm = cnvtdatetime(null), ddol.est_duration = null, ddol.act_duration = null,
     ddol.op_type = ddu_op_type, ddol.table_name = ddu_table_name, ddol.obj_name = ddu_obj_name,
     ddol.priority = ddu_priority, ddol.status = null, ddol.error_msg = null,
     ddol.operation = ddu_operation, ddol.on_error_operation = null, ddol.schema_date = cnvtdatetime(
      ddu_schema_date),
     ddol.schema_instance = 0, ddol.ready_to_run_ind = 0, ddol.appl_id = currdbhandle
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddu_check_user_exists(dcue_user,dcue_user_exists_ind)
   SET dm_err->eproc = concat("Check if the user ",dcue_user," exists.")
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET dcue_user_exists_ind = 0
   SELECT INTO "nl:"
    FROM dba_users d
    WHERE d.username=dcue_user
    DETAIL
     dcue_user_exists_ind = 1
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddu_determine_method(ddm_restricted_method_ind)
   SET dm_err->eproc = "Determining if restrict method is used to drop the user."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET ddm_restricted_method_ind = 0
   IF (ddu_user="V500"
    AND validate(drrr_responsefile_in_use,0)=1
    AND cnvtupper(program_stack_rs->qual[(program_stack_rs->cnt - 1)].name)="DM2_REFRESH_V500_USER"
    AND cnvtlower(curnode) != cnvtlower(validate(drrr_rf_data->tgt_db_node,"DM2NOTSET"))
    AND "LNX"=cnvtupper(validate(drrr_rf_data->tgt_db_node_os,"DM2NOTSET"))
    AND validate(drrr_rf_data->tgt_db_deploy_config,"z")="OP")
    SET ddm_restricted_method_ind = 1
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddu_drop_objects_serially(null)
   DECLARE ddos_mv_cnt = i4 WITH protect, noconstant(0)
   DECLARE ddos_tbl_cnt = i4 WITH protect, noconstant(0)
   DECLARE ddos_mv_str = vc WITH protect, noconstant("")
   DECLARE ddos_tbl_str = vc WITH protect, noconstant("")
   DECLARE ddos_ndx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = concat("Dropping ",ddu_user," objects serially.")
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET stat = initrec(ddu_excl_tbl)
   IF (ddu_get_tables_mviews(ddu_user)=0)
    RETURN(0)
   ENDIF
   IF ((((ddu_mv->mv_cnt > 0)) OR (size(ddu_tab->qual,5) > 0)) )
    IF ((ddu_mv->mv_cnt > 0))
     SET dm_err->eproc = concat("Dropping Materialzed Views for user ",ddu_user)
     CALL disp_msg(" ",dm_err->logfile,0)
     FOR (ddos_mv_cnt = 1 TO ddu_mv->mv_cnt)
       SET dm_err->eproc = concat("Dropping Materialized View:",ddu_mv->qual[ddos_mv_cnt].mv_name)
       IF ((dm_err->debug_flag > 0))
        CALL disp_msg(" ",dm_err->logfile,0)
       ENDIF
       SET ddos_mv_str = ""
       SET ddos_mv_str = concat("rdb asis(^ drop materialized view ",ddu_user,".",ddu_mv->qual[
        ddos_mv_cnt].mv_name,"^) go")
       IF (dm2_push_cmd(ddos_mv_str,1)=0)
        SET dm_err->err_ind = 0
       ENDIF
     ENDFOR
    ELSE
     SET dm_err->eproc = concat("No Materialized Views to drop for user ",ddu_user)
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    IF (size(ddu_tab->qual,5)=0)
     SET dm_err->eproc = concat("No tables exist for user ",ddu_user)
     CALL disp_msg(" ",dm_err->logfile,0)
    ELSE
     SET dm_err->eproc = concat("Starting to drop tables for user ",ddu_user)
     CALL disp_msg(" ",dm_err->logfile,0)
     SET ddu_tab->attempt = 1
     WHILE ((ddu_tab->attempt <= 2))
       FOR (ddos_tbl_cnt = 1 TO size(ddu_tab->qual,5))
         IF ((ddu_tab->qual[ddos_tbl_cnt].drop_success=0))
          SET dm_err->eproc = concat("Dropping table:",ddu_tab->qual[ddos_tbl_cnt].tbl_name)
          IF ((dm_err->debug_flag > 0))
           CALL disp_msg(" ",dm_err->logfile,0)
          ENDIF
          SET ddos_tbl_str = ""
          IF ((dm2_rdbms_version->level1 >= 10))
           SET ddos_tbl_str = concat("rdb asis(^ drop table ",ddu_user,".",ddu_tab->qual[ddos_tbl_cnt
            ].tbl_name," cascade constraints purge ^) go")
          ELSE
           SET ddos_tbl_str = concat("rdb asis(^ drop table ",ddu_user,".",ddu_tab->qual[ddos_tbl_cnt
            ].tbl_name," cascade constraints ^) go")
          ENDIF
          IF (dm2_push_cmd(ddos_tbl_str,1)=0)
           SET dm_err->err_ind = 0
           SET ddu_tab->qual[ddos_tbl_cnt].drop_success = 0
          ELSE
           SET ddu_tab->qual[ddos_tbl_cnt].drop_success = 1
          ENDIF
         ENDIF
       ENDFOR
       SET ddos_ndx = 0
       SET ddos_ndx = locateval(ddos_ndx,1,size(ddu_tab->qual,5),0,ddu_tab->qual[ddos_ndx].
        drop_success)
       IF (ddos_ndx > 0)
        SET ddu_tab->attempt = (ddu_tab->attempt+ 1)
       ELSE
        SET ddu_tab->attempt = 5
       ENDIF
     ENDWHILE
     IF ((ddu_tab->attempt < 5))
      SET dm_err->eproc = concat("Informational:Unable to drop all tables for user: ",ddu_user,".")
      CALL disp_msg(" ",dm_err->logfile,0)
     ELSE
      SET dm_err->eproc = concat("Finished dropping tables for user ",ddu_user)
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddu_drop_objects_in_parallel(null)
   DECLARE ddoip_info_domain = vc WITH protect, noconstant("")
   DECLARE ddoip_handle = vc WITH protect, noconstant("")
   DECLARE ddoip_appl_status = vc WITH protect, noconstant("")
   DECLARE ddoip_schema_date_offset = i2 WITH protect, noconstant(0)
   DECLARE ddoip_schema_date_search_continue = i2 WITH protect, noconstant(0)
   DECLARE ddoip_ses_attempts = i4 WITH protect, noconstant(0)
   DECLARE ddoip_iter = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = concat("Dropping ",ddu_user," objects in parallel.")
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET stat = alterlist(ddu_excl_tbl->qual,3)
   SET ddu_excl_tbl->qual[1].tbl_name = "DM2_DDL_OPS1"
   SET ddu_excl_tbl->qual[2].tbl_name = "DM2_DDL_OPS_LOG1"
   SET ddu_excl_tbl->qual[3].tbl_name = "DM_INFO"
   IF (ddu_get_tables_mviews(ddu_user)=0)
    RETURN(0)
   ENDIF
   IF ((((ddu_mv->mv_cnt > 0)) OR (size(ddu_tab->qual,5) > 0)) )
    SET ddoip_info_domain = "CLIN COPY - DROP OBJECTS"
    SET dm2_install_schema->dbase_name = '"TARGET"'
    SET dm2_install_schema->u_name = "V500"
    SET dm2_install_schema->p_word = dm2_install_schema->v500_p_word
    IF ((dm_err->debug_flag > 0))
    ...
    IF ((dm_err->err_ind=1))
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Determining if v500 drop objects is currently running, from dm_info."
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm_info di
     WHERE di.info_domain=ddoip_info_domain
     DETAIL
      ddoip_handle = di.info_char
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    IF (ddoip_handle > "")
     IF (assign(ddoip_appl_status,dm2_get_appl_status(ddoip_handle))="E")
      RETURN(0)
     ELSEIF (ddoip_appl_status="A")
      SET dm_err->err_ind = 1
     ...
     ELSE
      SET dm_err->eproc =
      "Updating row in dm_info to show that v500 drop objects is currently running."
      UPDATE  FROM dm_info di
       SET di.info_char = value(trim(currdbhandle,3))
       WHERE di.info_domain=ddoip_info_domain
        AND di.info_name=cnvtupper(dm2_install_schema->target_dbase_name)
       WITH nocounter
      ;end update
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ELSE
     SET dm_err->eproc =
     "Inserting row into dm_info to show that v500 drop objects is currently running."
     INSERT  FROM dm_info di
      SET di.info_domain = ddoip_info_domain, di.info_name = cnvtupper(dm2_install_schema->
        target_dbase_name), di.info_char = value(trim(currdbhandle,3))
      WITH nocounter
     ;end insert
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    SELECT INTO "nl:"
     ...
     WHERE ddo.process_option=ddoip_info_domain
     DETAIL
      ddu_run_id = ddo.run_id, ddu_schema_date = ddo.schema_date
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (curqual=0)
     ...
     SELECT INTO "nl:"
      ...
      HEAD REPORT
       ddu_run_id = 0
      DETAIL
       ddu_run_id = max_run_id
      FOOT REPORT
       ddu_run_id = (ddu_run_id+ 1)
     ...
     IF (check_error(dm_err->eproc)=1)
      ...
      RETURN(0)
     ENDIF
     SET ddoip_schema_date_search_continue = 1
     SET ddoip_schema_date_offset = 365
     WHILE (ddoip_schema_date_search_continue=1
      AND ddoip_schema_date_offset >= 335)
       SET ddoip_schema_date_offset = (ddoip_schema_date_offset - 1)
       SET ddu_schema_date = cnvtdatetime((curdate - ddoip_schema_date_offset),0)
       SET dm_err->eproc = "Determining usable schema date from dm2_ddl_ops."
       CALL disp_msg(" ",dm_err->logfile,0)
       SELECT INTO "nl:"
        FROM dm2_ddl_ops ddo
        WHERE ddo.schema_date=cnvtdatetime(ddu_schema_date)
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       ...
       ENDIF
       IF (curqual=0)
        SET ddoip_schema_date_search_continue = 0
       ENDIF
     ENDWHILE
     IF (ddoip_schema_date_search_continue=1)
      SET dm_err->err_ind = 1
     ...
     INSERT  FROM dm2_ddl_ops ddo
      SET ddo.run_id = ddu_run_id, ddo.gen_dt_tm = cnvtdatetime(curdate,curtime3), ddo.ocd = 0.0,
       ddo.schema_date = cnvtdatetime(ddu_schema_date), ddo.process_option = ddoip_info_domain, ddo
    ...
    IF (check_error(dm_err->eproc)=1)
     ...
     RETURN(0)
    ENDIF
    SET ddoip_ses_attempts = 1
    WHILE (ddoip_ses_attempts <= 4)
      FOR (ddoip_iter = 1 TO ddu_fk_cons->constraint_cnt)
        SET ddu_op_id = (ddu_op_id+ 1)
        SET ddu_op_type = "DROP FK CONSTRAINT"
        SET ddu_obj_name = ddu_fk_cons->qual[ddoip_iter].constraint_name
        SET ddu_table_name = ddu_fk_cons->qual[ddoip_iter].table_name
        SET ddu_priority = 1
        SET ddu_operation = concat("rdb asis(^ alter table ",ddu_user,".",ddu_fk_cons->qual[
         ddoip_iter].table_name," drop constraint ",
         ddu_fk_cons->qual[ddoip_iter].constraint_name,"^) go")
        IF (ddu_insert_ddl_op_row(null)=0)
         RETURN(0)
        ENDIF
      ENDFOR
      FOR (ddoip_iter = 1 TO ddu_mv->mv_cnt)
        SET ddu_op_id = (ddu_op_id+ 1)
        SET ddu_op_type = "DROP MATERIAILIZED VIEW"
        SET ddu_obj_name = ddu_mv->qual[ddoip_iter].mv_name
        SET ddu_table_name = ddu_mv->qual[ddoip_iter].mv_name
        SET ddu_priority = 2
        SET ddu_operation = concat("rdb asis(^ drop materialized view ",ddu_user,".",ddu_mv->qual[
         ddoip_iter].mv_name,"^) go")
        IF (ddu_insert_ddl_op_row(null)=0)
         RETURN(0)
        ENDIF
      ENDFOR
      FOR (ddoip_iter = 1 TO size(ddu_tab->qual,5))
        SET ddu_op_id = (ddu_op_id+ 1)
        SET ddu_op_type = "DROP TABLE"
        SET ddu_obj_name = ddu_tab->qual[ddoip_iter].tbl_name
        SET ddu_table_name = ddu_tab->qual[ddoip_iter].tbl_name
        SET ddu_priority = 3
        SET ddu_operation = concat("rdb asis(^ drop table ",ddu_user,".",ddu_tab->qual[ddoip_iter].
         tbl_name," cascade constraints purge ^) go")
        IF (ddu_insert_ddl_op_row(null)=0)
         RETURN(0)
        ENDIF
      ENDFOR
      SET dm2_install_schema->process_option = ddoip_info_domain
      SET dm2_install_schema->run_id = ddu_run_id
      ...
      IF (curqual > 0)
       ...
       SET dm_err->emsg = build(
        "There are still rows not marked as complete in dm2_ddl_ops_log for this run_id:",ddu_run_id)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ENDIF
      IF (ddu_get_tables_mviews(ddu_user)=0)
      ...
      ENDIF
      IF ((((ddu_mv->mv_cnt > 0)) OR (size(ddu_tab->qual,5) > 0)) )
       SET ddoip_ses_attempts = (ddoip_ses_attempts+ 1)
      ELSE
       SET ddoip_ses_attempts = 6
      ENDIF
    ENDWHILE
    IF ((((ddu_mv->mv_cnt > 0)) OR (size(ddu_tab->qual,5) > 0)) )
     SET dm_err->eproc = concat("Informational:Unable to drop all tables for user: ",ddu_user,".")
    ...
    ELSE
     ...
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SET stat = alterlist(ddu_excl_tbl->qual,0)
    IF (ddu_get_tables_mviews(ddu_user)=0)
     RETURN(0)
    ENDIF
    SET dm_err->eproc =
    "Dropping tables that were excluded earlier and/or attempting to drop the tables that were not dropped."
    CALL disp_msg(" ",dm_err->logfile,0)
    FOR (ddoip_iter = 1 TO size(ddu_tab->qual,5))
      IF (dm2_push_cmd(concat("rdb asis(^ drop table ",ddu_user,".",ddu_tab->qual[ddoip_iter].
        tbl_name," cascade constraints purge ^) go"),1)=0)
       SET dm_err->err_ind = 0
       SET dm_err->eproc = concat("Informational:Unable to drop ",ddu_tab->qual[ddoip_iter].tbl_name,
        " table for user: ",ddu_user,".")
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
    ENDFOR
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddu_prep_for_drop(null)
   SET dm_err->eproc = concat("Performing prep work to drop ",ddu_user," user.")
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   IF (dm2_get_dbase_name(ddu_dbname)=0)
    RETURN(0)
   ENDIF
   IF (ddu_dbname="")
    SET dm_err->err_ind = 1
   ...
   ENDIF
   IF (ddu_standalone=1)
    SET dm2_install_schema->dbase_name = ddu_dbname
    SET dm2_install_schema->u_name = "SYS"
    SET dm2_force_connect_string = 1
    EXECUTE dm2_connect_to_dbase "PC"
    SET dm2_force_connect_string = 0
    ...
    ENDIF
   ENDIF
   IF (validate(dm2_drop_user_no_confirmation,0) != 1)
    SET dm_err->eproc = "Issue confirmation report to user"
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SET message = window
    CALL clear(1,1)
    CALL box(1,1,16,132)
    CALL text(2,2,concat("Drop ",ddu_user," User Process"))
    CALL text(4,2,concat("Database Name : ",ddu_dbname))
    CALL text(6,2,concat("Continuing this process will permanently drop all ",ddu_user,
      " user objects and data!"))
    CALL text(8,2,"Enter 'C' to continue or 'Q' to quit (C or Q):")
    CALL accept(8,49,"p;cu",""
     WHERE curaccept IN ("C", "Q"))
    IF (curaccept="Q")
     SET message = nowindow
     SET dm_err->eproc = "Displaying confirmation report."
     SET dm_err->emsg = "User chose to quit from confirmation report."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ELSE
     CALL text(10,2,"Are you sure you wish to continue? (Y or N):")
     CALL accept(10,47,"p;cu",""
      WHERE curaccept IN ("Y", "N"))
     IF (curaccept="N")
      SET message = nowindow
      SET dm_err->eproc = "Displaying second confirmation report."
      SET dm_err->emsg = "User chose to quit from second confirmation report."
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
    ...
    SET message = nowindow
   ENDIF
   SET dm_err->eproc = concat("Continuing with process. User chose to drop user ",ddu_user)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_get_rdbms_version(null)=0)
    RETURN(0)
   ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE ddu_check_for_connections(dcfc_user,dcfc_connect_fnd_ind)
   SET dm_err->eproc = concat("Check if ",dcfc_user," user is currently connected.")
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM gv$session s
    WHERE s.username=dcfc_user
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_program
 IF (ddu_standalone=1)
  ...
  CALL disp_msg("DATABASE CONNECTION REMOVED. RE-ENTER CCL TO ESTABLISH NEW CONNECTION.",dm_err->
   logfile,1)
 ENDIF
 SET dm_err->eproc = "dm2_drop_user completed"
 CALL final_disp_msg("dm2_drop_user")
END GO
