CREATE PROGRAM acm_chg_person_org_reltn:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->person_org_reltn_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   person_org_reltn p
  SET p.person_org_reltn_id = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].
   person_org_reltn_id, p.beg_effective_dt_tm = cnvtdatetime(acm_request->person_org_reltn_qual[xref
    ->chg[d.seq].idx].beg_effective_dt_tm), p.contributor_system_cd =
   IF ((acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.empl_contact = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_contact, p
   .empl_contact_title = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_contact_title,
   p.empl_hire_dt_tm =
   IF ((acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_hire_dt_tm > 0)) cnvtdatetime(
     acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_hire_dt_tm)
   ELSE null
   ENDIF
   ,
   p.empl_occupation_cd = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_occupation_cd,
   p.empl_occupation_text = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].
   empl_occupation_text, p.empl_position = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].
   empl_position,
   p.empl_retire_dt_tm =
   IF ((acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_retire_dt_tm > 0)) cnvtdatetime
    (acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_retire_dt_tm)
   ELSE null
   ENDIF
   , p.empl_status_cd = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_status_cd, p
   .empl_term_dt_tm =
   IF ((acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_term_dt_tm > 0)) cnvtdatetime(
     acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_term_dt_tm)
   ELSE null
   ENDIF
   ,
   p.empl_title = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_title, p.empl_type_cd
    = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].empl_type_cd, p.end_effective_dt_tm =
   cnvtdatetime(acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].end_effective_dt_tm),
   p.free_text_ind = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].free_text_ind, p
   .ft_org_name = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].ft_org_name, p
   .internal_seq = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].internal_seq,
   p.organization_id = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].organization_id, p
   .person_id =
   IF ((acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].person_id > 0)) acm_request->
    person_org_reltn_qual[xref->chg[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].person_idx].
    person_id
   ENDIF
   , p.person_org_alias = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].person_org_alias,
   p.person_org_nbr = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].person_org_nbr, p
   .person_org_reltn_cd = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].
   person_org_reltn_cd, p.priority_seq = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].
   priority_seq,
   p.source_identifier = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].source_identifier,
   p.active_ind = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].active_ind, p
   .active_status_cd = acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].active_status_cd,
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.person_org_reltn_id=acm_request->person_org_reltn_qual[xref->chg[d.seq].idx].
   person_org_reltn_id))
  WITH nocounter, status(reply->person_org_reltn_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->person_org_reltn_qual[xref->chg[index].idx].status != 1))
    SET failed = update_error
    SET table_name = "PERSON_ORG_RELTN"
    GO TO exit_script
   ENDIF
 ENDFOR
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,p.person_org_reltn_id,acm_request->person_org_reltn_qual[xref->chg[t1
     ].idx].person_org_reltn_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.person_org_reltn_id,acm_request->person_org_reltn_qual[xref->
      chg[t1].idx].person_org_reltn_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->person_org_reltn_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->person_org_reltn_qual[idx].status = - (1), reply->person_org_reltn_qual[idx].
      person_org_reltn_id = acm_request->person_org_reltn_qual[idx].person_org_reltn_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->person_org_reltn_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].contributor_system_cd = p.contributor_system_cd
     ENDIF
     IF (findstring("EMPL_CONTACT,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_contact = p.empl_contact
     ENDIF
     IF (findstring("EMPL_CONTACT_TITLE,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_contact_title = p.empl_contact_title
     ENDIF
     IF (findstring("EMPL_HIRE_DT_TM,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_hire_dt_tm = p.empl_hire_dt_tm
     ENDIF
     IF (findstring("EMPL_OCCUPATION_CD,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_occupation_cd = p.empl_occupation_cd
     ENDIF
     IF (findstring("EMPL_OCCUPATION_TEXT,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_occupation_text = p.empl_occupation_text
     ENDIF
     IF (findstring("EMPL_POSITION,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_position = p.empl_position
     ENDIF
     IF (findstring("EMPL_RETIRE_DT_TM,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_retire_dt_tm = p.empl_retire_dt_tm
     ENDIF
     IF (findstring("EMPL_STATUS_CD,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_status_cd = p.empl_status_cd
     ENDIF
     IF (findstring("EMPL_TERM_DT_TM,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_term_dt_tm = p.empl_term_dt_tm
     ENDIF
     IF (findstring("EMPL_TITLE,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_title = p.empl_title
     ENDIF
     IF (findstring("EMPL_TYPE_CD,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].empl_type_cd = p.empl_type_cd
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("FREE_TEXT_IND,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].free_text_ind = p.free_text_ind
     ENDIF
     IF (findstring("FT_ORG_NAME,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].ft_org_name = p.ft_org_name
     ENDIF
     IF (findstring("INTERNAL_SEQ,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].internal_seq = p.internal_seq
     ENDIF
     IF (findstring("ORGANIZATION_ID,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].organization_id = p.organization_id
     ENDIF
     IF (findstring("PERSON_ID,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].person_id = p.person_id
     ENDIF
     IF (findstring("PERSON_ORG_ALIAS,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].person_org_alias = p.person_org_alias
     ENDIF
     IF (findstring("PERSON_ORG_NBR,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].person_org_nbr = p.person_org_nbr
     ENDIF
     IF (findstring("PERSON_ORG_RELTN_CD,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].person_org_reltn_cd = p.person_org_reltn_cd
     ENDIF
     IF (findstring("PRIORITY_SEQ,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].priority_seq = p.priority_seq
     ENDIF
     IF (findstring("SOURCE_IDENTIFIER,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].source_identifier = p.source_identifier
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->person_org_reltn_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->person_org_reltn_qual[xref->chg[index].idx].status=0))
