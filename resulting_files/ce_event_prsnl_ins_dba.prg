CREATE PROGRAM ce_event_prsnl_ins:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 INSERT  FROM ce_event_prsnl t,
  ...
  SET t.ce_event_prsnl_id = evaluate2(
   ...
   evaluate2(
    IF ((validate(request->lst[d.seq].action_organization_id,- (1))=- (1))) 0
    ELSE validate(request->lst[d.seq].action_organization_id,0)
    ENDIF
    ), t.action_organization_ft = evaluate2(
    IF (validate(request->lst[d.seq].action_organization_ft," ")=" ") null
    ELSE validate(request->lst[d.seq].action_organization_ft," ")
    ENDIF
    )
  PLAN (d)
   JOIN (t)
