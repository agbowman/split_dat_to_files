CREATE PROGRAM ags_exe_claimdet_load:dba
 CALL echo("***")
 CALL echo("***   BEG AGS_EXE_CLAIMDET_LOAD")
 CALL echo("***")
 ...
 DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_exe_claimdet_load_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_EXE_CLAIMDET_LOAD"
 CALL echo("***")
...
#skip_parse
 ...
 SELECT INTO "nl:"
  FROM ags_task t,
   ags_job j
  PLAN (t
   WHERE t.task_type="CLAIM DETAIL"
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("GET CLAIM DETAIL DATA :: Select Error :: ",trim(serrmsg
    ))
  SET serrmsg = log->qual[log->qual_knt].smsg
  GO TO exit_script
 ENDIF
 ...
 FOR (exe_idx = 1 TO work->qual_knt)
   ...
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   EXECUTE ags_claimdet_load value(current_task_id)
 ENDFOR
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_EXE_CLAIMDET_LOAD"
 CALL echo("***")
 CALL echo("***   END LOGGING")
 CALL echo("***")
 CALL handle_logging(sstatus_file_name,sstatus_email,ilog_status)
 CALL echo("***")
 CALL echo("***   END AGS_EXE_CLAIMDET_LOAD")
 CALL echo("***")
