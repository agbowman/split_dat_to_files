CREATE PROGRAM dm2_export_dbstats_tab:dba
 ...
 SUBROUTINE ddf_context_lock_maint(dclm_mode,dclm_process,dclm_process_code,dclm_owner,
  dclm_table_name,dclm_object_id,dclm_lock_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dedt_inowner = vc WITH protect, noconstant(" ")
 DECLARE dedt_intable = vc WITH protect, noconstant(" ")
 DECLARE dedt_in_to = vc WITH protect, noconstant(" ")
 DECLARE dedt_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dedt_step_beg_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dedt_step_end_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dedt_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dedt_emsg = vc WITH protect, noconstant(" ")
 DECLARE export_table_stats(ownname=vc,tabname=vc,stattab=vc,statid=vc,statown=vc,
  cascade=i4) = null WITH sql = "SYS.DBMS_STATS.EXPORT_TABLE_STATS", parameter
 ...
 IF (check_logfile("dm2_edbs_tab",".log","dm2_export_dbstats_tab LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_export_dbstats_tab"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dedt_inowner = trim(cnvtupper( $1),3)
 SET dedt_intable = trim( $2,3)
 SET dedt_in_to = trim(cnvtupper( $3),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = concat("Parameter usage: dm2_export_dbstats_tab  '<table_owner>'",
   "'<table_name>','<FROM>:[SYS_PENDING|LIVE]','<TO>:[STATID]''")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_add_detail_text("OWNER",dedt_inowner)
 CALL dm2_process_log_add_detail_text("TABLE",dedt_intable)
 CALL dm2_process_log_add_detail_text("TABLE_OWNER.TABLE_NAME",concat(dedt_inowner,".",dedt_intable))
 CALL dm2_process_log_add_detail_text("TO",dedt_in_to)
 ...
 SET dedt_log_id = dm2_process_event_rs->dm_process_event_id
 IF (ddf_autosuccess_init(null)=0)
  GO TO exit_script
 ELSEIF ((dgdt_prefs->autosuccess_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validate LIVE Statistics existence"
 SELECT INTO "nl:"
  FROM dba_tab_statistics d
  WHERE d.owner=dedt_inowner
   AND d.table_name=dedt_intable
 ...
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ELSEIF (curqual=0)
  SET dm_err->eproc = build("AutoSuccessing: No statistics found in DBA_TABLES for (",dedt_inowner,
   ".",dedt_intable,").")
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = build("Deleting Table Stats from DM_STAT_TABLE for (",dedt_inowner,".",
  dedt_intable,"), STATID (",
  dedt_in_to,")")
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 SET dedt_step_beg_dttm = cnvtdatetime(curdate,curtime3)
 CALL delete_table_stats(dedt_inowner,concat('"',dedt_intable,'"'),"DM_STAT_TABLE",dedt_in_to,"V500",
  cnvtbool(false))
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dedt_step_end_dttm = cnvtdatetime(curdate,curtime3)
 CALL dm2_process_log_add_detail_number("DELETE_TABLE_STATS:SECS",datetimediff(dedt_step_end_dttm,
   dedt_step_beg_dttm,5))
 IF (dm2_process_log_dtl_row(dedt_log_id,0)=0)
  GO TO exit_script
 ENDIF
 SET dedt_step_beg_dttm = cnvtdatetime(curdate,curtime3)
 SET dm_err->eproc = build("Exporting LIVE Stats to DM_STAT_TABLE for (",dedt_inowner,".",
  dedt_intable,") using STATID (",
  dedt_in_to,")")
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 CALL export_table_stats(dedt_inowner,concat('"',dedt_intable,'"'),"DM_STAT_TABLE",dedt_in_to,"V500",
  cnvtbool(false))
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dedt_step_end_dttm = cnvtdatetime(curdate,curtime3)
 CALL dm2_process_log_add_detail_number("EXPORT_TABLE_STATS:SECS",datetimediff(dedt_step_end_dttm,
   dedt_step_beg_dttm,5))
 IF (dm2_process_log_dtl_row(dedt_log_id,0)=0)
  GO TO exit_script
 ENDIF
#exit_script
 ...
 SET dedt_err_ind = dm_err->err_ind
 SET dedt_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_execution,dedt_log_id,1)
 SET dm_err->err_ind = dedt_err_ind
 SET dm_err->emsg = dedt_emsg
 SET dm_err->eproc = "Ending dm2_export_dbstats_tab"
 CALL final_disp_msg("dm2_edbs_tab")
END GO
