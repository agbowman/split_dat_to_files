CREATE PROGRAM bhs_ma_genview_nurse_com_v3:dba
 IF ( NOT (validate(request,0)))
  ...
  SET request->visit[1].encntr_id = 55380945
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (c
   ...
   nursecommunicationsocialservices_cd, nursecommunicationhcm_cd, agencycontactperson_cd,
   portableunitrequired_cd, patientgoinghomeonoxygen_cd, confirmedtransferstarttimedate_cd,
   pulmonarynurseappointment_cd, ambulanceservices_cd,
   phaseiisiteofcare_cd, cardiacorientationappointment_cd, onadmitearlyinterventionprograms_cd,
   onadmitvnahospicehomecare_cd, onadmitadultdayhealthcare_cd,
   onadmitresthomescommunityresidencesshelters_cd, onadmitnursinghomesskilledrehabfacilities_cd,
   onadmitchronichospitals_cd, onadmitmedicalequipcompanies_cd,
   dischargenursinghomesrehabfacilities_cd,
   ...
   dischargechronichospital_cd, needsservicesondischarge_cd, posthospitalservicesrequired_cd)
    AND c.event_end_dt_tm <= sysdate
    AND c.valid_until_dt_tm > sysdate
    AND ((c.encntr_id+ 0)=request->visit[1].encntr_id)
    AND c.view_level=1
    AND c.publish_flag=1
    AND ((c.result_status_cd+ 0) != inerror_cd)
    AND c.event_tag > " ")
   JOIN (cdr
  ...
  DETAIL
   ...
   ELSEIF (c.event_cd=confirmedtransferstarttimedate_cd)
    dlrec->seq[cnt].category = 3, dlrec->seq[cnt].result_sort = 20
   ELSEIF (c.event_cd=ambulanceservices_cd)
   ...
   ELSEIF (c.event_cd=nursecommunicationhcm_cd)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   ...
   o_pending_rev_cd))
    ...
    AND ((o.activity_type_cd+ 0) IN (admittransferdischarge_cd, communicationorders_cd, callmd_cd,
