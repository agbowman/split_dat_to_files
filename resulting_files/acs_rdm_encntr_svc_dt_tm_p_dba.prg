CREATE PROGRAM acs_rdm_encntr_svc_dt_tm_p:dba
 ...
 SET ms_info_domain_nm = "acs_rdm_encntr_svc_dt_tm_domain_nm"
 SET ms_range_prefix = "acs_rdm_encntr_svc_dt_tm_range_prefix"
 SET mn_num_children = 10
 IF (sbr_check_for_prev_rdm(11837,2,ms_info_domain_nm,ms_range_prefix) IN (2, 1))
  GO TO exit_program
 ENDIF
 IF (sbr_check_for_prev_rdm(11837,1,ms_info_domain_nm,ms_range_prefix) IN (2, 1))
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
 ...
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error selecting the min/max ID: ",ms_errmsg)
  GO TO exit_program
 ENDIF
