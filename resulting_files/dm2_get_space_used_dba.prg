CREATE PROGRAM dm2_get_space_used:dba
#start
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dm2_get_tbl_stats(null) = i4
 DECLARE dm2_get_ind_stats(null) = i4
 IF (validate(dm2_space_data->obj_cnt,- (1)) < 0)
  RECORD dm2_space_data(
    1 block_size = f8
    1 obj_cnt = i4
    1 obj[*]
      2 segment_name = vc
      ...
      2 table_name = vc
      2 tablespace = vc
      2 total_space_blocks = f8
      2 free_space_blocks = f8
      2 row_count = f8
      2 last_analyzed_dt_tm = dq8
  )
  SET dm2_space_data->block_size = 0.0
  SET dm2_space_data->obj_cnt = 0
 ENDIF
 DECLARE sd_rec_cnt = i4 WITH protect, noconstant(0)
 SET dm2_space_data->block_size = 8192.0
 SUBROUTINE dm2_get_tbl_stats(null)
   SELECT
    IF ((dm2_install_schema->process_option="INHOUSE")
     AND (inhouse_reply->table_name != "NONE"))
     FROM dm_tables_doc d,
      dm_user_tables_actual_stats t
     WHERE (d.table_name=inhouse_reply->table_name)
      AND d.table_name=t.table_name
    ELSEIF ((dm2_install_schema->dbase_type="ADMIN"))
     FROM dm_user_tables_actual_stats t
    ELSE
     FROM dm_tables_doc d,
      dm_user_tables_actual_stats t
    ...
    INTO "nl:"
    HEAD REPORT
     sd_rec_cnt = dm2_space_data->obj_cnt
     IF ((dm2_space_data->obj_cnt > 0))
      stat = alterlist(dm2_space_data->obj,(dm2_space_data->obj_cnt+ (50 - mod(dm2_space_data->
        obj_cnt,50))))
     ENDIF
    DETAIL
     sd_rec_cnt = (sd_rec_cnt+ 1)
     IF (mod(sd_rec_cnt,50)=1)
      stat = alterlist(dm2_space_data->obj,(sd_rec_cnt+ 49))
     ENDIF
     dm2_space_data->obj[sd_rec_cnt].segment_name = t.table_name, dm2_space_data->obj[sd_rec_cnt].
     segment_type = "TABLE", dm2_space_data->obj[sd_rec_cnt].table_name = t.table_name,
     dm2_space_data->obj[sd_rec_cnt].tablespace = t.tablespace_name, dm2_space_data->obj[sd_rec_cnt].
     total_space_blocks = t.blocks, dm2_space_data->obj[sd_rec_cnt].free_space_blocks = t
     .empty_blocks,
     dm2_space_data->obj[sd_rec_cnt].row_count = t.num_rows
     IF (t.last_analyzed=null)
      dm2_space_data->obj[sd_rec_cnt].last_analyzed_dt_tm = cnvtdatetime("01-JAN-1900")
     ELSE
      dm2_space_data->obj[sd_rec_cnt].last_analyzed_dt_tm = t.last_analyzed
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET stat = alterlist(dm2_space_data->obj,sd_rec_cnt)
   IF (curqual > 0)
    SET dm2_space_data->obj_cnt = sd_rec_cnt
    RETURN(1)
   ELSE
    RETURN(1)
   ENDIF
 END ;Subroutine
 SUBROUTINE dm2_get_ind_stats(null)
   SELECT
    IF ((dm2_install_schema->process_option="INHOUSE")
     AND (inhouse_reply->table_name != "NONE"))
     FROM dm_user_indexes_actual_stats i,
      dm_tables_doc d,
      dm_user_tables_actual_stats t
     WHERE (t.table_name=inhouse_reply->table_name)
      AND t.table_name=i.table_name
      AND i.table_name=d.table_name
      AND i.table_owner=cnvtupper(trim(currdbuser,3))
    ELSEIF ((dm2_install_schema->dbase_type="ADMIN"))
     FROM dm_user_indexes_actual_stats i,
      dm_user_tables_actual_stats t
    ...
    DETAIL
     ...
     IF (mod(sd_rec_cnt,50)=1)
      stat = alterlist(dm2_space_data->obj,(sd_rec_cnt+ 49))
     ENDIF
     dm2_space_data->obj[sd_rec_cnt].segment_name = i.index_name, dm2_space_data->obj[sd_rec_cnt].
     segment_type = "INDEX", dm2_space_data->obj[sd_rec_cnt].table_name = i.table_name,
     dm2_space_data->obj[sd_rec_cnt].tablespace = i.tablespace_name, dm2_space_data->obj[sd_rec_cnt].
     total_space_blocks = i.leaf_blocks, dm2_space_data->obj[sd_rec_cnt].free_space_blocks = 0.0,
     dm2_space_data->obj[sd_rec_cnt].row_count = t.num_rows
     IF (i.last_analyzed=null)
      dm2_space_data->obj[sd_rec_cnt].last_analyzed_dt_tm = cnvtdatetime("01-JAN-1900")
     ELSE
      dm2_space_data->obj[sd_rec_cnt].last_analyzed_dt_tm = i.last_analyzed
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 DECLARE calling_mode = vc WITH protect
 DECLARE gsu_cur_tbl_loc = i2 WITH private
 DECLARE gsu_tgt_tbl_loc = i2 WITH private
 DECLARE gsu_cur_dist_idx = i2 WITH private
 DECLARE gsu_tgt_dist_idx = i2 WITH private
 DECLARE size_with_tables = i4 WITH private
 DECLARE gsu_prev_tname = vc WITH public, noconstant("X")
 IF (check_logfile("dm2_get_space_used",".log","DM2_GET_SPACE_USED LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "BEGIN dm2_get_space_used"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET calling_mode = cnvtupper( $1)
 IF ( NOT (calling_mode IN ("C", "T", "B")))
  SET dm_err->eproc = "Invalid Input Parameter"
  CALL disp_msg(" ",dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (((calling_mode="C") OR (calling_mode="B")) )
  IF (value(size(cur_sch->tbl,5))=0)
   SET dm_err->err_ind = 1
   GO TO exit_program
  ENDIF
 ELSEIF (((calling_mode="T") OR (calling_mode="B")) )
  IF (value(size(tgtsch->tbl,5))=0)
   SET dm_err->err_ind = 1
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dm2_get_tbl_stats(null)=0)
  SET dm_err->eproc = "Error in dm2_get_tbl_stats"
  CALL disp_msg(" ",dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET size_with_tables = dm2_space_data->obj_cnt
 IF (dm2_get_ind_stats(null)=0)
  SET dm_err->eproc = "Error in dm2_get_ind_stats"
  CALL disp_msg(" ",dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (calling_mode="C")
  SET dm_err->eproc = "Populating Cur_Sch record structure with statistics"
 ELSEIF (calling_mode="T")
  SET dm_err->eproc = "Populating TgtSch record structure with statistics"
 ELSEIF (calling_mode="B")
  SET dm_err->eproc =
  "Populating TgtSch record structure and Cur_Sch record structure with statistics"
 ENDIF
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dm2_space_data->obj_cnt > 0))
  SELECT INTO "nl:"
   d.seq
   FROM (dummyt d  WITH seq = dm2_space_data->obj_cnt)
   PLAN (d)
   ORDER BY dm2_space_data->obj[d.seq].table_name
   DETAIL
    IF ((gsu_prev_tname != dm2_space_data->obj[d.seq].table_name))
     gsu_tgt_tbl_loc = 0, gsu_cur_tbl_loc = 0
     IF (((calling_mode="C") OR (calling_mode="B")) )
      gsu_cur_tbl_loc = locateval(gsu_cur_tbl_loc,1,cur_sch->tbl_cnt,dm2_space_data->obj[d.seq].
       table_name,cur_sch->tbl[gsu_cur_tbl_loc].tbl_name)
     ENDIF
     IF (((calling_mode="T") OR (calling_mode="B")) )
      gsu_tgt_tbl_loc = locateval(gsu_tgt_tbl_loc,1,tgtsch->tbl_cnt,dm2_space_data->obj[d.seq].
       table_name,tgtsch->tbl[gsu_tgt_tbl_loc].tbl_name)
     ENDIF
     gsu_prev_tname = dm2_space_data->obj[d.seq].table_name
    ENDIF
    IF ((dm2_space_data->obj[d.seq].segment_type="TABLE"))
     IF (((calling_mode="C") OR (calling_mode="B"))
      AND gsu_cur_tbl_loc > 0)
      cur_sch->tbl[gsu_cur_tbl_loc].bytes_allocated = ((dm2_space_data->obj[d.seq].free_space_blocks
      + dm2_space_data->obj[d.seq].total_space_blocks) * dm2_space_data->block_size), cur_sch->tbl[
      gsu_cur_tbl_loc].bytes_used = (dm2_space_data->obj[d.seq].total_space_blocks * dm2_space_data->
      block_size)
     ENDIF
     IF (((calling_mode="T") OR (calling_mode="B"))
      AND gsu_tgt_tbl_loc > 0)
      tgtsch->tbl[gsu_tgt_tbl_loc].cur_bytes_allocated = ((dm2_space_data->obj[d.seq].
      free_space_blocks+ dm2_space_data->obj[d.seq].total_space_blocks) * dm2_space_data->block_size),
      tgtsch->tbl[gsu_tgt_tbl_loc].cur_bytes_used = (dm2_space_data->obj[d.seq].total_space_blocks *
      dm2_space_data->block_size), tgtsch->tbl[gsu_tgt_tbl_loc].row_cnt = dm2_space_data->obj[d.seq].
      row_count,
      tgtsch->tbl[gsu_tgt_tbl_loc].last_analyzed_dt_tm = dm2_space_data->obj[d.seq].
      last_analyzed_dt_tm
     ENDIF
    ELSEIF ((dm2_space_data->obj[d.seq].segment_type="INDEX"))
     IF (((calling_mode="C") OR (calling_mode="B"))
      AND gsu_cur_tbl_loc > 0)
      gsu_cur_dist_idx = 0, gsu_cur_dist_idx = locateval(gsu_cur_dist_idx,1,cur_sch->tbl[
       gsu_cur_tbl_loc].ind_cnt,dm2_space_data->obj[d.seq].segment_name,cur_sch->tbl[gsu_cur_tbl_loc]
       .ind[gsu_cur_dist_idx].ind_name)
      IF (gsu_cur_dist_idx > 0)
       cur_sch->tbl[gsu_cur_tbl_loc].ind[gsu_cur_dist_idx].bytes_allocated = ((dm2_space_data->obj[d
       .seq].free_space_blocks+ dm2_space_data->obj[d.seq].total_space_blocks) * dm2_space_data->
       block_size), cur_sch->tbl[gsu_cur_tbl_loc].ind[gsu_cur_dist_idx].bytes_used = (dm2_space_data
       ->obj[d.seq].total_space_blocks * dm2_space_data->block_size)
      ENDIF
     ENDIF
     IF (((calling_mode="T") OR (calling_mode="B"))
      AND gsu_tgt_tbl_loc > 0)
      gsu_tgt_dist_idx = 0, gsu_tgt_dist_idx = locateval(gsu_tgt_dist_idx,1,tgtsch->tbl[
       gsu_tgt_tbl_loc].ind_cnt,dm2_space_data->obj[d.seq].segment_name,tgtsch->tbl[gsu_tgt_tbl_loc].
       ind[gsu_tgt_dist_idx].ind_name)
      IF (gsu_tgt_dist_idx > 0)
       tgtsch->tbl[gsu_tgt_tbl_loc].ind[gsu_tgt_dist_idx].cur_bytes_allocated = ((dm2_space_data->
       obj[d.seq].free_space_blocks+ dm2_space_data->obj[d.seq].total_space_blocks) * dm2_space_data
       ->block_size), tgtsch->tbl[gsu_tgt_tbl_loc].ind[gsu_tgt_dist_idx].cur_bytes_used = (
       dm2_space_data->obj[d.seq].total_space_blocks * dm2_space_data->block_size)
      ENDIF
     ENDIF
    ENDIF
   WITH nocounter
  ;end select
  SET dm_err->eproc = "updating table and index stats"
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Table and Index stats updated"
  CALL disp_msg(" ",dm_err->logfile,0)
 ELSE
  SET dm_err->eproc = "Table and Index stats not updated - no stats available."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
#exit_program
 IF ((dm_err->err_ind=1))
  SET dm_err->eproc = "EXITING DM2_GET_SPACE_USED WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_GET_SPACE_USED COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("dm2_get_space_used")
END GO
