CREATE PROGRAM dcp_get_extrnl_blocks:dba
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 DECLARE has_dismissed(null) = i4
 DECLARE iexternal_block = i2 WITH protect, noconstant(0)
 ...
 persist
 DECLARE uar_srvsetshort(p1=i4(value),p2=vc(ref),p3=i2(value)) = i1 WITH image_axp = "srvrtl",
 image_aix = "libsrv.a(libsrv.o)", uar = "SrvSetShort",
 persist
 DECLARE iperson_id = f8 WITH protect, constant(link_personid)
 DECLARE request_number = i4 WITH protect, constant(3202572)
 DECLARE hreq = i4 WITH protect, noconstant(0)
 DECLARE hrep = i4 WITH protect, noconstant(0)
 DECLARE nsrvstat = i2 WITH protect, noconstant(0)
 DECLARE hroleprofilehandle = i4 WITH protect, noconstant(0)
 DECLARE husercurrentorgs = i4 WITH protect, noconstant(0)
 DECLARE hsetroleprofile = i4 WITH protect, noconstant(0)
 DECLARE fusercao = f8 WITH protect, noconstant(0)
 DECLARE patients = i4 WITH protect, noconstant(0)
 DECLARE hconsent_type = i4 WITH protect, noconstant(0)
 DECLARE hactive_status = i4 WITH protect, noconstant(0)
 DECLARE icvstat = i4 WITH protect, noconstant(0)
 DECLARE iorg_org_reltn_code_set = i4 WITH protect, constant(369)
 DECLARE sorg_org_reltn_cdf_meaning = vc WITH protect, constant("CRGVRTOCRUNT")
 DECLARE forg_org_reltn_code_value = f8 WITH protect, noconstant(0)
 DECLARE iorg_type_code_set = i4 WITH protect, constant(278)
 DECLARE sorg_type_cdf_meaning = vc WITH protect, constant("CAREGIVER")
 DECLARE forg_type_code_value = f8 WITH protect, noconstant(0)
 DECLARE fuser_caregiver = f8 WITH protect, noconstant(0)
 DECLARE iblockedcacregivercnt = i4 WITH protect, noconstant(0)
 DECLARE start_timer = dq8 WITH noconstant(0)
 DECLARE is_caregiver = i2 WITH protect, noconstant(0)
 DECLARE alert_txt = vc WITH protect, constant(
  "Patient has External Blocks - Some data is not accessible")
 FREE RECORD opt_out_consents
 RECORD opt_out_consents(
   1 blocked_careunits[*]
     2 org_id = f8
 )
 FREE RECORD related_org_reply
 RECORD related_org_reply(
   1 qual_cnt = i4
   1 results_qual[*]
     2 related_orgs[*]
       3 org_id = f8
 ...
 FREE RECORD related_org_request
 RECORD related_org_request(
   1 call_echo_ind = i2
   1 org_qual[*]
     2 org_id = f8
     2 reltn_type_cd = f8
 ...
 FREE RECORD caregivers_by_blocks
 RECORD caregivers_by_blocks(
   1 caregiver[*]
     2 org_id = f8
 )
 SET log_misc1 = "DO NOT ADD ALERT"
 SET log_program_name = "dcp_get_extrnl_blocks"
 CALL log_message(concat("Begin script: ",log_program_name),log_level_debug)
 SET start_timer = cnvtdatetime(sysdate)
 IF (iperson_id <= 0)
  SET retval = 0
  SET log_message = "Error retrieving patient data from trigger."
  GO TO externalblocknotfound
 ENDIF
 IF (has_dismissed(null) > 0)
  CALL echo("User has already dismissed the alert")
  SET retval = 100
  SET log_message = "User has already dismissed the alert"
  GO TO externalblocknotfound
 ENDIF
 SET icvstat = uar_get_meaning_by_codeset(iorg_type_code_set,sorg_type_cdf_meaning,1,
  forg_type_code_value)
 IF (icvstat != 0)
  CALL echo("Failed to retrieve Caregiver code value")
  SET log_message = "Failed to retrieve Caregiver code value"
  SET retval = 0
  GO TO externalblocknotfound
 ENDIF
 SET icvstat = uar_get_meaning_by_codeset(iorg_org_reltn_code_set,sorg_org_reltn_cdf_meaning,1,
  forg_org_reltn_code_value)
 IF (icvstat != 0)
  CALL echo("Failed to retrieve ORG_ORG_RELTN code value")
  SET log_message = "Failed to retrieve ORG_ORG_RELTN code value"
  SET retval = 0
  GO TO externalblocknotfound
 ENDIF
 SET hmsg = uar_srvselectmessage(request_number)
 SET hreq = uar_srvcreaterequest(hmsg)
 SET hrep = uar_srvcreatereply(hmsg)
 SET patients = uar_srvadditem(hreq,"patients")
 SET nsrvstat = uar_srvsetdouble(patients,"patient_id",iperson_id)
 SET hconsent_type = uar_srvgetstruct(hreq,"consent_type")
 SET nsrvstat = uar_srvsetshort(hconsent_type,"opt_in_ind",0)
 SET nsrvstat = uar_srvsetshort(hconsent_type,"opt_out_ind",1)
 SET hactive_status = uar_srvgetstruct(hreq,"active_status")
 SET nsrvstat = uar_srvsetshort(hactive_status,"active_ind",1)
 SET nsrvstat = uar_srvsetshort(hactive_status,"active_and_inactive_ind",0)
 SET iret = uar_srvexecute(hmsg,hreq,hrep)
 DECLARE iconsent_cnt = i4
 DECLARE httransaction_status = i4
 DECLARE istatus_ind = i2
 DECLARE hconsents = i4
 DECLARE hretrievedpatientconsents = i4
 DECLARE hretrieved_consents_by_patient = i4
 DECLARE forg_id = f8
 SET htransaction_status = uar_srvgetstruct(hrep,nullterm("transaction_status"))
 SET istatus_ind = uar_srvgetshort(htransaction_status,nullterm("success_ind"))
 IF (istatus_ind=1)
  DECLARE iconsnetsbypatientcnt = i4
  SET iconsnetsbypatientcnt = uar_srvgetitemcount(hrep,nullterm("retrieved_consents_by_patient"))
  IF (iconsnetsbypatientcnt=1)
   SET hretrieved_consents_by_patient = uar_srvgetitem(hrep,nullterm("retrieved_consents_by_patient"),
    0)
   SET iconsent_cnt = uar_srvgetitemcount(hretrieved_consents_by_patient,nullterm(
     "retrievedPatientConsents"))
   FOR (i = 1 TO iconsent_cnt)
     SET hretrievedpatientconsents = uar_srvgetitem(hretrieved_consents_by_patient,nullterm(
       "retrievedPatientConsents"),(i - 1))
     SET hconsents = uar_srvgetstruct(hretrievedpatientconsents,nullterm("consent"))
     SET forg_id = uar_srvgetdouble(hconsents,nullterm("organization_id"))
     SET is_caregiver = is_caregiver_type(forg_id)
     IF (is_caregiver=1)
      SET iblockedcacregivercnt = size(caregivers_by_blocks->caregiver,5)
      SET stat = alterlist(caregivers_by_blocks->caregiver,(iblockedcacregivercnt+ 1))
      SET caregivers_by_blocks->caregiver[(iblockedcacregivercnt+ 1)].org_id = forg_id
     ELSE
      SET iblockedcareunitcnt = size(opt_out_consents->blocked_careunits,5)
      SET stat = alterlist(opt_out_consents->blocked_careunits,(iblockedcareunitcnt+ 1))
      SET opt_out_consents->blocked_careunits[(iblockedcareunitcnt+ 1)].org_id = forg_id
     ENDIF
   ENDFOR
  ELSE
   SET log_message = "Blockes not found"
   SET retval = 100
   GO TO externalblocknotfound
  ENDIF
 ELSE
  SET log_message = build("SRV [Req:",request_number,"] failed to retrieve blocks")
  SET retval = 0
 ...
 ENDIF
 SET iblockedcareunitcnt = size(opt_out_consents->blocked_careunits,5)
 IF (iblockedcareunitcnt > 0)
  SET stat = alterlist(related_org_request->related_org_qual,iblockedcareunitcnt)
  FOR (i = 1 TO iblockedcareunitcnt)
   SET related_org_request->related_org_qual[1].related_org_id = opt_out_consents->blocked_careunits[
   i].org_id
   SET related_org_request->related_org_qual[1].reltn_type_cd = forg_org_reltn_code_value
  ENDFOR
  EXECUTE pm_get_org_by_org  WITH replace(request,related_org_request), replace(reply,
   related_org_reply)
  IF ((related_org_reply->status_data.status="F"))
   CALL echo("pm_get_org_by_org failed to retrieve caregiver by careunit.")
   GO TO check_for_blocks
  ENDIF
  DECLARE icg_cnt = i4
  DECLARE j = i4 WITH protect, noconstant(1)
  DECLARE bunique = i1 WITH protect, noconstant(1)
  SET icg_cnt = size(related_org_reply->results_qual[1].related_orgs,5)
  FOR (i = 1 TO icg_cnt)
    SET iblockedcacregivercnt = size(caregivers_by_blocks->caregiver,5)
    SET j = 1
    SET bunique = 1
    WHILE (j <= iblockedcacregivercnt
     AND bunique=1)
     SET j += 1
     IF ((caregivers_by_blocks->caregiver[j].org_id != related_org_reply->results_qual[i].
     related_orgs[1].org_id))
      SET bunique = 1
     ELSE
      SET bunique = 0
     ENDIF
    ...
    IF (bunique=1)
     SET stat = alterlist(caregivers_by_blocks->caregiver,(iblockedcacregivercnt+ 1))
     SET caregivers_by_blocks->caregiver[(iblockedcacregivercnt+ 1)].org_id = related_org_reply->
     results_qual[i].related_orgs[1].org_i
    ENDIF
  ENDFOR
 ENDIF
#check_for_blocks
 SET iblockedcacregivercnt = size(caregivers_by_blocks->caregiver,5)
 IF (iblockedcacregivercnt > 0)
  SET hroleprofilehandle = uar_sacgetuserroleprofiledetails()
  IF (hroleprofilehandle != 0)
   SET fusercao = uar_srvgetdouble(hroleprofilehandle,nullterm("principal_cao"))
   IF (fusercao=0)
    DECLARE fusercapg = f8 WITH protect, noconstant(0)
    SET fusercapg = uar_srvgetdouble(hroleprofilehandle,nullterm("chart_access_personnel_group"))
    SELECT INTO "nl:"
     orgrel.organization_id
     FROM prsnl_group_org_reltn orgrel
     WHERE orgrel.prsnl_group_id=fusercapg
     DETAIL
      fusercao = orgrel.organization_id
     WITH nocounter, separator = " ", format,
      maxrec = 1
    ;end select
   ENDIF
   SET stat = alterlist(related_org_reply->results_qual,0)
   SET stat = alterlist(related_org_request->related_org_qual,1)
   SET related_org_request->related_org_qual[1].related_org_id = fusercao
   SET related_org_request->related_org_qual[1].reltn_type_cd = forg_org_reltn_code_value
   EXECUTE pm_get_org_by_org  WITH replace(request,related_org_request), replace(reply,
    related_org_reply)
   IF ((related_org_reply->status_data.status="S")
    AND (related_org_reply->qual_cnt=1))
    SET fuser_caregiver = related_org_reply->results_qual[1].related_orgs[1].org_id
    CALL echo(build("user's caregiver id == ",fuser_caregiver))
   ELSE
    SET log_message = "User caregiver not found"
    SET retval = 0
    GO TO externalblocknotfound
   ENDIF
  ELSE
   SET retval = 0
   SET log_message = "Role Profile not found in to the security context."
   GO TO externalblocknotfound
  ENDIF
  FOR (i = 1 TO iblockedcacregivercnt)
    IF (iexternal_block=1)
     EXECUTE goto externalblockfound
    ELSEIF ((fuser_caregiver != caregivers_by_blocks->caregiver[i].org_id))
     SET iexternal_block = 1
    ELSE
     CALL echo("blocked Caregiver and user's caregiver are same.")
    ENDIF
  ENDFOR
  SET retval = 100
  SET log_message = "External blocks do not exist"
  GO TO externalblocknotfound
 ELSE
  SET retval = 100
  SET log_message = "Blocks do not exist"
  GO TO externalblocknotfound
 ENDIF
#externalblocknotfound
 CALL echo("External blocks do not exist for the patient")
 CALL echo(build("External block status [",iexternal_block,"]"))
#externalblockfound
 SET retval = 100
 IF (iexternal_block=1)
  SET log_misc1 = "ADD EXTERNAL BLOCKS ALERT"
  SET log_message = "Blocks exist for the patient"
 ENDIF
 CALL log_message(build("Finished executing dcp_get_extrnl_blocks. Elapsed time in seconds: ",
   datetimediff(cnvtdatetime(sysdate),start_timer,6)),log_level_debug)
 SUBROUTINE (is_caregiver_type(org_id=f8) =i4)
   SET is_caregiver = 0
   SELECT INTO "nl:"
    FROM org_type_reltn o
    WHERE o.organization_id=org_id
     AND o.org_type_cd=forg_type_code_value
    WITH nocounter, separator = " ", format,
     maxrec = 1
   ;end select
   IF (curqual=1)
   ...
   RETURN(is_caregiver)
 END ;Subroutine
 SUBROUTINE has_dismissed(null)
   DECLARE foundid = i4 WITH noconstant(0)
   SELECT INTO "nl:"
    FROM passive_alert_action pa,
     passive_alert p
    WHERE pa.active_ind=1
     AND pa.beg_effective_dt_tm <= cnvtdatetime(sysdate)
     AND pa.end_effective_dt_tm >= cnvtdatetime(sysdate)
     AND (pa.prsnl_id=eks_common->personnel_id)
     AND pa.passive_alert_id=p.passive_alert_id
     AND p.person_id=iperson_id
     AND p.alert_txt=alert_txt
    GROUP BY pa.passive_alert_action_id
    HEAD REPORT
     IF (pa.passive_alert_action_id > 0)
      foundid = pa.passive_alert_action_id
     ENDIF
    WITH nocounter
   ;end select
   CALL echo(build("found dismissed alert :",foundid))
   RETURN(foundid)
 END ;Subroutine
END GO
