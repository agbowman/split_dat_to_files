CREATE PROGRAM cac_determine_bill_code_dtls:dba
 RECORD ncodereply(
 ...
 DECLARE step_id = i4 WITH protect, constant(3070026)
 ...
 DECLARE hdiagnosisterminologies = i4 WITH protect, noconstant(0)
 DECLARE hprocedureterminologies = i4 WITH protect, noconstant(0)
 ...
 DECLARE hcomputedcode = i4 WITH protect, noconstant(0)
 DECLARE hcomputedcodeguideline = i4 WITH protect, noconstant(0)
 DECLARE hcomputedcodecode = i4 WITH protect, noconstant(0)
 DECLARE hcomputedcodesecondary = i4 WITH protect, noconstant(0)
 DECLARE hcomputedcodeadvice = i4 WITH protect, noconstant(0)
 DECLARE cliniciancodecnt = i2 WITH protect, noconstant(0)
 DECLARE diagnosisterminologycnt = i2 WITH protect, noconstant(0)
 DECLARE procedureterminologycnt = i2 WITH protect, noconstant(0)
 ...
 ENDIF
 IF (validate(ncoderequest->diagnosis_terminologies))
  FOR (diagnosisterminologycnt = 1 TO value(size(ncoderequest->diagnosis_terminologies,5)))
    SET hdiagnosisterminologies = uar_srvadditem(hrequest,"diagnosis_terminologies")
    SET stat = uar_srvsetshort(hdiagnosisterminologies,"icd9_ind",ncoderequest->
     diagnosis_terminologies[diagnosisterminologycnt].icd9_ind)
    SET stat = uar_srvsetshort(hdiagnosisterminologies,"icd10_ind",ncoderequest->
     diagnosis_terminologies[diagnosisterminologycnt].icd10_ind)
  ENDFOR
 ENDIF
 IF (validate(ncoderequest->procedure_terminologies))
  FOR (procedureterminologycnt = 1 TO value(size(ncoderequest->procedure_terminologies,5)))
   SET hprocedureterminologies = uar_srvadditem(hrequest,"procedure_terminologies")
   SET stat = uar_srvsetshort(hprocedureterminologies,"cpt_ind",ncoderequest->
    procedure_terminologies[procedureterminologycnt].cpt_ind)
  ENDFOR
 ENDIF
 ...
 IF (diagnosessize > 0)
  ...
  FOR (diagnosiscnt = 1 TO diagnosessize)
    ...
    SET ncodereply->diagnoses[diagnosiscnt].code = uar_srvgetstringptr(hdiagnosis,"code")
    SET ncodereply->diagnoses[diagnosiscnt].description = uar_srvgetstringptr(hdiagnosis,
     "description")
    SET ncodereply->diagnoses[diagnosiscnt].document_text = uar_srvgetstringptr(hdiagnosis,
     "document_text")
    IF (validate(ncodereply->diagnoses.terminology_type))
     SET hdiagnosisterminology = uar_srvgetstruct(hdiagnosis,"terminology_type")
     SET ncodereply->diagnoses[diagnosiscnt].terminology_type.icd9_ind = uar_srvgetshort(
      hdiagnosisterminology,"icd9_ind")
     SET ncodereply->diagnoses[diagnosiscnt].terminology_type.icd10_ind = uar_srvgetshort(
      hdiagnosisterminology,"icd10_ind")
    ENDIF
  ENDFOR
 ENDIF
 ...
 IF (proceduressize > 0)
  ...
  FOR (procedurecnt = 1 TO proceduressize)
    ...
    SET ncodereply->procedures[procedurecnt].code = uar_srvgetstringptr(hprocedure,"code")
    SET ncodereply->procedures[procedurecnt].description = uar_srvgetstringptr(hprocedure,
     "description")
    SET ncodereply->procedures[procedurecnt].document_text = uar_srvgetstringptr(hprocedure,
     "document_text")
    IF (validate(ncodereply->procedures.terminology_type))
     SET hprocedureterminology = uar_srvgetstruct(hprocedure,"terminology_type")
     SET ncodereply->procedures[procedurecnt].terminology_type.cpt_ind = uar_srvgetshort(
      hprocedureterminology,"cpt_ind")
    ENDIF
  ENDFOR
 ENDIF
 SET ncodereply->advice_summary = uar_srvgetstringptr(hreply,"advice_summary")
 SET ncodereply->advice = uar_srvgetstringptr(hreply,"advice")
