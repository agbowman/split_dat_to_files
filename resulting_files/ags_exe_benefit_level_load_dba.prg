CREATE PROGRAM ags_exe_benefit_level_load:dba
 CALL echo("***")
 CALL echo("***   BEG AGS_EXE_BENEFIT_LEVEL_LOAD")
 CALL echo("***")
 ...
 DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_exe_benefit_level_load_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_EXE_BENEFIT_LEVEL_LOAD"
 CALL echo("***")
 ...
 WHILE (ipos < bs_length)
   ...
   IF (ipos < 1)
    ...
    SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
    SET log->qual[log->qual_knt].smsg = concat("PARSE WORKING_BATCH_SELECTION :: Parameter Error :: ",
     "Invalid batch_selection string: '<' not present :: ",trim(working_batch_selection))
    SET serrmsg = log->qual[log->qual_knt].smsg
    GO TO exit_script
   ENDIF
...
#skip_parse
 ...
 CALL echo("***")
 CALL echo("***")
 CALL echo("***   BEG Load BENEFIT LEVEL Data")
 CALL echo("***")
 SET log->qual_knt = (log->qual_knt+ 1)
 SET stat = alterlist(log->qual,log->qual_knt)
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "BEG >> Load BENEFIT LEVEL Data"
 FREE RECORD work
 RECORD work(
   1 qual_knt = i4
   1 qual[*]
 ...
 SELECT INTO "nl:"
  FROM ags_task t,
   ags_job j
  PLAN (t
   WHERE t.task_type="BENEFIT LEVEL"
 ...
 IF (ierrcode > 0)
  SET failed = select_error
  SET table_name = "GET BENEFIT LEVEL DATA"
  SET ilog_status = 1
  SET log->qual_knt = (log->qual_knt+ 1)
  SET stat = alterlist(log->qual,log->qual_knt)
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("GET BENEFIT LEVEL DATA :: Select Error :: ",trim(
    serrmsg))
  SET serrmsg = log->qual[log->qual_knt].smsg
  GO TO exit_script
 ENDIF
 ...
 FOR (exe_idx = 1 TO work->qual_knt)
   ...
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   EXECUTE ags_benefit_level_load value(current_task_id)
 ENDFOR
#skip_benefit_level_load
 SET log->qual_knt = (log->qual_knt+ 1)
 SET stat = alterlist(log->qual,log->qual_knt)
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> Load BENEFIT LEVEL Data"
 CALL echo("***")
 CALL echo("***   END Load BENEFIT LEVEL Data")
 CALL echo("***")
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_EXE_BENEFIT_LEVEL_LOAD"
 CALL echo("***")
 CALL echo("***   END LOGGING")
 CALL echo("***")
 CALL handle_logging(sstatus_file_name,sstatus_email,ilog_status)
 CALL echo("***")
 CALL echo("***   END AGS_EXE_BENEFIT_LEVEL_LOAD")
 CALL echo("***")
