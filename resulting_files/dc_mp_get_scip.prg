CREATE PROGRAM dc_mp_get_scip
 IF ( NOT (validate(scipeval)))
  RECORD scipeval(
    1 listcnt = i4
    1 colmncnt = i4
    1 list[*]
 ...
 SUBROUTINE iqrversioncontrolstring(category_topic_mean,beg_time)
   ...
   RETURN(version_number)
 END ;Subroutine
 DECLARE ver_num = f8 WITH protect, constant(iqrversioncontrol("SSI",cnvtdatetime(curdate,curtime3)))
 IF (( $2 != null))
  SET jrec = cnvtjsontorec( $2)
  IF (validate(debug_dc_mp_get_scip_ind,0)=1)
...
#exit_script
 IF (validate(_memory_reply_string)=1)
  SET _memory_reply_string = cnvtrectojson(scipreply)
 ELSE
  CALL echojson(scipreply, $1)
  SELECT INTO "nl:"
   DETAIL
    row + 0
   WITH nocounter
