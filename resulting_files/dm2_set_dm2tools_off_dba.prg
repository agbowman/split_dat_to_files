CREATE PROGRAM dm2_set_dm2tools_off:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_set_dm2tools_off",".log","dm2_set_dm2tools_off")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Checking existence of TOOLSET_USAGE option in DM_INFO."
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inserting into DM_INFO, turning off usage of DM2TOOLS."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  INSERT  FROM dm_info
   SET info_domain = concat("DM2_",trim(currdb),"_DB_OPTION"), info_name = "DM2_TOOLSET_USAGE",
    info_char = "N",
    info_date = cnvtdatetime("22-JUN-1996 00:00:00"), info_number = 444.0, updt_dt_tm = cnvtdatetime(
     curdate,curtime3),
  ...
  ;end insert
 ELSE
  SET dm_err->eproc = "Update DM_INFO, turning off usage of DM2TOOLS."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  UPDATE  FROM dm_info
   SET info_char = "N", info_date = cnvtdatetime("22-JUN-1996 00:00:00"), info_number = 444.0,
    updt_dt_tm = cnvtdatetime(curdate,curtime3)
   WHERE info_domain=concat("DM2_",trim(currdb),"_DB_OPTION")
    AND info_name="DM2_TOOLSET_USAGE"
   WITH nocounter
  ;end update
 ENDIF
 IF (check_error(dm_err->eproc)=1)
