CREATE PROGRAM bed_aud_fn_day_week:dba
 ...
 SET row_nbr = 0
 SELECT INTO "nl:"
  desc_key_cap = cnvtupper(cv.description)
  FROM code_value cv,
   person p
  PLAN (cv
   WHERE cv.code_set=27980
  ...
  ORDER BY desc_key_cap, cv.code_value, cnvtdatetime(cv.updt_dt_tm)
  HEAD REPORT
   xcnt = 0
  DETAIL
   ...
   .string_value = cv.description, reply->rowlist[row_nbr].celllist[3].string_value = cv.cdf_meaning
   IF (p.person_id > 0)
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("firstnet_day_week_audit.csv")
 ENDIF
