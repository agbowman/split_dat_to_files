CREATE PROGRAM cp_get_latest_comp_act_det:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Pathway Instance ID" = 0,
  "CP Component ID" = 0,
  "Pathway Action Detail Meaning (* for all)" = ""
  WITH outdev, path_inst_id, cp_comp_id,
  path_act_det_mean
 ...
 DECLARE pathway_instance_id = f8 WITH constant( $PATH_INST_ID), protect
 DECLARE pathway_component_id = f8 WITH constant( $CP_COMP_ID), protect
 DECLARE pathway_action_detail_meaning = vc WITH constant( $PATH_ACT_DET_MEAN), protect
 DECLARE main(null) = null
 FREE RECORD mpagereply
 RECORD mpagereply(
   1 cp_pathway_action_detail_id = f8
   1 action_detail_entity_name = vc
   1 action_detail_entity_id = f8
   1 action_detail_text = vc
   1 cp_action_detail_type_cd = f8
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 SUBROUTINE main(null)
   ...
   DECLARE pathway_comp_act_det_cd = f8 WITH constant(uar_get_code_by("MEANING",4003199,
     pathway_action_detail_meaning))
   IF (pathway_component_id > 0
    AND pathway_comp_act_det_cd > 0)
    SELECT INTO "nl:"
     FROM cp_pathway_action cpa,
      cp_pathway_action_detail cpad
     PLAN (cpa
      WHERE cpa.cp_component_id=pathway_component_id
       AND ((cpa.pathway_instance_id+ 0)=pathway_instance_id))
      JOIN (cpad
      WHERE cpad.cp_pathway_action_id=cpa.cp_pathway_action_id
       AND ((cpad.cp_action_detail_type_cd+ 0)=pathway_comp_act_det_cd))
     ORDER BY cpad.updt_dt_tm
     DETAIL
      mpagereply->action_detail_entity_id = cpad.action_detail_entity_id, mpagereply->
      action_detail_entity_name = cpad.action_detail_entity_name, mpagereply->
      cp_pathway_action_detail_id = cpad.cp_pathway_action_detail_id,
      mpagereply->action_detail_text = cpad.action_detail_text, mpagereply->cp_action_detail_type_cd
       = cpad.cp_action_detail_type_cd
     WITH nocounter
    ;end select
    CALL error_and_zero_check_rec(curqual,"CP_PATHWAY_ACTION_DETAIL","RETRIEVE",1,0,
     mpagereply)
   ELSE
    CALL reporterrorandexit("validate","Invalid parameters","0")
   ENDIF
