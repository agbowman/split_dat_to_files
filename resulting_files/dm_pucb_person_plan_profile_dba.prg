CREATE PROGRAM dm_pucb_person_plan_profile:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PUCB_PERSON_PLAN_PROFILE"
 ...
 SUBROUTINE cust_ucb_del(dummy)
   UPDATE  FROM person_plan_profile
    ...
    WHERE (person_plan_profile_id=rchildren->qual1[det_cnt].entity_id)
    WITH nocounter
   ;end update
   IF (curqual=0)
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0.0)
    ...
    SET dcipht_request->transaction_reason_txt = "PERSON_UCB_PERSON_PLAN_PROFILE"
