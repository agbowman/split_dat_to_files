CREATE PROGRAM bhs_rpt_asthma_qrtrly_practice:dba
 ...
 DECLARE email_list = vc WITH protect, noconstant(" ")
 DECLARE subject_line = vc WITH protect, noconstant(" ")
 ...
 FOR (i = 1 TO t_record->practice_cnt)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (d
     ...
     WHERE (ce.person_id=t_record->pat_qual[d.seq].person_id)
      ...
      AND ce.result_val IN ("Patient has never smoked",
     "Patient has not smoked in the last 12 months", "Patient has smoked in the last 12 months"))
   ...
   FOR (z = 1 TO t_record->pat_cnt)
    IF ((t_record->pat_qual[z].problem_ind=0))
     ...
     IF ((t_record->pat_qual[z].saba_ind=1)
      AND (((t_record->pat_qual[z].leuk_ind=1)) OR ((((t_record->pat_qual[z].is_ind=1)) OR ((((
     t_record->pat_qual[z].os_ind=1)) OR ((t_record->pat_qual[z].antichol_ind=1))) )) )) )
      SET severity_flag = 2
     ENDIF
     ...
     IF ((t_record->pat_qual[z].leuk_ind=1)
      AND (t_record->pat_qual[z].is_ind=1)
      AND (t_record->pat_qual[z].os_ind=1)
      AND (((t_record->pat_qual[z].saba_ind=1)) OR ((((t_record->pat_qual[z].laba_ind=1)) OR ((
     t_record->pat_qual[z].ca_ind=1))) )) )
      SET severity_flag = 3
     ENDIF
     IF ((t_record->pat_qual[z].leuk_ind=1)
   ...
   SELECT INTO "asthma_registry.xls"
    ...
    HEAD REPORT
     t_line = concat("Quarterly Asthma Registry for ",t_record->name), col 0, t_line,
    ...
    HEAD id
     ...
     address = trim(concat(t_record->pat_qual[d.seq].street1," ",t_record->pat_qual[d.seq].street2,
      ...
      char(9),t_record->pat_qual[d.seq].diagnois,char(9),t_record->pat_qual[d.seq].controller_med,
      char(9),
   ...
   IF (findfile("asthma_registry.xls")=1)
    ...
    SET subject_line = concat("Quarterly Asthma Registry Report for ",t_record->name)
