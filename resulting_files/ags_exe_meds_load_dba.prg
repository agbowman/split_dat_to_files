CREATE PROGRAM ags_exe_meds_load:dba
 CALL echo("***")
 CALL echo("***   BEG AGS_EXE_MEDS_LOAD")
 CALL echo("***")
 ...
 DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_exe_meds_load_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_EXE_MEDS_LOAD"
 CALL echo("***")
...
#skip_parse
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   work->qual[knt].ags_job_id = t.ags_job_id, work->qual[knt].ags_task_id = t.ags_task_id
  FOOT REPORT
   work->qual_knt = knt, stat = alterlist(work->qual,knt)
  WITH nocounter, maxqual(t,1000)
 ;end select
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  SET failed = select_error
 ...
 UPDATE  FROM ags_task t,
   (dummyt d  WITH seq = value(work->qual_knt))
  SET t.status = "PICKED", t.status_dt_tm = cnvtdatetime(curdate,curtime3)
  PLAN (d
   WHERE (work->qual[d.seq].ags_task_id > 0))
   JOIN (t
   WHERE (t.ags_task_id=work->qual[d.seq].ags_task_id))
  WITH nocounter
 ;end update
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  SET failed = update_error
  SET table_name = "UPDATE MEDS DATA"
  SET ilog_status = 1
  SET log->qual_knt = (log->qual_knt+ 1)
  SET stat = alterlist(log->qual,log->qual_knt)
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE MEDS DATA :: Update Error :: ",trim(serrmsg))
  SET serrmsg = log->qual[log->qual_knt].smsg
  GO TO exit_script
 ENDIF
