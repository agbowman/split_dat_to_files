CREATE PROGRAM cp_create_concept_pathway:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Input Pathway Name" = "",
  "Input Pathway Type Meaning" = ""
  WITH outdev, sinput_pathway_name, sinput_pathway_type_mean
 ...
 SUBROUTINE (filtercomponentdetails(nodeindx=i4,compindx=i4,comparefield=vc) =null WITH protect)
   ...
   CALL log_message(build("Exit filterComponentDetails(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SET log_program_name = "CP_CREATE_CONCEPT_PATHWAY"
 RECORD pathwayreply(
   1 pathway_id = f8
   1 concept_cd = f8
   1 status_data
 ...
 FREE RECORD conceptpwreply
 RECORD conceptpwreply(
   1 curqual = i4
   1 qual[*]
     2 status = i2
     2 error_num = i4
 ...
 DECLARE addnewconcept(null) = null WITH protect
 DECLARE createnewpathway(null) = null WITH protect
 DECLARE spath_name = vc WITH protect, constant(trim( $SINPUT_PATHWAY_NAME,3))
 DECLARE spath_input_type_mean = vc WITH protect, constant(trim( $SINPUT_PATHWAY_TYPE_MEAN,3))
 DECLARE dpath_input_type_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003197,trim(
     $SINPUT_PATHWAY_TYPE_MEAN,3)))
 DECLARE dpath_status_cd_active = f8 WITH protect, constant(uar_get_code_by("MEANING",4003198,
   "ACTIVE"))
 DECLARE duser_logic_domain = f8 WITH protect, constant(getuserlogicaldomain(reqinfo->updt_id))
 DECLARE dconcept_cd = f8 WITH protect, noconstant(0)
 CALL log_message(build2("Begin program ",log_program_name),log_level_debug)
 DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
 DECLARE existingconceptid = f8 WITH noconstant(0), private
 DECLARE existingpathwayid = f8 WITH noconstant(0), private
 SET existingconceptid = checkforexistingconcept(spath_name)
 SET existingpathwayid = checkforexistingactivepathway(spath_name,dpath_input_type_cd,
  duser_logic_domain)
 IF (existingpathwayid < 0)
  IF (existingconceptid < 0)
   CALL addnewconcept(null)
  ELSE
   SET dconcept_cd = existingconceptid
   SET pathwayreply->concept_cd = existingconceptid
  ENDIF
  CALL createnewpathway(null)
 ELSE
  SET pathwayreply->status_data.status = "F"
  CALL echo(build("Pathway: ",spath_name," already exists."))
 ENDIF
 SUBROUTINE addnewconcept(null)
   CALL log_message("Begin AddNewConcept()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   EXECUTE cp_add_concept "NOFORMS", nullterm(spath_name) WITH replace("CONCEPTREPLY",conceptpwreply)
   IF (size(conceptpwreply->qual,5)=1
    AND (conceptpwreply->status_data.status="S"))
    SET dconcept_cd = conceptpwreply->qual[1].code_value
    SET pathwayreply->concept_cd = conceptpwreply->qual[1].code_value
   ELSE
    SET stat = moverec(conceptpwreply->status_data,pathwayreply->status_data)
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit AddNewConcept(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE createnewpathway(null)
   CALL log_message("Begin AddNewConcept()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   EXECUTE cp_br_create_path_from_concept:dba "NOFORMS", nullterm(spath_name), value(dconcept_cd),
   nullterm(spath_input_type_mean)
   SET pathwayreply->pathway_id = mpagereply->pathway_id
   IF ((mpagereply->status_data.status="F"))
    SET pathwayreply->status_data.status = "F"
   ELSE
    SET pathwayreply->status_data.status = "S"
   ENDIF
   CALL log_message(build("Exit AddNewConcept(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (validate(debug_ind,0))
  CALL echorecord(pathwayreply)
 ENDIF
 CALL putjsonrecordtofile(pathwayreply)
 IF (( $OUTDEV != "NOFORMS"))
  FREE RECORD pathwayreply
 ENDIF
