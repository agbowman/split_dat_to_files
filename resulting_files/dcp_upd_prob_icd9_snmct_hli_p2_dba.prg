CREATE PROGRAM dcp_upd_prob_icd9_snmct_hli_p2:dba
 ...
 SUBROUTINE (sbr_p2_check_for_prev_rdm(readme_id=i4,instance=i4) =i4)
   ...
   ENDIF
 END ;Subroutine
 DECLARE parent_script_name = vc WITH protect, constant("DCP_UPD_PROB_ICD9_SNMCT_HLI_AUDIT")
 DECLARE range_name = vc WITH protect, constant("DCP_UPD_PROB_ICD9_SNMCT_HLI_AUDIT RANGE")
 DECLARE child_c_script_name = vc WITH protect, constant("dcp_upd_prob_icd9_snmct_hli_cc")
 DECLARE max_person_id_eval = vc WITH protect, constant("MAX PERSON_ID EVALUATED")
 DECLARE range_name_p2 = vc WITH protect, constant("DCP_UPD_PROB_ICD9_SNMCT_HLI_AUDIT CAT5 RANGE")
 DECLARE max_num_children = i4 WITH protect, constant(5)
 DECLARE batch_size_name = vc WITH protect, constant("DCP_UPD_PROB_ICD9_SNMCT_HLI_AUDIT BATCH SIZE")
 DECLARE oracle_hint_name = vc WITH protect, constant("DCP_UPD_PROB_ICD9_SNMCT_HLI_AUDIT ORAHINT")
 SET readme_data->status = "F"
 ...
 SET ms_info_domain_nm = parent_script_name
 SET ms_range = range_name_p2
 SET ms_max_name_p = max_person_id_eval
 SET ms_child_script = child_c_script_name
 SET mn_num_children = max_num_children
 SELECT INTO "nl:"
  max_val = max(person_id)
  FROM problem
  WHERE person_id > 0.0
  DETAIL
 ...
 DECLARE dhli = f8 WITH protect, noconstant(0.0)
 ...
 SELECT INTO "nl:"
  ...
  WHERE cv.code_set=dvocabtypecs
   ...
   AND cv.cdf_meaning IN ("ICD9", "SNMCT", "HLI.PFT")
  DETAIL
   CASE (cv.cdf_meaning)
    OF "ICD9":
     dicd9 = cv.code_value
 ...
 IF (error(ms_errmsg,0) != 0)
  ...
  GO TO exit_program
 ENDIF
 CALL echo(build("userOraHint in P: ",userorahint))
...
#exit_program
 ...
 ELSE
  COMMIT
  CALL sbr_add_dropped_object("INDEX","XIETEMPPROBICD9SNMCTHLI","Dropping index")
  CALL sbr_add_dropped_object("TABLE","TEMP_PROB_ICD9_SNMCT_HLI","Dropping table")
