CREATE PROGRAM bbt_upd_cxm_criteria:dba
 ...
 SUBROUTINE insertreqproc(nrequest,spfmt,ntargetreq,ndeststep,sservice,nreprocess,naction,srefpfmt,
  nseq)
   ...
   ENDCASE
 END ;Subroutine
 DECLARE sreadme_name = vc WITH protected, noconstant(" ")
 DECLARE status_code = i2 WITH protected, noconstant(0)
 DECLARE msgstr = vc WITH protected, noconstant(" ")
 DECLARE msgstatus = c1 WITH protected, noconstant(fillstring(1,"F"))
 DECLARE cur_not_equal_sec_cd = f8 WITH protected, noconstant(0.0)
 DECLARE aborh_cur_det_cd = f8 WITH protected, noconstant(0.0)
 DECLARE demo_aborh_diff_cd = f8 WITH protected, noconstant(0.0)
 DECLARE disc_cur_aborh_cd = f8 WITH protected, noconstant(0.0)
 DECLARE disc_cur_absc_cd = f8 WITH protected, noconstant(0.0)
 DECLARE disp_trans_aborh_cd = f8 WITH protected, noconstant(0.0)
 DECLARE cxm_criteria_code_set = i4 WITH protected, constant(19269)
 DECLARE cur_not_equal_sec_meaning = c12 WITH protected, constant("1STNOTEQ2ND")
 DECLARE aborh_cur_det_meaning = c12 WITH protected, constant("ABORH1STDET")
 DECLARE demo_aborh_diff_meaning = c12 WITH protected, constant("DEMABORHDIFF")
 DECLARE disc_cur_aborh_meaning = c12 WITH protected, constant("DSCRPNT1ST")
 DECLARE disc_cur_absc_meaning = c12 WITH protected, constant("DSCRPNTABSC")
 DECLARE disp_trans_aborh_meaning = c12 WITH protected, constant("DSPTRNABORH")
 SET serrormsg = fillstring(255," ")
 SET nerrorstatus = error(serrormsg,1)
 SET sreadme_name = "bbt_upd_cxm_criteria"
 CALL logscriptstart(sreadme_name)
 SELECT INTO "nl:"
  FROM code_value c
  WHERE c.code_set=cxm_criteria_code_set
   AND c.cdf_meaning IN (cur_not_equal_sec_meaning, aborh_cur_det_meaning, demo_aborh_diff_meaning,
  disc_cur_aborh_meaning, disc_cur_absc_meaning,
  disp_trans_aborh_meaning)
   AND c.active_ind=1
   AND c.code_value > 0
  DETAIL
   IF (c.cdf_meaning=cur_not_equal_sec_meaning)
    cur_not_equal_sec_cd = c.code_value
   ELSEIF (c.cdf_meaning=aborh_cur_det_meaning)
    aborh_cur_det_cd = c.code_value
   ELSEIF (c.cdf_meaning=demo_aborh_diff_meaning)
    demo_aborh_diff_cd = c.code_value
   ELSEIF (c.cdf_meaning=disc_cur_aborh_meaning)
    disc_cur_aborh_cd = c.code_value
   ELSEIF (c.cdf_meaning=disc_cur_absc_meaning)
    disc_cur_absc_cd = c.code_value
   ELSEIF (c.cdf_meaning=disp_trans_aborh_meaning)
    disp_trans_aborh_cd = c.code_value
   ENDIF
  WITH nocounter, forupdate(c)
 ;end select
 IF (cur_not_equal_sec_cd=0)
  SET status_code = 1
  SET msgstr = "Could not find 1STNOTEQ2ND in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 IF (aborh_cur_det_cd=0)
  SET status_code = 1
  SET msgstr = "Could not find ABORH1STDET in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 IF (demo_aborh_diff_cd=0)
  SET status_code = 1
  SET msgstr = "Could not find DEMABORHDIFF in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 IF (disc_cur_aborh_cd=0)
  SET status_code = 1
  SET msgstr = "Could not find DSCRPNT1ST in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 IF (disc_cur_absc_cd=0)
  SET status_code = 1
  SET msgstr = "Could not find DSCRPNTABSC in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 IF (disp_trans_aborh_cd=0)
  SET status_code = 1
  SET msgstr = "Could not find DSPTRNABORH in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 UPDATE  FROM code_value c
  SET c.description = "Cur ABO/Rh not equal 2nd ABO/Rh", c.definition =
   "A discrepancy exists between the current and second ABO/Rh determinations.", c.updt_dt_tm =
   cnvtdatetime(curdate,curtime3),
   c.updt_id = 0, c.updt_cnt = (c.updt_cnt+ 1), c.updt_task = 0,
   c.updt_applctx = 0
  WHERE c.code_value=cur_not_equal_sec_cd
  WITH nocounter
 ;end update
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  SET status_code = 1
  SET msgstr = "Could not update 1STNOTEQ2ND in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 UPDATE  FROM code_value c
  SET c.description = "Cur ABO/Rh Not Found", c.definition =
   "Current ABO/Rh determination could not be found.", c.updt_dt_tm = cnvtdatetime(curdate,curtime3),
   c.updt_id = 0, c.updt_cnt = (c.updt_cnt+ 1), c.updt_task = 0,
   c.updt_applctx = 0
  WHERE c.code_value=aborh_cur_det_cd
  WITH nocounter
 ;end update
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  SET status_code = 1
  SET msgstr = "Could not update ABORH1STDET in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 UPDATE  FROM code_value c
  SET c.definition =
   "A discrepancy exists between the current ABO/Rh determination and the patient's demographic ABO/Rh.",
   c.updt_dt_tm = cnvtdatetime(curdate,curtime3), c.updt_id = 0,
   c.updt_cnt = (c.updt_cnt+ 1), c.updt_task = 0, c.updt_applctx = 0
  WHERE c.code_value=demo_aborh_diff_cd
  WITH nocounter
 ;end update
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  SET status_code = 1
  SET msgstr = "Could not update DEMABORHDIFF in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 UPDATE  FROM code_value c
  SET c.description = "Discrepancy same sample cur det", c.definition =
   "A discrepancy exists between ABO/Rh result on same sample as current ABO/Rh determination.", c
   .updt_dt_tm = cnvtdatetime(curdate,curtime3),
   c.updt_id = 0, c.updt_cnt = (c.updt_cnt+ 1), c.updt_task = 0,
   c.updt_applctx = 0
  WHERE c.code_value=disc_cur_aborh_cd
  WITH nocounter
 ;end update
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  SET status_code = 1
  SET msgstr = "Could not update DSCRPNT1ST in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 UPDATE  FROM code_value c
  SET c.definition = "A discrepancy exists between antibody screen results on current sample.", c
   .updt_dt_tm = cnvtdatetime(curdate,curtime3), c.updt_id = 0,
   c.updt_cnt = (c.updt_cnt+ 1), c.updt_task = 0, c.updt_applctx = 0
  WHERE c.code_value=disc_cur_absc_cd
  WITH nocounter
 ;end update
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  SET status_code = 1
  SET msgstr = "Could not update DSCRPNTABSC in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 UPDATE  FROM code_value c
  SET c.description = "Disp/Trans ABORH", c.definition =
   "Products dispensed/transfused since drawn date/time of specimen for current ABO/RH determination.",
   c.updt_dt_tm = cnvtdatetime(curdate,curtime3),
   c.updt_id = 0, c.updt_cnt = (c.updt_cnt+ 1), c.updt_task = 0,
   c.updt_applctx = 0
  WHERE c.code_value=disp_trans_aborh_cd
  WITH nocounter
 ;end update
 SET serror_check = error(serrormsg,0)
 IF (nerrorstatus != 0)
  SET status_code = 1
  SET msgstr = "Could not update DSPTRNABORH in code_value table."
  CALL logmsg(msgstr,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
#exit_script
 ...
 IF (status_code=1)
  ROLLBACK
  SET msgstr = "Error updating code set 19269 - cxm criteria."
  SET msgstatus = "F"
 ELSEIF (status_code=0)
  SET msgstr = "Code set 19269- cxm criteria have been successfully updated."
  SET msgstatus = "S"
  COMMIT
 ENDIF
 CALL logstatus(msgstr,msgstatus)
 CALL logmsg(msgstr,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
