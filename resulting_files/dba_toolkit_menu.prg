CREATE PROGRAM dba_toolkit_menu
 PAINT
 ...
 ENDIF
 DECLARE dtm_toolset_usage = i2
 SET dtm_toolset_usage = dm2_toolset_usage(null)
 IF (dtm_toolset_usage=0)
  SET message = nowindow
  CALL echo("*")
  CALL echo("*")
  CALL echo("*")
 ...
 DECLARE oracle_version = i4 WITH public, noconstant(0)
 DECLARE cons_col_cnt = i4 WITH protect, noconstant(0)
 SELECT INTO "nl:"
 ...
 SELECT INTO "nl:"
  ...
  WHERE l.structtype="F"
   ...
   AND d.table_name="USER_CONSTRAINTS"
  HEAD REPORT
   cons_col_cnt = 0
  DETAIL
   cons_col_cnt = (cons_col_cnt+ 1)
  WITH nocounter
 ;end select
 IF (((oracle_version=7
  AND cons_col_cnt < 9) OR (((oracle_version=8
  AND cons_col_cnt < 16) OR (oracle_version=9
  AND cons_col_cnt < 20)) )) )
  CALL display_heading(1)
  CALL text(10,5,build("Current Oracle version is Oracle",oracle_version))
  CALL text(12,5,"CCL definition of Oracle system tables and views have not been updated")
  CALL text(14,5,"Please oragen3 to generate CCL definitions for the SYS user.")
  CALL pause(2)
  GO TO endprogram
 ENDIF
 CALL current_database(1)
#start
 CALL display_heading(1)
 IF (cursys != "VMS"
  AND cursys != "AXP"
  AND cursys != "AIX")
  CALL clear(23,2,79)
  CALL text(23,15,"Unknown Operating System")
  CALL pause(2)
  GO TO endprogram
 ENDIF
 CALL text(6,15,"1.   Administration")
 CALL text(7,15,"2.   Monitoring")
 CALL text(8,15,"3.   Storage Management")
 CALL text(9,15,"4.   Backup & Recovery")
 CALL text(10,15,"5.   Database Connection")
 CALL do_select(1)
 CASE (toolkit_req->selection)
  OF 1:
   GO TO admin
  OF 2:
   GO TO monitor
  OF 3:
   GO TO storagemgt
  OF 4:
   GO TO backuprecovery
  OF 5:
   CALL setup_db_connection(1)
  OF 0:
   GO TO endprogram
  ELSE
   CALL invalid_select(1)
   GO TO start
 ENDCASE
 GO TO start
#admin
 SET validate = off
 CALL display_heading(1)
 CALL text(3,33,"ADMINISTRATION")
 CALL text(6,15,"1.   DM Environment Maintenance")
 CALL do_select(1)
 IF (((cursys="VMS") OR (cursys="AXP")) )
  CASE (toolkit_req->selection)
   OF 1:
    EXECUTE dm_env_maint
   OF 0:
    GO TO start
   ELSE
    CALL invalid_select(1)
...
#monitor
 SET validate = off
 CALL display_heading(1)
 CALL text(3,35,"MONITORING")
 CALL text(6,15,"1.   Space Summary Report")
 CALL text(7,15,"2.   Space Trend Report")
 CALL text(8,15,"3.   Performance Summary and Trend Report")
 CALL text(9,15,"4.   Monitor Alert Log")
 CALL text(10,15,"5.   Database Performance Statistics")
 CALL text(11,15,"6.   Analyze Schema Report")
 CALL text(13,15,"7.   Admin DB Maintenance")
 CALL text(14,15,"       Manage Reference Tables, Purge Old Monitoring Data,")
 CALL text(15,15,"       Verify DB Links")
 CALL do_select(1)
 CASE (toolkit_req->selection)
  OF 1:
   CALL clear(1,1)
   GO TO spacesummary
  OF 2:
   EXECUTE dm_database_resizing
  OF 3:
   CALL dis_cerrec(1)
  OF 4:
   EXECUTE dba_alertmon
  OF 5:
   EXECUTE dba_stat_menu
  OF 6:
   CALL clear(1,1)
   GO TO analyzeschema
  OF 7:
   EXECUTE dba_manage_ref
  OF 0:
   GO TO start
  ELSE
   CALL invalid_select(1)
...
#spacesummary
 SET validate = off
 CALL display_heading(1)
 CALL text(3,35,"SPACE SUMMARY")
 CALL text(6,15,"1.   Collection Script Generation")
 CALL text(7,15,"2.   Space Summary Report")
 CALL text(8,15,"3.   Brief Instructions")
 CALL do_select(1)
 CASE (toolkit_req->selection)
  OF 1:
   EXECUTE dba_exe_space_gen
  OF 2:
   CALL clear(1,1)
   EXECUTE rpt_space
  OF 3:
   CALL clear(1,1)
   CALL display_heading(1)
   CALL text(3,25,"SPACE SUMMARY INSTRUCTIONS")
   CALL text(5,3,"1. In target database SQL, include the packages defined by")
   CALL text(6,3,"   pkg_dba_utility.sql, pkg_utility.sql, pkg_space.sql and")
   CALL text(7,3,"   pkg_reports_control.sql in this order.")
   CALL text(8,3,"2. In target database SQL, issue command 'grant analyze any to v500'")
   CALL text(9,3,"3. In target database SQL, issue command 'grant select any table to v500'")
   CALL text(10,3,"4. In target database SQL, issue command 'grant execute any procedure to v500'")
   CALL text(11,3,"5. In CCL, use the dba_toolkit_menu program -> Monitoring -> ")
   CALL text(12,3,"   Admin DB Maint. to set up the Ref_Instance_Id table.")
   CALL text(13,3,"6. In CCL, use the dba_toolkit_menu program -> Monitoring ->")
   CALL text(14,3,"   Space Summary Report -> Collection Script Generation to create the")
   CALL text(15,3,"   needed two files that will gather the space summary data.")
   CALL text(16,3,"7. Execute the 'run' output file from step 6 at the O/S level or in ")
   CALL text(17,3,"   operations")
   CALL text(18,3,"8. In CCL, use the dba_toolkit_menu program -> Monitoring ->")
   CALL text(19,3,"   Space Summary Report -> Space Summary Report to provide a")
   CALL text(20,3,"   report on the gathered data.")
   SET continue = " "
   CALL text(21,15,"Hit any key to continue")
   CALL clear(23,2,78)
   CALL accept(21,60,"P;CU "," ")
   GO TO spacesummary
  OF 0:
   GO TO start
  ELSE
...
#analyzeschema
 SET validate = off
 CALL display_heading(1)
 CALL text(3,35,"ANALYZE SCHEMA")
 CALL text(6,15,"1.   Analyze Schema Script Generation")
 CALL text(7,15,"2.   Analyze Schema Report")
 CALL text(8,15,"3.   Brief Instructions")
 CALL do_select(1)
 CASE (toolkit_req->selection)
  OF 1:
   EXECUTE dba_exe_analyze_gen
  OF 2:
   CALL clear(1,1)
   EXECUTE dba_rpt_analyze
  OF 3:
   CALL clear(1,1)
   CALL display_heading(1)
   CALL text(3,25,"ANALYZE SCHEMA INSTRUCTIONS")
   CALL text(5,3,"1. Install the Space Summary Report as described in the standard")
   CALL text(6,3,"   documentation.")
   CALL text(8,3,"2. In CCL, use the dba_toolkit_menu program -> Monitoring ->")
   CALL text(9,3,"   Analyze Schema Report -> Analyze Schema Script Generation to create the")
   CALL text(10,3,"   needed two files that will analyze the schema.")
   CALL text(12,3,"3. Execute the 'run' output file from step 2 at the O/S level or in ")
   CALL text(13,3,"   operations")
   CALL text(15,3,"4. In CCL, use the dba_toolkit_menu program -> Monitoring ->")
   CALL text(16,3,"   Analyze Schema Report -> Analyze Schema Report to provide a")
   CALL text(17,3,"   report on the any errors that may have occurred during the execution.")
   SET continue = " "
   CALL text(21,15,"Hit any key to continue")
   CALL clear(23,2,78)
   CALL accept(21,60,"P;CU "," ")
...
#storagemgt
 SET validate = off
 CALL display_heading(1)
 CALL text(3,30,"STORAGE MANAGEMENT")
 CALL text(6,15,"1.   Add/Extend Tablespace")
 CALL text(7,15,"2.   Coalesce Free Space")
 CALL text(8,15,"3.   Rebuild Index")
 CALL text(9,15,"4.   Rebuild Table")
 CALL text(10,15,"5.   Rebuild Tablespace")
 CALL do_select(1)
 CASE (toolkit_req->selection)
  OF 1:
   IF ((((dm2_sys_misc->cur_os="AXP")) OR ((dm2_sys_misc->cur_os IN ("AIX", "HPX"))
    AND validate(dm2_use_tspace_anyway,- (1))=722))
    AND (dm2_sys_misc->cur_os=dm2_sys_misc->cur_db_os))
    SET dm2menureq->prompt_mode = "NOPROMPT"
    SET dm2menureq->install_type = "MANUAL"
    SET dm2menureq->menu_option = 4
    EXECUTE dm2_tablespace_maint
   ELSE
    CALL clear(23,2,79)
    IF ((dm2_sys_misc->cur_os=dm2_sys_misc->cur_db_os))
     CALL text(23,15,"Tablespace Maintenance not available on current O/S platform.")
    ELSE
     CALL text(23,15,"Tablespace Maintenance not available for remote databases.")
    ENDIF
    CALL pause(2)
   ENDIF
  OF 2:
   EXECUTE dba_coalesce_free_space
  OF 3:
   EXECUTE dba_rebuild_index
  OF 4:
   EXECUTE dba_rebuild_table
  OF 5:
   EXECUTE dba_rebuild_tablespace
  OF 0:
   GO TO start
  ELSE
   CALL invalid_select(1)
...
#backuprecovery
 SET validate = off
 CALL display_heading(1)
 CALL text(3,31,"BACKUP & RECOVERY")
 CALL text(6,15,"1.   Hot and Cold Backup")
 CALL text(7,15,"2.   Export")
 CALL do_select(1)
 IF (((cursys="VMS") OR (cursys="AXP")) )
  CASE (toolkit_req->selection)
   OF 1:
 ...
 SUBROUTINE display_heading(x)
   ...
   CALL clear(3,2,78)
   CALL text(3,5,"DATABASE: ")
   CALL text(3,15,toolkit_req->current_db)
   CALL text(3,65,"SID: ")
   CALL text(3,70,toolkit_req->current_sid)
   CALL video(n)
 END ;Subroutine
 SUBROUTINE do_select(x)
   SET toolkit_req->selection = 0
   CALL text(20,15,"Your Selection(0 to Exit)?")
   CALL clear(23,2,78)
   CALL accept(20,60,"99",0)
   SET toolkit_req->selection = cnvtint(curaccept)
 END ;Subroutine
 SUBROUTINE invalid_select(x)
  CALL text(23,15,"Invalid selection...")
  CALL pause(2)
 END ;Subroutine
 SUBROUTINE vms_hotbackup(x)
   CALL clear(20,2,77)
   CALL box(11,5,21,75)
   CALL text(12,28,"-----  Hot Backup  -----")
   CALL text(13,7,"Domain:")
   CALL text(14,7,"Username/Password:")
   CALL text(15,7,"Tape/Disk:")
   CALL text(16,7,"Device:")
   CALL text(17,7,"ALL=Y/ALL=N/TSPACE:")
   CALL accept(13,27,"PPPPPPPPPPPPPPP;CU")
   SET domainname = trim(curaccept)
   CALL accept(14,27,"PPPPPPPPPPPPPPP;CU")
   SET userpswd = trim(curaccept)
   CALL accept(15,27,"PPPP;CU","TAPE")
   SET devicetype = trim(curaccept)
   WHILE (devicetype != "TAPE"
    AND devicetype != "DISK")
     CALL clear(23,2,77)
     CALL text(23,5,"Invalid Input.")
     CALL pause(1)
     CALL accept(15,27,"PPPP;CU")
     SET devicetype = trim(curaccept)
   ENDWHILE
   CALL clear(23,2,77)
   SET t_device = fillstring(45," ")
   SET device_elements = 5
   SET stat = memalloc(device_list,device_elements,"C45")
   SET d_cnt = 0
   CALL accept(16,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CU",
    "                                            ")
   SET t_device = trim(curaccept)
   WHILE (t_device="                                            ")
     CALL clear(23,2,77)
     CALL text(23,5,"Device name required.")
     CALL pause(2)
     CALL accept(16,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CU",
      "                                            ")
     SET t_device = trim(curaccept)
   ...
   IF (devicetype="TAPE")
    SET d_cnt = (d_cnt+ 1)
    SET device_list[d_cnt] = t_device
   ELSE
    WHILE (t_device != "                                            ")
      SET d_cnt = (d_cnt+ 1)
      IF (d_cnt > device_elements)
       SET device_elements = (device_elements+ 5)
       SET stat = memrealloc(device_list,device_elements,"C45")
      ENDIF
      SET device_list[d_cnt] = t_device
      SET t_device = fillstring(45," ")
      CALL clear(23,5,74)
      CALL text(23,5,"Enter next device or press return to finish.")
      CALL accept(16,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CU",
       "                                            ")
      SET t_device = trim(curaccept)
    ENDWHILE
    CALL clear(23,5,74)
   ENDIF
   CALL accept(17,27,"PPPPPP;CU","ALL=Y")
   SET backupmode = trim(curaccept)
   WHILE (backupmode != "ALL=Y"
    AND backupmode != "ALL=N"
    AND backupmode != "TSPACE")
     CALL clear(23,2,77)
     CALL text(23,5,"Invalid Input.")
     CALL pause(1)
     CALL accept(17,27,"PPPPPP;CU")
     SET backupmode = trim(curaccept)
   ENDWHILE
   CALL clear(23,2,77)
   SET tspace_cnt = 0
   IF (backupmode="TSPACE")
    SET tspace_elements = 5
    SET stat = memalloc(tspace_list,tspace_elements,"C30")
    SET t_tspace = fillstring(30," ")
    CALL text(18,7,"Tablespace Name:")
    CALL accept(18,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CU","                              ")
    SET t_tspace = trim(curaccept)
    WHILE (t_tspace="                              ")
      CALL clear(23,2,77)
      CALL text(23,5,"Tablespace name required.")
      CALL pause(2)
      CALL accept(18,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CU","                              ")
      SET t_tspace = trim(curaccept)
    ENDWHILE
    WHILE (t_tspace != "                              ")
      SET tspace_cnt = (tspace_cnt+ 1)
      IF (tspace_cnt > tspace_elements)
       SET tspace_elements = (tspace_elements+ 5)
       SET stat = memrealloc(tspace_list,tspace_elements,"C30")
      ENDIF
      SET tspace_list[tspace_cnt] = t_tspace
      SET t_tspace = fillstring(30," ")
      CALL clear(23,2,77)
      CALL text(23,5,"Enter next tablespace name or press return to finish.")
      CALL accept(18,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CU","                              ")
      SET t_tspace = trim(curaccept)
    ENDWHILE
    CALL clear(23,2,77)
   ENDIF
   CALL display_vms_hotbackup_info(1)
   CALL confirm_message(1)
   IF ((toolkit_req->confirm_ans="Y"))
    SET dcl_hotbackup = fillstring(240," ")
    SET dcl_hotbackup = trim(concat("@DBA_ROOT:[000000]HOT_BACKUP ",trim(domainname)," ",trim(
       userpswd)," ",
      trim(devicetype)))
    SET x = 1
    SET dcl_hotbackup = concat(trim(dcl_hotbackup)," ",trim(device_list[x]))
    SET x = (x+ 1)
    WHILE (x <= d_cnt)
     SET dcl_hotbackup = concat(trim(dcl_hotbackup),",",trim(device_list[x]))
     SET x = (x+ 1)
    ENDWHILE
    SET dcl_hotbackup = concat(trim(dcl_hotbackup)," ",trim(backupmode))
    IF (backupmode="TSPACE")
     SET x = 1
     SET dcl_hotbackup = concat(trim(dcl_hotbackup)," ",trim(tspace_list[x]))
     SET x = (x+ 1)
     WHILE (x <= tspace_cnt)
      SET dcl_hotbackup = concat(trim(dcl_hotbackup),",",trim(tspace_list[x]))
      SET x = (x+ 1)
     ENDWHILE
    ENDIF
    SET dcl_hotbackup_len = size(trim(dcl_hotbackup))
    SET dcl_hotbackup_status = 0
    CALL clear(1,1)
    CALL text(20,2,trim(dcl_hotbackup))
    CALL pause(3)
    CALL dcl(trim(dcl_hotbackup),dcl_hotbackup_len,dcl_hotbackup_status)
    CALL pause(3)
   ENDIF
 END ;Subroutine
 SUBROUTINE display_vms_hotbackup_info(z)
   SET x = 6
   FOR (x = 6 TO 21)
     CALL clear(x,2,78)
   ENDFOR
   CALL text(6,3,"Domain:")
   CALL text(6,10,domainname)
   CALL text(7,3,"User/Pswd:")
   CALL text(7,13,userpswd)
   CALL text(8,3,"Device Type:")
   CALL text(8,15,devicetype)
   CALL text(9,3,"Backup Mode:")
   CALL text(9,15,backupmode)
   CALL text(6,30,"Device(s):")
   SET x = 1
   SET line_num = 7
   FOR (x = 1 TO d_cnt)
    CALL text(line_num,32,concat(trim(cnvtstring(x)),". ",trim(device_list[x])))
    SET line_num = (line_num+ 1)
   ENDFOR
   IF (backupmode="TSPACE")
    CALL text(6,55,"Tablespace(s):")
    SET x = 1
    SET line_num = 7
    FOR (x = 1 TO tspace_cnt)
     CALL text(line_num,57,concat(trim(cnvtstring(x)),". ",trim(tspace_list[x])))
     SET line_num = (line_num+ 1)
    ENDFOR
   ENDIF
 END ;Subroutine
 SUBROUTINE vms_coldbackup(x)
   CALL box(12,5,20,75)
   CALL text(13,28,"-----  Cold Backup  -----")
   CALL text(14,7,"Domain:")
   CALL text(15,7,"Username/Password:")
   CALL text(16,7,"Tape Device:")
   CALL text(17,7,"ALL=Y/ALL=N:")
   CALL accept(14,27,"PPPPPPPPPPPPPPP;CU")
   SET param1 = trim(curaccept)
   CALL accept(15,27,"PPPPPPPPPPPPPPP;CU")
   SET param2 = trim(curaccept)
   CALL accept(16,27,"PPPPPPPPPPPPPP;CU")
   SET param3 = trim(curaccept)
   CALL accept(17,27,"PPPPPP;CU","ALL=Y")
   SET param4 = trim(curaccept)
   WHILE (param4 != "ALL=Y"
    AND param4 != "ALL=N")
     CALL clear(23,2,77)
     CALL text(23,5,"Invalid Input.")
   ...
   CALL confirm_message(1)
   IF ((toolkit_req->confirm_ans="Y"))
    SET dcl_coldbackup = concat("@DBA_ROOT:[000000]COLD_BACKUP ",trim(param1)," ",trim(param2)," ",
     trim(param3)," ",trim(param4))
    SET dcl_coldbackup_len = size(dcl_coldbackup)
    SET dcl_coldbackup_status = 0
    CALL clear(1,1)
    CALL dcl(dcl_coldbackup,dcl_coldbackup_len,dcl_coldbackup_status)
    CALL pause(2)
   ENDIF
 END ;Subroutine
 SUBROUTINE vms_expimpbackup(x)
   CALL box(12,5,20,75)
   CALL text(13,30,"-----  EXPORT  -----")
   CALL text(14,7,"Import/Export(I/E):")
   CALL text(15,7,"Full(Y/N):")
   CALL text(16,7,"Database Name:")
   CALL text(17,7,"Database Disk:")
   CALL clear(23,5,74)
   CALL text(23,5,"E for export, I for import")
   CALL accept(14,27,"P;CU","E")
   SET param1 = trim(curaccept)
   WHILE (param1 != "I"
    AND param1 != "E")
     CALL text(23,5,"Invalid Input.")
   ...
   ENDWHILE
   CALL clear(23,5,74)
   CALL text(23,5,"Y for full backup, N for interactive")
   CALL accept(15,27,"P;CU","Y")
   SET param2 = trim(curaccept)
   WHILE (param2 != "Y"
    AND param2 != "N")
     CALL text(23,5,"Invalid Input.")
   ...
   ENDWHILE
   CALL clear(23,5,74)
   CALL accept(16,27,"PPPPP;CU")
   SET param3 = trim(curaccept)
   CALL accept(17,27,"PPPPPPPPPPPPPPP;CU")
   SET param4 = trim(curaccept)
   SET param5 = fillstring(45," ")
   SET param6 = fillstring(45," ")
   IF (param2="Y")
    CALL text(18,7,"Dump File:")
    CALL text(19,7,"Username/Password:")
    CALL accept(18,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CU")
    SET param5 = trim(curaccept)
    CALL accept(19,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CU")
    SET param6 = trim(curaccept)
   ENDIF
   CALL confirm_message(1)
   IF ((toolkit_req->confirm_ans="Y"))
    SET dcl_expimpbackup = concat("@DBA_ROOT:[000000]EXPIMP_BACKUP ",trim(param1)," ",trim(param2),
     " ",
     trim(param3)," ",trim(param4)," ",trim(param5),
     " ",trim(param6))
    SET dcl_expimpbackup_len = size(dcl_expimpbackup)
    SET dcl_expimpbackup_status = 0
    CALL clear(1,1)
    CALL dcl(dcl_expimpbackup,dcl_expimpbackup_len,dcl_expimpbackup_status)
    CALL pause(2)
   ENDIF
 END ;Subroutine
 SUBROUTINE unix_coldbackup(x)
   CALL box(12,5,20,75)
   CALL text(13,28,"-----  Cold Backup  -----")
   CALL text(14,7,"Domain:")
   CALL text(15,7,"Username/Password:")
   CALL text(16,7,"Tape/Disk:")
   CALL accept(14,27,"PPPPPPPPPPPPPPP;CL")
   SET param1 = trim(curaccept)
   CALL accept(15,27,"PPPPPPPPP;CL")
   SET param2 = trim(curaccept)
   CALL accept(16,27,"PPPP;CL")
   SET param3 = trim(curaccept)
   WHILE (param3 != "tape"
    AND param3 != "disk")
     CALL text(23,5,"Invalid Input.")
   ...
   CALL clear(23,5,74)
   IF (param3="tape")
    CALL text(17,7,"Device:")
   ELSE
    CALL text(17,7,"Path:")
   ENDIF
   CALL accept(17,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CL")
   SET param4 = trim(curaccept)
   CALL confirm_message(1)
   IF ((toolkit_req->confirm_ans="Y"))
    SET dcl_coldbackup = concat("$cer_mgr/ora_backup.ksh ",trim(param1)," ",trim(param2)," ",
     trim(param3),"=",trim(param4)," cold;")
    SELECT INTO concat("/tmp/cold_backup_",trim(param1),".ksh")
     *
     FROM dummyt
     DETAIL
      col 0, "#!/usr/bin/ksh", row + 1,
      col 0, "su - oracle <<!", row + 1,
      col 0,
      CALL print(trim(dcl_coldbackup)), row + 1,
      col 0, "exit", row + 1,
      col 0, "!"
     WITH nocounter, format = stream, noheading,
      formfeed = none, maxrow = 1
    ;end select
    SET dcl_string2 = concat("chmod 777 /tmp/cold_backup_",trim(param1),".ksh")
    SET dcl_len2 = size(trim(dcl_string2))
    SET dcl_status2 = 0
    CALL dcl(dcl_string2,dcl_len2,dcl_status2)
    SET dcl_string3 = concat("/tmp/cold_backup_",trim(param1),".ksh")
    SET dcl_len3 = size(trim(dcl_string3))
    SET dcl_status3 = 0
    CALL clear(1,1)
    CALL text(20,1,trim(dcl_string3))
    CALL pause(3)
    CALL dcl(dcl_string3,dcl_len3,dcl_status3)
    CALL pause(9)
   ENDIF
 END ;Subroutine
 SUBROUTINE unix_hotbackup(x)
   CALL clear(20,2,77)
   CALL box(11,5,21,75)
   CALL text(12,28,"-----  Hot Backup  -----")
   CALL text(13,7,"Domain:")
   CALL text(14,7,"Username/Password:")
   CALL text(15,7,"Tape/Disk:")
   CALL text(17,7,"All/Tspace:")
   CALL accept(13,27,"PPPPPPPPPPPPPPP;CL")
   SET domainname = trim(curaccept)
   CALL accept(14,27,"PPPPPPPPP;CL")
   SET userpswd = trim(curaccept)
   CALL accept(15,27,"PPPP;CL","tape")
   SET devicetype = trim(curaccept)
   WHILE (devicetype != "tape"
    AND devicetype != "disk")
     CALL clear(23,2,77)
   ...
   SET stat = memalloc(device_list,device_elements,"C45")
   SET d_cnt = 0
   CALL accept(16,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CL",
    "                                            ")
   SET t_device = trim(curaccept)
   WHILE (t_device="                                            ")
     CALL clear(23,2,77)
     CALL text(23,5,"Device or Path name required.")
     CALL pause(2)
     CALL accept(16,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CU",
      "                                            ")
     SET t_device = trim(curaccept)
   ...
   SET d_cnt = (d_cnt+ 1)
   SET device_list[d_cnt] = t_device
   CALL accept(17,27,"PPPPPP;CL","all")
   SET backupmode = trim(curaccept)
   WHILE (backupmode != "all"
    AND backupmode != "tspace")
     CALL clear(23,2,77)
     CALL text(23,5,"Invalid Input.")
     CALL pause(1)
     CALL accept(17,27,"PPPPPP;CL")
     SET backupmode = trim(curaccept)
   ENDWHILE
   CALL clear(23,2,77)
   SET tspace_cnt = 0
   IF (backupmode="tspace")
    ...
    SET t_tspace = fillstring(30," ")
    CALL text(18,7,"Tablespace Name:")
    CALL accept(18,27,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CL","                              ")
    SET t_tspace = trim(curaccept)
    WHILE (t_tspace="                              ")
      CALL clear(23,2,77)
      CALL text(23,5,"Tablespace name required.")
    ...
    ENDWHILE
   ENDIF
   CALL display_unix_hotbackup_info(1)
   CALL confirm_message(1)
   IF ((toolkit_req->confirm_ans="Y"))
    SET dcl_hotbackup = fillstring(240," ")
    SET dcl_hotbackup = trim(concat("$cer_mgr/ora_backup.ksh ",trim(domainname)," ",trim(userpswd),
      " ",
      trim(devicetype),"=",trim(t_device)," hot -",trim(backupmode)))
    IF (backupmode="all")
     SET dcl_hotbackup = concat(trim(dcl_hotbackup),";")
    ELSE
     SET x = 1
     SET dcl_hotbackup = concat(trim(dcl_hotbackup)," ",trim(tspace_list[x]))
     SET x = (x+ 1)
     ...
     SET dcl_hotbackup = concat(trim(dcl_hotbackup),";")
    ENDIF
    SELECT INTO concat("/tmp/hot_backup_",trim(domainname),".ksh")
     *
     FROM dummyt
     DETAIL
      col 0, "#!/usr/bin/ksh", row + 1,
      col 0, "su - oracle <<!", row + 1,
      col 0,
      CALL print(trim(dcl_hotbackup)), row + 1,
      col 0, "exit", row + 1,
      col 0, "!"
     WITH nocounter, format = stream, noheading,
      formfeed = none, maxrow = 1
    ;end select
    SET dcl_string0 = concat("chmod 777 /tmp/hot_backup_",trim(domainname),".ksh")
    SET dcl_len0 = size(trim(dcl_string0))
    SET dcl_status0 = 0
    CALL dcl(dcl_string0,dcl_len0,dcl_status0)
    SET dcl_string1 = concat("/tmp/hot_backup_",trim(domainname),".ksh")
    SET dcl_len1 = size(trim(dcl_string1))
    SET dcl_status1 = 0
    CALL clear(1,1)
    CALL text(20,1,trim(dcl_string1))
    CALL pause(3)
    CALL dcl(dcl_string1,dcl_len1,dcl_status1)
    CALL pause(9)
   ENDIF
 END ;Subroutine
 SUBROUTINE display_unix_hotbackup_info(z)
   SET x = 6
   FOR (x = 6 TO 21)
     CALL clear(x,2,78)
   ENDFOR
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE setup_db_connection(v)
   SET validate = off
   CALL display_heading(1)
   CALL text(3,31,"DATABASE CONNECTION")
   SET connect_str = "                              "
   CALL text(10,15,"Enter connecting string: ")
   CALL text(11,15,"                         <username>/<password>@<tns_alias>")
   CALL text(12,15,"                         ex: cdba/cdba@admin1")
   CALL accept(10,40,"PPPPPPPPPPPPPPPPPPPPPPPPPPPPPP;CU","                              ")
   SET connect_str = trim(curaccept)
   IF (connect_str="                              ")
    CALL text(23,05,"Connecting string required. Exit...")
    CALL pause(1)
   ELSE
    SET connect_stat1 = "free define oraclesystem go"
    SET connect_stat2 = concat("define oraclesystem ",'"',nullterm(connect_str),'" go')
    CALL text(23,05,"Please wait...")
    CALL parser(connect_stat1)
    CALL parser(connect_stat2)
    CALL current_database(1)
   ENDIF
 END ;Subroutine
 SUBROUTINE current_database(w)
   SET toolkit_req->current_db = "          "
   SET toolkit_req->current_sid = "          "
   SELECT INTO "nl:"
    a.name
    FROM v$database a
    DETAIL
     toolkit_req->current_db = a.name
    WITH nocounter, format = stream, noheading,
     formfeed = none, maxrow = 1
   ;end select
   SELECT INTO "nl:"
   ...
   IF (curqual=1)
    SELECT INTO "nl:"
     ...
     WHERE status="OPEN"
     DETAIL
      toolkit_req->current_sid = b.instance
     WITH nocounter, format = stream, noheading,
      formfeed = none, maxrow = 1
    ;end select
   ELSE
    SELECT INTO "nl:"
     ...
     FROM v$parameter a,
      v$thread b
     WHERE a.name="thread"
      AND a.value=cnvtstring(b.thread#)
     DETAIL
      toolkit_req->current_sid = b.instance
     WITH nocounter, format = stream, noheading,
      formfeed = none, maxrow = 1
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dis_cerrec(dtm_option)
   IF (dtm_option=1)
    IF (oracle_version=7)
     CALL text(17,8,"Cerner recommends using utlbstat/utlestat to view these reports")
    ELSEIF (oracle_version >= 8)
     CALL text(17,8,"Cerner recommends using Statspack to view these reports")
    ENDIF
   ELSEIF (dtm_option=2)
    CALL text(17,8,"Please review the Cerner Backup/Recover Guide on the CKN")
   ENDIF
   CALL text(18,30,"Press enter to continue:")
   CALL accept(18,60,"P;E"," ")
 END ;Subroutine
 SUBROUTINE confirm_message(y)
   SET toolkit_req->confirm_ans = "N"
   CALL clear(23,5,74)
   CALL text(23,5,"Please Confirm the above information before continue. Continue(Y/N)?")
   CALL accept(23,74,"P;CU","N")
   SET toolkit_req->confirm_ans = curaccept
 END ;Subroutine
#endprogram
 CALL clear(23,2,79)
 CALL text(23,15,"Exit...")
END GO
