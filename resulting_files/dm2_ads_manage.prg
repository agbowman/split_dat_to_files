CREATE PROGRAM dm2_ads_manage
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dam_logfile_prefix = vc WITH protect, constant("dm2_ads_manage")
 DECLARE dam_continue_loop = i2 WITH protect, noconstant(1)
 DECLARE dam_continue_loop2 = i2 WITH protect, noconstant(1)
 DECLARE dam_continue_loop3 = i2 WITH protect, noconstant(1)
 DECLARE dam_option = vc WITH protect, noconstant("R")
 DECLARE dam_options_str = vc WITH protect, noconstant("")
 DECLARE dam_name_in_use = i2 WITH protect, noconstant(0)
 DECLARE dam_selected_ind = i2 WITH protect, noconstant(0)
 DECLARE dam_executing_ind = i2 WITH protect, noconstant(0)
 DECLARE dam_complete_ind = i2 WITH protect, noconstant(0)
 DECLARE dam_cur_ads_name = vc WITH protect, noconstant("")
 DECLARE dam_cur_ads_method = vc WITH protect, noconstant("")
 DECLARE dam_cur_ads_pct = f8 WITH protect, noconstant(1.0)
 DECLARE dam_help_msg = vc WITH protect, noconstant("")
 DECLARE dam_new_dac_id = f8 WITH protect, noconstant(0.0)
 DECLARE dam_rebuild_ind = i2 WITH protect, noconstant(0)
 DECLARE dam_cur_is_status = vc WITH protect, noconstant(" ")
 DECLARE dam_is_paused = i2 WITH protect, noconstant(0)
 DECLARE dam_last_status_time = vc WITH protect, noconstant(" ")
 DECLARE dam_last_status_msg = vc WITH protect, noconstant(" ")
 DECLARE dam_cnt = i2 WITH protect, noconstant(0)
 DECLARE dam_cur_logfile = vc WITH protect, noconstant(" ")
 DECLARE dam_cur_ref_logfile = vc WITH protect, noconstant(" ")
 DECLARE dam_ads_ind = i2 WITH protect, noconstant(0)
 DECLARE dam_default_row_cnt = f8 WITH protect, noconstant(10000.0)
 DECLARE dam_driverkey_menu(null) = i2
 DECLARE dam_childest_menu(null) = i2
 DECLARE dam_export_csv(dec_dir_name,dec_file_name) = i2
 DECLARE dam_import_csv(dic_dir_name,dic_file_name) = i2
 FREE RECORD dam_help_text
 RECORD dam_help_text(
   1 list[3]
     2 name = vc
     2 description = vc
 )
 SET dam_help_text->list[1].name = dpl_recent
 SET dam_help_text->list[1].description = "Most recent %"
 SET dam_help_text->list[2].name = dpl_interval
 SET dam_help_text->list[2].description = "Every Nth %"
 SET dam_help_text->list[3].name = dpl_custom
 SET dam_help_text->list[3].description = "CSV Input"
 SET width = 132
 SET message = nowindow
 IF (check_logfile(dam_logfile_prefix,".log","dm2_ads_manage")=0)
  GO TO exit_program
 ENDIF
 SET dm2_process_rs->process_name = value(dpl_sample)
 EXECUTE dm2_ads_validate_configs
 IF ((dm_err->err_ind > 0))
  GO TO exit_program
 ENDIF
 SET dam_selected_ind = 0
 IF (dar_clear_dads_list(null)=0)
  GO TO exit_program
 ENDIF
 IF (drr_ads_domain_check("",dam_ads_ind)=0)
  GO TO exit_program
 ENDIF
 IF (dam_ads_ind=1)
  SET message = window
  IF (dar_common_menu(2,"MANAGEMENT")=0)
   GO TO exit_program
  ENDIF
  CALL text(10,5,"Warning: This database is already a partial copy of another Source database.")
  CALL text(11,5,"Please consider this fact before using this database as a Source.")
  CALL text(12,5,"Press <Enter> to Continue.")
  CALL accept(12,32,"p;cu"," "
   WHERE curaccept IN (" "))
  SET message = nowindow
 ENDIF
 WHILE (dam_continue_loop=1)
   SET dam_rebuild_ind = 0
   SET message = window
   IF (dar_common_menu(2,"MANAGEMENT")=0)
    GO TO exit_program
   ENDIF
   SET dam_executing_ind = 0
   SET dam_complete_ind = 0
   IF (dam_selected_ind=1)
    IF (dar_sync_config_status(dads_list->config_id)=0)
     GO TO exit_program
    ENDIF
    IF ((((dads_list->config_status=dpl_complete)) OR ((dads_list->config_status=dpl_failed))) )
     SET dam_complete_ind = 1
     SET dam_executing_ind = 0
    ELSEIF ((dads_list->config_status=dpl_executing))
     SET dam_complete_ind = 0
     SET dam_executing_ind = 1
    ENDIF
    CALL text(10,5,build("1. Build Driver Table Keys ",value(evaluate(dam_executing_ind,1,
        "(*Unavailable*)",""))))
    CALL text(11,5,build("2. View Driver Keys Report ",value(evaluate(dam_complete_ind,0,
        "(*Unavailable*)",""))))
    CALL text(12,5,"3. Modify Current Configuration")
    CALL text(13,5,"4. Delete Current Configuration")
    SET dam_options_str = "Your Selection (0 to Exit)? "
   ELSE
    CALL text(10,5,"1. Select Configuration")
    CALL text(11,5,"2. Create New Configuration")
    SET dam_options_str = "Your Selection (0 to Exit)? "
   ENDIF
   CALL text(15,5,dam_options_str)
   CALL text(16,5,"(Enter R to refresh)")
   IF (dam_selected_ind=1)
    CALL accept(15,33,"P;CU","0"
     WHERE curaccept IN ("0", "3", "4", "1", "2",
     "R"))
   ELSE
    CALL accept(15,33,"P;CU","0"
     WHERE curaccept IN ("1", "2", "0", "R"))
   ENDIF
   SET dam_option = curaccept
   CASE (dam_option)
    OF "0":
     SET dam_continue_loop = 0
    OF "1":
     IF (dam_selected_ind=1)
      IF (dam_executing_ind=0)
       SET message = nowindow
       IF (dam_driverkey_menu(null)=0)
        GO TO exit_program
       ENDIF
      ENDIF
      SET dam_continue_loop = 1
     ELSE
      IF (dar_get_ads_config(null)=0)
       GO TO exit_program
      ENDIF
      IF ((dads_list->config_id > 0))
       ...
       ENDIF
       SET dam_continue_loop = 1
       SET dam_selected_ind = 1
       IF ((dads_list->config_status=dpl_executing))
        SET dam_executing_ind = 1
       ELSEIF ((dads_list->config_status=dpl_complete))
        SET dam_complete_ind = 1
       ELSE
        SET dam_complete_ind = 0
        SET dam_executing_ind = 0
    ...
    OF "2":
     ...
     ELSE
      ...
      IF ((dm_err->debug_flag=722))
       SET message = nowindow
      ENDIF
      SET dm_err->eproc = "Retrieve default sample row count from dm_info"
      SELECT INTO "nl:"
       FROM dm_info d
       WHERE d.info_domain="DM2_ADS_DATA"
        AND d.info_name="DEFAULT_ROW_CNT"
       DETAIL
        dam_default_row_cnt = d.info_number
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc))
       GO TO exit_program
      ENDIF
      SET dm_err->eproc = "Retrieve num_rows for person from dm_user_tables_actual_stats"
      SELECT INTO "nl:"
       num_rows_null_ind = nullind(d.num_rows)
       FROM dm_user_tables_actual_stats d
       WHERE d.table_name="PERSON"
       DETAIL
        IF (num_rows_null_ind=1)
         dam_cur_ads_pct = 1.0
        ELSE
         dam_cur_ads_pct = round((100.0 * (dam_default_row_cnt/ d.num_rows)),6)
         IF (dam_cur_ads_pct > 100)
          dam_cur_ads_pct = 100.0
         ENDIF
        ENDIF
       WITH nocounter
      ;end select
      ...
      ENDIF
      IF ((dm_err->debug_flag=722))
       CALL echo(build("dam_cur_ads_pct = ",dam_cur_ads_pct))
       SET message = window
      ENDIF
      IF (dar_common_menu(2,"DEFINE NEW SAMPLE")=0)
       GO TO exit_program
      ENDIF
      SET dam_name_in_use = 0
      SET dam_continue_loop2 = 1
      WHILE (dam_continue_loop2)
        CALL clear(12,2,30)
        IF (dam_name_in_use=1)
         CALL text(12,5,"Sample Name in use. Try again.")
        ELSE
         CALL text(12,5,"Enter Sample Name.")
        ENDIF
        CALL accept(5,18,"P(30);CU"
         WHERE curaccept > " ")
        SET dam_cur_ads_name = curaccept
        SELECT INTO "nl:"
         FROM dm_ads_config dac
         WHERE dac.config_name=dam_cur_ads_name
         WITH nocounter
        ;end select
        IF (check_error(dm_err->eproc))
         GO TO exit_program
        ENDIF
        IF (curqual > 0)
         SET dam_name_in_use = 1
        ELSE
         SET dam_name_in_use = 0
         SET dam_continue_loop2 = 0
        ENDIF
      ENDWHILE
      CALL clear(12,2,30)
      CALL text(12,5,"Enter Sample Type.")
      SET help = pos(10,60,10,60)
      SET help =
      SELECT INTO "nl:"
       type = concat(dam_help_text->list[d.seq].name,"         "), description = dam_help_text->list[
       d.seq].description
       FROM (dummyt d  WITH seq = 3)
       ORDER BY dam_help_text->list[d.seq].name
       WITH nocounter
      ;end select
      CALL accept(6,18,"A(15);CF"
       WHERE curaccept IN (dpl_interval, dpl_recent, dpl_custom))
      SET dam_cur_ads_method = curaccept
      SET help = off
      IF (dam_cur_ads_method != dpl_custom)
       CALL clear(12,2,30)
       CALL text(12,5,"Enter Sample Percent.")
       CALL accept(7,18,"N(9);",format(dam_cur_ads_pct,"##.######")
        WHERE cnvtreal(curaccept) >= 0.0
         AND cnvtreal(curaccept) <= 100.0)
       SET dam_cur_ads_pct = cnvtreal(curaccept)
       SET help = off
      ELSE
       SET dam_cur_ads_pct = 0.0
      ENDIF
      CALL clear(12,2,30)
      CALL text(18,5,"(S)ave, e(X)it:")
      CALL accept(18,22,"p;cu"
       WHERE curaccept IN ("S", "X"))
      CASE (curaccept)
       OF "X":
        SET dam_continue_loop = 1
       OF "S":
        SET dam_rebuild_ind = 1
        CALL text(16,5,
         "Creating New Sample. Driverkeys will need to be built. Press <Enter> to Continue")
        CALL accept(16,104,"p;cu"," "
         WHERE curaccept IN (" "))
        SET message = nowindow
        SET dm_err->eproc = "Getting next seq value"
        SELECT INTO "nl:"
         x = seq(dm_clinical_seq,nextval)
         FROM dual
         DETAIL
          dam_new_dac_id = cnvtreal(x)
         WITH nocounter
        ;end select
        IF (check_error(dm_err->eproc))
         GO TO exit_program
        ENDIF
        SET dm_err->eproc = "Inserting new ads row in dm_ads_config"
        INSERT  FROM dm_ads_config dac
         SET dac.dm_ads_config_id = dam_new_dac_id, dac.config_name = cnvtupper(dam_cur_ads_name),
          dac.sample_method = dam_cur_ads_method,
          dac.sample_percent_nbr = dam_cur_ads_pct, dac.config_status = dpl_needsbuild, dac
          .updt_dt_tm = cnvtdatetime(curdate,curtime3),
          dac.updt_id = 0, dac.updt_task = 0, dac.updt_applctx = 0,
          dac.updt_cnt = 0
         WITH nocounter
        ;end insert
        IF (check_error(dm_err->eproc)=1)
        ...
        ENDIF
        COMMIT
        SET dads_list->config_id = dam_new_dac_id
        IF (dar_get_config_data(dads_list->config_id)=0)
         GO TO exit_program
        ENDIF
        SET dam_selected_ind = 1
     ...
     ENDIF
    OF "3":
     IF (dar_common_menu(2,"MODIFY SAMPLE")=0)
      GO TO exit_program
     ENDIF
     CALL clear(12,2,30)
     CALL text(12,5,"Enter Sample Name.")
     CALL accept(5,18,"P(30);CU",dads_list->config_name)
     SET dam_cur_ads_name = curaccept
     CALL clear(12,2,30)
     CALL text(12,5,"Enter Sample Type.")
     SET dam_help_msg = concat(value(dpl_interval),",",value(dpl_recent),",",value(dpl_custom))
     SET help = pos(10,60,10,60)
     SET help = fix(value(dam_help_msg))
     CALL accept(6,18,"A(15);CF",dads_list->config_method
      WHERE curaccept IN (dpl_interval, dpl_recent, dpl_custom))
     SET dam_cur_ads_method = curaccept
     SET help = off
     IF (dam_cur_ads_method != dpl_custom)
      CALL clear(12,2,30)
      CALL text(12,5,"Enter Sample Percent.")
      CALL text(7,2,"Sample Percent: ")
      CALL accept(7,18,"N(9);",format(dads_list->config_pct,"##.######")
       WHERE cnvtreal(curaccept) >= 0.0
        AND cnvtreal(curaccept) <= 100.0)
      SET dam_cur_ads_pct = cnvtreal(curaccept)
     ENDIF
     CALL clear(12,2,30)
     CALL text(18,5,"(S)ave, e(X)it:")
     CALL accept(18,21,"p;cu"
      WHERE curaccept IN ("S", "X"))
     CASE (curaccept)
      OF "X":
       SET dam_continue_loop = 1
      OF "S":
       IF (((trim(dam_cur_ads_method) != trim(dads_list->config_method)) OR ((dam_cur_ads_pct !=
       dads_list->config_pct))) )
        SET dam_rebuild_ind = 1
        CALL text(16,5,
         "Modifying Sample. Driverkeys will need to be re-built. Press <Enter> to Continue")
        CALL accept(16,104,"p;cu"," "
         WHERE curaccept IN (" "))
       ELSEIF (trim(dam_cur_ads_name) != trim(dads_list->config_name))
        SET dam_rebuild_ind = 0
        UPDATE  FROM dm_ads_config dac
         SET dac.config_name = dam_cur_ads_name
       ...
       ENDIF
       SET message = nowindow
       IF (dam_rebuild_ind=1)
        SET dm_err->eproc = build2("Updating ads status in dm_ads_config for: ",dam_cur_ads_name)
        UPDATE  FROM dm_ads_config dac
         SET dac.config_name = dam_cur_ads_name, dac.sample_method = dam_cur_ads_method, dac
          .sample_percent_nbr = dam_cur_ads_pct,
          dac.config_status = evaluate(dam_rebuild_ind,1,dpl_needsbuild)
         WHERE (dac.dm_ads_config_id=dads_list->config_id)
         WITH nocounter
        ;end update
        IF (check_error(dm_err->eproc)=1)
        ...
        ENDIF
        COMMIT
        SET dm_err->eproc = build2("Deleting ads information from dm_ads_config_driver for: ",
         dam_cur_ads_name)
        SET dam_continue_loop3 = 1
        WHILE (dam_continue_loop3=1)
          DELETE  FROM dm_ads_config_driver dacd
           WHERE (dacd.dm_ads_config_id=dads_list->config_id)
           WITH nocounter, maxqual(dacd,10000)
          ;end delete
          IF (check_error(dm_err->eproc)=1)
           SET message = nowindow
           SET dm_err->err_ind = 1
          ...
          COMMIT
          IF (curqual=0)
           SET dam_continue_loop3 = 0
          ENDIF
        ENDWHILE
        SET dm_err->eproc = build2("Deleting ads information from dm_ads_config_extract for: ",
         dam_cur_ads_name)
        DELETE  FROM dm_ads_config_extract dace
         WHERE (dace.dm_ads_config_id=dads_list->config_id)
         WITH nocounter
        ;end delete
        IF (check_error(dm_err->eproc)=1)
         SET message = nowindow
     ...
     SET dam_continue_loop = 1
    OF "4":
     IF (dar_common_menu(2,"DELETE SAMPLE")=0)
      GO TO exit_program
     ENDIF
     CALL clear(12,2,30)
     CALL clear(13,2,30)
     CALL text(12,5,build("ALL information related to the Sample <",dads_list->config_name,
       "> will be deleted. "))
     CALL text(13,5,"Are you sure?  (Y)es, (N)o ")
     CALL accept(13,32,"A;CU"
      WHERE curaccept IN ("Y", "N"))
     SET message = nowindow
     CASE (curaccept)
      OF "N":
       SET dam_continue_loop = 1
      OF "Y":
       SET dm_err->eproc = build2("Deleting ads information from dm_ads_config _driver for: ",
        dads_list->config_name)
       SET dam_continue_loop3 = 1
       WHILE (dam_continue_loop3=1)
         DELETE  FROM dm_ads_config_driver dacd
          WHERE (dacd.dm_ads_config_id=dads_list->config_id)
       ...
       ENDIF
       COMMIT
       SET dm_err->eproc = build2("Deleting ads information from dm_ads_config for: ",dads_list->
        config_name)
       DELETE  FROM dm_ads_config dac
        WHERE (dac.dm_ads_config_id=dads_list->config_id)
        WITH nocounter
       ;end delete
       IF (check_error(dm_err->eproc)=1)
 ...
 ENDWHILE
 GO TO exit_program
 SUBROUTINE dam_driverkey_menu(null)
   DECLARE ddm_continue_loop = i2 WITH protect, noconstant(1)
   DECLARE ddm_continue_loop2 = i2 WITH protect, noconstant(0)
   DECLARE ddm_csv_dir_name = vc WITH protect, noconstant("")
   DECLARE ddm_csv_file_name = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Driver Table Key Generation Menu"
   WHILE (ddm_continue_loop=1)
     SET message = window
     IF (dar_common_menu(2,"BUILD DRIVER TABLE KEYS")=0)
      RETURN(0)
     ENDIF
     CALL text(10,5,"1. Auto-Build Driver Keys ")
     CALL text(11,5,"2. Import Driver Table Keys via CSV File ")
     CALL text(12,5,build("3. Export Driver Table Keys to CSV File ",value(evaluate(dam_complete_ind,
         0,"(*Unavailable*)",""))))
     CALL text(14,5,"Your selection (0 to exit)? ")
     CALL accept(14,33,"9;",0
      WHERE curaccept IN (0, 1, 2, 3))
     CASE (curaccept)
      OF 0:
       SET ddm_continue_loop = 0
      OF 1:
       IF ((dads_list->config_method=dpl_custom))
        IF (dar_common_menu(2,"AUTO-BUILD DRIVER TABLE KEYS")=0)
         RETURN(0)
        ENDIF
        CALL text(10,5,"Driver keys cannot be auto-generated for Custom Configurations.")
        CALL text(11,5,
         "If you wish to auto-generate driver keys, please modify the sample method to EVERYNTH or RECENT."
         )
        CALL text(12,5,
         "Note: If sample method is changed, then custom driver keys previously imported will be lost."
         )
        CALL text(14,5,"Press <Enter> to go back to the previous menu")
        CALL accept(14,50,"p;cu"," "
         WHERE curaccept IN (" "))
       ELSE
        SET dads_list->preserve_ind = 1
        IF ((dads_list->config_status=dpl_complete))
         IF (dar_common_menu(2,"AUTO-BUILD DRIVER TABLE KEYS")=0)
          RETURN(0)
         ENDIF
         CALL text(10,5,build2("Driver Keys were successfully built on ",format(dads_list->
            config_updt_dt_tm,";;Q")))
         CALL text(11,5,"Do you wish to Rebuild Driver Keys for ALL Driver Tables?")
         CALL text(14,5,"(R)ebuild ALL Driver Keys, e(X)it: ")
         CALL accept(14,41,"p;cu"," "
          WHERE curaccept IN ("R", "X"))
         SET message = nowindow
         CASE (curaccept)
          OF "R":
           SET dads_list->preserve_ind = 0
           EXECUTE dm2_ads_driverkey_gen
           IF ((dm_err->err_ind > 0))
            RETURN(0)
           ENDIF
          OF "B":
           SET ddm_continue_loop = 1
         ENDCASE
        ELSEIF ((dads_list->config_status=dpl_incomplete))
         SET ddm_continue_loop2 = 1
         WHILE (ddm_continue_loop2=1)
           IF (dar_common_menu(2,"AUTO-BUILD DRIVER TABLE KEYS")=0)
            RETURN(0)
           ENDIF
           SET message = window
           CALL text(10,5,
            "New Driver Table data has caused this previously completed Configuration to be marked as INCOMPLETE"
            )
           CALL text(11,5,"Do you wish to Rebuild Driver Keys for ALL Driver Tables? OR")
           CALL text(12,5,"Do you wish to rebuild DriverKeys for only the INCOMPLETE Driver Tables?")
           CALL text(14,5,
            "(A)LL Driver Tables, (I)ncomplete Driver Tables Only, (V)iew Detail Report, e(X)it:")
           CALL accept(14,89,"p;cu"," "
            WHERE curaccept IN ("A", "I", "V", "X"))
           SET message = nowindow
           CASE (curaccept)
            OF "A":
             SET dads_list->preserve_ind = 0
             SET ddm_continue_loop2 = 0
             SET message = nowindow
             EXECUTE dm2_ads_driverkey_gen
             IF ((dm_err->err_ind > 0))
              RETURN(0)
       ...
       ENDIF
      OF 2:
       SET ddm_imp_continue = 1
       WHILE (ddm_imp_continue=1)
         IF (dar_common_menu(2,"IMPORT DRIVER TABLE KEYS")=0)
          RETURN(0)
         ENDIF
         IF ((dads_list->config_method != dpl_custom)
          AND (dads_list->config_status != dpl_complete))
          CALL text(10,5,
           "Driverkey Generation Process MUST be complete in order to perform this action.")
          CALL text(12,5,"Press <Enter> to go back to the previous menu")
          CALL accept(12,50,"p;cu"," "
           WHERE curaccept IN (" "))
         ELSE
          CALL text(10,5,"CSV File format:  TABLE_NAME, DRIVER_KEYCOL_NAME, DRIVER_KEY_ID")
          CALL text(12,5,"Please provide an absolute directory path (i.e. without OS logicals)")
          CALL text(13,5,"Directory:")
          CALL text(14,5,
           "A file with extension *.csv is used for CSV Exporting and is required for CSV Importing")
          CALL text(15,5,"File Name:")
          CALL accept(13,17,"P(80);C",ddm_csv_dir_name
           WHERE curaccept > " ")
          SET ddm_csv_dir_name = curaccept
          CALL accept(15,17,"P(30);C",ddm_csv_file_name
           WHERE curaccept > " ")
          SET ddm_csv_file_name = curaccept
          IF (findstring(".csv",ddm_csv_file_name)=0)
           SET ddm_csv_file_name = concat(ddm_csv_file_name,".csv")
          ENDIF
          CALL text(17,5,"Is this information correct?  (Y)es, (N)o, e(X)it ")
          CALL accept(17,56,"A;CU","X"
           WHERE curaccept IN ("Y", "N", "X"))
         ENDIF
         CASE (curaccept)
          OF "Y":
           SET message = nowindow
           IF (dam_import_csv(ddm_csv_dir_name,ddm_csv_file_name)=0)
            RETURN(0)
           ENDIF
           SET dads_list->preserve_ind = 0
           SET ddm_imp_continue = 0
          OF "N":
           SET ddm_continue_loop = 0
          OF "X":
           SET ddm_imp_continue = 0
           SET ddm_csv_dir_name = ""
           SET ddm_csv_file_name = ""
          ELSE
           SET ddm_imp_continue = 0
         ENDCASE
       ENDWHILE
      OF 3:
       SET ddm_exp_continue = 1
       WHILE (ddm_exp_continue=1)
         IF (dar_common_menu(2,"EXPORT DRIVER TABLE KEYS")=0)
          RETURN(0)
         ENDIF
         IF ((dads_list->config_status != dpl_complete))
          CALL text(10,5,
         ...
         CASE (curaccept)
          OF "Y":
           SET message = nowindow
           IF (dam_export_csv(ddm_csv_dir_name,ddm_csv_file_name)=0)
            RETURN(0)
           ENDIF
           SET ddm_exp_continue = 0
          OF "N":
           SET ddm_continue_loop = 0
          OF "X":
           SET ddm_exp_continue = 0
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dam_export_csv(dec_dir_name,dec_file_name)
   DECLARE dec_file = vc WITH protect, noconstant("")
   DECLARE dec_str = vc WITH protect, noconstant("")
   DECLARE dec_cnt = i4 WITH protect, noconstant(0)
   FREE RECORD dec_driverkeys
   RECORD dec_driverkeys(
     01 cnt = i4
     01 qual[*]
       02 table_name = vc
       02 driver_keycol_name = vc
   ...
   SET dm_err->eproc = "Exporting driver keys to a CSV File"
   CALL disp_msg("",dm_err->logfile,0)
   SET dm_err->eproc = "Retrieving list of driver keys from dm_ads_config_driver"
   SELECT INTO "nl:"
    FROM dm_ads_config_driver dacd,
     dm_ads_extract dae
    PLAN (dacd
     WHERE (dacd.dm_ads_config_id=dads_list->config_id))
     JOIN (dae
     WHERE dacd.dm_ads_extract_id=dae.dm_ads_extract_id)
    ORDER BY dae.table_name
    HEAD REPORT
     dec_driverkeys->cnt = 0, stat = alterlist(dec_driverkeys->qual,0)
    DETAIL
     dec_driverkeys->cnt = (dec_driverkeys->cnt+ 1), stat = alterlist(dec_driverkeys->qual,
      dec_driverkeys->cnt), dec_driverkeys->qual[dec_driverkeys->cnt].table_name = trim(dae
      .table_name),
     dec_driverkeys->qual[dec_driverkeys->cnt].driver_keycol_name = trim(dae.driver_keycol_name),
     dec_driverkeys->qual[dec_driverkeys->cnt].driver_key_id = build(cnvtstring(dacd.driver_key_id))
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
    ...
    RETURN(0)
   ENDIF
   FOR (dec_cnt = 1 TO dec_driverkeys->cnt)
     SET dm2scramble->mode_ind = 0
     SET dm2scramble->in_text = dec_driverkeys->qual[dec_cnt].table_name
     IF (ds_scramble(null)=0)
      RETURN(0)
     ENDIF
     SET dec_driverkeys->qual[dec_cnt].table_name = dm2scramble->out_text
     SET dm2scramble->out_text = ""
     SET dm2scramble->in_text = dec_driverkeys->qual[dec_cnt].driver_keycol_name
     IF (ds_scramble(null)=0)
      RETURN(0)
     ENDIF
     SET dec_driverkeys->qual[dec_cnt].driver_keycol_name = dm2scramble->out_text
     SET dm2scramble->out_text = ""
   ENDFOR
   SET dm_err->eproc = concat("Generating ",dec_file_name," into ",dec_dir_name)
   CALL disp_msg("",dm_err->logfile,0)
   SET dm_err->eproc = concat("Removing ",dec_file_name)
   SET dec_file = concat(dec_dir_name,evaluate(cursys2,"AXP",concat(dec_file_name,";*"),concat("/",
      dec_file_name)))
   IF (dm2_findfile(dec_file) > 0)
    IF (dm2_push_dcl(concat(evaluate(cursys2,"AXP","del","rm")," ",dec_file))=0)
     RETURN(0)
    ENDIF
   ENDIF
   SET dec_file = concat(dec_dir_name,evaluate(cursys2,"AXP",dec_file_name,concat("/",dec_file_name))
    )
   SET dm_err->eproc = concat("Creating ",dec_file_name," in ",dec_dir_name)
   SELECT INTO value(dec_file)
    FROM dummyt d
    HEAD REPORT
     dec_str = "table_name,driver_keycol_name,driver_key_id", col 0, dec_str,
     row + 1
    DETAIL
     FOR (dec_cnt = 1 TO dec_driverkeys->cnt)
       dec_str = build(dec_driverkeys->qual[dec_cnt].table_name,",",dec_driverkeys->qual[dec_cnt].
        driver_keycol_name,",",dec_driverkeys->qual[dec_cnt].driver_key_id), col 0, dec_str,
       row + 1
     ENDFOR
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dam_import_csv(dic_dir_name,dic_file_name)
   DECLARE dic_file = vc WITH protect, noconstant("")
   DECLARE dic_str = vc WITH protect, noconstant("")
   DECLARE dic_cnt = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "importing driver keys to a CSV File"
   CALL disp_msg("",dm_err->logfile,0)
   IF ((dm2_sys_misc->cur_os != "AXP"))
    SET dic_dir_name = concat(dic_dir_name,"/")
   ENDIF
   SET dm_err->eproc = "Checking if the CSV File exists"
   SET dic_file = concat(dic_dir_name,evaluate(cursys2,"AXP",concat(dic_file_name,";*"),concat("/",
      dic_file_name)))
   IF (dm2_findfile(dic_file)=0)
    SET dm_err->emsg = build("CSV File for Driver Key list not found (",dic_file,")")
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   UPDATE  FROM dm_info d
    SET d.info_name = dic_file
    WHERE d.info_domain="ADS DPL_CUSTOM CSV NAME"
    WITH nocounter
   ;end update
   IF (curqual=0)
    INSERT  FROM dm_info d
     SET d.info_domain = "ADS DPL_CUSTOM CSV NAME", d.info_name = dic_file
     WITH nocounter
    ;end insert
   ENDIF
   SET dic_file = concat(dic_dir_name,evaluate(cursys2,"AXP",dic_file_name,concat("/",dic_file_name))
    )
   EXECUTE dm2_dbimport dic_file, "dm2_ads_driverkey_imp", 1000
   IF ((dm_err->err_ind=1))
    RETURN(0)
   ENDIF
   RETURN(1)
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "LEAVING dm2_ads_manage"
 CALL final_disp_msg(dam_logfile_prefix)
END GO
