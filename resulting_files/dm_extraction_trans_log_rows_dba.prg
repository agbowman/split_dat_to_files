CREATE PROGRAM dm_extraction_trans_log_rows:dba
 ...
 END ;Subroutine
 DECLARE min_days_to_keep = i4 WITH constant(3), protect
 DECLARE status_completed = i2 WITH constant(1), protect
 DECLARE status_failed = i2 WITH constant(2), protect
 SET reply->status_data.status = "F"
 SET reply->table_name = "EXTRACTION_TRANS_LOG"
 ...
 FOR (v_token_idx = 1 TO size(request->tokens,5))
   IF (trim(request->tokens[v_token_idx].token_str)="DAYSTOKEEP")
 ...
 IF (v_days_to_keep < min_days_to_keep)
  ...
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"BAD_DAYSTOKEEP",
 ...
 SELECT INTO "nl:"
  etl.rowid
  FROM extraction_trans_log etl
  WHERE etl.end_dt_tm IS NOT null
   AND etl.status_flag IN (status_completed, status_failed)
   AND parser(sbr_getrowidnotexists(
    "etl.updt_dt_tm < cnvtdatetime(curdate - v_days_to_keep, curtime3)","etl"))
   AND ((etl.extraction_trans_log_id+ 0) > 0)
  DETAIL
   v_rows += 1
   IF (mod(v_rows,100)=1)
    stat = alterlist(reply->rows,(v_rows+ 99))
   ENDIF
   reply->rows[v_rows].row_id = etl.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,v_rows)
  WITH nocounter, maxqual(etl,value(request->max_rows))
 ;end select
 SET v_errcode2 = error(v_errmsg2,1)
 IF (v_errcode2 != 0)
  SET reply->err_code = v_errcode2
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"ERROR_RETRIEVING_ROWS",
