CREATE PROGRAM cv_frpt_proc_performed_drv:dba
 PROMPT
 ...
 SELECT
  IF (( $ORG_ID=0.0)
   ...
   FROM cv_proc c
   PLAN (c
    WHERE c.proc_status_cd=signed_var
  ...
  DETAIL
   ...
   reply_obj->cv_list[nlistcnt].rpl_action_dt_tm = c.action_dt_tm, audit_record->qual[nlistcnt].
   order_id = c.order_id
  FOOT REPORT
...
#exit_script
 CALL cv_log_msg_post("MOD 007 16/08/2018 RT050705")
END GO
