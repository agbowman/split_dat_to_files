CREATE PROGRAM dm_him_tsk_activity_hst_rows:dba
 ...
 SET reply->table_name = "TASK_ACTIVITY_HISTORY"
 ...
 FOR (tok_ndx = 1 TO size(request->tokens,5))
   ...
   ENDIF
 ENDFOR
 IF (v_days_to_keep < 365)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"KEEPDAYS",
   "You must keep at least 365 days' (1 year's) worth of data.  You entered %1 days or did not enter any value.",
   "i",v_days_to_keep)
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(tah.task_activity_history_id)
   FROM task_activity_history tah
   WHERE tah.task_activity_history_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(tah.task_activity_history_id)
  FROM task_activity_history tah
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    tah.rowid
    FROM task_activity_history tah
    WHERE parser(sbr_getrowidnotexists("tah.task_activity_history_id between curMinID and curMaxID",
      "tah"))
     AND tah.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = tah.rowid
    WITH nocounter, maxqual(tah,value(rowsleft))
   ;end select
