CREATE PROGRAM dm_pucb_pregnancy_instance:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PUCB_PREGNANCY_INSTANCE"
 ...
 SUBROUTINE cust_ucb_upt(null)
  UPDATE  FROM pregnancy_instance p1
   ...
   (SELECT
    ...
    WHERE (p2.pregnancy_instance_id=rchildren->qual1[det_cnt].entity_id)))
   WITH nocounter
  ;end update
 ...
 SUBROUTINE cust_ucb_del2(null)
   UPDATE  FROM pregnancy_instance p1
    ...
    WHERE (p1.pregnancy_instance_id=rchildren->qual1[det_cnt].entity_id)
    WITH nocounter
   ;end update
   IF (curqual=0)
