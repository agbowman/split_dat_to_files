CREATE PROGRAM dm2_find_sequence_match_c1:dba
 ...
 FOR (col_loop_cnt = 1 TO dfsm_col_rec->col_cnt)
   IF ((dfsm_col_rec->col_qual[col_loop_cnt].data_type != "LONG")
    ...
    ENDIF
    IF (dfsm_c1_chk_col_cnt=4)
     SET fsm_sql->sql_lst[dfsm_c1_chk_col_cnt].sql_stmt = concat(" AND t1.",dfsm_c1_col_name," = t2.",
      dfsm_c1_col_name)
    ELSE
 ...
 SUBROUTINE dm2_sm_get_cols(dsgc_tbl_name)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     ->col_cnt].nullable = uttgt.nullable
    FOOT REPORT
