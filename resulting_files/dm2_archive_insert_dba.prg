CREATE PROGRAM dm2_archive_insert:dba
 ...
 SUBROUTINE update_time_window(null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE v_stmt_arc_ins = vc
 CALL parser("rdb ",0)
 FOR (cnt1 = 1 TO request->temp_size)
   SET v_stmt_arc_ins = arc_replace(pers_arc->statements[request->s_ndx].archive_insert[cnt1].stmt,
    pers_arc->statements[request->s_ndx].archive_insert[cnt1].link_ind,pers_arc->statements[request->
    s_ndx].archive_insert[cnt1].list_ind,pers_arc->statements[request->s_ndx].archive_insert[cnt1].
   ...
   IF ((cnt1=request->temp_size))
    CALL parser(v_stmt_arc_ins,1)
   ELSE
    CALL parser(v_stmt_arc_ins,0)
   ENDIF
   IF ((debug_mover->debug_level > 1))
    CALL echo(v_stmt_arc_ins)
   ENDIF
 ENDFOR
