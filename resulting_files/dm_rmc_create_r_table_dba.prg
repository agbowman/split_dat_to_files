CREATE PROGRAM dm_rmc_create_r_table:dba
 ...
 SUBROUTINE case_insensitive_replace(i_orig_str,i_find_str,i_new_str)
   ...
   RETURN(cir_ret_str)
 END ;Subroutine
 DECLARE index_create(i_table_name=vc,i_r_table_name=vc,i_rs_list=vc(ref)) = vc
 SUBROUTINE index_create(i_table_name,i_r_table_name,i_rs_list)
   DECLARE ic_pos = i4
   DECLARE ic_idx = i4
   DECLARE ic_index_str = vc
   DECLARE ic_parm_list = vc WITH protect, noconstant(" ")
   DECLARE ic_parm_str = vc WITH protect, noconstant(" ")
   DECLARE ic_pk_loop = i4
   DECLARE ic_xie1_str = vc
   DECLARE ic_ts_name = vc
   DECLARE ic_nxt_ext = f8
   DECLARE ic_init_ext = f8
   DECLARE ic_xpk_str = vc
   DECLARE ic_nxt_ext_str = vc
   DECLARE ic_md_ind = i2
   DECLARE ic_table_suffix = vc WITH protect, noconstant("")
   DECLARE ic_xpk_old = vc WITH protect, noconstant("")
   DECLARE ic_xpk_drop_str = vc WITH protect, noconstant("")
   DECLARE ic_ret = i4 WITH protect, noconstant(0)
   DECLARE ic_func_ind = i2 WITH protect, noconstant(0)
   DECLARE ic_xpk_func = vc WITH protect, noconstant(" ")
   DECLARE ic_func_str = vc WITH protect, noconstant(" ")
   DECLARE ic_num = i4 WITH protect, noconstant(0)
   FREE RECORD ic_md_col
   RECORD ic_md_col(
     1 cnt = i4
     1 qual[*]
       2 col_name = vc
   )
   SET ic_pos = locateval(ic_idx,1,i_rs_list->rs_tbl_cnt,i_table_name,i_rs_list->dtd_hold[ic_idx].
    tbl_name)
   IF (ic_pos=0)
    RETURN("Z")
   ENDIF
   SET ic_ret = dm2_get_rdbms_version(null)
   IF (ic_ret=0)
    RETURN("F")
   ENDIF
   IF ((dm2_rdbms_version->level1 > 11))
    SET ic_func_ind = 1
   ELSEIF ((dm2_rdbms_version->level1=11)
    AND (dm2_rdbms_version->level2 >= 2))
    SET ic_func_ind = 1
   ENDIF
   SET dm_err->eproc = concat("Checking to see if ",i_table_name," is merge-delete.")
   CALL disp_msg(" ",dm_err->logfile,0)
   SET ic_md_ind = 0
   SELECT INTO "nl:"
    FROM dm_tables_doc_local dtl,
     dm_columns_doc_local dcl
    WHERE (dtl.table_name=i_rs_list->dtd_hold[ic_pos].tbl_name)
     AND dtl.merge_delete_ind=1
     AND dcl.table_name=dtl.table_name
     AND dcl.merge_delete_ind=1
     AND  EXISTS (
    (SELECT
     "x"
     FROM user_tab_columns utc
     WHERE utc.table_name=dcl.table_name
      AND utc.column_name=dcl.column_name))
    HEAD REPORT
     ic_md_col->cnt = 0
    DETAIL
     ic_md_col->cnt = (ic_md_col->cnt+ 1), stat = alterlist(ic_md_col->qual,ic_md_col->cnt),
     ic_md_col->qual[ic_md_col->cnt].col_name = dcl.column_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   IF (curqual > 0)
    SET ic_md_ind = 1
   ENDIF
   SET dm_err->eproc = concat("Getting table suffix from dm_tables_doc")
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_tables_doc_local dtd
    WHERE dtd.table_name=i_table_name
     AND dtd.table_name=dtd.full_table_name
    DETAIL
     ic_table_suffix = dtd.table_suffix
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   IF (ic_func_ind=0)
    IF (ic_md_ind=1)
     FOR (ic_pk_loop = 1 TO size(i_rs_list->dtd_hold[ic_pos].pk_hold,5))
       SET ic_index_str = concat(ic_index_str,
        "decode(sign(rdds_status_flag-9000),-1,decode(rdds_delete_ind,0,",trim(i_rs_list->dtd_hold[
         ic_pos].pk_hold[ic_pk_loop].pk_name),",NULL),NULL),")
     ENDFOR
     FOR (ic_pk_loop = 1 TO ic_md_col->cnt)
       SET ic_index_str = concat(ic_index_str,
        "decode(sign(rdds_status_flag-9000),-1,decode(rdds_delete_ind,1,",trim(ic_md_col->qual[
         ic_pk_loop].col_name),",NULL),NULL),")
     ENDFOR
    ELSE
     FOR (ic_pk_loop = 1 TO size(i_rs_list->dtd_hold[ic_pos].pk_hold,5))
       SET ic_index_str = concat(ic_index_str,"decode(sign(rdds_status_flag-9000),-1,",trim(i_rs_list
         ->dtd_hold[ic_pos].pk_hold[ic_pk_loop].pk_name),",NULL),")
     ENDFOR
    ENDIF
    SET ic_index_str = concat(ic_index_str,"decode(sign(rdds_status_flag-9000),-1,0,NULL)")
   ELSE
    SET ic_index_str = "decode(sign(i_rdds_status_flag-9000),1,NULL,"
    IF (ic_md_ind=1)
     FOR (ic_pk_loop = 1 TO size(i_rs_list->dtd_hold[ic_pos].pk_hold,5))
       SET ic_index_str = concat(ic_index_str,
        "decode(sign(i_rdds_status_flag-9000),-1,decode(i_rdds_delete_ind,0,to_char(i_",trim(
         substring(1,28,i_rs_list->dtd_hold[ic_pos].pk_hold[ic_pk_loop].pk_name)),
        "),'NULL'),'NULL')|| chr(6) ||")
       SET ic_parm_list = concat(ic_parm_list,trim(i_rs_list->dtd_hold[ic_pos].pk_hold[ic_pk_loop].
         pk_name),",")
       SET ic_parm_str = concat(ic_parm_str,"i_",trim(substring(1,28,i_rs_list->dtd_hold[ic_pos].
          pk_hold[ic_pk_loop].pk_name))," ",i_r_table_name,
        ".",trim(i_rs_list->dtd_hold[ic_pos].pk_hold[ic_pk_loop].pk_name),"%type,")
     ENDFOR
     FOR (ic_pk_loop = 1 TO ic_md_col->cnt)
       SET ic_index_str = concat(ic_index_str,
        "decode(sign(i_rdds_status_flag-9000),-1,decode(i_rdds_delete_ind,1,to_char(i_",trim(
         substring(1,28,ic_md_col->qual[ic_pk_loop].col_name)),"),'NULL'),'NULL')|| chr(6) ||")
       SET ic_num = locateval(ic_num,1,size(i_rs_list->dtd_hold[ic_pos].pk_hold,5),ic_md_col->qual[
        ic_pk_loop].col_name,i_rs_list->dtd_hold[ic_pos].pk_hold[ic_num].pk_name)
       IF (ic_num=0)
        SET ic_parm_list = concat(ic_parm_list,trim(ic_md_col->qual[ic_pk_loop].col_name),",")
        SET ic_parm_str = concat(ic_parm_str,"i_",trim(substring(1,28,ic_md_col->qual[ic_pk_loop].
           col_name))," ",i_r_table_name,
         ".",trim(ic_md_col->qual[ic_pk_loop].col_name),"%type,")
       ENDIF
     ENDFOR
    ELSE
     FOR (ic_pk_loop = 1 TO size(i_rs_list->dtd_hold[ic_pos].pk_hold,5))
       SET ic_index_str = concat(ic_index_str,"decode(sign(i_rdds_status_flag-9000),-1,to_char(i_",
        trim(substring(1,28,i_rs_list->dtd_hold[ic_pos].pk_hold[ic_pk_loop].pk_name)),
        "),'NULL')||chr(6)||")
       SET ic_parm_list = concat(ic_parm_list,trim(i_rs_list->dtd_hold[ic_pos].pk_hold[ic_pk_loop].
         pk_name),",")
       SET ic_parm_str = concat(ic_parm_str,"i_",trim(substring(1,28,i_rs_list->dtd_hold[ic_pos].
     ...
     ENDFOR
    ENDIF
    SET ic_index_str = concat(ic_index_str,"decode(sign(i_rdds_status_flag-9000),-1,'0','NULL'))")
    SET ic_parm_list = concat(ic_parm_list,"rdds_delete_ind, rdds_status_flag")
    SET ic_parm_str = concat(ic_parm_str,"i_rdds_delete_ind number, i_rdds_status_flag number")
   ENDIF
   SET ic_xie1_str = concat("XIE1_$R_",ic_table_suffix)
   SET ic_xpk_str = concat("XPK_$R_",ic_table_suffix)
   SET ic_xpk_old = concat("XPK_$R_",trim(substring(1,20,i_table_name)))
   SET ic_xpk_func = concat("REFCHG_XPK$R_",ic_table_suffix)
   SET dm_err->eproc = concat("Obtaining Tablespace and extents for ",ic_xpk_str)
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM user_segments us
    WHERE us.segment_name=ic_xie1_str
     AND us.segment_type="INDEX"
    DETAIL
     ic_ts_name = us.tablespace_name, ic_nxt_ext = us.next_extent, ic_init_ext = us.initial_extent
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   IF (curqual=0)
    SELECT INTO "NL:"
     FROM user_indexes ui
     WHERE ui.index_name=ic_xie1_str
     DETAIL
      ic_ts_name = ui.tablespace_name, ic_nxt_ext = ui.next_extent, ic_init_ext = ui.initial_extent
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    ...
    ENDIF
    IF (curqual=0)
     SET dm_err->emsg = concat(ic_xie1_str," index was not found in user_segments")
     RETURN("F")
    ENDIF
   ENDIF
   SET dm_err->eproc = concat("Checking to see if ",ic_xpk_str," already exists.")
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM user_indexes ui
    WHERE ui.index_name IN (ic_xpk_str, ic_xpk_old)
     AND ui.table_name=i_r_table_name
    DETAIL
     ic_xpk_drop_str = ui.index_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ENDIF
   IF (curqual != 0)
    CALL parser(concat("rdb drop index ",trim(ic_xpk_drop_str)," go"))
   ENDIF
   IF (ic_nxt_ext=0)
    SET ic_nxt_ext_str = ""
   ELSE
    SET ic_nxt_ext_str = concat(" NEXT ",trim(cnvtstring(ic_nxt_ext)))
   ENDIF
   IF (ic_func_ind=0)
    SET dm_err->eproc = concat("Creating ",ic_xpk_str," index.")
    CALL disp_msg(" ",dm_err->logfile,0)
    CALL parser(concat("rdb CREATE UNIQUE INDEX ",ic_xpk_str," ON ",trim(i_r_table_name)),0)
    CALL parser(concat(" ( ",ic_index_str),0)
    CALL parser(concat(" ) LOGGING STORAGE(INITIAL ",build(ic_init_ext),ic_nxt_ext_str),0)
    CALL parser(concat(" ) TABLESPACE ",ic_ts_name," go "),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN("F")
    ENDIF
   ELSE
    SET ic_func_str = concat("create or replace function ",ic_xpk_func,"(",ic_parm_str,")",
     char(10),"  return varchar2",char(10)," deterministic",char(10),
     "  as",char(10),"unq_str varchar2(4000);",char(10),"begin",
     char(10),"select ",ic_index_str," into unq_str from dual;",char(10),
     "return unq_str;",char(10),"end ",ic_xpk_func,";")
    CALL parser(concat("rdb asis(^",ic_func_str,"^) go"),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN("F")
    ENDIF
    CALL parser(concat("rdb CREATE UNIQUE INDEX ",ic_xpk_str," ON ",trim(i_r_table_name)),0)
    CALL parser(concat(" ( ",ic_xpk_func,"(",ic_parm_list,")"),0)
    CALL parser(concat(" ) LOGGING STORAGE(INITIAL ",build(ic_init_ext),ic_nxt_ext_str),0)
    CALL parser(concat(" ) TABLESPACE ",ic_ts_name," go "),1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 IF (validate(dguc_reply->rs_tbl_cnt,0)=0)
  ...
  ENDIF
 ENDIF
 IF (check_logfile("DM_RMC_CREATE_R_TAB",".log","DM_RMC_CREATE_R_TABLE LogFile")=0)
  SET reply->status_data.status = "F"
  GO TO end_program
 ENDIF
 DECLARE v_extent_size = f8
 DECLARE v_$r_tab_name = vc
 DECLARE v_table_suffix = vc
 DECLARE v_tbl_cnt = i4
 DECLARE v_col_cnt = i4
 DECLARE v_ind_cnt = i4
 DECLARE v_ind_col_cnt = i4
 DECLARE v_appl_id = vc
 DECLARE v_pause_cnt = i2
 DECLARE v_continue_cnt = i2
 DECLARE v_create_row_ind = i2
 DECLARE v_db_name = vc
 DECLARE v_next_seq = f8
 DECLARE v_text = vc
 DECLARE for_cnt = i4 WITH protect
 DECLARE v_idx_return = vc
 DECLARE val_cnt = i4
 DECLARE drcrt_file_prefix = vc
 DECLARE drcrt_pre_lock_ind = i2 WITH protect, noconstant(0)
 IF (validate(drcrt_precreate_ind,4)=4
  AND validate(drcrt_precreate_ind,3)=3)
  DECLARE drcrt_precreate_ind = i2
  SET drcrt_precreate_ind = 0
 ENDIF
 SET reply->status_data.status = "S"
 SET reply->status_data.logfile = dm_err->logfile
 SELECT INTO "nl:"
  FROM v$database v
  DETAIL
   v_db_name = v.name
  WITH nocounter
 ;end select
 IF (check_error("Retrieving current db name from v$database") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET reply->status_data.status = "F"
  GO TO end_program
 ENDIF
 IF ((v_db_name != request->database_name))
  SET dm_err->eproc = concat("Calling database ",request->database_name,
   " does not match current database ",v_db_name)
  CALL disp_msg("",dm_err->logfile,1)
  SET reply->status_data.status = "F"
  GO TO end_program
 ENDIF
 SET dm_err->eproc = concat("Creating $R version of ",request->table_name)
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
 ...
 SELECT INTO "nl:"
  ...
  WHERE (dtd.table_name=request->table_name)
   AND dtd.table_name=dtd.full_table_name
  DETAIL
   v_table_suffix = dtd.table_suffix
  WITH nocounter
 ;end select
 IF (check_error("Retrieving table_suffix from dm_tables_doc") != 0)
  ...
  GO TO end_program
 ELSEIF (curqual=0)
  SET dm_err->emsg = "Table not found in dm_tables_doc, cannot create $R table."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET reply->status_data.status = "F"
  GO TO end_program
 ENDIF
 SET v_$r_tab_name = cutover_tab_name(request->table_name,v_table_suffix)
 SET drcrt_file_prefix = concat("dm2_rdds_",trim(cnvtlower(v_table_suffix)))
 SET dm_err->eproc = concat("Creating table ",v_$r_tab_name)
 CALL disp_msg("",dm_err->logfile,0)
 SET v_create_row_ind = 0
 CALL check_lock("RDDS $R CREATION",v_$r_tab_name,drl_reply)
 IF ((drl_reply->status="S"))
  SET drcrt_pre_lock_ind = 1
  SET v_create_row_ind = 1
 ENDIF
 SET drl_reply->status = ""
 SET drl_reply->status_msg = ""
 IF (drcrt_pre_lock_ind=0)
  SET v_continue_cnt = 0
  FOR (v_continue_cnt = 1 TO 5)
   CALL get_lock("RDDS $R CREATION",v_$r_tab_name,0,drl_reply)
   ...
   ELSE
    ...
    SELECT INTO "nl:"
     FROM user_tables ut
     WHERE ut.table_name=v_$r_tab_name
     WITH nocounter
    ;end select
    IF (check_error("Checking if table exists in User_tables ") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET reply->status_data.status = "F"
     GO TO end_program
    ENDIF
    IF (curqual > 0)
     SELECT INTO "nl:"
      FROM dtable d
      WHERE d.table_name=v_$r_tab_name
      WITH nocounter
     ;end select
     IF (check_error("Checking if table exists in Dtable ") != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET reply->status_data.status = "F"
      GO TO end_program
     ENDIF
 ...
 IF ((reply->status_data.status="F"))
  ...
  GO TO end_program
 ENDIF
 IF (v_create_row_ind=0)
  GO TO add_triggers
 ENDIF
 SET dm_err->eproc = "Executing DM2_INSTALL_SCHEMA..."
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_install_schema "DDL GEN", drcrt_file_prefix, v_db_name,
 request->table_name, "NONE", "NONE",
 "NONE"
 IF (check_error("Executing dm2_install_schema...") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET reply->status_data.status = "F"
  GO TO end_program
 ENDIF
 INSERT  FROM dm_info d
  (d.info_domain, d.info_name, d.updt_dt_tm)(SELECT
   d1.info_domain, replace(d1.info_name,concat(request->table_name,"-"),concat(v_$r_tab_name,"-")),
   cnvtdatetime(curdate,curtime3)
   FROM dm_info d1
   WHERE d1.info_domain="DM2_NBR_TO_FLOAT"
    AND d1.info_domain_id=0.0
    AND d1.info_name=patstring(concat(request->table_name,"-*"))
    AND  NOT ( EXISTS (
   (SELECT
    "x"
    FROM dm_info i
    WHERE i.info_domain="DM2_NBR_TO_FLOAT"
     AND i.info_domain_id=d1.info_domain_id
     AND i.info_name=replace(d1.info_name,concat(request->table_name,"-"),concat(v_$r_tab_name,"-")))
   )))
  WITH nocounter
 ;end insert
 IF (check_error("Copying NBR_TO_FLOAT rows...") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET reply->status_data.status = "F"
  GO TO end_program
 ENDIF
 IF (drcrt_precreate_ind=0)
  SET dm_err->eproc = concat("Executing oragen3 on.. ",v_$r_tab_name)
  CALL disp_msg(" ",dm_err->logfile,0)
  EXECUTE oragen3 v_$r_tab_name
  IF (check_error(concat("Executing oragen3 on ",v_$r_tab_name)) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET reply->status_data.status = "F"
   GO TO end_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Creating XPK_$R index"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET val_cnt = 0
 IF (locateval(val_cnt,1,dguc_reply->rs_tbl_cnt,request->table_name,dguc_reply->dtd_hold[val_cnt].
  tbl_name)=0)
  SET dguc_request->what_tables = request->table_name
  SET dguc_request->is_ref_ind = 1
  SET dguc_request->is_mrg_ind = 0
  SET dguc_request->only_special_ind = 0
  SET dguc_request->local_tables_ind = drcrt_precreate_ind
  ...
  ENDIF
 ENDIF
 SET v_idx_return = index_create(request->table_name,v_$r_tab_name,dguc_reply)
 IF (v_idx_return="F")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error returned from index_create(), view log_file for details."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_program
 ENDIF
#add_triggers
 IF ((request->trig_ind != 99))
  SET dm_err->eproc = "Creating RMC triggers "
  CALL disp_msg(" ",dm_err->logfile,0)
  CALL add_cutover_trg(request->table_name)
 ...
 IF (v_create_row_ind=1)
  SELECT INTO "NL:"
   ...
   WITH nocounter
  ;end select
  IF (check_error("Popping a new value from sequence") != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET reply->status_data.status = "F"
  ENDIF
  UPDATE  FROM dm_chg_log_audit
   SET log_id = 0, table_name = request->table_name, action = "RTBLCREATE",
    text = v_text, updt_applctx = cnvtreal(currdbhandle), updt_cnt = 0,
    updt_dt_tm = sysdate, updt_id = reqinfo->updt_id, updt_task = reqinfo->updt_task,
    audit_dt_tm = sysdate
   WHERE dm_chg_log_audit_id=v_next_seq
  ;end update
  IF (curqual=0)
   INSERT  FROM dm_chg_log_audit
    SET dm_chg_log_audit_id = v_next_seq, log_id = 0, table_name = request->table_name,
     action = "RTBLCREATE", text = v_text, updt_applctx = cnvtreal(currdbhandle),
     updt_cnt = 0, updt_dt_tm = sysdate, updt_id = reqinfo->updt_id,
     updt_task = reqinfo->updt_task, audit_dt_tm = sysdate
    WITH nocounter
   ;end insert
  ENDIF
  ...
  SET drl_reply->status_msg = ""
  IF (drcrt_pre_lock_ind=0)
   CALL remove_lock("RDDS $R CREATION",v_$r_tab_name,currdbhandle,drl_reply)
   IF ((drl_reply->status="F"))
    CALL disp_msg(drl_reply->status_msg,dm_err->logfile,1)
    SET reply->status_data.status = "F"
   ENDIF
 ...
 ENDIF
 IF ((reply->status_data.status="F"))
  SET v_text = concat("Failed creating $R version for ",request->table_name," ",dm_err->emsg)
 ELSE
  SET reply->status_data.status = "S"
  SET v_text = concat("Succeeded creating $R version for ",request->table_name)
 ENDIF
 SET dm_err->eproc = "...Ending dm_rmc_create_r_table"
 CALL final_disp_msg("DM_RMC_CREATE_R_TABLE")
END GO
