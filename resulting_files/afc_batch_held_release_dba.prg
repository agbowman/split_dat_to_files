CREATE PROGRAM afc_batch_held_release:dba
 IF ("Z"=validate(afc_batch_held_release_vrsn,"Z"))
  DECLARE afc_batch_held_release_vrsn = vc WITH noconstant("*708659.010")
 ENDIF
 SET afc_batch_held_release_vrsn = "708659.010"
 ...
 DECLARE nochargetype = f8 WITH protect, constant(uar_get_code_by("MEANING",13028,"NO CHARGE"))
 ...
 SET cnt = 1
 SET stat = uar_get_meaning_by_codeset(code_set,nullterm(cdf_meaning),cnt,orders_complete)
 CALL echo(build("the complete code is : ",orders_complete))
 SET held_flg = 3
 RECORD reply(
   1 status_data
     2 status = c1
 ...
 IF ((validate(ens_rel_request->charge_qual,- (1))=- (1)))
  RECORD ens_rel_request(
    1 charge_qual = i2
    1 charge[*]
      2 charge_item_id = f8
      2 charge_desc = vc
 ...
 ;end select
 IF (curqual > 0)
  CALL getheldchargesoforggroup(org_group_code_value,chargecount)
 ELSE
  CALL getheldcharges(chargecount)
 ENDIF
 IF (curqual != 0)
  CALL echo("Executing afc_ens_release_charge to release qualifying charges...")
  EXECUTE afc_ens_release_charge  WITH replace("REQUEST",ens_rel_request)
  IF (reqinfo->commit_ind)
   COMMIT
  ENDIF
  SET reply->status_data.status = "S"
  IF ((request->batch_selection="1"))
   EXECUTE afc_upt_charge_from_rad
  ENDIF
  IF (size(ens_rel_request->charge,5) > 0)
   SELECT INTO "nl:"
    FROM (dummyt d1  WITH seq = value(ens_rel_request->charge_qual)),
     charge c,
     interface_file i
    PLAN (d1)
     JOIN (c
     WHERE (c.charge_item_id=ens_rel_request->charge[d1.seq].charge_item_id))
     JOIN (i
     WHERE i.interface_file_id=c.interface_file_id)
    DETAIL
     IF (i.profit_type_cd > 0
      AND c.charge_type_cd != nochargetype)
      count2 += 1, afcprofit_req->remove_commit_ind = 1, stat = alterlist(afcprofit_req->charges,
       count2),
      afcprofit_req->charges[count2].charge_item_id = ens_rel_request->charge[d1.seq].charge_item_id
     ELSEIF (i.realtime_ind=1)
      count3 += 1, stat = alterlist(afcinterfacecharge_req->interface_charge,count3),
      afcinterfacecharge_req->interface_charge[count3].charge_item_id = ens_rel_request->charge[d1
      .seq].charge_item_id
     ENDIF
    WITH nocounter
   ;end select
  ...
  ELSEIF (size(afcinterfacecharge_req->interface_charge,5) > 0)
   EXECUTE afc_post_interface_charge  WITH replace("REQUEST",afcinterfacecharge_req)
  ENDIF
 ELSE
  CALL echo("No held charges qualified to be released.")
  SET reply->status_data.status = "Z"
 ENDIF
 SUBROUTINE (getheldcharges(prcount=i4(ref)) =i2 WITH protect)
  CALL logmessage("getHeldCharges","Entering",log_debug)
  SELECT INTO "nl:"
   c.charge_item_id, c.order_id, c.process_flg,
   ...
   PLAN (c
    ...
    WHERE o.order_id=c.order_id
     AND o.order_status_cd=orders_complete)
   HEAD REPORT
    stat = alterlist(ens_rel_request->charge,10)
   DETAIL
    prcount += 1
    IF (mod(prcount,10)=1
     AND prcount != 1)
     stat = alterlist(ens_rel_request->charge,(prcount+ 9))
    ENDIF
    ens_rel_request->charge_qual = prcount, ens_rel_request->charge[prcount].charge_item_id = c
    .charge_item_id, ens_rel_request->charge[prcount].process_flg = 0
   FOOT REPORT
    stat = alterlist(ens_rel_request->charge,prcount)
   WITH forupdate(c), nocounter
  ;end select
 END ;Subroutine
 SUBROUTINE (getheldchargesoforggroup(porggroupcodevalue=f8,prcount=i4(ref)) =i2 WITH protect)
  CALL logmessage("getHeldChargesOfOrgGroup","Entering",log_debug)
  SELECT INTO "nl:"
   c.charge_item_id, c.order_id, c.process_flg,
   o.order_id, o.order_status_cd
   FROM charge c,
