CREATE PROGRAM dm_rdm_card_out_prov_vw:dba
 ...
 DECLARE itblcolntexst = i2 WITH protect, noconstant(0)
 SET readme_data->status = "F"
 SET readme_data->message = "Readme failed: starting script dm_rdm_card_out_prov_vw.."
 SET itblcolntexst = chk_ccl_def_tbl("PROV_CARD_ENTITY")
 IF ((((itblcolntexst=- (1))) OR (itblcolntexst=1)) )
  SET readme_data->status = "S"
  SET readme_data->message = "Success: terminating as PROV_CARD_ENTITY table is not available"
  GO TO exit_script
 ENDIF
 SET itblcolntexst = chk_ccl_def_tbl("CV_PROC_HX")
 IF ((((itblcolntexst=- (1))) OR (itblcolntexst=1)) )
  SET readme_data->status = "S"
  SET readme_data->message = "Success: terminating as CV_PROC_HX table is not available"
  GO TO exit_script
 ENDIF
 SET itblcolntexst = chk_ccl_def_tbl_col("CV_PROC_HX","EVENT_ID")
 IF ((((itblcolntexst=- (1))) OR (itblcolntexst=1)) )
  SET readme_data->status = "S"
  SET readme_data->message = "Success: terminating as EVENT_ID not in CVPROC_HX"
  GO TO exit_script
 ENDIF
 SET itblcolntexst = chk_ccl_def_tbl_col("CV_PROC_HX","ENTRY_MODE_CD")
 IF ((((itblcolntexst=- (1))) OR (itblcolntexst=1)) )
  SET readme_data->status = "S"
  SET readme_data->message = "Success: terminating as ENTRY_MODE_CD not in CVPROC_HX"
  GO TO exit_script
 ENDIF
 SET itblcolntexst = chk_ccl_def_tbl_col("CV_PROC_HX","EVENT_CD")
 IF ((((itblcolntexst=- (1))) OR (itblcolntexst=1)) )
  SET readme_data->status = "S"
  SET readme_data->message = "Success: terminating as EVENT_CD not in CVPROC_HX"
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql "cer_install:DM_CARD_OUT_PROV_VW.sql"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 EXECUTE dm_readme_include_sql_chk "CARD_OUT_PROV_VW", "VIEW"
 IF ((dm_sql_reply->status="F"))
  SET readme_data->status = "F"
  SET readme_data->message = dm_sql_reply->msg
  GO TO exit_script
 ENDIF
 EXECUTE oragen3 "CARD_OUT_PROV_VW"
 IF ((dm_err->err_ind=1))
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error executing oragen3 during view creation: '",dm_err->emsg,
   "'")
  GO TO exit_script
 ENDIF
 SET readme_data->status = "S"
 ...
 SUBROUTINE chk_ccl_def_tbl(dtbl_name)
   ...
   IF (curqual=0)
    ...
    IF (((tbl_row_cnt=1) OR (tbl_row_cnt=0)) )
     CALL echo(build("tbl_row_cnt : ",tbl_row_cnt))
     RETURN(1)
    ELSE
     RETURN(- (1))
    ENDIF
