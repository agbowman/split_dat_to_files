CREATE PROGRAM ct_mp_get_pt_consents:dba
 PROMPT
 ...
 RECORD reply(
   ...
   1 consents[*]
     ...
     2 consent_received_tm_ind = i2
     2 consent_released_dt_tm = dq8
     2 consent_released_tm_ind = i2
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "ct_mp_get_pt_consents"
 ...
 SET last_mod = "000"
 SET mod_date = "Jul 25, 2017"
 SET stat = cnvtjsontorec( $JSONREQUEST)
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_pt_consents","Invalid JSON Request",
   "reply")
  GO TO exit_script
 ENDIF
 SET count = 0
 ...
 IF ((request->consentid != 0))
  ...
  SELECT
   IF (time_field_ind=2)
    not_returned_tm_ind = pc.not_returned_tm_ind, consent_released_tm_ind = pc
    .consent_released_tm_ind, consent_signed_tm_ind = pc.consent_signed_tm_ind,
    consent_received_tm_ind = pc.consent_received_tm_ind
   ELSE
    not_returned_tm_ind = - (1), consent_released_tm_ind = - (1), consent_signed_tm_ind = - (1),
    consent_received_tm_ind = - (1)
   ENDIF
   INTO "nl:"
   ...
   DETAIL
    ...
    consent_released_dt_tm = pc.consent_released_dt_tm, reply->consents[count].pt_consent_id = pc
    .pt_consent_id,
    reply->consents[count].consent_id = pc.consent_id, reply->consents[count].connbr = pc.consent_nbr,
    reply->consents[count].conupdtcnt = pc.updt_cnt,
    reply->consents[count].consenting_person_id = pc.consenting_person_id, reply->consents[count].
    consenting_person_name = p.name_full_formatted, reply->consents[count].consenting_organization_id
     = pc.consenting_organization_id,
    reply->consents[count].consenting_organization_name = orgc.org_name, reply->consents[count].
    consent_signed_dt_tm = pc.consent_signed_dt_tm, reply->consents[count].reason_for_consent_cd = pc
    .reason_for_consent_cd,
    reply->consents[count].reason_for_consent_mean = uar_get_code_meaning(pc.reason_for_consent_cd),
    reply->consents[count].consent_received_dt_tm = pc.consent_received_dt_tm
    IF (time_field_ind=2)
     reply->consents[count].not_returned_tm_ind = not_returned_tm_ind, reply->consents[count].
     consent_released_tm_ind = consent_released_tm_ind, reply->consents[count].consent_signed_tm_ind
      = consent_signed_tm_ind,
     reply->consents[count].consent_received_tm_ind = consent_received_tm_ind
    ENDIF
  ...
  IF ((request->regid != 0))
   ...
   SELECT
    ...
    DETAIL
     ...
     reply->consents[count].consenting_organization_name = orgc.org_name, reply->consents[count].
     consent_signed_dt_tm = pc.consent_signed_dt_tm, reply->consents[count].reason_for_consent_cd =
     pc.reason_for_consent_cd,
     reply->consents[count].reason_for_consent_mean = uar_get_code_meaning(pc.reason_for_consent_cd),
     reply->consents[count].consent_received_dt_tm = pc.consent_received_dt_tm
     IF (time_field_ind=2)
      reply->consents[count].not_returned_tm_ind = not_returned_tm_ind, reply->consents[count].
   ...
   IF ((request->eligid != 0))
    ...
    SELECT
     ...
     DETAIL
      ...
      IF (time_field_ind=2)
       reply->consents[count].not_returned_tm_ind = not_returned_tm_ind, reply->consents[count].
       consent_released_tm_ind = consent_released_tm_ind, reply->consents[count].
       consent_signed_tm_ind = consent_signed_tm_ind,
       reply->consents[count].consent_received_tm_ind = consent_received_tm_ind
      ENDIF
     WITH nocounter, outerjoin = d1
    ;end select
    ...
    ELSE
     ...
     SELECT
      ...
      DETAIL
       ...
       count].not_returned_dt_tm = pc.not_returned_dt_tm, reply->consents[count].
       consent_released_dt_tm = pc.consent_released_dt_tm,
       reply->consents[count].pt_consent_id = pc.pt_consent_id, reply->consents[count].consent_id =
       pc.consent_id, reply->consents[count].connbr = pc.consent_nbr,
       reply->consents[count].conupdtcnt = pc.updt_cnt, reply->consents[count].consenting_person_id
        = pc.consenting_person_id, reply->consents[count].consenting_person_name = p
       .name_full_formatted,
       reply->consents[count].consenting_organization_id = pc.consenting_organization_id, reply->
       consents[count].consenting_organization_name = orgc.org_name, reply->consents[count].
       consent_signed_dt_tm = pc.consent_signed_dt_tm,
       reply->consents[count].reason_for_consent_cd = pc.reason_for_consent_cd, reply->consents[count
       ].reason_for_consent_mean = uar_get_code_meaning(pc.reason_for_consent_cd), reply->consents[
       count].consent_received_dt_tm = pc.consent_received_dt_tm
       IF (time_field_ind=2)
        reply->consents[count].not_returned_tm_ind = not_returned_tm_ind, reply->consents[count].
        consent_released_tm_ind = consent_released_tm_ind, reply->consents[count].
        consent_signed_tm_ind = consent_signed_tm_ind,
