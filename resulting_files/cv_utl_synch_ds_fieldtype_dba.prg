CREATE PROGRAM cv_utl_synch_ds_fieldtype:dba
 ...
 ENDIF
 DECLARE datacnt = i4 WITH protect
 DECLARE commit_flag = i2 WITH protect
 DECLARE cdextencodeset = i4 WITH protect, constant(25290)
 SELECT INTO "nl:"
  x.field_type_cd, x.xref_id, cvex.field_value
  FROM cv_xref x,
   code_value_extension cvex
  PLAN (x
   WHERE x.field_type_cd != 0.0)
   JOIN (cvex
   WHERE cvex.code_value=x.field_type_cd
    AND cvex.code_set=cdextencodeset)
  ORDER BY x.field_type_cd
  DETAIL
   datacnt = (datacnt+ 1), stat = alterlist(syncdata->data,datacnt), syncdata->data[datacnt].xref_id
    = x.xref_id,
   syncdata->data[datacnt].field_type_cd = cvex.code_value, syncdata->data[datacnt].field_type = cvex
   .field_value
  WITH nocounter
 ;end select
 CALL echorecord(syncdata,"cer_temp:syncdata.dat")
 IF (curqual=0)
  SET cv_log_level = 0
  CALL cv_log_current_default(0)
  CALL cv_log_message("Selection error. No need for further processing. ....Exiting")
  GO TO exit_script
 ENDIF
 SET commit_flag = 1
 UPDATE  FROM cv_response r,
   (dummyt d2  WITH seq = value(datacnt))
  SET r.field_type_cd = syncdata->data[d2.seq].field_type_cd, r.field_type = syncdata->data[d2.seq].
   field_type, r.active_ind = 1,
   r.active_status_cd = reqdata->active_status_cd, r.active_status_dt_tm = cnvtdatetime(curdate,
    curtime3), r.beg_effective_dt_tm = cnvtdatetime(curdate,curtime3),
   r.updt_dt_tm = cnvtdatetime(curdate,curtime3), r.updt_cnt = 0, r.updt_id = reqinfo->updt_id,
   r.updt_task = reqinfo->updt_task, r.updt_applctx = reqinfo->updt_applctx
  PLAN (d2
   WHERE (syncdata->data[d2.seq].xref_id != 0.0))
   JOIN (r
   WHERE (r.xref_id=syncdata->data[d2.seq].xref_id)
    AND r.active_ind=1)
  WITH nocounter
 ;end update
 CALL cv_log_message(
  "Updated the meanings on the CV_RESPONSE Table. Please commit the data if you feel everything worked."
  )
#exit_script
 ...
 SUBROUTINE cv_log_destroyhandle(dummy)
   ...
   ENDIF
 END ;Subroutine
 DECLARE cv_utl_synch_ds_fieldtype_vrsn = vc WITH private, constant("001 BM9013 05/09/2007")
END GO
