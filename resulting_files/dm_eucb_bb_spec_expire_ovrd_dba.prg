CREATE PROGRAM dm_eucb_bb_spec_expire_ovrd:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_EUCB_BB_SPEC_EXPIRE_OVRD"
 ...
 SELECT INTO "nl:"
  FROM bb_spec_expire_ovrd bseo,
   ...
   orders o
  PLAN (bseo
   WHERE (bseo.encntr_id=request->xxx_uncombine[ucb_cnt].from_xxx_id))
   JOIN (c
   ...
   WHERE o.order_id=ocr.order_id
    AND ((o.encntr_id+ 0) != bseo.encntr_id))
  DETAIL
   IF ((o.encntr_id != request->xxx_uncombine[ucb_cnt].to_xxx_id))
    failscriptind = 1
   ENDIF
   speccount += 1
 ...
 FOR (specidx = 1 TO speccount)
   UPDATE  FROM bb_spec_expire_ovrd bseo
    SET bseo.encntr_id = request->xxx_uncombine[ucb_cnt].to_xxx_id, bseo.updt_id = reqinfo->updt_id,
