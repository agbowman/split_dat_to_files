CREATE PROGRAM dm2_mig_custccl_import
 ...
 DECLARE dmcci_loc = vc WITH protect, noconstant("")
 SET width = 132
 IF (check_logfile(dmcci_logfile_prefix,".log","dm2_mig_custccl_import")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_import"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmcci_loc = trim(logical("cer_install"))
 IF ((dm2_sys_misc->cur_os != "AXP"))
  SET dmcci_loc = concat(dmcci_loc,"/")
 ENDIF
 SET dm_err->eproc = "Checking if Custom CCL Script CSV File exists"
 IF (dm2_findfile(build(dmcci_loc,"dm2_mig_custccl_list.csv"))=0)
  SET dm_err->emsg =
  "CSV File for Custom CCL Script list not found (cer_install:dm2_mig_custccl_list.csv)"
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 EXECUTE dm2_dbimport "cer_install:dm2_mig_custccl_list.csv", "dm2_mig_custccl_imp", 1000
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_import"
 CALL final_disp_msg(dmcci_logfile_prefix)
END GO
