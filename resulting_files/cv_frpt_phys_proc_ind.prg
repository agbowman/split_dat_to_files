CREATE PROGRAM cv_frpt_phys_proc_ind
 PROMPT
 ...
 SUBROUTINE get_reply_obj(dummy)
  ...
  SELECT
   ...
   qual_proc_date = reply_obj->qual[d1.seq].proc_date, qual_proc_status_disp = substring(1,30,
    reply_obj->qual[d1.seq].proc_status_disp), qual_reason_for_proc = substring(1,50,reply_obj->qual[
    d1.seq].reason_for_proc),
   qual_patient_mrn = substring(1,30,reply_obj->qual[d1.seq].patient_mrn), qual_location = substring(
    1,30,reply_obj->qual[d1.seq].location), qual_sex_disp = substring(1,30,reply_obj->qual[d1.seq].
    sex_disp),
   qual_attending_phys = substring(1,30,reply_obj->qual[d1.seq].attending_phys), qual_patient_age =
   reply_obj->qual[d1.seq].patient_age, qual_admit_date = reply_obj->qual[d1.seq].admit_date,
   qual_patient_dodid = substring(1,30,reply_obj->qual[d1.seq].patient_dodid), qual_patient_cmrn =
   substring(1,30,reply_obj->qual[d1.seq].patient_cmrn), qual_person_alias_enabled = reply_obj->
   ...
   HEAD REPORT
    ...
    _d3 = qual_reason_for_proc, _d4 = qual_patient_mrn, _d5 = qual_location,
    _d6 = qual_sex_disp, _d7 = qual_attending_phys, _d8 = qual_patient_age,
    _d9 = qual_patient_dodid, _d10 = qual_patient_cmrn, _d11 = qual_person_alias_enabled,
    _fenddetail = (rptreport->m_pagewidth - rptreport->m_marginbottom), _fenddetail -=
    footpagesection(rpt_calcheight), sdate = format(cnvtdatetime(cnvtdate2( $START_DATE,"DD-MMM-YYYY"
       ),0),"@SHORTDATE"),
    ...
    ndaylightvar = 0, stimezone = datetimezonebyindex(curtimezoneapp,noffsetvar,ndaylightvar,7,
     cnvtdatetime(curdate,curtime)), total_cnt = 0,
    tprovider = 0, gtotal_cnt = 0, tgraph = 0,
    tgraph_cnt = 0, graph_max = 0, max_tgraph = 0,
    p_curr = 0, p_max = 0, _fenddetail = (rptreport->m_pagewidth - rptreport->m_marginbottom),
   ...
   DETAIL
    ...
    admit_date = format(cnvtdatetime(qual_admit_date),"@SHORTDATE;;Q"), admit_time = format(
     cnvtdatetime(qual_admit_date),"@TIMENOSECONDS;;S"), _fdrawheight = detailsection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
 ...
 SUBROUTINE (headreportsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.063),(offsety+ 1.318),(offsetx+ 9.761),(offsety+
     1.318))
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(uar_i18ngetmessage(_hi18nhandle,
        "SIGNEDPROCEDURES","Signed Physician Report with Indication")),char(0)))
 ...
 SUBROUTINE (headpagesectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 1.510)
    SET rptsd->m_width = 1.438
    SET rptsd->m_height = 0.323
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "PROCEDURE","Procedure"),char(0)))
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 3.310)
    SET rptsd->m_width = 2.000
    SET rptsd->m_height = 0.323
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "REASONFORPROCEDURE","Reason for Procedure"),char(0)))
    SET rptsd->m_y = (offsety+ 0.375)
    SET rptsd->m_x = (offsetx+ 1.510)
    SET rptsd->m_width = 1.438
    SET rptsd->m_height = 0.323
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "PERFORMEDDATE","Performed Date"),char(0)))
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 5.510)
    SET rptsd->m_width = 1.540
    SET rptsd->m_height = 0.323
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "PATIENTNAME","Patient Name"),char(0)))
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 7.110)
    SET rptsd->m_width = 0.925
    SET rptsd->m_height = 0.323
    ...
    IF (qual_person_alias_enabled="1")
     SET rptsd->m_y = (offsety+ 0.063)
     SET rptsd->m_x = (offsetx+ 8.010)
     SET rptsd->m_width = 1.435
     SET rptsd->m_height = 0.323
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "DODID","DODID"),char(0)))
     SET rptsd->m_y = (offsety+ 0.063)
     SET rptsd->m_x = (offsetx+ 9.210)
     SET rptsd->m_width = 1.435
     SET rptsd->m_height = 0.323
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
        "CMRN","CMRN"),char(0)))
    ...
    SET rptsd->m_height = 0.323
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "ADMITDATE","Admit Date"),char(0)))
    SET rptsd->m_y = (offsety+ 0.375)
    SET rptsd->m_x = (offsetx+ 3.310)
    SET rptsd->m_width = 1.240
    SET rptsd->m_height = 0.323
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "LOCATION","Location"),char(0)))
    SET rptsd->m_y = (offsety+ 0.375)
    SET rptsd->m_x = (offsetx+ 7.810)
    SET rptsd->m_width = 1.440
    SET rptsd->m_height = 0.323
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "ATTENDINGPHYS","Attending Phys"),char(0)))
    SET rptsd->m_y = (offsety+ 0.375)
    SET rptsd->m_x = (offsetx+ 7.110)
    SET rptsd->m_width = 0.635
    SET rptsd->m_height = 0.323
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,
       "GENDER","Gender"),char(0)))
    SET rptsd->m_y = (offsety+ 0.375)
    SET rptsd->m_x = (offsetx+ 9.410)
    SET rptsd->m_width = 0.635
    SET rptsd->m_height = 0.323
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(uar_i18ngetmessage(_hi18nhandle,"AGE",
       "Age"),char(0)))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
 ...
 SUBROUTINE (detailsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.208
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(qual_reason_for_proc,char(0)))
    SET rptsd->m_y = (offsety+ 0.063)
    SET rptsd->m_x = (offsetx+ 7.110)
    SET rptsd->m_width = 1.400
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(qual_patient_mrn),char(0)))
    IF (qual_person_alias_enabled="1")
     SET rptsd->m_y = (offsety+ 0.063)
     SET rptsd->m_x = (offsetx+ 8.010)
     SET rptsd->m_width = 1.400
     SET rptsd->m_height = 0.198
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(qual_patient_dodid),char(0)))
     SET rptsd->m_y = (offsety+ 0.063)
     SET rptsd->m_x = (offsetx+ 9.210)
     SET rptsd->m_width = 1.400
     SET rptsd->m_height = 0.198
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(qual_patient_cmrn),char(0)))
    ENDIF
    SET rptsd->m_y = (offsety+ 0.375)
    SET rptsd->m_x = (offsetx+ 3.310)
    SET rptsd->m_width = 1.354
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(qual_location),char(0)))
    SET rptsd->m_y = (offsety+ 0.375)
    SET rptsd->m_x = (offsetx+ 7.810)
    SET rptsd->m_width = 1.354
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(qual_attending_phys),char(0)))
    SET rptsd->m_y = (offsety+ 0.375)
    SET rptsd->m_x = (offsetx+ 7.110)
    SET rptsd->m_width = 0.448
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(qual_sex_disp),char(0)))
    SET rptsd->m_y = (offsety+ 0.365)
    SET rptsd->m_x = (offsetx+ 5.510)
    SET rptsd->m_width = 1.450
    SET rptsd->m_height = 0.208
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(concat(admit_date,uar_i18ngetmessage(
        _hi18nhandle,"AT",notrim(" at ")),admit_time),char(0)))
    SET rptsd->m_y = (offsety+ 0.365)
    SET rptsd->m_x = (offsetx+ 9.410)
    SET rptsd->m_width = 0.583
    SET rptsd->m_height = 0.208
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(qual_patient_age),char(0)))
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 ...
 SUBROUTINE (footprovidernamesectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 1.563),(offsety+ 0.063),(offsetx+ 9.761),(offsety+
     0.063))
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
 ...
 SUBROUTINE (footreportsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(uar_i18ngetmessage(_hi18nhandle,
        "TOTAL PROCEDURES","Total number of Signed Physician Reports:  "),trim(cnvtstring(gtotal_cnt)
        ),char(0))))
    SET rptsd->m_y = (offsety+ 0.688)
    SET rptsd->m_x = (offsetx+ 4.500)
 ...
 SUBROUTINE (graphsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptgraphrec->m_sytitle = uar_i18ngetmessage(_hi18nhandle,"SIGNEDPHYSREPORT",
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CV_FRPT_PHYS_PROC_IND"
 ...
 CALL cv_log_msg_post("MOD 001 21/08/2019 YH053032")
END GO
