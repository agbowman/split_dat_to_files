CREATE PROGRAM dm2_set_db_options:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_set_db_options",".log","Option_Setup")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning DM2_SET_DB_OPTIONS...."
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 SET reqinfo->updt_task = dm2_install_schema->dm2_updt_task_value
 DECLARE dm2_toolset_usage_sub(null) = null
 DECLARE lob_build_ind_sub(null) = null
 DECLARE new_tspace_type_sub(null) = null
 DECLARE cursor_commit_cnt_sub(null) = null
 DECLARE dmt_freelist_grp_sub(null) = null
 DECLARE lob_storage_bp_sub(null) = null
 DECLARE lob_pctversion_sub(null) = null
 DECLARE lob_chunk_sub(null) = null
 DECLARE lob_cache_sub(null) = null
 DECLARE dsdo_insert_hist_dm_info(null) = null
 DECLARE table_monitoring_sub(null) = null
 DECLARE table_monitoring_maxretry_sub(null) = null
 DECLARE db_optimizer_category_sub(null) = i2
 DECLARE dbstats_gather_method_sub(null) = null
 DECLARE cbf_maxrangegroups_sub(null) = null
 DECLARE resource_busy_maxretry_sub(null) = null
 DECLARE dbstats_chk_rpt_sub(null) = null
 DECLARE readme_space_calc_sub(null) = null
 DECLARE recompile_after_alter_tbl_sub(null) = null
 DECLARE add_nn_col_nobf(null) = i2
 DECLARE create_index_invisible(null) = i2
 DECLARE lob_securefile_ind_sub(null) = null
 DECLARE lob_retention_sub(null) = null
 DECLARE lob_maxsize_sub(null) = null
 DECLARE use_initprm_assign_dg_ind_sub(null) = i2
 DECLARE assign_dg_override_sub(null) = i2
 DECLARE degree_of_parallel_max_sub(null) = i2
 DECLARE degree_of_parallel_sub(null) = i2
 DECLARE dsdo_setup_sub(dsdo_type_in=vc) = null
 DECLARE dsdo_get_optidx_sub(null) = null
 DECLARE lob_currev_constant = f8 WITH protect, constant((currev+ (currevminor/ 10.0)))
 DECLARE dsdo_dm_info_exists = i2 WITH protect, noconstant(0)
 DECLARE dsdo_chk_if_diff = i2 WITH protect, noconstant(0)
 DECLARE dsdo_sys_ext_mgmt = vc WITH protect, noconstant("")
 DECLARE dsdo_cnt = i4 WITH protect, noconstant(0)
 DECLARE dsdo_option_idx = i4 WITH protect, noconstant(0)
 DECLARE dsdo_use_default = i2 WITH protect, noconstant(0)
 DECLARE dsdo_reset_to_default = i2 WITH protect, noconstant(0)
 DECLARE dsdo_update_info_num = i2 WITH protect, noconstant(0)
 DECLARE dsdo_set_use = i4 WITH protect, noconstant(0)
 IF ((validate(dm2_preview_db_options,- (1))=- (1)))
  DECLARE dm2_preview_db_options = i2 WITH protect, constant(0)
 ENDIF
 IF ((dm2_db_options->load_ind=1))
  SET dm_err->eproc = "Load indicator is 1, exiting program."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF ( NOT (dm2_preview_db_options IN (0, 1)))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Preview option is not set."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FREE RECORD tmp_option
 RECORD tmp_option(
   1 info_domain = vc
   1 info_name = vc
   1 info_char = vc
 ...
 FREE RECORD dm_info_options
 RECORD dm_info_options(
   1 cnt = i4
   1 qual[*]
     2 info_domain = vc
     2 info_name = vc
 ...
 SET dm_info_options->cnt = 0
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 SET dsdm_ret_hold = ""
 SET dm_err->eproc = "Checking to see if DM_INFO exists"
 SET dsdm_ret_hold = dm2_table_exists("DM_INFO")
 CASE (dsdm_ret_hold)
  OF "F":
   SET dsdo_dm_info_exists = 1
  OF "E":
   GO TO exit_program
  OF "N":
   SET dsdo_dm_info_exists = 0
 ENDCASE
 IF (dsdo_dm_info_exists=1)
  SET dm_err->eproc = "Get options from DM_INFO"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain=concat("DM2_",trim(currdb),"_DB_OPTION")
   DETAIL
    dsdo_cnt = (dsdo_cnt+ 1)
    IF (mod(dsdo_cnt,10)=1)
     stat = alterlist(dm_info_options->qual,(dsdo_cnt+ 9))
    ENDIF
    dm_info_options->qual[dsdo_cnt].info_domain = d.info_domain, dm_info_options->qual[dsdo_cnt].
    info_name = d.info_name, dm_info_options->qual[dsdo_cnt].info_char = d.info_char,
    dm_info_options->qual[dsdo_cnt].info_num = d.info_number, dm_info_options->qual[dsdo_cnt].
    info_date = d.info_date
   FOOT REPORT
    stat = alterlist(dm_info_options->qual,dsdo_cnt), dm_info_options->cnt = dsdo_cnt
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (currdb="ORACLE")
  SET dm_err->eproc = "Determining if SYSTEM Tablespace is LMT or DMT"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  SELECT INTO "nl:"
   FROM dba_tablespaces d
   WHERE d.tablespace_name="SYSTEM"
   DETAIL
    dsdo_sys_ext_mgmt = d.extent_management
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Unable to obtain EXTENT_MANAGEMENT for SYSTEM TSPACE."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 ...
 SET dm2_db_options->degree_of_parallel_max = "NOT_SET"
 SET dm2_db_options->degree_of_parallel = "NOT_SET"
 IF (validate(dm2_db_options->add_nn_col_nobf_ind,"-1") != "-1")
  SET dm2_db_options->add_nn_col_nobf_ind = "NOT_SET"
 ENDIF
 IF (validate(dm2_db_options->create_index_invisible,"-1") != "-1")
  SET dm2_db_options->create_index_invisible = "NOT_SET"
 ENDIF
 SET dm2_db_options->lob_securefile_ind = "NOT_SET"
 SET dm2_db_options->lob_retention = "NOT_SET"
 SET dm2_db_options->lob_maxsize = "NOT_SET"
 SET dm_err->eproc = "Process DM2_TOOLSET_USAGE option."
 CALL dm2_toolset_usage_sub(null)
 SET dm_err->eproc = "Process LOB_BUILD_IND option."
 CALL lob_build_ind_sub(null)
 IF ((dm2_db_options->lob_build_ind="Y"))
  SET dm_err->eproc = "Process LOB_SECUREFILE_IND option."
  CALL lob_securefile_ind_sub(null)
 ENDIF
 SET dm_err->eproc = "Process NEW_TSPACE_TYPE option."
 CALL new_tspace_type_sub(null)
 SET dm_err->eproc = "Process CURSOR_COMMIT_CNT option."
 CALL cursor_commit_cnt_sub(null)
 SET dm_err->eproc = "Process DMT_FREELIST_GRP option."
 CALL dmt_freelist_grp_sub(null)
 SET dm_err->eproc = "Process LOB_STORAGE_BP option."
 CALL lob_storage_bp_sub(null)
 SET dm_err->eproc = "Process LOB_PCT_VERSION option."
 CALL lob_pctversion_sub(null)
 SET dm_err->eproc = "Process LOB_CHUNK option."
 CALL lob_chunk_sub(null)
 SET dm_err->eproc = "Process LOB_CACHE option."
 CALL lob_cache_sub(null)
 IF ((dm2_db_options->lob_securefile_ind="Y"))
  SET dm_err->eproc = "Process LOB_RETENTION option."
  CALL lob_retention_sub(null)
 ENDIF
 IF ((dm2_db_options->lob_retention="MAX"))
  SET dm_err->eproc = "Process LOB_MAXSIZE option."
  CALL disp_msg("",dm_err->logfile,0)
  CALL lob_maxsize_sub(null)
 ENDIF
 SET dm_err->eproc = "Process DB_OPTIMIZER_CATEGORY option."
 IF (db_optimizer_category_sub(null)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Process DBSTATS_GATHER_METHOD option."
 CALL dbstats_gather_method_sub(null)
 SET dm_err->eproc = "Process TABLE MONITORING option."
 CALL table_monitoring_sub(null)
 SET dm_err->eproc = "Process TABLE MONITORING_MAXRETRY option."
 CALL table_monitoring_maxretry_sub(null)
 SET dm_err->eproc = "Process CBF_MAXRANGEGROUPS option."
 CALL cbf_maxrangegroups_sub(null)
 SET dm_err->eproc = "Process RESOURCE_BUSY_MAXRETRY option."
 CALL resource_busy_maxretry_sub(null)
 SET dm_err->eproc = "Process DBSTATS_CHK_RPT option."
 CALL dbstats_chk_rpt_sub(null)
 SET dm_err->eproc = "Process README_SPACE_CALC option."
 CALL readme_space_calc_sub(null)
 SET dm_err->eproc = "Process RECOMPILE_AFTER_ALTER_TBL option."
 CALL recompile_after_alter_tbl_sub(null)
 SET dm_err->eproc = "Process ADD_NN_COL_NOBF_IND option."
 IF (add_nn_col_nobf(null)=0)
  GO TO exit_program
 ENDIF
 IF (validate(dm2_db_options->create_index_invisible,"-1") != "-1")
  SET dm_err->eproc = "Process CREATE_INDEX_INVISIBLE option"
  IF (create_index_invisible(null)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF ((dm2_db_options->create_index_invisible="Y"))
  SET dm_err->eproc = "Process degree_of_parallel_max option."
  IF (degree_of_parallel_max_sub(null)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Process degree_of_parallel option."
  IF (degree_of_parallel_sub(null)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Process USE_INITPRM_ASSIGN_DG_IND option."
 IF (use_initprm_assign_dg_ind_sub(null)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Process ASSIGN_DG_OVERRIDE option"
 IF (assign_dg_override_sub(null)=0)
  GO TO exit_program
 ENDIF
 SET dm2_db_options->load_ind = 1
 SUBROUTINE dm2_toolset_usage_sub(null)
   DECLARE dtus_dm2_toolset_usage = i2 WITH protect, noconstant(0)
   CALL dsdo_setup_sub("DM2_TOOLSET_USAGE")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_date=cnvtdatetime("22-JUN-1996 00:00:00"))
      AND (dm_info_options->qual[dsdo_option_idx].info_char IN ("Y", "N")))
      SET dm2_db_options->dm2_toolset_usage = dm_info_options->qual[dsdo_option_idx].info_char
      IF ((((dm_info_options->qual[dsdo_option_idx].info_char="Y")
       AND (dm_info_options->qual[dsdo_option_idx].info_num=444.0)) OR ((((dm_info_options->qual[
      dsdo_option_idx].info_char="N")
       AND (dm_info_options->qual[dsdo_option_idx].info_num=888.0)) OR ( NOT ((dm_info_options->qual[
      dsdo_option_idx].info_num IN (444.0, 888.0))))) )) )
       SET dsdo_update_info_num = 1
      ENDIF
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ELSE
     SET dsdo_chk_if_diff = 1
     SET dsdo_use_default = 1
    ENDIF
   ELSE
    SET dsdo_use_default = 1
   ENDIF
   IF (dsdo_use_default=1)
    SET dtus_dm2_toolset_usage = dm2_toolset_usage(null)
    IF (dtus_dm2_toolset_usage=0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    IF (dtus_dm2_toolset_usage=1)
     SET dm2_db_options->dm2_toolset_usage = "Y"
    ELSE
     SET dm2_db_options->dm2_toolset_usage = "N"
    ENDIF
    IF (dsdo_chk_if_diff=1
     AND dsdo_option_idx > 0)
     IF ((((dm2_db_options->dm2_toolset_usage="Y")
      AND (dm_info_options->qual[dsdo_option_idx].info_num=444.0)) OR ((((dm2_db_options->
     dm2_toolset_usage="N")
      AND (dm_info_options->qual[dsdo_option_idx].info_num=888.0)) OR ( NOT ((dm_info_options->qual[
     dsdo_option_idx].info_num IN (444.0, 888.0))))) )) )
      SET dsdo_update_info_num = 1
    ...
    ENDIF
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (((dsdo_reset_to_default=1) OR (dsdo_update_info_num=1)) )) )
     IF ((dm2_db_options->dm2_toolset_usage="Y"))
      SET tmp_option->info_num = 888.0
     ELSE
      SET tmp_option->info_num = 444.0
     ENDIF
     IF (dsdo_update_info_num=1)
      SET tmp_option->info_char = dm_info_options->qual[dsdo_option_idx].info_char
      SET tmp_option->info_date = dm_info_options->qual[dsdo_option_idx].info_date
     ENDIF
     CALL dsdo_update_dm_info(dsdo_option_idx)
    ENDIF
   ENDIF
 END ;Subroutine
 SUBROUTINE lob_build_ind_sub(null)
   CALL dsdo_setup_sub("LOB_BUILD_IND")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_date=cnvtdatetime("22-JUN-1996 00:00:00"))
      AND (dm_info_options->qual[dsdo_option_idx].info_char IN ("Y", "N")))
      SET dm2_db_options->lob_build_ind = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    IF (dsdo_use_default=1)
     IF ((dm2_db_options->dm2_toolset_usage="Y")
      AND lob_currev_constant >= 8.2
      AND (((dm2_rdbms_version->level1 > 9)) OR ((((dm2_rdbms_version->level1=9)
      AND (dm2_rdbms_version->level2 > 2)) OR ((((dm2_rdbms_version->level1=9)
      AND (dm2_rdbms_version->level2=2)
      AND (dm2_rdbms_version->level3 > 0)) OR ((dm2_rdbms_version->level1=9)
      AND (dm2_rdbms_version->level2=2)
      AND (dm2_rdbms_version->level3=0)
      AND (dm2_rdbms_version->level4 >= 5))) )) )) )
      SET dm2_db_options->lob_build_ind = "Y"
     ELSE
      SET dm2_db_options->lob_build_ind = "N"
     ENDIF
    ENDIF
   ELSE
    SET dm2_db_options->lob_build_ind = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
     CALL dsdo_update_dm_info(dsdo_option_idx)
    ENDIF
   ENDIF
 END ;Subroutine
 SUBROUTINE lob_securefile_ind_sub(null)
   CALL dsdo_setup_sub("LOB_SECUREFILE_IND")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_char IN ("Y", "N")))
      SET dm2_db_options->lob_securefile_ind = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    IF (dsdo_use_default=1)
     IF ((((dm2_rdbms_version->level1 > 11)) OR ((((dm2_rdbms_version->level1=11)
      ...
      SET dm2_db_options->lob_securefile_ind = "Y"
     ELSE
      SET dm2_db_options->lob_securefile_ind = "N"
     ENDIF
    ENDIF
   ELSE
    SET dm2_db_options->lob_securefile_ind = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE new_tspace_type_sub(null)
   CALL dsdo_setup_sub("NEW_TSPACE_TYPE")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_date=cnvtdatetime("22-JUN-1996 00:00:00"))
      AND (dm_info_options->qual[dsdo_option_idx].info_char IN ("L", "D")))
      SET dm2_db_options->new_tspace_type = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     IF (dsdo_sys_ext_mgmt="LOCAL")
      SET dm2_db_options->new_tspace_type = "L"
     ELSEIF ((dm2_db_options->dm2_toolset_usage="Y")
      AND (dm2_rdbms_version->level1 >= 9))
      SET dm2_db_options->new_tspace_type = "L"
     ELSE
      SET dm2_db_options->new_tspace_type = "D"
     ENDIF
    ENDIF
   ELSE
    SET dm2_db_options->new_tspace_type = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE cursor_commit_cnt_sub(null)
   DECLARE dsdo_int = i4 WITH public, noconstant(0)
   CALL dsdo_setup_sub("CURSOR_COMMIT_CNT")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     SET dsdo_ret_hold = isnumeric(dm_info_options->qual[dsdo_option_idx].info_char)
     CASE (dsdo_ret_hold)
      OF 1:
       SET dsdo_int = cnvtint(dm_info_options->qual[dsdo_option_idx].info_char)
     ENDCASE
     IF (dsdo_ret_hold=1
      AND dsdo_int >= 1)
      SET dm2_db_options->cursor_commit_cnt = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->cursor_commit_cnt = "10000"
    ENDIF
   ELSE
    SET dm2_db_options->cursor_commit_cnt = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dmt_freelist_grp_sub(null)
   CALL dsdo_setup_sub("DMT_FREELIST_GRP")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF (isnumeric(dm_info_options->qual[dsdo_option_idx].info_char)=1)
      IF (cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) >= 0)
       SET dm2_db_options->dmt_freelist_grp = dm_info_options->qual[dsdo_option_idx].info_char
      ELSE
       SET dsdo_reset_to_default = 1
       SET dsdo_use_default = 1
      ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->dmt_freelist_grp = "0"
    ENDIF
   ELSE
    SET dm2_db_options->dmt_freelist_grp = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE lob_storage_bp_sub(null)
   CALL dsdo_setup_sub("LOB_STORAGE_BP")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_char IN ("KEEP", "RECYCLE", "DEFAULT")))
      SET dm2_db_options->lob_storage_bp = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->lob_storage_bp = "RECYCLE"
    ENDIF
   ELSE
    SET dm2_db_options->lob_storage_bp = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE lob_pctversion_sub(null)
   CALL dsdo_setup_sub("LOB_PCTVERSION")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF (isnumeric(dm_info_options->qual[dsdo_option_idx].info_char)=1)
      SET dm2_db_options->lob_pctversion = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->lob_pctversion = "5"
    ENDIF
   ELSE
    SET dm2_db_options->lob_pctversion = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE lob_chunk_sub(null)
   CALL dsdo_setup_sub("LOB_CHUNK")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF (isnumeric(dm_info_options->qual[dsdo_option_idx].info_char)=1)
      SET dm2_db_options->lob_chunk = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->lob_chunk = "8192"
    ENDIF
   ELSE
    SET dm2_db_options->lob_chunk = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE lob_cache_sub(null)
   CALL dsdo_setup_sub("LOB_CACHE")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_char IN ("CACHE", "NOCACHE")))
      SET dm2_db_options->lob_cache = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->lob_cache = "CACHE"
    ENDIF
   ELSE
    SET dm2_db_options->lob_cache = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE lob_retention_sub(null)
   CALL dsdo_setup_sub("LOB_RETENTION")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((((dm_info_options->qual[dsdo_option_idx].info_char IN ("MAX", "AUTO", "NONE"))) OR (
     substring(1,3,dm_info_options->qual[dsdo_option_idx].info_char)="MIN")) )
      SET dm2_db_options->lob_retention = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->lob_retention = "AUTO"
    ENDIF
   ELSE
    SET dm2_db_options->lob_retention = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE lob_maxsize_sub(null)
   DECLARE lms_size_nbr = vc WITH noconstant("")
   DECLARE lms_size_unit = vc WITH noconstant("")
   DECLARE lms_size_sep = i4 WITH noconstant(0)
   CALL dsdo_setup_sub("LOB_MAXSIZE")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     SET lms_size_sep = findstring(" ",dm_info_options->qual[dsdo_option_idx].info_char,1,0)
     SET lms_size_nbr = substring(1,(lms_size_sep - 1),dm_info_options->qual[dsdo_option_idx].
      info_char)
     SET lms_size_unit = substring((lms_size_sep+ 1),1,dm_info_options->qual[dsdo_option_idx].
      info_char)
     IF ((dm_err->debug_flag >= 1))
      CALL echo(build("lms_size_sep = ",lms_size_sep))
      CALL echo(build("lms_size_nbr = ",lms_size_nbr))
      CALL echo(build("lms_size_unit = ",lms_size_unit))
      CALL echo(isnumeric(lms_size_nbr))
     ENDIF
     IF (isnumeric(lms_size_nbr) != 0
      AND lms_size_unit IN ("B", "K", "M", "G", "T"))
      SET dm2_db_options->lob_maxsize = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->lob_maxsize = "n/a"
    ENDIF
   ELSE
    SET dm2_db_options->lob_maxsize = "n/a"
   ENDIF
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE db_optimizer_category_sub(null)
   CALL dsdo_setup_sub("DB_OPTIMIZER_CATEGORY")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_dm_info_exists=1)
     SELECT INTO "nl:"
      di.info_name
      FROM dm_info di
      WHERE cnvtupper(di.info_name)="*OPTIMIZER_MODE*"
       AND di.info_domain="DM_SET_SESSION_PARAMETERS"
      DETAIL
       IF (findstring("RULE",cnvtupper(di.info_name)) > 0)
        dm2_db_options->db_optimizer_category = "RULE"
       ELSE
        dm2_db_options->db_optimizer_category = "COST"
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error("Get DB_OPTIMIZER_CATEGORY from DM_INFO.")=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
    ENDIF
    IF ((dm2_db_options->db_optimizer_category="NOT_SET"))
     SELECT INTO "nl:"
      FROM v$parameter v
      WHERE cnvtupper(v.name)="OPTIMIZER_MODE"
      DETAIL
       IF (cnvtupper(v.value)="RULE")
        dm2_db_options->db_optimizer_category = "RULE"
       ELSE
        dm2_db_options->db_optimizer_category = "COST"
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error("Get DB_OPTIMIZER_CATEGORY from V$PARAMETER.")=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
    ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dbstats_gather_method_sub(null)
   CALL dsdo_setup_sub("DBSTATS_GATHER_METHOD")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_date=cnvtdatetime("07-JAN-2005 00:00:00"))
      AND (dm_info_options->qual[dsdo_option_idx].info_char IN ("DBMS_STATS", "ANALYZE")))
      SET dm2_db_options->dbstats_gather_method = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    IF (dsdo_use_default=1)
     IF ((dm2_rdbms_version->level1 >= 9))
      SET dm2_db_options->dbstats_gather_method = "DBMS_STATS"
     ELSE
      SET dm2_db_options->dbstats_gather_method = "ANALYZE"
     ENDIF
    ENDIF
   ELSE
    SET dm2_db_options->dbstats_gather_method = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE table_monitoring_sub(null)
   CALL dsdo_setup_sub("TABLE_MONITORING")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_date=cnvtdatetime("07-JAN-2005 00:00:00"))
      AND (dm_info_options->qual[dsdo_option_idx].info_char IN ("Y", "N")))
      SET dm2_db_options->table_monitoring = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     IF ((dm2_db_options->dbstats_gather_method="DBMS_STATS"))
      SET dm2_db_options->table_monitoring = "Y"
     ELSE
      SET dm2_db_options->table_monitoring = "N"
     ENDIF
    ENDIF
   ELSE
    SET dm2_db_options->table_monitoring = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE table_monitoring_maxretry_sub(null)
   CALL dsdo_setup_sub("TABLE_MONITORING_MAXRETRY")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_date=cnvtdatetime("07-JAN-2005 00:00:00"))
      AND cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) > 0
      AND cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) < 100)
      SET dm2_db_options->table_monitoring = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->table_monitoring_maxretry = "50"
    ENDIF
   ELSE
    SET dm2_db_options->table_monitoring_maxretry = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE cbf_maxrangegroups_sub(null)
   CALL dsdo_setup_sub("CBF_MAXRANGEGROUPS")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF (cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) > 0
      AND cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) < 100)
      SET dm2_db_options->cbf_maxrangegroups = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->cbf_maxrangegroups = "2"
    ENDIF
   ELSE
    SET dm2_db_options->cbf_maxrangegroups = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE resource_busy_maxretry_sub(null)
   CALL dsdo_setup_sub("RESOURCE_BUSY_MAXRETRY")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF (cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) > 0
      AND cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) <= 14440)
      SET dm2_db_options->resource_busy_maxretry = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dbstats_chk_rpt_sub(null)
   CALL dsdo_setup_sub("DBSTATS_CHK_RPT")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_char IN ("Y", "N")))
      SET dm2_db_options->dbstats_chk_rpt = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->dbstats_chk_rpt = "Y"
    ENDIF
   ELSE
    SET dm2_db_options->dbstats_chk_rpt = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dsdo_get_optidx_sub(null)
  DECLARE dsdo_idx = i4
  IF ((dm_info_options->cnt > 0))
   SET dsdo_option_idx = locateval(dsdo_idx,1,size(dm_info_options->qual,5),tmp_option->info_name,
    dm_info_options->qual[dsdo_idx].info_name)
  ENDIF
 END ;Subroutine
 SUBROUTINE dsdo_setup_sub(dsdo_type_in)
   SET tmp_option->info_domain = concat("DM2_",currdb,"_DB_OPTION")
   SET tmp_option->info_name = dsdo_type_in
   SET tmp_option->info_char = "CERNER_DEFAULT"
   SET tmp_option->info_date = cnvtdatetime("01-JAN-1900 00:00:00")
   SET tmp_option->info_num = 0.0
   SET dsdo_option_idx = 0
   SET dsdo_use_default = 0
   SET dsdo_reset_to_default = 0
   SET dsdo_update_info_num = 0
 END ;Subroutine
 SUBROUTINE dsdo_update_dm_info(dsdo_opt_idx)
   DECLARE dsdo_not_good = i4 WITH public, noconstant(0)
   SET dm_err->eproc = "Checking if option in DM_INFO"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE (di.info_domain=tmp_option->info_domain)
     AND (di.info_name=tmp_option->info_name)
    DETAIL
     IF (((dsdo_opt_idx=0) OR ((((di.info_char != dm_info_options->qual[dsdo_opt_idx].info_char)) OR
     ((di.info_date != dm_info_options->qual[dsdo_opt_idx].info_date))) )) )
      dsdo_not_good = 1
     ENDIF
    WITH nocounter
   ;end select
   ...
   IF (dsdo_not_good=1)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "DB_OPTION Values have changed since beginning of program, restart process."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (curqual=1)
    SET dm_err->eproc = "Updating DM_INFO with options"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    UPDATE  FROM dm_info
     SET info_char = tmp_option->info_char, info_date = cnvtdatetime(tmp_option->info_date),
      info_number = tmp_option->info_num,
      updt_dt_tm = cnvtdatetime(curdate,curtime3), updt_applctx = 0, updt_cnt = 0,
      updt_id = 0, updt_task = reqinfo->updt_task
     WHERE (info_domain=tmp_option->info_domain)
      AND (info_name=tmp_option->info_name)
     WITH nocounter
    ;end update
   ELSE
    IF (dsdo_opt_idx > 0)
     ...
     GO TO exit_program
    ENDIF
    SET dm_err->eproc = "Inserting options into DM_INFO"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    INSERT  FROM dm_info
     SET info_domain = tmp_option->info_domain, info_name = tmp_option->info_name, info_char =
      tmp_option->info_char,
      info_date = cnvtdatetime(tmp_option->info_date), info_number = tmp_option->info_num, updt_dt_tm
       = cnvtdatetime(curdate,curtime3),
      updt_applctx = 0, updt_cnt = 0, updt_id = 0,
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ELSE
    CALL dsdo_insert_hist_dm_info(null)
   ENDIF
 END ;Subroutine
 SUBROUTINE dsdo_insert_hist_dm_info(null)
   DECLARE hist_info_domain = vc WITH protect, noconstant("")
   DECLARE hist_info_name = vc WITH protect, noconstant("")
   DECLARE hist_value = vc WITH protect, noconstant("")
   SET hist_info_domain = concat(tmp_option->info_domain,"_HIST")
   SET hist_info_name = concat(tmp_option->info_name," - ",format(cnvtdatetime(curdate,curtime3),
     ";;q"))
   CALL dm2_push_cmd(concat("set hist_value = dm2_db_options->",tmp_option->info_name," go"),1)
   SET dm_err->eproc = concat("Logging option history for ",tmp_option->info_name)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   INSERT  FROM dm_info
    SET info_domain = hist_info_domain, info_name = hist_info_name, info_char = hist_value,
     info_date = cnvtdatetime(tmp_option->info_date), info_number = tmp_option->info_num, updt_dt_tm
      = cnvtdatetime(curdate,curtime3),
     updt_applctx = 0, updt_cnt = 0, updt_id = 0,
     updt_task = reqinfo->updt_task
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE readme_space_calc_sub(null)
   CALL dsdo_setup_sub("README_SPACE_CALC")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_date=cnvtdatetime("22-JUN-1996 00:00:00"))
      AND (dm_info_options->qual[dsdo_option_idx].info_char IN ("Y", "N")))
      SET dm2_db_options->readme_space_calc = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->readme_space_calc = "Y"
    ENDIF
   ELSE
    SET dm2_db_options->readme_space_calc = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE recompile_after_alter_tbl_sub(null)
   CALL dsdo_setup_sub("RECOMPILE_AFTER_ALTER_TBL")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_char IN ("Y", "N")))
      SET dm2_db_options->recompile_after_alter_tbl = dm_info_options->qual[dsdo_option_idx].
      info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
    ...
    ENDIF
    IF (dsdo_use_default=1)
     SET dm2_db_options->recompile_after_alter_tbl = "N"
    ENDIF
   ELSE
    SET dm2_db_options->recompile_after_alter_tbl = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE add_nn_col_nobf(null)
   CALL dsdo_setup_sub("ADD_NN_COL_NOBF_IND")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_char IN ("Y", "N"))
      AND validate(dm2_db_options->add_nn_col_nobf_ind,"-123") != "-123")
      SET dm2_db_options->add_nn_col_nobf_ind = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    IF (dsdo_use_default=1)
     IF (dm2_get_rdbms_version(null)=0)
      RETURN(0)
     ENDIF
     IF (validate(dm2_db_options->add_nn_col_nobf_ind,"-123") != "-123")
      IF ((dm2_rdbms_version->level1 >= 11))
       SET dm2_db_options->add_nn_col_nobf_ind = "N"
      ELSE
       SET dm2_db_options->add_nn_col_nobf_ind = "N"
      ENDIF
     ENDIF
   ...
   ELSE
    IF (validate(dm2_db_options->add_nn_col_nobf_ind,"-123") != "-123")
     SET dm2_db_options->add_nn_col_nobf_ind = "n/a"
    ENDIF
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE create_index_invisible(null)
   CALL dsdo_setup_sub("CREATE_INDEX_INVISIBLE")
   CALL dsdo_get_optidx_sub(null)
   IF (currdb="ORACLE")
    IF (dsdo_option_idx > 0
     AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
     IF ((dm_info_options->qual[dsdo_option_idx].info_char IN ("Y", "N")))
      SET dm2_db_options->create_index_invisible = dm_info_options->qual[dsdo_option_idx].info_char
     ELSE
      SET dsdo_reset_to_default = 1
      SET dsdo_use_default = 1
     ENDIF
    ...
    IF (dsdo_use_default=1)
     IF ((dm2_rdbms_version->level1 >= 11))
      SET dm2_db_options->create_index_invisible = "Y"
     ELSE
      SET dm2_db_options->create_index_invisible = "N"
     ENDIF
    ENDIF
   ELSE
    SET dm2_db_options->create_index_invisible = "n/a"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE use_initprm_assign_dg_ind_sub(null)
   CALL dsdo_setup_sub("USE_INITPRM_ASSIGN_DG_IND")
   CALL dsdo_get_optidx_sub(null)
   IF (dsdo_option_idx > 0
    AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
    IF ((dm_info_options->qual[dsdo_option_idx].info_char IN ("0", "1")))
     SET dm2_db_options->use_initprm_assign_dg_ind = dm_info_options->qual[dsdo_option_idx].info_char
    ELSE
     SET dsdo_reset_to_default = 1
     SET dsdo_use_default = 1
    ENDIF
   ...
   ENDIF
   IF (dsdo_use_default=1)
    SET dm2_db_options->use_initprm_assign_dg_ind = "1"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE assign_dg_override_sub(null)
   DECLARE dsdo_dg_state = vc WITH protect, noconstant("")
   CALL dsdo_setup_sub("ASSIGN_DG_OVERRIDE")
   CALL dsdo_get_optidx_sub(null)
   IF (dsdo_option_idx > 0
    AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
    SET dm_err->eproc = "Evaulation diskgroup information"
    IF ((dm_err->debug_flag > 2))
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
     FROM v$asm_diskgroup v
     WHERE (v.name=dm_info_options->qual[dsdo_option_idx].info_char)
     DETAIL
      dsdo_dg_state = cnvtupper(v.state)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    IF (dsdo_dg_state IN ("MOUNTED", "CONNECTED"))
     SET dm2_db_options->assign_dg_override = dm_info_options->qual[dsdo_option_idx].info_char
    ELSE
     SET dsdo_reset_to_default = 1
     SET dsdo_use_default = 1
    ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE degree_of_parallel_max_sub(null)
   DECLARE dopm_parallel_max_servers = i2 WITH protect, noconstant(0)
   CALL dsdo_setup_sub("DEGREE_OF_PARALLEL_MAX")
   CALL dsdo_get_optidx_sub(null)
   SELECT INTO "nl:"
    FROM v$parameter vp
    WHERE vp.name="parallel_max_servers"
    DETAIL
     dopm_parallel_max_servers = cnvtint(vp.value)
    WITH nocounter
   ;end select
   IF (check_error("Get parallel_max_servers parameter value.")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (dsdo_option_idx > 0
    AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
    IF (cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) >= 0
     AND cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) <= dopm_parallel_max_servers)
     SET dm2_db_options->degree_of_parallel_max = dm_info_options->qual[dsdo_option_idx].info_char
    ELSE
     SET dsdo_reset_to_default = 1
     SET dsdo_use_default = 1
    ENDIF
   ...
   ENDIF
   IF (dsdo_use_default=1)
    IF (cnvtint(dopm_parallel_max_servers) < 4)
     SET dm2_db_options->degree_of_parallel_max = cnvtstring(dopm_parallel_max_servers)
    ELSE
     SET dm2_db_options->degree_of_parallel_max = "4"
    ENDIF
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE degree_of_parallel_sub(null)
   CALL dsdo_setup_sub("DEGREE_OF_PARALLEL")
   CALL dsdo_get_optidx_sub(null)
   IF (dsdo_option_idx > 0
    AND (dm_info_options->qual[dsdo_option_idx].info_char != "CERNER_DEFAULT"))
    IF (cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) >= 0
     AND cnvtint(dm_info_options->qual[dsdo_option_idx].info_char) <= cnvtint(dm2_db_options->
     degree_of_parallel_max))
     IF ((((dm2_rdbms_version->level1 > 11)) OR ((((dm2_rdbms_version->level1=11)
      AND (dm2_rdbms_version->level2 > 2)) OR ((((dm2_rdbms_version->level1=11)
      AND (dm2_rdbms_version->level2=2)
      AND (dm2_rdbms_version->level3 > 0)) OR ((dm2_rdbms_version->level1=11)
      ...
      AND (dm2_rdbms_version->level3=0)
      AND (dm2_rdbms_version->level4 >= 4))) )) )) )
      SET dm2_db_options->degree_of_parallel = dm_info_options->qual[dsdo_option_idx].info_char
     ENDIF
    ELSE
     SET dsdo_reset_to_default = 1
     SET dsdo_use_default = 1
   ...
   ENDIF
   IF (dsdo_use_default=1)
    SET dm2_db_options->degree_of_parallel = "0"
   ENDIF
   IF (dsdo_dm_info_exists
    AND dm2_preview_db_options=0)
    IF (((dsdo_option_idx=0) OR (dsdo_reset_to_default=1)) )
...
#exit_program
 ...
 ELSE
  COMMIT
  SET dm_err->eproc = "Exiting DM_SET_DB_OPTIONS"
  CALL final_disp_msg("dm2_set_db_options")
 ENDIF
 IF ((dm_err->debug_flag >= 1))
  CALL echorecord(dm2_db_options)
  CALL echo(dm2_install_schema->curprog)
 ENDIF
 FREE RECORD tmp_option
 FREE RECORD dm_info_options
END GO
