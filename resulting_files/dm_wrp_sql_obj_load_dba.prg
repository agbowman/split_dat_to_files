CREATE PROGRAM dm_wrp_sql_obj_load:dba
 ...
 SET readme_data->message = "Readme Failed: Starting script dm_wrp_sql_obj_load.prg..."
 DECLARE dwsol_oe_ind = i4 WITH protect, noconstant(0)
 DECLARE dwsol_cur_id = f8 WITH protect, noconstant(0.0)
 SELECT INTO "NL:"
  FROM dm_info d
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="DM_ENV_ID"
  DETAIL
   dwsol_cur_id = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET readme_data->status = "F"
  ...
  GO TO exit_program
 ENDIF
 SET dwsol_oe_ind = check_open_event(dwsol_cur_id,0.0)
 IF (check_error(dm_err->eproc)=1)
  SET readme_data->status = "F"
 ...
 ENDIF
 IF (dwsol_oe_ind=0)
  EXECUTE dm_dbimport "cer_install:dm_refchg_sql_obj.csv", "dm_refchg_sql_obj_load", 100
  IF ((readme_data->status="F"))
   GO TO exit_program
  ENDIF
  EXECUTE dm_dbimport "cer_install:dm_refchg_sql_obj_text.csv", "dm_refchg_sql_obj_text_load", 500
  IF ((readme_data->status="F"))
   GO TO exit_program
  ENDIF
  EXECUTE dm_dbimport "cer_install:dm_refchg_sql_obj_parm.csv", "dm_refchg_sql_obj_parm_load", 100
  IF ((readme_data->status="F"))
   GO TO exit_program
  ENDIF
 ELSE
  ...
  "Success: DM_REFCHG_SQL_OBJ* tables were not loaded due to an already open event"
 ENDIF
#exit_program
