CREATE PROGRAM ct_mp_get_validate_target_acc:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "ProtMasterId" = 0.0
  WITH outdev, protmasterid
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "CT_MP_GET_VALIDATE_TARGET_ACC"
 ...
 DECLARE protmasterid = f8 WITH protect, noconstant(0.0)
 DECLARE protamendment = f8 WITH protect, noconstant(0.0)
 DECLARE log_domain_exists_ind_cfg = i2 WITH protect, noconstant(0)
 DECLARE log_domain_where_str_org = vc WITH protect, noconstant("1=1")
 DECLARE log_domain_where_str_cfg = vc WITH protect, noconstant("1=1")
 SET protmasterid =  $PROTMASTERID
 ...
 SET log_domain_exists_ind_cfg = checkdic("CT_PROT_TYPE_CONFIG.LOGICAL_DOMAIN_ID","A",0)
 IF (log_domain_exists_ind_cfg=2)
  ...
  SET log_domain_where_str_cfg = "cfg.logical_domain_id = domain_reply->logical_domain_id"
  SET log_domain_where_str_org = "org.logical_domain_id = domain_reply->logical_domain_id"
 ENDIF
 ...
 IF (protamendment > 0)
  SELECT INTO "nl:"
   ...
   PLAN (pa
    WHERE pa.prot_amendment_id=protamendment)
    JOIN (pm
    ...
    WHERE cfg.protocol_type_cd=pa.participation_type_cd
     ...
     AND parser(log_domain_where_str_cfg))
   DETAIL
    ...
    protmasterid = pa.prot_master_id,
 ...
 ELSE
  ...
  SELECT INTO "nl:"
   ...
   FROM prot_amendment pa,
    ...
    ct_prot_type_config cfg
   PLAN (pa
    WHERE pa.prot_master_id=protmasterid)
    JOIN (pm
    WHERE pm.prot_master_id=pa.prot_master_id)
    JOIN (cfg
    WHERE cfg.protocol_type_cd=pa.participation_type_cd
   ...
   HEAD pa.prot_amendment_id
    IF (open_found=0)
     reply->targetaccrual = pa.targeted_accrual, accrualrequired = pa.accrual_required_indc_cd,
     protamendment = pa.prot_amendment_id,
     parent_prot_master_id = pm.parent_prot_master_id,
  ...
  ENDIF
 ENDIF
 IF (parent_prot_master_id != protmasterid)
  SET collab_ind = 1
  SET open_found = 0
  SELECT INTO "nl:"
   pa.targeted_accrual
 ...
 IF (accrualrequired=yes_cd)
  IF (get_both_ind=1)
   ...
   SELECT INTO "nl:"
    ...
    FROM prot_role pr
    PLAN (pr
     WHERE pr.prot_amendment_id=protamendment
  ...
  ELSE
   ...
   SELECT INTO "NL:"
    ...
    PLAN (ec
     ...
     JOIN (org
     WHERE org.organization_id=ec.organization_id
      AND parser(log_domain_where_str_org))
    DETAIL
     aliaspoolcount += 1
     IF (aliaspoolcount=1)
