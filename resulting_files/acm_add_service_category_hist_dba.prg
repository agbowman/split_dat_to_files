CREATE PROGRAM acm_add_service_category_hist:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->service_category_hist_qual[xref->add[index].idx].svc_cat_hist_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(encounter_seq,nextval)
     FROM dual
     DETAIL
      acm_request->service_category_hist_qual[xref->add[index].idx].svc_cat_hist_id = cnvtreal(
       nextseqnum), reply->service_category_hist_qual[xref->add[index].idx].svc_cat_hist_id =
      cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->service_category_hist_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->service_category_hist_qual[xref->add[index].idx].svc_cat_hist_id = acm_request->
    service_category_hist_qual[xref->add[index].idx].svc_cat_hist_id
   ENDIF
   SET reply->service_category_hist_qual[xref->add[index].idx].status = 0
   IF ((acm_request->service_category_hist_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->service_category_hist_qual[xref->add[index].idx].beg_effective_dt_tm =
    cnvtdatetime(curdate,curtime3)
   ENDIF
   IF ((acm_request->service_category_hist_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->service_category_hist_qual[xref->add[index].idx].end_effective_dt_tm =
    cnvtdatetime("31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   service_category_hist s
  SET s.svc_cat_hist_id = acm_request->service_category_hist_qual[xref->add[d.seq].idx].
   svc_cat_hist_id, s.attend_prsnl_id = acm_request->service_category_hist_qual[xref->add[d.seq].idx]
   ...
   s.encntr_id =
   IF ((acm_request->service_category_hist_qual[xref->add[d.seq].idx].encntr_id > 0)) acm_request->
    service_category_hist_qual[xref->add[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->service_category_hist_qual[xref->add[d.seq].idx].
    encntr_idx].encntr_id
   ENDIF
   , s.end_effective_dt_tm = cnvtdatetime(acm_request->service_category_hist_qual[xref->add[d.seq].
    idx].end_effective_dt_tm), s.med_service_cd = acm_request->service_category_hist_qual[xref->add[d
   .seq].idx].med_service_cd,
   s.service_category_cd = acm_request->service_category_hist_qual[xref->add[d.seq].idx].
   service_category_cd, s.transaction_dt_tm =
   IF ((acm_request->service_category_hist_qual[xref->add[d.seq].idx].transaction_dt_tm > 0))
    cnvtdatetime(acm_request->service_category_hist_qual[xref->add[d.seq].idx].transaction_dt_tm)
   ELSE null
   ENDIF
   , s.active_ind =
   IF ((((acm_request->service_category_hist_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((
   acm_request->service_category_hist_qual[xref->add[d.seq].idx].active_status_cd=reqdata->
   active_status_cd))) ) 1
   ELSE 0
   ENDIF
   ,
   s.active_status_cd =
   IF ((acm_request->service_category_hist_qual[xref->add[d.seq].idx].active_status_cd > 0.0))
    acm_request->service_category_hist_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (s)
  WITH nocounter, status(reply->service_category_hist_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->service_category_hist_qual[xref->add[index].idx].status=0))
