CREATE PROGRAM bhs_behav_id:dba
 PROMPT
  ...
  "Approval:" = "",
  "Enter Approval text:" = ""
  WITH outdev, lstperson, s_plan_effective_dt_tm,
  s_review_due_dt_tm, s_plan_reason, s_plan_reason_ft,
  s_rationale_ft, s_clin_contact, s_clin_contact_ft,
  s_phys_search, f_phys_prsnl_id, s_pcp_search,
  f_pcp_prsnl_id, s_interventions, s_goals_outcomes,
  s_approval, s_approval_ft
 EXECUTE reportrtl
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "bhs_behav_id"
 ...
 DECLARE mn_reason_param = i2 WITH protect, constant(5)
 DECLARE mn_clin_param = i2 WITH protect, constant(8)
 DECLARE mn_apprv_param = i2 WITH protect, constant(16)
 ...
 DECLARE ms_approval = vc WITH protect, noconstant(" ")
 ...
 DECLARE ms_goals_outcomes = vc WITH protect, noconstant(" ")
 DECLARE ms_interventions = vc WITH protect, noconstant(" ")
 DECLARE ms_rationale = vc WITH protect, noconstant(" ")
 ...
 SELECT INTO "nl:"
  ...
  PLAN (b
   WHERE b.active_ind=1
    AND b.end_effective_dt_tm > cnvtdatetime( $S_PLAN_EFFECTIVE_DT_TM)
    AND b.person_id=mf_pat_id
    AND b.pat_type_flag=1)
  WITH nocounter
 ;end select
 IF (curqual > 0)
  SET ms_log =
  "And ED Care Plan already exists for this patient.  Please update or deactivate the current ED Care Plan."
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM person p
 ...
 IF (substring(1,1,ms_data_type)="C")
  IF (findstring("OTHER", $S_PLAN_REASON)=0)
   SET ms_reason = trim( $S_PLAN_REASON)
  ENDIF
 ELSEIF (substring(1,1,ms_data_type)="L")
  ...
  ENDFOR
 ENDIF
 IF (trim( $S_PLAN_REASON_FT) > " ")
  IF (trim(ms_reason) > " ")
   SET ms_reason = concat(ms_reason,"|OTHER-",trim( $S_PLAN_REASON_FT))
  ELSE
   SET ms_reason = concat(ms_reason,"OTHER-",trim( $S_PLAN_REASON_FT))
  ENDIF
 ENDIF
 ...
 IF (substring(1,1,ms_data_type)="C")
  IF (findstring("OTHER", $S_APPROVAL)=0)
   SET ms_approval = trim( $S_APPROVAL)
  ENDIF
 ELSEIF (substring(1,1,ms_data_type)="L")
  FOR (ml_cnt = 1 TO cnvtint(substring(2,(textlen(ms_data_type) - 1),ms_data_type)))
    ...
    IF (findstring("OTHER",ms_tmp)=0)
     IF (trim(ms_approval) <= " ")
      SET ms_approval = trim(ms_tmp)
     ELSE
      SET ms_approval = concat(ms_approval,"|",trim(ms_tmp))
     ENDIF
    ENDIF
  ENDFOR
 ENDIF
 IF (trim( $S_APPROVAL_FT) > " ")
  IF (trim(ms_approval) > " ")
   SET ms_approval = concat(ms_approval,"|OTHER-",trim( $S_APPROVAL_FT))
  ELSE
   SET ms_approval = concat(ms_approval,"OTHER-",trim( $S_APPROVAL_FT))
  ENDIF
 ENDIF
 SET ms_approval = trim(replace(ms_approval,":","-",0))
 SET ms_clin_contact = trim(replace(ms_clin_contact,":","-",0))
 SET ms_goals_outcomes = trim(replace( $S_GOALS_OUTCOMES,":","-",0))
 SET ms_interventions = trim(replace( $S_INTERVENTIONS,":","-",0))
 SET ms_rationale = trim(replace( $S_RATIONALE_FT,":","-",0))
 SET ms_reason = trim(replace(ms_reason,":","-",0))
 CALL echo("insert")
 CALL echo( $S_APPROVAL)
 CALL echo( $S_APPROVAL_FT)
 INSERT  FROM bhs_pat_behav_ident b
  SET b.active_ind = 1, b.approval = ms_approval, b.beg_effective_dt_tm = cnvtdatetime(
     $S_PLAN_EFFECTIVE_DT_TM),
   b.bhs_pat_behav_ident_id = seq(bhs_eks_seq,nextval), b.clinical_contact = ms_clin_contact, b
   .end_effective_dt_tm = cnvtdatetime("31-DEC-2100"),
   b.goal = ms_goals_outcomes, b.intervention = ms_interventions, b.person_id = mf_pat_id,
   b.rationale = ms_rationale, b.reason = ms_reason, b.review_dt_tm = cnvtdatetime(
     $S_REVIEW_DUE_DT_TM),
   b.updt_cnt = 0, b.updt_dt_tm = sysdate, b.updt_person_id = 0,
   b.pat_type_flag = 1
  WITH nocounter
 ;end insert
 COMMIT
 ...
 CALL sbr_printline("Effective Date of Care Plan-", $S_PLAN_EFFECTIVE_DT_TM)
 ...
 ENDIF
 CALL sbr_printline("Rationale-",ms_rationale)
 ...
 CALL sbr_printline("",concat("  ",ms_goals_outcomes))
 CALL sbr_printline("Approval","")
 SET ml_pos = 0
 SET ml_start = 1
 IF (findstring("|",ms_approval))
  WHILE (findstring("|",ms_approval,ml_start) > 0)
    SET ml_pos = findstring("|",ms_approval,ml_start)
    SET ms_tmp = substring(ml_start,(ml_pos - ml_start),ms_approval)
    SET ml_start = (ml_pos+ 1)
    CALL sbr_printline("",ms_tmp)
  ENDWHILE
  SET ms_tmp = substring(ml_start,((textlen(ms_approval) - ml_start)+ 1),ms_approval)
  CALL sbr_printline("",ms_tmp)
 ELSE
  CALL sbr_printline("",ms_approval)
 ENDIF
