CREATE PROGRAM dm2_mig_custccl_refresh
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmccr_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_refresh")
 DECLARE dmccr_env = vc WITH protect, noconstant("")
 DECLARE dmccr_node = vc WITH protect, noconstant("")
 DECLARE dmccr_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccr_os_type = vc WITH protect, noconstant("")
 DECLARE dmccr_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccr_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmccr_ndx = i4 WITH protect, noconstant(0)
 DECLARE dmccr_loc = i4 WITH protect, noconstant(0)
 DECLARE dmccr_num_ins = i4 WITH protect, noconstant(0)
 DECLARE dmccr_num_upd = i4 WITH protect, noconstant(0)
 DECLARE dmccr_num_del = i4 WITH protect, noconstant(0)
 FREE RECORD dmccr_scripts
 RECORD dmccr_scripts(
   1 cnt = i4
   1 qual[*]
     2 script_name = vc
     2 object_type = vc
 ...
 DECLARE dmccr_get_scripts(null) = i2
 SET width = 132
 IF (check_logfile(dmccr_logfile_prefix,".log","dm2_mig_custccl_refresh")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_refresh"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmccr_env = cnvtupper(trim(logical("environment")))
 SET dmccr_node = trim(curnode)
 SET dmccr_os_type = trim(cursys2)
 SET dm_err->eproc = concat("Obtaining environment id for current environment: ",dmccr_env)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE de.environment_name=dmccr_env
  DETAIL
   dmccr_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dmccr_env))
  CALL echo(concat("Current Environment Id: ",cnvtstring(dmccr_env_id)))
  CALL echo(concat("Current Node: ",dmccr_node))
  CALL echo(concat("Current Environment: ",dmccr_os_type))
 ENDIF
 SET dm_err->eproc = "Retrieve dma_env_node_id for current node/env"
 SELECT INTO "nl:"
  FROM dma_env_node dne
  WHERE dne.node_name=dmccr_node
   AND dne.environment_id=dmccr_env_id
  DETAIL
   dmccr_id = dne.dma_env_node_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (dmccr_id=0)
  ...
  GO TO exit_program
 ENDIF
 IF (dmccr_get_scripts(null)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag=511))
  CALL echorecord(dmccr_scripts)
 ENDIF
 SET dmccr_num_ins = dmccr_scripts->cnt
 SET dmccr_num_upd = 0
 SET dmccr_num_del = 0
 SET dm_err->eproc =
 "Insert new rows into dma_env_node_dprotect admin table for scripts (if row does not exist)"
 CALL disp_msg("",dm_err->logfile,0)
 ...
 SELECT INTO "nl:"
  ...
  WHERE dend.dma_env_node_id=dmccr_id
  DETAIL
   dmccr_loc = locateval(dmccr_ndx,1,dmccr_scripts->cnt,dend.object_name,dmccr_scripts->qual[
    dmccr_ndx].script_name,
    dend.group_nbr,dmccr_scripts->qual[dmccr_ndx].group_nbr)
   IF (dmccr_loc=0)
    dmccr_scripts->cnt = (dmccr_scripts->cnt+ 1), stat = alterlist(dmccr_scripts->qual,dmccr_scripts
     ->cnt), dmccr_scripts->qual[dmccr_scripts->cnt].script_name = dend.object_name,
    dmccr_scripts->qual[dmccr_scripts->cnt].object_type = dend.object_type_flag_txt, dmccr_scripts->
    qual[dmccr_scripts->cnt].user_name = dend.user_name, dmccr_scripts->qual[dmccr_scripts->cnt].
    group_nbr = dend.group_nbr,
    dmccr_scripts->qual[dmccr_scripts->cnt].op_flag = 3, dmccr_num_del = (dmccr_num_del+ 1),
    dmccr_num_ins = (dmccr_num_ins - 1)
   ELSE
    IF ((dmccr_scripts->qual[dmccr_loc].script_name=dend.object_name)
     AND (dmccr_scripts->qual[dmccr_loc].object_type=dend.object_type_flag_txt)
     AND (dmccr_scripts->qual[dmccr_loc].user_name=dend.user_name)
     AND (dmccr_scripts->qual[dmccr_loc].group_nbr=dend.group_nbr)
     AND (dmccr_scripts->qual[dmccr_loc].source_name=dend.source_name)
     AND (dmccr_scripts->qual[dmccr_loc].datestamp_txt=dend.datestamp_txt)
     AND (dmccr_scripts->qual[dmccr_loc].timestamp_txt=dend.timestamp_txt)
     AND (dmccr_scripts->qual[dmccr_loc].last_compile_dt_tm=cnvtdatetime(dend.last_compile_dt_tm))
     AND (dmccr_scripts->qual[dmccr_loc].binary_cnt=dend.binary_cnt)
     AND (dmccr_scripts->qual[dmccr_loc].app_major_version=dend.app_major_version_nbr)
     AND (dmccr_scripts->qual[dmccr_loc].app_minor_version=dend.app_minor_version_nbr)
     AND (dmccr_scripts->qual[dmccr_loc].ccl_version=dend.ccl_version_nbr))
     dmccr_scripts->qual[dmccr_loc].op_flag = 0
    ELSE
     dmccr_scripts->qual[dmccr_loc].op_flag = 2, dmccr_num_upd = (dmccr_num_upd+ 1), dmccr_num_ins =
     (dmccr_num_ins - 1)
    ENDIF
   ENDIF
  WITH nocounter
 ...
 IF (dmccr_num_ins > 0)
  ...
  INSERT  FROM dma_env_node_dprotect dend,
    (dummyt d  WITH seq = value(dmccr_scripts->cnt))
   SET dend.dma_dprotect_id = seq(dm_seq,nextval), dend.dma_env_node_id = dmccr_id, dend
    .object_type_flag_txt = dmccr_scripts->qual[d.seq].object_type,
    dend.object_name = dmccr_scripts->qual[d.seq].script_name, dend.group_nbr = dmccr_scripts->qual[d
    .seq].group_nbr, dend.user_name = dmccr_scripts->qual[d.seq].user_name,
    dend.source_name = dmccr_scripts->qual[d.seq].source_name, dend.last_compile_dt_tm = cnvtdatetime
    (dmccr_scripts->qual[d.seq].last_compile_dt_tm), dend.datestamp_txt = dmccr_scripts->qual[d.seq].
    datestamp_txt,
    dend.timestamp_txt = dmccr_scripts->qual[d.seq].timestamp_txt, dend.binary_cnt = dmccr_scripts->
    qual[d.seq].binary_cnt, dend.app_major_version_nbr = dmccr_scripts->qual[d.seq].app_major_version,
    dend.app_minor_version_nbr = dmccr_scripts->qual[d.seq].app_minor_version, dend.ccl_version_nbr
     = dmccr_scripts->qual[d.seq].ccl_version, dend.custom_ind = 1
   PLAN (d
    WHERE d.seq > 0
     AND (dmccr_scripts->qual[d.seq].op_flag=1))
    JOIN (dend)
 ...
 IF (dmccr_num_upd > 0)
  ...
  UPDATE  FROM dma_env_node_dprotect dend,
    (dummyt d  WITH seq = value(dmccr_scripts->cnt))
   SET dend.user_name = dmccr_scripts->qual[d.seq].user_name, dend.source_name = dmccr_scripts->qual[
    d.seq].source_name, dend.last_compile_dt_tm = cnvtdatetime(dmccr_scripts->qual[d.seq].
     last_compile_dt_tm),
    dend.datestamp_txt = dmccr_scripts->qual[d.seq].datestamp_txt, dend.timestamp_txt = dmccr_scripts
    ->qual[d.seq].timestamp_txt, dend.binary_cnt = dmccr_scripts->qual[d.seq].binary_cnt,
    dend.app_major_version_nbr = dmccr_scripts->qual[d.seq].app_major_version, dend
    .app_minor_version_nbr = dmccr_scripts->qual[d.seq].app_minor_version, dend.ccl_version_nbr =
    dmccr_scripts->qual[d.seq].ccl_version
   PLAN (d
    WHERE d.seq > 0
     AND (dmccr_scripts->qual[d.seq].op_flag=2))
    JOIN (dend
    WHERE dend.dma_env_node_id=dmccr_id
     AND (dend.object_type_flag_txt=dmccr_scripts->qual[d.seq].object_type)
     AND (dend.object_name=dmccr_scripts->qual[d.seq].script_name)
     AND (dend.group_nbr=dmccr_scripts->qual[d.seq].group_nbr))
   WITH nocounter, maxcommit = 1000
  ;end update
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
 ...
 IF (dmccr_num_del > 0)
  ...
  DELETE  FROM dma_env_node_dprotect dend,
   ...
   PLAN (d
    WHERE d.seq > 0
     AND (dmccr_scripts->qual[d.seq].op_flag=3))
    JOIN (dend
    WHERE dend.dma_env_node_id=dmccr_id
     AND (dend.object_type_flag_txt=dmccr_scripts->qual[d.seq].object_type)
     AND (dend.object_name=dmccr_scripts->qual[d.seq].script_name)
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE dmccr_get_scripts(null)
   ...
   SELECT INTO "nl:"
    ...
    WHERE d.object="P"
     AND d.group < 99
    HEAD REPORT
     dmccr_scripts->cnt = 0, stat = alterlist(dmccr_scripts->qual,0)
    DETAIL
     dmccr_scripts->cnt = (dmccr_scripts->cnt+ 1), stat = alterlist(dmccr_scripts->qual,dmccr_scripts
      ->cnt), dmccr_scripts->qual[dmccr_scripts->cnt].script_name = d.object_name,
     dmccr_scripts->qual[dmccr_scripts->cnt].object_type = d.object, dmccr_scripts->qual[
     dmccr_scripts->cnt].user_name = d.user_name, dmccr_scripts->qual[dmccr_scripts->cnt].source_name
      = d.source_name,
     dmccr_scripts->qual[dmccr_scripts->cnt].group_nbr = d.group, dmccr_scripts->qual[dmccr_scripts->
     cnt].datestamp_txt = format(d.datestamp,"MM/DD/YY;;D"), dmccr_scripts->qual[dmccr_scripts->cnt].
     timestamp_txt = format(d.timestamp,"HH:MM;;M"),
     dmccr_scripts->qual[dmccr_scripts->cnt].last_compile_dt_tm = cnvtdatetime(d.datestamp,d
      .timestamp), dmccr_scripts->qual[dmccr_scripts->cnt].binary_cnt = d.binary_cnt, dmccr_scripts->
     qual[dmccr_scripts->cnt].app_major_version = d.app_major_version,
     dmccr_scripts->qual[dmccr_scripts->cnt].app_minor_version = d.app_minor_version, dmccr_scripts->
     qual[dmccr_scripts->cnt].ccl_version = d.ccl_version, dmccr_scripts->qual[dmccr_scripts->cnt].
     custom_ind = 0,
     dmccr_scripts->qual[dmccr_scripts->cnt].op_flag = 1
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_refresh"
 CALL final_disp_msg(dmccr_logfile_prefix)
END GO
