CREATE PROGRAM dcpreqspd01:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SET admitting_dr_num = fillstring(10," ")
 SELECT INTO "nl:"
  p.person_id, e.encntr_id, ea.encntr_id,
  ...
  HEAD REPORT
   ...
   orders->facility = trim(uar_get_code_description(e.loc_facility_cd)), orders->nurse_unit = trim(
   ...
   orders->bed = trim(uar_get_code_display(e.loc_bed_cd)), orders->location = concat(orders->
    nurse_unit,"   ",orders->room,"/",orders->bed), orders->age = format(trim(cnvtage(cnvtdate(p
       .birth_dt_tm),curdate),3),"### #"),
  ...
  SELECT INTO "nl"
   ...
   HEAD c.event_cd
    ...
    IF (c.event_cd=710750
     AND hght_ind=0)
     pt_height = concat(trim(c.event_tag)," cm"), ht_date = concat("(",format(c.updt_dt_tm,
       "mm/dd/yy;;d"),")"), hght_ind = 1
    ELSEIF (c.event_cd=710741
     AND wght_ind=0)
     pt_weight = concat(trim(c.event_tag)," kg"), wt_date = concat("(",format(c.updt_dt_tm,
       "mm/dd/yy;;d"),")"), wght_ind = 1
 ...
 SELECT INTO "nl:"
  ...
  p2.position_cd, o.current_start_dt_tm
  FROM orders o,
   order_action oa,
   prsnl p,
   prsnl p2,
 ...
 SET cd = 0
 SELECT INTO "nl:"
  oef.oe_field_id, od.action_sequence, od.order_id
  ...
  PLAN (d1)
   ...
   JOIN (oef
   WHERE ((oef.oe_format_id+ 0)=orders->qual[d1.seq].oe_format_id)
 ...
 FOR (cqual = 1 TO value(num_of_orders))
   IF ((orders->qual[cqual].comments_ind="T"))
    SELECT INTO "nl:"
     ...
     HEAD oc.order_id
      orders->qual[cqual].comment_cnt = 0, comm_line = "Y"
     DETAIL
      b_cc = 1, b_s = 1
      WHILE (b_cc)
        IF (comm_line="Y")
         b_tmp_comment = substring(b_s,40,lt.long_text), t_l = 40
        ELSE
         b_tmp_comment = substring(b_s,50,lt.long_text), t_l = 50
        ENDIF
        comm_line = "N", b_e = findstring(b_linefeed,b_tmp_comment,1)
        IF (b_e)
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(request->printer_name)
   ...
   PLAN (d1)
   HEAD REPORT
    bc_dio = "{LPI/24}{CPI/8}{BCR/250}{FONT/28/7}", reg_dio = "{LPI/8}{CPI/20}{FONT/12}", first_page
     = "Y",
    reg_dio, "{fr/2}", "  ",
    row + 1
   HEAD PAGE
    line9 = fillstring(80,"_"), allg_line1 = fillstring(80," "), allg_line2 = fillstring(80," "),
    spaces = fillstring(40," "), qty = fillstring(10," "), pri_cd = fillstring(10," ")
   DETAIL
    lol = 0, eod = 0
    FOR (vv = 1 TO value(num_of_orders))
      ...
      IF (first_page="N")
       BREAK
      ENDIF
      first_page = "N", "{CPI/20}{POS/130/5}{b}",
      CALL print(trim(orders->name,3)),
      row + 1, "{CPI/20}{POS/130/15}{b}", "{b}",
      orders->fnbr, "{endb}", "{CPI/20}{POS/195/15}",
      "Pt. Loc: ", orders->location, row + 1,
      "{CPI/20}{POS/130/25}",
      CALL print(trim(provider)), " ",
      orders->qual[vv].order_provider_name, row + 1, "{CPI/20}{POS/130/35}",
      orders->age, " ",
      CALL print(substring(1,1,orders->sex)),
      " ", orders->dob, row + 1,
      "{CPI/20}{POS/130/45}", "{b}", orders->qual[vv].order_mnemonic,
      "{endb}", row + 1, "{CPI/20}{POS/130/90}",
      "Ordering Location: ", orders->order_location, row + 1,
      "{CPI/20}{POS/130/100}", orders->qual[vv].order_mnemonic, row + 1,
      "{CPI/20}{POS/130/150}{b}",
      CALL print(trim(orders->name,3)), row + 1,
      "{CPI/20}{POS/130/160}{b}", orders->fnbr, row + 1,
      "{CPI/20}{POS/195/160}", "Pt. Location: ", orders->location,
      row + 1, "{CPI/20}{POS/130/170}", "Admit Dr.:  ",
      orders->admitting_dr, row + 1, "{CPI/20}{POS/130/180}",
      orders->age, " ", orders->sex,
      " ", orders->dob, row + 1,
      "{CPI/20}{POS/130/190}", "Visit Reason: ",
      CALL print(substring(1,43,orders->admit_diagnosis)),
      row + 1, "{CPI/20}{POS/130/200}", "Ht: ",
      CALL print(trim(pt_height)), " ",
      CALL print(trim(ht_date)),
      "  Wt: ",
      CALL print(trim(pt_weight)), " ",
      CALL print(trim(wt_date)), row + 1
      IF (size(orders->qual[vv].order_mnemonic) > 54)
       "{CPI/20}{POS/130/210}", "{b}",
       CALL print(substring(1,54,orders->qual[vv].order_mnemonic)),
       "{endb}", row + 1, "{CPI/20}{POS/130/220}",
       "{b}",
       CALL print(substring(55,54,orders->qual[vv].order_mnemonic)), "{endb}",
       row + 1, ycol = 230
      ELSE
       "{CPI/20}{POS/130/210}", "{b}", orders->qual[vv].order_mnemonic,
       "{endb}", row + 1, ycol = 220
      ENDIF
      xcol = 130
      FOR (fsub = 1 TO 31)
        FOR (ww = 1 TO orders->qual[vv].detail_cnt)
          IF ((((orders->qual[vv].detail_qual[ww].group_seq=fsub)) OR (fsub=31
           ...
           ":  ", lol = (size(orders->qual[vv].detail_qual[ww].label_text)+ 5)
           IF (((lol+ size(orders->qual[vv].detail_qual[ww].display_value)) > 54))
            pos = 1, eol = 0
            WHILE ((pos < (size(orders->qual[vv].detail_qual[ww].display_value)+ 1))
             AND substring((pos+ 1),(size(orders->qual[vv].detail_qual[ww].display_value) - pos),
             orders->qual[vv].detail_qual[ww].display_value) > spaces)
              bw = pos, ew = findstring(" ",trim(orders->qual[vv].detail_qual[ww].display_value),(bw
               + 1))
              IF (ew=0)
               ew = (size(orders->qual[vv].detail_qual[ww].display_value)+ 1)
              ENDIF
              len = (ew - bw), x = movestring(orders->qual[vv].detail_qual[ww].display_value,bw,temp,
               1,len)
              IF ((((eol+ len)+ lol) < 55))
               "{b}",
               CALL print(trim(temp)), eol = (eol+ len)
              ELSE
               row + 1, lol = 0, xcol = xcol,
               ycol = (ycol+ 10),
               CALL print(calcpos(xcol,ycol)), "{b}",
               CALL print(trim(temp)), eol = len
              ENDIF
              temp = spaces, pos = (pos+ len)
            ENDWHILE
            row + 1
           ELSE
            IF ((orders->qual[vv].detail_qual[ww].field_id=668454))
             "{b}",
            ...
            ENDIF
           ENDIF
           xcol = 130, ycol = (ycol+ 10)
          ENDIF
        ENDFOR
      ENDFOR
      "{CPI/20}{POS/130/55}", "Qty: ", "{CPI/20}{POS/155/55}",
      qty, "{CPI/20}{POS/215/55}", pri_cd,
      row + 1, "{CPI/20}{POS/130/120}", "Qty: ",
      "{CPI/20}{POS/155/120}", qty, "{CPI/20}{POS/215/120}",
      pri_cd, row + 1
      IF ((orders->qual[vv].comments_ind="T")
       ...
       FOR (com_cnt = 1 TO ocnt)
        ...
        IF (ycol >= 335
         AND com_cnt < ocnt)
         CALL print(calcpos(xcol,ycol)), orders->qual[vv].comment_qual[com_cnt].comment_text, "{b}",
         "*", row + 1, com_cnt = ocnt
        ELSE
         CALL print(calcpos(xcol,ycol)), orders->qual[vv].comment_qual[com_cnt].comment_text, row + 1,
         ycol = (ycol+ 10)
        ENDIF
       ENDFOR
      ENDIF
      "{CPI/20}{POS/130/350}", "Ord Date/Time: ", signed,
      row + 1, "{CPI/20}{POS/130/360}", "Ord Dr: ",
      CALL print(trim(provider)), " ", orders->qual[vv].order_provider_name,
      row + 1, "{CPI/20}{POS/130/370}", "Ord Loc: ",
      orders->order_location, row + 1, "{CPI/20}{POS/200/370}",
      "Entered by: ", orders->qual[vv].action_prsnl_name, row + 1,
      "{CPI/20}{POS/130/380}", curdate, "  ",
      curtime, row + 1
    ENDFOR
   FOOT PAGE
    ...
    IF (dt_tm_dif_min > 15)
     IF (dt_tm_dif_min2 > 15)
      "{CPI/18}{POS/190/400}{b}", "*** REPRINT ***", row + 1
     ENDIF
    ENDIF
