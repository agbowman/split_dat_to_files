CREATE PROGRAM dm2_create_actind_triggers:dba
 ...
 DECLARE dai_tbl_name = vc WITH public
 DECLARE dai_tbl_len = vc
 DECLARE dai_tbl_c_ind = vc
 DECLARE dai_cnt = i4
 DECLARE dai_tcnt = i4
 DECLARE dai_pkg_process = i4 WITH protect, noconstant(0)
 DECLARE dai_pkg_num = i4 WITH protect, noconstant(0)
 DECLARE dcat_valid_cnt = i4 WITH protect, noconstant(0)
 DECLARE dai_retry_ceiling = i4 WITH public
 DECLARE regexp_replace() = c300
 DECLARE dcat_sd_in_use_ind = i2 WITH protect, noconstant(0)
 DECLARE dcat_trg_drop_ind = i2 WITH protect, noconstant(0)
 IF (validate(dm2_etu->mode," ")="PACKAGE")
  SET dai_pkg_process = 1
  SET dai_pkg_num = dm2_etu->pkg_num
 ELSE
  SET dai_pkg_process = 0
 ENDIF
 SET dai_tbl_name = cnvtupper( $1)
 IF (check_logfile("dm2_actind_trig",".log","DM2 ACTIVE INDICATOR TRIGGER LOGFILE")=0)
  GO TO exit_script
 ENDIF
 IF (currdb="SQLSRV")
 ...
 IF ((dm2_rdbms_version->level1 > 10))
  IF (dir_get_ddl_gen_retry(dai_retry_ceiling)=0)
   GO TO exit_script
  ENDIF
 ELSE
  SET dai_retry_ceiling = 0
 ENDIF
 SET dm_err->eproc = "Starting dm2_create_actind_triggers.prg"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT
  IF (dai_pkg_process=1)
   FROM dm2_user_tab_columns utc1,
    dm_tables_doc dtd,
    dm2_user_tables ut,
    dm_afd_tables dat
   PLAN (dat
    WHERE dat.alpha_feature_nbr=dai_pkg_num
     AND dat.owner=currdbuser)
    JOIN (ut
    WHERE ut.table_name=dat.table_name)
    JOIN (dtd
    WHERE dtd.table_name=ut.table_name
     AND dtd.drop_ind=0
     AND ut.temporary="N"
     AND ut.table_name != "MV*")
    JOIN (utc1
    WHERE utc1.table_name=ut.table_name)
  ...
  ELSE
   FROM dm2_user_tab_columns utc1,
    ...
    dm2_user_tables ut
   PLAN (ut
    WHERE ut.table_name=patstring(dai_tbl_name)
     AND ut.temporary="N"
     AND ut.table_name != "MV*")
    JOIN (dtd
    WHERE dtd.table_name=ut.table_name
  ...
  HEAD ut.table_name
   dcat_valid_cnt = 0
  DETAIL
   IF (((utc1.column_name="ACTIVE_IND") OR (((utc1.column_name="ACTIVE_STATUS_DT_TM"
    AND utc1.data_type IN ("DATE", "TIMESTAMP")) OR (((utc1.column_name="UPDT_ID"
    AND utc1.data_type IN ("NUMBER", "FLOAT", "INTEGER", "DOUBLE", "BIGINT")) OR (utc1.column_name=
   "ACTIVE_STATUS_PRSNL_ID"
    AND utc1.data_type IN ("NUMBER", "FLOAT", "INTEGER", "DOUBLE", "BIGINT"))) )) )) )
    dcat_valid_cnt = (dcat_valid_cnt+ 1)
   ENDIF
  FOOT  ut.table_name
   IF (dcat_valid_cnt=4)
    IF (findstring("$",ut.table_name)=0)
     tlist->tlist_cnt = (tlist->tlist_cnt+ 1), stat = alterlist(tlist->qual,tlist->tlist_cnt), tlist
     ->qual[tlist->tlist_cnt].tname = ut.table_name,
     tlist->qual[tlist->tlist_cnt].suffix = dtd.table_suffix, tlist->qual[tlist->tlist_cnt].
     trig_version = - (1)
     IF (currdb="DB2UDB")
      tlist->qual[tlist->tlist_cnt].c_name = build("TRG",dtd.table_suffix,"ACTIND$C"), tlist->qual[
      tlist->tlist_cnt].reg_name = build("TRG",dtd.table_suffix,"ACTIND")
     ELSE
      tlist->qual[tlist->tlist_cnt].c_name = build("TRG",trim(substring(1,25,ut.table_name)),"$C"),
      tlist->qual[tlist->tlist_cnt].reg_name = build("TRG",trim(substring(1,27,ut.table_name)))
     ENDIF
    ENDIF
   ENDIF
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ELSEIF (curqual=0)
  IF (dai_pkg_process=0)
   SET dm_err->eproc = concat("No tables named ",dai_tbl_name,
    " were found that require active indicator triggers.")
   CALL disp_msg(" ",dm_err->logfile,0)
  ELSE
   SET dm_err->eproc = concat("No tables on package ",dai_tbl_name,
    " were found that require active indicator triggers.")
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  GO TO exit_script
 ENDIF
 IF (dsr_sd_in_use_check("DM2NOTSET",dcat_sd_in_use_ind)=0)
  GO TO exit_script
 ENDIF
 IF (dcat_sd_in_use_ind=1)
 ...
 SELECT
  IF (dai_pkg_process=1)
   ...
   FROM dm2_user_triggers ut,
    dm_afd_tables a
   PLAN (a
    WHERE a.alpha_feature_nbr=dai_pkg_num
     AND a.owner=currdbuser)
    JOIN (ut
    WHERE ut.table_name=a.table_name
     AND ut.trigger_name="TRG*")
  ELSE
   tmp_version = regexp_replace(ut.description,"(.*/\*version_)(\d+)(\*/.*)","\2",1,0,
    "in")
   FROM dm2_user_triggers ut
  ...
  HEAD ut.table_name
   dai_tcnt = 0
   IF (findstring("$",ut.table_name)=0)
    dai_tcnt = locateval(dai_tcnt,1,tlist->tlist_cnt,ut.table_name,tlist->qual[dai_tcnt].tname)
   ELSE
    dai_tcnt = 0
   ENDIF
  DETAIL
   IF (dai_tcnt > 0)
    IF (((currdb="ORACLE"
     AND cnvtupper(ut.when_clause)="OLD.ACTIVE_IND <> NEW.ACTIVE_IND") OR (currdb="DB2UDB")) )
     tlist->qual[dai_tcnt].trig_version = cnvtint(tmp_version)
     IF ((ut.trigger_name=tlist->qual[dai_tcnt].c_name))
      tlist->qual[dai_tcnt].c_exists = "Y"
     ELSEIF ((ut.trigger_name=tlist->qual[dai_tcnt].reg_name))
      tlist->qual[dai_tcnt].reg_exists = "Y"
     ENDIF
    ENDIF
   ENDIF
  WITH nocounter
 ...
 IF ((dm_err->debug_flag > 3))
  CALL echorecord(tlist)
 ENDIF
 FOR (dai_cnt = 1 TO tlist->tlist_cnt)
   SET dcat_trg_drop_ind = 0
   IF (dcat_sd_in_use_ind=1
    AND (tlist->qual[dai_cnt].trig_version <= 0))
    IF (dsr_check_object_state(currdbuser,"TRIGGER",tlist->qual[dai_cnt].reg_name,"DROPPED",
     dcat_trg_drop_ind)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   IF ((((tlist->qual[dai_cnt].trig_version > 0)) OR (dcat_trg_drop_ind=1)) )
    CALL echo(concat("Schema Deployment now managing trigger ",tlist->qual[dai_cnt].reg_name))
   ELSE
    EXECUTE dm2_create_actind_triggers_c
    IF ((dm_err->err_ind=1))
     GO TO exit_script
    ENDIF
   ENDIF
 ENDFOR
#exit_script
 SET dm_err->eproc = "Active_ind Triggers Complete"
 CALL final_disp_msg("dm2_actind_trig")
 FREE RECORD tlist
 FREE RECORD str
END GO
