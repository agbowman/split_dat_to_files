CREATE PROGRAM bed_imp_prep:dba
 FREE SET reply
 RECORD reply(
   1 error_msg = vc
   1 status_data
 ...
 IF (audit_mode=0)
  SET title_txt = "Person Preparation or Post Import"
 ELSE
  SET title_txt = "Person Preparation or Post Import Audit Mode"
 ENDIF
 SET logfilename = "CCLUSERDIR:bed_imp_prep.log"
 SELECT INTO value(logfilename)
  HEAD REPORT
   curdate"dd-mmm-yyyy;;d", "-", curtime"hh:mm;;m",
   col + 1, title_txt, row + 2,
   col 2, "NAME", col 50,
   "PREP OR POST", col 70, "STATUS",
   row + 1
  WITH nocounter
 ;end select
 DECLARE temp_status = vc
 DECLARE prep_post_txt = vc
 DECLARE temp_text_type = vc
 SET reply->status_data.status = "F"
 SET req_cnt = 0
 SET req_cnt = size(requestin->list_0,5)
 SET prep_add_cnt = 0
 SET prep_update_cnt = 0
 SET post_add_cnt = 0
 SET post_update_cnt = 0
 SET error_cnt = 0
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 SET cs15149_prep_code_value = 0.0
 SELECT INTO "nl:"
  FROM code_value c
  PLAN (c
   WHERE c.code_set=15149
    AND c.cdf_meaning="PREAPPT")
  DETAIL
   cs15149_prep_code_value = c.code_value
  WITH nocounter
 ;end select
 SET cs15589_prep_code_value = 0.0
 SELECT INTO "nl:"
  FROM code_value c
  PLAN (c
   WHERE c.code_set=15589
    AND c.cdf_meaning="PREAPPT")
  DETAIL
   cs15589_prep_code_value = c.code_value
  WITH nocounter
 ;end select
 SET cs15149_post_code_value = 0.0
 SELECT INTO "nl:"
  FROM code_value c
  PLAN (c
   WHERE c.code_set=15149
    AND c.cdf_meaning="POSTAPPT")
  DETAIL
   cs15149_post_code_value = c.code_value
  WITH nocounter
 ;end select
 SET cs15589_post_code_value = 0.0
 SELECT INTO "nl:"
  FROM code_value c
  PLAN (c
   WHERE c.code_set=15589
    AND c.cdf_meaning="POSTAPPT")
  DETAIL
   cs15589_post_code_value = c.code_value
  WITH nocounter
 ;end select
 FOR (x = 1 TO req_cnt)
   SET skip_ind = 0
   ...
   SET prep_post_txt = " "
   SET temp_text_type = " "
   IF (cnvtupper(requestin->list_0[x].prep_or_post)="PREP")
    SET cs15149_code_value = cs15149_prep_code_value
    SET cs15589_code_value = cs15589_prep_code_value
    SET prep_post_txt = "Prep"
    SET temp_text_type = "PREAPPT"
   ELSEIF (cnvtupper(requestin->list_0[x].prep_or_post)="POST")
    SET cs15149_code_value = cs15149_post_code_value
    SET cs15589_code_value = cs15589_post_code_value
    SET prep_post_txt = "Post"
    SET temp_text_type = "POSTAPPT"
   ELSE
    SET temp_status = "ERROR: Prep or Post field not defined correctly."
    SET skip_ind = 1
   ENDIF
   IF (skip_ind=0)
    SET updt_ind = 0
      ...
      IF (error_ind=0)
       INSERT  FROM sch_template s
        SET s.template_id = new_template, s.version_dt_tm = cnvtdatetime("31-DEC-2100"), s.mnemonic
         ...
         cs15149_code_value,
         s.text_type_meaning = temp_text_type, s.info_sch_text_id = 0, s.text_id = new_long_text,
         ...
         cs15589_code_value,
         s.sub_text_meaning = temp_text_type, s.mnemonic_key_nls = null
        WITH nocounter
       ;end insert
       IF (curqual=0)
   ...
   IF (temp_status != "NOLOG")
    SELECT INTO value(logfilename)
     ...
     DETAIL
      ...
      prep_post_txt, col 70, temp_status,
      row + 1
     WITH nocounter, append
    ;end select
   ENDIF
   IF (temp_status="Added"
    AND prep_post_txt="Prep")
    SET prep_add_cnt = (prep_add_cnt+ 1)
   ELSEIF (temp_status="Added"
    AND prep_post_txt="Post")
    SET post_add_cnt = (post_add_cnt+ 1)
   ELSEIF (temp_status="Updated"
    AND prep_post_txt="Prep")
    SET prep_update_cnt = (prep_update_cnt+ 1)
   ELSEIF (temp_status="Updated"
    AND prep_post_txt="Post")
    SET post_update_cnt = (post_update_cnt+ 1)
 ...
 SELECT INTO value(logfilename)
  FROM (dummyt d  WITH seq = 1)
  DETAIL
   row + 2, col 1, "Total Number of Person Preparations Added:",
   prep_add_cnt"####", row + 1, col 1,
   "Total Number of Person Preparations Updated:", prep_update_cnt"####", row + 2,
   col 1, "Total Number of Person Posts Added:", post_add_cnt"####",
   row + 1, col 1, "Total Number of Person Posts Updated:",
   post_update_cnt"####", row + 2, col 1,
   "Total Number of Person Preparations/Posts Not Added/Updated:", error_cnt"####", row + 2,
   curdate"dd-mmm-yyyy;;d", "-", curtime"hh:mm;;m",
