CREATE PROGRAM dm_rdm_adr_updt_cat2:dba
 ...
 SET readme_data->message = "Readme Failed: Starting script dm_rdm_adr_updt_cat2..."
 ...
 IF (error(string_struct->ms_err_msg,0) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error selecting from dm_info for DM_RDM_ADR_UPDT_CAT2:",
   string_struct->ms_err_msg)
  GO TO exit_script
 ENDIF
 IF (curqual=0)
  INSERT  FROM dm_info di
   SET di.info_domain = ms_info_domain, di.info_name = ms_info_name, di.info_date = cnvtdatetime(
     curdate,curtime3),
    di.info_number = max_id
   WITH nocounter
  ;end insert
  IF (error(string_struct->ms_err_msg,0) != 0)
   ROLLBACK
   SET readme_data->status = "F"
   SET readme_data->message = concat("Failed to insert max_id value into DM_INFO table:",
    string_struct->ms_err_msg)
   GO TO exit_script
  ENDIF
  COMMIT
