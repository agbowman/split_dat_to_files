CREATE PROGRAM ags_benefit_level_backout:dba
 PROMPT
  "Backout Type (J, I, P) =" = "J",
  "AGS_JOB_ID (0.0) = " = 0.0
  WITH cbackout_type, djob_id
 CALL echo("***")
 CALL echo("***   BEG :: AGS_BENEFIT_LEVEL_BACKOUT")
 CALL echo("***")
 ...
 IF ( NOT (validate(log,0)))
  ...
  DECLARE sstatus_file_name = vc WITH private, noconstant(concat("ags_benefit_level_backout_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_BENEFIT_LEVEL_BACKOUT"
 ...
 IF (the_type="P"
  ...
  DELETE  FROM gs_benefit_level g,
   ...
   PLAN (d
    ...
    JOIN (g
    WHERE (g.person_id=person_rec->qual[d.seq].person_id))
  ...
  UPDATE  FROM ags_benefit_level_data b,
    (dummyt d  WITH seq = value(person_rec->qual_knt))
   SET b.status = "BACK OUT", b.status_dt_tm = cnvtdatetime(curdate,curtime3), b.person_id = 0.0
   PLAN (d
    WHERE d.seq > 0
     AND (person_rec->qual[d.seq].validated_ind > 0)
     AND (person_rec->qual[d.seq].person_id > 0))
    JOIN (b
    WHERE (b.person_id=person_rec->qual[d.seq].person_id))
   WITH nocounter, maxcommit = 5000
  ;end update
  SET ierrcode = error(serrmsg,1)
  IF (ierrcode > 0)
   ...
   SET log->qual[log->qual_knt].smsgtype = "ERROR"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_BENEFIT_LEVEL_DATA >> ",trim(serrmsg)
    )
   GO TO exit_script
  ENDIF
 ELSEIF (the_type="J"
 ...
 ELSEIF (the_type="I")
  ...
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(item_rec->qual_knt)),
    ags_benefit_level_data b,
    gs_benefit_level g
   PLAN (d
    WHERE d.seq > 0
     AND (item_rec->qual[d.seq].data_id > 0))
    JOIN (b
    WHERE (b.ags_benefit_level_data_id=item_rec->qual[d.seq].data_id)
     AND ((b.person_id+ 0) > 0))
    JOIN (g
    WHERE g.person_id=b.person_id)
   HEAD REPORT
    sknt = 0, stat = alterlist(benefit_di->qual,10)
   DETAIL
    sknt = (sknt+ 1)
    IF (mod(sknt,10)=1
     AND sknt != 1)
     stat = alterlist(benefit_di->qual,(sknt+ 9))
    ENDIF
    benefit_di->qual[sknt].item_id = g.gs_benefit_level_id
   FOOT REPORT
    benefit_di->qual_knt = sknt, stat = alterlist(benefit_di->qual,sknt)
   WITH nocounter
  ;end select
  SET ierrcode = error(serrmsg,1)
  IF (ierrcode > 0)
   ...
   SET log->qual[log->qual_knt].smsgtype = "ERROR"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("SELECT ERROR GS_BENEFIT_LEVEL >> ",trim(serrmsg))
   GO TO exit_script
  ENDIF
  IF ((benefit_di->qual_knt > 0))
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   DELETE  FROM gs_benefit_level g,
     (dummyt d  WITH seq = value(benefit_di->qual_knt))
    SET g.seq = 1
    PLAN (d
     WHERE d.seq > 0
      AND (benefit_di->qual[d.seq].item_id > 0))
     JOIN (g
     WHERE (g.gs_benefit_level_id=benefit_di->qual[d.seq].item_id))
    WITH nocounter, maxcommit = 5000
   ;end delete
   SET ierrcode = error(serrmsg,1)
   IF (ierrcode > 0)
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_BENEFIT_LEVEL_BACKOUT"
 IF (define_logging_sub=true)
  CALL echo("***")
 ...
 ENDIF
 CALL echo("***")
 CALL echo("***   END :: AGS_BENEFIT_LEVEL_BACKOUT")
 CALL echo("***")
