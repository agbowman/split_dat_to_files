CREATE PROGRAM dm_dm_merge_action_rows:dba
 ...
 END ;Subroutine
 DECLARE c_mod = c10 WITH noconstant("DM_PRG_MERGE_ACTION 000")
 ...
 DECLARE v_days_between_min = i4 WITH noconstant(14)
 DECLARE v_tablename = vc WITH noconstant("DM_MERGE_ACTION")
 ...
 ELSE
  ...
  ELSE
   ...
   IF (v_archi_version >= 2.0)
    SET v_parser_stmt = sbr_getrowidnotexists("dma.merge_id > 0","dma")
   ELSE
    SET v_parser_stmt = "dma.merge_id > 0"
   ENDIF
   SELECT INTO "nl:"
    dma.rowid
    FROM dm_merge_action dma
    WHERE dma.completed_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    ...
    DETAIL
     ...
     ENDIF
     reply->rows[v_rows].row_id = dma.rowid
    FOOT REPORT
     stat = alterlist(reply->rows,v_rows)
    WITH nocounter, maxqual(dma,value(request->max_rows))
   ;end select
