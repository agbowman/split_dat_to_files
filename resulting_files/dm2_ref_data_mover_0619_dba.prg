CREATE PROGRAM dm2_ref_data_mover_0619:dba
 ...
 FOR (col_num = 1 TO perm_col_cnt)
  ...
  ENDIF
 ENDFOR
 IF ((rs_0619->from_values.code_set=93))
  CALL orphan_child_tab("CODE_VALUE","NOMV03")
  SET drdm_mini_loop_status = "NOMV03"
  SET drdm_no_trans_ind = 1
  GO TO exit_0619
 ENDIF
 CALL echo("")
 ...
 IF ((rs_0619->from_values.active_type_cd != 0))
  SET found_rec = 0
  SET smt_ret_val = ""
  SET smt_ret_val = select_merge_translate(cnvtstring(rs_0619->from_values.active_type_cd),
   "CODE_VALUE")
  IF (smt_ret_val != "No Trans")
 ...
 IF ((rs_0619->from_values.data_status_cd != 0))
  SET smt_ret_val = ""
  SET smt_ret_val = select_merge_translate(cnvtstring(rs_0619->from_values.data_status_cd),
   "CODE_VALUE")
  IF (smt_ret_val != "No Trans")
 ...
 SET col_num = locateval(index_var,1,perm_col_cnt,"CODE_VALUE",dm2_ref_data_doc->tbl_qual[
  temp_tbl_cnt].col_qual[index_var].column_name)
 SET smt_ret_val = ""
 SET smt_ret_val = select_merge_translate(cnvtstring(rs_0619->from_values.code_value),"CODE_VALUE")
 IF (smt_ret_val != "No Trans")
 ...
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated=0))
  ...
  SET no_code_set = 0
  IF (temp_cs_cnt=0)
   SET code_vc = trim(cnvtstring(rs_0619->from_values.code_set))
  ...
  IF (no_insert_update != 1)
    ...
    IF (cki_valid_ind=1)
     ...
     IF (cki_cnt=1)
      ...
      IF (value_status > 0)
       ...
       SET code_vc = concat("CODE VALUE  ",cnvtstring(rs_0619->from_values.code_value))
       SET current_merges = (current_merges+ 1)
   ...
   ENDIF
   IF (found_val=0)
    IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].merge_ui_query_ni=1))
     ...
     IF ((dm2_ref_data_doc->cs_qual[temp_cs_cnt].display_dup_ind=1))
      ...
      ELSE
       ...
       SET drdm_no_trans_ind = 1
       CALL orphan_child_tab("CODE_VALUE","NOMV07")
       GO TO exit_0619
      ENDIF
     ENDIF
     ...
     IF ((cv_eval_return=- (2)))
      ...
      SET drdm_mini_loop_status = "NOMV06"
      SET drdm_no_trans_ind = 1
      CALL orphan_child_tab("CODE_VALUE","NOMV06")
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 0
      GO TO exit_0619
     ENDIF
    ELSE
     DECLARE cv_sub_val = vc
     SET cv_sub_val = trim(cnvtstring(rs_0619->from_values.code_value))
    ...
    IF (no_insert_update=0)
     ...
     ELSEIF ((cv_eval_return=- (4)))
      CALL orphan_child_tab("CODE_VALUE","ORPHAN")
      SET drdm_mini_loop_status = "ORPHAN"
      SET drdm_no_trans_ind = 1
     ELSEIF ((cv_eval_return=- (5)))
      CALL orphan_child_tab("CODE_VALUE","NOMV99")
      SET drdm_mini_loop_status = "NOMV99"
      SET drdm_no_trans_ind = 1
     ELSEIF ((cv_eval_return=- (3)))
      ...
      SET child_merge_audit->num[current_merges].text = concat(dm2_ref_data_doc->tbl_qual[
       temp_tbl_cnt].table_name,"  ",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].
       column_name)
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[col_num].translated = 1
     ELSE
      SET current_merges = (current_merges+ 1)
     ...
     ENDIF
     CALL rdds_del_except("CODE_VALUE",cnvtreal(rs_0619->from_values.code_value))
     SET drdm_from_con = cnvtstring(rs_0619->from_values.code_value)
     SET cv_imt_check = insert_merge_translate(cnvtreal(rs_0619->from_values.code_value),
    ...
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].translated=0))
     ...
     IF (rpt_table != ""
      ...
      IF (except_log_type="ORPHAN")
       ...
       ) OR ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[final_check_loop].pk_ind=1))) )
        SET fail_merges = (fail_merges+ 1)
        SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
       ...
       ENDIF
      ELSEIF (except_log_type IN ("NOMV*", "OLDVER", "BADLOG"))
       SET fail_merges = (fail_merges+ 1)
       SET fail_merge_audit->num[fail_merges].action = "FAILREASON"
       ...
       CALL orphan_child_tab("CODE_VALUE",except_log_type)
       CALL merge_audit(fail_merge_audit->num[fail_merges].action,fail_merge_audit->num[fail_merges].
        text,2)
       IF (drdm_error_out_ind=1)
        ROLLBACK
 ...
 IF (idcd_check=0)
  ...
  IF (nodelete_ind=0)
   ...
   IF ((drdm_chg->log[drdm_log_loop].process_flg != 6))
    ...
    ELSE
     SET nodelete_msg = concat("Could not process log_id ",trim(cnvtstring(drdm_chg->log[
 ...
 SUBROUTINE substitute_all(in_code,origin)
   ...
   SET ui_strip = concat("set c_ret = ",ui_strip,"cnvtreal('",trim(in_code),"'))")
   CALL echo(ui_strip)
   CALL parser(concat(ui_strip," go"),1)
   CASE (c_ret)
    OF - (1):
    ...
    OF - (9):
     ...
     SET substitute_return = - (2)
    OF - (10):
     SET insert_update_reason = concat("Code_Value from code_set 16730 is not translated.  ",
