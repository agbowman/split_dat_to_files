CREATE PROGRAM dm_dcp_care_team_rows:dba
 ...
 SET reply->table_name = "DCP_CARE_TEAM"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(dct.careteam_id)
   FROM dcp_care_team dct
   WHERE dct.careteam_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    dct.rowid
    FROM dcp_care_team dct
    WHERE parser(sbr_getrowidnotexists("dct.careteam_id between curMinID and curMaxID","dct"))
     AND  EXISTS (
    (SELECT
     dsa.careteam_id
     FROM dcp_shift_assignment dsa
     WHERE dsa.careteam_id=dct.careteam_id))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,100)=1)
      stat = alterlist(reply->rows,(rows+ 99))
     ENDIF
     reply->rows[rows].row_id = dct.rowid
    WITH nocounter, maxqual(dct,value(rowsleft))
   ;end select
