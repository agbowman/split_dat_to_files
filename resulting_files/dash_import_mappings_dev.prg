CREATE PROGRAM dash_import_mappings_dev
 PROMPT
 ...
 SET log_program_name = "DASH_IMPORT_MAPPINGS_DEV"
 SET response->status_data.status = "F"
 IF (validate(request->blob_in))
  IF ((request->blob_in > " "))
   SET json = request->blob_in
  ELSE
   CALL logdetails("No configuration string was supplied.  I can't complete the import.<br>",
    "request->blob_in",request->blob_in)
   GO TO exit_script
  ENDIF
 ELSE
  CALL logdetails("No configuration string was supplied.  I can't complete the import.<br>",
 ...
 WHILE (pos > 0)
   CALL logdetails(concat("<br/>processing: ",name),"NULL",null)
   SET failure = bor(failure,mainfunction(name,value))
   SET pos = getnextattrandvalue(json,pos,name,value)
 ENDWHILE
 IF (pos < 0)
  CALL logdetails("There was a problem reading the mapping object.","NULL",null)
 ENDIF
 IF (failure=0)
  CALL logdetails("<br/>The mappings were loaded successfully.","NULL",null)
 ELSE
  CALL logdetails("<br/>The mappings were loaded, but there where errors.","NULL",null)
 ENDIF
 ...
 SUBROUTINE mainfunction(name,value)
   ...
   ;end select
   IF (error(errmsg,0) > 0)
    CALL logdetails("Error trying to find the template:","errMsg",errmsg)
    GO TO exit_script
   ENDIF
   IF (compid=0.0)
    CALL logdetails(concat("<br/>Unable to find the template: ",name),"name",name)
    RETURN(1)
   ENDIF
   ...
   IF (error(errmsg,0) > 0)
    ROLLBACK
    CALL logdetails("Failed to delete the existing mappings: ","errMsg",errmsg)
    GO TO exit_script
   ELSE
    COMMIT
   ENDIF
   CALL processobject(compid,value,0.0)
   CALL logeffort(concat("mainFunction(): ",name),begin_date_time,response->results)
 END ;Subroutine
 SUBROUTINE processobject(compid,object,parentid)
   ...
   DECLARE compsettingid = f8 WITH protect, noconstant(0)
   ...
   WHILE (parentpos > 0)
     ...
     WHILE (childpos > 0)
       IF (name="mapping")
        ...
        ;end select
        IF (error(errmsg,0) > 0)
         CALL logdetails("Failed to get an ID for the mapping table:","errMsg",errmsg)
         GO TO exit_script
        ENDIF
       ...
       IF (childpos < 0)
        CALL logdetails("There was a problem reading the mapping object","childPos",childpos)
        GO TO exit_script
       ENDIF
     ENDWHILE
   ...
   ENDWHILE
   IF (parentpos < 0)
    CALL logdetails("There was a problem reading the JSON object.","parentPos",parentpos)
    GO TO exit_script
   ENDIF
   CALL echo(build("end ProcessObject-",substring(0,40,object),":",curtime3))
 END ;Subroutine
#exit_script
 SET stat = renamerec(response,record_data)
 SET tmpreply = cnvtrectojson(record_data)
 SET _memory_reply_string = tmpreply
