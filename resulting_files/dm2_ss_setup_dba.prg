CREATE PROGRAM dm2_ss_setup:dba
 ...
 ENDIF
 DECLARE dss_instance_cd_start = f8 WITH protect, noconstant(0.0)
 DECLARE dss_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dss_db_name = vc WITH protect, noconstant("")
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: Starting dm2_ss_setup.prg script"
 IF (check_logfile("dm2_ss_setup",".log","DM2_SS_SETUP LOGFILE")=0)
  SET readme_data->message = dm_err->emsg
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning DM2_SS_SETUP."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Getting environment_id from dm_info table."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="DM_ENV_ID"
  DETAIL
   dss_env_id = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Environment ID not found in dm_info."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET readme_data->message = dm_err->emsg
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Verifying row exists in ref_instance_id table."
 SELECT INTO "nl:"
  FROM ref_instance_id rii
  WHERE rii.environment_id=dss_env_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  SELECT INTO "nl:"
   max_val = max(rii.instance_cd)
   FROM ref_instance_id rii
   DETAIL
    dss_instance_cd_start = max_val
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  INSERT  FROM ref_instance_id rii
   SET rii.instance_cd = (dss_instance_cd_start+ 1), rii.instance_name = cnvtupper(concat(trim(
       dss_db_name),"1")), rii.db_name = cnvtupper(trim(dss_db_name)),
    rii.node_address = cnvtupper(concat(trim(dss_db_name),"1")), rii.environment_id = dss_env_id
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Verifying row exists in ref_reports table."
 SELECT INTO "nl:"
  FROM ref_reports rr
  WHERE rr.report_cd=1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  INSERT  FROM ref_reports rr
   SET rr.report_cd = 1, rr.report_name = "SPACE_SUMMARY", rr.report_desc = "Space Summary"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (dm2ss_verify_synonym(null)=0)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Verifying row exists in ref_report_parms table."
 SELECT INTO "nl:"
  FROM ref_report_parms rrp
  WHERE rrp.parm_cd=3
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF (curqual=0)
  INSERT  FROM ref_report_parms rrp
   SET rrp.parm_cd = 3, rrp.parm_name = "TABLESPACE", rrp.parm_prompt = "Tablespace",
    rrp.value_datatype = "VARCHAR"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Verifying row exists in ref_user_report table."
 SELECT INTO "nl:"
  FROM ref_user_report rur
  WHERE rur.report_cd=1
   AND rur.report_name="CERN_SPACE_ALL"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  INSERT  FROM ref_user_report rur
   SET rur.report_id = seq(report_sequence,nextval), rur.report_cd = 1, rur.report_name =
    "CERN_SPACE_ALL",
    rur.report_desc = "Collect all space information for the V500 user.", rur.updt_dt_tm =
    cnvtdatetime(curdate,curtime3), rur.updt_cnt = 0,
    rur.updt_task = reqinfo->updt_task, rur.updt_applctx = reqinfo->updt_applctx, rur.updt_id =
    reqinfo->updt_id
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Verifying row exists in ref_user_report_parms table."
 SELECT INTO "nl:"
  FROM ref_user_report_parms rurp
  WHERE (rurp.report_id=
  (SELECT
   rur.report_id
   FROM ref_user_report rur
   WHERE rur.report_cd=1
  ...
  INSERT  FROM ref_user_report_parms rurp
   SET rurp.report_parms_id = seq(report_sequence,nextval), rurp.report_id =
    (SELECT
     ...
     FROM ref_user_report rur
     WHERE rur.report_cd=1
      AND rur.report_name="CERN_SPACE_ALL"), rurp.parm_cd = 3,
    rurp.parm_seq = 1, rurp.parm_value = "*", rurp.updt_dt_tm = cnvtdatetime(curdate,curtime3),
    rurp.updt_cnt = 0, rurp.updt_task = reqinfo->updt_task, rurp.updt_applctx = reqinfo->updt_applctx,
    rurp.updt_id = reqinfo->updt_id
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_script
 COMMIT
 SET dm_err->eproc = "Ending DM2_SS_SETUP."
 CALL final_disp_msg("dm2_ss_setup")
 IF ((dm_err->err_ind=0))
  SET readme_data->status = "S"
  SET readme_data->message = "Success - All required tables were updated successfully."
 ENDIF
