CREATE PROGRAM aps_rdm_fix_acc_dt_tm:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_rdm_acc_dt_tm"
 CALL logscriptstart(sreadme_name)
 DECLARE lrec = i4 WITH protect, noconstant(0)
 DECLARE lrec2 = i4 WITH protect, noconstant(0)
 ...
 FREE RECORD utcmidjan1
 FREE RECORD accession_info
 FREE RECORD accession_info_2
 RECORD utcmidjan1(
   1 minus12 = dq8
   1 plus24 = dq8
 )
 RECORD accession_info(
   1 qual[*]
     2 accession_fmt_cd = f8
     2 accession_pool_id = f8
     2 assigned_date_time = dq8
     2 accession_seq_nbr = i4
     2 last_increment_date_time = dq8
     2 last_increment_value = f8
 )
 RECORD accession_info_2(
   1 qual[*]
     2 accession_pool_id = f8
     2 assigned_date_time = dq8
     2 accession_seq_nbr = i4
 ...
 IF (curutc=0)
  ...
  GO TO exit_script
 ENDIF
 SET utcmidjan1->minus12 = cnvtdatetimeutc(cnvtdatetime(cnvtdate(((12310000+ year(curdate)) - 1)),
   1200),2)
 SET utcmidjan1->plus24 = cnvtdatetimeutc(cnvtdatetime(cnvtdate((01020000+ year(curdate))),0),2)
 SELECT INTO "nl:"
  aax.accession_format_cd, aa.acc_assign_pool_id, aa.acc_assign_date,
  aa.accession_seq_nbr, aa.last_increment_dt_tm, aa.increment_value,
  aa.updt_cnt
  ...
  PLAN (aax
   ...
   JOIN (aa
   WHERE aax.accession_assignment_pool_id=aa.acc_assign_pool_id
    AND aa.acc_assign_date BETWEEN cnvtdatetime(utcmidjan1->minus12) AND cnvtdatetime(utcmidjan1->
    plus24))
  ORDER BY aa.acc_assign_pool_id, aa.accession_seq_nbr DESC
  HEAD REPORT
   lcount = 0, lcount2 = 0
  HEAD aa.acc_assign_pool_id
   IF (aa.acc_assign_date != cnvtdatetimeutc(cnvtdatetime(cnvtdate((01010000+ year(curdate))),0),1))
    lcount = (lcount+ 1)
    IF (lcount > size(accession_info->qual,5))
     stat = alterlist(accession_info->qual,(lcount+ 1))
    ENDIF
    accession_info->qual[lcount].accession_fmt_cd = aax.accession_format_cd, accession_info->qual[
    lcount].accession_pool_id = aa.acc_assign_pool_id, accession_info->qual[lcount].
    assigned_date_time = aa.acc_assign_date,
    accession_info->qual[lcount].accession_seq_nbr = aa.accession_seq_nbr, accession_info->qual[
    lcount].last_increment_date_time = aa.last_increment_dt_tm, accession_info->qual[lcount].
    last_increment_value = aa.increment_value
   ENDIF
  DETAIL
   IF (aa.acc_assign_date=cnvtdatetimeutc(cnvtdatetime(cnvtdate((01010000+ year(curdate))),0),1))
    lcount2 = (lcount2+ 1)
    IF (lcount2 > size(accession_info_2->qual,5))
     stat = alterlist(accession_info_2->qual,(lcount2+ 1))
    ENDIF
    accession_info_2->qual[lcount2].accession_pool_id = aa.acc_assign_pool_id, accession_info_2->
    qual[lcount2].assigned_date_time = aa.acc_assign_date, accession_info_2->qual[lcount2].
    accession_seq_nbr = aa.accession_seq_nbr,
    accession_info_2->qual[lcount2].update_cnt = aa.updt_cnt
   ENDIF
  FOOT REPORT
   stat = alterlist(accession_info->qual,lcount), stat = alterlist(accession_info_2->qual,lcount2)
  WITH nocounter
 ;end select
 FOR (lrec = 1 TO lcount)
   SET lrec2 = 1
   WHILE (lrec2 <= lcount2
    AND (accession_info->qual[lrec].accession_pool_id != accession_info_2->qual[lrec2].
   accession_pool_id))
     SET lrec2 = (lrec2+ 1)
   ENDWHILE
   IF (lrec2 <= lcount2)
    IF ((accession_info->qual[lrec].accession_seq_nbr > accession_info_2->qual[lrec2].
    accession_seq_nbr))
     UPDATE  FROM accession_assignment aa
      SET aa.accession_seq_nbr = accession_info->qual[lrec].accession_seq_nbr, aa
       .last_increment_dt_tm = cnvtdatetime(accession_info->qual[lrec].last_increment_date_time), aa
       .increment_value = accession_info->qual[lrec].last_increment_value,
       aa.updt_dt_tm = cnvtdatetime(curdate,curtime), aa.updt_id = 999999.0, aa.updt_task = reqinfo->
       updt_task,
       aa.updt_cnt = (accession_info_2->qual[lrec2].update_cnt+ 1), aa.updt_applctx = 999999
      WHERE aa.acc_assign_date=cnvtdatetime(accession_info_2->qual[lrec2].assigned_date_time)
       AND (aa.acc_assign_pool_id=accession_info_2->qual[lrec2].accession_pool_id)
      WITH nocounter
     ;end update
     SET lerrorcode = error(serrormessage,0)
     IF (lerrorcode > 0)
      ROLLBACK
      CALL logmsg("Error updating into accession_assignment.",((e_rdmstatus+ e_dispmsg)+ e_logtofile)
       )
   ...
   ELSE
    INSERT  FROM accession_assignment aa
     SET aa.acc_assign_date = cnvtdatetimeutc(cnvtdatetime(cnvtdate((01010000+ year(curdate))),0),1),
      aa.acc_assign_pool_id = accession_info->qual[lrec].accession_pool_id, aa.accession_seq_nbr =
      accession_info->qual[lrec].accession_seq_nbr,
      aa.last_increment_dt_tm = cnvtdatetime(accession_info->qual[lrec].last_increment_date_time), aa
      .increment_value = accession_info->qual[lrec].last_increment_value, aa.updt_dt_tm =
      cnvtdatetime(curdate,curtime),
      aa.updt_id = 999999.0, aa.updt_task = reqinfo->updt_task, aa.updt_cnt = 0,
      aa.updt_applctx = 999999
     WITH nocounter
    ;end insert
    SET lerrorcode = error(serrormessage,0)
    IF (lerrorcode > 0)
     ROLLBACK
     CALL logmsg("Error inserting into accession_assignment.",((e_rdmstatus+ e_dispmsg)+ e_logtofile)
      )
     CALL logmsg(serrormessage,((e_rdmstatus+ e_dispmsg)+ e_logtofile))
     SET readme_data->status = "F"
     SET readme_data->message = serrormessage
...
#exit_script
 IF ((readme_data->status="F"))
  SET readme_data->message = build("*********Failure*********")
  CALL logstatus("Execution failed - see log message for more details.","F")
 ELSE
  COMMIT
  SET readme_data->message = build("*********Success*********")
  CALL logstatus("Updating of accession_assignment table complete.","S")
 ENDIF
