CREATE PROGRAM bbt_rpt_aborh_result:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->aborh_tool = uar_i18ngetmessage(i18nhandle,"aborh_tool","ABORh BUILDING TOOL")
 SET captions->aborh_parameters = uar_i18ngetmessage(i18nhandle,"aborh_parameters",
  "RESULT ABO/Rh PARAMETERS")
 ...
 SET captions->stand_abo = uar_i18ngetmessage(i18nhandle,"stand_abo","STAND ABO")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_aborh_result", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  cv.code_value, cv.display, cv.description,
  cv.cdf_meaning, cv.collation_seq, cv.active_ind,
  cd.cdf_meaning, cd.display, ce.field_value"###############",
  c2.field_value"##########", aborh_disp = uar_get_code_display(cnvtreal(c2.field_value))
  FROM code_value cv,
   common_data_foundation cd,
   code_value_extension ce,
   code_value_extension c2
  PLAN (cv
   WHERE cv.code_set=1643)
   JOIN (cd
   WHERE cd.code_set=1643
    AND cd.cdf_meaning=cv.cdf_meaning)
   JOIN (ce
   WHERE cv.code_value=ce.code_value
    AND ce.field_name="ChartName")
   JOIN (c2
   WHERE c2.code_set=1643
    AND c2.code_value=cv.code_value
    AND c2.field_name="ABORH_cd")
  ORDER BY cv.collation_seq, cv.display, ce.field_value
  HEAD REPORT
   select_ok_ind = 0
  HEAD PAGE
   ...
   col 50, captions->aborh_tool, row + 1,
   col 48, captions->aborh_parameters, row + 1,
   line = fillstring(122,"-"), line, row + 1,
   col 4, captions->display_name, col 30,
   captions->description, col 50, captions->meaning,
   col 70, captions->chart_name, col 90,
   captions->stand_abo, col 114, captions->active,
   row + 1, line, row + 1
  HEAD cv.display
   col 2, cv.display, col 20,
   cv.description, col 50, cd.display
  HEAD ce.field_value
   IF (ce.field_name="ChartName")
    col 70, ce.field_value
   ENDIF
  DETAIL
   IF (c2.field_value != "0")
    col 90, aborh_disp
