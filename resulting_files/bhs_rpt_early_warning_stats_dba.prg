CREATE PROGRAM bhs_rpt_early_warning_stats:dba
 PROMPT
 ...
 SET bdate = datetimefind(cnvtdatetime((curdate - 7),0),"W","B","B")
 SET edate = datetimefind(cnvtdatetime((curdate - 7),0),"W","E","E")
 FREE RECORD ews
 RECORD ews(
   1 maxprobcnt = i4
   1 maxproblength = i4
 ...
 SELECT INTO "NL:"
  ...
  PLAN (b
   WHERE b.total_score >= 10
    AND b.insert_dt_tm BETWEEN cnvtdatetime(bdate) AND cnvtdatetime(edate))
   JOIN (e
  ...
  ORDER BY b.encntr_id, b.insert_dt_tm
  HEAD REPORT
  ...
  PLAN (d)
   ...
   (SELECT
    ...
    WHERE c.code_set=220
     ...
     AND ((c.display_key IN ("*ICU*", "CVC*", "CVIC*")) OR (c.display_key IN ("ED MAIN")))
 ...
 SELECT INTO value(var_output)
  ...
  substring(1,20,format(cnvtdatetime(ews->qual[d.seq].cunit[d1.seq].unitdttm),";;q")),
  this_encntr_status =
