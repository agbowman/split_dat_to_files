CREATE PROGRAM bed_aud_helix_ord_event_reltn:dba
 ...
 DECLARE caseint = f8 WITH public, noconstant(0.0)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=106
    AND cv.cdf_meaning="CI"
    AND cv.active_ind=1)
  DETAIL
   caseint = cv.code_value, act_type = cv.display
  WITH nocounter
 ;end select
 SELECT INTO "NL:"
  FROM code_value cv
  PLAN (cv
   WHERE cv.code_set=6000
    ...
    AND cv.active_ind=1)
  DETAIL
   lab = cv.code_value, cat_type = cv.display
  WITH nocounter
 ;end select
 ...
 IF ((request->skip_volume_check_ind=0))
  SELECT INTO "nl:"
   ...
   PLAN (oc
    WHERE oc.activity_type_cd=caseint
     AND oc.catalog_type_cd=lab
     AND oc.active_ind=1)
    JOIN (cver
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("helix_ord_to_evntcd_evntset.csv")
 ENDIF
