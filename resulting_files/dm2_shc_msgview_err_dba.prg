CREATE PROGRAM dm2_shc_msgview_err:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((((validate(servers_w_errors->cnt,- (1))=- (1))) OR ((validate(servers_w_errors->cnt,- (2))=- (2
 )))) )
  FREE RECORD servers_w_errors
  RECORD servers_w_errors(
    1 cnt = i4
    1 server[*]
      2 server_name = vc
      2 server_id = vc
  ...
  SET servers_w_errors->cnt = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI ESO-CQM Server"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "252"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI ESO Notify Server"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "275"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI ESO Server"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "253"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI Hold Release Server"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "276"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI Outbound ComClient Server"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "254"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI ESO Server Load #1"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "280"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI ESO Server Load #2"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "281"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI ESO Server Load #3"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "282"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI ESO Server Load #4"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "283"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI ESO Server Load #5"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "284"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
  SET servers_w_errors->cnt += 1
  SET stat = alterlist(servers_w_errors->server,servers_w_errors->cnt)
  SET servers_w_errors->server[servers_w_errors->cnt].server_name = "FSI ESO Server Batch #1"
  SET servers_w_errors->server[servers_w_errors->cnt].server_id = "285"
  SET servers_w_errors->server[servers_w_errors->cnt].err_ind = 0
 ENDIF
 DECLARE cnt = i4 WITH protect, noconstant(0)
 DECLARE cnt2 = i4 WITH protect, noconstant(0)
 DECLARE dsme_getscript = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsme_prev_errfile = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsme_dcl_str = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsme_del_str = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsme_status = c1 WITH protect, noconstant("X")
 DECLARE dsme_extension = vc WITH protect, noconstant(".ksh")
 DECLARE dsme_err_found_ind = i2 WITH protect, noconstant(0)
 DECLARE dsme_fatal_err_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_shc_msgview",".log","SYSTEM HEALTH CHECK MSGVIEW LOG FILE")=0)
  SET dsme_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking for MSGView errors."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ENDIF
 IF (cursys="AXP")
  SET dsme_extension = ".com"
 ENDIF
 IF (get_unique_file("dm2_get_msg_errs",dsme_extension)=0)
  SET dsme_status = "E"
 ENDIF
 SET dsme_getscript = dm_err->unique_fname
 SET dsme_prev_errfile = dm_err->errfile
 SET dm_err->errfile = replace(dsme_getscript,dsme_extension,".dat",0)
 IF (cursys="AIX")
  SET dsme_dcl_str = concat("$CCLUSERDIR/",dsme_getscript)
  SET dsme_del_str = "rm -f $CCLUSERDIR/dm2_get_msg_errs*"
 ELSE
  SET dsme_dcl_str = concat("@ccluserdir:",dsme_getscript)
  SET dsme_del_str = "delete ccluserdir:dm2_get_msg_errs*.*;*"
 ENDIF
 IF (dsme_status != "E")
  FOR (cnt = 1 TO servers_w_errors->cnt)
    SELECT INTO trim(dsme_getscript)
     FROM dual
     DETAIL
      IF (cursys="AIX")
       col + 0, "   msgview <<!"
      ELSE
       col + 0, "   $ msgview"
      ENDIF
      row + 1, col + 0, "   select cmb_0",
      servers_w_errors->server[cnt].server_id, row + 1, col + 0,
      '   search -data "*CCL*"'
      IF (cursys="AIX")
       row + 1, col + 0, "   exit",
       row + 1, col + 0, "   !"
    ...
    IF (dsme_status != "E"
     AND cursys="AIX")
     IF (dm2_push_dcl(concat("chmod ugo+x $CCLUSERDIR/",dsme_getscript))=0)
      SET dsme_status = "E"
     ENDIF
    ENDIF
    IF (dsme_status != "E")
     IF (dm2_push_dcl(dsme_dcl_str)=0)
      SET dsme_status = "E"
     ENDIF
    ENDIF
    IF (dsme_status != "E")
     ...
     ENDIF
    ENDIF
    IF (findstring(" err ",dm_err->errtext) > 0)
     SET servers_w_errors->server[cnt].err_ind = 1
     SET dsme_err_found_ind = 1
    ENDIF
  ENDFOR
 ENDIF
 SET dm_err->errfile = dsme_prev_errfile
 IF (dsme_status != "E")
  IF (dm2_push_dcl(dsme_del_str)=0)
   SET dsme_status = "E"
  ENDIF
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(servers_w_errors)
 ENDIF
 IF ((((dm_err->err_ind=1)) OR (dsme_status="E")) )
  SET dsme_status = "E"
 ELSEIF (dsme_err_found_ind=1)
  SET dsme_status = "R"
 ELSE
  SET dsme_status = "G"
 ENDIF
 IF (dshc_add_check_to_rs("MSGVIEW ERRORS",
  "Foreign System Interface Application Servers MSGVIEW Errors",0,dsme_status,dshc_ctf_millennium)=0)
  SET dsme_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 IF (dsme_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "CCL errors logged on the following servers may indicate issues sending outbound transactions.  ",
  "Contact the IAC to correct any CCL errors logged.")
 SELECT INTO "nl:"
  d.seq
  FROM (dummyt d  WITH seq = servers_w_errors->cnt)
  WHERE (servers_w_errors->server[d.seq].err_ind=1)
  DETAIL
   dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1, stat = alterlist(dshc->check[dshc->chk_cnt].
    desc_detail,dshc->check[dshc->chk_cnt].desc_dtl_cnt), dshc->check[dshc->chk_cnt].desc_detail[dshc
   ->check[dshc->chk_cnt].desc_dtl_cnt].name = "MSGVIEW Error",
   dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].value_prefix =
   "NOT_SET", dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].
   dtl_desc_text = "NOT_SET", dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].
   desc_dtl_cnt].type = 2,
   dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].str_value =
   servers_w_errors->server[d.seq].server_name
  WITH nocounter
 ;end select
#exit_script
 SET dm_err->eproc = "Finished checking MSGView for CCL errors."
 CALL final_disp_msg("dm2_shc_msgview")
 IF (dsme_fatal_err_ind=0)
  SET dm_err->err_ind = 0
 ENDIF
END GO
