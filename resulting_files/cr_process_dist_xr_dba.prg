CREATE PROGRAM cr_process_dist_xr:dba
 RECORD reply(
   1 qual[1]
 ...
 DECLARE prov_routing_ordering_clause = vc WITH noconstant("1=1")
...
")
 ...
 IF (person_level_cnt > 0)
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   HEAD REPORT
    person_r_cnt = 0
   DETAIL
    person_r_cnt += 1, provider_reltn_cnt += 1
 ...
 IF (encntr_level_cnt > 0)
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   HEAD REPORT
    encntr_r_cnt = 0
   DETAIL
    encntr_r_cnt += 1, provider_reltn_cnt += 1
    IF (mod(provider_reltn_cnt,10)=1)
     stat = alterlist(provider_reltn_rec->qual,(provider_reltn_cnt+ 9))
    ENDIF
 ...
 IF (order_level_cnt > 0)
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   HEAD REPORT
    order_r_cnt = 0
   DETAIL
    order_r_cnt += 1, provider_reltn_cnt += 1
    IF (mod(provider_reltn_cnt,10)=1)
     stat = alterlist(provider_reltn_rec->qual,(provider_reltn_cnt+ 9))
    ENDIF
 ...
 ELSEIF (scope=4)
  ...
  SELECT DISTINCT INTO "NL:"
   ...
   ORDER BY ce.accession_nbr, ce.encntr_id, ce.event_id,
    ce.order_id, 0
   HEAD REPORT
    do_nothing = 0
   ...
   HEAD ce.order_id
    IF (ce.order_id > 0)
     idx = 0, accnindex = locateval(idx,1,size(chart_rec->qual,5),ce.accession_nbr,chart_rec->qual[
      idx].chart_acc,
      ce.encntr_id,chart_rec->qual[idx].encntr_id), idx = 0
   ...
   SELECT DISTINCT INTO "NL:"
    ...
    HEAD clr.linked_event_id
     IF (scope=4)
      ...
      accnindex = locateval(idx,1,size(chart_rec->qual,5),ce.accession_nbr,chart_rec->qual[idx].
       chart_acc,
       ce.encntr_id,chart_rec->qual[idx].encntr_id)
     ENDIF
     idx = 0
     IF (checkxencntr=0
      AND ((scope IN (1, 2, 5)
    ...
    HEAD ce.order_id
     IF (ce.order_id > 0
      ...
      accnindex = locateval(idx,1,size(chart_rec->qual,5),ce.accession_nbr,chart_rec->qual[idx].
       chart_acc,
       ce.encntr_id,chart_rec->qual[idx].encntr_id), crindex = 0
     ...
     SELECT INTO "NL:"
      ...
      PLAN (d)
       ...
       WHERE (ce.parent_event_id=mbo_flat_rec->qual[d.seq].event_id)
        ...
        AND parser(cs_where_clause))
      ORDER BY ce.accession_nbr, ce.encntr_id, ce.order_id
      HEAD ce.accession_nbr
       do_nothing = 0
      HEAD ce.encntr_id
 ...
 IF (count3 > 0)
  ...
  SELECT INTO "nl:"
   ...
   ENDIF
   , scope_order3 = chart_rec->qual[d4.seq].encntr_id,
   ...
   mrn4 = decode(ea.seq,build(substring(197,4,format(cnvtalphanum(ea.alias),";p0")),substring(193,4,
      format(cnvtalphanum(ea.alias),";p0")))," "), mrn = substring(1,30,format(cnvtalphanum(ea.alias),
     "##############################"))
   ...
   ORDER BY scope_order2, scope_order3, cpr2.prsnl_person_id,
   ...
   HEAD scope_order3
    ...
    IF (scope=4)
     accnindex = locateval(idx,1,size(chart_rec->qual,5),scope_order2,chart_rec->qual[idx].chart_acc,
      scope_order3,chart_rec->qual[idx].encntr_id), order_cnt = 0, stat = alterlist(temp_request->
      qual[req_cnt].orders,size(chart_rec->qual[accnindex].orders,5))
   ...
   DETAIL
    ...
    ELSEIF (scope=6) cnvtstring(chart_rec->qual[d4.seq].event_id)
    ENDIF
    ,cpr2.prsnl_person_id,cpr2.chart_prsnl_r_type_cd,chart_rec->qual[d4.seq].encntr_id)
   FOOT  scope_order3
    ...
    DETAIL
     IF (od.oe_field_value > 0)
      ...
      IF (od.oe_field_meaning_id=oe_field_meaning_id_consultdoc)
       ...
       temp_request->qual[d7.seq].encntr_id), temp_request->qual[d7.seq].prov[(cnt1+ prov_cnt)].r_cd
        = consult_doc_cd
      ELSE
       CALL addrelationship(temp_request->qual[d7.seq].accession_nbr,od.oe_field_value,cc_provider_cd,
       temp_request->qual[d7.seq].encntr_id)
      ENDIF
     ENDIF
  ...
  IF (scope=4
   AND order_doc_ind=1
   AND max_order_count > 0)
   SET prov_routing_ordering_clause =
   " cop.charting_operations_id = outerjoin(charting_operations_id)"
   IF (prov_routing_flag=3
    AND  NOT (multitenancy=1
    AND is_logical_domain_enabled=0))
    SET prov_routing_ordering_clause = concat(prov_routing_ordering_clause,
     " or            cop.charting_operations_id = outerjoin(0)")
   ENDIF
   SELECT DISTINCT INTO "nl:"
    ...
    PLAN (d7
     ...
     WHERE (cop.prsnl_id= Outerjoin(oa.order_provider_id))
      AND parser(prov_routing_ordering_clause))
     JOIN (dx
    ...
    HEAD oa.order_id
     IF (((order_doc_flag=1) OR (order_doc_flag=2)) )
      IF (((((prov_routing_flag=2) OR (prov_routing_flag=3))
       ...
       order_doc_cd,temp_request->qual[d7.seq].encntr_id)
      ENDIF
     ENDIF
    DETAIL
     IF (order_doc_flag=3)
    ...
    FOR (j = 1 TO size(temp_request->qual[i].prov,5))
      IF ((temp_request->qual[i].prov[j].copy_ind=1))
       ...
       IF (ismultirelational(
        ...
        ENDIF
        ,temp_request->qual[i].prov[j].person_id,prelim_rec->qual[new_cnt].encntr_id))
...
"
 ...
 IF (req_cnt > 0)
  ...
  SELECT INTO "nl:"
   ...
   HEAD REPORT
    req_cnt = 0, request->output_dist = output_dist, request->batch_selection = trim(batch_selection),
...
#exit_script
 ...
 SUBROUTINE (addrelationship(identifier_id=vc(val),personnel_id=f8(val),reltn_type_cd=f8(val),
  encntr_id=f8(val)) =null)
   SET identidx = 0
   SET prsnlidx = 0
   SET reltnidx = 0
   SET idx = 0
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (ismultirelational(identifier_id=vc(val),personnel_id=f8(val),encntr_id=f8(val)) =i2)
