CREATE PROGRAM dm2_ref_data_mover_5319:dba
 ...
 DECLARE cust_pref_cd = f8
 DECLARE cust_pref_domain = vc
 DECLARE cust_pref_name = vc
 DECLARE cust_process_tab = vc
 DECLARE cust_pref_section = vc
 ...
 SET cust_pref_domain = get_value(cust_tab_name,"PREF_DOMAIN","FROM")
 SET cust_pref_cd = cnvtreal(get_value(cust_tab_name,"PREF_CD","FROM"))
 SET cust_pref_name = get_value(cust_tab_name,"PREF_NAME","FROM")
 SET cust_pref_section = get_value(cust_tab_name,"PREF_SECTION","FROM")
 CASE (cust_pref_domain)
  OF "PHARMNET":
  OF "PHARMNET-INPATIENT":
  OF "PHARMNET-RETAIL":
   CALL put_value(cust_tab_name,"PREF_CD",cnvtstring(cust_pref_cd))
  OF "CVNET":
   IF (cust_pref_section="OPTIONAL_FIELD*")
    SET cust_process_tab = "CV_XREF"
    SET cust_process_col = "XREF_ID"
   ELSE
    CALL put_value(cust_tab_name,"PREF_CD",cnvtstring(cust_pref_cd))
   ENDIF
  OF "PATHNET GLB":
   IF (cust_pref_name="ConsistentComment")
    SET cust_process_tab = "WP_TEMPLATE"
    SET cust_process_col = "TEMPLATE_ID"
   ELSEIF (cust_pref_name="SubSection IQ Exception")
 ...
 ENDCASE
 IF (cust_process_tab > " ")
  IF (cnvtreal(cust_pref_cd) > 0)
   SET new_value = select_merge_translate(cnvtstring(cust_pref_cd),cust_process_tab)
   IF (new_value != "No Trans")
    CALL put_value(cust_tab_name,"PREF_CD",cnvtstring(cust_pref_cd))
   ELSE
    SET cust_rpt_missing = report_missing(cust_process_tab,cust_process_col,cust_pref_cd)
 ...
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_5319
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"PREF_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"PREF_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"PREF_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"PREF_ID")
     ELSE
      GO TO exit_5319
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"PREF_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5319
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"PREF_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5319
   ELSE
