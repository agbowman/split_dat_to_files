CREATE PROGRAM dm_rxs_forgn_device_alert_rows:dba
 ...
 SET reply->table_name = "RXS_FOREIGN_DEVICE_ALERT"
 ...
 ELSE
  SELECT INTO "nl:"
   rfda.rowid
   FROM rxs_foreign_device_alert rfda
   WHERE parser(sbr_getrowidnotexists("rfda.updt_dt_tm < cnvtdatetime(v_days_back)","rfda"))
    AND ((rfda.rxs_foreign_device_alert_id+ 0) > 0)
   DETAIL
    row_cnt = (row_cnt+ 1)
    IF (mod(row_cnt,50)=1)
     stat = alterlist(reply->rows,(row_cnt+ 49))
    ENDIF
    reply->rows[row_cnt].row_id = rfda.rowid
   WITH nocounter, maxqual(rfda,value(request->max_rows))
  ;end select
...
#exit_script
 CALL echo("Last mod date: 11/15/10 last mod version:001")
END GO
