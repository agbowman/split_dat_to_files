CREATE PROGRAM dm_cdi_batch_summary_rows:dba
 ...
 SET reply->table_name = "CDI_BATCH_SUMMARY"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(bs.cdi_batch_summary_id)
    FROM cdi_batch_summary bs
    WHERE bs.cdi_batch_summary_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   seqval = max(bs.cdi_batch_summary_id)
   FROM cdi_batch_summary bs
   DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     bs.rowid
     FROM cdi_batch_summary bs
     WHERE parser(sbr_getrowidnotexists("bs.cdi_batch_summary_id between curMinID and curMaxID","bs")
      )
      AND bs.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
      AND bs.cur_auto_cnt=0
      AND bs.cur_man_cnt=0
      AND  NOT ( EXISTS (
     (SELECT
      b.cdi_ac_batch_id
      FROM cdi_ac_batch b
      WHERE b.cdi_ac_batch_id=bs.cdi_ac_batch_id
       AND b.cdi_ac_batch_id > 0
       AND ((b.updt_dt_tm >= cnvtdatetime((curdate - v_days_to_keep),curtime3)) OR (b.updt_dt_tm <
      cnvtdatetime((curdate - v_days_to_keep),curtime3)
       AND  EXISTS (
      (SELECT
       m.cdi_ac_batch_id
       FROM cdi_ac_batchmodule m
       WHERE m.cdi_ac_batch_id=b.cdi_ac_batch_id
        AND m.startdatetime >= cnvtdatetime((curdate - v_days_to_keep),curtime3))))) )))
      AND  NOT ( EXISTS (
     (SELECT
      t.cdi_trans_log_id
      FROM cdi_trans_log t
      WHERE t.external_batch_ident=bs.external_batch_ident
       AND t.create_dt_tm=bs.create_dt_tm
       AND t.updt_dt_tm >= cnvtdatetime((curdate - v_days_to_keep),curtime3))))
      AND ((bs.ac_rel_cnt - bs.combined_cnt) <= bs.complete_cnt)
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,50)=1)
       stat = alterlist(reply->rows,(rows+ 49))
      ENDIF
      reply->rows[rows].row_id = bs.rowid
     WITH nocounter, maxqual(bs,value(rowsleft))
    ;end select
