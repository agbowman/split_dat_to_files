CREATE PROGRAM dm_rc_cont_process_hist_rows:dba
 ...
 SET reply->table_name = "RC_CONT_PROCESS_HIST"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(ph.rc_cont_process_hist_id)
   FROM rc_cont_process_hist ph
   WHERE ph.rc_cont_process_hist_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(ph.rc_cont_process_hist_id)
  FROM rc_cont_process_hist ph
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    ph.rowid
    FROM rc_cont_process_hist ph
    WHERE ph.rc_cont_process_hist_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "ph.beg_effective_dt_tm < cnvtdatetime(curdate - v_days_to_keep, 0)","ph"))
    DETAIL
     v_rows += 1
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = ph.rowid
    WITH nocounter, maxqual(ph,value(v_rows_left))
   ;end select
