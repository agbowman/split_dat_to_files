CREATE PROGRAM dm2_ref_data_mover_0373:dba
 ...
 END ;Subroutine
 DECLARE from_cd = vc
 DECLARE src_org_id = f8
 DECLARE tgt_org_id = vc
 DECLARE cust_hp_id = f8
 DECLARE ui_cnt = i4
 DECLARE val = f8
 DECLARE ins_upd = i2
 DECLARE idcd_check = i2
 DECLARE cust_tab_name = vc
 DECLARE op_tab_name = vc
 DECLARE hp_check = i2
 DECLARE op_tab_string = vc
 DECLARE oplan_type = f8
 DECLARE to_plan_type = vc
 DECLARE plan_name = vc
 DECLARE plan_name_key = vc
 DECLARE hplan_type = vc
 DECLARE org_rpt_missing = vc
 DECLARE cust_to_hp_id = f8
 DECLARE utc_tab_string = vc
 DECLARE ut_tab_string = vc
 DECLARE fbs_tab_string = vc
 DECLARE fb_id_value = vc
 DECLARE tgt_fb_id = vc
 DECLARE fb_rpt_missing = vc
 DECLARE hp_id_value = vc
 FREE RECORD cust_hp
 RECORD cust_hp(
   1 qual[*]
     2 hp_id = f8
 ...
 SET op_tab_name = "ORG_PLAN_RELTN"
 SET cust_hp_id = cnvtreal(get_value(cust_tab_name,"HEALTH_PLAN_ID","FROM"))
 SET hplan_type = get_value(cust_tab_name,"PLAN_TYPE_CD","FROM")
 SET plan_name = get_value(cust_tab_name,"PLAN_NAME","FROM")
 SET plan_name_key = get_value(cust_tab_name,"PLAN_NAME_KEY","FROM")
 SET oplan_type = uar_get_code_by("MEANING",370,"CARRIER")
 IF (oplan_type <= 0)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "Carrier code_value couldn't be determined."
 ENDIF
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_0373
 ENDIF
 SET to_plan_type = select_merge_translate(cnvtstring(oplan_type),"CODE_VALUE")
 IF (to_plan_type="No Trans")
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "Carrier code_value couldn't be translated."
  SET org_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",oplan_type)
  CALL evaluate_rpt_missing(org_rpt_missing)
  SET org_rpt_missing = ""
  GO TO exit_0373
 ENDIF
 SET ui_cnt = 0
 SET hp_check = is_translated(cust_tab_name,"HEALTH_PLAN_ID")
 IF (hp_check=0)
  SET op_tab_string = dm2_get_rdds_tname(op_tab_name)
  SELECT INTO "NL:"
   FROM (parser(op_tab_string) b)
   WHERE b.health_plan_id=cust_hp_id
    AND b.org_plan_reltn_cd=oplan_type
   DETAIL
    ui_cnt = (ui_cnt+ 1), src_org_id = b.organization_id
   WITH nocounter
  ;end select
  ...
  IF (ui_cnt > 1)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script,
    ": There were multiple relations in the ORG_PLAN_RELTN table found.")
   CALL merge_audit("FAILREASON","There were multiple relations in the ORG_PLAN_RELTN table found.",1
    )
   IF (drdm_error_out_ind=1)
    ROLLBACK
    GO TO exit_0373
   ...
   GO TO exit_0373
  ENDIF
  SET utc_tab_string = dm2_get_rdds_tname("user_tab_columns")
  SELECT INTO "NL:"
   FROM dtableattr a,
    dtableattrl l,
    (parser(utc_tab_string) utc)
   PLAN (a
    WHERE a.table_name="HEALTH_PLAN")
    JOIN (l
    WHERE l.structtype="F"
     AND btest(l.stat,11)=0
     AND l.attr_name="FB_BENEFIT_SET_ID")
    JOIN (utc
    WHERE utc.table_name=a.table_name
     AND utc.column_name=l.attr_name)
   WITH nocounter
  ;end select
  IF (check_error(dm2_ref_data_reply->error_msg) > 0)
   SET dm2_ref_data_reply->error_ind = 1
   GO TO exit_0373
  ENDIF
  IF (curqual > 0)
   SET ut_tab_string = dm2_get_rdds_tname("user_tables")
   SELECT INTO "NL:"
    FROM dtableattr a,
     (parser(ut_tab_string) utc)
    PLAN (a
     WHERE a.table_name="FB_BENEFIT_SET")
     JOIN (utc
     WHERE utc.table_name=a.table_name)
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF (curqual > 0)
    SET fbs_tab_string = dm2_get_rdds_tname("FB_BENEFIT_SET")
    SET fb_id_value = get_value("FB_BENEFIT_SET","FB_BENEFIT_SET_ID","FROM")
    SELECT INTO "NL:"
     FROM (parser(fbs_tab_string) f)
     WHERE f.fb_benefit_set_id=cnvtreal(fb_id_value)
     WITH nocounter
    ;end select
    IF (check_error(dm2_ref_data_reply->error_msg) > 0)
     SET dm2_ref_data_reply->error_ind = 1
    ...
    ENDIF
    IF (curqual > 0)
     SET tgt_fb_id = select_merge_translate(fb_id_value,"FB_BENEFIT_SET")
     IF (tgt_fb_id="No Trans")
      SET dm2_ref_data_reply->error_ind = 1
      SET dm2_ref_data_reply->error_msg = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
       custom_script,": The FB_BENEFIT_SET_ID column has not been translated yet.")
      CALL merge_audit("FAILREASON","The FB_BENEFIT_SET_ID column has not been translated yet.",2)
      IF (drdm_error_out_ind=1)
       ROLLBACK
       GO TO exit_0373
      ENDIF
      SET fb_rpt_missing = report_missing("FB_BENEFIT_SET","FB_BENEFIT_SET_ID",cnvtreal(fb_id_value))
      CALL evaluate_rpt_missing(fb_rpt_missing)
      GO TO exit_0373
     ELSE
      CALL put_value("HEALTH_PLAN","FB_BENEFIT_SET_ID",tgt_fb_id)
     ENDIF
    ELSEIF (curqual=0)
     SET hp_id_value = get_value("HEALTH_PLAN","FB_BENEFIT_SET_ID","FROM")
     CALL put_value("HEALTH_PLAN","FB_BENEFIT_SET_ID",hp_id_value)
    ENDIF
   ELSEIF (curqual=0)
    SET hp_id_value = get_value("HEALTH_PLAN","FB_BENEFIT_SET_ID","FROM")
    CALL put_value("HEALTH_PLAN","FB_BENEFIT_SET_ID",hp_id_value)
   ENDIF
  ENDIF
  SET tgt_org_id = select_merge_translate(cnvtstring(src_org_id),"ORGANIZATION")
  IF (tgt_org_id="No Trans")
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script,
    ": The ORGANIZATION_ID column has not been translated yet.")
   CALL merge_audit("FAILREASON","The ORGANIZATION_ID column has not been translated yet.",2)
   IF (drdm_error_out_ind=1)
    ROLLBACK
    GO TO exit_0373
   ENDIF
   SET org_rpt_missing = report_missing("ORGANIZATION","ORGANIZATION_ID",src_org_id)
   CALL evaluate_rpt_missing(org_rpt_missing)
   GO TO exit_0373
  ENDIF
  SET ui_cnt = 0
  SELECT INTO "NL:"
   FROM health_plan h
   WHERE h.plan_type_cd=cnvtreal(hplan_type)
    AND h.plan_name=plan_name
    AND h.plan_name_key=plan_name_key
    AND h.health_plan_id IN (
   (SELECT
    o.health_plan_id
    FROM (parser(op_tab_name) o)
    WHERE o.org_plan_reltn_cd=cnvtreal(to_plan_type)
     AND o.organization_id=cnvtreal(tgt_org_id)))
   HEAD REPORT
    ui_cnt = 0, stat = alterlist(ui_query_eval_rec->qual,10)
   DETAIL
    ui_cnt = (ui_cnt+ 1)
    IF (mod(ui_cnt,10)=1
     AND ui_cnt != 1)
     stat = alterlist(ui_query_eval_rec->qual,(ui_cnt+ 9))
    ENDIF
    ui_query_eval_rec->qual[ui_cnt].root_entity_attr = h.health_plan_id
   FOOT REPORT
    stat = alterlist(ui_query_eval_rec->qual,ui_cnt)
   WITH nocounter
  ;end select
  IF (error(dm2_ref_data_reply->error_msg,0) > 0)
   SET dm2_ref_data_reply->error_ind = 1
   GO TO exit_0373
  ENDIF
  SET cust_to_hp_id = evaluate_exec_ui_query(ui_cnt,temp_tbl_cnt,perm_col_cnt)
  IF (cust_to_hp_id < 0)
   IF ((cust_to_hp_id=- (3)))
    SET val = get_seq(cust_tab_name,"HEALTH_PLAN_ID")
    IF (val > 0)
     CALL put_value(cust_tab_name,"HEALTH_PLAN_ID",cnvtstring(val))
     SET current_merges = (current_merges+ 1)
     SET child_merge_audit->num[current_merges].action = "NEWSEQ"
     SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  HEALTH_PLAN_ID")
    ELSE
     GO TO exit_0373
    ENDIF
   ELSEIF ((cust_to_hp_id=- (2)))
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "Multiple rows returned from UI query."
    CALL merge_audit("FAILREASON","There were multiple returned from the UI",1)
    IF (drdm_error_out_ind=1)
     ROLLBACK
     GO TO exit_0373
    ENDIF
    ...
    SET dm2_ref_data_reply->error_msg = "NOMV06"
    GO TO exit_0373
   ELSEIF ((cust_to_hp_id=- (1)))
    GO TO exit_0373
   ENDIF
  ELSE
   CALL put_value(cust_tab_name,"HEALTH_PLAN_ID",cnvtstring(cust_to_hp_id))
   SET current_merges = (current_merges+ 1)
  ...
  SET from_cd = get_value(cust_tab_name,"HEALTH_PLAN_ID","FROM")
  IF ((dm2_ref_data_reply->error_ind=1))
   GO TO exit_0373
  ELSE
   SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(from_cd)
  ENDIF
  SET from_cd = get_value(cust_tab_name,"HEALTH_PLAN_ID","TO")
  IF ((dm2_ref_data_reply->error_ind=1))
   GO TO exit_0373
  ELSE
   SET dm2_ref_data_reply->qual[1].to_value = cnvtreal(from_cd)
  ENDIF
