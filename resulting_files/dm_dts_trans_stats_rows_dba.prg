CREATE PROGRAM dm_dts_trans_stats_rows:dba
 ...
 SET reply->table_name = "DTS_TRANS_STATS"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(d.dts_trans_stats_id)
   FROM dts_trans_stats d
   WHERE d.dts_trans_stats_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(d.dts_trans_stats_id)
  FROM dts_trans_stats d
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    d.rowid
    FROM dts_trans_stats d
    WHERE parser(sbr_getrowidnotexists("d.dts_trans_stats_id between curMinID and curMaxID","d"))
     AND d.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
