CREATE PROGRAM dm_ins_dm_ref_domain_r:dba
 SET c_mod = "DM_INS_DM_REF_DOMAIN_R 000"
 SET debug_ind = 0
 IF ( NOT (validate(i_debug_ind,0)=0
  AND validate(i_debug_ind,1)=1))
  SET debug_ind = i_debug_ind
 ENDIF
 SET csv_str = "refdomain_grp_r.csv"
 ...
 IF (qual_cnt > 0)
  FOR (mrg_cnt = 1 TO qual_cnt)
   SELECT INTO "nl:"
    ...
    WHERE r.group_name=cnvtupper(requestin->list_0[mrg_cnt].group_name)
     AND r.ref_domain_name=cnvtupper(requestin->list_0[mrg_cnt].ref_domain_name)
    WITH nocounter
   ;end select
   IF (curqual=0)
    INSERT  FROM dm_ref_domain_r r
     SET r.group_name = cnvtupper(requestin->list_0[mrg_cnt].group_name), r.ref_domain_name =
      cnvtupper(requestin->list_0[mrg_cnt].ref_domain_name)
     WITH nocounter
    ;end insert
    COMMIT
   ENDIF
