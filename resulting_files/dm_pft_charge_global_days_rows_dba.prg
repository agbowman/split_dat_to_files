CREATE PROGRAM dm_pft_charge_global_days_rows:dba
 ...
 DECLARE date_to_persist = dq8
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 SET reply->table_name = "PFT_CHARGE_GLOBAL_DAYS"
 ...
 IF (months_to_keep < 6)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"MONTHSTOKEEP",
   "You must keep at least 6 month's worth of data.  You entered %1 months or did not enter any value.",
   "i",months_to_keep)
 ELSE
  SET date_to_persist = cnvtlookbehind(build(months_to_keep,",M"),cnvtdatetime(curdate,curtime3))
  SELECT INTO "nl:"
   pcg.rowid
   FROM pft_charge_global_days pcg
   WHERE parser(sbr_getrowidnotexists("pcg.updt_dt_tm < cnvtdatetime(date_to_persist)","pcg"))
    AND pcg.pft_charge_global_days_id != 0
   HEAD REPORT
    row_cnt = 0
   DETAIL
    row_cnt = (row_cnt+ 1)
    ...
    ENDIF
    reply->rows[row_cnt].row_id = pcg.rowid
   FOOT REPORT
    stat = alterlist(reply->rows,row_cnt)
   WITH nocounter, maxqual(pcg,value(request->max_rows))
  ;end select
