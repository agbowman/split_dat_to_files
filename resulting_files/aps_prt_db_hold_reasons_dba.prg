CREATE PROGRAM aps_prt_db_hold_reasons:dba
 ...
 SET captions->apsrpt = uar_i18ngetmessage(i18nhandle,"h1","REPORT:  APS_PRT_DB_HOLD_REASONS.PRG")
 ...
 SET captions->dbholdreasons = uar_i18ngetmessage(i18nhandle,"h7","DB HOLD REASONS TOOL")
 ...
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
  WHERE cv.code_set=1300
  HEAD REPORT
   spec_cnt = 0
  DETAIL
   spec_cnt = (spec_cnt+ 1), stat = alterlist(temp->qual,spec_cnt), temp->qual[spec_cnt].spec_display
    = cv.display,
   temp->qual[spec_cnt].spec_display_key = cv.display_key, temp->qual[spec_cnt].spec_active_ind = cv
   .active_ind
  WITH nocounter
 ;end select
#report_maker
 EXECUTE cpm_create_file_name_logical "apsDbHoldReasons", "dat", "x"
 ...
 SELECT INTO value(reply->print_status_data.print_filename)
  ...
  HEAD PAGE
   ...
   row + 1,
   CALL center(captions->dbholdreasons,0,132), col 110,
  ...
  DETAIL
   ...
   ELSE
    captions->active
   ENDIF
   col 10, temp->qual[d.seq].spec_display
   IF (((row+ 7) > maxrow))
    BREAK
   ENDIF
  FOOT PAGE
