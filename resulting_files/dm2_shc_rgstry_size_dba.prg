CREATE PROGRAM dm2_shc_rgstry_size:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsrs_err_status = c1 WITH protect, noconstant(" ")
 DECLARE dsrs_file = vc WITH protect, noconstant(" ")
 DECLARE dsrs_cmd1 = vc WITH protect, noconstant(" ")
 DECLARE dsrs_cmd2 = vc WITH protect, noconstant(" ")
 DECLARE dsrs_del_cmd = vc WITH protect, noconstant(" ")
 DECLARE dsrs_size = i4 WITH protect, noconstant(0)
 DECLARE dsrs_free = i4 WITH protect, noconstant(0)
 DECLARE tot_pgs_pct_free_ind = i2 WITH protect, noconstant(0)
 DECLARE dsrs_get_rgs_siz(null) = null
 IF (check_logfile("DM2_SHC_RGSTRY_SIZE",".log","DM2_SHC_RGSTRY_SIZE LOG FILE")=0)
  SET dsrs_err_status = "F"
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "System Health Check working on Registry Files Size ..."
 CALL disp_msg("",dm_err->logfile,0)
 ...
 IF (get_unique_file("dm2_dshc_output",".dat"))
  SET dsrs_file = dm_err->unique_fname
  IF (cursys="AXP")
   SET dsrs_cmd1 = concat("backup/ignore=interlock cer_reg:registry.dat  ccluserdir:",dsrs_file)
   SET dm_err->disp_dcl_err_ind = 0
   IF (dm2_push_dcl(dsrs_cmd1)=0)
    IF ((dm_err->err_ind=1))
    ...
    ELSEIF (findstring("is open for write by another user",dm_err->emsg)=0)
     ...
     SET dsrs_err_status = "E"
    ELSE
     CALL dsrs_get_rgs_siz(null)
    ENDIF
   ELSE
    CALL dsrs_get_rgs_siz(null)
   ENDIF
  ELSEIF (cursys="AIX")
   SET dsrs_cmd1 = concat("cp $cer_reg/registry.dat $CCLUSERDIR/",dsrs_file)
   IF (dm2_push_dcl(dsrs_cmd1)=0)
    SET dsrs_err_status = "E"
   ELSE
    CALL dsrs_get_rgs_siz(null)
  ...
  FREE SET file_loc
  IF (cursys="AIX")
   SET dsrs_del_cmd = concat("rm -f $CCLUSERDIR/",dsrs_file)
  ELSEIF (cursys="AXP")
   SET dsrs_del_cmd = concat("delete ccluserdir:",dsrs_file,";*")
  ENDIF
 ...
 ELSE
  SET dsrs_err_status = "E"
 ENDIF
 IF (dsrs_err_status != "E")
  IF ((dsrs_free >= (dsrs_size * 0.5)))
   SET tot_pgs_pct_free_ind = 1
  ELSE
   SET tot_pgs_pct_free_ind = 0
  ENDIF
  IF (dsrs_size > 50000
   AND tot_pgs_pct_free_ind=1)
   SET dsrs_err_status = "G"
  ELSE
   SET dsrs_err_status = "R"
  ENDIF
 ENDIF
 IF (dshc_add_check_to_rs("REGISTRY FILES SIZE","Registry Files Size",0,dsrs_err_status,
  dshc_ctf_millennium)=0)
  SET dsrs_err_status = "F"
  GO TO exit_script
 ENDIF
 IF (dsrs_err_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 ...
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Registry total pages"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Total Pages in Registry:"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 1
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].nbr_value = dsrs_size
 SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
 SET cnt = dshc->check[dshc->chk_cnt].desc_dtl_cnt
 SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,cnt)
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].name = "Total Pages 50% Free"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].value_prefix = "Total Pages 50% Free:"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].dtl_desc_text = "NOT_SET"
 SET dshc->check[dshc->chk_cnt].desc_detail[cnt].type = 2
 IF (tot_pgs_pct_free_ind=1)
 ...
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "Cerner recommends that the Millennium registry file contain at least 50,000 pages.  ",
  "Cerner also recommends that the registry be made up of at least 50 percent free pages.")
#exit_script
 IF ((dm_err->debug_flag >= 5))
  CALL echorecord(dshc)
  CALL echorecord(dm_err)
 ENDIF
 IF (dsrs_err_status="F")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "EXITING DM2_SHC_RGSTRY_SIZE WITH ERRORS"
 ELSEIF ((dm_err->err_ind=1))
  SET dm_err->err_ind = 0
  SET dm_err->eproc = "EXITING DM2_SHC_RGSTRY_SIZE WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_SHC_RGSTRY_SIZE COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("DM2_SHC_RGSTRY_SIZE")
 SUBROUTINE dsrs_get_rgs_siz(null)
  IF (cursys="AIX")
   SET dsrs_cmd2 = concat("$cer_exe/pagemgr -dump $CCLUSERDIR/",dsrs_file)
  ELSE
   SET dsrs_cmd2 = concat("mcr cer_exe:pagemgr -dump ccluserdir:",dsrs_file)
  ENDIF
  IF (dm2_push_dcl(dsrs_cmd2)=0)
   SET dsrs_err_status = "E"
  ELSE
   FREE DEFINE rtl2
   FREE SET file_loc
   ...
   SELECT INTO "nl:"
    ...
    FROM rtl2t r
    HEAD REPORT
     dsrs_len = 0, beg_pos = 0, dsrs_temp_str = fillstring(132," ")
    DETAIL
     dsrs_len = 0, beg_pos = 0, dsrs_temp_str = fillstring(132," "),
     beg_pos = findstring("size:",r.line)
     IF (beg_pos > 0)
      dsrs_len = textlen(trim(r.line))
      IF ((dm_err->debug_flag >= 2))
       CALL echo(build("len=",dsrs_len))
      ENDIF
      dsrs_temp_str = substring((beg_pos+ 5),dsrs_len,r.line)
      IF ((dm_err->debug_flag >= 2))
       CALL echo(build("dsrs_temp_str=",dsrs_temp_str))
      ENDIF
      WHILE (substring(1,1,dsrs_temp_str)=" ")
        dsrs_temp_str = replace(dsrs_temp_str," ","",1)
      ENDWHILE
      IF ((dm_err->debug_flag >= 2))
       CALL echo(build("dsrs_temp_str=",dsrs_temp_str))
      ENDIF
      beg_pos = findstring(" ",dsrs_temp_str)
      IF ((dm_err->debug_flag >= 2))
       CALL echo(build("beg_pos=",beg_pos))
      ENDIF
      dsrs_size = cnvtint(substring(1,(beg_pos - 1),dsrs_temp_str))
      IF ((dm_err->debug_flag >= 1))
       CALL echo(build("size=",dsrs_size))
      ENDIF
     ELSE
      beg_pos = findstring("free:",r.line)
      IF ((dm_err->debug_flag >= 2))
       CALL echo(build("beg_pos=",beg_pos))
      ENDIF
      IF (beg_pos > 0)
       ...
       IF ((dm_err->debug_flag >= 2))
        CALL echo(build("dsrs_temp_str=",dsrs_temp_str))
       ENDIF
       beg_pos = findstring(" ",dsrs_temp_str), dsrs_free = cnvtint(substring(1,(beg_pos - 1),
         dsrs_temp_str))
       IF ((dm_err->debug_flag >= 1))
        CALL echo(build("free=",dsrs_free))
       ENDIF
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(concat("Parse out registry size.")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dsrs_err_status = "E"
   ENDIF
  ENDIF
