CREATE PROGRAM dm_rmc_mover_0635:dba
 ...
 SUBROUTINE drmm_breakup_string(sbr_rs_reply)
   ...
   RETURN("S")
 END ;Subroutine
 DECLARE sbr_drdm_get_cv_trans_value(sbr_get_field_value=vc) = i2
 DECLARE sbr_drdm_get_prsnl_trans_value(i_field_value=vc) = i2
 DECLARE idcd_check = i2
 DECLARE cust_tab_name = vc
 DECLARE v_code_set = vc
 DECLARE v_code_value = vc
 DECLARE v_t_code_value = vc
 DECLARE v_field_name = vc
 DECLARE v_t_field_name = vc
 DECLARE v_str_temp = vc
 DECLARE v_cd = f8
 DECLARE v_alg_type = vc
 DECLARE v_field_value = vc
 DECLARE v_t_col_cnt = i4
 DECLARE v_cdf = vc
 DECLARE v_sbr_drdm_rtn_status = i2
 DECLARE v_sbr_drdm_prsnl_rtn_status = i2
 DECLARE v_code_meaning = vc
 DECLARE cust_err_val = i4
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET v_code_set = get_value(cust_tab_name,"CODE_SET","FROM")
 SET v_code_value = get_value(cust_tab_name,"CODE_VALUE","FROM")
 SET v_field_name = get_value(cust_tab_name,"FIELD_NAME","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_0635
 ENDIF
 SET v_sbr_drdm_rtn_status = 1
 SET v_t_col_cnt = - (1)
 SET v_str_temp = build(v_code_set,":",v_field_name)
 SELECT INTO "NL:"
  FROM code_value cv
  WHERE cv.display=v_str_temp
   AND cv.code_set=361571
   AND cv.active_ind=1
  DETAIL
   v_cd = cv.code_value
  WITH nocounter
 ;end select
 IF (check_error("Obtaining code value")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_0635
 ENDIF
 IF (curqual=0)
 ...
 ELSEIF (curqual > 1)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "NOMV45"
 ...
 IF (curqual=0)
  ...
  ENDIF
  IF (curqual=0)
   SET v_t_col_cnt = get_col_pos(cust_tab_name,"CODE_VALUE")
   IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[v_t_col_cnt].translated=1))
    SET v_t_code_value = get_value(cust_tab_name,"CODE_VALUE","TO")
    SELECT INTO "NL:"
     FROM code_value c
     WHERE c.code_value=cnvtreal(v_t_code_value)
     DETAIL
      v_str_temp = concat(v_code_set,"-",c.cdf_meaning)
     WITH nocounter
    ;end select
 ...
 IF (v_cd > 0)
  SELECT INTO "NL:"
   FROM code_value cv
   WHERE cv.code_value=v_cd
    AND cv.active_ind=1
   DETAIL
    v_alg_type = cv.cdf_meaning
   WITH nocounter
  ;end select
  IF (check_error("Obtaining CDF meaning")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_0635
  ENDIF
  SET v_field_value = get_value(cust_tab_name,"FIELD_VALUE","FROM")
  CASE (v_alg_type)
   OF "ALG1":
    SET v_sbr_drdm_rtn_status = sbr_drdm_get_cv_trans_value(v_field_value)
   OF "ALG2":
    SET v_t_col_cnt = get_col_pos(cust_tab_name,"CODE_VALUE")
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[v_t_col_cnt].translated=1))
     SET v_t_code_value = get_value(cust_tab_name,"CODE_VALUE","TO")
     SET v_t_field_name = get_value(cust_tab_name,"FIELD_NAME","TO")
     SELECT INTO "NL:"
      FROM code_value c
      WHERE c.code_value=cnvtreal(v_t_code_value)
      DETAIL
     ...
     IF (check_error("Obtaining CDF meaning")=1)
      ...
      GO TO exit_0635
     ENDIF
     IF (v_cdf="CYTO WSHEET")
      IF (v_t_field_name="History Group")
       SET v_sbr_drdm_rtn_status = sbr_drdm_get_cv_trans_value(v_field_value)
      ENDIF
     ENDIF
    ENDIF
   OF "ALG3":
    IF ( NOT (v_field_value IN ("0", "0.0", "0.")))
     SET temp_col_cnt = get_col_pos(cust_tab_name,"FIELD_VALUE")
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
    ENDIF
   OF "ALG4":
    SET v_sbr_drdm_prsnl_rtn_status = sbr_drdm_get_prsnl_trans_value(v_field_value)
   OF "BADLOG":
    SET dm2_ref_data_reply->error_ind = 1
 ...
 ENDIF
 IF (idcd_check=1)
  SET idcd_check = is_translated(cust_tab_name,"FIELD_VALUE")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row was not translated because of the FIELD_VALUE"
    SET current_merges = 0
    IF ((global_mover_rec->one_pass_ind=0))
     ROLLBACK
    ENDIF
 ...
 ENDIF
#exit_0635
 SUBROUTINE sbr_drdm_get_cv_trans_value(sbr_get_field_value)
   DECLARE v_temp_cd = f8
   DECLARE v_temp_cd_vc = vc
   DECLARE temp_col_cnt = i4
   DECLARE sbr_drdm_status = i2
   DECLARE v_rpt_missing = vc
   SET sbr_drdm_status = 1
   SET v_temp_cd = cnvtreal(sbr_get_field_value)
   IF (v_temp_cd != 0.0)
    SET v_temp_cd_vc = select_merge_translate(sbr_get_field_value,"CODE_VALUE")
    IF ((dm2_ref_data_reply->error_ind=1))
     SET sbr_drdm_status = 0
    ELSE
     IF (isnumeric(v_temp_cd_vc))
      CALL put_value(cust_tab_name,"FIELD_VALUE",v_temp_cd_vc)
      IF ((dm2_ref_data_reply->error_ind=1))
       SET sbr_drdm_status = 0
      ENDIF
     ELSE
      SET temp_col_cnt = get_col_pos(cust_tab_name,"FIELD_VALUE")
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
      SET v_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(sbr_get_field_value))
      CALL evaluate_rpt_missing(v_rpt_missing)
      SET sbr_drdm_status = 0
     ENDIF
    ENDIF
   ENDIF
   RETURN(sbr_drdm_status)
 END ;Subroutine
 SUBROUTINE sbr_drdm_get_prsnl_trans_value(i_field_value)
   DECLARE v_temp_prsnl = f8
   DECLARE v_temp_prsnl_vc = vc
   DECLARE temp_col_cnt = i4
   DECLARE sbr_drdm_status = i2
   DECLARE v_rpt_missing = vc
   SET sbr_drdm_status = 1
   SET v_temp_prsnl = cnvtreal(i_field_value)
   IF (v_temp_prsnl != 0.0)
    SET v_temp_prsnl_vc = select_merge_translate(i_field_value,"PRSNL")
    IF ((dm2_ref_data_reply->error_ind=1))
     SET sbr_drdm_status = 0
    ELSE
     IF (isnumeric(v_temp_prsnl_vc))
      CALL put_value(cust_tab_name,"FIELD_VALUE",v_temp_prsnl_vc)
      IF ((dm2_ref_data_reply->error_ind=1))
       SET sbr_drdm_status = 0
      ENDIF
     ELSE
      SET temp_col_cnt = get_col_pos(cust_tab_name,"FIELD_VALUE")
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
      SET v_rpt_missing = report_missing("PRSNL","PERSON_ID",v_temp_prsnl)
      CALL evaluate_rpt_missing(v_rpt_missing)
      SET sbr_drdm_status = 0
     ENDIF
    ENDIF
