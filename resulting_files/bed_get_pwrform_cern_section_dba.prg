CREATE PROGRAM bed_get_pwrform_cern_section:dba
 ...
 DECLARE section_uid = vc WITH protect, constant(request->section_uid)
 DECLARE dcp_section_ref_id = f8 WITH protect, noconstant(0.0)
 DECLARE statuslevel = vc WITH protect, noconstant("")
 DECLARE bailoutind = i2 WITH protect, constant(request->bailout_ind)
 DECLARE getsectionsbysectionuid(sectionuid=vc) = null
 DECLARE getexistingsectionbydescription(dummyvar=i2) = null
 SET reply->modified_status = none
 CALL getsectionsbysectionuid(section_uid)
 IF ((reply->section.dcp_section_ref_id > 0))
  SET dcp_section_ref_id = reply->section.dcp_section_ref_id
 ELSE
  CALL getexistingsectionbydescription(0)
  SET dcp_section_ref_id = reply->section.dcp_section_ref_id
 ENDIF
 IF (dcp_section_ref_id != 0)
  IF (aresectionsdifferent(section_uid,dcp_section_ref_id,0.0,"",statuslevel,
   bailoutind))
   SET reply->section.modified_status = modified
   SET reply->section.modified_status_level = statuslevel
  ENDIF
 ELSE
  SET reply->section.modified_status = added
  SET reply->section.modified_status_level = none
 ENDIF
#exit_script
 CALL bedexitscript(0)
 SUBROUTINE getsectionsbysectionuid(sectionuid)
   CALL bedlogmessage("getSectionsBySectionUid","Entering ...")
   ...
   SELECT INTO "nl:"
    FROM cnt_pf_section_r sr,
     ...
     dcp_section_ref d
    PLAN (sr
     WHERE sr.section_uid=sectionuid)
     JOIN (sk
    ...
    DETAIL
     reply->section.section_uid = sk.section_uid, reply->section.description = sk.section_definition,
     reply->section.width = s.section_width,
     reply->section.height = s.section_height, reply->section.sequence = sr.section_seq, reply->
     section.dcp_section_ref_id = sk.dcp_section_ref_id,
     reply->section.match_description = d.definition, reply->section.match_name = d.description,
     reply->section.match_width = d.width,
     reply->section.match_height = d.height, reply->section.ignore_ind = sr.ignore_ind, reply->
     section.name = sk.section_description,
     reply->section.modified_status = none, reply->section.modified_status_level = none
    WITH nocounter
   ;end select
   CALL bedlogmessage("getSectionsBySectionUid","Exiting ...")
 END ;Subroutine
 SUBROUTINE getexistingsectionbydescription(dummyvar)
   CALL bedlogmessage("getExistingSectionByDescription","Entering ...")
   SELECT INTO "nl:"
    FROM dcp_section_ref r
    PLAN (r
     WHERE (r.definition=reply->section.description)
      AND r.active_ind=true)
    DETAIL
     reply->section.dcp_section_ref_id = r.dcp_section_ref_id, reply->section.match_description = r
     .definition, reply->section.match_name = r.description,
     reply->section.match_width = r.width, reply->section.match_height = r.height
    WITH nocounter
   ;end select
   CALL bedlogmessage("getExistingSectionByDescription","Exiting ...")
 END ;Subroutine
END GO
