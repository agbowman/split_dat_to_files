CREATE PROGRAM ct_mp_get_prot_alias:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "ct_mp_get_prot_alias"
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getprotalias(null) = null WITH protect
 DECLARE stat = i4 WITH protect, noconstant(0)
 SET public::"reply"->status_data.status = "F"
 IF (( $JSONREQUEST=""))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_alias","Invalid JSON Request",
   PUBLIC::"reply")
  GO TO exit_script
 ENDIF
 SET stat = cnvtjsontorec( $JSONREQUEST)
 CALL getprotalias(null)
 FREE RECORD request
 SET public::"reply"->status_data.status = "S"
 SUBROUTINE getprotalias(null)
   CALL log_message("In GetProtAlias()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   ...
   IF (protcount > 0)
    ...
    SELECT
     ...
     PLAN (d)
      JOIN (p
      WHERE (p.prot_master_id=request->prot_list[d.seq].protocolid)
       AND p.end_effective_dt_tm > cnvtdatetime(sysdate))
      JOIN (a
      WHERE p.alias_pool_cd=a.alias_pool_cd)
     HEAD p.prot_master_id
      cnt = 0
     DETAIL
      cnt += 1
      IF (mod(cnt,10)=1)
       stat = alterlist(public::"reply"->prot_list[d.seq]."prot_info"."prot_aliases",(cnt+ 9))
      ENDIF
      public::"reply"->prot_list[d.seq]."protocolId" = request->prot_list[d.seq].protocolid, public::
      "reply"->prot_list[d.seq]."amendmentId" = request->prot_list[d.seq].amendmentid, public::
      "reply"->prot_list[d.seq]."prot_info".prot_aliases[cnt]."alias_pool_cd" = p.alias_pool_cd
      IF (trim(uar_get_code_display(p.alias_pool_cd)) != "")
       public::"reply"->prot_list[d.seq]."prot_info".prot_aliases[cnt]."alias_pool_disp" = concat(
        trim(uar_get_code_display(p.alias_pool_cd)),"-")
      ENDIF
      public::"reply"->prot_list[d.seq]."prot_info".prot_aliases[cnt]."alias" = concat(public::
       "reply"->prot_list[d.seq]."prot_info".prot_aliases[cnt]."alias_pool_disp",p.prot_alias),
      public::"reply"->prot_list[d.seq]."prot_info".prot_aliases[cnt]."alias_format" = a.format_mask
     FOOT  p.prot_master_id
      stat = alterlist(public::"reply"->prot_list[d.seq]."prot_info"."prot_aliases",cnt)
     WITH nocounter
    ;end select
   ELSE
    CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_get_prot_alias",
     "Did not receive any protocol ID",PUBLIC::"reply")
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit GetProtAlias(), Elapsed time in seconds:",datetimediff(cnvtdatetime(
