CREATE PROGRAM dm2_routine_tasks:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drt_return_ind = i2 WITH protect, noconstant(0)
 DECLARE drt_info_exists = i2 WITH protect, noconstant(0)
 DECLARE drt_global_error = i2 WITH protect, noconstant(0)
 DECLARE drt_update_msg = vc WITH protect, noconstant("")
 DECLARE drt_job_start_time = dq8 WITH protect, constant(cnvtdatetime(curdate,curtime3))
 DECLARE drt_cur_date = i4 WITH protect, constant(curdate)
 DECLARE drt_cur_day = i4 WITH protect, constant(day(curdate))
 DECLARE drt_cur_weekday = i4 WITH protect, constant(weekday(curdate))
 DECLARE drt_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE drt_audsid = vc WITH protect, noconstant("")
 DECLARE drt_di_update_ind = i2 WITH protect, noconstant(0)
 DECLARE drt_queue_table_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE drt_err_ind = i2 WITH protect, noconstant(0)
 DECLARE drt_stats_end_time = i4 WITH protect, noconstant(- (99))
 DECLARE drt_rt_end_time = i4 WITH protect, noconstant(- (99))
 DECLARE drt_nnv_end_time = i4 WITH protect, noconstant(- (99))
 DECLARE drt_gcc_end_time = i4 WITH protect, noconstant(- (99))
 DECLARE drt_cgd_end_time = i4 WITH protect, noconstant(- (99))
 DECLARE drt_should_task_run(task_name=vc,return_ind=i2(ref)) = i2
 DECLARE drt_update_status(drt_info_name=vc,drt_info_char=vc) = i2
 IF (check_logfile("dm2_routine_tasks",".log","DM2_ROUTINE_TASKS LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning DM2_ROUTINE_TASKS"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (validate(request->batch_selection,"X") != "X"
  AND validate(request->batch_selection,"Y") != "Y")
  SET dm_err->eproc = "Called from the OPSView Scheduler..."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 IF (dm2_table_and_ccldef_exists("DM_INFO",drt_info_exists)=0)
  GO TO exit_script
 ENDIF
 IF (drt_info_exists=0)
  SET dm_err->eproc =
  "Unable to determine if tasks should be run.  DM_INFO does not exist.  Using defaults."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking for active routine tasks process"
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_ROUTINE_TASKS"
   AND di.info_name="RUN_LOCK"
  DETAIL
   drt_audsid = di.info_char
 ...
 ENDIF
 IF (curqual > 0)
  IF (dm2_get_appl_status(drt_audsid)="A")
   ROLLBACK
   SET dm_err->eproc = "A dm2_routine_tasks process is already running. Exiting."
   CALL disp_msg("",dm_err->logfile,0)
   GO TO exit_script
  ELSE
   SET dm_err->eproc = "Update routine tasks lock row in dm_info with current audsid"
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Insert routine tasks lock row in dm_info with current audsid"
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2_ROUTINE_TASKS", di.info_name = "RUN_LOCK", di.info_char = currdbhandle
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Querying for existance of the DM_PROCESS_QUEUE table from dba_tables."
 SELECT INTO "nl:"
  FROM dba_tables dt
  WHERE dt.table_name="DM_PROCESS_QUEUE"
   AND dt.owner="V500"
  DETAIL
   drt_queue_table_exists_ind = 1
  WITH nocounter
 ;end select
 ...
 SET dm2_process_rs->process_name = dpl_routine_tasks
 CALL dm2_process_log_row(dpl_routine_tasks,dpl_execution,dpl_no_prev_id,1)
 SET drt_log_id = dm2_process_event_rs->dm_process_event_id
 SET dm2_process_rs->process_name = ""
 IF (drt_update_status("DM2_ROUTINE_TASKS","EXECUTING")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Getting information about DM2_RUNSTATS SYS from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE "DM2_ROUTINE_TASKS"=di.info_domain
   AND di.info_name="DM2_RUNSTATS SYS"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inserting monthly execution indicator row for DM2_RUNSTATS SYS"
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2_ROUTINE_TASKS", di.info_name = "DM2_RUNSTATS SYS", di.info_number = 4,
    di.info_long_id = 1, di.info_date = cnvtdatetime((curdate - 365),curtime3)
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc))
   ROLLBACK
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Checking for existence of end_time row for STATISTICS from DM_INFO"
 SELECT INTO "nl:"
  FROM dm_info di
  ...
  DETAIL
   CASE (di.info_name)
    OF dpq_statistics:
     drt_stats_end_time = di.info_number
    OF dpq_routine_tasks:
     drt_rt_end_time = di.info_number
    OF dpq_notnull_validate:
     drt_nnv_end_time = di.info_number
    OF dpq_index_coalesce:
     drt_gcc_end_time = di.info_number
   ENDCASE
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc))
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Manage dm_info rows for End of Time Window"
 IF ((drt_stats_end_time != - (99)))
  IF ((drt_rt_end_time=- (99)))
   SET drt_rt_end_time = drt_stats_end_time
   SET drt_stats_end_time = 360
  ENDIF
 ELSE
  IF ((drt_rt_end_time=- (99)))
   SET drt_rt_end_time = 360
  ENDIF
  SET drt_stats_end_time = 360
 ENDIF
 IF ((drt_nnv_end_time=- (99)))
  SET drt_nnv_end_time = 360
 ENDIF
 IF ((drt_gcc_end_time=- (99)))
  SET drt_gcc_end_time = 360
 ENDIF
 IF ((drt_cgd_end_time=- (99)))
  SET drt_cgd_end_time = 360
 ENDIF
 IF (dpq_manage_end_time(0,dpq_routine_tasks,drt_rt_end_time) != 1)
  GO TO exit_script
 ENDIF
 IF (dpq_manage_end_time(0,dpq_statistics,drt_stats_end_time) != 1)
  GO TO exit_script
 ENDIF
 IF (dpq_manage_end_time(0,dpq_notnull_validate,drt_nnv_end_time) != 1)
  GO TO exit_script
 ENDIF
 IF (dpq_manage_end_time(0,dpq_index_coalesce,drt_gcc_end_time) != 1)
  GO TO exit_script
 ENDIF
 IF (dpq_cleanup_old_procs(dpq_routine_tasks,1000)=0)
  GO TO exit_script
 ENDIF
 IF (drt_should_task_run("DM2_NOTNULL_VALIDATE",drt_return_ind)=0)
  GO TO exit_script
 ENDIF
 IF (drt_return_ind=1)
  IF (drt_update_status("DM2_NOTNULL_VALIDATE","EXECUTING")=0)
   GO TO exit_script
  ENDIF
  EXECUTE dm2_notnull_validate_gen
  IF ((dm_err->err_ind=1))
   SET drt_global_error = 1
  ENDIF
  SET drt_update_msg = evaluate(dm_err->err_ind,0,"SUCCESS",dm_err->emsg)
  IF (drt_update_status("DM2_NOTNULL_VALIDATE",drt_update_msg)=0)
   GO TO exit_script
  ENDIF
  SET dm_err->err_ind = 0
 ENDIF
 IF (drt_should_task_run("DM2_CBO_PLAN_HIST_TRACKING",drt_return_ind)=0)
  GO TO exit_script
 ENDIF
 IF (drt_return_ind=1)
  IF (drt_update_status("DM2_CBO_PLAN_HIST_TRACKING","EXECUTING")=0)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Executing dm2_plan_hist_tracking"
  CALL disp_msg("",dm_err->logfile,0)
  EXECUTE dm2_plan_hist_tracking
  IF ((dm_err->err_ind=1))
   SET drt_global_error = 1
  ENDIF
  SET drt_update_msg = evaluate(dm_err->err_ind,0,"SUCCESS",dm_err->emsg)
  IF (drt_update_status("DM2_CBO_PLAN_HIST_TRACKING",drt_update_msg)=0)
   GO TO exit_script
  ENDIF
  SET dm_err->err_ind = 0
 ENDIF
 ...
 ENDIF
 IF ((dm2_rdbms_version->level1 < 11))
  IF (drt_should_task_run("DM2_RUNSTATS SYS",drt_return_ind)=0)
   GO TO exit_script
  ENDIF
  IF (drt_return_ind=1)
   IF (drt_update_status("DM2_RUNSTATS SYS","EXECUTING")=0)
    GO TO exit_script
   ENDIF
   SET dpq_process_queue->process_type = dpq_statistics
   SET dpq_process_queue->op_type = dpq_gather
   SET dpq_process_queue->owner_name = "SYS"
   SET dpq_process_queue->object_type = dpq_schema
   SET dpq_process_queue->op_method = dpq_db
   SET dpq_process_queue->object_name = "*"
   SET dpq_process_queue->routine_tasks_ind = 1
   SET dpq_process_queue->operation_txt = concat("execute dm2_gather_dict_stats go")
   SET dpq_process_queue->priority = 40
   ...
   ENDIF
   SET drt_update_msg = evaluate(dm_err->err_ind,0,"SUCCESS",dm_err->emsg)
   IF (drt_update_status("DM2_RUNSTATS SYS",drt_update_msg)=0)
    GO TO exit_script
   ENDIF
   SET dm_err->err_ind = 0
  ENDIF
 ENDIF
 IF (drt_should_task_run("DM2_RUNSTATS CURRENT USER",drt_return_ind)=0)
  GO TO exit_script
 ENDIF
 IF (drt_return_ind=1)
  IF (drt_update_status("DM2_RUNSTATS CURRENT USER","EXECUTING")=0)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = concat("Executing dm2_runstats '",build(currdbuser),
   "', '*', 'GATHER_STALE', 10")
  CALL disp_msg("",dm_err->logfile,0)
  EXECUTE dm2_runstats value(currdbuser), "*", "GATHER_STALE",
  10
  IF ((dm_err->err_ind=1))
   SET drt_global_error = 1
  ENDIF
  SET drt_update_msg = evaluate(dm_err->err_ind,0,"SUCCESS",dm_err->emsg)
  IF (drt_update_status("DM2_RUNSTATS CURRENT USER",drt_update_msg)=0)
   GO TO exit_script
  ENDIF
  SET dm_err->err_ind = 0
 ENDIF
 IF (drt_should_task_run("DM2_GEN_COALESCE_CMDS",drt_return_ind)=0)
  GO TO exit_script
 ENDIF
 IF (drt_return_ind=1)
  IF (drt_update_status("DM2_GEN_COALESCE_CMDS","EXECUTING")=0)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Executing dm2_gen_coalesce_cmds"
  CALL disp_msg("",dm_err->logfile,0)
  EXECUTE dm2_gen_coalesce_cmds
  IF ((dm_err->err_ind=1))
   SET drt_global_error = 1
  ENDIF
  SET drt_update_msg = evaluate(dm_err->err_ind,0,"SUCCESS",dm_err->emsg)
  IF (drt_update_status("DM2_GEN_COALESCE_CMDS",drt_update_msg)=0)
   GO TO exit_script
  ENDIF
  SET dm_err->err_ind = 0
 ENDIF
 IF (drt_should_task_run("XNT_CLINICAL_RANGES",drt_return_ind)=0)
  GO TO exit_script
 ENDIF
 CALL echo(drt_return_ind)
 IF (drt_return_ind=1)
  IF (drt_update_status("XNT_CLINICAL_RANGES","EXECUTING")=0)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Executing dm2_xnt_gen_clin_ranges_cmds"
  CALL disp_msg("",dm_err->logfile,0)
  EXECUTE dm2_xnt_gen_clin_ranges_cmds
  IF ((dm_err->err_ind=1))
   SET drt_global_error = 1
  ENDIF
  SET drt_update_msg = evaluate(dm_err->err_ind,0,"SUCCESS",dm_err->emsg)
  IF (drt_update_status("XNT_CLINICAL_RANGES",drt_update_msg)=0)
   GO TO exit_script
  ENDIF
  SET dm_err->err_ind = 0
 ENDIF
 IF (drt_queue_table_exists_ind)
  IF ((validate(dm2_no_runner,- (1))=- (1))
   AND (validate(dm2_no_runner,- (2))=- (2)))
   EXECUTE dm2_process_queue_runner dpq_routine_tasks
  ENDIF
  IF ( NOT (dpq_remove_procs(dpq_routine_tasks,dpq_success,1000)))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 SUBROUTINE drt_should_task_run(dstr_task_name,dstr_return_ind)
   IF (dstr_task_name IN ("DM2_RUNSTATS SYS", "DM2_RUNSTATS CURRENT USER",
   "DM2_CBO_PLAN_HIST_TRACKING", "DM2_NOTNULL_VALIDATE", "DM2_GEN_COALESCE_CMDS"))
    SET dstr_return_ind = 1
   ELSE
    SET dstr_return_ind = 0
   ENDIF
   SET dm_err->eproc = "Getting information about task from dm_info."
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE "DM2_ROUTINE_TASKS"=di.info_domain
     AND di.info_name=dstr_task_name
    HEAD REPORT
     unit_of_time = 0, frequency_of_unit = 0, specific_unit = 0,
     prev_date = 0
    DETAIL
     dstr_return_ind = 0, unit_of_time = di.info_number, frequency_of_unit = di.info_long_id,
     specific_unit = di.updt_applctx, prev_date = cnvtdate(di.info_date)
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("unit_of_time:",evaluate(unit_of_time,2,"DAY",3,"WEEK",
        4,"MONTH"))),
      CALL echo(build("frequency_of_unit:",frequency_of_unit)),
      CALL echo(build("specific_unit:",evaluate(unit_of_time,2,evaluate(specific_unit,0,"SUN",1,"MON",
         2,"TUE",3,"WED",4,
         "THU",5,"FRI",6,"SAT",
         7,"N/A")))),
      CALL echo(build("prev_date:",prev_date)),
      CALL echo(build("drt_cur_date:",drt_cur_date)),
      CALL echo(build("drt_cur_day:",drt_cur_day)),
      CALL echo(build("drt_cur_weekday:",drt_cur_weekday))
     ENDIF
     CASE (unit_of_time)
      OF 0:
       dstr_return_ind = 0
      OF 2:
       IF (prev_date <= cnvtdate(cnvtlookbehind(build('"',frequency_of_unit,',D"'))))
        dstr_return_ind = 1
       ENDIF
      OF 3:
       IF (prev_date <= cnvtdate(cnvtlookbehind(build('"',frequency_of_unit,',W"'))))
        IF (((specific_unit=7) OR (specific_unit=drt_cur_weekday)) )
         dstr_return_ind = 1
        ENDIF
       ENDIF
      OF 4:
       IF (prev_date <= cnvtdate(cnvtlookbehind(build('"',frequency_of_unit,',M"'))))
        dstr_return_ind = 1
       ENDIF
     ENDCASE
    WITH nocounter
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drt_update_status(drt_info_name,drt_info_char)
   IF (drt_info_char="EXECUTING")
    SET dm_err->eproc = "Updating dm_info row with status and timing for start."
    UPDATE  FROM dm_info di
     SET di.info_char = "EXECUTING", di.info_date = cnvtdatetime(drt_job_start_time), di.updt_dt_tm
       = cnvtdatetime(curdate,curtime3),
      di.updt_id = 0
     WHERE "DM2_ROUTINE_TASKS"=di.info_domain
      AND di.info_name=drt_info_name
     WITH nocounter
    ;end update
   ELSE
    SET dm_err->eproc = "Updating dm_info row with status and timing for finish."
    UPDATE  FROM dm_info di
     SET di.info_char = drt_info_char, di.updt_id = (((cnvtdatetime(curdate,curtime3) - di.updt_dt_tm
      ) * 24) * 60)
     WHERE "DM2_ROUTINE_TASKS"=di.info_domain
      AND di.info_name=drt_info_name
     WITH nocounter
...
#exit_script
 ...
 SET drt_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 SET dm2_process_rs->process_name = dpl_routine_tasks
 CALL dm2_process_log_row(dpl_routine_tasks,dpl_execution,drt_log_id,1)
 SET dm2_process_rs->process_name = ""
 SET drt_update_msg = evaluate(drt_global_error,0,"SUCCESS","COMPLETE WITH ERRORS")
 CALL drt_update_status("DM2_ROUTINE_TASKS",drt_update_msg)
 SET dm_err->err_ind = drt_err_ind
 SET dm_err->eproc = "Remove locking row for current routine tasks process"
 DELETE  FROM dm_info di
  WHERE di.info_domain="DM2_ROUTINE_TASKS"
   AND di.info_name="RUN_LOCK"
  WITH nocounter
 ;end delete
 COMMIT
 SET dm_err->eproc = "Ending DM2_ROUTINE_TASKS"
 CALL final_disp_msg("dm2_routine_tasks")
