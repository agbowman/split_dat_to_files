CREATE PROGRAM bbt_solcap_2010h1_req_courier
 ...
 SET reply->solcap[1].identifier = "2010.1.00091.5"
 SET reply->solcap[2].identifier = "2010.1.00091.6"
 SET reply->solcap[1].degree_of_use_str = cnvtstring(bbtgetrequiredcourierdispenseassignpreference(
   0.0))
 SET reply->solcap[2].degree_of_use_str = cnvtstring(bbtgetrequiredcourierreturnproductspreference(
   0.0))
 FREE SET res_count
 ...
 SELECT
  ...
  DETAIL
   ...
   IF (mod(res_count,10)=1)
    stat = alterlist(reply->solcap[1].facility,(res_count+ 9)), stat = alterlist(reply->solcap[2].
     facility,(res_count+ 9))
   ENDIF
   reply->solcap[1].facility[res_count].display = cv.display, reply->solcap[1].facility[res_count].
   value_str = cnvtstring(bbtgetrequiredcourierdispenseassignpreference(cv.code_value)), reply->
   solcap[2].facility[res_count].display = cv.display,
   reply->solcap[2].facility[res_count].value_str = cnvtstring(
    bbtgetrequiredcourierreturnproductspreference(cv.code_value))
  FOOT REPORT
   stat = alterlist(reply->solcap[1].facility,res_count), stat = alterlist(reply->solcap[2].facility,
    res_count)
  WITH nocounter
 ;end select
END GO
