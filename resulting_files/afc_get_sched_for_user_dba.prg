CREATE PROGRAM afc_get_sched_for_user:dba
 ...
 IF ( NOT (validate(cs26078_sched_cd)))
  DECLARE cs26078_sched_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",26078,request->
    key1_entity_name))
  CALL echo(build("CS26078_SCHED_CD ",cnvtstring(cs26078_sched_cd,17,2)))
 ENDIF
 IF ( NOT (validate(cs14002_asa_sched_cd)))
  DECLARE cs14002_asa_sched_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",14002,"ASA"))
  CALL echo(build("CS14002_ASA_SCHED_CD ",cnvtstring(cs14002_asa_sched_cd,17,2)))
 ENDIF
 SET count1 = 0
 IF (cs26078_sched_cd > 0.0)
  SELECT
   IF (validate(request->schedule_meaning,"") != "")
    ...
    PLAN (por
     ...
     WHERE cv.code_value=cor.key1_id
      ...
      AND (cv.cdf_meaning=request->schedule_meaning)
      AND cv.active_ind=1)
   ELSE
    FROM prsnl_org_reltn por,
     cs_org_reltn cor
   ...
   HEAD cor.key1_id
    count1 += 1, reply->cs_org_reltn_qual = count1, stat = alterlist(reply->cs_org_reltn,count1),
    reply->cs_org_reltn[count1].cs_org_reltn_id = cor.cs_org_reltn_id, reply->cs_org_reltn[count1].
    organization_id = cor.organization_id, reply->cs_org_reltn[count1].cs_org_reltn_type_cd = cor
    .cs_org_reltn_type_cd,
    reply->cs_org_reltn[count1].key1_id = cor.key1_id, reply->cs_org_reltn[count1].active_ind = cor
    .active_ind
    IF (count1=1)
     reply->list_for_user = cnvtstring(cor.key1_id,17)
    ELSE
     reply->list_for_user = concat(reply->list_for_user,",",cnvtstring(cor.key1_id,17))
    ENDIF
   WITH nocounter
  ;end select
 ENDIF
 IF (cs14002_asa_sched_cd > 0.0
  AND (request->key1_entity_name="BC_SCHED")
  AND ((validate(request->schedule_meaning,"")="") OR (validate(request->schedule_meaning,"")="ASA"
 )) )
  SET count1 += 1
  SET reply->cs_org_reltn_qual = count1
  SET stat = alterlist(reply->cs_org_reltn,count1)
  SET reply->cs_org_reltn[count1].key1_id = cs14002_asa_sched_cd
  IF (count1=1)
   SET reply->list_for_user = cnvtstring(cs14002_asa_sched_cd,17)
  ELSE
   SET reply->list_for_user = concat(reply->list_for_user,",",cnvtstring(cs14002_asa_sched_cd,17))
  ENDIF
 ENDIF
 IF (curqual=0
  AND count1=0)
