CREATE PROGRAM dm_ccl_report_audit_rows:dba
 ...
 SET reply->table_name = "CCL_REPORT_AUDIT"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(cra.report_event_id)
   FROM ccl_report_audit cra
   WHERE cra.report_event_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(cra.report_event_id)
  FROM ccl_report_audit cra
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    cra.rowid
    FROM ccl_report_audit cra
    WHERE parser(sbr_getrowidnotexists("cra.report_event_id between curMinID and curMaxID","cra"))
     AND cra.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = cra.rowid
    WITH nocounter, maxqual(cra,value(rowsleft))
   ;end select
