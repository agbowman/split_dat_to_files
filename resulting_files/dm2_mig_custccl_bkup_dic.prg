CREATE PROGRAM dm2_mig_custccl_bkup_dic
 ...
 SUBROUTINE dm2_rename_login_default(sbr_rld_mode)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmccbd_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_bkup_dic")
 DECLARE dmccbd_id = f8 WITH protect, noconstant(0)
 DECLARE dmccbd_bk_dir = vc WITH protect, noconstant("")
 DECLARE dmccbd_cont = i2 WITH protect, noconstant(1)
 DECLARE dmccbd_env = vc WITH protect, noconstant("")
 DECLARE dmccbd_node = vc WITH protect, noconstant("")
 DECLARE dmccbd_env_id = f8 WITH protect, noconstant(0)
 DECLARE dmccbd_os_type = vc WITH protect, noconstant("")
 DECLARE dmccbd_id = f8 WITH protect, noconstant(0)
 DECLARE dmccbd_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmccbd_get_dicdat(dgd_loc=vc,dgd_file_name=vc) = i2
 SET width = 132
 IF (check_logfile(dmccbd_logfile_prefix,".log","dm2_mig_custccl_bkup_dic")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_bkup_dic"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmccbd_env = cnvtupper(trim(logical("environment")))
 SET dmccbd_node = trim(curnode)
 SET dmccbd_os_type = trim(cursys2)
 SET dmccbd_bk_dir = trim(logical("cer_temp"))
 IF (dmccbd_os_type="AXP")
  SET dm_err->emsg = "Script cannot be run on a VMS environment"
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  WHERE de.environment_name=dmccbd_env
  DETAIL
   dmccbd_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  ...
  WHERE dne.node_name=dmccbd_node
   AND dne.environment_id=dmccbd_env_id
  DETAIL
   dmccbd_id = dne.dma_env_node_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dmccbd_env))
  CALL echo(concat("Current Environment Id: ",cnvtstring(dmccbd_env_id)))
  CALL echo(concat("Current Node: ",dmccbd_node))
  CALL echo(concat("Current Environment: ",dmccbd_os_type))
  CALL echo(concat("Current Node/Env ID: ",cnvtstring(dmccbd_id)))
 ENDIF
 WHILE (dmccbd_cont=1)
   ...
   CALL clear(1,1)
   CALL box(1,1,22,131)
   CALL text(1,2,"Custom CCL Migration: Backup Target CCL Dictionary")
   CALL text(3,2,concat("Environment:",dmccbd_env," (",trim(cnvtstring(dmccbd_env_id)),")"))
   CALL text(4,2,concat("Node:",dmccbd_node))
   CALL text(6,2,
    "A backup copy (dm2migccclbkdic.dat/.idx) of the CCL Dictionary will be made before making any changes to it"
    )
   CALL text(7,2,"This process will default to $cer_temp, but you may provide any directory name")
   CALL text(8,2,"Please confirm the directory to store this file")
   CALL text(10,2,"Backup Directory:")
   CALL accept(10,20,"P(50);C",dmccbd_bk_dir
    WHERE  NOT (curaccept=" "))
   SET dmccbd_bk_dir = curaccept
   CALL text(12,2,"(C)ontinue, (M)odify, (Q)uit:")
   CALL accept(12,32,"A;cu"," "
    WHERE curaccept IN ("C", "M", "Q"))
   IF (curaccept="C")
    IF (dm2_find_dir(dmccbd_bk_dir)=0)
   ...
   ELSEIF (curaccept="Q")
    SET message = nowindow
    SET dmccbd_cont = 0
    SET dm_err->emsg = "User elected to quit from Target Backup of Target CCL Dictionary"
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ELSE
 ...
 ENDWHILE
 SET dm_err->eproc =
 "Insert a row into dma_env_node_attr admin table for backup location (if row does not exist)"
 SELECT INTO "nl:"
  FROM dma_env_node_attr dena
  WHERE dena.dma_env_node_id=dmccbd_id
   AND dena.attr_name="CCMIG_DIC_BKUP_DIR"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET dm_err->eproc = "Inserting row for ccl dic backup location"
  INSERT  FROM dma_env_node_attr dena
   SET dena.dma_attr_id = seq(dm_seq,nextval), dena.dma_env_node_id = dmccbd_id, dena.attr_name =
    "CCMIG_DIC_BKUP_DIR",
    dena.attr_value_txt = dmccbd_bk_dir
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ELSE
  SET dm_err->eproc = "Updating row for ccl dic backup location"
  UPDATE  FROM dma_env_node_attr dena
   SET dena.attr_value_txt = dmccbd_bk_dir
   WHERE dena.dma_env_node_id=dmccbd_id
    AND dena.attr_name="CCMIG_DIC_BKUP_DIR"
   WITH nocounter
  ;end update
  ...
  ENDIF
 ENDIF
 SET dmccbd_file_name = trim(build("dm2migcclbkdic",year(curdate),month(curdate),day(curdate),hour(
    curtime),
   minute(curtime)))
 SET dmccbd_bk_dir = concat(dmccbd_bk_dir,evaluate(dmccbd_os_type,"AXP","","/"))
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Backup directory: ",dmccbd_bk_dir))
  CALL echo(concat("backup file name: ",dmccbd_file_name))
 ENDIF
 IF (dmccbd_get_dicdat(dmccbd_bk_dir,trim(dmccbd_file_name))=0)
  GO TO exit_program
 ENDIF
 GO TO exit_program
 SUBROUTINE dmccbd_get_dicdat(dgd_loc,dgd_file_name)
   ...
   DECLARE dgd_os = vc WITH protect, noconstant("")
   DECLARE dgd_file_nodate = vc WITH protect, noconstant("dm2migcclbkdic")
   SET dm_err->eproc = "Backing up the current dicionary"
   CALL disp_msg("",dm_err->logfile,0)
   SET dgd_os = trim(logical("cursys2"))
   SET dgd_file_str = concat(dgd_cerinstall,evaluate(dgd_os,"AXP","dm_dict.dat;*","/dm_dict.dat"))
   SET dm_err->eproc = concat("Remove:",dgd_file_str)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   IF (dm2_findfile(dgd_file_str) > 0)
    IF (dm2_push_dcl(concat(evaluate(dgd_os,"AXP","del","rm")," ",dgd_file_str))=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (dgd_os != "AXP")
    ...
    ENDIF
   ENDIF
   SET dgd_file_str = concat(dgd_loc,evaluate(dgd_os,"AXP",concat(dgd_file_nodate,"*.*;*"),concat(
      dgd_file_nodate,"*.*")))
   SET dm_err->eproc = concat("Remove:",dgd_file_str)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ...
   SET dgd_file_str = concat(dgd_cerinstall,evaluate(dgd_os,"AXP","dm_dict.dat","/dm_dict.dat"))
   IF (dm2_findfile(dgd_file_str)=0)
    SET dm_err->emsg = concat("Error creating copy of dictionary. Dictionary copy does not exist.")
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dgd_str = concat(evaluate(dgd_os,"AXP","rename","mv")," ",dgd_file_str," ",dgd_loc,
    dgd_file_name,".dat")
   IF (dm2_push_dcl(dgd_str)=0)
    RETURN(0)
   ENDIF
   SET dgd_file_str = concat(dgd_cerinstall,evaluate(dgd_os,"AXP","dm_dict.idx","/dm_dict.idx"))
   IF (dgd_os != "AXP")
    SET dgd_str = concat("mv ",dgd_file_str," ",dgd_loc,dgd_file_name,
     ".idx")
    IF (dm2_push_dcl(dgd_str)=0)
     RETURN(0)
    ENDIF
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_bkup_dic"
 CALL final_disp_msg(dmccbd_logfile_prefix)
END GO
