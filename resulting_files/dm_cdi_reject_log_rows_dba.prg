CREATE PROGRAM dm_cdi_reject_log_rows:dba
 ...
 SET reply->table_name = "CDI_REJECT_LOG"
 ...
 SELECT INTO "nl:"
  cdl.rowid
  FROM cdi_reject_log cdl
  WHERE parser(sbr_getrowidnotexists(
    "cdl.updt_dt_tm < cnvtdatetime(curdate - v_days_to_keep, curtime3)","cdl"))
   AND ((cdl.cdi_reject_log_id+ 0) > 0)
  DETAIL
   v_rows = (v_rows+ 1)
   IF (mod(v_rows,100)=1)
    stat = alterlist(reply->rows,(v_rows+ 99))
   ENDIF
   reply->rows[v_rows].row_id = cdl.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,v_rows)
  WITH nocounter, maxqual(cdl,value(request->max_rows))
 ;end select
