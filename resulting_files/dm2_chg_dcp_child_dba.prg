CREATE PROGRAM dm2_chg_dcp_child:dba
 ...
 FOR (dfc_table_loop = 1 TO dfc_table_cnt)
  CALL parser(concat("'",dcp_tables->qual[dfc_table_loop].table_name,"'"),0)
 ...
 CALL parser(" order by d.table_name head report dfc_cnt = 0 stat = alterlist(dcp_dels->qual, 100)",0
  )
 CALL parser(" detail dfc_cnt = dfc_cnt + 1 if (mod(dfc_cnt,100) = 1 and dfc_cnt != 1)",0)
 CALL parser(
  " stat = alterlist(dcp_dels->qual, dfc_cnt + 99) endif dcp_dels->qual[dfc_cnt].log_id = d.log_id ",
  0)
 CALL parser(" dcp_dels->qual[dfc_cnt].pk_where = d.pk_where",0)
 CALL parser(
  " dcp_dels->qual[dfc_cnt].table_name = d.table_name foot report stat = alterlist(dcp_dels->qual,dfc_cnt)",
  0)
 CALL parser(" with nocounter go",1)
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading DCP* rows from the DM_CHG_LOG table into record structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
 ...
 FOR (dfc_loop = 1 TO dfc_cnt)
   DELETE  FROM dm_chg_log d
    WHERE (d.log_id=dcp_dels->qual[dfc_loop].log_id)
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc) != 0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Error deleting row in dm_chg_log."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ROLLBACK
    SET readme_data->status = "F"
    SET readme_data->message = dm_err->emsg
   ...
   ENDIF
   IF (dfc_error_ind=0)
    SET dfcc_table_cnt = locateval(dfc_extra_cnt,1,size(dcp_tables->qual,5),dcp_dels->qual[dfc_loop].
     table_name,dcp_tables->qual[dfc_extra_cnt].table_name)
    IF ((dcp_tables->qual[dfcc_table_cnt].trig_ind=0))
     SET dcp_tables->qual[dfcc_table_cnt].trig_ind = check_trig(dcp_tables->qual[dfcc_table_cnt].
      suffix)
     IF (check_error(dm_err->eproc) != 0)
      SET dm_err->err_ind = 1
     ...
     ENDIF
     IF ((dcp_tables->qual[dfcc_table_cnt].trig_ind=0))
      SET dcp_tables->qual[dfcc_table_cnt].add_trig_ind = 1
      EXECUTE dm2_add_chg_log_triggers dcp_tables->qual[dfcc_table_cnt].table_name, "REFCHG"
      SET dcp_tables->qual[dfcc_table_cnt].trig_ind = check_trig(dcp_tables->qual[dfcc_table_cnt].
       suffix)
      IF (((check_error(dm_err->eproc) != 0) OR ((dcp_tables->qual[dfcc_table_cnt].trig_ind=0))) )
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat("Trigger could not be added for the ",dcp_tables->qual[
        dfcc_table_cnt].table_name," table.")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       ROLLBACK
       SET readme_data->status = "F"
       SET readme_data->message = dm_err->emsg
     ...
     ENDIF
    ENDIF
    CALL parser(concat("update into ",dcp_dels->qual[dfc_loop].table_name," d "),0)
    CALL parser("set d.updt_id = d.updt_id",0)
    CALL parser(concat(dcp_dels->qual[dfc_loop].pk_where," go"),1)
    IF (check_error(dm_err->eproc) != 0)
     SET dm_err->err_ind = 1
