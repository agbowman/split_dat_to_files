CREATE PROGRAM dm_merge_setup_admin:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE column_missing(cm_table,cm_column)
   SET cm_flag = 1
   SELECT INTO "nl:"
    c.column_name
    FROM user_tab_columns c
    WHERE c.table_name=cnvtupper(trim(cm_table,3))
     AND c.column_name=cnvtupper(trim(cm_column,3))
    DETAIL
     cm_flag = 0
    WITH nocounter
   ;end select
 ...
 SUBROUTINE push(p_text)
  ...
  SET parser_buffer[parser_count] = p_text
 END ;Subroutine
 SUBROUTINE table_missing(tm_table)
   SET tm_flag = 1
   SELECT INTO "nl:"
    t.table_name
    FROM user_tables t
    WHERE t.table_name=cnvtupper(trim(tm_table,3))
    DETAIL
     tm_flag = 0
    WITH nocounter
   ;end select
   RETURN(tm_flag)
 END ;Subroutine
 SUBROUTINE pk_columns(pk_table)
   SET pk_col_cnt = 0
   SELECT INTO "nl:"
    FROM user_cons_columns uc,
     user_constraints u
    WHERE u.table_name=cnvtupper(trim(pk_table,3))
     AND u.constraint_type="P"
     AND uc.table_name=u.table_name
     AND uc.constraint_name=u.constraint_name
    DETAIL
     pk_col_cnt = (pk_col_cnt+ 1)
    WITH nocounter
   ;end select
   RETURN(pk_col_cnt)
 END ;Subroutine
 SUBROUTINE pk_col_pos(pcp_tname,pcp_cname,pcp_col_pos)
   SET pcp_flag = 0
   SELECT INTO "nl:"
    ucc.column_name
    ...
    PLAN (uc
     WHERE uc.table_name=pcp_tname
      AND uc.constraint_type="P")
     JOIN (ucc
     WHERE ucc.table_name=uc.table_name
      AND ucc.constraint_name=uc.constraint_name
      AND ucc.position=pcp_col_pos
      AND ucc.column_name=pcp_cname)
    DETAIL
     pcp_flag = 1
    WITH nocounter
   ;end select
   RETURN(pcp_flag)
 END ;Subroutine
 SUBROUTINE package_exists(dpe_package)
   SET dpe_flag = 0
   SELECT INTO "nl:"
    FROM user_objects u
    WHERE u.object_name=cnvtupper(dpe_package)
     AND u.object_type IN ("PACKAGE", "PACKAGE BODY")
    DETAIL
     dpe_flag = 1
    WITH nocounter
   ;end select
   RETURN(dpe_flag)
 END ;Subroutine
 SUBROUTINE dsa_get_index_col_cnt(icc_tbl_name,icc_idx_name)
   SET icc_col_cnt = 0
   SELECT INTO "nl:"
    FROM user_ind_columns uc
    WHERE uc.table_name=cnvtupper(trim(icc_tbl_name,3))
     AND uc.index_name=cnvtupper(trim(icc_idx_name,3))
    DETAIL
     icc_col_cnt = (icc_col_cnt+ 1)
    WITH nocounter
   ;end select
   RETURN(icc_col_cnt)
 END ;Subroutine
 SUBROUTINE dsa_get_index_col_pos(icp_tbl_name,icp_idx_name,icp_col_name)
   SET icp_pos = 0
   SELECT INTO "nl:"
    ucc.column_position
    FROM user_ind_columns ucc
    WHERE ucc.table_name=cnvtupper(trim(icp_tbl_name,3))
     AND ucc.index_name=cnvtupper(trim(icp_idx_name,3))
     AND ucc.column_name=cnvtupper(trim(icp_col_name,3))
    DETAIL
     icp_pos = ucc.column_position
    WITH nocounter
   ;end select
   RETURN(icp_pos)
 END ;Subroutine
 SUBROUTINE dsa_get_index_name(gin_tbl_name,gin_col_list)
   FREE RECORD gin_cols
   RECORD gin_cols(
     1 col_cnt = i4
     1 cols[*]
       2 col_name = vc
   )
   SET gin_cols->col_cnt = 0
   SET stat = alterlist(gin_cols->cols,0)
   SET gin_start_pos = 1
   SET gin_list_len = 0
   SET gin_comma_pos = 0
   SET gin_idx_name = fillstring(30," ")
   SET gin_idx_name = " "
   SET gin_list_len = size(gin_col_list,1)
   SET gin_comma_pos = findstring(",",gin_col_list,gin_start_pos)
   WHILE (gin_comma_pos > 0)
     SET gin_cols->col_cnt = (gin_cols->col_cnt+ 1)
     SET stat = alterlist(gin_cols->cols,gin_cols->col_cnt)
     SET gin_cols->cols[gin_cols->col_cnt].col_name = substring(gin_start_pos,(gin_comma_pos -
      gin_start_pos),gin_col_list)
     SET gin_start_pos = (gin_comma_pos+ 1)
     SET gin_comma_pos = findstring(",",gin_col_list,gin_start_pos)
   ENDWHILE
   SET gin_cols->col_cnt = (gin_cols->col_cnt+ 1)
   SET stat = alterlist(gin_cols->cols,gin_cols->col_cnt)
   SET gin_cols->cols[gin_cols->col_cnt].col_name = substring(gin_start_pos,((gin_list_len -
    gin_start_pos)+ 1),gin_col_list)
   IF ((gin_cols->col_cnt > 0))
    SELECT INTO "nl:"
     u.index_name, u.column_position
     FROM user_ind_columns u,
      (dummyt d  WITH seq = value(gin_cols->col_cnt))
     PLAN (u
      WHERE u.table_name=cnvtupper(trim(gin_tbl_name,3)))
      JOIN (d
      WHERE d.seq > 0
       AND u.column_position=d.seq
       AND (u.column_name=gin_cols->cols[d.seq].col_name))
     ORDER BY u.index_name, u.column_position
     HEAD u.index_name
      col_cnt = 0
     DETAIL
      col_cnt = (col_cnt+ 1)
     FOOT  u.index_name
      IF ((col_cnt=gin_cols->col_cnt))
       gin_idx_name = u.index_name
      ENDIF
     WITH nocounter
    ;end select
   ENDIF
   RETURN(gin_idx_name)
 END ;Subroutine
 SUBROUTINE dsa_check_nicknames(null)
   DECLARE bg_col_same_ind = i2
   DECLARE run_nick_create = i2
   DECLARE bg_cmp_cnt = i4
   DECLARE tbl_cnt = i4
   DECLARE col_cnt = i4
   DECLARE first_ind = vc
   FREE RECORD admin_rs
   RECORD admin_rs(
     1 admin_tbl_cnt = i4
     1 admin_tbl[*]
       2 admin_table_name = vc
       2 admin_col_cnt = i4
       2 admin_col[*]
         3 admin_col_name = vc
         3 admin_data_type = vc
         3 admin_data_length = i4
   )
   CALL parser(
    "rdb create nickname admin.dm2_user_tab_columns for admin.cdba.dm2_user_tab_columns go")
   COMMIT
   SELECT INTO "nl:"
    dutc.table_name, dutc.column_name
    FROM admin_tables at,
     (admin.dm2_user_tab_columns dutc)
    WHERE dutc.table_name != "DM2*"
     AND dutc.table_name != "DM_INFO"
     AND at.table_name=dutc.table_name
    ORDER BY dutc.table_name, dutc.column_name
    HEAD REPORT
     tbl_cnt = 0, stat = alterlist(admin_rs->admin_tbl,100)
    HEAD dutc.table_name
     col_cnt = 0, tbl_cnt = (tbl_cnt+ 1)
     IF (mod(tbl_cnt,100)=1
      AND tbl_cnt != 1)
      stat = alterlist(admin_rs->admin_tbl,(tbl_cnt+ 99))
     ENDIF
     admin_rs->admin_tbl[tbl_cnt].admin_table_name = dutc.table_name, admin_rs->admin_tbl_cnt =
     tbl_cnt, col_cnt = 0
    DETAIL
     col_cnt = (col_cnt+ 1)
     IF (mod(col_cnt,10)=1)
      stat = alterlist(admin_rs->admin_tbl[tbl_cnt].admin_col,(col_cnt+ 9))
     ENDIF
     admin_rs->admin_tbl[tbl_cnt].admin_col[col_cnt].admin_data_type = dutc.data_type, admin_rs->
     admin_tbl[tbl_cnt].admin_col[col_cnt].admin_col_name = dutc.column_name, admin_rs->admin_tbl[
     tbl_cnt].admin_col[col_cnt].admin_data_length = dutc.data_length,
     admin_rs->admin_tbl[tbl_cnt].admin_col_cnt = col_cnt
    FOOT  dutc.table_name
     stat = alterlist(admin_rs->admin_tbl[tbl_cnt].admin_col,col_cnt)
    FOOT REPORT
     stat = alterlist(admin_rs->admin_tbl,tbl_cnt)
    WITH nocounter
   ;end select
   CALL parser("rdb drop nickname admin.dm2_user_tab_columns go")
   COMMIT
   FREE RECORD nick_rs
   RECORD nick_rs(
     1 nick_tbl_cnt = i4
     1 nick_tbl[*]
       2 nick_table_name = vc
       2 nick_col_cnt = i4
       2 nick_col[*]
         3 nick_col_name = vc
         3 nick_data_type = vc
         3 nick_data_length = i4
   )
   SELECT INTO "nl:"
    du.table_name, du.column_name
    FROM dm2_user_tab_columns du,
     (dummyt d  WITH seq = value(admin_rs->admin_tbl_cnt))
    PLAN (d)
     JOIN (du
     WHERE (du.table_name=admin_rs->admin_tbl[d.seq].admin_table_name))
    ORDER BY du.table_name, du.column_name
    HEAD REPORT
     tbl_cnt = 0, stat = alterlist(nick_rs->nick_tbl,100)
    HEAD du.table_name
     col_cnt = 0, tbl_cnt = (tbl_cnt+ 1)
     IF (mod(tbl_cnt,100)=1
      AND tbl_cnt != 1)
      stat = alterlist(nick_rs->nick_tbl,(tbl_cnt+ 99))
     ENDIF
     nick_rs->nick_tbl[tbl_cnt].nick_table_name = du.table_name, nick_rs->nick_tbl_cnt = tbl_cnt
    DETAIL
     col_cnt = (col_cnt+ 1)
     IF (mod(col_cnt,10)=1)
      stat = alterlist(nick_rs->nick_tbl[tbl_cnt].nick_col,(col_cnt+ 9))
     ENDIF
     nick_rs->nick_tbl[tbl_cnt].nick_col[col_cnt].nick_col_name = du.column_name, nick_rs->nick_tbl[
     tbl_cnt].nick_col[col_cnt].nick_data_type = du.data_type, nick_rs->nick_tbl[tbl_cnt].nick_col[
     col_cnt].nick_data_length = du.data_length,
     nick_rs->nick_tbl[tbl_cnt].nick_col_cnt = col_cnt
    FOOT  du.table_name
     stat = alterlist(nick_rs->nick_tbl[tbl_cnt].nick_col,col_cnt)
    FOOT REPORT
     stat = alterlist(nick_rs->nick_tbl,tbl_cnt)
    WITH nocounter
   ;end select
   IF (value(size(nick_rs->nick_tbl,5)) != value(size(admin_rs->admin_tbl,5)))
    CALL echo("Number of nicknames are different from number of Admin tables")
    SET run_nick_create = 1
   ENDIF
   IF (run_nick_create=1)
    RETURN(0)
   ENDIF
   SELECT INTO "nl"
    FROM (dummyt d  WITH seq = admin_rs->admin_tbl_cnt),
     (dummyt d2  WITH seq = nick_rs->nick_tbl_cnt)
    PLAN (d)
     JOIN (d2
     WHERE (admin_rs->admin_tbl[d.seq].admin_table_name=nick_rs->nick_tbl[d2.seq].nick_table_name))
    HEAD REPORT
     bg_cmp_cnt = 0
    DETAIL
     IF ((admin_rs->admin_tbl[d.seq].admin_col_cnt=nick_rs->nick_tbl[d2.seq].nick_col_cnt))
      WHILE ((bg_cmp_cnt < admin_rs->admin_tbl[d.seq].admin_col_cnt)
       AND bg_col_same_ind != 1)
       bg_cmp_cnt = (bg_cmp_cnt+ 1),
       IF ((admin_rs->admin_tbl[d.seq].admin_col[bg_cmp_cnt].admin_col_name=nick_rs->nick_tbl[d2.seq]
       .nick_col[bg_cmp_cnt].nick_col_name)
        AND (admin_rs->admin_tbl[d.seq].admin_col[bg_cmp_cnt].admin_data_type=nick_rs->nick_tbl[d2
       .seq].nick_col[bg_cmp_cnt].nick_data_type)
        AND (admin_rs->admin_tbl[d.seq].admin_col[bg_cmp_cnt].admin_data_length=nick_rs->nick_tbl[
       tbl_cnt].nick_col[col_cnt].nick_data_length)
        AND bg_col_same_ind != 1)
        bg_col_same_ind = 0
       ELSE
        bg_col_same_ind = 1, run_nick_create = 1
       ENDIF
      ENDWHILE
     ELSE
      bg_col_same_ind = 1, run_nick_create = 1
   ...
   ;end select
   IF (run_nick_create=1)
    CALL echo("Data type or quantity differences b/t Nicknames and Admin.")
    RETURN(0)
   ENDIF
   FREE RECORD ccldef_rs
   RECORD ccldef_rs(
     1 ccldef_tbl_cnt = i4
     1 ccldef_tbl[*]
       2 ccldef_table_name = vc
       2 ccldef_col_cnt = i4
   )
   SELECT INTO "nl:"
    FROM dtable t,
     dtableattr a,
     dtableattrl l,
     (dummyt d  WITH seq = value(nick_rs->nick_tbl_cnt))
    PLAN (d)
     JOIN (t
     WHERE (nick_rs->nick_tbl[d.seq].nick_table_name=t.table_name))
     JOIN (a
    ...
    HEAD REPORT
     tbl_cnt = 0, stat = alterlist(ccldef_rs->ccldef_tbl,100)
    HEAD t.table_name
     tbl_cnt = (tbl_cnt+ 1)
     IF (mod(tbl_cnt,100)=1
      AND tbl_cnt != 1)
      stat = alterlist(ccldef_rs->ccldef_tbl,(tbl_cnt+ 99))
     ENDIF
     ccldef_rs->ccldef_tbl[tbl_cnt].ccldef_table_name = t.table_name, ccldef_rs->ccldef_tbl[tbl_cnt].
     ccldef_col_cnt = t.num_attr
    FOOT REPORT
     stat = alterlist(ccldef_rs->ccldef_tbl,tbl_cnt), ccldef_rs->ccldef_tbl_cnt = tbl_cnt
    WITH nocounter
   ;end select
   IF ((nick_rs->nick_tbl_cnt != ccldef_rs->ccldef_tbl_cnt))
    SET run_nick_create = 1
   ENDIF
   IF (run_nick_create=1)
    RETURN(0)
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = nick_rs->nick_tbl_cnt),
     (dummyt d2  WITH seq = ccldef_rs->ccldef_tbl_cnt)
    PLAN (d)
     JOIN (d2
     WHERE (nick_rs->nick_tbl[d.seq].nick_table_name=ccldef_rs->ccldef_tbl[d2.seq].ccldef_table_name)
     )
    HEAD REPORT
     run_nick_create = 0
    DETAIL
     IF ((nick_rs->nick_tbl[d.seq].nick_col_cnt != ccldef_rs->ccldef_tbl[d2.seq].ccldef_col_cnt))
      run_nick_create = 1
     ENDIF
    WITH nocounter
   ;end select
 ...
 END ;Subroutine
 IF (currdb="DB2UDB")
  DECLARE starting_pos = i2
  DECLARE compare_date = q8
  DECLARE schema_date = vc
  DECLARE stripped_date = q8
  DECLARE dosa_dcl_find = vc
  IF (findfile("ccluserdir:dosa_schema_date.dat")=1)
   SET stat = remove("ccluserdir:dosa_schema_date.dat")
  ENDIF
  SET dosa_dcl_find = "ls -t $cer_install/dm2a????????_t.dat"
  IF (dm2_push_dcl(dosa_dcl_find)=0)
   CALL echo("FAILURE: No schema_files were found in $cer_install")
   GO TO end_program
  ELSE
   CALL dm2_push_dcl("ls -t $cer_install/dm2a????????_t.dat>>dosa_schema_date.dat")
   SET logical testlog "ccluserdir:dosa_schema_date.dat"
   FREE DEFINE rtl
   DEFINE rtl "testlog"
   SELECT INTO "nl:"
    r.line
    FROM rtlt r
    HEAD REPORT
     compare_date = cnvtdate("01-JAN-1900")
    DETAIL
     starting_pos = findstring("dm2a",r.line)
     IF (starting_pos != 0)
      stripped_date = cnvtdate(substring((starting_pos+ 4),8,r.line))
      IF (stripped_date > compare_date)
       schema_date = format(stripped_date,"dd-mmm-yyyy;;d"), compare_date = stripped_date
  ...
  ENDIF
  IF (schema_date="")
   CALL echo("FAILURE: Invalid Schema date.")
   GO TO end_program
  ENDIF
  ...
  dm2_install_schema->p_word, "none", "none",
  "none"
  IF ((dm_err->err_ind=1))
   CALL echo(build("Error occured when calling dm2_install_schema",dm_err->logfile))
   GO TO end_program
  ENDIF
  IF ((tgtsch->diff_ind=1))
   CALL echo(build("Error occured when calling dm2_install_schema. Schema difference ind. ",tgtsch->
     diff_ind))
   GO TO end_program
  ENDIF
  ...
  IF (dsa_check_nicknames(null)=0)
   CALL echo("Getting ready to execute dm2_create_admin_nicknames.")
   EXECUTE dm2_create_admin_nicknames "*", dm2_install_schema->p_word
  ENDIF
 ENDIF
 EXECUTE dm2_create_system_defs
 SET u_name = fillstring(20," ")
 SET p_word = fillstring(20," ")
 SET errcode = 0
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   adm_link = cnvtlower(substring(1,(findstring(".",a.db_link) - 1),a.db_link))
  WITH nocounter
 ;end select
 IF (curqual=0)
  CALL echo("*****************************************************************")
 ...
 WHILE (done=0)
   ...
   SET connect_str = concat(trim(u_name),"/",trim(p_word),"@",trim(adm_link))
   ...
   ELSE
    ...
    ;end select
    IF (curqual > 0)
     CALL echo("Successful connection to ADMIN made.")
     SET done = 1
     EXECUTE dm2_create_system_defs
    ELSE
     CALL echo("Successful connection to ADMIN not made.")
     CALL echo("DM_SCHEMA_VERSION not found in user_objects.")
    ENDIF
   ENDIF
 ENDWHILE
 DECLARE dosa_chk_tspace = i2
 DECLARE dosa_chk_db = i2
 SET dosa_chk_tspace = 0
 SET dosa_chk_db = 0
 SELECT INTO "nl:"
 ...
 IF (curqual > 0)
  SET dosa_chk_db = 1
 ENDIF
 IF (dosa_chk_tspace=1
  AND dosa_chk_db=1)
  CALL echo("Not executed in Internal Rev Admin Database.")
  GO TO end_program
 ENDIF
 SET parser_count = 0
 ...
 IF (curqual=0)
  ...
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "feature_number number,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "archive_dt_tm date,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "product_area_number number default 0,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "product_area_name varchar2(100) )"
  SET parser_count = (parser_count+ 1)
  ...
  SET parser_buffer[parser_count] = "go"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb create index XIE2DM_ALPHA_FEATURES"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "on DM_ALPHA_FEATURES (product_area_number)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  SET x = 0
  IF (pk_columns("DM_ALPHA_FEATURES") != 1)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_alpha_features"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_alpha_features"
  ...
  ENDIF
 ENDIF
 IF (column_missing("DM_ALPHA_FEATURES","FEATURE_NUMBER"))
  CALL push("rdb alter table DM_ALPHA_FEATURES add (FEATURE_NUMBER number) go")
 ENDIF
 IF (column_missing("DM_ALPHA_FEATURES","ARCHIVE_DT_TM"))
  CALL push("rdb alter table DM_ALPHA_FEATURES add (ARCHIVE_DT_TM date) go")
 ENDIF
 IF (column_missing("DM_ALPHA_FEATURES","PRODUCT_AREA_NUMBER"))
  CALL push("rdb alter table DM_ALPHA_FEATURES add (PRODUCT_AREA_NUMBER number) go")
  CALL push("rdb alter table DM_ALPHA_FEATURES modify (PRODUCT_AREA_NUMBER default 0) go")
 ENDIF
 IF (column_missing("DM_ALPHA_FEATURES","PRODUCT_AREA_NAME"))
  CALL push("rdb alter table DM_ALPHA_FEATURES add (PRODUCT_AREA_NAME varchar2(100) ) go")
 ENDIF
 SELECT INTO "nl:"
  FROM user_indexes u
  WHERE u.table_name="DM_ALPHA_FEATURES"
   AND u.index_name="XIE2DM_ALPHA_FEATURES"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb create index XIE2DM_ALPHA_FEATURES on "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "DM_ALPHA_FEATURES (product_area_number) "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace I_TOOLKIT "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ...
 ELSE
  SET x = 0
  IF (pk_columns("DM_OCD_FEATURES") != 2)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_ocd_features"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
 ...
 IF (curqual=0)
  ...
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "inst_mode varchar2(15),"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "calling_script varchar2(30),"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "curr_migration_ind NUMBER )"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "storage (initial 16K next 16K)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace D_TOOLKIT"
 ...
 ELSE
  SET x = 0
  SET len = 0
  IF (pk_columns("DM_ALPHA_FEATURES_ENV") != 2)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_alpha_features_env"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
  ...
  ENDIF
  SET inst_mode_ind = 0
  SET calling_script_ind = 0
  SET curr_mig_ind = 0
  SELECT INTO "nl:"
   FROM user_tab_columns u
   WHERE u.table_name="DM_ALPHA_FEATURES_ENV"
    AND u.column_name IN ("STATUS", "INST_MODE", "CALLING_SCRIPT", "CURR_MIGRATION_IND")
   DETAIL
    IF (u.column_name="STATUS")
     len = u.data_length
    ELSEIF (u.column_name="INST_MODE")
     inst_mode_ind = 1
    ELSEIF (u.column_name="CALLING_SCRIPT")
     calling_script_ind = 1
    ELSEIF (u.column_name="CURR_MIGRATION_IND")
     curr_mig_ind = 1
    ENDIF
   WITH nocounter
  ;end select
  ...
  IF (inst_mode_ind=0)
   ...
   SET parser_buffer[parser_count] = "rdb alter table dm_alpha_features_env"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "add (inst_mode VARCHAR2(15))"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (calling_script_ind=0)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_alpha_features_env"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "add (calling_script VARCHAR2(30))"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (curr_mig_ind=0)
   ...
   SET parser_buffer[parser_count] = "rdb alter table dm_alpha_features_env"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "add (curr_migration_ind NUMBER)"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
  ENDIF
 ENDIF
 ...
 IF (curqual=0)
  ...
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "schema_date date)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_tables"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_TABLES"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,table_name)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
  CALL push("rdb alter table dm_afd_tables add schema_instance number default 0 go")
  CALL push("rdb alter table dm_afd_tables modify schema_instance not null go")
 ELSE
  SET x = 0
  SET schema_date_ind = 0
  SET feature_nbr_ind = 0
  SELECT INTO "nl:"
   FROM user_tab_columns u
   WHERE u.table_name="DM_AFD_TABLES"
    AND u.column_name IN ("FEATURE_NUMBER", "SCHEMA_DATE")
   DETAIL
    IF (u.column_name="FEATURE_NUMBER")
     feature_nbr_ind = 1
    ELSEIF (u.column_name="SCHEMA_DATE")
     schema_date_ind = 1
    ENDIF
   WITH nocounter
  ;end select
  IF (feature_nbr_ind=0)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_tables"
   SET parser_count = (parser_count+ 1)
   ...
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (schema_date_ind=0)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_tables"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "add (schema_date date)"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
  ENDIF
  SET drop_ind = 0
  SELECT INTO "nl:"
   ...
   WHERE u.table_name="DM_AFD_TABLES"
    AND u.constraint_name="XPKDM_AFD_TABLES"
    AND u.position=1
   DETAIL
    IF (u.column_name != "ALPHA_FEATURE_NBR")
     drop_ind = 1
    ENDIF
   WITH nocounter
  ;end select
  IF (drop_ind=1)
   ...
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_tables"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop constraint xpkdm_afd_tables"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (((curqual=0) OR (drop_ind=1)) )
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_tables"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_TABLES"
   ...
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (column_missing("DM_AFD_TABLES","SCHEMA_INSTANCE"))
   CALL push("rdb alter table dm_afd_tables add schema_instance number default 0 go")
   CALL push("rdb alter table dm_afd_tables modify schema_instance not null go")
  ENDIF
 ENDIF
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_columns"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_COLUMNS"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,table_name,column_name)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  ...
  SELECT INTO "nl:"
   FROM user_cons_columns u
   WHERE u.table_name="DM_AFD_COLUMNS"
    AND u.constraint_name="XPKDM_AFD_COLUMNS"
    AND u.position=1
   DETAIL
    IF (u.column_name != "ALPHA_FEATURE_NBR")
     drop_ind = 1
  ...
  IF (drop_ind=1)
   ...
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_columns"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop constraint xpkdm_afd_columns"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (((curqual=0) OR (drop_ind=1)) )
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_constraints"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_CONSTRAINTS"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,table_name,constraint_name)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
  CALL push("rdb alter table dm_afd_constraints")
  CALL push("add full_cons_name varchar2(30) go")
 ELSE
  SET x = 0
  SET drop_ind = 0
  SELECT INTO "nl:"
   FROM user_cons_columns u
   WHERE u.table_name="DM_AFD_CONSTRAINTS"
    AND u.constraint_name="XPKDM_AFD_CONSTRAINTS"
    AND u.position=1
   DETAIL
    IF (u.column_name != "ALPHA_FEATURE_NBR")
     drop_ind = 1
  ...
  IF (drop_ind=1)
   ...
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_constraints"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop constraint xpkdm_afd_constraints"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (((curqual=0) OR (drop_ind=1)) )
   ...
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (column_missing("DM_AFD_CONSTRAINTS","FULL_CONS_NAME"))
   CALL push("rdb alter table dm_afd_constraints")
   CALL push("add full_cons_name varchar2(30) go")
  ENDIF
 ENDIF
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_cons_columns"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_CONS_COLUMNS primary key"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "(alpha_feature_nbr,table_name,constraint_name,column_name)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  ...
  SELECT INTO "nl:"
   FROM user_cons_columns u
   WHERE u.table_name="DM_AFD_CONS_COLUMNS"
    AND u.constraint_name="XPKDM_AFD_CONS_COLUMNS"
    AND u.position=1
   DETAIL
    IF (u.column_name != "ALPHA_FEATURE_NBR")
     drop_ind = 1
  ...
  IF (drop_ind=1)
   ...
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_cons_columns"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop constraint xpkdm_afd_cons_columns"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (((curqual=0) OR (drop_ind=1)) )
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_indexes"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_INDEXES"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,table_name,index_name)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
  CALL push("rdb alter table dm_afd_indexes")
  CALL push("add full_ind_name varchar2(30) go")
 ELSE
  SET x = 0
  SET drop_ind = 0
  SELECT INTO "nl:"
   FROM user_cons_columns u
   WHERE u.table_name="DM_AFD_INDEXES"
    AND u.constraint_name="XPKDM_AFD_INDEXES"
    AND u.position=1
   DETAIL
    IF (u.column_name != "ALPHA_FEATURE_NBR")
     drop_ind = 1
  ...
  IF (drop_ind=1)
   ...
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_indexes"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop constraint xpkdm_afd_indexes"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (((curqual=0) OR (drop_ind=1)) )
   ...
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (column_missing("DM_AFD_INDEXES","FULL_IND_NAME"))
   CALL push("rdb alter table dm_afd_indexes")
   CALL push("add full_ind_name varchar2(30) go")
  ENDIF
 ENDIF
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "tablespace D_TOOLKIT as"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "(select index_name, table_name, column_name, column_position,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "updt_applctx,updt_dt_tm, updt_cnt, updt_id, updt_task"
  SET parser_count = (parser_count+ 1)
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_index_columns"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_INDEX_COLUMNS"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] =
  "primary key (alpha_feature_nbr,table_name,index_name, column_name)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  ...
  SELECT INTO "nl:"
   FROM user_cons_columns u
   WHERE u.table_name="DM_AFD_INDEX_COLUMNS"
    AND u.constraint_name="XPKDM_AFD_INDEX_COLUMNS"
    AND u.position=1
   DETAIL
    IF (u.column_name != "ALPHA_FEATURE_NBR")
     drop_ind = 1
  ...
  IF (drop_ind=1)
   ...
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_index_columns"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop constraint xpkdm_afd_index_columns"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (((curqual=0) OR (drop_ind=1)) )
   ...
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_index_columns"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_INDEX_COLUMNS primary key"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "(alpha_feature_nbr,table_name,index_name, column_name)"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "go"
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "tablespace D_TOOLKIT as"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "(select CODE_SET,DISPLAY,DISPLAY_KEY,DESCRIPTION,DEFINITION,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "TABLE_NAME,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] =
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_code_value_set"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_CODE_VALUE_SET"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,code_set)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  ...
  IF (curqual=0)
   ...
   SET parser_buffer[parser_count] = "go"
  ENDIF
  IF (pk_columns("DM_AFD_CODE_VALUE_SET") != 2)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_code_value_set"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
 ...
 SELECT INTO "nl:"
  FROM user_indexes u
  WHERE u.table_name="DM_AFD_CODE_VALUE_SET"
   AND u.index_name="XIE1DM_AFD_CODE_VALUE_SET"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb create index xie1dm_afd_code_value_set on "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "dm_afd_code_value_set(alpha_feature_nbr) "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace I_TOOLKIT "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ...
 IF (curqual=0)
  ...
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] =
  "(select code_value, code_set, cdf_meaning, display, display_key,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "description,definition,collation_seq,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] =
  "active_type_cd,active_ind,active_dt_tm,inactive_dt_tm,updt_applctx,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "updt_dt_tm,updt_cnt,updt_id,updt_task,begin_effective_dt_tm,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "end_effective_dt_tm, data_status_cd, data_status_dt_tm,"
  SET parser_count = (parser_count+ 1)
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_code_value"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_CODE_VALUE"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,code_set,code_value)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  SET x = 0
  IF (pk_columns("DM_AFD_CODE_VALUE") != 3)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_code_value"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
 ...
 SELECT INTO "nl:"
  FROM user_indexes u
  WHERE u.table_name="DM_AFD_CODE_VALUE"
   AND u.index_name="XIE1DM_AFD_CODE_VALUE"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb create index xie1dm_afd_code_value on "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "dm_afd_code_value(alpha_feature_nbr) "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace I_TOOLKIT "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ...
 SELECT INTO "nl:"
  FROM user_indexes u
  WHERE u.table_name="DM_AFD_CODE_VALUE"
   AND u.index_name="XIE2DM_AFD_CODE_VALUE"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb create index xie2dm_afd_code_value on "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "dm_afd_code_value(code_value) "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace I_TOOLKIT "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_common_data_foundation"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_COMMON_DATA_FOUND"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,code_set,cdf_meaning)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  SET x = 0
  IF (pk_columns("DM_AFD_COMMON_DATA_FOUNDATION") != 3)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_common_data_foundation"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
 ...
 SELECT INTO "nl:"
  FROM user_indexes u
  WHERE u.table_name="DM_AFD_COMMON_DATA_FOUNDATION"
   AND u.index_name="XIE1DM_AFD_COMMON_DATA_FOUND"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb create index xie1dm_afd_common_data_found on "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "dm_afd_common_data_foundation(alpha_feature_nbr) "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace I_TOOLKIT "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "storage (initial 16K next 16K)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace D_TOOLKIT as (select code_set,field_name,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "field_seq,field_type,field_len,field_prompt,field_in_mask,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "field_out_mask,validation_condition,validation_code_set,"
  SET parser_count = (parser_count+ 1)
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_code_set_extension"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_CODE_SET_EXTENSION"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,code_set,field_name)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  SET x = 0
  IF (pk_columns("DM_AFD_CODE_SET_EXTENSION") != 3)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_code_set_extension"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
  ...
  SELECT INTO "nl:"
   ...
   WHERE u.table_name="DM_AFD_CODE_SET_EXTENSION"
    AND u.constraint_name="XPKDM_AFD_CODE_SET_EXTENSION"
   WITH nocounter
  ;end select
  IF (curqual=0)
   SET parser_count = (parser_count+ 1)
 ...
 SELECT INTO "nl:"
  FROM user_indexes u
  WHERE u.table_name="DM_AFD_CODE_SET_EXTENSION"
   AND u.index_name="XIE1DM_AFD_CODE_SET_EXTEN"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb create index xie1dm_afd_code_set_exten on "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "dm_afd_code_set_extension(alpha_feature_nbr) "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace I_TOOLKIT "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ...
 IF (curqual=0)
  ...
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] =
  "tablespace D_TOOLKIT as (select code_set, contributor_source_cd,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "alias, code_value, primary_ind,updt_applctx,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "updt_dt_tm, updt_cnt, updt_id, updt_task , alias_type_meaning"
  SET parser_count = (parser_count+ 1)
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_code_value_alias"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_CODE_VALUE_ALIAS primary key "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] =
  "(alpha_feature_nbr,code_set,alias,code_value,contributor_source_cd)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  SET x = 0
  IF (pk_columns("DM_AFD_CODE_VALUE_ALIAS") != 5)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_code_value_alias"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
 ...
 SELECT INTO "nl:"
  FROM user_indexes u
  WHERE u.table_name="DM_AFD_CODE_VALUE_ALIAS"
   AND u.index_name="XIE1DM_AFD_CODE_VALUE_ALIAS"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb create index xie1dm_afd_code_value_alias on "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "dm_afd_code_value_alias(alpha_feature_nbr) "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace I_TOOLKIT "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ...
 SELECT INTO "nl:"
  FROM user_indexes u
  WHERE u.table_name="DM_AFD_CODE_VALUE_ALIAS"
   AND u.index_name="XIE2DM_AFD_CODE_VALUE_ALIAS"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb create index xie2dm_afd_code_value_alias on "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "dm_afd_code_value_alias(code_value) "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace I_TOOLKIT "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "storage (initial 16K next 16K)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace D_TOOLKIT as (select code_value,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "field_name, code_set, updt_applctx,updt_dt_tm,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "updt_cnt, updt_id, updt_task , field_type, field_value"
  SET parser_count = (parser_count+ 1)
  ...
  SET parser_buffer[parser_count] = "rdb alter table dm_afd_code_value_extension"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add constraint XPKDM_AFD_CODE_VALUE_EXTENSION"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "primary key(alpha_feature_nbr,code_set,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "field_name, code_value)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  SET x = 0
  IF (pk_columns("DM_AFD_CODE_VALUE_EXTENSION") != 4)
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_afd_code_value_extension"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
 ...
 SELECT INTO "nl:"
  FROM user_indexes u
  WHERE u.table_name="DM_AFD_CODE_VALUE_EXTENSION"
   AND u.index_name="XIE1DM_AFD_CODE_VALUE_EXTEN"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb create index xie1dm_afd_code_value_exten on "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "dm_afd_code_value_extension(alpha_feature_nbr) "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace I_TOOLKIT "
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "alpha_feature_nbr NUMBER not null,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "common_application_ind NUMBER)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "storage (initial 16K next 16K)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace D_TOOLKIT"
  ...
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "application_number)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  SET x = 0
  IF (column_missing("DM_OCD_APPLICATION","COMMON_APPLICATION_IND"))
   CALL push("rdb alter table DM_OCD_APPLICATION add COMMON_APPLICATION_IND number go")
  ENDIF
  IF (pk_columns("DM_OCD_APPLICATION") != 2)
   EXECUTE dm_ocd_fix_adm_atr "APP"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_ocd_application"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "alpha_feature_nbr NUMBER not null,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "old_task_number NUMBER)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "storage (initial 16K next 16K)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace D_TOOLKIT"
  ...
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "task_number)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  SET x = 0
  IF (column_missing("DM_OCD_TASK","OLD_TASK_NUMBER"))
   CALL push("rdb alter table DM_OCD_TASK add OLD_TASK_NUMBER number go")
  ENDIF
  IF (pk_columns("DM_OCD_TASK") != 2)
   EXECUTE dm_ocd_fix_adm_atr "TASK"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_ocd_task"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
 ...
 IF (curqual=0)
  ...
  SET parser_buffer[parser_count] = "alpha_feature_nbr NUMBER not null,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "cachegrace NUMBER,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "cachestale NUMBER,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "cachetrim VARCHAR2(20))"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "storage (initial 16K next 16K)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "tablespace D_TOOLKIT"
  ...
  SET parser_buffer[parser_count] = "primary key (alpha_feature_nbr,"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "request_number)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "using index tablespace I_TOOLKIT"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ELSE
  IF (pk_columns("DM_OCD_REQUEST") != 2)
   EXECUTE dm_ocd_fix_adm_atr "REQUEST"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_ocd_request"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
  ...
  IF (curqual < 4)
   ...
   SET parser_buffer[parser_count] = "rdb alter table dm_ocd_request"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "add (cachegrace NUMBER,"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "cachestale NUMBER,"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "cachetrim VARCHAR2(20))"
 ...
 ELSE
  SET x = 0
  IF (pk_columns("DM_OCD_APP_TASK_R") != 3)
   EXECUTE dm_ocd_fix_adm_atr "APP_TASK_R"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_ocd_app_task_r"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
 ...
 ELSE
  SET x = 0
  IF (pk_columns("DM_OCD_TASK_REQ_R") != 3)
   EXECUTE dm_ocd_fix_adm_atr "TASK_REQ_R"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "rdb alter table dm_ocd_task_req_r"
   SET parser_count = (parser_count+ 1)
   SET parser_buffer[parser_count] = "drop primary key cascade"
 ...
 SELECT INTO "nl:"
  FROM user_tab_columns u
  WHERE u.table_name="DM_TABLES_DOC"
   AND u.column_name="FREELIST_CNT"
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb alter table dm_tables_doc"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "add (FREELIST_CNT NUMBER DEFAULT 1)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ENDIF
 IF (column_missing("DM_ENVIRONMENT","PRODUCTION_IND"))
  CALL push("rdb alter table DM_ENVIRONMENT add (PRODUCTION_IND NUMBER) go")
  CALL push("rdb alter table DM_ENVIRONMENT modify (PRODUCTION_IND DEFAULT 0) go")
  CALL push("rdb update dm_environment d set d.production_ind=0 where d.production_ind is null go")
  CALL push("commit go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","DROP_IND"))
  CALL push("rdb alter table DM_TABLES_DOC add (DROP_IND NUMBER) go")
  CALL push("rdb alter table DM_TABLES_DOC modify (DROP_IND DEFAULT 0) go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","TABLE_SUFFIX"))
  CALL push("rdb alter table DM_TABLES_DOC add (TABLE_SUFFIX VARCHAR2(4)) go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","FULL_TABLE_NAME"))
  CALL push("rdb alter table DM_TABLES_DOC add (FULL_TABLE_NAME VARCHAR2(30)) go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","SUFFIXED_TABLE_NAME"))
  CALL push("rdb alter table DM_TABLES_DOC add (SUFFIXED_TABLE_NAME VARCHAR2(18)) go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","DEFAULT_ROW_IND"))
  CALL push("rdb alter table DM_TABLES_DOC add (DEFAULT_ROW_IND NUMBER) go")
  CALL push("rdb alter table DM_TABLES_DOC modify (DEFAULT_ROW_IND DEFAULT 0) go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","PERSON_CMB_TRIGGER_TYPE"))
  CALL push("rdb alter table DM_TABLES_DOC add (PERSON_CMB_TRIGGER_TYPE VARCHAR2(10)) go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","ENCNTR_CMB_TRIGGER_TYPE"))
  CALL push("rdb alter table DM_TABLES_DOC add (ENCNTR_CMB_TRIGGER_TYPE VARCHAR2(10)) go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","MERGE_UI_QUERY"))
  CALL push("rdb alter table DM_TABLES_DOC add (MERGE_UI_QUERY VARCHAR2(255)) go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","MERGEABLE_IND"))
  CALL push("rdb alter table DM_TABLES_DOC add (MERGEABLE_IND NUMBER) go")
  CALL push("rdb alter table DM_TABLES_DOC modify (MERGEABLE_IND DEFAULT 0) go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","MERGE_DELETE_IND"))
  CALL push("rdb alter table DM_TABLES_DOC add (MERGE_DELETE_IND NUMBER) go")
  CALL push("rdb alter table DM_TABLES_DOC modify (MERGE_DELETE_IND DEFAULT 0) go")
 ENDIF
 IF (column_missing("DM_TABLES_DOC","MERGE_ACTIVE_IND"))
  CALL push("rdb alter table DM_TABLES_DOC add (MERGE_ACTIVE_IND NUMBER) go")
  CALL push("rdb alter table DM_TABLES_DOC modify (MERGE_ACTIVE_IND DEFAULT 0) go")
 ENDIF
 IF (column_missing("DM_COLUMNS_DOC","ABSOLUTE_DATE_IND"))
  CALL push("rdb alter table DM_COLUMNS_DOC add (ABSOLUTE_DATE_IND NUMBER) go")
  CALL push("rdb alter table DM_COLUMNS_DOC modify (ABSOLUTE_DATE_IND DEFAULT 0) go")
 ENDIF
 IF (column_missing("DM_COLUMNS_DOC","MERGE_DELETE_IND"))
  CALL push("rdb alter table DM_COLUMNS_DOC add (MERGE_DELETE_IND NUMBER) go")
  CALL push("rdb alter table DM_COLUMNS_DOC modify (MERGE_DELETE_IND DEFAULT 0) go")
 ENDIF
 IF (table_missing("DM_TEMP_AFD_TABLES")=0)
  IF (column_missing("DM_TEMP_AFD_TABLES","SCHEMA_INSTANCE"))
   CALL push("rdb alter table dm_temp_afd_tables add schema_instance number default 0 go")
   CALL push("rdb alter table dm_temp_afd_tables modify schema_instance not null go")
  ENDIF
 ENDIF
 IF (column_missing("DM_ADM_TABLES","SCHEMA_INSTANCE"))
  CALL push("rdb alter table dm_adm_tables add schema_instance number default 0 go")
  CALL push("rdb alter table dm_adm_tables modify schema_instance not null go")
 ENDIF
 IF (dsa_get_index_col_cnt("DM_TABLES_DOC","XIE2DM_TABLES_DOC") > 0)
  CALL push("rdb drop index XIE2DM_TABLES_DOC go")
 ENDIF
 CALL push("rdb CREATE INDEX XIE2DM_TABLES_DOC ON DM_TABLES_DOC")
 CALL push("(FULL_TABLE_NAME)")
 CALL push("TABLESPACE I_TOOLKIT")
 CALL push("STORAGE (INITIAL 100K NEXT 100K) go")
 IF (dsa_get_index_col_cnt("DM_TABLES_DOC","XIE3DM_TABLES_DOC") > 0)
  CALL push("rdb drop index XIE3DM_TABLES_DOC go")
 ENDIF
 CALL push("rdb CREATE INDEX XIE3DM_TABLES_DOC ON DM_TABLES_DOC")
 CALL push("(SUFFIXED_TABLE_NAME)")
 CALL push("TABLESPACE I_TOOLKIT")
 CALL push("STORAGE (INITIAL 100K NEXT 100K) go")
 IF (table_missing("DM_TS_PRECEDENCE"))
  CALL push("rdb CREATE TABLE dm_ts_precedence")
  CALL push("(TABLE_NAME         varchar2(30) NOT NULL,")
  CALL push("PRECEDENCE          NUMBER NOT NULL,")
  CALL push("DATA_TABLESPACE     VARCHAR2(  30 ) NOT NULL,")
  CALL push("DATA_EXTENT_SIZE    NUMBER NOT NULL,")
  CALL push("INDEX_TABLESPACE    VARCHAR2(30) NOT NULL,")
  CALL push("INDEX_EXTENT_SIZE   NUMBER NOT NULL,")
  CALL push("LONG_TABLESPACE     VARCHAR2(30) NOT NULL,")
  CALL push("LONG_EXTENT_SIZE    NUMBER NOT NULL,")
  CALL push("UPDT_DT_TM DATE     DEFAULT sysdate NOT NULL,")
  CALL push("UPDT_APPLCTX        NUMBER DEFAULT 0 NOT NULL,")
  CALL push("UPDT_ID             NUMBER DEFAULT 0 NOT NULL,")
  CALL push("UPDT_CNT            NUMBER DEFAULT 0 NOT NULL,")
  CALL push("UPDT_TASK           NUMBER)")
  CALL push("PCTFREE 5")
  CALL push("TABLESPACE D_TOOLKIT")
  CALL push("STORAGE(INITIAL 80K NEXT 80K) go")
  CALL push("rdb ALTER TABLE dm_ts_precedence ADD CONSTRAINT XPKdm_ts_precedence")
  CALL push("PRIMARY KEY (TABLE_NAME,PRECEDENCE)")
  CALL push("USING INDEX TABLESPACE I_TOOLKIT")
  CALL push("STORAGE (INITIAL 40K NEXT 40K) go")
  CALL push("rdb ALTER TABLE dm_ts_precedence ADD CONSTRAINT XFK1DM_TS_PRECEDENCE")
  CALL push("FOREIGN KEY (TABLE_NAME)")
  CALL push("REFERENCES DM_TABLES_DOC (TABLE_NAME) disable go")
 ELSE
  SELECT INTO "nl:"
   FROM user_constraints u
   WHERE u.table_name="DM_TS_PRECEDENCE"
    AND u.constraint_name="XPK1DM_TS_PRECEDENCE"
   WITH nocounter
  ;end select
  IF (curqual=0)
   CALL push("rdb alter table DM_TS_PRECEDENCE drop primary key cascade go")
   CALL push("rdb ALTER TABLE dm_ts_precedence ADD CONSTRAINT XPKDM_TS_PRECEDENCE")
   CALL push("PRIMARY KEY (TABLE_NAME,PRECEDENCE)")
   CALL push("USING INDEX TABLESPACE I_TOOLKIT")
   CALL push("STORAGE (INITIAL 40K NEXT 40K) go")
  ELSE
   IF (pk_columns("DM_TS_PRECEDENCE") != 2)
    CALL push("rdb alter table DM_TS_PRECEDENCE drop primary key cascade go")
    CALL push("rdb ALTER TABLE dm_ts_precedence ADD CONSTRAINT XPKDM_TS_PRECEDENCE")
    CALL push("PRIMARY KEY (TABLE_NAME,PRECEDENCE)")
    CALL push("USING INDEX TABLESPACE I_TOOLKIT")
    CALL push("STORAGE (INITIAL 40K NEXT 40K) go")
   ELSE
    IF (((pk_col_pos("DM_TS_PRECEDENCE","TABLE_NAME",1)=0) OR (pk_col_pos("DM_TS_PRECEDENCE",
     "PRECEDENCE",2)=0)) )
     CALL push("rdb alter table DM_TS_PRECEDENCE drop primary key cascade go")
     CALL push("rdb ALTER TABLE dm_ts_precedence ADD CONSTRAINT XPKDM_TS_PRECEDENCE")
     CALL push("PRIMARY KEY (TABLE_NAME,PRECEDENCE)")
  ...
  SELECT INTO "nl:"
   FROM user_constraints u
   WHERE u.table_name="DM_TS_PRECEDENCE"
    AND u.constraint_name="XFK1DM_TS_PRECEDENCE"
   WITH nocounter
  ;end select
  IF (curqual=0)
   CALL push("rdb ALTER TABLE dm_ts_precedence ADD CONSTRAINT XFK1DM_TS_PRECEDENCE")
  ...
  ENDIF
 ENDIF
 IF (table_missing("OCD_README_COMPONENT"))
  CALL push("rdb CREATE TABLE ocd_readme_component")
  CALL push("(PRODUCT_AREA_NAME VARCHAR2(80),")
  CALL push(" END_STATE VARCHAR2(255),")
  CALL push(" MANUAL_IND NUMBER DEFAULT 1,")
  CALL push(" UPDT_DT_TM DATE ,")
  CALL push(" PRODUCT_AREA_NUMBER  NUMBER ,")
  CALL push(" COMPONENT_TYPE  VARCHAR2(40))")
  CALL push("PCTFREE 5")
  CALL push("TABLESPACE D_TOOLKIT")
  CALL push("STORAGE(INITIAL 120K NEXT 104K) go")
  CALL push("rdb ALTER TABLE OCD_README_COMPONENT ADD CONSTRAINT XPKOCD_README_COMPONENT")
  CALL push("PRIMARY KEY (END_STATE,COMPONENT_TYPE)")
  CALL push("USING INDEX TABLESPACE I_TOOLKIT")
  CALL push("STORAGE (INITIAL 40K NEXT 40K) go")
  CALL push("RDB CREATE  INDEX XIE1OCD_README_COMPONENT ON OCD_README_COMPONENT")
  CALL push("(PRODUCT_AREA_NAME)")
  CALL push("TABLESPACE I_TOOLKIT")
  CALL push("STORAGE (INITIAL 120K NEXT 104K) go")
  CALL push("RDB CREATE  INDEX XIE2OCD_README_COMPONENT ON OCD_README_COMPONENT")
  CALL push("(PRODUCT_AREA_NUMBER)")
  CALL push("TABLESPACE I_TOOLKIT")
  CALL push("STORAGE (INITIAL 120K NEXT 104K) go")
 ELSE
  SELECT INTO "nl:"
   FROM user_constraints u
   WHERE u.table_name="OCD_README_COMPONENT"
    AND u.constraint_name="XPKOCD_README_COMPONENT"
   WITH nocounter
  ;end select
  IF (curqual=0)
   CALL push("rdb ALTER TABLE OCD_README_COMPONENT ADD CONSTRAINT XPKOCD_README_COMPONENT")
   ...
   CALL push("STORAGE (INITIAL 120K NEXT 104K) go")
  ELSE
   IF (pk_columns("OCD_README_COMPONENT") != 2)
    CALL push("rdb alter table OCD_README_COMPONENT drop primary key cascade go")
    CALL push("rdb ALTER TABLE OCD_README_COMPONENT ADD CONSTRAINT XPKOCD_README_COMPONENT")
    CALL push("PRIMARY KEY (END_STATE,COMPONENT_TYPE)")
    CALL push("USING INDEX TABLESPACE I_TOOLKIT")
    CALL push("STORAGE (INITIAL 40K NEXT 40K) go")
   ELSE
    IF (((pk_col_pos("OCD_README_COMPONENT","END_STATE",1)=0) OR (pk_col_pos("OCD_README_COMPONENT",
     "COMPONENT_TYPE",2)=0)) )
     CALL push("rdb alter table OCD_README_COMPONENT drop primary key cascade go")
     CALL push("rdb ALTER TABLE OCD_README_COMPONENT ADD CONSTRAINT XPKOCD_README_COMPONENT")
     CALL push("PRIMARY KEY (END_STATE,COMPONENT_TYPE)")
   ...
   ENDIF
  ENDIF
  IF (dsa_get_index_col_cnt("OCD_README_COMPONENT","XIE1OCD_README_COMPONENT") > 0)
   CALL push("rdb drop index XIE1OCD_README_COMPONENT go")
   CALL push("RDB CREATE  INDEX XIE1OCD_README_COMPONENT ON OCD_README_COMPONENT")
   CALL push("(PRODUCT_AREA_NAME)")
   CALL push("TABLESPACE I_TOOLKIT")
   CALL push("STORAGE (INITIAL 120K NEXT 104K) go")
  ENDIF
  IF (dsa_get_index_col_cnt("OCD_README_COMPONENT","XIE2OCD_README_COMPONENT") > 0)
   CALL push("rdb drop index XIE2OCD_README_COMPONENT go")
   CALL push("RDB CREATE  INDEX XIE2OCD_README_COMPONENT ON OCD_README_COMPONENT")
   CALL push("(PRODUCT_AREA_NUMBER)")
   CALL push("TABLESPACE I_TOOLKIT")
   CALL push("STORAGE (INITIAL 120K NEXT 104K) go")
  ENDIF
 ENDIF
 IF (table_missing("DM_ENV_RELTN"))
  CALL push("rdb CREATE TABLE DM_ENV_RELTN")
  CALL push("(PARENT_ENV_ID NUMBER DEFAULT 0 NOT NULL,")
  CALL push("CHILD_ENV_ID NUMBER DEFAULT 0 NOT NULL,")
  CALL push("RELATIONSHIP_TYPE VARCHAR2(30) DEFAULT ' ' NOT NULL,")
  CALL push("POST_LINK_NAME VARCHAR2(30),")
  CALL push("PRE_LINK_NAME VARCHAR2(30))")
  CALL push("TABLESPACE D_TOOLKIT")
  CALL push("STORAGE(INITIAL 16K NEXT 16K) go")
  CALL push("RDB CREATE  UNIQUE INDEX XPKDM_ENV_RELTN ON DM_ENV_RELTN")
  CALL push("(PARENT_ENV_ID,CHILD_ENV_ID,RELATIONSHIP_TYPE)")
  CALL push("TABLESPACE I_TOOLKIT")
  CALL push("STORAGE (INITIAL 16K NEXT 16K) go")
  CALL push("rdb ALTER TABLE DM_ENV_RELTN ADD CONSTRAINT XPKDM_ENV_RELTN")
  CALL push("PRIMARY KEY (PARENT_ENV_ID,CHILD_ENV_ID,RELATIONSHIP_TYPE) go")
  CALL push("RDB CREATE  INDEX XIE1DM_ENV_RELTN ON DM_ENV_RELTN")
  CALL push("(CHILD_ENV_ID)")
  CALL push("TABLESPACE I_TOOLKIT")
  CALL push("STORAGE (INITIAL 16K NEXT 16K) go")
 ELSE
  SELECT INTO "nl:"
   FROM user_constraints u
   WHERE u.table_name="DM_ENV_RELTN"
    AND u.constraint_name="XPKDM_ENV_RELTN"
   WITH nocounter
  ;end select
  IF (curqual=0)
   CALL push("rdb drop index xpkdm_env_reltn go")
   CALL push("RDB CREATE  UNIQUE INDEX XPKDM_ENV_RELTN ON DM_ENV_RELTN")
   CALL push("(PARENT_ENV_ID,CHILD_ENV_ID,RELATIONSHIP_TYPE)")
   CALL push("TABLESPACE I_TOOLKIT")
   CALL push("STORAGE (INITIAL 16K NEXT 16K) go")
   ...
   CALL push("PRIMARY KEY (PARENT_ENV_ID,CHILD_ENV_ID,RELATIONSHIP_TYPE) go")
  ELSE
   IF (pk_columns("DM_ENV_RELTN") != 3)
    CALL push("rdb alter table DM_ENV_RELTN drop primary key cascade go")
    CALL push("rdb drop index xpkdm_env_reltn go")
    CALL push("RDB CREATE  UNIQUE INDEX XPKDM_ENV_RELTN ON DM_ENV_RELTN")
    CALL push("(PARENT_ENV_ID,CHILD_ENV_ID,RELATIONSHIP_TYPE)")
    CALL push("TABLESPACE I_TOOLKIT")
    ...
    CALL push("PRIMARY KEY (PARENT_ENV_ID,CHILD_ENV_ID,RELATIONSHIP_TYPE) go")
   ELSE
    IF (((pk_col_pos("DM_ENV_RELTN","PARENT_ENV_ID",1)=0) OR (((pk_col_pos("DM_ENV_RELTN",
     "CHILD_ENV_ID",2)=0) OR (pk_col_pos("DM_ENV_RELTN","RELATIONSHIP_TYPE",3)=0)) )) )
     CALL push("rdb alter table DM_ENV_RELTN drop primary key cascade go")
     CALL push("rdb drop index xpkdm_env_reltn go")
     CALL push("RDB CREATE  UNIQUE INDEX XPKDM_ENV_RELTN ON DM_ENV_RELTN")
     CALL push("(PARENT_ENV_ID,CHILD_ENV_ID,RELATIONSHIP_TYPE)")
  ...
  SELECT INTO "nl:"
   FROM user_indexes u
   WHERE u.table_name="DM_ENV_RELTN"
    AND u.index_name="XIE1DM_ENV_RELTN"
   WITH nocounter
  ;end select
  IF (curqual=0)
   CALL push("RDB CREATE  INDEX XIE1DM_ENV_RELTN ON DM_ENV_RELTN")
   ...
   CALL push("STORAGE (INITIAL 16K NEXT 16K) go")
  ELSE
   IF (dsa_get_index_col_cnt("DM_ENV_RELTN","XIE1DM_ENV_RELTN") != 1)
    CALL push("rdb drop index XIE1DM_ENV_RELTN go")
    CALL push("RDB CREATE  INDEX XIE1DM_ENV_RELTN ON DM_ENV_RELTN")
    CALL push("(CHILD_ENV_ID)")
    CALL push("TABLESPACE I_TOOLKIT")
    CALL push("STORAGE (INITIAL 16K NEXT 16K) go")
  ...
  ENDIF
 ENDIF
 IF (column_missing("DM_CODE_SET","MERGE_UI_QUERY"))
  CALL push("rdb alter table DM_CODE_SET add (merge_ui_query varchar2(255)) go")
 ENDIF
 IF (table_missing("DM_REQUEST_PROCESSING")=0)
  SELECT INTO "nl:"
   FROM user_constraints u
   WHERE u.table_name="DM_REQUEST_PROCESSING"
    AND u.constraint_name="XAK1DM_REQUEST_PROCESSING"
   WITH nocounter
  ;end select
  IF (curqual)
   CALL push("rdb alter table dm_request_processing drop constraint xak1dm_request_processing go")
  ENDIF
  IF (dsa_get_index_col_cnt("DM_REQUEST_PROCESSING","XAK1DM_REQUEST_PROCESSING") > 0)
   CALL push("rdb drop index XAK1DM_REQUEST_PROCESSING go")
  ENDIF
  CALL push("rdb CREATE UNIQUE INDEX XAK1DM_REQUEST_PROCESSING ON DM_REQUEST_PROCESSING")
  CALL push("(REQUEST_NUMBER,SEQUENCE,SCHEMA_DATE)")
  CALL push("TABLESPACE I_TOOLKIT")
  CALL push("STORAGE (INITIAL 120K NEXT 104K) go")
 ENDIF
 IF (column_missing("DM_INDEXES_DOC","FREELIST_CNT"))
  CALL push("rdb alter table DM_INDEXES_DOC add (FREELIST_CNT NUMBER) go")
  CALL push("rdb alter table DM_INDEXES_DOC modify (FREELIST_CNT DEFAULT 1) go")
 ENDIF
 IF (column_missing("DM_INDEXES_DOC","DROP_IND"))
  CALL push("rdb alter table DM_INDEXES_DOC add (DROP_IND NUMBER) go")
  CALL push("rdb alter table DM_INDEXES_DOC modify (DROP_IND DEFAULT 0) go")
 ENDIF
 SELECT INTO "nl:"
  FROM user_objects
  WHERE object_name="DM_INDDOC"
  WITH nocounter
 ;end select
 IF (curqual)
  CALL push("rdb alter trigger DM_INDDOC compile go")
 ENDIF
 SELECT INTO "nl:"
  FROM user_tab_columns u
  WHERE u.table_name="DM_SIZE_DB_VERSION"
   AND u.column_name="CORE_SIZE"
   AND u.data_type="NUMBER"
  WITH nocounter
 ;end select
 IF (curqual=1)
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "rdb alter table dm_size_db_version"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "modify (CORE_SIZE FLOAT)"
  SET parser_count = (parser_count+ 1)
  SET parser_buffer[parser_count] = "go"
 ENDIF
 SELECT INTO "nl:"
  FROM user_tab_columns u
  WHERE u.table_name="DM_REF_DOMAIN"
   AND u.column_name="DISPLAY_COLUMN"
   AND u.data_type="VARCHAR2"
   AND u.data_length < 255
  WITH nocounter
 ;end select
 IF (curqual=1)
  CALL push("rdb alter table dm_ref_domain modify ( DISPLAY_COLUMN VARCHAR2(255) ) go")
 ENDIF
 IF (table_missing("DM_README"))
  CALL push("rdb create table DM_README")
  CALL push("(README_ID number not null,")
  CALL push("FEATURE number not null,")
  CALL push("README_TYPE varchar2(12) null,")
  CALL push("README_CATEGORY varchar2(12) null,")
  CALL push("INSTANCE number default 0 null,")
  CALL push("DESCRIPTION varchar2(255) null,")
  CALL push("PARENT_README_ID number default 0 null,")
  CALL push("PRODUCT varchar2(40) null,")
  CALL push("RUN_ONCE_IND smallint default 0 null,")
  CALL push("EXECUTION varchar2(12) null,")
  CALL push("SCRIPT varchar2(100) null,")
  CALL push("CHECK_SCRIPT varchar2(100) null,")
  CALL push("DATA_FILE varchar2(100) null,")
  CALL push("BLOCKS number default 0 null,")
  CALL push("DRIVER_TABLE varchar2(40) null,")
  CALL push("EXECUTION_TIME float default 0 null,")
  CALL push("ACTIVE_IND smallint default 1 not null,")
  CALL push("UPDT_DT_TM date default sysdate not null,")
  CALL push("constraint XPKDM_README primary key (README_ID, INSTANCE)")
  CALL push("using index tablespace I_TOOLKIT storage (initial 100K next 50K))")
  CALL push("tablespace D_TOOLKIT storage (initial 100K next 50K) go")
 ELSE
  IF (pk_col_pos("DM_README","FEATURE",2))
   CALL push("rdb alter table DM_README drop primary key cascade go")
   CALL push("rdb alter table DM_README add constraint XPKDM_README")
   CALL push("primary key (README_ID, INSTANCE) using index tablespace I_TOOLKIT go")
  ENDIF
 ENDIF
 IF (table_missing("DM_OCD_README"))
  CALL push("rdb create table DM_OCD_README")
  CALL push("(OCD number not null,")
  CALL push("README_ID number not null,")
  CALL push("INSTANCE number default 0 not null,")
  CALL push("constraint XPKDM_OCD_README primary key (OCD, README_ID, INSTANCE)")
  CALL push("using index tablespace I_TOOLKIT storage (initial 50K next 10K))")
  CALL push("tablespace D_TOOLKIT storage (initial 50K next 10K) go")
  CALL push("rdb create unique index XIE1DM_OCD_README on DM_OCD_README")
  CALL push("(README_ID, INSTANCE, OCD)")
  CALL push("tablespace I_TOOLKIT storage (initial 50K next 10K) go")
 ELSE
  IF (column_missing("DM_OCD_README","INSTANCE"))
   CALL push("rdb alter table DM_OCD_README drop primary key go")
   CALL push("rdb drop index XIE1DM_OCD_README go")
   CALL push("rdb alter table DM_OCD_README add (INSTANCE number default 0 not null) go")
   CALL push("rdb create unique index XPKDM_OCD_README on DM_OCD_README")
   CALL push("(OCD, README_ID, INSTANCE)")
   CALL push("tablespace I_TOOLKIT storage (initial 50K next 10K) go")
   CALL push(
    "rdb alter table DM_OCD_README add constraint XPKDM_OCD_README primary key(OCD, README_ID, INSTANCE) go"
    )
   CALL push("rdb create unique index XIE1DM_OCD_README on DM_OCD_README")
   CALL push("(README_ID, INSTANCE, OCD)")
   CALL push("tablespace I_TOOLKIT storage (initial 50K next 10K) go")
  ENDIF
 ENDIF
 IF (table_missing("DM_OCD_LOG"))
  CALL push("rdb create table DM_OCD_LOG")
  CALL push("(ENVIRONMENT_ID number not null,")
  CALL push("PROJECT_TYPE varchar2(12) not null,")
  CALL push("PROJECT_NAME varchar2(100) not null,")
  CALL push("PROJECT_INSTANCE number default 0 not null,")
  CALL push("OCD number not null,")
  CALL push("BATCH_DT_TM date not null,")
  CALL push("STATUS varchar2(12) null,")
  CALL push("START_DT_TM date null,")
  CALL push("END_DT_TM date null,")
  CALL push("DRIVER_COUNT number null,")
  CALL push("ESTIMATED_TIME float null,  ")
  CALL push("MESSAGE varchar2(255) null,")
  CALL push("ACTIVE_IND smallint not null,")
  CALL push("UPDT_DT_TM date default sysdate not null,")
  CALL push(concat("constraint XPKDM_OCD_LOG primary key (ENVIRONMENT_ID, PROJECT_TYPE, ",
    "PROJECT_NAME, PROJECT_INSTANCE, OCD, BATCH_DT_TM)"))
  CALL push("using index tablespace I_TOOLKIT storage (initial 500K next 100K))")
  CALL push("tablespace D_TOOLKIT storage (initial 10K next 10K) go")
  CALL push("rdb create index XIE1DM_OCD_LOG on DM_OCD_LOG")
  CALL push("(PROJECT_TYPE, PROJECT_NAME)")
  CALL push("tablespace I_TOOLKIT storage (initial 50K next 10K) go")
  CALL push("rdb create index XIE2DM_OCD_LOG on DM_OCD_LOG")
  CALL push("(OCD)")
  CALL push("tablespace I_TOOLKIT storage (initial 50K next 10K) go")
 ENDIF
 IF (table_missing("DM_OCD_PRODUCT_AREA"))
  CALL push("rdb create table DM_OCD_PRODUCT_AREA")
  CALL push("(OCD number not null,")
  CALL push("PRODUCT_AREA_NUMBER number not null,")
  CALL push("PRODUCT_AREA_NAME varchar2(100) null,")
  CALL push("constraint XPKDM_OCD_PRODUCT_AREA primary key (OCD)")
  CALL push("using index tablespace I_TOOLKIT storage (initial 100K next 50K))")
  CALL push("tablespace D_TOOLKIT storage (initial 100K next 50K) go")
  CALL push("rdb create index XIE1DM_OCD_PRODUCT_AREA on DM_OCD_PRODUCT_AREA")
  CALL push("(PRODUCT_AREA_NUMBER)")
  CALL push("tablespace I_TOOLKIT storage (initial 50K next 10K) go")
 ENDIF
 IF (column_missing("DM_OCD_LOG","PROJECT_INSTANCE"))
  CALL push("rdb alter table DM_OCD_LOG add (PROJECT_INSTANCE number) go")
  CALL push("rdb alter table DM_OCD_LOG modify (PROJECT_INSTANCE default 0) go")
 ENDIF
 IF (table_missing("DM_ADM_PURGE_TEMPLATE"))
  CALL push("RDB CREATE TABLE dm_adm_purge_template")
  CALL push("(template_nbr NUMBER,")
  CALL push("feature_nbr  NUMBER,")
  CALL push("schema_dt_tm DATE,")
  CALL push("name         VARCHAR2(255),")
  CALL push("program_str  VARCHAR2(255),")
  CALL push("OCD          Number,")
  CALL push("active_ind   Number,")
  CALL push("updt_id      NUMBER NULL,")
  CALL push("updt_dt_tm   DATE   NULL,")
  CALL push("updt_task    NUMBER NULL,")
  CALL push("updt_cnt     NUMBER NULL,")
  CALL push("updt_applctx NUMBER NULL,")
  CALL push("CONSTRAINT xpkdm_adm_purge_template PRIMARY KEY (template_nbr, feature_nbr)")
  CALL push("USING INDEX pctfree 10 storage ")
  CALL push("       (initial 20K next 20K pctincrease 0) ")
  CALL push("     TABLESPACE i_toolkit) ")
  CALL push("TABLESPACE d_toolkit pctfree 5 pctused 80 storage ")
  CALL push("  (initial 50K next 50K pctincrease 0) ")
  CALL push("go")
 ENDIF
 IF (column_missing("DM_ADM_PURGE_TEMPLATE","OCD"))
  CALL push("rdb alter table DM_ADM_PURGE_TEMPLATE add (OCD NUMBER) go")
 ENDIF
 IF (table_missing("DM_ADM_PURGE_TOKEN"))
  CALL push("RDB CREATE TABLE dm_adm_purge_token")
  CALL push("(template_nbr NUMBER,")
  CALL push("feature_nbr  NUMBER,")
  CALL push("token_str    VARCHAR2(255),")
  CALL push("prompt_str   VARCHAR2(255),")
  CALL push("data_type_flag number,")
  CALL push("schema_dt_tm DATE,")
  CALL push("OCD          Number,")
  CALL push("updt_id      NUMBER NULL,")
  CALL push("updt_dt_tm   DATE   NULL,")
  ...
  CALL push("updt_cnt     NUMBER NULL,")
  CALL push("updt_applctx NUMBER NULL,")
  CALL push("CONSTRAINT xpkdm_adm_purge_token PRIMARY KEY (template_nbr, feature_nbr, token_str)")
  CALL push("     USING INDEX pctfree 10 storage ")
  CALL push("       (initial 20K next 20K pctincrease 0) ")
  CALL push("     TABLESPACE i_toolkit) ")
  CALL push("TABLESPACE d_toolkit pctfree 5 pctused 80 storage ")
  ...
  CALL push("go")
 ENDIF
 IF (column_missing("DM_ADM_PURGE_TOKEN","OCD"))
  CALL push("rdb alter table DM_ADM_PURGE_TOKEN add (OCD NUMBER) go")
 ENDIF
 IF (table_missing("DM_ADM_PURGE_TABLE"))
  CALL push("RDB CREATE TABLE dm_adm_purge_table")
  CALL push("(template_nbr     NUMBER,")
  CALL push("feature_nbr      NUMBER,")
  CALL push("parent_table     VARCHAR2(30),")
  CALL push("child_table      VARCHAR2(30),")
  CALL push("child_where      VARCHAR2(255),")
  CALL push("purge_type_flag  NUMBER,  /* (1=delete; 2=update to 0) */")
  CALL push("parent_col1      VARCHAR2(30),")
  CALL push("child_col1       VARCHAR2(30),")
  CALL push("parent_col2      VARCHAR2(30),")
  CALL push("child_col2       VARCHAR2(30),")
  CALL push("parent_col3      VARCHAR2(30),")
  CALL push("child_col3       VARCHAR2(30),")
  CALL push("parent_col4      VARCHAR2(30),")
  CALL push("child_col4       VARCHAR2(30),")
  CALL push("parent_col5      VARCHAR2(30),")
  CALL push("child_col5       VARCHAR2(30),")
  CALL push("schema_dt_tm     DATE,")
  CALL push("OCD              Number,")
  CALL push("updt_id          NUMBER NULL,")
  CALL push("updt_dt_tm       DATE   NULL,")
  ...
  CALL push("  (initial 50K next 50K pctincrease 0) ")
  CALL push("go")
  CALL push("rdb create index XIE1dm_adm_purge_table on dm_adm_purge_table ")
  CALL push("(template_nbr, feature_nbr, parent_table)")
  CALL push("tablespace i_toolkit  ")
  CALL push("STORAGE (INITIAL 20K NEXT 20K PCTINCREASE 10) go")
 ENDIF
 IF (column_missing("DM_ADM_PURGE_TABLE","OCD"))
  CALL push("rdb alter table DM_ADM_PURGE_TABLE add (OCD NUMBER) go")
 ENDIF
 IF (table_missing("DM_AFE_SHIP"))
  CALL push("RDB CREATE TABLE dm_afe_ship")
  CALL push("(alpha_feature_nbr   NUMBER       NOT NULL,")
  CALL push("environment_name     VARCHAR2(20) NOT NULL,")
  CALL push("start_dt_tm          DATE,")
  CALL push("end_dt_tm            DATE,")
  CALL push("status               VARCHAR2(100),")
  CALL push("inst_mode            VARCHAR2(15),")
  CALL push("calling_script       VARCHAR2(30),")
  CALL push("curr_migration_ind   VARCHAR2(22),")
  CALL push(
   "CONSTRAINT xpkdm_alpha_features_env_ship PRIMARY KEY (alpha_feature_nbr, environment_name)")
  CALL push("     USING INDEX pctfree 10 storage ")
  CALL push("       (initial 50K next 50K pctincrease 0) ")
  CALL push("     TABLESPACE i_toolkit) ")
  CALL push("TABLESPACE d_toolkit pctfree 10 pctused 40 storage ")
  CALL push("  (initial 50K next 50K pctincrease 0) ")
  CALL push("go")
 ENDIF
 IF (table_missing("DM_README_HIST_SHIP"))
  CALL push("RDB CREATE TABLE dm_readme_hist_ship")
  CALL push("(process_id          NUMBER               NOT NULL,")
  CALL push("environment_name     VARCHAR2(20)         NOT NULL,")
  CALL push("success_ind          NUMBER               NOT NULL,")
  CALL push("create_dt_tm         DATE                 NOT NULL,")
  CALL push("updt_dt_tm           DATE DEFAULT SYSDATE NOT NULL,")
  CALL push("updt_cnt             NUMBER               NOT NULL,")
  CALL push("CONSTRAINT xpkdm_readme_hist_ship PRIMARY KEY (process_id, environment_name)")
  CALL push("     USING INDEX pctfree 10 storage ")
  CALL push("       (initial 50K next 50K pctincrease 0) ")
  CALL push("     TABLESPACE i_toolkit) ")
  CALL push("TABLESPACE d_toolkit pctfree 10 pctused 40 storage ")
  ...
  CALL push("go")
 ENDIF
 SET spc_index = fillstring(30," ")
 SET spc_index = dsa_get_index_name("SPACE_OBJECTS","SEGMENT_NAME,SEGMENT_TYPE,OWNER")
 IF (spc_index > " ")
  SELECT INTO "nl:"
   FROM user_indexes u
   WHERE u.index_name=spc_index
    AND (u.next_extent < (104 * 1024.0))
   DETAIL
    CALL push(concat("rdb alter index ",spc_index)),
    CALL push("storage (NEXT 104K) go")
   WITH nocounter
  ;end select
 ELSE
  IF (dsa_get_index_col_cnt("SPACE_OBJECTS","XIE1SPACE_OBJECTS") > 0)
   CALL push("rdb drop index XIE1SPACE_OBJECTS go")
  ENDIF
  CALL push("rdb create index XIE1SPACE_OBJECTS on SPACE_OBJECTS ")
  CALL push("(SEGMENT_NAME,SEGMENT_TYPE,OWNER) tablespace I_TOOLKIT ")
  CALL push("storage (INITIAL 104K NEXT 104K) ")
  CALL push("go")
 ENDIF
 IF (dsa_get_index_col_cnt("SPACE_OBJECTS","XIF83SPACE_OBJECTS") > 0
  AND spc_index != "XIF83SPACE_OBJECTS")
  CALL push("rdb drop index XIF83SPACE_OBJECTS go")
 ENDIF
 FOR (count = 1 TO parser_count)
   IF (findstring("rdb",parser_buffer[count])=1)
    IF (findstring("alter t able",parser_buffer[count]) > 0)
 ...
 ENDFOR
 IF ((errlog->count > 0))
  SELECT INTO "DM_OCD_SETUP_ADMIN.LOG"
  ...
  ;end select
  CALL echo("*************************************************************")
  CALL echo("OCD ADMIN SETUP DID NOT COMPLETE SUCCESSFULLY. EXIT OUT OF CCL")
  CALL echo("DM_OCD_SETUP_ADMIN.LOG FILE IN CCLUSERDIR HAS THE ERROR MESSAGES")
  CALL echo("*************************************************************")
  CALL parser("free define oraclesystem go",1)
  GO TO end_program
 ELSE
  ...
  ;end select
  CALL echo("*************************************************************")
  CALL echo("OCD ADMIN SETUP SUCCESSFUL....PLEASE EXIT OUT OF CCL")
  CALL echo("DO NOT INSTALL OCDs WITHOUT EXITING OUT OF CCL")
  CALL echo("*************************************************************")
  CALL parser("free define oraclesystem go",1)
 ENDIF
#end_program
