CREATE PROGRAM aps_get_ft_term_candidates:dba
 ...
 RECORD reply(
   1 qual[20]
     ...
     2 review_case_id = f8
     2 initiating_case = c25
     2 review_case = c25
     2 review_case_unformatted = c25
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
...
#script
 ...
 SELECT INTO "nl:"
  pc.accession_nbr, pc_accession_nbr = decode(pc.seq,uar_fmt_accession(pc.accession_nbr,size(pc
     .accession_nbr,1)),""), pc2_accession_nbr = decode(pc.seq,uar_fmt_accession(pc2.accession_nbr,
    size(pc2.accession_nbr,1)),"")
  ...
  PLAN (ftcl)
   JOIN (pc
   WHERE pc.case_id=ftcl.review_case_id)
   JOIN (afe
   WHERE afe.followup_event_id=ftcl.followup_event_id)
   JOIN (pc2
   WHERE pc2.case_id=afe.case_id)
   JOIN (ap
   WHERE ap.prefix_id=pc2.prefix_id)
  HEAD REPORT
   ncnt = 0, service_resource_cd = 0.0
  DETAIL
   ...
   IF (isresourceviewable(service_resource_cd)=true)
    ...
    IF (mod(ncnt,20)=1
     AND ncnt != 1)
     stat = alter(reply->qual,(ncnt+ 19))
    ENDIF
    reply->qual[ncnt].followup_event_id = ftcl.followup_event_id, reply->qual[ncnt].review_case_id =
    ftcl.review_case_id, reply->qual[ncnt].initiating_case = pc2_accession_nbr,
    CALL echo(pc2_accession_nbr), reply->qual[ncnt].review_case = pc_accession_nbr,
    CALL echo(pc_accession_nbr),
    reply->qual[ncnt].review_case_unformatted = pc.accession_nbr
   ENDIF
  FOOT REPORT
 ...
 IF (curqual=0)
  ...
  SET reply->status_data.subeventstatus[1].targetobjectvalue = "FT_TERM_CANDIDATE_LIST"
