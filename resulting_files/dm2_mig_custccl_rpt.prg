CREATE PROGRAM dm2_mig_custccl_rpt
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmccr_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_rpt")
 DECLARE dmccr_master_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccr_master_node = vc WITH protect, noconstant("")
 DECLARE dmccr_master_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccr_master_env = vc WITH protect, noconstant("")
 DECLARE dmccr_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccr_cont = i2 WITH protect, noconstant(1)
 ...
 DECLARE dmccr_cerinstall = vc WITH protect, noconstant(trim(logical("cer_install")))
 DECLARE dmccr_file = vc WITH protect, noconstant("")
 FREE RECORD dmccr_cust_scripts
 RECORD dmccr_cust_scripts(
   1 cnt = i4
   1 qual[*]
     2 script_name = vc
     2 object_type = vc
     2 app_major_version = i4
     2 user_name = vc
     2 group_nbr = i4
     2 custom_ind = i2
 )
 DECLARE dmccr_get_cust_scripts(dgcs_id=f8) = i2
 DECLARE dmccr_view_cust_rpt(null) = i2
 SET width = 132
 IF (check_logfile(dmccr_logfile_prefix,".log","dm2_mig_custccl_rpt")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_rpt"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Getting env_node info for MASTER"
 SELECT INTO "nl:"
  ...
  DETAIL
   dmccr_master_id = dena.dma_env_node_id, dmccr_master_node = dne.node_name, dmccr_master_env_id =
   dne.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting environment name for MASTER"
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE de.environment_id=dmccr_master_env_id
  DETAIL
   dmccr_master_env = de.environment_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Getting dma_env_node_id for current node/env"
 SELECT INTO "nl:"
 ...
 IF ((dm_err->debug_flag > 0))
  ...
  CALL echo(concat("Current Node/Env ID: ",cnvtstring(dmccr_id)))
  CALL echo(concat("Master Environment: ",dmccr_master_env))
  CALL echo(concat("Master Environment Id: ",cnvtstring(dmccr_master_env_id)))
  CALL echo(concat("Master Node: ",dmccr_master_node))
  CALL echo(concat("Master Node/Env ID: ",cnvtstring(dmccr_master_id)))
 ENDIF
 IF (dmccr_get_cust_scripts(dmccr_master_id)=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  IF ((dm_err->debug_flag=511))
   CALL echorecord(dmccr_cust_scripts)
  ENDIF
  CALL echo(cnvtstring(dmccr_cust_scripts->cnt))
 ENDIF
 IF ((dmccr_cust_scripts->cnt > 0))
  WHILE (dmccr_cont=1)
    SET message = window
    SET width = 132
    CALL clear(1,1)
    CALL box(1,1,22,131)
    CALL text(1,2,"Custom CCL Migration: Custom CCL Script Report")
    CALL text(1,70,"DATE/TIME: ")
    CALL text(1,80,build(month(curdate),"/",day(curdate),"/",year(curdate),
      " ",hour(curtime),":",minute(curtime)))
    CALL text(3,2,concat("Master Environment:",dmccr_master_env," (",trim(cnvtstring(
        dmccr_master_env_id)),")"))
    CALL text(4,2,concat("Master Node:",dmccr_master_node))
    CALL text(6,2,"The list of Custom CCL scripts present in the MASTER are available to view")
    CALL text(7,2,
     "The list will be saved in cer_install as the following CSV file: dm2_mig_custccl_list.csv")
    CALL text(9,2,"(V)iew list of custom scripts, (C)ontinue, (Q)uit: ")
    CALL accept(9,56,"A;cu"," "
     WHERE curaccept IN ("V", "C", "Q"))
    SET message = nowindow
    IF (curaccept="V")
     IF (dmccr_view_cust_rpt(dmccr_id)=0)
      GO TO exit_program
     ENDIF
    ELSEIF (curaccept="Q")
     SET dm_err->emsg = "User elected to quit from Target Custom CCL Script Report"
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ELSE
  ...
  ENDWHILE
 ELSE
  SET dm_err->eproc = "No Custom CCL Scripts were found in MASTER"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Generating dm2_mig_custccl_list.csv into $cer_install"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Removing dm2_mig_custccl_list.csv"
 SET dmccr_file = concat(dmccr_cerinstall,evaluate(cursys2,"AXP","dm2_mig_custccl_list.csv;*",
   "/dm2_mig_custccl_list.csv"))
 IF (dm2_findfile(dmccr_file) > 0)
  IF (dm2_push_dcl(concat(evaluate(cursys2,"AXP","del","rm")," ",dmccr_file))=0)
   RETURN(0)
  ENDIF
 ENDIF
 SET dm_err->eproc = "Creating dm2_mig_custccl_list.csv"
 SELECT INTO "cer_install:dm2_mig_custccl_list.csv"
  FROM dummyt d
  HEAD REPORT
   dmccr_str = "script_name,group,custom_ind,app_major_version", col 0, dmccr_str,
   row + 1
  DETAIL
   FOR (dmccr_cnt = 1 TO dmccr_cust_scripts->cnt)
     dmccr_str = build(trim(dmccr_cust_scripts->qual[dmccr_cnt].script_name),",",cnvtstring(
       dmccr_cust_scripts->qual[dmccr_cnt].group_nbr),",",evaluate(dmccr_cust_scripts->qual[dmccr_cnt
       ].custom_ind,1,"Y","N"),
      ",",dmccr_cust_scripts->qual[dmccr_cnt].app_major_version), col 0, dmccr_str,
     row + 1
   ENDFOR
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE dmccr_get_cust_scripts(dgcs_id)
   SET dm_err->eproc = "Retrieving list of custom scripts for MASTER from dprotect."
   SELECT INTO "nl:"
    ...
    WHERE d.object_type_flag_txt="P"
     ...
     AND d.custom_ind=1
    HEAD REPORT
     dmccr_cust_scripts->cnt = 0, stat = alterlist(dmccr_cust_scripts->qual,0)
    DETAIL
     dmccr_cust_scripts->cnt = (dmccr_cust_scripts->cnt+ 1), stat = alterlist(dmccr_cust_scripts->
      qual,dmccr_cust_scripts->cnt), dmccr_cust_scripts->qual[dmccr_cust_scripts->cnt].script_name =
     d.object_name,
     dmccr_cust_scripts->qual[dmccr_cust_scripts->cnt].object_type = d.object_type_flag_txt,
     dmccr_cust_scripts->qual[dmccr_cust_scripts->cnt].user_name = d.user_name, dmccr_cust_scripts->
     qual[dmccr_cust_scripts->cnt].app_major_version = d.app_major_version_nbr,
     dmccr_cust_scripts->qual[dmccr_cust_scripts->cnt].group_nbr = d.group_nbr, dmccr_cust_scripts->
     qual[dmccr_cust_scripts->cnt].custom_ind = d.custom_ind
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmccr_view_cust_rpt(null)
   DECLARE dvcr_cnt = i4 WITH protect, noconstant(0)
   DECLARE dvcr_cmp_flag = i2 WITH protect, noconstant(0)
   DECLARE dvcr_rpt_file = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Generating Custom CCL Script Report"
   CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
   IF (get_unique_file("dm2_mig_cc_cust",".dat")=0)
    GO TO exit_program
   ELSE
    SET dvcr_rpt_file = dm_err->unique_fname
   ENDIF
   SELECT INTO value(dvcr_rpt_file)
    FROM dummyt d
    HEAD REPORT
     CALL print(fillstring(250,"-")), row + 1, "Custom Script Report",
     row + 1,
     CALL print(fillstring(250,"-")), row + 2,
     "The following is a list of custom scripts present in MASTER", row + 2
    DETAIL
     col 0, "CUSTOM CCL SCRIPT NAME ", col 40,
     "| USER NAME ", col 60, "| GROUP NUMBER ",
     col 75, "| CUSTOM ", col 85,
     "| APP MAJOR VERSION ", row + 1,
     CALL print(fillstring(250,"-")),
     row + 2, dvcr_cnt = 0
     FOR (dvcr_cnt = 1 TO dmccr_cust_scripts->cnt)
       col 0,
       CALL print(trim(dmccr_cust_scripts->qual[dvcr_cnt].script_name)), col 40,
       "| ",
       CALL print(trim(dmccr_cust_scripts->qual[dvcr_cnt].user_name)), col 60,
       "| ",
       CALL print(trim(cnvtstring(dmccr_cust_scripts->qual[dvcr_cnt].group_nbr))), col 75,
       "| ",
       CALL print(trim(evaluate(dmccr_cust_scripts->qual[dvcr_cnt].custom_ind,1,"Y","N"))), col 85,
       "| ",
       CALL print(trim(cnvtstring(dmccr_cust_scripts->qual[dvcr_cnt].app_major_version))), row + 1
     ENDFOR
   ...
   IF (check_error(dm_err->eproc))
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Displaying Custom CCL Script Report"
   IF (dm2_disp_file(dvcr_rpt_file,"Custom CCL Script Report")=0)
    RETURN(0)
   ENDIF
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_rpt"
