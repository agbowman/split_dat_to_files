CREATE PROGRAM bbt_rpt_ex_expspecres:dba
 ...
 DECLARE fillcol1 = c9 WITH protect, constant(fillstring(9,"-"))
 DECLARE fillcol2 = c10 WITH protect, constant(fillstring(10,"-"))
 DECLARE fillcol3 = c25 WITH protect, constant(fillstring(25,"-"))
 DECLARE fillcol4 = c23 WITH protect, constant(fillstring(23,"-"))
 DECLARE fillcol5 = c18 WITH protect, constant(fillstring(18,"-"))
 DECLARE fillcol6 = c24 WITH protect, constant(fillstring(24,"-"))
 DECLARE fillcol7 = c9 WITH protect, constant(fillstring(9,"-"))
 DECLARE fileprefix = c15 WITH protect, constant("BBT_RPT_EXPSPEC")
 DECLARE exception_type_expspecres = f8 WITH protect, constant(uar_get_code_by("MEANING",14072,
   "EXPSPECRES"))
 IF (exception_type_expspecres=0)
  ...
  SET reply->status_data.subeventstatus.targetobjectname = "EXCEPTION_TYPE_EXPSPECRES"
  SET reply->status_data.subeventstatus.targetobjectvalue = "0"
  GO TO exit_script
 ENDIF
 DECLARE exception_disp = vc WITH protect, constant(uar_get_code_display(exception_type_expspecres))
 ...
 DECLARE cur_loc_facility_cd = f8 WITH protect, noconstant(0.0)
 EXECUTE cpm_create_file_name_logical fileprefix, "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  bbe.default_expire_dt_tm, bbe.override_reason_cd, bbe.active_status_dt_tm,
  pr.username, p.name_full_formatted, p.birth_dt_tm,
  p.birth_tz, o.order_mnemonic, accn = cnvtacc(aor.accession),
  order_mnemonic = substring(1,18,o.order_mnemonic), override_reason_display = substring(1,24,
   uar_get_code_display(bbe.override_reason_cd)), username = substring(1,9,pr.username)
  FROM bb_exception bbe,
   ...
   (left JOIN encntr_alias ea ON ea.encntr_id=e.encntr_id
    ...
    AND ea.active_ind=1)
  PLAN (bbe
   WHERE bbe.exception_type_cd=exception_type_expspecres
   ...
   JOIN (ea)
  ORDER BY bbe.active_status_dt_tm, o.order_id, p.name_full_formatted,
   accn
  HEAD PAGE
   ...
   captions->as_of_date, col 118, curdate"@MEDIUMDATE;;d",
   ...
   row + 2, col 1, captions->results,
   col 12, captions->specimen, col 24,
   captions->name, row + 1, col 1,
   captions->verified, col 12, captions->expiration,
   col 24, captions->alias, row + 1,
   col 1, captions->datetime, col 12,
   captions->datetime, col 24, captions->dob,
   col 51, captions->accession, col 76,
   captions->orderable, col 96, captions->override_reason,
   col 122, captions->tech, row + 1,
   col 1, fillcol1, col 12,
   fillcol2, col 24, fillcol3,
   col 51, fillcol4, col 76,
   fillcol5, col 96, fillcol6,
   col 122, fillcol7, row + 1
  HEAD o.order_id
   mrn_alias = ""
   IF (showrecord(e.loc_facility_cd,request->facility_cd)=1
    AND cnvtdatetime(bbe.default_expire_dt_tm) > 0)
    datafoundflag = true
    IF (row > 50)
     BREAK
    ENDIF
    col 1, bbe.active_status_dt_tm"@MEDIUMDATE;;d", col 12,
    bbe.default_expire_dt_tm"@MEDIUMDATE;;d", col 24, p.name_full_formatted,
    col 51, accn, col 76,
    order_mnemonic, col 96, override_reason_display,
    col 122, username, row + 1,
    col 2, bbe.active_status_dt_tm"@TIMENOSECONDS;;M", col 13,
    bbe.default_expire_dt_tm"@TIMENOSECONDS;;M"
    IF (textlen(trim(ea.alias)) > 0)
     mrn_alias = cnvtalias(ea.alias,ea.alias_pool_cd)
    ENDIF
    col 24, mrn_alias, row + 1
    IF (curutc=1)
     birth_dt = format(datetimezone(p.birth_dt_tm,p.birth_tz),"@MEDIUMDATE;4;d")
    ELSE
     birth_dt = format(p.birth_dt_tm,"@MEDIUMDATE;;d")
    ENDIF
    col 24, birth_dt, row + 2
   ENDIF
  DETAIL
   row + 0
  FOOT PAGE
 ...
 SUBROUTINE (showrecord(loc_facility_cd=f8,target_facility_cd=f8) =i2)
   ...
   IF (target_facility_cd > 0)
    IF (cur_loc_facility_cd != loc_facility_cd)
     SET cur_loc_facility_cd = loc_facility_cd
     SET test_facility_cd = bbtgetflexspectestingfacility(loc_facility_cd)
    ENDIF
    IF (test_facility_cd=target_facility_cd)
     SET returnval = 1
    ELSE
     SET returnval = 0
    ENDIF
