CREATE PROGRAM cnt_imp_wv:dba
 CALL parser("dm2_set_context 'FIRE_REFCHG_TRG','NO' go")
 SET curalias wv_struct request->workingview_obj_list[wv_key_cnt].workingview_obj
 SET curalias section_struct request->workingview_obj_list[wv_key_cnt].workingview_obj.section_list[
 sec_index].section
 SET curalias wv_item_struct request->workingview_obj_list[wv_key_cnt].workingview_obj.section_list[
 sec_index].section.item_list[item_index].item
 SET curalias wv_item_da_dta_struct request->workingview_obj_list[wv_key_cnt].workingview_obj.
 section_list[sec_index].section.item_list[item_index].item.da_dta_list[da_dta_index].da_dta
 SET curalias con_exp request->workingview_obj_list[wv_key_cnt].workingview_obj.conditional_obj_list[
 wv_cond_idx].conditional_obj
 SET curalias con_com request->workingview_obj_list[wv_key_cnt].workingview_obj.conditional_obj_list[
 wv_cond_idx].conditional_obj.expression_comp_lst[wv_cond_com].expression_comp
 SET curalias con_dta request->workingview_obj_list[wv_key_cnt].workingview_obj.conditional_obj_list[
 wv_cond_idx].conditional_obj.conditional_dta_list[wv_cond_dta].conditional_dta
 DECLARE compare_dt_tm = dq8
 DECLARE new_wv = i2 WITH noconstant(0)
 DECLARE wv_uid = vc WITH noconstant("")
 DECLARE wv_replace = i2 WITH noconstant(0)
 DECLARE wv_section_uid = vc WITH noconstant("")
 ...
 DECLARE new_cond = i2 WITH noconstant(0)
 DECLARE con_uid = vc WITH noconstant("")
 DECLARE cond_replace = i2 WITH noconstant(0)
 DECLARE con_com_uid = vc WITH noconstant("")
 DECLARE con_dta_uid = vc WITH noconstant("")
 DECLARE wv_key_cnt = i4 WITH noconstant(0)
 DECLARE sec_index = i4 WITH noconstant(0)
 DECLARE item_index = i4 WITH noconstant(0)
 DECLARE da_dta_index = i4 WITH noconstant(0)
 DECLARE wv_ref_id = f8 WITH noconstant(0.0)
 ...
 END ;Subroutine
 CALL logging(0,"","","","",
  "","",concat("Content Library WV import, processing = ",build(size(request->workingview_obj_list,5)
    )," Working View(s)."),"",- (1))
 FOR (wv_key_cnt = 1 TO size(request->workingview_obj_list,5))
   IF ((wv_struct->exp_dt_tm=0))
    SET compare_dt_tm = sysdate
   ELSE
    SET compare_dt_tm = wv_struct->exp_dt_tm
   ENDIF
   SET new_wv = 1
   IF ((wv_struct->working_view_guid > " "))
    SET wv_uid = trim(wv_struct->working_view_guid)
    SET wv_uid = replace(wv_uid,"UNKNOWN!","CERNER!",1)
   ELSE
    SET wv_uid = concat("TEMP!",wv_struct->display_name)
    SELECT INTO "nl:"
     FROM cnt_uid_alias c
     PLAN (c
      WHERE c.cnt_uid_alias=wv_uid
       AND c.cnt_uid_domain="CNT_WV")
     DETAIL
      wv_uid = c.cnt_uid
     WITH check
    ;end select
   ...
   SELECT INTO "NL:"
    FROM cnt_wv_key c
    PLAN (c
     WHERE c.working_view_uid=wv_uid)
    DETAIL
     new_wv = 0
    WITH check
   ;end select
   ...
   IF (new_wv=0)
    ...
    IF (wv_replace=1)
     CALL logging(0,"","","","",
      "","",concat("Working view :",build(wv_key_cnt)," : ",wv_struct->display_name," - updated."),"",
      - (1))
     SET wv_ref_id = cnt_wv_update_old_rows(wv_struct->display_name)
     COMMIT
     CALL cnt_wv_working_view_update(0.0,wv_uid,0.0,wv_struct->location_disp,wv_struct->
      location_cduid,
      compare_dt_tm,wv_struct->current_working_view,wv_struct->display_name,wv_struct->view_state,0.0,
      wv_struct->position_disp,wv_struct->position_cduid,wv_struct->version_num,wv_struct->active_ind,
      wv_struct->display_name_pref,
      wv_ref_id)
     COMMIT
     CALL logging(0,"","","","",
      "","",concat("Working view :",build(wv_key_cnt)," : ",wv_struct->display_name," - Updated."),"",
    ...
    ELSE
     CALL logging(0,"","","","",
      "","",concat("The Working view :",wv_struct->display_name,
       " has older time stamp, hence NOT updating."),"",- (1))
    ENDIF
   ELSE
    SET wv_ref_id = cnt_wv_update_old_rows(wv_struct->display_name)
    COMMIT
    SET wv_uid = cnt_wv_working_view_insert(0.0,wv_uid,0.0,wv_struct->location_disp,wv_struct->
     location_cduid,
     compare_dt_tm,wv_struct->current_working_view,wv_struct->display_name,wv_struct->view_state,0.0,
     wv_struct->position_disp,wv_struct->position_cduid,wv_struct->version_num,wv_struct->active_ind,
     wv_struct->display_name_pref,
     wv_ref_id)
    IF ( NOT (wv_uid IN ("", " ", null)))
     CALL logging(0,"","","","",
      "","",concat("Working view :",build(wv_key_cnt)," : ",wv_struct->display_name," - Inserted."),
      "",- (1))
     COMMIT
    ELSE
     ROLLBACK
    ENDIF
   ENDIF
   IF (((new_wv=1) OR (wv_replace=1)) )
    DELETE  FROM cnt_wv_section_r c
     WHERE c.working_view_uid=wv_uid
     WITH check
    ;end delete
    COMMIT
    FOR (sec_index = 1 TO size(request->workingview_obj_list[wv_key_cnt].workingview_obj.section_list,
     5))
      IF ((section_struct->working_view_section_guid > " "))
       SET wv_section_uid = section_struct->working_view_section_guid
       SET wv_section_uid = replace(wv_section_uid,"UNKNOWN!","CERNER!",1)
      ELSE
       SET wv_section_uid = concat("TEMP!",section_struct->display_name)
       SELECT INTO "nl:"
        FROM cnt_uid_alias c
        PLAN (c
         WHERE c.cnt_uid_alias=wv_section_uid
          AND c.cnt_uid_domain="CNT_WV_SECTION_KEY")
        DETAIL
         wv_section_uid = c.cnt_uid
        WITH check
       ;end select
      ENDIF
      SET new_sect = 1
      SELECT INTO "nl:"
       FROM cnt_wv_section_key c
       PLAN (c
        WHERE c.wv_section_uid=wv_section_uid)
       DETAIL
        new_sect = 0
       WITH check
      ;end select
      ...
      IF (new_sect=0)
       ...
       ;end select
       IF (sect_replace=1)
        CALL cnt_wv_section_update(section_struct->working_view_section_id,wv_section_uid,
         compare_dt_tm,section_struct->event_set_name,section_struct->required_ind,
         section_struct->included_ind,section_struct->falloff_view_minutes,section_struct->
         section_type_flag,section_struct->display_name,section_struct->default_open_pref)
        COMMIT
        CALL logging(0,"","","","",
         "","",concat("Working view section:",section_struct->event_set_name," - Updated."),"",- (1))
       ENDIF
      ELSE
       SET wv_section_uid = cnt_wv_section_insert(0.0,wv_section_uid,compare_dt_tm,section_struct->
        event_set_name,section_struct->required_ind,
        section_struct->included_ind,section_struct->falloff_view_minutes,section_struct->
        section_type_flag,section_struct->display_name,section_struct->default_open_pref)
       IF ( NOT (wv_section_uid IN ("", " ", null)))
        CALL logging(0,"","","","",
         "","",concat("Working view section:",section_struct->event_set_name," - Inserted."),"",- (1)
         )
        COMMIT
       ELSE
        ROLLBACK
       ENDIF
      ENDIF
      SET temp_ret_val = add_wv_section_relation(wv_uid,wv_section_uid)
      IF (temp_ret_val=1)
       COMMIT
      ELSE
       ROLLBACK
      ENDIF
      DELETE  FROM cnt_wv_section_item_r c
       WHERE c.wv_section_uid=wv_section_uid
       WITH check
      ;end delete
      COMMIT
      FOR (item_index = 1 TO size(request->workingview_obj_list[wv_key_cnt].workingview_obj.
       section_list[sec_index].section.item_list,5))
        SET new_item = 1
        IF ((wv_item_struct->working_view_item_guid > " "))
         SET wv_item_guid = trim(wv_item_struct->working_view_item_guid)
         SET wv_item_guid = replace(wv_item_guid,"UNKNOWN!","CERNER!",1)
        ELSE
         SET wv_item_guid = concat("TEMP!",wv_item_struct->primitive_event_set_name)
         SELECT INTO "nl:"
          FROM cnt_uid_alias c
          PLAN (c
           WHERE c.cnt_uid_alias=wv_item_guid
            AND c.cnt_uid_domain="CNT_WV_ITEM")
          DETAIL
           wv_item_guid = c.cnt_uid
          WITH check
         ;end select
        ENDIF
        SELECT INTO "NL:"
         FROM cnt_wv_item_key c
         PLAN (c
          WHERE c.wv_item_uid=wv_item_guid)
         DETAIL
          new_item = 0
         WITH check
        ;end select
        ...
        IF (new_item=0)
         ...
         ;end select
         IF (item_replace=1)
          CALL cnt_wv_item_update(wv_item_struct->working_view_item_id,wv_item_guid,compare_dt_tm,
           wv_item_struct->primitive_event_set_name,wv_item_struct->parent_event_set_name,
           wv_item_struct->included_ind,wv_item_struct->falloff_view_minutes,wv_item_struct->
           task_assay_guid,wv_item_struct->disp_assoc_ind)
          COMMIT
          CALL logging(0,"","","","",
           "","",concat("Working view item : ",wv_item_struct->primitive_event_set_name," - Updated."
            ),"",- (1))
         ENDIF
        ELSE
         SET wv_item_guid = cnt_wv_item_insert(0.0,wv_item_guid,compare_dt_tm,wv_item_struct->
          primitive_event_set_name,wv_item_struct->parent_event_set_name,
          wv_item_struct->included_ind,wv_item_struct->falloff_view_minutes,wv_item_struct->
          task_assay_guid,wv_item_struct->disp_assoc_ind)
         IF ( NOT (wv_item_guid IN ("", " ", null)))
          COMMIT
          CALL logging(0,"","","","",
           "","",concat("Working view item : ",wv_item_struct->primitive_event_set_name,
            " - Inserted."),"",- (1))
         ELSE
          ROLLBACK
         ENDIF
        ENDIF
        SET temp_ret_val = add_wv_section_item_relation(wv_section_uid,wv_item_guid)
        IF (temp_ret_val=1)
         COMMIT
        ELSE
         ROLLBACK
        ENDIF
        IF ((wv_item_struct->disp_assoc_ind=1))
         DELETE  FROM cnt_wv_item_dta c
          WHERE c.wv_item_uid=wv_item_guid
          WITH check
         ;end delete
         COMMIT
         FOR (da_dta_index = 1 TO size(request->workingview_obj_list[wv_key_cnt].workingview_obj.
          section_list[sec_index].section.item_list[item_index].item.da_dta_list,5))
          SET temp_ret_val = add_item_da_dta_relation(wv_item_guid,wv_item_da_dta_struct->da_dta_uid)
          IF (temp_ret_val=1)
           COMMIT
          ELSE
           ROLLBACK
    ...
    ENDFOR
   ENDIF
   FOR (wv_cond_idx = 1 TO size(wv_struct->conditional_obj_list,5))
     SET new_cond = 1
     IF ((con_exp->cnt_cond_expression_key_uid > " "))
      SET con_uid = trim(con_exp->cnt_cond_expression_key_uid)
      SET con_uid = replace(con_uid,"UNKNOWN!","CERNER!",1)
     ELSE
      SET con_uid = concat("TEMP!",con_exp->cond_expression_name)
      SELECT INTO "nl:"
       FROM cnt_uid_alias c
       PLAN (c
        WHERE c.cnt_uid_alias=con_uid
         AND c.cnt_uid_domain="CNT_COND")
       DETAIL
        con_uid = c.cnt_uid
       WITH check
      ;end select
     ENDIF
     SELECT INTO "NL:"
      FROM cnt_cond_expression_key c
      PLAN (c
       WHERE c.cnt_cond_expression_key_uid=con_uid)
      DETAIL
       new_cond = 0
      WITH check
     ;end select
     ...
     IF (new_cond=0)
      ...
      ;end select
      IF (cond_replace=1)
       CALL cnt_wv_con_exp_update(con_exp->cond_expression_id,con_exp->cnt_cond_expression_key_uid,
        con_exp->active_ind,con_exp->cond_expression_name,con_exp->cond_expression_txt,
        con_exp->cond_postfix_txt,con_exp->multiple_ind,0.0,compare_dt_tm)
       COMMIT
       CALL logging(0,"","","","",
        "","",concat("Conditional Expression : ",con_exp->cond_expression_name," - Updated."),"",- (1
        ))
      ENDIF
     ELSE
      SET cond_uid = cnt_wv_con_exp_insert(con_exp->cond_expression_id,con_exp->
       cnt_cond_expression_key_uid,con_exp->active_ind,con_exp->cond_expression_name,con_exp->
       cond_expression_txt,
       con_exp->cond_postfix_txt,con_exp->multiple_ind,con_exp->prev_cond_expression_id,compare_dt_tm
       )
      IF ( NOT (cond_uid IN ("", " ", null)))
       COMMIT
       CALL logging(0,"","","","",
        "","",concat("Conditional Expression : ",con_exp->cond_expression_name," - Inserted."),"",- (
        1))
      ELSE
       ROLLBACK
      ENDIF
     ENDIF
     DELETE  FROM cnt_cond_exprsn_comp_key cec
      WHERE (cec.cond_exprsn_id=con_exp->cond_expression_id)
      WITH check
     ;end delete
     COMMIT
     FOR (wv_cond_com = 1 TO size(con_exp->expression_comp_lst,5))
      SET con_com_uid = cnt_wv_con_com_insert(con_com->cond_expression_id,con_com->
       cnt_cond_exprsn_comp_key_uid,con_com->active_ind,con_com->cond_comp_name,0.0,
       0.0,con_com->operator_cd_uid,0.0,con_com->required_ind,con_com->result_value,
       0.0,con_com->trigger_assay_cd_uid,compare_dt_tm,con_com->ar_uid)
      IF (con_com_uid > " ")
       COMMIT
       CALL logging(0,"","","","",
        "","",concat("Conditional component : ",con_com->cond_comp_name," - Inserted."),"",- (1))
      ELSE
       ROLLBACK
      ENDIF
     ENDFOR
     DELETE  FROM cnt_conditional_dta_key cd
      WHERE (cd.cond_expression_id=con_exp->cond_expression_id)
      WITH check
     ;end delete
     COMMIT
     FOR (wv_cond_dta = 1 TO size(con_exp->conditional_dta_list,5))
      SET con_dta_uid = cnt_wv_con_dta_insert(con_dta->active_ind,con_dta->age_from_nbr,0.0,con_dta->
       age_from_unit_cd_uid,con_dta->age_to_nbr,
       0.0,con_dta->age_to_unit_cd_uid,con_dta->cnt_conditional_dta_key_uid,0.0,con_dta->
       conditional_assay_cd_uid,
       0.0,con_dta->cond_expression_id,0.0,0.0,con_dta->gender_cd_uid,
       0.0,con_dta->location_cd_uid,0.0,con_dta->position_cd_uid,0.0,
       con_dta->required_ind,compare_dt_tm)
      IF (con_dta_uid > " ")
       COMMIT
       CALL logging(0,"","","","",
        "","",concat("Conditional DTA : ",build(con_dta->conditional_assay_cd)," - Inserted."),"",- (
        1))
      ELSE
       ROLLBACK
      ENDIF
   ...
   UPDATE  FROM cnt_wv_key c
    SET c.location_cd =
     (SELECT
      ...
      WHERE cv2.code_value_uid=c.location_cduid)
    WHERE c.location_cd=0.0
     AND c.location_cduid IN (
    (SELECT
     cv.code_value_uid
     FROM cnt_code_value_key cv
     WHERE c.location_cduid=cv.code_value_uid
   ...
   UPDATE  FROM cnt_wv_key c
    SET c.position_cd =
     (SELECT
      cv2.code_value
      FROM cnt_code_value_key cv2
      WHERE cv2.code_value_uid=c.position_cduid)
    WHERE c.position_cd=0.0
     AND c.position_cduid IN (
    (SELECT
     cv.code_value_uid
     FROM cnt_code_value_key cv
     WHERE c.position_cduid=cv.code_value_uid
      AND cv.cnt_code_value_key_id != 0.00))
    WITH nocounter
   ;end update
   UPDATE  FROM cnt_cond_exprsn_comp_key c
    SET c.operator_cd =
     (SELECT
      cv2.code_value
      FROM cnt_code_value_key cv2
      WHERE cv2.code_value_uid=c.operator_cd_uid)
    WHERE c.operator_cd=0.0
     AND c.operator_cd_uid IN (
    (SELECT
     cv.code_value_uid
     FROM cnt_code_value_key cv
     WHERE c.operator_cd_uid=cv.code_value_uid
      AND cv.cnt_code_value_key_id != 0.00))
    WITH nocounter
   ;end update
   UPDATE  FROM cnt_conditional_dta_key c
    SET c.age_from_unit_cd =
     (SELECT
      cv2.code_value
      FROM cnt_code_value_key cv2
      WHERE cv2.code_value_uid=c.age_from_unit_cd_uid)
    WHERE c.age_from_unit_cd=0.0
     AND c.age_from_unit_cd_uid IN (
    (SELECT
     cv.code_value_uid
     FROM cnt_code_value_key cv
     WHERE c.age_from_unit_cd_uid=cv.code_value_uid
      AND cv.cnt_code_value_key_id != 0.00))
    WITH nocounter
   ;end update
   UPDATE  FROM cnt_conditional_dta_key c
    SET c.age_to_unit_cd =
     (SELECT
      cv2.code_value
      FROM cnt_code_value_key cv2
      WHERE cv2.code_value_uid=c.age_to_unit_cd_uid)
    WHERE c.age_to_unit_cd=0.0
     AND c.age_to_unit_cd_uid IN (
    (SELECT
     cv.code_value_uid
     FROM cnt_code_value_key cv
     WHERE c.age_to_unit_cd_uid=cv.code_value_uid
      AND cv.cnt_code_value_key_id != 0.00))
    WITH nocounter
   ;end update
   UPDATE  FROM cnt_conditional_dta_key c
    SET c.gender_cd =
     (SELECT
      cv2.code_value
      FROM cnt_code_value_key cv2
      WHERE cv2.code_value_uid=c.gender_cd_uid)
    WHERE c.gender_cd=0.0
     AND c.gender_cd_uid IN (
    (SELECT
     cv.code_value_uid
     FROM cnt_code_value_key cv
     WHERE c.gender_cd_uid=cv.code_value_uid
      AND cv.cnt_code_value_key_id != 0.00))
    WITH nocounter
   ;end update
   UPDATE  FROM cnt_conditional_dta_key c
    SET c.location_cd =
     (SELECT
      cv2.code_value
      FROM cnt_code_value_key cv2
      WHERE cv2.code_value_uid=c.location_cd_uid)
    WHERE c.location_cd=0.0
     AND c.location_cd_uid IN (
    (SELECT
     cv.code_value_uid
     FROM cnt_code_value_key cv
     WHERE c.location_cd_uid=cv.code_value_uid
      AND cv.cnt_code_value_key_id != 0.00))
    WITH nocounter
   ;end update
   UPDATE  FROM cnt_conditional_dta_key c
    SET c.position_cd =
     (SELECT
      cv2.code_value
      FROM cnt_code_value_key cv2
      WHERE cv2.code_value_uid=c.position_cd_uid)
    WHERE c.position_cd=0.0
     AND c.position_cd_uid IN (
    (SELECT
     cv.code_value_uid
     FROM cnt_code_value_key cv
     WHERE c.position_cd_uid=cv.code_value_uid
      AND cv.cnt_code_value_key_id != 0.00))
    WITH nocounter
   ;end update
   COMMIT
   SET reply->qual[wv_key_cnt].success_ind = 1
 ENDFOR
