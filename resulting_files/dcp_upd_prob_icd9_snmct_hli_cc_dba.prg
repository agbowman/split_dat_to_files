CREATE PROGRAM dcp_upd_prob_icd9_snmct_hli_cc:dba
 ...
 INSERT  FROM temp_prob_icd9_snmct_hli
 ...
 DELETE  FROM temp_prob_icd9_snmct_hli
  WHERE problem_instance_id IN (
  (SELECT
   temp.problem_instance_id
   FROM temp_prob_icd9_snmct_hli temp
 ...
 WHILE (temp_cnt > 0)
   ...
   UPDATE  FROM temp_prob_icd9_snmct_hli temp
 ...
 IF (sbr_upd_id_evaluated(mf_max_id,ms_child_c_info_domain_nm,ms_child_c_max_name)=0)
  GO TO exit_program
 ENDIF
 CALL sbr_truncate_table("TEMP_PROB_ICD9_SNMCT_HLI")
#exit_program
 IF (checkprg("DM2_SET_CONTEXT") > 0)
