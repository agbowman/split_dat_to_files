CREATE PROGRAM dm2_translate_load_driver:dba
 ...
 SET dm2_install_schema->cdba_connect_str = "admin"
 SET dm2_install_schema->target_dbase_name =  $1
 SET dm2_install_schema->v500_p_word = "v500"
 SET dm2_install_schema->v500_connect_str =  $1
 SET tld_header = "Schema install driver output log"
 SET dm_err->logfile = "dm2_trn_load.log"
 CALL init_logfile(value(dm_err->logfile),tld_header)
 IF (cnvtupper(trim(substring(1,3, $2)))="OCD")
  SET tgtsch->alpha_feature_nbr = cnvtint(build(substring(4,6, $2)))
 ELSE
  SET tgtsch->schema_date = cnvtdate(cnvtint(build(substring(1,8, $2))))
 ENDIF
 SET dm_err->eproc = "Beginning Translate Load Driver Process"
 CALL disp_msg(" ",dm_err->logfile,0)
 EXECUTE dm2_translate_load
#exit_program
 SET dm_err->eproc = "Translate Load Driver Process Completed!"
