CREATE PROGRAM dm_rmc_run_stmt:dba
 ...
 SUBROUTINE move_circ_long(i_from_table,i_from_rtable,i_from_pk,i_from_prev_pk,i_from_fk,
  ...
  i_status_flag)
   ...
   ENDFOR
 END ;Subroutine
 DECLARE drrs_prepare_stmt(i_str=vc,i_end_ind=i2,i_rdb_asis_ind=i2,i_start_ind=i2,i_curdate=dq8,
  i_todaymid=dq8,i_midminus1=dq8) = vc
 DECLARE r_tbl_fix(i_table_name=vc,i_source_id=f8) = i2
 DECLARE processing_after_error(pae_repair_ind=i2(ref),pae_table_name=vc,pae_r_table_name=vc,
  pae_source_env_id=f8,pae_status_flag=i4,
  pae_context_name=vc,pae_ctx_null_ind=i2,pae_flag_to_set=i4,pae_stmt_num=i4,pae_error_ind=i2(ref),
  pae_rs_stmt=vc(ref),pae_dta_no_uar=i2) = i4
 DECLARE row_by_row(rbr_table_name=vc,rbr_r_table_name=vc,rbr_source_env_id=f8,rbr_status_flag=i4,
  rbr_context_name=vc,
  rbr_ctx_null_ind=i2,rbr_flag_to_set=i4,rbr_stmt_num=i4,rbr_error_ind=i2(ref),rbr_rs_stmt=vc(ref),
  rbr_dta_no_uar=i2) = i4
 DECLARE drrs_batch_prep(sp_str=vc,sp_stmt_num=i4,sp_source_env_id=f8) = vc
 DECLARE drrs_set_message(dsm_str=vc,dsm_r_tab=vc,dsm_live_tab=vc) = vc
 DECLARE drrs_chk_errors(dce_err_msg=vc,dce_early_xit=vc(ref)) = i4
 DECLARE v_move_long_ind = i2 WITH protect
 DECLARE v_curdate = dq8 WITH protect
 DECLARE v_todaymid = dq8 WITH protect
 DECLARE v_midminus1 = dq8 WITH protect
 DECLARE v_row_count = i4 WITH protect
 DECLARE v_end_ind_cnt = i4 WITH protect
 DECLARE v_rs_stmt_cnt = i4 WITH protect
 DECLARE v_start_ind = i2 WITH protect
 DECLARE v_stmt = vc WITH protect
 DECLARE v_batch_size = i4 WITH protect
 DECLARE v_replace_str = vc WITH protect
 DECLARE v_table_suffix = vc WITH protect
 DECLARE v_stmt_type = vc WITH protect
 DECLARE v_$r_table = vc WITH protect
 DECLARE v_long_pk_str = vc WITH protect
 DECLARE v_source_env_id = f8 WITH protect
 DECLARE v_tname_check = i2 WITH protect
 DECLARE v_context_to_set = vc WITH protect
 DECLARE v_curenv_id = f8 WITH protect
 DECLARE v_repair_ind = i2 WITH protect
 DECLARE v_flag_to_set = i4 WITH protect
 DECLARE v_rows_processed = i4 WITH protect
 DECLARE t_ndx = i4 WITH protect
 DECLARE s_ndx = i4 WITH protect
 DECLARE v_save_error = vc WITH protect
 DECLARE c_ndx = i4 WITH protect
 DECLARE v_stmt_failed_ind = i2 WITH protect
 DECLARE v_dta_no_uar = i2 WITH protect
 DECLARE v_more_rows_ind = i2 WITH protect
 DECLARE v_dcp_add_dta_version = c19 WITH constant("DCP_ADD_DTA_VERSION"), protect
 DECLARE v_ccl_vers = i4 WITH protect, noconstant(0)
 DECLARE drrs_msg = vc WITH protect, noconstant("")
 DECLARE dml_stmt = vc WITH protect, noconstant("")
 DECLARE v_cur_pos = i2 WITH protect, noconstant(0)
 DECLARE v_null_pos = i2 WITH protect, noconstant(0)
 DECLARE v_cutover_by_context_ind = i2 WITH protect, noconstant(0)
 DECLARE v_curqual = i2 WITH protect, noconstant(0)
 DECLARE v_table_incomplete = i2 WITH protect, noconstant(0)
 DECLARE v_start_time = dq8 WITH protect, noconstant(0.0)
 DECLARE v_end_time = dq8 WITH protect, noconstant(0.0)
 DECLARE drrs_hint_str = vc WITH protect, noconstant(" ")
 DECLARE trunc() = dq8
 RECORD rs_stmt(
   1 stmt[*]
     2 str = vc
 ...
 RECORD cut_early_xit(
   1 query_ind = i2
   1 qual[*]
     2 error = vc
 ) WITH protect
 RECORD contexts_to_cutover(
   1 values[*]
     2 str = vc
   1 all_ind = i2
   1 len = i4
 ) WITH protect
 IF (size(trim(request->table_name,1))=0)
  SET v_tname_check = - (1)
 ENDIF
 SET v_source_env_id = request->source_env_id
 SET v_move_long_ind = request->move_long_ind
 SET v_curdate = cnvtdatetime(curdate,curtime3)
 SET v_todaymid = cnvtdatetime(curdate,000000)
 SET v_midminus1 = datetimeadd(cnvtdatetime(curdate,235959),- (1))
 IF ((v_tname_check != - (1)))
  SET v_$r_table = cutover_tab_name(request->table_name,"")
  SET drrs_hint_str = concat("index_rs_asc(",v_$r_table," xie1_$r_",trim(substring((size(v_$r_table,1
      ) - 5),4,v_$r_table)),")")
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info d
  ...
  DETAIL
   v_curenv_id = d.info_number
  WITH nocounter
 ;end select
 IF (check_error("Obtaining Environment ID")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET v_ccl_vers = (((currev * 10000)+ (currevminor * 100))+ currevminor2)
 IF (v_ccl_vers > 80401)
  SET modify = rdblongerror
 ENDIF
 IF ((validate(err_to_continue->total,- (1))=- (1))
  AND (validate(err_to_continue->total,- (2))=- (2)))
  RECORD err_to_continue(
    1 total = i4
    1 qual[*]
      2 err_msg = vc
      2 status_flag = i4
 ...
 IF ((v_tname_check != - (1)))
  SELECT DISTINCT INTO "nl:"
   d.rdds_context_name
   FROM (parser(v_$r_table) d)
   WHERE d.rdds_status_flag < 9000
    AND d.rdds_source_env_id=v_source_env_id
   HEAD REPORT
    cnt = 0
   DETAIL
    cnt = (cnt+ 1)
    IF (mod(cnt,10)=1)
     stat = alterlist(rs_contexts->qual,(cnt+ 9))
    ENDIF
    IF (nullind(d.rdds_context_name)=1)
     rs_contexts->qual[cnt].context_name = "NULL", rs_contexts->qual[cnt].null_ind = 1
    ELSEIF (d.rdds_context_name="")
     rs_contexts->qual[cnt].context_name = ""
    ELSE
     rs_contexts->qual[cnt].context_name = notrim(d.rdds_context_name)
    ENDIF
   FOOT REPORT
    stat = alterlist(rs_contexts->qual,cnt)
   WITH nocounter
  ;end select
  IF (check_error(concat("Obtaining context names from the ",v_$r_table," table"))=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SELECT INTO "nl:"
   ...
   DETAIL
    v_cutover_by_context_ind = 1
   WITH nocounter
  ;end select
  IF (check_error("Checking for the cutover by context configuration row")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (size(rs_contexts->qual,5)=0)
  SET stat = alterlist(rs_contexts->qual,1)
  SET rs_contexts->qual[1].context_name = "NULL"
 ENDIF
 IF (v_cutover_by_context_ind=1
  AND (v_tname_check != - (1)))
  SELECT INTO "nl:"
   FROM dm_info d
   ...
   HEAD REPORT
    cnt = 0
   DETAIL
    cnt = (cnt+ 1), stat = alterlist(contexts_to_cutover->values,cnt), contexts_to_cutover->values[
    cnt].str = d.info_name
    IF (d.info_name="ALL")
     contexts_to_cutover->all_ind = 1
    ENDIF
   FOOT REPORT
    contexts_to_cutover->len = cnt
   WITH nocounter
  ;end select
  IF ((contexts_to_cutover->all_ind=0))
   IF ((contexts_to_cutover->len > 0))
    FOR (c_ndx = 1 TO size(rs_contexts->qual,5))
      IF (dcc_find_val(trim(rs_contexts->qual[c_ndx].context_name),"::",contexts_to_cutover)=1)
       SET v_cur_pos = (v_cur_pos+ 1)
       SET rs_contexts->qual[v_cur_pos].context_name = rs_contexts->qual[c_ndx].context_name
       SET rs_contexts->qual[v_cur_pos].null_ind = rs_contexts->qual[c_ndx].null_ind
      ENDIF
    ENDFOR
    SET stat = alterlist(rs_contexts->qual,v_cur_pos)
   ELSE
    SET dm_err->err_ind = 1
 ...
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="RDDS DTA_VERSION WITHOUT UARS"
   AND d.info_name="INSTALLED"
  WITH nocounter
 ;end select
 IF (check_error("Checking for existance of DTA_VERSION WITHOUT UARS row")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET v_dta_no_uar = curqual
 IF ((request->batch_ind=0))
  FOR (c_ndx = 1 TO size(rs_contexts->qual,5))
    SET v_context_to_set = rs_contexts->qual[c_ndx].context_name
    IF ((v_tname_check != - (1)))
     IF (size(trim(rs_contexts->qual[c_ndx].context_name),1) != 0)
      EXECUTE dm2_set_context "RDDS RMC/LOAD CONTEXT", rs_contexts->qual[c_ndx].context_name
     ELSE
      CALL parser(concat(
        "RDB ASIS(^ BEGIN DM2_CONTEXT_CONTROL('RDDS RMC/LOAD CONTEXT',''); END; ^) GO"),1)
     ENDIF
    ENDIF
    SET v_start_ind = 1
    FOR (s_ndx = 1 TO size(request->stmt,5))
     IF ((((request->stmt[s_ndx].move_long_str_ind=0)) OR (v_move_long_ind=1)) )
      SET v_stmt = drrs_prepare_stmt(request->stmt[s_ndx].str,request->stmt[s_ndx].end_ind,request->
       stmt[s_ndx].rdb_asis_ind,v_start_ind,v_curdate,
       v_todaymid,v_midminus1)
      SET v_stmt = replace(v_stmt,"=v_status_flag","< 9000",0)
      SET v_replace_str = build(v_source_env_id)
      SET v_stmt = replace(v_stmt,"v_source_env_id",v_replace_str,0)
      IF ((rs_contexts->qual[c_ndx].null_ind=1))
       SET v_stmt = replace(v_stmt,"=v_context_to_set"," is null",0)
      ENDIF
      SET v_stmt = replace(v_stmt,"r1.rowid=rbr_rowid","1=1",0)
      SET v_stmt = replace(v_stmt,"rowid=rbr_rowid","1=1",0)
      IF ((request->stmt[s_ndx].end_ind=1))
       CALL parser(v_stmt,1)
      ELSE
       CALL parser(v_stmt,0)
      ENDIF
     ENDIF
     IF ((request->stmt[s_ndx].end_ind=1))
      SET v_start_ind = 1
 ...
 ELSE
  SET v_start_ind = 1
  SET v_batch_size = 1000
  SELECT INTO "NL:"
   di.info_number
   FROM dm_info di
   WHERE di.info_domain="RDDS CONFIGURATION"
    AND di.info_name="CUTOVER BATCH SIZE"
   DETAIL
    v_batch_size = di.info_number
   WITH nocounter
  ;end select
  IF (v_batch_size < 1)
   SET v_batch_size = 1000
  ENDIF
  IF (check_error("Obtaining cutover batch size")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SELECT INTO "nl:"
   FROM dm_refchg_stat drs
   WHERE drs.stat_type="CUTOVER"
    AND drs.table_name=v_$r_table
    AND drs.source_env_id=v_source_env_id
   WITH nocounter
  ;end select
  IF (check_error("Checking for CUTOVER row in dm_refchg_stat")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (curqual=0)
   IF (v_cutover_by_context_ind=1
    AND (contexts_to_cutover->all_ind=0))
    SELECT INTO "NL:"
     cnt = count(*)
     FROM (parser(v_$r_table) r)
     WHERE r.rdds_source_env_id=v_source_env_id
      AND r.rdds_status_flag < 9000
      AND expand(v_cur_pos,1,size(rs_contexts->qual,5),r.rdds_context_name,rs_contexts->qual[
      v_cur_pos].context_name,
      0,rs_contexts->qual[v_cur_pos].null_ind)
     DETAIL
      v_row_count = cnt
     WITH nocounter
    ;end select
    SET v_null_pos = locateval(v_cur_pos,1,size(rs_contexts->qual,5),1,rs_contexts->qual[v_cur_pos].
     null_ind)
    IF (v_null_pos > 0)
     SELECT INTO "NL:"
      cnt = count(*)
      ...
      WHERE r.rdds_source_env_id=v_source_env_id
       AND r.rdds_status_flag < 9000
       AND r.rdds_context_name = null
      DETAIL
       v_row_count = (v_row_count+ cnt)
      WITH nocounter
     ;end select
    ENDIF
   ELSE
    ...
    ;end select
   ENDIF
   IF (check_error(concat("Obtaining count of rows from ",v_$r_table))=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SELECT INTO "NL:"
    FROM (dummyt d  WITH seq = size(request->stmt,5))
    WHERE (request->stmt[d.seq].end_ind=1)
    DETAIL
     v_end_ind_cnt = (v_end_ind_cnt+ 1)
    WITH nocounter
   ;end select
   INSERT  FROM dm_refchg_stat drs
    SET drs.dm_refchg_stat_id = seq(dm_clinical_seq,nextval), drs.rows_to_process = v_row_count, drs
     .stat_type = "CUTOVER",
     drs.table_name = v_$r_table, drs.source_env_id = v_source_env_id, drs.mrg_row_cnt = 0,
     drs.del_row_cnt = 0, drs.upd_row_cnt = 0, drs.stmt_cnt = v_end_ind_cnt
    WITH nocounter
   ;end insert
   IF (check_error(concat("Inserting into  dm_refchg_stat for table ",v_$r_table))=1)
    ROLLBACK
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
  ...
  FOR (s_ndx = 1 TO size(request->stmt,5))
    IF ((((request->stmt[s_ndx].move_long_str_ind=0)) OR (v_move_long_ind=1)) )
     SET v_rs_stmt_cnt = (v_rs_stmt_cnt+ 1)
     IF (v_rs_stmt_cnt=1)
      IF (cnvtupper(trim(request->stmt[s_ndx].str))="UPDATE*")
       SET v_stmt_type = "UPDATE"
      ELSEIF (cnvtupper(trim(request->stmt[s_ndx].str))="INSERT*")
       SET v_stmt_type = "INSERT"
      ELSEIF (cnvtupper(trim(request->stmt[s_ndx].str))="DELETE*")
       SET v_stmt_type = "DELETE"
      ELSEIF (cnvtupper(trim(request->stmt[s_ndx].str))="MERGE*")
       SET v_stmt_type = "MERGE"
      ELSE
       SET v_stmt_type = " "
      ENDIF
     ENDIF
     SET stat = alterlist(rs_stmt->stmt,v_rs_stmt_cnt)
     SET rs_stmt->stmt[v_rs_stmt_cnt].str = drrs_prepare_stmt(request->stmt[s_ndx].str,request->stmt[
      s_ndx].end_ind,request->stmt[s_ndx].rdb_asis_ind,v_start_ind,v_curdate,
      v_todaymid,v_midminus1)
     SET rs_stmt->stmt[v_rs_stmt_cnt].end_ind = request->stmt[s_ndx].end_ind
     IF ((request->stmt[s_ndx].end_ind=1))
      SET v_start_ind = 1
      SET v_stmt_num = (v_stmt_num+ 1)
      IF (((s_ndx=size(request->stmt,5)) OR ((s_ndx=(size(request->stmt,5) - 1))
       AND (request->stmt[size(request->stmt,5)].move_long_str_ind=1)
       AND v_move_long_ind=0)) )
       SET v_flag_to_set = 9999
      ELSE
       SET v_flag_to_set = v_stmt_num
      ENDIF
      FOR (rs_ndx = 1 TO size(rs_stmt->stmt,5))
        SET rs_stmt->stmt[rs_ndx].str = drrs_batch_prep(rs_stmt->stmt[rs_ndx].str,v_stmt_num,
         v_source_env_id)
      ENDFOR
      UPDATE  FROM dm_refchg_stat drs
       SET drs.cur_stmt_nbr = v_stmt_num, drs.cur_stmt_dt_tm = cnvtdatetime(curdate,curtime3)
       WHERE drs.stat_type="CUTOVER"
        AND drs.table_name=v_$r_table
        AND drs.source_env_id=v_source_env_id
       WITH nocounter
      ;end update
      IF (check_error(concat("Updating into  dm_refchg_stat for table ",v_$r_table))=1)
       ROLLBACK
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      IF (v_cutover_by_context_ind=1
       AND (contexts_to_cutover->all_ind=0))
       UPDATE  FROM (parser(v_$r_table) r_tab)
        SET r_tab.rdds_status_flag = (v_stmt_num - 1), r_tab.rdds_dt_tm = cnvtdatetime(curdate,
          curtime3)
        WHERE (r_tab.rdds_status_flag=- ((1 * v_stmt_num)))
         AND expand(v_cur_pos,1,size(rs_contexts->qual,5),r_tab.rdds_context_name,rs_contexts->qual[
         v_cur_pos].context_name,
         0,rs_contexts->qual[v_cur_pos].null_ind)
        WITH nocounter, orahintcbo(value(drrs_hint_str))
       ;end update
       SET v_null_pos = locateval(v_cur_pos,1,size(rs_contexts->qual,5),1,rs_contexts->qual[v_cur_pos
        ].null_ind)
       IF (v_null_pos > 0)
        UPDATE  FROM (parser(v_$r_table) r_tab)
         SET r_tab.rdds_status_flag = (v_stmt_num - 1), r_tab.rdds_dt_tm = cnvtdatetime(curdate,
           curtime3)
         WHERE (r_tab.rdds_status_flag=- ((1 * v_stmt_num)))
          AND r_tab.rdds_context_name = null
         WITH nocounter, orahintcbo(value(drrs_hint_str))
        ;end update
       ENDIF
      ELSE
       ...
       ;end update
      ENDIF
      IF (check_error(concat("Updating into ",v_$r_table))=1)
       ROLLBACK
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_program
      ENDIF
      FOR (c_ndx = 1 TO size(rs_contexts->qual,5))
        ...
        IF ((v_tname_check != - (1)))
         ...
         ENDIF
        ENDIF
        WHILE (v_more_rows_ind=1)
          IF ((rs_contexts->qual[c_ndx].null_ind=1))
           UPDATE  FROM (parser(v_$r_table) r_tab)
            SET r_tab.rdds_status_flag = - ((1 * v_stmt_num)), r_tab.rdds_dt_tm = cnvtdatetime(
              curdate,curtime3)
            WHERE (r_tab.rdds_status_flag=(v_stmt_num - 1))
             AND rdds_context_name = null
            WITH maxqual(r_tab,value(v_batch_size)), orahintcbo(value(drrs_hint_str))
           ;end update
          ELSE
           UPDATE  FROM (parser(v_$r_table) r_tab)
            SET r_tab.rdds_status_flag = - ((1 * v_stmt_num)), r_tab.rdds_dt_tm = cnvtdatetime(
              curdate,curtime3)
            WHERE (r_tab.rdds_status_flag=(v_stmt_num - 1))
             AND (r_tab.rdds_context_name=rs_contexts->qual[c_ndx].context_name)
            WITH maxqual(r_tab,value(v_batch_size)), orahintcbo(value(drrs_hint_str))
           ;end update
          ENDIF
          IF (check_error(concat("Updating into the next batch for ",v_$r_table))=1)
           ROLLBACK
           CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
           GO TO exit_program
          ENDIF
          COMMIT
          IF (curqual > 0)
           IF (curqual < v_batch_size)
            SET v_last_batch_ind = 1
           ELSE
            SET v_last_batch_ind = 0
           ENDIF
           IF (v_$r_table="DISCRETE_TASK_1291$R"
            AND v_stmt_type="UPDATE"
            AND v_dta_no_uar > 0)
            IF ((validate(request->merge_stmt_ind,- (100)) != - (100))
             AND (request->merge_stmt_ind=1))
             SET v_pae_ind = 1
             SET v_pae_error_ind = 0
             SET dm_err->err_ind = 0
             SET v_rows_processed = (v_rows_processed+ processing_after_error(v_repair_ind,request->
              table_name,v_$r_table,v_source_env_id,- ((1 * v_stmt_num)),
              rs_contexts->qual[c_ndx].context_name,rs_contexts->qual[c_ndx].null_ind,v_flag_to_set,
              v_stmt_num,v_pae_error_ind,
              rs_stmt,v_dta_no_uar))
             IF (v_pae_error_ind=1)
              SET v_stmt_failed_ind = 1
              IF (v_last_batch_ind=1)
               SET v_more_rows_ind = 0
              ENDIF
             ENDIF
            ENDIF
           ELSEIF (v_$r_table="PREFDIR_ENTRYD7109$R")
            SET dm_err->err_ind = 0
            SET v_pae_error_ind = 0
            SET v_pae_ind = 1
            SET v_rows_processed = (v_rows_processed+ processing_after_error(v_repair_ind,request->
           ...
           ELSE
            SET v_pae_ind = 0
            FOR (t_ndx = 1 TO size(rs_stmt->stmt,5))
              SET v_stmt = replace(rs_stmt->stmt[t_ndx].str,"r1.rowid=rbr_rowid","1=1",0)
              SET v_stmt = replace(v_stmt,"rowid=rbr_rowid","1=1",0)
              IF ((rs_contexts->qual[c_ndx].null_ind=1))
               SET v_stmt = replace(v_stmt,"=v_context_to_set"," is null",0)
              ENDIF
              IF ((rs_stmt->stmt[t_ndx].end_ind=0))
               CALL parser(v_stmt,1)
              ELSE
               CALL parser(v_stmt,0)
              ENDIF
              IF (check_error(dm_err->eproc)=1)
               IF (drrs_chk_errors(dm_err->emsg,cut_early_xit)=1)
                SET drrs_msg = drrs_set_message(dm_err->emsg,v_$r_table,request->table_name)
                SET dml_stmt = rs_stmt->stmt[1].str
                FOR (stmt_ndx = 2 TO size(rs_stmt->stmt,5))
                  SET dml_stmt = concat(dml_stmt," ",rs_stmt->stmt[stmt_ndx].str)
                ENDFOR
                IF (size(dml_stmt,1) > 4000)
                 SET dml_stmt = substring(1,4000,dml_stmt)
                ENDIF
                INSERT  FROM dm_refchg_warning
                 SET dm_refchg_warning_id = seq(dm_clinical_seq,nextval), source_env_id =
                  v_source_env_id, table_name = v_$r_table,
                  ...
                  dml_txt = dml_stmt, row_select_txt = "", error_msg = dm_err->emsg,
                  message = drrs_msg
                 WITH nocounter
                ;end insert
                IF (check_error(dm_err->eproc)=1)
               ...
               IF ((validate(request->merge_stmt_ind,- (100)) != - (100))
                ...
                SET v_rows_processed = (v_rows_processed+ processing_after_error(v_repair_ind,request
                 ->table_name,v_$r_table,v_source_env_id,- ((1 * v_stmt_num)),
                 rs_contexts->qual[c_ndx].context_name,rs_contexts->qual[c_ndx].null_ind,
                 v_flag_to_set,v_stmt_num,v_pae_error_ind,
                 rs_stmt,v_dta_no_uar))
                SET v_pae_ind = 1
                IF (v_pae_error_ind=1)
                 SET v_stmt_failed_ind = 1
               ...
               ENDIF
              ELSE
               SET v_rows_processed = curqual
              ENDIF
            ENDFOR
           ENDIF
           IF (v_pae_ind=0)
            IF ((rs_contexts->qual[c_ndx].null_ind=1))
             UPDATE  FROM (parser(v_$r_table))
              SET rdds_status_flag = v_flag_to_set, rdds_dt_tm = cnvtdatetime(curdate,curtime3)
              WHERE (rdds_status_flag=- ((1 * v_stmt_num)))
               AND rdds_context_name = null
              WITH nocounter, orahintcbo(value(drrs_hint_str))
             ;end update
            ELSE
             UPDATE  FROM (parser(v_$r_table))
              SET rdds_status_flag = v_flag_to_set, rdds_dt_tm = cnvtdatetime(curdate,curtime3)
              WHERE (rdds_status_flag=- ((1 * v_stmt_num)))
               AND (rdds_context_name=rs_contexts->qual[c_ndx].context_name)
              WITH nocounter, orahintcbo(value(drrs_hint_str))
             ;end update
            ENDIF
            IF (check_error(concat("Updating status flag on ",v_$r_table))=1)
             ROLLBACK
             CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
             GO TO exit_program
            ENDIF
           ENDIF
           IF (v_stmt_type="UPDATE")
            UPDATE  FROM dm_refchg_stat d
             SET d.upd_row_cnt = (d.upd_row_cnt+ v_rows_processed)
             WHERE d.stat_type="CUTOVER"
              AND d.table_name=v_$r_table
              AND d.source_env_id=v_source_env_id
             WITH nocounter
            ;end update
           ELSEIF (v_stmt_type="INSERT")
            UPDATE  FROM dm_refchg_stat d
             SET d.ins_row_cnt = (d.ins_row_cnt+ v_rows_processed)
             WHERE d.stat_type="CUTOVER"
              AND d.table_name=v_$r_table
              AND d.source_env_id=v_source_env_id
             WITH nocounter
            ;end update
           ELSEIF (v_stmt_type="DELETE")
            UPDATE  FROM dm_refchg_stat d
             SET d.del_row_cnt = (d.del_row_cnt+ v_rows_processed)
             WHERE d.stat_type="CUTOVER"
              AND d.table_name=v_$r_table
              AND d.source_env_id=v_source_env_id
             WITH nocounter
            ;end update
           ELSEIF (v_stmt_type="MERGE")
            UPDATE  FROM dm_refchg_stat d
             SET d.mrg_row_cnt = (d.mrg_row_cnt+ v_rows_processed)
             WHERE d.stat_type="CUTOVER"
              AND d.table_name=v_$r_table
              AND d.source_env_id=v_source_env_id
             WITH nocounter
            ;end update
           ENDIF
           IF (check_error(concat("Updating processed row count into  dm_refchg_stat for table ",
             v_$r_table))=1)
            ROLLBACK
            CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
            GO TO exit_program
        ...
        ENDWHILE
      ENDFOR
      SET stat = alterlist(rs_stmt->stmt,0)
      SET v_rs_stmt_cnt = 0
      IF (v_stmt_failed_ind=1)
       SET dm_err->err_ind = 1
       GO TO exit_program
      ENDIF
     ELSE
  ...
  IF (v_cutover_by_context_ind=1
   ...
   ;end select
   IF (check_error(concat("Checking for uncutover rows on the $R table ",v_$r_table))=1)
    ROLLBACK
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SET v_table_incomplete = curqual
   SET v_curqual = v_batch_size
   WHILE (v_curqual >= v_batch_size)
    IF ((contexts_to_cutover->all_ind=0)
     AND v_table_incomplete=1)
     DELETE  FROM dm_refchg_xlat_ctxt_r d
      WHERE (d.table_name=request->table_name)
       AND expand(v_cur_pos,1,contexts_to_cutover->len,d.context_name,contexts_to_cutover->values[
       v_cur_pos].str)
      WITH maxqual(d,v_batch_size)
     ;end delete
    ...
    ELSE
     ...
     SET v_curqual = curqual
    ENDIF
    IF (check_error(concat("Deleting from dm_refch_xlat_ctxt_r for table ",request->table_name))=1)
     ROLLBACK
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
   ENDWHILE
   COMMIT
   SET v_end_time = sysdate
   SET dm_err->eproc = concat("DM_REFCHG_XLAT_CTXT_R cleanup time for table ",request->table_name,
    " = ",build(datetimediff(v_end_time,v_start_time))," seconds.")
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
 ENDIF
 SUBROUTINE drrs_prepare_stmt(i_str,i_end_ind,i_rdb_asis_ind,i_start_ind,i_curdate,i_todaymid,
  i_midminus1)
   DECLARE s_ret_str = vc
   SET s_ret_str = i_str
   IF (i_rdb_asis_ind=1)
    SET s_ret_str = replace(s_ret_str,"v_curdate",concat("to_date('",format(i_curdate,
       "mm/dd/yyyy hh:mm:ss;;d"),"','mm/dd/yyyy hh24:mi:ss')"),0)
    SET s_ret_str = replace(s_ret_str,"v_todaymid",concat("to_date('",format(i_todaymid,
       "mm/dd/yyyy hh:mm:ss;;d"),"','mm/dd/yyyy hh24:mi:ss')"),0)
    SET s_ret_str = replace(s_ret_str,"v_midminus1",concat("to_date('",format(i_midminus1,
       "mm/dd/yyyy hh:mm:ss;;d"),"','mm/dd/yyyy hh24:mi:ss')"),0)
   ELSE
    SET s_ret_str = replace(s_ret_str,"v_curdate","cnvtdatetime(v_curdate)",0)
    SET s_ret_str = replace(s_ret_str,"v_todaymid","cnvtdatetime(v_todaymid)",0)
    SET s_ret_str = replace(s_ret_str,"v_midminus1","cnvtdatetime(v_midminus1)",0)
   ENDIF
   IF (i_end_ind=1)
    IF (((trim(cnvtupper(s_ret_str))="* END GO") OR (cnvtupper(s_ret_str)="END GO")) )
