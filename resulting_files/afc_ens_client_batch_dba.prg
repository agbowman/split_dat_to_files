CREATE PROGRAM afc_ens_client_batch:dba
 ...
 SET enscount = 0
 FOR (enscount = 1 TO request->client_batch_qual)
   CASE (request->client_batch[enscount].action_type)
    OF "ADD":
     SET action_begin = enscount
     SET action_end = enscount
     EXECUTE afc_add_client_batch
     IF (failed != false)
      GO TO check_error
     ENDIF
    OF "UPT":
...
#check_error
 IF (failed=false)
  IF (validate(reply->client_batch_qual,9999) != 9999)
   SET reply->client_batch_qual = request->client_batch_qual
  ENDIF
