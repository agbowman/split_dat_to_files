CREATE PROGRAM al_bhs_sch_remind_call:dba
 EXECUTE bhs_hlp_ftp
 ...
 DECLARE ms_file = vc WITH protect, constant(concat("sn_reminder_call_",trim(cnvtstring(rand(0),20),3
    ),"_",format(cnvtdatetime(sysdate),"YYYYMMDDHHMMSS;;d"),".dat"))
 CALL echo(ms_file)
 ...
 IF ((rem->cnt > 0))
  ...
  SET ms_ftp_path = "ciscore/surginet"
  ...
  SET ms_dclcom = concat("mv ",ms_file," ",trim(logical("BHSCUST"),3),"/surginet/reminder/",
   ms_file)
  CALL echo(ms_dclcom)
  CALL dcl(ms_dclcom,size(trim(ms_dclcom)),ml_stat)
 ENDIF
