CREATE PROGRAM bed_get_pm_rbc_parm_set_r:dba
 ...
 IF (size(request->reltn,5) > 0)
  IF ( NOT (getreltns(0)))
   CALL bederror("Could not return list of RBC relations.")
  ENDIF
 ELSE
 ...
 SUBROUTINE getreltns(dummyvar)
   ...
   SELECT INTO "nl:"
    ...
    HEAD REPORT
     lcnt = 0
    DETAIL
     IF (padpsr.pm_rbc_parm_set_r_id > 0.0)
      lcnt = (lcnt+ 1)
      IF (lcnt > size(reply->reltn,5))
       stat = alterlist(reply->reltn,(lcnt+ 9))
      ENDIF
      reply->reltn[lcnt].id = padpsr.pm_rbc_parm_set_r_id, reply->reltn[lcnt].rc_parameter_set_id =
      padpsr.rc_parameter_set_id, reply->reltn[lcnt].beg_effective_dt_tm = padpsr.beg_effective_dt_tm,
   ...
   IF (lcnt > 0
    ...
    SELECT INTO "nl:"
     ...
     facility_cd = reply->reltn[d.seq].loc_facility.code_value
     ...
     HEAD facility_cd
      ...
      ENDIF
     FOOT  encntr_type_cd
      reply->reltn_summary[ecnt].facility_cnt = fcnt
     FOOT REPORT
