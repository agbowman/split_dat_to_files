CREATE PROGRAM bb_rdm_import_questions:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bb_rdm_import_questions"
 CALL logscriptstart(sreadme_name)
 COMMIT
 CALL logstatus(build("Readme Failed:  Starting script ",sreadme_name,"."),"F")
 DECLARE getcodevaluefromcdf(code_set,cdf_meaning) = f8
 SUBROUTINE getcodevaluefromcdf(code_set,cdf_meaning)
   DECLARE return_code_value = f8
   SET return_code_value = 0.0
   SELECT INTO "nl:"
    c.code_value
    FROM code_value c
    PLAN (c
    ...
    HEAD c.code_value
     return_code_value = c.code_value
    WITH nocounter
   ;end select
   RETURN(return_code_value)
 END ;Subroutine
 DECLARE num_questions = i4
 DECLARE q_cnt = i4
 DECLARE cdf_meaning = c12
 DECLARE code_set = i4
 DECLARE code_value = f8
 ...
 DECLARE answer_id = f8
 DECLARE smsg = vc
 SET num_questions = size(requestin->list_0,5)
 IF (num_questions=0)
 ...
 ENDIF
 SET q_cnt = 1
 SET smsg = build("Number of questions going to import->",num_questions)
 CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 WHILE (q_cnt <= num_questions)
   SET smsg = build("Import question number->",q_cnt)
   CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
   ...
   SET code_value = 0.0
   SET code_set = 1660
   SET cdf_meaning = substring(1,12,requestin->list_0[q_cnt].module_mean)
   SET code_value = getcodevaluefromcdf(code_set,cdf_meaning)
   IF (code_value > 0)
    SET requestin->list_0[q_cnt].module_cd = cnvtstring(code_value,32,2)
   ELSE
    SET success_ind = 0
   ENDIF
   SET code_set = 0
   SET code_value = 0.0
   SET code_set = 1662
   SET cdf_meaning = requestin->list_0[q_cnt].process_mean
   SET code_value = getcodevaluefromcdf(code_set,cdf_meaning)
   IF (code_value > 0)
    SET requestin->list_0[q_cnt].process_cd = cnvtstring(code_value,32,2)
   ELSE
    SET success_ind = 0
   ENDIF
   SET code_set = 0
   SET code_value = 0.0
   SET code_set = 1661
   SET cdf_meaning = requestin->list_0[q_cnt].ques_mean
   SET code_value = getcodevaluefromcdf(code_set,cdf_meaning)
   IF (code_value > 0)
    SET requestin->list_0[q_cnt].question_cd = cnvtstring(code_value,32,2)
   ELSE
    SET success_ind = 0
   ENDIF
   SELECT INTO "NL:"
    q.question_cd
    FROM question q
    WHERE q.question_cd=cnvtreal(requestin->list_0[q_cnt].question_cd)
     AND q.process_cd=cnvtreal(requestin->list_0[q_cnt].process_cd)
     AND q.module_cd=cnvtreal(requestin->list_0[q_cnt].module_cd)
    DETAIL
     row + 0
    WITH nocounter
   ;end select
   IF (curqual=0
    ...
    INSERT  FROM question q
     SET q.module_cd = cnvtreal(requestin->list_0[q_cnt].module_cd), q.process_cd = cnvtreal(
       requestin->list_0[q_cnt].process_cd), q.question_cd = cnvtreal(requestin->list_0[q_cnt].
       question_cd),
      q.question = requestin->list_0[q_cnt].ques_text, q.sequence = cnvtint(requestin->list_0[q_cnt].
       resp_seq), q.response_flag = cnvtint(requestin->list_0[q_cnt].resp_flag),
      q.response_length = cnvtint(requestin->list_0[q_cnt].resp_length), q.code_set = cnvtint(
       requestin->list_0[q_cnt].resp_codeset), q.def_answer = requestin->list_0[q_cnt].def_answer,
      q.active_ind = cnvtint(requestin->list_0[q_cnt].active_ind), q.dwb_ind = cnvtint(requestin->
       list_0[q_cnt].dwb_ind), q.updt_id = 1,
      q.updt_task = reqinfo->updt_task, q.updt_applctx = 1, q.updt_cnt = 0,
    ...
    UPDATE  FROM question q
     SET q.module_cd = cnvtreal(requestin->list_0[q_cnt].module_cd), q.process_cd = cnvtreal(
      ...
      q.question = requestin->list_0[q_cnt].ques_text, q.sequence = cnvtint(requestin->list_0[q_cnt].
       resp_seq), q.response_flag = cnvtint(requestin->list_0[q_cnt].resp_flag),
      q.response_length = cnvtint(requestin->list_0[q_cnt].resp_length), q.code_set = cnvtint(
       requestin->list_0[q_cnt].resp_codeset), q.active_ind = cnvtint(requestin->list_0[q_cnt].
       active_ind),
      q.def_answer = requestin->list_0[q_cnt].def_answer, q.dwb_ind = cnvtint(requestin->list_0[q_cnt
       ].dwb_ind), q.updt_id = 1,
      q.updt_task = reqinfo->updt_task, q.updt_applctx = 1, q.updt_cnt = (q.updt_cnt+ 1),
      q.updt_dt_tm = cnvtdatetime(curdate,curtime3)
     WHERE q.question_cd=cnvtreal(requestin->list_0[q_cnt].question_cd)
      AND q.process_cd=cnvtreal(requestin->list_0[q_cnt].process_cd)
      AND q.module_cd=cnvtreal(requestin->list_0[q_cnt].module_cd)
   ...
   SELECT INTO "NL:"
    ans.question_cd
    FROM answer ans
    WHERE ans.question_cd=cnvtreal(requestin->list_0[q_cnt].question_cd)
     AND ans.process_cd=cnvtreal(requestin->list_0[q_cnt].process_cd)
     AND ans.module_cd=cnvtreal(requestin->list_0[q_cnt].module_cd)
   ...
   IF (curqual=0
    ...
    SET code_set = 1659
    SET code_value = 0.0
    SET cdf_meaning = requestin->list_0[q_cnt].def_answer
    SET code_value = getcodevaluefromcdf(code_set,cdf_meaning)
    IF (code_value > 0.00)
     SET answer = trim(cnvtstring(code_value,19,0))
     SELECT INTO "NL:"
      nextseqnum = seq(pathnet_seq,nextval)"#################;rp0"
      FROM dual
      DETAIL
       answer_id = cnvtreal(nextseqnum)
      WITH format
     ;end select
     IF (answer_id=0)
      SET success_ind = 0
     ...
     IF (success_ind=1)
      INSERT  FROM answer a
       SET a.module_cd = cnvtreal(requestin->list_0[q_cnt].module_cd), a.process_cd = cnvtreal(
         requestin->list_0[q_cnt].process_cd), a.question_cd = cnvtreal(requestin->list_0[q_cnt].
         question_cd),
        a.answer_id = answer_id, a.sequence = 1, a.answer = answer,
        a.active_ind = cnvtint(requestin->list_0[q_cnt].active_ind), a.active_dt_tm =
        IF ((requestin->list_0[q_cnt].active_ind="1")) cnvtdatetime(curdate,curtime3)
        ELSE null
        ENDIF
        , a.inactive_dt_tm =
        IF ((requestin->list_0[q_cnt].active_ind="0")) cnvtdatetime(curdate,curtime3)
        ELSE null
        ENDIF
        ,
        a.updt_id = 1, a.updt_task = reqinfo->updt_task, a.updt_dt_tm = cnvtdatetime(curdate,curtime3
         ),
   ...
   ENDIF
   IF (success_ind=0)
    SET smsg = build("Unsuccessful import for question number->",q_cnt)
    CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    ROLLBACK
   ELSE
    SET smsg = build("Successful import for question number->",q_cnt)
    CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
    COMMIT
   ENDIF
   SET q_cnt = (q_cnt+ 1)
