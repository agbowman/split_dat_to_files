CREATE PROGRAM cls_physstats_v2:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  oa.communication_type_cd, p.person_id, docname = substring(1,30,p.name_full_formatted),
 ...
 SELECT INTO value(output_dest)
  ...
  HEAD REPORT
   ...
   ELSE
    ...
    ELSEIF (( $PROMPT4="2"))
     col 1, "All Encounter Types"
    ENDIF
    row + 1, output_string = concat("DocName",",","Specialty",",","Facility",
     ",","MD Ord",",","MD Pct",",",
     "Prot Ord",",","Prot Pct",",","PhVb Ord",
     ",","PhVb Pct",",","SID Ord",",",
     "SID Pct",",","Writ Ord",",","Writ Pct",
     ",","Other Ord",",","Total Ord",",",
     "CPOE Rate"), col 1,
    output_string, row + 1
   ENDIF
  ...
  DETAIL
   ...
   ELSE
    docname = replace(docname,","," ",0), output_string = build(docname,",",phys_type,",",
     loc_facility_display,
     ",",phys->list[d.seq].mdcount,",",mdpct,",",
     phys->list[d.seq].prcount,",",prpct,",",phys->list[d.seq].pvcount,
     ",",pvpct,",",phys->list[d.seq].sidcount,",",
     sidpct,",",phys->list[d.seq].wrcount,",",wrpct,
