CREATE PROGRAM dm_ecmb_health_concern:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_HEALTH_CONCERN"
 ...
 SELECT INTO "nl:"
  ...
  WHERE (((frm.documented_encntr_id=request->xxx_combine[icombine].from_xxx_id)) OR ((frm
  .last_updt_encntr_id=request->xxx_combine[icombine].from_xxx_id)))
  DETAIL
   v_cust_count += 1
   ...
   .active_status_cd,
   rreclist->from_rec[v_cust_count].beg_effective_dt_tm = frm.beg_effective_dt_tm, rreclist->
   from_rec[v_cust_count].end_effective_dt_tm = frm.end_effective_dt_tm, rreclist->from_rec[
   v_cust_count].documented_encntr_id = frm.documented_encntr_id,
   rreclist->from_rec[v_cust_count].last_updt_encntr_id = frm.last_updt_encntr_id
 ...
 IF (v_cust_count > 0)
  FOR (v_cust_loopcount = 1 TO v_cust_count)
    IF ((rreclist->from_rec[v_cust_loopcount].active_status_cd != combinedaway)
     ...
     IF (add_to(rreclist->from_rec[v_cust_loopcount].from_id,request->xxx_combine[icombine].
      from_xxx_id,request->xxx_combine[icombine].to_xxx_id,rreclist->from_rec[v_cust_loopcount].
      documented_encntr_id,rreclist->from_rec[v_cust_loopcount].last_updt_encntr_id)=0)
      GO TO exit_sub
     ENDIF
  ...
  ENDFOR
 ENDIF
 SUBROUTINE add_to(s_at_from_pk_id,s_at_from_fk_id,s_at_to_fk_id,documented_encntr_id,
  last_updt_encntr_id)
   ...
   IF (documented_encntr_id=s_at_from_fk_id)
    SET documented_encntr_id = s_at_to_fk_id
   ENDIF
   IF (last_updt_encntr_id=s_at_from_fk_id)
    SET last_updt_encntr_id = s_at_to_fk_id
   ENDIF
   SELECT INTO "nl:"
   ...
   IF (at_active_size=0)
    SET stat = alterlist(dm_cmb_cust_cols->add_col_val,(at_active_size+ 7))
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 1)].col_name = "DOCUMENTED_ENCNTR_ID"
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 1)].col_value = build(documented_encntr_id)
    ...
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 3)].col_name = "HEALTH_CONCERN_INSTANCE_UUID"
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 3)].col_value = build(char(34),
     v_new_instance_uuid,char(34))
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 4)].col_name = "BEG_EFFECTIVE_DT_TM"
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 4)].col_value =
    "cnvtdatetime(v_current_dt_tm)"
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 5)].col_name = "LAST_UPDT_ID"
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 5)].col_value = build(reqinfo->updt_id)
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 6)].col_name = "LAST_UPDT_DT_TM"
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 6)].col_value =
    "cnvtdatetime(v_current_dt_tm)"
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 7)].col_name = "LAST_UPDT_ENCNTR_ID"
    SET dm_cmb_cust_cols->add_col_val[(at_active_size+ 7)].col_value = build(last_updt_encntr_id)
   ELSE
   ...
   SET request->xxx_combine_det[icombinedet].attribute_name = "DOCUMENTED_ENCNTR_ID"
   RETURN(1)
 END ;Subroutine
