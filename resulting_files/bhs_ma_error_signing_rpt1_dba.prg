CREATE PROGRAM bhs_ma_error_signing_rpt1:dba
 PROMPT
  "Output to File/Printer/MINE " = "MINE",
  "From Date (MMDDYY)" = "      ",
  "To Date (MMDDYY)" = "      "
 IF (( $2="      "))
  SET  $2 = format((curdate - 1),"MMDDYY;;D")
 ENDIF
 IF (( $3="      "))
  SET  $3 = format(curdate,"MMDDYY;;D")
 ENDIF
 RECORD dlrec(
   1 ord_total = i4
   1 seq[*]
 ...
 SELECT INTO "nl:"
  ...
  PLAN (orv
   ...
   WHERE orv.order_id=orn.order_id
    ...
    AND orn.notification_dt_tm BETWEEN cnvtdatetime( $2) AND cnvtdatetime( $3))
   JOIN (pl
 ...
 SELECT INTO  $1
  ...
  HEAD PAGE
   ...
   CALL print(calcpos(xcol2,yrow1)),  $2, " ",
    $3, rowplusone2,
   CALL print(calcpos(xcol1,yrow1)),
   "{color/14}", equal_line, "{color/0}",
   rowplusone2, yrow1 = (yrow1+ 4),
   CALL print(calcpos(xcol1,yrow1)),
   "{b/4}Name",
   ...
   CALL print(calcpos(xcol12,yrow1)),
   "{b/4}Type", yrow1 = (yrow1+ 4),
   CALL print(calcpos(xcol1,yrow1)),
   line1, rowplusone2,
   CALL print(calcpos(xcol1,yrow1)),
   "{B}The Following Were Signed In Error by: ", dlrec->seq[c].sign_doctor, "{endb}",
