CREATE PROGRAM dm_search_session_rows:dba
 ...
 DECLARE lerrcd = i4 WITH protect, noconstant(0)
 ...
 DECLARE lrows = i4 WITH protect, noconstant(0)
 DECLARE dbatchsize = f8 WITH protect, noconstant(50000.0)
 DECLARE dmaxid = f8 WITH protect, noconstant(0.0)
 DECLARE dcurminid = f8 WITH protect, noconstant(1.0)
 DECLARE dcurmaxid = f8 WITH protect, noconstant(0.0)
 SET reply->table_name = "SEARCH_SESSION"
 ...
 FOR (ltokenindex = 1 TO size(request->tokens,5))
   IF ((request->tokens[ltokenindex].token_str="DAYSTOKEEP"))
    SET ldaystokeep = ceil(cnvtreal(request->tokens[ltokenindex].value))
   ENDIF
 ENDFOR
 IF (ldaystokeep < 180)
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(ss.search_session_id)
   FROM search_session ss
   WHERE ss.search_session_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(ss.search_session_id)
  FROM search_session ss
  DETAIL
 ...
 WHILE (dcurminid <= dmaxid
  AND lrowsleft > 0)
   SELECT INTO "nl:"
    ss.rowid
    FROM search_session ss
    WHERE parser(sbr_getrowidnotexists("ss.search_session_id between dCurMinID and dCurMaxID","ss"))
     AND ss.session_dt_tm <= cnvtdatetime((curdate - ldaystokeep),235959)
    DETAIL
     lrows = (lrows+ 1)
     IF (mod(lrows,5000)=1)
      stat = alterlist(reply->rows,(lrows+ 4999))
     ENDIF
     reply->rows[lrows].row_id = ss.rowid
    WITH nocounter, maxqual(ss,value(lrowsleft))
   ;end select
   SET lerrcd = error(serrmsg,1)
   IF (lerrcd > 0)
    SET reply->err_code = lerrcd
   ...
   SET dcurmaxid = (dcurminid+ (dbatchsize - 1))
   SET lrowsleft = (request->max_rows - lrows)
 ENDWHILE
 SET stat = alterlist(reply->rows,lrows)
