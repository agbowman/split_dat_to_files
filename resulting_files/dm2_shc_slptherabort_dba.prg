CREATE PROGRAM dm2_shc_slptherabort:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((((validate(failed_scripts->cnt,- (1))=- (1))) OR ((validate(failed_scripts->cnt,- (2))=- (2))
 )) )
  RECORD failed_scripts(
    1 cnt = i4
    1 script[*]
      2 entry_number = vc
      2 script_name = vc
  )
  SET failed_scripts->cnt = 0
 ENDIF
 DECLARE cnt = i4 WITH protect, noconstant(0)
 DECLARE cnt2 = i4 WITH protect, noconstant(0)
 DECLARE dsta_getscript = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsta_prev_errfile = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsta_dcl_str = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsta_del_str = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsta_status = c1 WITH protect, noconstant("X")
 DECLARE dsta_extension = vc WITH protect, noconstant(".ksh")
 DECLARE dsta_err_found_ind = i2 WITH protect, noconstant(0)
 DECLARE temp_dt_tm_24hr_ago = f8 WITH protect, constant(cnvtdatetime(curdate,curtime))
 DECLARE dsta_prev_script = vc WITH protect, noconstant("NOT_SET")
 DECLARE dsta_fatal_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dsta_script_start_pos = i4 WITH protect, noconstant(0)
 DECLARE dsta_script_end_pos = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_shc_sta",".log","SYSTEM HEALTH CHECK SLEEPER THREAD ABORT LOG FILE")=0)
  SET dsta_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking MSGView for SleeperThreadAborting errors."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ENDIF
 IF (cursys="AXP")
  SET dsta_extension = ".com"
 ENDIF
 IF (get_unique_file("dm2_get_sta_errs_",dsta_extension)=0)
  SET dsta_status = "E"
 ENDIF
 IF (dsta_status != "E")
  SET dsta_getscript = dm_err->unique_fname
  SET dsta_prev_errfile = dm_err->errfile
  SET dm_err->errfile = replace(dsta_getscript,dsta_extension,".dat",0)
  IF (cursys="AIX")
   SET dsta_dcl_str = concat("$CCLUSERDIR/",dsta_getscript)
   SET dsta_del_str = "rm -f $CCLUSERDIR/dm2_get_sta_errs*"
  ELSE
   SET dsta_dcl_str = concat("@ccluserdir:",dsta_getscript)
   SET dsta_del_str = "delete ccluserdir:dm2_get_sta_errs*.*;*"
  ENDIF
  SET dm_err->eproc = "Writing msgview script."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO trim(dsta_getscript)
   FROM dual
   DETAIL
    IF (cursys="AIX")
     col + 0, "   msgview <<!"
    ELSE
     col + 0, "   $ msgview"
    ENDIF
    row + 1, col + 0, "   select cmb_0051",
    row + 1, col + 0, "   search -event SleeperThreadAborting"
    IF (cursys="AIX")
     row + 1, col + 0, "   exit",
     row + 1, col + 0, "   !"
    ENDIF
 ...
 IF (dsta_status != "E"
  AND cursys="AIX")
  IF (dm2_push_dcl(concat("chmod ugo+x $CCLUSERDIR/",dsta_getscript))=0)
   SET dsta_status = "E"
  ENDIF
 ENDIF
 IF (dsta_status != "E")
  IF (dm2_push_dcl(dsta_dcl_str)=0)
   SET dsta_status = "E"
  ENDIF
 ENDIF
 IF (dsta_status != "E")
 ...
 ENDIF
 IF (dsta_status != "E")
  SET dm_err->eproc = "Parsing msgview output."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   r.line
   FROM rtl2t r
   HEAD REPORT
    rpt_substr = fillstring(6," "), rpt_date = 0.0
   DETAIL
    rpt_substr = substring(1,6,r.line)
    IF (isnumeric(rpt_substr))
     rpt_date = cnvtdatetime(concat(format(cnvtdate2(build(substring(9,5,r.line),"/",year(curdate)),
         "MM/DD/YYYY"),"DD-MMM-YYYY;;D")," ",substring(15,5,r.line)))
     IF (datetimediff(temp_dt_tm_24hr_ago,rpt_date,3) < 24)
      failed_scripts->cnt += 1, stat = alterlist(failed_scripts->script,failed_scripts->cnt),
      failed_scripts->script[failed_scripts->cnt].entry_number = rpt_substr
     ENDIF
    ENDIF
   WITH nocounter
  ;end select
 ...
 ENDIF
 IF (dsta_status != "E")
  FOR (cnt = 1 TO failed_scripts->cnt)
    SET dsta_script_start_pos = 0
    SET dsta_script_end_pos = 0
    SET dm_err->eproc = "Writing msgview detail script."
    CALL disp_msg("",dm_err->logfile,0)
    IF (dsta_status != "E")
     SELECT INTO trim(dsta_getscript)
      FROM dual
      DETAIL
       ...
       row + 1, col + 0, "   select cmb_0051",
       row + 1, col + 0, "   show ",
       failed_scripts->script[cnt].entry_number
       IF (cursys="AIX")
        row + 1, col + 0, "   exit",
        row + 1, col + 0, "   !"
       ENDIF
    ...
    IF (dsta_status != "E")
     SELECT INTO "nl:"
      ...
      FROM rtl2t r
      DETAIL
       dsta_script_start_pos = findstring("scriptname::",r.line)
       IF (dsta_script_start_pos > 0)
        dsta_script_start_pos += 12, dsta_script_end_pos = findstring("}}",r.line,
         dsta_script_start_pos), failed_scripts->script[cnt].script_name = substring(
         dsta_script_start_pos,(dsta_script_end_pos - dsta_script_start_pos),r.line)
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error("Parsing msgview details.")=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      SET dsta_status = "E"
      SET cnt = failed_scripts->cnt
     ENDIF
 ...
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(failed_scripts)
 ENDIF
 IF ((((dm_err->err_ind=1)) OR (dsta_status="E")) )
  SET dsta_status = "E"
 ELSEIF ((failed_scripts->cnt > 0))
  SET dsta_status = "R"
 ELSE
  SET dsta_status = "G"
 ENDIF
 IF (dshc_add_check_to_rs("SleepThreadAbort Msg","SleeperThreadAborting Messages",0,dsta_status,
  dshc_ctf_millennium)=0)
  SET dsta_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 IF (dsta_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "When CCL scripts run by the CPM Script server (server #51) take long amounts of time ",
  "to run (longer than the CPM Script server property killtime) the CPM Script Server ",
  "will log a SleeperThreadAborting message into msgview and exit.  This behavior ",
  "prevents a backup of the cpmscript service.<SPACE>If more than 3 ",
  "SleeperThreadAborting messages are logged in 24 hours for any CCL script, then a ",
  "service request should be logged to the M SCRIPT PERF queue via the IAC to correct ",
  "the behavior of the offending CCL script.<SPACE>The following scripts have aborted ",
  "in the last 24 hours.  Contact the IAC to correct any CCL errors logged.")
 SELECT INTO "nl:"
  d.seq
  FROM (dummyt d  WITH seq = failed_scripts->cnt)
  ORDER BY failed_scripts->script[d.seq].script_name
  DETAIL
   IF ((failed_scripts->script[d.seq].script_name != dsta_prev_script))
    dsta_prev_script = failed_scripts->script[d.seq].script_name, dshc->check[dshc->chk_cnt].
    desc_dtl_cnt += 1, stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,dshc->check[dshc->
     chk_cnt].desc_dtl_cnt),
    dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].name =
    "Script Aborted", dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt]
    .value_prefix = "NOT_SET", dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].
    desc_dtl_cnt].dtl_desc_text = "NOT_SET",
    dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].type = 2, dshc->
    check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].str_value =
    failed_scripts->script[d.seq].script_name
   ENDIF
  WITH nocounter
 ;end select
#exit_script
 SET dm_err->eproc = "Finished checking MSGView for SleeperThreadAborting errors."
 CALL final_disp_msg("dm2_shc_sta")
 IF (dsta_fatal_err_ind=0)
  SET dm_err->err_ind = 0
 ENDIF
 FREE RECORD failed_scripts
END GO
