CREATE PROGRAM bbt_rpt_release:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->reason_tool = uar_i18ngetmessage(i18nhandle,"reason_tool","REASONS WITH MEANINGS")
 SET captions->release_product = uar_i18ngetmessage(i18nhandle,"release_product",
  "RELEASE PRODUCT REASONS")
 ...
 SET select_ok_ind = 0
 DECLARE display = vc WITH proctect, noconstant("")
 DECLARE desc = vc WITH protect, noconstant("")
 DECLARE meaning = vc WITH protect, noconstant("")
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_release", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  display = substring(1,37,cv.display), desc = substring(1,36,cv.description), cv.cdf_meaning,
  cv.active_ind, cv.collation_seq, cd.cdf_meaning,
  meaning = substring(1,38,cd.display)
  FROM code_value cv,
   common_data_foundation cd
  PLAN (cv
   WHERE cv.code_set=1616)
   JOIN (cd
   WHERE cd.code_set=outerjoin(1616)
  ...
  HEAD PAGE
   ...
   curdate"@DATECONDENSED;;d", col 51, captions->database_audit,
   col 108, captions->page_no, col 120,
   curpage"##", row + 1, col 7,
   captions->time, col 14, curtime"@TIMENOSECONDS;;M",
   col 48, captions->reason_tool, row + 1,
   col 47, captions->release_product, row + 2,
   line = fillstring(122,"-"), line, row + 1,
   col 4, captions->display_name, col 42,
   captions->description, col 79, captions->meaning,
   col 115, captions->active, row + 1,
   line, row + 2
  DETAIL
   col 2, display, col 40,
   desc, col 77, meaning
   IF (cv.active_ind=1)
    col 116, captions->yes
