CREATE PROGRAM bhs_athn_order_detail_sm
 IF (( $3 != 0))
 ...
 SELECT DISTINCT INTO  $1
  ...
  dx_string = trim(replace(replace(replace(replace(replace(order_list->qual[d1.seq].dx_string,"&",
        "&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),
  oc_stop_type_meaning = uar_get_code_meaning(oc.stop_type_cd), o_protocol_order_id = trim(replace(
    cnvtstring(o.protocol_order_id),".0*","",0),3),
  oc.ref_text_mask
  FROM (dummyt d1  WITH seq = size(order_list->qual,5)),
   orders o,
   order_detail od,
  ...
  HEAD o.order_id
   ...
   row + 1, v1083 = build("<ProtocolOrderId>",o_protocol_order_id,"</ProtocolOrderId>"), col + 1,
   v1083, row + 1, v1084 = build("<RefTextMask>",
    IF (oc.ref_text_mask > 0) "RefTextAvailable"
    ELSE "RefTextNotAvailable"
    ENDIF
    ,"</RefTextMask>"),
   col + 1, v1084, row + 1,
   v109 = build("<RefTaskID>",cnvtint(t.reference_task_id),"</RefTaskID>"), col + 1, v109,
   row + 1
   IF (o.dcp_clin_cat_cd=10576)
