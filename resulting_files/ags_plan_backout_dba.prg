CREATE PROGRAM ags_plan_backout:dba
 PROMPT
  "Backout Type (J, I, P) =" = "J",
  "AGS_JOB_ID (0.0) = " = 0.0
  WITH cbackout_type, djob_id
 SET ags_plan_backout_mod = "001 11/02/06"
 CALL echo("<===== AGS_PLAN_BACKOUT Begin =====>")
 CALL echo(concat("MOD:",ags_plan_backout_mod))
 ...
 IF ( NOT (validate(log,0)))
  ...
  DECLARE sstatus_file_name = vc WITH private, noconstant(concat("ags_plan_backout_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_PLAN_BACKOUT"
 SET log->qual_knt = (log->qual_knt+ 1)
 SET stat = alterlist(log->qual,log->qual_knt)
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 ...
 ELSEIF (cnvtupper(the_type)="I")
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    WHERE d.seq > 0
     AND (item_rec->qual[d.seq].data_id > 0.0))
    JOIN (a
    WHERE (a.ags_plan_data_id=item_rec->qual[d.seq].data_id))
   ORDER BY a.person_rx_plan_reltn_id
   HEAD REPORT
    lidx = 0, stat = alterlist(planrec->qual,10), lidx2 = 0,
    stat = alterlist(planrec->qual2,10)
 ...
 IF ((planrec->qual_knt > 0))
  ...
  DELETE  FROM person_rx_plan_reltn r,
   ...
   PLAN (d
    WHERE d.seq > 0
     AND (planrec->qual2[d.seq].person_rx_plan_reltn_id > 0.0))
    JOIN (r
  ...
  IF (ierrcode > 0)
   ...
   GO TO exit_script
  ENDIF
  UPDATE  FROM ags_plan_data a,
    (dummyt d  WITH seq = value(planrec->qual_knt))
   SET a.person_id = 0.0, a.health_plan_id = 0.0, a.person_rx_plan_reltn_id = 0.0,
    a.person_id = 0.0, a.status = "BACK OUT", a.status_dt_tm = cnvtdatetime(curdate,curtime3)
   PLAN (d
    WHERE d.seq > 0
     AND (planrec->qual[d.seq].ags_plan_data_id > 0))
    JOIN (a
  ...
  IF (ierrcode > 0)
   ...
   SET log->qual[log->qual_knt].smsgtype = "ERROR"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("UPDATE Error :: ErrMsg :: ",trim(serrmsg))
   GO TO exit_script
  ENDIF
 ENDIF
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> ags_plan_backout"
 IF (define_logging_sub=true)
  CALL echo("***")
 ...
 CALL echo("<===== AGS_PLAN_BACKOUT End =====>")
END GO
