CREATE PROGRAM cp_get_concept_list:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 DECLARE concept_cs = i4 WITH protect, constant(4003132)
 RECORD report_data(
   1 codes[*]
     2 code = f8
     2 sequence = i4
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   code_cnt += 1
   IF (code_cnt > size(report_data->codes,5))
    stat = alterlist(report_data->codes,(code_cnt+ 9))
   ENDIF
   report_data->codes[code_cnt].code = cv.code_value, report_data->codes[code_cnt].sequence = cv
   .collation_seq, report_data->codes[code_cnt].display = cv.display,
   report_data->codes[code_cnt].concept_cki = cv.concept_cki
  FOOT REPORT
   stat = alterlist(report_data->codes,code_cnt)
  WITH nocounter
 ;end select
 CALL error_and_zero_check_rec(curqual,"CODE_VALUE","GetCodesByCodeSet",1,1,
  report_data)
