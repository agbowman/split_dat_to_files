CREATE PROGRAM dm2_rpt_schema_diff:dba
 ...
 SUBROUTINE drr_chk_active_runners(dcar_dblink,dcar_count_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drsd_logfile = vc WITH protect, noconstant("dm2rtpschemadiff")
 DECLARE drsd_diff_file = vc WITH public, noconstant(" ")
 DECLARE drsd_str = vc WITH protect, noconstant(" ")
 DECLARE drsd_tbl_cnt = i4 WITH protect, noconstant(0)
 DECLARE drsd_ind_cnt = i4 WITH protect, noconstant(0)
 DECLARE drsd_newobj_ind = i2 WITH protect, noconstant(0)
 DECLARE dcd_bf_tblidx = i4 WITH protect, noconstant(0)
 DECLARE dcd_bf_colidx = i4 WITH protect, noconstant(0)
 DECLARE bf_col = i4 WITH protect, noconstant(0)
 DECLARE bf_op = i4 WITH protect, noconstant(0)
 DECLARE drsd_iter = i4 WITH protect, noconstant(0)
 DECLARE drsd_iter2 = i4 WITH protect, noconstant(0)
 DECLARE drsd_mv_tblidx = i4 WITH protect, noconstant(0)
 DECLARE drsd_temp_tspace_name = vc WITH protect, noconstant(" ")
 DECLARE drsd_event_dt_name = vc WITH protect, noconstant(" ")
 DECLARE drsd_ddl_excl_ind = i2 WITH protect, noconstant(0)
 DECLARE drsd_user_cnt = i2 WITH protect, noconstant(0)
 DECLARE drsd_priv_cnt = i2 WITH protect, noconstant(0)
 DECLARE drsd_quota_cnt = i2 WITH protect, noconstant(0)
 DECLARE drsd_tmp_idx = i2 WITH protect, noconstant(0)
 IF (check_logfile(drsd_logfile,".log","DM2_RPT_SCHEMA_DIFF BEGIN")=0)
  GO TO end_program
 ENDIF
 IF ((dm2_common1->snapshot_id=1))
  SET drsd_diff_file = build(dm2_install_schema->logfile_prefix,"_1.rpt")
  SET drsd_event_dt_name = "RPT:SCHEMA_DIFF_SNAP1"
 ELSEIF ((dm2_common1->snapshot_id=2))
  SET drsd_diff_file = build(dm2_install_schema->logfile_prefix,"_2.rpt")
  SET drsd_event_dt_name = "RPT:SCHEMA_DIFF_SNAP2"
 ELSE
  SET drsd_diff_file = build(dm2_install_schema->logfile_prefix,".rpt")
  SET drsd_event_dt_name = "RPT:SCHEMA_DIFF"
 ENDIF
 FOR (drsd_tbl_cnt = 1 TO tgtsch->tbl_cnt)
  IF ((tgtsch->tbl[drsd_tbl_cnt].new_ind=1))
   SET drsd_newobj_ind = 1
   SET drsd_tbl_cnt = tgtsch->tbl_cnt
  ELSE
   FOR (drsd_ind_cnt = 1 TO tgtsch->tbl[drsd_tbl_cnt].ind_cnt)
     IF ((tgtsch->tbl[drsd_tbl_cnt].ind[drsd_ind_cnt].new_ind=1))
      SET drsd_newobj_ind = 1
      SET drsd_ind_cnt = tgtsch->tbl[drsd_tbl_cnt].ind_cnt
      SET drsd_tbl_cnt = tgtsch->tbl_cnt
     ENDIF
   ENDFOR
  ENDIF
  IF ((tgtsch->tbl[drsd_tbl_cnt].mview_build_ind=1))
   IF (dmvr_get_mv_metadata(tgtsch->tbl[drsd_tbl_cnt].tbl_name,tgtsch->tbl[drsd_tbl_cnt].
    schema_instance)=0)
    GO TO end_program
   ENDIF
   SET tgtsch->tbl[drsd_tbl_cnt].mview_log_cnt = mv->mlog_cnt
   SET stat = alterlist(tgtsch->tbl[drsd_tbl_cnt].mview_logs,mv->mlog_cnt)
   FOR (drsd_iter2 = 1 TO mv->mlog_cnt)
     SET tgtsch->tbl[drsd_tbl_cnt].mview_logs[drsd_iter2].table_name = mv->mlog[drsd_iter2].
     table_name
   ENDFOR
   SET tgtsch->tbl[drsd_tbl_cnt].mview_piece_cnt = mv->piece_cnt
   SET stat = alterlist(tgtsch->tbl[drsd_tbl_cnt].mview_piece,mv->piece_cnt)
   FOR (drsd_iter2 = 1 TO mv->piece_cnt)
     SET tgtsch->tbl[drsd_tbl_cnt].mview_piece[drsd_iter2].txt = mv->txt[drsd_iter2].piece
   ENDFOR
  ENDIF
 ENDFOR
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(dir_dropped_objects)
 ENDIF
 FREE RECORD tmp_backfill_rpt
 RECORD tmp_backfill_rpt(
   1 tbl_cnt = i4
   1 tbl[*]
     2 tbl_name = vc
     2 col_cnt = i4
 ...
 FOR (bf_op = 1 TO tgtsch->backfill_ops_cnt)
   IF ((tgtsch->backfill_ops[bf_op].reset_backfill=1)
    AND (tgtsch->backfill_ops[bf_op].tgt_tbl_idx=0))
    SET dcd_bf_tblidx = locateval(dcd_bf_tblidx,1,tmp_backfill_rpt->tbl_cnt,tgtsch->backfill_ops[
     bf_op].table_name,tmp_backfill_rpt->tbl[dcd_bf_tblidx].tbl_name)
    IF (dcd_bf_tblidx=0)
     SET tmp_backfill_rpt->tbl_cnt = (tmp_backfill_rpt->tbl_cnt+ 1)
     SET stat = alterlist(tmp_backfill_rpt->tbl,tmp_backfill_rpt->tbl_cnt)
     SET tmp_backfill_rpt->tbl[tmp_backfill_rpt->tbl_cnt].tbl_name = tgtsch->backfill_ops[bf_op].
     table_name
     SET tmp_backfill_rpt->tbl[tmp_backfill_rpt->tbl_cnt].col_cnt = tgtsch->backfill_ops[bf_op].
     col_cnt
     SET stat = alterlist(tmp_backfill_rpt->tbl[tmp_backfill_rpt->tbl_cnt].col,tmp_backfill_rpt->tbl[
      tmp_backfill_rpt->tbl_cnt].col_cnt)
     FOR (bf_col = 1 TO tgtsch->backfill_ops[bf_op].col_cnt)
       SET tmp_backfill_rpt->tbl[tmp_backfill_rpt->tbl_cnt].col[bf_col].col_name = tgtsch->
       backfill_ops[bf_op].col[bf_col].col_name
     ENDFOR
    ELSE
     FOR (bf_col = 1 TO tgtsch->backfill_ops[bf_op].col_cnt)
       IF (locateval(dcd_bf_colidx,1,tmp_backfill_rpt->tbl[dcd_bf_tblidx].col_cnt,tgtsch->
        backfill_ops[bf_op].col[bf_col].col_name,tmp_backfill_rpt->tbl[dcd_bf_tblidx].col[
        dcd_bf_colidx].col_name)=0)
        SET tmp_backfill_rpt->tbl[dcd_bf_tblidx].col_cnt = tgtsch->backfill_ops[bf_op].col_cnt
        SET stat = alterlist(tmp_backfill_rpt->tbl[dcd_bf_tblidx].col,tmp_backfill_rpt->tbl[
         dcd_bf_tblidx].col_cnt)
        SET tmp_backfill_rpt->tbl[dcd_bf_tblidx].col[tmp_backfill_rpt->tbl[dcd_bf_tblidx].col_cnt].
        col_name = tgtsch->backfill_ops[bf_op].col[bf_col].col_name
       ENDIF
     ENDFOR
    ENDIF
   ENDIF
 ENDFOR
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(tmp_backfill_rpt)
 ENDIF
 SET dm_err->eproc = "Creating Schema Difference report"
 SET diff_count = 0
 SELECT INTO value(drsd_diff_file)
  FROM dual
  HEAD REPORT
   iter = 0
  DETAIL
   ...
   row + 1, row + 1
   IF (drsd_newobj_ind=1)
    "You can assign custom tablespaces for new tables and indexes by creating a tablespace mapping from ",
    row + 1,
    "a Cerner-recommended tablespace to your own custom tablespace.  To manage tablespace mappings, ",
    row + 1, "use dm2_domain_maint -> Installation Configuration -> Tablespace Mapping.", row + 1,
    row + 1
   ENDIF
   FOR (drsd_user_cnt = 1 TO tgtsch->user_cnt)
     IF ((tgtsch->user[drsd_user_cnt].new_ind=1))
      row + 3, "User ", tgtsch->user[drsd_user_cnt].user_name,
      row + 1, "-------------------------------------------------------", row + 2,
      col 1, "New User: ", tgtsch->user[drsd_user_cnt].user_name,
      row + 1, col 5, "Default Tablespace: ",
      tgtsch->user[drsd_user_cnt].default_tspace, row + 1, col 5,
      "Temporary Tablespace: ", tgtsch->user[drsd_user_cnt].temp_tspace, row + 1,
      col 1, "Grant the following system privileges to ", col + 0,
      tgtsch->user[drsd_user_cnt].user_name, col + 0, " user:",
      row + 1
      FOR (drsd_priv_cnt = 1 TO tgtsch->user[drsd_user_cnt].priv_cnt)
        col 5, tgtsch->user[drsd_user_cnt].privs[drsd_priv_cnt].priv_name, row + 1
      ENDFOR
      col 1, "Create the following unlimited tablespace quotas for ", col + 0,
      tgtsch->user[drsd_user_cnt].user_name, col + 0, " user:",
      row + 1
      FOR (drsd_quota_cnt = 1 TO tgtsch->user[drsd_user_cnt].quota_cnt)
        col 5, tgtsch->user[drsd_user_cnt].quota[drsd_quota_cnt].tspace_name, row + 1
      ENDFOR
     ELSE
      row + 3, "User ", tgtsch->user[drsd_user_cnt].user_name,
      row + 1, "-------------------------------------------------------", row + 2,
     ...
     ENDIF
   ENDFOR
   IF ((tmp_backfill_rpt->tbl_cnt > 0))
    FOR (bf_op = 1 TO tmp_backfill_rpt->tbl_cnt)
      row + 1, "TABLE ", tmp_backfill_rpt->tbl[bf_op].tbl_name,
      row + 1, "---------------------------------------------", row + 1,
      "Column Backfill on columns:", row + 1
      FOR (bf_col = 1 TO tmp_backfill_rpt->tbl[bf_op].col_cnt)
        col 11, tmp_backfill_rpt->tbl[bf_op].col[bf_col].col_name, row + 1
      ENDFOR
      row + 1
    ENDFOR
    row + 1
   ENDIF
   FOR (t_tbl = 1 TO tgtsch->tbl_cnt)
     drsd_ddl_excl_ind = 0
     IF ((tgtsch->tbl[t_tbl].new_ind=1))
      IF ((tgtsch->tbl[t_tbl].mview_flag=1))
       row + 1, "MATERIALIZED VIEW TABLE ", tgtsch->tbl[t_tbl].tbl_name,
       row + 1, "---------------------------------------------", row + 1,
       row + 1, "New Materialized View Table ", tgtsch->tbl[t_tbl].tbl_name,
       row + 1
      ELSE
       row + 1, "TABLE ", tgtsch->tbl[t_tbl].tbl_name,
       row + 1, "---------------------------------------------", row + 1,
       row + 1, "New Table ", tgtsch->tbl[t_tbl].tbl_name,
       row + 1
      ENDIF
      row + 1
      IF ((tgtsch->tbl[t_tbl].ddl_excl_ind=1))
       drsd_ddl_excl_ind = 1, "**Schema changes will NOT be made for table ", tgtsch->tbl[t_tbl].
       tbl_name,
       " by Install Tools ", row + 1,
       "due to exclusion that might be caused by partitioned objects. Please make schema changes recommended manually ",
       row + 1,
       "using example of ddls displayed in DDL Exclusion report later in process.  No action will result in process failure. ",
       row + 2
      ENDIF
      col 2, "Column Name", col 45,
      "Type", col 55, "Length",
      col 63, "Nullable", col 73,
      "Default", row + 1
      FOR (t_col = 1 TO tgtsch->tbl[t_tbl].tbl_col_cnt)
        IF ((tgtsch->tbl[t_tbl].tbl_col[t_col].virtual_column="YES"))
         col 2, tgtsch->tbl[t_tbl].tbl_col[t_col].col_name, " (virtual)"
        ELSE
         col 2, tgtsch->tbl[t_tbl].tbl_col[t_col].col_name
        ENDIF
        col 45, tgtsch->tbl[t_tbl].tbl_col[t_col].data_type
        IF ((tgtsch->tbl[t_tbl].tbl_col[t_col].data_type="*CHAR*"))
         col 56, tgtsch->tbl[t_tbl].tbl_col[t_col].data_length"#####"
        ELSE
         col 60, "-"
        ENDIF
        col 66, tgtsch->tbl[t_tbl].tbl_col[t_col].nullable, col 73,
        tgtsch->tbl[t_tbl].tbl_col[t_col].data_default, row + 1
      ENDFOR
      "Tablespace ", tgtsch->tbl[t_tbl].tspace_name
      IF ((tgtsch->tbl[t_tbl].ttspace_assignment_choice="CUSTOM TABLESPACE MAPPING"))
       col + 2, "(assigned from a custom tablespace mapping)"
      ENDIF
      row + 1
      IF ((tgtsch->tbl[t_tbl].new_lob_col_ind=1))
       "LOB Tablespace ", tgtsch->tbl[t_tbl].long_tspace
       IF ((tgtsch->tbl[t_tbl].ltspace_assignment_choice="CUSTOM TABLESPACE MAPPING"))
        col + 2, "(assigned from a custom tablespace mapping)"
       ENDIF
       row + 1
      ENDIF
      row + 1
     ELSEIF ((tgtsch->tbl[t_tbl].diff_ind=1))
      c_tbl = tgtsch->tbl[t_tbl].cur_idx
      IF (c_tbl > 0)
       IF ((tgtsch->tbl[t_tbl].mview_flag != 1))
        row + 1, "Table ", tgtsch->tbl[t_tbl].tbl_name,
        row + 1, "---------------------------------------------------"
       ELSEIF ((tgtsch->tbl[t_tbl].mview_flag=1))
        row + 1, "Materialized View ", tgtsch->tbl[t_tbl].tbl_name,
        row + 1, "---------------------------------------------------"
       ENDIF
       row + 1
       IF ((tgtsch->tbl[t_tbl].mview_build_ind=1)
        AND (tgtsch->tbl[t_tbl].mview_flag=1))
        row + 1, "Materialized View will be built on prebuilt table ", row + 1
       ENDIF
       IF ((tgtsch->tbl[t_tbl].ddl_excl_ind=1))
        drsd_ddl_excl_ind = 1, "**Schema changes will NOT be made for table ", tgtsch->tbl[t_tbl].
        tbl_name,
       ...
       new_cols = 0
       FOR (t_col = 1 TO tgtsch->tbl[t_tbl].tbl_col_cnt)
         IF ((tgtsch->tbl[t_tbl].tbl_col[t_col].new_ind=1))
          IF (new_cols=0)
           "New column(s) on table ", tgtsch->tbl[t_tbl].tbl_name, row + 1,
           col 10, "Column Name", col 53,
           "Type", col 63, "Length",
           col 71, "Nullable", col 81,
           "Backfill", col 91, "Default",
           row + 1
          ENDIF
          new_cols = 1
          IF ((tgtsch->tbl[t_tbl].tbl_col[t_col].virtual_column="YES"))
           col 10, tgtsch->tbl[t_tbl].tbl_col[t_col].col_name, " (virtual)"
          ELSE
           col 10, tgtsch->tbl[t_tbl].tbl_col[t_col].col_name
          ENDIF
          col 53, tgtsch->tbl[t_tbl].tbl_col[t_col].data_type
          IF ((tgtsch->tbl[t_tbl].tbl_col[t_col].data_type="*CHAR*"))
           col 64, tgtsch->tbl[t_tbl].tbl_col[t_col].data_length"#####"
          ELSE
           col 68, "-"
          ENDIF
          col 74, tgtsch->tbl[t_tbl].tbl_col[t_col].nullable
          IF ((tgtsch->tbl[t_tbl].tbl_col[t_col].diff_backfill=1))
           col 84, "Y"
          ELSE
           col 84, "N"
          ENDIF
          col 91, tgtsch->tbl[t_tbl].tbl_col[t_col].data_default, row + 1
         ENDIF
       ENDFOR
       IF (new_cols > 0)
        row + 1
       ENDIF
       IF ((tgtsch->tbl[t_tbl].child_tbl_ind=1))
        "*Table added to schema picture because it is a child table to a parent table on this incoming package/batch.",
        row + 1
       ENDIF
       IF ((tgtsch->tbl[t_tbl].new_lob_col_ind=1))
        "LOB Tablespace ", tgtsch->tbl[t_tbl].long_tspace
       ...
       diff_cols = 0
       FOR (t_col = 1 TO tgtsch->tbl[t_tbl].tbl_col_cnt)
         IF ((((tgtsch->tbl[t_tbl].tbl_col[t_col].diff_dtype_ind > 0)) OR ((((tgtsch->tbl[t_tbl].
         tbl_col[t_col].diff_dlength_ind > 0)) OR ((((tgtsch->tbl[t_tbl].tbl_col[t_col].
         diff_nullable_ind > 0)) OR ((((tgtsch->tbl[t_tbl].tbl_col[t_col].diff_default_ind > 0)) OR (
         (tgtsch->tbl[t_tbl].tbl_col[t_col].diff_backfill > 0))) )) )) )) )
          c_col = tgtsch->tbl[t_tbl].tbl_col[t_col].cur_idx
          IF (c_col > 0)
           IF (diff_cols=0)
            "The following columns are different on table ", tgtsch->tbl[t_tbl].tbl_name, row + 1,
            col 10, "Column Name", col 43,
            "Type", col 53, "Length",
            col 61, "Nullable", col 71,
            "Backfill", col 81, "Default",
            row + 1
           ENDIF
           diff_cols = 1, col 0, "Current",
           col 10, cur_sch->tbl[c_tbl].tbl_col[c_col].col_name, col 43,
           cur_sch->tbl[c_tbl].tbl_col[c_col].data_type
           IF ((cur_sch->tbl[c_tbl].tbl_col[c_col].data_type="*CHAR*"))
            col 54, cur_sch->tbl[c_tbl].tbl_col[c_col].data_length"#####"
           ELSE
            col 58, "-"
           ENDIF
           col 64, cur_sch->tbl[c_tbl].tbl_col[c_col].nullable, col 81,
           cur_sch->tbl[c_tbl].tbl_col[c_col].data_default, row + 1, col 0,
           "Target", col 10, tgtsch->tbl[t_tbl].tbl_col[t_col].col_name,
           col 43, tgtsch->tbl[t_tbl].tbl_col[t_col].data_type
           IF ((tgtsch->tbl[t_tbl].tbl_col[t_col].data_type="*CHAR*"))
            col 54, tgtsch->tbl[t_tbl].tbl_col[t_col].data_length"#####"
           ELSE
            col 58, "-"
           ENDIF
           col 64, tgtsch->tbl[t_tbl].tbl_col[t_col].nullable
           IF ((tgtsch->tbl[t_tbl].tbl_col[t_col].diff_backfill=1))
            col 74, "Y"
           ENDIF
           col 81, tgtsch->tbl[t_tbl].tbl_col[t_col].data_default, row + 1
           IF ((tgtsch->tbl[t_tbl].tbl_col[t_col].diff_dtype_ind=2))
            col 10, "WARNING: This data type change is invalid.", row + 1
           ENDIF
           IF ((tgtsch->tbl[t_tbl].tbl_col[t_col].diff_dlength_ind=2))
            col 10, "WARNING: This data length change is invalid.", row + 1
           ENDIF
           row + 1
          ENDIF
         ENDIF
      ...
      ENDIF
     ENDIF
     FOR (td_cons = 1 TO tgtsch->tbl[t_tbl].cons_drop_cnt)
       col 2, "Drop "
       IF ((tgtsch->tbl[t_tbl].cons_drop[td_cons].cons_type="P"))
        col + 0, "primary key "
       ELSEIF ((tgtsch->tbl[t_tbl].cons_drop[td_cons].cons_type="U"))
        col + 0, "unique "
       ELSEIF ((tgtsch->tbl[t_tbl].cons_drop[td_cons].cons_type="R"))
        col + 0, "foreign key "
       ENDIF
       col + 0, "constraint ", tgtsch->tbl[t_tbl].cons_drop[td_cons].cons_name,
       col + 1, "on column(s) ", row + 1,
       cur_cons_idx = tgtsch->tbl[t_tbl].cons_drop[td_cons].cur_cons_idx
       FOR (td_cons_col = 1 TO cur_sch->tbl[c_tbl].cons[cur_cons_idx].cons_col_cnt)
         col 2, cur_sch->tbl[c_tbl].cons[cur_cons_idx].cons_col[td_cons_col].col_name, row + 1
       ENDFOR
       IF ((tgtsch->tbl[t_tbl].cons_drop[td_cons].cons_type="R"))
        col 2, "Parent table ", cur_sch->tbl[c_tbl].cons[cur_cons_idx].parent_table,
        row + 1, col 2, "Parent table column ",
        cur_sch->tbl[c_tbl].cons[cur_cons_idx].parent_table_columns, row + 1, col 2,
        "Delete rule ", cur_sch->tbl[c_tbl].cons[cur_cons_idx].delete_rule, row + 1
       ENDIF
       row + 1
       IF (drsd_ddl_excl_ind != 1)
        IF ((tgtsch->tbl[t_tbl].cons_drop[td_cons].ddl_excl_ind=1))
         row + 1, "Schema changes will NOT be made for constraint ", tgtsch->tbl[t_tbl].cons_drop[
         td_cons].cons_name,
         " by Install Tools ", row + 1,
         "due to exclusion that might be caused by partitioned objects. Please make schema changes recommended manually ",
         row + 1,
       ...
       ENDIF
     ENDFOR
     FOR (t_cons = 1 TO tgtsch->tbl[t_tbl].cons_cnt)
      IF ((tgtsch->tbl[t_tbl].cons[t_cons].build_ind=1))
       col 2, "Create "
       IF ((tgtsch->tbl[t_tbl].cons[t_cons].cons_type="P"))
        col + 0, "primary key "
       ELSEIF ((tgtsch->tbl[t_tbl].cons[t_cons].cons_type="U"))
        col + 0, "unique "
       ELSEIF ((tgtsch->tbl[t_tbl].cons[t_cons].cons_type="R"))
        col + 0, "foreign key "
       ENDIF
       col + 0, "constraint ", tgtsch->tbl[t_tbl].cons[t_cons].cons_name,
       " on column(s) ", row + 1
       FOR (tc = 1 TO tgtsch->tbl[t_tbl].cons[t_cons].cons_col_cnt)
         col 2, tgtsch->tbl[t_tbl].cons[t_cons].cons_col[tc].col_name, row + 1
       ENDFOR
       IF ((tgtsch->tbl[t_tbl].cons[t_cons].cons_type="R"))
        col 2, "Parent table ", tgtsch->tbl[t_tbl].cons[t_cons].parent_table,
        row + 1, col 2, "Parent table column(s) ",
        tgtsch->tbl[t_tbl].cons[t_cons].parent_table_columns, row + 1, col 2,
        "Delete rule ", tgtsch->tbl[t_tbl].cons[t_cons].delete_rule, row + 1
       ENDIF
       row + 1
      ELSEIF ((tgtsch->tbl[t_tbl].cons[t_cons].diff_status_ind=1))
       c_cons = tgtsch->tbl[t_tbl].cons[t_cons].cur_idx, col 2, "Alter constraint status of ",
       tgtsch->tbl[t_tbl].cons[t_cons].cons_name, row + 1, col 2,
       "From "
       IF ((cur_sch->tbl[c_tbl].cons[c_cons].status_ind=1))
        col + 0, "ENABLED"
       ELSE
        col + 0, "DISABLED"
       ENDIF
       col + 1, "to "
       IF ((tgtsch->tbl[t_tbl].cons[t_cons].status_ind=1))
        col + 0, "ENABLED"
       ELSE
        col + 0, "DISABLED"
       ENDIF
       row + 1
      ELSEIF ((tgtsch->tbl[t_tbl].cons[t_cons].new_ind=1)
       AND (tgtsch->tbl[t_tbl].cons[t_cons].cons_type="R"))
       col + 0, "foreign key constraint ", tgtsch->tbl[t_tbl].cons[t_cons].cons_name,
       row + 1, col + 0, " was skipped because the parent primary key constraint does not exist.",
       row + 1
      ENDIF
      ,
      IF ((tgtsch->tbl[t_tbl].cons[t_cons].build_ind=1)
       AND drsd_ddl_excl_ind != 1)
       IF ((tgtsch->tbl[t_tbl].cons[t_cons].ddl_excl_ind=1))
        row + 1, "Schema changes will NOT be made for constraint ", tgtsch->tbl[t_tbl].cons[t_cons].
        cons_name,
        " by Install Tools ", row + 1,
        "due to exclusion that might be caused by partitioned objects. Please make schema changes recommended manually ",
        row + 1,
      ...
      ENDIF
     ENDFOR
     FOR (r_ind = 1 TO tgtsch->tbl[t_tbl].ind_rename_cnt)
      IF ((tgtsch->tbl[t_tbl].ind_rename[r_ind].rename_with_drop_ind=1))
       col 2, "RENAME INDEX (WITH DROP) ", col + 0,
       tgtsch->tbl[t_tbl].ind_rename[r_ind].temp_ind_name, col + 1, "to ",
       col + 0, tgtsch->tbl[t_tbl].ind_rename[r_ind].cur_ind_name, row + 1
      ELSEIF ((tgtsch->tbl[t_tbl].ind_rename[r_ind].rename_cur_ind=1)
       AND (tgtsch->tbl[t_tbl].ind_rename[r_ind].rename_temp_ind=1))
       col 2, "RENAME INDEX ", col + 0,
       tgtsch->tbl[t_tbl].ind_rename[r_ind].cur_ind_name, col + 1, "to ",
       col + 0, tgtsch->tbl[t_tbl].ind_rename[r_ind].final_ind_name, row + 1
      ELSEIF ((((tgtsch->tbl[t_tbl].ind_rename[r_ind].drop_cur_ind=1)) OR ((tgtsch->tbl[t_tbl].
      ind_rename[r_ind].rename_cur_ind=1)
       AND (tgtsch->tbl[t_tbl].ind_rename[r_ind].drop_temp_ind=1))) )
       col 2, "DROP ", cur_ind_idx = tgtsch->tbl[t_tbl].ind_rename[r_ind].cur_ind_idx
       IF ((cur_sch->tbl[c_tbl].ind[cur_ind_idx].unique_ind=1))
        col + 0, "unique"
       ELSE
        col + 0, "nonunique"
       ENDIF
       IF ((cur_sch->tbl[c_tbl].ind[cur_ind_idx].index_type="FUNCTION-BASED NORMAL"))
        col + 0, ", function-based"
       ENDIF
       col + 1, "index ", col + 0,
       tgtsch->tbl[t_tbl].ind_rename[r_ind].cur_ind_name, col + 1, "on columns ",
       row + 1
       FOR (c_ind_col = 1 TO cur_sch->tbl[c_tbl].ind[cur_ind_idx].ind_col_cnt)
         col 2, cur_sch->tbl[c_tbl].ind[cur_ind_idx].ind_col[c_ind_col].col_name, row + 1
       ENDFOR
       col 2, "Tablespace ", tgtsch->tbl[t_tbl].ind_rename[r_ind].cur_ind_tspace_name,
       row + 1, row + 1
      ELSEIF ((tgtsch->tbl[t_tbl].ind_rename[r_ind].drop_cur_ind=0)
       AND (tgtsch->tbl[t_tbl].ind_rename[r_ind].rename_cur_ind=0)
       AND (tgtsch->tbl[t_tbl].ind_rename[r_ind].drop_temp_ind=1))
       drsd_tmp_idx = 0, drsd_tmp_idx = locateval(drsd_tmp_idx,1,tgtsch->tbl[t_tbl].ind_cnt,tgtsch->
        tbl[t_tbl].ind_rename[r_ind].temp_ind_name,tgtsch->tbl[t_tbl].ind[drsd_tmp_idx].ind_name)
       IF (drsd_tmp_idx=0)
        drsd_tmp_idx = locateval(drsd_tmp_idx,1,tgtsch->tbl[t_tbl].ind_cnt,tgtsch->tbl[t_tbl].
         ind_rename[r_ind].cur_ind_name,tgtsch->tbl[t_tbl].ind[drsd_tmp_idx].ind_name)
        IF (drsd_tmp_idx > 0)
         IF ((tgtsch->tbl[t_tbl].ind[drsd_tmp_idx].visibility_change_ind=0))
          col 2, "Drop Index ", col + 0,
          tgtsch->tbl[t_tbl].ind_rename[r_ind].temp_ind_name, col + 0, " (EXTRANEOUS)",
          row + 1
         ENDIF
        ENDIF
       ENDIF
      ...
      ,
      IF (drsd_ddl_excl_ind != 1)
       IF ((tgtsch->tbl[t_tbl].ind_rename[r_ind].ddl_excl_ind=1))
        row + 1, "Schema changes will NOT be made for index ", tgtsch->tbl[t_tbl].ind_rename[r_ind].
        cur_ind_name,
        " by Install Tools ", row + 1,
        "due to exclusion that might be caused by partitioned objects. Please make schema changes recommended manually ",
        row + 1,
      ...
      ENDIF
     ENDFOR
     FOR (t_ind = 1 TO tgtsch->tbl[t_tbl].ind_drop_cnt)
       IF (t_ind=1)
        row + 1
       ENDIF
       col 2,
       CALL print(concat("Drop Index ",tgtsch->tbl[t_tbl].ind_drop[t_ind].ind_name," (EXTRANEOUS)")),
       row + 1
       IF ((t_ind=tgtsch->tbl[t_tbl].ind_drop_cnt))
        row + 1
       ENDIF
     ENDFOR
     FOR (t_ind = 1 TO tgtsch->tbl[t_tbl].ind_cnt)
       IF ((tgtsch->tbl[t_tbl].ind[t_ind].temp_ind=0))
        IF ((tgtsch->tbl[t_tbl].ind[t_ind].build_ind=1)
         AND (tgtsch->tbl[t_tbl].ind[t_ind].temp_ind=0))
         IF ((tgtsch->tbl[t_tbl].ind[t_ind].replace_ind=1))
          col 2, "Replace "
         ELSEIF ((tgtsch->tbl[t_tbl].ind[t_ind].rebuild_ind=1))
          col 2, "Rebuild "
         ELSE
          col 2, "Create "
         ENDIF
         IF ((tgtsch->tbl[t_tbl].ind[t_ind].unique_ind=1))
          "unique"
         ELSE
          "nonunique"
         ENDIF
         IF ((tgtsch->tbl[t_tbl].ind[t_ind].index_type="FUNCTION-BASED NORMAL"))
          col + 0, ", function-based"
         ENDIF
         col + 1, "index ", tgtsch->tbl[t_tbl].ind[t_ind].ind_name,
         " on column(s) ", row + 1
         FOR (tc = 1 TO tgtsch->tbl[t_tbl].ind[t_ind].ind_col_cnt)
           col 2, tgtsch->tbl[t_tbl].ind[t_ind].ind_col[tc].col_name, row + 1
         ENDFOR
         col 2, "Tablespace ", tgtsch->tbl[t_tbl].ind[t_ind].tspace_name
         IF ((tgtsch->tbl[t_tbl].itspace_assignment_choice="CUSTOM TABLESPACE MAPPING"))
          col + 2, "(assigned from a custom tablespace mapping)"
         ENDIF
         row + 1
         IF ((tgtsch->tbl[t_tbl].ind[t_ind].part_ind=1))
          IF ((tgtsch->tbl[t_tbl].ind[t_ind].locality="GLOBAL"))
           col 2, "GLOBAL PARTITION BY ", tgtsch->tbl[t_tbl].ind[t_ind].partitioning_type,
           " on column(s) ", row + 1
           FOR (tc = 1 TO tgtsch->tbl[t_tbl].ind[t_ind].part_col_cnt)
             col 2, tgtsch->tbl[t_tbl].ind[t_ind].part_col[tc].column_name, row + 1
           ENDFOR
          ENDIF
         ENDIF
         IF ((tgtsch->tbl[t_tbl].ind[t_ind].interval != ""))
          col 2, "Interval", col 12,
          tgtsch->tbl[t_tbl].ind[t_ind].interval, row + 1
         ENDIF
         IF ((tgtsch->tbl[t_tbl].ind[t_ind].rebuild_ind=1))
          FOR (rbuild_cnt = 1 TO tgtsch->tbl[t_tbl].ind_rebuild_cnt)
            IF ((tgtsch->tbl[t_tbl].ind_rebuild[rbuild_cnt].final_ind_name=tgtsch->tbl[t_tbl].ind[
            t_ind].ind_name))
             col 2, tgtsch->tbl[t_tbl].ind_rebuild[rbuild_cnt].cons_name, col + 1,
             "constraint's associated index modified as part of index rebuild"
            ENDIF
          ENDFOR
          row + 1
         ENDIF
         IF ((dm2_install_schema->process_option != "CLIN COPY")
          AND  NOT (substring(1,2,trim(tgtsch->tbl[t_tbl].ind[t_ind].tspace_name)) IN ("D_", "I_",
         "L_"))
          AND currdbuser IN ("V500", "CDBA"))
          col 10, "WARNING: The amount of space needed for the above index will not be calculated ",
          row + 1,
          drsd_str = "", drsd_str = concat("         because the index will reside in ",trim(tgtsch->
            tbl[t_tbl].ind[t_ind].tspace_name)," tablespace."), col 10,
          drsd_str, row + 1
         ENDIF
         row + 1
        ELSEIF (validate(tgtsch->tbl[t_tbl].ind[t_ind].visibility_change_ind,- (1))=1)
         col 2, "Modify INVISIBLE index ", tgtsch->tbl[t_tbl].ind[t_ind].ind_name,
         " to be VISIBLE", row + 1
        ENDIF
        IF ((((tgtsch->tbl[t_tbl].ind[t_ind].build_ind=1)) OR (validate(tgtsch->tbl[t_tbl].ind[t_ind]
         .visibility_change_ind,- (1))=1)) )
         IF ((tgtsch->tbl[t_tbl].ind[t_ind].ddl_excl_ind=1)
          AND drsd_ddl_excl_ind != 1)
          row + 1, "Schema changes will NOT be made for index ", tgtsch->tbl[t_tbl].ind[t_ind].
          ind_name,
          " by Install Tools ", row + 1,
          "due to exclusion that might be caused by partitioned objects. Please make schema changes recommended manually ",
          row + 1,
     ...
     IF ((tgtsch->tbl[t_tbl].mview_build_ind=1))
      IF ((tgtsch->tbl[t_tbl].mview_flag=1))
       row + 1, "  Materialized View Logs will be validated/created on the following tables:", row +
       1
       FOR (dsrd_iter = 1 TO tgtsch->tbl[t_tbl].mview_log_cnt)
         col + 4, tgtsch->tbl[t_tbl].mview_logs[dsrd_iter].table_name, row + 1
       ENDFOR
      ENDIF
      row + 1
      IF ((tgtsch->tbl[t_tbl].mview_flag=1))
       "  The Materialized View will be built with the following query:", row + 1
      ELSE
       "  View creation text: ", row + 1
      ENDIF
      FOR (dsrd_iter = 1 TO tgtsch->tbl[t_tbl].mview_piece_cnt)
        col + 4, tgtsch->tbl[t_tbl].mview_piece[dsrd_iter].txt, row + 1
      ENDFOR
     ENDIF
   ENDFOR
   IF ((dm2_common1->snapshot_id=1)
    AND (dir_dropped_objects->rpt_drp_obj_ind=1))
    row + 3, "Objects Not Included in Schema Snapshot", row + 1,
    "-------------------------------------------------------", row + 2,
    "The following objects were not included because they were dropped",
    row + 1, "through the Conditional Build or Obsolete Objects Processes.", row + 2,
    col 1, "Table Name", col 32,
    "Object Name", col 63, "Object Type",
    col 75, "Reason", row + 2
    FOR (iter = 1 TO dir_dropped_objects->obj_cnt)
      col 1, dir_dropped_objects->obj[iter].table_name, col 32,
      dir_dropped_objects->obj[iter].name, col 63, dir_dropped_objects->obj[iter].type,
      col 75, dir_dropped_objects->obj[iter].reason, row + 1
    ENDFOR
   ENDIF
  WITH nocounter, format = variable, formfeed = none,
   maxcol = 1000
 ...
 ENDIF
 IF ((((dm2_common1->snapshot_id=1)
  AND (dm2_install_schema->process_option IN ("PREVIEW", "ADMIN UPGRADE", "CLIN UPGRADE", "CHECK",
 "CLIN COPY"))) OR ((dm2_common1->snapshot_id=2)
  AND (dm2_install_schema->process_option IN ("ADMIN CREATE", "ADMIN UPGRADE", "NONPROD CREATE",
 "CLIN UPGRADE")))) )
  IF (dir_silmode_requested_ind=1)
   CALL dir_add_silmode_entry("Schema Difference Report",drsd_diff_file)
   SET dm_err->eproc = concat("Silent Mode:  Schema Difference Report was bypassed.  Report exits",
    " in CCLUSERDIR under file name: ",drsd_diff_file)
   CALL disp_msg("",dm_err->logfile,0)
  ELSE
   IF (dm2_disp_file(drsd_diff_file,"Schema Difference Report")=0)
    GO TO end_program
   ENDIF
  ENDIF
  IF ((dir_ui_misc->dm_process_event_id > 0))
   IF (drr_assign_file_to_installs(drsd_event_dt_name,drsd_diff_file,dir_ui_misc->dm_process_event_id
    )=0)
    GO TO end_program
   ENDIF
  ENDIF
...
#end_program
 IF ((dm_err->err_ind=1))
  SET dm_err->eproc = "EXITING DM2_RPT_SCHEMA_DIFF WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_RPT_SCHEMA_DIFF COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg(drsd_logfile)
END GO
