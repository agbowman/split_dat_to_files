CREATE PROGRAM cp_get_node_codesets
 PROMPT
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE getcodevalues(null) = null WITH protect
 DECLARE concept_cd_codeset = i4 WITH constant(4003132), protect
 DECLARE node_type_cd_codeset = i4 WITH constant(4003131), protect
 ...
 SUBROUTINE getcodevalues(null)
   CALL log_message("In prepareRequest()",log_level_debug)
   ...
   SELECT INTO "NL:"
    ...
    WHERE cv.active_ind=1
     ...
     AND cv.code_set IN (concept_cd_codeset, node_type_cd_codeset)
    ORDER BY cv.code_set
    HEAD REPORT
     conceptcdcnt = 0, nodetypecnt = 0
    DETAIL
     IF (cv.code_set=concept_cd_codeset)
      conceptcdcnt += 1
      IF (conceptcdcnt > size(nodecodesets->concept_list,5))
       stat = alterlist(nodecodesets->concept_list,(conceptcdcnt+ 10))
      ENDIF
      nodecodesets->concept_list[conceptcdcnt].concept_cd = cv.code_value, nodecodesets->
      concept_list[conceptcdcnt].concept_cd_disp = trim(cv.display,3)
     ELSEIF (cv.code_set=node_type_cd_codeset)
      nodetypecnt += 1
      IF (nodetypecnt > size(nodecodesets->node_type_list,5))
       stat = alterlist(nodecodesets->node_type_list,(nodetypecnt+ 10))
      ENDIF
      nodecodesets->node_type_list[nodetypecnt].node_type_cd = cv.code_value, nodecodesets->
      node_type_list[nodetypecnt].node_type_cd_disp = trim(cv.display,3)
     ENDIF
    FOOT REPORT
     stat = alterlist(nodecodesets->node_type_list,nodetypecnt), stat = alterlist(nodecodesets->
      concept_list,conceptcdcnt)
    WITH nocounter
   ;end select
   CALL log_message(build("Exit getCodeValues(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (validate(debug_ind,0)=1)
  CALL echorecord(nodecodesets)
 ENDIF
 CALL putjsonrecordtofile(nodecodesets)
