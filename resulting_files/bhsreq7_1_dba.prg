CREATE PROGRAM bhsreq7_1:dba
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   IF (((oa.action_type_cd IN (activatestudentorder_cd, order_cd, modify_cd, cancelreorder_cd,
   canceldc_cd,
   cancel_cd, activate_cd)) OR (oa.action_type_cd=order_cd
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(request->printer_name)
   ...
   FOOT PAGE
    ...
    "  ", curtime, "{endb}",
    row + 1, "{cpi/12}{pos/300/687}", "{b/8}Patient: ",
    "{pos/345/687}", orders->name, "{pos/465/687}",
    "{b/10}Location: ", orders->location, row + 1,
    "{pos/300/698}", "CMRN #: ", orders->cmrn,
    "{pos/410/698}", "DOB: ", orders->dob,
    "{pos/490/698}", "ADM: ", orders->admit_dt,
    row + 1, "{pos/300/709}", "{b/5}MRN # ",
    orders->mrn, "{pos/410/709}", "Age: ",
    orders->age, "{pos/490/709}", "Sex: ",
    orders->sex, row + 1, "{pos/300/720}",
    "Attending MD: ", orders->attending, row + 1,
    "{pos/490/731}", "Page # ", curpage,
    row + 1, "{cpi/10}{pos/30/693}{b}", orders->facility,
    row + 1, "{cpi/10}{pos/60/708}{b}", "Laboratory Order Requisition",
    "{font/8}{cpi/12}{pos/50/720}", "BHSREQ7_1"
