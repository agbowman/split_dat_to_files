CREATE PROGRAM dm_ins_cbo_stat:dba
 ...
 SET readme_data->message = "**** FAILED - Starting script dm_ins_cbo_stat ****"
 IF (check_logfile("DM_INS_CBO_STAT",".log","DM_INS_CBO_STAT LOG FILE")=0)
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
 ...
 IF (check_error(dm_err->eproc) > 0)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Update DM_INFO table."
 CALL disp_msg(" ",dm_err->logfile,0)
 INSERT  FROM dm_info dm,
  ...
  SET dm.info_domain = requestin->list_0[d.seq].info_domain, dm.info_name = requestin->list_0[d.seq].
   ...
   dm.info_number = cnvtreal(requestin->list_0[d.seq].info_number), dm.updt_task = reqinfo->updt_task,
   dm.updt_applctx = 0
  PLAN (d
 ...
 UPDATE  FROM dm_info dm,
   (dummyt d  WITH seq = value(size(requestin->list_0,5)))
  SET dm.info_char = requestin->list_0[d.seq].info_char, dm.info_number = cnvtreal(requestin->list_0[
    d.seq].info_number), dm.updt_task = reqinfo->updt_task
  PLAN (d
   WHERE (requestin->list_0[d.seq].exists_ind=cnvtstring(1)))
   JOIN (dm
   WHERE (dm.info_domain=requestin->list_0[d.seq].info_domain)
    AND (dm.info_name=requestin->list_0[d.seq].info_name)
    AND dm.updt_applctx=0)
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc) > 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Check if all rows were inserted on dm_info table."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dshc_cnt = 0
 SELECT INTO "nl:"
  dm.info_domain, dm.info_name, dm.info_number
 ...
 IF (dshc_cnt=size(requestin->list_0,5))
  ...
  SET readme_data->message = "Success - all dm_info rows inserted successfully."
  COMMIT
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = "Failed - not all dm_info rows inserted successfully."
 ENDIF
#exit_script
 IF ((dm_err->err_ind=1))
 ...
 ELSE
  ...
  CALL echo("##################################################")
 ENDIF
 SET dm_err->eproc = "DM_INS_CBO_STAT complete."
 CALL final_disp_msg("DM_INS_CBO_STAT")
END GO
