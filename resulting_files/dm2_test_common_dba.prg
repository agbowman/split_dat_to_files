CREATE PROGRAM dm2_test_common:dba
 SET dm2_debug_flag = 1
 ...
 IF (currdb="SQLSRV")
  RDB drop view dm2_tst_cr_view
  END ;Rdb
  RDB create view dm2_tst_cr_view as select * from admin . admin . cdba . dm2_user_tables
  END ;Rdb
  COMMIT
  EXECUTE oragen3 "dm2_tst_cr_view"
 ELSE
  IF (dm2_push_cmd("rdb create nickname v500.dm2_tst_cr_view for admin.cdba.dm2_user_tables go",1)=0)
   ROLLBACK
   GO TO exit_program
  ELSE
   COMMIT
 ...
 ENDIF
 SELECT INTO "nl:"
  FROM dm2_tst_cr_view
  WITH nocounter
 ;end select
 IF (check_error("Select on view")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dm2_set_autocommit(0)=0)
 ...
 IF (currdb="SQLSRV")
  RDB drop view dm2_tst_cr_view
  END ;Rdb
  DROP VIEW dm2_tst_cr_view
  COMMIT
 ELSE
  IF (dm2_push_cmd("rdb drop nickname v500.dm2_tst_cr_view go",1)=0)
   ROLLBACK
   GO TO exit_program
  ELSE
   COMMIT
