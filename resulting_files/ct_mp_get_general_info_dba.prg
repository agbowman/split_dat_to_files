CREATE PROGRAM ct_mp_get_general_info:dba
 PROMPT
 ...
 FREE RECORD request
 RECORD PUBLIC::"reply"(
   1 "prescreen_type_exists" = i2
   1 "org_security_ind" = i2
   1 "enroll_status_ind" = i2
   1 "prot_list"[*]
     2 "protocolId" = f8
     2 "amendmentId" = f8
     2 "currentDBDate" = vc
     2 "prescreen_type_flag" = i2
     2 "prot_info"
       3 "prot_mnemonic" = vc
       3 "amendment_nbr" = vc
       3 "prot_activated_date" = vc
       3 "prot_activated_date_db" = dq8
       3 "prot_status_cd" = f8
       3 "prot_status_disp" = vc
       3 "prot_status_mean" = vc
       3 "principal_investigator" = vc
       3 "prot_type_cd" = f8
       3 "prot_type_meaning" = vc
       3 "prot_stratification_cd" = f8
       3 "prot_stratification_meaning" = vc
       3 "prot_study_type_cd" = f8
       3 "prot_study_type_meaning" = vc
       3 "registry_ind" = i2
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 DECLARE currentdate = vc WITH protect, noconstant("")
 DECLARE stat = i4 WITH protect, noconstant(0)
 DECLARE prescreen_type_col_exists = i2 WITH protect, noconstant(0)
 ...
 SET prescreen_type_col_exists = checkdic("PROT_MASTER.PRESCREEN_TYPE_FLAG","A",0)
 SET public::"reply"->"enroll_status_ind" = checkdic("PT_PROT_REG.STATUS_ENUM","A",0)
 SET public::"reply"->"prescreen_type_exists" = prescreen_type_col_exists
 ...
 IF (protcount > 0)
  ...
  SET currentdate = format(curdate,"@SHORTDATE4YR")
 ...
 ENDIF
 SET public::"reply"->"org_security_ind" = org_security_ind
 SUBROUTINE getprotinfo(null)
   CALL log_message("In GetProtInfo()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(cnvtdatetime(sysdate)), private
   SELECT
    IF (prescreen_type_col_exists=2)
     prescreen_type_flag = pm.prescreen_type_flag
    ELSE
    ...
    DETAIL
     ...
     "reply"->prot_list[d.seq]."currentDBDate" = currentdate, public::"reply"->prot_list[d.seq].
     "prot_info"."prot_status_disp" = uar_get_code_display(pm.prot_status_cd),
     public::"reply"->prot_list[d.seq]."prot_info"."prot_status_mean" = uar_get_code_meaning(pm
      .prot_status_cd), public::"reply"->prot_list[d.seq]."prot_info"."prot_study_type_cd" = pm
     .prot_type_cd, public::"reply"->prot_list[d.seq]."prot_info"."prot_study_type_meaning" =
     uar_get_code_meaning(pm.prot_type_cd),
     public::"reply"->prot_list[d.seq]."prescreen_type_flag" = prescreen_type_flag
    WITH nocounter
   ;end select
   SELECT
    ...
    DETAIL
     public::"reply"->prot_list[d.seq]."prot_info"."prot_type_cd" = pa.participation_type_cd,
     public::"reply"->prot_list[d.seq]."prot_info"."prot_stratification_cd" = pa
     .enroll_stratification_type_cd, public::"reply"->prot_list[d.seq]."prot_info".
     "prot_stratification_meaning" = uar_get_code_meaning(pa.enroll_stratification_type_cd)
     IF (cfg.config_value_cd=yes_cd)
      public::"reply"->prot_list[d.seq]."prot_info"."registry_ind" = 1
     ELSE
      public::"reply"->prot_list[d.seq]."prot_info"."registry_ind" = 0
     ENDIF
     IF (pa.amendment_dt_tm < cnvtdatetime("31-DEC-2100 00:00:00.00"))
      public::"reply"->prot_list[d.seq]."prot_info"."prot_activated_date" = format(pa.amendment_dt_tm,
       "@SHORTDATE4YR"), public::"reply"->prot_list[d.seq]."prot_info"."prot_activated_date_db" = pa
      .amendment_dt_tm
     ENDIF
     ...
     IF (pa.revision_ind=1)
      public::"reply"->prot_list[d.seq]."prot_info"."amendment_nbr" = build2(public::"reply"->
       prot_list[d.seq]."prot_info"."amendment_nbr"," - ",revisionlabel," ",pa.revision_nbr_txt)
     ENDIF
    WITH nocounter
   ;end select
   SELECT
