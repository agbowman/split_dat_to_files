CREATE PROGRAM dm2_aload_dm_ts_precedence
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_dtp",".log","dm2_aload_dm_ts_precedence")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_dm_ts_precedence"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dadtp_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dadtp_file = vc WITH protect, noconstant( $2)
 DECLARE dadtp_expected_rows = i4 WITH protect, noconstant( $3)
 DECLARE dadtp_batch = i4 WITH protect, noconstant(0)
 DECLARE dadtp_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dadtp_pkg,dadtp_file_loc_ret,dadtp_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dadtp_batch=0)
  SET dadtp_batch = 5000
 ENDIF
 IF (dadtp_expected_rows > 0)
  EXECUTE dm2_dbimport concat(dadtp_file_loc_ret,dadtp_file), "dm2_aload_adoc_dtp", dadtp_batch
  CALL echo(concat(dadtp_file_loc_ret,dadtp_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_dm_ts_precedence completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_dm_ts_precedence")
END GO
