CREATE PROGRAM ce_inv_time_result_upd:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 SET error_code = 0
 UPDATE  FROM ce_inv_time_result t,
  ...
  SET t.event_id = evaluate2(
    ...
    ENDIF
    ), t.start_dt_tm = evaluate2(
    IF ((request->lst[d.seq].start_dt_tm_ind=1)) null
    ELSE cnvtdatetimeutc(request->lst[d.seq].start_dt_tm)
    ENDIF
    ),
   t.end_dt_tm = evaluate2(
    IF ((request->lst[d.seq].end_dt_tm_ind=1)) null
    ELSE cnvtdatetimeutc(request->lst[d.seq].end_dt_tm)
    ENDIF
  ...
  PLAN (d)
   ...
   WHERE (t.event_id=request->lst[d.seq].event_id)
    AND (t.item_id=request->lst[d.seq].item_id)
    AND (t.start_dt_tm=
   IF ((request->lst[d.seq].start_dt_tm_ind=1)) null
   ELSE cnvtdatetimeutc(request->lst[d.seq].start_dt_tm)
   ENDIF
