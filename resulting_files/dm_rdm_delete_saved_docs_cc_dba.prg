CREATE PROGRAM dm_rdm_delete_saved_docs_cc:dba
 ...
 SUBROUTINE sbr_parallel_debug_echo(ms_msg)
   ...
   ENDIF
 END ;Subroutine
 DECLARE sbr_get_min_max(mf_min_range_id=f8(ref),mf_max_range_id=f8(ref)) = null
 DECLARE sbr_handle_cc_error(null) = i2
 DECLARE sbr_upd_id_evaluated(mf_max_id=f8,ms_info_domain_nm=vc,ms_max_name=vc) = i2
 ...
 ENDIF
 SUBROUTINE sbr_get_min_max(mf_min_range_id,mf_max_range_id)
 ...
 SUBROUTINE sbr_handle_cc_error(null)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE sbr_upd_id_evaluated(mf_max_id,ms_info_domain_nm,ms_max_name)
   ...
   DECLARE errmsg = vc WITH protect, noconstant("")
   SET mf_runtime = datetimediff(cnvtdatetime(curdate,curtime3),dq_starttime,5)
   CALL sbr_parallel_debug_echo(concat("Runtime: ",cnvtstring(mf_runtime)))
   UPDATE  FROM dm_info di
    SET di.info_number = mf_max_id, di.info_date = cnvtdatetime(curdate,curtime3), di.updt_cnt = (di
     .updt_cnt+ 1),
   ...
   UPDATE  FROM dm_parallel_readme_stats dprs
    SET dprs.total_elapsed_tm = (dprs.total_elapsed_tm+ mf_runtime), dprs.min_batch_tm =
     ...
     mf_runtime** 2)), dprs.updt_dt_tm = cnvtdatetime(curdate,curtime3),
 ...
 SET ms_child_c_info_domain_nm = "DM_RDM_DELETE_NON_VIEWABLE_SAVED_DOCS"
 ...
 CALL echo("")
 INSERT  FROM temp_task_activity temp
  (temp.task_id)(SELECT
   ta.task_id
