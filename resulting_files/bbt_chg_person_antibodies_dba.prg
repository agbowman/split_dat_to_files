CREATE PROGRAM bbt_chg_person_antibodies:dba
 DECLARE script_name = c40 WITH public, constant("BBT_CHG_PERSON_ANTIBODIES")
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 FOR (i = 1 TO size(request->antibodylist,5))
   SELECT INTO "nl:"
    pa.person_antibody_id
    FROM person_antibody pa
    WHERE (pa.person_antibody_id=request->antibodylist[i].person_antibody_id)
     AND (pa.updt_cnt=request->antibodylist[i].updt_cnt)
    WITH nocounter, forupdate(pa)
   ;end select
   IF (curqual=0)
    CALL fill_out_status_data("F","PERSON_ANTIBODY",concat("[",
      "FAILED to find PERSON_ANTIBODY--person_antibody_id = ",trim(cnvtstring(request->antibodylist[i
        ].person_antibody_id,32,2)),", ","updt_cnt=",
      trim(cnvtstring(request->antibodylist[i].updt_cnt,2,0)),".","]"))
    GO TO exit_script
   ENDIF
   UPDATE  FROM person_antibody pa
    SET pa.active_ind = 0, pa.updt_cnt = (pa.updt_cnt+ 1), pa.updt_dt_tm = cnvtdatetime(curdate,
     ...
     .removal_reason_cd = request->antibodylist[i].removal_reason_cd,
     pa.removal_notes = request->antibodylist[i].removal_comment, pa.updt_id = reqinfo->updt_id, pa
   ...
   IF (curqual=0)
    CALL fill_out_status_data("F","PERSON_ANTIBODY",concat("[",
      "Failed to update the active_ind for the ",trim(cnvtstring(request->antibodylist[i].
        person_antibody_id,32,0)),".","]"))
    GO TO exit_script
   ENDIF
 ENDFOR
