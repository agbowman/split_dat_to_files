CREATE PROGRAM acm_chg_encntr_info_hist:dba
 ...
 RECORD temp_record(
   1 encntr_info_hist_qual[*]
     2 encntr_id = f8
     2 encntr_info_hist_id = f8
     2 encntr_info_id = f8
 ...
 DECLARE tracking_bit = i4 WITH noconstant(255)
 DECLARE updt_cnt = i4 WITH noconstant(0)
 DECLARE index = i4 WITH protect, noconstant(0)
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->encntr_info_qual[xref->chg[index].idx].status = 0
 ENDFOR
 SET stat = alterlist(temp_record->encntr_info_hist_qual,xref->chg_cnt)
 SET eih_qual_cntr = 0
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(xref->chg_cnt))
  PLAN (d)
  DETAIL
   item_idx = xref->chg[d.seq].idx, chg_str = acm_request->encntr_info_qual[item_idx].chg_str, chgbit
    = 0
   IF (findstring("INFO_TYPE_CD,",chg_str) != 0)
    chgbit = bor(1,chgbit)
   ENDIF
   IF (findstring("INFO_SUB_TYPE_CD,",chg_str) != 0)
    chgbit = bor(2,chgbit)
   ENDIF
   IF (findstring("CHARTABLE_IND,",chg_str) != 0)
    chgbit = bor(4,chgbit)
   ENDIF
   IF (findstring("VALUE_DT_TM,",chg_str) != 0)
    chgbit = bor(8,chgbit)
   ENDIF
   IF (findstring("VALUE_NUMERIC,",chg_str) != 0)
    chgbit = bor(16,chgbit)
   ENDIF
   IF (findstring("VALUE_LONG_TEXT_ID,",chg_str) != 0)
   ...
   ENDIF
   IF (chgbit > 0)
    eih_qual_cntr = (eih_qual_cntr+ 1), temp_record->encntr_info_hist_qual[eih_qual_cntr].change_bit
     = chgbit, temp_record->encntr_info_hist_qual[eih_qual_cntr].item_idx = item_idx,
    temp_record->encntr_info_hist_qual[eih_qual_cntr].encntr_id = acm_request->encntr_info_qual[
    item_idx].encntr_id, temp_record->encntr_info_hist_qual[eih_qual_cntr].encntr_info_id =
    acm_request->encntr_info_qual[item_idx].encntr_info_id, temp_record->encntr_info_hist_qual[
    eih_qual_cntr].pm_hist_tracking_id = acm_request->encntr_info_qual[item_idx].pm_hist_tracking_id,
    temp_record->encntr_info_hist_qual[eih_qual_cntr].transaction_dt_tm = acm_request->
    encntr_info_qual[item_idx].transaction_dt_tm, temp_record->encntr_info_hist_qual[eih_qual_cntr].
    info_type_cd = acm_request->encntr_info_qual[item_idx].info_type_cd, temp_record->
    encntr_info_hist_qual[eih_qual_cntr].info_sub_type_cd = acm_request->encntr_info_qual[item_idx].
    info_sub_type_cd,
    temp_record->encntr_info_hist_qual[eih_qual_cntr].chartable_ind = acm_request->encntr_info_qual[
    item_idx].chartable_ind, temp_record->encntr_info_hist_qual[eih_qual_cntr].value_dt_tm =
    acm_request->encntr_info_qual[item_idx].value_dt_tm, temp_record->encntr_info_hist_qual[
    eih_qual_cntr].value_numeric = acm_request->encntr_info_qual[item_idx].value_numeric,
    temp_record->encntr_info_hist_qual[eih_qual_cntr].value_long_text_id = acm_request->
    encntr_info_qual[item_idx].long_text_id, temp_record->encntr_info_hist_qual[eih_qual_cntr].
    internal_seq = acm_request->encntr_info_qual[item_idx].internal_seq, temp_record->
    encntr_info_hist_qual[eih_qual_cntr].priority_seq = acm_request->encntr_info_qual[item_idx].
    priority_seq
   ELSE
    reply->encntr_info_qual[item_idx].status = 1
   ENDIF
  WITH nocounter
 ;end select
 SET stat = alterlist(temp_record->encntr_info_hist_qual,eih_qual_cntr)
 IF (eih_qual_cntr > 0)
  INSERT  FROM encntr_info_hist eih,
    (dummyt d  WITH seq = value(eih_qual_cntr))
   SET eih.encntr_id = temp_record->encntr_info_hist_qual[d.seq].encntr_id, eih.encntr_info_hist_id
     = seq(encounter_seq,nextval), eih.encntr_info_id = temp_record->encntr_info_hist_qual[d.seq].
    encntr_info_id,
    eih.pm_hist_tracking_id = temp_record->encntr_info_hist_qual[d.seq].pm_hist_tracking_id, eih
    .transaction_dt_tm = cnvtdatetime(temp_record->encntr_info_hist_qual[d.seq].transaction_dt_tm),
    eih.info_type_cd = temp_record->encntr_info_hist_qual[d.seq].info_type_cd,
    eih.info_sub_type_cd = temp_record->encntr_info_hist_qual[d.seq].info_sub_type_cd, eih
    .chartable_ind = temp_record->encntr_info_hist_qual[d.seq].chartable_ind, eih.value_dt_tm =
    cnvtdatetime(temp_record->encntr_info_hist_qual[d.seq].value_dt_tm),
    eih.value_numeric = temp_record->encntr_info_hist_qual[d.seq].value_numeric, eih
    .value_long_text_id = temp_record->encntr_info_hist_qual[d.seq].value_long_text_id, eih
    .internal_seq = temp_record->encntr_info_hist_qual[d.seq].internal_seq,
    eih.priority_seq = temp_record->encntr_info_hist_qual[d.seq].priority_seq, eih.tracking_bit =
    tracking_bit, eih.change_bit = temp_record->encntr_info_hist_qual[d.seq].change_bit,
    eih.active_ind = 1, eih.active_status_cd = reqdata->active_status_cd, eih.active_status_dt_tm =
    cnvtdatetime(curdate,curtime3),
    eih.active_status_prsnl_id = reqinfo->updt_id, eih.updt_dt_tm = cnvtdatetime(curdate,curtime3),
    eih.updt_id = reqinfo->updt_id,
    eih.updt_applctx = reqinfo->updt_applctx, eih.updt_task = reqinfo->updt_task, eih.updt_cnt =
    updt_cnt
   PLAN (d)
    JOIN (eih)
   WITH nocounter, status(reply->encntr_info_qual[temp_record->encntr_info_hist_qual[d.seq].item_idx]
    .status)
  ;end insert
 ENDIF
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->encntr_info_qual[xref->chg[index].idx].status=0))
    SET failed = insert_error
    SET table_name = "ENCNTR_INFO_HIST"
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
