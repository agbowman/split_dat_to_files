CREATE PROGRAM bed_imp_defsched:dba
 FREE SET reply
 RECORD reply(
   1 error_msg = vc
   1 status_data
 ...
 SUBROUTINE new_template(t)
  IF (dup_template=0)
   ...
   ENDIF
   IF (mod(begtime,5) > 0)
    SET begtime = (floor((begtime/ 10)) * 10)
   ENDIF
   ...
   IF (endtime=0)
    ...
    ENDIF
   ENDIF
   IF (((endtime > 2355) OR (endtime < 0005)) )
    SET endtime = 2400
   ENDIF
   IF (mod(endtime,5) > 0)
    SET endtime = (floor((endtime/ 10)) * 10)
   ENDIF
   IF ((requestin->list_0[x].apply_begin_date > " "))
    SET fdate = trim(concat(requestin->list_0[x].apply_begin_date,"    "))
    SET fstat = format_date(x)
    IF (substring(7,1,fdate)="0")
     SET temp_dt1 = cnvtdate2(fdate,"MM/DD/YY")
    ELSE
     SET temp_dt1 = cnvtdate2(fdate,"MM/DD/YYYY")
    ENDIF
   ENDIF
   IF ((requestin->list_0[x].apply_end_date > " "))
    ...
    SET fstat = format_date(x)
    IF (substring(7,1,fdate)="0")
     SET temp_dt2 = cnvtdate2(fdate,"MM/DD/YY")
    ELSE
     SET temp_dt2 = cnvtdate2(fdate,"MM/DD/YYYY")
    ENDIF
   ENDIF
