CREATE PROGRAM bhs_athn_add_clin_doc_v2
 RECORD orequest(
   1 ensure_type = i2
 ...
 SET orequest->clin_event[1].clinsig_updt_dt_tm = cnvtdatetime(sysdate)
 ...
 SET orequest->clin_event[1].valid_from_dt_tm = cnvtdatetime(sysdate)
 IF (( $14=0))
  SET orequest->clin_event[1].event_tag = uar_get_code_display(cnvtreal( $4))
 ELSE
  SET orequest->clin_event[1].event_tag = "In Progress"
 ENDIF
 ...
 SET orequest->clin_event[1].updt_dt_tm = cnvtdatetime(sysdate)
 ...
 DECLARE format_cd = f8
 SET format_cd = uar_get_code_by("MEANING",23, $13)
 IF (format_cd=564029427)
 ...
 SET orequest->clin_event[1].valid_from_dt_tm = cnvtdatetime(sysdate)
 SET orequest->clin_event[1].event_title_text = ""
 IF (( $14=0))
  SET orequest->clin_event[1].event_tag = uar_get_code_display(cnvtreal( $4))
 ELSE
  SET orequest->clin_event[1].event_tag = "In Progress"
 ENDIF
 SET orequest->clin_event[1].event_end_dt_tm = cnvtdatetime(sysdate)
 SET orequest->clin_event[1].record_status_cd = active_cd
 IF (( $14=0))
  SET orequest->clin_event[1].result_status_cd = authverified_cd
 ELSEIF (( $14=1))
 ...
 SET orequest->clin_event[1].blob_result[1].format_cd = format_cd
