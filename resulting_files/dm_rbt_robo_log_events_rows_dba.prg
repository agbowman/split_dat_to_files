CREATE PROGRAM dm_rbt_robo_log_events_rows:dba
 ...
 SET reply->table_name = "ROBOTICS_LOG_EVENTS"
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(re.robotics_service_resource_cd)
    FROM robotics_log_events re
    WHERE re.robotics_service_resource_cd > 0
     AND re.sequence > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   seqval = max(re.robotics_service_resource_cd)
   FROM robotics_log_events re
   DETAIL
   ...
   AND rowsleft > 0)
    SELECT INTO "nl:"
     ...
     WHERE parser(sbr_getrowidnotexists(
       "re.robotics_service_resource_cd between curMinID and curMaxID","re"))
      AND re.sequence > 0
      AND re.updt_dt_tm < cnvtdatetime((curdate - days_to_keep),curtime3)
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,50)=1)
       stat = alterlist(reply->rows,(rows+ 49))
      ENDIF
      reply->rows[rows].row_id = re.rowid
     WITH nocounter, maxqual(re,value(rowsleft))
    ;end select
    SET reply->err_code = error(reply->err_msg,1)
    IF ((reply->err_code > 0))
     SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERROR",
      "Failed in collecting rows: %1","s",nullterm(reply->err_msg))
     SET reply->status_data.status = "F"
     GO TO exit_script
    ENDIF
