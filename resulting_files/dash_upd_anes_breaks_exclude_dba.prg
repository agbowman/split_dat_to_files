CREATE PROGRAM dash_upd_anes_breaks_exclude:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Provider ID:" = 0,
  "Excluded:" = 0
  WITH outdev, providerid, newexcluded
 ...
 SET log_program_name = "dash_upd_anes_breaks_exclude"
 ...
 INSERT  FROM dash_break_hist
  ...
  exclude_ind, updt_cnt)(SELECT
   ...
   FROM dash_break db
   WHERE (db.provider_id= $PROVIDERID))
 ;end insert
 ...
 UPDATE  FROM dash_break db
  SET db.exclude_ind =  $NEWEXCLUDED, db.updt_cnt = (db.updt_cnt+ 1), db.status_dt_tm = cnvtdatetime(
    curdate,curtime)
  WHERE (db.provider_id= $PROVIDERID)
 ;end update
 IF (error(err_msg,0) > 0)
...
#exit_script
 IF (failed=1)
  ...
  CALL log_message(concat("Error updating excluded: ",err_msg),log_level_debug)
 ELSE
