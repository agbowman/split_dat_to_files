CREATE PROGRAM bhs_rptamb:dba
 ...
 DECLARE f_amb_unit = f8 WITH protect
 SET becont = 0
 ...
 DECLARE mf_preferred_pharmacy_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",355,
   "DEFAULTPATIENTPREFERREDPHARMACY"))
 ...
 DECLARE mf_email_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",43,"EXTERNALSECURE"))
 DECLARE mf_tobacco_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",14003,"SHXTOBACCOUSE"
   ))
 DECLARE ml_app_id = i4 WITH protect, constant(3202004)
 DECLARE ml_task_id = i4 WITH protect, constant(3202004)
 DECLARE ml_request_id = i4 WITH protect, constant(3202501)
 DECLARE ml_pharm_count = i4 WITH protect, noconstant(0)
 DECLARE ml_pi_pharm = i4 WITH protect, noconstant(0)
 DECLARE ml_od_pharm_index = i4 WITH protect, noconstant(0)
 DECLARE ml_index = i4 WITH protect, noconstant(0)
 DECLARE ml_pat_count = i4 WITH protect, noconstant(0)
 DECLARE info = vc WITH protect, noconstant("")
 SET operations = 0
 ...
 IF (validate(request->batch_selection))
  ...
  SET stop_dt = trim(format(cnvtlookahead("8,D",sysdate),"dd-mmm-yyyy 23:59:59;;d"),3)
  SET operations = 1
 ...
 ELSE
  SET f_amb_unit =  $UNIT
  CALL echo(build("f_amb_unit_menu = ",f_amb_unit))
  SET start_dt =  $BEG_DT
  SET stop_dt =  $END_DT
 ...
 SET select_starttime = cnvtdatetime(sysdate)
 SELECT INTO "nl:"
  ...
  PLAN (sa
   WHERE sa.beg_dt_tm BETWEEN cnvtdatetime(start_dt) AND (cnvtdatetime(stop_dt)+ 0)
    AND sa.appt_location_cd=f_amb_unit
  ...
  ORDER BY sa1_resource_disp, sa.sch_event_id, sa.updt_dt_tm DESC
  HEAD REPORT
  ...
  WITH nocounter
 ;end select
 SET select_endtime = cnvtdatetime(sysdate)
  ...
  SELECT INTO "nl:"
   ...
   HEAD d1.seq
    ...
    seq[d1.seq].f_sex_cd = p.sex_cd, dlrec->seq[d1.seq].birth_dt_tm = format(p.birth_dt_tm,
     "MM/DD/YYYY ;;q"),
    dlrec->seq[d1.seq].pcpdoc_name = pr.name_full_formatted, dlrec->seq[d1.seq].s_tobacco =
    "No tobacco usage info found"
   WITH nocounter, outerjoin = d2
  ;end select
  SELECT INTO "nl:"
   sa_resource_disp = uar_get_code_display(sa.resource_cd)
   FROM sch_appt sa,
    phone ph,
    phone ph2,
    address adr
   PLAN (sa
    WHERE expand(ml_pat_count,1,size(dlrec->seq,5),sa.sch_event_id,dlrec->seq[ml_pat_count].
     sch_event_id)
     AND sa.beg_dt_tm BETWEEN cnvtdatetime(start_dt) AND cnvtdatetime(stop_dt)
     AND sa.appt_location_cd=f_amb_unit
     AND sa.encntr_id > 0
     AND sa.sch_state_cd IN (rescheduled_sch, pending_sch, scheduled_sch, appointment_sch,
    finalized_sch,
    confirmed_sch))
    JOIN (ph
    WHERE (ph.parent_entity_id= Outerjoin(sa.person_id))
    ...
    JOIN (ph2
    WHERE (ph2.parent_entity_id= Outerjoin(sa.person_id))
     AND (ph2.phone_type_cd= Outerjoin(mf_email_cd))
    ...
    JOIN (adr
    WHERE (adr.parent_entity_id= Outerjoin(sa.person_id))
     AND (adr.address_type_cd= Outerjoin(mf_home_address_cd))
     AND (adr.active_ind= Outerjoin(1))
     AND (adr.end_effective_dt_tm> Outerjoin(cnvtdatetime(sysdate)))
     AND (adr.address_type_seq= Outerjoin(1)) )
   ORDER BY sa_resource_disp, sa.sch_event_id
   HEAD REPORT
    ml_index = 0, ml_pat_count = 0
   HEAD sa.sch_event_id
    ml_index = locateval(ml_pat_count,1,size(dlrec->seq,5),sa.sch_event_id,dlrec->seq[ml_pat_count].
     sch_event_id), dlrec->seq[ml_index].s_address = concat(trim(adr.street_addr,3)," ",trim(adr.city,
      3)," ",trim(adr.state,3),
     " ",trim(adr.zipcode,3)), dlrec->seq[ml_index].s_phone_number = trim(ph.phone_num,3)
    IF (size(trim(ph2.phone_num),3) > 0)
     dlrec->seq[ml_index].s_email = trim(ph2.phone_num,3)
    ELSE
     dlrec->seq[ml_index].s_email = "No e-mail address found"
    ENDIF
   WITH nocounter, expand = 1
  ;end select
  FOR (ml_pat_count = 1 TO size(dlrec->seq,5))
    SET pharmrequest->ids[1].id = ""
    SET stat = alterlist(m_pharmrec->pharmacies,0)
    SELECT INTO "nl:"
     FROM person_info pi,
      long_text lt
     PLAN (pi
      WHERE (pi.person_id=dlrec->seq[ml_pat_count].person_id))
      JOIN (lt
      WHERE lt.long_text_id=pi.long_text_id)
     ORDER BY pi.updt_dt_tm DESC
     HEAD REPORT
      ml_pharm_count = 0
     DETAIL
      IF (lt.long_text_id > 0)
       IF (size(trim(pharmrequest->ids[1].id,3))=0)
        ml_pharm_count += 1, stat = alterlist(m_pharmrec->pharmacies,ml_pharm_count), m_pharmrec->
        pharmacies[ml_pharm_count].f_update_time = pi.updt_dt_tm,
        m_pharmrec->pharmacies[ml_pharm_count].n_active_ind = pi.active_ind, m_pharmrec->pharmacies[
        ml_pharm_count].s_id = trim(lt.long_text,3)
        IF (pi.active_ind=1
         AND pi.value_numeric=1)
         pharmrequest->ids[1].id = trim(lt.long_text,3)
        ENDIF
       ENDIF
      ENDIF
     WITH nocounter
    ...
    IF (size(trim(pharmrequest->ids[1].id,3))=0)
     SELECT INTO "nl:"
      FROM orders o,
       order_detail od
      PLAN (o
       WHERE (o.person_id=dlrec->seq[ml_pat_count].person_id))
       JOIN (od
       WHERE od.order_id=o.order_id
        AND od.oe_field_meaning="ROUTINGPHARMACYID")
      ORDER BY od.updt_dt_tm DESC
      HEAD REPORT
       ml_pharm_count = 0, ml_od_pharm_index = 0, ml_pi_pharm = locateval(ml_pharm_count,1,size(
         m_pharmrec->pharmacies,5),1,m_pharmrec->pharmacies[ml_pharm_count].n_active_ind)
      DETAIL
       IF (size(trim(pharmrequest->ids[1].id,3))=0)
        IF (size(m_pharmrec->pharmacies,5)=0)
         pharmrequest->ids[1].id = trim(od.oe_field_display_value,3),
         CALL echo("pharmacy first select"),
         CALL echo(trim(od.oe_field_display_value,3))
        ELSE
         IF (((ml_pi_pharm=0) OR ((od.updt_dt_tm > m_pharmrec->pharmacies[ml_pi_pharm].f_update_time)
         )) )
          ml_od_pharm_index = locateval(ml_pharm_count,1,size(m_pharmrec->pharmacies,5),trim(od
            .oe_field_display_value,3),m_pharmrec->pharmacies[ml_pharm_count].s_id)
          IF (ml_od_pharm_index=0)
           pharmrequest->ids[1].id = trim(od.oe_field_display_value,3)
          ELSE
           IF ((((m_pharmrec->pharmacies[ml_od_pharm_index].n_active_ind=1)) OR ((m_pharmrec->
           pharmacies[ml_od_pharm_index].f_update_time < od.updt_dt_tm))) )
            pharmrequest->ids[1].id = trim(od.oe_field_display_value,3)
           ENDIF
          ENDIF
         ELSE
          pharmrequest->ids[1].id = m_pharmrec->pharmacies[ml_pi_pharm].s_id
         ENDIF
        ENDIF
       ENDIF
      WITH nocounter
    ...
    ENDIF
    IF (size(trim(pharmrequest->ids[1].id,3))=0)
     SET ml_pi_pharm = locateval(ml_pharm_count,1,size(m_pharmrec->pharmacies,5),1,m_pharmrec->
      pharmacies[ml_pharm_count].n_active_ind)
     IF (ml_pi_pharm=0)
      SET dlrec->seq[ml_pat_count].s_pharmacy = "No Pharmacy Information found"
     ELSE
      SET pharmrequest->ids[1].id = m_pharmrec->pharmacies[ml_pi_pharm].s_id
     ENDIF
    ENDIF
    IF (size(trim(pharmrequest->ids[1].id,3)) > 0)
     SET stat = tdbexecute(ml_app_id,ml_task_id,ml_request_id,"REC",pharmrequest,
      "REC",pharmreply)
     SET dlrec->seq[ml_pat_count].s_pharmacy = pharmreply->pharmacies[1].pharmacy_name
     SET dlrec->seq[ml_pat_count].s_pharm_addr = concat(pharmreply->pharmacies[1].
      pharmacy_contributions[1].addresses[1].street_address_lines[1].street_address_line," ",
      pharmreply->pharmacies[1].pharmacy_contributions[1].addresses[1].city,", ",pharmreply->
      pharmacies[1].pharmacy_contributions[1].addresses[1].state)
     CALL echo(build("pharm name: ",dlrec->seq[ml_pat_count].s_pharmacy,", pharmacy addr: ",dlrec->
       seq[ml_pat_count].s_pharm_addr))
    ENDIF
  ENDFOR
  SELECT INTO "nl:"
   sch_resource_disp = uar_get_code_display(sch.resource_cd)
   FROM sch_appt sch,
    shx_activity sa,
    shx_response sr,
    ...
    nomenclature n
   PLAN (sch
    WHERE expand(ml_pat_count,1,size(dlrec->seq,5),sch.sch_event_id,dlrec->seq[ml_pat_count].
     sch_event_id)
     AND sch.beg_dt_tm BETWEEN cnvtdatetime(start_dt) AND cnvtdatetime(stop_dt)
     AND sch.appt_location_cd=f_amb_unit
     AND sch.encntr_id > 0
     AND sch.sch_state_cd IN (rescheduled_sch, pending_sch, scheduled_sch, appointment_sch,
    finalized_sch,
    confirmed_sch))
    JOIN (sa
    WHERE sa.person_id=sch.person_id
     AND sa.active_ind=1
     AND sa.end_effective_dt_tm > cnvtdatetime(sysdate))
    JOIN (sr
    WHERE sr.shx_activity_id=sa.shx_activity_id
     AND sr.task_assay_cd=mf_tobacco_cd
     AND sr.active_ind=1
     AND sr.end_effective_dt_tm > cnvtdatetime(sysdate))
    JOIN (sar
    WHERE sar.shx_response_id=sr.shx_response_id
     AND sar.active_ind=1
     AND sar.end_effective_dt_tm > cnvtdatetime(sysdate))
    JOIN (n
    WHERE n.nomenclature_id=sar.nomenclature_id
     AND n.active_ind=1
     AND n.end_effective_dt_tm > cnvtdatetime(sysdate))
   ORDER BY sch_resource_disp, sch.sch_event_id, sr.beg_effective_dt_tm DESC
   HEAD REPORT
    ml_index = 0, ml_pat_count = 0
   HEAD sch.sch_event_id
    ml_index = locateval(ml_pat_count,1,size(dlrec->seq,5),sch.sch_event_id,dlrec->seq[ml_pat_count].
     sch_event_id)
    IF (size(trim(n.source_string,3)) > 0)
     dlrec->seq[ml_index].s_tobacco = n.source_string
    ENDIF
   WITH nocounter, expand = 1
  ;end select
 ENDIF
   ...
   HEAD p.problem_id
    IF (((n.source_string > " ") OR (p.problem_ftdesc > " "))
     ...
     IF (p.nomenclature_id > 0)
      dlrec->seq[d1.seq].problem[prob_cnt].text = trim(substring(1,90,n.source_string))
     ELSE
      dlrec->seq[d1.seq].problem[prob_cnt].text = trim(substring(1,90,p.problem_ftdesc))
     ENDIF
     ...
     (dlrec->seq[d1.seq].problem[prob_cnt].status,": ",trim(substring(1,90,dlrec->seq[d1.seq].
        problem[prob_cnt].text)))
    ENDIF
 ...
 IF (size(dlrec->seq,5) > 0)
  SELECT INTO "nl:"
   ...
   HEAD d1.seq
    ...
    CALL echo("%%%%%% 1")
   HEAD o.order_id
    cnt += 1, stat = alterlist(dlrec->seq[d1.seq].meds,cnt),
    CALL echo("%%%%%% 2"),
    ...
    IF (o.order_comment_ind=1)
     ...
     CALL echo("%%%%%% 2.11")
    ENDIF
    dlrec->seq[d1.seq].meds[cnt].display_line = trim(o.clinical_display_line)
    IF (size(trim(lt.long_text,3)) > 0)
     dlrec->seq[d1.seq].meds[cnt].display_line = concat(dlrec->seq[d1.seq].meds[cnt].display_line,
      ", ",trim(lt.long_text))
    ENDIF
 ...
 SET select_starttime = cnvtdatetime(sysdate)
 CALL echo("get expectations")
 IF (size(dlrec->seq,5) > 0)
  SELECT INTO "nl:"
   p.name_full_formatted, he.expect_name, hr.system_freq_val,
   uar_get_code_display(hr.system_freq_unit_cd), hr.due_dt_tm, hr.last_satisfaction_dt_tm,
   sat.expect_sat_name
   FROM person p,
   ...
   PLAN (d1)
    ...
    WHERE (p.person_id=dlrec->seq[d1.seq].person_id))
    JOIN (hr
    WHERE hr.person_id=p.person_id
    ...
    JOIN (sat
    WHERE sat.expect_id=hr.expect_id
     AND sat.active_ind=1
     AND sat.entry_type_cd > 0)
   ORDER BY p.person_id, he.expect_id
   HEAD REPORT
    cnt = 0, due_label = " Not Due Until "
   HEAD p.person_id
    cnt = 0, stat = alterlist(dlrec->seq[d1.seq].hm,10), starttime = cnvtdatetime(curdate,curtime),
    calctime = cnvtdatetime(hr.due_dt_tm)
   HEAD he.expect_id
    starttime = cnvtdatetime(curdate,curtime), calctime = cnvtdatetime(hr.due_dt_tm)
    IF (hst.due_duration > 0)
     calctime = datetimeadd(hr.due_dt_tm,hst.due_duration)
    ENDIF
    IF (hr.due_dt_tm > starttime)
     due_label = " Not Due Until "
    ELSEIF (hr.due_dt_tm < starttime
     AND starttime > calctime
     AND hst.due_duration > 0)
     due_label = " Due "
    ELSE
     due_label = " Overdue "
    ENDIF
    IF (he.expect_name > " ")
     cnt += 1
     IF (mod(cnt,10)=1)
      stat = alterlist(dlrec->seq[d1.seq].hm,(cnt+ 9))
     ENDIF
     CALL echo(build(he.expect_name," , ",hr.system_freq_val))
     IF (hr.system_freq_val > 0)
      info = concat(trim(he.expect_name),"; ",trim(hes.priority_meaning),"; ",due_label,
       trim(format(cnvtdatetime(hr.due_dt_tm),"mm/dd/yyyy;;d"),3),"; Q",trim(cnvtstring(hr
         .system_freq_val)),trim(uar_get_code_display(hr.system_freq_unit_cd)))
     ELSE
      info = concat(trim(he.expect_name),"; ",trim(hes.priority_meaning),";",due_label,
       trim(format(cnvtdatetime(hr.due_dt_tm),"mm/dd/yyyy;;d"),3)," VARIABLE")
     ENDIF
     dlrec->seq[d1.seq].hm[cnt].step_desc = info
    ENDIF
   FOOT  p.person_id
    stat = alterlist(dlrec->seq[d1.seq].hm,cnt), cnt = 0
   WITH nocounter
  ;end select
 ENDIF
 ...
 IF (size(dlrec->seq,5) > 0)
  SELECT INTO "nl:"
   name = uar_get_code_display(ce.event_cd), ce.performed_dt_tm
   ...
   PLAN (d1)
    JOIN (esc
    WHERE esc.event_set_name="IMMUNIZATIONS")
    JOIN (ese
    WHERE esc.event_set_cd=ese.event_set_cd)
    JOIN (ce
    WHERE ce.event_cd=ese.event_cd
     AND (ce.person_id=dlrec->seq[d1.seq].person_id)
     AND ce.view_level=1
     AND ce.publish_flag=1
     AND ce.valid_until_dt_tm=cnvtdatetime("31-dec-2100,00:00:00")
     AND  NOT (ce.result_status_cd IN (inerror_cd, notdone_cd))
     AND ce.event_tag > " ")
   ORDER BY ce.person_id, ce.event_cd, ce.performed_dt_tm
   HEAD REPORT
    cnt = 0
   HEAD ce.person_id
    cnt = 0, stat = alterlist(dlrec->seq[d1.seq].immunization,10)
   DETAIL
    cnt += 1
    IF (mod(cnt,10)=1)
     stat = alterlist(dlrec->seq[d1.seq].immunization,(cnt+ 9))
    ENDIF
    dlrec->seq[d1.seq].immunization[cnt].name = uar_get_code_display(ce.event_cd), dlrec->seq[d1.seq]
    .immunization[cnt].given_date = substring(1,14,format(ce.performed_dt_tm,"@SHORTDATE"))
   FOOT  d1.seq
    stat = alterlist(dlrec->seq[d1.seq].immunization,cnt), cnt = 0
   WITH nocounter
  ;end select
 ENDIF
 ...
 IF (size(dlrec->seq,5) > 0)
  SELECT DISTINCT INTO "nl:"
   ...
   HEAD REPORT
    cnt = 0
   HEAD d1.seq
    cnt += 1, stat = alterlist(dlrec->seq[d1.seq].measurements,cnt)
   HEAD c.event_cd
    CASE (c.event_cd)
     OF weight_cd:
 ...
 SUBROUTINE (headpageabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (detailedpatinfo(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = detailedpatinfoabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (detailedpatinfoabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __phone_number = vc WITH noconstant(build2(dlrec->seq[cnt_pat].s_phone_number,char(0))),
   protect
   DECLARE __address = vc WITH noconstant(build2(dlrec->seq[cnt_pat].s_address,char(0))), protect
   DECLARE __tobaccouse = vc WITH noconstant(build2(dlrec->seq[cnt_pat].s_tobacco,char(0))), protect
   IF (size(trim(dlrec->seq[cnt_pat].s_email,3)) > 0)
    DECLARE __emailaddress = vc WITH noconstant(build2(dlrec->seq[cnt_pat].s_email,char(0))), protect
   ENDIF
   DECLARE __pharmacy12 = vc WITH noconstant(build2(concat(dlrec->seq[cnt_pat].s_pharmacy," ",dlrec->
      seq[cnt_pat].s_pharm_addr),char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Assign Resource Alternate:",char(0)))
    ...
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__phone_number)
    SET rptsd->m_flags = 4
    SET rptsd->m_y = (offsety+ 0.375)
    ...
    SET rptsd->m_height = 0.260
    SET _dummyfont = uar_rptsetfont(_hreport,_times10b0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Email Address:",char(0)))
    SET rptsd->m_y = (offsety+ 1.250)
    SET rptsd->m_x = (offsetx+ 3.438)
    SET rptsd->m_width = 1.042
    SET rptsd->m_height = 0.188
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Tobacco Use:",char(0)))
    SET rptsd->m_flags = 0
    SET rptsd->m_y = (offsety+ 1.250)
    SET rptsd->m_x = (offsetx+ 5.125)
    SET rptsd->m_width = 2.375
    SET rptsd->m_height = 0.260
    SET _dummyfont = uar_rptsetfont(_hreport,_times100)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__tobaccouse)
    SET rptsd->m_y = (offsety+ 1.250)
    ...
    IF (size(trim(dlrec->seq[cnt_pat].s_email,3)) > 0)
     SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__emailaddress)
    ENDIF
    SET rptsd->m_flags = 4
    ...
    SET _dummyfont = uar_rptsetfont(_hreport,_times10b0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Pharmacy:",char(0)))
    SET rptsd->m_flags = 0
    SET rptsd->m_y = (offsety+ 0.938)
    SET rptsd->m_x = (offsetx+ 3.438)
    ...
    SET rptsd->m_height = 0.260
    SET _dummyfont = uar_rptsetfont(_hreport,_times100)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__pharmacy12)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 ...
 SUBROUTINE (active_conditions_headabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.162),(offsetx+ 7.250),(offsety+
     0.162))
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
 ...
 SUBROUTINE (procedures_headabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.271),(offsetx+ 7.250),(offsety+
     0.271))
    SET rptsd->m_y = (offsety+ 0.125)
 ...
 SUBROUTINE (medtitleabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8 WITH
  protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (medicationdetails(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = medicationdetailsabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (medicationdetailsabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.380000), private
   DECLARE __display_line = vc WITH noconstant(build2(dlrec->seq[cnt_pat].meds[x].display_line,char(0
      ))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 4
    SET rptsd->m_borders = rpt_sdnoborders
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_times100)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__display_line)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 ...
 SUBROUTINE (medinstructionsabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =f8
  WITH protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (no_medications(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = no_medicationsabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (no_medicationsabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.270000), private
   IF ( NOT (( $ML_MODE=1)))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_times100)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("no medication data",char(0)))
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (hm_head(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = hm_headabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (hm_headabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_times10b0)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Expectations",char(0)))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.287),(offsetx+ 7.250),(offsety+
     0.287))
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (hm(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = hmabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (hmabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.340000), private
   DECLARE __hm = vc WITH noconstant(build2(dlrec->seq[cnt_pat].hm[x].step_desc,char(0))), protect
   IF ( NOT (( $ML_MODE=0)))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_times100)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__hm)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 ...
 SUBROUTINE (immun_headabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.350),(offsetx+ 7.250),(offsety+
     0.350))
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 ...
 SUBROUTINE (immunabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   DECLARE sectionheight = f8 WITH noconstant(0.270000), private
   DECLARE __immunization = vc WITH noconstant(build2(dlrec->seq[cnt_pat].immunization[x].name,char(0
      ))), protect
   DECLARE __immunization_date = vc WITH noconstant(build2(dlrec->seq[cnt_pat].immunization[x].
     given_date,char(0))), protect
   IF ( NOT (( $ML_MODE=0)))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
 ...
 SUBROUTINE (measurements_headabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (measurements(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = measurementsabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (measurementsabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE (worksheetspace(ncalc=i2) =f8 WITH protect)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = worksheetspaceabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE (worksheetspaceabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "TEST_BHS_RPTAMB"
 ...
 SUBROUTINE (sub_head_page(beg_report=i2) =null)
  ...
  SET d0 = detailedpatinfo(rpt_render)
 END ;Subroutine
 ...
 FOR (cnt_pat = 1 TO size(dlrec->seq,5))
   ...
   ENDIF
   IF ((((_yoffset+ allergies_head(rpt_calcheight))+ worksheetspace(rpt_calcheight)) > page_size))
    SET d0 = sub_foot_page(0)
    SET d0 = sub_head_page(0)
   ENDIF
   SET d0 = worksheetspace(rpt_render)
   SET d0 = allergies_head(rpt_render)
   IF (size(dlrec->seq[cnt_pat].allergy,5) > 0)
    FOR (x = 1 TO size(dlrec->seq[cnt_pat].allergy,5))
      SET remain_space = (page_size - _yoffset)
      IF ((((_yoffset+ allergies(rpt_calcheight,remain_space,becont))+ worksheetspace(rpt_calcheight)
      ) > page_size))
       SET d0 = sub_foot_page(0)
       SET d0 = sub_head_page(0)
       SET d0 = worksheetspace(rpt_render)
   ...
   IF (size(dlrec->seq[cnt_pat].meds,5) > 0)
    IF ((((_yoffset+ medication_head(rpt_calcheight))+ worksheetspace(rpt_calcheight)) > page_size))
     SET d0 = sub_foot_page(0)
     SET d0 = sub_head_page(0)
    ENDIF
    SET d0 = worksheetspace(rpt_render)
    ...
    FOR (x = 1 TO size(dlrec->seq[cnt_pat].meds,5))
      SET remain_space = (page_size - _yoffset)
      IF (((((_yoffset+ medtitle(rpt_calcheight,remain_space,becont))+ medicationdetails(
       rpt_calcheight))+ worksheetspace(rpt_calcheight)) > page_size))
       SET d0 = sub_foot_page(0)
       SET d0 = sub_head_page(0)
       SET d0 = worksheetspace(rpt_render)
       SET d0 = medication_head(rpt_render)
      ...
      SET d0 = medicationdetails(rpt_render)
    ENDFOR
    IF (((((_yoffset+ medication_head(rpt_calcheight))+ worksheetspace(rpt_calcheight))+
    no_medications(rpt_calcheight)) > page_size))
     SET d0 = sub_foot_page(0)
     SET d0 = sub_head_page(0)
    ENDIF
   ENDIF
   IF (size(dlrec->seq[cnt_pat].hm,5) > 0)
    IF ((((_yoffset+ hm_head(rpt_calcheight))+ worksheetspace(rpt_calcheight)) > page_size))
     SET d0 = sub_foot_page(0)
     SET d0 = sub_head_page(0)
    ENDIF
    SET d0 = hm_head(rpt_render)
    FOR (x = 1 TO size(dlrec->seq[cnt_pat].hm,5))
     IF ((((_yoffset+ hm_head(rpt_calcheight))+ worksheetspace(rpt_calcheight)) > page_size))
      SET d0 = sub_foot_page(0)
      SET d0 = sub_head_page(0)
      SET d0 = worksheetspace(rpt_render)
      SET d0 = hm_head(rpt_render)
      CALL echo("tried to render the hm section")
     ENDIF
     SET d0 = hm(rpt_render)
    ENDFOR
   ENDIF
   IF (size(dlrec->seq[cnt_pat].immunization,5) > 0)
    IF ((((_yoffset+ immun_head(rpt_calcheight))+ worksheetspace(rpt_calcheight)) > page_size))
     SET d0 = sub_foot_page(0)
     SET d0 = sub_head_page(0)
    ENDIF
    SET d0 = immun_head(rpt_render)
    FOR (x = 1 TO size(dlrec->seq[cnt_pat].immunization,5))
     IF ((((_yoffset+ immun_head(rpt_calcheight))+ worksheetspace(rpt_calcheight)) > page_size))
      SET d0 = sub_foot_page(0)
      SET d0 = sub_head_page(0)
      SET d0 = worksheetspace(rpt_render)
    ...
    ENDFOR
   ENDIF
   IF ((((_yoffset+ measurements_head(rpt_calcheight))+ worksheetspace(rpt_calcheight)) > page_size))
    SET d0 = sub_foot_page(0)
    SET d0 = sub_head_page(0)
   ENDIF
   SET d0 = worksheetspace(rpt_render)
   ...
   FOR (x = 1 TO size(dlrec->seq[cnt_pat].measurements,5))
    IF ((((_yoffset+ measurements(rpt_calcheight))+ worksheetspace(rpt_calcheight)) > page_size))
     SET d0 = sub_foot_page(0)
     SET d0 = sub_head_page(0)
     SET d0 = worksheetspace(rpt_render)
     SET d0 = measurements_head(rpt_render)
 ...
 IF (operations=1)
  SET spool value(var_output) value( $OUTDEV) WITH nodeleted
 ENDIF
 CALL echo("End Record")
 SET last_mod =
 "006 02/21/2017 SH013356 412115658       modified the downtime ambulatory report to include more fields"
#exit_prg
END GO
