CREATE PROGRAM dm2_create_ddl:dba
 ...
 SUBROUTINE drr_del_preserved_ts(dcdl_tgt_db_name)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(ddr_ddl_to_execute->cnt,- (1))=- (1))
  AND (validate(ddr_ddl_to_execute->cnt,- (2))=- (2)))
  FREE RECORD ddr_ddl_to_execute
  RECORD ddr_ddl_to_execute(
    1 cnt = i4
    1 qual[*]
      2 tbl_name = vc
      2 obj_name = vc
  ...
  SET ddr_ddl_to_execute->cnt = 0
 ENDIF
 DECLARE ddr_schema_runner(null) = i2
 SUBROUTINE ddr_schema_runner(null)
   DECLARE dsr_cnt = i4 WITH protect, noconstant(0)
   DECLARE dsr_cmd_cnt = i4 WITH protect, noconstant(0)
   IF ((ddr_ddl_to_execute->cnt=0))
    SET dm_err->eproc = "No DDL to execute"
    CALL disp_msg("",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   FREE RECORD dsr_priority_list
   RECORD dsr_priority_list(
     1 qual[*]
       2 priority = i4
   )
   SET dm_err->eproc = "Get DDL into priority order"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(ddr_ddl_to_execute->cnt))
    ORDER BY ddr_ddl_to_execute->qual[d.seq].priority
    DETAIL
     IF (dsr_cnt=0)
      dsr_cnt = (dsr_cnt+ 1), stat = alterlist(dsr_priority_list->qual,dsr_cnt), dsr_priority_list->
      qual[dsr_cnt].priority = ddr_ddl_to_execute->qual[d.seq].priority
     ELSE
      IF ((dsr_priority_list->qual[dsr_cnt].priority != ddr_ddl_to_execute->qual[d.seq].priority))
       dsr_cnt = (dsr_cnt+ 1), stat = alterlist(dsr_priority_list->qual,dsr_cnt), dsr_priority_list->
       qual[dsr_cnt].priority = ddr_ddl_to_execute->qual[d.seq].priority
      ENDIF
     ENDIF
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   FOR (dsr_cnt = 1 TO size(dsr_priority_list->qual,5))
     SET dm_err->eproc = build("Executing DDL for Priority:",dsr_priority_list->qual[dsr_cnt].
      priority)
     CALL disp_msg(" ",dm_err->logfile,0)
     FOR (dsr_cmd_cnt = 1 TO value(ddr_ddl_to_execute->cnt))
       IF ((ddr_ddl_to_execute->qual[dsr_cmd_cnt].priority=dsr_priority_list->qual[dsr_cnt].priority)
       )
        IF (dm2_push_cmd(ddr_ddl_to_execute->qual[dsr_cmd_cnt].operation,1)=0)
         SET ddr_ddl_to_execute->qual[dsr_cmd_cnt].status = "ERROR"
         SET ddr_ddl_to_execute->qual[dsr_cmd_cnt].err_msg = dm_err->emsg
         IF ((dm_err->debug_flag > 2))
          CALL echorecord(ddr_ddl_to_execute)
         ENDIF
         RETURN(0)
        ELSE
         SET ddr_ddl_to_execute->qual[dsr_cmd_cnt].status = "COMPLETE"
        ENDIF
       ENDIF
     ENDFOR
   ENDFOR
   IF ((dm_err->debug_flag > 2))
    CALL echorecord(ddr_ddl_to_execute)
    CALL echorecord(dsr_priority_list)
   ENDIF
   RETURN(1)
 END ;Subroutine
 ...
 END ;Subroutine
 IF ((validate(dacr_driver_info->driver_cnt,- (1))=- (1)))
  FREE RECORD dacr_driver_info
  RECORD dacr_driver_info(
    1 driver[*]
      2 table_name = vc
      2 extract_id = f8
      2 min_driver_key_id = f8
      2 max_driver_key_id = f8
      2 ranges[*]
        3 lower_bound = f8
        3 upper_bound = f8
      2 range_cnt = i4
 ...
 ENDIF
 DECLARE dacr_get_range(i_min=f8,i_max=f8,i_batch_size=i4,i_range_num=i4,io_low=f8(ref),
  io_high=f8(ref)) = null
 DECLARE dacr_get_driver_table_ranges(i_driver_table_name=vc,i_config_id=f8,i_batch_size=i4) = i2
 SUBROUTINE dacr_get_driver_table_ranges(i_driver_table_name,i_config_id,i_batch_size)
   DECLARE dgdtr_idx = i2 WITH protect, noconstant(0)
   DECLARE dgdtr_continue_ind = i2 WITH protect, noconstant(1)
   DECLARE dgdtr_low = f8 WITH protect, noconstant(0.0)
   DECLARE dgdtr_high = f8 WITH protect, noconstant(0.0)
   DECLARE dgdtr_prev_scramble_ind = i2 WITH protect, noconstant(0)
   SET dgdtr_idx = locateval(dgdtr_idx,1,dacr_driver_info->driver_cnt,i_driver_table_name,
    dacr_driver_info->driver[dgdtr_idx].table_name)
   IF (dgdtr_idx=0)
    SET dacr_driver_info->driver_cnt = (dacr_driver_info->driver_cnt+ 1)
    SET dgdtr_idx = dacr_driver_info->driver_cnt
    SET stat = alterlist(dacr_driver_info->driver,dacr_driver_info->driver_cnt)
    SET dacr_driver_info->driver[dgdtr_idx].table_name = i_driver_table_name
    SET dm2scramble->in_text = i_driver_table_name
    SET dgdtr_prev_scramble_ind = dm2scramble->mode_ind
    SET dm2scramble->mode_ind = 1
    CALL ds_scramble(null)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(- (1))
    ENDIF
    SELECT INTO "nl:"
     FROM dm_ads_extract@repl_source d
     WHERE trim(d.table_name,3)=trim(dm2scramble->out_text,3)
      AND d.driver_table_ind=1
     DETAIL
      dacr_driver_info->driver[dgdtr_idx].extract_id = d.dm_ads_extract_id
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    ...
    ENDIF
    SELECT INTO "nl:"
     x = max(d.dm_ads_config_driver_id), y = min(d.dm_ads_config_driver_id)
     FROM dm_ads_config_driver@repl_source d
     WHERE d.dm_ads_config_id=i_config_id
      AND (d.dm_ads_extract_id=dacr_driver_info->driver[dgdtr_idx].extract_id)
     DETAIL
      dacr_driver_info->driver[dgdtr_idx].max_driver_key_id = x, dacr_driver_info->driver[dgdtr_idx].
      min_driver_key_id = y
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     ...
     RETURN(- (1))
    ENDIF
    SET dm2scramble->mode_ind = dgdtr_prev_scramble_ind
   ENDIF
   SET stat = alterlist(dacr_driver_info->driver[dgdtr_idx].ranges,0)
   SET dacr_driver_info->driver[dgdtr_idx].range_cnt = 0
   WHILE (dgdtr_continue_ind=1)
     CALL dacr_get_range(dacr_driver_info->driver[dgdtr_idx].min_driver_key_id,dacr_driver_info->
      driver[dgdtr_idx].max_driver_key_id,i_batch_size,(dacr_driver_info->driver[dgdtr_idx].range_cnt
      + 1),dgdtr_low,
      dgdtr_high)
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     ...
     ENDIF
     IF ((dgdtr_low=- (1)))
      SET dgdtr_continue_ind = 0
     ELSE
      SET dacr_driver_info->driver[dgdtr_idx].range_cnt = (dacr_driver_info->driver[dgdtr_idx].
      range_cnt+ 1)
      SET stat = alterlist(dacr_driver_info->driver[dgdtr_idx].ranges,dacr_driver_info->driver[
       dgdtr_idx].range_cnt)
      SET dacr_driver_info->driver[dgdtr_idx].ranges[dacr_driver_info->driver[dgdtr_idx].range_cnt].
      lower_bound = dgdtr_low
      SET dacr_driver_info->driver[dgdtr_idx].ranges[dacr_driver_info->driver[dgdtr_idx].range_cnt].
      upper_bound = dgdtr_high
     ENDIF
   ENDWHILE
   RETURN(dgdtr_idx)
 END ;Subroutine
 SUBROUTINE dacr_get_range(i_min,i_max,i_batch_size,i_range_num,io_low,io_high)
   DECLARE dgr_max_range = i4 WITH protect, noconstant(0)
   SET dgr_max_range = ceil((((i_max+ 1) - i_min)/ i_batch_size))
   SET io_low = - (1)
   SET io_high = - (1)
   IF (i_range_num <= dgr_max_range)
    SET io_low = (i_min+ ((i_range_num - 1) * i_batch_size))
    SET io_high = ((io_low+ i_batch_size) - 1)
    IF (io_high > i_max)
     SET io_high = i_max
    ENDIF
   ENDIF
 END ;Subroutine
 IF ((validate(dpr_obj_list->tbl_cnt,- (1))=- (1))
  AND (validate(dpr_obj_list->tbl_cnt,- (2))=- (2)))
  FREE RECORD dpr_obj_list
  RECORD dpr_obj_list(
    1 tbl_cnt = i4
    1 idx_cnt = i4
    1 obj_tbl[*]
 ...
 ENDIF
 IF ((validate(dpr_ddl_excl->cnt,- (1))=- (1))
  AND (validate(dpr_ddl_excl->cnt,- (2))=- (2)))
  FREE RECORD dpr_ddl_excl
  RECORD dpr_ddl_excl(
    1 cnt = i4
    1 qual[*]
      2 table_name = vc
 ...
 ENDIF
 DECLARE dpr_sub_ddl_excl(dsde_db_link=vc) = i2
 DECLARE dpr_rpt_ddl_excl(drde_file=vc(ref)) = i2
 DECLARE dpr_eval_db_partition(dedp_part_ind=i2(ref)) = i2
 DECLARE dpr_eval_db_partition_option(dedp_part_opt_ind=i2(ref)) = i2
 DECLARE dpr_create_part_proc(null) = i2
 DECLARE dpr_identify_partition_usage(dipu_check_usage_ind=i2,dipu_part_enabled_ind=i2(ref),
  dipu_part_usage_ind=i2(ref)) = i2
 SUBROUTINE dpr_sub_ddl_excl(dsde_db_link)
   IF (validate(dm2_bypass_ddl_excl_ind,- (1))=1)
    SET dm_err->eproc = "bypassing the partition_table - external flag set"
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   IF (((validate(drr_clin_copy_data->ads_chosen_ind,- (1))=1) OR (((validate(drr_clin_copy_data->
    purge_chosen_ind,- (1))=1) OR (((validate(drr_clin_copy_data->process,"DM2NOTSET")="MIGRATION")
    OR ((dm2_sys_misc->cur_db_os="AXP"))) )) )) )
    SET dm_err->eproc =
    "bypassing the partition_table - partition table not supported chosen process"
    CALL disp_msg(" ",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   DECLARE dpr_tbl_var = i4 WITH protect, noconstant(0)
   DECLARE dpr_idx_var = i4 WITH protect, noconstant(0)
   DECLARE dpr_di_exists = i2 WITH protect, noconstant(0)
   SET dpr_obj_list->tbl_cnt = 0
   SET dpr_obj_list->idx_cnt = 0
   SET stat = alterlist(dpr_obj_list->obj_tbl,0)
   SET stat = alterlist(dpr_obj_list->obj_idx,0)
   IF ((dm2_install_schema->process_option != "CLIN COPY"))
    IF (dsde_db_link > " ")
     SET dm_err->eproc = concat("Check for the tables in dba_tab_partitions using link ",dsde_db_link
      )
    ELSE
     SET dm_err->eproc = concat("Check for the tables in dba_tab_partitions.")
    ENDIF
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT
     IF (dsde_db_link > " ")
      FROM (parser(concat("dba_tab_partitions@",dsde_db_link)) dtp)
     ELSE
      FROM dba_tab_partitions dtp
     ENDIF
     DISTINCT INTO "nl:"
     dtp.table_name
     WHERE dtp.table_owner=currdbuser
     DETAIL
      dpr_obj_list->tbl_cnt = (dpr_obj_list->tbl_cnt+ 1)
      IF (mod(dpr_obj_list->tbl_cnt,10)=1)
       stat = alterlist(dpr_obj_list->obj_tbl,(dpr_obj_list->tbl_cnt+ 9))
      ENDIF
      dpr_obj_list->obj_tbl[dpr_obj_list->tbl_cnt].dpr_tbl = dtp.table_name, dpr_obj_list->obj_tbl[
      dpr_obj_list->tbl_cnt].dpr_owner = dtp.table_owner
     FOOT REPORT
      stat = alterlist(dpr_obj_list->obj_tbl,dpr_obj_list->tbl_cnt)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    IF (dsde_db_link > " ")
     SET dm_err->eproc = concat("Check for the indexes in dba_ind_partitions using link ",
      dsde_db_link)
    ELSE
     SET dm_err->eproc = concat("Check for the indexes in dba_ind_partitions.")
    ENDIF
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT
     IF (dsde_db_link > " ")
      FROM (parser(concat("dba_ind_partitions@",dsde_db_link)) dip)
     ELSE
      FROM dba_ind_partitions dip
     ENDIF
     DISTINCT INTO "nl:"
     dip.index_name
     WHERE dip.index_owner=currdbuser
     DETAIL
      dpr_obj_list->idx_cnt = (dpr_obj_list->idx_cnt+ 1)
      IF (mod(dpr_obj_list->idx_cnt,10)=1)
       stat = alterlist(dpr_obj_list->obj_idx,(dpr_obj_list->idx_cnt+ 9))
      ENDIF
      dpr_obj_list->obj_idx[dpr_obj_list->idx_cnt].dpr_idx = dip.index_name, dpr_obj_list->obj_idx[
      dpr_obj_list->idx_cnt].dpr_owner = dip.index_owner
     FOOT REPORT
      stat = alterlist(dpr_obj_list->obj_idx,dpr_obj_list->idx_cnt)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (dsde_db_link <= " ")
    SET dm_err->eproc = "Check if DM_INFO exists"
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg("",dm_err->logfile,0)
    ENDIF
    IF (dm2_table_and_ccldef_exists("DM_INFO",dpr_di_exists)=0)
     RETURN(0)
    ENDIF
    IF (dpr_di_exists=0)
     SET dm_err->eproc = "DM_INFO table or CCLDEF does not exist"
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg("",dm_err->logfile,0)
     ENDIF
     IF ((dm_err->debug_flag > 0))
      CALL echorecord(dpr_obj_list)
     ENDIF
     RETURN(1)
    ENDIF
   ENDIF
   SET dm_err->eproc = concat("Check for excluded tables and indexes in dm_info.")
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dpr_tbl_var = 0
   SET dpr_idx_var = 0
   SELECT
    IF (dsde_db_link > " ")
     FROM (parser(concat("dm_info@",dsde_db_link)) di)
    ELSE
     FROM dm_info di
    ENDIF
    INTO "nl:"
    WHERE di.info_domain="DM2_EXCL_DDL*"
    DETAIL
     IF (trim(cnvtupper(di.info_domain))="DM2_EXCL_DDL_TABLE")
      dpr_tbl_var = locateval(dpr_tbl_var,1,dpr_obj_list->tbl_cnt,trim(cnvtupper(di.info_char)),
       dpr_obj_list->obj_tbl[dpr_tbl_var].dpr_owner,
       trim(cnvtupper(di.info_name)),dpr_obj_list->obj_tbl[dpr_tbl_var].dpr_tbl)
      IF (dpr_tbl_var=0)
       dpr_obj_list->tbl_cnt = (dpr_obj_list->tbl_cnt+ 1), stat = alterlist(dpr_obj_list->obj_tbl,
        dpr_obj_list->tbl_cnt), dpr_obj_list->obj_tbl[dpr_obj_list->tbl_cnt].dpr_tbl = trim(cnvtupper
        (di.info_name)),
       dpr_obj_list->obj_tbl[dpr_obj_list->tbl_cnt].dpr_owner = trim(cnvtupper(di.info_char))
      ENDIF
     ENDIF
     IF (trim(cnvtupper(di.info_domain))="DM2_EXCL_DDL_INDEX")
      dpr_idx_var = locateval(dpr_idx_var,1,dpr_obj_list->idx_cnt,trim(cnvtupper(di.info_char)),
       dpr_obj_list->obj_idx[dpr_idx_var].dpr_owner,
       trim(cnvtupper(di.info_name)),dpr_obj_list->obj_idx[dpr_idx_var].dpr_idx)
      IF (dpr_idx_var=0)
       dpr_obj_list->idx_cnt = (dpr_obj_list->idx_cnt+ 1), stat = alterlist(dpr_obj_list->obj_idx,
        dpr_obj_list->idx_cnt), dpr_obj_list->obj_idx[dpr_obj_list->idx_cnt].dpr_idx = trim(cnvtupper
        (di.info_name)),
       dpr_obj_list->obj_idx[dpr_obj_list->idx_cnt].dpr_owner = trim(cnvtupper(di.info_char))
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dpr_rpt_ddl_excl(drde_file)
   DECLARE drde_cnt = i4 WITH protect, noconstant(0)
   DECLARE drde_cmd_cnt = i4 WITH protect, noconstant(0)
   DECLARE drde_line = c120 WITH protect, constant(fillstring(120,"-"))
   DECLARE drde_str = vc WITH protect, noconstant("")
   DECLARE drde_str_cntr = i4 WITH protect, noconstant(0)
   IF ((dpr_ddl_excl->cnt=0))
    SET dm_err->eproc = "No DDL excluded."
    CALL disp_msg("",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   IF (get_unique_file("dm2_ddl_excl",".rpt")=0)
    RETURN(0)
   ENDIF
   SET drde_file = dm_err->unique_fname
   IF (validate(drrr_responsefile_in_use,0)=1)
    SET drde_file = build(drrr_misc_data->active_dir,drde_file)
   ENDIF
   SET dm_err->eproc = concat("Create DDL exclusion report at ",drde_file)
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO value(drde_file)
    FROM (dummyt d  WITH seq = value(dpr_ddl_excl->cnt))
    ORDER BY dpr_ddl_excl->qual[d.seq].priority, dpr_ddl_excl->qual[d.seq].op_type
    HEAD REPORT
     row + 1, col 1,
     "***************************WARNING:   DDL exclusions have been detected.*******************************",
     row + 1, col 1,
     "DDL commands displayed in this report are excluded by Install Tools process.  Please make schema changes",
     row + 1, col 1, "recommended manually.  No action will result in process failure.",
     row + 1, col 1,
     "DDL commands displayed below should only be used as reference.  Not all DDLs should be executed as it is.",
     row + 1, col 1,
     "********************************************************************************************************",
     row + 2, col 0, "PRIORITY",
     col 10, "TABLE NAME", col 42,
     "OBJECT_NAME", col 74, "OP TYPE",
     row + 1, col 0, drde_line,
     row + 1
    DETAIL
     drde_str = trim(cnvtstring(dpr_ddl_excl->qual[d.seq].priority)), col 4, drde_str,
     col 10, dpr_ddl_excl->qual[d.seq].table_name, col 42,
     dpr_ddl_excl->qual[d.seq].obj_name, col 74, dpr_ddl_excl->qual[d.seq].op_type,
     row + 1, col 5, "OPERATION:"
     IF (size(trim(dpr_ddl_excl->qual[d.seq].operation)) > 77)
      drde_str_cntr = 1
      WHILE (drde_str_cntr < size(trim(dpr_ddl_excl->qual[d.seq].operation)))
        row + 1, drde_str = fillstring(77," "), drde_str = substring(drde_str_cntr,77,trim(
          dpr_ddl_excl->qual[d.seq].operation)),
        col 16, drde_str, drde_str_cntr = (drde_str_cntr+ 77)
      ENDWHILE
     ELSE
      row + 1, drde_str = fillstring(77," "), drde_str = trim(dpr_ddl_excl->qual[d.seq].operation),
      col 16, drde_str
     ENDIF
     row + 2
    FOOT REPORT
   ...
   ENDIF
   IF ((dm_err->debug_flag > 2))
    CALL echorecord(dpr_ddl_excl)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dpr_eval_db_partition(dedp_part_ind)
   SET dm_err->eproc = "Evaluate if partition is enabled.."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dedp_part_ind = 0
   SELECT INTO "nl:"
    FROM v$option p
    WHERE p.parameter="Partitioning"
     AND p.value="TRUE"
    DETAIL
     dedp_part_ind = 1
    WITH nocounter
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dpr_eval_db_partition_option(dedp_part_opt_ind)
   SET dm_err->eproc = "Evaluate to land partitioned object.."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dedp_part_opt_ind = 0
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="DM2_DB_OPTION"
     AND d.info_name="PARTITIONING"
     AND d.info_number=1
    DETAIL
     dedp_part_opt_ind = 1
    WITH nocounter
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dpr_create_part_proc(null)
   DECLARE dpr_row_ind = i2 WITH protect, noconstant(0)
   DECLARE dpr_version_match_ind = i2 WITH protect, noconstant(0)
   DECLARE dpr_version_str = vc WITH protect, noconstant("/*Version: 1.0*/")
   SET dm_err->eproc = "Evaluate DM2_SET_DB_PARTITIONING_OPTION procedure "
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_objects do
    WHERE do.object_name="DM2_SET_DB_PARTITIONING_OPTION"
     AND do.object_type="PROCEDURE"
     AND status="VALID"
     AND do.owner=currdbuser
    DETAIL
     dpr_row_ind = 1
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF (dpr_row_ind=1)
    SET dm_err->eproc = concat("Check if the object type needs recompile")
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     dpr_version_cnt = count(*)
     FROM dba_source ds
     WHERE ds.name="DM2_SET_DB_PARTITIONING_OPTION"
      AND ds.owner=currdbuser
      AND ds.type="PROCEDURE"
      AND findstring(dpr_version_str,ds.text) > 0
     DETAIL
      dpr_version_match_ind = evaluate(dpr_version_cnt,0,0,1)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc) != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
   ENDIF
   IF (((dpr_row_ind=0) OR (dpr_version_match_ind=0)) )
    SET dm_err->eproc = "Creating procedure to manage dm_info row"
    CALL disp_msg(" ",dm_err->logfile,0)
    CALL dm2_push_cmd(concat("rdb asis(^CREATE OR REPLACE PROCEDURE ",trim(currdbuser,3),
      ".DM2_SET_DB_PARTITIONING_OPTION ",dpr_version_str,"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ (ENABLE VARCHAR2)^)"),0)
    CALL dm2_push_cmd(concat("asis(^ AS",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ BEGIN",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ IF ENABLE = 'YES' THEN",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ MERGE  INTO V500.DM_INFO D ",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ USING DUAL ON (D.INFO_DOMAIN = 'DM2_DB_OPTION' AND ",char(10),
      "^)"),0)
    CALL dm2_push_cmd(concat("asis(^ D.INFO_NAME  = 'PARTITIONING')",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ WHEN MATCHED THEN",char(10),"^)"),0)
    CALL dm2_push_cmd(concat(
      "asis(^ UPDATE SET D.INFO_NUMBER = 1, D.UPDT_DT_TM = sysdate WHERE INFO_NUMBER = 0",char(10),
      "^)"),0)
    CALL dm2_push_cmd(concat("asis(^ WHEN NOT MATCHED THEN",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ INSERT (INFO_DOMAIN, INFO_NAME, INFO_NUMBER, UPDT_DT_TM) ",char(
       10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ VALUES('DM2_DB_OPTION', 'PARTITIONING', 1, SYSDATE); ",char(10),
      "^)"),0)
    CALL dm2_push_cmd(concat("asis(^ COMMIT;",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ ELSIF ENABLE = 'NO' THEN",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ DELETE  V500.DM_INFO D",char(10),"^)"),0)
    CALL dm2_push_cmd(concat(
      "asis(^ WHERE D.INFO_DOMAIN = 'DM2_DB_OPTION' AND D.INFO_NAME  = 'PARTITIONING';",char(10),"^)"
      ),0)
    CALL dm2_push_cmd(concat("asis(^ COMMIT;",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ END IF;",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ EXCEPTION",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ WHEN OTHERS THEN ^)"),0)
    CALL dm2_push_cmd(concat("asis(^ ROLLBACK;",char(10),"^)"),0)
    CALL dm2_push_cmd(concat("asis(^ raise_application_error(-20001,'An error was encountered - ' ^)"
      ),0)
    CALL dm2_push_cmd(concat("asis(^ ||SQLCODE||' -ERROR- '||SQLERRM);",char(10),"^)"),0)
