CREATE PROGRAM djh_md_addr_phone_csv:dba
 PROMPT
  ...
  HEAD REPORT
   ...
   FOR (y = 1 TO size(md_alias->qual[d.seq],5))
     xperson_id = format(md_alias->qual[y].personid,"#########"), d_update = format(md_alias->qual[y]
      .demog_updt,"yyyy-mm-dd;;d"), output_string = build(y,',"',xperson_id,'","',md_alias->qual[y].
      name,
      ...
      md_alias->qual[y].license_nbr,'","',md_alias->qual[y].ext_id,'","',md_alias->qual[y].b_addr_1,
      '","',md_alias->qual[y].b_addr_2,'","',md_alias->qual[y].b_addr_3,'","',
      md_alias->qual[y].b_addr_4,'","',md_alias->qual[y].b_city,'","',md_alias->qual[y].b_state,
      '","',md_alias->qual[y].b_zip,'","',md_alias->qual[y].b_phone,'","',
      md_alias->qual[y].ez_addr_1,'","',md_alias->qual[y].ez_addr_2,'","',md_alias->qual[y].ez_addr_3,
      '","',md_alias->qual[y].ez_addr_4,'","',md_alias->qual[y].ez_city,'","',
      md_alias->qual[y].ez_state,'","',md_alias->qual[y].ez_zip,'","',md_alias->qual[y].ez_phone,
      '",'),
     col 1, output_string
     IF ( NOT (curendreport))
      row + 1
 ...
 IF (email_ind=1)
  ...
  SET filename_out = concat(format(curdate,"YYYY-MM-DD;;D"),"_MD_ADDR_PHONE.csv")
  DECLARE subject_line = vc
  SET subject_line = concat(curprog,"-V1.x - MD ADDR PHONE ",curnode)
