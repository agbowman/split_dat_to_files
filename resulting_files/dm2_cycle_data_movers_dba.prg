CREATE PROGRAM dm2_cycle_data_movers:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dcdm_logfile = vc WITH protect, noconstant(" ")
 IF ((dm_err->logfile="NONE"))
  SET dcdm_logfile = "NONE"
  SET dm_err->logfile = "DM2_LOGFILE_NOTSET"
 ELSE
  SET dcdm_logfile = dm_err->logfile
 ENDIF
 SET dm_err->eproc = "Beginning dm2_cycle_data_movers"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dcdm_env_id = f8 WITH public, noconstant(0.0)
 DECLARE dcdm_info_domain = vc
 DECLARE dcdm_tot_mover = i4 WITH public, noconstant(0)
 DECLARE dcdm_running = i4 WITH public, noconstant(0)
 DECLARE dcdm_to_run = i4 WITH public, noconstant(0)
 DECLARE dcdm_flag = i2 WITH public, noconstant(0)
 DECLARE dcdm_backdoor = i4
 ...
 ENDIF
 SET dcdm_flag = 1
 SET dcdm_info_domain = build("MERGE*",cnvtstring(dcdm_env_id,20,0))
 IF (dcdm_flag=1)
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain=patstring(dcdm_info_domain)
    AND di.info_name IN ("RDDS MOVERS TO RUN", "RDDS MOVERS RUNNING")
   DETAIL
    IF (di.info_name="RDDS MOVERS TO RUN")
     dcdm_to_run = (dcdm_to_run+ di.info_number)
    ELSEIF (di.info_name="RDDS MOVERS RUNNING")
     dcdm_running = (dcdm_running+ di.info_number)
    ENDIF
   FOOT REPORT
    dcdm_tot_mover = (dcdm_to_run - dcdm_running)
   WITH nocounter
  ;end select
 ELSE
  SELECT INTO "nl:"
   tot = sum(di.info_number)
   FROM dm_info di
   WHERE di.info_domain=patstring(dcdm_info_domain)
  ...
  ;end select
 ENDIF
 IF (check_error("Query total number of RDDS MOVERS TO RUN") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_name="RDDS SERVER BACKDOOR"
   AND di.info_domain="DATA MANAGEMENT"
  WITH nocounter
 ;end select
 IF (check_error("Query for RDDS SERVER BACKDOOR row") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dcdm_backdoor = curqual
 IF (dcdm_backdoor > 0)
  FOR (dcdm_lp = 1 TO dcdm_tot_mover)
    EXECUTE dm2_run_data_movers_asynch
    IF ((dm_err->err_ind=1))
     GO TO exit_program
    ENDIF
    IF (dcdm_lp != dcdm_tot_mover)
     SET dm_err->eproc = "Pause 15 seconds before running next mover"
     CALL disp_msg(" ",dm_err->logfile,0)
     FOR (lp = 1 TO 8)
       CALL pause(2)
     ENDFOR
 ...
 ENDIF
#exit_program
 SET dm_err->eproc = "...Ending dm2_cycle_data_movers"
 CALL final_disp_msg(" ")
 SET dm_err->logfile = dcdm_logfile
END GO
