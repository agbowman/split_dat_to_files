CREATE PROGRAM bed_get_mltm_drc_content:dba
 FREE SET reply
 RECORD reply(
   1 new_drc_info[*]
     2 drc_identifier = vc
 ...
 IF ((request->premise_type_flag=2))
  SET parse_txt = "m.route_id > 0"
 ELSEIF ((request->premise_type_flag=4))
  SET parse_txt = "m.renal_operator_txt > ' '"
 ELSEIF ((request->premise_type_flag=5))
  SET parse_txt = "m.corrected_gest_age_oper_txt > ' '"
 ELSEIF ((request->premise_type_flag=6))
  SET parse_txt = "m.liver_desc > ' '"
 ELSEIF ((request->premise_type_flag=7))
  SET parse_txt = "m.condition_concept_cki > ' '"
 ENDIF
 IF ((request->premise_type_flag != 7))
  SELECT INTO "nl:"
   FROM mltm_drc_premise m,
    code_value cv,
    dcp_entity_reltn d,
   ...
   PLAN (m
    ...
    (SELECT
     dp.drc_premise_id
     FROM drc_premise dp
     WHERE dp.dose_range_check_id=m.dose_range_check_id
      AND dp.drc_identifier=m.drc_identifier))))
    JOIN (cv
   ...
   HEAD m.drc_identifier
    ...
    reply->new_drc_info[cnt].age_high_nbr = m.age_high_nbr, reply->new_drc_info[cnt].age_unit_disp =
    m.age_unit_disp, reply->new_drc_info[cnt].age_unit_cki = m.age_unit_cki,
  ...
  SELECT INTO "nl:"
   ...
   HEAD m.dose_range_type_id
    ...
    new_drc_info[cnt].dose_range_list[sub_cnt].low_dose_value = m.low_dose_value,
    reply->new_drc_info[cnt].dose_range_list[sub_cnt].high_dose_value = m.high_dose_value, reply->
    new_drc_info[cnt].dose_range_list[sub_cnt].dose_unit_disp = m.dose_unit_disp, reply->
    new_drc_info[cnt].dose_range_list[sub_cnt].dose_unit_cki = m.dose_unit_cki,
    reply->new_drc_info[cnt].dose_range_list[sub_cnt].comment_txt = m.comment_txt, ftemp = findstring
    ("LEXI!",m.drc_identifier,1,1)
    IF (ftemp > 0)
     reply->new_drc_info[cnt].dose_range_list[sub_cnt].source = "Lexi-Comp"
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY m.drc_identifier, m.dose_range_type_id
  HEAD REPORT
   cnt = size(reply->new_drc_info,5), list_cnt = 0, stat = alterlist(reply->new_drc_info,100)
  HEAD m.drc_identifier
   sub_cnt = 0, sub_list_cnt = 0, list_cnt = (list_cnt+ 1),
   cnt = (cnt+ 1)
   IF (list_cnt > 100)
 ...
 SELECT INTO "nl:"
  ...
  HEAD dp.parent_premise_id
   ...
   ENDIF
   reply->current_drc_info[cnt].parent_premise_id = dp.parent_premise_id, reply->current_drc_info[cnt
   ].active_ind = 0, stat = alterlist(reply->current_drc_info[cnt].dose_range_list,5),
  ...
  FOOT  dp.parent_premise_id
   stat = alterlist(reply->current_drc_info[cnt].dose_range_list,sub_cnt), stat = alterlist(temp_curr
    ->current_drc_info[cnt].dose_range_list,sub_cnt)
  FOOT REPORT
