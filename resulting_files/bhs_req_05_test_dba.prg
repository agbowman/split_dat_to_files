CREATE PROGRAM bhs_req_05_test:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter, outerjoin = d1, dontcare = pa,
   ...
   dontcare = epr
 ;end select
 CALL echo(build("temp loc cd :",temp_cd))
 SET temp_loc_disp = trim(uar_get_code_display(temp_cd))
 CALL echo(build("temp loc disp :",temp_loc_disp))
 SET height_cd = 0
 SET weight_cd = 0
 SELECT INTO "nl:"
  FROM code_value cv
 ...
 SELECT INTO "NL:"
  ...
  PLAN (d1)
   ...
   (SELECT DISTINCT
    ...
    WHERE cve.code_set=14281
     AND cve.field_name="REQ_PRINT_FLG"
     AND substring(10,1,cve.field_value)="Y")))) )
   JOIN (pl
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(request->printer_name)
   oid = orders->qual[d1.seq].order_id
   FROM (dummyt d1  WITH seq = value(ord_count))
   PLAN (d1)
   HEAD REPORT
    xcol = 20, hold_set_ind = 0, line1 = fillstring(115,"_"),
    break_variable = 0, lastodxcol = 0, temp_loc_disp = fillstring(30,"")
   HEAD PAGE
    ...
    od_cnt = 0,
    CALL echo(build("{page :",curpage,"}{d1.seq :",d1.seq))
   HEAD oid
    IF (hold_set_ind=0)
     hold_oe_format_id = orders->qual[d1.seq].oe_format_id, hold_activity_type_cd = orders->qual[d1
     .seq].activity_type_cd, hold_set_ind = 1
    ELSE
     hold_activity_type_cd = orders->qual[d1.seq].activity_type_cd, hold_oe_format_id = orders->qual[
     d1.seq].oe_format_id
    ENDIF
    xcol = 30, ycol = (ycol+ 10),
    CALL print(calcpos(xcol,ycol)),
    "{cpi/12}{b}", line1, "{endb}",
    row + 1, xcol = 40, ycol = (ycol+ 21),
    CALL print(calcpos(xcol,ycol)), "{cpi/14}Ordering MD: ", orders->qual[d1.seq].order_dr,
    row + 1, xcol = 40, ycol = (ycol+ 11),
    CALL print(calcpos(xcol,ycol)), "Order Type: ", orders->qual[d1.seq].type,
    row + 1, xcol = 40, ycol = (ycol+ 11),
    CALL print(calcpos(xcol,ycol)), "Order Action: ", orders->qual[d1.seq].action,
    row + 1, xcol = 30, ycol = (ycol+ 15),
    CALL print(calcpos(xcol,ycol)), "{cpi/12}{b}Procedure: ", orders->qual[d1.seq].mnemonic,
    "{endb}", row + 1, xcol = 300,
    CALL print(calcpos(xcol,ycol)), "{b}Order ID: ", orders->qual[d1.seq].order_id"#########",
    "{endb}", row + 1, xcol = 30,
    ycol = (ycol+ 15),
    CALL print(calcpos(xcol,ycol)), "{cpi/12}Accession #: ",
    orders->qual[d1.seq].accession, row + 1, od_cnt = 0
   DETAIL
    od_cnt = size(orders->qual[d1.seq].d_qual,5)
    FOR (x = 1 TO od_cnt)
      IF ((orders->qual[d1.seq].d_qual[x].print_ind=0))
       orders->qual[d1.seq].d_qual[x].print_ind = 1
       IF (lastodxcol=30)
        xcol = 300, lastodxcol = xcol
       ELSE
        row + 1, xcol = 30, lastodxcol = xcol,
        ycol = (ycol+ 12)
       ENDIF
       CALL print(calcpos(xcol,ycol))
       IF (((size(orders->qual[d1.seq].d_qual[x].label_text)+ size(orders->qual[d1.seq].d_qual[x].
        value)) > 57))
        CALL print(substring(1,57,concat(orders->qual[d1.seq].d_qual[x].label_text,": ",orders->qual[
          d1.seq].d_qual[x].value))), "..."
       ELSE
        orders->qual[d1.seq].d_qual[x].label_text, ": ", orders->qual[d1.seq].d_qual[x].value
       ENDIF
      ENDIF
    ENDFOR
    IF ((orders->qual[d1.seq].comment_ind=1)
     AND (orders->qual[d1.seq].com_ln_cnt > 0))
     IF ((orders->qual[d1.seq].com_ln_cnt > 10))
      ocnt = 10
     ELSE
      ocnt = orders->qual[d1.seq].com_ln_cnt
     ENDIF
     xcol = 30, ycol = (ycol+ 15)
     FOR (com_cnt = 1 TO ocnt)
       CALL print(calcpos(xcol,ycol)), "{cpi/14}", orders->qual[d1.seq].com_ln_qual[com_cnt].com_line,
       row + 1, ycol = (ycol+ 11)
     ENDFOR
     ycol = (ycol+ 12)
    ENDIF
   ...
   FOOT PAGE
    ...
    "{cpi/12}{pos/300/676}{b}", "Printed: ", curdate,
    "  ", curtime, "{endb}",
    row + 1
    IF (temp_cd > 0)
     temp_loc_disp = uar_get_code_display(temp_cd), "{pos/465/676}", "{b/10}Temp Loc: ",
     temp_loc_disp, row + 1
    ELSE
     "{pos/465/676}", "{b/10}Location: ", orders->location,
     row + 1
    ENDIF
    "{cpi/12}{pos/300/687}", "{b/8}Patient: ", "{pos/345/687}",
    orders->name, "{pos/300/698}", "CMRN #: ",
    orders->cmrn, "{pos/410/698}", "DOB: ",
    orders->dob, "{pos/490/698}", "ADM: ",
    orders->admit_dt, row + 1, "{pos/300/709}",
    "{b/5}MRN # ", orders->mrn, "{pos/410/709}",
    "Age: ", orders->age, "{pos/490/709}",
    "Sex: ", orders->sex, row + 1,
    "{pos/300/720}", "Attending MD: ", orders->attending,
    row + 1, "{pos/490/731}", "Page # ",
    curpage, row + 1, "{cpi/10}{pos/30/693}{b}",
    orders->facility, row + 1, "{cpi/10}{pos/60/708}{b}",
    "Laboratory Order Requisition", "{font/8}{cpi/12}{pos/50/720}", "BHS_REQ_05"
