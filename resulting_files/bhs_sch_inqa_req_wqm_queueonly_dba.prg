CREATE PROGRAM bhs_sch_inqa_req_wqm_queueonly:dba
 ...
 DECLARE follow_up_date_text = vc WITH constant(uar_i18ngetmessage(i18nhandle,"key1",
   "Follow-up date - "))
 DECLARE follow_up_date_timestamp = dq8 WITH public, noconstant(0)
  ...
  SELECT INTO "nl:"
   ...
   PLAN (a
    WHERE (a.queue_id=t_record->queue_id)
     ...
     AND a.earliest_dt_tm >= cnvtdatetime(t_record->beg_dt_tm)
     AND a.earliest_dt_tm <= cnvtdatetime(t_record->end_dt_tm)
     AND a.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
    JOIN (ea
   ...
   HEAD a.sch_action_id
    ...
    ENDIF
    sch_inqa_reply->query_qual[sch_inqa_reply->query_qual_cnt].images = "N"
   FOOT REPORT
 ...
 FOR (i = 1 TO listsize)
   ...
   SET sch_inqa_reply->query_qual[sch_inqa_reply->query_qual_cnt].images = "N"
