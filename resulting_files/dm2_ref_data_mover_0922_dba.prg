CREATE PROGRAM dm2_ref_data_mover_0922:dba
 ...
 IF (src_task_num >= 117000
  AND src_task_num <= 118999)
  IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
   SET cust_cnt = 0
   SET cust_new_task = 0
   IF (src_task_num <= 117999)
    SET src_tbl_conv = dm2_get_rdds_tname("PM_FLX_CONVERSATION")
    SET cust_min_task = 117000
    SET cust_max_task = 117999
    SELECT INTO "NL:"
     FROM (parser(src_tbl_conv) p)
     WHERE (p.task=rs_0922->from_values.task_number)
     DETAIL
      src_conv_desc = p.description
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (curqual > 0)
     SELECT INTO "NL:"
      ...
      PLAN (a
       ...
       JOIN (p
       WHERE p.task=a.task_number
        AND p.description=src_conv_desc)
      DETAIL
       cust_new_task = a.task_number, cust_cnt = (cust_cnt+ 1)
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSEIF (cust_cnt > 1)
    SET dm_err->err_ind = 0
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "There were multiple task_numbers found with the UI query"
    GO TO exit_0922
   ELSE
