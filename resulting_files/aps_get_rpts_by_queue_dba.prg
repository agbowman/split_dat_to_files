CREATE PROGRAM aps_get_rpts_by_queue:dba
 ...
 SELECT INTO "nl:"
  ...
  pc.case_id, sd.short_description, prr.primary_ind,
  crc.catalog_cd, cyto_rpt = decode(crc.seq,1,0), pr1.name_full_formatted,
  pr2.name_full_formatted, pr3.name_full_formatted, pr4.name_full_formatted,
  ...
  HEAD rt.report_id
   ...
   IF (isresourceviewable(service_resource_cd)=true)
    IF (report_cnt=0)
     ...
     case_cnt].chr_ind = pc.chr_ind, reply->case_qual[case_cnt].pc_updt_cnt = pc.updt_cnt,
     ...
     case_cnt].req_physician_name = pr3.name_full_formatted, reply->case_qual[case_cnt].
     case_comment_long_text_id = pc.comments_long_text_id,
    ...
    report_qual[report_cnt].report_queue_seq = rqr.sequence, reply->case_qual[case_cnt].report_qual[
    ...
    resp_pathologist_name = pr4.name_full_formatted,
    reply->case_qual[case_cnt].report_qual[report_cnt].resp_resident_id = rt.responsible_resident_id,
    reply->case_qual[case_cnt].report_qual[report_cnt].resp_resident_name = pr5.name_full_formatted,
    reply->case_qual[case_cnt].report_qual[report_cnt].request_priority_cd = rt.priority_cd,
    reply->case_qual[case_cnt].report_qual[report_cnt].report_comment_long_text_id = rt
    .comments_long_text_id, reply->case_qual[case_cnt].report_qual[report_cnt].status_cd = cr
    .status_cd, reply->case_qual[case_cnt].report_qual[report_cnt].report_sequence = cr
    .report_sequence,
    reply->case_qual[case_cnt].report_qual[report_cnt].catalog_cd = cr.catalog_cd, reply->case_qual[
    case_cnt].report_qual[report_cnt].hold_cd = rt.hold_cd, reply->case_qual[case_cnt].report_qual[
    report_cnt].hold_comment_long_text_id = rt.hold_comment_long_text_id,
    reply->case_qual[case_cnt].report_qual[report_cnt].updt_cnt = rt.updt_cnt, reply->case_qual[
    case_cnt].report_qual[report_cnt].cr_updt_cnt = cr.updt_cnt, reply->case_qual[case_cnt].
    report_qual[report_cnt].last_edit_dt_tm = cnvtdatetime(rt.last_edit_dt_tm),
    reply->case_qual[case_cnt].report_qual[report_cnt].description = sd.description, reply->
    case_qual[case_cnt].report_qual[report_cnt].short_description = sd.short_description, reply->
    case_qual[case_cnt].report_qual[report_cnt].cyto_report_ind = cyto_rpt,
    reply->case_qual[case_cnt].report_qual[report_cnt].primary_ind = prr.primary_ind
 ...
 ;end select
 IF (curqual=0)
  CALL handle_errors("SELECT","Z","TABLE","REPORT_BY_QUEUE:CASE INFORMATION")
 ...
 ;end select
 SELECT INTO "nl:"
  d1.seq, p.person_id, pr.name_full_formatted,
  cs.specimen_description, cp.physician_id, spec_exists = decode(cs.seq,"Y","N"),
 ...
 ENDIF
 SELECT INTO "nl:"
  reply->case_qual[d1.seq].encntr_id, frmt_mrn = cnvtalias(ea.alias,ea.alias_pool_cd), ea.*
  FROM (dummyt d1  WITH seq = value(size(reply->case_qual,5))),
   encntr_alias ea
  PLAN (d1)
   JOIN (ea
