CREATE PROGRAM bhs_sfe_outcomes_summary2:dba
 PROMPT
 ...
 DECLARE expand_stop = i4 WITH noconstant(50), protect
 CALL echo("2")
 ...
 IF (mod(actual_size,50) != 0)
  SET expand_total = (actual_size+ (50 - mod(actual_size,50)))
 ...
 SELECT INTO "NL:"
  ...
  FROM clinical_event ce,
   person p,
   encntr_alias ea,
   (dummyt d  WITH seq = value((expand_total/ 50)))
  PLAN (d
   WHERE assign(expand_start,evaluate(d.seq,1,1,(expand_start+ 50)))
    AND assign(expand_stop,(expand_start+ 49)))
   JOIN (ce
 ...
 ELSE
  ...
  SELECT INTO  $OUTDEV
   encntr = formactlist2->qual[d.seq].encntr_id
