CREATE PROGRAM ce_med_result_upd:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 DECLARE bolus_validate = vc WITH protect, constant(
  "validate(request->lst[d.seq].bolus_type_cd, t.bolus_type_cd)")
 DECLARE error_msg = vc WITH protect, noconstant("")
 SET error_code = 0
 UPDATE  FROM ce_med_result t,
  ...
  SET t.event_id = evaluate2(
   ...
   t.admin_start_dt_tm = evaluate2(
    ...
    ENDIF
    ), t.admin_start_tz = request->lst[d.seq].admin_start_tz, t.admin_end_dt_tm = evaluate2(
    IF ((request->lst[d.seq].admin_end_dt_tm_ind=1)) null
    ELSE cnvtdatetimeutc(request->lst[d.seq].admin_end_dt_tm)
    ENDIF
    ),
   t.admin_end_tz = request->lst[d.seq].admin_end_tz, t.admin_route_cd = evaluate2(
   ...
   t.admin_strength = evaluate2(
    ...
    ENDIF
    ), t.substance_lot_number = request->lst[d.seq].substance_lot_number,
   t.substance_exp_dt_tm = evaluate2(
   ...
   t.iv_event_cd = evaluate2(
    IF ((request->lst[d.seq].iv_event_cd=- (1))) 0
    ELSE request->lst[d.seq].iv_event_cd
    ENDIF
   ...
   t.valid_until_dt_tm = evaluate2(
    ...
    ENDIF
    ), t.infused_volume = evaluate2(
    IF ((request->lst[d.seq].infused_volume_ind=1)) null
    ELSE request->lst[d.seq].infused_volume
    ENDIF
    ), t.infused_volume_unit_cd = evaluate2(
    IF ((request->lst[d.seq].infused_volume_unit_cd=- (1))) 0
    ELSE request->lst[d.seq].infused_volume_unit_cd
    ENDIF
    ),
   t.remaining_volume = evaluate2(
    IF ((request->lst[d.seq].remaining_volume_ind=1)) null
    ELSE request->lst[d.seq].remaining_volume
    ENDIF
    ), t.remaining_volume_unit_cd = evaluate2(
    IF ((request->lst[d.seq].remaining_volume_unit_cd=- (1))) 0
    ELSE request->lst[d.seq].remaining_volume_unit_cd
    ENDIF
    ), t.synonym_id = evaluate2(
    IF ((request->lst[d.seq].synonym_id=- (1))) 0
    ELSE request->lst[d.seq].synonym_id
    ENDIF
    ),
   ...
   .immunization_type_cd = request->lst[d.seq].immunization_type_cd,
   t.weight_value = evaluate2(
    IF ((request->lst[d.seq].weight_value=- (1))) 0
    ELSE request->lst[d.seq].weight_value
    ENDIF
    ), t.weight_unit_cd = evaluate2(
    IF ((request->lst[d.seq].weight_unit_cd=- (1))) 0
    ELSE request->lst[d.seq].weight_unit_cd
    ENDIF
    ), t.bolus_type_cd = evaluate2(
    IF (parser(bolus_validate)=0.0) null
    ELSE parser(bolus_validate)
    ENDIF
    )
  PLAN (d)
   JOIN (t
   WHERE (t.event_id=request->lst[d.seq].event_id)
    AND t.admin_start_dt_tm=cnvtdatetimeutc(request->lst[d.seq].admin_start_dt_tm)
