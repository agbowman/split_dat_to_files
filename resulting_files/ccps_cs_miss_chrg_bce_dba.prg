CREATE PROGRAM ccps_cs_miss_chrg_bce:dba
 PROMPT
 ...
 CALL createldorgparsers(reqinfo->updt_id,"o.logical_domain_id",4,"e.organization_id")
 IF (((ld_parser="0=1") OR (organization_parser="0=1")) )
  ...
  GO TO exit_prg
 ENDIF
 FREE RECORD missed_bce_charges
 RECORD missed_bce_charges(
   1 mbc_cnt = i4
   1 mbc_detail[*]
     2 org_name = vc
     2 org_id = f8
 ...
 SELECT INTO "nl:"
  FROM bce_event_log bel,
   ...
   prsnl p3
  PLAN (bel
   WHERE bel.updt_dt_tm BETWEEN cnvtdatetime(beg_dt_tm) AND cnvtdatetime(end_dt_tm)
    AND  NOT ( EXISTS (
   (SELECT
    c.bill_item_id
    FROM charge c
    WHERE c.bill_item_id=bel.bill_item_id
     AND c.active_ind=1))))
   JOIN (e
   WHERE e.encntr_id=bel.encntr_id
   ...
   JOIN (bi
   WHERE bi.bill_item_id=bel.bill_item_id
   ...
   JOIN (p3
   WHERE p3.person_id=bel.updt_id)
   JOIN (eafin
   WHERE eafin.encntr_id=outerjoin(bel.encntr_id)
   ...
   JOIN (p2
   WHERE p2.person_id=outerjoin(bel.ren_phys_id)
  ...
  HEAD o.organization_id
   mbc_cnt = (mbc_cnt+ 1)
   IF (mod(mbc_cnt,10)=1)
    stat = alterlist(missed_bce_charges->mbc_detail,(mbc_cnt+ 9))
   ENDIF
   missed_bce_charges->mbc_detail[mbc_cnt].org_name = o.org_name, missed_bce_charges->mbc_detail[
   mbc_cnt].org_id = o.organization_id, missed_cnt = 0
  DETAIL
   missed_cnt = (missed_cnt+ 1)
   IF (mod(missed_cnt,10)=1)
    stat = alterlist(missed_bce_charges->mbc_detail[mbc_cnt].missed_detail,(missed_cnt+ 9))
   ENDIF
   missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[missed_cnt].patient_name = p
   .name_full_formatted, missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[missed_cnt].fin =
   cnvtalias(eafin.alias,eafin.alias_pool_cd), missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[
   missed_cnt].name_last_key = p.name_last_key,
   missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[missed_cnt].name_first_key = p
   .name_first_key, missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[missed_cnt].activity_type
    = uar_get_code_display(bi.ext_owner_cd), missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[
   missed_cnt].charge_desc = bel.charge_description,
   missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[missed_cnt].service_d_t = build2(format(bel
     .service_dt_tm,"@SHORTDATE")," ",format(bel.service_dt_tm,"@TIMENOSECONDS")), missed_bce_charges
   ->mbc_detail[mbc_cnt].missed_detail[missed_cnt].quantity = bel.quantity, missed_bce_charges->
   mbc_detail[mbc_cnt].missed_detail[missed_cnt].rendering_phys = p2.name_full_formatted,
   missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[missed_cnt].username = p3.username,
   missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[missed_cnt].bill_item_id = bel.bill_item_id,
   missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[missed_cnt].bce_event_log_id = bel
   .bce_event_log_id,
   missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[missed_cnt].encounter_id = bel.encntr_id,
   missed_bce_charges->mbc_detail[mbc_cnt].missed_detail[missed_cnt].person_id = bel.person_id
  FOOT  o.organization_id
   stat = alterlist(missed_bce_charges->mbc_detail[mbc_cnt].missed_detail,missed_cnt),
   missed_bce_charges->mbc_detail[mbc_cnt].missed_cnt = missed_cnt
  FOOT REPORT
   stat = alterlist(missed_bce_charges->mbc_detail,mbc_cnt), missed_bce_charges->mbc_cnt = mbc_cnt
  WITH nocounter
 ;end select
 EXECUTE reportrtl
 ...
 SUBROUTINE headpagesection0abs(ncalc,offsetx,offsety)
   ...
   DECLARE __fieldname0 = vc WITH noconstant(build2(missed_bce_charges->mbc_detail[i].org_name,char(0
      ))), protect
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 1.094),(offsetx+ 10.500),(offsety
     + 1.094))
    ...
    SET rptsd->m_width = 1.552
    SET rptsd->m_height = 0.260
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("RENDERING PHYSICIAN",char(0)))
    SET rptsd->m_y = (offsety+ 0.844)
    SET rptsd->m_x = (offsetx+ 7.000)
    SET rptsd->m_width = 0.625
    SET rptsd->m_height = 0.260
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("QUANTITY",char(0)))
    SET rptsd->m_flags = 4
    SET rptsd->m_y = (offsety+ 0.844)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
      "Missed Charges Submitted via Batch Charge Entry Report",char(0)))
 ...
 SUBROUTINE headorgsection0abs(ncalc,offsetx,offsety)
   ...
   DECLARE __fieldname2 = vc WITH noconstant(build2(concat("Total Missed Charges for ",trim(
       missed_bce_charges->mbc_detail[i].org_name),":  ",trim(cnvtstring(missed_bce_charges->
        mbc_detail[i].missed_cnt))),char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 16
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
 ...
 SUBROUTINE detailsection0abs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   DECLARE __fieldname4 = vc WITH noconstant(build2(missed_bce_charges->mbc_detail[i].missed_detail[
     ii].charge_desc,char(0))), protect
   DECLARE __fieldname7 = vc WITH noconstant(build2(missed_bce_charges->mbc_detail[i].missed_detail[
     ii].activity_type,char(0))), protect
   DECLARE __fieldname0 = vc WITH noconstant(build2(missed_bce_charges->mbc_detail[i].missed_detail[
     ii].service_d_t,char(0))), protect
   DECLARE __fieldname1 = vc WITH noconstant(build2(missed_bce_charges->mbc_detail[i].missed_detail[
     ii].quantity,char(0))), protect
   DECLARE __fieldname2 = vc WITH noconstant(build2(missed_bce_charges->mbc_detail[i].missed_detail[
     ii].rendering_phys,char(0))), protect
   DECLARE __fieldname3 = vc WITH noconstant(build2(missed_bce_charges->mbc_detail[i].missed_detail[
     ii].patient_name,char(0))), protect
   DECLARE __fieldname6 = vc WITH noconstant(build2(missed_bce_charges->mbc_detail[i].missed_detail[
     ii].fin,char(0))), protect
   DECLARE __fieldname5 = vc WITH noconstant(build2(trim(missed_bce_charges->mbc_detail[i].
      missed_detail[ii].username),char(0))), protect
   IF (bcontinue=0)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CCPS_CS_MISS_CHRG_BCE"
 ...
 IF ((missed_bce_charges->mbc_cnt > 0))
  SELECT INTO  $OUTDEV
   org_name_sort = cnvtupper(missed_bce_charges->mbc_detail[d.seq].org_name), patient_name_sort =
   concat(missed_bce_charges->mbc_detail[d.seq].missed_detail[d1.seq].name_last_key," ",
    missed_bce_charges->mbc_detail[d.seq].missed_detail[d1.seq].name_first_key," ",trim(cnvtstring(
      missed_bce_charges->mbc_detail[d.seq].missed_detail[d1.seq].person_id))), charge_desc_sort =
   cnvtupper(missed_bce_charges->mbc_detail[d.seq].missed_detail[d1.seq].charge_desc)
   FROM (dummyt d  WITH seq = value(missed_bce_charges->mbc_cnt)),
    (dummyt d1  WITH seq = 1)
   PLAN (d
    WHERE maxrec(d1,missed_bce_charges->mbc_detail[d.seq].missed_cnt))
    JOIN (d1)
   ORDER BY org_name_sort, patient_name_sort, charge_desc_sort
   HEAD REPORT
    first_org = 1
   HEAD PAGE
    i = d.seq, d0 = headpagesection0(rpt_render)
   HEAD org_name_sort
    IF (first_org=1)
     first_org = 0
    ELSE
     d0 = pagebreak(0), BREAK
    ENDIF
 ...
 ELSE
  SELECT INTO  $OUTDEV
   ...
   HEAD REPORT
    display_message =
    "There are no missed charges submitted via Batch Charge Entry for the selected prompts.", d0 =
...
#exit_prg
 IF (ld_failure_flag=1)
  ...
  SUBROUTINE headpagesection0abs(ncalc,offsetx,offsety)
    DECLARE sectionheight = f8 WITH noconstant(1.150000), private
    DECLARE __fieldname0 = vc WITH noconstant(build2(missed_bce_charges->mbc_detail[i].org_name,char(
       0))), protect
    IF (ncalc=rpt_render)
     SET rptsd->m_flags = 64
     SET rptsd->m_borders = rpt_sdnoborders
  ...
  SET d0 = finalizereport( $OUTDEV)
 ENDIF
 SET last_mod = "004 05/28/12 MP9098"
END GO
