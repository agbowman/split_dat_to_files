CREATE PROGRAM cp_add_concept_group_dtl
 IF (validate(request) != 1)
  RECORD request(
    1 detail_list[*]
      ...
      2 intention_cd = f8
      2 concept_entity_id = f8
      2 concept_entity_name = vc
      2 concept_entity_ident = vc
      2 concept_entity_text = vc
  )
 ENDIF
 RECORD reply(
   1 detail_list[*]
     ...
     2 concept_entity_ident = vc
     2 concept_entity_text = vc
     2 concept_entity_disp = vc
     2 cp_concept_group_dtl_id = f8
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE detaillistsize = i4 WITH noconstant(size(request->detail_list,5)), protect
   DECLARE concept_cd_codeset = i4 WITH constant(4003132), protect
   DECLARE concept_detail_reltn_cd_codeset = i4 WITH constant(4003133), protect
   DECLARE intention_cd_codeset = i4 WITH constant(4003278), protect
   IF (detaillistsize <= 0)
    CALL reporterrorandexit("PERFORM","Validate","Detail list must be nonzero in size.")
   ENDIF
   FOR (x = 1 TO detaillistsize)
     CALL verifynonzerocodevalue(request->detail_list[x].concept_cd,concept_cd_codeset,"Concept")
     CALL verifynonzerocodevalue(request->detail_list[x].concept_group_cd,
      concept_detail_reltn_cd_codeset,"Concept Detail Realtionship")
     CALL verifynonzerocodevalue(request->detail_list[x].intention_cd,intention_cd_codeset,
      "Intention")
   ENDFOR
 ...
 SUBROUTINE insertrows(null)
   CALL log_message("In insertRow()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE x = i4 WITH noconstant(0), protect
   DECLARE dcurconceptcd = f8 WITH noconstant(0), protect
   DECLARE dcurconceptgroupcd = f8 WITH noconstant(0), protect
   DECLARE dcurintentioncd = f8 WITH noconstant(0), protect
   DECLARE cp_concept_group_dtl_id = f8 WITH noconstant(0), protect
   DECLARE skipdetailind = i4 WITH noconstant(0), protect
   IF (checkdic("CP_CONCEPT_GROUP","T",0) != 2
    AND checkdic("CP_CONCEPT_GROUP_DTL","T",0) != 2)
    CALL reporterrorandexit("PERFORM","INSERT",
     " CP_CONCEPT_GROUP and CP_CONCEPT_GROUP_DTL do not exist or permission is not granted to access it."
     )
   ENDIF
   SET stat = alterlist(reply->detail_list,size(request->detail_list,5))
   FOR (x = 1 TO size(request->detail_list,5))
     SET dcurconceptcd = request->detail_list[x].concept_cd
     SET dcurconceptgroupcd = request->detail_list[x].concept_group_cd
     SET dcurintentioncd = request->detail_list[x].intention_cd
     SET dconceptgroupid = checkforexistinggroup(dcurconceptcd,dcurconceptgroupcd,dcurintentioncd)
     CALL echo(build2("Exisiting Group ID:",dconceptgroupid))
     IF (dconceptgroupid=0.0)
      SET dconceptgroupid = addconceptgroup(dcurconceptcd,dcurconceptgroupcd,dcurintentioncd)
     ENDIF
     IF (dconceptgroupid=0.0)
      CALL reporterror("PERFORM","INSERT"," Unable to get CP_CONCEPT_GROUP_ID.")
      ROLLBACK
      GO TO exit_script
     ENDIF
     IF ((request->detail_list[x].concept_entity_id <= 0)
      AND (request->detail_list[x].concept_entity_name=""))
      IF ((request->detail_list[x].concept_entity_text="")
       AND (request->detail_list[x].concept_entity_ident=""))
       SET skipdetailind = 1
      ENDIF
     ENDIF
     IF (skipdetailind=0)
      SET cp_concept_group_dtl_id = 0.0
      SELECT INTO "nl:"
       nextseqnum = seq(reference_seq,nextval)
       FROM dual
       DETAIL
        cp_concept_group_dtl_id = nextseqnum
       WITH nocounter
      ;end select
      CALL echo(build2("Next CP_CONCEPT_GROUP_DTL_ID:",cp_concept_group_dtl_id))
      IF (cp_concept_group_dtl_id=0.0)
       CALL reporterror("PERFORM","INSERT",
        " Unable to generate cp_concept_group_dtl_id from REFERENCE_SEQ.")
       ROLLBACK
       GO TO exit_script
      ENDIF
      INSERT  FROM cp_concept_group_dtl ccgd
       SET ccgd.cp_concept_group_id = dconceptgroupid, ccgd.cp_concept_group_dtl_id =
        cp_concept_group_dtl_id, ccgd.concept_entity_id = request->detail_list[x].concept_entity_id,
        ccgd.concept_entity_name = request->detail_list[x].concept_entity_name, ccgd
        .concept_entity_ident = request->detail_list[x].concept_entity_ident, ccgd
        .concept_entity_text = request->detail_list[x].concept_entity_text
       WITH nocounter
      ;end insert
      CALL error_and_zero_check(curqual,"INSERT","cp_concept_group_dtl",1,1)
      SET reply->detail_list[x].concept_cd = request->detail_list[x].concept_cd
      SET reply->detail_list[x].cp_concept_group_dtl_id = cp_concept_group_dtl_id
      SET reply->detail_list[x].concept_entity_id = request->detail_list[x].concept_entity_id
      SET reply->detail_list[x].concept_entity_name = request->detail_list[x].concept_entity_name
      SET reply->detail_list[x].concept_group_cd = request->detail_list[x].concept_group_cd
      SET reply->detail_list[x].concept_entity_ident = request->detail_list[x].concept_entity_ident
      SET reply->detail_list[x].concept_entity_text = request->detail_list[x].concept_entity_text
      SET reply->detail_list[x].concept_entity_disp = request->detail_list[x].concept_entity_text
     ENDIF
   ENDFOR
   SET reqinfo->commit_ind = 1
   CALL log_message(build("Exit insertRow(), Elapsed time in seconds:",((curtime3 - begin_curtime3)/
     100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (checkforexistinggroup(dconceptcd=f8,dconceptgroupcd=f8,dintentioncd=f8) =f8 WITH protect
  )
   CALL log_message("In CheckForExistingGroup()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE dconceptgroupid = f8 WITH noconstant(0), protect
   SELECT INTO "NL:"
    FROM cp_concept_group ccg
    WHERE ccg.concept_group_cd=dconceptgroupcd
     AND ccg.intention_cd=dintentioncd
     AND ccg.concept_cd=dconceptcd
    DETAIL
     dconceptgroupid = ccg.cp_concept_group_id
    WITH nocounter
   ;end select
   CALL log_message(build("Exit CheckForExistingGroup(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(dconceptgroupid)
 END ;Subroutine
 SUBROUTINE (addconceptgroup(dconceptcd=f8,dconceptgroupcd=f8,dintentioncd=f8) =f8 WITH protect)
   CALL log_message("In AddConceptGroup()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE dconceptgroupid = f8 WITH noconstant(0), protect
   RECORD conceptgroupreq(
     1 group_list[*]
       2 concept_group_cd = f8
       2 concept_cd = f8
       2 intention_cd = f8
   ...
   RECORD conceptgrouprep(
     1 group_list[*]
       2 concept_group_cd = f8
       2 concept_cd = f8
       2 intention_cd = f8
       2 rule_ind = i2
       2 cp_concept_group_id = f8
     1 status_data
       2 status = c1
       2 subeventstatus[1]
         3 operationname = c25
   ...
   SET stat = alterlist(conceptgroupreq->group_list,1)
   SET conceptgroupreq->group_list[1].concept_group_cd = dconceptgroupcd
   SET conceptgroupreq->group_list[1].concept_cd = dconceptcd
   SET conceptgroupreq->group_list[1].intention_cd = dintentioncd
   SET conceptgroupreq->group_list[1].rule_ind = 0
   EXECUTE cp_add_concept_group  WITH replace("REQUEST","CONCEPTGROUPREQ"), replace("REPLY",
    "CONCEPTGROUPREP")
   IF ((conceptgrouprep->status_data.status="S"))
    SET dconceptgroupid = conceptgrouprep->group_list[1].cp_concept_group_id
   ELSE
    CALL reporterrorandexit("INSERT","CP_CONCEPT_GROUP","Unable to add new concept group.")
   ENDIF
   CALL log_message(build("Exit AddConceptGroup(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(dconceptgroupid)
 END ;Subroutine
 ...
 SUBROUTINE (verifynonzerocodevalue(codevalue=f8,codeset=i4,codesetname=vc) =null WITH protect)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE codevaluevalid = i2 WITH noconstant(0), protect
   IF (codevalue > 0)
