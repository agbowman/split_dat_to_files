CREATE PROGRAM bhs_docs_meds_recon_rpt:dba
 PROMPT
 ...
 SET home_med_recon_cd = uar_get_code_by("DISPLAYKEY",72,"MEDRECONCILIATION")
 SET parent_cd = uar_get_code_by("DISPLAYKEY",24,"C")
 ...
 SELECT DISTINCT INTO  $OUTDEV
  ...
  attend_dr = trim(p1.name_full_formatted), loc_dt2 = e.reg_dt_tm"mm/dd/yy hh:mm;;d", discharge_dt =
  e.disch_dt_tm"mm/dd/yy hh:mm;;d",
  loc_dt = elh.updt_dt_tm"mm/dd/yy hh:mm;;d", loc = trim(uar_get_code_display(elh.loc_nurse_unit_cd)),
  signed_within_24 = ce.result_val,
  complete_dt = ce.verified_dt_tm"@SHORTDATETIME", status =
  ...
  PLAN (ed
   ...
   WHERE ce.encntr_id=outerjoin(e.encntr_id)
    ...
    AND ce.result_val=outerjoin( $MEDRECRESULT)
    AND ce.event_reltn_cd=outerjoin(parent_cd)
  ...
  HEAD PAGE
   ...
   CALL print(calcpos(325,(y_pos+ 0))), "Medication Reconciliation Detail Report", row + 1,
   row + 1,
   ...
   CALL print(calcpos(700,(y_pos+ 56))),
   "Med",
   CALL print(calcpos(700,(y_pos+ 64))), "Result",
   row + 1,
   ...
   "Unit Admit Time",
   CALL print(calcpos(530,(y_pos+ 72))), "Disch Time",
   CALL print(calcpos(585,(y_pos+ 72))), "MRN",
   CALL print(calcpos(630,(y_pos+ 72))),
   "Completed",
   CALL print(calcpos(700,(y_pos+ 72))), "Entered",
  ...
  DETAIL
   ...
   ENDIF
   row + 1,
   CALL print(calcpos(534,(y_pos+ 0))), discharge_dt,
   row + 1,
