CREATE PROGRAM cv_da_add_cv_step_ref:dba
 IF (validate(stat)=0)
 ...
 SUBROUTINE (cv_log_msg_post(script_vrsn=vc) =null)
  ...
  ENDIF
 END ;Subroutine
 DECLARE cv_da_add_cv_step_ref_vrsn = vc WITH private, constant("20060905")
 ...
 WHILE (i <= size(request->objarray,5))
   SET did = 0.0
   IF (validate(request->objarray[i].task_assay_cd,- (0.00001)) <= 0.0)
    SET did = id_list->item[i].id
    SET request->objarray[i].task_assay_cd = did
   ELSE
    SET did = request->objarray[i].task_assay_cd
   ENDIF
   IF (validate(reply->insert_ids)=1)
    SET reply->insert_ids[i].id = did
   ENDIF
   INSERT  FROM cv_step_ref c
    SET c.task_assay_cd = did, c.step_level_flag =
     IF ((validate(request->objarray[i].step_level_flag,- (1)) != - (1))) validate(request->objarray[
       i].step_level_flag,- (1))
     ELSE 0
     ENDIF
     , c.doc_type_cd =
     ...
     ,
     c.proc_status_cd =
     IF ((validate(request->objarray[i].proc_status_cd,- (0.00001)) != - (0.00001))) validate(request
       ->objarray[i].proc_status_cd,- (0.00001))
     ELSE 0.0
     ENDIF
     , c.study_reltn_flag =
 ...
 END ;Subroutine
#end_program
 CALL cv_log_msg_post("002 12/01/2017 MG023115")
END GO
