CREATE PROGRAM dash_cqd_incomp_proc:dba
 PROMPT
 ...
 END ;Subroutine
 RECORD json_data(
   1 num_recurrent_sigmoid_looping = i4
   1 num_anatomic_abnormality = i4
   1 num_poor_prep = i4
   1 num_stricture = i4
   1 num_other = i4
   1 denominator = i4
   1 pct_recurrent_sigmoid_looping = f8
   1 pct_anatomic_abnormality = f8
   1 pct_poor_prep = f8
   1 pct_stricture = f8
   1 pct_other = f8
 )
 SET terms->qual_cnt = 6
 SET stat = alterlist(terms->qual,terms->qual_cnt)
 SET terms->qual[1].term_disp = "Procedure incomplete+"
 SET terms->qual[1].term_id = tidprocedureincomplete
 SET terms->qual[2].term_disp = "Recurrent sigmoid looping"
 SET terms->qual[2].term_id = tidrecurrentsigmoidlooping
 SET terms->qual[3].term_disp = "Anatomic abnormality"
 SET terms->qual[3].term_id = tidanatomicabnormality
 SET terms->qual[4].term_disp = "Poor prep"
 SET terms->qual[4].term_id = tidpoorprep
 SET terms->qual[5].term_disp = "Stricture"
 SET terms->qual[5].term_id = tidstricture
 SET terms->qual[6].term_disp = "OTHER"
 SET terms->qual[6].term_id = tidother
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY st.scd_story_id, st.scd_sentence_id, st.scr_term_id
  HEAD st.scd_sentence_id
   incomplete_ind = 0, recurrent_sigmoid_looping_cnt = 0, anatomic_abnormality_cnt = 0,
   poor_prep_cnt = 0, stricture_cnt = 0, other_cnt = 0
  DETAIL
   IF (st.scr_term_id != tidprocedureincomplete)
    ...
    ENDIF
   ENDIF
   CALL echo(build("st.scr_term_id: ",st.scr_term_id))
   CASE (st.scr_term_id)
    OF tidprocedureincomplete:
     incomplete_ind = 1
    OF tidrecurrentsigmoidlooping:
     recurrent_sigmoid_looping_cnt = (recurrent_sigmoid_looping_cnt+ 1)
    OF tidanatomicabnormality:
     anatomic_abnormality_cnt = (anatomic_abnormality_cnt+ 1)
    OF tidpoorprep:
     poor_prep_cnt = (poor_prep_cnt+ 1)
    OF tidstricture:
     stricture_cnt = (stricture_cnt+ 1)
    OF tidother:
     other_cnt = (other_cnt+ 1)
   ENDCASE
  FOOT  st.scd_sentence_id
   CALL echo(build("incomplete_ind: ",incomplete_ind))
   IF (incomplete_ind=1)
    json_data->denominator = (json_data->denominator+ 1), json_data->num_recurrent_sigmoid_looping =
    (json_data->num_recurrent_sigmoid_looping+ recurrent_sigmoid_looping_cnt), json_data->
    num_anatomic_abnormality = (json_data->num_anatomic_abnormality+ anatomic_abnormality_cnt),
    json_data->num_poor_prep = (json_data->num_poor_prep+ poor_prep_cnt), json_data->num_stricture =
    (json_data->num_stricture+ stricture_cnt), json_data->num_other = (json_data->num_other+
    other_cnt)
   ENDIF
  FOOT REPORT
   json_data->pct_recurrent_sigmoid_looping = cnvtreal(((json_data->num_recurrent_sigmoid_looping/
    cnvtreal(json_data->denominator)) * 100)), json_data->pct_anatomic_abnormality = cnvtreal(((
    json_data->num_anatomic_abnormality/ cnvtreal(json_data->denominator)) * 100)), json_data->
   pct_poor_prep = cnvtreal(((json_data->num_poor_prep/ cnvtreal(json_data->denominator)) * 100)),
   json_data->pct_stricture = cnvtreal(((json_data->num_stricture/ cnvtreal(json_data->denominator))
     * 100)), json_data->pct_other = cnvtreal(((json_data->num_other/ cnvtreal(json_data->denominator
     )) * 100))
  WITH nocounter, expand = 1
 ;end select
 CALL echorecord(json_data)
#exit_script
 IF ((((((json_data->pct_recurrent_sigmoid_looping+ json_data->pct_anatomic_abnormality)+ json_data->
 pct_poor_prep)+ json_data->pct_stricture)+ json_data->pct_other) > 0))
  SET chart_data->results = build2("[[",'["Recurrent sigmoid looping",',trim(cnvtstring(json_data->
     pct_recurrent_sigmoid_looping,5,1)),"],",'["Anatomic abnormality",',
   trim(cnvtstring(json_data->pct_anatomic_abnormality,5,1)),"],",'["Poor prep",',trim(cnvtstring(
     json_data->pct_poor_prep,5,1)),"],",
   '["Stricture",',trim(cnvtstring(json_data->pct_stricture,5,1)),"],",'["Other",',trim(cnvtstring(
     json_data->pct_other,5,1)),
   "]","]]")
 ENDIF
