CREATE PROGRAM bhs_gen_disch_dt_tm_null_cond:dba
 SET trace = nocost
   ...
   ELSE
    ...
    FOR (i = 1 TO encntr_count)
      UPDATE  FROM encounter e
       SET e.disch_dt_tm = cnvtdatetime(encntrs->encntr[i].reg_dt_tm), e.reg_dt_tm = cnvtdatetime(
         encntrs->encntr[i].reg_dt_tm), e.depart_dt_tm = cnvtdatetime(encntrs->encntr[i].reg_dt_tm),
        e.updt_task = 3366, e.updt_cnt = (e.updt_cnt+ 1), e.updt_dt_tm = cnvtdatetime(curdate,
         curtime3),
      ...
      UPDATE  FROM encntr_loc_hist elh
       SET elh.end_effective_dt_tm = cnvtdatetime(curdate,curtime3), elh.updt_task = 3366, elh
        .updt_cnt = (elh.updt_cnt+ 1),
        elh.updt_dt_tm = cnvtdatetime(curdate,curtime3), elh.updt_id = 3366
       WHERE (elh.encntr_id=encntrs->encntr[i].encntr_id)
      ...
      UPDATE  FROM encntr_domain ed
       SET ed.end_effective_dt_tm = cnvtdatetime(encntrs->encntr[i].reg_dt_tm), ed.updt_task = 3366,
        ed.updt_cnt = (ed.updt_cnt+ 1),
        ed.updt_dt_tm = cnvtdatetime(curdate,curtime3), ed.updt_id = 3366
       WHERE (ed.encntr_id=encntrs->encntr[i].encntr_id)
