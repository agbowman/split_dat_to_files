CREATE PROGRAM dm_upt_pa_criteria_value:dba
 ...
 FOR (cv_cnt = 1 TO nbr_of_cv)
   IF ((request->criteria_id != 0)
    ...
    UPDATE  FROM pa_criteria_value
     SET value =
      IF ((request->crit_value[cv_cnt].value_null_ind=1)) null
      ELSE trim(cnvtupper(request->crit_value[cv_cnt].value))
      ENDIF
      , updt_cnt = (updt_cnt+ 1), updt_dt_tm = cnvtdatetime(curdate,curtime3),
      updt_id = reqinfo->updt_id, updt_task = reqinfo->updt_task, updt_applctx = reqinfo->
      updt_applctx
