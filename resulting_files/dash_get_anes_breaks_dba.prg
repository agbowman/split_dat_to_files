CREATE PROGRAM dash_get_anes_breaks:dba
 PROMPT
 ...
 DECLARE idx = i4 WITH protect, noconstant(0)
 SET log_program_name = "dash_get_anes_breaks"
 CALL log_message(concat("Begin script ",log_program_name),log_level_debug)
 SET chart_data->status_data.status = "Z"
 INSERT  FROM dash_break_hist
  (provider_id, status_flag, position_ident,
  provider_type_flag, record_dt_tm, status_dt_tm,
  exclude_ind, updt_cnt)(SELECT
   db.provider_id, db.status_flag, db.position_ident,
   db.provider_type_flag, db.record_dt_tm, db.status_dt_tm,
   db.exclude_ind, db.updt_cnt
   FROM dash_break db
   WHERE db.record_dt_tm < cnvtlookbehind("90, D"))
 ;end insert
 IF (error(err_msg,0) > 0)
  SET failed = 1
  GO TO exit_script
 ENDIF
 DELETE  FROM dash_break db
  WHERE db.record_dt_tm < cnvtlookbehind("90, D")
 ;end delete
 IF (error(err_msg,0) > 0)
  SET failed = 1
 ...
 SELECT INTO "nl:"
  ...
  (SELECT DISTINCT
   ...
   FROM sa_prsnl_activity_time pat,
    (inner JOIN sa_prsnl_activity pa ON pa.sa_prsnl_activity_id=pat.sa_prsnl_activity_id
     AND pa.active_ind=1),
    (inner JOIN sa_anesthesia_record ar ON ar.sa_anesthesia_record_id=pa.sa_anesthesia_record_id
     AND ar.active_ind=1),
    (inner JOIN surgical_case sc ON sc.surg_case_id=ar.surgical_case_id
     AND sc.active_ind=1
     AND (sc.inst_cd= $INSTITUTIONID))
   WHERE pat.active_ind=1
    AND datetimecmp(pat.start_dt_tm,cnvtdatetime(curdate,0))=0))
  ORDER BY p.person_id
  HEAD REPORT
   idx = 0
  DETAIL
   idx = (idx+ 1)
   IF (idx > size(chart_data->results,5))
    stat = alterlist(chart_data->results,(idx+ 9))
   ENDIF
   chart_data->results[idx].providerid = p.person_id, chart_data->results[idx].name = p
   .name_full_formatted, chart_data->results[idx].lname = p.name_last,
   chart_data->results[idx].position = uar_get_code_description(p.position_cd)
  FOOT REPORT
   stat = alterlist(chart_data->results,idx)
  WITH nocounter
 ;end select
 IF (error(err_msg,0) > 0)
  SET failed = 1
  GO TO exit_script
 ENDIF
 IF (size(chart_data->results,5)=0)
  CALL echo("No personel found for today")
 ...
 SELECT INTO "nl:"
  db.provider_id
  FROM dash_break db
  WHERE expand(idx,1,size(chart_data->results,5),db.provider_id,chart_data->results[idx].providerid)
  DETAIL
   pos = locateval(idx,1,size(chart_data->results,5),db.provider_id,chart_data->results[idx].
    providerid)
   IF (pos > 0)
    chart_data->results[pos].exists_flag = 1
   ENDIF
  WITH nocounter
 ;end select
 IF (error(err_msg,0) > 0)
  ...
  GO TO exit_script
 ENDIF
 INSERT  FROM dash_break db,
   (dummyt d1  WITH seq = value(size(chart_data->results,5)))
  SET db.dash_break_id = seq(surgery_seq,nextval), db.provider_id = chart_data->results[d1.seq].
   providerid, db.position_ident = chart_data->results[d1.seq].position,
   db.provider_type_flag = 1, db.record_dt_tm = cnvtdatetime(curdate,0), db.status_dt_tm =
   cnvtdatetime(curdate,curtime),
   db.exclude_ind = 0, db.status_flag = 1, db.updt_dt_tm = cnvtdatetime(curdate,curtime),
   db.updt_id = reqinfo->updt_id, db.updt_cnt = 0
  PLAN (d1
   WHERE  NOT ((chart_data->results[d1.seq].exists_flag=1)))
   JOIN (db)
 ...
 INSERT  FROM dash_break_hist
  ...
  exclude_ind, updt_cnt)(SELECT
   ...
   FROM dash_break db
   WHERE expand(idx,1,size(chart_data->results,5),db.provider_id,chart_data->results[idx].providerid)
    AND db.record_dt_tm != cnvtdatetime(curdate,0)
   WITH rdbarrayinsert = 100)
 ;end insert
 IF (error(err_msg,0) > 0)
  SET failed = 1
  GO TO exit_script
 ENDIF
 UPDATE  FROM dash_break db,
   (dummyt d1  WITH seq = value(size(chart_data->results,5)))
  SET db.status_flag = 1, db.position_ident = chart_data->results[d1.seq].position, db.record_dt_tm
    = cnvtdatetime(curdate,0),
   db.status_dt_tm = cnvtdatetime(curdate,curtime), db.exclude_ind = 0, db.updt_dt_tm = cnvtdatetime(
    curdate,curtime),
   db.updt_id = reqinfo->updt_id, db.updt_cnt = (db.updt_cnt+ 1)
  PLAN (d1)
   JOIN (db
   WHERE (db.provider_id=chart_data->results[d1.seq].providerid)
    AND db.record_dt_tm != cnvtdatetime(curdate,0))
 ;end update
 IF (error(err_msg,0) > 0)
  SET failed = 1
 ...
 SELECT INTO "nl:"
  ...
  PLAN (db)
   JOIN (d1
   WHERE (chart_data->results[d1.seq].providerid=db.provider_id))
  ORDER BY db.provider_id
  HEAD REPORT
   idx = 0
  DETAIL
   idx = (idx+ 1), chart_data->results[idx].status = db.status_flag, chart_data->results[idx].exclude
    = db.exclude_ind
  WITH nocounter
 ;end select
 IF (error(err_msg,0) > 0)
...
#exit_script
 IF (failed=1)
  ...
  SET chart_data->status_data.status = "F"
  CALL log_message(concat("Error updating all statuses: ",err_msg),log_level_debug)
 ELSE
