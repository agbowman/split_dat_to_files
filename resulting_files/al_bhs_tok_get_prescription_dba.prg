CREATE PROGRAM al_bhs_tok_get_prescription:dba
    ...
    SELECT INTO "nl:"
     ...
     FOOT REPORT
      IF (size(trim(prsn_orders->list[l_idx].s_volume_dose,3)) > 0
       ...
       AND size(trim(prsn_orders->list[l_idx].s_str_dose,3)) > 0
       ...
       prsn_orders->list[l_idx].s_gen_order_disp_line = concat(prsn_orders->list[l_idx].s_volume_dose,
        " ",prsn_orders->list[l_idx].s_volume_dose_unit," = ",prsn_orders->list[l_idx].s_str_dose,
  ...
  FOR (l_idx = 1 TO prsn_orders->l_cnt)
    IF (l_idx=1)
     SET s_text = concat(s_text,
      "<tr><td><p><b>Prescription</b></p></td><td><p><b>Display</b></p></td></tr>")
    ENDIF
    SET prsn_orders->list[l_idx].s_gen_order_disp_line = concat(prsn_orders->list[l_idx].
     s_gen_order_disp_line," <br><br> Next Dose:____________________________")
    SET s_text = concat(s_text,"<tr><td><p>",prsn_orders->list[l_idx].s_order_name,"</p></td><td><p>",
     prsn_orders->list[l_idx].s_gen_order_disp_line,
     "</p></td></tr>")
  ENDFOR
 ELSE
  SET s_text = concat(s_text,"<tr><td><p>No new prescriptions during this visit.</p></td></tr>")
 ENDIF
