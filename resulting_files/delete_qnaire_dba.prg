CREATE PROGRAM delete_qnaire:dba
 PAINT
 ...
 SET valid = 0
 WHILE (valid=0)
   CALL text(1,1,"Please Enter an ALIASID from CKE_QUESTIONNAIRE to be deleted: ")
   CALL text(2,1,"aliasid:")
   CALL accept(2,10,"p(100);c"," ")
   SET p1 = curaccept
   IF (p1="")
    CALL text(6,1,build("please enter a valid aliasid, (case sensitive)."))
    SET valid = 0
   ELSE
    SELECT INTO "nl:"
     aliasid
    ...
    IF (curqual=1)
     CALL clear(6,1)
     CALL text(6,1,build("ALIASID: '",p1,"' found on CKE_QUESTIONNAIRE."))
     SET valid = 1
    ELSE
     CALL text(6,1,build("did not find ALIASID: '",p1,"' on CKE_QUESTIONNAIRE."))
     SET valid = 0
    ENDIF
   ENDIF
   IF (valid=0)
    CALL text(7,1,"enter 'Y' to enter another aliasid: ")
    CALL accept(7,38,"p;cu"," ")
    IF (curaccept != "Y")
     RETURN
    ELSE
     SET valid = 0
 ...
 ENDWHILE
 CALL clear(1,1)
 SET info->alias_id = p1
 SELECT INTO "nl:"
  qn.questionnaire_ident
  FROM cke_questionnaire qn
  WHERE (qn.aliasid=info->alias_id)
  DETAIL
   info->qnaire_id = qn.questionnaire_ident
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  qnel.element_entity_ident, qnel.qnaire_element_ident, qnel.exp_template_id
  ...
  DETAIL
   ...
   qnaire_elem_id = qnel.qnaire_element_ident, info->list1[list1_cnt].exp_temp_id = qnel
   .exp_template_id
  WITH nocounter
 ;end select
 ...
 DELETE  FROM cke_questionnaire qnaire
  WHERE (qnaire.aliasid=info->alias_id)
 ;end delete
