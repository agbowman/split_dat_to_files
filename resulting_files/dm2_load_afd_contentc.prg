CREATE PROGRAM dm2_load_afd_contentc
 ...
 DECLARE dlacc_req_size = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_load_afd_cntntc",".log","dm2_load_afd_contentc")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_load_afd_contentc"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SET dlacc_req_size = size(requestin->list_0,5)
 IF (dcfr_pop_coldic_rec(dlac_load_table)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Insert into ",dcfr_col_list->tbl," ...")
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 CALL dm2_push_cmd(concat("insert into ",dcfr_col_list->tbl,
   " dcv ,(dummyt d with seq = value(dlacc_req_size) )"),0)
 CALL dm2_push_cmd("set",0)
 IF (dcfr_build_col_list(null)=0)
  ROLLBACK
  GO TO exit_program
 ENDIF
 CALL dm2_push_cmd(",dcv.updt_applctx = 0 ",0)
 CALL dm2_push_cmd(",dcv.updt_dt_tm = cnvtdatetime(dlac_current_date) ",0)
 CALL dm2_push_cmd(",dcv.updt_cnt = 0 ",0)
 CALL dm2_push_cmd(",dcv.updt_id = 0 ",0)
 CALL dm2_push_cmd(",dcv.updt_task = 0 ",0)
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_load_afd_contentc completed"
 ENDIF
 CALL final_disp_msg("dm2_load_afd_cntntc")
END GO
