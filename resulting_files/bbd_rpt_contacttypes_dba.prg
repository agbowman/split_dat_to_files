CREATE PROGRAM bbd_rpt_contacttypes:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 SELECT INTO "cer_temp:bbdcontacttypes.txt"
  c.*
  FROM code_value c
  PLAN (c
  ...
  HEAD PAGE
   ...
   CALL center("C O N T A C T   T Y P E S   R E P O R T",1,125),
   col 107, "Time:", col 121,
   curtime"hh:mm;;m", row + 1, col 107,
   "As of Date:", col 119, curdate"ddmmmyy;;d",
   row + 3, col 12, "Contact Type Display",
   col 45, "Contact Type Description", col 79,
  ...
  FOOT PAGE
   row 57, col 1, line,
   row + 1, col 1, "Report ID: BBD_RPT_CONTACTTYPES",
   col 58, "Page:", col 64,
 ...
 IF (report_complete_ind="Y")
  ...
  SET reply->status_data.subeventstatus[count1].targetobjectname = "bbd_rpt_contacttypes"
