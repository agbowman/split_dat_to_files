CREATE PROGRAM dm2_mig_restart_cleanup:dba
 ...
 SUBROUTINE dcdur_get_db_owner_pwds(dgdop_in_dbname)
   ...
   RETURN(1)
 END ;Subroutine
 RECORD dmrc_tablespaces(
   1 cnt = i4
   1 qual[*]
     2 tablespace_name = vc
     2 tablespace_command = vc
 )
 RECORD dmrc_grant_cmds(
   1 count = i4
   1 qual[*]
     2 command = vc
     2 owner = vc
 )
 FREE RECORD dmrc_user_list
 RECORD dmrc_user_list(
   1 change_ind = i2
   1 cnt = i4
   1 qual[*]
     2 user = vc
 )
 DECLARE dmrc_iter = i4 WITH protect, noconstant(0)
 DECLARE dmrc_cmd = vc WITH protect, noconstant("")
 DECLARE dmrc_missing_owner_ind = i2 WITH protect, noconstant(0)
 DECLARE dmrc_missing_users = vc WITH protect, noconstant("-")
 DECLARE dmrc_node_list = vc WITH protect, noconstant("-")
 DECLARE dmrc_user_idx = i4 WITH protect, noconstant(0)
 DECLARE dmrc_gg_del_loc = vc WITH protect, noconstant("")
 DECLARE dmrc_dbx_ind = i2 WITH protect, noconstant(0)
 DECLARE dmrc_coalesce_tablespaces(null) = i2
 DECLARE dmrc_create_users(null) = i2
 DECLARE dmrc_remove_cron(null) = i2
 DECLARE dm2_mig_db_ind = i2 WITH protect, noconstant(1)
 SUBROUTINE dmrc_coalesce_tablespaces(null)
   SET dm_err->eproc = "Selecting tablespace names to coalesce from dba_tablespaces"
   SELECT INTO "nl:"
    FROM dba_tablespaces dt
    ...
    HEAD REPORT
     dmrc_temp = 0
    DETAIL
     dmrc_tablespaces->cnt = (dmrc_tablespaces->cnt+ 1)
     IF ((dmrc_tablespaces->cnt > dmrc_temp))
      dmrc_temp = (dmrc_temp+ 50), stat = alterlist(dmrc_tablespaces->qual,dmrc_temp)
     ENDIF
     dmrc_tablespaces->qual[dmrc_tablespaces->cnt].tablespace_command = concat(
      "rdb alter tablespace ",dt.tablespace_name," coalesce go"), dmrc_tablespaces->qual[
     dmrc_tablespaces->cnt].tablespace_name = dt.tablespace_name
    FOOT REPORT
     stat = alterlist(dmrc_tablespaces->qual,dmrc_tablespaces->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   FOR (dmrc_iter = 1 TO dmrc_tablespaces->cnt)
     SET dm_err->eproc = build("Coalescing tablespace ",dmrc_tablespaces->qual[dmrc_iter].
      tablespace_name)
     CALL disp_msg("",dm_err->logfile,0)
     IF (dm2_push_cmd(dmrc_tablespaces->qual[dmrc_iter].tablespace_command,1)=0)
      RETURN(0)
     ENDIF
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmrc_create_users(null)
   EXECUTE dm2_create_users "ALL"
   IF (dm_err->err_ind)
    RETURN(0)
   ENDIF
   EXECUTE dm2_mig_create_nonv500_users
   IF (dm_err->err_ind)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmrc_remove_cron(null)
   DECLARE drc_cur_node = vc WITH protect, noconstant("")
   DECLARE drc_same_node_ind = i2 WITH protect, noconstant(0)
   SET drc_cur_node = curnode
   SET dm_err->eproc = "Selecting node name from gv$instance"
   SELECT INTO "nl:"
    FROM gv$instance v
    DETAIL
     IF (cnvtupper(v.host_name)=cnvtupper(drc_cur_node))
      drc_same_node_ind = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc))
    ...
    RETURN(0)
   ENDIF
   IF (drc_same_node_ind)
    IF (dm2_push_dcl(build(dmrc_gg_del_loc,"/dirtmp/dm2ei_crontab_remove.ksh"))=0)
     RETURN(0)
    ENDIF
   ELSE
    SET message = window
    CALL clear(1,1)
    CALL box(1,1,24,131)
    CALL text(2,2,"Database Migration Process - Restart cleanup")
    CALL text(4,2,concat("On the Node ",dmr_mig_data->tgt_nodes[1].node_name,
      ", execute the following as root:"))
    CALL text(5,2,build("$",dmrc_gg_del_loc,"/dirtmp/dm2ei_crontab_remove.ksh"))
    CALL text(23,2,
     "Upon successful completion of the above script, select (C) to continue or (Q) to quit:_")
    CALL accept(23,88,"A;CU","C"
     WHERE curaccept IN ("C", "Q"))
    SET message = nowindow
    IF (curaccept="Q")
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "User chose to quit at dm2ei_crontab_remove.ksh step."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_mig_restartclean",".log","dm2_mig_restartclean")=0)
  GO TO exit_script
 ENDIF
 IF (dmr_prompt_connect_data("SOURCE","V500","PC")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Check to see if any Migration rows exist in dm_info."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG*"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual > 0)
  ...
  SELECT INTO "nl:"
   ...
   WHERE di.info_domain="DM2_MIG_GG_DATA"
    AND di.info_name="GG_DEL_DIR"
   DETAIL
    dmrc_gg_del_loc = di.info_char
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ELSEIF (size(trim(dmrc_gg_del_loc,3))=0)
   SET dm_err->err_ind = 1
  ...
  ENDIF
 ENDIF
 IF (substring(size(trim(dmrc_gg_del_loc,3)),1,dmrc_gg_del_loc)="/")
  SET dmrc_gg_del_loc = trim(replace(dmrc_gg_del_loc,"/","",2),3)
 ENDIF
 SET dm_err->eproc = "Gathering list of migration users"
 SELECT INTO "nl:"
  FROM dm_info di
  ...
  DETAIL
   dmrc_cnt = (dmrc_cnt+ 1)
   IF (mod(dmrc_cnt,10)=1)
    stat = alterlist(dmrc_user_list->qual,(dmrc_cnt+ 9))
   ENDIF
   dmrc_user_list->qual[dmrc_cnt].user = trim(di.info_name,3)
  FOOT REPORT
   stat = alterlist(dmrc_user_list->qual,dmrc_cnt), dmrc_user_list->cnt = dmrc_cnt
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmrc_user_list)
 ENDIF
 IF ((dmrc_user_list->cnt=0))
 ...
 IF (dmr_get_node_name(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking if DBX is used or not in the migration"
 SELECT INTO "nl:"
  FROM dm2_admin_dm_info d
  WHERE d.info_domain="DM2MIG_BDM_OPTION"
   AND d.info_name="DBX_IN_USE"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SET dmr_mig_data->tgt_node_cnt = dmr_node->cnt
 SET stat = alterlist(dmr_mig_data->tgt_nodes,dmr_node->cnt)
 FOR (dmrc_iter = 1 TO dmr_node->cnt)
   SET dmr_mig_data->tgt_nodes[dmrc_iter].node_name = cnvtlower(dmr_node->qual[dmrc_iter].node_name)
   SET dmr_mig_data->tgt_nodes[dmrc_iter].instance_name = cnvtlower(dmr_node->qual[dmrc_iter].
    instance_name)
   SET dmrc_node_list = concat(dmrc_node_list,cnvtlower(dmr_node->qual[dmrc_iter].node_name),", ")
 ENDFOR
 SET dmrc_node_list = substring(2,(textlen(dmrc_node_list) - 2),dmrc_node_list)
 IF (textlen(dmrc_node_list) > 115)
  SET dmrc_node_list = concat(substring(1,112,dmrc_node_list),"...")
 ENDIF
 WHILE (true)
   SET message = window
   SET width = 132
   ...
   CALL box(1,1,24,131)
   CALL text(2,2,"Database Migration Process - Restart cleanup")
   CALL text(3,2,"TARGET")
   CALL text(4,2,build("Database Name: ",dmr_mig_data->tgt_db_name))
   CALL text(5,2,build("Database Created Date:",format(dmr_mig_data->tgt_created_date,";;q")))
   CALL text(6,2,build("Database Operating System: ",dmr_mig_data->tgt_db_os))
   CALL text(7,2,build("Database Oracle Version: ",dmr_mig_data->tgt_ora_version))
   CALL text(8,2,build("Database Nodes: ",dmrc_node_list))
   CALL text(10,2,
    "WARNING!  The database above will be cleaned up in order to restart migration process.")
   CALL text(11,2,
    "Please confirm that database listed above is the TARGET database before continuing")
   CALL text(12,2,"(C)ontinue, (Q)uit:_")
   CALL accept(12,22,"A;CU"," "
    WHERE curaccept IN ("C", "Q"))
   IF (curaccept="C")
    CALL text(14,2,"Are you sure you want to continue? Y/N:_")
    CALL accept(14,42,"A;CU"," "
     WHERE curaccept IN ("Y", "N"))
    IF (curaccept="Y")
     CALL text(16,2,"Are you ABSOLUTELY sure? (A=Absolutely, N=No):_")
     CALL accept(16,49,"A;CU"," "
      WHERE curaccept IN ("A", "N"))
     IF (curaccept="A")
      CALL text(18,2,"Confirm if you have used DBX for this migration: (Y/N):_")
      CALL accept(18,58,"A;CU"," "
       WHERE curaccept IN ("Y", "N"))
      IF (curaccept="N")
       CALL clear(1,1)
       SET message = nowindow
       IF (dmrc_dbx_ind > 0)
        SET dm_err->err_ind = 1
        SET dm_err->emsg = "DBX was used for this migration"
        CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
        GO TO exit_script
       ENDIF
       SET dm2_drop_user_no_confirmation = 1
       FOR (dmrc_iter = 1 TO dmrc_user_list->cnt)
         IF ((dmrc_user_list->qual[dmrc_iter].user IN ("V500", "V500_EVENT", "V500_REF", "V500_READ")
         ))
          IF (dcdur_prompt_tspaces(value(dmrc_user_list->qual[dmrc_iter].user),dmr_mig_data->
           tgt_db_name,dmrc_user_idx)=0)
           GO TO exit_script
          ENDIF
         ENDIF
         EXECUTE dm2_drop_user value(dmrc_user_list->qual[dmrc_iter].user)
         IF ((dm_err->err_ind=1))
          GO TO exit_script
         ENDIF
       ENDFOR
       IF (dmrc_coalesce_tablespaces(null)=0)
        GO TO exit_script
       ENDIF
       IF (dmrc_create_users(null)=0)
        GO TO exit_script
       ENDIF
       IF (dmr_prompt_connect_data("TARGET","SYS","CO")=0)
        GO TO exit_script
       ENDIF
       SET dm_err->eproc = "Verifying target users exist from dba_users/dual"
       SELECT INTO "nl:"
        FROM dual du,
         (dummyt d  WITH seq = value(dmrc_user_list->cnt))
        PLAN (d)
         JOIN (du
         WHERE  NOT ( EXISTS (
         (SELECT
          1
          FROM dba_users dbu
          WHERE (dbu.username=dmrc_user_list->qual[d.seq].user)))))
        DETAIL
         dmrc_missing_owner_ind = 1, dmrc_missing_users = build(dmrc_missing_users,dmrc_user_list->
          qual[d.seq].user,", ")
        FOOT REPORT
         dmrc_missing_users = substring(2,(textlen(dmrc_missing_users) - 2),dmrc_missing_users)
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc)=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        GO TO exit_script
       ENDIF
       IF (dmrc_missing_owner_ind)
        SET dm_err->eproc = build(
         "WARNING: The following migration users were not created successfully:",char(10),char(13),
         dmrc_missing_users)
        CALL disp_msg("",dm_err->logfile,0)
       ENDIF
      ELSE
       IF (dmrc_dbx_ind=0)
        SET dm_err->err_ind = 1
        SET dm_err->emsg = "DBX was not used for this migration"
        CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
        GO TO exit_script
       ENDIF
      ENDIF
      ...
      SET message = nowindow
      IF (dmrc_gg_del_loc > "")
       IF (dmrc_remove_cron(null)=0)
        GO TO exit_script
       ENDIF
      ENDIF
      SET dm_err->eproc = "Removing contents of ggdelivery/dirtmp directory"
      CALL disp_msg("",dm_err->logfile,0)
      IF ((dm2_sys_misc->cur_os="AXP"))
       SET dmrc_cmd = "delete GGDELIVERY:[DIRTMP.EXEC]*.*;*"
       IF (dm2_find_dir("GGDELIVERY:[DIRTMP.EXEC]"))
        SET dm_err->disp_dcl_err_ind = 0
        IF (dm2_push_dcl(dmrc_cmd)=0)
         IF ((dm_err->err_ind=1))
          GO TO exit_script
        ...
        ENDIF
       ENDIF
       SET dmrc_cmd = "delete GGDELIVERY:[DIRTMP.LOG]*.*;*"
       IF (dm2_find_dir("GGDELIVERY:[DIRTMP.LOG]"))
        SET dm_err->disp_dcl_err_ind = 0
        IF (dm2_push_dcl(dmrc_cmd)=0)
         IF ((dm_err->err_ind=1))
          GO TO exit_script
       ...
       ENDIF
      ENDIF
      SET dmrc_cmd = evaluate(dm2_sys_misc->cur_os,"AXP","delete GGDELIVERY:[DIRTMP]*.*;*",
       "rm -fr /ggdelivery/dirtmp/*")
     ...
     ELSE
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "User chose to quit at third confirmation step."
      GO TO exit_script
     ENDIF
    ELSE
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "User chose to quit at second confirmation step."
     GO TO exit_script
    ENDIF
   ELSE
    SET dm_err->err_ind = 1
...
#exit_script
 ...
 SET message = nowindow
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = build("DM2_MIG_RESTART_CLEANUP completes successfully",evaluate(
    dmrc_missing_owner_ind,1," with WARNINGS.",""))
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 ...
 CALL echo("**************************************************************************")
 CALL echo("*")
 CALL final_disp_msg("dm2_mig_restartclean")
END GO
