CREATE PROGRAM bed_get_portal_urls:dba
 ...
 DECLARE getportalurls(null) = i2
 CALL getportalurls(null)
 SUBROUTINE getportalurls(null)
   CALL bedlogmessage("getPortalURLs","Entering ...")
   DECLARE cnt = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM br_portal_url b
    WHERE b.logical_domain_id=log_domain_id
     AND b.active_ind=1
    ORDER BY cnvtupper(b.br_portal_url)
    DETAIL
     cnt = (cnt+ 1), stat = alterlist(reply->urls,cnt), reply->urls[cnt].br_portal_url_id = b
     .br_portal_url_id,
     reply->urls[cnt].br_portal_url = b.br_portal_url
    WITH nocounter
   ;end select
   CALL bederrorcheck("Failed to get URLs")
   CALL bedlogmessage("getPortalURLs","Exiting ...")
 END ;Subroutine
#exit_script
