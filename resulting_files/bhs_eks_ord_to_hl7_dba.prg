CREATE PROGRAM bhs_eks_ord_to_hl7:dba
 ...
 DECLARE mf_orgdoc_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",320,
   "ORGANIZATIONDOCTOR"))
 DECLARE mf_npi_cd = f8 WITH protect, constant(uar_get_code_by_cki("CKI.CODEVALUE!2160654021"))
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   prsn_info->marital_type_cd = p.marital_type_cd, prsn_info->dob = format(cnvtdatetimeutc(
     datetimezone(p.birth_dt_tm,p.birth_tz),1),"YYYYMMDD;;q")
 ...
 IF ((prsn_info->cnt=0))
  SELECT INTO "nl:"
   FROM nomen_entity_reltn ner,
    nomenclature n
   PLAN (ner
    WHERE ner.parent_entity_id=trigger_orderid
 ...
 SELECT INTO "nl:"
  ...
  PLAN (e
   ...
   WHERE (pa2.person_id= Outerjoin(p.person_id))
    ...
    AND (pa2.end_effective_dt_tm>= Outerjoin(cnvtdatetime(sysdate)))
    AND (pa2.prsnl_alias_type_cd= Outerjoin(mf_npi_cd)) )
  ...
  DETAIL
   IF (epr.expire_dt_tm = null)
    ...
    prsn_info->s_attend_npi = trim(pa2.alias,3)
   ENDIF
  FOOT REPORT
   ...
   prsn_info->pv1_hl7_slice = concat("PV1|1||||||",prsn_info->s_attend_alias,"^",prsn_info->
    s_attend_name_last,"^",
    prsn_info->s_attend_name_first,"^^^^^^^^^^ORGANIZATION DOCTOR~",prsn_info->s_attend_npi,"^",
    prsn_info->s_attend_name_last,
    "^",prsn_info->s_attend_name_first,"^^^^^^^^^^NPI","|")
  WITH nocounter
 ;end select
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   prsn_info->ord[1].s_ord_provider_npi = trim(pa2.alias,3)
  FOOT REPORT
 ...
 SET prsn_info->ord[1].obr_hl7_slice = build("OBR|1","|",cnvtstring(trigger_orderid,20),
  ...
  "|","|",evaluate(ms_orc_type,"CA",trim("",3),trim(prsn_info->ord[1].ord_collected_date,3)),"|","|",
  "|","|","|","|","|",
  ...
  prsn_info->ord[1].ord_provider_name_first,"^^^^^^^^^^ORGANIZATION DOCTOR~",prsn_info->ord[1].
  s_ord_provider_npi,"^",prsn_info->ord[1].ord_provider_name_last,
  "^",prsn_info->ord[1].ord_provider_name_first,"^^^^^^^^^^NPI","|","|",
  "|","|","|","|",format(cnvtdatetime(prsn_info->ord[1].action_dt_tm),"YYYYMMDDHHMMSS;;q"),
  "|","|",prsn_info->ord[1].ord_activity_type_meaning,"|",evaluate(ms_orc_type,"CA","X",trim("",3)),
  "|","|")
