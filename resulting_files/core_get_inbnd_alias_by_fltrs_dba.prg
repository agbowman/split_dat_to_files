CREATE PROGRAM core_get_inbnd_alias_by_fltrs:dba
 ...
 DECLARE cgiabf_filterstring = vc WITH protect, noconstant(" ")
 DECLARE cgiabf_alias = vc WITH protect, noconstant("tbl_alias")
 DECLARE cgiabf_list_size = i4 WITH protect, constant(size(request->filterlist,5))
 DECLARE cgiabf_err_msg = vc WITH protect, noconstant(" ")
 DECLARE cgiabf_table_name = vc WITH protect, noconstant(" ")
 DECLARE cgiabf_column_name = vc WITH protect, noconstant(" ")
 DECLARE detail_cnt = i4 WITH protect, noconstant(0)
 DECLARE default_block_size = i4 WITH protect, constant(50000)
 SET reply->status_data.status = "F"
 IF ((request->code_set <= 0.0))
  SET failed = "T"
  GO TO exit_script
 ENDIF
 IF ((((request->block_size=0)) OR ((request->block_size > default_block_size))) )
  SET request->block_size = default_block_size
 ENDIF
 IF (column_name_validator(request)=false)
  SET failed = "T"
  SET cgiabf_err_msg = "Invalid column name provided."
  GO TO exit_script
 ENDIF
 IF (table_name_validator(request)=false)
  SET failed = "T"
  SET cgiabf_err_msg = "Invalid table name provided."
  GO TO exit_script
 ENDIF
 IF (cgiabf_list_size > 0)
  IF (cgiabf_list_size > 1)
   SET failed = "T"
   GO TO exit_script
   SET cgiabf_err_msg = "Too many search terms."
  ENDIF
  SET cgiabf_table_name = cnvtupper(request->filterlist[1].table_name)
  SET cgiabf_column_name = cnvtupper(request->filterlist[1].column_name)
  SET cgiabf_filterstring = filterstring_builder(cgiabf_filterstring,cgiabf_alias,request)
  IF (cgiabf_table_name="CODE_VALUE")
   SET cgiabf_filterstring = replace(cgiabf_filterstring,cgiabf_alias,"cv")
  ELSEIF (cgiabf_table_name="CODE_VALUE_EXTENSION"
   AND cgiabf_column_name="FIELD_VALUE")
   SET cgiabf_filterstring = replace(cgiabf_filterstring,cgiabf_alias,"cve")
  ELSEIF (cgiabf_table_name="CODE_VALUE_ALIAS"
   AND cgiabf_column_name="ALIAS")
   SET cgiabf_filterstring = replace(cgiabf_filterstring,cgiabf_alias,"cva")
  ELSEIF (cgiabf_table_name="CODE_VALUE_ALIAS"
   AND cgiabf_column_name="CONTRIBUTOR_SOURCE_CD")
   SET cgiabf_filterstring = build2("cs73.DISPLAY_KEY = '",trim(cnvtalphanum(cnvtupper(request->
       filterlist[1].column_value))),"*'")
  ELSEIF (cgiabf_table_name="CODE_VALUE_OUTBOUND"
   AND cgiabf_column_name="ALIAS")
   SET cgiabf_filterstring = replace(cgiabf_filterstring,cgiabf_alias,"cvo")
  ELSEIF (cgiabf_table_name="CODE_VALUE_OUTBOUND"
   AND cgiabf_column_name="CONTRIBUTOR_SOURCE_CD")
   SET cgiabf_filterstring = build2("cs73.DISPLAY_KEY = '",trim(cnvtalphanum(cnvtupper(request->
       filterlist[1].column_value))),"*'")
  ELSE
   SET cgiabf_filterstring = "1 = 1"
  ENDIF
 ELSE
  SET cgiabf_filterstring = "1 = 1"
 ENDIF
 IF ((request->contributor_source_cd > 0.0))
  ...
  SELECT
   IF (cgiabf_table_name="CODE_VALUE")
    ...
    PLAN (cv
     WHERE parser(cgiabf_filterstring)
      AND (cv.code_set=request->code_set))
     JOIN (cva
     WHERE (cva.code_value= Outerjoin(cv.code_value))
      AND (cva.contributor_source_cd= Outerjoin(request->contributor_source_cd)) )
   ELSEIF (cgiabf_table_name="CODE_VALUE_ALIAS"
    AND cgiabf_column_name="ALIAS")
    FROM code_value cv,
     code_value_alias cva
    PLAN (cva
     ...
     WHERE (cv.code_set=request->code_set)
      AND (cva.code_value= Outerjoin(cv.code_value)) )
   ELSEIF (cgiabf_table_name="CODE_VALUE_OUTBOUND"
    AND cgiabf_column_name="ALIAS")
   ...
   ELSEIF (cgiabf_table_name="CODE_VALUE_ALIAS"
    ...
    FROM code_value cv,
     code_value_alias cva
    PLAN (cva
     WHERE (cva.contributor_source_cd= Outerjoin(request->contributor_source_cd))
      AND  EXISTS (
     (SELECT
      "x"
   ...
   ELSEIF (cgiabf_table_name="CODE_VALUE_EXTENSION"
    ...
    PLAN (cve
     WHERE (cve.code_set=request->code_set)
      AND parser(cgiabf_filterstring))
     JOIN (cv
     WHERE (cv.code_set=request->code_set)
   ...
   cva.primary_ind, cv.display, cv.display_key
   ...
   DETAIL
    ...
    IF ((a_cnt < request->block_size)
     ...
     reply->alias_list[a_cnt].contributor_source_disp = cont_src_disp, reply->alias_list[a_cnt].
 ...
 ELSE
  ...
  SELECT INTO "nl:"
   ...
   PLAN (cv
    ...
    JOIN (cva
    WHERE (cva.contributor_source_cd= Outerjoin(cv.code_value))
     AND (cva.code_set= Outerjoin(request->code_set))
     AND (cva.code_value= Outerjoin(request->code_value)) )
   ORDER BY cv.code_value
   HEAD REPORT
    a_cnt = 0, stat = alterlist(reply->alias_list,request->block_size)
   DETAIL
    ...
    IF ((a_cnt < request->block_size)
     ...
     reply->alias_list[a_cnt].contributor_source_disp = cv.display, reply->alias_list[a_cnt].
     primary_ind = cva.primary_ind
    ENDIF
   FOOT REPORT
    stat = alterlist(reply->alias_list,a_cnt)
