CREATE PROGRAM afc_da_add_chrg_batch_dtl_fld:dba
 SET afc_da_add_chrg_batch_dtl_fld_vrsn = 20130816
 ...
 DECLARE add_charge_batch_detail_field(nstart=i4,nstop=i4) = null WITH protect
 ...
 CALL add_charge_batch_detail_field(mndastart,mndastop)
 SUBROUTINE add_charge_batch_detail_field(nstart,nstop)
   ...
   FOR (i = 1 TO size(request->objarray,5))
     IF (validate(request->objarray[i].charge_batch_detail_field_id,- (0.00001)) <= 0.0)
      SELECT INTO "nl:"
       ...
       WITH format, counter
      ;end select
      IF (validate(request->objarray[i].charge_batch_detail_field_id))
       SET request->objarray[i].charge_batch_detail_field_id = mrsstatus->objarray[i].daid
      ENDIF
      ...
      ENDIF
     ELSE
      SET mrsstatus->objarray[i].daid = request->objarray[i].charge_batch_detail_field_id
     ENDIF
   ENDFOR
   INSERT  FROM charge_batch_detail_field c,
     (dummyt dt  WITH seq = value(size(request->objarray,5)))
    SET c.charge_batch_detail_field_id = mrsstatus->objarray[dt.seq].daid, c.charge_batch_detail_id
      =
