CREATE PROGRAM bhs_phy_mailing_rpt_je:dba
 PROMPT
 ...
 DECLARE ms_facility = vc WITH protect, constant(trim(uar_get_code_display(cnvtreal( $MF_FACILITY)),3
   ))
 DECLARE ms_outfile = vc WITH protect, constant(concat("phy_mailing",format(sysdate,
    "YYYYMMDDhhmmss;;D"),".csv"))
 DECLARE ms_subject = vc WITH protect, constant(concat("PHYSICIAN MAILING REPORT FOR RADIOLOGY ",
   ms_search_date," ",ms_facility))
 ...
 SELECT INTO value(ms_output)
  FROM (dummyt d  WITH seq = value(size(temp->qual,5)))
  HEAD REPORT
   ms_line = concat("Physician Mailing report for Radiology ",ms_search_date," ",ms_facility), row 0,
   col 0,
   ms_line, ms_line = concat("Patient Name, MRN, FIN, Accession",
    ", Final Date, Facility, Patient Location",
    ", Ordering Physician, Ordering Physician AutoFax, Consulting Physician",
    ", Consulting Physician Autofax, Patient Type, Order Description, Financial Class"), row + 1,
   col 0, ms_line
  DETAIL
   ms_line = build('"',temp->qual[d.seq].s_patient_name,'","',temp->qual[d.seq].s_mrn,'","',
    ...
    temp->qual[d.seq].s_order_phy,'","',evaluate(temp->qual[d.seq].n_order_phys_autofax_ind,1,"Yes",
     "No"),'","',temp->qual[d.seq].s_consult_phy,
    '","',evaluate(temp->qual[d.seq].n_consult_phys_autofax_ind,1,"Yes","No"),'","',temp->qual[d.seq]
    .s_patient_type,'","',
    temp->qual[d.seq].s_order_description,'","',temp->qual[d.seq].s_financial_class,'"'), row + 1,
   col 0,
   ms_line
...
#exit_script
 IF (ms_status != "SUCCESS")
  CALL echo(build2("facility: ",ms_facility," ",ms_error))
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
    col 0, "{PS/792 0 translate 90 rotate/}", "{F/1}{CPI/7}",
    CALL print(calcpos(10,10)), "Physician Mailing report for Radiology", "{F/1}{CPI/7}",
    CALL print(calcpos(10,10)), ms_facility"{F/1}{CPI/14}",
    CALL print(calcpos(10,30)),
    ms_error
