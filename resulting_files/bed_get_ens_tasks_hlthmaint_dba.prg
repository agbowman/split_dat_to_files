CREATE PROGRAM bed_get_ens_tasks_hlthmaint:dba
 FREE SET reply
 RECORD reply(
   1 status_data
     2 status = c1
 ...
 SET vprequest->vplist[1].view_name = "HEALTHMAINT"
 ...
 SET vcprequest->vcplist[1].view_name = "HEALTHMAINT"
 SET vcprequest->vcplist[1].view_seq = 0
 SET vcprequest->vcplist[1].comp_name = "HEALTHMAINT"
 ...
 ELSEIF ((request->action="2"))
  ...
  IF ((vpreply->vplist[1].view_prefs_id > 0))
   ...
   ELSEIF ((nvpreply->nvplist[2].name_value_prefs_id=0))
    ...
    SET nvprequest->nvplist[1].pvc_value = "13"
    SET nvprequest->nvplist[1].action_flag = "1"
    SET trace = recpersist
    EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
     nvpreply)
  ...
  IF ((vcpreply->vcplist[1].view_comp_prefs_id > 0))
   ...
   ELSE
    ...
    IF (hold_nvp5=0)
     ...
     SET nvprequest->nvplist[1].pvc_value = "33252"
     SET trace = recpersist
     EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
      nvpreply)
    ENDIF
  ...
  IF ((dpreply->dplist[1].detail_prefs_id > 0))
   SELECT INTO "NL:"
    ...
    WHERE nvp.parent_entity_name="DETAIL_PREFS"
     ...
     AND nvp.pvc_name="SHOW_HEALTHMAINTENANCE"
 ...
 SUBROUTINE complete_vcp_nvp(dummy_parm2)
   ...
   SET nvprequest->nvplist[1].pvc_value = "CPSUIHealthMaintenance.dll"
   ...
   SET nvprequest->nvplist[5].pvc_value = "33252"
 END ;Subroutine
 SUBROUTINE complete_encsumm_dp_nvp(dummy_parm2)
   ...
   SET nvprequest->nvplist[1].pvc_name = "SHOW_HEALTHMAINTENANCE"
