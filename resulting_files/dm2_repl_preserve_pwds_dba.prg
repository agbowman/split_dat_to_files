CREATE PROGRAM dm2_repl_preserve_pwds:dba
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drpp_ccl_ver = i4 WITH protect, constant((((cnvtint(currev) * 10000)+ (cnvtint(currevminor)
   * 100))+ cnvtint(currevminor2)))
 DECLARE drpp_cnt = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2replpreservepwds",".log","Starting dm2_repl_preserve_pwds...")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_repl_preserve_pwds"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Updating dm_info row for preserved password count"
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inserting dm_info row for preserved password count"
  INSERT  FROM dm2_admin_dm_info di
   SET di.info_domain = "DM2_REPLICATE_USER_PWDS_CNT", di.info_name = cnvtupper(trim(currdbname)), di
    .info_number = 0
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 COMMIT
 IF (drpp_ccl_ver < 80205)
  SET dm_err->eproc = "CCL version does not support functionality.  Exiting process."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF (dcdur_preserve_pwds(currdbname)=0)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  FROM dm2_admin_dm_info d
  WHERE d.info_domain="DM2_REPLICATE_USER_PWDS"
   AND d.info_name=patstring(cnvtupper(concat(trim(currdbname),"*")))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET drpp_cnt = curqual
 UPDATE  FROM dm2_admin_dm_info di
  SET di.info_number = drpp_cnt
  WHERE di.info_domain="DM2_REPLICATE_USER_PWDS_CNT"
   AND di.info_name=cnvtupper(trim(currdbname))
  WITH nocounter
 ;end update
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Ending dm2_repl_preserve_pwds"
 CALL final_disp_msg("dm2replpreservepwds")
END GO
