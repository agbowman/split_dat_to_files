CREATE PROGRAM cust_concept_tags_r
 PROMPT
 ...
 DECLARE stablename = vc WITH protect, constant("CUST_CONCEPT_TAGS_R")
 ...
 IF (checkdic(stablename,"T",0)=0)
  SELECT INTO TABLE cust_concept_tags_r
   cust_concept_tags_r_id = type("f8"), cust_concept_id = type("f8"), cust_concept_tags_id = type(
    "f8"),
   ...
   FROM dummyt d1
   WITH indexunique(cust_concept_tags_r_id), owner = value(stableowner), index(cust_concept_id),
    index(cust_concept_tags_id), synonym = "cust_concept_tags_r", organization = p
  ;end select
  DROP TABLE cust_concept_tags_r
  EXECUTE oragen3 "V500_BHS.CUST_CONCEPT_TAGS_R"
