CREATE PROGRAM bhs_athn_order_detail_ls_v5
 ...
 RECORD order_list(
   1 qual[*]
     ...
     2 original_order_communication_type_cd = vc
     2 original_order_communication_type_mean = vc
     2 original_order_communication_type_disp = vc
     2 order_status_cd = vc
     ...
     2 update_datetime = vc
     2 status_datetime = vc
     2 prescription_order_id = vc
     2 accession_id = f8
     2 unformatted_number = vc
     2 formatted_number = vc
     2 activity_sub_type_cd = vc
     2 activity_sub_type_disp = vc
     2 activity_sub_type_mean = vc
     2 prn_indicator = i2
     ...
     2 discontinue_indicator = i2
     2 discontinue_datetime = vc
     2 discontinue_datetimezone = vc
     2 discontinue_type = vc
     2 witness_flag = i2
     2 pathway_plan = vc
     2 proposal_status = vc
     2 ingredient_display = vc
     2 dcdisplay_days = vc
     2 default_stop_duration = f8
     2 stop_duration_unit_cd = f8
     2 stop_duration_unit = vc
     2 catlog_stop_type_cd = vc
     2 catlog_stop_type_mean = vc
     ...
     2 reporting_priority_cd = vc
     2 reporting_priority_disp = vc
     2 reporting_priority_mean = vc
     2 collection_priority_cd = vc
     2 collection_priority_disp = vc
     2 collection_priority_mean = vc
   1 error_msg = vc
   1 total_count = i4
 )
 SELECT INTO "nl:"
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter, separator = " ", format,
   time = 60
 ;end select
 SET order_list->total_count = size(order_list->qual,5)
 IF ((size(order_list->qual,5) >  $8))
  SET order_list->error_msg = "COUNT_EXCEEDED"
  GO TO exit_script
 ENDIF
 SELECT DISTINCT INTO "NL:"
  ...
  HEAD o.order_id
   ...
   order_list->qual[d1.seq].original_order_communication_type_cd = cnvtstring(o
    .latest_communication_type_cd), order_list->qual[d1.seq].original_order_communication_type_disp
    = uar_get_code_display(o.latest_communication_type_cd), order_list->qual[d1.seq].
   original_order_communication_type_mean = uar_get_code_meaning(o.latest_communication_type_cd),
   ...
   order_list->qual[d1.seq].stop_type_cd = o.stop_type_cd, order_list->qual[d1.seq].stop_type_disp =
   uar_get_code_display(o.stop_type_cd),
   order_list->qual[d1.seq].stop_type_mean = uar_get_code_meaning(o.stop_type_cd), order_list->qual[
   d1.seq].orderable_type_flag = trim(substring(0,8,
     IF (o.orderable_type_flag=0) "NORMAL"
     ENDIF
     ),3), order_list->qual[d1.seq].action_sequence = cnvtstring(oa.action_sequence),
   order_list->qual[d1.seq].update_datetime = format(o.updt_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"),
   order_list->qual[d1.seq].status_datetime = format(o.status_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"),
   order_list->qual[d1.seq].prescription_order_id = cnvtstring(o.prescription_order_id),
   order_list->qual[d1.seq].accession_id = ac.accession_id, order_list->qual[d1.seq].
   unformatted_number = trim(ac.accession,3), order_list->qual[d1.seq].formatted_number =
   ...
   ENDIF
   ,
   order_list->qual[d1.seq].activity_sub_type_cd = cnvtstring(oc.activity_subtype_cd), order_list->
   qual[d1.seq].activity_sub_type_disp = uar_get_code_display(oc.activity_subtype_cd), order_list->
   qual[d1.seq].activity_sub_type_mean = uar_get_code_meaning(oc.activity_subtype_cd),
   ...
   discontinue_indicator = o.discontinue_ind, order_list->qual[d1.seq].discontinue_datetime = format(
    o.discontinue_effective_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"),
   order_list->qual[d1.seq].discontinue_datetimezone = substring(21,3,datetimezoneformat(o
     .discontinue_effective_dt_tm,o.discontinue_effective_tz,"MM/dd/yyyy hh:mm:ss ZZZ",curtimezonedef
     )), order_list->qual[d1.seq].discontinue_type = uar_get_code_display(o.discontinue_type_cd),
   order_list->qual[d1.seq].witness_flag = os.witness_flag,
   order_list->qual[d1.seq].pathway_plan = trim(p.display_description,3), order_list->qual[d1.seq].
   proposal_status = uar_get_code_display(op.proposal_status_cd), order_list->qual[d1.seq].
   dcdisplay_days = cnvtstring(oc.dc_display_days),
   order_list->qual[d1.seq].default_stop_duration = oc.stop_duration, order_list->qual[d1.seq].
   stop_duration_unit_cd = oc.stop_duration_unit_cd, order_list->qual[d1.seq].stop_duration_unit =
   uar_get_code_meaning(oc.stop_duration_unit_cd),
   order_list->qual[d1.seq].catlog_stop_type_cd = cnvtstring(oc.stop_type_cd), order_list->qual[d1
   .seq].catlog_stop_type_mean = uar_get_code_meaning(oc.stop_type_cd), order_list->qual[d1.seq].
   ...
   ENDIF
   , order_list->qual[d1.seq].reftask_id = cnvtstring(t.reference_task_id),
   order_list->qual[d1.seq].frequency = cnvtstring(fs.frequency_cd), order_list->qual[d1.seq].
   stop_datetime = format(od1.oe_field_dt_tm_value,"MM/DD/YYYY HH:MM;;D"), order_list->qual[d1.seq].
   stopdate_timezone = substring(21,3,datetimezoneformat(od1.oe_field_dt_tm_value,od1.oe_field_tz,
     "MM/dd/yyyy hh:mm:ss ZZZ",curtimezonedef))
   IF (o.dcp_clin_cat_cd=10576)
    order_list->qual[d1.seq].reporting_priority_cd = cnvtstring(ol.report_priority_cd), order_list->
    qual[d1.seq].reporting_priority_disp = uar_get_code_display(ol.report_priority_cd), order_list->
    qual[d1.seq].reporting_priority_mean = uar_get_code_meaning(ol.report_priority_cd),
    order_list->qual[d1.seq].collection_priority_cd = cnvtstring(ol.collection_priority_cd),
    order_list->qual[d1.seq].collection_priority_disp = uar_get_code_display(ol
     .collection_priority_cd), order_list->qual[d1.seq].collection_priority_mean =
    uar_get_code_meaning(ol.collection_priority_cd)
   ENDIF
   IF (o.dcp_clin_cat_cd=10573)
    order_list->qual[d1.seq].reporting_priority_cd = cnvtstring(orr.priority_cd), order_list->qual[d1
    .seq].reporting_priority_disp = uar_get_code_display(orr.priority_cd), order_list->qual[d1.seq].
    reporting_priority_mean = uar_get_code_meaning(orr.priority_cd)
   ENDIF
  WITH time = 90
 ;end select
#exit_script
