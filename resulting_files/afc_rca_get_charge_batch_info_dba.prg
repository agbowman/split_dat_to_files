CREATE PROGRAM afc_rca_get_charge_batch_info:dba
 ...
 ENDIF
 DECLARE cntencntr = i4 WITH protect, noconstant(0)
 DECLARE batchtotal = f8 WITH protect, noconstant(0.0)
 CALL beginservice("228867.003")
 CALL logmessage("Main","Begining main processing",log_debug)
 CALL logmessage("Main","Get list of unique encounters",log_debug)
 SELECT INTO "nl:"
  FROM charge_batch c,
  ...
  PLAN (c
   ...
   JOIN (d
   WHERE d.charge_batch_id=outerjoin(c.charge_batch_id)
    AND d.active_ind=outerjoin(1))
   JOIN (e
   WHERE e.encntr_id=outerjoin(d.encntr_id)
    AND e.active_ind=outerjoin(1))
  ORDER BY c.charge_batch_id, e.encntr_id
  ...
  HEAD e.encntr_id
   IF (e.encntr_id > 0)
    cntencntr = (cntencntr+ 1), stat = alterlist(reply->encounter,cntencntr), reply->encounter[
    cntencntr].encntrid = e.encntr_id,
    reply->encounter[cntencntr].registrationdate = cnvtdatetime(e.reg_dt_tm), reply->encounter[
    cntencntr].personid = e.person_id
   ENDIF
  DETAIL
   IF (d.charge_batch_detail_id > 0)
    batchtotal = (batchtotal+ d.service_item_price_amt)
   ENDIF
  WITH nocounter
 ;end select
 IF ((reply->batchstatus <= 0))
  CALL exitservicenodata("No records found for given batch id",go_to_exit_script)
 ENDIF
 SET reply->batchtotal = batchtotal
 IF (size(reply->encounter,5) > 0)
  CALL logmessage("Main","Get FIN for each encounter",log_debug)
  SELECT INTO "nl:"
   FROM encntr_alias ea,
    (dummyt d1  WITH seq = value(size(reply->encounter,5)))
   PLAN (d1)
    JOIN (ea
    WHERE (ea.encntr_id=reply->encounter[d1.seq].encntrid)
     AND ea.encntr_alias_type_cd=cs319_fin_nbr_cd
     AND ea.beg_effective_dt_tm <= cnvtdatetime(curdate,curtime3)
     AND ea.end_effective_dt_tm > cnvtdatetime(curdate,curtime3)
     AND ea.active_ind=1)
   DETAIL
    reply->encounter[d1.seq].encntrnumber = ea.alias
   WITH nocounter
  ;end select
 ENDIF
