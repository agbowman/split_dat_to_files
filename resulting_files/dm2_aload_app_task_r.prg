CREATE PROGRAM dm2_aload_app_task_r
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_apptaskrel",".log","dm2_aload_app_task_r")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_app_task_r"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE daatr_pkg = i4 WITH protect, noconstant( $1)
 DECLARE daatr_file = vc WITH protect, noconstant( $2)
 DECLARE daatr_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE daatr_batch = i4 WITH protect, noconstant(0)
 DECLARE daatr_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE daatr_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(daatr_pkg,daatr_file_loc_ret,daatr_batch)=0)
  GO TO exit_program
 ENDIF
 IF (daatr_batch=0)
  SET daatr_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_OCD_APP_TASK_R for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND daatr_rows_returned=10000)
   DELETE  FROM dm_ocd_app_task_r d
    WHERE d.alpha_feature_nbr=daatr_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET daatr_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",daatr_rows_returned))
   ENDIF
 ENDWHILE
 IF (daatr_row_cnt > 0)
  EXECUTE dm2_dbimport concat(daatr_file_loc_ret,daatr_file), "dm2_aload_atr_apptaskr", daatr_batch
  CALL echo(concat(daatr_file_loc_ret,daatr_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_aload_app_task_r completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("dm2_aload_app_task_r")
END GO
