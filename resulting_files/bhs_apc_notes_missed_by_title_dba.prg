CREATE PROGRAM bhs_apc_notes_missed_by_title:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Note title search str" = "",
  "Note:" = 0,
  "Begin Date" = "CURDATE",
  "End Date" = "CURDATE"
  WITH outdev, s_search_str, f_pattern_id,
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_APC_NOTES_MISSED_BY_TITLE"
 ...
 DECLARE mf_pattern_id = f8 WITH protect, constant(cnvtreal( $F_PATTERN_ID))
 DECLARE ms_type = vc WITH protect, constant(trim(cnvtupper("APC")))
 DECLARE ms_mode = vc WITH protect, constant(trim(cnvtupper("APC")))
 ...
 DECLARE ml_apc_age_hrs = i4 WITH protect, constant(48)
 DECLARE ms_look_interval = vc WITH protect, constant("48,H")
 ...
 CALL echo(build2("mf_DOC_CD: ",mf_doc_cd))
 CALL echo(build2("mf_SIGN_CD: ",mf_sign_cd))
 ...
 ENDIF
 IF (ms_mode="APC")
  SET ms_apc_beg_dt_tm = ms_beg_dt_tm
  SET ms_apc_end_dt_tm = ms_end_dt_tm
 ...
 ;end select
 IF (ms_mode="PCP")
  CALL echo("this script for researching APC only")
  GO TO exit_script
 ELSEIF (ms_mode="APC")
  IF (ms_type != "NUANCE")
   ...
   SELECT DISTINCT INTO "nl:"
    ...
    FROM encounter e,
     ...
     ce_blob cb
    PLAN (ssp
     WHERE ssp.scr_pattern_id=mf_pattern_id)
     JOIN (s
     ...
     WHERE e.encntr_id=s.encounter_id
      AND e.active_ind=1)
     JOIN (srp
     WHERE srp.scr_pattern_id=mf_pattern_id
    ...
    HEAD REPORT
     pl_cnt = 0,
     CALL echo(build2("scr_pattern_id: ",ssp.scr_pattern_id)),
     CALL echo(build2("display: ",srp.display)),
     CALL echo(build2("definition: ",srp.definition))
    HEAD cep.event_id
     pl_cnt = (pl_cnt+ 1)
 ...
 FOR (ml_cnt = 1 TO size(m_rec->fax,5))
   ...
   SELECT INTO "nl:"
    ...
    HEAD REPORT
     pl_cnt = 0, pl_num = 0, ps_blob_out = fillstring(64000," "),
    ...
    HEAD cb.event_id
     ...
     IF (textlen(trim(cb.blob_contents)) > 0)
      ...
      IF (ms_mode="APC"
       ...
       pl_beg = findstring("Report sent to all consultants:",ms_blob,1)
       IF (pl_beg > 0)
     ...
     FOR (ml_recip_cnt = 1 TO size(m_rec->fax[ml_idx].recip,5))
      ...
      IF ((m_rec->fax[ml_idx].recip[ml_recip_cnt].n_exclude_ind=0)
       ...
       IF (textlen(trim(ms_fin_nbr)) > 0)
        ...
        SET ms_filename_prefix = replace(ms_filename_prefix,".","")
        SET ms_filename_prefix = concat("fax4_",substring(4,5,ms_filename_prefix),"_")
 ...
 ENDFOR
 CALL echo(concat("Notes found: ",trim(cnvtstring(size(m_rec->fax,5)))))
 SET y = 0
 SET x = 0
 FOR (x = 1 TO size(m_rec->fax,5))
   IF (size(m_rec->fax[x].recip,5) > 0)
    SET y = (y+ 1)
   ENDIF
 ENDFOR
 CALL echo(concat("Notes with recipient list: ",trim(cnvtstring(y))))
 SET reply->status_data[1].status = "S"
 CALL sbr_upd_dm_info(ms_end_dt_tm)
 IF (mn_csv_ind=1)
