CREATE PROGRAM bed_get_convs_by_field_label:dba
 FREE SET reply
 RECORD reply(
   1 fields[*]
     2 id = f8
 ...
 SELECT INTO "NL:"
  ...
  DETAIL
   IF (cnvtupper(cnvtalphanum(plabel))=cnvtupper(cnvtalphanum(request->field_label)))
    ...
    temp->fields[fcnt].user_defined_ind = pfp.user_defined_ind, temp->fields[fcnt].conv_id = pfp
    .parent_entity_id, temp->fields[fcnt].conv_description = pfc.description,
    temp->fields[fcnt].conv_action_data_source_id = pfa.data_source_id
   ENDIF
  WITH nocounter
 ;end select
 ...
 FOR (f = 1 TO fcnt)
   ...
   ELSE
    IF ((temp->fields[f].user_defined_ind=1))
     SET field_code_value = 0.0
     SELECT INTO "NL"
      FROM code_value cv
      WHERE cv.code_set=356
       AND (cv.description=temp->fields[f].description)
      DETAIL
       field_code_value = cv.code_value
      WITH nocounter
     ;end select
     IF (field_code_value=0)
      SELECT INTO "NL"
       FROM code_value cv
       WHERE cv.code_set=356
        AND (cv.display=temp->fields[f].description)
       DETAIL
        field_code_value = cv.code_value
       WITH nocounter
      ;end select
      IF (field_code_value=0)
       SELECT INTO "NL"
        FROM code_value cv
        WHERE cv.code_set=356
         AND (cv.cdf_meaning=temp->fields[f].description)
        DETAIL
         field_code_value = cv.code_value
        WITH nocounter
       ;end select
     ...
     ENDIF
     IF (field_code_value > 0)
      SET field_level = fillstring(32," ")
      SELECT INTO "NL:"
       FROM code_value_extension cve
       WHERE cve.code_value=field_code_value
        AND cve.field_name="LEVEL"
       DETAIL
        field_level = cve.field_value
       WITH nocounter
      ;end select
      IF (field_level="PERSON")
       SET work_name = "Person User Defined"
      ELSEIF (field_level="ENCOUNTER")
       SET work_name = "Encounter User Defined"
      ENDIF
      SET temp->fields[f].name = concat(work_name,"/",temp->fields[f].description)
     ENDIF
    ELSE
     SET pcnt = 0
     ...
     FOR (p = 1 TO pcnt)
       SET work_name = trim(request->field_label)
       SET next_parent_id = 0.0
       SET data_source_id = 0.0
       SELECT INTO "NL:"
        FROM pm_flx_data_source pfds
      ...
      "PERSON.ENCOUNTER.TRANSFER.LOC_BED_CD")))
       FOR (l = 1 TO lcnt)
         IF ((loc_trees->parents[l].data_source_id=temp->fields[f].conv_action_data_source_id))
          SET temp->fields[f].name = concat(loc_trees->parents[l].name,"/",request->field_label)
         ENDIF
       ENDFOR
      ENDIF
     ENDIF
 ...
 FOR (f = 1 TO fcnt)
   SET reply->fields[f].id = temp->fields[f].id
   IF ((temp->fields[f].name > " "))
    SET reply->fields[f].name = temp->fields[f].name
   ELSE
    SET reply->fields[f].name = temp->fields[f].field
   ENDIF
   SET reply->fields[f].label = temp->fields[f].label
   SET reply->fields[f].required_ind = temp->fields[f].required_ind
   SET reply->fields[f].display_only_ind = temp->fields[f].display_only_ind
   SET reply->fields[f].conv_id = temp->fields[f].conv_id
   SET reply->fields[f].conv_description = temp->fields[f].conv_description
 ENDFOR
 IF (fcnt=0)
