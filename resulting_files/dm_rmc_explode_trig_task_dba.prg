CREATE PROGRAM dm_rmc_explode_trig_task:dba
 ...
 ENDIF
 DECLARE rett_where_str = vc WITH protect, noconstant("")
 DECLARE rett_loop = i4 WITH protect, noconstant(0)
 DECLARE rett_new_ind = i2 WITH protect, noconstant(1)
 DECLARE rett_reason = vc WITH protect, noconstant("")
 DECLARE rett_trigger_name_ndx = i4 WITH protect, noconstant(0)
 DECLARE rett_drop_stmt = vc WITH protect, noconstant("")
 DECLARE rett_batch_size = i4 WITH protect, noconstant(0)
 DECLARE rett_prsnl_person_ind = i2 WITH protect, noconstant(0)
 DECLARE rett_task_level = i4 WITH protect, noconstant(0)
 DECLARE rett_status = vc WITH protect, noconstant(" ")
 DECLARE rett_check_status = i4 WITH protect, noconstant(0)
 IF ((validate(drtw_task_level,- (123))=- (123)))
  DECLARE drtw_task_level = i4 WITH protect, noconstant(0)
 ENDIF
 IF (validate(drtw_process_name,"-123")="-123")
  DECLARE drtw_process_name = vc WITH protect, noconstant("REFRESH TRIGGER PROCESS")
 ELSE
  SET rett_new_ind = 0
 ENDIF
 IF (drtw_process_name="OPEN EVENT PROCESS")
  SET rett_reason = "Automatic Regeneration via open event"
 ELSEIF (validate(trg_regen_reason,"X")="X")
  SET rett_reason = "NOT_SET"
 ELSE
  SET rett_reason = trg_regen_reason
 ENDIF
 IF (drtw_task_level=0)
  SET rett_task_level = 1
 ELSE
  SET rett_task_level = (drtw_task_level+ 10)
 ENDIF
 FREE RECORD rett_tabs
 RECORD rett_tabs(
   1 batch_cnt = i4
   1 batch_qual[*]
     2 batch_num = i4
 ...
 FREE RECORD rett_trig_drop
 RECORD rett_trig_drop(
   1 cnt = i4
   1 qual[*]
     2 trigger_name = vc
 )
 IF (check_logfile("dm_rmc_explode_trig",".log","DM_RMC_EXPLODE_TRIG_TASK LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_explode
 ENDIF
 IF (validate(rett_request->what_tables,"-1234")="-1234")
  FREE RECORD rett_request
  RECORD rett_request(
    1 what_tables = vc
    1 req_special[*]
      2 sp_tbl = vc
  ...
  SET rett_request->what_tables = "*"
 ENDIF
 SET dm_err->eproc = "Starting DM_RMC_EXPLODE_TRIG_TASK"
 IF ((rett_request->environment_id=0.0))
  SELECT INTO "NL:"
   FROM dm_info d
   WHERE d.info_domain="DATA MANAGEMENT"
    AND d.info_name="DM_ENV_ID"
   DETAIL
    task_info->environment_id = d.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "No environment_id was found.  Please run DM_SET_ENV_ID"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_explode
  ENDIF
 ELSE
  SET task_info->environment_id = rett_request->environment_id
 ENDIF
 IF (drtw_process_name="REFRESH TRIGGER PROCESS"
  AND rett_new_ind > 0)
  SET rett_check_status = drtq_check_task_process("REFRESH TRIGGER PROCESS")
  IF (rett_check_status IN (1, 2))
   SET rett_status = drtq_delete_task_process("REFRESH TRIGGER PROCESS")
   IF (rett_status="F")
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_explode
   ENDIF
  ELSEIF (rett_check_status=3)
   SET rett_status = drtq_reset_task_process("REFRESH TRIGGER PROCESS")
   IF (rett_status="F")
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_explode
   ENDIF
   GO TO exit_explode
  ELSEIF (rett_check_status=4)
   SET dm_err->err_ind = 1
   SET dm_err->emsg =
   "Can not add new tasks to DM_REFCHG_TASK_QUEUE, because tasks already exist and are being processed"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_explode
  ELSE
   SET dm_err->err_ind = 1
  ...
  ENDIF
 ENDIF
 IF ((rett_request->what_tables != char(42)))
  IF (size(rett_request->req_special,5) > 0)
   SET rett_where_str = "(("
  ELSE
   SET rett_where_str = " "
  ENDIF
  SET rett_where_str = concat(rett_where_str,
   " dtd.table_name = patstring(rett_request->what_tables) ")
  IF (size(rett_request->req_special,5) > 0)
   SET rett_where_str = concat(rett_where_str,") or  dtd.table_name in (")
   FOR (my_cnt = 1 TO size(rett_request->req_special,5))
    IF (my_cnt != 1)
     SET rett_where_str = concat(rett_where_str,",")
    ENDIF
    SET rett_where_str = concat(rett_where_str,'"',rett_request->req_special[my_cnt].sp_tbl,'"')
   ENDFOR
   SET rett_where_str = concat(rett_where_str,"))")
  ENDIF
 ELSE
  SET rett_where_str = " 1 = 1 "
 ENDIF
 SELECT INTO "NL:"
  FROM dm_info d
  WHERE d.info_domain="RDDS TRIGGER BATCHING"
   AND d.info_name="BATCH SIZE"
  DETAIL
   rett_batch_size = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET rett_batch_size = 50
 ENDIF
 SET dm_err->eproc = "Gathering table list to refresh triggers on"
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   rett_tabs->batch_cnt = 1, stat = alterlist(rett_tabs->batch_qual,1), rett_tabs->batch_qual[1].
   batch_num = 1
  DETAIL
   IF (dtd.table_name IN ("PRSNL", "PERSON"))
    IF (rett_prsnl_person_ind=0)
     rett_prsnl_person_ind = 1
     IF ((rett_tabs->batch_qual[rett_tabs->batch_cnt].tab_cnt=rett_batch_size))
      rett_tabs->batch_cnt = (rett_tabs->batch_cnt+ 1), stat = alterlist(rett_tabs->batch_qual,
       rett_tabs->batch_cnt), rett_tabs->batch_qual[rett_tabs->batch_cnt].batch_num = rett_tabs->
      batch_cnt
     ENDIF
     rett_tabs->batch_qual[rett_tabs->batch_cnt].tab_cnt = (rett_tabs->batch_qual[rett_tabs->
     batch_cnt].tab_cnt+ 1), stat = alterlist(rett_tabs->batch_qual[rett_tabs->batch_cnt].qual,
      rett_tabs->batch_qual[rett_tabs->batch_cnt].tab_cnt), rett_tabs->batch_qual[rett_tabs->
     batch_cnt].qual[rett_tabs->batch_qual[rett_tabs->batch_cnt].tab_cnt].table_name = dtd.table_name
    ENDIF
   ELSE
    IF ((rett_tabs->batch_qual[rett_tabs->batch_cnt].tab_cnt=rett_batch_size))
     rett_tabs->batch_cnt = (rett_tabs->batch_cnt+ 1), stat = alterlist(rett_tabs->batch_qual,
     ...
     batch_cnt
    ENDIF
    rett_tabs->batch_qual[rett_tabs->batch_cnt].tab_cnt = (rett_tabs->batch_qual[rett_tabs->batch_cnt
    ].tab_cnt+ 1), stat = alterlist(rett_tabs->batch_qual[rett_tabs->batch_cnt].qual,rett_tabs->
     batch_qual[rett_tabs->batch_cnt].tab_cnt), rett_tabs->batch_qual[rett_tabs->batch_cnt].qual[
    rett_tabs->batch_qual[rett_tabs->batch_cnt].tab_cnt].table_name = dtd.table_name
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 IF ((rett_request->what_tables=char(42)))
  IF (size(derg_reply->child_env_list,5)=0)
   SET derg_request->env_id = task_info->environment_id
  ...
  FOR (rett_loop = 1 TO size(derg_reply->child_env_list,5))
    SET auto_ver_request->qual[rett_loop].rdds_event = "Add Environment Triggers"
    SET auto_ver_request->qual[rett_loop].cur_environment_id = task_info->environment_id
    SET auto_ver_request->qual[rett_loop].paired_environment_id = derg_reply->child_env_list[
    rett_loop].env_id
    IF (rett_reason="NOT_SET")
     SET auto_ver_request->qual[rett_loop].event_reason = "Regeneration via EXPLODE script"
    ELSE
     SET auto_ver_request->qual[rett_loop].event_reason = rett_reason
    ENDIF
  ENDFOR
  EXECUTE dm_rmc_auto_verify_setup
  IF ((auto_ver_reply->status="F"))
  ...
  ELSE
   COMMIT
  ENDIF
  SET rett_events->cnt = size(derg_reply->child_env_list,5)
  SET stat = alterlist(rett_events->qual,rett_events->cnt)
  FOR (rett_loop = 1 TO rett_events->cnt)
    SET rett_events->qual[rett_loop].target_id = derg_reply->child_env_list[rett_loop].env_id
    SELECT INTO "NL:"
     y = max(d.dm_rdds_event_log_id)
     FROM dm_rdds_event_log d
     WHERE d.rdds_event_key="ADDENVIRONMENTTRIGGERS"
      AND (d.cur_environment_id=task_info->environment_id)
      AND (d.paired_environment_id=rett_events->qual[rett_loop].target_id)
     DETAIL
      rett_events->qual[rett_loop].event_id = y
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SET stat = alterlist(task_info->qual,rett_tabs->batch_cnt)
 FOR (rett_loop = 1 TO rett_tabs->batch_cnt)
   SET task_info->total = (task_info->total+ 1)
   SET task_info->qual[task_info->total].task_level = rett_task_level
   SET task_info->qual[task_info->total].task_desc = concat("Refreshing Logging Triggers: Batch ",
    trim(cnvtstring(rett_loop)))
   SET task_info->qual[task_info->total].task_name = concat("execute DM_RMC_RUN_TRG_BATCH ",trim(
     cnvtstring(rett_loop))," go")
   IF (rett_reason != "NOT_SET")
    SET task_info->qual[task_info->total].task_request = concat(task_info->qual[task_info->total].
     task_request,"<VAR_DEF>declare trg_regen_reason = vc go</VAR_DEF> ",
     "<VAL_SET>set trg_regen_reason = '",rett_reason,"' go</VAL_SET> ")
   ENDIF
   IF ((rett_request->what_tables=char(42)))
    SET task_info->qual[task_info->total].task_request = concat(task_info->qual[task_info->total].
     task_request,"<VAR_DEF>declare trg_all_trigs_ind = i2 go</VAR_DEF> ",
     "<VAL_SET>set trg_all_trigs_ind = 1 go</VAL_SET> ",
     "<REC_DEF>record trg_event_info (1 cnt = i4 1 qual[*] 2 event_id = f8 2 target_id = f8) go</REC_DEF>",
     "<VAL_SET>set stat = alterlist(trg_event_info->qual, ",
     trim(cnvtstring(rett_events->cnt)),") go</VAL_SET>","<VAL_SET>set trg_event_info->cnt = ",trim(
      cnvtstring(rett_events->cnt))," go</VAL_SET>")
    FOR (rett_trigger_name_ndx = 1 TO rett_events->cnt)
      SET task_info->qual[task_info->total].task_request = concat(task_info->qual[task_info->total].
       task_request,"<VAL_SET>set trg_event_info->qual[",trim(cnvtstring(rett_trigger_name_ndx)),
       "].event_id = ",trim(cnvtstring(rett_events->qual[rett_trigger_name_ndx].event_id)),
       ".0 go</VAL_SET>","<VAL_SET>set trg_event_info->qual[",trim(cnvtstring(rett_trigger_name_ndx)),
       "].target_id = ",trim(cnvtstring(rett_events->qual[rett_trigger_name_ndx].target_id)),
       ".0 go</VAL_SET>")
    ENDFOR
   ENDIF
 ENDFOR
 IF ((rett_request->what_tables=char(42)))
  SET dm_err->eproc = "Calling DM_CREATE_RDDS_LOGON_TRG"
  EXECUTE dm_create_rdds_logon_trg
  IF (check_error(dm_err->eproc) > 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_explode
  ENDIF
  SET dm_err->eproc = "Calling DM_RMC_CHECK_UNMAPPED"
  EXECUTE dm_rmc_check_unmapped
  IF (check_error(dm_err->eproc) > 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_explode
  ENDIF
  SET dm_err->eproc = "Calling DM2_RDDS_METADATA_REFRESH"
  EXECUTE dm2_rdds_metadata_refresh "NONE"
  IF (check_error(dm_err->eproc) > 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_explode
  ENDIF
  SET dm_err->eproc = "Calling DM_RDDS_UPDATE_TRIG_PROC"
  ...
  SELECT INTO "NL:"
   ...
   DETAIL
    rett_trigger_name_ndx = 0, rett_trigger_name_ndx = locateval(rett_trigger_name_ndx,1,
     rett_trig_drop->cnt,ut.trigger_name,rett_trig_drop->qual[rett_trigger_name_ndx].trigger_name)
    IF (rett_trigger_name_ndx=0)
     rett_trig_drop->cnt = (rett_trig_drop->cnt+ 1), stat = alterlist(rett_trig_drop->qual,
      rett_trig_drop->cnt), rett_trig_drop->qual[rett_trig_drop->cnt].trigger_name = ut.trigger_name
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_explode
  ENDIF
  FOR (rett_trigger_name_ndx = 1 TO rett_trig_drop->cnt)
   SET rett_drop_stmt = concat("rdb drop trigger ",rett_trig_drop->qual[rett_trigger_name_ndx].
    trigger_name," go")
   CALL dm2_push_cmd(rett_drop_stmt,1)
  ENDFOR
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_explode
  ENDIF
 ENDIF
 IF ((rett_tabs->batch_cnt > 0))
  DELETE  FROM dm_info
   WHERE info_domain="RDDS TRIGGER BATCHING"
    AND info_name != "BATCH_SIZE"
   WITH nocounter
  ;end delete
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  FOR (rett_loop = 1 TO rett_tabs->batch_cnt)
   INSERT  FROM dm_info dc,
     (dummyt d  WITH seq = rett_tabs->batch_qual[rett_loop].tab_cnt)
    SET dc.info_name = rett_tabs->batch_qual[rett_loop].qual[d.seq].table_name, dc.info_domain =
     "RDDS TRIGGER BATCHING", dc.info_number = rett_loop
    PLAN (d)
     JOIN (dc
     WHERE (dc.info_name=rett_tabs->batch_qual[rett_loop].qual[d.seq].table_name)
      AND dc.info_domain="RDDS TRIGGER BATCHING")
    WITH nocounter
   ;end insert
   IF (check_error("Inserting batch data into DM_INFO") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ROLLBACK
    GO TO exit_explode
   ENDIF
  ENDFOR
  COMMIT
  SET task_info->process_name = drtw_process_name
  SET dm_err->eproc = "Inserting create r table tasks."
  SET drcp_status = drtq_insert_task_process(task_info,rett_new_ind)
  IF (drcp_status="F")
   CALL disp_msg("Call to drtq_insert_task_process returned an error",dm_err->logfile,1)
   GO TO exit_explode
  ENDIF
 ENDIF
#exit_explode
