CREATE PROGRAM bbt_send_product_status_change:dba
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 FREE RECORD prod_info
 RECORD prod_info(
   1 products[*]
     2 product_id = f8
     2 product_nbr = vc
     2 unit_product_code = vc
     2 status_flag = i2
 ...
 DECLARE ninventorydevicesprefind = i2 WITH protect, noconstant(0)
 DECLARE get_product_information(null) = null
 DECLARE make_fsi_call_for_status_change(null) = null
 ...
 SUBROUTINE (bbtgetprodtagverifypreference(facility_code=f8(value)) =vc)
   ...
   ENDIF
 END ;Subroutine
 SET ninventorydevicesprefind = bbtgetinterfaceddevicespreference(0.0)
 IF (ninventorydevicesprefind > 0)
  SET nproductcnt = size(request->products,5)
  SET stat = alterlist(prod_info->products,nproductcnt)
  CALL get_product_information(null)
  IF (validate(request->debug_ind,0) > 0)
   CALL echorecord(prod_info)
  ENDIF
  FOR (nidx = 1 TO nproductcnt)
    IF ((prod_info->products[nidx].interface_flag > 0))
     CALL make_fsi_call_for_status_change(null)
    ENDIF
  ENDFOR
 ENDIF
 GO TO set_status
 SUBROUTINE get_product_information(null)
   SELECT
    p.product_nbr, p.product_type_barcode, l.organization_id
    ...
    ORDER BY d.seq
    HEAD d.seq
     prod_info->products[d.seq].org_id = l.organization_id, prod_info->products[d.seq].interface_flag
      = p.interfaced_device_flag, prod_info->products[d.seq].product_nbr = p.product_nbr,
     prod_info->products[d.seq].unit_product_code = p.product_type_barcode, prod_info->products[d.seq
     ].product_id = p.product_id, prod_info->products[d.seq].status_flag = request->products[d.seq].
     status_flag,
     prod_info->products[d.seq].dereservation_dt_tm = request->products[d.seq].dereservation_dt_tm,
     prod_info->products[d.seq].reason_cd = request->products[d.seq].reason_cd
    WITH nocounter
   ;end select
 END ;Subroutine
 SUBROUTINE (errorhandler(operationstatus=c1(value),targetobjectname=vc(value),targetobjectvalue=vc(
   ...
   GO TO exit_script
 END ;Subroutine
 SUBROUTINE make_fsi_call_for_status_change(null)
   ...
   SET hmsg = uar_srvselectmessage(1215067)
   ...
   SET stat = uar_srvsetstring(hcqmstruct,"ContribRefnum",nullterm(concat(format(cnvtdatetime(sysdate
        ),";;q")," SC")))
   ...
   SET stat = uar_srvsetstring(hcqmstruct,"Subtype",nullterm("SC"))
   ...
   SET stat = uar_srvsetdouble(htrig,"organization_id",prod_info->products[nidx].org_id)
   IF (size(trim(prod_info->products[nidx].product_nbr))=13)
    IF (substring(1,1,prod_info->products[nidx].product_nbr)="!")
     SET prodnbr = prod_info->products[nidx].product_nbr
    ELSE
     SET prodnbr = concat(prod_info->products[nidx].product_nbr,calculatecheckdigit(prod_info->
       products[nidx].product_nbr))
    ENDIF
   ELSE
    SET prodnbr = prod_info->products[nidx].product_nbr
   ENDIF
   SET stat = uar_srvsetstring(htrig,"unit_number",nullterm(prodnbr))
   SET stat = uar_srvsetstring(htrig,"unit_product_code",nullterm(prod_info->products[nidx].
     unit_product_code))
   SET stat = uar_srvsetshort(htrig,"status_flag",prod_info->products[nidx].status_flag)
   SET stat = uar_srvsetstring(htrig,"description",nullterm(uar_get_code_display(prod_info->products[
      nidx].reason_cd)))
   SET stat = uar_srvsetdate(htrig,"dereservation_dt_tm",cnvtdatetime(prod_info->products[nidx].
     dereservation_dt_tm))
