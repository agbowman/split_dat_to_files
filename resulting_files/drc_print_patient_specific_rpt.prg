CREATE PROGRAM drc_print_patient_specific_rpt
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Clinical Conditions Data (Yes/No)" = "YES",
  "Post Menstrual Age Data (Yes/No)" = "YES",
  "Hepatic Dysfunction Data (Yes/No)" = "YES",
  "Creatinine Clearance Data (Yes/No)" = "YES"
  WITH outdev, condprompt, pmaprompt,
  hepaticprompt, crclprompt
 ...
 DECLARE prem_type = vc WITH public, noconstant(" ")
 ...
 IF (((( $CRCLPROMPT="YES")) OR (((( $PMAPROMPT="YES")) OR (( $CONDPROMPT="YES"))) )) )
  SET prem_type = build("(","dpa.premise_type_flag in (")
  IF (( $CRCLPROMPT="YES"))
   SET prem_type = build(prem_type,"4")
  ENDIF
  IF (( $PMAPROMPT="YES")
   AND ( $CRCLPROMPT="YES"))
   SET prem_type = build(prem_type,", 5")
  ELSEIF (( $PMAPROMPT="YES"))
   SET prem_type = build(prem_type,"5")
  ENDIF
  IF (( $CONDPROMPT="YES")
   AND ((( $CRCLPROMPT="YES")) OR (( $PMAPROMPT="YES"))) )
   SET prem_type = build(prem_type,", 7")
  ELSEIF (( $CONDPROMPT="YES"))
   SET prem_type = build(prem_type,"7")
  ENDIF
  SET prem_type = build(prem_type,")",")")
  IF (( $HEPATICPROMPT="YES"))
   SET prem_type = build(prem_type," OR (dpa.premise_type_flag = 6 and dpa.value1 = 1) ")
  ENDIF
 ELSEIF (( $HEPATICPROMPT="YES"))
  SET prem_type = build("dpa.premise_type_flag = 6 and dpa.value1 = 1")
 ELSE
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dfr
   ...
   WHERE dp.dose_range_check_id=drc.dose_range_check_id
    ...
    AND dp.active_ind=1
    AND (dp.drc_premise_id=
   (SELECT
    dpa.parent_premise_id
    FROM drc_premise dpa
    WHERE parser(prem_type))))
   JOIN (dp2
  ...
  *
  ...
  HEAD d2.seq
   ...
   FOR (x = 1 TO sorted_ranges->groupers[d1.seq].premises[d2.seq].dose_range_cnt)
     ...
     "##########;L;T", col 115,
     sorted_ranges->groupers[d1.seq].premises[d2.seq].dose_range[x].max_dose"###.##;R;T", col 123,
