CREATE PROGRAM dm2_ads_purge_runner:dba
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_ads_purge_run",".log","dm2_ads_purge_runner")=0)
  GO TO exit_program
 ENDIF
 FREE RECORD ddr_execlist
 RECORD ddr_execlist(
   1 exec_cnt = i4
   1 config_id = f8
   1 config_name = vc
 ...
 FREE RECORD ddr_reply
 RECORD ddr_reply(
   1 error_ind = i2
 )
 FREE RECORD ddr_inact_runners
 RECORD ddr_inact_runners(
   1 list[*]
     2 op_id = f8
 )
 DECLARE ddr_batch_size = i4 WITH protect, noconstant(0)
 DECLARE ddr_continue = i2 WITH protect, noconstant(1)
 DECLARE ddr_priority = f8 WITH protect, noconstant(1000.0)
 DECLARE ddr_priority_max = f8 WITH protect, noconstant(0.0)
 DECLARE ddr_wait_cnt = i4 WITH protect, noconstant(0)
 DECLARE ddr_runners_running = i4 WITH protect, noconstant(0)
 DECLARE ddr_runner_cnt = i4 WITH protect, noconstant(0)
 DECLARE ddr_purge_fail_ind = i2 WITH protect, noconstant(0)
 DECLARE ddr_inact_run_cnt = i2 WITH protect, noconstant(0)
 SET dm_err->eproc = "Starting dm2_ads_purge_runner"
 CALL disp_msg("",dm_err->logfile,0)
 EXECUTE dm2_set_context "FIRE_REFCHG_TRG", "NO"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Retrieving ADS Purge sample information"
 SELECT INTO "nl:"
  FROM dm_ads_config dac
  WHERE dac.sample_method=dpl_purge
  DETAIL
   ddr_execlist->config_id = dac.dm_ads_config_id, ddr_execlist->config_name = dac.config_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Updating ADS Purge sample status"
 UPDATE  FROM dm_ads_config d
  SET d.config_status = "PURGE_RUNNING", d.updt_applctx = reqinfo->updt_applctx, d.updt_cnt = (d
   .updt_cnt+ 1),
   d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_id = reqinfo->updt_id, d.updt_task = reqinfo
   ->updt_task
  WHERE (d.dm_ads_config_id=ddr_execlist->config_id)
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Updating ADS Purge runner information"
 UPDATE  FROM dm_info d
  SET d.info_number = (d.info_number+ 1), d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d
   .updt_applctx = reqinfo->updt_applctx,
   d.updt_cnt = (d.updt_cnt+ 1), d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_id = reqinfo->
   updt_id,
 ...
 IF (curqual=0)
  INSERT  FROM dm_info d
   SET d.info_domain = "ADS PURGE RUNNER", d.info_name = "RUNNING", d.info_number = 1,
  ...
  ENDIF
  IF (curqual=0)
   SET dm_err->eproc = "No delete operations found on DM2_DDL_OPS_LOG"
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_program
  ENDIF
  COMMIT
 ENDIF
 SET dm_err->eproc = "Retrieving delete batch size from DM_INFO"
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DATABASE MANAGEMENT"
   AND d.info_name="DDR BATCH SIZE"
  DETAIL
   ddr_batch_size = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (curqual=0)
  SET ddr_batch_size = 1
 ENDIF
 SET dm_err->eproc = "Retrieving max priority level for deletes from DM2_DDL_OPS_LOG"
 SELECT INTO "nl:"
  ddr_max = max(d.priority)
  FROM dm2_ddl_ops_log d
  WHERE op_type="PURGE ACTIVITY DATA"
  DETAIL
   ddr_priority_max = ddr_max
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 WHILE (ddr_continue=1)
   SET ddr_execlist->exec_cnt = 0
   SET stat = alterlist(ddr_execlist->exec,0)
   SET dm_err->eproc = "Retrieving batch of deletes to process from DM2_DDL_OPS_LOG"
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="ADS PURGE RUNNER"
     AND d.info_name="MAX RUNNERS"
    DETAIL
     ddr_runner_cnt = d.info_number
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (ddr_runner_cnt=0)
    SET ddr_continue = 0
   ELSE
    SET dm_err->eproc = "Retrieving incomplete DM2_DDL_OPS_LOG commands to run"
    SELECT INTO "nl:"
     FROM dm2_ddl_ops_log d
     WHERE d.op_type="PURGE ACTIVITY DATA"
      AND d.status="INCOMPLETE"
      AND d.priority=ddr_priority
     DETAIL
      ddr_execlist->exec_cnt = (ddr_execlist->exec_cnt+ 1)
      IF (mod(ddr_execlist->exec_cnt,10)=1)
       stat = alterlist(ddr_execlist->exec,(ddr_execlist->exec_cnt+ 9))
      ENDIF
      ddr_execlist->exec[ddr_execlist->exec_cnt].table_name = d.table_name, ddr_execlist->exec[
      ddr_execlist->exec_cnt].op_id = d.op_id, ddr_execlist->exec[ddr_execlist->exec_cnt].operation
       = d.operation,
      ddr_execlist->exec[ddr_execlist->exec_cnt].priority = d.priority
     WITH maxqual(d,value(ddr_batch_size)), forupdatewait(d)
    ;end select
    SET stat = alterlist(ddr_execlist->exec,ddr_execlist->exec_cnt)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    IF ((ddr_execlist->exec_cnt > 0))
     SET dm_err->eproc = "Calling dm2_ads_purge_runner_c child program... "
     CALL disp_msg(" ",dm_err->logfile,0)
     SET dm_err->eproc = "Executing DM2_ADS_PURGE_RUNNER_C"
     EXECUTE dm2_ads_purge_runner_c  WITH replace("REQUEST","DDR_EXECLIST"), replace("REPLY",
      "DDR_REPLY")
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
    ELSE
     SET dm_err->eproc = "Validating all operations on current level are complete"
     SET dm_err->eproc = "Validating all ADS Purge DDL has been executed"
     SELECT INTO "nl:"
      FROM dm2_ddl_ops_log d
      WHERE d.op_type="PURGE ACTIVITY DATA"
       AND d.status="RUNNING"
     ...
     IF (curqual > 0)
      CALL pause(5)
      SET ddr_wait_cnt = (ddr_wait_cnt+ 1)
      IF (ddr_wait_cnt >= 120)
       SET ddr_continue = 0
      ENDIF
     ELSE
      SET dm_err->eproc = "Validating all ADS Purge DDL has run successfully"
      SELECT INTO "nl:"
       FROM dm2_ddl_ops_log d
       WHERE d.op_type="PURGE ACTIVITY DATA"
        AND d.status="FAILED"
      ...
      ENDIF
      IF (curqual > 0)
       SET ddr_purge_fail_ind = 1
      ENDIF
      IF ((((ddr_reply->error_ind=1)) OR (ddr_purge_fail_ind=1)) )
       SET ddr_continue = 0
      ELSE
       SET ddr_priority = (ddr_priority+ 1)
       IF (ddr_priority > ddr_priority_max)
        SET ddr_continue = 0
       ENDIF
      ENDIF
     ENDIF
 ...
 ENDWHILE
#exit_program
 IF (ddr_purge_fail_ind=0)
  SET dm_err->eproc = "Updating ADS Purge runner rows"
  UPDATE  FROM dm_info d
   SET d.info_number = (d.info_number+ 1), d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d
    .updt_applctx = reqinfo->updt_applctx,
    d.updt_cnt = (d.updt_cnt+ 1), d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_id = reqinfo
  ...
  IF (curqual=0)
   INSERT  FROM dm_info d
    SET d.info_domain = "ADS PURGE RUNNER", d.info_name = "COMPLETE", d.info_number = 1,
     d.updt_applctx = reqinfo->updt_applctx, d.updt_cnt = 0, d.updt_dt_tm = cnvtdatetime(curdate,
      curtime3),
     d.updt_id = reqinfo->updt_id, d.updt_task = reqinfo->updt_task
    WITH nocounter
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Checking DM2_DDL_OPS_LOG for hung ADS Purge processes"
 SELECT INTO "nl:"
  FROM dm2_ddl_ops_log d
  WHERE d.op_type="PURGE ACTIVITY DATA"
   AND d.status="RUNNING"
   AND  NOT (d.appl_id IN (
  (SELECT
   audsid
   FROM gv$session)))
  DETAIL
   ddr_inact_run_cnt = (ddr_inact_run_cnt+ 1), stat = alterlist(ddr_inact_runners->list,
    ddr_inact_run_cnt), ddr_inact_runners->list[ddr_inact_run_cnt].op_id = d.op_id
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO end_program
 ENDIF
 IF (ddr_inact_run_cnt > 0)
  UPDATE  FROM dm2_ddl_ops_log ddol,
    (dummyt d  WITH seq = ddr_inact_run_cnt)
   SET ddol.status = "INCOMPLETE", ddol.appl_id = currdbhandle
   PLAN (d)
    JOIN (ddol
    WHERE (ddol.op_id=ddr_inact_runners->list[d.seq].op_id))
   WITH nocounter
  ;end update
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Updating ADS Purge runner DM_INFO rows"
 UPDATE  FROM dm_info d
  SET d.info_number = ((d.info_number - ddr_inact_run_cnt) - 1), d.updt_applctx = reqinfo->
   updt_applctx, d.updt_cnt = (d.updt_cnt+ 1),
   d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_id = reqinfo->updt_id, d.updt_task = reqinfo
   ->updt_task
  WHERE d.info_domain="ADS PURGE RUNNER"
   AND d.info_name="RUNNING"
 ...
 SELECT INTO "nl:"
  ...
  WHERE d.info_domain="ADS PURGE RUNNER"
   AND d.info_name="RUNNING"
  DETAIL
   ddr_runners_running = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 COMMIT
 IF (ddr_runners_running=0
  AND ddr_purge_fail_ind=0)
  UPDATE  FROM dm2_ddl_ops d
   SET d.last_checkpoint = "COMPLETE"
   WHERE d.process_option="PURGE"
    AND d.last_checkpoint="INITIAL"
   WITH nocounter
  ;end update
  UPDATE  FROM dm_ads_config d
   SET d.config_status = "PURGE_COMPLETE", d.updt_applctx = reqinfo->updt_applctx, d.updt_cnt = (d
    .updt_cnt+ 1),
 ...
 ENDIF
#end_program
 SET dm_err->eproc = "Ending dm2_ads_purge_runner"
 CALL final_disp_msg(" ")
END GO
