CREATE PROGRAM bed_get_res_sch_nurse:dba
 FREE SET reply
 RECORD reply(
   1 resources[*]
 ...
 SELECT INTO "nl:"
  ...
  PLAN (b
   WHERE b.br_nv_key1="SCHRESGROUP"
    AND b.br_name="NURSE")
  DETAIL
   nurse_id = b.br_name_value_id, nurse_txt = build(b.br_name_value_id)
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM br_name_value b,
  ...
  PLAN (b
   WHERE b.br_nv_key1="SCHRESGROUPRES"
    AND b.br_value=nurse_txt)
   JOIN (s
   WHERE s.resource_cd=cnvtint(b.br_name)
