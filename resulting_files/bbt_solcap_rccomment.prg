CREATE PROGRAM bbt_solcap_rccomment
 ...
 SET reply->solcap[1].identifier = "2015.2.00152.1"
 SET reply->solcap[1].degree_of_use_num = 0
 SET reply->solcap[1].distinct_user_count = 0
 SET reply->solcap[1].degree_of_use_str = bbtcorrectcommentpromptpreference(0.0)
 FREE SET res_count
 DECLARE res_count = i4 WITH noconstant(0)
 DECLARE yes_count = i4 WITH noconstant(0)
 SELECT
  ...
  DETAIL
   ...
   value_str = bbtcorrectcommentpromptpreference(cv.code_value)
   IF ((reply->solcap[1].facility[res_count].value_str="Yes"))
    yes_count += 1
   ENDIF
  FOOT REPORT
   stat = alterlist(reply->solcap[1].facility,res_count), reply->solcap[1].degree_of_use_num =
   yes_count
   IF (yes_count > 0)
