CREATE PROGRAM dm_ins_merge_translates:dba
 ...
 DECLARE dcm_cnttbl = i4 WITH protect
 DECLARE dcm_total = i4 WITH protect
 DECLARE dimt_target_env = f8 WITH protect
 DECLARE dimt_source_env = f8 WITH protect
 SET dcm_cnt = 0
 SET dcm_count = 0
 SET dcm_value_cnt = 0
 SET dcm_more_del = 1
 SET dcm_cnttbl = 0
 SET dcm_total = 0
 SET dimt_target_env = 0.0
 SET dimt_source_env = 0.0
 DECLARE trans_spec(null) = null
 DECLARE trans_spec1(null) = null
 CALL check_logfile("dm_ins_merge_trans",".log","DM_INS_MERGE_TRANSLATES")
 SET dm_err->eproc = "Start dm_ins_merge_translates"
 SET dimt_source_env =  $1
 SET dimt_target_env =  $2
 CALL echo(build("Source environment ID:",dimt_source_env))
 CALL echo(build("Target environment ID:",dimt_target_env))
 ...
 SET spec_tbl->spec_total = 5
 SET stat = alterlist(spec_tbl->list,spec_tbl->spec_total)
 SET spec_tbl->list[1].table_name = "PERSON"
 SET spec_tbl->list[2].table_name = "PERSON_NAME"
 SET spec_tbl->list[3].table_name = "PERSON_ALIAS"
 SET spec_tbl->list[4].table_name = "ADDRESS"
 SET spec_tbl->list[5].table_name = "PHONE"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (utc.column_name="DCP_SECTION_INSTANCE_ID")
    ...
    "DCP_SECTION_REF_ID",
    dcm_trans_table->list[(dcm_cnt+ 1)].distinct_ind = 1, dcm_cnt += 1
   ENDIF
 ...
 CALL echo("Deleting unnecessary rows from table dm_merge_translate....")
 WHILE (dcm_more_del)
   DELETE  FROM dm_merge_translate d
    WHERE d.env_source_id=dimt_source_env
     AND d.env_target_id=dimt_target_env
    WITH nocounter, maxqual(d,20000)
   ;end delete
   SET err_code = error(errmsg,0)
   IF (err_code > 0)
    CALL echo(fillstring(80,"*"))
    CALL echo("ERROR occurred when deleting rows from dm_merge_translate!")
    CALL echo(errmsg)
    CALL echo("Please resolve this issue and then execute this program again.")
    CALL echo(fillstring(80,"*"))
    GO TO exit_program
   ENDIF
   SET dcm_more_del = curqual
   COMMIT
   CALL echo("delete 20000 rows successfully")
 ENDWHILE
 FREE RECORD dcm_trans_value
 RECORD dcm_trans_value(
   1 value_cnt = i4
   1 value[*]
     2 from_to_value = f8
 )
 SET dcm_total = (dcm_cnt+ spec_tbl->spec_total)
 WHILE (dcm_count < dcm_cnt)
   SET dcm_count += 1
   SET dcm_trans_value->value_cnt = 0
   CALL echo("*")
   CALL echo("**************************************************")
   CALL echo(build("Working on table:"," ",dcm_trans_table->list[dcm_count].table_name," ","(",
     dcm_count,")","of total","(",dcm_total,
     ")"))
   CALL echo("**************************************************")
   CALL echo("*")
   EXECUTE dm_ins_merge_trans_child dcm_trans_table->list[dcm_count].table_name, dcm_trans_table->
   list[dcm_count].column_name, dimt_source_env,
   dimt_target_env, dcm_trans_table->list[dcm_count].distinct_ind
   IF ((dm_err->err_ind=1))
    SET dm_err->eproc = concat("ERROR: ",dcm_trans_table->list[dcm_count].table_name)
    SET dcm_count = (dcm_cnt+ 1)
    CALL disp_msg(" ",dm_err->logfile,0)
    GO TO exit_program
   ENDIF
 ENDWHILE
 CALL trans_spec1(null)
 SUBROUTINE trans_spec(null)
   DECLARE spc_cnt = i4 WITH protect
   SET spc_cnt = 0
   IF (size(spec_tbl->list,5) > 0)
    SELECT INTO "nl:"
     FROM user_cons_columns ucc,
      user_constraints uc,
      (dummyt d  WITH seq = value(spec_tbl->spec_total))
     PLAN (d)
      JOIN (uc
      WHERE (uc.table_name=spec_tbl->list[d.seq].table_name)
       AND uc.constraint_type="P"
     ...
     DETAIL
      spec_tbl->list[d.seq].column_name = ucc.column_name
     WITH nocounter
    ;end select
   ENDIF
   FOR (spec_lp = 1 TO spec_tbl->spec_total)
     SET spc_cnt = 0
     SET stat = alterlist(dcm_trans_value->value,0)
     SET dcm_trans_value->value_cnt = 0
     CALL echo("*")
     CALL echo("**************************************************")
     CALL echo(build("Working on table:"," ",spec_tbl->list[spec_lp].table_name," ","(",
       (dcm_cnt+ spec_lp),")","of total","(",dcm_total,
       ")"))
     CALL echo("**************************************************")
     CALL echo("*")
     IF ((spec_tbl->list[spec_lp].table_name IN ("PERSON", "PERSON_NAME", "PERSON_ALIAS")))
      SELECT INTO "nl:"
       FROM (value(spec_tbl->list[spec_lp].table_name) p),
        prsnl pr
       WHERE p.person_id=pr.person_id
       DETAIL
        spc_cnt += 1
        IF (mod(spc_cnt,1000)=1)
         stat = alterlist(dcm_trans_value->value,(spc_cnt+ 999))
        ENDIF
        dcm_trans_value->value[spc_cnt].from_to_value = parser(build("p.",spec_tbl->list[spec_lp].
          column_name))
       FOOT REPORT
        stat = alterlist(dcm_trans_value->value,spc_cnt), dcm_trans_value->value_cnt = spc_cnt
       WITH nocounter
      ;end select
     ELSEIF ((spec_tbl->list[spec_lp].table_name IN ("PHONE", "ADDRESS")))
      SELECT INTO "nl:"
       FROM (value(spec_tbl->list[spec_lp].table_name) p),
        prsnl pr
       WHERE p.parent_entity_name IN ("PRSNL", "PERSON")
        AND p.parent_entity_id=pr.person_id
       DETAIL
        spc_cnt += 1
        IF (mod(spc_cnt,1000)=1)
         stat = alterlist(dcm_trans_value->value,(spc_cnt+ 999))
      ...
      ;end select
     ENDIF
     CALL dmt_insert(spec_tbl->list[spec_lp].table_name)
   ENDFOR
 END ;Subroutine
 SUBROUTINE trans_spec1(null)
   DECLARE spc_cnt = i4 WITH protect
   DECLARE spc_max = f8 WITH protect
   DECLARE spc_var = f8 WITH protect
   DECLARE spc_cflag = i2 WITH protect
   DECLARE spc_sta = f8 WITH protect
   SET spc_cnt = 0
   IF (size(spec_tbl->list,5) > 0)
    SELECT INTO "nl:"
     FROM user_cons_columns ucc,
   ...
   FOR (spec_lp = 1 TO spec_tbl->spec_total)
     ...
     CALL echo("**************************************************")
     CALL echo("*")
     SET dm_err->eproc = concat("ERROR on table ",spec_tbl->list[spec_lp].table_name)
     EXECUTE dm_ins_merge_trans_child spec_tbl->list[spec_lp].table_name, spec_tbl->list[spec_lp].
     column_name, dimt_source_env,
     dimt_target_env, 0
     IF ((dm_err->err_ind=1))
      SET dm_err->eproc = concat("ERROR: ",spec_tbl->list[spec_lp].table_name)
      CALL disp_msg(" ",dm_err->logfile,0)
      GO TO exit_program
     ENDIF
   ENDFOR
 END ;Subroutine
 SUBROUTINE (dmt_insert(dmt_tbl=vc) =null)
   DECLARE dcm_finished = i4 WITH protect
   DECLARE dcm_insert_cnt = i4 WITH protect
   SET dcm_finished = 0
   SET dcm_insert_cnt = 0
   SET dcm_cnttbl += 1
   WHILE ((dcm_finished < dcm_trans_value->value_cnt))
     SET dcm_finished = (dcm_insert_cnt+ 500)
     CALL echo("*")
     CALL echo("**************************************************")
     CALL echo("*")
     CALL echo(build("Insert into dm_merge_translate:"," ",dmt_tbl," ","(",
       dcm_cnttbl,")","of total","(",dcm_total,
       ")"))
     CALL echo("*")
     CALL echo("**************************************************")
     CALL echo("*")
     ...
     INSERT  FROM dm_merge_translate dmt,
       (dummyt d  WITH seq = value(dcm_finished))
      SET dmt.from_value = dcm_trans_value->value[d.seq].from_to_value, dmt.table_name = dmt_tbl, dmt
       .env_source_id = dimt_source_env,
       dmt.env_target_id = dimt_target_env, dmt.to_value = dcm_trans_value->value[d.seq].
       from_to_value, dmt.status_flg = 0,
       dmt.error_nbr = 0, dmt.error_msg = null, dmt.merge_id = 0
      PLAN (d
       WHERE d.seq > dcm_insert_cnt
        AND (d.seq <= dcm_trans_value->value_cnt))
       JOIN (dmt)
      WITH nocounter
     ;end insert
     SET err_code = error(errmsg,0)
     IF (err_code > 0)
      CALL echo(fillstring(80,"*"))
      CALL echo("ERROR occurred when inserting rows into dm_merge_translate!")
      CALL echo(errmsg)
      CALL echo("Please resolve this issue and then execute this program again.")
      CALL echo(fillstring(80,"*"))
      GO TO exit_program
     ENDIF
     COMMIT
     SET dcm_insert_cnt = dcm_finished
   ENDWHILE
 END ;Subroutine
#exit_program
 CALL echo("dm_ins_merge_translates is finished")
END GO
