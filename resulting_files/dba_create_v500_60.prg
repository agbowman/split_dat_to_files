CREATE PROGRAM dba_create_v500_60
 SET message = window
 FREE SET ts_data
 RECORD ts_data(
   1 tname = vc
 ...
 SUBROUTINE aix_gen_temp_rb(x)
   ...
   SELECT INTO concat("script_dir","/create_rbs.sql")
    ...
    DETAIL
     ...
     ELSE
      col 0, "datafile '/dev/r",
      CALL print(trim(a.file_name)),
      "'", " size ",
      CALL print(cnvtstring(a.file_size)),
      " reuse"
     ENDIF
     row + 1, col 0, "default storage ",
     row + 1, col 0, "  (initial        1048576",
     row + 1, col 0, "   next           1048576",
     row + 1, col 0, "   pctincrease    0)",
     row + 1, col 0, "   extent management dictionary;",
     row + 1, col 0, row + 1
    WITH nocounter, format = stream, noheading,
     formfeed = none, maxrow = 1
   ;end select
