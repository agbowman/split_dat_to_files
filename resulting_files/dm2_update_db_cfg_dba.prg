CREATE PROGRAM dm2_update_db_cfg:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE compare_cfg_values(cfg_name=vc,db_val=vc) = null
 IF (check_logfile("dm2_update_db_cfg",".log","DM2_UPDATE_DB_CFG LOGFILE")=0)
  GO TO exit_script
 ENDIF
 IF (currdb IN ("DB2UDB", "SQLSRV"))
  SET dm_err->eproc = "Beginning DM2_UPDATE_DB_CFG"
  CALL disp_msg(" ",dm_err->logfile,0)
 ELSE
  SET dm_err->eproc = "DM2_UPDATE_DB_CFG only for use on DB2 and SQL Server databases."
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 IF (currdb="DB2UDB")
  DECLARE parm_file = vc WITH protect, constant(trim(cnvtupper( $1),3))
  DECLARE db_file = vc WITH protect, noconstant(" ")
  DECLARE push_str = vc WITH protect, noconstant(" ")
  DECLARE max_parms = i4 WITH protect, noconstant(0)
  DECLARE db_name = vc WITH protect, constant(trim(currdblink,3))
  DECLARE dcl_str = vc WITH protect, noconstant(" ")
  DECLARE dm2_mod = vc WITH private, constant("003")
  SET dm_err->eproc = concat("Attaching to instance ",db_name)
  CALL disp_msg(" ",dm_err->logfile,0)
  IF ((dm2_install_schema->process_option="NONE"))
   SET dm2_install_schema->dbase_name = db_name
   SET dm2_install_schema->u_name = cnvtlower(currdbuser)
   EXECUTE dm2_connect_to_dbase "PO"
   IF ((dm_err->err_ind=1))
    GO TO exit_script
   ENDIF
  ENDIF
  IF (dm2_push_dcl(concat("db2 attach to ",db_name," user ",dm2_install_schema->u_name," using ",
    cnvtlower(dm2_install_schema->p_word)))=0)
   GO TO exit_script
  ENDIF
  IF (((parm_file="ADMIN") OR (parm_file="NONPROD")) )
   SET max_parms = 25
  ELSEIF (parm_file="PROD")
   SET max_parms = 27
  ELSE
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Invalid parameter specified.  Use 'PROD','NONPROD', or 'ADMIN'."
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_script
  ENDIF
  FREE RECORD cfg_parms
  RECORD cfg_parms(
    1 list[max_parms]
      2 type = vc
      2 upd_parm = vc
  ...
  SET cfg_parms->list[1].type = "DB"
  SET cfg_parms->list[1].upd_parm = "DBHEAP"
  SET cfg_parms->list[1].from_val = "1200"
  SET cfg_parms->list[2].type = "DB"
  SET cfg_parms->list[2].upd_parm = "LOGBUFSZ"
  SET cfg_parms->list[2].from_val = "8"
  SET cfg_parms->list[3].type = "DB"
  SET cfg_parms->list[3].upd_parm = "LOCKLIST"
  SET cfg_parms->list[3].from_val = "100"
  SET cfg_parms->list[4].type = "DB"
  SET cfg_parms->list[4].upd_parm = "APP_CTL_HEAP_SZ"
  SET cfg_parms->list[4].from_val = "128"
  SET cfg_parms->list[5].type = "DB"
  SET cfg_parms->list[5].upd_parm = "SORTHEAP"
  SET cfg_parms->list[5].from_val = "256"
  SET cfg_parms->list[6].type = "DB"
  SET cfg_parms->list[6].upd_parm = "APPLHEAPSZ"
  SET cfg_parms->list[6].from_val = "256"
  SET cfg_parms->list[7].type = "DB"
  SET cfg_parms->list[7].upd_parm = "STMTHEAP"
  SET cfg_parms->list[7].from_val = "2048"
  SET cfg_parms->list[8].type = "DB"
  SET cfg_parms->list[8].upd_parm = "PCKCACHESZ"
  SET cfg_parms->list[8].from_val = "(MAXAPPLS*8)"
  SET cfg_parms->list[9].type = "DB"
  SET cfg_parms->list[9].upd_parm = "MAXLOCKS"
  SET cfg_parms->list[9].from_val = "10"
  SET cfg_parms->list[10].type = "DB"
  SET cfg_parms->list[10].upd_parm = "CHNGPGS_THRESH"
  SET cfg_parms->list[10].from_val = "60"
  SET cfg_parms->list[11].type = "DB"
  SET cfg_parms->list[11].upd_parm = "NUM_IOCLEANERS"
  SET cfg_parms->list[11].from_val = "1"
  SET cfg_parms->list[12].type = "DB"
  SET cfg_parms->list[12].upd_parm = "NUM_IOSERVERS"
  SET cfg_parms->list[12].from_val = "3"
  SET cfg_parms->list[13].type = "DB"
  SET cfg_parms->list[13].upd_parm = "MAXAPPLS"
  SET cfg_parms->list[13].from_val = "AUTOMATIC"
  SET cfg_parms->list[14].type = "DB"
  SET cfg_parms->list[14].upd_parm = "LOGFILSIZ"
  SET cfg_parms->list[14].from_val = "1000"
  SET cfg_parms->list[15].type = "DB"
  SET cfg_parms->list[15].upd_parm = "LOGPRIMARY"
  SET cfg_parms->list[15].from_val = "3"
  SET cfg_parms->list[16].type = "DB"
  SET cfg_parms->list[16].upd_parm = "LOGSECOND"
  SET cfg_parms->list[16].from_val = "2"
  SET cfg_parms->list[17].type = "DB"
  SET cfg_parms->list[17].upd_parm = "NEWLOGPATH"
  SET cfg_parms->list[17].from_val = concat("/db2/",db_name,"/NODE0000/SQL00001/SQLOGDIR/")
  SET cfg_parms->list[18].type = "DB"
  SET cfg_parms->list[18].upd_parm = "STAT_HEAP_SZ"
  SET cfg_parms->list[18].from_val = "4384"
  SET cfg_parms->list[19].type = "DBM"
  SET cfg_parms->list[19].upd_parm = "JAVA_HEAP_SZ"
  SET cfg_parms->list[19].from_val = "512"
  SET cfg_parms->list[20].type = "DBM"
  SET cfg_parms->list[20].upd_parm = "MAXAGENTS"
  SET cfg_parms->list[20].from_val = "200"
  SET cfg_parms->list[21].type = "DBM"
  SET cfg_parms->list[21].upd_parm = "FEDERATED"
  SET cfg_parms->list[21].from_val = "NO"
  SET cfg_parms->list[22].type = "DBM"
  SET cfg_parms->list[22].upd_parm = "INTRA_PARALLEL"
  SET cfg_parms->list[22].from_val = "YES"
  SET cfg_parms->list[23].type = "DBM"
  SET cfg_parms->list[23].upd_parm = "DFT_MON_STMT"
  SET cfg_parms->list[23].from_val = "OFF"
  SET cfg_parms->list[24].type = "DBM"
  SET cfg_parms->list[24].upd_parm = "DFT_MON_SORT"
  SET cfg_parms->list[24].from_val = "OFF"
  SET cfg_parms->list[25].type = "DBM"
  SET cfg_parms->list[25].upd_parm = "NUM_INITAGENTS"
  SET cfg_parms->list[25].from_val = "0"
  IF (((parm_file="NONPROD") OR (parm_file="ADMIN")) )
   SET cfg_parms->list[1].to_val = "10000"
   SET cfg_parms->list[2].to_val = "1250"
   SET cfg_parms->list[3].to_val = "1000"
   SET cfg_parms->list[4].to_val = "1024"
   SET cfg_parms->list[5].to_val = "2048"
   SET cfg_parms->list[6].to_val = "2000"
   SET cfg_parms->list[7].to_val = "16384"
   SET cfg_parms->list[8].to_val = "3000"
   SET cfg_parms->list[9].to_val = "6"
   SET cfg_parms->list[10].to_val = "80"
   SET cfg_parms->list[11].to_val = "20"
   SET cfg_parms->list[12].to_val = "40"
   SET cfg_parms->list[13].to_val = "200"
   SET cfg_parms->list[14].to_val = "20000"
   SET cfg_parms->list[15].to_val = "12"
   SET cfg_parms->list[16].to_val = "10"
   SET cfg_parms->list[17].to_val = concat("/db2/log/",db_name,"/")
   SET cfg_parms->list[18].to_val = "15000"
   SET cfg_parms->list[19].to_val = "2048"
   SET cfg_parms->list[20].to_val = "800"
   SET cfg_parms->list[21].to_val = "YES"
   SET cfg_parms->list[22].to_val = "NO"
   SET cfg_parms->list[23].to_val = "ON"
   SET cfg_parms->list[24].to_val = "ON"
   SET cfg_parms->list[25].to_val = "200"
  ELSEIF (parm_file="PROD")
   SET cfg_parms->list[26].type = "DB"
   SET cfg_parms->list[26].upd_parm = "LOGRETAIN"
   SET cfg_parms->list[26].from_val = "OFF"
   SET cfg_parms->list[27].type = "DB"
   SET cfg_parms->list[27].upd_parm = "USEREXIT"
   SET cfg_parms->list[27].from_val = "OFF"
   SET cfg_parms->list[1].to_val = "10000"
   SET cfg_parms->list[2].to_val = "1250"
   SET cfg_parms->list[3].to_val = "1000"
   SET cfg_parms->list[4].to_val = "1024"
   SET cfg_parms->list[5].to_val = "2048"
   SET cfg_parms->list[6].to_val = "5000"
   SET cfg_parms->list[7].to_val = "8192"
   SET cfg_parms->list[8].to_val = "6000"
   SET cfg_parms->list[9].to_val = "6"
   SET cfg_parms->list[10].to_val = "80"
   SET cfg_parms->list[11].to_val = "20"
   SET cfg_parms->list[12].to_val = "40"
   SET cfg_parms->list[13].to_val = "400"
   SET cfg_parms->list[14].to_val = "20000"
   SET cfg_parms->list[15].to_val = "12"
   SET cfg_parms->list[16].to_val = "10"
   SET cfg_parms->list[17].to_val = concat("/db2/log/",db_name,"/")
   SET cfg_parms->list[18].to_val = "7500"
   SET cfg_parms->list[19].to_val = "2048"
   SET cfg_parms->list[20].to_val = "800"
   SET cfg_parms->list[21].to_val = "YES"
   SET cfg_parms->list[22].to_val = "NO"
   ...
   SET cfg_parms->list[24].to_val = "ON"
   SET cfg_parms->list[25].to_val = "200"
   SET cfg_parms->list[26].to_val = "RECOVERY"
   SET cfg_parms->list[27].to_val = "ON"
  ENDIF
  ...
  ENDIF
  SET dm_err->eproc = "Generating a unique file name"
  IF (get_unique_file("db_cfg",".dat")=0)
   GO TO exit_script
  ENDIF
  SET db_file = dm_err->unique_fname
  SET push_str = concat("db2 get db cfg for ",db_name," > ",trim(db_file)," 2>&1")
  SET dm_err->eproc = "Writing current values for database configuration parameters to file."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dm2_push_dcl(push_str)=0)
   GO TO exit_script
  ENDIF
  SET push_str = concat("db2 get dbm cfg >> ",trim(db_file)," 2>&1")
  SET dm_err->eproc = "Writing current values for database manager configuration parameters to file."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dm2_push_dcl(push_str)=0)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Filling out dm2parse record structure."
  FOR (parse_cnt = 1 TO max_parms)
    IF ((cfg_parms->list[parse_cnt].upd_parm != "NEWLOGPATH"))
     SET push_str = concat("set dm2parse->attr",trim(cnvtstring(parse_cnt))," = '(",cfg_parms->list[
      parse_cnt].upd_parm,")' go")
    ELSE
     SET push_str = concat("set dm2parse->attr",trim(cnvtstring(parse_cnt))," = ",
      "'Path to log files' go")
    ENDIF
    IF (dm2_push_cmd(push_str,1)=0)
     GO TO exit_script
    ENDIF
    SET push_str = concat("set dm2parse->attr",trim(cnvtstring(parse_cnt)),"sep = '=' go")
    IF (dm2_push_cmd(push_str,1)=0)
     GO TO exit_script
    ENDIF
  ENDFOR
  SET dm_err->eproc = concat("Parsing ",db_file," for parameter values")
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dm2parse_db2_output(max_parms,db_file,"V")=0)
   GO TO exit_script
  ENDIF
  FOR (p_cnt = 1 TO max_parms)
   SET push_str = concat("dm2parse->qual[1].attr",trim(cnvtstring(p_cnt)),"val go")
   IF ((cfg_parms->list[p_cnt].from_val=parser(push_str)))
    SET dm_err->eproc = concat("Updating ",cfg_parms->list[p_cnt].upd_parm,
     " configuration parameter.")
    IF ((cfg_parms->list[p_cnt].type="DB"))
     SET dcl_str = concat("db2 update db cfg for ",db_name," using ",cfg_parms->list[p_cnt].upd_parm,
      " ",
      cfg_parms->list[p_cnt].to_val)
    ELSEIF ((cfg_parms->list[p_cnt].type="DBM"))
     SET dcl_str = concat("db2 update dbm cfg using ",cfg_parms->list[p_cnt].upd_parm," ",cfg_parms->
      list[p_cnt].to_val)
    ENDIF
    IF (dm2_push_dcl(dcl_str)=0)
     GO TO exit_script
    ENDIF
   ENDIF
  ENDFOR
 ELSEIF (currdb="SQLSRV")
  ...
  DECLARE dudc_cfg_change = i2 WITH protect, noconstant(0)
  DECLARE cfgx = i4 WITH protect, noconstant(0)
  DECLARE cfgx_pos = i4 WITH protect, noconstant(0)
  FREE RECORD dbcfg
  RECORD dbcfg(
    1 parm_cnt = i4
    1 list[*]
  ...
  CALL load_cfg_rs(1,"ANSI_NULL_DEFAULT","OFF","ON")
  CALL load_cfg_rs(2,"ANSI_NULLS","OFF","ON")
  CALL load_cfg_rs(3,"ANSI_WARNINGS","OFF","ON")
  CALL load_cfg_rs(4,"AUTO_CLOSE","OFF","OFF")
  CALL load_cfg_rs(5,"AUTO_CREATE_STATISTICS","ON","ON")
  CALL load_cfg_rs(6,"AUTO_UPDATE_STATISTICS","ON","ON")
  CALL load_cfg_rs(7,"AUTO_SHRINK","OFF","OFF")
  CALL load_cfg_rs(8,"CURSOR_CLOSE_ON_COMMIT","OFF","OFF")
  CALL load_cfg_rs(9,"CURSOR_DEFAULT","GLOBAL","GLOBAL")
  CALL load_cfg_rs(10,"CONCAT_NULL_YIELDS_NULL","OFF","ON")
  CALL load_cfg_rs(11,"QUOTED_IDENTIFIER","OFF","ON")
  CALL load_cfg_rs(12,"RECURSIVE_TRIGGERS","OFF","OFF")
  CALL load_cfg_rs(13,"TORN_PAGE_DETECTION","ON","OFF")
  IF (currdbname="TDS")
   CALL load_cfg_rs(14,"RECOVERY","FULL","FULL")
  ELSE
   CALL load_cfg_rs(14,"RECOVERY","FULL","SIMPLE")
  ENDIF
  CALL load_cfg_rs(15,"READ_WRITE","READ_WRITE","READ_WRITE")
  CALL load_cfg_rs(16,"MULTI_USER","MULTI_USER","MULTI_USER")
  SET dm_err->eproc = "Generating a unique file name"
  IF (get_unique_file("db_cfg",".dat")=0)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = concat("Retrieving current db config parameters and writing them to ",dm_err->
   unique_fname," file.")
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO dm_err->unique_fname
   db.name
   FROM sysdatabases db
   WHERE db.name=currdbname
   DETAIL
    row + 1, " Database Configuration Parameters ", row + 1,
    " --------------------------------- ", row + 1, row + 1,
    "Database Name:                  ", db.name, row + 1,
    "Database Id:                    ", db.dbid, row + 1,
    "Mode:                           ", db.mode, row + 1,
    "Status:                         ", db.status, row + 1,
    "Status 2:                       ", db.status2, row + 1,
    "Creation Date:                  ", db.crdate, row + 1,
    "Category:                       ", db.category, row + 1,
    "Compatibility Level:            ", db.cmptlevel, row + 1,
    "Primary File Name:              ", db.filename, row + 1,
    "Version:                        ", db.version, row + 1,
    "Collation Name:                 ", db.collation_name, row + 1,
    "ANSI Nulls Default:             ", db.ansinulldefault, row + 1,
    "ANSI Nulls Enabled:             ", db.ansinulls, row + 1,
    "ANSI Padding Enabled:           ", db.ansipadding, row + 1,
    "ANSI Warnings Enabled:          ", db.ansiwarnings, row + 1,
    "Arithmetic Abort Enabled:       ", db.ansiarithmeticabort, row + 1,
    "Auto Close Enabled:             ", db.autoclose, row + 1,
    "Auto Create Stats:              ", db.autocreatestats, row + 1,
    "Auto Update Stats:              ", db.autoupdatestats, row + 1,
    "Auto Shrink Enabled:            ", db.autoshrink, row + 1,
    "Close Cursor on Commit Enabled: ", db.closecursorsoncommit, row + 1,
    "Full Text Enabled:              ", db.fulltext, row + 1,
    "In Standby:                     ", db.instandby, row + 1,
    "Local Cursors Default:          ", db.localcursors, row + 1,
    "Merge Published Enabled:        ", db.mergepublished, row + 1,
    "Null Concatenation Yields Null: ", db.nullconcat, row + 1,
    "Numeric Round Abort Enabled:    ", db.numericroundabort, row + 1,
    "Quoted Identifiers Enabled:     ", db.quotedidentifier, row + 1,
    "Recursive Triggers Enabled:     ", db.recursivetriggers, row + 1,
    "Subscribed:                     ", db.subscribed, row + 1,
    "Torn Page Detection Enabled:    ", db.tornpagedetect, row + 1,
    "Recovery Model:                 ", db.recoverymodel, row + 1,
    "SQL Server Sort Order:          ", db.sqlsortorder, row + 1,
    "Database Status:                ", db.dbstatus, row + 1,
    "Updateability:                  ", db.updateability, row + 1,
    "User Access:                    ", db.useraccess, dudc_cfg_change = false,
    CALL compare_cfg_values("ANSI_NULL_DEFAULT",db.ansinulldefault),
    CALL compare_cfg_values("ANSI_NULLS",db.ansinulls),
    CALL compare_cfg_values("ANSI_WARNINGS",db.ansiwarnings),
    CALL compare_cfg_values("AUTO_CLOSE",db.autoclose),
    CALL compare_cfg_values("AUTO_CREATE_STATISTICS",db.autocreatestats),
    CALL compare_cfg_values("AUTO_UPDATE_STATISTICS",db.autoupdatestats),
    CALL compare_cfg_values("AUTO_SHRINK",db.autoshrink),
    CALL compare_cfg_values("CURSOR_CLOSE_ON_COMMIT",db.closecursorsoncommit),
    CALL compare_cfg_values("CURSOR_DEFAULT",db.localcursors),
    CALL compare_cfg_values("CONCAT_NULL_YIELDS_NULL",db.nullconcat),
    CALL compare_cfg_values("QUOTED_IDENTIFIER",db.quotedidentifier),
    CALL compare_cfg_values("RECURSIVE_TRIGGERS",db.recursivetriggers),
    CALL compare_cfg_values("TORN_PAGE_DETECTION",db.tornpagedetect),
    CALL compare_cfg_values("RECOVERY",db.recoverymodel),
    CALL compare_cfg_values("READ_WRITE",db.updateability),
    CALL compare_cfg_values("MULTI_USER",db.useraccess)
   WITH nocounter, maxcol = 400
  ;end select
  IF (check_error("Writing current values for database manager configuration parameters to file.")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  IF ((dm_err->debug_flag=1))
   CALL echorecord(dbcfg)
  ENDIF
  IF (dudc_cfg_change=true)
   SET dm_err->eproc = "Altering Database Configuration Parameters"
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dm2_set_autocommit(1)=0)
    RETURN(0)
   ENDIF
   CALL dm2_push_cmd(concat("RDB ALTER DATABASE ",trim(currdbname)," SET "),0)
   SET cntx = 1
   FOR (cntx = 1 TO dbcfg->parm_cnt)
     IF ((dbcfg->list[cntx].modify_ind=true))
      IF (cntx=1)
       CALL dm2_push_cmd(concat(" ",dbcfg->list[cntx].name," ",dbcfg->list[cntx].to_val),0)
      ELSE
       CALL dm2_push_cmd(concat(",",dbcfg->list[cntx].name," ",dbcfg->list[cntx].to_val),0)
      ENDIF
     ENDIF
   ENDFOR
   CALL dm2_push_cmd("WITH NO_WAIT ",0)
   CALL dm2_push_cmd("GO ",1)
   IF (check_error("Altering Database Configuration Parameters")=1)
    ROLLBACK
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ELSE
   ...
   ENDIF
  ELSE
   SET dm_err->eproc = "No Changes Necessary to Database Configuration Parameters."
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
 ENDIF
 SUBROUTINE load_cfg_rs(posx,cfg_name,from_val,to_val)
   SET stat = alterlist(dbcfg->list,posx)
   SET dbcfg->list[posx].name = cfg_name
   SET dbcfg->list[posx].from_val = from_val
   SET dbcfg->list[posx].to_val = to_val
   SET dbcfg->list[posx].modify_ind = false
   IF ((posx > dbcfg->parm_cnt))
    SET dbcfg->parm_cnt = posx
   ENDIF
 END ;Subroutine
 SUBROUTINE compare_cfg_values(cfg_name,db_val)
  SET cfgx_pos = locateval(cfgx,1,dbcfg->parm_cnt,cfg_name,dbcfg->list[cfgx].name)
  IF ((dbcfg->list[cfgx_pos].from_val=db_val))
   IF ((dbcfg->list[cfgx_pos].to_val != db_val))
    SET dbcfg->list[cfgx_pos].modify_ind = true
    SET dudc_cfg_change = true
   ENDIF
  ENDIF
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "ENDING DM2_UPDATE_DB_CFG"
 CALL final_disp_msg("dm2_update_db_cfg")
 FREE RECORD cfg_parms
 FREE RECORD dm2parse
END GO
