CREATE PROGRAM bhs_multum_ddi_audit_monthly
 PROMPT
   ...
   SELECT INTO "nl:"
    ...
    PLAN (ede
     ...
     WHERE e.encntr_id=ede.encntr_id
      AND ((( $PAT_TYPE="1")
      AND e.encntr_type_cd IN (obs_enc_type_cd, inp_enc_type_cd, day_enc_type_cd,
 ...
 SELECT INTO value(output_dest)
  ...
  FROM (dummyt d  WITH seq = value(size(eks2->list,5))),
   ...
   dummyt d2
  PLAN (d
   WHERE (eks2->list[d.seq].interact.orig_ord_as_flag IN (1, 0))
  ...
  DETAIL
   ...
   IF (report_type="2")
    ...
    interact_ord_phys_pos = substring(1,40,uar_get_code_display(eks2->list[d.seq].interact.
      physician_position_cd)), trigger_ord_phys_pos = substring(1,40,uar_get_code_display(eks2->list[
      d.seq].trigger.physician_position_cd)),
