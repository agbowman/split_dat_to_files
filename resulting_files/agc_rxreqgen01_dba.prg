CREATE PROGRAM agc_rxreqgen01:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SET request->person_id = 763267
 SET request->print_prsnl_id = 0
 SET request->order_qual[1].order_id = 868296
 SET request->order_qual[1].encntr_id = 1135244
 SET request->order_qual[1].conversation_id = 0
 SET request->printer_name = "NPI7A166A"
 ...
 FOR (i = 1 TO tprint_req->job_knt)
   SELECT INTO value(tprint_req->job[i].print_loc)
    org = substring(1,30,o.org_name), location = uar_get_code_display(tprint_req->job[i].location_cd),
    street = concat(trim(tprint_req->job[i].street_addr1),trim(tprint_req->job[i].street_addr2)),
    csz = concat(trim(tprint_req->job[i].city),trim(tprint_req->job[i].state),trim(tprint_req->job[i]
      .zip)), phone = tprint_req->job[i].phone
    ...
    HEAD REPORT
     ...
     col 0,
     CALL center(org,0,66), row + 1,
     col 0,
     CALL center(location,0,66), row + 1,
     col 0,
     CALL center(street,0,66), row + 1,
     col 0,
     CALL center(csz,0,66), row + 1,
     col 0,
     CALL center(phone,0,66), row + 1
    WITH dio = 8
   ;end select
 ENDFOR
#exit_script
