CREATE PROGRAM bhs_athn_write_dose_calc_v2
 FREE RECORD result
 RECORD result(
   1 order_id = f8
   1 synonym_id = f8
...
#exit_script
 ...
 SUBROUTINE writetofile(null)
   ...
   IF ((result->synonym_id > 0))
    SET tmp_filename = concat("ATHN_SCDOSECALCLONGTEXT_",trim(cnvtstring(result->order_id),3),"_",
     trim(cnvtstring(result->synonym_id),3),".TMP")
   ELSE
    SET tmp_filename = concat("ATHN_SCDOSECALCLONGTEXT_",trim(cnvtstring(result->order_id),3),".TMP")
   ENDIF
