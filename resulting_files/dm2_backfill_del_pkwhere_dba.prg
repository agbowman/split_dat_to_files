CREATE PROGRAM dm2_backfill_del_pkwhere:dba
 ...
 ENDIF
 DECLARE cnvt_del_pkwhere(sbr_table_name=vc,sbr_pkwhere=vc) = vc
 DECLARE dbdp_pk_cnt = i4
 DECLARE dbdp_prog_loop = i4
 DECLARE dbdp_newpk_check = vc
 FREE RECORD pk_holder
 RECORD pk_holder(
   1 qual[*]
     2 log_id = f8
     2 from_pk = vc
 ...
 IF (check_logfile("dm2_bckfll_d_",".log","dm2_bckfll_d LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_backfill_del"
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "NL:"
  FROM dm_chg_log d
  WHERE d.delete_ind=1
  DETAIL
   IF (findstring("DM_TRANSLATE",cnvtupper(d.pk_where),1,0)=0)
    dbdp_pk_cnt = (dbdp_pk_cnt+ 1), stat = alterlist(pk_holder->qual,dbdp_pk_cnt), pk_holder->qual[
    dbdp_pk_cnt].log_id = d.log_id,
    pk_holder->qual[dbdp_pk_cnt].from_pk = d.pk_where, pk_holder->qual[dbdp_pk_cnt].table_name = d
    .table_name
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading delete pk_where strings from DM_CHG_LOG into record structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Generating new pk_where statements"
 CALL disp_msg("",dm_err->logfile,0)
 SET readme_data->status = "F"
 SET readme_data->message = dm_err->emsg
 EXECUTE dm_readme_status
 FOR (dbdp_prog_loop = 1 TO dbdp_pk_cnt)
  SET dbdp_newpk_check = cnvt_del_pkwhere(pk_holder->qual[dbdp_prog_loop].table_name,pk_holder->qual[
   dbdp_prog_loop].from_pk)
  IF (findstring("ERROR",dbdp_newpk_check,1,0)=0)
   SET pk_holder->qual[dbdp_prog_loop].to_pk = dbdp_newpk_check
   SET pk_holder->qual[dbdp_prog_loop].upd_ind = 1
  ELSE
   SET pk_holder->qual[dbdp_prog_loop].to_pk = dbdp_newpk_check
  ENDIF
 ENDFOR
 SET dm_err->eproc = "Back-filling pk_where statements..."
 CALL disp_msg("",dm_err->logfile,0)
 SET readme_data->status = "F"
 SET readme_data->message = dm_err->emsg
 EXECUTE dm_readme_status
 FOR (dbdp_prog_loop = 1 TO dbdp_pk_cnt)
   IF ((pk_holder->qual[dbdp_prog_loop].upd_ind=1))
    UPDATE  FROM dm_chg_log d
     SET d.pk_where = pk_holder->qual[dbdp_prog_loop].to_pk
     WHERE (d.log_id=pk_holder->qual[dbdp_prog_loop].log_id)
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc) != 0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Error back-filling pk_where column in DM_TABLES_DOC"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ROLLBACK
     SET readme_data->status = "F"
     SET readme_data->message = dm_err->emsg
     SET dbdp_prog_loop = dbdp_pk_cnt
    ELSE
     COMMIT
    ENDIF
   ENDIF
 ...
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error back-filling pk_where in DM_CHG_LOG"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = dm_err->emsg
  GO TO exit_script
 ELSE
  SET dm_err->eproc = "Finished back-fill of DM_CHG_LOG"
  SET readme_data->status = "S"
 ENDIF
 SUBROUTINE cnvt_del_pkwhere(sbr_table_name,sbr_pkwhere)
   DECLARE sbr_equal_cnt = i4
   DECLARE sbr_under_cnt = i4
   DECLARE ret_pkwhere = vc
   DECLARE sbr_id_check = vc
   DECLARE sbr_sub = vc
   DECLARE sbr_old_str = vc
   DECLARE sbr_eq_ind = i2
   DECLARE sbr_work_str = vc
   DECLARE sbr_num_end_cnt = i4
   DECLARE sbr_number = vc
   DECLARE sbr_col_name = vc
   DECLARE sbr_col_cnt = i4
   DECLARE sbr_next_eq = i4
   DECLARE sbr_prev_spc = i4
   DECLARE sbr_pass_str = vc
   DECLARE sbr_dummy_str = vc
   DECLARE sbr_id_cd_ind = i2
   DECLARE sbr_space_ind = i2
   SET sbr_old_str = sbr_pkwhere
   SET sbr_equal_cnt = 1
   IF (findstring("_ID",sbr_old_str,1,0)=0
    AND findstring("_CD",sbr_old_str,1,0)=0)
    SET sbr_id_cd_ind = 0
   ELSE
    SET sbr_id_cd_ind = 1
    SET sbr_work_str = "CONCAT('"
   ENDIF
   IF (findstring("CODE_VALUE",sbr_old_str,1,0) != 0
    AND findstring("_CODE_VALUE",sbr_old_str,1,0)=0
    AND findstring("CODE_VALUE_",sbr_old_str,1,0)=0)
    SET sbr_id_cd_ind = 1
    SET sbr_work_str = "CONCAT('"
   ENDIF
   WHILE (sbr_equal_cnt > 0)
    SET sbr_equal_cnt = findstring("=",sbr_old_str,1,0)
    IF (sbr_equal_cnt > 0)
     SET sbr_eq_ind = 1
     IF (substring((sbr_equal_cnt+ 1),1,sbr_old_str)=" ")
      SET sbr_space_ind = 1
     ELSE
      SET sbr_space_ind = 0
     ENDIF
     SET sbr_sub = substring(1,(sbr_equal_cnt - 2),sbr_old_str)
     SET sbr_col_cnt = findstring(" ",sbr_sub,1,1)
     SET sbr_col_name = trim(substring((sbr_col_cnt+ 1),(sbr_equal_cnt - 2),sbr_sub))
     SET sbr_under_cnt = findstring("_",sbr_sub,1,1)
     IF (sbr_under_cnt=0)
      SET sbr_next_eq = findstring("=",sbr_old_str,(sbr_equal_cnt+ 1),0)
      IF (sbr_next_eq > 0)
       IF (substring((sbr_next_eq+ 1),1,sbr_old_str)=" ")
        SET sbr_space_ind = 1
       ELSE
        SET sbr_space_ind = 0
       ENDIF
       SET sbr_dummy_str = substring(1,(sbr_next_eq - 2),sbr_old_str)
       SET sbr_prev_spc = findstring(" ",sbr_dummy_str,1,1)
       SET sbr_pass_str = substring(1,sbr_prev_spc,sbr_old_str)
      ELSE
       SET sbr_pass_str = sbr_old_str
       SET sbr_prev_spc = size(sbr_old_str)
      ENDIF
      SET sbr_work_str = concat(sbr_work_str,sbr_pass_str)
      SET sbr_old_str = substring(sbr_prev_spc,size(sbr_old_str),sbr_old_str)
     ELSE
      SET sbr_id_check = substring(sbr_under_cnt,size(sbr_sub),sbr_sub)
      IF (findstring("_ID",sbr_id_check,1,0)=0
       AND findstring("_CD",sbr_id_check,1,0)=0
       AND sbr_col_name != "CODE_VALUE")
       SET sbr_next_eq = findstring("=",sbr_old_str,(sbr_equal_cnt+ 1),0)
       IF (sbr_next_eq > 0)
        SET sbr_dummy_str = substring(1,(sbr_next_eq - 2),sbr_old_str)
        SET sbr_prev_spc = findstring(" ",sbr_dummy_str,1,1)
       ...
       SET sbr_old_str = substring(sbr_prev_spc,size(sbr_old_str),sbr_old_str)
      ELSE
       SET sbr_num_end_cnt = findstring(" ",sbr_old_str,((sbr_equal_cnt+ sbr_space_ind)+ 1),0)
       IF (sbr_num_end_cnt=0)
        SET sbr_number = trim(substring((sbr_equal_cnt+ 2),size(sbr_old_str),sbr_old_str))
        SET sbr_num_end_cnt = size(sbr_old_str)
       ELSE
        SET sbr_number = trim(substring(((sbr_equal_cnt+ sbr_space_ind)+ 1),(((sbr_num_end_cnt -
          sbr_equal_cnt) - sbr_space_ind) - 1),sbr_old_str))
       ENDIF
       SET sbr_work_str = concat(sbr_work_str,trim(substring(1,sbr_equal_cnt,sbr_old_str)),
        "',dm_translate('",sbr_table_name,"','",
        sbr_col_name,"','",sbr_number,"'), '")
       SET sbr_old_str = substring(sbr_num_end_cnt,size(sbr_old_str),sbr_old_str)
      ENDIF
     ENDIF
    ENDIF
   ENDWHILE
   IF (sbr_eq_ind=0)
    SET ret_pkwhere = "ERROR: Could not find an equal sign in string"
   ELSE
    SET ret_pkwhere = sbr_work_str
   ENDIF
   IF (sbr_id_cd_ind=1)
    SET ret_pkwhere = concat(ret_pkwhere,"')")
   ENDIF
   RETURN(ret_pkwhere)
 END ;Subroutine
#exit_script
