CREATE PROGRAM bhs_prax_pat_immunizations
 ...
 SELECT INTO "NL:"
  ...
  PLAN (vesc
   WHERE vesc.event_set_name_key="IMMUNIZATIONS")
   JOIN (vese
   WHERE vese.event_set_cd=vesc.event_set_cd)
  DETAIL
   vcnt = (vcnt+ 1), stat = alterlist(event_codes->qual,vcnt), event_codes->qual[vcnt].event_cd =
   vese.event_cd
  WITH time = 10
 ;end select
 SELECT INTO  $1
  ...
  HEAD c.event_id
   ...
   "</EventPrsnlAction>", row + 1, cm_admn_dos = build("<AdministrationDose>",cm.admin_dosage,
    "</AdministrationDose>"),
