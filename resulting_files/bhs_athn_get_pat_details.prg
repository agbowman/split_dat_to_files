CREATE PROGRAM bhs_athn_get_pat_details
 ...
 RECORD patients(
   ...
   1 enc_type = vc
   1 enc_facility = vc
   1 enc_facility_cd = f8
   1 enc_location = vc
   1 enc_location_cd = f8
   1 pcp = vc
 ...
 DECLARE enc_facility_cd = f8 WITH protect, noconstant(0)
 DECLARE enc_location = vc WITH protect, noconstant("")
 DECLARE enc_location_cd = f8 WITH protect, noconstant(0)
 DECLARE pcp = vc WITH protect, noconstant("")
 DECLARE chief_complaint = vc WITH protect, noconstant("")
 DECLARE ethnicity_cd = f8 WITH protect, noconstant(0)
 DECLARE ethnicity_display = vc WITH protect, noconstant("")
 DECLARE pref_language = vc WITH protect, noconstant("")
 DECLARE body_weight = vc WITH protect, noconstant("")
 DECLARE body_weight_unit = vc WITH protect, noconstant("")
 ...
 DECLARE v111 = vc WITH protect, noconstant("")
 DECLARE v112 = vc WITH protect, noconstant("")
 DECLARE v113 = vc WITH protect, noconstant("")
 ...
 DECLARE vh1 = vc WITH protect, noconstant("")
 DECLARE vw1 = vc WITH protect, noconstant("")
 DECLARE vc1 = vc WITH protect, noconstant("")
 DECLARE v71 = vc WITH protect, noconstant("")
 DECLARE v72 = vc WITH protect, noconstant("")
 DECLARE v73 = vc WITH protect, noconstant("")
 SELECT INTO "NL:"
  ...
  HEAD REPORT
   patidx = 0
  HEAD p.person_id
   patidx += 1, patients->person_id = cnvtint(p.person_id), patients->encntr_id = cnvtint(e.encntr_id
    ),
   ...
   patients->enc_facility = trim(replace(replace(replace(replace(replace(trim(uar_get_code_display(e
     ...
     "&quot;",0),3), patients->enc_facility_cd = e.loc_facility_cd, patients->enc_location = trim(
    replace(replace(replace(replace(replace(trim(uar_get_code_display(e.loc_nurse_unit_cd),3),"&",
         "&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),
   patients->enc_location_cd = e.loc_nurse_unit_cd, patients->pre_reg_dt_tm = format(e.pre_reg_dt_tm,
    "MM/DD/YYYY HH:MM:SS;;D"), patients->reg_dt_tm = format(e.reg_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"),
   ...
   patients->sex = trim(uar_get_code_display(p.sex_cd),3), patients->ethnicity_cd = p.ethnic_grp_cd,
   patients->ethnicity_display = trim(uar_get_code_display(p.ethnic_grp_cd),3)
 ...
 SELECT INTO  $1
  ...
  HEAD REPORT
   ...
   IF (patidx > 0)
    ...
    enc_facility = patients->enc_facility, enc_facility_cd = patients->enc_facility_cd, enc_location
     = patients->enc_location,
    enc_location_cd = patients->enc_location_cd, pcp = patients->pcp, chief_complaint = patients->
    chief_complaint,
    ethnicity_cd = patients->ethnicity_cd, ethnicity_display = patients->ethnicity_display,
    pref_language = patients->pref_language,
    body_weight = patients->body_weight, body_weight_unit = patients->body_weight_unit, pre_reg_dt_tm
     = patients->pre_reg_dt_tm,
    reg_dt_tm = patients->reg_dt_tm, disch_dt_tm = patients->disch_dt_tm, inpat_admit_dt_tm =
    patients->inpat_admit_dt_tm,
    arrive_dt_tm = patients->arrive_dt_tm, est_arrive_dt_tm = patients->est_arrive_dt_tm,
    name_full_formatted = patients->name_full_formatted,
    sex_cd = patients->sex_cd, sex = patients->sex, mrn = patients->mrn,
    dob = patients->dob, age = patients->age, fname = patients->fname,
    mname = patients->mname, lname = patients->lname, street_addr1 = patients->street_addr1,
    street_addr2 = patients->street_addr2, street_addr3 = patients->street_addr3, street_addr4 =
    patients->street_addr4,
    zipcode = patients->zipcode, city = patients->city, state = patients->state,
    country = patients->country, county = patients->county, ph_home = patients->ph_home,
    ph_work = patients->ph_work, ph_cell = patients->ph_cell
   ELSE
    person_id = 0, encntr_id = 0, enc_created_dt_tm = "",
    fin = "", enc_status = "", enc_type = "",
    enc_facility = "", enc_facility_cd = 0, enc_location = "",
    enc_location_cd = 0, pcp = "", chief_complaint = "",
    ethnicity_cd = 0, ethnicity_display = "", pref_language = "",
    body_weight = "", pre_reg_dt_tm = "", reg_dt_tm = "",
   ...
   v42, row + 1, v4 = build("<LastVisitDateTime>",trim(enc_created_dt_tm,3),"</LastVisitDateTime>"),
   col + 1, v4, row + 1,
   v5 = build("<ChiefComplaint>",trim(chief_complaint,3),"</ChiefComplaint>"), col + 1, v5,
   row + 1, v6 = build("<PreferredLanguage>",trim(pref_language,3),"</PreferredLanguage>"), col + 1,
   v6, row + 1, v7 = build("<DosingBodyWeight>",trim(body_weight,3),"</DosingBodyWeight>"),
   col + 1, v7, row + 1,
   v71 = build("<DosingBodyWeightUnitCd>",trim(body_weight_unit,3),"</DosingBodyWeightUnitCd>"), col
    + 1, v71,
   row + 1, v72 = build("<EthnicityCd>",cnvtstring(ethnicity_cd),"</EthnicityCd>"), col + 1,
   v72, row + 1, v73 = build("<EthnicityDisplay>",trim(replace(replace(replace(replace(replace(
          ethnicity_display,"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0
      ),3),"</EthnicityDisplay>"),
   col + 1, v73, row + 1,
   v8 = build("<FIN>",trim(fin,3),"</FIN>"), col + 1, v8,
   row + 1, v9 = build("<EncounterStatus>",trim(patients->enc_status,3),"</EncounterStatus>"), col +
   1,
   v9, row + 1, v10 = build("<EncounterType>",trim(enc_type,3),"</EncounterType>"),
   col + 1, v10, row + 1,
   v11 = build("<EncounterLocFacility>",trim(enc_facility,3),"</EncounterLocFacility>"), col + 1, v11,
   row + 1, v111 = build("<EncounterLocation>",trim(enc_location,3),"</EncounterLocation>"), col + 1,
   v111, row + 1, v112 = build("<locationId>",cnvtstring(enc_location_cd),"</locationId>"),
   col + 1, v112, row + 1,
   v113 = build("<EncounterLocFacilityId>",cnvtstring(enc_facility_cd),"</EncounterLocFacilityId>"),
   col + 1, v113,
   row + 1, v12 = build("<RegistrationDateTime>",trim(reg_dt_tm,3),"</RegistrationDateTime>"), col +
   1,
   v12, row + 1, v13 = build("<PreRegistrationDateTime>",trim(pre_reg_dt_tm,3),
    "</PreRegistrationDateTime>"),
   col + 1, v13, row + 1,
   v14 = build("<InpatientAdmitDateTime>",trim(inpat_admit_dt_tm,3),"</InpatientAdmitDateTime>"), col
    + 1, v14,
   row + 1, v15 = build("<DischargeDateTime>",trim(disch_dt_tm,3),"</DischargeDateTime>"), col + 1,
   v15, row + 1, v16 = build("<ArriveDateTime>",trim(arrive_dt_tm,3),"</ArriveDateTime>"),
   col + 1, v16, row + 1,
   v17 = build("<EstimatedArriveDateTime>",trim(est_arrive_dt_tm,3),"</EstimatedArriveDateTime>"),
   col + 1, v17,
   row + 1, col + 1, "<Patient>",
   row + 1, v18 = build("<FullName>",trim(name_full_formatted,3),"</FullName>"), col + 1,
   v18, row + 1, v19 = build("<SexCd>",cnvtint(sex_cd),"</SexCd>"),
   col + 1, v19, row + 1,
   v20 = build("<Sex>",trim(sex,3),"</Sex>"), col + 1, v20,
   row + 1, v21 = build("<MRN>",trim(mrn,3),"</MRN>"), col + 1,
   v21, row + 1, v22 = build("<DateOfBirth>",trim(dob,3),"</DateOfBirth>"),
   col + 1, v22, row + 1,
   v23 = build("<Age>",trim(age,3),"</Age>"), col + 1, v23,
   row + 1, v24 = build("<Fname>",trim(fname,3),"</Fname>"), col + 1,
   v24, row + 1, v25 = build("<Mname>",trim(mname,3),"</Mname>"),
   col + 1, v25, row + 1,
   v26 = build("<Lname>",trim(lname,3),"</Lname>"), col + 1, v26,
   row + 1, v27 = build("<StreetAddr1>",trim(street_addr1,3),"</StreetAddr1>"), col + 1,
   v27, row + 1, v28 = build("<StreetAddr2>",trim(street_addr2,3),"</StreetAddr2>"),
   col + 1, v28, row + 1,
   v29 = build("<StreetAddr3>",trim(street_addr3,3),"</StreetAddr3>"), col + 1, v29,
   row + 1, v30 = build("<StreetAddr4>",trim(street_addr4,3),"</StreetAddr4>"), col + 1,
   v30, row + 1, v31 = build("<ZipCode>",trim(zipcode,3),"</ZipCode>"),
   col + 1, v31, row + 1,
   v32 = build("<City>",trim(city,3),"</City>"), col + 1, v32,
   row + 1, v33 = build("<State>",trim(state,3),"</State>"), col + 1,
   v33, row + 1, v34 = build("<Country>",trim(country,3),"</Country>"),
   col + 1, v34, row + 1,
   v35 = build("<County>",trim(county,3),"</County>"), col + 1, v35,
   row + 1, col + 1, "</Patient>",
   row + 1, vh1 = build("<HomePhoneNumber>",trim(ph_home,3),"</HomePhoneNumber>"), col + 1,
   vh1, row + 1, vw1 = build("<WorkPhoneNumber>",trim(ph_work,3),"</WorkPhoneNumber>"),
   col + 1, vw1, row + 1,
   vc1 = build("<CellPhoneNumber>",trim(ph_cell,3),"</CellPhoneNumber>"), col + 1, vc1,
   row + 1
  FOOT REPORT
