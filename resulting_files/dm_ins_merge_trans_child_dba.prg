CREATE PROGRAM dm_ins_merge_trans_child:dba
 ...
 SUBROUTINE (dir_get_storage_type(dgst_db_link=vc) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dimtc_tbl_name = vc
 DECLARE dimtc_col_name = vc
 DECLARE dimtc_dis_ind = i2 WITH protect, noconstant(0)
 DECLARE dimtc_src_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dimtc_tgt_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE con_flag = i2 WITH protect, noconstant(0)
 DECLARE dimtc_max_v = f8 WITH protect, noconstant(0.0)
 DECLARE dimtc_min_v = f8 WITH protect, noconstant(0.0)
 DECLARE dimtc_con_v = i4 WITH protect, constant(3000)
 DECLARE dimtc_var_v = f8 WITH protect, noconstant(0.0)
 SET dimtc_tbl_name = cnvtupper( $1)
 SET dimtc_col_name = cnvtupper( $2)
 SET dimtc_src_env_id =  $3
 SET dimtc_tgt_env_id =  $4
 SET dimtc_dis_ind =  $5
 SET dm_err->eproc = "Retrive the max value"
 SELECT INTO "nl:"
  mv = max(parser(dimtc_col_name))"#############.##"
  FROM (parser(dimtc_tbl_name))
  DETAIL
   rptbuf = reportinfo(2), dimtc_max_v = cnvtreal(substring(1,15,rptbuf))
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF (dimtc_max_v=0)
  SET dm_err->eproc = concat(dimtc_tbl_name," is empty. Go to next table")
  CALL echo(dm_err->eproc)
  GO TO exit_program
 ENDIF
 WHILE (con_flag=0)
   SET dimtc_sta_v = dimtc_var_v
   SET dimtc_var_v += dimtc_con_v
   IF (dimtc_var_v > dimtc_max_v)
    SET dimtc_var_v = dimtc_max_v
    SET con_flag = 1
   ENDIF
   SET dm_err->eproc = concat("Working on table ",dimtc_tbl_name)
   EXECUTE dm_ins_mrg_trans_kid
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
    SET con_flag = 1
    GO TO exit_program
