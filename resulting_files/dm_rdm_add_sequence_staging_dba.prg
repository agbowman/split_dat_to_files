CREATE PROGRAM dm_rdm_add_sequence_staging:dba
 ...
 SET readme_data->message = "Readme failed: starting script dm_rdm_add_sequence_staging..."
 DECLARE errmsg = vc WITH protect, noconstant("")
 DECLARE seqexists = i2 WITH protect, noconstant(0)
 DECLARE sequencename = vc WITH protect, noconstant("ORDER_STAGING_SEQ")
 SELECT INTO "nl:"
 ...
 IF (error(errmsg,0))
  ...
  GO TO exit_script
 ENDIF
 CALL parser(concat("rdb create or replace public synonym ",sequencename))
 CALL parser(concat("for ",sequencename))
 CALL parser("go")
 IF (error(errmsg,0))
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to create public synonym ",sequencename,": ",errmsg)
  GO TO exit_script
 ENDIF
 UPDATE  FROM dm_sequences ds
  SET ds.cache = 2000, ds.updt_applctx = reqinfo->updt_applctx, ds.updt_dt_tm = cnvtdatetime(sysdate),
