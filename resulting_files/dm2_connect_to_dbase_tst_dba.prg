CREATE PROGRAM dm2_connect_to_dbase_tst:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 SET dm_err->logfile = "dm2_test_dbcnct.log"
 CALL init_logfile(value(dm_err->logfile),"DM2_TEST_CONNECT_TO_DBASE")
 SET dm_err->debug_flag = 1
 CALL echo("TESTING DM2_CONNECT_TO_DBASE")
 CALL echo("  ")
 CALL echo(concat("Process option = ", $1))
 CALL echo(concat("Database name  = ", $2))
 CALL echo(concat("User Name      = ", $3))
 CALL echo(concat("Password       = ", $4))
 CALL echo(concat("Connect string = ", $5))
 CALL echo("  ")
 SET dm2_install_schema->dbase_name = cnvtupper( $2)
 SET dm2_install_schema->u_name = cnvtupper( $3)
 SET dm2_install_schema->p_word = cnvtupper( $4)
 IF (currdb="DB2UDB")
  SET dm2_install_schema->connect_str = cnvtupper( $2)
 ELSE
  SET dm2_install_schema->connect_str = cnvtupper( $5)
 ENDIF
 EXECUTE dm2_connect_to_dbase  $1
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ELSEIF ((dm2_install_schema->u_name="QUIT")
  AND (dm2_install_schema->p_word="QUIT")
  AND (dm2_install_schema->connect_str="QUIT"))
  CALL echo(concat("User quit connection to ",dm2_install_schema->dbase_name,"!"))
 ENDIF
#exit_program
 SET dm_err->eproc = "Testing of dm2_connect_to_dbase complete!"
 CALL final_disp_msg("dm2_test_dbcnct")
END GO
