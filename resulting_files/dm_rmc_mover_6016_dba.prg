CREATE PROGRAM dm_rmc_mover_6016:dba
 ...
 DECLARE cust_control_number = f8
 DECLARE cust_result_id = f8
 DECLARE cust_field_value = f8
 DECLARE cust_src_table_fld_master = vc
 DECLARE cust_src_table_al_reltn = vc
 DECLARE cust_src_table_al_result = vc
 DECLARE cust_col = vc
 DECLARE cust_rpt_missing = vc
 DECLARE cust_item = vc
 DECLARE cust_err_val = i4
 SET cust_src_table_al_result = dm2_get_rdds_tname("GL_ALIAS_FIELD_RESULT")
 SET cust_src_table_al_reltn = dm2_get_rdds_tname("GL_ALIAS_FIELD_RELTN")
 SET cust_src_table_fld_master = dm2_get_rdds_tname("GL_FIELD_MASTER")
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET temp_col_cnt = get_col_pos(cust_tab_name,"GL_FIELD_VALUE")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
 SET cust_field_value = cnvtint(get_value(cust_tab_name,"GL_FIELD_VALUE","FROM"))
 SET cust_result_id = cnvtint(get_value(cust_tab_name,"GL_ALIAS_FIELD_RESULT_ID","FROM"))
 SELECT INTO "NL:"
  gm.control_nbr, grltn.gl_field_master_id, gar.gl_alias_field_reltn_id
  FROM (parser(cust_src_table_fld_master) gm),
   (parser(cust_src_table_al_reltn) grltn),
   (parser(cust_src_table_al_result) gar)
  PLAN (gm)
   JOIN (grltn
   WHERE grltn.gl_field_master_id=gm.gl_field_master_id)
   JOIN (gar
   WHERE gar.gl_alias_field_reltn_id=grltn.gl_alias_field_reltn_id
    AND gar.gl_alias_field_result_id=cust_result_id)
  DETAIL
   cust_control_number = gm.control_nbr
  WITH nocounter
 ;end select
 CASE (cust_control_number)
  OF 787806:
   SET cust_item = "HEALTH_PLAN"
   SET cust_col = "HEALTH_PLAN_ID"
  OF 787807:
  OF 787808:
   SET cust_item = "PRSNL"
   SET cust_col = "PERSON_ID"
  OF 787812:
   SET cust_item = "BILL_TEMPL"
   SET cust_col = "BILL_TEMPL_ID"
  OF 787819:
  OF 787822:
   SET cust_item = "ORGANIZATION"
   SET cust_col = "ORGANIZATION_ID"
  OF 787817:
  OF 787815:
   SET cust_item = "BILL_ITEM"
   SET cust_col = "BILL_ITEM_ID"
  OF 787818:
   SET cust_item = "CHARGE_MOD"
   SET cust_col = "CHARGE_MOD_ID"
  OF 787801:
  OF 787802:
  OF 787803:
  OF 787804:
  ...
  OF 787820:
  OF 787823:
   SET cust_item = "CODE_VALUE"
   SET cust_col = "CODE_VALUE"
  ELSE
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV26"
   CALL merge_audit("FAILREASON",concat("ERROR: The GL_FIELD_VALUE value of ",cnvtstring(
      cust_control_number)," is not supported at this time"),1)
   GO TO exit_6016
 ENDCASE
 IF (cust_item > " ")
  IF (cust_field_value > 0)
   SET new_value = select_merge_translate(cnvtstring(cust_field_value),cust_item)
   IF (isnumeric(new_value))
    CALL put_value(cust_tab_name,"GL_FIELD_VALUE",new_value)
   ELSE
    SET cust_rpt_missing = report_missing(cust_item,cust_col,cust_field_value)
    CALL evaluate_rpt_missing(cust_rpt_missing)
   ENDIF
  ELSE
   CALL put_value(cust_tab_name,"GL_FIELD_VALUE",cnvtstring(cust_field_value))
  ENDIF
 ENDIF
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_6016
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"GL_ALIAS_FIELD_RESULT_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"GL_ALIAS_FIELD_RESULT_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"GL_ALIAS_FIELD_RESULT_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,
       "GL_ALIAS_FIELD_RESULT_ID")
     ELSE
      GO TO exit_6016
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"GL_ALIAS_FIELD_RESULT_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6016
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"GL_ALIAS_FIELD_RESULT_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6016
   ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"GL_ALIAS_FIELD_RESULT_ID")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg =
    "This row wasn't translated because of the GL_ALIAS_FIELD_RESULT_ID column"
