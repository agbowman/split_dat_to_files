CREATE PROGRAM cp_del_concept_group_dtl
 PROMPT
  "Output to File/Printer/MINE:" = "MINE",
  "List of Concept Group ID:" = 0.0
  WITH outdev, cgids
 FREE RECORD cgdlist
 RECORD cgdlist(
   1 cnt = i4
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE deleterows(null) = null WITH protect
 DECLARE lstat = i4 WITH protect, noconstant(0)
 DECLARE dconceptgroupid = f8 WITH protect, noconstant(0.0)
 ...
 SET reply->status_data.status = "F"
 CALL getparametervalues(2,cgdlist)
 IF (validate(debug_ind,0)=1)
  CALL echorecord(cgdlist)
 ENDIF
 IF ((cgdlist->cnt > 0))
 ...
 ELSE
  ...
  SET reply->status_data.subeventstatus.targetobjectvalue = build2(
   "CP_CONCEPT_GROUP_DTL_ID list is empty.")
  GO TO exit_script
 ENDIF
 SET reply->status_data.status = "S"
 SUBROUTINE (checkconceptgroupid(idx=i4) =i4 WITH protect)
   CALL log_message("In CheckConceptGroupId()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SELECT INTO "nl:"
    FROM cp_concept_group_dtl ccgd
    WHERE (ccgd.cp_concept_group_dtl_id=cgdlist->qual[idx].value)
    DETAIL
     dconceptgroupid = ccgd.cp_concept_group_id
    WITH nocounter
   ;end select
   CALL log_message(build("Exit CheckConceptGroupId(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(curqual)
 END ;Subroutine
 SUBROUTINE (checkconceptgroupattr(idx=i4) =i4 WITH protect)
   CALL log_message("In CheckConceptGroupAttr()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SET lstat = initrec(cgdalist)
   SELECT INTO "nl:"
    FROM cp_concept_group_dtl_attr ccgda
    WHERE (ccgda.cp_concept_group_dtl_id=cgdlist->qual[idx].value)
    WITH nocounter
   ;end select
   IF (validate(debug_ind,0)=1)
    CALL log_message(build(curqual," Attributes found."),log_level_debug)
   ENDIF
   CALL log_message(build("Exit CheckConceptGroupAttr(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(curqual)
 END ;Subroutine
 SUBROUTINE (checkconceptgroupsize(id=f8) =i4 WITH protect)
   CALL log_message("In CheckConceptGroupSize()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE lqual = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM cp_concept_group_dtl ccgd
    WHERE ccgd.cp_concept_group_id=id
    WITH nocounter
   ;end select
   SET lqual = curqual
   IF (curqual=0)
    CALL log_message(build("Deleting CP_CONCEPT_GROUP_ID[",id,"]..."),log_level_debug)
    DELETE  FROM cp_concept_group ccg
     WHERE ccg.cp_concept_group_id=id
     WITH nocounter
    ;end delete
   ENDIF
   CALL log_message(build("Exit CheckConceptGroupSize(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(lqual)
 END ;Subroutine
 SUBROUTINE deleterows(null)
   ...
   DECLARE xsize = i4 WITH protect, constant(size(cgdlist->qual,5))
   DECLARE x = i4 WITH protect, noconstant(0)
   DECLARE attrcnt = i4 WITH protect, noconstant(0)
   DECLARE dtlcnt = i4 WITH protect, noconstant(0)
   IF (checkdic("CP_CONCEPT_GROUP_DTL","T",0) != 2)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus.operationname = "PERFORM"
    SET reply->status_data.subeventstatus.operationstatus = "F"
    SET reply->status_data.subeventstatus.targetobjectname = "DELETE"
    SET reply->status_data.subeventstatus.targetobjectvalue = build2(
     "CP_CONCEPT_GROUP_DTL Table does not exist ","or Permission is not granted to access.")
    GO TO exit_script
   ENDIF
   FOR (x = 1 TO xsize)
     SET dconceptgroupid = 0.0
     SET dtlcnt = 0
     SET attrcnt = 0
     SET dtlcnt = checkconceptgroupid(x)
     IF (dtlcnt > 0)
      SET attrcnt = checkconceptgroupattr(x)
      IF (attrcnt > 0)
       DELETE  FROM cp_concept_group_dtl_attr ccgda
        WHERE (ccgda.cp_concept_group_dtl_id=cgdlist->qual[x].value)
        WITH nocounter
       ;end delete
       SET lstat = error_and_zero_check_rec(1,"DELETE",
        "ERROR DELETING FROM CP_CONCEPT_GROUP_DTL_ATTR",1,1,
        reply)
      ENDIF
      IF (validate(debug_ind,0)=1)
       CALL echo(build("Deleting CP_CONCEPT_GROUP_DTL_ID[",cgdlist->qual[x].value,"]"))
      ENDIF
      DELETE  FROM cp_concept_group_dtl ccgd
       WHERE (ccgd.cp_concept_group_dtl_id=cgdlist->qual[x].value)
       WITH nocounter
      ;end delete
      SET lstat = error_and_zero_check_rec(1,"DELETE","ERROR DELETING FROM CP_CONCEPT_GROUP_DTL",1,1,
       reply)
     ELSE
      SET reply->status_data.status = "F"
      SET reply->status_data.subeventstatus.operationname = "PERFORM"
      SET reply->status_data.subeventstatus.operationstatus = "F"
      SET reply->status_data.subeventstatus.targetobjectname = "SELECT"
      SET reply->status_data.subeventstatus.targetobjectvalue = build2("CP_CONCEPT_GROUP_DTL_ID[",
       cgdlist->qual[x].value,"] does not exist on the CP_CONCEPT_GROUP_DTL table.")
      GO TO exit_script
     ENDIF
     IF (dconceptgroupid > 0.0)
      SET lstat = checkconceptgroupsize(dconceptgroupid)
     ENDIF
   ENDFOR
