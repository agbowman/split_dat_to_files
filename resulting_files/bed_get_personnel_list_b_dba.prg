CREATE PROGRAM bed_get_personnel_list_b:dba
 ...
 ENDIF
 DECLARE checkpersonnelcnt(dummyvar=i2) = null
 SET reply->status_data.status = "F"
 SET reply->too_many_results_ind = 0
 DECLARE error_msg = vc
 DECLARE error_flag = vc WITH protect, noconstant("F")
 SET wcard = "*"
 SET pcnt = 0
 SET ocnt = 0
 SET listcnt = 0
 ...
 ENDIF
 IF (validate(request->external_ind,0)=1)
  SET plistparse = concat(plistparse," and p.external_ind = 1")
 ELSEIF (validate(request->external_ind,0)=2)
  SET plistparse = concat(plistparse," and p.external_ind = 0")
 ENDIF
 ...
 IF ((request->person_id > 0.0))
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    reply->prsnl_list[pcnt].active_ind = p.active_ind, reply->prsnl_list[pcnt].beg_effective_dt_tm =
    p.beg_effective_dt_tm, reply->prsnl_list[pcnt].end_effective_dt_tm = p.end_effective_dt_tm,
    reply->prsnl_list[pcnt].external_ind = p.external_ind
  ...
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    reply->prsnl_list[pcnt].position_code_value = p.position_cd, reply->prsnl_list[pcnt].active_ind
     = p.active_ind, reply->prsnl_list[pcnt].beg_effective_dt_tm = p.beg_effective_dt_tm,
    reply->prsnl_list[pcnt].end_effective_dt_tm = p.end_effective_dt_tm, reply->prsnl_list[pcnt].
    external_ind = p.external_ind
    IF ((request->load.get_org_ind=1))
     reply->prsnl_list[pcnt].org_ind = 1
    ENDIF
   ...
   SELECT INTO "nl:"
    ...
    HEAD p.person_id
     ...
     reply->prsnl_list[pcnt].position_code_value = p.position_cd, reply->prsnl_list[pcnt].active_ind
      = p.active_ind, reply->prsnl_list[pcnt].beg_effective_dt_tm = p.beg_effective_dt_tm,
     reply->prsnl_list[pcnt].end_effective_dt_tm = p.end_effective_dt_tm, reply->prsnl_list[pcnt].
     external_ind = p.external_ind, reply->prsnl_list[pcnt].confid_level_code_value = cv.code_value,
     reply->prsnl_list[pcnt].confid_level_disp = cv.display, reply->prsnl_list[pcnt].
 ...
 ENDIF
 SUBROUTINE checkpersonnelcnt(dummyvar)
