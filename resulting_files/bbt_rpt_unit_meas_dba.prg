CREATE PROGRAM bbt_rpt_unit_meas:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 RECORD captions(
   ...
   1 page_no = vc
   1 reason_tool = vc
   1 units_of_measure = vc
   1 table_for_all_labs = vc
   1 do_not_delete = vc
   1 display_name = vc
   1 description = vc
   1 active = vc
 ...
 SET captions->units_of_measure = uar_i18ngetmessage(i18nhandle,"units_of_measure","UNITS OF MEASURE"
  )
 SET captions->table_for_all_labs = uar_i18ngetmessage(i18nhandle,"table_for_all_labs",
  "THIS IS A TABLE USED BY ALL LAB AREAS")
 SET captions->do_not_delete = uar_i18ngetmessage(i18nhandle,"do_not_delete",
  "DO NOT DELETE OR MODIFY UNLESS YOU ARE SURE THE UNITS ARE YOURS")
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_unit_measure", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  HEAD PAGE
   ...
   col 51, captions->units_of_measure, row + 1,
   col 38, captions->table_for_all_labs, row + 1,
   col 30, captions->do_not_delete, row + 2,
   line = fillstring(122,"-"), line, row + 1,
   col 4, captions->display_name, col 30,
   captions->description, col 62, captions->active,
   row + 1, line, row + 2
  DETAIL
