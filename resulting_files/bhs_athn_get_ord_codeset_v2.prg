CREATE PROGRAM bhs_athn_get_ord_codeset_v2
 DECLARE input_string = vc WITH constant(cnvtupper(trim( $3)))
 DECLARE str_tomorrowam = f8 WITH protect, constant(uar_get_code_by("DISPLAY_KEY",1304,"TOMORROWAM"))
 IF (input_string="AT")
 ...
 SELECT DISTINCT INTO  $1
  ...
  PLAN (d
   ...
   WHERE opf.priority_cd=outerjoin(d.entity2_id)
    AND opf.oe_format_id=outerjoin( $5)
    AND opf.active_ind=outerjoin(1))
  ...
  DETAIL
   ...
   row + 1, r_meaning = build("<Meaning>",meaning,"</Meaning>"), col + 1,
   r_meaning, row + 1
  FOOT  d.entity2_id
   col 1, "</CodeValues>", row + 1
  FOOT REPORT
