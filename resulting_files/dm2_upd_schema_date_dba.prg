CREATE PROGRAM dm2_upd_schema_date:dba
 SET compile = debug
 DECLARE c_mod = vc WITH private, constant("000")
 DECLARE dusd_logfile = vc WITH protect, constant("dm2updschdate")
 DECLARE cdomain_dm2_schema_dt = vc WITH protect, constant("DM2_SCHEMA_DATE")
 DECLARE cdomain_data_mgt = vc WITH protect, constant("DATA MANAGEMENT")
 DECLARE cname_user_last_updt = vc WITH protect, constant("USERLASTUPDT")
 DECLARE table_exists_on_dm_info() = i2
 DECLARE insert_table_on_dm_info() = null
 DECLARE update_schema_date_for_table() = null
 DECLARE update_user_last_update_row() = null
 FREE RECORD dusd_date
 RECORD dusd_date(
   1 in_schema_dt = dq8
   1 table_info_dt = dq8
 )
 DECLARE in_schema_dt_vc = vc WITH protect, noconstant(" ")
 DECLARE in_table_name = vc WITH protect, noconstant(" ")
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile(dusd_logfile,".log","DM2_UPD_SCHEMA_DATE LOG FILE BEGIN ")=1)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_upd_schema_date"
 CALL disp_msg("",dm_err->logfile,0)
 CALL edit_parameter_input(null)
 IF (in_table_name > " ")
  IF (table_exists_on_dm_info(null)=true)
   IF ((dusd_date->table_info_dt < dusd_date->in_schema_dt))
    CALL update_schema_date_for_table(null)
    CALL update_user_last_update_row(null)
   ENDIF
  ELSE
   CALL insert_table_on_dm_info(null)
   CALL update_user_last_update_row(null)
  ENDIF
 ELSE
  CALL update_user_last_update_row(null)
 ENDIF
 SUBROUTINE edit_parameter_input(null)
   SET in_table_name = cnvtupper(trim( $1))
   SET in_schema_dt_vc =  $2
   IF (dm2_val_sch_date_str(in_schema_dt_vc)=1)
    SET dusd_date->in_schema_dt = cnvtdatetime(in_schema_dt_vc)
   ELSE
    GO TO end_program
   ENDIF
   IF ((dm_err->debug_flag=1))
    CALL echo(build("Input Parameter Table:", $1))
    CALL echo(build("Input Parameter Date :", $2))
   ENDIF
 END ;Subroutine
 SUBROUTINE table_exists_on_dm_info(null)
   IF ((dm_err->debug_flag=1))
    SET dm_err->eproc = concat("Determine if DM_INFO row exists for table ",in_table_name)
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET dusd_date->table_info_dt = null
   SELECT INTO "nl:"
    i.info_date
    FROM dm_info i
    WHERE i.info_domain=value(cdomain_dm2_schema_dt)
     AND i.info_name=in_table_name
    DETAIL
     dusd_date->table_info_dt = i.info_date
    WITH nocounter
   ;end select
   IF (check_error("Table_Exists_on_DM_INFO")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ELSE
    IF (curqual > 0)
     RETURN(true)
   ...
   ENDIF
   IF ((dm_err->debug_flag=1))
    CALL echo(build("Info_Date:",dusd_date->info_date))
   ENDIF
   IF ((dm_err->err_ind > 0))
    GO TO end_program
   ENDIF
 END ;Subroutine
 SUBROUTINE insert_table_on_dm_info(null)
   IF ((dm_err->debug_flag=1))
    SET dm_err->eproc = "Insert New DM_INFO Row"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   INSERT  FROM dm_info i
    SET i.info_domain = value(cdomain_dm2_schema_dt), i.info_name = in_table_name, i.info_date =
     cnvtdatetime(dusd_date->in_schema_dt),
     i.info_char = "", i.info_number = 0, i.info_long_id = 0,
     i.updt_applctx = 0, i.updt_dt_tm = cnvtdatetime(curdate,curtime3), i.updt_cnt = 0,
     i.updt_id = 0, i.updt_task = 0
    WITH nocounter
   ;end insert
   IF (check_error("Insert_Table_on_DM_INFO")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
   IF ((dm_err->debug_flag=1))
    CALL echo(build("Inserted Schema Date Rows:",curqual))
   ENDIF
   IF ((dm_err->err_ind > 0))
    GO TO end_program
   ENDIF
 END ;Subroutine
 SUBROUTINE update_schema_date_for_table(null)
   IF ((dm_err->debug_flag=1))
    SET dm_err->eproc = "Update Schema Date row on DM_INFO"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   UPDATE  FROM dm_info i
    SET i.info_date = cnvtdatetime(dusd_date->in_schema_dt), i.updt_dt_tm = cnvtdatetime(curdate,
      curtime3), i.updt_cnt = (i.updt_cnt+ 1)
    WHERE i.info_domain=value(cdomain_dm2_schema_dt)
     AND i.info_name=in_table_name
    WITH nocounter
   ;end update
   IF (check_error("Update_Schema_Date_For_Table")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
   IF ((dm_err->debug_flag=1))
    CALL echo(build("Updated Schema Date Rows:",curqual))
   ENDIF
   IF ((dm_err->err_ind > 0))
    GO TO end_program
   ENDIF
 END ;Subroutine
 SUBROUTINE update_user_last_update_row(null)
   IF ((dm_err->debug_flag=1))
    SET dm_err->eproc = "Update or Insert the User Last Update Row on DM_INFO"
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   UPDATE  FROM dm_info i
    SET i.info_date = cnvtdatetime(curdate,curtime3), i.updt_cnt = (i.updt_cnt+ 1)
    WHERE i.info_domain=cdomain_data_mgt
     AND i.info_name=cname_user_last_updt
    WITH nocounter
   ;end update
   IF (check_error("Update_User_Last_Update_Row")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ENDIF
   IF (curqual=0)
    INSERT  FROM dm_info i
     SET i.info_domain = value(cdomain_data_mgt), i.info_name = value(cname_user_last_updt), i
     ...
     WITH nocounter
    ;end insert
    IF (check_error("Insert_User_Last_Update_Row")=true)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ENDIF
    IF ((dm_err->debug_flag=1))
     CALL echo(build("Inserted Last User Update Rows:",curqual))
    ENDIF
   ELSE
    IF ((dm_err->debug_flag=1))
     CALL echo(build("Updated Last User Update Rows:",curqual))
    ENDIF
   ENDIF
   IF ((dm_err->err_ind > 0))
    GO TO end_program
