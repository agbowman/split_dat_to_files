CREATE PROGRAM bed_ens_rx_fill_cycle_loc:dba
 FREE SET reply
 RECORD reply(
   1 error_msg = vc
   1 status_data
 ...
 FOR (x = 1 TO cnt)
   ...
   SELECT INTO "nl:"
    ...
    HEAD f.dispense_category_cd
     ccnt = (ccnt+ 1), stat = alterlist(temp->qual,ccnt), temp->qual[ccnt].cat_cd = f
     .dispense_category_cd
    WITH nocounter
   ;end select
   SET lcnt = size(request->fill_cycles[x].locations,5)
   FOR (y = 1 TO lcnt)
    IF ((request->fill_cycles[x].locations[y].action_flag=1))
     FOR (z = 1 TO ccnt)
       ...
       SELECT INTO "nl:"
        ...
        PLAN (f
         WHERE (f.fill_batch_cd=request->fill_cycles[x].code_value)
          AND (f.location_cd=request->fill_cycles[x].locations[y].code_value)
          AND (f.dispense_category_cd=temp->qual[z].cat_cd))
        DETAIL
         insert_fcb_row = 0
        WITH nocounter
       ;end select
       IF (insert_fcb_row=1)
        ...
        INSERT  FROM fill_cycle_batch f
         SET f.fill_batch_cd = request->fill_cycles[x].code_value, f.location_cd = request->
          fill_cycles[x].locations[y].code_value, f.dispense_category_cd = temp->qual[z].cat_cd,
       ...
       SELECT INTO "nl:"
        FROM fill_cycle f
        PLAN (f
         WHERE (f.location_cd=request->fill_cycles[x].locations[y].code_value)
          AND (f.dispense_category_cd=temp->qual[z].cat_cd))
        DETAIL
         insert_fc_row = 0
        WITH nocounter
       ...
       IF (insert_fc_row=1)
        ...
        INSERT  FROM fill_cycle f
         SET f.location_cd = request->fill_cycles[x].locations[y].code_value, f.dispense_category_cd
           = temp->qual[z].cat_cd, f.from_dt_tm = null,
     ...
     ENDFOR
    ENDIF
    IF ((request->fill_cycles[x].locations[y].action_flag=3))
     FOR (z = 1 TO ccnt)
       SET update_fb_row = 1
       SET ierrcode = 0
