CREATE PROGRAM bed_aud_bb_refrange:dba
 ...
 IF ((request->skip_volume_check_ind=0))
  SELECT INTO "nl:"
   ...
   PLAN (dta
    WHERE dta.activity_type_cd=bb_cd
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dta
   WHERE dta.active_ind=1
    AND dta.activity_type_cd=bb_cd)
   JOIN (cvdta
...
#skip_processing
 ...
 SET reply->collist[26].hide_ind = 1
 ...
 SET reply->collist[38].header_text = "Numeric Map Minimum Decimal Place"
 ...
 FOR (x = 1 TO tcnt)
   ...
   FOR (y = 1 TO rcnt)
     IF ((temp->dlist[x].rrf[y].skip_ind=0))
      ...
      IF ((temp->dlist[x].task_assay_cd > 0))
       SET reply->rowlist[row_nbr].celllist[39].string_value = cnvtstring(temp->dlist[x].
        task_assay_cd)
      ELSE
       SET reply->rowlist[row_nbr].celllist[39].string_value = " "
      ENDIF
      IF ((temp->dlist[x].rrf[y].service_resource_cd > 0))
       SET reply->rowlist[row_nbr].celllist[40].string_value = cnvtstring(temp->dlist[x].rrf[y].
        service_resource_cd)
      ELSE
       SET reply->rowlist[row_nbr].celllist[40].string_value = " "
      ENDIF
      IF ((temp->dlist[x].rrf[y].reference_range_factor_id > 0))
       SET reply->rowlist[row_nbr].celllist[41].string_value = cnvtstring(temp->dlist[x].rrf[y].
      ...
      FOR (z = 1 TO acnt)
        ...
        SET reply->rowlist[row_nbr].celllist[23].string_value = trim(temp->dlist[x].rrf[y].al[z].term
         )
   ...
   IF (rcnt=0
    ...
    SET reply->rowlist[row_nbr].celllist[3].string_value = trim(temp->dlist[x].dtadesc)
    IF ((temp->dlist[x].task_assay_cd > 0))
     SET reply->rowlist[row_nbr].celllist[39].string_value = cnvtstring(temp->dlist[x].task_assay_cd)
    ELSE
     SET reply->rowlist[row_nbr].celllist[39].string_value = " "
    ENDIF
   ENDIF
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("bb_refrange_audit.csv")
 ENDIF
