CREATE PROGRAM dm_empi_process_log_rows:dba
 ...
 SET reply->table_name = "EMPI_PROCESS_LOG"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 SET i18nhandle = 0
 SET stat = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 IF (validate(last_mod,"NOMOD")="NOMOD")
  DECLARE last_mod = c5 WITH protect, noconstant("")
 ENDIF
 SET last_mod = "186432"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(epl.empi_process_log_id)
   FROM empi_process_log epl
   WHERE epl.empi_process_log_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(epl.empi_process_log_id)
  FROM empi_process_log epl
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    epl.rowid
    FROM empi_process_log epl
    WHERE parser(sbr_getrowidnotexists("epl.empi_process_log_id between curMinID and curMaxID","epl")
     )
     AND epl.create_dt_tm > cnvtdatetime("01-JAN-1800 00:00:00.00")
     AND epl.create_dt_tm <= cnvtdatetime((curdate - days_to_keep),curtime3)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = epl.rowid
    WITH nocounter, maxqual(epl,value(rowsleft))
   ;end select
