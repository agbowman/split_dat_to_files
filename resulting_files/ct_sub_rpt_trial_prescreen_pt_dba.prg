CREATE PROGRAM ct_sub_rpt_trial_prescreen_pt:dba
 PROMPT
 ...
 DECLARE get_prescreen_patient(dummy) = null WITH protect
 ...
 DECLARE headp_person_idsection(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE headp_person_idsectionabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) =
 f8 WITH protect
 ...
 DECLARE _remlabel_rpt_patient = i4 WITH noconstant(1), protect
...
"
  ENDIF
 END ;Subroutine
 SUBROUTINE get_prescreen_patient(dummy)
   SELECT
    IF (( $EVALBY < 2))
    ...
    HEAD PAGE
     ...
     IF ((eksctrequest->opsind < 1))
      _bcontheadpagesection = 0, dummy_val = headpagesection(rpt_render,((rptreport->m_pageheight -
       rptreport->m_marginbottom) - _yoffset),_bcontheadpagesection), dummy_val =
      headpagesection_line(rpt_render)
     ENDIF
      ...
      IF ((eksctrequest->opsind < 1))
       tmp_name = concat(trim(p.name_last),", ",trim(p.name_first)," (",trim(cnvtstring(e.person_id)),
        ")"), tmp_encntr = concat(trim(uar_get_code_display(e.encntr_type_cd))," (",trim(cnvtstring(e
          .encntr_id)),")"), tmp_sex = trim(uar_get_code_display(p.sex_cd)),
       tmp_age = trim(cnvtage(p.birth_dt_tm),3), tmp_reg_dt_tm = format(e.reg_dt_tm,"@MEDIUMDATETIME"
        ), tmp_race = trim(uar_get_code_display(p.race_cd)),
       tmp_facility = trim(uar_get_code_display(e.loc_facility_cd)), _bcontheadp_person_idsection = 0,
       bfirsttime = 1
 ...
 SUBROUTINE headpagesectionabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(1.480000), private
   ...
   DECLARE drawheight_label_rpt_patient = f8 WITH noconstant(0.0), private
   ...
   DECLARE __label_rpt_pt_inquiry_rpt = vc WITH noconstant(build2(label->rpt_pt_inquiry_rpt,char(0))),
   protect
   DECLARE __date = vc WITH noconstant(build2(trim(datestr),char(0))), protect
   DECLARE __label_rpt_patient = vc WITH noconstant(build2(label->rpt_patient,char(0))), protect
   DECLARE __label_rpt_encounter = vc WITH noconstant(build2(label->rpt_encounter,char(0))), protect
   DECLARE __label_rpt_gender = vc WITH noconstant(build2(label->rpt_gender,char(0))), protect
   DECLARE __label_rpt_age = vc WITH noconstant(build2(label->rpt_age,char(0))), protect
   DECLARE __label_rpt_reg_dt = vc WITH noconstant(build2(label->rpt_reg_dt,char(0))), protect
   DECLARE __label_rpt_race = vc WITH noconstant(build2(label->rpt_race,char(0))), protect
   DECLARE __label_rpt_facility = vc WITH noconstant(build2(label->rpt_facility,char(0))), protect
   ...
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ELSE
    SET rptsd->m_y = (offsety+ 1.117)
   ENDIF
   SET rptsd->m_x = (offsetx+ 0.000)
   SET rptsd->m_width = 1.125
   SET rptsd->m_height = ((offsety+ maxheight) - rptsd->m_y)
   SET _dummyfont = uar_rptsetfont(_hreport,_courier8b0)
   SET _holdremlabel_rpt_patient = _remlabel_rpt_patient
   IF (_remlabel_rpt_patient > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_remlabel_rpt_patient,((
       size(__label_rpt_patient) - _remlabel_rpt_patient)+ 1),__label_rpt_patient)))
    SET drawheight_label_rpt_patient = rptsd->m_height
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _remlabel_rpt_patient = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remlabel_rpt_patient,((size(
        __label_rpt_patient) - _remlabel_rpt_patient)+ 1),__label_rpt_patient)))))
     SET _remlabel_rpt_patient = (_remlabel_rpt_patient+ rptsd->m_drawlength)
    ELSE
     SET _remlabel_rpt_patient = 0
    ENDIF
    SET growsum = (growsum+ _remlabel_rpt_patient)
   ENDIF
   SET rptsd->m_flags = 5
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   IF (_remlabel_rpt_encounter > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remlabel_rpt_encounter,((size(
     ...
     SET _remlabel_rpt_encounter = (_remlabel_rpt_encounter+ rptsd->m_drawlength)
    ELSE
     SET _remlabel_rpt_encounter = 0
    ENDIF
    SET growsum = (growsum+ _remlabel_rpt_encounter)
   ENDIF
   SET rptsd->m_flags = 5
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   IF (_remlabel_rpt_gender > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remlabel_rpt_gender,((size(
     ...
     SET _remlabel_rpt_gender = (_remlabel_rpt_gender+ rptsd->m_drawlength)
    ELSE
     SET _remlabel_rpt_gender = 0
    ENDIF
    SET growsum = (growsum+ _remlabel_rpt_gender)
   ENDIF
   SET rptsd->m_flags = 5
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   IF (_remlabel_rpt_age > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remlabel_rpt_age,((size(__label_rpt_age)
     ...
     SET _remlabel_rpt_age = (_remlabel_rpt_age+ rptsd->m_drawlength)
    ELSE
     SET _remlabel_rpt_age = 0
    ENDIF
    SET growsum = (growsum+ _remlabel_rpt_age)
   ENDIF
   SET rptsd->m_flags = 5
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   IF (_remlabel_rpt_reg_dt > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remlabel_rpt_reg_dt,((size(
     ...
     SET _remlabel_rpt_reg_dt = (_remlabel_rpt_reg_dt+ rptsd->m_drawlength)
    ELSE
     SET _remlabel_rpt_reg_dt = 0
    ENDIF
    SET growsum = (growsum+ _remlabel_rpt_reg_dt)
   ENDIF
   SET rptsd->m_flags = 5
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   IF (_remlabel_rpt_race > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remlabel_rpt_race,((size(__label_rpt_race
     ...
     SET _remlabel_rpt_race = (_remlabel_rpt_race+ rptsd->m_drawlength)
    ELSE
     SET _remlabel_rpt_race = 0
    ENDIF
    SET growsum = (growsum+ _remlabel_rpt_race)
   ENDIF
   SET rptsd->m_flags = 5
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   IF (_remlabel_rpt_facility > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remlabel_rpt_facility,((size(
     ...
     SET _remlabel_rpt_facility = (_remlabel_rpt_facility+ rptsd->m_drawlength)
    ELSE
     SET _remlabel_rpt_facility = 0
    ENDIF
    SET growsum = (growsum+ _remlabel_rpt_facility)
   ENDIF
   SET rptsd->m_flags = 5
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ELSE
    SET rptsd->m_y = (offsety+ 0.592)
   ENDIF
   SET rptsd->m_x = (offsetx+ 0.000)
   SET rptsd->m_width = 8.000
   SET rptsd->m_height = ((offsety+ maxheight) - rptsd->m_y)
   ...
   SET rptsd->m_x = (offsetx+ 0.000)
   SET rptsd->m_width = 1.125
   SET rptsd->m_height = drawheight_label_rpt_patient
   SET _dummyfont = uar_rptsetfont(_hreport,_courier8b0)
   IF (ncalc=rpt_render
    AND _holdremlabel_rpt_patient > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdremlabel_rpt_patient,
       ((size(__label_rpt_patient) - _holdremlabel_rpt_patient)+ 1),__label_rpt_patient)))
   ELSE
    SET _remlabel_rpt_patient = _holdremlabel_rpt_patient
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.951),(offsetx+ 8.000),(offsety+
     0.951))
   ENDIF
   IF (ncalc=rpt_render
    AND bcontinue=0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 1.025),(offsetx+ 8.000),(offsety+
     1.025))
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
 ...
 SUBROUTINE headpagesection_lineabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.034),(offsetx+ 8.000),(offsety+
     0.034))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.000),(offsety+ 0.101),(offsetx+ 8.000),(offsety+
     0.101))
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE headp_person_idsection(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = headp_person_idsectionabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE headp_person_idsectionabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.250000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   ...
   IF (_remencounter > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remencounter,((size(__encounter) -
     ...
     SET _remencounter = (_remencounter+ rptsd->m_drawlength)
    ELSE
     SET _remencounter = 0
    ENDIF
    SET growsum = (growsum+ _remencounter)
   ENDIF
   SET rptsd->m_flags = 5
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   IF (_remgender > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remgender,((size(__gender) - _remgender)
       + 1),__gender)))))
     SET _remgender = (_remgender+ rptsd->m_drawlength)
    ELSE
     SET _remgender = 0
    ENDIF
    SET growsum = (growsum+ _remgender)
   ENDIF
   SET rptsd->m_flags = 5
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   IF (_remreg_dt_tm > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remreg_dt_tm,((size(__reg_dt_tm) -
     ...
     SET _remreg_dt_tm = (_remreg_dt_tm+ rptsd->m_drawlength)
    ELSE
     SET _remreg_dt_tm = 0
    ENDIF
    SET growsum = (growsum+ _remreg_dt_tm)
   ENDIF
   SET rptsd->m_flags = 5
   IF (bcontinue)
    SET rptsd->m_y = offsety
   ...
   IF (_remrace > 0)
    ...
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remrace,((size(__race) - _remrace)+ 1),
       __race)))))
     SET _remrace = (_remrace+ rptsd->m_drawlength)
    ELSE
     SET _remrace = 0
    ENDIF
    SET growsum = (growsum+ _remrace)
   ENDIF
   SET rptsd->m_flags = 4
   IF (bcontinue)
    SET rptsd->m_y = offsety
 ...
 SUBROUTINE footreportsection_opsind_1abs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   DECLARE drawheight_date = f8 WITH noconstant(0.0), private
   DECLARE drawheight_label_total = f8 WITH noconstant(0.0), private
   DECLARE __label_title = vc WITH noconstant(build2(label->rpt_title,char(0))), protect
   DECLARE __date = vc WITH noconstant(build2(trim(datestr),char(0))), protect
   DECLARE __label_total = vc WITH noconstant(build2(tmp_total,char(0))), protect
   IF (bcontinue=0)
 ...
 SUBROUTINE initializereport(dummy)
   IF (_bsubreport=0)
    ...
    SET rptreport->m_reportname = "CT_SUB_RPT_TRIAL_PRESCREEN_PT"
