CREATE PROGRAM afc_apply_partial_credit:dba
 SET afc_apply_partial_credit = "CHARGSRV-14536.020"
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 DECLARE intervaladdcreditcnt = i4 WITH noconstant(0)
 DECLARE accnt = i4 WITH noconstant(1)
 ...
 DECLARE dsuspensersn = f8 WITH noconstant(0.0)
 DECLARE cdf_meaning = c12
 DECLARE cnt = i4
 DECLARE code_set = i4
 DECLARE dsuspensecd = f8 WITH noconstant(0.0)
 DECLARE rpcnt = i4 WITH noconstant(0)
 DECLARE dquantity = f8 WITH noconstant(0.0)
 DECLARE reprocessind = i2 WITH noconstant(0)
 DECLARE litemcount = i4 WITH noconstant(0)
 DECLARE rtind2 = i2 WITH noconstant(0)
 ...
 DECLARE ifi = f8 WITH noconstant(0.0)
 DECLARE rtind = i4 WITH noconstant(0)
 DECLARE pftind = i4 WITH noconstant(0)
 DECLARE 13019_mod_rsn_cd = f8 WITH noconstant(0.0)
 DECLARE 4001989_adjust_cd = f8 WITH noconstant(0.0)
 DECLARE 4001989_increase_cd = f8 WITH noconstant(0.0)
 DECLARE nincreaseind = i2 WITH noconstant(0)
 DECLARE chargeupdatedind = i2 WITH noconstant(false)
 IF ( NOT (validate(interfaced)))
  DECLARE interfaced = i4 WITH protect, constant(999)
 ENDIF
 IF ( NOT (validate(posted)))
  DECLARE posted = i4 WITH protect, constant(100)
 ENDIF
 CALL echo("Entering AFC_APPLY_PARTIAL_CREDIT")
 CALL logmsg(curprog,"Entering AFC_APPLY_PARTIAL_CREDIT",log_debug)
 SET appid = 951020
 SET taskid = 951020
 SET reqid = 951359
 SET happ = 0
 ...
 CALL echo(build("Request Charge Item Id: ",request->charge_item_id))
 CALL logmsg(curprog,build("Request Charge Item Id: ",request->charge_item_id),log_debug)
 IF ( NOT (validate(cs13019_changelog)))
  DECLARE cs13019_changelog = f8 WITH protect, constant(getcodevalue(13019,"CHANGELOG",0))
 ENDIF
 SET cdf_meaning = "PARTCREDIT"
 SET cnt = 1
 SET code_set = 13030
 SET stat = uar_get_meaning_by_codeset(code_set,nullterm(cdf_meaning),cnt,dsuspensersn)
 SET cdf_meaning = "SUSPENSE"
 SET cnt = 1
 SET code_set = 13019
 SET stat = uar_get_meaning_by_codeset(code_set,nullterm(cdf_meaning),cnt,dsuspensecd)
 SET cdf_meaning = "RELEASED"
 SET cnt = 1
 SET code_set = 13029
 SET stat = uar_get_meaning_by_codeset(code_set,nullterm(cdf_meaning),cnt,dreprocesscd)
 ...
 SET stat = uar_get_meaning_by_codeset(4001989,"ADJUST",1,4001989_adjust_cd)
 SET stat = uar_get_meaning_by_codeset(4001989,"INCREASE",1,4001989_increase_cd)
 CALL echo(build("MOD_RSN_CD: ",13019_mod_rsn_cd))
 CALL echo(build("ADJUST_CD: ",4001989_adjust_cd))
 CALL echo(build("INCREASE_CD: ",4001989_increase_cd))
 ...
 IF ((lockchargereply->status_data.status != "S"))
  ...
  GO TO end_program
 ENDIF
 CALL echo("Executing AFC_CHARGE_FIND")
 ...
 CALL echo(build("Item_Interval_Id: ",chargefind_reply->charge_items[1].item_interval_id))
 ...
 ENDIF
 CALL echo(build("ITEM_INTERVAL_ID: ",chargefind_reply->charge_items[1].item_interval_id))
 CALL echo(build("INTERVAL_TEMPLATE_CD: ",itc))
 IF ((((chargefind_reply->charge_items[1].item_interval_id > 0)) OR (itc > 0)) )
  ...
  CALL echo("Executing AFC_CHARGE_EVENT_FIND")
  CALL logmsg(curprog,"Executing AFC_CHARGE_EVENT_FIND",log_debug)
  ...
  IF ((chargeeventfind_reply->status_data.status="F"))
   CALL echo(build("Charge Event Find Reply Status is ",chargeeventfind_reply->status_data.status,
     " Ending Program"))
   CALL logmsg(curprog,build("Charge Event Find Reply Status is ",chargeeventfind_reply->status_data.
     status," Ending Program"),log_debug)
   SET reply->status_data.status = "F"
   GO TO end_program
  ELSE
   FOR (ceacnt = 1 TO size(chargeeventfind_reply->charge_event_act,5))
     IF ((chargefind_reply->charge_items[1].charge_event_act_id=chargeeventfind_reply->
     charge_event_act[ceacnt].charge_event_act_id))
      SET stat = alterlist(cea_record->charge_event_act,1)
      SET dquantity = chargeeventfind_reply->charge_event_act[ceacnt].quantity
      SET cea_record->charge_event_act[1].charge_event_act_id = chargeeventfind_reply->
      charge_event_act[ceacnt].charge_event_act_id
      SET cea_record->charge_event_act[1].charge_event_id = chargeeventfind_reply->charge_event_act[
      ceacnt].charge_event_id
      SET cea_record->charge_event_act[1].cea_type_cd = chargeeventfind_reply->charge_event_act[
      ceacnt].cea_type_cd
      SET cea_record->charge_event_act[1].cea_prsnl_id = chargeeventfind_reply->charge_event_act[
      ceacnt].cea_prsnl_id
      SET cea_record->charge_event_act[1].service_resource_cd = chargeeventfind_reply->
      charge_event_act[ceacnt].service_resource_cd
      SET cea_record->charge_event_act[1].service_dt_tm = chargeeventfind_reply->charge_event_act[
      ceacnt].service_dt_tm
      SET cea_record->charge_event_act[1].charge_dt_tm = chargeeventfind_reply->charge_event_act[
      ceacnt].charge_dt_tm
      SET cea_record->charge_event_act[1].charge_type_cd = chargeeventfind_reply->charge_event_act[
      ceacnt].charge_type_cd
      SET cea_record->charge_event_act[1].reference_range_factor_id = chargeeventfind_reply->
      charge_event_act[ceacnt].reference_range_factor_id
      SET cea_record->charge_event_act[1].alpha_nomen_id = chargeeventfind_reply->charge_event_act[
      ceacnt].alpha_nomen_id
      SET cea_record->charge_event_act[1].quantity = chargeeventfind_reply->charge_event_act[ceacnt].
      quantity
      SET cea_record->charge_event_act[1].units = chargeeventfind_reply->charge_event_act[ceacnt].
      units
      SET cea_record->charge_event_act[1].unit_type_cd = chargeeventfind_reply->charge_event_act[
      ceacnt].unit_type_cd
      SET cea_record->charge_event_act[1].patient_loc_cd = chargeeventfind_reply->charge_event_act[
      ceacnt].patient_loc_cd
      SET cea_record->charge_event_act[1].service_loc_cd = chargeeventfind_reply->charge_event_act[
      ceacnt].service_loc_cd
      SET cea_record->charge_event_act[1].reason_cd = chargeeventfind_reply->charge_event_act[ceacnt]
      .reason_cd
      SET cea_record->charge_event_act[1].updt_cnt = chargeeventfind_reply->charge_event_act[ceacnt].
      updt_cnt
      SET cea_record->charge_event_act[1].updt_dt_tm = chargeeventfind_reply->charge_event_act[ceacnt
      ].updt_dt_tm
      SET cea_record->charge_event_act[1].updt_task = chargeeventfind_reply->charge_event_act[ceacnt]
      .updt_task
      SET cea_record->charge_event_act[1].active_ind = chargeeventfind_reply->charge_event_act[ceacnt
      ].active_ind
      SET cea_record->charge_event_act[1].insert_dt_tm = chargeeventfind_reply->charge_event_act[
      ceacnt].insert_dt_tm
      SET cea_record->charge_event_act[1].updt_id = chargeeventfind_reply->charge_event_act[ceacnt].
      updt_id
      SET cea_record->charge_event_act[1].updt_applctx = chargeeventfind_reply->charge_event_act[
      ceacnt].updt_applctx
      SET cea_record->charge_event_act[1].in_lab_dt_tm = chargeeventfind_reply->charge_event_act[
      ceacnt].in_lab_dt_tm
      SET cea_record->charge_event_act[1].accession_id = chargeeventfind_reply->charge_event_act[
      ceacnt].accession_id
      SET cea_record->charge_event_act[1].repeat_ind = chargeeventfind_reply->charge_event_act[ceacnt
      ].repeat_ind
      SET cea_record->charge_event_act[1].result = chargeeventfind_reply->charge_event_act[ceacnt].
      result
      SET cea_record->charge_event_act[1].cea_misc1 = chargeeventfind_reply->charge_event_act[ceacnt]
      .cea_misc1
      SET cea_record->charge_event_act[1].cea_misc1_id = chargeeventfind_reply->charge_event_act[
      ceacnt].cea_misc1_id
      SET cea_record->charge_event_act[1].cea_misc2 = chargeeventfind_reply->charge_event_act[ceacnt]
      .cea_misc2
      SET cea_record->charge_event_act[1].cea_misc2_id = chargeeventfind_reply->charge_event_act[
      ceacnt].cea_misc2_id
      SET cea_record->charge_event_act[1].cea_misc3 = chargeeventfind_reply->charge_event_act[ceacnt]
      .cea_misc3
      SET cea_record->charge_event_act[1].cea_misc3_id = chargeeventfind_reply->charge_event_act[
      ceacnt].cea_misc3_id
      SET cea_record->charge_event_act[1].cea_misc4_id = chargeeventfind_reply->charge_event_act[
      ceacnt].cea_misc4_id
      SET cea_record->charge_event_act[1].srv_diag1_id = chargeeventfind_reply->charge_event_act[
      ceacnt].srv_diag1_id
      SET cea_record->charge_event_act[1].srv_diag2_id = chargeeventfind_reply->charge_event_act[
      ceacnt].srv_diag2_id
      SET cea_record->charge_event_act[1].srv_diag3_id = chargeeventfind_reply->charge_event_act[
      ceacnt].srv_diag3_id
      SET cea_record->charge_event_act[1].srv_diag4_id = chargeeventfind_reply->charge_event_act[
      ceacnt].srv_diag4_id
      SET cea_record->charge_event_act[1].srv_diag_cd = chargeeventfind_reply->charge_event_act[
      ceacnt].srv_diag_cd
      SET cea_record->charge_event_act[1].misc_ind = chargeeventfind_reply->charge_event_act[ceacnt].
      misc_ind
      SET cea_record->charge_event_act[1].cea_misc5_id = chargeeventfind_reply->charge_event_act[
      ceacnt].cea_misc5_id
      SET cea_record->charge_event_act[1].cea_misc6_id = chargeeventfind_reply->charge_event_act[
      ceacnt].cea_misc6_id
      SET cea_record->charge_event_act[1].cea_misc7_id = chargeeventfind_reply->charge_event_act[
      ceacnt].cea_misc7_id
      SET cea_record->charge_event_act[1].activity_dt_tm = chargeeventfind_reply->charge_event_act[
      ceacnt].activity_dt_tm
      SET cea_record->charge_event_act[1].priority_cd = chargeeventfind_reply->charge_event_act[
      ceacnt].priority_cd
      SET cea_record->charge_event_act[1].item_price = chargeeventfind_reply->charge_event_act[ceacnt
      ].item_price
      SET cea_record->charge_event_act[1].item_ext_price = chargeeventfind_reply->charge_event_act[
      ceacnt].item_ext_price
      SET cea_record->charge_event_act[1].item_copay = chargeeventfind_reply->charge_event_act[ceacnt
      ].item_copay
      SET cea_record->charge_event_act[1].discount_amount = chargeeventfind_reply->charge_event_act[
      ceacnt].discount_amount
      SET cea_record->charge_event_act[1].item_reimbursement = chargeeventfind_reply->
      charge_event_act[ceacnt].item_reimbursement
      SET cea_record->charge_event_act[1].item_deductible_amt = chargeeventfind_reply->
      charge_event_act[ceacnt].item_deductible_amt
      SET cea_record->charge_event_act[1].patient_responsibility_flag = chargeeventfind_reply->
      charge_event_act[ceacnt].patient_responsibility_flag
      SET ceacnt = size(chargeeventfind_reply->charge_event_act,5)
      CALL echo(build("Found Matching Charge Event, Charge Event Act ID: ",cea_record->
        charge_event_act[1].charge_event_act_id))
      CALL logmsg(curprog,build("Found Matching Charge Event, Charge Event Act ID: ",cea_record->
        charge_event_act[1].charge_event_act_id),log_debug)
     ENDIF
   ENDFOR
  ENDIF
 ENDIF
 IF ((request->item_quantity != dquantity))
  SET reprocessind = 1
  CALL echo("*****************************************************************************")
  CALL echo("  Request->Item_Quantity Not Equal to Charge Find Quantity, ReprocessInd = 1")
  CALL echo("*****************************************************************************")
  IF ((request->item_quantity > dquantity))
   SET nincreaseind = 1
  ENDIF
 ENDIF
 IF (reprocessind=0)
  IF ((request->charge_description != chargefind_reply->charge_items[1].charge_description)
   AND (((chargefind_reply->charge_items[1].process_flg=interfaced)) OR ((chargefind_reply->
  charge_items[1].process_flg=posted))) )
   SET reprocessind = 1
   CALL echo("*****************************************************************************")
   CALL echo("  Request->Charge_Description Not Equal to Charge Find Description, ReprocessInd = 1")
   CALL echo("*****************************************************************************")
  ELSEIF ((request->item_price != chargefind_reply->charge_items[1].item_price)
   AND (((chargefind_reply->charge_items[1].process_flg=interfaced)) OR ((chargefind_reply->
  charge_items[1].process_flg=posted))) )
   SET reprocessind = 1
   CALL echo("*****************************************************************************")
   CALL echo("  Request->item_price Not Equal to Charge Find Price, ReprocessInd = 1")
   CALL echo("*****************************************************************************")
  ENDIF
 ENDIF
 IF (reprocessind=1)
  IF ((((chargefind_reply->charge_items[1].item_interval_id > 0)) OR (((itc > 0) OR (
  uar_get_code_meaning(chargefind_reply->charge_items[1].activity_type_cd)="PHARMACY")) )) )
   CALL get_all_related_charges(0)
   UPDATE  FROM charge_event_act cea
    SET cea.quantity = request->item_quantity, cea.item_price = request->item_price, cea
     .item_ext_price = (request->item_quantity * request->item_price),
     cea.updt_id = reqinfo->updt_id, cea.updt_applctx = reqinfo->updt_applctx, cea.updt_task =
     reqinfo->updt_task,
  ...
  SET addcredit_request->charge[1].charge_item_id = chargefind_reply->charge_items[1].charge_item_id
  SET addcredit_request->charge[1].suspense_rsn_cd = request->suspense_rsn_cd
  SET addcredit_request->charge[1].reason_comment = request->reason_comment
  FOR (cnt2 = 1 TO size(intervalcharges->charge_items,5))
    SET addcredit_request->charge_qual += 1
    SET stat = alterlist(addcredit_request->charge,addcredit_request->charge_qual)
    SET addcredit_request->charge[(cnt2+ 1)].charge_item_id = intervalcharges->charge_items[cnt2].
    charge_item_id
    SET addcredit_request->charge[(cnt2+ 1)].suspense_rsn_cd = request->suspense_rsn_cd
    SET addcredit_request->charge[(cnt2+ 1)].reason_comment = request->reason_comment
  ENDFOR
  CALL echorecord(addcredit_request)
  CALL echo("Executing AFC_ADD_CREDIT for primary charge")
  CALL logmsg(curprog,"Executing AFC_ADD_CREDIT for primary charge",log_debug)
  ...
  IF ((addcredit_reply->status_data.status="F"))
   CALL echo(build("Add Credit Reply Status is ",addcredit_reply->status_data.status,
     " Ending Program"))
   CALL logmsg(curprog,build("Add Credit Reply Status is ",addcredit_reply->status_data.status,
     " Ending Program"),log_debug)
   SET reply->status_data.status = "F"
   GO TO end_program
  ELSE
   CALL echo("Checking on need to post or interface credits")
   CALL logmsg(curprog,"Checking on need to post or interface credits",log_debug)
   CALL echorecord(addcredit_reply)
   SELECT INTO "nl:"
    FROM interface_file i,
     (dummyt d1  WITH seq = value(size(addcredit_reply->charge,5)))
    PLAN (d1)
     JOIN (i
     WHERE (i.interface_file_id=addcredit_reply->charge[d1.seq].interface_file_id))
    HEAD d1.seq
     IF (i.realtime_ind=1)
      addcredit_reply->charge[d1.seq].interface_flag = 1
     ELSEIF (i.profit_type_cd > 0)
      addcredit_reply->charge[d1.seq].interface_flag = 2
     ENDIF
    WITH nocounter
   ;end select
   FOR (rtcnt = 1 TO size(addcredit_reply->charge,5))
     IF ((addcredit_reply->charge[rtcnt].interface_flag=1))
      SET rtind += 1
      SET stat = alterlist(afcinterfacecharge_request->interface_charge,rtind)
      SET afcinterfacecharge_request->interface_charge[rtind].charge_item_id = addcredit_reply->
      charge[rtcnt].charge_item_id
     ELSEIF ((addcredit_reply->charge[rtcnt].interface_flag=2))
      SET pftind += 1
      SET stat = alterlist(afcprofit_request->charges,pftind)
      SET afcprofit_request->charges[pftind].charge_item_id = addcredit_reply->charge[rtcnt].
      charge_item_id
     ENDIF
   ENDFOR
   IF (size(afcinterfacecharge_request->interface_charge,5) > 0)
    CALL echo("Executing Afc_Post_Interface_Charge")
    CALL echorecord(afcinterfacecharge_request)
    CALL logmsg(curprog,"Executing Afc_Post_Interface_Charge",log_debug)
  ...
  IF ((request->item_quantity > 0))
   SET stat = alterlist(addcharge_request->objarray,(size(intervalcharges->charge_items,5)+ 1))
   ...
   charge_description
   SET addcharge_request->objarray[1].price_sched_id = chargefind_reply->charge_items[1].
   price_sched_id
   ...
   item_extended_price
   SET addcharge_request->objarray[1].item_allowable = chargefind_reply->charge_items[1].
   item_allowable
   SET addcharge_request->objarray[1].item_copay = chargefind_reply->charge_items[1].item_copay
   SET addcharge_request->objarray[1].charge_type_cd = chargefind_reply->charge_items[1].
   charge_type_cd
   SET addcharge_request->objarray[1].research_acct_id = chargefind_reply->charge_items[1].
   research_acct_id
   IF (13019_mod_rsn_cd <= 0)
    SET addcharge_request->objarray[1].suspense_rsn_cd = request->suspense_rsn_cd
    SET addcharge_request->objarray[1].reason_comment = request->reason_comment
   ENDIF
   SET addcharge_request->objarray[1].posted_cd = chargefind_reply->charge_items[1].posted_cd
   SET addcharge_request->objarray[1].process_flg = 1
   SET addcharge_request->objarray[1].service_dt_tm = chargefind_reply->charge_items[1].service_dt_tm
   SET addcharge_request->objarray[1].activity_dt_tm = chargefind_reply->charge_items[1].
   activity_dt_tm
   ...
   def_bill_item_id
   SET addcharge_request->objarray[1].verify_phys_id = chargefind_reply->charge_items[1].
   verify_phys_id
   ...
   SET addcharge_request->objarray[1].institution_cd = chargefind_reply->charge_items[1].
   institution_cd
   SET addcharge_request->objarray[1].level5_cd = chargefind_reply->charge_items[1].level5_cd
   SET addcharge_request->objarray[1].med_service_cd = chargefind_reply->charge_items[1].
   med_service_cd
   ...
   SET addcharge_request->objarray[1].cost_center_cd = chargefind_reply->charge_items[1].
   cost_center_cd
   ...
   SET addcharge_request->objarray[1].health_plan_id = chargefind_reply->charge_items[1].
   health_plan_id
   ...
   SET addcharge_request->objarray[1].alpha_nomen_id = chargefind_reply->charge_items[1].
   alpha_nomen_id
   SET addcharge_request->objarray[1].item_price_adj_amt = chargefind_reply->charge_items[1].
   item_price_adj_amt
   IF ((chargefind_reply->charge_items[1].item_interval_id > 0.0))
    SET addcharge_request->objarray[1].server_process_flag = 0
   ELSE
   ...
   FOR (iaccnt = 1 TO size(intervalcharges->charge_items,5))
     SET accnt += 1
     SET addcharge_request->objarray[accnt].charge_item_id = 0.0
     SET addcharge_request->objarray[accnt].parent_charge_item_id = intervalcharges->charge_items[
     iaccnt].charge_item_id
     SET addcharge_request->objarray[accnt].charge_event_act_id = intervalcharges->charge_items[
     iaccnt].charge_event_act_id
     SET addcharge_request->objarray[accnt].charge_event_id = intervalcharges->charge_items[iaccnt].
     charge_event_id
     SET addcharge_request->objarray[accnt].bill_item_id = intervalcharges->charge_items[iaccnt].
     bill_item_id
     SET addcharge_request->objarray[accnt].order_id = intervalcharges->charge_items[iaccnt].order_id
     SET addcharge_request->objarray[accnt].encntr_id = intervalcharges->charge_items[iaccnt].
     encntr_id
     SET addcharge_request->objarray[accnt].person_id = intervalcharges->charge_items[iaccnt].
     person_id
     SET addcharge_request->objarray[accnt].payor_id = intervalcharges->charge_items[iaccnt].payor_id
     SET addcharge_request->objarray[accnt].perf_loc_cd = intervalcharges->charge_items[iaccnt].
     perf_loc_cd
     SET addcharge_request->objarray[accnt].ord_loc_cd = intervalcharges->charge_items[iaccnt].
     ord_loc_cd
     SET addcharge_request->objarray[accnt].ord_phys_id = intervalcharges->charge_items[iaccnt].
     ord_phys_id
     SET addcharge_request->objarray[accnt].perf_phys_id = intervalcharges->charge_items[iaccnt].
     perf_phys_id
     SET addcharge_request->objarray[accnt].charge_description = intervalcharges->charge_items[iaccnt
     ].charge_description
     SET addcharge_request->objarray[accnt].price_sched_id = intervalcharges->charge_items[iaccnt].
     price_sched_id
     SET addcharge_request->objarray[accnt].item_quantity = intervalcharges->charge_items[iaccnt].
     item_quantity
     SET addcharge_request->objarray[accnt].item_price = intervalcharges->charge_items[iaccnt].
     item_price
     SET addcharge_request->objarray[accnt].item_extended_price = intervalcharges->charge_items[
     iaccnt].item_extended_price
     SET addcharge_request->objarray[accnt].item_allowable = intervalcharges->charge_items[iaccnt].
     item_allowable
     SET addcharge_request->objarray[accnt].item_copay = intervalcharges->charge_items[iaccnt].
     item_copay
     SET addcharge_request->objarray[accnt].charge_type_cd = intervalcharges->charge_items[iaccnt].
     charge_type_cd
     SET addcharge_request->objarray[accnt].research_acct_id = intervalcharges->charge_items[iaccnt].
     research_acct_id
     IF (13019_mod_rsn_cd <= 0)
      SET addcharge_request->objarray[accnt].suspense_rsn_cd = request->suspense_rsn_cd
      SET addcharge_request->objarray[accnt].reason_comment = request->reason_comment
     ENDIF
     SET addcharge_request->objarray[accnt].posted_cd = intervalcharges->charge_items[iaccnt].
     posted_cd
     SET addcharge_request->objarray[accnt].process_flg = 1
     SET addcharge_request->objarray[accnt].service_dt_tm = intervalcharges->charge_items[iaccnt].
     service_dt_tm
     SET addcharge_request->objarray[accnt].activity_dt_tm = intervalcharges->charge_items[iaccnt].
     activity_dt_tm
     SET addcharge_request->objarray[accnt].interface_file_id = intervalcharges->charge_items[iaccnt]
     .interface_file_id
     SET addcharge_request->objarray[accnt].tier_group_cd = intervalcharges->charge_items[iaccnt].
     tier_group_cd
     SET addcharge_request->objarray[accnt].def_bill_item_id = intervalcharges->charge_items[iaccnt].
     def_bill_item_id
     SET addcharge_request->objarray[accnt].verify_phys_id = intervalcharges->charge_items[iaccnt].
     verify_phys_id
     SET addcharge_request->objarray[accnt].gross_price = intervalcharges->charge_items[iaccnt].
     gross_price
     SET addcharge_request->objarray[accnt].discount_amount = intervalcharges->charge_items[iaccnt].
     discount_amount
     SET addcharge_request->objarray[accnt].manual_ind = intervalcharges->charge_items[iaccnt].
     manual_ind
     SET addcharge_request->objarray[accnt].activity_type_cd = intervalcharges->charge_items[iaccnt].
     activity_type_cd
     SET addcharge_request->objarray[accnt].activity_sub_type_cd = intervalcharges->charge_items[
     iaccnt].activity_sub_type_cd
     SET addcharge_request->objarray[accnt].provider_specialty_cd = intervalcharges->charge_items[
     iaccnt].provider_specialty_cd
     SET addcharge_request->objarray[accnt].admit_type_cd = intervalcharges->charge_items[iaccnt].
     admit_type_cd
     SET addcharge_request->objarray[accnt].department_cd = intervalcharges->charge_items[iaccnt].
     department_cd
     SET addcharge_request->objarray[accnt].institution_cd = intervalcharges->charge_items[iaccnt].
     institution_cd
     SET addcharge_request->objarray[accnt].level5_cd = intervalcharges->charge_items[iaccnt].
     level5_cd
     SET addcharge_request->objarray[accnt].med_service_cd = intervalcharges->charge_items[iaccnt].
     med_service_cd
     SET addcharge_request->objarray[accnt].section_cd = intervalcharges->charge_items[iaccnt].
     section_cd
     SET addcharge_request->objarray[accnt].subsection_cd = intervalcharges->charge_items[iaccnt].
     subsection_cd
     SET addcharge_request->objarray[accnt].abn_status_cd = intervalcharges->charge_items[iaccnt].
     abn_status_cd
     SET addcharge_request->objarray[accnt].cost_center_cd = intervalcharges->charge_items[iaccnt].
     cost_center_cd
     SET addcharge_request->objarray[accnt].inst_fin_nbr = intervalcharges->charge_items[iaccnt].
     inst_fin_nbr
     SET addcharge_request->objarray[accnt].fin_class_cd = intervalcharges->charge_items[iaccnt].
     fin_class_cd
     SET addcharge_request->objarray[accnt].health_plan_id = intervalcharges->charge_items[iaccnt].
     health_plan_id
     SET addcharge_request->objarray[accnt].item_interval_id = intervalcharges->charge_items[iaccnt].
     item_interval_id
     SET addcharge_request->objarray[accnt].item_list_price = intervalcharges->charge_items[iaccnt].
     item_list_price
     SET addcharge_request->objarray[accnt].item_reimbursement = intervalcharges->charge_items[iaccnt
     ].item_reimbursement
     SET addcharge_request->objarray[accnt].list_price_sched_id = intervalcharges->charge_items[
     iaccnt].list_price_sched_id
     SET addcharge_request->objarray[accnt].payor_type_cd = intervalcharges->charge_items[iaccnt].
     payor_type_cd
     SET addcharge_request->objarray[accnt].epsdt_ind = intervalcharges->charge_items[iaccnt].
     epsdt_ind
     SET addcharge_request->objarray[accnt].ref_phys_id = intervalcharges->charge_items[iaccnt].
     ref_phys_id
     SET addcharge_request->objarray[accnt].start_dt_tm = intervalcharges->charge_items[iaccnt].
     start_dt_tm
     SET addcharge_request->objarray[accnt].stop_dt_tm = intervalcharges->charge_items[iaccnt].
     stop_dt_tm
     SET addcharge_request->objarray[accnt].alpha_nomen_id = intervalcharges->charge_items[iaccnt].
     alpha_nomen_id
     SET addcharge_request->objarray[accnt].server_process_flag = intervalcharges->charge_items[
     iaccnt].server_process_flag
     SET addcharge_request->objarray[accnt].item_deductible_amt = intervalcharges->charge_items[
     iaccnt].item_deductible_amt
     SET addcharge_request->objarray[accnt].item_price_adj_amt = intervalcharges->charge_items[iaccnt
     ].item_price_adj_amt
     SET addcharge_request->objarray[accnt].patient_responsibility_flag = intervalcharges->
     charge_items[iaccnt].patient_responsibility_flag
     SET addcharge_request->objarray[accnt].active_ind = 1
   ENDFOR
   CALL echorecord(addcharge_request)
   CALL echo("Executing AFC_ADD_CHARGE for primary and interval charges")
   CALL logmsg(curprog,"Executing AFC_ADD_CHARGE for primary and interval charges",log_debug)
   ...
   IF ((addcharge_reply->status_data.status="F"))
    CALL echo(build("Add Charge Reply Status is ",addcharge_reply->status_data.status,
      " Ending Program"))
    ...
    GO TO end_program
   ELSE
    FOR (chrgcnt = 1 TO size(addcharge_reply->mod_objs[1].mod_recs,5))
      SET addcharge_request->objarray[chrgcnt].charge_item_id = cnvtreal(addcharge_reply->mod_objs[1]
       .mod_recs[chrgcnt].pk_values)
    ENDFOR
    CALL echo("||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||")
    CALL echo("  New Add Charge Request Structure After Being Populated with New Charge Item Id's")
    CALL echo("||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||")
    CALL echorecord(addcharge_request)
    CALL echo("Checking on need to post new charges")
    CALL logmsg(curprog,"Checking on need to post new charges",log_debug)
    SET stat = alterlist(addcharge_flag->flags,size(addcharge_request->objarray,5))
    SELECT INTO "nl:"
     FROM interface_file i,
      (dummyt d1  WITH seq = value(size(addcharge_request->objarray,5)))
     PLAN (d1)
      JOIN (i
      WHERE (i.interface_file_id=addcharge_request->objarray[d1.seq].interface_file_id))
     DETAIL
      IF (i.profit_type_cd > 0)
       addcharge_flag->flags[d1.seq].post_flag = 1
      ENDIF
     WITH nocounter
    ;end select
    CALL echorecord(addcharge_flag)
    CALL echo(build("pftInd = ",pftind))
   ENDIF
   IF ((chargefind_reply->charge_items[1].item_interval_id <= 0)
    AND itc <= 0)
    CALL echo("Not an interval so need to populate the charge_mods for new charge")
    CALL logmsg(curprog,"Not an interval so need to populate the charge_mods for new charge",
     log_debug)
    SET stat = alterlist(addchargemodreq->charge_mod,size(chargefind_reply->charge_items[1].
      charge_mods,5))
    FOR (acmcnt = 1 TO size(chargefind_reply->charge_items[1].charge_mods,5))
      IF ((chargefind_reply->charge_items[1].charge_mods[acmcnt].charge_mod_type_cd !=
      cs13019_changelog)
       AND (chargefind_reply->charge_items[1].charge_mods[acmcnt].active_ind=true))
       ...
       SET addchargemodreq->charge_mod[acmcnt].field1 = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field1
       SET addchargemodreq->charge_mod[acmcnt].field2 = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field2
       SET addchargemodreq->charge_mod[acmcnt].field3 = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field3
       SET addchargemodreq->charge_mod[acmcnt].field4 = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field4
       SET addchargemodreq->charge_mod[acmcnt].field5 = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field5
       SET addchargemodreq->charge_mod[acmcnt].field6 = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field6
       SET addchargemodreq->charge_mod[acmcnt].field7 = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field7
       SET addchargemodreq->charge_mod[acmcnt].field8 = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field8
       SET addchargemodreq->charge_mod[acmcnt].field9 = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field9
       SET addchargemodreq->charge_mod[acmcnt].field10 = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field10
       SET addchargemodreq->charge_mod[acmcnt].activity_dt_tm = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].activity_dt_tm
       SET addchargemodreq->charge_mod[acmcnt].field1_id = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field1_id
       SET addchargemodreq->charge_mod[acmcnt].field2_id = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field2_id
       SET addchargemodreq->charge_mod[acmcnt].field3_id = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field3_id
       SET addchargemodreq->charge_mod[acmcnt].field4_id = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field4_id
       SET addchargemodreq->charge_mod[acmcnt].field5_id = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].field5_id
       SET addchargemodreq->charge_mod[acmcnt].nomen_id = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].nomen_id
       SET addchargemodreq->charge_mod[acmcnt].cm1_nbr = chargefind_reply->charge_items[1].
       charge_mods[acmcnt].cm1_nbr
    ...
    CALL echo("Executing AFC_ADD_CHARGE_MOD to add charge mods to new charges")
    CALL logmsg(curprog,"Executing AFC_ADD_CHARGE_MOD to add charge mods to new charges",log_debug)
    ...
    IF ((addchargemodrep->status_data.status="F"))
     ...
     CALL echo(build("AFC_ADD_CHARGE_MOD Reply Status is ",addcharge_reply->status_data.status,
       " Ending Program"))
     CALL logmsg(curprog,build("AFC_ADD_CHARGE_MOD Reply Status is ",addcharge_reply->status_data.
       status," Ending Program"),log_debug)
    ELSE
     SET reply->status_data.status = "S"
    ENDIF
   ENDIF
   SET stat = alterlist(cm_request->charge_mod,size(addcharge_reply->mod_objs[1].mod_recs,5))
   IF ((chargefind_reply->charge_items[1].item_interval_id=0.0))
    FOR (cmcnt = 1 TO size(addcharge_reply->mod_objs[1].mod_recs,5))
      SET cm_request->charge_mod[cmcnt].action_type = "ADD"
      SET cm_request->charge_mod[cmcnt].charge_item_id = cnvtreal(addcharge_reply->mod_objs[1].
       mod_recs[cmcnt].pk_values)
      SET cm_request->charge_mod[cmcnt].charge_mod_type_cd = dsuspensecd
      SET cm_request->charge_mod[cmcnt].field1_id = dsuspensersn
      SET cm_request->charge_mod[cmcnt].field6 = uar_get_code_display(dsuspensersn)
      SET cm_request->charge_mod_qual += 1
    ENDFOR
   ENDIF
   IF ((((request->suspense_rsn_cd > 0)) OR (trim(request->reason_comment) != ""))
    AND 13019_mod_rsn_cd > 0
    AND (((chargefind_reply->charge_items[1].item_interval_id <= 0)) OR (itc <= 0
    AND (request->item_quantity > 0))) )
    CALL logmsg(curprog,"Adding modify reason to addchargemodreq",log_debug)
    SET cmcnt = (cm_request->charge_mod_qual+ 1)
    SET cm_request->charge_mod_qual = cmcnt
    SET stat = alterlist(cm_request->charge_mod,cmcnt)
    SET cm_request->charge_mod[cmcnt].action_type = "ADD"
    SET cm_request->charge_mod[cmcnt].charge_item_id = cnvtreal(addcharge_reply->mod_objs[1].
     mod_recs[1].pk_values)
    SET cm_request->charge_mod[cmcnt].charge_mod_type_cd = 13019_mod_rsn_cd
    IF (nincreaseind=1)
     SET cm_request->charge_mod[cmcnt].field1_id = 4001989_increase_cd
    ELSE
     SET cm_request->charge_mod[cmcnt].field1_id = 4001989_adjust_cd
    ENDIF
    SET cm_request->charge_mod[cmcnt].field2_id = request->suspense_rsn_cd
    IF (nincreaseind=1)
     SET cm_request->charge_mod[cmcnt].field6 = "The quantity was increased"
    ELSE
     SET cm_request->charge_mod[cmcnt].field6 = "The charge was adjusted"
    ENDIF
    SET cm_request->charge_mod[cmcnt].field7 = request->reason_comment
    IF (nincreaseind=1)
     SET cm_request->charge_mod[cmcnt].field8 = concat("The quantity was increased from ",trim(
       cnvtstring(dquantity))," to ",trim(cnvtstring(request->item_quantity)))
    ENDIF
    SET cm_request->charge_mod[cmcnt].activity_dt_tm = cnvtdatetime(sysdate)
   ENDIF
   SET cm_reply->status_data.status = "Z"
   SET action_begin = 1
   SET action_end = cm_request->charge_mod_qual
   CALL echo("Executing AFC_ADD_CHARGE_MOD to add the suspense reason")
   CALL echorecord(cm_request)
   CALL logmsg(curprog,"Executing AFC_ADD_CHARGE_MOD to add the suspense reason",log_debug)
   EXECUTE afc_add_charge_mod  WITH replace("REQUEST",cm_request), replace("REPLY",cm_reply)
   CALL echorecord(cm_reply)
   IF ((cm_reply->status_data.status="F"))
    CALL echo(build("Add Charge Mod Reply Status is ",cm_reply->status_data.status," Ending Program")
     )
    CALL logmsg(curprog,build("Add Charge Mod Reply Status is ",cm_reply->status_data.status,
      " Ending Program"),log_debug)
    SET reply->status_data.status = "F"
    GO TO end_program
   ENDIF
  ENDIF
  IF ((((chargefind_reply->charge_items[1].item_interval_id > 0)
   AND (request->item_quantity > 0)) OR (itc > 0
   AND (request->item_quantity > 0))) )
   CALL echo("Beginning Synchronous Release")
   CALL logmsg(curprog,"Beginning Synchronous Release",log_debug)
   SET reproc_request->process_type_cd = dsuspensecd
   FOR (nreprocessrequest = 1 TO size(addcharge_reply->mod_objs[1].mod_recs,5))
   ...
   IF (size(reproc_request->process_event,5) > 0)
    ...
    IF (crmstatus=0)
     ...
     IF (crmstatus=0)
      ...
      IF (crmstatus=0)
       IF (hreq=0)
        CALL echo("Failure on begin request")
       ELSE
        SET hrequest = uar_crmgetrequest(hreq)
        IF (hrequest=0)
        ...
        ELSE
         ...
         SET stat = uar_srvsetdouble(hrequest,"process_type_cd",dreprocesscd)
         FOR (nreprocessloop = 1 TO size(reproc_request->process_event,5))
           ...
           FOR (nreprocessloop2 = 1 TO size(reproc_request->process_event[nreprocessloop].charge_item,
            5))
       ...
       IF (crmstatus=0)
        CALL echo(build("Synchronous Perform Succeeded, CRM Status is  ",crmstatus))
        CALL logmsg(curprog,build("Synchronous Perform Succeeded, CRM Status is  ",crmstatus),
         log_debug)
        SET hreply = uar_crmgetreply(hreq)
        IF (hreply > 0)
        ...
        ENDIF
       ELSE
        CALL echo(build("Synchronous Perform Failed, CRM Status is ",crmstatus))
        CALL logmsg(curprog,build("Synchronous Perform Failed, CRM Status is ",crmstatus),log_debug)
        GO TO end_program
       ENDIF
       CALL uar_crmendreq(hreq)
      ELSE
       CALL logmsg(curprog,"Failure on BeginReq",log_debug)
      ENDIF
      CALL uar_crmendtask(htask)
     ELSE
      CALL logmsg(curprog,"Failure on BeginTask",log_debug)
     ENDIF
     CALL uar_crmendapp(happ)
    ELSE
     CALL logmsg(curprog,"Failure on BeginApp",log_debug)
    ENDIF
   ENDIF
   CALL echo("End Synchronous Release")
   CALL logmsg(curprog,"End Synchronous Release",log_debug)
   CALL echo("Checking to see if any interval charges need to be inactivated")
   CALL logmsg(curprog,"Checking to see if any interval charges need to be inactivated",log_debug)
   FREE SET cm_request
   RECORD cm_request(
     1 charge_mod_qual = i2
     1 charge_mod[*]
   ...
   IF ((((request->suspense_rsn_cd > 0)) OR (trim(request->reason_comment) != ""))
    AND 13019_mod_rsn_cd > 0)
    CALL logmsg(curprog,"Adding modify reason to the now release interval charge(s)",log_debug)
    FOR (lloopcount = 1 TO size(syncrelease_reply->charge_items,5))
      SET cmcnt = lloopcount
      SET cm_request->charge_mod_qual = cmcnt
      SET stat = alterlist(cm_request->charge_mod,cmcnt)
      SET cm_request->charge_mod[cmcnt].action_type = "ADD"
      SET cm_request->charge_mod[cmcnt].charge_item_id = syncrelease_reply->charge_items[cmcnt].
      charge_item_id
      SET cm_request->charge_mod[cmcnt].charge_mod_type_cd = 13019_mod_rsn_cd
      IF (nincreaseind=1)
       SET cm_request->charge_mod[cmcnt].field1_id = 4001989_increase_cd
    ...
    SET action_begin = 1
    SET action_end = cm_request->charge_mod_qual
    CALL echo("Executing AFC_ADD_CHARGE_MOD to add modify reason to now released interval charge")
    CALL echorecord(cm_request)
    CALL logmsg(curprog,
     "Executing AFC_ADD_CHARGE_MOD to add modify reason to now release interval charge",log_debug)
    EXECUTE afc_add_charge_mod  WITH replace("REQUEST",cm_request), replace("REPLY",cm_reply)
    CALL echorecord(cm_reply)
    IF ((cm_reply->status_data.status="F"))
     CALL echo(build("Add Charge Mod Reply Status is ",cm_reply->status_data.status," Ending Program"
   ...
   ENDIF
   IF ((((chargefind_reply->charge_items[1].item_interval_id > 0)) OR (itc > 0)) )
    FOR (loopcnt = 1 TO size(addcharge_request->objarray,5))
      SELECT INTO "nl:"
       FROM charge c,
        item_interval_table iit,
        interval_table it
       PLAN (c
        WHERE (c.charge_item_id=addcharge_request->objarray[loopcnt].charge_item_id))
        JOIN (iit
        WHERE iit.item_interval_id=c.item_interval_id)
        JOIN (it
        WHERE it.interval_id=iit.interval_id)
       DETAIL
        begvalue = it.beg_value, utc = it.unit_type_cd
       WITH nocounter
      ;end select
      CALL echo(build("Beginning Value:  ",begvalue))
      CALL echo(build("Unit Type Code: ",utc))
      CALL logmsg(curprog,build("Unit Type Code: ",utc),log_debug)
      SELECT INTO "nl:"
       FROM code_value_extension cve
       ...
       WITH nocounter
      ;end select
      CALL echo(build("Field Value: ",fieldvalue))
      CALL logmsg(curprog,build("Field Value: ",fieldvalue),log_debug)
      IF (fieldvalue=0)
       SET fieldvalue = 1
       CALL echo("Field Value = 0, setting Field Value to 1")
       CALL logmsg(curprog,"Field Value = 0, setting Field Value to 1",log_debug)
      ENDIF
      SET iresult = ceil((request->item_quantity/ fieldvalue))
      CALL echo(build("iResult:  ",iresult))
      IF (iresult < begvalue)
       UPDATE  FROM charge c
        ...
        WHERE (charge_item_id=addcharge_request->objarray[loopcnt].charge_item_id)
       ;end update
       CALL echo("SETTING ACTIVE IND TO 0 FOR CHARGE THAT NO LONGER QUALIFIES")
      ELSE
       IF ((addcharge_flag->flags[loopcnt].post_flag=1))
        SET pftind += 1
        SET stat = alterlist(afcprofit_request->charges,pftind)
        SET afcprofit_request->charges[pftind].charge_item_id = addcharge_request->objarray[loopcnt].
        charge_item_id
       ENDIF
      ENDIF
    ENDFOR
  ...
  IF (size(afcprofit_request->charges,5) > 0)
   CALL echo("Executing pft_nt_chrg_billing")
   CALL echorecord(afcprofit_request)
   CALL logmsg(curprog,"Executing pft_nt_chrg_billing",log_debug)
   COMMIT
 ...
 IF ((chargefind_reply->charge_items[1].item_interval_id <= 0)
  ...
  CALL echorecord(addcharge_reply)
  IF (reprocessind=0)
   CALL getoldchargeitemdetails(request->charge_item_id)
  ENDIF
  CALL echo("Item_Interval_ID <=0 and Interval_Template_Cd <= 0")
  CALL logmsg(curprog,"Item_Interval_ID <=0 and Interval_Template_Cd <= 0",log_debug)
  IF ((request->charge_description != chargefind_reply->charge_items[1].charge_description)
   AND (request->item_quantity=chargefind_reply->charge_items[1].item_quantity))
   CALL echo(
    "Charge description has changed, quantity has not...updating original charge description")
   CALL logmsg(curprog,
    "Charge description has changed, quantity has not...updating original charge description",
    log_debug)
   UPDATE  FROM charge c
    SET c.charge_description = request->charge_description, c.process_flg = 0
    WHERE (c.charge_item_id=
    IF ((chargefind_reply->charge_items[1].process_flg=0)) request->charge_item_id
    ELSE cnvtreal(addcharge_reply->mod_objs[1].mod_recs[1].pk_values)
    ENDIF
    )
   ;end update
   IF ((chargefind_reply->charge_items[1].process_flg != interfaced)
    AND (chargefind_reply->charge_items[1].process_flg != posted))
    IF ( NOT (chargeupdatedind))
     UPDATE  FROM charge c
      SET c.updt_cnt = (c.updt_cnt+ 1)
      WHERE (c.charge_item_id=request->charge_item_id)
     ;end update
     SET chargeupdatedind = true
    ENDIF
   ENDIF
  ELSEIF ((((request->charge_description != chargefind_reply->charge_items[1].charge_description)
   AND (request->item_quantity != chargefind_reply->charge_items[1].item_quantity)) OR ((request->
  item_quantity != chargefind_reply->charge_items[1].item_quantity))) )
   CALL echo(
    "Charge description and quantity changed or just quantity changed...updating new charge description"
    )
   CALL logmsg(curprog,
    "Charge description and quantity changed or just quantity changed...updating new charge description",
    log_debug)
   UPDATE  FROM charge c
    SET c.charge_description = request->charge_description, c.process_flg = 0, c.item_price = request
     ->item_price,
     c.item_quantity = request->item_quantity, c.item_extended_price = (request->item_quantity *
     request->item_price)
    WHERE c.charge_item_id=cnvtreal(addcharge_reply->mod_objs[1].mod_recs[1].pk_values)
   ;end update
  ENDIF
  IF ((request->item_price != chargefind_reply->charge_items[1].item_price)
   AND (request->item_quantity=chargefind_reply->charge_items[1].item_quantity))
   CALL echo(
    "Item price changed......quantity did not....updating item price and item extended price")
   CALL logmsg(curprog,
    "Item price changed......quantity did not....updating item price and item extended price",
    log_debug)
   UPDATE  FROM charge c
    SET c.item_price = request->item_price, c.item_extended_price = (request->item_price * request->
     item_quantity), c.process_flg = 0
    WHERE (c.charge_item_id=
    IF ((((chargefind_reply->charge_items[1].process_flg=interfaced)) OR ((chargefind_reply->
    charge_items[1].process_flg=posted))) ) cnvtreal(addcharge_reply->mod_objs[1].mod_recs[1].
      pk_values)
    ELSE request->charge_item_id
    ENDIF
    )
   ;end update
   IF ((chargefind_reply->charge_items[1].process_flg != interfaced)
  ...
  ENDIF
  IF (reprocessind=0)
   SET changelog->charge_qual = 1
   SET changelog->charge[1].charge_item_id = request->charge_item_id
   SET changelog->charge[1].charge_description = request->charge_description
   SET changelog->charge[1].item_quantity = request->item_quantity
   SET changelog->charge[1].item_price = request->item_price
   CALL savechargechangelogattributes(changelog)
  ELSEIF (reprocessind=1)
   CALL savechangelogdetails(request->charge_item_id,addcharge_request->objarray[1].charge_item_id)
  ENDIF
  IF ((chargefind_reply->charge_items[1].process_flg=interfaced))
   SELECT INTO "nl:"
    FROM interface_file i
    ...
    DETAIL
     rtind2 = i.realtime_ind, ifi = i.interface_file_id
    WITH nocounter
   ;end select
   CALL echo(build("Realtime Ind = ",rtind2," for Interface File ID: ",ifi))
   CALL logmsg(curprog,build("Realtime Ind = ",rtind2," for Interface File ID: ",ifi),log_debug)
   IF (rtind2=1)
    ...
    SET publishupdatedchargerequest->charges[1].charge_item_id = afcinterfacecharge_request->
    interface_charge[1].charge_item_id
   ENDIF
  ELSEIF ((chargefind_reply->charge_items[1].process_flg=posted))
   SET stat = alterlist(afcprofit_request->charges,2)
   SET afcprofit_request->charges[1].charge_item_id = addcredit_reply->charge[1].charge_item_id
   SET afcprofit_request->charges[2].charge_item_id = cnvtreal(addcharge_reply->mod_objs[1].mod_recs[
    1].pk_values)
   CALL echo("Executing pft_nt_chrg_billing")
   CALL echorecord(afcprofit_request)
   CALL logmsg(curprog,"Executing pft_nt_chrg_billing",log_debug)
   ...
   ENDIF
   SET stat = alterlist(publishupdatedchargerequest->charges,1)
   SET publishupdatedchargerequest->charges[1].charge_item_id = afcprofit_request->charges[2].
   charge_item_id
  ENDIF
  IF (size(publishupdatedchargerequest->charges,5) > 0)
   IF ( NOT (publisheventforchargeupdate(publishupdatedchargerequest,chargefind_reply->charge_items[1
    ].process_flg)))
    CALL logmsg(curprog,"Failed to publish the Charge Update Event",log_debug)
   ENDIF
  ENDIF
  IF (size(publishupdatedchargerequest->charges,5) > 0)
   IF ( NOT (publishunbilledinvoiceworkitem(publishupdatedchargerequest)))
    CALL logmsg(curprog,"Error Publishing WorkItem for the invoice",log_error)
   ENDIF
  ENDIF
 ENDIF
 SUBROUTINE get_all_related_charges(cc)
   CALL echo("Entering sub-routine get_all_related_charges")
   CALL logmsg(curprog,"Entering sub-routine get_all_related_charges",log_debug)
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     CALL echo(build("Parent_entity_id: ",peid))
    WITH nocounter
   ;end select
   CALL logmsg(curprog,build("Parent_entity_id: ",peid),log_debug)
   CALL echo("Getting interval charges related to primary charge")
   CALL logmsg(curprog,"Getting interval charges related to primary charge",log_debug)
   SET ccount = 0
   SELECT INTO "nl:"
    FROM item_interval_table iit,
     charge c
    PLAN (iit
     WHERE iit.parent_entity_name="PRICE_SCHED_ITEMS"
      AND iit.parent_entity_id=peid
      AND iit.active_ind=1
      AND (iit.item_interval_id != chargefind_reply->charge_items[1].item_interval_id))
     JOIN (c
    ...
    DETAIL
     ccount += 1, stat = alterlist(intervalcharges->charge_items,ccount), intervalcharges->
     charge_items[ccount].charge_item_id = c.charge_item_id,
     intervalcharges->charge_items[ccount].parent_charge_item_id = c.parent_charge_item_id,
     intervalcharges->charge_items[ccount].charge_event_act_id = c.charge_event_act_id,
     intervalcharges->charge_items[ccount].charge_event_id = c.charge_event_id,
     intervalcharges->charge_items[ccount].bill_item_id = c.bill_item_id, intervalcharges->
     charge_items[ccount].order_id = c.order_id, intervalcharges->charge_items[ccount].encntr_id = c
     .encntr_id,
     intervalcharges->charge_items[ccount].person_id = c.person_id, intervalcharges->charge_items[
     ccount].payor_id = c.payor_id, intervalcharges->charge_items[ccount].ord_loc_cd = c.ord_loc_cd,
     intervalcharges->charge_items[ccount].perf_loc_cd = c.perf_loc_cd, intervalcharges->
     charge_items[ccount].ord_phys_id = c.ord_phys_id, intervalcharges->charge_items[ccount].
     perf_phys_id = c.perf_phys_id,
     intervalcharges->charge_items[ccount].charge_description = c.charge_description, intervalcharges
     ->charge_items[ccount].price_sched_id = c.price_sched_id, intervalcharges->charge_items[ccount].
     item_quantity = c.item_quantity,
     intervalcharges->charge_items[ccount].item_price = c.item_price, intervalcharges->charge_items[
     ccount].item_extended_price = c.item_extended_price, intervalcharges->charge_items[ccount].
     item_allowable = c.item_allowable,
     intervalcharges->charge_items[ccount].item_copay = c.item_copay, intervalcharges->charge_items[
     ccount].charge_type_cd = c.charge_type_cd, intervalcharges->charge_items[ccount].
     research_acct_id = c.research_acct_id,
     intervalcharges->charge_items[ccount].suspense_rsn_cd = c.suspense_rsn_cd, intervalcharges->
     charge_items[ccount].reason_comment = c.reason_comment, intervalcharges->charge_items[ccount].
     posted_cd = c.posted_cd,
     intervalcharges->charge_items[ccount].posted_dt_tm = c.posted_dt_tm, intervalcharges->
     charge_items[ccount].process_flg = c.process_flg, intervalcharges->charge_items[ccount].
     service_dt_tm = c.service_dt_tm,
     intervalcharges->charge_items[ccount].activity_dt_tm = c.activity_dt_tm, intervalcharges->
     charge_items[ccount].updt_cnt = c.updt_cnt, intervalcharges->charge_items[ccount].updt_dt_tm = c
     .updt_dt_tm,
     intervalcharges->charge_items[ccount].updt_id = c.updt_id, intervalcharges->charge_items[ccount]
     .updt_task = c.updt_task, intervalcharges->charge_items[ccount].updt_applctx = c.updt_applctx,
     intervalcharges->charge_items[ccount].active_status_dt_tm = c.active_status_dt_tm,
     intervalcharges->charge_items[ccount].active_status_prsnl_id = c.active_status_prsnl_id,
     intervalcharges->charge_items[ccount].active_ind = c.active_ind,
     intervalcharges->charge_items[ccount].active_status_cd = c.active_status_cd, intervalcharges->
     charge_items[ccount].beg_effective_dt_tm = c.beg_effective_dt_tm, intervalcharges->charge_items[
     ccount].end_effective_dt_tm = c.end_effective_dt_tm,
     intervalcharges->charge_items[ccount].credited_dt_tm = c.credited_dt_tm, intervalcharges->
     charge_items[ccount].adjusted_dt_tm = c.adjusted_dt_tm, intervalcharges->charge_items[ccount].
     interface_file_id = c.interface_file_id,
     intervalcharges->charge_items[ccount].tier_group_cd = c.tier_group_cd, intervalcharges->
     charge_items[ccount].def_bill_item_id = c.def_bill_item_id, intervalcharges->charge_items[ccount
     ].verify_phys_id = c.verify_phys_id,
     intervalcharges->charge_items[ccount].gross_price = c.gross_price, intervalcharges->
     charge_items[ccount].discount_amount = c.discount_amount, intervalcharges->charge_items[ccount].
     manual_ind = c.manual_ind,
     intervalcharges->charge_items[ccount].combine_ind = c.combine_ind, intervalcharges->
     charge_items[ccount].activity_type_cd = c.activity_type_cd, intervalcharges->charge_items[ccount
     ].activity_sub_type_cd = c.activity_sub_type_cd,
     intervalcharges->charge_items[ccount].provider_specialty_cd = c.provider_specialty_cd,
     intervalcharges->charge_items[ccount].admit_type_cd = c.admit_type_cd, intervalcharges->
     charge_items[ccount].bundle_id = c.bundle_id,
     intervalcharges->charge_items[ccount].department_cd = c.department_cd, intervalcharges->
     charge_items[ccount].institution_cd = c.institution_cd, intervalcharges->charge_items[ccount].
     level5_cd = c.level5_cd,
     intervalcharges->charge_items[ccount].med_service_cd = c.med_service_cd, intervalcharges->
     charge_items[ccount].section_cd = c.section_cd, intervalcharges->charge_items[ccount].
     subsection_cd = c.subsection_cd,
     intervalcharges->charge_items[ccount].abn_status_cd = c.abn_status_cd, intervalcharges->
     charge_items[ccount].cost_center_cd = c.cost_center_cd, intervalcharges->charge_items[ccount].
     inst_fin_nbr = c.inst_fin_nbr,
     intervalcharges->charge_items[ccount].fin_class_cd = c.fin_class_cd, intervalcharges->
     charge_items[ccount].health_plan_id = c.health_plan_id, intervalcharges->charge_items[ccount].
     item_interval_id = c.item_interval_id,
     intervalcharges->charge_items[ccount].item_list_price = c.item_list_price, intervalcharges->
     charge_items[ccount].item_reimbursement = c.item_reimbursement, intervalcharges->charge_items[
     ccount].list_price_sched_id = c.list_price_sched_id,
     intervalcharges->charge_items[ccount].payor_type_cd = c.payor_type_cd, intervalcharges->
     charge_items[ccount].epsdt_ind = c.epsdt_ind, intervalcharges->charge_items[ccount].ref_phys_id
      = c.ref_phys_id,
     intervalcharges->charge_items[ccount].start_dt_tm = c.start_dt_tm, intervalcharges->
     charge_items[ccount].stop_dt_tm = c.stop_dt_tm, intervalcharges->charge_items[ccount].
     alpha_nomen_id = c.alpha_nomen_id,
     intervalcharges->charge_items[ccount].server_process_flag = c.server_process_flag,
     intervalcharges->charge_items[ccount].offset_charge_item_id = c.offset_charge_item_id,
     intervalcharges->charge_items[ccount].item_deductible_amt = c.item_deductible_amt,
     intervalcharges->charge_items[ccount].patient_responsibility_flag = c
     .patient_responsibility_flag
    WITH nocounter
   ;end select
   CALL echorecord(intervalcharges)
   CALL echo("Leaving subroutine get_all_related_charges")
   CALL logmsg(curprog,"Leaving subroutine get_all_related_charges",log_debug)
 END ;Subroutine
 SET reply->status_data.status = "S"
 SET reqinfo->commit_ind = 1
#end_program
 CALL echo("Freeing record sets")
 CALL logmsg(curprog,"Freeing record sets",log_debug)
 FREE SET chargefind_request
 FREE SET chargefind_reply
 FREE SET chargeeventfind_request
 FREE SET chargeeventfind_reply
 FREE SET cea_record
 FREE SET intervalcharges
 ...
 FREE SET cm_request
 FREE SET cm_reply
 FREE SET reproc_request
 FREE SET syncrelease_reply
 FREE SET afcprofit_request
 FREE SET afcprofit_reply
 FREE SET afcinterfacecharge_request
 FREE SET afcinterfacecharge_reply
 FREE SET addcharge_flag
