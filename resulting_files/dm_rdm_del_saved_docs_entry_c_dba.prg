CREATE PROGRAM dm_rdm_del_saved_docs_entry_c:dba
 ...
 IF (((error(ms_errmsg,0) != 0) OR (task_activity_cd_saved_doc <= 0)) )
  ...
  GO TO exit_program
 ENDIF
 SET ms_child_info_domain_nm = "DM_RDM_DEL_SAVED_DOCS_ENTRY_MODE_CODE"
 ...
 IF (sbr_fetch_min_max(ms_child_info_domain_nm,ms_range_prefix,ms_child_max_name,mf_max_range_id,
  ...
  GO TO exit_program
 ENDIF
 IF (sbr_sync_ccl_tmp_def("TEMP_ENTRYMODE_TASK_ACTIVITY")=0)
  GO TO exit_program
 ENDIF
...
#exit_program
 IF ((readme_data->status="F"))
  ROLLBACK
 ELSE
  CALL sbr_truncate_table("TEMP_ENTRYMODE_TASK_ACTIVITY")
 ENDIF
