CREATE PROGRAM ch_submit_beta:dba
 RECORD reply(
   1 qual[1]
     2 line = c132
 ...
 SELECT DISTINCT INTO value(outfile)
  ...
  ORDER BY cr.request_dt_tm, cr.dist_run_dt_tm, cr.dist_run_type_cd,
   cr.chart_request_id
  HEAD REPORT
   line1 = fillstring(124,"*"), pat_count = 0, last_pat = 0,
   dist_count = 0, row 1, tempstr = uar_i18ngetmessage(i18nhandle,"DIST_JOURNAL_SUMMARY",
    "D I S T R I B U T I O N  J O U R N A L"),
   CALL center(tempstr,0,125), tempstr = uar_i18ngetmessage(i18nhandle,"PAGE","PAGE:"), col 110,
  ...
  HEAD PAGE
   IF (curpage > 1)
    row 1, tempstr = uar_i18ngetmessage(i18nhandle,"PAGE","PAGE:"), col 110,
    tempstr, col + 2, curpage"###",
    row + 1
   ENDIF
  ...
  HEAD cr.dist_run_type_cd
   ...
   CALL print(substring(1,50,cd.dist_descr)), tempstr = uar_i18ngetmessage(i18nhandle,
    "DIST_RUN_DATE_TIME","Run Date/Time:"), col 80,
   tempstr, col + 2,
   ...
   tempstr = uar_i18ngetmessage(i18nhandle,"DIST_RUN_TYPE","Run Type:"), col 80, tempstr,
   col + 2, dist_run_type, row + 2,
   tempstr = uar_i18ngetmessage(i18nhandle,"PATIENT_NAME","NAME"), col 1, tempstr,
   tempstr = uar_i18ngetmessage(i18nhandle,"PRINTER","PRINTER"), col 27, tempstr,
   tempstr = uar_i18ngetmessage(i18nhandle,"PAGES","PAGES"), col 38, tempstr,
   tempstr = uar_i18ngetmessage(i18nhandle,"PATIENT_TYPE","PT. TYPE"), col 44, tempstr,
   col 60
   IF (cr.scope_flag=1)
    tempstrperson
   ...
   ELSEIF (cr.scope_flag=3)
    tempstrorder
   ENDIF
   tempstr = uar_i18ngetmessage(i18nhandle,"ORGANIZATION","ORG"), col 81, tempstr,
   tempstr = uar_i18ngetmessage(i18nhandle,"UNIT_ROOM_BED","UNIT/ROOM/BED"), col 97, tempstr,
   row + 1, last_pat = p.person_id
  DETAIL
   pat_count = (pat_count+ 1), pat_dist_count = (pat_dist_count+ 1), row + 1,
   col 1,
   CALL print(substring(1,25,p.name_full_formatted)), col 27,
   CALL print(substring(1,10,output_description)), col 38,
   CALL print(pages),
   col 44,
   CALL print(substring(1,15,patient_type)), col 60
   IF (cr.scope_flag=1)
   ...
   ENDIF
   col 81,
   CALL print(substring(1,15,o.org_name)), col 97,
   CALL print(concat(trim(substring(1,12,nurse_unit)),"/",trim(substring(1,10,room)),"/",trim(
  ...
  FOOT REPORT
   ...
   row + 1
  WITH outerjoin = d1, outerjoin = d2, compress
 ;end select
