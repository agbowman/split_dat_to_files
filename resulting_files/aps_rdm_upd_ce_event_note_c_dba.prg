CREATE PROGRAM aps_rdm_upd_ce_event_note_c:dba
 ...
 ENDIF
 SUBROUTINE (xntr_append_where(xntr_table_name=vc,xntr_alias=vc) =vc)
 ...
 SUBROUTINE xntr_bypass_readme_status(null)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (xntr_add_metadata(xntr_readme_id=f8,xntr_instance=i4,xntr_table_name=vc) =i2)
   ...
   IF (xntr_dxrt_exists_ind=0)
    INSERT  FROM dm_xntr_readme_table dxrt
     SET dxrt.dm_xntr_readme_table_id = seq(dm_clinical_seq,nextval), dxrt.readme_id = xntr_readme_id,
      ...
      dxrt.updt_dt_tm = cnvtdatetime(sysdate), dxrt.updt_id = reqinfo->updt_id, dxrt.updt_task =
      reqinfo->updt_task
     WITH nocounter
    ;end insert
    IF (error(xntr_dxrt_errmsg,0) != 0)
 ...
 IF (xntr_add_metadata(readme_data->readme_id,readme_data->instance,"CE_EVENT_NOTE") != 0)
  GO TO exit_program
 ENDIF
 ...
 SET ms_child_info_domain_nm = "aps_rdm_ce_event_note_update"
 SET ms_range_prefix = "ce_event_note range"
 SET ms_child_max_name = "MAX ce_event_note_id EVALUATED"
 SET ms_child_script = "aps_rdm_upd_ce_event_note_cc"
 SET mf_min_range_id = xntr_alt_min_id(null)
 IF (mf_min_range_id != xntr_default_id)
  SET mf_max_range_id = xntr_alt_max_id(null)
 ELSE
