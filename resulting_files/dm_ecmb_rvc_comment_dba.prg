CREATE PROGRAM dm_ecmb_rvc_comment:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_RVC_COMMENT"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = frm.rvc_comment_id, rreclist->from_rec[v_cust_count1].
 ...
 IF (v_cust_count1 > 0)
  FOR (v_cust_loopcount1 = 1 TO v_cust_count1)
    CALL upt_from(rreclist->from_rec[v_cust_loopcount1].from_id,request->xxx_combine[icombine].
     to_xxx_id)
  ENDFOR
  SET stat = alterlist(rcommententitylist->entity_list,1)
  SET rcommententitylist->entity_list[1].encntr_ind = 1
  SET rcommententitylist->entity_list[1].parent_entity_id = request->xxx_combine[icombine].to_xxx_id
  CALL rvc_comment_write_comment_for_entity(null)
 ENDIF
 SUBROUTINE upt_from(s_uf_pk_id,s_uf_to_fk_id)
   UPDATE  FROM rvc_comment frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_task = reqinfo->
     updt_task,
     frm.updt_dt_tm = cnvtdatetime(sysdate), frm.parent_entity_id = s_uf_to_fk_id
    WHERE frm.rvc_comment_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "RVC_COMMENT"
