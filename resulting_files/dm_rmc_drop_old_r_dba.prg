CREATE PROGRAM dm_rmc_drop_old_r:dba
 DECLARE dmrmc_drop_r(r_tab_name=vc) = null
 ...
 SUBROUTINE create_r_only(cro_tab_name,cro_r_tab_name,cro_trig_ind,cro_remove_lock_ind)
   ...
   RETURN(cro_return)
 END ;Subroutine
 FREE RECORD dmrmc_drop_r_list
 RECORD dmrmc_drop_r_list(
   1 list[*]
     2 table_name = vc
     2 table_created = f8
 )
 DECLARE dmrmc_r_cnt = i4
 DECLARE dmrmc_looper = i4
 IF ((validate(request->drop_after_day,- (1))=- (1)))
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dmrmc_r_cnt = (dmrmc_r_cnt+ 1)
   IF (mod(dmrmc_r_cnt,500)=1)
    stat = alterlist(dmrmc_drop_r_list->list,(dmrmc_r_cnt+ 499))
   ENDIF
   dmrmc_drop_r_list->list[dmrmc_r_cnt].table_name = ut.table_name, dmrmc_drop_r_list->list[
   dmrmc_r_cnt].table_created = uo.created
  FOOT REPORT
   stat = alterlist(dmrmc_drop_r_list->list,dmrmc_r_cnt)
  WITH nocounter
 ;end select
 IF (check_error("Get the list of $R tables") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FOR (dmrmc_looper = 1 TO dmrmc_r_cnt)
   SELECT INTO "nl:"
    FROM (parser(dmrmc_drop_r_list->list[dmrmc_looper].table_name) d)
    WHERE d.rdds_status_flag < 9000
    WITH nocounter, maxqual(d,1)
   ;end select
   IF (check_error("Check for uncut data in $R") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF (curqual=0)
    SELECT INTO "nl:"
     FROM (parser(dmrmc_drop_r_list->list[dmrmc_looper].table_name) d)
     WHERE d.rdds_dt_tm > cnvtdatetime((curdate - request->drop_after_day),curtime3)
     WITH nocounter, maxqual(d,1)
    ;end select
    IF (check_error("Check for recent data in $R") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    IF (curqual=0)
     CALL dmrmc_drop_r(dmrmc_drop_r_list->list[dmrmc_looper].table_name)
    ENDIF
   ENDIF
 ENDFOR
 SUBROUTINE dmrmc_drop_r(r_tab_name)
   DECLARE dmrmc_live_t_name = vc
   DECLARE dmrmc_drop_ret = vc
   SET dmrmc_live_t_name = get_reg_tab_name(r_tab_name,"")
   SET dmrmc_drop_ret = drop_$r_tab(dmrmc_live_t_name)
   IF (dmrmc_drop_ret="F")
    SET dm_err->emsg = "Could not drop qualifying old $R"
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
