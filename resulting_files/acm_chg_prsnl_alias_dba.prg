CREATE PROGRAM acm_chg_prsnl_alias:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->prsnl_alias_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
   prsnl_alias p
  SET p.prsnl_alias_id = acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].prsnl_alias_id, p.alias
    = trim(acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].alias,3), p.alias_pool_cd =
   acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].alias_pool_cd,
   p.beg_effective_dt_tm = cnvtdatetime(acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].
    beg_effective_dt_tm), p.contributor_system_cd =
   IF ((acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0)) acm_request
    ->prsnl_alias_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , p.end_effective_dt_tm = cnvtdatetime(acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].
    end_effective_dt_tm),
   p.person_id = acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].person_id, p
   .prsnl_alias_sub_type_cd = acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].
   prsnl_alias_sub_type_cd, p.prsnl_alias_type_cd = acm_request->prsnl_alias_qual[xref->chg[d.seq].
   idx].prsnl_alias_type_cd,
   p.active_ind = acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].active_ind, p.active_status_cd
    = acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].active_status_cd, p.active_status_prsnl_id
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.prsnl_alias_id=acm_request->prsnl_alias_qual[xref->chg[d.seq].idx].prsnl_alias_id))
  WITH nocounter, status(reply->prsnl_alias_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->prsnl_alias_qual[xref->chg[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM prsnl_alias p
    WHERE expand(t1,f_val,t_val,p.prsnl_alias_id,acm_request->prsnl_alias_qual[xref->chg[t1].idx].
     prsnl_alias_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.prsnl_alias_id,acm_request->prsnl_alias_qual[xref->chg[t1].idx].
      prsnl_alias_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((p.updt_cnt=acm_request->prsnl_alias_qual[idx].updt_cnt)) OR ((acm_request->force_updt_ind
     =1))) )
      reply->prsnl_alias_qual[idx].status = - (1), reply->prsnl_alias_qual[idx].prsnl_alias_id =
      acm_request->prsnl_alias_qual[idx].prsnl_alias_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->prsnl_alias_qual[idx].chg_str
     IF (findstring("ALIAS,",chg_str)=0)
      acm_request->prsnl_alias_qual[idx].alias = p.alias
     ENDIF
     IF (findstring("ALIAS_POOL_CD,",chg_str)=0)
      acm_request->prsnl_alias_qual[idx].alias_pool_cd = p.alias_pool_cd
     ENDIF
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_alias_qual[idx].beg_effective_dt_tm = p.beg_effective_dt_tm
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->prsnl_alias_qual[idx].contributor_system_cd = p.contributor_system_cd
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->prsnl_alias_qual[idx].end_effective_dt_tm = p.end_effective_dt_tm
     ENDIF
     IF (findstring("PERSON_ID,",chg_str)=0)
      acm_request->prsnl_alias_qual[idx].person_id = p.person_id
     ENDIF
     IF (findstring("PRSNL_ALIAS_SUB_TYPE_CD,",chg_str)=0)
      acm_request->prsnl_alias_qual[idx].prsnl_alias_sub_type_cd = p.prsnl_alias_sub_type_cd
     ENDIF
     IF (findstring("PRSNL_ALIAS_TYPE_CD,",chg_str)=0)
      acm_request->prsnl_alias_qual[idx].prsnl_alias_type_cd = p.prsnl_alias_type_cd
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->prsnl_alias_qual[idx].active_ind = p.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->prsnl_alias_qual[idx].active_status_cd = p.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_alias_qual[xref->chg[index].idx].status=0))
