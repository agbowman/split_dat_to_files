CREATE PROGRAM cp_add_pathway:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Pathway Name:" = "",
  "Pathway Type CODE_VALUE:" = "",
  "Pathway Status CDF:" = "",
  "Entry Node ID:" = 0.0,
  "Concept Code Value:" = 0.0,
  "Owner Prsnl ID" = 0.0
  WITH outdev, pname, ptype,
  pstat, enid, ccd,
  prsnlid
 FREE RECORD reply
 RECORD reply(
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "CP_ADD_PATHWAY"
 DECLARE verifyinputs(null) = null WITH protect
 DECLARE insertrows(null) = i4 WITH protect
 DECLARE lstat = i4 WITH protect, noconstant(0)
 DECLARE pw_name = vc WITH protect, constant(trim( $PNAME))
 DECLARE entry_node_id = f8 WITH protect, constant( $ENID)
 DECLARE pw_type_cd = f8 WITH protect, constant( $PTYPE)
 DECLARE pw_status_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003198, $PSTAT))
 DECLARE concept_cd = f8 WITH protect, constant( $CCD)
 DECLARE owner_prsnl_id = f8 WITH protect, constant( $PRSNLID)
 ...
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE pathway_type_cs = i4 WITH protect, constant(4003197)
   DECLARE pathway_status_cs = i4 WITH protect, constant(4003198)
   DECLARE concept_cd_cs = i4 WITH protect, constant(4003132)
   CALL echo(build("Verify pathway_name is valid: ",trim( $PNAME)))
   IF (size(trim( $PNAME,3),1)=0)
    SET reply->status_data.status = "F"
    ...
    SET reply->status_data.subeventstatus.targetobjectvalue = build2(
     "Pathway Name must be valid string with length > 0.")
    GO TO exit_script
   ENDIF
   CALL echo(build("Verify entry_node_id is valid: ",entry_node_id))
   IF (entry_node_id > 0)
    SET codevaluevalid = checknodeid(entry_node_id)
    IF (codevaluevalid=0)
     SET reply->status_data.status = "F"
     SET reply->status_data.subeventstatus.operationname = "PERFORM"
     SET reply->status_data.subeventstatus.operationstatus = "F"
     SET reply->status_data.subeventstatus.targetobjectname = "Validate"
     SET reply->status_data.subeventstatus.targetobjectvalue = build2("Entry Node ID ",trim(
       cnvtstring(entry_node_id),3)," does not exist or is inactive.")
     GO TO exit_script
    ENDIF
   ENDIF
   CALL echo(build("Verify pathway_type_cd is valid: ",pw_type_cd))
   IF (pw_type_cd > 0)
    SET codevaluevalid = checkcodevalue(pw_type_cd,pathway_type_cs)
    IF (codevaluevalid != 1)
     SET reply->status_data.status = "F"
     SET reply->status_data.subeventstatus.operationname = "PERFORM"
     SET reply->status_data.subeventstatus.operationstatus = "F"
     SET reply->status_data.subeventstatus.targetobjectname = "Validate"
     SET reply->status_data.subeventstatus.targetobjectvalue = build2("Pathway Type CD ",trim(
       cnvtstring(pw_type_cd),3)," does not exist or is inactive.")
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->status_data.status = "F"
    ...
    SET reply->status_data.subeventstatus.targetobjectname = "Validate"
    SET reply->status_data.subeventstatus.targetobjectvalue = build2(
     "Pathway Type CD must be non-zero.")
    GO TO exit_script
   ENDIF
   CALL echo(build("Verify pathway_status_cd is valid: ",pw_status_cd))
   IF (pw_status_cd > 0)
    SET codevaluevalid = checkcodevalue(pw_status_cd,pathway_status_cs)
    IF (codevaluevalid != 1)
     SET reply->status_data.status = "F"
     SET reply->status_data.subeventstatus.operationname = "PERFORM"
     SET reply->status_data.subeventstatus.operationstatus = "F"
     SET reply->status_data.subeventstatus.targetobjectname = "Validate"
     SET reply->status_data.subeventstatus.targetobjectvalue = build2("Pathway Status CD ",trim(
       cnvtstring(pw_status_cd),3)," does not exist or is inactive.")
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->status_data.status = "F"
    ...
    SET reply->status_data.subeventstatus.targetobjectname = "Validate"
    SET reply->status_data.subeventstatus.targetobjectvalue = build2(
     "Pathway Status CD must be non-zero.")
    GO TO exit_script
   ENDIF
   CALL echo(build("Verify pathway_status_cd is valid: ",pw_status_cd))
   IF (concept_cd > 0)
    SET codevaluevalid = checkcodevalue(concept_cd,concept_cd_cs)
    IF (codevaluevalid != 1)
     SET reply->status_data.status = "F"
     SET reply->status_data.subeventstatus.operationname = "PERFORM"
     SET reply->status_data.subeventstatus.operationstatus = "F"
     SET reply->status_data.subeventstatus.targetobjectname = "Validate"
     SET reply->status_data.subeventstatus.targetobjectvalue = build2("Concept CD ",trim(cnvtstring(
        concept_cd),3)," does not exist or is inactive.")
     GO TO exit_script
    ENDIF
   ENDIF
 ...
 SUBROUTINE (checkcodevalue(codevalue=f8,codeset=i4) =i4 WITH protect)
   ...
   RETURN(curqual)
 END ;Subroutine
 SUBROUTINE (checknodeid(entry_node_id=f8) =i4 WITH protect)
   CALL log_message("In checkNodeId()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SELECT INTO "nl:"
    FROM cp_node cn
    WHERE cn.cp_node_id=entry_node_id
 ...
 SUBROUTINE insertrows(null)
   ...
   DECLARE cp_pathway_id = f8 WITH protect, noconstant(0)
   DECLARE cp_node_behavior_id = f8 WITH protect, noconstant(0)
   IF (checkdic("CP_PATHWAY","T",0) != 2)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus.operationname = "PERFORM"
    SET reply->status_data.subeventstatus.operationstatus = "F"
    SET reply->status_data.subeventstatus.targetobjectname = "INSERT"
    SET reply->status_data.subeventstatus.targetobjectvalue = build2(
     "CP_PATHWAY Table does not exist","or Permission is not granted to access.")
    GO TO exit_script
   ENDIF
   IF (checkdic("CP_NODE_BEHAVIOR","T",0) != 2)
    ...
    GO TO exit_script
   ENDIF
   SET cp_pathway_id = 0.0
   SELECT INTO "nl:"
    nextseqnum = seq(reference_seq,nextval)
    FROM dual
    DETAIL
     cp_pathway_id = nextseqnum
    WITH nocounter
   ;end select
   CALL echo(build2("Next CP_PATHWAY_ID:",cp_pathway_id))
   IF (cp_pathway_id=0.0)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus.operationname = "PERFORM"
    SET reply->status_data.subeventstatus.operationstatus = "F"
    SET reply->status_data.subeventstatus.targetobjectname = "INSERT"
    SET reply->status_data.subeventstatus.targetobjectvalue = build2(
     "Unable to generate cp_pathway_id from REFERENCE_SEQ.")
    ROLLBACK
    GO TO exit_script
   ENDIF
   SET cp_node_behavior_id = 0.0
   IF (entry_node_id > 0)
    SELECT INTO "nl:"
     nextseqnum = seq(reference_seq,nextval)
     FROM dual
     DETAIL
      cp_node_behavior_id = nextseqnum
     WITH nocounter
    ;end select
    CALL echo(build2("Next CP_NODE_BEHAVIOR:",cp_node_behavior_id))
    IF (cp_pathway_id=0.0)
     SET reply->status_data.status = "F"
     SET reply->status_data.subeventstatus.operationname = "PERFORM"
     SET reply->status_data.subeventstatus.operationstatus = "F"
    ...
    ENDIF
   ENDIF
   INSERT  FROM cp_pathway cp
    SET cp.cp_pathway_id = cp_pathway_id, cp.pathway_name = trim(pw_name), cp.pathway_type_cd =
     pw_type_cd,
     cp.pathway_status_cd = pw_status_cd, cp.cp_concept_cd = concept_cd, cp.owner_prsnl_id =
     owner_prsnl_id,
     cp.updt_id = reqinfo->updt_id, cp.updt_applctx = reqinfo->updt_applctx, cp.updt_task = reqinfo->
     updt_task,
     cp.updt_dt_tm = cnvtdatetime(sysdate), cp.updt_cnt = 0, cp.active_ind = 1,
     cp.logical_domain_id = user_logic_domain_id
    WITH nocounter
   ;end insert
   CALL error_and_zero_check_rec(1,"INSERT","ERROR INSERTING INTO CP_PATHWAY",1,1,
    reply)
   IF (entry_node_id > 0)
    INSERT  FROM cp_node_behavior cp
     SET cp.cp_node_behavior_id = cp_node_behavior_id, cp.cp_node_id = entry_node_id, cp.updt_id =
      reqinfo->updt_id,
      cp.updt_applctx = reqinfo->updt_applctx, cp.updt_task = reqinfo->updt_task, cp.updt_dt_tm =
      cnvtdatetime(sysdate),
      cp.updt_cnt = 0
     WITH nocounter
    ;end insert
    CALL error_and_zero_check_rec(1,"INSERT","ERROR INSERTING INTO CP_NODE_BEHAVIOR",1,1,
     reply)
   ENDIF
   SET reply->pwlist[1].cp_pathway_id = cp_pathway_id
   SET reply->pwlist[1].entry_node_id = entry_node_id
   SET reply->pwlist[1].pathway_name = trim(pw_name)
   SET reply->pwlist[1].pathway_type_cd = pw_type_cd
   SET reply->pwlist[1].pathway_status_cd = pw_status_cd
   SET reply->pwlist[1].concept_cd = concept_cd
