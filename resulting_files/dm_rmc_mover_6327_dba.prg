CREATE PROGRAM dm_rmc_mover_6327:dba
 ...
 DECLARE cust_link = vc
 DECLARE cust_column_cnt = i4
 DECLARE cust_tp_id = vc
 DECLARE cust_src_tp = vc
 DECLARE cust_src_tpc = vc
 DECLARE cust_comp_type_cd = f8
 DECLARE cust_ctc_cdf_meaning = vc
 DECLARE cust_sub_comp_type_cd = vc
 DECLARE cust_alg_type = vc
 DECLARE cust_sctc_cdf_meaning = vc
 DECLARE cust_sub_comp_name = vc
 DECLARE cust_rpt_missing = vc
 DECLARE cust_src_cv = vc
 DECLARE cust_new_value = vc
 DECLARE cust_curqual = i4
 DECLARE temp_col_cnt = i4
 DECLARE new_cust_sub_comp_pref = vc
 DECLARE bad_delim_ind = i2
 DECLARE cust_i = i4
 DECLARE cust_val = f8
 DECLARE size_delim_rs = i4
 DECLARE targ_sub_comp_type = f8
 DECLARE targ_track_pref_id = f8
 DECLARE evaluate_id = f8
 DECLARE cust_cnt = i4
 DECLARE trans_list_element(subpos=i4,subtable=vc,subcolumn=vc) = vc
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_link = dm2_ref_data_doc->post_link_name
 SET cust_column_cnt = get_col_pos(cust_tab_name,"SUB_COMP_PREF")
 SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_column_cnt].translated = 0
 SET cust_column_cnt = get_col_pos(cust_tab_name,"TRACK_PREF_ID")
 SET cust_tp_id = get_value(cust_tab_name,"TRACK_PREF_ID","FROM")
 SET cust_src_tp = dm2_get_rdds_tname("TRACK_PREFS")
 SET cust_src_tpc = dm2_get_rdds_tname("TRACK_COMP_PREFS")
 SET dm_err->eproc = "Obtaining TRACK_PREFS.COMP_TYPE_CD value."
 SELECT INTO "NL:"
  tp.comp_type_cd
  FROM (parser(cust_src_tp) tp)
  WHERE tp.track_pref_id=cnvtreal(cust_tp_id)
  DETAIL
   cust_comp_type_cd = tp.comp_type_cd
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "Could not obtain TRACK_PREFS.COMP_TYPE_CD value"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_6327
 ENDIF
 IF (curqual=0)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "ORPHAN"
  SET dm_err->emsg = "Error: TRACK_COMP_PREFS row references an ORPHAN TRACK_PREFS row"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET bad_delim_ind = 1
  SET dm_err->err_ind = 0
  GO TO exit_6327
 ENDIF
 SET cust_src_cv = dm2_get_rdds_tname("CODE_VALUE")
 SET dm_err->eproc = "Obtaining CDF_MEANING of TRACK_PREFS.COMP_TYPE_CD value."
 SELECT INTO "NL:"
  cv.cdf_meaning
  FROM (parser(cust_src_cv) cv)
  WHERE cv.code_value=cnvtreal(cust_comp_type_cd)
   AND cv.code_set IN (20500, 20314, 20316)
  DETAIL
   cust_ctc_cdf_meaning = cv.cdf_meaning
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "Could not obtain CDF_MEANING of TRACK_PREFS.COMP_TYPE_CD"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_6327
 ENDIF
 SET cust_column_cnt = get_col_pos(cust_tab_name,"SUB_COMP_TYPE_CD")
 SET cust_sub_comp_type_cd = get_value(cust_tab_name,"SUB_COMP_TYPE_CD","FROM")
 IF (cnvtreal(cust_sub_comp_type_cd)=0)
  SET dm_err->eproc = "Obtaining CDF_MEANING from 4002167 with SUB_COMP_TYPE_CD of 0."
  SELECT INTO "NL:"
   cv.cdf_meaning
   FROM code_value cv
   WHERE cv.display=cust_ctc_cdf_meaning
    AND cv.definition="0"
    AND cv.code_set=4002167
    AND cv.active_ind=1
   DETAIL
    cust_alg_type = cv.cdf_meaning
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "Could not obtain CDF_MEANING of with SUB_COMP_TYPE_CD of 0"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_6327
  ENDIF
 ELSEIF (cnvtreal(cust_sub_comp_type_cd) > 0)
  SET cust_src_cv = dm2_get_rdds_tname("CODE_VALUE")
  SET dm_err->eproc = concat("Obtaining CDF_MEANING from 4002167 with SUB_COMP_TYPE_CD",
   cust_sub_comp_type_cd)
  SELECT INTO "NL:"
   cv.cdf_meaning
   FROM (parser(cust_src_cv) cv)
   WHERE cv.code_value=cnvtreal(cust_sub_comp_type_cd)
    AND cv.code_set IN (20500, 20316)
   DETAIL
    cust_sctc_cdf_meaning = cv.cdf_meaning
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "Could not obtain CDF_MEANING for SUB_COMP_TYPE_CD"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_6327
  ENDIF
  SET dm_err->eproc = concat("Obtaining CDF_MEANING from 4002167 with SUB_COMP_TYPE_CD",
  ...
  SELECT INTO "NL:"
   ...
   FROM code_value cv
   WHERE cv.display=cust_ctc_cdf_meaning
    AND cv.definition=cust_sctc_cdf_meaning
    AND cv.code_set=4002167
    AND cv.active_ind=1
   DETAIL
    cust_alg_type = cv.cdf_meaning
  ...
  IF (check_error(dm_err->eproc)=1)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "Could not obtain CDF_MEANING of with SUB_COMP_TYPE_CD"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_6327
  ENDIF
  SET cust_curqual = curqual
  IF (cust_curqual=0)
   SET dm_err->eproc = "Obtaining CDF_MEANING from 4002167 where definition = NONE"
   SELECT INTO "NL:"
    cv.cdf_meaning
    FROM code_value cv
    WHERE cv.display=cust_ctc_cdf_meaning
     AND cv.definition="NONE"
     AND cv.code_set=4002167
     AND cv.active_ind=1
    DETAIL
     cust_alg_type = cv.cdf_meaning
   ...
   IF (check_error(dm_err->eproc)=1)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "Could not obtain CDF_MEANING where definition = NONE"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_6327
   ENDIF
  ENDIF
 ENDIF
 IF (cust_alg_type="ALG9")
  SET cust_sub_comp_name = get_value(cust_tab_name,"SUB_COMP_NAME","FROM")
  CASE (cust_sub_comp_name)
   OF "FnColViewId":
    SET cust_alg_type = "ALG11"
   OF patstring("FnFormId*",0):
    SET cust_alg_type = "ALG15"
   OF "FnGrpViewId":
    SET cust_alg_type = "ALG16"
   OF "FnIRLocationCode":
   OF "FnLocViewGuide":
   OF "FnLocationCd":
   OF "Clinical Tab Code":
   OF "Patient Tab Code":
   OF "FnLocViewCode":
    SET cust_alg_type = "ALG1"
   OF "FnBuildDefType":
   OF "FnTabRefresh":
   OF "Clinical Tab Code Name":
   OF "Clinical Tab Prefs":
   OF "Patient Tab Prefs":
   OF "Patient Tab Code Name":
    SET cust_alg_type = "ALG0"
   ELSE
    SET dm_err->eproc = "Evaluating TRACK_COMP_PREFS.SUB_COMP_NAME for ALG9"
    SET dm_err->emsg = concat("Unexpected sub_comp_name of ",cust_sub_comp_name,
     " for TRACK_COMP_PREFS row")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 0
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "NOMV51"
    SET bad_delim_ind = 1
    GO TO exit_6327
  ENDCASE
 ENDIF
 SET cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
 CALL delim_string(cust_sub_comp_pref,";")
 SET size_delim_rs = size(delim_rs->qual,5)
 IF (size_delim_rs=0)
  SET stat = alterlist(delim_rs->qual,1)
  SET delim_rs->qual[1].val = cust_sub_comp_pref
  SET size_delim_rs = size(delim_rs->qual,5)
 ENDIF
 SET dm_err->eproc = concat("Evaluating TRACK_COMP_PREFS row as ",cust_alg_type)
 CASE (cust_alg_type)
  OF "ALG0":
   SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
  OF "ALG1":
   IF (trim(cust_sub_comp_pref) > " ")
    FOR (cust_i = 1 TO size(delim_rs->qual,5))
      IF (trim(delim_rs->qual[cust_i].val) > " ")
       IF (isnumeric(delim_rs->qual[cust_i].val) IN (1, 2))
        SET cust_new_value = select_merge_translate(delim_rs->qual[cust_i].val,"CODE_VALUE")
        IF (isnumeric(cust_new_value))
         IF (cust_i=1)
          SET new_cust_sub_comp_pref = cust_new_value
         ELSE
          SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",cust_new_value)
         ENDIF
        ELSE
       ...
       ELSE
        SET dm2_ref_data_reply->error_ind = 1
        SET dm2_ref_data_reply->error_msg = "NOMV51"
        SET dm_err->emsg = "Error: Unexpected value found in SUB_COMP_PREF, expecting a code_value"
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET dm_err->err_ind = 0
        GO TO exit_6327
       ENDIF
      ELSE
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";")
      ENDIF
    ENDFOR
   ELSE
    SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
  ...
  OF "ALG2":
   IF (size_delim_rs != 0)
    IF (size_delim_rs != 1)
     SET dm_err->emsg = "Data on the track_comp_prefs row is different than expected"
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV51"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dm_err->err_ind = 0
     GO TO exit_6327
    ENDIF
    SET new_cust_sub_comp_pref = trans_list_element(1,"ORDER_CATALOG_SYNONYM","SYNONYM_ID")
   ELSE
    SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
   ENDIF
  OF "ALG3":
   IF (size_delim_rs != 0)
    IF (size_delim_rs != 1)
     ...
     GO TO exit_6327
    ENDIF
    SET new_cust_sub_comp_pref = trans_list_element(1,"TRACK_EVENT","TRACK_EVENT_ID")
   ELSE
    SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
   ENDIF
  OF "ALG4":
   IF (trim(cust_sub_comp_pref) > " ")
    ...
    ENDIF
    SET new_cust_sub_comp_pref = trans_list_element(1,"TRACK_EVENT","TRACK_EVENT_ID")
    SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(2,"CODE_VALUE",
      "CODE_VALUE"))
    SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(3,"TRACK_EVENT",
      "TRACK_EVENT_ID"))
    SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(4,"CODE_VALUE",
      "CODE_VALUE"))
   ELSE
    SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
   ENDIF
  OF "ALG5":
   IF (trim(cust_sub_comp_pref) > " ")
    FOR (cust_i = 1 TO size_delim_rs)
      IF (cust_i=1)
       SET new_cust_sub_comp_pref = delim_rs->qual[cust_i].val
      ELSEIF (((cust_i=5) OR (cust_i=10)) )
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(cust_i,
         "TRACK_EVENT","TRACK_EVENT_ID"))
      ELSEIF (cust_i=11)
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(cust_i,
         "CODE_VALUE","CODE_VALUE"))
      ELSE
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",delim_rs->qual[cust_i].val)
      ENDIF
    ENDFOR
   ELSE
    SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
   ...
   IF (trim(cust_sub_comp_pref) > " ")
    FOR (cust_i = 1 TO size_delim_rs)
      IF (cust_i=1)
       SET new_cust_sub_comp_pref = delim_rs->qual[cust_i].val
      ELSEIF (cust_i=2)
       IF ((delim_rs->qual[cust_i].val="4200000"))
        SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";","4200000")
       ELSE
        SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(cust_i,
          "APPLICATION_TASK","TASK_NUMBER"))
       ENDIF
      ELSEIF (((cust_i=5) OR (cust_i=10)) )
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(cust_i,
         "TRACK_EVENT","TRACK_EVENT_ID"))
  ...
  OF "ALG7":
   IF (trim(cust_sub_comp_pref) > " ")
    FOR (cust_i = 1 TO size_delim_rs)
      ...
      ELSEIF (cust_i=2)
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(cust_i,
         "DCP_FORMS_REF","DCP_FORMS_INSTANCE_ID"))
      ELSEIF (((cust_i=5) OR (cust_i=10)) )
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(cust_i,
         "TRACK_EVENT","TRACK_EVENT_ID"))
      ELSEIF (cust_i=11)
  ...
  OF "ALG8":
   IF (trim(cust_sub_comp_pref) > " ")
    FOR (cust_i = 1 TO size_delim_rs)
      IF (cust_i=1)
       SET new_cust_sub_comp_pref = delim_rs->qual[cust_i].val
      ELSEIF (((cust_i=2) OR (((cust_i=5) OR (cust_i=10)) )) )
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(cust_i,
         "TRACK_EVENT","TRACK_EVENT_ID"))
      ELSEIF (cust_i=11)
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(cust_i,
  ...
  OF "ALG10":
   IF (trim(cust_sub_comp_pref) > " ")
    SET new_cust_sub_comp_pref = delim_rs->qual[1].val
    IF (size_delim_rs > 1)
     CASE (delim_rs->qual[1].val)
      OF "1":
       IF ((delim_rs->qual[cust_i].val="4200000"))
        SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";","4200000")
       ELSE
        SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(2,
          "APPLICATION_TASK","TASK_NUMBER"))
       ENDIF
       IF (size(delim_rs->qual,5) > 2)
        FOR (cust_i = 3 TO size(delim_rs->qual,5))
          SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",delim_rs->qual[cust_i].val)
        ENDFOR
       ENDIF
      OF "2":
       ...
       ENDIF
      OF "3":
       FOR (cust_i = 2 TO size(delim_rs->qual,5))
         SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",delim_rs->qual[cust_i].val)
       ENDFOR
      ELSE
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = "NOMV51"
       SET dm_err->emsg = "Error: Unexpected value found in SUB_COMP_PREF"
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET bad_delim_ind = 1
       SET dm_err->err_ind = 0
       GO TO exit_6327
  ...
  OF "ALG11":
   IF (trim(cust_sub_comp_pref) > " ")
    IF (size_delim_rs != 1)
     ...
     GO TO exit_6327
    ENDIF
    SET new_cust_sub_comp_pref = trans_list_element(1,"PREDEFINED_PREFS","PREDEFINED_PREFS_ID")
   ELSE
    SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
   ENDIF
  OF "ALG12":
   IF (trim(cust_sub_comp_pref) > " ")
    FOR (cust_i = 1 TO size(delim_rs->qual,5))
      IF (cust_i=1)
       SET new_cust_sub_comp_pref = trans_list_element(cust_i,"TRACK_REFERENCE","TRACKING_REF_ID")
      ELSE
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(cust_i,
         "TRACK_REFERENCE","TRACKING_REF_ID"))
      ENDIF
    ENDFOR
   ELSE
    SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
  ...
  OF "ALG13":
   IF (trim(cust_sub_comp_pref) > " ")
    SET new_cust_sub_comp_pref = trans_list_element(1,"CODE_VALUE","CODE_VALUE")
    IF (size(delim_rs->qual,5) > 1)
     FOR (cust_i = 2 TO size(delim_rs->qual,5))
       SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",delim_rs->qual[cust_i].val)
     ENDFOR
    ENDIF
  ...
  OF "ALG14":
   IF (trim(cust_sub_comp_pref) > " ")
    IF (size_delim_rs < 2)
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV51"
     SET dm_err->emsg = "Error: Data on the track_comp_prefs row is different than expected"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET bad_delim_ind = 1
     SET dm_err->err_ind = 0
     GO TO exit_6327
  ...
  OF "ALG15":
   IF (trim(cust_sub_comp_pref) > " ")
    IF (size_delim_rs != 1)
     ...
     GO TO exit_6327
    ENDIF
    SET new_cust_sub_comp_pref = trans_list_element(1,"DCP_FORMS_REF","DCP_FORM_INSTANCE_ID")
   ELSE
    SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
   ENDIF
  OF "ALG16":
   IF (trim(cust_sub_comp_pref) > " ")
    IF (size_delim_rs != 1)
     ...
     GO TO exit_6327
    ENDIF
    SET new_cust_sub_comp_pref = trans_list_element(1,"TRACK_PREFS","TRACK_PREF_ID")
   ELSE
    SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
   ENDIF
  OF "ALG17":
   IF (trim(cust_sub_comp_pref) > " ")
    ...
    ENDIF
    SET new_cust_sub_comp_pref = delim_rs->qual[1].val
    IF ((delim_rs->qual[2].val="4200000"))
     SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";","4200000")
    ELSE
     SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";",trans_list_element(2,
       "APPLICATION_TASK","TASK_NUMBER"))
  ...
  OF "ALG18":
   IF (trim(cust_sub_comp_pref) > " ")
    ...
    IF ((delim_rs->qual[2].val="4200000"))
     SET new_cust_sub_comp_pref = concat(new_cust_sub_comp_pref,";","4200000")
    ELSE
     SET new_cust_sub_comp_pref = trans_list_element(1,"APPLICATION_TASK","TASK_NUMBER")
    ENDIF
   ELSE
    SET new_cust_sub_comp_pref = get_value(cust_tab_name,"SUB_COMP_PREF","FROM")
   ENDIF
  ELSE
   SET dm_err->eproc = "Evaluating TRACK_COMP_PREFS row ALG type"
   SET dm_err->emsg = build("Unexpected ALG type of ",cust_alg_type," for track_comp_prefs table")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV51"
   GO TO exit_6327
 ...
 ENDIF
 IF (bad_delim_ind=0)
  CALL put_value(cust_tab_name,"SUB_COMP_PREF",new_cust_sub_comp_pref)
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"TRACK_PREF_COMP_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   SET targ_sub_comp_type = cnvtreal(get_value(cust_tab_name,"SUB_COMP_TYPE_CD","TO"))
   SET targ_track_pref_id = cnvtreal(get_value(cust_tab_name,"TRACK_PREF_ID","TO"))
   SET cust_sub_comp_name = get_value(cust_tab_name,"SUB_COMP_NAME","TO")
   SET cust_cnt = 0
   SELECT INTO "nl:"
    FROM track_comp_prefs tcp
    WHERE tcp.sub_comp_type_cd=targ_sub_comp_type
     AND tcp.track_pref_id=targ_track_pref_id
     AND tcp.sub_comp_name=cust_sub_comp_name
    DETAIL
     cust_cnt = (cust_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,cust_cnt), ui_query_eval_rec
     ->qual[cust_cnt].root_entity_attr = tcp.track_pref_comp_id
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "Error trying to find translation for track_pref_comp_id"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_6327
   ENDIF
   SET cust_val = evaluate_exec_ui_query(cust_cnt,temp_tbl_cnt,perm_col_cnt)
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"TRACK_PREF_COMP_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"TRACK_PREF_COMP_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  TRACK_PREF_COMP_ID")
     ELSE
      GO TO exit_6327
     ENDIF
    ELSEIF ((cust_val=- (2)))
     ...
     SELECT INTO "nl:"
      FROM track_comp_prefs tcp
      WHERE tcp.sub_comp_pref=cust_sub_comp_pref
       AND tcp.sub_comp_type_cd=targ_sub_comp_type
       AND tcp.track_pref_id=targ_track_pref_id
       AND tcp.sub_comp_name=cust_sub_comp_name
      DETAIL
       cust_cnt = (cust_cnt+ 1), stat = alterlist(ui_query_eval_rec->qual,cust_cnt),
       ui_query_eval_rec->qual[cust_cnt].root_entity_attr = tcp.track_pref_comp_id
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      SET dm2_ref_data_reply->error_ind = 1
      ...
      GO TO exit_6327
     ENDIF
     SET evaluate_id = evaluate_exec_ui_query(cust_cnt,temp_tbl_cnt,perm_col_cnt)
     IF (evaluate_id < 0)
      IF ((evaluate_id=- (3)))
       SET cust_val = get_seq(cust_tab_name,"TRACK_PREF_COMP_ID")
       IF (cust_val > 0)
       ...
       ELSE
        GO TO exit_6327
       ENDIF
      ELSEIF ((evaluate_id=- (2)))
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = "NOMV06"
       CALL echo("Multiple rows were found in the target")
       GO TO exit_6327
      ELSE
       GO TO exit_6327
      ENDIF
     ELSE
      CALL put_value(cust_tab_name,"TRACK_PREF_COMP_ID",cnvtstring(evaluate_id))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  TRACK_PREF_COMP_ID")
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
     ENDIF
   ...
   SET cust_from_cd = get_value(cust_tab_name,"TRACK_PREF_COMP_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6327
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"TRACK_PREF_COMP_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_6327
   ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"SUB_COMP_PREF")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg =
    "This row wasn't translated because of the SUB_COMP_PREF column"
 ...
 ELSE
  ...
  ENDIF
 ENDIF
 SUBROUTINE trans_list_element(subpos,subtable,subcolumn)
   DECLARE subret = vc
   IF (trim(delim_rs->qual[subpos].val) > " ")
    IF (isnumeric(delim_rs->qual[subpos].val) IN (1, 2))
     SET cust_new_value = select_merge_translate(delim_rs->qual[subpos].val,subtable)
     IF (isnumeric(cust_new_value))
      SET cust_new_value = replace(delim_rs->qual[subpos].val,trim(cnvtstring(delim_rs->qual[subpos].
         val,20)),trim(cnvtstring(cust_new_value,20)),1)
      SET subret = cust_new_value
     ELSE
      SET cust_rpt_missing = report_missing(subtable,subcolumn,cnvtreal(delim_rs->qual[subpos].val))
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET bad_delim_ind = 1
     ENDIF
    ELSE
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV51"
     SET dm_err->emsg = concat("Error: Unexpected value found in SUB_COMP_PREF, expecting a ",
      subtable,".",subcolumn)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET bad_delim_ind = 1
     SET dm_err->err_ind = 0
    ENDIF
   ELSE
    SET subret = delim_rs->qual[subpos].val
   ENDIF
   RETURN(subret)
 END ;Subroutine
#exit_6327
