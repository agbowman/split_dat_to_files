CREATE PROGRAM bhs_rpt_hsrn_test:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  FROM health_plan hp,
   ...
   person p
  PLAN (hp
   WHERE hp.plan_name_key="BEHEALTHYACO"
    AND hp.active_ind=1)
   JOIN (ppr
  ...
  HEAD p.name_first_key
   null
  HEAD p.person_id
   ml_pcnt += 1, m_plans->l_pcnt = ml_pcnt, stat = alterlist(m_plans->plst,ml_pcnt),
   m_plans->plst[ml_pcnt].f_person_id = p.person_id, m_plans->plst[ml_pcnt].c_patient_name = trim(p
    .name_full_formatted,3), m_plans->plst[ml_pcnt].c_dob = format(p.birth_dt_tm,"mm/dd/yyyy;;D"),
   m_plans->plst[ml_pcnt].c_hne_id = build(ppr.member_nbr)
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM clinical_event ce,
   ...
   nomenclature n
  PLAN (ce
   WHERE expand(ml_pidx,1,size(m_plans->plst,5),ce.person_id,m_plans->plst[ml_pidx].f_person_id)
  ...
  HEAD REPORT
   ml_ecnt = 0
  HEAD ce.encntr_id
   ml_ppos = 0, ml_ppos = locateval(ml_pnum,1,m_plans->l_pcnt,ce.person_id,m_plans->plst[ml_pnum].
    f_person_id), ml_ecnt += 1,
   m_hsrn->l_ecnt = ml_ecnt, stat = alterlist(m_hsrn->elst,ml_ecnt), m_hsrn->elst[ml_ecnt].
   f_encntr_id = e.encntr_id,
   m_hsrn->elst[ml_ecnt].c_patient_name = build(p.name_full_formatted), m_hsrn->elst[ml_ecnt].c_dob
    = format(p.birth_dt_tm,"mm/dd/yyyy;;D"), m_hsrn->elst[ml_ecnt].c_location = build(
    uar_get_code_display(e.location_cd)),
   m_hsrn->elst[ml_ecnt].c_hne_id = trim(m_plans->plst[ml_ppos].c_hne_id,3), m_hsrn->elst[ml_ecnt].
   c_hrsn_date = format(ce.performed_dt_tm,"mm/dd/yyyy;;D"), m_hsrn->elst[ml_ecnt].
   c_hrsn_signing_user = build(pr.name_full_formatted),
  ...
  HEAD ce.task_assay_cd
   ml_dtapos = 0, ml_dtapos = locateval(ml_dtanum,1,m_dtas->l_dcnt,ce.task_assay_cd,m_dtas->dtalst[
    ml_dtanum].f_task_assay_cd), ml_respos = 0,
   ml_respos = locateval(ml_resnum,1,m_dtas->dtalst[ml_dtapos].l_rescnt,n.nomenclature_id,m_dtas->
    dtalst[ml_dtapos].reslst[ml_resnum].f_nomenclature_id)
   IF (ml_dtapos > 0
    AND ml_respos > 0)
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 SET ms_address_list = "matt.butler@baystatehealth.org"
