CREATE PROGRAM bhs_ma_clinical_summary2:dba
 SET vversion = "V7"
 ...
 IF ((request->visit_cnt > 0))
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   ELSEIF (c.event_cd=contactproxyphonenumber_cd) 10
  ...
  ;end select
  SELECT DISTINCT INTO "nl:"
   sorter = replace(build(" -5- ",uar_get_code_display(c5.event_cd)," -4- ",uar_get_code_display(c4
      .event_cd)," -3- ",
     uar_get_code_display(c3.event_cd)," -2- ",uar_get_code_display(c2.event_cd)),"DCP GENERIC CODE",
    " ",0)
   FROM (dummyt dd  WITH seq = dlrec->seq, request->visit_cnt),
    clinical_event c,
    ce_date_result cd,
   ...
   PLAN (dd)
    ...
    JOIN (c3
    WHERE outerjoin(c2.parent_event_id)=c3.event_id)
    JOIN (c4
    WHERE outerjoin(c3.parent_event_id)=c4.event_id)
    JOIN (c5
    WHERE outerjoin(c4.parent_event_id)=c5.event_id)
   ORDER BY c.encntr_id, sorter, cnvtdatetime(c.event_end_dt_tm),
    c.event_id
   HEAD c.encntr_id
   ...
   DETAIL
    ...
    dlrec->seq[dd.seq].d0events[cnt].header2_txt = sorter
    ...
    ENDIF
    dlrec->seq[dd.seq].d0events[cnt].event_cd_disp = concat(uar_get_code_display(c.event_cd),"--",
     cnvtstring(c.view_level)), dlrec->seq[dd.seq].d0events[cnt].event_dt_tm = substring(1,14,format(
      c.event_end_dt_tm,"@SHORTDATETIME;;Q")), dlrec->seq[dd.seq].d0events[cnt].result = c.result_val
    IF (c.event_id=cd.event_id)
     CASE (cd.date_type_flag)
      OF 0:
    ...
    FOR (i = 1 TO dlrec->encntr_total)
      ...
      IF ((dlrec->seq[i].total_diag > 0))
       ...
       FOR (a = 1 TO dlrec->seq[i].total_diag)
        tempstring = concat(trim(dlrec->seq[i].diagnosis[a].diag_type_desc),dlrec->seq[i].diagnosis[a
         ].source_string),line_wrap
       ENDFOR
      ENDIF
      ...
      FOR (a = 1 TO dlrec->seq[i].diet_total)
       tempstring = concat("{b}",dlrec->seq[i].diet[a].order_mnemonic,"{endb} ",dlrec->seq[i].diet[a]
      ...
      FOR (a = 1 TO dlrec->seq[i].resp_total)
       tempstring = concat("{b}",dlrec->seq[i].resp[a].order_mnemonic,"{endb} ",dlrec->seq[i].resp[a]
        .clinical_display_line),line_wrap
      ENDFOR
      ...
      FOR (a = 1 TO dlrec->seq[i].asmt_total)
       tempstring = concat("{b}",dlrec->seq[i].asmt[a].order_mnemonic,"{endb} ",dlrec->seq[i].asmt[a]
        .clinical_display_line),line_wrap
      ENDFOR
      rowplusone,
      ...
      ENDFOR
      rowplusone, title_string = "Documentation:", last_header1_txt = fillstring(200," "),
      last_header2_txt = fillstring(200," "), last_parent_event = fillstring(200," "), xcolvar = (
      xcol1+ 10)
      FOR (a = 1 TO (dlrec->seq[i].d0events_total - 1))
        ...
        IF ((dlrec->seq[i].d0events[a].parent_event > " "))
         ...
         tempstring = concat("{b}   ",dlrec->seq[i].d0events[a].event_cd_disp,"{endb} ",dlrec->seq[i]
          .d0events[a].result,"   ",
          dlrec->seq[i].d0events[a].event_dt_tm," ",cnvtstring(dlrec->seq[i].d0events[a].event_id),
          " ",cnvtstring(dlrec->seq[i].d0events[a].event_cd)), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].d0events[a].event_cd_disp,"{endb} ",dlrec->seq[i].
          d0events[a].result,"   ",
          dlrec->seq[i].d0events[a].event_dt_tm," ",cnvtstring(dlrec->seq[i].d0events[a].event_id),
