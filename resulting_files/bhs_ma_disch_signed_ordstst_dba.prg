CREATE PROGRAM bhs_ma_disch_signed_ordstst:dba
 PROMPT
 ...
 DECLARE tempcomment = vc WITH noconstant(" ")
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     IF (lt.long_text_id > 0)
      comment_string = concat("COMMENT: ",check(trim(lt.long_text,3))), eol1 = findstring(end_line,
      ...
      FOR (zz = 1 TO ord_cnt)
        ...
        ELSE
         WHILE (temp_pos <= temp_len)
           ...
           ENDIF
           temp_pos = (temp_pos+ sub_len), rowplusone
         ENDWHILE
        ENDIF
        IF ((data->qual[xx].orders[zz].comment_cnt > 0))
         ...
         FOR (x2 = 1 TO data->qual[xx].orders[zz].comment_cnt)
           IF (yrow > 650)
            BREAK
           ENDIF
           tempcomment = data->qual[xx].orders[zz].comments[x2].comment
           IF (x2=1)
            CALL print(calcpos(x_col1,yrow)), tempcomment
           ELSE
            CALL print(calcpos((x_col1+ 38),yrow)), tempcomment
           ENDIF
           rowplusone
         ENDFOR
        ENDIF
