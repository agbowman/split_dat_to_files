CREATE PROGRAM bh_rc_rl_mt_medical:dba
 CALL echo("***** bh_rc_rl_mt_medical.prg - 677918 *****")
   ...
   FOR (lchgcnt = 1 TO size(request->integer_params,5))
     CASE (request->integer_params[lchgcnt].key_txt)
      OF "MAJOR_MEDICAL_MONTHLY":
       SET tmprec->major_medical_annual = (request->integer_params[lchgcnt].integer_value * 12)
     ENDCASE
   ENDFOR
 END ;Subroutine
 DECLARE createreply(null) = null
 SUBROUTINE createreply(null)
  CALL addtoreplyintegers("MAJOR_MEDICAL_ANNUAL",tmprec->major_medical_annual)
  CALL echorecord(tmprec)
 END ;Subroutine
 FREE RECORD tmprec
 RECORD tmprec(
   1 major_medical_annual = i4
 )
