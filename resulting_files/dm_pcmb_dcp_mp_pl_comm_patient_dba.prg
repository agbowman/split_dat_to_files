CREATE PROGRAM dm_pcmb_dcp_mp_pl_comm_patient:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PCMB_DCP_MP_PL_COMM_PATIENT"
 ...
 IF ((request->xxx_combine[icombine].encntr_id=0))
  SELECT INTO "nl:"
   ...
   WHERE (from_comm_patient.person_id=request->xxx_combine[icombine].from_xxx_id)
   DETAIL
    from_row_count = (from_row_count+ 1)
    IF (mod(from_row_count,10)=1)
     stat = alterlist(rreclist->from_rec,(from_row_count+ 9))
    ENDIF
    rreclist->from_rec[from_row_count].from_id = from_comm_patient.dcp_mp_pl_comm_patient_id,
    rreclist->from_rec[from_row_count].active_ind = from_comm_patient.active_ind, rreclist->from_rec[
 ...
 ELSE
  SELECT INTO "nl:"
   FROM dcp_mp_pl_comm_patient from_comm_patient
   WHERE (from_comm_patient.person_id=request->xxx_combine[icombine].from_xxx_id)
    AND (from_comm_patient.encntr_id=request->xxx_combine[icombine].encntr_id)
   DETAIL
    from_row_count = (from_row_count+ 1)
    IF (mod(from_row_count,10)=1)
     stat = alterlist(rreclist->from_rec,(from_row_count+ 9))
    ENDIF
    rreclist->from_rec[from_row_count].from_id = from_comm_patient.dcp_mp_pl_comm_patient_id,
    rreclist->from_rec[from_row_count].active_ind = from_comm_patient.active_ind
   FOOT REPORT
    stat = alterlist(rreclist->from_rec,from_row_count)
   WITH forupdatewait(from_comm_patient)
  ;end select
 ...
 IF (from_row_count > 0)
  SELECT INTO "nl:"
   ...
   WHERE (to_comm_patient.person_id=request->xxx_combine[icombine].to_xxx_id)
   DETAIL
   ...
   UPDATE  FROM dcp_mp_pl_comm_patient from_comm_patient
    SET from_comm_patient.updt_cnt = (from_comm_patient.updt_cnt+ 1), from_comm_patient.updt_id =
     ...
     from_comm_patient.updt_task = reqinfo->updt_task, from_comm_patient.updt_dt_tm = cnvtdatetime(
      curdate,curtime3), from_comm_patient.person_id = s_uf_to_fk_id
