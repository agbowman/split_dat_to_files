CREATE PROGRAM cnt_imp_equations
 CALL parser("dm2_set_context 'FIRE_REFCHG_TRG','NO' go")
 CALL parser("rdb alter table cnt_equation disable constraint XFK1CNT_EQUATION go")
 CALL parser(
  "rdb alter table cnt_equation_component disable constraint XFK1CNT_EQUATION_COMPONENT go")
 DECLARE new_struct = vc WITH noconstant(" ")
 SET new_struct = validate(request->equation_list[1].equation.age_from_minutes,"N")
 IF (new_struct="N")
  SET curalias equation_struct request->equation_obj_list[eq_a].equation_obj
 ELSE
  SET curalias equation_struct request->equation_list[eq_a].equation
 ENDIF
 SET curalias eq_comp_struct request->equation_list[eq_a].equation.equation_component_list[eqc_a].
 equation_component
 ...
 DECLARE prev_eq_uid = vc WITH noconstant("")
 DECLARE ita_uid = vc WITH noconstant("")
 DECLARE eq_uid = vc WITH noconstant("")
 DECLARE eq_a = i4 WITH noconstant(0)
 DECLARE eqc_a = i4 WITH noconstant(0)
 ...
 DECLARE tmp_equation_id = f8 WITH noconstant(0.0)
 DECLARE compare_dt_tm = dq8
 DECLARE equation_replace = i2 WITH noconstant(0)
 DECLARE new_equation = i2 WITH noconstant(0)
 DECLARE pos = i4 WITH noconstant(0)
 DECLARE num = i4 WITH noconstant(0)
 DECLARE overwrite_equation = i2 WITH noconstant(0)
 FREE RECORD tmp_dta
 RECORD tmp_dta(
   1 qual[*]
     2 dta_uid = vc
 )
 IF (validate(request->equation_obj_list[1].equation_obj.exported_dt_tm,0) != 0)
  SET compare_dt_tm = request->equation_obj_list[1].equation_obj.exported_dt_tm
 ELSE
 ...
 SUBROUTINE (add_item_da_dta_relation(wv_item_guid=vc,da_assay_cd_uid=vc) =i2)
   ...
   RETURN(return_val)
 END ;Subroutine
 DECLARE struct_size = i4 WITH noconstant(0)
 IF (new_struct="N")
  SET struct_size = size(request->equation_obj_list,5)
 ELSE
  SET struct_size = size(request->equation_list,5)
 ENDIF
 IF (new_struct="N")
  FOR (eq_a = 1 TO struct_size)
    SET prev_eq_uid = eq_uid
    SET ta_uid = ""
    SET equation_replace = 0
    IF ((equation_struct->task_assay_uid > " "))
     SET ta_uid = equation_struct->task_assay_uid
     SET ta_uid = replace(ta_uid,"UNKNOWN!","CERNER!",1)
    ELSEIF ((equation_struct->dta_mnemonic > " "))
     SET ta_uid = concat("TEMP!",equation_struct->dta_mnemonic)
     SET ta_uid = cnt_dta_chk_uid(ta_uid)
    ENDIF
    SET eq_uid = ""
    IF ((equation_struct->equation_uid > " "))
     SET eq_uid = equation_struct->equation_uid
    ELSE
     SET eq_uid = concat(ta_uid,"!",trim(equation_struct->serv_cduid),"!",trim(equation_struct->
       sex_code_cduid),
      "!")
     SET eq_uid = concat(eq_uid,trim(equation_struct->species_cduid),"!",trim(cnvtstring(
        equation_struct->age_from_minutes)),"!")
     SET eq_uid = concat(eq_uid,trim(cnvtstring(equation_struct->age_to_minutes)))
     SET eq_uid = substring(1,100,eq_uid)
    ENDIF
    SELECT INTO "cnt_imp_equations.log"
     FROM dual
     DETAIL
      col 0, "eq_uid:", col 10,
      eq_uid, row + 1
    ...
    ;end select
    SET pos = 0
    IF (eq_a > size(tmp_dta->qual,5))
     SET stat = alterlist(tmp_dta->qual,(eq_a+ 5))
    ENDIF
    SET pos = locateval(num,1,size(tmp_dta->qual,5),ta_uid,tmp_dta->qual[num].dta_uid)
    IF (pos=0
     AND equation_replace=1)
     SET overwrite_equation = 1
     SELECT INTO "nl:"
      FROM cnt_equation c,
       cnt_dta_key2 cd
      PLAN (cd
      ...
      DETAIL
       overwrite_equation = 0
      WITH check
     ;end select
     IF (overwrite_equation=1)
      DELETE  FROM cnt_equation_component c
       WHERE ((c.task_assay_uid=ta_uid) OR (c.equation_uid=eq_uid))
       WITH check
      ;end delete
     ...
     ENDIF
    ENDIF
    SET tmp_dta->qual[eq_a].dta_uid = ta_uid
    IF (equation_replace=1)
     SET age_to_min = 0
     SET age_from_min = 0
     SELECT INTO "nl:"
      FROM cnt_code_value_key c
      PLAN (c
       WHERE c.code_set=340
        AND (((equation_struct->age_from_units_cduid=c.code_value_uid)) OR (concat(
        "&DISPLAY_KEY&340&",trim(cnvtupper(cnvtalphanum(equation_struct->age_from_units_display))))=c
       .code_value_uid_alias)) )
      DETAIL
       CASE (c.code_value_uid)
        OF cduid_years:
         age_from_min = (min_year * equation_struct->age_from_minutes)
        OF cduid_months:
         age_from_min = (min_month * equation_struct->age_from_minutes)
        OF cduid_weeks:
         age_from_min = (min_week * equation_struct->age_from_minutes)
        OF cduid_days:
         age_from_min = (min_day * equation_struct->age_from_minutes)
        OF cduid_hours:
         age_from_min = (min_hour * equation_struct->age_from_minutes)
        OF cduid_minutes:
         age_from_min = (min_minute * equation_struct->age_from_minutes)
       ENDCASE
      WITH check
     ;end select
     IF (eq_uid != prev_eq_uid)
      ...
      SELECT INTO "nl:"
       FROM cnt_equation c
       PLAN (c
        WHERE c.equation_uid=eq_uid)
       DETAIL
        new_equation = 0, tmp_equation_id = c.cnt_equation_id
       WITH check
      ;end select
      IF (new_equation=0)
       UPDATE  FROM cnt_equation c
        SET c.active_ind = 1, c.age_from_minutes = equation_struct->age_from_minutes, c
         .age_from_units_cd = 0.0,
         c.age_from_units_cduid =
         IF ((equation_struct->age_from_units_cduid > " ")) trim(equation_struct->
           age_from_units_cduid)
         ELSEIF ((equation_struct->age_from_units_display > " ")) concat("&DISPLAY_KEY&340&",trim(
            cnvtupper(cnvtalphanum(equation_struct->age_from_units_display))))
         ELSE " "
         ENDIF
         , c.age_to_minutes = equation_struct->age_to_minutes, c.age_to_units_cd = 0.0,
         c.age_to_units_cduid =
         IF ((equation_struct->age_to_units_cduid > " ")) trim(equation_struct->age_to_units_cduid)
         ELSEIF ((equation_struct->age_to_units_display > " ")) concat("&DISPLAY_KEY&340&",trim(
            cnvtupper(cnvtalphanum(equation_struct->age_to_units_display))))
         ELSE " "
         ENDIF
         , c.cnt_dta_key_id = tmp_dta_key_id, c.default_ind = equation_struct->default_ind,
         c.equation_descripton = equation_struct->equation_description, c.equation_postfix =
         equation_struct->equation_postfix, c.gestational_age_ind = equation_struct->
         gestational_age_ind,
         c.script_name = equation_struct->script, c.service_resource_cd = 0.0, c
         .service_resource_cduid =
         IF ((equation_struct->serv_cduid > " ")) trim(equation_struct->serv_cduid)
         ELSEIF (trim(equation_struct->serv_cd_display) > " ") concat("&DISPLAY&221&",trim(
            equation_struct->serv_cd_display))
         ELSE " "
         ENDIF
         ,
         c.sex_cd = 0.0, c.sex_cduid =
         IF ((equation_struct->sex_code_cduid > " ")) trim(equation_struct->sex_code_cduid)
         ELSEIF (trim(equation_struct->sex_code_display) > " ") concat("&DISPLAY_KEY&57&",trim(
            cnvtalphanum(cnvtupper(equation_struct->sex_code_display))))
         ELSE " "
         ENDIF
         , c.species_cd = 0.0,
         c.species_cduid =
         IF ((equation_struct->species_cduid > " ")) trim(equation_struct->species_cduid)
         ELSEIF (trim(equation_struct->species_cd_display) > " ") concat("&DISPLAY&226&",trim(
            equation_struct->species_cd_display))
         ELSE " "
         ENDIF
         , c.task_assay_uid = ta_uid, c.unknown_age_ind = equation_struct->unknown_age_ind,
         c.updt_applctx = reqinfo->updt_applctx, c.updt_cnt = (c.updt_cnt+ 1), c.updt_dt_tm =
         cnvtdatetime(sysdate),
       ...
       ;end update
      ELSE
       SET tmp_equation_id = 0.0
       SELECT INTO "nl:"
        tmp_id = seq(reference_seq,nextval)
        FROM dual
        DETAIL
         tmp_equation_id = tmp_id
        WITH format, counter
       ;end select
       INSERT  FROM cnt_equation c
        SET c.active_ind = 1, c.age_from_minutes = equation_struct->age_from_minutes, c
         ...
         ELSE " "
         ENDIF
         , c.cnt_dta_key_id = tmp_dta_key_id, c.cnt_equation_id = tmp_equation_id,
         c.default_ind = equation_struct->default_ind, c.equation_descripton = equation_struct->
         equation_description, c.equation_id = 0.0,
         c.equation_postfix = equation_struct->equation_postfix, c.equation_uid = eq_uid, c
         .gestational_age_ind = equation_struct->gestational_age_ind,
         c.script_name = equation_struct->script, c.service_resource_cd = 0.0, c
         .service_resource_cduid =
         IF ((equation_struct->serv_cduid > " ")) trim(equation_struct->serv_cduid)
         ELSEIF (trim(equation_struct->serv_cd_display) > " ") concat("&DISPLAY&221&",trim(
     ...
     ENDIF
     SET ita_uid = ""
     IF ((equation_struct->included_assay_uid > " "))
      SET ita_uid = equation_struct->included_assay_uid
      SET ita_uid = replace(ita_uid,"UNKNOWN!","CERNER!",1)
     ELSEIF ((equation_struct->comp_mnemonic > " "))
      SET ita_uid = concat("TEMP!",equation_struct->comp_mnemonic)
      SET ita_uid = cnt_dta_chk_uid(ita_uid)
     ENDIF
     SELECT INTO "nl:"
      FROM cnt_equation_component c
      WHERE c.equation_uid=eq_uid
       AND (c.component_sequence=equation_struct->sequence)
      WITH check
     ;end select
     IF (curqual < 1)
      INSERT  FROM cnt_equation_component cec
       SET cec.age_ind = equation_struct->age_ind, cec.cnt_equation_component_id = seq(reference_seq,
         nextval), cec.cnt_equation_id = tmp_equation_id,
        cec.component_flag = equation_struct->component_flag, cec.component_name = equation_struct->
        name, cec.component_sequence = equation_struct->sequence,
        cec.constant_value = equation_struct->constant_value, cec.cross_drawn_dt_tm_ind =
        equation_struct->cross_drawn_dt_tm_ind, cec.equation_uid = eq_uid,
        cec.included_assay_uid = ita_uid, cec.look_time_direction_flag = 0, cec.octal_value =
        equation_struct->octal_value,
        cec.race_ind = equation_struct->race_ind, cec.result_req_flag = equation_struct->
        result_req_flag, cec.result_status_cd = 0.0,
        cec.result_status_cduid =
        IF ((equation_struct->result_status_cduid > " ")) trim(equation_struct->result_status_cduid)
        ELSEIF (trim(equation_struct->result_status_display) > " ") concat("&DISPLAY&1901&",trim(
           equation_struct->result_status_display))
        ELSE " "
        ENDIF
        , cec.sex_ind = equation_struct->sex_ind, cec.time_window_back_minutes = equation_struct->
        time_window_back_minutes,
        cec.time_window_minutes = equation_struct->time_window_minutes, cec.units_cd = 0.0, cec
        .units_cduid =
        IF ((equation_struct->units_cduid > " ")) trim(equation_struct->units_cduid)
        ELSEIF (trim(equation_struct->units_cd_display) > " ") concat("&DISPLAY&54&",trim(
           equation_struct->units_cd_display))
        ELSE " "
        ENDIF
        ,
        cec.updt_applctx = reqinfo->updt_applctx, cec.updt_cnt = 0, cec.updt_dt_tm = sysdate,
        cec.updt_id = reqinfo->updt_id, cec.updt_task = reqinfo->updt_task, cec.variable_prompt =
        equation_struct->variable_prompt,
        cec.task_assay_uid = ta_uid
      ;end insert
     ENDIF
    ENDIF
  ENDFOR
  ...
  FOR (eq_a = 1 TO struct_size)
    ...
    INSERT  FROM cnt_equation c
     SET c.active_ind = 1, c.age_from_minutes = equation_struct->age_from_minutes, c
      .age_from_units_cd = 0.0,
      c.age_from_units_cduid =
      IF ((equation_struct->age_from_units_disp > " ")) concat("&DISPLAY&66&",trim(equation_struct->
         age_from_units_disp))
      ELSE " "
      ENDIF
      , c.age_to_minutes = equation_struct->age_to_minutes, c.age_to_units_cd = 0.0,
      c.age_to_units_cduid =
      IF ((equation_struct->age_to_units_disp > " ")) concat("&DISPLAY&66&",trim(equation_struct->
         age_to_units_disp))
      ELSE " "
      ENDIF
      , c.cnt_dta_key_id = 0.0, c.cnt_equation_id = seq(reference_seq,nextval),
      c.default_ind = equation_struct->defalut_ind, c.equation_descripton = equation_struct->
      equation_description, c.equation_id = 0.0,
      c.equation_postfix = equation_struct->equation_postfix, c.gestational_age_ind = equation_struct
      ->gestational_age_ind, c.script_name = equation_struct->script,
      c.service_resource_cd = 0.0, c.service_resource_cduid =
      IF ((equation_struct->service_resource_disp > " ")) concat("&DISPLAY&221&",trim(equation_struct
         ->service_resource_disp))
      ELSE " "
      ENDIF
      , c.sex_cd = 0.0,
      c.sex_cduid =
      IF ((equation_struct->sex_disp > " ")) concat("&DISPLAY&57&",trim(equation_struct->sex_disp))
      ELSE " "
      ENDIF
      , c.species_cd = 0.0, c.species_cduid =
      IF ((equation_struct->species_disp > " ")) concat("&DISPLAY&226&",trim(equation_struct->
         species_disp))
      ELSE " "
      ENDIF
      ,
      c.task_assay_uid = ta_uid, c.unknown_age_ind = equation_struct->unknown_age_ind, c.updt_applctx
       = reqinfo->updt_applctx,
 ...
 ;end update
 UPDATE  FROM cnt_equation_component c
  SET c.result_status_cduid =
   (SELECT
    cv.code_value_uid
    FROM cnt_code_value_key cv
    WHERE cv.code_value_uid_alias=c.result_status_cduid), c.result_status_cd =
   (SELECT
    cv2.code_value
    FROM cnt_code_value_key cv2
    WHERE cv2.code_value_uid_alias=c.result_status_cduid)
  WHERE c.result_status_cduid="&*"
   AND c.result_status_cduid IN (
  (SELECT
   cv.code_value_uid_alias
   FROM cnt_code_value_key cv
   WHERE c.result_status_cduid=cv.code_value_uid_alias))
  WITH nocounter
 ;end update
 UPDATE  FROM cnt_equation_component c
  SET c.result_status_cd =
   (SELECT
    cv2.code_value
    FROM cnt_code_value_key cv2
    WHERE cv2.code_value_uid=c.result_status_cduid)
  WHERE c.result_status_cd=0.0
   AND c.result_status_cduid IN (
  (SELECT
   cv.code_value_uid
   FROM cnt_code_value_key cv
   WHERE c.result_status_cduid=cv.code_value_uid
    AND cv.cnt_code_value_key_id != 0.00))
  WITH nocounter
 ;end update
#end_of_script
