CREATE PROGRAM dm_rmc_task_mon:dba
 ...
 SUBROUTINE drtq_view_task_process(i_process_name)
   ...
   SET message = nowindow
 END ;Subroutine
 DECLARE drtm_process_name = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm_rmc_task_mon",".log","DM_RMC_TASK_MON LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm_rmc_task_mon"
 CALL disp_msg("",dm_err->logfile,0)
 IF (reflect(parameter(1,0)) != "C*")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Expected syntax:  dm_rmc_task_mon <process_name>"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSE
  SET drtm_process_name =  $1
 ENDIF
 SET dm_err->eproc = concat("Displaying progress for ",drtm_process_name)
 CALL disp_msg("",dm_err->logfile,0)
 CALL drtq_view_task_process(drtm_process_name)
#exit_program
 SET dm_err->eproc = "...Ending dm_rmc_task_mon"
 CALL final_disp_msg("dm_rmc_task_mon")
END GO
