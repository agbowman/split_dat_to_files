CREATE PROGRAM dcp_upd_prob_icd9_snmct_imo_cc:dba
 ...
 INSERT  FROM temp_prob_icd9_snmct_imo
  ...
  new_problem_instance_id)(SELECT
   ...
   FROM nomenclature n,
    problem p,
    nomenclature n2
   WHERE n.source_vocabulary_cd IN (dicd9, dsnmct)
 ...
 DELETE  FROM temp_prob_icd9_snmct_imo
  WHERE problem_instance_id IN (
  (SELECT
   temp.problem_instance_id
   FROM temp_prob_icd9_snmct_imo temp
 ...
 WHILE (temp_cnt > 0)
   ...
   UPDATE  FROM temp_prob_icd9_snmct_imo temp
 ...
 IF (sbr_upd_id_evaluated(mf_max_id,ms_child_c_info_domain_nm,ms_child_c_max_name)=0)
  GO TO exit_program
 ENDIF
 CALL sbr_truncate_table("TEMP_PROB_ICD9_SNMCT_IMO")
#exit_program
 IF (checkprg("DM2_SET_CONTEXT") > 0)
