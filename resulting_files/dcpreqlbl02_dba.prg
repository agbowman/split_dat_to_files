CREATE PROGRAM dcpreqlbl02:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  ph.phone_id, pa.person_id, o.current_start_dt_tm
  FROM orders o,
   order_action oa,
   prsnl p,
   prsnl p2,
  ...
  SELECT INTO value(request->printer_name)
   ...
   DETAIL
    FOR (vv = 1 TO value(num_of_orders))
      ...
      row + 1, "{CPI/13}{POS/135/670}{b}", orders->name,
      row + 1, "{CPI/13}{POS/135/680}{b}", orders->fnbr,
      row + 1, "{CPI/13}{POS/185/680}", "Pt Loc: ",
      orders->location, row + 1, "{CPI/13}{POS/135/690}",
      CALL print(trim(provider)), " ", orders->qual[vv].order_provider_name,
      row + 1, "{CPI/13}{POS/135/700}", orders->age,
      "  ",
      CALL print(substring(1,1,orders->sex)), "  ",
      orders->dob, " SS#", orders->ssn,
      ...
      CALL print(trim(provider)), " ", orders->qual[vv].order_provider_name,
      row + 1, "{CPI/13}{POS/315/700}", orders->age,
      "  ",
      CALL print(substring(1,1,orders->sex)), "  ",
      orders->dob, " SS#", orders->ssn,
      row + 1, "{CPI/13}{POS/315/710}", orders->qual[vv].order_mnemonic,
      row + 1, "{CPI/13}{POS/35/740}", "Printed Dt/Tm: ",
      curdate"MM/DD/YYYY;;D", " ", curtime"HH:MM;;s",
      "{CPI/13}{POS/195/740}", "Ordered Dt/Tm: ", signed,
