CREATE PROGRAM dcp_upd_prob_icd9_snmct_hli_p:dba
 ...
 IF (error(ms_errmsg,0) != 0)
  ...
  GO TO exit_program
 ENDIF
 IF (((dicd9 <= 0.0) OR (((dsnmct <= 0.0) OR (dhli <= 0.0)) )) )
  SET readme_data->status = "F"
  SET readme_data->message = "Code values for ICD9, SNMCT or HLI in Codeset 400 do not exist"
  GO TO exit_program
 ENDIF
 IF (((dinactive <= 0.0) OR (dactive <= 0.0)) )
  SET readme_data->status = "F"
  SET readme_data->message = "Code values for INACTIVE or ACTIVE in Codeset 48 do not exist"
  GO TO exit_program
 ENDIF
 IF (dauth <= 0.0)
  SET readme_data->status = "F"
  SET readme_data->message = "Code values for AUTH in Codeset 8 do not exist"
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  max_val = max(person_id), min_val = min(person_id)
  FROM problem
  WHERE person_id > 0.0
  DETAIL
   mf_max_range_id = cnvtreal(max_val), mf_min_range_id = maxval(cnvtreal(min_val),1.0)
  WITH nocounter
 ;end select
 IF (error(ms_errmsg,0) != 0)
  SET readme_data->status = "F"
 ...
 IF (sbr_run_parent(mf_max_range_id,mf_min_range_id,ms_range_prefix,ms_info_domain_nm,mn_num_children
  ...
  GO TO exit_program
 ENDIF
 CALL sbr_add_sql_stmt("TABLE","TEMP_PROB_ICD9_SNMCT_HLI",concat(
   "create global temporary table TEMP_PROB_ICD9_SNMCT_HLI(person_id number,",
   "problem_instance_id number,","dest_nomenclature_id number,","new_problem_instance_id number,",
   "new_problem_instance_uuid varchar2(255)) on commit preserve rows"),"Creating temporary table")
 CALL sbr_add_sql_stmt("INDEX","XIETEMPPROBICD9SNMCTHLI",
  "create index XIETEMPPROBICD9SNMCTHLI ON TEMP_PROB_ICD9_SNMCT_HLI(problem_instance_id)",
  "Creating index")
