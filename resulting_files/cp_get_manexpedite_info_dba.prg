CREATE PROGRAM cp_get_manexpedite_info:dba
 FREE RECORD reply
 RECORD reply(
   1 person_id = f8
   1 honor_org_sec_ind = i2
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = trim(curprog)
 ...
 SELECT INTO "nl:"
  ...
  FROM prsnl p
  WHERE p.username=trim(cnvtupper(request->username))
  HEAD REPORT
 ...
 SELECT INTO "nl:"
  d.info_domain, d.info_name
  FROM dm_info d
  WHERE d.info_domain="CLINICAL REPORTING XR"
   AND d.info_name="TEMPLATE ORG FILTERING"
  HEAD REPORT
   reply->honor_org_sec_ind = d.info_number
  WITH nocounter
 ;end select
#exit_script
