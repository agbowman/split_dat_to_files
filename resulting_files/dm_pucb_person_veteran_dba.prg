CREATE PROGRAM dm_pucb_person_veteran:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PUCB_PERSON_VETERAN"
 ...
 SUBROUTINE cust_ucb_upt(dummy)
   IF ( NOT (restore_column_values("PERSON_VETERAN",request->parent_table,request->xxx_uncombine[
    ucb_cnt].xxx_combine_id,request->xxx_uncombine[ucb_cnt].from_xxx_id,request->xxx_uncombine[
    ucb_cnt].to_xxx_id)))
 ...
 SUBROUTINE initialize_hist_id(null)
  IF (v_hist_init_ind=0)
   ...
   SET dcipht_request->transaction_reason_txt = "DM_PUCB_PERSON_VETERAN"
