CREATE PROGRAM dm2_aload_code_value_group
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_aload_cvg",".log","dm2_aload_code_value_group")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_aload_code_value_group"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dacvg_pkg = i4 WITH protect, noconstant( $1)
 DECLARE dacvg_file = vc WITH protect, noconstant( $2)
 DECLARE dacvg_row_cnt = i4 WITH protect, noconstant( $3)
 DECLARE dacvg_batch = i4 WITH protect, noconstant(0)
 DECLARE dacvg_rows_returned = i4 WITH protect, noconstant(10000)
 DECLARE dacvg_file_loc_ret = vc WITH protect, noconstant("")
 IF (dac_prelim(dacvg_pkg,dacvg_file_loc_ret,dacvg_batch)=0)
  GO TO exit_program
 ENDIF
 IF (dacvg_batch=0)
  SET dacvg_batch = 5000
 ENDIF
 SET dm_err->eproc = "Clean existing data from DM_AFD_CODE_VALUE_GROUP for package."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 WHILE ((dm_err->err_ind=0)
  AND dacvg_rows_returned=10000)
   DELETE  FROM dm_afd_code_value_group d
    WHERE d.alpha_feature_nbr=dacvg_pkg
   ...
   ELSE
    COMMIT
   ENDIF
   SET dacvg_rows_returned = curqual
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("Rows deleted:",dacvg_rows_returned))
   ENDIF
 ENDWHILE
 IF (dacvg_row_cnt > 0)
  EXECUTE dm2_dbimport concat(dacvg_file_loc_ret,dacvg_file), "dm2_aload_cs_cvg", dacvg_batch
  CALL echo(concat(dacvg_file_loc_ret,dacvg_file))
 ENDIF
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_aload_code_value_group completed."
 ELSE
  CALL parser(";x",1)
 ENDIF
 CALL final_disp_msg("Dm2_aload_code_value_group")
END GO
