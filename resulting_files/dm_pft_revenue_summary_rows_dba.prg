CREATE PROGRAM dm_pft_revenue_summary_rows:dba
 ...
 FOR (tok_ndx = 1 TO size(request->tokens,5))
   IF ((request->tokens[tok_ndx].token_str="MONTHSTOKEEP"))
    SET v_months_to_keep = ceil(cnvtreal(request->tokens[tok_ndx].value))
   ENDIF
 ENDFOR
 IF (v_months_to_keep < 1)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"MONTHSTOKEEP",
   "You must keep at least 1 month's worth of data.  You entered %1 months or did not enter any value.",
   "i",v_months_to_keep)
  GO TO exit_script
 ENDIF
 SET purge_dt_tm = cnvtagedatetime(0,v_months_to_keep,0,0)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(prs.pft_revenue_summary_id)
   FROM pft_revenue_summary prs
   WHERE prs.pft_revenue_summary_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(prs.pft_revenue_summary_id)
  FROM pft_revenue_summary prs
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    prs.rowid
    FROM pft_revenue_summary prs
    WHERE parser(sbr_getrowidnotexists("prs.pft_revenue_summary_id between curMinID and curMaxID",
      "prs"))
     AND prs.updt_dt_tm <= cnvtdatetime(purge_dt_tm)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,50)=1)
      stat = alterlist(reply->rows,(rows+ 49))
     ENDIF
     reply->rows[rows].row_id = prs.rowid
    WITH nocounter, maxqual(prs,value(rowsleft))
   ;end select
