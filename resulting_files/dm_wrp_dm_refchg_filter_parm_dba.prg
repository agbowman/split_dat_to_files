CREATE PROGRAM dm_wrp_dm_refchg_filter_parm:dba
 ...
 SET readme_data->message = "Readme Failed: Starting script dm_wrp_dm_refchg_filter_parm.prg..."
 DECLARE dwdrf_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dwdrf_oe_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm_wrp_dm_fltr",".log","dm_wrp_dm_refchg_filter_parm LOG FILE...") != 1)
  SET dm_err->err_ind = 1
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dwdrf_oe_ind = check_open_event(dwdrf_env_id,0.0)
 IF (check_error(dm_err->eproc)=1)
  SET readme_data->status = "F"
 ...
 ENDIF
 IF (dwdrf_oe_ind=0)
  EXECUTE dm_dbimport "cer_install:dm_refchg_filter_parm.csv", "dm_imp_dm_refchg_filter_parm", 100
 ELSE
  SET readme_data->status = "S"
  SET readme_data->message =
  "SUCCESS: DM_REFCHG_FILTER* tables were not loaded do to an already open event"
 ENDIF
#exit_script
