CREATE PROGRAM bmdi_get_assoc_by_device:dba
 RECORD reply(
   1 assoc_list[*]
     2 association_id = f8
 ...
 DECLARE indexwhile = i2 WITH private, noconstant(0)
 DECLARE current_location_cd = f8 WITH noconstant(0.0)
 DECLARE i = i2 WITH noconstant(0)
 DECLARE a = i2 WITH private, noconstant(0)
 DECLARE indexreqlist = i4 WITH private, noconstant(0)
 DECLARE cnt = i4 WITH private, noconstant(0)
 SET indexreqlist = size(request->assoc_list,5)
 DECLARE reqstr = vc WITH private, noconstant("")
 DECLARE hierarchy = i2 WITH noconstant(0)
 FOR (a = 1 TO indexreqlist)
  SELECT
   IF (value(request->assoc_list[a].association_id) > 0)
    PLAN (d)
     JOIN (badt
     WHERE (badt.association_id=request->assoc_list[d.seq].association_id)
   ...
   ELSEIF (value(request->assoc_list[a].device_id) > 0)
    PLAN (d)
     JOIN (bmd
     WHERE (bmd.monitored_device_id=request->assoc_list[d.seq].device_id))
     JOIN (badt
   ...
   ELSE
    PLAN (d)
     JOIN (bmd
     WHERE (bmd.device_alias=request->assoc_list[d.seq].device_barcode))
     JOIN (badt
     WHERE badt.device_cd=bmd.device_cd
      AND badt.location_cd=bmd.location_cd
      AND badt.active_ind=1)
   ...
   ENDIF
   INTO "nl:"
   FROM (dummyt d  WITH seq = value(a)),
   ...
   DETAIL
    ...
    reply->assoc_list[cnt].device_category_type_cd = sm.device_category_type_cd, reply->assoc_list[
    cnt].device_category_type_disp = uar_get_code_display(sm.device_category_type_cd), reply->
    assoc_list[cnt].device_category_type_desc = uar_get_code_description(sm.device_category_type_cd),
    reply->assoc_list[cnt].device_category_type_mean = uar_get_code_meaning(sm
     .device_category_type_cd), reply->assoc_list[cnt].person_alias_type_cd = bapr
    .person_alias_type_cd, reply->assoc_list[cnt].person_alias_type_disp = uar_get_code_display(bapr
     .person_alias_type_cd),
    reply->assoc_list[cnt].person_alias_type_desc = uar_get_code_description(bapr
     .person_alias_type_cd), reply->assoc_list[cnt].person_alias_type_mean = uar_get_code_meaning(
     bapr.person_alias_type_cd), reply->assoc_list[cnt].person_alias = bapr.person_alias,
    reply->assoc_list[cnt].person_id = badt.person_id, reply->assoc_list[cnt].person_name = bapr
    .person_name, reply->assoc_list[cnt].encntr_id = bapr.encntr_id,
    reply->assoc_list[cnt].encntr_alias_type_cd = bapr.encntr_alias_type_cd, reply->assoc_list[cnt].
    encntr_alias_type_disp = uar_get_code_display(bapr.encntr_alias_type_cd), reply->assoc_list[cnt].
    encntr_alias_type_desc = uar_get_code_description(bapr.encntr_alias_type_cd),
    reply->assoc_list[cnt].encntr_alias_type_mean = uar_get_code_meaning(bapr.encntr_alias_type_cd),
    reply->assoc_list[cnt].encntr_alias = bapr.encntr_alias, reply->assoc_list[cnt].person_weight =
    bapr.person_weight,
    reply->assoc_list[cnt].weight_units_cd = bapr.weight_units_cd, reply->assoc_list[cnt].
    weight_units_disp = uar_get_code_display(bapr.weight_units_cd), reply->assoc_list[cnt].
    weight_units_desc = uar_get_code_description(bapr.weight_units_cd),
    reply->assoc_list[cnt].weight_units_mean = uar_get_code_meaning(bapr.weight_units_cd), reply->
    assoc_list[cnt].person_height = bapr.person_height, reply->assoc_list[cnt].height_units_cd = bapr
    .height_units_cd,
    reply->assoc_list[cnt].height_units_disp = uar_get_code_display(bapr.height_units_cd), reply->
    assoc_list[cnt].height_units_desc = uar_get_code_description(bapr.height_units_cd), reply->
    assoc_list[cnt].height_units_mean = uar_get_code_meaning(bapr.height_units_cd),
    reply->assoc_list[cnt].person_gender_cd = bapr.person_gender_cd, reply->assoc_list[cnt].
    person_gender_disp = uar_get_code_display(bapr.person_gender_cd), reply->assoc_list[cnt].
    person_gender_desc = uar_get_code_description(bapr.person_gender_cd),
    reply->assoc_list[cnt].person_gender_mean = uar_get_code_meaning(bapr.person_gender_cd), reply->
    assoc_list[cnt].person_birth_dt_tm = bapr.person_birth_dt_tm, reply->assoc_list[cnt].mobile_ind
     = bmd.mobile_ind,
    reply->assoc_list[cnt].association_limit_cnt = bmd.association_limit_cnt, reply->assoc_list[cnt].
    status_flag = bapr.status_flag, reply->assoc_list[cnt].status_message = bapr.status_message,
    reply->assoc_list[cnt].association_prsnl_id = bapr.association_prsnl_id, reply->assoc_list[cnt].
    dis_association_prsnl_id = bapr.dis_association_prsnl_id
  ...
  IF (curqual < 1)
   SELECT
    IF (value(request->assoc_list[a].device_id) > 0)
     PLAN (d)
      JOIN (bmd
      WHERE (bmd.monitored_device_id=request->assoc_list[d.seq].device_id))
      JOIN (badt
    ...
    DETAIL
     ...
     IF (mod(cnt,10)=1)
      stat = alterlist(reply->assoc_list,(cnt+ 9))
     ENDIF
     reply->assoc_list[cnt].device_id = bmd.monitored_device_id, reply->assoc_list[cnt].device_alias
      = bmd.device_alias, reply->assoc_list[cnt].device_cd = badt.device_cd,
     reply->assoc_list[cnt].device_disp = uar_get_code_display(bmd.device_cd), reply->assoc_list[cnt]
     .device_desc = uar_get_code_description(bmd.device_cd), reply->assoc_list[cnt].device_mean =
     uar_get_code_meaning(bmd.device_cd),
     reply->assoc_list[cnt].device_category_cd = sm.device_category_cd, reply->assoc_list[cnt].
     device_category_disp = uar_get_code_display(sm.device_category_cd), reply->assoc_list[cnt].
     device_category_desc = uar_get_code_description(sm.device_category_cd),
     reply->assoc_list[cnt].device_category_mean = uar_get_code_meaning(sm.device_category_cd), reply
     ->assoc_list[cnt].device_category_type_cd = sm.device_category_type_cd, reply->assoc_list[cnt].
     device_category_type_disp = uar_get_code_display(sm.device_category_type_cd),
     reply->assoc_list[cnt].device_category_type_desc = uar_get_code_description(sm
      .device_category_type_cd), reply->assoc_list[cnt].device_category_type_mean =
     uar_get_code_meaning(sm.device_category_type_cd), reply->assoc_list[cnt].mobile_ind = bmd
     .mobile_ind,
     reply->assoc_list[cnt].association_limit_cnt = bmd.association_limit_cnt, stat = alterlist(reply
      ->assoc_list[cnt].loc_list,1), reply->assoc_list[cnt].loc_list[1].location_cd = badt
     .location_cd,
     reply->assoc_list[cnt].loc_list[1].location_disp = uar_get_code_display(badt.location_cd), reply
     ->assoc_list[cnt].loc_list[1].location_desc = uar_get_code_description(badt.location_cd), reply
     ->assoc_list[cnt].loc_list[1].location_mean = uar_get_code_meaning(badt.location_cd),
     reply->assoc_list[cnt].loc_list[1].codeset = uar_get_code_set(badt.location_cd,reply->
      assoc_list[cnt].loc_list[1].location_disp,reply->assoc_list[cnt].loc_list[1].location_mean,
      reply->assoc_list[cnt].loc_list[1].location_desc), reply->assoc_list[cnt].loc_list[1].hierarchy
      = 1
    FOOT REPORT
     stat = alterlist(reply->assoc_list,cnt)
    WITH nocounter
   ;end select
   IF (curqual < 1)
    SET reqstr = request->assoc_list[a].device_barcode
 ...
 FOR (i = 1 TO size(reply->assoc_list,5))
   IF ((reply->assoc_list[i].loc_list[1].codeset=resource_codeset))
    ...
    SELECT INTO "nl:"
     ...
     WHERE sr.service_resource_cd=current_location_cd
     DETAIL
...
#set_exit_status
 IF (i < 1)
  ...
  ELSE
   ...
   IF (size(reqstr,4) > 0)
    SET reply->status_data.subeventstatus[1].targetobjectvalue = concat(reply->status_data.
     subeventstatus[1].targetobjectvalue," - device_alias = ",reqstr)
   ENDIF
  ENDIF
  SET reply->status_data.subeventstatus[1].operationname = "SELECT"
  SET reply->status_data.subeventstatus[1].operationstatus = "F"
  SET reply->status_data.subeventstatus[1].targetobjectname = "bmdi_get_assoc_by_device"
  IF (ierrcode > 0)
   SET reply->status_data.subeventstatus[2].operationname = "SELECT"
   SET reply->status_data.subeventstatus[2].operationstatus = "F"
   SET reply->status_data.subeventstatus[2].targetobjectname = "bmdi_get_assoc_by_device"
