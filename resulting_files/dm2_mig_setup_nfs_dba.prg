CREATE PROGRAM dm2_mig_setup_nfs:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmsn_logfile_prefix = vc WITH protect, constant("dm2_mig_setup_nfs")
 DECLARE dmsn_cmd = vc WITH protect, noconstant("")
 DECLARE dmsn_stg_hostname = vc WITH protect, noconstant("")
 DECLARE dmsn_stg_root_uid = vc WITH protect, noconstant("")
 DECLARE dmsn_stg_root_gid = vc WITH protect, noconstant("")
 DECLARE dmsn_stg_oracle_uid = vc WITH protect, noconstant("")
 DECLARE dmsn_stg_oracle_gid = vc WITH protect, noconstant("")
 DECLARE dmsn_src_hostname = vc WITH protect, noconstant("")
 DECLARE dmsn_src_root_uid = vc WITH protect, noconstant("")
 DECLARE dmsn_src_root_gid = vc WITH protect, noconstant("")
 DECLARE dmsn_src_oracle_uid = vc WITH protect, noconstant("")
 DECLARE dmsn_src_oracle_gid = vc WITH protect, noconstant("")
 DECLARE dmsn_tgt_hostname = vc WITH protect, noconstant("")
 DECLARE dmsn_tgt_root_uid = vc WITH protect, noconstant("")
 DECLARE dmsn_tgt_root_gid = vc WITH protect, noconstant("")
 DECLARE dmsn_tgt_oracle_uid = vc WITH protect, noconstant("")
 DECLARE dmsn_tgt_oracle_gid = vc WITH protect, noconstant("")
 DECLARE dmsn_stg_exp_file = vc WITH protect, noconstant("dm2_mig_stg_exports_tmp.dat")
 DECLARE dmsn_src_exp_file = vc WITH protect, noconstant("dm2_mig_src_exports_tmp.dat")
 DECLARE dmsn_tgt_exp_file = vc WITH protect, noconstant("dm2_mig_tgt_exports_tmp.dat")
 DECLARE dmsn_stg_nfsmnt_ksh = vc WITH protect, noconstant("")
 DECLARE dmsn_tgt_nfsmnt_ksh = vc WITH protect, noconstant("")
 DECLARE dmsn_db_only_migration = i2 WITH protect, noconstant(1)
 DECLARE dmsn_node_name = vc WITH protect, noconstant("")
 DECLARE dmsn_gg_path = vc WITH protect, noconstant("")
 DECLARE dmsn_arch_path = vc WITH protect, noconstant("")
 DECLARE dmsn_exp_exist = i2 WITH protect, noconstant(0)
 DECLARE dmsn_gg_exist = i2 WITH protect, noconstant(0)
 DECLARE dmsn_arch_exist = i2 WITH protect, noconstant(0)
 DECLARE dmsn_gg_cmd = vc WITH protect, noconstant("")
 DECLARE dmsn_arch_cmd = vc WITH protect, noconstant("")
 DECLARE dmsn_src_nfsmnt_com = vc WITH protect, noconstant("")
 DECLARE dmsn_currow = i4 WITH protect, noconstant(0)
 DECLARE dmsn_node = i4 WITH protect, noconstant(0)
 DECLARE dmsn_domain_user = vc WITH protect, noconstant("")
 DECLARE dmsn_work_type = vc WITH protect, noconstant("")
 DECLARE dmsn_from_db_node = i2 WITH protect, noconstant(0)
 DECLARE dmsn_using_ftp = i2 WITH protect, noconstant(0)
 DECLARE dmsn_cap_loc = vc WITH protect, noconstant("")
 DECLARE dmsn_del_loc = vc WITH protect, noconstant("")
 DECLARE dmsn_run_ksh_prompt(drkp_type=vc,drkp_run=i2,drkp_node_type=vc,drkp_ksh_name=vc) = i2
 DECLARE dmsn_collect_data(dck_run=i2,dck_node_type=vc,dck_hostname=vc(ref),dck_root_uid=vc(ref),
  dck_root_gid=vc(ref),
  dck_oracle_uid=vc(ref),dck_oracle_gid=vc(ref)) = i2
 DECLARE dmsn_read_exp_dat(dred_exp_file=vc(ref),dred_exp_exist=i2(ref),dred_gg_exist=i2(ref),
  dred_arch_exist=i2(ref)) = i2
 DECLARE dmsn_get_domain_user(dgdu_domain_user=vc(ref)) = i2
 DECLARE dmsn_mount_dir(dmd_type=vc,dmd_host_name=vc,dmd_node_type=vc) = i2
 SET width = 132
 IF (check_logfile(dmsn_logfile_prefix,".log","dm2_mig_setup_nfs")=0)
  GO TO exit_program
 ENDIF
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Retrieving GG Capture and Delivery Directory paths."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_GG_DATA"
  ...
  DETAIL
   IF (di.info_name="GG_CAP_DIR")
    dmsn_cap_loc = trim(di.info_char,3)
   ELSE
    dmsn_del_loc = trim(di.info_char,3)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSEIF (((size(trim(dmsn_cap_loc,3))=0) OR (size(trim(dmsn_del_loc,3))=0)) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No DM_INFO rows exist for Capture/Delivery Directories path.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (substring(size(trim(dmsn_cap_loc,3)),1,dmsn_cap_loc)="/")
  SET dmsn_cap_loc = trim(replace(dmsn_cap_loc,"/","",2),3)
 ENDIF
 IF (substring(size(trim(dmsn_del_loc,3)),1,dmsn_del_loc)="/")
  SET dmsn_del_loc = trim(replace(dmsn_del_loc,"/","",2),3)
 ENDIF
 SET dm_err->eproc = "Verify FTP usage for file copy."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info
  WHERE info_domain="DM2_MIG_DATA"
   AND info_name="TARGET_FTP_LOGIN"
  DETAIL
   dmsn_using_ftp = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET dmsn_arch_path = concat("/nfsarch/v500/db_",cnvtlower(dmr_mig_data->src_db_name),"/arch")
 IF ((dm_err->debug_flag > 0))
  CALL echo(build("arch_path=",dmsn_arch_path))
 ENDIF
 SET message = window
 CALL clear(1,1)
 CALL box(1,1,15,120)
 CALL text(2,5,"Would you like to perform CAPTURE or DELIVERY work?")
 CALL text(3,5,"Enter 'C' for CAPTURE, 'D' for DELIVERY, or 'Q' to QUIT:")
 CALL accept(3,64,"P;CU"," "
  WHERE curaccept IN ("C", "D", "Q"))
 CALL clear(1,1)
 SET message = nowindow
 CASE (curaccept)
  OF "C":
   SET dmsn_work_type = "CAPTURE"
  OF "D":
   SET dmsn_work_type = "DELIVERY"
  OF "Q":
   SET dm_err->emsg = "User elected to quit from Setup NFS."
   SET dm_err->err_ind = 1
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
 ENDCASE
 SET dmsn_node_name = curnode
 SET message = window
 CALL clear(1,1)
 CALL box(1,1,15,120)
 SET help = fix("AIX,HPX,LNX")
 CALL text(2,2,"What OS type is the TARGET database host (AIX,HPX,LNX)?")
 CALL text(3,2,"Enter the Name of the TARGET database host:")
 CALL accept(2,62,"A(3);CU"," "
  WHERE build(curaccept) IN ("HPX", "AIX", "LNX"))
 SET dmr_mig_data->tgt_db_os = curaccept
 CALL accept(3,48,"P(50);C")
 SET stat = alterlist(dmr_mig_data->tgt_nodes,1)
 SET dmr_mig_data->tgt_nodes[1].node_name = trim(cnvtlower(curaccept),3)
 CALL clear(1,1)
 SET message = nowindow
 IF ((dm2_sys_misc->cur_os="AXP"))
  SET dmsn_from_db_node = 0
 ELSEIF (dmsn_work_type="DELIVERY")
  IF (cnvtupper(dmr_mig_data->tgt_nodes[1].node_name)=cnvtupper(dmsn_node_name))
   SET dmsn_from_db_node = 1
  ENDIF
  CALL echo(concat("HOST NAME:",dmr_mig_data->tgt_nodes[1].node_name))
 ELSE
  SET dm_err->eproc = "Check if running from Database Node"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM gv$instance v
   DETAIL
    IF (cnvtupper(v.host_name)=cnvtupper(dmsn_node_name))
     dmsn_from_db_node = 1
    ENDIF
    CALL echo(concat("HOST NAME:",v.host_name))
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm2_sys_misc->cur_os="AXP"))
  IF (dmsn_get_domain_user(dmsn_domain_user)=0)
   GO TO exit_program
  ENDIF
 ELSE
  IF (cnvtupper(curuser) != "ROOT")
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Making sure current user is the ROOT user."
   SET dm_err->emsg = "Current user is not the ROOT user."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ELSE
   SET dmsn_domain_user = "ROOT"
  ENDIF
 ENDIF
 IF (dmsn_work_type="CAPTURE"
  AND (dm2_sys_misc->cur_os="AXP"))
  IF (dmsn_collect_data(0,"stg",dmsn_stg_hostname,dmsn_stg_root_uid,dmsn_stg_root_gid,
   dmsn_stg_oracle_uid,dmsn_stg_oracle_gid)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmsn_work_type="CAPTURE"
  AND (dm2_sys_misc->cur_os != "AXP"))
  IF (dmsn_collect_data(dmsn_from_db_node,"src",dmsn_src_hostname,dmsn_src_root_uid,dmsn_src_root_gid,
   dmsn_src_oracle_uid,dmsn_src_oracle_gid)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmsn_work_type="DELIVERY")
  IF (dmsn_collect_data(dmsn_from_db_node,"tgt",dmsn_tgt_hostname,dmsn_tgt_root_uid,dmsn_tgt_root_gid,
   dmsn_tgt_oracle_uid,dmsn_tgt_oracle_gid)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmsn_work_type="CAPTURE")
  SET dmsn_gg_path = dmr_mig_data->gg_capture_dir
 ELSEIF (dmsn_work_type="DELIVERY")
  SET dmsn_gg_path = dmr_mig_data->gg_delivery_dir
 ENDIF
 IF (dmsn_work_type="CAPTURE"
  AND (dm2_sys_misc->cur_os="AXP"))
  IF (dmsn_read_exp_dat(dmsn_stg_exp_file,dmsn_exp_exist,dmsn_gg_exist,dmsn_arch_exist)=0)
   GO TO exit_program
  ENDIF
  SET dmsn_stg_nfsmnt_ksh = concat(dm2_install_schema->ccluserdir,"dm2_mig_stg_nfsmnt.ksh")
  SET dm_err->eproc = concat("create ",dmsn_stg_nfsmnt_ksh)
  SELECT INTO value(dmsn_stg_nfsmnt_ksh)
   ...
   DETAIL
    ...
    col 0, "then", row + 1,
    col 0, '    echo " "', row + 1,
    col 0, '    echo "INFORMATION: Logon as root and run again."', row + 1,
    col 0, '    echo " "', row + 1,
    col 0, "    exit 1", row + 1,
    col 0, "fi", row + 2,
    dmsn_cmd = "if [[ ! -d /nfsarch ]]", col 0, dmsn_cmd,
    row + 1, col 0, "then",
    row + 1, dmsn_cmd = concat("   mkdir /nfsarch"), col 0,
    dmsn_cmd, row + 1, col 0,
    "fi", row + 2, dmsn_cmd = "chown oracle:dba /nfsarch",
    col 0, dmsn_cmd, row + 1,
    dmsn_cmd = "if [[ ! -d /nfsarch/v500 ]]", col 0, dmsn_cmd,
    row + 1, col 0, "then",
    row + 1, dmsn_cmd = concat("   mkdir /nfsarch/v500"), col 0,
    dmsn_cmd, row + 1, col 0,
    "fi", row + 2, dmsn_cmd = "chown oracle:dba /nfsarch/v500",
    col 0, dmsn_cmd, row + 1,
    dmsn_cmd = concat("if [[ ! -d /nfsarch/v500/db_",cnvtlower(dmr_mig_data->src_db_name)," ]]"), col
     0, dmsn_cmd,
    row + 1, col 0, "then",
    row + 1, dmsn_cmd = concat("   mkdir /nfsarch/v500/db_",cnvtlower(dmr_mig_data->src_db_name)),
    col 0,
    dmsn_cmd, row + 1, col 0,
    "fi", row + 2, dmsn_cmd = concat("chown oracle:dba /nfsarch/v500/db_",cnvtlower(dmr_mig_data->
      src_db_name)),
    col 0, dmsn_cmd, row + 1,
    dmsn_cmd = concat("if [[ ! -d ",dmsn_arch_path," ]]"), col 0, dmsn_cmd,
    row + 1, col 0, "then",
    row + 1, dmsn_cmd = concat("   mkdir ",dmsn_arch_path), col 0,
    dmsn_cmd, row + 1, col 0,
    "fi", row + 2, dmsn_cmd = concat("chown oracle:dba ",dmsn_arch_path),
    col 0, dmsn_cmd, row + 1,
    dmsn_gg_cmd = concat('echo "',dmsn_gg_path," *(rw,all_squash,anonuid=",dmsn_stg_oracle_uid,
     ",anongid=",
     dmsn_stg_oracle_gid,')" >> /etc/exports'), dmsn_arch_cmd = concat(
     'echo "/nfsarch *(rw,all_squash,anonuid=',dmsn_stg_oracle_uid,",anongid=",dmsn_stg_oracle_gid,
     ')" >> /etc/exports')
    IF (dmsn_exp_exist=0)
     col 0, dmsn_gg_cmd, row + 1
     IF (dmsn_using_ftp=0)
      col 0, dmsn_arch_cmd, row + 1
     ENDIF
    ELSE
     col 0, 'DateStamp=`date +"%Y%m%d%H%M%S"`', row + 1,
     col 0, "BackupFile=/etc/exports_${DateStamp}.old", row + 1,
     col 0, "cp /etc/exports ${BackupFile}", row + 1,
     col 0, "if [[ $? != 0 ]]", row + 1,
     col 0, "then", row + 1,
     col 0, '   echo "Failed to backup /etc/exports"', row + 1,
     col 0, "   exit 1", row + 1,
     col 0, "fi", row + 1
     IF (dmsn_gg_exist=0)
      col 0, dmsn_gg_cmd, row + 1
     ENDIF
     IF (dmsn_arch_exist=0
      AND dmsn_using_ftp=0)
      col 0, dmsn_arch_cmd, row + 1
     ENDIF
    ENDIF
    col 0, "exportfs -r", row + 1,
    col 0, "if [[ $? != 0 ]]", row + 1,
    col 0, "then", row + 1,
    col 0, '   echo "exportfs ERROR"', row + 1,
    col 0, "   exit 1", row + 1,
    col 0, "fi", row + 1,
    col 0, 'echo "dm2_mig_stg_nfsmnt.ksh is executed successfully."', row + 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (dmsn_run_ksh_prompt("export",dmsn_from_db_node,"stg","dm2_mig_stg_nfsmnt.ksh")=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmsn_work_type="CAPTURE"
  AND (dm2_sys_misc->cur_os != "AXP"))
  IF (dmsn_read_exp_dat(dmsn_src_exp_file,dmsn_exp_exist,dmsn_gg_exist,dmsn_arch_exist)=0)
   GO TO exit_program
  ENDIF
  SET dmsn_src_nfsmnt_ksh = concat(dm2_install_schema->ccluserdir,"dm2_mig_src_nfsmnt.ksh")
  SET dm_err->eproc = concat("create ",dmsn_src_nfsmnt_ksh)
  SELECT INTO value(dmsn_src_nfsmnt_ksh)
   FROM (dummyt t  WITH seq = 1)
   DETAIL
    col 0, "#!/bin/ksh", row + 1,
    col 0, 'if [[ `whoami` != "root" ]]', row + 1,
    ...
    col 0, "    exit 1", row + 1,
    col 0, "fi", row + 2
    IF ((dmr_mig_data->src_db_os="HPX"))
     col 0, "hpux_use_export=`uname -r | grep B.11.2 | wc -l` ", row + 1,
     dmsn_cmd = concat('echo "',dmsn_gg_path," -anon=",dmsn_src_oracle_uid,'" >> /etc/exports')
    ELSEIF ((dmr_mig_data->src_db_os="AIX"))
     dmsn_cmd = concat("mknfsexp -d ",dmsn_gg_path," -t rw -a ",dmsn_src_oracle_uid)
    ELSEIF ((dmr_mig_data->src_db_os="LNX"))
     dmsn_cmd = concat('echo "',dmsn_gg_path," *(rw,all_squash,anonuid=",dmsn_src_oracle_uid,
      ",anongid=",
      dmsn_src_oracle_gid,')" >> /etc/exports')
    ENDIF
    IF (dmsn_exp_exist=0)
     IF ((dmr_mig_data->src_db_os="HPX"))
      col 0, "if [[ ${hpux_use_export} -eq 1 ]] then ", row + 1
     ENDIF
     col 0, dmsn_cmd, row + 1
     IF ((dmr_mig_data->src_db_os="HPX"))
      col 0, "fi", row + 1
    ...
    IF ((dmr_mig_data->src_db_os="HPX"))
     col 0, "if [[ ${hpux_use_export} -eq 1 ]] then   ", row + 1,
     col 0, "exportfs -a", row + 1,
     col 0, "else   ", row + 1,
     CALL print(concat("    share -F nfs -o rw ",dmsn_gg_path)), row + 1, col 0,
     "fi ", row + 1
    ELSEIF ((dmr_mig_data->src_db_os="LNX"))
     col 0, "exportfs -r", row + 1
    ENDIF
    IF ((dmr_mig_data->src_db_os IN ("HPX", "LNX")))
     col 0, "if [[ $? != 0 ]]", row + 1,
     col 0, "then", row + 1,
     col 0, '   echo "exportfs ERROR"', row + 1,
     col 0, "   exit 1", row + 1,
     col 0, "fi", row + 1
    ENDIF
    col 0, 'echo "dm2_mig_src_nfsmnt.ksh is executed successfully."', row + 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (dmsn_run_ksh_prompt("export",dmsn_from_db_node,"src","dm2_mig_src_nfsmnt.ksh")=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmsn_work_type="DELIVERY")
  IF (dmsn_read_exp_dat(dmsn_tgt_exp_file,dmsn_exp_exist,dmsn_gg_exist,dmsn_arch_exist)=0)
   GO TO exit_program
  ENDIF
  SET dmsn_tgt_nfsmnt_ksh = concat(dm2_install_schema->ccluserdir,"dm2_mig_tgt_nfsmnt.ksh")
  SET dm_err->eproc = concat("create ",dmsn_tgt_nfsmnt_ksh)
  SELECT INTO value(dmsn_tgt_nfsmnt_ksh)
   FROM (dummyt t  WITH seq = 1)
   DETAIL
    col 0, "#!/bin/ksh", row + 1,
    col 0, 'if [[ `whoami` != "root" ]]', row + 1,
    ...
    col 0, "    exit 1", row + 1,
    col 0, "fi", row + 2
    IF ((dmr_mig_data->tgt_db_os="HPX"))
     col 0, "hpux_use_export=`uname -r | grep B.11.2 | wc -l` ", row + 1,
     dmsn_cmd = concat('echo "',dmsn_gg_path," -anon=",dmsn_tgt_oracle_uid,'" >> /etc/exports')
    ELSEIF ((dmr_mig_data->tgt_db_os="AIX"))
     dmsn_cmd = concat("mknfsexp -d ",dmsn_gg_path," -t rw -a ",dmsn_tgt_oracle_uid)
    ELSEIF ((dmr_mig_data->tgt_db_os="LNX"))
     dmsn_cmd = concat('echo "',dmsn_gg_path," *(rw,all_squash,anonuid=",dmsn_tgt_oracle_uid,
      ",anongid=",
      dmsn_tgt_oracle_gid,')" >> /etc/exports')
    ENDIF
    IF (dmsn_exp_exist=0)
     IF ((dmr_mig_data->tgt_db_os="HPX"))
      col 0, "if [[ ${hpux_use_export} -eq 1 ]] then ", row + 1
    ...
    ELSEIF ((dmr_mig_data->tgt_db_os="LNX"))
     col 0, "exportfs -r", row + 1
    ENDIF
    IF ((dmr_mig_data->tgt_db_os IN ("HPX", "LNX")))
     col 0, "if [[ $? != 0 ]]", row + 1,
     col 0, "then", row + 1,
     col 0, '   echo "exportfs ERROR"', row + 1,
     col 0, "   exit 1", row + 1,
     col 0, "fi", row + 1
    ENDIF
    col 0, 'echo "dm2_mig_tgt_nfsmnt.ksh is executed successfully."', row + 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  IF (dmsn_run_ksh_prompt("export",dmsn_from_db_node,"tgt","dm2_mig_tgt_nfsmnt.ksh")=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmsn_work_type="CAPTURE")
  IF ((dm2_sys_misc->cur_os="AXP"))
   SET dmsn_cmd = "sho dev ggcapture"
   SET dm_err->disp_dcl_err_ind = 0
   IF (dm2_push_dcl(dmsn_cmd)=1)
    SET dmsn_cmd = "tcpip dismount ggcapture"
    SET dm_err->disp_dcl_err_ind = 0
    CALL dm2_push_dcl(dmsn_cmd)
    SET dmsn_cmd = "sho dev ggcapture"
    SET dm_err->disp_dcl_err_ind = 0
    IF (dm2_push_dcl(dmsn_cmd)=0)
     CALL echo(dm_err->errtext)
     IF (findstring("no such device available",dm_err->errtext,1,0)=0)
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
    ELSE
     SET dm_err->err_ind = 1
     SET dm_err->eproc = "verify ggcapture is dismounted."
     SET dm_err->emsg = "Failed to dismount ggcapture"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
   ENDIF
   SET dmsn_cmd = concat('tcpip mount dnfs0: ggcapture ggcapture /host="',dmsn_stg_hostname,
    '" /path="',dmsn_gg_path,'" /timeout="00:00:05" /user=',
    dmsn_domain_user," /noadf")
   SET dm_err->disp_dcl_err_ind = 0
   CALL dm2_push_dcl(dmsn_cmd)
   SET dmsn_cmd = "sho dev ggcapture"
   IF (dm2_push_dcl(dmsn_cmd)=0)
   ...
   ENDIF
  ELSE
   IF (dmsn_mount_dir(dmsn_work_type,dmsn_src_hostname,"src")=0)
    GO TO exit_program
   ENDIF
  ENDIF
 ENDIF
 IF ((dm2_sys_misc->cur_os="AXP")
  AND dmsn_work_type="CAPTURE"
  AND dmsn_using_ftp=0)
  SET dmsn_src_nfsmnt_com = "ccluserdir:dm2_mig_src_nfsmnt.com"
  SET dm_err->eproc = concat("create ",dmsn_src_nfsmnt_com)
  SELECT INTO value(dmsn_src_nfsmnt_com)
   FROM (dummyt t  WITH seq = 1)
   DETAIL
    dmsn_cmd = '$cur_node=f$edit(f$getsyi("nodename"), "upcase")', col 0, dmsn_cmd,
    row + 1, dmsn_cmd = "$sho dev nfsarch", col 0,
    dmsn_cmd, row + 1, dmsn_cmd = "$if $status",
    col 0, dmsn_cmd, row + 1,
    dmsn_cmd = "$then", col 0, dmsn_cmd,
    row + 1, dmsn_cmd = "$  tcpip dismount nfsarch", col 0,
    dmsn_cmd, row + 1, dmsn_cmd = "$endif",
    col 0, dmsn_cmd, row + 1,
    dmsn_arch_cmd = concat('$tcpip mount dnfs0: nfsarch nfsarch /host="',dmsn_stg_hostname,
     '" /path="/nfsarch" /structure=5 /timeout="00:00:05" /user=',dmsn_domain_user," /noadf"), col 0,
    dmsn_arch_cmd,
    row + 1, dmsn_cmd = "$sho dev nfsarch", col 0,
    dmsn_cmd, row + 1, dmsn_cmd = "$if .not. $status",
    col 0, dmsn_cmd, row + 1,
    dmsn_cmd = "$then", col 0, dmsn_cmd,
    row + 1, dmsn_cmd = '$  write SYS$OUTPUT "Error - nfsarch mount failed"', col 0,
    dmsn_cmd, row + 1, dmsn_cmd = "$  exit",
    col 0, dmsn_cmd, row + 1,
    dmsn_cmd = "$endif", col 0, dmsn_cmd,
    row + 1, dmsn_cmd = '$write sys$output "DM2_MIG_SRC_NFSMNT is executed successfully"', col 0,
    dmsn_cmd, row + 1
  ...
  IF (check_error(dm_err->eproc) != 0)
   ...
   GO TO exit_program
  ENDIF
  SET dmsn_cmd = concat("@",dmsn_src_nfsmnt_com)
  IF (dm2_push_dcl(dmsn_cmd)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmsn_work_type="DELIVERY")
  IF ((dm2_sys_misc->cur_os="AXP"))
   SET dmsn_cmd = "sho dev ggdelivery"
   SET dm_err->disp_dcl_err_ind = 0
   IF (dm2_push_dcl(dmsn_cmd)=1)
    SET dmsn_cmd = "tcpip dismount ggdelivery"
    SET dm_err->disp_dcl_err_ind = 0
    CALL dm2_push_dcl(dmsn_cmd)
    SET dmsn_cmd = "sho dev ggdelivery"
    SET dm_err->disp_dcl_err_ind = 0
    ...
    ELSE
     SET dm_err->err_ind = 1
     SET dm_err->eproc = "verify ggdelivery is dismounted."
     SET dm_err->emsg = "Failed to dismount ggdelivery"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
   ENDIF
   SET dmsn_cmd = concat('tcpip mount dnfs0: ggdelivery ggdelivery /host="',dmsn_tgt_hostname,
    '" /path="',dmsn_gg_path,'" /structure=5 /noadf')
   SET dm_err->disp_dcl_err_ind = 0
   CALL dm2_push_dcl(dmsn_cmd)
   SET dmsn_cmd = "sho dev ggdelivery"
   IF (dm2_push_dcl(dmsn_cmd)=0)
   ...
   ENDIF
  ELSE
   IF (dmsn_mount_dir(dmsn_work_type,dmsn_tgt_hostname,"tgt")=0)
    GO TO exit_program
   ENDIF
  ENDIF
 ENDIF
 SET dm_err->eproc = "Inserting/Updating DM_INFO row for GoldenGate Capture/Delivery path."
 SET dmsn_infoname = build("GG_",dmsn_work_type,"_DIR")
 UPDATE  FROM dm_info di
  SET di.info_char = dmsn_gg_path
  WHERE info_domain="DM2_MIG_DATA"
   AND info_name=dmsn_infoname
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc) > 0)
  ROLLBACK
 ...
 ELSEIF (curqual=0)
  INSERT  FROM dm_info di
   SET di.info_char = dmsn_gg_path, info_domain = "DM2_MIG_DATA", info_name = dmsn_infoname
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc) > 0)
   ROLLBACK
 ...
 COMMIT
 GO TO exit_program
 SUBROUTINE dmsn_mount_dir(dmd_type,dmd_host_name,dmd_node_type)
   DECLARE dmd_nfsmnt_ksh = vc WITH protect, noconstant("ccluserdir:dm2_mig_nfsmnt.ksh")
   DECLARE dmd_link_name = vc WITH protect, noconstant(evaluate(dmd_type,"CAPTURE","/ggcapture",
     "/ggdelivery"))
   DECLARE dmd_str = vc WITH protect, noconstant("")
   IF (dmsn_from_db_node != 1)
    SET dm_err->eproc = concat("create ",dmd_nfsmnt_ksh)
    SELECT INTO value(dmd_nfsmnt_ksh)
     FROM (dummyt t  WITH seq = 1)
     DETAIL
      col 0, "#!/bin/ksh", row + 1,
      col 0, 'if [[ `whoami` != "root" ]]', row + 1,
      ...
      col 0, "    exit 1", row + 1,
      col 0, "fi", row + 2,
      dmd_str = concat("if [[ ! -d ",dmd_link_name," ]]"), col 0, dmd_str,
      row + 1, col 0, "then",
      row + 1, dmd_str = concat("mkdir ",dmd_link_name), col 0,
      dmd_str, row + 1, col 0,
      "   if [[ $? -ne 0 ]]", row + 1, col 0,
      "   then", row + 1, dmd_str = concat('echo "Failed to create ',dmd_link_name,' directory."'),
      col 0, dmd_str, row + 1,
      col 0, "      exit 1", row + 1,
      col 0, "   fi", row + 1,
      col 0, "fi", row + 1,
      dmd_str = concat("mount ",dmd_host_name,":",dmsn_gg_path," ",
       dmd_link_name), col 0, dmd_str,
      row + 1, col 0, "if [[ $? -ne 0 ]]",
      row + 1, col 0, "then",
      row + 1, dmd_str = concat('echo "Failed to mount ',dmd_link_name,'"'), col 0,
      dmd_str, row + 1, col 0,
      "   exit 1", row + 1, col 0,
      "fi", row + 1, col 0,
      'echo "dm2_mig_nfsmnt.ksh completed successfully"', row + 1
    ...
    IF (check_error(dm_err->eproc) != 0)
     ...
     RETURN(0)
    ENDIF
    IF (dmsn_run_ksh_prompt("mount",1,dmd_node_type,"dm2_mig_nfsmnt.ksh")=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmsn_collect_data(dck_run,dck_node_type,dck_hostname,dck_root_uid,dck_root_gid,
  dck_oracle_uid,dck_oracle_gid)
   DECLARE dck_file_prefix = vc WITH protect, noconstant("")
   DECLARE dck_old_file = vc WITH protect, noconstant("")
   DECLARE dck_cmd = vc WITH protect, noconstant("")
   DECLARE dck_ksh_file = vc WITH protect, noconstant("")
   DECLARE dck_dat_file = vc WITH protect, noconstant("")
   DECLARE dck_export_file = vc WITH protect, noconstant("")
   DECLARE dck_prompt = i2 WITH protect, noconstant(1)
   DECLARE dck_tmp_name = vc WITH protect, noconstant("")
   DECLARE dck_tmp_value = vc WITH protect, noconstant("")
   DECLARE dck_node = vc WITH protect, noconstant("")
   DECLARE dck_row = i4 WITH protect, noconstant(0)
   IF (dck_node_type="stg")
    SET dck_file_prefix = "dm2_mig_stg_"
   ELSEIF (dck_node_type="tgt")
    SET dck_file_prefix = "dm2_mig_tgt_"
   ELSEIF (dck_node_type="src")
    SET dck_file_prefix = "dm2_mig_src_"
   ENDIF
   SET dck_ksh_file = concat(dm2_install_schema->ccluserdir,dck_file_prefix,"data.ksh")
   SET dck_dat_file = concat(dck_file_prefix,"data.dat")
   SET dck_export_file = concat(dck_file_prefix,"exports_tmp.dat")
   SET dm_err->eproc = concat("Create ksh file ",dck_ksh_file)
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO value(dck_ksh_file)
    FROM (dummyt t  WITH seq = 1)
    DETAIL
     col 0, "#!/bin/ksh", row + 1,
     col 0, 'if [[ `whoami` != "root" ]]', row + 1,
     ...
     col 0, "    exit 1", row + 1,
     col 0, "fi", row + 1,
     col 0, "hostname=`hostname`", row + 1,
     col 0, "if [[ $? = 1 ]]", row + 1,
     col 0, "then", row + 1,
     col 0, '    echo "ERROR:hostname not found."', row + 1,
     col 0, "    exit 1", row + 1,
     col 0, "fi", row + 1,
     dck_cmd = concat('echo "hostname=${hostname}" > ',dck_dat_file), col 0, dck_cmd,
     row + 1, col 0, "root_uid=`id -u root`",
     row + 1, col 0, "if [[ $? = 1 ]]",
     row + 1, col 0, "then",
     row + 1, col 0, '    echo "ERROR:uid not found for root."',
     row + 1, col 0, "    exit 1",
     row + 1, col 0, "fi",
     row + 1, dck_cmd = concat('echo "root_uid=${root_uid}" >> ',dck_dat_file), col 0,
     dck_cmd, row + 1, col 0,
     "root_gid=`id -g root`", row + 1, col 0,
     "if [[ $? = 1 ]]", row + 1, col 0,
     "then", row + 1, col 0,
     '    echo "ERROR:gid not found for root."', row + 1, col 0,
     "    exit 1", row + 1, col 0,
     "fi", row + 1, dck_cmd = concat('echo "root_gid=${root_gid}" >> ',dck_dat_file),
     col 0, dck_cmd, row + 1,
     col 0, "oracle_uid=`id -u oracle`", row + 1,
     col 0, "if [[ $? = 1 ]]", row + 1,
     col 0, "then", row + 1,
     col 0, '    echo "ERROR:uid not found for oracle."', row + 1,
     col 0, "    exit 1", row + 1,
     col 0, "fi", row + 1,
     dck_cmd = concat('echo "oracle_uid=${oracle_uid}" >> ',dck_dat_file), col 0, dck_cmd,
     row + 1, col 0, "oracle_gid=`id -g oracle`",
     row + 1, col 0, "if [[ $? = 1 ]]",
     row + 1, col 0, "then",
     row + 1, col 0, '    echo "ERROR:gid not found for oracle."',
     row + 1, col 0, "    exit 1",
     row + 1, col 0, "fi",
     row + 1, dck_cmd = concat('echo "oracle_gid=${oracle_gid}" >> ',dck_dat_file), col 0,
     dck_cmd, row + 1, col 0,
     "if [[ ! -f /etc/exports ]]", row + 1, col 0,
     "then", row + 1, dck_cmd = concat('   echo "/etc/exports does not exists" > ',dck_export_file),
     col 0, dck_cmd, row + 1,
     col 0, "else", row + 1,
     dck_cmd = concat("   cp /etc/exports ",dck_export_file), col 0, dck_cmd,
     row + 1, col 0, "   if [[ $? = 1 ]]",
     row + 1, col 0, "   then",
     row + 1, col 0, '       echo "ERROR:Failed to copy /etc/exports"',
     row + 1, col 0, "       exit 1",
     row + 1, col 0, "   fi",
     row + 2, col 0, "fi",
     row + 1
     IF (dck_node_type IN ("src", "stg"))
      col 0, "gg_dir=", dmsn_cap_loc,
      row + 1
     ELSE
      col 0, "gg_dir=", dmsn_del_loc,
      row + 1
     ENDIF
     col 0, "if [[ -z $gg_dir ]]", row + 1,
     col 0, "then", row + 1,
     col 0, 'echo " "', row + 1,
     col 0, 'echo "ERROR: GoldenGate Capture Software Install Directory invalid."', row + 1,
     col 0, 'echo " "', row + 1,
     col 0, "exit 1", row + 1,
     col 0, "fi", row + 1
     IF (dck_node_type IN ("src", "stg"))
      dck_cmd = concat("echo capture_dir=${gg_dir} >> ",dck_dat_file), col 0, dck_cmd,
      row + 1
     ELSE
      dck_cmd = concat("echo delivery_dir=${gg_dir} >> ",dck_dat_file), col 0, dck_cmd,
      row + 1
     ENDIF
     dck_cmd = concat('echo "',dck_file_prefix,'data.ksh executed successfully."'), col 0, dck_cmd,
     row + 1
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   WHILE (dck_prompt=1)
     IF ((dm2_sys_misc->cur_os="AXP"))
      SET dck_old_file = concat(dm2_install_schema->ccluserdir,dck_file_prefix,"*.dat;*")
      SET dck_cmd = concat("delete ",dck_old_file)
     ELSE
      SET dck_old_file = concat(dm2_install_schema->ccluserdir,dck_file_prefix,"*.dat")
      SET dck_cmd = concat("rm -f ",dck_old_file)
     ENDIF
     IF (dm2_findfile(dck_old_file)=1)
      IF (dm2_push_dcl(dck_cmd)=0)
       RETURN(0)
      ENDIF
     ELSEIF ((dm_err->err_ind=1))
      RETURN(0)
     ENDIF
     IF (dck_run=1)
      SET dck_cmd = concat("chmod u+x $CCLUSERDIR/",dck_file_prefix,"data.ksh")
      IF (dm2_push_dcl(dck_cmd)=0)
       RETURN(0)
      ENDIF
      IF (dm2_push_dcl(concat(". $CCLUSERDIR/",dck_file_prefix,"data.ksh"))=0)
       RETURN(0)
      ENDIF
      IF (((dm2_findfile(dck_dat_file)=0) OR (dm2_findfile(dck_export_file)=0)) )
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat("Error executing:",dck_file_prefix,
        "data.ksh. Output files not found in CCLUSERDIR (",dck_export_file,",",
        dck_dat_file,")")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ENDIF
     ...
     IF (dck_run=0)
      ...
      CALL box(1,1,20,120)
      IF (dck_node_type="stg")
       CALL text(2,5,"Data Collection on the Linux Node:")
       CALL text(4,5,"Note : Moving files should be done from Linux node to preserve file format.")
       CALL text(6,5,concat("Move the file below located in CCLUSERDIR to the Linux Node ",
         "and change file permissions appropriately:"))
       SET dck_node = "Linux Node"
       SET dck_row = 6
      ELSEIF (dck_node_type="tgt")
       CALL text(2,5,concat("Data Collection on the Delivery (",dmr_mig_data->tgt_nodes[1].node_name,
         ") Node:"))
       CALL text(4,5,concat("Move the file below located in CCLUSERDIR to the Node ",dmr_mig_data->
         tgt_nodes[1].node_name," and change file permissions appropriately:"))
       SET dck_node = concat("Node ",dmr_mig_data->tgt_nodes[1].node_name)
       SET dck_row = 4
      ELSEIF (dck_node_type="src")
       CALL text(2,5,concat("Data Collection on the Capture (",dmr_mig_data->src_nodes[1].node_name,
         ") Node:"))
       CALL text(4,5,concat("Move the file below located in CCLUSERDIR to the Node ",dmr_mig_data->
         src_nodes[1].node_name," and change file permissions appropriately:"))
       SET dck_node = concat("Node ",dmr_mig_data->src_nodes[1].node_name)
       SET dck_row = 4
      ENDIF
      SET dck_row = (dck_row+ 1)
      CALL text(dck_row,10,concat(dck_file_prefix,"data.ksh"))
      SET dck_row = (dck_row+ 2)
      CALL text(dck_row,5,concat("On the ",dck_node,
        ", as the ROOT user, execute the KSH file that was moved as follows:"))
      SET dck_row = (dck_row+ 1)
      CALL text(dck_row,10,concat("$. ",dck_file_prefix,"data.ksh"))
      SET dck_row = (dck_row+ 2)
      CALL text(dck_row,5,
       "Move the following files created from the execution of the above KSH to CCLUSERDIR on this node "
       )
      SET dck_row = (dck_row+ 1)
      CALL text(dck_row,5,"and change file permissions appropriately:")
      SET dck_row = (dck_row+ 1)
      CALL text(dck_row,10,dck_dat_file)
      SET dck_row = (dck_row+ 1)
      CALL text(dck_row,10,dck_export_file)
      SET dck_row = (dck_row+ 2)
      CALL text(dck_row,5,
       "Upon successful completion of these steps, enter 'C' to continue or 'Q' to quit (C or Q): ")
      CALL accept(dck_row,100,"P;CU"," "
       WHERE curaccept IN ("C", "Q"))
      IF (curaccept="Q")
       RETURN(0)
      ENDIF
      SET message = nowindow
      IF (dm2_findfile(dck_dat_file)=1
       AND dm2_findfile(dck_export_file)=1)
       SET dck_prompt = 0
      ELSE
       IF ((dm_err->err_ind=1))
        RETURN(0)
     ...
     ENDIF
   ENDWHILE
   SET dck_dat_file = concat(dm2_install_schema->ccluserdir,dck_dat_file)
   FREE DEFINE rtl2
   FREE SET file_loc
   SET logical file_loc value(dck_dat_file)
   DEFINE rtl2 "file_loc"
   SELECT INTO "nl:"
    r.line
    FROM rtl2t r
    HEAD REPORT
     beg_pos = 0, len = 0
    DETAIL
     beg_pos = 0, len = 0, dck_tmp_name = "",
     dck_tmp_value = "", len = size(trim(r.line)), beg_pos = findstring("=",r.line,1,0)
     IF (beg_pos > 0)
      dck_tmp_name = substring(1,(beg_pos - 1),r.line), dck_tmp_value = substring((beg_pos+ 1),(len
        - beg_pos),r.line)
     ENDIF
     CASE (dck_tmp_name)
      OF "hostname":
       dck_hostname = dck_tmp_value
      OF "root_uid":
       dck_root_uid = dck_tmp_value
      OF "root_gid":
       dck_root_gid = dck_tmp_value
      OF "oracle_uid":
       dck_oracle_uid = dck_tmp_value
      OF "oracle_gid":
       dck_oracle_gid = dck_tmp_value
      OF "capture_dir":
       dmr_mig_data->gg_capture_dir = dck_tmp_value
      OF "delivery_dir":
       dmr_mig_data->gg_delivery_dir = dck_tmp_value
     ENDCASE
    FOOT REPORT
     row + 0
    WITH nocounter
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmsn_read_exp_dat(dred_exp_file,dred_exp_exist,dred_gg_exist,dred_arch_exist)
   SET dred_exp_exist = 0
   SET dred_gg_exist = 0
   SET dred_arch_exist = 0
   SET dred_exp_file = build(dm2_install_schema->ccluserdir,dred_exp_file)
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("export_file =",dred_exp_file))
   ENDIF
   SET dm_err->eproc = concat("Read ",dred_exp_file)
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   FREE DEFINE rtl2
   FREE SET file_loc
   SET logical file_loc value(dred_exp_file)
   DEFINE rtl2 "file_loc"
   SELECT INTO "nl:"
    r.line
    FROM rtl2t r
    DETAIL
     IF ((dm_err->debug_flag > 0))
      CALL echo(r.line)
     ENDIF
     IF (findstring("/etc/exports does not exists",trim(r.line),1,0) > 0)
      dred_exp_exist = 0,
      CALL cancel(1)
     ELSE
      dred_exp_exist = 1
     ENDIF
     IF (findstring(dmsn_gg_path,trim(r.line),1,0) > 0)
      dred_gg_exist = 1
     ENDIF
     IF (findstring("/nfsarch",trim(r.line),1,0) > 0)
      dred_arch_exist = 1
     ENDIF
    WITH nocounter
   ;end select
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("exp_exist =",dred_exp_exist))
    CALL echo(build("gg_exist =",dred_gg_exist))
    CALL echo(build("arch_exist =",dred_arch_exist))
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmsn_run_ksh_prompt(drkp_type,drkp_run,drkp_node_type,drkp_ksh_name)
   DECLARE drkp_node = vc WITH protect, noconstant("")
   DECLARE drkp_row = i2 WITH protect, noconstant(0)
   IF (drkp_run=1)
    IF (drkp_node_type="src")
     IF (drkp_type="export")
      SET dm_err->eproc = concat("Export GoldenGate Home Directories on the Capture (",
       dmsn_src_hostname,") Node:")
     ELSE
      SET dm_err->eproc = concat("Mount /ggcapture (",dmsn_src_hostname,")")
     ENDIF
    ELSEIF (drkp_node_type="tgt")
     IF (drkp_type="export")
      SET dm_err->eproc = concat("Export GoldenGate Home Directories on the Delivery (",
       dmsn_tgt_hostname,") Node:")
     ELSE
      SET dm_err->eproc = concat("Mount /ggdelivery (",dmsn_tgt_hostname,") ")
     ENDIF
    ELSEIF (drkp_node_type="stg")
     SET dm_err->eproc = "NFS Mount Directories on the Linux Node"
    ENDIF
    CALL disp_msg("",dm_err->logfile,0)
    IF (dm2_push_dcl(concat(". $CCLUSERDIR/",drkp_ksh_name))=0)
     RETURN(0)
    ENDIF
    RETURN(1)
   ENDIF
   ...
   CALL clear(1,1)
   CALL box(1,1,15,120)
   IF (drkp_node_type="stg")
    CALL text(2,5,"NFS Mount Directories on the Linux Node:")
    CALL text(4,5,"Note : Moving files should be done from Linux node to preserve file format.")
    CALL text(6,5,concat("Move the file below located in CCLUSERDIR to the Linux Node ",
      "and change file permissions appropriately:"))
    SET drkp_node = "Linux node"
    SET drkp_row = 6
   ELSEIF (drkp_node_type="tgt")
    IF (drkp_type="export")
     CALL text(2,5,concat("Export GoldenGate Home Directories on the Delivery (",dmsn_tgt_hostname,
       ") Node:"))
    ELSE
     CALL text(2,5,concat("Mount NFS Directories on the Delivery (",dmsn_tgt_hostname,") Node:"))
    ENDIF
    CALL text(4,5,concat("Move the file below located in CCLUSERDIR to the Node ",dmsn_tgt_hostname,
      " and change file permissions appropriately:"))
    SET drkp_node = concat("node ",dmsn_tgt_hostname)
    SET drkp_row = 4
   ELSEIF (drkp_node_type="src")
    IF (drkp_type="export")
     CALL text(2,5,concat("Export GoldenGate Home Directories on the Capture (",dmsn_src_hostname,
       ") Node:"))
    ELSE
     CALL text(2,5,concat("Mount NFS Directories on the Capture (",dmsn_src_hostname,") Node:"))
    ENDIF
    CALL text(4,5,concat("Move the file below located in CCLUSERDIR to the Node ",dmsn_src_hostname,
      " and change file permissions appropriately:"))
    SET drkp_node = concat("node ",dmsn_src_hostname)
    SET drkp_row = 4
   ENDIF
   SET drkp_row = (drkp_row+ 1)
   CALL text(drkp_row,10,drkp_ksh_name)
   SET drkp_row = (drkp_row+ 2)
   CALL text(drkp_row,5,concat("On the ",drkp_node,
     ", as the ROOT user, execute the KSH file that was moved as follows:"))
   SET drkp_row = (drkp_row+ 1)
   CALL text(drkp_row,10,concat("$. ",drkp_ksh_name))
   SET drkp_row = (drkp_row+ 2)
   CALL text(drkp_row,5,
    "Upon successful completion of these steps, enter 'C' to continue or 'Q' to quit (C or Q): ")
   CALL accept(drkp_row,100,"P;CU"," "
    WHERE curaccept IN ("C", "Q"))
   SET message = nowindow
   IF (curaccept="Q")
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmsn_get_domain_user(dgdu_domain_user)
   DECLARE dgdu_env_name = vc WITH protect, noconstant(" ")
   DECLARE dgdu_cmd = vc WITH protect, noconstant(" ")
   DECLARE dgdu_start_pos = i4 WITH protect, noconstant(0)
   DECLARE dgdu_end_pos = i4 WITH protect, noconstant(0)
   DECLARE dgdu_src_uic_com = vc WITH protect, noconstant(" ")
   SET dgdu_src_uic_com = "ccluserdir:dm2_mig_src_uic.com"
   SET dgdu_env_name = logical("environment")
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("Environment Name = ",dgdu_env_name))
   ENDIF
   IF (((dgdu_env_name=" ") OR (dgdu_env_name="")) )
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Validating environment name."
    SET dm_err->emsg = "Invalid environment name."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dgdu_cmd = concat("lreg -getp environment\",dgdu_env_name,
    " LocalUserName ;show symbol LREG_RESULT")
   IF ((dm_err->debug_flag > 0))
    CALL echo("*")
    CALL echo(concat("call dcl executing: ",dgdu_cmd))
    CALL echo("*")
   ENDIF
   SET dm_err->disp_dcl_err_ind = 0
   IF (dm2_push_dcl(dgdu_cmd)=0)
    IF ((dm_err->err_ind=1))
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ...
   SET dm_err->errtext = replace(dm_err->errtext,'"',"",1)
   IF (findstring("%DCL-W-UNDSYM",dm_err->errtext) > 0)
    SET dgdu_domain_user = " "
   ELSE
    SET dgdu_domain_user = trim(dm_err->errtext,3)
   ENDIF
   IF (dgdu_domain_user=" ")
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Retreiving domain user from registry."
    ...
    RETURN(0)
   ENDIF
   IF (cnvtupper(curuser) != cnvtupper(dgdu_domain_user))
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Making sure current user is the domain user."
    SET dm_err->emsg = "Current user is not the domain user."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_program
 ...
 IF ((dm_err->err_ind=1))
  ...
  CALL echo("*")
 ENDIF
 SET dm_err->eproc = "DM2_MIG_SETUP_NFS completed successfully"
 CALL final_disp_msg(dmsn_logfile_prefix)
END GO
