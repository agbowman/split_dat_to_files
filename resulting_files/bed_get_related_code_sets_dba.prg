CREATE PROGRAM bed_get_related_code_sets:dba
 ...
 ENDIF
 IF ( NOT (validate(eventcodesreply,0)))
  RECORD eventcodesreply(
    1 eventcodereplystructure[*]
      2 event_set_name = vc
      2 event_codes[*]
        3 code_value = f8
        3 description = vc
        3 display = vc
        3 identifier = vc
        3 concept_cki_ind = i2
        3 definition = vc
  ) WITH protect
 ENDIF
 IF ( NOT (validate(eventcodesreplyfromdta,0)))
  RECORD eventcodesreplyfromdta(
    1 event_codes[*]
      2 code_value = f8
      2 display = vc
      2 identifier = vc
      2 concept_cki_ind = i2
      2 definition = vc
      2 event_cd = f8
  ) WITH protect
 ENDIF
 IF ( NOT (validate(eventsetreply,0)))
  RECORD eventsetreply(
    1 event_set_reltns[*]
      2 code_value = f8
      2 display = vc
      2 identifier = vc
 ...
 ENDIF
 IF ( NOT (validate(dtacodesreply,0)))
  RECORD dtacodesreply(
    1 dta_code_reltns[*]
      2 code_value = f8
      2 display = vc
      2 identifier = vc
 ...
 ENDIF
 IF ( NOT (validate(dtacodesfromeventsetreply,0)))
  RECORD dtacodesfromeventsetreply(
    1 dta_code_reltns_from_event_set[*]
      2 event_set_name = vc
      2 dtas[*]
        3 code_value = f8
        3 display = vc
 ...
 SUBROUTINE getcodesetfromcodevalue(codevalue)
   ...
   RETURN(codeset)
 END ;Subroutine
 DECLARE event_code_size = i4 WITH protect, constant(size(request->event_codes,5))
 DECLARE event_set_size = i4 WITH protect, constant(size(request->event_sets,5))
 DECLARE dta_size = i4 WITH protect, constant(size(request->dtas,5))
 DECLARE order_catalog_size = i4 WITH protect, constant(size(request->order_catalogs,5))
 DECLARE order_codes_size = i4 WITH protect, constant(size(request->order_codes,5))
 CALL bedbeginscript(0)
 IF (event_set_size > 0)
  CALL geteventcodefromeventset(request,eventcodesreply)
  CALL populatereplyforeventcodes(0)
  CALL getdtafromeventcode(request,dtacodesreply)
  CALL populatereplyfordta(0)
 ENDIF
 IF (event_code_size > 0)
  CALL geteventsetfromeventcode(request,eventsetreply)
  CALL populatereplyforeventset(0)
  SET stat = initrec(dtacodesreply)
  CALL getdtafromeventcode(request,dtacodesreply)
  CALL populatereplyfordta(0)
 ENDIF
 IF (dta_size > 0)
  CALL geteventcodefromdta(request,eventcodesreplyfromdta)
  CALL populatereplyforgeteventcodefromdta(0)
  SET stat = initrec(eventsetreply)
  CALL geteventsetfromeventcode(request,eventsetreply)
  CALL populatereplyforeventset(0)
 ENDIF
 IF (order_catalog_size > 0)
  CALL getordersfromcodeset200(request,ordersreply)
  CALL populatereplyforcodeset200(0)
 ENDIF
 IF (order_codes_size > 0)
  CALL getordersfromorderscatalog(request,ordersreply)
  CALL populatereplyforordercatalog(0)
 ENDIF
 CALL populatemissingtermfortabletypecodeandorderable(0)
 IF (validate(debug,0)=1)
  CALL echorecord(reply)
 ENDIF
#exit_script
 CALL bedexitscript(0)
 SUBROUTINE populatereplyforeventcodes(dummyvar)
   CALL bedlogmessage("populateReplyForEventCodes","Entering ...")
   DECLARE eventsetcnt = i4 WITH protect, noconstant(0)
   DECLARE diffidentcnt = i4 WITH protect, noconstant(0)
   DECLARE missinglinkcnt = i4 WITH protect, noconstant(0)
   DECLARE index = i4 WITH protect, noconstant(0)
   DECLARE repcnt = i4 WITH protect, noconstant(0)
   DECLARE repindex = i4 WITH protect, noconstant(0)
   DECLARE eventcdsrep = i4 WITH protect, noconstant(0)
   DECLARE eventcodesreplysize = i4 WITH protect, noconstant(0)
   DECLARE diffidentindex = i4 WITH protect, noconstant(0)
   DECLARE missinglinkindex = i4 WITH protect, noconstant(0)
   FOR (eventsetcnt = 1 TO size(request->event_sets,5))
    SET index = locateval(index,1,size(eventcodesreply->eventcodereplystructure,5),cnvtupper(request
      ->event_sets[eventsetcnt].event_set_name),cnvtupper(eventcodesreply->eventcodereplystructure[
      index].event_set_name))
    IF (index > 0)
     FOR (eventcdsrep = 1 TO size(eventcodesreply->eventcodereplystructure[index].event_codes,5))
       SET repcnt = locateval(repindex,1,size(reply->relatedcodesets,5),eventcodesreply->
        eventcodereplystructure[index].event_codes[eventcdsrep].display,reply->relatedcodesets[
        repindex].display,
        eventcodesreply->eventcodereplystructure[index].event_codes[eventcdsrep].definition,reply->
        relatedcodesets[repindex].definition,eventcodesreply->eventcodereplystructure[index].
        event_codes[eventcdsrep].code_value,reply->relatedcodesets[repindex].code_value)
       IF (repcnt=0)
        SET repcnt = (size(reply->relatedcodesets,5)+ 1)
        SET stat = alterlist(reply->relatedcodesets,repcnt)
        SET reply->relatedcodesets[repcnt].identifier = request->event_sets[eventsetcnt].concept_cki
        SET reply->relatedcodesets[repcnt].display = eventcodesreply->eventcodereplystructure[index].
        event_codes[eventcdsrep].display
        SET reply->relatedcodesets[repcnt].definition = eventcodesreply->eventcodereplystructure[
        index].event_codes[eventcdsrep].definition
        SET reply->relatedcodesets[repcnt].code_value = eventcodesreply->eventcodereplystructure[
        index].event_codes[eventcdsrep].code_value
       ENDIF
       CALL addcodevaluefromrequesttoreply(repcnt,geteventsetcodefromeventsetname(request->
         event_sets[eventsetcnt].event_set_name))
       IF ((eventcodesreply->eventcodereplystructure[index].event_codes[eventcdsrep].concept_cki_ind=
       0))
        SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
          code_sets_with_diff_ident,5),72,reply->relatedcodesets[repcnt].code_sets_with_diff_ident[
         diffidentindex].code_set)
        IF (diffidentindex=0)
         SET diffidentcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_diff_ident,5)+ 1)
         SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_diff_ident,diffidentcnt)
         SET reply->relatedcodesets[repcnt].code_sets_with_diff_ident[diffidentcnt].code_set = 72
         SET reply->relatedcodesets[repcnt].code_sets_with_diff_ident[diffidentcnt].empty_ident_ind
          = 1
        ENDIF
       ELSEIF ((eventcodesreply->eventcodereplystructure[index].event_codes[eventcdsrep].
       concept_cki_ind=2))
        SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
          code_sets_with_diff_ident,5),72,reply->relatedcodesets[repcnt].code_sets_with_diff_ident[
         diffidentindex].code_set)
     ...
     ENDFOR
    ELSE
     SET repcnt = locateval(repindex,1,size(reply->relatedcodesets,5),request->event_sets[eventsetcnt
      ].event_set_name,reply->relatedcodesets[repindex].display)
     IF (repcnt=0)
      SET repcnt = (size(reply->relatedcodesets,5)+ 1)
      SET stat = alterlist(reply->relatedcodesets,repcnt)
      SET reply->relatedcodesets[repcnt].identifier = request->event_sets[eventsetcnt].concept_cki
      SET reply->relatedcodesets[repcnt].display = request->event_sets[eventsetcnt].event_set_name
      SET reply->relatedcodesets[repcnt].definition = getdefinitionfromcodevalue(
       geteventsetcodefromeventsetname(request->event_sets[eventsetcnt].event_set_name))
     ENDIF
     CALL addcodevaluefromrequesttoreply(repcnt,geteventsetcodefromeventsetname(request->event_sets[
       eventsetcnt].event_set_name))
     SET missinglinkindex = locateval(missinglinkindex,1,size(reply->relatedcodesets[repcnt].
       code_sets_with_missing_link,5),72,reply->relatedcodesets[repcnt].code_sets_with_missing_link[
      missinglinkindex].code_set)
     IF (missinglinkindex=0)
      SET missinglinkcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_missing_link,5)+ 1)
      SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_missing_link,missinglinkcnt)
      SET reply->relatedcodesets[repcnt].code_sets_with_missing_link[missinglinkcnt].code_set = 72
     ENDIF
     SET missinglinkindex = locateval(missinglinkindex,1,size(reply->relatedcodesets[repcnt].
       code_sets_with_missing_link,5),14003,reply->relatedcodesets[repcnt].
      code_sets_with_missing_link[missinglinkindex].code_set)
     IF (missinglinkindex=0)
      SET missinglinkcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_missing_link,5)+ 1)
      SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_missing_link,missinglinkcnt)
      SET reply->relatedcodesets[repcnt].code_sets_with_missing_link[missinglinkcnt].code_set = 14003
     ENDIF
    ENDIF
   ENDFOR
   IF (validate(debug,0)=1)
    CALL echorecord(reply)
   ENDIF
   CALL bedlogmessage("populateReplyForEventCodes","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatereplyforeventset(dummyvar)
   CALL bedlogmessage("populateReplyForEventSet","Entering ...")
   DECLARE index = i4 WITH protect, noconstant(0)
   DECLARE diffidentcnt = i4 WITH protect, noconstant(0)
   DECLARE missinglinkcnt = i4 WITH protect, noconstant(0)
   DECLARE repcnt = i4 WITH protect, noconstant(0)
   ...
   DECLARE definition = vc WITH protect, noconstant("")
   DECLARE codevalue = f8 WITH protect, noconstant(0.0)
   CALL logdebugmessage("Request->event codes size:",size(request->event_codes,5))
   FOR (cnt = 1 TO size(request->event_codes,5))
     CALL logdebugmessage("Event Code:",request->event_codes[cnt].code_value)
     SET index = locateval(index,1,size(eventsetreply->event_set_reltns,5),request->event_codes[cnt].
      code_value,eventsetreply->event_set_reltns[index].code_value)
     CALL logdebugmessage("Index:",index)
     IF (((index=0) OR (index > 0
      AND trim(eventsetreply->event_set_reltns[index].display)="")) )
      SET display = getdisplayfromcodevalue(request->event_codes[cnt].code_value)
      SET definition = getdefinitionfromcodevalue(request->event_codes[cnt].code_value)
      SET codevalue = request->event_codes[cnt].code_value
     ELSE
      SET display = eventsetreply->event_set_reltns[index].display
      SET definition = eventsetreply->event_set_reltns[index].definition
      SET codevalue = eventsetreply->event_set_reltns[index].event_set_cd
     ENDIF
     SET repcnt = locateval(repindex,1,size(reply->relatedcodesets,5),display,reply->relatedcodesets[
      repindex].display,
      definition,reply->relatedcodesets[repindex].definition,codevalue,reply->relatedcodesets[
      repindex].code_value)
     CALL logdebugmessage("repCnt:",repcnt)
     IF (repcnt=0)
      SET repcnt = (size(reply->relatedcodesets,5)+ 1)
      SET stat = alterlist(reply->relatedcodesets,repcnt)
      SET reply->relatedcodesets[repcnt].code_value = codevalue
      SET reply->relatedcodesets[repcnt].identifier = request->event_codes[cnt].concept_cki
      SET reply->relatedcodesets[repcnt].display = display
      SET reply->relatedcodesets[repcnt].definition = definition
     ENDIF
     IF ((request->event_codes[cnt].requested_code > 0))
      CALL addcodevaluefromrequesttoreply(repcnt,request->event_codes[cnt].requested_code)
     ELSE
      CALL addcodevaluefromrequesttoreply(repcnt,request->event_codes[cnt].code_value)
     ENDIF
     IF (index > 0)
      IF ((eventsetreply->event_set_reltns[index].concept_cki_ind=0))
       SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
         code_sets_with_diff_ident,5),93,reply->relatedcodesets[repcnt].code_sets_with_diff_ident[
        diffidentindex].code_set)
       IF (diffidentindex=0)
        SET diffidentcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_diff_ident,5)+ 1)
        SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_diff_ident,diffidentcnt)
        SET reply->relatedcodesets[repcnt].code_sets_with_diff_ident[diffidentcnt].code_set = 93
        SET reply->relatedcodesets[repcnt].code_sets_with_diff_ident[diffidentcnt].empty_ident_ind =
        1
       ENDIF
      ELSEIF ((eventsetreply->event_set_reltns[index].concept_cki_ind=2))
       SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
         code_sets_with_diff_ident,5),93,reply->relatedcodesets[repcnt].code_sets_with_diff_ident[
        diffidentindex].code_set)
       IF (diffidentindex=0)
     ...
     ELSE
      SET missinglinkindex = locateval(missinglinkindex,1,size(reply->relatedcodesets[repcnt].
        code_sets_with_missing_link,5),93,reply->relatedcodesets[repcnt].code_sets_with_missing_link[
       missinglinkindex].code_set)
      IF (missinglinkindex=0)
       SET missinglinkcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_missing_link,5)+ 1)
       SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_missing_link,missinglinkcnt
        )
       SET reply->relatedcodesets[repcnt].code_sets_with_missing_link[missinglinkcnt].code_set = 93
      ENDIF
     ENDIF
   ENDFOR
   IF (validate(debug,0)=1)
    CALL echorecord(reply)
   ENDIF
   CALL bedlogmessage("populateReplyForEventSet","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatereplyfordta(dummyvar)
   CALL bedlogmessage("populateReplyForDTA","Entering ...")
   ...
   DECLARE activitytypecd = f8 WITH protect, noconstant(0.0)
   DECLARE codevalue = f8 WITH protect, noconstant(0.0)
   DECLARE matchfound = i2 WITH protect, noconstant(0.0)
   FOR (cnt = 1 TO size(request->event_codes,5))
     CALL logdebugmessage("Cnt:",cnt)
     CALL logdebugmessage("Event Code:",request->event_codes[cnt].code_value)
     SET matchfound = false
     FOR (index = 1 TO size(dtacodesreply->dta_code_reltns,5))
       IF ((request->event_codes[cnt].code_value=dtacodesreply->dta_code_reltns[index].code_value))
        CALL logdebugmessage("Match found:",index)
        SET matchfound = true
        SET display = dtacodesreply->dta_code_reltns[index].display
        SET definition = dtacodesreply->dta_code_reltns[index].definition
        SET activitytypecd = getactivitytypecodefromtaskassaycode(dtacodesreply->dta_code_reltns[
         index].task_assay_cd)
        SET codevalue = dtacodesreply->dta_code_reltns[index].task_assay_cd
        CALL logdebugmessage("Display:",display)
        CALL logdebugmessage("Definition:",definition)
        CALL logdebugmessage("Activity Type Code:",activitytypecd)
        CALL logdebugmessage("Code Value:",codevalue)
        SET repcnt = locateval(repindex,1,size(reply->relatedcodesets,5),display,reply->
         relatedcodesets[repindex].display,
         definition,reply->relatedcodesets[repindex].definition,codevalue,reply->relatedcodesets[
         repindex].code_value)
        CALL logdebugmessage("repCnt:",repcnt)
        IF (repcnt > 0
         AND (reply->relatedcodesets[repcnt].activity_type_cd=0.0))
         SET reply->relatedcodesets[repcnt].activity_type_cd = activitytypecd
        ELSEIF (locateval(repindex,1,size(reply->relatedcodesets,5),display,reply->relatedcodesets[
         repindex].display,
         definition,reply->relatedcodesets[repindex].definition,activitytypecd,reply->
         relatedcodesets[repindex].activity_type_cd,codevalue,
         reply->relatedcodesets[repindex].code_value)=0)
         SET repcnt = 0
        ELSE
         SET repcnt = locateval(repindex,1,size(reply->relatedcodesets,5),display,reply->
          relatedcodesets[repindex].display,
          definition,reply->relatedcodesets[repindex].definition,activitytypecd,reply->
          relatedcodesets[repindex].activity_type_cd,codevalue,
          reply->relatedcodesets[repindex].code_value)
        ENDIF
        CALL logdebugmessage("repCnt:",repcnt)
        IF (repcnt=0)
         SET repcnt = (size(reply->relatedcodesets,5)+ 1)
        ...
        ELSE
         CALL addcodevaluefromrequesttoreply(repcnt,request->event_codes[cnt].code_value)
        ENDIF
        IF ((dtacodesreply->dta_code_reltns[index].concept_cki_ind=0))
         SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
           code_sets_with_diff_ident,5),14003,reply->relatedcodesets[repcnt].
          code_sets_with_diff_ident[diffidentindex].code_set)
         IF (diffidentindex=0)
          SET diffidentcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_diff_ident,5)+ 1)
          SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_diff_ident,diffidentcnt)
          SET reply->relatedcodesets[repcnt].code_sets_with_diff_ident[diffidentcnt].code_set = 14003
          SET reply->relatedcodesets[repcnt].code_sets_with_diff_ident[diffidentcnt].empty_ident_ind
           = 1
         ENDIF
        ELSEIF ((dtacodesreply->dta_code_reltns[index].concept_cki_ind=2))
         SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
           code_sets_with_diff_ident,5),14003,reply->relatedcodesets[repcnt].
          code_sets_with_diff_ident[diffidentindex].code_set)
         IF (diffidentindex=0)
       ...
       ELSEIF (matchfound=false
        AND index=size(dtacodesreply->dta_code_reltns,5))
        CALL logdebugmessage("No match found:",request->event_codes[cnt].code_value)
        SET display = getdisplayfromcodevalue(request->event_codes[cnt].code_value)
        SET definition = getdefinitionfromcodevalue(request->event_codes[cnt].code_value)
        ...
        SET missinglinkindex = locateval(missinglinkindex,1,size(reply->relatedcodesets[repcnt].
          code_sets_with_missing_link,5),14003,reply->relatedcodesets[repcnt].
         code_sets_with_missing_link[missinglinkindex].code_set)
        CALL logdebugmessage("missingLinkIndex:",missinglinkindex)
        IF (missinglinkindex=0)
         SET missinglinkcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_missing_link,5)+ 1)
         SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_missing_link,
          missinglinkcnt)
         SET reply->relatedcodesets[repcnt].code_sets_with_missing_link[missinglinkcnt].code_set =
         14003
        ENDIF
       ENDIF
     ENDFOR
   ...
   IF (validate(debug,0)=1)
    CALL echorecord(reply)
   ENDIF
   CALL bedlogmessage("populateReplyForDTA","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatereplyforgeteventcodefromdta(dummyvar)
   CALL bedlogmessage("populateReplyForGetEventCodeFromDTA","Entering ...")
   DECLARE index = i4 WITH protect, noconstant(0)
   DECLARE diffidentcnt = i4 WITH protect, noconstant(0)
   DECLARE missinglinkcnt = i4 WITH protect, noconstant(0)
   DECLARE repcnt = i4 WITH protect, noconstant(0)
   ...
   DECLARE display = vc WITH protect, noconstant("")
   DECLARE definition = vc WITH protect, noconstant("")
   DECLARE missinglinkindexfor93 = i4 WITH protect, noconstant(0)
   DECLARE missinglinkcntfor93 = i4 WITH protect, noconstant(0)
   DECLARE activitytypecd = f8 WITH protect, noconstant(0.0)
   DECLARE codevalue = f8 WITH protect, noconstant(0.0)
   FOR (cnt = 1 TO size(request->dtas,5))
     CALL logdebugmessage("Task Assay Code:",request->dtas[cnt].code_value)
     SET index = locateval(index,1,size(eventcodesreplyfromdta->event_codes,5),request->dtas[cnt].
      code_value,eventcodesreplyfromdta->event_codes[index].code_value)
     CALL logdebugmessage("Index:",index)
     IF (((index=0) OR (index > 0
      AND trim(eventcodesreplyfromdta->event_codes[index].display)="")) )
      SET display = getdisplayfromcodevalue(request->dtas[cnt].code_value)
      SET definition = getdefinitionfromcodevalue(request->dtas[cnt].code_value)
      SET activitytypecd = getactivitytypecodefromtaskassaycode(request->dtas[cnt].code_value)
      SET codevalue = request->dtas[cnt].code_value
     ELSE
      SET display = eventcodesreplyfromdta->event_codes[index].display
      SET definition = eventcodesreplyfromdta->event_codes[index].definition
      SET activitytypecd = 0.0
      SET codevalue = eventcodesreplyfromdta->event_codes[index].event_cd
     ENDIF
     CALL logdebugmessage("Display:",display)
     CALL logdebugmessage("Definition:",definition)
     CALL logdebugmessage("Activity Type Code:",activitytypecd)
     ...
     IF (repcnt=0)
      ...
      SET stat = alterlist(reply->relatedcodesets,repcnt)
      SET reply->relatedcodesets[repcnt].code_value = codevalue
      SET reply->relatedcodesets[repcnt].identifier = request->dtas[cnt].concept_cki
      SET reply->relatedcodesets[repcnt].display = display
      SET reply->relatedcodesets[repcnt].definition = definition
      SET reply->relatedcodesets[repcnt].activity_type_cd = activitytypecd
     ENDIF
     CALL addcodevaluefromrequesttoreply(repcnt,request->dtas[cnt].code_value)
     IF (index > 0)
      IF ((eventcodesreplyfromdta->event_codes[index].concept_cki_ind=0))
       SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
         code_sets_with_diff_ident,5),72,reply->relatedcodesets[repcnt].code_sets_with_diff_ident[
        diffidentindex].code_set)
       IF (diffidentindex=0)
        ...
        1
       ENDIF
      ELSEIF ((eventcodesreplyfromdta->event_codes[index].concept_cki_ind=2))
       SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
         code_sets_with_diff_ident,5),72,reply->relatedcodesets[repcnt].code_sets_with_diff_ident[
        diffidentindex].code_set)
       IF (diffidentindex=0)
     ...
     ELSE
      ...
      IF (missinglinkindex=0)
       ...
       SET reply->relatedcodesets[repcnt].code_sets_with_missing_link[missinglinkcnt].code_set = 72
      ENDIF
      SET missinglinkindexfor93 = locateval(missinglinkindexfor93,1,size(reply->relatedcodesets[
        repcnt].code_sets_with_missing_link,5),93,reply->relatedcodesets[repcnt].
       code_sets_with_missing_link[missinglinkindexfor93].code_set)
      IF (missinglinkindexfor93=0)
       SET missinglinkcntfor93 = (size(reply->relatedcodesets[repcnt].code_sets_with_missing_link,5)
       + 1)
       SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_missing_link,
        missinglinkcntfor93)
       SET reply->relatedcodesets[repcnt].code_sets_with_missing_link[missinglinkcntfor93].code_set
        = 93
      ENDIF
     ENDIF
   ENDFOR
   IF (validate(debug,0)=1)
    CALL echorecord(reply)
   ENDIF
   CALL bedlogmessage("populateReplyForGetEventCodeFromDTA","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatemissingtermfortabletypecodeandorderable(dummyvar)
   CALL bedlogmessage("populateMissingTermForTableTypeCodeAndOrderable","Entering ...")
   DECLARE reprelatedcodesetscnt = i4 WITH protect, noconstant(0)
   DECLARE missinglinkcnt = i4 WITH protect, noconstant(0)
   DECLARE index = i4 WITH protect, noconstant(0)
   DECLARE binfiledisplay = vc WITH protect, noconstant("")
   DECLARE binfileidentifier = vc WITH protect, noconstant("")
   FOR (reprelatedcodesetscnt = 1 TO size(reply->relatedcodesets,5))
     FOR (missinglinkcnt = 1 TO size(reply->relatedcodesets[reprelatedcodesetscnt].
      code_sets_with_missing_link,5))
       SET binfiledisplay = ""
       SET binfileidentifier = ""
       CALL logdebugmessage("Identifier:",reply->relatedcodesets[reprelatedcodesetscnt].identifier)
       IF (size(request->event_codes,5) > 0)
        SET index = locateval(index,1,size(request->event_codes,5),reply->relatedcodesets[
         reprelatedcodesetscnt].identifier,request->event_codes[index].concept_cki)
        IF (index > 0)
         SET binfiledisplay = request->event_codes[index].display
         SET binfileidentifier = request->event_codes[index].concept_cki
        ENDIF
       ENDIF
       IF (size(request->event_sets,5) > 0
        AND trim(binfiledisplay)=""
        AND trim(binfileidentifier)="")
        SET index = locateval(index,1,size(request->event_sets,5),reply->relatedcodesets[
         reprelatedcodesetscnt].identifier,request->event_sets[index].concept_cki)
        IF (index > 0)
         SET binfiledisplay = request->event_sets[index].display
         SET binfileidentifier = request->event_sets[index].concept_cki
        ENDIF
       ENDIF
       IF (size(request->dtas,5) > 0
        AND trim(binfiledisplay)=""
        AND trim(binfileidentifier)="")
        SET index = locateval(index,1,size(request->dtas,5),reply->relatedcodesets[
         reprelatedcodesetscnt].identifier,request->dtas[index].concept_cki)
        IF (index > 0)
         SET binfiledisplay = request->dtas[index].display
         SET binfileidentifier = request->dtas[index].concept_cki
        ENDIF
       ENDIF
       IF (size(request->order_codes,5) > 0
        AND trim(binfiledisplay)=""
        AND trim(binfileidentifier)="")
        SET index = locateval(index,1,size(request->order_codes,5),reply->relatedcodesets[
         reprelatedcodesetscnt].identifier,request->order_codes[index].concept_cki)
        IF (index > 0)
         SET binfiledisplay = request->order_codes[index].display
         SET binfileidentifier = request->order_codes[index].concept_cki
        ENDIF
       ENDIF
       IF (size(request->order_catalogs,5) > 0
        AND trim(binfiledisplay)=""
        AND trim(binfileidentifier)="")
        SET index = locateval(index,1,size(request->order_catalogs,5),reply->relatedcodesets[
         reprelatedcodesetscnt].identifier,request->order_catalogs[index].concept_cki)
        IF (index > 0)
         SET binfiledisplay = request->order_catalogs[index].display
         SET binfileidentifier = request->order_catalogs[index].concept_cki
        ENDIF
       ENDIF
       CALL logdebugmessage("binFileDisplay:",binfiledisplay)
       CALL logdebugmessage("binFileIdentifier:",binfileidentifier)
       CALL logdebugmessage("Codeset:",reply->relatedcodesets[reprelatedcodesetscnt].
        code_sets_with_missing_link[missinglinkcnt].code_set)
       IF (trim(binfiledisplay) != ""
        AND trim(binfileidentifier) != "")
        IF ((reply->relatedcodesets[reprelatedcodesetscnt].code_sets_with_missing_link[missinglinkcnt
        ].code_set=- (1)))
         SELECT INTO "nl:"
          FROM order_catalog oc
          PLAN (oc
           WHERE ((oc.primary_mnemonic=trim(binfiledisplay)) OR (oc.concept_cki=trim(
            binfileidentifier)))
            AND oc.active_ind=1)
          WITH nocounter
         ;end select
        ELSE
         SELECT INTO "nl:"
          FROM code_value cv
          PLAN (cv
           WHERE (cv.code_set=reply->relatedcodesets[reprelatedcodesetscnt].
           code_sets_with_missing_link[missinglinkcnt].code_set)
            AND cv.active_ind=1
            AND ((cv.display=trim(binfiledisplay)) OR (cv.concept_cki=trim(binfileidentifier))) )
          WITH nocounter
         ;end select
        ENDIF
        CALL logdebugmessage("order catalog curqual value:",curqual)
        IF (curqual=0)
         SET missingtermcnt = (size(reply->relatedcodesets[reprelatedcodesetscnt].
          code_sets_with_missing_term,5)+ 1)
         SET stat = alterlist(reply->relatedcodesets[reprelatedcodesetscnt].
          code_sets_with_missing_term,missingtermcnt)
         SET reply->relatedcodesets[reprelatedcodesetscnt].code_sets_with_missing_term[missingtermcnt
         ].code_set = reply->relatedcodesets[reprelatedcodesetscnt].code_sets_with_missing_link[
         missinglinkcnt].code_set
        ENDIF
       ENDIF
     ENDFOR
   ENDFOR
   CALL bedlogmessage("populateMissingTermForTableTypeCodeAndOrderable","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatereplyforcodeset200(dummyvar)
   CALL bedlogmessage("populateReplyForCodeSet200","Entering ...")
   DECLARE index = i4 WITH protect, noconstant(0)
   DECLARE diffidentcnt = i4 WITH protect, noconstant(0)
   DECLARE missinglinkcnt = i4 WITH protect, noconstant(0)
   DECLARE repcnt = i4 WITH protect, noconstant(0)
   ...
   DECLARE missinglinkindex = i4 WITH protect, noconstant(0)
   DECLARE display = vc WITH protect, noconstant("")
   CALL logdebugmessage("request->order_catalogs size:",size(request->order_catalogs,5))
   FOR (cnt = 1 TO size(request->order_catalogs,5))
     SET index = locateval(index,1,size(ordersreply->orders,5),request->order_catalogs[cnt].
      catalog_cd,ordersreply->orders[index].code_value)
     CALL logdebugmessage("Match Found if index > 0:",index)
     IF (((index=0) OR (index > 0
      AND trim(ordersreply->orders[index].display)="")) )
      SET display = request->order_catalogs[cnt].display
     ELSE
      SET display = ordersreply->orders[index].display
     ENDIF
     CALL logdebugmessage("Display:",display)
     SET repcnt = locateval(repindex,1,size(reply->relatedcodesets,5),display,reply->relatedcodesets[
      repindex].display)
     IF (repcnt=0)
      SET repcnt = (size(reply->relatedcodesets,5)+ 1)
      SET stat = alterlist(reply->relatedcodesets,repcnt)
      SET reply->relatedcodesets[repcnt].code_value = request->order_catalogs[cnt].catalog_cd
      SET reply->relatedcodesets[repcnt].identifier = request->order_catalogs[cnt].concept_cki
      SET reply->relatedcodesets[repcnt].display = display
     ENDIF
     CALL addcodevaluefromrequesttoreply(repcnt,request->order_catalogs[cnt].catalog_cd)
     IF (index > 0)
      IF ((ordersreply->orders[index].concept_cki_ind=0))
       SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
         code_sets_with_diff_ident,5),200,reply->relatedcodesets[repcnt].code_sets_with_diff_ident[
        diffidentindex].code_set)
       IF (diffidentindex=0)
        SET diffidentcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_diff_ident,5)+ 1)
        SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_diff_ident,diffidentcnt)
        SET reply->relatedcodesets[repcnt].code_sets_with_diff_ident[diffidentcnt].code_set = 200
        SET reply->relatedcodesets[repcnt].code_sets_with_diff_ident[diffidentcnt].empty_ident_ind =
        1
       ENDIF
      ELSEIF ((ordersreply->orders[index].concept_cki_ind=2))
       SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
         code_sets_with_diff_ident,5),200,reply->relatedcodesets[repcnt].code_sets_with_diff_ident[
        diffidentindex].code_set)
       IF (diffidentindex=0)
     ...
     ELSE
      SET missinglinkindex = locateval(missinglinkindex,1,size(reply->relatedcodesets[repcnt].
        code_sets_with_missing_link,5),200,reply->relatedcodesets[repcnt].
       code_sets_with_missing_link[missinglinkindex].code_set)
      IF (missinglinkindex=0)
       SET missinglinkcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_missing_link,5)+ 1)
       SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_missing_link,missinglinkcnt
        )
       SET reply->relatedcodesets[repcnt].code_sets_with_missing_link[missinglinkcnt].code_set = 200
      ENDIF
     ENDIF
   ENDFOR
   IF (validate(debug,0)=1)
    CALL echorecord(reply)
   ENDIF
   CALL bedlogmessage("populateReplyForCodeSet200","Exiting ...")
 END ;Subroutine
 SUBROUTINE populatereplyforordercatalog(dummyvar)
   CALL bedlogmessage("populateReplyForOrderCatalog","Entering ...")
   DECLARE index = i4 WITH protect, noconstant(0)
   DECLARE diffidentcnt = i4 WITH protect, noconstant(0)
   DECLARE missinglinkcnt = i4 WITH protect, noconstant(0)
   DECLARE repcnt = i4 WITH protect, noconstant(0)
   ...
   DECLARE display = vc WITH protect, noconstant("")
   DECLARE definition = vc WITH protect, noconstant("")
   CALL logdebugmessage("request->order_codes size:",size(request->order_codes,5))
   FOR (cnt = 1 TO size(request->order_codes,5))
     SET index = locateval(index,1,size(ordersreply->orders,5),request->order_codes[cnt].code_value,
      ordersreply->orders[index].code_value)
     CALL logdebugmessage("Match Found if index > 0:",index)
     IF (((index=0) OR (index > 0
      AND trim(ordersreply->orders[index].display)="")) )
      SET display = request->order_codes[cnt].display
      SET definition = getdefinitionfromcodevalue(request->order_codes[cnt].code_value)
     ELSE
      SET display = ordersreply->orders[index].display
      SET definition = getdefinitionfromcodevalue(ordersreply->orders[index].code_value)
     ENDIF
     CALL logdebugmessage("Display:",display)
     CALL logdebugmessage("Definition:",definition)
     SET repcnt = locateval(repindex,1,size(reply->relatedcodesets,5),display,reply->relatedcodesets[
      repindex].display,
      definition,reply->relatedcodesets[repindex].definition)
     IF (repcnt=0)
      SET repcnt = (size(reply->relatedcodesets,5)+ 1)
      SET stat = alterlist(reply->relatedcodesets,repcnt)
      SET reply->relatedcodesets[repcnt].code_value = request->order_codes[cnt].code_value
      SET reply->relatedcodesets[repcnt].identifier = request->order_codes[cnt].concept_cki
      SET reply->relatedcodesets[repcnt].display = display
      SET reply->relatedcodesets[repcnt].definition = definition
     ENDIF
     CALL addcodevaluefromrequesttoreply(repcnt,request->order_codes[cnt].code_value)
     IF (index > 0)
      IF ((ordersreply->orders[index].concept_cki_ind=0))
       SET diffidentindex = locateval(diffidentindex,1,size(reply->relatedcodesets[repcnt].
         code_sets_with_diff_ident,5),- (1),reply->relatedcodesets[repcnt].code_sets_with_diff_ident[
        diffidentindex].code_set)
       IF (diffidentindex=0)
        SET diffidentcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_diff_ident,5)+ 1)
        SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_diff_ident,diffidentcnt)
        SET reply->relatedcodesets[repcnt].code_sets_with_diff_ident[diffidentcnt].code_set = - (1)
        SET reply->relatedcodesets[repcnt].code_sets_with_diff_ident[diffidentcnt].empty_ident_ind =
        1
       ENDIF
      ELSEIF ((ordersreply->orders[index].concept_cki_ind=2))
     ...
     ELSE
      SET missinglinkindex = locateval(missinglinkindex,1,size(reply->relatedcodesets[repcnt].
        code_sets_with_missing_link,5),- (1),reply->relatedcodesets[repcnt].
       code_sets_with_missing_link[missinglinkindex].code_set)
      IF (missinglinkindex=0)
       SET missinglinkcnt = (size(reply->relatedcodesets[repcnt].code_sets_with_missing_link,5)+ 1)
       SET stat = alterlist(reply->relatedcodesets[repcnt].code_sets_with_missing_link,missinglinkcnt
        )
       SET reply->relatedcodesets[repcnt].code_sets_with_missing_link[missinglinkcnt].code_set = - (1
       )
      ENDIF
     ENDIF
   ENDFOR
   IF (validate(debug,0)=1)
    CALL echorecord(reply)
   ENDIF
   CALL bedlogmessage("populateReplyForOrderCatalog","Exiting ...")
 END ;Subroutine
 SUBROUTINE addcodevaluefromrequesttoreply(repcnt,codevaluefromrequest)
   CALL bedlogmessage("addCodeValueFromRequestToReply","Entering ...")
   DECLARE cvreqcnt = i4 WITH protect, noconstant(0)
   DECLARE cvreqindex = i4 WITH protect, noconstant(0)
   CALL logdebugmessage("repCnt:",repcnt)
   CALL logdebugmessage("codeValueFromRequest:",codevaluefromrequest)
   SET cvreqcnt = locateval(cvreqindex,1,size(reply->relatedcodesets[repcnt].code_value_from_request,
     5),codevaluefromrequest,reply->relatedcodesets[repcnt].code_value_from_request[cvreqindex].
    code_value)
   CALL logdebugmessage("cvReqCnt:",cvreqcnt)
   IF (cvreqcnt=0)
    SET cvreqcnt = (size(reply->relatedcodesets[repcnt].code_value_from_request,5)+ 1)
    SET stat = alterlist(reply->relatedcodesets[repcnt].code_value_from_request,cvreqcnt)
    SET reply->relatedcodesets[repcnt].code_value_from_request[cvreqcnt].code_value =
    codevaluefromrequest
   ENDIF
   CALL logdebugmessage("cvReqCnt:",cvreqcnt)
   CALL bedlogmessage("addCodeValueFromRequestToReply","Exiting ...")
 END ;Subroutine
END GO
