CREATE PROGRAM cp_add_component_json
 PROMPT
  ...
  "Concept Group Cd" = "",
  "Component Seq Text" = ""
  WITH outdev, nodeid, comptype,
  conceptgrp, compseqtxt
 FREE RECORD request
 RECORD request(
   1 component_list[*]
     2 cp_node_id = f8
 ...
 RECORD mpagereply(
   1 cp_node_id = f8
   1 comp_type_cd = f8
   1 comp_type_cd_meaning = vc
   1 concept_group_cd = f8
   1 cp_component_id = f8
   1 component_seq_txt = vc
   1 comp_type_disp = vc
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 DECLARE comptypecd = f8 WITH constant(uar_get_code_by("MEANING",comp_type_cd_codeset, $COMPTYPE)),
 protect
 DECLARE conceptgrpcd = f8 WITH constant(uar_get_code_by("MEANING",concept_grp_cd_codeset,
    $CONCEPTGRP)), protect
 ...
 SUBROUTINE verifyinputs(null)
   ...
   ENDIF
   CALL verifynonzerocodevalue(comptypecd,comp_type_cd_codeset,"Component Type")
   CALL checkcodevalue(conceptgrpcd,concept_grp_cd_codeset)
   SELECT INTO "NL:"
    FROM cp_node cn
 ...
 SUBROUTINE insertrows(null)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SET stat = alterlist(request->component_list,1)
   SET request->component_list[1].cp_node_id = nodeid
   SET request->component_list[1].concept_group_cd = conceptgrpcd
   SET request->component_list[1].comp_type_cd = comptypecd
   SET request->component_list[1].component_seq_txt = compseqtxt
   ...
   IF (size(reply->component_list,5) > 0)
    SET mpagereply->comp_type_cd = reply->component_list[1].comp_type_cd
    SET mpagereply->comp_type_disp = uar_get_code_display(reply->component_list[1].comp_type_cd)
    SET mpagereply->comp_type_cd_meaning = uar_get_code_meaning(reply->component_list[1].comp_type_cd
     )
    SET mpagereply->component_seq_txt = reply->component_list[1].component_seq_txt
    SET mpagereply->concept_group_cd = reply->component_list[1].concept_group_cd
    SET mpagereply->cp_component_id = reply->component_list[1].cp_component_id
    SET mpagereply->cp_node_id = reply->component_list[1].cp_node_id
   ENDIF
   SET stat = moverec(reply->status_data,mpagereply->status_data)
