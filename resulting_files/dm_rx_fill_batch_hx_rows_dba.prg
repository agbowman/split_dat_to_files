CREATE PROGRAM dm_rx_fill_batch_hx_rows:dba
 ...
 SET reply->table_name = "FILL_BATCH_HX"
 ...
 FOR (i = 1 TO size(request->tokens,5))
   IF ((request->tokens[i].token_str="DAYSTOKEEP"))
    SET v_days_to_keep = ceil(cnvtreal(request->tokens[i].value))
   ENDIF
 ENDFOR
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(fbh.fill_hx_id)
    FROM fill_batch_hx fbh
    WHERE fbh.fill_hx_id > 0
    DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     fbh.rowid
     FROM fill_batch_hx fbh
     WHERE parser(sbr_getrowidnotexists("fbh.fill_hx_id between curMinID and curMaxID","fbh"))
      AND ((fbh.to_dt_tm+ 0) < cnvtdatetime((curdate - v_days_to_keep),curtime3))
     DETAIL
      v_rec_count = (v_rec_count+ 1)
      IF (mod(v_rec_count,20)=1)
       stat = alterlist(reply->rows,(v_rec_count+ 19))
      ENDIF
      reply->rows[v_rec_count].row_id = fbh.rowid
     WITH nocounter, maxqual(fbh,value(rowsleft))
    ;end select
