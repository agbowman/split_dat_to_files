CREATE PROGRAM bhs_rpt_matern_hepb:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  HEAD e.person_id
   ...
   f_delivery_date = cdr.result_dt_tm,
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ppr
   ...
   WHERE p.person_id=ppr.related_person_id
    AND p.birth_dt_tm > cnvtdatetimeutc(datetimezone(cnvtdatetime((curdate - 30),curtime3),p.birth_tz
     ),1))
   JOIN (pa
   WHERE (pa.person_id= Outerjoin(p.person_id))
    AND (pa.active_ind= Outerjoin(1))
  ...
  HEAD p.person_id
   m_rec->qual[ml_idx1].l_ch_cnt += 1, stat = alterlist(m_rec->qual[ml_idx1].qual,m_rec->qual[ml_idx1
    ].l_ch_cnt), m_rec->qual[ml_idx1].qual[m_rec->qual[ml_idx1].l_ch_cnt].f_child_person_id = p
   .person_id,
 ...
 IF (ms_outdev="OPS")
  ...
  DECLARE ms_email = vc WITH protect, constant("obhepatitisbcompliance@bhs.org")
  EXECUTE bhs_ma_email_file
  SET ms_tmp = concat("HepB Report: ",format(cnvtdatetime(sysdate),"YYYYMMDDHHMMSS;;q"))
