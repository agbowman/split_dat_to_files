CREATE PROGRAM cv_upd_proc_ind:dba
 ...
 UPDATE  FROM cv_proc p
  SET p.proc_indicator = request->data_import_indicator, p.updt_task = reqinfo->updt_task, p.updt_id
    = reqinfo->updt_id,
   p.updt_applctx = reqinfo->updt_applctx, p.updt_dt_tm = cnvtdatetime(sysdate), p.updt_cnt = (p
   .updt_cnt+ 1)
