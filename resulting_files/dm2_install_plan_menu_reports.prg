CREATE PROGRAM dm2_install_plan_menu_reports
 ...
 ENDIF
 DECLARE dipmr_logfile_prefix = vc WITH protect, constant("dm2_inst_plan_rpt")
 DECLARE dipmr_continue_loop = i2 WITH protect, noconstant(1)
 DECLARE dipmr_option = i2 WITH protect, noconstant(0)
 DECLARE dipmr_disp_install_itinerary(ddit_install_plan=f8) = i2
 DECLARE dipmr_show_logfiles(dsl_install_plan=f8,dsl_dpe_id=f8) = i2
 DECLARE dipmr_show_all_rpt(dsar_install_plan=f8,dsar_dpe_id=f8) = i2
 DECLARE dipmr_set_ddl_rpt_options(dsdro_install_plan=f8) = i2
 SET width = 132
 IF (check_logfile(dipmr_logfile_prefix,".log","dm2_install_plan_menu_rpt")=0)
  CALL dipr_disp_error_msg(null)
  GO TO exit_program
 ENDIF
 ...
 IF ((dipm_misc_data->install_plan_id=0))
  ...
  ENDIF
 ENDIF
 IF ((dipm_misc_data->cur_dpe_id=0))
  IF (dipr_get_cur_dpe_data(dipm_misc_data->install_plan_id)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 WHILE (dipmr_continue_loop=1)
   SET message = window
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL text(2,2,"INSTALL PLAN MENU [REPORTS]")
   CALL text(2,70,"DATE/TIME: ")
   CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
   CALL text(5,2,"Plan Number: ")
   CALL text(5,18,drr_flex_sched->pkg_number)
   CALL text(7,2,"1. View Install Event Summary ")
   CALL text(8,2,"2. View Install Log Files")
   CALL text(9,2,"3. View Active Install Processes (Install, Runners, Monitoring)")
   CALL text(10,2,"4. View Readme Execution Reports")
   CALL text(11,2,"5. View Schema (DDL) Reports")
   CALL text(12,2,"6. View ALL Install Reports")
   CALL text(14,2,"Your selection or 0 to go back: ")
   CALL accept(14,34,"9",0
    WHERE curaccept IN (0, 1, 2, 3, 4,
    5, 6))
   SET dipmr_option = curaccept
   SET message = nowindow
   CASE (dipmr_option)
    OF 0:
     SET dipmr_continue_loop = 0
    OF 1:
     IF (dipr_get_install_itinerary(dipm_misc_data->install_plan_id)=0)
      GO TO exit_program
     ENDIF
     IF (dipmr_disp_install_itinerary(dipm_misc_data->install_plan_id)=0)
      GO TO exit_program
     ENDIF
     SET dipmr_continue_loop = 1
    OF 2:
     IF (dipmr_show_logfiles(dipm_misc_data->install_plan_id,dipm_misc_data->cur_dpe_id)=0)
      GO TO exit_program
     ENDIF
     SET dipmr_continue_loop = 1
    OF 3:
     ...
     SET dipmr_continue_loop = 1
    OF 4:
     EXECUTE dm_readme_ocd_log (dipm_misc_data->install_plan_id * - (1))
     IF ((dm_err->err_ind > 0))
      GO TO exit_program
     ENDIF
     SET dipmr_continue_loop = 1
    OF 5:
     IF (dipmr_set_ddl_rpt_options(dipm_misc_data->install_plan_id)=0)
      GO TO exit_program
     ENDIF
     ...
     SET dipmr_continue_loop = 1
    OF 6:
     IF (dipmr_show_all_rpt(dipm_misc_data->install_plan_id,dipm_misc_data->cur_dpe_id)=0)
      GO TO exit_program
     ENDIF
     SET dipmr_continue_loop = 1
   ENDCASE
 ENDWHILE
 GO TO exit_program
 SUBROUTINE dipmr_disp_install_itinerary(ddit_install_plan)
   DECLARE ddit_file = vc WITH protect, noconstant("")
   DECLARE ddit_cnt = i2 WITH protect, noconstant(0)
   DECLARE ddit_cur_mode = vc WITH protect, noconstant("DM2NOTSET")
   DECLARE ddit_prev_mode = vc WITH protect, noconstant("DM2NOTSET")
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dip_itin_rs)
   ENDIF
   IF (get_unique_file("dipmr_itn",".dat")=0)
    RETURN(0)
   ELSE
    SET ddit_file = dm_err->unique_fname
   ENDIF
   SET dm_err->eproc = "Creating file for Package Install Event Summary"
   SELECT INTO value(ddit_file)
    FROM dummyt d
    HEAD REPORT
     CALL print(fillstring(250,"-")), row + 1, "INSTALL PLAN MENU [INSTALL EVENT SUMMARY]",
     row + 1,
     CALL print(build("File Name: ",ddit_file)), row + 1,
     CALL print(fillstring(250,"-")), row + 1,
     CALL print(concat("  Plan Number: ",drr_flex_sched->pkg_number)),
     row + 1
    DETAIL
     CALL print(fillstring(250,"-")), row + 1, col 0,
     "STEP NO. ", col 10, "| STEP NAME ",
     col 50, "| EVENT STATUS", col 65,
     "| BEGIN DATE", col 85, "| END DATE",
     row + 1,
     CALL print(fillstring(250,"-")), row + 2,
     ddit_cnt = 0
     FOR (ddit_cnt = 1 TO dip_itin_rs->itin_cnt)
       ddit_cur_mode = dip_itin_rs->itin_step[ddit_cnt].install_mode
       IF (cnvtupper(ddit_cur_mode) != cnvtupper(ddit_prev_mode))
        row + 1, col 0,
        CALL print(concat(fillstring(45,"*"),format(cnvtupper(ddit_cur_mode),"#############;C;"),
         fillstring(45,"*"))),
        row + 2
       ENDIF
       ddit_prev_mode = ddit_cur_mode, col 0,
       CALL print(build(dip_itin_rs->itin_step[ddit_cnt].step_number)),
       col 10, "| ",
       CALL print(dip_itin_rs->itin_step[ddit_cnt].step_name),
       col 50, "| ",
       CALL print(evaluate(trim(dip_itin_rs->itin_step[ddit_cnt].event_status),"","READY",dip_itin_rs
        ->itin_step[ddit_cnt].event_status)),
       col 65, "| ",
       CALL print(format(dip_itin_rs->itin_step[ddit_cnt].begin_dt_tm,"DD-MMM-YY HH:MM;;D")),
       col 85, "| ",
       CALL print(format(dip_itin_rs->itin_step[ddit_cnt].end_dt_tm,"DD-MMM-YY HH:MM;;D")),
       row + 1
     ENDFOR
   ...
   IF (check_error(dm_err->eproc))
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Displaying Package Install Event Summary"
   IF (dm2_disp_file(ddit_file,"Install Event Summary")=0)
    RETURN(0)
   ENDIF
   IF (check_error(dm_err->eproc))
    CALL dipr_disp_error_msg(null)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dipmr_set_ddl_rpt_options(dsdro_install_plan)
   DECLARE dsdro_run_id = f8 WITH protect, noconstant(0.0)
   SET dm_err->eproc = "Obtaining run_id of current package install from dm2_ddl_ops"
   SELECT INTO "nl:"
    FROM dm2_ddl_ops ddo
    WHERE (ddo.ocd=(dsdro_install_plan * - (1)))
     AND ddo.process_option="CLIN UPGRADE"
    ORDER BY ddo.run_id DESC
    DETAIL
     dsdro_run_id = ddo.run_id
   ...
   ENDIF
   SET dm2rpting->prompt_mode = "PROMPT"
   SET dm2rpting->run_id = dsdro_run_id
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dipmr_show_all_rpt(dsar_install_plan,dsar_dpe_id)
   DECLARE dsar_continue = i2 WITH protect, noconstant(1)
   DECLARE dsar_loc = vc WITH protect, noconstant("")
   DECLARE dsar_idx = i2 WITH protect, noconstant(0)
   DECLARE dsar_invalid = i2 WITH protect, noconstant(0)
   DECLARE dsar_lines = i4 WITH protect, noconstant(0)
   DECLARE dsar_max_lines = i4 WITH protect, noconstant(20000)
   DECLARE dsar_accept = vc WITH protect, noconstant("")
   DECLARE dsar_text = vc WITH protect, noconstant("")
   DECLARE dsar_cnt = i2 WITH protect, noconstant(0)
   DECLARE dsar_file = vc WITH protect, noconstant("")
   DECLARE dsar_type = vc WITH protect, noconstant("")
   DECLARE dsar_option1 = vc WITH protect, noconstant("")
   DECLARE dsar_option2 = vc WITH protect, noconstant("")
   DECLARE dsar_option3 = vc WITH protect, noconstant("")
   FREE RECORD dsar_reports
   RECORD dsar_reports(
     1 cnt = i4
     1 qual[*]
       2 dpe_id = f8
   ...
   FREE RECORD dsar_reports_help_list
   RECORD dsar_reports_help_list(
     1 cnt = i4
     1 qual[*]
       2 dtl_id = f8
       2 rpt_name = vc
   ...
   SET dm_err->eproc = "Processing reports for all modes in current package install"
   IF ((dm_err->debug_flag > 0))
    CALL dipr_disp_error_msg(null)
   ENDIF
   SELECT INTO "nl:"
    ...
    PLAN (dp
     ...
     JOIN (dpe
     WHERE dp.dm_process_id=dpe.dm_process_id
      AND dpe.install_plan_id=dsar_install_plan)
     JOIN (dped
     WHERE dpe.dm_process_event_id=dped.dm_process_event_id
      AND dped.detail_type="RPT:*")
    ORDER BY dpe.dm_process_event_id DESC, dped.detail_dt_tm DESC
    HEAD REPORT
     dsar_cnt = 0
    DETAIL
     dsar_cnt = (dsar_cnt+ 1)
     IF (mod(dsar_cnt,10)=1)
      stat = alterlist(dsar_reports->qual,(dsar_cnt+ 9))
     ENDIF
     dsar_reports->qual[dsar_cnt].dpe_id = dpe.dm_process_event_id, dsar_reports->qual[dsar_cnt].
     dtl_id = dped.dm_process_event_dtl_id, dsar_reports->qual[dsar_cnt].rpt_name = dped.detail_type,
     dsar_reports->qual[dsar_cnt].rpt_file_name = dped.detail_text, dsar_reports->qual[dsar_cnt].
     rpt_file_date = cnvtdatetime(dped.detail_dt_tm), dsar_reports->qual[dsar_cnt].is_current =
     evaluate(dsar_reports->qual[dsar_cnt].dpe_id,dsar_dpe_id,1,0)
    FOOT REPORT
     dsar_reports->cnt = dsar_cnt, stat = alterlist(dsar_reports->qual,dsar_reports->cnt)
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL dipr_disp_error_msg(null)
    RETURN(0)
   ENDIF
   FOR (dsar_idx = 1 TO dsar_reports->cnt)
    IF ((dm2_sys_misc->cur_os != "AXP"))
     SET dsar_loc = concat(trim(logical("ccluserdir")),"/")
    ELSE
     SET dsar_loc = trim(logical("ccluserdir"))
    ENDIF
    IF (dm2_findfile(build(dsar_loc,dsar_reports->qual[dsar_idx].rpt_file_name))=0)
     IF ((dm_err->err_ind=1))
      RETURN(0)
     ELSE
      SET dsar_reports->qual[dsar_idx].rpt_exists = 0
     ENDIF
    ELSE
     SET dsar_reports->qual[dsar_idx].rpt_exists = 1
    ENDIF
   ENDFOR
   IF ((dm_err->debug_flag > 0))
    CALL echo(cnvtstring(dsar_dpe_id))
    CALL echorecord(dsar_reports)
   ENDIF
   SET dm_err->eproc = "Processing reports for help list"
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = size(dsar_reports->qual,5))
    WHERE (dsar_reports->qual[d.seq].rpt_exists=1)
    HEAD REPORT
     dsar_cnt = 0
    DETAIL
     dsar_cnt = (dsar_cnt+ 1)
     IF (mod(dsar_cnt,10)=1)
      stat = alterlist(dsar_reports_help_list->qual,(dsar_cnt+ 9))
     ENDIF
     dsar_reports_help_list->qual[dsar_cnt].dtl_id = dsar_reports->qual[d.seq].dtl_id,
     dsar_reports_help_list->qual[dsar_cnt].rpt_name = dsar_reports->qual[d.seq].rpt_name,
     dsar_reports_help_list->qual[dsar_cnt].rpt_file_name = dsar_reports->qual[d.seq].rpt_file_name,
     dsar_reports_help_list->qual[dsar_cnt].date_str = format(dsar_reports->qual[d.seq].rpt_file_date,
      "DD-MMM-YY HH:MM;;D")
    FOOT REPORT
     dsar_reports_help_list->cnt = dsar_cnt, stat = alterlist(dsar_reports_help_list->qual,
      dsar_reports_help_list->cnt)
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL dipr_disp_error_msg(null)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Retrieving max display value from dm_info"
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="DM2_INSTALL_PLAN_MENU"
     AND di.info_name="FILE_DISPLAY_SIZE"
    DETAIL
     dsar_max_lines = di.info_number
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL dipr_disp_error_msg(null)
    GO TO exit_program
   ENDIF
   WHILE (dsar_continue=1)
     SET message = window
     CALL clear(1,1)
     CALL box(1,1,3,132)
     CALL text(2,2,"INSTALL PLAN MENU [VIEW ALL REPORTS]")
     CALL text(2,70,"DATE/TIME: ")
     CALL text(2,80,format(cnvtdatetime(curdate,curtime3),"DD-MMM-YYYY HH:MM:SS;;D"))
     IF ((dsar_reports->cnt=0))
      CALL text(5,2,"No package Install Reports Exist. Press <enter> to go back.")
      CALL accept(5,62,"p;cu"," "
       WHERE curaccept IN (" "))
      SET dsar_continue = 0
     ELSE
      CALL text(5,2,"(C)hoose Report, (R)eport Details, (B)ack: ")
      CALL accept(5,44,"p;cu","B"
       WHERE curaccept IN ("C", "R", "B"))
      SET dsar_option1 = curaccept
      CASE (dsar_option1)
       OF "B":
        SET dsar_continue = 0
       OF "C":
        CALL clear(5,1)
        IF ((dsar_reports_help_list->cnt=0))
         CALL text(5,2,"No package Install Reports are available to view. Press <enter> to go back.")
         CALL accept(5,78,"p;cu"," "
          WHERE curaccept IN (" "))
        ELSE
         CALL text(5,2,"Please select report name from list.")
         CALL text(7,2,"Report File: ")
         SET help = pos(5,50,10,60)
         SET help =
         SELECT INTO "nl:"
          report_name = substring(1,30,dsar_reports_help_list->qual[d.seq].rpt_name), create_dt_tm =
          dsar_reports_help_list->qual[d.seq].date_str
          FROM (dummyt d  WITH seq = size(dsar_reports_help_list->qual,5))
          WITH nocounter
         ;end select
         CALL accept(7,16,"A(30);CF")
         SET dsar_idx = 0
         SET dsar_option2 = curaccept
         SET dsar_invalid = 0
         IF (curhelp > 0)
          SET dsar_type = dsar_reports_help_list->qual[curhelp].rpt_name
          SET dsar_file = dsar_reports_help_list->qual[curhelp].rpt_file_name
         ELSE
          SET dsar_idx = locateval(dsar_cnt,1,size(dsar_reports_help_list->qual,5),curaccept,
           dsar_reports_help_list->qual[dsar_cnt].rpt_name)
          IF (dsar_idx > 0)
           SET dsar_type = dsar_reports_help_list->qual[dsar_idx].rpt_name
           SET dsar_file = dsar_reports_help_list->qual[dsar_idx].rpt_file_name
           SET help = off
          ELSE
           CALL text(9,2,"Report File Does Not Exist in CCLUSERDIR. Press <enter> to go back.")
           CALL accept(9,69,"p;cu"," "
            WHERE curaccept IN (" "))
           SET dsar_invalid = 1
          ENDIF
         ENDIF
         IF (dsar_invalid=0)
          CALL clear(5,1)
          CALL text(7,2,"Report Name: ")
          CALL text(7,16,dsar_type)
          CALL text(8,2,"Report File: ")
          CALL text(8,16,dsar_file)
          CALL text(10,2,"Display First ")
          CALL text(10,22,"Lines from the Report")
          CALL text(11,2,concat("(Enter a value between 1 and ",build(dsar_max_lines),")"))
          CALL accept(10,16,"9(5)",200
           WHERE curaccept >= 0
            AND curaccept <= dsar_max_lines)
          SET dsar_lines = curaccept
          CALL text(15,2,"(V)iew Report, (B)ack: ")
          CALL accept(15,38,"p;cu","V"
           WHERE curaccept IN ("V", "B"))
          SET dsar_option3 = curaccept
          SET message = nowindow
          CASE (dsar_option3)
           OF "B":
            SET dsar_continue = 1
           OF "V":
            SET dm_err->eproc = concat("Displaying selected report: ",dsar_loc,dsar_file)
            SET logical file_full value(concat(dsar_loc,dsar_file))
            FREE DEFINE rtl2
            DEFINE rtl2 "file_full"
            SELECT INTO mine
             t.line
             FROM rtl2t t
             HEAD REPORT
              col 30,
              CALL print(dsar_file), row + 1
             DETAIL
              col 0, t.line, row + 1
             FOOT REPORT
              row + 0
             WITH nocounter, maxcol = 5000, maxrec = dsar_lines
            ;end select
            IF (check_error(dm_err->eproc)=1)
             CALL dipr_disp_error_msg(null)
             RETURN(0)
       ...
       OF "R":
        SET message = nowindow
        SET dm_err->eproc = "Generating Package Install Report Detail Summary"
        CALL dipr_disp_error_msg(null)
        IF (get_unique_file("dipmr_dtl_rpt",".dat")=0)
         RETURN(0)
        ELSE
         SET dsar_file = dm_err->unique_fname
        ENDIF
        SELECT INTO value(dsar_file)
         FROM dummyt d
         HEAD REPORT
          CALL print(fillstring(250,"-")), row + 1, "INSTALL PLAN MENU [INSTALL REPORTS DETAILS]",
          row + 1,
          CALL print(fillstring(250,"-")), row + 2,
          CALL print(concat("  Plan Number: ",drr_flex_sched->pkg_number)), row + 1
         DETAIL
          CALL print(fillstring(250,"-")), row + 1, col 0,
          "REPORT NAME ", col 30, "| FILE NAME ",
          col 70, "| CREATED ON", col 90,
          "| EXISTS", col 100, "| CURRENT",
          row + 1,
          CALL print(fillstring(250,"-")), row + 2,
          dsar_cnt = 0
          FOR (dsar_cnt = 1 TO dsar_reports->cnt)
            col 0,
            CALL print(dsar_reports->qual[dsar_cnt].rpt_name), col 30,
            "| ",
            CALL print(dsar_reports->qual[dsar_cnt].rpt_file_name), col 70,
            "| ",
            CALL print(format(dsar_reports->qual[dsar_cnt].rpt_file_date,"DD-MMM-YY HH:MM;;D")), col
            90,
            "| ",
            CALL print(evaluate(dsar_reports->qual[dsar_cnt].rpt_exists,1,"YES",0,"NO")), col 100,
            "| ",
            CALL print(evaluate(dsar_reports->qual[dsar_cnt].is_current,1,"YES",0,"NO")), row + 1
          ENDFOR
         WITH nocounter, maxcol = 500, formfeed = none
        ;end select
        IF (check_error(dm_err->eproc))
         ...
         RETURN(0)
        ENDIF
        SET dm_err->eproc = "Displaying Package Install Report Detail Summary"
        IF (dm2_disp_file(dsar_file,"Install Report Details")=0)
         RETURN(0)
        ENDIF
        IF (check_error(dm_err->eproc))
         CALL dipr_disp_error_msg(null)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dipmr_show_logfiles(dsl_install_plan,dsl_dpe_id)
   DECLARE dsl_continue = i2 WITH protect, noconstant(1)
   DECLARE dsl_loc = vc WITH protect, noconstant("")
   DECLARE dsl_idx = i2 WITH protect, noconstant(0)
   DECLARE dsl_sort_id = i2 WITH protect, noconstant(1)
   DECLARE dsl_invalid = i2 WITH protect, noconstant(0)
   DECLARE dsl_lines = i4 WITH protect, noconstant(0)
   DECLARE dsl_max_lines = i4 WITH protect, noconstant(20000)
   DECLARE dsl_accept = vc WITH protect, noconstant("")
   DECLARE dsl_cnt = i2 WITH protect, noconstant(0)
   DECLARE dsl_init = i2 WITH protect, noconstant(1)
   DECLARE dsl_file = vc WITH protect, noconstant("")
   DECLARE dsl_type = vc WITH protect, noconstant("")
   DECLARE dsl_option1 = vc WITH protect, noconstant("")
   DECLARE dsl_option2 = vc WITH protect, noconstant("")
   DECLARE dsl_option3 = vc WITH protect, noconstant("")
   DECLARE dsl_cmd = vc WITH protect, noconstant("")
   DECLARE dsl_tail_file = vc WITH protect, noconstant("")
   FREE RECORD dsl_logs
   RECORD dsl_logs(
     1 cnt = i4
     1 qual[*]
       2 dpe_id = f8
   ...
   FREE RECORD dsl_logs_help_list
   RECORD dsl_logs_help_list(
     1 cnt = i4
     1 qual[*]
       2 dpe_id = f8
       2 log_name = vc
   ...
   SET dm_err->eproc = "Processing logfiles for all modes in current package install"
   IF ((dm_err->debug_flag > 0))
    CALL dipr_disp_error_msg(null)
   ENDIF
   SELECT INTO "nl:"
    ...
    PLAN (dp
     ...
     JOIN (dpe
     WHERE dp.dm_process_id=dpe.dm_process_id
      AND dpe.install_plan_id=dsl_install_plan)
     JOIN (dped
     WHERE dpe.dm_process_event_id=dped.dm_process_event_id
      AND dped.detail_type="LOGFILE:*")
    ORDER BY dpe.dm_process_event_id DESC, dped.detail_dt_tm DESC
    HEAD REPORT
     dsl_cnt = 0
    DETAIL
     dsl_cnt = (dsl_cnt+ 1)
     IF (mod(dsl_cnt,10)=1)
      stat = alterlist(dsl_logs->qual,(dsl_cnt+ 9))
     ENDIF
     dsl_logs->qual[dsl_cnt].dpe_id = dpe.dm_process_event_id, dsl_logs->qual[dsl_cnt].dtl_id = dped
     .dm_process_event_dtl_id, dsl_logs->qual[dsl_cnt].log_name = dped.detail_type,
     dsl_logs->qual[dsl_cnt].log_file_name = dped.detail_text, dsl_logs->qual[dsl_cnt].log_file_date
      = cnvtdatetime(dped.detail_dt_tm), dsl_logs->qual[dsl_cnt].date_str = format(dsl_logs->qual[
      dsl_cnt].log_file_date,"DD-MMM-YY HH:MM;;D"),
     dsl_logs->qual[dsl_cnt].is_current = evaluate(dsl_logs->qual[dsl_cnt].dpe_id,dsl_dpe_id,1,0),
     dsl_logs->qual[dsl_cnt].mode = evaluate(dp.process_name,value(dpl_package_install),
      "Package Install",value(dpl_install_monitor),"Monitoring")
     IF ((dsl_logs->qual[dsl_cnt].mode="Monitoring"))
      dsl_logs->qual[dsl_cnt].is_current = 2
     ENDIF
    FOOT REPORT
     dsl_logs->cnt = dsl_cnt, stat = alterlist(dsl_logs->qual,dsl_logs->cnt)
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL dipr_disp_error_msg(null)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Obtaining modes for all package install log files"
   IF ((dm_err->debug_flag > 0))
    CALL dipr_disp_error_msg(null)
   ENDIF
   SELECT INTO "nl:"
    FROM dm_process_event_dtl dped,
     (dummyt d  WITH seq = size(dsl_logs->qual,5))
    PLAN (d
     WHERE d.seq > 0
      AND (dsl_logs->qual[d.seq].mode="Package Install"))
     JOIN (dped
     WHERE (dped.dm_process_event_id=dsl_logs->qual[d.seq].dpe_id)
      AND dped.detail_type="INSTALL_MODE")
    DETAIL
     IF ((dsl_logs->qual[d.seq].log_name="LOGFILE:MAIN*"))
      dsl_logs->qual[d.seq].mode = concat(trim(dped.detail_text),":Main")
     ELSEIF ((dsl_logs->qual[d.seq].log_name="LOGFILE:RUNNER*"))
      dsl_logs->qual[d.seq].mode = concat(trim(dped.detail_text),":Runner")
     ELSEIF ((dsl_logs->qual[d.seq].log_name="LOGFILE:BACKGROUND*"))
      dsl_logs->qual[d.seq].mode = concat(trim(dped.detail_text),":Reflections Log")
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Setting sort_ids for logfiles"
   IF ((dm_err->debug_flag > 0))
    CALL dipr_disp_error_msg(null)
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = size(dsl_logs->qual,5))
    ORDER BY dsl_logs->qual[d.seq].dpe_id DESC, dsl_logs->qual[d.seq].log_file_date DESC
    HEAD REPORT
     dsl_cnt = 2
    DETAIL
     dsl_cnt = (dsl_cnt+ 1)
     IF ((dsl_logs->qual[d.seq].dpe_id=dsl_dpe_id)
      AND (dsl_logs->qual[d.seq].mode != "Monitoring"))
      IF ((dsl_logs->qual[d.seq].log_name="LOGFILE:MAIN"))
       dsl_logs->qual[d.seq].sort_id = 2, dsl_cnt = (dsl_cnt - 1)
      ELSEIF ((dsl_logs->qual[d.seq].log_name="LOGFILE:BACKGROUND*"))
       dsl_logs->qual[d.seq].sort_id = 1, dsl_cnt = (dsl_cnt - 1)
      ELSE
       dsl_logs->qual[d.seq].sort_id = dsl_cnt
      ENDIF
     ELSE
      dsl_logs->qual[d.seq].sort_id = dsl_cnt
     ENDIF
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   FOR (dsl_idx = 1 TO dsl_logs->cnt)
    IF ((dm2_sys_misc->cur_os != "AXP"))
     SET dsl_loc = concat(trim(logical("ccluserdir")),"/")
    ELSE
     SET dsl_loc = trim(logical("ccluserdir"))
    ENDIF
    IF (dm2_findfile(build(dsl_loc,dsl_logs->qual[dsl_idx].log_file_name))=0)
     IF ((dm_err->err_ind=1))
      RETURN(0)
     ELSE
      SET dsl_logs->qual[dsl_idx].log_exists = 0
     ENDIF
    ELSE
     SET dsl_logs->qual[dsl_idx].log_exists = 1
    ENDIF
   ENDFOR
   IF ((dm_err->debug_flag > 0))
    CALL echo(cnvtstring(dsl_dpe_id))
    CALL echorecord(dsl_logs)
   ENDIF
   SET dm_err->eproc = "Processing logs for help list"
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = size(dsl_logs->qual,5))
    WHERE (dsl_logs->qual[d.seq].log_exists=1)
     AND (dsl_logs->qual[d.seq].mode != "Monitoring")
    ORDER BY dsl_logs->qual[d.seq].sort_id
    HEAD REPORT
     dsl_cnt = 0
    DETAIL
     dsl_cnt = (dsl_cnt+ 1)
     IF (mod(dsl_cnt,10)=1)
      stat = alterlist(dsl_logs_help_list->qual,(dsl_cnt+ 9))
     ENDIF
     dsl_logs_help_list->qual[dsl_cnt].dpe_id = dsl_logs->qual[d.seq].dpe_id, dsl_logs_help_list->
     qual[dsl_cnt].log_name = dsl_logs->qual[d.seq].log_name, dsl_logs_help_list->qual[dsl_cnt].mode
      = dsl_logs->qual[d.seq].mode,
     dsl_logs_help_list->qual[dsl_cnt].log_file_name = dsl_logs->qual[d.seq].log_file_name,
     dsl_logs_help_list->qual[dsl_cnt].date_str = dsl_logs->qual[d.seq].date_str
    FOOT REPORT
     dsl_logs_help_list->cnt = dsl_cnt, stat = alterlist(dsl_logs_help_list->qual,dsl_logs_help_list
      ->cnt)
    WITH nocounter, nullreport
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   WHILE (dsl_continue=1)
     SET message = window
     CALL clear(1,1)
     CALL box(1,1,3,132)
     CALL text(2,2,"INSTALL PLAN MENU [VIEW ALL LOGFILES]")
     CALL text(2,70,"DATE/TIME: ")
     CALL text(2,80,concat(build(month(curdate),"/",day(curdate),"/",year(curdate)),"  ",build(hour(
         curtime),":",minute(curtime))))
     IF ((dsl_logs->cnt=0))
      CALL text(5,2,"No package Install Logfiles Exist. Press <enter> to go back.")
      CALL accept(5,62,"p;cu"," "
       WHERE curaccept IN (" "))
      SET dsl_continue = 0
     ELSE
      CALL text(5,2,"(C)hoose Logfile, (L)ogfile details, (B)ack: ")
      CALL accept(5,48,"p;cu","B"
       WHERE curaccept IN ("C", "L", "B"))
      SET dsl_option1 = curaccept
      CASE (dsl_option1)
       OF "B":
        SET dsl_continue = 0
       OF "C":
        CALL clear(5,1)
        IF ((dsl_logs_help_list->cnt=0))
         CALL text(5,2,"No package Install Logfiles are available to view. Press <enter> to go back."
          )
         CALL accept(5,78,"p;cu"," "
          WHERE curaccept IN (" "))
        ELSE
         CALL text(5,2,"Please select logfile name from list.")
         CALL text(7,2,"Logfile Type: ")
         SET help = pos(5,50,10,80)
         SET help =
         SELECT INTO "nl:"
          process = substring(1,25,dsl_logs_help_list->qual[d.seq].mode), logfile_name = substring(1,
           25,dsl_logs_help_list->qual[d.seq].log_name), create_dt_tm = dsl_logs_help_list->qual[d
          .seq].date_str
          FROM (dummyt d  WITH seq = size(dsl_logs_help_list->qual,5))
          WITH nocounter
         ;end select
         CALL accept(7,16,"A(30);CF")
         SET dsl_idx = 0
         SET dsl_option2 = curaccept
         SET dsl_invalid = 0
         IF (curhelp > 0)
          SET dsl_file = dsl_logs_help_list->qual[curhelp].log_file_name
          SET dsl_type = dsl_logs_help_list->qual[curhelp].log_name
         ELSE
          SET dsl_cnt = 0
          SET dsl_idx = locateval(dsl_cnt,1,size(dsl_logs_help_list->qual,5),curaccept,
           dsl_logs_help_list->qual[dsl_cnt].log_name)
          IF (dsl_idx > 0)
           SET dsl_file = dsl_logs_help_list->qual[dsl_idx].log_file_name
           SET help = off
          ELSE
           CALL text(9,2,"Logfile Does Not Exist in CCLUSERDIR. Press <enter> to go back.")
           CALL accept(9,65,"p;cu"," "
            WHERE curaccept IN (" "))
           SET dsl_invalid = 1
          ENDIF
         ENDIF
         IF (dsl_invalid=0)
          CALL clear(5,1)
          CALL text(7,2,"Logfile Type: ")
          CALL text(7,16,dsl_type)
          CALL text(8,2,"Logfile Name: ")
          CALL text(8,16,dsl_file)
          CALL text(10,2,"Display Last ")
          CALL text(10,22,"Lines from the Logfile")
          CALL text(11,2,concat("(Enter a value between 1 and ",build(dsl_max_lines),")"))
          CALL accept(10,16,"9(5)",200
           WHERE curaccept >= 0
            AND curaccept <= dsl_max_lines)
          SET dsl_lines = curaccept
          CALL text(15,2,"(V)iew Logfile, (B)ack: ")
          CALL accept(15,27,"p;cu","V"
           WHERE curaccept IN ("V", "B"))
          SET dsl_option3 = curaccept
          SET message = nowindow
          CASE (dsl_option3)
           OF "B":
            SET dsl_continue = 1
           OF "V":
            IF (get_unique_file("dipmr_tail_log",".dat")=0)
             RETURN(0)
            ELSE
             SET dsl_tail_file = dm_err->unique_fname
            ENDIF
            SET dm_err->eproc = concat("Obtaining tail of selected logfile: ",dsl_tail_file)
            IF ((dm2_sys_misc->cur_os="AXP"))
             SET dsl_cmd = concat("type /tail=",build(dsl_lines)," ",dsl_loc,dsl_file)
            ELSE
             SET dsl_cmd = concat("tail -n ",build(dsl_lines)," ",dsl_loc,dsl_file)
            ENDIF
            SET dm_err->errfile = dsl_tail_file
            IF (dm2_push_dcl(dsl_cmd)=0)
             RETURN(0)
            ENDIF
            SET dm_err->errfile = "NONE"
            SET dm_err->eproc = "Displaying Selected Logfile"
            IF (dm2_disp_file(dsl_tail_file,concat("Tail of Logfile: ",dsl_file))=0)
             RETURN(0)
            ENDIF
            IF (check_error(dm_err->eproc)=1)
             CALL dipr_disp_error_msg(null)
             RETURN(0)
            ENDIF
            SET dm_err->eproc = concat("Removing tail file: ",dsl_tail_file)
            IF ((dm2_sys_misc->cur_os="AXP"))
             SET dsl_tail_file = concat(dsl_tail_file,";*")
             IF (dm2_push_dcl(concat("del ",dsl_loc,dsl_tail_file))=0)
              RETURN(0)
             ENDIF
            ELSE
             IF (dm2_push_dcl(concat("rm ",dsl_loc,dsl_tail_file))=0)
              RETURN(0)
             ENDIF
            ENDIF
            SET dsl_continue = 1
       ...
       OF "L":
        SET message = nowindow
        SET dm_err->eproc = "Generating Package Install Logfile Summary"
        CALL dipr_disp_error_msg(null)
        IF (get_unique_file("dipmr_dtl_log",".dat")=0)
         RETURN(0)
        ELSE
         SET dsl_file = dm_err->unique_fname
        ENDIF
        SELECT INTO value(dsl_file)
         FROM dummyt d
         HEAD REPORT
          CALL print(fillstring(250,"-")), row + 1, "INSTALL PLAN MENU [INSTALL LOGFILES DETAILS]",
          row + 1,
          CALL print(fillstring(250,"-")), row + 2,
          CALL print(concat("  Plan Number: ",drr_flex_sched->pkg_number)), row + 1
         DETAIL
          CALL print(fillstring(250,"-")), row + 1, col 0,
          "PROCESS ", col 25, "| LOGFILE TYPE ",
          col 50, "| LOGFILE NAME ", col 85,
          "| CREATED ON", col 105, "| EXISTS",
          col 115, "| CURRENT", row + 1,
          CALL print(fillstring(250,"-")), row + 2, dsl_cnt = 0
          FOR (dsl_cnt = 1 TO dsl_logs->cnt)
            col 0,
            CALL print(dsl_logs->qual[dsl_cnt].mode), col 25,
            "| ",
            CALL print(dsl_logs->qual[dsl_cnt].log_name), col 50,
            "| ",
            CALL print(dsl_logs->qual[dsl_cnt].log_file_name), col 85,
            "| ",
            CALL print(format(dsl_logs->qual[dsl_cnt].log_file_date,"DD-MMM-YY HH:MM;;D")), col 105,
            "| ",
            CALL print(evaluate(dsl_logs->qual[dsl_cnt].log_exists,1,"YES",0,"NO",
             "N/A")), col 115,
            "| ",
            CALL print(evaluate(dsl_logs->qual[dsl_cnt].is_current,1,"YES",0,"NO",
             "N/A")), row + 1
          ENDFOR
         WITH nocounter, maxcol = 500, formfeed = none
        ;end select
        IF (check_error(dm_err->eproc))
         ...
         RETURN(0)
        ENDIF
        SET dm_err->eproc = "Displaying Package Install Logfile Details"
        IF (dm2_disp_file(dsl_file,"Install Report Details")=0)
         RETURN(0)
        ENDIF
        IF (check_error(dm_err->eproc))
         CALL dipr_disp_error_msg(null)
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "LEAVING dm2_install_plan_menu_reports"
 CALL final_disp_msg(dipmr_logfile_prefix)
END GO
