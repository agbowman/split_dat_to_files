CREATE PROGRAM bb_rdm_chg_cdf_display:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bb_rdm_chg_cdf_display "
 CALL logscriptstart(sreadme_name)
 UPDATE  FROM common_data_foundation cdf
  SET cdf.display = "Modifier/Attribute", cdf.definition = "Modifier/Attribute"
  WHERE cdf.code_set=1612
   AND cdf.cdf_meaning="SPTYP"
  WITH nocounter
 ;end update
 SET found = 0
 SELECT INTO "nl:"
  *
  ...
  WHERE cdf.code_set=1612
   AND cdf.cdf_meaning="SPTYP"
   AND cdf.display="Modifier/Attribute"
  DETAIL
   found = 1
  WITH nocounter
 ;end select
 ...
 ELSE
  ROLLBACK
  CALL logstatus(build(sreadme_name,"Execution Failed.  CDF display not updated."),"F")
 ENDIF
