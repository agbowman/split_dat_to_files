CREATE PROGRAM acm_update_gmp_gateway:dba
 ...
 DECLARE reg_practice_cd = f8 WITH public, constant(loadcodevalue(338,"REGPRACTICE",0))
 ...
 RECORD temp_data(
   ...
   1 person_qual[*]
     ...
     2 person_org_reltn_qual_cnt = i4
     2 person_org_reltn_qual[*]
       3 person_org_reltn_id = f8
       3 organization_id = f8
 )
 IF ((request->person_qual_cnt > 0))
  FOR (count = 1 TO request->person_qual_cnt)
    IF ((request->person_qual[count].person_id > 0))
     SET temp_data->person_qual_cnt += 1
     SET stat = alterlist(temp_data->person_qual,temp_data->person_qual_cnt)
     SET temp_data->person_qual[temp_data->person_qual_cnt].person_id = request->person_qual[count].
     person_id
    ENDIF
  ENDFOR
  IF ((temp_data->person_qual_cnt > 0))
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(temp_data->person_qual_cnt)),
     person_org_reltn por
    PLAN (d
     WHERE (temp_data->person_qual[d.seq].person_id > 0))
     JOIN (por
     WHERE (por.person_id=temp_data->person_qual[d.seq].person_id)
    ...
    DETAIL
     temp_data->person_qual[d.seq].person_org_reltn_qual_cnt += 1, stat = alterlist(temp_data->
      person_qual[d.seq].person_org_reltn_qual,temp_data->person_qual[d.seq].
      person_org_reltn_qual_cnt), temp_data->person_qual[d.seq].person_org_reltn_qual[temp_data->
     person_qual[d.seq].person_org_reltn_qual_cnt].person_org_reltn_id = por.person_org_reltn_id,
     temp_data->person_qual[d.seq].person_org_reltn_qual[temp_data->person_qual[d.seq].
     person_org_reltn_qual_cnt].organization_id = por.organization_id
    WITH nocounter
   ;end select
  ENDIF
  FOR (count = 1 TO temp_data->person_qual_cnt)
    IF ((temp_data->person_qual[count].person_org_reltn_qual_cnt > 0))
     SET temp_data->person_qual[count].organization_id = temp_data->person_qual[count].
     person_org_reltn_qual[1].organization_id
     FREE RECORD acm_gmp_request
     RECORD acm_gmp_request(
       1 person_id = f8
       1 gmp_prsnl_id = f8
       1 gp_practice_org_id = f8
     )
     FREE RECORD acm_gmp_reply
     RECORD acm_gmp_reply(
       1 gmp_prsnl_id = f8
       1 gp_practice_org_id = f8
       1 status_data
         2 status = c1
     ...
     SET acm_gmp_request->person_id = temp_data->person_qual[count].person_id
     SET acm_gmp_request->gp_practice_org_id = temp_data->person_qual[count].organization_id
     EXECUTE acm_update_gmp  WITH replace("REQUEST","ACM_GMP_REQUEST"), replace("REPLY",
      "ACM_GMP_REPLY")
     IF ((acm_gmp_reply->status_data.status="F"))
      EXECUTE sch_msgview "acm_update_gmp_gateway", nullterm(build(
        "########## ACM_UPDATE_GMP - build failure for patient [",acm_gmp_request->person_id,
        "] REGPRACTICE [",acm_gmp_request->gp_practice_org_id,"].")), 0
     ELSE
      IF ((acm_gmp_reply->gmp_prsnl_id > 0))
       EXECUTE sch_msgview "acm_update_gmp_gateway", nullterm(build(
         "********** ACM_UPDATE_GMP - build success for patient [",acm_gmp_request->person_id,
         "] REGPRACTICE [",acm_gmp_reply->gp_practice_org_id,"], PCP [",
         acm_gmp_reply->gmp_prsnl_id,"]")), 2
      ELSE
       EXECUTE sch_msgview "acm_update_gmp_gateway", nullterm(build(
         "********** ACM_UPDATE_GMP - no PCP for patient [",acm_gmp_request->person_id,
         "] REGPRACTICE [",acm_gmp_reply->gp_practice_org_id,"], PCP [",
         acm_gmp_reply->gmp_prsnl_id,"]")), 2
      ENDIF
     ENDIF
    ELSE
     EXECUTE sch_msgview "acm_update_gmp_gateway", nullterm(build(
       "********** ACM_UPDATE_GMP - no REGPRACTICE relationships for patient [",temp_data->
       person_qual[count].person_id,"]")), 2
    ENDIF
  ENDFOR
 ENDIF
#exit_script
