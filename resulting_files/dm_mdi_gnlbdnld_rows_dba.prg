CREATE PROGRAM dm_mdi_gnlbdnld_rows:dba
 ...
 SET reply->table_name = "CQM_GNLBDNLD_QUE"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(q.queue_id)
   FROM cqm_gnlbdnld_que q
   WHERE q.queue_id > 0
   DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    ...
    (SELECT
     a.trigger_id
     FROM cqm_gnlbdnld_tr_1 a
     WHERE a.queue_id=q.queue_id
      AND a.create_dt_tm < cnvtdatetime((curdate - days_to_keep),0)
      AND ((a.trigger_id+ 0) > 0)
      AND ((a.active_ind=0) OR (((a.process_status_flag+ 0)=90))) ))) OR (q.create_dt_tm <
    cnvtdatetime((curdate - days_to_keep),0)))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,100)=1)
