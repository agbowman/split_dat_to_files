CREATE PROGRAM dm_rmc_mover_1173:dba
 ...
 SUBROUTINE drmm_breakup_string(sbr_rs_reply)
   ...
   RETURN("S")
 END ;Subroutine
 DECLARE find_parent(cur_position=i4,parent_name=vc,parent_value=f8) = i4
 DECLARE alg_type = vc
 DECLARE merge_id = f8
 DECLARE merge_name = vc
 DECLARE merge_id_vc = vc
 DECLARE cust_tab_name = vc
 DECLARE pvc_value = f8
 DECLARE pvc_value_vc = vc
 DECLARE delim_cnt = i4
 DECLARE delim_vc = vc
 DECLARE bad_delim_ind = i2
 DECLARE pvc_value_check = i2
 DECLARE no_query = i2
 DECLARE val = f8
 DECLARE from_id = f8
 DECLARE new_id = f8
 ...
 DECLARE ins_upd = i2
 DECLARE cd_value = f8
 DECLARE cust_display = vc
 DECLARE cust_query_return = f8
 DECLARE cust_col_name = vc
 DECLARE cust_from_cnt = vc
 DECLARE cust_table_name = vc
 DECLARE cust_query_cnt = i4
 DECLARE cust_loop_cnt = i4
 DECLARE cust_insert_loop = i4
 DECLARE spec_pvc_name = vc
 DECLARE translated_prior = i2
 DECLARE prefs_col = i4
 DECLARE prefs_loop = i4
 DECLARE prefs_oldver_ind = i2
 DECLARE dcp_sect_name = vc
 DECLARE dcp_inp_name = vc
 DECLARE nvp_imt_check = i2
 DECLARE nvp_zero_col = i4
 DECLARE fndstring_val = i4
 DECLARE compare_cnt = i4
 DECLARE delim_list = vc
 DECLARE nvp_cdf_mean = vc
 DECLARE nvp_cv_tab_name = vc
 DECLARE pre_delim_string = vc
 DECLARE post_delim_string = vc
 DECLARE delim_list = vc
 DECLARE cust_pe_name = vc
 DECLARE new_delim_list = vc
 DECLARE cust_rpt_missing = vc
 DECLARE cust_err_val = i4
 DECLARE pvc_value_pos = i4
 DECLARE cust_pe_id = f8
 DECLARE drdm_semi_ind = i2
 FREE RECORD cust_ui_query
 RECORD cust_ui_query(
   1 qual[*]
     2 val = f8
 )
 DECLARE get_dyn_name(sbr_pvc_name=vc) = vc
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET translated_prior = 0
 SET pvc_value_pos = get_col_pos(cust_tab_name,"PVC_VALUE")
 SET cust_display = get_value(cust_tab_name,"PVC_NAME","FROM")
 IF (((trim(cust_display)="") OR (cust_display=" ")) )
  SET alg_type = "ALG0"
 ELSE
  SELECT INTO "NL:"
   FROM code_value c
   WHERE c.code_set=255350
    AND c.display=cust_display
    AND c.active_ind=1
   DETAIL
  ...
  ENDIF
  IF (curqual=0)
   SET spec_pvc_name = get_dyn_name(cust_display)
   CALL echo(spec_pvc_name)
   IF (spec_pvc_name != "")
    SELECT INTO "NL:"
     FROM code_value c
     WHERE c.code_set=255350
      AND c.display=spec_pvc_name
      AND c.active_ind=1
     DETAIL
      cd_value = c.code_value
     WITH nocounter
  ...
  ENDIF
 ENDIF
 SET nvp_cv_tab_name = dm2_get_rdds_tname("CODE_VALUE")
 IF ((rs_1173->from_values.parent_entity_name="DCP_INPUT_REF"))
  SET dcp_sect_name = dm2_get_rdds_tname("DCP_SECTION_REF")
  SET dcp_inp_name = dm2_get_rdds_tname("DCP_INPUT_REF")
  SET pref_oldver_ind = 0
  SELECT INTO "NL:"
   FROM (parser(dcp_sect_name) d)
   WHERE d.dcp_section_instance_id IN (
   (SELECT
    i.dcp_section_instance_id
    FROM (parser(dcp_inp_name) i)
    WHERE (i.dcp_input_ref_id=rs_1173->from_values.parent_entity_id)))
   DETAIL
    IF (d.active_ind=1)
     prefs_oldver_ind = 0
    ELSE
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   SET dm_err->err_ind = 0
  ENDIF
  IF (prefs_oldver_ind=1)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "OLDVER"
   GO TO exit_1173
  ENDIF
 ENDIF
 IF (((trim(cust_display)="") OR (((cust_display=" ") OR (cd_value=0)) )) )
  SET alg_type = "ALG0"
 ELSE
  SELECT INTO "NL:"
   FROM code_value c
   WHERE c.code_value=cd_value
    AND c.active_ind=1
   DETAIL
    alg_type = c.cdf_meaning
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 CASE (alg_type)
  OF "ALG0":
   SET pvc_value_vc = get_value(cust_tab_name,"PVC_VALUE","TO")
   CALL echo(concat("PVC_VALUE OF '",pvc_value_vc,"' has been translated."))
  OF "ALG1":
   SET merge_id = cnvtreal(get_value(cust_tab_name,"MERGE_ID","FROM"))
   IF (merge_id != 0)
    SET prefs_col = locateval(prefs_loop,1,perm_col_cnt,"MERGE_ID",dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[prefs_loop].column_name)
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[prefs_col].translated=1))
     CALL put_value(cust_tab_name,"PVC_VALUE",cnvtstring(rs_1173->to_values.merge_id))
    ELSE
     SET temp_col_cnt = get_col_pos(cust_tab_name,"MERGE_ID")
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
    ENDIF
   ELSE
    SET pvc_value = cnvtreal(get_value(cust_tab_name,"PVC_VALUE","FROM"))
    IF (pvc_value != 0)
     SET pvc_value_vc = select_merge_translate(cnvtstring(pvc_value),"CODE_VALUE")
     IF (isnumeric(pvc_value_vc))
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ELSE
      SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",pvc_value)
  ...
  OF "ALG2":
   ...
   IF (merge_id != 0)
    SET prefs_col = locateval(prefs_loop,1,perm_col_cnt,"MERGE_ID",dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[prefs_loop].column_name)
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[prefs_col].translated=1))
     CALL put_value(cust_tab_name,"MERGE_ID",cnvtstring(rs_1173->to_values.merge_id))
    ELSE
     SET temp_col_cnt = get_col_pos(cust_tab_name,"MERGE_ID")
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
  ...
  OF "ALG4":
   ...
   ELSE
    SET pvc_value = cnvtreal(get_value(cust_tab_name,"PVC_VALUE","FROM"))
    IF (pvc_value > 0)
     SET pvc_value_vc = select_merge_translate(cnvtstring(pvc_value),"ALT_SEL_CAT")
     IF (isnumeric(pvc_value_vc))
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ELSE
      SET cust_rpt_missing = report_missing("ALT_SEL_CAT","ALT_SEL_CATEGORY_ID",pvc_value)
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     ENDIF
  ...
  OF "ALG5":
   ...
   ELSE
    SET pvc_value = cnvtreal(get_value(cust_tab_name,"PVC_VALUE","FROM"))
    IF (pvc_value > 0)
     SET pvc_value_vc = select_merge_translate(cnvtstring(pvc_value),"NOMEN_CATEGORY")
     IF (isnumeric(pvc_value_vc))
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ELSE
      SET cust_rpt_missing = report_missing("NOMEN_CATEGORY","NOMEN_CATEGORY_ID",pvc_value)
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     ENDIF
  ...
  OF "ALG6":
   ...
   ELSE
    SET pvc_value = cnvtreal(get_value(cust_tab_name,"PVC_VALUE","FROM"))
    IF (pvc_value > 0)
     SET pvc_value_vc = select_merge_translate(cnvtstring(pvc_value),"SCH_REPORT")
     IF (isnumeric(pvc_value_vc))
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ELSE
      SET cust_rpt_missing = report_missing("SCH_REPORT","SCH_REPORT_ID",pvc_value)
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     ENDIF
   ...
   ENDIF
  OF "ALG7":
   SET pvc_value_vc = get_value(cust_tab_name,"PVC_VALUE","FROM")
   SET drdm_semi_ind = findstring(";",pvc_value_vc,1,0)
   IF (drdm_semi_ind > 0)
    CALL delim_string(pvc_value_vc,";")
   ELSE
    CALL delim_string(pvc_value_vc,",")
   ENDIF
   IF (size(delim_rs->qual,5)=0)
    SET stat = alterlist(delim_rs->qual,1)
    SET delim_rs->qual[1].val = pvc_value_vc
   ENDIF
   SET pvc_value_vc = ""
   SET bad_delim_ind = 0
   FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
     IF (cnvtreal(delim_rs->qual[delim_cnt].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,"ALT_SEL_CAT")
      IF (isnumeric(delim_vc))
       IF (pvc_value_vc="")
        SET pvc_value_vc = delim_vc
       ELSE
        IF (drdm_semi_ind > 0)
         SET pvc_value_vc = concat(pvc_value_vc,";",delim_vc)
        ELSE
         SET pvc_value_vc = concat(pvc_value_vc,",",delim_vc)
        ENDIF
       ENDIF
      ELSE
       SET cust_rpt_missing = report_missing("ALT_SEL_CAT","ALT_SEL_CATEGORY_ID",cnvtreal(delim_rs->
         qual[delim_cnt].val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     ...
     ELSE
      IF (pvc_value_vc="")
       SET pvc_value_vc = delim_rs->qual[delim_cnt].val
      ELSE
       IF (drdm_semi_ind > 0)
        SET pvc_value_vc = concat(pvc_value_vc,";",delim_rs->qual[delim_cnt].val)
       ELSE
        SET pvc_value_vc = concat(pvc_value_vc,",",delim_rs->qual[delim_cnt].val)
       ENDIF
      ENDIF
  ...
  OF "ALG8":
   ...
   ELSE
    SET pvc_value = cnvtreal(get_value(cust_tab_name,"PVC_VALUE","FROM"))
    IF (pvc_value > 0)
     SET pvc_value_vc = select_merge_translate(cnvtstring(pvc_value),"TIME_SCALE")
     IF (isnumeric(pvc_value_vc))
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ELSE
      SET cust_rpt_missing = report_missing("TIME_SCALE","TIME_SCALE_ID",pvc_value)
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     ENDIF
  ...
  OF "ALG9":
   ...
   ELSE
    SET pvc_value = cnvtreal(get_value(cust_tab_name,"PVC_VALUE","FROM"))
    IF (pvc_value > 0)
     SET pvc_value_vc = select_merge_translate(cnvtstring(pvc_value),"NOTE_TYPE")
     IF (isnumeric(pvc_value_vc))
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ELSE
      SET cust_rpt_missing = report_missing("NOTE_TYPE","NOTE_TYPE_ID",pvc_value)
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     ENDIF
  ...
  OF "ALG11":
   ...
   CALL delim_string(pvc_value_vc,",")
   SET bad_delim_ind = 0
   IF (size(delim_rs->qual,5) > 3)
    IF (trim(cnvtupper(delim_rs->qual[1].val))="DEF"
     AND trim(cnvtupper(delim_rs->qual[2].val))="C")
     SET delim_vc = select_merge_translate(delim_rs->qual[3].val,"TL_TIME_FRAME")
     IF (isnumeric(delim_vc))
      SET delim_rs->qual[3].val = delim_vc
      SET pvc_value_vc = ""
      FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
        IF (delim_cnt=1)
         SET pvc_value_vc = delim_rs->qual[delim_cnt].val
      ...
      ENDFOR
     ELSE
      SET cust_rpt_missing = report_missing("TL_TIME_FRAME","TL_TIME_FRAME_ID",cnvtreal(delim_rs->
        qual[3].val))
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET bad_delim_ind = 1
      SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
    ...
    ELSE
     SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 1
    ENDIF
   ELSE
    SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 1
  ...
  OF "ALG12":
   ...
   SET bad_delim_ind = 0
   SET pvc_value_vc = ""
   IF (((trim(cnvtupper(delim_rs->qual[1].val))="TRKGROUP") OR (((trim(cnvtupper(delim_rs->qual[1].
     val))="TRKCASETRK") OR (trim(cnvtupper(delim_rs->qual[1].val))="TRKPRVLIST")) )) )
    IF (size(delim_rs->qual,5) >= 12)
     IF (cnvtreal(delim_rs->qual[3].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[3].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[3].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[3].val
         ))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[4].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[4].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[4].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[4].val
         ))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[5].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[5].val,"PREDEFINED_PREFS")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[5].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("PREDEFINED_PREFS","PREDEFINED_PREFS_ID",cnvtreal(
         delim_rs->qual[5].val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[6].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[6].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[6].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[6].val
         ))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[8].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[8].val,"PREDEFINED_PREFS")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[8].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("PREDEFINED_PREFS","PREDEFINED_PREFS_ID",cnvtreal(
         delim_rs->qual[8].val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[9].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[9].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[9].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[9].val
         ))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[11].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[11].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[11].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[11].
         val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[12].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[12].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[12].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[12].
         val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
    ...
    ELSE
     ...
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
    ENDIF
   ELSEIF (((trim(cnvtupper(delim_rs->qual[1].val))="TRKBEDLIST") OR (trim(cnvtupper(delim_rs->qual[1
     ].val))="LOCATION")) )
    IF (size(delim_rs->qual,5) >= 16)
     SET delim_list = delim_rs->qual[2].val
     IF (cnvtreal(delim_rs->qual[3].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[3].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[3].val = delim_vc
     ...
     ENDIF
     IF (cnvtreal(delim_rs->qual[5].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[5].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[5].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[5].val
         ))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     ...
     IF (cnvtreal(delim_rs->qual[6].val) > 0)
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[7].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[7].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[7].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[7].val
         ))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     ...
     ENDIF
     IF (cnvtreal(delim_rs->qual[8].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[8].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[8].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[8].val
         ))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     ...
     ENDIF
     IF (cnvtreal(delim_rs->qual[9].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[9].val,"PREDEFINED_PREFS")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[9].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("PREDEFINED_PREFS","PREDEFINED_PREFS_ID",cnvtreal(
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[10].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[10].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[10].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[10].
         val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     ...
     ENDIF
     IF (cnvtreal(delim_rs->qual[12].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[12].val,"PREDEFINED_PREFS")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[12].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("PREDEFINED_PREFS","PREDEFINED_PREFS_ID",cnvtreal(
         delim_rs->qual[12].val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
       SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[13].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[13].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[13].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[13].
         val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[15].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[15].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[15].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[15].
         val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      ...
      ENDIF
     ENDIF
     IF (cnvtreal(delim_rs->qual[16].val) > 0)
      SET delim_vc = select_merge_translate(delim_rs->qual[16].val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET delim_rs->qual[16].val = delim_vc
      ELSE
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[16].
         val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET bad_delim_ind = 1
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
   ...
   IF (bad_delim_ind=0)
    FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
      ...
      ENDIF
    ENDFOR
    IF (trim(cnvtupper(delim_rs->qual[1].val))="LOCATION")
     SET stat = alterlist(delim_rs->qual,0)
     CALL delim_string(delim_list,",")
     IF (cnvtint(delim_rs->qual[4].val) > 0)
      FOR (delim_cnt = 5 TO (4+ cnvtint(delim_rs->qual[4].val)))
        IF (cnvtreal(delim_rs->qual[delim_cnt].val) > 0)
         SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,"CODE_VALUE")
         IF (isnumeric(delim_vc))
          SET delim_rs->qual[delim_cnt].val = delim_vc
         ELSE
          SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[
            delim_cnt].val))
          CALL evaluate_rpt_missing(cust_rpt_missing)
          SET bad_delim_ind = 1
          SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     ...
     ENDIF
     IF (bad_delim_ind=0)
      SET pre_delim_string = ""
      FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
        IF (delim_cnt=1)
         SET pre_delim_string = delim_rs->qual[delim_cnt].val
        ELSE
         SET pre_delim_string = concat(pre_delim_string,",",delim_rs->qual[delim_cnt].val)
        ENDIF
      ENDFOR
      SET pvc_value_vc = replace(pvc_value_vc,delim_list,pre_delim_string,0)
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ENDIF
    ELSE
     CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
  ...
  OF "ALG13":
   SET pvc_value = cnvtreal(get_value(cust_tab_name,"PVC_VALUE","FROM"))
   IF (pvc_value > 0)
    SET pvc_value_vc = select_merge_translate(cnvtstring(pvc_value),"PRSNL")
    IF (isnumeric(pvc_value_vc))
     CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
    ELSE
     SET cust_rpt_missing = report_missing("PRSNL","PERSON_ID",pvc_value)
     CALL evaluate_rpt_missing(cust_rpt_missing)
     SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
    ENDIF
  ...
  OF "ALG14":
   SET pvc_value_vc = get_value(cust_tab_name,"PVC_VALUE","FROM")
   SET fndstring_val = findstring("^",pvc_value_vc,1,0)
   SET delim_list = substring(1,(fndstring_val - 1),pvc_value_vc)
   SET delim_vc = select_merge_translate(delim_list,"CODE_VALUE")
   IF ( NOT (isnumeric(delim_vc)))
    SET bad_delim_ind = 1
    SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_list))
    CALL evaluate_rpt_missing(cust_rpt_missing)
    SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
   ELSE
    SELECT INTO "NL:"
     FROM (parser(nvp_cv_tab_name) c)
     WHERE c.code_value=cnvtreal(delim_vc)
     DETAIL
      nvp_cdf_mean = c.cdf_meaning
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (bad_delim_ind=0)
    IF (nvp_cdf_mean IN ("TEXT", "DATERANGE", "DATECALC", "DATE", "ENCNTR",
    "PERSON"))
     SET pvc_value_vc = replace(pvc_value_vc,delim_list,delim_vc,0)
     CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
    ELSEIF (nvp_cdf_mean="CODESET")
     SET pvc_value_vc = replace(pvc_value_vc,delim_list,delim_vc,0)
     SET delim_cnt = 0
     SET delim_cnt = findstring(";",pvc_value_vc,1,0)
     SET compare_cnt = findstring(",",pvc_value_vc,1,1)
     IF (delim_cnt != 0
      AND compare_cnt != 0)
      SET pre_delim_string = substring(1,delim_cnt,pvc_value_vc)
      SET post_delim_string = substring(compare_cnt,((size(pvc_value_vc) - compare_cnt)+ 1),
       pvc_value_vc)
      SET delim_list = substring((delim_cnt+ 1),((compare_cnt - delim_cnt) - 1),pvc_value_vc)
      IF (size(delim_list) > 0)
       CALL delim_string(delim_list,";")
       SET bad_delim_ind = 0
       SET compare_cnt = cnvtint(substring(2,(findstring("^",post_delim_string,1,0) - 2),
         post_delim_string))
       IF (compare_cnt=1)
        SET stat = alterlist(delim_rs->qual,1)
        SET delim_rs->qual[1].val = delim_list
       ENDIF
       IF (size(delim_rs->qual,5)=compare_cnt)
        FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
          IF (cnvtreal(delim_rs->qual[delim_cnt].val) > 0)
           SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,"CODE_VALUE")
           IF (isnumeric(delim_vc))
        ...
        IF (bad_delim_ind=0)
         FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
           IF (delim_cnt > 1)
            SET new_delim_list = concat(new_delim_list,";",delim_rs->qual[delim_cnt].val)
           ELSE
            SET new_delim_list = delim_rs->qual[delim_cnt].val
           ENDIF
         ENDFOR
         SET pvc_value_vc = concat(pre_delim_string,new_delim_list,post_delim_string)
         CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
        ELSE
         CALL echo("A value in a delimited list could not be translated.")
         SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
         SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
        ENDIF
       ELSE
        CALL echo("A character being searched for could not be found.")
        SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
        SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
        SET dm2_ref_data_reply->error_ind = 1
        SET dm2_ref_data_reply->error_msg = "NOMV12"
        CALL merge_audit("FAILREASON","A character being searched for could not be found.",2)
       ENDIF
      ELSE
       CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
      ENDIF
     ELSE
      CALL echo("There was an unexpected number returned from a delimited list.")
      SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
      SET dm2_ref_data_reply->error_ind = 1
      SET dm2_ref_data_reply->error_msg = "NOMV12"
      CALL merge_audit("FAILREASON","There was an unexpected number returned from a delimited list.",
       2)
     ENDIF
    ELSEIF (nvp_cdf_mean="TRKREF")
     SET pvc_value_vc = replace(pvc_value_vc,delim_list,delim_vc,0)
     SET delim_cnt = 0
     SET delim_cnt = findstring(";",pvc_value_vc,1,0)
     SET compare_cnt = findstring(",",pvc_value_vc,1,1)
     IF (delim_cnt != 0
      ...
      SET post_delim_string = substring(compare_cnt,((size(pvc_value_vc) - compare_cnt)+ 1),
       pvc_value_vc)
      SET delim_list = substring((delim_cnt+ 1),((compare_cnt - delim_cnt) - 1),pvc_value_vc)
      SET substr_val = substring((findstring("^",pre_delim_string,1,1)+ 1),((size(pre_delim_string)
        - findstring("^",pre_delim_string,1,1)) - 1),pre_delim_string)
      SET delim_vc = select_merge_translate(substr_val,"CODE_VALUE")
      IF (isnumeric(delim_vc))
       SET pre_delim_string = replace(pre_delim_string,substr_val,delim_vc,0)
      ELSE
       SET bad_delim_ind = 1
       SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(substr_val))
       CALL evaluate_rpt_missing(cust_rpt_missing)
       SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
       SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
      ENDIF
      IF (bad_delim_ind=0
       AND size(delim_list) > 0)
       CALL delim_string(delim_list,";")
       SET bad_delim_ind = 0
       SET compare_cnt = cnvtint(substring(2,(findstring("^",post_delim_string,1,0) - 2),
         post_delim_string))
       ...
       IF (size(delim_rs->qual,5)=compare_cnt)
        FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
          IF (cnvtreal(delim_rs->qual[delim_cnt].val) > 0)
           SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,"TRACK_REFERENCE")
           IF (isnumeric(delim_vc))
            SET delim_rs->qual[delim_cnt].val = delim_vc
           ELSE
            SET cust_rpt_missing = report_missing("TRACK_REFERENCE","TRACKING_REF_ID",cnvtreal(
              delim_rs->qual[delim_cnt].val))
            CALL evaluate_rpt_missing(cust_rpt_missing)
            SET bad_delim_ind = 1
            SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
            SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     ...
     ELSE
      ...
      CALL merge_audit("FAILREASON","A character being searched for could not be found.",2)
     ENDIF
    ELSEIF (nvp_cdf_mean IN ("TRKEVENT", "LOC", "PROVIDER"))
     IF (nvp_cdf_mean="TRKEVENT")
      SET cust_pe_name = "TRACK_EVENT"
     ELSEIF (nvp_cdf_mean="LOC")
      SET cust_pe_name = "CODE_VALUE"
     ELSE
      SET cust_pe_name = "PRSNL"
     ENDIF
     SET pvc_value_vc = replace(pvc_value_vc,delim_list,delim_vc,0)
     SET delim_cnt = 0
     SET delim_cnt = findstring("^",pvc_value_vc,1,1)
     SET pre_delim_string = substring(1,(delim_cnt - 1),pvc_value_vc)
     SET compare_cnt = findstring(",",pre_delim_string,1,1)
     SET delim_cnt = findstring("^",pre_delim_string,1,1)
     IF (delim_cnt != 0
      AND compare_cnt != 0
      AND delim_cnt < compare_cnt)
      SET post_delim_string = substring(compare_cnt,((size(pvc_value_vc) - compare_cnt)+ 1),
       pvc_value_vc)
      SET pre_delim_string = substring(1,delim_cnt,pvc_value_vc)
      SET delim_list = substring((delim_cnt+ 1),((compare_cnt - delim_cnt) - 1),pvc_value_vc)
      ...
      IF (size(delim_rs->qual,5)=compare_cnt)
       FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
         IF (cnvtreal(delim_rs->qual[delim_cnt].val) > 0)
          SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,cust_pe_name)
          IF (isnumeric(delim_vc))
           SET delim_rs->qual[delim_cnt].val = delim_vc
          ELSE
           IF (cust_pe_name="CODE_VALUE")
            SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[
              delim_cnt].val))
           ELSEIF (cust_pe_name="TRACK_EVENT")
            SET cust_rpt_missing = report_missing("TRACK_EVENT","TRACK_EVENT_ID",cnvtreal(delim_rs->
              qual[delim_cnt].val))
           ELSE
            SET cust_rpt_missing = report_missing("PRSNL","PERSON_ID",cnvtreal(delim_rs->qual[
              delim_cnt].val))
           ENDIF
           CALL evaluate_rpt_missing(cust_rpt_missing)
           SET bad_delim_ind = 1
     ...
     ENDIF
    ELSE
     CALL echo("There was an unexpected cdf_meaning returned from the delimted list.")
     SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV12"
     CALL merge_audit("FAILREASON",
      "There was an unexpected cdf_meaning returned from the delimted list.",2)
    ENDIF
   ENDIF
  OF "ALG15":
   SET pvc_value_vc = get_value(cust_tab_name,"PVC_VALUE","FROM")
   ...
   IF (fndstring_val > 0)
    SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
    SET compare_cnt = cnvtint(substring(1,(fndstring_val - 1),pvc_value_vc))
    SET delim_list = substring((fndstring_val+ 1),(size(pvc_value_vc) - fndstring_val),pvc_value_vc)
    IF (delim_list > "")
     SET pvc_value_vc = ""
     CALL delim_string(delim_list,",")
     SET bad_delim_ind = 0
     ...
     IF (size(delim_rs->qual,5)=compare_cnt)
      ...
      ENDFOR
      IF (bad_delim_ind=0)
       SET pvc_value_vc = concat(trim(cnvtstring(compare_cnt)),";")
       FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
         IF (delim_cnt > 1)
          SET pvc_value_vc = concat(pvc_value_vc,",",delim_rs->qual[delim_cnt].val)
         ELSE
  ...
  OF "ALG16":
   ...
   IF (pvc_value_vc="")
    CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
   ELSE
    IF (cust_display="ColumnViewInfo")
     CALL delim_string(delim_list,"^")
     SET bad_delim_ind = 0
     FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
       IF (delim_cnt IN (3, 5, 7))
        SET compare_cnt = 0
        SET compare_cnt = findstring(";",delim_rs->qual[delim_cnt].val,1,0)
        IF (compare_cnt=0)
         SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
         SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
        ELSE
         SET nvp_cdf_mean = substring(1,compare_cnt,delim_rs->qual[delim_cnt].val)
         SET post_delim_string = substring((compare_cnt+ 1),(size(delim_rs->qual[delim_cnt].val) -
          compare_cnt),delim_rs->qual[delim_cnt].val)
         IF (nvp_cdf_mean IN ("TEETICONCOL", "TEETTEXTCOL", "TEETSTATCOL", "TEEVTTYPE", "TEALLIASCOL",
         "TERESULTSCNT", "TEORDINDCNT", "TEORDINFO", "TERESULTSVAL"))
          SET delim_vc = select_merge_translate(post_delim_string,"CODE_VALUE")
         ELSEIF (nvp_cdf_mean="TEEVENTCOL")
          SET delim_vc = select_merge_translate(post_delim_string,"TRACK_EVENT")
         ELSEIF (nvp_cdf_mean="TEPRVROLE")
          SET delim_vc = select_merge_translate(post_delim_string,"TRACK_REFERENCE")
         ELSEIF (nvp_cdf_mean IN ("TESURCREDT", "TESURCREDTT", "TESURCREICO", "TESURCRENM",
         "TESURCRESET",
         "TESURCRESTT", "TESURCRETM", "TESURCRETME"))
          SET delim_vc = select_merge_translate(post_delim_string,"TRACK_COLLECTION")
         ELSEIF (nvp_cdf_mean="NONE")
          SET delim_vc = "0"
         ELSE
          SET delim_vc = select_merge_translate(post_delim_string,"CODE_VALUE")
         ENDIF
         IF ( NOT (isnumeric(delim_vc)))
          SET bad_delim_ind = 1
          IF (nvp_cdf_mean IN ("TEETICONCOL", "TEETTEXTCOL", "TEETSTATCOL", "TEEVTTYPE",
          "TEALLIASCOL",
          "TERESULTSCNT", "TEORDINDCNT", "TEORDINFO", "TERESULTSVAL"))
           SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(post_delim_string
             ))
          ELSEIF (nvp_cdf_mean="TEEVENTCOL")
           SET cust_rpt_missing = report_missing("TRACK_EVENT","TRACK_EVENT_ID",cnvtreal(
             post_delim_string))
          ELSEIF (nvp_cdf_mean="TEPRVROLE")
           SET cust_rpt_missing = report_missing("TRACK_REFERENCE","TRACKING_REF_ID",cnvtreal(
             post_delim_string))
          ...
          "TESURCRESET",
          "TESURCRESTT", "TESURCRETM", "TESURCRETME"))
           SET cust_rpt_missing = report_missing("TRACK_COLLECTION","TRACK_COLLECTION_ID",cnvtreal(
             post_delim_string))
          ELSE
           SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(post_delim_string
             ))
          ...
          SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
         ELSE
          SET delim_rs->qual[delim_cnt].val = concat(nvp_cdf_mean,";",delim_vc)
         ENDIF
        ENDIF
       ENDIF
     ENDFOR
     IF (bad_delim_ind=0)
      ...
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ENDIF
    ELSEIF (cust_display="GroupRowsBy")
     SET delim_cnt = findstring("~",pvc_value_vc,1,0)
     SET nvp_cdf_mean = substring(1,(delim_cnt - 1),pvc_value_vc)
     SET post_delim_string = substring((delim_cnt+ 1),(size(pvc_value_vc) - delim_cnt),pvc_value_vc)
     IF (nvp_cdf_mean IN ("TEETICONCOL", "TEETTEXTCOL", "TEETSTATCOL", "TEEVTTYPE", "TEALLIASCOL",
     "TERESULTSCNT", "TEORDINDCNT", "TEORDINFO", "TERESULTSVAL"))
      SET delim_vc = select_merge_translate(post_delim_string,"CODE_VALUE")
     ELSEIF (nvp_cdf_mean="TEEVENTCOL")
     ...
     ELSEIF (nvp_cdf_mean="TEPRVROLE")
      SET delim_vc = select_merge_translate(post_delim_string,"TRACK_REFERENCE")
     ELSEIF (nvp_cdf_mean IN ("TESURCREDT", "TESURCREDTT", "TESURCREICO", "TESURCRENM", "TESURCRESET",
     "TESURCRESTT", "TESURCRETM", "TESURCRETME"))
      SET delim_vc = select_merge_translate(post_delim_string,"TRACK_COLLECTION")
     ELSEIF (nvp_cdf_mean="NONE")
      SET delim_vc = "0"
     ...
     ENDIF
     IF (isnumeric(delim_vc))
      SET pvc_value_vc = concat(nvp_cdf_mean,"~",delim_vc)
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ELSE
      IF (nvp_cdf_mean IN ("TEETICONCOL", "TEETTEXTCOL", "TEETSTATCOL", "TEEVTTYPE", "TEALLIASCOL",
      "TERESULTSCNT", "TEORDINDCNT", "TEORDINFO", "TERESULTSVAL"))
      ...
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     ENDIF
    ELSEIF (cust_display="GrouperRowInfo")
     SET bad_delim_ind = 0
     CALL delim_string(delim_list,";")
     FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
       SET compare_cnt = 0
     ...
     IF (bad_delim_ind=0)
      ...
      ENDFOR
      IF (size(delim_rs->qual,5) > 0)
       SET pvc_value_vc = concat(pvc_value_vc,";")
      ENDIF
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ENDIF
    ELSE
     CALL echo("Unexpected PVC_NAME for ALG16 Preference")
     SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg = "NOMV12"
     CALL merge_audit("FAILREASON","Unexpected PVC_NAME for ALG16 Preference",2)
    ENDIF
   ENDIF
  OF "ALG17":
   SET pvc_value_vc = get_value(cust_tab_name,"PVC_VALUE","FROM")
   SET compare_cnt = findstring("^",pvc_value_vc,1,0)
   IF (compare_cnt > 0)
    SET pre_delim_string = substring(1,(compare_cnt - 1),pvc_value_vc)
    IF (pre_delim_string="TEMERGECOL")
     SET bad_delim_ind = 0
     SET compare_cnt = findstring("~",pvc_value_vc,1,0)
     IF (compare_cnt > 0)
      SET pre_delim_string = substring(1,compare_cnt,pvc_value_vc)
      SET delim_list = substring((compare_cnt+ 1),(size(pvc_value_vc) - compare_cnt),pvc_value_vc)
      CALL delim_string(delim_list,"~")
      IF (size(delim_rs->qual,5)=0)
       SET stat = alterlist(delim_rs->qual,1)
       SET delim_rs->qual[1].val = delim_list
      ...
      FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
       ...
       IF (compare_cnt > 0)
        SET post_delim_string = substring((compare_cnt+ 1),(size(delim_rs->qual[delim_cnt].val) -
         compare_cnt),delim_rs->qual[delim_cnt].val)
        IF ((delim_rs->qual[delim_cnt].val=patstring("TEEVENTCOL*")))
         SET delim_vc = select_merge_translate(post_delim_string,"TRACK_EVENT")
         IF (isnumeric(delim_vc))
          SET delim_rs->qual[delim_cnt].val = replace(delim_rs->qual[delim_cnt].val,post_delim_string,
           delim_vc,0)
         ELSE
          SET cust_rpt_missing = report_missing("TRACK_EVENT","TRACK_EVENT_ID",cnvtreal(
            post_delim_string))
          ...
          SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
         ENDIF
        ELSEIF ((delim_rs->qual[delim_cnt].val=patstring("TEPRVROLE*")))
         SET delim_vc = select_merge_translate(post_delim_string,"TRACK_REFERENCE")
         IF (isnumeric(delim_vc))
          SET delim_rs->qual[delim_cnt].val = replace(delim_rs->qual[delim_cnt].val,post_delim_string,
           delim_vc,0)
         ELSE
          ...
          SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
         ENDIF
        ELSEIF ((delim_rs->qual[delim_cnt].val=patstring("TEALIAS*")))
         SET delim_vc = select_merge_translate(post_delim_string,"CODE_VALUE")
         IF (isnumeric(delim_vc))
          SET delim_rs->qual[delim_cnt].val = replace(delim_rs->qual[delim_cnt].val,post_delim_string,
           delim_vc,0)
      ...
      IF (bad_delim_ind=0)
       FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
         ...
         ENDIF
       ENDFOR
       CALL put_value(cust_tab_name,"PVC_VALUE",pre_delim_string)
      ENDIF
     ELSE
      CALL put_value(cust_tab_name,"PVC_VALUE",pvc_value_vc)
     ENDIF
    ELSEIF (pre_delim_string IN ("TEETICONCOL", "TEETTEXTCOL", "TEETSTATCOL"))
     SET cust_pe_id = cnvtreal(get_value(cust_tab_name,"PARENT_ENTITY_ID","FROM"))
     SET compare_cnt = findstring("^",pvc_value_vc,1,1)
     SET delim_list = substring((compare_cnt+ 1),(size(pvc_value_vc) - compare_cnt),pvc_value_vc)
     SET pre_delim_string = substring(1,compare_cnt,pvc_value_vc)
     CALL delim_string(delim_list,";")
     ...
     SELECT INTO "nl:"
      ...
      WHERE pp.predefined_prefs_id=cust_pe_id
       AND pp.predefined_type_meaning="SNCASETYPE"
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     ELSE
      FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
       SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,"TRACK_COLLECTION")
       IF ( NOT (isnumeric(delim_vc)))
        SET cust_rpt_missing = report_missing("TRACK_COLLECTION","TRACK_COLLECTION_ID",cnvtreal(
          delim_rs->qual[delim_cnt].val))
        CALL evaluate_rpt_missing(cust_rpt_missing)
    ...
    ELSE
     SET bad_delim_ind = 0
     CALL delim_string(pvc_value_vc,"^")
     FOR (delim_cnt = 1 TO 5)
       IF (delim_cnt=1)
        SET delim_list = delim_rs->qual[delim_cnt].val
       ELSE
        SET delim_list = concat(delim_list,"^",delim_rs->qual[delim_cnt].val)
       ENDIF
     ENDFOR
     SET delim_cnt = size(delim_rs->qual,5)
     IF (delim_cnt > 5)
      SET delim_list = concat(delim_list,"^")
      SET post_delim_string = delim_rs->qual[delim_cnt].val
      SET nvp_cdf_mean = delim_rs->qual[6].val
      IF ((delim_rs->qual[1].val="TEEVENTCOL"))
       CALL delim_string(post_delim_string,";")
       IF (size(delim_rs->qual,5)=0)
        SET stat = alterlist(delim_rs->qual,1)
        SET delim_rs->qual[1].val = nvp_cdf_mean
       ENDIF
       FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
        SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,"TRACK_EVENT")
        IF (isnumeric(delim_vc))
         SET delim_rs->qual[delim_cnt].val = delim_vc
        ELSE
         SET cust_rpt_missing = report_missing("TRACK_EVENT","TRACK_EVENT_ID",cnvtreal(delim_rs->
        ...
        ENDIF
       ENDFOR
      ELSEIF ((delim_rs->qual[1].val="TEPRVROLE"))
       CALL delim_string(post_delim_string,";")
       IF (size(delim_rs->qual,5)=0)
        SET stat = alterlist(delim_rs->qual,1)
        SET delim_rs->qual[1].val = nvp_cdf_mean
       ...
       FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
        ...
        IF (isnumeric(delim_vc))
         SET delim_rs->qual[delim_cnt].val = delim_vc
        ELSE
         SET cust_rpt_missing = report_missing("TRACK_REFERENCE","TRACKING_REF_ID",cnvtreal(delim_rs
           ->qual[delim_cnt].val))
         CALL evaluate_rpt_missing(cust_rpt_missing)
         SET bad_delim_ind = 1
         SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
        ...
        ENDIF
       ENDFOR
      ELSEIF ((delim_rs->qual[1].val IN ("TERESULTSCNT", "TERESULTSVAL")))
       CALL delim_string(post_delim_string,";")
       IF (size(delim_rs->qual,5)=0)
        SET stat = alterlist(delim_rs->qual,1)
        SET delim_rs->qual[1].val = nvp_cdf_mean
       ...
       FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
         SET compare_cnt = findstring("~",delim_rs->qual[delim_cnt].val,1,0)
         SET pre_delim_string = substring((compare_cnt+ 1),(size(delim_rs->qual[delim_cnt].val) -
          compare_cnt),delim_rs->qual[delim_cnt].val)
         SET delim_vc = select_merge_translate(pre_delim_string,"CODE_VALUE")
         IF (isnumeric(delim_vc))
          SET delim_rs->qual[delim_cnt].val = replace(delim_rs->qual[delim_cnt].val,pre_delim_string,
           delim_vc,0)
         ENDIF
       ENDFOR
      ELSE
      ...
      IF (bad_delim_ind=0)
       FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
         ...
         ENDIF
       ENDFOR
       CALL put_value(cust_tab_name,"PVC_VALUE",delim_list)
      ENDIF
     ELSE
      CALL put_value(cust_tab_name,"PVC_VALUE",delim_list)
     ENDIF
  ...
  OF "ALG18":
   ...
   IF (compare_cnt > 0)
    ...
    ENDIF
    SET delim_list = ""
    IF (pre_delim_string IN ("TEACCOMPBY", "TEADMNTFY", "TEADMSRC", "TEALLERGYICO", "TEAPPTTYPE",
    "TEAPPTSTATS", "TEARRMODE", "TEATTNTFY", "TEBADGESTATE", "TEBEDSTATUS",
    "TEBEDSTICON", "TECODINGICO", "TEDISCHDISP", "TEDOCSTATICO", "TEENCNTRTYPE",
    "TEFAMPRESNT", "TEFINCLASS", "TEFINCLICON", "TETRKREASON", "TEISOLATION",
    "TEMEDSRV", "TEPCPNTFY", "TEREFNTFY", "TESECCONFLVL", "TESEX",
    "PEDEFRELTN", "PEPRVTYPE", "TEBED", "TEBASEBED", "PELOCATION",
    "TEALLERGYIND", "TECODINGIND", "TEDOCSTAT", "TETRKCODE"))
     FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
      IF (isnumeric(delim_rs->qual[delim_cnt].val) IN (1, 2))
       SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,"CODE_VALUE")
       IF (isnumeric(delim_vc))
        SET delim_rs->qual[delim_cnt].val = delim_vc
       ELSE
      ...
      ENDIF
     ENDFOR
    ELSEIF (pre_delim_string IN ("TEACUITY", "TEACUITYICON", "TEBEDTEAM", "TEREGSTATUS",
    "TEREGSTICON",
    "TESPECIALTY", "TESPECICON", "PEPRVROLE", "PEPRVTASK", "TETEAMICON",
    "TETEAM", "TEPRVROLE"))
     FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
      IF (isnumeric(delim_rs->qual[delim_cnt].val) IN (1, 2))
       IF (delim_cnt=1)
      ...
      ENDIF
     ENDFOR
    ELSEIF (pre_delim_string IN ("TEETEXTCOL", "TEEVENT"))
     FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
      IF (isnumeric(delim_rs->qual[delim_cnt].val) IN (1, 2))
       IF (delim_cnt=1)
        SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,"CODE_VALUE")
      ...
      ENDIF
     ENDFOR
    ELSEIF (pre_delim_string IN ("TEEVENTCOL"))
     FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
      IF (isnumeric(delim_rs->qual[delim_cnt].val) IN (1, 2))
       IF (delim_cnt=1)
        SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,"TRACK_EVENT")
     ...
     ENDFOR
    ELSE
     SET delim_vc = select_merge_translate(delim_rs->qual[1].val,"CODE_VALUE")
     IF (isnumeric(delim_vc))
      SET delim_rs->qual[1].val = delim_vc
     ELSE
      SET cust_rpt_missing = report_missing("CODE_VALUE","CODE_VALUE",cnvtreal(delim_rs->qual[1].val)
       )
      CALL evaluate_rpt_missing(cust_rpt_missing)
      SET bad_delim_ind = 1
      SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
    ...
    ENDIF
    IF (bad_delim_ind=0)
     SET pre_delim_string = concat(pre_delim_string,"^",delim_list)
     CALL put_value(cust_tab_name,"PVC_VALUE",pre_delim_string)
    ENDIF
   ELSE
    SET bad_delim_ind = 0
    ...
    "TETEAM", "TEPRVROLE"))
     FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
      ...
      ENDIF
     ENDFOR
    ELSEIF (pre_delim_string IN ("TEETEXTCOL", "TEEVENTCOL", "TEEVENT"))
     FOR (delim_cnt = 1 TO size(delim_rs->qual,5))
      IF (isnumeric(delim_rs->qual[delim_cnt].val) IN (1, 2))
       SET delim_vc = select_merge_translate(delim_rs->qual[delim_cnt].val,"TRACK_EVENT")
       IF (isnumeric(delim_vc))
        SET delim_rs->qual[delim_cnt].val = delim_vc
       ELSE
        SET cust_rpt_missing = report_missing("TRACK_EVENT","TRACK_EVENT_ID",cnvtreal(delim_rs->qual[
          delim_cnt].val))
        CALL evaluate_rpt_missing(cust_rpt_missing)
        SET bad_delim_ind = 1
        SET temp_col_cnt = get_col_pos(cust_tab_name,"PVC_VALUE")
  ...
  ELSE
   ...
   GO TO exit_1173
 ENDCASE
 SET temp_col_cnt = get_col_pos(cust_tab_name,"NAME_VALUE_PREFS_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (no_query=1)
   ...
   IF (val < 0)
    IF ((val=- (3)))
     SET val = get_seq(cust_tab_name,"NAME_VALUE_PREFS_ID")
     IF (val > 0)
      CALL put_value(cust_tab_name,"NAME_VALUE_PREFS_ID",cnvtstring(val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  NAME_VALUE_PREFS_ID")
     ELSE
      GO TO exit_1173
     ENDIF
    ELSE
   ...
   ELSE
    ...
    CALL put_value(cust_tab_name,"NAME_VALUE_PREFS_ID",cnvtstring(val))
   ENDIF
   SET from_id = cnvtreal(get_value(cust_tab_name,"NAME_VALUE_PREFS_ID","FROM"))
   ...
   ELSE
    ...
    SET dm2_ref_data_reply->qual[1].from_value = from_id
   ENDIF
   SET new_id = cnvtreal(get_value(cust_tab_name,"NAME_VALUE_PREFS_ID","TO"))
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_1173
   ELSE
    SET dm2_ref_data_reply->qual[1].to_value = new_id
   ENDIF
  ...
  ENDIF
 ELSE
  SET stat = alterlist(cust_ui_query->qual,1)
  SET cust_ui_query->qual[1].val = cnvtreal(get_value(cust_tab_name,"NAME_VALUE_PREFS_ID","TO"))
  SET translated_prior = 1
 ENDIF
 SET idcd_check = 0
 CALL echo("")
 CALL echo("")
 ...
 IF (drdm_error_out_ind=1)
  GO TO exit_1173
 ENDIF
 SET pvc_value_check = is_translated(cust_tab_name,"PVC_VALUE")
 IF (pvc_value_check=0)
  SET cust_err_val = get_err_val(null)
  IF (cust_err_val=0)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg =
   "The PVC_VALUE column on the NAME_VALUE_PREFS table is not translated"
  ENDIF
  SET drdm_no_trans_ind = 1
  SET idcd_check = 0
 ENDIF
 ...
 ENDIF
 SUBROUTINE get_dyn_name(sbr_pvc_name)
   DECLARE sbr_return = vc
   DECLARE sbr_size = i4
   DECLARE sbr_dyn_loop = i4
   DECLARE proc_num = i2
   DECLARE size_num_loop = i4
   DECLARE dec_cnt = i4
   SET sbr_return = ""
   SET proc_num = 0
   SET last_num = 0
   SET sbr_size = size(sbr_pvc_name)
   SET dec_cnt = sbr_size
   IF (isnumeric(substring(sbr_size,1,sbr_pvc_name))=1)
    WHILE (dec_cnt != 1)
     IF (isnumeric(substring(dec_cnt,1,sbr_pvc_name))=0)
      IF (proc_num=1)
       FOR (size_num_loop = (dec_cnt+ 1) TO sbr_size)
         IF (isnumeric(substring(size_num_loop,1,sbr_pvc_name))=0)
          SET sbr_return = concat(substring(1,dec_cnt,sbr_pvc_name),"#",substring(size_num_loop,
            sbr_size,sbr_return))
          SET size_num_loop = sbr_size
         ELSEIF (size_num_loop=sbr_size)
          SET sbr_return = concat(substring(1,dec_cnt,sbr_pvc_name),"#")
         ENDIF
       ENDFOR
       SET proc_num = 0
      ENDIF
   ...
   ENDIF
   FREE SET sbr_size
   FREE SET sbr_dyn_loop
   FREE SET proc_num
   FREE SET size_num_loop
   FREE SET dec_cnt
   RETURN(sbr_return)
 END ;Subroutine
 SUBROUTINE find_parent(cur_position,parent_name,parent_value)
   DECLARE return_pos = i4
   DECLARE fp_column = vc
   DECLARE fp_done_ind = i4
   DECLARE fp_dcle_name = vc
   SET return_pos = drdm_chg->log[cur_position].next_to_process
   IF (parent_name IN ("DETAIL_PREFS", "VIEW_PREFS", "VIEW_COMP_PREFS"))
    WHILE ((((drdm_chg->log[return_pos].table_name != parent_name)) OR ( NOT ((drdm_chg->log[
    return_pos].process_flg IN (4, 5))))) )
      SET return_pos = drdm_chg->log[return_pos].next_to_process
    ENDWHILE
   ELSE
    WHILE ((drdm_chg->log[cur_position].next_to_process > cur_position))
      SET cur_position = drdm_chg->log[cur_position].next_to_process
    ENDWHILE
    SET return_pos = drdm_chg->log[cur_position].next_to_process
   ENDIF
   SET fp_dcle_name = dm2_get_rdds_tname("DM_CHG_LOG_EXCEPTION")
   SET fp_column = concat(parent_name,"_ID")
   UPDATE  FROM (parser(fp_dcle_name) d)
    SET d.log_type = dm2_ref_data_reply->error_msg
    WHERE d.table_name=parent_name
     AND d.column_name=fp_column
     AND d.from_value=parent_value
   ...
   ENDIF
   IF (curqual=0)
    INSERT  FROM (parser(fp_dcle_name) d)
     SET d.log_type = dm2_ref_data_reply->error_msg, d.table_name = parent_name, d.column_name =
      fp_column,
      d.from_value = parent_value, d.target_env_id = dm2_ref_data_doc->env_target_id
     WITH nocounter
    ;end insert
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
...
#exit_1173
 ...
 FREE SET merge_id
 FREE SET merge_name
 FREE SET merge_id_vc
 FREE SET cust_tab_name
 FREE SET pvc_value
 FREE SET pvc_value_vc
 FREE SET delim_cnt
 FREE SET delim_vc
 FREE SET bad_delim_ind
 FREE SET pvc_value_check
 FREE SET no_query
 FREE SET val
 FREE SET from_id
 FREE SET new_id
 ...
 FREE SET ins_upd
 FREE SET cd_value
 FREE SET cust_display
 FREE SET cust_query_return
 FREE SET cust_col_name
 FREE SET cust_from_cnt
 FREE SET cust_table_name
 FREE SET cust_query_cnt
 FREE SET cust_loop_cnt
 FREE SET cust_insert_loop
END GO
