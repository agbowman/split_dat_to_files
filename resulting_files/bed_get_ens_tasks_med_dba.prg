CREATE PROGRAM bed_get_ens_tasks_med:dba
 FREE SET reply
 RECORD reply(
   1 status_data
     2 status_list[*]
 ...
 SET stat = alterlist(pvrequest->pvlist,5)
 SET stat = alterlist(pvreply->pvlist,5)
 ...
 ENDIF
 IF ((request->action="0"))
  SET easysc_auth_exists = 0
  CALL complete_easysc_vp(dummy_parm1)
  ...
  IF ((vpreply->vplist[1].view_prefs_id > 0))
   CALL complete_easysc_vp_nvp(dummy_parm1)
   ...
   IF ((nvpreply->nvplist[1].name_value_prefs_id > 0)
    ...
    CALL complete_easysc_vcp(dummy_parm1)
    ...
    IF ((vcpreply->vcplist[1].view_comp_prefs_id > 0))
     CALL complete_easysc_vcp_nvp(dummy_parm1)
     ...
     IF ((nvpreply->nvplist[1].name_value_prefs_id > 0)
      ...
      SET easysc_auth_exists = 1
     ENDIF
    ENDIF
   ENDIF
  ENDIF
  SET medpro_auth_exists = 0
  CALL complete_medpro_vp(dummy_parm1)
  SET vprequest->vplist[1].action_flag = "0"
  SET trace = recpersist
  EXECUTE bed_get_ens_view_prefs  WITH replace("REQUEST",vprequest), replace("REPLY",vpreply)
  IF ((vpreply->vplist[1].view_prefs_id > 0))
   CALL complete_medpro_vp_nvp(dummy_parm1)
   SET nvprequest->nvplist[1].parent_entity_id = vpreply->vplist[1].view_prefs_id
   SET nvprequest->nvplist[2].parent_entity_id = vpreply->vplist[1].view_prefs_id
   SET nvprequest->nvplist[1].action_flag = "0"
   SET nvprequest->nvplist[2].action_flag = "0"
   ...
   IF ((nvpreply->nvplist[1].name_value_prefs_id > 0)
    AND (nvpreply->nvplist[2].name_value_prefs_id > 0))
    CALL complete_medpro_vcp(dummy_parm1)
    SET vcprequest->vcplist[1].action_flag = "0"
    SET trace = recpersist
    EXECUTE bed_get_ens_view_comp_prefs  WITH replace("REQUEST",vcprequest), replace("REPLY",vcpreply
     )
    IF ((vcpreply->vcplist[1].view_comp_prefs_id > 0))
     CALL complete_medpro_vcp_nvp(dummy_parm1)
     SET nvprequest->nvplist[1].parent_entity_id = vcpreply->vcplist[1].view_comp_prefs_id
     SET nvprequest->nvplist[2].parent_entity_id = vcpreply->vcplist[1].view_comp_prefs_id
     SET nvprequest->nvplist[3].parent_entity_id = vcpreply->vcplist[1].view_comp_prefs_id
     SET nvprequest->nvplist[4].parent_entity_id = vcpreply->vcplist[1].view_comp_prefs_id
  ...
  IF ((dpreply->dplist[1].detail_prefs_id > 0))
   SELECT INTO "NL:"
    ...
    WHERE nvp.parent_entity_name="DETAIL_PREFS"
     ...
     AND nvp.pvc_name="COMMAND_33462"
  ...
  IF ((((dpreply->dplist[1].detail_prefs_id=0)) OR (nvp_exists=0)) )
   ...
   ENDIF
  ENDIF
  SET prescribe_priv_allowed = 0
  SET refillrenew_priv_allowed = 0
  SET hxorders_priv_allowed = 0
  SET rxreprint_priv_allowed = 0
  SET wophyssig_priv_allowed = 0
  SET pvrequest->pvlist[1].priv_cdf_meaning = "PRESCRIBE"
  SET pvrequest->pvlist[2].priv_cdf_meaning = "REFILLRENEW"
  SET pvrequest->pvlist[3].priv_cdf_meaning = "HXORDERS"
  SET pvrequest->pvlist[4].priv_cdf_meaning = "RXREPRINT"
  SET pvrequest->pvlist[5].priv_cdf_meaning = "W/OPHYSSIG"
  ...
  IF ((pvreply->pvlist[1].privilege_id=0))
   SET prescribe_priv_allowed = 1
  ENDIF
  IF ((pvreply->pvlist[2].privilege_id=0))
   SET refillrenew_priv_allowed = 1
  ENDIF
  IF ((pvreply->pvlist[3].privilege_id=0))
   SET hxorders_priv_allowed = 1
  ENDIF
  IF ((pvreply->pvlist[4].privilege_id=0))
   SET rxreprint_priv_allowed = 1
  ENDIF
  IF ((pvreply->pvlist[5].privilege_id=0))
   SET wophyssig_priv_allowed = 1
  ENDIF
  SET ordertorx_pref_value = " "
  SET rxtoacuteorder_pref_value = " "
  SET rxtoamborder_pref_value = " "
  SET dprequest->dplist[1].position_cd = request->position_cd
  SET dprequest->dplist[1].view_name = "MEDPROFILE"
  SET dprequest->dplist[1].comp_name = "MEDPROFILE"
  SET dprequest->dplist[1].action_flag = "0"
  SET trace = recpersist
  EXECUTE bed_get_ens_detail_prefs  WITH replace("REQUEST",dprequest), replace("REPLY",dpreply)
  ...
  IF (app_detail_prefs_id > 0)
   SELECT INTO "NL:"
    ...
    WHERE nvp.parent_entity_name="DETAIL_PREFS"
     ...
     AND nvp.active_ind=1
     AND nvp.pvc_name IN ("ENABLE_CNVT_TO_RX", "ENABLE_CNVT_TO_INPT_RX", "ENABLE_CNVT_TO_OMA")
    DETAIL
     IF (nvp.pvc_name="ENABLE_CNVT_TO_RX")
      ordertorx_pref_value = nvp.pvc_value
     ELSEIF (nvp.pvc_name="ENABLE_CNVT_TO_INPT_RX")
      rxtoacuteorder_pref_value = nvp.pvc_value
     ELSEIF (nvp.pvc_name="ENABLE_CNVT_TO_OMA")
      rxtoamborder_pref_value = nvp.pvc_value
     ENDIF
    WITH nocounter
   ;end select
  ENDIF
  ...
  IF (medpro_auth_exists=1
   AND easysc_auth_exists=1
   AND menu_auth_value="1"
   AND prescribe_priv_allowed=1
   AND refillrenew_priv_allowed=1
   AND hxorders_priv_allowed=1)
   SET reply->status_data.status_list[3].status = "1"
   IF (ordertorx_pref_value="1")
    SET reply->status_data.status_list[5].status = "1"
   ENDIF
   IF (rxtoacuteorder_pref_value="1")
    SET reply->status_data.status_list[6].status = "1"
   ENDIF
   IF (rxtoamborder_pref_value="1")
  ...
  ELSE
   ...
   ENDIF
  ENDIF
  IF (rxreprint_priv_allowed=1
   AND (((reply->status_data.status_list[1].status="1")) OR ((((reply->status_data.status_list[2].
  status="1")) OR ((reply->status_data.status_list[3].status="1"))) )) )
   SET reply->status_data.status_list[4].status = "1"
  ENDIF
 ELSEIF ((request->action="2"))
  SET viewmeds_off_on_ind = request->task_list[1].on_off_ind
  SET updpastcurr_off_on_ind = request->task_list[2].on_off_ind
  SET updmeds_off_on_ind = request->task_list[3].on_off_ind
  SET rxreprint_off_on_ind = request->task_list[4].on_off_ind
  SET ordertorx_off_on_ind = request->task_list[5].on_off_ind
  SET rxtoacute_off_on_ind = request->task_list[6].on_off_ind
  SET rxtoamb_off_on_ind = request->task_list[7].on_off_ind
  IF (((viewmeds_off_on_ind=1) OR (((updpastcurr_off_on_ind=1) OR (updmeds_off_on_ind=1)) )) )
   CALL complete_medpro_vp(dummy_parm1)
   SET vprequest->vplist[1].action_flag = "0"
   SET trace = recpersist
   EXECUTE bed_get_ens_view_prefs  WITH replace("REQUEST",vprequest), replace("REPLY",vpreply)
   ...
   IF ((vpreply->vplist[1].view_prefs_id > 0))
    ...
    ELSEIF ((nvpreply->nvplist[2].name_value_prefs_id=0))
     ...
     SET nvprequest->nvplist[1].pvc_value = "21"
     SET nvprequest->nvplist[1].action_flag = "1"
     SET trace = recpersist
     EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
      nvpreply)
  ...
  ELSE
   ...
   ENDIF
  ENDIF
  IF (((updpastcurr_off_on_ind=1) OR (updmeds_off_on_ind=1)) )
   CALL complete_easysc_vp(dummy_parm1)
   SET vprequest->vplist[1].action_flag = "0"
   SET trace = recpersist
   EXECUTE bed_get_ens_view_prefs  WITH replace("REQUEST",vprequest), replace("REPLY",vpreply)
   ...
   IF ((vpreply->vplist[1].view_prefs_id > 0))
    ...
    ELSEIF ((nvpreply->nvplist[2].name_value_prefs_id=0))
     ...
     SET stat = alterlist(nvpreply->nvplist,1)
     SET nvprequest->nvplist[1].pvc_name = "DISPLAY_SEQ"
     SET nvprequest->nvplist[1].pvc_value = "7"
     SET nvprequest->nvplist[1].action_flag = "1"
     SET trace = recpersist
     EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
      nvpreply)
   ...
   IF ((vcpreply->vcplist[1].view_comp_prefs_id > 0))
    ...
    ELSE
     ...
     IF (hold_nvp5=0)
      SET nvprequest->nvplist[1].pvc_name = "COMMAND_ID"
      SET nvprequest->nvplist[1].pvc_value = "33247"
      SET trace = recpersist
      EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
       nvpreply)
     ENDIF
  ...
  SET nvprequest->nvplist[1].pvc_name = "COMMAND_33462"
  IF (updmeds_off_on_ind=1)
   IF (nvp_exists > 0)
  ...
  IF ((dpreply->dplist[1].detail_prefs_id > 0))
   SELECT INTO "NL:"
    ...
    WHERE nvp.parent_entity_name="DETAIL_PREFS"
     AND (nvp.parent_entity_id=dpreply->dplist[1].detail_prefs_id)
     AND nvp.pvc_name="SHOW_MEDICATIONS"
     AND nvp.active_ind=1
    DETAIL
  ...
  IF (updmeds_off_on_ind=1)
   IF (prescribe_priv_allowed=0)
    ...
    SET prescribe_priv_allowed = 1
   ENDIF
   IF (refillrenew_priv_allowed=0)
    SET pvrequest->pvlist[1].priv_cdf_meaning = "REFILLRENEW"
    SET pvrequest->pvlist[1].action_flag = "3"
    SET trace = recpersist
    EXECUTE bed_get_ens_privilege  WITH replace("REQUEST",pvrequest), replace("REPLY",pvreply)
    SET refillrenew_priv_allowed = 1
   ENDIF
   IF (hxorders_priv_allowed=0)
    SET pvrequest->pvlist[1].priv_cdf_meaning = "HXORDERS"
    SET pvrequest->pvlist[1].action_flag = "3"
    SET trace = recpersist
    EXECUTE bed_get_ens_privilege  WITH replace("REQUEST",pvrequest), replace("REPLY",pvreply)
    SET hxorders_priv_allowed = 1
   ENDIF
  ELSEIF (updpastcurr_off_on_ind=1)
   IF (hxorders_priv_allowed=0)
    SET pvrequest->pvlist[1].priv_cdf_meaning = "HXORDERS"
    SET pvrequest->pvlist[1].action_flag = "3"
    SET trace = recpersist
  ...
  IF (medstu_ind=1)
   ...
   ENDIF
  ENDIF
  IF (rxreprint_off_on_ind=1)
   IF (rxreprint_priv_allowed=0)
    SET pvrequest->pvlist[1].priv_cdf_meaning = "RXREPRINT"
    SET pvrequest->pvlist[1].action_flag = "3"
    SET trace = recpersist
    EXECUTE bed_get_ens_privilege  WITH replace("REQUEST",pvrequest), replace("REPLY",pvreply)
    SET rxreprint_priv_allowed = 1
  ...
  ELSE
   ...
   ENDIF
  ENDIF
  SET cnvt_to_rx_exists = 0
  SET cnvt_to_rx_value = " "
  SET cnvt_to_inpt_rx_exists = 0
  SET cnvt_to_inpt_rx_value = " "
  SET cnvt_to_oma_exists = 0
  SET cnvt_to_oma_value = " "
  SET dpreply->dplist[1].detail_prefs_id = 0.0
  SET dprequest->dplist[1].position_cd = request->position_cd
  SET dprequest->dplist[1].view_name = "MEDPROFILE"
  SET dprequest->dplist[1].comp_name = "MEDPROFILE"
  ...
  IF ((dpreply->dplist[1].detail_prefs_id > 0))
   SELECT INTO "NL:"
    ...
    DETAIL
     IF (nvp.pvc_name="ENABLE_CNVT_TO_RX")
      cnvt_to_rx_exists = 1, cnvt_to_rx_value = nvp.pvc_value
     ELSEIF (nvp.pvc_name="ENABLE_CNVT_TO_INPT_RX")
      cnvt_to_inpt_rx_exists = 1, cnvt_to_inpt_rx_value = nvp.pvc_value
     ELSEIF (nvp.pvc_name="ENABLE_CNVT_TO_OMA")
      cnvt_to_oma_exists = 1, cnvt_to_oma_value = nvp.pvc_value
     ENDIF
    WITH nocounter
   ;end select
  ELSE
  ...
  SET nvprequest->nvplist[1].parent_entity_name = "DETAIL_PREFS"
  SET nvprequest->nvplist[1].parent_entity_id = dpreply->dplist[1].detail_prefs_id
  SET nvprequest->nvplist[1].pvc_name = "ENABLE_CNVT_TO_RX"
  IF (updmeds_off_on_ind=1
   AND ordertorx_off_on_ind=1)
   IF (cnvt_to_rx_exists > 0)
    IF (cnvt_to_rx_value != "1")
     SET nvprequest->nvplist[1].pvc_value = "1"
     SET nvprequest->nvplist[1].action_flag = "2"
     SET trace = recpersist
     EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
  ...
  ELSE
   ...
   ENDIF
  ENDIF
  SET nvprequest->nvplist[1].pvc_name = "ENABLE_CNVT_TO_INPT_RX"
  IF (updmeds_off_on_ind=1
   AND rxtoacute_off_on_ind=1)
   IF (cnvt_to_inpt_rx_exists > 0)
    IF (cnvt_to_inpt_rx_value != "1")
     SET nvprequest->nvplist[1].pvc_value = "1"
     SET nvprequest->nvplist[1].action_flag = "2"
     SET trace = recpersist
     EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
  ...
  ELSE
   ...
   ENDIF
  ENDIF
  SET nvprequest->nvplist[1].pvc_name = "ENABLE_CNVT_TO_OMA"
  IF (updmeds_off_on_ind=1
   AND rxtoamb_off_on_ind=1)
   IF (cnvt_to_oma_exists > 0)
    IF (cnvt_to_oma_value != "1")
     SET nvprequest->nvplist[1].pvc_value = "1"
     SET nvprequest->nvplist[1].action_flag = "2"
     SET trace = recpersist
     EXECUTE bed_get_ens_name_value_prefs  WITH replace("REQUEST",nvprequest), replace("REPLY",
 ...
 ENDIF
 GO TO exitscript
 SUBROUTINE complete_easysc_vp(dummy_parm2)
   ...
   SET vprequest->vplist[1].view_name = "EASYSCRIPT"
   SET vprequest->vplist[1].view_seq = 0
 END ;Subroutine
 SUBROUTINE complete_easysc_vp_nvp(dummy_parm2)
   ...
   SET nvprequest->nvplist[1].pvc_value = "EasyScript"
   SET nvprequest->nvplist[2].parent_entity_name = "VIEW_PREFS"
   SET nvprequest->nvplist[2].pvc_name = "DISPLAY_SEQ"
   SET nvprequest->nvplist[2].pvc_value = "7"
 END ;Subroutine
 SUBROUTINE complete_easysc_vcp(dummy_parm2)
   ...
   SET vcprequest->vcplist[1].view_name = "EASYSCRIPT"
   SET vcprequest->vcplist[1].view_seq = 0
   SET vcprequest->vcplist[1].comp_name = "EASYSCRIPT"
   SET vcprequest->vcplist[1].comp_seq = 0
 END ;Subroutine
 SUBROUTINE complete_easysc_vcp_nvp(dummy_parm2)
   ...
   SET nvprequest->nvplist[1].pvc_value = "CPSEasyScript.dll"
   ...
   SET nvprequest->nvplist[5].pvc_value = "33247"
 END ;Subroutine
 SUBROUTINE complete_medpro_vp(dummy_parm2)
   SET stat = alterlist(vprequest->vplist,1)
   SET vprequest->vplist[1].application_number = request->application_number
   SET vprequest->vplist[1].position_cd = request->position_cd
   SET vprequest->vplist[1].prsnl_id = request->prsnl_id
   SET vprequest->vplist[1].frame_type = "CHART"
   SET vprequest->vplist[1].view_name = "MEDPROFILE"
   SET vprequest->vplist[1].view_seq = 0
 END ;Subroutine
 SUBROUTINE complete_medpro_vp_nvp(dummy_parm2)
   SET stat = alterlist(nvprequest->nvplist,2)
   SET stat = alterlist(nvpreply->nvplist,2)
   SET nvprequest->nvplist[1].parent_entity_name = "VIEW_PREFS"
   SET nvprequest->nvplist[1].pvc_name = "VIEW_CAPTION"
   SET nvprequest->nvplist[1].pvc_value = "Medications"
   SET nvprequest->nvplist[2].parent_entity_name = "VIEW_PREFS"
   SET nvprequest->nvplist[2].pvc_name = "DISPLAY_SEQ"
   SET nvprequest->nvplist[2].pvc_value = "21"
 END ;Subroutine
 SUBROUTINE complete_medpro_vcp(dummy_parm2)
   SET stat = alterlist(vcprequest->vcplist,1)
   SET vcprequest->vcplist[1].application_number = request->application_number
   SET vcprequest->vcplist[1].position_cd = request->position_cd
   SET vcprequest->vcplist[1].prsnl_id = request->prsnl_id
   SET vcprequest->vcplist[1].view_name = "MEDPROFILE"
   SET vcprequest->vcplist[1].view_seq = 0
   SET vcprequest->vcplist[1].comp_name = "MEDPROFILE"
   SET vcprequest->vcplist[1].comp_seq = 0
 END ;Subroutine
 SUBROUTINE complete_medpro_vcp_nvp(dummy_parm2)
   SET stat = alterlist(nvprequest->nvplist,5)
   SET stat = alterlist(nvpreply->nvplist,5)
   SET nvprequest->nvplist[1].parent_entity_name = "VIEW_COMP_PREFS"
   SET nvprequest->nvplist[1].pvc_name = "COMP_DLLNAME"
   SET nvprequest->nvplist[1].pvc_value = "CPSMedProfile.dll"
   SET nvprequest->nvplist[2].parent_entity_name = "VIEW_COMP_PREFS"
   SET nvprequest->nvplist[2].pvc_name = "COMP_TYPE"
   SET nvprequest->nvplist[2].pvc_value = "0"
   SET nvprequest->nvplist[3].parent_entity_name = "VIEW_COMP_PREFS"
 ...
 SUBROUTINE complete_encsumm_dp_nvp(dummy_parm2)
   ...
   SET stat = alterlist(nvpreply->nvplist,1)
   SET nvprequest->nvplist[1].parent_entity_name = "DETAIL_PREFS"
   SET nvprequest->nvplist[1].pvc_name = "SHOW_MEDICATIONS"
   SET nvprequest->nvplist[1].pvc_value = "1"
 END ;Subroutine
#exitscript
END GO
