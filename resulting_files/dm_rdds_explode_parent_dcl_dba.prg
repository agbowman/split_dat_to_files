CREATE PROGRAM dm_rdds_explode_parent_dcl:dba
 ...
 SUBROUTINE drcm_get_pk_str(dgps_rec,dgps_tab_name)
   ...
   RETURN(dgps_ret_str)
 END ;Subroutine
 DECLARE parent_cnt = i4 WITH noconstant(1)
 DECLARE drepd_chld_dcl_cnt = i4
 DECLARE drepd_ret = vc
 DECLARE drepd_temp_tbl = vc
 DECLARE drepd_i = i4
 DECLARE drepd_j = i4
 DECLARE drepd_k = i4
 DECLARE drepd_m = i4
 DECLARE drepd_n = i4
 DECLARE drepd_l = i4
 DECLARE drepd_seq = vc
 DECLARE drepd_dblink = vc
 DECLARE drepd_dblink_ind = i2 WITH noconstant(2)
 DECLARE stmt_cnt = i4
 DECLARE drepd_temp_suffix = vc
 DECLARE drepd_temp_md_ind = i2
 DECLARE dsce_dcla_seq = f8
 DECLARE from_stmt = vc
 DECLARE where_stmt = vc
 DECLARE fixed_from_chld = vc
 DECLARE fixed_from_grnd = vc
 DECLARE fixed_where_chld = vc
 DECLARE fixed_where_grnd = vc
 DECLARE subselect = vc
 DECLARE original_pk_where = vc
 FREE RECORD drepd_pkw_data
 RECORD drepd_pkw_data(
   1 data[*]
     2 table_name = vc
     2 pkw_function = vc
     2 pkw_iu_str = vc
 ...
 FREE RECORD drepd_filter_data
 RECORD drepd_filter_data(
   1 data[*]
     2 table_name = vc
     ...
     2 filter_del_str = vc
     2 filter_declare = vc
     2 filter_check_ind = i2
     2 filter[*]
       3 parm_name = vc
 )
 FREE RECORD drepd_pc_tree
 RECORD drepd_pc_tree(
   1 parent_table = vc
   1 child_count = i4
   1 t_suffix = vc
   1 t_alias = vc
 ...
 FREE RECORD temp_new_pkw_dcl
 RECORD temp_new_pkw_dcl(
   1 qual[*]
     2 new_child_pkw = vc
 )
 FREE RECORD drepd_child_pkw
 RECORD drepd_child_pkw(
   1 qual[*]
     2 new_child_pkw = vc
 )
 FREE RECORD drepd_grandchild_pkw
 RECORD drepd_grandchild_pkw(
   1 qual[*]
     2 new_child_pkw = vc
 )
 FREE RECORD drepd_parser
 RECORD drepd_parser(
   1 stmt[*]
     2 str = vc
 )
 DECLARE get_max_pkw_vers(pkw_table_name=vc) = f8
 DECLARE get_table_meta_data(i_table_name=vc,t_suffix=vc(ref),t_md_ind=i2(ref)) = null
 DECLARE get_primary_key(pk_table_name=vc) = vc
 DECLARE explode_parent_child(pkw_function=vc,from_clause=vc,where_clause=vc,child_table=vc,
  child_pkw_vers=f8,
  insert_dcl_flag=i4) = vc
 DECLARE replace_pkw(child_pkw_str=vc,child_alias=vc,parent_alias=vc,child_parent_id_col=vc,
  parent_pk_col=vc,
  child_parent_tab_col=vc,parent_tab_name=vc) = vc
 DECLARE find_versioned_row(sbr_rs_pos=i4) = vc
 DECLARE get_versioned_ind(vers_table_name=vc) = i2
 SET dm_err->eproc = "Exploding parent dm_chg_log row..."
 IF (check_logfile("dm_sc_explode",".log","dm_sc_explode LOG")=0)
  ...
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program_drepd
 ENDIF
 IF ( NOT (validate(request,0)))
  SET dm_err->emsg = "REQUEST STRUCTURE NOT DEFINED"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program_drepd
 ENDIF
 ...
 ENDIF
 IF ((request->qual[parent_cnt].dblink > ""))
  SET drepd_dblink = concat(trim(replace(request->qual[parent_cnt].dblink,"@","")),".WORLD")
  SELECT INTO "NL:"
   FROM all_synonyms a
   WHERE a.synonym_name="RDDS_CHG_LOG_SEQ"
   DETAIL
    IF (a.db_link=drepd_dblink)
     drepd_dblink_ind = 1
    ELSE
     drepd_dblink_ind = 0
    ENDIF
  ...
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET dm_err->emsg = "Error determining if RDDS_CHG_LOG_SEQ exists"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET reply->status = "F"
   SET reply->message = "DCL Explode: Error getting information from ALL_SYNONYMS"
   GO TO exit_program_drepd
  ENDIF
  IF (drepd_dblink_ind=0)
   CALL parser("RDB DROP PUBLIC SYNONYM RDDS_CHG_LOG_SEQ GO",1)
   CALL parser(concat("RDB CREATE PUBLIC SYNONYM RDDS_CHG_LOG_SEQ FOR v500.DM_CLINICAL_SEQ",request->
     qual[parent_cnt].dblink," GO"),1)
  ELSEIF (curqual=0)
   CALL parser(concat("RDB CREATE PUBLIC SYNONYM RDDS_CHG_LOG_SEQ FOR v500.DM_CLINICAL_SEQ",request->
     qual[parent_cnt].dblink," GO"),1)
  ENDIF
  SET drepd_seq = "RDDS_CHG_LOG_SEQ"
 ELSE
  SET drepd_seq = "DM_CLINICAL_SEQ"
 ENDIF
 SET reply->status = "S"
 SET drepd_pc_tree->parent_table = request->qual[parent_cnt].parent_table_name
 SET dm_err->eproc = concat("Exploding parent dcl: Creating tree for ultimate parent ",drepd_pc_tree
  ->parent_table)
 CALL disp_msg(" ",dm_err->logfile,0)
 SET drepd_pc_tree->child_count = 0
 CALL get_table_meta_data(drepd_pc_tree->parent_table,drepd_temp_suffix,drepd_temp_md_ind)
 SET drepd_pc_tree->t_suffix = drepd_temp_suffix
 SET drepd_pc_tree->t_alias = concat("t",drepd_pc_tree->t_suffix)
 SET drepd_pc_tree->t_md_ind = drepd_temp_md_ind
 SET drepd_pc_tree->t_vers_ind = get_versioned_ind(drepd_pc_tree->parent_table)
 IF ((request->qual[parent_cnt].dblink > ""))
  SET drepd_pc_tree->pkw_vers_id = 0
 ELSE
  SET drepd_pc_tree->pkw_vers_id = get_max_pkw_vers(drepd_pc_tree->parent_table)
 ENDIF
 SET drepd_pc_tree->pk_col = get_primary_key(drepd_pc_tree->parent_table)
 SELECT INTO "NL:"
  drtr.child_table, drtr.parent_id_col, drtr.parent_tab_col
  FROM (parser(concat("dm_refchg_tab_r",trim(request->qual[parent_cnt].dblink))) drtr)
  WHERE drtr.parent_table=patstring(concat("*",drepd_pc_tree->parent_table))
   AND drtr.process_type != "MOVER ONLY"
  DETAIL
   drepd_pc_tree->child_count = (drepd_pc_tree->child_count+ 1), stat = alterlist(drepd_pc_tree->
    child_cnt,drepd_pc_tree->child_count), drepd_pc_tree->child_cnt[drepd_pc_tree->child_count].
   child_table = replace(replace(drtr.child_table,"&",""),"%",""),
   drepd_pc_tree->child_cnt[drepd_pc_tree->child_count].parent_id_col = drtr.parent_id_col,
   drepd_pc_tree->child_cnt[drepd_pc_tree->child_count].parent_tab_col = drtr.parent_tab_col
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = "Error getting child tables from dm_refchg_tab_r"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET reply->status = "F"
  SET reply->message = "DCL Explode: Error getting child tables from dm_refchg_tab_r"
  GO TO exit_program_drepd
 ENDIF
 FOR (drepd_i = 1 TO drepd_pc_tree->child_count)
   SET dm_err->eproc = concat("Exploding parent dcl row: Finding child tables for ",drepd_pc_tree->
    child_cnt[drepd_i].child_table)
   CALL disp_msg(" ",dm_err->logfile,0)
   SET drepd_j = 0
   SELECT INTO "NL:"
    drtr.child_table, drtr.parent_id_col, drtr.parent_tab_col
    FROM (parser(concat("dm_refchg_tab_r",trim(request->qual[parent_cnt].dblink))) drtr)
    WHERE drtr.parent_table=patstring(concat("*",drepd_pc_tree->child_cnt[drepd_i].child_table))
     AND drtr.process_type != "MOVER ONLY"
    DETAIL
     drepd_j = (drepd_j+ 1), stat = alterlist(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt,drepd_j),
     drepd_pc_tree->child_cnt[drepd_i].grandchild_cnt = drepd_j,
     drepd_temp_tbl = replace(replace(replace(drtr.child_table,"^",""),"&",""),"%",""),
     drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].gchild_table = drepd_temp_tbl,
     drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].parent_id_col = drtr.parent_id_col,
     drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].parent_tab_col = drtr.parent_tab_col
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->emsg = "Error getting grandchild tables from dm_refcht_tab_r"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: Error getting grandchild tables from dm_refcht_tab_r"
    GO TO exit_program_drepd
   ENDIF
   CALL get_table_meta_data(drepd_pc_tree->child_cnt[drepd_i].child_table,drepd_temp_suffix,
    drepd_temp_md_ind)
   SET drepd_pc_tree->child_cnt[drepd_i].t_suffix = drepd_temp_suffix
   SET drepd_pc_tree->child_cnt[drepd_i].t_alias = concat("t",drepd_pc_tree->child_cnt[drepd_i].
    t_suffix)
   SET drepd_pc_tree->child_cnt[drepd_i].t_md_ind = drepd_temp_md_ind
   IF ((request->qual[parent_cnt].dblink > ""))
    SET drepd_pc_tree->child_cnt[drepd_i].pkw_vers_id = 0
   ELSE
    SET drepd_pc_tree->child_cnt[drepd_i].pkw_vers_id = get_max_pkw_vers(drepd_pc_tree->child_cnt[
     drepd_i].child_table)
   ENDIF
   IF (drepd_j > 0)
    SET drepd_pc_tree->child_cnt[drepd_i].pk_col = get_primary_key(drepd_pc_tree->child_cnt[drepd_i].
     child_table)
   ENDIF
   SET stat = alterlist(drepd_pkw_data->data,0)
   SET stat = alterlist(drepd_filter_data->data,0)
   SET drepd_ret = pk_where_info(drepd_pc_tree->child_cnt[drepd_i].child_table,drepd_pc_tree->
    child_cnt[drepd_i].t_suffix,drepd_pc_tree->child_cnt[drepd_i].t_alias,"",drepd_pkw_data)
   SET dm_err->eproc = concat("Exploding parent dcl: Finding PK_WHERE Info")
   IF (drepd_ret="S")
    CALL parser(drepd_pkw_data->data[1].pkw_declare,1)
    SET drepd_pc_tree->child_cnt[drepd_i].pkw_iu_str = drepd_pkw_data->data[1].pkw_iu_str
   ELSEIF (drepd_ret="E")
    SET dm_err->emsg = concat("The pk_where function for ",drepd_pc_tree->child_cnt[drepd_i].
     child_table," does not exist")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: PK_WHERE function doesn't exist."
    GO TO exit_program_drepd
   ELSEIF (drepd_ret="P")
    SET dm_err->emsg = concat("The pk_where function parameters for ",drepd_pc_tree->child_cnt[
     drepd_i].child_table," do not exist")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: PK_WHERE function parameters."
    GO TO exit_program_drepd
   ELSEIF (drepd_ret="F")
    SET dm_err->emsg = concat("Error finding pk_where function information for ",drepd_pc_tree->
     child_cnt[drepd_i].child_table)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: Finding PK_WHERE function."
    GO TO exit_program_drepd
   ENDIF
   SET drepd_ret = filter_info(drepd_pc_tree->child_cnt[drepd_i].child_table,drepd_pc_tree->
    child_cnt[drepd_i].t_suffix,drepd_pc_tree->child_cnt[drepd_i].t_alias,"",drepd_filter_data)
   SET dm_err->eproc = concat("Exploding parent dcl: Finding FILTER Info")
   IF (drepd_ret="S")
    CALL parser(drepd_filter_data->data[1].filter_declare,1)
    SET drepd_pc_tree->child_cnt[drepd_i].fltr_add_str = drepd_filter_data->data[1].filter_add_str
   ELSEIF (drepd_ret="E")
    SET dm_err->eproc = concat("The filter function for ",drepd_pc_tree->child_cnt[drepd_i].
     child_table," does not exist")
    CALL disp_msg("",dm_err->logfile,0)
   ELSEIF (drepd_ret="P")
    SET dm_err->emsg = concat("The filter function parameters for ",drepd_pc_tree->child_cnt[drepd_i]
     .child_table," do not exist")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: FILTER function parameters."
    GO TO exit_program_drepd
   ELSEIF (drepd_ret="F")
    SET dm_err->emsg = concat("Error finding filter function information for ",drepd_pc_tree->
     child_cnt[drepd_i].child_table)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: Finding FILTER function."
    GO TO exit_program_drepd
   ENDIF
   IF ((drepd_pc_tree->t_md_ind=1))
    IF ((drepd_pc_tree->child_cnt[drepd_i].t_md_ind=1))
     SET drepd_pc_tree->child_cnt[drepd_i].t_ins_flg = 3
    ELSE
     SET drepd_pc_tree->child_cnt[drepd_i].t_ins_flg = 1
    ENDIF
   ELSE
    IF ((drepd_pc_tree->child_cnt[drepd_i].t_md_ind=1))
     SET drepd_pc_tree->child_cnt[drepd_i].t_ins_flg = 2
     IF ( NOT ((request->qual[parent_cnt].dblink > "")))
      SET drepd_pc_tree->child_cnt[drepd_i].pkw_iu_str = replace_pkw(drepd_pc_tree->child_cnt[drepd_i
       ].pkw_iu_str,drepd_pc_tree->child_cnt[drepd_i].t_alias,drepd_pc_tree->t_alias,drepd_pc_tree->
       child_cnt[drepd_i].parent_id_col,drepd_pc_tree->pk_col,
       drepd_pc_tree->child_cnt[drepd_i].parent_tab_col,drepd_pc_tree->parent_table)
     ENDIF
    ELSE
     SET drepd_pc_tree->child_cnt[drepd_i].t_ins_flg = 1
    ENDIF
   ENDIF
   IF ((drepd_pc_tree->parent_table="SEGMENT_REFERENCE")
    AND (drepd_pc_tree->child_cnt[drepd_i].child_table="SEG_GRP_SEQ_R"))
    SET drepd_pc_tree->child_cnt[drepd_i].t_ins_flg = 2
    IF ( NOT ((request->qual[parent_cnt].dblink > "")))
     SET drepd_pc_tree->child_cnt[drepd_i].pkw_iu_str = replace(drepd_pc_tree->child_cnt[drepd_i].
      pkw_iu_str,drepd_pc_tree->child_cnt[drepd_i].parent_id_col,drepd_pc_tree->pk_col,1)
     SET drepd_pc_tree->child_cnt[drepd_i].pkw_iu_str = replace(drepd_pc_tree->child_cnt[drepd_i].
      pkw_iu_str,concat(drepd_pc_tree->child_cnt[drepd_i].t_alias,".SEG_GRP_SEQ_R_ID"),"0.0",1)
     SET drepd_pc_tree->child_cnt[drepd_i].pkw_iu_str = replace(drepd_pc_tree->child_cnt[drepd_i].
      pkw_iu_str,drepd_pc_tree->child_cnt[drepd_i].t_alias,drepd_pc_tree->t_alias,1)
    ENDIF
   ENDIF
   FOR (drepd_k = 1 TO drepd_pc_tree->child_cnt[drepd_i].grandchild_cnt)
     SET dm_err->eproc = concat("Exploding parent dcl row: Finding child tables for ",drepd_pc_tree->
      child_cnt[drepd_i].gchild_cnt[drepd_k].gchild_table)
     CALL disp_msg(" ",dm_err->logfile,0)
     SET drepd_l = 0
     SELECT INTO "NL:"
      drtr.child_table, drtr.parent_id_col, drtr.parent_tab_col
      FROM (parser(concat("dm_refchg_tab_r",trim(request->qual[parent_cnt].dblink))) drtr)
      WHERE drtr.parent_table=patstring(concat("*",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
        drepd_k].gchild_table))
       AND drtr.process_type != "MOVER ONLY"
      DETAIL
       drepd_l = (drepd_l+ 1), stat = alterlist(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k]
        .grt_gchild_cnt,drepd_l), drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].
       grt_grandchild_cnt = drepd_l,
       drepd_temp_tbl = replace(replace(replace(drtr.child_table,"^",""),"&",""),"%",""),
       drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].child_table =
       drepd_temp_tbl, drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].
       parent_id_col = drtr.parent_id_col,
       drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].parent_tab_col
        = drtr.parent_tab_col
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      SET dm_err->emsg = "Error getting great grandchild tables from dm_refcht_tab_r"
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET reply->status = "F"
      SET reply->message = "DCL Explode: Error getting great grandchild tables from dm_refcht_tab_r"
      GO TO exit_program_drepd
     ENDIF
     CALL get_table_meta_data(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].gchild_table,
      drepd_temp_suffix,drepd_temp_md_ind)
     SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_suffix = drepd_temp_suffix
     SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_alias = concat("t",drepd_pc_tree->
      child_cnt[drepd_i].gchild_cnt[drepd_k].t_suffix)
     SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_md_ind = drepd_temp_md_ind
     IF ((request->qual[parent_cnt].dblink > ""))
      SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].pkw_vers_id = 0
     ELSE
      SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].pkw_vers_id = get_max_pkw_vers(
       drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].gchild_table)
     ENDIF
     IF (drepd_l > 0)
      SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].pk_col = get_primary_key(
       drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].gchild_table)
     ENDIF
     SET stat = alterlist(drepd_pkw_data->data,0)
     SET stat = alterlist(drepd_filter_data->data,0)
     SET drepd_ret = pk_where_info(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].gchild_table,
      drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_suffix,drepd_pc_tree->child_cnt[drepd_i
      ].gchild_cnt[drepd_k].t_alias,"",drepd_pkw_data)
     SET dm_err->eproc = concat("Exploding parent dcl: Finding PK_WHERE Info")
     IF (drepd_ret="S")
      CALL parser(drepd_pkw_data->data[1].pkw_declare,1)
      SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].pkw_iu_str = drepd_pkw_data->data[1].
      pkw_iu_str
     ELSEIF (drepd_ret="E")
      SET dm_err->emsg = concat("The pk_where function for ",drepd_pc_tree->child_cnt[drepd_i].
       gchild_cnt[drepd_k].gchild_table," does not exist")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET reply->status = "F"
      SET reply->message = "DCL Explode: PK_WHERE function doesn't exist."
      GO TO exit_program_drepd
     ELSEIF (drepd_ret="P")
      SET dm_err->emsg = concat("The pk_where function parameters for ",drepd_pc_tree->child_cnt[
       drepd_i].gchild_cnt[drepd_k].gchild_table," do not exist")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET reply->status = "F"
      SET reply->message = "DCL Explode: Finding PK_WHERE function parameters."
      GO TO exit_program_drepd
     ELSEIF (drepd_ret="F")
      SET dm_err->emsg = concat("Error finding pk_where function information for ",drepd_pc_tree->
       child_cnt[drepd_i].gchild_cnt[drepd_k].gchild_table)
      ...
      GO TO exit_program_drepd
     ENDIF
     SET drepd_ret = filter_info(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].gchild_table,
      drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_suffix,drepd_pc_tree->child_cnt[drepd_i
      ].gchild_cnt[drepd_k].t_alias,"",drepd_filter_data)
     SET dm_err->eproc = concat("Exploding parent dcl: Finding FILTER Info")
     IF (drepd_ret="S")
      CALL parser(drepd_filter_data->data[1].filter_declare,1)
      SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].fltr_add_str = drepd_filter_data->
      data[1].filter_add_str
     ELSEIF (drepd_ret="E")
      SET dm_err->emsg = concat("The filter function for ",drepd_pc_tree->child_cnt[drepd_i].
       gchild_cnt[drepd_k].gchild_table," does not exist")
      CALL disp_msg("",dm_err->logfile,0)
     ELSEIF (drepd_ret="P")
      SET dm_err->emsg = concat("The filter function parameters for ",drepd_pc_tree->child_cnt[
       drepd_i].gchild_cnt[drepd_k].gchild_table," do not exist")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET reply->status = "F"
      SET reply->message = "DCL Explode: Finding FILTER function parameters."
      GO TO exit_program_drepd
     ELSEIF (drepd_ret="F")
      SET dm_err->emsg = concat("Error finding filter function information for ",drepd_pc_tree->
       child_cnt[drepd_i].gchild_cnt[drepd_k].gchild_table)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET reply->status = "F"
      SET reply->message = "DCL Explode: FILTER function doesn't exist."
      GO TO exit_program_drepd
     ENDIF
     IF ((drepd_pc_tree->child_cnt[drepd_i].t_md_ind=1))
      IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_md_ind=1))
       SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_ins_flg = 3
      ELSE
       SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_ins_flg = 1
      ENDIF
     ELSE
      IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_md_ind=1))
       SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_ins_flg = 2
       IF ( NOT ((request->qual[parent_cnt].dblink > "")))
        SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].pkw_iu_str = replace_pkw(
         drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].pkw_iu_str,drepd_pc_tree->child_cnt[
         drepd_i].gchild_cnt[drepd_k].t_alias,drepd_pc_tree->child_cnt[drepd_i].t_alias,drepd_pc_tree
         ->child_cnt[drepd_i].gchild_cnt[drepd_k].parent_id_col,drepd_pc_tree->child_cnt[drepd_i].
         pk_col,
         drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].parent_tab_col,drepd_pc_tree->
         child_cnt[drepd_i].child_table)
       ENDIF
      ELSE
       SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_ins_flg = 1
      ENDIF
     ENDIF
     FOR (drepd_l = 1 TO size(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt,5)
      )
       CALL get_table_meta_data(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[
        drepd_l].child_table,drepd_temp_suffix,drepd_temp_md_ind)
       SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].t_suffix =
       drepd_temp_suffix
       SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].t_alias =
       concat("t",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].
        t_suffix)
       SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].t_md_ind =
       drepd_temp_md_ind
       IF ((request->qual[parent_cnt].dblink > ""))
        SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].pkw_vers_id
         = 0
       ELSE
        SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].pkw_vers_id
         = get_max_pkw_vers(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[
         drepd_l].child_table)
       ENDIF
       SET stat = alterlist(drepd_pkw_data->data,0)
       SET stat = alterlist(drepd_filter_data->data,0)
       SET drepd_ret = pk_where_info(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].
        grt_gchild_cnt[drepd_l].child_table,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].
        grt_gchild_cnt[drepd_l].t_suffix,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].
        grt_gchild_cnt[drepd_l].t_alias,"",drepd_pkw_data)
       SET dm_err->eproc = concat("Exploding parent dcl: Finding PK_WHERE Info")
       IF (drepd_ret="S")
        CALL parser(drepd_pkw_data->data[1].pkw_declare,1)
        SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].pkw_iu_str
         = drepd_pkw_data->data[1].pkw_iu_str
       ELSEIF (drepd_ret="E")
        SET dm_err->emsg = concat("The pk_where function for ",drepd_pc_tree->child_cnt[drepd_i].
         gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].child_table," does not exist")
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET reply->status = "F"
        SET reply->message = "DCL Explode: PK_WHERE function doesn't exist."
        GO TO exit_program_drepd
       ELSEIF (drepd_ret="P")
        SET dm_err->emsg = concat("The pk_where function paremeters for ",drepd_pc_tree->child_cnt[
         drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].child_table," do not exist")
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET reply->status = "F"
        SET reply->message = "DCL Explode: Finding PK_WHERE function parameters."
        GO TO exit_program_drepd
       ELSEIF (drepd_ret="F")
        SET dm_err->emsg = concat("Error finding pk_where function information for ",drepd_pc_tree->
         child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].child_table)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET reply->status = "F"
        SET reply->message = "DCL Explode: PK_WHERE function doesn't exist."
        GO TO exit_program_drepd
       ENDIF
       SET drepd_ret = filter_info(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].
        grt_gchild_cnt[drepd_l].child_table,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].
        grt_gchild_cnt[drepd_l].t_suffix,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].
        grt_gchild_cnt[drepd_l].t_alias,"",drepd_filter_data)
       SET dm_err->eproc = concat("Exploding parent dcl: Finding FILTER Info")
       IF (drepd_ret="S")
        CALL parser(drepd_filter_data->data[1].filter_declare,1)
        SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].
        fltr_add_str = drepd_filter_data->data[1].filter_add_str
       ELSEIF (drepd_ret="E")
        SET dm_err->eproc = concat("The filter function for ",drepd_pc_tree->child_cnt[drepd_i].
         gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].child_table," does not exist")
        CALL disp_msg("",dm_err->logfile,0)
       ELSEIF (drepd_ret="P")
        SET dm_err->emsg = concat("The filter function paremeters for ",drepd_pc_tree->child_cnt[
         drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].child_table," do not exist")
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET reply->status = "F"
        SET reply->message = "DCL Explode: FILTER function parameters do not exist exist."
        GO TO exit_program_drepd
       ELSEIF (drepd_ret="F")
        SET dm_err->emsg = concat("Error finding filter function information for ",drepd_pc_tree->
         child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].child_table)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET reply->status = "F"
        SET reply->message = "DCL Explode: Getting FILTER function error."
        GO TO exit_program_drepd
       ENDIF
       IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_md_ind=1))
        IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].t_md_ind=1
        ))
         SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].t_ins_flg
          = 3
        ELSE
         SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].t_ins_flg
          = 1
       ...
       ELSE
        IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].t_md_ind=1))
         ...
         IF ( NOT ((request->qual[parent_cnt].dblink > "")))
          SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].
          pkw_iu_str = replace_pkw(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].
           grt_gchild_cnt[drepd_l].pkw_iu_str,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].
           grt_gchild_cnt[drepd_l].t_alias,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].
           t_alias,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].
           parent_id_col,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].pk_col,
           drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].
           parent_tab_col,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].gchild_table)
         ENDIF
        ELSE
         SET drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_k].grt_gchild_cnt[drepd_l].t_ins_flg
          = 1
       ...
       SELECT INTO "NL:"
        FROM (parser(concat("dm_refchg_tab_r",trim(request->qual[parent_cnt].dblink))) drtr)
        WHERE drtr.parent_table=patstring(concat("*",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
          drepd_k].grt_gchild_cnt[drepd_l].child_table))
         AND drtr.process_type != "MOVER ONLY"
        WITH nocounter
       ;end select
       IF (curqual > 0)
        SET dm_err->emsg = "Root level table exceeds depth of 4 levels"
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET reply->status = "F"
        SET reply->message = "DCL Explode: Root level table exceeds depth of 4 levels"
        GO TO exit_program_drepd
       ENDIF
       IF (check_error(dm_err->eproc)=1)
        SET reply->status = "F"
        SET reply->message = "DCL Explode: Checking for tables beyond tree depth of 4"
        GO TO exit_program_drepd
       ENDIF
     ENDFOR
   ENDFOR
 ENDFOR
 FOR (req_idx = 1 TO size(request->qual,5))
   SET dm_err->eproc = concat("Exploding parent dcl: Exploding parent row ",trim(cnvtstring(req_idx)),
    " of ",trim(cnvtstring(size(request->qual,5))))
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (findstring(drepd_pc_tree->t_alias,request->qual[req_idx].pk_where)=0)
    SET dm_err->emsg = concat("Invalid pk_where for ",drepd_pc_tree->parent_table," of ",request->
     qual[req_idx].pk_where)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "B"
    SET reply->message = "DCL Explode: Found pk_where without a table alias"
    GO TO exit_program_drepd
   ENDIF
   IF ((drepd_pc_tree->t_vers_ind=1))
    SET original_pk_where = request->qual[req_idx].pk_where
    SET request->qual[req_idx].pk_where = find_versioned_row(req_idx)
   ENDIF
   SET from_stmt = concat(drepd_pc_tree->parent_table,trim(request->qual[parent_cnt].dblink)," ",
    drepd_pc_tree->t_alias)
   SET fixed_from_chld = from_stmt
   SET where_stmt = request->qual[req_idx].pk_where
   SET fixed_where_chld = where_stmt
   FOR (drepd_i = 1 TO drepd_pc_tree->child_count)
     SET dm_err->eproc = concat("Exploding parent dcl: Exploding child table ",trim(cnvtstring(
        drepd_i))," of ",cnvtstring(drepd_pc_tree->child_count))
     CALL disp_msg(" ",dm_err->logfile,0)
     IF ((((drepd_pc_tree->child_cnt[drepd_i].t_ins_flg != 2)) OR ((request->qual[parent_cnt].dblink
      > ""))) )
      SET from_stmt = concat(drepd_pc_tree->child_cnt[drepd_i].child_table,trim(request->qual[
        parent_cnt].dblink)," ",drepd_pc_tree->child_cnt[drepd_i].t_alias,", ",
       fixed_from_chld)
     ENDIF
     SET fixed_from_grnd = from_stmt
     IF ((drepd_pc_tree->child_cnt[drepd_i].t_ins_flg=2)
      AND  NOT ((request->qual[parent_cnt].dblink > "")))
      SET subselect = concat("EXISTS(SELECT 'x' FROM ",drepd_pc_tree->child_cnt[drepd_i].child_table,
       trim(request->qual[parent_cnt].dblink)," ",drepd_pc_tree->child_cnt[drepd_i].t_alias,
       " "," WHERE ",drepd_pc_tree->child_cnt[drepd_i].t_alias,".",drepd_pc_tree->child_cnt[drepd_i].
       parent_id_col,
       " = ",drepd_pc_tree->t_alias,".",drepd_pc_tree->pk_col)
      IF ((drepd_pc_tree->child_cnt[drepd_i].parent_tab_col > ""))
       SET subselect = concat(subselect," AND ",drepd_pc_tree->child_cnt[drepd_i].t_alias,".",
        drepd_pc_tree->child_cnt[drepd_i].parent_tab_col,
        " = '",drepd_pc_tree->parent_table,"'")
      ENDIF
      IF ((drepd_pc_tree->child_cnt[drepd_i].fltr_add_str > ""))
       SET subselect = concat(subselect," AND ",drepd_pc_tree->child_cnt[drepd_i].fltr_add_str," = 1"
        )
      ENDIF
      SET where_stmt = concat(fixed_where_chld," AND ",subselect,")")
     ELSE
      SET where_stmt = concat(fixed_where_chld," AND ",drepd_pc_tree->t_alias,".",drepd_pc_tree->
       pk_col,
       " = ",drepd_pc_tree->child_cnt[drepd_i].t_alias,".",drepd_pc_tree->child_cnt[drepd_i].
       parent_id_col)
      IF ((drepd_pc_tree->child_cnt[drepd_i].parent_tab_col > ""))
       SET where_stmt = concat(where_stmt," AND ",drepd_pc_tree->child_cnt[drepd_i].t_alias,".",
        drepd_pc_tree->child_cnt[drepd_i].parent_tab_col,
        ' = "',drepd_pc_tree->parent_table,'"')
      ENDIF
      IF ((drepd_pc_tree->child_cnt[drepd_i].fltr_add_str > ""))
       SET where_stmt = concat(where_stmt," AND ",drepd_pc_tree->child_cnt[drepd_i].fltr_add_str,
        " = 1")
      ENDIF
     ENDIF
     SET fixed_where_grnd = where_stmt
     SET drepd_ret = explode_parent_child(drepd_pc_tree->child_cnt[drepd_i].pkw_iu_str,from_stmt,
      where_stmt,drepd_pc_tree->child_cnt[drepd_i].child_table,drepd_pc_tree->child_cnt[drepd_i].
      pkw_vers_id,
      drepd_pc_tree->child_cnt[drepd_i].t_ins_flg)
     IF (drepd_ret="F")
      SET reply->status = "F"
      SET reply->message = concat("Error exploding child table ",drepd_pc_tree->child_cnt[drepd_i].
       child_table)
      GO TO exit_program_drepd
     ENDIF
     FOR (drepd_j = 1 TO drepd_pc_tree->child_cnt[drepd_i].grandchild_cnt)
       SET dm_err->eproc = concat("Exploding parent dcl: Exploding grandchild table ",trim(cnvtstring
         (drepd_j))," of ",cnvtstring(drepd_pc_tree->child_cnt[drepd_i].grandchild_cnt))
       CALL disp_msg(" ",dm_err->logfile,0)
       IF ((((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].t_ins_flg != 2)) OR ((request->
       qual[parent_cnt].dblink > ""))) )
        SET from_stmt = concat(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].gchild_table,
         trim(request->qual[parent_cnt].dblink)," ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
         drepd_j].t_alias,", ",
         fixed_from_grnd)
       ENDIF
       SET fixed_from_grt = from_stmt
       IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].t_ins_flg=2)
        AND  NOT ((request->qual[parent_cnt].dblink > "")))
        SET subselect = concat("EXISTS(SELECT 'x' FROM ",drepd_pc_tree->child_cnt[drepd_i].
         gchild_cnt[drepd_j].gchild_table,trim(request->qual[parent_cnt].dblink)," ",drepd_pc_tree->
         child_cnt[drepd_i].gchild_cnt[drepd_j].t_alias,
         " "," WHERE ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].t_alias,".",
         drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].parent_id_col,
         " = ",drepd_pc_tree->child_cnt[drepd_i].t_alias,".",drepd_pc_tree->child_cnt[drepd_i].pk_col
         )
        IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].parent_tab_col > ""))
         SET subselect = concat(subselect," AND ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
          drepd_j].t_alias,".",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].parent_tab_col,
          " = '",drepd_pc_tree->child_cnt[drepd_i].child_table,"'")
        ENDIF
        IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].fltr_add_str > ""))
         SET subselect = concat(subselect," AND ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
          drepd_j].fltr_add_str," = 1")
        ENDIF
        SET where_stmt = concat(fixed_where_grnd," AND ",subselect,")")
       ELSE
        SET where_stmt = concat(fixed_where_grnd," AND ",drepd_pc_tree->child_cnt[drepd_i].t_alias,
         ".",drepd_pc_tree->child_cnt[drepd_i].pk_col,
         " = ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].t_alias,".",drepd_pc_tree->
         child_cnt[drepd_i].gchild_cnt[drepd_j].parent_id_col)
        IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].parent_tab_col > ""))
         SET where_stmt = concat(where_stmt," AND ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
          drepd_j].t_alias,".",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].parent_tab_col,
          ' = "',drepd_pc_tree->child_cnt[drepd_i].child_table,'"')
        ENDIF
        IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].fltr_add_str > ""))
         SET where_stmt = concat(where_stmt," AND ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
          drepd_j].fltr_add_str," = 1")
        ENDIF
        SET fixed_where_grt = where_stmt
       ENDIF
       SET drepd_ret = explode_parent_child(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].
        pkw_iu_str,from_stmt,where_stmt,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].
        gchild_table,drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].pkw_vers_id,
        drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].t_ins_flg)
       IF (drepd_ret="F")
        SET reply->status = "F"
        SET reply->message = concat("Error exploding grandchild table ",drepd_pc_tree->child_cnt[
         drepd_i].gchild_cnt[drepd_j].gchild_table)
        GO TO exit_program_drepd
       ENDIF
       FOR (drepd_k = 1 TO drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_grandchild_cnt)
         SET dm_err->eproc = concat("Exploding parent dcl: Exploding great-grandchild table ",trim(
           cnvtstring(drepd_k))," of ",cnvtstring(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
           drepd_j].grt_grandchild_cnt))
         CALL disp_msg(" ",dm_err->logfile,0)
         IF ((((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].
         t_ins_flg != 2)) OR ((request->qual[parent_cnt].dblink > ""))) )
          SET from_stmt = concat(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].
           grt_gchild_cnt[drepd_k].child_table,trim(request->qual[parent_cnt].dblink)," ",
           drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].t_alias,", ",
           fixed_from_grt)
         ENDIF
         IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].t_ins_flg
         =2)
          AND  NOT ((request->qual[parent_cnt].dblink > "")))
          SET subselect = concat("EXISTS(SELECT 'x' FROM ",drepd_pc_tree->child_cnt[drepd_i].
           gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].child_table,trim(request->qual[parent_cnt].
            dblink)," ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k]
           .t_alias,
           " WHERE ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].
           t_alias,".",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].
           parent_id_col," = ",
           drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].t_alias,".",drepd_pc_tree->
           child_cnt[drepd_i].gchild_cnt[drepd_j].pk_col)
          IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].
          parent_tab_col > ""))
           SET subselect = concat(subselect,", AND ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
            drepd_j].grt_gchild_cnt[drepd_k].t_alias,".",drepd_pc_tree->child_cnt[drepd_i].
            gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].parent_tab_col,
            " = '",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].child_table,"'")
          ENDIF
          IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].
          fltr_add_str > ""))
           SET subselect = concat(subselect," AND ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
            drepd_j].grt_gchild_cnt[drepd_k].fltr_add_str," = 1")
          ENDIF
          SET where_stmt = concat(fixed_where_grt," AND ",subselect,")")
         ELSE
          SET where_stmt = concat(fixed_where_grt," AND ",drepd_pc_tree->child_cnt[drepd_i].
           gchild_cnt[drepd_j].t_alias,".",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].
           pk_col,
           " = ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].
           t_alias,".",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].
           parent_id_col)
          IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].parent_tab_col > ""))
           SET where_stmt = concat(where_stmt," AND ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
            drepd_j].grt_gchild_cnt[drepd_k].t_alias,".",drepd_pc_tree->child_cnt[drepd_i].
            gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].parent_tab_col,
            ' = "',drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].gchild_table,'"')
          ENDIF
          IF ((drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].fltr_add_str > ""))
           SET where_stmt = concat(where_stmt," AND ",drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[
            drepd_j].fltr_add_str," = 1")
         ...
         ENDIF
         SET drepd_ret = explode_parent_child(drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].
          grt_gchild_cnt[drepd_k].pkw_iu_str,from_stmt,where_stmt,drepd_pc_tree->child_cnt[drepd_i].
          gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].child_table,drepd_pc_tree->child_cnt[drepd_i].
          gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].pkw_vers_id,
          drepd_pc_tree->child_cnt[drepd_i].gchild_cnt[drepd_j].grt_gchild_cnt[drepd_k].t_ins_flg)
         IF (drepd_ret="F")
          SET reply->status = "F"
          SET reply->message = concat("Error exploding great-grandchild table: ",drepd_pc_tree->
           child_cnt[drepd_i].gchild_cnt[drepd_j].gchild_table)
          GO TO exit_program_drepd
         ENDIF
       ENDFOR
     ENDFOR
   ...
   SET dm_err->eproc = concat(
    "Exploding parent dcl row: Updating ultimate parent to updt_applictx = 4310001")
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ((drepd_pc_tree->t_vers_ind=1))
    SET request->qual[req_idx].pk_where = original_pk_where
   ENDIF
   IF ((request->qual[req_idx].ctxt_ind=1))
    UPDATE  FROM (parser(concat("dm_chg_log",trim(request->qual[req_idx].dblink))) dcl)
     SET dcl.updt_applctx = 4310001.0
     WHERE (dcl.target_env_id=request->qual[req_idx].target_env_id)
      AND (dcl.table_name=request->qual[req_idx].parent_table_name)
      AND (dcl.log_type=request->qual[req_idx].log_type)
      AND dcl.delete_ind=0
      AND (dcl.pk_where=request->qual[req_idx].pk_where)
      AND dcl.context_name = null
     WITH nocounter
    ;end update
    IF ((validate(request_rows->current_cnt,- (1)) != - (1)))
     SET request_rows->current_cnt = (request_rows->current_cnt+ curqual)
    ENDIF
   ELSE
    UPDATE  FROM (parser(concat("dm_chg_log",trim(request->qual[req_idx].dblink))) dcl)
     SET dcl.updt_applctx = 4310001.0
     WHERE (dcl.target_env_id=request->qual[req_idx].target_env_id)
      ...
      AND dcl.delete_ind=0
      AND (dcl.pk_where=request->qual[req_idx].pk_where)
      AND (dcl.context_name=request->qual[req_idx].context_name)
     WITH nocounter
    ;end update
    IF ((validate(request_rows->current_cnt,- (1)) != - (1)))
     SET request_rows->current_cnt = (request_rows->current_cnt+ curqual)
   ...
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
    SET dm_err->emsg = "Error updating ultimate parent to updt_applictx = 4310001."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: Error updating ultimate parent to updt_applictx = 4310001."
    GO TO exit_program_drepd
   ELSE
    IF ((request->qual[req_idx].commit_ind=1))
     COMMIT
    ENDIF
    SET reply->status = "S"
    SET reply->message = "DCL row was exploded."
    SET dm_err->eproc = concat("Exploding parent dcl row: DCL row was exploded")
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF ((validate(request_rows->current_cnt,- (1)) != - (1)))
    SELECT INTO "NL:"
    ...
    UPDATE  FROM dm_chg_log_audit dcla
     SET dcla.dm_chg_log_audit_id = dsce_dcla_seq, dcla.log_id = 0, dcla.action = "CMIT BCKFL",
      dcla.table_name = request->qual[req_idx].parent_table_name, dcla.text = concat(cnvtstring(
        request_rows->current_cnt)," of ",cnvtstring(request_rows->total_cnt)," rows for ",request->
       qual[req_idx].parent_table_name,
       " has been processed."), dcla.updt_task = 4310001,
    ...
    IF (check_error(dm_err->eproc)=1)
     ROLLBACK
     SET dm_err->emsg = "Error inserting explode status into dm_chg_log_audit"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET reply->status = "F"
     SET reply->message = "Error inserting explode status into dm_chg_log_audit"
    ...
    IF (curqual=0)
     INSERT  FROM dm_chg_log_audit dcla
      SET dcla.dm_chg_log_audit_id = dsce_dcla_seq, dcla.log_id = 0, dcla.action = "CMIT BCKFL",
       dcla.table_name = request->qual[req_idx].parent_table_name, dcla.text = concat(cnvtstring(
         request_rows->current_cnt)," of ",trim(cnvtstring(request_rows->total_cnt))," rows for ",
        request->qual[req_idx].parent_table_name,
        " has been processed."), dcla.updt_task = 4310001,
       dcla.updt_cnt = 0, dcla.updt_dt_tm = cnvtdatetime(curdate,curtime3), dcla.updt_id = 0,
       dcla.updt_applctx = 0, audit_dt_tm = cnvtdatetime(curdate,curtime3)
     ;end insert
   ...
   ENDIF
 ENDFOR
 SUBROUTINE get_max_pkw_vers(pkw_table_name)
   SET dm_err->eproc = concat("Exploding parent dcl row: Finding max dm_refchg_pkw_vers_id for ",
    pkw_table_name)
   CALL disp_msg(" ",dm_err->logfile,0)
   DECLARE sub_max_pkw_vers = f8
   SELECT INTO "NL:"
    drpv.dm_refchg_pkw_vers_id
    FROM dm_refchg_pkw_vers drpv
    WHERE drpv.table_name=pkw_table_name
     AND drpv.active_ind=1
    DETAIL
     sub_max_pkw_vers = drpv.dm_refchg_pkw_vers_id
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->emsg = "Error finding pkw_vers_id."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: Error finding max pkw_vers_id."
    GO TO exit_program_drepd
   ENDIF
   IF (curqual=1)
    RETURN(sub_max_pkw_vers)
   ELSE
    SET dm_err->emsg = concat("Could not find a dm_refchg_pkw_vers_id for ",pkw_table_name)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: Error finding max pkw_vers_id."
    GO TO exit_program_drepd
   ENDIF
 END ;Subroutine
 SUBROUTINE get_table_meta_data(i_table_name,t_suffix,t_md_ind)
   SET dm_err->eproc = concat("Exploding parent dcl row: Finding table meta data for ",i_table_name)
   CALL disp_msg(" ",dm_err->logfile,0)
   DECLARE sub_tbl_sfx = vc
   SELECT INTO "NL:"
    dtd.table_suffix, dtd.merge_delete_ind
    FROM dm_tables_doc_local dtd
    WHERE dtd.table_name=i_table_name
    DETAIL
     t_suffix = dtd.table_suffix, t_md_ind = dtd.merge_delete_ind
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->emsg = "Error finding table meta data."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: Error finding table meta data."
    GO TO exit_program_drepd
   ENDIF
   IF (curqual=0)
    SET dm_err->emsg = concat("Could not find meta data for ",i_table_name)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: Error finding table meta data."
    GO TO exit_program_drepd
   ENDIF
 END ;Subroutine
 SUBROUTINE get_primary_key(pk_table_name)
   SET dm_err->eproc = concat("Exploding parent dcl row: Finding primary key for ",pk_table_name)
   CALL disp_msg(" ",dm_err->logfile,0)
   DECLARE sub_pk_col = vc
   IF (pk_table_name="SEGMENT_REFERENCE")
    SET sub_pk_col = "SEG_CD"
    RETURN(sub_pk_col)
   ENDIF
   SELECT INTO "NL:"
    dcd.column_name
    ...
    WHERE dcd.table_name=pk_table_name
     AND dcd.table_name=dcd.root_entity_name
     AND dcd.column_name=dcd.root_entity_attr
     AND utc.table_name=dcd.table_name
     AND utc.column_name=dcd.column_name
    DETAIL
     sub_pk_col = dcd.column_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->emsg = "Error finding parent table primary key."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: Error finding parent table primary key."
    GO TO exit_program_drepd
   ENDIF
   IF (curqual=1)
    RETURN(sub_pk_col)
   ELSE
    SET dm_err->emsg = concat(" ",pk_table_name)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "DCL Explode: Could not find a primary key."
    GO TO exit_program_drepd
   ENDIF
 END ;Subroutine
 SUBROUTINE explode_parent_child(pkw_function,from_clause,where_clause,child_table,child_pkw_vers,
  insert_dcl_flag)
   DECLARE epd_return = vc
   DECLARE epd_pk_cnt = i4
   DECLARE epd_for_loop = i4
   SET drepd_chld_dcl_cnt = 0
   SET stat = alterlist(temp_new_pkw_dcl->qual,drepd_chld_dcl_cnt)
   SET stmt_cnt = 1
   SET stat = alterlist(drepd_parser->stmt,0)
   SET stat = alterlist(drepd_parser->stmt,30)
   SET dm_err->eproc = "Exploding a row"
   IF ((((request->qual[parent_cnt].dblink > "")) OR (insert_dcl_flag=3)) )
    IF (insert_dcl_flag=2)
     SET drepd_parser->stmt[stmt_cnt].str = concat("select DISTINCT into 'NL:' drepd_pkw = ",
      pkw_function)
    ELSE
     SET drepd_parser->stmt[stmt_cnt].str = concat("select into 'NL:' drepd_pkw = ",pkw_function)
    ENDIF
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = " from "
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = concat(from_clause)
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = concat(where_clause)
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = " detail "
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = " drepd_chld_dcl_cnt = drepd_chld_dcl_cnt + 1 "
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str =
    " stat = alterlist(temp_new_pkw_dcl->qual,drepd_chld_dcl_cnt) "
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str =
    " temp_new_pkw_dcl->qual[drepd_chld_dcl_cnt].new_child_pkw = drepd_pkw "
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = " with nocounter go"
    SET stmt_cnt = size(drepd_parser->stmt,5)
    SET stat = alterlist(drepd_parser->stmt,stmt_cnt)
    EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DREPD_PARSER")
    IF (check_error(dm_err->eproc)=1)
     SET dm_err->emsg = concat("Error creating pk_where for table: ",child_table)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN("F")
    ENDIF
    IF (insert_dcl_flag=3)
     FOR (epd_pk_cnt = 1 TO size(temp_new_pkw_dcl->qual,5))
      IF ((request->qual[req_idx].ctxt_ind=1))
       SET epd_return = call_ins_log(child_table,temp_new_pkw_dcl->qual[epd_pk_cnt].new_child_pkw,
        request->qual[req_idx].log_type,0,0.0,
        0,4310001,"",request->qual[req_idx].target_env_id,child_pkw_vers,
        request->qual[parent_cnt].dblink)
      ELSE
       SET epd_return = call_ins_log(child_table,temp_new_pkw_dcl->qual[epd_pk_cnt].new_child_pkw,
        request->qual[req_idx].log_type,0,0.0,
        0,4310001,request->qual[req_idx].context_name,request->qual[req_idx].target_env_id,
        child_pkw_vers,
        request->qual[parent_cnt].dblink)
      ENDIF
      IF (epd_return="F")
       SET dm_err->eproc = concat("Error exploding pk_where values for table: ",child_table)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN("F")
      ENDIF
     ENDFOR
   ...
   SET stmt_cnt = 0
   IF ((request->qual[parent_cnt].dblink > ""))
    FOR (epd_for_loop = 1 TO size(temp_new_pkw_dcl->qual,5))
      SET stat = alterlist(drepd_parser->stmt,16)
      SET drepd_parser->stmt[1].str = concat("insert into dm_chg_log",trim(request->qual[parent_cnt].
        dblink)," dcl")
      SET drepd_parser->stmt[2].str = concat("set dcl.log_id = seq(",drepd_seq,",NEXTVAL),")
      SET drepd_parser->stmt[3].str = concat("dcl.target_env_id =",trim(cnvtstring(request->qual[
         req_idx].target_env_id)),".0,")
      SET drepd_parser->stmt[4].str = concat('dcl.log_type = "',request->qual[req_idx].log_type,'",')
      SET drepd_parser->stmt[5].str = concat('dcl.table_name = "',child_table,'",')
      SET drepd_parser->stmt[6].str = concat('dcl.pk_where = "',temp_new_pkw_dcl->qual[epd_for_loop].
       new_child_pkw,'",')
      SET drepd_parser->stmt[7].str = concat("dcl.delete_ind = 0,")
      SET drepd_parser->stmt[8].str = concat("dcl.chg_dt_tm = cnvtdatetime(CURDATE, CURTIME3),")
      SET drepd_parser->stmt[9].str = concat("dcl.pk_where_value = ",'sqlpassthru("',
       "dbms_utility.get_hash_value",trim(request->qual[parent_cnt].dblink),"('",
       temp_new_pkw_dcl->qual[epd_for_loop].new_child_pkw,^',0,1073741824.0)"),^)
      IF ((request->qual[req_idx].ctxt_ind=1))
       SET drepd_parser->stmt[10].str = concat("dcl.context_name = NULL,")
      ELSE
       SET drepd_parser->stmt[10].str = concat('dcl.context_name = "',request->qual[req_idx].
        context_name,'",')
      ENDIF
      SET drepd_parser->stmt[11].str = concat("dcl.rdbhandle = trim(CURRDBHANDLE),")
      SET drepd_parser->stmt[12].str = concat("dcl.updt_id = 0.0,")
      SET drepd_parser->stmt[13].str = concat("dcl.updt_dt_tm = cnvtdatetime(CURDATE, CURTIME3),")
      SET drepd_parser->stmt[14].str = concat("dcl.updt_task = 0,")
      SET drepd_parser->stmt[15].str = concat("dcl.updt_applctx = 4310001.0")
      SET drepd_parser->stmt[16].str = " with nocounter go"
      SET stmt_cnt = size(drepd_parser->stmt,5)
      SET stat = alterlist(drepd_parser->stmt,stmt_cnt)
      EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DREPD_PARSER")
      IF (check_error(dm_err->eproc)=1)
       SET dm_err->eproc = "Error exploding pk_where values for child rows."
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET reply->status = "F"
       SET reply->message = "DCL EXPLODE: Error exploding pk_where values for child rows."
       RETURN("F")
      ENDIF
    ENDFOR
    RETURN("S")
   ELSE
    SET stat = alterlist(drepd_parser->stmt,40)
    SET drepd_parser->stmt[1].str = concat("insert into dm_chg_log",trim(request->qual[parent_cnt].
      dblink)," dcl")
    SET drepd_parser->stmt[2].str = "(dcl.log_id,"
    SET drepd_parser->stmt[3].str = "dcl.target_env_id,"
    SET drepd_parser->stmt[4].str = "dcl.log_type,"
    SET drepd_parser->stmt[5].str = "dcl.table_name,"
    SET drepd_parser->stmt[6].str = "dcl.pk_where,"
    SET drepd_parser->stmt[7].str = "dcl.delete_ind,"
    SET drepd_parser->stmt[8].str = "dcl.chg_dt_tm,"
    SET drepd_parser->stmt[9].str = "dcl.pk_where_value,"
    SET drepd_parser->stmt[10].str = "dcl.context_name,"
    SET drepd_parser->stmt[11].str = "dcl.rdbhandle,"
    SET drepd_parser->stmt[12].str = "dcl.updt_id,"
    SET drepd_parser->stmt[13].str = "dcl.updt_dt_tm,"
    SET drepd_parser->stmt[14].str = "dcl.updt_task,"
    SET drepd_parser->stmt[15].str = "dcl.updt_applctx,"
    IF ((request->qual[parent_cnt].dblink=""))
     SET drepd_parser->stmt[16].str = "dcl.updt_cnt,"
     SET drepd_parser->stmt[17].str = "dcl.dm_refchg_pkw_vers_id)"
     SET stmt_cnt = 18
    ELSE
     SET drepd_parser->stmt[16].str = "dcl.updt_cnt)"
     SET stmt_cnt = 17
    ENDIF
    SET drepd_parser->stmt[stmt_cnt].str = "( select "
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = concat("seq(",drepd_seq,",NEXTVAL),")
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = concat(trim(cnvtstring(request->qual[req_idx].
       target_env_id)),".0,")
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = concat('"',request->qual[req_idx].log_type,'",')
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = concat('"',child_table,'",')
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = concat(pkw_function,",")
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = "0,"
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = "cnvtdatetime(CURDATE, CURTIME3),"
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = concat('sqlpassthru("dbms_utility.get_hash_value',"(",
     pkw_function,',0,1073741824.0)"),')
    SET stmt_cnt = (stmt_cnt+ 1)
    IF ((request->qual[req_idx].ctxt_ind=1))
     SET drepd_parser->stmt[stmt_cnt].str = "NULL,"
     SET stmt_cnt = (stmt_cnt+ 1)
    ELSE
     SET drepd_parser->stmt[stmt_cnt].str = concat('"',request->qual[req_idx].context_name,'",')
     SET stmt_cnt = (stmt_cnt+ 1)
    ENDIF
    SET drepd_parser->stmt[stmt_cnt].str = "trim(CURRDBHANDLE),"
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = "0,"
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = "cnvtdatetime(CURDATE, CURTIME3),"
    ...
    SET drepd_parser->stmt[stmt_cnt].str = "0,"
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = "4310001.0,"
    SET stmt_cnt = (stmt_cnt+ 1)
    IF ((request->qual[parent_cnt].dblink=""))
     SET drepd_parser->stmt[stmt_cnt].str = "0,"
     SET stmt_cnt = (stmt_cnt+ 1)
     SET drepd_parser->stmt[stmt_cnt].str = concat(trim(cnvtstring(child_pkw_vers)),".0 ")
     SET stmt_cnt = (stmt_cnt+ 1)
    ELSE
     SET drepd_parser->stmt[stmt_cnt].str = "0"
     SET stmt_cnt = (stmt_cnt+ 1)
    ENDIF
    SET drepd_parser->stmt[stmt_cnt].str = concat(" from ",from_clause)
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = concat(where_clause)
    SET stmt_cnt = (stmt_cnt+ 1)
    SET drepd_parser->stmt[stmt_cnt].str = " with nocounter ) go"
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE replace_pkw(child_pkw_str,child_alias,parent_alias,child_parent_id_col,parent_pk_col,
  child_parent_tab_col,parent_tab_name)
   DECLARE pkw_iu_str = vc
   SET pkw_iu_str = replace(child_pkw_str,child_parent_id_col,parent_pk_col,1)
   SET pkw_iu_str = replace(pkw_iu_str,child_alias,parent_alias,1)
   IF (child_parent_tab_col > "")
    SET pkw_iu_str = replace(pkw_iu_str,concat(child_alias,".",child_parent_tab_col),concat("'",
      parent_tab_name,"'"),1)
   ENDIF
   RETURN(pkw_iu_str)
 END ;Subroutine
 SUBROUTINE find_versioned_row(sbr_rs_pos)
   SET dm_err->eproc = "INSIDE find_versioned_row subroutine"
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE fvr_tab_suffix = vc
   DECLARE fvr_table_name = vc
   DECLARE fvr_original_pkw = vc
   DECLARE fvr_new_pkw = vc
   DECLARE fvr_dblink = vc
   DECLARE fvr_new_pkw_val = f8
   SET fvr_tab_suffix = drepd_pc_tree->t_alias
   SET fvr_table_name = drepd_pc_tree->parent_table
   SET fvr_dblink = request->qual[sbr_rs_pos].dblink
   SET fvr_original_pkw = request->qual[sbr_rs_pos].pk_where
   SET fvr_pk_col = drepd_pc_tree->pk_col
   SET stmt_cnt = 0
   SET stat = alterlist(drepd_parser->stmt,0)
   SET stmt_cnt = 4
   SET stat = alterlist(drepd_parser->stmt,stmt_cnt)
   SET drepd_parser->stmt[1].str = "select into 'NL:'"
   SET drepd_parser->stmt[2].str = concat("from ",fvr_table_name,trim(fvr_dblink)," ",fvr_tab_suffix)
   SET drepd_parser->stmt[4].str = "with nocounter go"
   SET fvr_new_pkw = fvr_original_pkw
   SET drepd_parser->stmt[3].str = fvr_new_pkw
   SET stmt_cnt = size(drepd_parser->stmt,5)
   SET stat = alterlist(drepd_parser->stmt,stmt_cnt)
   EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DREPD_PARSER")
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->eproc = "Version row verify row exists."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "Version row verify row exists."
    GO TO exit_program_drepd
   ENDIF
   IF (curqual <= 1)
    RETURN(fvr_original_pkw)
   ELSE
    SET fvr_new_pkw = concat(fvr_original_pkw," and ",fvr_tab_suffix,".","ACTIVE_IND = 1")
    SET drepd_parser->stmt[3].str = fvr_new_pkw
    SET stmt_cnt = size(drepd_parser->stmt,5)
    SET stat = alterlist(drepd_parser->stmt,stmt_cnt)
    EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DREPD_PARSER")
    IF (check_error(dm_err->eproc)=1)
     SET dm_err->eproc = "Query version row with active_ind."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET reply->status = "F"
     SET reply->message = "Query version row with active_ind."
     GO TO exit_program_drepd
    ENDIF
    IF (curqual > 0)
     RETURN(fvr_new_pkw)
    ELSE
     IF ( NOT (fvr_table_name IN ("GROUP_REFERENCE", "INPUT_FORM_REFERENCE", "INTERP_TASK_ASSAY")))
      SET fvr_new_pkw = concat(fvr_original_pkw," and ",fvr_tab_suffix,".","BEG_EFFECTIVE_DT_TM < ",
       "cnvtdatetime(curdate,curtime3) and ",fvr_tab_suffix,".","END_EFFECTIVE_DT_TM > ",
       "cnvtdatetime(curdate,curtime3)")
      SET drepd_parser->stmt[3].str = fvr_new_pkw
      SET stmt_cnt = size(drepd_parser->stmt,5)
      SET stat = alterlist(drepd_parser->stmt,stmt_cnt)
      EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DREPD_PARSER")
      IF (check_error(dm_err->eproc)=1)
       SET dm_err->eproc = "Query version row with *effective_dt_tm columns."
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET reply->status = "F"
       SET reply->message = "Query version row with *effective_dt_tm columns."
       GO TO exit_program_drepd
      ENDIF
      IF (curqual > 0)
       RETURN(fvr_new_pkw)
     ...
     SET fvr_new_pkw = fvr_original_pkw
     SET drepd_parser->stmt[1].str = "select into 'NL:'"
     SET drepd_parser->stmt[2].str = concat("from ",fvr_table_name,trim(fvr_dblink)," ",
      fvr_tab_suffix)
     SET drepd_parser->stmt[3].str = fvr_new_pkw
     SET drepd_parser->stmt[4].str = concat(" order by ",fvr_tab_suffix,".updt_dt_tm desc ")
     SET drepd_parser->stmt[5].str = " detail "
     SET drepd_parser->stmt[6].str = concat(" fvr_new_pkw_val = ",fvr_tab_suffix,".",fvr_pk_col)
     SET drepd_parser->stmt[7].str = "with nocounter, maxrec = 1 go"
     SET stmt_cnt = size(drepd_parser->stmt,5)
     SET stat = alterlist(drepd_parser->stmt,stmt_cnt)
     EXECUTE dm_rmc_pkw_parse1  WITH replace("REQUEST","DREPD_PARSER")
     IF (check_error(dm_err->eproc)=1)
      SET dm_err->eproc = "Query version row using updt_dt_tm columns."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET reply->status = "F"
      SET reply->message = "Query version row using updt_dt_tm columns."
      GO TO exit_program_drepd
     ENDIF
     IF (curqual > 0)
      SET fvr_new_pkw = concat(fvr_original_pkw," AND ",fvr_tab_suffix,".",fvr_pk_col,
       " = ",trim(cnvtstring(fvr_new_pkw_val)),".0")
      RETURN(fvr_new_pkw)
     ENDIF
    ENDIF
   ENDIF
 END ;Subroutine
 SUBROUTINE get_versioned_ind(vers_table_name)
   SELECT INTO "NL:"
    FROM code_value cv
    WHERE cv.code_set=255351
     AND cv.display=vers_table_name
     AND cv.cdf_meaning IN ("ALG1", "ALG3", "ALG2", "ALG4")
     AND cv.active_ind=1
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->eproc = "Determine if table is versioned."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET reply->status = "F"
    SET reply->message = "Determine if table is versioned."
    GO TO exit_program_drepd
   ENDIF
   IF (curqual > 0)
    RETURN(1)
   ELSE
    IF (vers_table_name IN ("DCP_FORMS_REF", "DCP_SECTION_REF"))
     RETURN(1)
    ELSE
     RETURN(0)
    ENDIF
