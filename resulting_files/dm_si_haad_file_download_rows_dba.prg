CREATE PROGRAM dm_si_haad_file_download_rows:dba
 ...
 SET reply->table_name = "SI_HAAD_FILE_POLL"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(fp.si_haad_file_poll_id)
   FROM si_haad_file_poll fp
   WHERE fp.si_haad_file_poll_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(fp.si_haad_file_poll_id)
  FROM si_haad_file_poll fp
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT DISTINCT INTO "nl:"
    fp.rowid
    FROM si_haad_file_poll fp,
     si_haad_file_download fd
    PLAN (fp
     WHERE fp.si_haad_file_poll_id > 0
      AND fp.end_dt_tm != null
      AND fp.si_haad_file_poll_id BETWEEN v_cur_min_id AND v_cur_max_id)
     JOIN (fd
     WHERE fd.si_haad_file_download_id > 0
      AND fd.si_haad_file_poll_id=fp.si_haad_file_poll_id
      AND fd.processed_flag=0
      AND  NOT (fd.si_haad_file_poll_id IN (
     (SELECT
      si_haad_file_poll_id
      FROM si_haad_file_download
      WHERE processed_flag=1)))
      AND parser(sbr_getrowidnotexists(
       "fd.updt_dt_tm < cnvtdatetime(curdate - v_days_to_keep,curtime3)","fd")))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = fp.rowid
    WITH nocounter, maxqual(fp,value(v_rows_left))
   ;end select
