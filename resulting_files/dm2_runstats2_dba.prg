CREATE PROGRAM dm2_runstats2:dba
 ...
 SUBROUTINE dir_perform_wait_interval(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dr_iter = i4 WITH protect, noconstant(0)
 DECLARE dr_iter2 = i4 WITH protect, noconstant(0)
 DECLARE curr_table = vc WITH protect, noconstant("")
 DECLARE prev_table = vc WITH protect, noconstant("")
 DECLARE dr_string_num = i4 WITH protect, noconstant(0)
 ...
 SUBROUTINE dri_fill_table_stats(null)
   ...
   SELECT INTO "nl:"
    ...
    WHERE di.info_domain="DM2_RUNSTATS"
     AND di.info_name="DB_STALE_PCT_THRESH"
    DETAIL
   ...
   SELECT INTO "nl:"
    FROM dm_info di
    WHERE di.info_domain="DM2_RUNSTATS"
     AND di.info_name="DB_ROWS_TO_SAMPLE_MIN"
    HEAD REPORT
     table_stats->rows_to_sample_min = 5000
    DETAIL
   ...
   SELECT INTO "nl:"
    ...
    HEAD REPORT
     table_stats->rows_to_sample = 100000
    DETAIL
   ...
   ENDIF
   SET dm_err->eproc =
   "Selecting tables to gather statistics on from dm2_dba_tables/all_tab_modifications."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_dba_tables u,
     all_tab_modifications atm
    WHERE (u.owner=table_stats->owner)
     AND u.table_name=patstring(table_stats->table_name)
     AND outerjoin(u.table_name)=atm.table_name
    ORDER BY u.table_name
    HEAD REPORT
     table_cnt = 0, stat = alterlist(table_stats->stats_cnt,table_cnt)
    DETAIL
     table_cnt = (table_cnt+ 1)
     IF (mod(table_cnt,50)=1)
      stat = alterlist(table_stats->stats_cnt,(table_cnt+ 49))
     ENDIF
     table_stats->stats_cnt[table_cnt].owner = trim(u.owner), table_stats->stats_cnt[table_cnt].
     tabname = trim(u.table_name), table_stats->stats_cnt[table_cnt].monitoring = trim(u.monitoring),
     ...
     .num_rows,
     table_stats->stats_cnt[table_cnt].empty_stats_ind = nullind(u.last_analyzed), table_stats->
     stats_cnt[table_cnt].table_mods = nullval(((atm.inserts+ atm.updates)+ atm.deletes),0.0),
     table_stats->stats_cnt[table_cnt].inserts = nullval(atm.inserts,0.0),
     table_stats->stats_cnt[table_cnt].updates = nullval(atm.updates,0.0), table_stats->stats_cnt[
     table_cnt].deletes = nullval(atm.deletes,0.0), table_stats->stats_cnt[table_cnt].last_mod_dt_tm
      = nullval(atm.timestamp,0.0),
     table_stats->stats_cnt[table_cnt].mod_trunc_ind = evaluate(atm.truncated,"YES",1,0)
     IF ((table_stats->gather_option IN ("GATHER_STALE", "LIST_STALE")))
      ...
      ELSE
       IF ((table_stats->stats_cnt[table_cnt].deletes > table_stats->stats_cnt[table_cnt].inserts))
        table_stats->stats_cnt[table_cnt].stale_pct = 0
       ELSEIF ((table_stats->stats_cnt[table_cnt].empty_stats_ind=1))
       ...
       ELSEIF ((table_stats->stats_cnt[table_cnt].num_rows=0)
        ...
        table_stats->stats_cnt[table_cnt].stale_pct = 100
       ELSE
        IF ((table_stats->stats_cnt[table_cnt].mod_trunc_ind=1))
         table_stats->stats_cnt[table_cnt].stale_pct = 100
        ELSE
         table_stats->stats_cnt[table_cnt].stale_pct = least(100,round(((((table_stats->stats_cnt[
           ...
           table_cnt].deletes)/ table_stats->stats_cnt[table_cnt].num_rows) * 100),0))
        ENDIF
       ENDIF
      ENDIF
     ENDIF
   ...
   IF ((table_stats->tbl_cnt=0))
    ...
    RETURN(0)
   ENDIF
   FOR (dr_iter = 1 TO table_stats->tbl_cnt)
     IF ((table_stats->stats_cnt[dr_iter].gather_ind=1))
      SET dm_err->eproc =
      "Getting column names from dm2_user_tab_columns/dm2_user_ind_columns for histograms."
      SELECT INTO "nl:"
       FROM dm2_user_tab_columns dutc,
        dm2_user_ind_columns duic
       WHERE outerjoin(dutc.table_name)=duic.table_name
        AND outerjoin(dutc.column_name)=duic.column_name
        AND (dutc.table_name=table_stats->stats_cnt[dr_iter].tabname)
       ORDER BY dutc.table_name, dutc.column_name
       HEAD REPORT
        index_col_cnt = 0, hist_columns->column_cnt = 0, stat = alterlist(hist_columns->columns,
         hist_columns->column_cnt)
       HEAD dutc.column_name
        hist_columns->column_cnt = (hist_columns->column_cnt+ 1), stat = alterlist(hist_columns->
         columns,hist_columns->column_cnt), hist_columns->columns[hist_columns->column_cnt].
        column_name = dutc.column_name
        IF (trim(duic.index_name)="")
         hist_columns->columns[hist_columns->column_cnt].method_opt = "SIZE 1"
        ELSE
         hist_columns->columns[hist_columns->column_cnt].method_opt = "SIZE 75", index_col_cnt = (
         index_col_cnt+ 1)
        ENDIF
       WITH nocounter, nullreport
      ;end select
      ...
      ENDIF
      IF (index_col_cnt > 0
       AND (hist_columns->column_cnt > 0))
       SET dm_err->eproc = "Concatenating method_opt string for table."
       SELECT INTO "nl:"
        column_name = hist_columns->columns[d.seq].column_name, method_opt = hist_columns->columns[d
        .seq].method_opt
        FROM (dummyt d  WITH seq = value(hist_columns->column_cnt))
        ORDER BY method_opt DESC, column_name
        HEAD REPORT
         table_stats->stats_cnt[dr_iter].method_opt = "~"
        DETAIL
         table_stats->stats_cnt[dr_iter].method_opt = concat(table_stats->stats_cnt[dr_iter].
          method_opt,", ",hist_columns->columns[d.seq].column_name," ",hist_columns->columns[d.seq].
          method_opt)
        FOOT REPORT
         table_stats->stats_cnt[dr_iter].method_opt = replace(table_stats->stats_cnt[dr_iter].
          method_opt,"~","",1), table_stats->stats_cnt[dr_iter].method_opt = replace(table_stats->
          stats_cnt[dr_iter].method_opt,",","",1), table_stats->stats_cnt[dr_iter].method_opt = trim(
          table_stats->stats_cnt[dr_iter].method_opt,2),
         table_stats->stats_cnt[dr_iter].method_opt = concat("FOR COLUMNS ",table_stats->stats_cnt[
          dr_iter].method_opt)
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc)=1)
       ...
       ENDIF
      ELSE
       SET table_stats->stats_cnt[dr_iter].method_opt = "FOR ALL COLUMNS SIZE 1"
      ENDIF
     ENDIF
   ENDFOR
   RETURN(1)
 ...
 SUBROUTINE dri_enable_table_monitoring(null)
   DECLARE de_attempt = i2 WITH protect, noconstant(0)
   DECLARE de_count = i4 WITH protect, noconstant(0)
   ...
   IF ((table_stats->owner IN ("SYS", "SYSTEM")))
    ...
    CALL disp_msg(" ",dm_err->logfile,0)
   ELSE
    FOR (de_count = 1 TO table_stats->tbl_cnt)
      IF ((table_stats->stats_cnt[de_count].monitoring="NO"))
       SET de_attempt = 1
       WHILE ((de_attempt != - (1))
        AND de_attempt <= cnvtint(dm2_db_options->table_monitoring_maxretry))
         IF ((dm_err->debug_flag >= 2))
          SET dm_err->eproc = concat("Enable monitoring for table ",trim(table_stats->stats_cnt[
            de_count].tabname)," attempt ",cnvtstring(de_attempt))
          CALL disp_msg(" ",dm_err->logfile,0)
         ENDIF
         SET dclcom = concat('RDB ASIS("ALTER TABLE ',trim(table_stats->stats_cnt[de_count].tabname),
          ' MONITORING ") go')
         IF (dm2_push_cmd(dclcom,1)=0)
          IF (((findstring("ORA-00054",dm_err->emsg) > 0) OR (findstring("ORA-30006",dm_err->emsg) >
          0)) )
           SET de_attempt = (de_attempt+ 1)
           CALL pause(5)
          ELSE
           SET de_attempt = - (1)
          ENDIF
   ...
   ENDIF
 END ;Subroutine
 DECLARE current_user = vc WITH protect, constant(currdbuser)
 DECLARE db_name = vc WITH protect, constant(cnvtlower(currdblink))
 ...
 DECLARE dr_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dr_err_emsg = vc WITH protect, noconstant(" ")
 DECLARE dr_err_eproc = vc WITH protect, noconstant("")
 DECLARE dr_beg_time = dq8 WITH protect, noconstant(0.0)
 DECLARE dr_beg_tbl_time = dq8 WITH protect, noconstant(0.0)
 DECLARE dr_end_time = dq8 WITH protect, noconstant(0.0)
 DECLARE dr_analy_time = f8 WITH protect, noconstant(0.0)
 DECLARE dr_tbl_time = f8 WITH protect, noconstant(0.0)
 DECLARE dr_sys_user = i2 WITH protect, noconstant(0)
 DECLARE dr_list_stale_report(null) = i2
 IF (check_logfile("dm2_runstats2",".log","DM2_RUNSTATS2 LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dr_err_ind = dm_err->err_ind
 SET dr_err_emsg = dm_err->emsg
 SET dr_err_eproc = dm_err->eproc
 SET dm_err->eproc = "Beginning DM2_RUNSTATS2"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET table_stats->owner = trim(cnvtupper( $1),3)
 SET table_stats->table_name = trim(cnvtupper( $2),3)
 SET table_stats->gather_option = trim(cnvtupper( $3),3)
 SET table_stats->stale_pct_threshold = greatest(0.0,least(100.0,cnvtreal( $4)))
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg =
  "Parameter usage: dm2_runstats2  '<owner>','<table_name>','<gather option>',<stale pct value>"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating <gather_option> input parameter."
 IF ( NOT ((table_stats->gather_option IN ("GATHER_STALE", "GATHER", "LIST_STALE"))))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Valid values are 'GATHER_STALE', 'GATHER', 'LIST_STALE'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ((table_stats->owner IN ("SYS", "SYSTEM")))
  SET dr_sys_user = 1
 ENDIF
 IF (validate(request->batch_selection,"X") != "X"
  AND validate(request->batch_selection,"Y") != "Y")
  SET dm_err->eproc = concat("Called from the OPSView Scheduler...")
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 EXECUTE dm2_set_db_options
 IF ((dm_err->err_ind=1))
  IF ((dm2_db_options->dbstats_gather_method="NOT_SET"))
   SET dm_err->eproc =
   "Unable to determine proper method of gathering database statistics.  Exiting..."
   CALL disp_msg("",dm_err->logfile,0)
   GO TO exit_script
  ENDIF
  IF ((dm2_db_options->table_monitoring="NOT_SET"))
   SET dm2_db_options->table_monitoring = "N"
   SET dm_err->err_ind = 0
   SET dm_err->eproc = "Unable to determine if table monitoring is active.  Defaulting to N."
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
 ENDIF
 IF (dr_sys_user=0)
  IF (dri_fill_table_stats(null)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (currdb="DB2UDB")
  SET dm_err->eproc = "DB2 database detected, connecting to database"
  CALL disp_msg(" ",dm_err->logfile,0)
  SET dclcom = concat("db2 connect to ",db_name," user ",cnvtlower(current_user))
  IF (dm2_push_dcl(dclcom)=0)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "EXECUTING RUNSTATS ON TABLES"
  CALL disp_msg(" ",dm_err->logfile,0)
  FOR (cnt_stuff = 1 TO table_stats->tbl_cnt)
    SET dclcom = concat("db2 -c runstats on table ",build(table_stats->owner),".",build(table_stats->
      stats_cnt[cnt_stuff].tabname)," with distribution and indexes all shrlevel change")
    IF (dm2_push_dcl(dclcom)=0)
     GO TO exit_script
    ENDIF
    SET dm_err->eproc = concat("Stats generated for ",table_stats->stats_cnt[cnt_stuff].tabname,
     " table.")
    CALL disp_msg(" ",dm_err->logfile,0)
  ENDFOR
  CALL dm2_push_dcl("db2 connect reset")
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
 ENDIF
 IF (currdb="ORACLE")
  IF (dr_sys_user=1
   ...
   SET dm_err->eproc = concat("Auto Exit - Statistics for user ",table_stats->owner,
    " only gathered for Oracle 9 or higher")
   CALL disp_msg(" ",dm_err->logfile,0)
   GO TO exit_script
  ENDIF
  IF ((dm2_db_options->table_monitoring="Y"))
   CALL dri_enable_table_monitoring(null)
  ENDIF
  SET dm_err->eproc = "ORACLE database detected, generating database statistics on tables..."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF ((dm2_db_options->dbstats_gather_method="DBMS_STATS"))
   IF (dr_sys_user=1)
    SET table_stats->level = "SYSUSER"
   ELSEIF ((table_stats->table_name=char(42)))
    SET table_stats->level = "SCHEMA"
   ELSE
    SET table_stats->level = "TABLE"
   ENDIF
   IF ((table_stats->level="SYSUSER"))
    IF ((table_stats->gather_option != "GATHER"))
     SET dm_err->eproc = "Auto Exit - GATHER is the only valid gather_option for a SYS/SYSTEM user."
     CALL disp_msg(" ",dm_err->logfile,0)
     GO TO exit_script
    ELSE
     SET dm_err->eproc = concat("Calling dm2_flexstats2 for ",table_stats->owner)
     CALL disp_msg(" ",dm_err->logfile,0)
     SET dr_beg_time = cnvtdatetime(curdate,curtime3)
     EXECUTE dm2_flexstats2 "SYSUSER", value(table_stats->owner), "GATHER",
     100, ""
     SET dr_end_time = cnvtdatetime(curdate,curtime3)
     IF ((dm_err->debug_flag >= 1))
      SET dr_analy_time = datetimediff(dr_end_time,dr_beg_time,4)
      SET dm_err->eproc = build("Stats Gather time for all tables selected = ",dr_analy_time,
       " minutes.")
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     SET dm_err->err_ind = 0
    ENDIF
   ELSE
    IF ((table_stats->gather_option="LIST_STALE"))
     IF (dr_list_stale_report(null)=0)
      GO TO exit_script
     ENDIF
     GO TO exit_script
    ELSE
     SET dr_beg_time = cnvtdatetime(curdate,curtime3)
     FOR (item_count = 1 TO table_stats->tbl_cnt)
       IF ((table_stats->stats_cnt[item_count].gather_ind=1))
        SET dm_err->eproc = build("Calling dm2_flexstats2 'TABLE'",",'",table_stats->stats_cnt[
         item_count].owner,"'",",'",
         table_stats->stats_cnt[item_count].tabname,"'",",",table_stats->stats_cnt[item_count].
         est_pct,",'",
         table_stats->stats_cnt[item_count].method_opt,"'")
        CALL disp_msg(" ",dm_err->logfile,0)
        EXECUTE dm2_flexstats2 "TABLE", value(table_stats->stats_cnt[item_count].owner), value(
         table_stats->stats_cnt[item_count].tabname),
        value(table_stats->stats_cnt[item_count].est_pct), value(table_stats->stats_cnt[item_count].
         method_opt)
        SET dm_err->err_ind = 0
       ENDIF
     ENDFOR
  ...
  ELSE
   FOR (item_count = 1 TO table_stats->tbl_cnt)
     SET dm_err->eproc = concat("Generating database statistics with ANALYZE for Table: ",table_stats
      ->stats_cnt[item_count].tabname)
     CALL disp_msg(" ",dm_err->logfile,0)
     SET dclcom = concat("rdb analyze table ",table_stats->stats_cnt[item_count].tabname,
      " estimate statistics sample 15 percent go")
     IF ((dm_err->debug_flag >= 1))
      SET dr_beg_tbl_time = cnvtdatetime(curdate,curtime3)
      IF (item_count=1)
       SET dr_beg_time = dr_beg_tbl_time
      ENDIF
     ENDIF
     IF (dm2_push_cmd(dclcom,1)=0)
      SET dm_err->err_ind = 0
     ...
     IF ((dm_err->debug_flag >= 1))
      SET dr_end_time = cnvtdatetime(curdate,curtime3)
      SET dr_tbl_time = datetimediff(dr_end_time,dr_beg_tbl_time,5)
      SET dm_err->eproc = build("Stats Gather time for table: ",table_stats->stats_cnt[item_count].
       tabname,"=",dr_tbl_time," seconds.")
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     SET dm_err->eproc = concat("Database Statistic gathering completed using ANALYZE for Table: ",
      table_stats->stats_cnt[item_count].tabname," table.")
     CALL disp_msg(" ",dm_err->logfile,0)
   ENDFOR
   IF ((dm_err->debug_flag >= 1))
    SET dr_analy_time = datetimediff(dr_end_time,dr_beg_time,4)
 ...
 IF (currdb="SQLSRV")
  SET dm_err->eproc = concat(
   "DM2_RUNSTATS2:  SQL Server database detected.  Bypassing statistics processing",
   "  since SQL Server creates and updates statistics automatically.")
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SUBROUTINE dr_list_stale_report(null)
   DECLARE dlsr_report_destination = vc WITH protect, noconstant("")
   SET dlsr_report_destination = concat("dm2_liststale_",format(cnvtdatetime(curdate,curtime3),
     "MMDDYYHHMM;;Q"))
   SET dm_err->eproc = "Outputting list_stale_report data to file from record structure."
   SELECT INTO build(dlsr_report_destination)
    FROM (dummyt d  WITH seq = value(table_stats->tbl_cnt))
    WHERE (table_stats->stats_cnt[d.seq].stale_ind=1)
    ORDER BY table_stats->stats_cnt[d.seq].stale_pct DESC, table_stats->stats_cnt[d.seq].tabname
    HEAD REPORT
     row + 0, col 0, "** LIST_STALE Report **",
     row + 2, col 0,
     "This report will show tables with stale database statistics according to the Stale percentage threshold supplied.",
     row + 2, col 0, "Definitions:",
     row + 1, col 0, "Stale Pct: The percentage of rows (value between 0 and 100) that have",
     row + 1, col 0, "           changed since the last time stats were gathered for the table.",
     row + 2, col 0, "** Criteria specified **",
     row + 1, col 0, "Table Name:",
     col 25, table_stats->table_name, row + 1,
     col 0, "Stale Pct Threshold:", col 25,
     table_stats->stale_pct_threshold";L", row + 1, col 0,
     "Row Sample Size:", col 25, table_stats->rows_to_sample";L",
     row + 2, col 0, "Stale Table List:",
     row + 2, col 31, "Stale",
     row + 1, col 0, "Table Name",
     col 31, "Pct", col 37,
     "Num Rows", col 48, "Table Mods",
     col 59, "Last Analyzed", col 74,
     "Last Mod Dt/Tm"
    DETAIL
     row + 1, col 0, table_stats->stats_cnt[d.seq].tabname";L",
     col 31, table_stats->stats_cnt[d.seq].stale_pct";L"
     IF ((table_stats->stats_cnt[d.seq].empty_stats_ind=1))
      col 37, "No Stats", col 59,
      "No Stats"
     ELSE
      col 37, table_stats->stats_cnt[d.seq].num_rows";L", col 48,
      table_stats->stats_cnt[d.seq].table_mods";L", col 59, table_stats->stats_cnt[d.seq].
      last_analyzed"MM/DD/YY HH:MM;;Q",
      col 74, table_stats->stats_cnt[d.seq].last_mod_dt_tm"MM/DD/YY HH:MM;;Q"
     ENDIF
    FOOT REPORT
     row + 1,
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Reading data from file to CCL displayer."
   FREE DEFINE rtl2
   DEFINE rtl2 build(dlsr_report_destination)
   SELECT INTO mine
    FROM rtl2t t
    DETAIL
     col 0,
...
#exit_script
 ...
 ENDIF
 SET dm_err->eproc = "Ending DM2_RUNSTATS2."
 CALL final_disp_msg("dm2_runstats2")
 SET dm_err->err_ind = dr_err_ind
 SET dm_err->emsg = dr_err_emsg
 SET dm_err->eproc = dr_err_eproc
END GO
