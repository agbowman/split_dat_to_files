CREATE PROGRAM bhs_mak_defic_by_phys_det_lyt
 PROMPT
 ...
 SUBROUTINE fillqualwithfacilitynames(organizations)
  CALL echo(concat("fillqualwithfacilitynames: ",organizations))
 ...
 SUBROUTINE himgetnamesfromtable(data,tablename,name,id)
   ...
   SET jetmp = build2('select into "nl:"'," DATA_NAME = substring(1,200,d.",name,")",",DATA_ID = d.",
    id," "," from ",tablename," d ",
 ...
 SUBROUTINE himrendernodatareport(datasize,outputdevice)
   ...
   ELSE
    ...
    FOR (x = 1 TO size(data->qual,5))
     SET data->qual[x].s_chart_age = cnvthrstodays(data->qual[x].chart_age)
     FOR (y = 1 TO size(data->qual[x].defic_qual,5))
       SET data->qual[x].defic_qual[y].s_defic_age = cnvthrstodays(data->qual[x].defic_qual[y].
        defic_age)
     ENDFOR
    ENDFOR
    SELECT INTO value( $OUTDEV)
     physician_name = substring(1,100,data->qual[d1.seq].physician_name_full_formatted), patient_name
      = substring(1,100,data->qual[d1.seq].patient_name_full_formatted), mrn = substring(1,20,data->
      qual[d1.seq].mrn),
     fin = substring(1,20,data->qual[d1.seq].fin), discharge_date = substring(1,50,trim(format(data->
        qual[d1.seq].disch_dt_tm,"mm/dd/yy;;d"))), deficiency = substring(1,100,data->qual[d1.seq].
      defic_qual[d2.seq].deficiency_name),
     status = substring(1,30,data->qual[d1.seq].defic_qual[d2.seq].status), deficiency_age =
     substring(1,50,data->qual[d1.seq].defic_qual[d2.seq].s_defic_age), chart_age = substring(1,20,
      concat(trim(cnvtstring(data->qual[d1.seq].chart_age))," days")),
     location = substring(1,50,data->qual[d1.seq].location)
     FROM (dummyt d1  WITH seq = value(size(data->qual,5))),
      dummyt d2
     PLAN (d1
      WHERE maxrec(d2,size(data->qual[d1.seq].defic_qual,5)))
      JOIN (d2)
     ORDER BY physician_name, patient_name
 ...
 END ;Subroutine
 DECLARE cnvthrstodays(hours=i4) = vc WITH protect
 SUBROUTINE cnvthrstodays(hours)
   DECLARE ms_tmp = vc WITH protect, noconstant(" ")
   DECLARE ml_hours = i4
   DECLARE ml_days = i4
   IF (hours > 24)
    SET ml_days = (hours/ 24)
    SET ml_hours = mod(hours,24)
    SET ms_tmp = concat(trim(cnvtstring(ml_days))," days ",trim(cnvtstring(ml_hours))," hours")
   ELSE
    SET ml_hours = hours
    SET ms_tmp = concat(trim(cnvtstring(ml_hours))," hours")
   ENDIF
   RETURN(ms_tmp)
 END ;Subroutine
  ...
  SELECT
   ...
   HEAD REPORT
    ...
    allphysicians = uar_i18ngetmessage(i18nhandlehim,"ALLPHYSICIANS","All Physicians"), physicianlist
     = makelistofqualitemnames(physicians,allphysicians), _vcwriteln = build2("<tr>",
     ...
     " class='FieldName072' colspan='12'>",physicianlist,"</td>","</tr>"),
    _htmlfilestat = uar_fwrite(_vcwriteln,1,textlen(_vcwriteln),_htmlfilehandle), pageofpage =
    rpt_pageofpage
 ...
 SUBROUTINE sectionabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   SELECT
    ...
    HEAD REPORT
     ...
     _bcontfieldname07 = 0, _bcontfieldname08 = 0, _fdrawheight = fieldname00(rpt_calcheight)
