CREATE PROGRAM aps_get_rpts_by_case:dba
 ...
 RECORD reply(
   ...
   1 rpt_qual[*]
     ...
     2 cr_updt_cnt = i4
     2 report_comment = vc
     2 report_comment_long_text_id = f8
     2 report_long_text_updt_cnt = i4
     2 blob_bitmap = i4
 ...
 SELECT INTO "nl:"
  join_path = decode(rt.seq,"R",cs.seq,"S",cp.seq,
   "P"," "), t_tag_group_id = decode(t.seq,t.tag_group_id,0.0), t_tag_sequence = decode(t.seq,t
   .tag_sequence,0),
  pc.case_id, p.person_id, rt.report_id,
  cr.report_sequence, crc.catalog_cd, sd.short_description,
  cs.specimen_description, pr1.name_full_formatted
  ...
  ORDER BY prr.reporting_sequence, cr.report_sequence, t_tag_group_id,
   t_tag_sequence
  HEAD REPORT
   stat = alterlist(reply->rpt_qual,max_rpt_cnt), stat = alterlist(reply->spec_qual,max_spec_cnt),
   stat = alterlist(reply->phys_qual,max_phys_cnt),
   reply->case_id = pc.case_id, reply->case_type_cd = pc.case_type_cd, reply->encntr_id = pc
   .encntr_id,
   reply->prefix_cd = pc.prefix_id, reply->accession_nbr = pc.accession_nbr, reply->blob_bitmap = pc
   .blob_bitmap,
   reply->dataset_uid = pc.dataset_uid, reply->resp_pathologist_id = pc.responsible_pathologist_id,
   reply->resp_pathologist_name = pr2.name_full_formatted,
   reply->resp_resident_id = pc.responsible_resident_id, reply->resp_resident_name = pr3
   .name_full_formatted, reply->req_physician_id = pc.requesting_physician_id,
   reply->req_physician_name = pr4.name_full_formatted, reply->case_collect_dt_tm = cnvtdatetime(pc
    .case_collect_dt_tm), reply->case_received_dt_tm = cnvtdatetime(pc.case_received_dt_tm),
   reply->updt_cnt = pc.updt_cnt, reply->person_id = p.person_id, reply->person_name = p
   .name_full_formatted,
   reply->sex_cd = p.sex_cd, reply->age = formatage(p.birth_dt_tm,p.deceased_dt_tm,"CHRONOAGE"),
   reply->birth_dt_tm = cnvtdatetime(p.birth_dt_tm),
   reply->birth_tz = validate(p.birth_tz,0), reply->deceased_dt_tm = cnvtdatetime(p.deceased_dt_tm),
   reply->chr_ind = pc.chr_ind,
   reply->case_comment_long_text_id = pc.comments_long_text_id, reply->loc_facility_cd = e
   .loc_facility_cd, reply->accessioned_dt_tm = pc.accessioned_dt_tm,
   ...
   .imaging_service_resource_cd, service_resource_cd = 0.0
  DETAIL
   CASE (join_path)
    OF "R":
     ...
     IF (isresourceviewable(service_resource_cd)=true)
      ...
      .blob_bitmap, reply->rpt_qual[rpt_cnt].report_sequence = cr.report_sequence,
      reply->rpt_qual[rpt_cnt].hold_cd = rt.hold_cd, reply->rpt_qual[rpt_cnt].
      hold_comment_long_text_id = rt.hold_comment_long_text_id, reply->rpt_qual[rpt_cnt].status_cd =
      cr.status_cd,
      reply->rpt_qual[rpt_cnt].primary_ind = prr.primary_ind, reply->rpt_qual[rpt_cnt].catalog_cd =
      cr.catalog_cd, reply->rpt_qual[rpt_cnt].description = sd.description,
      ...
      ENDIF
      reply->rpt_qual[rpt_cnt].updt_cnt = rt.updt_cnt, reply->rpt_qual[rpt_cnt].cr_updt_cnt = cr
      .updt_cnt, reply->rpt_qual[rpt_cnt].report_comment_long_text_id = rt.comments_long_text_id,
      reply->rpt_qual[rpt_cnt].main_report_complete_dt_tm = cnvtdatetime(pc.main_report_cmplete_dt_tm
       )
      IF ((reply->rpt_qual[rpt_cnt].primary_ind=1))
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   lt_hold_comment_updt_cnt = lt.updt_cnt
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  lt.long_text_id
  ...
  DETAIL
   reply->case_comment = trim(lt.long_text), reply->case_long_text_updt_cnt = lt.updt_cnt
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  lt.long_text_id
  FROM long_text lt,
   (dummyt d1  WITH seq = value(size(reply->rpt_qual,5)))
  PLAN (d1
   WHERE (reply->rpt_qual[d1.seq].report_comment_long_text_id > 0))
   JOIN (lt
   WHERE (reply->rpt_qual[d1.seq].report_comment_long_text_id=lt.long_text_id))
  DETAIL
   reply->rpt_qual[d1.seq].report_comment = trim(lt.long_text), reply->rpt_qual[d1.seq].
   report_long_text_updt_cnt = lt.updt_cnt
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
 ...
 IF ((request->execute_flag > 0)
  ...
  IF ((reply->rpt_qual[1].cyto_report_ind=1))
