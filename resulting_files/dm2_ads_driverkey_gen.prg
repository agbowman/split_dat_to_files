CREATE PROGRAM dm2_ads_driverkey_gen
 ...
 SUBROUTINE ds_scramble(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dadg_logfile_prefix = vc WITH protect, constant("dm2_ads_drv_gen")
 DECLARE dadg_table = vc WITH protect, noconstant("*")
 DECLARE dadg_dpe_id = f8 WITH protect, noconstant(0.0)
 DECLARE dadg_already_running = i2 WITH protect, noconstant(0)
 DECLARE dadg_process_extracts(dpe_config_id=f8,dpe_table=vc) = i2
 DECLARE dadg_process_driverkey_sizing(null) = i2
 SET width = 132
 IF (check_logfile(dadg_logfile_prefix,".log","dm2_ads_driverkey_gen")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_ads_driverkey_gen"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm2_process_rs->process_name = dpl_sample
 IF (dar_check_ads_execution(dadg_already_running)=0)
  GO TO exit_program
 ENDIF
 IF (dadg_already_running=1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "A Sample Process is currently EXECUTING. Exiting process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dads_list->config_id=0))
  ...
  IF (dar_get_config_data(dads_list->config_id)=0)
   GO TO exit_program
  ENDIF
  IF (dar_get_config_dtl(dads_list->config_id,dadg_table)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 CALL drr_insert_runner_row(value(dpl_drivergen_runner),cnvtstring(currdbhandle),cnvtstring(dads_list
   ->config_id),1,0.0)
 SET dadg_dpe_id = dm2_process_event_rs->dm_process_event_id
 IF (dadg_process_extracts(dads_list->config_id,dadg_table)=0)
  GO TO exit_program
 ENDIF
 IF ((validate(dm2_no_ads_runner,- (1))=- (1))
  AND (validate(dm2_no_ads_runner,- (2))=- (2)))
  EXECUTE dm2_ads_runner
 ENDIF
 GO TO exit_program
 SUBROUTINE dadg_process_extracts(dpe_config_id,dpe_table)
   DECLARE dpe_cnt = i4 WITH protect, noconstant(0)
   DECLARE dpe_iter = i4 WITH protect, noconstant(0)
   DECLARE dpe_cur_schema_instance = i4 WITH protect, noconstant(0)
   DECLARE dpe_dace_id = f8 WITH protect, noconstant(0.0)
   DECLARE dpe_found_ind = i2 WITH protect, noconstant(0)
   DECLARE dpe_continue = i2 WITH protect, noconstant(1)
   SET dm_err->eproc = build2("Processing Sample config extracts to generate driver keys for: ",
    dpe_config_id)
   CALL disp_msg("",dm_err->logfile,0)
   IF (dpq_remove_procs(value(dpl_sample),value(dpq_failure),10000)=0)
    RETURN(0)
   ENDIF
   IF (dpq_remove_procs(value(dpl_sample),value(dpq_success),10000)=0)
    RETURN(0)
   ENDIF
   IF (dar_sync_config_status(dpe_config_id)=0)
    RETURN(0)
   ENDIF
   IF ((((dads_list->config_method=dpl_custom)) OR ((dads_list->config_method="CUSTOM-DELETE"))) )
    SELECT DISTINCT INTO "nl:"
     dae.owner_name, dae.table_name, dae.table_extract_nbr
     FROM dm_ads_config_driver d,
      dm_ads_extract dae
     WHERE d.dm_ads_config_id=dpe_config_id
      AND d.dm_ads_extract_id=dae.dm_ads_extract_id
     HEAD REPORT
      dgel_cnt = 0, stat = alterlist(dads_list->extract_qual,dgel_cnt)
     DETAIL
      dgel_cnt = (dgel_cnt+ 1)
      ...
      ENDIF
      dads_list->extract_qual[dgel_cnt].extract_id = dae.dm_ads_extract_id, dads_list->extract_qual[
      dgel_cnt].owner_name = trim(dae.owner_name), dads_list->extract_qual[dgel_cnt].table_name =
      trim(dae.table_name),
      dads_list->extract_qual[dgel_cnt].table_extract_nbr = dae.table_extract_nbr, dads_list->
      extract_qual[dgel_cnt].table_extract_instance_nbr = dae.table_extract_instance_nbr, dads_list->
      extract_qual[dgel_cnt].active_ind = dae.active_ind,
      ...
      updt_task = dae.updt_task, dads_list->extract_qual[dgel_cnt].updt_cnt = dae.updt_cnt,
      dads_list->extract_qual[dgel_cnt].updt_dt_tm = cnvtdatetime(dae.updt_dt_tm), dads_list->
      extract_qual[dgel_cnt].updt_applctx = dae.updt_applctx, dads_list->extract_qual[dgel_cnt].
      status = dpl_ready,
      dads_list->extract_qual[dgel_cnt].full_key = concat(trim(dae.owner_name),trim(dae.table_name),
       cnvtstring(dae.table_extract_nbr)), dads_list->extract_qual[dgel_cnt].row_sample = - (1),
      dads_list->extract_qual[dgel_cnt].dpq_id = 0,
      dads_list->extract_qual[dgel_cnt].table_exists_ind = 0, dads_list->extract_qual[dgel_cnt].
      tgt_sample_method = dae.extract_method, dads_list->extract_qual[dgel_cnt].tgt_sample_pct =
    ...
    ENDIF
   ELSE
    IF (dar_get_extract_list(1,dpe_table,- (1),0)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF ((dads_list->extract_cnt=0))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "No driver table extract information found in dm_ads_extract"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (dar_scr_dads_list(0)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Verifying existance of tables"
   SELECT INTO "nl:"
    FROM dba_tables dt,
     (dummyt d  WITH seq = value(dads_list->extract_cnt))
    PLAN (d
     WHERE d.seq > 0)
     JOIN (dt
     WHERE (dt.owner=dads_list->extract_qual[d.seq].owner_name)
      AND (dt.table_name=dads_list->extract_qual[d.seq].table_name))
    DETAIL
     dads_list->extract_qual[d.seq].table_exists_ind = 1
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
    ...
    RETURN(0)
   ENDIF
   IF ((dads_list->preserve_ind=1))
    SET dm_err->eproc = "Marking completed extracts in dm_ads_config_extract to be skipped"
    CALL disp_msg("",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm_ads_config_extract dace,
      (dummyt d  WITH seq = value(dads_list->extract_cnt))
     PLAN (d
      WHERE d.seq > 0)
      JOIN (dace
      WHERE dace.dm_ads_config_id=dpe_config_id
       AND dace.status_txt=dpl_complete
       AND (dace.dm_ads_extract_id=dads_list->extract_qual[d.seq].extract_id))
     DETAIL
      dads_list->extract_qual[d.seq].skip_ind = 1
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     SET dm_err->err_ind = 1
   ...
   IF ((dads_list->config_method != dpl_custom))
    IF (dadg_process_driverkey_sizing(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   FOR (dpe_iter = 1 TO dads_list->extract_cnt)
     SET dm_err->eproc = build("Current key being processed (<",dpe_iter,"> of <",dads_list->
      extract_cnt,">) - ",
      dads_list->extract_qual[dpe_iter].table_name,";",dads_list->extract_qual[dpe_iter].
      table_extract_nbr)
     CALL disp_msg("",dm_err->logfile,0)
     IF ((dads_list->extract_qual[dpe_iter].skip_ind=0)
      AND (dads_list->extract_qual[dpe_iter].table_exists_ind=1))
      SET dm_err->eproc = build2("Removing old rows in dm_ads_config_extract for: ",dads_list->
       extract_qual[dpe_iter].table_name)
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg("",dm_err->logfile,0)
      ENDIF
      DELETE  FROM dm_ads_config_extract dace
       WHERE dace.dm_ads_config_id=dpe_config_id
        AND (dace.dm_ads_extract_id=dads_list->extract_qual[dpe_iter].extract_id)
       WITH nocounter
      ;end delete
      IF (check_error(dm_err->eproc)=1)
       SET dm_err->err_ind = 1
      ...
      ENDIF
      COMMIT
      SET dm_err->eproc = build2("Removing old rows in dm_ads_config_driver for: ",dads_list->
       extract_qual[dpe_iter].table_name)
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg("",dm_err->logfile,0)
      ENDIF
      SET dpe_continue = 1
      WHILE (dpe_continue=1)
        DELETE  FROM dm_ads_config_driver dacd
         WHERE dacd.dm_ads_config_id=dpe_config_id
          AND (dacd.dm_ads_extract_id=dads_list->extract_qual[dpe_iter].extract_id)
      ...
      SELECT INTO "nl:"
       ...
       FROM dual
       DETAIL
        dpe_dace_id = cnvtreal(x)
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc))
       RETURN(0)
      ENDIF
      SET dm_err->eproc = build2("Inserting config rows into dm_ads_config_extract for: ",
       dpe_config_id)
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg("",dm_err->logfile,0)
      ENDIF
      INSERT  FROM dm_ads_config_extract dace
       SET dace.dm_ads_config_extract_id = dpe_dace_id, dace.dm_ads_config_id = dads_list->config_id,
        dace.dm_ads_extract_id = dads_list->extract_qual[dpe_iter].extract_id,
        dace.status_txt = dads_list->extract_qual[dpe_iter].status, dace.target_sample_method =
        dads_list->extract_qual[dpe_iter].tgt_sample_method, dace.target_sample_percent_nbr =
        dads_list->extract_qual[dpe_iter].tgt_sample_pct,
        dace.row_sample_nbr = dads_list->extract_qual[dpe_iter].row_sample, dace.message_txt =
        dads_list->extract_qual[dpe_iter].message, dace.dm_process_queue_id = 0.0,
        dace.begin_dt_tm = cnvtdatetime(dads_list->extract_qual[dpe_iter].begin_dt_tm), dace
        .end_dt_tm = cnvtdatetime(dads_list->extract_qual[dpe_iter].end_dt_tm), dace.updt_dt_tm =
        cnvtdatetime(curdate,curtime3),
        dace.updt_id = 0, dace.updt_task = 0, dace.updt_applctx = 0,
        dace.updt_cnt = 0
       WITH nocounter
      ;end insert
      IF (check_error(dm_err->eproc)=1)
      ...
      ENDIF
      COMMIT
      IF ((dads_list->extract_qual[dpe_iter].status=dpl_ready))
       SET dpq_process_queue->process_type = dpl_sample
       SET dpq_process_queue->op_type = dpl_driverkeygen
       SET dpq_process_queue->owner_name = dads_list->extract_qual[dpe_iter].owner_name
       SET dpq_process_queue->object_type = dpq_table
       SET dpq_process_queue->op_method = dpq_db
       SET dpq_process_queue->object_name = trim(cnvtstring(dpe_dace_id))
       SET dpq_process_queue->priority = 200
       SET dpq_process_queue->operation_txt = concat("EXECUTE DM2_ADS_DRIVERKEY_BUILD ",build(
         dpe_dace_id)," GO")
       IF (dpq_process_queue_row(null)=0)
        RETURN(0)
       ENDIF
       SET dm_err->eproc = "Updating config row in dm_ads_config_extract with dm_process_queue_id"
       IF ((dm_err->debug_flag > 0))
        CALL disp_msg("",dm_err->logfile,0)
       ENDIF
       UPDATE  FROM dm_ads_config_extract dace
        SET dace.dm_process_queue_id = dpq_process_queue->dm_process_queue_id
        WHERE dace.dm_ads_config_extract_id=dpe_dace_id
        WITH nocounter
       ;end update
       IF (check_error(dm_err->eproc)=1)
        SET dm_err->err_ind = 1
     ...
     ENDIF
   ENDFOR
   SET dm_err->eproc = "Removing rows from dm_process_queue for extracts that no longer exist"
   CALL disp_msg("",dm_err->logfile,0)
   DELETE  FROM dm_process_queue dpq
    WHERE dpq.process_type=dpl_sample
     AND  NOT ( EXISTS (
    (SELECT
     dace.dm_process_queue_id
     FROM dm_ads_config_extract dace
     WHERE dpq.dm_process_queue_id=dace.dm_process_queue_id)))
    WITH nocounter
   ;end delete
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dadg_process_driverkey_sizing(null)
   DECLARE dpds_tab_ts = vc WITH protect, noconstant("DM2NOTSET")
   DECLARE dpds_ind1_ts = vc WITH protect, noconstant("DM2NOTSET")
   DECLARE dpds_ind2_ts = vc WITH protect, noconstant("DM2NOTSET")
   DECLARE dpds_continue = i2 WITH protect, noconstant(1)
   DECLARE dpds_tab_row_size = i4 WITH protect, constant(70)
   DECLARE dpds_ind1_row_size = i4 WITH protect, constant(18)
   DECLARE dpds_ind2_row_size = i4 WITH protect, constant(34)
   DECLARE dpds_total_tab_size = f8 WITH protect, noconstant(0.0)
   DECLARE dpds_total_ind1_size = f8 WITH protect, noconstant(0.0)
   DECLARE dpds_total_ind2_size = f8 WITH protect, noconstant(0.0)
   DECLARE dpds_tabts_free_space = f8 WITH protect, noconstant(0.0)
   DECLARE dpds_ind1ts_free_space = f8 WITH protect, noconstant(0.0)
   DECLARE dpds_ind2ts_free_space = f8 WITH protect, noconstant(0.0)
   DECLARE dpds_tmp = f8 WITH protect, noconstant(0.0)
   FREE RECORD dadg_size_info
   RECORD dadg_size_info(
     1 cnt = i4
     1 qual[*]
       2 table_name = vc
       2 row_size = i4
       2 num_rows = f8
       2 sample_num_rows = f8
       2 tab_size = f8
       2 ind1_size = f8
       2 ind2_size = f8
   )
   SET dm_err->eproc = "Processing driver tables to obtain space information"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    ...
    PLAN (d
     WHERE d.seq > 0
      AND (dads_list->extract_qual[d.seq].skip_ind=0)
      AND (dads_list->extract_qual[d.seq].table_exists_ind=1)
      AND (dads_list->extract_qual[d.seq].driver_table_ind=1))
     JOIN (dt
     WHERE (dt.table_name=dads_list->extract_qual[d.seq].table_name))
    HEAD REPORT
     dpds_total_tab_size = 0, dpds_total_ind1_size = 0, dpds_total_ind2_size = 0,
     dadg_size_info->cnt = 0, stat = alterlist(dadg_size_info->qual,dadg_size_info->cnt)
    DETAIL
     dadg_size_info->cnt = (dadg_size_info->cnt+ 1), stat = alterlist(dadg_size_info->qual,
      dadg_size_info->cnt), dadg_size_info->qual[dadg_size_info->cnt].table_name = dads_list->
     extract_qual[d.seq].table_name,
     dadg_size_info->qual[dadg_size_info->cnt].num_rows = dt.num_rows, dadg_size_info->qual[
     dadg_size_info->cnt].sample_num_rows = ceil(((dads_list->config_pct * dadg_size_info->qual[
      dadg_size_info->cnt].num_rows)/ 100)), dadg_size_info->qual[dadg_size_info->cnt].tab_size =
     cnvtreal((dadg_size_info->qual[dadg_size_info->cnt].sample_num_rows * dpds_tab_row_size)),
     dadg_size_info->qual[dadg_size_info->cnt].ind1_size = cnvtreal((dadg_size_info->qual[
      dadg_size_info->cnt].sample_num_rows * dpds_ind1_row_size)), dadg_size_info->qual[
     dadg_size_info->cnt].ind2_size = cnvtreal((dadg_size_info->qual[dadg_size_info->cnt].
      sample_num_rows * dpds_ind2_row_size)), dpds_total_tab_size = (dpds_total_tab_size+
     dadg_size_info->qual[dadg_size_info->cnt].tab_size),
     dpds_total_ind1_size = (dpds_total_ind1_size+ dadg_size_info->qual[dadg_size_info->cnt].
     ind1_size), dpds_total_ind2_size = (dpds_total_ind2_size+ dadg_size_info->qual[dadg_size_info->
     cnt].ind2_size)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dadg_size_info)
    CALL echo(build("dpds_total_tab_size: ",dpds_total_tab_size))
    CALL echo(build("dpds_total_ind1_size: ",dpds_total_ind1_size))
    CALL echo(build("dpds_total_ind2_size: ",dpds_total_ind2_size))
   ENDIF
   SET dm_err->eproc = "Retrieving data and index tablespaces for dm_ads_config_driver"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_tables dt
    WHERE dt.table_name="DM_ADS_CONFIG_DRIVER"
    DETAIL
     dpds_tab_ts = dt.tablespace_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET dm_err->err_ind = 1
   ...
   SELECT INTO "nl:"
    FROM dba_indexes di
    WHERE di.index_name IN ("XPKDM_ADS_CONFIG_DRIVER", "XAK1DM_ADS_CONFIG_DRIVER")
    DETAIL
     IF (di.index_name="XPKDM_ADS_CONFIG_DRIVER")
      dpds_ind1_ts = di.tablespace_name
     ELSE
      dpds_ind2_ts = di.tablespace_name
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   IF (dpds_ind1_ts=dpds_ind2_ts)
    SET dpds_total_ind1_size = (dpds_total_ind1_size+ dpds_total_ind2_size)
   ENDIF
   SET dm_err->eproc = "Calculating free space in tablespaces"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_free_space dfs
    WHERE ((dfs.tablespace_name=dpds_tab_ts) OR (((dfs.tablespace_name=dpds_ind1_ts) OR (dfs
    .tablespace_name=dpds_ind2_ts)) ))
    DETAIL
     CASE (dfs.tablespace_name)
      OF value(dpds_tab_ts):
       dpds_tabts_free_space = (dpds_tabts_free_space+ dfs.bytes)
      OF value(dpds_ind1_ts):
       dpds_ind1ts_free_space = (dpds_ind1ts_free_space+ dfs.bytes)
      OF value(dpds_ind2_ts):
       dpds_ind2ts_free_space = (dpds_ind2ts_free_space+ dfs.bytes)
     ENDCASE
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
   SET message = window
   IF (dar_common_menu(2,"TABLESPACE SIZING")=0)
    RETURN(0)
   ENDIF
   CALL text(10,5,"NOTE: This report is dependent on database statistics.")
   CALL text(12,5,"The Driverkey generation process will add data to the following tablespaces")
   IF (dpds_ind1_ts != dpds_ind2_ts)
    CALL text(13,5,concat(dpds_tab_ts,", ",dpds_ind1_ts,", ",dpds_ind2_ts))
   ELSE
    CALL text(13,5,concat(dpds_tab_ts,", ",dpds_ind1_ts))
   ENDIF
   CALL text(15,5,"Please verify the space requirements before continuing")
   CALL text(17,5,"TABLESPACE NAME")
   CALL text(17,40,"FREE SPACE (MB)")
   CALL text(17,60,"SPACE NEEDED (MB)")
   CALL text(18,5,fillstring(75,"-"))
   CALL text(19,5,build(dpds_tab_ts))
   CALL text(19,40,build(((dpds_tabts_free_space/ 1024.0)/ 1024.0)))
   CALL text(19,60,build(((dpds_total_tab_size/ 1024.0)/ 1024.0)))
   CALL text(20,5,build(dpds_ind1_ts))
   CALL text(20,40,build(((dpds_ind1ts_free_space/ 1024.0)/ 1024.0)))
   CALL text(20,60,build(((dpds_total_ind1_size/ 1024.0)/ 1024.0)))
   IF (dpds_ind1_ts != dpds_ind2_ts)
    CALL text(21,5,build(dpds_ind2_ts))
    CALL text(21,40,build(((dpds_ind2ts_free_space/ 1024.0)/ 1024.0)))
    CALL text(21,60,build(((dpds_total_ind2_size/ 1024.0)/ 1024.0)))
   ENDIF
   CALL text(23,5,"Enter C to continue or Q to quit: ")
   CALL accept(23,40,"P;CU","C"
    WHERE curaccept IN ("C", "Q"))
   SET message = nowindow
   IF (curaccept="Q")
    RETURN(0)
 ...
 END ;Subroutine
#exit_program
 SET dir_ui_misc->dm_process_event_id = dadg_dpe_id
 SET dm2_process_event_rs->dm_process_event_id = dadg_dpe_id
 CALL drr_remove_runner_row(dpl_drivergen_runner,cnvtstring(currdbhandle))
 CALL dar_sync_config_status(dads_list->config_id)
 CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 SET dm_err->eproc = "Ending dm2_ads_driverkey_gen"
 CALL final_disp_msg(dadg_logfile_prefix)
END GO
