CREATE PROGRAM cp_upd_node_bhvr:dba
 PROMPT
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "cp_upd_node_bhvr"
 DECLARE verifyinputs(null) = null WITH protect
 DECLARE updaterows(null) = i4 WITH protect
 ...
 SUBROUTINE verifyinputs(null)
   ...
   FOR (x = 1 TO nlistsize)
     ...
     IF (size(trim(request->qual[x].reaction_entity_name,3),1)=0)
      ...
      GO TO exit_script
     ENDIF
     IF ((request->qual[x].reaction_entity_id=0))
      SET reply->status_data.status = "F"
      SET reply->status_data.subeventstatus.operationname = "PERFORM"
      SET reply->status_data.subeventstatus.operationstatus = "F"
      SET reply->status_data.subeventstatus.targetobjectname = "Validate"
      SET reply->status_data.subeventstatus.targetobjectvalue = build2(
       "REACTION_ENTITY_ID for list seq ",trim(cnvtstring(x),3)," must be non-zero.")
      GO TO exit_script
     ENDIF
 ...
 SUBROUTINE updaterows(null)
   CALL log_message("In updateRows()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH private, constant(curtime3)
   ...
   FOR (x = 1 TO xsize)
     ...
     ENDIF
     UPDATE  FROM cp_node_behavior cnb
      SET cnb.cp_node_id = request->qual[x].cp_node_id, cnb.reaction_entity_id = request->qual[x].
       reaction_entity_id, cnb.reaction_entity_name = trim(request->qual[x].reaction_entity_name),
       cnb.response_ident = trim(cur_response_ident), cnb.long_response_ident = trim(
        cur_long_response_ident), cnb.reaction_type_mean = trim(request->qual[x].reaction_type_mean),
       cnb.updt_applctx = reqinfo->updt_applctx, cnb.updt_cnt = (cnb.updt_cnt+ 1), cnb.updt_dt_tm =
       cnvtdatetime(sysdate),
       cnb.updt_id = reqinfo->updt_id, cnb.updt_task = reqinfo->updt_task
      WHERE (cnb.cp_node_behavior_id=request->qual[x].cp_node_behavior_id)
      WITH nocounter
     ;end update
     SET lstat = error_and_zero_check_rec(1,"INSERT","ERROR INSERTING INTO CP_NODE_BEHAVIOR",1,1,
      reply)
     SET reply->qual[x].cp_node_behavior_id = request->qual[x].cp_node_behavior_id
     SET reply->qual[x].reaction_entity_name = request->qual[x].reaction_entity_name
     SET reply->qual[x].reaction_entity_id = request->qual[x].reaction_entity_id
     SET reply->qual[x].response_ident = request->qual[x].response_ident
     SET reply->qual[x].reaction_type_mean = request->qual[x].reaction_type_mean
     SET reply->qual[x].cp_node_id = request->qual[x].cp_node_id
   ENDFOR
   CALL log_message(build("Exit updateRows(), Elapsed time in seconds:",((curtime3 - begin_curtime3)
     / 100.0)),log_level_debug)
 END ;Subroutine
#set_return
