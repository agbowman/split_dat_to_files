CREATE PROGRAM dm_rdm_upd_erx_synonym_ids_p2:dba
 ...
 DECLARE erx_type = i4 WITH protect, constant(1)
 SET ms_info_domain_nm = "DM_RDM_UPD_ERX_SYNONYM_IDS"
 SET ms_range = "FINAL RANGE"
 SET ms_max_name_p = "MAX IB_RX_REQ_DRUG_ID EVALUATED"
 SET ms_child_script = "dm_rdm_upd_erx_synonym_ids_cc"
 SET mn_num_children = 5
 SELECT INTO "nl:"
  max_id = max(irrd.ib_rx_req_drug_id)
  FROM ib_rx_req_drug irrd,
   ib_rx_req irr
  PLAN (irrd
   WHERE irrd.synonym_id=0)
   JOIN (irr
   WHERE irr.ib_rx_req_id=irrd.ib_rx_req_id
    AND irr.req_type_flag=erx_type)
  DETAIL
...
#exit_program
 ...
 ELSE
  COMMIT
  CALL sbr_add_dropped_object("INDEX","XPKTEMP_IB_RX_REQ_DRUG","Dropping index")
  CALL sbr_add_dropped_object("TABLE","TEMP_IB_RX_REQ_DRUG","Dropping table")
