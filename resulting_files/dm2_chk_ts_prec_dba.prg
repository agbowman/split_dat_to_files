CREATE PROGRAM dm2_chk_ts_prec:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE ctp_logfile_prefix = vc WITH protect, constant("dm2_chk_ts_prec")
 DECLARE ctp_pkg_num = i4 WITH protect
 DECLARE ctp_table_size = i4 WITH protect, noconstant(0)
 DECLARE ctp_dyn_where = vc WITH protect, noconstant(" ")
 DECLARE ctp_tbl_idx = i4 WITH protect, noconstant(0)
 DECLARE ctp_pre_idx = i4 WITH protect, noconstant(0)
 IF (check_logfile(ctp_logfile_prefix,".log","DM2_CHK_TS_PREC LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning maintenance check on DM_TS_PRECEDENCE...."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET ctp_pkg_num =  $1
 ...
 IF (validate(ts_prec->qual[1].table_name,"@")="@")
  ...
  SET dm_err->eproc = "Confirming record structure [ts_prec] existence."
 ...
 IF (validate(ts_prec_maint->qual[1].maint_type,"@")="@")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Record structure required for this process is not established."
  SET dm_err->eproc = "Confirming record structure [ts_prec_maint] existence."
  SET dm_err->user_action = "Please establish required record structure."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET ctp_table_size = size(ts_prec->qual,5)
 SET stat = alterlist(ts_prec_maint->qual,ctp_table_size)
 IF (cnvtupper(currdb) IN ("DB2UDB", "SQLSRV"))
  SET ctp_dyn_where = "at.table_name = td.suffixed_table_name"
 ELSE
  SET ctp_dyn_where = "at.table_name = td.full_table_name"
 ENDIF
 IF (dm2_set_autocommit(1)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Determining maintenance type on DM_TS_PRECEDENCE."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET ts_prec_maint->num_of_inserts = 0
 SET ts_prec_maint->num_of_updates = 0
 SET ts_prec_maint->num_of_none = 0
 FOR (ctp_tbl_idx = 1 TO ctp_table_size)
  SET ts_prec_maint->qual[ctp_tbl_idx].maint_type = "I"
  SET ts_prec_maint->num_of_inserts = (ts_prec_maint->num_of_inserts+ 1)
 ENDFOR
 SELECT
  IF (ctp_pkg_num > 0)
   FROM dm_ts_precedence tp,
    dm_afd_tables at,
    dm_tables_doc td
   WHERE at.alpha_feature_nbr=ctp_pkg_num
    AND at.owner=currdbuser
    AND parser(ctp_dyn_where)
    AND td.table_name=tp.table_name
  ...
  HEAD tp.table_name
   ctp_tbl_idx = 0, ctp_tbl_idx = locateval(ctp_tbl_idx,1,ctp_table_size,tp.table_name,ts_prec->qual[
    ctp_tbl_idx].table_name)
  DETAIL
   IF (ctp_tbl_idx > 0)
    ctp_pre_idx = 0, ctp_pre_idx = locateval(ctp_pre_idx,ctp_tbl_idx,ctp_table_size,tp.precedence,
     ts_prec->qual[ctp_pre_idx].precedence)
    IF (ctp_pre_idx > 0)
     IF ((ts_prec->qual[ctp_pre_idx].table_name=ts_prec->qual[ctp_tbl_idx].table_name))
      IF ((ts_prec->qual[ctp_pre_idx].updt_cnt >= tp.updt_cnt))
       ts_prec_maint->qual[ctp_pre_idx].maint_type = "U", ts_prec_maint->num_of_updates = (
       ts_prec_maint->num_of_updates+ 1), ts_prec_maint->num_of_inserts = (ts_prec_maint->
       num_of_inserts - 1)
      ELSE
       ts_prec_maint->qual[ctp_pre_idx].maint_type = "N", ts_prec_maint->num_of_none = (ts_prec_maint
       ->num_of_none+ 1), ts_prec_maint->num_of_inserts = (ts_prec_maint->num_of_inserts - 1)
      ENDIF
     ENDIF
    ENDIF
   ENDIF
 ...
 ELSEIF (validate(inhouse_reply->table_name,"@")="@"
  ...
  AND curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No Table/Precedence Informatin Found"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = concat("Maintenance to DM_TS_PRECEDENCE includes: ",cnvtstring(ts_prec_maint->
   num_of_inserts,9,0)," Inserts  ",cnvtstring(ts_prec_maint->num_of_updates,9,0)," Updates  ",
  cnvtstring(ts_prec_maint->num_of_none,9,0)," w/ no changes.")
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dm2_set_autocommit(0)=0)
  GO TO exit_program
 ENDIF
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Maintenance check on DM_TS_PRECEDENCE Complete"
 CALL final_disp_msg(ctp_logfile_prefix)
END GO
