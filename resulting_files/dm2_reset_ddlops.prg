CREATE PROGRAM dm2_reset_ddlops
 ...
 ENDIF
 DECLARE drd_ddlops_col_cnt = i2 WITH protect, noconstant(0)
 DECLARE drd_tspace_size1_col_cnt = i2 WITH protect, noconstant(0)
 DECLARE drd_remove_file(drf_file_in=vc) = i2
 DECLARE drd_remove_table_and_view(drtv_table_in=vc,drtv_view_in=vc) = i2
 SET readme_data->status = "F"
 IF (check_logfile("dm2_reset_ddlops",".log","dm2_reset_ddlops_log_file...") != 1)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Unable to Create Log File",dm_err->emsg)
  GO TO exit_program
 ENDIF
 SET readme_data->message = "Checking if inhouse domain"
 ...
 IF ((inhouse_misc->inhouse_domain=0))
  ...
  SET readme_data->message = "Auto-Success. Only run for inhouse domains."
  SET dm_err->eproc = readme_data->message
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET readme_data->message = "Check column count for DM2_DDL_OPS_LOG1."
 SET dm_err->eproc = readme_data->message
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm2_user_tab_columns
  WHERE table_name="DM2_DDL_OPS_LOG1"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat(readme_data->message,dm_err->emsg)
  GO TO exit_program
 ELSE
  SET drd_ddlops_col_cnt = curqual
  IF ((dm_err->debug_flag > 0))
   CALL echo(concat("DM2_DDL_OPS_LOG1 column count:",trim(cnvtstring(drd_ddlops_col_cnt))))
  ENDIF
 ENDIF
 SET readme_data->message = "Check column count for DM2_TSPACE_SIZE1."
 SET dm_err->eproc = readme_data->message
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm2_user_tab_columns
  WHERE table_name="DM2_TSPACE_SIZE1"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  SET readme_data->status = "F"
  ...
  GO TO exit_program
 ELSE
  SET drd_tspace_size1_col_cnt = curqual
  IF ((dm_err->debug_flag > 0))
   CALL echo(concat("DM2_TSPACE_SIZE1 column count:",trim(cnvtstring(drd_tspace_size1_col_cnt))))
  ENDIF
 ENDIF
 IF (((drd_ddlops_col_cnt != 25) OR (drd_tspace_size1_col_cnt != 11)) )
  SET dm_err->eproc = "Column counts are different, performing cleanup of flat files."
  SET readme_data->message = dm_err->eproc
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  CASE (cursys)
   OF "AIX":
    IF (drd_remove_file(build(dm2_install_schema->ccluserdir,"dm2_ddlops.dat"))=0)
     SET readme_data->status = "F"
     GO TO exit_program
    ENDIF
    IF (drd_remove_file(build(dm2_install_schema->ccluserdir,"dm2_ddlops.idx"))=0)
     SET readme_data->status = "F"
     GO TO exit_program
    ENDIF
    IF (drd_remove_file(build(dm2_install_schema->ccluserdir,"dm2_ddlops_aix.dat"))=0)
     SET readme_data->status = "F"
     GO TO exit_program
    ENDIF
    IF (drd_remove_file(build(dm2_install_schema->ccluserdir,"dm2_ddlops_aix.idx"))=0)
     SET readme_data->status = "F"
     GO TO exit_program
    ENDIF
   OF "AXP":
    IF (drd_remove_file(build(dm2_install_schema->ccluserdir,"dm2_ddlops.dat"))=0)
     ...
     GO TO exit_program
    ENDIF
    IF (drd_remove_file(build(dm2_install_schema->ccluserdir,"dm2_ddlops_vms.dat"))=0)
     SET readme_data->status = "F"
     GO TO exit_program
    ENDIF
  ENDCASE
 ELSE
  SET readme_data->status = "S"
  SET dm_err->eproc = "No changes were required. Readme successful."
  SET readme_data->message = dm_err->eproc
  GO TO exit_program
 ENDIF
 IF (drd_ddlops_col_cnt != 25)
  IF (drd_remove_table_and_view("DM2_DDL_OPS_LOG1","DM2_DDL_OPS_LOG")=0)
   SET readme_data->status = "F"
   GO TO exit_program
  ENDIF
 ENDIF
 IF (drd_tspace_size1_col_cnt != 11)
  IF (drd_remove_table_and_view("DM2_TSPACE_SIZE1","DM2_TSPACE_SIZE")=0)
   SET readme_data->status = "F"
   GO TO exit_program
  ENDIF
 ENDIF
 SET readme_data->message = "Cleanup completed. Readme Successful"
 SET dm_err->eproc = readme_data->message
 CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 SET readme_data->status = "S"
 GO TO exit_program
 SUBROUTINE drd_remove_file(drf_file_in)
   DECLARE drf_removed_ind = i2 WITH protect, noconstant(0)
   SET dm_err->eproc = concat("Finding:",drf_file_in)
   SET readme_data->message = dm_err->eproc
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   IF (findfile(drf_file_in)=1)
    SET dm_err->eproc = concat("Removing:",drf_file_in)
    SET readme_data->message = dm_err->eproc
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    CASE (cursys)
     OF "AIX":
      CALL dm2_push_dcl(concat("rm ",drf_file_in))
     OF "AXP":
      CALL dm2_push_dcl(concat("delete ",drf_file_in,";*"))
    ENDCASE
    IF ((dm_err->err_ind=1))
     SET readme_data->status = "F"
     SET readme_data->message = concat(readme_data->message,dm_err->emsg)
     RETURN(0)
    ENDIF
    SET drf_removed_ind = 1
   ENDIF
   IF (drf_removed_ind=1)
    SET dm_err->eproc = concat("Verify removal of:",drf_file_in)
    SET readme_data->message = dm_err->eproc
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    IF (findfile(drf_file_in)=1)
     SET dm_err->eproc = concat("Failed to remove file.",drf_file_in," still exists.")
     SET readme_data->message = dm_err->eproc
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drd_remove_table_and_view(drtv_table_in,drtv_view_in)
   DECLARE drtv_remove_table_ind = i2 WITH protect, noconstant(0)
   DECLARE drtv_remove_view_ind = i2 WITH protect, noconstant(0)
   SET dm_err->eproc = concat("Checking if view:",drtv_view_in," exists.")
   SET readme_data->message = dm_err->eproc
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_user_views
    WHERE view_name=cnvtupper(drtv_view_in)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    RETURN(0)
   ENDIF
   IF (curqual=1)
    SET dm_err->eproc = concat("View:",drtv_view_in," exists, dropping view.")
    SET readme_data->message = dm_err->eproc
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    IF (dm2_push_cmd(concat("rdb drop view ",drtv_view_in," go"),1)=0)
     RETURN(0)
    ENDIF
    SET drtv_remove_view_ind = 1
   ENDIF
   SET dm_err->eproc = concat("Checking if table:",drtv_table_in," exists.")
   SET readme_data->message = dm_err->eproc
   CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_user_tables
    WHERE table_name=cnvtupper(drtv_table_in)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    RETURN(0)
   ENDIF
   IF (curqual=1)
    SET dm_err->eproc = concat("Table:",drtv_table_in," exists, dropping table.")
    SET readme_data->message = dm_err->eproc
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    IF (dm2_push_cmd(concat("rdb drop table ",drtv_table_in," go"),1)=0)
     RETURN(0)
    ENDIF
    SET drtv_remove_table_ind = 1
   ENDIF
   IF (drtv_remove_view_ind=1)
    SET dm_err->eproc = concat("Verify dropping of view:",drtv_view_in)
    SET readme_data->message = dm_err->eproc
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm2_user_views
    ...
    ENDIF
    IF (curqual=1)
     SET dm_err->eproc = concat("Failed to drop view.",drtv_view_in,".")
     SET readme_data->message = dm_err->eproc
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   IF (drtv_remove_table_ind=1)
    SET dm_err->eproc = concat("Verify dropping of table:",drtv_table_in)
    SET readme_data->message = dm_err->eproc
    CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm2_user_tables
    ...
    ENDIF
    IF (curqual=1)
     SET dm_err->eproc = concat("Failed to drop table.",drtv_view_in,".")
     SET readme_data->message = dm_err->eproc
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
