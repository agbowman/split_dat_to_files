CREATE PROGRAM dm2_rdm_load_drr_plan:dba
 ...
 SUBROUTINE dm2_get_program_stack(null)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_rdm_load_drr_plan",".log","dm2_rdm_load_drr_plan")=0)
  GO TO exit_program
 ENDIF
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: Starting script DM2_RDM_LOAD_DRR_PLAN..."
 SET dm_err->eproc = "Starting dm2_rdm_load_drr_plan"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE drldg_tbl = vc WITH protect, noconstant("")
 DECLARE drldg_req_size = i4 WITH protect, noconstant(0)
 DECLARE drldp_cnt = i4 WITH protect, noconstant(0)
 SET drldg_tbl = "DRR_PLAN"
 SET drldg_req_size = size(requestin->list_0,5)
 SET dm_err->eproc = concat("Load csv data into table ",drldg_tbl)
 CALL echorecord(requestin)
 FOR (drldp_cnt = 1 TO drldg_req_size)
  EXECUTE drr_load_cust_extsvc requestin->list_0[drldp_cnt].service_key, requestin->list_0[drldp_cnt]
  .process, requestin->list_0[drldp_cnt].task,
  requestin->list_0[drldp_cnt].task_detail, cnvtint(requestin->list_0[drldp_cnt].active_ind)
  IF ((readme_data->status="F"))
   GO TO exit_script
  ENDIF
 ENDFOR
 SET readme_data->status = "S"
 SET readme_data->message = "Success: Script processed successfully"
 COMMIT
#exit_script
 IF ((dm_err->err_ind=1))
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to insert into ",drldg_tbl," : ",dm_err->emsg)
 ELSE
  SET readme_data->status = "S"
  SET readme_data->message = concat("Success: ",drldg_tbl," batch data loaded successfully.")
  COMMIT
 ENDIF
END GO
