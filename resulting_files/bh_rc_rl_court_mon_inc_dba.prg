CREATE PROGRAM bh_rc_rl_court_mon_inc:dba
 CALL echo("***** bh_rc_rl_court_mon_inc.prg - 677918 *****")
 ...
 SUBROUTINE filltemprec(null)
   FOR (lchgcnt = 1 TO size(request->integer_params,5))
     CASE (cnvtupper(trim(request->integer_params[lchgcnt].key_txt,3)))
      OF "COURT_OBLIG_MONTH":
       SET tmprec->court_oblig_annual = (request->integer_params[lchgcnt].integer_value * 12)
     ENDCASE
   ENDFOR
 END ;Subroutine
 DECLARE createreply(null) = null
 SUBROUTINE createreply(null)
  CALL addtoreplyintegers("COURT_OBLIG_ANNUAL",tmprec->court_oblig_annual)
  CALL echorecord(tmprec)
 END ;Subroutine
 FREE RECORD tmprec
 RECORD tmprec(
   1 court_oblig_annual = i4
 )
