CREATE PROGRAM dm2_test_common_routines2:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 SET dm_err->debug_flag = 2
 SET tst_return_val = 0
 CALL echo("* TEST 1 - Should see the log file get created successfully / return should = 1 *")
 SET dm_err->eproc = "Testing check_logfile subroutine"
 SET tst_return_val = check_logfile("Test_routines",".log","Test Common Routines Logfile")
 CALL echo(build("return value = ",tst_return_val))
 CALL echo("  ")
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm_err->user_action = "NONE"
 CALL echo("* TEST 14 -  *")
 SET dm_err->eproc = "Testing dm2_setup_dbase_env for DM_SEQ."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm2_install_schema->v500_p_word = "v500"
 SET dm2_install_schema->v500_connect_str = "cerner_bds1"
 SET dm2_install_schema->target_dbase_name = "test"
 SET dm2_install_schema->target_env_name = logical("environment")
 SET tst_return_val = dm2_setup_dbase_env(null)
 CALL echo(build("return value = ",tst_return_val))
 SELECT INTO noforms
  d.environment_name, d.environment_id
