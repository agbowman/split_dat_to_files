CREATE PROGRAM al_bhs_scan_compliance:dba
 PROMPT
 ...
 DECLARE mf_cs220_bmcinptpsych_cd = f8 WITH protect, noconstant(0)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=220
    ...
    AND cv.display_key="BMCINPTPSYCH")
  DETAIL
   mf_cs220_bmcinptpsych_cd = cv.code_value
  WITH nocounter
 ;end select
 IF (mc_any_status="C")
  SET mn_nall = 1
 ...
 ELSE
  ...
  ;end select
 ENDIF
 IF (( $MF_FACILITY=mf_cs220_bmcinptpsych_cd))
  SELECT INTO "nl:"
   FROM code_value cv
   PLAN (cv
    WHERE expand(ml_expand_cnt,1,size(audit_request->unit,5),cv.code_value,audit_request->unit[
     ml_expand_cnt].nurse_unit_cd))
   DETAIL
    IF (trim(cv.display_key,3)="CAPTU")
     audit_request->unit_cnt += 1, stat = alterlist(audit_request->unit,audit_request->unit_cnt),
     audit_request->unit[audit_request->unit_cnt].nurse_unit_cd = 102330558.0
    ENDIF
 ...
 SELECT INTO  $OUTDEV
  ...
  PLAN (d
   ...
   WHERE elh.encntr_id=ce.encntr_id
    ...
    AND ( $MF_FACILITY != mf_cs220_bmcinptpsych_cd)) OR (((( $MF_FACILITY=mf_cs220_bmcinptpsych_cd)
    AND elh.loc_nurse_unit_cd=mae.nurse_unit_cd
    AND elh.loc_nurse_unit_cd != 102330558.00) OR (( $MF_FACILITY=mf_cs220_bmcinptpsych_cd)
    AND elh.loc_nurse_unit_cd=1174782179.00
    AND mae.nurse_unit_cd=102330558.00)) ))
  ...
  FOOT  mae.nurse_unit_cd
   ...
   ml_pu_pt_scanned += ml_pt_scanned,
   ml_pu_med_scanned += ml_med_scanned, ml_pu_pt_selected += ml_pt_selected, ml_pu_med_selected +=
   ml_med_selected,
  ...
  FOOT  mae.prsnl_id
   ...
   ml_pospt_compliance_total += ml_pu_pt_scanned,
   ml_posmed_compliance_total += ml_pu_med_scanned, mf_pu_pt_percent = ((cnvtreal(ml_pu_pt_scanned)/
   cnvtreal(ml_pu_pt_total)) * 100), mf_pu_med_percent = ((cnvtreal(ml_pu_med_scanned)/ cnvtreal(
    ml_pu_med_total)) * 100)
   IF (ml_cnt > 1)
