CREATE PROGRAM bhs_rpt_import_order_list:dba
 ...
 SET filename = concat("bhsordlistimport",format(cnvtdatetime(curdate,curtime3),"MMDDYYYYHHMM;;d"))
 ...
 SET stat = alterlist(allord->qual,listsize)
 FOR (x = 1 TO listsize)
 ...
 CALL echorecord(allord)
 COMMIT
 CALL echorecord(allord,filename)
 ...
 SELECT INTO value(filename)
  ...
  status = substring(1,100,allord->qual[d.seq].status_txt), updt_dt_tm = substring(1,20,format(
    cnvtdatetime(updt_dt_tm),";;q")), updt_id = prsnlid
