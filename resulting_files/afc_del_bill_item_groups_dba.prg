CREATE PROGRAM afc_del_bill_item_groups:dba
 ...
 CALL del_bill_item_groups(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE del_bill_item_groups(del_begin,del_end)
   FOR (x = del_begin TO del_end)
    UPDATE  FROM bill_item_groups b
     SET b.active_ind = false, b.active_status_cd = nullcheck(active_code,request->bill_item_groups[x
       ].active_status_cd,
       IF ((request->bill_item_groups[x].active_status_cd=0)) 0
       ELSE 1
       ENDIF
       ), b.active_status_prsnl_id = reqinfo->updt_id,
      b.active_status_dt_tm = cnvtdatetime(sysdate), b.updt_cnt = (b.updt_cnt+ 1), b.updt_dt_tm =
