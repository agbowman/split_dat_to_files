CREATE PROGRAM dm2_create_admin_synonyms:dba
 ...
 SUBROUTINE dsl_create_dm_flags_objs(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dcas_logfile_prefix = vc WITH protect, constant("dm2_cre_adm_synms")
 DECLARE dcas_admn_tname = vc WITH protect, noconstant(" ")
 DECLARE dcas_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcas_link_name = vc WITH protect, noconstant(" ")
 DECLARE dcas_exist = i2 WITH protect, noconstant(0)
 DECLARE dcas_ndx = i4 WITH protect, noconstant(0)
 DECLARE dcas_db_link_cnt = i4 WITH protect, noconstant(0)
 DECLARE dcas_dot_pos = i4 WITH protect, noconstant(0)
 DECLARE dcas_get_list_of_admin_tables(null) = i2
 DECLARE dcas_chk_for_clin_seq(cfcs_syn_to_test=vc,cfcs_syn_exist=i2(ref)) = i2
 DECLARE dcas_get_list_of_admin_sequences(null) = i2
 FREE RECORD dcas_synonym_list
 RECORD dcas_synonym_list(
   1 cnt = i4
   1 qual[*]
     2 synonym = vc
     2 seq_ind = i2
 ...
 SET dcas_synonym_list->cnt = 0
 IF (check_logfile(dcas_logfile_prefix,".log","DM2_CREATE_ADMIN_SYNONYMS LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning Create Admin Synonyms Process"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dcas_link_name = build(trim( $1,3))
 SET dcas_admn_tname = cnvtupper(build(trim( $2,3)))
 IF (textlen(dcas_admn_tname)=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Admin Table Specified ",dcas_admn_tname," is invalid.")
  SET dm_err->eproc = "Admin Table Validation"
  SET dm_err->user_action = concat("Please specify a valid admin table. ")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (textlen(dcas_link_name)=0)
  SET dm_err->eproc = concat("Evaluating database link name (",dcas_link_name,").")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Database link name invalid."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (dcas_link_name="DM2NOTSET")
  SET dm_err->eproc = "Getting database link."
  SET dcas_link_name = dm2_get_db_link(null)
 ENDIF
 IF (dcas_link_name="DM2_ERROR")
  GO TO exit_program
 ELSEIF (dcas_link_name="DM2_UNKNOWN")
  IF (validate(dm2_create_admin_link_name,"X") != "X")
   SET dm2_server_link->server_name = dm2_create_admin_link_name
  ELSE
   IF ((dir_ui_misc->auto_install_ind=1))
    SET dm_err->eproc = "Getting Database link"
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Unable to determine admin database link."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SET message = window
   CALL clear(1,1)
   CALL text(1,1,"Enter the connect string for the synonyms (e.g.  admin1):")
   CALL accept(1,60,"P(30);c",dm2_server_link->server_name
    WHERE  NOT (curaccept=" "))
   SET dm2_server_link->server_name = cnvtupper(build(curaccept))
   SET message = nowindow
   SET dm_err->eproc = "Verify database link."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    dl.db_link
    FROM all_db_links dl
    WHERE parser(build(" dl.db_link = '",dm2_server_link->server_name,"*'"))
    DETAIL
     dcas_dot_pos = findstring(".",dl.db_link,1)
     IF (dcas_dot_pos > 0)
      dcas_link_name = substring(1,(dcas_dot_pos - 1),dl.db_link)
     ELSE
      dcas_link_name = dl.db_link
     ENDIF
     IF (cnvtupper(dcas_link_name)=cnvtupper(dm2_server_link->server_name))
      dcas_db_link_cnt = (dcas_db_link_cnt+ 1)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   IF (dcas_db_link_cnt=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "The connect string entered is invalid."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ELSEIF (dcas_db_link_cnt > 1)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "The connect string entered cannot be validated due to multiple matches."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
  ENDIF
 ELSE
  SET dm2_server_link->server_name = dcas_link_name
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(build("db_link =",dm2_server_link->server_name))
 ENDIF
 SET dm2_server_link->user = "CDBA"
 IF (dm2_set_inhouse_domain(null)=0)
  GO TO exit_program
 ENDIF
 IF (((dcas_admn_tname=char(42)) OR (dcas_admn_tname="DM_ENVIRONMENT")) )
  IF (check_dm2tools_nicknames(1)=0)
   GO TO exit_program
  ENDIF
 ELSE
  ...
  ENDIF
 ENDIF
 IF (dcas_get_list_of_admin_tables(null)=0)
  GO TO exit_program
 ENDIF
 IF (dcas_get_list_of_admin_sequences(null)=0)
  GO TO exit_program
 ENDIF
 IF ((dcas_synonym_list->cnt=0))
  SET dm_err->eproc = "Checking for synonyms that need to be created"
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No Admin tables/sequences matched '",dcas_admn_tname,
   "' and select criteria - no synonyms were created.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Creating synonyms for admin tables/sequences."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dm_err->debug_flag > 20))
  CALL echorecord(dcas_synonym_list)
 ENDIF
 FOR (dcas_cnt = 1 TO dcas_synonym_list->cnt)
   SET dm2_nickname_info->nickname = dcas_synonym_list->qual[dcas_cnt].synonym
   SET dm2_nickname_info->drop_ind = dcas_synonym_list->qual[dcas_cnt].drop_ind
   SET dm2_nickname_info->create_ind = 1
   SET dm2_nickname_info->local_owner = "NONE"
   SET dm2_nickname_info->server = dm2_server_link->server_name
   SET dm2_nickname_info->remote_table = dcas_synonym_list->qual[dcas_cnt].synonym
   SET dm2_nickname_info->remote_owner = "CDBA"
   SET dm2_nickname_info->link_server = "NONE"
   SET dm2_nickname_info->col_list1 = "NONE"
   SET dm2_nickname_info->col_list2 = "NONE"
   SET dm2_nickname_info->create_view_ind = 0
   SET dcas_ndx = 0
   SET dcas_ndx = locateval(dcas_ndx,1,dsl_dmtools_adm_tables->cnt,dm2_nickname_info->nickname,
    dsl_dmtools_adm_tables->tbl[dcas_ndx].table_name)
   IF (dcas_ndx > 0)
    SET dm2_nickname_info->create_view_ind = 1
   ENDIF
   IF ((dcas_synonym_list->qual[dcas_cnt].seq_ind=1))
    IF (dcas_chk_for_clin_seq(dcas_synonym_list->qual[dcas_cnt].synonym,dcas_exist)=0)
     GO TO exit_program
    ENDIF
    IF ((dcas_synonym_list->qual[dcas_cnt].synonym != "DM_SEQ")
     AND dcas_exist=1)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("The sequence [",dcas_synonym_list->qual[dcas_cnt].synonym,
      "] exists in clinical,","so a synonym with the same name cannot be created")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    IF (dcas_exist=0)
   ...
   ELSE
    IF (dm2_create_nickname(null)=0)
     GO TO exit_program
    ENDIF
    EXECUTE oragen3 build("cdba.",dcas_synonym_list->qual[dcas_cnt].synonym,"@",dm2_server_link->
     server_name)
    IF ((dm_err->err_ind=1))
     GO TO exit_program
    ENDIF
    IF (dcas_ndx > 0)
     SET dsl_dmtools_adm_tables->tbl[dcas_ndx].drop_ind = 0
     SET dm_err->eproc = "Checking whether synonym exists."
     SELECT INTO "nl:"
      FROM dm2_dba_synonyms ds
      WHERE (ds.synonym_name=dsl_dmtools_adm_tables->tbl[dcas_ndx].synonym_name)
       AND ds.owner="PUBLIC"
      DETAIL
       dsl_dmtools_adm_tables->tbl[dcas_ndx].drop_ind = 1
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
     IF (dsl_create_spec_admin_synonyms(dcas_ndx)=0)
      GO TO exit_program
     ENDIF
    ENDIF
   ENDIF
 ...
 IF ((inhouse_misc->inhouse_domain=1))
  SET dcas_ndx = 0
  SET dcas_ndx = locateval(dcas_ndx,1,dcas_synonym_list->cnt,"DM_README_DEV",dcas_synonym_list->qual[
   dcas_ndx].synonym)
  IF (dcas_ndx > 0)
   SET dm_err->eproc = "Oragen DM_README (special view on Admin table DM_README_DEV)."
   CALL disp_msg(" ",dm_err->logfile,0)
   EXECUTE oragen3 build("cdba.dm_readme@",dm2_server_link->server_name)
   IF ((dm_err->err_ind=1))
    GO TO exit_program
   ENDIF
  ENDIF
 ENDIF
 GO TO exit_program
 SUBROUTINE dcas_get_list_of_admin_tables(null)
   DECLARE gloat_dyn_in_clause = vc WITH public, noconstant("")
   DECLARE gloat_syn_ind = c1 WITH protect, noconstant(" ")
   DECLARE gloat_dm2_admin_dm_info_exists = i2 WITH protect, noconstant(0)
   SET dm_err->eproc = "Determining whether synonym dm2_admin_dm_info exists"
   SET gloat_syn_ind = dm2_table_exists("DM2_ADMIN_DM_INFO")
   IF ((dm_err->err_ind=1))
    RETURN(0)
   ENDIF
   IF (gloat_syn_ind="F")
    SET gloat_dm2_admin_dm_info_exists = 1
   ENDIF
   SET gloat_dyn_in_clause = dm2_fill_nick_except("at")
   SET dm_err->eproc = "Getting list of ADMIN tables that synonyms need to be created."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT
    IF (gloat_dm2_admin_dm_info_exists=0)
     FROM dm2_admin_tables at,
      dm2_dba_synonyms ds
     PLAN (at
      WHERE at.table_name=patstring(dcas_admn_tname)
       AND parser(gloat_dyn_in_clause))
      JOIN (ds
      WHERE outerjoin(at.table_name)=ds.synonym_name
       AND ds.owner=outerjoin("PUBLIC"))
    ELSE
     FROM dm2_admin_tables at,
      dm2_dba_synonyms ds,
     ...
     PLAN (adm
      ...
      JOIN (at
      WHERE at.table_name=adm.info_name
       AND at.table_name=patstring(dcas_admn_tname)
       AND parser(gloat_dyn_in_clause))
      JOIN (ds
      WHERE outerjoin(at.table_name)=ds.synonym_name
       AND ds.owner=outerjoin("PUBLIC"))
    ...
    HEAD REPORT
     stat = alterlist(dcas_synonym_list->qual,10), dcas_synonym_list->cnt = 0
    DETAIL
     dcas_synonym_list->cnt = (dcas_synonym_list->cnt+ 1)
     IF (mod(dcas_synonym_list->cnt,10)=1
      AND (dcas_synonym_list->cnt != 1))
      stat = alterlist(dcas_synonym_list->qual,(dcas_synonym_list->cnt+ 9))
     ENDIF
     dcas_synonym_list->qual[dcas_synonym_list->cnt].synonym = at.table_name, dcas_synonym_list->
     qual[dcas_synonym_list->cnt].seq_ind = 0, dcas_synonym_list->qual[dcas_synonym_list->cnt].
     drop_ind = 0
     IF (ds.synonym_name > " ")
      dcas_synonym_list->qual[dcas_synonym_list->cnt].drop_ind = 1
     ENDIF
     dcas_synonym_list->qual[dcas_synonym_list->cnt].col_cnt = 0
    FOOT REPORT
     stat = alterlist(dcas_synonym_list->qual,dcas_synonym_list->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcas_get_list_of_admin_sequences(null)
   DECLARE gloas_dyn_in_clause = vc WITH public, noconstant("")
   DECLARE gloas_syn_ind = c1 WITH protect, noconstant(" ")
   SET gloas_dyn_in_clause = dm2_fill_seq_list("us","sequence_name")
   SET dm_err->eproc = "Getting list of ADMIN sequences that synonyms need to be created."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_admin_sequences us,
     dm2_dba_synonyms ds
    PLAN (us
     WHERE us.sequence_name=patstring(dcas_admn_tname)
      AND parser(gloas_dyn_in_clause))
     JOIN (ds
     WHERE outerjoin(us.sequence_name)=ds.synonym_name
      AND ds.owner=outerjoin("PUBLIC"))
    ORDER BY us.sequence_name
    DETAIL
     dcas_synonym_list->cnt = (dcas_synonym_list->cnt+ 1), stat = alterlist(dcas_synonym_list->qual,
      dcas_synonym_list->cnt), dcas_synonym_list->qual[dcas_synonym_list->cnt].synonym = us
     .sequence_name,
     dcas_synonym_list->qual[dcas_synonym_list->cnt].seq_ind = 1, dcas_synonym_list->qual[
     dcas_synonym_list->cnt].drop_ind = 0
     IF (ds.synonym_name > " ")
      dcas_synonym_list->qual[dcas_synonym_list->cnt].drop_ind = 1
     ENDIF
     dcas_synonym_list->qual[dcas_synonym_list->cnt].col_cnt = 0
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcas_chk_for_clin_seq(cfcs_syn_to_test,cfcs_syn_exist)
   SET dm_err->eproc = "Check to see if clinical sequence exists with same name."
   SELECT INTO "nl:"
    FROM dm2_user_sequences us
    WHERE us.sequence_name=cfcs_syn_to_test
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual > 0)
    SET cfcs_syn_exist = 1
   ELSE
    SET cfcs_syn_exist = 0
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Create Admin Synonyms Process Completed!"
 CALL final_disp_msg(dcas_logfile_prefix)
END GO
