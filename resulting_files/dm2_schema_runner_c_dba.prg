CREATE PROGRAM dm2_schema_runner_c:dba
 DECLARE cdb2_db = vc WITH protect, constant("DB2UDB")
 DECLARE dsrc_orig_emsg = vc WITH public, noconstant("")
 DECLARE dsrc_updlock_ind = i2 WITH protect, noconstant(0)
 DECLARE dsrc_act_duration = f8 WITH protect, noconstant(0.0)
 DECLARE dsrc_end_dt_tm = f8 WITH protect, noconstant(0.0)
 DECLARE dsrc_err_ret = i2 WITH protect, noconstant(0)
 DECLARE dsrc_log_loc_ret = vc WITH protect, noconstant("")
 DECLARE dsrc_status = i2 WITH protect, noconstant(0)
 DECLARE dsrc_backfill_fallout = i2 WITH protect, noconstant(0)
 DECLARE dsrc_pause_time = i4 WITH protect, noconstant(0)
 DECLARE update_lock_row() = null
 DECLARE update_log_table(ult_opid=f8,ult_status=vc,ult_err_msg=vc) = null
 DECLARE dsr_execute_command(dec_operation=vc) = null
 DECLARE dsr_execute_oe_command(dsreoc_pword=vc) = null
 DECLARE dsr_sqlsrv_import_errchk(dsrsie_oe_operation=vc) = null
 DECLARE dsr_create_sql_err_chk(dsrte_object_type=vc,dsrte_object_name=vc) = i2
 ...
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO end_program
 ENDIF
 IF ((validate(dsr_cleanup_timestamp,- (1))=- (1)))
  SET dm_err->eproc = "Checking how script was executed."
  SET dm_err->emsg = "Cannot run this script standalone."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_program
 ENDIF
 IF (dsr_cmd_retry_cnt > 1)
  SET dsrc_updlock_ind = 1
 ELSE
  CALL update_lock_row(null)
  IF ((dm_err->err_ind=1))
   GO TO end_program
  ENDIF
 ENDIF
 IF (dsrc_updlock_ind=1)
  CALL dsr_execute_command(dsr_command)
  IF ((dm_err->err_ind=1))
   SET dsr_cmd_successful = 0
   SET dm_err->err_ind = 0
   IF (dsrc_backfill_fallout)
    CALL update_log_table(dsr_op_id,"NULL",dsrc_orig_emsg)
   ELSEIF (dsr_cmd_error=1
    AND dsr_cmd_retry_cnt < cnvtint(dm2_db_options->resource_busy_maxretry))
    CALL update_log_table(dsr_op_id,"RUNNING",dsrc_orig_emsg)
   ELSE
    CALL update_log_table(dsr_op_id,"ERROR",dsrc_orig_emsg)
   ENDIF
   IF ((dm_err->err_ind=1))
    GO TO end_program
   ENDIF
  ELSE
   IF (dsr_compile=1)
    EXECUTE dm2_compile_objects "TABLE", dsr_table_name
    SET dm_err->err_ind = 0
   ENDIF
   SET dsr_cmd_successful = 1
   CALL update_log_table(dsr_op_id,"COMPLETE"," ")
   IF ((dm_err->err_ind=1))
    GO TO end_program
   ENDIF
   IF (dsr_exec_context != "CLIN COPY"
    AND dsrc_pause_time > 0)
    CALL pause(dsrc_pause_time)
   ENDIF
  ENDIF
 ENDIF
 GO TO end_program
 SUBROUTINE update_lock_row(null)
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = build("Update/Lock op_id:",dsr_execlist->exec[dsreidx].op_id)
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   IF ((dm_err->debug_flag > 1))
    CALL echo(concat("Current date and time = ",format(cnvtdatetime(curdate,curtime),
       "mm/dd/yyyy hh:mm:ss;;d")))
    CALL echo(concat("Last cleanup date and time = ",format(dsr_cleanup_timestamp,
       "mm/dd/yyyy hh:mm:ss;;d")))
    CALL echo(concat("Number of minutes elapsed = ",cnvtstring(datetimediff(cnvtdatetime(curdate,
         curtime),cnvtdatetime(dsr_cleanup_timestamp),4),4,3)))
   ENDIF
   IF (datetimediff(cnvtdatetime(curdate,curtime),cnvtdatetime(dsr_cleanup_timestamp),4) > 5)
    SET dsr_cleanup_timestamp = cnvtdatetime(curdate,curtime)
    IF (dm2_cleanup_stranded_appl(null)=0)
     GO TO end_program
    ENDIF
    IF (dsr_dm_info_exists=1)
     IF (drr_check_runner_status("DM2_SCHEMA_RUNNER",dsr_currdbhandle,dsrc_status)=0)
      GO TO end_program
     ENDIF
     IF (dsrc_status=0)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "Schema runner has been marked to stop."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO end_program
     ENDIF
    ENDIF
   ENDIF
   SET dsr_begin_dt_tm = cnvtdatetime(curdate,curtime3)
   IF (dsr_serial_op_types != "('DM2NOTSET')"
    AND dsr_exec_context="CLIN COPY")
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = "RUNNING", ddol.begin_dt_tm = cnvtdatetime(dsr_begin_dt_tm), ddol.logfile_name
       = dm_err->logfile,
      ddol.appl_id = currdbhandle
     WHERE (ddol.op_id=dsr_execlist->exec[dsreidx].op_id)
      AND ddol.status = null
      AND ((parser(concat("ddol.op_type in ",dsr_serial_op_types))
      AND  NOT ( EXISTS (
     (SELECT
      "x"
      FROM dm2_ddl_ops_log x
      WHERE parser(concat("x.op_type in ",dsr_serial_op_types))
       AND x.status="RUNNING"
       AND x.run_id=ddol.run_id)))) OR (parser(concat("ddol.op_type not in ",dsr_serial_op_types))))
     WITH nocounter
    ;end update
   ELSEIF (dsr_serial_op_types != "('DM2NOTSET')"
    AND dsr_exec_context != "CLIN COPY")
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = "RUNNING", ddol.begin_dt_tm = cnvtdatetime(dsr_begin_dt_tm), ddol.logfile_name
       = dm_err->logfile,
     ...
     WHERE (ddol.op_id=dsr_execlist->exec[dsreidx].op_id)
      AND ddol.status = null
      AND  NOT (ddol.table_name IN (
     (SELECT
      ddol2.table_name
      FROM dm2_ddl_ops_log ddol2
      WHERE ddol2.status="RUNNING"
       AND ddol2.op_type != "RANGE BACKFILL"
       AND ddol2.table_name != "-1")))
      AND ((parser(concat("ddol.op_type in ",dsr_serial_op_types))
      AND  NOT ( EXISTS (
     (SELECT
      "x"
   ...
   ELSE
    ...
    ;end update
   ENDIF
   IF (check_error("Updating a dm2_ddl_ops_log DDL row for execution.")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO end_program
   ELSE
    SET dsr_updt_cnt = (dsr_updt_cnt+ 1)
    IF (curqual > 0)
     COMMIT
     SET dsrc_updlock_ind = 1
     SET dsr_updt_fnd_cnt = (dsr_updt_fnd_cnt+ 1)
     SET dsr_updt_constolen_cnt = 0
     SELECT INTO "nl:"
      FROM dm2_ddl_ops_log ddol1
      WHERE (ddol1.op_id=dsr_execlist->exec[dsreidx].op_id)
      DETAIL
       dsr_run_id = ddol1.run_id, dsr_op_id = ddol1.op_id, dsr_op_type = ddol1.op_type,
       dsr_command = ddol1.operation, dsr_oe_command = ddol1.on_error_operation, dsr_obj_name = ddol1
       .obj_name,
       dsr_table_name = ddol1.table_name, dsr_priority = ddol1.priority, dsrc_pause_time = ddol1
       .wait_val
      WITH nocounter
     ;end select
     IF (check_error(build("Selecting execution information from dm2_ddl_ops_log for op_id: ",
       dsr_execlist->exec[dsreidx].op_id))=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO end_program
     ELSE
      IF (curqual != 1)
       SET dm_err->err_ind = 1
       SET dm_err->emsg = build(
        "Error selecting execution information from dm2_ddl_ops_log for op_id: ",dsr_execlist->exec[
        dsreidx].op_id,".  Row not found.")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO end_program
      ENDIF
     ENDIF
    ELSE
     ROLLBACK
     SET dsrc_updlock_ind = 0
     SET dsr_updt_stolen_cnt = (dsr_updt_stolen_cnt+ 1)
     SET dsr_updt_constolen_cnt = (dsr_updt_constolen_cnt+ 1)
    ENDIF
    IF ((dm_err->debug_flag > 3))
     CALL echo(build("dsr_updt_cnt:",dsr_updt_cnt))
     CALL echo(build("dsr_updt_fnd_cnt:",dsr_updt_fnd_cnt))
     CALL echo(build("dsr_updt_stolen_cnt:",dsr_updt_stolen_cnt))
     CALL echo(build("dsr_updt_constolen_cnt:",dsr_updt_constolen_cnt))
    ENDIF
   ENDIF
   IF ((dm_err->debug_flag > 0)
    AND dsrc_updlock_ind=1)
    CALL echo(build("Op Id        :",dsr_op_id))
    CALL echo(build("Op Type      :",dsr_op_type))
    CALL echo(build("Command      :",dsr_command))
    CALL echo(build("Error Command:",dsr_oe_command))
   ENDIF
 END ;Subroutine
 SUBROUTINE update_log_table(ult_opid,ult_status_text,ult_err_msg)
   DECLARE ult_local_msg = vc WITH protect, noconstant(" ")
   DECLARE dsrc_main_appl_id = vc WITH protect, noconstant(" ")
   DECLARE ult_status_text_hold = vc WITH protect, noconstant(ult_status_text)
   DECLARE ult_notify_ind = i2 WITH protect, noconstant(0)
   DECLARE ult_half_maxretry = i4 WITH protect, noconstant(0)
   SET ult_local_msg = ult_err_msg
   SET ult_notify_ind = 0
   SET ult_half_maxretry = ceil((cnvtint(dm2_db_options->resource_busy_maxretry)/ 2))
   IF ((dm_err->debug_flag > 0))
    SET dm_err->eproc = build("Updating DM2_DDL_OPS_LOG.STATUS to: ",ult_status_text_hold,
     " for OP_ID: ",ult_opid)
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET dsrc_end_dt_tm = cnvtdatetime(curdate,curtime3)
   SET dsrc_act_duration = ((dsrc_end_dt_tm - dsr_begin_dt_tm)/ 10000000)
   IF (ult_status_text_hold="NULL"
    AND dsr_dm_info_exists)
    SELECT INTO "nl:"
     FROM dm_info di
     WHERE di.info_domain="DM2_SET_READY_TO_RUN"
      AND di.info_char=cnvtstring(dsr_run_id)
     DETAIL
      dsrc_main_appl_id = di.info_name
     WITH nocounter
    ;end select
    IF (check_error("Getting appl_id for runner from DM_INFO")=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     CALL disp_msg("The above error message is ignorable",0)
    ENDIF
    IF (((curqual=0) OR ((dm_err->err_ind=1))) )
     SET dm_err->err_ind = 0
     SET ult_status_text_hold = "ERROR"
    ENDIF
   ENDIF
   IF (ult_status_text_hold IN ("ERROR", "RUNNING"))
    IF (dsr_cmd_retry_cnt > 1)
     SET ult_local_msg = concat(build(dsr_cmd_retry_cnt)," OF ",dm2_db_options->
      resource_busy_maxretry," DDL ATTEMPTS: HARD PARSE (",build(dsr_cmd_hrdprs_retry_cnt),
      ")|RESOURCE BUSY (",build(dsr_cmd_rscbsy_retry_cnt),")|LIBRARY LOCK (",build(
       dsr_cmd_liblock_retry_cnt),").")
    ENDIF
    IF (((dsr_cmd_retry_cnt=ult_half_maxretry) OR (dsr_cmd_retry_cnt=cnvtint(dm2_db_options->
     resource_busy_maxretry))) )
     SET ult_notify_ind = 1
    ELSEIF (dsr_cmd_retry_cnt > ult_half_maxretry)
     IF (mod(dsr_cmd_retry_cnt,ceil((cnvtint(dm2_db_options->resource_busy_maxretry)/ 10)))=0)
      SET ult_notify_ind = 1
     ENDIF
    ENDIF
    IF ((dnotify->status=1)
     AND (dnotify->suppression_flag=0)
     AND ult_notify_ind=1)
     SET dsr_available_retry = (cnvtint(dm2_db_options->resource_busy_maxretry) - dsr_cmd_retry_cnt)
     SET dnotify->process = "INSTALLPLAN"
     SET dnotify->install_status = dpl_executing
     SET dnotify->mode = "DDL EXECUTION"
     IF (dsr_cmd_retry_cnt=cnvtint(dm2_db_options->resource_busy_maxretry))
      SET dnotify->event = concat("Schema retry ceiling reached for OP_ID ",build(cnvtint(ult_opid)))
      SET dnotify->msgtype = dpl_failure
     ELSE
      SET dnotify->event = concat("Schema retry event encountered for OP_ID ",build(cnvtint(ult_opid)
        ))
      SET dnotify->msgtype = dpl_warning
     ENDIF
     CALL dn_add_body_text(concat("DDL execution has been attempted ",trim(cnvtstring(
         dsr_cmd_retry_cnt))," times over the last ",trim(cnvtstring(ceil((dsrc_act_duration/ 60)))),
       " minutes."),0)
     IF (dsr_cmd_retry_cnt=ult_half_maxretry)
      CALL dn_add_body_text(concat("There are ",trim(cnvtstring(ult_half_maxretry)),
        " attempts remaining.","If condition not resolved, DDL will be failed in ",trim(cnvtstring(
          ceil((dsrc_act_duration/ 60)))),
        " minutes."),0)
     ELSEIF (dsr_cmd_retry_cnt != cnvtint(dm2_db_options->resource_busy_maxretry))
      CALL dn_add_body_text(concat("There are ",trim(cnvtstring(dsr_available_retry)),
        " attempts remaining.","If condition not resolved, DDL will be failed in ",trim(cnvtstring(
          ceil((dsr_available_retry * ((dsrc_act_duration/ dsr_cmd_retry_cnt)/ 60))))),
        " minutes."),0)
     ENDIF
     IF (dsr_cmd_retry_cnt=cnvtint(dm2_db_options->resource_busy_maxretry))
      CALL dn_add_body_text("Retry count was exhausted and the DDL failed. ",0)
     ENDIF
     CALL dn_add_body_text(" ",0)
     CALL dn_add_body_text("The error message is:",0)
     CALL dn_add_body_text(" ",0)
     CALL dn_add_body_text(ult_local_msg,0)
     CALL dn_add_body_text(" ",0)
     CALL dn_add_body_text("The operation being executed is: ",0)
     CALL dn_add_body_text(dsr_command,0)
     CALL dn_add_body_text(" ",0)
     CALL dn_add_body_text(concat("Logfile: ",dm_err->logfile),0)
     CALL dn_add_body_text(" ",0)
     CALL dn_notify(null)
    ENDIF
    SET ult_local_msg = substring(1,4000,ult_local_msg)
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = ult_status_text_hold, ddol.end_dt_tm = cnvtdatetime(dsrc_end_dt_tm), ddol
      .act_duration = (dsrc_act_duration+ nullval(ddol.act_duration,0.0)),
      ddol.error_msg = ult_local_msg
     WHERE ddol.op_id=ult_opid
     WITH nocounter
    ;end update
   ELSEIF (ult_status_text_hold="COMPLETE")
    IF (dsr_cmd_retry_cnt > 1)
     SET ult_local_msg = concat("DDL COMPLETE AFTER ",build(dsr_cmd_retry_cnt),
      " ATTEMPTS: HARD PARSE (",build(dsr_cmd_hrdprs_retry_cnt),")|RESOURCE BUSY (",
      build(dsr_cmd_rscbsy_retry_cnt),")|LIBRARY LOCK (",build(dsr_cmd_liblock_retry_cnt),") OF ",
      dm2_db_options->resource_busy_maxretry,
      ".")
     IF ((dnotify->status=1)
      AND (dnotify->suppression_flag=0))
      SET dnotify->process = "INSTALLPLAN"
      SET dnotify->install_status = dpl_executing
      SET dnotify->event = concat("Resolved: Schema retry event for OP_ID ",build(cnvtint(ult_opid)))
      SET dnotify->mode = "DDL EXECUTION"
      SET dnotify->msgtype = dpl_progress
      CALL dn_add_body_text(concat("DDL retry event resolved after ",build(dsr_cmd_retry_cnt),
        " attempts over the last ",trim(cnvtstring(ceil((dsrc_act_duration/ 60))))," minutes:"),0)
      CALL dn_add_body_text(" ",0)
      CALL dn_add_body_text(ult_local_msg,0)
      CALL dn_add_body_text(" ",0)
      CALL dn_add_body_text("The operation executed was: ",0)
      CALL dn_add_body_text(dsr_command,0)
      CALL dn_add_body_text(" ",0)
      CALL dn_add_body_text(concat("Logfile: ",dm_err->logfile),0)
      CALL dn_add_body_text(" ",0)
    ...
    UPDATE  FROM dm2_ddl_ops_log ddol
     ...
     WITH nocounter
    ;end update
   ELSEIF (ult_status_text_hold="NULL")
    UPDATE  FROM dm2_ddl_ops_log ddol
     SET ddol.status = null, ddol.ready_to_run_ind = 1, ddol.begin_dt_tm = cnvtdatetime(null),
      ddol.end_dt_tm = cnvtdatetime(null), ddol.act_duration = (dsrc_act_duration+ nullval(ddol
       .act_duration,0.0)), ddol.error_msg = null,
      ddol.appl_id = dsrc_main_appl_id
     WHERE ddol.op_id=ult_opid
     WITH nocounter
    ;end update
   ENDIF
   IF ((dm_err->debug_flag > 3))
    CALL echo(build("Status     :",ult_status_text_hold))
    CALL echo(build("Op Id      :",ult_opid))
    CALL echo(build("Begin Time :",dsr_begin_dt_tm))
    CALL echo(build("End Time   :",dsrc_end_dt_tm))
    CALL echo(build("Duration   :",dsrc_act_duration))
    CALL echo(build("Error Msg  :",ult_local_msg))
   ENDIF
   IF (check_error("Update_Log_Table")=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ROLLBACK
   ELSE
    COMMIT
   ...
   IF ((dm_err->debug_flag > 3))
    SELECT INTO "nl:"
     FROM dm2_ddl_ops_log a
     WHERE a.op_id=ult_opid
     DETAIL
      CALL echo(fillstring(60,"-")), row + 1,
      CALL echo(build("******>  RUN_ID= (",a.run_id,") OP_ID = (",a.op_id,") ,table_name=",
       a.table_name," ,obj_name= ",a.obj_name,"<******")),
      row + 1,
      CALL echo(build(" op_type=",a.op_type,",priority=",a.priority,",schema_instance=",
       a.schema_instance," ,gen_id=",a.gen_id,",gen_dt_tm=",format(a.gen_dt_tm,";;Q"),
       ",begin_dt_tm=",format(a.begin_dt_tm,";;Q"),",end_dt_tm=",format(a.end_dt_tm,";;Q"),
       " ,status=",
       a.status,",error_msg=",a.error_msg,",operation=",a.operation,
       ",on_error_operation=",a.on_error_operation)), row + 1
     WITH nocounter
    ;end select
   ENDIF
 END ;Subroutine
 SUBROUTINE dsr_execute_command(dec_operation)
   DECLARE dsrec_pword = vc WITH protect, noconstant("")
   DECLARE dsrec_operation = vc WITH protect, noconstant(" ")
   DECLARE dsrec_cmd_type = vc WITH protect, noconstant(" ")
   DECLARE dsrec_exist = i2 WITH protect, noconstant(0)
   DECLARE dsrec_start_pos = i4 WITH protect, noconstant(0)
   DECLARE dsrec_end_pos = i4 WITH protect, noconstant(0)
   DECLARE dsrec_err_cnt = i4 WITH protect, noconstant(0)
   DECLARE dsrec_err_msg = vc WITH protect, noconstant(" ")
   DECLARE dsrec_logfile_name = vc WITH protect, noconstant("")
   DECLARE dsrec_temp_err = vc WITH protect, noconstant("")
   DECLARE dsrec_err_file = vc WITH protect, noconstant("")
   DECLARE dsrec_copy_text = vc WITH protect, noconstant("")
   DECLARE dsrec_dcl_stat = i4 WITH protect, noconstant(0)
   DECLARE dsrec_in_parse_ind = i2 WITH protect, noconstant(0)
   DECLARE dsrec_ret_msg = vc WITH public, noconstant("")
   CASE (substring(1,6,dec_operation))
    OF "%DCL1%":
     SET dsrec_cmd_type = "DM2_PUSH_DCL"
    OF "%DCL2%":
     SET dsrec_cmd_type = "DB2_PUSH_DCL_W_CONNECT"
    OF "%DCL3%":
     SET dsrec_cmd_type = "DM2_PUSH_DCL_NO_CHK"
    ELSE
     SET dsrec_cmd_type = "DM2_PUSH_CMD"
   ENDCASE
   IF (currdbuser="CDBA")
    SET dsrec_pword = dm2_install_schema->cdba_p_word
   ELSE
    SET dsrec_pword = dm2_install_schema->v500_p_word
   ENDIF
   SET dsrec_operation = dec_operation
   IF (dir_ddl_token_replacement(dsrec_operation)=0)
    GO TO end_program
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat(dsrec_cmd_type,": ",dsrec_operation))
   ENDIF
   IF (dsr_table_name != "-1"
    AND dsrec_cmd_type="DM2_PUSH_CMD")
    IF (dir_check_in_parse(currdbuser,dsr_table_name,dsrec_in_parse_ind,dsrec_ret_msg)=0)
     GO TO end_program
    ELSE
     IF (dsrec_in_parse_ind=1)
      SET dm_err->eproc = concat("Preparing to execute command: ",dsrec_operation)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = concat("Hard parse event found against ",trim(dsr_table_name)," table.")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      SET dsr_cmd_error = 1
      SET dsr_cmd_hrdprs_error = 1
      SET dsr_cmd_hrdprs_retry_cnt = (dsr_cmd_hrdprs_retry_cnt+ 1)
     ELSE
      SET dsr_cmd_error = 0
      SET dsr_cmd_hrdprs_error = 0
     ENDIF
    ENDIF
   ENDIF
   IF ((dm_err->err_ind=0))
    CASE (dsrec_cmd_type)
     OF "DM2_PUSH_CMD":
      CALL dm2_push_cmd(dsrec_operation,1)
     OF "DM2_PUSH_DCL":
      CALL dm2_push_dcl(dsrec_operation)
     OF "DM2_PUSH_DCL_NO_CHK":
      SET dm_err->disp_dcl_err_ind = 0
      CALL dm2_push_dcl(dsrec_operation)
      SET dsrec_err_file = dm_err->errfile
     OF "DB2_PUSH_DCL_W_CONNECT":
      CALL db2_push_dcl_w_connect(dm2_install_schema->dbase_name,dm2_install_schema->u_name,
       dsrec_pword,dsrec_operation,1)
    ENDCASE
   ENDIF
   IF ((dm_err->err_ind=1)
    AND (drr_runner_misc->runner_identifier="RR_RUNNER_DO*"))
    SET dm_err->err_ind = 0
   ENDIF
   IF (dsr_priority=60
    AND (dm_err->err_ind=0))
    SET dm_err->eproc = "Check for successful execution of backfill"
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm_info di
     WHERE di.info_domain IN ("DM2CBF_IN_PROGRESS", "DM2RBF_IN_PROGRESS", "DM2PBF_IN_PROGRESS")
      AND di.info_name=trim(cnvtstring(dsr_op_id))
     WITH nocounter
    ;end select
    IF (curqual > 0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Failure in backfill ddl operation"
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ENDIF
   ENDIF
   IF ((dm_err->err_ind=1)
    AND dsrec_cmd_type="DM2_PUSH_CMD"
    AND (dm_err->emsg="*ORA-20556*"))
    SET dsrc_backfill_fallout = 1
   ENDIF
   IF ((dm_err->err_ind=0)
    AND currdb="SQLSRV"
    AND dsr_op_type="IMPORT/LOAD")
    IF (dsr_sqlsrv_import_errchk(dsrec_operation) != 1)
     CALL echo("Bad return from import check subroutine")
     GO TO end_program
    ENDIF
   ENDIF
   IF ((dm_err->err_ind=0)
    AND currdb="ORACLE"
    AND dsr_op_type="CREATE SQL*")
    IF (dsr_create_sql_err_chk(substring(12,(size(dsr_op_type) - 11),dsr_op_type),dsr_obj_name)=0)
     GO TO end_program
    ENDIF
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("dsr_execute_command->dm_err->err_ind: ",dm_err->err_ind))
   ENDIF
   IF ((((dm_err->err_ind > 0)
    AND dsr_cmd_hrdprs_error=0) OR (dsrec_cmd_type="DM2_PUSH_DCL_NO_CHK")) )
    IF (currdb="ORACLE"
     AND ((findstring("ORA-00054",dm_err->emsg) > 0) OR (((findstring("ORA-14450",dm_err->emsg) > 0)
     OR (((findstring("ORA-14452",dm_err->emsg) > 0) OR (findstring("ORA-14411",dm_err->emsg) > 0))
    )) )) )
     SET dsr_cmd_error = 1
     SET dsr_cmd_rscbsy_error = 1
     SET dsr_cmd_rscbsy_retry_cnt = (dsr_cmd_rscbsy_retry_cnt+ 1)
    ELSEIF (currdb="ORACLE"
     AND findstring("ORA-04022",dm_err->emsg) > 0)
     SET dsr_cmd_error = 1
     SET dsr_cmd_liblock_error = 1
     SET dsr_cmd_liblock_retry_cnt = (dsr_cmd_liblock_retry_cnt+ 1)
    ELSE
     SET dsr_cmd_error = 0
     SET dsr_cmd_rscbsy_error = 0
     SET dsr_cmd_liblock_error = 0
    ENDIF
    IF (dsrec_cmd_type="DM2_PUSH_DCL_NO_CHK")
     IF (dsr_op_type IN ("IMPORT*", "EXPORT*"))
      SET dsrc_log_loc_ret = ""
      IF (drr_get_dmp_log_loc(dsr_op_id,dsrc_log_loc_ret)=0)
       GO TO end_program
      ENDIF
      IF (dsrc_log_loc_ret IN ("NOT_VALID_OP_ID", "NO_FILE_IN_COMMAND"))
       SET dm_err->eproc = concat("Location Returned:",dsrc_log_loc_ret)
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat(dsr_op_type," Log file not found in location according to ",
        dsr_op_type," command.")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(null)
      ELSEIF (findstring("NO_FILE_IN_OS:",dsrc_log_loc_ret,1,0) > 0)
       SET dsrec_logfile_name = substring((findstring(":",dsrc_log_loc_ret,1,0)+ 1),(size(
         dsrc_log_loc_ret) - findstring(":",dsrc_log_loc_ret,1,0)),dsrc_log_loc_ret)
       SELECT INTO value(dsrec_logfile_name)
        FROM (dummyt d  WITH seq = 1)
        DETAIL
         dsrec_temp_err = substring(1,400,dm_err->errtext), col 0,
         "LOG FILE NOT FOUND - ERROR RECEIVED:",
         dsrec_temp_err, row + 1
        WITH nocounter, format = variable, formfeed = none,
         maxrow = 1, maxcol = 512
       ;end select
       SET dm_err->eproc = concat(build("Looking for log file used in op_id.",dsr_op_id))
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat("LOG FILE NOT FOUND - ERROR RECEIVED:",dm_err->errtext)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(null)
      ENDIF
      SET dsrc_err_ret = 0
      IF (drr_check_log_for_errors(dsr_op_id,dsrc_log_loc_ret,1,dsrc_err_ret)=0)
       GO TO end_program
      ENDIF
      IF ((dm_err->err_ind=0))
       IF (dsrc_err_ret=1)
        FOR (dsrec_err_cnt = 1 TO drr_errors_encountered->qual[1].error_cnt)
          IF (dsrec_err_cnt=1)
           SET dsrec_err_msg = concat("View logfile at:",drr_errors_encountered->qual[dsrec_err_cnt].
            logfile_name)
           SET dsrec_err_msg = concat(dsrec_err_msg," - ",trim(drr_errors_encountered->qual[1].qual[
             dsrec_err_cnt].error))
          ELSE
           SET dsrec_err_msg = concat(dsrec_err_msg," - ",trim(drr_errors_encountered->qual[1].qual[
             dsrec_err_cnt].error))
          ENDIF
        ENDFOR
        SET dsrec_err_msg = substring(1,1000,dsrec_err_msg)
       ENDIF
      ELSE
       SET dsrc_err_ret = 1
      ENDIF
      IF (dsrc_err_ret=0)
       SET dm_err->eproc =
       "No errors found in log file, check .err file for IMP/EXP operation errors"
       IF ((dm_err->debug_flag > 0))
        CALL disp_msg("",dm_err->logfile,0)
       ENDIF
       IF (drr_check_log_for_errors(dsr_op_id,dsrec_err_file,1,dsrc_err_ret)=0)
        GO TO end_program
       ENDIF
       IF ((dm_err->err_ind=0))
        IF (dsrc_err_ret=1)
         ...
         SET dsrec_err_msg = substring(1,1000,dsrec_err_msg)
         IF ((dm2_sys_misc->cur_os="AXP"))
          SET dsrec_copy_text = concat("copy ",dsrec_err_file," ",dsrc_log_loc_ret)
         ELSE
          SET dsrec_copy_text = concat("cp ",dsrec_err_file," ",dsrc_log_loc_ret)
         ENDIF
         SET dm_err->eproc = concat("Errors found in secondary check file. Copying secondary: ",
          dsrec_err_file," to primary: ",dsrc_log_loc_ret,".")
         CALL disp_msg("",dm_err->logfile,0)
         CALL dcl(dsrec_copy_text,textlen(dsrec_copy_text),dsrec_dcl_stat)
         IF (dsrec_dcl_stat=0)
          SET dm_err->emsg = concat(
           "Errors found when copying secondary check file to primary check file.",
           "Command being executed:",dsrec_copy_text)
          SET dm_err->user_action = "Verify that destination directory has available space."
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         ENDIF
        ENDIF
       ELSE
       ...
       ENDIF
      ENDIF
      SET dm_err->err_ind = dsrc_err_ret
      IF ((dm_err->err_ind=0))
       COMMIT
       RETURN(null)
      ENDIF
     ENDIF
    ENDIF
    IF ((inhouse_misc->nonfk_err_ind=0)
     AND (dm2_install_schema->process_option="INHOUSE"))
     IF (dsr_op_type="CREATE FK CONSTRAINT")
      IF ((dm_err->debug_flag > 0))
       CALL echo(dm_err->emsg)
      ENDIF
      IF (currdb="DB2UDB")
       SET dsrec_exist = findstring("SQL0204N",dm_err->emsg)
      ELSEIF (currdb="ORACLE")
       IF (((findstring("ORA-00942",dm_err->emsg) > 0) OR (findstring("ORA-02270",dm_err->emsg) > 0
       )) )
        SET dsrec_exist = 1
       ELSE
        SET dsrec_exist = 0
       ENDIF
      ELSEIF (currdb="SQLSRV")
       SET dsrec_exist = findstring("SQLSRV-01767",dm_err->emsg)
      ENDIF
      IF (dsrec_exist=0)
       SET inhouse_misc->nonfk_err_ind = 1
      ELSE
       IF ((inhouse_misc->fk_err_ind=0))
        SET dsrec_start_pos = findstring(" REFERENCES ",dsrec_operation,1,0)
        SET dsrec_end_pos = findstring("(",dsrec_operation,dsrec_start_pos,0)
        SET inhouse_misc->fk_parent_table = trim(substring((dsrec_start_pos+ 12),((dsrec_end_pos - 12
          ) - dsrec_start_pos),dsrec_operation))
        SET inhouse_misc->fk_err_ind = 1
       ENDIF
      ENDIF
      IF ((dm_err->debug_flag > 0))
       CALL echorecord(inhouse_misc)
      ENDIF
     ELSE
      SET inhouse_misc->nonfk_err_ind = 1
     ENDIF
    ...
    CASE (dsrec_cmd_type)
     OF "DM2_PUSH_CMD":
      SET dsrc_orig_emsg = dm_err->emsg
      ROLLBACK
      IF ((dm_err->debug_flag > 0))
       CALL echo("Rollback Occurred.")
      ENDIF
     OF "DM2_PUSH_DCL":
      SET dsrc_orig_emsg = dm_err->errtext
     OF "DB2_PUSH_DCL_W_CONNECT":
      SET dsrc_orig_emsg = dm_err->errtext
     OF "DM2_PUSH_DCL_NO_CHK":
      IF (dsr_op_type IN ("IMPORT*", "EXPORT*"))
       SET dsrc_orig_emsg = concat("Oracle Errors Encountered:",dsrec_err_msg)
      ELSE
       SET dsrc_orig_emsg = dm_err->errtext
      ENDIF
    ENDCASE
    IF (dsr_cmd_hrdprs_error=0)
     CALL dsr_execute_oe_command(dsrec_pword)
     SET dm_err->err_ind = 1
    ENDIF
   ELSE
    COMMIT
   ENDIF
 END ;Subroutine
 SUBROUTINE dsr_execute_oe_command(dsreoc_pword)
   DECLARE dsreoc_cmd_type_oe = vc WITH private, noconstant(" ")
   DECLARE dsreoc_oe_operation = vc WITH private, noconstant(" ")
   IF (textlen(trim(dsr_oe_command)) > 0
    AND substring(1,1,dsr_oe_command) != ";")
    SET dm_err->err_ind = 0
    CASE (substring(1,6,dsr_oe_command))
     OF "%DCL1%":
      SET dsreoc_cmd_type_oe = "DM2_PUSH_DCL"
     OF "%DCL2%":
      SET dsreoc_cmd_type_oe = "DB2_PUSH_DCL_W_CONNECT"
     ELSE
      SET dsreoc_cmd_type_oe = "DM2_PUSH_CMD"
    ENDCASE
    SET dsreoc_oe_operation = replace(dsr_oe_command,"%DCL1%","",0)
    SET dsreoc_oe_operation = replace(dsreoc_oe_operation,"%DCL2%","",0)
    SET dsreoc_oe_operation = replace(dsreoc_oe_operation,"%FLOC%",dm2_install_schema->cer_install,
     0)
    SET dsreoc_oe_operation = replace(dsreoc_oe_operation,"%FLOC2%",dm2_install_schema->ccluserdir,
     0)
    SET dsreoc_oe_operation = replace(dsreoc_oe_operation,"%SNAME%",dm2_install_schema->servername,
     0)
    SET dsreoc_oe_operation = replace(dsreoc_oe_operation,"%UNAME%",trim(currdbuser),0)
    SET dsreoc_oe_operation = replace(dsreoc_oe_operation,"%PWD%",dsreoc_pword,0)
    SET dsreoc_oe_operation = replace(dsreoc_oe_operation,"%DBASE%",trim(validate(currdbname," ")),
     0)
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("dsreoc_cmd_type_oe: ",dsreoc_cmd_type_oe))
    ENDIF
    CASE (dsreoc_cmd_type_oe)
     OF "DM2_PUSH_CMD":
      CALL dm2_push_cmd(dsreoc_oe_operation,1)
     OF "DM2_PUSH_DCL":
      CALL dm2_push_dcl(dsreoc_oe_operation)
     OF "DB2_PUSH_DCL_W_CONNECT":
      CALL db2_push_dcl_w_connect(dm2_install_schema->dbase_name,dm2_install_schema->u_name,
       dsreoc_pword,dsreoc_oe_operation,1)
    ENDCASE
    IF ((dm_err->err_ind=0))
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat("Committing the On Error Command: ",dsreoc_oe_operation))
     ENDIF
     COMMIT
    ELSE
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat("Rolling back the On Error Command: ",dsreoc_oe_operation))
     ENDIF
     ROLLBACK
    ENDIF
   ELSE
    IF ((dm_err->debug_flag > 0))
     CALL echo(
      "***  The was no on_error command found or executed for the operation that errored out ***")
    ENDIF
   ENDIF
 END ;Subroutine
 SUBROUTINE dsr_sqlsrv_import_errchk(dsrsie_oe_operation)
   DECLARE dsrsie_err_filename = vc WITH private, noconstant(" ")
   DECLARE dsrsie_begpos = i4 WITH protect, noconstant(0)
   DECLARE dsrsie_fileendpos = i4 WITH protect, noconstant(0)
   DECLARE dsrsie_filenamelength = i4 WITH protect, noconstant(0)
   DECLARE dsrsie_import_error_fnd = i2 WITH protect, noconstant(0)
   SET dsrsie_begpos = findstring("-e",dsrsie_oe_operation,1)
   SET dsrsie_fileendpos = findstring("exp.err",dsrsie_oe_operation,1)
   SET dsrsie_filenamelength = (((dsrsie_fileendpos+ 6) - (dsrsie_begpos+ 3))+ 1)
   SET dsrsie_err_filename = substring((dsrsie_begpos+ 3),dsrsie_filenamelength,dsrsie_oe_operation)
   IF ((dm_err->debug_flag > 0))
    CALL echo("***  Performing special error log checks for SQL Server import command ***")
    CALL echo(concat("***  Parsing:  ",dm_err->errfile," for errors..."))
   ENDIF
   SET dm_err->eproc = "Checking for import errors."
   IF (parse_errfile(dm_err->errfile)=0)
    RETURN(0)
   ENDIF
   IF (findstring("Error",dm_err->errtext,1) > 0)
    SET dsrc_orig_emsg = dm_err->errtext
    SET dsrsie_import_error_fnd = (dsrsie_import_error_fnd+ 1)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("***  Parsing special import specific error file: ",dsrsie_err_filename,
      " for errors..."))
   ENDIF
   IF (parse_errfile(dsrsie_err_filename) != 1)
    RETURN(0)
   ELSE
    IF (textlen(trim(dm_err->errtext)) > 0)
     IF (dsrsie_import_error_fnd > 0)
      SET dsrc_orig_emsg = concat(dsrc_orig_emsg,
       "....Additional information related to this import error is in ","file ",dsrsie_err_filename)
     ELSE
      SET dsrc_orig_emsg = dm_err->errtext
     ENDIF
     SET dsrsie_import_error_fnd = (dsrsie_import_error_fnd+ 1)
   ...
   ENDIF
   IF (dsrsie_import_error_fnd > 0)
    CALL disp_msg(dsrc_orig_emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    SET dm_err->errtext = dsrc_orig_emsg
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dsr_create_sql_err_chk(dsrte_object_type,dsrte_object_name)
   DECLARE dsr_err_msg = vc WITH protect, noconstant("")
   DECLARE dcsec_trimmed_obj_name = vc WITH protect, noconstant("")
   IF (dsrte_object_type != "TRIGGER")
    SET dm_err->emsg = concat("Object type <",dsrte_object_type,"> is not currently supported.")
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (findstring(".",dsrte_object_name,1) > 0)
    SET dcsec_trimmed_obj_name = substring((findstring(".",dsrte_object_name,1)+ 1),((textlen(
      dsrte_object_name) - findstring(".",dsrte_object_name,1))+ 1),dsrte_object_name)
   ELSE
    SET dcsec_trimmed_obj_name = dsrte_object_name
   ENDIF
   SET dm_err->eproc = "Selecting from dba_errors to see if object was created successfully."
   SELECT INTO "nl:"
    FROM dba_errors ue
    WHERE ue.type=dsrte_object_type
     AND ue.name=dcsec_trimmed_obj_name
    ORDER BY ue.sequence, ue.line, ue.position
    DETAIL
     dsr_err_msg = concat(dsr_err_msg,"  ",ue.text)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSEIF (curqual > 0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = dsr_err_msg
    RETURN(1)
   ENDIF
   SET dm_err->eproc = "Selecting from user_objects to see if object is in a valid status."
   SELECT INTO "nl:"
    FROM dba_objects uo
    WHERE uo.object_type=dsrte_object_type
     AND uo.object_name=dcsec_trimmed_obj_name
     AND "VALID"=uo.status
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   ELSEIF (curqual=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Object not in a valid status."
    RETURN(1)
   ENDIF
   RETURN(1)
 END ;Subroutine
