CREATE PROGRAM bhs_athn_get_order_comment
 DECLARE orderid = f8 WITH protect, constant( $2)
 SELECT INTO  $1
  comment_dt = datetimezoneformat(l.active_status_dt_tm,curtimezonesys,"MM/dd/yyyy HH:mm:ss",
   curtimezonedef), comments = substring(1,31500,trim(replace(replace(replace(replace(replace(l
         .long_text,"&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3)),
  comment_type = trim(replace(replace(replace(replace(replace(uar_get_code_display(o.comment_type_cd),
        "&","&amp;",0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3),
