CREATE PROGRAM dm_eucb_encntr_data_not_coll:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_EUCB_ENCNTR_DATA_NOT_COLL"
 ...
 SUBROUTINE cust_ucb_del(dummy)
   UPDATE  FROM encntr_data_not_coll
    ...
    WHERE (encntr_data_not_coll_id=rchildren->qual1[det_cnt].entity_id)
    WITH nocounter
   ;end update
   IF (curqual=0)
   ...
   IF (add_hist(rchildren->qual1[det_cnt].entity_id)=0)
    ...
    SET error_table = rchildren->qual1[det_cnt].entity_name
    SET request->error_message = substring(1,132,build(
      "Could not insert history for encntr_data_not_coll with pk=",rchildren->qual1[det_cnt].
      entity_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 ...
 SUBROUTINE initialize_hist_id(null)
  IF (v_hist_init_ind=0)
   ...
   SET dcipht_request->transaction_reason_txt = "DM_EUCB_ENCNTR_DATA_NOT_COLL"
