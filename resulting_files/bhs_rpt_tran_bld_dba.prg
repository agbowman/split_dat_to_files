CREATE PROGRAM bhs_rpt_tran_bld:dba
 PROMPT
 ...
 DECLARE mf_factorunitofmeasure = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,
   "FACTORUNITOFMEASURE")), protect
 DECLARE mf_factordosegiven = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,"FACTORDOSEGIVEN")),
 protect
 DECLARE mf_iftransfusiontimeis241minexplain = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,
   "IFTRANSFUSIONTIMEIS241MINEXPLAIN")), protect
 DECLARE mf_istransfusiontime241minutes = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,
   "ISTRANSFUSIONTIME241MINUTES")), protect
 DECLARE mf_transfusiontimetocompletion = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,
   "TRANSFUSIONTIMETOCOMPLETION")), protect
 ...
 ELSEIF (datetimediff(end_date_qual,beg_date_qual) > 15)
  SELECT INTO  $OUTDEV
   FROM dummyt
   HEAD REPORT
    msg1 = "Your date range is larger than 15 days.", msg2 = "  Please retry.", col 0,
 ...
 IF (( $FTPFILES=1))
  ...
  SET var_output = build(logical("bhscust"),"/ftp/bhs_rpt_tran_bld/",name_fac,"_","forms_",
   from_month,"_",from_day,"_",from_year,
   "_",time1,".csv")
  SET var_output1 = build(logical("bhscust"),"/ftp/bhs_rpt_tran_bld/",name_fac,"_","labs_",
   from_month,"_",from_day,"_",from_year,
   "_",time1,".csv")
  SET var_output2 = build("trans_med_by_class_",month1,"_",day1,"_",
   time1,".csv")
  SET var_output3 = build(logical("bhscust"),"/ftp/bhs_rpt_tran_bld/",name_fac,"_","pre_meds",
   from_month,"_",from_day,"_",from_year,
   "_",time1,".csv")
  SET filedelimiter1 = '"'
  SET filedelimiter2 = ","
 ...
 RECORD forms(
   1 patientslist[*]
     ...
     2 forminstanceslist[*]
       ...
       3 s_factorunitofmeasure = vc
       3 s_factordosegiven = vc
       3 s_istransfusiontime241minutes = vc
       3 s_iftransfusiontimeis241minexplain = vc
       3 s_transfusiontimetocompletion = vc
       3 s_bprnwitnessreqd = vc
       3 s_bptransfusedrnwitreqd = vc
       3 s_bptransfusedrnwitnotreqd = vc
       3 s_convalescentplasmavol_cd = vc
       3 s_otherfactorderivativetransfused = vc
 ) WITH protect
 DECLARE mf_bloodproductcomment = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,
   "BLOODPRODUCTCOMMENT")), protect
 ...
 DECLARE mf_bprnwitnessreqd_cd = f8 WITH protect, constant(validatecodevalue("DISPLAYKEY",72,
   "BLOODPRODUCTRNWITNESSREQUIRED"))
 DECLARE mf_convalescentplasmavol_cd = f8 WITH protect, constant(validatecodevalue("DISPLAYKEY",72,
   "CONVALESCENTPLASMAVOL"))
 DECLARE mf_bptransfusedrnwitnessreqd_cd = f8 WITH protect, constant(validatecodevalue("DISPLAY",72,
   "Blood Products Transfused-RN Wit Reqd"))
 DECLARE mf_bptransfusedrnwitnotreqd_cd = f8 WITH protect, constant(validatecodevalue("DISPLAYKEY",72,
   "BLOODPRODUCTSTRANSFUSEDRNWITNOTREQD"))
 DECLARE mf_otherfactorderivativetransfused_cd = f8 WITH protect, constant(validatecodevalue(
   "DISPLAYKEY",72,"OTHERFACTORDERIVATIVETRANSFUSED"))
 ...
 DECLARE bloodproductcomment = vc WITH noconstant(" ")
 ...
 SELECT INTO "nl:"
  ...
  HEAD el.encntr_id
   ...
   forms->patientslist[i_patient_cnt].age = trim(cnvtage(cnvtdatetimeutc(datetimezone(p.birth_dt_tm,p
       .birth_tz),1)),3), forms->patientslist[i_patient_cnt].admit_date = format(e.reg_dt_tm,
    "mm/dd/yy hh:mm;;d"), forms->patientslist[i_patient_cnt].discharge_date = format(e.disch_dt_tm,
    "mm/dd/yy hh:mm;;d"),
  ...
  WITH nocounter
 ;end select
 SET endtime = cnvtdatetime(sysdate)
 CALL echo("##################")
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1
   ...
   unitidnumberlotnumber, mf_bloodproductcomment, mf_transfusionstarttime,
   mf_temperaturestart, mf_temperatureroutestart, mf_pulseratestart, mf_respiratoryratestart,
   mf_systolicbloodpressurestart,
   mf_diastolicbloodpressurestart, mf_oxygensaturationstart, autotransfusionstarttime,
   ...
   dbpstartautotransfusion, oxygensatstartautotransfusion,
   mf_transfusionstartplus15min, mf_temperature15min, mf_temperatureroute15min, mf_pulserate15min,
   mf_respiratoryrate15min,
   mf_systolicbloodpressure15min, mf_diastolicbloodpressure15min, mf_oxygensaturation15min,
   ...
   oxygensat15minautotransfusion, mf_transfusionendtime, mf_temperatureend, mf_temperaturerouteend,
   mf_pulserateend,
   mf_respiratoryrateend, mf_systolicbloodpressureend, mf_diastolicbloodpressureend,
   mf_oxygensaturationend, autotransfusionstoptime,
   temperatureendautotransfusion, temperaturerouteendautotransfusion, pulserateendautotransfusion,
   respiratoryrateendautotransfusion, sbpendautotransfusion,
   dbpendautotransfusion, oxygensatendautotransfusion, mf_albuminvol, mf_cryoprecipitate,
   mf_factorviia,
   mf_factorviiivol, mf_factorixcomplex, mf_factorixvol, mf_ffp, mf_granulocytes,
   mf_ivig, mf_platelets, mf_rbcvol, mf_rhimmuneglobulin, mf_bloodproductamountinfused,
   ...
   meanarterialpressure15min_calc, meanarterialpressuredelta1_calc, meanarterialpressuredelta2_calc,
   meanarterialpressureend_calc, meanarterialpressurestart_calc,
   oxygensaturationdelta1_calc, oxygensaturationdelta2_calc, pulsepressure15min_calc,
   pulsepressure15minautotransfusion_calc, pulsepressuredelta1_calc,
   pulsepressuredelta1autotransfusion_calc, pulsepressuredelta2_calc,
   pulsepressuredelta2autotransfusion_calc, pulsepressureend_calc,
   pulsepressureendautotransfusion_calc,
   pulsepressurestart_calc, pulsepressurestartautotransfusion_calc, pulseratedelta1_calc,
   pulseratedelta1autotransfusion_calc, pulseratedelta2_calc,
   pulseratedelta2autotransfusion_calc, respiratoryratedelta1_calc,
   respiratoryratedelta1autotransfusion_calc, respiratoryratedelta2_calc,
   respiratoryratedelta2autotransfusion_calc,
   systolicbloodpressuredelta1_calc, systolicbloodpressuredelta2_calc, temperaturedelta1_calc,
   temperaturedelta1autotransfusion_calc, temperaturedelta2_calc,
   temperaturedelta2autotransfusion_calc, transfusionratecryoprecipitate_calc,
   transfusionrateffp_calc, transfusionrateivig_calc, transfusionrateplatelets_calc,
   transfusionraterbcs_calc, transfusiontime_calc, additionalunitdocumentation,
   mf_factorunitofmeasure, mf_factordosegiven,
   mf_iftransfusiontimeis241minexplain, mf_istransfusiontime241minutes,
   mf_transfusiontimetocompletion, mf_bprnwitnessreqd_cd, mf_convalescentplasmavol_cd,
   mf_bptransfusedrnwitnessreqd_cd, mf_bptransfusedrnwitnotreqd_cd,
   mf_otherfactorderivativetransfused_cd))
   JOIN (pr
  ...
  HEAD ce2.event_cd
   ...
   ELSEIF (ce2.event_cd IN (mf_transfusionstarttime, autotransfusionstarttime))
    ...
    d2.seq].d_transfusionstarttime = cnvtdatetime(s_tran_result), i_vital_cnt += 1
   ...
   ELSEIF (ce2.event_cd IN (mf_transfusionstartplus15min, autotransfusionstartplus15min))
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_transfusionstartplus15min = format(
     cnvtdatetime(s_tran_result),"mm/dd/yy hh:mm;;d"), i_vital_cnt += 1
   ...
   ELSEIF (ce2.event_cd IN (mf_transfusionendtime, autotransfusionstoptime))
    ...
    d_transfusionendtime = cnvtdatetime(s_tran_result), i_vital_cnt += 1
   ...
   ELSEIF (ce2.event_cd=mf_ffp)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_ffp = s_tran_result, i_infused_cnt += 1
   ...
   ELSEIF (ce2.event_cd=mf_ivig)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_ivig = s_tran_result, i_infused_cnt += 1
   ...
   ELSEIF (ce2.event_cd=mf_transfusionreactiondescription)
    ...
    s_tran_result, i_reaction_cnt += 1
   ...
   ELSEIF (ce2.event_cd IN (mf_bloodproductcomment))
    CALL echo(build("blood commment ce2_result_val == ",ce2.result_val)),
    CALL echo("MF_BLOODPRODUCTCOMMENT start"), forms->patientslist[d1.seq].forminstanceslist[d2.seq].
    s_bloodproductcomment = ce2.result_val,
    CALL echo("MF_BLOODPRODUCTCOMMENT end")
   ELSEIF (ce2.event_cd=additionalunitdocumentation)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].additional_unit_documentation =
    s_tran_result
   ELSEIF (ce2.event_cd=mf_factorunitofmeasure)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_factorunitofmeasure = trim(s_tran_result,
     3)
   ELSEIF (ce2.event_cd=mf_factordosegiven)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_factordosegiven = trim(s_tran_result,3)
   ELSEIF (ce2.event_cd=mf_iftransfusiontimeis241minexplain)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_iftransfusiontimeis241minexplain = trim(
     s_tran_result,3)
   ELSEIF (ce2.event_cd=mf_istransfusiontime241minutes)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_istransfusiontime241minutes = trim(
     s_tran_result,3)
   ELSEIF (ce2.event_cd IN (mf_transfusiontimetocompletion))
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_transfusiontimetocompletion = concat(trim
     (ce2.result_val,3)," ",trim(uar_get_code_display(ce2.result_units_cd),3))
   ELSEIF (ce2.event_cd=mf_bprnwitnessreqd_cd)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_bprnwitnessreqd = trim(s_tran_result,3)
   ELSEIF (ce2.event_cd=mf_bptransfusedrnwitnessreqd_cd)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_bptransfusedrnwitreqd = trim(
     s_tran_result,3)
   ELSEIF (ce2.event_cd=mf_bptransfusedrnwitnotreqd_cd)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_bptransfusedrnwitnotreqd = trim(
     s_tran_result,3)
   ELSEIF (ce2.event_cd=mf_otherfactorderivativetransfused_cd)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_otherfactorderivativetransfused = trim(
     s_tran_result,3)
   ELSEIF (ce2.event_cd=mf_convalescentplasmavol_cd)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_convalescentplasmavol_cd = trim(
     s_tran_result,3)
   ENDIF
  FOOT  ce2.event_cd
   stat = 0
  FOOT  d2.seq
   IF (i_consent_cnt=1
    ...
    AND i_witness_cnt=1)
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_incompliance = "Yes", formscomplete += 1
   ELSE
    forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_incompliance = "No", formsincomplete += 1
   ENDIF
   totalforms += 1, old_d1seq = d1.seq
  FOOT  d1.seq
   i_instance_cnt = 0
 ...
 SELECT INTO value(trim(var_output))
  ...
  additionalunitdocumentation = trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].
   additional_unit_documentation), blood_product_commment = trim(replace(replace(forms->patientslist[
     d1.seq].forminstanceslist[d2.seq].s_bloodproductcomment,char(13)," "),char(10)," ")),
  factorunitofmeasure = substring(1,25,trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].
    s_factorunitofmeasure)),
  factordosegiven = substring(1,25,trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].
    s_factordosegiven)), transfusiontimetocompletion = substring(1,25,trim(forms->patientslist[d1.seq
    ].forminstanceslist[d2.seq].s_transfusiontimetocompletion)), istransfusiontime241minutes =
  substring(1,25,trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].
    s_istransfusiontime241minutes)),
  iftransfusiontime241minexplain = substring(1,25,trim(forms->patientslist[d1.seq].forminstanceslist[
    d2.seq].s_iftransfusiontimeis241minexplain)), bloodproductrnwitnessrequired = substring(1,25,trim
   (forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_bprnwitnessreqd)),
  bloodproductstransfusedrnwitnessrequired = substring(1,25,trim(forms->patientslist[d1.seq].
    forminstanceslist[d2.seq].s_bptransfusedrnwitreqd)),
  bloodproductstransfusedrnwitnessnotrequired = substring(1,25,trim(forms->patientslist[d1.seq].
    forminstanceslist[d2.seq].s_bptransfusedrnwitnotreqd)), convalescentplasmavol = substring(1,25,
   trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].s_convalescentplasmavol_cd)),
  otherfactorderivativetransfused = substring(1,25,trim(forms->patientslist[d1.seq].
    forminstanceslist[d2.seq].s_otherfactorderivativetransfused))
  FROM (dummyt d1  WITH seq = value(size(forms->patientslist,5))),
   (dummyt d2  WITH seq = 1)
  PLAN (d1
   WHERE maxrec(d2,size(forms->patientslist[d1.seq].forminstanceslist,5))
