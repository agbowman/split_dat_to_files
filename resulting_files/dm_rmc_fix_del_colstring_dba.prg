CREATE PROGRAM dm_rmc_fix_del_colstring:dba
 ...
 END ;Subroutine
 IF ( NOT (validate(drfdc_request,0)))
  FREE RECORD drfdc_request
  RECORD drfdc_request(
    1 log_id = f8
    1 pk_where = vc
    1 table_name = vc
    1 table_suffix = vc
    1 pk_where_hash = f8
    1 ptam_match_hash = f8
    1 context_set_ind = i2
    1 update_ind = i2
    1 log_type = vc
    1 context_name = vc
    1 target_env_id = f8
    1 nd_pk_where_hash = f8
  )
  FREE RECORD drfdc_reply
  RECORD drfdc_reply(
    1 status = c1
    1 err_ind = i2
    1 emsg = vc
 ...
 ENDIF
 IF ( NOT (validate(drpp_request,0)))
  FREE RECORD drpp_request
  RECORD drpp_request(
    1 pk_where = vc
    1 table_name = vc
  )
  FREE RECORD drpp_reply
  RECORD drpp_reply(
    1 col_cnt = i4
    1 cols[*]
      2 col_name = vc
 ...
 DECLARE v_pkw_in = vc WITH protect
 DECLARE v_tab_in = vc WITH protect
 DECLARE v_col_idx = i4 WITH protect
 DECLARE v_colstring = vc WITH protect
 DECLARE v_flag = i4 WITH protect
 DECLARE v_replace_str = vc WITH protect
 DECLARE v_type = c1 WITH protect
 DECLARE v_dt_tm_val = vc WITH protect
 DECLARE v_str_val = vc WITH protect
 DECLARE v_num_val = vc WITH protect
 DECLARE v_new_colstring = vc WITH protect
 DECLARE v_tab_suffix = vc WITH protect
 DECLARE v_ptam_result = f8 WITH protect
 DECLARE v_update_ind = i2 WITH protect
 DECLARE v_nd_pkw = vc WITH protect
 DECLARE v_nd_ptam_result = f8 WITH protect
 DECLARE drfdc_log_type = vc WITH protect
 DECLARE refchg_colstring_get_col() = c4000
 DECLARE dm2_context_control_wrapper() = i2
 DECLARE sys_context() = c4000
 DECLARE refchg_colstring_replace() = i2
 FREE RECORD row_info
 RECORD row_info(
   1 col_cnt = i4
   1 pk_col[*]
     2 col_name = vc
     2 data_type = vc
     2 value = vc
     2 colstring_value = vc
     2 replace_ind = i2
 )
 SET drfdc_reply->status = "F"
 IF ((drfdc_request->pk_where > ""))
  SET v_pkw_in = drfdc_request->pk_where
 ELSE
  SET v_pkw_in =  $1
 ENDIF
 IF ((drfdc_request->table_name > ""))
  SET v_tab_in = drfdc_request->table_name
 ELSE
  SET v_tab_in =  $2
 ENDIF
 IF ((drfdc_request->table_suffix > ""))
  SET v_tab_suffix = drfdc_request->table_suffix
 ELSE
  SELECT INTO "nl:"
   FROM dm_tables_doc d
   WHERE d.table_name=d.full_table_name
    AND d.table_name=v_tab_in
   DETAIL
    v_tab_suffix = d.table_suffix
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   SET drfdc_reply->err_ind = 1
   SET drfdc_reply->emsg = dm_err->emsg
   GO TO exit_program
  ENDIF
 ENDIF
 IF (validate(drfdc_colstring,"VALID")="VALID")
  DECLARE drfdc_colstring = vc
  SET drfdc_colstring = ""
 ENDIF
 IF ((drfdc_request->context_set_ind=0)
  AND (drfdc_request->log_id > 0.0))
  SELECT INTO "nl:"
   dm2_context_control_wrapper("RDDS_COL_STRING",d.col_string)
   FROM dm_chg_log d
   WHERE (d.log_id=drfdc_request->log_id)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   GO TO exit_program
  ENDIF
 ELSEIF (drfdc_colstring > " "
  AND (drfdc_request->context_set_ind=0))
  SELECT INTO "nl:"
   dm2_context_control_wrapper("RDDS_COL_STRING",drfdc_colstring)
   FROM dual
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_program
  ENDIF
 ELSEIF ((drfdc_request->context_set_ind=0))
  CALL echo("Invalid set-up the col_string context needs to be set or a log_id should be passed in.")
  GO TO exit_program
 ENDIF
 SET drpp_request->pk_where = v_pkw_in
 SET drpp_request->table_name = v_tab_in
 EXECUTE dm_rmc_parse_pkw
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  SET drfdc_reply->err_ind = 1
  SET drfdc_reply->emsg = dm_err->emsg
  GO TO exit_program
 ENDIF
 IF ((drpp_reply->col_cnt > 0))
  SET row_info->col_cnt = drpp_reply->col_cnt
  SET stat = alterlist(row_info->pk_col,row_info->col_cnt)
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(row_info->col_cnt))
   DETAIL
    row_info->pk_col[d.seq].col_name = drpp_reply->cols[d.seq].col_name, row_info->pk_col[d.seq].
    data_type = drpp_reply->cols[d.seq].data_type, row_info->pk_col[d.seq].value = drpp_reply->cols[d
    .seq].col_value
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
 ENDIF
 FOR (v_idx = 1 TO row_info->col_cnt)
   IF ((row_info->pk_col[v_idx].data_type IN ("Q8", "DQ8")))
    SET v_flag = 2
   ELSEIF ((row_info->pk_col[v_idx].data_type="*C*"))
    SET v_flag = 1
   ELSE
    SET v_flag = 0
   ENDIF
   SELECT INTO "nl:"
    v_str = refchg_colstring_get_col(sys_context("CERNER","RDDS_COL_STRING",4000),row_info->pk_col[
     v_idx].col_name,v_flag,0," ",
     0,0,"FROM")
    FROM dual
    DETAIL
     row_info->pk_col[v_idx].colstring_value = v_str
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   FREE SET pkw
   FREE SET cstr
   CALL parser(concat("declare pkw = ",row_info->pk_col[v_idx].data_type," go"),1)
   CALL parser(concat("declare cstr = ",row_info->pk_col[v_idx].data_type," go"),1)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET drfdc_reply->err_ind = 1
    SET drfdc_reply->emsg = dm_err->emsg
   ...
   SET null_ind = 0
   SET tspace_ind = 0
   IF ((((row_info->pk_col[v_idx].value="NULL")) OR ((row_info->pk_col[v_idx].colstring_value="NULL")
   )) )
    SET null_ind = 1
   ELSEIF ((row_info->pk_col[v_idx].data_type IN ("Q8", "DQ8")))
    CALL parser(concat("set pkw = ",row_info->pk_col[v_idx].value," go"),1)
    CALL parser(concat("set cstr = ",row_info->pk_col[v_idx].colstring_value," go"),1)
   ELSEIF ((row_info->pk_col[v_idx].data_type="*C*"))
    CALL parser(concat("set pkw = ",row_info->pk_col[v_idx].value," go"),1)
    CALL parser(concat("set cstr = ",row_info->pk_col[v_idx].colstring_value," go"),1)
    IF ((row_info->pk_col[v_idx].data_type="VC")
     AND findstring("CONCAT",row_info->pk_col[v_idx].value)=0)
     IF (((size(row_info->pk_col[v_idx].value,1) - 2) != size(pkw,1))
      AND findstring("CONCAT",row_info->pk_col[v_idx].value)=0
      AND findstring("CONCAT",row_info->pk_col[v_idx].colstring_value)=0)
      SET tspace_ind = 1
     ENDIF
    ENDIF
   ELSEIF ((row_info->pk_col[v_idx].data_type="I*"))
    CALL parser(concat("set pkw = cnvtint(",row_info->pk_col[v_idx].value,") go"),1)
    CALL parser(concat("set cstr = cnvtint(",row_info->pk_col[v_idx].colstring_value,") go"),1)
   ELSE
    CALL parser(concat("set pkw = cnvtreal(",row_info->pk_col[v_idx].value,") go"),1)
    CALL parser(concat("set cstr = cnvtreal(",row_info->pk_col[v_idx].colstring_value,") go"),1)
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET drfdc_reply->err_ind = 1
    ...
    GO TO exit_program
   ENDIF
   IF ((row_info->pk_col[v_idx].value="NULL")
    AND (row_info->pk_col[v_idx].colstring_value="NULL"))
    SET row_info->pk_col[v_idx].replace_ind = 0
   ELSEIF (tspace_ind=1)
    IF ((row_info->pk_col[v_idx].value=row_info->pk_col[v_idx].colstring_value))
     SET row_info->pk_col[v_idx].replace_ind = 0
    ELSE
     SET row_info->pk_col[v_idx].replace_ind = 1
    ENDIF
   ELSEIF (pkw=cstr
    AND null_ind != 1)
    SET row_info->pk_col[v_idx].replace_ind = 0
   ...
   ENDIF
 ENDFOR
 IF ((drfdc_request->update_ind=1)
  AND (drfdc_request->log_id > 0.0)
  AND (drfdc_request->log_type="BADMRG"))
  SET v_nd_pkw = drmm_get_pk_where(v_tab_in,v_tab_suffix,0," ")
  CALL drmm_get_ptam_match_query(0.0,v_tab_in,v_tab_suffix,"")
  SET v_nd_ptam_result = drmm_get_ptam_match_result(0.0,0.0,0.0,"",1,
   "")
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   SET drfdc_reply->err_ind = 1
   ...
   GO TO exit_program
  ENDIF
  SET v_nd_pkw = replace_carrot_symbol(v_nd_pkw)
  CALL parser(concat("RDB ASIS(^ BEGIN proc_refchg_ins_dcl('",v_tab_in,"',^)"),0)
  CALL parser(concat("ASIS(^ ",v_nd_pkw,",^)"),0)
  CALL parser(concat("ASIS(^ dbms_utility.get_hash_value(",v_nd_pkw,",0,1073741824.0)",
    ",'REFCHG',0,0,0,0^)"),0)
  CALL parser(concat("ASIS(^,'",trim(drfdc_request->context_name),"',",trim(cnvtstring(drfdc_request
      ->target_env_id)),".0,sys_context('CERNER','RDDS_PTAM_MATCH_QUERY',2000),",
    trim(cnvtstring(v_nd_ptam_result)),".0,sys_context('CERNER','RDDS_COL_STRING',4000)",",",trim(
     cnvtstring(drfdc_request->nd_pk_where_hash)),".0,",
    trim(cnvtstring(drfdc_request->ptam_match_hash)),".0,0.0^)"),0)
  CALL parser(concat("ASIS(^","); END; ^) go"),1)
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   SET drfdc_reply->err_ind = 1
   SET drfdc_reply->emsg = dm_err->emsg
 ...
 ENDIF
 FOR (v_idx = 1 TO row_info->col_cnt)
   IF ((row_info->pk_col[v_idx].replace_ind=1))
    SET v_replace_str = concat(
     "select into 'nl:' refchg_colstring_replace(SYS_CONTEXT('CERNER','RDDS_COL_STRING',4000),",
     "row_info->pk_col[v_idx].col_name, ")
    SET v_dt_tm_val = "NULL"
    SET v_str_val = "NULL"
    SET v_num_val = "NULL"
    IF ((row_info->pk_col[v_idx].data_type IN ("Q8", "DQ8")))
     SET v_type = "2"
     SET v_dt_tm_val = row_info->pk_col[v_idx].value
    ELSEIF ((row_info->pk_col[v_idx].data_type="*C*"))
     SET v_type = "1"
     SET v_str_val = row_info->pk_col[v_idx].value
    ELSE
     SET v_type = "3"
     SET v_num_val = row_info->pk_col[v_idx].value
    ENDIF
    SET v_replace_str = concat(v_replace_str,"v_type,",v_str_val,",",v_dt_tm_val,
     ",",v_num_val,") from dual with nocounter,notrim go")
    CALL parser(v_replace_str,1)
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     SET drfdc_reply->err_ind = 1
     SET drfdc_reply->emsg = dm_err->emsg
 ...
 ENDFOR
 CALL drmm_get_ptam_match_query(0.0,v_tab_in,v_tab_suffix,"")
 SET v_ptam_result = drmm_get_ptam_match_result(0.0,0.0,0.0,"",1,
  "")
 SET drfdc_reply->ptam_match_result = v_ptam_result
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  SET drfdc_reply->err_ind = 1
  SET drfdc_reply->emsg = dm_err->emsg
 ...
 IF ((drfdc_request->update_ind=1)
  AND v_update_ind=1
  AND (drfdc_request->log_id > 0.0))
  SET dm_err->eproc = "Fixing PTAM columns."
  UPDATE  FROM dm_chg_log d
   SET d.log_type = "REFCHG", d.col_string = sys_context("CERNER","RDDS_COL_STRING",4000), d
    .ptam_match_query = sys_context("CERNER","RDDS_PTAM_MATCH_QUERY",2000),
    d.ptam_match_result = v_ptam_result, d.pk_where_hash = drfdc_request->pk_where_hash, d
    .ptam_match_hash = drfdc_request->ptam_match_hash
   WHERE (d.log_id=drfdc_request->log_id)
  ...
  ELSE
   COMMIT
  ENDIF
 ELSEIF ((drfdc_request->update_ind=1)
  AND v_update_ind=0
  AND (drfdc_request->log_id > 0.0))
  IF ((drfdc_request->log_type="BADMRG"))
   SET drfdc_log_type = "MERGED"
  ELSE
   SET drfdc_log_type = "REFCHG"
  ENDIF
  UPDATE  FROM dm_chg_log d
   SET d.log_type = drfdc_log_type, d.updt_applctx = 431000901
   WHERE (d.log_id=drfdc_request->log_id)
   WITH nocounter, notrim
  ;end update
  IF (check_error(dm_err->eproc)=1)
