CREATE PROGRAM dm_mp_amb_org_sat_rows:dba
 ...
 SET reply->table_name = "MP_AMB_ORG_SAT"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 SELECT INTO "nl:"
  sat.rowid
  FROM mp_amb_org_sat sat
  WHERE ((sat.status_dt_tm+ 0) < cnvtdatetime(cnvtlookbehind(lookbehindstring)))
   AND sat.mp_amb_org_sat_id != 0
  DETAIL
   row_cnt = (row_cnt+ 1)
   IF (mod(row_cnt,100)=1)
    stat = alterlist(reply->rows,(row_cnt+ 99))
   ENDIF
   reply->rows[row_cnt].row_id = sat.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,row_cnt)
  WITH nocounter, maxqual(sat,value(cnvtint(request->max_rows)))
 ;end select
 ...
 IF (v_err_code2 != 0)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERROR","Failed in row collection: %1",
   "s",v_errmsg2)
  SET stat = alterlist(reply->rows,0)
  GO TO exit_script
 ENDIF
