CREATE PROGRAM bhs_prax_get_mar_comments
 ...
 DECLARE eventid = f8 WITH protect, constant(request->person[1].person_id)
 SELECT INTO value(moutputdevice)
  ...
  .note_prsnl_id,
  notedatetime = format(c.note_dt_tm,"MM/DD/YYYY HH:MM;;D"), notetimezone = substring(21,3,
   datetimezoneformat(c.note_dt_tm,c.note_tz,"MM/dd/yyyy hh:mm:ss ZZZ",curtimezonedef)),
  ...
  HEAD l.long_blob_id
   ...
   resulttype, row + 1, v1 = build("<NoteDateTime>",concat(notedatetime," ",notetimezone),
    "</NoteDateTime>"),
   col + 1, v1, row + 1,
