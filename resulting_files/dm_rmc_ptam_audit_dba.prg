CREATE PROGRAM dm_rmc_ptam_audit:dba
 ...
 SUBROUTINE dgi_get_global_info(dgi_global)
   ...
   RETURN(0)
 END ;Subroutine
 DECLARE drpa_src_id = f8 WITH protect, noconstant(0.0)
 DECLARE drpa_tgt_id = f8 WITH protect, noconstant(0.0)
 DECLARE drpa_tab_pos = i4 WITH protect, noconstant(0)
 DECLARE drpa_pos = i4 WITH protect, noconstant(0)
 DECLARE drpa_start = i4 WITH protect, noconstant(0)
 DECLARE drpa_idx = i4 WITH protect, noconstant(0)
 DECLARE drpa_pk_ind = i2 WITH protect, noconstant(0)
 DECLARE drpa_row_idx = i4 WITH protect, noconstant(0)
 FREE RECORD drpr_master_rs
 RECORD drpr_master_rs(
   1 dcl_row_cnt = i4
   1 table_name = vc
   1 pk_ind = i2
   1 dcl_row_qual[*]
 ...
 SET dm_err->eproc = "Starting dm_rmc_ptam_audit..."
 IF (check_logfile("dm_rmc_ptam_audit",".log","DM_RMC_PTAM_AUDIT LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ( NOT (validate(drpa_request,0)))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Required record structure has not been declared."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (daf_is_blank(drpa_request->table_name))
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "Request record structure not correctly populated ensure to provide at least a table_name"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ROLLBACK
 ...
 ELSEIF (daf_is_blank(drgi_global->db_link))
  ...
  GO TO exit_program
 ENDIF
 SET tgsc_req->db_link = drgi_global->db_link
 SET drpa_src_id = drgi_global->source_id
 SET drpa_tgt_id = drgi_global->target_id
 SET drpa_tab_pos = locateval(drpa_pos,1,tmc_md_info->tbl_cnt,drpa_request->table_name,tmc_md_info->
  tbl_qual[drpa_pos].table_name)
 IF (drpa_tab_pos=0)
  CALL drm_md_wrp(drpa_request->table_name,tgsc_req->db_link)
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET drpa_tab_pos = locateval(drpa_pos,1,tmc_md_info->tbl_cnt,drpa_request->table_name,tmc_md_info->
   tbl_qual[drpa_pos].table_name)
 ENDIF
 IF ((tmc_md_info->tbl_qual[drpa_tab_pos].multi_col_pk_ind=0))
  SET drpa_pk_ind = 1
 ENDIF
 SET tgsc_req->target_env_id = drpa_tgt_id
 SET tgsc_req->log_type = "NOMV19"
 SET tgsc_req->table_name = drpa_request->table_name
 IF ((drpa_request->ctxt_cnt > 0))
  SET tgsc_req->ctxt_cnt = drpa_request->ctxt_cnt
  SET stat = alterlist(tgsc_req->ctxt_qual,tgsc_req->ctxt_cnt)
  FOR (drpa_idx = 1 TO drpa_request->ctxt_cnt)
    SET tgsc_req->ctxt_qual[drpa_idx].ctxt_name = drpa_request->ctxt_qual[drpa_idx].ctxt_name
  ENDFOR
 ENDIF
 FOR (drpa_idx = 1 TO tmc_md_info->tbl_qual[drpa_tab_pos].col_cnt)
   IF ((tmc_md_info->tbl_qual[drpa_tab_pos].col_qual[drpa_idx].pk_ind=1))
    SET tgsc_req->xlat_col_cnt = (tgsc_req->xlat_col_cnt+ 1)
    SET stat = alterlist(tgsc_req->xlat_col_qual,tgsc_req->xlat_col_cnt)
    SET tgsc_req->xlat_col_qual[tgsc_req->xlat_col_cnt].xlat_col_name = tmc_md_info->tbl_qual[
    drpa_tab_pos].col_qual[drpa_idx].column_name
   ENDIF
 ENDFOR
 EXECUTE dm_rmc_get_source_rows
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drpr_master_rs->table_name = drpa_request->table_name
 SET drpr_master_rs->pk_ind = drpa_pk_ind
 FOR (drpa_idx = 1 TO tgsc_rep->row_cnt)
   IF ((((tgsc_rep->row_qual[drpa_idx].src_cnt > 0)) OR ((tgsc_rep->row_qual[drpa_idx].delete_ind=1)
   )) )
    SET drpr_master_rs->dcl_row_cnt = (drpr_master_rs->dcl_row_cnt+ 1)
    SET stat = alterlist(drpr_master_rs->dcl_row_qual,drpr_master_rs->dcl_row_cnt)
    SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].log_id = tgsc_rep->row_qual[
    drpa_idx].log_id
    SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].updt_id = tgsc_rep->row_qual[
    drpa_idx].updt_id
    SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].blocking_log_id = tgsc_rep->
    row_qual[drpa_idx].blocking_log_id
    SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].context_name = tgsc_rep->row_qual[
    drpa_idx].context_name
    SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].table_row_cnt = tgsc_rep->row_qual[
    drpa_idx].src_cnt
    SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].env_flg = 1
    SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].delete_ind = tgsc_rep->row_qual[
    drpa_idx].delete_ind
    SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].pk_where = tgsc_rep->row_qual[
    drpa_idx].pk_where
    SET stat = moverec(tgsc_rep->row_qual[drpa_idx].qual,drpr_master_rs->dcl_row_qual[drpr_master_rs
     ->dcl_row_cnt].table_row_qual)
   ENDIF
 ENDFOR
 SET stat = initrec(tgsc_rep)
 SET tgsc_req->log_type = "NOMV20"
 EXECUTE dm_rmc_get_source_rows
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FOR (drpa_idx = 1 TO tgsc_rep->row_cnt)
   IF (locateval(drpa_pos,1,drpr_master_rs->dcl_row_cnt,tgsc_rep->row_qual[drpa_idx].pk_where,
    drpr_master_rs->dcl_row_qual[drpa_pos].pk_where)=0
    AND (((tgsc_rep->row_qual[drpa_idx].src_cnt > 0)) OR ((tgsc_rep->row_qual[drpa_idx].delete_ind=1)
   )) )
    SET drpr_master_rs->dcl_row_cnt = (drpr_master_rs->dcl_row_cnt+ 1)
    SET stat = alterlist(drpr_master_rs->dcl_row_qual,drpr_master_rs->dcl_row_cnt)
    SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].log_id = tgsc_rep->row_qual[
   ...
   ENDIF
 ENDFOR
 SET stat = initrec(tgsc_req)
 SET stat = initrec(tgsc_rep)
 IF ((drpr_master_rs->dcl_row_cnt > 0))
  SET tgsc_req->target_env_id = drpa_src_id
  SET tgsc_req->table_name = drpa_request->table_name
  SET tgsc_req->log_cnt = drpr_master_rs->dcl_row_cnt
  SET stat = alterlist(tgsc_req->log_qual,tgsc_req->log_cnt)
  FOR (drpa_idx = 1 TO drpr_master_rs->dcl_row_cnt)
    SET tgsc_req->log_qual[drpa_idx].log_id = drpr_master_rs->dcl_row_qual[drpa_idx].blocking_log_id
  ENDFOR
  FOR (drpa_idx = 1 TO tmc_md_info->tbl_qual[drpa_tab_pos].col_cnt)
    IF ((tmc_md_info->tbl_qual[drpa_tab_pos].col_qual[drpa_idx].pk_ind=1))
     SET tgsc_req->xlat_col_cnt = (tgsc_req->xlat_col_cnt+ 1)
  ...
  FOR (drpa_idx = 1 TO tgsc_rep->row_cnt)
    IF ((((tgsc_rep->row_qual[drpa_idx].src_cnt > 0)) OR ((tgsc_rep->row_qual[drpa_idx].delete_ind=1)
     ...
     drpa_idx].updt_id
     SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].blocking_log_id = tgsc_rep->
     row_qual[drpa_idx].log_id
     SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].context_name = tgsc_rep->row_qual[
     drpa_idx].context_name
     SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].table_row_cnt = tgsc_rep->
     row_qual[drpa_idx].src_cnt
     SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].env_flg = 2
     SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].delete_ind = tgsc_rep->row_qual[
     drpa_idx].delete_ind
     SET drpr_master_rs->dcl_row_qual[drpr_master_rs->dcl_row_cnt].pk_where = tgsc_rep->row_qual[
     drpa_idx].pk_where
 ...
 ELSE
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Error: No PTAM rows were found for table ",drpa_request->table_name,
   " and the contexts passed in.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 EXECUTE dm_rmc_load_row_data  WITH replace("DRLRD_REQUEST","DRPR_MASTER_RS")
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
#exit_program
 SET dm_err->eproc = "...Ending dm_rmc_ptam_audit"
 CALL final_disp_msg("dm_rmc_ptam_audit")
END GO
