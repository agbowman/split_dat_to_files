CREATE PROGRAM ct_build_replaceprice:dba
 PAINT
 IF ("Z"=validate(ct_build_replaceprice_vrsn,"Z"))
  DECLARE ct_build_replaceprice_vrsn = vc WITH noconstant("65970.004")
 ENDIF
 RECORD ct_detail(
   1 detail_list[*]
     2 detailtype_cd = f8
...
#top
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY d.rule_entity_id
  HEAD d.rule_entity_id
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ct_detail->detail_list[d1.seq].detail_name = c.cdf_meaning
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  c.cdf_meaning
...
#the_second_prompt
 ...
 CASE (curscroll)
  OF 0:
   CASE (curaccept)
    OF "A":
     GO TO add_replaceprice
    OF "M":
     GO TO modify_replaceprice
    OF "D":
     GO TO delete_replaceprice
    OF "Q":
     GO TO end_prog
   ENDCASE
  ELSE
...
#add_replaceprice
 ...
 CALL text(9,3,"Result Factor:")
 CALL text(11,3,"Beg Date:")
 CALL text(13,3,"End Date:")
 ...
 CALL text(11,14,format(curdate,"DD-MMM-YYYY;;D"))
 CALL accept(11,14,"xx-xxx-xxxx;ucs",format(curdate,"DD-MMM-YYYY;;D"))
 SET v_beg_date = cnvtdatetime(curaccept)
 CALL text(13,14,format(v_end_date,"DD-MMM-YYYY;;D"))
 CALL accept(13,14,"xx-xxx-xxxx;ucs",format(v_end_date,"DD-MMM-YYYY;;D"))
 ...
 SET request->ct_rule_detail[1].operator_cd = cv->equal_operator
 IF (lookup_option=0)
...
#modify_replaceprice
 CALL clear(1,1)
 IF ((ct_detail->detail_list[cur_rec].result_detail_id=0))
  GO TO top
 ENDIF
 CALL box(1,1,15,70)
 CALL text(2,2,"Rule Name:")
 ...
 CALL text(8,5,"Result_Factor:")
 CALL text(10,5,"Beg Date:")
 ...
 ELSE
  CALL text(4,43,"0")
  CALL text(6,21,ct_detail->detali_list[cur_rec].cpt4_cd)
 ENDIF
 CALL text(8,20,format(ct_detail->detail_list[cur_rec].ct_result_factor,"###.##"))
 CALL text(10,15,format(ct_detail->detail_list[cur_rec].beg_date,"dd-mmm-yyyy;;d"))
 CALL text(12,15,format(ct_detail->detail_list[cur_rec].end_date,"dd-mmm-yyyy;;d"))
 SET help =
 SELECT
  nomenclature = n.nomenclature_id"########################################;l", n.source_identifier
  "##########;l", n.source_string
  ...
  WITH nocounter
 ;end select
 CALL accept(4,21,"9(40);cp",ct_detail->detail_list[cur_rec].ruleentity_id)
 SET v_nomen_id = cnvtreal(curaccept)
 SET help = off
 ...
 IF (lookup_option=0)
  ...
  SET help = off
  CALL accept(6,21,ct_detail->detail_list[cur_rec].cpt4_cd)
 ELSE
  CALL clear(15,25)
  CALL box(1,1,15,70)
  CALL text(15,25,"No Help Available")
  CALL accept(6,21,"x(10);cp",ct_detail->detail_list[cur_rec].ruleentity_id)
  SET ct_detail->detail_list[cur_rec].ruleentity_id = cnvtint(curaccept)
  SET v_nomen_id = cnvtint(curaccept)
 ENDIF
 CALL accept(8,20,"###.##;c",ct_detail->detail_list[cur_rec].ct_result_factor)
 SET v_result_factor = cnvtreal(curaccept)
 CALL text(10,15,format(ct_detail->detail_list[cur_rec].beg_date,"DD-MMM-YYYY;;D"))
 CALL accept(10,15,"xx-xxx-xxxx;ucs",format(ct_detail->detail_list[cur_rec].beg_date,"DD-MMM-YYYY;;D"
   ))
 SET v_beg_date = cnvtdatetime(curaccept)
 CALL text(12,15,format(ct_detail->detail_list[cur_rec].end_date,"DD-MMM-YYYY;;D"))
 CALL accept(12,15,"xx-xxx-xxxx;ucs",format(ct_detail->detail_list[cur_rec].end_date,"DD-MMM-YYYY;;D"
   ))
 SET v_end_date = cnvtdatetime(curaccept)
 EXECUTE afc_ccl_msgbox "Are you sure you want to Modify?", "Replaceprice", "YN"
 IF ((response->yes_ind=1))
  GO TO modifing_replaceprice
 ELSEIF ((response->no_ind=1))
  GO TO top
 ENDIF
#modifing_replaceprice
 SET request->ct_rule_detail_qual = 1
 ...
 GO TO top
#delete_replaceprice
 EXECUTE afc_ccl_msgbox "Are you sure you want to Delete?", "Replaceprice", "YN"
 IF ((response->yes_ind=1))
