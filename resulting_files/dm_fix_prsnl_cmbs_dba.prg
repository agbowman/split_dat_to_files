CREATE PROGRAM dm_fix_prsnl_cmbs:dba
 EXECUTE dm_readme_import "dm_combine_tables.csv", "dm_dm_cmb_exception_import", 100,
 0
 SELECT INTO "nl:"
 ...
 SELECT INTO "nl:"
  FROM dm_cmb_exception dce
  WHERE dce.child_entity IN ("AP_FT_EVENT", "HLA_XM_RES_TRAY", "PERSON_HLA_AB_SCREEN",
  "PERSON_HLA_AB_SCN_AUDIT", "PERSON_HLA_TYPE",
  "PERSON_HLA_XM")
   AND dce.script_name IN ("PERSON_CMB_AP_FT_EVENT", "PERSON_CMB_HLA_XM_RES_TRAY",
  "PERSON_CMB_PERSON_HLA_AB_SCRN", "PERSON_CMB_PERSON_HLA_AB_SCN_A", "PERSON_CMB_PERSON_HLA_TYPE",
  "PERSON_CMB_PERSON_HLA_XM")
   AND dce.operation_type="COMBINE"
   AND dce.parent_entity="PERSON"
  WITH nocounter
 ;end select
 IF (curqual=6)
  CALL echo("PERSON rows successfully inserted into dm_cmb_exception.")
  COMMIT
 ELSE
  CALL echo("Error in loading dm_cmb_exception")
  ROLLBACK
 ...
 DELETE  FROM dm_cmb_exception dce
  WHERE dce.child_entity="PERSON_HLA_XM_AUDIT"
   AND dce.operation_type="COMBINE"
   AND dce.parent_entity="PERSON"
  WITH nocounter
 ;end delete
 ...
 ;end select
 IF (curqual=0)
  CALL echo("PERSON_HLA_XM_AUDIT row on dm_cmb_exception was successfully deleted.")
  COMMIT
 ELSE
  CALL echo("Error deleting PERSON_HLA_XM_AUDIT row from dm_cmb_exception.")
  ROLLBACK
  GO TO exit_script
 ENDIF
 DELETE  FROM dm_cmb_exception dce
