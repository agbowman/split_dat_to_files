CREATE PROGRAM afc_ens_cs_org_reltn:dba
 ...
 SET hafc_ens_cs_org_reltn = 0
 SET istatus = 0
 SET serrmsg = fillstring(132," ")
 SET reply->status_data.status = "F"
 SET table_name = "CS_ORG_RELTN"
 SET incrementseq = 0
 IF ((request->cs_org_reltn_qual > 0))
  SET reply->cs_org_reltn_qual = request->cs_org_reltn_qual
  FOR (inx0 = 1 TO request->cs_org_reltn_qual)
    CASE (request->cs_org_reltn[inx0].action_type)
     OF "ADD":
      SET action_begin = inx0
      SET action_end = inx0
      EXECUTE afc_add_cs_org_reltn
      IF (failed != false)
       GO TO check_error
      ENDIF
     OF "DEL":
