CREATE PROGRAM aps_get_case_help_1:dba
...
#script
 ...
 SET reply->context_ind = 0
 DECLARE attempted_years_cnt = i4 WITH protect, noconstant(0)
 DECLARE maxqualrows = i4 WITH protect, noconstant(100)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   service_resource_cd = ap.service_resource_cd, stat = isresourceviewable(service_resource_cd)
  WITH nocounter
 ;end select
 ...
 IF (validate(context->context_ind,0) != 0)
  SET given_prefix_cd = context->prefix_cd
  SET given_case_year = context->case_year
  SET given_case_number = context->case_number
 ELSE
  SET given_prefix_cd = request->prefix_cd
  SET given_case_year = request->case_year
  SET given_case_number = request->case_number
 ENDIF
 SET mrn_alias_type_cd = uar_get_code_by("MEANING",319,"MRN")
#main_select
 SET max_num = given_case_number
 ...
 SELECT INTO "nl:"
  ...
  PLAN (pc
   WHERE pc.case_year=given_case_year
    AND pc.prefix_id=given_prefix_cd
  ...
  DETAIL
   IF (case_cnt=maxqualrows)
    IF ((reply->context_ind=0))
     reply->context_ind = 1, context->context_ind = 1, context->prefix_cd = pc.prefix_id,
     context->case_year = pc.case_year, context->case_number = pc.case_number
    ENDIF
   ELSE
    case_cnt += 1
  ...
  IF (min_num > 1)
   SELECT INTO "nl:"
    ...
    WHERE pc.case_year=given_case_year
     ...
     AND pc.cancel_cd IN (null, 0)
    DETAIL
     given_case_number = my_max_num
    WITH nocounter
   ;end select
  ENDIF
  IF (given_case_number=0
   AND attempted_years_cnt <= 5)
   SET given_case_year -= 1
 ...
 IF (case_cnt > 0)
  SELECT INTO "nl:"
   pc.case_id, case_collect_dt_tm = cnvtdatetime(pc.case_collect_dt_tm), patient_exists = decode(p
    .seq,"Y","N"),
   p.person_id, p.name_full_formatted, pr.name_full_formatted,
   spec_exists = decode(cs.seq,"Y","N"), cs.specimen_description, t_tag_group_id = decode(t.seq,t
    .tag_group_id,0.0),
   t_tag_sequence = decode(t.seq,t.tag_sequence,0), t_tag_disp = decode(t.seq,t.tag_disp," "),
   nullind_p_deceased_dt_tm = nullind(p.deceased_dt_tm)
   ...
   HEAD d1.seq
    ...
    ENDIF
    reply->qual[cnt].case_id = pc.case_id, reply->qual[cnt].fill_ext_accession_id = pc.case_id, reply
    ->qual[cnt].case_collect_dt_tm = cnvtdatetime(pc.case_collect_dt_tm),
    ...
    IF (patient_exists="Y")
     ...
     reply->qual[cnt].birth_dt_tm = cnvtdatetime(p.birth_dt_tm)
   ...
   DETAIL
    IF (spec_exists="Y")
     ...
     ENDIF
     reply->qual[cnt].specimen_cnt = cnt1, reply->qual[cnt].spec_qual[cnt1].specimen_description = cs
     .specimen_description, reply->qual[cnt].spec_qual[cnt1].specimen_tag_display = t_tag_disp
    ENDIF
   FOOT  d1.seq
    IF (cnt1=0)
