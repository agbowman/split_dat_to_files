CREATE PROGRAM dac_231263_testobjectnls
 ...
 SET dac_test_reply->message = "Test failed: starting test dac_231263_testObjectNLS..."
 ...
 IF (error(dt_errmsg,0) != 0)
  SET dac_test_reply->status = "F"
  SET dac_test_reply->message = concat("Failed to look up CERN_NLS_SORT: ",dt_errmsg)
  GO TO exit_test
 ELSEIF (curqual != 0)
  CALL parser(concat("rdb asis(^ drop function CERN_NLS_SORT ^)go"))
  IF (error(dt_errmsg,0) != 0)
   SET dac_test_reply->status = "F"
   SET dac_test_reply->message = concat("Failed to drop CERN_NLS_SORT function: ",dt_errmsg)
   GO TO exit_test
  ENDIF
  SET dt_funccheck = 1
 ENDIF
 ...
 IF ((readme_data->status="S"))
  ...
  SET dac_test_reply->message = readme_data->message
 ELSE
  SET dac_test_reply->status = "F"
  SET dac_test_reply->message = concat("Failure to report success:",readme_data->message)
 ENDIF
 IF (dt_funccheck=1)
