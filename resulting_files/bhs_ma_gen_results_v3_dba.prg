CREATE PROGRAM bhs_ma_gen_results_v3:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (o
   WHERE o.encntr_id=eid
    AND ((((o.activity_type_cd+ 0)=restraints)) OR (((o.catalog_cd+ 0) IN (seizure, suicide,
   companion, activity))
 ...
 IF ((results->falls > " "))
  SET lidx = (lidx+ 1)
  SET stat = alterlist(drec->line_qual,lidx)
  SET temp_disp1 = concat("Falls Risk Score: ",wr,results->falls)
  SET drec->line_qual[lidx].disp_line = concat(rh2b,trim(temp_disp1),reol)
 ELSEIF ((results->falls_ped > " "))
  SET lidx = (lidx+ 1)
  SET stat = alterlist(drec->line_qual,lidx)
