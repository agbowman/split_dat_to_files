CREATE PROGRAM bb_rdm_upd_person_aborh_rslt:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bb_rdm_upd_person_aborh_rslt"
 CALL logscriptstart(sreadme_name)
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: Starting bb_rdm_upd_person_aborh_rslt script"
 COMMIT
 DECLARE lstandardaborhcodeset = i4 WITH constant(1640)
 DECLARE lresultaborhcodeset = i4 WITH constant(1643)
 DECLARE caborh_cd = c8 WITH constant("ABORH_cd")
 DECLARE lrecordcnt = i4 WITH noconstant(0)
 DECLARE lstandardsize = i4 WITH noconstant(0)
 DECLARE lresultsize = i4 WITH noconstant(0)
 DECLARE nfound = i2 WITH noconstant(0)
 DECLARE lloopctrl = i4 WITH noconstant(0)
 DECLARE smessagelog = c132 WITH public, noconstant("")
 DECLARE lfounderrors = i4 WITH noconstant(0)
 DECLARE ndone = i2 WITH noconstant(0)
 DECLARE cpar_seq_ctrl = c18 WITH constant("PAR_SEQ_CTRL")
 DECLARE pathnet_bbt = c11 WITH constant("PATHNET_BBT")
 DECLARE dlastseq = f8 WITH noconstant(0.0)
 DECLARE ndm_row_exists = i2 WITH noconstant(0)
 DECLARE ldm_updt_cnt = i4 WITH noconstant(0)
 DECLARE dmaxpar_id = f8 WITH noconstant(0.0)
 DECLARE dmaxseq = f8 WITH noconstant(0.0)
 DECLARE nzerocnt = i2 WITH noconstant(0)
 RECORD standardaborhs(
   1 standard[*]
 ...
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
  WHERE cv.code_set=lstandardaborhcodeset
   AND cv.code_value > 0.0
  ORDER BY cv.code_value
  HEAD REPORT
   lrecordcnt = 0
  DETAIL
   lrecordcnt = (lrecordcnt+ 1), stat = alterlist(standardaborhs->standard,lrecordcnt),
   standardaborhs->standard[lrecordcnt].standard_aborh_cd = cv.code_value
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET smessagelog = "No code values found on code_set 1640"
  CALL logstatus(build(sreadme_name,"--No code_values found on code_set 1640."),"S")
  CALL logmsg(smessagelog,(e_dispmsg+ e_logtofile))
  SET readme_data->status = "S"
  SET readme_data->message = "No code values found on code_set 1640."
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  ...
  PLAN (cv
   WHERE cv.code_set=lresultaborhcodeset)
   JOIN (cve
  ...
  HEAD REPORT
   lrecordcnt = 0
  DETAIL
   lrecordcnt = (lrecordcnt+ 1), stat = alterlist(resultaborhs->result,lrecordcnt), resultaborhs->
   result[lrecordcnt].result_aborh_cd = cv.code_value,
   resultaborhs->result[lrecordcnt].standard_aborh_cd = cnvtreal(trim(cve.field_value))
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET smessagelog = "No code values found on code_set 1643"
  CALL logstatus(build(sreadme_name,"--No code_values found on code_set 1643."),"S")
  CALL logmsg(smessagelog,(e_dispmsg+ e_logtofile))
  SET readme_data->status = "S"
  SET readme_data->message = "No code values found on code_set 1643."
  GO TO exit_script
 ENDIF
 SET dlastseq = 0.0
 SELECT INTO "nl:"
  ...
  WHERE dm.info_domain=pathnet_bbt
   AND dm.info_name=cpar_seq_ctrl
  DETAIL
   dlastseq = dm.info_number, ldm_updt_cnt = dm.updt_cnt
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET stat = gm_i_dm_info2388_f8("INFO_NUMBER",0.0,1,0)
  SET stat = gm_i_dm_info2388_vc("INFO_NAME",cpar_seq_ctrl,1,0)
  SET stat = gm_i_dm_info2388_vc("INFO_DOMAIN",pathnet_bbt,1,0)
  SET stat = gm_i_dm_info2388_dq8("INFO_DATE",cnvtdatetime(curdate,curtime3),1,0)
  ...
  ELSE
   ROLLBACK
   CALL logstatus(build(sreadme_name," Execution Failed.  Unable to update DM_INFO."),"F")
   SET readme_data->status = "F"
   SET readme_data->message = "Gold Master Update Script Failure."
   GO TO exit_script
  ENDIF
 ENDIF
 SELECT INTO "nl:"
  par_seq = par.person_aborh_rs_id
  FROM person_aborh_result par
  WHERE par.person_aborh_rs_id > dlastseq
  DETAIL
   row + 0
  FOOT REPORT
   dmaxpar_id = max(par_seq)
  WITH nocounter
 ;end select
 SET ndone = 0
 SET nzerocnt = 0
 WHILE (ndone=0)
   ...
   SET dmaxseq = least(dmaxpar_id,(dlastseq+ 5000))
   SELECT INTO "nl:"
    par.person_aborh_rs_id, par.result_cd, par.standard_aborh_cd
    FROM person_aborh_result par
    WHERE par.standard_aborh_cd=0.0
     AND par.person_aborh_rs_id != 0.0
     AND par.person_aborh_rs_id >= dlastseq
     AND par.person_aborh_rs_id <= dmaxseq
    ORDER BY par.person_aborh_rs_id
    HEAD REPORT
     lfounderrors = 0, lrecordcnt = 0, nfound = 0,
     lstandardsize = size(standardaborhs->standard,5), lresultsize = size(resultaborhs->result,5),
     stat = alterlist(updates->updates,5000)
    DETAIL
     nfound = 0, lloopctrl = 1
     WHILE (nfound=0
      AND lloopctrl <= lstandardsize)
       IF ((par.result_cd=standardaborhs->standard[lloopctrl].standard_aborh_cd))
        lrecordcnt = (lrecordcnt+ 1), updates->updates[lrecordcnt].person_aborh_rs_id = par
        .person_aborh_rs_id, updates->updates[lrecordcnt].standard_aborh_cd = par.result_cd,
        nfound = 1
       ELSE
     ...
     IF (nfound=0)
      lloopctrl = 1
      WHILE (nfound=0
       AND lloopctrl <= lresultsize)
        IF ((par.result_cd=resultaborhs->result[lloopctrl].result_aborh_cd))
         lrecordcnt = (lrecordcnt+ 1), updates->updates[lrecordcnt].person_aborh_rs_id = par
         .person_aborh_rs_id, updates->updates[lrecordcnt].standard_aborh_cd = resultaborhs->result[
         lloopctrl].standard_aborh_cd,
         nfound = 1
        ELSE
         lloopctrl = (lloopctrl+ 1)
        ENDIF
     ...
     ENDIF
     IF (nfound=0)
      lfounderrors = (lfounderrors+ 1), stat = alterlist(errors->errors,lfounderrors), errors->
      errors[lfounderrors].person_aborh_rs_id = par.person_aborh_rs_id
     ENDIF
    FOOT REPORT
     stat = alterlist(updates->updates,lrecordcnt)
    WITH nocounter
   ;end select
   IF (lfounderrors > 0)
    SET smessagelog = ""
    FOR (lloopctrl = 1 TO size(errors->errors,5))
     SET smessagelog = build("Error processing person_aborh_rs_id: ",errors->errors[lloopctrl].
      person_aborh_rs_id)
     CALL logmsg(smessagelog,(e_dispmsg+ e_logtofile))
    ENDFOR
   ENDIF
   SET dlastseq = (dlastseq+ 5000)
   IF (curqual=0)
    SET nzerocnt = (nzerocnt+ 1)
    IF (nzerocnt > 5)
     SELECT INTO "nl:"
      par_seq = par.person_aborh_rs_id
      FROM person_aborh_result par
   ...
   ELSE
    SET nzerocnt = 0
    UPDATE  FROM person_aborh_result par,
      (dummyt d  WITH seq = lrecordcnt)
     SET par.standard_aborh_cd = updates->updates[d.seq].standard_aborh_cd
     PLAN (d)
      JOIN (par
      WHERE (par.person_aborh_rs_id=updates->updates[d.seq].person_aborh_rs_id))
     WITH nocounter
    ;end update
    SET gm_u_dm_info2388_req->allow_partial_ind = 0
    SET gm_u_dm_info2388_req->force_updt_ind = 0
    SET stat = gm_u_dm_info2388_f8("INFO_NUMBER",dmaxseq,1,0,0)
    SET stat = gm_u_dm_info2388_vc("INFO_NAME",cpar_seq_ctrl,1,0,1)
    SET stat = gm_u_dm_info2388_vc("INFO_DOMAIN",pathnet_bbt,1,0,1)
    SET stat = gm_u_dm_info2388_dq8("INFO_DATE",cnvtdatetime(curdate,curtime3),1,0,0)
    SET stat = gm_u_dm_info2388_i4("UPDT_CNT",ldm_updt_cnt,1,0,1)
    SET ldm_updt_cnt = (ldm_updt_cnt+ 1)
 ...
 IF (lfounderrors=0)
  ...
  ;end select
  IF (curqual=0)
   CALL logstatus(build(sreadme_name,"Execution Successful."),"S")
   SET readme_data->status = "S"
   SET readme_data->message = "Readme Successful"
  ELSE
   CALL logstatus(build(sreadme_name,"Execution Failed.  Standard_aborh_cd's exist with zero values."
     ),"F")
   SET readme_data->status = "F"
   SET readme_data->message = "Standard_aborh_cd's exist with zero values."
  ENDIF
 ELSE
  CALL logstatus(build(sreadme_name,
    "--Errors found during processing.  See the Readme Log file for more information."),"F")
  SET readme_data->status = "F"
  SET readme_data->message = "Errors found during processing."
 ENDIF
#exit_script
 FREE SET standardaborhs
 FREE SET resultaborhs
 FREE SET updates
 FREE SET errors
