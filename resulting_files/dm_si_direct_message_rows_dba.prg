CREATE PROGRAM dm_si_direct_message_rows:dba
 ...
 SET reply->table_name = "SI_DIRECT_MESSAGE"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(s.si_direct_message_id)
   FROM si_direct_message s
   WHERE s.si_direct_message_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(s.si_direct_message_id)
  FROM si_direct_message s
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    s.rowid
    FROM si_direct_message s
    WHERE parser(sbr_getrowidnotexists("s.si_direct_message_id between v_cur_min_id and v_cur_max_id",
      "s"))
     AND s.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),0)
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = s.rowid
