CREATE PROGRAM dm_rmc_create_and_xfer_xlats:dba
 ...
 SUBROUTINE xta_dbl_hlv(i_gap)
   ...
   RETURN(xdh_gap)
 END ;Subroutine
 IF (check_logfile("DM_RMC_CXX",".log","DM_RMC_CXX LogFile")=0)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 IF (validate(request->chain_name,"-99")="-99")
  CALL echo("*******------------------------------------------************")
  CALL echo("******* REQUEST structure not declared properly. ************")
  CALL echo("*******------------------------------------------************")
  GO TO end_program
 ENDIF
 FREE RECORD cxx_request
 RECORD cxx_request(
   1 src_env_id = f8
   1 trg_env_id = f8
   1 new_trg_env_id = f8
 ...
 FREE RECORD cxx2_request
 RECORD cxx2_request(
   1 dm_rdds_chain_id = f8
 )
 DECLARE cxx_lst_loop = i4
 DECLARE cxx_ret = vc
 SET cxx_ret = dm_check_syn(null)
 IF (cxx_ret="F")
  SET dm_err->eproc = "Error occured while checking synonyms"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Obtaining current environment_id"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info a
  ...
  DETAIL
   cxx_request->new_trg_env_id = a.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 IF (curqual=0)
  ...
  GO TO end_program
 ENDIF
 SET cxx_request->src_env_id = request->list[1].env_id
 SET cxx_request->trg_env_id = request->list[size(request->list,5)].env_id
 SET cxx_request->dm_rdds_chain_id = get_chain_id(trim(request->chain_name,3))
 SET cxx2_request->dm_rdds_chain_id = cxx_request->dm_rdds_chain_id
 FOR (cxx_lst_loop = 2 TO size(request->list,5))
   SET dm_err->eproc = concat("Verify that push to admin has been performed for pair: ",trim(
     cnvtstring(request->list[(cxx_lst_loop - 1)].env_id))," and ",trim(cnvtstring(request->list[
      cxx_lst_loop].env_id)))
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_rdds_chain_child drcc
    WHERE (drcc.dm_rdds_chain_id=cxx_request->dm_rdds_chain_id)
     AND (drcc.from_env_id=request->list[(cxx_lst_loop - 1)].env_id)
     AND (drcc.to_env_id=request->list[cxx_lst_loop].env_id)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->eproc = concat("Push to admin from : ",trim(cnvtstring(request->list[(cxx_lst_loop -
       1)].env_id))," to ",trim(cnvtstring(request->list[cxx_lst_loop].env_id))," has not occured.")
    CALL disp_msg(" ",dm_err->logfile,0)
    GO TO end_program
   ENDIF
   SELECT INTO "nl:"
    FROM dm_rdds_chain_child drcc
    WHERE (drcc.dm_rdds_chain_id=cxx_request->dm_rdds_chain_id)
     AND (drcc.from_env_id=request->list[cxx_lst_loop].env_id)
     AND (drcc.to_env_id=request->list[(cxx_lst_loop - 1)].env_id)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   IF (curqual=0)
    SET dm_err->eproc = concat("Push to admin from : ",trim(cnvtstring(request->list[cxx_lst_loop].
       env_id))," to ",trim(cnvtstring(request->list[(cxx_lst_loop - 1)].env_id))," has not occured."
     )
    CALL disp_msg(" ",dm_err->logfile,0)
    GO TO end_program
   ENDIF
   UPDATE  FROM dm_rdds_chain_child drcc
    SET drcc.chain_seq = (cxx_lst_loop - 1)
    WHERE (drcc.dm_rdds_chain_id=cxx_request->dm_rdds_chain_id)
     AND (drcc.from_env_id=request->list[(cxx_lst_loop - 1)].env_id)
     AND (drcc.to_env_id=request->list[cxx_lst_loop].env_id)
   ;end update
   ...
   ENDIF
 ENDFOR
 EXECUTE dm_rmc_create_xlats_in_admin  WITH replace("REQUEST","CXX2_REQUEST")
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
 EXECUTE dm_rmc_xfer_new_xlats  WITH replace("REQUEST","CXX_REQUEST")
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO end_program
 ENDIF
