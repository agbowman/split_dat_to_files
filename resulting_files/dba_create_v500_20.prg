CREATE PROGRAM dba_create_v500_20
 SET message = window
 ...
 SUBROUTINE aix_gen_init(x)
   ...
   SELECT INTO concat("init_dir","/init",cnvtlower(dbname),"1.ora")
    ...
    FOOT REPORT
     ...
     IF ((dm_env_import_request->base_oracle_version_int >= 10))
      col 0, "log_archive_dest_1 = ",
      CALL print(concat("'LOCATION=",dba_string,"/arch","'")),
      row + 1, col 0, "log_archive_dest_state_1 = enable",
      row + 1
     ELSE
      col 0, "log_archive_dest = ",
      CALL print(dba_string),
      "/arch", row + 1
     ENDIF
    WITH nocounter, format = stream, noheading,
   ...
   IF ((dm_env_import_request->target_undo_ind=1))
    SELECT INTO concat("init_dir","/init",cnvtlower(dbname),"1.ora")
     FROM dm_env_files a
     WHERE environment_id=env_id
      AND a.file_type="UNDO"
     DETAIL
      col 0, "undo_management = auto", row + 1,
      col 0, "undo_tablespace = ",
      CALL print(cnvtlower(a.tablespace_name)),
      row + 1, col 0, "undo_retention = 3600"
     WITH nocounter, format = stream, append,
      noheading, formfeed = none, maxrow = 1
    ;end select
   ENDIF
   SELECT INTO concat("init_dir","/config",cnvtlower(dbname),".ora")
    ...
    DETAIL
     IF (validate(cursys2,"AIX")="HPX")
      devpathprefix = concat(trim(a.disk_name),"/r")
     ELSE
      devpathprefix = "/dev/r"
     ENDIF
     IF (a.cntl_file_num=1)
      CALL print(concat(trim(devpathprefix),trim(a.file_name)))
     ELSE
      col 0, "                ",
      CALL print(concat(trim(devpathprefix),trim(a.file_name)))
     ENDIF
