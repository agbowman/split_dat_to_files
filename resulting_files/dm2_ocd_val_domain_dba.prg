CREATE PROGRAM dm2_ocd_val_domain:dba
 ...
 RECORD admin_rs(
   ...
   1 nick_tbl_cnt = i4
   1 ccl_def_cnt = i4
   1 ccl_def_diff_ind = i2
   1 view_cnt = i2
   1 view_col_cnt_diff_ind = i2
   1 admin_tbl[*]
     2 admin_table_name = vc
     2 nick_found_ind = i2
     2 ccl_def_found_ind = i2
     2 admin_col_cnt = i4
     2 ccl_def_col_cnt = i4
     2 view_col_cnt = i4
     2 needs_view_ind = i2
     2 view_found_ind = i2
     2 admin_col[*]
 ...
 RECORD admin_seq_rs(
   ...
   1 admin_seq[*]
     2 admin_seq_name = vc
     2 nick_found_ind = i2
     2 clin_seq_found_ind = i2
 )
 SET admin_seq_rs->admin_seq_cnt = 0
 SET admin_seq_rs->nick_seq_cnt = 0
 SET admin_seq_rs->clin_seq_cnt = 0
 DECLARE create_admin_nicknames_ind = i2 WITH protect, noconstant(0)
 DECLARE dovd_link_name = vc WITH protect, noconstant(" ")
 DECLARE dovd_check_views = i2 WITH protect, noconstant(0)
 DECLARE dovd_check_defs = i2 WITH protect, noconstant(0)
 DECLARE dovd_check_view_cols = i2 WITH protect, noconstant(0)
 DECLARE dovd_fail_ind = i2 WITH protect, noconstant(0)
 DECLARE load_admin_rs() = i2
 DECLARE compare_nicknames_to_admin() = i2
 DECLARE compare_ccl_defs_to_admin() = i2
 DECLARE log_differences() = i2
 DECLARE check_domain_setup_executing() = i2
 DECLARE write_domain_setup_row() = i2
 DECLARE delete_domain_setup_row() = i2
 DECLARE load_admin_seq_rs() = i2
 DECLARE compare_views_to_admin() = i2
 DECLARE compare_view_cols_to_admin_cols() = i2
 IF (check_logfile("dm2_ocd_val_domain",".log","Starting dm2_ocd_val_domain")=0)
  GO TO end_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_ocd_val_domain"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm2_server_link->user = "CDBA"
 SET dm2_server_link->dbase = "ADMIN"
 IF (dir_get_admin_db_link(0,dovd_link_name,dovd_fail_ind)=0)
  GO TO end_program
 ENDIF
 SET dm2_server_link->server_name = dovd_link_name
 IF (create_admin_nicknames_ind=0)
  IF (check_dm2tools_nicknames(0)=0)
   GO TO end_program
  ENDIF
  IF (load_admin_rs(null)=0)
   GO TO end_program
  ENDIF
  IF (load_admin_seq_rs(null)=0)
   GO TO end_program
  ENDIF
  IF (compare_nicknames_to_admin(null)=0)
   GO TO end_program
  ELSE
   IF ((((admin_rs->admin_tbl_cnt != admin_rs->nick_tbl_cnt)) OR ((((admin_seq_rs->admin_seq_cnt !=
   admin_seq_rs->nick_seq_cnt)) OR ((admin_seq_rs->clin_seq_cnt > 0))) )) )
    IF (log_differences(null)=0)
     GO TO end_program
    ENDIF
   ELSE
    SET dovd_check_defs = 1
    IF (compare_ccl_defs_to_admin(null)=0)
     GO TO end_program
    ELSE
     IF ((((admin_rs->admin_tbl_cnt != admin_rs->ccl_def_cnt)) OR ((admin_rs->ccl_def_diff_ind=1))) )
      IF (log_differences(null)=0)
       GO TO end_program
      ENDIF
     ELSE
      SET dovd_check_views = 1
      IF (compare_views_to_admin(null)=0)
       GO TO end_program
      ELSE
       IF ((admin_rs->view_cnt != dsl_dmtools_adm_tables->cnt))
        IF (log_differences(null)=0)
         GO TO end_program
        ENDIF
       ELSE
        SET dovd_check_view_cols = 1
        IF (compare_view_cols_to_admin_cols(null)=0)
         GO TO end_program
        ELSE
         IF ((admin_rs->view_col_cnt_diff_ind=1))
          IF (log_differences(null)=0)
           GO TO end_program
          ENDIF
         ENDIF
  ...
  ENDIF
 ENDIF
 IF (create_admin_nicknames_ind=1)
  CASE (check_domain_setup_executing(null))
   OF 0:
    GO TO end_program
   OF 1:
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Checking for an existing execution of domain setup"
    SET dm_err->emsg = "Another process is currently executing domain setup"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO end_program
   OF 2:
    IF (write_domain_setup_row(null)=0)
     GO TO end_program
    ENDIF
    IF ((dm2_install_schema->cdba_p_word="NONE")
     ...
     IF ((dm_err->err_ind=1))
      SET dm_err->err_ind = 0
      CALL delete_domain_setup_row(null)
      SET dm_err->err_ind = 1
      GO TO end_program
     ENDIF
     SET dm2_install_schema->cdba_p_word = dm2_install_schema->p_word
    ENDIF
    EXECUTE dm2_create_admin_synonyms dovd_link_name, "*"
    IF ((dm_err->err_ind=1))
     SET dm_err->err_ind = 0
     CALL delete_domain_setup_row(null)
     SET dm_err->err_ind = 1
     GO TO end_program
    ENDIF
    IF (delete_domain_setup_row(null)=0)
     GO TO end_program
    ENDIF
  ENDCASE
 ELSE
  SET dm_err->eproc = "All necessary Admin components have been found"
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 FREE RECORD admin_rs
 SUBROUTINE load_admin_rs(null)
   DECLARE tbl_cnt = i4 WITH protect, noconstant(0)
   DECLARE col_cnt = i4 WITH protect, noconstant(0)
   DECLARE lar_idx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Loading Admin Record Structure"
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE lar_dyn_in_clause = vc WITH public, noconstant("")
   SET lar_dyn_in_clause = dm2_fill_nick_except("atc")
   SELECT INTO "nl:"
    atc.table_name, atc.column_name
    FROM dm2_admin_tables at,
     dm2_admin_tab_columns atc,
     dm2_admin_dm_info adm
    WHERE adm.info_domain="DM2_ADMIN_TABLE"
     AND atc.table_name=adm.info_name
     AND parser(lar_dyn_in_clause)
    ...
    HEAD atc.table_name
     ...
     ENDIF
     admin_rs->admin_tbl[tbl_cnt].admin_table_name = atc.table_name, admin_rs->admin_tbl_cnt =
     tbl_cnt, col_cnt = 0,
     lar_idx = 0, lar_idx = locateval(lar_idx,1,dsl_dmtools_adm_tables->cnt,atc.table_name,
      dsl_dmtools_adm_tables->tbl[lar_idx].table_name)
     IF (lar_idx > 0)
      admin_rs->admin_tbl[tbl_cnt].needs_view_ind = 1
     ENDIF
    DETAIL
     col_cnt = (col_cnt+ 1)
     IF (mod(col_cnt,10)=1)
     ...
     ENDIF
     admin_rs->admin_tbl[tbl_cnt].admin_col[col_cnt].admin_data_type = atc.data_type, admin_rs->
     admin_tbl[tbl_cnt].admin_col[col_cnt].admin_col_name = atc.column_name, admin_rs->admin_tbl[
     tbl_cnt].admin_col[col_cnt].admin_data_length = atc.data_length,
    ...
    WITH nocounter
   ;end select
   IF (check_error("load_admin_rs")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE compare_nicknames_to_admin(null)
   SET dm_err->eproc = "Comparing Nicknames to Admin Record Structure"
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE indx = i4 WITH protect, noconstant(0)
   DECLARE tblx = i4 WITH protect, noconstant(0)
   DECLARE colx = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM dm2_dba_synonyms du,
     dm2_admin_tables at
    PLAN (at)
     JOIN (du
     WHERE du.synonym_name=at.table_name
      AND du.owner="PUBLIC"
      AND du.table_owner="CDBA")
    ORDER BY du.table_name
    DETAIL
     indx = 0, tblx = 0, tblx = locateval(indx,1,admin_rs->admin_tbl_cnt,du.table_name,admin_rs->
      admin_tbl[indx].admin_table_name)
     IF (tblx > 0)
      admin_rs->admin_tbl[tblx].nick_found_ind = 1, admin_rs->nick_tbl_cnt = (admin_rs->nick_tbl_cnt
      + 1)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error("Query for public synonyms pointing to Admin tables")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET cnta_dyn_in_clause = dm2_fill_seq_list("du","table_name")
   SELECT INTO "nl:"
    FROM dm2_dba_synonyms du
    WHERE parser(cnta_dyn_in_clause)
     AND du.owner="PUBLIC"
    DETAIL
     seqx = 0, seqx = locateval(seqx,1,admin_seq_rs->admin_seq_cnt,du.table_name,admin_seq_rs->
      admin_seq[seqx].admin_seq_name)
     IF (seqx > 0)
      admin_seq_rs->admin_seq[seqx].nick_found_ind = 1, admin_seq_rs->nick_seq_cnt = (admin_seq_rs->
      nick_seq_cnt+ 1)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error("Query for public synonyms pointing to Admin sequences")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET cnta_dyn_in_clause = dm2_fill_seq_list("us","sequence_name")
   SELECT INTO "nl:"
    FROM dm2_user_sequences us
    WHERE parser(cnta_dyn_in_clause)
    DETAIL
     seqx = 0, seqx = locateval(seqx,1,admin_seq_rs->admin_seq_cnt,us.sequence_name,admin_seq_rs->
      admin_seq[seqx].admin_seq_name)
     IF (seqx > 0)
      admin_seq_rs->admin_seq[seqx].clin_seq_found_ind = 1, admin_seq_rs->clin_seq_cnt = (
      admin_seq_rs->clin_seq_cnt+ 1)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error("Query for user sequences matching to Admin sequences")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("admin_tbl_cnt =",admin_rs->admin_tbl_cnt))
    CALL echo(build("nick_tbl_cnt =",admin_rs->nick_tbl_cnt))
    CALL echo(build("clin_seq_cnt =",admin_seq_rs->clin_seq_cnt))
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE compare_views_to_admin(null)
   DECLARE tblx = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM dm2_user_views v
    ORDER BY v.view_name
    DETAIL
     tblx = 0, tblx = locateval(tblx,1,admin_rs->admin_tbl_cnt,v.view_name,admin_rs->admin_tbl[tblx].
      admin_table_name)
     IF (tblx > 0
      AND (admin_rs->admin_tbl[tblx].needs_view_ind=1))
      admin_rs->admin_tbl[tblx].view_found_ind = 1, admin_rs->view_cnt = (admin_rs->view_cnt+ 1)
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error("Query for user views pointing to Admin tables")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("view_cnt =",admin_rs->view_cnt))
    CALL echo(build("spec_adm_tables_cnt =",dsl_dmtools_adm_tables->cnt))
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE compare_ccl_defs_to_admin(null)
   SET dm_err->eproc = "Comparing CCL Definitions to Admin Record Structure"
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE tblx = i4 WITH protect, noconstant(0)
   DECLARE colx = i4 WITH protect, noconstant(0)
   DECLARE ccdta_col_fnd_ind = i2 WITH protect, noconstant(0)
   DECLARE ccdta_coldef_fnd_ind = i2 WITH protect, noconstant(0)
   DECLARE ccdta_data_type = vc WITH protect, noconstant("")
   FOR (tblx = 1 TO admin_rs->admin_tbl_cnt)
     IF (checkdic(cnvtupper(admin_rs->admin_tbl[tblx].admin_table_name),"T",0)=2)
      SET admin_rs->admin_tbl[tblx].ccl_def_found_ind = 1
      SET admin_rs->ccl_def_cnt = (admin_rs->ccl_def_cnt+ 1)
      FOR (colx = 1 TO admin_rs->admin_tbl[tblx].admin_col_cnt)
        SET ccdta_col_fnd_ind = 0
        SET ccdta_coldef_fnd_ind = 0
        SET ccdta_data_type = ""
        IF (dm2_table_column_exists("",cnvtupper(admin_rs->admin_tbl[tblx].admin_table_name),
         cnvtupper(admin_rs->admin_tbl[tblx].admin_col[colx].admin_col_name),0,1,
         1,ccdta_col_fnd_ind,ccdta_coldef_fnd_ind,ccdta_data_type)=0)
         RETURN(0)
        ENDIF
        IF (ccdta_coldef_fnd_ind=1)
         SET admin_rs->admin_tbl[tblx].ccl_def_col_cnt = (admin_rs->admin_tbl[tblx].ccl_def_col_cnt+
         1)
        ENDIF
      ENDFOR
      IF ((admin_rs->admin_tbl[tblx].admin_col_cnt != admin_rs->admin_tbl[tblx].ccl_def_col_cnt))
       SET admin_rs->ccl_def_diff_ind = 1
      ENDIF
     ENDIF
   ENDFOR
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("ccl_def_cnt =",admin_rs->ccl_def_cnt))
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE compare_view_cols_to_admin_cols(null)
   DECLARE indx = i4 WITH protect, noconstant(0)
   DECLARE tblx = i4 WITH protect, noconstant(0)
   DECLARE colx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Comparing View Column Count to Admin Record Structure"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl"
    FROM dm2_user_tab_cols t,
     dm2_admin_tables at
    PLAN (at)
     JOIN (t
     WHERE at.table_name=t.table_name)
    ORDER BY t.table_name
    HEAD t.table_name
     indx = 0, tblx = 0, tblx = locateval(indx,1,admin_rs->admin_tbl_cnt,t.table_name,admin_rs->
      admin_tbl[indx].admin_table_name)
     IF (tblx > 0
      AND (admin_rs->admin_tbl[tblx].view_found_ind=0))
      tblx = 0
     ENDIF
    DETAIL
     IF (tblx > 0)
      indx = 0, colx = locateval(indx,1,admin_rs->admin_tbl[tblx].admin_col_cnt,t.column_name,
       admin_rs->admin_tbl[tblx].admin_col[indx].admin_col_name)
      IF (colx > 0)
       admin_rs->admin_tbl[tblx].view_col_cnt = (admin_rs->admin_tbl[tblx].view_col_cnt+ 1)
      ENDIF
     ENDIF
    FOOT  t.table_name
     IF (tblx > 0)
      IF ((admin_rs->admin_tbl[tblx].admin_col_cnt != admin_rs->admin_tbl[tblx].view_col_cnt))
       admin_rs->view_col_cnt_diff_ind = 1
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("view_col_cnt_diff_ind =",admin_rs->view_col_cnt_diff_ind))
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE log_differences(null)
   DECLARE ld_line = vc WITH protect, noconstant("")
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("diff_log=",dm_err->logfile))
   ENDIF
   SELECT INTO dm_err->logfile
    FROM (dummyt d  WITH seq = value(admin_rs->admin_tbl_cnt))
    HEAD REPORT
     row + 1, "Admin / CCL Definition Differences", row + 1,
     "---------------------------------------------", row + 1, row + 1,
     "Admin Table Count:                 ", admin_rs->admin_tbl_cnt, row + 1,
     "Admin Tables needing View Count :  ", dsl_dmtools_adm_tables->cnt, row + 1,
     "View Count:                        ", admin_rs->view_cnt
     IF ((admin_rs->view_col_cnt_diff_ind=1))
      col 50, "***** Admin to View Column Count Differences Exist *****"
     ENDIF
     row + 1, "CCL Definition Table Count:        ", admin_rs->ccl_def_cnt
     IF ((admin_rs->ccl_def_diff_ind=1))
      col 50, "***** Admin to CCL Definition Column Count Differences Exist *****"
     ENDIF
     row + 1, row + 1, col 37,
     "Admin Col ", col 48, " CCL Def  ",
     row + 1, col 01, "Table Name",
     col 37, "  Count   ", col 48,
     "Col Count ", col 59, "Differences",
     row + 1, col 01, "-----------------------------------",
     col 37, "----------", col 48,
     "----------", col 59, "-------------------------------------------------------------"
    DETAIL
     row + 1, col 01, admin_rs->admin_tbl[d.seq].admin_table_name,
     col 36, admin_rs->admin_tbl[d.seq].admin_col_cnt, col 47,
     admin_rs->admin_tbl[d.seq].ccl_def_col_cnt
     IF ((admin_rs->admin_tbl[d.seq].nick_found_ind=0))
      col 59, "***** Synonym Not Found ******"
     ELSEIF ((admin_rs->admin_tbl[d.seq].nick_found_ind=1))
      IF ((admin_rs->admin_tbl[d.seq].ccl_def_found_ind=0)
       AND dovd_check_defs=1)
       col 59, "***** CCL Definition Not Found ******"
      ELSEIF ((admin_rs->admin_tbl[d.seq].ccl_def_found_ind=1))
       IF ((admin_rs->admin_tbl[d.seq].admin_col_cnt != admin_rs->admin_tbl[d.seq].ccl_def_col_cnt))
        col 59, "***** Admin to CCL Definition Column Count Differences *****"
       ELSEIF ((admin_rs->admin_tbl[d.seq].admin_col_cnt=admin_rs->admin_tbl[d.seq].ccl_def_col_cnt))
        IF ((admin_rs->admin_tbl[d.seq].view_found_ind=0)
         AND (admin_rs->admin_tbl[d.seq].needs_view_ind=1)
         AND dovd_check_views=1)
         col 59, "***** View Not Found *****"
        ELSEIF ((admin_rs->admin_tbl[d.seq].view_found_ind=1)
         AND (admin_rs->admin_tbl[d.seq].needs_view_ind=1))
         IF ((admin_rs->admin_tbl[d.seq].admin_col_cnt != admin_rs->admin_tbl[d.seq].view_col_cnt)
          AND dovd_check_view_cols=1)
          ld_line = concat("***** Admin (",trim(cnvtstring(admin_rs->admin_tbl[d.seq].admin_col_cnt)),
           ") to View (",trim(cnvtstring(admin_rs->admin_tbl[d.seq].view_col_cnt)),
           ") Column Level Differences *****"), col 59, ld_line
         ENDIF
        ENDIF
       ENDIF
      ENDIF
   ...
   ;end select
   SELECT INTO dm_err->logfile
    FROM (dummyt d  WITH seq = value(admin_seq_rs->admin_seq_cnt))
    HEAD REPORT
     row + 5, row + 1, "Admin Sequence / Synonym Differences",
     row + 1, "---------------------------------------------", row + 1,
     row + 1, "Admin Sequence Count:          ", admin_seq_rs->admin_seq_cnt,
     row + 1, "Synonym Count:                 ", admin_seq_rs->nick_seq_cnt,
     row + 1, "Clinical Sequence Count:       ", admin_seq_rs->clin_seq_cnt
     IF ( NOT ((admin_seq_rs->admin_seq_cnt=admin_seq_rs->nick_seq_cnt)))
      col 50, "***** Admin Sequences to Clinical Synonym Count Differences Exist *****"
     ENDIF
     IF ((admin_seq_rs->clin_seq_cnt > 0))
      col 50, "***** Admin Sequences Also Exist on Clinical *****"
     ENDIF
     row + 1, row + 1, col 01,
     "Sequence Name", col 37, "Differences",
     row + 1, col 01, "-----------------------------------",
     col 37, "----------"
    DETAIL
     row + 1, col 01, admin_seq_rs->admin_seq[d.seq].admin_seq_name
     IF ((admin_seq_rs->admin_seq[d.seq].nick_found_ind=0))
      row + 1, col 37, "***** Synonym Not Found ******"
     ENDIF
     IF ((admin_seq_rs->admin_seq[d.seq].clin_seq_found_ind=1))
      row + 1, col 37, "***** Clinical Sequence Found ******"
     ENDIF
    FOOT REPORT
     row + 0
    WITH nocounter, append
   ;end select
   IF (check_error("write_domain_setup_row")=true)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET create_admin_nicknames_ind = 1
   RETURN(1)
 END ;Subroutine
 SUBROUTINE check_domain_setup_executing(null)
  SELECT INTO "nl:"
   i.info_name
   FROM dm_info i
   WHERE i.info_domain="DATA MANAGEMENT"
   ...
   WITH nocounter
  ;end select
  IF (check_error("check_domain_setup_executing")=true)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   RETURN(0)
  ELSE
   IF (curqual > 0)
  ...
  ENDIF
 END ;Subroutine
 SUBROUTINE write_domain_setup_row(null)
  INSERT  FROM dm_info i
   SET i.info_domain = "DATA MANAGEMENT", i.info_name = "DOMAIN SETUP CURRENTLY EXECUTING", i
    ...
    i.info_char = currdbhandle, i.info_number = 0, i.info_long_id = 0,
  ...
  ENDIF
 END ;Subroutine
 SUBROUTINE delete_domain_setup_row(null)
  DELETE  FROM dm_info i
   WHERE i.info_domain="DATA MANAGEMENT"
    AND i.info_name="DOMAIN SETUP CURRENTLY EXECUTING"
   WITH nocounter
  ;end delete
  IF (check_error("delete_domain_setup_row")=true)
   ROLLBACK
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   RETURN(0)
  ELSE
  ...
  ENDIF
 END ;Subroutine
 SUBROUTINE load_admin_seq_rs(null)
   SET dm_err->eproc = "Loading Admin Sequences Record Structure"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET lasr_dyn_in_clause = dm2_fill_seq_list("us","sequence_name")
   SELECT INTO "nl:"
    FROM dm2_admin_sequences us
    WHERE parser(lasr_dyn_in_clause)
    ORDER BY us.sequence_name
    HEAD REPORT
     seq_cnt = 0, stat = alterlist(admin_seq_rs->admin_seq,5)
    DETAIL
     seq_cnt = (seq_cnt+ 1)
     IF (mod(seq_cnt,5)=1
      AND seq_cnt != 1)
      stat = alterlist(admin_seq_rs->admin_seq,(seq_cnt+ 4))
     ENDIF
     admin_seq_rs->admin_seq[seq_cnt].admin_seq_name = us.sequence_name
    FOOT REPORT
     admin_seq_rs->admin_seq_cnt = seq_cnt, stat = alterlist(admin_seq_rs->admin_seq,seq_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 END ;Subroutine
#end_program
 SET dm_err->eproc = "Ending dm2_ocd_val_domain."
 CALL final_disp_msg("dm2_ocd_val_domain")
END GO
