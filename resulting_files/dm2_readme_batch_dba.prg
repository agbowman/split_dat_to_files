CREATE PROGRAM dm2_readme_batch:dba
 ...
 SUBROUTINE dmai_get_cur_mod_act(dgcma_mod_out,dmgca_act_out)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD drb_owner_list
 RECORD drb_owner_list(
   1 cnt = i4
   1 qual[*]
     2 owner = vc
 )
 DECLARE drb_initialize(null) = i2
 DECLARE drb_setup_inhouse(null) = i2
 DECLARE drb_setup_outhouse(null) = i2
 DECLARE drb_prepare_batch(null) = i2
 DECLARE drb_rr_check_readme_failures(rcrf_env_id=f8,rcrf_batch_dt_tm=f8,rcrf_readme_error_ind=i2(ref
   )) = i2
 DECLARE drb_rr_finish_readmes(rwfr_env_id=f8,rwfr_batch_dt_tm=f8,rwfr_inhouse=i2,rwfr_chk_ind=i2,
  rwfr_running_ind=i2(ref),
  rwfr_null_ind=i2(ref)) = i2
 DECLARE drb_process_owner(dpo_sbr_owner=vc) = i2
 DECLARE drb_readme_error_ret = i2 WITH protect, noconstant(0)
 DECLARE drb_pkg_status = i2 WITH protect, noconstant(0)
 DECLARE drb_runner_timestamp = f8 WITH protect, noconstant(0.0)
 DECLARE drb_running_ind = i2 WITH protect, noconstant(0)
 DECLARE drb_null_ind = i2 WITH protect, noconstant(0)
 DECLARE drb_display_msg = vc WITH protect, noconstant(" ")
 DECLARE drb_display_msg2 = vc WITH protect, noconstant(" ")
 DECLARE drb_paused_ind = i2 WITH protect, noconstant(0)
 DECLARE drb_readme_cnt = i4 WITH protect, noconstant(0)
 DECLARE drb_readme_comp_cnt = i4 WITH protect, noconstant(0)
 DECLARE drb_readme_exec_cnt = i4 WITH protect, noconstant(0)
 DECLARE drb_last_readme = vc WITH protect, noconstant("")
 DECLARE drb_monitor_status = i2 WITH protect, noconstant(0)
 DECLARE drb_accept = vc WITH protect, noconstant("")
 DECLARE drb_init_owner = vc WITH protect, noconstant(" ")
 DECLARE drb_dbname = vc WITH protect, noconstant(" ")
 DECLARE drb_cnct_str = vc WITH protect, noconstant(" ")
 DECLARE drb_idx = i4 WITH protect, noconstant(0)
 DECLARE drb_readme_ind = i2 WITH protect, noconstant(0)
 DECLARE drb_module_name = vc WITH protect, constant("DM_INSTALL_PROCESS")
 DECLARE drb_action_name = vc WITH protect, constant("MAIN_RUNNER")
 DECLARE drb_original_module = vc WITH protect, noconstant("")
 DECLARE drb_original_action = vc WITH protect, noconstant("")
 DECLARE drb_mod_act_set_ind = i2 WITH protect, noconstant(0)
 DECLARE drb_killed_readme_ind = i2 WITH protect, noconstant(0)
 DECLARE drb_v500_pwd = vc WITH protect, noconstant("DM2NOTSET")
 SET drb_runner_timestamp = cnvtlookbehind("10,MIN",cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)))
 IF (check_logfile("dm2_readme_batch",".log","dm2_readme_batch")=0)
  GO TO exit_program
 ENDIF
 IF (drb_initialize(null)=0)
  GO TO exit_program
 ENDIF
 IF ((inhouse_misc->inhouse_domain=1))
  IF (currdbuser != "V500")
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Verify initial database user is V500."
   SET dm_err->emsg = concat("Initial user (",trim(currdbuser),") invalid.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
  SET drb_init_owner = currdbuser
  IF (drb_process_owner(drb_init_owner)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Getting additional owners to process."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT DISTINCT INTO "nl:"
   dtd.owner
   FROM dm_tables_doc_all dtd,
    dba_users du
   WHERE  NOT (dtd.owner IN (drb_init_owner, "CDBA"))
    AND dtd.owner=du.username
   ORDER BY dtd.owner
   HEAD REPORT
    drb_owner_list->cnt = 0, stat = alterlist(drb_owner_list->qual,0)
   DETAIL
    drb_owner_list->cnt = (drb_owner_list->cnt+ 1)
    IF (mod(drb_owner_list->cnt,5)=1)
     stat = alterlist(drb_owner_list->qual,(drb_owner_list->cnt+ 4))
    ENDIF
    drb_owner_list->qual[drb_owner_list->cnt].owner = dtd.owner
   FOOT REPORT
    stat = alterlist(drb_owner_list->qual,drb_owner_list->cnt)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echorecord(drb_owner_list)
  ENDIF
  IF ((drb_owner_list->cnt > 0))
   FOR (drb_idx = 1 TO drb_owner_list->cnt)
     IF (drb_process_owner(drb_owner_list->qual[drb_idx].owner)=0)
      IF ((dm_err->err_ind > 0))
       SET dm_err->err_ind = 0
       SET dm_err->eproc = build(
        "Process of owner was unsuccessful - continuing on with inhouse readme process.")
       CALL disp_msg("",dm_err->logfile,0)
      ENDIF
     ENDIF
   ENDFOR
  ENDIF
 ELSE
  IF (drb_setup_outhouse(null)=0)
   GO TO exit_program
  ENDIF
  IF (dm2_rr_clean_stranded_readmes(dm2_rr_misc->env_id)=0)
   GO TO exit_program
  ENDIF
  IF (drb_prepare_batch(null)=0)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Checking if any readmes are present"
  SELECT INTO "nl:"
   FROM dm_ocd_log l,
    dm_readme d
   WHERE (l.environment_id=dm2_rr_misc->env_id)
    AND l.project_type=dm2_rr_readme
    AND l.batch_dt_tm BETWEEN cnvtdatetime("01-JAN-1800") AND cnvtdatetime("31-DEC-2100")
    AND l.start_dt_tm = null
    AND l.active_ind=1
    AND l.project_name=trim(cnvtstring(d.readme_id),3)
    AND l.project_instance=d.instance
   DETAIL
    drb_readme_ind = 1
   WITH nocounter, maxqual(l,1)
  ;end select
  ...
  ENDIF
  IF (drb_readme_ind=1)
   IF ((drr_flex_sched->pkg_using_schedule=0))
    IF (drr_use_flexible_schedule(0,"n/a","n/a",drb_accept)=0)
     GO TO exit_program
    ENDIF
   ENDIF
   IF ((drr_flex_sched->pkg_using_schedule=0))
    ...
    ENDIF
    IF ((dm2_rdbms_version->level1 >= 11))
     IF (dmai_get_cur_mod_act(drb_original_module,drb_original_action)=0)
      GO TO exit_program
     ENDIF
     CALL dmai_set_mod_act(drb_module_name,drb_action_name)
     SET drb_mod_act_set_ind = 1
    ENDIF
    EXECUTE dm2_readme_runner
    IF ((dm_err->err_ind=1))
     GO TO exit_program
    ENDIF
    IF (drb_rr_finish_readmes(dm2_rr_misc->env_id,dm2_rr_misc->batch_dt_tm,inhouse_misc->
     inhouse_domain,0,drb_running_ind,
     drb_null_ind)=0)
     GO TO exit_program
    ENDIF
   ELSE
    SET drb_running_ind = 1
    SET drb_null_ind = 1
    WHILE (((drb_running_ind=1) OR (drb_null_ind=1)) )
      IF ((dm_err->debug_flag > 1))
       CALL echo(concat("Current date and time = ",format(cnvtdatetime(curdate,curtime3),";;q")))
       CALL echo(concat("Last runner date and time = ",format(drb_runner_timestamp,";;q")))
       CALL echo(concat("Number of minutes elapsed = ",cnvtstring(datetimediff(cnvtdatetime(curdate,
            curtime3),cnvtdatetime(drb_runner_timestamp),4),4,3)))
      ENDIF
      IF (datetimediff(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),cnvtdatetimeutc(cnvtdatetime(
         drb_runner_timestamp)),4) > 5)
       SET drb_paused_ind = 1
       WHILE (drb_paused_ind=1)
         SET drb_runner_timestamp = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3))
         SET drb_pkg_status = 0
         IF (drr_get_process_status("DM2_INSTALL_PKG",abs(cnvtreal(drr_flex_sched->pkg_number)),
          drb_pkg_status)=0)
          GO TO exit_program
         ENDIF
         SET dir_ui_misc->install_status = drb_pkg_status
         IF ((dir_ui_misc->install_status=0))
          SET dm_err->err_ind = 1
          SET dm_err->emsg = "Package install has been marked to stop."
          SET dm_err->eproc = "Checking if package being installed has been marked to stop."
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          GO TO exit_program
         ELSEIF ((dir_ui_misc->install_status=2))
          IF (dipr_update_install_itinerary(dir_ui_misc->install_status,dm2_process_event_rs->
           itinerary_process_event_id,abs(cnvtreal(drr_flex_sched->pkg_number)))=0)
           GO TO exit_program
          ENDIF
          CALL dpl_upd_dped_last_status(dir_ui_misc->dm_process_event_id,concat("As of ",format(
             cnvtdatetime(curdate,curtime3),"dd-mmm-yyyy hh:mm:ss;;d"),
            ",readme execution is in a PAUSED state."),0.0,cnvtdatetime(curdate,curtime3))
          SET drb_monitor_status = 0
          IF (drr_get_process_status("DM2_INSTALL_MONITOR",abs(cnvtreal(drr_flex_sched->pkg_number)),
           drb_monitor_status)=0)
           GO TO exit_program
          ENDIF
          IF (drb_monitor_status=0)
           IF (drr_submit_background_process(trim(currdbuser,3),dm2_install_schema->v500_p_word,
            ...
            abs(cnvtreal(drr_flex_sched->pkg_number)),drr_flex_sched->pkg_install_mode)=0)
            GO TO exit_program
           ENDIF
          ENDIF
          ...
          ENDIF
         ELSE
          SET drb_paused_ind = 0
          IF ((dm2_process_event_rs->ui_allowed_ind=1))
           IF (dipr_update_install_itinerary(dir_ui_misc->install_status,dm2_process_event_rs->
            itinerary_process_event_id,abs(cnvtreal(drr_flex_sched->pkg_number)))=0)
            GO TO exit_program
           ENDIF
           SET dm_err->eproc = "Getting readme counts."
           SELECT INTO "nl:"
            FROM dm_ocd_log dol
            WHERE (dol.environment_id=dm2_rr_misc->env_id)
             AND dol.project_type=dm2_rr_readme
             AND dol.batch_dt_tm=cnvtdatetimeutc(dm2_rr_misc->batch_dt_tm)
            ORDER BY dol.start_dt_tm
            HEAD REPORT
             drb_readme_cnt = 0, drb_readme_comp_cnt = 0, drb_readme_exec_cnt = 0,
             drb_last_readme = " "
            DETAIL
             drb_readme_cnt = (drb_readme_cnt+ 1)
             IF (dol.status=dm2_rr_done)
              drb_readme_comp_cnt = (drb_readme_comp_cnt+ 1)
             ENDIF
             IF (dol.status=dm2_rr_running)
              drb_readme_exec_cnt = (drb_readme_exec_cnt+ 1)
             ENDIF
             drb_last_readme = dol.project_name
            WITH nocounter
           ;end select
           IF (check_error(dm_err->eproc)=1)
            CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
           ...
           IF (drb_readme_cnt > 0)
            CALL dpl_upd_dped_last_status(dir_ui_misc->dm_process_event_id,concat(
              "Readmes are executing: ",trim(cnvtstring(drb_readme_comp_cnt))," of ",trim(cnvtstring(
                drb_readme_cnt))," are complete. Latest readme executed/ing: ",
              drb_last_readme,"."),0.0,cnvtdatetime(curdate,curtime3))
           ENDIF
          ENDIF
         ENDIF
       ENDWHILE
       IF (drb_null_ind=1)
        IF ((dir_ui_misc->install_status=1))
         IF (drr_maintain_runners(trim(currdbuser,3),dm2_install_schema->v500_p_word,
          dm2_install_schema->v500_connect_str,dir_batch_queue,"README")=0)
          GO TO exit_program
         ENDIF
        ENDIF
       ENDIF
      ENDIF
      IF (drb_rr_finish_readmes(dm2_rr_misc->env_id,dm2_rr_misc->batch_dt_tm,inhouse_misc->
       inhouse_domain,1,drb_running_ind,
       drb_null_ind)=0)
       GO TO exit_program
      ENDIF
      IF (((drb_running_ind=1) OR (drb_null_ind=1)) )
       IF (drb_null_ind=1)
        SET drb_display_msg = concat("As of ",format(cnvtdatetimeutc(cnvtdatetime(
            drb_runner_timestamp)),"mm/dd/yyyy hh:mm:ss;;d"))
        SET drb_display_msg = concat(drb_display_msg,", the flexible schedule states that ",trim(
          cnvtstring(drr_flex_sched->num_sched_runners))," runner(s) should be running.")
        IF ((drr_flex_sched->num_runners_to_start > 0))
         SET drb_display_msg2 = concat(trim(cnvtstring(drr_flex_sched->num_runners_to_start)),
          " runner(s) were ","started to meet number of runner(s) that should be running.")
        ELSEIF ((drr_flex_sched->num_runners_to_stop > 0))
         SET drb_display_msg2 = concat(trim(cnvtstring(drr_flex_sched->num_runners_to_stop)),
          " runner(s) were ","stopped to meet number of runner(s) that should be running.")
        ELSE
         SET drb_display_msg2 =
         "Currently running the specified number of runners from flexible schedule."
        ENDIF
        CALL echo("**************************************************")
        CALL echo(drb_display_msg)
        CALL echo(drb_display_msg2)
        CALL echo("**************************************************")
       ENDIF
       CALL echo(" ")
       CALL echo("**************************************************")
       CALL echo(concat(format(cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
          "mm/dd/yyyy hh:mm:ss;;d"),"  Waiting on background runners to complete readmes..."))
       CALL echo("**************************************************")
       CALL pause(30)
      ENDIF
    ENDWHILE
   ENDIF
   IF (drb_rr_check_readme_failures(dm2_rr_misc->env_id,dm2_rr_misc->batch_dt_tm,drb_readme_error_ret
    )=0)
    GO TO exit_program
   ENDIF
   IF (drb_readme_error_ret=1)
    SELECT INTO "nl:"
     FROM dm_ocd_log dol
     WHERE (dol.environment_id=dm2_rr_misc->env_id)
      AND dol.status=dm2_rr_failed
      AND dol.message=dir_kill_clause
      AND dol.project_type=dm2_rr_readme
      AND dol.batch_dt_tm=cnvtdatetimeutc(dm2_rr_misc->batch_dt_tm)
     WITH nocounter, maxqual(dol,1)
    ;end select
    IF (check_error("Check if install failed due to concurrency detected.")=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_program
    ENDIF
    IF (curqual > 0)
     SET drb_killed_readme_ind = 1
    ENDIF
    SET dm_err->emsg = concat("One or more errors were encountered",evaluate(drb_killed_readme_ind,1,
      " and one or more killed ",0," "),"executing readmes.")
   ENDIF
   SET dm2_rr_misc->readme_errors_ind = drb_readme_error_ret
  ENDIF
 ENDIF
 GO TO exit_program
 SUBROUTINE drb_initialize(null)
   DECLARE di_env_hold = f8 WITH protect, noconstant(0.0)
   IF (validate(rb_ocd,1)=1
    AND validate(rb_ocd,2)=2)
    SET dm2_rr_misc->package_number = 0
   ELSE
    SET dm2_rr_misc->package_number = rb_ocd
   ENDIF
   SET reqinfo->updt_task = 15301
   IF ((dm2_rr_misc->env_id=0.0))
    IF (dm2_get_env_data(1,di_env_hold)=0)
     RETURN(0)
    ENDIF
    SET dm2_rr_misc->env_id = di_env_hold
   ENDIF
   SET dm2_rr_misc->batch_dt_tm = cnvtdatetime(curdate,curtime3)
   SET dm_err->eproc = "Perform INHOUSE DOMAIN check"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   ...
   IF ((inhouse_misc->inhouse_domain=0))
    IF (validate(rb_execution,"X")="X"
     AND validate(rb_execution,"Y")="Y")
     SET dm_err->eproc = "Check for mode of execution."
     SET dm_err->emsg = "No mode of execution was specified."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ELSE
     SET dm2_rr_misc->execution = rb_execution
    ENDIF
   ELSE
    ...
    ENDIF
   ENDIF
   IF (inhouse_misc->inhouse_domain)
    SET dm2_rr_misc->manual_readme_id = cnvtint( $1)
   ENDIF
   IF (dm2_rr_misc->manual_readme_id)
    SET dm2_rr_misc->low_proj_name = trim(cnvtstring(dm2_rr_misc->manual_readme_id),3)
    SET dm2_rr_misc->high_proj_name = dm2_rr_misc->low_proj_name
   ELSE
    SET dm2_rr_misc->low_proj_name = "0"
    SET dm2_rr_misc->high_proj_name = "999999999"
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_setup_inhouse(null)
   DECLARE dsi_rb_cnt = i4 WITH protect, noconstant(0)
   DECLARE dsi_inactive_cnt = i4 WITH protect, noconstant(0)
   IF (dm2_rr_misc->manual_readme_id)
    SET dm_err->eproc = "Clear project status."
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    UPDATE  FROM dm_project_status_env s
     SET s.dm_status = null, s.dm_status_dt_tm = null, s.dm_status_msg = null,
      s.processed_flag = 0
     WHERE (s.environment_id=dm2_rr_misc->env_id)
      AND s.proj_type=dm2_rr_readme
      AND s.proj_name=trim(cnvtstring(dm2_rr_misc->manual_readme_id),3)
      AND (s.source_set_instance=
     (SELECT
      max(r.source_set_instance)
      FROM dm_project_status_env r
      WHERE (r.environment_id=dm2_rr_misc->env_id)
       AND r.proj_type=dm2_rr_readme
       AND r.proj_name=s.proj_name
      WITH nocounter))
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
    ...
    ELSE
     IF (curqual=0)
      SET dm_err->emsg = concat("No readme information found for readme ",trim(cnvtstring(dm2_rr_misc
         ->manual_readme_id),3),".  Make sure this readme has been built in Feature Tracker.")
      SET dm_err->err_ind = 1
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      RETURN(0)
     ELSE
    ...
    ENDIF
   ENDIF
   SET dm2_rr_misc->process_type = "IH"
   IF (drr_load_readmes_to_run(null)=0)
    RETURN(0)
   ENDIF
   SET dsi_rb_cnt = 0
   SET dsi_inactive_cnt = 0
   SET dsi_rb_cnt = size(drr_readmes_to_run->inactive,5)
   IF (dsi_rb_cnt)
    SET dm_err->eproc = "Mark inactive readmes as successful"
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    UPDATE  FROM dm_project_status_env s,
      (dummyt d  WITH seq = value(dsi_rb_cnt))
     SET s.dm_status = dm2_rr_done, s.dm_status_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3
        )), s.dm_status_msg = "This readme is inactive.  Automatically marked as successful."
     PLAN (d)
      JOIN (s
      WHERE s.proj_type=dm2_rr_readme
       AND (s.proj_name=drr_readmes_to_run->inactive[d.seq].name)
       AND (s.environment_id=dm2_rr_misc->env_id)
       AND (s.source_set_instance <= drr_readmes_to_run->inactive[d.seq].instance))
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
     ROLLBACK
     ...
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Delete  readmes from dm_ocd_log."
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    DELETE  FROM dm_ocd_log l,
     ...
     PLAN (d)
      ...
      WHERE l.project_type=dm2_rr_readme
       AND (l.project_name=drr_readmes_to_run->inactive[d.seq].name)
       AND (l.environment_id=dm2_rr_misc->env_id))
     WITH nocounter
    ;end delete
    IF (check_error(dm_err->eproc)=1)
     ...
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Insert readmes into dm_ocd_log."
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    INSERT  FROM dm_ocd_log l,
      (dummyt d  WITH seq = value(dsi_rb_cnt))
     SET l.status = dm2_rr_done, l.start_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)), l
      .end_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
      l.updt_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)), l.message =
      "This readme is inactive.  Automatically marked as successful.", l.project_type = dm2_rr_readme,
      l.project_name = drr_readmes_to_run->inactive[d.seq].name, l.environment_id = dm2_rr_misc->
      env_id, l.project_instance = drr_readmes_to_run->inactive[d.seq].instance,
      l.ocd = 0, l.batch_dt_tm = cnvtdatetimeutc(dm2_rr_misc->batch_dt_tm), l.active_ind = 1,
      l.appl_ident = trim(validate(currdbhandle," "))
     PLAN (d)
      JOIN (l)
     WITH nocounter
    ;end insert
    ...
    COMMIT
   ENDIF
   IF (size(drr_readmes_to_run->readme,5))
    SET dm_err->eproc = "Mark space check readmes as successful"
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    UPDATE  FROM dm_project_status_env s,
      (dummyt d  WITH seq = value(size(drr_readmes_to_run->readme,5)))
     SET s.dm_status = dm2_rr_done, s.dm_status_dt_tm = cnvtdatetime(curdate,curtime3), s
      .dm_status_msg = "This readme is for tablespace check.  Automatically marked as successful."
     PLAN (d
      WHERE (drr_readmes_to_run->readme[d.seq].execution IN ("PRESPCHK", "POSTSPCHK")))
      JOIN (s
      WHERE s.proj_type=dm2_rr_readme
       AND (s.proj_name=drr_readmes_to_run->readme[d.seq].name)
       AND (s.environment_id=dm2_rr_misc->env_id)
       AND (s.source_set_instance <= drr_readmes_to_run->readme[d.seq].instance))
     WITH nocounter
    ;end update
    IF (check_error(dm_err->eproc)=1)
     ROLLBACK
    ...
    ENDIF
    COMMIT
    SET dm_err->eproc = "Mark timer readmes as successful"
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    UPDATE  FROM dm_project_status_env s,
      (dummyt d  WITH seq = value(size(drr_readmes_to_run->readme,5)))
     SET s.dm_status = dm2_rr_done, s.dm_status_dt_tm = cnvtdatetime(curdate,curtime3), s
      .dm_status_msg = "This readme is for timing estimates.  Automatically marked as successful."
     PLAN (d
      WHERE (drr_readmes_to_run->readme[d.seq].execution IN ("RDMTIMER")))
      JOIN (s
      WHERE s.proj_type=dm2_rr_readme
       AND (s.proj_name=drr_readmes_to_run->readme[d.seq].name)
       AND (s.environment_id=dm2_rr_misc->env_id)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_setup_outhouse(null)
   IF (validate(doc->source_ocd_cnt,1)=1
    AND validate(doc->source_ocd_cnt,2)=2)
    SET dm2_rr_misc->process_type = "PI"
   ELSE
    SET dm2_rr_misc->process_type = "MM"
   ENDIF
   SET dm_err->eproc = "Cleanup RESET status"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   UPDATE  FROM dm_ocd_log dol
    SET dol.status = dm2_rr_failed, dol.start_dt_tm = cnvtdatetime(curdate,curtime3), dol.message =
     "RESET readme set to FAILED due to Install restart."
    WHERE dol.project_type=dm2_rr_readme
     AND dol.status=dm2_rr_reset
     AND dol.start_dt_tm = null
     AND (dol.environment_id=dm2_rr_misc->env_id)
     AND (dol.ocd=dm2_rr_misc->package_number)
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
   ...
   IF (drr_load_readmes_to_run(null)=0)
    RETURN(0)
   ENDIF
   IF (drr_load_timing_readmes(null)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_prepare_batch(null)
   DECLARE dpb_rb_cnt = i4 WITH protect, noconstant(0)
   DECLARE dpb_row_cnt = f8 WITH protect, noconstant(0.0)
   DECLARE dpb_exists = i2 WITH protect, noconstant(0)
   DECLARE dpb_cnt = i4 WITH protect, noconstant(0)
   DECLARE dpb_rows = f8 WITH protect, noconstant(0.0)
   DECLARE dpb_seconds = f8 WITH protect, noconstant(0.0)
   DECLARE dpb_minutes = f8 WITH protect, noconstant(0.0)
   DECLARE drtr_timing_script = vc WITH public, noconstant(" ")
   DECLARE drtr_timing_readme = f8 WITH public, noconstant(0.0)
   DECLARE drtr_timing_instance = i4 WITH public, noconstant(0)
   DECLARE drtr_timing_ocd = i4 WITH public, noconstant(0)
   DECLARE drtr_env_id = f8 WITH public, noconstant(0.0)
   DECLARE drtr_parent_readme = f8 WITH public, noconstant(0.0)
   FOR (dpb_cnt = 1 TO drr_readmes_to_run->timer_readme_cnt)
     SET drtr_timing_script = value(drr_readmes_to_run->timer_readme[dpb_cnt].script)
     SET drtr_timing_readme = value(drr_readmes_to_run->timer_readme[dpb_cnt].readme_id)
     SET drtr_timing_instance = value(drr_readmes_to_run->timer_readme[dpb_cnt].instance)
     SET drtr_timing_ocd = value(drr_readmes_to_run->timer_readme[dpb_cnt].ocd)
     SET drtr_env_id = dm2_rr_misc->env_id
     SET drtr_parent_readme = value(drr_readmes_to_run->timer_readme[dpb_cnt].parent_readme_id)
     IF (checkprg(drtr_timing_script) > 0)
      SET dpb_exists = 1
     ELSE
      SET dpb_exists = 0
      CALL echo(build("Timing script not found: ",drtr_timing_script))
     ENDIF
     IF (dpb_exists=1
      AND (drr_readmes_to_run->timer_readme[dpb_cnt].skip=0))
      EXECUTE value(drtr_timing_script)
     ENDIF
   ENDFOR
   FOR (dpb_rb_cnt = 1 TO size(drr_readmes_to_run->readme,5))
     IF (drr_readmes_to_run->readme[dpb_rb_cnt].skip)
      IF ((dm_err->debug_flag > 0))
       SET dm_err->eproc = concat("Skipped readme:",drr_readmes_to_run->readme[dpb_rb_cnt].name)
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
     ELSE
      IF ((drr_readmes_to_run->readme[dpb_rb_cnt].estimated_time > 0.0))
       SET dpb_minutes = drr_readmes_to_run->readme[dpb_rb_cnt].estimated_time
       SET drr_readmes_to_run->readme[dpb_rb_cnt].execution_time = ((dpb_minutes * 60.0)/ ((24.0 *
       60.0) * 60.0))
      ELSE
       SET dpb_seconds = drr_readmes_to_run->readme[dpb_rb_cnt].execution_time
       IF (size(trim(drr_readmes_to_run->readme[dpb_rb_cnt].driver_table,3)))
        IF (dir_row_count(drr_readmes_to_run->readme[dpb_rb_cnt].driver_table,dpb_row_cnt)=0)
         RETURN(0)
        ENDIF
        IF (dpb_row_cnt)
         SET dm_err->eproc = build("Gather time estimates for readme:",drr_readmes_to_run->readme[
          dpb_rb_cnt].readme_id)
         IF ((dm_err->debug_flag > 0))
          CALL disp_msg(" ",dm_err->logfile,0)
         ENDIF
         SELECT INTO "nl:"
          l.driver_count
          FROM dm_ocd_log l
          WHERE l.project_type="README"
           AND l.project_name=trim(cnvtstring(drr_readmes_to_run->readme[dpb_rb_cnt].readme_id),3)
           AND (l.project_instance=drr_readmes_to_run->readme[dpb_rb_cnt].instance)
           AND l.status="SUCCESS"
           AND l.start_dt_tm IS NOT null
           AND l.end_dt_tm IS NOT null
          DETAIL
           IF (l.driver_count)
            dpb_rows = (dpb_rows+ l.driver_count), dpb_seconds = (dpb_seconds+ datetimediff(l
         ...
         IF (check_error(dm_err->eproc)=1)
          ...
          RETURN(0)
         ENDIF
         SET drr_readmes_to_run->readme[dpb_rb_cnt].driver_count = dpb_row_cnt
         IF (dpb_rows)
          SET dpb_seconds = ((dpb_seconds/ dpb_rows) * dpb_row_cnt)
         ELSE
          SET dpb_seconds = (drr_readmes_to_run->readme[dpb_rb_cnt].execution_time * dpb_row_cnt)
         ENDIF
        ENDIF
       ENDIF
       SET drr_readmes_to_run->readme[dpb_rb_cnt].estimated_time = (dpb_seconds/ ((24.0 * 60.0) *
       60.0))
      ENDIF
      IF ((dm_err->debug_flag >= 2))
       CALL echorecord(drr_readmes_to_run)
      ENDIF
      SET dm_err->eproc = "Clearing readme from DM_OCD_LOG"
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      DELETE  FROM dm_ocd_log l
       WHERE (l.environment_id=dm2_rr_misc->env_id)
        AND l.project_type=dm2_rr_readme
        AND (l.project_name=drr_readmes_to_run->readme[dpb_rb_cnt].name)
        AND (l.ocd=drr_readmes_to_run->readme[dpb_rb_cnt].ocd)
       WITH nocounter
      ;end delete
      IF (check_error(dm_err->eproc)=1)
       ROLLBACK
       ...
       RETURN(0)
      ENDIF
      SET dm_err->eproc = "Inserting readmes into dm_ocd_log."
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      INSERT  FROM dm_ocd_log l
       SET l.environment_id = dm2_rr_misc->env_id, l.project_type = dm2_rr_readme, l.project_name =
        drr_readmes_to_run->readme[dpb_rb_cnt].name,
        l.project_instance = drr_readmes_to_run->readme[dpb_rb_cnt].instance, l.ocd =
        drr_readmes_to_run->readme[dpb_rb_cnt].ocd, l.batch_dt_tm = cnvtdatetimeutc(dm2_rr_misc->
         batch_dt_tm),
        l.driver_count = drr_readmes_to_run->readme[dpb_rb_cnt].driver_count, l.estimated_time =
        drr_readmes_to_run->readme[dpb_rb_cnt].estimated_time, l.active_ind = 1,
        l.updt_dt_tm = cnvtdatetimeutc(dm2_rr_misc->batch_dt_tm), l.appl_ident = trim(validate(
          currdbhandle," "))
       WITH nocounter
      ;end insert
      IF (check_error(dm_err->eproc)=1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_rr_check_readme_failures(rcrf_env_id,rcrf_batch_dt_tm,rcrf_readme_error_ind)
   SET dm_err->eproc = "Checking for readme failures during installation."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM dm_ocd_log d
    WHERE d.environment_id=rcrf_env_id
     AND d.project_type=dm2_rr_readme
     AND d.batch_dt_tm=cnvtdatetimeutc(rcrf_batch_dt_tm)
     AND d.status=dm2_rr_failed
    WITH nocounter, maxqual(d,1)
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   ...
   ENDIF
   IF (curqual=0)
    SET rcrf_readme_error_ind = 0
   ELSE
    SET rcrf_readme_error_ind = 1
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_rr_finish_readmes(rwfr_env_id,rwfr_batch_dt_tm,rwfr_inhouse,rwfr_chk_ind,
  rwfr_running_ind,rwfr_null_ind)
   DECLARE rwfr_wait = i2 WITH protect, noconstant(0)
   DECLARE rwfr_readme_hold = vc WITH protect, noconstant(" ")
   DECLARE rwfr_more = i2 WITH protect, noconstant(0)
   WHILE (rwfr_wait=0)
     IF (rwfr_inhouse=0)
      IF (dm2_rr_clean_stranded_readmes(rwfr_env_id)=0)
       RETURN(0)
      ENDIF
     ENDIF
     IF (rwfr_inhouse=1)
      SET dm_err->eproc = "Update status of pending readmes not executed because of parent failure"
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      UPDATE  FROM dm_project_status_env s
       SET s.dm_status = dm2_rr_failed, s.dm_status_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,
          curtime3)), s.dm_status_msg = "Automatically failed because of a parent readme failure."
       WHERE s.environment_id=rwfr_env_id
        AND s.proj_type=dm2_rr_readme
        AND s.dm_status = null
        AND  EXISTS (
       (SELECT
        l.ocd
        FROM dm_ocd_log l
        WHERE l.environment_id=rwfr_env_id
         AND l.project_type=dm2_rr_readme
         AND l.project_name=s.proj_name
         AND l.project_instance=s.source_set_instance
         AND l.status = null
         AND  EXISTS (
        (SELECT
         r.readme_id
         ...
         WHERE r.readme_id=cnvtint(l.project_name)
          ...
          AND r.parent_readme_id > 0
          AND x.environment_id=rwfr_env_id
          AND x.project_type=dm2_rr_readme
          AND x.project_name=trim(cnvtstring(r.parent_readme_id),3)
          AND x.status=dm2_rr_failed))))
       WITH nocounter
      ;end update
      IF (check_error(dm_err->eproc)=1)
       ROLLBACK
     ...
     WHILE (rwfr_more)
       ...
       ENDIF
       UPDATE  FROM dm_ocd_log l
        SET l.status = dm2_rr_failed, l.start_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
         l.end_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)),
         l.message = "Automatically failed because of a parent readme failure.", l.appl_ident = trim(
          validate(currdbhandle," "))
        WHERE l.environment_id=rwfr_env_id
         AND l.project_type=dm2_rr_readme
         AND l.status = null
         AND  EXISTS (
     ...
     COMMIT
     IF (rwfr_inhouse=0)
      SET dm_err->eproc = "Checking for running readmes"
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg(" ",dm_err->logfile,0)
      ENDIF
      SET rwfr_running_ind = 0
      SET rwfr_null_ind = 0
      SELECT INTO "nl:"
       FROM dm_ocd_log d
       WHERE d.environment_id=rwfr_env_id
        AND d.project_type=dm2_rr_readme
        AND d.batch_dt_tm=cnvtdatetimeutc(rwfr_batch_dt_tm)
        AND ((d.status=dm2_rr_running) OR (((d.status = null) OR (d.status=dm2_rr_reset)) ))
       DETAIL
        IF (((rwfr_running_ind=0) OR (rwfr_null_ind=0)) )
         rwfr_readme_hold = d.project_name
         IF (d.status=dm2_rr_running)
          rwfr_running_ind = 1
         ELSE
          rwfr_null_ind = 1
         ENDIF
      ...
      IF (curqual=0)
       SET rwfr_wait = 1
       SET dm_err->eproc = concat("Finished executing readmes.")
       CALL disp_msg(" ",dm_err->logfile,0)
      ELSE
       IF (rwfr_chk_ind=1)
        SET rwfr_wait = 1
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drb_process_owner(dpo_sbr_owner)
   DECLARE dpo_full_con_str = vc WITH protect, noconstant(" ")
   DECLARE dpo_fnd1 = i2 WITH protect, noconstant(0)
   DECLARE dpo_start_pos = i4 WITH protect, noconstant(0)
   DECLARE dpo_end_pos = i4 WITH protect, noconstant(0)
   DECLARE dpo_di_exists = i2 WITH protect, noconstant(0)
   DECLARE dpo_pswd = vc WITH protect, noconstant(" ")
   SET dm_err->eproc = concat("Processing owner ",dpo_sbr_owner)
   CALL disp_msg(" ",dm_err->logfile,0)
   IF (dpo_sbr_owner != currdbuser)
    SET dm_err->eproc = concat("Connecting to owner ",dpo_sbr_owner)
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dm2_install_schema->dbase_name = drb_dbname
    SET dm2_install_schema->u_name = dpo_sbr_owner
    SET dm2_install_schema->p_word = dpo_sbr_owner
    SET dm2_install_schema->connect_str = drb_cnct_str
    EXECUTE dm2_connect_to_dbase "CO"
    IF ((dm_err->err_ind=1))
     RETURN(0)
    ENDIF
   ENDIF
   IF (dm2_system_defs_init(0)=0)
    SET dm_err->eproc = concat("Bad return from dm2_create_sysdefs_init subroutine that makes sure",
     " required database objects exists.")
    CALL disp_msg(dm_err->eproc,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = concat("Check if DM_INFO table exists under owner ",dpo_sbr_owner)
   CALL disp_msg("",dm_err->logfile,0)
   IF (dm2_table_and_ccldef_exists("DM_INFO",dpo_di_exists)=0)
    RETURN(0)
   ENDIF
   IF (dpo_di_exists=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("DM_INFO table does not exist under owner ",dpo_sbr_owner)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   SET inhouse_misc->inhouse_domain = - (1)
   IF (dm2_set_inhouse_domain(null)=0)
    RETURN(0)
   ELSEIF ((inhouse_misc->inhouse_domain=0))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Domain is not considered an inhouse domain, ending process."
    SET dm_err->eproc = concat("Determining if inhouse process should continue for owner ",
     dpo_sbr_owner,".")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Getting environment id from DM_INFO/DM_ENVIRONMENT"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    b.environment_id
    FROM dm_info a,
    ...
    DETAIL
     dm2_rr_misc->env_id = b.environment_id, dpo_full_con_str = b.v500_connect_string
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ELSEIF (curqual=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Environment id not found.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (((findstring("/",dpo_full_con_str)=0) OR (findstring("@",dpo_full_con_str)=0
    AND trim(currdblink,3)="")) )
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("V500 connect information invalid on dm_environment.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   SET drb_dbname = validate(currdbname,"TEST")
   IF (trim(drb_cnct_str,3)="")
    SET dpo_start_pos = (findstring("/",dpo_full_con_str)+ 1)
    SET dpo_end_pos = findstring("@",dpo_full_con_str,dpo_start_pos)
    IF (dpo_end_pos=0)
     SET dpo_end_pos = textlen(dpo_full_con_str)
    ELSE
     SET dpo_end_pos = (dpo_end_pos - 1)
    ENDIF
    SET dpo_pswd = substring(dpo_start_pos,((dpo_end_pos - dpo_start_pos)+ 1),dpo_full_con_str)
    IF (dpo_sbr_owner="V500")
     SET drb_v500_pwd = dpo_pswd
    ENDIF
    SET drb_cnct_str = currdblink
    IF (trim(drb_cnct_str,3)="")
     SET dpo_start_pos = (findstring("@",dpo_full_con_str)+ 1)
     SET dpo_end_pos = textlen(dpo_full_con_str)
     SET drb_cnct_str = substring(dpo_start_pos,((dpo_end_pos - dpo_start_pos)+ 1),dpo_full_con_str)
    ENDIF
   ENDIF
   SET dm_err->eproc =
   "Auto-success DM_PROJECT_STATUS_ENV rows for current environment and not current owner."
   CALL disp_msg(" ",dm_err->logfile,0)
   UPDATE  FROM dm_project_status_env s
    SET s.dm_status = dm2_rr_done, s.dm_status_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3)
      ), s.dm_status_msg =
     "This readme is not to be executed under current user.  Automatically marked as successful."
    WHERE (s.environment_id=dm2_rr_misc->env_id)
     AND s.proj_type=dm2_rr_readme
     AND cnvtint(s.proj_name) > 0
     AND s.proj_name BETWEEN dm2_rr_misc->low_proj_name AND dm2_rr_misc->high_proj_name
    ...
    (SELECT
     "X"
     FROM dm_readme dr
     WHERE dr.owner=dpo_sbr_owner
      AND dr.readme_id=cnvtint(s.proj_name)
      AND dr.instance=s.source_set_instance)))
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
...
#exit_program
 IF ((inhouse_misc->inhouse_domain=1))
  IF (currdbuser != "V500")
   ...
   IF (drb_v500_pwd != "DM2NOTSET")
    SET drb_cnct_str = build("v500/",drb_v500_pwd,"@",drb_cnct_str)
    CALL parser(concat("define oraclesystem '",drb_cnct_str,"' go"),1)
   ELSE
    CALL echo("*")
    CALL echo("**************************************************************************")
 ...
 ELSE
  IF ((dm2_rdbms_version->level1 >= 11)
   AND drb_mod_act_set_ind=1)
   CALL dmai_set_mod_act(drb_original_module,drb_original_action)
  ENDIF
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "ENDING DM2_README_BATCH"
 ENDIF
 CALL final_disp_msg("dm2_readme_batch")
END GO
