CREATE PROGRAM dm2_mig_clean_dbobjects:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 IF ( NOT (validate(dmcr_cleanup->cnt)))
  FREE RECORD dmcr_cleanup
  RECORD dmcr_cleanup(
    1 confirm_ind = i2
    1 cnt = i4
    1 mode = vc
  ...
  SET dmcr_cleanup->mode = "DM2NOTSET"
  SET dmcr_cleanup->user = "DM2NOTSET"
 ENDIF
 DECLARE dmcr_display_cleanup_report(null) = i2
 DECLARE dmcr_confirm_cleanup(null) = i2
 DECLARE dmcr_process_cleanup(null) = i2
 DECLARE dmcr_clean_ccldef(dcc_obj_in=vc) = i2
 SUBROUTINE dmcr_display_cleanup_report(null)
   SET dm_err->eproc = "Displaying report to CCL Displayer from record structure."
   SELECT INTO mine
    owner = substring(1,30,dmcr_cleanup->qual[d.seq].owner), object_name = substring(1,30,
     dmcr_cleanup->qual[d.seq].object_name), object_type = substring(1,25,dmcr_cleanup->qual[d.seq].
     object_type),
    table_name = substring(1,30,dmcr_cleanup->qual[d.seq].table_name), sort_order = dmcr_cleanup->
    qual[d.seq].sort_order
    FROM (dummyt d  WITH seq = value(dmcr_cleanup->cnt))
    ORDER BY sort_order, owner, table_name,
     object_name
    HEAD REPORT
     col 0, "Database Migration Object Cleanup Report", row + 2,
     col 0, "The following database migration specific objects will be removed.", row + 1,
     col 0, "Please review before continuing.", row + 2
    HEAD PAGE
     col 1, "OBJECT_TYPE", col 26,
     "OWNER", col 44, "OBJECT_NAME",
     col 75, "TABLE_NAME", row + 1,
     col 0,
    ...
    DETAIL
     IF (dmcr_cleanup->cnt)
      ...
      col 25, "|", owner,
      col 43, "|", object_name
      IF (object_type IN ("SUPPLEMENTAL LOG GROUP", "COLUMN", "TRIGGER", "INDEX"))
       col 74, "|", table_name
      ELSE
       col 74, "|**"
      ENDIF
     ...
     ENDIF
    FOOT REPORT
     IF ( NOT (dmcr_cleanup->cnt))
      col 0, "There are no objects to be removed."
     ENDIF
     row + 3, col 0,
     CALL print(fillstring(126,"*")),
     CALL center(" END OF REPORT ",1,126)
    WITH nocounter, format = variable, maxcol = 1000
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmcr_confirm_cleanup(null)
   DECLARE dcc_menu_cont = i2 WITH protect, noconstant(1)
   WHILE (dcc_menu_cont)
     SET width = 132
     SET message = window
     CALL clear(1,1)
     CALL box(1,1,24,131)
     CALL text(2,2,"Please confirm if the objects in the Migration Cleanup Report can be removed")
     CALL text(4,2,"(R)emove objects and Continue")
     CALL text(5,2,"(V)iew Cleanup Report")
     CALL text(6,2,"(Q)uit")
     CALL text(8,2,"Selection:")
     CALL accept(8,14,"A;CU"," "
      WHERE curaccept IN ("R", "V", "Q"))
     CASE (curaccept)
      OF "R":
       SET dmcr_cleanup->confirm_ind = 1
       SET dcc_menu_cont = 0
      OF "V":
       IF (dmcr_display_cleanup_report(null)=0)
        SET message = nowindow
        CALL clear(1,1)
        RETURN(0)
       ENDIF
      OF "Q":
       ...
       CALL clear(1,1)
       SET dm_err->err_ind = 1
       SET dm_err->emsg = "User Quit from DB Object Cleanup prompt."
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
     ENDCASE
   ENDWHILE
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmcr_process_cleanup(null)
   DECLARE dpc_iter = i4 WITH protect, noconstant(0)
   DECLARE dpc_full_object_name = vc WITH protect, noconstant("")
   DECLARE dpc_purge_suffix = vc WITH protect, noconstant(" ")
   DECLARE dpc_push_cmd = vc WITH protect, noconstant("")
   DECLARE dpc_full_table_name = vc WITH protect, noconstant(" ")
   DECLARE dpc_cmd_rscbsy_retry_cnt = i4 WITH protect, noconstant(0)
   DECLARE dpc_cmd_rscbsy_error = i2 WITH protect, noconstant(0)
   IF (dm2_get_rdbms_version(null)=0)
    RETURN(0)
   ENDIF
   IF ((dm2_rdbms_version->level1 > 9))
    SET dpc_purge_suffix = " PURGE "
   ENDIF
   FOR (dpc_iter = 1 TO dmcr_cleanup->cnt)
     SET dpc_full_object_name = build(dmcr_cleanup->qual[dpc_iter].owner,".",dmcr_cleanup->qual[
      dpc_iter].object_name)
     SET dpc_full_table_name = build(dmcr_cleanup->qual[dpc_iter].owner,".",dmcr_cleanup->qual[
      dpc_iter].table_name)
     CASE (dmcr_cleanup->qual[dpc_iter].object_type)
      OF "DBLINK":
       SET dpc_push_cmd = concat("RDB DROP PUBLIC DATABASE LINK ",dmcr_cleanup->qual[dpc_iter].
        object_name," GO")
      OF "TABLE":
       SET dpc_push_cmd = concat("RDB DROP TABLE ",dpc_full_object_name,dpc_purge_suffix," GO")
      OF "INDEX":
       SET dpc_push_cmd = concat("RDB DROP INDEX ",dpc_full_object_name," GO")
      OF "TRIGGER":
       SET dpc_push_cmd = concat("RDB DROP TRIGGER ",dpc_full_object_name," GO")
      OF "SEQUENCE":
       SET dpc_push_cmd = concat("RDB DROP SEQUENCE ",dpc_full_object_name," GO")
      OF "SYNONYM":
       SET dpc_push_cmd = concat("RDB DROP PUBLIC SYNONYM ",dmcr_cleanup->qual[dpc_iter].object_name,
        " GO")
      OF "VIEW":
       SET dpc_push_cmd = concat("RDB DROP VIEW ",dpc_full_object_name," GO")
      OF "SUPPLEMENTAL LOG GROUP":
       SET dpc_push_cmd = concat("RDB ALTER TABLE ",build(dmcr_cleanup->qual[dpc_iter].owner,".",
         dmcr_cleanup->qual[dpc_iter].table_name)," DROP SUPPLEMENTAL LOG GROUP ",trim(dmcr_cleanup->
         qual[dpc_iter].object_name)," GO")
      OF "COLUMN":
       SET dpc_push_cmd = concat("RDB ALTER TABLE ",dpc_full_table_name," DROP COLUMN ",dmcr_cleanup
        ->qual[dpc_iter].object_name," GO")
      OF "SCHEDULER JOB":
       SET dpc_push_cmd = concat(" rdb asis( ^ begin dbms_scheduler.drop_job('",dpc_full_object_name,
        "'); end;^) GO")
     ENDCASE
     IF ((dmcr_cleanup->qual[dpc_iter].object_type IN ("TABLE", "SYNONYM", "VIEW")))
      IF (dmcr_clean_ccldef(dmcr_cleanup->qual[dpc_iter].object_name)=0)
       RETURN(0)
      ENDIF
     ENDIF
     SET dm_err->eproc = "Execute drop command"
     IF ((dmcr_cleanup->qual[dpc_iter].object_type="SUPPLEMENTAL LOG GROUP"))
      SET dpc_cmd_rscbsy_retry_cnt = 0
      SET dpc_cmd_rscbsy_error = 1
      WHILE (dpc_cmd_rscbsy_error=1
       AND dpc_cmd_rscbsy_retry_cnt < cnvtint(dm2_db_options->resource_busy_maxretry))
        SET dm_err->err_ind = 0
        SET dpc_cmd_rscbsy_error = 0
        EXECUTE dm2_exec_child_cmd "CMD", dpc_push_cmd
        CALL check_error(dm_err->eproc)
        IF ((dm_err->err_ind > 0))
         IF (((findstring("ORA-00054",dm_err->emsg) > 0) OR (((findstring("ORA-14450",dm_err->emsg)
          > 0) OR (((findstring("ORA-14452",dm_err->emsg) > 0) OR (findstring("ORA-14411",dm_err->
          emsg) > 0)) )) )) )
          SET dpc_cmd_rscbsy_error = 1
         ELSE
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         ...
         ENDIF
        ENDIF
        IF (dpc_cmd_rscbsy_error=1)
         CALL pause(3)
         SET dpc_cmd_rscbsy_retry_cnt = (dpc_cmd_rscbsy_retry_cnt+ 1)
        ENDIF
      ENDWHILE
      IF ((dm_err->err_ind > 0))
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDFOR
   IF ((dm_err->debug_flag > 0))
    CALL echorecord(dmcr_cleanup)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmcr_clean_ccldef(dcc_obj_in)
   DECLARE dcc_cmd = vc WITH protect, noconstant("")
   IF (checkdic(cnvtupper(dcc_obj_in),"T",0)=2)
    SET dm_err->eproc = "Execute drop command"
    SET dcc_cmd = concat("drop table ",dcc_obj_in," go")
    IF ((dm_err->debug_flag > 0))
     CALL echo(dcc_cmd)
    ENDIF
    EXECUTE dm2_exec_child_cmd "CMD", dcc_cmd
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD dmcd_sl_table
 RECORD dmcd_sl_table(
   1 cnt = i4
   1 qual[*]
     2 owner = vc
     2 table_name = vc
 )
 SET dmcd_sl_table->cnt = 0
 DECLARE dmcd_idx = i4 WITH protect, noconstant(0)
 DECLARE dmcd_sl_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmcd_ind_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmcd_trg_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmcd_dep_cnt = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_clean_dbob",".log","dm2_mig_clean_dbobjects LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_clean_dbobjects"
 CALL disp_msg("",dm_err->logfile,0)
 IF ((dmcr_cleanup->user="DM2NOTSET")
  AND (dmcr_cleanup->mode="DM2NOTSET"))
  IF (dmr_validate_src_and_tgt(0,1)=0)
   GO TO exit_script
  ENDIF
  SET width = 132
  SET message = window
  CALL clear(1,1)
  CALL box(1,1,24,131)
  CALL text(2,2,concat("Database Migration Cleanup is about to be performed on ",dmr_mig_data->
    tgt_db_name,"."))
  CALL text(4,2,
   "Completing this step will require any subsequent migrations for this database to be started from the beginning."
   )
  CALL text(6,2,concat("Please confirm you are connected to the correct database: ",dmr_mig_data->
    tgt_db_name))
  CALL text(10,2,"(C)ontinue, (Q)uit:")
  CALL accept(10,22,"A;CU"," "
   WHERE curaccept IN ("C", "Q"))
  IF (curaccept="Q")
   GO TO exit_script
  ENDIF
 ...
 IF ((dm_err->err_ind > 0))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Loading tables that have Supplemental log group on dm2_mig_seq_id."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_log_group_columns dlgc
  WHERE dlgc.column_name="DM2_MIG_SEQ_ID"
  ORDER BY dlgc.owner, dlgc.table_name
  HEAD REPORT
   dmcd_sl_table->cnt = 0, stat = alterlist(dmcd_sl_table->qual,0)
  HEAD dlgc.owner
   row + 0
  HEAD dlgc.table_name
   dmcd_sl_table->cnt = (dmcd_sl_table->cnt+ 1)
   IF (mod(dmcd_sl_table->cnt,500)=1)
    stat = alterlist(dmcd_sl_table->qual,(dmcd_sl_table->cnt+ 499))
   ENDIF
   dmcd_sl_table->qual[dmcd_sl_table->cnt].owner = dlgc.owner, dmcd_sl_table->qual[dmcd_sl_table->cnt
   ].table_name = dlgc.table_name
  FOOT REPORT
   stat = alterlist(dmcd_sl_table->qual,dmcd_sl_table->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 IF ((dm_err->debug_flag > 5))
  CALL echorecord(dmcd_sl_table)
 ENDIF
 SET dm_err->eproc = "Cleaning up database objects built for migration use only."
 CALL disp_msg("",dm_err->logfile,0)
 IF ((dmcr_cleanup->user="DM2NOTSET"))
  SET dm_err->eproc = "Determining if table exists from user_tables."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM user_tables ut
   WHERE ut.table_name="GGS_CHECKPOINT"
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF (curqual)
   SET dmcr_cleanup->cnt = 1
   SET stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->cnt)
   SET dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = "V500"
   SET dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = "GGS_CHECKPOINT"
   SET dmcr_cleanup->qual[dmcr_cleanup->cnt].object_type = "TABLE"
   SET dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order = 1
  ENDIF
  SET dm_err->eproc = "Determining if migration database links exist."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dba_db_links d
   WHERE d.db_link=patstring("DM2MIGINST*")
   DETAIL
    dmcr_cleanup->cnt = (dmcr_cleanup->cnt+ 1), stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->cnt
     ), dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = d.owner
    IF (findstring(".",d.db_link,1,1) > 0)
     dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = substring(1,(findstring(".",d.db_link,1,1)
       - 1),d.db_link)
    ELSE
     dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = d.db_link
    ENDIF
    dmcr_cleanup->qual[dmcr_cleanup->cnt].object_type = "DBLINK", dmcr_cleanup->qual[dmcr_cleanup->
    cnt].sort_order = 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Determining list of migration indexes from dba_indexes."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT
  IF ((dmcr_cleanup->user="DM2NOTSET"))
   FROM dba_indexes di,
    dba_ind_columns ic
   WHERE di.index_type="NORMAL"
    AND di.owner=ic.index_owner
    AND di.index_name=ic.index_name
    AND ic.column_position=1
    AND ic.column_name="DM2_MIG_SEQ_ID"
  ELSE
   FROM dba_indexes di,
    dba_ind_columns ic
   WHERE di.index_type="NORMAL"
    AND (di.owner=dmcr_cleanup->user)
    AND di.owner=ic.index_owner
    AND di.index_name=ic.index_name
    AND ic.column_position=1
    AND ic.column_name="DM2_MIG_SEQ_ID"
  ...
  DETAIL
   IF (locateval(dmcd_idx,1,dmcd_sl_table->cnt,di.table_owner,dmcd_sl_table->qual[dmcd_idx].owner,
    di.table_name,dmcd_sl_table->qual[dmcd_idx].table_name)=0)
    dmcr_cleanup->cnt = (dmcr_cleanup->cnt+ 1), stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->cnt
     ), dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = di.owner,
    dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = di.index_name, dmcr_cleanup->qual[
    dmcr_cleanup->cnt].table_name = di.table_name, dmcr_cleanup->qual[dmcr_cleanup->cnt].object_type
     = "INDEX",
    dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order = 2
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining list of migration triggers from dba_triggers."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT
  IF ((dmcr_cleanup->user="DM2NOTSET"))
   FROM dba_triggers dt
   WHERE dt.trigger_name="TRG_DM2_MIG*"
  ELSE
   FROM dba_triggers dt
   WHERE dt.trigger_name="TRG_DM2_MIG*"
    AND (dt.owner=dmcr_cleanup->user)
  ENDIF
  INTO "nl:"
  ORDER BY dt.table_owner, dt.table_name, dt.trigger_name
  DETAIL
   IF (locateval(dmcd_idx,1,dmcd_sl_table->cnt,dt.table_owner,dmcd_sl_table->qual[dmcd_idx].owner,
    dt.table_name,dmcd_sl_table->qual[dmcd_idx].table_name)=0)
    dmcr_cleanup->cnt = (dmcr_cleanup->cnt+ 1), stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->cnt
     ), dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = dt.owner,
    dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = dt.trigger_name, dmcr_cleanup->qual[
    dmcr_cleanup->cnt].table_name = dt.table_name, dmcr_cleanup->qual[dmcr_cleanup->cnt].object_type
     = "TRIGGER",
    dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order = 3
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dmcr_cleanup->user="DM2NOTSET"))
  SET dm_err->eproc = "Determining list of migration views from user_views."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM user_views uv
   WHERE ((uv.view_name="DM2MIG*") OR (uv.view_name="DM2VDAT*"))
   DETAIL
    IF (((uv.view_name="DM2VDAT*1"
     AND uv.text="*AS UNIQUE_ID*") OR (((uv.view_name="DM2VDATS*"
     AND uv.text="*SRC*") OR (((uv.view_name="DM2VDATT*"
     AND uv.text="*TGT*") OR (((uv.view_name="DM2VDATU*"
     AND uv.text="*SRC*UNION ALL*") OR (uv.view_name="DM2MIG*")) )) )) )) )
     dmcr_cleanup->cnt = (dmcr_cleanup->cnt+ 1), stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->
      cnt), dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = "V500",
     dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = uv.view_name, dmcr_cleanup->qual[
     dmcr_cleanup->cnt].object_type = "VIEW", dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order = 4
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dmcr_cleanup->user != "DM2NOTSET")
  AND (dmcr_cleanup->mode="DOWNTIME"))
  SET dm_err->eproc = "Determining list of migration columns from dba_tab_columns."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dba_tab_columns dtc
   WHERE (dtc.owner=dmcr_cleanup->user)
    AND dtc.column_name="DM2_MIG_SEQ_ID"
   DETAIL
    dmcr_cleanup->cnt = (dmcr_cleanup->cnt+ 1), stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->cnt
     ), dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = dtc.owner,
    dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = dtc.column_name, dmcr_cleanup->qual[
    dmcr_cleanup->cnt].table_name = dtc.table_name, dmcr_cleanup->qual[dmcr_cleanup->cnt].object_type
     = "COLUMN",
    dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order = 5
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dmcr_cleanup->user="DM2NOTSET"))
  SET dm_err->eproc = "Determining list of scheduler jobs."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dba_scheduler_jobs dsj
   WHERE dsj.job_name IN ("DM2MIGJ_INS_DTT", "DM2MIGJ_DEL_DTT")
    AND dsj.owner="V500"
   DETAIL
    dmcr_cleanup->cnt = (dmcr_cleanup->cnt+ 1), stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->cnt
     ), dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = dsj.owner,
    dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = dsj.job_name, dmcr_cleanup->qual[dmcr_cleanup
    ->cnt].table_name = dsj.job_type, dmcr_cleanup->qual[dmcr_cleanup->cnt].object_type =
    "SCHEDULER JOB",
    dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order = 6
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (dmcr_display_cleanup_report(null)=0)
  GO TO exit_script
 ENDIF
 IF (dmcr_confirm_cleanup(null)=0)
  GO TO exit_script
 ENDIF
 IF (dmcr_cleanup->confirm_ind)
  IF (dmcr_process_cleanup(null)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((dmcr_cleanup->user="DM2NOTSET"))
  SET dm_err->eproc = "Determine if supplemental log groups exist on DM2_MIG_SEQ_ID column."
  CALL disp_msg("",dm_err->logfile,0)
  SET dmcd_sl_cnt = 0
  SELECT INTO "nl:"
   FROM dba_log_group_columns dlgc
   ...
   DETAIL
    dmcd_sl_cnt = 1
   WITH nocounter, maxqual(dlgc,1)
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = trim(concat(evaluate(dmcd_sl_cnt,0,"No ",""),
    "SUPP LOG GROUPS exist on DM2_MIG_SEQ_ID column."),3)
  CALL disp_msg("",dm_err->logfile,0)
  SET dm_err->eproc = "Determine if indexes exist on DM2_MIG_SEQ_ID column."
  CALL disp_msg("",dm_err->logfile,0)
  SET dmcd_ind_cnt = 0
  SELECT INTO "nl:"
   FROM dba_indexes di,
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = trim(concat(evaluate(dmcd_ind_cnt,0,"No ",""),
    "INDEXES exist on DM2_MIG_SEQ_ID column."),3)
  CALL disp_msg("",dm_err->logfile,0)
  SET dm_err->eproc = "Determine if triggers exist with TRG_DM2_MIG* trigger name prefix."
  CALL disp_msg("",dm_err->logfile,0)
  SET dmcd_trg_cnt = 0
  SELECT INTO "nl:"
   FROM dba_triggers dt
   ...
   DETAIL
    dmcd_trg_cnt = 1
   WITH nocounter, maxqual(dt,1)
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = trim(concat(evaluate(dmcd_trg_cnt,0,"No ",""),
    "TRIGGERS exist with TRG_DM2_MIG* prefix."),3)
  CALL disp_msg("",dm_err->logfile,0)
  SET dm_err->eproc = "Determining if objects exist that depend on sequence DM2_MIG_SEQ."
  CALL disp_msg("",dm_err->logfile,0)
  SET dmcd_dep_cnt = 0
  SELECT INTO "nl:"
   FROM dba_dependencies dd
   WHERE dd.referenced_name="DM2_MIG_SEQ"
    AND dd.type != "SYNONYM"
    AND dd.referenced_type="SEQUENCE"
   DETAIL
    dmcd_dep_cnt = 1
   WITH nocounter, maxqual(dd,1)
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = trim(concat(evaluate(dmcd_dep_cnt,0,"No ",""),
    "OBJECTS exist that depend on sequence DM2_MIG_SEQ."),3)
  CALL disp_msg("",dm_err->logfile,0)
  IF (dmcd_sl_cnt=0
   AND dmcd_ind_cnt=0
   AND dmcd_trg_cnt=0
   AND dmcd_dep_cnt=0)
   SET dmcr_cleanup->cnt = 0
   SET stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->cnt)
   SET dm_err->eproc = "Determining list of migration sequences from all_sequences."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM all_sequences asq
    WHERE asq.sequence_name="DM2_MIG*"
    DETAIL
     dmcr_cleanup->cnt = (dmcr_cleanup->cnt+ 1), stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->
      cnt), dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = asq.sequence_owner,
     dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = asq.sequence_name, dmcr_cleanup->qual[
     dmcr_cleanup->cnt].object_type = "SEQUENCE", dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order =
     1
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = "Determining list of migration synonyms from all_synonyms."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM all_synonyms asy
    WHERE asy.synonym_name="DM2_MIG*"
    DETAIL
     dmcr_cleanup->cnt = (dmcr_cleanup->cnt+ 1), stat = alterlist(dmcr_cleanup->qual,dmcr_cleanup->
      cnt), dmcr_cleanup->qual[dmcr_cleanup->cnt].owner = asy.owner,
     dmcr_cleanup->qual[dmcr_cleanup->cnt].object_name = asy.synonym_name, dmcr_cleanup->qual[
     dmcr_cleanup->cnt].object_type = "SYNONYM", dmcr_cleanup->qual[dmcr_cleanup->cnt].sort_order = 2
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 GO TO exit_script
#exit_script
 SET dm_err->eproc = "Ending dm2_mig_clean_dbobjects."
 CALL final_disp_msg("dm2_mig_clean_dbob")
END GO
