CREATE PROGRAM cp_del_component_json
 PROMPT
 ...
 DECLARE compid = f8 WITH constant( $COMPID), protect
 ...
 SUBROUTINE deleterows(null)
   ...
   SET comp_del_request->qual[1].cp_node_id = nodeid
   SET comp_del_request->qual[1].cp_component_id = compid
   SET comp_del_request->qual[1].comp_type_cd = comptypecd
   CALL echorecord(comp_del_request)
   EXECUTE cp_del_component
   CALL echorecord(comp_del_reply)
   IF (size(comp_del_reply->component_list,5) > 0)
    SET stat = alterlist(mpagereply->component_list,1)
    SET mpagereply->component_list[1].comp_type_cd = comp_del_reply->component_list[1].comp_type_cd
    SET mpagereply->component_list[1].cp_component_id = comp_del_reply->component_list[1].
    cp_component_id
    SET mpagereply->component_list[1].cp_node_id = comp_del_reply->component_list[1].cp_node_id
