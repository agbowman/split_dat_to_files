CREATE PROGRAM bed_aud_rad_word_templates
 ...
 DECLARE orgs = vc
 DECLARE radiology_activity_cd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",106,
   "RADIOLOGY"))
  ...
  SELECT INTO "nl:"
   ...
   PLAN (wt
    WHERE wt.template_type_cd IN (template_type_cd, letter_type_cd)
     AND wt.activity_type_cd=radiology_activity_cd
     AND wt.active_ind=1)
    JOIN (fer
 ...
 SELECT INTO "nl:"
  ...
  HEAD wt.short_desc
   ...
   templates->qual[cnt].short_desc = wt.short_desc, templates->qual[cnt].template_id = wt.template_id
...
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("radnet_word_templates_audit.csv")
 ENDIF
