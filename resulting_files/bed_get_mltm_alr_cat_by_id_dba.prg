CREATE PROGRAM bed_get_mltm_alr_cat_by_id:dba
 ...
 DECLARE cnt = i2
 SELECT INTO "nl:"
  FROM (dummyt d1  WITH seq = value(size(request->allergy_category_ids,5))),
   mltm_alr_category mac
  PLAN (d1)
   JOIN (mac
   WHERE (mac.alr_category_id=request->allergy_category_ids[d1.seq].alr_category_id))
