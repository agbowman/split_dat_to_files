CREATE PROGRAM dm2_mig_create_dtt_rep:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmcdr_sql_string = vc WITH protect, noconstant("")
 DECLARE dmcdr_job_exist = i2 WITH protect, noconstant(0)
 DECLARE dmcdr_src_max_ind = i2 WITH protect, noconstant(0)
 DECLARE dmcdr_tgt_max_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_mig_create_dtt_rep",".log","dm2_mig_create_dtt_rep")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_create_dtt_rep"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Get target connect information"
 ...
 IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Check if v500.dm_txn_tracking table exists in Target"
 SELECT INTO "nl:"
  FROM dba_tables dt
  WHERE dt.table_name="DM_TXN_TRACKING"
   AND dt.owner="V500"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ELSEIF (curqual=0)
  SET dm_err->eproc = "Remove EXCLUDE_DM_TXN_TRACKING dm_info row if it already exists"
  DELETE  FROM dm_info d
   WHERE info_domain="DM2_MIG_DTT_EXCL"
  ...
  ELSE
   COMMIT
  ENDIF
  SET dm_err->eproc = "Insert EXCLUDE_DM_TXN_TRACKING dm_info row"
  INSERT  FROM dm_info d
   SET d.info_domain = "DM2_MIG_DTT_EXCL", d.info_name = "EXCLUDE_DM_TXN_TRACKING", d.info_number = 1
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ELSE
   COMMIT
  ENDIF
  SET dm_err->eproc = "Table V500.DM_TXN_TRACKING does not exist in Target"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Compile procedure dm2mig_ins_dtt."
 IF ((validate(skip_rdb_reader,- (1))=- (1)))
  SET rdb_reader_ind = 1
 ENDIF
 SET dris_skip_test_file = 1
 EXECUTE dm_readme_include_sql "cer_install:dm2mig_ins_dtt.sql"
 EXECUTE dm_readme_include_sql_chk "dm2mig_ins_dtt", "procedure"
 IF ((dm_sql_reply->status="F"))
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Compile procedure dm2mig_del_dtt."
 IF ((validate(skip_rdb_reader,- (1))=- (1)))
  SET rdb_reader_ind = 1
 ENDIF
 SET dris_skip_test_file = 1
 EXECUTE dm_readme_include_sql "cer_install:dm2mig_del_dtt.sql"
 EXECUTE dm_readme_include_sql_chk "dm2mig_del_dtt", "procedure"
 IF ((dm_sql_reply->status="F"))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = dm_sql_reply->msg
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Check if INITIAL_START_ROW_SCN row exists"
 SELECT INTO "nl:"
  FROM dm_info
  WHERE info_domain="DM2_MIG_INS_DTT"
   AND info_name="INITIAL_START_ROW_SCN"
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSEIF (curqual=0)
  SET dm_err->eproc = "Get max ROW_SCN from Source"
  SELECT INTO "nl:"
   max = max(dt.row_scn)
   FROM dm_txn_tracking@ref_data_link dt
   DETAIL
    IF (max > 0)
     dmcdr_src_max_ind = 1
    ENDIF
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Get max ROW_SCN from Target"
  SELECT INTO "nl:"
   max = max(dt.row_scn)
   FROM dm_txn_tracking dt
   DETAIL
    IF (max > 0)
     dmcdr_tgt_max_ind = 1
    ENDIF
  ...
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  IF (dmcdr_src_max_ind=1
   AND dmcdr_tgt_max_ind=0)
   SET dm_err->emsg = "DM_TXN_TRACKING is empty in Target, but not in Source."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Insert INITIAL_START_ROW_SCN row into dm_info"
  INSERT  FROM dm_info
   SET info_domain = "DM2_MIG_INS_DTT", info_name = "INITIAL_START_ROW_SCN", info_number =
    (SELECT
     max(dt.row_scn)
     FROM dm_txn_tracking dt)
   WITH nocounter
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Check for DM2MIGJ_INS_DTT job"
 SELECT INTO "nl:"
  x = count(*)
  FROM dba_scheduler_jobs dj
  WHERE dj.job_name="DM2MIGJ_INS_DTT"
   AND dj.owner="V500"
  DETAIL
   dmcdr_job_exist = x
  WITH nocounter
 ...
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ENDIF
 IF (dmcdr_job_exist > 0)
  SET dmcdr_job_exist = 0
  SET dmcdr_sql_string = concat(" rdb asis( ^ "," begin ",
   " dbms_scheduler.drop_job('V500.DM2MIGJ_INS_DTT');"," end;^) go")
  IF (dm2_push_cmd(dmcdr_sql_string,1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Create job DM2MIGJ_INS_DTT"
 SET dmcdr_sql_string = concat(" rdb asis( ^  "," begin  ","   dbms_scheduler.create_job( ",
  "   job_name =>'DM2MIGJ_INS_DTT', ","   job_type => 'STORED_PROCEDURE', ",
  "   job_action => 'V500.DM2MIG_INS_DTT', ","   repeat_interval => 'FREQ=MINUTELY;INTERVAL=1;', ",
  "   start_date => NULL , ","   auto_drop => FALSE , ","   enabled => TRUE );   ",
  " exception  ","   when others then  ","   raise; "," end; "," ^ ) go")
 IF (dm2_push_cmd(dmcdr_sql_string,1)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Check for DM2MIGJ_DEL_DTT job"
 SET dmcdr_job_exist = 0
 SELECT INTO "nl:"
  x = count(*)
  FROM dba_scheduler_jobs dj
  WHERE dj.job_name="DM2MIGJ_DEL_DTT"
   AND dj.owner="V500"
  DETAIL
   dmcdr_job_exist = x
  WITH nocounter
 ...
 IF (dmcdr_job_exist > 0)
  SET dmcdr_job_exist = 0
  SET dmcdr_sql_string = concat(" rdb asis( ^ "," begin ",
   " dbms_scheduler.drop_job('V500.DM2MIGJ_DEL_DTT');"," end;^) go")
  IF (dm2_push_cmd(dmcdr_sql_string,1)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Create job DM2MIGJ_DEL_DTT"
 SET dmcdr_sql_string = concat(" rdb asis( ^  "," begin  ","   dbms_scheduler.create_job( ",
  "   job_name =>'DM2MIGJ_DEL_DTT', ","   job_type => 'STORED_PROCEDURE', ",
  "   job_action => 'V500.DM2MIG_DEL_DTT', ","   repeat_interval => 'FREQ=DAILY;INTERVAL=1;', ",
  "   start_date => NULL , ","   auto_drop => FALSE , ","   enabled => TRUE );   ",
  " exception  ","   when others then  ","   raise; "," end; "," ^ ) go")
 IF (dm2_push_cmd(dmcdr_sql_string,1)=0)
  GO TO exit_script
 ENDIF
#exit_script
 CALL final_disp_msg("dm2_mig_create_dtt_rep")
END GO
