CREATE PROGRAM dm2_wrp_table_relationships:dba
 ...
 SUBROUTINE dac_load_cload(lc_pkg_in)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_wrp_table_relationships",".log","dm2_wrp_table_relationships")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_wrp_table_relationships"
 CALL disp_msg(" ",dm_err->logfile,0)
 DECLARE dwtr_batch = i4 WITH protect, noconstant(0)
 DECLARE dwtr_tbl = vc WITH protect, noconstant("")
 DECLARE dwtr_tmp_tbl = vc WITH protect, noconstant("")
 DECLARE dwtr_tbl_exists = i2 WITH protect, noconstant(0)
 DECLARE dwtr_tmp_tbl_exists = i2 WITH protect, noconstant(0)
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: Starting script DM2_WRP_TABLE_RELATIONSHIPS..."
 SET dwtr_tbl = "DM_TABLE_RELATIONSHIPS"
 SET dwtr_tmp_tbl = "DM_TABLE_RELATIONSHIPS_GTTP"
 SET dm_err->eproc = concat("Check if table ",dwtr_tbl," exists.")
 IF (dm2_table_and_ccldef_exists(dwtr_tbl,dwtr_tbl_exists)=0)
  GO TO exit_script
 ENDIF
 IF (dwtr_tbl_exists=0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed: Table ",dwtr_tbl," does not exist.")
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Check if table ",dwtr_tmp_tbl," exists.")
 IF (dm2_table_and_ccldef_exists(dwtr_tmp_tbl,dwtr_tmp_tbl_exists)=0)
  GO TO exit_script
 ENDIF
 IF (dwtr_tmp_tbl_exists=0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed: Table ",dwtr_tmp_tbl," does not exist.")
  GO TO exit_script
 ENDIF
 IF (dwtr_batch=0)
  SET dwtr_batch = 1000
 ENDIF
 EXECUTE dm2_dbimport "cer_install:dm_table_relationships_metadata.csv", "dm2_aload_dtr_gttp",
 dwtr_batch
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to insert into ",dwtr_tmp_tbl," : ",dm_err->emsg)
  GO TO exit_script
 ELSE
  COMMIT
 ENDIF
 ...
 IF ((dm_err->err_ind=1))
  ...
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to load ",dwtr_tbl," table : ",dm_err->emsg)
  GO TO exit_script
 ELSE
  COMMIT
 ENDIF
 SET readme_data->status = "S"
 SET readme_data->message = concat("Success: ",dwtr_tbl," table loaded successfully.")
#exit_script
