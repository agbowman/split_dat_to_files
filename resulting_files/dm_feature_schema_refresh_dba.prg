CREATE PROGRAM dm_feature_schema_refresh:dba
 FREE SET all
 ...
 FOR (cntw = 1 TO feature_table_list->table_count)
   IF ((feature_table_list->table_name[cntw].fill_status != 1)
    ...
    IF ((feature_table_list->nbr_errors > 0))
     ...
     SET feature_table_list->nbr_errors = 0
     SET err_str = concat("Schema compare error before refresh. Table ",feature_table_list->
      table_name[cntw].tname,"has ",cnvtstring(eature_table_list->table_name[cntw].error_flag),
      " differences")
     SELECT INTO value(filename5)
      *
      FROM dual
 ...
 FOR (cntw = 1 TO feature_table_list->table_count)
   IF ((feature_table_list->table_name[cntw].fill_status != 1)
    AND (feature_table_list->table_name[cntw].backout_ind != 1)
    AND (feature_table_list->table_name[cntw].error_flag > 0))
    CALL echo("Before calling DM_SCHEMA_COMP")
    EXECUTE dm_schema_comp "A", trim(feature_table_list->table_name[cntw].tname), cnvtdatetime(
     feature_table_list->table_name[cntw].schema_dt_tm)
    IF ((feature_table_list->param_error=1))
     SET feature_table_list->param_error = 0
    ...
    IF ((feature_table_list->nbr_errors > 0))
      ...
      *
      ...
      WITH format = stream, noheading, formfeed = none,
       maxcol = 512, maxrow = 1, append
     ;end select
     CALL update_dm_feature_tbls1(re_dummy)
    ELSE
     CALL update_dm_feature_tbls2(re_dummy)
    ENDIF
   ENDIF
   SET error_flg = 0
   IF ((feature_table_list->table_name[cntw].backout_ind=1))
    CALL update_dm_feature_tbls2(re_dummy)
    SET y = concat('select into "nl:" * from ',feature_table_list->table_name[cntw].tname,
     " with nocounter go ")
    CALL parser(y,1)
    IF (curqual != 0)
     SET err_str = concat("The table ",feature_table_list->table_name[cntw].tname,
      " in the backed out feature ",feature_table_list->table_name[cntw].feature_number,
      " was not dropped")
     SET error_flg = 1
     CALL update_dm_feature_tbls1(re_dummy)
    ENDIF
   ...
   ENDIF
 ENDFOR
 SUBROUTINE update_dm_feature_tbls1(dummy)
