CREATE PROGRAM dm2_mig_utc_schema_sync:dba
 ...
 SUBROUTINE dm2_rename_login_default(sbr_rld_mode)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD dmuss_tblcol_rec
 RECORD dmuss_tblcol_rec(
   1 tcnt = i4
   1 sync_from_src = i2
   1 tbl[*]
     2 tbl_name = vc
 ...
 SET dmuss_tblcol_rec->tcnt = 0
 SET dmuss_tblcol_rec->sync_from_src = 0
 FREE RECORD dmuss_col_cmp
 RECORD dmuss_col_cmp(
   1 tcnt = i4
   1 tbl[*]
     2 tbl_name = vc
     2 utc_col_cnt = i4
     2 orig_col_cnt = i4
     2 excl_utc_dt_cnt = i4
     2 mismatch_ind = i2
     2 ccnt = i4
     2 col[*]
       3 col_name = vc
 ...
 SET dmuss_col_cmp->tcnt = 0
 FREE RECORD dmuss_ddl_ops_log_data
 RECORD dmuss_ddl_ops_log_data(
   1 cnt = i4
   1 qual[*]
     2 op_id = f8
     2 table_name = vc
 ...
 SET dmuss_ddl_ops_log_data->cnt = 0
 FREE RECORD dmuss_td_tables
 RECORD dmuss_td_tables(
   1 cnt = i4
   1 tbl[*]
     2 table_name = vc
     2 table_suffix = vc
 )
 SET dmuss_td_tables->cnt = 0
 FREE RECORD dmuss_fail_tables
 RECORD dmuss_fail_tables(
   1 cnt = i4
   1 qual[*]
     2 table_name = vc
 )
 SET dmuss_fail_tables->cnt = 0
 FREE RECORD dmuss_real_tables
 RECORD dmuss_real_tables(
   1 cnt = i4
   1 qual[*]
     2 table_name = vc
 )
 SET dmuss_real_tables->cnt = 0
 DECLARE dmuss_dbtype = vc WITH protect, noconstant("")
 DECLARE dmuss_locndx = i4 WITH protect, noconstant(0)
 DECLARE dmuss_loop_var = i4 WITH protect, noconstant(0)
 DECLARE dmuss_col_var = i4 WITH protect, noconstant(0)
 DECLARE dmuss_str = vc WITH protect, noconstant("")
 DECLARE dmuss_dbase_name = vc WITH protect, noconstant(" ")
 DECLARE dmuss_tab_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmuss_tab_col_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmuss_db_link = vc WITH protect, noconstant("")
 DECLARE dmuss_standalone = i2 WITH protect, noconstant(0)
 DECLARE dmuss_calling_script = vc WITH protect, noconstant("")
 DECLARE dmuss_retry_ceiling = i4 WITH protect, noconstant(0)
 DECLARE dmuss_rpt_file = vc WITH protect, noconstant("dm2_mig_sync_fail_tables")
 DECLARE dmuss_table = vc WITH protect, noconstant("")
 DECLARE dmuss_lob_storage_bp = vc WITH protect, noconstant("")
 DECLARE dmuss_use_securefile = i2 WITH protect, noconstant(0)
 DECLARE dmuss_dminfo_securefile_ind = i2 WITH protect, noconstant(0)
 DECLARE dmuss_suffix = vc WITH protect, noconstant("")
 DECLARE dmuss_td_idx = i4 WITH protect, noconstant(0)
 DECLARE dmuss_validate_db_link(dvdl_db_link=vc) = i2
 DECLARE dmuss_db_sync_src(null) = i2
 DECLARE dmuss_db_sync_tgt(null) = i2
 DECLARE dmuss_tgt_excl_optype(null) = i2
 DECLARE dmuss_rscbsy_logic(null) = i2
 DECLARE dmuss_add_column(null) = i2
 DECLARE dmuss_chk_current_db(null) = i2
 DECLARE dmuss_load_exclusion(dle_dbname=vc,dle_content=vc) = i2
 IF ((dm2_install_schema->curprog="NONE"))
  SET dmuss_standalone = 1
 ENDIF
 IF (check_logfile("dm2_mig_utc_schema_sync",".log","DM2_MIG_UTC_SCHEMA_SYNC LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning DM2_MIG_UTC_SCHEMA_SYNC"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dmuss_dbtype = trim(cnvtupper( $1),3)
 SET dm_err->eproc = "Validating <db_type> input parameter."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ( NOT (dmuss_dbtype IN ("SOURCE", "TARGET")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat(dmuss_dbtype,"is invalid. Valid values are 'SOURCE','TARGET.'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dmuss_standalone=1)
  SET dm_err->eproc = "Validate script execution."
  SET dm_err->emsg =
  "DM2_MIG_UTC_SCHEMA_SYNC cannot be run standalone!  Execute appropriate wrapper script."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF ((dm2_rdbms_version->level1 > 10))
  IF (dir_get_ddl_gen_retry(dmuss_retry_ceiling)=0)
   GO TO exit_script
  ENDIF
 ELSE
  SET dmuss_retry_ceiling = 0
 ENDIF
 SET dm_err->eproc = "Check for securefile build indicator in source"
 SELECT INTO "nl:"
  FROM dm_info d
  WHERE d.info_domain="DM2_SECUREFILE"
   AND d.info_name="SECUREFILE_BUILD_IND"
  DETAIL
   dmuss_dminfo_securefile_ind = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dmuss_db_link = "MIG_TARGET"
 IF (dmuss_validate_db_link(dmuss_db_link)=0)
  GO TO exit_script
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   stat = alterlist(dmuss_td_tables->tbl,10)
  DETAIL
   dmuss_td_tables->cnt = (dmuss_td_tables->cnt+ 1)
   IF (mod(dmuss_td_tables->cnt,10)=1
    AND (dmuss_td_tables->cnt != 1))
    stat = alterlist(dmuss_td_tables->tbl,(dmuss_td_tables->cnt+ 9))
   ENDIF
   dmuss_td_tables->tbl[dmuss_td_tables->cnt].table_name = td.table_name, dmuss_td_tables->tbl[
   dmuss_td_tables->cnt].table_suffix = td.table_suffix
  FOOT REPORT
   stat = alterlist(dmuss_td_tables->tbl,dmuss_td_tables->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF (dmuss_dbtype="TARGET")
  SET dm_err->eproc = "Load corresponding real tables associated to existing $R tables into memory."
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dba_tables d
   WHERE d.owner="V500"
    AND d.table_name="*$R"
   DETAIL
    dmuss_td_idx = 0, dmuss_suffix = substring((size(trim(d.table_name)) - 5),4,d.table_name),
    dmuss_td_idx = locateval(dmuss_td_idx,1,dmuss_td_tables->cnt,dmuss_suffix,dmuss_td_tables->tbl[
     dmuss_td_idx].table_suffix)
    IF (dmuss_td_idx > 0)
     dmuss_real_tables->cnt = (dmuss_real_tables->cnt+ 1), stat = alterlist(dmuss_real_tables->qual,
      dmuss_real_tables->cnt), dmuss_real_tables->qual[dmuss_real_tables->cnt].table_name =
     dmuss_td_tables->tbl[dmuss_td_idx].table_name
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_script
  ENDIF
  IF ((dmuss_real_tables->cnt=0))
   SET dm_err->eproc = "No $R tables exist in Source."
   CALL disp_msg(" ",dm_err->logfile,0)
  ELSE
   SET dm_err->eproc = "$R tables exist in Source."
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  IF ((dm_err->debug_flag > 2))
   CALL echorecord(dmuss_real_tables)
  ENDIF
 ENDIF
 ...
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_script
 ENDIF
 IF (dmuss_dminfo_securefile_ind=1)
  IF ((((dm2_rdbms_version->level1 > 11)) OR ((((dm2_rdbms_version->level1=11)
   ...
   IF ((validate(dm2_migutc_bypass_secfile,- (1))=- (1)))
    SET dmuss_use_securefile = 1
    EXECUTE dm2_set_db_options
    IF ((dm_err->err_ind > 0))
     GO TO exit_script
    ENDIF
    SET dmuss_lob_storage_bp = dm2_db_options->lob_storage_bp
   ELSE
    SET dmuss_use_securefile = 0
   ENDIF
  ELSE
   SET dmuss_use_securefile = 0
  ENDIF
 ELSE
  SET dmuss_use_securefile = 0
 ENDIF
 SET dmuss_db_link = "REF_DATA_LINK"
 IF (dmuss_validate_db_link(dmuss_db_link)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_get_dbase_name(dmuss_dbase_name)=0)
  GO TO exit_script
 ENDIF
 IF (dus_load_spec_cols(dmuss_dbase_name)=0)
  GO TO exit_script
 ENDIF
 IF (dm2_fill_sch_except("LOCAL")=0)
  GO TO exit_script
 ENDIF
 IF (dmuss_dbtype="SOURCE")
  IF (dmuss_db_sync_src(null)=0)
   GO TO exit_script
  ENDIF
 ELSE
  IF (dmuss_db_sync_tgt(null)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((validate(dm2_mig_utc_bypass_due_to_rdds,- (1))=- (1)))
  IF (dmuss_chk_current_db(null)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 EXECUTE dm2_set_db_options
 ...
 IF (dmuss_dbtype="SOURCE")
  ...
  ENDIF
 ENDIF
 IF ((dmuss_tblcol_rec->tcnt > 0))
  FOR (dmuss_locndx = 1 TO dmuss_tblcol_rec->tcnt)
   IF ((dmuss_tblcol_rec->tbl[dmuss_locndx].ddl_operation > ""))
    SELECT INTO "nl:"
     FROM user_tables u
     WHERE (u.table_name=dmuss_tblcol_rec->tbl[dmuss_locndx].tbl_name)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    IF (curqual=0)
     SET dm_err->eproc = concat("CREATE TABLE DDL Execution for ",dmuss_tblcol_rec->tbl[dmuss_locndx]
      .tbl_name)
     CALL disp_msg(" ",dm_err->logfile,0)
     SET dmuss_str = dmuss_tblcol_rec->tbl[dmuss_locndx].ddl_operation
     IF (dmuss_rscbsy_logic(null)=0)
      GO TO exit_script
     ELSE
      IF (dmuss_dbtype="TARGET")
       IF (dmuss_load_exclusion(dmuss_dbase_name,dmuss_tblcol_rec->tbl[dmuss_locndx].tbl_name)=0)
        GO TO exit_script
       ENDIF
      ENDIF
     ENDIF
    ENDIF
   ENDIF
   IF ((dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt > 0))
    IF (dmuss_add_column(null)=0)
     GO TO exit_script
    ENDIF
   ENDIF
  ENDFOR
 ...
 IF (dmuss_dbtype="SOURCE")
  ...
  ENDIF
 ELSE
  IF (dmuss_tgt_excl_optype(null)=0)
   GO TO exit_script
  ENDIF
  EXECUTE dm2_mig_utc_compare_tables "TARGET"
  IF ((dm_err->err_ind=1))
   GO TO exit_script
  ENDIF
  IF ((dum_utc_invalid_tables->cnt > 0))
   SET dm_err->eproc = "Check for Tables/Columns Sync failed."
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Found Tables/Columns mismatch. Cannot progress further."
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (dmuss_dbtype="SOURCE")
  IF ((dmuss_fail_tables->cnt > 0))
   IF (get_unique_file(dmuss_rpt_file,".rpt")=0)
    GO TO exit_script
   ENDIF
   SET dmuss_rpt_file = dm_err->unique_fname
   SET dm_err->eproc = "Create table failure report."
   SELECT INTO value(dmuss_rpt_file)
    FROM (dummyt d  WITH d.seq = 1)
    HEAD REPORT
     col 20, "Tables Failed Sync on Source due to Contention", row + 1,
     col 20, "----------------------------------------------", row + 1,
     row + 2, col 2, "Table Name"
    DETAIL
     FOR (dmuss_locndx = 1 TO dmuss_fail_tables->cnt)
       row + 2, col 2, dmuss_fail_tables->qual[dmuss_locndx].table_name
     ENDFOR
    WITH nocounter, maxcol = 250, maxrow = 1,
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_script
   ENDIF
   IF (dm2_disp_file(dmuss_rpt_file," ")=0)
    GO TO exit_script
   ENDIF
   SET dm_err->eproc = "UTC sync Source tables from Target."
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Tables Failed Sync on Source due to contention. ","Review report ",
    dmuss_rpt_file," in CCLUSERDIR for further details.  Restart during minimal activity.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ELSE
   SET dm_err->eproc = "Tables/Columns synced on Source successfully."
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
 ENDIF
 GO TO exit_script
 SUBROUTINE dmuss_validate_db_link(dvdl_db_link)
   SET dm_err->eproc = concat("Querying from ALL_DB_LINKS to see if ",dmuss_db_link," dblink exist.")
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM all_db_links adl
    WHERE parser(build(" adl.db_link = '",dmuss_db_link,".*'"))
    DETAIL
     dmuss_db_link = adl.db_link
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->eproc = concat("Check to see if ",dmuss_db_link," dblink exist.")
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat(dmuss_db_link," dblink does not exist. Cannot progress further.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ELSE
    SET dm_err->eproc = concat("Validating ",dmuss_db_link," database link")
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM (value(build("V$DATABASE@",dmuss_db_link)))
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ELSEIF (curqual=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Error selecting against ",dmuss_db_link," - v$database")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmuss_db_sync_src(null)
   DECLARE ddss_td_idx = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Determine schema differences between Source and Target to sync Source."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dm_err->eproc = "Query DM2_DDL_OPS_LOG in Target to look for any invalid rows."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log d
    WHERE d.run_id IN (
    (SELECT
     x.run_id
     FROM dm2_ddl_ops x
     WHERE x.process_option="UTC CONVERSION UPTIME"))
     AND d.op_type IN ("ADD COLUMN")
     AND ((d.status != "COMPLETE") OR (d.status = null))
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   IF (curqual > 0)
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Check for invalid status rows in Target DM2_DDL_OPS_LOG failed."
    SET dm_err->emsg = "Cleanup the invalid rows in Target DM2_DDL_OPS_LOG and restart."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Query DM2_DDL_OPS_LOG in Target to look for COMPLETE status rows."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log d
    WHERE d.run_id IN (
   ...
   IF (curqual=0)
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Check for COMPLETE status rows in Target DM2_DDL_OPS_LOG failed."
    SET dm_err->emsg = "NO DDL has been completed."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc =
   "Loading ADD COLUMN completed rows from Target dm2_ddl_ops_log into Record structure."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log d,
     dba_tab_columns u
    ...
    (SELECT
     ...
     AND d.status="COMPLETE"
     AND d.owner_name=u.owner
     AND d.table_name=u.table_name
     AND d.obj_name=u.column_name
    ORDER BY d.owner_name, d.table_name, u.column_id
    HEAD REPORT
     dmuss_tab_cnt = 0, dmuss_tab_col_cnt = 0
    HEAD d.table_name
     ddss_td_idx = 0
     IF (d.owner_name="V500"
      AND (dmuss_td_tables->cnt > 0))
      ddss_td_idx = locateval(ddss_td_idx,1,dmuss_td_tables->cnt,d.table_name,dmuss_td_tables->tbl[
       ddss_td_idx].table_name)
     ENDIF
     IF (ddss_td_idx > 0)
      dmuss_tab_cnt = (dmuss_tab_cnt+ 1), stat = alterlist(dmuss_tblcol_rec->tbl,dmuss_tab_cnt),
      dmuss_tblcol_rec->tbl[dmuss_tab_cnt].tbl_name = d.table_name
     ENDIF
    DETAIL
     IF (ddss_td_idx > 0)
      dmuss_tab_col_cnt = (dmuss_tab_col_cnt+ 1), stat = alterlist(dmuss_tblcol_rec->tbl[
       dmuss_tab_cnt].col,dmuss_tab_col_cnt), dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[
      dmuss_tab_col_cnt].col_name = d.obj_name,
      dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tab_col_cnt].col_id = u.column_id,
      dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tab_col_cnt].ddl_operation = d.operation
     ENDIF
    FOOT  d.table_name
     IF (ddss_td_idx > 0)
      stat = alterlist(dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col,dmuss_tab_col_cnt), dmuss_tblcol_rec
      ->tbl[dmuss_tab_cnt].ccnt = dmuss_tab_col_cnt, dmuss_tab_col_cnt = 0
     ENDIF
    FOOT REPORT
     stat = alterlist(dmuss_tblcol_rec->tbl,dmuss_tab_cnt), dmuss_tblcol_rec->tcnt = dmuss_tab_cnt
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 2))
    CALL echorecord(dmuss_tblcol_rec)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmuss_db_sync_tgt(null)
   DECLARE ddst_mig_utc_sync_plan_id = i4 WITH public, noconstant(0)
   DECLARE ddst_utc_col_name = vc WITH protect, noconstant("")
   DECLARE ddst_orig_col_name = vc WITH protect, noconstant("")
   DECLARE ddst_run_id_str = vc WITH protect, noconstant("")
   DECLARE ddst_var = i4 WITH public, noconstant(0)
   DECLARE ddst_start_pt = i4 WITH public, noconstant(0)
   DECLARE ddst_tspace_pt = i4 WITH public, noconstant(0)
   DECLARE ddst_start_str = vc WITH protect, noconstant("")
   DECLARE ddst_tspace_name = vc WITH protect, noconstant("")
   DECLARE ddst_storage_str = vc WITH protect, noconstant("")
   DECLARE ddst_tspace_str = vc WITH protect, noconstant("")
   DECLARE ddst_mig_dbtype_entry = vc WITH protect, noconstant("")
   DECLARE ddst_ddl_to_process = i2 WITH public, noconstant(0)
   DECLARE ddst_t = i4 WITH protect, noconstant(0)
   DECLARE ddst_tc = i4 WITH protect, noconstant(0)
   DECLARE ddst_tbl_fnd = i4 WITH protect, noconstant(0)
   DECLARE ddst_col_fnd = i4 WITH protect, noconstant(0)
   DECLARE ddst_ddl_operation = vc WITH protect, noconstant("")
   FREE RECORD ddst_run_id
   RECORD ddst_run_id(
     1 cnt = i4
     1 qual[*]
       2 run_id = f8
   )
   SET ddst_run_id->cnt = 0
   FREE RECORD ddst_src_tbls
   RECORD ddst_src_tbls(
     1 tcnt = i4
     1 tgt_sync_ind = i2
     1 tbl[*]
       2 tbl_name = vc
   ...
   SET ddst_src_tbls->tcnt = 0
   SET ddst_src_tbls->tgt_sync_ind = 0
   FREE RECORD ddst_tgt_tbls
   RECORD ddst_tgt_tbls(
     1 tcnt = i4
     1 tbl[*]
       2 tbl_name = vc
       2 ccnt = i4
   ...
   SET ddst_tgt_tbls->tcnt = 0
   SET dm_err->eproc = "Determine schema differences between Source and Target to sync Target."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET ddst_mig_dbtype_entry = dmr_mig_data->cur_db_type
   IF ((dm_err->debug_flag > 2))
    CALL echo(concat("Storing off dbtype entry: ",ddst_mig_dbtype_entry))
   ENDIF
   IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
    IF (dmr_prompt_connect_data("TARGET","V500","CO")=0)
     RETURN(0)
   ...
   IF ((dmuss_real_tables->cnt > 0))
    SET dm_err->eproc =
    "Load all tables in Target that have a table in Source with corresponding $R table."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dba_tab_cols utc,
      dba_tables t
     WHERE utc.owner=currdbuser
      AND utc.owner=t.owner
      AND utc.table_name=t.table_name
      AND t.temporary="N"
     ORDER BY utc.table_name, utc.column_id, utc.column_name
     HEAD utc.table_name
      ddst_tbl_fnd = 0, ddst_tbl_fnd = locateval(ddst_tbl_fnd,1,dmuss_real_tables->cnt,utc.table_name,
       dmuss_real_tables->qual[ddst_tbl_fnd].table_name)
      IF (ddst_tbl_fnd > 0)
       ddst_tgt_tbls->tcnt = (ddst_tgt_tbls->tcnt+ 1), stat = alterlist(ddst_tgt_tbls->tbl,
        ddst_tgt_tbls->tcnt), ddst_tgt_tbls->tbl[ddst_tgt_tbls->tcnt].tbl_name = utc.table_name,
       ddst_tbl_fnd = ddst_tgt_tbls->tcnt
      ENDIF
     DETAIL
      IF (ddst_tbl_fnd > 0)
       ddst_tgt_tbls->tbl[ddst_tbl_fnd].ccnt = (ddst_tgt_tbls->tbl[ddst_tbl_fnd].ccnt+ 1), stat =
       alterlist(ddst_tgt_tbls->tbl[ddst_tbl_fnd].col,ddst_tgt_tbls->tbl[ddst_tbl_fnd].ccnt),
       ddst_tgt_tbls->tbl[ddst_tbl_fnd].col[ddst_tgt_tbls->tbl[ddst_tbl_fnd].ccnt].col_name = utc
       .column_name,
       ddst_tgt_tbls->tbl[ddst_tbl_fnd].col[ddst_tgt_tbls->tbl[ddst_tbl_fnd].ccnt].col_id = utc
       .column_id
      ENDIF
     WITH nocounter
    ;end select
   ...
   ENDIF
   IF ((dmuss_real_tables->cnt > 0)
    AND (ddst_tgt_tbls->tcnt > 0))
    SET dm_err->eproc = "Load all tables in Source that have a corresponding $R table."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dba_tab_cols utc,
      dba_tables t
     ...
     HEAD utc.table_name
      ddst_tbl_fnd = 0, ddst_tbl_fnd = locateval(ddst_tbl_fnd,1,dmuss_real_tables->cnt,utc.table_name,
       dmuss_real_tables->qual[ddst_tbl_fnd].table_name)
      IF (ddst_tbl_fnd > 0)
       ddst_src_tbls->tcnt = (ddst_src_tbls->tcnt+ 1), stat = alterlist(ddst_src_tbls->tbl,
        ddst_src_tbls->tcnt), ddst_src_tbls->tbl[ddst_src_tbls->tcnt].tbl_name = utc.table_name,
       ddst_src_tbls->tbl[ddst_src_tbls->tcnt].tgt_sync_ind = 0, ddst_src_tbls->tbl[ddst_src_tbls->
       tcnt].tgt_idx = 0, ddst_src_tbls->tbl[ddst_src_tbls->tcnt].ddl_fnd = 0,
       ddst_tbl_fnd = ddst_src_tbls->tcnt
      ENDIF
     DETAIL
      IF (ddst_tbl_fnd > 0)
       ddst_src_tbls->tbl[ddst_tbl_fnd].ccnt = (ddst_src_tbls->tbl[ddst_tbl_fnd].ccnt+ 1), stat =
       alterlist(ddst_src_tbls->tbl[ddst_tbl_fnd].col,ddst_src_tbls->tbl[ddst_tbl_fnd].ccnt),
       ddst_src_tbls->tbl[ddst_tbl_fnd].col[ddst_src_tbls->tbl[ddst_tbl_fnd].ccnt].col_name = utc
       .column_name,
       ddst_src_tbls->tbl[ddst_tbl_fnd].col[ddst_src_tbls->tbl[ddst_tbl_fnd].ccnt].col_id = utc
       .column_id, ddst_src_tbls->tbl[ddst_tbl_fnd].col[ddst_src_tbls->tbl[ddst_tbl_fnd].ccnt].
       data_type = utc.data_type, ddst_src_tbls->tbl[ddst_tbl_fnd].col[ddst_src_tbls->tbl[
       ddst_tbl_fnd].ccnt].data_length = utc.data_length
       IF (utc.data_type IN ("BLOB", "CLOB"))
        ddst_src_tbls->tbl[ddst_tbl_fnd].col[ddst_src_tbls->tbl[ddst_tbl_fnd].ccnt].lob_ind = 1
       ENDIF
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     ...
     RETURN(0)
    ENDIF
    IF ((ddst_src_tbls->tcnt > 0))
     SET dm_err->eproc = concat("Loop through Source tables having corresponding $R table and ",
      "evalaute against Target tables for missing columns.")
     CALL disp_msg(" ",dm_err->logfile,0)
     FOR (ddst_t = 1 TO ddst_src_tbls->tcnt)
       SET ddst_tbl_fnd = 0
       SET ddst_tbl_fnd = locateval(ddst_tbl_fnd,1,ddst_tgt_tbls->tcnt,ddst_src_tbls->tbl[ddst_t].
        tbl_name,ddst_tgt_tbls->tbl[ddst_tbl_fnd].tbl_name)
       IF (ddst_tbl_fnd > 0)
        FOR (ddst_tc = 1 TO ddst_src_tbls->tbl[ddst_t].ccnt)
          SET ddst_col_fnd = 0
          SET ddst_col_fnd = locateval(ddst_col_fnd,1,ddst_tgt_tbls->tbl[ddst_tbl_fnd].ccnt,
           ddst_src_tbls->tbl[ddst_t].col[ddst_tc].col_name,ddst_tgt_tbls->tbl[ddst_tbl_fnd].col[
           ddst_col_fnd].col_name)
          IF (ddst_col_fnd=0)
           SET ddst_src_tbls->tgt_sync_ind = 1
           SET ddst_src_tbls->tbl[ddst_t].tgt_sync_ind = 1
           SET ddst_src_tbls->tbl[ddst_t].tgt_idx = ddst_tbl_fnd
           SET ddst_tc = ddst_src_tbls->tbl[ddst_t].ccnt
          ENDIF
        ENDFOR
       ENDIF
     ENDFOR
    ENDIF
    IF ((ddst_src_tbls->tgt_sync_ind=1))
     SET dm_err->eproc = "Source tables having corresponding $R table are out-of-sync with Target."
     CALL disp_msg(" ",dm_err->logfile,0)
    ELSE
     SET dm_err->eproc =
     "Source tables having corresponding $R table are not out-of-sync with Target."
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    IF ((dm_err->debug_flag > 2))
     CALL echorecord(ddst_src_tbls)
    ENDIF
   ENDIF
   SET dm_err->eproc = "Query DM2_DDL_OPS in Source for run id."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    ...
    (SELECT
     run_id
     FROM dm2_ddl_ops
     WHERE process_option="DATABASE MIGRATION"))
    ORDER BY ddo.run_id
    HEAD REPORT
     stat = alterlist(ddst_run_id->qual,10)
    DETAIL
     ddst_run_id->cnt = (ddst_run_id->cnt+ 1)
     IF (mod(ddst_run_id->cnt,10)=1
      AND (ddst_run_id->cnt != 1))
      stat = alterlist(ddst_run_id->qual,(ddst_run_id->cnt+ 9))
     ENDIF
     ddst_run_id->qual[ddst_run_id->cnt].run_id = ddo.run_id
    FOOT REPORT
     stat = alterlist(ddst_run_id->qual,ddst_run_id->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF ((ddst_run_id->cnt=0))
    SET dm_err->eproc =
    "Query DM2_DDL_OPS in Source for all run ids post Source setup.  No run ids found."
    CALL disp_msg(" ",dm_err->logfile,0)
   ELSE
    IF ((dm_err->debug_flag > 2))
     CALL echorecord(ddst_run_id)
    ENDIF
    FOR (ddst_var = 1 TO ddst_run_id->cnt)
      SET ddst_run_id_str = build(ddst_run_id_str,",",ddst_run_id->qual[ddst_var].run_id)
    ENDFOR
    SET ddst_run_id_str = replace(ddst_run_id_str,",","",1)
    SET dm_err->eproc = build("Processing CREATE TABLE and ADD COLUMN DDL for qualified run ids (",
     ddst_run_id_str,").")
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF ((ddst_run_id->cnt=0)
    AND (ddst_src_tbls->tgt_sync_ind=0))
    SET dm_err->eproc = "Source and Target in sync.  No Target sync work needed."
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF ((ddst_run_id->cnt > 0))
    SET dm_err->eproc = "Query DM2_DDL_OPS_LOG in Source to look for any invalid rows."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm2_ddl_ops_log d,
      (dummyt dt  WITH seq = value(ddst_run_id->cnt))
     PLAN (d
      WHERE d.op_type IN ("CREATE TABLE", "ADD COLUMN")
       AND ((d.status != "COMPLETE") OR (d.status = null)) )
      JOIN (dt
      WHERE (d.run_id=ddst_run_id->qual[dt.seq].run_id))
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (curqual > 0)
     SET dm_err->err_ind = 1
     SET dm_err->eproc = "Check for invalid status rows in Source DM2_DDL_OPS_LOG failed."
     SET dm_err->emsg = "Cleanup the invalid rows in Source DM2_DDL_OPS_LOG and restart."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Query DM2_DDL_OPS_LOG in Source to look for COMPLETE status rows."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm2_ddl_ops_log d,
      (dummyt dt  WITH seq = value(ddst_run_id->cnt))
    ...
    ENDIF
    IF (curqual=0)
     SET dm_err->eproc = "There are no CREATE Tables/ADD Columns in Source."
     CALL disp_msg(" ",dm_err->logfile,0)
    ELSE
     SET ddst_ddl_to_process = 1
    ENDIF
   ENDIF
   IF (ddst_ddl_to_process=1)
    SET dm_err->eproc =
    "Loading CREATE TABLE rows from Source dm2_ddl_ops_log into Record structure."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm2_ddl_ops_log d,
      user_tables ut,
     ...
     PLAN (dt
      WHERE dt.seq > 0)
      JOIN (d
      WHERE d.op_type="CREATE TABLE"
       AND d.status="COMPLETE"
       AND (d.run_id=ddst_run_id->qual[dt.seq].run_id))
      JOIN (ut
      WHERE d.table_name=ut.table_name)
     ORDER BY d.table_name
     DETAIL
      dmuss_tblcol_rec->tcnt = (dmuss_tblcol_rec->tcnt+ 1), stat = alterlist(dmuss_tblcol_rec->tbl,
       dmuss_tblcol_rec->tcnt), dmuss_tblcol_rec->tbl[dmuss_tblcol_rec->tcnt].tbl_name = d.obj_name,
      dmuss_tblcol_rec->tbl[dmuss_tblcol_rec->tcnt].ddl_operation = d.operation
      IF (dmuss_use_securefile=1
       AND findstring("STORE AS BASICFILE",d.operation) > 0)
       ddst_start_pt = 0, ddst_tspace_pt = 0, ddst_start_str = "",
       ddst_tspace_name = "", ddst_storage_str = "", ddst_tspace_str = "",
       ddst_start_pt = (findstring("STORE AS BASICFILE",d.operation)+ size("STORE AS BASICFILE")),
       ddst_start_str = substring(1,(ddst_start_pt - 1),d.operation), ddst_storage_str = substring(
        ddst_start_pt,(size(d.operation) - ddst_start_pt),d.operation),
       ddst_tspace_pt = (findstring("TABLESPACE ",ddst_storage_str)+ size("TABLESPACE ")),
       ddst_tspace_str = substring(ddst_tspace_pt,(size(ddst_storage_str) - ddst_tspace_pt),
        ddst_storage_str), ddst_tspace_name = substring(1,findstring(" ",ddst_tspace_str),
        ddst_tspace_str),
       dmuss_tblcol_rec->tbl[dmuss_tblcol_rec->tcnt].ddl_operation = concat(trim(replace(
          ddst_start_str,"STORE AS BASICFILE","STORE AS SECUREFILE")),"(TABLESPACE ",trim(
         ddst_tspace_name,3)," ENABLE STORAGE IN ROW STORAGE(PCTINCREASE 0 BUFFER_POOL ",
        dm2_db_options->lob_storage_bp,
        ") CACHE ) ^) go")
       IF ((dm_err->debug_flag > 2))
        CALL echo(concat("Adapted for securefile: ",dmuss_tblcol_rec->tbl[dmuss_tblcol_rec->tcnt].
         ddl_operation))
       ENDIF
      ENDIF
     WITH nocounter
    ...
    ENDIF
    SET dm_err->eproc =
    "Loading ADD COLUMN rows from Source dm2_ddl_ops_log into Record structure to update OBJ_NAME column."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm2_ddl_ops_log d,
      (dummyt dt  WITH seq = value(ddst_run_id->cnt))
     PLAN (d
      WHERE d.op_type="ADD COLUMN"
       AND d.status="COMPLETE"
       AND d.obj_name="UTC_TMPO_*")
      JOIN (dt
      WHERE (d.run_id=ddst_run_id->qual[dt.seq].run_id))
     DETAIL
      dmuss_ddl_ops_log_data->cnt = (dmuss_ddl_ops_log_data->cnt+ 1), stat = alterlist(
       dmuss_ddl_ops_log_data->qual,dmuss_ddl_ops_log_data->cnt), dmuss_ddl_ops_log_data->qual[
      dmuss_ddl_ops_log_data->cnt].op_id = d.op_id,
      dmuss_ddl_ops_log_data->qual[dmuss_ddl_ops_log_data->cnt].table_name = d.table_name,
      dmuss_ddl_ops_log_data->qual[dmuss_ddl_ops_log_data->cnt].obj_name = d.obj_name
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    ENDIF
    IF ((dm_err->debug_flag > 2))
     CALL echorecord(dmuss_ddl_ops_log_data)
    ENDIF
    FOR (dmuss_loop_var = 1 TO dmuss_ddl_ops_log_data->cnt)
      SET ddst_orig_col_name = substring(10,(size(dmuss_ddl_ops_log_data->qual[dmuss_loop_var].
        obj_name) - 9),dmuss_ddl_ops_log_data->qual[dmuss_loop_var].obj_name)
      SET dm_err->eproc =
      "Query Source dba_tab_columns to retrieve column_id and derive UTC_TMP1 Column name."
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg("",dm_err->logfile,0)
      ENDIF
      SELECT INTO "nl:"
       FROM dba_tab_columns u
       WHERE u.column_name=ddst_orig_col_name
        AND (u.table_name=dmuss_ddl_ops_log_data->qual[dmuss_loop_var].table_name)
        AND u.owner=currdbuser
       DETAIL
        ddst_utc_col_name = build("UTC_TMP1_",cnvtint(u.column_id))
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc)=1)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      ...
      IF (curqual=0)
       SET dm_err->err_ind = 1
       SET dm_err->emsg = concat(ddst_orig_col_name," Column does not exist in dba_tab_columns.")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ENDIF
      SET dm_err->eproc = "Update Source dm2_ddl_ops_log OBJ_NAME with derived UTC_TMP1 Column name."
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg("",dm_err->logfile,0)
      ENDIF
      UPDATE  FROM dm2_ddl_ops_log d
       SET d.obj_name = ddst_utc_col_name
       WHERE (d.op_id=dmuss_ddl_ops_log_data->qual[dmuss_loop_var].op_id)
       WITH nocounter
      ;end update
      IF (check_error(dm_err->eproc)=1)
       ROLLBACK
      ...
      ENDIF
    ENDFOR
    SET dm_err->eproc = "Loading ADD COLUMN rows from Source dm2_ddl_ops_log into Record structure."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     FROM dm2_ddl_ops_log d,
      dba_tab_columns u,
     ...
     PLAN (dt
      ...
      WHERE d.table_name=u.table_name
       AND u.owner=currdbuser
       AND d.obj_name=u.column_name)
     ORDER BY d.table_name, u.column_id
     HEAD d.table_name
      dmuss_tab_cnt = 0, dmuss_tab_cnt = locateval(dmuss_tab_cnt,1,dmuss_tblcol_rec->tcnt,d
       .table_name,dmuss_tblcol_rec->tbl[dmuss_tab_cnt].tbl_name)
      IF (dmuss_tab_cnt=0)
       dmuss_tblcol_rec->tcnt = (dmuss_tblcol_rec->tcnt+ 1), stat = alterlist(dmuss_tblcol_rec->tbl,
        dmuss_tblcol_rec->tcnt), dmuss_tab_cnt = dmuss_tblcol_rec->tcnt,
       dmuss_tblcol_rec->tbl[dmuss_tab_cnt].tbl_name = d.table_name
      ENDIF
      IF ((ddst_src_tbls->tgt_sync_ind=1))
       dmuss_tab_cnt2 = 0, dmuss_tab_cnt2 = locateval(dmuss_tab_cnt2,1,ddst_src_tbls->tcnt,d
        .table_name,ddst_src_tbls->tbl[dmuss_tab_cnt2].tbl_name)
       IF (dmuss_tab_cnt2 > 0
        AND (ddst_src_tbls->tbl[dmuss_tab_cnt2].tgt_sync_ind=1))
        dmuss_tblcol_rec->sync_from_src = 1, dmuss_tblcol_rec->tbl[dmuss_tab_cnt].sync_from_src = 1,
        dmuss_tblcol_rec->tbl[dmuss_tab_cnt].src_idx = dmuss_tab_cnt2,
        ddst_src_tbls->tbl[dmuss_tab_cnt2].ddl_fnd = 1
       ENDIF
      ENDIF
     DETAIL
      dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt = (dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt+ 1),
      stat = alterlist(dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col,dmuss_tblcol_rec->tbl[dmuss_tab_cnt].
       ccnt), dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt].
      col_name = d.obj_name,
      dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt].col_id = u
      .column_id, dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt]
      .data_type = u.data_type
      IF (u.data_type IN ("BLOB", "CLOB"))
       dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt].lob_ind =
       1
      ENDIF
      dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt].
      ddl_operation = d.operation
      IF (dmuss_use_securefile=1
       AND findstring("STORE AS BASICFILE",d.operation) > 0)
       ddst_start_pt = 0, ddst_tspace_pt = 0, ddst_start_str = "",
       ddst_tspace_name = "", ddst_storage_str = "", ddst_tspace_str = "",
       ...
       ddst_tspace_str = substring(ddst_tspace_pt,(size(ddst_storage_str) - ddst_tspace_pt),
        ...
        ddst_tspace_str),
       dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt].
       ddl_operation = concat(trim(replace(ddst_start_str,"STORE AS BASICFILE","STORE AS SECUREFILE")
         ),"(TABLESPACE ",trim(ddst_tspace_name,3),
        " ENABLE STORAGE IN ROW STORAGE(PCTINCREASE 0 BUFFER_POOL ",dm2_db_options->lob_storage_bp,
        ") CACHE ) ^) go")
       IF ((dm_err->debug_flag > 2))
        CALL echo(concat("Adapted for securefile: ",dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[
         dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt].ddl_operation))
       ENDIF
      ENDIF
     FOOT  d.table_name
      row + 0
    ...
    ENDIF
   ENDIF
   IF ((dmuss_tblcol_rec->sync_from_src=1))
    SET dm_err->eproc = "Loading revised ADD COLUMN rows from Source metadata (RDDS mover synced)."
    CALL disp_msg(" ",dm_err->logfile,0)
    FOR (dmuss_locndx = 1 TO dmuss_tblcol_rec->tcnt)
      IF ((dmuss_tblcol_rec->tbl[dmuss_locndx].sync_from_src=1))
       SET ddst_col_fnd = 0
       SET ddst_ddl_operation = "DM2NOTSET"
       SET ddst_col_fnd = locateval(ddst_col_fnd,1,dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt,1,
        dmuss_tblcol_rec->tbl[dmuss_locndx].col[ddst_col_fnd].lob_ind)
       IF (ddst_col_fnd > 0)
        SET ddst_ddl_operation = dmuss_tblcol_rec->tbl[dmuss_locndx].col[ddst_col_fnd].ddl_operation
       ENDIF
       SET dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt = 0
       SET stat = alterlist(dmuss_tblcol_rec->tbl[dmuss_locndx].col,dmuss_tblcol_rec->tbl[
        dmuss_tab_cnt].ccnt)
       SET ddst_t = dmuss_tblcol_rec->tbl[dmuss_locndx].src_idx
       FOR (ddst_tc = 1 TO ddst_src_tbls->tbl[ddst_t].ccnt)
         SET dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt = (dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt+ 1)
         SET stat = alterlist(dmuss_tblcol_rec->tbl[dmuss_locndx].col,dmuss_tblcol_rec->tbl[
          dmuss_locndx].ccnt)
         SET dmuss_tblcol_rec->tbl[dmuss_locndx].col[dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt].
         col_name = ddst_src_tbls->tbl[ddst_t].col[ddst_tc].col_name
         SET dmuss_tblcol_rec->tbl[dmuss_locndx].col[dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt].col_id
          = ddst_src_tbls->tbl[ddst_t].col[ddst_tc].col_id
         SET dmuss_tblcol_rec->tbl[dmuss_locndx].col[dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt].
         data_type = ddst_src_tbls->tbl[ddst_t].col[ddst_tc].data_type
         IF ((ddst_src_tbls->tbl[ddst_t].col[ddst_tc].lob_ind=1)
          AND ddst_ddl_operation != "DM2NOTSET")
          SET dmuss_tblcol_rec->tbl[dmuss_locndx].col[dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt].
          ddl_operation = ddst_ddl_operation
         ELSE
          IF ((ddst_src_tbls->tbl[ddst_t].col[ddst_tc].data_type IN ("CHAR", "VARCHAR", "VARCHAR2",
          "RAW")))
           SET dmuss_str = concat("RDB ASIS (^ ALTER TABLE ",trim(currdbuser),'."',trim(
             dmuss_tblcol_rec->tbl[dmuss_locndx].tbl_name),'" ADD ',
            trim(ddst_src_tbls->tbl[ddst_t].col[ddst_tc].col_name)," ",trim(ddst_src_tbls->tbl[ddst_t
             ].col[ddst_tc].data_type),"(",trim(cnvtstring(ddst_src_tbls->tbl[ddst_t].col[ddst_tc].
              data_length)),
            ") NULL ^) GO")
          ELSE
           SET dmuss_str = concat("RDB ASIS (^ ALTER TABLE ",trim(currdbuser),'."',trim(
             dmuss_tblcol_rec->tbl[dmuss_locndx].tbl_name),'" ADD ',
            trim(ddst_src_tbls->tbl[ddst_t].col[ddst_tc].col_name)," ",trim(ddst_src_tbls->tbl[ddst_t
             ].col[ddst_tc].data_type)," NULL ^) GO")
          ENDIF
          SET dmuss_tblcol_rec->tbl[dmuss_locndx].col[dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt].
          ddl_operation = dmuss_str
         ENDIF
       ENDFOR
      ENDIF
    ENDFOR
   ENDIF
   IF ((ddst_src_tbls->tgt_sync_ind=1))
    SET dm_err->eproc = "Loading new ADD COLUMN rows from Source metadata (RDDS mover synced)."
    CALL disp_msg(" ",dm_err->logfile,0)
    FOR (dmuss_locndx = 1 TO ddst_src_tbls->tcnt)
      IF ((ddst_src_tbls->tbl[dmuss_locndx].tgt_sync_ind=1)
       AND (ddst_src_tbls->tbl[dmuss_locndx].ddl_fnd=0))
       SET dmuss_tblcol_rec->tcnt = (dmuss_tblcol_rec->tcnt+ 1)
       SET stat = alterlist(dmuss_tblcol_rec->tbl,dmuss_tblcol_rec->tcnt)
       SET dmuss_tab_cnt = dmuss_tblcol_rec->tcnt
       SET dmuss_tblcol_rec->tbl[dmuss_tab_cnt].tbl_name = ddst_src_tbls->tbl[dmuss_locndx].tbl_name
       FOR (ddst_tc = 1 TO ddst_src_tbls->tbl[dmuss_locndx].ccnt)
         SET dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt = (dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt+
         1)
         SET stat = alterlist(dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col,dmuss_tblcol_rec->tbl[
          dmuss_tab_cnt].ccnt)
         SET dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt].
         col_name = ddst_src_tbls->tbl[dmuss_locndx].col[ddst_tc].col_name
         SET dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt].
         col_id = ddst_src_tbls->tbl[dmuss_locndx].col[ddst_tc].col_id
         SET dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt].
         data_type = ddst_src_tbls->tbl[dmuss_locndx].col[ddst_tc].data_type
         IF ((ddst_src_tbls->tbl[dmuss_locndx].col[ddst_tc].data_type IN ("CHAR", "VARCHAR",
         "VARCHAR2", "RAW")))
          SET dmuss_str = concat("RDB ASIS (^ ALTER TABLE ",trim(currdbuser),'."',trim(ddst_src_tbls
            ->tbl[dmuss_locndx].tbl_name),'" ADD ',
           trim(ddst_src_tbls->tbl[dmuss_locndx].col[ddst_tc].col_name)," ",trim(ddst_src_tbls->tbl[
            dmuss_locndx].col[ddst_tc].data_type),"(",trim(cnvtstring(ddst_src_tbls->tbl[dmuss_locndx
             ].col[ddst_tc].data_length)),
           ") NULL ^) GO")
         ELSE
          SET dmuss_str = concat("RDB ASIS (^ ALTER TABLE ",trim(currdbuser),'."',trim(ddst_src_tbls
            ->tbl[dmuss_locndx].tbl_name),'" ADD ',
           trim(ddst_src_tbls->tbl[dmuss_locndx].col[ddst_tc].col_name)," ",trim(ddst_src_tbls->tbl[
            dmuss_locndx].col[ddst_tc].data_type)," NULL ^) GO")
         ENDIF
         SET dmuss_tblcol_rec->tbl[dmuss_tab_cnt].col[dmuss_tblcol_rec->tbl[dmuss_tab_cnt].ccnt].
         ddl_operation = dmuss_str
       ENDFOR
   ...
   IF ((dm_err->debug_flag > 2))
    CALL echorecord(dmuss_tblcol_rec)
   ENDIF
   IF ((((dmr_mig_data->cur_db_type != ddst_mig_dbtype_entry)) OR (currdbuser != "V500")) )
    IF ((dm_err->debug_flag > 2))
     CALL echo(concat("Connecting back to: ",ddst_mig_dbtype_entry))
    ENDIF
    IF (dmr_prompt_connect_data(ddst_mig_dbtype_entry,"V500","CO")=0)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmuss_tgt_excl_optype(null)
   DECLARE dteo_fname = vc WITH protect, noconstant("")
   DECLARE dteo_parm_loc = vc WITH protect, noconstant("")
   DECLARE dteo_tbl_name = vc WITH protect, noconstant(" ")
   DECLARE dteo_col_name = vc WITH protect, noconstant(" ")
   DECLARE dteo_pos = i4 WITH protect, noconstant(0)
   DECLARE dteo_iter = i4 WITH protect, noconstant(0)
   RECORD dteo_content(
     1 cnt = i4
     1 qual[*]
       2 value = vc
   )
   SET dteo_content->cnt = 0
   SET dm_err->eproc = "Create files for each CREATE TABLE ADD and COLUMN operaion."
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dteo_parm_loc = "/ggdelivery/dirprm"
   IF (dm2_find_dir(dteo_parm_loc)=0)
    SET message = window
    CALL clear(1,1)
    CALL box(1,1,15,120)
    CALL text(3,2,"Enter Delivery parameter file directory location :")
    CALL accept(3,70,"P(30);C",dteo_parm_loc
     WHERE curaccept != "")
    SET dteo_parm_loc = trim(curaccept)
    SET message = nowindow
   ENDIF
   IF (dm2_find_dir(dteo_parm_loc)=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Delivery parm directory entered [",dteo_parm_loc,"] does not exist.")
    SET dm_err->eproc = "Verify delivery parm directory exists."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Loading Delivery exclusion rows from Admin dm_info."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_admin_dm_info di
    WHERE di.info_domain=patstring(cnvtupper(build(dmuss_dbase_name,"_UTC_DATA - DELIV EXCLUDE")))
    ORDER BY di.info_name
    DETAIL
     dteo_content->cnt = (dteo_content->cnt+ 1), stat = alterlist(dteo_content->qual,dteo_content->
      cnt), dteo_content->qual[dteo_content->cnt].value = di.info_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (substring(size(dteo_parm_loc),1,dteo_parm_loc) != "/")
    SET dteo_parm_loc = concat(trim(dteo_parm_loc),"/")
   ENDIF
   SET dteo_fname = concat(trim(dteo_parm_loc),"batches.mac")
   SELECT INTO value(dteo_fname)
    FROM dummyt d
    HEAD REPORT
     col 0,
     CALL print("MACRO #batches"), row + 1,
     col 0,
     CALL print("BEGIN"), row + 1
    DETAIL
     IF ((dteo_content->cnt > 0))
      FOR (dteo_iter = 1 TO dteo_content->cnt)
        IF (findstring(".",dteo_content->qual[dteo_iter].value)=0)
         IF ((dmr_mig_data->src_ora_level1=11)
          AND (dmr_mig_data->tgt_ora_level1 > 11))
          col 0,
          CALL print(concat("EXCLUDE OPTYPE CREATE OBJTYPE TABLE OBJNAME ",dmr_mig_data->tgt_db_name,
           ".V500.",trim(dteo_content->qual[dteo_iter].value)," &")), row + 1
         ELSE
          col 0,
          CALL print(concat("EXCLUDE OPTYPE CREATE OBJTYPE TABLE OBJNAME V500.",trim(dteo_content->
            qual[dteo_iter].value)," &")), row + 1
         ENDIF
        ELSE
         dteo_pos = findstring(".",dteo_content->qual[dteo_iter].value,1,1), dteo_tbl_name =
         substring(1,(dteo_pos - 1),dteo_content->qual[dteo_iter].value), dteo_col_name = substring((
          dteo_pos+ 1),size(dteo_content->qual[dteo_iter].value),dteo_content->qual[dteo_iter].value)
         IF ((dmr_mig_data->src_ora_level1=11)
          AND (dmr_mig_data->tgt_ora_level1 > 11))
          col 0,
          CALL print(concat("EXCLUDE OPTYPE ALTER OBJTYPE TABLE OBJNAME ",dmr_mig_data->tgt_db_name,
           ".V500.",dteo_tbl_name,^ INSTRWORDS ' " ADD ^,
           dteo_col_name,^ "' &^)), row + 1
         ELSE
          col 0,
          CALL print(concat("EXCLUDE OPTYPE ALTER OBJTYPE TABLE OBJNAME V500.",dteo_tbl_name,
           ^ INSTRWORDS ' " ADD ^,dteo_col_name,^ "' &^)), row + 1
         ENDIF
        ENDIF
      ENDFOR
     ENDIF
    FOOT REPORT
     IF ((dmr_mig_data->src_ora_level1=11)
      ...
      col 0,
      CALL print(concat("EXCLUDE OPTYPE CREATE OBJTYPE TABLE OBJNAME ",dmr_mig_data->tgt_db_name,
       ".V500.DM2_MIG_FAKE_BATCH2")), row + 1
     ELSE
      col 0,
      CALL print("EXCLUDE OPTYPE CREATE OBJTYPE TABLE OBJNAME V500.DM2_MIG_FAKE_BATCH2"), row + 1
     ENDIF
     col 0,
     CALL print("END;"), row + 1
    WITH nocounter, maxrow = 1, format = lfstream,
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmuss_rscbsy_logic(null)
   DECLARE drl_cmd_rscbsy_retry_cnt = i4 WITH protect, noconstant(0)
   DECLARE drl_cmd_rscbsy_error = i2 WITH protect, noconstant(0)
   SET drl_cmd_rscbsy_retry_cnt = 0
   SET drl_cmd_rscbsy_error = 1
   WHILE (drl_cmd_rscbsy_error=1
    AND drl_cmd_rscbsy_retry_cnt < cnvtint(dm2_db_options->resource_busy_maxretry))
     SET dm_err->err_ind = 0
     SET drl_cmd_rscbsy_error = 0
     EXECUTE dm2_exec_child_cmd "CMD", dmuss_str
     ...
     IF ((dm_err->err_ind > 0))
      ...
      ENDIF
     ENDIF
     IF (drl_cmd_rscbsy_error=1)
      CALL pause(3)
      SET drl_cmd_rscbsy_retry_cnt = (drl_cmd_rscbsy_retry_cnt+ 1)
     ENDIF
   ENDWHILE
   IF (drl_cmd_rscbsy_error=1)
    IF (dmuss_dbtype="SOURCE")
     SET dm_err->err_ind = 1
     SET dm_err->eproc = concat("Issuing alter table for ",dmuss_table," to sync up UTC columns.")
     SET dm_err->emsg =
     "Resource busy retry logic reached max count, cannot continue with alter table."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     SET dm_err->err_ind = 0
     SET dmuss_fail_tables->cnt = (dmuss_fail_tables->cnt+ 1)
     SET stat = alterlist(dmuss_fail_tables->qual,dmuss_fail_tables->cnt)
     SET dmuss_fail_tables->qual[dmuss_fail_tables->cnt].table_name = dmuss_table
     RETURN(1)
    ELSE
     SET dm_err->eproc =
     "DM2_MIG_UTC_SCHEMA_SYNC is checking for Resource Busy retry for DDL creation."
     SET dm_err->emsg =
     "DM2_MIG_UTC_SCHEMA_SYNC has reached the maximum count for resource busy retry count."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmuss_add_column(null)
   DECLARE dac_blocking_str = vc WITH protect, noconstant("")
   DECLARE dac_loop = i2 WITH protect, noconstant(0)
   DECLARE dac_in_parse_ind = i2 WITH protect, noconstant(1)
   DECLARE dac_time_in_sec = i2 WITH protect, noconstant(0)
   DECLARE dac_run_cmd = i2 WITH protect, noconstant(0)
   DECLARE dac_excl_str = vc WITH protect, noconstant("")
   SET dmuss_str = ""
   FOR (dmuss_col_var = 1 TO dmuss_tblcol_rec->tbl[dmuss_locndx].ccnt)
     SET dm_err->eproc = concat("Query DBA_TAB_COLUMNS to check if ",dmuss_tblcol_rec->tbl[
      dmuss_locndx].col[dmuss_col_var].col_name," column already exists in table ",trim(
       dmuss_tblcol_rec->tbl[dmuss_locndx].tbl_name),".")
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg("",dm_err->logfile,0)
     ENDIF
     SELECT INTO "nl:"
      FROM dba_tab_columns u
      WHERE (u.column_name=dmuss_tblcol_rec->tbl[dmuss_locndx].col[dmuss_col_var].col_name)
       AND (u.table_name=dmuss_tblcol_rec->tbl[dmuss_locndx].tbl_name)
       AND u.owner=currdbuser
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
     ...
     IF (curqual=0)
      IF (dmuss_dbtype="SOURCE")
       SET dac_run_cmd = 1
       IF (dmuss_str="")
        SET dmuss_str = concat("RDB ASIS (^ ALTER TABLE ",trim(currdbuser),'."',trim(dmuss_tblcol_rec
          ->tbl[dmuss_locndx].tbl_name),'" ADD (',
         trim(dmuss_tblcol_rec->tbl[dmuss_locndx].col[dmuss_col_var].col_name)," DATE NULL")
       ELSE
        SET dmuss_str = concat(dmuss_str,", ",trim(dmuss_tblcol_rec->tbl[dmuss_locndx].col[
          dmuss_col_var].col_name)," DATE NULL")
       ENDIF
      ELSE
       SET dm_err->eproc = concat("ADD COLUMN DDL Execution for Table ",dmuss_tblcol_rec->tbl[
        dmuss_locndx].tbl_name)
       CALL disp_msg(" ",dm_err->logfile,0)
       SET dmuss_str = dmuss_tblcol_rec->tbl[dmuss_locndx].col[dmuss_col_var].ddl_operation
       IF (dmuss_rscbsy_logic(null)=0)
        RETURN(0)
       ELSE
        SET dac_excl_str = cnvtupper(build(dmuss_tblcol_rec->tbl[dmuss_locndx].tbl_name,".",
          dmuss_tblcol_rec->tbl[dmuss_locndx].col[dmuss_col_var].col_name))
        IF (dmuss_load_exclusion(dmuss_dbase_name,dac_excl_str)=0)
         RETURN(0)
        ENDIF
       ENDIF
      ENDIF
   ...
   IF (dmuss_dbtype="SOURCE"
    AND dac_run_cmd=1)
    SET dmuss_str = concat(dmuss_str,") /*DM2MIG SOURCE DDL ONLY*/^) GO")
    SET dmuss_table = dmuss_tblcol_rec->tbl[dmuss_locndx].tbl_name
    IF (dmuss_retry_ceiling > 0)
     WHILE (dac_in_parse_ind=1
      AND dac_loop < dmuss_retry_ceiling)
       IF (dir_check_in_parse(currdbuser,dmuss_table,dac_in_parse_ind,dac_blocking_str)=0)
        RETURN(0)
       ENDIF
       SET dac_loop = (dac_loop+ 1)
       IF (dac_in_parse_ind=1)
        SET dm_err->eproc = concat("Pausing 10 seconds due to hard parse event detected on ",trim(
          currdbuser),".",dmuss_table,". Attempt ",
         trim(cnvtstring(dac_loop)),"/",trim(cnvtstring(dmuss_retry_ceiling)),".")
        CALL disp_msg(" ",dm_err->logfile,0)
        CALL pause(10)
        IF (dac_loop=dmuss_retry_ceiling)
         SET dac_time_in_sec = (dmuss_retry_ceiling * 10)
         SET dm_err->err_ind = 1
         SET dm_err->eproc = concat("Validating ",dmuss_table," table is NOT part of a ",
          "long parse event prior to issuing alter table to sync up UTC columns.")
         SET dm_err->emsg = concat("Parse event found against ",trim(dmuss_table),
          " table, cannot continue with alter table. Wait time exceeded (",trim(cnvtstring(
            dac_time_in_sec))," seconds). ",
          dac_blocking_str)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
         SET dm_err->err_ind = 0
         SET dmuss_fail_tables->cnt = (dmuss_fail_tables->cnt+ 1)
     ...
     ENDWHILE
    ENDIF
    SET dm_err->eproc = concat("ADD COLUMN DDL Execution for Table ",dmuss_table)
    CALL disp_msg(" ",dm_err->logfile,0)
    IF (dmuss_rscbsy_logic(null)=0)
     RETURN(0)
    ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmuss_chk_current_db(null)
   DECLARE dccd_tbl_fnd = i4 WITH protect, noconstant(0)
   DECLARE dccd_col_fnd = i4 WITH protect, noconstant(0)
   DECLARE dccd_temp = i2 WITH protect, noconstant(0)
   DECLARE dccd_mngd_tables = vc WITH protect, noconstant("")
   DECLARE dccd_mismatch_ind = i4 WITH protect, noconstant(0)
   DECLARE dccd_rpt_file = vc WITH protect, noconstant(" ")
   DECLARE dccd_ndx = i2 WITH protect, noconstant(0)
   DECLARE dccd_excl_ind = i2 WITH protect, noconstant(0)
   DECLARE dccd_utc_col_name = vc WITH protect, noconstant(" ")
   DECLARE dccd_td_idx = i4 WITH protect, noconstant(0)
   SET dccd_mngd_tables = ""
   IF (dmr_load_managed_tables(dccd_mngd_tables)=0)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Check to see if Current Database is in sync by having UTC_TMP Columns."
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT
    IF (dmuss_dbtype="SOURCE")
     FROM dba_tab_cols utc,
      dba_tables t
     WHERE  NOT (expand(dccd_ndx,1,dm2_sch_except->tcnt,utc.table_name,dm2_sch_except->tbl[dccd_ndx].
      tbl_name))
      AND t.table_name != "MVJ*"
      AND t.temporary="N"
      AND t.owner=currdbuser
      AND parser(concat(" t.table_name not in (",dccd_mngd_tables,")"))
      AND t.owner=utc.owner
      AND t.table_name=utc.table_name
      AND utc.data_type="DATE"
      AND  NOT (utc.hidden_column="YES"
      AND utc.virtual_column="NO")
     ORDER BY utc.table_name, utc.column_id
    ELSE
     FROM dba_tab_cols@ref_data_link utc,
      dba_tables@ref_data_link t
     WHERE  NOT (expand(dccd_ndx,1,dm2_sch_except->tcnt,utc.table_name,dm2_sch_except->tbl[dccd_ndx].
      tbl_name))
      AND t.table_name != "MVJ*"
      AND t.temporary="N"
    ...
    ENDIF
    INTO "nl:"
    utc.table_name, utc.column_name, utc.column_id
    HEAD REPORT
     dmuss_tab_cnt = 0, dmuss_tab_col_cnt = 0
    HEAD utc.table_name
     dccd_td_idx = 0
     IF ((dmuss_td_tables->cnt > 0))
      dccd_td_idx = locateval(dccd_td_idx,1,dmuss_td_tables->cnt,utc.table_name,dmuss_td_tables->tbl[
       dccd_td_idx].table_name)
     ENDIF
     IF (dccd_td_idx > 0)
      dccd_tbl_fnd = 0, dccd_tbl_fnd = locateval(dccd_tbl_fnd,1,dus_std_convert_list->tbl_cnt,utc
       .table_name,dus_std_convert_list->tbl[dccd_tbl_fnd].table_name), dmuss_tab_cnt = (
      dmuss_tab_cnt+ 1),
      stat = alterlist(dmuss_col_cmp->tbl,dmuss_tab_cnt), dmuss_col_cmp->tbl[dmuss_tab_cnt].tbl_name
       = utc.table_name
     ENDIF
    DETAIL
     IF (dccd_td_idx > 0)
      dccd_excl_ind = 0
      IF (dccd_tbl_fnd > 0)
       dccd_col_fnd = 0, dccd_col_fnd = locateval(dccd_col_fnd,1,dus_std_convert_list->tbl[
        dccd_tbl_fnd].col_cnt,utc.column_name,dus_std_convert_list->tbl[dccd_tbl_fnd].col[
        dccd_col_fnd].column_name)
       IF (dccd_col_fnd > 0
        AND (dus_std_convert_list->tbl[dccd_tbl_fnd].col[dccd_col_fnd].no_convert_ind=1))
        dmuss_col_cmp->tbl[dmuss_tab_cnt].excl_utc_dt_cnt = (dmuss_col_cmp->tbl[dmuss_tab_cnt].
        excl_utc_dt_cnt+ 1), dccd_excl_ind = 1
       ENDIF
      ENDIF
      dmuss_tab_col_cnt = (dmuss_tab_col_cnt+ 1), stat = alterlist(dmuss_col_cmp->tbl[dmuss_tab_cnt].
       col,dmuss_tab_col_cnt), dmuss_col_cmp->tbl[dmuss_tab_cnt].col[dmuss_tab_col_cnt].col_name =
      utc.column_name,
      dmuss_col_cmp->tbl[dmuss_tab_cnt].col[dmuss_tab_col_cnt].col_id = utc.column_id
      IF (dccd_excl_ind=1)
       dmuss_col_cmp->tbl[dmuss_tab_cnt].col[dmuss_tab_col_cnt].excl_dt_col = 1
      ENDIF
      IF (findstring("UTC_TMP1_",utc.column_name,1,0) > 0)
       dmuss_col_cmp->tbl[dmuss_tab_cnt].utc_col_cnt = (dmuss_col_cmp->tbl[dmuss_tab_cnt].utc_col_cnt
       + 1)
      ELSE
       dmuss_col_cmp->tbl[dmuss_tab_cnt].orig_col_cnt = (dmuss_col_cmp->tbl[dmuss_tab_cnt].
       orig_col_cnt+ 1)
      ENDIF
     ENDIF
    FOOT  utc.table_name
     IF (dccd_td_idx > 0)
      stat = alterlist(dmuss_col_cmp->tbl[dmuss_tab_cnt].col,dmuss_tab_col_cnt), dmuss_col_cmp->tbl[
      dmuss_tab_cnt].ccnt = dmuss_tab_col_cnt, dmuss_tab_col_cnt = 0
     ENDIF
    FOOT REPORT
     stat = alterlist(dmuss_col_cmp->tbl,dmuss_tab_cnt), dmuss_col_cmp->tcnt = dmuss_tab_cnt
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF ((dm_err->debug_flag > 2))
    CALL echorecord(dmuss_col_cmp)
   ENDIF
   SET dm_err->eproc = concat("Check to see if Date Column has its counter part UTC Column.")
   CALL disp_msg(" ",dm_err->logfile,0)
   FOR (dmuss_loop_var = 1 TO dmuss_col_cmp->tcnt)
     IF (((dmuss_col_cmp->tbl[dmuss_loop_var].utc_col_cnt+ dmuss_col_cmp->tbl[dmuss_loop_var].
     excl_utc_dt_cnt)=dmuss_col_cmp->tbl[dmuss_loop_var].orig_col_cnt))
      FOR (dmuss_col_var = 1 TO dmuss_col_cmp->tbl[dmuss_loop_var].ccnt)
        IF ((dmuss_col_cmp->tbl[dmuss_loop_var].col[dmuss_col_var].excl_dt_col=0))
         IF (substring(1,9,dmuss_col_cmp->tbl[dmuss_loop_var].col[dmuss_col_var].col_name) !=
         "UTC_TMP1_")
          SET dccd_utc_col_name = build("UTC_TMP1_",dmuss_col_cmp->tbl[dmuss_loop_var].col[
           dmuss_col_var].col_id)
          SET dccd_temp = 0
          SET dccd_temp = locateval(dccd_temp,1,dmuss_col_cmp->tbl[dmuss_loop_var].ccnt,
           dccd_utc_col_name,dmuss_col_cmp->tbl[dmuss_loop_var].col[dccd_temp].col_name)
          IF (dccd_temp=0)
           SET dmuss_col_cmp->tbl[dmuss_loop_var].mismatch_ind = 1
           SET dccd_mismatch_ind = 1
           SET dum_utc_invalid_tables->cnt = (dum_utc_invalid_tables->cnt+ 1)
           SET stat = alterlist(dum_utc_invalid_tables->qual,dum_utc_invalid_tables->cnt)
           SET dum_utc_invalid_tables->qual[dum_utc_invalid_tables->cnt].table_name = dmuss_col_cmp->
           tbl[dmuss_loop_var].tbl_name
           SET dmuss_col_var = dmuss_col_cmp->tbl[dmuss_loop_var].ccnt
          ENDIF
         ENDIF
        ENDIF
      ENDFOR
     ELSE
      ...
      SET dum_utc_invalid_tables->cnt = (dum_utc_invalid_tables->cnt+ 1)
      SET stat = alterlist(dum_utc_invalid_tables->qual,dum_utc_invalid_tables->cnt)
      SET dum_utc_invalid_tables->qual[dum_utc_invalid_tables->cnt].table_name = dmuss_col_cmp->tbl[
      dmuss_loop_var].tbl_name
     ENDIF
   ENDFOR
   IF ((dm_err->debug_flag > 3))
    CALL echorecord(dmuss_col_cmp)
   ENDIF
   IF (dccd_mismatch_ind=1)
    SET dccd_rpt_file = "dm2_mig_tables_cols_mismatch"
    IF (get_unique_file(dccd_rpt_file,".rpt")=0)
     RETURN(0)
    ENDIF
    SET dccd_rpt_file = dm_err->unique_fname
    SELECT INTO value(dccd_rpt_file)
     FROM (dummyt d  WITH d.seq = 1)
     HEAD REPORT
      col 40, "Tables Out of Sync Between Source and Target.", row + 1,
      col 40, "--------------------------------------------", row + 1,
      row + 2, col 2, "Table Name"
     DETAIL
      FOR (dmuss_loop_var = 1 TO dmuss_col_cmp->tcnt)
        IF ((dmuss_col_cmp->tbl[dmuss_loop_var].mismatch_ind=1))
         row + 2, col 2, dmuss_col_cmp->tbl[dmuss_loop_var].tbl_name
        ENDIF
      ENDFOR
     WITH nocounter, maxcol = 250, maxrow = 1,
      formfeed = none, nullreport
    ...
    IF (check_error(dm_err->eproc)=1)
     ...
     RETURN(0)
    ENDIF
    IF (dm2_disp_file(dccd_rpt_file," ")=0)
     RETURN(0)
    ENDIF
    SET dm_err->eproc = "Tables Out of Sync."
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Found Tables/Columns mismatch. Cannot progress further."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmuss_load_exclusion(dle_dbname,dle_content)
   SET dm_err->eproc = "Load delivery exclusion content into Admin DM_INFO"
   CALL disp_msg("",dm_err->logfile,0)
   MERGE INTO dm2_admin_dm_info di
   USING DUAL ON (di.info_domain=patstring(cnvtupper(build(dle_dbname,"_UTC_DATA - DELIV EXCLUDE")))
    AND di.info_name=dle_content)
   WHEN MATCHED THEN
   (UPDATE
   ...
   (INSERT  FROM di
    ...
    VALUES(patstring(cnvtupper(build(dle_dbname,"_UTC_DATA - DELIV EXCLUDE"))), dle_content,
    cnvtdatetime(curdate,curtime3))
    WITH nocounter
   ;end insert
   )
 ...
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "Ending DM2_MIG_UTC_SCHEMA_SYNC"
 CALL final_disp_msg("dm2_mig_utc_schema_sync")
END GO
