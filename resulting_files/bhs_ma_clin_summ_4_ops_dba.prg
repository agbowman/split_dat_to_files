CREATE PROGRAM bhs_ma_clin_summ_4_ops:dba
 PROMPT
 ...
 SET vversion = "OPERATIONS"
 SET nu = cnvtupper(cnvtalphanum( $LOC))
 DECLARE nurse_unit_cd = f8
 SET nurse_unit_cd = 0.0
 SELECT INTO "nl:"
  cv = c.code_value, dk = c.display_key
  FROM code_value c
  ...
  WITH nocounter
 ;end select
 CALL echo(build("nurse_unit_cd =",nurse_unit_cd))
 DECLARE activestatus_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",261,"ACTIVE"))
 SELECT INTO "nl:"
  e.encntr_id, e.person_id
  FROM encntr_domain e,
   encounter enc
  PLAN (e
   WHERE e.loc_nurse_unit_cd=nurse_unit_cd)
   JOIN (enc
   WHERE enc.encntr_id=e.encntr_id
    AND enc.active_ind=1
    AND enc.end_effective_dt_tm >= cnvtdatetime(curdate,curtime3)
    AND enc.disch_dt_tm=null
    AND enc.encntr_status_cd=activestatus_cd)
  ...
  HEAD e.person_id
   p = (p+ 1), stat = alterlist(request->person,p), request->person[p].person_id = e.person_id,
   request->person_cnt = p
  HEAD e.encntr_id
   e = (e+ 1), stat = alterlist(request->visit,e), request->visit[e].encntr_id = e.encntr_id,
   request->visit_cnt = e
  WITH nocounter
 ;end select
 ...
 DECLARE rpt_title = c20 WITH public, constant("PROD/CLINSUM")
 ...
 DECLARE ea_mrn_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",319,"MRN"))
 ...
 DECLARE onadmitresthomescommresshelters_cd = f8 WITH public, constant(uar_get_code_by("DISPLAYKEY",
   72,"ONADMITRESTHOMESCOMMRESSHELTERS"))
 DECLARE onadmitnursinghomesrehabfacilities_cd = f8 WITH public, constant(uar_get_code_by(
 ...
 IF ((request->visit_cnt > 0))
  SELECT INTO "nl:"
   ...
   PLAN (d)
    ...
    WHERE ea2.encntr_id=outerjoin(e.encntr_id)
     AND ea2.encntr_alias_type_cd=outerjoin(ea_mrn_cd)
   ...
   DETAIL
    ...
    dlrec->seq[cnt].mrn = ea2.alias, dlrec->seq[cnt].person_id = p.person_id, dlrec->seq[cnt].cmrn =
    pa.alias,
    dlrec->seq[cnt].nurse_unit = nurse_unit, dlrec->seq[cnt].pt_loc = pt_loc, dlrec->seq[cnt].
    isolation_disp = isolation_disp,
    dlrec->seq[cnt].org_name = org.org_name, stat = alterlist(dlrec->seq[cnt].diagnosis,1), dlrec->
    seq[cnt].diagnosis[1].diag_type_desc = "Reason For Visit",
    dlrec->seq[cnt].diagnosis[1].source_string = e.reason_for_visit, dlrec->seq[cnt].diagnosis[1].
    diag_dt_tm = substring(1,14,format(e.reg_dt_tm,"@SHORTDATETIME;;Q")), dlrec->seq[cnt].total_diag
     = 1
   FOOT REPORT
    stat = alterlist(dlrec->seq,cnt), dlrec->encntr_total = cnt
   WITH nocounter
  ;end select
  CALL echo(build("dlrec cnt :",dlrec->encntr_total))
  IF (curqual=0)
   GO TO endprog
  ENDIF
  SELECT INTO "nl:"
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   WITH nocounter
  ;end select
  CALL echo("I am in events section")
  SELECT DISTINCT INTO "nl:"
   sort_order =
   ...
   WITH nocounter
  ;end select
  CALL echo("I finished event section")
  CALL echo("I am in readin diagnosis info")
  SELECT DISTINCT INTO "nl:"
   FROM (dummyt dd  WITH seq = value(dlrec->encntr_total)),
    diagnosis d,
    nomenclature n
   ...
   HEAD d.encntr_id
    cnt = dlrec->seq[dd.seq].total_diag, stat = alterlist(dlrec->seq[dd.seq].diagnosis,10)
   DETAIL
    cnt = (cnt+ 1)
    IF (mod(cnt,10)=1)
   ...
   WITH nocounter
  ;end select
  CALL echo("I finished readin diagnosis ")
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   WITH nocounter
  ;end select
  CALL echo("I finished cheif complaint")
  CALL echo("I am in events section2")
  SELECT DISTINCT INTO "nl:"
   sort_order =
   ...
   WITH nocounter
  ;end select
  CALL echo("I finished events section2")
  CALL echo("I am in events section3")
  SELECT DISTINCT INTO "nl:"
   sort_order =
   ...
   WITH nocounter
  ;end select
  CALL echo("I finished events section3")
  CALL echo("I am in orders section 3.5")
  SELECT INTO "nl:"
   sort_order =
   ...
   ELSEIF (o.activity_type_cd=infusiontherapy_cd) 5
   ELSE 99
   ENDIF
   FROM (dummyt dd  WITH seq = value(dlrec->encntr_total)),
    orders o
  ...
  SELECT DISTINCT INTO "nl:"
   ...
   ELSEIF (c.event_cd=onadmitmedicalequipcompanies_cd) 19
   ...
   ELSEIF (c.event_cd=onadmitearlyinterventionprograms_cd) 12
   ELSEIF (c.event_cd=onadmitvnahospicehomecare_cd) 11
   ELSEIF (c.event_cd=onadmitadultdayhealthcare_cd) 10
   ELSEIF (c.event_cd=onadmitresthomescommresshelters_cd) 9
   ELSEIF (c.event_cd=onadmitnursinghomesrehabfacilities_cd) 8
   ELSEIF (c.event_cd=onadmitchronichospitals_cd) 7
   ...
   PLAN (dd)
    ...
    onadmitearlyinterventionprograms_cd, onadmitvnahospicehomecare_cd, onadmitadultdayhealthcare_cd,
    onadmitresthomescommresshelters_cd, onadmitnursinghomesrehabfacilities_cd,
    onadmitchronichospitals_cd, onadmitmedicalequipcompanies_cd,
    dischargenursinghomesrehabfacilities_cd, dischargeresthomesresidencesshelters_cd,
    dischargeadultdayhealthcare_cd,
    dischargevnahospicehomecare_cd, dischargeearlyinterventionprograms_cd,
    dischargemedicalequipmentcompanies_cd)
     AND (dlrec->seq[dd.seq].encntr_id=(c.encntr_id+ 0))
     AND c.view_level=1
     AND c.publish_flag=1
     AND c.valid_until_dt_tm=cnvtdatetime("31-dec-2100,00:00:00")
  ...
  SELECT INTO "nl:"
   ...
   ORDER BY o.encntr_id, sort_order, cnvtdatetime(o.orig_order_dt_tm),
    o.order_id
   HEAD o.encntr_id
    cnt = 0, stat = alterlist(dlrec->seq[dd.seq].rorders,10)
   DETAIL
    cnt = (cnt+ 1),
    CALL echo(build(dlrec->seq[dd.seq].encntr_id,cnt))
    IF (mod(cnt,10)=1)
  ...
  SELECT INTO "nl:"
   ...
   WITH nocounter
  ;end select
  CALL echo("start doctor info section")
  SELECT DISTINCT INTO "nl:"
   FROM (dummyt dd  WITH seq = value(dlrec->encntr_total)),
   ...
   WITH nocounter
  ;end select
  CALL echo("end doctor info section")
  CALL echo("start PCP info")
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(dlrec->encntr_total)),
    prsnl pl2,
   ...
   WITH nocounter
  ;end select
  CALL echo("end PCP info")
  CALL echo("start document section")
  SELECT DISTINCT INTO "nl:"
   FROM (dummyt dd  WITH seq = value(dlrec->encntr_total)),
    clinical_event c,
    ce_date_result cd,
   ...
   WITH nocounter
  ;end select
  CALL echo("end document section")
  SELECT INTO request->output_device
   HEAD REPORT
    i = 1, "{f/8}{cpi/14}{lpi/8}", row + 1,
   ...
   HEAD PAGE
    IF ((i <= dlrec->encntr_total))
     ...
     curtime,
     CALL print(calcpos(xcol4,yrow1)), "{b}Downtime Clinical Summary {endb}",
     vversion,
     ...
     CALL print(calcpos(xcol2,yrow1)),
     "CMRN: ", dlrec->seq[i].cmrn,
     CALL print(calcpos((xcol4 - 5),yrow1)),
     "Acct #: ", dlrec->seq[i].account_nbr,
     CALL print(calcpos((xcol6 - 3),yrow1)),
   ...
   DETAIL
    FOR (i = 1 TO dlrec->encntr_total)
      ...
      FOR (a = 1 TO dlrec->seq[i].total_al)
        ...
        "{endb}", rowplusone
      ENDFOR
      xcolvar = xcol1, tempstring = dlrec->seq[i].code_status, line_wrap,
      ...
      tempstring = dlrec->seq[i].isolation_disp, line_wrap, xcolvar = xcol3
      IF ((dlrec->seq[i].problem_total > 0))
       CALL print(calcpos(xcol1,yrow1)), "{b}Problem List:{endb}", last_title = "Problem List"
       FOR (a2 = 1 TO dlrec->seq[i].problem_total)
        tempstring = concat(dlrec->seq[i].problem[a2].full_text," "),line_wrap
       ENDFOR
      ENDIF
      xcolvar = xcol1
      FOR (a3 = 1 TO dlrec->seq[i].section1_total)
       tempstring = concat("{b}",dlrec->seq[i].section1_events[a3].event_cd_disp,"{endb} ",dlrec->
        seq[i].section1_events[a3].result),line_wrap
      ENDFOR
      xcolvar = (xcol3 - 10), wrapcol = 88
      IF ((dlrec->seq[i].total_diag > 0))
       CALL print(calcpos(xcol1,yrow1)), "{b}Diagnosis:{endb}", last_title = "Diagnosis"
       FOR (a4 = 1 TO dlrec->seq[i].total_diag)
        tempstring = concat(trim(dlrec->seq[i].diagnosis[a4].source_string,3)," (",trim(dlrec->seq[i]
          .diagnosis[a4].diag_type_desc,3),")   "),line_wrap
       ENDFOR
      ENDIF
      wrapcol = 120, xcolvar = xcol1
      FOR (a5 = 1 TO dlrec->seq[i].section2_total)
       tempstring = concat("{b}",dlrec->seq[i].section2_events[a5].event_cd_disp,"{endb} ",dlrec->
        seq[i].section2_events[a5].result),line_wrap
      ENDFOR
      title_string = "Nurse Communication:", title_print, last_title = title_string,
      rowplusone, xcolvar = xcol1
      FOR (a6 = 1 TO dlrec->seq[i].section3_total)
       tempstring = concat("{b}",dlrec->seq[i].section3_events[a6].event_cd_disp,"{endb} ",dlrec->
        seq[i].section3_events[a6].result),line_wrap
      ENDFOR
      xcolvar = xcol1
      FOR (a7 = 1 TO dlrec->seq[i].orders35_total)
        IF ((dlrec->seq[i].orders35[a7].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].orders35[a7].order_mnemonic,"{endb} ",dlrec->seq[i].
          orders35[a7].clinical_display_line,"  --  COMMENT --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].orders35[a7].order_mnemonic,"{endb} ",dlrec->seq[i].
          orders35[a7].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      title_string = "Discharge Communication:", title_print, last_title = title_string,
      rowplusone, xcolvar = xcol1
      FOR (a8 = 1 TO dlrec->seq[i].section4_total)
       tempstring = concat("{b}",dlrec->seq[i].section4_events[a8].event_cd_disp,"{endb} ",dlrec->
        seq[i].section4_events[a8].result),line_wrap
      ENDFOR
      title_string = "Patient Care Orders:", title_print, last_title = title_string,
      rowplusone,
      CALL print(calcpos(xcol3,yrow1)), "{b}{u}  Diet  {endu}{endb}",
      rowplusone, xcolvar = xcol1, last_title = "Patient Care Orders: Diet"
      FOR (a9 = 1 TO dlrec->seq[i].diet_total)
        IF ((dlrec->seq[i].diet[a9].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].diet[a9].order_mnemonic,"{endb} ",dlrec->seq[i].
          diet[a9].clinical_display_line,"  -- COMMENT --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].diet[a9].order_mnemonic,"{endb} ",dlrec->seq[i].
          diet[a9].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      CALL print(calcpos(xcol3,yrow1)), "{b}{u}  Respiratory Therapy  {endu}{endb}", last_title =
      "Patient Care Orders: Respiratory Therapy",
      rowplusone, xcolvar = xcol1
      FOR (aa = 1 TO dlrec->seq[i].resp_total)
        IF ((dlrec->seq[i].resp[aa].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].resp[aa].order_mnemonic,"{endb} ",dlrec->seq[i].
          resp[aa].clinical_display_line,"  -- COMMENT --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].resp[aa].order_mnemonic,"{endb} ",dlrec->seq[i].
          resp[aa].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      ...
      rowplusone, xcolvar = xcol1
      FOR (ab = 1 TO dlrec->seq[i].asmt_total)
        IF ((dlrec->seq[i].asmt[ab].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].asmt[ab].order_mnemonic,"{endb} ",dlrec->seq[i].
          asmt[ab].clinical_display_line,"  --  COMMENT  --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].asmt[ab].order_mnemonic,"{endb} ",dlrec->seq[i].
          asmt[ab].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      rowplusone,
      CALL print(calcpos(xcol3,yrow1)), "{b}{u}  Meds/Invasive Lines/Tubes/Drains  {endu}{endb}",
      last_title = "Patient Care Orders: Meds/Invasive Lines/Tubes/Drains", rowplusone, xcolvar =
      xcol1
      FOR (ac = 1 TO dlrec->seq[i].ltd_total)
        IF ((dlrec->seq[i].ltd[ac].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].ltd[ac].order_mnemonic,"{endb} ",dlrec->seq[i].ltd[
          ac].clinical_display_line,"  --  COMMENT  --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].ltd[ac].order_mnemonic,"{endb} ",dlrec->seq[i].ltd[
          ac].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      ...
      FOR (ad = 1 TO dlrec->seq[i].number_of_meds)
        IF (((yrow1+ 36) > 650))
         yrow1 = 651, rowplusone
        ENDIF
        IF (((ad=1) OR (ad > 1
         AND (dlrec->seq[i].meds[ad].mso > dlrec->seq[i].meds[(ad - 1)].mso))) )
         CALL print(calcpos((xcol1+ 27),yrow1)), "{b}{color/20}", mso[dlrec->seq[i].meds[ad].mso],
         "{endb}{color/0}", rowplusone
        ENDIF
        IF ((dlrec->seq[i].meds[ad].ioi=1))
         "{color/20}"
        ENDIF
        CALL print(calcpos((xcol1+ 7),yrow1)), "{b}", dlrec->seq[i].meds[ad].mnemonic,
        "{endb} ", "     ", dlrec->seq[i].meds[ad].order_status_disp,
        "{endb}", "   ", dlrec->seq[i].meds[ad].need_rx_verify_str
        IF ((dlrec->seq[i].meds[ad].order_comment_ind=1))
         CALL print("  --  COMMENT  --  ")
        ENDIF
        breakflag = 0, rowplusone2, tempstring = dlrec->seq[i].meds[ad].display_line,
        xcolvar = xcol2, line_wrap, "{color/0}"
        IF ((dlrec->seq[i].meds[ad].mso=1))
         breakflag = 1
        ENDIF
      ENDFOR
      ...
      rowplusone, xcolvar = xcol1
      FOR (ae = 1 TO dlrec->seq[i].rorders_total)
        IF ((dlrec->seq[i].rorders[ae].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].rorders[ae].order_mnemonic,"{endb} ",dlrec->seq[i].
          rorders[ae].clinical_display_line,"  --  COMMENT  --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].rorders[ae].order_mnemonic,"{endb} ",dlrec->seq[i].
          rorders[ae].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      rowplusone,
      ...
      FOR (af = 1 TO dlrec->seq[i].cporders_total)
        IF ((dlrec->seq[i].cporders[af].order_comment_ind=1))
         tempstring = concat("{b}",dlrec->seq[i].cporders[af].order_mnemonic,"{endb} ",dlrec->seq[i].
          cporders[af].clinical_display_line,"  --  COMMENT  --  "), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].cporders[af].order_mnemonic,"{endb} ",dlrec->seq[i].
          cporders[af].clinical_display_line), line_wrap
        ENDIF
      ENDFOR
      rowplusone, title_string = "Documentation:", last_header1_txt = fillstring(60," "),
      last_header2_txt = fillstring(60," "), last_parent_event = fillstring(60," "), xcolvar = (xcol1
      + 10)
      FOR (ag = 1 TO dlrec->seq[i].d0events_total)
        IF ((dlrec->seq[i].d0events[ag].header1_txt != last_header1_txt))
         last_header1_txt = dlrec->seq[i].d0events[ag].header1_txt, title_string = dlrec->seq[i].
         d0events[ag].header1_txt, last_title = title_string,
         title_print, rowplusone
        ENDIF
        IF ((dlrec->seq[i].d0events[ag].header2_txt != last_header2_txt))
         last_header2_txt = dlrec->seq[i].d0events[ag].header2_txt
         IF ((ag=dlrec->seq[i].d0events_total)
          AND yrow1 > 500)
          yrow1 = (yrow1+ 10), row + 1
          IF (yrow1 > 500)
           BREAK
         ...
         ELSE
          rowplusone
         ENDIF
         CALL print(calcpos(xcol1,yrow1)), "{b}{u} ", last_header2_txt,
         " {endu}{endb}", last_title = concat("Documentation: ",last_header2_txt), rowplusone
        ENDIF
        IF ((dlrec->seq[i].d0events[ag].parent_event > " "))
         IF ((dlrec->seq[i].d0events[ag].parent_event != last_parent_event))
          last_parent_event = dlrec->seq[i].d0events[ag].parent_event
          IF ((dlrec->seq[i].d0events[ag].parent_event != dlrec->seq[i].d0events[ag].event_cd_disp))
          ...
          ENDIF
         ENDIF
         tempstring = concat("{b}   ",dlrec->seq[i].d0events[ag].event_cd_disp,"{endb} ",dlrec->seq[i
          ].d0events[ag].result,"   (",
          dlrec->seq[i].d0events[ag].event_dt_tm,")"), line_wrap
        ELSE
         tempstring = concat("{b}",dlrec->seq[i].d0events[ag].event_cd_disp,"{endb} ",dlrec->seq[i].
          d0events[ag].result,"   (",
          dlrec->seq[i].d0events[ag].event_dt_tm,")"), line_wrap
        ENDIF
      ENDFOR
      lastflag = 1, rowplusone,
      CALL print(calcpos(xcol5,yrow1)),
      "** End of Clinical Summary **", row + 1
    ENDFOR
    lastflag = 1, rowplusone, rowplusone,
    rowplusone, rowplusone, rowplusone,
    CALL print(calcpos((xcol5+ 20),yrow1)), "** End of Report **", row + 1,
    yrow1 = 650
    FOR (i3 = 1 TO 10)
      CALL print(calcpos(xcol1,(yrow1+ i3))), "{color/12}", equal_line,
      "{color/0}", row + 1
    ENDFOR
