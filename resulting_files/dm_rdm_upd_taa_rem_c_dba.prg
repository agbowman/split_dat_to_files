CREATE PROGRAM dm_rdm_upd_taa_rem_c:dba
 ...
 DECLARE notification_type_cd = f8 WITH public, noconstant(0.0)
 DECLARE task_type_cd_reminder = f8 WITH public, noconstant(0.0)
 SET ms_child_info_domain_nm = "DM_RDM_UPD_TAA_REM2"
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 CALL echo(build("notification_type_cd-->",notification_type_cd))
 ...
 IF (((error(ms_errmsg,0) != 0) OR (task_type_cd_reminder <= 0)) )
  SET readme_data->status = "F"
  SET readme_data->message = concat("Error loading code value REMINDER from codeset 3406: ",ms_errmsg
   )
  GO TO exit_program
 ENDIF
