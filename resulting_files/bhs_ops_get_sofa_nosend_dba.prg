CREATE PROGRAM bhs_ops_get_sofa_nosend:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  HEAD e.encntr_id
   ...
   CALL echo(build2("unit: ",uar_get_code_display(e.loc_nurse_unit_cd))),
   CALL echo(build2("age days: ",datetimediff(sysdate,p.birth_dt_tm))),
   CALL echo(build2(uar_get_code_display(o.catalog_cd)," ",uar_get_code_display(o.catalog_type_cd)))
 ...
 SELECT INTO value( $OUTDEV)
  name = p.name_full_formatted, fin = ea.alias, person_id = m_rec->pat[d.seq].f_encntr_id,
  encntr_id = m_rec->pat[d.seq].f_person_id
