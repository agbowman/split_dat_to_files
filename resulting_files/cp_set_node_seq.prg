CREATE PROGRAM cp_set_node_seq
 PROMPT
  "Output to File/Printer/MINE" = "MINE"
  WITH outdev
 FREE RECORD node_seq
 RECORD node_seq(
   1 nodes[*]
     2 cp_node_id = f8
     2 node_name = vc
 ...
 IF (validate(node_seq_reply) != 1)
  RECORD node_seq_reply(
    1 status_data
      2 status = c1
      2 subeventstatus[1]
        3 operationname = c25
 ...
 SUBROUTINE (setauditinitreplystatusbyprogramreplystatus(reply=vc(ref)) =null WITH protect)
  ...
  ENDIF
 END ;Subroutine
 DECLARE getnodedetails(null) = null WITH protect
 DECLARE updatenodes(null) = null WITH protect
 ...
 CALL getnodedetails(null)
 CALL updatenodes(null)
 SUBROUTINE getnodedetails(null)
   CALL log_message("In getNodeDetails()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE jrec = i4
   CALL echo(request->blob_in)
   IF (validate(request->blob_in))
    IF ((request->blob_in > " "))
     CALL logblobindata("CP_SET_NODE_SEQ",request->blob_in, $OUTDEV)
     SET jrec = cnvtjsontorec(request->blob_in)
    ENDIF
   ENDIF
   CALL echorecord(node_seq)
   CALL log_message(build("Exit getNodeDetails(),Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE updatenodes(null)
   CALL log_message("In updateNodes()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE len = i4 WITH noconstant(0), protect
   SET len = size(node_seq->nodes,5)
   IF (len > 0)
    UPDATE  FROM cp_node cn,
      (dummyt d  WITH seq = value(len))
     SET cn.collation_seq = node_seq->nodes[d.seq].sequence, cn.updt_cnt = (cn.updt_cnt+ 1), cn
      .updt_dt_tm = cnvtdatetime(sysdate),
      cn.updt_applctx = reqinfo->updt_applctx, cn.updt_task = reqinfo->updt_task
     PLAN (d)
      JOIN (cn
      WHERE (cn.cp_node_id=node_seq->nodes[d.seq].cp_node_id))
     WITH nocounter
    ;end update
    CALL error_and_zero_check_rec(curqual,"Update cp_set_node_seq","Could not update collation_seq",1,
     1,
     node_seq_reply)
    SET node_seq_reply->status_data.status = "S"
   ELSE
    SET node_seq_reply->status_data.status = "F"
   ENDIF
   CALL log_message(build("Exit updateNodes(),Elapsed time in seconds:",((curtime3 - begin_curtime3)
     / 100.0)),log_level_debug)
 END ;Subroutine
#exit_script
 CALL setauditinitreplystatusbyprogramreplystatus(node_seq_reply)
 IF ((node_seq_reply->status_data.status="S"))
  SET reqinfo->commit_ind = 1
 ELSE
  SET reqinfo->commit_ind = 0
 ENDIF
 CALL echo(build("Commit_ind 1 - ",reqinfo->commit_ind))
 CALL putjsonrecordtofile(node_seq_reply)
