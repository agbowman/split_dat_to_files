CREATE PROGRAM dm2_ref_data_mover_1153
 DECLARE cust_catalog_cd_s = f8
 DECLARE cust_catalog_cd = f8
 DECLARE comp_seq_cnt = i4
 DECLARE cust_tab_name_s = vc
 DECLARE cust_cs_loop = i4
 DECLARE cust_col_cnt = i4
 DECLARE cust_col_num = i4
 DECLARE cust_check_loop = i4
 DECLARE cust_nodelete_ind = i2
 DECLARE cust_chg_log = vc
 DECLARE cust_merge_loop = i4
 DECLARE cust_index_var = i4
 DECLARE cust_cur_merges = i4
 DECLARE cust_fail_merges = i4
 DECLARE cust_del_msg = vc
 DECLARE cust_sel_ret = vc
 DECLARE cust_where_clause = vc
 DECLARE cust_parser_cnt = i4
 DECLARE cust_col_name = vc
 DECLARE passed_var = vc
 DECLARE drdm_dmt_tab = vc
 DECLARE cust_loop = i4
 DECLARE cust_from_val = f8
 DECLARE cust_to_val = f8
 DECLARE cust_from_str = vc
 DECLARE chg_log_smry_name = vc
 DECLARE cust_eq_cnt = i4
 DECLARE cust_sp_cnt = i4
 DECLARE except_tab = vc
 DECLARE except_log_type = vc
 DECLARE rpt_table = vc
 DECLARE rpt_column = vc
 DECLARE rpt_from = f8
 DECLARE vc_from_val = vc
 DECLARE orphan_ind = i2
 DECLARE err_msg = vc
 DECLARE err_ind = i2
 DECLARE dd_col_pos = i4
 DECLARE dd_col_loop = i4
 DECLARE dd_data_type = vc
 DECLARE pe_name = vc
 DECLARE pe_srch = i4
 DECLARE pe_fnd = i4
 DECLARE drdm_parent_col = vc
 DECLARE dd_i_domain = vc
 DECLARE dd_i_name = vc
 DECLARE cust_master_dt = f8
 DECLARE cust_phase_ind = i2
 DECLARE cust_tm_error_ind = i2
 DECLARE input_col_num = i4
 DECLARE to_input_ref = vc
 DECLARE index_var = i4
 DECLARE cdf_mean_val = vc
 DECLARE tab_name_link = vc
 DECLARE src_tabl_name = vc
 DECLARE pe_tab_name = vc
 DECLARE cust_more_rows = i2
 DECLARE cust_mend_dt = f8
 ...
 SUBROUTINE dm2_rdds_get_tgt_id(s_gmti_tgt_rs)
   ...
   RETURN(drgmti_return_val)
 END ;Subroutine
 DECLARE cs_insert(null) = null
 DECLARE cs_merge_audit(action=vc,text=vc) = null
 DECLARE cs_find_p_e_col(cs_p_e_name=vc,cs_p_e_col=i4) = vc
 SET perm_col_cnt = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt
 SET cust_master_dt = cnvtdatetime(curdate,000000)
 SET cust_mend_dt = datetimeadd(cnvtdatetime(curdate,235959),- (1))
 SET cust_tab_name_s = dm2_get_rdds_tname(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name)
 SET cust_more_rows = 0
 SET cust_phase_ind = 0
 CALL parser('select into "NL:"',0)
 CALL parser(concat("from ",cust_tab_name_s," t1153"),0)
 CALL parser(drdm_chg->log[drdm_log_loop].pk_where,0)
 CALL parser(" and beg_effective_dt_tm > cnvtdatetime(cust_master_dt) and active_ind = 1 ",0)
 CALL parser(" detail",0)
 CALL parser("    cust_more_rows = 1",0)
 CALL parser(" with nocounter go",1)
#phase
 IF (cust_phase_ind=0)
  CALL parser('select into "NL:"',0)
  CALL parser(concat("from ",cust_tab_name_s," t1153"),0)
  CALL parser(drdm_chg->log[drdm_log_loop].pk_where,0)
  CALL parser(" and beg_effective_dt_tm <= cnvtdatetime(cust_master_dt) and ",0)
  CALL parser(" end_effective_dt_tm >= cnvtdatetime(cust_master_dt) and active_ind = 1",0)
  CALL parser(" head report",0)
  CALL parser("    comp_seq_cnt = 0",0)
  CALL parser("    stat = alterlist(cust_cs_rows->qual, 10)",0)
  CALL parser(" detail",0)
  CALL parser("    comp_seq_cnt = comp_seq_cnt + 1",0)
  CALL parser("    if (mod(comp_seq_cnt, 10) = 1 and comp_seq_cnt != 1)",0)
  CALL parser("       stat = alterlist(cust_cs_rows->qual, comp_seq_cnt + 9)",0)
  CALL parser("    endif",0)
  CALL parser("    cust_cs_rows->qual[comp_seq_cnt].tier_cell_id = t1153.tier_cell_id",0)
 ...
 ENDIF
 IF (curqual=0
  AND cust_phase_ind=0)
  SET drdm_mini_loop_status = "NO SRC"
  GO TO exit_1153
 ENDIF
 FOR (cust_cs_loop = 1 TO comp_seq_cnt)
   SELECT INTO "nl:"
    var1 = nullind(t1153.active_ind), var2 = nullind(t1153.active_status_cd), var3 = nullind(t1153
     .active_status_dt_tm),
    var4 = nullind(t1153.active_status_prsnl_id), var5 = nullind(t1153.beg_effective_dt_tm), var6 =
    nullind(t1153.end_effective_dt_tm),
    var7 = nullind(t1153.tier_cell_entity_name), var8 = nullind(t1153.tier_cell_id), var9 = nullind(
     t1153.tier_cell_string),
    var10 = nullind(t1153.tier_cell_type_cd), var11 = nullind(t1153.tier_cell_value), var12 = nullind
    (t1153.tier_cell_value_id),
    var13 = nullind(t1153.tier_col_num), var14 = nullind(t1153.tier_group_cd), var15 = nullind(t1153
     .tier_row_num),
    var16 = nullind(t1153.updt_applctx), var17 = nullind(t1153.updt_cnt), var18 = nullind(t1153
     .updt_dt_tm),
    var19 = nullind(t1153.updt_id), var20 = nullind(t1153.updt_task)
    FROM (parser(cust_tab_name_s) t1153)
    WHERE (t1153.tier_cell_id=cust_cs_rows->qual[cust_cs_loop].tier_cell_id)
    DETAIL
     rs_1153->from_values.active_ind = t1153.active_ind, rs_1153->to_values.active_ind = t1153
     .active_ind, cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"ACTIVE_IND",
      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].column_name),
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].check_null = var1,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.active_status_cd = t1153.active_status_cd,
     cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"ACTIVE_STATUS_CD",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[cust_col_num].check_null = var2, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt]
     .col_qual[cust_col_num].translated = 0,
     rs_1153->from_values.active_status_dt_tm = t1153.active_status_dt_tm, rs_1153->to_values.
     active_status_dt_tm = t1153.active_status_dt_tm, cust_col_num = locateval(cust_index_var,1,
      perm_col_cnt,"ACTIVE_STATUS_DT_TM",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
      cust_index_var].column_name),
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].check_null = var3,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.active_status_prsnl_id = t1153.active_status_prsnl_id,
     cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"ACTIVE_STATUS_PRSNL_ID",dm2_ref_data_doc
      ->tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[cust_col_num].check_null = var4, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt]
     .col_qual[cust_col_num].translated = 0,
     rs_1153->from_values.beg_effective_dt_tm = t1153.beg_effective_dt_tm, rs_1153->to_values.
     beg_effective_dt_tm = t1153.beg_effective_dt_tm, cust_col_num = locateval(cust_index_var,1,
      perm_col_cnt,"BEG_EFFECTIVE_DT_TM",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
      cust_index_var].column_name),
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].check_null = var5,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.end_effective_dt_tm = t1153.end_effective_dt_tm,
     rs_1153->to_values.end_effective_dt_tm = t1153.end_effective_dt_tm, cust_col_num = locateval(
      cust_index_var,1,perm_col_cnt,"END_EFFECTIVE_DT_TM",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
      col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     cust_col_num].check_null = var6,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.tier_cell_entity_name = t1153.tier_cell_entity_name, rs_1153->to_values.
     tier_cell_entity_name = t1153.tier_cell_entity_name,
     cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"TIER_CELL_ENTITY_NAME",dm2_ref_data_doc
      ->tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[cust_col_num].check_null = var7, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt]
     .col_qual[cust_col_num].translated = 0,
     rs_1153->from_values.tier_cell_id = t1153.tier_cell_id, cust_col_num = locateval(cust_index_var,
      1,perm_col_cnt,"TIER_CELL_ID",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_index_var]
      .column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].check_null =
     var8,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.tier_cell_string = t1153.tier_cell_string, rs_1153->to_values.tier_cell_string =
     t1153.tier_cell_string,
     cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"TIER_CELL_STRING",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[cust_col_num].check_null = var9, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt]
     .col_qual[cust_col_num].translated = 0,
     rs_1153->from_values.tier_cell_type_cd = t1153.tier_cell_type_cd, cust_col_num = locateval(
      cust_index_var,1,perm_col_cnt,"TIER_CELL_TYPE_CD",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
      col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     cust_col_num].check_null = var10,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.tier_cell_value = t1153.tier_cell_value, rs_1153->to_values.tier_cell_value = t1153
     .tier_cell_value,
     cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"TIER_CELL_VALUE",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[cust_col_num].check_null = var11, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt
     ].col_qual[cust_col_num].translated = 0,
     rs_1153->from_values.tier_cell_value_id = t1153.tier_cell_value_id, cust_col_num = locateval(
      cust_index_var,1,perm_col_cnt,"TIER_CELL_VALUE_ID",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
      col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     cust_col_num].check_null = var12,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.tier_col_num = t1153.tier_col_num, rs_1153->to_values.tier_col_num = t1153
     .tier_col_num,
     cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"TIER_COL_NUM",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[cust_col_num].check_null = var13, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt
     ].col_qual[cust_col_num].translated = 0,
     rs_1153->from_values.tier_group_cd = t1153.tier_group_cd, cust_col_num = locateval(
      cust_index_var,1,perm_col_cnt,"TIER_GROUP_CD",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
      col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     cust_col_num].check_null = var14,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.tier_row_num = t1153.tier_row_num, rs_1153->to_values.tier_row_num = t1153
     .tier_row_num,
     cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"TIER_ROW_NUM",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[cust_col_num].check_null = var15, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt
     ].col_qual[cust_col_num].translated = 0,
     rs_1153->from_values.updt_applctx = t1153.updt_applctx, rs_1153->to_values.updt_applctx = t1153
     .updt_applctx, cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"UPDT_APPLCTX",
      dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].column_name),
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].check_null = var16,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.updt_cnt = t1153.updt_cnt,
     rs_1153->to_values.updt_cnt = t1153.updt_cnt, cust_col_num = locateval(cust_index_var,1,
      perm_col_cnt,"UPDT_CNT",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].
      column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].check_null =
     var17,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.updt_dt_tm = t1153.updt_dt_tm, rs_1153->to_values.updt_dt_tm = t1153.updt_dt_tm,
     cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"UPDT_DT_TM",dm2_ref_data_doc->tbl_qual[
      temp_tbl_cnt].col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
     col_qual[cust_col_num].check_null = var18, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     cust_col_num].translated = 0,
     rs_1153->from_values.updt_id = t1153.updt_id, cust_col_num = locateval(cust_index_var,1,
      perm_col_cnt,"UPDT_ID",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_index_var].
      column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].check_null =
     var19,
     dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_num].translated = 0, rs_1153->
     from_values.updt_task = t1153.updt_task, rs_1153->to_values.updt_task = t1153.updt_task,
     cust_col_num = locateval(cust_index_var,1,perm_col_cnt,"UPDT_TASK",dm2_ref_data_doc->tbl_qual[
      temp_tbl_cnt].col_qual[cust_index_var].column_name), dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
     col_qual[cust_col_num].check_null = var20, dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
     cust_col_num].translated = 0
    WITH nocounter
   ;end select
   IF (error(err_msg,0) > 0)
    CALL echo(err_msg)
    SET cust_cs_loop = comp_seq_cnt
    SET cust_tm_error_ind = 1
   ENDIF
   IF (curqual > 0)
    SET rs_1153->log_id = drdm_chg->log[drdm_log_loop].log_id
    CALL echo("")
    CALL echo("")
    ...
    CALL echo("")
    CALL echo("")
    SET rs_1153->from_table_name = "TIER_MATRIX"
    SET rs_1153->to_table_name = "TIER_MATRIX"
    SET no_insert_update = 0
    FOR (cust_col_loop = 1 TO perm_col_cnt)
      IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_loop].idcd_ind=0))
       SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_col_loop].translated = 1
      ENDIF
    ENDFOR
    IF (cust_phase_ind=0)
     SET rs_1153->to_values.beg_effective_dt_tm = cust_master_dt
    ENDIF
    SET input_col_num = locateval(index_var,1,perm_col_cnt,"UPDT_ID",dm2_ref_data_doc->tbl_qual[
     temp_tbl_cnt].col_qual[index_var].column_name)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
    SET rs_1153->to_values.updt_id = 0
    SET input_col_num = locateval(index_var,1,perm_col_cnt,"ACTIVE_STATUS_PRSNL_ID",dm2_ref_data_doc
     ->tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
    SET rs_1153->to_values.updt_id = 0
    IF ((rs_1153->from_values.active_status_cd != 0))
     SET found_rec = 0
     SET to_input_ref = select_merge_translate(cnvtstring(rs_1153->from_values.active_status_cd),
      "CODE_VALUE")
     IF (to_input_ref != "No Trans")
      SET rs_1153->to_values.active_status_cd = cnvtreal(to_input_ref)
      SET input_col_num = locateval(index_var,1,perm_col_cnt,"ACTIVE_STATUS_CD",dm2_ref_data_doc->
       tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
     ENDIF
    ELSE
     SET rs_1153->to_values.active_status_cd = 0
     SET input_col_num = locateval(index_var,1,perm_col_cnt,"ACTIVE_STATUS_CD",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
    ENDIF
    IF ((rs_1153->from_values.tier_cell_type_cd != 0))
     SET found_rec = 0
     SET to_input_ref = select_merge_translate(cnvtstring(rs_1153->from_values.tier_cell_type_cd),
      "CODE_VALUE")
     IF (to_input_ref != "No Trans")
      SET rs_1153->to_values.tier_cell_type_cd = cnvtreal(to_input_ref)
      SET input_col_num = locateval(index_var,1,perm_col_cnt,"TIER_CELL_TYPE_CD",dm2_ref_data_doc->
       tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
     ENDIF
    ELSE
     SET rs_1153->to_values.tier_cell_type_cd = 0
     SET input_col_num = locateval(index_var,1,perm_col_cnt,"TIER_CELL_TYPE_CD",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
    ENDIF
    IF ((rs_1153->from_values.tier_group_cd != 0))
     SET found_rec = 0
     SET to_input_ref = select_merge_translate(cnvtstring(rs_1153->from_values.tier_group_cd),
      "CODE_VALUE")
     IF (to_input_ref != "No Trans")
      SET rs_1153->to_values.tier_group_cd = cnvtreal(to_input_ref)
      SET input_col_num = locateval(index_var,1,perm_col_cnt,"TIER_GROUP_CD",dm2_ref_data_doc->
       tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
     ENDIF
    ELSE
     SET rs_1153->to_values.tier_group_cd = 0
     SET input_col_num = locateval(index_var,1,perm_col_cnt,"TIER_GROUP_CD",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
    ENDIF
    SET tab_name_link = dm2_get_rdds_tname("CODE_VALUE")
    SET src_tabl_name = dm2_get_rdds_tname("TIER_MATRIX")
    SET input_col_num = locateval(index_var,1,perm_col_cnt,"TIER_CELL_VALUE_ID",dm2_ref_data_doc->
     tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
    SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 0
    SET input_col_num = locateval(index_var,1,perm_col_cnt,"TIER_CELL_TYPE_CD",dm2_ref_data_doc->
     tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
    IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated=0))
     ROLLBACK
     SET err_msg = "The TIER_CELL_TYPE_CD column was not translated"
     SET cust_tm_error_ind = 1
     SET cust_cs_loop = comp_seq_cnt
    ELSE
     SELECT INTO "NL:"
      FROM (parser(tab_name_link) c)
      WHERE (c.code_value=rs_1153->to_values.tier_cell_type_cd)
       AND c.code_set=13036
      DETAIL
       cdf_mean_val = c.cdf_meaning
      WITH nocounter
     ;end select
     CASE (cdf_mean_val)
      OF "HEALTHPLAN":
       SET pe_tab_name = "HEALTH_PLAN"
      OF "INTERFACE":
       SET pe_tab_name = "INTERFACE_FILE"
      OF "ORG":
       SET pe_tab_name = "ORGANIZATION"
      OF "PRICESCHED":
       SET pe_tab_name = "PRICE_SCHED"
      OF "ADD ON":
       SET pe_tab_name = "BILL_ITEM"
      OF "LPRICESCHED":
       SET pe_tab_name = "PRICE_SCHED"
      ELSE
       SET pe_tab_name = "CODE_VALUE"
     ENDCASE
     IF ((rs_1153->from_values.tier_cell_value_id != 0))
      SET to_input_ref = select_merge_translate(cnvtstring(rs_1153->from_values.tier_cell_value_id),
       pe_tab_name)
      IF (to_input_ref != "No Trans")
       SET rs_1153->to_values.tier_cell_value_id = cnvtreal(to_input_ref)
       SET input_col_num = locateval(index_var,1,perm_col_cnt,"TIER_CELL_VALUE_ID",dm2_ref_data_doc->
        tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
       SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
      ENDIF
     ELSE
      SET rs_1153->to_values.tier_cell_value_id = 0
      SET input_col_num = locateval(index_var,1,perm_col_cnt,"TIER_CELL_VALUE_ID",dm2_ref_data_doc->
       tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
     ENDIF
     SELECT INTO "NL:"
      y = seq(price_sched_seq,nextval)
      FROM dual
      DETAIL
       rs_1153->to_values.tier_cell_id = y
      WITH nocounter
     ;end select
     IF (error(err_msg,0) > 0)
      CALL echo(err_msg)
      ...
      SET cust_cs_loop = comp_seq_cnt
     ENDIF
     SET input_col_num = locateval(index_var,1,perm_col_cnt,"TIER_CELL_ID",dm2_ref_data_doc->
      tbl_qual[temp_tbl_cnt].col_qual[index_var].column_name)
     SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[input_col_num].translated = 1
    ENDIF
    IF (cust_tm_error_ind=0)
     ...
     CALL echo("")
     CALL echo("")
     FOR (cust_check_loop = 1 TO perm_col_cnt)
       IF (value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].idcd_ind)=1)
        IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].translated=0))
         ...
         ""))
          SET rpt_table = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].
          root_entity_name
          SET rpt_column = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].
          root_entity_attr
          CALL parser(concat("set rpt_from = RS_1153->from_values.",dm2_ref_data_doc->tbl_qual[
            temp_tbl_cnt].col_qual[cust_check_loop].column_name," go"),1)
         ENDIF
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].parent_entity_col
          ...
          CALL parser(concat("set rpt_from = RS_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,
            "->from_values.",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].
            column_name," go"),1)
          SET rpt_col_value = value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop
           ].parent_entity_col)
          ...
          IF (rpt_col_pos > 0)
           ...
           IF (rpt_data_type IN ("VC", "C*"))
            ...
            IF (rpt_fnd != 0)
             IF (pe_tab_name="")
              SET rpt_parent_col = "CODE_VALUE"
             ELSE
              SET rpt_parent_col = pe_tab_name
             ENDIF
             IF (rpt_parent_col != ""
              AND rpt_parent_col != " ")
             ...
             ELSE
              ...
              SET rpt_i_name = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[
               cust_check_loop].column_name,":",rpt_parent_col)
              SELECT INTO "NL:"
               FROM dm_info d
         ...
         IF (rpt_table != ""
          AND rpt_from != 0)
          SET except_tab = dm2_get_rdds_tname("DM_CHG_LOG_EXCEPTION")
          IF (rpt_table IN ("DCP_SECTION_REF", "DCP_FORMS_REF"))
           SELECT INTO "NL:"
            FROM (parser(except_tab) d)
            WHERE (d.target_env_id=dm2_ref_data_doc->env_target_id)
             AND d.table_name=rpt_table
             AND d.from_value=rpt_from
            DETAIL
             except_log_type = d.log_type, rpt_column = d.column_name
            WITH nocounter
           ;end select
          ELSE
           SELECT INTO "NL:"
          ...
          IF (curqual=0)
           ROLLBACK
           INSERT  FROM (parser(except_tab) d)
            SET d.log_type = "NOXLAT", d.table_name = rpt_table, d.column_name = rpt_column,
             d.from_value = rpt_from, d.target_env_id = dm2_ref_data_doc->env_target_id
            WITH nocounter
           ;end insert
           IF (error(err_msg,0) > 0)
            CALL echo(err_msg)
         ...
         SET idcd_check = 1
         SET drdm_no_trans_ind = 1
         SET err_msg = concat("This log_id ","was not translated because of the ",dm2_ref_data_doc->
          tbl_qual[temp_tbl_cnt].col_qual[cust_check_loop].column_name," column.")
         CALL echo("")
         CALL echo("")
         CALL echo(err_msg)
         CALL echo("")
         CALL echo("")
         SET cust_fail_merges = (cust_fail_merges+ 1)
         SET fail_merge_audit->num[cust_fail_merges].action = "FAILREASON"
         SET fail_merge_audit->num[cust_fail_merges].text = err_msg
         ROLLBACK
         SET cust_cs_loop = comp_seq_cnt
         SET cust_tm_error_ind = 1
         CALL cs_merge_audit(fail_merge_audit->num[cust_fail_merges].action,fail_merge_audit->num[
          cust_fail_merges].text)
         IF (drdm_error_out_ind=1)
          ROLLBACK
     ...
     IF (idcd_check=0)
      IF (cust_cs_loop=1)
       IF (cust_phase_ind=0)
        UPDATE  FROM tier_matrix c
         SET c.end_effective_dt_tm = cnvtdatetime(cust_mend_dt)
         WHERE (c.tier_group_cd=rs_1153->to_values.tier_group_cd)
          AND c.active_ind=1
          AND c.beg_effective_dt_tm <= cnvtdatetime(cust_master_dt)
          AND c.end_effective_dt_tm > cnvtdatetime(cust_master_dt)
         WITH nocounter
        ;end update
       ELSE
        UPDATE  FROM tier_matrix c
      ...
      IF (cust_nodelete_ind=0)
       IF (cust_cs_loop=comp_seq_cnt)
        IF (cust_more_rows=cust_phase_ind)
         SET update_cnt = 0
         SET cust_chg_log = dm2_get_rdds_tname("DM_CHG_LOG")
         SELECT INTO "nl:"
          FROM (parser(cust_chg_log) dc)
         ...
         ELSE
          SET cust_del_msg = concat("Could not delete log_id ",trim(cnvtstring(drdm_chg->log[
             drdm_log_loop].log_id)),
           " because it has been updated since the mover picked it up. It will be merged next pass.")
          CALL echo("")
          CALL echo("")
          CALL echo(cust_del_msg)
          CALL echo("")
          CALL echo("")
          SET cust_fail_merges = (cust_fail_merges+ 1)
          SET fail_merge_audit->num[cust_fail_merges].action = "FAILREASON"
          SET fail_merge_audit->num[cust_fail_merges].text = cust_del_msg
          ROLLBACK
          CALL cs_merge_audit(fail_merge_audit->num[cust_fail_merges].action,fail_merge_audit->num[
           cust_fail_merges].text)
          IF (drdm_error_out_ind=1)
           ROLLBACK
           SET cust_cs_loop = (comp_seq_cnt+ 1)
           SET cust_tm_error_ind = 1
          ELSE
           COMMIT
           SET cust_cur_merges = 0
          ENDIF
         ENDIF
        ENDIF
       ENDIF
       FOR (cust_merge_loop = 1 TO cust_cur_merges)
        CALL cs_merge_audit(child_merge_audit->num[cust_merge_loop].action,child_merge_audit->num[
         cust_merge_loop].text)
        IF (drdm_error_out_ind=1)
         ROLLBACK
         SET cust_merge_loop = cust_cur_merges
        ENDIF
       ENDFOR
       IF (drdm_error_out_ind=1)
        SET cust_cs_loop = (comp_seq_cnt+ 1)
       ...
       ENDIF
      ELSE
       SET cust_del_msg = concat("Could not insert log_id ",cnvtstring(drdm_chg->log[drdm_log_loop].
         log_id))
       CALL echo("")
       CALL echo("")
       CALL echo(cust_del_msg)
    ...
    ENDIF
   ENDIF
   IF (cust_tm_error_ind=1)
    SET cust_cs_loop = comp_seq_cnt
   ENDIF
 ENDFOR
 IF (cust_phase_ind=0
  AND cust_tm_error_ind=0
  AND cust_more_rows=1)
  SET cust_phase_ind = 1
  GO TO phase
 ENDIF
 IF (cust_tm_error_ind=1)
  CALL cs_merge_audit("FAILREASON",err_msg)
 ENDIF
 SUBROUTINE cs_insert(null)
   SET rs_1153->to_values.updt_dt_tm = cnvtdatetime(curdate,curtime3)
   SET rs_1153->to_values.updt_cnt = 0
   SET rs_1153->to_values.updt_task = 4310001
   CALL echo("")
   CALL echo("")
   CALL echo("*******************INSERTING ROW******************")
   CALL echo("")
   CALL echo("")
   INSERT  FROM tier_matrix c
    SET c.active_ind = rs_1153->to_values.active_ind, c.active_status_cd = rs_1153->to_values.
     active_status_cd, c.active_status_dt_tm = cnvtdatetime(rs_1153->to_values.active_status_dt_tm),
     c.active_status_prsnl_id = rs_1153->to_values.active_status_prsnl_id, c.beg_effective_dt_tm =
     cnvtdatetime(rs_1153->to_values.beg_effective_dt_tm), c.end_effective_dt_tm = cnvtdatetime(
      rs_1153->to_values.end_effective_dt_tm),
     c.tier_cell_entity_name = rs_1153->to_values.tier_cell_entity_name, c.tier_cell_id = rs_1153->
     to_values.tier_cell_id, c.tier_cell_string = rs_1153->to_values.tier_cell_string,
     c.tier_cell_type_cd = rs_1153->to_values.tier_cell_type_cd, c.tier_cell_value = rs_1153->
     to_values.tier_cell_value, c.tier_cell_value_id = rs_1153->to_values.tier_cell_value_id,
     c.tier_col_num = rs_1153->to_values.tier_col_num, c.tier_group_cd = rs_1153->to_values.
     tier_group_cd, c.tier_row_num = rs_1153->to_values.tier_row_num,
     c.updt_applctx = rs_1153->to_values.updt_applctx, c.updt_cnt = rs_1153->to_values.updt_cnt, c
     .updt_dt_tm = cnvtdatetime(rs_1153->to_values.updt_dt_tm),
     c.updt_id = rs_1153->to_values.updt_id, c.updt_task = rs_1153->to_values.updt_task
    WITH nocounter
   ;end insert
   IF (error(err_msg,0) > 0)
    CALL echo(err_msg)
    SET cust_nodelete_ind = 1
    SET cust_tm_error_ind = 1
    SET err_ind = 0
   ENDIF
   SET cust_cur_merges = (cust_cur_merges+ 1)
   SET child_merge_audit->num[cust_cur_merges].action = "INSERT"
   SET child_merge_audit->num[cust_cur_merges].text = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].
   table_name
 END ;Subroutine
 SUBROUTINE cs_merge_audit(action,text)
   DECLARE ma_next_seq = f8
   DECLARE ma_del_ind = i2
   IF (drdm_log_level=1
   ...
   ELSE
    ...
    ELSE
     UPDATE  FROM dm_chg_log_audit dm
      SET dm.audit_dt_tm = cnvtdatetime(curdate,curtime3), dm.updt_dt_tm = cnvtdatetime(curdate,
        curtime3), dm.action = action,
       dm.text = text, dm.table_name = "TIER_MATRIX", dm.log_id = rs_1153->log_id
      WHERE dm.dm_chg_log_audit_id=ma_next_seq
      WITH nocounter
     ;end update
     IF (error(err_msg,0) > 0)
     ...
     IF (curqual=0)
      INSERT  FROM dm_chg_log_audit dm
       SET dm.audit_dt_tm = cnvtdatetime(curdate,curtime3), dm.updt_dt_tm = cnvtdatetime(curdate,
         curtime3), dm.action = action,
        dm.text = text, dm.table_name = "TIER_MATRIX", dm.log_id = rs_1153->log_id,
        dm.dm_chg_log_audit_id = ma_next_seq
       WITH nocounter
      ;end insert
      IF (error(err_msg,0) > 0)
       CALL echo(err_msg)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE cs_find_p_e_col(cs_p_e_name,cs_p_e_col)
   DECLARE p_e_name = vc
   DECLARE r_e_name = vc
   DECLARE p_e_col = vc
   DECLARE tbl_loop = i4
   ...
   DECLARE i_name = vc
   DECLARE tm_loop = i4
   DECLARE tm_dummy_cnt = i4
   DECLARE ptm_tbl_pos = i4
   DECLARE temp_r_e_name = vc
   SET p_e_name = "INVALIDTABLE"
   SET r_e_name = cs_p_e_name
   SET info_alias = ""
   SET id_ind = 0
   SET pk_num = 0
   SET pk_name = ""
   ...
   WHILE (p_e_name != r_e_name)
     ...
     IF (pk_pos=0)
      ...
      IF (curqual=0)
       SET i_domain = concat("RDDS_PE_ABBREV:",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name)
       SET i_name = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[cs_p_e_col].
        parent_entity_col,":",p_e_name)
       SELECT INTO "NL:"
        FROM dm_info d
        WHERE d.info_domain="RDDS_PE_ABBREVIATIONS"
     ...
     IF (pk_pos != 0)
      ...
      IF (p_e_name != "INVALIDTABLE")
       ...
       IF (p_e_tbl_pos != 0)
        ...
        ENDIF
       ELSE
        SET tm_dummy_cnt = temp_tbl_cnt
        SET ptm_tbl_pos = fill_rs("TABLE",p_e_name)
        SET temp_tbl_cnt = tm_dummy_cnt
        IF (ptm_tbl_pos=0)
         CALL echo("Information not found in column level metadata for parent_entity_col")
         SET p_e_name = "INVALIDTABLE"
         SET r_e_name = p_e_name
        ELSE
         SET temp_r_e_name = r_e_name
         FOR (tm_loop = 1 TO dm2_ref_data_doc->tbl_qual[ptm_tbl_pos].col_cnt)
           IF ((dm2_ref_data_doc->tbl_qual[ptm_tbl_pos].col_qual[tm_loop].column_name=p_e_col))
            SET r_e_name = dm2_ref_data_doc->tbl_qual[ptm_tbl_pos].col_qual[tm_pos].root_entity_name
           ENDIF
         ENDFOR
         IF (r_e_name=temp_r_e_name)
          CALL echo("Information not found in column level metadata for parent_entity_col")
          SET p_e_name = "INVALIDTABLE"
          SET r_e_name = p_e_name
         ENDIF
 ...
 END ;Subroutine
#exit_1153
 FREE SET cust_catalog_cd_s
 FREE SET cust_catalog_cd
 FREE SET comp_seq_cnt
 FREE SET cust_tab_name_s
 FREE SET cust_cs_loop
 FREE SET cust_col_cnt
 FREE SET cust_col_num
 FREE SET cust_cust_index_var
 FREE SET cust_check_loop
 FREE SET cust_nodelete_ind
 FREE SET cust_chg_log
 FREE SET cust_merge_loop
 FREE SET cust_cur_merges
 FREE SET cust_fail_merges
 FREE SET cust_del_msg
 FREE SET cust_where_clause
 FREE SET cust_parser_cnt
 FREE SET cust_col_name
 FREE SET passed_var
 FREE SET cust_loop
 FREE SET cust_from_val
 FREE SET cust_to_val
 FREE SET cust_from_str
END GO
