CREATE PROGRAM dm_oen_si_message_log_rows:dba
 ...
 IF (days_to_keep < 1)
  ...
  GO TO the_end
 ENDIF
 SET reply->table_name = "SI_MESSAGE_LOG"
 SET reply->rows_between_commit = min(10000,max_rows)
 SET v_days_back = cnvtlookbehind(build(days_to_keep,",D"))
 SELECT INTO "nl:"
  sml.rowid
  FROM si_message_log sml
  WHERE parser(sbr_getrowidnotexists("sml.updt_dt_tm < cnvtdatetime(v_days_back)","sml"))
  DETAIL
   rows = (rows+ 1)
   IF (mod(rows,10)=1)
    stat = alterlist(reply->rows,(rows+ 9))
   ENDIF
   reply->rows[rows].row_id = sml.rowid
  WITH nocounter, maxqual(sml,value(request->max_rows))
 ;end select
