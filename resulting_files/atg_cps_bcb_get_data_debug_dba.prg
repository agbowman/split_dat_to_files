CREATE PROGRAM atg_cps_bcb_get_data_debug:dba
 EXECUTE crmrtl
 EXECUTE srvrtl
 ...
 ENDWHILE
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(size(atg_alg_reply->allergy,5))),
   mltm_bcb_alg_map2 m
  PLAN (d
   WHERE d.seq <= size(atg_alg_reply->allergy,5))
   JOIN (m
   WHERE (m.drug_identifier=atg_alg_reply->allergy[d.seq].concept_identifier))
  DETAIL
   IF ((data->alg_bcb_codes > " "))
    data->alg_bcb_codes = concat(data->alg_bcb_codes,"@",trim(m.bcb_code,3))
   ELSE
    data->alg_bcb_codes = trim(m.bcb_code,3)
   ENDIF
  WITH nocounter
 ;end select
 SET errcode = error(errmsg,0)
 WHILE (errcode > 0)
   ...
   SET errcode = error(errmsg,0)
 ENDWHILE
 SET stat = atginitdiagnosis(null)
 SET stat = atggetdiagnosis(0.0,mpersonid,mencntrid,0)
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(size(atg_diag_reply->diag,5)))
  PLAN (d
   WHERE d.seq <= size(atg_diag_reply->diag,5)
    AND (atg_diag_reply->diag[d.seq].source_vocabulary_cd=cicd10ca))
  DETAIL
   IF ((data->diag_bcb_codes > " "))
    data->diag_bcb_codes = concat(data->diag_bcb_codes,"@",trim(atg_diag_reply->diag[d.seq].
      source_identifier))
   ELSE
    data->diag_bcb_codes = trim(atg_diag_reply->diag[d.seq].source_identifier)
   ENDIF
  WITH nocounter
 ;end select
 SET errcode = error(errmsg,0)
 ...
 CALL echoxml(request,"atg_cps_debug_request.xml")
 CALL echoxml(reply,"atg_cps_debug_reply.xml")
 CALL echoxml(reqinfo,"atg_cps_debug_reqinfo.xml")
 CALL echoxml(data,"atg_cps_debug_data.xml")
#exit_script
 CALL echorecord(data)
 CALL echo(build("Last_Mod: ","002 12/13/2010"))
END GO
