CREATE PROGRAM cp_complete_pathway_instance:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Pathway Instance ID" = 0
  WITH outdev, pathway_instance_id
 ...
 DECLARE save_doc_action_detail_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",4003199,
   "SAVEDOC"))
 DECLARE wkf_workflow_id = f8 WITH protect, constant(cnvtreal( $PATHWAY_INSTANCE_ID))
 ...
 SUBROUTINE main(null)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE wkf_workflow_id = f8 WITH protect, noconstant(0)
   SET wkf_workflow_id = getworkflowid(cnvtreal( $PATHWAY_INSTANCE_ID))
   IF (validate(debug_ind))
    CALL echo(build(" wkf_workflow_id -- > ",wkf_workflow_id))
   ENDIF
   IF (wkf_workflow_id > 0)
    CALL completeworkflow(wkf_workflow_id)
   ENDIF
   CALL log_message(build("Exit main(), Elapsed time in seconds:",((curtime3 - begin_curtime3)/ 100.0
     )),log_level_debug)
 END ;Subroutine
 SUBROUTINE (getworkflowid(pathway_instance_id=f8) =f8 WITH protect)
   CALL log_message("Begin getWorkflowId()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE wkf_workflow_id = f8 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    ...
    PLAN (cpa
     ...
     JOIN (crsl
     WHERE crsl.event_id=cpad.action_detail_entity_id)
     JOIN (wc
     WHERE wc.component_entity_name="CE_RESULT_SET_LINK"
      AND wc.component_entity_id=crsl.result_set_id)
     JOIN (ww
     WHERE ww.wkf_workflow_id=wc.wkf_workflow_id
      AND ww.end_dt_tm=null)
    ORDER BY ww.wkf_workflow_id
    HEAD ww.wkf_workflow_id
     wkf_workflow_id = ww.wkf_workflow_id
    WITH expand = 1
   ;end select
   CALL error_and_zero_check_rec(curqual,"WKF_COMPONENT","loadStructureComponents",1,0,
    reply)
   CALL log_message(build("Exit getWorkflowId(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(wkf_workflow_id)
 END ;Subroutine
 SUBROUTINE (completeworkflow(wkf_workflow_id=f8) =null WITH protect)
   UPDATE  FROM wkf_workflow w
    SET w.end_dt_tm = cnvtdatetime(sysdate), w.updt_dt_tm = cnvtdatetime(sysdate), w.updt_cnt = (w
     .updt_cnt+ 1),
     w.updt_id = reqinfo->updt_id, w.updt_task = reqinfo->updt_task, w.updt_applctx = reqinfo->
     updt_applctx
    WHERE w.wkf_workflow_id=wkf_workflow_id
    WITH nocounter
   ;end update
 END ;Subroutine
#exit_script
