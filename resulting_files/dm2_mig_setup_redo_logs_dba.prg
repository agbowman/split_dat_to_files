CREATE PROGRAM dm2_mig_setup_redo_logs:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD dmsrl_redo
 RECORD dmsrl_redo(
   1 database_name = vc
   1 maxlogmembers = i4
   1 nfs_redo_needed_ind = i2
   1 group_full_string = vc
   1 group_full_ind = i2
   1 group_cnt = i4
   1 groups[*]
     2 group_num = i4
     2 thread_num = f8
     2 invalid_exists_ind = i2
     2 nfs_redo_exists_ind = i2
     2 group_full_ind = i2
     2 member_cnt = i4
     2 members[*]
       3 member_name = vc
 )
 FREE RECORD dmsrl_threads
 RECORD dmsrl_threads(
   1 thread_cnt = i4
   1 threads[*]
     2 thread_num = f8
     2 instance_name = vc
 ...
 DECLARE dmsrl_index = i4 WITH protect, noconstant(0)
 DECLARE dmsrl_iter = i4 WITH protect, noconstant(0)
 DECLARE dmsrl_iter2 = i4 WITH protect, noconstant(0)
 DECLARE dmsrl_sup_act_ind = i2 WITH protect, noconstant(0)
 DECLARE dmsrl_current_db_host = vc WITH protect, noconstant(" ")
 DECLARE dmsrl_fill_redo_logs(null) = i2
 DECLARE dmsrl_get_maxlogmembers(null) = i2
 IF (check_logfile("dm2_mig_setup_redo",".log","DM2_MIG_SETUP_REDO_LOGS LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_setup_redo_logs"
 CALL disp_msg("",dm_err->logfile,0)
 IF (dmr_prompt_retrieve_mig_data(0,1,0,1)=0)
  GO TO exit_script
 ENDIF
 ...
 IF (cursys != "AXP")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "DM2_MIG_SETUP_REDO_LOGS can only be used from a VMS node."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Validating NFS devices."
 IF (dm2_push_dcl('write sys$output f$getdvi("nfsarch","ACPTYPE")')=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSEIF (parse_errfile(dm_err->errfile)=0)
  GO TO exit_script
 ENDIF
 IF (cnvtupper(trim(dm_err->errtext,3)) != "F11V5")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "NFSARCH is not an ODS-5 device."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dmsrl_redo->database_name = cnvtupper(dmr_mig_data->src_db_name)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining current threads from v$thread."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM v$thread vt
  HEAD REPORT
   dmsrl_threads->thread_cnt = 0, stat = alterlist(dmsrl_threads->threads,0)
  DETAIL
   dmsrl_threads->thread_cnt = (dmsrl_threads->thread_cnt+ 1), stat = alterlist(dmsrl_threads->
    threads,dmsrl_threads->thread_cnt), dmsrl_threads->threads[dmsrl_threads->thread_cnt].thread_num
    = vt.thread#,
   dmsrl_threads->threads[dmsrl_threads->thread_cnt].instance_name = vt.instance
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmsrl_threads)
 ENDIF
 SET dm_err->eproc = "Determining the currently connected instance from v$instance."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM v$instance vi
  DETAIL
   dmsrl_current_db_host = vi.host_name, dmsrl_index = locateval(dmsrl_index,1,dmsrl_threads->
    thread_cnt,vi.instance_name,dmsrl_threads->threads[dmsrl_index].instance_name)
   IF (dmsrl_index > 0)
    dmsrl_threads->threads[dmsrl_index].connect_string = dmr_mig_data->src_v500_cnct_str
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 IF (dmr_prompt_connect_data("SOURCE","SYS","CO")=0)
  GO TO exit_script
 ENDIF
 IF (dmsrl_get_maxlogmembers(null)=0)
  SET dm_err->err_ind = 1
 ENDIF
 IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
  GO TO exit_script
 ...
 IF ((dm_err->err_ind > 0))
  GO TO exit_script
 ENDIF
 IF (dmsrl_fill_redo_logs(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if any redo log groups can't support an additional member."
 IF ((dmsrl_redo->nfs_redo_needed_ind=1))
  SET dmsrl_redo->group_full_string = "["
  FOR (dmsrl_iter = 1 TO dmsrl_redo->group_cnt)
    IF ((dmsrl_redo->groups[dmsrl_iter].nfs_redo_exists_ind=0)
     AND ((dmsrl_redo->groups[dmsrl_iter].member_cnt+ 1) > dmsrl_redo->maxlogmembers))
     SET dmsrl_redo->group_full_ind = 1
     IF ((dmsrl_redo->group_full_string="["))
      SET dmsrl_redo->group_full_string = concat(dmsrl_redo->group_full_string,build(dmsrl_redo->
        groups[dmsrl_iter].group_num))
     ELSE
      SET dmsrl_redo->group_full_string = concat(dmsrl_redo->group_full_string,",",build(dmsrl_redo->
        groups[dmsrl_iter].group_num))
     ENDIF
    ENDIF
  ENDFOR
  SET dmsrl_redo->group_full_string = concat(dmsrl_redo->group_full_string,"]")
  IF ((dmsrl_redo->group_full_ind=1))
   SET dm_err->err_ind = 1
   SET dm_err->emsg = concat("Redo log group(s) ",build(dmsrl_redo->group_full_string),
    " is/are full.")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Configuring redo logs for the migration process."
  FOR (dmsrl_iter = 1 TO dmsrl_redo->group_cnt)
    IF ((dmsrl_redo->groups[dmsrl_iter].nfs_redo_exists_ind=0))
     IF (dm2_push_cmd(concat("RDB ALTER DATABASE ADD LOGFILE MEMBER 'NFSREDO:[V500.DB_",dmsrl_redo->
       database_name,"]ORA_GG_LOG",build(dmsrl_redo->groups[dmsrl_iter].group_num),".RDO' TO GROUP ",
       build(dmsrl_redo->groups[dmsrl_iter].group_num)," GO"),1)=0)
      GO TO exit_script
     ENDIF
    ENDIF
  ENDFOR
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Determining if all redo log groups have been configured properly."
 IF ((dmsrl_redo->nfs_redo_needed_ind=1))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "All redo log groups have NOT been configured for the migration process."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Attempting to switch logfiles so there are no remaining INVALID logfiles."
 FOR (dmsrl_iter = 1 TO dmsrl_threads->thread_cnt)
   FOR (dmsrl_iter2 = 1 TO dmsrl_threads->threads[dmsrl_iter].group_cnt)
     IF ((dmsrl_threads->threads[dmsrl_iter].invalid_exists_ind=1))
      IF (size(dmsrl_threads->threads[dmsrl_iter].connect_string)=0)
       SET dm2_install_schema->dbase_name = dmsrl_threads->threads[dmsrl_iter].instance_name
       SET dm2_install_schema->u_name = "V500"
       EXECUTE dm2_connect_to_dbase "PC"
       IF ((dm_err->err_ind=1))
        GO TO exit_script
       ENDIF
       SET dmsrl_threads->threads[dmsrl_iter].connect_string = dm2_install_schema->connect_str
      ENDIF
      IF (dm2_push_cmd("RDB ALTER SYSTEM SWITCH LOGFILE GO",1)=0)
       GO TO exit_script
      ENDIF
      IF (dmsrl_fill_redo_logs(null)=0)
       GO TO exit_script
      ENDIF
      SET dmsrl_threads->threads[dmsrl_iter].switch_ind = 1
     ENDIF
   ENDFOR
 ENDFOR
 FOR (dmsrl_iter = 1 TO dmsrl_threads->thread_cnt)
   IF ((dmsrl_threads->threads[dmsrl_iter].invalid_exists_ind=1))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Unable to VALIDATE all logfiles."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
 ENDFOR
 SET dm_err->eproc = "Determining if supplemental logging was recently activated from dm_info."
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 IF (dmsrl_sup_act_ind)
  FOR (dmsrl_iter = 1 TO dmsrl_threads->thread_cnt)
    IF ( NOT (dmsrl_threads->threads[dmsrl_iter].switch_ind))
     IF (size(dmsrl_threads->threads[dmsrl_iter].connect_string)=0)
      SET dm2_install_schema->dbase_name = dmsrl_threads->threads[dmsrl_iter].instance_name
      SET dm2_install_schema->u_name = "V500"
      EXECUTE dm2_connect_to_dbase "PC"
  ...
  COMMIT
 ENDIF
 IF ((dmsrl_redo->group_cnt > 0))
  SET dm_err->eproc = "Generating dm2_mig_setup_softlinks.ksh in CCLUSERDIR."
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO "dm2_mig_setup_softlinks.ksh"
   FROM (dummyt d  WITH seq = value(dmsrl_redo->group_cnt))
   DETAIL
    FOR (dmsrl_iter = 1 TO dmsrl_redo->groups[d.seq].member_cnt)
      IF ((dmsrl_redo->groups[d.seq].members[dmsrl_iter].member_name != "NFSREDO*"))
       CALL print(concat("if [[ ( -e /nfsredo/v500/db_",cnvtlower(dmsrl_redo->database_name),
        "/ORA_GG_LOG",build(dmsrl_redo->groups[d.seq].group_num),".RDO ) && ( ! -h /nfsredo/v500/db_",
        cnvtlower(dmsrl_redo->database_name),"/",build(substring((findstring("]",dmsrl_redo->groups[d
           .seq].members[dmsrl_iter].member_name,1,1)+ 1),size(dmsrl_redo->groups[d.seq].members[
           dmsrl_iter].member_name),dmsrl_redo->groups[d.seq].members[dmsrl_iter].member_name)),
        " ) ]]")), row + 1,
       CALL print("  then"),
       row + 1,
       CALL print(concat("    ln -s /nfsredo/v500/db_",cnvtlower(dmsrl_redo->database_name),
        "/ORA_GG_LOG",build(dmsrl_redo->groups[d.seq].group_num),".RDO /nfsredo/v500/db_",
        cnvtlower(dmsrl_redo->database_name),"/",build(substring((findstring("]",dmsrl_redo->groups[d
           .seq].members[dmsrl_iter].member_name,1,1)+ 1),size(dmsrl_redo->groups[d.seq].members[
           dmsrl_iter].member_name),dmsrl_redo->groups[d.seq].members[dmsrl_iter].member_name)))),
       row + 1,
    ...
    ENDFOR
   FOOT REPORT
    CALL print('echo "dm2_mig_setup_softlinks.ksh is executed successfully."'), row + 1
  ...
  CALL clear(1,1)
  CALL box(1,1,24,131)
  CALL text(2,2,"Create Symbolic (Soft) Links for Redo Members on the Linux Node")
  CALL text(4,2,"Note : Moving files should be done from Linux node to preserve file format.")
  CALL text(6,2,"Move the file below located in CCLUSERDIR to the Linux Node:")
  CALL text(7,5,"dm2_mig_setup_softlinks.ksh")
  CALL text(9,2,
   "On the Linux Node, as the ROOT user, execute the KSH file that was moved as follows:")
  CALL text(10,5,"$. dm2_mig_setup_softlinks.ksh")
  CALL text(12,2,"Upon successful completion of these steps, enter 'C' to complete:")
  CALL accept(12,68,"A;CU"
   WHERE curaccept="C")
  CALL clear(1,1)
  SET message = nowindow
 ENDIF
 GO TO exit_script
 SUBROUTINE dmsrl_get_maxlogmembers(null)
   IF (dm2_push_cmd("DROP TABLE X$KCCDI GO",1) != 1)
    RETURN(0)
   ENDIF
   IF (dm2_push_cmd("DROP DDLRECORD X$KCCDI FROM DATABASE V500 WITH DEPS_DELETED GO",1) != 1)
    RETURN(0)
   ENDIF
   IF (dm2_push_cmd(
    "CREATE DDLRECORD X$KCCDI FROM DATABASE V500 TABLE X$KCCDI 1 DIMLM = I4 CCL(DIMLM) END TABLE X$KCCDI GO",
    1) != 1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Determining maxlogmembers from x$kccdi."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM x$kccdi x
    DETAIL
     dmsrl_redo->maxlogmembers = x.dimlm
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->emsg = "Unable to determine maxlogmembers."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ELSEIF ((dmsrl_redo->maxlogmembers <= 1))
    SET dm_err->emsg = "maxlogmembers is <= 1."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmsrl_fill_redo_logs(null)
   DECLARE iter = i4 WITH protect, noconstant(0)
   DECLARE iter2 = i4 WITH protect, noconstant(0)
   SET dmsrl_redo->nfs_redo_needed_ind = 0
   FOR (iter = 1 TO dmsrl_threads->thread_cnt)
     SET dmsrl_threads->threads[iter].invalid_exists_ind = 0
   ENDFOR
   SET dm_err->eproc =
   "Loading redo log group members from v$log/v$logfile (excluding current instance host)."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM v$log vl,
     v$logfile vlf
    WHERE vl.group#=vlf.group#
     AND  NOT (vl.thread# IN (
    (SELECT
     i.thread#
     FROM gv$instance i
     WHERE i.host_name=dmsrl_current_db_host)))
    ORDER BY vlf.group#, vlf.member
    HEAD REPORT
     dmsrl_redo->group_cnt = 0, stat = alterlist(dmsrl_redo->groups,0)
    HEAD vlf.group#
     dmsrl_redo->group_cnt = (dmsrl_redo->group_cnt+ 1), stat = alterlist(dmsrl_redo->groups,
      dmsrl_redo->group_cnt), dmsrl_redo->groups[dmsrl_redo->group_cnt].group_num = vlf.group#,
     stat = alterlist(dmsrl_redo->groups[dmsrl_redo->group_cnt].members,cnvtint(vl.members)),
     dmsrl_redo->groups[dmsrl_redo->group_cnt].invalid_exists_ind = 0, dmsrl_redo->groups[dmsrl_redo
     ->group_cnt].thread_num = vl.thread#
    DETAIL
     dmsrl_redo->groups[dmsrl_redo->group_cnt].member_cnt = (dmsrl_redo->groups[dmsrl_redo->group_cnt
     ].member_cnt+ 1), dmsrl_redo->groups[dmsrl_redo->group_cnt].members[dmsrl_redo->groups[
     dmsrl_redo->group_cnt].member_cnt].member_name = vlf.member
     IF (vlf.status="INVALID")
      dmsrl_redo->groups[dmsrl_redo->group_cnt].invalid_exists_ind = 1, dmsrl_index = locateval(
       dmsrl_index,1,dmsrl_threads->thread_cnt,vl.thread#,dmsrl_threads->threads[dmsrl_index].
       thread_num)
      IF (dmsrl_index > 0)
       dmsrl_threads->threads[dmsrl_index].invalid_exists_ind = 1
      ENDIF
     ENDIF
     IF (vlf.member="NFSREDO*")
      dmsrl_redo->groups[dmsrl_redo->group_cnt].nfs_redo_exists_ind = 1
     ENDIF
    FOOT REPORT
     FOR (iter = 1 TO dmsrl_redo->group_cnt)
       IF ((dmsrl_redo->groups[iter].nfs_redo_exists_ind=0))
        dmsrl_redo->nfs_redo_needed_ind = 1, iter = (dmsrl_redo->group_cnt+ 1)
       ENDIF
     ENDFOR
     FOR (iter = 1 TO dmsrl_threads->thread_cnt)
      dmsrl_threads->threads[iter].group_cnt = 0,
      FOR (iter2 = 1 TO dmsrl_redo->group_cnt)
        IF ((dmsrl_threads->threads[iter].thread_num=dmsrl_redo->groups[iter2].thread_num))
         dmsrl_threads->threads[iter].group_cnt = (dmsrl_threads->threads[iter].group_cnt+ 1)
        ENDIF
      ENDFOR
     ENDFOR
    WITH nocounter
 ...
 END ;Subroutine
#exit_script
 CALL dm2_push_cmd("DROP TABLE X$KCCDI GO",1)
 CALL dm2_push_cmd("DROP DDLRECORD X$KCCDI FROM DATABASE V500 WITH DEPS_DELETED GO",1)
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_setup_redo_logs completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_setup_redo")
END GO
