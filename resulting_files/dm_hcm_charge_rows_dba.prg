CREATE PROGRAM dm_hcm_charge_rows:dba
 ...
 FOR (token_idx = 1 TO size(request->tokens,5))
   IF ((request->tokens[token_idx].token_str="YEARSTOKEEP"))
    SET years_to_keep = ceil(cnvtreal(request->tokens[token_idx].value))
   ENDIF
 ENDFOR
 IF (years_to_keep < 1)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"TOKENOUTOFRANGE",
   "You must keep at least 1 year's worth of data.  You entered %1 years or did not enter any value.",
   "i",years_to_keep)
  GO TO end_script
 ENDIF
 SET purge_dt_tm = cnvtagedatetime(years_to_keep,0,0,0)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(hc.hcm_charge_id)
   FROM hcm_charge hc
   WHERE hc.hcm_charge_id > 0
   DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    hc.rowid
    FROM hcm_charge hc
    WHERE hc.hcm_charge_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists("hc.created_dt_tm <= cnvtdatetime(purge_dt_tm)","hc"))
     AND hc.hcm_charge_id != 0
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = hc.rowid
    FOOT REPORT
     stat = alterlist(reply->rows,v_rows)
    WITH nocounter, maxqual(hc,value(request->max_rows))
   ;end select
