CREATE PROGRAM dm2_mig_revoke_ulimit_ts
 ...
 SUBROUTINE dm2_get_rdbms_version(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmrut_user_cnt = i4 WITH protect, noconstant(0)
 FREE RECORD dmrut_user
 RECORD dmrut_user(
   1 user_cnt = i2
   1 qual[*]
     2 user_name = vc
 )
 IF (check_logfile("dm2_migrvkulmt",".log","dm2_mig_revoke_ulimit_ts")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_revoke_ulimit_ts"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (dmr_prompt_connect_data("TARGET","SYS","PC")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Obtain users that should have unlimited tablespace priv revoked."
 SELECT INTO "nl:"
  ...
  (SELECT
   ...
   WHERE ((di.info_domain="DM2_MIG_USER") OR (di.info_domain="DM2_MIG_DT_USER"
    AND di.info_char="1")) ))
   AND  NOT (t.grantee IN (
  (SELECT
   s.grantee
   FROM dba_sys_privs@ref_data_link s
   WHERE s.privilege="UNLIMITED TABLESPACE")))
  DETAIL
   dmrut_user->user_cnt = (dmrut_user->user_cnt+ 1), stat = alterlist(dmrut_user->qual,dmrut_user->
    user_cnt), dmrut_user->qual[dmrut_user->user_cnt].user_name = t.grantee
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dmrut_user->user_cnt > 0))
  FOR (dmrut_user_cnt = 1 TO dmrut_user->user_cnt)
    SET dm_err->eproc = concat("Revoke UNLIMITED TABLESPACE privilege for ",dmrut_user->qual[
     dmrut_user_cnt].user_name)
    CALL disp_msg(" ",dm_err->logfile,0)
    IF (dm2_push_cmd(concat("rdb asis(^revoke unlimited tablespace from ",dmrut_user->qual[
      dmrut_user_cnt].user_name,"^) go"),1)=0)
     GO TO exit_program
    ENDIF
  ENDFOR
 ENDIF
...
#exit_program
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_revoke_ulimit_ts completed"
 ENDIF
 CALL final_disp_msg("dm2_migrvkulmt")
END GO
