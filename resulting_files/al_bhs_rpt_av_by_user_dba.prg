CREATE PROGRAM al_bhs_rpt_av_by_user:dba
 PROMPT
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  DETAIL
   ...
   temp1->qual[cnt].s_order_detail = trim(o.order_detail_display_line,3)
  FOOT REPORT
 ...
 IF (size(temp1->qual,5) > 0)
  ...
  SELECT INTO  $1
   ...
   priority = trim(substring(1,200,temp1->qual[d.seq].s_priority),3), route = trim(substring(1,200,
     temp1->qual[d.seq].s_route),3), order_date = trim(substring(1,50,temp1->qual[d.seq].odate),3),
   pat_loc = trim(substring(1,50,temp1->qual[d.seq].loc),3), fin = trim(substring(1,50,temp1->qual[d
     .seq].fin),3), mrn = trim(substring(1,50,temp1->qual[d.seq].mrn),3),
   verify_date = trim(substring(1,50,temp1->qual[d.seq].rdate),3), verify_by = trim(substring(1,200,
     temp1->qual[d.seq].rprsnl),3), tat = trim(substring(1,50,temp1->qual[d.seq].tatformatted),3),
   tat_in_mins = trim(substring(1,50,temp1->qual[d.seq].s_tat_mins),3)
   FROM (dummyt d  WITH seq = value(size(temp1->qual,5)))
   PLAN (d
    WHERE d.seq > 0)
