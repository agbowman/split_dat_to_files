CREATE PROGRAM dm2_process_queue_runner:dba
 ...
 ENDIF
 DECLARE dpqr_begin_dt_tm = dq8 WITH protect, constant(cnvtdatetime(curdate,curtime3))
 DECLARE dpqr_iter = i4 WITH protect, noconstant(0)
 DECLARE dpqr_continue_ind = i2 WITH protect, noconstant(0)
 DECLARE dpqr_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dpqr_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dpqr_proc_type_name = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_dpq_runner",".log","dm2_process_queue_runner LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_process_queue_runner"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Validating input parameter"
 SET dpqr_proc_type_name =  $1
 IF (((check_error(dm_err->eproc)=1) OR ( NOT (dpqr_proc_type_name > " "))) )
  SET dm_err->emsg = concat("Parameter usage: dm2_process_queue_runner '<PROCESS_TYPE>'")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm2_process_event_rs->status = dpl_executing
 SET dm2_process_rs->process_name = dpl_process_queue_runner
 CALL dm2_process_log_add_detail_text(dpl_proc_queue_runner_type,dpqr_proc_type_name)
 IF (dm2_process_log_row(dpl_process_queue_runner,dpl_execution,dpl_no_prev_id,1)=0)
  GO TO exit_script
 ENDIF
 SET dpqr_log_id = dm2_process_event_rs->dm_process_event_id
 SET dm2_process_rs->process_name = ""
 IF (dpq_cleanup_old_procs(dpqr_proc_type_name,1000)=0)
  GO TO exit_script
 ENDIF
 IF (dpq_populate_queue_rows(dpqr_proc_type_name)=0)
  GO TO exit_script
 ENDIF
 FOR (dpqr_iter = 1 TO dpq_proc_list->proc_cnt)
   SET dpq_process_queue->dm_process_queue_id = dpq_proc_list->qual[dpqr_iter].dm_process_queue_id
   SET dpq_process_queue->operation_txt = dpq_proc_list->qual[dpqr_iter].operation_txt
   SET dpq_process_queue->op_method = dpq_proc_list->qual[dpqr_iter].op_method
   IF (dpq_check_end_time(dpqr_begin_dt_tm,dpqr_proc_type_name,dpqr_continue_ind)=0)
    GO TO exit_script
   ELSEIF ( NOT (dpqr_continue_ind))
    SET dm_err->eproc = "End time window reached, ending dm2_process_queue_runner"
    CALL disp_msg("",dm_err->logfile,0)
    SET dpqr_iter = (dpq_proc_list->proc_cnt+ 1)
   ELSE
    IF ( NOT (dpq_lock_queue_row(dpqr_continue_ind)))
     GO TO exit_script
    ELSEIF (dpqr_continue_ind)
     IF ( NOT (dpq_execute_command(dpqr_continue_ind)))
      GO TO exit_script
     ELSEIF (dpqr_continue_ind=0)
      SET dm_err->err_ind = 0
     ENDIF
     IF (dpq_update_queue_row(dpqr_continue_ind)=0)
      GO TO exit_script
     ENDIF
    ENDIF
   ENDIF
...
#exit_script
 ...
 SET dpqr_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_process_queue_runner,dpl_execution,dpqr_log_id,1)
 SET dm2_process_rs->process_name = ""
 SET dm_err->err_ind = dpqr_err_ind
 SET dm_err->eproc = "Ending dm2_process_queue_runner"
 CALL final_disp_msg("dm2_process_queue_runner")
