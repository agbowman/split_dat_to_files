CREATE PROGRAM dm2_src_chg_log_summary:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("Source_Summary",".log","DM2_SRC_CHG_LOG_SUM LOGFILE")=0)
  SET dm_err->err_ind = 1
 ...
 SELECT INTO mine
  d.target_env_id, d.table_name, d.log_type,
  date_changed = max(d.updt_dt_tm)";;q", row_count = count(d.table_name)
  FROM dm_chg_log d
  WHERE d.log_type IN ("MERGED", "REFCHG")
  GROUP BY d.target_env_id, d.log_type, d.table_name
  ORDER BY d.target_env_id, d.log_type, d.table_name
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
