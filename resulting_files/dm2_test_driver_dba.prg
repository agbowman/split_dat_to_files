CREATE PROGRAM dm2_test_driver:dba
 ...
 ENDIF
 IF (validate(td_logfile,"X")="X")
  SET td_logfile = "DM2_TEST_DRIVER"
 ENDIF
 SET dm_err->logfile = td_logfile
 CALL init_logfile(dm_err->logfile,"DM2_TEST_DRIVER LOGFILE")
 IF ((validate(td_debug_flag,- (1)) != - (1)))
  SET dm_err->debug_flag = td_debug_flag
 ENDIF
 IF ((validate(td_skip_open_file_ind,- (1))=- (1)))
  SET td_skip_open_file_ind = 0
 ENDIF
 DECLARE td_sch_date_str = vc WITH protect
 DECLARE td_sch_date_fmt = f8 WITH protect
 DECLARE td_file_prefix = vc WITH protect
 DECLARE td_dest_dir = vc WITH protect
 DECLARE td_sch_type = vc WITH protect
 DECLARE td_ocd_str = vc WITH protect
 DECLARE td_mod_ind = i4 WITH protect, noconstant(0)
 DECLARE dm2_call_prog = vc WITH public, noconstant("DM2_TEST_DRIVER")
 SET td_sch_type = cnvtupper( $2)
 IF (td_sch_type="SD")
  SET td_sch_date_str =  $1
  SET td_sch_date_fmt = cnvtdate2(td_sch_date_str,"DD-MMM-YYYY")
  SET td_file_prefix = cnvtalphanum(format(td_sch_date_fmt,"MM/DD/YYYY;;D"))
 ELSEIF (td_sch_type="OCD")
  SET td_ocd_str =  $1
  SET td_file_prefix = build(td_ocd_str,"ocd")
 ENDIF
 CALL echo(concat("td_file_prefix=",td_file_prefix))
 IF (td_skip_open_file_ind=0)
  IF (open_sch_files(td_mod_ind,td_file_prefix,"CCLUSERDIR")=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((dm2_exec->prog_cnt > 0))
  FOR (td_cnt = 1 TO dm2_exec->prog_cnt)
   CALL parser(concat("execute ",dm2_exec->qual[td_cnt].prog_name," go"))
   IF ((dm_err->err_ind=1))
    GO TO exit_script
   ENDIF
  ENDFOR
 ELSE
  SET dm_err->eproc = "NO PROGRAMS TO EXECUTE"
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "DM2_TEST_DRIVER COMPLETED SUCCESSFULLY"
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
END GO
