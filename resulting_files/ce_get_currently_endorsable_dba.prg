CREATE PROGRAM ce_get_currently_endorsable:dba
 ...
 SELECT INTO "nl:"
  ...
  WHERE expand(req_idx,1,req_size,cea.event_id,request->event_id_set[req_idx].event_id)
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt += 1
   ...
   ENDIF
   reply->result_set[cnt].event_id = cea.event_id
  FOOT REPORT
 ...
 WHILE (error_code != 0)
   ...
   SET reply->status_data.subeventstatus[size].targetobjectname = "CE_EVENT_ACTION"
 ...
 IF (cnt=0)
  ...
  SET reply->status_data.subeventstatus[1].targetobjectname = "CE_EVENT_ACTION"
