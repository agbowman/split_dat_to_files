CREATE PROGRAM dm_pucb_mp_primed_pop:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PUCB_MP_PRIMED_POP"
 ...
 SUBROUTINE cust_ucb_upt(dummy)
   SELECT INTO "NL:"
    ...
    WHERE mdp.person_id IN (request->xxx_uncombine[ucb_cnt].from_xxx_id, request->xxx_uncombine[
    ucb_cnt].to_xxx_id)
   ...
   DELETE  FROM mp_primed_view_act mda
    WHERE mda.person_id IN (request->xxx_uncombine[ucb_cnt].from_xxx_id, request->xxx_uncombine[
    ucb_cnt].to_xxx_id)
    WITH nocounter
   ;end delete
