CREATE PROGRAM dm_ccb_csf_cva_filter:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (cva
   ...
   JOIN (cvs
   WHERE cva.code_set=cvs.code_set)
 ...
 IF (errcode != 0)
  SET errmsg = concat(
   "Unable to read from CODE_VALUE_ALIAS or CODE_VALUE_SET while searching on ALIAS:",errmsg)
  SET status = "F"
  GO TO exit_program
 ENDIF
#exit_program
