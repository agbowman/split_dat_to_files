CREATE PROGRAM cv_utl_get_case_summary:dba
 PROMPT
  "Please enter cv_case_id for the case you want to view = " = " ",
  "View Detail Record Information(Y/N) [N] = " = "N",
  "View Long Text Records(Y/N) [Y] = " = "Y"
 SET input_case_id = cnvtreal( $1)
 SET view_choice = cnvtupper( $2)
 SET view_lt = cnvtupper( $3)
 SET failure = "F"
 SET cv_case = 0
 ...
 IF (view_choice="N")
  ...
  IF (view_lt="Y")
   ...
   ;end select
   CALL echo("**********************************************************")
   CALL echo("Please check cv_case etc. info above long_text!")
  ...
  IF (view_lt="Y")
   SELECT
    ...
    WHERE cc.cv_case_id=input_case_id
     ...
     AND ccdr.case_dataset_r_id=ccfr.case_dataset_r_id
     AND lt.long_text_id=ccfr.long_text_id
     AND lt.parent_entity_name="CV_CASE*"
    WITH nocounter
   ;end select
   SELECT
    lt.*
