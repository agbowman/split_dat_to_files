CREATE PROGRAM dm_pft_soltn_capability_rows:dba
 ...
 SET reply->table_name = "SOLUTION_CAPABILITY"
 ...
 IF (days_to_keep < 5)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"DAYSTOKEEP",
   "You must keep at least 5 day's worth of data.  You entered %1 days or did not enter any value.",
   "i",days_to_keep)
  GO TO exit_script
 ELSE
  IF (batch_ndx=1)
  ...
  WHILE (curmindate <= maxdate
   AND rowsleft > 0)
    SELECT INTO "nl:"
     FROM solution_capability sc
     WHERE sc.updt_dt_tm >= cnvtdatetime(curmindate)
      AND sc.updt_dt_tm <= cnvtdatetime(curmaxdate)
      AND parser(sbr_getrowidnotexists("sc.solution_capability_id+0 > 0.0 ","sc"))
     DETAIL
      v_rows = (v_rows+ 1)
      IF (mod(v_rows,10)=1)
     ...
     WITH nocounter, maxqual(sc,value(rowsleft))
    ;end select
 ...
 SUBROUTINE sbr_get_min_date(null)
   DECLARE mindate = dq8
   SELECT INTO "nl:"
    seqval = min(sc.updt_dt_tm)
    FROM solution_capability sc
    WHERE sc.solution_capability_id > 0.0
    DETAIL
     mindate = seqval
    WITH nocounter
   ;end select
   ...
   IF (v_err_code2 != 0)
    SET reply->err_code = v_err_code2
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"SELECTERROR",
     "Failed to select minimum date:%1","s",nullterm(v_errmsg2))
    GO TO exit_script
   ENDIF
   RETURN(mindate)
 END ;Subroutine
