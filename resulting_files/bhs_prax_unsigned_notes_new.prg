CREATE PROGRAM bhs_prax_unsigned_notes_new
 DECLARE pid = f8
 DECLARE eid = f8
 DECLARE prsnl_id = f8
 SET pid =  $2
 ...
 SELECT DISTINCT INTO  $1
  ...
  saved_dt = format(ce.performed_dt_tm,"MM/DD/YYYY HH:MM:SS;;D"), saved_dt_tz = substring(21,3,
   datetimezoneformat(ce.performed_dt_tm,ce.performed_tz,"MM/dd/yyyy hh:mm:ss ZZZ",curtimezonedef)),
  ...
  ENDIF
  , updt_dt_tm = format(ce.updt_dt_tm,"MM/DD/YYYY HH:MM:SS;;D")
  FROM encounter e,
   clinical_event ce,
   prsnl p,
   ce_event_prsnl cep,
  ...
  PLAN (e
   ...
   WHERE ce.encntr_id=e.encntr_id
    ...
    AND ce.person_id > 0
    AND ce.result_status_cd IN (33, 31, 34)
    AND ce.view_level=1
   ...
   WHERE cep.event_id=ce.event_id
    ...
    AND cep.action_status_cd=614384.00
