CREATE PROGRAM ams_pft_get_crit_facilities:dba
 ...
 IF (size(pft_reply->facilities,5) > 0)
  ...
  FOR (loopcnt = 1 TO size(pft_reply->facilities,5))
    SET recordpos = getnextrecord(0)
    SET stat = setstringfield(recordpos,disppos,substring(1,100,pft_reply->facilities[loopcnt].
      organizationname))
    SET stat = setrealfield(recordpos,valuepos,pft_reply->facilities[loopcnt].organizationid)
  ENDFOR
