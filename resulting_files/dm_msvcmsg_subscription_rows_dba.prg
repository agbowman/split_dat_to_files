CREATE PROGRAM dm_msvcmsg_subscription_rows:dba
 ...
 FREE RECORD expired_subs
 RECORD expired_subs(
   1 subscriptions[*]
     2 subscription_id = f8
 )
 SET i18nhandle = 0
 ...
 SET reply->table_name = "SUBSCRIPTION"
 SET reply->rows_between_commit = 1000
 IF ((request->max_rows < 10))
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"s1",
   "Max rows must be greater than or equal to 10.  You entered %1 rows or did not enter any value.",
   "i",request->max_rows)
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(s.subscription_id)
   FROM subscription s
   WHERE s.subscription_id > 0
   DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    s.rowid
    FROM subscription s
    WHERE s.subscription_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists("s.expiration_dt_tm+0 < cnvtdatetime(curdate, curtime3)","s"))
     AND  NOT ( EXISTS (
    (SELECT
     snr.subscription_id
     FROM subscription_new_results snr
     WHERE snr.subscription_id=s.subscription_id)))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99)), stat = alterlist(expired_subs->subscriptions,(
       v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = s.rowid, expired_subs->subscriptions[v_rows].subscription_id = s
     .subscription_id
    WITH nocounter, maxqual(s,value(v_rows_left))
   ;end select
 ...
 SET stat = alterlist(expired_subs->subscriptions,v_rows)
 IF ((request->purge_flag != c_audit))
  IF (v_rows > 0)
   UPDATE  FROM subscrptn_hist sh,
     (dummyt d  WITH seq = value(v_rows))
    SET sh.purge_dt_tm = cnvtdatetime(curdate,curtime3), sh.updt_dt_tm = cnvtdatetime(curdate,
      curtime3), sh.updt_id = reqinfo->updt_id,
     sh.updt_task = reqinfo->updt_task, sh.updt_cnt = (sh.updt_cnt+ 1), sh.updt_applctx = reqinfo->
     updt_applctx
    PLAN (d)
     JOIN (sh
     WHERE (sh.subscription_id=expired_subs->subscriptions[d.seq].subscription_id))
    WITH nocounter
   ;end update
   SET v_errcode2 = error(v_errmsg2,1)
   IF (v_errcode2 != 0)
    ...
    SET reply->err_code = v_errcode2
    SET reply->status_data.status = "F"
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"HISTUPD",
     "Failed in updating SUBSCRIPTN_HIST: %1","s",nullterm(v_errmsg2))
    GO TO exit_script
   ENDIF
   UPDATE  FROM subscrptn_filter_msg_hist sfmh,
     (dummyt d  WITH seq = value(v_rows))
    SET sfmh.purge_dt_tm = cnvtdatetime(curdate,curtime3), sfmh.updt_dt_tm = cnvtdatetime(curdate,
      curtime3), sfmh.updt_id = reqinfo->updt_id,
     sfmh.updt_task = reqinfo->updt_task, sfmh.updt_cnt = (sfmh.updt_cnt+ 1), sfmh.updt_applctx =
     reqinfo->updt_applctx
    PLAN (d)
     JOIN (sfmh
     WHERE (sfmh.subscription_id=expired_subs->subscriptions[d.seq].subscription_id))
    WITH nocounter
   ;end update
   SET v_errcode2 = error(v_errmsg2,1)
   IF (v_errcode2 != 0)
    ...
    SET reply->err_code = v_errcode2
    SET reply->status_data.status = "F"
    SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"MSGHISTUPD",
     "Failed in updating SUBSCRIPTN_FILTER_MSG_HIST: %1","s",nullterm(v_errmsg2))
    GO TO exit_script
   ELSE
    COMMIT
   ENDIF
  ENDIF
  DELETE  FROM subscrptn_hist sh
   WHERE sh.purge_dt_tm <= cnvtdatetime((curdate - 180),curtime3)
    AND sh.purge_dt_tm != null
    AND ((sh.subscrptn_hist_id+ 0) != 0)
   WITH nocounter, maxqual(sh,value(request->max_rows))
  ;end delete
  SET v_errcode2 = error(v_errmsg2,1)
  IF (v_errcode2 != 0)
   ROLLBACK
   SET reply->err_code = v_errcode2
   SET reply->status_data.status = "F"
   SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"HISTDEL",
    "Failed in deleting from SUBSCRTPN_HIST: %1","s",nullterm(v_errmsg2))
   GO TO exit_script
  ENDIF
  DELETE  FROM subscrptn_filter_msg_hist sfmh
   WHERE sfmh.purge_dt_tm <= cnvtdatetime((curdate - 180),curtime3)
    AND sfmh.purge_dt_tm != null
    AND ((sfmh.subscrptn_filter_msg_hist_id+ 0) != 0)
   WITH nocounter, maxqual(sfmh,value(request->max_rows))
  ;end delete
  SET v_errcode2 = error(v_errmsg2,1)
  IF (v_errcode2 != 0)
   ROLLBACK
   SET reply->err_code = v_errcode2
   SET reply->status_data.status = "F"
   SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"MSGHISTDEL",
    "Failed in deleting from SUBSCRIPTN_FILTER_MSG_HIST: %1","s",nullterm(v_errmsg2))
   GO TO exit_script
  ELSE
   COMMIT
  ENDIF
