CREATE PROGRAM afc_da_upt_charge_batch_dtl_cd:dba
 SET afc_da_upt_charge_batch_dtl_cd_vrsn = 20090219
 ...
 SET msdatablename = "CHARGE_BATCH_DETAIL_CODE"
 ...
 DECLARE upt_charge_batch_detail_code(nstart=i4,nstop=i4) = null WITH protect
 ...
 CALL upt_charge_batch_detail_code(mndastart,mndastop)
 SUBROUTINE upt_charge_batch_detail_code(nstart,nstop)
   DECLARE i = i4 WITH noconstant(0), protect
   DECLARE blocked = i2 WITH noconstant(0), protect
   SET blocked = false
   SET i = nstart
   FOR (i = 1 TO size(request->objarray,5))
     IF (validate(request->objarray[i].charge_batch_detail_code_id,- (0.00001)) <= 0.0)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (dt)
     JOIN (c
     WHERE (c.charge_batch_detail_code_id=request->objarray[dt.seq].charge_batch_detail_code_id))
    DETAIL
     mndamodrec = (mndamodrec+ 1), mndamodfld = 0, reply->mod_objs[mndamodobj].mod_recs[mndamodrec].
     table_name = msdatablename,
     reply->mod_objs[mndamodobj].mod_recs[mndamodrec].pk_values = build(c.charge_batch_detail_code_id
      )
     ...
     ENDIF
     IF (validate(request->objarray[dt.seq].parent_entity_name,c.parent_entity_name) != char(128)
      AND validate(request->objarray[dt.seq].parent_entity_name,c.parent_entity_name) != c
     .parent_entity_name)
      CALL logfieldmodified("PARENT_ENTITY_NAME","C30",validate(request->objarray[dt.seq].
       parent_entity_name,c.parent_entity_name),c.parent_entity_name)
     ENDIF
     IF ((validate(request->objarray[dt.seq].parent_entity_id,c.parent_entity_id) != - (0.00001))
      AND validate(request->objarray[dt.seq].parent_entity_id,c.parent_entity_id) != c
     .parent_entity_id)
      CALL logfieldmodified("PARENT_ENTITY_ID","F8",build(validate(request->objarray[dt.seq].
        parent_entity_id,c.parent_entity_id)),build(c.parent_entity_id))
     ENDIF
     IF (validate(request->objarray[dt.seq].type_ident,c.type_ident) != char(128)
      AND validate(request->objarray[dt.seq].type_ident,c.type_ident) != c.type_ident)
      CALL logfieldmodified("TYPE_IDENT","C50",validate(request->objarray[dt.seq].type_ident,c
       .type_ident),c.type_ident)
     ENDIF
     ...
     ENDIF
     IF ((validate(request->objarray[dt.seq].type_cd,c.type_cd) != - (0.00001))
      AND validate(request->objarray[dt.seq].type_cd,c.type_cd) != c.type_cd)
      CALL logfieldmodified("TYPE_CD","F8",build(validate(request->objarray[dt.seq].type_cd,c.type_cd
        )),build(c.type_cd))
     ENDIF
   ...
   FOR (i = 1 TO size(mrsstatus->objarray,5))
     ...
     ENDIF
   ENDFOR
   UPDATE  FROM charge_batch_detail_code c,
     (dummyt dt  WITH seq = value(size(request->objarray,5)))
    SET c.charge_batch_detail_id =
     ...
     IF (validate(request->objarray[dt.seq].parent_entity_name,char(128)) != char(128)) validate(
       request->objarray[dt.seq].parent_entity_name,char(128))
     ELSE c.parent_entity_name
     ENDIF
     , c.parent_entity_id =
     IF ((validate(request->objarray[dt.seq].parent_entity_id,- (0.00001)) != - (0.00001))) validate(
       request->objarray[dt.seq].parent_entity_id,- (0.00001))
     ELSE c.parent_entity_id
     ENDIF
     ,
     c.type_ident =
     IF (validate(request->objarray[dt.seq].type_ident,char(128)) != char(128)) validate(request->
       objarray[dt.seq].type_ident,char(128))
     ELSE c.type_ident
     ENDIF
     , c.priority_seq =
     ...
     ENDIF
     , c.type_cd =
     IF ((validate(request->objarray[dt.seq].type_cd,- (0.00001)) != - (0.00001))) validate(request->
       objarray[dt.seq].type_cd,- (0.00001))
     ELSE c.type_cd
     ENDIF
     ,
     c.active_ind =
