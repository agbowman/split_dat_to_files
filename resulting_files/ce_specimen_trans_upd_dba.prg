CREATE PROGRAM ce_specimen_trans_upd:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 SET error_code = 0
 UPDATE  FROM ce_specimen_trans t,
  ...
  SET t.event_id = evaluate2(
    ...
    ENDIF
    ), t.transfer_dt_tm = evaluate2(
    IF ((request->lst[d.seq].transfer_dt_tm_ind=1)) null
    ELSE cnvtdatetimeutc(request->lst[d.seq].transfer_dt_tm)
    ENDIF
    ),
   t.transfer_prsnl_id = evaluate2(
    IF ((request->lst[d.seq].transfer_prsnl_id=- (1))) 0
    ELSE request->lst[d.seq].transfer_prsnl_id
    ENDIF
    ), t.transfer_loc_cd = evaluate2(
    IF ((request->lst[d.seq].transfer_loc_cd=- (1))) 0
    ELSE request->lst[d.seq].transfer_loc_cd
    ENDIF
    ), t.receive_dt_tm = evaluate2(
    IF ((request->lst[d.seq].receive_dt_tm_ind=1)) null
    ELSE cnvtdatetimeutc(request->lst[d.seq].receive_dt_tm)
    ENDIF
    ),
   t.receive_prsnl_id = evaluate2(
    IF ((request->lst[d.seq].receive_prsnl_id=- (1))) 0
    ELSE request->lst[d.seq].receive_prsnl_id
    ENDIF
    ), t.receive_loc_cd = evaluate2(
    IF ((request->lst[d.seq].receive_loc_cd=- (1))) 0
    ELSE request->lst[d.seq].receive_loc_cd
    ENDIF
  ...
  PLAN (d)
   ...
   WHERE (t.event_id=request->lst[d.seq].event_id)
    AND (t.sequence_nbr=request->lst[d.seq].sequence_nbr)
