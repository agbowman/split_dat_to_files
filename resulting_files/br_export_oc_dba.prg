CREATE PROGRAM br_export_oc:dba
 RECORD temp(
   1 oclist[5000]
     2 catalog_cd = f8
     2 catalog_type = vc
 ...
 SELECT INTO "nl:"
  ...
  PLAN (c
   ...
   JOIN (c3
   WHERE c3.code_value=outerjoin(ocs.activity_subtype_cd)
    AND c3.br_client_id=outerjoin(0)
    AND c3.start_version_nbr=outerjoin(start_version))
  HEAD oc.catalog_cd
   cnt = (cnt+ 1)
   IF (cnt > 5000)
