CREATE PROGRAM bb_chk_import_ques_resp:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "bb_chk_import_ques_resp"
 ...
 DECLARE yes_response_cd = f8 WITH protect, noconstant(0.0)
 DECLARE no_response_cd = f8 WITH protect, noconstant(0.0)
 ...
 DECLARE response_code_set = i4 WITH protect, constant(1659)
 DECLARE yes_cdf_meaning = c12 WITH protect, constant("Y")
 DECLARE no_cdf_meaning = c12 WITH protect, constant("N")
 DECLARE smsg = vc WITH protect, noconstant("")
 SELECT INTO "nl:"
  c.code_value
  FROM code_value c
  PLAN (c
   WHERE c.code_set IN (question_code_set, response_code_set)
    AND c.cdf_meaning IN (dispense_notice_cdf_meaning, yes_cdf_meaning, no_cdf_meaning)
    AND c.active_ind=1)
  DETAIL
   IF (c.code_set=question_code_set
    AND c.cdf_meaning=dispense_notice_cdf_meaning)
    dispense_notice_ques_cd = c.code_value
   ELSEIF (c.code_set=response_code_set
    AND c.cdf_meaning=yes_cdf_meaning)
    yes_response_cd = c.code_value
   ELSEIF (c.code_set=response_code_set
    AND c.cdf_meaning=no_cdf_meaning)
    no_response_cd = c.code_value
   ENDIF
  WITH nocounter
 ;end select
 IF (dispense_notice_ques_cd=0.0)
  ...
  GO TO exit_script
 ENDIF
 IF (yes_response_cd=0.0)
  SET success_ind = 0
  SET smsg = concat("Yes response code lookup failed->","Y")
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 IF (no_response_cd=0.0)
  SET success_ind = 0
  SET smsg = concat("No response code lookup failed->","N")
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM valid_response r
  WHERE r.question_cd=dispense_notice_ques_cd
   AND r.response_cd IN (yes_response_cd, no_response_cd)
  WITH nocounter
 ;end select
 IF (curqual != 2)
  SET success_ind = 0
  SET smsg = 'Failed to find 2 responses for "PRODDSNOTICE" question in VALID_RESPONSE table.'
  CALL logmsg(smsg,(e_dispmsg+ e_logtofile))
 ENDIF
#exit_script
 IF (success_ind=0)
 ...
 ELSE
  CALL logmsg("Inserted all VALID_RESPONSE rows successfully.",((e_rdmstatus+ e_dispmsg)+ e_logtofile
   ))
  CALL logstatus("Inserted all VALID_RESPONSE rows successfully.","S")
 ENDIF
