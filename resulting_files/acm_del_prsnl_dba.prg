CREATE PROGRAM acm_del_prsnl:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.person_id=acm_request->prsnl_qual[xref->del[d.seq].idx].person_id))
  WITH nocounter, status(reply->prsnl_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->prsnl_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM prsnl p
    WHERE expand(t1,f_val,t_val,p.person_id,acm_request->prsnl_qual[xref->del[t1].idx].person_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.person_id,acm_request->prsnl_qual[xref->del[t1].idx].person_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_qual[xref->del[index].idx].status=0))
