CREATE PROGRAM bhs_adm_meds_recon_rpt:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Select Organization",
  "Select Nursing Unit /s" = value(),
 ...
 SELECT DISTINCT INTO  $OUTDEV
  ...
  IF (datetimediff(ce.verified_dt_tm,e.reg_dt_tm,3) <= 24
   AND ce.result_status_cd=25) "Y"
  ELSEIF (datetimediff(ce.verified_dt_tm,e.reg_dt_tm,3) > 24
   AND ce.result_status_cd=25) "N"
  ELSE " "
  ENDIF
  ...
  PLAN (ed
   ...
   WHERE ce.encntr_id=outerjoin(e.encntr_id)
    AND ce.event_cd=outerjoin(48188461)
    AND ce.event_reltn_cd=outerjoin(132)
    AND ce.publish_flag=outerjoin(1)
    AND ce.result_status_cd=outerjoin(25))
   JOIN (d1)
   JOIN (elh
   WHERE ((elh.encntr_id=e.encntr_id
    AND ((elh.end_effective_dt_tm+ 0) < cnvtdatetime(cnvtdate(12312006),0000))
    AND ((elh.encntr_type_cd+ 0) IN (679668.00, 309308.00, 309312.00))
  ...
  HEAD PAGE
   ...
   CALL print(calcpos(312,(y_pos+ 0))), "Admission Medication Reconciliation Report", row + 1,
   row + 1,
   ...
   row + 1, row + 1,
   CALL print(calcpos(52,(y_pos+ 72))),
   "Attending",
   CALL print(calcpos(120,(y_pos+ 72))), "Nurse Unit",
   CALL print(calcpos(178,(y_pos+ 72))), "Patient",
   CALL print(calcpos(315,(y_pos+ 72))),
   "IP Admit Time",
   CALL print(calcpos(395,(y_pos+ 72))), "Admit Loc",
   CALL print(calcpos(455,(y_pos+ 72))), "Unit Admit Time",
   CALL print(calcpos(530,(y_pos+ 72))),
   "Accnt Num",
   CALL print(calcpos(585,(y_pos+ 72))), "Med Rec",
   CALL print(calcpos(630,(y_pos+ 72))), "Completed",
   CALL print(calcpos(670,(y_pos+ 72))),
   "24 Hr Flag",
   CALL print(calcpos(720,(y_pos+ 72))), "Task Status",
   row + 1, row + 1, y_val = ((792 - y_pos) - 103),
   "{PS/newpath 2 setlinewidth   36 ", y_val, " moveto  745 ",
  ...
  DETAIL
   ...
   row + 1,
   CALL print(calcpos(690,(y_pos+ 0))), signed_within_24,
   row + 1,
   CALL print(calcpos(720,(y_pos+ 0))), status,
   y_pos = (y_pos+ 13)
  FOOT  nurse
   IF (((y_pos+ 152) >= 612))
    y_pos = 0, BREAK
   ...
   not_c = count(ce.event_id
    WHERE ce.result_status_cd != 25), all_c = count(ce.event_id), all_c2 = count(ce.event_id),
   ...
   " %", row + 1, row + 1,
   y_val = ((792 - y_pos) - 62), "{PS/newpath 2 setlinewidth  216 ", y_val,
   " moveto  574 ", y_val, " lineto stroke 216 ",
   y_val, " moveto/}", row + 1,
   row + 1,
   CALL print(calcpos(216,(y_pos+ 54))), "Total Complete & Not Complete for",
   row + 1,
   CALL print(calcpos(350,(y_pos+ 54))), nurse,
   row + 1,
   CALL print(calcpos(414,(y_pos+ 54))), all_c2,
   y_pos = (y_pos+ 114)
  FOOT REPORT
   IF (all_c2 >= 1)
    IF (((y_pos+ 152) >= 612))
    ...
    c_lt_24 = count(ce.event_id
     ...
     WHERE ce.result_status_cd=25),
    not_c = count(ce.event_id
     WHERE ce.result_status_cd != 2543), all_c = count(ce.event_id), all_c2 = count(ce.event_id),
    per_c_lt_24 = (((c_lt_24 * 100)/ (all_c * 100)) * 100), per_c_mt_24 = (((c_mt_24 * 100)/ (all_c
     * 100)) * 100), per_not_c = (((not_c * 100)/ (all_c * 100)) * 100),
    per_comp = (((comp * 100)/ (all_c * 100)) * 100), row + 1, "{F/5}{CPI/14}",
    ...
    y_val, " moveto/}", row + 1,
    row + 1,
    CALL print(calcpos(216,(y_pos+ 90))), "Total Complete and Not Complete",
    row + 1,
