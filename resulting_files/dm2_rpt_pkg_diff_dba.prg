CREATE PROGRAM dm2_rpt_pkg_diff:dba
 ...
 END ;Subroutine
 DECLARE drpd_load_pkg(dlp_src_env_id,dlp_tgt_env_id) = i2
 DECLARE drpd_src_env_name = vc WITH protect, noconstant("")
 DECLARE drpd_src_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drpd_tgt_env_name = vc WITH protect, noconstant("")
 DECLARE drpd_tgt_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drpd_str = vc WITH protect, noconstant("")
 DECLARE drpd_file = vc WITH protect, noconstant("")
 FREE RECORD drpd_pkg
 RECORD drpd_pkg(
   1 cnt = i4
   1 pkg[*]
     2 pkg_nbr = f8
 ...
 SET message = nowindow
 IF (check_logfile("dm2_rpt_pkg_diff",".log","dm2_rpt_pkg_diff log file")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM2_RPT_PKG_DIFF."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET drpd_src_env_name = trim( $1)
 SET drpd_tgt_env_name = trim( $2)
 IF (dehr_chk_env(drpd_src_env_name,drpd_src_env_id)=0)
  GO TO exit_script
 ENDIF
 IF (dehr_chk_env(drpd_tgt_env_name,drpd_tgt_env_id)=0)
  GO TO exit_script
 ENDIF
 IF (drpd_load_pkg(drpd_src_env_id,drpd_tgt_env_id)=0)
  GO TO exit_script
 ENDIF
 IF (get_unique_file("dm2_pkg_diff",".rpt")=0)
  GO TO exit_script
 ENDIF
 SET drpd_file = dm_err->unique_fname
 IF (validate(drrr_responsefile_in_use,0)=1)
  SET drpd_file = build(drrr_misc_data->active_dir,drpd_file)
 ENDIF
 SET dm_err->eproc = concat("Report package in environment ",drpd_src_env_name,
  " and not in environment ",drpd_tgt_env_name," in file ",
  trim(drpd_file)," in ccluserdir.")
 CALL disp_msg("",dm_err->logfile,0)
 IF ((drpd_pkg->cnt > 0))
  SELECT INTO value(drpd_file)
   FROM (dummyt t  WITH seq = drpd_pkg->cnt)
   HEAD REPORT
    col 50, "Package Difference Report", row + 2,
    col 0, "Source Environment Name :  ", drpd_src_env_name,
    row + 1, col 0, "Target Environment Name :  ",
    drpd_tgt_env_name, row + 2, col 0,
    "Packages installed into Source after Source Environment History Snapshot taken: ", row + 2, row
     + 1,
    col 0, "Package", col 15,
    "Start_dt_tm", col 40, "End_dt_tm",
    col 65, "Status", row + 1
   DETAIL
    drpd_str = cnvtstring(drpd_pkg->pkg[t.seq].pkg_nbr), col 0, drpd_str,
    drpd_str = format(cnvtdatetime(drpd_pkg->pkg[t.seq].start_dt_tm),";;q"), col 15, drpd_str,
    drpd_str = format(cnvtdatetime(drpd_pkg->pkg[t.seq].end_dt_tm),";;q"), col 40, drpd_str,
    col 65, drpd_pkg->pkg[t.seq].status, row + 1
 ...
 ELSE
  SELECT INTO value(drpd_file)
   ...
   HEAD REPORT
    ...
    row + 1, col 0, "Target Environment Name :  ",
    drpd_tgt_env_name, row + 2, col 0,
    "Packages in Source match packages in Target. ", row + 2, row + 1
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (validate(drrr_responsefile_in_use,0)=1)
  SET dm_err->eproc = concat("Skipping display of Package Difference Report (",trim(drpd_file,3),")")
  CALL disp_msg("",dm_err->logfile,0)
  IF ((drer_email_list->email_cnt > 0)
   AND (drpd_pkg->cnt > 0))
   SET drer_email_det->msgtype = "ACTIONREQ"
   ...
   SET drer_email_det->step = "Package Difference Report"
   ...
   SET drer_email_det->attachment = drpd_file
   CALL drer_add_body_text(concat("Package Difference Report was generated at ",format(drer_email_det
      ->status_dt_tm,";;q")),1)
   CALL drer_add_body_text(concat("User Action : Please review the report to ensure ",
     "missing packages listed are accurate.  Follow steps documented in the Replicate/Refresh doc for proper action."
     ),0)
   CALL drer_add_body_text(concat("Report file name : ",trim(drpd_file,3)),0)
 ...
 ELSE
  IF ((dm2_install_schema->process_option="CLIN COPY")
   ...
   SET drer_email_det->emsg = dm_err->emsg
   SET drer_email_det->user_action = dm_err->user_action
   CALL drer_add_body_text(concat("Package Difference Report ","was displayed at ",format(
      drer_email_det->status_dt_tm,";;q")),1)
   CALL drer_add_body_text(concat("User Action : Return to dm2_domain_maint main session and ",
     "review Package Difference Report displayed on the screen.  Press <enter> to continue."),0)
   CALL drer_add_body_text(concat("Report file name is ccluserdir: ",drpd_file),0)
   IF (drer_compose_email(null)=1)
    CALL drer_send_email(drer_email_det->subject,drer_email_det->file_name,drer_email_det->
     email_level)
   ENDIF
   CALL drer_reset_pre_err(null)
  ENDIF
  IF (dm2_disp_file(drpd_file,"Package Difference Report")=0)
   GO TO exit_script
  ENDIF
 ENDIF
 GO TO exit_script
 SUBROUTINE drpd_load_pkg(dlp_src_env_id,dlp_tgt_env_id)
   SET dm_err->eproc = build("Load packages are in environment ",dlp_src_env_id,
    " and not in environment ",dlp_tgt_env_id)
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm_alpha_features_env d
    WHERE d.environment_id=dlp_src_env_id
     AND d.alpha_feature_nbr >= 0
     AND  NOT ( EXISTS (
    (SELECT
     "x"
     FROM dm_alpha_features_env a
     WHERE a.environment_id=dlp_tgt_env_id
      AND a.alpha_feature_nbr >= 0
      AND d.alpha_feature_nbr=a.alpha_feature_nbr)))
    ORDER BY d.start_dt_tm
    DETAIL
     drpd_pkg->cnt = (drpd_pkg->cnt+ 1)
     IF (mod(drpd_pkg->cnt,10)=1)
      stat = alterlist(drpd_pkg->pkg,(drpd_pkg->cnt+ 9))
     ENDIF
     drpd_pkg->pkg[drpd_pkg->cnt].pkg_nbr = d.alpha_feature_nbr, drpd_pkg->pkg[drpd_pkg->cnt].
     start_dt_tm = d.start_dt_tm, drpd_pkg->pkg[drpd_pkg->cnt].end_dt_tm = d.end_dt_tm,
     drpd_pkg->pkg[drpd_pkg->cnt].status = d.status
    FOOT REPORT
     stat = alterlist(drpd_pkg->pkg,drpd_pkg->cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "dm2_rpt_pkg_diff completed."
 CALL final_disp_msg("dm2_rpt_pkg_diff")
END GO
