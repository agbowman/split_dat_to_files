CREATE PROGRAM dm2_dup_chg_child:dba
 ...
 ENDIF
 DECLARE ddcr_child_id = f8
 SET ddcr_child_id =  $1
 UPDATE  FROM dm_chg_log d
  SET d.log_type = "DUPROW"
  WHERE ((d.log_id+ 0) > ddcr_child_id)
   AND (d.pk_where_value=dup_holder->qual[1].pk_value)
   AND (d.pk_where=dup_holder->qual[1].pk_where)
   AND ((d.target_env_id+ 0)=dup_holder->qual[1].target_env_id)
   AND (cnvtupper(d.table_name)=dup_holder->qual[1].table_name)
   AND (d.delete_ind=dup_holder->qual[1].del_ind)
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error deleting duplicate pk_where rows"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
