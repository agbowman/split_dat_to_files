CREATE PROGRAM cp_del_wkf_component:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Workflow Component Id" = 0.0
  WITH outdev, wkf_comp_id
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 DECLARE wkf_component_id = f8 WITH protect, constant( $WKF_COMP_ID)
 DECLARE main(null) = null
 DECLARE inactivateworkflowcomponent(null) = null
 RECORD reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
 ...
 SUBROUTINE (reporterrorandexit(operationname=vc,targetobjectname=vc,targetobjectvalue=vc) =null)
  ...
  GO TO exit_script
 END ;Subroutine
 SUBROUTINE (reporterror(operationname=vc,targetobjectname=vc,targetobjectvalue=vc) =null)
   SET reply->status_data.status = "F"
 ...
 END ;Subroutine
 SUBROUTINE inactivateworkflowcomponent(null)
  DELETE  FROM wkf_component wc
   WHERE wc.wkf_component_id=wkf_component_id
   WITH nocounter
  ;end delete
  IF (curqual=0)
   CALL reporterrorandexit("PERFORM","Inactivate","WKF_COMPONENT_ID is invalid.")
  ELSE
   COMMIT
  ENDIF
 END ;Subroutine
 SUBROUTINE main(null)
   IF (wkf_component_id > 0)
    CALL inactivateworkflowcomponent(null)
   ELSE
    CALL reporterrorandexit("PERFORM","main","WKF_COMPONENT_ID is undefined.")
   ENDIF
 END ;Subroutine
 SET reply->status_data.status = "F"
 CALL main(null)
