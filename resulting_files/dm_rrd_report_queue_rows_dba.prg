CREATE PROGRAM dm_rrd_report_queue_rows:dba
 ...
 SET reply->table_name = "REPORT_QUEUE"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(rq.output_handle_id)
   FROM report_queue rq
   WHERE rq.output_handle_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(rq.output_handle_id)
  FROM report_queue rq
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    rq.rowid
    FROM report_queue rq
    WHERE parser(sbr_getrowidnotexists("rq.output_handle_id between curMinID and curMaxID","rq"))
     AND ((rq.converted_file_name="UNCONVERTED") OR (rq.converted_file_name="INPROCESS=>*"))
     AND rq.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rows += 1
     IF (mod(rows,100)=1)
      stat = alterlist(reply->rows,(rows+ 99))
     ENDIF
     reply->rows[rows].row_id = rq.rowid
    WITH nocounter, maxqual(rq,value(rowsleft))
   ;end select
