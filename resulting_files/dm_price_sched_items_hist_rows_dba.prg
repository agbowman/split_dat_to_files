CREATE PROGRAM dm_price_sched_items_hist_rows:dba
 ...
 SET reply->table_name = "PRICE_SCHED_ITEMS_HIST"
 ...
 IF (v_days_to_keep < 30)
  ...
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"k1",
   "You must keep at least 30 days worth of data.  You entered %1 days.","i",v_days_to_keep)
  GO TO exit_script
 ENDIF
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(psih.price_sched_items_hist_id)
   FROM price_sched_items_hist psih
   WHERE psih.price_sched_items_hist_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(psih.price_sched_items_hist_id)
  FROM price_sched_items_hist psih
  WHERE psih.price_sched_items_hist_id > 0.0
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    FROM price_sched_items_hist psih
    WHERE psih.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
     AND psih.permanent_del_ind=1
     AND psih.price_sched_items_hist_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("psih.price_sched_items_hist_id != 0","psih"))
    DETAIL
     row_cnt = (row_cnt+ 1)
     IF (mod(row_cnt,100)=1)
      stat = alterlist(reply->rows,(row_cnt+ 99))
     ENDIF
     reply->rows[row_cnt].row_id = psih.rowid
    WITH nocounter, maxqual(psih,value(rowsleft))
   ;end select
