CREATE PROGRAM ec_profiler_m18:dba
 ...
 SET clinician_var = uar_get_code_by("MEANING",30200,"CLINICIAN")
 SELECT INTO "nl"
  FROM code_value cv,
   track_group tg,
   tracking_checkin tc,
  ...
  PLAN (cv
   ...
   (SELECT
    ...
    WHERE ((ce.encntr_id+ 0)=ti.encntr_id)
     AND ce.person_id=ti.person_id
     AND ce.event_end_dt_tm BETWEEN cnvtdatetime(request->start_dt_tm) AND cnvtdatetime(request->
     stop_dt_tm)
     AND ce.event_end_dt_tm BETWEEN tc.checkin_dt_tm AND tc.checkout_dt_tm
     AND ce.source_cd=clinician_var
    WITH maxqual(ce,1))))
   JOIN (lg1
