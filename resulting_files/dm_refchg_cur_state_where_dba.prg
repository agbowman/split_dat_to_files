CREATE PROGRAM dm_refchg_cur_state_where:dba
 ...
 SUBROUTINE drcs_get_func_name(dgfn_table_name,dgfn_type)
   ...
   RETURN(dgfn_func_str)
 END ;Subroutine
 IF (check_logfile("DM_REFCHG_CSW",".log","DM_REFCHG_CSW LogFile")=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DECLARE drcsw_loop = i4 WITH protect
 DECLARE drcsw_func_name = vc WITH protect, noconstant(" ")
 DECLARE drcsw_table_name = vc WITH protect, noconstant(" ")
 SET drcsw_table_name = cnvtupper(trim( $1,3))
 SET drcsw_func_name = drcs_get_func_name(drcsw_table_name,"PK_WHERE")
 IF (check_error(dm_err->eproc)=1)
  SET drgqt_reply->err_ind = 1
  SET drgqt_reply->err_msg = dm_err->emsg
  GO TO exit_program
 ELSEIF (drcsw_func_name <= " ")
  SET drgqt_reply->err_ind = 1
 ...
 SET drgqt_request->tbl_name = drcsw_table_name
 ...
 IF ((refchg_collist_query->err_ind=1))
  ...
  GO TO exit_program
 ENDIF
 SET drgqt_reply->pkw_template = concat(drcsw_func_name,"(")
 FOR (drcsw_loop = 1 TO size(refchg_collist_query->qual,5))
   SET drgqt_reply->pkw_template = concat(drgqt_reply->pkw_template,"i_",refchg_collist_query->qual[
    drcsw_loop].column_name,",")
 ENDFOR
 SET drgqt_reply->pkw_template = concat(drgqt_reply->pkw_template,"i_db_link,i_cs_pk_ind)")
#exit_program
END GO
