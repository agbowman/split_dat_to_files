CREATE PROGRAM bhs_rw_cam_picu_genview:dba
 ...
 ELSE
  ...
  GO TO exit_script
 ENDIF
 SET work->form_desc = "Confusion Assessment Method for Pedi ICU"
 SELECT INTO "NL:"
  e.person_id
  FROM encounter e
  PLAN (e
 ...
 IF ((work->s_cnt=0))
  ...
  GO TO print_output
 ENDIF
 SET reply->text = build2(beg_doc,"\fs20\b\ul Confusion Assessment Method (PCAM-ICU)\b0\ul0\par",
  newline)
