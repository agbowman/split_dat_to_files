CREATE PROGRAM bhs_req_preg_layout:dba
 PROMPT
  "program_name" = ""
  WITH program_name
 ...
 DECLARE mf_reqstartdttm_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",16449,
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   action_type_cd IN (activatestudentorder_cd, activate_cd, modify_cd))
    ...
    AND od.oe_field_id IN (mf_icd9_cd, mf_consulting_md)) OR (od.oe_field_id=mf_reqstartdttm_cd)) ))
   )
   JOIN (of1
  ...
  DETAIL
   ...
   IF (odflag=1)
    ...
    IF (od.oe_field_meaning="REQSTARTDTTM")
     orders->qual[d1.seq].d_qual[dc].clin_line_ind = 1
    ELSE
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SET ms_filename = build2("bhs_req_preg_",trim(new_timedisp),".dat")
  ...
  SUBROUTINE initializereport(dummy)
    ...
    SET rptreport->m_reportname = "BHS_REQ_PREG_LAYOUT"
