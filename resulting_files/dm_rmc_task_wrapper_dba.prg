CREATE PROGRAM dm_rmc_task_wrapper:dba
 ...
 SUBROUTINE get_lock(i_info_domain,i_info_name,i_retry_limit,io_reply)
   ...
   ENDIF
 END ;Subroutine
 DECLARE drtw_process_name = vc WITH protect, noconstant("")
 DECLARE drtw_task_level = i4 WITH protect, noconstant(0)
 DECLARE drtw_task = vc WITH protect, noconstant("")
 DECLARE drtw_task_id = f8 WITH protect, noconstant(0.0)
 DECLARE drtw_task_request = vc WITH protect, noconstant("")
 DECLARE drtw_task_reply = vc WITH protect, noconstant("")
 DECLARE drtw_done_ind = i4 WITH protect, noconstant(0)
 DECLARE drtw_status = c1 WITH protect, noconstant("F")
 DECLARE drtw_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drtw_process_cnt = i4 WITH protect, noconstant(0)
 DECLARE drtw_error_cnt = i4 WITH protect, noconstant(0)
 DECLARE drtw_retry_cnt = i4 WITH protect, noconstant(0)
 SET dm_err->eproc = "Starting dm_rmc_task_wrapper"
 IF (check_logfile("dm_rmc_task_wrp",".log","DM_RMC_TASK_WRP LOG")=0)
  SET dm_err->err_ind = 1
 ...
 IF (reflect(parameter(1,0)) != "C*")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Expected syntax:  dm_rmc_task_wrapper <process_name>"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET drtw_process_name = cnvtupper( $1)
 IF (((validate(drtw_dbun,"-1")="-1") OR (((validate(drtw_dbpw,"-1")="-1") OR (validate(drtw_dbcon,
  "-1")="-1")) )) )
  DECLARE drtw_dbun = vc WITH protect, noconstant("NOT_SET")
  DECLARE drtw_dbpw = vc WITH protect, noconstant("NOT_SET")
  DECLARE drtw_dbcon = vc WITH protect, noconstant("NOT_SET")
 ENDIF
 SET dm_err->eproc = "Gathering environment_id."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="DM_ENV_ID"
  DETAIL
   drtw_env_id = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 IF (drtw_env_id <= 0.0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "ENVIRONMENT_ID NOT FOUND"
  SET dm_err->user_action = "PLEASE RUN DM_SET_ENV_ID"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Updating any dead tasks back to QUEUED status."
 UPDATE  FROM dm_refchg_task_queue d
  SET d.task_status = "QUEUED", d.task_retry_cnt = (d.task_retry_cnt - 1), d.rdbhandle_value = - (1),
   d.log_file = null, d.begin_dt_tm = null, d.end_dt_tm = null,
   d.updt_id = reqinfo->updt_id, d.updt_dt_tm = cnvtdatetime(curdate,curtime3), d.updt_applctx =
   reqinfo->updt_applctx,
   d.updt_task = reqinfo->updt_task, d.updt_cnt = (d.updt_cnt+ 1)
 ...
 IF (curqual > 0)
  UPDATE  FROM dm_refchg_task_queue d
   SET d.task_status = "ERROR", d.error_text =
    "Task has failed on all retries, session id remains inactive"
   WHERE (d.rdbhandle_value=- (1))
    AND d.task_status="QUEUED"
    AND d.task_retry_cnt=0
   WITH nocounter
  ;end update
  IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Gathering next task to run."
 SELECT INTO "nl:"
  FROM dm_refchg_task_queue d
  WHERE d.process_name=patstring(drtw_process_name)
   AND d.task_status="READY"
  DETAIL
   drtw_task_level = d.task_level, drtw_task = d.task_name, drtw_task_id = d.dm_refchg_task_queue_id,
   drtw_task_request = d.task_request, drtw_task_reply = d.task_reply
  WITH nocounter, forupdatewait(d), maxqual(d,1)
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (curqual=1)
  SET drtw_status = drtq_update_task_process(drtw_task_id,dm_err->logfile,"RUNNING","")
  IF (drtw_status="F")
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = concat("Executing task ",drtw_task)
  CALL disp_msg(" ",dm_err->logfile,0)
  EXECUTE dm_rmc_execute_task drtw_task, drtw_task_request, drtw_task_reply,
  drtw_task_id
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ENDIF
 ENDIF
 SET dm_err->eproc = "Updating QUEUED rows to be READY."
 UPDATE  FROM dm_refchg_task_queue d
  SET d.task_status = "READY", d.updt_id = reqinfo->updt_id, d.updt_dt_tm = cnvtdatetime(curdate,
    curtime3),
   d.updt_applctx = reqinfo->updt_applctx, d.updt_task = reqinfo->updt_task, d.updt_cnt = (d.updt_cnt
   + 1)
  WHERE d.process_name=patstring(drtw_process_name)
   AND d.task_status="QUEUED"
   AND d.task_level IN (
  (SELECT
   ...
   WHERE r.process_name=d.process_name
    AND r.task_status="QUEUED"))
   AND  NOT ( EXISTS (
  (SELECT
   "x"
   FROM dm_refchg_task_queue t
   WHERE t.process_name=d.process_name
    AND t.task_status IN ("ERROR", "READY", "RUNNING")
    AND t.task_level <= d.task_level)))
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 SET stat = initrec(auto_ver_request)
 SET stat = initrec(auto_ver_reply)
 FOR (drtw_retry_cnt = 1 TO 5)
   SET drl_reply->status = ""
   SET drl_reply->status_msg = ""
   CALL get_lock("RDDS LOG TASK ERROR",drtw_process_name,0,drl_reply)
   ...
   ELSEIF ((drl_reply->status="Z"))
    CALL pause(10)
   ELSE
    SET dm_err->eproc = "Checking for tasks in ERROR"
    SELECT INTO "nl:"
     FROM dm_refchg_task_queue d
     WHERE d.process_name=patstring(drtw_process_name)
      AND d.task_status="ERROR"
      AND (d.begin_dt_tm >=
     (SELECT
      nullval(max(event_dt_tm),cnvtdatetime("01-JAN-1800 00:00:00"))
      FROM dm_rdds_event_log l
      WHERE l.cur_environment_id=drtw_env_id
       AND l.rdds_event_key="TASKQUEUEERROR"
       AND l.event_reason=d.process_name))
     ORDER BY d.process_name
     HEAD REPORT
      drtw_process_cnt = 0
     HEAD d.process_name
      drtw_process_cnt = (drtw_process_cnt+ 1), stat = alterlist(auto_ver_request->qual,
       drtw_process_cnt), auto_ver_request->qual[drtw_process_cnt].rdds_event = "Task Queue Error",
      auto_ver_request->qual[drtw_process_cnt].cur_environment_id = drtw_env_id, auto_ver_request->
      qual[drtw_process_cnt].paired_environment_id = 0, auto_ver_request->qual[drtw_process_cnt].
      event_reason = d.process_name,
      drtw_error_cnt = 0
     DETAIL
      drtw_error_cnt = (drtw_error_cnt+ 1), stat = alterlist(auto_ver_request->qual[drtw_process_cnt]
       .detail_qual,drtw_error_cnt), auto_ver_request->qual[drtw_process_cnt].detail_qual[
      drtw_error_cnt].event_detail1_txt = d.task_desc,
      auto_ver_request->qual[drtw_process_cnt].detail_qual[drtw_error_cnt].event_detail2_txt = d
      .error_text, auto_ver_request->qual[drtw_process_cnt].detail_qual[drtw_error_cnt].
      event_detail3_txt = d.log_file
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     SET drl_reply->status = ""
     SET drl_reply->status_msg = ""
     CALL remove_lock("RDDS LOG TASK ERROR",drtw_process_name,currdbhandle,drl_reply)
     GO TO exit_program
    ENDIF
    IF (size(auto_ver_request->qual,5) > 0)
     EXECUTE dm_rmc_auto_verify_setup
     IF ((auto_ver_reply->status="F"))
      ROLLBACK
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    SET drl_reply->status_msg = ""
    CALL remove_lock("RDDS LOG TASK ERROR",drtw_process_name,currdbhandle,drl_reply)
    SET drtw_retry_cnt = 5
   ENDIF
 ENDFOR
 FOR (drtw_retry_cnt = 1 TO 5)
   SET drl_reply->status = ""
   SET drl_reply->status_msg = ""
   CALL get_lock("RDDS LOG TASK FINISHED",drtw_process_name,0,drl_reply)
   IF ((drl_reply->status="F"))
    CALL disp_msg(drl_reply->status_msg,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    GO TO exit_program
   ELSEIF ((drl_reply->status="Z"))
    CALL pause(10)
   ELSE
    SET dm_err->eproc = "Checking if all tasks are FINISHED."
    SELECT DISTINCT INTO "nl:"
     d.process_name
     FROM dm_refchg_task_queue d
     WHERE d.process_name=patstring(drtw_process_name)
      AND d.task_status="FINISHED"
      AND  NOT ( EXISTS (
     (SELECT
      "x"
      FROM dm_rdds_event_log l
      WHERE l.cur_environment_id=drtw_env_id
       AND l.rdds_event_key="TASKQUEUEFINISHED"
       AND l.event_reason=d.process_name
       AND l.event_dt_tm >= d.end_dt_tm)))
      AND  NOT ( EXISTS (
     (SELECT
      "x"
      FROM dm_refchg_task_queue r
      WHERE r.process_name=d.process_name
       AND r.task_status != "FINISHED")))
     HEAD REPORT
      drtw_process_cnt = 0
     DETAIL
      drtw_process_cnt = (drtw_process_cnt+ 1), stat = alterlist(auto_ver_request->qual,
       drtw_process_cnt), auto_ver_request->qual[drtw_process_cnt].rdds_event = "Task Queue Finished",
      auto_ver_request->qual[drtw_process_cnt].cur_environment_id = drtw_env_id, auto_ver_request->
      qual[drtw_process_cnt].paired_environment_id = 0, auto_ver_request->qual[drtw_process_cnt].
      event_reason = d.process_name
     WITH nocounter
    ...
    IF (check_error(dm_err->eproc)=1)
     ...
     SET drl_reply->status = ""
     SET drl_reply->status_msg = ""
     CALL remove_lock("RDDS LOG TASK FINISHED",drtw_process_name,currdbhandle,drl_reply)
     GO TO exit_program
    ENDIF
    IF (size(auto_ver_request->qual,5) > 0)
     EXECUTE dm_rmc_auto_verify_setup
 ...
 ENDFOR
#exit_program
 SET dm_err->eproc = "Finished executing dm_rmc_task_wrapper."
 CALL final_disp_msg("dm_rmc_task_wrp")
END GO
