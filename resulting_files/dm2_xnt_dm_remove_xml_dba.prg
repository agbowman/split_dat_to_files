CREATE PROGRAM dm2_xnt_dm_remove_xml:dba
 ...
 DECLARE dxrx_uid = vc WITH protect, noconstant("")
 DECLARE dxrx_version = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_xnt_remove",".log","DM2_XNT_REMOVE Log...") != 1)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting script to remove XML media from CAMM database"
 IF (((reflect(parameter(2,0)) != "I*"
  AND reflect(parameter(2,0)) != "F*") OR (reflect(parameter(1,0)) != "C*")) )
  SET dm_err->err_ind = 1
  SET (dm - err->emsg) = "Expected syntax: dm2_xnt_dm_remove_xml <UUID>, <VERSION>"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ELSE
  SET dxrx_uid =  $1
  SET dxrx_version =  $2
 ENDIF
 DECLARE uar_dmsm_deletemediaobject(p1=vc(ref),p2=i4(value)) = i4 WITH image_axp = "dmsmanagement",
 image_aix = "libdmsmanagement.a(libdmsmanagement.o)", image_win = "dmsmanagement",
 uar = "DMSM_DeleteMediaObject", persist
 SET dxrx_status = uar_dmsm_deletemediaobject(nullterm(dxrx_uid),dxrx_version)
 IF (check_error("Removing media from CAMM") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
#exit_script
