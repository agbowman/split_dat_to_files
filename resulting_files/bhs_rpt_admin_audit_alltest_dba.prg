CREATE PROGRAM bhs_rpt_admin_audit_alltest:dba
 PROMPT
  "output:" = "MINE",
 ...
 IF (( $RUNTYPE=0))
  SELECT INTO value(var_output)
   ...
   schedule_code = "n/a", admin_datetime = substring(1,20,work->encntrs[d.seq].meds[d1.seq].ordevent[
    d2.seq].admin_datetime), drug_cost = 0,
   status =
 ...
 ELSE
  ...
  IF (gl_bhs_prod_flag=1)
   ...
   SET ipaddress = "172.17.3.15"
   SET folderdir = "/tempfiles/hold/egate/premier"
   SET username = "egprod2"
  ...
  CALL ftpfile(filenamein,filenameout,ipaddress,folderdir,username,
   password)
  IF (findfile(trim(filenamein))=1)
   SET stat = remove(trim(filenamein))
   IF (stat=0)
