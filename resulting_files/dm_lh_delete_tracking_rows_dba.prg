CREATE PROGRAM dm_lh_delete_tracking_rows:dba
 ...
 SET reply->table_name = "LH_DELETE_TRACKING"
 ...
 IF (v_days_to_keep < 30)
  ...
  GO TO exit_script
 ENDIF
 SET v_max_day = datetimeadd(v_cur_day,(v_days_to_keep * - (1)))
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(ldt.lh_delete_tracking_id)
   FROM lh_delete_tracking ldt
   WHERE ldt.lh_delete_tracking_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(ldt.lh_delete_tracking_id)
  FROM lh_delete_tracking ldt
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    ldt.rowid
    FROM lh_delete_tracking ldt
    WHERE ldt.lh_delete_tracking_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "ldt.updt_dt_tm between cnvtdatetime(v_min_day) and cnvtdatetime(v_max_day)","ldt"))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = ldt.rowid
    WITH nocounter, maxqual(ldt,value(v_rows_left))
   ;end select
