CREATE PROGRAM dm_txtfnd_pm_worklist
 ...
 SUBROUTINE dst_add_log_det(dald_log_id,dald_status,dald_msg,dald_start_dt_tm,dald_end_dt_tm)
   ...
   RETURN(0)
 END ;Subroutine
 DECLARE dtpw_cnt = i4 WITH protect, noconstant(0)
 DECLARE dtpw_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dtpw_emsg = vc WITH protect, noconstant(" ")
 DECLARE dtpw_idx = i4 WITH protect, noconstant(0)
 DECLARE dtpw_num = i4 WITH protect, noconstant(0)
 DECLARE dtpw_pos = i4 WITH protect, noconstant(0)
 FREE RECORD dtpw_attrib
 RECORD dtpw_attrib(
   1 cnt = i4
   1 qual[*]
     2 column_name = vc
 ...
 IF (check_logfile("dm_tf_pm_wklst",".log","DM_TXTFND_PM_WORKLIST LOG")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to Create Log File"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM_TXTFND_PM_WORKLIST..."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 IF (dtla_detail_id=0.0)
  ...
  SELECT INTO "nl:"
   ...
   WHERE d.detail_type_flag=2
    AND d.active_ind=1
    AND d.detail_script_name="DM_TXTFND_PM_WORKLIST"
   DETAIL
  ...
  IF (curqual=0)
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "No active data found in DM_TEXT_FIND_DETAIL for script DM_TXTFND_PM_WORKLIST."
   IF ((script_rec->dm_text_find_log_id > 0.0))
    CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",dm_err->emsg,0.0,0.0)
   ENDIF
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM_TXTFND_ATTRIBUTE:PM_QUE_WORK_LIST"
   AND  EXISTS (
  (SELECT
   "x"
   FROM user_tab_columns utc
   WHERE utc.table_name="PM_QUE_WORK_LIST"
    AND utc.column_name=di.info_name))
  ORDER BY di.info_name
  DETAIL
   dtpw_attrib->cnt = (dtpw_attrib->cnt+ 1), stat = alterlist(dtpw_attrib->qual,dtpw_attrib->cnt),
   dtpw_attrib->qual[dtpw_attrib->cnt].column_name = di.info_name,
   dtpw_attrib->qual[dtpw_attrib->cnt].column_label = di.info_char, dtpw_attrib->qual[dtpw_attrib->
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No attribute data found for PM_QUE_WORK_LIST."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat("Querying for Attribute list:",
    dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ...
 SET dm_err->eproc = "Gathering PM WORKLIST scripts"
 SELECT DISTINCT INTO "nl:"
  pw.script
  FROM pm_que_work_list pw
  WHERE pw.script IS NOT null
   AND pw.script > " "
  HEAD REPORT
   script_rec->dm_text_find_detail_id = dtla_detail_id, dtpw_cnt = 0
  DETAIL
   dtpw_cnt = (dtpw_cnt+ 1)
   IF (mod(dtpw_cnt,10)=1)
    stat = alterlist(dtg->qual,(dtpw_cnt+ 9))
   ENDIF
   dtg->qual[dtpw_cnt].s_name = pw.script
  FOOT REPORT
   dtg->cnt = dtpw_cnt, stat = alterlist(dtg->qual,dtg->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for PM Worklist scripts:",dm_err->emsg),0.0,0.0)
  SET dtpw_err_ind = dm_err->err_ind
  SET dtpw_emsg = dm_err->emsg
  SET dm_err->err_ind = 0
 ENDIF
 IF ((dtg->cnt > 0)
  AND dtpw_err_ind=0)
  SET dm_err->eproc = "Gathering attribute information from PM_QUE_WORK_LIST"
  CALL parser("select into 'nl:'",0)
  CALL parser("  from pm_que_work_list pw",0)
  CALL parser(" where expand(dtpw_num, 1, dtg->cnt,pw.script, dtg->qual[dtpw_num].s_name)",0)
  CALL parser(" order by pw.script, pw.updt_dt_tm desc")
  CALL parser("head pw.script",0)
  CALL parser("   dtpw_pos = locateval(dtpw_num, 1, dtg->cnt,pw.script, dtg->qual[dtpw_num].s_name)",
   0)
  CALL parser("detail",0)
  CALL parser(" if (dtg->qual[dtpw_pos].data_source <= ' ')",0)
  CALL parser("   dtg->qual[dtpw_pos].data_source = pw.display",0)
  FOR (dtpw_idx = 1 TO dtpw_attrib->cnt)
    CALL parser("   dtg->qual[dtpw_pos].att_cnt = dtg->qual[dtpw_pos].att_cnt + 1",0)
    CALL parser("   stat = alterlist(dtg->qual[dtpw_pos].att_qual, dtg->qual[dtpw_pos].att_cnt)",0)
    CALL parser(concat(
      "      dtg->qual[dtpw_pos].att_qual[dtg->qual[dtpw_pos].att_cnt].tab_name = 'PM_QUE_WORK_LIST'"
      ),0)
    CALL parser(concat(
      "   dtg->qual[dtpw_pos].att_qual[dtg->qual[dtpw_pos].att_cnt].col_name = dtpw_attrib->qual[",
      trim(cnvtstring(dtpw_idx)),"].column_name"),0)
    CALL parser(concat(
      "   dtg->qual[dtpw_pos].att_qual[dtg->qual[dtpw_pos].att_cnt].col_txt = dtpw_attrib->qual[",
      trim(cnvtstring(dtpw_idx)),"].column_label"),0)
    IF ((dtpw_attrib->qual[dtpw_idx].column_flg=3))
     CALL parser(concat(
       "   dtg->qual[dtpw_pos].att_qual[dtg->qual[dtpw_pos].att_cnt].col_val = format(pw.",
       dtpw_attrib->qual[dtpw_idx].column_name,",';;q')"),0)
    ELSEIF ((dtpw_attrib->qual[dtpw_idx].column_flg=2))
     CALL parser(concat(
       "   dtg->qual[dtpw_pos].att_qual[dtg->qual[dtpw_pos].att_cnt].col_val = cnvtstring(pw.",
       dtpw_attrib->qual[dtpw_idx].column_name,",20)"),0)
    ELSE
     CALL parser(concat("   dtg->qual[dtpw_pos].att_qual[dtg->qual[dtpw_pos].att_cnt].col_val = pw.",
       dtpw_attrib->qual[dtpw_idx].column_name),0)
    ENDIF
  ENDFOR
  CALL parser(" endif with nocounter, expand = 1 go",1)
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SET stat = alterlist(script_rec->script_qual,0)
 SET stat = initrec(dtg)
 SET dm_err->eproc = "Gathering PM_WORKLIST FKs to LONG_TEXT_REFERENCE"
 SELECT DISTINCT INTO "nl:"
  pw.exec_script_long_text_id
  FROM pm_que_work_list pw
  WHERE pw.exec_script_long_text_id > 0.0
   AND  EXISTS (
  (SELECT
   "x"
   FROM long_text_reference l
   WHERE l.long_text_id=pw.exec_script_long_text_id))
  HEAD REPORT
   script_rec->dm_text_find_detail_id = dtla_detail_id, dtpw_cnt = 0
  DETAIL
   dtpw_cnt = (dtpw_cnt+ 1)
   IF (mod(dtpw_cnt,10)=1)
    stat = alterlist(dtg->qual,(dtpw_cnt+ 9))
   ENDIF
   dtg->qual[dtpw_cnt].pe_name = "LONG_TEXT_REFERENCE", dtg->qual[dtpw_cnt].pe_id = pw
   .exec_script_long_text_id, dtg->qual[dtpw_cnt].pe_col = "LONG_TEXT_ID",
   dtg->qual[dtpw_cnt].srch_col = "LONG_TEXT"
  FOOT REPORT
   dtg->cnt = dtpw_cnt, stat = alterlist(dtg->qual,dtg->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL dst_add_log_det(script_rec->dm_text_find_log_id,"ERROR",concat(
    "Querying for LONG_TEXT_REFERENCE FKs:",dm_err->emsg),0.0,0.0)
  GO TO exit_script
 ENDIF
 IF ((dtg->cnt > 0))
  SET dm_err->eproc = "Gathering attribute information from PM_QUE_WORK_LIST"
  CALL parser("select into 'nl:'",0)
  CALL parser("  from pm_que_work_list pw",0)
  CALL parser(" where expand(dtpw_num, 1, dtg->cnt,pw.exec_script_long_text_id, ",0)
  CALL parser("          dtg->qual[dtpw_num].pe_id)",0)
  CALL parser(" order by pw.exec_script_long_text_id, pw.updt_dt_tm desc ",0)
  CALL parser("head pw.exec_script_long_text_id",0)
  CALL parser(
   "   dtpw_pos = locateval(dtpw_num, 1, dtg->cnt,pw.exec_script_long_text_id,dtg->qual[dtpw_num].pe_id)",
   0)
  CALL parser("detail",0)
  CALL parser(" if (dtg->qual[dtpw_pos].data_source <= ' ')",0)
  CALL parser("   dtg->qual[dtpw_pos].data_source = pw.display",0)
 ...
 ENDIF
#exit_script
 SET dm_err->eproc = "...Ending DM_TXTFND_PM_WORKLIST"
 CALL final_disp_msg("dm_tf_pm_wklst")
 IF (dtpw_err_ind=1
  AND (dm_err->err_ind=0))
  SET dm_err->err_ind = dtpw_err_ind
  SET dm_err->emsg = dtpw_emsg
 ENDIF
END GO
