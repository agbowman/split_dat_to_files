CREATE PROGRAM dm_rmc_dclcur_backfill:dba
 ...
 END ;Subroutine
 IF (check_logfile("DM_RMC_DCLCUR",".log","DM_RMC_DCLCUR LogFile")=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO end_program
 ENDIF
 IF ((validate(drdb_request->src_env_id,- (99))=- (99)))
  FREE RECORD drdb_request
  RECORD drdb_request(
    1 src_env_id = f8
    1 trg_env_id = f8
  )
  SET drdb_request->src_env_id =  $1
  SET drdb_request->trg_env_id =  $2
 ENDIF
 IF (validate(drdb_reply->status,"-99")="-99")
  FREE RECORD drdb_reply
  RECORD drdb_reply(
    1 status = vc
    1 err_msg = vc
  )
 ENDIF
 FREE RECORD drdb_tbls
 RECORD drdb_tbls(
   1 cnt = i4
   1 list[*]
     2 table_name = vc
 ...
 FREE RECORD drcp_request
 RECORD drcp_request(
   1 table_name = vc
   1 pk_string = vc
 )
 FREE RECORD drcp_reply
 RECORD drcp_reply(
   1 status = vc
   1 err_msg = vc
 )
 DECLARE drdb_info_name_str = vc
 DECLARE drdb_hash_loop = i4
 DECLARE drdb_qual = i4 WITH noconstant(100000)
 DECLARE drdb_main_loop = i4
 DECLARE drdb_main_ind = i2
 DECLARE drdb_log_loop = i4
 DECLARE drdb_ptam_result = f8
 IF ((drdb_request->trg_env_id <= 0))
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="DATA MANAGEMENT"
    AND di.info_name="DM_ENV_ID"
   DETAIL
    drdb_request->trg_env_id = di.info_number
   WITH nocounter
  ;end select
  IF (check_error("Obtaining target_id") != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   SET drdb_reply->status = "F"
   SET drdb_reply->err_msg = dm_err->emsg
   GO TO end_program
  ENDIF
  IF (curqual=0)
   SET dm_err->err_ind = 1
  ...
  ENDIF
 ENDIF
 SET drdb_info_name_str = concat(trim(cnvtstring(drdb_request->src_env_id)),trim(cnvtstring(
    drdb_request->trg_env_id)))
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="RDDS_CURSTATE_BACKFILL"
   AND di.info_name=drdb_info_name_str
  WITH nocounter
 ;end select
 IF (check_error("Check for RDDS_CURSTATE_BACKFILL dm_info row") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  SET drdb_reply->status = "F"
  SET drdb_reply->err_msg = dm_err->emsg
  GO TO end_program
 ENDIF
 IF (curqual=1)
  SET drdb_reply->status = "S"
  SET drdb_reply->err_msg = "RDDS_CURSTATE_BACKFILL row already exist, so backfill will not run"
  GO TO end_program
 ENDIF
 SELECT DISTINCT INTO "nl:"
  dcl.table_name
  FROM dm_chg_log dcl
  WHERE (dcl.target_env_id=drdb_request->src_env_id)
   AND log_type IN ("REFCHG", "NORDDS")
   AND dcl.delete_ind=0
  HEAD REPORT
   drdb_tbls->cnt = 0
  DETAIL
   drdb_tbls->cnt = (drdb_tbls->cnt+ 1)
   IF (mod(drdb_tbls->cnt,100)=1)
    stat = alterlist(drdb_tbls->list,(drdb_tbls->cnt+ 99))
   ENDIF
   drdb_tbls->list[drdb_tbls->cnt].table_name = dcl.table_name, drdb_tbls->list[drdb_tbls->cnt].
   vers_str = "AAA", drdb_tbls->list[drdb_tbls->cnt].pk_hash = - (999),
   drdb_tbls->list[drdb_tbls->cnt].ptam_hash = - (999)
  FOOT REPORT
   stat = alterlist(drdb_tbls->list,drdb_tbls->cnt)
  WITH nocounter
 ;end select
 IF ((drdb_tbls->cnt > 0))
  DECLARE drdb_idx = i4
  DECLARE drdb_idx2 = i4
  SELECT INTO "nl:"
   FROM dm_refchg_pkw_vers drpv
   WHERE expand(drdb_idx,1,drdb_tbls->cnt,drpv.table_name,drdb_tbls->list[drdb_idx].table_name)
    AND drpv.active_ind=1
   DETAIL
    drdb_idx2 = 0, drdb_idx2 = locateval(drdb_idx,1,drdb_tbls->cnt,drpv.table_name,drdb_tbls->list[
     drdb_idx].table_name)
    IF (drdb_idx2 > 0)
     drdb_tbls->list[drdb_idx2].vers_str = drpv.pkw_format, drdb_tbls->list[drdb_idx2].vers_id = drpv
     .dm_refchg_pkw_vers_id
    ENDIF
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   FROM dm_pk_where dpw
   WHERE expand(drdb_idx,1,drdb_tbls->cnt,dpw.table_name,drdb_tbls->list[drdb_idx].table_name)
    AND dpw.delete_ind=0
   DETAIL
    drdb_idx2 = 0, drdb_idx2 = locateval(drdb_idx,1,drdb_tbls->cnt,dpw.table_name,drdb_tbls->list[
     drdb_idx].table_name)
    IF (drdb_idx2 > 0)
     drdb_tbls->list[drdb_idx2].pk_hash = dpw.pk_where_hash, drdb_tbls->list[drdb_idx2].ptam_hash =
     dpw.ptam_match_hash
    ENDIF
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   ...
   WHERE expand(drdb_idx,1,drdb_tbls->cnt,dtdl.table_name,drdb_tbls->list[drdb_idx].table_name)
   DETAIL
    drdb_idx2 = 0, drdb_idx2 = locateval(drdb_idx,1,drdb_tbls->cnt,dtdl.table_name,drdb_tbls->list[
     drdb_idx].table_name)
    IF (drdb_idx2 > 0)
     drdb_tbls->list[drdb_idx2].table_suffix = dtdl.table_suffix
    ENDIF
   WITH nocounter
  ;end select
 ENDIF
 FOR (drdb_hash_loop = 1 TO drdb_tbls->cnt)
   SET drcp_request->table_name = drdb_tbls->list[drdb_hash_loop].table_name
   SET drcp_request->pk_string = drdb_tbls->list[drdb_hash_loop].vers_str
   EXECUTE dm_rmc_correct_pk_hash  WITH replace("DRC_REQUEST","DRCP_REQUEST"), replace("DRC_REPLY",
    "DRCP_REPLY")
   IF ((drcp_reply->status="S"))
    SET drdb_tbls->list[drdb_hash_loop].valid_ind = 1
   ENDIF
 ENDFOR
 FOR (drdb_main_loop = 1 TO drdb_tbls->cnt)
   IF ((drdb_tbls->list[drdb_main_loop].valid_ind=1))
    SET drdb_main_ind = 0
    WHILE (drdb_main_ind=0)
     UPDATE  FROM dm_chg_log dcl
      SET dcl.pk_where_hash = drdb_tbls->list[drdb_main_loop].pk_hash, dcl.ptam_match_hash =
       drdb_tbls->list[drdb_main_loop].ptam_hash
      WHERE (dcl.table_name=drdb_tbls->list[drdb_main_loop].table_name)
       AND (dcl.target_env_id=drdb_request->src_env_id)
       AND dcl.log_type IN ("REFCHG", "NORDDS")
       AND dcl.delete_ind=0
       AND (dcl.dm_refchg_pkw_vers_id=drdb_tbls->list[drdb_main_loop].vers_id)
       AND (((dcl.pk_where_hash != drdb_tbls->list[drdb_main_loop].pk_hash)) OR (dcl.pk_where_hash =
      null))
       AND (((dcl.ptam_match_hash != drdb_tbls->list[drdb_main_loop].ptam_hash)) OR (dcl
      .ptam_match_hash = null))
      WITH maxqual(dcl,value(drdb_qual))
     ;end update
     IF (check_error("Updating PTAM hash and PK hash") != 0)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      SET drdb_reply->status = "F"
      SET drdb_reply->err_msg = dm_err->emsg
      GO TO end_program
     ELSE
      IF (curqual < drdb_qual)
       SET drdb_main_ind = 1
      ENDIF
     ENDIF
    ENDWHILE
    ...
    WHILE (drdb_main_ind=0)
      SELECT INTO "nl:"
       ...
       WHERE (dcl.table_name=drdb_tbls->list[drdb_main_loop].table_name)
        ...
        AND dcl.delete_ind=0
        AND (dcl.dm_refchg_pkw_vers_id=drdb_tbls->list[drdb_main_loop].vers_id)
        AND (dcl.pk_where_hash=drdb_tbls->list[drdb_main_loop].pk_hash)
        AND (dcl.ptam_match_hash=drdb_tbls->list[drdb_main_loop].ptam_hash)
        AND ((dcl.ptam_match_result=0) OR (dcl.ptam_match_result = null))
       HEAD REPORT
        drdb_log->cnt = 0, stat = alterlist(drdb_log->list,drdb_log->cnt)
       DETAIL
        drdb_log->cnt = (drdb_log->cnt+ 1)
        IF (mod(drdb_log->cnt,100)=1)
         stat = alterlist(drdb_log->list,(drdb_log->cnt+ 99))
        ENDIF
        drdb_log->list[drdb_log->cnt].log_id = dcl.log_id, drdb_log->list[drdb_log->cnt].pk_where =
        dcl.pk_where
       FOOT REPORT
        stat = alterlist(drdb_log->list,drdb_log->cnt)
       WITH maxqual(dcl,1000)
      ;end select
      IF ((drdb_log->cnt < 1000))
       SET drdb_main_ind = 1
      ENDIF
      IF ((drdb_log->cnt != 0))
       FOR (drdb_log_loop = 1 TO drdb_log->cnt)
         CALL drmm_get_col_string(drdb_log->list[drdb_log_loop].pk_where,0.0,drdb_tbls->list[
          drdb_main_loop].table_name,drdb_tbls->list[drdb_main_loop].table_suffix,"")
         CALL drmm_get_ptam_match_query(0.0,drdb_tbls->list[drdb_main_loop].table_name,drdb_tbls->
          list[drdb_main_loop].table_suffix,"")
         SET drdb_ptam_result = drmm_get_ptam_match_result(0.0,0.0,0.0,"",1,
          "FROM")
         UPDATE  FROM dm_chg_log dcl
          SET dcl.col_string = sys_context("CERNER","RDDS_COL_STRING",4000), dcl.ptam_match_query =
           sys_context("CERNER","RDDS_PTAM_MATCH_QUERY",2000), dcl.ptam_match_result =
           drdb_ptam_result
          WHERE (dcl.log_id=drdb_log->list[drdb_log_loop].log_id)
         ;end update
         IF (check_error("Updating PTAM hash and PK hash") != 0)
          CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
          SET drdb_reply->status = "F"
 ...
 INSERT  FROM dm_info di
  SET di.info_domain = "RDDS_CURSTATE_BACKFILL", di.info_name = drdb_info_name_str, di.updt_cnt = 0,
   di.updt_applctx = reqinfo->updt_applctx, di.updt_dt_tm = cnvtdatetime(curdate,curtime3), di
   .updt_task = reqinfo->updt_task,
   di.updt_id = reqinfo->updt_id
  WITH nocounter
 ;end insert
 IF (check_error("Inserting RDDS_CURSTATE_BACKFILL dm_info row") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  ROLLBACK
  SET drdb_reply->status = "F"
  SET drdb_reply->err_msg = dm_err->emsg
