CREATE PROGRAM dm2_mig_custccl_diff
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmccd_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_diff")
 DECLARE dmccd_master_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccd_tgt_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccd_cont = i2 WITH protect, noconstant(1)
 DECLARE dmccd_env = vc WITH protect, noconstant("")
 DECLARE dmccd_node = vc WITH protect, noconstant("")
 DECLARE dmccd_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccd_os_type = vc WITH protect, noconstant("")
 DECLARE dmccd_cnt = i4 WITH protect, noconstant(0)
 FREE RECORD dmccd_cust_scripts
 RECORD dmccd_cust_scripts(
   1 cnt = i4
   1 scripts[*]
     2 dma_dprotect_id = f8
     2 script_name = vc
     2 group_nbr = i4
     2 object_type_flag_txt = vc
 )
 DECLARE dmccd_get_cust_scripts(dgcs_master_id=f8,dgcs_tgt_id=f8) = i2
 SET width = 132
 IF (check_logfile(dmccd_logfile_prefix,".log","dm2_mig_custccl_diff")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_diff"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmccd_env = trim(logical("environment"))
 SET dmccd_node = trim(curnode)
 SET dmccd_os_type = trim(cursys2)
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dmccd_master_id = dena.dma_env_node_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF (dmccd_tgt_id=0)
  SET dm_err->emsg = "A valid entry for TARGET does not exist in the dma_env_node_attr table"
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dmccd_env))
  CALL echo(concat("Master Node/Env Id: ",cnvtstring(dmccd_master_id)))
  CALL echo(concat("Target Node/Env Id: ",cnvtstring(dmccd_tgt_id)))
  CALL echo(concat("Current Node: ",dmccd_node))
 ENDIF
 IF (dmccd_get_cust_scripts(dmccd_master_id,dmccd_tgt_id)=0)
  GO TO exit_program
 ENDIF
 IF ((dmccd_cust_scripts->cnt=0))
  SET dm_err->eproc = "No difference in scripts between MASTER and TARGET"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
  IF ((dm_err->debug_flag=511))
   CALL echorecord(dmccd_cust_scripts)
  ENDIF
  CALL echo(dmccd_cust_scripts->cnt)
 ENDIF
 SET dm_err->eproc =
 "Updating custom and merge indicators for custom scripts in dma_env_node_dprotect"
 CALL disp_msg("",dm_err->logfile,0)
 UPDATE  FROM dma_env_node_dprotect dend,
   (dummyt d  WITH seq = value(dmccd_cust_scripts->cnt))
  SET dend.custom_ind = 1, dend.merge_ind = 1
  PLAN (d
   WHERE d.seq > 0)
   JOIN (dend
   WHERE (dend.dma_dprotect_id=dmccd_cust_scripts->scripts[d.seq].dma_dprotect_id))
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Updating merge indicator for MASTER custom scripts in dma_env_node_dprotect"
 CALL disp_msg("",dm_err->logfile,0)
 UPDATE  FROM dma_env_node_dprotect d
  SET d.merge_ind = 1
  ...
  (SELECT
   di.info_name
   FROM dm_info di
   WHERE di.info_domain="DM2_APPMIG_CUSTCCL_AUTOMERGE"
 ...
 COMMIT
 GO TO exit_program
 SUBROUTINE dmccd_get_cust_scripts(dgcs_master_id,dgcs_tgt_id)
   DECLARE dgcs_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgcs_tgt_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgcs_u_cnt = i4 WITH protect, noconstant(0)
   DECLARE dgcs_ndx = i4 WITH protect, noconstant(0)
   IF (((dgcs_master_id=0) OR (dgcs_tgt_id=0)) )
    SET dm_err->emsg = concat("No Valid MASTER and/or TARGET Information was found. ","Master ID: ",
     dgcs_master_id,"Target ID: ",dgcs_tgt_id)
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Retrieving scripts present in master but not in target"
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dma_env_node_dprotect d1
    WHERE d1.dma_env_node_id=dgcs_master_id
     AND  NOT ( EXISTS (
    (SELECT
     "x"
     FROM dma_env_node_dprotect d2
     WHERE d2.dma_env_node_id=dgcs_tgt_id
      AND d2.object_type_flag_txt=d1.object_type_flag_txt
      AND d2.object_name=d1.object_name
      AND d2.group_nbr=d1.group_nbr)))
    ORDER BY d1.object_name
    HEAD REPORT
     dmccd_cust_scripts->cnt = 0, stat = alterlist(dmccd_cust_scripts->scripts,0)
    DETAIL
     dmccd_cust_scripts->cnt = (dmccd_cust_scripts->cnt+ 1), stat = alterlist(dmccd_cust_scripts->
      scripts,dmccd_cust_scripts->cnt), dmccd_cust_scripts->scripts[dmccd_cust_scripts->cnt].
     dma_dprotect_id = d1.dma_dprotect_id,
     dmccd_cust_scripts->scripts[dmccd_cust_scripts->cnt].script_name = d1.object_name,
     dmccd_cust_scripts->scripts[dmccd_cust_scripts->cnt].object_type_flag_txt = d1
     .object_type_flag_txt, dmccd_cust_scripts->scripts[dmccd_cust_scripts->cnt].group_nbr = d1
     .group_nbr
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
 ...
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_diff"
 CALL final_disp_msg(dmccd_logfile_prefix)
END GO
