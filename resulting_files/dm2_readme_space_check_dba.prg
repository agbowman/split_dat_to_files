CREATE PROGRAM dm2_readme_space_check:dba
 ...
 ENDIF
 DECLARE drsc_log_prefix = vc WITH protect, constant("dm2_rdm_spcchk")
 DECLARE drsc_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drsc_install_mode = vc WITH protect, noconstant("NOT_SET")
 DECLARE drsc_exec_type = vc WITH protect, noconstant("NOT_SET")
 DECLARE drsc_cont_ind = i2 WITH protect, noconstant(1)
 DECLARE drsc_exists1_ind = i2 WITH protect, noconstant(0)
 DECLARE drsc_exists2_ind = i2 WITH protect, noconstant(0)
 DECLARE drsc_rcnt = i4 WITH protect, noconstant(0)
 DECLARE drsc_scnt = i4 WITH protect, noconstant(0)
 DECLARE drsc_tcnt = i4 WITH protect, noconstant(0)
 DECLARE drsc_ccnt = i4 WITH protect, noconstant(0)
 DECLARE drsc_tloc = i4 WITH protect, noconstant(0)
 DECLARE drsc_cloc = i4 WITH protect, noconstant(0)
 DECLARE drsc_spcchk_flag = i2 WITH protect, noconstant(0)
 DECLARE drsc_scrm_to_run = i4 WITH protect, noconstant(0)
 DECLARE drsc_rpt_file = vc WITH protect, noconstant("NOT_SET")
 DECLARE drsc_prev_err_ind = i2 WITH protect, noconstant(0)
 DECLARE drsc_prev_emsg = vc WITH protect, noconstant("NOT_SET")
 DECLARE drsc_prev_eproc = vc WITH protect, noconstant("NOT_SET")
 DECLARE drsc_prev_useract = vc WITH protect, noconstant("NOT_SET")
 DECLARE drsc_check_for_script(dcfs_script_name=vc,dcfs_exists_ind=i2(ref)) = i2
 SET dm2_rr_misc->package_number =  $1
 SET dm2_rr_misc->execution = "ALL"
 SET dm2_rr_spcchk->space_needed = 0
 SET dm2_rr_spc_needs->space_needed = 0
 IF (check_logfile(drsc_log_prefix,".log","dm2_readme_space_check")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting DM2_README_SPACE_CHECK"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dm2_rr_misc->env_id=0.0))
  IF (dm2_get_env_data(1,drsc_env_id)=0)
   GO TO exit_program
  ENDIF
  SET dm2_rr_misc->env_id = drsc_env_id
 ENDIF
 SET dm_err->eproc = "Validating input parameters."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (((isnumeric( $1)=0) OR (( $1=0))) )
  SET dm_err->emsg = "Package number was not numeric, or was zero."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ( NOT (cnvtupper( $2) IN ("EXPRESS", "BATCHEXPRESS", "UPTIME", "BATCHUP", "DOWNTIME",
 "BATCHDOWN", "POSTINST", "BATCHPOST")))
  SET dm_err->emsg = concat("Installation Mode <", $2,"> was not valid.")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET drsc_install_mode = cnvtupper( $2)
 IF ( NOT (cnvtupper( $3) IN ("PRESPCHK", "POSTSPCHK")))
  SET dm_err->emsg = concat("Execution type <", $3,"> was not valid.")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET drsc_exec_type = cnvtupper( $3)
 EXECUTE dm2_set_db_options
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 IF (currdb != "ORACLE")
  SET dm_err->eproc = "Readme Space Check is available only for Oracle."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ELSEIF ((dm2_db_options->readme_space_calc != "Y"))
  SET dm_err->eproc = "Readme Space Check is currently disabled."
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 IF ((drr_readmes_to_run->readme_cnt > 0))
  IF (drr_load_space_chk_readmes(drsc_exec_type,drsc_spcchk_flag)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (drsc_spcchk_flag=1)
  FOR (drsc_rcnt = 1 TO drr_readmes_to_run->readme_cnt)
    IF ((drr_readmes_to_run->readme[drsc_rcnt].spchk_readme_cnt > 0))
     IF ((drr_readmes_to_run->readme[drsc_rcnt].spchk_readme_cnt > 1))
      SET drsc_scrm_to_run = 0
      FOR (drsc_scnt = 1 TO drr_readmes_to_run->readme[drsc_rcnt].spchk_readme_cnt)
        IF (drsc_scrm_to_run=0)
         SET drsc_scrm_to_run = drsc_scnt
        ELSE
         IF ((drr_readmes_to_run->readme[drsc_rcnt].spchk_readme[drsc_scnt].ocd > drr_readmes_to_run
         ->readme[drsc_rcnt].spchk_readme[drsc_scrm_to_run].ocd))
          SET drsc_scrm_to_run = drsc_scnt
         ENDIF
        ENDIF
      ENDFOR
     ELSE
      SET drsc_scrm_to_run = 1
     ENDIF
     IF (drsc_check_for_script(drr_readmes_to_run->readme[drsc_rcnt].spchk_readme[drsc_scrm_to_run].
      script,drsc_exists1_ind)=0)
      GO TO exit_program
     ENDIF
     IF (drsc_exists1_ind=1)
      SET drs_spcchk_readme_data->orig_readme_id = drr_readmes_to_run->readme[drsc_rcnt].readme_id
      SET drs_spcchk_readme_data->orig_readme_execution = drr_readmes_to_run->readme[drsc_rcnt].
      execution
      SET drs_spcchk_readme_data->orig_readme_instance = drr_readmes_to_run->readme[drsc_rcnt].
      instance
      IF ((dm_err->debug_flag >= 2))
       CALL echorecord(drs_spcchk_readme_data)
      ENDIF
      SET drsc_prev_err_ind = dm_err->err_ind
      SET drsc_prev_emsg = dm_err->emsg
      SET drsc_prev_eproc = dm_err->eproc
      SET drsc_prev_useract = dm_err->user_action
      SET dm_err->err_ind = 0
      SET dm_err->emsg = ""
      SET dm_err->eproc = ""
      SET dm_err->user_action = ""
      EXECUTE value(drr_readmes_to_run->readme[drsc_rcnt].spchk_readme[drsc_scrm_to_run].script)
      SET dm_err->err_ind = drsc_prev_err_ind
      SET dm_err->emsg = drsc_prev_emsg
      SET dm_err->eproc = drsc_prev_eproc
      SET dm_err->user_action = drsc_prev_useract
      IF ((drs_spcchk_data->tbl_cnt > 0))
       SET dm2_rr_spcchk->space_needed = 1
       SET dm2_rr_spcchk->readme_cnt = (dm2_rr_spcchk->readme_cnt+ 1)
       SET stat = alterlist(dm2_rr_spcchk->readme_list,dm2_rr_spcchk->readme_cnt)
       SET dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_cnt = drs_spcchk_data->tbl_cnt
       SET stat = alterlist(dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_list,
        dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_cnt)
       FOR (drsc_tcnt = 1 TO drs_spcchk_data->tbl_cnt)
         SET dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_list[drsc_tcnt].table_name =
         drs_spcchk_data->qual[drsc_tcnt].table_name
         SET dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_list[drsc_tcnt].
         large_data_loaded = drs_spcchk_data->qual[drsc_tcnt].large_data_loaded
         SET dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_list[drsc_tcnt].insert_row_cnt
          = drs_spcchk_data->qual[drsc_tcnt].insert_row_cnt
         SET dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_list[drsc_tcnt].col_updt_cnt
          = drs_spcchk_data->qual[drsc_tcnt].updt_col_cnt
         IF ((drs_spcchk_data->qual[drsc_tcnt].updt_col_cnt > 0))
          SET stat = alterlist(dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_list[
           drsc_tcnt].col_updt,dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_list[
           drsc_tcnt].col_updt_cnt)
          FOR (drsc_ccnt = 1 TO drs_spcchk_data->qual[drsc_tcnt].updt_col_cnt)
           SET dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_list[drsc_tcnt].col_updt[
           drsc_ccnt].column_name = drs_spcchk_data->qual[drsc_tcnt].update_cols[drsc_ccnt].
           column_name
           SET dm2_rr_spcchk->readme_list[dm2_rr_spcchk->readme_cnt].tbl_list[drsc_tcnt].col_updt[
           drsc_ccnt].update_row_cnt = drs_spcchk_data->qual[drsc_tcnt].update_cols[drsc_ccnt].
           update_row_cnt
          ENDFOR
         ENDIF
       ENDFOR
  ...
  ENDFOR
 ENDIF
 IF ((dm2_rr_spcchk->space_needed=1))
  SET dm2_rr_spc_needs->space_needed = dm2_rr_spcchk->space_needed
  SET drsc_rcnt = 0
  SET drsc_tcnt = 0
  SET drsc_ccnt = 0
  FOR (drsc_rcnt = 1 TO dm2_rr_spcchk->readme_cnt)
    FOR (drsc_tcnt = 1 TO dm2_rr_spcchk->readme_list[drsc_rcnt].tbl_cnt)
     IF ((dm2_rr_spc_needs->tbl_cnt > 0))
      SET drsc_tloc = locateval(drsc_tloc,1,dm2_rr_spc_needs->tbl_cnt,dm2_rr_spcchk->readme_list[
       drsc_rcnt].tbl_list[drsc_tcnt].table_name,dm2_rr_spc_needs->tbl[drsc_tloc].table_name)
     ELSE
      SET drsc_tloc = 0
     ENDIF
     IF (drsc_tloc=0)
      SET dm2_rr_spc_needs->tbl_cnt = (dm2_rr_spc_needs->tbl_cnt+ 1)
      SET stat = alterlist(dm2_rr_spc_needs->tbl,dm2_rr_spc_needs->tbl_cnt)
      SET drsc_tloc = dm2_rr_spc_needs->tbl_cnt
      SET dm2_rr_spc_needs->tbl[drsc_tloc].table_name = dm2_rr_spcchk->readme_list[drsc_rcnt].
      tbl_list[drsc_tcnt].table_name
      SET dm2_rr_spc_needs->tbl[drsc_tloc].skip_ind = 0
      SET dm2_rr_spc_needs->tbl[drsc_tloc].large_data_loaded = dm2_rr_spcchk->readme_list[drsc_rcnt].
      tbl_list[drsc_tcnt].large_data_loaded
      SET dm2_rr_spc_needs->tbl[drsc_tloc].insert_row_cnt = dm2_rr_spcchk->readme_list[drsc_rcnt].
      tbl_list[drsc_tcnt].insert_row_cnt
      SET dm2_rr_spc_needs->tbl[drsc_tloc].col_updt_cnt = dm2_rr_spcchk->readme_list[drsc_rcnt].
      tbl_list[drsc_tcnt].col_updt_cnt
      IF ((dm2_rr_spc_needs->tbl[drsc_tloc].col_updt_cnt > 0))
       SET stat = alterlist(dm2_rr_spc_needs->tbl[drsc_tloc].col_updt,dm2_rr_spc_needs->tbl[drsc_tloc
        ].col_updt_cnt)
       FOR (drsc_ccnt = 1 TO dm2_rr_spc_needs->tbl[drsc_tloc].col_updt_cnt)
        SET dm2_rr_spc_needs->tbl[drsc_tloc].col_updt[drsc_ccnt].update_row_cnt = dm2_rr_spcchk->
        readme_list[drsc_rcnt].tbl_list[drsc_tcnt].col_updt[drsc_ccnt].update_row_cnt
        SET dm2_rr_spc_needs->tbl[drsc_tloc].col_updt[drsc_ccnt].column_name = dm2_rr_spcchk->
        readme_list[drsc_rcnt].tbl_list[drsc_tcnt].col_updt[drsc_ccnt].column_name
       ENDFOR
      ENDIF
      SET dm2_rr_spc_needs->tbl[drsc_tloc].tgt_idx = 0
      SET dm2_rr_spc_needs->tbl[drsc_tloc].cur_idx = 0
      SET dm2_rr_spc_needs->tbl[drsc_tloc].space_needed = 0.00
      SET dm2_rr_spc_needs->tbl[drsc_tloc].ind_cnt = 0
     ELSE
      IF ((dm2_rr_spcchk->readme_list[drsc_rcnt].tbl_list[drsc_tcnt].large_data_loaded=1))
       SET dm2_rr_spc_needs->tbl[drsc_tloc].large_data_loaded = 1
      ENDIF
      SET dm2_rr_spc_needs->tbl[drsc_tloc].insert_row_cnt = (dm2_rr_spc_needs->tbl[drsc_tloc].
      insert_row_cnt+ dm2_rr_spcchk->readme_list[drsc_rcnt].tbl_list[drsc_tcnt].insert_row_cnt)
      IF ((dm2_rr_spcchk->readme_list[drsc_rcnt].tbl_list[drsc_tcnt].col_updt_cnt > 0))
       FOR (drsc_ccnt = 1 TO dm2_rr_spcchk->readme_list[drsc_rcnt].tbl_list[drsc_tcnt].col_updt_cnt)
        IF ((dm2_rr_spc_needs->tbl[drsc_tloc].col_updt_cnt > 0))
         SET drsc_cloc = locateval(drsc_cloc,1,dm2_rr_spc_needs->tbl[drsc_tloc].col_updt_cnt,
          dm2_rr_spcchk->readme_list[drsc_rcnt].tbl_list[drsc_tcnt].col_updt[drsc_ccnt].column_name,
          dm2_rr_spc_needs->tbl[drsc_tloc].col_updt[drsc_cloc].column_name)
        ELSE
         SET drsc_cloc = 0
        ENDIF
        IF (drsc_cloc=0)
         SET dm2_rr_spc_needs->tbl[drsc_tloc].col_updt_cnt = (dm2_rr_spc_needs->tbl[drsc_tloc].
         col_updt_cnt+ 1)
         SET stat = alterlist(dm2_rr_spc_needs->tbl[drsc_tloc].col_updt,dm2_rr_spc_needs->tbl[
          drsc_tloc].col_updt_cnt)
         SET drsc_cloc = dm2_rr_spc_needs->tbl[drsc_tloc].col_updt_cnt
         SET dm2_rr_spc_needs->tbl[drsc_tloc].col_updt[drsc_cloc].update_row_cnt = dm2_rr_spcchk->
         readme_list[drsc_rcnt].tbl_list[drsc_tcnt].col_updt[drsc_ccnt].update_row_cnt
         SET dm2_rr_spc_needs->tbl[drsc_tloc].col_updt[drsc_cloc].column_name = dm2_rr_spcchk->
         readme_list[drsc_rcnt].tbl_list[drsc_tcnt].col_updt[drsc_ccnt].column_name
        ELSE
         SET dm2_rr_spc_needs->tbl[drsc_tloc].col_updt[drsc_cloc].update_row_cnt = (dm2_rr_spc_needs
         ->tbl[drsc_tloc].col_updt[drsc_cloc].update_row_cnt+ dm2_rr_spcchk->readme_list[drsc_rcnt].
         tbl_list[drsc_tcnt].col_updt[drsc_ccnt].update_row_cnt)
        ENDIF
       ENDFOR
      ENDIF
     ENDIF
 ...
 ENDIF
 IF ((dm_err->debug_flag >= 2))
  SET drsc_rpt_file = build(dm2_install_schema->schema_prefix,dm2_install_schema->file_prefix,
   "_spcchk.log")
  SELECT INTO value(drsc_rpt_file)
   d.seq
   FROM (dummyt d  WITH seq = value(dm2_rr_spcchk->readme_cnt))
   HEAD REPORT
    "readme count = ", dm2_rr_spcchk->readme_cnt, row + 1,
    "space_needed = ", dm2_rr_spcchk->space_needed, row + 1,
    "preup_space_needed = ", dm2_rr_spcchk->preup_space_needed
   DETAIL
    row + 1, "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~", row + 1,
    "readme_id = ", dm2_rr_spcchk->readme_list[d.seq].readme_id, row + 1,
    "spcchk_readme_id = ", dm2_rr_spcchk->readme_list[d.seq].spcchk_readme_id, row + 1,
    "script = ", dm2_rr_spcchk->readme_list[d.seq].script, row + 1,
    "tbl_cnt = ", dm2_rr_spcchk->readme_list[d.seq].tbl_cnt, row + 1
    IF ((dm2_rr_spcchk->readme_list[d.seq].tbl_cnt > 0))
     row + 2, "TABLES"
     FOR (t_cnt = 1 TO dm2_rr_spcchk->readme_list[d.seq].tbl_cnt)
       row + 1, col 3, "------------------",
       row + 1, col 3, "table_name = ",
       dm2_rr_spcchk->readme_list[d.seq].tbl_list[t_cnt].table_name, row + 1, col 3,
       "large_data_loaded = ", dm2_rr_spcchk->readme_list[d.seq].tbl_list[t_cnt].large_data_loaded,
       row + 1,
       col 3, "insert_row_cnt = ", dm2_rr_spcchk->readme_list[d.seq].tbl_list[t_cnt].insert_row_cnt,
       row + 1, col 3, "col_updt_cnt = ",
       dm2_rr_spcchk->readme_list[d.seq].tbl_list[t_cnt].col_updt_cnt
       IF ((dm2_rr_spcchk->readme_list[d.seq].tbl_list[t_cnt].col_updt_cnt > 0))
        row + 2, col 3, "COLUMNS"
        FOR (c_cnt = 1 TO dm2_rr_spcchk->readme_list[d.seq].tbl_list[t_cnt].col_updt_cnt)
          row + 1, col 6, "------------------",
          row + 1, col 6, "column_name",
          dm2_rr_spcchk->readme_list[d.seq].tbl_list[t_cnt].col_updt[c_cnt].column_name, row + 1, col
           6,
          "update_row_cnt", dm2_rr_spcchk->readme_list[d.seq].tbl_list[t_cnt].col_updt[c_cnt].
          update_row_cnt
        ENDFOR
       ENDIF
     ENDFOR
  ...
  ;end select
 ENDIF
 SUBROUTINE drsc_check_for_script(dcfs_script_name,dcfs_exists_ind)
   SET dm_err->eproc = concat("Searching for ",cnvtupper(dcfs_script_name),
    " in Discern Explorer dictionary.")
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    d.object_name
    FROM dprotect d
    WHERE d.object_name=cnvtupper(dcfs_script_name)
     AND d.group=0
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
   IF (curqual > 0)
    SET dcfs_exists_ind = 1
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_program
 SET dm_err->eproc = "Finished performing Readme Space Check."
 CALL final_disp_msg(drsc_log_prefix)
END GO
