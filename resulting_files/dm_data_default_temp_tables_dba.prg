CREATE PROGRAM dm_data_default_temp_tables:dba
 RECORD data_len(
   1 qual[*]
     2 len = i4
     2 name = vc
 ...
 SELECT DISTINCT
  ...
  FROM dm_user_tab_cols dt
  WHERE dt.data_default="*TO_DATE*"
  ORDER BY dt.data_default
  HEAD REPORT
   col 0,
   "This report shows all the distinct data_defaults with length > 40 that exist in the dm_user_tab_cols.",
   row + 1,
   "Only data defaults of length < 40 are acceptable.", row + 2,
  ...
  DETAIL
   IF (textlen(trim(dt.data_default)) > 40)
    data_len->qual[count].len = textlen(trim(dt.data_default)), data_len->qual[count].name = trim(dt
     .data_default), col 0,
 ...
 FOR (xt = 1 TO count)
  UPDATE  FROM dm_user_tab_cols dt
   SET dt.data_default = "TO_DATE ( '12/31/2100' , 'MM/DD/YYYY' )"
   WHERE (dt.data_default=data_len->qual[xt].name)
   WITH nocounter
  ;end update
  COMMIT
 ENDFOR
 SELECT DISTINCT
  length = textlen(trim(dt.data_default)), dt.data_default
  FROM dm_user_tab_cols dt
  WHERE dt.data_default="TO_DATE ( '12/31/2100' , 'MM/DD/YYYY' )"
  ORDER BY dt.data_default
  HEAD REPORT
