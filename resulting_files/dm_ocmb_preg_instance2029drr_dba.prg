CREATE PROGRAM dm_ocmb_preg_instance2029drr:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  IF (debug_ind=1)
   CALL echo("********DM_OCMB_PREG_INSTANCE2029DRR - maintaining exception")
  ENDIF
  SET stat = alterlist(dcem_request->qual,1)
  SET dcem_request->qual[1].parent_entity = "ORGANIZATION"
  SET dcem_request->qual[1].child_entity = "PREGNANCY_INSTANCE2029DRR"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_OCMB_PREG_INSTANCE2029DRR"
 ...
 ENDIF
 IF (debug_ind=1)
  CALL echo(build("********DM_OCMB_PREG_INSTANCE2029DRR - from_xxx_id = ",request->xxx_combine[
    icombine].from_xxx_id))
  CALL echo(build("********DM_OCMB_PREG_INSTANCE2029DRR - to_xxx_id = ",request->xxx_combine[icombine
    ].to_xxx_id))
 ENDIF
 SELECT INTO "nl:"
  frm.*
  FROM pregnancy_instance2029drr frm
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].pk_col1 = frm.pregnancy_instance_id, rreclist->from_rec[
   ...
   end_effective_dt_tm = frm.end_effective_dt_tm, rreclist->from_rec[v_cust_count1].preg_end_dt_tm =
   frm.preg_end_dt_tm,
   rreclist->from_rec[v_cust_count1].pregnancy_id = frm.pregnancy_id
  FOOT REPORT
 ...
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   tu.*
   FROM pregnancy_instance2029drr tu
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count2].pk_col1 = tu.pregnancy_instance_id, rreclist->to_rec[
    ...
    end_effective_dt_tm = tu.end_effective_dt_tm, rreclist->to_rec[v_cust_count2].preg_end_dt_tm = tu
    .preg_end_dt_tm,
    rreclist->to_rec[v_cust_count2].pregnancy_id = tu.pregnancy_id
   FOOT REPORT
    ...
    IF ((rreclist->from_rec[v_cust_loopcount].active_ind=1)
     ...
     IF (preg_pos=0)
      FOR (v_cust_loopcount2 = 1 TO v_cust_count2)
        IF ((rreclist->to_rec[v_cust_loopcount2].person_id=rreclist->from_rec[v_cust_loopcount].
        person_id)
  ...
  IF (debug_ind=1)
   ...
   CALL echorecord(conflict_pregs)
  ENDIF
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
 ...
 SUBROUTINE del_from(s_df_pk_id,s_df_prev_act_ind,s_df_prev_act_status,s_df_prev_end_eff_dt_tm)
   UPDATE  FROM pregnancy_instance2029drr frm
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "PREGNANCY_INSTANCE2029DRR"
   ...
   SET request->xxx_combine_det[icombinedet].entity_pk[1].col_name = "PREGNANCY_INSTANCE_ID"
   ...
   IF (curqual=0)
    ...
    SET request->error_message = substring(1,132,build("Could not inactivate pregnancy_instance_id=",
      s_df_pk_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 ...
 SUBROUTINE updt_from(s_uf_pk_id)
   ...
   IF (curqual=0)
    ...
    SET request->error_message = build(
     "Could not update PREGNANCY_INSTANCE2029DRR record with pregnancy_instance_id = ",s_uf_pk_id)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
