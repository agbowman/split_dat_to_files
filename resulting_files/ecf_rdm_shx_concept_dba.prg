CREATE PROGRAM ecf_rdm_shx_concept:dba
 ...
 SUBROUTINE kia_script_inc(pkg,ccl_obj)
   ...
   CALL parser(obj_prs)
 END ;Subroutine
 DECLARE kia_concept_filename = vc WITH public, constant("shx_concept.csv")
 DECLARE kia_cur_ver_beg_dt = vc WITH public, noconstant("")
 SET kia_tbl_desc = "SHX CONCEPT"
 SET kia_short_desc = "SHX"
 SET kia_rdm_log_name = "ecf_rdm_shx_concept.log"
 SET kia_rdm_nbr = 9825
 SET kia_rdm_vocab_mean = "CERNER"
 SET readme_data->status = "F"
 ...
 EXECUTE kia_dm_dbimport "cer_install:shx_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("ECF_RDM_SHX_CONCEPT Begin> ",format(cnvtdatetime(curdate,curtime3),
 ...
 CALL kia_chk_status_info("LOAD SOCIAL HISTORY CONCEPT BASELINE DATA","check")
 IF (kia_obj_suc_ind=0)
  CALL echo("DELETING CMT_CONCEPT_LOAD_NS ROWS...")
  DELETE  FROM cmt_concept_load_ns c
   WHERE ((c.concept_cki="CERNER*") OR (c.concept_cki="SNOMED*"))
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  EXECUTE kia_sqlldr "cmt_concept_load_ns", kia_concept_filename, 0
  CALL kia_chk_script_failure("kia_sqlldr")
  CALL kia_set_status_success("LOAD SOCIAL HISTORY CONCEPT BASELINE DATA")
 ENDIF
 ...
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(9824,"SHX XMAP"))
 SET kia_version_chk = (kia_version_chk+ kia_chk_status_readme(9825,"SHX CONCEPT"))
 CALL echo(build("README'S ALREADY COMPLETED:",kia_version_chk))
 IF (kia_version_chk=2)
...
#db2_exit
 ...
 SUBROUTINE my_echo(local_message)
   CALL echo("*************************************************")
   CALL echo(local_message)
   CALL echo("*************************************************")
 END ;Subroutine
END GO
