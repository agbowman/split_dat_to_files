CREATE PROGRAM cp_add_component_detail_json:dba
 PROMPT
  ...
  comptext, versiontext, versionflag,
  sourceflag
 FREE RECORD request
 RECORD request(
   1 comp_detail_list[*]
 ...
 DECLARE determineversionnbr(null) = null WITH protect
 DECLARE verified = i2 WITH noconstant(0), protect
 DECLARE component_ident = vc WITH protect, noconstant("")
 DECLARE component_text = vc WITH protect, noconstant("")
 ...
 SUBROUTINE verifyinputs(null)
   ...
   SELECT INTO "NL:"
    ...
    WHERE (cc.cp_component_id= $COMPID)
    WITH nocounter
   ;end select
   IF (curqual=0)
 ...
 SUBROUTINE insertrows(null)
   CALL log_message("In insertRows()",log_level_debug)
   SET component_text = decodexmlspecialcharacters( $COMPTEXT)
   SET component_text = encodeinternationalcharacters(component_text)
   SET component_ident = decodexmlspecialcharacters( $COMPIDENT)
   SET component_ident = encodeinternationalcharacters(component_ident)
   SET stat = alterlist(request->comp_detail_list,1)
   SET request->comp_detail_list[1].cp_component_id =  $COMPID
   SET request->comp_detail_list[1].component_text = component_text
   SET request->comp_detail_list[1].component_ident = component_ident
   SET request->comp_detail_list[1].component_entity_name = trim( $ENTITYNM)
   SET request->comp_detail_list[1].component_entity_id =  $ENTITYID
   SET request->comp_detail_list[1].comp_detail_reltn_cd =  $RELTNCD
   SET request->comp_detail_list[1].version_text =  $VERSIONTEXT
   SET request->comp_detail_list[1].version_flag =  $VERSIONFLAG
   SET request->comp_detail_list[1].source_flag =  $SOURCEFLAG
   CALL determineversionnbr(null)
   EXECUTE cp_add_component_detail  WITH replace("REPLY","MPAGEREPLY")
   IF ((mpagereply->status_data.status="S"))
    COMMIT
   ENDIF
   IF (validate(debug_ind,0)=1)
    CALL echo(build("MPAGEREPLY->status_data.status -----> ",mpagereply->status_data.status))
    CALL echorecord(request)
    CALL echorecord(mpagereply)
   ENDIF
   CALL log_message("Exit insertRows()",log_level_debug)
 END ;Subroutine
 SUBROUTINE determineversionnbr(null)
   CALL log_message("In determineVersionNbr()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE doccontenttypecd = f8 WITH constant(uar_get_code_by("MEANING",4003134,"DOCCONTENT")),
   protect
   DECLARE doccontentdetailid = f8 WITH noconstant(0), protect
   SELECT INTO "nl:"
    FROM cp_component_detail ccd
    WHERE ccd.component_detail_reltn_cd=doccontenttypecd
     AND (ccd.cp_component_id= $COMPID)
    ORDER BY ccd.version_nbr DESC
    HEAD REPORT
     request->comp_detail_list[1].version_nbr = (ccd.version_nbr+ 1), doccontentdetailid = ccd
     .cp_component_detail_id
    WITH nocounter
   ;end select
   IF ((request->comp_detail_list[1].version_nbr=1)
    AND (request->comp_detail_list[1].comp_detail_reltn_cd=doccontenttypecd))
    CALL updateoriginaldoccontent(doccontentdetailid)
   ENDIF
   IF ((request->comp_detail_list[1].version_nbr=0)
    AND (request->comp_detail_list[1].comp_detail_reltn_cd=doccontenttypecd))
    SET request->comp_detail_list[1].default_ind = 1
    SET request->comp_detail_list[1].version_nbr = 1
   ENDIF
   CALL log_message(build("Exit determineVersionNbr(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (checkcodevalue(codevalue=f8,codeset=i4) =i2 WITH protect)
   ...
   CALL log_message(build("Exit checkCodeValue(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE (updateoriginaldoccontent(doccontentid=f8) =null WITH protect)
   CALL log_message("In updateOriginalDocContent()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   UPDATE  FROM cp_component_detail ccd
    SET ccd.default_ind = 1
    WHERE ccd.cp_component_detail_id=doccontentid
    WITH nocounter
   ;end update
   SET reqinfo->commit_ind = 1
   CALL log_message(build("Exit updateOriginalDocContent(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
