CREATE PROGRAM dm_pcmb_ehi_queue_person_reltn:dba
 ...
 DECLARE from_request_type_tflg = vc WITH protect, noconstant(" ")
 DECLARE from_extract_status_txt = vc WITH protect, noconstant(" ")
 CALL dm_cmb_get_context(0)
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  SET stat = alterlist(dcem_request->qual,1)
  SET dcem_request->qual[1].parent_entity = "PERSON"
  SET dcem_request->qual[1].child_entity = "EHI_QUEUE_PERSON_RELTN"
  SET dcem_request->qual[1].op_type = "COMBINE"
  SET dcem_request->qual[1].script_name = "DM_PCMB_EHI_QUEUE_PERSON_RELTN"
 ...
 SELECT INTO "nl:"
  frm.*
  FROM ehi_queue_person_reltn frm
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = frm.ehi_queue_person_reltn_id, rreclist->from_rec[
   v_cust_count1].active_ind = frm.active_ind, rreclist->from_rec[v_cust_count1].active_status_cd =
   frm.active_status_cd,
   rreclist->from_rec[v_cust_count1].ehi_queue_id = frm.ehi_queue_id
  WITH forupdatewait(frm)
 ;end select
 IF (v_cust_count1 > 0)
  SELECT INTO "nl:"
   tu.*
   FROM ehi_queue_person_reltn tu
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[v_cust_count2].to_id = tu.ehi_queue_person_reltn_id, rreclist->to_rec[
    v_cust_count2].active_ind = tu.active_ind, rreclist->to_rec[v_cust_count2].active_status_cd = tu
    .active_status_cd,
    rreclist->to_rec[v_cust_count2].ehi_queue_id = tu.ehi_queue_id
   WITH forupdatewait(tu)
  ;end select
  FOR (v_cust_loopcount = 1 TO v_cust_count1)
   SELECT INTO "nl:"
    FROM ehi_queue eq
    WHERE (eq.ehi_queue_id=rreclist->from_rec[v_cust_loopcount].ehi_queue_id)
    DETAIL
     from_extract_status_txt = eq.extract_status_txt, from_request_type_tflg = eq.request_type_tflg
    WITH nocounter
   ;end select
   IF (curqual > 0)
    SET to_id_present = 0
    IF (from_request_type_tflg="MULTI")
     SET to_id_present = locateval(to_id_present,1,v_cust_count2,rreclist->from_rec[v_cust_loopcount]
      .ehi_queue_id,rreclist->to_rec[to_id_present].ehi_queue_id)
    ENDIF
    IF (from_extract_status_txt="QUEUED"
     AND to_id_present=0)
 ...
 SUBROUTINE (del_from(s_df_pk_id=f8,s_df_to_fk_id=f8,s_df_prev_act_ind=i2,s_df_prev_act_status=f8) =
  i4)
   UPDATE  FROM ehi_queue_person_reltn frm
    ...
    WHERE frm.ehi_queue_person_reltn_id=s_df_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "EHI_QUEUE_PERSON_RELTN"
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =i4)
   UPDATE  FROM ehi_queue_person_reltn frm
    SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo
     ...
     s_uf_to_fk_id
    WHERE frm.ehi_queue_person_reltn_id=s_uf_pk_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
