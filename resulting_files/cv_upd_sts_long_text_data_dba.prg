CREATE PROGRAM cv_upd_sts_long_text_data:dba
 ...
 DECLARE upp_sts_failed = c1 WITH protect, noconstant("F")
 ...
 DECLARE cv_dataset = vc WITH protect, constant("CV_DATASET")
 ...
 DECLARE ds_int_name = vc WITH protect, noconstant("STS")
 SELECT INTO "nl:"
  FROM cv_dataset cd
 ...
 SELECT INTO "nl:"
  ...
  WHERE ccdr.cv_case_id > 0
   AND ccdr.dataset_id=cv_dataset_id
  HEAD REPORT
   upd_ds_cnt = 0
  DETAIL
 ...
 SELECT INTO "nl:"
  ...
  FROM cv_case_file_row ccfr,
   (dummyt d  WITH seq = value(size(upd_data_rec->dataset,5)))
  PLAN (d)
   JOIN (ccfr
   WHERE (ccfr.case_dataset_r_id=upd_data_rec->dataset[d.seq].dataset_r_id))
  ORDER BY d.seq
  HEAD REPORT
   cnt = 0
  HEAD d.seq
   cnt = 0
  DETAIL
   cnt = (cnt+ 1), stat = alterlist(upd_data_rec->dataset[d.seq].case_file_row,cnt), upd_data_rec->
 ...
 SELECT INTO "nl:"
  FROM long_text lt,
   (dummyt d  WITH seq = value(size(upd_data_rec->dataset,5))),
   (dummyt d2  WITH seq = value(upd_data_rec->max_case_file_row_cnt))
  PLAN (d)
   JOIN (d2
   WHERE d2.seq <= size(upd_data_rec->dataset[d.seq].case_file_row,5))
   JOIN (lt
   WHERE (lt.parent_entity_id=upd_data_rec->dataset[d.seq].case_file_row[d2.seq].cv_case_file_row_id)
    AND lt.parent_entity_name=cv_case_file_row)
  DETAIL
   lt_updt_cnt = (lt_updt_cnt+ 1), upd_data_rec->dataset[d.seq].case_file_row[d2.seq].long_text = lt
   .long_text, upd_data_rec->dataset[d.seq].case_file_row[d2.seq].long_text = build(trim(upd_data_rec
     ->dataset[d.seq].case_file_row[d2.seq].long_text,3),"|||||||||||||||||||||||||||||||||||")
  WITH nocounter
 ;end select
 IF (curqual=0)
 ...
 IF (curqual != lt_updt_cnt)
  SET upp_sts_failed = "T"
  CALL cv_log_message("Failed in select long_text for lock the row!")
  GO TO exit_script
 ENDIF
 ...
 UPDATE  FROM long_text lt,
   (dummyt d  WITH seq = value(size(upd_data_rec->dataset,5))),
   (dummyt d2  WITH seq = value(upd_data_rec->max_case_file_row_cnt))
  SET lt.long_text = upd_data_rec->dataset[d.seq].case_file_row[d2.seq].long_text
  PLAN (d)
   JOIN (d2
   WHERE d2.seq <= size(upd_data_rec->dataset[d.seq].case_file_row,5))
   JOIN (lt
