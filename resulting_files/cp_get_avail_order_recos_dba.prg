CREATE PROGRAM cp_get_avail_order_recos:dba
 PROMPT
 ...
 FREE RECORD viewableencntrlist
 RECORD viewableencntrlist(
   1 cnt = i4
   1 qual[*]
     2 value = f8
 )
 DECLARE main(null) = null WITH protect
 DECLARE retrieveflexorders(null) = null WITH protect
 DECLARE loadorderedsynonyms(null) = null WITH protect
 DECLARE loadorderedplans(null) = null WITH protect
 DECLARE loadorderedregimens(null) = null WITH protect
 DECLARE loadordersentenceinfo(null) = null WITH protect
 DECLARE orderslookback = dq8 WITH protect, noconstant(cnvtlookbehind("2,Y",cnvtdatetime(sysdate)))
 DECLARE reg_cancelled_status_cd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",4002501,
   "CANCELLED"))
 DECLARE pp_void_status_cd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",16769,"VOID"))
 DECLARE order_void_status_cd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",6004,"DELETED")
  )
 DECLARE order_void_w_res_status_cd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",6004,
   "VOIDEDWRSLT"))
 DECLARE order_discontinued_status_cd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",6004,
   "DISCONTINUED"))
 DECLARE order_future_status_cd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",6004,"FUTURE"
   ))
 ...
 SET reco_reply->status_data.status = "F"
 CALL main(null)
 SET reco_reply->status_data.status = "S"
 SUBROUTINE main(null)
   CALL log_message("Begin main()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   CALL retrieveflexorders(null)
   IF ((reco_reply->status_data.status="S")
    AND ( $VIEWABLEENCNTRS != 0.0))
    CALL getparametervalues(11,viewableencntrlist)
    SET viewableencntrlist->cnt += 1
    SET stat = alterlist(viewableencntrlist->qual,viewableencntrlist->cnt)
    IF (validate(debug_ind))
     CALL echorecord(viewableencntrlist)
    ENDIF
    CALL buildorderlists(null)
    IF (validate(debug_ind))
     CALL echorecord(flat_orders)
    ENDIF
    CALL loadorderedsynonyms(null)
    CALL loadorderedplans(null)
    CALL loadorderedregimens(null)
    CALL loadordersentenceinfo(null)
   ELSE
    GO TO exit_script
   ENDIF
 ...
 END ;Subroutine
 SUBROUTINE retrieveflexorders(null)
   CALL log_message("Begin retrieveFlexOrders()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   FREE RECORD order_flex_reply
   RECORD order_flex_reply(
     1 person_id = f8
     1 encntr_id = f8
     1 cbt_mean = vc
     1 intention_mean = vc
   ...
   EXECUTE cpm_get_orders_flex "NOFORMS", value( $PERSONID), value( $ENCNTRID),
   value( $PRSNLID), value( $CONCEPTCD), value( $CONCEPTGROUPCD),
   value( $INTENTIONCD),  $ALTSELCATIDS, value( $OCID),
   value( $VENUETYPE) WITH replace("MPAGEREPLY","ORDER_FLEX_REPLY")
   SET stat = moverec(order_flex_reply->parent,reco_reply->parent)
   SET stat = moverec(order_flex_reply->venue_type_list,reco_reply->venue_type_list)
   SET reco_reply->cbt_mean = order_flex_reply->cbt_mean
   SET reco_reply->encntr_id = order_flex_reply->encntr_id
   SET reco_reply->intention_mean = order_flex_reply->intention_mean
   SET reco_reply->oc_id = order_flex_reply->oc_id
   SET reco_reply->person_id = order_flex_reply->person_id
   SET stat = moverec(order_flex_reply->status_data,reco_reply->status_data)
   CALL log_message(build("Exit retrieveFlexOrders(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE buildorderlists(null)
   CALL log_message("Begin buildOrderLists()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE p_cntr = i4 WITH protect, noconstant(0)
   DECLARE p_size = i4 WITH protect, noconstant(0)
   DECLARE c_cntr = i4 WITH protect, noconstant(0)
   DECLARE c_size = i4 WITH protect, noconstant(0)
   SET p_size = size(reco_reply->parent,5)
   FOR (p_cntr = 1 TO p_size)
    SET c_size = size(reco_reply->parent[p_cntr].child,5)
    FOR (c_cntr = 1 TO c_size)
      IF ((reco_reply->parent[p_cntr].child[c_cntr].synonym_id > 0))
       SET flat_orders->synonym_cnt += 1
       SET stat = alterlist(flat_orders->synonyms,flat_orders->synonym_cnt)
       SET flat_orders->synonyms[flat_orders->synonym_cnt].id = reco_reply->parent[p_cntr].child[
       c_cntr].synonym_id
       SET flat_orders->synonyms[flat_orders->synonym_cnt].display = reco_reply->parent[p_cntr].
       child[c_cntr].synonym
       IF ((reco_reply->parent[p_cntr].child[c_cntr].sentence_id > 0))
        SET flat_orders->sentence_cnt += 1
        SET stat = alterlist(flat_orders->sentences,flat_orders->sentence_cnt)
        SET flat_orders->sentences[flat_orders->sentence_cnt].id = reco_reply->parent[p_cntr].child[
        c_cntr].sentence_id
        SET flat_orders->sentences[flat_orders->sentence_cnt].display = reco_reply->parent[p_cntr].
        child[c_cntr].synonym
       ENDIF
      ELSEIF ((reco_reply->parent[p_cntr].child[c_cntr].path_cat_id > 0))
       SET flat_orders->plan_cnt += 1
       SET stat = alterlist(flat_orders->plans,flat_orders->plan_cnt)
       SET flat_orders->plans[flat_orders->plan_cnt].id = reco_reply->parent[p_cntr].child[c_cntr].
       path_cat_id
       SET flat_orders->plans[flat_orders->plan_cnt].display = reco_reply->parent[p_cntr].child[
       c_cntr].plan_description
      ELSEIF ((reco_reply->parent[p_cntr].child[c_cntr].reg_cat_id > 0))
       SET flat_orders->regimen_cnt += 1
       SET stat = alterlist(flat_orders->regimens,flat_orders->regimen_cnt)
       SET flat_orders->regimens[flat_orders->regimen_cnt].id = reco_reply->parent[p_cntr].child[
       c_cntr].reg_cat_id
       SET flat_orders->regimens[flat_orders->regimen_cnt].display = reco_reply->parent[p_cntr].
       child[c_cntr].reg_cat_syn_display
      ENDIF
    ENDFOR
   ENDFOR
   CALL log_message(build("Exit buildOrderLists(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE loadorderedsynonyms(null)
   CALL log_message("Begin loadOrderedSynonyms()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE s_cntr = i4 WITH protect, noconstant(0)
   DECLARE s_idx = i4 WITH protect, noconstant(0)
   DECLARE e_cntr = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM orders o
    PLAN (o
     WHERE o.orig_order_dt_tm >= cnvtdatetime(orderslookback)
      AND o.product_id >= 0
      AND  NOT (o.order_status_cd IN (order_void_status_cd, order_void_w_res_status_cd,
     order_discontinued_status_cd))
      AND o.activity_type_cd >= 0
      AND expand(s_cntr,1,flat_orders->synonym_cnt,o.synonym_id,flat_orders->synonyms[s_cntr].id)
      AND ((o.person_id+ 0)= $PERSONID)
      AND expand(e_cntr,1,viewableencntrlist->cnt,(o.encntr_id+ 0),viewableencntrlist->qual[e_cntr].
      value))
    ORDER BY o.synonym_id, o.orig_order_dt_tm DESC
    HEAD o.synonym_id
     reco_reply->ordered_info.cnt += 1, s_idx = reco_reply->ordered_info.cnt, stat = alterlist(
      reco_reply->ordered_info.qual,reco_reply->ordered_info.cnt),
     reco_reply->ordered_info.qual[s_idx].parent_entity_name = "ORDER_CATALOG_SYNONYM", reco_reply->
     ordered_info.qual[s_idx].parent_entity_id = o.synonym_id
    HEAD o.orig_order_dt_tm
     IF ((reco_reply->ordered_info.qual[s_idx].last_ordered_dt_tm=0))
      reco_reply->ordered_info.qual[s_idx].last_ordered_dt_tm = o.orig_order_dt_tm, reco_reply->
      ordered_info.qual[s_idx].last_ordered_dt_tm_disp = format(o.orig_order_dt_tm,"@SHORTDATE")
      IF (o.order_status_cd=order_future_status_cd)
       reco_reply->ordered_info.qual[s_idx].future_ind = 1
      ENDIF
     ENDIF
    WITH expand = 1
   ;end select
   CALL log_message(build("Exit loadOrderedSynonyms(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE loadorderedplans(null)
   CALL log_message("Begin loadOrderedPlans()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE p_cntr = i4 WITH protect, noconstant(0)
   ...
   SELECT INTO "nl:"
    FROM pathway p
    PLAN (p
     WHERE expand(p_cntr,1,flat_orders->plan_cnt,p.pw_cat_group_id,flat_orders->plans[p_cntr].id)
      AND ((p.person_id+ 0)= $PERSONID)
      AND ((p.order_dt_tm+ 0) >= cnvtdatetime(orderslookback))
      AND  NOT (((p.pw_status_cd+ 0) IN (pp_void_status_cd)))
      AND expand(e_cntr,1,viewableencntrlist->cnt,(p.encntr_id+ 0),viewableencntrlist->qual[e_cntr].
      value))
    ORDER BY p.pw_cat_group_id, p.order_dt_tm
    HEAD p.pw_cat_group_id
     reco_reply->ordered_info.cnt += 1, p_idx = reco_reply->ordered_info.cnt, stat = alterlist(
      reco_reply->ordered_info.qual,reco_reply->ordered_info.cnt),
     reco_reply->ordered_info.qual[p_idx].parent_entity_name = "PATHWAY_CATALOG", reco_reply->
     ordered_info.qual[p_idx].parent_entity_id = p.pw_cat_group_id
    HEAD p.order_dt_tm
     IF ((reco_reply->ordered_info.qual[p_idx].last_ordered_dt_tm=0))
      reco_reply->ordered_info.qual[p_idx].last_ordered_dt_tm = p.order_dt_tm, reco_reply->
      ordered_info.qual[p_idx].last_ordered_dt_tm_disp = format(p.order_dt_tm,"@SHORTDATE")
     ENDIF
    WITH expand = 1
   ;end select
   CALL log_message(build("Exit loadOrderedPlans(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE loadorderedregimens(null)
   CALL log_message("Begin loadOrderedRegimens()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE r_cntr = i4 WITH protect, noconstant(0)
   DECLARE r_idx = i4 WITH protect, noconstant(0)
   DECLARE e_cntr = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM regimen r
    PLAN (r
     WHERE expand(r_cntr,1,flat_orders->regimen_cnt,r.regimen_catalog_id,flat_orders->regimens[r_cntr
      ].id)
      AND ((r.person_id+ 0)= $PERSONID)
      AND ((r.order_dt_tm+ 0) >= cnvtdatetime(orderslookback))
      AND  NOT (((r.regimen_status_cd+ 0) IN (reg_cancelled_status_cd)))
      AND expand(e_cntr,1,viewableencntrlist->cnt,(r.encntr_id+ 0),viewableencntrlist->qual[e_cntr].
      value))
    ORDER BY r.regimen_catalog_id, r.order_dt_tm
    HEAD r.regimen_catalog_id
     reco_reply->ordered_info.cnt += 1, r_idx = reco_reply->ordered_info.cnt, stat = alterlist(
      reco_reply->ordered_info.qual,reco_reply->ordered_info.cnt),
     reco_reply->ordered_info.qual[r_idx].parent_entity_name = "REGIMEN_CATALOG", reco_reply->
     ordered_info.qual[r_idx].parent_entity_id = r.regimen_catalog_id
    HEAD r.order_dt_tm
     IF ((reco_reply->ordered_info.qual[r_idx].last_ordered_dt_tm=0))
      reco_reply->ordered_info.qual[r_idx].last_ordered_dt_tm = r.order_dt_tm, reco_reply->
      ordered_info.qual[r_idx].last_ordered_dt_tm_disp = format(r.order_dt_tm,"@SHORTDATE")
     ENDIF
    WITH expand = 1
   ;end select
   CALL log_message(build("Exit loadOrderedRegimens(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE loadordersentenceinfo(null)
   CALL log_message("Begin loadOrderSentenceInfo()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE s_cntr = i4 WITH protect, noconstant(0)
   DECLARE search_cntr = i4 WITH protect, noconstant(0)
   DECLARE s_idx = i4 WITH protect, noconstant(0)
   DECLARE p_cntr = i4 WITH protect, noconstant(0)
   DECLARE p_size = i4 WITH protect, noconstant(size(reco_reply->parent,5))
   DECLARE os_idx = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM order_sentence os
    PLAN (os
     WHERE expand(s_cntr,1,flat_orders->sentence_cnt,os.order_sentence_id,flat_orders->sentences[
      s_cntr].id))
    ORDER BY os.order_sentence_id
    HEAD os.order_sentence_id
     reco_reply->usage_flags.cnt += 1, s_idx = reco_reply->usage_flags.cnt, stat = alterlist(
      reco_reply->usage_flags.qual,reco_reply->usage_flags.cnt),
     reco_reply->usage_flags.qual[s_idx].sentence_id = os.order_sentence_id, reco_reply->usage_flags.
     qual[s_idx].usage_flag = os.usage_flag
     FOR (p_cntr = 1 TO p_size)
      os_idx = locateval(search_cntr,1,size(reco_reply->parent[p_cntr].child,5),os.order_sentence_id,
       reco_reply->parent[p_cntr].child[search_cntr].sentence_id,
       "",reco_reply->parent[p_cntr].child[search_cntr].sentence),
      IF (os_idx > 0)
       reco_reply->parent[p_cntr].child[os_idx].sentence = os.order_sentence_display_line
      ENDIF
     ENDFOR
    WITH expand = 1
   ;end select
   CALL log_message(build("Exit loadOrderSentenceInfo(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
#exit_script
 IF (validate(debug_ind,0))
  CALL echorecord(reco_reply)
 ENDIF
 IF (( $OUTDEV != "NOFORMS"))
  CALL putjsonrecordtofile(reco_reply)
  FREE RECORD reco_reply
 ENDIF
