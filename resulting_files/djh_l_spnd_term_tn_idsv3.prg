CREATE PROGRAM djh_l_spnd_term_tn_idsv3
 PROMPT
  "Number of Days Back:" = 30,
 ...
 SELECT INTO  $OUTDEV
  ...
  FROM prsnl p,
   prsnl p1
  PLAN (p
   WHERE ((p.active_status_cd=194) OR (p.active_ind=0))
    AND p.username="TN*"
    AND p.end_effective_dt_tm <= cnvtdatetime(curdate,235959)
    AND p.end_effective_dt_tm >= cnvtdatetime((curdate -  $DYBK),0))
   JOIN (p1
   WHERE p.updt_id=p1.person_id)
  ORDER BY newenddt DESC, p_position_disp, p1.name_full_formatted
  HEAD REPORT
   y_pos = 18,
