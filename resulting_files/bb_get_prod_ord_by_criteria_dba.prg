CREATE PROGRAM bb_get_prod_ord_by_criteria:dba
 RECORD reply(
   1 order_list[*]
     2 product_number = vc
     2 product_id = f8
 ...
 DECLARE ncaresetparent = i2 WITH protect, constant(1)
 DECLARE nsupergroupparent = i2 WITH protect, constant(4)
 DECLARE ncareplanparent = i2 WITH protect, constant(16)
 ...
 DECLARE sordered_cdf = c12 WITH protect, constant("ORDERED")
 DECLARE sinprocess_cdf = c12 WITH protect, constant("LABINPROCESS")
 DECLARE slabcollected_cdf = c12 WITH protect, constant("LABCOLLECTED")
 ...
 DECLARE dordered_cv = f8 WITH protect, noconstant(0.0)
 DECLARE dinprocess_cv = f8 WITH protect, noconstant(0.0)
 DECLARE dlabcollected_cv = f8 WITH protect, noconstant(0.0)
 ...
 DECLARE ntotal = i2 WITH protect, noconstant(0)
 ...
 ENDIF
 SELECT INTO "nl:"
  o.order_id, p.product_id, p.product_nbr
  FROM orders o,
   product p,
  ...
  PLAN (o
   WHERE o.orig_order_dt_tm BETWEEN cnvtdatetime(request->ordered_from_dt_tm) AND cnvtdatetime(
    ...
    AND ((o.dept_status_cd+ 0) IN (dordered_cv, dinprocess_cv, dlabcollected_cv, dinlab_cv))
    AND  NOT (((o.order_status_cd+ 0) IN (dcanceled_cv, dcompleted_cv, ddiscontinued_cv)))
    AND ((o.product_id+ 0) > 0)
    AND  NOT (((o.cs_flag+ 0) IN (ncaresetparent, nsupergroupparent, ncareplanparent)))
   ...
   WHERE p.product_id=o.product_id
    AND (((request->owner_inventory_cd=0)) OR ((request->owner_inventory_cd > 0)
    AND (p.cur_owner_area_cd=request->owner_inventory_cd)))
    AND (((request->inventory_area_cd=0)) OR ((request->inventory_area_cd > 0)
    AND (p.cur_inv_area_cd=request->inventory_area_cd))) )
   JOIN (d)
   JOIN (w
  ...
  HEAD o.order_id
   ...
   IF ((request->not_on_worksheet_ind=1))
    IF (w.worklist_detail_id=0)
     nordercnt = (nordercnt+ 1), reply->order_list[nordercnt].product_number = p.product_nbr, reply->
     order_list[nordercnt].product_id = p.product_id,
     reply->order_list[nordercnt].order_id = o.order_id
    ENDIF
   ELSE
    nordercnt = (nordercnt+ 1), reply->order_list[nordercnt].product_number = p.product_nbr, reply->
    order_list[nordercnt].product_id = p.product_id,
  ...
  DETAIL
   row + 0
  FOOT REPORT
   lstat = alterlist(reply->order_list,nordercnt), ntotal = nordercnt, select_ok_flag = 1
 ...
 SUBROUTINE getcodevalues(null)
   ...
   IF (ddiscontinued_cv=0)
    ...
    RETURN(0)
   ENDIF
   SET lstat = uar_get_meaning_by_codeset(ldept_status_cs,nullterm(sordered_cdf),code_cnt,dordered_cv
    )
   IF (dordered_cv=0)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus[1].operationname = "bb_get_prod_ord_by_criteria.prg"
    ...
    SET reply->status_data.subeventstatus[1].targetobjectname = "uar_get_meaning_by_codeset"
    SET reply->status_data.subeventstatus[1].targetobjectvalue =
    "Unable to retrieve the code_value for the cdf_meaning ORDERED in code_set 14281."
    SET reply->status = "F"
    RETURN(0)
   ENDIF
   ...
   IF (dinprocess_cv=0)
    ...
    RETURN(0)
   ENDIF
   SET lstat = uar_get_meaning_by_codeset(ldept_status_cs,nullterm(slabcollected_cdf),code_cnt,
    dlabcollected_cv)
   IF (dlabcollected_cv=0)
    SET reply->status_data.status = "F"
    SET reply->status_data.subeventstatus[1].operationname = "bb_get_prod_ord_by_criteria.prg"
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].targetobjectname = "uar_get_meaning_by_codeset"
    SET reply->status_data.subeventstatus[1].targetobjectvalue =
    "Unable to retrieve the code_value for the cdf_meaning LABCOLLECTED in code_set 14281."
    SET reply->status = "F"
    RETURN(0)
   ENDIF
