CREATE PROGRAM dm2_ads_purge_reset:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dapr_config_id = f8 WITH protect, noconstant(0.0)
 DECLARE dapr_continue_loop = i2 WITH protect, noconstant(0)
 DECLARE dapr_ddl_run_id = f8 WITH protect, noconstant(0.0)
 SET dapr_config_id = cnvtreal( $1)
 FREE RECORD dapaip_reply
 RECORD dapaip_reply(
   1 status = c1
   1 status_msg = vc
 )
 IF (check_logfile("start_over",".log","dm2_ads_purge_start_over")=0)
  GO TO exit_program
 ENDIF
 ...
 SELECT INTO "nl:"
  FROM dm2_ddl_ops_log ddol
  WHERE ddol.op_type="PURGE ACTIVITY DATA"
   AND ddol.gen_id=dapr_config_id
  DETAIL
   dapr_ddl_run_id = ddol.run_id
  WITH nocounter
 ;end select
 IF (dapr_ddl_run_id > 0)
  DELETE  FROM dm2_ddl_ops ddo
   WHERE ddo.run_id=dapr_ddl_run_id
    AND ddo.process_option="PURGE SOURCE"
   WITH nocounter
  ;end delete
  ...
  ENDIF
 ENDIF
 EXECUTE dm2_ads_purge_act_inact_person dapr_config_id, 1 WITH replace("REPLY","DAPAIP_REPLY")
 IF ((dapaip_reply->status="F"))
  SET dm_err->err_ind = 1
  CALL disp_msg(dapaip_reply->status_msg,dm_err->logfile,1)
  ROLLBACK
  GO TO exit_program
 ENDIF
 SET dapr_continue_loop3 = 1
 WHILE (dapr_continue_loop3=1)
   DELETE  FROM dm_ads_config_driver dacd
    WHERE dacd.dm_ads_config_id=dapr_config_id
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   IF (((curqual=0) OR (curqual < 10000)) )
    SET dapr_continue_loop3 = 0
   ENDIF
 ENDWHILE
 DELETE  FROM dm_ads_config_extract dace
  WHERE dace.dm_ads_config_id=dapr_config_id
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
 ...
 DELETE  FROM dm_ads_config dac
  WHERE dac.dm_ads_config_id=dapr_config_id
  WITH nocounter
 ;end delete
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->err_ind = 1
 ...
 SELECT INTO "nl:"
  FROM dba_db_links d
  WHERE d.db_link="PURGE_TGT.WORLD"
  WITH nocounter
 ;end select
 IF (curqual > 0)
  CALL parser("rdb drop public database link purge_tgt go")
 ENDIF
#exit_program
END GO
