CREATE PROGRAM bbt_upd_lab_results:dba
 ...
 SUBROUTINE (check_log_level(arg_log_level=i4) =i2)
   ...
   ENDIF
 END ;Subroutine
 SET log_program_name = "bbt_upd_lab_results"
 RECORD reply(
   1 event_dt_tm = dq8
   1 event_tz = i4
   1 orders[*]
   ...
   1 err_pat_cell_on_prd_ord_ind = i2
   1 err_pat_cell_on_prd_ord_id = f8
   1 err_duplicate_order_ind = i2
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 SUBROUTINE (addtimezonerequest(service_resource_cd=f8) =null)
 ...
 END ;Subroutine
 SUBROUTINE (getrequestedtimezone(service_resource_cd=f8) =i4)
   DECLARE idx = i4 WITH protect, noconstant(0)
 ...
 SUBROUTINE exit_1050064(happ,htask,hreq)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE (gettimezoneshortname(tz=i4) =vc)
 ...
 SUBROUTINE (gm_i_person_rh_phen2989_dq8(icol_name=vc,ival=f8,iqual=i4,null_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 EXECUTE gm_person_rh_phen2989_def "U"
 SUBROUTINE (gm_u_person_rh_phen2989_f8(icol_name=vc,ival=f8,iqual=i4,null_ind=i2,wq_ind=i2) =i2)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_person_rh_phen2989_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_person_rh_phen2989_req->qual,iqual)
    IF (stat=0)
     CALL echo("can not expand request structure")
     RETURN(0)
    ENDIF
   ENDIF
   CASE (cnvtlower(icol_name))
    OF "person_rh_pheno_rs_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->person_rh_pheno_rs_idf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].person_rh_pheno_rs_id = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->person_rh_pheno_rs_idw = 1
     ENDIF
    OF "person_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->person_idf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].person_id = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->person_idw = 1
     ENDIF
    OF "encntr_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->encntr_idf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].encntr_id = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->encntr_idw = 1
     ENDIF
    OF "result_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->result_idf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].result_id = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->result_idw = 1
     ENDIF
    OF "nomenclature_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->nomenclature_idf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].nomenclature_id = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->nomenclature_idw = 1
     ENDIF
    OF "person_rh_phenotype_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->person_rh_phenotype_idf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].person_rh_phenotype_id = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->person_rh_phenotype_idw = 1
     ENDIF
    OF "active_status_cd":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->active_status_cdf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].active_status_cd = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->active_status_cdw = 1
     ENDIF
    OF "active_status_prsnl_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->active_status_prsnl_idf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].active_status_prsnl_id = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->active_status_prsnl_idw = 1
     ENDIF
    OF "donor_rh_phenotype_id":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->donor_rh_phenotype_idf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].donor_rh_phenotype_id = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->donor_rh_phenotype_idw = 1
     ENDIF
    ELSE
     CALL echo("invalid column name passed")
     RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (gm_u_person_rh_phen2989_i2(icol_name=vc,ival=i2,iqual=i4,null_ind=i2,wq_ind=i2) =i2)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_person_rh_phen2989_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_person_rh_phen2989_req->qual,iqual)
    IF (stat=0)
   ...
   CASE (cnvtlower(icol_name))
    OF "active_ind":
     IF (null_ind=1)
      SET gm_u_person_rh_phen2989_req->active_indf = 2
     ELSE
      SET gm_u_person_rh_phen2989_req->active_indf = 1
     ENDIF
     SET gm_u_person_rh_phen2989_req->qual[iqual].active_ind = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->active_indw = 1
     ENDIF
    ELSE
     CALL echo("invalid column name passed")
     RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (gm_u_person_rh_phen2989_i4(icol_name=vc,ival=i4,iqual=i4,null_ind=i2,wq_ind=i2) =i2)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_person_rh_phen2989_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_person_rh_phen2989_req->qual,iqual)
    IF (stat=0)
   ...
   CASE (cnvtlower(icol_name))
    OF "updt_cnt":
     IF (null_ind=1)
      ...
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->updt_cntf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].updt_cnt = ival
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->updt_cntw = 1
     ENDIF
    ELSE
     CALL echo("invalid column name passed")
     RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (gm_u_person_rh_phen2989_dq8(icol_name=vc,ival=f8,iqual=i4,null_ind=i2,wq_ind=i2) =i2)
   DECLARE stat = i2 WITH protect, noconstant(0)
   IF (size(gm_u_person_rh_phen2989_req->qual,5) < iqual)
    SET stat = alterlist(gm_u_person_rh_phen2989_req->qual,iqual)
    IF (stat=0)
   ...
   CASE (cnvtlower(icol_name))
    OF "updt_dt_tm":
     IF (null_ind=1)
      ...
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->updt_dt_tmf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].updt_dt_tm = cnvtdatetime(ival)
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->updt_dt_tmw = 1
     ENDIF
    OF "active_status_dt_tm":
     IF (null_ind=1)
      CALL echo("error can not set this column to null")
      RETURN(0)
     ENDIF
     SET gm_u_person_rh_phen2989_req->active_status_dt_tmf = 1
     SET gm_u_person_rh_phen2989_req->qual[iqual].active_status_dt_tm = cnvtdatetime(ival)
     IF (wq_ind=1)
      SET gm_u_person_rh_phen2989_req->active_status_dt_tmw = 1
     ENDIF
    ELSE
     CALL echo("invalid column name passed")
     RETURN(0)
 ...
 DECLARE result_status_pending_cdf = c12 WITH public, constant("PENDING")
 DECLARE result_status_in_lab_cdf = c12 WITH public, constant("INLAB")
 DECLARE result_status_performed_cdf = c12 WITH public, constant("PERFORMED")
 DECLARE result_status_old_perf_cdf = c12 WITH public, constant("OLDPERFORMED")
 ...
 DECLARE assigned_cdf = c12 WITH public, constant("1")
 ...
 DECLARE result_status_performed_disp = c40
 ...
 DECLARE assigned_cd = f8 WITH protect, noconstant(0.0)
 DECLARE cv_cnt = i4
 ...
 DECLARE service_resource_tz = i4 WITH noconstant(0)
 ...
 DECLARE sub_bb_except_cxm_id = f8 WITH protect, noconstant(0.0)
 DECLARE sub_bb_exc_cxm_product_id = f8 WITH protect, noconstant(0.0)
 DECLARE prod_inv_area_cd = f8 WITH protect, noconstant(0.0)
...
#script
 ...
 FOR (oidx = 1 TO nbr_of_orders)
   IF ((request->orders[oidx].order_load_dt_tm != null))
    IF (read_existing_result(request->orders[oidx].order_id,request->orders[oidx].order_load_dt_tm)=1
    )
     SET failed = "T"
     SET status_count += 1
     IF (status_count > 1)
     ...
     SET reply->status_data.subeventstatus[status_count].targetobjectvalue = concat(
      "Results have already been performed/verified for Accession.")
     SET reply->err_duplicate_order_ind = 1
     GO TO exit_script
    ENDIF
   ENDIF
 ENDFOR
 ...
 ENDIF
 SET stat = uar_get_meaning_by_codeset(result_status_codeset,result_status_pending_cdf,1,
  result_status_pending_cd)
 IF (result_status_pending_cd=0.0)
  SET failed = "T"
  CALL subevent_add("UAR","F",result_status_codeset,"result_status_pending_cd")
 ENDIF
 SET stat = uar_get_meaning_by_codeset(result_status_codeset,result_status_in_lab_cdf,1,
  result_status_in_lab_cd)
 IF (result_status_in_lab_cd=0.0)
  SET failed = "T"
  CALL subevent_add("UAR","F",result_status_codeset,"result_status_in_lab_cd")
 ENDIF
 SET stat = uar_get_meaning_by_codeset(result_status_codeset,result_status_performed_cdf,1,
  result_status_performed_cd)
 IF (result_status_performed_cd=0.0)
  SET failed = "T"
  CALL subevent_add("UAR","F",result_status_codeset,"result_status_performed_cd")
 ENDIF
 SET result_status_performed_disp = uar_get_code_display(result_status_performed_cd)
 IF (trim(result_status_performed_disp)="")
  SET failed = "T"
  CALL subevent_add("UAR","F",result_status_codeset,"result_status_performed_disp")
 ENDIF
 SET stat = uar_get_meaning_by_codeset(result_status_codeset,result_status_old_perf_cdf,1,
  result_status_old_perf_cd)
 IF (result_status_old_perf_cd=0.0)
  SET failed = "T"
  CALL subevent_add("UAR","F",result_status_codeset,"result_status_old_perf_cd")
 ENDIF
 ...
 ENDIF
 SET stat = uar_get_meaning_by_codeset(product_state_codeset,assigned_cdf,1,assigned_cd)
 IF (assigned_cd=0.0)
  SET failed = "T"
  CALL subevent_add("UAR","F",product_state_codeset,"assigned_cd")
 ENDIF
...
#exit_load_interceptor
 ...
 FOR (oidx = 1 TO nbr_of_orders)
   ...
   FOR (aidx = 1 TO nbr_of_assays)
     ...
     IF ((request->use_req_dt_tm_ind=1))
      SET reply->orders[oidx].assays[aidx].perform_dt_tm = cnvtdatetime(request->orders[oidx].assays[
       aidx].perform_dt_tm)
     ELSE
      SET request->orders[oidx].assays[aidx].perform_dt_tm = cnvtdatetime(request->event_dt_tm)
      SET reply->orders[oidx].assays[aidx].perform_dt_tm = cnvtdatetime(request->event_dt_tm)
     ENDIF
     ...
     IF (curutc=1)
      IF ((request->orders[oidx].assays[aidx].interface_flag != 1))
       SET reply->orders[oidx].assays[aidx].perform_tz = curtimezoneapp
      ELSE
       IF (aidx=1)
        CALL addtimezonerequest(request->orders[oidx].assays[aidx].service_resource_cd)
        IF (loadrequestedtimezone(0)=1)
         SET service_resource_tz = getrequestedtimezone(request->orders[oidx].assays[aidx].
          service_resource_cd)
        ELSE
         SET service_resource_tz = curtimezoneapp
        ENDIF
       ENDIF
       SET reply->event_tz = service_resource_tz
       SET reply->orders[oidx].assays[aidx].perform_tz = service_resource_tz
      ENDIF
     ELSE
     ...
     IF ((request->orders[oidx].assays[aidx].inprogress_prod_event_id > 0))
      ...
      ELSEIF ((request->orders[oidx].assays[aidx].product_id != hold_product_id))
       SET bb_result_seq = 0.0
      ENDIF
     ELSEIF ((request->orders[oidx].assays[aidx].bb_control_cell_cd > 0))
      IF ((request->orders[oidx].assays[aidx].bb_result_id > 0))
       SET bb_result_seq = request->orders[oidx].assays[aidx].bb_result_id
     ...
     ELSEIF ((request->orders[oidx].assays[aidx].product_id > 0)
      AND (request->orders[oidx].assays[aidx].inprogress_prod_event_id <= 0))
      IF ((request->orders[oidx].assays[aidx].bb_result_id > 0))
       SET bb_result_seq = request->orders[oidx].assays[aidx].bb_result_id
      ...
      orders[oidx].assays[aidx].next_row_ind=1))) )
       SET bb_result_seq = 0.0
      ENDIF
     ELSEIF ((request->orders[oidx].assays[aidx].bb_result_id_yn="Y"))
      IF ((request->orders[oidx].assays[aidx].bb_result_id > 0))
       SET bb_result_seq = request->orders[oidx].assays[aidx].bb_result_id
      ELSEIF ((request->orders[oidx].assays[aidx].next_row_ind=1))
       SET bb_result_seq = 0.0
      ENDIF
     ELSE
      SET bb_result_seq = 0.0
     ...
     ELSE
      ...
      bb_control_cell_cd
     ENDIF
     IF (in_progress_prev_update="Y")
      SET reply->orders[oidx].assays[aidx].inprogress_updt_cnt = (request->orders[oidx].assays[aidx].
      prod_state_updt_cnt+ 1)
     ELSE
      SET reply->orders[oidx].assays[aidx].inprogress_updt_cnt = request->orders[oidx].assays[aidx].
      prod_state_updt_cnt
     ENDIF
     ...
     IF ((request->orders[oidx].donor_ind=0))
      IF ((request->orders[oidx].assays[aidx].inprogress_prod_event_id > 0)
       ...
       orders[oidx].assays[aidx].crossmatch_verify_yn="Y"))) )
        ...
        ENDIF
       ENDIF
      ELSEIF ((request->orders[oidx].bb_processing_mean=xm_order_processing_mean))
      ...
      ELSEIF ((request->orders[oidx].assays[aidx].antibody_verify_yn="Y")
       ...
       IF (gsub_person_antibody_status != "OK")
        IF (gsub_person_antibody_status="FS")
         SET failed = "T"
         SET status_count += 1
         IF (status_count > 1)
          SET stat = alter(reply->status_data.subeventstatus,(status_count+ 1))
        ...
        ELSEIF (gsub_person_antibody_status="FA")
         SET failed = "T"
         SET status_count += 1
         IF (status_count > 1)
          SET stat = alter(reply->status_data.subeventstatus,(status_count+ 1))
      ...
      ELSEIF ((request->orders[oidx].assays[aidx].antigen_verify_yn="Y")
       AND (request->orders[oidx].assays[aidx].bb_result_code_set_cd > 0))
       IF ((request->orders[oidx].patient_order_ind=0))
        SET failed = "T"
        SET reply->err_pat_cell_on_prd_ord_ind = 1
        SET reply->err_pat_cell_on_prd_ord_id = request->orders[oidx].order_id
        GO TO exit_script
       ENDIF
       SET gsub_person_antigen_status = "  "
       ...
       IF (gsub_person_antigen_status != "OK")
        IF (gsub_person_antigen_status="FS")
         ...
         SET reply->status_data.subeventstatus[status_count].targetobjectvalue =
         "Unable to insert person antigen due to next sequence number"
      ...
      ELSEIF ((request->orders[oidx].assays[aidx].special_testing_verify_yn="Y")
       ...
       IF ((request->orders[oidx].assays[aidx].product_test_special_test_yn="Y"))
        SET product_id = request->orders[oidx].person_id
       ELSE
        SET product_id = request->orders[oidx].assays[aidx].product_id
       ENDIF
       SET opposite_found_ind = 0
       SELECT INTO "nl:"
        st.product_id, st.special_testing_cd, st.active_ind,
        cv.code_value, cv.code_set, cv.cdf_meaning,
        cve.code_set, cve.code_value, cve.field_name,
        cve.field_value
       ...
       IF (opposite_found_ind=1)
        SET reply->opposite_found_product_id = product_id
        ...
        GO TO exit_script
       ELSE
        CALL add_special_testing(product_id,request->orders[oidx].assays[aidx].bb_result_code_set_cd,
         1,0.0,1,
        ...
        IF (gsub_special_testing_status != "OK")
         ...
         GO TO exit_script
        ENDIF
        CALL add_special_testing_result(new_special_testing_id,product_id,reply->orders[oidx].assays[
         aidx].result_id,bb_result_seq,1,
         reqdata->active_status_cd,cnvtdatetime(current->system_dt_tm),request->event_personnel_id)
      ...
      ELSEIF ((request->orders[oidx].assays[aidx].rh_phenotype_verify_yn="Y")
       ...
       SET new_person_rh_phenotype_id = 0.0
       IF ((request->orders[oidx].assays[aidx].upd_rh_phenotype_yn="Y"))
        ...
        ELSEIF (gsub_rh_phenotype_status != "OK")
         ...
         SET reply->status_data.subeventstatus[status_count].targetobjectname = "person_rh_phenotype"
         SET reply->status_data.subeventstatus[status_count].targetobjectvalue = concat(
          "Could not insert person_rh_phenotype row due to invalid insert status (",
          gsub_rh_phenotype_status,") for order_id = ",cnvtstring(request->orders[oidx].order_id,32,2
           ))
         GO TO exit_script
        ENDIF
        ...
        FOR (rh_a = 1 TO rh_a_cnt)
          ...
          ELSEIF (gsub_person_antigen_status != "OK")
           ...
           SET reply->status_data.subeventstatus[status_count].targetobjectname = "person_antigen"
           SET reply->status_data.subeventstatus[status_count].targetobjectvalue = concat(
            "Could not insert person_antigen row due to invalid insert status (",
            gsub_person_antigen_status,") for order_id = ",cnvtstring(request->orders[oidx].order_id,
             32,2))
           GO TO exit_script
          ENDIF
        ENDFOR
       ...
       ELSEIF (gsub_rh_phenotype_status != "OK")
        ...
        "person_rh_pheno_result"
        SET reply->status_data.subeventstatus[status_count].targetobjectvalue = concat(
         "Could not insert person_rh_pheno_result row due to invalid insert status (",
         gsub_rh_phenotype_status,") for order_id = ",cnvtstring(request->orders[oidx].order_id,32,2)
         )
        GO TO exit_script
       ENDIF
        ...
        IF ((request->orders[oidx].assays[aidx].rh_phenotype_id > 0))
         ...
         FOR (rh_a = 1 TO rh_a_cnt)
          ...
          IF (gsub_rh_phenotype_status="FL")
           ...
           SET reply->status_data.subeventstatus[status_count].targetobjectname = "special_testing"
           SET reply->status_data.subeventstatus[status_count].targetobjectvalue = concat(
            "Lock special_testing row for update failed for special_testing_id = ",cnvtstring(request
        ...
        FOR (rh_a = 1 TO rh_a_cnt)
          ...
          ELSEIF (gsub_special_testing_status != "OK")
           ...
           SET reply->status_data.subeventstatus[status_count].targetobjectvalue = concat(
            ...
            gsub_special_testing_status,") for order_id = ",cnvtstring(request->orders[oidx].order_id,
             32,2))
           GO TO exit_script
          ENDIF
          CALL add_special_testing_result(new_special_testing_id,request->orders[oidx].person_id,
           reply->orders[oidx].assays[aidx].result_id,0.0,1,
           reqdata->active_status_cd,cnvtdatetime(current->system_dt_tm),request->event_personnel_id)
          IF (gsub_spc_tst_result_status != "OK")
           IF (gsub_spc_tst_result_status="FA")
            SET failed = "T"
     ...
     IF ((request->review_queue_ind=0))
      ...
      IF (nbr_of_excepts > 0)
       FOR (cntr = 1 TO nbr_of_excepts)
         ...
         SET sub_bb_exception_id = 0.0
         IF ((request->orders[oidx].assays[aidx].result_status_cd != result_status_in_review_cd))
          CALL add_bb_exception(request->orders[oidx].order_person_id,request->orders[oidx].order_id,
           request->event_personnel_id,cnvtdatetime(request->event_dt_tm),product_event_id,
           request->orders[oidx].assays[aidx].exceptlist[cntr].exception_type_mean,request->orders[
           oidx].assays[aidx].exceptlist[cntr].override_reason_cd,re_event_type_cd,request->orders[
           oidx].assays[aidx].result_id,perf_result_seq,
          ...
          ENDIF
         ELSE
          CALL add_bb_inactive_exception(request->orders[oidx].order_person_id,request->orders[oidx].
           order_id,request->event_personnel_id,cnvtdatetime(request->event_dt_tm),product_event_id,
           request->orders[oidx].assays[aidx].exceptlist[cntr].exception_type_mean,request->orders[
           oidx].assays[aidx].exceptlist[cntr].override_reason_cd,re_event_type_cd,request->orders[
           oidx].assays[aidx].result_id,perf_result_seq,
           request->orders[oidx].assays[aidx].exceptlist[cntr].from_abo_cd,request->orders[oidx].
   ...
   IF ((request->orders[oidx].cxm_ind=1))
    SET exception_status = "I"
    SET bb_exception_id = 0.0
    SET sub_bb_exception_id = 0.0
    CALL add_bb_exception(request->orders[oidx].order_person_id,request->orders[oidx].order_id,
     request->event_personnel_id,cnvtdatetime(request->event_dt_tm),0.0,
     request->orders[oidx].order_except_type_mean,0.0,0.0,0.0,0.0,
     request->orders[oidx].order_except_from_abo_cd,request->orders[oidx].order_except_from_rh_cd,0.0,
     0.0,0.0)
    SET sub_bb_exception_id = bb_exception_id
    IF (exception_status="F")
     SET failed = "T"
     ...
     GO TO exit_script
    ENDIF
    FOR (idx = 1 TO request->orders[oidx].order_except_sub_except_cnt)
      SET sub_bb_except_cxm_id = 0.0
      SET sub_bb_except_cxm_id = next_pathnet_seq(0)
      IF (sub_bb_except_cxm_id > 0)
       INSERT  FROM bb_except_cxm bec
        SET bec.bb_except_cxm_id = sub_bb_except_cxm_id, bec.message_cd = request->orders[oidx].
         order_except_sub_exceptlist[idx].message_cd, bec.override_reason_cd = request->orders[oidx].
         order_except_sub_exceptlist[idx].reason_cd,
         bec.bb_exception_id = sub_bb_exception_id, bec.updt_cnt = 0, bec.updt_dt_tm = cnvtdatetime(
          sysdate),
         bec.updt_id = reqinfo->updt_id, bec.updt_task = reqinfo->updt_task, bec.updt_applctx =
         reqinfo->updt_applctx,
         bec.active_ind = 1, bec.active_status_cd = reqdata->active_status_cd, bec
         .active_status_dt_tm = cnvtdatetime(sysdate),
         bec.active_status_prsnl_id = reqinfo->updt_id
        WITH nocounter
       ;end insert
       IF (curqual=0)
        SET failed = "T"
        ...
        SET reply->status_data.subeventstatus[status_count].operationname = "INSERT"
        SET reply->status_data.subeventstatus[status_count].operationstatus = "F"
        SET reply->status_data.subeventstatus[status_count].targetobjectname = "bb_except_cxm"
        SET reply->status_data.subeventstatus[status_count].targetobjectvalue =
        "Failure inserting bb_except_cxm row"
        GO TO exit_script
       ENDIF
      ELSE
       SET failed = "T"
       ...
       SET reply->status_data.subeventstatus[status_count].targetobjectname = "bb_except_cxm"
       SET reply->status_data.subeventstatus[status_count].targetobjectvalue =
       "Unable to insert bb_except_cxm row -- failure retrieving pathnet_seq for bb_except_cxm_id"
       GO TO exit_script
      ENDIF
    ENDFOR
    FOR (idx = 1 TO request->orders[oidx].order_except_prod_cnt)
      SET sub_bb_exc_cxm_product_id = 0.0
      SET sub_bb_exc_cxm_product_id = next_pathnet_seq(0)
      IF (sub_bb_exc_cxm_product_id > 0)
       INSERT  FROM bb_exc_cxm_product becp
        SET becp.bb_exc_cxm_product_id = sub_bb_exc_cxm_product_id, becp.bb_exception_id =
         sub_bb_exception_id, becp.product_id = request->orders[oidx].order_except_prodlist[idx].
         product_id,
         becp.product_cd = request->orders[oidx].order_except_prodlist[idx].product_cd, becp.abo_cd
          = request->orders[oidx].order_except_prodlist[idx].abo_cd, becp.rh_cd = request->orders[
         oidx].order_except_prodlist[idx].rh_cd,
         becp.crossmatch_expire_dt_tm = cnvtdatetime(request->orders[oidx].order_except_prodlist[idx]
          .crossmatch_expire_dt_tm), becp.product_event_id = request->orders[oidx].
         order_except_prodlist[idx].product_event_id, becp.updt_cnt = 0,
         becp.updt_dt_tm = cnvtdatetime(sysdate), becp.updt_id = reqinfo->updt_id, becp.updt_task =
         reqinfo->updt_task,
         becp.updt_applctx = reqinfo->updt_applctx, becp.active_ind = 1, becp.active_status_cd =
         reqdata->active_status_cd,
         becp.active_status_dt_tm = cnvtdatetime(sysdate), becp.active_status_prsnl_id = reqinfo->
         updt_id
        WITH nocounter
       ;end insert
       IF (curqual=0)
        ...
        SET reply->status_data.subeventstatus[status_count].operationname = "INSERT"
        SET reply->status_data.subeventstatus[status_count].operationstatus = "F"
        SET reply->status_data.subeventstatus[status_count].targetobjectname = "bb_exc_cxm_product"
        SET reply->status_data.subeventstatus[status_count].targetobjectvalue =
        "failure inserting bb_exc_cxm_product row"
        GO TO exit_script
       ENDIF
      ELSE
       SET failed = "T"
       ...
       SET reply->status_data.subeventstatus[status_count].targetobjectname = "bb_exc_cxm_product"
       SET reply->status_data.subeventstatus[status_count].targetobjectvalue =
       "Unable to insert bb_except_cxm row -- failure retrieving pathnet_seq for bb_exc_cxm_product_id"
       GO TO exit_script
      ENDIF
    ENDFOR
   ENDIF
 ...
 SUBROUTINE (update_crossmatch_bbidnbr_or_xm_reason(sub_ucb_order_id=f8,sub_ucb_bb_id_nbr=vc,
  sub_ucb_xm_reason_cd=f8) =i4)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (pe
     ...
     WHERE xm.product_event_id=pe.product_event_id
      AND ((((trim(xm.bb_id_nbr)=null) OR (trim(xm.bb_id_nbr)=""))
      AND trim(sub_ucb_bb_id_nbr) != "") OR (xm.xm_reason_cd=0.0
      AND sub_ucb_xm_reason_cd > 0)) )
    HEAD REPORT
     pe_xm_cnt = 0
    DETAIL
     ...
     FOR (pe_xm = 1 TO pe_xm_cnt)
       IF ((pe_xm_rec->pe_xm[pe_xm].status != 1))
        ...
        SET reply->status_data.subeventstatus[status_count].targetobjectvalue = concat(
         ...
         cnvtstring(sub_ucb_order_id,32,2),".  Product_Event_Id = ",cnvtstring(pe_xm_rex->pe_xm[pe_xm
          ].product_event_id,32,2))
        RETURN(0)
       ENDIF
     ENDFOR
     RETURN(1)
   ...
   ENDIF
 END ;Subroutine
 DECLARE process_new_result() = i4
 SUBROUTINE process_new_result(null)
 ...
 SUBROUTINE process_updated_result(null)
   ...
   ELSE
    ...
    result_status_in_review_cd, result_status_verified_cd)))
     ...
     ENDIF
     IF (curr_result_status_cd IN (result_status_performed_cd, result_status_in_review_cd)
      AND (request->orders[oidx].assays[aidx].result_status_cd IN (result_status_in_review_cd,
     result_status_verified_cd))
      AND (request->orders[oidx].assays[aidx].perform_ind=0))
      IF ((request->orders[oidx].assays[aidx].result_status_cd=result_status_verified_cd))
       IF (update_perform_result(request->orders[oidx].assays[aidx].result_id,perf_result_seq,
 ...
 END ;Subroutine
 DECLARE bb_result_seq = f8
 DECLARE insert_result() = i4
 SUBROUTINE insert_result(null)
   IF ((request->orders[oidx].assays[aidx].inprogress_prod_event_id > 0))
    IF ((request->orders[oidx].assays[aidx].bb_result_id <= 0))
   ...
   ELSEIF ((request->orders[oidx].assays[aidx].bb_result_id_yn="Y"))
    IF ((request->orders[oidx].assays[aidx].next_row_ind=1)
     AND (request->orders[oidx].assays[aidx].bb_result_id <= 0.0))
     SELECT INTO "nl:"
      next_seq_nbr = seq(pathnet_seq,nextval)
      FROM dual
      DETAIL
   ...
   INSERT  FROM result r
    SET r.result_id = request->orders[oidx].assays[aidx].result_id, r.bb_result_id = bb_result_seq, r
     ...
     r.result_status_cd = request->orders[oidx].assays[aidx].result_status_cd, r.chartable_flag =
     request->orders[oidx].assays[aidx].chartable_flag, r.security_level_cd = request->orders[oidx].
     assays[aidx].security_level_cd,
     r.repeat_number = request->orders[oidx].assays[aidx].repeat_number, r.bb_control_cell_cd =
     request->orders[oidx].assays[aidx].bb_control_cell_cd, r.person_id =
     IF ((request->orders[oidx].patient_order_ind=1)
      AND (request->orders[oidx].person_id != null)
      AND (request->orders[oidx].person_id > 0)) request->orders[oidx].person_id
     ELSE 0
     ENDIF
     ,
     r.bb_group_id = request->orders[oidx].assays[aidx].bb_group_id, r.lot_information_id = request->
     orders[oidx].assays[aidx].lot_information_id, r.updt_dt_tm = cnvtdatetime(sysdate),
   ...
   ELSE
    UPDATE  FROM result r
     SET r.call_back_ind = request->orders[oidx].assays[aidx].call_back_ind, r.result_status_cd =
      ...
      cnvtdatetime(sysdate), r.updt_id = reqinfo->updt_id,
 ...
 SUBROUTINE insert_perform_result(arg_result_id,arg_parent_perf_rslt_id)
   ...
   INSERT  FROM perform_result pr
    SET pr.perform_result_id = perf_result_seq, pr.result_id = arg_result_id, pr
     ...
     ELSE null
     ENDIF
     , pr.interface_status_flag =
     IF (validate(request->orders[oidx].assays[aidx].interface_status_flag)) request->orders[oidx].
      assays[aidx].interface_status_flag
     ELSE 0
     ENDIF
     ...
     pr.normal_cd = request->orders[oidx].assays[aidx].normal_cd,
     pr.critical_cd = request->orders[oidx].assays[aidx].critical_cd, pr.review_cd = request->orders[
     oidx].assays[aidx].review_cd, pr.delta_cd = request->orders[oidx].assays[aidx].delta_cd,
     pr.notify_cd = request->orders[oidx].assays[aidx].notify_cd, pr.units_cd = request->orders[oidx]
     .assays[aidx].units_cd, pr.normal_low = request->orders[oidx].assays[aidx].normal_low,
     pr.normal_high = request->orders[oidx].assays[aidx].normal_high, pr.normal_alpha =
     IF (trim(request->orders[oidx].assays[aidx].normal_alpha) > " ") request->orders[oidx].assays[
      aidx].normal_alpha
     ELSE null
     ENDIF
     , pr.dilution_factor = request->orders[oidx].assays[aidx].dilution_factor,
     pr.resource_error_codes = request->orders[oidx].assays[aidx].resource_error_codes, pr
     .equation_id = request->orders[oidx].assays[aidx].equation_id, pr.multiplex_resource_cd =
     request->orders[oidx].assays[aidx].multiplex_resource_cd,
     pr.interp_override_ind = request->orders[oidx].assays[aidx].interp_override_ind, pr
     .interface_flag = request->orders[oidx].assays[aidx].interface_flag, pr.updt_dt_tm =
     cnvtdatetime(sysdate),
     pr.updt_id = reqinfo->updt_id, pr.updt_task = reqinfo->updt_task, pr.updt_applctx = reqinfo->
     updt_applctx,
     pr.updt_cnt = 0, pr.perform_tz =
 ...
 SUBROUTINE read_perform_result(arg_result_id,arg_perf_result_id)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     .parent_perform_result_id, reply->orders[oidx].assays[aidx].perform_dt_tm = pr.perform_dt_tm,
     "@SHORTDATETIME"
    WITH nocounter, format, format(date,"dd-mmm-yyyy hh:mm:ss;;d")
   ;end select
   RETURN(curqual)
 END ;Subroutine
 ...
 SUBROUTINE insert_result_event(arg_result_id,arg_perf_result_id,arg_event_type_cd,arg_event_reason)
   ...
   SELECT INTO "nl:"
    ...
    ORDER BY re.result_id, re.event_sequence DESC
    HEAD re.result_id
 ...
 SUBROUTINE insert_result_comment(arg_result_id)
   ...
   INSERT  FROM result_comment rc
    SET rc.result_id = arg_result_id, rc.action_sequence = (last_action_seq+ 1), rc.comment_type_cd
     ...
     rc.long_text_id = long_text_seq, rc.comment_prsnl_id =
     IF ((request->orders[oidx].assays[aidx].result_comment[rcidx].comment_prsnl_id=0)) reqinfo->
      updt_id
     ELSE request->orders[oidx].assays[aidx].result_comment[rcidx].comment_prsnl_id
     ENDIF
 ...
 SUBROUTINE update_patient_aborh(null)
   IF ((request->orders[oidx].assays[aidx].upd_pat_hist_aborh_yn="Y"))
    ...
    IF (pa_rec_cnt=0)
     ...
     ENDIF
    ELSEIF (pa_rec_cnt=1)
     IF ((((pa_abo_cd != request->orders[oidx].assays[aidx].orig_abo_cd)) OR ((((pa_rh_cd != request
     ->orders[oidx].assays[aidx].orig_rh_cd)) OR ((((pa_person_aborh_id != request->orders[oidx].
     assays[aidx].person_aborh_id)) OR ((pa_updt_cnt != request->orders[oidx].assays[aidx].
     person_aborh_updt_cnt))) )) )) )
      SET failed = "T"
      SET status_count += 1
      IF (status_count > 1)
       SET stat = alter(reply->status_data.subeventstatus,(status_count+ 1))
    ...
    IF ((((request->orders[oidx].assays[aidx].new_abo_cd > 0)) OR ((request->orders[oidx].assays[aidx
     ...
     ELSEIF (abo_rh_test="Y")
      ...
      ENDIF
     ENDIF
     SET reply->orders[oidx].assays[aidx].new_aborh_updt_cnt = 0
     SET reply->orders[oidx].assays[aidx].person_aborh_id = person_aborh_id
    ELSE
     SET write_aborh_result = "Y"
     SELECT
      person_aborh_id
     ...
     SET reply->orders[oidx].assays[aidx].person_aborh_id = request->orders[oidx].assays[aidx].
     person_aborh_id
    ENDIF
   ELSE
   ...
   IF (write_aborh_result="Y")
    ...
    CALL add_aborh_result(request->orders[oidx].assays[aidx].specimen_id,request->orders[oidx].
     assays[aidx].container_id,cnvtdatetime(request->orders[oidx].assays[aidx].drawn_dt_tm),
     person_aborh_id,request->orders[oidx].person_id,
 ...
 SUBROUTINE update_product_aborh(null)
   ...
   IF ((request->orders[oidx].assays[aidx].upd_blood_product_yn="Y"))
    ...
    IF (product_rh_test_only="Y")
     ...
     ELSE
      SET gsub_blood_product_status = "  "
      CALL upd_blood_product(product_id,request->orders[oidx].assays[aidx].product_orig_abo_cd,
    ...
    ELSEIF (product_abo_test_only="Y")
     ...
     ELSE
      IF ((request->orders[oidx].assays[aidx].product_orig_abo_cd=0))
       CALL upd_blood_product(product_id,request->orders[oidx].assays[aidx].product_new_abo_cd,
    ...
    ELSEIF (product_abo_rh_test="Y")
     ...
     IF ((request->orders[oidx].assays[aidx].product_orig_abo_cd=0)
      ...
      CALL upd_blood_product(product_id,request->orders[oidx].assays[aidx].product_new_abo_cd,request
       ->orders[oidx].assays[aidx].product_new_rh_cd,1,request->orders[oidx].assays[aidx].
       blood_product_updt_cnt,
       1)
      IF (gsub_blood_product_status != "OK")
       RETURN(0)
   ...
   IF ((request->orders[oidx].assays[aidx].upd_product_conf_yn="Y"))
    ...
    CALL add_product_event(product_id,0,0,request->orders[oidx].order_id,0,
     confirmed_cd,cnvtdatetime(request->event_dt_tm),request->event_personnel_id,0,
   ...
   IF ((request->orders[oidx].assays[aidx].upd_product_avail_yn="Y"))
    ...
    SELECT
     ...
     DETAIL
      prod_inv_area_cd = p.cur_inv_area_cd
     WITH nocounter
    ;end select
    CALL add_product_event_with_inventory_area_cd(product_id,0,0,0,0,
     ...
     request->event_personnel_id,prod_inv_area_cd)
    IF (gsub_product_event_status="FS")
     SET failed = "T"
     SET status_count += 1
     IF (status_count > 1)
   ...
   IF ((request->orders[oidx].assays[aidx].add_product_unconf_yn="Y"))
    ...
    SET gsub_product_event_status = "  "
    CALL add_product_event(product_id,0,0,0,0,
     unconfirmed_cd,cnvtdatetime(request->event_dt_tm),request->event_personnel_id,0,0,
     0,0,1,reqdata->active_status_cd,cnvtdatetime(current->system_dt_tm),
     request->event_personnel_id)
    IF (gsub_product_event_status="FS")
     SET failed = "T"
   ...
   IF (write_result="Y")
    SET write_result = "N"
    CALL add_abo_testing(product_id,reply->orders[oidx].assays[aidx].result_id,request->orders[oidx].
     assays[aidx].product_new_abo_cd,request->orders[oidx].assays[aidx].product_new_rh_cd,
 ...
 SUBROUTINE update_crossmatch(null)
   ...
   IF ((request->orders[oidx].assays[aidx].crossmatch_verify_yn="Y"))
    IF (in_progress_prev_update="Y")
     SET request->orders[oidx].assays[aidx].prod_state_updt_cnt += 1
    ENDIF
    SET in_progress_prev_update = "N"
    IF (update_in_progress(bb_result_seq,0)=0)
     RETURN(0)
    ENDIF
    SET reply->orders[oidx].assays[aidx].inprogress_updt_cnt = (request->orders[oidx].assays[aidx].
    prod_state_updt_cnt+ 1)
    IF ((request->orders[oidx].assays[aidx].update_to_xm_yn="Y"))
     IF (update_available(hold_product_id)=0)
      RETURN(0)
     ENDIF
     IF (update_assigned(hold_product_id,request->orders[oidx].person_id)=0)
      RETURN(0)
     ENDIF
     SET product_event_id = 0.0
     ...
     CALL add_product_event(request->orders[oidx].assays[aidx].product_id,request->orders[oidx].
      ...
      crossmatch_cd,cnvtdatetime(request->event_dt_tm),request->event_personnel_id,0,
      IF ((request->orders[oidx].assays[aidx].except_cnt > 0)) 1
      ELSE 0
      ENDIF
      ,
     ...
     IF ((request->orders[oidx].cxm_ind=1))
      SET idx = 0
      FOR (idx = 1 TO request->orders[oidx].order_except_prod_cnt)
        IF ((request->orders[oidx].order_except_prodlist[idx].product_id=request->orders[oidx].
        assays[aidx].product_id))
         SET request->orders[oidx].order_except_prodlist[idx].product_event_id = product_event_id
         SET request->orders[oidx].order_except_prodlist[idx].crossmatch_expire_dt_tm = request->
   ...
   ELSE
    IF ((request->orders[oidx].assays[aidx].bb_result_id <= 0)
     AND in_progress_prev_update="N")
     SET in_progress_prev_update = "Y"
     IF (update_in_progress(bb_result_seq,1)=0)
      RETURN(0)
     ENDIF
     SET reply->orders[oidx].assays[aidx].inprogress_updt_cnt = (request->orders[oidx].assays[aidx].
     prod_state_updt_cnt+ 1)
 ...
 SUBROUTINE update_bb_order_cell(arg_bb_result_id)
   ...
   ELSE
    UPDATE  FROM bb_order_cell oc
     SET oc.bb_result_id = arg_bb_result_id, oc.updt_dt_tm = cnvtdatetime(sysdate), oc.updt_id =
      reqinfo->updt_id,
 ...
 SUBROUTINE insert_images(none)
   ...
   DELETE  FROM blob_reference br,
    ...
    PLAN (d
     ...
     result_status_in_review_cd, result_status_performed_cd)))
     JOIN (br
     WHERE (br.blob_ref_id=request->orders[oidx].assays[aidx].images[d.seq].blob_ref_id)
   ...
   FOR (pn_dtl_cnt = 1 TO 8)
     ...
     CASE (pn_dtl_cnt)
      OF 1:
       ...
       SET sub_detail_value = (last_event_seq+ 1)
       SET sub_detail_desc = ""
      OF 2:
 ...
 SUBROUTINE (execute_maintain_review_items(none=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (update_assigned(dproductid=f8,dpersonid=f8) =i4)
   SET return_value = 0
   SET dprod_event_id = 0.0
   SELECT INTO "nl:"
    p.*
    FROM product_event p
    WHERE p.product_id=dproductid
     AND p.person_id=dpersonid
     AND p.event_type_cd=assigned_cd
     AND p.active_ind=1
    DETAIL
     dprod_event_id = p.product_event_id
    WITH nocounter, forupdate(p)
   ...
   ELSE
    ...
    IF (curqual=0)
     ...
     SET reply->status_data.subeventstatus[status_count].targetobjectname = "Product Event"
     SET reply->status_data.subeventstatus[status_count].targetobjectvalue =
     "Unable to update assigned product event"
     SET return_value = 0
    ELSE
     SELECT INTO "nl:"
      a.*
      FROM assign a
      WHERE a.product_event_id=dprod_event_id
       AND a.active_ind=1
     ...
     ELSE
      UPDATE  FROM assign a
       SET a.active_ind = 0, a.active_status_cd = reqdata->active_status_cd, a.updt_cnt = (a.updt_cnt
        + 1),
      ...
      IF (curqual=0)
       ...
       SET reply->status_data.subeventstatus[status_count].operationname = "CHANGE"
       SET reply->status_data.subeventstatus[status_count].operationstatus = "F"
       SET reply->status_data.subeventstatus[status_count].targetobjectname = "Assign"
       SET reply->status_data.subeventstatus[status_count].targetobjectvalue =
       "Unable to update row on assign table."
       SET return_value = 0
      ELSE
       SET return_value = 1
      ENDIF
   ...
   RETURN(return_value)
 END ;Subroutine
 SUBROUTINE (read_existing_result(sub_order_id=f8,sub_order_dttm=dq8) =i2)
   DECLARE dminute = f8
   SELECT INTO "nl:"
    re.event_dt_tm
    FROM result r,
     result_event re
    PLAN (r
     WHERE r.order_id=sub_order_id)
     JOIN (re
     WHERE re.result_id=r.result_id)
    ORDER BY re.event_dt_tm DESC
    DETAIL
     dminute = datetimediff(cnvtdatetime(sub_order_dttm),cnvtdatetime(re.event_dt_tm),5)
...
#exit_script
 ...
 FREE RECORD gm_u_person_rh_phen2989_req
 FREE RECORD gm_u_person_rh_phen2989_rep
 FREE SET rh_a_rec
 FREE SET current
END GO
