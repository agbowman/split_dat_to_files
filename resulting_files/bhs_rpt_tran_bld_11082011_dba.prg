CREATE PROGRAM bhs_rpt_tran_bld_11082011:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dfr
   ...
   WHERE el.encntr_id=ea.encntr_id
    AND el.active_ind=1
    AND ((nurseunitcd != 99999999
    AND el.loc_facility_cd=nurseunitcd) OR (nurseunitcd=99999999))
   ...
   WHERE e.person_id=p.person_id
    AND p.active_ind=1)
  ORDER BY el.loc_nurse_unit_cd, el.encntr_id
  HEAD REPORT
  ...
  HEAD el.encntr_id
   ...
   IF (i_patient_cnt > size(forms->patientslist,5))
    stat = alterlist(forms->patientslist,(i_patient_cnt+ 10))
   ENDIF
   ...
   i_patient_cnt].f_nurseunit_cd = el.loc_nurse_unit_cd,
   forms->patientslist[i_patient_cnt].f_encntr_id = el.encntr_id, forms->patientslist[i_patient_cnt].
   f_person_id = e.person_id, forms->patientslist[i_patient_cnt].s_person_name = concat(trim(p
     .name_first,3)," ",trim(p.name_last,3)),
   forms->patientslist[i_patient_cnt].s_finnbr = check(trim(ea.alias,3)), forms->patientslist[
   i_patient_cnt].mrn = check(trim(ea1.alias,3)), forms->patientslist[i_patient_cnt].facility =
   facility,
   forms->patientslist[i_patient_cnt].patient_type = encntr_type, forms->patientslist[i_patient_cnt].
   location = nurse_unit, forms->patientslist[i_patient_cnt].age = trim(cnvtage(p.birth_dt_tm),3),
   forms->patientslist[i_patient_cnt].admit_date = format(e.reg_dt_tm,"mm/dd/yy hh:mm;;d"), forms->
   patientslist[i_patient_cnt].discharge_date = format(e.disch_dt_tm,"mm/dd/yy hh:mm;;d"), forms->
   patientslist[i_patient_cnt].service = trim(med_service,3),
   forms->patientslist[i_patient_cnt].primary_provider = trim(plan_name,3), forms->patientslist[
   i_patient_cnt].beg_unit = el.beg_effective_dt_tm, forms->patientslist[i_patient_cnt].end_unit = el
   .end_effective_dt_tm,
   forms->patientslist[i_patient_cnt].abo = "No  Data", forms->patientslist[i_patient_cnt].problems
    = "No Data"
  FOOT  el.loc_nurse_unit_cd
 ...
 SET starttime = cnvtdatetime(curdate,curtime3)
 SELECT INTO "nl:"
  nurseunit = uar_get_code_display(forms->patientslist[d1.seq].f_nurseunit_cd), name = forms->
  patientslist[d1.seq].s_person_name, ce.parent_event_id,
  ce2.event_cd
  FROM (dummyt d1  WITH seq = value(size(forms->patientslist,5))),
   clinical_event ce,
   clinical_event ce1,
  ...
  ORDER BY d1.seq, ce.parent_event_id, ce2.event_cd DESC
  HEAD REPORT
  ...
  HEAD d1.seq
   ...
   i_witness_cnt = 0, stat = alterlist(forms->patientslist[d1.seq].forminstanceslist,10)
  HEAD ce.parent_event_id
   CALL echo(build("head ce.parent_event_id = ",ce.parent_event_id)), i_trandta_cnt = 0,
   i_consent_cnt = 0,
   i_vital_cnt = 0, i_infused_cnt = 0, i_reaction_cnt = 0,
   i_witness_cnt = 0, i_instance_cnt = (i_instance_cnt+ 1),
   CALL echo(build("i_instance_cnt = ",i_instance_cnt))
   IF (mod(i_instance_cnt,10)=1
    AND i_instance_cnt != 1)
    stat = alterlist(forms->patientslist[d1.seq].forminstanceslist,(i_instance_cnt+ 9))
   ENDIF
   forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].f_parent_event_id = ce
   .parent_event_id, forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].order_id = ce
   .order_id, forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].formname =
   uar_get_code_display(ce.event_cd),
   forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_performed_on = build(format(ce
     .performed_dt_tm,";;q"),"_",ce.parent_event_id), forms->patientslist[d1.seq].forminstanceslist[
   i_instance_cnt].d_performed_dt_tm = ce.performed_dt_tm, forms->patientslist[d1.seq].
   forminstanceslist[i_instance_cnt].weight = "No weight available",
  ...
  HEAD ce2.event_cd
   ...
   IF (ce3.event_cd=mf_consentsignedcurrentornotapplicable)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    s_consentsignedcurrentornotapplicable = ce3.event_tag, i_consent_cnt = (i_consent_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_transfusionstarttime, autotransfusionstarttime))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_transfusionstarttime =
    s_tran_result, forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].
    d_transfusionstarttime = cnvtdatetime(s_tran_result), i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_temperaturestart, temperaturestartautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_temperaturestart = s_tran_result,
    i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_temperatureroutestart, temperatureroutestartautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_temperatureroutestart =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_pulseratestart, pulseratestartautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_pulseratestart = s_tran_result,
    i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_respiratoryratestart, respiratoryratestartautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_respiratoryratestart =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_systolicbloodpressurestart, sbpstartautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_systolicbloodpressurestart =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_diastolicbloodpressurestart, dbpstartautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_diastolicbloodpressurestart =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_oxygensaturationstart, oxygensatstartautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_oxygensaturationstart =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_transfusionstartplus15min, autotransfusionstartplus15min))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_transfusionstartplus15min =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_temperature15min, temperature15minautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_temperature15min = s_tran_result,
    i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_temperatureroute15min, temperatureroute15minautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_temperatureroute15min =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_pulserate15min, pulserate15minautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_pulserate15min = s_tran_result,
    i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_respiratoryrate15min, respiratoryrate15minautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_respiratoryrate15min =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_systolicbloodpressure15min, sbp15minautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_systolicbloodpressure15min =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_diastolicbloodpressure15min, dbp15minautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_diastolicbloodpressure15min =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_oxygensaturation15min, oxygensat15minautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_oxygensaturation15min =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_transfusionendtime, autotransfusionstoptime))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_transfusionendtime =
    s_tran_result, forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].d_transfusionendtime
     = cnvtdatetime(s_tran_result), i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_temperatureend, temperatureendautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_temperatureend = s_tran_result,
    i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_temperaturerouteend, temperaturerouteendautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_temperaturerouteend =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_pulserateend, pulserateendautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_pulserateend = s_tran_result,
    i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_respiratoryrateend, respiratoryrateendautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_respiratoryrateend =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_systolicbloodpressureend, sbpendautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_systolicbloodpressureend =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_diastolicbloodpressureend, dbpendautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_diastolicbloodpressureend =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd IN (mf_oxygensaturationend, oxygensatendautotransfusion))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_oxygensaturationend =
    s_tran_result, i_vital_cnt = (i_vital_cnt+ 1)
   ELSEIF (ce2.event_cd=volumeinfusedautotransfusion)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_volumeinfusedautotransfusion =
    s_tran_result, i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_albuminvol)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_albuminvol = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_cryoprecipitate)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_cryoprecipitate = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_factorviia)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].sfactorviia = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_factorviiivol)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_factorviiivol = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_factorixcomplex)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_factorixcomplex = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_factorixvol)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_factorixvol = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_ffp)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_ffp = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_granulocytes)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_granulocytes = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_ivig)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_ivig = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_platelets)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_platelets = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_rbcvol)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_rbcvol = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_rhimmuneglobulin)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_rhimmuneglobulin = s_tran_result,
    i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_bloodproductamountinfused)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_bloodproductamountinfused =
    s_tran_result, i_infused_cnt = (i_infused_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_transfusionreactiondescription)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_transfusionreactiondescription =
    s_tran_result, i_reaction_cnt = (i_reaction_cnt+ 1)
   ELSEIF (ce2.event_cd=mf_nursestatementofattestation)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_nursestatementofattestation =
    s_tran_result, i_witness_cnt = 1
   ELSEIF (ce2.event_cd=transfusioncompatibilityresult)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].transfusioncompatibilityresult =
    s_tran_result
   ELSEIF (ce2.event_cd=unitidnumberlotnumber)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].unitidnumberlotnumber =
    s_tran_result
   ELSEIF (ce2.event_cd=diastolicbloodpressuredelta1_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].diastolicbloodpressuredelta1_calc
     = s_tran_result
   ELSEIF (ce2.event_cd=diastolicbloodpressuredelta2_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].diastolicbloodpressuredelta2_calc
     = s_tran_result
   ELSEIF (ce2.event_cd=meanarterialpressure15min_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].meanarterialpressure15min_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=meanarterialpressuredelta1_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].meanarterialpressuredelta1_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=meanarterialpressuredelta2_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].meanarterialpressuredelta2_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=meanarterialpressureend_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].meanarterialpressureend_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=meanarterialpressurestart_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].meanarterialpressurestart_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=oxygensaturationdelta1_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].oxygensaturationdelta1_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=oxygensaturationdelta2_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].oxygensaturationdelta2_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (pulsepressure15min_calc, pulsepressure15minautotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].pulsepressure15min_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (pulsepressuredelta1_calc, pulsepressuredelta1autotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].pulsepressuredelta1_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (pulsepressuredelta2_calc, pulsepressuredelta2autotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].pulsepressuredelta2_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (pulsepressureend_calc, pulsepressureendautotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].pulsepressureend_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (pulsepressurestart_calc, pulsepressurestartautotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].pulsepressurestart_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (pulseratedelta1_calc, pulseratedelta1autotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].pulseratedelta1_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (pulsepressuredelta2_calc, pulsepressuredelta2autotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].pulseratedelta2_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (respiratoryratedelta1_calc, respiratoryratedelta1autotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].respiratoryratedelta1_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (respiratoryratedelta2_calc, respiratoryratedelta2autotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].respiratoryratedelta2_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=systolicbloodpressuredelta1_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].systolicbloodpressuredelta1_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=systolicbloodpressuredelta2_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].systolicbloodpressuredelta2_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (temperaturedelta1_calc, temperaturedelta1autotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].temperaturedelta1_calc =
    s_tran_result
   ELSEIF (ce2.event_cd IN (temperaturedelta2_calc, temperaturedelta2autotransfusion_calc))
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].temperaturedelta2_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=transfusionratecryoprecipitate_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].transfusionratecryoprecipitate_calc
     = s_tran_result
   ELSEIF (ce2.event_cd=transfusionrateffp_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].transfusionrateffp_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=transfusionrateivig_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].transfusionrateivig_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=transfusionrateplatelets_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].transfusionrateplatelets_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=transfusionraterbcs_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].transfusionraterbcs_calc =
    s_tran_result
   ELSEIF (ce2.event_cd=transfusiontime_calc)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].transfusiontime_calc =
    s_tran_result
   ENDIF
  FOOT  ce2.event_cd
   stat = 0
  FOOT  ce.parent_event_id
   IF (i_consent_cnt=1
    ...
    AND i_witness_cnt=1)
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_incompliance = "Yes",
    formscomplete = (formscomplete+ 1)
   ELSE
    forms->patientslist[d1.seq].forminstanceslist[i_instance_cnt].s_incompliance = "No",
    formsincomplete = (formsincomplete+ 1)
   ENDIF
   totalforms = (totalforms+ 1), old_d1seq = d1.seq
  FOOT  d1.seq
   stat = alterlist(forms->patientslist[old_d1seq].forminstanceslist,i_instance_cnt), i_instance_cnt
    = 0
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ELSEIF (ce.event_cd IN (rh, abo, bloodtype, rhtestonly1, rhtestonly2))
    cnt_abo = (cnt_abo+ 1)
    IF (cnt_abo=1)
     forms->patientslist[d1.seq].abo = concat(trim(uar_get_code_display(ce.event_cd)),": ",trim(ce
       .result_val)," ",trim(uar_get_code_display(ce.result_units_cd)),
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY ce.parent_event_id, ce.event_end_dt_tm
  HEAD REPORT
 ...
 SELECT INTO value(trim(var_output))
  ...
  ELSE substring(1,30,trim(forms->patientslist[d1.seq].s_nurseunitname))
  ENDIF
  ...
  bmi = forms->patientslist[d1.seq].forminstanceslist[d2.seq].bmi, abo_result = substring(1,30,trim(
    forms->patientslist[d1.seq].abo)), compatiblity_result = substring(1,30,trim(forms->patientslist[
    d1.seq].forminstanceslist[d2.seq].transfusioncompatibilityresult)),
  unit_id_lot_number = substring(1,30,trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].
    unitidnumberlotnumber)), smoke_cessation = substring(1,100,trim(forms->patientslist[d1.seq].
    forminstanceslist[d2.seq].smokingcessation)), transfusion_order = substring(1,150,trim(forms->
    patientslist[d1.seq].forminstanceslist[d2.seq].order_name_w_phys)),
  ...
  valid_start = trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].valid_transfusionstarttime
   ), transfusionstarttime = substring(1,25,trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq
    ].s_transfusionstarttime)),
  temperaturestart = substring(1,25,trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].
  ...
  oxygensaturationstart = substring(1,25,trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].
    s_oxygensaturationstart)), transfusionstartplus15min = substring(1,25,trim(forms->patientslist[d1
  ...
  transfusionendtime = substring(1,25,trim(forms->patientslist[d1.seq].forminstanceslist[d2.seq].
