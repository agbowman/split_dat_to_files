CREATE PROGRAM cp_delete_component_detail
 ...
 SET comp_detail_reply->status_data.status = "F"
 CALL verifyinputs(null)
 CALL deleterows(null)
 SET comp_detail_reply->status_data.status = "S"
 SUBROUTINE verifyinputs(null)
   ...
   DECLARE comp_detail_reltn_cd_codeset = i4 WITH protect, constant(4003134)
   DECLARE x = i4 WITH protect, noconstant(0)
   DECLARE complistsize = i4 WITH protect, noconstant(size(request->qual,5))
   IF (complistsize <= 0)
   ...
   FOR (x = 1 TO complistsize)
     ...
     SELECT INTO "NL:"
      ...
      WHERE (ccd.cp_component_detail_id=request->qual[x].cp_component_detail_id)
      WITH nocounter
     ;end select
     IF (curqual=0)
 ...
 SUBROUTINE deleterows(null)
   ...
   ENDIF
   SET stat = alterlist(comp_detail_reply->qual,size(request->qual,5))
   FOR (x = 1 TO size(request->qual,5))
     SELECT INTO "nl:"
      FROM cp_component_detail ccd
      WHERE (ccd.cp_component_detail_id=request->qual[x].cp_component_detail_id)
      DETAIL
       comp_detail_reply->qual[x].cp_component_detail_id = ccd.cp_component_detail_id
      WITH nocounter
     ;end select
     IF (curqual=0)
 ...
 SUBROUTINE (reporterror(operationname=vc,targetobjectname=vc,targetobjectvalue=vc) =null WITH
  protect)
   ...
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SET comp_detail_reply->status_data.status = "F"
   SET comp_detail_reply->status_data.subeventstatus.operationname = operationname
   SET comp_detail_reply->status_data.subeventstatus.operationstatus = "F"
   SET comp_detail_reply->status_data.subeventstatus.targetobjectname = targetobjectname
   SET comp_detail_reply->status_data.subeventstatus.targetobjectvalue = targetobjectvalue
 ...
 SUBROUTINE (verifynonzerocodevalue(codevalue=f8,codeset=i4,codesetname=vc) =null WITH protect)
   ...
   CALL echo(build("codeValue : ",codevalue))
   IF (codevalue > 0)
...
#exit_script
 IF (validate(debug_ind,0)=1)
  CALL echorecord(comp_detail_reply)
 ENDIF
