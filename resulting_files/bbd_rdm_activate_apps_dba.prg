CREATE PROGRAM bbd_rdm_activate_apps:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "BBD_RDM_ACTIVATE_APPS"
 CALL logscriptstart(sreadme_name)
 SELECT INTO "nl:"
  FROM dm_info d
  PLAN (d
   WHERE d.info_domain="PATHNET BBD"
    AND d.info_name="ENABLE BBD APPS"
    AND d.info_char="Y")
  WITH nocounter
 ;end select
 IF (curqual=0)
  CALL logstatus(build(sreadme_name,"--No Information to process."),"S")
  GO TO exit_script
 ENDIF
 CALL stamp_app(225048)
 CALL stamp_app(225049)
 CALL stamp_app(225051)
 CALL stamp_app(225052)
 CALL stamp_app(225055)
 CALL stamp_app(225056)
 CALL stamp_app(225057)
 CALL stamp_app(225058)
 CALL stamp_app(225059)
 CALL stamp_app(225064)
 CALL stamp_app(225066)
 CALL stamp_app(225068)
 CALL stamp_app(225069)
 CALL stamp_app(225070)
 CALL stamp_app(225074)
 CALL stamp_app(225078)
 CALL stamp_app(225088)
 ...
 CALL stamp_app(225062)
 CALL stamp_app(225063)
 CALL stamp_app(225075)
 CALL stamp_app(225077)
 SELECT INTO "nl:"
  ap.*
  FROM application ap
  WHERE ap.application_number IN (225048, 225049, 225051, 225052, 225055,
  225056, 225057, 225058, 225059, 225064,
  225066, 225068, 225069, 225070, 225074,
  225078, 225088)
   AND ap.active_ind=0
  WITH nocounter
 ;end select
 IF (curqual != 0)
  CALL logstatus(build(sreadme_name,"--Failed! Application numbers not activated."),"F")
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  ap.*
  FROM application ap
  WHERE ap.application_number IN (225039, 225040, 225043, 225046, 224054,
  225060, 225062, 225063, 225075, 225077)
   AND ap.active_ind=1
  WITH nocounter
 ;end select
 IF (curqual=0)
 ...
 ELSE
  CALL logstatus(build(sreadme_name,"--Failed! Application numbers not inactivated."),"F")
 ENDIF
#exit_script
