 ...
 SELECT INTO "NL:"
  ...
  PLAN (d1)
   ...
   WHERE (o.order_id=request->order_qual[d1.seq].order_id)
    AND o.order_status_cd=order_cd
   ...
   (SELECT DISTINCT
    ...
    WHERE cve.code_set=14281
     ...
     AND substring(5,1,cve.field_value)="Y"))) OR (cnvtlookbehind("30,S") > oa.action_dt_tm
    AND oa.dept_status_cd IN (
   (SELECT DISTINCT
    cve.code_value
    FROM code_value_extension cve
    WHERE cve.code_set=14281
     AND cve.field_name="REQ_PRINT_FLG"
     AND substring(6,1,cve.field_value)="Y")))) )
   JOIN (pl
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(request->printer_name)
   ...
   HEAD PAGE
    ...
    "DOB:", xcol = 250,
    CALL print(calcpos(xcol,xrow)),
    orders->dob, xrow += 14, xcol = 50,
    row + 1, col 1,
    CALL print(calcpos(xcol,xrow)),
    "Sex:", xcol = 100,
    CALL print(calcpos(xcol,xrow)),
    orders->sex, xrow += 20, xcol = 50,
    row + 1, col 1,
    CALL print(calcpos(xcol,xrow)),
    "Primary Diagnosis:", xcol = 145,
    CALL print(calcpos(xcol,xrow)),
    orders->admit_diagnosis, xrow += 20, xcol = 44,
    row + 1,
