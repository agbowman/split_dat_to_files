CREATE PROGRAM bed_aud_sch_eqpt
 SET last_mod = "120029"
 CALL echo("**** BED_AUD_SCH_EQPT.PRG LAST MOD: 120029 ****")
...
#bed_error_subroutines_exit
 ...
 SUBROUTINE getcodecdfmeaning(codevalue)
   ...
   RETURN(resultstring)
 END ;Subroutine
 DECLARE objectidenttype_description = f8 WITH constant(getcodevalueby(11000,
   codevalue_matchfield_cdfmeaning,"DESC")), protected
 DECLARE objectidenttype_itemnumber = f8 WITH constant(getcodevalueby(11000,
   codevalue_matchfield_cdfmeaning,"ITEM_NBR")), protected
 DECLARE objecttype_equipmentmaster = f8 WITH constant(getcodevalueby(11001,
   codevalue_matchfield_cdfmeaning,"ITEM_EQP")), protected
 DECLARE objecttype_itemmaster = f8 WITH constant(getcodevalueby(11001,
   codevalue_matchfield_cdfmeaning,"ITEM_MASTER")), protected
 DECLARE column_01_header_text = vc WITH constant(uar_i18ngetmessage(i18nhandle,
   "Column_01_Header_Text","Item Number")), protected
 DECLARE column_02_header_text = vc WITH constant(uar_i18ngetmessage(i18nhandle,
   "Column_02_Header_Text","Item Description")), protected
 DECLARE column_03_header_text = vc WITH constant(uar_i18ngetmessage(i18nhandle,
   "Column_03_Header_Text","Item Location")), protected
 DECLARE column_04_header_text = vc WITH constant(uar_i18ngetmessage(i18nhandle,
   "Column_04_Header_Text","Schedulable Quantity")), protected
 ...
 SET stat = addcolumn(column_04_header_text,column_datatype_number,column_hide_no)
 SELECT INTO "nl:"
  FROM sch_resource sr,
   item_control_info ici,
   object_identifier_index oi_a,
   object_identifier_index oi_b,
   code_value cv
  PLAN (sr
   WHERE sr.active_ind=1)
   JOIN (ici
   WHERE ici.location_cd=sr.item_location_cd
    AND ici.item_id=sr.item_id
    AND ici.sch_qty > 0)
   JOIN (oi_a
   WHERE oi_a.object_id=ici.item_id
    AND oi_a.identifier_type_cd=objectidenttype_description
    AND oi_a.object_type_cd IN (objecttype_equipmentmaster, objecttype_itemmaster)
    AND oi_a.active_ind=1
    AND oi_a.generic_object=0)
   JOIN (oi_b
   WHERE oi_b.object_id=outerjoin(oi_a.object_id)
    AND oi_b.identifier_type_cd=outerjoin(objectidenttype_itemnumber)
    AND oi_b.active_ind=outerjoin(1)
    AND oi_b.generic_object=outerjoin(0))
   JOIN (cv
   WHERE cv.code_value=outerjoin(ici.location_cd)
    AND cv.active_ind=outerjoin(1))
  ORDER BY oi_b.value DESC, cv.display
  HEAD REPORT
   rowindex = 0
  DETAIL
   rowindex = (rowindex+ 1)
   ...
   stat = setcellvalue(1,rowindex,column_datatype_string,null,0,
    0.0,oi_b.value,cell_display_regular), stat = setcellvalue(2,rowindex,column_datatype_string,null,
    0,
    0.0,oi_a.value,cell_display_regular), stat = setcellvalue(3,rowindex,column_datatype_string,null,
    0,
    0.0,cv.display,cell_display_regular),
   stat = setcellvalue(4,rowindex,column_datatype_number,null,ici.sch_qty,
