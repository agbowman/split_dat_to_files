CREATE PROGRAM acm_del_encntr_loc_hist:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_loc_hist_id=acm_request->encntr_loc_hist_qual[xref->del[d.seq].idx].
   encntr_loc_hist_id))
  WITH nocounter, status(reply->encntr_loc_hist_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->encntr_loc_hist_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,e.encntr_loc_hist_id,acm_request->encntr_loc_hist_qual[xref->del[t1].
     idx].encntr_loc_hist_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,e.encntr_loc_hist_id,acm_request->encntr_loc_hist_qual[xref->del[
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->encntr_loc_hist_qual[xref->del[index].idx].status=0))
