CREATE PROGRAM bhs_translate_ccl_dict:dba
 ...
 DECLARE mn_object_pos4 = i4 WITH protect, noconstant(0)
 ...
 SELECT INTO "nl:"
  FROM ccl_report_audit cra,
   prsnl pr
  PLAN (cra
   WHERE cra.object_type="CCLREPORT"
    AND expand(lndx,1,mn_object_cnt,cnvtupper(cra.object_name),ccl_objects->list[lndx].name_dba))
   JOIN (pr
   WHERE pr.active_ind=1
    AND cra.updt_id=pr.person_id)
  ORDER BY cra.object_name, cra.updt_dt_tm DESC
  HEAD cra.object_name
   mn_object_pos4 = locateval(lndx2,1,mn_object_cnt,cnvtupper(cra.object_name),ccl_objects->list[
    lndx2].name_dba)
   IF (mn_object_pos4 > 0)
    ccl_objects->list[mn_object_pos4].last_run_by = trim(pr.name_full_formatted,3)
   ENDIF
  WITH nocounter, expand = 1
 ;end select
 FOR (mn_for_cnt = 1 TO mn_object_cnt)
   SELECT INTO value("bhscust:bhs_trans_ccl_dict.dat")
    ...
    DETAIL
     ...
     row + 1, col 0, "<<LAST_RUN_BY:",
     col + 1, ccl_objects->list[mn_for_cnt].last_run_by, col + 1,
     ">>", row + 1
    WITH append
   ;end select
