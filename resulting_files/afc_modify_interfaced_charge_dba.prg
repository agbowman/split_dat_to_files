CREATE PROGRAM afc_modify_interfaced_charge:dba
 SET afc_modify_interfaced_charge = "CHARGSRV-15575.069"
 RECORD reply(
   1 charge_qual = i2
   1 dequeued_ind = i2
   1 charge[*]
 ...
 FREE RECORD chrgadjrequest
 RECORD chrgadjrequest(
   1 objarray[*]
     2 activity_id = f8
     2 trans_type_cd = f8
 ...
 SET count2 = 0
 SET dequeue_ind = 0
 SET dcorspactivityid = 0.0
 DECLARE credit_type_cd = f8
 DECLARE code_set = i4
 DECLARE cdf_meaning = c12
 DECLARE suspense_rsn_code = f8 WITH public, noconstant(0.0)
 DECLARE comment_rsn_code = f8 WITH public, noconstant(0.0)
 DECLARE adj_code = f8 WITH public, noconstant(0.0)
 DECLARE icd9cd = f8 WITH public, noconstant(0.0)
 ...
 DECLARE new_ci_id = f8 WITH noconstant(0.0)
 DECLARE new_cm_id = f8 WITH noconstant(0.0)
 DECLARE new_cem_id = f8 WITH noconstant(0.0)
 DECLARE new_n_id = f8 WITH noconstant(0.0)
 DECLARE 13019_mod_rsn_cd = f8
 DECLARE 4001989_modify_cd = f8
 DECLARE active_code = f8
 DECLARE physmodificationind = i2 WITH protect, noconstant(false)
 DECLARE billcodemodificationind = i2 WITH protect, noconstant(false)
 DECLARE researchacctpostchargessync = i2 WITH protect, constant(false)
 DECLARE chargeposition = i4 WITH protect, noconstant(0)
 ...
 ENDIF
 IF ( NOT (validate(cs29321_latechrgrev_cd)))
  DECLARE cs29321_latechrgrev_cd = f8 WITH protect, constant(getcodevalue(29321,"LATECHRGREV",0))
 ENDIF
 SET code_set = 13028
 SET cdf_meaning = "CR"
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,1,credit_type_cd)
 DECLARE chargeicd9 = f8
 SET code_set = 23549
 SET cdf_meaning = "CHARGEICD9"
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,1,chargeicd9)
 SET stat = uar_get_meaning_by_codeset(20549,"LT CHRG PROC",1,suspense_rsn_code)
 SET stat = uar_get_meaning_by_codeset(18937,"LT CHRG PROC",1,comment_rsn_code)
 SET stat = uar_get_meaning_by_codeset(18649,"ADJUST",1,adj_code)
 ...
 CALL echo(build("suspense_rsn_cd: ",suspense_rsn_code))
 CALL echo(build("comment_rsn_code: ",comment_rsn_code))
 CALL echo(build("adj_code: ",adj_code))
 DECLARE chargecpt4 = f8
 SET code_set = 23549
 SET cdf_meaning = "CHARGECPT4"
 SET cnt = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cnt,chargecpt4)
 DECLARE queue_event_cd = f8
 SET code_set = 29322
 SET cdf_meaning = "WRITOFCREDIT"
 SET cnt = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cnt,queue_event_cd)
 CALL echo(build("QUEUE_EVENT_CD: ",queue_event_cd))
 DECLARE entity_type_cd = f8
 SET code_set = 29320
 SET cdf_meaning = "CLAIM"
 SET cnt = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cnt,entity_type_cd)
 ...
 IF ( NOT (getlogicaldomain(ld_concept_organization,logicaldomainid)))
  CALL logmsg(curprog,"Failed to obtain logical domain",log_debug)
  SET reply->status_data.status = "F"
  GO TO end_program
 ENDIF
 SET lockchargerequest->charge_item_id = request->charge_item_id
 SET lockchargerequest->updt_cnt = validate(request->updt_cnt,0)
 SET lockchargerequest->updt_cnt_ind = validate(request->updt_cnt_ind,0)
 IF (validate(debug,- (1)) > 0)
  CALL echo("Executing AFC_LOCK_CHARGE_FOR_UPDATE")
 ENDIF
 ...
 IF ((lockchargereply->status_data.status != "S"))
  IF (validate(debug,- (1)) > 0)
   CALL echo(build("afc_lock_charge_for_update Reply Status is ",lockchargereply->status_data.status,
     " Ending Program"))
  ENDIF
  CALL logmsg(curprog,build("afc_lock_charge_for_update Reply Status is ",lockchargereply->
    status_data.status," Ending Program"),log_error)
  SET reply->status_data.status = "F"
  SET reply->status_data.subeventstatus[1].operationname = lockchargereply->status_data.
  subeventstatus[1].operationname
  SET reply->status_data.subeventstatus[1].operationstatus = lockchargereply->status_data.
  subeventstatus[1].operationstatus
  SET reply->status_data.subeventstatus[1].targetobjectname = lockchargereply->status_data.
  subeventstatus[1].targetobjectname
  SET reply->status_data.subeventstatus[1].targetobjectvalue = lockchargereply->status_data.
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   charge->parent_charge_item_id = request->charge_item_id, charge->charge_event_act_id = c
   .charge_event_act_id, charge->charge_event_id = c.charge_event_id,
   charge->bill_item_id = c.bill_item_id, charge->order_id = c.order_id, charge->encntr_id = c
   .encntr_id,
   charge->person_id = c.person_id, charge->payor_id = c.payor_id, charge->ord_loc_cd = c.ord_loc_cd,
   charge->perf_loc_cd =
   IF ((((request->perf_loc_cd > 0)) OR ((request->perf_loc_cd=- (1)))) )
    IF ((request->perf_loc_cd=- (1))) 0
    ELSE request->perf_loc_cd
    ENDIF
   ELSE c.perf_loc_cd
   ENDIF
   , charge->ord_phys_id =
   IF ((((request->ord_phys_id > 0)) OR ((request->ord_phys_id=- (1)))) )
    IF ((request->ord_phys_id=- (1))) 0
    ELSE request->ord_phys_id
    ENDIF
   ELSE c.ord_phys_id
   ENDIF
   , charge->perf_phys_id = c.perf_phys_id,
   charge->charge_description =
   IF (textlen(trim(request->charge_description,3)) > 0) request->charge_description
   ELSE c.charge_description
   ENDIF
   ,
   CALL echo(build("charge description: ",charge->charge_description)), charge->price_sched_id = c
   .price_sched_id,
   charge->item_quantity =
   IF ((request->item_quantity > 0)) request->item_quantity
   ELSE c.item_quantity
   ENDIF
   IF ((request->charge_type_cd=credit_type_cd))
    charge->item_price = (c.item_price * - (1)), charge->item_extended_price = (c.item_extended_price
     * - (1))
   ELSE
    charge->item_price =
    IF ((request->item_price > 0)) request->item_price
    ELSEIF ((request->item_price=- (1))) 0
    ELSE c.item_price
    ENDIF
    , charge->item_extended_price =
    IF ((request->item_extended_price > 0)) request->item_extended_price
    ELSEIF ((request->item_extended_price=- (1))) 0
    ELSE c.item_extended_price
    ENDIF
   ENDIF
   charge->item_allowable = c.item_allowable, charge->item_copay =
   IF ((((request->item_copay > 0)) OR ((request->item_copay=- (1)))) )
    IF ((request->item_copay=- (1))) 0
    ELSE request->item_copay
    ENDIF
   ELSE c.item_copay
   ENDIF
   IF ((request->charge_type_cd > 0.00))
    charge->charge_type_cd = request->charge_type_cd
   ELSE
    charge->charge_type_cd = c.charge_type_cd
   ENDIF
   charge->research_acct_id =
   IF ((((request->research_acct_id > 0)) OR ((request->research_acct_id=- (1)))) )
    IF ((request->research_acct_id=- (1))
     AND c.research_acct_id=0) 0
    ELSEIF ((request->research_acct_id=- (1))
     AND c.research_acct_id > 0) - (1)
    ELSEIF ((c.research_acct_id != request->research_acct_id)) c.research_acct_id
    ELSE request->research_acct_id
    ENDIF
   ELSE c.research_acct_id
   ENDIF
   , charge->suspense_rsn_cd =
   IF ((request->suspense_rsn_cd > 0)) request->suspense_rsn_cd
   ELSE c.suspense_rsn_cd
   ENDIF
   , charge->reason_comment =
   IF ((request->suspense_rsn_cd > 0)) request->reason_comment
   ELSE c.reason_comment
   ENDIF
   ,
   charge->posted_cd = c.posted_cd, charge->posted_dt_tm = cnvtdatetime(c.posted_dt_tm), charge->
   process_flg = request->process_flg,
   charge->service_dt_tm =
   IF ((request->service_dt_tm > 0)) cnvtdatetime(request->service_dt_tm)
   ELSE cnvtdatetime(c.service_dt_tm)
   ENDIF
   , charge->activity_dt_tm = cnvtdatetime(c.activity_dt_tm), charge->updt_cnt = 0,
   charge->updt_dt_tm = cnvtdatetime(sysdate), charge->updt_id = reqinfo->updt_id, charge->updt_task
    = reqinfo->updt_task,
   charge->updt_applctx = reqinfo->updt_applctx, charge->active_ind = c.active_ind, charge->
   active_status_cd = c.active_status_cd,
   charge->active_status_dt_tm = cnvtdatetime(c.active_status_dt_tm), charge->active_status_prsnl_id
    = c.active_status_prsnl_id, charge->beg_effective_dt_tm = cnvtdatetime(c.beg_effective_dt_tm),
   charge->end_effective_dt_tm = cnvtdatetime(c.end_effective_dt_tm), charge->credited_dt_tm =
   cnvtdatetime(sysdate), charge->adjusted_dt_tm = cnvtdatetime(c.adjusted_dt_tm),
   charge->interface_file_id = c.interface_file_id, charge->tier_group_cd = c.tier_group_cd, charge->
   def_bill_item_id = c.def_bill_item_id,
   charge->verify_phys_id =
   IF ((((request->verify_phys_id > 0)) OR ((request->verify_phys_id=- (1)))) )
    IF ((request->verify_phys_id=- (1))) 0
    ELSE request->verify_phys_id
    ENDIF
   ELSE c.verify_phys_id
   ENDIF
   , charge->gross_price = c.gross_price, charge->discount_amount = c.discount_amount,
   charge->item_price_adj_amt = c.item_price_adj_amt, charge->manual_ind = c.manual_ind, charge->
   combine_ind = c.combine_ind,
   charge->activity_type_cd = c.activity_type_cd, charge->activity_sub_type_cd = c
   .activity_sub_type_cd, charge->provider_specialty_cd = c.provider_specialty_cd,
   charge->activity_dt_tm = cnvtdatetime(c.activity_dt_tm), charge->admit_type_cd = c.admit_type_cd,
   charge->bundle_id = c.bundle_id,
   charge->department_cd = c.department_cd, charge->institution_cd = c.institution_cd, charge->
   level5_cd = c.level5_cd,
   charge->med_service_cd = c.med_service_cd, charge->section_cd = c.section_cd, charge->
   subsection_cd = c.subsection_cd,
   charge->abn_status_cd =
   IF ((((request->abn_status_cd > 0)) OR ((request->abn_status_cd=- (1)))) )
    IF ((request->abn_status_cd=- (1))) 0
    ELSE request->abn_status_cd
    ENDIF
   ELSE c.abn_status_cd
   ENDIF
   , charge->cost_center_cd = c.cost_center_cd, charge->inst_fin_nbr = c.inst_fin_nbr,
   charge->fin_class_cd = c.fin_class_cd, charge->health_plan_id = c.health_plan_id, charge->
   item_interval_id = c.item_interval_id,
   charge->item_list_price = c.item_list_price, charge->item_reimbursement = c.item_reimbursement,
   charge->list_price_sched_id = c.list_price_sched_id,
   charge->payor_type_cd = c.payor_type_cd, charge->epsdt_ind = c.epsdt_ind, charge->ref_phys_id = c
   .ref_phys_id,
   charge->start_dt_tm = cnvtdatetime(c.start_dt_tm), charge->stop_dt_tm = cnvtdatetime(c.stop_dt_tm),
   charge->server_process_flag = c.server_process_flag,
   charge->offset_charge_item_id =
   IF ((request->charge_type_cd=credit_type_cd)) request->charge_item_id
   ELSE 0
   ENDIF
   , charge->item_deductible_amt =
   IF ((((request->item_deductible_amt > 0)) OR ((request->item_deductible_amt=- (1)))) )
    IF ((request->item_deductible_amt=- (1))) 0
    ELSE request->item_deductible_amt
    ENDIF
   ELSE c.item_deductible_amt
   ENDIF
   , charge->patient_responsibility_flag =
   IF ((((request->patient_responsibility_flag > 0)) OR ((request->patient_responsibility_flag=- (1))
   )) )
    IF ((request->patient_responsibility_flag=- (1))) 0
    ELSE request->patient_responsibility_flag
    ENDIF
   ELSE c.patient_responsibility_flag
   ENDIF
   ,
   charge->original_org_id = c.original_org_id
  WITH nocounter
 ;end select
 IF (curqual < 1)
  SET reply->status_data.status = "F"
 ...
 IF ((((request->research_acct_id > 0)
  AND (request->research_acct_id != charge->research_acct_id)) OR ((request->research_acct_id=- (1))
  AND (charge->research_acct_id=- (1)))) )
  IF ( NOT (modifyresearchacct(request->research_acct_id,charge->charge_event_id,charge->
   charge_event_act_id,request->suspense_rsn_cd,request->reason_comment)))
  ...
  GO TO end_program
 ENDIF
 IF ((request->service_dt_tm > 0))
  SET charge->original_org_id = determineoriginalorgforcharge(charge->encntr_id,cnvtdatetime(request
    ->service_dt_tm))
 ENDIF
 IF ((request->late_charge_processing_ind=0))
  SET new_ci_id = 0.0
  SELECT INTO "nl:"
   aa = seq(charge_event_seq,nextval)"##################;rp0"
   FROM dual
   DETAIL
    new_ci_id = cnvtreal(aa)
   WITH format, counter
  ;end select
  SET charge->charge_item_id = new_ci_id
  CALL echo(build("NEW CHARGE_ITEM_ID IS::   ",new_ci_id))
  IF ((request->charge_type_cd=credit_type_cd))
   SET tmp_offset_charge_item_id = new_ci_id
  ELSE
   SET tmp_offset_charge_item_id = 0
  ENDIF
  INSERT  FROM charge c
   SET c.charge_item_id = new_ci_id, c.parent_charge_item_id = charge->parent_charge_item_id, c
    .charge_event_act_id = charge->charge_event_act_id,
    c.charge_event_id = charge->charge_event_id, c.bill_item_id = charge->bill_item_id, c.order_id =
    charge->order_id,
    c.encntr_id = charge->encntr_id, c.person_id = charge->person_id, c.payor_id = charge->payor_id,
    c.ord_loc_cd = charge->ord_loc_cd, c.perf_loc_cd = charge->perf_loc_cd, c.ord_phys_id = charge->
    ord_phys_id,
    c.perf_phys_id = charge->perf_phys_id, c.charge_description = charge->charge_description, c
    .price_sched_id = charge->price_sched_id,
    c.item_quantity = charge->item_quantity, c.charge_type_cd = charge->charge_type_cd, c.item_price
     = charge->item_price,
    c.item_extended_price = charge->item_extended_price, c.item_allowable = charge->item_allowable, c
    .item_copay = charge->item_copay,
    c.research_acct_id = charge->research_acct_id, c.suspense_rsn_cd = evaluate(4001989_modify_cd,0.0,
     charge->suspense_rsn_cd,0.0), c.reason_comment = evaluate(4001989_modify_cd,0.0,charge->
     reason_comment,""),
    c.posted_cd = charge->posted_cd, c.posted_dt_tm = cnvtdatetime(charge->posted_dt_tm), c
    .process_flg = charge->process_flg,
    c.service_dt_tm = cnvtdatetime(charge->service_dt_tm), c.activity_dt_tm = cnvtdatetime(charge->
     activity_dt_tm), c.updt_cnt = charge->updt_cnt,
    c.updt_dt_tm = cnvtdatetime(charge->updt_dt_tm), c.updt_id = charge->updt_id, c.updt_task =
    charge->updt_task,
    c.updt_applctx = charge->updt_applctx, c.active_ind = charge->active_ind, c.active_status_cd =
    charge->active_status_cd,
    c.active_status_dt_tm = cnvtdatetime(charge->active_status_dt_tm), c.active_status_prsnl_id =
    charge->active_status_prsnl_id, c.beg_effective_dt_tm = cnvtdatetime(charge->beg_effective_dt_tm),
    c.end_effective_dt_tm = cnvtdatetime(charge->end_effective_dt_tm), c.credited_dt_tm =
    cnvtdatetime(charge->credited_dt_tm), c.adjusted_dt_tm = cnvtdatetime(charge->adjusted_dt_tm),
    c.interface_file_id = charge->interface_file_id, c.tier_group_cd = charge->tier_group_cd, c
    .def_bill_item_id = charge->def_bill_item_id,
    c.verify_phys_id = charge->verify_phys_id, c.gross_price = charge->gross_price, c.discount_amount
     = charge->discount_amount,
    c.item_price_adj_amt = charge->item_price_adj_amt, c.manual_ind = charge->manual_ind, c
    .combine_ind = charge->combine_ind,
    c.activity_type_cd = charge->activity_type_cd, c.activity_sub_type_cd = charge->
    activity_sub_type_cd, c.provider_specialty_cd = charge->provider_specialty_cd,
    c.activity_dt_tm = cnvtdatetime(charge->activity_dt_tm), c.admit_type_cd = charge->admit_type_cd,
    c.bundle_id = charge->bundle_id,
    c.department_cd = charge->department_cd, c.institution_cd = charge->institution_cd, c.level5_cd
     = charge->level5_cd,
    c.med_service_cd = charge->med_service_cd, c.section_cd = charge->section_cd, c.subsection_cd =
    charge->subsection_cd,
    c.abn_status_cd = charge->abn_status_cd, c.cost_center_cd = charge->cost_center_cd, c
    .inst_fin_nbr = charge->inst_fin_nbr,
    c.fin_class_cd = charge->fin_class_cd, c.health_plan_id = charge->health_plan_id, c
    .item_interval_id = charge->item_interval_id,
    c.item_list_price = charge->item_list_price, c.item_reimbursement = charge->item_reimbursement, c
    .list_price_sched_id = charge->list_price_sched_id,
    c.payor_type_cd = charge->payor_type_cd, c.epsdt_ind = charge->epsdt_ind, c.ref_phys_id = charge
    ->epsdt_ind,
    c.start_dt_tm = cnvtdatetime(charge->start_dt_tm), c.stop_dt_tm = cnvtdatetime(charge->stop_dt_tm
     ), c.server_process_flag = evaluate(validate(request->server_process_flg,0),- (1),0,charge->
     server_process_flag),
    c.offset_charge_item_id = charge->offset_charge_item_id, c.item_deductible_amt = charge->
    item_deductible_amt, c.patient_responsibility_flag = charge->patient_responsibility_flag,
    c.posted_id = charge->updt_id, c.original_org_id = charge->original_org_id, c.original_encntr_id
     =
    IF (validate(charge->parent_charge_item_id,0)=0) charge->encntr_id
    ELSE
     (SELECT
      c.original_encntr_id
      FROM charge c
      WHERE (c.charge_item_id=charge->parent_charge_item_id))
    ENDIF
   WITH nocounter
  ;end insert
  IF (curqual < 1)
   SET reply->status_data.status = "F"
  ENDIF
  SET stat = alterlist(reply->charge,1)
  SET reply->charge[1].charge_item_id = new_ci_id
  IF ((request->charge_mod_qual <= 0)
   AND (request->modified_copy <= 0))
   SELECT INTO "nl:"
    FROM charge_mod cm
    WHERE (cm.charge_item_id=request->charge_item_id)
     AND cm.charge_mod_type_cd != cs13019_changelog
    DETAIL
     count1 += 1, stat = alterlist(charge->charge_mod,count1), charge->charge_mod[count1].
     charge_item_id = charge->charge_item_id,
     charge->charge_mod[count1].charge_mod_type_cd = cm.charge_mod_type_cd, charge->charge_mod[count1
     ].field1 = cm.field1, charge->charge_mod[count1].field2 = cm.field2,
     charge->charge_mod[count1].field3 = cm.field3, charge->charge_mod[count1].field4 = cm.field4,
     charge->charge_mod[count1].field5 = cm.field5,
     charge->charge_mod[count1].field6 = cm.field6, charge->charge_mod[count1].field7 = cm.field7,
     charge->charge_mod[count1].field8 = cm.field8,
     charge->charge_mod[count1].field9 = cm.field9, charge->charge_mod[count1].field10 = cm.field10,
     charge->charge_mod[count1].activity_dt_tm = cnvtdatetime(cm.activity_dt_tm),
     charge->charge_mod[count1].updt_cnt = 0, charge->charge_mod[count1].updt_dt_tm = cnvtdatetime(
      sysdate), charge->charge_mod[count1].updt_id = reqinfo->updt_id,
     charge->charge_mod[count1].updt_task = reqinfo->updt_task, charge->charge_mod[count1].
     updt_applctx = reqinfo->updt_applctx, charge->charge_mod[count1].active_ind = cm.active_ind,
     charge->charge_mod[count1].active_status_cd = cm.active_status_cd, charge->charge_mod[count1].
     active_status_dt_tm = cnvtdatetime(cm.active_status_dt_tm), charge->charge_mod[count1].
     active_status_prsnl_id = cm.active_status_prsnl_id,
     charge->charge_mod[count1].beg_effective_dt_tm = cm.beg_effective_dt_tm, charge->charge_mod[
     count1].end_effective_dt_tm = cm.end_effective_dt_tm, charge->charge_mod[count1].code1_cd = cm
     .code1_cd,
     charge->charge_mod[count1].nomen_id = cm.nomen_id, charge->charge_mod[count1].field1_id = cm
     .field1_id, charge->charge_mod[count1].field2_id = cm.field2_id,
     charge->charge_mod[count1].field3_id = cm.field3_id, charge->charge_mod[count1].field4_id = cm
     .field4_id, charge->charge_mod[count1].field5_id = cm.field5_id,
     charge->charge_mod[count1].cm1_nbr = cm.cm1_nbr, charge->charge_mod[count1].charge_mod_source_cd
      = cm.charge_mod_source_cd
    WITH nocounter
   ;end select
   SET charge->charge_mod_qual = count1
   IF ((((request->suspense_rsn_cd > 0)) OR (trim(request->reason_comment) != ""))
    AND 4001989_modify_cd > 0)
    SET count1 += 1
    SET charge->charge_mod_qual = count1
    SET stat = alterlist(charge->charge_mod,count1)
    SET charge->charge_mod[count1].charge_item_id = charge->charge_item_id
    SET charge->charge_mod[count1].charge_mod_type_cd = 13019_mod_rsn_cd
    SET charge->charge_mod[count1].field1_id = 4001989_modify_cd
    SET charge->charge_mod[count1].field6 = "The charge was modified"
    SET charge->charge_mod[count1].field7 = request->reason_comment
    SET charge->charge_mod[count1].field2_id = request->suspense_rsn_cd
    SET charge->charge_mod[count1].activity_dt_tm = cnvtdatetime(sysdate)
    SET charge->charge_mod[count1].updt_cnt = 0
    SET charge->charge_mod[count1].updt_dt_tm = cnvtdatetime(sysdate)
    SET charge->charge_mod[count1].updt_id = reqinfo->updt_id
    SET charge->charge_mod[count1].updt_task = reqinfo->updt_task
    SET charge->charge_mod[count1].updt_applctx = reqinfo->updt_applctx
    SET charge->charge_mod[count1].active_ind = 1
    SET charge->charge_mod[count1].active_status_cd = active_code
    SET charge->charge_mod[count1].active_status_dt_tm = cnvtdatetime(sysdate)
    SET charge->charge_mod[count1].active_status_prsnl_id = reqinfo->updt_id
    SET charge->charge_mod[count1].beg_effective_dt_tm = cnvtdatetime(sysdate)
    SET charge->charge_mod[count1].end_effective_dt_tm = cnvtdatetime("31-DEC-2100 23:59:59.99")
   ENDIF
  ELSE
   SET charge->charge_mod_qual = request->charge_mod_qual
   SET stat = alterlist(charge->charge_mod,request->charge_mod_qual)
   FOR (count1 = 1 TO request->charge_mod_qual)
     IF ((request->charge_mod[count1].charge_mod_type_cd != 13019_mod_rsn_cd))
      SET charge->charge_mod[count1].charge_item_id = charge->charge_item_id
      SET charge->charge_mod[count1].charge_mod_type_cd = request->charge_mod[count1].
      charge_mod_type_cd
      SET charge->charge_mod[count1].field6 = request->charge_mod[count1].field6
      SET charge->charge_mod[count1].field7 = request->charge_mod[count1].field7
      SET charge->charge_mod[count1].updt_cnt = 0
      SET charge->charge_mod[count1].updt_dt_tm = cnvtdatetime(sysdate)
      SET charge->charge_mod[count1].updt_id = reqinfo->updt_id
      SET charge->charge_mod[count1].updt_task = reqinfo->updt_task
      ...
      SET charge->charge_mod[count1].beg_effective_dt_tm = cnvtdatetime(sysdate)
      SET charge->charge_mod[count1].end_effective_dt_tm = cnvtdatetime("31-DEC-2100 23:59:59.99")
      SET charge->charge_mod[count1].code1_cd = request->charge_mod[count1].code1_cd
      SET charge->charge_mod[count1].nomen_id = request->charge_mod[count1].nomen_id
      SET charge->charge_mod[count1].field1_id = request->charge_mod[count1].field1_id
      SET charge->charge_mod[count1].field2_id = request->charge_mod[count1].field2_id
      SET charge->charge_mod[count1].field3_id = request->charge_mod[count1].field3_id
      SET charge->charge_mod[count1].field4_id = request->charge_mod[count1].field4_id
      SET charge->charge_mod[count1].field5_id = request->charge_mod[count1].field5_id
      SET charge->charge_mod[count1].cm1_nbr = request->charge_mod[count1].cm1_nbr
      IF (validate(request->charge_mod[count1].field1,char(128)) != char(128))
       SET charge->charge_mod[count1].field1 = request->charge_mod[count1].field1
      ENDIF
      IF (validate(request->charge_mod[count1].field2,char(128)) != char(128))
       SET charge->charge_mod[count1].field2 = request->charge_mod[count1].field2
      ENDIF
      IF (validate(request->charge_mod[count1].activity_dt_tm,0.0) > 0.0)
       SET charge->charge_mod[count1].activity_dt_tm = request->charge_mod[count1].activity_dt_tm
      ENDIF
      IF (validate(request->charge_mod[count1].charge_mod_source_cd,0.0) > 0.0)
       SET charge->charge_mod[count1].charge_mod_source_cd = validate(request->charge_mod[count1].
        charge_mod_source_cd,0.0)
      ELSE
       SELECT INTO "nl:"
        FROM charge_mod cm
        WHERE (cm.charge_item_id=request->charge_item_id)
         AND (cm.field6=request->charge_mod[count1].field6)
         AND (cm.field1_id=request->charge_mod[count1].field1_id)
         AND (cm.field2_id=request->charge_mod[count1].field2_id)
         AND active_ind=1
        DETAIL
         charge->charge_mod[count1].charge_mod_source_cd = cm.charge_mod_source_cd
        WITH nocounter
       ;end select
      ENDIF
     ENDIF
   ...
   IF ((((request->suspense_rsn_cd > 0)) OR (trim(request->reason_comment) != ""))
    AND 4001989_modify_cd > 0)
    SET count1 = size(charge->charge_mod,5)
    SELECT INTO "nl:"
     FROM charge_mod cm
     WHERE (cm.charge_item_id=request->charge_item_id)
      AND cm.charge_mod_type_cd=13019_mod_rsn_cd
     DETAIL
      count1 += 1, stat = alterlist(charge->charge_mod,count1), charge->charge_mod_qual = count1,
      charge->charge_mod[count1].charge_item_id = charge->charge_item_id, charge->charge_mod[count1].
      charge_mod_type_cd = 13019_mod_rsn_cd, charge->charge_mod[count1].field1_id = cm.field1_id,
      charge->charge_mod[count1].field6 = cm.field6, charge->charge_mod[count1].field7 = cm.field7,
      charge->charge_mod[count1].field2_id = cm.field2_id,
      charge->charge_mod[count1].activity_dt_tm = cm.activity_dt_tm, charge->charge_mod[count1].
      updt_cnt = cm.updt_cnt, charge->charge_mod[count1].updt_dt_tm = cm.updt_dt_tm,
      charge->charge_mod[count1].updt_id = cm.updt_id, charge->charge_mod[count1].updt_task = cm
      .updt_task, charge->charge_mod[count1].updt_applctx = cm.updt_applctx,
      charge->charge_mod[count1].active_ind = cm.active_ind, charge->charge_mod[count1].
      active_status_cd = cm.active_status_cd, charge->charge_mod[count1].active_status_dt_tm = cm
      .active_status_dt_tm,
      charge->charge_mod[count1].active_status_prsnl_id = cm.active_status_prsnl_id, charge->
      charge_mod[count1].beg_effective_dt_tm = cm.beg_effective_dt_tm, charge->charge_mod[count1].
      end_effective_dt_tm = cm.end_effective_dt_tm
     WITH nocounter
    ;end select
    SET count1 += 1
    SET charge->charge_mod_qual = count1
    ...
    SET charge->charge_mod[count1].active_status_prsnl_id = reqinfo->updt_id
    SET charge->charge_mod[count1].beg_effective_dt_tm = cnvtdatetime(sysdate)
    SET charge->charge_mod[count1].end_effective_dt_tm = cnvtdatetime("31-DEC-2100 23:59:59.00")
   ENDIF
  ENDIF
  DECLARE cmcnt = i4 WITH protect, noconstant(0)
  DECLARE x = i4 WITH protect, noconstant(1)
  FOR (x = 1 TO charge->charge_mod_qual)
    SET new_cm_id = 0.0
    SELECT INTO "nl:"
     bb = seq(charge_event_seq,nextval)"##################;rp0"
     FROM dual
     DETAIL
      new_cm_id = cnvtreal(bb)
     WITH format, counter
    ;end select
    SET charge->charge_mod[x].charge_mod_id = new_cm_id
    SET cmcnt += 1
    SET stat = alterlist(cmodreq->objarray,cmcnt)
    SET cmodreq->objarray[cmcnt].action_type = "ADD"
    SET cmodreq->objarray[cmcnt].charge_mod_id = new_cm_id
    SET cmodreq->objarray[cmcnt].charge_item_id = charge->charge_mod[x].charge_item_id
    SET cmodreq->objarray[cmcnt].charge_mod_type_cd = charge->charge_mod[x].charge_mod_type_cd
    SET cmodreq->objarray[cmcnt].field1 = charge->charge_mod[x].field1
    SET cmodreq->objarray[cmcnt].field2 = charge->charge_mod[x].field2
    SET cmodreq->objarray[cmcnt].field3 = charge->charge_mod[x].field3
    SET cmodreq->objarray[cmcnt].field4 = charge->charge_mod[x].field4
    SET cmodreq->objarray[cmcnt].field5 = charge->charge_mod[x].field5
    SET cmodreq->objarray[cmcnt].field6 = charge->charge_mod[x].field6
    SET cmodreq->objarray[cmcnt].field7 = charge->charge_mod[x].field7
    SET cmodreq->objarray[cmcnt].field8 = charge->charge_mod[x].field8
    SET cmodreq->objarray[cmcnt].field9 = charge->charge_mod[x].field9
    SET cmodreq->objarray[cmcnt].field10 = charge->charge_mod[x].field10
    SET cmodreq->objarray[cmcnt].activity_dt_tm = cnvtdatetime(charge->charge_mod[x].activity_dt_tm)
    SET cmodreq->objarray[cmcnt].updt_cnt = charge->charge_mod[x].updt_cnt
    SET cmodreq->objarray[cmcnt].active_ind = charge->charge_mod[x].active_ind
    SET cmodreq->objarray[cmcnt].active_status_cd = charge->charge_mod[x].active_status_cd
    SET cmodreq->objarray[cmcnt].active_status_dt_tm = cnvtdatetime(charge->charge_mod[x].
     active_status_dt_tm)
    SET cmodreq->objarray[cmcnt].active_status_prsnl_id = charge->charge_mod[x].
    active_status_prsnl_id
    SET cmodreq->objarray[cmcnt].beg_effective_dt_tm = cnvtdatetime(charge->charge_mod[x].
     beg_effective_dt_tm)
    SET cmodreq->objarray[cmcnt].end_effective_dt_tm = cnvtdatetime(charge->charge_mod[x].
     end_effective_dt_tm)
    SET cmodreq->objarray[cmcnt].code1_cd = charge->charge_mod[x].code1_cd
    SET cmodreq->objarray[cmcnt].nomen_id = charge->charge_mod[x].nomen_id
    SET cmodreq->objarray[cmcnt].field1_id = charge->charge_mod[x].field1_id
    SET cmodreq->objarray[cmcnt].field2_id = charge->charge_mod[x].field2_id
    SET cmodreq->objarray[cmcnt].field3_id = charge->charge_mod[x].field3_id
    SET cmodreq->objarray[cmcnt].field4_id = charge->charge_mod[x].field4_id
    SET cmodreq->objarray[cmcnt].field5_id = charge->charge_mod[x].field5_id
    SET cmodreq->objarray[cmcnt].cm1_nbr = charge->charge_mod[x].cm1_nbr
    SET cmodreq->objarray[cmcnt].charge_mod_source_cd = charge->charge_mod[x].charge_mod_source_cd
  ENDFOR
  IF (size(cmodreq->objarray,5) <= 0)
   CALL echo("No charge_mods to add")
  ELSE
   EXECUTE afc_val_charge_mod  WITH replace("REQUEST",cmodreq), replace("REPLY",cmrep)
   IF ((cmrep->status_data.status != "S"))
    CALL logmessage(curprog,"afc_val_charge_mod did not return success",log_debug)
    IF (validate(debug,- (1)) > 0)
     CALL echorecord(cmodreq)
     CALL echorecord(cmrep)
    ENDIF
    SET failed = insert_error
   ENDIF
  ENDIF
  DECLARE oldcptcount = i4 WITH protect, noconstant(0.0)
  DECLARE newcptcount = i4 WITH protect, noconstant(0.0)
  DECLARE matchcptcount = i4 WITH protect, noconstant(0.0)
  SET billcodemodificationind = true
  IF (checktierforcptmod(request->charge_item_id)=false)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (c
     WHERE (c.charge_item_id=request->charge_item_id)
      AND c.active_ind=true)
     JOIN (cv
     WHERE cv.code_value=c.field1_id
      AND cv.code_set=cvs14002_afc_schedule_type
      AND cv.cdf_meaning="MODIFIER")
    WITH nocounter
   ;end select
   SET oldcptcount = curqual
   SELECT INTO "nl:"
    FROM charge_mod c2,
     code_value cv
    PLAN (c2
     WHERE c2.charge_item_id=new_ci_id
      AND c2.active_ind=true)
     JOIN (cv
     WHERE cv.code_value=c2.field1_id
      AND cv.code_set=cvs14002_afc_schedule_type
      AND cv.cdf_meaning="MODIFIER")
    WITH nocounter
   ;end select
   SET newcptcount = curqual
   IF (oldcptcount=newcptcount)
    SELECT INTO "nl:"
     ...
     PLAN (c
      ...
      WHERE c2.charge_item_id=new_ci_id
       AND c2.active_ind=true
       AND c2.field2_id=c.field2_id
       AND c.field3_id=c2.field3_id)
      JOIN (cv
      WHERE cv.code_value=c.field1_id
       AND cv.code_set=cvs14002_afc_schedule_type
       AND cv.cdf_meaning="MODIFIER")
      JOIN (cv2
      WHERE cv2.code_value=c2.field1_id
       AND cv2.code_set=cvs14002_afc_schedule_type
       AND cv2.cdf_meaning="MODIFIER")
     WITH nocounter
    ;end select
    SET matchcptcount = curqual
  ...
  ENDIF
  IF ( NOT (loadmodifiersdetail(0)))
   CALL logmessage(curprog,"No Modifiers details present",log_debug)
  ENDIF
  IF ((request->charge_mod_qual > 0))
   DECLARE billcdcnt = i4 WITH protect, noconstant(0)
   ...
   FOR (i = 1 TO request->charge_mod_qual)
     SET codeset14002meaning = uar_get_code_meaning(request->charge_mod[i].field1_id)
     SET codeset13019meaning = uar_get_code_meaning(request->charge_mod[i].charge_mod_type_cd)
     CALL echo(build("CodeSet13019Meaning: ",codeset13019meaning))
     IF (((codeset14002meaning="ICD9") OR (((codeset14002meaning="MODIFIER"
      AND validate(request->charge_mod[i].charge_mod_source_cd,0.0) != cs4518006_manually_add) OR (((
     codeset13019meaning="PROMPT") OR (((codeset13019meaning="USER DEF") OR (((codeset13019meaning=
     "FLEX") OR (codeset14002meaning="NDC")) )) )) )) )) )
      IF (((validate(request->charge_mod[i].action_type,char(128))="ADD") OR (validate(request->
       charge_mod[i].action_type,char(128))="UPT"
       AND validate(request->charge_mod[i].charge_event_mod_id,0.0)=0.0)) )
       SELECT INTO "nl:"
        ...
        WITH format, counter
       ;end select
       IF (validate(request->charge_mod[i].charge_event_mod_id))
        SET request->charge_mod[i].charge_event_mod_id = new_cem_id
       ENDIF
       SET billcdcnt += 1
       ...
       SET addchargeeventmodreq->objarray[billcdcnt].charge_event_id = charge->charge_event_id
       SET addchargeeventmodreq->objarray[billcdcnt].charge_event_mod_type_cd = request->charge_mod[i
       ].charge_mod_type_cd
       SET addchargeeventmodreq->objarray[billcdcnt].field6 = request->charge_mod[i].field6
       SET addchargeeventmodreq->objarray[billcdcnt].field7 = request->charge_mod[i].field7
       SET addchargeeventmodreq->objarray[billcdcnt].nomen_id = request->charge_mod[i].nomen_id
       SET addchargeeventmodreq->objarray[billcdcnt].field1_id = request->charge_mod[i].field1_id
       SET addchargeeventmodreq->objarray[billcdcnt].field2_id = request->charge_mod[i].field2_id
       SET addchargeeventmodreq->objarray[billcdcnt].field3_id = request->charge_mod[i].field3_id
       SET addchargeeventmodreq->objarray[billcdcnt].field4_id = request->charge_mod[i].field4_id
       SET addchargeeventmodreq->objarray[billcdcnt].field5_id = request->charge_mod[i].field5_id
       ...
       SET addchargeeventmodreq->objarray[billcdcnt].field1 = request->charge_mod[i].field1
       SET addchargeeventmodreq->objarray[billcdcnt].field2 = request->charge_mod[i].field2
       SET addchargeeventmodreq->objarray[billcdcnt].activity_dt_tm = cnvtdatetime(request->
        charge_mod[i].activity_dt_tm)
       SET addchargeeventmodreq->objarray[billcdcnt].cm1_nbr = request->charge_mod[i].cm1_nbr
       SET addchargeeventmodreq->objarray[billcdcnt].code1_cd = request->charge_mod[i].code1_cd
      ELSEIF (validate(request->charge_mod[i].action_type,char(128))="UPT")
       SET billcdcnt2 += 1
       SET stat = alterlist(uptchargeeventmodreq->objarray,billcdcnt2)
       SELECT INTO "nl:"
        FROM charge_event_mod cem
        WHERE cem.charge_event_mod_id=validate(request->charge_mod[i].charge_event_mod_id,- (999))
        DETAIL
         uptchargeeventmodreq->objarray[billcdcnt2].action_type = "UPT", uptchargeeventmodreq->
         objarray[billcdcnt2].charge_event_mod_id = request->charge_mod[i].charge_event_mod_id,
         uptchargeeventmodreq->objarray[billcdcnt2].charge_event_id = cem.charge_event_id,
         uptchargeeventmodreq->objarray[billcdcnt2].charge_event_mod_type_cd = request->charge_mod[i]
         .charge_mod_type_cd, uptchargeeventmodreq->objarray[billcdcnt2].field3 = cem.field3,
         uptchargeeventmodreq->objarray[billcdcnt2].field4 = cem.field4,
         uptchargeeventmodreq->objarray[billcdcnt2].field5 = cem.field5, uptchargeeventmodreq->
         objarray[billcdcnt2].field6 = request->charge_mod[i].field6, uptchargeeventmodreq->objarray[
         billcdcnt2].field7 = request->charge_mod[i].field7,
         uptchargeeventmodreq->objarray[billcdcnt2].field8 = cem.field8, uptchargeeventmodreq->
         objarray[billcdcnt2].field9 = cem.field9, uptchargeeventmodreq->objarray[billcdcnt2].field10
          = cem.field10,
         uptchargeeventmodreq->objarray[billcdcnt2].nomen_id = request->charge_mod[i].nomen_id,
         uptchargeeventmodreq->objarray[billcdcnt2].field1_id = request->charge_mod[i].field1_id,
         uptchargeeventmodreq->objarray[billcdcnt2].field2_id = request->charge_mod[i].field2_id,
         uptchargeeventmodreq->objarray[billcdcnt2].field3_id = request->charge_mod[i].field3_id,
         uptchargeeventmodreq->objarray[billcdcnt2].field4_id = request->charge_mod[i].field4_id,
         uptchargeeventmodreq->objarray[billcdcnt2].field5_id = request->charge_mod[i].field5_id,
         ...
         IF (validate(request->charge_mod[i].field1,char(128)) != char(128))
          uptchargeeventmodreq->objarray[billcdcnt2].field1 = request->charge_mod[i].field1
         ELSE
          uptchargeeventmodreq->objarray[billcdcnt2].field1 = cem.field1
         ENDIF
         IF (validate(request->charge_mod[i].field2,char(128)) != char(128))
          uptchargeeventmodreq->objarray[billcdcnt2].field2 = request->charge_mod[i].field2
         ELSE
          uptchargeeventmodreq->objarray[billcdcnt2].field2 = cem.field2
         ENDIF
         IF (validate(request->charge_mod[i].activity_dt_tm,0.0) > 0.0)
          uptchargeeventmodreq->objarray[billcdcnt2].activity_dt_tm = cnvtdatetime(request->
           charge_mod[i].activity_dt_tm)
         ELSE
          uptchargeeventmodreq->objarray[billcdcnt2].activity_dt_tm = cem.activity_dt_tm
         ENDIF
         IF ((validate(request->charge_mod[i].cm1_nbr,- (0.00001)) != - (0.00001)))
          uptchargeeventmodreq->objarray[billcdcnt2].cm1_nbr = request->charge_mod[i].cm1_nbr
         ELSE
          uptchargeeventmodreq->objarray[billcdcnt2].cm1_nbr = cem.cm1_nbr
         ENDIF
         IF ((validate(request->charge_mod[i].code1_cd,- (0.00001)) != - (0.00001)))
          uptchargeeventmodreq->objarray[billcdcnt2].code1_cd = request->charge_mod[i].code1_cd
         ELSE
     ...
     ELSEIF (codeset14002meaning="MODIFIER"
      AND validate(request->charge_mod[i].charge_mod_source_cd,0.0)=cs4518006_manually_add
      AND ((validate(request->charge_mod[i].action_type,char(128))="UPT") OR (validate(request->
      charge_mod[i].action_type,char(128))="ADD")) )
      SET modifierpos = locateval(idx,1,size(chargemodifiersdetail->modifierslist,5),request->
       charge_mod[i].field6,chargemodifiersdetail->modifierslist[idx].field6)
      IF (modifierpos > 0)
       SELECT INTO "nl:"
        FROM charge c,
        ...
        PLAN (c
         ...
         WHERE cem.charge_event_id=c.charge_event_id
          AND (cem.field6=request->charge_mod[i].field6)
       ...
       ;end select
       IF (curqual > 0)
        IF (uptchargeeventmodactiveind(cemid,request->charge_mod[i].field6))
  ...
  SELECT INTO "nl:"
   ...
   WHERE (c.charge_item_id=request->charge_item_id)
   DETAIL
    temp_charge_act_id = c.charge_event_act_id, tempprovspeccd = c.provider_specialty_cd,
    tempperfphysid = c.perf_phys_id
   WITH nocounter
  ;end select
  CALL getproviderspecialty(tempperfphysid)
  IF ((request->ord_phys_id != 0))
   IF ((request->ord_phys_id=- (1)))
    SELECT INTO "nl:"
     ...
     ELSE
      IF (tempprovspeccd != 0)
       UPDATE  FROM charge c
        SET c.provider_specialty_cd = 0.0
        WHERE (c.charge_item_id=reply->charge[1].charge_item_id)
       ;end update
      ENDIF
     ENDIF
     IF (checktierforordphys(request->charge_item_id))
      SET physmodificationind = true
     ELSEIF (checktierforproviderspec(request->charge_item_id))
   ...
   ELSE
    ...
    IF (temp_phys_id=0)
     INSERT  FROM charge_event_act_prsnl ceap
      SET ceap.prsnl_id = request->ord_phys_id, ceap.prsnl_type_cd = ord_phys_code, ceap
     ...
     ELSE
      IF (newprovspeccd != 0)
       UPDATE  FROM charge c
        SET c.provider_specialty_cd = newprovspeccd
        WHERE (c.charge_item_id=reply->charge[1].charge_item_id)
       ;end update
      ENDIF
     ENDIF
    ELSEIF ((temp_phys_id != request->ord_phys_id))
     UPDATE  FROM charge_event_act_prsnl ceap
      SET ceap.prsnl_id = request->ord_phys_id, ceap.updt_cnt = (ceap.updt_cnt+ 1), ceap.updt_dt_tm
   ...
   ENDIF
  ENDIF
  IF ((request->verify_phys_id != 0))
   IF ((request->verify_phys_id=- (1)))
    SELECT INTO "nl:"
     FROM charge_event_act_prsnl ceap
     WHERE ceap.charge_event_act_id=temp_charge_act_id
    ...
    IF (curqual > 0)
     ...
     ELSE
      ...
      ENDIF
     ENDIF
     IF (checktierforrendphys(request->charge_item_id))
      SET physmodificationind = true
     ELSEIF (checktierforproviderspec(request->charge_item_id))
      SET provspecind = true
     ENDIF
   ...
   ELSE
    ...
    IF (temp_phys_id=0)
     INSERT  FROM charge_event_act_prsnl ceap
      SET ceap.prsnl_id = request->verify_phys_id, ceap.prsnl_type_cd = ver_phys_code, ceap
       .charge_event_act_id = temp_charge_act_id,
       ceap.updt_cnt = 0, ceap.updt_dt_tm = cnvtdatetime(sysdate), ceap.updt_id = reqinfo->updt_id,
       ceap.updt_task = reqinfo->updt_task, ceap.updt_applctx = reqinfo->updt_applctx
      WITH nocounter
     ...
     ELSE
      ...
      ENDIF
     ENDIF
    ELSEIF ((temp_phys_id != request->verify_phys_id))
     UPDATE  FROM charge_event_act_prsnl ceap
      SET ceap.prsnl_id = request->verify_phys_id, ceap.updt_cnt = (ceap.updt_cnt+ 1), ceap
   ...
   ENDIF
  ENDIF
  IF ((reply->charge[1].charge_item_id > 0.0)
   AND ((physmodificationind) OR (((provspecind) OR (billcodemodificationind)) ))
   AND (charge->charge_type_cd != credit_type_cd))
   UPDATE  FROM charge c
    SET c.server_process_flag = 0, c.process_flg = 1
    WHERE (c.charge_item_id=reply->charge[1].charge_item_id)
   ;end update
   CALL getduplicateaddonforcredit(0)
   CALL performchargereprocess(reply->charge[1].charge_item_id)
   CALL getchargestobeposted(0)
  ENDIF
  IF ((request->abn_status_cd != 0))
   SELECT INTO "nl:"
    FROM charge_event ce,
     charge c
    PLAN (c
   ...
   IF ((request->abn_status_cd=- (1)))
    ...
    ENDIF
   ELSE
    IF ((temp_abn_status_cd != request->abn_status_cd))
     UPDATE  FROM charge_event ce
      SET ce.abn_status_cd = request->abn_status_cd
      WHERE ce.charge_event_id=temp_charge_ev_id
      WITH nocounter
     ;end update
    ENDIF
  ...
  UPDATE  FROM charge_event_act cea
   SET cea.item_copay = charge->item_copay, cea.item_deductible_amt = charge->item_deductible_amt,
    cea.patient_responsibility_flag = charge->patient_responsibility_flag
   WHERE (cea.charge_event_act_id=charge->charge_event_act_id)
  ;end update
  DECLARE codeset14002meaning = c12
  DECLARE code_value = f8
  FOR (x = 1 TO charge->charge_mod_qual)
    SET code_value = charge->charge_mod[x].field1_id
    ...
    "CPT4")) )) )
     ...
     SELECT INTO "nl:"
      FROM charge c
      WHERE (c.charge_item_id=charge->charge_mod[x].charge_item_id)
      DETAIL
       person_id = c.person_id, encntr_id = c.encntr_id
      WITH nocounter
     ;end select
     SET new_n_id = 0.0
     SELECT INTO "nl:"
     ...
     INSERT  FROM nomen_entity_reltn n
      SET n.nomen_entity_reltn_id = new_n_id, n.nomenclature_id = charge->charge_mod[x].nomen_id, n
       .parent_entity_id = charge->charge_mod[x].charge_item_id,
       n.parent_entity_name = "CHARGE", n.child_entity_id = charge->charge_mod[x].nomen_id, n
       .child_entity_name = "NOMENCLATURE",
       n.reltn_type_cd =
       IF (codeset14002meaning="CPT4") chargecpt4
       ELSE chargeicd9
       ENDIF
       , n.beg_effective_dt_tm = cnvtdatetime(curdate,curtime), n.end_effective_dt_tm = cnvtdatetime(
        "31-DEC-2100 23:59:59.99"),
       n.updt_dt_tm = cnvtdatetime(curdate,curtime), n.updt_id = reqinfo->updt_id, n.updt_task =
       reqinfo->updt_task,
       n.updt_cnt = 0, n.updt_applctx = reqinfo->updt_applctx, n.person_id = person_id,
       n.encntr_id = encntr_id, n.active_ind = 1
      WITH nocounter
     ;end insert
    ENDIF
  ENDFOR
  IF ((charge->charge_type_cd != credit_type_cd))
   UPDATE  FROM charge c
  ...
  IF ((request->charge_type_cd=credit_type_cd))
   UPDATE  FROM charge c
    SET c.offset_charge_item_id = tmp_offset_charge_item_id
    WHERE (c.charge_item_id=request->charge_item_id)
    WITH nocounter
   ;end update
  ENDIF
  IF (validate(request->donocommitind,- (1)) != 1)
   SET reqinfo->commit_ind = 1
  ENDIF
 ELSE
  SET dactivityid = 0.0
  DECLARE dbohpreltnid = f8 WITH protect, noconstant(0.0)
  DECLARE benefitorderid = f8 WITH protect, noconstant(0.0)
  DECLARE totaladjustment = f8 WITH protect, noconstant(0.0)
  DECLARE pftchargeid = f8 WITH protect, noconstant(0.0)
  DECLARE pfttransaliasid = f8 WITH protect, noconstant(0.0)
  SELECT INTO "nl:"
   FROM pft_charge pc,
    pft_charge_bo_reltn pcbr,
    bo_hp_reltn bohp,
    benefit_order bo
   PLAN (pc
    WHERE (pc.charge_item_id=request->charge_item_id))
    JOIN (pcbr
    WHERE pcbr.pft_charge_id=pc.pft_charge_id)
    JOIN (bohp
    WHERE bohp.benefit_order_id=pcbr.benefit_order_id
     AND bohp.priority_seq=1
     AND bohp.active_ind=1
     AND  NOT (bohp.bo_hp_status_cd IN (cs24451_invalid_cd, cs24451_cancelled_cd)))
    JOIN (bo
    WHERE bo.benefit_order_id=bohp.benefit_order_id
     AND bo.active_ind=1
     AND  NOT (bo.bo_status_cd IN (cs24451_invalid_cd, cs24451_cancelled_cd)))
   DETAIL
    dactivityid = pc.activity_id, dbohpreltnid = bohp.bo_hp_reltn_id, benefitorderid = bohp
    .benefit_order_id,
  ...
  SELECT INTO "nl:"
   FROM pft_trans_reltn ptr
   WHERE ptr.parent_entity_id=pftchargeid
    AND ptr.parent_entity_name="PFTCHARGE"
    AND ptr.trans_type_cd=cs18649_adjustment_cd
    AND ptr.active_ind=true
   DETAIL
    totaladjustment += evaluate(ptr.dr_cr_flag,2,(ptr.amount * - (1)),ptr.amount)
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   FROM pft_trans_alias pta
   WHERE pta.trans_type_cd=cs18649_adjustment_cd
    AND pta.trans_sub_type_cd=suspense_rsn_code
    AND pta.trans_reason_cd=comment_rsn_code
    AND pta.active_ind=true
    AND pta.logical_domain_id=logicaldomainid
   ORDER BY pta.dr_cr_flag
   HEAD pta.dr_cr_flag
    pfttransaliasid = pta.pft_trans_alias_id
   WITH nocounter
  ;end select
  SET mnstat = alterlist(chrgadjrequest->objarray,1)
  SET chrgadjrequest->objarray[1].trans_type_cd = adj_code
  SET chrgadjrequest->objarray[1].trans_sub_type_cd = suspense_rsn_code
  SET chrgadjrequest->objarray[1].trans_alias_id = pfttransaliasid
  SET chrgadjrequest->objarray[1].amount = (round((charge->item_extended_price+ totaladjustment),2)
   * - (1))
  SET chrgadjrequest->objarray[1].trans_reason_cd = comment_rsn_code
  SET chrgadjrequest->objarray[1].suppress_flag = 3
  SET chrgadjrequest->objarray[1].activity_id = dactivityid
  SET chrgadjrequest->objarray[1].bo_hp_reltn_id = dbohpreltnid
  EXECUTE pft_apply_doll_adj_for_charge  WITH replace("REQUEST",chrgadjrequest), replace("REPLY",
   chrgadjreply)
  IF ((chrgadjreply->status_data.status != "S"))
   SET reply->status_data.status = "F"
   GO TO end_program
  ELSE
   SET reply->status_data.status = "S"
  ...
  UPDATE  FROM pft_charge pc
   SET pc.late_chrg_flag = 4
   WHERE (pc.charge_item_id=request->charge_item_id)
    AND pc.late_chrg_flag != 0
   WITH nocounter
  ;end update
  IF ((request->late_charge_processing_ind=1))
   CALL processlatechargebalance(benefitorderid)
   SELECT INTO "nl:"
    FROM charge c,
     pft_charge pc,
     pft_charge_bo_reltn pcbr,
     bo_hp_reltn bhr
    PLAN (c
     WHERE (c.encntr_id=charge->encntr_id)
      AND (c.charge_item_id != request->charge_item_id)
      AND c.active_ind=1)
     JOIN (pc
     WHERE pc.charge_item_id=c.charge_item_id
      AND pc.late_chrg_flag IN (1, 5))
     JOIN (pcbr
     WHERE pcbr.pft_charge_id=pc.pft_charge_id
     ...
     WHERE bhr.benefit_order_id=pcbr.benefit_order_id
      ...
      AND bhr.bo_hp_reltn_id=dbohpreltnid
    ...
    SELECT INTO "nl:"
     ...
     PLAN (pc
      ...
      WHERE bhr.benefit_order_id=pcbr.benefit_order_id
       ...
       AND bhr.active_ind=1)
      JOIN (br
      WHERE br.parent_entity_name="BO_HP_RELTN"
       AND br.parent_entity_id=bhr.bo_hp_reltn_id)
      JOIN (pqi
      WHERE pqi.corsp_activity_id=br.corsp_activity_id
       AND pqi.pft_entity_status_cd=cs29321_latechrgrev_cd
       AND pqi.active_ind=1)
     DETAIL
 ...
 ENDIF
 IF ((charge->charge_type_cd != credit_type_cd)
  AND size(reply->charge,5) > 0)
  CALL savechangelogdetails(request->charge_item_id,reply->charge[1].charge_item_id)
 ENDIF
 IF (size(reply->charge,5) > 0)
  SET reply->status_data.status = "S"
  SET chargeposition = locateval(chargeindex,1,size(ocrep->charge_items),reply->charge[1].
   charge_item_id,ocrep->charge_items[chargeindex].charge_item_id)
  IF (chargeposition > 0)
   SET stat = alterlist(publishupdatedchargerequest->charges,1)
   SET publishupdatedchargerequest->charges[1].charge_item_id = reply->charge[1].charge_item_id
   IF ( NOT (publisheventforchargeupdate(publishupdatedchargerequest,ocrep->charge_items[
    chargeposition].process_flg)))
 ...
 SUBROUTINE (getduplicateaddonforcredit(dummy=i2) =null)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     charge_event_id = c.charge_event_id, tier_group_cd = c.tier_group_cd, bill_item_ext_owner_cd = b
     .ext_owner_cd
    WITH nocounter
   ;end select
    ...
    SELECT INTO "nl:"
     ...
     PLAN (c
      ...
      WHERE c2.bill_item_id=b.bill_item_id
       AND (c2.charge_item_id=reply->charge[1].charge_item_id))
     WITH nocounter
    ;end select
    IF (curqual > 0)
     IF (validate(debug,- (1)) > 0)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     crchrgcount += 1, stat = alterlist(addcreditreq->charge,crchrgcount), addcreditreq->charge_qual
      = crchrgcount,
     addcreditreq->charge[crchrgcount].charge_item_id = c.charge_item_id, addcreditreq->charge[
     crchrgcount].late_charge_processing_ind = 0.0, addcreditreq->charge[crchrgcount].reason_comment
      = request->reason_comment,
     addcreditreq->charge[crchrgcount].suspense_rsn_cd = request->suspense_rsn_cd
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    FROM charge c,
   ...
   IF (crchrgcount > 0)
    ...
    ENDIF
    EXECUTE afc_add_credit  WITH replace("REQUEST",addcreditreq), replace("REPLY",addcredit_reply)
    IF ((addcredit_reply->status_data.status="F"))
     CALL echo(build("Add Credit Reply Status is F Ending Program"))
     CALL logmsg(curprog,build("Add Credit Reply Status is F Ending Program"),log_debug)
     SET reply->status_data.status = "F"
    ENDIF
   ENDIF
 END ;Subroutine
 SUBROUTINE (getchargestobeposted(dummy=i2) =null)
   DECLARE charge_event_id = f8 WITH protect, noconstant(0.0)
   DECLARE tier_group_cd = f8 WITH protect, noconstant(0.0)
   DECLARE chrgcount = i4 WITH protect, noconstant(size(reply->charge,5))
   SELECT INTO "nl:"
    FROM charge c
    WHERE (c.charge_item_id=reply->charge[1].charge_item_id)
    DETAIL
     charge_event_id = c.charge_event_id, tier_group_cd = c.tier_group_cd
    WITH nocounter
   ;end select
   SELECT INTO "nl:"
    FROM charge c
    WHERE (c.charge_item_id != reply->charge[1].charge_item_id)
     ...
     AND c.active_ind=1
    DETAIL
     chrgcount += 1, stat = alterlist(reply->charge,chrgcount), reply->charge[chrgcount].
     charge_item_id = c.charge_item_id
    WITH nocounter
   ;end select
 END ;Subroutine
 SUBROUTINE performchargereprocess(chargeitemid)
   DECLARE hreq = i4
   ...
   DECLARE hprocess = i4
   DECLARE hcharge = i4
   DECLARE hchargeeventmod = i4
   DECLARE hchargemod = i4
   ...
   SELECT INTO "nl:"
    FROM charge_event_mod cem,
     code_value cv
    PLAN (cem
     WHERE (cem.charge_event_id=charge->charge_event_id)
      AND cem.active_ind=1
      AND  NOT (expand(idx,1,size(request->charge_mod,5),cem.charge_event_mod_id,validate(request->
       charge_mod[idx].charge_event_mod_id,0.0)))
      AND  NOT (expand(idx1,1,size(request->charge_mod,5),cem.field6,request->charge_mod[idx1].field6,
      "MODIFIER",uar_get_code_meaning(request->charge_mod[idx1].field1_id))))
     JOIN (cv
   ...
   SELECT INTO "nl:"
    ...
    PLAN (cm
     WHERE (cm.charge_item_id=request->charge_item_id)
      ...
      AND  NOT (expand(idx,1,size(request->charge_mod,5),cm.field6,request->charge_mod[idx].field6,
      0.0,validate(request->charge_mod[idx].charge_event_mod_id,0.0),0.0,validate(request->
       charge_mod[idx].charge_mod_source_cd,0.0))))
     JOIN (cv
   ...
   IF (iret=0)
    ...
    IF (iret=0)
     ...
     IF (iret=0)
      ...
      SET srvstat = uar_srvsetdouble(hprocess,"charge_event_id",charge->charge_event_id)
 ...
 SUBROUTINE (loaddiagnosisdetail(dummyvar=i2) =i2)
   ...
   RETURN(true)
 END ;Subroutine
 SUBROUTINE (loadmodifiersdetail(dummyvar=i2) =i2)
   CALL logmessage(curprog,"Entering loadModifiersDetail...",log_debug)
   ...
   SELECT INTO "nl:"
    ...
    DETAIL
     ...
     chargemodifiersdetail->modifierslist[modreccnt].field2_id = cm.field2_id
    WITH nocounter
   ;end select
   IF (validate(debug,- (1)) > 0)
   ...
   ENDIF
   IF (modreccnt=0)
    CALL logmessage(curprog,"Exiting loadModifiersDetail...",log_debug)
    RETURN(false)
   ENDIF
   CALL logmessage(curprog,"Exiting loadModifiersDetail...",log_debug)
   RETURN(true)
 ...
 SUBROUTINE (inactivedxinchargeeventmod(dummyvar=i2) =i2)
   ...
   FOR (dxidxcnt = 1 TO size(chargediagnosisdetail->diagnosislist,5))
     ...
     SELECT INTO "nl:"
      FROM (dummyt d1  WITH seq = size(request->charge_mod,5))
      PLAN (d1
       WHERE (request->charge_mod[d1.seq].field6=chargediagnosisdetail->diagnosislist[dxidxcnt].
       field6)
        AND uar_get_code_meaning(request->charge_mod[d1.seq].field1_id)=cdfmeaning)
      DETAIL
       modexistsind = true
      WITH nocounter
     ;end select
     ...
     IF (modexistsind=false)
      IF (getchargeeventmodid(request->charge_item_id,chargediagnosisdetail->diagnosislist[dxidxcnt].
       field6,cdfmeaning,chargeeventmodid))
     ...
     IF (checkdminfotoremoveparentcodes(0))
      ...
      SELECT INTO "nl:"
       ...
       PLAN (d1
        ...
        field6)
         AND uar_get_code_meaning(request->charge_mod[d1.seq].field1_id)=cdfmeaning
         AND (request->charge_mod[d1.seq].field2_id=chargediagnosisdetail->diagnosislist[dxidxcnt].
        field2_id))
       DETAIL
        modexistsind = true
       WITH nocounter
      ;end select
      IF (modexistsind=false)
       IF (inactivateparentcodes(request->charge_item_id,chargediagnosisdetail->diagnosislist[
        dxidxcnt].field6,cdfmeaning))
 ...
 SUBROUTINE (inactivemodinchargeeventmod(dummyvar=i2) =i2)
   ...
   FOR (modidxcnt = 1 TO size(chargemodifiersdetail->modifierslist,5))
     ...
     SELECT INTO "nl:"
      FROM (dummyt d1  WITH seq = size(request->charge_mod,5))
      PLAN (d1
       WHERE (request->charge_mod[d1.seq].field6=chargemodifiersdetail->modifierslist[modidxcnt].
       field6)
        AND validate(request->charge_mod[d1.seq].charge_event_mod_id,0.0) > 0
        AND uar_get_code_meaning(request->charge_mod[d1.seq].field1_id)=cdfmeaning)
      DETAIL
       modexistsind = true
      WITH nocounter
     ...
     SET chargeeventmodid = 0.0
     IF (modexistsind=false)
      IF (getchargeeventmodid(request->charge_item_id,chargemodifiersdetail->modifierslist[modidxcnt]
       .field6,cdfmeaning,chargeeventmodid))
     ...
     IF (checkdminfotoremoveparentcodes(0))
      ...
      SELECT INTO "nl:"
       ...
       PLAN (d1
        ...
        field6)
         AND uar_get_code_meaning(request->charge_mod[d1.seq].field1_id)=cdfmeaning
         AND (request->charge_mod[d1.seq].field2_id=chargemodifiersdetail->modifierslist[modidxcnt].
        field2_id)
         AND validate(request->charge_mod[d1.seq].charge_mod_source_cd,0.0) != cs4518006_manually_add
        )
       DETAIL
        modexistsind = true
       WITH nocounter
      ;end select
      IF (modexistsind=false)
       IF (inactivateparentcodes(request->charge_item_id,chargemodifiersdetail->modifierslist[
        modidxcnt].field6,cdfmeaning))
        CALL logmessage(curprog,"Successfully removed parent Diagnosis codes.",log_debug)
       ELSE
        CALL logmessage(curprog,"inactivateParentCodes failed",log_debug)
        CALL logmessage(curprog,"Exiting inactiveDXInChargeEventMod...",log_debug)
 ...
 SUBROUTINE (getproviderspecialty(perfphysid=f8) =null)
   ...
   IF ((request->verify_phys_id > 0))
    SET providerid = request->verify_phys_id
   ELSEIF (perfphysid > 0)
    SET providerid = perfphysid
   ELSEIF ((request->ord_phys_id > 0))
    SET providerid = request->ord_phys_id
   ENDIF
   IF ((request->perf_loc_cd > 0))
    SET spcloccd = request->perf_loc_cd
   ELSE
    SELECT INTO "nl:"
     FROM charge c,
      encounter e
