CREATE PROGRAM dm2_get_package_history:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE loh_db_name = vc WITH public, constant("strt")
 DECLARE loh_file_name = vc WITH public, noconstant("dm2_not_set")
 DECLARE dgph_str = vc WITH public, noconstant("")
 IF (check_logfile("dm2_get_pkg_hist",".log","DM2_GET_PACKAGE_HISTORY LOG FILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Capturing Package history."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = concat("Querying dm_alpha_features_env for Package history to create ",
  loh_file_name,".")
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SET loh_file_name = concat("dm2_ni_load_hist_afe_",loh_db_name,".csv")
 SELECT INTO value(loh_file_name)
  FROM dm_info di,
   dm_alpha_features_env d
  WHERE di.info_domain="DATA MANAGEMENT"
   AND di.info_name="DM_ENV_ID"
   AND d.environment_id=di.info_number
   AND d.alpha_feature_nbr > 0
  ORDER BY d.alpha_feature_nbr
  HEAD REPORT
   dgph_str = concat("ALPHA_FEATURE_NBR",",ENVIRONMENT_ID",",START_DT_TM",",END_DT_TM",",STATUS",
    ",INST_MODE",",CALLING_SCRIPT",",CURR_MIGRATION_IND"), col 0, dgph_str,
   row + 1
  DETAIL
   dgph_str = evaluate(nullind(d.alpha_feature_nbr),1,"::DM2NULLVALUE::",cnvtstring(d
     .alpha_feature_nbr)), dgph_str = build('"',dgph_str,'"'), dgph_str,
   dgph_str = build(',"<environment_id>"'), dgph_str, dgph_str = evaluate(nullind(d.start_dt_tm),1,
    "::DM2NULLVALUE::",format(cnvtdatetimeutc(d.start_dt_tm,1),"dd-mmm-yyyy hh:mm:ss;;d")),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.end_dt_tm),1,
    "::DM2NULLVALUE::",format(cnvtdatetimeutc(d.end_dt_tm,1),"dd-mmm-yyyy hh:mm:ss;;d")),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.status),1,
    "::DM2NULLVALUE::",trim(replace(check(d.status," "),",","~",0))),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.inst_mode),1,
    "::DM2NULLVALUE::",trim(replace(check(d.inst_mode," "),",","~",0))),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.calling_script),1,
    "::DM2NULLVALUE::",trim(replace(check(d.calling_script," "),",","~",0))),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.curr_migration_ind),1,
    "::DM2NULLVALUE::",cnvtstring(d.curr_migration_ind)),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, row + 1
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Querying dm_rdds_req_install_env for Package history to create ",
  loh_file_name,".")
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SET loh_file_name = concat("dm2_ni_load_hist_drrie_",loh_db_name,".csv")
 SELECT INTO value(loh_file_name)
  ...
  (SELECT
   ...
   WHERE di.info_domain="DATA MANAGEMENT"
    AND di.info_name="DM_ENV_ID"))
  ORDER BY d.req_name, d.req_type, d.req_version_nbr
  HEAD REPORT
   dgph_str = concat("ENVIRONMENT_ID",",DM_RDDS_RIE_ID",",REQ_TYPE",",REQ_NAME",",REQ_VERSION_NBR",
    ",README_ID",",README_INSTANCE",",ACTIVE_IND",",ACTIVE_STATUS_DT_TM",",ACTIVE_STATUS_PRSNL_ID",
    ",ACTIVE_STATUS_CD",",UPDT_ID",",UPDT_DT_TM",",UPDT_TASK",",UPDT_APPLCTX",
    ",UPDT_CNT"), col 0, dgph_str,
   row + 1
  DETAIL
   dgph_str = build('"<environment_id>"'), dgph_str, dgph_str = build(',"seq(dm_seq,nextval)"'),
   dgph_str, dgph_str = evaluate(nullind(d.req_type),1,"::DM2NULLVALUE::",trim(replace(check(d
       .req_type," "),",","~",0))), dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.req_name),1,"::DM2NULLVALUE::",trim(replace(check(d
       .req_name," "),",","~",0))), dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.req_version_nbr),1,"::DM2NULLVALUE::",cnvtstring(d
     .req_version_nbr)), dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.readme_id),1,"::DM2NULLVALUE::",cnvtstring(d.readme_id)),
   dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.readme_instance),1,"::DM2NULLVALUE::",cnvtstring(d
     .readme_instance)), dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.active_ind),1,"::DM2NULLVALUE::",cnvtstring(d.active_ind)),
   dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.active_status_dt_tm),1,"::DM2NULLVALUE::",format(
     cnvtdatetimeutc(d.active_status_dt_tm,1),"dd-mmm-yyyy hh:mm:ss;;d")), dgph_str = build(',"',
    dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.active_status_prsnl_id),1,"::DM2NULLVALUE::",cnvtstring(d
     .active_status_prsnl_id)), dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.active_status_cd),1,"::DM2NULLVALUE::",cnvtstring(d
     .active_status_cd)), dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.updt_id),1,"::DM2NULLVALUE::",cnvtstring(d.updt_id)),
   dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.updt_dt_tm),1,"::DM2NULLVALUE::",format(cnvtdatetimeutc(d
      .updt_dt_tm,1),"dd-mmm-yyyy hh:mm:ss;;d")), dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.updt_task),1,"::DM2NULLVALUE::",cnvtstring(d.updt_task)),
   dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.updt_applctx),1,"::DM2NULLVALUE::",cnvtstring(d
     .updt_applctx)), dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.updt_cnt),1,"::DM2NULLVALUE::",cnvtstring(d.updt_cnt)),
   dgph_str = build(',"',dgph_str,'"'),
   dgph_str, row + 1
  WITH nocounter, format = variable, noformfeed,
   maxrow = 1, maxcol = 5000
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 SET loh_file_name = concat("dm2_ni_load_hist_dol_",loh_db_name,".csv")
 SET dm_err->eproc = concat("Querying dm_ocd_log for Package history to create ",loh_file_name,".")
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 SELECT INTO value(loh_file_name)
  ...
  HEAD REPORT
   dgph_str = concat("APPL_IDENT",",ENVIRONMENT_ID",",PROJECT_TYPE",",PROJECT_NAME",
    ",PROJECT_INSTANCE",
    ",OCD",",BATCH_DT_TM",",STATUS",",START_DT_TM",",END_DT_TM",
    ",DRIVER_COUNT",",ESTIMATED_TIME",",MESSAGE",",ACTIVE_IND",",UPDT_DT_TM"), col 0, dgph_str,
   row + 1
  DETAIL
   dgph_str = evaluate(nullind(d.appl_ident),1,"::DM2NULLVALUE::",trim(replace(check(d.appl_ident," "
       ),",","~",0))), dgph_str = build('"',dgph_str,'"'), dgph_str,
   dgph_str = build(',"<environment_id>"'), dgph_str, dgph_str = evaluate(nullind(d.project_type),1,
    "::DM2NULLVALUE::",trim(replace(check(d.project_type," "),",","~",0))),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.project_name),1,
    "::DM2NULLVALUE::",trim(replace(check(d.project_name," "),",","~",0))),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.project_instance),1,
    "::DM2NULLVALUE::",cnvtstring(d.project_instance)),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.ocd),1,
    "::DM2NULLVALUE::",cnvtstring(d.ocd)),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.batch_dt_tm),1,
    "::DM2NULLVALUE::",format(cnvtdatetimeutc(d.batch_dt_tm,1),"dd-mmm-yyyy hh:mm:ss;;d")),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.status),1,
    "::DM2NULLVALUE::",trim(replace(check(d.status," "),",","~",0))),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.start_dt_tm),1,
    "::DM2NULLVALUE::",format(cnvtdatetimeutc(d.start_dt_tm,1),"dd-mmm-yyyy hh:mm:ss;;d")),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.end_dt_tm),1,
    "::DM2NULLVALUE::",format(cnvtdatetimeutc(d.end_dt_tm,1),"dd-mmm-yyyy hh:mm:ss;;d")),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = evaluate(nullind(d.driver_count),1,
    "::DM2NULLVALUE::",cnvtstring(d.driver_count)),
   dgph_str = build(',"',dgph_str,'"'), dgph_str, dgph_str = build(',"',d.estimated_time,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.message),1,"::DM2NULLVALUE::",trim(replace(check(d.message,
       " "),",","~",0))), dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.active_ind),1,"::DM2NULLVALUE::",cnvtstring(d.active_ind)),
   dgph_str = build(',"',dgph_str,'"'),
   dgph_str, dgph_str = evaluate(nullind(d.updt_dt_tm),1,"::DM2NULLVALUE::",format(cnvtdatetimeutc(d
      .updt_dt_tm,1),"dd-mmm-yyyy hh:mm:ss;;d")), dgph_str = build(',"',dgph_str,'"'),
 ...
 ENDIF
#exit_script
 SET dm_err->eproc = "Package history captured successfully."
 CALL final_disp_msg("dm2_get_pkg_hist")
END GO
