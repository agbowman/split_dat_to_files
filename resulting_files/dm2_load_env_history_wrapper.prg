CREATE PROGRAM dm2_load_env_history_wrapper
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dlew_path = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dlew_csv_file = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dlew_src_env = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dlew_src_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dlew_tgt_env = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dlew_tgt_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dlew_tbl_fnd = i2 WITH protect, noconstant(0)
 DECLARE dlew_idx = i4 WITH protect, noconstant(0)
 DECLARE dlew_tbl_name = vc WITH protect, noconstant("")
 DECLARE dlew_row_count = i4 WITH protect, noconstant(0)
 DECLARE dlew_updt_count = i4 WITH protect, noconstant(0)
 DECLARE dlew_file_delim = c1 WITH protect, noconstant(evaluate(dm2_sys_misc->cur_os,"AXP","]","/"))
 IF (check_logfile("dm2_load_env_wrap",".log","dm2_load_env_history_wrapper")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM2_LOAD_ENV_HISTORY_WRAPPER."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dlew_path = trim( $1)
 SET dlew_src_env = trim( $2)
 SET dlew_tgt_env = trim( $3)
 IF (findstring(dlew_file_delim,trim(dlew_path),1,1) != size(trim(dlew_path)))
  SET dlew_path = concat(trim(dlew_path),dlew_file_delim)
 ENDIF
 IF (validate(drrr_rf_data->mode,"DM2NOTSET") != "ISOLATED")
  IF (dehr_chk_env(dlew_src_env,dlew_src_env_id)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (dehr_chk_env(dlew_tgt_env,dlew_tgt_env_id)=0)
  GO TO exit_script
 ENDIF
 IF (dehr_verify_admin_synonym(null)=0)
  GO TO exit_script
 ENDIF
 IF (drr_validate_adm_env_csv(dlew_path,dlew_src_env)=0)
  RETURN(0)
 ENDIF
 IF (dehr_load_adm_tables(null)=0)
  GO TO exit_script
 ENDIF
 IF ((dehr_table_list->total_nbr != drr_env_hist_misc->cnt))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Compare Admin table count between summary file and DM_INFO table."
  SET dm_err->emsg = concat("The number of Admin env tables in summary file ",trim(cnvtstring(
     drr_env_hist_misc->cnt))," does not match the number of Admin tables in DM_INFO ",trim(
    cnvtstring(dehr_table_list->total_nbr)))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 FOR (dlew_idx = 1 TO dehr_table_list->cnt)
   IF (locateval(dlew_tbl_fnd,1,drr_env_hist_misc->cnt,dehr_table_list->qual[dlew_idx].table_name,
    drr_env_hist_misc->qual[dlew_tbl_fnd].table_name)=0)
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Find Admin Environment table in summary file."
    SET dm_err->emsg = concat("Fail to find table ",dehr_table_list->qual[dlew_idx].table_name)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ENDIF
 ENDFOR
 FOR (dlew_idx = 1 TO drr_env_hist_misc->cnt)
   SET dlew_tbl_name = drr_env_hist_misc->qual[dlew_idx].table_name
   SET dlew_row_count = cnvtint(drr_env_hist_misc->qual[dlew_idx].row_count)
   IF (dlew_row_count > 0)
    SET dlew_csv_file = concat(dlew_path,drr_env_hist_misc->qual[dlew_idx].csv_file_name)
    EXECUTE dm2_dbimport dlew_csv_file, "dm2_load_env_history", 1000
    SET dm_err->eproc = build("Get table ",dlew_tbl_name," row count in Target for environment_id ",
     dlew_tgt_env_id)
    CALL disp_msg("",dm_err->logfile,0)
    SET dlew_updt_count = 0
    SELECT INTO "nl:"
     FROM (parser(dlew_tbl_name) d)
     WHERE parser(build("d.environment_id = ",dlew_tgt_env_id))
     DETAIL
      dlew_updt_count = (dlew_updt_count+ 1)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     ROLLBACK
    ...
    ENDIF
    IF ((dm_err->debug_flag > 0))
     CALL echo(build("row count in target : ",dlew_updt_count))
     CALL echo(build("row count in csv : ",dlew_row_count))
    ENDIF
    IF (dlew_updt_count != dlew_row_count)
     ROLLBACK
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Row count does not match row count in csv."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     GO TO exit_script
    ELSEIF ((dir_env_maint_rs->process != "REPLICATE"))
     COMMIT
    ENDIF
   ENDIF
 ENDFOR
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_load_env_history_wrapper completed"
 ENDIF
 CALL final_disp_msg("dm2_load_env_wrap")
END GO
