CREATE PROGRAM afc_get_clients:dba
 RECORD reply(
 ...
 SET cnt = 1
 SET stat = uar_get_meaning_by_codeset(code_set,cdf_meaning,cnt,client)
 SET code_set = 48
 ...
 SELECT INTO "nl:"
  ...
  ORDER BY o.org_name
  DETAIL
   count1 += 1, stat = alterlist(reply->organization,count1), reply->organization[count1].org_name =
   o.org_name,
   reply->organization[count1].organization_id = o.organization_id
  WITH nocounter
 ;end select
 SET stat = alterlist(reply->organization,count1)
