CREATE PROGRAM dm2_retry_imp_exp
 ...
 END ;Subroutine
 DECLARE dcprom_tspacesize_install_type(null) = i2
 DECLARE dcprom_install_type(null) = i2
 DECLARE dcprom_ddlops_install_type(null) = i2
 DECLARE dcprom_ocd_exists_ind = i2 WITH protect, noconstant(0)
 DECLARE dcprom_schema_date_exists_ind = i2 WITH protect, noconstant(0)
 IF (validate(dm2comprom->install_mode," ")=" ")
  FREE RECORD dm2comprom
  RECORD dm2comprom(
    1 install_mode = vc
    1 schema_date = vc
    1 package_number = i4
    1 package_number_vc = vc
    1 run_id = i4
    1 gen_dt_tm = dq8
    1 process_option = vc
    1 last_checkpoint = vc
  ...
  SET dm2comprom->install_mode = "DM2NOTSET"
  SET dm2comprom->schema_date = " "
  SET dm2comprom->package_number = 0
  SET dm2comprom->process_option = "DM2NOTSET"
 ENDIF
 FREE RECORD dcprom_sch_date
 RECORD dcprom_sch_date(
   1 qual[*]
     2 schema_date = dq8
     2 run_id = f8
 )
 FREE RECORD dcprom_ocd
 RECORD dcprom_ocd(
   1 dcprom_install_plan_cnt = i4
   1 qual[*]
     2 dcprom_abs_pkg = i4
     2 dcprom_pkg = i4
     2 dcprom_checkpoint = vc
     2 dcprom_run_id = f8
   1 i_plan[*]
     2 dcprom_abs_pkg = i4
     2 dcprom_pkg = i4
 ...
 SUBROUTINE dcprom_ddlops_install_type(null)
   DECLARE dcprom_proceed_ind = i2 WITH protect, noconstant(0)
   DECLARE dcprom_ddl_num_cnt = i4 WITH protect, noconstant(0)
   DECLARE dcprom_schema_ind = i2 WITH protect, noconstant(0)
   DECLARE dcprom_ocd_ind = i2 WITH protect, noconstant(0)
   DECLARE dcprom_pkg_cnt = i4 WITH protect, noconstant(0)
   DECLARE dcprom_plan_cnt = i4 WITH protect, noconstant(0)
   DECLARE dcprom_exit_ind = i2 WITH protect, noconstant(0)
   DECLARE dsdp_cnt = i4 WITH protect, noconstant(0)
   SET dm_err->eproc = "Find DDL Op rows to report on."
   CALL disp_msg("",dm_err->logfile,0)
   SET dcprom_ocd->dcprom_install_plan_cnt = 0
   SET stat = alterlist(dcprom_ocd->qual,0)
   SET stat = alterlist(dcprom_ocd->i_plan,0)
   SET stat = alterlist(dcprom_sch_date->qual,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops d
    WHERE d.run_id > 0
    ...
    DETAIL
     IF (d.ocd != 0)
      IF (d.ocd < 0)
       dcprom_plan_cnt = (dcprom_plan_cnt+ 1), stat = alterlist(dcprom_ocd->i_plan,dcprom_plan_cnt),
       dcprom_ocd->i_plan[dcprom_plan_cnt].dcprom_abs_pkg = abs(d.ocd),
       dcprom_ocd->i_plan[dcprom_plan_cnt].dcprom_pkg = d.ocd, dcprom_ocd->i_plan[dcprom_plan_cnt].
       dcprom_checkpoint = d.last_checkpoint, dcprom_ocd->i_plan[dcprom_plan_cnt].dcprom_run_id = d
       .run_id
      ELSE
       dcprom_pkg_cnt = (dcprom_pkg_cnt+ 1), stat = alterlist(dcprom_ocd->qual,dcprom_pkg_cnt),
       dcprom_ocd->qual[dcprom_pkg_cnt].dcprom_abs_pkg = abs(d.ocd),
       dcprom_ocd->qual[dcprom_pkg_cnt].dcprom_pkg = d.ocd, dcprom_ocd->qual[dcprom_pkg_cnt].
       dcprom_checkpoint = d.last_checkpoint, dcprom_ocd->qual[dcprom_pkg_cnt].dcprom_run_id = d
       .run_id
      ENDIF
     ELSEIF (d.schema_date > 0)
      dsdp_cnt = (dsdp_cnt+ 1), stat = alterlist(dcprom_sch_date->qual,dsdp_cnt), dcprom_sch_date->
      qual[dsdp_cnt].schema_date = d.schema_date,
      dcprom_sch_date->qual[dsdp_cnt].run_id = d.run_id
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   IF (size(dcprom_ocd->qual,5)=0
    AND size(dcprom_sch_date->qual,5)=0
    AND size(dcprom_ocd->i_plan,5)=0)
    SET dm_err->eproc = "No installation activity to report on."
    CALL disp_msg("",dm_err->logfile,0)
    RETURN(1)
   ENDIF
   WHILE (dcprom_exit_ind=0)
     IF ((dm2comprom->install_mode="DM2NOTSET"))
      CALL dcprom_install_type(null)
     ENDIF
     CASE (dm2comprom->install_mode)
      OF "BATCH_INSTALL_PLAN":
       IF (size(dcprom_ocd->i_plan,5) > 0)
        SET help = pos(4,1,10,100)
        SET help =
        SELECT INTO "nl:"
         batch_install_plan = cnvtreal(dcprom_ocd->i_plan[d.seq].dcprom_abs_pkg)
         FROM (dummyt d  WITH seq = size(dcprom_ocd->i_plan,5))
         WITH nocounter
        ;end select
        CALL clear(15,1,132)
        CALL text(15,3,"Choose Batch Install Plan from List:")
        CALL accept(15,41,"9(11);F")
        IF (curhelp > 0)
         SET dm2comprom->package_number = dcprom_ocd->i_plan[curhelp].dcprom_pkg
         SET dm2comprom->run_id = dcprom_ocd->i_plan[curhelp].dcprom_run_id
         SET help = off
         RETURN(1)
        ELSE
         SET dm2comprom->package_number = (curaccept * - (1))
         SET dcprom_pkg_cnt = 0
         SET dcprom_pkg_cnt = locateval(dcprom_pkg_cnt,1,size(dcprom_ocd->i_plan,5),dm2comprom->
          package_number,dcprom_ocd->i_plan[dcprom_pkg_cnt].dcprom_pkg)
         IF (dcprom_pkg_cnt > 0)
          SET dm2comprom->run_id = dcprom_ocd->i_plan[dcprom_pkg_cnt].dcprom_run_id
          SET help = off
          RETURN(1)
         ELSE
          CALL clear(16,1,132)
          CALL text(16,3,"Invalid Batch Install Plan.")
          CALL pause(3)
         ENDIF
        ENDIF
       ELSE
        CALL clear(16,1,132)
        CALL text(16,3,"There are no Batch Install Plans to choose from.")
        CALL pause(3)
       ENDIF
       SET help = off
      OF "PACKAGE":
       IF (size(dcprom_ocd->qual,5) > 0)
        ...
        SET help =
        SELECT INTO "nl:"
         package_number = cnvtreal(dcprom_ocd->qual[d.seq].dcprom_abs_pkg)
         FROM (dummyt d  WITH seq = size(dcprom_ocd->qual,5))
         WITH nocounter
        ;end select
        CALL clear(15,1,132)
        CALL text(15,3,"Choose Package from List:")
        CALL accept(15,30,"9(11);F")
        IF (curhelp > 0)
         SET dm2comprom->package_number = dcprom_ocd->qual[curhelp].dcprom_pkg
         SET dm2comprom->run_id = dcprom_ocd->qual[curhelp].dcprom_run_id
         SET help = off
         RETURN(1)
        ELSE
         SET dm2comprom->package_number = curaccept
         SET dcprom_pkg_cnt = 0
         SET dcprom_pkg_cnt = locateval(dcprom_pkg_cnt,1,size(dcprom_ocd->qual,5),dm2comprom->
          package_number,dcprom_ocd->qual[dcprom_pkg_cnt].dcprom_pkg)
         IF (dcprom_pkg_cnt > 0)
          SET dm2comprom->run_id = dcprom_ocd->qual[dcprom_pkg_cnt].dcprom_run_id
          SET help = off
          RETURN(1)
         ELSE
          CALL clear(16,1,132)
          CALL text(16,3,"Invalid Package.")
          CALL pause(3)
          CALL clear(16,1,132)
         ENDIF
        ENDIF
       ELSE
        CALL clear(16,1,132)
        CALL text(16,3,"There are no Packages to choose from.")
        CALL pause(3)
       ENDIF
       SET help = off
      OF "SCHEMA_DATE":
       IF (size(dcprom_sch_date,5) > 0)
        SET help = pos(4,1,10,100)
        SET help =
        SELECT INTO "nl:"
         schema_date = dcprom_sch_date->qual[d.seq].schema_date"DD-MMM-YYYY;;D"
         FROM (dummyt d  WITH seq = size(dcprom_sch_date->qual,5))
         WITH nocounter
        ;end select
        CALL clear(15,1,132)
        CALL text(15,3,"Choose Schema Date from List:")
        CALL accept(15,37,"P(11);CUF")
        IF (curhelp=0)
         SET dm2comprom->schema_date = curaccept
         SET dsdp_cnt = 0
         SET dsdp_cnt = locateval(dsdp_cnt,1,size(dcprom_sch_date->qual,5),cnvtdatetime(dm2comprom->
           schema_date),dcprom_sch_date->qual[dsdp_cnt].schema_date)
         IF (dsdp_cnt > 0)
          SET dm2comprom->run_id = dcprom_sch_date->qual[dsdp_cnt].run_id
          SET help = off
          RETURN(1)
         ELSE
          CALL clear(16,1,132)
          CALL text(16,3,"Invalid Schema_Date.")
          CALL pause(3)
          CALL clear(16,1,132)
         ENDIF
        ELSE
         SET dm2comprom->schema_date = format(dcprom_sch_date->qual[curhelp].schema_date,
          "dd-mmm-yyyy;L;D")
         SET dm2comprom->run_id = dcprom_sch_date->qual[curhelp].run_id
         SET help = off
         RETURN(1)
        ENDIF
       ELSE
        CALL clear(16,1,132)
        CALL text(16,3,"There are no Schema Dates to choose from.")
        CALL pause(3)
        CALL clear(16,1,132)
       ENDIF
       SET help = off
      OF "DM2NOTSET":
       SET dcprom_exit_ind = 1
       RETURN(1)
     ENDCASE
     SET dm2comprom->install_mode = "DM2NOTSET"
   ENDWHILE
 END ;Subroutine
 SUBROUTINE dcprom_tspacesize_install_type(null)
   DECLARE dtit_no_schema = i2 WITH protect, noconstant(0)
   DECLARE dtit_sd_cnt = i4 WITH protect, noconstant(0)
   DECLARE dtit_pkg_cnt = i4 WITH protect, noconstant(0)
   DECLARE dtit_search = vc WITH protect, noconstant("")
   DECLARE dtit_batch_cnt = i4 WITH protect, noconstant(0)
   FREE RECORD dcprom_ts_hold
   RECORD dcprom_ts_hold(
     1 sd[*]
       2 schema_date = vc
     1 i_plan[*]
       2 batch_pkg_disp = vc
   ...
   IF ((dm2comprom->install_mode="DM2NOTSET"))
    CALL dcprom_install_type(null)
   ENDIF
   SET dm_err->eproc = "Getting values from DM2_TSPACE_SIZE"
   SELECT DISTINCT INTO "nl:"
    u.install_type_value
    FROM dm2_tspace_size u
    WHERE u.install_type=evaluate(dm2comprom->install_mode,"BATCH_INSTALL_PLAN","PACKAGE","PACKAGE",
     "PACKAGE",
     "SCHEMA_DATE")
     AND ((u.install_status != "SUCCESS") OR (u.install_status = null))
    ORDER BY u.install_type, u.install_type_value
    DETAIL
     IF (u.install_type="PACKAGE")
      IF (cnvtint(u.install_type_value) < 0)
       dtit_batch_cnt = (dtit_batch_cnt+ 1), stat = alterlist(dcprom_ts_hold->i_plan,dtit_batch_cnt),
       dcprom_ts_hold->i_plan[dtit_batch_cnt].batch_pkg_disp = cnvtstring(abs(cnvtint(u
          .install_type_value))),
       dcprom_ts_hold->i_plan[dtit_batch_cnt].batch_pkg_real = u.install_type_value
      ELSE
       dtit_pkg_cnt = (dtit_pkg_cnt+ 1), stat = alterlist(dcprom_ts_hold->pkg,dtit_pkg_cnt),
       dcprom_ts_hold->pkg[dtit_pkg_cnt].pkg_number = u.install_type_value
      ENDIF
     ELSEIF (u.install_type="SCHEMA_DATE")
      dtit_sd_cnt = (dtit_sd_cnt+ 1), stat = alterlist(dcprom_ts_hold->sd,dtit_sd_cnt),
      dcprom_ts_hold->sd[dtit_sd_cnt].schema_date = u.install_type_value
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
   ...
   ENDIF
   IF (curqual=0)
    CALL text(15,50,concat("There are no tablespace entries that need to be processed for ",
      dm2comprom->install_mode,"."))
    RETURN(0)
   ENDIF
   SET help = pos(7,80,14,30)
   CASE (dm2comprom->install_mode)
    OF "SCHEMA_DATE":
     SET help =
     SELECT INTO "nl:"
      schema_date = dcprom_ts_hold->sd[d.seq].schema_date
      FROM (dummyt d  WITH seq = size(dcprom_ts_hold->sd,5))
      WITH nocounter
     ;end select
    OF "PACKAGE":
     SET help =
     SELECT DISTINCT INTO "nl:"
      package_number = dcprom_ts_hold->pkg[d.seq].pkg_number
      FROM (dummyt d  WITH seq = size(dcprom_ts_hold->pkg,5))
      WITH nocounter
     ;end select
    OF "BATCH_INSTALL_PLAN":
     SET help =
     SELECT DISTINCT INTO "nl:"
      package_number = dcprom_ts_hold->i_plan[d.seq].batch_pkg_disp
      FROM (dummyt d  WITH seq = size(dcprom_ts_hold->i_plan,5))
      WITH nocounter
     ;end select
   ENDCASE
   SET dtit_pkg_cnt = 0
   SET dtit_sd_cnt = 0
   SET dtit_batch_cnt = 0
   CASE (dm2comprom->install_mode)
    OF "PACKAGE":
     CALL text(14,21,"Package Number:")
     IF (size(dcprom_ts_hold->pkg,5) > 0)
      CALL accept(14,38,"9(11);F"," ")
      IF (curhelp=0)
       SET dtit_pkg_cnt = locateval(dtit_pkg_cnt,1,size(dcprom_ts_hold->pkg,5),cnvtstring(curaccept),
        dcprom_ts_hold->pkg[dtit_pkg_cnt].pkg_number)
       IF (dtit_pkg_cnt > 0)
        SET dm2comprom->package_number_vc = dcprom_ts_hold->pkg[dtit_pkg_cnt].pkg_number
        SET dm2comprom->package_number = cnvtint(dcprom_ts_hold->pkg[dtit_pkg_cnt].pkg_number)
       ELSE
        CALL text(15,50,concat("There are no  tablespace entries Package Number:",trim(cnvtstring(
            curaccept)),"."))
        RETURN(0)
       ENDIF
      ELSE
       SET dm2comprom->package_number = cnvtint(dcprom_ts_hold->pkg[curhelp].pkg_number)
       SET dm2comprom->package_number_vc = dcprom_ts_hold->pkg[curhelp].pkg_number
      ENDIF
     ELSE
      CALL clear(16,1,132)
      CALL text(16,3,"There are no Packages to choose from.")
     ...
     ENDIF
    OF "BATCH_INSTALL_PLAN":
     CALL text(14,18,"Batch Install Plan:")
     IF (size(dcprom_ts_hold->i_plan,5) > 0)
      CALL accept(14,38,"9(11);F"," ")
      IF (curhelp=0)
       SET dtit_batch_cnt = locateval(dtit_batch_cnt,1,size(dcprom_ts_hold->i_plan,5),cnvtstring((
         curaccept * - (1))),dcprom_ts_hold->i_plan[dtit_batch_cnt].batch_pkg_real)
       IF (dtit_batch_cnt > 0)
        SET dm2comprom->package_number = cnvtint(dcprom_ts_hold->i_plan[dtit_batch_cnt].
         batch_pkg_real)
       ELSE
        CALL text(15,50,concat("There are no  tablespace entries Package Number:",trim(cnvtstring(
            curaccept)),"."))
       ...
       ENDIF
      ELSE
       SET dm2comprom->package_number = cnvtint(dcprom_ts_hold->i_plan[curhelp].batch_pkg_real)
      ENDIF
     ELSE
      CALL clear(16,1,132)
      CALL text(16,3,"There are no Batch Install Plans to choose from.")
     ...
     ENDIF
    OF "SCHEMA_DATE":
     CALL text(14,24,"Schema Date:")
     IF (size(dcprom_ts_hold->sd,5) > 0)
      CALL accept(14,38,"P(11);cuF"," ")
      IF (curhelp=0)
       SET dtit_sd_cnt = locateval(dtit_sd_cnt,1,size(dcprom_ts_hold->sd,5),curaccept,dcprom_ts_hold
        ->sd[dtit_sd_cnt].schema_date)
       IF (dtit_sd_cnt > 0)
        SET dm2comprom->schema_date = dcprom_ts_hold->sd[dtit_sd_cnt].schema_date
       ELSE
        CALL text(15,50,concat("There are no  tablespace entries for Schema Date:",trim(curaccept),
          "."))
        RETURN(0)
       ENDIF
      ELSE
       SET dm2comprom->schema_date = dcprom_ts_hold->sd[curhelp].schema_date
      ENDIF
     ELSE
      CALL clear(16,1,132)
      CALL text(16,3,"There are no Schema Dates to choose from.")
   ...
   SET help = off
   CALL clear(22,3,131)
   CALL text(22,3,"CORRECT (Y/N)?")
   CALL accept(22,20,"p;cu","Y"
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dcprom_install_type(null)
   CALL text(2,2,"INSTALLATION TYPE PROMPT")
   CALL box(1,1,7,132)
   CALL text(4,3,
    "  Please specify whether you would like to process by Package Number, Schema Date, or Batch Install Plan."
    )
   CALL text(6,3,
    "  Enter 'P' for Package, 'S' for Schema Date , 'B' for Batch Install Plan, 'X' to exit:")
   CALL line(3,1,132,xhor)
   CALL accept(6,93,"p;cu","P"
    WHERE cnvtupper(curaccept) IN ("S", "P", "X", "B"))
   CASE (curaccept)
    OF "X":
     SET dm2comprom->install_mode = "DM2NOTSET"
    OF "P":
     SET dm2comprom->install_mode = "PACKAGE"
    OF "S":
     SET dm2comprom->install_mode = "SCHEMA_DATE"
    OF "B":
     SET dm2comprom->install_mode = "BATCH_INSTALL_PLAN"
   ENDCASE
   RETURN(1)
 END ;Subroutine
 ...
 SUBROUTINE dfr_get_file(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_retry_imp_exp",".log","dm2_retry_imp_exp")=0)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag=722))
  SET message = nowindow
 ELSE
  SET message = window
 ENDIF
 DECLARE dmmt_confirm_export_info(null) = i2
 DECLARE dmmt_scroll(ds_rec_str=vc) = i2
 DECLARE dmmt_screen_header(null) = i2
 DECLARE dmmt_driver(dd_leave=i2) = i2
 DECLARE dmmt_work_menu(dwm_screen_return=vc(ref)) = i2
 DECLARE dmmt_intermediate_work_menu(diwm_screen_return=vc(ref)) = i2
 DECLARE fill_dmmt_screen(null) = i2
 DECLARE dmmt_fill_intermediate_screen(null) = i2
 DECLARE dmmt_chk_opt_vals(dcov_all_filled_out=i2(ref)) = i2
 DECLARE dmmt_restart_ddl_rows(null) = i2
 DECLARE dmmt_view_logfile(dvl_log_in=vc) = i2
 DECLARE dmmt_prelim(null) = i2
 DECLARE drie_view_logfile2(dvl_log_dir=vc,dvl_log=vc) = i2
 DECLARE dmmt_screen_refresh(null) = null
 DECLARE dmmt_xfer_errors(null) = i2
 DECLARE dmmt_fail_tbl_import_ops(dftio_run_id=i4,dftio_tbl_name=vc,dftio_op_type=vc) = i2
 DECLARE dmmt_op_status_cnts(dosc_loop_tbl_name=vc,dosc_loop_op_type=vc,dosc_total_cnt=i4(ref),
  dosc_running_cnt=i4(ref),dosc_ready_cnt=i4(ref),
  dosc_complete_cnt=i4(ref),dosc_error_cnt=i4(ref)) = i2
 DECLARE dmmt_chk_tbl_eligible_restart(dcter_run_id=i4,dcter_tbl_name=vc,dcter_op_type=vc,
  dcter_eligible_ind=i2(ref)) = i2
 DECLARE dmmt_split_tbls_retry_screen(null) = i2
 DECLARE dmmt_dbase_name = vc WITH protect, noconstant(" ")
 DECLARE dmmt_src_dbase_name = vc WITH protect, noconstant(" ")
 DECLARE dmmt_tab_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmmt_leave = i2 WITH protect, noconstant(0)
 DECLARE dmmt_failed_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmmt_log_loc_hold = vc WITH protect, noconstant("")
 DECLARE dmmt_err_ret = i2 WITH protect, noconstant(0)
 DECLARE dmmt_file_delim = c1 WITH protect, noconstant("")
 DECLARE dmmt_daso_success_ret = i2 WITH protect, noconstant(0)
 DECLARE dmmt_del_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmmt_op_type_in = vc WITH protect, noconstant("")
 DECLARE dmmt_priority_group = vc WITH protect, noconstant("")
 DECLARE dmmt_ndx = i4 WITH protect, noconstant(0)
 DECLARE dmmt_from_priority = i4 WITH protect, noconstant(0)
 DECLARE dmmt_to_priority = i4 WITH protect, noconstant(0)
 DECLARE dmmt_standalone = i2 WITH protect, noconstant(0)
 DECLARE dmmt_force_reset = i2 WITH protect, noconstant(0)
 DECLARE drie_msg = vc WITH protect, noconstant("DM2NOTSET")
 DECLARE dmmt_failed_tbl_exists = i4 WITH protect, noconstant(0)
 DECLARE dmmt_chunk_tbl_exists = i4 WITH protect, noconstant(0)
 DECLARE dmmt_auto_fail_emsg = vc WITH protect, constant(
  "<<<CER-01234: error - setting current operation as auto-fail>>>")
 DECLARE dmmt_exp_preserve = i2 WITH protect, noconstant(0)
 DECLARE drie_log_dir = vc WITH protect, noconstant("")
 SET dmmt_priority_group =  $1
 SET dmmt_ndx = locateval(dmmt_ndx,1,drr_priority_group_matrix->cnt,dmmt_priority_group,
  drr_priority_group_matrix->priority_group[dmmt_ndx].group_name)
 IF (dmmt_ndx=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Priority group ",dmmt_priority_group," is invalid.")
  SET dm_err->eproc = "Priority Group Validation"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dmmt_from_priority = drr_priority_group_matrix->priority_group[dmmt_ndx].priority_from_range
 SET dmmt_to_priority = drr_priority_group_matrix->priority_group[dmmt_ndx].priority_to_range
 SET dmmt_file_delim = "/"
 FREE RECORD dmmt_screen
 RECORD dmmt_screen(
   1 top_line = i4
   1 bottom_line = i4
   1 cur_line = i4
 ...
 FREE RECORD dmmt_screen_save
 RECORD dmmt_screen_save(
   1 top_line = i4
   1 bottom_line = i4
   1 cur_line = i4
   1 max_scroll = i4
 ...
 FREE RECORD dmmt_err_save
 RECORD dmmt_err_save(
   1 tbl_cnt = i4
   1 temp_dir = vc
   1 tbl[*]
     2 op_id = f8
 ...
 FREE RECORD dmmt_failed_ops
 RECORD dmmt_failed_ops(
   1 op_cnt = i4
   1 op[*]
     2 op_id = f8
     2 expimp_log = vc
 ...
 FREE RECORD dmmt_failed_ops_save
 RECORD dmmt_failed_ops_save(
   1 op_cnt = i4
   1 op[*]
     2 op_id = f8
     2 expimp_log = vc
 ...
 FREE RECORD dmmt_all_failed_ops
 RECORD dmmt_all_failed_ops(
   1 op_cnt = i4
   1 op[*]
     2 op_id = f8
     2 expimp_log = vc
 ...
 IF ((dm2_install_schema->run_id=0))
  IF (dmmt_prelim(null)=0)
   GO TO exit_program
  ENDIF
  SET dmmt_standalone = 1
 ENDIF
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 WHILE (dmmt_leave=0)
   SET dmmt_screen->top_line = 1
   SET dmmt_screen->cur_line = 1
   SET dmmt_screen->bottom_line = 0
   SET dmmt_screen->max_scroll = 14
   SET stat = initrec(dmmt_failed_ops)
   SET stat = initrec(dmmt_all_failed_ops)
   SET dmmt_failed_cnt = 0
   SET dm_err->eproc = "Loading failed operations."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log d
    WHERE (d.run_id=dm2_install_schema->run_id)
     AND d.priority >= dmmt_from_priority
     AND d.priority <= dmmt_to_priority
     AND d.op_type IN ("IMPORT*", "EXPORT*")
     AND ((d.status="ERROR"
     AND d.error_msg = null) OR (((d.status="ERROR"
     AND substring(1,14,d.error_msg) != "Application Id") OR (d.status="ERROR"
     AND d.error_msg != dmmt_auto_fail_emsg)) ))
    DETAIL
     dmmt_failed_tbl_exists = 0
     IF (locateval(dmmt_failed_tbl_exists,1,dmmt_failed_cnt,d.table_name,dmmt_failed_ops->op[
      dmmt_failed_tbl_exists].tbl,
      d.obj_name,dmmt_failed_ops->op[dmmt_failed_cnt].obj)=0)
      dmmt_failed_cnt = (dmmt_failed_cnt+ 1)
      IF (mod(dmmt_failed_cnt,10)=1)
       stat = alterlist(dmmt_failed_ops->op,(dmmt_failed_cnt+ 9))
      ENDIF
      dmmt_failed_ops->op[dmmt_failed_cnt].op_id = d.op_id, dmmt_failed_ops->op[dmmt_failed_cnt].cmd
       = d.operation, dmmt_failed_ops->op[dmmt_failed_cnt].tbl = d.table_name,
      dmmt_failed_ops->op[dmmt_failed_cnt].obj = d.obj_name, dmmt_failed_ops->op[dmmt_failed_cnt].
      op_type = d.op_type, dmmt_failed_ops->op[dmmt_failed_cnt].expimp_log = d.logfile_name
     ENDIF
     dmmt_all_failed_ops->op_cnt = (dmmt_all_failed_ops->op_cnt+ 1)
     IF (mod(dmmt_all_failed_ops->op_cnt,10)=1)
      stat = alterlist(dmmt_all_failed_ops->op,(dmmt_all_failed_ops->op_cnt+ 9))
     ENDIF
     dmmt_chunk_tbl_exists = dmmt_all_failed_ops->op_cnt, dmmt_all_failed_ops->op[
     dmmt_chunk_tbl_exists].op_id = d.op_id, dmmt_all_failed_ops->op[dmmt_chunk_tbl_exists].cmd = d
     .operation,
     dmmt_all_failed_ops->op[dmmt_chunk_tbl_exists].tbl = d.table_name, dmmt_all_failed_ops->op[
     dmmt_chunk_tbl_exists].obj = d.obj_name, dmmt_all_failed_ops->op[dmmt_chunk_tbl_exists].op_type
      = d.op_type,
     dmmt_all_failed_ops->op[dmmt_chunk_tbl_exists].expimp_log = d.logfile_name
     IF (findstring("EXPORT-PRESERVED",trim(d.op_type,4),1,0) > 0)
      dmmt_exp_preserve = 1
     ENDIF
    FOOT REPORT
     dmmt_failed_ops->op_cnt = dmmt_failed_cnt, stat = alterlist(dmmt_failed_ops->op,dmmt_failed_cnt),
     stat = alterlist(dmmt_all_failed_ops->op,dmmt_all_failed_ops->op_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   IF (curqual=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("There are no failed IMPORT/EXPORT operations for Run Id:",build(
      dm2_install_schema->run_id))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   IF ((dm_err->debug_flag > 721))
    CALL echo(build("dmmt_exp_preserve = ",dmmt_exp_preserve))
    CALL echorecord(dmmt_failed_ops)
    CALL echorecord(dmmt_all_failed_ops)
   ENDIF
   IF (dmmt_exp_preserve=0)
    IF (drr_load_chunk_imp_tbls("REF_DATA_LINK",1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM2_ADMIN_DM_INFO","S","ALL","")=0)
    GO TO exit_program
   ENDIF
   IF ((der_expimp_data->dp_node_type="DB"))
    IF ((der_expimp_data->tgt_temp_dir > "")
     AND (der_expimp_data->tgt_temp_dir != "DM2NOTSET"))
     SET drie_log_dir = der_expimp_data->tgt_temp_dir
    ENDIF
   ELSE
    IF ((der_expimp_data->tgt_app_temp_dir > "")
     AND (der_expimp_data->tgt_app_temp_dir != "DM2NOTSET"))
     SET drie_log_dir = der_expimp_data->tgt_app_temp_dir
    ENDIF
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(concat("drie_log_dir = ",drie_log_dir))
   ENDIF
   SET dm_err->eproc = "Getting target host_name from v$instance."
   SELECT INTO "nl:"
    FROM v$instance vi
   ...
   ENDIF
   SET dmmt_failed_cnt = 0
   FOR (dmmt_failed_cnt = 1 TO dmmt_failed_ops->op_cnt)
     IF (drie_log_dir > "")
      SET dmmt_failed_ops->op[dmmt_failed_cnt].expimp_log = build(drie_log_dir,"/",cnvtlower(
        der_expimp_data->tgt_dbname),"_exp/",dmmt_failed_ops->op[dmmt_failed_cnt].expimp_log)
     ENDIF
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat("expimp_log = ",dmmt_failed_ops->op[dmmt_failed_cnt].expimp_log))
     ENDIF
     IF (dmmt_failed_cnt=1)
      IF (drr_check_log_for_errors(0.0," ",2,dmmt_err_ret)=0)
       GO TO exit_program
      ENDIF
      IF (drr_check_log_for_errors(dmmt_failed_ops->op[dmmt_failed_cnt].op_id,dmmt_failed_ops->op[
       dmmt_failed_cnt].expimp_log,2,dmmt_err_ret)=0)
       GO TO exit_program
      ENDIF
     ELSE
      IF (drr_check_log_for_errors(dmmt_failed_ops->op[dmmt_failed_cnt].op_id,dmmt_failed_ops->op[
     ...
     IF (dmmt_err_ret=0)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = concat("OP_ID:",build(dmmt_failed_ops->op[dmmt_failed_cnt].op_id),
       " in a FAILED status, but no errors found in log.")
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_program
     ENDIF
   ENDFOR
   IF (dmmt_xfer_errors(null)=0)
    GO TO exit_program
   ENDIF
   IF ((dm_err->debug_flag > 721))
    CALL echorecord(dmmt_err)
   ENDIF
   IF (dmmt_force_reset=1)
    IF (dmmt_restart_ddl_rows(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (dmmt_driver(dmmt_leave)=0)
    GO TO exit_program
   ENDIF
   IF (dmmt_leave=1)
    GO TO exit_program
 ...
 ENDWHILE
 GO TO exit_program
 SUBROUTINE dmmt_prelim(null)
   DECLARE dp_max = f8 WITH protect, noconstant(0.0)
   IF (dm2_get_dbase_name(dmmt_dbase_name)=0)
    RETURN(0)
   ELSEIF (dmmt_dbase_name=" ")
   ...
   ELSE
    ...
    SET dm2_install_schema->src_v500_connect_str = dm2_install_schema->connect_str
   ENDIF
   IF (dm2_get_dbase_name(dmmt_src_dbase_name)=0)
    RETURN(0)
   ELSEIF (dmmt_src_dbase_name=" ")
    SET dm_err->emsg = "Source database name could not be obtained"
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   SET dm2_install_schema->dbase_name = dmmt_dbase_name
   SET dm2_install_schema->u_name = "V500"
   SET dm2_force_connect_string = 1
   EXECUTE dm2_connect_to_dbase "PC"
   SET dm2_force_connect_string = 0
   ...
   ELSE
    ...
    SET dm2_install_schema->v500_connect_str = dm2_install_schema->connect_str
   ENDIF
   IF (dmmt_src_dbase_name=dmmt_dbase_name)
    SET dm_err->emsg = concat("Source:",dmmt_src_dbase_name," and Target:",dmmt_dbase_name,
     " databases may not be the same.")
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    RETURN(0)
   ELSE
    SET dm_err->eproc = concat("Source:",dmmt_src_dbase_name," and Target:",dmmt_dbase_name)
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   ...
   SELECT INTO "nl:"
    ...
    (SELECT
     ...
     WHERE r.process_option="CLIN COPY"))
    DETAIL
     dm2_install_schema->run_id = d.run_id
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->emsg = "No CLIN COPY operation has completed in this domain, unable to run."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_view_logfile(dvl_log_in)
   FREE DEFINE rtl2
   SET logical dvl_logfile_logical dvl_log_in
   DEFINE rtl2 "dvl_logfile_logical"
   SELECT INTO mine
    logfile = t.line
    FROM rtl2t t
    WHERE t.line > " "
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drie_view_logfile2(dvl_log_dir,dvl_log)
   DECLARE dvl2_log = vc WITH protect, noconstant("")
   IF ((((der_expimp_data->dp_node_type="APP")) OR ((der_expimp_data->dp_node_type="DB")
    AND cnvtupper(curnode)=cnvtupper(der_expimp_data->tgt_db_node))) )
    SET dvl2_log = build(dvl_log_dir,dvl_log)
    FREE DEFINE rtl3
    SET logical dvl_logfile_logical dvl2_log
    DEFINE rtl3 "dvl_logfile_logical"
    SELECT INTO mine
     logfile = t.line
     FROM rtl3t t
     WITH nocounter
    ...
    ENDIF
   ELSE
    SET dvl2_log = concat(dm2_install_schema->ccluserdir,"dm2_rie_datapump_tmp.log")
    IF (dm2_findfile(dvl2_log) > 0)
     IF (dm2_push_dcl(concat("rm ",dvl2_log))=0)
      RETURN(0)
     ENDIF
    ENDIF
    SET dm2ftpr->user_name = "oracle"
    ...
    CALL dfr_add_getops_line(" "," "," "," "," ",
     1)
    CALL dfr_add_getops_line(" ",dm2_install_schema->ccluserdir,"dm2_rie_datapump_tmp.log",
     dvl_log_dir,dvl_log,
     0)
    IF (dfr_get_file(null)=0)
     RETURN(0)
    ENDIF
    IF ( NOT (dm2_findfile(dvl2_log)))
     SET dm_err->eproc = "Attempting to copy remote log file to CCLUSDERDIR:dm2_rie_datapump_tmp.log"
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "Could not find file in CCLUSERDIR."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    FREE DEFINE rtl3
    SET logical dvl_logfile_logical "dm2_rie_datapump_tmp.log"
    DEFINE rtl3 "dvl_logfile_logical"
    SELECT INTO mine
     logfile = t.line
     FROM rtl3t t
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_chk_opt_vals(dcov_all_filled_out)
   DECLARE dcov_opt_cnt = i4 WITH protect, noconstant(0)
   SET dcov_all_filled_out = 1
   FOR (dcov_opt_cnt = 1 TO dmmt_err->tbl_cnt)
     IF ((dmmt_err->tbl[dcov_opt_cnt].need_opt=1)
      AND (dmmt_err->tbl[dcov_opt_cnt].opt_chosen="-"))
      SET dcov_all_filled_out = 0
      SET dcov_opt_cnt = dmmt_err->tbl_cnt
     ENDIF
   ENDFOR
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_xfer_errors(null)
   DECLARE dxe_overall_cnt = i4 WITH protect, noconstant(0)
   DECLARE dxe_str_cnt = i4 WITH protect, noconstant(0)
   DECLARE dxe_str = vc WITH protect, noconstant("")
   DECLARE dxe_seq_cnt = i4 WITH protect, noconstant(0)
   DECLARE dxe_first = i2 WITH protect, noconstant(0)
   DECLARE dxe_ndx = i4 WITH protect, noconstant(0)
   DECLARE dxe_chunk_tbl_idx = i4 WITH protect, noconstant(0)
   SET dmmt_err->tbl_cnt = size(drr_errors_encountered->qual,5)
   SET dm_err->eproc = "Loading local structure with failed operations."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    FROM (dummyt d  WITH seq = value(drr_errors_encountered->cmd_cnt))
    DETAIL
     dxe_first = 1
     FOR (dxe_sub_cnt = 1 TO drr_errors_encountered->qual[d.seq].error_cnt)
       dxe_str = "", dxe_str_cnt = 1, dxe_seq_cnt = 0
       WHILE (dxe_str_cnt < size(trim(drr_errors_encountered->qual[d.seq].qual[dxe_sub_cnt].
         error_desc)))
         dxe_overall_cnt = (dxe_overall_cnt+ 1), stat = alterlist(dmmt_err->tbl,dxe_overall_cnt)
         IF (dxe_first=1)
          dmmt_err->tbl[dxe_overall_cnt].need_opt = 1, dmmt_err->tbl[dxe_overall_cnt].opt_chosen =
          "-", dmmt_err->tbl[dxe_overall_cnt].op_id = drr_errors_encountered->qual[d.seq].dee_op_id,
          dmmt_err->tbl[dxe_overall_cnt].cmd_log_full = drr_errors_encountered->qual[d.seq].
          logfile_name, dmmt_err->tbl[dxe_overall_cnt].cmd_log = substring((findstring(
            dmmt_file_delim,drr_errors_encountered->qual[d.seq].logfile_name,1,1)+ 1),(size(
            drr_errors_encountered->qual[d.seq].logfile_name) - findstring(dmmt_file_delim,
            drr_errors_encountered->qual[d.seq].logfile_name,1,1)),drr_errors_encountered->qual[d.seq
           ].logfile_name), dmmt_err->tbl[dxe_overall_cnt].cmd_log_dir = substring(1,findstring(
            dmmt_file_delim,dmmt_err->tbl[dxe_overall_cnt].cmd_log_full,1,1),dmmt_err->tbl[
           dxe_overall_cnt].cmd_log_full),
          dmmt_err->tbl[dxe_overall_cnt].tbl_seq = d.seq
          IF ((dmmt_err->temp_dir=""))
           dmmt_err->temp_dir = dmmt_err->tbl[dxe_overall_cnt].cmd_log_dir
          ENDIF
          IF ((dmmt_err->temp_dir=""))
           dmmt_err->temp_dir = dmmt_err->tbl[dxe_overall_cnt].cmd_log_dir
          ENDIF
          dmmt_err->tbl[dxe_overall_cnt].force_reset_ind = drr_errors_encountered->qual[d.seq].
          force_reset_ind
          IF ((drr_errors_encountered->qual[d.seq].force_reset_ind=1))
           dmmt_force_reset = 1, dmmt_err->tbl[dxe_overall_cnt].opt_chosen = "R"
          ENDIF
         ELSE
          dxe_seq_cnt = (dxe_seq_cnt+ 1), dmmt_err->tbl[dxe_overall_cnt].need_opt = 0, dxe_str =
          fillstring(40," "),
          dxe_str = substring(dxe_str_cnt,40,trim(drr_errors_encountered->qual[d.seq].qual[
            dxe_sub_cnt].error_desc)), dxe_str_cnt = (dxe_str_cnt+ 40), dmmt_err->tbl[dxe_overall_cnt
          ].op_id = drr_errors_encountered->qual[d.seq].dee_op_id,
          dmmt_err->tbl[dxe_overall_cnt].cmd_err = drr_errors_encountered->qual[d.seq].qual[
          dxe_sub_cnt].error, dmmt_err->tbl[dxe_overall_cnt].cmd_error_desc = trim(dxe_str), dmmt_err
          ->tbl[dxe_overall_cnt].cmd_error_seq = dxe_seq_cnt,
          dmmt_err->tbl[dxe_overall_cnt].tbl_seq = d.seq
         ENDIF
         dxe_first = 0, dxe_ndx = 0, dxe_ndx = locateval(dxe_ndx,1,dmmt_failed_ops->op_cnt,dmmt_err->
          tbl[dxe_overall_cnt].op_id,dmmt_failed_ops->op[dxe_ndx].op_id)
         IF (dxe_ndx > 0)
          IF (findstring("IMPORT",dmmt_failed_ops->op[dxe_ndx].op_type,1) > 0)
           dmmt_err->tbl[dxe_overall_cnt].cmd_type = "IMPORT"
          ELSE
           dmmt_err->tbl[dxe_overall_cnt].cmd_type = "EXPORT"
          ENDIF
          dmmt_err->tbl[dxe_overall_cnt].cmd = dmmt_failed_ops->op[dxe_ndx].cmd, dmmt_err->tbl[
          dxe_overall_cnt].tbl = dmmt_failed_ops->op[dxe_ndx].tbl, dmmt_err->tbl[dxe_overall_cnt].obj
           = dmmt_failed_ops->op[dxe_ndx].obj,
          dmmt_err->tbl[dxe_overall_cnt].op_type = dmmt_failed_ops->op[dxe_ndx].op_type,
          dxe_chunk_tbl_idx = 0, dxe_chunk_tbl_idx = locateval(dxe_chunk_tbl_idx,1,drr_chunk_imp_tbls
           ->tbl_cnt,dmmt_err->tbl[dxe_overall_cnt].tbl,drr_chunk_imp_tbls->tbl[dxe_chunk_tbl_idx].
           table_name)
          IF ((((((dmmt_err->tbl[dxe_overall_cnt].op_type="EXPORT/IMPORT - REFERENCE DATA (REMOTE)"))
           OR ((dmmt_err->tbl[dxe_overall_cnt].op_type="EXPORT/IMPORT - MIXED DATA (REMOTE)")))
           AND (dmmt_err->tbl[dxe_overall_cnt].obj="Split*")) OR (dxe_chunk_tbl_idx > 0)) )
           dmmt_err->tbl[dxe_overall_cnt].split_ind = 1
          ENDIF
         ENDIF
       ENDWHILE
     ENDFOR
    FOOT REPORT
     dmmt_err->tbl_cnt = size(dmmt_err->tbl,5)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SUBROUTINE dmmt_driver(dd_leave)
   ...
   ENDWHILE
   IF (dd_leave=1)
    IF ((dir_install_misc->ddl_failed_ind=1))
     SET dm_err->err_ind = 1
     SET dm_err->emsg = "User quit before all operations completed successfully."
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ENDIF
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_intermediate_work_menu(diwm_screen_return)
   DECLARE diwm_screen_return = vc WITH protect, noconstant("")
   DECLARE diwm_loop_tbl_name = vc WITH protect, noconstant("")
   DECLARE diwm_loop_op_type = vc WITH protect, noconstant("")
   DECLARE diwm_total_cnt = i4 WITH protect, noconstant(0)
   DECLARE diwm_running_cnt = i4 WITH protect, noconstant(0)
   DECLARE diwm_ready_cnt = i4 WITH protect, noconstant(0)
   DECLARE diwm_complete_cnt = i4 WITH protect, noconstant(0)
   DECLARE diwm_error_cnt = i4 WITH protect, noconstant(0)
   DECLARE diwm_op_ind = i4 WITH protect, noconstant(0)
   DECLARE diwm_op_idx = i4 WITH protect, noconstant(0)
   SET diwm_loop_tbl_name = dmmt_err->tbl[dmmt_screen->cur_line].tbl
   SET diwm_loop_op_type = dmmt_err->tbl[dmmt_screen->cur_line].op_type
   CALL clear(1,1)
   IF (dmmt_intermediate_screen_header(null)=0)
    RETURN(0)
   ENDIF
   SET display_line = 8
   IF ((dmmt_screen->cur_line <= 1))
    SET dmmt_screen->top_line = 1
    SET dmmt_screen->cur_line = 1
    SET dmmt_screen->bottom_line = 0
    SET dmmt_screen->max_scroll = 14
   ELSE
    SET dmmt_screen->top_line = dmmt_screen->top_line
    SET dmmt_screen->cur_line = dmmt_screen->cur_line
    SET dmmt_screen->bottom_line = dmmt_screen->bottom_line
    SET dmmt_screen->max_scroll = 14
   ENDIF
   IF (dmmt_fill_intermediate_screen(null)=0)
    RETURN(0)
   ENDIF
   SET diwm_screen_return = ""
   CASE (dmmt_intermediate_scroll("dmmt_screen"))
    OF "Q":
     SET diwm_screen_return = "QUIT"
     SET dmmt_screen->top_line = dmmt_screen_save->top_line
     SET dmmt_screen->cur_line = dmmt_screen_save->cur_line
     SET dmmt_screen->bottom_line = dmmt_screen_save->bottom_line
     SET dmmt_screen->max_scroll = 14
     SET dmmt_screen->max_value = dmmt_screen_save->max_value
     CALL dmmt_err_save_rs(0)
     CALL dmmt_failed_ops_save_rs(0)
    OF "V":
     IF (drie_view_logfile2(dmmt_err->tbl[dmmt_screen->cur_line].cmd_log_dir,dmmt_err->tbl[
      dmmt_screen->cur_line].cmd_log)=0)
      SET drie_msg = "Unable to access log file."
      SET dm_err->err_ind = 0
      SET dm_err->emsg = ""
     ENDIF
     CALL clear(1,1)
    ...
    OF "R":
     ...
     ENDIF
     SET stat = initrec(dmmt_failed_ops)
     SET dm_err->eproc = concat("Fetching the updated statuses of the import operations for ",
      diwm_loop_tbl_name," table")
     CALL disp_msg("",dm_err->logfile,0)
     SELECT INTO "nl:"
      FROM dm2_ddl_ops_log ddol
      WHERE (ddol.run_id=dm2_install_schema->run_id)
       AND ddol.table_name=diwm_loop_tbl_name
       AND ddol.op_type=diwm_loop_op_type
       AND ddol.status="ERROR"
       AND ((ddol.error_msg != dmmt_auto_fail_emsg) OR (substring(1,14,ddol.error_msg) !=
      "Application Id"
       AND ddol.ready_to_run_ind=1))
      DETAIL
       IF (locateval(diwm_op_ind,1,dmmt_failed_ops->op_cnt,ddol.op_id,dmmt_failed_ops->op[diwm_op_ind
        ].op_id)=0)
        dmmt_failed_ops->op_cnt = (dmmt_failed_ops->op_cnt+ 1)
        IF (mod(dmmt_failed_ops->op_cnt,10)=1)
         stat = alterlist(dmmt_failed_ops->op,(dmmt_failed_ops->op_cnt+ 9))
        ENDIF
        diwm_op_idx = dmmt_failed_ops->op_cnt, dmmt_failed_ops->op[diwm_op_idx].op_id = ddol.op_id,
        dmmt_failed_ops->op[diwm_op_idx].cmd = ddol.operation,
        dmmt_failed_ops->op[diwm_op_idx].tbl = ddol.table_name, dmmt_failed_ops->op[diwm_op_idx].obj
         = ddol.obj_name, dmmt_failed_ops->op[diwm_op_idx].op_type = ddol.op_type
        IF (drie_log_dir > "")
         dmmt_failed_ops->op[diwm_op_idx].expimp_log = build(drie_log_dir,"/",cnvtlower(
           der_expimp_data->tgt_dbname),"_exp/",ddol.logfile_name)
        ENDIF
       ELSE
        dmmt_failed_ops->op[diwm_op_ind].op_id = ddol.op_id, dmmt_failed_ops->op[diwm_op_ind].cmd =
        ddol.operation, dmmt_failed_ops->op[diwm_op_ind].tbl = ddol.table_name,
        dmmt_failed_ops->op[diwm_op_ind].obj = ddol.obj_name, dmmt_failed_ops->op[diwm_op_ind].
        op_type = ddol.op_type, dmmt_failed_ops->op[diwm_op_ind].expimp_log = ddol.logfile_name
       ENDIF
      FOOT REPORT
       stat = alterlist(dmmt_failed_ops->op,dmmt_failed_ops->op_cnt)
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc)=1)
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      RETURN(0)
     ENDIF
     CALL dmmt_screen_refresh(null)
    OF "F":
     IF (dmmt_fail_tbl_import_ops(dm2_install_schema->run_id,dmmt_err->tbl[dmmt_screen->cur_line].tbl,
      dmmt_err->tbl[dmmt_screen->cur_line].op_type)=0)
      RETURN(0)
     ENDIF
   ENDCASE
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_screen_header(null)
   CALL clear(1,1)
   CALL text(1,1,concat("*** ",dmmt_err->tbl[1].cmd_type," Retry Screen ***"))
   CALL text(2,2,concat("The following ",dmmt_err->tbl[1].cmd_type,
     "(s) had errors.  Please correct before continuing. "))
   CALL text(3,2,concat("To restart/success ",dmmt_err->tbl[1].cmd_type,
     "(s) again, update the Action column appropriately."))
   CALL text(6,4,"Action       Table/Object Name              Log File Name              Error")
   CALL line(7,1,132,horizontal)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_intermediate_screen_header(null)
   DECLARE dish_loop_tbl_name = vc WITH protect, noconstant("")
   DECLARE dish_loop_op_type = vc WITH protect, noconstant("")
   DECLARE dish_total_cnt = i4 WITH protect, noconstant(0)
   DECLARE dish_running_cnt = i4 WITH protect, noconstant(0)
   DECLARE dish_ready_cnt = i4 WITH protect, noconstant(0)
   DECLARE dish_complete_cnt = i4 WITH protect, noconstant(0)
   DECLARE dish_error_cnt = i4 WITH protect, noconstant(0)
   SET dish_loop_tbl_name = dmmt_err->tbl[dmmt_screen->cur_line].tbl
   SET dish_loop_op_type = dmmt_err->tbl[dmmt_screen->cur_line].op_type
   CALL clear(1,1)
   CALL text(1,1,concat("*** Listing Failed Splits for ",dish_loop_tbl_name," ***"))
   IF (dmmt_op_status_cnts(dish_loop_tbl_name,dish_loop_op_type,dish_total_cnt,dish_running_cnt,
    dish_ready_cnt,
    dish_complete_cnt,dish_error_cnt)=0)
    RETURN(0)
   ENDIF
   CALL text(3,2,build("Split Info => Total: ",dish_total_cnt,"; Ready: ",dish_ready_cnt,
     "; Complete: ",
     dish_complete_cnt,"; Running: ",dish_running_cnt,"; Error: ",dish_error_cnt))
   CALL text(6,4,"Split Name              Log File Name              Error")
   CALL line(7,1,132,horizontal)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_work_menu(dwm_screen_return)
   DECLARE dwm_continue = i2 WITH protect, noconstant(0)
   DECLARE dwm_op_ind = i4 WITH protect, noconstant(0)
   DECLARE dwm_screen_return = vc WITH protect, noconstant("")
   DECLARE dwm_intermediate_screen_return = vc WITH protect, noconstant("")
   DECLARE dwm_restart_eligible_ind = i2 WITH protect, noconstant(0)
   IF (dmmt_exp_preserve=0)
    IF (drr_load_chunk_imp_tbls("REF_DATA_LINK",1)=0)
     RETURN(0)
    ENDIF
   ...
   ELSE
    ...
    SET dmmt_screen->max_scroll = 14
   ENDIF
   IF (fill_dmmt_screen(null)=0)
    RETURN(0)
   ENDIF
   SET dwm_screen_return = ""
   CASE (dmmt_scroll("dmmt_screen"))
    OF "Q":
     SET dwm_screen_return = "QUIT"
    OF "R":
     SET dwm_restart_eligible_ind = 0
     IF ((dmmt_err->tbl[dmmt_screen->cur_line].split_ind=1))
      IF (dmmt_chk_tbl_eligible_restart(dm2_install_schema->run_id,dmmt_err->tbl[dmmt_screen->
       cur_line].tbl,dmmt_err->tbl[dmmt_screen->cur_line].op_type,dwm_restart_eligible_ind)=0)
       RETURN(0)
      ENDIF
     ENDIF
     IF ((((dmmt_err->tbl[dmmt_screen->cur_line].split_ind=0)) OR (dwm_restart_eligible_ind=1)) )
      SET dmmt_err->tbl[dmmt_screen->cur_line].opt_chosen = "R"
     ELSEIF (dwm_restart_eligible_ind=0)
      CALL text(4,2,concat(dmmt_err->tbl[dmmt_screen->cur_line].tbl,
        " table cannot be restarted due to running/yet-to-run operations."))
     ENDIF
    OF "S":
     SET dmmt_err->tbl[dmmt_screen->cur_line].opt_chosen = "S"
    OF "C":
     IF (dmmt_restart_ddl_rows(null)=0)
      RETURN(0)
     ENDIF
     SET dwm_screen_return = "CONTINUE"
    OF "V":
     CALL echorecord(dmmt_err)
     IF ((dmmt_err->tbl[dmmt_screen->cur_line].split_ind=1))
      IF (dmmt_split_tbls_retry_screen(null)=0)
       RETURN(0)
      ENDIF
     ELSE
      IF (drie_view_logfile2(dmmt_err->tbl[dmmt_screen->cur_line].cmd_log_dir,dmmt_err->tbl[
     ...
     ENDIF
    OF "T":
     FOR (drdr_cnt = 1 TO dmmt_err->tbl_cnt)
       IF ((dmmt_err->tbl[drdr_cnt].need_opt=1))
        SET dwm_restart_eligible_ind = 0
        IF ((dmmt_err->tbl[drdr_cnt].split_ind=1))
         IF (dmmt_chk_tbl_eligible_restart(dm2_install_schema->run_id,dmmt_err->tbl[drdr_cnt].tbl,
          dmmt_err->tbl[drdr_cnt].op_type,dwm_restart_eligible_ind)=0)
          RETURN(0)
         ENDIF
        ENDIF
        IF ((((dmmt_err->tbl[drdr_cnt].split_ind=0)) OR (dwm_restart_eligible_ind=1)) )
         SET dmmt_err->tbl[drdr_cnt].opt_chosen = "R"
        ELSEIF (dwm_restart_eligible_ind=0)
         CALL text(4,2,concat(dmmt_err->tbl[drdr_cnt].tbl,
           " table cannot be restarted due to running/yet-to-run operations."))
        ENDIF
       ENDIF
     ENDFOR
    OF "A":
     FOR (drdr_cnt = 1 TO dmmt_err->tbl_cnt)
       IF ((dmmt_err->tbl[drdr_cnt].need_opt=1))
        SET dmmt_err->tbl[drdr_cnt].opt_chosen = "S"
       ENDIF
     ENDFOR
    OF "F":
     IF (dmmt_fail_tbl_import_ops(dm2_install_schema->run_id,dmmt_err->tbl[dmmt_screen->cur_line].tbl,
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_restart_ddl_rows(null)
   DECLARE drdr_cnt = i4 WITH protect, noconstant(0)
   DECLARE drdr_status = vc WITH protect, noconstant("")
   DECLARE drdr_create_truncates = i4 WITH protect, noconstant(0)
   DECLARE drdr_chunk_tbl_idx = i4 WITH protect, noconstant(0)
   DECLARE drdr_tbl_idx = i4 WITH protect, noconstant(0)
   DECLARE drdr_tbl_exists_idx = i4 WITH protect, noconstant(0)
   DECLARE drdr_retry_idx = i4 WITH protect, noconstant(0)
   DECLARE drdr_drie_idx = i4 WITH protect, noconstant(0)
   DECLARE drdr_ddol_error_cnt = i4 WITH protect, noconstant(0)
   SET drr_retry_imp_data->create_chunk_cmds = 0
   FOR (drdr_cnt = 1 TO dmmt_err->tbl_cnt)
     IF ((dmmt_err->tbl[drdr_cnt].need_opt=1))
      IF ((dmmt_err->tbl[drdr_cnt].opt_chosen="S"))
       SET dm_err->eproc = concat("Marking row as AUTO-SUCCESS for op_id ",build(dmmt_err->tbl[
         drdr_cnt].op_id))
       UPDATE  FROM dm2_ddl_ops_log l
        SET l.status = "COMPLETE", l.error_msg = "User chose to AUTO-SUCCESS operation", l.appl_id =
         dm2_install_schema->appl_id
        WHERE (l.op_id=dmmt_err->tbl[drdr_cnt].op_id)
         AND l.status="ERROR"
        WITH nocounter
       ;end update
       IF (check_error(dm_err->eproc)=1)
   ...
   FOR (drdr_cnt = 1 TO dmmt_err->tbl_cnt)
     IF ((dmmt_err->tbl[drdr_cnt].need_opt=1))
      IF ((dmmt_err->tbl[drdr_cnt].opt_chosen="R"))
       SET drdr_chunk_tbl_idx = 0
       IF ((dmmt_err->tbl[drdr_cnt].split_ind=1))
        SET drdr_chunk_tbl_idx = locateval(drdr_chunk_tbl_idx,1,drr_chunk_imp_tbls->tbl_cnt,dmmt_err
         ->tbl[drdr_cnt].tbl,drr_chunk_imp_tbls->tbl[drdr_chunk_tbl_idx].table_name)
        IF (drdr_chunk_tbl_idx=0)
         SET dm_err->eproc = concat("Verifying if chunk metadata exists for ",dmmt_err->tbl[drdr_cnt]
          .tbl," table.")
         SET dm_err->emsg = concat("Chunk metadata not found for ",dmmt_err->tbl[drdr_cnt].tbl,
          " table.")
         SET dm_err->err_ind = 1
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         RETURN(0)
        ELSE
         SET dm_err->eproc = concat("Verifying if there are any ddl operations of ",dmmt_err->tbl[
          drdr_cnt].tbl," are yet to run or still running.")
         SELECT INTO "nl:"
          FROM dm2_ddl_ops_log ddol
          WHERE (ddol.run_id=dm2_install_schema->run_id)
           AND (ddol.table_name=dmmt_err->tbl[drdr_cnt].tbl)
           AND (ddol.op_type=dmmt_err->tbl[drdr_cnt].op_type)
           AND ddol.status="ERROR"
           AND ddol.ready_to_run_ind=1
          DETAIL
           drdr_tbl_exists_idx = 0
           IF (locateval(drdr_tbl_exists_idx,1,drr_retry_imp_data->tbl_cnt,dmmt_err->tbl[drdr_cnt].
            tbl,drr_retry_imp_data->tbl[drdr_tbl_exists_idx].table_name)=0)
            drr_retry_imp_data->tbl_cnt = (drr_retry_imp_data->tbl_cnt+ 1), stat = alterlist(
             drr_retry_imp_data->tbl,drr_retry_imp_data->tbl_cnt), drdr_tbl_idx = drr_retry_imp_data
            ->tbl_cnt,
            drr_retry_imp_data->tbl[drdr_tbl_idx].owner = drr_chunk_imp_tbls->tbl[drdr_chunk_tbl_idx]
            .owner, drr_retry_imp_data->tbl[drdr_tbl_idx].table_name = dmmt_err->tbl[drdr_cnt].tbl,
            drr_retry_imp_data->tbl[drdr_tbl_idx].op_type = dmmt_err->tbl[drdr_cnt].op_type
           ENDIF
          WITH nocounter
         ;end select
         IF (check_error(dm_err->eproc)=1)
          ...
          RETURN(0)
         ENDIF
         IF ((drr_retry_imp_data->tbl_cnt > 0))
          SET drr_retry_imp_data->create_chunk_cmds = 1
         ENDIF
        ENDIF
       ELSE
        SET dm_err->eproc = concat("Resetting row for op_id ",build(dmmt_err->tbl[drdr_cnt].op_id))
        IF ((dm_err->debug_flag > 0))
         CALL disp_msg(" ",dm_err->logfile,0)
        ENDIF
        UPDATE  FROM dm2_ddl_ops_log l
         SET l.status = null, l.error_msg = "", l.parent_execution_order = evaluate(dmmt_err->tbl[
           drdr_cnt].force_reset_ind,1,3,l.parent_execution_order)
         WHERE (l.op_id=dmmt_err->tbl[drdr_cnt].op_id)
          AND l.status="ERROR"
         WITH nocounter
        ;end update
   ...
   ENDFOR
   IF ((dm_err->debug_flag > 721))
    CALL echorecord(drr_retry_imp_data)
   ENDIF
   IF ((drr_retry_imp_data->create_chunk_cmds=1))
    SET dm2_install_schema->appl_id = currdbhandle
    EXECUTE dm2_create_ddl
    IF ((dm_err->err_ind=1))
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE fill_dmmt_screen(null)
   DECLARE fds_screen_cnt = i4 WITH protect, noconstant(0)
   DECLARE fds_first = i2 WITH protect, noconstant(0)
   DECLARE fds_text = vc WITH protect, noconstant("")
   DECLARE fds_running_cnt = i4 WITH protect, noconstant(0)
   DECLARE fds_ready_cnt = i4 WITH protect, noconstant(0)
   DECLARE fds_complete_cnt = i4 WITH protect, noconstant(0)
   DECLARE fds_error_cnt = i4 WITH protect, noconstant(0)
   DECLARE fds_total_cnt = i4 WITH protect, noconstant(0)
   SET display_line = 8
   SET dmmt_screen->max_scroll = 14
   IF (((dmmt_screen->top_line+ 14) > dmmt_screen->max_value))
    SET dmmt_screen->bottom_line = dmmt_screen->max_value
   ELSE
    SET dmmt_screen->bottom_line = (dmmt_screen->top_line+ 13)
   ENDIF
   FOR (display_loop = dmmt_screen->top_line TO dmmt_screen->bottom_line)
     IF ((dmmt_err->tbl[display_loop].split_ind=1))
      SET dm_err->eproc = "Checking the splits' status from dm2_ddl_ops_log"
      IF (dmmt_op_status_cnts(dmmt_err->tbl[display_loop].tbl,dmmt_err->tbl[display_loop].op_type,
       fds_total_cnt,fds_running_cnt,fds_ready_cnt,
       fds_complete_cnt,fds_error_cnt)=0)
       RETURN(0)
      ENDIF
     ENDIF
     IF ((display_loop=dmmt_screen->cur_line)
      AND (dmmt_err->tbl[display_loop].need_opt=1))
      CALL video(r)
     ELSE
      CALL video(n)
     ENDIF
     CALL clear(display_line,1,132)
     IF ((dmmt_err->tbl[display_loop].need_opt=1))
      IF ((dmmt_err->tbl[display_loop].tbl != "-1"))
       SET fds_text = dmmt_err->tbl[display_loop].tbl
      ELSEIF ((dmmt_err->tbl[display_loop].obj != "-1"))
       SET fds_text = dmmt_err->tbl[display_loop].obj
      ELSE
       SET fds_text = replace(dmmt_err->tbl[display_loop].op_type,"(REMOTE)","",2)
       SET fds_text = replace(fds_text,"EXPORT -","",1)
       SET fds_text = replace(fds_text,"IMPORT -","",1)
       SET fds_text = trim(replace(fds_text,"EXPORT/IMPORT -","",1),3)
      ENDIF
      CALL text(display_line,1,cnvtstring(dmmt_err->tbl[display_loop].tbl_seq))
      CALL text(display_line,17,fds_text)
      IF ((dmmt_err->tbl[display_loop].split_ind != 1))
       IF (size(dmmt_err->tbl[display_loop].cmd_log_full) < 66)
        CALL text(display_line,48,dmmt_err->tbl[display_loop].cmd_log_full)
       ELSE
        CALL text(display_line,48,dmmt_err->tbl[display_loop].cmd_log)
       ENDIF
      ELSE
       CALL text(display_line,48,build("Split Info:  Total: ",fds_total_cnt,"; Ready: ",fds_ready_cnt,
         "; Complete: ",
         fds_complete_cnt,"; Running: ",fds_running_cnt,"; Error: ",fds_error_cnt))
      ENDIF
      CALL text(display_line,4,dmmt_err->tbl[display_loop].opt_chosen)
     ELSE
      IF ((dmmt_err->tbl[display_loop].cmd_error_seq=1))
       CALL text(display_line,75,dmmt_err->tbl[display_loop].cmd_err)
      ENDIF
      CALL text(display_line,86,dmmt_err->tbl[display_loop].cmd_error_desc)
     ENDIF
   ...
   ENDIF
   CALL video(n)
   IF ((dmmt_screen->max_value > dmmt_screen->bottom_line)
    AND (dmmt_screen->top_line=1))
    CALL clear(23,90,42)
    CALL text(23,95,"(Reached top, use <PgDown> to scroll)")
   ELSEIF ((dmmt_screen->max_value > dmmt_screen->bottom_line))
    CALL clear(23,90,42)
    CALL text(23,90,"(Use <Up>/<Down> keys to scroll)")
   ELSE
    CALL clear(23,90,42)
    CALL text(23,95,"(Reached bottom, use <PgUp> to scroll)")
   ENDIF
 END ;Subroutine
 SUBROUTINE dmmt_op_status_cnts(dosc_tbl_name,dosc_op_type,dosc_total_cnt,dosc_running_cnt,
  dosc_ready_cnt,dosc_complete_cnt,dosc_error_cnt)
   SET dm_err->eproc = concat(
    "Querying dm2_ddl_ops_log to fetch the statuses of the import operations for ",dosc_tbl_name,
    " table")
   CALL disp_msg("",dm_err->logfile,0)
   SET dosc_total_cnt = 0
   SET dosc_running_cnt = 0
   SET dosc_ready_cnt = 0
   SET dosc_complete_cnt = 0
   SET dosc_error_cnt = 0
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE (ddol.run_id=dm2_install_schema->run_id)
     AND ddol.table_name=dosc_tbl_name
     AND ddol.op_type=dosc_op_type
    DETAIL
     dosc_total_cnt = (dosc_total_cnt+ 1)
     IF (ddol.status="RUNNING")
      dosc_running_cnt = (dosc_running_cnt+ 1)
     ELSEIF (ddol.status=null)
      dosc_ready_cnt = (dosc_ready_cnt+ 1)
     ELSEIF (ddol.status="COMPLETE")
      dosc_complete_cnt = (dosc_complete_cnt+ 1)
     ELSEIF (ddol.status="ERROR")
      dosc_error_cnt = (dosc_error_cnt+ 1)
     ENDIF
    WITH nocounter
   ;end select
   IF ((dm_err->debug_flag > 721))
    CALL echo(build("dosc_tbl_name:",dosc_tbl_name))
    CALL echo(build("dosc_total_cnt:",dosc_total_cnt))
    CALL echo(build("dosc_running_cnt:",dosc_running_cnt))
    CALL echo(build("dosc_ready_cnt:",dosc_ready_cnt))
    CALL echo(build("dosc_complete_cnt:",dosc_complete_cnt))
    CALL echo(build("dosc_error_cnt:",dosc_error_cnt))
   ENDIF
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_scroll(ds_rec_str)
   ...
   DECLARE ds_valid_value = i4 WITH protect, noconstant(0)
   ...
   IF (dmmt_chk_opt_vals(ds_disp_cont)=0)
    RETURN(0)
   ENDIF
   WHILE (ds_scroll_accept=" ")
     CALL line(22,1,132,hor)
     CALL clear(23,1,94)
     CALL clear(24,1,120)
     CALL clear(4,1,132)
     IF ((dmmt_err->tbl[dmmt_screen->cur_line].split_ind != 1))
      SET dm_err->eproc = concat("*****Chunk metadata not found for table: ",dmmt_err->tbl[
       dmmt_screen->cur_line].tbl,"*****")
      SET dwm_disp_txt = dm_err->eproc
      CALL text(23,1,"(V)iew Log,(C)ontinue,(Q)uit,(R)estart,(S)uccess,Restar(T) All,Success (A)ll:")
     ELSE
      CALL text(23,1,
       "(V)iew Logs,(C)ontinue,(Q)uit,(R)estart,(S)uccess,Restar(T) All,Success (A)ll,(F)ail Table:")
     ENDIF
     IF (dmmt_force_reset=1)
      CALL text(24,2,concat("NOTE: Those imports using pipes that failed with a EXP-00002 ",
        "have automatically been reset to use a dmp file."))
     ENDIF
     IF (drie_msg != "DM2NOTSET")
      CALL text(24,2,concat("WARNING: ",drie_msg))
     ENDIF
     SET drie_msg = "DM2NOTSET"
     SET ds_valid_value = 0
     IF ((dmmt_err->tbl[dmmt_screen->cur_line].split_ind != 1))
      CALL accept(23,79,"p;cus"," "
       WHERE curaccept IN ("R", "C", "S", "Q", "V",
       "T", "A"))
     ELSE
      CALL accept(23,93,"p;cus"," "
       WHERE curaccept IN ("R", "C", "S", "Q", "V",
       "T", "A", "F"))
     ENDIF
     CASE (curscroll)
      ...
      OF 1:
       WHILE (ds_valid_value=0)
         IF (ds_cur_line=ds_bottom_line)
          SET ds_valid_value = 1
         ...
         IF ((dmmt_err->tbl[dmmt_screen->cur_line].need_opt=1))
          SET ds_valid_value = 1
         ENDIF
         IF (ds_valid_value=1)
          CALL parser(concat("call fill_",ds_rec_str,"(1) go"))
         ENDIF
       ENDWHILE
      OF 2:
       WHILE (ds_valid_value=0)
   ...
   IF ((dmmt_err->tbl[dmmt_screen->cur_line].need_opt=1))
    CASE (curaccept)
     ...
     OF "Q":
      SET ds_scroll_accept = "Q"
     OF "C":
      SET ds_scroll_accept = "C"
     OF "V":
      SET ds_scroll_accept = "V"
     OF "T":
      SET ds_scroll_accept = "T"
     OF "A":
      SET ds_scroll_accept = "A"
     OF "F":
      SET ds_scroll_accept = "F"
    ENDCASE
   ENDIF
   RETURN(ds_scroll_accept)
 END ;Subroutine
 SUBROUTINE dmmt_fill_intermediate_screen(null)
   DECLARE fds_screen_cnt = i4 WITH protect, noconstant(0)
   DECLARE fds_first = i2 WITH protect, noconstant(0)
   DECLARE fds_text = vc WITH protect, noconstant("")
   DECLARE dfis_failed_tbl_idx = i4 WITH protect, noconstant(0)
   SET display_line = 8
   SET dmmt_screen->max_scroll = 14
   IF (((dmmt_screen->top_line+ 14) > dmmt_screen->max_value))
    SET dmmt_screen->bottom_line = dmmt_screen->max_value
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE dmmt_intermediate_scroll(ds_rec_str)
   DECLARE ds_new_start = i4 WITH protect, noconstant(0)
   DECLARE ds_top_line = i4 WITH protect, noconstant(0)
   DECLARE ds_bottom_line = i4 WITH protect, noconstant(0)
   DECLARE ds_max_value = i4 WITH protect, noconstant(0)
   ...
   WHILE (ds_scroll_accept=" ")
     ...
     CALL clear(23,1,94)
     CALL clear(24,1,120)
     CALL text(23,2,"(V)iew Log, (Q)uit, (R)efresh, (F)ail Table:")
     IF (dmmt_force_reset=1)
      CALL text(24,2,concat("NOTE: Those imports using pipes that failed with a EXP-00002 ",
        "have automatically been reset to use a dmp file."))
     ENDIF
     ...
     SET drie_msg = "DM2NOTSET"
     SET ds_valid_value = 0
     CALL accept(23,47,"p;cus"," "
      WHERE curaccept IN ("V", "Q", "R", "F"))
     CASE (curscroll)
      OF 0:
       SET ds_scroll_accept = curaccept
      OF 1:
       WHILE (ds_valid_value=0)
         ...
         ENDIF
         IF (ds_valid_value=1)
          CALL parser(concat("call dmmt_fill_intermediate_screen(1) go"))
         ENDIF
       ENDWHILE
      OF 2:
       WHILE (ds_valid_value=0)
   ...
   RETURN(ds_scroll_accept)
 END ;Subroutine
 SUBROUTINE dmmt_err_save_rs(desr_save_ind)
   DECLARE desr_loop_var = i4 WITH protect, noconstant(0)
   IF (desr_save_ind=1)
    SET dm_err->eproc =
    "Saving the current state of dmmt_err to dmmt_err_save to be loaded back later"
    CALL disp_msg("",dm_err->logfile,0)
    IF ((dmmt_err->tbl_cnt=0))
     SET dm_err->eproc = "dmmt_err is empty. Nothing to save. Skipping."
     CALL disp_msg("",dm_err->logfile,0)
     RETURN(1)
    ENDIF
    SET stat = initrec(dmmt_err_save)
    SET dmmt_err_save->temp_dir = dmmt_err->temp_dir
    SET dmmt_err_save->tbl_cnt = dmmt_err->tbl_cnt
    SET stat = alterlist(dmmt_err_save->tbl,dmmt_err_save->tbl_cnt)
    FOR (desr_loop_var = 1 TO dmmt_err->tbl_cnt)
      SET dmmt_err_save->tbl[desr_loop_var].op_id = dmmt_err->tbl[desr_loop_var].op_id
      SET dmmt_err_save->tbl[desr_loop_var].tbl_seq = dmmt_err->tbl[desr_loop_var].tbl_seq
      SET dmmt_err_save->tbl[desr_loop_var].need_opt = dmmt_err->tbl[desr_loop_var].need_opt
      SET dmmt_err_save->tbl[desr_loop_var].opt_chosen = dmmt_err->tbl[desr_loop_var].opt_chosen
      SET dmmt_err_save->tbl[desr_loop_var].cmd_log = dmmt_err->tbl[desr_loop_var].cmd_log
      SET dmmt_err_save->tbl[desr_loop_var].cmd_log_full = dmmt_err->tbl[desr_loop_var].cmd_log_full
      SET dmmt_err_save->tbl[desr_loop_var].cmd_log_dir = dmmt_err->tbl[desr_loop_var].cmd_log_dir
      SET dmmt_err_save->tbl[desr_loop_var].tbl = dmmt_err->tbl[desr_loop_var].tbl
      SET dmmt_err_save->tbl[desr_loop_var].obj = dmmt_err->tbl[desr_loop_var].obj
      SET dmmt_err_save->tbl[desr_loop_var].cmd = dmmt_err->tbl[desr_loop_var].cmd
      SET dmmt_err_save->tbl[desr_loop_var].cmd_type = dmmt_err->tbl[desr_loop_var].cmd_type
      SET dmmt_err_save->tbl[desr_loop_var].cmd_err = dmmt_err->tbl[desr_loop_var].cmd_err
      SET dmmt_err_save->tbl[desr_loop_var].op_type = dmmt_err->tbl[desr_loop_var].op_type
      SET dmmt_err_save->tbl[desr_loop_var].cmd_error_desc = dmmt_err->tbl[desr_loop_var].
      cmd_error_desc
      SET dmmt_err_save->tbl[desr_loop_var].cmd_error_seq = dmmt_err->tbl[desr_loop_var].
      cmd_error_seq
      SET dmmt_err_save->tbl[desr_loop_var].force_reset_ind = dmmt_err->tbl[desr_loop_var].
      force_reset_ind
      SET dmmt_err_save->tbl[desr_loop_var].split_ind = dmmt_err->tbl[desr_loop_var].split_ind
    ENDFOR
   ELSE
    SET dm_err->eproc = "Reverting the screen state from dmmt_err_save to dmmt_err"
    CALL disp_msg("",dm_err->logfile,0)
    IF ((dmmt_err->tbl_cnt=0))
     SET dm_err->eproc = "dmmt_err_save is empty. Nothing to revert to. Skipping."
     CALL disp_msg("",dm_err->logfile,0)
     RETURN(1)
    ENDIF
    SET stat = initrec(dmmt_err)
    SET dmmt_err->temp_dir = dmmt_err_save->temp_dir
    SET dmmt_err->tbl_cnt = dmmt_err_save->tbl_cnt
    SET stat = alterlist(dmmt_err->tbl,dmmt_err->tbl_cnt)
    FOR (desr_loop_var = 1 TO dmmt_err_save->tbl_cnt)
      SET dmmt_err->tbl[desr_loop_var].op_id = dmmt_err_save->tbl[desr_loop_var].op_id
      SET dmmt_err->tbl[desr_loop_var].tbl_seq = dmmt_err_save->tbl[desr_loop_var].tbl_seq
      SET dmmt_err->tbl[desr_loop_var].need_opt = dmmt_err_save->tbl[desr_loop_var].need_opt
      SET dmmt_err->tbl[desr_loop_var].opt_chosen = dmmt_err_save->tbl[desr_loop_var].opt_chosen
      SET dmmt_err->tbl[desr_loop_var].cmd_log = dmmt_err_save->tbl[desr_loop_var].cmd_log
      SET dmmt_err->tbl[desr_loop_var].cmd_log_full = dmmt_err_save->tbl[desr_loop_var].cmd_log_full
      SET dmmt_err->tbl[desr_loop_var].cmd_log_dir = dmmt_err_save->tbl[desr_loop_var].cmd_log_dir
      SET dmmt_err->tbl[desr_loop_var].tbl = dmmt_err_save->tbl[desr_loop_var].tbl
      SET dmmt_err->tbl[desr_loop_var].obj = dmmt_err_save->tbl[desr_loop_var].obj
      SET dmmt_err->tbl[desr_loop_var].cmd = dmmt_err_save->tbl[desr_loop_var].cmd
      SET dmmt_err->tbl[desr_loop_var].cmd_type = dmmt_err_save->tbl[desr_loop_var].cmd_type
      SET dmmt_err->tbl[desr_loop_var].cmd_err = dmmt_err_save->tbl[desr_loop_var].cmd_err
      SET dmmt_err->tbl[desr_loop_var].op_type = dmmt_err_save->tbl[desr_loop_var].op_type
      SET dmmt_err->tbl[desr_loop_var].cmd_error_desc = dmmt_err_save->tbl[desr_loop_var].
      cmd_error_desc
      SET dmmt_err->tbl[desr_loop_var].cmd_error_seq = dmmt_err_save->tbl[desr_loop_var].
      cmd_error_seq
      SET dmmt_err->tbl[desr_loop_var].force_reset_ind = dmmt_err_save->tbl[desr_loop_var].
      force_reset_ind
      SET dmmt_err->tbl[desr_loop_var].split_ind = dmmt_err_save->tbl[desr_loop_var].split_ind
    ENDFOR
   ENDIF
   IF ((dm_err->debug_flag > 721))
    CALL echorecord(dmmt_err)
    CALL echorecord(dmmt_err_save)
   ENDIF
 END ;Subroutine
 SUBROUTINE dmmt_failed_ops_save_rs(dfosr_save_ind)
   DECLARE dfosr_loop_var = i4 WITH protect, noconstant(0)
   DECLARE dfosr_op_exists = i4 WITH protect, noconstant(0)
   DECLARE dfosr_tbl_op_cnt = i4 WITH protect, noconstant(0)
   DECLARE dfosr_start_ind = i4 WITH protect, noconstant(0)
   IF (dfosr_save_ind=1)
    SET dm_err->eproc =
    "Saving the current state of dmmt_failed_ops to dmmt_failed_ops_save to be loaded back later"
    CALL disp_msg("",dm_err->logfile,0)
    IF ((dmmt_failed_ops->op_cnt=0))
     SET dm_err->eproc = "dmmt_failed_ops is empty. Nothing to save. Skipping."
     CALL disp_msg("",dm_err->logfile,0)
     RETURN(1)
    ENDIF
    SET stat = initrec(dmmt_failed_ops_save)
    SET dmmt_failed_ops_save->op_cnt = dmmt_failed_ops->op_cnt
    SET stat = alterlist(dmmt_failed_ops_save->op,dmmt_failed_ops_save->op_cnt)
    FOR (dfosr_loop_var = 1 TO dmmt_failed_ops->op_cnt)
      SET dmmt_failed_ops_save->op[dfosr_loop_var].op_id = dmmt_failed_ops->op[dfosr_loop_var].op_id
      SET dmmt_failed_ops_save->op[dfosr_loop_var].cmd = dmmt_failed_ops->op[dfosr_loop_var].cmd
      SET dmmt_failed_ops_save->op[dfosr_loop_var].tbl = dmmt_failed_ops->op[dfosr_loop_var].tbl
      SET dmmt_failed_ops_save->op[dfosr_loop_var].obj = dmmt_failed_ops->op[dfosr_loop_var].obj
      SET dmmt_failed_ops_save->op[dfosr_loop_var].op_type = dmmt_failed_ops->op[dfosr_loop_var].
      op_type
      SET dmmt_failed_ops_save->op[dfosr_loop_var].expimp_log = dmmt_failed_ops->op[dfosr_loop_var].
      expimp_log
    ENDFOR
    SET stat = initrec(dmmt_failed_ops)
    FOR (dfosr_loop_var = 1 TO dmmt_all_failed_ops->op_cnt)
      IF ((dmmt_all_failed_ops->op[dfosr_loop_var].tbl=dmmt_err_save->tbl[dmmt_screen->cur_line].tbl)
      )
       SET dfosr_tbl_op_cnt = (dfosr_tbl_op_cnt+ 1)
      ENDIF
    ENDFOR
    SET dfosr_start_ind = 1
    FOR (dfosr_loop_var = 1 TO dfosr_tbl_op_cnt)
      IF (locateval(dfosr_op_exists,dfosr_start_ind,dmmt_all_failed_ops->op_cnt,dmmt_err_save->tbl[
       dmmt_screen->cur_line].tbl,dmmt_all_failed_ops->op[dfosr_op_exists].tbl) > 0)
       IF ((dmmt_all_failed_ops->op[dfosr_op_exists].tbl=dmmt_err_save->tbl[dmmt_screen->cur_line].
       tbl))
        SET dmmt_failed_ops->op_cnt = (dmmt_failed_ops->op_cnt+ 1)
        IF (mod(dmmt_failed_ops->op_cnt,10)=1)
         SET stat = alterlist(dmmt_failed_ops->op,(dmmt_failed_ops->op_cnt+ 9))
        ENDIF
        SET dmmt_failed_ops->op[dfosr_loop_var].op_id = dmmt_all_failed_ops->op[dfosr_op_exists].
        op_id
        SET dmmt_failed_ops->op[dfosr_loop_var].cmd = dmmt_all_failed_ops->op[dfosr_op_exists].cmd
        SET dmmt_failed_ops->op[dfosr_loop_var].tbl = dmmt_all_failed_ops->op[dfosr_op_exists].tbl
        SET dmmt_failed_ops->op[dfosr_loop_var].obj = dmmt_all_failed_ops->op[dfosr_op_exists].obj
        SET dmmt_failed_ops->op[dfosr_loop_var].op_type = dmmt_all_failed_ops->op[dfosr_op_exists].
        op_type
        SET dmmt_failed_ops->op[dfosr_loop_var].expimp_log = dmmt_all_failed_ops->op[dfosr_op_exists]
        .expimp_log
       ENDIF
       SET dfosr_start_ind = (dfosr_op_exists+ 1)
       SET dfosr_op_exists = 0
      ENDIF
    ENDFOR
    SET stat = alterlist(dmmt_failed_ops->op,dmmt_failed_ops->op_cnt)
   ELSE
    SET dm_err->eproc = "Reverting the screen state from dmmt_failed_ops_save to dmmt_err"
    CALL disp_msg("",dm_err->logfile,0)
    IF ((dmmt_failed_ops_save->op_cnt=0))
     SET dm_err->eproc = "dmmt_failed_ops_save is empty. Nothing to revert to. Skipping."
     CALL disp_msg("",dm_err->logfile,0)
     RETURN(1)
    ENDIF
    SET stat = initrec(dmmt_failed_ops)
    SET dmmt_failed_ops->op_cnt = dmmt_failed_ops_save->op_cnt
    SET stat = alterlist(dmmt_failed_ops->op,dmmt_failed_ops->op_cnt)
    FOR (dfosr_loop_var = 1 TO dmmt_failed_ops_save->op_cnt)
      SET dmmt_failed_ops->op[dfosr_loop_var].op_id = dmmt_failed_ops_save->op[dfosr_loop_var].op_id
      SET dmmt_failed_ops->op[dfosr_loop_var].cmd = dmmt_failed_ops_save->op[dfosr_loop_var].cmd
      SET dmmt_failed_ops->op[dfosr_loop_var].tbl = dmmt_failed_ops_save->op[dfosr_loop_var].tbl
      SET dmmt_failed_ops->op[dfosr_loop_var].obj = dmmt_failed_ops_save->op[dfosr_loop_var].obj
      SET dmmt_failed_ops->op[dfosr_loop_var].op_type = dmmt_failed_ops_save->op[dfosr_loop_var].
      op_type
      SET dmmt_failed_ops->op[dfosr_loop_var].expimp_log = dmmt_failed_ops_save->op[dfosr_loop_var].
      expimp_log
    ENDFOR
   ENDIF
   IF ((dm_err->debug_flag > 721))
    CALL echorecord(dmmt_failed_ops)
    CALL echorecord(dmmt_failed_ops_save)
   ENDIF
 END ;Subroutine
 SUBROUTINE dmmt_screen_refresh(null)
   DECLARE dsr_failed_cnt = i4 WITH protect, noconstant(0)
   FOR (dsr_failed_cnt = 1 TO dmmt_failed_ops->op_cnt)
    IF (dsr_failed_cnt=1)
     IF (drr_check_log_for_errors(0.0," ",2,dmmt_err_ret)=0)
      RETURN(0)
     ENDIF
     IF (drr_check_log_for_errors(dmmt_failed_ops->op[dsr_failed_cnt].op_id,dmmt_failed_ops->op[
      dsr_failed_cnt].expimp_log,2,dmmt_err_ret)=0)
      RETURN(0)
     ENDIF
    ELSE
     IF (drr_check_log_for_errors(dmmt_failed_ops->op[dsr_failed_cnt].op_id,dmmt_failed_ops->op[
    ...
    IF (dmmt_err_ret=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("OP_ID:",build(dmmt_failed_ops->op[dsr_failed_cnt].op_id),
      " in a FAILED status, but no errors found in log.")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ...
   IF (dmmt_xfer_errors(null)=0)
    RETURN(0)
   ENDIF
   SET dmmt_screen->cur_line = 0
   SET dmmt_screen->max_value = dmmt_err->tbl_cnt
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_chk_tbl_eligible_restart(dcter_run_id,dcter_tbl_name,dcter_op_type,
  dcter_eligible_ind)
   SET dcter_eligible_ind = 0
   SET dm_err->eproc = concat("Checking if ",dcter_tbl_name,
    " table is eligible for restart operation.")
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dm2_ddl_ops_log ddol
    WHERE ddol.run_id=dcter_run_id
     AND ddol.table_name=dcter_tbl_name
     AND ddol.op_type=dcter_op_type
     AND ((ddol.status="RUNNING") OR (ddol.status = null))
     AND ddol.ready_to_run_ind=1
   ...
   ENDIF
   IF (curqual=0)
    SET dcter_eligible_ind = 1
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_split_tbls_retry_screen(null)
   SET dmmt_screen_save->top_line = dmmt_screen->top_line
   SET dmmt_screen_save->cur_line = dmmt_screen->cur_line
   SET dmmt_screen_save->bottom_line = dmmt_screen->bottom_line
   SET dmmt_screen_save->max_scroll = 14
   SET dmmt_screen_save->max_value = dmmt_screen->max_value
   CALL dmmt_err_save_rs(1)
   SET stat = initrec(dmmt_err)
   CALL dmmt_failed_ops_save_rs(1)
   SET dmmt_failed_cnt = 0
   FOR (dmmt_failed_cnt = 1 TO dmmt_failed_ops->op_cnt)
     IF (drie_log_dir > "")
      SET dmmt_failed_ops->op[dmmt_failed_cnt].expimp_log = build(drie_log_dir,"/",cnvtlower(
   ...
   SET dmmt_screen->cur_line = 0
   SET dmmt_screen->max_value = dmmt_err->tbl_cnt
   WHILE (dwm_continue=0)
     SET dwm_intermediate_screen_return = " "
     IF (dmmt_intermediate_work_menu(dwm_intermediate_screen_return)=0)
      RETURN(0)
     ENDIF
     CASE (dwm_intermediate_screen_return)
      OF "QUIT":
       SET dwm_continue = 1
     ENDCASE
   ENDWHILE
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmmt_fail_tbl_import_ops(dftio_run_id,dftio_tbl_name,dftio_op_type)
   SET dm_err->eproc =
   "Updating the statuses of the split operations to ERROR for the current table."
   CALL disp_msg("",dm_err->logfile,0)
   UPDATE  FROM dm2_ddl_ops_log l
    SET l.status = "ERROR", l.error_msg = dmmt_auto_fail_emsg
    WHERE l.run_id=dftio_run_id
     AND l.table_name=dftio_tbl_name
     AND l.op_type=dftio_op_type
     AND l.status=null
     AND l.ready_to_run_ind=1
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
...
#exit_program
 IF ((dm_err->debug_flag > 721))
  ...
  CALL echorecord(drr_clin_copy_data)
 ENDIF
 CALL final_disp_msg("dm2_retry_imp_exp")
END GO
