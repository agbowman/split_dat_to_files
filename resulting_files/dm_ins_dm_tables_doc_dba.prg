CREATE PROGRAM dm_ins_dm_tables_doc:dba
 SET c_mod = "DM_INS_DM_TABLES_DOC 000"
 ...
 IF (qual_cnt > 0)
  ...
  IF (update_ind)
   ...
   UPDATE  FROM dm_tables_doc r,
     (dummyt d  WITH seq = value(qual_cnt))
    SET r.reference_ind = cnvtint(requestin->list_0[d.seq].reference_ind), r.human_reqd_ind = cnvtint
     (requestin->list_0[d.seq].human_reqd_ind)
    PLAN (d
     WHERE d.seq > 0)
     JOIN (r
     WHERE r.table_name=cnvtupper(requestin->list_0[d.seq].table_name))
  ...
  INSERT  FROM dm_tables_doc r,
    (dummyt d  WITH seq = value(qual_cnt))
   SET r.table_name = cnvtupper(requestin->list_0[d.seq].table_name), r.reference_ind = cnvtint(
     requestin->list_0[d.seq].reference_ind), r.human_reqd_ind = cnvtint(requestin->list_0[d.seq].
     human_reqd_ind)
   PLAN (d
