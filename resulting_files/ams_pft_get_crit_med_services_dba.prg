CREATE PROGRAM ams_pft_get_crit_med_services:dba
 ...
 IF (size(pft_reply->medservices,5) > 0)
  ...
  FOR (loopcnt = 1 TO size(pft_reply->medservices,5))
    SET recordpos = getnextrecord(0)
    SET stat = setstringfield(recordpos,disppos,substring(1,100,pft_reply->medservices[loopcnt].
      medservicedisplay))
    SET stat = setrealfield(recordpos,valuepos,pft_reply->medservices[loopcnt].medservicecd)
  ENDFOR
