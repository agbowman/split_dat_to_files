CREATE PROGRAM bhsreq_cls:dba
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   IF (o.activity_type_cd=modify_cd)
    orders->qual[ord_cnt].activity = uar_get_code_display(order_cd), orders->qual[ord_cnt].
    activity_type_cd = order_cd
   ELSE
   ...
   IF (o.catalog_type_cd=pharmacy_cd)
    ...
    ENDIF
   ENDIF
   IF (oa.action_type_cd IN (transfercancel_cd, suspend_cd, activatestudentorder_cd, order_cd,
   modify_cd,
   resume_cd, discontinue_cd, void_cd, cancelreorder_cd, canceldc_cd,
   cancel_cd, activate_cd)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   ...
   action_type_cd IN (activatestudentorder_cd, activate_cd))
 ...
 IF ((((orders->spoolout_ind=1)) OR ((request->print_prsnl_id > 0))) )
  ...
  SET tempfile1a = build(concat("cer_temp:dcpreq62","_",new_timedisp),".dat")
  ...
  SELECT INTO value(request->printer_name)
   ...
   HEAD PAGE
    ...
    FOR (i = 1 TO problem->problem_total)
      ...
      ENDIF
    ENDFOR
    line_wrap, xrow = (xrow+ 20), xcol = 44,
    row + 1,
   ...
   DETAIL
    FOR (vv = 1 TO value(order_cnt))
      ...
      xrow = (xrow+ 10), xcol = 470,
      CALL print(calcpos(xcol,xrow)),
      "{cpi/14}BHSREQ_CLS", row + 1
    ENDFOR
   WITH nocounter, maxrow = 800, maxcol = 750,
    dio = postscript
  ;end select
  CALL echo(cnvtupper(trim(orders->qual[1].comment,3)))
  IF (substring(1,5,cnvtupper(trim(orders->qual[1].comment,3)))="DEBUG")
   SET dev_pos = findstring("DEV=",cnvtupper(trim(orders->qual[1].comment,3)),1,0)
   CALL echo(dev_pos)
   SET dev_strt_pos = (findstring('"',cnvtupper(trim(orders->qual[1].comment,3)),(dev_pos+ 4),0)+ 1)
   CALL echo(dev_strt_pos)
   SET dev_stop_pos = findstring('"',cnvtupper(trim(orders->qual[1].comment,3)),(dev_strt_pos+ 1),0)
   CALL echo(dev_stop_pos)
   IF (((dev_stop_pos - dev_strt_pos) < 51))
    SET dev_len = (dev_stop_pos - dev_strt_pos)
    CALL echo(dev_len)
   ELSE
    SET dev_len = 0
    CALL echo(dev_len)
   ...
   IF (dev_pos != 0
    ...
    AND dev_stop_pos != 0
    AND dev_len != 0)
    SET dev = trim(substring(dev_strt_pos,dev_len,cnvtupper(trim(orders->qual[1].comment,3))),3)
    CALL echo(dev)
    SET request->printer_name = trim(dev,3)
