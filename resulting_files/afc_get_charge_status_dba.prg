CREATE PROGRAM afc_get_charge_status:dba
 SET afc_get_charge_status = "500186.FT.001"
 ...
 IF (validate(debug,- (1)) > 0)
  CALL echo("Entering AFC_GET_CHARGE_STATUS")
 ENDIF
 CALL logmsg(curprog,"Entering AFC_GET_CHARGE_STATUS",log_debug)
 SET reply->status_data.status = "F"
 SET table_name = "CHARGE"
 IF (validate(debug,- (1)) > 0)
  CALL echo("Calling isChargeValid subroutine")
 ENDIF
 CALL logmsg(curprog,"Calling isChargeValid subroutine",log_debug)
 CALL ischargevalid(request->charge_item_id,request->updt_cnt)
 IF (validate(debug,- (1)) > 0)
  CALL echo("Exiting isChargeValid subroutine")
 ENDIF
 CALL logmsg(curprog,"Exiting isChargeValid subroutine",log_debug)
 IF (failed=false)
  SET reply->status_data.status = "S"
 ELSE
  CASE (failed)
  ...
  CALL logmsg(curprog,build("Reply Status is ",reply->status_data.status," Ending Program"),log_error
   )
  CALL logmsg(curprog,build("Operation Name is ",reply->status_data.subeventstatus[1].operationname,
    " Ending Program"),log_error)
 ENDIF
 GO TO end_program
 IF (validate(ischargevalid,char(128))=char(128))
  SUBROUTINE (ischargevalid(pchargeitemid=f8,pchargeupdtcnt=i4) =null)
    CALL logmsg(curprog,"Entering isChargeValid() subroutine",log_debug)
    DECLARE curr_updt_cnt = i4 WITH noconstant(0)
    SELECT INTO "nl:"
     FROM charge c
     WHERE c.charge_item_id=pchargeitemid
     DETAIL
      curr_updt_cnt = c.updt_cnt
     WITH nocounter
    ;end select
    IF (curqual=0)
     ...
     RETURN
    ENDIF
    IF (curr_updt_cnt != pchargeupdtcnt)
     SET failed = updt_cnt_error
     RETURN
    ENDIF
    CALL logmsg(curprog,"Exiting isChargeValid() subroutine",log_debug)
  END ;Subroutine
