CREATE PROGRAM bed_run_xrays:dba
 ...
 IF (((datetimefind(cnvtdatetime(curdate,0),"W","B","B")=cnvtdatetime(curdate,0)) OR (forceperiodic=1
  ...
  IF (happ=0)
   ...
   IF (iret != 0)
    CALL esmerror("Error obtaining application handle",esmexit)
    GO TO end_script
   ENDIF
  ENDIF
  SET iret = uar_crmbegintask(happ,taskid,htask)
  IF (((iret != 0) OR (htask=0)) )
   CALL esmerror("Error calling CrmBeginTask",esmexit)
   GO TO end_script
  ENDIF
  SET iret = uar_crmbeginreq(htask,0,reqid,hreq)
  IF (((iret != 0) OR (hreq=0)) )
   CALL esmerror("Error calling CrmBeginReq",esmexit)
   GO TO end_script
  ENDIF
  SET iret = uar_crmperform(hreq)
  IF (iret != 0)
   CALL esmerror("Error calling CrmPerform",esmexit)
   GO TO end_script
  ENDIF
 ENDIF
#end_script
