CREATE PROGRAM dm2_ref_data_version:dba
 ...
 SET vers_magic_date = cnvtdatetime("31-DEC-2100 23:59:59")
 ...
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].beg_col_name="")
  ...
  GO TO exit_versions
 ENDIF
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].end_col_name="")
  AND vers_alg="ALG2")
  SET dm2_ref_data_reply->error_ind = 1
 ...
 CASE (vers_alg)
  ...
  OF "ALG4":
   IF (vers_active_ind=0)
    ...
    IF ((vers_query_ret=- (2)))
     SET dm2_ref_data_reply->error_ind = 1
     SET dm2_ref_data_reply->error_msg =
     "Multiple active rows were found in source for a versioned table."
    ...
    ELSEIF ((vers_query_ret=- (3)))
     IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind=1)
      ...
      IF (vers_translated=1)
       ...
       IF (vers_query_ret > 0)
        ...
        IF (vers_alg != "ALG4")
         SET drdm_parser->statement[1].frag = concat("update into ",vers_table_name,
          " dc set dc.active_ind = 0, ")
         SET vers_parser_cnt = 1
         IF (vers_task_ind=1)
         ...
         SET drdm_parser->statement[vers_parser_cnt].frag = concat(" dc.",dm2_ref_data_doc->tbl_qual[
          temp_tbl_cnt].end_col_name,"=cnvtdatetime(vers_cur_date) where ")
         SET vers_parser_cnt = (vers_parser_cnt+ 1)
         SET drdm_parser->statement[vers_parser_cnt].frag = concat("dc.",vers_primary_key,
          " = vers_query_ret "," and dc.active_ind = 1 go")
         CALL parse_statements(vers_parser_cnt)
        ELSE
         CALL vers_mass_update("INACTIVATE")
        ENDIF
       ...
       ELSEIF ((vers_query_ret=- (3)))
        ...
        IF (vers_alg != "ALG4")
         ...
         CALL version_exception(vers_table_name,vers_primary_key,vers_from_id)
        ENDIF
       ENDIF
       IF (nodelete_ind=1)
        SET dm2_ref_data_reply->error_ind = 1
       ...
       CALL parser(concat("set rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
         dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].end_col_name," = vers_magic_date go"),1)
       IF (version_nbr_col != "")
        SET ui_query_rec->table_name = vers_table_name
        SET ui_query_rec->dom = "TO"
        SET stat = alterlist(ui_query_rec->qual,1)
        SET ui_query_rec->qual[1].qtype = concat("ORDER:",version_nbr_col)
        IF (version_nbr_col != "")
         SET stat = alterlist(ui_query_rec->qual[1].other_pk_col,1)
         SET ui_query_rec->qual[1].other_pk_col[1].col_name = version_nbr_col
        ENDIF
        SET vers_query_ret = exec_ui_query(temp_tbl_cnt,perm_col_cnt)
        SET ui_query_eval_rec->qual[1].additional_attr = ui_query_eval_rec->qual[size(
         ui_query_eval_rec->qual,5)].additional_attr
        CALL parser(concat("set rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
          version_nbr_col," = cnvtint(ui_query_eval_rec->qual[1].additional_attr)+1 go"),1)
       ...
       IF (vers_alg != "ALG4")
        ...
        IF (vers_val > 0)
         CALL put_value(vers_table_name,vers_primary_key,cnvtstring(vers_val))
         SET current_merges = (current_merges+ 1)
        ...
        ENDIF
        SET vers_from_id = cnvtreal(get_value(vers_table_name,vers_primary_key,"FROM"))
        SET cust_ins_upd_ind = select_merge_translate(cnvtstring(vers_from_id),vers_table_name)
        IF (cust_ins_upd_ind="No Trans")
     ...
     ELSE
      ...
      IF ((vers_query_ret=- (3)))
       ...
       IF (vers_translated=1)
        ...
        IF (vers_query_ret > 0)
         ...
         IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].effective_col_ind=1))
          SET vers_parser_cnt = (vers_parser_cnt+ 1)
          SET drdm_parser->statement[vers_parser_cnt].frag = concat(" ,dc.",dm2_ref_data_doc->
           tbl_qual[temp_tbl_cnt].end_col_name," = cnvtdatetime(vers_cur_date) ")
         ENDIF
         SET vers_parser_cnt = (vers_parser_cnt+ 1)
         IF (vers_alg != "ALG4")
          SET drdm_parser->statement[vers_parser_cnt].frag = concat(" where dc.",vers_primary_key,
           " = vers_query_ret "," and dc.active_ind = 1 go")
         ELSE
          CALL vers_mass_update("INACTIVATE")
         ENDIF
      ...
      ELSEIF ((vers_query_ret=- (2)))
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg =
       "Multiple effective rows were found in source for a versioned table."
      ELSEIF ((vers_query_ret=- (1)))
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg =
    ...
    IF (vers_translated=0)
     ...
     IF (vers_translated=1)
      ...
      ELSE
       ...
       IF (vers_query_ret > 0)
        IF (vers_alg != "ALG4")
         SET drdm_parser->statement[1].frag = concat("update into ",vers_table_name," dc set dc.",
          dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].end_col_name," = cnvtdatetime(vers_cur_date) ")
         SET vers_parser_cnt = 1
         IF (vers_task_ind=1)
          SET vers_parser_cnt = (vers_parser_cnt+ 1)
          SET drdm_parser->statement[vers_parser_cnt].frag = " ,dc.updt_task = 4310001 "
         ...
         CALL parse_statements(vers_parser_cnt)
        ELSE
         CALL vers_mass_update("END-EFFECTIVE")
        ENDIF
        IF (nodelete_ind=1)
         SET dm2_ref_data_reply->error_ind = 1
         SET dm2_ref_data_reply->error_msg = dm_err->emsg
     ...
     IF (version_nbr_col != "")
      ...
      IF ((vers_query_ret=- (2)))
       SET ui_query_eval_rec->qual[1].additional_attr = ui_query_eval_rec->qual[size(
        ui_query_eval_rec->qual,5)].additional_attr
      ELSEIF ((vers_query_ret=- (3)))
       SET stat = alterlist(ui_query_eval_rec->qual,1)
       SET ui_query_eval_rec->qual[1].additional_attr = "0"
      ENDIF
      CALL parser(concat("set rs_",dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].suffix,"->to_values.",
        version_nbr_col," = cnvtint(ui_query_eval_rec->qual[1].additional_attr)+1 go"),1)
      SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_nbr_pos].translated = 1
  ...
  OF "ALG2":
   ...
   FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
     ...
     vers_primary_key)
      AND (dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[vers_loop].column_name="PREV*"))
    ...
    IF (vers_translated=0)
     ...
     IF (vers_translated=1)
      ...
      IF ((vers_query_ret=- (2)))
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
        ": A query for effective rows in the target returned multiple values.")
       GO TO exit_versions
      ELSEIF (vers_query_ret > 0)
       ...
       ELSE
        GO TO exit_versions
       ENDIF
       SET drdm_parser->statement[1].frag = concat("insert into ",vers_table_name," dc (")
       SET drdm_parser_cnt = 2
       FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
         SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" dc.",dm2_ref_data_doc->tbl_qual[
       ...
       FOR (vers_loop = 1 TO dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_cnt)
        ...
        vers_primary_key))
         SET drdm_parser->statement[drdm_parser_cnt].frag = concat(trim(cnvtstring(vers_new_val)),
          ".0")
         SET drdm_parser_cnt = (drdm_parser_cnt+ 1)
       ...
       SET drdm_parser->statement[drdm_parser_cnt].frag = concat(" from ",vers_table_name,
        " d where d.",vers_primary_key," = vers_val) go")
       CALL parse_statements(drdm_parser_cnt)
      ELSEIF ((vers_query_ret=- (1)))
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = concat(vers_table_name,
      ...
      IF (vers_ins_sel_ind=0)
       CALL put_value(vers_table_name,vers_hist_name,cnvtstring(vers_val))
      ENDIF
      CALL insert_update_row(temp_tbl_cnt,perm_col_cnt)
     ELSE
      SET vers_translated = is_translated(vers_table_name,"ALL")
 ...
 SUBROUTINE vers_mass_update(vers_type)
   ...
   ENDIF
   SET vers_parser_cnt = (vers_parser_cnt+ 1)
   SET drdm_parser->statement[vers_parser_cnt].frag = " go"
   CALL parse_statements(vers_parser_cnt)
 END ;Subroutine
#exit_versions
