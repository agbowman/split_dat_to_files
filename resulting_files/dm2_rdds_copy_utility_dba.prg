CREATE PROGRAM dm2_rdds_copy_utility:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drcu_rel_type_con = c15 WITH constant("REFERENCE MERGE"), protect
 DECLARE drcu_valid_input_ind = i2 WITH protect
 DECLARE drcu_str = vc WITH protect
 DECLARE drcu_from_id = f8 WITH protect
 DECLARE drcu_to_id = f8 WITH protect
 DECLARE drcu_copy_done = i2 WITH protect
 DECLARE drcu_copy_cnt = i4 WITH protect
 DECLARE drcu_min_log = f8 WITH protect
 DECLARE drcu_max_log = f8 WITH protect
 DECLARE drcu_max_batch = f8 WITH protect
 DECLARE drcu_from_name = vc WITH protect
 DECLARE drcu_to_name = vc WITH protect
 DECLARE drcu_copy_err_ind = i2 WITH protect
 DECLARE drcu_dcopy_finished = i4 WITH protect
 DECLARE drcu_dcopy_insert_cnt = i4 WITH protect
 DECLARE drcc_min_cnt = f8 WITH protect
 DECLARE drcc_max_cnt = f8 WITH protect
 DECLARE drcu_loop = i2 WITH protect
 DECLARE drcu_question1 = c1 WITH protect, noconstant("")
 DECLARE drcu_question2 = c1 WITH protect, noconstant("")
 DECLARE drcu_confirm_row = i4 WITH protect, noconstant(15)
 DECLARE drcc_col_count = i4 WITH protect, noconstant(0)
 IF ((validate(drcu_range_inc,- (2))=- (2)))
  DECLARE drcu_range_inc = f8 WITH protect, noconstant(3000.0)
 ENDIF
 IF (check_logfile("dm2_rdds_copy_util",".log","DM2_RDDS_COPY_UTIL LOGFILE")=0)
  SET dm_err->err_ind = 1
 ...
 SET drcu_loop = 0
 WHILE (drcu_loop=0)
   SET drcu_valid_input_ind = 0
   SET dm_err->eproc = "Starting to build window"
   CALL disp_msg(" ",dm_err->logfile,0)
   SET width = 132
   SET message = window
   CALL video(n)
   CALL clear(1,1)
   CALL box(1,1,23,132)
   CALL text(3,57,"TARGET ENVIRONMENTS")
   CALL line(5,1,131,horizontal)
   CALL text(7,10,"Please select the target environment ID that rows should be copied from: ")
   CALL text(22,2,"For <HELP> press 'Shift + F5'")
   WHILE (drcu_valid_input_ind=0)
     SET id_cnt = 0
     SET help = pos(13,10,8,100)
     SET help =
     SELECT INTO "NL:"
      de.environment_id, de.environment_name, de.description
      FROM dm_environment de
     ...
     ;end select
     SET validate = 2
     CALL accept(7,83,"9(11)",0)
     IF (curaccept=0)
      GO TO exit_script
     ENDIF
     SET drcu_from_id = curaccept
     SET drcu_valid_input_ind = 0
     SELECT INTO "NL:"
      d.target_env_id
      FROM dm_chg_log d
      WHERE d.target_env_id=drcu_from_id
      DETAIL
       drcu_valid_input_ind = 1
      WITH nocounter, maxqual(d,1)
     ;end select
     IF (drcu_valid_input_ind=0)
      CALL text(24,1,"No rows in DM_CHG_LOG for target selected")
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "There are no valid rows in the DM_CHG_LOG table"
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      GO TO exit_script
     ENDIF
   ENDWHILE
   ...
   SET help = off
   SET drcu_valid_input_ind = 0
   CALL text(9,10,"Please enter the new environment id that these rows will be copied to: ")
   WHILE (drcu_valid_input_ind=0)
     SET help = pos(13,10,8,100)
     SET help =
     SELECT INTO "NL:"
      de.environment_id, de.environment_name, de.description
      FROM dm_environment de
      WHERE de.environment_id != drcu_from_id
      WITH nocounter
     ;end select
     SET validate = required
     SET validate =
     ...
     ;end select
     SET validate = 2
     CALL accept(9,80,"9(11)",0)
     SET drcu_to_id = curaccept
     SET drcu_valid_input_ind = 0
     SELECT INTO "NL:"
      d.environment_id
      FROM dm_environment d
      WHERE d.environment_id=drcu_to_id
       AND d.environment_id != drcu_from_id
      DETAIL
       drcu_valid_input_ind = 1
      WITH nocounter
     ;end select
   ...
   SELECT INTO "NL:"
    FROM dm_environment d
    WHERE d.environment_id=drcu_from_id
    DETAIL
     drcu_from_name = d.environment_name
    WITH nocounter
   ;end select
   SELECT INTO "NL:"
    FROM dm_environment d
    WHERE d.environment_id=drcu_to_id
    DETAIL
     drcu_to_name = d.environment_name
    WITH nocounter
   ;end select
   CALL text(11,10,"Do you want to reset the change log rows during the copy? (Y/N)")
   CALL text(12,14,"Resetting the change log will cause all log rows that had previously")
   SET drcu_str = concat("processed from ",build(cnvtint(drcu_from_id))," -> ",drcu_from_name,
    " to process again during the")
   CALL text(13,14,drcu_str)
   SET drcu_str = concat(build(cnvtint(drcu_to_id))," -> ",drcu_to_name," merge.")
   CALL text(14,14,drcu_str)
   CALL accept(11,74,"P;CU","N"
    WHERE curaccept IN ("Y", "N"))
   SET drcu_question1 = curaccept
   IF (drcu_question1="N")
    CALL text(15,10,"Do you want to copy all DM_CHG_LOG rows? (Y/N)")
    SET drcu_str = concat("Choosing ",char(34),"N",char(34),
     " will only copy rows that haven't already been merged.")
    CALL text(16,14,drcu_str)
    CALL accept(15,57,"P;CU","Y"
     WHERE curaccept IN ("Y", "N"))
    SET drcu_question2 = curaccept
    SET drcu_confirm_row = 17
   ENDIF
   IF (drcu_question1="Y")
    SET drcu_str = concat("Do you want to copy and reset DM_CHG_LOG rows from ",drcu_from_name," to ",
     drcu_to_name,"? (Y/N)")
   ELSEIF (drcu_question2="N")
    SET drcu_str = concat("Do you want to copy only non-merged rows from DM_CHG_LOG from ",
     drcu_from_name," to ",drcu_to_name,"? (Y/N)")
   ELSE
    SET drcu_str = concat("Do you want to copy all DM_CHG_LOG rows from ",drcu_from_name," to ",
     drcu_to_name,"? (Y/N)")
   ENDIF
   CALL text(drcu_confirm_row,10,drcu_str)
   CALL accept(drcu_confirm_row,(size(drcu_str)+ 11),"P;CU","Y"
    WHERE curaccept IN ("Y", "N"))
   IF (curaccept="Y")
    SET drcu_loop = 1
    CALL clear(1,1)
    SET message = nowindow
    SET dm_err->eproc = "Starting copy process"
   ELSE
    CALL text((drcu_confirm_row+ 2),10,
     "Do you want to try another selection or quit? Y to continue, N to quit")
    CALL accept((drcu_confirm_row+ 2),85,"P;CU","Y"
     WHERE curaccept IN ("Y", "N"))
    IF (curaccept="N")
     SET drcu_loop = 1
     GO TO exit_script
   ...
   ENDIF
 ENDWHILE
 SET dm_err->eproc = "Copying DM_CHG_LOG rows"
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2 RDDS COPY UTILITY"
   AND di.info_name=concat("SRC:",cnvtstring(drcu_from_id),":TRGT:",cnvtstring(drcu_to_id))
  DETAIL
   drcc_min_cnt = di.info_number
  WITH nocounter
 ;end select
 IF (curqual=0)
  SELECT INTO "nl:"
  ...
  ;end select
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2 RDDS COPY UTILITY", di.info_name = concat("SRC:",cnvtstring(drcu_from_id
      ),":TRGT:",cnvtstring(drcu_to_id)), di.info_number = drcc_min_cnt,
    di.updt_dt_tm = cnvtdatetime(curdate,curtime3)
   WITH nocounter
  ;end insert
  IF (check_error("ERROR occurred when inserting minimum into DM_INFO") != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ROLLBACK
   GO TO exit_script
  ELSE
 ...
 SELECT INTO "nl:"
  FROM dtable t,
   dtableattr a,
   dtableattrl l
  WHERE t.table_name="DM_CHG_LOG"
   AND t.table_name=a.table_name
   AND l.structtype="F"
   AND btest(l.stat,11)=0
   AND  NOT (l.attr_name IN ("TARGET_ENV_ID", "LOG_ID", "LOG_TYPE", "CHG_DT_TM", "RDBHANDLE"))
  DETAIL
   drcc_col_count = (drcc_col_count+ 1)
   IF (mod(drcc_col_count,10)=1)
    stat = alterlist(columns->list,(drcc_col_count+ 9))
   ENDIF
   columns->list[drcc_col_count].name = l.attr_name
  FOOT REPORT
   stat = alterlist(columns->list,drcc_col_count)
  WITH nocounter
 ;end select
 SET drcu_copy_done = 0
 WHILE (drcc_min_cnt <= drcc_max_cnt
  AND drcu_copy_done=0)
   EXECUTE dm2_rdds_copy_child value(drcc_min_cnt), value(drcu_to_id), value(drcu_from_id),
   value(drcu_range_inc)
   IF (drcu_copy_err_ind=1)
    SET drcu_copy_done = 1
   ELSE
    UPDATE  FROM dm_info di
     SET di.info_number = minval((drcc_min_cnt+ drcu_range_inc),drcc_max_cnt), di.updt_cnt = (di
      .updt_cnt+ 1), di.updt_dt_tm = cnvtdatetime(curdate,curtime3)
     WHERE di.info_domain="DM2 RDDS COPY UTILITY"
      AND di.info_name=concat("SRC:",cnvtstring(drcu_from_id),":TRGT:",cnvtstring(drcu_to_id))
     WITH nocounter
    ;end update
    IF (check_error("ERROR occurred when updating minimum in DM_INFO") != 0)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET drcu_copy_done = 1
     ROLLBACK
    ELSE
    ...
    ENDIF
   ENDIF
   SET drcc_min_cnt = (drcc_min_cnt+ drcu_range_inc)
   IF (drcc_min_cnt > drcc_max_cnt)
    SET drcu_copy_done = 1
   ENDIF
 ENDWHILE
 IF (drcu_copy_err_ind != 1)
  SET dm_err->eproc = "Finished copying transactions."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
#exit_script
 FREE RECORD columns
 CALL final_disp_msg("dm2_rdds_copy_utility")
END GO
