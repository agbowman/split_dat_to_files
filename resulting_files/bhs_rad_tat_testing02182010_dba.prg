CREATE PROGRAM bhs_rad_tat_testing02182010:dba
 PROMPT
 ...
 IF (edlocation=2)
  SET edlogic = "elh.loc_nurse_unit_cd  NOT IN (Eda, Edpedi, Edgta, Edmain)"
 ELSE
 ...
 SELECT INTO "NL:"
  ...
  WHERE cv.code_set=220
   ...
   AND cv.display_key IN ("EDA", "EDPEDI", "EDGTA", "EDMAIN")
   AND cv.active_ind=1
  DETAIL
 ...
 SELECT INTO "NL:"
  ...
  FOOT REPORT
   ...
   IF (( $TOTALSONLY=1))
    stat = alterlist(event->sectionlist,(cntsection+ 2)), totalrptcfcnt = (totalrptcfcnt+
    complete_to_final), event->sectionlist[(cntsection+ 2)].section_name = "TOTALS",
    event->sectionlist[(cntsection+ 2)].totalorders = totalrptordcnt, event->sectionlist[(cntsection
    + 2)].avetatcf = (totalrptcfcnt/ totalrptordcnt)
   ENDIF
  WITH nocounter
 ;end select
