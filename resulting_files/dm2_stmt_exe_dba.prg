CREATE PROGRAM dm2_stmt_exe:dba
 ...
 SET v_type =  $1
 SET v_name =  $2
 FOR (sb_ndx = 1 TO size(request->data,5))
   CALL echo(request->data[sb_ndx].stmt,1)
   SET request->data[sb_ndx].stmt = replace(request->data[sb_ndx].stmt,";",concat(";",char(10)),0)
   CALL dm2_push_cmd(request->data[sb_ndx].stmt,1)
   SET request->data[sb_ndx].stmt = ""
 ENDFOR
 SET stat = alterlist(request->data,0)
 IF (check_error(concat("Build: Error in building ",v_type," ",v_name)) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET dm_err->err_ind = 1
  SET reply->err_ind = 1
 ELSE
