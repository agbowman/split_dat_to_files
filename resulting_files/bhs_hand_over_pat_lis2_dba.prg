CREATE PROGRAM bhs_hand_over_pat_lis2:dba
 PROMPT
 ...
 DECLARE headpersonid(ncalc=i2) = f8 WITH protect
 DECLARE headpersonidabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE allergytitle(ncalc=i2) = f8 WITH protect
 DECLARE allergytitleabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE allergy(ncalc=i2) = f8 WITH protect
 DECLARE allergyabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE eventtitle(ncalc=i2) = f8 WITH protect
 DECLARE eventtitleabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE persondetail2(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE persondetail2abs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8 WITH
 protect
 DECLARE stickynotesec(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE stickynotesecabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8 WITH
 protect
 DECLARE pagebreaksec(ncalc=i2) = f8 WITH protect
 DECLARE pagebreaksecabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 ...
 DECLARE _remsticknotetxt = i2 WITH noconstant(1), protect
 ...
 SUBROUTINE headpageabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ - (0.033)),(offsety+ 0.696),(offsetx+ 7.500),(
     offsety+ 0.696))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
 ...
 SUBROUTINE headpersonidabs(ncalc,offsetx,offsety)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE allergytitle(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = allergytitleabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE allergytitleabs(ncalc,offsetx,offsety)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE allergy(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = allergyabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE allergyabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.180000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE eventtitle(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = eventtitleabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE eventtitleabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.180000), private
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
 ...
 SUBROUTINE persondetail2abs(ncalc,offsetx,offsety,maxheight,bcontinue)
   ...
   IF (_rempending_list > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_rempending_list,((size(
        eventval) - _rempending_list)+ 1),eventval)))
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _rempending_list = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_rempending_list,((size(eventval) -
       _rempending_list)+ 1),eventval)))))
   ...
   IF (ncalc=rpt_render
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdrempending_list,((
       size(eventval) - _holdrempending_list)+ 1),eventval)))
   ELSE
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE stickynotesec(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = stickynotesecabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE stickynotesecabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.380000), private
   DECLARE growsum = i4 WITH noconstant(0), private
   IF (bcontinue=0)
   ...
   IF (_remsticknotetxt > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_remsticknotetxt,((size(
        stickynotevalue) - _remsticknotetxt)+ 1),stickynotevalue)))
    IF ((rptsd->m_height > ((offsety+ sectionheight) - rptsd->m_y)))
     SET sectionheight = ((rptsd->m_y+ _fdrawheight) - offsety)
    ENDIF
    IF ((rptsd->m_drawlength=0))
     SET _remsticknotetxt = 0
    ELSEIF ((rptsd->m_drawlength < size(nullterm(substring(_remsticknotetxt,((size(stickynotevalue)
        - _remsticknotetxt)+ 1),stickynotevalue)))))
     SET _remsticknotetxt = (_remsticknotetxt+ rptsd->m_drawlength)
    ELSE
     SET _remsticknotetxt = 0
    ENDIF
    SET growsum = (growsum+ _remsticknotetxt)
   ENDIF
   SET rptsd->m_flags = 4
   IF (ncalc=rpt_render
    AND _holdremsticknotetxt > 0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,nullterm(substring(_holdremsticknotetxt,((
       size(stickynotevalue) - _holdremsticknotetxt)+ 1),stickynotevalue)))
   ELSE
    SET _remsticknotetxt = _holdremsticknotetxt
   ENDIF
   SET rptsd->m_y = (offsety+ 0.050)
   SET rptsd->m_x = (offsetx+ 0.500)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE pagebreaksec(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = pagebreaksecabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE pagebreaksecabs(ncalc,offsetx,offsety)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_HAND_OVER_PAT_LIS2"
 ...
 SELECT INTO "NL:"
  ...
  HEAD a.person_id
   allergycnt = 0
  HEAD a.allergy_id
   allergycnt = (allergycnt+ 1), stat = alterlist(patientlevelinfo->qual[d.seq].allergies,allergycnt),
 ...
 SELECT INTO "NL:"
  ...
  PLAN (d)
   ...
   code_status_cd5,
   code_status_cd6, code_status_cd7)) OR (o.order_id=0.0
  ...
  ORDER BY p.person_id, eventsequence, ce1.event_cd,
   ce1.updt_dt_tm DESC
  HEAD REPORT
   topiccnt = 0,
