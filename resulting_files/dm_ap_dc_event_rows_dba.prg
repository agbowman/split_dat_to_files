CREATE PROGRAM dm_ap_dc_event_rows:dba
 ...
 DECLARE minimum_months_to_keep = i4 WITH protect, constant(60)
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    ade.rowid
    FROM ap_dc_event ade
    WHERE parser(sbr_getrowidnotexists("ade.event_id between curMinID and curMaxID","ade"))
     AND ((ade.cancel_dt_tm <= cnvtdatetime(purge_dt_tm)) OR (ade.complete_dt_tm <= cnvtdatetime(
     purge_dt_tm)))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,100)=1)
     ...
     ENDIF
     reply->rows[rows].row_id = ade.rowid
    WITH nocounter, maxqual(ade,value(rowsleft))
   ;end select
