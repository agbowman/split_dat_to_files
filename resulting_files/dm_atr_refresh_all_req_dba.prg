CREATE PROGRAM dm_atr_refresh_all_req:dba
 SET inhouse_ind = 0
 IF (trim(ref)="C")
  SET inhouse_ind = 1
 ENDIF
  ...
  *
  ...
  HEAD REPORT
   ...
   row + 1, "    2 cachetime = i4", row + 1,
   "    2 epilog_script = c30", row + 1, "    2 prolog_script = c30",
   row + 1, "    2 write_to_que_ind = i2", row + 1,
   "    2 active_ind = i2", row + 1, "    2 active_dt_tm = dq8",
   row + 1, "    2 inactive_dt_tm = dq8", row + 18,
   "    2 text = vc", row + 1, "    2 feature_number = i4",
   row + 1, "    2 schema_date = dq8", row + 1,
   "    2 deleted_ind = i2", row + 1, "    2 cachegrace = i4",
   row + 1, "    2 cachestale = i4", row + 1,
   "    2 cachetrim = c20", row + 1, "    2 requestclass = i4",
   row + 1, ") go", row + 1,
