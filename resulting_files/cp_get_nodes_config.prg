CREATE PROGRAM cp_get_nodes_config
 ...
 SUBROUTINE getnodes(null)
   ...
   SELECT INTO "NL:"
    ...
    PLAN (cn
     ...
     JOIN (cp)
     JOIN (ccd)
    ORDER BY cn.collation_seq, cn.cp_node_id, cp.cp_component_id,
     ccd.cp_component_detail_id
    HEAD REPORT
    ...
    HEAD cn.cp_node_id
     ...
     active_ind = cn.active_ind, reply->node_list[nodecnt].publish_ind = cn.publish_ind,
     reply->node_list[nodecnt].sequence = cn.collation_seq, reply->node_list[nodecnt].intention_cd =
     cn.intention_cd, reply->node_list[nodecnt].intention_cd_disp = uar_get_code_display(cn
      .intention_cd),
     reply->node_list[nodecnt].intention_cd_meaning = uar_get_code_meaning(cn.intention_cd), reply->
     node_list[nodecnt].node_name = cn.node_name, reply->node_list[nodecnt].treatment_line_cd = cn
     .treatment_line_cd,
     reply->node_list[nodecnt].category_mean = cn.category_mean, reply->node_list[nodecnt].sequence
      = cn.collation_seq
...
#exit_script
 IF (validate(debug_ind,0)=1)
  CALL echo("CP_GET_NODES_CONFIG EXIT SCRIPT")
