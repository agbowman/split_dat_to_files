CREATE PROGRAM dm2_ads_purge_menu:dba
 ...
 END ;Subroutine
 DECLARE dapm_src_dbase_name = vc WITH protect, noconstant("")
 DECLARE dapm_src_db_pass = vc WITH protect, noconstant("")
 DECLARE dapm_src_conn_str = vc WITH protect, noconstant("")
 DECLARE dapm_max_runners = i4 WITH protect, noconstant(0)
 DECLARE dapm_runners_set = i2 WITH protect, noconstant(0)
 DECLARE dapm_ndx = i2 WITH protect, noconstant(0)
 DECLARE dapm_nohup_logfile = vc WITH protect, noconstant("")
 DECLARE dapm_cur_logfile = vc WITH protect, noconstant("")
 DECLARE dapm_dcl_stat = i4 WITH protect, noconstant(0)
 DECLARE dapm_runners_running = i4 WITH protect, noconstant(0)
 DECLARE dapm_ops_completed = i4 WITH protect, noconstant(0)
 DECLARE dapm_ops_left = i4 WITH protect, noconstant(0)
 DECLARE dapm_ops_failed = i4 WITH protect, noconstant(0)
 DECLARE dapm_run_id = f8 WITH protect, noconstant(0.0)
 DECLARE dapm_progress_refresh = i2 WITH protect, noconstant(0)
 DECLARE dapm_header(null) = i2
 FREE RECORD dapm_running
 RECORD dapm_running(
   1 op[*]
     2 table_name = vc
   1 count = i2
 )
 IF (check_logfile("dm2_ads_purge_menu",".log","dm2_ads_purge_menu")=0)
  GO TO exit_program
 ENDIF
 ...
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dapm_src_db_pass = dm2_install_schema->p_word
 SET dapm_src_conn_str = dm2_install_schema->connect_str
 IF (dm2_get_dbase_name(dapm_src_dbase_name)=0)
  GO TO exit_program
 ENDIF
 WHILE (true)
   IF ((dm_err->debug_flag != 722))
    SET message = window
   ENDIF
   CALL dapm_header(null)
   CALL text(5,10,"1. Start Runners")
   CALL text(7,10,"2. Stop Runners")
   CALL text(9,10,"3. View Progress")
   CALL text(12,10,"Your Selection (0 to Exit)?")
   CALL accept(12,38,"X;CU"," "
    WHERE curaccept IN ("0", "1", "2", "3"))
   CASE (curaccept)
    OF "0":
     GO TO exit_program
    OF "1":
     SET dapm_runners_set = 0
     SET dapm_runners_running = 0
     SET dapm_max_runners = 0
     SET dm_err->eproc = "Get max runner value"
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg("",dm_err->logfile,0)
     ENDIF
     SELECT INTO "nl:"
      FROM dm_info d
      WHERE d.info_domain="ADS PURGE RUNNER"
       AND d.info_name="MAX RUNNERS"
      DETAIL
       dapm_max_runners = d.info_number, dapm_runners_set = 1
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc))
      ...
      GO TO exit_program
     ENDIF
     IF (dapm_runners_set=0)
      INSERT  FROM dm_info d
       SET d.info_domain = "ADS PURGE RUNNER", d.info_name = "MAX RUNNERS", d.info_number = 0
       WITH nocounter
      ;end insert
      IF (check_error(dm_err->eproc))
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     SELECT INTO "nl:"
      ...
      DETAIL
       dapm_runners_running = d.info_number
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc))
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     ...
     ENDIF
     CALL dapm_header(null)
     CALL text(5,10,concat("Number of Runners currently running: ",trim(cnvtstring(
         dapm_runners_running))))
     IF (dapm_max_runners=0
      AND dapm_runners_running > 0)
      CALL text(7,10,
       "NOTE: Runners were stopped by user request but there are some that are currently finishing work"
       )
      CALL text(8,16,"on a long-running operation and will stop once those operations are complete")
     ENDIF
     CALL text(10,10,"Number of additional runners to start (0 to exit to main menu): ")
     CALL accept(10,74,"9",0)
     SET dapm_runners_set = curaccept
     IF (dapm_runners_set > 0)
      CALL clear(10,2,90)
      CALL text(10,10,"Runners are starting...Please wait")
      FOR (dapm_ndx = 1 TO dapm_runners_set)
        SET dapm_cur_logfile = dm_err->unique_fname
        IF (get_unique_file("ads_purge_runner",".log")=0)
         SET dapm_nohup_logfile = "ads_purge_runner.log"
         SET dm_err->err_ind = 0
        ELSE
         SET dapm_nohup_logfile = dm_err->unique_fname
        ENDIF
        SET dm_err->unique_fname = dapm_cur_logfile
        CALL pause(5)
        SET dapm_execute_str = concat("nohup $cer_proc/ads_purge_runner.ksh ",dapm_src_db_pass," ",
         dapm_src_conn_str," > $CCLUSERDIR/",
         dapm_nohup_logfile," 2>&1 &")
        CALL dcl(dapm_execute_str,size(dapm_execute_str),dapm_dcl_stat)
        IF (dapm_dcl_stat=0)
         SET dm_err->eproc = concat("Error connecting to: ",dapm_dcl_stat)
         CALL disp_msg(" ",dm_err->logfile,0)
         GO TO exit_program
        ENDIF
      ENDFOR
      UPDATE  FROM dm_info d
       SET d.info_number = (d.info_number+ dapm_runners_set)
       WHERE d.info_domain="ADS PURGE RUNNER"
        AND d.info_name="MAX RUNNERS"
       WITH nocounter
      ;end update
      ...
      ENDIF
      CALL clear(10,2,90)
      CALL text(10,10,"Runners started. Choose option 3 from the main menu to monitor the progress.")
      CALL text(12,10,"Press Enter to return to the main menu.")
      CALL accept(12,51,"P;E"," ")
     ENDIF
    OF "2":
     SET dapm_runners_set = 0
     SET dm_err->eproc = "Get max runner value"
     ...
     IF (check_error(dm_err->eproc))
      ...
      GO TO exit_program
     ENDIF
     SET dm_err->eproc = "Set max runner value"
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg("",dm_err->logfile,0)
     ENDIF
     CALL dapm_header(null)
     SET dm_err->eproc = "Insert/Update max runner value"
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg("",dm_err->logfile,0)
     ENDIF
     IF (dapm_runners_set > 0)
     ...
     ENDIF
     CALL text(5,10,
      "Runners will stop after they have finished processing any operations in progress.")
     CALL text(7,10,"Press Enter to return to the main menu.")
     CALL accept(7,51,"P;E"," ")
    OF "3":
     IF (dapm_run_id=0.0)
      SELECT INTO "nl:"
       FROM dm2_ddl_ops ddo
       WHERE ddo.process_option="PURGE SOURCE"
       DETAIL
        dapm_run_id = ddo.run_id
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc))
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      ...
      ENDIF
     ENDIF
     SET dapm_progress_refresh = 1
     WHILE (dapm_progress_refresh=1)
       SET dapm_runners_running = 0
       SELECT INTO "nl:"
        FROM dm_info d
        WHERE d.info_domain="ADS PURGE RUNNER"
       ...
       SELECT INTO "nl:"
        FROM dm2_ddl_ops_log d
        WHERE d.run_id=dapm_run_id
         AND d.op_type="PURGE ACTIVITY DATA"
        HEAD REPORT
         dapm_ops_left = 0, dapm_ops_failed = 0, dapm_ops_completed = 0,
         dapm_running->count = 0, stat = alterlist(dapm_running->op,dapm_running->count)
        DETAIL
         IF (d.status="INCOMPLETE")
          dapm_ops_left = (dapm_ops_left+ 1)
         ELSEIF (d.status="FAILED")
          dapm_ops_failed = (dapm_ops_failed+ 1)
         ELSEIF (d.status="RUNNING")
          dapm_ops_left = (dapm_ops_left+ 1), dapm_running->count = (dapm_running->count+ 1), stat =
          alterlist(dapm_running->op,dapm_running->count),
          dapm_running->op[dapm_running->count].table_name = d.table_name
         ELSE
          dapm_ops_completed = (dapm_ops_completed+ 1)
         ENDIF
        WITH nocounter
       ;end select
       IF (check_error(dm_err->eproc))
       ...
       ENDIF
       CALL dapm_header(null)
       CALL text(5,10,concat("Operations complete: ",trim(cnvtstring(dapm_ops_completed))))
       CALL text(6,10,concat("Operations remaining: ",trim(cnvtstring(dapm_ops_left))))
       IF (dapm_ops_failed > 0)
        CALL video(b)
       ENDIF
       CALL text(7,10,concat("Failed operations: ",trim(cnvtstring(dapm_ops_failed))))
       IF (dapm_ops_failed > 0)
        CALL video(n)
       ENDIF
       CALL text(10,10,concat("Number of runners currently running : ",trim(cnvtstring(
           dapm_runners_running))))
       SET accept = time(30)
       IF (dapm_ops_failed > 0)
        CALL text(16,10,"(V)iew Operations, (F)ailed Operations Retry, (R)efresh Screen, (E)xit:")
        CALL accept(16,82,"A;CU","R"
         WHERE cnvtupper(curaccept) IN ("V", "F", "R", "E"))
       ELSE
        CALL text(16,10,"(V)iew Operations, (R)efresh Screen, (E)xit:")
        CALL accept(16,82,"A;CU","R"
       ...
       CASE (curaccept)
        OF "E":
         SET dapm_progress_refresh = 0
        OF "F":
         UPDATE  FROM dm2_ddl_ops_log d
          SET d.status = "INCOMPLETE", d.error_msg = ""
          WHERE d.op_type="PURGE ACTIVITY DATA"
           AND d.status="FAILED"
          WITH nocounter
         ;end update
         IF (check_error(dm_err->eproc))
        ...
        OF "V":
         CALL clear(16,2,130)
         CALL text(16,10,"(R)unning, (F)ailed, (C)ompleted, (A)ll, (E)xit:")
         CALL clear(18,2,130)
         CALL accept(16,59,"A;CU"," "
          WHERE cnvtupper(curaccept) IN ("R", "F", "C", "A", "E"))
         CASE (cnvtupper(curaccept))
          OF "R":
           SET dm2rpting->status_criteria =
           "ddol.status = 'RUNNING' and ddol.op_type  = 'PURGE ACTIVITY DATA' "
           SET dm2rpting->report_option = "Running Operations"
          OF "F":
           SET dm2rpting->status_criteria =
           "ddol.status = 'FAILED' and ddol.op_type   = 'PURGE ACTIVITY DATA' "
           SET dm2rpting->report_option = "Failed Operations"
          OF "C":
           SET dm2rpting->status_criteria =
           "ddol.status = 'COMPLETE' and ddol.op_type   = 'PURGE ACTIVITY DATA' "
           SET dm2rpting->report_option = "Completed Operations"
          OF "A":
           SET dm2rpting->status_criteria = concat(
            "ddol.status in ('COMPLETE','RUNNING','FAILED','INCOMPLETE') ",
            " and ddol.op_type   = 'PURGE ACTIVITY DATA'")
           SET dm2rpting->report_option = "All Operations"
         ENDCASE
         IF (cnvtupper(curaccept) != "E")
          SET message = nowindow
          SET dm2rpting->prompt_mode = "NOPROMPT"
          SET dm2rpting->run_id = dapm_run_id
          SET dm2rpting->install_mode = "SCHEMA_DATE"
          SET dm2rpting->schema_date = format(curdate,"DD-MMM-YYYY;;D")
...
#exit_program
 ...
 SET message = nowindow
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_ads_purge_menu has completed"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 IF ((validate(dm_ads_purge_main_menu,- (1))=- (1)))
  EXECUTE dm2_connect_to_dbase "DO"
 ENDIF
 SUBROUTINE dapm_header(null)
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL box(1,1,24,132)
   CALL text(2,2,concat("ACTIVITY DATA SAMPLE (ADS) PURGE - MANAGE PURGE PROCESS      ",
     "**Deletes will be run against the ",dapm_src_dbase_name," database**"))
   RETURN(1)
 END ;Subroutine
END GO
