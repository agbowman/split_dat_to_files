CREATE PROGRAM dm_ecmb_sch_warning:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_SCH_WARNING"
 ...
 SELECT INTO "nl:"
  w.*
  FROM sch_warning w
  WHERE (w.parent_id=request->xxx_combine[icombine].from_xxx_id)
   AND w.parent_table="ENCOUNTER"
   AND w.warn_type_meaning="SUSPENDPHY"
  DETAIL
   v_cust_count1 += 1
   IF (mod(v_cust_count1,10)=1)
    stat = alterlist(rreclist->from_rec,(v_cust_count1+ 9))
   ENDIF
   rreclist->from_rec[v_cust_count1].from_id = w.sch_warn_id
 ...
 SUBROUTINE (upt_from(s_uf_pk_id=f8,s_uf_to_fk_id=f8) =null)
   UPDATE  FROM sch_warning w
    SET w.updt_cnt = (w.updt_cnt+ 1), w.updt_id = reqinfo->updt_id, w.updt_applctx = reqinfo->
     updt_applctx,
     w.updt_task = reqinfo->updt_task, w.updt_dt_tm = cnvtdatetime(sysdate), w.parent_id =
     s_uf_to_fk_id
   ...
   SET request->xxx_combine_det[icombinedet].attribute_name = "SCH_WARN_ID"
   IF (curqual=0)
