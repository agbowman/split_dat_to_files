CREATE PROGRAM acm_chg_encntr_person_reltn:dba
 ...
 FOR (index = 1 TO xref->chg_cnt)
   SET reply->encntr_person_reltn_qual[xref->chg[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->chg_cnt)),
  ...
  SET e.encntr_person_reltn_id = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].
   encntr_person_reltn_id, e.beg_effective_dt_tm = cnvtdatetime(acm_request->
    encntr_person_reltn_qual[xref->chg[d.seq].idx].beg_effective_dt_tm), e.contact_role_cd =
   acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].contact_role_cd,
   e.contributor_system_cd =
   IF ((acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , e.encntr_id =
   IF ((acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].encntr_id > 0)) acm_request->
    encntr_person_reltn_qual[xref->chg[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].encntr_idx]
    .encntr_id
   ENDIF
   , e.end_effective_dt_tm = cnvtdatetime(acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx]
    .end_effective_dt_tm),
   e.free_text_cd = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].free_text_cd, e
   .ft_rel_person_name = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].
   ft_rel_person_name, e.genetic_relationship_ind = acm_request->encntr_person_reltn_qual[xref->chg[d
   .seq].idx].genetic_relationship_ind,
   e.internal_seq = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].internal_seq, e
   .living_with_ind = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].living_with_ind, e
   .person_reltn_cd = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].person_reltn_cd,
   e.person_reltn_type_cd = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].
   person_reltn_type_cd, e.priority_seq = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx]
   .priority_seq, e.related_person_id = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].
   related_person_id,
   e.related_person_reltn_cd = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].
   related_person_reltn_cd, e.visitation_allowed_cd = acm_request->encntr_person_reltn_qual[xref->
   chg[d.seq].idx].visitation_allowed_cd, e.active_ind = acm_request->encntr_person_reltn_qual[xref->
   chg[d.seq].idx].active_ind,
   e.active_status_cd = acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].active_status_cd,
  ...
  PLAN (d)
   JOIN (e
   WHERE (e.encntr_person_reltn_id=acm_request->encntr_person_reltn_qual[xref->chg[d.seq].idx].
   encntr_person_reltn_id))
  WITH nocounter, status(reply->encntr_person_reltn_qual[xref->chg[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->chg_cnt)
   IF ((reply->encntr_person_reltn_qual[xref->chg[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    ...
    WHERE expand(t1,f_val,t_val,e.encntr_person_reltn_id,acm_request->encntr_person_reltn_qual[xref->
     chg[t1].idx].encntr_person_reltn_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,e.encntr_person_reltn_id,acm_request->encntr_person_reltn_qual[
      xref->chg[t1].idx].encntr_person_reltn_id,
      max_val), idx = xref->chg[t2].idx
     IF ((((e.updt_cnt=acm_request->encntr_person_reltn_qual[idx].updt_cnt)) OR ((acm_request->
     force_updt_ind=1))) )
      reply->encntr_person_reltn_qual[idx].status = - (1), reply->encntr_person_reltn_qual[idx].
      encntr_person_reltn_id = acm_request->encntr_person_reltn_qual[idx].encntr_person_reltn_id
     ELSE
      failed = update_cnt_error
     ENDIF
     chg_str = acm_request->encntr_person_reltn_qual[idx].chg_str
     IF (findstring("BEG_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].beg_effective_dt_tm = e.beg_effective_dt_tm
     ENDIF
     IF (findstring("CONTACT_ROLE_CD,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].contact_role_cd = e.contact_role_cd
     ENDIF
     IF (findstring("CONTRIBUTOR_SYSTEM_CD,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].contributor_system_cd = e.contributor_system_cd
     ENDIF
     IF (findstring("ENCNTR_ID,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].encntr_id = e.encntr_id
     ENDIF
     IF (findstring("END_EFFECTIVE_DT_TM,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].end_effective_dt_tm = e.end_effective_dt_tm
     ENDIF
     IF (findstring("FREE_TEXT_CD,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].free_text_cd = e.free_text_cd
     ENDIF
     IF (findstring("FT_REL_PERSON_NAME,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].ft_rel_person_name = e.ft_rel_person_name
     ENDIF
     IF (findstring("GENETIC_RELATIONSHIP_IND,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].genetic_relationship_ind = e
      .genetic_relationship_ind
     ENDIF
     IF (findstring("INTERNAL_SEQ,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].internal_seq = e.internal_seq
     ENDIF
     IF (findstring("LIVING_WITH_IND,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].living_with_ind = e.living_with_ind
     ENDIF
     IF (findstring("PERSON_RELTN_CD,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].person_reltn_cd = e.person_reltn_cd
     ENDIF
     IF (findstring("PERSON_RELTN_TYPE_CD,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].person_reltn_type_cd = e.person_reltn_type_cd
     ENDIF
     IF (findstring("PRIORITY_SEQ,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].priority_seq = e.priority_seq
     ENDIF
     IF (findstring("RELATED_PERSON_ID,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].related_person_id = e.related_person_id
     ENDIF
     IF (findstring("RELATED_PERSON_RELTN_CD,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].related_person_reltn_cd = e.related_person_reltn_cd
     ENDIF
     IF (findstring("VISITATION_ALLOWED_CD,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].visitation_allowed_cd = e.visitation_allowed_cd
     ENDIF
     IF (findstring("ACTIVE_IND,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].active_ind = e.active_ind
     ENDIF
     IF (findstring("ACTIVE_STATUS_CD,",chg_str)=0)
      acm_request->encntr_person_reltn_qual[idx].active_status_cd = e.active_status_cd
     ENDIF
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->encntr_person_reltn_qual[xref->chg[index].idx].status=0))
