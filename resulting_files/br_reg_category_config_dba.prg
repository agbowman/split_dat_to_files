CREATE PROGRAM br_reg_category_config:dba
 ...
 SET readme_data->message = "Readme Failed: Starting <br_reg_category_config.prg> script"
 ...
 INSERT  FROM br_datamart_category b,
  ...
  SET b.br_datamart_category_id = seq(bedrock_seq,nextval), b.category_name = requestin->list_0[d.seq
   ...
   b.category_topic_mean = cnvtupper(br_existsinfo->list_0[d.seq].category_topic_mean), b.script_name
    = br_existsinfo->list_0[d.seq].script_name, b.updt_cnt = 0,
 ...
 IF (errcode > 0)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failure updating datamart categories >> ",errmsg)
  GO TO exit_script
 ELSE
  COMMIT
 ENDIF
 SET readme_data->status = "S"
 SET readme_data->message = "Readme Succeeded: <br_reg_category_config.prg> script"
#exit_script
 FREE RECORD br_existsinfo
END GO
