CREATE PROGRAM dm2_mig_create_macro_di
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dmcmd_logfile_prefix = vc WITH protect, constant("dm2_mig_create_macdi")
 DECLARE dmcmd_dir = vc WITH protect, noconstant("")
 DECLARE dmcmd_goldengate_version = i2 WITH protect, noconstant(0)
 IF (check_logfile(dmcmd_logfile_prefix,".log","dm2_mig_create_macro_di")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Verifying and storing input parameter."
 SET dmcmd_dir = trim( $1,3)
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating directory input parameter."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (size(dmcmd_dir)=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Invalid directory path value."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 IF (dmr_get_di_filter(null)=0)
  GO TO exit_program
 ENDIF
 IF (dmr_create_di_macro(dmcmd_dir,dmcmd_goldengate_version)=0)
  GO TO exit_program
 ENDIF
 GO TO exit_program
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "dm2_mig_create_macro_di completed successfully"
 CALL final_disp_msg(dmcmd_logfile_prefix)
END GO
