CREATE PROGRAM dm2_ref_data_mover_7817:dba
   ...
   ELSE
    ...
    IF ((cust_to_orig_id=- (3)))
     ...
     ELSE
      ...
      ELSEIF ((cust_to_orig_id=- (2)))
       SET dm2_ref_data_reply->error_ind = 1
       SET dm2_ref_data_reply->error_msg = "NOMV02"
       GO TO exit_7817
      ENDIF
     ENDIF
    ELSEIF (cust_to_orig_id > 0)
 ...
 SUBROUTINE cust_insert_dummy(null)
   ...
   INSERT  FROM sa_ref_action
    SET original_ref_action_id = cid_new_value, sa_ref_action_id = cid_new_value, version_nbr = 1,
     ...
     updt_id = 0, updt_dt_tm = cnvtdatetime(curdate,curtime3)
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 0
