CREATE PROGRAM dm2_mig_mock_final_trans:dba
 ...
 SUBROUTINE dmr_create_di_macro(dcdm_in_dir,dcdm_gg_version)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dm2_mig_mock_finaltrans_ind = i2 WITH protect, noconstant(1)
 IF (check_logfile("dm2_migmockft",".log","dm2_mig_mock_final_trans")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Starting dm2_mig_mock_final_trans"
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ENDIF
 IF (curqual=0)
  SET dm_err->eproc = "Inserting READ_ONLY_COMPLETE row."
  CALL disp_msg(" ",dm_err->logfile,0)
  INSERT  FROM dm2_admin_dm_info
   SET info_domain = "DM2MIG_FINAL_TRANS_CHECKPOINT", info_name = "READ_ONLY_COMPLETE"
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc)=1)
   ROLLBACK
 ...
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Deleting READ_ONLY_COMPLETE row."
 CALL disp_msg(" ",dm_err->logfile,0)
 DELETE  FROM dm2_admin_dm_info
  WHERE info_domain="DM2MIG_FINAL_TRANS_CHECKPOINT"
   AND info_name="READ_ONLY_COMPLETE"
  WITH nocounter
 ;end delete
...
#exit_program
 IF ((dm_err->err_ind=1))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ELSE
  SET dm_err->eproc = "dm2_mig_mock_final_trans completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_migmockft")
END GO
