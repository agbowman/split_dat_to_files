CREATE PROGRAM 1_ccl_eds_ffl
 ...
 DECLARE _times14biu0 = i4 WITH noconstant(0), public
 ...
 SUBROUTINE layoutsection0abs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    SET _rptstat = uar_rptrect(_hreport,(offsetx+ 0.06),(offsety+ 0.38),3.88,1.19,
    ...
    SET _oldfont = uar_rptsetfont(_hreport,_times14biu0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Baystate Medical Center",char(0)))
    SET rptsd->m_flags = 4
    SET rptsd->m_padding = bor(bor(rpt_sdtopborder,rpt_sdleftborder),rpt_sdrightborder)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Update Date and Time:",char(0)))
    ...
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_code128,(offsetx+ 0.19),(offsety+ 1.69))
    SET rptbce->m_recsize = 88
    SET rptbce->m_width = 2.67
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "1_ccl_eds_ffl"
 ...
 SUBROUTINE _createfonts(dummy)
   ...
   SET _times14biu0 = uar_rptcreatefont(_hreport,rptfont)
   SET rptfont->m_pointsize = 10
   SET rptfont->m_italic = rpt_off
   SET rptfont->m_underline = rpt_off
 ...
 END ;Subroutine
 DECLARE get_name_ids(dummy) = null WITH public
 SUBROUTINE get_name_ids(ncalc)
