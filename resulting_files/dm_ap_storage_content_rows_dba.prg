CREATE PROGRAM dm_ap_storage_content_rows:dba
 ...
 SET reply->table_name = "STORAGE_CONTENT"
 SET reply->rows_between_commit = 50
 DECLARE v_months_to_keep = i4 WITH protect, noconstant(- (1))
 DECLARE minimum_months_to_keep = i4 WITH protect, constant(120)
 ...
 SET stat = uar_get_meaning_by_codeset(2061,nullterm("DISCARDED"),1,ddiscardedstatuscd)
 IF (ddiscardedstatuscd=0.0)
  SET reply->err_code = - (1)
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18ngetmessage(i18nhandle,"DISCARDEDCD",
   "Unable to find code value by CDF meaning 'DISCARDED' in code set 2061.")
  GO TO exit_script
 ENDIF
 FOR (v_tok_ndx = 1 TO size(request->tokens,5))
   IF ((request->tokens[v_tok_ndx].token_str="MONTHSTOKEEP"))
    SET v_months_to_keep = ceil(cnvtreal(request->tokens[v_tok_ndx].value))
   ENDIF
 ENDFOR
 IF (v_months_to_keep < minimum_months_to_keep)
  SET reply->status_data.status = "F"
  SET reply->err_code = - (1)
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"BADMONTH",
 ...
 SELECT INTO "nl:"
  ...
  WHERE sc.updt_dt_tm <= cnvtdatetime(cnvtlookbehind(build(v_months_to_keep,",M")))
   AND sc.storage_content_id != 0
   AND parser(sbr_getrowidnotexists("sc.content_status_cd = dDiscardedStatusCd","sc"))
  DETAIL
   v_rows = (v_rows+ 1)
   IF (mod(v_rows,50)=1)
    stat = alterlist(reply->rows,(v_rows+ 49))
   ENDIF
   reply->rows[v_rows].row_id = sc.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,v_rows)
  WITH nocounter, maxqual(sc,value(request->max_rows))
 ;end select
 SET reply->err_code = error(v_errmsg2,0)
 IF ((reply->err_code != 0))
  SET reply->status_data.status = "F"
  SET reply->err_msg = uar_i18nbuildmessage(i18nhandle,"COLLECTERR",
   "Failed collecting rows for purging: %1","s",nullterm(v_errmsg2))
  GO TO exit_script
 ENDIF
