CREATE PROGRAM dm2_refresh_v500_user
 ...
 SUBROUTINE der_compile_oos_pkg(dcop_src_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drvu_tgt_dbase_name = vc WITH protect, noconstant("")
 DECLARE drvu_src_dbase_name = vc WITH protect, noconstant("")
 DECLARE drvu_src_envid = f8 WITH protect, noconstant(0.0)
 DECLARE drvu_tgt_envid = f8 WITH protect, noconstant(0.0)
 DECLARE drvu_tgt_env_name = vc WITH protect, noconstant("")
 DECLARE drvu_src_env_name = vc WITH protect, noconstant("")
 DECLARE drvu_new_envid = f8 WITH protect, noconstant(0.0)
 DECLARE drvu_sys_pword = vc WITH protect, noconstant("")
 DECLARE drvu_sys_connect_str = vc WITH protect, noconstant("")
 DECLARE drvu_drop_tspace = vc WITH protect, noconstant("")
 DECLARE drvu_user_ts_map_ind = i2 WITH protect, noconstant(0)
 DECLARE drvu_standalone = i2 WITH protect, noconstant(0)
 DECLARE drvu_sys_uname = vc WITH protect, noconstant("")
 DECLARE drvu_tgt_adb_ind = i2 WITH protect, noconstant(0)
 DECLARE drvu_get_envinfo(dge_envname=vc(ref),dge_envid=f8(ref)) = i2
 DECLARE drvu_confirm_screen(null) = i2
 DECLARE drvu_setup(null) = i2
 DECLARE drvu_temp_v500_pwd = vc WITH protect, constant("DM2refresh_temp_pwd1")
 DECLARE drvu_get_clin_tspaces = i2 WITH protect, noconstant(1)
 IF ((dm2_install_schema->curprog="NONE"))
  SET drvu_standalone = 1
 ENDIF
 IF (check_logfile("dm2_ref_v500",".log","dm2_refresh_v500_user")=0)
  GO TO exit_program
 ENDIF
 IF (drvu_standalone=1
  AND (validate(dm2_bypass_rv5u_standalone,- (2))=- (2)))
  SET dm_err->eproc = "Verifying if dm2_refresh_v500_user script is executed standalone."
  SET dm_err->emsg = "Cannot run dm2_refresh_v500_user script standalone."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (validate(dm2_force_drop_all_tablespaces,0)=1)
  SET drvu_get_clin_tspaces = 0
 ENDIF
 SET dm_err->eproc = "Starting dm2_refresh_v500_user"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (drvu_setup(null)=0)
  GO TO exit_program
 ENDIF
 SET des_env_data->env_name = drvu_tgt_env_name
 IF (des_get_env_data(1)=0)
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 SET dm2_install_schema->dbase_name = '"TARGET"'
 SET dm2_install_schema->u_name = drvu_sys_uname
 SET dm2_install_schema->p_word = drvu_sys_pword
 SET dm2_install_schema->connect_str = drvu_sys_connect_str
 EXECUTE dm2_connect_to_dbase "CO"
 IF ((dm_err->err_ind=1))
  RETURN(0)
 ENDIF
 IF (drvu_tgt_adb_ind=0)
  IF (dcdur_prompt_tspaces("V500",cnvtupper(currdbname),drvu_user_ts_map_ind)=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (drvu_tgt_adb_ind=1
  AND validate(drrr_responsefile_in_use,0)=1)
  EXECUTE dm2_create_database_link drrr_rf_data->tgt_db_adm_db_link_name, drrr_rf_data->
  adm_db_link_cnct_desc, drrr_rf_data->adm_db_user,
  drrr_rf_data->adm_db_user_pwd, drrr_rf_data->adm_db_link_host, drrr_rf_data->adm_db_link_port,
  drrr_rf_data->adm_db_link_svc_nm, drrr_rf_data->adm_db_cred_nm, 1,
  1
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
 ...
 IF (validate(drrr_responsefile_in_use,- (1))=1)
  SET dm2_drop_user_no_confirmation = 1
 ENDIF
 IF ((validate(dm2_bypass_drop_user_v500,- (1))=- (1)))
  EXECUTE dm2_drop_user "V500"
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
 ENDIF
 IF (drvu_drop_tspace="Y")
  EXECUTE dm2_drop_tspaces
  IF ((dm_err->err_ind=1))
   GO TO exit_program
  ENDIF
 ENDIF
 SET dcdur_user_data->tgt_sys_user = drvu_sys_uname
 SET dcdur_user_data->tgt_sys_pwd = drvu_sys_pword
 EXECUTE dm2_create_users "V500"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 IF (drvu_tgt_adb_ind=1
  ...
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echo(concat("Before dropping database link/credential, connected to ",trim(currdbname)," as ",
     currdbuser,"."))
  ENDIF
  IF (drr_drop_db_link(drrr_rf_data->tgt_db_adm_db_link_name)=0)
   GO TO exit_program
  ENDIF
  EXECUTE dm2_manage_database_credential "DROP", drrr_rf_data->adm_db_cred_nm, drrr_rf_data->
  adm_db_user,
  ""
  IF ((dm_err->err_ind=1))
   GO TO exit_program
 ...
 IF (validate(drrr_responsefile_in_use,- (1))=1)
  IF ((validate(dm2_bypass_dp_oos,- (1))=- (1)))
   IF ((((drrr_rf_data->src_db_deploy_config IN ("ADB", "OCI"))) OR ((drrr_rf_data->
   tgt_db_deploy_config IN ("ADB", "OCI")))) )
    SET dm_err->eproc = "Validate DBMS_CLOUD in Target"
    CALL disp_msg(" ",dm_err->logfile,0)
    SET der_expimp_data->tgt_sys_pass = dcdur_user_data->tgt_sys_pwd
 ...
 ENDIF
 GO TO exit_program
 SUBROUTINE drvu_get_envinfo(dge_envname,dge_envid)
   SET dm_err->eproc = "Query for environment_Name and environment_id"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   SELECT INTO "nl:"
    ...
    DETAIL
     IF (d.info_name="DM_ENV_NAME")
      dge_envname = d.info_char
     ELSE
      dge_envid = d.info_number
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    ...
    RETURN(0)
   ENDIF
   IF (((dge_envname="") OR (dge_envid=0)) )
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Environment is incomplete. Environment_Name:",dge_envname,
     " Environment_Id:",build(dge_envid))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drvu_confirm_screen(null)
   SET message = window
   CALL clear(1,1)
   CALL box(1,1,8,132)
   CALL text(3,2,
    "Do you want to drop Cerner clinical tablespaces? (Y or N):    (Note:  Only empty tablespaces will be dropped.)"
    )
   CALL accept(3,61,"p;cu",""
    WHERE curaccept IN ("Y", "N"))
   SET drvu_drop_tspace = curaccept
   CALL clear(1,1)
   CALL box(1,1,24,132)
   CALL text(2,2,"SOURCE and TARGET Database Information Confirmation")
   CALL text(4,2,"SOURCE")
   CALL text(4,60,"TARGET")
   CALL text(5,2,concat("Environment Name : ",trim(drvu_src_env_name)))
   CALL text(5,60,concat("Environment Name : ",trim(drvu_tgt_env_name)))
   CALL text(6,2,concat("Database Name : ",trim(drvu_src_dbase_name)))
   CALL text(6,60,concat("Database Name : ",trim(drvu_tgt_dbase_name)))
   IF (drvu_drop_tspace="Y")
    CALL text(8,2,"****USER HAS ELECTED TO REMOVE CERNER CLINICAL TABLESPACES****!")
   ELSE
    CALL text(8,2,"User has elected to keep Cerner clinical tablespaces.")
   ENDIF
   CALL text(10,2,
    "Please review the above information before proceeding on with the DROP and CREATION of the V500 user"
    )
   CALL text(11,2,concat("in environment ",drvu_tgt_env_name,", database ",drvu_tgt_dbase_name,"."))
   CALL text(13,2,concat("Continuing this process will create a new environment id for the ",
     drvu_tgt_env_name))
   CALL text(14,2,concat(
     "environment to force the creation of new history and will permanently drop ",
     "all V500 objects and data in database ",drvu_tgt_dbase_name,"!"))
   CALL text(16,2,"Enter 'C' to continue or 'Q' to quit (C or Q):")
   CALL accept(16,49,"p;cu",""
    WHERE curaccept IN ("C", "Q"))
   SET message = nowindow
   IF (curaccept="Q")
    SET message = nowindow
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE drvu_setup(null)
   DECLARE ds_v500_exists = i2 WITH protect, noconstant(0)
   DECLARE ds_v500_connect_str = vc WITH protect, noconstant("DM2NOTSET")
   DECLARE ds_pos1 = i4 WITH protect, noconstant(0)
   DECLARE ds_v500_pwd_set_to_temp = i2 WITH protect, noconstant(0)
   DECLARE ds_env_selection_count = i4 WITH protect, noconstant(0)
   SET dm2_install_schema->dbase_name = '"SOURCE"'
   SET dm2_install_schema->u_name = "V500"
   IF (validate(drrr_responsefile_in_use,- (1))=1)
   ...
   ENDIF
   SET drvu_src_dbase_name = currdbname
   SET dm_err->eproc = "Get source environment_name and environment_id"
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg(" ",dm_err->logfile,0)
   ENDIF
   IF (drvu_get_envinfo(drvu_src_env_name,drvu_src_envid)=0)
    RETURN(0)
   ENDIF
   IF (drvu_get_clin_tspaces=1)
    IF (dtr_load_clin_tspaces(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   SET dm2_install_schema->dbase_name = '"TARGET"'
   ...
   IF ((dm_err->err_ind=1))
    RETURN(0)
   ENDIF
   SET drvu_sys_uname = dm2_install_schema->u_name
   SET drvu_sys_pword = dm2_install_schema->p_word
   SET drvu_sys_connect_str = dm2_install_schema->connect_str
   IF (dm2_adb_check("",drvu_tgt_adb_ind)=0)
    RETURN(0)
   ENDIF
   SET drvu_tgt_dbase_name = currdbname
   IF (drvu_tgt_dbase_name=drvu_src_dbase_name
    AND (validate(dm2_allow_same_db_name,- (1))=- (1)))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Source:",drvu_src_dbase_name," and Target:",trim(drvu_tgt_dbase_name),
     " databases may not be the same.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (validate(drrr_responsefile_in_use,- (1))=1)
    IF (cnvtupper(drvu_tgt_dbase_name) != cnvtupper(drrr_rf_data->tgt_db_name))
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Current database name (",drvu_tgt_dbase_name,
      ") does not match Target database name"," specified in response file (",drrr_rf_data->
      tgt_db_name,
      ")")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->eproc = "Verify that V500 user is a valid user in target"
   CALL disp_msg(" ",dm_err->logfile,0)
   SELECT INTO "nl:"
    FROM dba_users du
    WHERE cnvtupper(du.username)="V500"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual > 0)
    SET ds_v500_exists = 1
   ENDIF
   IF (validate(drrr_responsefile_in_use,0)=0)
    SET dm_err->eproc = "Displaying prompt for Target Environment Name."
    CALL disp_msg(" ",dm_err->logfile,0)
    SET width = 132
    SET message = window
    CALL box(1,1,8,80)
    CALL text(3,2,"Help is available via <Shift><F5>")
    ...
    SELECT DISTINCT
     ...
     WITH nocounter
    ;end select
    CALL accept(5,30,"P(20);CU"," "
     WHERE build(curaccept) > "")
    SET help = off
    SET drvu_tgt_env_name = cnvtupper(build(curaccept))
    SET message = nowindow
    CALL clear(1,1)
   ENDIF
   IF (validate(drrr_responsefile_in_use,0)=1
    AND ds_v500_exists=1
    AND (validate(dm2_bypass_drop_user_v500,- (1))=- (1)))
    SET dm_err->eproc = concat("Set Target V500 user password to temporary value to prevent ",
     "'reconnections' allowing V500 user to be dropped.")
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    IF (dm2_push_cmd(concat('rdb asis(^alter user V500 identified by "',drvu_temp_v500_pwd,'"^) go'),
     1)=0)
     RETURN(0)
    ENDIF
   ENDIF
   ...
   ENDIF
   IF (validate(drrr_responsefile_in_use,- (1))=1)
    SET drvu_tgt_env_name = drrr_rf_data->tgt_db_env_name
    SET dm_err->eproc = "Get environment_id via environment name."
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    SELECT INTO "nl:"
     FROM dm_environment d
     WHERE cnvtupper(d.environment_name)=drvu_tgt_env_name
     DETAIL
      drvu_tgt_envid = d.environment_id, ds_v500_connect_str = trim(d.v500_connect_string)
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    IF (curqual=0)
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Unable to obtain environment_id for environment_Name:",
      drvu_tgt_env_name)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    ...
    SET drvu_drop_tspace = evaluate(drrr_rf_data->tgt_drop_tablespaces,"YES","Y","N")
    IF (ds_v500_exists=1
     AND (validate(dm2_bypass_drop_user_v500,- (1))=- (1)))
     SET ds_v500_connect_str = build("V500/",dm2_install_schema->v500_p_word,"@",dm2_install_schema->
      v500_connect_str)
     SET dm_err->eproc = "Update connect string for V500 in dm_environment"
     IF ((dm_err->debug_flag > 0))
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
     UPDATE  FROM dm_environment d
      SET d.v500_connect_string = ds_v500_connect_str
      WHERE d.environment_id=drvu_tgt_envid
      WITH nocounter
     ;end update
     IF (check_error(dm_err->eproc)=1)
      ROLLBACK
     ...
     ENDIF
     COMMIT
     SET dm2_install_schema->v500_p_word = drvu_temp_v500_pwd
    ENDIF
   ELSE
    IF (ds_v500_exists=1)
     SET dm2_install_schema->dbase_name = '"TARGET"'
     SET dm2_install_schema->u_name = "V500"
     SET dm2_install_schema->p_word = drvu_temp_v500_pwd
     SET dm2_install_schema->connect_str = drvu_sys_connect_str
     EXECUTE dm2_connect_to_dbase "CO"
     IF ((dm_err->err_ind=1))
      SET dm_err->err_ind = 0
      SET dm_err->eproc =
      "Above error message is ignorable.  Will proceed with prompt for V500 connect information."
      CALL disp_msg(" ",dm_err->logfile,0)
     ELSE
      SET ds_v500_pwd_set_to_temp = 1
     ENDIF
     IF (ds_v500_pwd_set_to_temp=0)
      SET dm2_install_schema->dbase_name = '"TARGET"'
      SET dm2_install_schema->u_name = "V500"
      SET dm2_force_connect_string = 1
      EXECUTE dm2_connect_to_dbase "PC"
    ...
    IF (ds_v500_exists=1
     ...
     COMMIT
    ENDIF
    IF (((ds_v500_exists=0) OR (ds_v500_pwd_set_to_temp=1)) )
     IF (ds_v500_connect_str != "DM2NOTSET"
      AND trim(ds_v500_connect_str) != "")
      IF (findstring("V500/",ds_v500_connect_str,1,0) > 0)
       SET ds_pos1 = findstring("@",ds_v500_connect_str,1,0)
       IF (ds_pos1 > 0)
        SET dm2_install_schema->v500_p_word = substring(6,(ds_pos1 - 6),ds_v500_connect_str)
       ELSE
        SET dm2_install_schema->v500_p_word = substring(6,(textlen(ds_v500_connect_str) - 6),
         ds_v500_connect_str)
       ENDIF
       SET ds_v500_pwd_set_to_temp = 0
      ENDIF
     ENDIF
...
#exit_program
 IF ((dm_err->err_ind=1))
  ...
  SET dm_err->emsg = concat(dm_err->emsg,"  DATABASE CONNECTION REMOVED.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  CALL final_disp_msg("dm2_ref_v500")
 ELSE
  CALL parser("free define oraclesystem go",1)
  SET dm_err->eproc = concat("Script dm2_refresh_v500_user completed successfully.  ",
   "DATABASE CONNECTION REMOVED. RE-ENTER CCL TO ESTABLISH NEW CONNECTION.")
  CALL final_disp_msg("dm2_ref_v500")
 ENDIF
END GO
