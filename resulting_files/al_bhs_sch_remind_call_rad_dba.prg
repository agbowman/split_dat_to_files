CREATE PROGRAM al_bhs_sch_remind_call_rad:dba
 EXECUTE bhs_hlp_ftp
 ...
 DECLARE ms_file = vc WITH protect, constant(concat("rad_reminder_call_",trim(cnvtstring(rand(0),20),
 ...
 SELECT INTO "nl:"
  ...
  WITH nocounter
 ;end select
 IF (ms_weekday="Sunday")
  CALL echo("EXIT, SHOULD NOT RUN")
  GO TO exit_script
 ELSEIF (ms_weekday="Saturday")
  SET mf_start_date = cnvtdatetime((curdate+ 5),000000)
  SET mf_stop_date = cnvtdatetime((curdate+ 6),235959)
 ELSE
  SET mf_start_date = cnvtdatetime((curdate+ 5),000000)
