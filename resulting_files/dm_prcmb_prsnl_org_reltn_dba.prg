CREATE PROGRAM dm_prcmb_prsnl_org_reltn:dba
 ...
 ENDIF
 DECLARE rev_end_eff(res_por_id=f8,res_tu_cnt=i4) = i2
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_PRCMB_PRSNL_ORG_RELTN"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   .prev_end_eff_dt_tm = frm.end_effective_dt_tm
 ...
 IF ((rev_cmb_request->reverse_ind=1))
  SELECT INTO "nl:"
   ...
   DETAIL
    ...
    ENDIF
    rreclist->to_rec[count2].prsnl_org_reltn_id = tu.prsnl_org_reltn_id, rreclist->to_rec[count2].
    prev_end_eff_dt_tm = tu.end_effective_dt_tm
   FOOT REPORT
  ...
  IF (count2 > 0)
   FOR (loopcount = 1 TO count2)
     IF (revend_eff(rreclist->to_rec[loopcount].prsnl_org_reltn_id,loopcount)=0)
      GO TO end_of_program
     ENDIF
   ENDFOR
  ENDIF
  IF (count1 > 0)
   FOR (loopcount = 1 TO count1)
     IF (upt_from(rreclist->from_rec[loopcount].prsnl_org_reltn_id)=0)
      GO TO end_of_program
     ENDIF
   ENDFOR
  ENDIF
 ...
 SUBROUTINE end_eff(s_por_id,s_frm_count)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE revend_eff(res_por_id,res_tu_count)
   UPDATE  FROM prsnl_org_reltn por
    SET por.end_effective_dt_tm = cnvtdatetime(sysdate), por.updt_dt_tm = cnvtdatetime(sysdate), por
     .updt_id = reqinfo->updt_id,
     por.updt_applctx = reqinfo->updt_applctx, por.updt_task = reqinfo->updt_task, por.updt_cnt = (
     por.updt_cnt+ 1)
    WHERE por.prsnl_org_reltn_id=res_por_id
    WITH nocounter
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
   ...
   SET request->xxx_combine_det[icombinedet].entity_name = "PRSNL_ORG_RELTN"
   SET request->xxx_combine_det[icombinedet].attribute_name = "PERSON_ID"
   SET request->xxx_combine_det[icombinedet].prev_end_eff_dt_tm = rreclist->to_rec[res_tu_count].
   prev_end_eff_dt_tm
   SET stat = alterlist(request->xxx_combine_det[icombinedet].entity_pk,1)
   SET request->xxx_combine_det[icombinedet].entity_pk[1].col_name = "PRSNL_ORG_RELTN_ID"
   SET request->xxx_combine_det[icombinedet].entity_pk[1].data_type = "NUMBER"
   SET request->xxx_combine_det[icombinedet].entity_pk[1].data_number = res_por_id
   IF (curqual=0)
    SET failed = delete_error
    SET request->error_message = build(
     "DEL_FROM: No values found on the prsnl_org_reltn table with prsnl_org_reltn_id = ",res_por_id)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (upt_from(ufs_por_id=f8) =i2)
   UPDATE  FROM prsnl_org_reltn frm
    ...
    WHERE frm.prsnl_org_reltn_id=ufs_por_id
   ;end update
   SET icombinedet += 1
   SET stat = alterlist(request->xxx_combine_det,icombinedet)
   SET request->xxx_combine_det[icombinedet].combine_action_cd = upt
   SET request->xxx_combine_det[icombinedet].entity_id = ufs_por_id
   SET request->xxx_combine_det[icombinedet].entity_name = "PRSNL_ORG_RELTN"
   SET request->xxx_combine_det[icombinedet].attribute_name = "PERSON_ID"
   SET stat = alterlist(request->xxx_combine_det[icombinedet].entity_pk,1)
   SET request->xxx_combine_det[icombinedet].entity_pk[1].col_name = "PRSNL_ORG_RELTN_ID"
   SET request->xxx_combine_det[icombinedet].entity_pk[1].data_type = "NUMBER"
   SET request->xxx_combine_det[icombinedet].entity_pk[1].data_number = ufs_por_id
   IF (curqual=0)
    SET failed = update_error
    SET request->error_message = concat("Couldn't update prsnl_org_reltn record with person_id=",
     cnvtstring(ufs_por_id))
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
