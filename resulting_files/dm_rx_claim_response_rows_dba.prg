CREATE PROGRAM dm_rx_claim_response_rows:dba
 ...
 SET reply->table_name = "RX_CLAIM_RESPONSE"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minimumvalue = min(cr.rx_claim_response_id)
   FROM rx_claim_response cr
   WHERE cr.rx_claim_response_id > 0.0
   DETAIL
 ...
 ENDIF
 SELECT INTO "n1:"
  maximumvalue = max(cr.rx_claim_response_id)
  FROM rx_claim_response cr
  DETAIL
 ...
 WHILE (dbatchlowerboundid <= dmaximumid
  AND lrowsleft > 0)
   SELECT INTO "nl:"
    cr.rx_claim_response_id
    FROM rx_claim_response cr
    WHERE cr.rx_claim_response_id > 0
     AND parser(sbr_getrowidnotexists(
      "cr.rx_claim_response_id between dbatchLowerBoundId and dbatchUpperBoundId","cr"))
     AND cr.updt_dt_tm <= cnvtdatetime((curdate - ldaystokeep),235959)
    DETAIL
     recordcount += 1
     IF (mod(recordcount,100)=1)
      stat = alterlist(reply->rows,(recordcount+ 99))
     ENDIF
     reply->rows[recordcount].row_id = cr.rowid
    WITH nocounter, maxqual(cr,value(lrowsleft))
   ;end select
