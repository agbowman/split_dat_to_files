CREATE PROGRAM dm_mic_bpa_worklist_rows:dba
 ...
 SET reply->table_name = "MIC_BPA_WORKLIST"
 ...
 SELECT INTO "nl:"
  mb.rowid
  FROM mic_bpa_worklist mb
  WHERE parser(sbr_getrowidnotexists("mb.worklist_dt_tm <= cnvtdatetime(purge_dt_tm)","mb"))
   AND mb.finalized_ind=1
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = (cnt+ 1)
   IF (mod(cnt,20)=1)
    stat = alterlist(reply->rows,(cnt+ 19))
   ENDIF
   reply->rows[cnt].row_id = mb.rowid
  FOOT REPORT
   stat = alterlist(reply->rows,cnt)
  WITH nocounter, maxqual(mb,value(cnvtint(request->max_rows)))
 ;end select
