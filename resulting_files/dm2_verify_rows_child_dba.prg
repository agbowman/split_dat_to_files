CREATE PROGRAM dm2_verify_rows_child:dba
 ...
 DECLARE dvr_cnt = i4
 DECLARE dvr_loop = i4
 DECLARE dvr_error_ind = i2
 SELECT INTO "NL:"
  FROM dm_chg_log d
  WHERE d.table_name=dvr_table
   AND d.log_id >= dvr_min
   AND (d.log_id < (dvr_min+ 10000))
   AND d.delete_ind=0
   AND d.log_type="REFCHG"
  HEAD REPORT
   dvr_cnt = 0, stat = alterlist(rowstofix->qual,1000)
  DETAIL
   dvr_cnt = (dvr_cnt+ 1)
   IF (mod(dvr_cnt,1000)=1
    AND dvr_cnt != 1)
    stat = alterlist(rowstofix->qual,(dvr_cnt+ 999))
   ENDIF
   rowstofix->qual[dvr_cnt].pk_where = d.pk_where, rowstofix->qual[dvr_cnt].log_id = d.log_id
  FOOT REPORT
   stat = alterlist(rowstofix->qual,dvr_cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (dvr_cnt=0)
  GO TO exit_child
 ENDIF
 FOR (dvr_loop = 1 TO dvr_cnt)
   UPDATE  FROM dm_chg_log
    SET log_type = "BADLOG"
    WHERE (log_id=rowstofix->qual[dvr_loop].log_id)
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dvr_error_ind = 1
    SET dvr_loop = dvr_cnt
   ENDIF
   CALL parser(concat("update into ",dvr_table," ",dvr_suffix," set ",
     dvr_column," = ",dvr_column),0)
   CALL parser(concat(rowstofix->qual[dvr_loop].pk_where," go "),1)
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dvr_error_ind = 1
    ROLLBACK
