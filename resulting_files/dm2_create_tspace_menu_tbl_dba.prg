CREATE PROGRAM dm2_create_tspace_menu_tbl:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE schema_dt_str = c8 WITH public, noconstant("")
 DECLARE overall_size = f8 WITH public, noconstant(0.0)
 DECLARE tbl_name = vc WITH public, noconstant(" ")
 DECLARE v_db2_str = vc WITH public, noconstant(" ")
 DECLARE db2_stat = i4 WITH public, noconstant(0)
 DECLARE temp_file = vc WITH public, noconstant(" ")
 DECLARE total_space = f8 WITH public, noconstant(0.0)
 DECLARE ts_ratio = f8 WITH public, noconstant(0.0)
 DECLARE bytes_needed = f8 WITH public, noconstant(0.0)
 DECLARE temp_bytes = f8 WITH public, noconstant(0.0)
 DECLARE temp_ttl_extents = f8 WITH public, noconstant(0.0)
 DECLARE temp_floor = f8 WITH public, noconstant(0.0)
 DECLARE factor_nbr = f8 WITH public, noconstant(0.0)
 DECLARE min_tbl_ext = i4 WITH public, constant(2)
 DECLARE min_long_ext = i4 WITH public, constant(4)
 DECLARE min_ind_ext = i4 WITH public, constant(2)
 DECLARE temp_long_ext = f8 WITH public, noconstant(0.0)
 DECLARE data_floor = f8 WITH public, noconstant(0.0)
 DECLARE long_floor = f8 WITH public, noconstant(0.0)
 DECLARE long_ext_to_add = f8 WITH public, noconstant(0.0)
 DECLARE first_long_idx = i4 WITH public, noconstant(0)
 DECLARE mb_for_ddlops = i4 WITH public, noconstant(0)
 DECLARE ext_for_ddlops = i4 WITH public, noconstant(0)
 DECLARE dtp_temp_bytes = f8 WITH public, noconstant(0.0)
 DECLARE tmp_tname = vc WITH public, noconstant
 DECLARE tmp_cname = vc WITH public, noconstant
 DECLARE ext_ceil_bytes = f8 WITH public, constant(4194304.0)
 DECLARE ext_overhead = f8 WITH public, constant(4.0)
 DECLARE db2_std_extsize = f8 WITH public, constant(262144.0)
 DECLARE ctm_prec = i4 WITH public
 DECLARE ctm_default_tspace = vc WITH public
 RECORD dm_tspaces(
   1 long_cnt = i4
   1 lqual[*]
     2 table_name = vc
   ...
   1 qual[*]
     ...
     2 segment_type = vc
     2 ttl_extents = f8
   1 total_data_space = i4
   1 total_index_space = i4
   1 total_long_space = i4
 )
 DECLARE create_tbl(null) = i2
 DECLARE get_tspace_detail(null) = i2
 DECLARE insert_table(null) = i2
 IF (check_logfile("dm2_rpt_tspace",".err"," "))
  SET dm_err->errfile = dm_err->logfile
 ENDIF
 SET schema_dt_str =  $1
 SET overall_size = cnvtreal( $2)
 SET factor_nbr = cnvtreal( $3)
 IF ( NOT (create_tbl(null)))
  GO TO exit_script
 ENDIF
 IF ( NOT (get_tspace_detail(null)))
  GO TO exit_script
 ENDIF
 IF ( NOT (insert_table(null)))
  GO TO exit_script
 ENDIF
 CALL echo(build("Overall size passed in (Mbytes) = ",(overall_size/ 1048576)))
 CALL echo(build("Total space used (Mbytes) = ",(total_space/ 1048576)))
 CALL echo(build("Overall size:Total space ratio =  ",ts_ratio))
 SUBROUTINE create_tbl(null)
   IF (((cnvtupper(currdbuser)="CDBA") OR (cnvtupper(currdbuser)="ADMIN_TMP")) )
    SET tbl_name = concat("dm2a",schema_dt_str,"_tmpsize")
   ELSEIF (cnvtupper(currdbuser)="V500")
    SET tbl_name = concat("dm2c",schema_dt_str,"_tmpsize")
   ELSE
    SET dm_err->eproc = "Invalid user"
    CALL disp_msg(dm_err->eproc,dm_err->logfile,0)
    RETURN(0)
   ENDIF
   SET v_db2_str = concat("rdb create table ",tbl_name,"(tspace_name  varchar2(18) not null,",
    " cur_bytes_allocated float,"," bytes_needed float,",
    " new_ind number,"," extent_size_bytes float,"," install_status varchar2(20),",
    " err_msg varchar2(500)) go")
   IF (dm2_push_cmd(v_db2_str,1))
    SET v_db2_str = concat('oragen3 "',tbl_name,'" go')
    IF (dm2_push_cmd(v_db2_str,1))
     RETURN(1)
    ELSE
     RETURN(0)
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE get_tspace_detail(null)
   DECLARE found = i4
   DECLARE table_tspace = vc
   DECLARE long_tspace = vc
   IF (currdbuser != "ADMIN_TMP"
    ...
    SELECT INTO "nl:"
     FROM dm_ts_precedence dtp,
      user_segments us,
      user_tables ut
     WHERE us.segment_name=dtp.table_name
      AND us.segment_type="TABLE"
      AND dtp.precedence=1
      AND substring(1,1,us.tablespace_name) IN ("I", "D", "L")
      AND ut.table_name=us.segment_name
     ORDER BY dtp.data_tablespace
     HEAD dtp.data_tablespace
      dm_tspaces->tspace_cnt = (dm_tspaces->tspace_cnt+ 1), stat = alterlist(dm_tspaces->qual,
       dm_tspaces->tspace_cnt), dm_tspaces->qual[dm_tspaces->tspace_cnt].tspace_name = dtp
      .data_tablespace,
      dm_tspaces->qual[dm_tspaces->tspace_cnt].extent = db2_std_extsize, dm_tspaces->qual[dm_tspaces
      ->tspace_cnt].bytes = 0, dm_tspaces->qual[dm_tspaces->tspace_cnt].segment_type = "TABLE",
      dm_tspaces->qual[dm_tspaces->tspace_cnt].ttl_extents = 0, cnt = 0, temp_bytes = 0.0,
      temp_ttl_extents = 0, temp_floor = (dm_tspaces->qual[dm_tspaces->tspace_cnt].extent *
      min_tbl_ext)
     DETAIL
      temp_bytes = ((ut.num_rows * 8)+ (us.bytes * factor_nbr))
      IF (temp_bytes < temp_floor)
       temp_ttl_extents = (temp_ttl_extents+ min_tbl_ext)
      ELSE
       temp_ttl_extents = (temp_ttl_extents+ ceil((temp_bytes/ dm_tspaces->qual[dm_tspaces->
        tspace_cnt].extent)))
      ENDIF
      cnt = (cnt+ 1)
     FOOT  dtp.data_tablespace
      temp_ttl_extents = ceil((temp_ttl_extents * 1.2)), dm_tspaces->qual[dm_tspaces->tspace_cnt].
      ttl_extents = (temp_ttl_extents+ ext_overhead), dm_tspaces->qual[dm_tspaces->tspace_cnt].bytes
       = (dm_tspaces->qual[dm_tspaces->tspace_cnt].ttl_extents * dm_tspaces->qual[dm_tspaces->
      tspace_cnt].extent)
     WITH nocounter
    ;end select
    SET ctm_prec = 1
    SET ctm_default_tspace = "D_A_SMALL"
   ELSE
    SET dm_tspaces->tspace_cnt = (dm_tspaces->tspace_cnt+ 1)
    SET stat = alterlist(dm_tspaces->qual,dm_tspaces->tspace_cnt)
    SET dm_tspaces->qual[dm_tspaces->tspace_cnt].tspace_name = "D_TOOLKIT"
    SET dm_tspaces->qual[dm_tspaces->tspace_cnt].extent = db2_std_extsize
    SET dm_tspaces->qual[dm_tspaces->tspace_cnt].bytes = 0
    SET dm_tspaces->qual[dm_tspaces->tspace_cnt].segment_type = "TABLE"
    SET dm_tspaces->qual[dm_tspaces->tspace_cnt].ttl_extents = 0
    SET ctm_prec = 0
    SET ctm_default_tspace = "D_TOOLKIT"
    SELECT INTO "nl:"
     ...
     WHERE us.segment_type="TABLE"
      AND ut.table_name=us.segment_name
     HEAD REPORT
      temp_bytes = 0.0, temp_ttl_extents = 0, temp_floor = (dm_tspaces->qual[dm_tspaces->tspace_cnt].
      extent * min_tbl_ext),
      cnt = 0
     DETAIL
      temp_bytes = ((ut.num_rows * 8)+ (us.bytes * factor_nbr))
   ...
   IF (currdbuser != "ADMIN_TMP"
    AND currdbuser != "CDBA")
    SELECT INTO "nl:"
     y = sum(utc1.avg_col_len), ut.table_name, dtp.long_tablespace,
     ut.avg_row_len, ut.num_rows, dtp.data_tablespace,
     dtp.long_extent_size
     FROM dm_ts_precedence dtp,
      user_tables ut,
      user_tab_columns utc1,
      user_tab_columns utc
     WHERE utc.table_name=dtp.table_name
      AND utc.table_name=ut.table_name
      AND utc1.table_name=ut.table_name
      AND dtp.precedence=1
      AND utc.data_type="LONG*"
      AND substring(1,1,dtp.long_tablespace) IN ("D", "I", "L")
     GROUP BY ut.table_name, dtp.long_tablespace, ut.avg_row_len,
      ut.num_rows, dtp.data_tablespace, dtp.long_extent_size
     HEAD REPORT
      first_long_idx = (dm_tspaces->tspace_cnt+ 1)
     DETAIL
      tmp_tname = ut.table_name, tmp_cname = utc1.column_name, data_floor = (min_tbl_ext *
      db2_std_extsize),
      long_floor = (min_long_ext * db2_std_extsize), temp_long_ext = 0.0, long_ext_to_add = 0.0,
      dm_tspaces->long_cnt = (dm_tspaces->long_cnt+ 1), stat = alterlist(dm_tspaces->lqual,dm_tspaces
       ->long_cnt), dm_tspaces->lqual[dm_tspaces->long_cnt].tspace_name = dtp.long_tablespace,
      dm_tspaces->lqual[dm_tspaces->long_cnt].table_name = ut.table_name, dm_tspaces->lqual[
      dm_tspaces->long_cnt].avg_row_len = ut.avg_row_len, dm_tspaces->lqual[dm_tspaces->long_cnt].
      avg_col_len = y,
      dm_tspaces->lqual[dm_tspaces->long_cnt].num_rows = ut.num_rows, dm_tspaces->lqual[dm_tspaces->
      long_cnt].bytes = ((ut.avg_row_len - y) * ut.num_rows)
      IF ((dm_tspaces->lqual[dm_tspaces->long_cnt].bytes < 0))
       dm_tspaces->lqual[dm_tspaces->long_cnt].bytes = (1024 * 1024)
      ENDIF
      found = 0, temp_long_ext = ceil((dm_tspaces->lqual[dm_tspaces->long_cnt].bytes/ cnvtreal(
        db2_std_extsize)))
      FOR (i = 1 TO dm_tspaces->tspace_cnt)
        IF (dtp.data_tablespace != dtp.long_tablespace)
         IF ((dm_tspaces->qual[i].tspace_name=dtp.data_tablespace))
          dm_tspaces->lqual[dm_tspaces->long_cnt].data_tspace_ndx = i
         ENDIF
         IF ((dm_tspaces->qual[i].tspace_name=dtp.long_tablespace))
          found = 1, dm_tspaces->qual[i].l_tbl_cnt = (dm_tspaces->qual[i].l_tbl_cnt+ 1)
          IF (((dm_tspaces->lqual[dm_tspaces->long_cnt].bytes * factor_nbr) > long_floor))
           long_ext_to_add = ceil(((dm_tspaces->lqual[dm_tspaces->long_cnt].bytes * factor_nbr)/
            dm_tspaces->qual[i].extent))
          ELSE
           long_ext_to_add = min_long_ext
          ENDIF
          dm_tspaces->qual[i].ttl_extents = (dm_tspaces->qual[i].ttl_extents+ long_ext_to_add)
         ENDIF
        ELSE
         found = 1
        ENDIF
      ...
      IF (found=0)
       dm_tspaces->tspace_cnt = (dm_tspaces->tspace_cnt+ 1), stat = alterlist(dm_tspaces->qual,
        dm_tspaces->tspace_cnt), dm_tspaces->qual[dm_tspaces->tspace_cnt].l_tbl_cnt = (dm_tspaces->
       qual[dm_tspaces->tspace_cnt].l_tbl_cnt+ 1),
       dm_tspaces->qual[dm_tspaces->tspace_cnt].tspace_name = dtp.long_tablespace, dm_tspaces->qual[
       dm_tspaces->tspace_cnt].extent = db2_std_extsize, dm_tspaces->qual[dm_tspaces->tspace_cnt].
       bytes = 0,
       dm_tspaces->qual[dm_tspaces->tspace_cnt].segment_type = "LONG"
       IF (((dm_tspaces->lqual[dm_tspaces->long_cnt].bytes * factor_nbr) > long_floor))
        long_ext_to_add = ceil(((dm_tspaces->lqual[dm_tspaces->long_cnt].bytes * factor_nbr)/
         db2_std_extsize))
       ELSE
      ...
      ENDIF
     FOOT REPORT
      FOR (tc_i = first_long_idx TO dm_tspaces->tspace_cnt)
       dm_tspaces->qual[tc_i].ttl_extents = (ceil((dm_tspaces->qual[tc_i].ttl_extents * 2.0))+
       ext_overhead),dm_tspaces->qual[tc_i].bytes = (dm_tspaces->qual[tc_i].ttl_extents * dm_tspaces
       ->qual[tc_i].extent)
      ENDFOR
     WITH nocounter
    ;end select
   ...
   IF (currdbuser != "ADMIN_TMP"
    ...
    SELECT INTO "nl:"
     ...
     WHERE us.segment_name=ui.index_name
      AND dtp.table_name=ui.table_name
      AND us.segment_type="INDEX"
      AND dtp.precedence=1
      AND substring(1,1,us.tablespace_name) IN ("D", "I", "L")
     ORDER BY dtp.index_tablespace, dtp.table_name
     HEAD dtp.index_tablespace
      dm_tspaces->tspace_cnt = (dm_tspaces->tspace_cnt+ 1), stat = alterlist(dm_tspaces->qual,
       dm_tspaces->tspace_cnt), dm_tspaces->qual[dm_tspaces->tspace_cnt].tspace_name = dtp
      .index_tablespace,
      dm_tspaces->qual[dm_tspaces->tspace_cnt].extent = db2_std_extsize, dm_tspaces->qual[dm_tspaces
      ->tspace_cnt].bytes = 0, dm_tspaces->qual[dm_tspaces->tspace_cnt].segment_type = "INDEX",
      cnt = 0, temp_floor = (dm_tspaces->qual[dm_tspaces->tspace_cnt].extent * min_ind_ext),
      temp_ttl_extents = 0.0
     HEAD dtp.table_name
      temp_bytes = 0.0
     DETAIL
      temp_bytes = ((temp_bytes+ (us.bytes * factor_nbr))+ (ui.num_rows * 20)), cnt = (cnt+ 1)
     FOOT  dtp.table_name
      IF (temp_bytes < temp_floor)
       temp_ttl_extents = (temp_ttl_extents+ min_ind_ext)
      ELSE
       temp_ttl_extents = (temp_ttl_extents+ ceil((temp_bytes/ dm_tspaces->qual[dm_tspaces->
        tspace_cnt].extent)))
      ENDIF
     FOOT  dtp.index_tablespace
      temp_ttl_extents = ceil((temp_ttl_extents * 1.2)), dm_tspaces->qual[dm_tspaces->tspace_cnt].
      ttl_extents = (temp_ttl_extents+ ext_overhead), dm_tspaces->qual[dm_tspaces->tspace_cnt].bytes
       = (dm_tspaces->qual[dm_tspaces->tspace_cnt].ttl_extents * dm_tspaces->qual[dm_tspaces->
      tspace_cnt].extent)
    ...
    ;end select
    SET ctm_prec = 1
    SET ctm_default_tspace = "I_A_SMALL"
   ELSE
    SET dm_tspaces->tspace_cnt = (dm_tspaces->tspace_cnt+ 1)
    SET stat = alterlist(dm_tspaces->qual,dm_tspaces->tspace_cnt)
    SET dm_tspaces->qual[dm_tspaces->tspace_cnt].tspace_name = "I_TOOLKIT"
    SET dm_tspaces->qual[dm_tspaces->tspace_cnt].extent = db2_std_extsize
    SET dm_tspaces->qual[dm_tspaces->tspace_cnt].bytes = 0
    SET dm_tspaces->qual[dm_tspaces->tspace_cnt].segment_type = "INDEX"
    SET dm_tspaces->qual[dm_tspaces->tspace_cnt].ttl_extents = 0
    SET ctm_prec = 0
    SET ctm_default_tspace = "I_TOOLKIT"
    SELECT INTO "nl:"
     ...
     HEAD REPORT
      cnt = 0, cnt = 0, temp_floor = (dm_tspaces->qual[dm_tspaces->tspace_cnt].extent * min_ind_ext),
      temp_ttl_extents = 0.0
     HEAD ui.table_name
      temp_bytes = 0.0
     DETAIL
    ...
    ;end select
    FOR (i = 1 TO dm_tspaces->tspace_cnt)
      IF ((dm_tspaces->qual[i].segment_type="TABLE"))
       SET found = 0
       FOR (j = 1 TO dm_tspaces->tspace_cnt)
         IF ((dm_tspaces->qual[j].segment_type="INDEX")
          AND (replace(dm_tspaces->qual[i].tspace_name,"D_","I_",1)=dm_tspaces->qual[j].tspace_name))
          SET found = 1
         ENDIF
       ENDFOR
       IF (found=0)
        SET dm_tspaces->tspace_cnt = (dm_tspaces->tspace_cnt+ 1)
        SET stat = alterlist(dm_tspaces->qual,dm_tspaces->tspace_cnt)
        SET dm_tspaces->qual[dm_tspaces->tspace_cnt].tspace_name = replace(dm_tspaces->qual[i].
         tspace_name,"D_","I_",1)
        SET dm_tspaces->qual[dm_tspaces->tspace_cnt].segment_type = "INDEX"
        SET dm_tspaces->qual[dm_tspaces->tspace_cnt].bytes = 1
        SET dm_tspaces->qual[dm_tspaces->tspace_cnt].extent = 1
       ENDIF
      ENDIF
    ENDFOR
   ENDIF
   SET total_space = 0.0
   FOR (i = 1 TO dm_tspaces->tspace_cnt)
     SET total_space = (dm_tspaces->qual[i].bytes+ total_space)
   ENDFOR
   FOR (dtp_i = 1 TO dm_tspaces->tspace_cnt)
     SET mb_for_ddlops = 0
     SET ext_for_ddlops = 0
     SET dtp_temp_bytes = 0
     CASE (dm_tspaces->qual[dtp_i].tspace_name)
      OF "D_TOOLKIT":
       SET mb_for_ddlops = 5
      OF "I_TOOLKIT":
       SET mb_for_ddlops = 5
      OF "D_A_SMALL":
       SET mb_for_ddlops = 16
      OF "I_A_SMALL":
       SET mb_for_ddlops = 16
      OF "L_A_SMALL":
       SET mb_for_ddlops = 50
     ENDCASE
     IF (mb_for_ddlops > 0)
      SET ext_for_ddlops = ceil((((mb_for_ddlops * 1024) * 1024)/ dm_tspaces->qual[dtp_i].extent))
      SET dtp_temp_bytes = dm_tspaces->qual[dtp_i].bytes
      SET dm_tspaces->qual[dtp_i].ttl_extents = (dm_tspaces->qual[dtp_i].ttl_extents+ ext_for_ddlops)
      SET dm_tspaces->qual[dtp_i].bytes = (dm_tspaces->qual[dtp_i].ttl_extents * dm_tspaces->qual[
      dtp_i].extent)
     ENDIF
   ENDFOR
 END ;Subroutine
 SUBROUTINE insert_table(null)
   SET ts_ratio = ((overall_size * 1048576)/ total_space)
   IF (ts_ratio < 1)
    SET ts_ratio = 1
    SET overall_size = total_space
   ENDIF
   SET v_db2_str = " "
   SET v_db2_str = concat("insert into ",tbl_name," tbl, ",
    "(dummyt d with seq = value(size(dm_tspaces->qual,5))) ",
    "set tbl.tspace_name = dm_tspaces->qual[d.seq].tspace_name, ",
    "tbl.cur_bytes_allocated = 0, ","tbl.bytes_needed = (dm_tspaces->qual[d.seq].bytes * ts_ratio), ",
    "tbl.new_ind = 1, ","tbl.install_status = ^NONE^,",
    "tbl.extent_size_bytes = dm_tspaces->qual[d.seq].extent ",
    "plan d ","join tbl ","with nocounter go")
   IF (dm2_push_cmd(v_db2_str,1))
    IF (dm2_push_cmd("commit go",1))
     RETURN(1)
    ELSE
     RETURN(0)
    ENDIF
 ...
 END ;Subroutine
#exit_script
 FREE RECORD dm_tspaces
 IF ((dm_err->err_ind=1))
  SET dm_err->eproc = "EXITING DM2_CREATE_TSPACE_MENU_TBL WITH ERRORS"
 ELSE
  SET dm_err->eproc = "DM2_CREATE_TSPACE_MENU_TBL COMPLETED SUCCESSFULLY"
 ENDIF
 CALL final_disp_msg("dm2_rpt_tspace")
END GO
