CREATE PROGRAM dm_dcp_io_encntr_rows:dba
 ...
 SET reply->table_name = "ENCNTR_EVENT_SET_IO"
 SET reply->rows_between_commit = minval(10000,request->max_rows)
 DECLARE cutoff_dt_tm = dq8 WITH protect, noconstant(0.0)
 ...
 SET cutoff_dt_tm = cnvtlookbehind("10D",cnvtdatetime(curdate,curtime3))
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(ee.encntr_event_set_io_id)
   FROM encntr_event_set_io ee
   WHERE ee.encntr_event_set_io_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(ee.encntr_event_set_io_id)
  FROM encntr_event_set_io ee
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    ee.rowid
    FROM encntr_event_set_io ee
    WHERE parser(sbr_getrowidnotexists("ee.encntr_event_set_io_id between curMinID and curMaxID","ee"
      ))
     AND  EXISTS (
    (SELECT
     e.encntr_id
     FROM encounter e
     WHERE e.encntr_id=ee.encntr_id
      AND e.disch_dt_tm < cnvtdatetime(cutoff_dt_tm)))
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,100)=1)
      stat = alterlist(reply->rows,(rows+ 99))
     ENDIF
     reply->rows[rows].row_id = ee.rowid
    WITH nocounter, maxqual(ee,value(rowsleft))
   ;end select
