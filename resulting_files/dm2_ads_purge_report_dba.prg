CREATE PROGRAM dm2_ads_purge_report:dba
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("dm2_ads_purge_rpt",".log","dm2_ads_purge_report")=0)
  GO TO exit_program
 ENDIF
 DECLARE dap_config_cnt = i4 WITH protect, noconstant(0)
 DECLARE dap_tbl_cnt = i4 WITH protect, noconstant(0)
 DECLARE dap_option = i2 WITH protect, noconstant(0)
 DECLARE dap_continue = i2 WITH protect, noconstant(0)
 DECLARE dap_current_config_id = f8 WITH protect, noconstant(0)
 FREE RECORD dap_hist
 RECORD dap_hist(
   1 list[*]
     2 config_name = vc
     2 config_id = f8
 ...
 SET dm_err->eproc = "Retrieving current ADS Purge config information"
 SELECT INTO "nl:"
  FROM dm_ads_config dac
  WHERE dac.sample_method="CUSTOM-DELETE"
  DETAIL
   dap_current_config_id = dac.dm_ads_config_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Retrieving historical ADS Purge data"
 SELECT INTO "nl:"
  FROM dm_ads_purge_hist daph
  ORDER BY daph.dm_ads_config_id DESC, daph.purge_dt_tm
  HEAD daph.dm_ads_config_id
   dap_config_cnt = (dap_config_cnt+ 1), stat = alterlist(dap_hist->list,dap_config_cnt), dap_hist->
   list[dap_config_cnt].config_id = daph.dm_ads_config_id,
   dap_hist->list[dap_config_cnt].config_name = daph.config_name, dap_hist->list[dap_config_cnt].
   purge_dt_tm = daph.purge_dt_tm, dap_hist->list[dap_config_cnt].purge_user = daph.purge_user
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 SET dap_continue = 1
 WHILE (dap_continue=1)
   SET width = 132
   SET message = window
   CALL clear(1,1)
   CALL box(1,1,3,132)
   CALL box(1,1,24,132)
   CALL text(2,2,"ACTIVITY DATA SAMPLE (ADS) PURGE - REPORTING")
   CALL text(5,5,"Select an ADS Purge Sample from below for reporting:")
   CALL text(7,9,"PURGE Date/Time")
   CALL text(7,31,"ADS Purge Sample Name")
   CALL text(8,9,fillstring(20,"-"))
   CALL text(8,31,fillstring(50,"-"))
   FOR (dap_loop_cnt = 1 TO dap_config_cnt)
     CALL text((dap_loop_cnt+ 8),6,build(dap_loop_cnt,". "))
     CALL text((dap_loop_cnt+ 8),9,format(dap_hist->list[dap_loop_cnt].purge_dt_tm,
       "DD-MMM-YYYY HH:MM:SS;;D"))
     CALL text((dap_loop_cnt+ 8),31,dap_hist->list[dap_loop_cnt].config_name)
     IF ((dap_current_config_id=dap_hist->list[dap_loop_cnt].config_id))
      CALL text((dap_loop_cnt+ 8),(textlen(dap_hist->list[dap_loop_cnt].config_name)+ 31),
       "   ** Current Process **")
     ENDIF
     IF (dap_config_cnt >= 12)
      SET dap_loop_cnt = dap_config_cnt
     ENDIF
   ENDFOR
   CALL text((dap_config_cnt+ 10),5,"Your Selection (0 to Exit)? ")
   CALL accept((dap_config_cnt+ 10),34,"9",0)
   SET dap_option = curaccept
   IF (dap_option=0)
    SET dap_continue = 0
   ELSEIF (dap_option > dap_config_cnt)
    SET dap_continue = 1
   ELSE
    SET message = nowindow
    SET dm_err->eproc = "Reporting ADS Purge historical data"
    SELECT INTO mine
     FROM dm_ads_purge_hist daph
     WHERE (daph.dm_ads_config_id=dap_hist->list[dap_option].config_id)
     HEAD REPORT
      CALL print(fillstring(250,"-")), row + 1, col 0,
      CALL print("ACTIVITY DATA SAMPLE PURGE [PURGE ACTIVITY DETAIL REPORT]"), col 80,
      CALL print(build("REPORT DATE/TIME:",format(cnvtdatetime(curdate,curtime3),
        "DD-MMM-YYYY HH:MM:SS;;D"))),
      row + 1,
      CALL print(fillstring(250,"-")), row + 1,
      col 0, "SAMPLE NAME: ", col 20,
      CALL print(trim(daph.config_name)), row + 1,
      CALL print(fillstring(250,"-")),
      row + 1, row + 1, col 0,
      "TABLE NAME", col 40, "| PURGE COUNT ",
      col 60, "| PURGE DT/TM ", col 90,
      "| PURGE USER ", row + 1,
      CALL print(fillstring(250,"-")),
      row + 1
     DETAIL
      col 0, daph.table_name, col 40,
      "| ", daph.purge_count, col 60,
      "| ",
      CALL print(format(daph.purge_dt_tm,"DD-MMM-YYYY HH:MM:SS;;D")), col 90,
      "| ", daph.purge_user, row + 1
...
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "LEAVING dm2_ads_purge_report"
 CALL final_disp_msg("dm2_ads_purge_rpt")
END GO
