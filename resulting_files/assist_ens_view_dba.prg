CREATE PROGRAM assist_ens_view:dba
 ...
 ENDIF
 SET hassist_ens_view = 0
 SET istatus = 0
 SET serrmsg = fillstring(132," ")
 SET ierrcode = error(serrmsg,1)
 SET cd_found = false
 SET new_code_value = 0
 SET result = alter(reply->view_seq,request->view_seq_qual)
 SET reply->status_data.status = "F"
 SET action = substring(1,3,request->action_type)
 CASE (action)
  OF "ADD":
   IF ((request->view_seq_qual != 0))
    CALL validate_view(0)
    WHILE (cd_found=true)
     CALL del_view(0)
     CALL validate_view(0)
    ENDWHILE
    FOR (inx = 1 TO request->view_seq_qual)
      CALL add_view(inx)
    ENDFOR
   ENDIF
  OF "DEL":
...
#check_error
 ...
 SUBROUTINE add_view(add_qual)
   ...
   SET new_code_value = 0
   SET reply->view_seq[add_qual].success_ind = false
   SELECT INTO "nl:"
   ...
   INSERT  FROM code_value cv
    SET cv.code_value = new_code_value, cv.code_set = 344, cv.cdf_meaning = null,
     cv.display = "ASSIST VIEW DATA", cv.display_key = "ASSIST VIEW DATA", cv.description =
     "DEFINES ASSIST VIEW",
   ...
   INSERT  FROM code_value_extension cve
    SET cve.code_set = 344, cve.field_name = "VIEW_NAME", cve.code_value = new_code_value,
   ...
   SET table_name = "CODE_VALUE_EXTENSION"
   INSERT  FROM code_value_extension cve
    SET cve.code_set = 344, cve.field_name = "TABLE_NAME", cve.code_value = new_code_value,
   ...
   SET table_name = "CODE_VALUE_EXTENSION"
   INSERT  FROM code_value_extension cve
    SET cve.code_set = 344, cve.field_name = "OWNER_NAME", cve.code_value = new_code_value,
   ...
   SET table_name = "CODE_VALUE_EXTENSION"
   INSERT  FROM code_value_extension cve
    SET cve.code_set = 344, cve.field_name = "COLUMN_NAME", cve.code_value = new_code_value,
     cve.field_type = 1, cve.field_value = request->view_seq[add_qual].column_name, cve.updt_dt_tm =
   ...
   IF (curqual=0)
    ...
    GO TO check_error
   ELSE
    SET reply->view_seq[add_qual].column_name = request->view_seq[add_qual].column_name
   ENDIF
   SET table_name = "CODE_VALUE_EXTENSION"
   INSERT  FROM code_value_extension cve
    SET cve.code_set = 344, cve.field_name = "VIEW_DESCRIPTION", cve.code_value = new_code_value,
     cve.field_type = 1, cve.field_value = request->view_description, cve.updt_dt_tm = cnvtdatetime(
      curdate,curtime),
     cve.updt_id = reqinfo->updt_id, cve.updt_cnt = 1, cve.updt_task = reqinfo->updt_task,
     cve.updt_applctx = reqinfo->updt_applctx
    WHERE cve.code_set=344
     AND cve.field_name="VIEW_DESCRIPTION"
     AND cve.code_value=new_code_value
    WITH nocounter
   ;end insert
   IF (curqual=0)
    ...
    GO TO check_error
   ELSE
    SET reply->view_description = request->view_description
   ENDIF
   SET table_name = "CODE_VALUE_EXTENSION"
   INSERT  FROM code_value_extension cve
    SET cve.code_set = 344, cve.field_name = "SEQUENCE", cve.code_value = new_code_value,
     cve.field_type = 1, cve.field_value = cnvtstring(request->view_seq[add_qual].sequence), cve
     .updt_dt_tm = cnvtdatetime(curdate,curtime),
     cve.updt_id = reqinfo->updt_id, cve.updt_cnt = 1, cve.updt_task = reqinfo->updt_task,
     cve.updt_applctx = reqinfo->updt_applctx
    WHERE cve.code_set=344
     AND cve.field_name="SEQUENCE"
     AND cve.code_value=new_code_value
    WITH nocounter
   ;end insert
   IF (curqual=0)
    ...
    GO TO check_error
   ELSE
    SET reply->view_seq[add_qual].sequence = request->view_seq[add_qual].sequence
   ENDIF
   SET table_name = "CODE_VALUE_EXTENSION"
   INSERT  FROM code_value_extension cve
    SET cve.code_set = 344, cve.field_name = "CODE_SHOW_IND", cve.code_value = new_code_value,
     cve.field_type = 1, cve.field_value = cnvtstring(request->view_seq[add_qual].code_show_ind), cve
     .updt_dt_tm = cnvtdatetime(curdate,curtime),
     cve.updt_id = reqinfo->updt_id, cve.updt_cnt = 1, cve.updt_task = reqinfo->updt_task,
     cve.updt_applctx = reqinfo->updt_applctx
    WHERE cve.code_set=344
     AND cve.field_name="CODE_SHOW_IND"
     AND cve.code_value=new_code_value
    WITH nocounter
   ;end insert
   IF (curqual=0)
    ...
    GO TO check_error
   ELSE
    SET reply->view_seq[add_qual].code_show_ind = request->view_seq[add_qual].code_show_ind
   ENDIF
   SET reply->view_seq[add_qual].success_ind = true
 END ;Subroutine
 ...
 SUBROUTINE validate_view_det(validate_qual)
   ...
   SELECT INTO "nl:"
    ...
    WHERE c.code_set=344
     ...
     AND e4.field_name="SEQUENCE"
     AND e4.field_value=concat(trim(cnvtstring(request->view_seq[validate_qual].sequence),3))
    DETAIL
     new_code_value = c.code_value
    WITH nocounter
   ;end select
