CREATE PROGRAM dm_ecmb_bb_spec_expire_ovrd:dba
 ...
 RECORD rreclist(
   1 person_id = f8
   1 from_encntr_facility_cd = f8
   1 from_testing_facility_cd = f8
   1 to_encntr_facility_cd = f8
   1 to_testing_facility_cd = f8
   1 from_rec[*]
     2 from_id = f8
     2 active_ind = i4
 ...
 DECLARE v_cust_from_spec_cnt = i4 WITH protect, noconstant(0)
 ...
 DECLARE v_cust_max_exp_dt_tm = q8 WITH protect, noconstant(0.0)
 DECLARE v_cust_get_spec_cnt = i4 WITH protect, noconstant(0)
 DECLARE v_cust_get_spec_idx = i4 WITH protect, noconstant(0)
 DECLARE v_cust_get_ord_cnt = i4 WITH protect, noconstant(0)
 DECLARE v_cust_get_ord_idx = i4 WITH protect, noconstant(0)
 DECLARE v_cust_i_idx = i4 WITH protect, noconstant(0)
 DECLARE v_cust_upd_req_spec_cnt = i4 WITH protect, noconstant(0)
 DECLARE v_cust_upd_req_orders_cnt = i4 WITH protect, noconstant(0)
 DECLARE v_cust_upd_req_prod_cnt = i4 WITH protect, noconstant(0)
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_ECMB_BB_SPEC_EXPIRE_OVRD"
 ...
 IF (curqual > 0)
  ...
  SET request->error_message = substring(1,132,
   "Found a custom encounter combine script for the orders table, fail.")
  GO TO exit_sub
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  FROM encounter e
  PLAN (e
   WHERE e.encntr_id IN (request->xxx_combine[icombine].from_xxx_id, request->xxx_combine[icombine].
   to_xxx_id)
    AND  EXISTS (
   (SELECT
    o.order_id
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ENDIF
   rreclist->from_rec[v_cust_from_rec_cnt].from_id = frm.encntr_id, rreclist->from_rec[
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   CASE (e.encntr_id)
    OF request->xxx_combine[icombine].from_xxx_id:
     rreclist->from_encntr_facility_cd = e.loc_facility_cd,rreclist->person_id = e.person_id
    OF request->xxx_combine[icombine].to_xxx_id:
     rreclist->to_encntr_facility_cd = e.loc_facility_cd
   ENDCASE
  WITH nocounter
 ;end select
 SET rreclist->from_testing_facility_cd = bbtgetflexspectestingfacility(rreclist->
  from_encntr_facility_cd)
 IF ((rreclist->from_testing_facility_cd=- (1)))
  SET failed = data_error
  SET request->error_message = substring(1,132,"Get testing facility cd failed for FROM Encntr.")
  GO TO exit_sub
 ENDIF
 SET rreclist->to_testing_facility_cd = bbtgetflexspectestingfacility(rreclist->to_encntr_facility_cd
  )
 IF ((rreclist->to_testing_facility_cd=- (1)))
  SET failed = data_error
  SET request->error_message = substring(1,132,"Get testing facility cd failed for TO Encntr.")
  GO TO exit_sub
 ENDIF
 IF ((rreclist->from_testing_facility_cd=rreclist->to_testing_facility_cd))
  GO TO post_updates
 ENDIF
 SET v_cust_stat = initrec(getflexrequest)
 SET getflexrequest->alert_ind = "N"
 SET v_cust_stat = alterlist(getflexrequest->personlist,1)
 SET getflexrequest->personlist[1].person_id = rreclist->person_id
 SET getflexrequest->personlist[1].filter_encntr_id = request->xxx_combine[icombine].to_xxx_id
 SET getflexrequest->personlist[1].encntr_facility_cd = rreclist->from_encntr_facility_cd
 ...
 IF ((getflexreply->status_data.status="F"))
  SET failed = data_error
  SET request->error_message = substring(1,132,
   "Call of bbt_get_avail_flex_specs returned failed status.")
  GO TO exit_sub
 ENDIF
 SELECT INTO "nl:"
  ec.*, ecd.*
  ...
  PLAN (ec
   WHERE (ec.from_encntr_id=request->xxx_combine[icombine].from_xxx_id)
    AND (ec.to_encntr_id=request->xxx_combine[icombine].to_xxx_id)
    AND ec.active_ind=1)
   JOIN (ecd
   WHERE ecd.encntr_combine_id=ec.encntr_combine_id
    AND ecd.entity_name="ORDERS"
    AND ecd.active_ind=1)
  HEAD REPORT
   v_cust_get_spec_cnt = size(getflexreply->personlist[1].specimen,5)
  DETAIL
   FOR (v_cust_get_spec_idx = 1 TO v_cust_get_spec_cnt)
    v_cust_get_ord_cnt = size(getflexreply->personlist[1].specimen[v_cust_get_spec_idx].orders,5),
    FOR (v_cust_get_ord_idx = 1 TO v_cust_get_ord_cnt)
      IF ((getflexreply->personlist[1].specimen[v_cust_get_spec_idx].orders[v_cust_get_ord_idx].
      order_id=ecd.entity_id))
       v_cust_from_spec_cnt += 1
       IF (v_cust_from_spec_cnt > size(rreclist->from_specimen,5))
        v_cust_stat = alterlist(rreclist->from_specimen,(v_cust_from_spec_cnt+ 9))
       ENDIF
       rreclist->from_specimen[v_cust_from_spec_cnt].specimen_id = getflexreply->personlist[1].
       specimen[v_cust_get_spec_idx].specimen_id
      ENDIF
    ENDFOR
   ENDFOR
  FOOT REPORT
   v_cust_stat = alterlist(rreclist->from_specimen,v_cust_from_spec_cnt)
  WITH nocounter
 ;end select
 IF ((loadflexparams(rreclist->to_encntr_facility_cd)=- (1)))
  SET failed = data_error
  SET request->error_message = substring(1,132,
   "Get TO encounter flexible specimen preferences failed.")
  GO TO exit_sub
 ENDIF
 SET v_cust_get_spec_cnt = size(getflexreply->personlist[1].specimen,5)
 FOR (v_cust_get_spec_idx = 1 TO v_cust_get_spec_cnt)
   SET v_cust_idx_hold = 0
   SET v_cust_idx_hold = locateval(v_cust_i_idx,1,v_cust_from_spec_cnt,getflexreply->personlist[1].
    specimen[v_cust_get_spec_idx].specimen_id,rreclist->from_specimen[v_cust_i_idx].specimen_id)
   IF (v_cust_idx_hold > 0)
    IF ((flex_param_out->flex_on_ind=1))
     SET v_cust_exp_dt_tm = getflexexpiration(rreclist->person_id,0.0,getflexreply->personlist[1].
      specimen[v_cust_get_spec_idx].drawn_dt_tm,rreclist->to_encntr_facility_cd,0)
     ...
     ENDIF
     IF (trim(flex_param_out->expiration_unit_type_mean)="D")
      SET v_cust_max_exp_dt_tm = datetimeadd(cnvtdatetime(cnvtdate(getflexreply->personlist[1].
         specimen[v_cust_get_spec_idx].drawn_dt_tm),235959),flex_param_out->max_spec_validity)
     ELSE
      SET v_cust_max_exp_dt_tm = datetimeadd(getflexreply->personlist[1].specimen[v_cust_get_spec_idx
       ].drawn_dt_tm,flex_param_out->max_spec_validity)
     ENDIF
     IF (v_cust_exp_dt_tm > v_cust_max_exp_dt_tm)
      SET v_cust_exp_dt_tm = v_cust_max_exp_dt_tm
     ENDIF
     IF ((v_cust_exp_dt_tm < getflexreply->personlist[1].specimen[v_cust_get_spec_idx].expire_dt_tm))
      CALL addrectoflexupdrequest(null)
     ENDIF
    ELSE
     SET v_cust_exp_dt_tm = 0.0
     CALL addrectoflexupdrequest(null)
    ENDIF
   ENDIF
 ENDFOR
...
#post_updates
 UPDATE  FROM bb_spec_expire_ovrd frm
  SET frm.updt_cnt = (frm.updt_cnt+ 1), frm.updt_id = reqinfo->updt_id, frm.updt_applctx = reqinfo->
   updt_applctx,
   frm.updt_task = reqinfo->updt_task, frm.updt_dt_tm = cnvtdatetime(v_cust_cur_dt_tm), frm.encntr_id
 ...
 SUBROUTINE addrectoflexupdrequest(null)
   ...
   IF (v_cust_upd_req_spec_cnt=0)
    ...
    SET flexupdrequest->persons[1].person_id = rreclist->person_id
   ENDIF
   ...
   SET flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].facility_cd = rreclist->
   to_encntr_facility_cd
   SET flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].encntr_id = request->
   xxx_combine[icombine].to_xxx_id
   SET flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].testing_facility_cd = rreclist->
   to_testing_facility_cd
   ...
   v_cust_exp_dt_tm
   SET v_cust_upd_req_orders_cnt = 0
   SET v_cust_get_ord_cnt = size(getflexreply->personlist[1].specimen[v_cust_get_spec_idx].orders,5)
   FOR (v_cust_get_ord_idx = 1 TO v_cust_get_ord_cnt)
     SET v_cust_upd_req_orders_cnt += 1
     IF (v_cust_upd_req_orders_cnt > size(flexupdrequest->persons[1].specimens[
      v_cust_upd_req_spec_cnt].orders,5))
      SET v_cust_stat = alterlist(flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].
       orders,(v_cust_upd_req_orders_cnt+ 9))
     ENDIF
     SET flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].orders[
     v_cust_upd_req_orders_cnt].order_id = getflexreply->personlist[1].specimen[v_cust_get_spec_idx].
     orders[v_cust_get_ord_idx].order_id
     SET v_cust_upd_req_prod_cnt = 0
     SET getproductscount = size(getflexreply->personlist[1].specimen[v_cust_get_spec_idx].orders[
      v_cust_upd_req_orders_cnt].products,5)
     FOR (getproductsindex = 1 TO getproductscount)
       SET v_cust_upd_req_prod_cnt += 1
       IF (v_cust_upd_req_prod_cnt > size(flexupdrequest->persons[1].specimens[
        v_cust_upd_req_spec_cnt].orders[v_cust_upd_req_orders_cnt].products,5))
        SET v_cust_stat = alterlist(flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].
         orders[v_cust_upd_req_orders_cnt].products,(v_cust_upd_req_prod_cnt+ 9))
       ENDIF
       SET flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].orders[
       v_cust_upd_req_orders_cnt].products[v_cust_upd_req_prod_cnt].product_id = getflexreply->
       personlist[1].specimen[v_cust_get_spec_idx].orders[v_cust_get_ord_idx].products[
       getproductsindex].product_id
       SET flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].orders[
       v_cust_upd_req_orders_cnt].products[v_cust_upd_req_prod_cnt].product_event_id = getflexreply->
       personlist[1].specimen[v_cust_get_spec_idx].orders[v_cust_get_ord_idx].products[
       getproductsindex].product_event_id
       SET flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].orders[
       v_cust_upd_req_orders_cnt].products[v_cust_upd_req_prod_cnt].product_type_cd = getflexreply->
       personlist[1].specimen[v_cust_get_spec_idx].orders[v_cust_get_ord_idx].products[
       getproductsindex].product_type_cd
       SET flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].orders[
       v_cust_upd_req_orders_cnt].products[v_cust_upd_req_prod_cnt].product_type_disp = getflexreply
       ->personlist[1].specimen[v_cust_get_spec_idx].orders[v_cust_get_ord_idx].products[
       getproductsindex].product_type_disp
       SET flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].orders[
       v_cust_upd_req_orders_cnt].products[v_cust_upd_req_prod_cnt].old_xm_expire_dt_tm =
       getflexreply->personlist[1].specimen[v_cust_get_spec_idx].orders[v_cust_get_ord_idx].products[
       getproductsindex].crossmatch_expire_dt_tm
     ENDFOR
     SET v_cust_stat = alterlist(flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].
      orders[v_cust_upd_req_orders_cnt].products,v_cust_upd_req_prod_cnt)
   ENDFOR
   SET v_cust_stat = alterlist(flexupdrequest->persons[1].specimens[v_cust_upd_req_spec_cnt].orders,
    v_cust_upd_req_orders_cnt)
   RETURN(1)
 END ;Subroutine
 SUBROUTINE addrecstobbreviewqueue(null)
   DECLARE parent_name = c32 WITH protect, constant(substring(1,32,"BB_SPEC_EXPIRE_OVRD"))
   DECLARE newpathnetseq = f8 WITH protect, noconstant(0.0)
   DECLARE updreplyspecidx = i4 WITH protect, noconstant(0)
   DECLARE updreplyspeccnt = i4 WITH protect, noconstant(0)
   SET updreplyspeccnt = size(flexupdreply->persons[1].specimens,5)
   FOR (updreplyspecidx = 1 TO updreplyspeccnt)
     IF ((flexupdreply->persons[1].specimens[updreplyspecidx].new_override_id > 0.0))
      ...
      SET v_cust_idx_hold = locateval(v_cust_i_idx,1,v_cust_get_spec_cnt,flexupdreply->persons[1].
       specimens[updreplyspecidx].specimen_id,getflexreply->personlist[1].specimen[v_cust_i_idx].
       specimen_id)
      IF (v_cust_idx_hold <= 0)
       SET failed = data_error
       SET request->error_message = substring(1,132,
        "Failed to locate from specimen id in getFlexReply")
       RETURN(0)
      ENDIF
      INSERT  FROM bb_review_queue brq
       SET brq.active_ind = 1, brq.active_status_cd = reqdata->active_status_cd, brq
        ...
        brq.active_status_prsnl_id = reqinfo->updt_id, brq.bb_review_queue_id = newpathnetseq, brq
        .from_encntr_id = request->xxx_combine[icombine].from_xxx_id,
        brq.from_parent_entity_id = getflexreply->personlist[1].specimen[v_cust_idx_hold].override_id,
        brq.from_person_id = rreclist->person_id, brq.parent_entity_name = parent_name,
        brq.to_encntr_id = request->xxx_combine[icombine].to_xxx_id, brq.to_parent_entity_id =
        flexupdreply->persons[1].specimens[updreplyspecidx].new_override_id, brq.to_person_id =
        rreclist->person_id,
...
#exit_sub
 FREE SET rreclist
 FREE SET getflexrequest
 FREE SET flexupdrequest
END GO
