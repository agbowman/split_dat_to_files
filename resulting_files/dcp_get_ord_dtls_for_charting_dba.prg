CREATE PROGRAM dcp_get_ord_dtls_for_charting:dba
 ...
 DECLARE order_ingredient_where = c500 WITH noconstant(fillstring(500," "))
 ...
 DECLARE product_order_id = f8 WITH noconstant(0.0)
 DECLARE product_action_sequence = i4 WITH noconstant(0)
 ...
 DECLARE template_dose_sequence = i4 WITH noconstant(0)
 ...
 DECLARE coreind = i2 WITH noconstant(0)
 DECLARE parent_entity_id = f8 WITH protect, noconstant(0.0)
 DECLARE rx_verify_action_sequence = i4 WITH protect, noconstant(0)
 DECLARE done = i2 WITH protect, noconstant(0)
 DECLARE rxverify_oa_noverifyneeded = i2 WITH protect, constant(0)
 DECLARE rxverify_oa_needsverify = i2 WITH protect, constant(1)
 DECLARE rxverify_oa_superceded = i2 WITH protect, constant(2)
 DECLARE rxverify_oa_verified = i2 WITH protect, constant(3)
 DECLARE rxverify_oa_rejected = i2 WITH protect, constant(4)
 DECLARE rxverify_oa_reviewed = i2 WITH protect, constant(5)
 DECLARE rxverify_orders_noverifyneeded = i2 WITH protect, constant(0)
 DECLARE rxverify_orders_needsverify = i2 WITH protect, constant(1)
 DECLARE rxverify_orders_rejected = i2 WITH protect, constant(2)
 ...
 IF (validate(request->check_core_ind)
  AND (request->check_core_ind=1))
  SELECT INTO "nl:"
   FROM orders o,
    order_action oa
   PLAN (o
   ...
   HEAD REPORT
    IF ((request->action_sequence=oa.action_sequence))
     reply->core_ind = oa.core_ind, coreind = 1
    ENDIF
   WITH nocounter
  ;end select
  GO TO exit_script
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   reply->updated_to_verified_flag = 0
 ...
 IF ((items_to_check->qual[1].verify_success_ind=1))
  ...
  SET reply->updated_to_verified_flag = 1
 ENDIF
 SELECT INTO "nl:"
  FROM orders o,
   orders ot,
  ...
  HEAD REPORT
   detail_cnt = 0
  HEAD o.order_id
   reply->person_id = o.person_id, reply->encntr_id = o.encntr_id, reply->hna_order_mnemonic = o
   .hna_order_mnemonic,
   reply->ordered_as_mnemonic = o.ordered_as_mnemonic, reply->order_mnemonic = o.order_mnemonic,
   reply->catalog_cd = o.catalog_cd,
   reply->catalog_type_cd = o.catalog_type_cd, reply->activity_type_cd = o.activity_type_cd, reply->
   iv_ind = o.iv_ind,
   reply->orig_order_dt_tm = cnvtdatetime(o.orig_order_dt_tm), reply->orig_order_tz = o.orig_order_tz,
   reply->template_order_id = ot.order_id,
   reply->template_core_action_sequence = o.template_core_action_sequence, reply->need_rx_verify_ind
    = - (1), reply->need_rx_clin_review_flag = - (1),
   reply->prn_ind = o.prn_ind, reply->orderable_type_flag = o.orderable_type_flag, reply->
   dcp_clin_cat_cd = o.dcp_clin_cat_cd,
   reply->med_order_type_cd = o.med_order_type_cd, reply->event_cd = cve.event_cd, reply->
   freq_type_flag = o.freq_type_flag,
   reply->last_action_sequence = o.last_action_sequence, reply->updt_cnt = o.updt_cnt, reply->
   oe_format_id = o.oe_format_id,
   reply->synonym_id = o.synonym_id, last_core_action_seq = o.last_core_action_sequence, reply->
   dosing_method_flag = o.dosing_method_flag,
   reply->template_dose_sequence = o.template_dose_sequence, reply->projected_stop_dt_tm = o
   .projected_stop_dt_tm, reply->projected_stop_tz = o.projected_stop_tz,
   reply->stop_type_cd = o.stop_type_cd
   ...
   IF (o.template_order_id > 0)
    template_order_query_id = o.template_order_id, parent_entity_id = o.template_order_id,
    rx_verify_action_sequence = action_seq_for_ingreds,
    product_order_id = o.template_order_id, product_action_sequence = o.template_core_action_sequence,
    ...
    template_dose_sequence = o.template_dose_sequence,
   ...
   ELSE
    ...
    rx_verify_action_sequence = o.last_action_sequence,
    product_order_id = o.order_id, product_action_sequence = reply->core_action_sequence, reply->
    ...
    reply->comment_type_mask = o.comment_type_mask, order_ingredient_where = concat(trim(
     ...
     " where oi2.order_id = oi.order_id and"," oi2.action_sequence <= ",trim(cnvtstring(
       action_seq_for_ingreds)),")")
   ENDIF
   IF (validate(request->template_dose_sequence))
    IF ((request->template_dose_sequence > 0))
     template_dose_sequence = request->template_dose_sequence
    ENDIF
   ENDIF
  HEAD oa.order_id
 ...
 SELECT INTO "nl:"
  FROM order_action oa
  WHERE oa.order_id=parent_entity_id
  ORDER BY oa.action_sequence
  HEAD REPORT
   action_cnt = 0
  DETAIL
 ...
 FOR (i = 1 TO size(review_flag_map->action,5))
   ...
   ENDIF
 ENDFOR
 SET actionseq = rx_verify_action_sequence
 IF ((reply->template_order_id=0))
  SELECT INTO "nl:"
   FROM order_action oa
   WHERE oa.order_id=parent_entity_id
    AND oa.action_sequence <= rx_verify_action_sequence
   ORDER BY oa.action_sequence DESC
   HEAD REPORT
    bfound = 0
   DETAIL
  ...
  ENDIF
 ENDIF
 FOR (i = rx_verify_action_sequence TO size(review_flag_map->action,5))
   IF ((review_flag_map->action[i].mapped_clin_review_flag != 5)
    AND (review_flag_map->action[i].mapped_clin_review_flag != 4))
    SET actionseq = i
    SET i = (size(review_flag_map->action,5)+ 1)
   ENDIF
 ENDFOR
 IF (actionseq <= size(review_flag_map->action,5))
  SET reply->need_rx_verify_ind = review_flag_map->action[actionseq].needs_verify_ind
  SET reply->need_rx_clin_review_flag = review_flag_map->action[actionseq].mapped_clin_review_flag
 ENDIF
 SET map_request->mapping_ind = 3
 SET map_request->map_from_value = reply->need_rx_verify_ind
 SET modify = nopredeclare
 EXECUTE dcp_map_clin_review_flag  WITH replace("REQUEST","MAP_REQUEST"), replace("REPLY","MAP_REPLY"
  )
 SET modify = predeclare
 SET reply->need_rx_verify_ind = map_reply->map_to_value
  ...
  SELECT INTO "nl:"
   ...
   PLAN (oi
    ...
    WHERE (oid.order_id= Outerjoin(oi.order_id))
     ...
     AND (oid.dose_sequence= Outerjoin(template_dose_sequence)) )
   HEAD REPORT
   ...
   HEAD oi.comp_sequence
    ...
    clinically_significant_flag = oi.clinically_significant_flag, reply->ingred_qual[ingred_cnt].
    include_in_total_volume_flag = oi.include_in_total_volume_flag,
    reply->ingred_qual[ingred_cnt].witness_required_ind = ocs.witness_flag, reply->ingred_qual[
    ingred_cnt].iv_seq = oi.iv_seq, reply->ingred_qual[ingred_cnt].normalized_rate = oi
    .normalized_rate,
    reply->ingred_qual[ingred_cnt].normalized_rate_unit_cd = oi.normalized_rate_unit_cd, reply->
    ingred_qual[ingred_cnt].concentration = oi.concentration, reply->ingred_qual[ingred_cnt].
    concentration_unit_cd = oi.concentration_unit_cd,
    reply->ingred_qual[ingred_cnt].ingredient_rate_conversion_ind = ocs
    .ingredient_rate_conversion_ind, reply->ingred_qual[ingred_cnt].clinically_significant_flag = oi
    .clinically_significant_flag, reply->ingred_qual[ingred_cnt].ingredient_source_flag = oi
    .ingredient_source_flag,
    reply->ingred_qual[ingred_cnt].cki = ocs.cki, reply->ingred_qual[ingred_cnt].
    last_admin_disp_basis_flag = ocs.last_admin_disp_basis_flag, reply->ingred_qual[ingred_cnt].
    med_interval_warn_flag = ocs.med_interval_warn_flag,
    reply->ingred_qual[ingred_cnt].autoprog_syn_ind = ocs.autoprog_syn_ind
 ...
 ENDIF
 SELECT INTO "nl:"
  FROM (dummyt d  WITH seq = value(size(reply->ingred_qual,5))),
   order_product op,
   medication_definition md
  PLAN (d)
   JOIN (op
   WHERE op.order_id=product_order_id
    AND op.action_sequence >= product_action_sequence
    AND (op.ingred_sequence=reply->ingred_qual[d.seq].comp_sequence))
   JOIN (md
   WHERE (md.item_id= Outerjoin(op.item_id)) )
  ORDER BY op.ingred_sequence, op.action_sequence
  HEAD REPORT
   IF (op.order_id > 0)
    product_found = 1, reply->product_action_seq = op.action_sequence
   ENDIF
  HEAD op.ingred_sequence
   product_cnt = 0, product_action_sequence = op.action_sequence
  DETAIL
   IF (op.action_sequence=product_action_sequence)
    product_cnt += 1, stat = alterlist(reply->ingred_qual[d.seq].product_qual,product_cnt), reply->
    ingred_qual[d.seq].product_qual[product_cnt].item_id = op.item_id,
    reply->ingred_qual[d.seq].product_qual[product_cnt].dispense_category_cd = md
    .dispense_category_cd, reply->ingred_qual[d.seq].product_qual[product_cnt].dose_quantity = op
    .dose_quantity, reply->ingred_qual[d.seq].product_qual[product_cnt].dose_quantity_unit_cd = op
 ...
 IF (product_found=0)
  SELECT INTO "nl:"
   ...
   PLAN (d)
    ...
    JOIN (md
    WHERE (md.item_id= Outerjoin(op.item_id)) )
   ORDER BY op.ingred_sequence, op.action_sequence DESC
   HEAD REPORT
    IF (op.order_id > 0)
     product_found = 1, reply->product_action_seq = op.action_sequence
    ENDIF
 ...
 ;end select
#exit_script
 IF (((detail_cnt > 0) OR (((ingred_cnt > 0) OR (((product_cnt > 0) OR (coreind=1)) )) )) )
  SET reply->status_data.status = "S"
 ELSE
  SET reply->status_data.status = "Z"
 ENDIF
 SET last_mod = "59"
 SET mod_date = "07/29/2022"
 SET modify = nopredeclare
END GO
