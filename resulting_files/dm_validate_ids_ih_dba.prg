CREATE PROGRAM dm_validate_ids_ih:dba
 SET i = 0
 SET j = 0
 SET k = 0
 SET m = 0
 ...
 SET condition1 = "The table in the parent_entity_col has no primary key or is an invalid table name"
 SET condition2 = "Table has both a root_entity_pair and a parent_entity_col filled out"
 ...
 SET valid_schema_date = 0
 SET schema_date = cnvtdatetime( $1)
 CALL echo("Validating the Schema date you entered...")
 SELECT DISTINCT INTO "nl:"
  t.schema_date
  FROM dm_tables t
  WHERE t.schema_date=cnvtdatetime( $1)
  WITH nocounter
 ;end select
 IF (curqual > 0)
  SET valid_schema_date = 1
 ELSE
  SET valid_schema_date = 0
 ENDIF
 IF (valid_schema_date=1)
  CALL echo("Executing DM_TEMP_TABLES...")
  EXECUTE dm_temp_tables
  COMMIT
  CALL echo("Deleting rows from DM_INVALID_TABLE_VALUE table...")
  DELETE  FROM dm_invalid_table_value
   WHERE 1=1
  ;end delete
  COMMIT
  CALL echo("Checking for the existence of the DM_TABLE_CONSTRUCTION_ERR table...")
  SELECT INTO "nl:"
   ut.table_name
   FROM user_tables ut
  ...
  ;end select
  IF (curqual=0)
   CALL echo("Creating the DM_TABLE_CONSTRUCTION_ERR table...")
   SET n = (n+ 1)
  ...
  ELSE
   CALL echo("Deleting rows from DM_TABLE_CONSTRUCTION_ERR table...")
  ...
  CALL echo("Retreiving table and column data, This may take a minute or two...")
  SELECT INTO "nl:"
   ...
   PLAN (utc
    ...
    WHERE dc.table_name=utc.table_name
     ...
     AND dc.schema_date=cnvtdatetime( $1))
  ...
  FOR (i = 1 TO parentdata->num)
    ...
    FOR (k = 1 TO temp->num)
      ...
      ELSE
       CALL echo("*********************** ERROR ***************************")
       CALL echo(concat("Table_name = ",temp->list[k].table_name))
       CALL echo(concat("Column_name = ",temp->list[k].column_name))
       CALL echo(concat("Pe_col_name = ",temp->list[k].pe_col_name))
       CALL echo(concat("Pe_table_name = ",temp->list[k].pe_table_name))
       CALL echo(concat("Pe_primary_key = ",temp->list[k].pe_primary_key))
       CALL echo("*********************************************************")
      ENDIF
    ENDFOR
    SET k = 0
    FOR (k = 1 TO temp->num)
      IF ((temp->list[k].pe_primary_key > " "))
       ...
       SET n = (n+ 1)
       SET buff[n] = build("where t2.",temp->list[k].pe_primary_key," = t1.",temp->list[k].
        column_name,")")
       SET n = (n+ 1)
       SET buff[n] = " detail "
       SET n = (n+ 1)
  ...
  FREE SET badconstr
 ELSE
  CALL echo("Invalid schema date. Please select a valid schema_date.")
  SELECT DISTINCT
   t.schema_date
   FROM dm_tables t
   WITH nocounter
