CREATE PROGRAM bed_cpy_fn_tab_by_trk_group:dba
 FREE SET reply
 RECORD reply(
   1 error_msg = vc
   1 status_data
 ...
 SET group_cnt = size(request->from_tracking_groups,5)
 ...
 SELECT INTO "NL:"
  ...
  HEAD d.seq
   search_string = trim(cnvtstring(request->from_tracking_groups[d.seq].code_value))
  DETAIL
   IF (findstring(search_string,nvp.pvc_value))
    vcount = (vcount+ 1), tot_vcount = (tot_vcount+ 1)
    IF (vcount > 20)
     stat = alterlist(view_list->vlist,(tot_vcount+ 20)), vcount = 1
    ENDIF
    view_list->vlist[tot_vcount].view_seq = dp.view_seq
   ENDIF
  WITH nocounter
 ;end select
