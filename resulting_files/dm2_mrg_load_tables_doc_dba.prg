CREATE PROGRAM dm2_mrg_load_tables_doc:dba
 ...
 SET dim_filename = "cer_install:dm2_mrg_tables_doc.csv"
 SET readme_data->status = "F"
 SET readme_data->message = "Readme Failed: Starting dm2_mrg_load_tables_doc.prg script..."
 IF (check_logfile("dm2_load_mrg_tbls_",".log","dm2_mrg_load_tables_doc LOG FILE...") != 1)
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_mrg_load_tables_doc..."
 CALL disp_msg("",dm_err->logfile,0)
 IF (findfile(dim_filename)=0)
  SET readme_data->status = "F"
  SET readme_data->message = "Cannot find dm2_mrg_tables_doc.csv in CER_INSTALL."
  SET dm_err->emsg = "Cannot find dm2_mrg_tables_doc.csv in CER_INSTALL."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF (merge_check_cnt > 0)
  ...
  IF (inhouse_ind=1)
   ...
   GO TO exit_script
  ENDIF
  SET dm_err->eproc = "Updating dm_tables_doc..."
  CALL disp_msg("",dm_err->logfile,0)
  SET readme_data->status = "F"
  SET readme_data->message = "Updating dm_tables_doc.."
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = merge_check_cnt),
    dm_tables_doc dtd
   PLAN (d)
    JOIN (dtd
    WHERE (dtd.table_name=requestin->list_0[d.seq].table_name))
   DETAIL
  ...
  UPDATE  FROM dm_tables_doc dtd,
   ...
   SET dtd.mergeable_ind = cnvtint(requestin->list_0[d.seq].mergeable_ind), dtd.merge_ui_query =
    requestin->list_0[d.seq].merge_ui_query, dtd.table_suffix = requestin->list_0[d.seq].table_suffix,
    dtd.full_table_name = trim(requestin->list_0[d.seq].full_table_name), dtd.suffixed_table_name =
    trim(requestin->list_0[d.seq].suffixed_table_name), dtd.reference_ind = cnvtint(requestin->
     list_0[d.seq].reference_ind),
    dtd.merge_delete_ind = cnvtint(requestin->list_0[d.seq].merge_delete_ind), dtd.merge_active_ind
     = cnvtint(requestin->list_0[d.seq].merge_active_ind), dtd.updt_cnt = cnvtint(trim(requestin->
      list_0[d.seq].updt_cnt)),
    dtd.default_row_ind = cnvtint(requestin->list_0[d.seq].default_row_ind), dtd
    .person_cmb_trigger_type = trim(requestin->list_0[d.seq].person_cmb_trigger_type), dtd
    .encntr_cmb_trigger_type = trim(requestin->list_0[d.seq].encntr_cmb_trigger_type)
   PLAN (d
    WHERE (rs_exist->qual[d.seq].is_there=1))
    JOIN (dtd
    WHERE (dtd.table_name=requestin->list_0[d.seq].table_name)
     AND dtd.updt_cnt <= cnvtint(requestin->list_0[d.seq].updt_cnt))
   WITH nocounter
  ;end update
  SET after_cnt = curqual
  SET ddtd_curqual = (ddtd_curqual+ curqual)
  SET readme_data->message = "Inserting into dm_tables_doc.."
  INSERT  FROM dm_tables_doc dtd,
    (dummyt d  WITH seq = value(size(rs_exist->qual,5)))
   SET dtd.mergeable_ind = cnvtint(requestin->list_0[d.seq].mergeable_ind), dtd.merge_ui_query = trim
    (requestin->list_0[d.seq].merge_ui_query), dtd.merge_delete_ind = cnvtint(requestin->list_0[d.seq
     ].merge_delete_ind),
    dtd.merge_active_ind = cnvtint(requestin->list_0[d.seq].merge_active_ind), dtd.table_name = trim(
     requestin->list_0[d.seq].table_name), dtd.table_suffix = trim(requestin->list_0[d.seq].
     table_suffix),
    dtd.full_table_name = trim(requestin->list_0[d.seq].full_table_name), dtd.suffixed_table_name =
    trim(requestin->list_0[d.seq].suffixed_table_name), dtd.reference_ind = cnvtint(requestin->
     list_0[d.seq].reference_ind),
    dtd.updt_cnt = cnvtint(trim(requestin->list_0[d.seq].updt_cnt)), dtd.default_row_ind = cnvtint(
     requestin->list_0[d.seq].default_row_ind), dtd.person_cmb_trigger_type = trim(requestin->list_0[
     d.seq].person_cmb_trigger_type),
    dtd.encntr_cmb_trigger_type = trim(requestin->list_0[d.seq].encntr_cmb_trigger_type)
   PLAN (d
 ...
 ELSE
  ...
  SET readme_data->message = concat("Readme Failed.  The request structure has not been ",
   "populated with dm2_mrg_tables_doc.csv information.")
 ...
 IF (check_error(dm_err->eproc) != 0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Error loading dm_tables_doc with info in dm2_mrg_tables_doc.csv."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ROLLBACK
  SET readme_data->status = "F"
  SET readme_data->message = "Error loading dm_tables_doc with info in dm2_mrg_tables_doc.csv."
  GO TO exit_script
 ELSE
  SET readme_data->status = "S"
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (dtd
   WHERE dtd.table_name=trim(requestin->list_0[d.seq].table_name)
    AND dtd.reference_ind=cnvtint(requestin->list_0[d.seq].reference_ind)
    AND dtd.merge_delete_ind=cnvtint(requestin->list_0[d.seq].merge_delete_ind)
    AND dtd.merge_active_ind=cnvtint(requestin->list_0[d.seq].merge_active_ind)
    AND dtd.updt_cnt=cnvtint(trim(requestin->list_0[d.seq].updt_cnt)))
  DETAIL
   dim_cnt = (dim_cnt+ 1)
  WITH nocounter
 ;end select
 ...
 ELSE
  SET readme_data->status = "F"
  SET readme_data->message = "Check failed. Rows in request != rows in local dm_tables_doc."
 ENDIF
#exit_script
 IF ((readme_data->status="S"))
  IF (inhouse_ind=1)
   SET readme_data->message = "Not executed since we are in inhouse domain."
  ELSE
   SET readme_data->message = "DM_TABLES_DOC has been updated successfully."
  ENDIF
 ELSE
  ROLLBACK
 ENDIF
 CALL echorecord(readme_data)
 SET dm_err->eproc = "...Ending dm2_mrg_load_tables_doc"
 CALL final_disp_msg("dm2_load_mrg_tbls_")
