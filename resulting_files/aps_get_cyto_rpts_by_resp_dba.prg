CREATE PROGRAM aps_get_cyto_rpts_by_resp:dba
...
#script
 ...
 ENDIF
 SELECT INTO "nl:"
  rt.report_id, cr.case_id, join_path = decode(cs.seq,"S",cp.seq,"P"," "),
  t_tag_group_id = decode(t.seq,t.tag_group_id,0.0), t_tag_sequence = decode(t.seq,t.tag_sequence,0)
  FROM (dummyt d3  WITH seq = value(size(temp->qual,5))),
   report_task rt,
   case_report cr,
  ...
  PLAN (d3)
   ...
   WHERE parser(
    IF ((request->service_resource_cd > 0))
     " temp->qual[d3.seq].service_resource_cd = rt.service_resource_cd"
    ELSE "0 = 0"
    ENDIF
    )
    AND parser(
  ...
  ORDER BY pc.accession_nbr, cr.report_id, join_path DESC,
   t_tag_group_id, t_tag_sequence
  HEAD REPORT
   access_to_resource_ind = 0, cnt = 0, service_resource_cd = 0.0
  HEAD rt.report_id
   ...
   IF (isresourceviewable(service_resource_cd)=true)
    ...
    reply->rpt_qual[cnt].report_id = rt.report_id, reply->rpt_qual[cnt].request_priority_cd = rt
    .priority_cd, reply->rpt_qual[cnt].status_cd = cr.status_cd,
    reply->rpt_qual[cnt].report_sequence = cr.report_sequence, reply->rpt_qual[cnt].catalog_cd = cr
    .catalog_cd, reply->rpt_qual[cnt].hold_cd = rt.hold_cd,
    reply->rpt_qual[cnt].hold_comment_long_text_id = rt.hold_comment_long_text_id
