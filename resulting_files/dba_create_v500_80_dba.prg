CREATE PROGRAM dba_create_v500_80:dba
 SET message = window
 ...
 DECLARE dc80_nls_path = vc WITH protect, noconstant("")
 DECLARE dc80_nls_name = vc WITH protect, noconstant("")
 ...
 IF ((dm_env_import_request->base_oracle_version_int >= 10))
  SET dc80_nls_name = "ORA_NLS10"
  SET dc80_nls_path = concat("'ORACLE_HOME'","/nls/data")
 ELSE
  SET dc80_nls_name = "ORA_NLS33"
  SET dc80_nls_path = concat("'ORACLE_HOME'","/ocommon/nls/admin/data")
 ENDIF
 SELECT INTO "nl:"
  a.environment_name
 ...
 ELSE
  IF ((dm_env_import_request->character_set != "US7ASCII"))
   ...
   CALL text(12,11,concat("\",unix_whval))
   CALL text(13,13,"\environment")
   CALL text(15,15,concat("NLS_LANG = ",nullterm(concat("AMERICAN_AMERICA.",cnvtupper(
        dm_env_import_request->character_set)))))
   CALL text(16,15,concat(nullterm(dc80_nls_name)," = ",nullterm(dc80_nls_path)))
   CALL clear(23,05,74)
   CALL text(23,05,"Please confirm the above registry information.  Continue(Y/N)? ")
   CALL accept(23,69,"P;CUS","Y")
   IF (curaccept="N")
   ...
   SELECT INTO concat("/tmp/v500_crt_",dbname,"_81.ksh")
    ...
    DETAIL
     ...
     IF (nullterm(cnvtupper(dm_env_import_request->character_set)) != "US7ASCII")
      col 0, "lreg -crek \\environment\\",
      CALL print(nullterm(cnvtlower(t_env_name))),
      CALL print(concat("\\definitions\\",unix_whval,"\\environment")), row + 1, col 0,
      "lreg -setp \\environment\\",
      CALL print(nullterm(cnvtlower(t_env_name))),
      CALL print(concat("\\definitions\\",unix_whval,"\\environment NLS_LANG ")),
      CALL print(nullterm(concat("AMERICAN_AMERICA.",cnvtupper(dm_env_import_request->character_set))
       )), row + 1, col 0,
      "lreg -setp \\environment\\",
      CALL print(nullterm(cnvtlower(t_env_name))),
      CALL print(concat("\\definitions\\",unix_whval,"\\environment ")),
      CALL print(nullterm(dc80_nls_name)), " ", '"',
      CALL print(nullterm(dc80_nls_path)), '"', row + 1
     ENDIF
     col 0, "lreg -crek \\DbInstance\\",
