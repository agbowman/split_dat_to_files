CREATE PROGRAM dm2_shc_shr_srvc:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(srv->srvcnt,- (1))=- (1))
  AND (validate(srv->srvcnt,- (2))=- (2)))
  RECORD srv(
    1 srvcnt = i4
    1 srv[*]
      2 server_name = vc
  ...
  SET srv->srvcnt = 0
  SET srv->srvcnt += 1
  SET stat = alterlist(srv->srv,srv->srvcnt)
  SET srv->srv[srv->srvcnt].server_id = "51"
  SET srv->srv[srv->srvcnt].server_name = "CPM Script"
  SET srv->srv[srv->srvcnt].ssp_binding = "NOT CONFIGURED"
  SET srv->srvcnt += 1
  SET stat = alterlist(srv->srv,srv->srvcnt)
  SET srv->srv[srv->srvcnt].server_id = "54"
  SET srv->srv[srv->srvcnt].server_name = "CPM Async Script"
  SET srv->srv[srv->srvcnt].ssp_binding = "NOT CONFIGURED"
  SET srv->srvcnt += 1
  SET stat = alterlist(srv->srv,srv->srvcnt)
  SET srv->srv[srv->srvcnt].server_id = "55"
  SET srv->srv[srv->srvcnt].server_name = "CPM Process"
  SET srv->srv[srv->srvcnt].ssp_binding = "NOT CONFIGURED"
  SET srv->srvcnt += 1
  SET stat = alterlist(srv->srv,srv->srvcnt)
  SET srv->srv[srv->srvcnt].server_id = "56"
  SET srv->srv[srv->srvcnt].server_name = "CPM Script Batch"
  SET srv->srv[srv->srvcnt].ssp_binding = "NOT CONFIGURED"
  SET srv->srvcnt += 1
  SET stat = alterlist(srv->srv,srv->srvcnt)
  SET srv->srv[srv->srvcnt].server_id = "67"
  SET srv->srv[srv->srvcnt].server_name = "CPM Async Script Class 001"
  SET srv->srv[srv->srvcnt].ssp_binding = "NOT CONFIGURED"
  SET srv->srvcnt += 1
  SET stat = alterlist(srv->srv,srv->srvcnt)
  SET srv->srv[srv->srvcnt].server_id = "200"
  SET srv->srv[srv->srvcnt].server_name = "Clinical Event Server"
  SET srv->srv[srv->srvcnt].ssp_binding = "NOT CONFIGURED"
  SET srv->srvcnt += 1
  SET stat = alterlist(srv->srv,srv->srvcnt)
  SET srv->srv[srv->srvcnt].server_id = "205"
  SET srv->srv[srv->srvcnt].server_name = "Clinical Event Server for ESI"
  SET srv->srv[srv->srvcnt].ssp_binding = "NOT CONFIGURED"
 ENDIF
 DECLARE dssp_status = c1 WITH protect, noconstant("X")
 DECLARE dssp_ret_val = i2 WITH protect, noconstant(0)
 DECLARE dssp_ssp_instances = i4 WITH protect, noconstant(0)
 DECLARE dssp_duplicate_ind = i2 WITH protect, noconstant(0)
 DECLARE dssp_missing_ind = i2 WITH protect, noconstant(0)
 DECLARE dssp_fatal_err_ind = i2 WITH protect, noconstant(0)
 DECLARE srv_cnt1 = i4 WITH protect, noconstant(0)
 DECLARE srv_cnt2 = i4 WITH protect, noconstant(0)
 DECLARE srv_cnt3 = i4 WITH protect, noconstant(0)
 DECLARE dssp_min_ssp_instances = i4 WITH protect, constant(8)
 IF (check_logfile("dm2_shc_ssp",".log","SYSTEM HEALTH CHECK SHARED SERVICE PROXY LOG FILE")=0)
  SET dssp_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking the configuration of Shared Service Proxies."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 SET dshc_lreg_getp->key_name = concat("\node\",dshc_env_info->node_name,"\domain\",dshc_env_info->
  domain_name,"\servers\1")
 SET dshc_lreg_getp->propname = "NumInstances"
 SET dshc_lreg_getp->value_type = 1
 SET dssp_ret_val = dshc_lreg_getp(null)
 IF (dssp_ret_val=0)
  SET dssp_status = "E"
 ELSE
  SET dssp_ssp_instances = cnvtint(dshc_lreg_getp->propvalue)
 ENDIF
 IF (dssp_status != "E")
  FOR (srv_cnt1 = 1 TO srv->srvcnt)
    SET dshc_lreg_getp->key_name = concat("\node\",dshc_env_info->node_name,"\domain\",dshc_env_info
     ->domain_name,"\servers\",
     srv->srv[srv_cnt1].server_id,"\prop")
    SET dshc_lreg_getp->propname = '"SSC Binding"'
    SET dshc_lreg_getp->value_type = 2
    SET dssp_ret_val = dshc_lreg_getp(null)
    IF (((dssp_ret_val=0) OR ((dshc_lreg_getp->propvalue=""))) )
     SET dssp_missing_ind = 1
    ELSE
     SET srv->srv[srv_cnt1].ssp_binding = dshc_lreg_getp->propvalue
     FOR (srv_cnt2 = 1 TO (srv->srvcnt - 1))
       IF ((srv->srv[srv_cnt1].ssp_binding=srv->srv[srv_cnt2].ssp_binding))
        SET dssp_duplicate_ind = 1
       ENDIF
     ENDFOR
    ENDIF
  ENDFOR
 ENDIF
 IF ((((dm_err->err_ind=1)) OR (dssp_status="E")) )
  SET dssp_status = "E"
 ELSEIF (dssp_ssp_instances >= dssp_min_ssp_instances
  AND dssp_missing_ind=0
  AND dssp_duplicate_ind=0)
  SET dssp_status = "G"
 ELSE
  SET dssp_status = "R"
 ENDIF
 IF (dshc_add_check_to_rs("SHARED SERVICE PROXIES","Shared Service Proxy Configuration",0,dssp_status,
  dshc_ctf_millennium)=0)
  SET dssp_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 IF (dssp_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 ...
 "Shared Service Proxy Instances"
 SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].value_prefix =
 build("Is the standard Shared Service Proxy configured with at least (",dssp_min_ssp_instances,
  ") instances?")
 ...
 IF (dssp_ssp_instances >= dssp_min_ssp_instances)
 ...
 ELSE
  ...
  " NO"
 ENDIF
 FOR (srv_cnt3 = 1 TO srv->srvcnt)
   SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
   SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,dshc->check[dshc->chk_cnt].
    desc_dtl_cnt)
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].name =
   "Servers Not Bound to a Dedicated SSP"
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].value_prefix
    = "NOT_SET"
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].dtl_desc_text
    =
   "Following are the affected application servers and their current dedicated Shared Service Proxy binding:"
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].type = 2
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].str_value =
   concat(srv->srv[srv_cnt3].server_name," (SCP ",srv->srv[srv_cnt3].server_id,"): ",srv->srv[
    srv_cnt3].ssp_binding)
 ENDFOR
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "Cerner recommends that the standard Shared Service Proxy be configured to run at least (",build(
   dssp_min_ssp_instances),") instances in a production domain.  Cerner also recommends that each ",
  "application server listed below be bound to its own unique dedicated Shared Service Proxy. ")
 CALL echo(dshc->check[dshc->chk_cnt].check_desc_text)
#exit_script
 SET dm_err->eproc = "Finished checking Shared Service Proxy configuration."
 CALL final_disp_msg("dm2_shc_ssp")
 IF (dssp_fatal_err_ind=0)
  SET dm_err->err_ind = 0
 ENDIF
 FREE RECORD srv
END GO
