CREATE PROGRAM dm_pm_hist_tracking_rows:dba
 ...
 SET reply->table_name = "PM_HIST_TRACKING"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(pht.pm_hist_tracking_id)
   FROM pm_hist_tracking pht
   WHERE pht.pm_hist_tracking_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(pht.pm_hist_tracking_id)
  FROM pm_hist_tracking pht
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    pht.rowid
    FROM pm_hist_tracking pht
    WHERE pht.pm_hist_tracking_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists(
      "pht.transaction_dt_tm < cnvtdatetime((curdate - days_to_keep), 0)","pht"))
    DETAIL
     rows += 1
     IF (mod(rows,500)=1)
      stat = alterlist(reply->rows,(rows+ 499))
     ENDIF
     reply->rows[rows].row_id = pht.rowid
    WITH nocounter, maxqual(pht,value(rowsleft))
   ;end select
