CREATE PROGRAM dm2_test_das:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 SET dm_err->logfile = "dm2_test_das.log"
 CALL init_logfile(dm_err->logfile,"TEST DM2_APPLY_SIZING")
 SET das_tspace = "D_A_SMALL"
 SET das_itspace = "I_A_SMALL"
 SET das_ltspace = "L_A_SMALL"
 SET tgtsch->tbl_cnt = 6
 SET stat = alterlist(tgtsch->tbl,tgtsch->tbl_cnt)
 SET cur_sch->tbl_cnt = 2
 SET stat = alterlist(cur_sch->tbl,cur_sch->tbl_cnt)
 CALL ins_ts_prec("DM2_DAS_TEST1","A_MEDIUM",1)
 CALL ins_ts_prec("DM2_DAS_TEST1","SYS_MGMT",2)
 CALL ins_ts_prec("DM2_DAS_TEST2","A_MEDIUM",1)
 CALL ins_ts_prec("DM2_DAS_TEST2","SYS_MGMT",2)
 CALL ins_ts_prec("DM2_DAS_TEST3","A_LARGE",1)
 CALL ins_ts_prec("DM2_DAS_TEST3","SYS_MGMT",2)
 CALL ins_ts_prec("DM2_DAS_TEST4","A_TESTDAS",1)
 CALL ins_ts_prec("DM2_DAS_TEST4","R_SMALL",2)
 CALL ins_ts_prec("DM2_DAS_TEST5","A_TESTDAS",1)
 CALL ins_ts_prec("DM2_DAS_TEST5","R_TESTDAS",2)
 SET tgtsch->tbl[1].tbl_name = "DM2_DAS_TEST1"
 SET tgtsch->tbl[1].cur_idx = 1
 SET tgtsch->tbl[1].diff_ind = 1
 SET cur_sch->tbl[1].tbl_name = "DM2_DAS_TEST1"
 SET cur_sch->tbl[1].tspace_name = "D_A_MEDIUM"
 SET cur_sch->tbl[1].ind_tspace = "I_A_MEDIUM"
 SET cur_sch->tbl[1].long_tspace = "L_A_MEDIUM"
 SET tgtsch->tbl[2].tbl_name = "DM2_DAS_TEST2"
 SET tgtsch->tbl[2].cur_idx = 2
 SET tgtsch->tbl[2].diff_ind = 1
 SET cur_sch->tbl[2].tbl_name = "DM2_DAS_TEST1"
 SET cur_sch->tbl[2].tspace_name = "D_SYS_MGMT"
 SET cur_sch->tbl[2].ind_tspace = "I_SYS_MGMT"
 SET cur_sch->tbl[2].long_tspace = "L_SYS_MGMT"
 SET tgtsch->tbl[3].tbl_name = "DM2_DAS_TEST3"
 SET tgtsch->tbl[3].cur_idx = 0
 SET tgtsch->tbl[3].new_ind = 1
 SET tgtsch->tbl[4].tbl_name = "DM2_DAS_TEST4"
 SET tgtsch->tbl[4].cur_idx = 0
 SET tgtsch->tbl[4].new_ind = 1
 SET tgtsch->tbl[5].tbl_name = "DM2_DAS_TEST5"
 SET tgtsch->tbl[5].cur_idx = 0
 SET tgtsch->tbl[5].new_ind = 1
 SET tgtsch->tbl[6].tbl_name = "DM2_DAS_TEST6"
 SET tgtsch->tbl[6].cur_idx = 0
 SET tgtsch->tbl[6].new_ind = 1
 FOR (x = 1 TO tgtsch->tbl_cnt)
   SET tgtsch->tbl[x].tspace_name = das_tspace
   SET tgtsch->tbl[x].ind_tspace = das_itspace
   SET tgtsch->tbl[x].long_tspace = das_ltspace
 ENDFOR
 EXECUTE dm2_apply_sizing
 CALL echo("BACK FROM APPLY SIZING")
 CALL echo(tgtsch->tbl_cnt)
 ROLLBACK
 IF ((dm_err->err_ind=0))
  FOR (x = 1 TO tgtsch->tbl_cnt)
    CALL echo("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
    CALL echo(concat("TABLE NAME:",tgtsch->tbl[x].tbl_name))
    CALL echo(concat("TSPACE NAME:",tgtsch->tbl[x].tspace_name))
    CALL echo(concat("IND TSPACE NAME:",tgtsch->tbl[x].ind_tspace))
    CALL echo(concat("LONG TSPACE NAME:",tgtsch->tbl[x].long_tspace))
  ENDFOR
 ENDIF
 SUBROUTINE ins_ts_prec(tp_tname,tp_tsname,tp_tsprec)
   DECLARE dtspace = vc
   DECLARE itspace = vc
   DECLARE ltspace = vc
   SET dtspace = build("D_",tp_tsname)
   SET itspace = build("I_",tp_tsname)
   SET ltspace = build("L_",tp_tsname)
   INSERT  FROM dm_ts_precedence t
    SET t.table_name = tp_tname, t.precedence = tp_tsprec, t.data_tablespace = dtspace,
     t.index_tablespace = itspace, t.long_tablespace = ltspace, t.data_extent_size = 163840,
     t.index_extent_size = 163840, t.long_extent_size = 163840, t.updt_id = 0,
     t.updt_cnt = 0, t.updt_applctx = 0, t.updt_dt_tm = cnvtdatetime(curdate,curtime3),
     t.updt_task = 0
    WITH nocounter
   ;end insert
 END ;Subroutine
