CREATE PROGRAM dm_rmc_xml_clob:dba
 ...
 SUBROUTINE dm2_get_program_stack(null)
   ...
   ENDIF
 END ;Subroutine
 IF (check_logfile("DM_RMC_XML_CLOB",".log","DM_RMC_XML_CLOB LogFile")=0)
  SET dm_err->err_ind = 1
  GO TO exit_program
 ENDIF
 IF ((validate(request->mode,- (1))=- (1)))
  FREE RECORD request
  RECORD request(
    1 mode = i2
    1 context_name = vc
  ...
  IF (((reflect(parameter(1,0)) != "I*"
   AND reflect(parameter(1,0)) != "F*") OR (((reflect(parameter(2,0)) != "C*") OR (reflect(parameter(
    3,0)) != "C*")) )) )
   SET dm_err->err_ind = 1
   SET dm_err->emsg = "Expected syntax: dm_rmc_xml_clob <mode>, '<context_name>', '<XML or JSON>'"
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   GO TO exit_program
  ELSE
   SET request->mode =  $1
   SET request->context_name =  $2
   SET request->encoded_type =  $3
  ENDIF
 ENDIF
 FREE RECORD drxc_request
 RECORD drxc_request(
   1 source_env_id = f8
   1 target_env_id = f8
 )
 DECLARE drxc_env_pair = vc WITH protect, noconstant("")
 DECLARE drxc_map_id = f8 WITH protect, noconstant(0)
 DECLARE drxc_func_name = vc WITH protect, noconstant("")
 SET modify maxvarlen 268435456
 IF ((((request->source_env_id=0)) OR ((request->target_env_id=0))) )
  SET dm_err->eproc = "Obtaining target environment id"
  SELECT INTO "NL:"
   FROM dm_info d
   WHERE info_domain="DATA MANAGEMENT"
    AND info_name="DM_ENV_ID"
   DETAIL
    drxc_request->target_env_id = d.info_number
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET dm_err->eproc = "Obtaining source environment id"
  SELECT INTO "NL:"
   FROM dm_rdds_event_log d
   WHERE (d.cur_environment_id=drxc_request->target_env_id)
   ...
   (SELECT
    ...
    WHERE (d1.cur_environment_id=drxc_request->target_env_id)
     AND d1.rdds_event_key="ENDREFERENCEDATASYNC")))
   DETAIL
    drxc_request->source_env_id = d.paired_environment_id
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ELSE
  SET drxc_request->source_env_id = request->source_env_id
  SET drxc_request->target_env_id = request->target_env_id
 ENDIF
 SET drxc_env_pair = concat(trim(cnvtstring(drxc_request->source_env_id),3),"::",trim(cnvtstring(
    drxc_request->target_env_id),3))
 SET dm_err->eproc = "Retrieving environment pair id value from DM_INFO"
 SELECT INTO "NL:"
  FROM dm_info di
  WHERE di.info_domain="RDDS ENV PAIR"
   AND di.info_name=drxc_env_pair
  DETAIL
   drxc_map_id = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (drxc_map_id=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("Unable to retrieve map_id from DM_INFO for env pair:",drxc_env_pair)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ((request->encoded_type="XML"))
  SET drxc_func_name = concat("rdds_xml_clob_wrp_",trim(cnvtstring(drxc_map_id)))
 ELSE
  SET drxc_func_name = concat("rdds_xlat_json_",trim(cnvtstring(drxc_map_id)))
 ENDIF
 CALL parser(concat("declare ",drxc_func_name,"() = c524288 go"))
 SET dm_err->eproc = "Updating DM_CLOB_GTTD with translated CLOB values"
 CALL parser(concat("update into dm_clob_gttd dcg ","  set dcg.dm_clob = ",drxc_func_name,"(",trim(
    cnvtstring(request->mode),3),
   ", dcg.dm_clob,'",request->context_name,"')"," where 1=1"," go"))
#exit_program
END GO
