CREATE PROGRAM dm2_mig_expimp_menu:dba
 ...
 RECORD dmem_stg_info(
   1 node_cnt = i4
   1 nodes[*]
     2 node_name = vc
     2 instance_name = vc
 )
 DECLARE dmem_quit_ind = i2 WITH protect, noconstant(0)
 DECLARE dmem_def_file_loc = vc WITH protect, noconstant(evaluate(dm2_sys_misc->cur_os,"AXP",
   "GGDELIVERY:[DIRTMP]","/ggdelivery/dirtmp"))
 DECLARE dmem_file_loc = vc WITH protect, noconstant(evaluate(dm2_sys_misc->cur_os,"AXP",
   "GGDELIVERY:[DIRTMP]","/ggdelivery/dirtmp/"))
 DECLARE dmem_max_imports = i4 WITH protect, noconstant(0)
 DECLARE dmem_import_pattern = vc WITH protect, noconstant("DM2_NOTSET")
 DECLARE dmem_expimp_driver_status_msg = vc WITH protect, noconstant("")
 DECLARE dmem_read_only_mode = i2 WITH protect, noconstant(0)
 DECLARE dmem_index = i4 WITH protect, noconstant(0)
 DECLARE dmem_stg_db_name = vc WITH protect, noconstant("")
 DECLARE dmem_stg_created_date = f8 WITH protect, noconstant(0.0)
 DECLARE dmem_iter = i4 WITH protect, noconstant(0)
 DECLARE dmem_create_table_done = i2 WITH protect, noconstant(0)
 DECLARE dmem_ut_expimp_driver_done = i2 WITH protect, noconstant(0)
 DECLARE dmem_ut_expimp_data_done = i2 WITH protect, noconstant(0)
 DECLARE dmem_ut_expimp_schema_done = i2 WITH protect, noconstant(0)
 DECLARE dmem_mig_replicate_done = i2 WITH protect, noconstant(0)
 DECLARE dmem_dt_expimp_driver_done = i2 WITH protect, noconstant(0)
 DECLARE dmem_dt_expimp_data_done = i2 WITH protect, noconstant(0)
 DECLARE dmem_src_file = vc WITH protect, noconstant("")
 DECLARE dmem_tgt_file = vc WITH protect, noconstant("")
 DECLARE dmem_downtime_complete = i2 WITH protect, noconstant(0)
 DECLARE dmem_ut_expimp_setup_done = i2 WITH protect, noconstant(0)
 DECLARE dmem_using_dbx = i2 WITH protect, noconstant(0)
 DECLARE dmem_process_imports_screen(mode=vc,process=vc) = i2
 DECLARE dmem_generate_config_file(null) = i2
 DECLARE dmem_update_max_imports(null) = i2
 DECLARE dmem_set_checkpoints(null) = i2
 DECLARE dmem_manage_checkpoints(dmc_info_domain=vc,dmc_info_name=vc) = i2
 DECLARE dmem_manage_files(dmf_src_file=vc,dmf_tgt_file=vc) = i2
 DECLARE dmem_read_config_file(drcf_maximports_out=i4(ref),drcf_import_pattern_out=vc(ref)) = i2
 DECLARE dmem_stage_prompt(null) = i2
 IF (check_logfile("dm2_mig_expimp_menu",".log","dm2_mig_expimp_menu LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_expimp_menu"
 CALL disp_msg("",dm_err->logfile,0)
 IF ( NOT (dmr_prompt_retrieve_mig_data(0,1,1,0)))
  GO TO exit_script
 ENDIF
 IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
  IF ( NOT (dmr_prompt_connect_data("SOURCE","V500","CO")))
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Querying for max imports from dm_info."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_BULK_DATA_MOVE"
   AND di.info_name="MAXIMPORTS"
  DETAIL
   dmem_max_imports = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc))
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ENDIF
 IF ( NOT (curqual))
  SET dm_err->eproc = "Inserting new max imports row into dm_info."
  INSERT  FROM dm_info di
   SET di.info_domain = "DM2_MIG_BULK_DATA_MOVE", di.info_name = "MAXIMPORTS", di.info_number = 0
   WITH nocounter
  ;end insert
  IF (check_error(dm_err->eproc))
   ROLLBACK
  ...
  COMMIT
 ENDIF
 SET dm_err->eproc = "Retrieving GG Delivery Directory path."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2_MIG_GG_DATA"
   AND di.info_name="GG_DEL_DIR"
  DETAIL
   dmr_expimp->tgt_file_loc = trim(concat(di.info_char,"/dirtmp"),10)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSEIF (size(trim(dmr_expimp->tgt_file_loc,3))=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat("No DM_INFO rows exist for Delivery Directory path.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Check BDM option"
 SELECT INTO "nl:"
  dbx_cnt = count(*)
  FROM dm2_admin_dm_info di
  WHERE di.info_domain="DM2MIG_BDM_OPTION"
   AND di.info_name="DBX_IN_USE"
  DETAIL
   IF (dbx_cnt > 0)
    dmem_using_dbx = 1
   ELSE
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 IF (substring(size(trim(dmr_expimp->tgt_file_loc,3)),1,dmr_expimp->tgt_file_loc)="/")
  SET dmr_expimp->tgt_file_loc = trim(replace(dmr_expimp->tgt_file_loc,"/","",2),3)
 ENDIF
 IF ( NOT (dmr_directory_prompt(2)))
  GO TO exit_script
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dmr_expimp)
 ENDIF
 IF (dmem_set_checkpoints(null)=0)
  GO TO exit_script
 ENDIF
 IF ((dmr_expimp->mode="UPTIME"))
  IF (dmem_stage_prompt(null)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 ...
 WHILE (true)
   ...
   CALL clear(1,1)
   CALL box(1,1,24,131)
   CALL text(2,2,"Export/Import Utility")
   CALL text(2,115,concat("Mode:",dmr_expimp->mode))
   IF ((dmr_expimp->step="SETUP"))
    IF ((dmr_expimp->process="INITIAL SETUP"))
     CALL text(4,10,"1. Setup")
     CALL text(6,10,"2. Generate Imports [<Unavailable>]")
    ELSE
     CALL text(4,10,"1. Setup [Completed]")
     CALL text(6,10,"2. Generate Imports ")
    ENDIF
   ELSE
    CALL text(4,10,"1. Setup [Completed]")
    CALL text(6,10,"2. Generate Imports [Completed]")
   ENDIF
   IF ((dmr_expimp->src_ksh_loc != dmem_def_file_loc))
    CALL text(8,10,"3. Process Imports [<Unavailable>]")
   ELSEIF ((dmr_expimp->step="EXECUTE")
    AND (dmr_expimp->step_ready=1))
    CALL text(8,10,"3. Process Imports")
   ELSEIF ((dmr_expimp->step="EXECUTE")
    AND (dmr_expimp->step_ready=0))
    CALL text(8,10,"3. Process Imports [Holding]")
   ELSEIF ((dmr_expimp->step != "EXECUTE"))
    CALL text(8,10,"3. Process Imports [<Unavailable>]")
   ENDIF
   CALL text(16,10,"Your Selection (0 to Exit)?")
   IF ((dmr_expimp->step="SETUP")
    AND (dmr_expimp->process="INITIAL SETUP"))
    CALL accept(16,38,"9;",0
     WHERE curaccept IN (0, 1))
   ELSEIF ((((dmr_expimp->step="SETUP")) OR ((dmr_expimp->src_ksh_loc != dmem_def_file_loc))) )
    CALL accept(16,38,"9;",0
     WHERE curaccept IN (0, 1, 2))
   ELSEIF ((dmr_expimp->step="EXECUTE")
    AND (dmr_expimp->step_ready=1)
    AND (dmr_expimp->src_ksh_loc=dmem_def_file_loc))
    CALL accept(16,38,"9;",0
     WHERE curaccept IN (0, 3))
   ELSEIF ((dmr_expimp->step="EXECUTE")
    AND (dmr_expimp->step_ready=0))
    CALL accept(16,38,"9;",0
     WHERE curaccept IN (0, 1, 2))
   ...
   CASE (curaccept)
    ...
    OF 1:
     IF ((dmr_expimp->step="SETUP"))
      IF ( NOT (dmr_directory_prompt(1)))
       ...
       ENDIF
      ENDIF
      SET dm_err->eproc = "Remove the INITIAL SETUP complete indicator"
      DELETE  FROM dm_info d
       WHERE d.info_domain="DM2_MIGRATION"
        AND d.info_name="DM2_MIG_UT_EXPIMP_SETUP_COMPLETE"
       WITH nocounter
      ;end delete
      IF (check_error(dm_err->eproc)=1)
       ROLLBACK
      ...
      IF (dmem_quit_ind=1)
       SET dmem_quit_ind = 0
      ELSEIF ((dmr_expimp->src_ksh_loc=dmem_def_file_loc))
       IF ((dmr_expimp->mode="UPTIME"))
        SET dmem_src_file = "CER_INSTALL:dm2ei_setup.txt"
        SET dmem_tgt_file = build(dmem_file_loc,"dm2ei_setup.ksh")
        IF (dmem_manage_files(dmem_src_file,dmem_tgt_file)=0)
         GO TO exit_script
        ENDIF
        SET dmem_src_file = "CER_INSTALL:dm2ei_crontab_add.txt"
        SET dmem_tgt_file = build(dmem_file_loc,"dm2ei_crontab_add.ksh")
        IF (dmem_manage_files(dmem_src_file,dmem_tgt_file)=0)
         GO TO exit_script
        ENDIF
        SET dmem_src_file = "CER_INSTALL:dm2ei_crontab_remove.txt"
        SET dmem_tgt_file = build(dmem_file_loc,"dm2ei_crontab_remove.ksh")
        IF (dmem_manage_files(dmem_src_file,dmem_tgt_file)=0)
         GO TO exit_script
        ENDIF
        SET dmem_src_file = "CER_INSTALL:dm2ei_cron.txt"
        SET dmem_tgt_file = build(dmem_file_loc,"dm2ei_cron.ksh")
        IF (dmem_manage_files(dmem_src_file,dmem_tgt_file)=0)
         GO TO exit_script
        ENDIF
       ELSE
        IF (((findfile(build(dmem_file_loc,"dm2ei_setup.ksh"))=0) OR (((findfile(build(dmem_file_loc,
          "dm2ei_crontab_add.ksh"))=0) OR (((findfile(build(dmem_file_loc,"dm2ei_crontab_remove.ksh")
         )=0) OR (findfile(build(dmem_file_loc,"dm2ei_cron.ksh"))=0)) )) )) )
         SET dm_err->err_ind = 1
         SET dm_err->emsg = "One or more of required components do not exist."
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         GO TO exit_script
        ENDIF
       ENDIF
       ...
       CALL clear(1,1)
       CALL box(1,1,24,131)
       CALL text(2,2,"Database Migration Process (Bulk Data Move) - Setup")
       CALL text(4,2,concat("On the Node ",dmr_mig_data->tgt_nodes[1].node_name,
         ", execute the following as the oracle user:"))
       CALL text(5,5,concat("$chmod 777 ",dmr_expimp->tgt_file_loc,"/*.*"))
       CALL text(6,5,concat("$",dmr_expimp->tgt_file_loc,"/dm2ei_setup.ksh"))
       CALL text(8,2,"Upon successful completion, please enter 'C' to Continue or 'Q' to Quit:")
       CALL accept(8,75,"A;CU"," "
        WHERE curaccept IN ("C", "Q"))
       CASE (curaccept)
        OF "Q":
         SET dmem_quit_ind = 1
         GO TO exit_script
        OF "C":
         SET dm_err->eproc = "Insert the INITIAL SETUP complete indicator"
         INSERT  FROM dm_info d
          SET d.info_domain = "DM2_MIGRATION", d.info_name = "DM2_MIG_UT_EXPIMP_SETUP_COMPLETE"
          WITH nocounter
         ;end insert
         IF (check_error(dm_err->eproc)=1)
          ROLLBACK
    ...
    OF 2:
     IF ((dmr_expimp->step="SETUP"))
      ...
      CALL clear(1,1)
      IF ((dmr_expimp->mode="UPTIME"))
       SET dmem_import_pattern = "dm2_.*_ei_ut"
      ELSEIF ((dmr_expimp->mode="DOWNTIME"))
       SET dmem_import_pattern = "dm2_.*_ei_dd"
      ENDIF
      SET dmem_max_imports = 0
      IF ( NOT (dmem_update_max_imports(null)))
       GO TO exit_script
      ENDIF
      IF ((dmr_expimp->src_ksh_loc=dmem_def_file_loc))
       IF ( NOT (dmem_generate_config_file(null)))
        GO TO exit_script
       ENDIF
      ENDIF
      EXECUTE dm2_mig_expimp_driver
      IF (dm_err->err_ind)
       GO TO exit_script
      ENDIF
     ENDIF
    OF 3:
     ...
     CALL clear(1,1)
     IF ((((dmr_expimp->mode="UPTIME")
      AND (dmr_expimp->process="DATA")) OR ((((dmr_expimp->mode="UPTIME")
      AND (dmr_expimp->process="SCHEMA")) OR ((dmr_expimp->mode="DOWNTIME")
      AND (dmr_expimp->process="DATA"))) )) )
      IF ( NOT (dmem_process_imports_screen(dmr_expimp->mode,dmr_expimp->process)))
       IF (dm_err->err_ind)
        GO TO exit_script
       ENDIF
      ENDIF
 ...
 ENDWHILE
 GO TO exit_script
 SUBROUTINE dmem_stage_prompt(null)
   SET message = window
   CALL clear(1,1)
   CALL box(1,1,4,131)
   CALL text(2,5,"Is a staging database being used for the database migration? (Y/N)")
   CALL accept(2,80,"P;CU"," "
    WHERE curaccept IN ("Y", "N"))
   SET message = nowindow
   IF (curaccept="Y")
    SET dm2_install_schema->dbase_name = '"STAGING DB"'
    ...
    IF ((dm_err->err_ind=1))
     RETURN(0)
    ENDIF
    SET dmr_expimp->stg_v500_p_word = dm2_install_schema->p_word
    SET dmr_expimp->stg_v500_cnct_str = dm2_install_schema->connect_str
    IF (dmr_get_db_info(dmem_stg_db_name,dmem_stg_created_date)=0)
     RETURN(0)
    ENDIF
    SET dmr_expimp->stg_db_name = dmem_stg_db_name
    SET dmr_expimp->stg_created_date = dmem_stg_created_date
    IF (dmr_get_node_name(null)=0)
     RETURN(0)
    ENDIF
    SET dmem_stg_info->node_cnt = dmr_node->cnt
    SET stat = alterlist(dmem_stg_info->nodes,dmr_node->cnt)
    FOR (dmem_iter = 1 TO dmr_node->cnt)
     SET dmem_stg_info->nodes[dmem_iter].node_name = cnvtlower(dmr_node->qual[dmem_iter].node_name)
     SET dmem_stg_info->nodes[dmem_iter].instance_name = cnvtlower(dmr_node->qual[dmem_iter].
      instance_name)
    ENDFOR
    IF (dm2_get_rdbms_version(null)=0)
     RETURN(0)
    ...
    IF ((dm2_rdbms_version->level1 > 11))
     SET dm_err->eproc = "Querying for read only mode from STAGINGs v$pdbs"
     SELECT INTO "nl:"
      FROM v$pdbs v
      DETAIL
       IF (v.open_mode="READ ONLY")
        dmem_read_only_mode = 1
       ENDIF
      WITH nocounter
     ;end select
    ELSE
     SET dm_err->eproc = "Querying for read only mode from STAGINGs v$database"
     SELECT INTO "nl:"
      FROM v$database v
      DETAIL
       IF (v.open_mode="READ ONLY")
    ...
    ENDIF
   ELSE
    SET dmr_expimp->stg_v500_cnct_str = " "
   ENDIF
   IF ((dmem_stg_info->node_cnt > 0))
    FOR (dmem_iter = 1 TO dmr_mig_data->src_node_cnt)
      IF (locateval(dmem_index,1,dmem_stg_info->node_cnt,dmr_mig_data->src_nodes[dmem_iter].node_name,
       dmem_stg_info->nodes[dmem_index].node_name)
       AND dmem_read_only_mode=0)
       SET dm_err->emsg =
       "Cannot have a staging db with same node name, which is not in read only mode"
       SET dm_err->err_ind = 1
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ENDIF
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmem_read_config_file(drcf_maximports_out,drcf_import_pattern_out)
   DECLARE drcf_empty_file = i2 WITH protect, noconstant(1)
   DECLARE drcf_maximp_found = i2 WITH protect, noconstant(0)
   DECLARE drcf_imppat_found = i2 WITH protect, noconstant(0)
   DECLARE drcf_dest = vc WITH protect, noconstant(build(dmem_file_loc,"dm2ei_config.txt"))
   IF ((dm_err->debug_flag > 0))
    CALL echo(dmem_file_loc)
   ENDIF
   IF (findfile(build(dmem_file_loc,"dm2ei_config.txt"))=0)
    SET dm_err->eproc = "Verify that dm2ei_config.txt exists."
    SET dm_err->emsg = build(dmem_file_loc,"dm2ei_config.txt does not exist")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    RETURN(0)
   ENDIF
   FREE DEFINE rtl2
   SET logical drcf_dest_log drcf_dest
   DEFINE rtl2 "drcf_dest_log"  WITH nomodify
   SET dm_err->eproc = concat("Reading dm2ei_config.txt to determine maximports and import pattern.")
   SELECT INTO "nl:"
    FROM rtl2t r
    DETAIL
     drcf_empty_file = 0
     IF (findstring("maximports=",r.line,1,1) > 0)
      drcf_maximports_out = cnvtint(trim(substring((findstring("=",r.line,1,1)+ 1),(textlen(r.line)
          - findstring("=",r.line,1,1)),r.line))), drcf_maximp_found = 1
     ENDIF
     IF (findstring("import_pattern=",r.line,1,1) > 0)
      drcf_import_pattern_out = trim(substring((findstring("=",r.line,1,1)+ 1),(textlen(r.line) -
        findstring("=",r.line,1,1)),r.line)), drcf_imppat_found = 1
     ENDIF
     IF ((dm_err->debug_flag > 0))
      CALL echo(r.line)
     ENDIF
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("import_pattern=",drcf_import_pattern_out))
    CALL echo(build("maximports=",drcf_maximports_out))
   ENDIF
   IF (drcf_empty_file=1)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "dm2ei_config.txt is empty."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ELSEIF (((drcf_imppat_found=0) OR (drcf_maximp_found=0)) )
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "dm2ei_config.txt is incomplete."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmem_manage_files(dmf_src_file,dmf_tgt_file)
   DECLARE dmf_temp_line = vc WITH protect, noconstant("")
   FREE DEFINE rtl2
   SET logical file_name dmf_src_file
   DEFINE rtl2 "file_name"  WITH nomodify
   SET dm_err->eproc = concat("Creating:",dmf_tgt_file)
   CALL disp_msg("",dm_err->logfile,0)
   SELECT INTO value(dmf_tgt_file)
    FROM rtl2t r
    DETAIL
     IF (findstring("dm2ei_setup.ksh",dmf_tgt_file,1,1) > 0)
      dmf_temp_line = replace(r.line,"<gg_del_dir>",dmr_expimp->tgt_file_loc), dmf_temp_line =
      replace(dmf_temp_line,"<oracle_home_source>",dmr_expimp->src_ora_home), dmf_temp_line = replace
      (dmf_temp_line,"<oracle_home_target>",dmr_expimp->tgt_ora_home),
      CALL print(dmf_temp_line), row + 1
     ELSE
      dmf_temp_line = replace(r.line,"<gg_del_dir>",dmr_expimp->tgt_file_loc),
      CALL print(dmf_temp_line), row + 1
     ENDIF
    WITH nocounter, maxrow = 1, maxcol = 2001,
     format = variable, format = lfstream, formfeed = none
   ;end select
   IF (check_error(dm_err->eproc))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmem_manage_checkpoints(dmc_info_domain,dmc_info_name)
   DECLARE dmc_db_type = vc WITH protect, noconstant(dmr_mig_data->cur_db_type)
   IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
    IF ( NOT (dmr_prompt_connect_data("SOURCE","V500","CO")))
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->eproc = "Querying for checkpoint row from dm_info."
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain=dmc_info_domain
     AND d.info_name=dmc_info_name
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dm_err->eproc = "Insert checkpoint row into dm_info."
    INSERT  FROM dm_info d
     SET d.info_domain = dmc_info_domain, d.info_name = dmc_info_name
     WITH nocounter
    ;end insert
    IF (check_error(dm_err->eproc))
     ROLLBACK
    ...
    ENDIF
   ENDIF
   IF (dmc_db_type != "SOURCE")
    IF ( NOT (dmr_prompt_connect_data(dmc_db_type,"V500","CO")))
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmem_set_checkpoints(null)
   DECLARE dsc_db_type = vc WITH protect, noconstant(dmr_mig_data->cur_db_type)
   IF ((dmr_mig_data->cur_db_type != "SOURCE"))
    IF ( NOT (dmr_prompt_connect_data("SOURCE","V500","CO")))
     RETURN(0)
    ENDIF
   ENDIF
   SET dmem_ut_expimp_driver_done = 0
   SET dmem_ut_expimp_data_done = 0
   SET dmem_ut_expimp_schema_done = 0
   SET dmem_mig_replicate_done = 0
   SET dmem_dt_expimp_driver_done = 0
   SET dmem_dt_expimp_data_done = 0
   SET dmem_ut_expimp_setup_done = 0
   SET dm_err->eproc = "Querying for checkpoint rows from dm_info."
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain IN ("DM2_MIGRATION", "DM2_MIG_REPLICATE")
    DETAIL
     IF (d.info_domain="DM2_MIG_REPLICATE"
      AND d.info_name="SUCCESSFUL_CREATE_TABLES")
      dmem_create_table_done = 1
     ELSEIF (d.info_domain="DM2_MIG_REPLICATE"
      AND d.info_name="SUCCESSFUL_EXECUTION")
      dmem_mig_replicate_done = 1
     ELSEIF (d.info_domain="DM2_MIGRATION"
      AND d.info_name="DM2_MIG_UT_EXPIMP_DRIVER_STATUS"
      AND d.info_number=0)
      dmem_ut_expimp_driver_done = 1
     ELSEIF (d.info_domain="DM2_MIGRATION"
      AND d.info_name="DM2_MIG_UT_EXPIMP_DATA")
      dmem_ut_expimp_data_done = 1
     ELSEIF (d.info_domain="DM2_MIGRATION"
      AND d.info_name="DM2_MIG_UT_EXPIMP_SCHEMA")
      dmem_ut_expimp_schema_done = 1
     ELSEIF (d.info_domain="DM2_MIGRATION"
      AND d.info_name="DM2_MIG_DT_EXPIMP_DRIVER_STATUS")
      dmem_dt_expimp_driver_done = 1
     ELSEIF (d.info_domain="DM2_MIGRATION"
      AND d.info_name="DM2_MIG_DT_EXPIMP_DATA")
      dmem_dt_expimp_data_done = 1
     ELSEIF (d.info_domain="DM2_MIGRATION"
      AND d.info_name="DM2_MIG_UT_EXPIMP_SETUP_COMPLETE")
      dmem_ut_expimp_setup_done = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc))
   ...
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("dmem_create_table_done    :",dmem_create_table_done))
    CALL echo(build("dmem_ut_expimp_driver_done:",dmem_ut_expimp_driver_done))
    CALL echo(build("dmem_ut_expimp_data_done  :",dmem_ut_expimp_data_done))
    CALL echo(build("dmem_ut_expimp_schema_done:",dmem_ut_expimp_schema_done))
    CALL echo(build("dmem_mig_replicate_done   :",dmem_mig_replicate_done))
    CALL echo(build("dmem_dt_expimp_driver_done:",dmem_dt_expimp_driver_done))
    CALL echo(build("dmem_dt_expimp_data_done  :",dmem_dt_expimp_data_done))
   ENDIF
   IF (dmem_using_dbx=1)
    IF (dmem_ut_expimp_setup_done=0)
     SET dmr_expimp->mode = "UPTIME"
     SET dmr_expimp->process = "INITIAL SETUP"
     SET dmr_expimp->step = "SETUP"
    ELSE
     SET dmr_expimp->mode = "DOWNTIME"
     SET dmr_expimp->process = "DATA"
     IF (dmem_dt_expimp_driver_done=0)
      SET dmr_expimp->step = "SETUP"
     ELSE
      SET dmr_expimp->step = "EXECUTE"
      IF (dmem_dt_expimp_data_done=0)
       SET dmr_expimp->step_ready = 1
      ELSE
       SET dmr_expimp->step_ready = 0
       CALL clear(1,1)
       SET message = nowindow
       SET dm_err->eproc = "Downtime Export/Import Completed Successfully"
       CALL disp_msg("",dm_err->logfile,0)
       SET dmem_downtime_complete = 1
       GO TO exit_script
      ENDIF
     ENDIF
    ENDIF
   ELSE
    IF (dmem_ut_expimp_schema_done=1)
     SET dmr_expimp->mode = "DOWNTIME"
     SET dmr_expimp->process = "DATA"
     IF (dmem_dt_expimp_driver_done=0)
      SET dmr_expimp->step = "SETUP"
    ...
    ELSE
     ...
     IF (dmem_ut_expimp_setup_done=0)
      SET dmr_expimp->process = "INITIAL SETUP"
      SET dmr_expimp->step = "SETUP"
     ELSEIF (dmem_ut_expimp_driver_done=0)
      SET dmr_expimp->process = "DATA"
      SET dmr_expimp->step = "SETUP"
     ELSE
      SET dmr_expimp->step = "EXECUTE"
      IF (dmem_ut_expimp_data_done=0)
       SET dmr_expimp->process = "DATA"
       IF (dmem_create_table_done=0)
        SET dmr_expimp->step_ready = 0
       ELSE
        SET dmr_expimp->step_ready = 1
       ENDIF
      ELSE
       SET dmr_expimp->process = "SCHEMA"
       IF (dmem_mig_replicate_done=0)
        SET dmr_expimp->step_ready = 0
       ELSE
        SET dmr_expimp->step_ready = 1
       ENDIF
    ...
    ENDIF
   ENDIF
   IF (dsc_db_type != "SOURCE")
    IF ( NOT (dmr_prompt_connect_data(dsc_db_type,"V500","CO")))
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmem_process_imports_screen(mode,process)
   DECLARE dpis_mode = vc WITH protect, noconstant(cnvtupper(mode))
   DECLARE dpis_process = vc WITH protect, noconstant(cnvtupper(process))
   DECLARE dpis_resumable_cnt = i4 WITH protect, noconstant(0)
   DECLARE dpis_imports_total = i4 WITH protect, noconstant(0)
   DECLARE dpis_imports_completed = i4 WITH protect, noconstant(0)
   DECLARE dpis_imports_remaining = i4 WITH protect, noconstant(0)
   DECLARE dpis_imports_failed = i4 WITH protect, noconstant(0)
   DECLARE dpis_imports_running = i4 WITH protect, noconstant(0)
   DECLARE dpis_local_pattern = vc WITH protect, noconstant("")
   DECLARE dpis_num = i4 WITH protect, noconstant(0)
   DECLARE dpis_vert_ptr = i2 WITH protect, noconstant(0)
   DECLARE dpis_iter = i2 WITH protect, noconstant(0)
   DECLARE dpis_comma_ptr = i4 WITH protect, noconstant(0)
   DECLARE dpis_page_ptr = i4 WITH protect, noconstant(1)
   DECLARE dpis_cur_accept = vc WITH protect, noconstant("")
   DECLARE dpis_cmd = vc WITH protect, noconstant("")
   DECLARE dpis_start_time = dq8 WITH protect, noconstant(0.0)
   DECLARE dpis_head_foot_cnt = i2 WITH protect, noconstant(0)
   DECLARE dpis_import_cntr = i2 WITH protect, noconstant(0)
   DECLARE dpis_import_cntr_total = i2 WITH protect, constant(5)
   DECLARE dpis_sup_iter = i2 WITH protect, noconstant(0)
   DECLARE dpis_ndx = i4 WITH protect, noconstant(0)
   DECLARE dpis_maximports = i4 WITH protect, noconstant(0)
   DECLARE dpis_import_pattern = vc WITH protect, noconstant("")
   DECLARE dpis_top_while = i2 WITH protect, noconstant(0)
   FREE RECORD dmem_schema_ids
   RECORD dmem_schema_ids(
     1 schema_cnt = i4
     1 qual[*]
       2 owner_name = vc
       2 schema_id = f8
   )
   FREE RECORD dmem_imp_progress
   RECORD dmem_imp_progress(
     1 imp_cnt = i4
     1 list[*]
       2 owner = vc
       2 table_name = vc
   ...
   IF (dmem_read_config_file(dpis_maximports,dpis_import_pattern)=0)
    RETURN(0)
   ENDIF
   SET dmem_import_pattern = dpis_import_pattern
   SET dpis_local_pattern = substring((findstring("_",dpis_import_pattern,1,1)+ 1),(textlen(
     dpis_import_pattern) - findstring("_",dpis_import_pattern,1,1)),dpis_import_pattern)
   IF ((dmr_mig_data->cur_db_type != "TARGET"))
    IF ( NOT (dmr_prompt_connect_data("TARGET","V500","CO")))
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->eproc = "Querying for all schema users"
   SELECT INTO "nl:"
    FROM dba_users d
    HEAD REPORT
     dmem_schema_ids->schema_cnt = 0
    DETAIL
     dmem_schema_ids->schema_cnt = (dmem_schema_ids->schema_cnt+ 1), stat = alterlist(dmem_schema_ids
      ->qual,dmem_schema_ids->schema_cnt), dmem_schema_ids->qual[dmem_schema_ids->schema_cnt].
     owner_name = d.username,
     dmem_schema_ids->qual[dmem_schema_ids->schema_cnt].schema_id = d.user_id
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   WHILE (true)
     SET message = nowindow
     CALL clear(1,1)
     SET dm_err->eproc = "Querying for resumable sessions from gv$resumable."
     SELECT INTO "nl:"
      cnt = count(*)
      FROM gv$resumable gvr
      WHERE gvr.status="SUSPENDED"
      DETAIL
       dpis_resumable_cnt = cnt
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc))
      CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
      RETURN(0)
     ENDIF
     SET dpis_import_cntr = 0
     SET dpis_imports_total = - (1)
     SET dpis_imports_completed = - (1)
     SET dpis_imports_remaining = - (1)
     SET dpis_imports_failed = - (1)
     SET dpis_imports_running = - (1)
     IF (findfile(build(dmem_file_loc,"overall_status.log")))
      SET dm_err->eproc = "Reading import status data from file."
      FREE DEFINE rtl
      SET logical dpis_file_loc value(build(dmem_file_loc,"overall_status.log"))
      DEFINE rtl "dpis_file_loc"
      SELECT INTO "nl:"
       name = substring(1,50,substring(1,(findstring("=",r.line) - 1),r.line)), val = substring(1,50,
        substring((findstring("=",r.line)+ 1),size(r.line),r.line))
       FROM rtlt r
       DETAIL
        CASE (name)
         OF build("total_",dpis_local_pattern):
          dpis_imports_total = cnvtint(val),dpis_import_cntr = (dpis_import_cntr+ 1)
         OF build("unexecuted_",dpis_local_pattern):
          dpis_imports_remaining = cnvtint(val),dpis_import_cntr = (dpis_import_cntr+ 1)
         OF build("success_",dpis_local_pattern):
          dpis_imports_completed = cnvtint(val),dpis_import_cntr = (dpis_import_cntr+ 1)
         OF build("failed_",dpis_local_pattern):
          dpis_imports_failed = cnvtint(val),dpis_import_cntr = (dpis_import_cntr+ 1)
         OF build("running_",dpis_local_pattern):
          dpis_imports_running = cnvtint(val),dpis_import_cntr = (dpis_import_cntr+ 1)
        ENDCASE
       WITH nocounter
      ;end select
      IF (check_error(dm_err->eproc))
       ...
       RETURN(0)
      ENDIF
      SET dmem_failed_ei_xfer->failed_cnt = 0
      SET stat = alterlist(dmem_failed_ei_xfer->qual,dmem_failed_ei_xfer->failed_cnt)
      IF (dpis_import_cntr=dpis_import_cntr_total)
       IF (findfile(build(dmem_file_loc,"mig_error.log"))
        AND dpis_imports_failed > 0)
        SET dpis_head_foot_cnt = 0
        WHILE (dpis_head_foot_cnt < 2)
          SET dpis_head_foot_cnt = 0
          SET dm_err->eproc = "Reading error status data from file."
          SET dmem_failed_ei_xfer->failed_cnt = 0
          SET stat = alterlist(dmem_failed_ei_xfer->qual,dmem_failed_ei_xfer->failed_cnt)
          FREE DEFINE rtl
          SET logical dpis_file_loc value(build(dmem_file_loc,"mig_error.log"))
          DEFINE rtl "dpis_file_loc"
          SELECT INTO "nl:"
           FROM rtlt r
           DETAIL
            IF (trim(r.line) IN ("MIG_EXPIMP_HEADER", "MIG_EXPIMP_FOOTER"))
             dpis_head_foot_cnt = (dpis_head_foot_cnt+ 1)
            ELSEIF (trim(r.line) > "")
             dmem_failed_ei_xfer->failed_cnt = (dmem_failed_ei_xfer->failed_cnt+ 1)
             IF (mod(dmem_failed_ei_xfer->failed_cnt,50)=1)
              stat = alterlist(dmem_failed_ei_xfer->qual,(dmem_failed_ei_xfer->failed_cnt+ 49))
             ENDIF
             IF (dpis_local_pattern="ucpts")
              dmem_failed_ei_xfer->qual[dmem_failed_ei_xfer->failed_cnt].owner = trim(r.line),
              dmem_failed_ei_xfer->qual[dmem_failed_ei_xfer->failed_cnt].ksh_file_name = evaluate(
               dm2_sys_misc->cur_os,"AXP",concat("ggdelivery:[dirtmp.exec]dm2_",trim(cnvtlower(
                  dmem_failed_ei_xfer->qual[dmem_failed_ei_xfer->failed_cnt].owner)),"_ei_ucpts.ksh"),
               concat("/ggdelivery/dirtmp/exec/dm2_",trim(cnvtlower(dmem_failed_ei_xfer->qual[
                  dmem_failed_ei_xfer->failed_cnt].owner)),"_ei_ucpts.ksh")), dmem_failed_ei_xfer->
              qual[dmem_failed_ei_xfer->failed_cnt].log_file_name = evaluate(dm2_sys_misc->cur_os,
               "AXP",concat("ggdelivery:[dirtmp.log]dm2_",trim(cnvtlower(dmem_failed_ei_xfer->qual[
                  dmem_failed_ei_xfer->failed_cnt].owner)),"_ei_",dpis_local_pattern,"*.log"),concat(
                "/ggdelivery/dirtmp/log/dm2_",trim(cnvtlower(dmem_failed_ei_xfer->qual[
                  dmem_failed_ei_xfer->failed_cnt].owner)),"_ei_",dpis_local_pattern,"*.log"))
             ELSE
              dmem_failed_ei_xfer->qual[dmem_failed_ei_xfer->failed_cnt].object_id = cnvtreal(trim(r
                .line))
             ENDIF
            ENDIF
           FOOT REPORT
            stat = alterlist(dmem_failed_ei_xfer->qual,dmem_failed_ei_xfer->failed_cnt)
           WITH nocounter
          ;end select
          IF (check_error(dm_err->eproc))
           CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
           RETURN(0)
          ENDIF
          IF (dpis_head_foot_cnt < 2)
           CALL pause(5)
          ENDIF
        ENDWHILE
        IF ((dmem_failed_ei_xfer->failed_cnt > 0)
         AND dpis_local_pattern != "ucpts")
         SET dm_err->eproc = "Selecting table_names and owners from dba_objects"
         SELECT INTO "nl:"
          FROM dba_objects@ref_data_link do,
           (dummyt d  WITH seq = value(dmem_failed_ei_xfer->failed_cnt))
          PLAN (d
           WHERE d.seq > 0)
           JOIN (do
           WHERE (do.object_id=dmem_failed_ei_xfer->qual[d.seq].object_id))
          DETAIL
           dmem_failed_ei_xfer->qual[d.seq].owner = do.owner, dmem_failed_ei_xfer->qual[d.seq].
           table_name = do.object_name, dmem_failed_ei_xfer->qual[d.seq].ksh_file_name = evaluate(
            dm2_sys_misc->cur_os,"AXP",concat("GGDELIVERY:[DIRTMP.EXEC]dm2_",trim(cnvtlower(
               dmem_failed_ei_xfer->qual[d.seq].owner)),"_ei_",dpis_local_pattern,"_*",
             trim(cnvtstring(dmem_failed_ei_xfer->qual[d.seq].object_id)),".ksh"),concat(
             "/ggdelivery/dirtmp/exec/dm2_",trim(cnvtlower(dmem_failed_ei_xfer->qual[d.seq].owner)),
             "_ei_",dpis_local_pattern,"_*",
             trim(cnvtstring(dmem_failed_ei_xfer->qual[d.seq].object_id)),".ksh")),
           dmem_failed_ei_xfer->qual[d.seq].log_file_name = evaluate(dm2_sys_misc->cur_os,"AXP",
            concat("GGDELIVERY:[DIRTMP.log]dm2_",trim(cnvtlower(dmem_failed_ei_xfer->qual[d.seq].
               owner)),"_ei_",dpis_local_pattern,"_err_",
             trim(cnvtstring(dmem_failed_ei_xfer->qual[d.seq].object_id)),".log"),concat(
             "/ggdelivery/dirtmp/log/dm2_",trim(cnvtlower(dmem_failed_ei_xfer->qual[d.seq].owner)),
             "_ei_",dpis_local_pattern,"_err_",
             trim(cnvtstring(dmem_failed_ei_xfer->qual[d.seq].object_id)),".log"))
          WITH nocounter
         ;end select
         IF (check_error(dm_err->eproc))
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        ...
        ENDIF
        IF ((dm_err->debug_flag > 0))
         CALL echorecord(dmem_failed_ei_xfer)
        ENDIF
        SET dmem_failed_ei->failed_cnt = 0
        SET stat = alterlist(dmem_failed_ei->qual,dmem_failed_ei->failed_cnt)
        FOR (dpis_iter = 1 TO dmem_failed_ei_xfer->failed_cnt)
          IF (dm2_findfile(dmem_failed_ei_xfer->qual[dpis_iter].ksh_file_name) > 0
           AND dm2_findfile(dmem_failed_ei_xfer->qual[dpis_iter].log_file_name) > 0)
           SET dm_err->eproc = concat("Marking ",dmem_failed_ei_xfer->qual[dpis_iter].ksh_file_name,
            " as eligible for reset.")
           CALL disp_msg("",dm_err->logfile,0)
           SET dmem_failed_ei->failed_cnt = (dmem_failed_ei->failed_cnt+ 1)
           SET stat = alterlist(dmem_failed_ei->qual,dmem_failed_ei->failed_cnt)
           SET dmem_failed_ei->qual[dmem_failed_ei->failed_cnt].owner = dmem_failed_ei_xfer->qual[
           dpis_iter].owner
           SET dmem_failed_ei->qual[dmem_failed_ei->failed_cnt].table_name = dmem_failed_ei_xfer->
           qual[dpis_iter].table_name
           SET dmem_failed_ei->qual[dmem_failed_ei->failed_cnt].object_id = dmem_failed_ei_xfer->
           qual[dpis_iter].object_id
           SET dmem_failed_ei->qual[dmem_failed_ei->failed_cnt].log_file_name = dmem_failed_ei_xfer->
           qual[dpis_iter].log_file_name
           SET dmem_failed_ei->qual[dmem_failed_ei->failed_cnt].ksh_file_name = dmem_failed_ei_xfer->
           qual[dpis_iter].ksh_file_name
          ELSE
           CALL echo(concat(dmem_failed_ei_xfer->qual[dpis_iter].log_file_name,
             " not found. Bypassing reset."))
          ENDIF
        ENDFOR
       ENDIF
       IF ((dm_err->debug_flag > 0))
        CALL echorecord(dmem_failed_ei)
       ENDIF
      ENDIF
     ENDIF
     SET dpis_top_while = 0
     IF (dpis_imports_failed=0
      AND dpis_imports_remaining=0
      AND dpis_imports_running=0
      AND dpis_imports_completed=dpis_imports_total
      AND dpis_imports_total > 0
      AND dpis_local_pattern="ut")
      SET dmem_max_imports = 0
      IF ( NOT (dmem_update_max_imports(null)))
       RETURN(0)
      ENDIF
      SET dmem_import_pattern = "dm2_.*_ei_ud"
      SET dpis_import_pattern = dmem_import_pattern
      SET dpis_local_pattern = "ud"
      SET dpis_top_while = 1
      IF ( NOT (dmem_generate_config_file(null)))
       RETURN(0)
      ENDIF
      IF ((dm_err->debug_flag != 722))
      ...
      SET width = 132
      CALL clear(1,1)
      CALL text(5,1,"Waiting two minutes for crontab job to sync up (switching modes to DATA)...")
      CALL pause(65)
     ELSEIF (dpis_imports_failed=0
      AND dpis_imports_remaining=0
      AND dpis_imports_running=0
      AND dpis_imports_completed=dpis_imports_total
      AND dpis_imports_total > 0
      AND dpis_local_pattern="ud"
      AND mode="UPTIME"
      AND process="DATA")
      IF (dmem_manage_checkpoints("DM2_MIGRATION","DM2_MIG_UT_EXPIMP_DATA")=0)
       RETURN(0)
      ENDIF
      SET dmem_import_pattern = "dm2_.*_ei_ui"
      SET dpis_import_pattern = dmem_import_pattern
      SET dpis_local_pattern = "ui"
      SET dpis_top_while = 1
      SET dmem_max_imports = 0
      IF ( NOT (dmem_update_max_imports(null)))
       RETURN(0)
      ...
      SET width = 132
      CALL clear(1,1)
      CALL text(5,1,"Waiting two minutes for crontab job to sync up (switching modes to SCHEMA)...")
      CALL pause(65)
      RETURN(1)
     ELSEIF (dpis_imports_failed=0
      AND dpis_imports_remaining=0
      ...
      AND dpis_imports_completed=dpis_imports_total
      AND dpis_imports_total > 0
      AND dpis_local_pattern="ui")
      SET dmem_import_pattern = "dm2_.*_ei_ucpts"
      SET dpis_import_pattern = dmem_import_pattern
      SET dpis_local_pattern = "ucpts"
      SET dpis_top_while = 1
      IF ( NOT (dmem_generate_config_file(null)))
       RETURN(0)
      ENDIF
      ...
      CALL clear(1,1)
      CALL text(5,1,
       "Waiting two minutes for crontab job to sync up (switching modes to non-mill PTS)...")
      CALL pause(65)
     ELSEIF (dpis_imports_failed=0
      AND dpis_imports_remaining=0
      AND dpis_imports_running=0
      ...
      AND dpis_imports_total > 0
      AND dpis_local_pattern="ucpts")
      IF (dmem_manage_checkpoints("DM2_MIGRATION","DM2_MIG_UT_EXPIMP_SCHEMA")=0)
       RETURN(0)
      ENDIF
     ELSEIF (dpis_imports_failed=0
      AND dpis_imports_remaining=0
      ...
      AND dpis_imports_completed=dpis_imports_total
      AND dpis_imports_total > 0
      AND dpis_local_pattern="dd")
      IF (dmem_manage_checkpoints("DM2_MIGRATION","DM2_MIG_DT_EXPIMP_DATA")=0)
       RETURN(0)
      ENDIF
     ENDIF
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("dpis_mode                 ::",dpis_mode))
      CALL echo(build("dpis_process              ::",dpis_process))
      CALL echo(build("dpis_import_pattern       ::",dpis_import_pattern))
      CALL echo(build("dpis_imports_total        ::",dpis_imports_total))
      CALL echo(build("dpis_imports_completed    ::",dpis_imports_completed))
      CALL echo(build("dpis_imports_remaining    ::",dpis_imports_remaining))
      CALL echo(build("dpis_imports_running      ::",dpis_imports_running))
      CALL echo(build("dpis_imports_failed       ::",dpis_imports_failed))
      CALL echo(build("dmem_max_imports          ::",dmem_max_imports))
      CALL echo(build("dpis_resumable_cnt        ::",dpis_resumable_cnt))
      CALL echo(build("dpis_import_cntr_total    ::",dpis_import_cntr_total))
      CALL echo(build("dpis_import_cntr          ::",dpis_import_cntr))
      CALL echo(build("dmem_failed_ei->failed_cnt::",dmem_failed_ei->failed_cnt))
     ENDIF
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat("MODE,",mode))
      CALL echo(concat("PROCESS,",process))
      CALL echo(concat("DPIS_IMPORT_PATTERN,",dpis_import_pattern))
      CALL echo(concat("DPIS_LOCAL_PATTERN,",dpis_local_pattern))
     ENDIF
     IF (dmem_using_dbx=0)
      IF (((mode="UPTIME"
       AND process="DATA"
       AND dpis_import_pattern="dm2_.*_ei_ut"
       AND dpis_local_pattern="ut") OR (((mode="UPTIME"
       AND process="DATA"
       AND dpis_import_pattern="dm2_.*_ei_ud"
       AND dpis_local_pattern="ud") OR (((mode="UPTIME"
       AND process="SCHEMA"
       AND dpis_import_pattern="dm2_.*_ei_ui"
       AND dpis_local_pattern="ui") OR (((mode="UPTIME"
       AND process="SCHEMA"
       AND dpis_import_pattern="dm2_.*_ei_ucpts"
       AND dpis_local_pattern="ucpts") OR (mode="DOWNTIME"
       AND dpis_import_pattern="dm2_.*_ei_dd"
       AND dpis_local_pattern="dd")) )) )) )) )
       IF ((dm_err->debug_flag > 0))
        CALL echo("Validation Successful")
       ENDIF
      ELSE
       SET dm_err->eproc = "Validate import pattern and mode of execution"
       SET dm_err->emsg = "Invalid combination of execution parameters"
       SET dm_err->err_ind = 1
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ENDIF
     ...
     IF (dpis_top_while=0)
      ...
      CALL text(2,2,"Export/Import Utility ")
      CALL text(2,115,concat("Mode:",dmr_expimp->mode))
      CALL text(4,2,concat("Pattern Description: ",dpis_import_pattern))
      CALL text(6,2,"** Import Status **")
      CALL text(7,2,"Total Imports:")
      CALL text(7,25,build(evaluate(cnvtstring(dpis_imports_total),"-1","?",cnvtstring(
          dpis_imports_total))))
      CALL text(8,2,"Imports Completed:")
      CALL text(8,25,build(evaluate(cnvtstring(dpis_imports_completed),"-1","?",cnvtstring(
          dpis_imports_completed))))
      CALL text(9,2,"Imports Remaining:")
      CALL text(9,25,build(evaluate(cnvtstring(dpis_imports_remaining),"-1","?",cnvtstring(
          dpis_imports_remaining))))
      CALL text(10,2,"Imports Running:")
      CALL text(10,25,build(evaluate(cnvtstring(dpis_imports_running),"-1","?",cnvtstring(
          dpis_imports_running))))
      IF (dpis_imports_failed > 0)
       CALL video(b)
      ENDIF
      CALL text(11,2,"Imports Failed:")
      CALL text(11,25,build(evaluate(cnvtstring(dpis_imports_failed),"-1","?",cnvtstring(
          dpis_imports_failed))))
      CALL video(n)
      CALL text(12,2,"Imports Suspended:")
      CALL text(12,25,build(dpis_resumable_cnt))
      CALL text(14,2,concat("Max number of parallel imports to execute: ",build(dmem_max_imports)))
      IF (dpis_import_cntr != dpis_import_cntr_total)
       SET accept = time(30)
       CALL text(16,2,"(R)efresh Screen, (Q)uit:")
       CALL text(18,2,"WARNING!  Not all import statuses retrieved (?), waiting for refreshed data!")
       CALL accept(16,27,"A;CU","R"
        WHERE curaccept IN ("R", "Q", "r", "q"))
       SET accept = notime
      ELSEIF (dpis_imports_failed > 0
       AND (dmem_failed_ei->failed_cnt > 0))
       CALL text(16,2,"Reset (F)ailed Imports, (R)efresh Screen, (M)odify, (Q)uit:")
       CALL text(18,2,"WARNING!  Not all table data was moved successfully!")
       SET accept = time(30)
       CALL accept(16,62,"A;CU","R"
        WHERE curaccept IN ("F", "R", "Q", "f", "r",
        "q", "m", "M"))
       SET accept = notime
      ELSEIF (((dpis_imports_remaining > 0) OR (dpis_imports_running > 0)) )
       SET accept = time(30)
       IF (dpis_process="DATA")
        CALL text(16,2,"(V)iew progress, (M)odify, (R)efresh Screen, (Q)uit:")
        CALL accept(16,54,"A;CU","R"
         WHERE curaccept IN ("M", "R", "Q", "V", "m",
         "r", "q", "v"))
       ELSE
        CALL text(16,2,"(M)odify, (R)efresh Screen, (Q)uit:")
        CALL accept(16,37,"A;CU","R"
         WHERE curaccept IN ("M", "R", "Q", "m", "r",
         "q"))
       ENDIF
       SET accept = notime
      ELSEIF (dpis_imports_completed != dpis_imports_total)
       SET message = nowindow
       SET dm_err->err_ind = 1
       SET dm_err->eproc = "Check that all imports completed successfully."
       SET dm_err->emsg = "Total import count does not match successful import count."
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ELSE
       CALL text(16,2,"(M)odify, (C)ontinue, (Q)uit: ")
       CALL accept(16,32,"A;CU"
        WHERE curaccept IN ("M", "C", "Q", "m", "c",
        "q"))
      ENDIF
      CASE (cnvtupper(curaccept))
       OF "F":
        IF (dpis_imports_failed > 0
         AND (dmem_failed_ei->failed_cnt > 0))
         SET dpis_cur_accept = "R"
         WHILE ( NOT (dpis_cur_accept IN ("C", "Q")))
           IF ((dm_err->debug_flag != 722))
            SET message = window
           ENDIF
           SET width = 132
           CALL clear(1,1)
           CALL box(1,1,24,131)
           CALL text(2,2,"Database Migration Process (Bulk Data Move) - Resolve Failed Imports")
           CALL text(4,2,concat("The following tables did not move successfully.",
             " Enter 'C' to continue and reset the EXP/IMP ksh scripts for these tables."))
           CALL text(7,2,
            "PreReq Step: Clean up any orphaned Oracle and/or o/s processes spawned from the ")
           CALL text(8,2,"bulk data move kshs (see migration doc for more details).")
           CALL text(10,2,"Owner")
           IF (dpis_local_pattern != "ucpts")
            CALL text(10,23,"Failed Table")
           ENDIF
           FOR (dpis_iter = (((dpis_page_ptr - 1) * 11)+ 1) TO least(dmem_failed_ei->failed_cnt,(((
            dpis_page_ptr - 1) * 11)+ 12)))
            CALL text((10+ (dpis_iter - ((dpis_page_ptr - 1) * 11))),2,dmem_failed_ei->qual[dpis_iter
             ].owner)
            IF (dpis_local_pattern != "ucpts")
             CALL text((10+ (dpis_iter - ((dpis_page_ptr - 1) * 11))),23,dmem_failed_ei->qual[
              dpis_iter].table_name)
            ENDIF
           ENDFOR
           IF ((dmem_failed_ei->failed_cnt > 11))
            CALL text(22,2,"More information available.  Use PgUp/PgDown to scroll.")
           ENDIF
           CALL text(23,2,"(C)ontinue and Reset, (Q)uit):_")
           CALL accept(23,32,"p;cus"," "
            WHERE curaccept IN ("Q", "C"))
           CASE (curscroll)
            OF 0:
             SET dpis_cur_accept = curaccept
             CASE (curaccept)
              OF "C":
               SET message = nowindow
               FOR (dpis_iter = 1 TO dmem_failed_ei->failed_cnt)
                 IF (dpis_local_pattern="ucpts")
                  SET dm_err->eproc = concat("Resetting ",dmem_failed_ei->qual[dpis_iter].owner,":",
                   build(dpis_iter)," of ",
                   build(dmem_failed_ei->failed_cnt))
                 ELSE
                  SET dm_err->eproc = concat("Resetting ",dmem_failed_ei->qual[dpis_iter].owner,".",
                   dmem_failed_ei->qual[dpis_iter].table_name,":",
                   build(dpis_iter)," of ",build(dmem_failed_ei->failed_cnt))
                 ENDIF
                 CALL disp_msg("",dm_err->logfile,0)
                 IF (dpis_local_pattern="ucpts")
                  SET dpis_cmd = evaluate(dm2_sys_misc->cur_os,"AXP",concat(
                    "rename GGDELIVERY:[DIRTMP.EXEC]dm2_",dmem_failed_ei->qual[dpis_iter].owner,
                    "_ei_ucpts.ksh ",dmem_file_loc),concat("mv /ggdelivery/dirtmp/exec/dm2_",
                    dmem_failed_ei->qual[dpis_iter].owner,"_ei_ucpts.ksh ",dmem_file_loc))
                 ELSE
                  SET dpis_cmd = concat(evaluate(dm2_sys_misc->cur_os,"AXP",
                    "rename GGDELIVERY:[DIRTMP.EXEC]*_%","mv /ggdelivery/dirtmp/exec/*_?"),trim(
                    cnvtstring(dmem_failed_ei->qual[dpis_iter].object_id)),".ksh ",dmem_file_loc)
                 ENDIF
                 SET dm_err->disp_dcl_err_ind = 0
                 IF (dm2_push_dcl(dpis_cmd) != 1)
                  IF ((dm_err->err_ind=1))
                   RETURN(0)
                  ELSEIF ((dm_err->emsg IN ("*RMS-E-FNF*", "*0653-401*")))
                 ...
                 IF (dpis_local_pattern="ucpts")
                  SET dpis_cmd = evaluate(dm2_sys_misc->cur_os,"AXP",concat(
                    "delete ggdelivery:[dirtmp.log]dm2_",dmem_failed_ei->qual[dpis_iter].owner,"_ei_",
                    dpis_local_pattern,".log"),concat("rm -f /ggdelivery/dirtmp/log/dm2_",
                    dmem_failed_ei->qual[dpis_iter].owner,"_ei_",dpis_local_pattern,".log"))
                 ELSE
                  SET dpis_cmd = concat(evaluate(dm2_sys_misc->cur_os,"AXP",
                    "delete ggdelivery:[dirtmp.log]","rm -f /ggdelivery/dirtmp/log/"),"dm2_*_ei_",
                   dpis_local_pattern,"_*",trim(cnvtstring(dmem_failed_ei->qual[dpis_iter].object_id)
                    ),
                   ".log")
                 ENDIF
                 IF ((dm2_sys_misc->cur_os="AXP"))
                  SET dpis_cmd = concat(dpis_cmd,";*")
                 ENDIF
                 SET dm_err->disp_dcl_err_ind = 0
                 IF (dm2_push_dcl(dpis_cmd) != 1)
                  IF ((dm_err->err_ind=1))
                   RETURN(0)
                  ELSEIF ((dm_err->emsg="*RMS-E-FNF*"))
                   CALL echo("The above error is ignorable.")
                  ELSE
                   SET dm_err->err_ind = 1
                   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
               ...
               SET width = 132
               CALL clear(1,1)
               CALL text(5,1,"Waiting a minute for crontab job to sync up import statuses...")
               WHILE (datetimeadd(dpis_start_time,(1.0/ 1440.0)) > cnvtdatetime(curdate,curtime3))
                 CALL pause(1)
               ENDWHILE
               CALL pause(60)
             ENDCASE
            OF 5:
             SET dpis_page_ptr = greatest((dpis_page_ptr - 1),1)
            OF 6:
             SET dpis_page_ptr = least((dpis_page_ptr+ 1),ceil((cnvtreal(dmem_failed_ei->failed_cnt)
               / 11)))
           ENDCASE
         ENDWHILE
        ENDIF
       OF "Q":
        RETURN(0)
       OF "M":
        CALL accept(14,45,"99")
        IF (dmem_max_imports != curaccept)
         SET dmem_max_imports = curaccept
         IF ( NOT (dmem_update_max_imports(null)))
          RETURN(0)
         ENDIF
         IF ( NOT (dmem_generate_config_file(null)))
       ...
       OF "C":
        RETURN(1)
       OF "V":
        WHILE (cnvtupper(curaccept) != "G")
          SET message = nowindow
          SET dm_err->eproc = "Querying for currently running imports from v$sql"
          SELECT INTO "nl:"
           FROM v$sql s
           WHERE s.sql_text='INSERT *INTO "*'
            AND command_type=2
            AND open_versions > 0
           HEAD REPORT
            dmem_imp_progress->imp_cnt = 0
           DETAIL
            dpis_owner_beg_ptr = (findstring('INTO "',s.sql_text)+ 6), dpis_owner_end_ptr = (
            findstring(".",substring(dpis_owner_beg_ptr,30,s.sql_text)) - 1), dpis_table_end_ptr = (
            findstring('"',substring(((dpis_owner_beg_ptr+ dpis_owner_end_ptr)+ 2),35,s.sql_text)) -
            1),
            dmem_imp_progress->imp_cnt = (dmem_imp_progress->imp_cnt+ 1), stat = alterlist(
             dmem_imp_progress->list,dmem_imp_progress->imp_cnt), dmem_imp_progress->list[
            dmem_imp_progress->imp_cnt].owner = dmem_schema_ids->qual[locateval(dpis_num,1,
             dmem_schema_ids->schema_cnt,s.parsing_schema_id,dmem_schema_ids->qual[dpis_num].
             schema_id)].owner_name,
            dmem_imp_progress->list[dmem_imp_progress->imp_cnt].table_name = evaluate(
             dpis_owner_end_ptr,- (1),substring(dpis_owner_beg_ptr,(dpis_table_end_ptr+ 1),s.sql_text
              ),substring(((dpis_owner_beg_ptr+ dpis_owner_end_ptr)+ 2),dpis_table_end_ptr,s.sql_text
              )), dmem_imp_progress->list[dmem_imp_progress->imp_cnt].processed_rows = s
            .rows_processed
           WITH nocounter
          ;end select
          IF (check_error(dm_err->eproc))
          ...
          ENDIF
          IF ((dm_err->debug_flag > 1))
           CALL echorecord(dmem_imp_progress)
          ENDIF
          IF ((dmr_expimp->stg_v500_cnct_str != " "))
           SET dm2_install_schema->dbase_name = '"STAGING DB"'
           SET dm2_install_schema->u_name = "V500"
           SET dm2_install_schema->p_word = dmr_expimp->stg_v500_p_word
           SET dm2_install_schema->connect_str = dmr_expimp->stg_v500_cnct_str
           EXECUTE dm2_connect_to_dbase "CO"
           IF (dm_err->err_ind)
            RETURN(0)
           ENDIF
          ENDIF
          IF ((dmem_imp_progress->imp_cnt > 0))
           SET dm_err->eproc =
           "Querying for row counts from dm_dba_tables_actual_stats@ref_data_link"
           SELECT
            IF ((dmr_expimp->stg_v500_cnct_str > " "))
             FROM dm_dba_tables_actual_stats dt,
              (dummyt d  WITH seq = value(dmem_imp_progress->imp_cnt))
             PLAN (d
              WHERE d.seq > 0)
              JOIN (dt
              WHERE (dt.table_name=dmem_imp_progress->list[d.seq].table_name)
               AND (dt.owner=dmem_imp_progress->list[d.seq].owner))
            ELSE
             FROM dm_dba_tables_actual_stats@ref_data_link dt,
              (dummyt d  WITH seq = value(dmem_imp_progress->imp_cnt))
             PLAN (d
              WHERE d.seq > 0)
              JOIN (dt
            ...
            INTO "nl:"
            DETAIL
             dmem_imp_progress->list[d.seq].total_rows = dt.num_rows
            WITH nocounter
           ;end select
           IF (check_error(dm_err->eproc))
            CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          ...
          CALL clear(1,1)
          CALL box(1,1,24,131)
          CALL text(2,2,"Database Migration Process (Bulk Data Move) - View Progress ")
          CALL text(4,2,build("Max number of parallel imports to execute: ",dmem_max_imports))
          CALL text(6,2,"Owner")
          CALL text(6,23,"Table")
          CALL text(6,54,"Total Rows")
          CALL text(6,67,"Rows Processed")
          FOR (dpis_iter = 1 TO least(dmem_imp_progress->imp_cnt,15))
            SET dpis_vert_ptr = (dpis_iter+ 6)
            CALL text(dpis_vert_ptr,2,build(dmem_imp_progress->list[dpis_iter].owner))
            CALL text(dpis_vert_ptr,23,build(dmem_imp_progress->list[dpis_iter].table_name))
            CALL text(dpis_vert_ptr,54,build(format(dmem_imp_progress->list[dpis_iter].total_rows,
               "############;T(1)")))
            CALL text(dpis_vert_ptr,69,build(format(dmem_imp_progress->list[dpis_iter].processed_rows,
               "############;T(1)")))
          ENDFOR
          IF ((dmem_imp_progress->imp_cnt > 15))
           CALL text(22,2,
            "Since more than 15 runners active, could not display all runners; Select (A) for a report of all."
            )
          ELSEIF ((dmem_imp_progress->imp_cnt=0))
           CALL text(22,2,"No table imports are in progress.")
          ENDIF
          CALL text(23,2,evaluate2(
            IF ((dmem_imp_progress->imp_cnt > 15)) "View (A)ll, (R)efresh, (G)o back:"
            ELSE "(R)efresh, (G)o back: "
            ENDIF
            ))
          SET accept = time(30)
          IF ((dmem_imp_progress->imp_cnt > 15))
           CALL accept(23,35,"A;CU","R"
            WHERE curaccept IN ("A", "G", "R", "a", "r",
            "g"))
          ELSE
           CALL accept(23,23,"A;CU","R"
            WHERE curaccept IN ("G", "g", "R", "r"))
          ENDIF
          SET accept = notime
          SET message = nowindow
          CASE (cnvtupper(curaccept))
           OF "A":
            SET dm_err->eproc = "Displaying report"
            SELECT INTO "mine"
             FROM (dummyt d  WITH seq = value(dmem_imp_progress->imp_cnt))
             PLAN (d)
             HEAD REPORT
              col 0, row 0, "Owner",
              col 24, "Table", col 60,
              "Total Rows", col 78, "Rows Processed",
              row + 1,
              CALL print(fillstring(130,"*")), row + 1
             DETAIL
              col 0,
              CALL print(dmem_imp_progress->list[d.seq].owner), col 24,
              CALL print(dmem_imp_progress->list[d.seq].table_name), col 60,
              CALL print(format(dmem_imp_progress->list[d.seq].total_rows,"############;T(1)")),
              col 80,
              CALL print(format(dmem_imp_progress->list[d.seq].processed_rows,"############;T(1)")),
              row + 1
             WITH nocounter, formfeed = none, maxcol = 131,
              format = variable
            ;end select
            IF (check_error(dm_err->eproc))
             CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDWHILE
 END ;Subroutine
 SUBROUTINE dmem_generate_config_file(null)
   SET dm_err->eproc = "Creating dm2ei_config.txt"
   SELECT INTO value(build(dmem_file_loc,"dm2ei_config.txt"))
    FROM dummyt d
    DETAIL
     "maximports=", dmem_max_imports, row + 1,
     "import_pattern=", dmem_import_pattern, row + 1
    WITH nocounter, maxrow = 1, maxcol = 132,
     format = variable, format = lfstream, formfeed = none
   ;end select
   IF (check_error(dm_err->eproc))
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dmem_update_max_imports(null)
   DECLARE dumi_db_type = vc WITH protect, noconstant(dmr_mig_data->cur_db_type)
   IF ((dmr_mig_data->cur_db_type != "SOURCE"))
    IF ( NOT (dmr_prompt_connect_data("SOURCE","V500","CO")))
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->eproc = "Updating max imports row in dm_info."
   UPDATE  FROM dm_info di
    SET di.info_number = dmem_max_imports
    WHERE di.info_domain="DM2_MIG_BULK_DATA_MOVE"
     AND di.info_name="MAXIMPORTS"
    WITH nocounter
   ;end update
   ...
   ENDIF
   COMMIT
   IF (dumi_db_type != "SOURCE")
    IF ( NOT (dmr_prompt_connect_data(dumi_db_type,"V500","CO")))
     RETURN(0)
    ENDIF
   ENDIF
   RETURN(1)
...
#exit_script
 ...
 ENDIF
 IF (dmem_quit_ind)
  SET dm_err->eproc = "Please re-execute dm2_mig_expimp_menu with the necessary changes."
  CALL disp_msg("User chose to quit.",dm_err->logfile,1)
 ENDIF
 IF (dmem_downtime_complete=0)
  SET dm_err->eproc = "dm2_mig_expimp_menu completed successfully."
 ENDIF
 CALL final_disp_msg("dm2_mig_expimp_menu")
END GO
