CREATE PROGRAM djh_get_md_aliases_3:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (pr
   WHERE pr.physician_ind >= 0
    ...
    AND pr.name_last_key="SMITH*"
    AND pr.name_first_key="*"
  ...
  WITH nocounter, time = 60, maxrec = 10
 ;end select
 SELECT INTO "nl:"
  FROM phone ph,
 ...
 SELECT INTO  $1
  ...
  DETAIL
   ...
   "DEA: ", md_list->qual[d.seq].dea_nbr, row + 2,
   col 5, "Aliases: ", col 82,
   "Start DT  End DT", row + 1
