CREATE PROGRAM afc_get_charge_desc_master:dba
 DECLARE afc_get_charge_desc_master = vc WITH constant("CHARGSRV-13768.004")
 IF ( NOT (validate(reply)))
  RECORD reply(
    1 cdmitems[*]
      2 chargedescmasterid = f8
      2 cdmcodetxt = vc
      2 description = vc
      2 servicetypeflag = i2
      2 updtcnt = i4
      2 updtdttm = dq8
    1 pft_status_data
      2 status = c1
 ...
 IF ( NOT (validate(getlogicaldomainsystemuser)))
  ...
  END ;Subroutine
 ENDIF
 SUBROUTINE (processsearchtext(searchtext=vc) =vc)
   DECLARE finalstring = vc WITH protect, noconstant(searchtext)
   SET finalstring = replace(finalstring,"\","\\")
   SET finalstring = replace(finalstring,"*","\*")
   SET finalstring = replace(finalstring,"?","\?")
   SET finalstring = trim(cnvtupper(finalstring))
   IF (validate(debug,- (1)) > 0)
    CALL echo(build2("processSearchText: finalString = ",finalstring))
   ENDIF
   RETURN(finalstring)
 END ;Subroutine
 IF ( NOT (validate(starts_with)))
  DECLARE starts_with = i2 WITH protect, constant(0)
 ENDIF
 IF ( NOT (validate(contains)))
  DECLARE contains = i2 WITH protect, constant(1)
 ENDIF
 IF ( NOT (validate(exact_match)))
  DECLARE exact_match = i2 WITH protect, constant(2)
 ENDIF
 IF ( NOT (validate(description_search)))
  DECLARE description_search = i2 WITH protect, constant(0)
 ENDIF
 IF ( NOT (validate(code_search)))
  DECLARE code_search = i2 WITH protect, constant(1)
 ENDIF
 IF ( NOT (validate(id_search)))
  DECLARE id_search = i2 WITH protect, constant(2)
 ENDIF
 DECLARE flogicaldomainid = f8 WITH protect, noconstant(0.0)
 DECLARE scdmquerysearchstring = vc WITH protect, noconstant("")
 DECLARE sorderbystring = vc WITH protect, noconstant("")
 DECLARE icdmcounter = i4 WITH protect, noconstant(0)
 IF ( NOT (getlogicaldomain(ld_concept_pft_event,flogicaldomainid)))
  CALL exitservicefailure("Invalid logical domain ID",1)
 ENDIF
 IF ((request->searchbyflg=id_search))
  IF (cnvtint(trim(request->searchtext))=0)
   CALL exitservicefailure("Invalid search text for searching by CDM ID",1)
  ENDIF
  SET scdmquerysearchstring = build2("cdm.charge_desc_master_id = ",trim(request->searchtext))
  SET sorderbystring = "cdm.charge_desc_master_id"
 ELSE
  CASE (request->searchbyflg)
   OF description_search:
    SET scdmquerysearchstring = build2("CNVTUPPER(cdm.description) =")
   OF code_search:
    SET scdmquerysearchstring = build2("CNVTUPPER(cdm.cdm_code_txt) =")
   ELSE
    CALL exitservicefailure("Invalid search flag parameter",1)
  ENDCASE
  SET sorderbystring = evaluate(request->searchbyflg,1,"cdm.cdm_code_txt","cdm.description")
  SET sorderbystring = concat("CNVTNLSSORT(",sorderbystring,")")
  CASE (request->searchmatching)
   OF starts_with:
    SET scdmquerysearchstring = concat(scdmquerysearchstring,build2(" '",processsearchtext(request->
       searchtext),"*'"))
   OF contains:
    SET scdmquerysearchstring = concat(scdmquerysearchstring,build2(" '*",processsearchtext(request->
       searchtext),"*'"))
   OF exact_match:
    SET scdmquerysearchstring = concat(scdmquerysearchstring,build2(" '",processsearchtext(request->
       searchtext),"'"))
   ELSE
    CALL exitservicefailure("Invalid search matching parameter",1)
  ENDCASE
 ENDIF
 SELECT INTO "nl:"
  FROM charge_desc_master cdm
  WHERE parser(scdmquerysearchstring)
   AND cdm.logical_domain_id=flogicaldomainid
   AND cdm.active_ind=1
  ORDER BY parser(sorderbystring)
  DETAIL
   icdmcounter += 1, stat = alterlist(reply->cdmitems,icdmcounter), reply->cdmitems[icdmcounter].
   chargedescmasterid = cdm.charge_desc_master_id,
   reply->cdmitems[icdmcounter].cdmcodetxt = cdm.cdm_code_txt, reply->cdmitems[icdmcounter].
   description = cdm.description, reply->cdmitems[icdmcounter].servicetypeflag = cdm
   .service_type_flag,
   reply->cdmitems[icdmcounter].updtcnt = cdm.updt_cnt, reply->cdmitems[icdmcounter].updtdttm = cdm
   .updt_dt_tm
  WITH nocounter
 ;end select
 CALL exitservicesuccess("Success for afc_get_charge_desc_master...")
#exit_script
#end_program
END GO
