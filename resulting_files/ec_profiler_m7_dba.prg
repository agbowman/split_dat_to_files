CREATE PROGRAM ec_profiler_m7:dba
 ...
 SELECT INTO "nl"
  ...
  PLAN (cv
   ...
   WHERE te.tracking_id=ti.tracking_id
    AND te.complete_dt_tm BETWEEN cnvtdatetime(request->start_dt_tm) AND cnvtdatetime(request->
    stop_dt_tm)
    AND te.complete_dt_tm BETWEEN tc.checkin_dt_tm AND tc.checkout_dt_tm
    AND te.complete_id > 0)
   JOIN (t
   WHERE t.track_event_id=te.track_event_id)
   JOIN (pl
   WHERE pl.person_id=te.complete_id)
  ORDER BY lg2.parent_loc_cd, pl.position_cd, t.description
  HEAD REPORT
   facilitycnt = 0
  HEAD lg2.parent_loc_cd
   ...
   reply->facilities[facilitycnt].facility_cd = lg2.parent_loc_cd
  HEAD pl.position_cd
   ...
   reply->facilities[facilitycnt].positions[positioncnt].position_cd = pl.position_cd, reply->
  ...
  HEAD t.description
   ...
   facilities[facilitycnt].positions[positioncnt].details[detailcnt].detail_value_txt = t.description
  WITH nocounter
 ;end select
