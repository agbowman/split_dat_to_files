CREATE PROGRAM cps_chk_muldru_imp_concept:dba
 ...
 IF (curqual < 1)
  FREE DEFINE rtl2
  DEFINE rtl2 "CPS_IMP__CONCEPT.LOG"
  SELECT INTO "nl:"
   err_msg = r.line
   FROM rtl2t r
   HEAD REPORT
  ...
  IF (curqual < 1)
   ...
   SET request->setup_proc[1].error_msg = concat("FAILURE on ",vocab," Concept Import ",format(
     cnvtdatetime(curdate,curtime3),"dd-mmm-yyyy hh:mm;;q"),"  : Could not find CCLUSERDIR:CPS_IMP_",
    vocab,"_CONCEPT.LOG  look for ","CPS*CONCEPT.LOG files for more information")
   GO TO exit_script
  ELSE
   SET log_file_name = "CPS_IMP__CONCEPT.LOG"
  ENDIF
 ELSE
  SET log_file_name = concat("CPS_IMP_",vocab,"_CONCEPT.LOG")
 ENDIF
 WHILE (knt >= 1
  AND continue=true)
   IF (substring(31,7,err_log->qual[knt].msg)="SUCCESS")
    ...
    IF (datetimediff(cnvtdatetime(beg_dt_tm),cnvtdatetime(substring(39,20,err_log->qual[knt].msg)))
     ...
     SET request->setup_proc[1].error_msg = concat("SUCCESS on ",vocab," Concept Import ",format(
   ...
   ELSEIF (substring(31,7,err_log->qual[knt].msg)="WARNING")
    ...
    IF (datetimediff(cnvtdatetime(beg_dt_tm),cnvtdatetime(substring(39,20,err_log->qual[knt].msg)))
     < time_dif)
     SET request->setup_proc[1].success_ind = 1
     SET request->setup_proc[1].error_msg = concat("WARNING on ",vocab," Concept Import ",format(
       cnvtdatetime(curdate,curtime3),"dd-mmm-yyyy hh:mm;;q"),"  :See log file CCLUSERDIR:",
