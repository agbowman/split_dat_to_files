CREATE PROGRAM acm_write_phone:dba
 ...
 DECLARE ubound = i4 WITH protect, noconstant(reply->phone_qual_cnt)
 SET stat = alterlist(reply->phone_qual,reply->phone_qual_cnt)
 ...
 FOR (index = lbound TO ubound)
   CASE (acm_request->phone_qual[index].action_flag)
    OF add_action:
    ...
    ELSE
     SET reply->phone_qual[index].phone_id = acm_request->phone_qual[index].phone_id
     SET reply->phone_qual[index].status = 1
     SET reply->status_data.status = "S"
   ENDCASE
 ENDFOR
 IF ((xref->del_cnt > 0))
  EXECUTE acm_del_phone
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->act_cnt > 0))
  EXECUTE acm_act_phone
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->chg_cnt > 0))
  EXECUTE acm_chg_phone
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((xref->add_cnt > 0))
  EXECUTE acm_add_phone
  IF ((reply->status_data.status="F"))
   GO TO exit_script
  ENDIF
 ENDIF
