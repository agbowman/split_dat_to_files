CREATE PROGRAM cp_upd_pathway:dba
 PROMPT
  "Output to File/Printer/MINE:" = "MINE",
  "Pathway Ids:" = 0.0,
  "Request JSON:" = ""
  WITH outdev, inact_pw_id, request_json
 RECORD pathwaynodes(
   1 node_list[*]
     2 node_id = f8
 )
 RECORD pathway_updt_reply(
   1 pathway_id = f8
   1 pathway_name = vc
   1 pathway_type = vc
   1 pathway_status = f8
 ...
 IF ( NOT (validate(updatepathwayrec)))
  RECORD updatepathwayrec(
    1 newpathwayname = vc
    1 pathwaytypecd = f8
    1 pathwaydiagramurl = vc
    1 newtreatmentlinename = vc
    1 treatmentlineid = f8
  )
 ENDIF
 ...
 DECLARE checkpathwayid(null) = i4 WITH protect
 DECLARE checkduplicate(null) = null WITH protect
 DECLARE checkduplicatetl(null) = null WITH protect
 DECLARE inactivatenodes(null) = null WITH protect
 DECLARE inactivatepathway(null) = null WITH protect
 DECLARE updatepathwaytypecd(null) = null WITH protect
 DECLARE setpathwaydiagram(null) = null WITH protect
 DECLARE renamepathway(null) = null WITH protect
 DECLARE renametreatmentline(null) = null WITH protect
 DECLARE updatebedrock(null) = null WITH protect
 DECLARE updatebedrocktl(null) = null WITH protect
 DECLARE lstat = i4 WITH protect, noconstant(0)
 DECLARE dpathwayid = f8 WITH protect, constant( $INACT_PW_ID)
 DECLARE dinactivecd = f8 WITH protect, constant(uar_get_code_by("meaning",4003198,"inactive"))
 DECLARE currentdatetime2 = dq8 WITH constant(curtime3), private
 DECLARE spathwayname = vc WITH noconstant
 DECLARE streatmentlinename = vc WITH noconstant
 DECLARE scategorymean = vc WITH noconstant, protect
 ...
 SUBROUTINE main(null)
   ...
   IF (( $REQUEST_JSON != ""))
    SET stat = cnvtjsontorec( $REQUEST_JSON)
   ENDIF
   SET pathway_updt_reply->status_data.status = "F"
   SET lstat = checkpathwayid(null)
   IF (( $REQUEST_JSON != ""))
    CALL echorecord(updatepathwayrec)
    IF ((updatepathwayrec->newpathwayname != ""))
     CALL renamepathway(null)
    ELSEIF ((updatepathwayrec->newtreatmentlinename != "")
     AND (updatepathwayrec->treatmentlineid > 0.0))
     CALL renametreatmentline(null)
    ELSE
     CALL setpathwaydiagram(null)
    ENDIF
   ELSE
    CALL inactivatenodes(null)
    CALL inactivatepathway(null)
   ENDIF
   SET pathway_updt_reply->status_data.status = "S"
   CALL log_message(build("exit main(), elapsed time in seconds:",((curtime3 - begin_curtime3)/ 100.0
     )),log_level_debug)
   RETURN(curqual)
 END ;Subroutine
 SUBROUTINE checkpathwayid(null)
   CALL log_message("in checkPathwayId()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE timestampsuffix = vc WITH noconstant(" "), private
   SELECT INTO "nl:"
    FROM cp_pathway cp
    WHERE cp.cp_pathway_id=dpathwayid
    DETAIL
     pathway_updt_reply->pathway_id = cp.cp_pathway_id, pathway_updt_reply->pathway_name = trim(cp
      .pathway_name), pathway_updt_reply->pathway_status = cp.pathway_status_cd,
     pathway_updt_reply->pathway_type = uar_get_code_display(cp.pathway_type_cd)
    WITH nocounter
   ;end select
   SET timestampsuffix = concat("_inactivated_",format(curdate,"mm_dd_yyyy;;d"),"_",format(curtime3,
     "hhmm;3;m"))
   SET spathwayname = concat(trim(substring(1,(100 - size(timestampsuffix)),pathway_updt_reply->
      pathway_name)),timestampsuffix)
   IF (validate(debug_ind,0)=1)
    CALL echo(build("inactive status : ",dinactivecd))
    CALL echorecord(pathway_updt_reply)
   ENDIF
   CALL log_message(build("exit checkPathwayId(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(curqual)
 END ;Subroutine
 SUBROUTINE checkduplicate(null)
   CALL log_message("in checkDuplicate()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE duplicatepathway = vc
   SELECT INTO "nl:"
    FROM cp_pathway cp
    WHERE (cp.pathway_name=updatepathwayrec->newpathwayname)
    DETAIL
     duplicatepathway = cp.pathway_name
    WITH nocounter
   ;end select
   IF (size(duplicatepathway) > 0)
    SET pathway_updt_reply->status_data.status = "F"
    SET pathway_updt_reply->status_data.subeventstatus[1].targetobjectname = concat(
     "Duplicate pathway found")
    SET pathway_updt_reply->status_data.subeventstatus[1].targetobjectvalue = serrmsg
    GO TO exit_script
   ENDIF
   CALL log_message(build("exit checkDuplicate(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE checkduplicatetl(null)
   CALL log_message("in checkDuplicateTL()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE duplicatetreatmentline = vc
   SELECT INTO "nl:"
    FROM cp_node cn
    WHERE (cn.node_name=updatepathwayrec->newtreatmentlinename)
     AND cn.cp_pathway_id=dpathwayid
    DETAIL
     duplicatetreatmentline = cn.node_name
    WITH nocounter
   ;end select
   IF (size(duplicatetreatmentline) > 0)
    SET pathway_updt_reply->status_data.status = "F"
    SET pathway_updt_reply->status_data.subeventstatus[1].targetobjectname = concat(
     "Duplicate treatment line found")
    SET pathway_updt_reply->status_data.subeventstatus[1].targetobjectvalue = serrmsg
    GO TO exit_script
   ENDIF
   CALL log_message(build("exit checkDuplicateTL(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE inactivatenodes(null)
   CALL log_message("in inactivateNodes()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE count = i4 WITH protect, noconstant(0)
   DECLARE nodessize = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM cp_node cn
    WHERE cn.cp_pathway_id=dpathwayid
    DETAIL
     count += 1, stat = alterlist(pathwaynodes->node_list,count), pathwaynodes->node_list[count].
     node_id = cn.cp_node_id
    WITH nocounter
   ;end select
   SET nodesize = size(pathwaynodes->node_list,5)
   FOR (i = 1 TO nodesize)
    EXECUTE cp_br_inact_node_json "mine", pathwaynodes->node_list[i].node_id
    IF ((reply->status_data.status="F"))
     CALL populate_subeventstatus_rec("inactivateNodes","F",reply->status_data.targetobjectname,reply
      ->status_data.targetobjectvalue,pathway_updt_reply)
    ENDIF
   ENDFOR
   CALL log_message(build("exit inactivateNodes(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE inactivatepathway(null)
   CALL log_message("in inactivatePathway()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   IF (dpathwayid > 0
    AND lstat > 0)
    UPDATE  FROM cp_pathway cp
     SET cp.updt_applctx = reqinfo->updt_applctx, cp.updt_cnt = (cp.updt_cnt+ 1), cp.updt_dt_tm =
      cnvtdatetime(sysdate),
      cp.updt_id = reqinfo->updt_id, cp.pathway_status_cd = dinactivecd, cp.updt_task = reqinfo->
      updt_task,
      cp.pathway_name = spathwayname, cp.active_ind = 0
     WHERE cp.cp_pathway_id=dpathwayid
     WITH nocounter
    ;end update
   ENDIF
   CALL log_message(build("exit inactivatePathway(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE setpathwaydiagram(null)
   CALL log_message("in setPathwayDiagram()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   IF (dpathwayid > 0
    AND lstat > 0)
    UPDATE  FROM cp_pathway cp
     SET cp.updt_applctx = reqinfo->updt_applctx, cp.updt_cnt = (cp.updt_cnt+ 1), cp.updt_dt_tm =
      cnvtdatetime(sysdate),
      cp.updt_id = reqinfo->updt_id, cp.updt_task = reqinfo->updt_task, cp.pathway_diagram_url =
      updatepathwayrec->pathwaydiagramurl
     WHERE cp.cp_pathway_id=dpathwayid
     WITH nocounter
    ;end update
   ENDIF
   CALL log_message(build("exit setPathwayDiagram(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE renamepathway(null)
   CALL log_message("in renamePathway()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   CALL checkduplicate(null)
   IF (dpathwayid > 0
    AND lstat > 0)
    UPDATE  FROM cp_pathway cp
     SET cp.updt_applctx = reqinfo->updt_applctx, cp.updt_cnt = (cp.updt_cnt+ 1), cp.updt_dt_tm =
      cnvtdatetime(sysdate),
      cp.updt_id = reqinfo->updt_id, cp.updt_task = reqinfo->updt_task, cp.pathway_name =
      updatepathwayrec->newpathwayname
     WHERE cp.cp_pathway_id=dpathwayid
     WITH nocounter
    ;end update
   ENDIF
   CALL updatebedrock(null)
   CALL log_message(build("exit renamePathway(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE renametreatmentline(null)
   CALL log_message("in renameTreatmentLine()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   CALL checkduplicatetl(null)
   IF (dpathwayid > 0
    AND lstat > 0)
    SELECT INTO "nl:"
     FROM cp_node cn
     WHERE (cn.cp_node_id=updatepathwayrec->treatmentlineid)
     DETAIL
      streatmentlinename = cn.node_name, scategorymean = cn.category_mean
     WITH nocounter
    ;end select
    UPDATE  FROM cp_node cn
     SET cn.updt_applctx = reqinfo->updt_applctx, cn.updt_cnt = (cn.updt_cnt+ 1), cn.updt_dt_tm =
      cnvtdatetime(sysdate),
      cn.updt_id = reqinfo->updt_id, cn.updt_task = reqinfo->updt_task, cn.node_name =
      updatepathwayrec->newtreatmentlinename,
      cn.node_display = updatepathwayrec->newtreatmentlinename
     WHERE (cn.cp_node_id=updatepathwayrec->treatmentlineid)
     WITH nocounter
    ;end update
   ENDIF
   CALL updatebedrocktl(null)
   CALL log_message(build("exit renameTreatmentLine(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE updatebedrock(null)
   CALL log_message("in updateBedrock()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE count = i4 WITH protect, noconstant(0)
   DECLARE brsize = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM br_datamart_category br,
     cp_node cn
    PLAN (cn
     WHERE (cn.cp_pathway_id=pathway_updt_reply->pathway_id))
     JOIN (br
     WHERE br.category_mean=cn.category_mean)
    DETAIL
     count += 1, stat = alterlist(br_category->brlist,count), br_category->brlist[count].
     category_name = br.category_name,
     br_category->brlist[count].br_datamart_category_id = br.br_datamart_category_id
    WITH nocounter
   ;end select
   SET brsize = size(br_category->brlist,5)
   FOR (i = 1 TO brsize)
     SET stat = initrec(bed_ens_req)
     SET stat = initrec(bed_ens_rep)
     SET bed_ens_req->display = replace(br_category->brlist[i].category_name,trim(trim(
        pathway_updt_reply->pathway_name,4),1),trim(trim(updatepathwayrec->newpathwayname,4),1),1)
     SET bed_ens_req->id = br_category->brlist[i].br_datamart_category_id
     SET bed_ens_req->action_flag = 2
     EXECUTE bed_ens_custom_mpage  WITH replace("REQUEST",bed_ens_req)
     IF (validate(debug_ind,0)=1)
      CALL echorecord(bed_ens_req)
      CALL echorecord(bed_ens_rep)
     ENDIF
   ENDFOR
   CALL log_message(build("exit updateBedrock(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE updatebedrocktl(null)
   CALL log_message("in updateBedrockTL()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   SELECT INTO "nl:"
    FROM br_datamart_category br
    WHERE br.category_mean=scategorymean
    DETAIL
     stat = alterlist(br_category->brlist,1), br_category->brlist[1].category_name = br.category_name,
     br_category->brlist[1].br_datamart_category_id = br.br_datamart_category_id
    WITH nocounter
   ;end select
   SET stat = initrec(bed_ens_req)
   SET stat = initrec(bed_ens_rep)
   SET bed_ens_req->display = replace(br_category->brlist[1].category_name,trim(trim(
      streatmentlinename,4),1),trim(trim(updatepathwayrec->newtreatmentlinename,4),1),2)
   SET bed_ens_req->id = br_category->brlist[1].br_datamart_category_id
   SET bed_ens_req->action_flag = 2
   EXECUTE bed_ens_custom_mpage  WITH replace("REQUEST",bed_ens_req)
   IF (validate(debug_ind,0)=1)
    CALL echorecord(bed_ens_req)
    CALL echorecord(bed_ens_rep)
   ENDIF
   CALL log_message(build("exit updateBedrockTL(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
...
#exit_script
 IF (( $OUTDEV != "noforms"))
  CALL putjsonrecordtofile(pathway_updt_reply)
 ENDIF
 IF (validate(debug_ind,0)=1)
  CALL echorecord(pathway_updt_reply)
 ENDIF
 CALL log_message(concat("exiting script: ",log_program_name),log_level_debug)
 CALL log_message(build("total time in seconds:",((curtime3 - currentdatetime2)/ 100.0)),
  log_level_debug)
END GO
