CREATE PROGRAM aps_prt_db_specimen_adequacy:dba
 ...
 SET captions->apsrpt = uar_i18ngetmessage(i18nhandle,"h1",
  "REPORT:  APS_PRT_DB_SPECIMEN_ADEQUACY.PRG")
 ...
 SET captions->dbspecadequacy = uar_i18ngetmessage(i18nhandle,"h8","DB SPECIMEN ADEQUACY")
 SET captions->ppage = uar_i18ngetmessage(i18nhandle,"h9","PAGE:")
 SET captions->status = uar_i18ngetmessage(i18nhandle,"h10","STATUS")
 SET captions->description = uar_i18ngetmessage(i18nhandle,"h11","DESCRIPTION")
 ...
 SET captions->continued1 = uar_i18ngetmessage(i18nhandle,"f1","CONTINUED...")
 ...
 ;end select
#report_maker
 EXECUTE cpm_create_file_name_logical "apsDbSpecAdeq", "dat", "x"
 ...
 SELECT INTO value(reply->print_status_data.print_filename)
  ...
  HEAD PAGE
   ...
   row + 1,
   CALL center(captions->dbspecadequacy,0,132), col 110,
   captions->ppage, col 117, curpage"###",
   row + 2, row + 1, col 0,
   captions->status, col 10, captions->description,
   row + 1, col 0, "--------",
   col 10, "---------------------------------------"
  DETAIL
   row + 1, col 0
   ...
   ELSE
    captions->active
   ENDIF
   col 10, temp->qual[d.seq].spec_description
   IF (((row+ 7) > maxrow))
    BREAK
   ENDIF
  FOOT PAGE
