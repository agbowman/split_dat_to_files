CREATE PROGRAM dm2_create_ora_adm_nicknames:dba
 DECLARE dcoan_logfile_prefix = vc WITH protect, constant("dm2_creoraadm_nkname")
 DECLARE dcoan_node = vc WITH protect, noconstant(" ")
 DECLARE dcoan_user = vc WITH protect, noconstant(" ")
 DECLARE dcoan_pswd = vc WITH protect, noconstant(" ")
 DECLARE dcoan_wrapper = vc WITH protect, noconstant(" ")
 DECLARE dcoan_table = vc WITH protect, noconstant(" ")
 DECLARE dcoan_server = vc WITH protect, noconstant(" ")
 ...
 SUBROUTINE dsl_create_dm_flags_objs(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile(dcoan_logfile_prefix,".log","DM2_CREATE_ORA_ADM_NICKNAMES LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm2_install_schema->special_ih_process = 1
 IF (currdb="ORACLE")
 ...
 SET dm_err->eproc = "Beginning Create Oracle Admin Nicknames Process...."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dcoan_node = build(cnvtupper(trim( $1)))
 IF (dcoan_node=" ")
  SET dm_err->err_ind = 1
  ...
  GO TO exit_program
 ENDIF
 SET dcoan_user = build(cnvtupper(trim( $2)))
 IF (dcoan_user=" ")
  SET dm_err->err_ind = 1
 ...
 IF (dcoan_pswd=" ")
  ...
  GO TO exit_program
 ENDIF
 SET dcoan_wrapper = build(cnvtupper(trim( $4)))
 IF (dcoan_wrapper=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Wrapper type invalid"
  SET dm_err->eproc = "Wrapper type Validation"
  SET dm_err->user_action = "Please specify a valid wrapper type (SQLNET or NET8)"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dcoan_table = build(cnvtupper(trim( $5)))
 IF (dcoan_table=" ")
  SET dm_err->err_ind = 1
  ...
  GO TO exit_program
 ENDIF
 SET dcoan_server = build(cnvtupper(trim( $6)))
 IF (dcoan_server=" ")
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "[Linked] Server name invalid"
  SET dm_err->eproc = "[Linked] Server name Validation"
  SET dm_err->user_action = "Please specify a valid [Linked] Server name"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 CALL echo(concat("         Node Name/Linked Server ($1) = ",dcoan_node))
 CALL echo(concat("                       User Name ($2) = ",dcoan_user))
 CALL echo(concat("                   User Password ($3) = ",dcoan_pswd))
 CALL echo(concat("                    Wrapper Type ($3) = ",dcoan_wrapper))
 CALL echo(concat("                      Table Name ($5) = ",dcoan_table))
 CALL echo(concat("            [Linked] Server Name ($6) = ",dcoan_server))
 IF (dcoan_table=char(42))
  SET dm2_server_link->drop_server_ind = 1
 ENDIF
 SET dm2_server_link->wrapper = dcoan_wrapper
 SET dm2_server_link->server_rdbms = "ORACLE"
 SET dm2_server_link->server_type = "ORACLE"
 SET dm2_server_link->server_version = "8.1"
 SET dm2_server_link->user = dcoan_user
 SET dm2_server_link->password = dcoan_pswd
 SET dm2_server_link->node = dcoan_node
 SET dm2_server_link->dbase = "ADMIN"
 IF (currdb="DB2UDB")
  SET dm2_server_link->option_vntb = "Y"
  SET dm2_server_link->server_name = dcoan_server
  SET dm2_server_link->node = dcoan_node
  SET dm2_server_link->hostname = "NONE"
 ELSE
  SET dm2_server_link->server_name = build(dcoan_server,cnvtupper(currdbname))
  SET dm2_server_link->node = "NONE"
  SET dm2_server_link->hostname = dcoan_node
 ENDIF
 EXECUTE dm2_create_admin_nicknames dcoan_table, value(dcoan_pswd)
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm2_install_schema->oragen3_ignore_dm_columns_doc = 0
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Ending Create Oracle Admin Nicknames Process"
 CALL final_disp_msg(dcoan_logfile_prefix)
END GO
