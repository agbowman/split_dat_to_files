CREATE PROGRAM bhs_rptstd_re_sch_appt_notify:dba
 ...
 SELECT INTO  $1
  ...
  FROM dummyt d
  HEAD REPORT
   y_pos = 0, "{F/4}{CPI/8}{LPI/4}", "{POS/180/60}{B}Same-Day Reschedule Notification",
  ...
  DETAIL
   t_time = format(t_report->appt_time,"@TIMENOSECONDS"), format_text_request->raw_text = concat(
    t_report->patient_qual[1].name," has been rescheduled for a ",t_report->primary_synonym," with ",
    t_report->primary_resource,
    " at ",t_time), format_text_request->chars_per_line = 90,
   CALL format_text(1), y_pos = 80, row + 1
   FOR (i = 1 TO format_text_reply->qual_cnt)
   ...
   IF (y_pos >= 720)
    y_pos = 30, BREAK
   ENDIF
   CALL print(calcpos(30,y_pos)), "{CPI/10}{LPI/5}{B}Appointment Details: {ENDB}", row + 1
   IF ((t_report->detail_qual_cnt != 0))
    FOR (j = 1 TO t_report->detail_qual_cnt)
      y_pos = (y_pos+ 15)
      IF ((temp->qual[j].break_line_nbr > 1))
       FOR (k = 1 TO (temp->qual[j].break_line_nbr - 1))
         IF (y_pos >= 720)
          y_pos = 30, BREAK
         ENDIF
         CALL print(calcpos(30,y_pos)), "{CPI/10}{LPI/5}{B}", temp->qual[j].text_qual[k].text_string,
         y_pos = (y_pos+ 15), row + 1
       ENDFOR
      ENDIF
      t_start = (temp->qual[j].break_pos+ 1), t_len = (textlen(temp->qual[j].text_qual[temp->qual[j].
       break_line_nbr].text_string) - temp->qual[j].break_pos), t_string = substring(1,temp->qual[j].
       break_pos,temp->qual[j].text_qual[temp->qual[j].break_line_nbr].text_string),
      t_string2 = substring(t_start,t_len,temp->qual[j].text_qual[temp->qual[j].break_line_nbr].
       text_string)
      IF (y_pos >= 720)
       y_pos = 30, BREAK
      ENDIF
      CALL print(calcpos(30,y_pos)), "{CPI/10}{LPI/5}{B}", t_string,
      " ", "{ENDB}{CPI/10}{LPI/5}", t_string2,
      row + 1
      IF ((temp->qual[j].text_cnt > temp->qual[j].break_line_nbr))
       FOR (k = (temp->qual[j].break_line_nbr+ 1) TO temp->qual[j].text_cnt)
         y_pos = (y_pos+ 15)
         IF (y_pos >= 720)
          y_pos = 30, BREAK
         ENDIF
         CALL print(calcpos(30,y_pos)), "{CPI/10}{LPI/5}", temp->qual[j].text_qual[k].text_string,
         row + 1
       ENDFOR
      ENDIF
    ENDFOR
   ENDIF
   IF ((t_report->comment_qual_cnt != 0))
    y_pos = (y_pos+ 24)
    IF (y_pos >= 720)
     y_pos = 30, BREAK
    ENDIF
    CALL print(calcpos(30,y_pos)), "{CPI/10}{LPI/5}{B}Appointment Comments: {ENDB}", row + 1,
    y_pos = (y_pos+ 15)
    FOR (j = 1 TO t_report->comment_qual_cnt)
      t_size = size(t_report->comment_qual[j].text)
      IF (j=1)
       IF (y_pos >= 720)
        y_pos = 30, BREAK
       ENDIF
       CALL print(calcpos(60,y_pos)), "{CPI/10}{LPI/5}{B} + ", t_report->comment_qual[j].
       text_type_disp,
       row + 1
      ELSEIF ((t_report->comment_qual[j].text_type_cd != t_report->comment_qual[(j - 1)].text_type_cd
      ))
       IF (y_pos >= 720)
        y_pos = 30, BREAK
       ENDIF
      ...
      IF (y_pos >= 720)
       y_pos = 30, BREAK
      ENDIF
      CALL print(calcpos(90,y_pos)), "{CPI/10}{LPI/5}{B} * ", t_report->comment_qual[j].sub_text_disp,
      row + 1, mark = 1
      WHILE (t_size >= 80)
        t_string = substring(mark,80,t_report->comment_qual[j].text), ind1 = 0, flag = 1,
        i = 0
        WHILE (flag
         AND i <= 80)
         i = (i+ 1),
         IF (ichar(substring(i,1,t_string)) IN (10, 13))
          ind1 = i, flag = 0
         ENDIF
        ENDWHILE
        IF (flag=0)
         t_string = substring(mark,(ind1 - 1),t_report->comment_qual[j].text)
        ELSE
         FOR (i = 1 TO 80)
           IF (substring(i,1,t_string) IN (" ", ",", ".", "-", "\",
           "/"))
            ind1 = i
           ENDIF
         ENDFOR
         IF (ind1=0)
          ind1 = 80
         ENDIF
         t_string = substring(mark,ind1,t_report->comment_qual[j].text)
        ENDIF
        mark = (mark+ ind1), t_size = (t_size - ind1)
        IF (t_string != null)
         y_pos = (y_pos+ 15)
         IF (y_pos >= 720)
          y_pos = 30, BREAK
         ENDIF
         CALL print(calcpos(90,y_pos)), "{ENDB}{CPI/10}{LPI/5}    ", t_string,
         row + 1
        ENDIF
      ENDWHILE
      WHILE (t_size > 0)
        u = (t_size+ mark), t_string = substring(mark,t_size,t_report->comment_qual[j].text), ind1 =
        0,
        flag = 1, i = 0
        WHILE (flag
         AND i <= t_size)
        ...
        ENDWHILE
        IF (flag=0)
         t_string = substring(mark,(ind1 - 1),t_report->comment_qual[j].text), mark = (mark+ ind1),
         t_size = (t_size - ind1)
        ELSE
         t_size = 0
        ENDIF
