CREATE PROGRAM dm2_set_env_id:dba
 ...
 ENDIF
 DECLARE dsei_cur_envid = f8 WITH protect, noconstant(0.00)
 DECLARE dsei_new_envid = f8 WITH protect, noconstant(0.00)
 DECLARE dsei_envname = vc WITH protect, noconstant("DM2_NOT_SET")
 DECLARE dsei_dbname = vc WITH protect, noconstant("DM2_NOT_SET")
 DECLARE dsei_cur_dbname = vc WITH protect, noconstant("DM2_NOT_SET")
 DECLARE dsei_new_dbname = vc WITH protect, noconstant("DM2_NOT_SET")
 DECLARE dsei_cur_envname = vc WITH protect, noconstant("DM2_NOT_SET")
 DECLARE dsei_new_envname = vc WITH protect, noconstant("DM2_NOT_SET")
 DECLARE dsei_env_desc = vc WITH protect, noconstant("DM2_NOT_SET")
 DECLARE dsei_rowcnt = i4 WITH protect, noconstant(0)
 DECLARE dsei_restart_ind = i2 WITH protect, noconstant(1)
 DECLARE dsei_tmp_emsg = vc WITH protect, noconstant("DM2_NOT_SET")
 DECLARE dsei_tmp_envname = vc WITH protect, noconstant("")
 DECLARE dsei_prefix_log = vc WITH protect, noconstant("dm2_seteid")
 SET reqinfo->updt_task = dm2_install_schema->dm2_updt_task_value
 DECLARE dsei_log_env_id_event(dleie_cur_id=f8,dleie_new_id=f8) = i2
 IF (check_logfile(dsei_prefix_log,".log","Starting DM2_SET_ENV_ID")=0)
  GO TO exit_program
 ENDIF
 IF (currdb="SQLSRV")
  IF (dm2_set_autocommit(1)=0)
 ...
 CALL clear(1,1)
 CALL box(1,1,5,132)
 CALL text(3,38,"***  VERIFY AND UPDATE ENVIRONMENT INFORMATION  ***")
 CALL text(7,3,"This program will attempt to update the environment id in the")
 CALL text(8,3,"DM_INFO table so that other programs can function properly.")
 IF ((dm_err->debug_flag > 1))
  CALL clear(9,1)
  CALL text(10,3,dm_err->eproc)
 ENDIF
 SET dsei_dbname = currdbname
 IF (dsei_dbname=" ")
  SET message = nowindow
 ...
 IF ((dm_err->debug_flag > 1))
  SET message = nowindow
  CALL echo(build("dsei_dbname = ",dsei_dbname))
 ENDIF
 SET dm_err->eproc = "Loading local environment information."
 CALL disp_msg("",dm_err->logfile,0)
 IF ((dm_err->debug_flag > 1))
  CALL clear(9,1)
  CALL text(10,3,dm_err->eproc)
 ENDIF
 SELECT INTO "nl:"
  info_char_null_ind = nullind(di.info_char)
  FROM dm_info di
  WHERE di.info_name IN ("DM_ENV_ID", "DM_ENV_NAME")
   AND di.info_domain="DATA MANAGEMENT"
  DETAIL
   IF (di.info_name="DM_ENV_ID")
    dsei_rowcnt = (dsei_rowcnt+ 1), dsei_cur_envid = di.info_number
    IF (info_char_null_ind=0)
     dsei_cur_dbname = di.info_char
    ENDIF
   ELSEIF (di.info_name="DM_ENV_NAME")
    dsei_rowcnt = (dsei_rowcnt+ 1), dsei_cur_envname = di.info_char
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echo(build("dsei_cur_dbname = ",dsei_cur_dbname))
  CALL echo(build("dsei_cur_envid = ",dsei_cur_envid))
  CALL echo(build("dsei_cur_envname = ",dsei_cur_envname))
 ENDIF
 IF (dsei_cur_envid > 0)
  SET dm_err->eproc = build("Confirming and loading environment_id ",dsei_cur_envid,
   "in dm_environment.")
  CALL disp_msg("",dm_err->logfile,0)
  IF ((dm_err->debug_flag > 1))
   CALL clear(9,1)
  ...
  SELECT INTO "nl:"
   FROM dm_environment de
   WHERE de.environment_id=dsei_cur_envid
   DETAIL
    dsei_envname = de.environment_name, dsei_env_desc = de.description
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   SET message = nowindow
 ...
 ENDIF
 IF (dsei_cur_envid > 0)
  IF (dsei_cur_envname != dsei_envname)
   SET dm_err->eproc = "Invalid environment name.  Deleting local record from dm_info."
   CALL disp_msg("",dm_err->logfile,0)
   IF ((dm_err->debug_flag > 1))
    CALL clear(9,1)
    CALL text(10,3,dm_err->eproc)
   ...
   IF (check_error(dm_err->eproc)=1)
    ...
    GO TO exit_program
   ENDIF
   SET dm_err->eproc = build("Inserting environment name <",dsei_envname,"> into dm_info.")
   CALL disp_msg("",dm_err->logfile,0)
   IF ((dm_err->debug_flag > 1))
    CALL clear(9,1)
    CALL text(10,3,dm_err->eproc)
   ENDIF
   INSERT  FROM dm_info di
    SET di.info_domain = "DATA MANAGEMENT", di.info_name = "DM_ENV_NAME", di.info_char = dsei_envname,
     di.updt_dt_tm = cnvtdatetime(curdate,curtime3), di.updt_id = 90001, di.updt_task = reqinfo->
     updt_task,
     di.updt_cnt = 0, di.updt_applctx = 90001
    WITH nocounter
   ;end insert
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
   ...
   COMMIT
  ENDIF
  IF (((dsei_dbname=dsei_cur_dbname) OR (dsei_cur_dbname="DM2_NOT_SET")) )
   CALL text(10,3,"Your current settings indicate that you are in the following environment:")
   CALL text(11,2,"           Environment Name:")
   CALL text(12,2,"                Description:")
   CALL text(13,2,"             Environment ID:")
   CALL text(15,3,"You are currently connected to the following database:")
   CALL text(11,33,dsei_envname)
   CALL text(12,33,dsei_env_desc)
   CALL text(13,33,cnvtstring(dsei_cur_envid))
   CALL text(15,59,dsei_dbname)
   CALL text(17,3,"Are these settings correct ? (Y/N)")
   CALL accept(17,39,"P;CU"," "
    WHERE curaccept IN ("Y", "N"))
   IF (curaccept="Y")
    IF (dsei_cur_dbname="DM2_NOT_SET")
     SET dm_err->eproc = build("Updating database name <",dsei_dbname,"> into dm_info.")
     CALL disp_msg("",dm_err->logfile,0)
     IF ((dm_err->debug_flag > 1))
      CALL clear(9,1)
      CALL text(10,3,dm_err->eproc)
   ...
   ENDIF
  ENDIF
 ELSEIF (dsei_cur_envid=0)
  SET dm_err->eproc = "Deleting local environment id and environment name."
  CALL disp_msg("",dm_err->logfile,0)
  IF ((dm_err->debug_flag > 1))
   CALL clear(9,1)
   CALL text(10,3,dm_err->eproc)
  ...
  COMMIT
 ENDIF
 WHILE (dsei_restart_ind=1)
   SET dsei_restart_ind = 0
   SET dm_err->err_ind = 0
   SET dsei_tmp_envname = ""
   WHILE (dsei_tmp_envname="")
     SET dm_err->eproc = "Populating environment name list from admin."
     CALL disp_msg("",dm_err->logfile,0)
     CALL clear(9,1)
     IF (dsei_cur_envid=0)
      CALL text(20,2,"There is no currently selected environment.  Please choose one.")
     ELSEIF (dsei_dbname != dsei_cur_dbname)
      CALL text(20,2,
       "The current database name does not match the environment currently selected.  Please choose a new one."
       )
     ENDIF
     CALL text(11,3,"Please choose the environment name:")
     SET help =
     SELECT
      d.environment_name, d.database_name, d.description
      FROM dm_environment d
      ORDER BY d.environment_name
      WITH nocounter
     ;end select
     CALL accept(11,38,"P(20);CUF")
     SET dsei_tmp_envname = curaccept
     IF (check_error(dm_err->eproc)=1)
      SET message = nowindow
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      GO TO exit_program
     ENDIF
     CALL text(18,3,"Press C to Continue, X to Exit or R to repeat:")
     CALL accept(18,50,"P;CU","X"
      WHERE curaccept IN ("C", "X", "R"))
     IF (curaccept="X")
      SET dm_err->emsg =
      "Selecting environment from admin, but user chose to exit/quit out of prompt."
      SET dm_err->err_ind = 1
      SET message = nowindow
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      GO TO exit_program
   ...
   SET help = off
   SET validate = off
   SET dm_err->eproc = "Loading environment information from admin."
   CALL disp_msg("",dm_err->logfile,0)
   IF ((dm_err->debug_flag > 1))
    CALL clear(9,1)
    CALL text(10,3,dm_err->eproc)
   ...
   SELECT INTO "nl:"
    FROM dm_environment d
    WHERE d.environment_name=dsei_tmp_envname
    DETAIL
     dsei_new_envid = d.environment_id, dsei_new_envname = d.environment_name, dsei_new_dbname =
     cnvtupper(d.database_name),
     dsei_env_desc = d.description
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    SET message = nowindow
    ...
    GO TO exit_program
   ENDIF
   IF (dsei_new_dbname != dsei_dbname)
    CALL clear(9,1)
    CALL text(10,3,"The environment id was NOT updated successfully!")
    CALL text(12,3,build("Error:  The database name <",dsei_dbname,"> for the new environment id <",
      dsei_new_envid,"> "))
    CALL text(13,3,build("        does not match the database name <",dsei_new_dbname,">."))
    CALL text(18,3,"Press X to Exit or R to repeat:")
    CALL accept(18,35,"P;CU","X"
     WHERE curaccept IN ("X", "R"))
    IF (curaccept="X")
     SET dm_err->emsg = build("The database name <",dsei_dbname,"> for the new environment id <",
      dsei_new_envid,"> does not match the database name <",
      dsei_new_dbname,">.")
     SET dm_err->err_ind = 1
     SET message = nowindow
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     GO TO exit_program
    ENDIF
    SET dsei_restart_ind = 1
   ENDIF
   IF (dsei_restart_ind=0)
    SET dir_env_maint_rs->src_env_id = dsei_cur_envid
    SET dir_env_maint_rs->tgt_env_id = dsei_new_envid
    SET dir_env_maint_rs->tgt_hist_fnd = 0
    IF (dsei_cur_envid > 0
     AND dsei_cur_envid != dsei_new_envid
     AND validate(dm2_bypass_env_hist_chk,2) != 1)
     SET message = nowindow
    ...
    IF ((dir_env_maint_rs->tgt_hist_fnd=1))
     ...
     CALL text(8,3,"DM_INFO table so that other programs can function properly.")
     CALL text(10,3,"The environment id was NOT updated successfully!")
     CALL text(12,3,build("Error:  Found history for environment id <",dsei_new_envid,
       ">, and environment name <",dsei_new_envname,">! "))
     CALL text(13,3,build("        Environment <",dsei_new_envname,
       "> cannot be used to update the environment."))
     CALL text(18,3,"Press X to Exit or R to repeat:")
     CALL accept(18,35,"P;CU","X"
      WHERE curaccept IN ("X", "R"))
     IF (curaccept="X")
      SET dm_err->emsg = build("Found history for environment id <",dsei_new_envid,
       ">, environment name <",dsei_new_envname,">!  Environment <",
       dsei_new_envname,"> cannot be used to update the environment.")
      SET dm_err->err_ind = 1
      SET message = nowindow
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      GO TO exit_program
   ...
   IF (dsei_restart_ind=0)
    IF (dsei_cur_envid > 0)
     SET dm_err->eproc = "Updating environment id and environment name."
     IF ((dm_err->debug_flag > 1))
      CALL clear(9,1)
      CALL text(10,3,dm_err->eproc)
     ENDIF
     UPDATE  FROM dm_info di
      SET di.info_number = dsei_new_envid, di.info_char = dsei_new_dbname
      WHERE di.info_domain="DATA MANAGEMENT"
       AND di.info_name="DM_ENV_ID"
      WITH nocounter
     ;end update
     ...
     IF ((dm_err->err_ind=0))
      UPDATE  FROM dm_info di
       SET di.info_char = dsei_new_envname
       WHERE di.info_domain="DATA MANAGEMENT"
        AND di.info_name="DM_ENV_NAME"
       WITH nocounter
      ;end update
     ...
     IF ((dm_err->err_ind=0))
      COMMIT
      SET message = nowindow
      IF (dsei_log_env_id_event(dsei_cur_envid,dsei_new_envid)=0)
       GO TO exit_program
      ENDIF
      SET message = window
     ENDIF
    ELSE
     SET dm_err->eproc = "Inserting new environment id and environment name."
     IF ((dm_err->debug_flag > 1))
      CALL clear(9,1)
      CALL text(10,3,dm_err->eproc)
     ENDIF
     INSERT  FROM dm_info di
      SET di.info_number = dsei_new_envid, di.info_char = dsei_new_dbname, di.info_domain =
       "DATA MANAGEMENT",
       di.info_name = "DM_ENV_ID", di.updt_dt_tm = cnvtdatetime(curdate,curtime3), di.updt_id = 90001,
       di.updt_task = reqinfo->updt_task, di.updt_cnt = 0, di.updt_applctx = 90001
      WITH nocounter
     ;end insert
     IF (check_error(dm_err->eproc)=1)
      ROLLBACK
     ...
     IF ((dm_err->err_ind=0))
      INSERT  FROM dm_info di
       SET di.info_char = dsei_new_envname, di.info_domain = "DATA MANAGEMENT", di.info_name =
        "DM_ENV_NAME",
        di.updt_dt_tm = cnvtdatetime(curdate,curtime3), di.updt_id = 90001, di.updt_task = reqinfo->
        updt_task,
        di.updt_cnt = 0, di.updt_applctx = 90001
    ...
    IF ((dm_err->err_ind=1))
     CALL clear(9,1)
     CALL text(10,3,"The environment id was NOT updated successfully!")
     CALL text(12,3,concat("Error:  ",substring(1,120,dm_err->emsg)))
     CALL text(18,3,"Press X to Exit or R to repeat:")
     CALL accept(18,35,"P;CU","X"
      WHERE curaccept IN ("X", "R"))
     IF (curaccept="X")
   ...
   ENDIF
   IF (dsei_cur_envid > 0
    AND dsei_new_envid != dsei_cur_envid
    AND dsei_restart_ind=0)
    SET message = nowindow
    EXECUTE dm2_post_env_copy_maint
    SET message = window
    IF ((dm_err->err_ind=1))
     SET dsei_tmp_emsg = dm_err->emsg
     SET dm_err->emsg = ""
     SET dm_err->err_ind = 0
     SET dm_err->eproc = "Restoring previous environment id and environment name."
     UPDATE  FROM dm_info di
      SET di.info_number = dsei_cur_envid, di.info_char = evaluate(dsei_cur_dbname,"DM2_NOT_SET",null,
        dsei_cur_dbname)
      WHERE di.info_domain="DATA MANAGEMENT"
       AND di.info_name="DM_ENV_ID"
      WITH nocounter
     ...
     ENDIF
     UPDATE  FROM dm_info di
      SET di.info_char = dsei_cur_envname
      WHERE di.info_domain="DATA MANAGEMENT"
       AND di.info_name="DM_ENV_NAME"
      WITH nocounter
     ;end update
     ...
     ELSE
      COMMIT
      SET message = nowindow
      IF (dsei_log_env_id_event(dsei_new_envid,dsei_cur_envid)=0)
       GO TO exit_program
      ENDIF
      SET message = window
     ENDIF
     ...
     CALL text(8,3,"DM_INFO table so that other programs can function properly.")
     CALL text(10,3,"The environment id was NOT updated successfully!")
     CALL text(12,3,"Process: Environment Copy Maintenance")
     CALL text(13,3,concat("Error:   ",substring(1,120,dsei_tmp_emsg)))
     IF ((dm_err->err_ind=1))
      CALL text(15,3,concat("Process: ",dm_err->eproc))
      CALL text(16,3,concat("Error:   ",substring(1,120,dm_err->emsg)))
      CALL text(18,3,"This is a fatal error.  Press ENTER to continue.")
      CALL accept(18,55,"AA;D","OK")
      SET dsei_restart_ind = 0
     ELSE
      CALL text(18,3,"Press X to Exit or R to repeat:")
      CALL accept(18,35,"P;CU","X"
   ...
   IF (dsei_restart_ind=0
    ...
    CALL text(7,3,"This program will attempt to update the environment id in the")
    CALL text(8,3,"DM_INFO table so that other programs can function properly.")
    CALL text(10,3,"The environment has been updated successfully!")
    CALL text(11,2,"           Environment Name:")
    CALL text(12,2,"                Description:")
    CALL text(13,2,"             Environment ID:")
    CALL text(15,3,"You are currently connected to the following database:")
    CALL text(11,33,dsei_new_envname)
    CALL text(12,33,dsei_env_desc)
    CALL text(13,33,cnvtstring(dsei_new_envid))
    CALL text(15,59,dsei_new_dbname)
    CALL text(18,3,"Press X to Exit or R to repeat:")
    CALL accept(18,35,"P;CU","X"
     WHERE curaccept IN ("X", "R"))
    IF (curaccept="R")
   ...
   ENDIF
 ENDWHILE
 SUBROUTINE dsei_log_env_id_event(dleie_cur_id,dleie_new_id)
  IF (checkprg("DM_RMC_AUTO_VERIFY_SETUP") > 0)
   SET stat = alterlist(auto_ver_request->qual,1)
   SET auto_ver_request->qual[1].rdds_event = "ENV ID UPDATED"
   SET auto_ver_request->qual[1].cur_environment_id = dleie_cur_id
   SET auto_ver_request->qual[1].paired_environment_id = dleie_new_id
   SET reqinfo->updt_applctx = 90001
   SET reqinfo->updt_id = 90001
   EXECUTE dm_rmc_auto_verify_setup
   IF ((dm_err->err_ind=1))
    ROLLBACK
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ELSEIF ((auto_ver_reply->status="S")
    AND cnvtupper(auto_ver_reply->status_msg)="ADMIN IS NOT AVAILABLE")
    ROLLBACK
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Inserting ENV ID maintenance event into event tables."
    SET dm_err->emsg = "ADMIN is not available"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   COMMIT
...
#exit_program
 ...
 ENDIF
 SET dm_err->eproc = "Finished verifying environment ID and environment name."
 CALL final_disp_msg(dsei_prefix_log)
END GO
