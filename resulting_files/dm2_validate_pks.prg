CREATE PROGRAM dm2_validate_pks
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dvp_owner = vc WITH protect, noconstant( $1)
 DECLARE dvp_table = vc WITH protect, noconstant( $2)
 DECLARE dvp_process = vc WITH protect, noconstant(cnvtupper( $3))
 DECLARE dvp_cnt = i4 WITH protect, noconstant(0)
 DECLARE dvp_retry_cnt = i4 WITH protect, noconstant(0)
 DECLARE dvp_cmd = vc WITH protect, noconstant("")
 DECLARE dvp_load_pk_list(null) = i2
 FREE RECORD dvp_pk
 RECORD dvp_pk(
   1 pk_cnt = i4
   1 pks[*]
 ...
 IF (check_logfile("dm2_validate_pks",".log","dm2_validate_pks")=0)
  GO TO exit_script
 ENDIF
 ...
 IF ((dm_err->err_ind > 0))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting dm2_validate_pks"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ( NOT (dvp_process IN ("MIGRATION TARGET", "MIGRATION SOURCE", "STANDALONE")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = concat(dvp_process," is invalid as a mode of exectution.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Alter session to ALL_ROWS."
 IF (dm2_push_cmd("rdb alter session set optimizer_mode=all_rows go",1)=0)
  GO TO exit_script
 ENDIF
 IF (dvp_load_pk_list(null)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Found ",build(dvp_pk->pk_cnt),
  " primary key constraints which need validated.")
 CALL disp_msg("",dm_err->logfile,0)
 IF ((dvp_pk->pk_cnt=0))
  GO TO exit_script
 ENDIF
 FOR (dvp_cnt = 1 TO dvp_pk->pk_cnt)
   SET dm_err->eproc = concat("Validating constraint ",dvp_pk->pks[dvp_cnt].pk_name," for ",dvp_pk->
    pks[dvp_cnt].owner,'."',
    dvp_pk->pks[dvp_cnt].table_name,'". This is ',build(dvp_cnt)," of ",build(dvp_pk->pk_cnt),
    ".")
   CALL disp_msg(" ",dm_err->logfile,0)
   SET dvp_cmd = concat('RDB ASIS(^ALTER TABLE "',dvp_pk->pks[dvp_cnt].owner,'"."',dvp_pk->pks[
    dvp_cnt].table_name,'" MODIFY CONSTRAINT "',
    dvp_pk->pks[dvp_cnt].pk_name,'" VALIDATE ^) GO')
   SET dvp_retry_cnt = 0
   WHILE (dvp_retry_cnt <= cnvtint(dm2_db_options->resource_busy_maxretry))
     EXECUTE dm2_exec_child_cmd "CMD", dvp_cmd
     CALL check_error(dm_err->eproc)
     IF ((dm_err->err_ind=1))
      IF (((findstring("ORA-00054",dm_err->emsg) > 0) OR (((findstring("ORA-14450",dm_err->emsg) > 0)
       OR (findstring("ORA-14452",dm_err->emsg) > 0)) )) )
       SET dvp_retry_cnt = (dvp_retry_cnt+ 1)
       IF (dvp_retry_cnt > cnvtint(dm2_db_options->resource_busy_maxretry))
        GO TO exit_script
       ELSE
        SET dm_err->err_ind = 0
        SET dm_err->eproc = concat("Pausing 3 seconds. Retrying validation of ",dvp_pk->pks[dvp_cnt].
         pk_name,". ",build(dvp_retry_cnt)," of ",
      ...
      ENDIF
     ELSE
      SET dvp_retry_cnt = (cnvtint(dm2_db_options->resource_busy_maxretry)+ 1)
     ENDIF
   ENDWHILE
 ENDFOR
 IF (dvp_load_pk_list(null)=0)
 ...
 ENDIF
 IF ((dvp_pk->pk_cnt > 0))
  SET dm_err->emsg = concat("ERROR: Found ",build(dvp_pk->pk_cnt),
   " primary key constraints which still need to be validated.")
  SET dm_err->err_ind = 1
  CALL echorecord(dvp_pk)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ELSE
  SET dm_err->eproc = "Primary keys are validated."
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 GO TO exit_script
 SUBROUTINE dvp_load_pk_list(null)
   SET dvp_pk->pk_cnt = 0
   SET stat = alterlist(dvp_pk->pks,dvp_pk->pk_cnt)
   SET dm_err->eproc = "Load list of primary key constraints which need to be validated."
   CALL disp_msg("",dm_err->logfile,0)
   SELECT
    IF (dvp_process="MIGRATION SOURCE")
    ...
    ELSEIF (dvp_process="MIGRATION TARGET")
     FROM dba_constraints dc
     WHERE dc.constraint_type="P"
      AND dc.validated="NOT VALIDATED"
      AND dc.owner IN (
     ...
     (SELECT
      ...
      FROM dm_info@ref_data_link
      WHERE info_domain="DM2_MIG_EXCL")))
    ELSEIF (dvp_process="STANDALONE")
     FROM dba_constraints dc
     WHERE dc.constraint_type="P"
      AND dc.validated="NOT VALIDATED"
      AND dc.owner=patstring(dvp_owner)
      AND dc.table_name=patstring(dvp_table)
    ELSE
    ENDIF
    INTO "nl:"
    DETAIL
     dvp_pk->pk_cnt = (dvp_pk->pk_cnt+ 1)
     IF (mod(dvp_pk->pk_cnt,100)=1)
      stat = alterlist(dvp_pk->pks,(dvp_pk->pk_cnt+ 99))
     ENDIF
     dvp_pk->pks[dvp_pk->pk_cnt].pk_name = dc.constraint_name, dvp_pk->pks[dvp_pk->pk_cnt].table_name
      = dc.table_name, dvp_pk->pks[dvp_pk->pk_cnt].owner = dc.owner
    FOOT REPORT
     stat = alterlist(dvp_pk->pks,dvp_pk->pk_cnt)
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "dm2_validate_pks completed"
 CALL final_disp_msg("dm2_validate_pks")
END GO
