CREATE PROGRAM cp_load_structure_doc:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Clinical Document Instance Identifier" = "",
  "Document Event Id" = 0.0,
  "Term Decoration Id" = 0.0,
  "Return Lowercase Indicator" = 1,
  "Component Version Number" = 0,
  "Component ID" = 0
  WITH outdev, clin_id, event_id,
  decor_id, lowercase_ind, comp_vrsn_nbr,
  comp_id
 ...
 SUBROUTINE (filtercomponentdetails(nodeindx=i4,compindx=i4,comparefield=vc) =null WITH protect)
   ...
   CALL log_message(build("Exit filterComponentDetails(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SET log_program_name = "cp_load_structure_doc"
 DECLARE script_start_tm = dq8 WITH constant(curtime3), private
 DECLARE strdocvrsnnbr = i4 WITH protect, noconstant(0)
 DECLARE strdocclnid = vc WITH protect, noconstant("")
 DECLARE strdocclninstid = vc WITH protect, noconstant("")
 DECLARE recorddatajsonified = vc WITH protect, noconstant("")
 DECLARE main(null) = null
 DECLARE displaylogmessage(vc,dq8) = null
 DECLARE getdyndoctemplate(null) = null
 DECLARE getlongtextvalues(null) = null
 DECLARE getstrdocversionnbr(null) = i4 WITH protect
 RECORD temp_reply(
   1 section_ref[*]
 ...
 ENDIF
 SUBROUTINE displaylogmessage(message,start_tm)
   CALL log_message(build(message," Elapsed time in seconds:",((curtime3 - start_tm)/ 100.0)),
    log_level_debug)
 END ;Subroutine
 SUBROUTINE getdyndoctemplate(null)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   CALL log_message("In getDynDocTemplate()",log_level_debug)
   EXECUTE inn_get_dd_template_by_clnid "NOFORMS", strdocclnid, strdocvrsnnbr,
   strdocclninstid WITH replace("TEMP_REPLY","TEMP_REPLY")
   SET stat = moverec(temp_reply->section_ref,record_data->section_ref)
   IF (strdocclninstid > "")
    SET temp_reply->section_ref[1].template_rltns[1].dd_sref_templ_instance_ident = strdocclninstid
   ENDIF
   CALL displaylogmessage("Exit getDynDocTemplate()",start_tm)
 END ;Subroutine
   ...
   SELECT INTO "nl:"
    ...
    HEAD lt.long_text_id
     ...
     CASE (lt.long_text_id)
      OF  $EVENT_ID:
       record_data->document_events.json = notrim(outbuf)
      OF  $DECOR_ID:
       record_data->term_decorations.json = notrim(outbuf)
     ENDCASE
    WITH nocounter
   ;end select
   CALL displaylogmessage("Exit getLongTextValues()",start_tm)
 END ;Subroutine
 SUBROUTINE getsectionconceptcki(null)
   DECLARE start_tm = dq8 WITH constant(curtime3), private
   CALL log_message("In getSectionConceptCKI()",log_level_debug)
   SELECT INTO "nl:"
    FROM dd_sref_section dss
    PLAN (dss
     WHERE (dss.dd_sref_section_id=record_data->section_ref[1].dd_sref_section_id))
    HEAD REPORT
     record_data->concept_cki = dss.concept_cki
    WITH nocounter
   ;end select
   CALL displaylogmessage("Exit getSectionConceptCKI()",start_tm)
 END ;Subroutine
 SUBROUTINE getstrdocversionnbr(null)
   CALL log_message("In the retrieveVersionNumber subroutine",log_level_debug)
   DECLARE begin_date_time = dq8 WITH private, constant(curtime3)
   DECLARE tempversionnumber = i4 WITH protect, noconstant(0)
   DECLARE doccontenttypecd = f8 WITH constant(uar_get_code_by("MEANING",4003134,"DOCCONTENT")),
   protect
   SELECT INTO "nl:"
    FROM dd_sref_template ddst,
     cp_component_detail ccd
    PLAN (ccd
     WHERE (ccd.cp_component_id= $COMP_ID)
      AND ccd.component_detail_reltn_cd=doccontenttypecd
      AND (ccd.version_nbr= $COMP_VRSN_NBR))
     JOIN (ddst
     WHERE ddst.dd_sref_tmpl_instance_ident=ccd.component_ident)
    DETAIL
     tempversionnumber = ddst.version_nbr, strdocclnid = ddst.cln_ident
    WITH nocounter
   ;end select
   IF (tempversionnumber > 0)
    SET strdocclninstid = ""
   ENDIF
   CALL log_message(build("Exit retrieveVersionNumber(), Elapsed time in seconds:",((curtime3 -
 ...
 SUBROUTINE main(null)
   ...
   DECLARE clin_ident = vc WITH protect, noconstant("")
   SET clin_ident = decodexmlspecialcharacters( $CLIN_ID)
   SET strdocclninstid = nullterm(clin_ident)
   IF (( $COMP_VRSN_NBR > 0))
    SET strdocvrsnnbr = getstrdocversionnbr(null)
   ENDIF
   CALL getdyndoctemplate(null)
   CALL getlongtextvalues(null)
   CALL getsectionconceptcki(null)
   CALL displaylogmessage("Exit Main()",start_tm)
 END ;Subroutine
 SET record_data->status_data.status = "F"
 CALL log_message(concat("Begin script: ",log_program_name),log_level_debug)
 CALL main(null)
 IF (size(record_data->section_ref,5) > 0)
  SET record_data->status_data.status = "S"
 ELSE
  SET record_data->status_data.status = "Z"
 ENDIF
 IF ((record_data->document_events.json=""))
  SET record_data->document_events.json = "[]"
 ENDIF
 IF ((record_data->term_decorations.json=""))
  SET record_data->term_decorations.json = "[]"
 ENDIF
 CALL log_message(concat("Exiting script: ",log_program_name),log_level_debug)
 CALL displaylogmessage("Script Execution",script_start_tm)
 SET recorddatajsonified = cnvtrectojson(record_data)
 SET recorddatajsonified = decodeinternationalcharacters(recorddatajsonified)
 SET stat = initrec(record_data)
 SET stat1 = cnvtjsontorec(recorddatajsonified)
 IF (stat != 0)
  CALL log_message("Failed to empty record_data record structure",log_level_debug)
 ENDIF
 IF (stat1 != 0)
  CALL log_message("Failed to convert decoded Json to record strcuture",log_level_debug)
 ENDIF
 IF (( $OUTDEV != "NOFORMS"))
  IF ( $LOWERCASE_IND)
   CALL putstringtofile(cnvtrectojson(record_data,2))
  ELSE
   CALL putstringtofile(recorddatajsonified)
  ENDIF
 ENDIF
END GO
