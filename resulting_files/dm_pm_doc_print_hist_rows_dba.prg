CREATE PROGRAM dm_pm_doc_print_hist_rows:dba
 ...
 SET reply->table_name = "PM_DOC_PRINT_HIST"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   seqval = min(pdph.pm_doc_print_hist_id)
   FROM pm_doc_print_hist pdph
   WHERE pdph.pm_doc_print_hist_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  seqval = max(pdph.pm_doc_print_hist_id)
  FROM pm_doc_print_hist pdph
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    pdph.rowid
    FROM pm_doc_print_hist pdph
    WHERE parser(sbr_getrowidnotexists("pdph.pm_doc_print_hist_id between curMinID and curMaxID",
      "pdph"))
     AND pdph.printed_dt_tm <= cnvtdatetime((curdate - days_to_keep),235959)
    DETAIL
     rows = (rows+ 1)
     IF (mod(rows,500)=1)
      stat = alterlist(reply->rows,(rows+ 499))
     ENDIF
     reply->rows[rows].row_id = pdph.rowid
    WITH nocounter, maxqual(pdph,value(rowsleft))
   ;end select
