CREATE PROGRAM dm2_admin_oracle_2_db2:dba
 DECLARE convert_data_defaults(data_type=vc,data_default=vc,source_rdbms=vc) = vc
 DECLARE ace_str = vc
 DECLARE data_type_loop_int = i2
 DECLARE display_message_str = vc
 DECLARE update_counter_int = i4
 DECLARE counter_string = vc
 DECLARE tab_name_str = vc
 DECLARE col_name_str = vc
 DECLARE alias_name_str = vc
 DECLARE error_check_counter_int = i4
 DECLARE go_stmt_flag_ind = i4
 DECLARE update_error_check_ind = i2
 DECLARE date_record_int = i4
 DECLARE data_dflt_str = vc
 DECLARE data_type_str = vc
 DECLARE short_string = vc
 DECLARE omittance_flg = i2
 DECLARE exclusion_str = vc
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 SET update_error_check_ind = check_logfile("Oracle_2_DB2_Cnvrsn",".log","dm2_admin_oracle_2_db2")
 IF (update_error_check_ind=0)
  GO TO exit_program
 ENDIF
 CALL disp_msg("Script is starting","Oracle_2_DB2_Cnvrsn",0)
 SET omittance_flg = 0
 ...
 ENDIF
 IF (((currdb="ORACLE") OR (ace_str="DB2UDB")) )
  IF (currdbuser != "ADMIN_TMP")
   CALL disp_msg("The current user is not admin, exiting script.","Oracle_2_DB2_Cnvrsn",1)
   GO TO exit_program
  ENDIF
 ELSE
  CALL disp_msg("The current database is DB2.","Oracle_2_DB2_Cnvrsn",1)
 ENDIF
 ...
 SELECT INTO "NL:"
  ...
  WHERE dm.data_type IN ("DATE", "TIMESTAMP")
   AND ((dm.data_default="SYSDATE") OR (dm.data_default IN ("TO_DATE*")))
  HEAD REPORT
 ...
 ENDFOR
 CALL echo("Data_default's have been changed in record structure, beginning update.")
 IF (size_array > 0)
 ...
 IF (dm2_nocommit_ind=0)
  COMMIT
 ENDIF
 CALL disp_msg("All data defaults of type date updated, starting to change data types",
  "Oracle_2_DB2_Cnvrsn",0)
 ...
 FOR (data_type_loop_int = 1 TO 1)
   ...
   UPDATE  FROM (value(daod_data_types->table_names[data_type_loop_int].name) t)
    ...
    WHERE  NOT (column_name IN ("*\_ID", "*\_CD", "*\_COMPL", "*\_VALUE"))
     AND ((data_type="NUMBER") OR (data_type="FLOAT"))
   ;end update
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ...
   ENDIF
 ENDFOR
 CALL disp_msg("All data types updated, starting to change constraint names","Oracle_2_DB2_Cnvrsn",0)
 ...
 FOR (update_counter_int = 1 TO 2)
   ...
   SET daod_update_stmt->stmt[update_counter_int].frags[5].stmt_frag = concat("Where ",
    " not exists (Select 'x' from ",tab_name_str," DMY",", dm_tables_doc dtd where dtd.table_name = "
    )
   ...
   FOR (error_check_counter_int = 1 TO 6)
     ...
     ELSE
      ...
      CALL disp_msg(display_message_str,"Oracle_2_DB2_Cnvrsn",0)
      CALL disp_msg("Script is now exiting","Oracle_2_DB2_Cnvrsn",0)
      GO TO exit_program
     ENDIF
   ENDFOR
   IF (dm2_nocommit_ind=0)
 ...
 IF (dm2_nocommit_ind=0)
  COMMIT
 ENDIF
 CALL disp_msg("All column indexes/constraints updated, starting to suffix table names",
  "Oracle_2_DB2_Cnvrsn",0)
 ...
 FOR (update_counter_int = 1 TO 17)
  ...
  ENDWHILE
 ENDFOR
 CALL disp_msg("Table names changed to suffixed table name, freeing record structures and variables",
  "Oracle_2_DB2_Cnvrsn",0)
 UPDATE  FROM dm_ts_precedence
  SET data_extent_size = 262144, index_extent_size = 262144, long_extent_size = 262144
   ...
   ELSEIF (((data_type="DATE") OR (data_type="TIMESTAMP")) )
    ...
    IF (source_rdbms="ORACLE")
     IF (substring(1,6,data_default_scrunched)="TODATE")
      SET data_default_return = concat('"SYSIBM"."TIMESTAMP"(',"'",substring(11,4,
        data_default_scrunched),"-",substring(7,2,data_default_scrunched),
       "-",substring(9,2,data_default_scrunched),"-",substring(15,2,data_default_scrunched),".",
       substring(17,2,data_default_scrunched),".",substring(19,2,data_default_scrunched),".000000')")
     ELSEIF (substring(1,7,data_default_scrunched)="SYSDATE")
      SET data_default_return = concat("CURRENT TIMESTAMP")
     ELSE
      SET data_default_return = data_default
     ENDIF
    ELSE
     IF (substring(1,15,data_default_scrunched)="SYSIBMTIMESTAMP")
      SET data_default_return = concat("TO_DATE('",substring(20,2,data_default_scrunched),"/",
       substring(22,2,data_default_scrunched),"/",
       substring(16,4,data_default_scrunched)," ",substring(24,2,data_default_scrunched),":",
       substring(26,2,data_default_scrunched),
       ":",substring(27,2,data_default_scrunched),"', 'MM/DD/YYYY HH24:MI:SS')")
     ELSEIF (substring(1,16,data_default_scrunched)="CURRENTTIMESTAMP")
      SET data_default_return = build("SYSDATE")
     ELSE
      SET data_default_return = data_default
     ENDIF
    ENDIF
...
#exit_program
 ...
 CALL final_disp_msg("Oracle_2_DB2_Cnvrsn")
END GO
