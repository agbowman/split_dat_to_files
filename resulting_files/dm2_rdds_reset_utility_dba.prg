CREATE PROGRAM dm2_rdds_reset_utility:dba
 ...
 DECLARE str = vc
 DECLARE from_id = f8
 DECLARE copy_done = i2
 DECLARE from_name = vc
 DECLARE copy_err_ind = i2
 DECLARE drcc_min_cnt = f8
 DECLARE drcc_max_cnt = f8
 IF (check_logfile("dm2_rdds_rst_util",".log","DM2_RDDS_RST_UTIL LOGFILE")=0)
  SET dm_err->err_ind = 1
 ...
 CALL text(7,10,"Please select the target environment ID that will be reset: ")
...
#redo_from
 ...
 IF (curaccept=0)
  GO TO exit_script
 ENDIF
 SET from_id = curaccept
 SET valid_input_ind = 0
 SELECT INTO "NL:"
  d.target_env_id
  FROM dm_chg_log d
  WHERE d.target_env_id=from_id
   AND d.log_type IN ("MERGED", "PROCES", "NOMV00")
  DETAIL
   valid_input_ind = 1
  WITH nocounter, maxqual(d,1)
 ;end select
 IF (valid_input_ind=0)
  CALL text(24,1,"There are no rows to reset for the selected target environment.")
  GO TO redo_from
 ENDIF
 SET validate = off
 SET help = off
 SELECT INTO "nl:"
  ...
  WHERE d.environment_id=from_id
  DETAIL
   from_name = d.environment_name
  WITH nocounter
 ;end select
 SET str = concat("Do you want to reset transactions for ",from_name,"? (Y/N)")
 CALL text(11,10,str)
 CALL accept(11,(56+ size(from_name)),"P;CU","Y"
 ...
 ELSE
  CALL clear(1,1)
  SET message = nowindow
  SET dm_err->eproc = "Starting reset process"
 ENDIF
 SELECT INTO "nl:"
  the = min(log_id)"#############.##"
  FROM dm_chg_log d
  DETAIL
   rptbuf = reportinfo(2), drcc_min_cnt = cnvtreal(substring(1,15,rptbuf))
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  the = max(log_id)"#############.##"
  FROM dm_chg_log d
  DETAIL
   rptbuf = reportinfo(2), drcc_max_cnt = cnvtreal(substring(1,15,rptbuf))
  WITH nocounter
 ;end select
 SET copy_done = 0
 WHILE (drcc_min_cnt <= drcc_max_cnt
  AND copy_done=0)
   EXECUTE dm2_rdds_reset_child
   IF (copy_err_ind=1)
    SET copy_done = 1
   ENDIF
   SET drcc_min_cnt = (drcc_min_cnt+ 3000)
   IF (drcc_min_cnt > drcc_max_cnt)
    SET copy_done = 1
   ENDIF
 ENDWHILE
 IF (copy_err_ind != 1)
  SET dm_err->eproc = "Finished reseting transactions."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
#exit_script
 FREE SET rel_type_con
 FREE SET valid_input_ind
 FREE SET str
 FREE SET from_id
 FREE SET copy_done
END GO
