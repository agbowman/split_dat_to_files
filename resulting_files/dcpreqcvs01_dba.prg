CREATE PROGRAM dcpreqcvs01:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  p.position_cd, pa.person_id, p2.person_id,
  ...
  SELECT INTO value(request->printer_name)
   ...
   DETAIL
    FOR (vv = 1 TO value(num_of_orders))
      ...
      "{CPI/12}{POS/35/230}", "Order: ", row + 1,
      "{CPI/10}{POS/110/230}", "{b}", orders->qual[vv].order_mnemonic,
      "{endb}", row + 1, xcol = 110,
      ycol = 245
      FOR (fsub = 1 TO 31)
      ...
      " SS#", orders->ssn, row + 1,
      "{CPI/13}{POS/135/710}", orders->qual[vv].order_mnemonic, row + 1,
      "{CPI/13}{POS/315/670}{b}", orders->name, row + 1,
      "{CPI/13}{POS/315/680}{b}", orders->fnbr, row + 1,
      "{CPI/13}{POS/365/680}", "Pt Loc: ", orders->location,
      row + 1, "{CPI/13}{POS/315/690}",
      CALL print(trim(provider)),
      " ", orders->qual[vv].order_provider_name, row + 1,
      "{CPI/13}{POS/315/700}", orders->age2, " ",
      CALL print(substring(1,1,orders->sex)), " ", orders->dob,
      " SS#", orders->ssn, row + 1,
      "{CPI/13}{POS/315/710}", orders->qual[vv].order_mnemonic, row + 1,
      "{CPI/13}{POS/35/740}", "Printed Dt/Tm: ", curdate"MM/DD/YYYY;;D",
      " ", curtime"HH:MM;;M", row + 1,
      "{CPI/13}{POS/185/740}", "Ordered Dt/Tm: ", signed,
      row + 1, "{CPI/13}{POS/340/740}", "Entered by: ",
      orders->qual[vv].action_prsnl_name, row + 1, "{CPI/13}{POS/520/740}",
      "Page: ", curpage"###", row + 1
    ENDFOR
   FOOT PAGE
