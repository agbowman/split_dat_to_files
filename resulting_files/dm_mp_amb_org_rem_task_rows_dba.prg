CREATE PROGRAM dm_mp_amb_org_rem_task_rows:dba
 ...
 SET reply->table_name = "MP_AMB_ORG_REM_TASK"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(rt.mp_amb_org_rem_task_id)
   FROM mp_amb_org_rem_task rt
   WHERE rt.mp_amb_org_rem_task_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(rt.mp_amb_org_rem_task_id)
  FROM mp_amb_org_rem_task rt
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    rt.rowid
    FROM mp_amb_org_rem_task rt
    WHERE rt.mp_amb_org_rem_task_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "rt.updt_dt_tm < cnvtdatetime((curdate - v_days_to_keep), curtime3)","rt"))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = rt.rowid
    WITH nocounter, maxqual(rt,value(v_rows_left))
   ;end select
