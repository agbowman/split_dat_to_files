CREATE PROGRAM clairvia_ant_dschg_br_stndrd:dba
 PROMPT
  "Printer" = "MINE",
  "Facility" = 0,
  "Nurse Unit" = 0,
 ...
 SET reply->status_data.subeventstatus[1].targetobjectvalue = "Failure, script did not complete."
 ...
 DECLARE fac_parser = vc WITH protect
 DECLARE nurse_unit_parser = vc WITH protect
 DECLARE trans_cnt = i4 WITH protect, noconstant(0)
 DECLARE begin_dt_tm = dq8 WITH constant(parsedateprompt( $STARTDTTM,curdate,000000)), protect
 ...
 DECLARE dsch_default_time = vc WITH protect
 DECLARE nfilterpos = i4 WITH protect, noconstant(0)
 DECLARE nidx = i4 WITH protect, noconstant(0)
 DECLARE nvalsize = i4 WITH protect, noconstant(0)
 DECLARE nvalidx = i4 WITH protect, noconstant(0)
 DECLARE temp_date_val = vc WITH protect
 DECLARE temp_time_val = vc WITH protect
 ...
 DECLARE alias_str = vc WITH protect
 ...
 DECLARE dateext = f8 WITH protect, noconstant(0)
 DECLARE timeext = f8 WITH protect, noconstant(0)
 DECLARE string_size = i4 WITH protect, noconstant(0)
 DECLARE output_string = vc WITH protect
 DECLARE time_ind = i2 WITH protect, noconstant(0)
 SUBROUTINE parsedatepromptwithcurtime3(date_str)
   ...
   RETURN(_return_val)
 END ;Subroutine
 IF (((ispromptempty(parameter2( $FACILITY))) OR (ispromptany(parameter2( $FACILITY)))) )
  SET fac_parser = "1=1"
 ELSE
  SET fac_parser = trim(getpromptlist(parameter2( $FACILITY),"e.loc_facility_cd"),3)
 ENDIF
 IF (((ispromptempty(parameter2( $NURSE))) OR (ispromptany(parameter2( $NURSE)))) )
  SET nurse_unit_parser = "1=1"
 ELSE
  SET nurse_unit_parser = trim(getpromptlist(parameter2( $NURSE),"e.loc_nurse_unit_cd"),3)
 ENDIF
 SET stat = initrec(filter)
 SET filter->maincatmean = "MP_CLAIRVIA"
 SET filter->allind = 0
 SET filter->filterscnt = 6
 SET stat = alterlist(filter->filters,filter->filterscnt)
 SET filter->filters[1].fileventmean = "DISCH_REPORT_LOCATION"
 SET filter->filters[1].fileventcatmean = "LINK_ENTRY"
 SET filter->filters[2].fileventmean = "DISCH_REPORT_MNEMONIC"
 SET filter->filters[2].fileventcatmean = "LINK_ENTRY"
 SET filter->filters[3].fileventmean = "FIN_FORMAT"
 SET filter->filters[3].fileventcatmean = "LINK_ENTRY"
 SET filter->filters[4].fileventmean = "ENCNTR_TYPES"
 SET filter->filters[4].fileventcatmean = "ENCNTR_TYPE_CLASS_CDS"
 SET filter->filters[5].fileventmean = "DISCH_DATE"
 SET filter->filters[5].fileventcatmean = "EVENT"
 SET filter->filters[6].fileventmean = "DEFAULT_TIME"
 SET filter->filters[6].fileventcatmean = "LINK_ENTRY"
 CALL getbrfilters(filter->allind)
 CALL loadbrflexvalues( $FACILITY)
 CALL loadbedrockfreetextvalue("DISCH_REPORT_LOCATION","ALPHA",br_prefs->report_location)
 CALL loadbedrockfreetextvalue("DISCH_REPORT_MNEMONIC","ALPHA",br_prefs->report_name)
 CALL loadbedrockfreetextvalue("FIN_FORMAT","ALPHA",br_prefs->fin_format)
 CALL loadbedrockfreetextvalue("DEFAULT_TIME","ALPHA",br_prefs->default_time)
 SET nfilterpos = locateval(nidx,1,size(filter->filters,5),"ENCNTR_TYPES",filter->filters[nidx].
  fileventmean)
 SET nvalsize = size(filter->filters[nfilterpos].values,5)
 SET br_prefs->encntr_types_cnt = nvalsize
 SET stat = alterlist(br_prefs->encntr_types,br_prefs->encntr_types_cnt)
 FOR (nvalidx = 1 TO br_prefs->encntr_types_cnt)
   SET br_prefs->encntr_types[nvalidx].encntr_type_cd = filter->filters[nfilterpos].values[nvalidx].
   valeventcd
 ENDFOR
 SET nfilterpos = locateval(nidx,1,size(filter->filters,5),"DISCH_DATE",filter->filters[nidx].
  fileventmean)
 SET br_prefs->date_ce_cd = filter->filters[nfilterpos].values[1].valeventcd
 CALL echorecord(br_prefs)
 IF (textlen(br_prefs->report_name) > 0)
  SET filename = build2("CLAIRVIA_ANT_DISCH","_",br_prefs->report_name,"_",format(curdate,
    "mmddyyyy;;d"),
   "_",format(curtime3,"hhmmss;3;m"),".txt")
 ELSE
  SET filename = build2("CLAIRVIA_ANT_DISCH","_",br_prefs->report_name,"_",format(curdate,
    "mmddyyyy;;d"),
   "_",format(curtime3,"hhmmss;3;m"),".txt")
 ...
 CALL echo(filewrite)
 IF (textlen(br_prefs->fin_format) <= 0)
  SET fin_ind = 0
 ELSE
  SET fin_ind = 1
  SET fin_format = concat(br_prefs->fin_format,";P0")
 ENDIF
 IF (((ispromptempty(parameter2( $STARTDTTM))) OR (ispromptempty(parameter2( $ENDDTTM)))) )
 ...
 ELSE
  ...
  IF (((cnvtint(begin_dt_tm)=0) OR (cnvtint(end_dt_tm)=0)) )
   CALL logmsg(build2("Invalid Date Range.  Enter valid dates and date range of up to 8 days.  ",
     " Start: ",format(begin_dt_tm,"dd-mm-yyyy hh:mm;;d")," End: ",format(end_dt_tm,
      "dd-mm-yyyy hh:mm;;d"),
     " Hours: ",datediff),ccps_log_error)
   GO TO exit_script
  ENDIF
 ENDIF
 SELECT INTO "NL:"
  ...
  PLAN (e
   WHERE parser(fac_parser)
    AND parser(nurse_unit_parser)
    AND expand(enc_idx,1,br_prefs->encntr_types_cnt,e.encntr_type_class_cd,br_prefs->encntr_types[
    enc_idx].encntr_type_cd))
   JOIN (ea
   ...
   WHERE ce.person_id=e.person_id
    ...
    AND (ce.event_cd=br_prefs->date_ce_cd)
  ...
  HEAD ce.encntr_id
   ...
   IF (mod(trans_cnt,1000)=1)
    stat = alterlist(transaction->transactions,(trans_cnt+ 999))
   ENDIF
   IF (fin_ind=1)
    alias_str = format(ea.alias,fin_format)
   ELSE
    alias_str = ea.alias
   ENDIF
   ...
   trans_cnt].result_val = trim(ce.result_val,3), transaction->transactions[trans_cnt].trans_string
    = build2(trim(format(ce.event_end_dt_tm,"YYYYMMDD|HHMM;;Q")),"|",trim(uar_get_displaykey(e
      .loc_facility_cd),3),"|",trim(alias_str),
    "|"),
   temp_date_val = substring(3,8,ce.result_val), transaction->transactions[trans_cnt].trans_string =
   build2(transaction->transactions[trans_cnt].trans_string,temp_date_val,br_prefs->default_time)
...
#exit_script
 ...
 IF (manual_run_flag=0)
  CALL set_dminfo_date("CCPS",build(curprog,"_",trim(cnvtupper(br_prefs->report_name))),dm_save_date)
 ENDIF
 IF ((transaction->trans_qual_cnt=0))
 ...
 ENDIF
 SET last_mod =
 "001 11/11/2019 MK6280 Anticipated Discharge Date Standardization with Bedrock Extract"
END GO
