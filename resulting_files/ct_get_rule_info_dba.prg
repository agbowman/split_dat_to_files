CREATE PROGRAM ct_get_rule_info:dba
 RECORD reply(
   1 rule_list[10]
     2 ct_rule_id = f8
     2 description = c100
 ...
 SELECT
  ...
  FROM ct_rule r
  WHERE (r.ct_rule_id=request->ct_rule_id)
  DETAIL
   cnt = (cnt+ 1)
   IF (mod(cnt,10)=1
    AND cnt != 1)
   ...
   reply->rule_list[cnt].duration_cd = r.duration_cd, reply->rule_list[cnt].beg_effective_dt_tm = r
   .beg_effective_dt_tm, reply->rule_list[cnt].end_effective_dt_tm = r.end_effective_dt_tm
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET reply->status_data.status = "S"
