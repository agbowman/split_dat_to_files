CREATE PROGRAM dcp_get_tasks_by_tasks_assign:dba
 ...
 DECLARE task_id_count = i4 WITH constant(size(request->task_list,5)), protect
...
")
  ...
  SET stat = alterlist(temptaskrec->task_list,task_id_count)
  FOR (i = 1 TO task_id_count)
    SET temptaskrec->task_list[i].task_id = request->task_list[i].task_id
  ENDFOR
  SELECT INTO "nl:"
  ...
  ;end select
  SELECT INTO "nl:"
   FROM (dummyt d  WITH seq = value(task_id_count)),
    task_activity ta
   PLAN (d)
    JOIN (ta
    WHERE (ta.task_id=temptaskrec->task_list[d.seq].task_id)
     AND ((ta.person_id+ 0) > context->last_person_id)
     AND parser(trim(expandclauses)))
