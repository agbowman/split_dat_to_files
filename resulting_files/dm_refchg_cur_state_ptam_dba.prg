CREATE PROGRAM dm_refchg_cur_state_ptam:dba
 ...
 SUBROUTINE drcs_get_func_name(dgfn_table_name,dgfn_type)
   ...
   RETURN(dgfn_func_str)
 END ;Subroutine
 IF (check_logfile("DM_REFCHG_CSP",".log","DM_REFCHG_CSP LogFile")=0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 DECLARE drcsp_loop = i4 WITH protect
 DECLARE drcsp_table_name = vc WITH protect, noconstant(" ")
 DECLARE drcsp_func_name = vc WITH protect, noconstant(" ")
 SET drcsp_table_name = cnvtupper(trim( $1,3))
 SET drcsp_func_name = drcs_get_func_name(drcsp_table_name,"PTAM")
 IF (check_error(dm_err->eproc)=1)
  SET drgqt_reply->err_ind = 1
  SET drgqt_reply->err_msg = dm_err->emsg
  GO TO exit_program
 ELSEIF (drcsp_func_name <= " ")
  SET drgqt_reply->err_ind = 1
  SET drgqt_reply->err_msg = concat("No template function was found for the current state table ",
   drcsp_table_name)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET stat = alterlist(drgqt_request->cols,0)
 SET drgqt_request->tbl_name = drcsp_table_name
 SET drgqt_request->tbl_suffix = cnvtupper(trim( $2,3))
 SET drgqt_request->template_type = "PTAM"
 SET stat = alterlist(refchg_collist_query->qual,0)
 SET refchg_collist_query->err_ind = 0
 SET refchg_collist_query->err_msg = ""
 EXECUTE dm_refchg_cur_state_parm drgqt_request->tbl_name
 IF ((refchg_collist_query->err_ind=1))
  SET drgqt_reply->err_ind = 1
  SET drgqt_reply->err_msg = refchg_collist_query->err_msg
  GO TO exit_program
 ENDIF
 SET drgqt_reply->pkw_template = concat(drcsp_func_name,"(")
 FOR (drcsp_loop = 1 TO size(refchg_collist_query->qual,5))
   SET drgqt_reply->pkw_template = concat(drgqt_reply->pkw_template,"i_",refchg_collist_query->qual[
    drcsp_loop].column_name,",")
 ENDFOR
 SET drgqt_reply->pkw_template = concat(drgqt_reply->pkw_template,"i_db_link)")
#exit_program
END GO
