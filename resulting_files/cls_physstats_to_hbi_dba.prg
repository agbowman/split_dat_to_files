CREATE PROGRAM cls_physstats_to_hbi:dba
 PROMPT
 ...
 DECLARE mf_ext_id = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",320,"EXTERNALIDENTIFIER")
  )
 ...
 SELECT DISTINCT INTO value(output_dest)
  ...
  PLAN (oa
   ...
   WHERE pa.person_id=p.person_id
    AND pa.active_ind=1
    AND pa.prsnl_alias_type_cd=mf_ext_id)
  ORDER BY p.name_full_formatted, o.order_id
  HEAD REPORT
   cpoe_1_cnt = 0, proto_2_cnt = 0, phone_3_cnt = 0,
   down_4_cnt = 0, writ_5_cnt = 0, other_6_cnt = 0,
   bad_cnt = 0, sum_cpoe_cnt = 0, output_line = concat("Counter",",","Physician",",","Encounter Type",
    ",","Activity Type",",","Order Type",",",
    "Facility",",","Fin",",","Patient Name",
    ",","Order",",","Action Prsnl",",",
    "Order ID",",","Order Date",",","Doc Nbr",
    ",","User Name",",","Position"),
  ...
  DETAIL
   IF (oa.action_personnel_id=oa.order_provider_id)
    IF (oa.action_personnel_id != 1)
     sum_cpoe_cnt = (sum_cpoe_cnt+ 1)
    ENDIF
   ENDIF
   IF (sort_order=1)
    cpoe_1_cnt = (cpoe_1_cnt+ 1)
   ELSEIF (sort_order=2)
    proto_2_cnt = (proto_2_cnt+ 1)
   ELSEIF (sort_order=3)
    phone_3_cnt = (phone_3_cnt+ 1)
   ELSEIF (sort_order=4)
    down_4_cnt = (down_4_cnt+ 1)
   ELSEIF (sort_order=5)
    writ_5_cnt = (writ_5_cnt+ 1)
   ELSEIF (sort_order=6)
    other_6_cnt = (other_6_cnt+ 1)
   ELSE
    bad_cnt = (bad_cnt+ 1)
   ENDIF
   i = 1, physname = replace(trim(p.name_full_formatted),",","",0), ms_tmp_str = format(trim(pa.alias,
     3),"#####;p0"),
   output_line = concat(trim(cnvtstring(i)),",",trim(physname,3),",",trim(uar_get_code_display(e
      .encntr_type_cd)),
   ...
   output_line = concat(output_line,',"',trim(uar_get_code_display(e.loc_facility_cd)),'","',trim(ea
    ...
    trim(replace(p2.name_full_formatted,'"'," ",0)),'",',cnvtstring(o.order_id),",",format(o
     .orig_order_dt_tm,"MM/DD/YYYY;;D"),
    ",",ms_tmp_str,",",trim(p.username,3),",",
    trim(uar_get_code_display(p.position_cd),3)), col 1, output_line
   IF (output_line > " ")
    row + 1
   ELSE
    row- (1)
