CREATE PROGRAM dm2_shc_tdb_cach:dba
 ...
 SUBROUTINE (dir_get_storage_type(dgst_db_link=vc) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(dshc_ctf_millennium,- (1))=- (1))
  AND (validate(dshc_ctf_millennium,- (2))=- (2)))
  DECLARE dshc_ctf_millennium = i4 WITH public, constant(1)
  DECLARE dshc_ctf_database = i4 WITH public, constant(2)
  DECLARE dshc_ctf_tcpip = i4 WITH public, constant(3)
  DECLARE dshc_ctf_os = i4 WITH public, constant(4)
 ENDIF
 IF (validate(dshc_generic_err_msg,"X")="X"
  AND validate(dshc_generic_err_msg,"Y")="Y")
  DECLARE dshc_generic_err_msg = vc WITH public, noconstant(
   "Unable to perform this system check.  Please contact your system administrator for assistance.")
 ENDIF
 IF (validate(dshc_generic_good_msg,"X")="X"
  AND validate(dshc_generic_good_msg,"Y")="Y")
  DECLARE dshc_generic_good_msg = vc WITH public, noconstant(concat(
    "This system check is in compliance with Cerner's standard configuration.  ",
    "No action needs to be taken at this time."))
 ENDIF
 DECLARE dshc_lreg_getp(null) = i2
 IF ((validate(dshc->chk_cnt,- (1))=- (1))
  AND (validate(dshc->chk_cnt,- (2))=- (2)))
  FREE RECORD dshc
  RECORD dshc(
    1 process_option = vc
    1 chk_cnt = i4
    1 check[*]
      ...
      2 good_ind = i2
      2 load_to_dbm = i4
      2 check_type_flag = i4
      2 check_desc_text = vc
      2 desc_dtl_cnt = i4
      2 desc_detail[*]
        3 name = vc
        3 value_prefix = vc
        ...
        3 date_value = dq8
      2 action_ind = i2
      2 take_action_ind = i2
      2 action_cmd_type_ind = i2
      2 action_cnt = i4
      2 action[*]
        3 cmd = vc
  )
  SET dshc->chk_cnt = 0
 ENDIF
 IF ((validate(dshc_env_info->loaded_ind,- (1))=- (1))
  AND (validate(dshc_env_info->loaded_ind,- (2))=- (2)))
  FREE RECORD dshc_env_info
  RECORD dshc_env_info(
    1 loaded_ind = i2
    1 database_name = vc
    1 node_name = vc
    1 domain_name = vc
    1 domain_user_name = vc
    1 domain_user_pwd = vc
  )
  SET dshc_env_info->loaded_ind = 0
  SET dshc_env_info->database_name = "NOT_SET"
  SET dshc_env_info->node_name = "NOT_SET"
  SET dshc_env_info->domain_name = "NOT_SET"
  SET dshc_env_info->domain_user_name = "NOT_SET"
  SET dshc_env_info->domain_user_pwd = "NOT_SET"
 ENDIF
 IF (validate(dshc_lreg_getp->key_name,"X")="X"
  AND validate(dshc_lreg_getp->key_name,"Z")="Z")
  FREE RECORD dshc_lreg_getp
  RECORD dshc_lreg_getp(
    1 key_name = vc
    1 propname = vc
    1 value_type = i2
    1 propvalue = vc
 ...
 ENDIF
 SUBROUTINE (dshc_add_check_to_rs(s_dactr_name=vc,s_dactr_long_name=vc,s_dactr_load_to_dbm=i4,
  s_dactr_status=c1,s_dactr_check_type=i4) =i2)
   SET dm_err->eproc = "Adding a new check to the list."
   CALL disp_msg(" ",dm_err->logfile,0)
   IF ( NOT (s_dactr_load_to_dbm IN (0, 1)))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Load to Database Monitoring indicator was not 0 or 1."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF ( NOT (s_dactr_status IN ("E", "G", "R")))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Status was not valid."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (size(s_dactr_name) > 40)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Name was greater than 40 characters."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (size(s_dactr_long_name) > 80)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Long name was greater than 80 characters."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   SET dshc->chk_cnt += 1
   SET stat = alterlist(dshc->check,dshc->chk_cnt)
   SET dshc->check[dshc->chk_cnt].name = s_dactr_name
   SET dshc->check[dshc->chk_cnt].long_name = s_dactr_long_name
   SET dshc->check[dshc->chk_cnt].load_to_dbm = s_dactr_load_to_dbm
   IF (((s_dactr_check_type > 4) OR (s_dactr_check_type < 1)) )
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Check Type Flag was not valid."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ELSE
    SET dshc->check[dshc->chk_cnt].check_type_flag = s_dactr_check_type
   ENDIF
   IF (s_dactr_status="E")
    SET dshc->check[dshc->chk_cnt].error_ind = 1
    SET dshc->check[dshc->chk_cnt].good_ind = 0
   ELSEIF (s_dactr_status="G")
    SET dshc->check[dshc->chk_cnt].error_ind = 0
    SET dshc->check[dshc->chk_cnt].good_ind = 1
   ELSEIF (s_dactr_status="R")
    SET dshc->check[dshc->chk_cnt].error_ind = 0
    SET dshc->check[dshc->chk_cnt].good_ind = 0
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE dshc_lreg_getp(null)
   DECLARE s_dlg_lreg_cmd = vc WITH public, noconstant("not_set")
   DECLARE s_dlg_vms_check = vc WITH public, noconstant("not_set")
   DECLARE s_dlg_aix_bs_pos = i4 WITH public, noconstant(0)
   SET dshc_lreg_getp->propvalue = ""
   IF ((dm_err->debug_flag > 1))
    CALL echorecord(dshc_lreg_getp)
   ENDIF
   IF (cursys="AXP")
    SET dm_err->eproc = "Checking current process for SYSLCK privilege."
    SET s_dlg_vms_check = 'PROCPRIV = F$PRIVILEGE("SYSLCK") ; SHOW SYMBOL PROCPRIV'
    IF (dm2_push_dcl(s_dlg_vms_check)=0)
     RETURN(0)
    ENDIF
    IF (parse_errfile(dm_err->errfile)=0)
     RETURN(0)
    ENDIF
    SET dm_err->errtext = replace(dm_err->errtext,'PROCPRIV = "',"",0)
    SET dm_err->errtext = replace(dm_err->errtext,'"',"",1)
    IF ((dm_err->errtext="FALSE"))
     SET dm_err->emsg = "Current OpenVMS user does not have SYSLCK privilege."
     SET dm_err->user_action = "Contact your system administrator to obtain this privilege."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
   ENDIF
   SET dm_err->eproc = "Getting a value from the Cerner Millennium registry."
   IF ( NOT ((dshc_lreg_getp->value_type IN (1, 2))))
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Value Type Validation for subroutine call to dshc_lreg_getp"
    SET dm_err->emsg = "Invalid value type, must be 1 or 2"
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   IF (findstring("\\",dshc_lreg_getp->key_name,1))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "The key_name value supplied contained inappropriate syntax, '\\'."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (cursys="AIX")
    SET dshc_lreg_getp->key_name = replace(dshc_lreg_getp->key_name,"\","\\",0)
   ENDIF
   SET s_dlg_lreg_cmd = concat("lreg -getp ",dshc_lreg_getp->key_name," ",dshc_lreg_getp->propname)
   IF (cursys="AXP")
    SET s_dlg_lreg_cmd = concat(s_dlg_lreg_cmd," ;show symbol LREG_RESULT")
   ENDIF
   SET dm_err->disp_dcl_err_ind = 0
   IF (dm2_push_dcl(s_dlg_lreg_cmd)=0)
    IF ((dm_err->err_ind=1))
     RETURN(0)
    ENDIF
   ENDIF
   ...
   IF (cursys="AXP")
    ...
    SET dm_err->errtext = replace(dm_err->errtext,'"',"",1)
   ENDIF
   IF (trim(dm_err->errtext,3) != "")
    IF ((dshc_lreg_getp->value_type=1))
     IF (isnumeric(trim(dm_err->errtext,3))=0)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "The registry value was expected to be, but was not numeric."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      RETURN(0)
     ENDIF
    ELSE
     IF (isnumeric(trim(dm_err->errtext,3)) > 0)
      SET dm_err->err_ind = 1
      SET dm_err->emsg = "The registry value was expected to be, but was not a string."
      CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
      RETURN(0)
     ENDIF
    ENDIF
   ...
   IF (findstring("%DCL-W-UNDSYM",dm_err->errtext) > 0)
    SET dshc_lreg_getp->propvalue = ""
   ELSE
    SET dshc_lreg_getp->propvalue = trim(dm_err->errtext,3)
   ENDIF
   RETURN(1)
 END ;Subroutine
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   IF ((dshc_env_info->loaded_ind=1))
    RETURN(1)
   ENDIF
   SET dshc_env_info->database_name = currdbname
   SET dshc_env_info->node_name = curnode
   SET dshc_lreg_getp->key_name = concat("\environment\",trim(logical("environment"),3))
   SET dshc_lreg_getp->propname = "domain"
   SET dshc_lreg_getp->value_type = 2
   IF (dshc_lreg_getp(null)=0)
    RETURN(0)
   ENDIF
   IF ((dshc_lreg_getp->propvalue=""))
    SET dm_err->err_ind = 1
    SET dm_err->emsg = "Unable to get domain name from registry."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ELSE
    SET dshc_env_info->domain_name = dshc_lreg_getp->propvalue
   ENDIF
   IF (s_dlei_prompt_ind=1)
    SET dm_err->eproc = "Prompting for domain account user name and password."
    SET width = 132
    SET message = window
    CALL text(2,3,"Domain Account Information",wide)
    CALL box(3,3,9,130)
    WHILE ((dshc_env_info->domain_user_name="NOT_SET"))
      CALL text(5,5,"Please enter domain user name:")
      CALL accept(5,40,"P(30);C")
      SET dshc_env_info->domain_user_name = trim(curaccept)
      IF ((dshc_env_info->domain_user_name=""))
       SET dshc_env_info->domain_user_name = "NOT_SET"
      ENDIF
    ENDWHILE
    WHILE ((dshc_env_info->domain_user_pwd="NOT_SET"))
      CALL text(7,5,"Please enter domain user password:")
      CALL accept(7,40,"P(30);CE")
      SET dshc_env_info->domain_user_pwd = trim(curaccept)
      IF ((dshc_env_info->domain_user_pwd=""))
       SET dshc_env_info->domain_user_pwd = "NOT_SET"
      ENDIF
    ENDWHILE
    CALL text(10,3,"Enter 'C' to continue or 'Q' to continue later. (C or Q):")
    CALL accept(10,60,"P;CU","C"
     WHERE curaccept IN ("Q", "C"))
    SET message = nowindow
    IF (curaccept="Q")
     SET dm_err->emsg = "The user chose to quit."
     SET dm_err->err_ind = 1
     CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
     RETURN(0)
    ENDIF
   ENDIF
   SET dshc_env_info->loaded_ind = 1
   IF ((dm_err->debug_flag > 1))
    CALL echorecord(dshc_env_info)
   ENDIF
   RETURN(1)
 END ;Subroutine
 IF ((validate(dstc_servers->server_cnt,- (1))=- (1))
  AND (validate(dstc_servers->server_cnt,- (2))=- (2)))
  RECORD dstc_servers(
    1 server_cnt = i4
    1 master_exist = i2
    1 slave_exist = i2
  ...
  SET dstc_servers->server_cnt = 0
  SET dstc_servers->master_exist = 0
  SET dstc_servers->slave_exist = 0
  SET dstc_servers->total_entries = "NOT_SET"
  SET dstc_servers->server_cnt = 2
  SET stat = alterlist(dstc_servers->server,dstc_servers->server_cnt)
  SET dstc_servers->server[1].server_name = "Transaction Database Master"
  SET dstc_servers->server[1].cache_value = "NOT CACHING"
  SET dstc_servers->server[1].server_id = "NOT_SET"
  SET dstc_servers->server[1].exist_ind = 0
  SET dstc_servers->server[2].server_name = "Transaction Database Slave"
  SET dstc_servers->server[2].cache_value = "NOT CACHING"
  SET dstc_servers->server[2].server_id = "NOT_SET"
  SET dstc_servers->server[2].exist_ind = 0
 ENDIF
 DECLARE dstc_fatal_err_ind = i2 WITH private, noconstant(0)
 DECLARE dstc_getfile = vc WITH protect, noconstant("NOT_SET")
 DECLARE dstc_extension = vc WITH protect, noconstant(".ksh")
 DECLARE dstc_output = vc WITH protect, noconstant("NOT_SET")
 DECLARE dstc_status = c1 WITH protect, noconstant("X")
 DECLARE dstc_exec_cmd = vc WITH protect, noconstant("NOT_SET")
 DECLARE dstc_regfile = vc WITH protect, noconstant("NOT_SET")
 DECLARE dstc_reg_prefix = vc WITH protect, noconstant("NOT_SET")
 DECLARE dstc_cnt = i4 WITH protect, noconstant(0)
 DECLARE dstc_cnt2 = i4 WITH protect, noconstant(0)
 DECLARE dstc_cache_too_small = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_shc_tdb_",".log","SYSTEM HEALTH CHECK TDB CACHING LOG FILE")=0)
  SET dstc_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking for TDB entries that are not cached."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dshc->process_option="REPORT"))
  IF (dshc_load_env_info(1)=0)
   SET dstc_fatal_err_ind = 1
   GO TO exit_script
  ENDIF
 ELSE
  IF (dshc_load_env_info(0)=0)
   SET dstc_fatal_err_ind = 1
   GO TO exit_script
  ENDIF
 ENDIF
 IF (cursys="AXP")
  SET dstc_extension = ".com"
 ENDIF
 IF (get_unique_file("dm2_shc_tdb",dstc_extension)=0)
  SET dstc_status = "E"
 ENDIF
 IF (dstc_status != "E")
  SET dstc_getfile = dm_err->unique_fname
  SET dstc_output = replace(dstc_getfile,dstc_extension,".dat",0)
  SELECT INTO trim(dstc_getfile)
   FROM dual
   DETAIL
    IF (cursys="AIX")
     col + 0, "$cer_exe/tdbview system <<!", row + 1
    ELSE
     col + 0, "$ mcr cer_exe:tdbview system", row + 1
    ENDIF
    col + 0, dshc_env_info->domain_user_name, row + 1,
    col + 0, dshc_env_info->domain_name, row + 1,
    col + 0, dshc_env_info->domain_user_pwd
    IF (cursys="AIX")
     row + 1, col + 0, "dir -out $CCLUSERDIR/",
     dstc_output, row + 1, col + 0,
     "exit", row + 1, col + 0,
     "!"
    ELSE
     row + 1, col + 0, "dir -out CCLUSERDIR:",
   ...
   WITH nocounter, format = variable, formfeed = none,
    maxrow = 1
  ;end select
  IF (check_error("Writing tdbview script.")=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
   SET dstc_status = "E"
  ENDIF
 ENDIF
 IF (dstc_status != "E"
  AND cursys="AIX")
  SET dstc_exec_cmd = concat("chmod ugo+x $CCLUSERDIR/",dstc_getfile)
  IF (dm2_push_dcl(dstc_exec_cmd)=0)
   SET dstc_status = "E"
  ENDIF
 ENDIF
 IF (dstc_status != "E")
  IF (cursys="AIX")
   SET dstc_exec_cmd = concat("$CCLUSERDIR/",dstc_getfile)
  ELSE
   SET dstc_exec_cmd = concat("@CCLUSERDIR:",dstc_getfile)
  ENDIF
  SET dm_err->eproc = "Searching for the total number of TDB entries."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (dm2_push_dcl(dstc_exec_cmd)=0)
   SET dstc_status = "E"
  ENDIF
 ENDIF
 IF (dstc_status != "E")
  SET dm_err->eproc = "Reading the total number of TDB entries."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (cursys="AIX")
   SET dstc_exec_cmd = concat("cat $CCLUSERDIR/",dstc_output," | wc -l")
  ELSE
   SET dstc_exec_cmd = concat("search /nooutput/statistics CCLUSERDIR:",dstc_output," ffff")
  ENDIF
  IF (dm2_push_dcl(dstc_exec_cmd)=0)
   SET dstc_status = "E"
  ENDIF
 ...
 IF (dstc_status != "E")
  ...
  ELSE
   IF (cursys="AIX")
    SET dstc_servers->total_entries = trim(dm_err->errtext,3)
   ELSE
    SET dstc_te_start_pos = findstring("Records searched:",dm_err->errtext)
    IF (dstc_te_start_pos > 0)
     SET dstc_te_start_pos += 17
     SET dstc_te_end_pos = findstring("       Direct",dm_err->errtext)
     IF (dstc_te_end_pos > 0)
      SET dstc_servers->total_entries = trim(substring(dstc_te_start_pos,(dstc_te_end_pos -
        dstc_te_start_pos),dm_err->errtext),3)
      SET dstc_servers->total_entries = build((cnvtint(dstc_servers->total_entries) - 2))
     ENDIF
    ENDIF
   ENDIF
   IF (cnvtint(dstc_servers->total_entries) <= 0)
    SET dm_err->emsg = "Failed to get the total number of TDB entries."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    SET dstc_status = "E"
   ENDIF
 ...
 ENDIF
 IF (dstc_status != "E")
  SET dm_err->eproc = "Getting the number of cached TDB entries."
  CALL disp_msg(" ",dm_err->logfile,0)
  IF (get_unique_file("dm2_reg",".reg")=0)
   SET dstc_status = "E"
  ENDIF
 ENDIF
 IF (dstc_status != "E")
  SET dstc_regfile = dm_err->unique_fname
  SET dstc_reg_prefix = substring(1,(size(dstc_regfile) - 4),dstc_regfile)
  SELECT INTO trim(dstc_getfile)
   FROM dual
   DETAIL
    IF (cursys="AIX")
     col + 0, "lregview <<!", row + 1,
     col + 0, "copy \\\system\node\", dshc_env_info->node_name,
     "\domain\", dshc_env_info->domain_name, "\servers \\\",
     dstc_reg_prefix, " -sub"
    ELSE
     col + 0, "lregview", row + 1,
     col + 0, "copy \\system\node\", dshc_env_info->node_name,
 ...
 IF (dstc_status != "E")
  ...
  ENDIF
 ENDIF
 FOR (dstc_cnt = 1 TO dstc_servers->server_cnt)
   IF (dstc_status != "E")
    IF (cursys="AIX")
     SET dstc_exec_cmd = concat('grep -p "',dstc_servers->server[dstc_cnt].server_name,'" $cer_reg/',
      dstc_regfile)
    ELSE
     SET dstc_exec_cmd = concat("search cer_reg:",dstc_regfile,' "',dstc_servers->server[dstc_cnt].
      server_name,'" /window=(8,2) /nowarnings')
    ENDIF
    IF (dm2_push_dcl(dstc_exec_cmd)=0)
     SET dstc_status = "E"
    ENDIF
   ...
   ENDIF
   IF (dstc_status != "E")
    SET dm_err->eproc = "Parsing output from TDBView."
    CALL disp_msg(" ",dm_err->logfile,0)
    SELECT INTO "nl:"
     r.line
     FROM rtl2t r
     HEAD REPORT
      rpt_sid_start_pos = 0, rpt_sid_end_pos = 0
     DETAIL
      rpt_sid_start_pos = findstring("[\",r.line)
      IF (rpt_sid_start_pos > 0)
       rpt_sid_start_pos += 2, rpt_sid_end_pos = findstring("]",r.line), dstc_servers->server[
       dstc_cnt].server_id = substring(rpt_sid_start_pos,(rpt_sid_end_pos - rpt_sid_start_pos),r.line
        ),
       dstc_servers->server[dstc_cnt].exist_ind = 1
      ENDIF
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
   IF (dstc_status != "E"
    AND (dstc_servers->server[dstc_cnt].server_id != "NOT_SET"))
    SET dshc_lreg_getp->key_name = concat("\node\",dshc_env_info->node_name,"\domain\",dshc_env_info
     ->domain_name,"\servers\",
     dstc_servers->server[dstc_cnt].server_id)
    SET dshc_lreg_getp->propname = "ServerParams"
    SET dshc_lreg_getp->value_type = 2
    IF (dshc_lreg_getp(null)=0)
     SET dstc_status = "E"
    ELSE
     SET dstc_ce_start_pos = findstring("-cache ",dshc_lreg_getp->propvalue)
     IF (dstc_ce_start_pos > 0)
      SET dstc_ce_start_pos += 7
      SET dstc_ce_end_pos = (findstring(" ",dshc_lreg_getp->propvalue,dstc_ce_start_pos) - 1)
      IF (dstc_ce_end_pos <= 0)
       SET dstc_ce_end_pos = size(dshc_lreg_getp->propvalue)
      ENDIF
      SET dstc_servers->server[dstc_cnt].cache_value = substring(dstc_ce_start_pos,((dstc_ce_end_pos
        - dstc_ce_start_pos)+ 1),dshc_lreg_getp->propvalue)
     ENDIF
    ENDIF
   ENDIF
 ENDFOR
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(dstc_servers)
 ENDIF
 IF ((dstc_servers->server[1].exist_ind=1)
  AND cnvtint(dstc_servers->server[1].cache_value) < cnvtint(dstc_servers->total_entries))
  SET dstc_cache_too_small = 1
 ENDIF
 IF ((dstc_servers->server[2].exist_ind=1)
  AND cnvtint(dstc_servers->server[2].cache_value) < cnvtint(dstc_servers->total_entries))
  SET dstc_cache_too_small = 1
 ENDIF
 IF ((((dm_err->err_ind=1)) OR (dstc_status="E")) )
  SET dstc_status = "E"
 ELSEIF (((dstc_cache_too_small=1) OR ((dstc_servers->server[1].exist_ind=0)
  AND (dstc_servers->server[2].exist_ind=0))) )
  SET dstc_status = "R"
 ELSE
  SET dstc_status = "G"
 ENDIF
 IF (dshc_add_check_to_rs("TDB Caching","TDB Caching",0,dstc_status,dshc_ctf_millennium)=0)
  SET dsss_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 IF (dstc_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
 SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,dshc->check[dshc->chk_cnt].desc_dtl_cnt)
 SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].name =
 "Total TDB Entries"
 SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].value_prefix =
 "Total TDB entries:"
 SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].dtl_desc_text =
 "NOT_SET"
 SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].type = 2
 SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].str_value =
 dstc_servers->total_entries
 IF ((dstc_servers->server[1].exist_ind=0)
  AND (dstc_servers->server[2].exist_ind=0))
  SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
  SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,dshc->check[dshc->chk_cnt].desc_dtl_cnt
   )
  ...
  SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].dtl_desc_text
   =
  "No Transaction Database server was detected.  Consider defining a Transaction Database Slave on this node."
  SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].type = 2
 ELSE
  IF ((dstc_servers->server[1].exist_ind=1))
   SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
   SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,dshc->check[dshc->chk_cnt].
    desc_dtl_cnt)
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].name =
   "Total Set to Cache"
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].value_prefix
   ...
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].type = 2
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].str_value =
   dstc_servers->server[1].cache_value
  ENDIF
  IF ((dstc_servers->server[2].exist_ind=1))
   SET dshc->check[dshc->chk_cnt].desc_dtl_cnt += 1
   SET stat = alterlist(dshc->check[dshc->chk_cnt].desc_detail,dshc->check[dshc->chk_cnt].
   ...
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].type = 2
   SET dshc->check[dshc->chk_cnt].desc_detail[dshc->check[dshc->chk_cnt].desc_dtl_cnt].str_value =
   dstc_servers->server[2].cache_value
  ENDIF
 ENDIF
 SET dshc->check[dshc->chk_cnt].check_desc_text = concat(
  "To achieve the best performance of the TDB servers, caching should be set up so all TDB entries are cached.  This caching",
  ' is controlled by the SCP parameter "-cache" for the Transaction Database Master or Transaction Database Slave servers.',
  "  Make sure the TDB servers are caching more entries than there are TDB entries.")
#exit_script
 SET dm_err->eproc = "Finished checking for TDB entries that are not cached."
 CALL final_disp_msg("dm2_shc_tdb_")
 IF (dstc_fatal_err_ind=0)
  SET dm_err->err_ind = 0
 ENDIF
END GO
