CREATE PROGRAM dm_sch_action_rows:dba
 ...
 IF (v_days_to_keep < g_minimum_keep_days)
  ...
  SET reply->status_data.status = "F"
 ELSE
  SET reply->table_name = "SCH_ACTION"
  SET reply->rows_between_commit = g_rows_between_commit
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(sa.sch_action_id)
    FROM sch_action sa
    WHERE sa.sch_action_id > 0
    DETAIL
  ...
  WHILE (curminid <= maxid
   AND rowsleft > 0)
    SELECT INTO "nl:"
     a.rowid
     FROM sch_action a
     WHERE a.sch_action_id BETWEEN curminid AND curmaxid
      AND a.parent_table IN ("SCH_REPORT", "SCH_QUERY")
      AND parser(sbr_getrowidnotexists(
       "a.action_dt_tm < cnvtdatetime((curdate - v_days_to_keep), curtime3)","a"))
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,100)=1)
