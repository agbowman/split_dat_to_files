CREATE PROGRAM bbt_tag_cross_barcode:dba
 RECORD antibody(
   1 antibodylist[10]
     2 antibody_cd = f8
     2 antibody_disp = c15
 ...
 RECORD captions(
   ...
   1 crossmatch_tech = vc
   1 crossmatch_dt_tm = vc
 ...
 SET captions->crossmatch_interp = uar_i18ngetmessage(i18nhandle,"crossmatch_interp",
  "Crossmatch interp:")
 SET captions->crossmatch_tech = uar_i18ngetmessage(i18nhandle,"crossmatch_tech","Crossmatch tech:")
 SET captions->crossmatch_dt_tm = uar_i18ngetmessage(i18nhandle,"crossmatch_dt_tm",
  "Crossmatch date/time:")
 ...
 EXECUTE cpm_create_file_name_logical "bbt_tag_xm_bar", "dat", "x"
 ...
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  DETAIL
   ...
   nfont_0_15,
   CALL print(calcpos(200,36)), captions->crossmatch_transfusion, row + 1,
   nxpos1 = 36, nxpos2 = 90, nxpos3 = 306,
   nxpos4 = 405, nypos = 72, nfont_31_9
   IF (textlen(trim(patient_name_barcode)) > 0)
    patient_name_barcode = concat("*",patient_name_barcode,"*"),
   ...
   row + 1, nypos += 18,
   CALL print(calcpos(nxpos1,nypos)),
   captions->accession, row + 1,
   CALL print(calcpos(nxpos2,nypos)),
   accession, row + 1, nfont_31_9,
   nypos = 72
   ...
   captions->product_type, row + 1,
   CALL print(calcpos(nxpos4,nypos)),
   product_disp, row + 1, nypos += 18,
   CALL print(calcpos(nxpos3,nypos)), captions->product_expiration, row + 1
   ...
   row + 1, nypos += 18,
   CALL print(calcpos(nxpos3,nypos)),
   captions->crossmatch_interp, row + 1,
   CALL print(calcpos(nxpos4,nypos)),
   xm_result_value_alpha, row + 1, nypos += 18,
   CALL print(calcpos(nxpos3,nypos)), captions->crossmatch_tech, row + 1,
   CALL print(calcpos(nxpos4,nypos)), xm_result_event_prsnl_username, row + 1,
   nypos += 18,
   CALL print(calcpos(nxpos3,nypos)), captions->crossmatch_dt_tm,
   row + 1
   IF (xm_result_event_dt_tm > 0)
    crossmatch_date_time = concat(format(cnvtdatetime(xm_result_event_dt_tm),"@SHORTDATE4YR")," ",
     format(cnvtdatetime(xm_result_event_dt_tm),"@TIMENOSECONDS"))
   ENDIF
   CALL print(calcpos((nxpos4+ 9),nypos)), crossmatch_date_time, row + 1,
   nypos += 18,
   CALL print(calcpos(nxpos3,nypos)), captions->crossmatch_expiration,
   row + 1
   IF (xm_expire_dt_tm > 0)
    crossmatch_expiration = concat(format(cnvtdatetime(xm_expire_dt_tm),"@SHORTDATE4YR")," ",format(
      cnvtdatetime(xm_expire_dt_tm),"@TIMENOSECONDS")),
    CALL print(calcpos((nxpos4+ 9),nypos)), crossmatch_expiration
   ENDIF
   row + 1, nfont_0_16, rpt_row = (row+ 9),
   row rpt_row, col 004, captions->leukofiltered,
   col 031, captions->irradiated, col 058,
   captions->cmv_neg, col 088, captions->vol_reduced,
   rpt_row += 1, row rpt_row, col 004,
   captions->autologous, col 031, captions->directed,
   col 058, captions->hla_typed, col 088,
   captions->hgb_s_neg, rpt_row += 1, row rpt_row,
   col 004, captions->iga_deficient, col 031,
   captions->other, col 058, captions->absc,
   rpt_row += 2, row rpt_row, col 004,
   captions->return_to_bb, col 052, captions->in_30_min,
   rpt_row += 1, row rpt_row, col 004,
   captions->refrigerator, nfont_0_16, rpt_row += 2,
   row rpt_row, col 002, captions->before_transfusion,
   col 053, captions->identified_recipient, col 094,
   captions->clerical_check, rpt_row += 1, row rpt_row,
   col 002, captions->of_the_wristband, col 052,
   captions->number_on_form, col 094, captions->released_by,
   rpt_row += 1, row rpt_row, col 002,
   captions->further_certify, col 057, captions->unit_number,
   col 094, captions->date_time, rpt_row += 1,
   row rpt_row, col 002, captions->stated_on_form,
   rpt_row += 1, row rpt_row, col 041,
   signature27, col 071, signature12,
   col 094, captions->transported_by, rpt_row += 1,
   row rpt_row, col 046, captions->transfusionist,
   col 071, captions->time_started, col 094,
   captions->received_by, rpt_row += 1, row rpt_row,
   col 094, captions->date_time, rpt_row += 1,
   row rpt_row, col 041, signature27,
   col 071, signature12, rpt_row += 1,
   row rpt_row, col 046, captions->transfusionist,
   col 071, captions->time_ended, rpt_row += 7,
   row rpt_row, col 082, captions->hospital_name,
   rpt_row += 1, row rpt_row, col 082,
   captions->hospital_address, rpt_row += 1, row rpt_row,
   col 082, captions->hospital_address, rpt_row += 1,
   row rpt_row, col 082, captions->clia_num,
   rpt_row += 1, row rpt_row, col 038,
   captions->original_to_chart, rpt_row += 1, row rpt_row,
   col 041, captions->copy_to_lab
  FOOT  d.seq
   IF (d.seq > 0
