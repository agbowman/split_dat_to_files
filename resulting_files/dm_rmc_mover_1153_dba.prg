CREATE PROGRAM dm_rmc_mover_1153:dba
 ...
 DECLARE cust_field_value = vc
 DECLARE cust_new_value = vc
 DECLARE cust_cdf_mean_val = vc
 DECLARE cust_to_input_ref = vc
 DECLARE cust_new_seq = f8
 DECLARE cust_pe_pk_col_name = vc
 DECLARE cust_pe_pk_col_value = vc
 ...
 SET cust_tname_fc = dm2_get_rdds_tname("TIER_MATRIX")
 SET tab_name_link = dm2_get_rdds_tname("CODE_VALUE")
 SELECT INTO "NL:"
  FROM (parser(tab_name_link) c)
  WHERE (c.code_value=rs_1153->from_values.tier_cell_type_cd)
   AND c.code_set=13036
  DETAIL
   cust_cdf_mean_val = c.cdf_meaning
  WITH nocounter
 ;end select
 CASE (cust_cdf_mean_val)
  OF "HEALTHPLAN":
   SET pe_tab_name = "HEALTH_PLAN"
   SET cust_pe_pk_col_name = "HEALTH_PLAN_ID"
  OF "INTERFACE":
   SET pe_tab_name = "INTERFACE_FILE"
   SET cust_pe_pk_col_name = "INTERFACE_FILE_ID"
  OF "ORG":
   SET pe_tab_name = "ORGANIZATION"
   SET cust_pe_pk_col_name = "ORGANIZATION_ID"
  OF "PRICESCHED":
   SET pe_tab_name = "PRICE_SCHED"
   SET cust_pe_pk_col_name = "PRICE_SCHED_ID"
  OF "ADD ON":
   SET pe_tab_name = "BILL_ITEM"
   SET cust_pe_pk_col_name = "BILL_ITEM_ID"
  OF "LPRICESCHED":
   SET pe_tab_name = "PRICE_SCHED"
   SET cust_pe_pk_col_name = "PRICE_SCHED_ID"
  ELSE
   SET pe_tab_name = "CODE_VALUE"
   SET cust_pe_pk_col_name = "CODE_VALUE"
 ENDCASE
 IF ((rs_1153->from_values.tier_cell_value_id != 0))
  SET cust_to_input_ref = select_merge_translate(cnvtstring(rs_1153->from_values.tier_cell_value_id),
   pe_tab_name)
  IF (isnumeric(cust_to_input_ref))
   CALL put_value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,"TIER_CELL_VALUE_ID",
    cust_to_input_ref)
  ELSE
   SET cust_rpt_missing = report_missing(pe_tab_name,cust_pe_pk_col_name,rs_1153->from_values.
    tier_cell_value_id)
   CALL evaluate_rpt_missing(cust_rpt_missing)
   SET temp_col_cnt = get_col_pos(cust_tab_name,"TIER_CELL_VALUE_ID")
   SET dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated = 0
   CALL is_translated(cust_tab_name,"ALL")
   GO TO exit_1153
  ENDIF
 ELSE
  CALL put_value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,"TIER_CELL_VALUE_ID","0")
 ENDIF
 SET cust_new_seq = get_seq(cust_tab_name,"TIER_CELL_ID")
 CALL put_value(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name,"TIER_CELL_ID",cnvtstring(
   cust_new_seq))
 SET current_merges = (current_merges+ 1)
 SET child_merge_audit->num[current_merges].action = "NEWSEQ"
 SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  TIER_CELL_ID")
