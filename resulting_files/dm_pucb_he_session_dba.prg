CREATE PROGRAM dm_pucb_he_session:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "dm_pucb_he_session"
 ...
 SUBROUTINE cust_ucb_upt(null)
   UPDATE  FROM he_session hs
    SET hs.updt_cnt = (hs.updt_cnt+ 1), hs.updt_id = reqinfo->updt_id, hs.updt_applctx = reqinfo->
     ...
     hs.priority = 2
    WHERE (((hs.parent_entity_id=request->xxx_uncombine[ucb_cnt].to_xxx_id)) OR ((hs.parent_entity_id
    =request->xxx_uncombine[ucb_cnt].from_xxx_id)))
     AND hs.parent_entity_name="PERSON"
    WITH nocounter
   ;end update
   IF (curqual=0)
