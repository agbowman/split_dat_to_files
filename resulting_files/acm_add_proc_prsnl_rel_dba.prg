CREATE PROGRAM acm_add_proc_prsnl_rel:dba
 ...
 ENDIF
 IF ( NOT (validate(add_proc_prsnl_rel_request)))
  SET failed = not_found
  SET table_name = "add_proc_prsnl_rel_request was not defined by calling script"
  GO TO exit_script
 ENDIF
 IF ( NOT (validate(add_proc_prsnl_rel_reply)))
 ...
 SET add_proc_prsnl_rel_reply->qual_cnt = size(add_proc_prsnl_rel_request->qual,5)
 SET stat = alterlist(add_proc_prsnl_rel_reply->qual,add_proc_prsnl_rel_reply->qual_cnt)
 IF ((add_proc_prsnl_rel_reply->qual_cnt=0))
  SET failed = attribute_error
  SET table_name = "No procedure personnel relationships passed in"
  GO TO exit_script
 ENDIF
 FOR (i = 1 TO add_proc_prsnl_rel_reply->qual_cnt)
   SET generic_ind = true
   IF (generic_ind)
    IF ((add_proc_prsnl_rel_request->qual[i].proc_prsnl_reltn_id=0))
     SELECT INTO "nl:"
      nextseqnum = seq(reference_seq,nextval)
      FROM dual
      DETAIL
       add_proc_prsnl_rel_reply->qual[i].proc_prsnl_reltn_id = nextseqnum, add_proc_prsnl_rel_request
       ->qual[i].proc_prsnl_reltn_id = nextseqnum
      WITH nocounter
     ;end select
     IF (curqual=0)
     ...
     ENDIF
    ELSE
     SET add_proc_prsnl_rel_reply->qual[i].proc_prsnl_reltn_id = add_proc_prsnl_rel_request->qual[i].
     proc_prsnl_reltn_id
    ENDIF
   ENDIF
   IF ( NOT (generic_ind))
    SET add_proc_prsnl_rel_reply->qual[i].status = gen_nbr_error
    SET failed = gen_nbr_error
    SET table_name = "Failed to generate a sequence number"
    GO TO exit_script
   ENDIF
 ENDFOR
 INSERT  FROM proc_prsnl_reltn t,
   (dummyt d  WITH seq = value(add_proc_prsnl_rel_reply->qual_cnt))
  SET t.proc_prsnl_reltn_id = add_proc_prsnl_rel_request->qual[d.seq].proc_prsnl_reltn_id, t
   .prsnl_person_id = add_proc_prsnl_rel_request->qual[d.seq].prsnl_person_id, t.proc_prsnl_reltn_cd
    = add_proc_prsnl_rel_request->qual[d.seq].proc_prsnl_reltn_cd,
   t.procedure_id = add_proc_prsnl_rel_request->qual[d.seq].procedure_id, t.updt_cnt = 0, t
   .updt_dt_tm = cnvtdatetime(curdate,curtime3),
   t.updt_id = reqinfo->updt_id, t.updt_task = reqinfo->updt_task, t.updt_applctx = reqinfo->
   updt_applctx,
   t.active_ind = add_proc_prsnl_rel_request->qual[d.seq].active_ind, t.active_status_cd =
   IF ((add_proc_prsnl_rel_request->qual[d.seq].active_status_cd > 0)) add_proc_prsnl_rel_request->
    qual[d.seq].active_status_cd
   ELSE
    IF ((add_proc_prsnl_rel_request->qual[d.seq].active_ind=true)) reqdata->active_status_cd
   ...
   t.active_status_prsnl_id = reqinfo->updt_id, t.beg_effective_dt_tm = cnvtdatetime(curdate,curtime3
    ), t.end_effective_dt_tm = cnvtdatetime("31-DEC-2100 00:00:00.00"),
   t.contributor_system_cd =
   IF ((add_proc_prsnl_rel_request->qual[d.seq].contributor_system_cd > 0))
    add_proc_prsnl_rel_request->qual[d.seq].contributor_system_cd
   ELSE reqdata->contributor_system_cd
   ENDIF
   , t.free_text_cd = add_proc_prsnl_rel_request->qual[d.seq].free_text_cd, t.ft_prsnl_name =
   add_proc_prsnl_rel_request->qual[d.seq].ft_prsnl_name,
   t.proc_prsnl_ft_ind = add_proc_prsnl_rel_request->qual[d.seq].proc_prsnl_ft_ind, t.proc_ft_prsnl
    = trim(add_proc_prsnl_rel_request->qual[d.seq].proc_ft_prsnl)
  PLAN (d)
   JOIN (t)
  WITH nocounter, status(add_proc_prsnl_rel_reply->qual[d.seq].status)
 ;end insert
 FOR (i = 1 TO add_proc_prsnl_rel_reply->qual_cnt)
   IF ((add_proc_prsnl_rel_reply->qual[i].status=0))
    SET failed = insert_error
    SET add_proc_prsnl_rel_reply->qual[i].status = insert_error
    GO TO exit_script
   ENDIF
 ENDFOR
#exit_script
