CREATE PROGRAM dm_sch_date_com_res_rows:dba
 ...
 ELSE
  ...
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    seqval = min(a.date_comment_id)
    FROM sch_date_comment a
    WHERE a.date_comment_id > 0
    DETAIL
   ...
   AND rowsleft > 0)
    SELECT INTO "nl:"
     ...
     WHERE a.date_comment_id BETWEEN curminid AND curmaxid
      AND a.parent_table="CODE_VALUE"
      AND a.text_type_meaning="RESCOMMENT"
      AND parser(sbr_getrowidnotexists(
       "a.beg_dt_tm < cnvtdatetime((curdate - v_days_to_keep), curtime3)","a"))
     DETAIL
      rows = (rows+ 1)
      IF (mod(rows,100)=1)
