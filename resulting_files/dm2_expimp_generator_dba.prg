CREATE PROGRAM dm2_expimp_generator:dba
 ...
 SUBROUTINE dm2_rename_login_default(sbr_rld_mode)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE deg_disp_ora_version_err(null) = null
 DECLARE deg_fix_loc(dfl_file_loc=vc(ref)) = null
 DECLARE deg_get_node_name(dgn_node_name=vc(ref)) = i2
 DECLARE deg_create_par_file(dcp_file_name=vc,dcp_query=vc,dcp_mode=i2) = i2
 DECLARE deg_create_ksh(dck_ksh_name=vc,dck_pipe_file=vc,dck_par_file=vc,dck_commit_ind=i2,
  dck_from_nbr=i4,
  dck_to_nbr=i4,dck_table_name=vc) = i2
 DECLARE deg_get_nls_lang(dgn_nls_lang=vc(ref)) = i2
 DECLARE deg_export_prefix = vc WITH protect, noconstant("")
 DECLARE deg_import_prefix = vc WITH protect, noconstant("")
 DECLARE deg_file_prefix = vc WITH protect, noconstant("")
 DECLARE deg_data_chunk_size = f8 WITH protect, noconstant(1000000.0)
 DECLARE deg_unique_nbr = vc WITH protect, noconstant(" ")
 DECLARE deg_tgt_db_os = vc WITH protect, noconstant("")
 DECLARE deg_tgt_ora_home = vc WITH protect, noconstant("")
 DECLARE deg_src_ora_home = vc WITH protect, noconstant("")
 DECLARE deg_tgt_file_loc = vc WITH protect, noconstant("")
 DECLARE deg_loc_length = i4 WITH protect, noconstant(0)
 DECLARE deg_tgt_db_name = vc WITH protect, noconstant("")
 DECLARE deg_src_db_name = vc WITH protect, noconstant("")
 DECLARE deg_tgt_node_name = vc WITH protect, noconstant("")
 DECLARE deg_tmp_data_type = vc WITH protect, noconstant("")
 DECLARE deg_exp_utility_location = vc WITH protect, noconstant("")
 DECLARE deg_imp_utility_location = vc WITH protect, noconstant("")
 DECLARE deg_col_max = f8 WITH protect, noconstant(0.0)
 DECLARE deg_pipe_file_name = vc WITH protect, noconstant("")
 DECLARE deg_low_range = f8 WITH protect, noconstant(0.0)
 DECLARE deg_high_range = f8 WITH protect, noconstant(0.0)
 DECLARE deg_chunk = i4 WITH protect, noconstant(0)
 DECLARE deg_line = vc WITH protect, noconstant("")
 DECLARE deg_query = vc WITH protect, noconstant("")
 DECLARE deg_cnt = i4 WITH protect, noconstant(0)
 DECLARE deg_ksh_name = vc WITH protect, noconstant("")
 DECLARE deg_par_file_cnt = i4 WITH protect, noconstant(0)
 DECLARE deg_new_par_file = i2 WITH protect, noconstant(0)
 DECLARE deg_par_row_cnt = f8 WITH protect, noconstant(0.0)
 DECLARE deg_par_tbl_cnt = i4 WITH protect, noconstant(0)
 DECLARE deg_tbl = i4 WITH protect, noconstant(0)
 DECLARE deg_cnt = i4 WITH protect, noconstant(0)
 DECLARE deg_max_tbl_size = i4 WITH protect, constant(400)
 DECLARE deg_tgt_nls_lang = vc WITH protect, noconstant("")
 DECLARE deg_src_nls_lang = vc WITH protect, noconstant("")
 DECLARE deg_end_pos = i4 WITH protect, noconstant(0)
 DECLARE deg_cmd = vc WITH protect, noconstant("")
 DECLARE deg_expimp_spc_tbls = i2 WITH protect, noconstant(0)
 DECLARE deg_expimp_nondoc_tbls = i2 WITH protect, noconstant(0)
 DECLARE deg_old_file = vc WITH protect, noconstant("")
 DECLARE deg_sql_file = vc WITH protect, noconstant("")
 DECLARE deg_row = i4 WITH protect, noconstant(0)
 DECLARE deg_src_ksh_loc = vc WITH protect, noconstant("")
 DECLARE deg_mig_ind = i2 WITH protect, noconstant(0)
 IF ((validate(dm2_expimp_special_tables,- (1))=- (1))
  AND (validate(dm2_expimp_special_tables,- (999))=- (999)))
  SET deg_expimp_spc_tbls = 0
 ELSE
  SET deg_expimp_spc_tbls = 1
 ENDIF
 IF ((validate(dm2_expimp_nondoc_tables,- (1))=- (1))
  AND (validate(dm2_expimp_nondoc_tables,- (999))=- (999)))
  SET deg_expimp_nondoc_tbls = 0
 ELSE
  SET deg_expimp_nondoc_tbls = 1
 ENDIF
 SET deg_unique_nbr = trim(cnvtstring(format(cnvtdatetime(curdate,curtime),"ccyymmddhhmm;;d")))
 IF (deg_expimp_spc_tbls=0
  AND deg_expimp_nondoc_tbls=0)
  SET deg_export_prefix = build("dm2_dbei_",deg_unique_nbr,"_exp")
  SET deg_import_prefix = build("dm2_dbei_",deg_unique_nbr,"_imp")
  SET deg_file_prefix = build("dm2_dbei_",deg_unique_nbr,"_")
 ELSEIF (deg_expimp_spc_tbls=1)
  SET deg_export_prefix = build("dm2_sdbei_",deg_unique_nbr,"_exp")
  SET deg_import_prefix = build("dm2_sdbei_",deg_unique_nbr,"_imp")
  SET deg_file_prefix = build("dm2_sdbei_",deg_unique_nbr,"_")
 ELSEIF (deg_expimp_nondoc_tbls=1)
  SET deg_export_prefix = build("dm2_ndbei_",deg_unique_nbr,"_exp")
  SET deg_import_prefix = build("dm2_ndbei_",deg_unique_nbr,"_imp")
  SET deg_file_prefix = build("dm2_ndbei_",deg_unique_nbr,"_")
 ENDIF
 ...
 SET deg_file_data->cnt = 0
 FREE RECORD deg_par_table_list
 RECORD deg_par_table_list(
   1 cnt = i4
   1 qual[*]
     2 table_txt = vc
 )
 SET deg_par_table_list->cnt = 0
 FREE RECORD deg_src_ora_version
 RECORD deg_src_ora_version(
   1 version = vc
   1 level1 = i2
   1 level2 = i2
   1 level3 = i2
 ...
 FREE RECORD deg_tgt_ora_version
 RECORD deg_tgt_ora_version(
   1 version = vc
   1 level1 = i2
   1 level2 = i2
   1 level3 = i2
 ...
 IF (check_logfile("dm2_expimp_generator",".log","dm2_expimp_generator")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Starting DM2_EXPIMP_GENERATOR"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET width = 132
 IF (deg_expimp_spc_tbls=1
  AND deg_expimp_nondoc_tbls=1)
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Check modes from dm2_expimp_generator."
  SET dm_err->emsg = "Special tables and Non-documented tables can not be run at the same time."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_script
 ENDIF
 SET deg_src_ora_version->version = dm2_rdbms_version->version
 SET deg_src_ora_version->level1 = dm2_rdbms_version->level1
 SET deg_src_ora_version->level2 = dm2_rdbms_version->level2
 SET deg_src_ora_version->level3 = dm2_rdbms_version->level3
 SET deg_src_ora_version->level4 = dm2_rdbms_version->level4
 ...
 IF (dm2_get_dbase_name(deg_src_db_name)=0)
  GO TO exit_script
 ENDIF
 IF (deg_get_nls_lang(deg_src_nls_lang)=0)
  GO TO exit_script
 ENDIF
 ...
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_script
 ENDIF
 SET deg_tgt_ora_version->version = dm2_rdbms_version->version
 SET deg_tgt_ora_version->level1 = dm2_rdbms_version->level1
 SET deg_tgt_ora_version->level2 = dm2_rdbms_version->level2
 SET deg_tgt_ora_version->level3 = dm2_rdbms_version->level3
 SET deg_tgt_ora_version->level4 = dm2_rdbms_version->level4
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(deg_tgt_ora_version)
  CALL echorecord(deg_src_ora_version)
 ENDIF
 IF ((deg_tgt_ora_version->level1 >= 9))
  IF ((deg_tgt_ora_version->level1 > deg_src_ora_version->level1))
   SET deg_diff_ora_version = 1
  ELSEIF ((deg_tgt_ora_version->level1 < deg_src_ora_version->level1))
   CALL deg_disp_ora_version_err(null)
   GO TO exit_script
  ELSE
   IF ((deg_tgt_ora_version->level2 > deg_src_ora_version->level2))
    SET deg_diff_ora_version = 1
   ELSEIF ((deg_tgt_ora_version->level2 < deg_src_ora_version->level2))
    CALL deg_disp_ora_version_err(null)
    GO TO exit_script
   ELSE
    IF ((deg_tgt_ora_version->level3 > deg_src_ora_version->level3))
     SET deg_diff_ora_version = 1
    ELSEIF ((deg_tgt_ora_version->level3 < deg_src_ora_version->level3))
     CALL deg_disp_ora_version_err(null)
     GO TO exit_script
    ELSE
     IF ((deg_tgt_ora_version->level4 > deg_src_ora_version->level4))
      SET deg_diff_ora_version = 1
     ELSEIF ((deg_tgt_ora_version->level4 < deg_src_ora_version->level4))
      CALL deg_disp_ora_version_err(null)
      GO TO exit_script
     ELSE
      SET deg_diff_ora_version = 0
     ENDIF
    ENDIF
   ENDIF
  ENDIF
 ELSE
  ...
  SET dm_err->emsg = concat("Target database Oracle version (",deg_tgt_ora_version->version,
   ") has to be 9 and higher.")
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET deg_tgt_db_os = validate(currdbsys,cursys)
 IF (size(deg_tgt_db_os) != 3)
  SET deg_tgt_db_os = substring(1,(findstring(":",deg_tgt_db_os,1,1) - 1),deg_tgt_db_os)
 ENDIF
 IF (deg_tgt_db_os="AXP")
  SET dm_err->err_ind = 1
  ...
  GO TO exit_script
 ENDIF
 IF (dm2_get_dbase_name(deg_tgt_db_name)=0)
  GO TO exit_script
 ENDIF
 IF (deg_get_node_name(deg_tgt_node_name)=0)
  GO TO exit_script
 ENDIF
 IF (deg_get_nls_lang(deg_tgt_nls_lang)=0)
  GO TO exit_script
 ENDIF
 ...
 CALL clear(1,1)
 CALL box(1,1,15,131)
 CALL text(2,5,"Create Export/Import Files")
 CALL text(4,5,"Are you performing a database migration (Y or N):")
 CALL accept(4,60,"P;CU"
 ...
 ELSEIF (curaccept="N")
  SET deg_mig_ind = 0
 ENDIF
 CALL text(6,5,concat("Oracle Home for Oracle ",deg_src_ora_version->version," on node ",
   deg_tgt_node_name," :"))
 CALL accept(6,60,"P(50);C"
  WHERE  NOT (curaccept=" "))
 SET deg_src_ora_home = trim(curaccept)
 CALL deg_fix_loc(deg_src_ora_home)
 SET deg_row = 6
 IF (deg_diff_ora_version=1)
  SET deg_row = (deg_row+ 2)
  CALL text(deg_row,5,concat("Oracle Home for Oracle ",deg_tgt_ora_version->version," on node ",
    deg_tgt_node_name," :"))
  CALL accept(deg_row,60,"P(50);C"
   WHERE  NOT (curaccept=" "))
  SET deg_tgt_ora_home = trim(curaccept)
  CALL deg_fix_loc(deg_tgt_ora_home)
 ELSE
  SET deg_tgt_ora_home = deg_src_ora_home
 ENDIF
 IF (deg_mig_ind=0)
  SET deg_row = (deg_row+ 2)
  CALL text(deg_row,5,"File location on Target : ")
  SET deg_row = (deg_row+ 1)
  CALL text(deg_row,10,
   "*where ksh files should be copied to and the pipe files export/import log files & nohup log files will be created."
   )
  CALL accept((deg_row - 1),50,"P(50);C"," "
   WHERE  NOT (curaccept=" "))
  SET deg_tgt_file_loc = trim(curaccept)
  CALL deg_fix_loc(deg_tgt_file_loc)
  SET deg_src_ksh_loc = "ccluserdir:"
 ELSEIF (deg_mig_ind=1)
  SET deg_tgt_file_loc = "/home/oracle/gg/dirtmp"
  SET deg_src_ksh_loc = "ggdelivery:[dirtmp]"
 ENDIF
 SET deg_row = (deg_row+ 2)
 CALL text(deg_row,5,"Enter 'C' to continue or 'Q' to quit (C or Q): ")
 CALL accept(deg_row,60,"P;CU"," "
  WHERE curaccept IN ("C", "Q"))
 IF (curaccept="Q")
  SET message = nowindow
  GO TO exit_script
 ENDIF
 SET message = nowindow
 IF (dm2_find_dir(deg_src_ksh_loc)=0)
  GO TO exit_script
 ENDIF
 SET message = window
 CALL clear(1,1)
 CALL box(1,1,20,131)
 CALL text(2,55,"Summary Screen")
 SET deg_row = 2
 IF (deg_mig_ind=1)
  SET deg_row = (deg_row+ 1)
  CALL text(deg_row,5,"*This is part of the database migration process.")
 ENDIF
 SET deg_row = (deg_row+ 2)
 CALL text(deg_row,5,"TARGET")
 CALL text(deg_row,60,"SOURCE")
 SET deg_row = (deg_row+ 1)
 CALL text(deg_row,5,concat("Database Name : ",deg_tgt_db_name))
 CALL text(deg_row,60,concat("Database Name : ",deg_src_db_name))
 SET deg_row = (deg_row+ 1)
 CALL text(deg_row,5,concat("Oracle Version : ",deg_tgt_ora_version->version))
 CALL text(deg_row,60,concat("Oracle Version : ",deg_src_ora_version->version))
 SET deg_row = (deg_row+ 2)
 CALL text(deg_row,5,concat("Oracle Home for Oracle ",deg_tgt_ora_version->version," on Target node ",
   deg_tgt_node_name," : ",
   deg_tgt_ora_home))
 IF (deg_diff_ora_version=1)
  SET deg_row = (deg_row+ 2)
  CALL text(deg_row,5,concat("Oracle Home for Oracle ",deg_src_ora_version->version,
    " on Target node ",deg_tgt_node_name," : ",
    deg_src_ora_home))
 ENDIF
 SET deg_row = (deg_row+ 2)
 CALL text(deg_row,5,concat("KSH File Location : ",deg_src_ksh_loc))
 SET deg_row = (deg_row+ 2)
 CALL text(deg_row,5,concat("TARGET File Location : ",deg_tgt_file_loc))
 SET deg_row = (deg_row+ 1)
 IF (deg_mig_ind=1)
  CALL text(deg_row,10,
   "* where the pipe files, export/import log files & nohup log files will be created")
 ELSE
  CALL text(deg_row,10,
   "* where ksh files should be copied to and the pipe files, export/import log files & nohup log files will be created"
   )
 ...
 IF (curaccept="Q")
  GO TO exit_script
 ENDIF
 SET deg_exp_utility_location = concat(deg_src_ora_home,"/bin")
 SET deg_imp_utility_location = concat(deg_tgt_ora_home,"/bin")
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("deg_exp_utility_location =",deg_exp_utility_location))
  CALL echo(concat("deg_imp_utility_location =",deg_imp_utility_location))
 ENDIF
 SET dm2_install_schema->dbase_name = '"SOURCE"'
 SET dm2_install_schema->u_name = "V500"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   deg_data_chunk_size = d.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 SELECT
  ...
  ELSEIF (deg_expimp_spc_tbls=1)
   ...
   PLAN (u)
    ...
    WHERE d.info_domain="DM2_MIG_EXCL"
     AND d.info_name=concat("V500.",u.table_name)
     AND d.info_number=10
     AND  NOT ( EXISTS (
    (SELECT
     "x"
     FROM dm_info x
     WHERE x.info_domain="DM2_MIG_NOACTIVITY_TABLE"
      AND d.info_name=concat("V500.",x.info_name)))))
  ELSEIF (deg_expimp_nondoc_tbls=1)
   FROM user_tables u,
    dm_info d
   PLAN (u)
  ...
  DETAIL
   ...
   deg_table_data->tbl_cnt].row_cnt = u.num_rows
  FOOT REPORT
 ...
 ELSEIF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "No tables found."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determine tables with single column numeric unique indexes."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  utc.table_name, utc.data_type, ui.index_name,
  uic.column_name
  ...
  HEAD REPORT
   tbl_skip = 0, single_col_index = 0, tbl_idx = 0,
   deg_tmp_data_type = ""
  HEAD ui.table_name
   tbl_skip = 0, tbl_idx = 0
   IF (locateval(tbl_idx,1,deg_table_data->tbl_cnt,ui.table_name,deg_table_data->qual[tbl_idx].
    table_name)=0)
    tbl_skip = 1
   ENDIF
  HEAD ui.index_name
  ...
  DETAIL
   IF (tbl_skip=0)
    IF (uic.column_position=1)
     single_col_index = 1, deg_tmp_data_type = utc.data_type
    ELSE
     single_col_index = 0
    ENDIF
   ENDIF
  FOOT  ui.index_name
   IF (single_col_index=1
    AND deg_tmp_data_type IN ("NUMBER", "FLOAT")
    AND tbl_skip=0)
    tbl_skip = 1
    IF ((dm_err->debug_flag > 0))
     CALL echo(concat("Table ",ui.table_name," has single numeric unique index - ",ui.index_name))
    ENDIF
    deg_table_data->qual[tbl_idx].use_index = 1, deg_table_data->qual[tbl_idx].index_column = uic
    .column_name
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 FOR (deg_tbl = 1 TO deg_table_data->tbl_cnt)
   IF ((deg_table_data->qual[deg_tbl].use_index=1))
    SET dm_err->eproc = concat("Getting Max value for ",deg_table_data->qual[deg_tbl].index_column,
     " on table:",deg_table_data->qual[deg_tbl].table_name)
    IF (dm2_push_cmd(concat("select into 'nl:' col_val_max = max(",trim(deg_table_data->qual[deg_tbl]
       .index_column),") from ",trim(deg_table_data->qual[deg_tbl].table_name),
      " detail deg_col_max = col_val_max with nocounter go"),1)=0)
     GO TO exit_script
    ENDIF
    IF ((dm_err->debug_flag > 2))
     CALL echo(build("tbl_name=",deg_table_data->qual[deg_tbl].table_name,",col_name=",deg_table_data
       ->qual[deg_tbl].index_column,",max=",
       deg_col_max))
    ENDIF
    SET deg_table_data->qual[deg_tbl].key_max_value = deg_col_max
   ENDIF
 ENDFOR
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   tbl_idx = 0
   IF (locateval(tbl_idx,1,deg_table_data->tbl_cnt,utc.table_name,deg_table_data->qual[tbl_idx].
    table_name) > 0)
    cnt = (cnt+ 1)
    IF ((dm_err->debug_flag > 0))
     CALL echo(build("tbl_name=",deg_table_data->qual[tbl_idx].table_name,",lob_col_name=",utc
      .column_name))
    ENDIF
 ...
 ENDIF
 FOR (deg_tbl = 1 TO deg_table_data->tbl_cnt)
   IF ((deg_table_data->qual[deg_tbl].lob_ind=1))
    SET deg_par_file_cnt = (deg_par_file_cnt+ 1)
    SET deg_table_data->qual[deg_tbl].par_nbr_from = deg_par_file_cnt
    SET deg_table_data->qual[deg_tbl].par_nbr_to = 0
    SET deg_file_data->cnt = (deg_file_data->cnt+ 1)
    IF (mod(deg_file_data->cnt,100)=1)
     SET stat = alterlist(deg_file_data->qual,(deg_file_data->cnt+ 99))
    ENDIF
    SET deg_file_data->qual[deg_file_data->cnt].tables = deg_table_data->qual[deg_tbl].table_name
    SET deg_file_data->qual[deg_file_data->cnt].par_from_nbr = deg_par_file_cnt
    SET deg_file_data->qual[deg_file_data->cnt].par_to_nbr = 0
    IF ((deg_table_data->qual[deg_tbl].row_cnt > deg_data_chunk_size))
     SET deg_file_data->qual[deg_file_data->cnt].commit_ind = 1
    ENDIF
   ENDIF
 ENDFOR
 SET deg_new_par_file = 1
 SET deg_par_row_cnt = 0.0
 SET deg_par_tbl_cnt = 0
 FOR (deg_tbl = 1 TO deg_table_data->tbl_cnt)
   IF ((deg_table_data->qual[deg_tbl].lob_ind=0))
    IF ((deg_table_data->qual[deg_tbl].row_cnt < deg_data_chunk_size))
     IF (deg_new_par_file=0)
      IF (((((deg_par_row_cnt+ deg_table_data->qual[deg_tbl].row_cnt) > deg_data_chunk_size)) OR (((
      deg_par_tbl_cnt+ 1) > deg_max_tbl_size))) )
       SET deg_new_par_file = 1
      ENDIF
     ENDIF
     IF (deg_new_par_file=1)
      SET deg_par_file_cnt = (deg_par_file_cnt+ 1)
      SET deg_file_data->cnt = (deg_file_data->cnt+ 1)
      IF (mod(deg_file_data->cnt,100)=1)
       SET stat = alterlist(deg_file_data->qual,(deg_file_data->cnt+ 99))
      ENDIF
      SET deg_file_data->qual[deg_file_data->cnt].commit_ind = 0
      SET deg_file_data->qual[deg_file_data->cnt].par_from_nbr = deg_par_file_cnt
      SET deg_file_data->qual[deg_file_data->cnt].par_to_nbr = 0
      SET deg_par_row_cnt = 0.0
      SET deg_new_par_file = 0
      SET deg_par_tbl_cnt = 0
     ENDIF
     SET deg_table_data->qual[deg_tbl].par_nbr_from = deg_par_file_cnt
     SET deg_table_data->qual[deg_tbl].par_nbr_to = 0
     SET deg_par_row_cnt = (deg_par_row_cnt+ deg_table_data->qual[deg_tbl].row_cnt)
     SET deg_par_tbl_cnt = (deg_par_tbl_cnt+ 1)
     SET deg_file_data->qual[deg_file_data->cnt].tables = concat(deg_file_data->qual[deg_file_data->
      cnt].tables,deg_table_data->qual[deg_tbl].table_name,",")
     IF ((dm_err->debug_flag > 0))
      CALL echo(build("par_file_from_nbr =",deg_table_data->qual[deg_tbl].par_nbr_from))
      CALL echo(build("par_file_to_nbr =",deg_table_data->qual[deg_tbl].par_nbr_to))
      CALL echo(build("deg_par_row_cnt =",deg_par_row_cnt))
      CALL echo(build("deg_par_tbl_cnt =",deg_par_tbl_cnt))
     ENDIF
    ELSEIF ((deg_table_data->qual[deg_tbl].row_cnt >= deg_data_chunk_size))
     IF ((((deg_table_data->qual[deg_tbl].use_index=0)) OR ((deg_table_data->qual[deg_tbl].row_cnt=
     deg_data_chunk_size))) )
      SET deg_par_file_cnt = (deg_par_file_cnt+ 1)
      SET deg_table_data->qual[deg_tbl].par_nbr_from = deg_par_file_cnt
      SET deg_table_data->qual[deg_tbl].par_nbr_to = 0
      ...
      SET deg_file_data->qual[deg_file_data->cnt].commit_ind = 1
     ELSE
      SET deg_table_data->qual[deg_tbl].chunk_cnt = ceil((deg_table_data->qual[deg_tbl].key_max_value
       / deg_data_chunk_size))
      SET deg_table_data->qual[deg_tbl].par_nbr_from = (deg_par_file_cnt+ 1)
      SET deg_par_file_cnt = (deg_par_file_cnt+ deg_table_data->qual[deg_tbl].chunk_cnt)
      SET deg_table_data->qual[deg_tbl].par_nbr_to = deg_par_file_cnt
      SET deg_new_par_file = 1
      SET deg_file_data->cnt = (deg_file_data->cnt+ 1)
      IF (mod(deg_file_data->cnt,100)=1)
       SET stat = alterlist(deg_file_data->qual,(deg_file_data->cnt+ 99))
      ENDIF
      SET deg_file_data->qual[deg_file_data->cnt].tables = deg_table_data->qual[deg_tbl].table_name
      SET deg_file_data->qual[deg_file_data->cnt].par_from_nbr = deg_table_data->qual[deg_tbl].
      par_nbr_from
      SET deg_file_data->qual[deg_file_data->cnt].par_to_nbr = deg_table_data->qual[deg_tbl].
      par_nbr_to
      SET deg_file_data->qual[deg_file_data->cnt].commit_ind = 0
      SET deg_file_data->qual[deg_file_data->cnt].tbl_idx = deg_tbl
     ENDIF
    ENDIF
   ENDIF
 ENDFOR
 SET stat = alterlist(deg_file_data->qual,deg_file_data->cnt)
 IF ((dm_err->debug_flag > 4))
  CALL echorecord(deg_table_data)
 ENDIF
 SET dm_err->eproc = "Clean up exm/imp files."
 IF (deg_expimp_spc_tbls=0
  AND deg_expimp_nondoc_tbls=0)
  SET deg_old_file = concat(deg_src_ksh_loc,"dm2_dbei*.*;*")
 ELSEIF (deg_expimp_spc_tbls=1)
  SET deg_old_file = concat(deg_src_ksh_loc,"dm2_sdbei*.*;*")
 ELSEIF (deg_expimp_nondoc_tbls=1)
  SET deg_old_file = concat(deg_src_ksh_loc,"dm2_ndbei*.*;*")
 ENDIF
 ...
 ELSEIF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 IF (((deg_expimp_spc_tbls=1) OR (deg_expimp_nondoc_tbls=1)) )
  SET deg_sql_file = concat(deg_file_prefix,"truncates.sql")
  SET dm_err->eproc = concat("Create sql file ",deg_sql_file)
  CALL disp_msg("",dm_err->logfile,0)
  SELECT INTO value(deg_sql_file)
   FROM (dummyt t  WITH seq = value(deg_table_data->tbl_cnt))
   HEAD REPORT
    col 0, "set pages 0", row + 1,
    col 0, "set feedback off", row + 1,
    col 0, "set termout off", row + 1,
    col 0, "set verify off", row + 1
   DETAIL
    deg_cmd = concat("truncate table ",trim(deg_table_data->qual[t.seq].table_name)," reuse storage;"
     ), col 0, deg_cmd,
    row + 1
   FOOT REPORT
    col 0, "exit"
  ...
  ENDIF
 ENDIF
 FOR (deg_cnt = 1 TO deg_file_data->cnt)
   SET deg_par_table_list->cnt = 0
   SET stat = alterlist(deg_par_table_list->qual,deg_par_table_list->cnt)
   IF ((deg_file_data->qual[deg_cnt].par_from_nbr > 0)
    AND (deg_file_data->qual[deg_cnt].par_to_nbr=0))
    SET deg_par_file_name = build(deg_src_ksh_loc,deg_export_prefix,format(deg_file_data->qual[
      deg_cnt].par_from_nbr,"####;P0"),".par")
    CALL echo(build("deg_par_file_name=",deg_par_file_name))
    SET deg_ksh_name = build(deg_file_prefix,format(deg_file_data->qual[deg_cnt].par_from_nbr,
      "####;P0"),".ksh")
    CALL echo(build("deg_ksh_name=",deg_ksh_name))
    SET deg_pipe_file_name = build(deg_tgt_file_loc,"/",deg_export_prefix,"_",format(deg_file_data->
      qual[deg_cnt].par_from_nbr,"####;P0"))
    SET deg_file_data->qual[deg_cnt].ksh_file_name = build(deg_file_prefix,format(deg_file_data->
      qual[deg_cnt].par_from_nbr,"####;P0"))
    SET deg_file_data->qual[deg_cnt].tables = replace(deg_file_data->qual[deg_cnt].tables,",","",2)
    SET deg_char_cnt = size(deg_file_data->qual[deg_cnt].tables)
    IF (deg_char_cnt > 503)
     WHILE (deg_char_cnt > 503)
       SET deg_end_pos = findstring(",",substring(1,503,deg_file_data->qual[deg_cnt].tables),1,1)
       SET deg_par_table_list->cnt = (deg_par_table_list->cnt+ 1)
       SET stat = alterlist(deg_par_table_list->qual,deg_par_table_list->cnt)
       SET deg_par_table_list->qual[deg_par_table_list->cnt].table_txt = substring(1,deg_end_pos,
        deg_file_data->qual[deg_cnt].tables)
       SET deg_file_data->qual[deg_cnt].tables = substring(deg_end_pos,deg_char_cnt,deg_file_data->
        qual[deg_cnt].tables)
       SET deg_char_cnt = size(deg_file_data->qual[deg_cnt].tables)
     ENDWHILE
     SET deg_par_table_list->cnt = (deg_par_table_list->cnt+ 1)
     SET stat = alterlist(deg_par_table_list->qual,deg_par_table_list->cnt)
     SET deg_par_table_list->qual[deg_par_table_list->cnt].table_txt = concat(deg_file_data->qual[
      deg_cnt].tables,")")
     SET deg_par_table_list->qual[1].table_txt = concat("tables=(",deg_par_table_list->qual[1].
      table_txt)
    ELSE
     SET deg_par_table_list->cnt = (deg_par_table_list->cnt+ 1)
     SET stat = alterlist(deg_par_table_list->qual,deg_par_table_list->cnt)
     SET deg_par_table_list->qual[deg_par_table_list->cnt].table_txt = concat("tables=(",
      deg_file_data->qual[deg_cnt].tables,")")
    ENDIF
    IF (deg_create_par_file(deg_par_file_name,"",deg_expimp_nondoc_tbls)=0)
     GO TO exit_script
    ENDIF
    SET deg_par_file_name = build(deg_tgt_file_loc,"/",deg_export_prefix,format(deg_file_data->qual[
      deg_cnt].par_from_nbr,"####;P0"),".par")
    IF (deg_create_ksh(deg_ksh_name,deg_pipe_file_name,deg_par_file_name,deg_file_data->qual[deg_cnt]
     .commit_ind,deg_file_data->qual[deg_cnt].par_from_nbr,
     0," ")=0)
     GO TO exit_script
    ENDIF
   ELSEIF ((deg_file_data->qual[deg_cnt].par_from_nbr > 0)
    AND (deg_file_data->qual[deg_cnt].par_to_nbr > 0))
    SET deg_ksh_name = build(deg_file_prefix,format(deg_file_data->qual[deg_cnt].par_from_nbr,
      "####;P0"),".ksh")
    SET deg_pipe_file_name = build(deg_tgt_file_loc,"/",deg_export_prefix,"_")
    SET deg_file_data->qual[deg_cnt].ksh_file_name = build(deg_file_prefix,format(deg_file_data->
      qual[deg_cnt].par_from_nbr,"####;P0"))
    SET deg_low_range = 0
    SET deg_high_range = 0
    FOR (deg_par_nbr = deg_file_data->qual[deg_cnt].par_from_nbr TO deg_file_data->qual[deg_cnt].
    par_to_nbr)
      SET deg_par_file_name = build(deg_src_ksh_loc,deg_export_prefix,format(deg_par_nbr,"####;P0"),
       ".par")
      SET deg_par_table_list->cnt = 1
      SET stat = alterlist(deg_par_table_list->qual,deg_par_table_list->cnt)
      SET deg_par_table_list->qual[deg_par_table_list->cnt].table_txt = concat("tables=(",
       deg_file_data->qual[deg_cnt].tables,")")
      SET deg_low_range = deg_high_range
      IF ((deg_par_nbr=deg_file_data->qual[deg_cnt].par_to_nbr))
       SET deg_query = concat('"where ',deg_table_data->qual[deg_file_data->qual[deg_cnt].tbl_idx].
        index_column," > ",trim(cnvtstring(deg_low_range)),'"')
      ELSE
       SET deg_high_range = (deg_high_range+ deg_data_chunk_size)
       IF (deg_low_range=0)
        SET deg_query = concat('"where ',deg_table_data->qual[deg_file_data->qual[deg_cnt].tbl_idx].
         index_column," between ",trim(cnvtstring(deg_low_range))," and ",
         trim(cnvtstring(deg_high_range)),'"')
       ELSE
        SET deg_query = concat('"where ',deg_table_data->qual[deg_file_data->qual[deg_cnt].tbl_idx].
         index_column," between ",trim(cnvtstring((deg_low_range+ 1)))," and ",
         trim(cnvtstring(deg_high_range)),'"')
       ENDIF
      ENDIF
      IF (deg_create_par_file(deg_par_file_name,deg_query,deg_expimp_nondoc_tbls)=0)
       GO TO exit_script
      ENDIF
    ENDFOR
    IF (deg_create_ksh(deg_ksh_name,deg_pipe_file_name,deg_par_file_name,deg_file_data->qual[deg_cnt]
     .commit_ind,deg_file_data->qual[deg_cnt].par_from_nbr,
     deg_file_data->qual[deg_cnt].par_to_nbr,deg_file_data->qual[deg_cnt].tables)=0)
     GO TO exit_script
    ENDIF
   ENDIF
 ENDFOR
 IF ((dm_err->debug_flag > 4))
  CALL echorecord(deg_file_data)
 ENDIF
 CALL clear(1,1)
 CALL box(1,1,20,131)
 CALL text(2,50,"Final Report")
 SET deg_row = 2
 IF (deg_mig_ind=0)
  SET deg_row = (deg_row+ 2)
  CALL text(deg_row,2,concat("Move the following files from ",deg_src_ksh_loc,
    " on the SOURCE node to ",deg_tgt_file_loc," on the TARGET node ",
    deg_tgt_node_name," : "))
  SET deg_row = (deg_row+ 1)
  CALL text(deg_row,8,build(deg_file_prefix,"*.*"))
  SET deg_row = (deg_row+ 2)
  CALL text(deg_row,2,concat(
    "Once files are moved from SOURCE to TARGET, execute the following on the TARGET node ",
    deg_tgt_node_name," : "))
  SET deg_row = (deg_row+ 2)
  CALL text(deg_row,5,"Change permissions for those files moved to TARGET:")
  SET deg_row = (deg_row+ 1)
  CALL text(deg_row,10,concat("chmod 775 ",deg_tgt_file_loc,"/",deg_file_prefix,"*.*"))
 ENDIF
 IF (((deg_expimp_spc_tbls=1) OR (deg_expimp_nondoc_tbls=1)) )
  SET deg_row = (deg_row+ 2)
  CALL text(deg_row,5,concat("Execute the following sql file on TARGET node ",deg_tgt_node_name,
    " to clear any data loaded from initial export/import:"))
  SET deg_row = (deg_row+ 1)
  CALL text(deg_row,10,concat("export ORACLE_HOME='",deg_tgt_ora_home,"'"))
  SET deg_row = (deg_row+ 1)
  CALL text(deg_row,10,concat(deg_imp_utility_location,"/sqlplus -s v500/",dm2_install_schema->
    v500_p_word,"@",dm2_install_schema->v500_connect_str,
    " @",deg_tgt_file_loc,"/",deg_sql_file))
 ENDIF
 SET deg_row = (deg_row+ 2)
 CALL text(deg_row,5,concat("Execute the ksh files with the following prefix from TARGET node ",
   deg_tgt_node_name,":"))
 SET deg_row = (deg_row+ 1)
 CALL text(deg_row,10,concat(deg_file_prefix,"*.ksh"))
 SET deg_row = (deg_row+ 2)
 CALL text(deg_row,5,"Enter 'C' to complete this process (C): ")
 CALL accept(deg_row,60,"P;CU","C"
 ...
 SUBROUTINE deg_get_nls_lang(dgn_nls_lang)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE deg_disp_ora_version_err(null)
   SET dm_err->err_ind = 1
   SET dm_err->eproc = "Compare Source oracle version with Target oracle version."
   SET dm_err->emsg = concat("Target oracle version ",deg_tgt_ora_version->version,
    " can not be lower than Source oracle version ",deg_src_ora_version->version)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 END ;Subroutine
 SUBROUTINE deg_fix_loc(dfl_file_loc)
   SET dfl_loc_length = size(dfl_file_loc)
   IF (substring(dfl_loc_length,1,dfl_file_loc)="/")
    SET dfl_file_loc = replace(dfl_file_loc,"/","",2)
   ENDIF
   IF ((dm_err->debug_flag > 0))
    CALL clear(24,1,131)
    CALL text(24,1,concat("file_loc =",dfl_file_loc))
    CALL pause(2)
   ENDIF
 END ;Subroutine
 SUBROUTINE deg_create_par_file(dcp_file_name,dcp_query,dcp_mode)
   SET dm_err->eproc = concat("Create par file ",dcp_file_name)
   CALL disp_msg("",dm_err->logfile,0)
   DECLARE dcp_list = i4 WITH protect, noconstant(0)
   SELECT INTO value(dcp_file_name)
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     FOR (dcp_list = 1 TO deg_par_table_list->cnt)
       col + 0, deg_par_table_list->qual[dcp_list].table_txt, row + 1
     ENDFOR
     col 0, "statistics=none", row + 1,
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE deg_create_ksh(dck_ksh_name,dck_pipe_file,dck_par_file,dck_commit_ind,dck_from_nbr,
  dck_to_nbr,dck_table_name)
   SET dm_err->eproc = concat("Create ksh file :",dck_ksh_name)
   CALL disp_msg("",dm_err->logfile,0)
   ...
   SET dck_err_logfile = replace(dck_ksh_name,"ksh","log",2)
   SET dck_ksh_name = build(deg_src_ksh_loc,dck_ksh_name)
   SELECT INTO value(dck_ksh_name)
    FROM (dummyt d  WITH seq = 1)
    DETAIL
     col 0, "#!/bin/ksh", row + 1,
     deg_line = concat("if [[ ! -d ",deg_tgt_file_loc," ]]"), col 0, deg_line,
     row + 1, deg_line = "then", col 0,
     deg_line, row + 1, deg_line = concat('   echo "CER-0000:error - directory ',deg_tgt_file_loc,
      ' not found" >>',dck_err_logfile),
     col 0, deg_line, row + 1,
     col 0, "   exit 1", row + 1,
     col 0, "fi", row + 2,
     deg_line = concat("if [[ ! -d ",deg_src_ora_home," ]]"), col 0, deg_line,
     row + 1, deg_line = "then", col 0,
     deg_line, row + 1, deg_line = concat('   echo "CER-0000:error - directory ',deg_src_ora_home,
      ' not found" >>',dck_err_logfile),
     col 0, deg_line, row + 1,
     col 0, "   exit 1", row + 1,
     col 0, "fi", row + 2
     IF (deg_diff_ora_version=1)
      deg_line = concat("if [[ ! -d ",deg_tgt_ora_home," ]]"), col 0, deg_line,
      row + 1, deg_line = "then", col 0,
      deg_line, row + 1, deg_line = concat('   echo "CER-0000:error - directory ',deg_tgt_ora_home,
       ' not found" >>',dck_err_logfile),
      col 0, deg_line, row + 1,
      col 0, "   exit 1", row + 1,
      col 0, "fi", row + 2
     ENDIF
     IF (dck_to_nbr=0)
      deg_line = concat("export ORACLE_HOME='",deg_src_ora_home,"'"), col 0, deg_line,
      row + 1, deg_line = concat("export ORACLE_SID='",dm2_install_schema->src_v500_connect_str,"'"),
      col 0,
      deg_line, row + 1, deg_line = concat("export NLS_LANG='",deg_src_nls_lang,"'"),
      col 0, deg_line, row + 1,
      deg_line = concat("if [[ ! -f ",trim(dck_par_file)," ]]"), col 0, deg_line,
      row + 1, deg_line = "then", col 0,
      deg_line, row + 1, deg_line = concat('   echo "CER-0000:error - ',trim(dck_par_file),
       ' not found" >>',dck_err_logfile),
      col 0, deg_line, row + 1,
      col 0, "   exit 1", row + 1,
      col 0, "fi", row + 2,
      dck_exp_logfile = concat(deg_tgt_file_loc,"/",deg_export_prefix,format(dck_from_nbr,"####;P0"),
       ".log"), deg_line = concat("date >> ",dck_exp_logfile), col 0,
      deg_line, row + 1, deg_line = concat("mknod ",dck_pipe_file," p"),
      col 0, deg_line, row + 1,
      deg_line = concat("nohup ",deg_exp_utility_location,"/exp v500/",dm2_install_schema->
       src_v500_p_word,"@",
       dm2_install_schema->src_v500_connect_str," log=",dck_exp_logfile," file=",dck_pipe_file,
       " parfile=",dck_par_file," &"), col 0, deg_line,
      row + 1, deg_line = concat("date >> ",dck_exp_logfile), col 0,
      deg_line, row + 1, deg_line = concat("export ORACLE_HOME='",deg_tgt_ora_home,"'"),
      col 0, deg_line, row + 1,
      deg_line = concat("export ORACLE_SID='",dm2_install_schema->v500_connect_str,"'"), col 0,
      deg_line,
      row + 1, deg_line = concat("export NLS_LANG='",deg_tgt_nls_lang,"'"), col 0,
      deg_line, row + 1, dck_imp_logfile = concat(deg_tgt_file_loc,"/",deg_import_prefix,format(
        dck_from_nbr,"####;P0"),".log"),
      deg_line = concat("date >> ",dck_imp_logfile), col 0, deg_line,
      row + 1, deg_line = concat(deg_imp_utility_location,"/imp v500/",dm2_install_schema->
       v500_p_word,"@",dm2_install_schema->v500_connect_str,
       " log=",dck_imp_logfile," file=",dck_pipe_file," ignore=y")
      IF (dck_commit_ind=0)
       deg_line = concat(deg_line," commit=n")
      ELSE
       deg_line = concat(deg_line," commit=y")
      ENDIF
      deg_line = concat(deg_line,
       " buffer=1000000 fromuser=v500 touser=v500 resumable=y resumable_name='Import",trim(cnvtstring
        (dck_from_nbr)),"'  resumable_timeout= 28800"), col 0, deg_line,
      row + 1, deg_line = concat("rm ",dck_pipe_file), col 0,
      deg_line, row + 1, deg_line = concat("date >> ",dck_imp_logfile),
      col 0, deg_line, row + 1
     ELSE
      FOR (dck_par_nbr = dck_from_nbr TO dck_to_nbr)
        deg_line = concat("export ORACLE_HOME='",deg_src_ora_home,"'"), col 0, deg_line,
        row + 1, deg_line = concat("export ORACLE_SID='",dm2_install_schema->src_v500_connect_str,"'"
         ), col 0,
        deg_line, row + 1, deg_line = concat("export NLS_LANG='",deg_src_nls_lang,"'"),
        col 0, deg_line, row + 1,
        dck_par_file = concat(deg_tgt_file_loc,"/",deg_export_prefix,trim(cnvtstring(dck_par_nbr)),
         ".par"), deg_line = concat("if [[ ! -f ",trim(dck_par_file)," ]]"), col 0,
        deg_line, row + 1, deg_line = "then",
        col 0, deg_line, row + 1,
        deg_line = concat('   echo "CER-0000:error - ',trim(dck_par_file),' not found" >>',
         dck_err_logfile), col 0, deg_line,
        row + 1, col 0, "   exit 1",
        row + 1, col 0, "fi",
        row + 2, dck_exp_logfile = concat(deg_tgt_file_loc,"/",deg_export_prefix,format(dck_par_nbr,
          "####;P0"),".log"), deg_line = concat("date >> ",dck_exp_logfile),
        col 0, deg_line, row + 1,
        deg_line = concat("mknod ",dck_pipe_file,trim(cnvtstring(dck_par_nbr))," p"), col 0, deg_line,
        row + 1, deg_line = concat("nohup ",deg_exp_utility_location,"/exp v500/",dm2_install_schema
         ->src_v500_p_word,"@",
         dm2_install_schema->src_v500_connect_str," log=",dck_exp_logfile," file=",dck_pipe_file,
         format(dck_par_nbr,"####;P0")," parfile=",dck_par_file," &"), col 0,
        deg_line, row + 1, deg_line = concat("date >> ",dck_exp_logfile),
        col 0, deg_line, row + 1,
        deg_line = concat("export ORACLE_HOME='",deg_tgt_ora_home,"'"), col 0, deg_line,
        row + 1, deg_line = concat("export ORACLE_SID='",dm2_install_schema->v500_connect_str,"'"),
        col 0,
        deg_line, row + 1, deg_line = concat("export NLS_LANG='",deg_tgt_nls_lang,"'"),
        col 0, deg_line, row + 1,
        dck_imp_logfile = concat(deg_tgt_file_loc,"/",deg_import_prefix,format(dck_par_nbr,"####;P0"),
         ".log"), deg_line = concat("date >> ",dck_imp_logfile), col 0,
        deg_line, row + 1, deg_line = concat(deg_imp_utility_location,"/imp v500/",dm2_install_schema
         ->v500_p_word,"@",dm2_install_schema->v500_connect_str,
         " log=",dck_imp_logfile," file=",dck_pipe_file,format(dck_par_nbr,"####;P0"),
         " ignore=y ",
         "commit=n buffer=1000000 fromuser=v500 touser=v500 resumable=y resumable_name='Import_",
         dck_table_name,trim(cnvtstring(dck_par_nbr)),"'  resumable_timeout= 28800"),
        col 0, deg_line, row + 1,
        deg_line = concat("rm ",dck_pipe_file,trim(cnvtstring(dck_par_nbr))), col 0, deg_line,
...
#exit_script
 SET message = nowindow
 FREE SET deg_expimp_spc_tbls
 FREE SET deg_expimp_nondoc_tbls
 ...
 CALL echo("**************************************************************************")
 CALL echo("*")
 SET dm_err->eproc = "Ending DM2_EXPIMP_GENERATOR"
 CALL final_disp_msg("dm2_expimp_generator")
END GO
