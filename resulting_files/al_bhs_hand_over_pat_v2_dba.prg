CREATE PROGRAM al_bhs_hand_over_pat_v2:dba
 PROMPT
 ...
 DECLARE ml_etype_idx = i4 WITH protect, noconstant(0)
 ...
 FOR (i = 1 TO num_arguments)
   ...
   parent_entity_id
   IF (uar_get_code_meaning(request->patient_list_type_cd)="PROVIDERGRP")
    IF ((request->arguments[i].argument_name="encntr_type"))
     SET ml_etype_idx += 1
     SET stat = alterlist(request->encntr_type_filters,ml_etype_idx)
     SET request->encntr_type_filters[ml_etype_idx].encntr_type_cd = request->arguments[i].
     parent_entity_id
    ENDIF
    IF (trim(request->arguments[i].argument_name)="provider_group_id")
     SET request->arguments[i].argument_name = "prsnl_group_id"
    ENDIF
    IF (trim(request->arguments[i].argument_name)="disch_mins"
     AND trim(request->arguments[i].argument_value)="-1")
     SET request->arguments[i].argument_name = "patient_status_flag"
     SET request->arguments[i].argument_value = "3"
    ENDIF
   ENDIF
 ENDFOR
 IF (((uar_get_code_meaning(request->patient_list_type_cd) != "PROVIDERGRP") OR (size(dgapl_reply->
  patient_lists[selection].encntr_type_filters,5) > 0)) )
