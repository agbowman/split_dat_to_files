CREATE PROGRAM dcp_discontinue_phases:dba
 SET modify = predeclare
 DECLARE s_script_name = vc WITH protect, constant("dcp_discontinue_phases")
 DECLARE l_phase_count = i4 WITH protect, constant(value(size(request->phases,5)))
 DECLARE phase_status_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",16769,"DISCONTINUED"))
 DECLARE phase_action_cd = f8 WITH protect, constant(uar_get_code_by("MEANING",16809,"DISCONTINUE"))
 ...
 SELECT INTO "nl:"
  ...
  HEAD pw.pathway_id
   ...
   IF (idx > 0)
    ...
    phase_data->phases[lphaseactioncount].new.dc_reason_cd = request->phases[idx].dc_reason_cd,
    phase_data->phases[lphaseactioncount].old.pw_status_cd = pw.pw_status_cd, phase_data->phases[
    lphaseactioncount].old.action_type_cd = phase_action_cd,
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(phase_data->phases_count)),
  ...
  SET pw.pw_status_cd = phase_status_cd, pw.status_dt_tm = cnvtdatetime(dq8_current_date_time), pw
   ...
   pw.status_prsnl_id = reqinfo->updt_id, pw.calc_end_dt_tm = cnvtdatetime(dq8_current_date_time), pw
   .calc_end_tz = phase_data->phases[d.seq].new.encntr_tz,
   pw.calc_end_estimated_ind = 0, pw.discontinued_dt_tm = cnvtdatetime(dq8_current_date_time), pw
   .discontinued_tz = phase_data->phases[d.seq].new.encntr_tz,
   pw.discontinued_ind = 1, pw.dc_reason_cd = phase_data->phases[d.seq].new.dc_reason_cd, pw
   .last_action_seq = phase_data->phases[d.seq].pw_action_seq,
   pw.updt_dt_tm = cnvtdatetime(dq8_current_date_time), pw.updt_id = reqinfo->updt_id, pw.updt_task
    = reqinfo->updt_task,
   pw.updt_cnt = phase_data->phases[d.seq].updt_cnt, pw.updt_applctx = reqinfo->updt_applctx
  PLAN (d)
   JOIN (pw
