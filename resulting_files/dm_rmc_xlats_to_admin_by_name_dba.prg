CREATE PROGRAM dm_rmc_xlats_to_admin_by_name:dba
 ...
 SUBROUTINE xta_dbl_hlv(i_gap)
   ...
   RETURN(xdh_gap)
 END ;Subroutine
 IF (check_logfile("DM_RMC_XTA_NM",".log","DM_RMC_XTA_NM LogFile")=0)
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (validate(request->chain_name,"-99")="-99")
  ...
  SET request->chain_name = trim(cnvtupper( $1),3)
  SET request->source_env_id =  $2
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 FREE RECORD xta_request
 RECORD xta_request(
   1 src_env_id1 = f8
   1 src_env_id2 = f8
   1 trg_env_id = f8
 ...
 DECLARE xtan_return = vc
 SET xtan_return = dm_check_syn(null)
 IF (xtan_return="F")
  SET dm_err->eproc = "Error occured while checking synonyms"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 SET xta_request->src_env_id1 = request->source_env_id
 SET xta_request->src_env_id2 = 0
 SET xta_request->chain_seq = 0
 SET xta_request->chain_id = get_chain_id(request->chain_name)
 IF ((xta_request->chain_id <= 0))
  SET dm_err->eproc = "Error occured while trying to obtain chain_id"
  CALL disp_msg(" ",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   xta_request->trg_env_id = a.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ...
 IF (curqual=0)
  ...
  GO TO exit_program
 ENDIF
 EXECUTE dm_rmc_xlats_to_admin  WITH replace("REQUEST","XTA_REQUEST")
#exit_program
 FREE RECORD xta_request
END GO
