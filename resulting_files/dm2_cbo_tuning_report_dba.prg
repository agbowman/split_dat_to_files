CREATE PROGRAM dm2_cbo_tuning_report:dba
 ...
 SUBROUTINE dcr_get_opt_mode_by_name(opt_name)
   RETURN(evaluate(opt_name,"RULE",1,"CHOOSE",2,
    ...
    "ALL_ROWS",8,0))
 END ;Subroutine
 IF (check_logfile("dm2_cbo_tuning_rpt",".log","DM2_CBO_TUNING_REPORT LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_cbo_tuning_report"
 CALL disp_msg("",dm_err->logfile,0)
 SET dm_err->eproc = "Selecting tuned status data from dm_sql_perf"
 SELECT INTO mine
  dsp.tgt_opt_mode, dsp.tuning_status, counts = count(dsp.dm_sql_id)
  ...
  GROUP BY dsp.tgt_opt_mode, dsp.tuning_status
  ORDER BY dsp.tuning_status, dsp.tgt_opt_mode
  HEAD REPORT
   found = 0,
   CALL print("CBO Reports            Tuned Status by Best Optimizer Mode Summary"), row + 1,
   CALL print(fillstring(131,"-")), row + 2, col 0,
   "Optimizer Mode", col 15, "Tuning Status",
   col 30, "Count", row + 1,
   col 0, "--------------", col 15,
   "-------------", col 30, "-----"
  DETAIL
   found = 1, row + 1, col 0,
   CALL print(dcr_get_opt_mode_by_num(dsp.tgt_opt_mode)), col 15, dsp.tuning_status";L",
   col 30, counts";L"
  FOOT REPORT
   IF (found=0)
 ...
 GO TO exit_script
#exit_script
 SET dm_err->eproc = "Ending dm2_cbo_tuning_report"
 CALL final_disp_msg("dm2_cbo_tuning_rpt")
