CREATE PROGRAM ce_result_set_link_upd:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 SET error_code = 0
 UPDATE  FROM ce_result_set_link t,
  ...
  SET t.event_id = evaluate2(
    ...
    ENDIF
    ), t.result_set_id = evaluate2(
    IF ((request->lst[d.seq].result_set_id=- (1))) 0
    ELSE request->lst[d.seq].result_set_id
    ENDIF
    ), t.entry_type_cd = evaluate2(
    IF ((request->lst[d.seq].entry_type_cd=- (1))) 0
    ELSE request->lst[d.seq].entry_type_cd
    ENDIF
    ),
   t.relation_type_cd = evaluate2(
    IF ((request->lst[d.seq].relation_type_cd=- (1))) 0
    ELSE request->lst[d.seq].relation_type_cd
    ENDIF
  ...
  PLAN (d)
   ...
   WHERE (t.event_id=request->lst[d.seq].event_id)
    AND (t.result_set_id=request->lst[d.seq].result_set_id)
