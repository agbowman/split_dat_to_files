CREATE PROGRAM edw_doc_response_orphan:dba
 ...
 SET ds_item_cnt = 0
 SET ret = loadorphans("DOCRESP",trim(v_lregview_db))
 FOR (i = 1 TO size(reply_orphans->qual,5))
   SET ds_item_cnt = (ds_item_cnt+ 1)
   IF (mod(ds_item_cnt,100)=1)
    SET stat = alterlist(doc_response_keys->qual,(ds_item_cnt+ 99))
   ENDIF
   SET doc_response_keys->qual[ds_item_cnt].doc_activity_sk = reply_orphans->qual[i].orphan_key
 ENDFOR
 SELECT INTO "nl:"
  enc_nk = parser(parser_line)
  FROM (dummyt d  WITH seq = value(ds_item_cnt)),
  ...
  PLAN (d
   WHERE ds_item_cnt > 0)
   JOIN (da
   WHERE (da.dcp_forms_activity_id=doc_response_keys->qual[d.seq].doc_activity_sk))
   JOIN (dac
  ...
  HEAD ce1.event_id
   doc_response_keys->qual[d.seq].dcp_section_instance_id = dcr.dcp_section_instance_id,
   doc_response_keys->qual[d.seq].form_event_id = ce.event_id, doc_response_keys->qual[d.seq].
   encounter_nk = enc_nk,
   doc_response_keys->qual[d.seq].loc_facility_cd = encounter.loc_facility_cd, doc_response_keys->
   qual[d.seq].doc_activity_sk = da.dcp_forms_activity_id, doc_response_keys->qual[d.seq].
   component_ref = dac.component_cd
   IF (ce.authentic_flag != 1
    AND ce.result_status_cd IN (inerror_cd, in_error_cd))
    doc_response_keys->qual[d.seq].active_ind = 0
   ELSE
    doc_response_keys->qual[d.seq].active_ind = 1
   ENDIF
   doc_response_keys->qual[d.seq].dcp_form_instance_id = dfr.dcp_form_instance_id, doc_response_keys
   ->qual[d.seq].ce1_event_id = ce1.event_id
  FOOT REPORT
