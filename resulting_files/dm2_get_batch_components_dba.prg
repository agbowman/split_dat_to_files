CREATE PROGRAM dm2_get_batch_components:dba
 ...
 DECLARE cleanup_batch(sbr_batch_id=i4) = i2
 DECLARE dgbc_eid = i4 WITH public, noconstant(0)
 DECLARE dgbc_plan_id = i4 WITH public, noconstant(0)
 DECLARE dgbc_package_int = i4 WITH public, noconstant(0)
 DECLARE dgbc_tab_cnt = i4 WITH public, noconstant(0)
 DECLARE dgbc_cvs_cnt = i4 WITH public, noconstant(0)
 DECLARE dgbc_app_cnt = i4 WITH public, noconstant(0)
 DECLARE dgbc_tsk_cnt = i4 WITH public, noconstant(0)
 DECLARE dgbc_req_cnt = i4 WITH public, noconstant(0)
 DECLARE dgbc_atr_cnt = i4 WITH public, noconstant(0)
 DECLARE dgbc_trr_cnt = i4 WITH public, noconstant(0)
 DECLARE dgbc_rdm_cnt = i4 WITH public, noconstant(0)
 DECLARE dgbc_prg_cnt = i4 WITH protect, noconstant(0)
 IF (( $1 <= 0))
  SET dm_err->eproc = "Validating Install Plan ID."
  SET dm_err->emsg = "Operation failed. Install Plan ID was not valid."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ELSE
  SET dgbc_plan_id =  $1
  SET dgbc_package_int = (dgbc_plan_id * - (1))
 ENDIF
 IF (cleanup_batch(dgbc_package_int)=0)
  GO TO exit_program
 ENDIF
 SELECT INTO "nl:"
  d.info_number
  ...
  WHERE d.info_domain="DATA MANAGEMENT"
   AND d.info_name="DM_ENV_ID"
  DETAIL
   dgbc_eid = d.info_number
  WITH nocounter
 ;end select
 IF (check_error("Getting environment ID.")=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
 ENDIF
 IF ( NOT (dgbc_eid))
  SET dm_err->eproc = "Validating environment ID."
  SET dm_err->emsg = "Operation failed. Environment ID was not valid."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echo("After initialization.")
  CALL trace(7)
 ENDIF
 IF ((dm_err->debug_flag > 0))
  SET dm_err->eproc = build("dgbc_plan_id=",dgbc_plan_id,";dgbc_package_int=",dgbc_package_int,
   ";dgbc_eid=",
   dgbc_eid)
  CALL disp_msg(" ",dm_err->logfile,0)
  IF ((dm_err->err_ind=1))
 ...
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echo("Before schema.")
  CALL trace(7)
 ENDIF
 SELECT
  IF ((dm2_db_options->dm2_toolset_usage="Y"))
   ORDER BY dat.table_name, dat.schema_instance, dat.alpha_feature_nbr
  ELSE
   ORDER BY dat.table_name, dat.schema_date, dat.alpha_feature_nbr
  ENDIF
  INTO "nl:"
  dat.table_name, dat.alpha_feature_nbr
  FROM dm_afd_tables dat,
   dm_install_plan dip
  WHERE dip.install_plan_id=dgbc_plan_id
   AND dip.package_number=dat.alpha_feature_nbr
   AND dat.owner=currdbuser
  HEAD REPORT
   tabcnt = 0, stat = alterlist(dgbc_data->dat_qual,100)
  HEAD dat.table_name
   tabcnt = (tabcnt+ 1)
   IF (mod(tabcnt,100)=1
    AND tabcnt != 1)
    stat = alterlist(dgbc_data->dat_qual,(tabcnt+ 99))
   ENDIF
  FOOT  dat.table_name
   dgbc_data->dat_qual[tabcnt].owner = dat.owner, dgbc_data->dat_qual[tabcnt].table_name = dat
   .table_name, dgbc_data->dat_qual[tabcnt].alpha_feature_nbr = dat.alpha_feature_nbr
   IF ((dm_err->debug_flag > 0))
    IF ((dm2_db_options->dm2_toolset_usage="Y"))
     CALL echo(build("table name: [",dat.table_name,"] schema instance: [",dat.schema_instance,
      "] package number: [",
      dat.alpha_feature_nbr,"]"))
    ELSE
     CALL echo(build("table name: [",dat.table_name,"] schema date: [",dat.schema_date,
      "] package number: [",
      dat.alpha_feature_nbr,"]"))
    ENDIF
   ENDIF
  FOOT REPORT
   stat = alterlist(dgbc_data->dat_qual,tabcnt)
  WITH nocounter
 ;end select
 IF (check_error("Installation Failed. Error while getting the latest tables for the plan.")=1)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  GO TO exit_program
 ENDIF
 FOR (dgbc_tab_cnt = 1 TO size(dgbc_data->dat_qual,5))
   INSERT  FROM dm_afd_tables
    (table_name, tablespace_name, pct_increase,
    pct_used, pct_free, alpha_feature_nbr,
    schema_date, schema_instance, feature_number,
    updt_dt_tm, updt_id, updt_task,
    updt_cnt, updt_applctx, owner)(SELECT
     dat.table_name, dat.tablespace_name, dat.pct_increase,
     dat.pct_used, dat.pct_free, dgbc_package_int,
     dat.schema_date, dat.schema_instance, dat.feature_number,
     cnvtdatetime(curdate,curtime3), 0, 0,
     0, 0, dat.owner
     FROM dm_afd_tables dat
     WHERE (dat.owner=dgbc_data->dat_qual[dgbc_tab_cnt].owner)
      AND (dat.table_name=dgbc_data->dat_qual[dgbc_tab_cnt].table_name)
      AND (dat.alpha_feature_nbr=dgbc_data->dat_qual[dgbc_tab_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error("Installation Failed. Error while adding the latest tables to the plan schema.")=1
   )
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ...
   INSERT  FROM dm_afd_columns
    ...
    data_default, updt_dt_tm, updt_id,
    updt_task, updt_cnt, updt_applctx,
    owner, virtual_column)(SELECT
     dac.table_name, dgbc_package_int, dac.column_name,
     dac.column_seq, dac.data_type, dac.data_length,
     dac.data_precision, dac.data_scale, dac.nullable,
     dac.data_default, cnvtdatetime(curdate,curtime3), 0,
     0, 0, 0,
     dac.owner, dac.virtual_column
     FROM dm_afd_columns dac
     WHERE (dac.owner=dgbc_data->dat_qual[dgbc_tab_cnt].owner)
      AND (dac.table_name=dgbc_data->dat_qual[dgbc_tab_cnt].table_name)
      AND (dac.alpha_feature_nbr=dgbc_data->dat_qual[dgbc_tab_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error("Installation Failed. Error while adding the latest columns to the plan schema.")=
   1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ...
   INSERT  FROM dm_afd_indexes
    ...
    unique_ind, full_ind_name, updt_dt_tm,
    updt_id, updt_task, updt_cnt,
    updt_applctx, owner, index_type)(SELECT
     dai.index_name, dgbc_package_int, dai.table_name,
     dai.tablespace_name, dai.pct_increase, dai.pct_free,
     dai.unique_ind, dai.full_ind_name, cnvtdatetime(curdate,curtime3),
     0, 0, 0,
     0, dai.owner, dai.index_type
     FROM dm_afd_indexes dai
     WHERE (dai.owner=dgbc_data->dat_qual[dgbc_tab_cnt].owner)
      AND (dai.table_name=dgbc_data->dat_qual[dgbc_tab_cnt].table_name)
      AND (dai.alpha_feature_nbr=dgbc_data->dat_qual[dgbc_tab_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error("Installation Failed. Error while adding the latest indexes to the plan schema.")=
   1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ...
   INSERT  FROM dm_afd_index_columns
    ...
    column_name, column_position, updt_dt_tm,
    updt_id, updt_task, updt_cnt,
    updt_applctx, owner)(SELECT
     dic.index_name, dic.table_name, dgbc_package_int,
     dic.column_name, dic.column_position, cnvtdatetime(curdate,curtime3),
     0, 0, 0,
     0, dic.owner
     FROM dm_afd_index_columns dic
     WHERE (dic.owner=dgbc_data->dat_qual[dgbc_tab_cnt].owner)
      AND (dic.table_name=dgbc_data->dat_qual[dgbc_tab_cnt].table_name)
      AND (dic.alpha_feature_nbr=dgbc_data->dat_qual[dgbc_tab_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest index columns to the plan schema.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
   ...
   INSERT  FROM dm_afd_constraints
    ...
    parent_table_columns, r_constraint_name, full_cons_name,
    updt_dt_tm, updt_id, updt_task,
    updt_cnt, updt_applctx, owner,
    delete_rule)(SELECT
     dcon.table_name, dgbc_package_int, dcon.constraint_name,
     dcon.constraint_type, dcon.parent_table_name, dcon.status_ind,
     dcon.parent_table_columns, dcon.r_constraint_name, dcon.full_cons_name,
     cnvtdatetime(curdate,curtime3), 0, 0,
     0, 0, dcon.owner,
     dcon.delete_rule
     FROM dm_afd_constraints dcon
     WHERE (dcon.owner=dgbc_data->dat_qual[dgbc_tab_cnt].owner)
      AND (dcon.table_name=dgbc_data->dat_qual[dgbc_tab_cnt].table_name)
      AND (dcon.alpha_feature_nbr=dgbc_data->dat_qual[dgbc_tab_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest constraints to the plan schema.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
   ...
   INSERT  FROM dm_afd_cons_columns
    (table_name, alpha_feature_nbr, constraint_name,
    column_name, position, updt_dt_tm,
    updt_id, updt_task, updt_cnt,
    updt_applctx, owner)(SELECT
     dacc.table_name, dgbc_package_int, dacc.constraint_name,
     dacc.column_name, dacc.position, cnvtdatetime(curdate,curtime3),
     0, 0, 0,
     0, dacc.owner
     FROM dm_afd_cons_columns dacc
     WHERE (dacc.owner=dgbc_data->dat_qual[dgbc_tab_cnt].owner)
      AND (dacc.table_name=dgbc_data->dat_qual[dgbc_tab_cnt].table_name)
      AND (dacc.alpha_feature_nbr=dgbc_data->dat_qual[dgbc_tab_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest constraint columns to the plan schema.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dm_afd_part_objects
    (owner, table_name, object_type,
    object_name, alpha_feature_nbr, schema_instance,
    partitioning_type, subpartitioning_type, partition_count,
    subpartition_count, interval, autolist,
    indexing_off_ind, row_mvmnt_ind, locality,
    partial_ind, updt_id, updt_dt_tm,
    updt_task, updt_applctx, updt_cnt)(SELECT
     dapo.owner, dapo.table_name, dapo.object_type,
     dapo.object_name, dgbc_package_int, dapo.schema_instance,
     dapo.partitioning_type, dapo.subpartitioning_type, dapo.partition_count,
     dapo.subpartition_count, dapo.interval, dapo.autolist,
     dapo.indexing_off_ind, dapo.row_mvmnt_ind, dapo.locality,
     dapo.partial_ind, 0, cnvtdatetime(curdate,curtime3),
     0, 0, 0
     FROM dm_afd_part_objects dapo
     WHERE (dapo.owner=dgbc_data->dat_qual[dgbc_tab_cnt].owner)
      AND (dapo.table_name=dgbc_data->dat_qual[dgbc_tab_cnt].table_name)
      AND (dapo.alpha_feature_nbr=dgbc_data->dat_qual[dgbc_tab_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest partitioned objects to the plan schema.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dm_afd_part_subpart_objects
    (owner, table_name, object_type,
    object_name, alpha_feature_nbr, schema_instance,
    partition_name, subpartition_name, high_value,
    partition_position, subpartition_position, tablespace_name,
    indexing_off_ind, updt_id, updt_dt_tm,
    updt_task, updt_applctx, updt_cnt)(SELECT
     dapso.owner, dapso.table_name, dapso.object_type,
     dapso.object_name, dgbc_package_int, dapso.schema_instance,
     dapso.partition_name, dapso.subpartition_name, dapso.high_value,
     dapso.partition_position, dapso.subpartition_position, dapso.tablespace_name,
     dapso.indexing_off_ind, 0, cnvtdatetime(curdate,curtime3),
     0, 0, 0
     FROM dm_afd_part_subpart_objects dapso
     WHERE (dapso.owner=dgbc_data->dat_qual[dgbc_tab_cnt].owner)
      AND (dapso.table_name=dgbc_data->dat_qual[dgbc_tab_cnt].table_name)
      AND (dapso.alpha_feature_nbr=dgbc_data->dat_qual[dgbc_tab_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest partition and subpartition objects to the plan schema."
    )=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dm_afd_part_subpart_tspaces
    (owner, table_name, object_type,
    object_name, alpha_feature_nbr, schema_instance,
    part_type, tablespace_name, updt_id,
    updt_dt_tm, updt_task, updt_applctx,
    updt_cnt)(SELECT
     dapst.owner, dapst.table_name, dapst.object_type,
     dapst.object_name, dgbc_package_int, dapst.schema_instance,
     dapst.part_type, dapst.tablespace_name, 0,
     cnvtdatetime(curdate,curtime3), 0, 0,
     0
     FROM dm_afd_part_subpart_tspaces dapst
     WHERE (dapst.owner=dgbc_data->dat_qual[dgbc_tab_cnt].owner)
      AND (dapst.table_name=dgbc_data->dat_qual[dgbc_tab_cnt].table_name)
      AND (dapst.alpha_feature_nbr=dgbc_data->dat_qual[dgbc_tab_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest partitioned tablespace to the plan schema.")=
   1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dm_afd_part_subpart_keycols
    (owner, table_name, object_type,
    object_name, alpha_feature_nbr, schema_instance,
    part_type, column_name, column_position,
    updt_id, updt_dt_tm, updt_task,
    updt_applctx, updt_cnt)(SELECT
     dapsk.owner, dapsk.table_name, dapsk.object_type,
     dapsk.object_name, dgbc_package_int, dapsk.schema_instance,
     dapsk.part_type, dapsk.column_name, dapsk.column_position,
     0, cnvtdatetime(curdate,curtime3), 0,
     0, 0
     FROM dm_afd_part_subpart_keycols dapsk
     WHERE (dapsk.owner=dgbc_data->dat_qual[dgbc_tab_cnt].owner)
      AND (dapsk.table_name=dgbc_data->dat_qual[dgbc_tab_cnt].table_name)
      AND (dapsk.alpha_feature_nbr=dgbc_data->dat_qual[dgbc_tab_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest subpartitioned key columns to the plan schema."
    )=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ...
   ENDIF
 ENDFOR
 SET stat = alterlist(dgbc_data->dat_qual,0)
 IF ((dm_err->debug_flag > 1))
  CALL echo("After schema.")
  CALL trace(7)
 ENDIF
 SELECT INTO "nl:"
  cvs.code_set, cvs.updt_dt_tm, cvs.alpha_feature_nbr
  ...
  HEAD REPORT
   cvscnt = 0, rpt_best_csi_pkg = 0, rpt_best_csi = 0,
   rpt_use_csi = 0
  HEAD cvs.code_set
   rpt_best_csi_pkg = 0, rpt_best_csi = 0, rpt_use_csi = 1,
   cvscnt = (cvscnt+ 1)
   IF (mod(cvscnt,100)=1)
    stat = alterlist(dgbc_data->cvs_qual,(cvscnt+ 99))
   ENDIF
   dgbc_data->cvs_qual[cvscnt].code_set = cvs.code_set
  DETAIL
   IF (cvs.code_set_instance=0)
    rpt_use_csi = 0
   ENDIF
   IF (rpt_use_csi=1
    AND cvs.code_set_instance > rpt_best_csi)
    rpt_best_csi = cvs.code_set_instance, rpt_best_csi_pkg = cvs.alpha_feature_nbr
   ENDIF
  FOOT  cvs.code_set
   IF (rpt_use_csi=1)
    dgbc_data->cvs_qual[cvscnt].alpha_feature_nbr = rpt_best_csi_pkg
   ELSE
    dgbc_data->cvs_qual[cvscnt].alpha_feature_nbr = cvs.alpha_feature_nbr
   ENDIF
  FOOT REPORT
   stat = alterlist(dgbc_data->cvs_qual,cvscnt)
  WITH nocounter
 ;end select
 IF (check_error("Installation Failed. Error while getting the latest code sets for the plan.")=1)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  GO TO exit_program
 ENDIF
 FOR (dgbc_cvs_cnt = 1 TO size(dgbc_data->cvs_qual,5))
   INSERT  FROM dm_afd_code_value_set
    (code_set, display, display_key,
    description, definition, table_name,
    contributor, owner_module, cache_ind,
    extension_ind, add_access_ind, chg_access_ind,
    del_access_ind, inq_access_ind, domain_qualifier_ind,
    domain_code_set, code_set_hits, code_values_cnt,
    def_dup_rule_flag, cdf_meaning_dup_ind, display_key_dup_ind,
    active_ind_dup_ind, display_dup_ind, alias_dup_ind,
    alpha_feature_nbr, feature_number, definition_dup_ind,
    updt_dt_tm, updt_id, updt_task,
    updt_cnt, updt_applctx, code_set_instance,
    schema_date)(SELECT
     ...
     cvs.domain_code_set, cvs.code_set_hits, cvs.code_values_cnt,
     cvs.def_dup_rule_flag, cvs.cdf_meaning_dup_ind, cvs.display_key_dup_ind,
     cvs.active_ind_dup_ind, cvs.display_dup_ind, cvs.alias_dup_ind,
     dgbc_package_int, cvs.feature_number, cvs.definition_dup_ind,
     cvs.updt_dt_tm, 0, 0,
     0, 0, cvs.code_set_instance,
     cvs.schema_date
     FROM dm_afd_code_value_set cvs
     WHERE (cvs.code_set=dgbc_data->cvs_qual[dgbc_cvs_cnt].code_set)
      AND (cvs.alpha_feature_nbr=dgbc_data->cvs_qual[dgbc_cvs_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error("Installation Failed. Error while adding the latest code sets to the plan.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dm_afd_code_set_extension
    (code_set, field_name, field_seq,
    field_type, field_len, field_prompt,
    field_in_mask, field_out_mask, validation_condition,
    validation_code_set, action_field, field_default,
    field_help, alpha_feature_nbr, updt_dt_tm,
    updt_id, updt_task, updt_cnt,
    updt_applctx)(SELECT
     cse.code_set, cse.field_name, cse.field_seq,
     ...
     cse.field_help, dgbc_package_int, cse.updt_dt_tm,
     0, 0, 0,
     0
     FROM dm_afd_code_set_extension cse
     WHERE (cse.code_set=dgbc_data->cvs_qual[dgbc_cvs_cnt].code_set)
      AND (cse.alpha_feature_nbr=dgbc_data->cvs_qual[dgbc_cvs_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest code set extensions to the plan.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dm_afd_code_value
    (code_value, code_set, cdf_meaning,
    display, display_key, description,
    definition, collation_seq, active_type_cd,
    active_ind, active_dt_tm, inactive_dt_tm,
    begin_effective_dt_tm, end_effective_dt_tm, data_status_cd,
    data_status_dt_tm, data_status_prsnl_id, active_status_prsnl_id,
    cki, alpha_feature_nbr, updt_dt_tm,
    updt_id, updt_task, updt_cnt,
    updt_applctx)(SELECT
     ...
     cv.active_ind, cv.active_dt_tm, cv.inactive_dt_tm,
     cv.begin_effective_dt_tm, cv.end_effective_dt_tm, cv.data_status_cd,
     cv.data_status_dt_tm, cv.data_status_prsnl_id, cv.active_status_prsnl_id,
     cv.cki, dgbc_package_int, cv.updt_dt_tm,
     0, 0, 0,
     0
     FROM dm_afd_code_value cv
     WHERE (cv.code_set=dgbc_data->cvs_qual[dgbc_cvs_cnt].code_set)
      AND (cv.alpha_feature_nbr=dgbc_data->cvs_qual[dgbc_cvs_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error("Installation Failed. Error while adding the latest code values to the plan.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dm_afd_code_value_alias
    (code_set, contributor_source_cd, alias,
    code_value, primary_ind, alias_type_meaning,
    alpha_feature_nbr, updt_dt_tm, updt_id,
    updt_task, updt_cnt, updt_applctx)(SELECT
     cva.code_set, cva.contributor_source_cd, cva.alias,
     cva.code_value, cva.primary_ind, cva.alias_type_meaning,
     dgbc_package_int, cva.updt_dt_tm, 0,
     0, 0, 0
     FROM dm_afd_code_value_alias cva
     WHERE (cva.code_set=dgbc_data->cvs_qual[dgbc_cvs_cnt].code_set)
      AND (cva.alpha_feature_nbr=dgbc_data->cvs_qual[dgbc_cvs_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest code value aliases to the plan.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dm_afd_code_value_extension
    (code_value, field_name, code_set,
    field_type, field_value, alpha_feature_nbr,
    updt_dt_tm, updt_id, updt_task,
    updt_cnt, updt_applctx)(SELECT
     cve.code_value, cve.field_name, cve.code_set,
     cve.field_type, cve.field_value, dgbc_package_int,
     cve.updt_dt_tm, 0, 0,
     0, 0
     FROM dm_afd_code_value_extension cve
     WHERE (cve.code_set=dgbc_data->cvs_qual[dgbc_cvs_cnt].code_set)
      AND (cve.alpha_feature_nbr=dgbc_data->cvs_qual[dgbc_cvs_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest code value extensions to the plan.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dm_afd_code_value_group
    (code_set, parent_code_value, child_code_value,
    collation_seq, child_code_set, alpha_feature_nbr,
    updt_dt_tm, updt_id, updt_task,
    updt_cnt, updt_applctx)(SELECT
     cvg.code_set, cvg.parent_code_value, cvg.child_code_value,
     cvg.collation_seq, cvg.child_code_set, dgbc_package_int,
     cvg.updt_dt_tm, 0, 0,
     0, 0
     FROM dm_afd_code_value_group cvg
     WHERE (cvg.code_set=dgbc_data->cvs_qual[dgbc_cvs_cnt].code_set)
      AND (cvg.alpha_feature_nbr=dgbc_data->cvs_qual[dgbc_cvs_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest code value groups to the plan.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
    COMMIT
   ENDIF
   INSERT  FROM dm_afd_common_data_foundation
    (code_set, cdf_meaning, display,
    definition, alpha_feature_nbr, updt_dt_tm,
    updt_id, updt_task, updt_cnt,
    updt_applctx)(SELECT
     cdf.code_set, cdf.cdf_meaning, cdf.display,
     cdf.definition, dgbc_package_int, cdf.updt_dt_tm,
     0, 0, 0,
     0
     FROM dm_afd_common_data_foundation cdf
     WHERE (cdf.code_set=dgbc_data->cvs_qual[dgbc_cvs_cnt].code_set)
      AND (cdf.alpha_feature_nbr=dgbc_data->cvs_qual[dgbc_cvs_cnt].alpha_feature_nbr))
    WITH nocounter
   ;end insert
   IF (check_error("Installation Failed. Error while adding the latest CDF meanings to the plan.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ELSE
   ...
   ENDIF
 ENDFOR
 SET stat = alterlist(dgbc_data->cvs_qual,0)
 IF ((dm_err->debug_flag > 1))
  CALL echo("After cdf_meanings.")
  CALL trace(7)
 ENDIF
 SELECT INTO "nl:"
  doa.application_number, doa.schema_date, doa.alpha_feature_nbr
  FROM dm_ocd_application doa,
   dm_install_plan dip
  WHERE dip.install_plan_id=dgbc_plan_id
   AND dip.package_number=doa.alpha_feature_nbr
  ORDER BY doa.application_number, doa.schema_date, doa.alpha_feature_nbr
  HEAD REPORT
   appcnt = 0, stat = alterlist(dgbc_data->app_qual,100)
  HEAD doa.application_number
   appcnt = (appcnt+ 1)
   IF (mod(appcnt,100)=1
    AND appcnt != 1)
    stat = alterlist(dgbc_data->app_qual,(appcnt+ 99))
   ENDIF
  FOOT  doa.application_number
   dgbc_data->app_qual[appcnt].application_number = doa.application_number, dgbc_data->app_qual[
   appcnt].alpha_feature_nbr = doa.alpha_feature_nbr
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("application_number: [",doa.application_number,"] schema_date: [",format(doa
      .schema_date,";;Q"),"] package number: [",
     doa.alpha_feature_nbr,"]"))
   ENDIF
  FOOT REPORT
   stat = alterlist(dgbc_data->app_qual,appcnt)
  WITH nocounter
 ;end select
 IF (check_error("Installation Failed. Error while getting the latest applications for the plan.")=1)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  GO TO exit_program
 ENDIF
 FOR (dgbc_app_cnt = 1 TO size(dgbc_data->app_qual,5))
  INSERT  FROM dm_ocd_application
   ...
   common_application_ind, feature_number, deleted_ind,
   schema_date, updt_dt_tm, updt_id,
   updt_task, updt_cnt, updt_applctx)(SELECT
    doa.application_number, dgbc_package_int, doa.owner,
    doa.description, doa.active_dt_tm, doa.active_ind,
    doa.last_localized_dt_tm, doa.text, doa.inactive_dt_tm,
    doa.log_access_ind, doa.application_ini_ind, doa.object_name,
    doa.direct_access_ind, doa.log_level, doa.request_log_level,
    doa.min_version_required, doa.disable_cache_ind, doa.module,
    doa.common_application_ind, doa.feature_number, doa.deleted_ind,
    doa.schema_date, cnvtdatetime(curdate,curtime3), 0,
    0, 0, 0
    FROM dm_ocd_application doa
    WHERE (doa.application_number=dgbc_data->app_qual[dgbc_app_cnt].application_number)
     AND (doa.alpha_feature_nbr=dgbc_data->app_qual[dgbc_app_cnt].alpha_feature_nbr))
   WITH nocounter
  ;end insert
  IF (check_error("Installation Failed. Error while adding the latest applications to the plan.")=1)
   CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
   ROLLBACK
   GO TO exit_program
  ELSE
  ...
  ENDIF
 ENDFOR
 SET stat = alterlist(dgbc_data->app_qual,0)
 IF ((dm_err->debug_flag > 1))
  CALL echo("After applications.")
  CALL trace(7)
 ENDIF
 SELECT INTO "nl:"
  dot.task_number, dot.schema_date, dot.alpha_feature_nbr
  FROM dm_ocd_task dot,
   dm_install_plan dip
  WHERE dip.install_plan_id=dgbc_plan_id
   AND dip.package_number=dot.alpha_feature_nbr
  ORDER BY dot.task_number, dot.schema_date, dot.alpha_feature_nbr
  HEAD REPORT
   tskcnt = 0, stat = alterlist(dgbc_data->tsk_qual,100)
  HEAD dot.task_number
   tskcnt = (tskcnt+ 1)
   IF (mod(tskcnt,100)=1
    AND tskcnt != 1)
    stat = alterlist(dgbc_data->tsk_qual,(tskcnt+ 99))
   ENDIF
  FOOT  dot.task_number
   dgbc_data->tsk_qual[tskcnt].task_number = dot.task_number, dgbc_data->tsk_qual[tskcnt].
   alpha_feature_nbr = dot.alpha_feature_nbr
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("application_number: [",dot.task_number,"] schema_date: [",format(dot.schema_date,
      ";;Q"),"] package number: [",
     dot.alpha_feature_nbr,"]"))
   ENDIF
  FOOT REPORT
   stat = alterlist(dgbc_data->tsk_qual,tskcnt)
  WITH nocounter
 ;end select
 IF (check_error("Installation Failed. Error while getting the latest applications for the plan.")=1)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  GO TO exit_program
 ENDIF
 FOR (dgbc_tsk_cnt = 1 TO size(dgbc_data->tsk_qual,5))
  INSERT  FROM dm_ocd_task
   ...
   old_task_number, feature_number, deleted_ind,
   schema_date, updt_dt_tm, updt_id,
   updt_task, updt_cnt, updt_applctx)(SELECT
    dot.task_number, dgbc_package_int, dot.description,
    dot.active_dt_tm, dot.active_ind, dot.text,
    dot.inactive_dt_tm, dot.subordinate_task_ind, dot.optional_required_flag,
    dot.old_task_number, dot.feature_number, dot.deleted_ind,
    dot.schema_date, cnvtdatetime(curdate,curtime3), 0,
    0, 0, 0
    FROM dm_ocd_task dot
    WHERE (dot.task_number=dgbc_data->tsk_qual[dgbc_tsk_cnt].task_number)
     AND (dot.alpha_feature_nbr=dgbc_data->tsk_qual[dgbc_tsk_cnt].alpha_feature_nbr))
   WITH nocounter
  ;end insert
  IF (check_error("Installation Failed. Error while adding the latest tasks to the plan schema.")=1)
   CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
   ROLLBACK
   GO TO exit_program
  ELSE
  ...
  ENDIF
 ENDFOR
 SET stat = alterlist(dgbc_data->tsk_qual,0)
 IF ((dm_err->debug_flag > 1))
  CALL echo("After tasks.")
  CALL trace(7)
 ENDIF
 SELECT INTO "nl:"
  req.request_number, req.schema_date, req.alpha_feature_nbr
  FROM dm_ocd_request req,
   dm_install_plan dip
  WHERE dip.install_plan_id=dgbc_plan_id
   AND dip.package_number=req.alpha_feature_nbr
  ORDER BY req.request_number, req.schema_date, req.alpha_feature_nbr
  HEAD REPORT
   reqcnt = 0, stat = alterlist(dgbc_data->req_qual,100)
  HEAD req.request_number
   reqcnt = (reqcnt+ 1)
   IF (mod(reqcnt,100)=1
    AND reqcnt != 1)
    stat = alterlist(dgbc_data->req_qual,(reqcnt+ 99))
   ENDIF
  FOOT  req.request_number
   dgbc_data->req_qual[reqcnt].request_number = req.request_number, dgbc_data->req_qual[reqcnt].
   alpha_feature_nbr = req.alpha_feature_nbr
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("request_number: [",req.request_number,"] schema_date: [",format(req.schema_date,
      ";;Q"),"] package number: [",
     req.alpha_feature_nbr,"]"))
   ENDIF
  FOOT REPORT
   stat = alterlist(dgbc_data->req_qual,reqcnt)
  WITH nocounter
 ;end select
 IF (check_error("Installation Failed. Error while getting the latest requests for the plan.")=1)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  GO TO exit_program
 ENDIF
 FOR (dgbc_req_cnt = 1 TO size(dgbc_data->req_qual,5))
  INSERT  FROM dm_ocd_request
   (request_number, alpha_feature_nbr, description,
   request_name, text, active_ind,
   active_dt_tm, inactive_dt_tm, prolog_script,
   epilog_script, write_to_que_ind, requestclass,
   cachetime, cachegrace, cachestale,
   cachetrim, feature_number, deleted_ind,
   schema_date, updt_dt_tm, updt_id,
   updt_task, updt_cnt, updt_applctx)(SELECT
    req.request_number, dgbc_package_int, req.description,
    req.request_name, req.text, req.active_ind,
    req.active_dt_tm, req.inactive_dt_tm, req.prolog_script,
    req.epilog_script, req.write_to_que_ind, req.requestclass,
    req.cachetime, req.cachegrace, req.cachestale,
    req.cachetrim, req.feature_number, req.deleted_ind,
    req.schema_date, cnvtdatetime(curdate,curtime3), 0,
    0, 0, 0
    FROM dm_ocd_request req
    WHERE (req.request_number=dgbc_data->req_qual[dgbc_req_cnt].request_number)
     AND (req.alpha_feature_nbr=dgbc_data->req_qual[dgbc_req_cnt].alpha_feature_nbr))
   WITH nocounter
  ;end insert
  IF (check_error("Installation Failed. Error while adding the latest requests to the plan schema.")=
  1)
   CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
   ROLLBACK
   GO TO exit_program
  ...
  ENDIF
 ENDFOR
 SET stat = alterlist(dgbc_data->req_qual,0)
 IF ((dm_err->debug_flag > 1))
  CALL echo("After requests.")
  CALL trace(7)
 ENDIF
 SELECT INTO "nl:"
  atr.application_number, atr.task_number, atr.schema_date,
  atr.alpha_feature_nbr
  FROM dm_ocd_app_task_r atr,
   dm_install_plan dip
  WHERE dip.install_plan_id=dgbc_plan_id
   AND dip.package_number=atr.alpha_feature_nbr
  ORDER BY atr.application_number, atr.task_number, atr.schema_date,
   atr.alpha_feature_nbr
  HEAD REPORT
   atrcnt = 0, stat = alterlist(dgbc_data->atr_qual,100)
  HEAD atr.application_number
   row + 1
  HEAD atr.task_number
   atrcnt = (atrcnt+ 1)
   IF (mod(atrcnt,100)=1
    AND atrcnt != 1)
    stat = alterlist(dgbc_data->atr_qual,(atrcnt+ 99))
   ENDIF
  FOOT  atr.task_number
   dgbc_data->atr_qual[atrcnt].application_number = atr.application_number, dgbc_data->atr_qual[
   atrcnt].task_number = atr.task_number, dgbc_data->atr_qual[atrcnt].alpha_feature_nbr = atr
   .alpha_feature_nbr
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("application_number: [",atr.application_number,"] task_number: [",atr.task_number,
     "] schema_date: [",
     format(atr.schema_date,";;Q"),"] package number: [",atr.alpha_feature_nbr,"]"))
   ENDIF
  FOOT REPORT
   stat = alterlist(dgbc_data->atr_qual,atrcnt)
  WITH nocounter
 ;end select
 IF (check_error(
  "Installation Failed. Error while getting the latest app/task relationships for the plan.")=1)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  GO TO exit_program
 ENDIF
 FOR (dgbc_atr_cnt = 1 TO size(dgbc_data->atr_qual,5))
  INSERT  FROM dm_ocd_app_task_r
   (application_number, task_number, alpha_feature_nbr,
   feature_number, deleted_ind, schema_date,
   updt_dt_tm, updt_id, updt_task,
   updt_cnt, updt_applctx)(SELECT
    atr.application_number, atr.task_number, dgbc_package_int,
    atr.feature_number, atr.deleted_ind, atr.schema_date,
    cnvtdatetime(curdate,curtime3), 0, 0,
    0, 0
    FROM dm_ocd_app_task_r atr
    WHERE (atr.application_number=dgbc_data->atr_qual[dgbc_atr_cnt].application_number)
     AND (atr.task_number=dgbc_data->atr_qual[dgbc_atr_cnt].task_number)
     AND (atr.alpha_feature_nbr=dgbc_data->atr_qual[dgbc_atr_cnt].alpha_feature_nbr))
   WITH nocounter
  ;end insert
  IF (check_error(
   "Installation Failed. Error while adding the latest app/task relationships to the plan schema.")=1
  )
   CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
   ROLLBACK
   GO TO exit_program
  ...
  ENDIF
 ENDFOR
 SET stat = alterlist(dgbc_data->atr_qual,0)
 IF ((dm_err->debug_flag > 1))
  CALL echo("After app-task relationships.")
  CALL trace(7)
 ENDIF
 SELECT INTO "nl:"
  trr.task_number, trr.request_number, trr.schema_date,
  trr.alpha_feature_nbr
  FROM dm_ocd_task_req_r trr,
   dm_install_plan dip
  WHERE dip.install_plan_id=dgbc_plan_id
   AND dip.package_number=trr.alpha_feature_nbr
  ORDER BY trr.task_number, trr.request_number, trr.schema_date,
   trr.alpha_feature_nbr
  HEAD REPORT
   trrcnt = 0, stat = alterlist(dgbc_data->trr_qual,100)
  HEAD trr.task_number
   row + 1
  HEAD trr.request_number
   trrcnt = (trrcnt+ 1)
   IF (mod(trrcnt,100)=1
    AND trrcnt != 1)
    stat = alterlist(dgbc_data->trr_qual,(trrcnt+ 99))
   ENDIF
  FOOT  trr.request_number
   dgbc_data->trr_qual[trrcnt].task_number = trr.task_number, dgbc_data->trr_qual[trrcnt].
   request_number = trr.request_number, dgbc_data->trr_qual[trrcnt].alpha_feature_nbr = trr
   .alpha_feature_nbr
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("task_number: [",trr.task_number,"] request_number: [",trr.request_number,
     "] schema_date: [",
     format(trr.schema_date,";;Q"),"] package number: [",trr.alpha_feature_nbr,"]"))
   ENDIF
  FOOT REPORT
   stat = alterlist(dgbc_data->trr_qual,trrcnt)
  WITH nocounter
 ;end select
 IF (check_error(
  "Installation Failed. Error while getting the latest task/request relationships for the plan.")=1)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  GO TO exit_program
 ENDIF
 FOR (dgbc_trr_cnt = 1 TO size(dgbc_data->trr_qual,5))
  INSERT  FROM dm_ocd_task_req_r
   (task_number, request_number, alpha_feature_nbr,
   feature_number, deleted_ind, schema_date,
   updt_dt_tm, updt_id, updt_task,
   updt_cnt, updt_applctx)(SELECT
    trr.task_number, trr.request_number, dgbc_package_int,
    trr.feature_number, trr.deleted_ind, trr.schema_date,
    cnvtdatetime(curdate,curtime3), 0, 0,
    0, 0
    FROM dm_ocd_task_req_r trr
    WHERE (trr.request_number=dgbc_data->trr_qual[dgbc_trr_cnt].request_number)
     AND (trr.task_number=dgbc_data->trr_qual[dgbc_trr_cnt].task_number)
     AND (trr.alpha_feature_nbr=dgbc_data->trr_qual[dgbc_trr_cnt].alpha_feature_nbr))
   WITH nocounter
  ;end insert
  IF (check_error(
   "Installation Failed. Error while adding the latest task/request relationships to the plan schema."
   )=1)
   CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
   ROLLBACK
   GO TO exit_program
  ...
  ENDIF
 ENDFOR
 SET stat = alterlist(dgbc_data->trr_qual,0)
 IF ((dm_err->debug_flag > 1))
  CALL echo("After task-request relationships.")
  CALL trace(7)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_adm_purge_template dapt,
   dm_install_plan dip
  WHERE dip.install_plan_id=dgbc_plan_id
   AND dapt.ocd=dip.package_number
   AND dapt.owner=currdbuser
  ORDER BY dapt.template_nbr, dapt.schema_dt_tm DESC, dapt.ocd DESC
  HEAD REPORT
   dgbc_data->prgtemp_cnt = 0, stat = alterlist(dgbc_data->prgtmp_qual,dgbc_data->prgtemp_cnt)
  HEAD dapt.template_nbr
   dgbc_data->prgtemp_cnt = (dgbc_data->prgtemp_cnt+ 1)
   IF (mod(dgbc_data->prgtemp_cnt,50)=1)
    stat = alterlist(dgbc_data->prgtmp_qual,(dgbc_data->prgtemp_cnt+ 49))
   ENDIF
   dgbc_data->prgtmp_qual[dgbc_data->prgtemp_cnt].template_nbr = dapt.template_nbr, dgbc_data->
   prgtmp_qual[dgbc_data->prgtemp_cnt].ocd = dapt.ocd
   IF ((dm_err->debug_flag > 0))
    CALL echo(build("purge_template_number: [",dapt.template_nbr,"] schema_date: [",format(dapt
      .schema_dt_tm,";;Q"),"] package number: [",
     dapt.ocd,"]"))
   ENDIF
  FOOT REPORT
   stat = alterlist(dgbc_data->prgtmp_qual,dgbc_data->prgtemp_cnt)
  WITH nocounter
 ;end select
 IF (check_error("Installation Failed. Error while getting the latest purge templates for the plan.")
 =1)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  GO TO exit_program
 ENDIF
 FOR (dgbc_prg_cnt = 1 TO dgbc_data->prgtemp_cnt)
   INSERT  FROM dm_adm_purge_template
    (template_nbr, feature_nbr, schema_dt_tm,
    name, program_str, active_ind,
    ocd, updt_id, updt_dt_tm,
    updt_task, updt_cnt, updt_applctx,
    owner)(SELECT
     dapt.template_nbr, dapt.feature_nbr, dapt.schema_dt_tm,
     dapt.name, dapt.program_str, dapt.active_ind,
     dgbc_package_int, dapt.updt_id, dapt.updt_dt_tm,
     dapt.updt_task, dapt.updt_cnt, dapt.updt_applctx,
     dapt.owner
     FROM dm_adm_purge_template dapt
     WHERE (dapt.template_nbr=dgbc_data->prgtmp_qual[dgbc_prg_cnt].template_nbr)
      AND (dapt.ocd=dgbc_data->prgtmp_qual[dgbc_prg_cnt].ocd))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest purge templates to the plan schema.")=1)
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ENDIF
   INSERT  FROM dm_adm_purge_token
    (template_nbr, feature_nbr, token_str,
    prompt_str, data_type_flag, schema_dt_tm,
    ocd, updt_id, updt_dt_tm,
    updt_task, updt_cnt, updt_applctx)(SELECT
     dapt.template_nbr, dapt.feature_nbr, dapt.token_str,
     dapt.prompt_str, dapt.data_type_flag, dapt.schema_dt_tm,
     dgbc_package_int, dapt.updt_id, dapt.updt_dt_tm,
     dapt.updt_task, dapt.updt_cnt, dapt.updt_applctx
     FROM dm_adm_purge_token dapt
     WHERE (dapt.template_nbr=dgbc_data->prgtmp_qual[dgbc_prg_cnt].template_nbr)
      AND (dapt.ocd=dgbc_data->prgtmp_qual[dgbc_prg_cnt].ocd))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest purge template tokens to the plan schema.")=1
   )
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ENDIF
   INSERT  FROM dm_adm_purge_table
    (template_nbr, feature_nbr, parent_table,
    child_table, child_where, purge_type_flag,
    parent_col1, child_col1, parent_col2,
    child_col2, parent_col3, child_col3,
    parent_col4, child_col4, parent_col5,
    child_col5, schema_dt_tm, ocd,
    updt_id, updt_dt_tm, updt_task,
    updt_cnt, updt_applctx)(SELECT
     dapt.template_nbr, dapt.feature_nbr, dapt.parent_table,
     dapt.child_table, dapt.child_where, dapt.purge_type_flag,
     dapt.parent_col1, dapt.child_col1, dapt.parent_col2,
     dapt.child_col2, dapt.parent_col3, dapt.child_col3,
     dapt.parent_col4, dapt.child_col4, dapt.parent_col5,
     dapt.child_col5, dapt.schema_dt_tm, dgbc_package_int,
     dapt.updt_id, dapt.updt_dt_tm, dapt.updt_task,
     dapt.updt_cnt, dapt.updt_applctx
     FROM dm_adm_purge_table dapt
     WHERE (dapt.template_nbr=dgbc_data->prgtmp_qual[dgbc_prg_cnt].template_nbr)
      AND (dapt.ocd=dgbc_data->prgtmp_qual[dgbc_prg_cnt].ocd))
    WITH nocounter
   ;end insert
   IF (check_error(
    "Installation Failed. Error while adding the latest purge template tables to the plan schema.")=1
   )
    CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
    ROLLBACK
    GO TO exit_program
   ...
   COMMIT
 ENDFOR
 IF (get_batch_dm_ocd_readme(cnvtreal(dgbc_package_int),cnvtreal(dgbc_eid))=0)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  ROLLBACK
  GO TO exit_program
 ENDIF
 SET stat = alterlist(dgbc_data->rdm_qual,0)
 IF ((dm_err->debug_flag > 1))
  CALL echo("After readmes.")
  CALL trace(7)
 ENDIF
 DECLARE dgbc_rev_num = f8
 SET dgbc_rev_num = 0.00
 SELECT INTO "nl:"
  daf.rev_number
  FROM dm_alpha_features daf,
   dm_install_plan dip
  WHERE dip.install_plan_id=dgbc_plan_id
   AND daf.alpha_feature_nbr=dip.package_number
   AND daf.owner=currdbuser
  ORDER BY daf.rev_number
  DETAIL
   dgbc_rev_num = daf.rev_number
  WITH nocounter
 ;end select
 IF (check_error("Installation Failed. Error while getting the latest revision number for the plan.")
 =1)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  GO TO exit_program
 ENDIF
 INSERT  FROM dm_alpha_features d
  SET d.alpha_feature_nbr = dgbc_package_int, d.description = "Batch Install Plan", d.rev_number =
   dgbc_rev_num,
   d.sponsor_client_id = null, d.create_dt_tm = cnvtdatetimeutc(cnvtdatetime(curdate,curtime3),2), d
   .feature_number = 0,
   d.product_area_number = 0, d.owner = currdbuser
  WITH nocounter
 ;end insert
 IF (check_error("Installation Failed. Error while adding the Plan ID in DM_ALPHA_FEATURES.")=1)
  CALL end_status(dm_err->eproc,dgbc_eid,dgbc_package_int)
  ROLLBACK
  GO TO exit_program
 ELSE
  COMMIT
 ENDIF
 SUBROUTINE cleanup_batch(sbr_batch_id)
   SET dm_err->eproc = "Cleaning up DM_AFD tables for the Batch Install Plan."
   IF (sbr_batch_id >= 0)
    SET dm_err->emsg = "Operation Failed.  The Batch Plan ID was invalid."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   SELECT INTO "nl:"
    ...
    WHERE d.alpha_feature_nbr=sbr_batch_id
     AND d.owner=currdbuser
    WITH nocounter
   ;end select
   IF (check_error("Error occurred while checking for the Batch Install Plan in DM_ALPHA_FEATURES.")=
   1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   IF (curqual)
    SET dm_err->eproc = "Checking for the Batch Install Plan in DM_ALPHA_FEATURES."
    SET dm_err->emsg = "Operation Failed.  The Batch Plan ID is listed in DM_ALPHA_FEATURES."
    SET dm_err->err_ind = 1
    CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
    RETURN(0)
   ENDIF
   SET dm_err->eproc = "Deleting the Batch Install Plan from DM_AFD tables."
   IF (delete_loop("dm_afd_part_subpart_keycols","alpha_feature_nbr",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_part_subpart_tspaces","alpha_feature_nbr",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_part_subpart_objects","alpha_feature_nbr",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_part_objects","alpha_feature_nbr",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_cons_columns","alpha_feature_nbr",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_constraints","alpha_feature_nbr",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_index_columns","alpha_feature_nbr",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_indexes","alpha_feature_nbr",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_columns","alpha_feature_nbr",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_tables","alpha_feature_nbr",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_common_data_foundation","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_code_value_group","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_code_value_extension","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_code_value_alias","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_code_value","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_code_set_extension","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_afd_code_value_set","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_ocd_task_req_r","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_ocd_app_task_r","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_ocd_request","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_ocd_task","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_ocd_application","alpha_feature_nbr",build(sbr_batch_id),0)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_ocd_readme","ocd",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_adm_purge_template","ocd",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_adm_purge_table","ocd",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   IF (delete_loop("dm_adm_purge_token","ocd",build(sbr_batch_id),1)=0)
    RETURN(0)
   ENDIF
   RETURN(1)
 END ;Subroutine
#exit_program
 FREE RECORD dgbc_data
 FREE SET dgbc_rev_num
 FREE SET dgbc_eid
 FREE SET dgbc_plan_id
 FREE SET dgbc_package_int
 FREE SET dgbc_tab_cnt
 FREE SET dgbc_cvs_cnt
 FREE SET dgbc_app_cnt
 FREE SET dgbc_tsk_cnt
 FREE SET dgbc_req_cnt
 FREE SET dgbc_atr_cnt
 FREE SET dgbc_trr_cnt
 FREE SET dgbc_rdm_cnt
 CALL dm2_prg_maint("END")
END GO
