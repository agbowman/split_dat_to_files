CREATE PROGRAM cv_frpt_proc_performed
 PROMPT
 ...
 IF (validate(i18nuar_def,999)=999)
  ...
  persist
 ENDIF
 EXECUTE cv_frpt_proc_performed_drv "NL:",  $START_DATE,  $END_DATE,
  $ORG_ID
 ...
 SUBROUTINE get_reply_obj(dummy)
  ...
  SELECT
   ...
   cv_list_rpl_action_dt_tm = reply_obj->cv_list[d1.seq].rpl_action_dt_tm, cv_year = year(reply_obj->
    cv_list[d1.seq].rpl_action_dt_tm),
   cv_month = month(reply_obj->cv_list[d1.seq].rpl_action_dt_tm)
   FROM (dummyt d1  WITH seq = value(size(reply_obj->cv_list,5)))
   PLAN (d1)
   ORDER BY cv_year, cv_list_rpl_catalog_disp, cv_month
   HEAD REPORT
    _d0 = cv_list_rpl_catalog_disp, _d1 = cv_list_rpl_action_dt_tm, sdate = format(cnvtdatetime(
    ...
    ndaylightvar = 0, stimezone = datetimezonebyindex(curtimezoneapp,noffsetvar,ndaylightvar,7,
     cnvtdatetime(curdate,curtime)), total_month = 0,
    total_year = 0, gtotal_cnt = 0, total_cnt = 0,
    ...
    m_series[1].name = "Procedures Performed",
   ...
   HEAD cv_month
    _fdrawheight = headmonthsection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = headmonthsection(rpt_render)
   DETAIL
    total_month = (total_month+ 1), _fdrawheight = detailsection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = detailsection(rpt_render)
   FOOT  cv_month
    total_year = (total_year+ total_month), total_cnt = (total_month+ total_cnt), _fdrawheight =
    footmonthsection(rpt_calcheight)
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = footmonthsection(rpt_render), total_month = 0
   FOOT  cv_list_rpl_catalog_disp
    tgraph = total_cnt, gtotal_cnt = (gtotal_cnt+ total_year), p_curr = total_cnt
    IF (p_curr >= p_max)
     p_max = p_curr
    ENDIF
    ...
    IF (((_yoffset+ _fdrawheight) > _fenddetail))
     BREAK
    ENDIF
    dummy_val = footcatalogdispsection(rpt_render), total_cnt = 0, total_year = 0,
    stat = alterlist(rptgraphrec->m_labels,_nfieldscnt), rptgraphrec->m_labels[_nfieldscnt].label =
    build2(build2(trim(cnvtstring(cv_year))," - ",trim(cv_list_rpl_catalog_disp)),char(0)), stat =
    alterlist(rptgraphrec->m_series[1].y_values,_nfieldscnt),
 ...
 SUBROUTINE headreportsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.313),(offsety+ 1.255),(offsetx+ 6.751),(offsety+
     1.255))
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(uar_i18ngetmessage(_hi18nhandle,
        "PROCEDURESPERFORMED","Signed Procedures by Type and Month")),char(0)))
 ...
 SUBROUTINE headmonthsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(format(cv_list_rpl_action_dt_tm,
        "MMMMMMMMM yyyy;;d")),char(0)))
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 ...
 SUBROUTINE footmonthsectionabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_height = 0.188
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(cnvtstring(total_month)),char(0))
     )
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
 ...
 SUBROUTINE footcatalogdispsectionabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.490000), private
   IF ( NOT (((( $U_VIEW=1)) OR (( $U_VIEW=3)))
    AND size(reply_obj->cv_list,5) > 0))
    RETURN(0.0)
   ENDIF
   IF (ncalc=rpt_render)
    ...
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(build2(uar_i18ngetmessage(_hi18nhandle,
        "YRTOTAL","Total: ")),char(0)))
    SET rptsd->m_flags = 32
    SET rptsd->m_y = (offsety+ 0.188)
    SET rptsd->m_x = (offsetx+ 5.875)
    SET rptsd->m_width = 0.938
    SET rptsd->m_height = 0.250
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(trim(cnvtstring(total_year)),char(0)))
    SET _dummypen = uar_rptsetpen(_hreport,_pen14s3c0)
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 3.250),(offsety+ 0.125),(offsetx+ 6.500),(offsety+
     0.125))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
 ...
 SUBROUTINE graphsectionabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(4.490000), private
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptgraphrec->m_fwidth = 7.250000
    SET rptgraphrec->m_fheight = 4.188000
    SET rptgraphrec->m_stitle = ""
    SET rptgraphrec->m_ssubtitle = ""
    SET rptgraphrec->m_sxtitle = ""
    SET rptgraphrec->m_sytitle = uar_i18ngetmessage(_hi18nhandle,"TOTALGRAPH","Total")
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CV_FRPT_PROC_PERFORMED"
