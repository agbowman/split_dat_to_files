CREATE PROGRAM dcp_upd_dx_icd9_hli_syn_cc:dba
 ...
 SET ms_child_c_info_domain_nm = "DCP_UPD_DX_ALL"
 ...
 ELSE
  GO TO exit_program
 ENDIF
 INSERT  FROM temp_dx_icd9_hli_syn dx
  (diagnosis_id, nomenclature_id)(SELECT
   d.diagnosis_id, n2.nomenclature_id
   ...
   WHERE n2.cmti=ctm.cmti
    ...
    AND ((n1.nomenclature_id=d.originating_nomenclature_id) OR (d.originating_nomenclature_id <= 0.0
    ...
    AND d.updt_dt_tm >= cnvtdatetime(dtbegin)
 ...
 IF (sbr_handle_cc_error(null)=0)
  GO TO exit_program
 ENDIF
 UPDATE  FROM diagnosis d
  SET d.originating_nomenclature_id =
   (SELECT
    dx.nomenclature_id
    FROM temp_dx_icd9_hli_syn dx
    WHERE dx.diagnosis_id=d.diagnosis_id), d.updt_dt_tm = cnvtdatetime(sysdate), d.updt_id = reqinfo
   ->updt_id,
  ...
  (SELECT
   dx.diagnosis_id
   FROM temp_dx_icd9_hli_syn dx
   WHERE dx.nomenclature_id BETWEEN mf_min_id AND mf_max_id))
 ;end update
 IF (sbr_handle_cc_error(null)=0)
  ROLLBACK
  SET readme_data->status = "F"
