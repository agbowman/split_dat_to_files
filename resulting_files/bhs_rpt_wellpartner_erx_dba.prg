CREATE PROGRAM bhs_rpt_wellpartner_erx:dba
 PROMPT
  "Begin Date" = "CURDATE",
  "End Date" = "CURDATE",
  "Test file?" = "no"
 ...
 IF (gl_bhs_prod_flag=1)
  SET ms_loc_dir = "/cerner/d_p627/bhscust/erx_wellpartner/"
 ENDIF
 ...
 ENDFOR
 IF (size(m_rec->erx,5) > 0)
  SET ms_timestamp = trim(format(sysdate,"yyyymmdd;;d"),3)
  FOR (ml_file_cnt = 1 TO 2)
    SET stat = initrec(frec)
    IF (ml_file_cnt=1)
     SET ms_file_name = concat("erx_baymc",ms_timestamp,".txt")
     SET ms_sponsor_code = "BAYMC"
    ELSE
     SET ms_file_name = concat("erx_bynh",ms_timestamp,".txt")
     SET ms_sponsor_code = "BYNH"
    ENDIF
    ...
    SET frec->file_buf = concat("Sponsor Code|",
     "Patient Last Name|Patient First Name|DOB|Zip Code|MRN|","Facility|",
    ...
    FOR (ml_loop = 1 TO size(m_rec->erx,5))
      IF ((m_rec->erx[ml_loop].n_skip=0))
       ...
       SET ms_line = concat(ms_sponsor_code,"|",m_rec->erx[ml_loop].s_pat_name_last,"|",m_rec->erx[
        ml_loop].s_pat_name_first,
        "|",m_rec->erx[ml_loop].s_pat_dob,"|",m_rec->erx[ml_loop].s_pat_zip,"|",
        m_rec->erx[ml_loop].s_mrn,"|",m_rec->erx[ml_loop].s_facility,"|","0000000000|",
        m_rec->erx[ml_loop].s_pharm_ncpdp,"|",m_rec->erx[ml_loop].s_prov_npi,"|",m_rec->erx[ml_loop].
        s_prov_dea,
        "|",m_rec->erx[ml_loop].s_pres_name_last,"|",m_rec->erx[ml_loop].s_pres_name_first,"|",
        ms_tmp,"|",m_rec->erx[ml_loop].s_written_loc_desc,"|",m_rec->erx[ml_loop].s_written_dt,
        "|",m_rec->erx[ml_loop].s_written_tm,"|",trim(cnvtstring(m_rec->erx[ml_loop].f_order_id),3),
        "|",
        m_rec->erx[ml_loop].s_order_status,"|",m_rec->erx[ml_loop].s_ndc_desc,"|",m_rec->erx[ml_loop]
        .s_drug_desc,
        "|",m_rec->erx[ml_loop].s_drug_qty,"|",m_rec->erx[ml_loop].s_drug_ndc,"|",
        m_rec->erx[ml_loop].s_fin,"|",m_rec->erx[ml_loop].s_diag_cd,"|",m_rec->erx[ml_loop].
        s_diag_src,
        "|",m_rec->erx[ml_loop].s_diag_prim,"|",m_rec->erx[ml_loop].s_pharmacy,char(10))
