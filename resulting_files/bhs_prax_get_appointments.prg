CREATE PROGRAM bhs_prax_get_appointments
  ...
  SELECT INTO "NL:"
   ...
   HEAD lg2.parent_loc_cd
    IF (locidx > 0)
     ...
     IF (pos=0)
      loccnt = (loccnt+ 1), locations->list[loccnt].location_cd = lg.child_loc_cd, locations->list[
      loccnt].facility_cd = lg2.parent_loc_cd,
 ...
 IF (valid_appt_found_ind=1)
  ...
  SELECT INTO "NL:"
   ...
   HEAD e.encntr_id
    ...
    IF (encidx > 0)
     encounters->list[encidx].fin = ea.alias
    ENDIF
   WITH nocounter, time = 30, expand = 1
  ;end select
   ...
   SELECT INTO value(moutputdevice)
    ...
    DETAIL
     ...
     IF (state_cd_pos > 0
      ...
      IF (encidx > 0)
       fin = encounters->list[encidx].fin
      ELSE
       fin = ""
      ENDIF
      ...
      v18, row + 1, v19 = build("<LocationCd>",cnvtint(rep_get_appt_person->qual[1].appointment[d.seq
        ].location_cd),"</LocationCd>"),
      col + 1, v19, row + 1,
      v20 = build("<LocationDisp>",trim(replace(replace(replace(replace(replace(rep_get_appt_person->
             qual[1].appointment[d.seq].location_freetext,"&","&amp;",0),"<","&lt;",0),">","&gt;",0),
          "'","&apos;",0),'"',"&quot;",0),3),"</LocationDisp>"), col + 1, v20,
      row + 1, locidx = locateval(idx,1,loccnt,rep_get_appt_person->qual[1].appointment[d.seq].
