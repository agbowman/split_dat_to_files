CREATE PROGRAM acs_rdm_encntr_svc_dt_tm_cc:dba
 CALL echo("START")
 ...
 SUBROUTINE (sbr_upd_id_evaluated(mf_max_id=f8,ms_info_domain_nm=vc,ms_max_name=vc) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE pm_sbr_handle_cc_error(null)
   DECLARE errmsg = vc WITH protect, noconstant("")
   IF (error(errmsg,0) != 0)
    IF (((findstring("ORA-01555",errmsg) != 0) OR (((findstring("ORA-01650",errmsg) != 0) OR (((
    findstring("ORA-01562",errmsg) != 0) OR (((findstring("ORA-30036",errmsg) != 0) OR (((findstring(
 ...
 DECLARE new_pm_hist_tracking_id = f8 WITH noconstant(0.0), protect
 SET ms_child_c_info_domain_nm = "acs_rdm_encntr_svc_dt_tm_domain_nm"
 SET ms_child_c_max_name = "acs_rdm_encntr_svc_dt_tm_child_max_name"
 ...
 UPDATE  FROM encounter e
  SET e.reg_dt_tm = e.reg_dt_tm, e.updt_task = 694239
  WHERE e.service_dt_tm = null
   AND ((e.arrive_dt_tm IS NOT null) OR (((e.pre_reg_dt_tm IS NOT null) OR (((e.reg_dt_tm IS NOT null
  ) OR (e.est_arrive_dt_tm IS NOT null)) )) ))
   AND e.encntr_id >= mf_min_id
   AND e.encntr_id <= mf_max_id
  WITH nocounter
 ;end update
 IF (pm_sbr_handle_cc_error(null)=0)
  GO TO exit_program
 ENDIF
