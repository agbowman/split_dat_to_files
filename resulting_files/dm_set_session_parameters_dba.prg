CREATE PROGRAM dm_set_session_parameters:dba
 ...
 SELECT INTO "nl:"
  ...
  FROM dm_info d
  WHERE d.info_domain="DM_SET_SESSION_PARAMETERS"
  ORDER BY d.info_number
  HEAD REPORT
   cnt = 0
  DETAIL
   cnt = (cnt+ 1), stat = alterlist(rec1->qual,cnt), rec1->qual[cnt].info = trim(d.info_char,3)
  WITH nocounter
 ;end select
 SET dm_err->eproc = "Executing DM_INFO Session Parameter command"
 IF (value(size(rec1->qual,5)) > 0)
  FOR (x = 1 TO value(size(rec1->qual,5)))
   SET temp->temp_str = concat("RDB ASIS(^",rec1->qual[x].info,"^) GO")
   CALL parser(temp->temp_str,1)
  ENDFOR
 ENDIF
#exit_script
END GO
