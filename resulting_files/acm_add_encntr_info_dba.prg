CREATE PROGRAM acm_add_encntr_info:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->encntr_info_qual[xref->add[index].idx].encntr_info_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(encounter_seq,nextval)
     FROM dual
     DETAIL
      acm_request->encntr_info_qual[xref->add[index].idx].encntr_info_id = cnvtreal(nextseqnum),
      reply->encntr_info_qual[xref->add[index].idx].encntr_info_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->encntr_info_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->encntr_info_qual[xref->add[index].idx].encntr_info_id = acm_request->encntr_info_qual[
    xref->add[index].idx].encntr_info_id
   ENDIF
   SET reply->encntr_info_qual[xref->add[index].idx].status = 0
   IF ((acm_request->encntr_info_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->encntr_info_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->encntr_info_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->encntr_info_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   encntr_info e
  SET e.encntr_info_id = acm_request->encntr_info_qual[xref->add[d.seq].idx].encntr_info_id, e
   .beg_effective_dt_tm = cnvtdatetime(acm_request->encntr_info_qual[xref->add[d.seq].idx].
    beg_effective_dt_tm), e.chartable_ind = acm_request->encntr_info_qual[xref->add[d.seq].idx].
   chartable_ind,
   e.contributor_system_cd =
   IF ((acm_request->encntr_info_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0)) acm_request
    ->encntr_info_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , e.encntr_id =
   IF ((acm_request->encntr_info_qual[xref->add[d.seq].idx].encntr_id > 0)) acm_request->
    encntr_info_qual[xref->add[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->encntr_info_qual[xref->add[d.seq].idx].encntr_idx].
    encntr_id
   ENDIF
   , e.end_effective_dt_tm = cnvtdatetime(acm_request->encntr_info_qual[xref->add[d.seq].idx].
    end_effective_dt_tm),
   e.info_sub_type_cd = acm_request->encntr_info_qual[xref->add[d.seq].idx].info_sub_type_cd, e
   .info_type_cd = acm_request->encntr_info_qual[xref->add[d.seq].idx].info_type_cd, e.internal_seq
    = acm_request->encntr_info_qual[xref->add[d.seq].idx].internal_seq,
   e.long_text_id = acm_request->encntr_info_qual[xref->add[d.seq].idx].long_text_id, e.priority_seq
    = acm_request->encntr_info_qual[xref->add[d.seq].idx].priority_seq, e.value_cd = acm_request->
   encntr_info_qual[xref->add[d.seq].idx].value_cd,
   e.value_dt_tm =
   IF ((acm_request->encntr_info_qual[xref->add[d.seq].idx].value_dt_tm > 0)) cnvtdatetime(
     acm_request->encntr_info_qual[xref->add[d.seq].idx].value_dt_tm)
   ELSE null
   ENDIF
   , e.value_numeric = acm_request->encntr_info_qual[xref->add[d.seq].idx].value_numeric, e
   .active_ind =
   IF ((((acm_request->encntr_info_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((acm_request
   ->encntr_info_qual[xref->add[d.seq].idx].active_status_cd=reqdata->active_status_cd))) ) 1
   ELSE 0
   ENDIF
   ,
   e.active_status_cd =
   IF ((acm_request->encntr_info_qual[xref->add[d.seq].idx].active_status_cd > 0.0)) acm_request->
    encntr_info_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (e)
  WITH nocounter, status(reply->encntr_info_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->encntr_info_qual[xref->add[index].idx].status=0))
 ...
 ENDFOR
 IF (acm_hist_ind=1)
  EXECUTE acm_add_encntr_info_hist
