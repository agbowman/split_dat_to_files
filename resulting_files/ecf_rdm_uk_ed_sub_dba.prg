CREATE PROGRAM ecf_rdm_uk_ed_sub:dba
 ...
 SUBROUTINE kia_script_inc(pkg,ccl_obj)
   ...
   CALL parser(obj_prs)
 END ;Subroutine
 DECLARE kia_nomen_csv_filename = vc WITH public, constant("uk_ed_sub_nomen.csv")
 DECLARE kia_concept_filename = vc WITH public, constant("uk_ed_sub_concept.csv")
 DECLARE kia_cur_ver_beg_dt = vc WITH public, noconstant("")
 SET kia_tbl_desc = "UK ED SUBSET"
 SET kia_short_desc = "UK ED SUBSET"
 SET kia_rdm_log_name = "ecf_rdm_uk_ed_sub.log"
 SET kia_rdm_nbr = 10016
 SET kia_rdm_vocab_mean = "SNMUKEMED"
 SET readme_data->status = "F"
 ...
 EXECUTE kia_dm_dbimport "cer_install:uk_ed_sub_version.csv", "kia_get_rdm_vars", 100,
 0
 CALL kia_chk_script_failure("KIA_GET_RDM_VARS")
 SET start_time_var = cnvtdatetime(curdate,curtime3)
 CALL log_sub(concat("ecf_rdm_uk_ed_sub Begin> ",format(cnvtdatetime(curdate,curtime3),
 ...
 CALL kia_chk_status_info("LOAD UK ED Subset NOMEN BASELINE DATA","check")
 IF (kia_obj_suc_ind=0)
  ...
  CALL kia_chk_error(1)
  COMMIT
  EXECUTE kia_sqlldr "nomenclature_load_ns", kia_nomen_csv_filename, 1
  CALL kia_chk_script_failure("kia_sqlldr")
  CALL kia_set_status_success("LOAD UK ED Subset NOMEN BASELINE DATA")
 ENDIF
 ...
 ENDIF
 CALL kia_chk_status_info("LOAD UK ED Subset CONCEPT BASELINE DATA","check")
 IF (kia_obj_suc_ind=0)
  CALL echo("DELETING CMT_CONCEPT_LOAD_NS ROWS...")
  DELETE  FROM cmt_concept_load_ns c
   WHERE ((c.concept_cki="SNMUKEMED*") OR (c.concept_cki="SNOMED*"))
   WITH nocounter
  ;end delete
  CALL kia_chk_error(1)
  COMMIT
  CALL kia_load_file(kia_concept_filename,"kia_imp_concept_load_ns","kia_imp_concept_load_ns.log")
  CALL kia_set_status_success("LOAD UK ED Subset CONCEPT BASELINE DATA")
 ENDIF
