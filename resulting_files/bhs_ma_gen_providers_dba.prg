CREATE PROGRAM bhs_ma_gen_providers:dba
 ...
 DECLARE primary_rn_cd = f8 WITH constant(uar_get_code_by("DISPLAYKEY",72,"PRIMARYNURSE")), protect
 DECLARE pid = f8
 DECLARE language = f8 WITH public, constant(uar_get_code_by("displaykey",72,"LANGUAGESPOKENV001"))
 DECLARE mandatedreport = f8 WITH public, constant(uar_get_code_by("displaykey",72,"MANDATEDREPORT"))
 DECLARE protectiveagencies = f8 WITH public, constant(uar_get_code_by("displaykey",72,
   "PROTECTIVEAGENCIES"))
 DECLARE protectivecomments = f8 WITH public, constant(uar_get_code_by("displaykey",72,
   "PROTECTIVECOMMENTS"))
 DECLARE safetyabuse = f8 WITH public, constant(uar_get_code_by("displaykey",72,"SAFETYABUSE"))
 DECLARE visitorrestriction = f8 WITH public, constant(uar_get_code_by("displaykey",72,
   "VISITORRESTRICTIONS"))
 SET resident_oe_field_id = 963911.00
 SET covering_resident_dta = uar_get_code_by("displaykey",72,"COVERINGPHYSICIAN")
 SET covering_resident_pager_dta = 156227920.00
 ...
 SELECT INTO "nl:"
  ...
  HEAD ce.event_cd
   ...
   dischargenursingfacilities, dischargevnahomecare))
    ...
    IF (ce.event_class_cd=223)
     providers->disch_plan[c].disch_content = concat(substring(7,2,ce.result_val),"/",substring(9,2,
       ce.result_val),"/",substring(3,4,ce.result_val),
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ce
   WHERE ce.encntr_id=eid
    AND ce.event_title_text="Social Work Consult*"
    AND ce.view_level=1)
   JOIN (pr
   WHERE pr.person_id=ce.updt_id)
  ORDER BY ce.event_start_dt_tm
  DETAIL
   providers->sworker = pr.name_full_formatted
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  FROM clinical_event ce,
 ...
 SELECT INTO "nl:"
  ...
  PLAN (ce
   WHERE ce.encntr_id=eid
    AND ((ce.event_cd+ 0) IN (mandatedreport, protectiveagencies, protectivecomments, safetyabuse,
   visitorrestriction,
   language))
    AND ce.view_level=1
    AND ce.valid_until_dt_tm > sysdate)
  ORDER BY ce.event_start_dt_tm
  DETAIL
   IF (ce.event_cd=mandatedreport)
    info->mandrpt = trim(ce.event_tag)
   ELSEIF (ce.event_cd=protectiveagencies)
    info->protagency = trim(ce.event_tag)
   ELSEIF (ce.event_cd=protectivecomments)
    info->protcmt = trim(ce.event_tag)
   ELSEIF (ce.event_cd=safetyabuse)
    info->safetyabuse = trim(ce.event_tag)
   ELSEIF (ce.event_cd=language)
    info->language = trim(ce.event_tag)
   ELSEIF (ce.event_cd=visitorrestriction)
    info->visitrest = trim(ce.result_val)
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  PLAN (ce
   WHERE ce.encntr_id=eid
    AND ((ce.event_cd+ 0) IN (primary_rn_cd))
    AND ce.view_level=1
    AND ce.valid_until_dt_tm > sysdate)
  ORDER BY ce.event_start_dt_tm
  DETAIL
   IF (ce.event_cd=primary_rn_cd)
    providers->rn = trim(ce.event_tag)
   ENDIF
  WITH nocounter
 ;end select
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (sa.active_ind=1
    AND sa.purge_ind=0
    AND p="BHS RN")
    providers->primaryrn = trim(p.name_full_formatted)
   ENDIF
  WITH format(date,";;q")
 ;end select
 SET x = 1
 ...
 SET lidx = (lidx+ 1)
 SET stat = alterlist(drec->line_qual,lidx)
 SET temp_disp1 = concat("Primary RN: ",wr,providers->primaryrn)
 SET drec->line_qual[lidx].disp_line = concat(rh2b,trim(temp_disp1),reol)
 SET lidx = (lidx+ 1)
 SET stat = alterlist(drec->line_qual,lidx)
 ...
 IF (size(providers->consultsord,5) > 0)
  ...
  IF (size(providers->consultsord,5) > 1)
   FOR (x = 2 TO size(providers->consultsord,5))
 ...
 FOR (x = 1 TO size(providers->disch_plan,5))
   ...
   SET drec->line_qual[lidx].disp_line = concat(rh2b,trim(temp_disp1),reol)
 ENDFOR
 IF ((providers->sworker > " "))
  SET lidx = (lidx+ 1)
  SET stat = alterlist(drec->line_qual,lidx)
  SET temp_disp1 = concat("Social Worker: ",wr,providers->sworker)
  SET drec->line_qual[lidx].disp_line = concat(rh2b,trim(temp_disp1),reol)
 ENDIF
 IF ((info->mandrpt > " "))
  SET lidx = (lidx+ 1)
  SET stat = alterlist(drec->line_qual,lidx)
  SET temp_disp1 = concat("Mandated Report: ",wr,info->mandrpt)
  SET drec->line_qual[lidx].disp_line = concat(rh2b,trim(temp_disp1),reol)
 ENDIF
 IF ((info->protagency > " "))
  SET lidx = (lidx+ 1)
  SET stat = alterlist(drec->line_qual,lidx)
  SET temp_disp1 = concat("Protective Agencies: ",wr,info->protagency)
  SET drec->line_qual[lidx].disp_line = concat(rh2b,trim(temp_disp1),reol)
 ENDIF
 IF ((info->protcmt > " "))
  SET lidx = (lidx+ 1)
  SET stat = alterlist(drec->line_qual,lidx)
  SET temp_disp1 = concat("Protective Comments: ",wr,info->protcmt)
  SET drec->line_qual[lidx].disp_line = concat(rh2b,trim(temp_disp1),reol)
 ENDIF
 IF ((info->safetyabuse > " "))
  SET lidx = (lidx+ 1)
  SET stat = alterlist(drec->line_qual,lidx)
  SET temp_disp1 = concat("Safety/Abuse: ",wr,info->safetyabuse)
  SET drec->line_qual[lidx].disp_line = concat(rh2b,trim(temp_disp1),reol)
 ENDIF
 IF ((info->visitrest > " "))
  SET lidx = (lidx+ 1)
  SET stat = alterlist(drec->line_qual,lidx)
  SET temp_disp1 = concat("Visitor Restriction: ",wr,info->visitrest)
  SET drec->line_qual[lidx].disp_line = concat(rh2b,trim(temp_disp1),reol)
 ENDIF
 SET lidx = (lidx+ 1)
 SET stat = alterlist(drec->line_qual,lidx)
 SET temp_disp1 = concat("Language: ",wr,info->language)
 SET drec->line_qual[lidx].disp_line = concat(rh2b,trim(temp_disp1),reol)
 FOR (x = 1 TO lidx)
