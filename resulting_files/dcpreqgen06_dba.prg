CREATE PROGRAM dcpreqgen06:dba
 RECORD request(
   1 person_id = f8
   1 print_prsnl_id = f8
   1 order_qual[*]
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   ...
   format(e.reg_dt_tm,"@SHORTDATE"), orders->dischg_dt = format(e.disch_dt_tm,"@SHORTDATE")
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   HEAD PAGE
    ...
    "{CPI/10}{POS/200/8}", orders->facility, row + 1,
    "{CPI/12}{POS/44/90}", "{box/85/1}", "{CPI/12}{POS/51/100}",
    "Patient Information", row + 1, "{CPI/12}{POS/50/125}",
    "Patient:", "{CPI/12}{POS/125/125}",
    CALL print(trim(orders->name,3)),
    "{CPI/12}{POS/380/125}", "MRN#:", "{CPI/12}{POS/425/125}",
    CALL print(trim(orders->mrn,3)), row + 1, "{CPI/12}{POS/50/138}",
    "Location:", "{CPI/12}{POS/125/138}",
    CALL print(trim(orders->location,3)),
    "{CPI/12}{POS/380/138}", "Fin#:", "{CPI/12}{POS/425/138}",
    CALL print(trim(orders->fnbr,3)), row + 1, col 1,
    "{CPI/12}{POS/50/152}", "Attending MD:", "{CPI/12}{POS/125/152}",
    CALL print(trim(orders->attending,3)), row + 1, col 1,
    "{CPI/12}{POS/50/172}", "LOS:", "{CPI/12}{POS/80/172}",
    CALL print(orders->los), " Days", "{CPI/12}{POS/220/172}",
    "Admit Date:", "{CPI/12}{POS/280/172}",
    CALL print(trim(orders->admit_dt,3)),
    "{CPI/12}{POS/380/172}", "Patient Type:", "{CPI/12}{POS/450/172}",
    CALL print(trim(orders->pat_type,3)), row + 1, col 1,
    "{CPI/12}{POS/50/192}", "Age:", "{CPI/12}{POS/80/192}",
    CALL print(trim(orders->age,3)), "{CPI/12}{POS/220/192}", "DOB:",
    "{CPI/12}{POS/250/192}",
    CALL print(trim(orders->dob,3)), "{CPI/12}{POS/380/192}",
    "Height:", "{CPI/12}{POS/425/192}",
    CALL print(trim(orders->height,3)),
    row + 1, col 1, "{CPI/12}{POS/50/206}",
    "Sex:", "{CPI/12}{POS/100/206}",
    CALL print(trim(orders->sex,3)),
    "{CPI/12}{POS/380/206}", "Weight:", "{CPI/12}{POS/425/206}",
    CALL print(trim(orders->weight,3)), row + 1, col 1,
    "{CPI/12}{POS/50/226}", "Allergies:"
    IF ((allergy->line_cnt > 0))
     FOR (zz = 1 TO allergy->line_cnt)
      IF (zz > 1)
       "{CPI/12}{POS/125/238}", allergy->line_qual[zz].line
      ELSE
       "{CPI/12}{POS/125/226}", allergy->line_qual[zz].line
      ENDIF
      ,row + 1
     ENDFOR
    ENDIF
    row + 1, col 1, "{CPI/12}{POS/50/252}",
    "Primary Diagnosis:"
    IF (textlen(orders->admit_diagnosis) > 55)
     "{CPI/12}{POS/145/252}",
     CALL print(trim(concat(substring(1,52,orders->admit_diagnosis),"..."))), row + 1
    ELSE
     "{CPI/12}{POS/145/252}",
     CALL print(orders->admit_diagnosis), row + 1
    ENDIF
    "{CPI/12}{POS/44/272}", "{box/85/1}", "{CPI/12}{POS/50/282}",
    "Order Details"
    IF (saved_pos > 0)
     "{CPI/12}{POS/50/307}", "Procedure: ", "{CPI/12}{POS/110/307}",
     CALL print(trim(orders->qual[saved_pos].mnemonic,3)), "{B}{CPI/10}{POS/230/28}",
     CALL print(concat(orders->qual[saved_pos].catalog," REQUISITION")),
     "{endb}", row + 1, "{CPI/10}{POS/1/48}",
     CALL center(concat("*******************",trim(uar_get_code_display(orders->qual[saved_pos].
        action_type_cd)),"*******************"),1,190), row + 1, "{pos/50/720}{f/6}Order ID #: ",
     "{f/4}", orders->qual[saved_pos].order_id"#########", row + 1,
     "{CPI/12}{POS/50/321}", "Accession: ", "{CPI/12}{POS/110/321}",
     CALL print(trim(orders->qual[saved_pos].accession,3)), saved_pos = 0
    ENDIF
   DETAIL
    FOR (vv = 1 TO value(ord_cnt))
      IF ((orders->qual[vv].display_ind=1))
       IF (first_page="N")
        BREAK
       ENDIF
       first_page = "N", row + 1, "{CPI/12}{POS/50/307}",
       "Procedure: ", "{CPI/12}{POS/110/307}",
       CALL print(trim(orders->qual[vv].mnemonic,3)),
       "{B}{CPI/10}{POS/230/28}",
       CALL print(concat(orders->qual[vv].catalog," REQUISITION")), "{endb}",
       row + 1, "{CPI/10}{POS/1/48}",
       CALL center(concat("*******************",trim(uar_get_code_display(orders->qual[vv].
          action_type_cd)),"*******************"),1,190),
       row + 1, "{CPI/12}{POS/50/321}", "Accession: ",
       "{CPI/12}{POS/110/321}",
       CALL print(trim(orders->qual[vv].accession,3)), row + 1,
       "{pos/50/720}{f/6}Order ID #: ", "{f/4}", orders->qual[vv].order_id"#########",
       row + 1, xcol = 50, xrow = 356
       FOR (fsub = 1 TO 31)
         FOR (ww = 1 TO orders->qual[vv].d_cnt)
          IF ((((orders->qual[vv].d_qual[ww].group_seq=fsub)) OR (fsub=31
           ...
           IF (textlen(trim(orders->qual[vv].d_qual[ww].value,3)) > 0)
            ...
            "  ", row + 1, xcol = 212
            IF (textlen(orders->qual[vv].d_qual[ww].label_text) > 42)
             xrow = (xrow+ 12)
            ENDIF
            IF ((orders->qual[vv].d_qual[ww].value_cnt > 1))
             FOR (dsub = 1 TO orders->qual[vv].d_qual[ww].value_cnt)
               CALL print(calcpos(xcol,xrow)), "{b}", orders->qual[vv].d_qual[ww].value_qual[dsub].
               value_line,
               "{endb}", row + 1, xrow = (xrow+ 12)
               IF (xrow > 680
                AND (dsub < orders->qual[vv].d_qual[ww].value_cnt))
                CALL print(calcpos(xcol,xrow)), "{b}", "**Continued on next page**",
                "{endb}", saved_pos = vv, BREAK,
                xcol = 50, xrow = 356, "{CPI/13}",
                CALL print(calcpos(xcol,xrow)),
             ...
             ENDFOR
            ELSE
             CALL print(calcpos(xcol,xrow)), "{b}", orders->qual[vv].d_qual[ww].value,
             "{endb}", row + 1, xrow = (xrow+ 12)
            ENDIF
            xcol = 50
           ENDIF
       ...
       ENDIF
       "{CPI/12}",
       CALL print(calcpos(50,xrow)), "Ordering MD:",
       CALL print(calcpos(130,xrow)),
       CALL print(trim(orders->qual[vv].order_dr,3)),
       CALL print(calcpos(350,xrow)),
       "Order Date/Time:",
       CALL print(calcpos(450,xrow)),
       CALL print(trim(orders->qual[vv].signed_dt,3)),
       row + 1, xrow = (xrow+ 12),
       CALL print(calcpos(50,xrow)),
       "Ordering Type:",
       CALL print(calcpos(130,xrow)),
       CALL print(trim(orders->qual[vv].type,3)),
       row + 1, xrow = (xrow+ 12),
       CALL print(calcpos(50,xrow)),
       "Ordering entered by:",
       CALL print(calcpos(160,xrow)),
       CALL print(trim(orders->qual[vv].enter_by,3)),
       CALL print(calcpos(350,xrow)), "Printed Date/Time:",
       CALL print(calcpos(450,xrow)),
       curdate, " ", curtime,
       row + 1, xrow = (xrow+ 15),
       CALL print(calcpos(44,xrow)),
       "{box/85/1}", xrow = (xrow+ 10),
       CALL print(calcpos(50,xrow)),
       "Ordering Comments", xrow = (xrow+ 25)
       IF ((orders->qual[vv].com_ln_cnt > 0))
        IF ((orders->qual[vv].com_ln_cnt > 7))
         o_cnt = 7
        ELSE
         o_cnt = orders->qual[vv].com_ln_cnt
        ENDIF
        xcol = 70
        FOR (w = 1 TO o_cnt)
          CALL print(calcpos(xcol,xrow)), "{b}", orders->qual[vv].com_ln_qual[w].com_line,
          "{endb}", xrow = (xrow+ 15)
        ENDFOR
        IF ((orders->qual[vv].com_ln_cnt > 7))
         row + 1,
         CALL print(calcpos(xcol,xrow)), "{cpi/12}",
         "**** Please check chart for further comments ****", row + 1
        ENDIF
       ENDIF
      ENDIF
    ENDFOR
