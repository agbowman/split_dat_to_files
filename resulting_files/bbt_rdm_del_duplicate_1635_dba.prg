...
#start_script
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   row_cnt = 0, stat = alterlist(codeset1635->codevaluerows,5)
  DETAIL
   row_cnt = (row_cnt+ 1)
   IF (mod(row_cnt,5)=1
    AND row_cnt != 1)
   ...
   ENDIF
   codeset1635->codevaluerows[row_cnt].code_value = cv.code_value, codeset1635->codevaluerows[row_cnt
   ].cdf_meaning = cv.cdf_meaning, codeset1635->codevaluerows[row_cnt].active_ind = cv.active_ind,
   bnoupdates = 0
  FOOT REPORT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d_cv)
   ...
   JOIN (d_sd)
   JOIN (sd
   WHERE sd.bb_processing_cd=cv.code_value)
...
#exit_script
 IF (bnoupdates=1)
  ...
  CALL logmsg("No duplicate rows on 1635 code_set inactivated.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
 ELSE
