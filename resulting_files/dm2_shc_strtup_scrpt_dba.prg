CREATE PROGRAM dm2_shc_strtup_scrpt:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(srvlist->scnt,- (1))=- (1))
  AND (validate(srvlist->scnt,- (2))=- (2)))
  RECORD srvlist(
    1 scnt = i4
    1 srv[*]
      2 server_name = vc
  ...
  SET srvlist->scnt = 0
 ENDIF
 DECLARE dsss_status = c1 WITH private, noconstant("X")
 DECLARE dsss_fatal_err_ind = i2 WITH private, noconstant(0)
 DECLARE dsss_dcl_cmd = vc WITH private, noconstant("NOT_SET")
 DECLARE dsss_debug_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm2_shc_startup",".log","SYSTEM HEALTH CHECK STARTUP SCRIPT LOG FILE")=0)
  SET dsss_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Checking the configuration of startup scripts for servers."
 CALL disp_msg(" ",dm_err->logfile,0)
 ...
 ENDIF
 IF (cursys="AXP")
  SET dsss_dcl_cmd = concat("mcr cer_exe:alter_server -domain ",dshc_env_info->domain_name,
   ' -verbose -display "startup script"')
 ELSEIF (cursys="AIX")
  SET dsss_dcl_cmd = concat("$cer_exe/alter_server -domain ",dshc_env_info->domain_name,
   ' -verbose -display "startup script"')
 ENDIF
 IF (dm2_push_dcl(dsss_dcl_cmd)=0)
  SET dsss_status = "E"
 ENDIF
 IF (dsss_status != "E")
  FREE DEFINE rtl2
  ...
  SET logical file_loc value(dm_err->errfile)
  DEFINE rtl2 "file_loc"
  SET dm_err->eproc = "Parsing output for server startup script settings."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,0)
  SELECT INTO "nl:"
   r.line
   FROM rtl2t r
   HEAD REPORT
    rpt_srvid_start_pos = 0, rpt_srvid_end_pos = 0, rpt_srtscr_start_pos = 0,
    rpt_srtscr_end_pos = 0
   DETAIL
    IF (cursys="AIX")
     rpt_srvid_start_pos = findstring("server #",r.line)
     IF (rpt_srvid_start_pos > 0)
      rpt_srvid_start_pos += 8, rpt_srvid_end_pos = findstring(" has ",r.line), srvlist->scnt += 1,
      stat = alterlist(srvlist->srv,srvlist->scnt), srvlist->srv[srvlist->scnt].server_id = substring
      (rpt_srvid_start_pos,(rpt_srvid_end_pos - rpt_srvid_start_pos),r.line), rpt_srtscr_start_pos =
      (findstring(" = ",r.line)+ 2),
      rpt_srtscr_end_pos = size(trim(r.line,3)), srvlist->srv[srvlist->scnt].startup_script =
      substring(rpt_srtscr_start_pos,((rpt_srtscr_end_pos - rpt_srtscr_start_pos)+ 1),trim(r.line,3))
     ENDIF
    ELSE
     rpt_srvid_start_pos = findstring("server #",r.line)
     IF (rpt_srvid_start_pos > 0)
      rpt_srvid_start_pos += 8, rpt_srvid_end_pos = findstring(' "',r.line), srvlist->scnt += 1,
      stat = alterlist(srvlist->srv,srvlist->scnt), srvlist->srv[srvlist->scnt].server_id = substring
      (rpt_srvid_start_pos,(rpt_srvid_end_pos - rpt_srvid_start_pos),r.line)
     ELSE
      rpt_srtscr_start_pos = findstring(" = ",r.line)
      IF (rpt_srtscr_start_pos > 0)
       rpt_srtscr_end_pos = size(trim(r.line,3)), srvlist->srv[srvlist->scnt].startup_script =
       substring(rpt_srtscr_start_pos,((rpt_srtscr_end_pos - rpt_srtscr_start_pos)+ 1),trim(r.line,3)
        )
       IF (cnvtlower(srvlist->srv[srvlist->scnt].startup_script) IN ("test", "debug", "esodbg"))
        dsss_debug_ind = 1
       ENDIF
      ENDIF
     ENDIF
 ...
 ENDIF
 IF ((dm_err->debug_flag > 1))
  CALL echorecord(srvlist)
 ENDIF
 IF ((((dm_err->err_ind=1)) OR (dsss_status="E")) )
  SET dsss_status = "E"
 ELSEIF (dsss_debug_ind=1)
  SET dsss_status = "R"
 ELSE
  SET dsss_status = "G"
 ENDIF
 IF (dshc_add_check_to_rs("SERVERS W/ TEST STARTUP SCRIPTS","Startup Script",0,dsss_status,
  dshc_ctf_millennium)=0)
  SET dsss_fatal_err_ind = 1
  GO TO exit_script
 ENDIF
 IF (dsss_status IN ("E", "G"))
  GO TO exit_script
 ENDIF
 FOR (dsss_cnt = 1 TO srvlist->scnt)
   IF ((srvlist->srv[dsss_cnt].startup_script IN ("test", "debug", "esodbg")))
    ...
    concat("Server #",srvlist->srv[dsss_cnt].server_id," -- startup script = ",srvlist->srv[dsss_cnt]
     .startup_script)
   ENDIF
 ENDFOR
 SET dshc->check[dshc->chk_cnt].check_desc_text =
 'The following server definitions have the "startup script" property containing the words "test", "debug" or "esodbg":'
#exit_script
 SET dm_err->eproc = "Finished checking server startup scripts."
 CALL final_disp_msg("dm2_shc_startup")
 IF (dsss_fatal_err_ind=0)
  SET dm_err->err_ind = 0
 ENDIF
 FREE RECORD srvlist
END GO
