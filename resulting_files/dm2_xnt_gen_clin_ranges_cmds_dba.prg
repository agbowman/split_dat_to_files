CREATE PROGRAM dm2_xnt_gen_clin_ranges_cmds:dba
 ...
 SUBROUTINE dpq_cleanup_stranded_procs(dcsp_proc_type,dcsp_maxcommit)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dxgc_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dxgc_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dxgc_idx = i4 WITH protect, noconstant(0)
 DECLARE dxgc_idx2 = i4 WITH protect, noconstant(0)
 FREE RECORD dxgc_tbl
 RECORD dxgc_tbl(
   1 cnt = i4
   1 qual[*]
     2 tbl_name = vc
 ...
 IF (check_logfile("dm2_xnt_gen_crc",".log","DM2_XNT_GEN_CRC LogFile...") != 1)
  GO TO exit_program
 ENDIF
 SET dm2_process_event_rs->status = dpl_executing
 SET dm2_process_rs->process_name = dpl_xnt_clinical_ranges
 IF (dm2_process_log_row(dpl_xnt_clinical_ranges,dpl_execution,dpl_no_prev_id,1)=0)
  GO TO exit_program
 ENDIF
 SET dxgc_log_id = dm2_process_event_rs->dm_process_event_id
 SET dm_err->eproc = "Obtain range processed tables"
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dm_info di
  ...
  (SELECT DISTINCT
   ...
   WHERE t.purge_type_flag=5
    AND t.template_nbr IN (
   (SELECT
    p.template_nbr
    FROM dm_purge_template p
    WHERE cnvtupper(p.program_str)="XNT"
     AND p.active_ind=1
     AND (p.schema_dt_tm=
    (SELECT
     max(p1.schema_dt_tm)
     FROM dm_purge_template p1
     WHERE p1.template_nbr=p.template_nbr))))))
  HEAD REPORT
   dxgc_tbl->cnt = 0
  DETAIL
   dxgc_tbl->cnt = (dxgc_tbl->cnt+ 1), stat = alterlist(dxgc_tbl->qual,dxgc_tbl->cnt), dxgc_tbl->
   qual[dxgc_tbl->cnt].tbl_name = trim(di.info_name),
   dxgc_tbl->qual[dxgc_tbl->cnt].clm_name = trim(di.info_char), dxgc_tbl->qual[dxgc_tbl->cnt].rng_num
    = di.info_number
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF (curqual != 0)
  SET dm_err->eproc = "Obtain custom ranges"
  CALL disp_msg(" ",dm_err->logfile,0)
  SELECT INTO "nl:"
   FROM dm_info di
   WHERE di.info_domain="DM2_XNT_CUSTOM_RANGE_NBR"
    AND expand(dxgc_idx,1,dxgc_tbl->cnt,di.info_name,dxgc_tbl->qual[dxgc_idx].tbl_name)
   DETAIL
    dxgc_idx2 = locateval(dxgc_idx,1,dxgc_tbl->cnt,di.info_name,dxgc_tbl->qual[dxgc_idx].tbl_name)
    IF (dxgc_idx2 > 0)
     dxgc_tbl->qual[dxgc_idx2].rng_num = di.info_number
    ENDIF
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   ...
   GO TO exit_program
  ENDIF
  FOR (dxgc_idx = 1 TO dxgc_tbl->cnt)
    SET dpq_process_queue->process_type = dpq_clinical_ranges
    ...
    SET dpq_process_queue->object_name = dxgc_tbl->qual[dxgc_idx].tbl_name
    SET dpq_process_queue->routine_tasks_ind = 1
    SET dpq_process_queue->operation_txt = concat("execute dm2_get_clinical_ranges 'XNT','V500','",
     dxgc_tbl->qual[dxgc_idx].tbl_name,"','",dxgc_tbl->qual[dxgc_idx].clm_name,"',",
     trim(cnvtstring(dxgc_tbl->qual[dxgc_idx].rng_num))," go")
    SET dpq_process_queue->priority = 450
...
#exit_program
 ...
 SET dxgc_err_ind = dm_err->err_ind
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_xnt_clinical_ranges,dpl_execution,dxgc_log_id,1)
 SET dm2_process_rs->process_name = ""
 SET dm_err->err_ind = dxgc_err_ind
 SET dm_err->eproc = "Ending dm2_xnt_gen_clin_ranges_cmds"
 CALL final_disp_msg("dm2_xnt_gen_crc")
END GO
