CREATE PROGRAM dm2_mig_custccl_merge_minidic
 ...
 SUBROUTINE retrieve_data(sbr_srch_str,sbr_sprtr,sbr_rd_str)
   ...
   ENDIF
 END ;Subroutine
 DECLARE dmccmm_logfile_prefix = vc WITH protect, constant("dm2_mig_cc_mergedic")
 DECLARE dmccmm_type = vc WITH protect, noconstant("")
 DECLARE dmccmm_cont = i2 WITH protect, noconstant(1)
 DECLARE dmccmm_env = vc WITH protect, noconstant("")
 DECLARE dmccmm_node = vc WITH protect, noconstant("")
 DECLARE dmccmm_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccmm_os_type = vc WITH protect, noconstant("")
 DECLARE dmccmm_master_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccmm_master_node = vc WITH protect, noconstant("")
 DECLARE dmccmm_master_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccmm_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmccmm_loc = vc WITH protect, noconstant("")
 DECLARE dmccmm_file = vc WITH protect, noconstant("")
 DECLARE dmccmm_cmd = vc WITH protect, noconstant("")
 DECLARE dmccmm_tgt_id = f8 WITH protect, noconstant(0.0)
 DECLARE dmccmm_tgt_os = vc WITH protect, noconstant("")
 DECLARE dmccmm_tgt_env = vc WITH protect, noconstant("")
 DECLARE dmccmm_tgt_list = vc WITH protect, noconstant("")
 DECLARE dmccmm_datetime = vc WITH protect, noconstant("")
 DECLARE dmccmm_ok = i2 WITH protect, noconstant(0)
 DECLARE dmccmm_ccluserdir = vc WITH protect, noconstant("")
 SET width = 132
 IF (check_logfile(dmccmm_logfile_prefix,".log","dm2_mig_custccl_merge_minidic")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_mig_custccl_merge_minidic"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmccmm_env = trim(logical("environment"))
 SET dmccmm_node = trim(curnode)
 SET dmccmm_os_type = trim(cursys2)
 SET dm_err->eproc = concat("Obtaining environment id for current environment: ",dmccmm_env)
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  FROM dm_environment de
  WHERE de.environment_name=cnvtupper(dmccmm_env)
  DETAIL
   dmccmm_env_id = de.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating execution from TARGET"
 SELECT INTO "nl:"
  FROM dma_env_node_attr dena,
   dma_env_node dne
  PLAN (dne
   WHERE dne.node_name=dmccmm_node
    AND dne.environment_id=dmccmm_env_id)
   JOIN (dena
   WHERE dena.dma_env_node_id=dne.dma_env_node_id
    AND dena.attr_name="CCMIG_SETUPTYPE")
  DETAIL
   IF (dena.attr_value_txt="TARGET")
    dmccmm_tgt_id = dne.dma_env_node_id, dmccmm_cont = 1
   ELSE
    dmccmm_cont = 0
   ENDIF
  WITH nocounter
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF (((dmccmm_cont=0) OR (dmccmm_tgt_id=0)) )
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "This script is not being run from a valid TARGET"
  SET dm_err->user_action = "Re-run this script from a valid TARGET node"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Validating existence of ccldic_vms_unix_post_merge"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 IF (checkprg("CCLDIC_VMS_UNIX_POST_MERGE")=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "The ccldic_vms_unix_post_merge script does not exist"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   dmccmm_master_id = dena.dma_env_node_id, dmccmm_master_node = dne.node_name, dmccmm_master_env_id
    = dne.environment_id
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("Current Environment: ",dmccmm_env))
  CALL echo(concat("Current Environment Id: ",cnvtstring(dmccmm_env_id)))
  CALL echo(concat("Current Node: ",dmccmm_node))
  CALL echo(concat("Current OS: ",dmccmm_os_type))
  CALL echo(concat("Current Target Id: ",cnvtstring(dmccmm_tgt_id)))
  CALL echo(concat("Current Master Id: ",cnvtstring(dmccmm_master_id)))
  CALL echo(concat("Current Master node: ",dmccmm_master_node))
  CALL echo(concat("Current Master Env Id: ",cnvtstring(dmccmm_master_env_id)))
 ENDIF
 SET dm_err->eproc =
 "Retrieve the CCMIGFILENAME row from dma_env_node_attr admin table for flat ccl binary minidic file"
 CALL disp_msg("",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dma_env_node_attr dena
  WHERE dena.dma_env_node_id=dmccmm_master_id
   AND dena.attr_name="CCMIGFILENAME"
  DETAIL
   dmccmm_file = dena.attr_value_txt
  WITH nocounter
 ;end select
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Unable to retrieve CCMIGFILENAME from admin table"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ELSE
  IF ((dm2_sys_misc->cur_os != "AXP"))
   SET dmccmm_loc = concat(trim(logical("cer_temp")),"/")
  ELSE
   SET dmccmm_loc = trim(logical("cer_temp"))
  ENDIF
  IF (dm2_findfile(build(dmccmm_loc,dmccmm_file))=0)
   IF (dm2_findfile(build(dmccmm_loc,cnvtupper(dmccmm_file)))=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat("Unable to to find ",dmccmm_file," in $cer_temp")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ELSE
    IF ((dm2_sys_misc->cur_os="AXP"))
     SET dmccmm_cmd = concat("rename ",dmccmm_loc,cnvtupper(dmccmm_file)," ",dmccmm_loc,
      dmccmm_file)
    ELSE
     SET dmccmm_cmd = concat("mv ",dmccmm_loc,cnvtupper(dmccmm_file)," ",dmccmm_loc,
      dmccmm_file)
    ENDIF
    IF (dm2_push_dcl(dmccmm_cmd)=0)
     GO TO exit_script
    ENDIF
   ENDIF
  ENDIF
  IF ((dm2_sys_misc->cur_os="AXP"))
   SET dmccmm_cmd = concat("rename ",dmccmm_loc,dmccmm_file," ",dmccmm_loc,
    "ccldictmp2.dat")
  ELSE
   SET dmccmm_cmd = concat("mv ",dmccmm_loc,dmccmm_file," ",dmccmm_loc,
    "ccldictmp2.dat")
  ENDIF
  IF (dm2_push_dcl(dmccmm_cmd)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 SET dm_err->eproc = "Set CCLUSERDIR to point to $cer_temp"
 CALL disp_msg("",dm_err->logfile,0)
 SET dmccmm_ccluserdir = trim(logical("ccluserdir"))
 SET modify ccluserdir value(logical("cer_temp"))
 IF ((dm_err->debug_flag > 0))
  CALL echo(value(logical("ccluserdir")))
 ENDIF
 SET dm_err->eproc = "Executing ccldic_vms_unix_post_merge"
 CALL disp_msg("",dm_err->logfile,0)
 EXECUTE ccldic_vms_unix_post_merge "N"
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Set CCLUSERDIR to point back to original value"
 CALL disp_msg("",dm_err->logfile,0)
 SET modify ccluserdir value(dmccmm_ccluserdir)
 IF ((dm_err->debug_flag > 0))
  CALL echo(value(logical("ccluserdir")))
 ENDIF
 SET dm_err->eproc = "Refreshing dma_env_node_dprotect with current set of scripts from dprotect"
 CALL disp_msg("",dm_err->logfile,0)
 EXECUTE dm2_mig_custccl_refresh
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "Ending dm2_mig_custccl_merge_minidic"
 CALL final_disp_msg(dmccmm_logfile_prefix)
END GO
