CREATE PROGRAM dm2_ref_data_mover_5740:dba
 ...
 DECLARE cust_cycle_name = vc
 DECLARE cust_err_val = i4
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_cycle_name = get_value(cust_tab_name,"CYCLE_NAME","TO")
 IF (cust_cycle_name="Formal Payment Plan*")
  SET dm2_ref_data_reply->error_ind = 1
  SET dm2_ref_data_reply->error_msg = "BADLOG"
  GO TO exit_5740
 ELSE
  CALL put_value(cust_tab_name,"CYCLE_NAME",cust_cycle_name)
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"STATEMENT_CYCLE_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"STATEMENT_CYCLE_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"STATEMENT_CYCLE_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"STATEMENT_CYCLE_ID")
     ELSE
      GO TO exit_5740
     ENDIF
    ELSE
   ...
   SET cust_from_cd = get_value(cust_tab_name,"STATEMENT_CYCLE_ID","FROM")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5740
   ELSE
    SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_cd)
   ENDIF
   SET cust_new_cd = get_value(cust_tab_name,"STATEMENT_CYCLE_ID","TO")
   IF ((dm2_ref_data_reply->error_ind=1))
    GO TO exit_5740
   ELSE
 ...
 IF (cust_idcd_check=1)
  SET cust_idcd_check = is_translated(cust_tab_name,"CYCLE_NAME")
  ...
  ELSE
   ...
   IF (cust_err_val=0)
    SET dm2_ref_data_reply->error_ind = 1
    SET dm2_ref_data_reply->error_msg = "This row wasn't translated because of the CYCLE_NAME"
   ENDIF
