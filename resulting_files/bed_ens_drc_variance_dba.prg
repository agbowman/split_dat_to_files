CREATE PROGRAM bed_ens_drc_variance:dba
 ...
 ENDIF
 IF ( NOT (validate(premise_list,0)))
  RECORD premise_list(
    1 qual[*]
      2 drc_premise_id = f8
  ) WITH protect
 ...
 IF ( NOT (validate(isfeaturetoggleenabled)))
  ...
  END ;Subroutine
 ENDIF
 DECLARE min_var = vc WITH protect, noconstant(" ")
 DECLARE max_var = vc WITH protect, noconstant(" ")
 DECLARE var_grouper = vc WITH protect, noconstant(" ")
 DECLARE premise_cnt = i4 WITH protect, noconstant(0)
 DECLARE v_ver_seq = i4 WITH protect, noconstant(0)
 DECLARE drc_cnt = i4 WITH protect, noconstant(0)
 CALL bedbeginscript(0)
 IF ((request->variance_flag=1))
  SET var_grouper = build2(var_grouper,"dfr.drc_group_id")
 ELSE
  SET var_grouper = build(var_grouper,request->drc_group_id)
 ENDIF
 IF ((request->update_min_range_flag=1))
  SET min_var = build(min_var,(request->update_min_range_value/ 100.0))
 ELSE
  SET min_var = build2(min_var,"ddr.min_variance_pct")
 ENDIF
 IF ((request->update_max_range_flag=1))
  SET max_var = build(max_var,(request->update_max_range_value/ 100.0))
 ELSE
  SET max_var = build2(max_var,"ddr.max_variance_pct")
 ENDIF
 SELECT INTO "nl:"
  FROM drc_form_reltn dfr,
   drc_premise dp
  PLAN (dfr
   WHERE dfr.drc_group_id=parser(var_grouper))
   JOIN (dp
  ...
  HEAD REPORT
   premise_cnt = 0
  DETAIL
   premise_cnt = (premise_cnt+ 1)
   IF (((premise_cnt=1) OR (mod(premise_cnt,10)=1)) )
    stat = alterlist(premise_list->qual,(premise_cnt+ 9))
   ENDIF
   premise_list->qual[premise_cnt].drc_premise_id = dp.drc_premise_id
  FOOT REPORT
   stat = alterlist(premise_list->qual,premise_cnt)
  WITH nocounter
 ;end select
 IF (size(premise_list->qual,5) > 0)
  UPDATE  FROM (dummyt d  WITH seq = size(premise_list->qual,5)),
    drc_dose_range ddr
   SET ddr.custom_ind = 1, ddr.min_variance_pct = parser(min_var), ddr.max_variance_pct = parser(
     max_var),
    ddr.updt_applctx = reqinfo->updt_applctx, ddr.updt_dt_tm = cnvtdatetime(curdate,curtime3), ddr
    .updt_id = reqinfo->updt_id,
    ddr.updt_task = reqinfo->updt_task
   PLAN (d)
    JOIN (ddr
    WHERE (ddr.drc_premise_id=premise_list->qual[d.seq].drc_premise_id))
   WITH nocounter
  ;end update
  DECLARE ddrcnt = i4 WITH protect, noconstant(0)
  FREE RECORD ddr_ids
  ...
  SELECT INTO "nl:"
   FROM (dummyt d1  WITH seq = size(premise_list->qual,5)),
    drc_dose_range ddr
   PLAN (d1)
    JOIN (ddr
    WHERE (ddr.drc_premise_id=premise_list->qual[d1.seq].drc_premise_id)
     AND ddr.active_ind=1)
   ORDER BY ddr.drc_dose_range_id
   HEAD ddr.drc_dose_range_id
    ddrcnt = (ddrcnt+ 1), stat = alterlist(ddr_ids->list,ddrcnt), ddr_ids->list[ddrcnt].
    dose_range_check_id = ddr.drc_dose_range_id
   WITH nocounter
  ;end select
  FOR (ddridx = 1 TO ddrcnt)
    CALL insert_version_row_drc_dose_range_ver_table(ddr_ids->list[ddridx].dose_range_check_id)
  ENDFOR
 ENDIF
#exit_script
