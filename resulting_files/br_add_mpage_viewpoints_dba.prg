CREATE PROGRAM br_add_mpage_viewpoints:dba
 ...
 SET readme_data->message = "Readme Failed: Starting <br_add_mpage_viewpoints.prg> script"
 ...
 IF (curqual=0)
  ...
  "Readme Succeeded: <br_add_mpage_viewpoints.prg> script, missing MPage solution"
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
 ...
 IF (curqual=0)
  SET readme_data->status = "S"
  SET readme_data->message =
  "Readme Succeeded: <br_add_mpage_viewpoints.prg> script, missing MPage content"
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  FROM br_step bs
  PLAN (bs
   WHERE bs.step_mean="MPAGEVIEWPOINTS")
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed checking existence of MPAGEVIEWPOINTS row: ",errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_step bs
   SET bs.step_mean = "MPAGEVIEWPOINTS", bs.step_disp = "ViewPoint Setup", bs.step_type = "IMPMAINT",
 ...
 SELECT INTO "nl:"
  ...
  WHERE bcir.br_client_id=1
   ...
   AND bcir.item_mean="MPAGEVIEWPOINTS"
  WITH nocounter
 ;end select
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to verify existence of STEP/MPAGEVIEWPOINTS row: ",errmsg
   )
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_client_item_reltn bcir
   SET bcir.br_client_item_reltn_id = seq(bedrock_seq,nextval), bcir.br_client_id = 1, bcir.item_type
     = "STEP",
    bcir.item_mean = "MPAGEVIEWPOINTS", bcir.item_display = "ViewPoint Setup", bcir.step_cat_mean =
    "CORE",
 ...
 IF (error(errmsg,0) > 0)
  SET readme_data->status = "F"
  SET readme_data->message = concat("Failed to check for existence of COREM/MPAGEVIEWPOINTS row: ",
   errmsg)
  GO TO exit_script
 ELSEIF (curqual=0)
  INSERT  FROM br_client_sol_step bcss
   SET bcss.br_client_id = 1, bcss.solution_mean = "COREM", bcss.step_mean = "MPAGEVIEWPOINTS",
    bcss.sequence = 120, bcss.updt_cnt = 0, bcss.updt_dt_tm = cnvtdatetime(curdate,curtime3),
 ...
 ENDIF
 SET readme_data->status = "S"
 SET readme_data->message = "Readme Succeeded: <br_add_mpage_viewpoints.prg> script"
#exit_script
