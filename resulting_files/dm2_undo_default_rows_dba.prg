CREATE PROGRAM dm2_undo_default_rows:dba
 ...
 DECLARE dadr_logfile_name = vc WITH protect, noconstant("dm2_undodr")
 ...
 IF (check_logfile(dadr_logfile_name,".log","DM2_UNDO_DEFAULT_ROWS.PRG ...") != 1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "BEGINNING DM2_UNDO_DEFAULT_ROWS.PRG ..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dadr_tname = cnvtupper(trim( $1))
 IF (textlen(trim(dadr_tname))=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg =
  "An input parameter was not supplied.  Usage: DM2_UNDO_DEFAULT_ROW '<TABLE NAME>' GO"
  SET dm_err->eproc = "DM2_UNDO_DEFAULT_ROWS"
  SET dm_err->user_action = "Please enter a table name as an input parameter."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 IF (findstring(char(42),dadr_tname) > 0
  AND (dm_err->debug_flag != 214))
  SET dadr_wildcard_found = 1
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "A wildcard is not allowed for this process."
  SET dm_err->eproc = "DM2_UNDO_DEFAULT_ROWS"
  SET dm_err->user_action = "Please enter the full table name as the input parameter."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_program
 ENDIF
 SET dadr_ignore_admin_ind = 0
 ...
 SELECT
  ...
  ELSEIF (dadr_ignore_admin_ind=0)
   ...
   WHERE dtd.table_name=dutc.table_name
    AND dtd.table_name=dtd.suffixed_table_name
    AND dtd.table_name=patstring(dadr_tname)
    AND dtd.table_name=dutc.table_name
    AND dtd.table_name=duc.table_name
    AND dtd.table_name=ducc.table_name
  ...
  HEAD dutc.table_name
   ...
   dadr_tbl->tbl[dadr_tcnt].tname = dutc.table_name, dadr_tbl->tbl[dadr_tcnt].short_tname = validate(
    dtd.table_suffix," ")
   IF ((dm_err->debug_flag > 0))
 ...
 IF (check_error(concat(
   "DM2_UNDO_DEFAULT_ROWS.PRG: Error on Main select to identify default row tables candidates ")) !=
 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 ...
 SELECT
  IF (currdb="ORACLE"
   ...
   WHERE dtd.table_name=patstring(dadr_tname)
    ...
    AND utc.table_name=dunc.table_name
 ...
 ;end select
 IF (check_error(concat(
   "DM2_UNDO_DEFAULT_ROWS.PRG: Error on Select2 from user_tab_columns and dm_tables_doc ")) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((dm_err->debug_flag > 0))
 ...
 IF ((dadr_tbl->tcnt > 0))
  IF (currdb != "SQLSRV"
   ...
   ;end select
   IF (check_error(concat(
     "DM2_UNDO_DEFAULT_ROWS.PRG: Error on Select3 from dm2_user_triggers and dm_tables_doc ")) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
  ENDIF
  ...
  ENDIF
  FOR (ix1 = 1 TO dadr_tbl->tcnt)
    EXECUTE dm2_undo_default_rows_c
  ENDFOR
 ELSE
 ...
 IF (dm2_set_autocommit(0)=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "DM2_UNDO_DEFAULT_ROWS is complete."
 IF (dadr_error_cnt > 0)
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "ENDING DM2_UNDO_DEFAULT_ROWS.PRG"
