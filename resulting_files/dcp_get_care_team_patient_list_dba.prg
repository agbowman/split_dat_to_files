CREATE PROGRAM dcp_get_care_team_patient_list:dba
 RECORD reply(
   1 qual[*]
     2 person_id = f8
     2 encntr_id = f8
 ...
 SELECT INTO "nl:"
  ...
  PLAN (epgr
   WHERE (epgr.prsnl_group_id=request->prsnl_group_id)
    AND epgr.end_effective_dt_tm >= cnvtdatetime(e_dt_tm))
   JOIN (e
   WHERE e.encntr_id=epgr.encntr_id)
   JOIN (ed
   WHERE ed.encntr_id=e.encntr_id
    AND ed.end_effective_dt_tm >= cnvtdatetime(e_dt_tm)
    AND ed.active_ind=1)
   JOIN (d
  ...
  DETAIL
   ...
   IF (epgr.end_effective_dt_tm > cnvtdatetime(cur_dt_tm))
