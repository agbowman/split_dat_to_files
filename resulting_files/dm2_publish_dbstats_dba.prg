CREATE PROGRAM dm2_publish_dbstats:dba
 ...
 SUBROUTINE dmai_get_cur_mod_act(dgcma_mod_out,dmgca_act_out)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dpd_inowner = vc WITH protect, noconstant(" ")
 DECLARE dpd_intable = vc WITH protect, noconstant(" ")
 DECLARE dpd_mode = vc WITH protect, noconstant(" ")
 DECLARE dpd_log_id = f8 WITH protect, noconstant(0.0)
 DECLARE dpd_step_beg_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dpd_step_end_dttm = f8 WITH protect, noconstant(0.0)
 DECLARE dpd_idx = i4 WITH protect, noconstant(0)
 DECLARE dpd_group_option = vc WITH protect, noconstant("BYTABLE")
 DECLARE dpd_tabidx = i4 WITH protect, noconstant(0)
 DECLARE dpd_idxidx = i4 WITH protect, noconstant(0)
 DECLARE dpd_cmd = vc WITH protect, noconstant(" ")
 DECLARE dpd_proccnt = i4 WITH protect, noconstant(0)
 DECLARE dpd_err_ind = i2 WITH protect, noconstant(0)
 DECLARE dpd_emsg = vc WITH protect, noconstant("")
 DECLARE dpd_tab_pub = vc WITH protect, noconstant(" ")
 DECLARE dpd_module_name = vc WITH protect, constant("DM_STATS_PROCESS")
 DECLARE dpd_action_publish = vc WITH protect, constant("STATS_PUBLISH")
 DECLARE dpd_action_adjust = vc WITH protect, constant("STATS_ADJUST")
 DECLARE dpd_original_module = vc WITH protect, noconstant("")
 DECLARE dpd_original_action = vc WITH protect, noconstant("")
 DECLARE dpd_loc = i4 WITH protect, noconstant(0)
 DECLARE dpd_retry_ind = i4 WITH protect, noconstant(1)
 DECLARE dpd_retry_count = i4 WITH protect, noconstant(0)
 DECLARE dpd_lock_available_ind = i2 WITH protect, noconstant(0)
 DECLARE dpd_lock_obtained_ind = i2 WITH protect, noconstant(0)
 DECLARE dpd_object_id = f8 WITH protect, noconstant(0.0)
 DECLARE set_table_prefs(ownname=vc,tabname=vc,pname=vc,pvalue=vc) = null WITH sql =
 "SYS.DBMS_STATS.SET_TABLE_PREFS", parameter
 FREE RECORD dpd_publist
 RECORD dpd_publist(
   1 tabcnt = i4
   1 tab[*]
     2 table_name = vc
 ...
 IF (check_logfile("dm2_pdbs",".log","dm2_publish_dbstats LOGFILE")=0)
  GO TO exit_script
 ENDIF
 IF (dmai_get_cur_mod_act(dpd_original_module,dpd_original_action)=0)
  GO TO exit_script
 ENDIF
 CALL dmai_set_mod_act(dpd_module_name,dpd_action_publish)
 SET dm_err->eproc = "Beginning dm2_publish_dbstats"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET dpd_inowner = trim(cnvtupper( $1),3)
 SET dpd_intable = trim( $2,3)
 SET dpd_mode = trim(cnvtupper( $3),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = "Parameter usage: dm2_publish_dbstats  '<owner>','<table_name>','<mode>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 IF ( NOT (dpd_mode IN ("PREVIEW", "PUBLISH")))
  SET dm_err->emsg = "Valid input parameters for MODE are PREVIEW|PUBLISH"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm2_process_event_rs->status = dpl_executing
 CALL dm2_process_log_add_detail_text("OWNER",dpd_inowner)
 CALL dm2_process_log_add_detail_text("TABLE_NAME",dpd_intable)
 CALL dm2_process_log_add_detail_text("TABLE_OWNER.TABLE_NAME",concat(dpd_inowner,".",dpd_intable))
 CALL dm2_process_log_add_detail_text("MODE",dpd_mode)
 ...
 SET dpd_log_id = dm2_process_event_rs->dm_process_event_id
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   IF (d.info_name="GROUPING")
    dpd_group_option = cnvtupper(d.info_char)
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ENDIF
 CALL dm2_process_log_add_detail_text("GROUPING",dpd_group_option)
 IF (dm2_process_log_dtl_row(dpd_log_id,0)=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Get STAGED table data from DM_STAT_TABLE meeting input criteria."
 SELECT INTO "nl:"
  FROM dm_stat_table d,
   dba_tables dt
  WHERE d.statid="DMSTG_T"
   AND d.c5=patstring(dpd_inowner)
   AND d.c1=patstring(dpd_intable)
   AND d.type="T"
   AND d.c5=dt.owner
   AND d.c1=dt.table_name
  ORDER BY d.c5, d.c1
  DETAIL
   dpd_publist->tabcnt = (dpd_publist->tabcnt+ 1), stat = alterlist(dpd_publist->tab,dpd_publist->
    tabcnt), dpd_publist->tab[dpd_publist->tabcnt].table_name = d.c1,
   dpd_publist->tab[dpd_publist->tabcnt].table_owner = d.c5, dpd_publist->tab[dpd_publist->tabcnt].
   dst_ind = 1, dpd_publist->tab[dpd_publist->tabcnt].tab_status = "PUBLISH",
   dpd_publist->tab[dpd_publist->tabcnt].allind_status = "PUBLISH"
  WITH nocounter
 ;end select
 SET dm_err->eproc = "Alter the publish list to exclude table from being published"
 SELECT INTO "nl:"
  FROM dm_info di,
   (dummyt d  WITH seq = size(dpd_publist->tab,5))
  PLAN (d)
   JOIN (di
   WHERE di.info_domain="DM2PDBS:EXCLUDE_TABLE"
    AND di.info_name=concat(trim(dpd_publist->tab[d.seq].table_owner),".",trim(dpd_publist->tab[d.seq
     ].table_name)))
  DETAIL
   dpd_publist->tab[d.seq].exclude_ind = 1
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Get STAGED index data from DM_STAT_TABLE meeting input criteria."
 SELECT INTO "nl:"
  FROM dm_stat_table d,
   dba_indexes di,
   dba_objects do
  WHERE d.statid="DMSTG_I*"
   ...
   AND d.c5=di.owner
   AND di.table_owner=patstring(dpd_inowner)
   AND di.table_name=patstring(dpd_intable)
   AND d.statid=concat("DMSTG_I",cnvtstring(do.object_id,20))
   AND d.c5=do.owner
   AND di.owner=do.owner
   AND di.index_name=do.object_name
  ORDER BY di.owner, di.table_name, di.index_name
  HEAD di.owner
   row + 0, icnt = 0
  HEAD di.table_name
   locndx = 0, locndx = locateval(locndx,1,dpd_publist->tabcnt,di.table_name,dpd_publist->tab[locndx]
    .table_name,
    di.table_owner,dpd_publist->tab[locndx].table_owner)
   IF (locndx=0)
    dpd_publist->tabcnt = (dpd_publist->tabcnt+ 1), stat = alterlist(dpd_publist->tab,dpd_publist->
     tabcnt), dpd_publist->tab[dpd_publist->tabcnt].table_name = di.table_name,
    dpd_publist->tab[dpd_publist->tabcnt].table_owner = di.table_owner, dpd_publist->tab[dpd_publist
    ->tabcnt].tab_status = "PUBLISH", dpd_publist->tab[dpd_publist->tabcnt].dst_ind = 0,
    locndx = dpd_publist->tabcnt
   ENDIF
  DETAIL
   dpd_publist->tab[locndx].ind_cnt = (dpd_publist->tab[locndx].ind_cnt+ 1), icnt = dpd_publist->tab[
   locndx].ind_cnt, stat = alterlist(dpd_publist->tab[locndx].ind,icnt),
   dpd_publist->tab[locndx].ind[icnt].index_name = di.index_name, dpd_publist->tab[locndx].ind[icnt].
   index_owner = di.owner, dpd_publist->tab[locndx].ind[icnt].table_owner = di.table_owner,
   dpd_publist->tab[locndx].ind[icnt].index_statid = d.statid, dpd_publist->tab[locndx].ind[icnt].
   dpq_status = "PUBLISH"
   IF ((dpd_publist->tab[locndx].exclude_ind=1))
    dpd_publist->tab[locndx].ind[icnt].exclude_ind = 1
   ENDIF
  WITH nocounter
 ;end select
 SET dm_err->eproc = "Alter the publish list to exclude table from being published"
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE di.info_domain="DM2PDBS:EXCLUDE_INDEX"
  DETAIL
   FOR (dpd_idx = 1 TO dpd_publist->tabcnt)
    locndx = 0,
    IF (locateval(locndx,1,dpd_publist->tab[dpd_idx].ind_cnt,di.info_name,concat(trim(dpd_publist->
       tab[dpd_idx].ind[locndx].index_owner),".",trim(dpd_publist->tab[dpd_idx].ind[locndx].
       index_name))) > 0)
     dpd_publist->tab[dpd_idx].ind[locndx].exclude_ind = 1
    ENDIF
   ENDFOR
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ENDIF
 IF ((dpd_publist->tabcnt > 0))
  SELECT INTO "nl:"
   FROM dm_process_queue d
   WHERE d.process_type IN (dpq_statistics, dpq_freq_statistics)
    AND d.op_type=dpq_gather
    AND d.object_type=dpq_table
    AND d.process_status != dpq_success
    AND d.owner_name=patstring(dpd_inowner)
   HEAD d.object_name
    locndx = 0, locndx = locateval(locndx,1,dpd_publist->tabcnt,d.object_name,dpd_publist->tab[locndx
     ].table_name,
     d.owner_name,dpd_publist->tab[locndx].table_owner)
    IF (locndx > 0)
     dpd_publist->tab[locndx].tab_status = d.process_status
    ENDIF
   WITH nocounter
  ;end select
  SELECT INTO "nl:"
   ...
   WHERE d.process_type IN (dpq_statistics, dpq_freq_statistics)
    ...
    AND di.table_owner=patstring(dpd_inowner)
    AND di.table_name=patstring(dpd_intable)
   ORDER BY di.table_owner, di.index_name
   DETAIL
    locndx = 0, locndx2 = 0, locndx = locateval(locndx,1,dpd_publist->tabcnt,di.table_name,
     dpd_publist->tab[locndx].table_name,
     di.table_owner,dpd_publist->tab[locndx].table_owner)
    IF (locndx > 0)
     IF ((dpd_publist->tab[locndx].allind_status="PUBLISH"))
      dpd_publist->tab[locndx].allind_status = d.process_status
     ENDIF
     locndx2 = locateval(locndx2,1,dpd_publist->tab[locndx].ind_cnt,di.index_name,dpd_publist->tab[
      locndx].ind[locndx2].index_name)
     IF (locndx2 > 0)
      dpd_publist->tab[locndx].ind[locndx2].dpq_status = d.process_status
     ENDIF
    ENDIF
   WITH nocounter
  ;end select
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(dpd_publist)
 ENDIF
 FOR (dpd_idx = 1 TO dpd_publist->tabcnt)
  IF ((dpd_publist->tab[dpd_idx].exclude_ind=1))
   CALL echo(concat("Table :",concat(dpd_publist->tab[dpd_idx].table_owner,".",dpd_publist->tab[
      dpd_idx].table_name)," manually excluded"))
  ENDIF
  FOR (dpd_loc = 1 TO dpd_publist->tab[dpd_idx].ind_cnt)
    IF ((dpd_publist->tab[dpd_idx].ind[dpd_loc].exclude_ind=1))
     CALL echo(concat("Index : ",concat(dpd_publist->tab[dpd_idx].ind[dpd_loc].index_owner,".",
        dpd_publist->tab[dpd_idx].ind[dpd_loc].index_name)," manually excluded"))
    ENDIF
  ENDFOR
 ENDFOR
 IF (dpd_mode="PUBLISH")
  IF ((dgdt_prefs->context_lock_ind=- (1)))
   ...
   ENDIF
  ENDIF
  FOR (dpd_tabidx = 1 TO dpd_publist->tabcnt)
    IF (((dpd_group_option="BYOBJECT") OR (dpd_group_option="BYTABLE"
     AND (dpd_publist->tab[dpd_tabidx].tab_status="PUBLISH")
     AND (dpd_publist->tab[dpd_tabidx].allind_status="PUBLISH"))) )
     SET dm_err->eproc = "Obtain current publish status"
     SET dpd_tab_pub = "TRUE"
     SELECT INTO "nl:"
      FROM dba_tab_stat_prefs d
      WHERE (d.owner=dpd_publist->tab[dpd_tabidx].table_owner)
       AND (d.table_name=dpd_publist->tab[dpd_tabidx].table_name)
      DETAIL
       IF (trim(cnvtupper(d.preference_name))="PUBLISH")
        dpd_tab_pub = trim(cnvtupper(d.preference_value))
       ENDIF
      WITH nocounter
     ;end select
     IF (check_error(dm_err->eproc) != 0)
      ...
      GO TO exit_script
     ENDIF
     IF (dpd_tab_pub="FALSE")
      IF ((dgdt_prefs->context_lock_ind=1))
       SET dpd_object_id = 0.0
       IF (ddf_get_object_id(dpd_publist->tab[dpd_tabidx].table_owner,dpd_publist->tab[dpd_tabidx].
        table_name,dpd_object_id)=0)
        GO TO exit_script
       ENDIF
      ENDIF
      WHILE (dpd_retry_ind=1)
        IF (ddf_context_lock_maint("CHECK","STAT",1,dpd_publist->tab[dpd_tabidx].table_owner,
         dpd_publist->tab[dpd_tabidx].table_name,
         dpd_object_id,dpd_lock_available_ind)=0)
         GO TO exit_script
        ENDIF
        IF (dpd_lock_available_ind=1)
         IF (ddf_context_lock_maint("GET","STAT",2,dpd_publist->tab[dpd_tabidx].table_owner,
          dpd_publist->tab[dpd_tabidx].table_name,
          dpd_object_id,dpd_lock_obtained_ind)=0)
          GO TO exit_script
         ENDIF
         IF (dpd_lock_obtained_ind=1)
          SET dpd_retry_ind = 0
         ENDIF
        ENDIF
        IF (dpd_lock_obtained_ind=0)
         IF (dpd_retry_count > 10)
          SET dm_err->err_ind = 1
          SET dm_err->emsg = concat("Unable to obtain publish stats lock for Table: ",dpd_publist->
           tab[dpd_tabidx].table_name,
           ". Table stats will be published during next cycle. This error can be ignored.")
          CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
          GO TO exit_script
         ELSE
          SET dm_err->eproc = "Waiting to obtain publish stats lock"
          CALL disp_msg(" ",dm_err->logfile,0)
          CALL pause(30)
          SET dpd_retry_count = (dpd_retry_count+ 1)
         ENDIF
        ENDIF
      ENDWHILE
      SET dm_err->eproc = "Call dbms_stats.SET_TABLE_PREFS to set PUBLISH to TRUE"
      CALL set_table_prefs(dpd_publist->tab[dpd_tabidx].table_owner,concat('"',dpd_publist->tab[
        dpd_tabidx].table_name,'"'),"PUBLISH","TRUE")
      IF (check_error(dm_err->eproc) != 0)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_script
      ENDIF
     ENDIF
     IF ((dpd_publist->tab[dpd_tabidx].dst_ind=1))
      IF ((dpd_publist->tab[dpd_tabidx].tab_status="PUBLISH")
       AND (dpd_publist->tab[dpd_tabidx].exclude_ind=0))
       CALL echo(build("Publish table for:",dpd_publist->tab[dpd_tabidx].table_name))
       EXECUTE dm2_publish_dbstats_tab dpd_publist->tab[dpd_tabidx].table_owner, dpd_publist->tab[
       dpd_tabidx].table_name, "DMSTG_T"
       IF ((dm_err->err_ind=1))
        GO TO exit_script
       ENDIF
       CALL echo(build("Export live to actual for:",dpd_publist->tab[dpd_tabidx].table_name))
       EXECUTE dm2_export_dbstats_tab dpd_publist->tab[dpd_tabidx].table_owner, dpd_publist->tab[
       dpd_tabidx].table_name, "ACTUAL"
       IF ((dm_err->err_ind=1))
        GO TO exit_script
       ENDIF
      ENDIF
     ENDIF
     FOR (dpd_idxidx = 1 TO dpd_publist->tab[dpd_tabidx].ind_cnt)
       IF ((dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].dpq_status="PUBLISH")
        AND (dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].exclude_ind=0))
        CALL echo(build("Publishing index :",dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].index_name)
         )
        EXECUTE dm2_publish_dbstats_ind dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].index_owner,
        dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].index_name, dpd_publist->tab[dpd_tabidx].ind[
        dpd_idxidx].index_statid
        IF ((dm_err->err_ind=1))
         GO TO exit_script
        ENDIF
        CALL echo(build("Exporting index (",dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].index_name,
          ") to ACTUAL"))
        EXECUTE dm2_export_dbstats_ind dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].index_owner,
        dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].index_name, "ACTUAL"
        IF ((dm_err->err_ind=1))
         GO TO exit_script
        ENDIF
       ENDIF
     ...
     IF (dpd_tab_pub="FALSE")
      ...
      CALL set_table_prefs(dpd_publist->tab[dpd_tabidx].table_owner,concat('"',dpd_publist->tab[
        dpd_tabidx].table_name,'"'),"PUBLISH","FALSE")
      IF (check_error(dm_err->eproc) != 0)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       GO TO exit_script
      ENDIF
     ENDIF
     IF (dpd_lock_obtained_ind=1)
      IF (ddf_context_lock_maint("RELEASE","STAT",2,dpd_publist->tab[dpd_tabidx].table_owner,
       dpd_publist->tab[dpd_tabidx].table_name,
       dpd_object_id,dpd_lock_obtained_ind)=0)
       GO TO exit_script
      ENDIF
     ENDIF
    ELSE
     CALL echo(build("STAGED statistics not published for table:",dpd_publist->tab[dpd_tabidx].
       table_name))
     CALL dm2_process_log_add_detail_text("BYTABLE OBJECTS BYPASSED FOR",concat(dpd_publist->tab[
       dpd_tabidx].table_owner,".",dpd_publist->tab[dpd_tabidx].table_name,",TABSTATUS:",dpd_publist
       ->tab[dpd_tabidx].tab_status,
       ",ALLINDSTATUS:",dpd_publist->tab[dpd_tabidx].allind_status))
     IF (dm2_process_log_dtl_row(dpd_log_id,0)=0)
      GO TO exit_script
     ENDIF
    ENDIF
  ENDFOR
  CALL dmai_set_mod_act(dpd_module_name,dpd_action_adjust)
  SET dm_err->eproc = "Retreiving DBSTATS Publishing options from DM_INFO"
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:ADJUSTHIDATE"
    AND d.info_name="EXECUTE_FROM_PUBLISH"
    AND d.info_number=0
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
  ...
  ENDIF
  IF (curqual=0
   AND dpd_intable=char(42))
   EXECUTE dm2_adjust_dbstats_tab "HIDATE", "*", "*",
   "LIVE"
   IF ((dm_err->err_ind=1))
    GO TO exit_script
   ENDIF
   CALL dmai_set_mod_act(dpd_module_name,dpd_action_adjust)
   SET dm_err->eproc = "Get MINMAX_INITIAL row from DM_INFO."
   SELECT INTO "nl:"
    FROM dm_info d
    WHERE d.info_domain="DM2_DBSTATS_ADJUSTMENT"
     AND d.info_name="MINMAX_INITIAL"
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc) != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   ELSEIF (curqual=0)
    EXECUTE dm2_adjust_dbstats_tab "MINMAX", "*", "*",
    "LIVE"
    CALL dmai_set_mod_act(dpd_module_name,dpd_action_adjust)
    IF ((dm_err->err_ind=1))
     GO TO exit_script
    ENDIF
    SET dm_err->eproc = "Insert MINMAX_INITIAL row from DM_INFO."
    INSERT  FROM dm_info d
     SET d.info_domain = "DM2_DBSTATS_ADJUSTMENT", d.info_name = "MINMAX_INITIAL", d.updt_dt_tm =
   ...
   ENDIF
  ENDIF
  SET dm_err->eproc = "Get DM2GDBS:PUBLISHADJUSTPROC row from DM_INFO."
  SELECT INTO "nl:"
   FROM dm_info d
   WHERE d.info_domain="DM2GDBS:PUBLISHADJUSTPROC"
   DETAIL
    dpd_procs->cnt = (dpd_procs->cnt+ 1), stat = alterlist(dpd_procs->qual,dpd_procs->cnt), dpd_procs
    ->qual[dpd_procs->cnt].proc_name = trim(d.info_name),
    dpd_procs->qual[dpd_procs->cnt].proc_cmd = trim(d.info_char)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echorecord(dpd_procs)
  ENDIF
  IF ((dpd_procs->cnt > 0))
   FOR (dpd_proccnt = 1 TO dpd_procs->cnt)
     SET dpd_cmd = dpd_procs->qual[dpd_proccnt].proc_cmd
     SET dpd_cmd = replace(dpd_cmd,"<OBJECT_OWNER>",dpd_inowner)
     SET dpd_cmd = replace(dpd_cmd,"<OBJECT_NAME>",dpd_intable)
     SET dpd_step_beg_dt_tm = cnvtdatetime(curdate,curtime3)
     SET dpd_cmd = concat("RDB ASIS(^ begin ",dpd_cmd,"; end; ^) go")
     IF (dm2_push_cmd(dpd_cmd,1)=0)
      CALL disp_msg(" ",dm_err->logfile,1)
      IF ((dm_err->err_ind=1))
       CALL dm2_process_log_add_detail_text(concat(dpd_procs->qual[dpd_proccnt].proc_name,":ERROR"),
        dm_err->emsg)
       IF (dm2_process_log_dtl_row(dpd_log_id,1)=0)
        GO TO exit_script
       ENDIF
       SET dm_err->err_ind = 0
       SET dm_err->emsg = " "
      ENDIF
     ENDIF
     SET dpd_step_end_dt_tm = cnvtdatetime(curdate,curtime3)
     CALL dm2_process_log_add_detail_number(concat(dpd_procs->qual[dpd_proccnt].proc_name,":SECS"),
      datetimediff(dpd_step_end_dttm,dpd_step_beg_dttm,5))
     CALL dm2_process_log_add_detail_text(concat(dpd_procs->qual[dpd_proccnt].proc_name,":CMD"),
      dpd_cmd)
     IF (dm2_process_log_dtl_row(dpd_log_id,0)=0)
      GO TO exit_script
     ENDIF
 ...
 IF (dpd_mode="PREVIEW")
  SELECT INTO "MINE"
   ...
   HEAD REPORT
    col 0,
    "********************* DATABASE STATISTICS PUBLISHING PREVIEW REPORT **********************", row
     + 1,
    col 0, "----- Input Criteria -----", row + 1,
    col 2, "OWNER: ", col 30,
    CALL print(trim(dpd_inowner)), row + 1, col 2,
    "TABLE_NAME: ", col 30,
    CALL print(trim(dpd_intable)),
    row + 1, col 2, "PUBLISH GROUPING: ",
    col 30,
    CALL print(trim(dpd_group_option)), row + 2,
    col 0, "------- Details ----------", row + 1,
    col 0, "TABLE_OWNER", col 15,
    "TABLE_NAME", col 46, "OBJECT_NAME",
    col 77, "T/I", col 81,
    "PUBLISH STATUS", row + 1, col 0,
    CALL print(fillstring(14,"-")), col 15,
    CALL print(fillstring(30,"-")),
    col 46,
    CALL print(fillstring(30,"-")), col 77,
    "---", col 81,
    CALL print(fillstring(30,"-"))
    IF ((dpd_publist->tabcnt=0))
     row + 1, col 10, "No objects found in DM_STAT_TABLE to publish based on input criteria"
    ELSE
     FOR (dpd_tabidx = 1 TO dpd_publist->tabcnt)
      IF ((dpd_publist->tab[dpd_tabidx].dst_ind=1))
       row + 1, col 0,
       CALL print(trim(dpd_publist->tab[dpd_tabidx].table_owner)),
       col 15,
       CALL print(trim(dpd_publist->tab[dpd_tabidx].table_name)), col 46,
       CALL print(trim(dpd_publist->tab[dpd_tabidx].table_name)), col 78, "T"
       IF ((dpd_publist->tab[dpd_tabidx].exclude_ind=0))
        IF ((dpd_publist->tab[dpd_tabidx].tab_status != "PUBLISH"))
         col 81,
         CALL print(build("Waiting, Table Status:",dpd_publist->tab[dpd_tabidx].tab_status))
        ELSEIF (dpd_group_option="BYTABLE"
         AND (dpd_publist->tab[dpd_tabidx].allind_status != "PUBLISH"))
         col 81,
         CALL print(build("Waiting, Index Status:",dpd_publist->tab[dpd_tabidx].allind_status))
        ELSE
         col 81, "READY TO PUBLISH"
        ENDIF
       ELSE
        col 81, "MANUALLY EXCLUDED"
       ENDIF
      ENDIF
      ,
      FOR (dpd_idxidx = 1 TO dpd_publist->tab[dpd_tabidx].ind_cnt)
        row + 1, col 0,
        CALL print(trim(substring(1,14,dpd_publist->tab[dpd_tabidx].table_owner))),
        col 15,
        CALL print(trim(dpd_publist->tab[dpd_tabidx].table_name)), col 46,
        CALL print(trim(dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].index_name)), col 78, "I"
        IF ((dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].exclude_ind=0))
         IF ((dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].dpq_status != "PUBLISH"))
          col 81,
          CALL print(build("Waiting, Index Status:",dpd_publist->tab[dpd_tabidx].ind[dpd_idxidx].
           dpq_status))
         ELSEIF (dpd_group_option="BYTABLE"
          AND (dpd_publist->tab[dpd_tabidx].tab_status != "PUBLISH"))
          col 81,
         ...
         ELSEIF (dpd_group_option="BYTABLE"
          AND (dpd_publist->tab[dpd_tabidx].allind_status != "PUBLISH"))
          col 81,
          CALL print(build("Waiting, Sibling Index Status:",dpd_publist->tab[dpd_tabidx].
           allind_status))
         ELSE
          col 81, "READY TO PUBLISH"
         ENDIF
...
#exit_script
 ...
 SET dpd_err_ind = dm_err->err_ind
 SET dpd_emsg = dm_err->emsg
 SET dm_err->err_ind = 0
 CALL dm2_process_log_row(dpl_statistics,dpl_execution,dpd_log_id,1)
 IF (dpd_lock_obtained_ind=1
  AND dpd_err_ind=1)
  CALL ddf_context_lock_maint("RELEASE","STAT",2,dpd_publist->tab[dpd_tabidx].table_owner,dpd_publist
   ->tab[dpd_tabidx].table_name,
   dpd_object_id,dpd_lock_obtained_ind)
 ENDIF
 SET dm_err->err_ind = dpd_err_ind
 SET dm_err->emsg = dpd_emsg
 SET dm_err->eproc = "Ending dm2_publish_dbstats"
 CALL dmai_set_mod_act(dpd_original_module,dpd_original_action)
 CALL final_disp_msg("dm2_pdbs")
END GO
