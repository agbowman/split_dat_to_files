CREATE PROGRAM bed_aud_dta_event_r:dba
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dta
   ...
   JOIN (cvec
   WHERE cvec.code_value=cver.event_cd)
  ...
  DETAIL
   ...
   .task_assay_cd,
   temp->dlist[dcnt].dtaname = dta.mnemonic, temp->dlist[dcnt].event_cd = cver.event_cd, temp->dlist[
   dcnt].event_disp = cvec.display
  WITH nocounter
 ;end select
 ...
 SET reply->collist[2].header_text = "Event Code Display"
 SET reply->collist[2].data_type = 1
 SET reply->collist[2].hide_ind = 0
 SET reply->collist[3].header_text = "Display Discrepancy"
 SET reply->collist[3].data_type = 1
 SET reply->collist[3].hide_ind = 0
 SET reply->collist[4].header_text = "task_assay_cd"
 SET reply->collist[4].data_type = 2
 SET reply->collist[4].hide_ind = 1
 SET reply->collist[5].header_text = "event_cd"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   row_nbr].celllist[2].string_value = temp->dlist[d.seq].event_disp
   IF ((temp->dlist[d.seq].dtaname != temp->dlist[d.seq].event_disp))
    reply->rowlist[row_nbr].celllist[3].string_value = "X"
   ENDIF
   reply->rowlist[row_nbr].celllist[4].double_value = temp->dlist[d.seq].task_assay_cd, reply->
   rowlist[row_nbr].celllist[5].double_value = temp->dlist[d.seq].event_cd
  WITH nocounter
 ;end select
#exit_script
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("dta_event_reltn_audit.csv")
 ENDIF
