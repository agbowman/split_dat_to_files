CREATE PROGRAM dm2_sys_hlth_chk_rpt:dba
 ...
 SUBROUTINE (dshc_load_env_info(s_dlei_prompt_ind=i2) =i2)
   ...
   RETURN(1)
 END ;Subroutine
 IF ((validate(dshc_wrap->line_cnt,- (1))=- (1))
  AND (validate(dshc_wrap->line_cnt,- (2))=- (2)))
  FREE RECORD dshc_wrap
  RECORD dshc_wrap(
    1 line_cnt = i4
    1 source_text = vc
    1 wrap_length = i4
  ...
  SET dshc_wrap->line_cnt = 0
 ENDIF
 DECLARE dshcr_fname = vc WITH protect, noconstant("NOT_SET")
 DECLARE detcnt = i4 WITH protect, noconstant(0)
 DECLARE wrpcnt4 = i4 WITH private, noconstant(0)
 DECLARE wrpcnt1 = i4 WITH private, noconstant(0)
 DECLARE wrpcnt2 = i4 WITH private, noconstant(0)
 DECLARE wrpcnt3 = i4 WITH private, noconstant(0)
 DECLARE dshcr_max_ln_len = i4 WITH private, constant(128)
 DECLARE rpt_line = vc WITH protect, noconstant(fillstring(120,"-"))
 DECLARE dshcr_prev_chktyp = i4 WITH protect, noconstant(0)
 DECLARE dshcr_prev_cdt = vc WITH protect, noconstant("NOT_SET")
 DECLARE macro_text_pos = i4 WITH protect, noconstant(0)
 DECLARE macro_prev_pos = i4 WITH protect, noconstant(0)
 DECLARE macro_nextsp_pos = i4 WITH protect, noconstant(0)
 DECLARE macro_break_pos = i4 WITH protect, noconstant(0)
 DECLARE macro_next_token_pos = i4 WITH protect, noconstant(0)
 DECLARE rpt_tmp_txt = vc WITH protect, noconstant("NOT_SET")
 IF (check_logfile("dm2_shc_rpt",".log","SYSTEM HEALTH CHECK REPORT LOG FILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Generating System Health Check report."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ((dshc->chk_cnt=0))
  SET dm_err->emsg = "No checks have been performed."
  SET dm_err->user_action = "Please use DM2_SYS_HLTH_CHK_MENU to perform the System Health Check."
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,dm_err->err_ind)
  GO TO exit_script
 ENDIF
 ...
 ENDIF
 IF (get_unique_file("dm2_shc_",".rpt")=0)
  GO TO exit_script
 ENDIF
 SET dshcr_fname = dm_err->unique_fname
 SELECT INTO value(dshcr_fname)
  d.seq
  FROM (dummyt d  WITH seq = dshc->chk_cnt)
  HEAD REPORT
   col + 0, "System Health Check Report", row + 1,
   col + 0, "=======================================", row + 1,
   row + 1,
   MACRO (wrap_text)
    macro_text_pos = 0, macro_prev_pos = 0, macro_nextsp_pos = 0,
    macro_break_pos = 0, macro_next_token_pos = 0, dshc_wrap->line_cnt = 0,
    stat = alterlist(dshc_wrap->line,dshc_wrap->line_cnt)
    WHILE (macro_text_pos < size(dshc_wrap->source_text))
      dshc_wrap->line_cnt += 1, stat = alterlist(dshc_wrap->line,dshc_wrap->line_cnt), macro_prev_pos
       = (macro_text_pos+ 1)
      IF ((size(dshc_wrap->source_text) > (macro_text_pos+ (dshc_wrap->wrap_length - 1))))
       macro_text_pos += (dshc_wrap->wrap_length - 1)
      ELSE
       macro_text_pos = size(dshc_wrap->source_text)
      ENDIF
      macro_next_token_pos = findstring("<SPACE>",dshc_wrap->source_text,macro_prev_pos)
      IF (macro_next_token_pos <= macro_text_pos
       AND macro_next_token_pos != 0)
       macro_break_pos = macro_next_token_pos, macro_text_pos = (macro_next_token_pos+ 6)
      ELSEIF (macro_text_pos < size(dshc_wrap->source_text))
       macro_nextsp_pos = findstring(" ",dshc_wrap->source_text,macro_prev_pos,0)
       IF ((dm_err->debug_flag > 0))
        CALL echo(build(macro_break_pos,"::",macro_nextsp_pos,"::",macro_text_pos))
       ENDIF
       WHILE (macro_nextsp_pos < macro_text_pos)
         macro_break_pos = macro_nextsp_pos, macro_nextsp_pos = findstring(" ",dshc_wrap->source_text,
          (macro_break_pos+ 1),0)
         IF ((dm_err->debug_flag > 0))
          CALL echo(build(macro_break_pos,"::",macro_nextsp_pos,"::",macro_text_pos))
         ENDIF
         IF (macro_nextsp_pos=0)
          macro_nextsp_pos = macro_text_pos
         ENDIF
         IF ((dm_err->debug_flag > 0))
          CALL echo(build(macro_break_pos,"::",macro_nextsp_pos,"::",macro_text_pos))
         ENDIF
       ENDWHILE
       macro_text_pos = macro_break_pos
      ELSE
       macro_break_pos = (macro_text_pos+ 1)
      ENDIF
      dshc_wrap->line[dshc_wrap->line_cnt].text = substring(macro_prev_pos,(macro_break_pos -
       macro_prev_pos),dshc_wrap->source_text)
      IF ((dm_err->debug_flag > 0))
       CALL echo(substring(macro_prev_pos,(macro_break_pos - macro_prev_pos),dshc_wrap->source_text))
      ENDIF
      IF (macro_break_pos=macro_next_token_pos)
       dshc_wrap->line_cnt += 1, stat = alterlist(dshc_wrap->line,dshc_wrap->line_cnt), dshc_wrap->
       line[dshc_wrap->line_cnt].text = " "
      ENDIF
    ENDWHILE
   ENDMACRO
  DETAIL
   IF ((dshcr_prev_chktyp != dshc->check[d.seq].check_type_flag))
    dshcr_prev_chktyp = dshc->check[d.seq].check_type_flag, row + 1, row + 1
    CASE (dshc->check[d.seq].check_type_flag)
     OF dshc_ctf_millennium:
      dshcr_prev_chktyp = dshc->check[d.seq].check_type_flag,row + 1,col + 0,
      CALL center("Cerner Millennium Configuration",1,120)row + 1,col + 0,
      CALL center("---------------------------------------",1,120)
     OF dshc_ctf_database:
      dshcr_prev_chktyp = dshc->check[d.seq].check_type_flag,row + 1,col + 0,
      CALL center("Database Configuration",1,120)row + 1,col + 0,
      CALL center("---------------------------------------",1,120)
     OF dshc_ctf_tcpip:
      dshcr_prev_chktyp = dshc->check[d.seq].check_type_flag,row + 1,col + 0,
      CALL center("TCP/IP Configuration",1,120)row + 1,col + 0,
      CALL center("---------------------------------------",1,120)
     OF dshc_ctf_os:
      dshcr_prev_chktyp = dshc->check[d.seq].check_type_flag,row + 1,col + 0,
      CALL center("Operating System-level Configuration",1,120)row + 1,col + 0,
      CALL center("---------------------------------------",1,120)
    ENDCASE
   ENDIF
   row + 1, row + 1, col + 0,
   dshc->check[d.seq].long_name, row + 1, col + 0,
   rpt_line, row + 1, row + 1
   IF ((dshc->check[d.seq].error_ind=1))
    col + 2, dshc_generic_err_msg, row + 1
   ELSEIF ((dshc->check[d.seq].good_ind=1))
    col + 2, dshc_generic_good_msg, row + 1
   ELSE
    dshc_wrap->source_text = dshc->check[d.seq].check_desc_text, dshc_wrap->wrap_length = 128,
    wrap_text
    FOR (wrpcnt3 = 1 TO dshc_wrap->line_cnt)
      col + 2, dshc_wrap->line[wrpcnt3].text, row + 1
    ENDFOR
    IF ((dm_err->debug_flag > 1))
     CALL echorecord(dshc_wrap)
    ENDIF
    row + 1
    FOR (detcnt = 1 TO dshc->check[d.seq].desc_dtl_cnt)
     IF ((dshcr_prev_cdt != dshc->check[d.seq].desc_detail[detcnt].dtl_desc_text)
      AND (dshc->check[d.seq].desc_detail[detcnt].dtl_desc_text != "NOT_SET"))
      dshcr_prev_cdt = dshc->check[d.seq].desc_detail[detcnt].dtl_desc_text, row + 1, dshc_wrap->
      source_text = dshc->check[d.seq].desc_detail[detcnt].dtl_desc_text,
      dshc_wrap->wrap_length = 128, wrap_text
      FOR (wrpcnt4 = 1 TO dshc_wrap->line_cnt)
        col + 2, dshc_wrap->line[wrpcnt4].text, row + 1
      ENDFOR
      row + 1
      IF ((dm_err->debug_flag > 1))
       CALL echorecord(dshc_wrap)
     ...
     ENDIF
     ,
     IF ((dshc->check[d.seq].desc_detail[detcnt].value_prefix != "NOT_SET"))
      IF ((dshc->check[d.seq].desc_detail[detcnt].type=1))
       col + 5, dshc->check[d.seq].desc_detail[detcnt].value_prefix, col + 3,
       dshc->check[d.seq].desc_detail[detcnt].nbr_value, row + 1
      ELSEIF ((dshc->check[d.seq].desc_detail[detcnt].type=2))
       dshc_wrap->source_text = concat(dshc->check[d.seq].desc_detail[detcnt].value_prefix,"  ",dshc
        ->check[d.seq].desc_detail[detcnt].str_value), dshc_wrap->wrap_length = 125, wrap_text
       FOR (wrpcnt1 = 1 TO dshc_wrap->line_cnt)
         col + 5, dshc_wrap->line[wrpcnt1].text, row + 1
       ENDFOR
       IF ((dm_err->debug_flag > 1))
        CALL echorecord(dshc_wrap)
       ENDIF
      ELSEIF ((dshc->check[d.seq].desc_detail[detcnt].type=3))
       rpt_tmp_txt = format(dshc->check[d.seq].desc_detail[detcnt].date_value,";;Q"), col + 5, dshc->
       check[d.seq].desc_detail[detcnt].value_prefix,
       col + 3, rpt_tmp_txt, row + 1
      ENDIF
     ELSE
      IF ((dshc->check[d.seq].desc_detail[detcnt].type=1))
       col + 5, dshc->check[d.seq].desc_detail[detcnt].nbr_value, row + 1
      ELSEIF ((dshc->check[d.seq].desc_detail[detcnt].type=2))
       dshc_wrap->source_text = dshc->check[d.seq].desc_detail[detcnt].str_value, dshc_wrap->
       wrap_length = 125, wrap_text
       FOR (wrpcnt2 = 1 TO dshc_wrap->line_cnt)
         col + 5, dshc_wrap->line[wrpcnt2].text, row + 1
       ENDFOR
       IF ((dm_err->debug_flag > 1))
        CALL echorecord(dshc_wrap)
       ENDIF
      ELSEIF ((dshc->check[d.seq].desc_detail[detcnt].type=3))
       rpt_tmp_txt = format(dshc->check[d.seq].desc_detail[detcnt].date_value,";;Q"), col + 5,
       rpt_tmp_txt,
       row + 1
      ENDIF
     ENDIF
 ...
 ENDIF
 IF (cursys="AIX")
  SET dm_err->eproc = concat("The System Health Check report is available at $CCLUSERDIR/",
   dshcr_fname)
 ELSEIF (cursys="AXP")
  SET dm_err->eproc = concat("The System Health Check report is available at CCLUSERDIR:",dshcr_fname
   )
 ENDIF
 CALL disp_msg(" ",dm_err->logfile,dm_err->err_ind)
#exit_script
 SET dm_err->eproc = "Finished displaying the System Health Check Report."
 CALL final_disp_msg("dm2_shc_rpt")
 FREE RECORD dshc_wrap
END GO
