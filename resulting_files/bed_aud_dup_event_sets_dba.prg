CREATE PROGRAM bed_aud_dup_event_sets:dba
 ...
 SELECT INTO "NL:"
  ...
  PLAN (cv
   ...
   WHERE e.event_set_cd=cv.code_value
    AND e.event_set_level=0)
  DETAIL
   total_event_sets = (total_event_sets+ 1)
  WITH nocounter
 ;end select
 SET row_nbr = 0
 FOR (x = 1 TO tcnt)
 ...
 SET reply->statlist[1].statistic_meaning = "DUPEVENTSETSRESSEC"
 SET reply->statlist[1].total_items = total_event_sets
...
#exit_script
 ...
 IF ((reply->high_volume_flag IN (1, 2)))
  SET reply->output_filename = build("dup_event_sets_in_all_results_section.csv")
 ENDIF
