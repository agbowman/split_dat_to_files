CREATE PROGRAM dm_ocd_output_test:dba
 ...
 SET flist->cnt = 0
 SET stat = alterlist(flist->qual,10)
 SELECT INTO "nl:"
  a.feature_number, a.alpha_feature_nbr, a.schema_date
  FROM dm_afd_tables a
  WHERE (a.table_name=tab_list->qual[cnt].table_name)
   AND a.alpha_feature_nbr != afd_nbr
  DETAIL
   flist->cnt = (flist->cnt+ 1), stat = alterlist(flist->qual,flist->cnt), flist->qual[flist->cnt].
   ocd_number = a.alpha_feature_nbr,
   flist->qual[flist->cnt].feature_number = a.feature_number, flist->qual[flist->cnt].schema_date = a
   .schema_date
  WITH nocounter
 ;end select
 SELECT INTO value(fname)
  FROM (dummyt d  WITH seq = value(flist->cnt))
  DETAIL
   "update into dm_afd_tables c", row + 1, tempstr = build("set c.schema_date = cnvtdatetime('",
    format(flist->qual[d.seq].schema_date,"dd-mmm-yyyy hh:mm:ss;;D"),"')"),
   tempstr, row + 1, tempstr = build("where c.table_name = '",tab_list->qual[cnt].table_name,"' and"),
   tempstr, row + 1, tempstr = build("c.feature_number = ",flist->qual[d.seq].feature_number," and "),
   tempstr, row + 1, tempstr = build("c.alpha_feature_nbr = ",flist->qual[d.seq].ocd_number),
   tempstr, row + 1, "with nocounter go",
   row + 1, "commit go", row + 2
  WITH nocounter, append, maxcol = 512,
   format = variable, formfeed = none, maxrow = 2
 ...
 SELECT INTO value(fname)
  ...
  HEAD d.table_name
   ...
   row + 1, "1 table_name = vc", row + 1,
   "1 data_model_section = vc", row + 1, "1 description = vc",
   row + 1, "1 definition = vc", row + 1,
   "1 primary_update_script = vc", row + 1, "1 primary_insert_script = vc",
   row + 1, "1 primary_delete_script = vc", row + 1,
   "1 static_size_flg = i4", row + 1, "1 static_rows = i4",
   row + 1, "1 reads_flg = i4", row + 1,
   "1 update_flg = i4", row + 1, "1 insert_flg = i4",
   row + 1, "1 delete_flg = i4", row + 1,
   "1 core_ind = i2", row + 1, "1 bytes_per_row = i4",
   row + 1, "1 reference_ind = i2", row + 1,
   "1 pct_free  = i4", row + 1, "1 pct_used = i4",
   row + 1, "1 bpr_mean = f8", row + 1,
   "1 bpr_min = f8", row + 1, "1 bpr_max = f8",
   row + 1, "1 bpr_std_dev = f8", row + 1,
   "1 human_reqd_ind = i2", row + 1, "1 purge_except_ind = i2",
   row + 1, "1 freelist_cnt = i4  ) go", row + 2,
   ldesc = 0
   IF (size(trim(t.definition)) > 100)
    ...
    tempstr, row + 1
   ENDIF
   tempstr = build("set tbl_doc->table_name = '",t.table_name,"' go"), tempstr, row + 1,
   tempstr = build("set tbl_doc->data_model_section = '",t.data_model_section,"' go"), tempstr, row
    + 1,
   tempstr = build("set tbl_doc->description = '",t.description,"' go"), tempstr, row + 1
   IF (ldesc=1)
    tempstr = "set tbl_doc->definition = concat(trim(def1),trim(def2),trim(def3),trim(def4)) go"
   ELSE
    tempstr = build("set tbl_doc->definition = '",replace(t.definition,'"',"'",0),"' go")
   ENDIF
   tempstr, row + 1, tempstr = build("set tbl_doc->primary_update_script = '",trim(t
     .primary_update_script),"' go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->primary_insert_script = '",trim(t
     .primary_insert_script),"' go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->primary_delete_script = '",trim(t
     .primary_delete_script),"' go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->static_size_flg = ",t.static_size_flg," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->static_rows = ",t.static_rows," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->reads_flg = ",t.reads_flg," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->update_flg = ",t.update_flg," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->insert_flg = ",t.insert_flg," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->delete_flg = ",t.delete_flg," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->core_ind = ",t.core_ind," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->bytes_per_row = ",t.bytes_per_row," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->reference_ind = ",t.reference_ind," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->pct_free = ",t.pct_free," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->pct_used = ",t.pct_used," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->bpr_mean = ",t.bpr_mean," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->bpr_min = ",t.bpr_min," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->bpr_max = ",t.bpr_max," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->bpr_std_dev = ",t.bpr_std_dev," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->human_reqd_ind = ",t.human_reqd_ind," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->purge_except_ind = ",t.purge_except_ind," go"),
   tempstr, row + 1, tempstr = build("set tbl_doc->freelist_cnt= ",t.freelist_cnt," go"),
   tempstr, row + 2, row + 1,
   "free set col_doc go", row + 2, "record col_doc (",
   row + 1, "1 table_name = vc", row + 1,
   "1 cnt = i4", row + 1, "1 col[*]",
   row + 1, "2 column_name = vc", row + 1,
   "2 sequence_name = vc", row + 1, "2 code_set = i4",
   row + 1, "2 description = vc", row + 1,
   "2 definition = vc", row + 1, "2 flag_ind = i2",
   ...
   row + 2, tempstr = build("set col_doc->table_name = '",t.table_name,"' go"), tempstr,
   row + 1, cknt = 0
  DETAIL
   lcoldesc = 0
   IF (size(trim(d.definition)) > 90)
    ...
    row + 1
   ENDIF
   cknt = (cknt+ 1), row + 1, tempstr = build("set stat = alterlist(col_doc->col,",cknt,") go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->column_name = '",d.column_name,
    "' go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->sequence_name = '",trim(d
     .sequence_name),"' go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->code_set = ",d.code_set," go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->description = '",d.description,
    "' go"),
   tempstr, row + 1
   IF (lcoldesc=1)
    tempstr = build("set col_doc->col[",cknt,
     "]->definition = concat(trim(cdef1),trim(cdef2),trim(cdef3),trim(cdef4)) go")
   ELSE
    tempstr = build("set col_doc->col[",cknt,"]->definition = '",replace(d.definition,'"',"'",0),
     "' go")
   ENDIF
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->flag_ind = ",d.flag_ind," go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->unique_ident_ind = ",d
    .unique_ident_ind," go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->root_entity_name = '",trim(d
     .root_entity_name),"' go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->root_entity_attr = '",trim(d
     .root_entity_attr),"' go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->constant_value ='",trim(d
     .constant_value),"' go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->parent_entity_col='",trim(d
     .parent_entity_col),"' go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->exception_flg = ",d.exception_flg,
    " go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->defining_attribute_ind = ",d
    .defining_attribute_ind," go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->merge_updateable_ind = ",d
    .merge_updateable_ind," go"),
   tempstr, row + 1, tempstr = build("set col_doc->col[",cknt,"]->nls_col_ind = ",d.nls_col_ind," go"
    ),
   tempstr, row + 1
  FOOT  d.table_name
   tempstr = build("set col_doc->cnt = ",cknt," go"), tempstr, row + 2,
   "execute dm_ocd_tbl_doc go", row + 2
  WITH nocounter, append, maxcol = 512,
   format = variable, formfeed = none, maxrow = 2
 ;end select
END GO
