CREATE PROGRAM dm_eucb_referral_entity_reltn:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_EUCB_REFERRAL_ENTITY_RELTN"
 ...
 SUBROUTINE cust_ucb_del(dummy)
   UPDATE  FROM referral_entity_reltn
    SET active_ind = rchildren->qual1[det_cnt].prev_active_ind, active_status_cd = rchildren->qual1[
     ...
     parent_entity_id = request->xxx_uncombine[ucb_cnt].to_xxx_id
    WHERE (referral_entity_reltn_id=rchildren->qual1[det_cnt].entity_id)
    WITH nocounter
   ;end update
   IF (curqual=0)
