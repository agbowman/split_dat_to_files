CREATE PROGRAM cp_del_add_comp_details:dba
 PROMPT
  ...
  "Comp text" = "",
  "Node Id" = 0.0
  WITH outdev, compid, reltncd,
  entityid, entitynm, compident,
  comptext, nodeid
 FREE RECORD del_comp_detail_request
 RECORD del_comp_detail_request(
   1 qual[*]
 ...
 FREE RECORD del_comp_detail_reply
 RECORD del_comp_detail_reply(
   1 qual[*]
     2 cp_component_detail_id = f8
     2 cp_component_id = f8
     2 comp_detail_reltn_cd = f8
 ...
 FREE RECORD add_comp_details_req
 RECORD add_comp_details_req(
   1 comp_detail_list[*]
     2 cp_component_id = f8
     2 comp_detail_reltn_cd = f8
     2 component_entity_id = f8
 ...
 FREE RECORD add_comp_details_rep
 RECORD add_comp_details_rep(
   1 comp_detail_list[*]
     2 cp_component_detail_id = f8
     2 cp_component_id = f8
     2 comp_detail_reltn_cd = f8
 ...
 FREE RECORD cp_del_add_comp_details_reply
 RECORD cp_del_add_comp_details_reply(
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 DECLARE deletecomponentdetails(null) = null WITH protect
 DECLARE deletenodebehavior(null) = null WITH protect
 DECLARE addcomponentdetails(null) = vc WITH protect
 DECLARE componentid = f8 WITH protect, constant( $COMPID)
 DECLARE relationcd = f8 WITH protect, constant( $RELTNCD)
 DECLARE entityid = f8 WITH protect, constant( $ENTITYID)
 DECLARE entityname = vc WITH protect, constant( $ENTITYNM)
 DECLARE compident = vc WITH protect, constant( $COMPIDENT)
 DECLARE comptext = vc WITH protect, constant( $COMPTEXT)
 DECLARE nodeid = f8 WITH protect, constant( $NODEID)
 SUBROUTINE main(null)
   CALL log_message("In main()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(curtime3), private
   CALL deletecomponentdetails(null)
   CALL deletenodebehavior(null)
   IF (compident > " "
    AND comptext > " "
    AND relationcd > 0)
    CALL addcomponentdetails(null)
   ENDIF
   CALL log_message(build("Exit main(), Elapsed time in seconds:",((curtime3 - begin_date_time)/ 100)
     ),log_level_debug)
 END ;Subroutine
 SUBROUTINE deletecomponentdetails(null)
   CALL log_message("In deleteComponentDetails()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(curtime3), private
   ...
   SELECT INTO "nl:"
    ...
    PLAN (ccd
     WHERE ccd.cp_component_id=componentid)
    HEAD REPORT
     dcnt = 0
    HEAD ccd.cp_component_detail_id
     dcnt += 1
     IF (dcnt > size(del_comp_detail_request->qual,5))
      stat = alterlist(del_comp_detail_request->qual,(dcnt+ 5))
     ENDIF
     del_comp_detail_request->qual[dcnt].cp_component_id = ccd.cp_component_id,
     del_comp_detail_request->qual[dcnt].cp_component_detail_id = ccd.cp_component_detail_id,
     del_comp_detail_request->qual[dcnt].comp_detail_reltn_cd = ccd.component_detail_reltn_cd,
     del_comp_detail_request->qual[dcnt].component_entity_id = ccd.component_entity_id,
     del_comp_detail_request->qual[dcnt].component_entity_name = ccd.component_entity_name,
     del_comp_detail_request->qual[dcnt].component_ident = ccd.component_ident,
     del_comp_detail_request->qual[dcnt].component_text = ccd.component_text
    FOOT REPORT
     stat = alterlist(del_comp_detail_request->qual,dcnt)
    WITH nocounter
   ;end select
   IF (size(del_comp_detail_request->qual,5) > 0)
    EXECUTE cp_delete_component_detail  WITH replace(request,del_comp_detail_request)
   ELSE
    CALL echo("Unable to find CP_COMPONENT_DETAIL rows to delete.")
   ENDIF
   CALL log_message(build("Exit deleteComponentDetails(), Elapsed time in seconds:",((curtime3 -
     begin_date_time)/ 100)),log_level_debug)
 END ;Subroutine
 SUBROUTINE deletenodebehavior(null)
   CALL log_message("In deleteNodeBehavior()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(curtime3), private
   DECLARE bhvrcnt = i4 WITH protect, noconstant(0)
   SELECT INTO "nl:"
    FROM cp_node_behavior cnb
    PLAN (cnb
     WHERE cnb.cp_node_id=nodeid)
    HEAD REPORT
     bhvrcnt = 0
   ...
   ENDIF
   CALL log_message(build("Exit deleteNodeBehavior(), Elapsed time in seconds:",((curtime3 -
     begin_date_time)/ 100)),log_level_debug)
 END ;Subroutine
 SUBROUTINE addcomponentdetails(null)
   CALL log_message("In addComponentDetails()",log_level_debug)
   DECLARE begin_date_time = dq8 WITH constant(curtime3), private
   SET stat = alterlist(add_comp_details_req->comp_detail_list,1)
   SET add_comp_details_req->comp_detail_list[1].cp_component_id = componentid
   SET add_comp_details_req->comp_detail_list[1].component_text = comptext
   SET add_comp_details_req->comp_detail_list[1].component_ident = compident
   SET add_comp_details_req->comp_detail_list[1].component_entity_name = entityname
   SET add_comp_details_req->comp_detail_list[1].component_entity_id = entityid
   SET add_comp_details_req->comp_detail_list[1].comp_detail_reltn_cd = relationcd
   EXECUTE cp_add_component_detail  WITH replace("REQUEST","ADD_COMP_DETAILS_REQ"), replace("REPLY",
    "ADD_COMP_DETAILS_REP")
   IF ((add_comp_details_rep->status_data.status="S"))
    COMMIT
   ELSE
    IF (validate(debug_ind,0)=1)
     CALL echorecord(add_comp_details_rep)
    ENDIF
    CALL reporterrorandexit("PERFORM","INSERT"," CP_COMP_DETAILS")
   ENDIF
 ...
 SUBROUTINE (reporterrorandexit(operationname=vc,targetobjectname=vc,targetobjectvalue=vc) =null
  WITH protect)
   ...
   SET cp_del_add_comp_details_reply->status_data.status = "F"
   SET cp_del_add_comp_details_reply->status_data.subeventstatus.operationname = operationname
   SET cp_del_add_comp_details_reply->status_data.subeventstatus.operationstatus = "F"
   SET cp_del_add_comp_details_reply->status_data.subeventstatus.targetobjectname = targetobjectname
   SET cp_del_add_comp_details_reply->status_data.subeventstatus.targetobjectvalue =
   targetobjectvalue
   GO TO exit_script
 END ;Subroutine
 SET cp_del_add_comp_details_reply->status_data.status = "S"
 CALL main(null)
#exit_script
 IF (validate(debug_ind,0)=1)
  CALL echorecord(cp_del_add_comp_details_reply)
 ENDIF
 CALL putjsonrecordtofile(cp_del_add_comp_details_reply)
END GO
