CREATE PROGRAM afc_rca_get_flds_for_svc_item:dba
 ...
 IF ( NOT (validate(getlogicaldomainsystemuser)))
  ...
  END ;Subroutine
 ENDIF
 DECLARE fieldvalue = vc WITH protect, noconstant("")
 DECLARE populateanesthesiaflexfields(null) = null
 RECORD asareply(
   1 categories[*]
     2 categoryname = vc
     2 categorycd = f8
     2 categorymeaning = vc
     2 fields[*]
       3 fieldcode = f8
       3 fieldrequiredind = i2
       3 fielddisplay = vc
       3 fieldtype = vc
       3 optionlist[*]
         4 optionvalue = vc
 ...
 SUBROUTINE populateanesthesiaflexfields(null)
   DECLARE svcidx = i4 WITH private, noconstant(1)
   DECLARE catidx = i4 WITH private, noconstant(0)
   CALL logmessage("populateAnesthesiaFlexFields","Entering",log_debug)
   SET stat = alterlist(asareply->categories,1)
   SET asareply->categories[1].categoryname = uar_get_code_display(cs4002353_anesthesia_cd)
   SET asareply->categories[1].categorycd = cs4002353_anesthesia_cd
   SET asareply->categories[1].categorymeaning = uar_get_code_meaning(cs4002353_anesthesia_cd)
   SET stat = alterlist(asareply->categories[1].fields,14)
   IF ( NOT (getfieldvalue(cs4002352_casetime_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[1].fieldtype = "RANGE"
   ELSE
    SET asareply->categories[1].fields[1].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[1].fielddisplay = uar_get_code_display(cs4002352_casetime_cd)
   SET asareply->categories[1].fields[1].fieldmeaning = uar_get_code_meaning(cs4002352_casetime_cd)
   SET asareply->categories[1].fields[1].fieldcode = cs4002352_casetime_cd
   SET asareply->categories[1].fields[1].fieldrequiredind = true
   IF ( NOT (getfieldvalue(cs4002352_physrelief_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[2].fieldtype = "PROVIDERTIME"
   ELSE
    SET asareply->categories[1].fields[2].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[2].fielddisplay = uar_get_code_display(cs4002352_physrelief_cd)
   SET asareply->categories[1].fields[2].fieldmeaning = uar_get_code_meaning(cs4002352_physrelief_cd)
   SET asareply->categories[1].fields[2].fieldcode = cs4002352_physrelief_cd
   IF ( NOT (getfieldvalue(cs4002352_crnarelief_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[3].fieldtype = "PROVIDERTIME"
   ELSE
    SET asareply->categories[1].fields[3].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[3].fielddisplay = uar_get_code_display(cs4002352_crnarelief_cd)
   SET asareply->categories[1].fields[3].fieldmeaning = uar_get_code_meaning(cs4002352_crnarelief_cd)
   SET asareply->categories[1].fields[3].fieldcode = cs4002352_crnarelief_cd
   IF ( NOT (getfieldvalue(cs4002352_holdtime_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[4].fieldtype = "PROVIDERTIME"
   ELSE
    SET asareply->categories[1].fields[4].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[4].fielddisplay = uar_get_code_display(cs4002352_holdtime_cd)
   SET asareply->categories[1].fields[4].fieldmeaning = uar_get_code_meaning(cs4002352_holdtime_cd)
   SET asareply->categories[1].fields[4].fieldcode = cs4002352_holdtime_cd
   IF ( NOT (getfieldvalue(cs4002352_surgeon_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[5].fieldtype = "PROVLOOKUP"
   ELSE
    SET asareply->categories[1].fields[5].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[5].fielddisplay = uar_get_code_display(cs4002352_surgeon_cd)
   SET asareply->categories[1].fields[5].fieldmeaning = uar_get_code_meaning(cs4002352_surgeon_cd)
   SET asareply->categories[1].fields[5].fieldcode = cs4002352_surgeon_cd
   IF ( NOT (getfieldvalue(cs4002352_clmlvlnote_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[6].fieldtype = "STRING"
   ELSE
    SET asareply->categories[1].fields[6].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[6].fielddisplay = uar_get_code_display(cs4002352_clmlvlnote_cd)
   SET asareply->categories[1].fields[6].fieldmeaning = uar_get_code_meaning(cs4002352_clmlvlnote_cd)
   SET asareply->categories[1].fields[6].fieldcode = cs4002352_clmlvlnote_cd
   IF ( NOT (getfieldvalue(cs4002352_linelvlnote_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[7].fieldtype = "STRING"
   ELSE
    SET asareply->categories[1].fields[7].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[7].fielddisplay = uar_get_code_display(cs4002352_linelvlnote_cd
    )
   SET asareply->categories[1].fields[7].fieldmeaning = uar_get_code_meaning(cs4002352_linelvlnote_cd
    )
   SET asareply->categories[1].fields[7].fieldcode = cs4002352_linelvlnote_cd
   IF ( NOT (getfieldvalue(cs4002352_asacode_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[8].fieldtype = "CODE"
   ELSE
    SET asareply->categories[1].fields[8].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[8].fielddisplay = uar_get_code_display(cs4002352_asacode_cd)
   SET asareply->categories[1].fields[8].fieldmeaning = uar_get_code_meaning(cs4002352_asacode_cd)
   SET asareply->categories[1].fields[8].fieldcode = cs4002352_asacode_cd
   IF ( NOT (getfieldvalue(cs4002352_anesthesiologist_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[9].fieldtype = "PROVLOOKUP"
   ELSE
    SET asareply->categories[1].fields[9].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[9].fielddisplay = uar_get_code_display(
    cs4002352_anesthesiologist_cd)
   SET asareply->categories[1].fields[9].fieldmeaning = uar_get_code_meaning(
    cs4002352_anesthesiologist_cd)
   SET asareply->categories[1].fields[9].fieldcode = cs4002352_anesthesiologist_cd
   IF ( NOT (getfieldvalue(cs4002352_crnaaa_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[10].fieldtype = "PROVLOOKUP"
   ELSE
    SET asareply->categories[1].fields[10].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[10].fielddisplay = uar_get_code_display(cs4002352_crnaaa_cd)
   SET asareply->categories[1].fields[10].fieldmeaning = uar_get_code_meaning(cs4002352_crnaaa_cd)
   SET asareply->categories[1].fields[10].fieldcode = cs4002352_crnaaa_cd
   IF ( NOT (getfieldvalue(cs4002352_cptcode_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[11].fieldtype = "STRING"
   ELSE
    SET asareply->categories[1].fields[11].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[11].fielddisplay = uar_get_code_display(cs4002352_cptcode_cd)
   SET asareply->categories[1].fields[11].fieldmeaning = uar_get_code_meaning(cs4002352_cptcode_cd)
   SET asareply->categories[1].fields[11].fieldcode = cs4002352_cptcode_cd
   IF ( NOT (getfieldvalue(cs4002352_claimnotecd_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[12].fieldtype = "CODE"
   ELSE
    SET asareply->categories[1].fields[12].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[12].fielddisplay = uar_get_code_display(
    cs4002352_claimnotecd_cd)
   SET asareply->categories[1].fields[12].fieldmeaning = uar_get_code_meaning(
    cs4002352_claimnotecd_cd)
   SET asareply->categories[1].fields[12].fieldcode = cs4002352_claimnotecd_cd
   SET stat = alterlist(asareply->categories[1].fields[12].optionlist,1)
   SET asareply->categories[1].fields[12].optionlist[1].optionvalue = uar_get_code_meaning(
    cs26391_add_cd)
   SET asareply->categories[1].fields[12].optionlist[1].optioncd = cs26391_add_cd
   IF ( NOT (getfieldvalue(cs4002352_linenotecd_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[13].fieldtype = "CODE"
   ELSE
    SET asareply->categories[1].fields[13].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[13].fielddisplay = uar_get_code_display(cs4002352_linenotecd_cd
    )
   SET asareply->categories[1].fields[13].fieldmeaning = uar_get_code_meaning(cs4002352_linenotecd_cd
    )
   SET asareply->categories[1].fields[13].fieldcode = cs4002352_linenotecd_cd
   SET stat = alterlist(asareply->categories[1].fields[13].optionlist,1)
   SET asareply->categories[1].fields[13].optionlist[1].optionvalue = uar_get_code_meaning(
    cs26391_add_cd)
   SET asareply->categories[1].fields[13].optionlist[1].optioncd = cs26391_add_cd
   IF ( NOT (getfieldvalue(cs4002352_fieldavoidance_cd,fieldvalue,type)))
    SET asareply->categories[1].fields[14].fieldtype = "INDICATOR"
   ELSE
    SET asareply->categories[1].fields[14].fieldtype = fieldvalue
   ENDIF
   SET asareply->categories[1].fields[14].fielddisplay = uar_get_code_display(
    cs4002352_fieldavoidance_cd)
   SET asareply->categories[1].fields[14].fieldmeaning = uar_get_code_meaning(
    cs4002352_fieldavoidance_cd)
   SET asareply->categories[1].fields[14].fieldcode = cs4002352_fieldavoidance_cd
   SET stat = alterlist(asareply->categories[1].fields[14].optionlist,2)
   SET asareply->categories[1].fields[14].optionlist[1].optionvalue = uar_i18ngetmessage(i18nhandle,
    "k1","Yes")
   SET asareply->categories[1].fields[14].optionlist[1].optioncd = 1
   SET asareply->categories[1].fields[14].optionlist[2].optionvalue = uar_i18ngetmessage(i18nhandle,
    "k1","No")
   SET asareply->categories[1].fields[14].optionlist[2].optioncd = 2
 END ;Subroutine
 SUBROUTINE (getfieldvalue(pcodevalue=f8,prfieldvalue=vc(ref),pfieldname=vc) =i2)
   CALL logmessage("getFieldValue","Entering",log_debug)
   SELECT INTO "nl:"
    FROM code_value_extension cve
    WHERE cve.code_value=pcodevalue
     AND cve.field_name=pfieldname
    DETAIL
     prfieldvalue = cve.field_value
    WITH nocounter
   ;end select
   IF (prfieldvalue="")
    CALL addtracemessage("getFieldValue",build(
      "Unable to locate code value extension field value for code value: ",pcodevalue))
    CALL logmessage("getFieldValue","Exit",log_debug)
    RETURN(false)
   ELSE
    CALL logmessage("getFieldValue","Exit",log_debug)
    RETURN(true)
 ...
 CALL beginservice("CHARGSRV-14215.012")
 ...
 ENDIF
 FREE RECORD sptransaliasreq
 RECORD sptransaliasreq(
   1 trans_type = i2
 )
 FREE RECORD sptransaliasrep
 RECORD sptransaliasrep(
   1 transaliaslist[*]
     2 trans_alias_id = f8
     2 trans_alias_disp = vc
     2 trans_type_cd = f8
 ...
 DECLARE lineitemoverride = vc WITH protect, constant("LINEITEMOVER")
 ...
 ENDIF
 IF ( NOT (validate(cs4002353_anesthesia_cd)))
  DECLARE cs4002353_anesthesia_cd = f8 WITH protect, constant(getcodevalue(4002353,"ANESTHESIA",1))
 ENDIF
 IF ( NOT (validate(cs4002352_asacode_cd)))
  DECLARE cs4002352_asacode_cd = f8 WITH protect, constant(getcodevalue(4002352,"ASACODE",1))
 ENDIF
 IF ( NOT (validate(cs4002352_cptcode_cd)))
  DECLARE cs4002352_cptcode_cd = f8 WITH protect, constant(getcodevalue(4002352,"CPTCODE",1))
 ENDIF
 IF ( NOT (validate(cs4002352_casetime_cd)))
  DECLARE cs4002352_casetime_cd = f8 WITH protect, constant(getcodevalue(4002352,"CASETIME",1))
 ENDIF
 IF ( NOT (validate(cs4002352_holdtime_cd)))
  DECLARE cs4002352_holdtime_cd = f8 WITH protect, constant(getcodevalue(4002352,"HOLDTIME",1))
 ENDIF
 IF ( NOT (validate(cs4002352_crnarelief_cd)))
  DECLARE cs4002352_crnarelief_cd = f8 WITH protect, constant(getcodevalue(4002352,"CRNARELIEF",1))
 ENDIF
 IF ( NOT (validate(cs4002352_physrelief_cd)))
  DECLARE cs4002352_physrelief_cd = f8 WITH protect, constant(getcodevalue(4002352,"PHYSRELIEF",1))
 ENDIF
 IF ( NOT (validate(cs4002352_surgeon_cd)))
  DECLARE cs4002352_surgeon_cd = f8 WITH protect, constant(getcodevalue(4002352,"SURGEON",1))
 ENDIF
 IF ( NOT (validate(cs4002352_anesthesiologist_cd)))
  DECLARE cs4002352_anesthesiologist_cd = f8 WITH protect, constant(getcodevalue(4002352,
    "ANESTHESGIST",1))
 ENDIF
 IF ( NOT (validate(cs4002352_crnaaa_cd)))
  DECLARE cs4002352_crnaaa_cd = f8 WITH protect, constant(getcodevalue(4002352,"CRNAAA",1))
 ENDIF
 IF ( NOT (validate(cs4002352_clmlvlnote_cd)))
  DECLARE cs4002352_clmlvlnote_cd = f8 WITH protect, constant(getcodevalue(4002352,"CLMLVLNOTE",1))
 ENDIF
 IF ( NOT (validate(cs4002352_linelvlnote_cd)))
  DECLARE cs4002352_linelvlnote_cd = f8 WITH protect, constant(getcodevalue(4002352,"LINELVLNOTE",1))
 ENDIF
 IF ( NOT (validate(cs4002352_fieldavoidance_cd)))
  DECLARE cs4002352_fieldavoidance_cd = f8 WITH protect, constant(getcodevalue(4002352,"FLDAVOID",1))
 ENDIF
 IF ( NOT (validate(cs4002352_lineitemover_cd)))
  DECLARE cs4002352_lineitemover_cd = f8 WITH protect, constant(getcodevalue(4002352,"LINEITEMOVER",1
    ))
 ENDIF
 IF ( NOT (validate(cs4002352_claimnotecd_cd)))
  DECLARE cs4002352_claimnotecd_cd = f8 WITH protect, constant(getcodevalue(4002352,"CLAIMNOTECD",1))
 ENDIF
 IF ( NOT (validate(cs4002352_linenotecd_cd)))
  DECLARE cs4002352_linenotecd_cd = f8 WITH protect, constant(getcodevalue(4002352,"LINENOTECD",1))
 ENDIF
 IF ( NOT (validate(cs13019_bill_code_cd)))
  DECLARE cs13019_bill_code_cd = f8 WITH protect, constant(getcodevalue(13019,"BILL CODE",0))
 ENDIF
 IF ( NOT (validate(cs14002_asa_cd)))
  DECLARE cs14002_asa_cd = f8 WITH protect, constant(getcodevalue(14002,"ASA",0))
 ENDIF
 IF ( NOT (validate(cs26391_add_cd)))
  DECLARE cs26391_add_cd = f8 WITH protect, constant(getcodevalue(26391,"ADD",1))
 ENDIF
 IF ( NOT (validate(cs4002352_service_connected)))
  DECLARE cs4002352_service_connected = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",
    4002352,"SERVICECONNECTED"))
 ENDIF
 IF ( NOT (validate(cs4002352_special_authority)))
  DECLARE cs4002352_special_authority = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",
    4002352,"SPECIALAUTHORITY"))
 ENDIF
 IF ( NOT (validate(cs4002353_other)))
  DECLARE cs4002353_other = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",4002353,"OTHER"))
 ENDIF
 ...
 DECLARE cntbi = i4 WITH public, noconstant(0)
 DECLARE cntcat = i4 WITH public, noconstant(0)
 DECLARE cntfields = i4 WITH public, noconstant(0)
 DECLARE cntoptions = i4 WITH public, noconstant(0)
 DECLARE type = vc WITH public, constant("TYPE")
 DECLARE indicator = vc WITH public, constant("INDICATOR")
 DECLARE code = vc WITH public, constant("CODE")
 DECLARE codeset = vc WITH public, constant("CODE SET")
 DECLARE data_type = vc WITH public, constant("DATA_TYPE")
 ...
 DECLARE isanesthesiaserviceitem = i2 WITH public, noconstant(false)
 DECLARE isscsaenabled = i2 WITH protect, noconstant(false)
 DECLARE scsacount = i4 WITH public, noconstant(0)
 DECLARE fldidx = i4 WITH public, noconstant(0)
 DECLARE catidx = i4 WITH public, noconstant(0)
 DECLARE fieldidx = i4 WITH public, noconstant(0)
 DECLARE curqualsc = i4 WITH public, noconstant(0)
 DECLARE curqualsa = i4 WITH public, noconstant(0)
 DECLARE sccount = i4 WITH public, noconstant(0)
 DECLARE sacount = i4 WITH public, noconstant(0)
 DECLARE appendasacategoryfields(null) = null
 DECLARE getlineitemoverrideflexfields(null) = null
 CALL logmessage("Main","Begin afc_rca_get_flds_for_svc_item.",log_debug)
 IF (size(request->serviceitems,5)=0)
  CALL exitservicenodata("No service items found in request.",go_to_exit_script)
 ENDIF
 ...
 ENDIF
 IF (isscsaenabled)
  CALL getnonanesthesiacategoriesandfields(0)
  SELECT INTO "nl:"
   FROM bill_item_modifier bim,
    (dummyt d1  WITH seq = value(size(request->serviceitems,5)))
   PLAN (d1)
    JOIN (bim
    WHERE (bim.bill_item_id=request->serviceitems[d1.seq].serviceitemid)
     AND bim.bill_item_type_cd=cs13019_flex_cd
     AND bim.key2_id=cs4002353_other
     AND bim.key1_id=cs4002352_service_connected
     AND bim.active_ind=1)
   WITH nocounter
  ;end select
  SET curqualsc = curqual
  SELECT INTO "nl:"
   FROM bill_item_modifier bim,
    (dummyt d1  WITH seq = value(size(request->serviceitems,5)))
   PLAN (d1)
    ...
    WHERE (bim.bill_item_id=request->serviceitems[d1.seq].serviceitemid)
     AND bim.bill_item_type_cd=cs13019_flex_cd
     AND bim.key2_id=cs4002353_other
     AND bim.key1_id=cs4002352_special_authority
     AND bim.active_ind=1)
   WITH nocounter
  ;end select
  SET curqualsa = curqual
  IF (size(reply->serviceitems[1].categories,5) > 0)
   FOR (catidx = 1 TO size(reply->serviceitems[1].categories,5))
     IF ((reply->serviceitems[1].categories[catidx].categoryname=uar_get_code_display(cs4002353_other
      )))
      SET fieldsize = size(reply->serviceitems[1].categories[catidx].fields,5)
      FOR (fieldidx = 1 TO fieldsize)
        IF (curqualsc=0
         AND curqualsa=0
         AND scsacount=0)
         SET fldidx = (fieldsize+ 1)
         SET stat = alterlist(reply->serviceitems[1].categories[catidx].fields,(fieldsize+ 2))
         SET reply->serviceitems[1].categories[catidx].fields[fldidx].fieldcode =
         cs4002352_service_connected
         SET reply->serviceitems[1].categories[catidx].fields[fldidx].fielddisplay =
         uar_get_code_display(cs4002352_service_connected)
         SET reply->serviceitems[1].categories[catidx].fields[fldidx].fieldrequiredind = false
         SET reply->serviceitems[1].categories[catidx].fields[fldidx].fieldmeaning =
         uar_get_code_meaning(cs4002352_service_connected)
         SET fldidx += 1
         SET reply->serviceitems[1].categories[catidx].fields[fldidx].fieldcode =
         cs4002352_special_authority
         SET reply->serviceitems[1].categories[catidx].fields[fldidx].fielddisplay =
         uar_get_code_display(cs4002352_special_authority)
         SET reply->serviceitems[1].categories[catidx].fields[fldidx].fieldrequiredind = false
         SET reply->serviceitems[1].categories[catidx].fields[fldidx].fieldmeaning =
         uar_get_code_meaning(cs4002352_special_authority)
         SET scsacount += 1
         SET sccount += 1
         SET sacount += 1
         SET fieldidx = (fieldsize+ 1)
        ELSEIF (curqualsc=0
         AND sccount=0)
         IF ((reply->serviceitems[1].categories[catidx].fields[fieldidx].fieldcode !=
         cs4002352_service_connected))
          SET fldidx = (fieldsize+ 1)
          SET stat = alterlist(reply->serviceitems[1].categories[catidx].fields,(fieldsize+ 1))
          SET reply->serviceitems[1].categories[catidx].fields[fldidx].fieldcode =
          cs4002352_service_connected
          SET reply->serviceitems[1].categories[catidx].fields[fldidx].fielddisplay =
          uar_get_code_display(cs4002352_service_connected)
  ...
  IF (curqualsa=0
   AND curqualsc=0
   AND scsacount=0)
   IF (size(reply->serviceitems,5)=0)
    SET stat = alterlist(reply->serviceitems,1)
    SET reply->serviceitems[1].serviceitemid = request->serviceitems[1].serviceitemid
   ENDIF
   SET cntcat += 1
   SET stat = alterlist(reply->serviceitems[1].categories,cntcat)
   SET reply->serviceitems[1].categories[cntcat].categoryname = uar_get_code_display(cs4002353_other)
   CALL setcategorycodevalue(1,cntcat,cs4002353_other)
   SET stat = alterlist(reply->serviceitems[1].categories[cntcat].fields,2)
   SET reply->serviceitems[1].categories[cntcat].fields[1].fieldcode = cs4002352_service_connected
   SET reply->serviceitems[1].categories[cntcat].fields[1].fielddisplay = uar_get_code_display(
    cs4002352_service_connected)
   SET reply->serviceitems[1].categories[cntcat].fields[1].fieldrequiredind = false
   SET reply->serviceitems[1].categories[cntcat].fields[1].fieldmeaning = uar_get_code_meaning(
    cs4002352_service_connected)
   SET reply->serviceitems[1].categories[cntcat].fields[2].fieldcode = cs4002352_special_authority
   SET reply->serviceitems[1].categories[cntcat].fields[2].fielddisplay = uar_get_code_display(
    cs4002352_special_authority)
   SET reply->serviceitems[1].categories[cntcat].fields[2].fieldrequiredind = false
   SET reply->serviceitems[1].categories[cntcat].fields[2].fieldmeaning = uar_get_code_meaning(
    cs4002352_special_authority)
  ENDIF
 ELSE
  CALL getnonanesthesiacategoriesandfields(0)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d1)
   JOIN (b
   WHERE (b.bill_item_id=request->serviceitems[d1.seq].serviceitemid)
   ...
   WHERE bim.bill_item_id=b.bill_item_id
    ...
    AND bim.key1_id=cs14002_asa_cd
    AND ((bim.bim1_int=1) OR (bim.key2_id=1))
    AND bim.active_ind=true)
  DETAIL
   isanesthesiaserviceitem = true
  WITH nocounter
 ;end select
 IF (validate(request->serviceitems[1].chargeid))
  CALL getlineitemoverrideflexfields(null)
 ENDIF
 IF (size(reply->serviceitems,5) > 0)
  CALL logmessage("Main","Get flex field type",log_debug)
  SELECT INTO "nl:"
   FROM (dummyt d1  WITH seq = value(size(reply->serviceitems,5))),
    (dummyt d2  WITH seq = 1),
    (dummyt d3  WITH seq = 1),
    code_value_extension cve
   PLAN (d1
    WHERE maxrec(d2,size(reply->serviceitems[d1.seq].categories,5)))
    JOIN (d2
    WHERE maxrec(d3,size(reply->serviceitems[d1.seq].categories[d2.seq].fields,5)))
    JOIN (d3)
    JOIN (cve
    WHERE (cve.code_value=reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].fieldcode)
     AND cve.field_name IN (type, data_type))
   DETAIL
    reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].fieldtype = cve.field_value
   WITH nocounter
  ;end select
  CALL logmessage("Main","Populate yes/no value for indicator types",log_debug)
  SELECT INTO "nl:"
   FROM (dummyt d1  WITH seq = value(size(reply->serviceitems,5))),
    (dummyt d2  WITH seq = 1),
    (dummyt d3  WITH seq = 1)
   PLAN (d1
    ...
    WHERE maxrec(d3,size(reply->serviceitems[d1.seq].categories[d2.seq].fields,5)))
    JOIN (d3
    WHERE (reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].fieldtype=indicator))
   DETAIL
    stat = alterlist(reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].optionlist,2),
    reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].optionlist[1].optionvalue =
    uar_i18ngetmessage(i18nhandle,"k1","Yes"), reply->serviceitems[d1.seq].categories[d2.seq].fields[
    d3.seq].optionlist[1].optioncd = 1,
    reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].optionlist[2].optionvalue =
    uar_i18ngetmessage(i18nhandle,"k1","No"), reply->serviceitems[d1.seq].categories[d2.seq].fields[
    d3.seq].optionlist[2].optioncd = 0
   WITH nocounter
  ;end select
  CALL logmessage("Main","Populate code set list values for code types",log_debug)
  SELECT INTO "nl:"
   ldseq = d3.seq
   FROM (dummyt d1  WITH seq = value(size(reply->serviceitems,5))),
    (dummyt d2  WITH seq = 1),
   ...
   PLAN (d1
    ...
    WHERE maxrec(d3,size(reply->serviceitems[d1.seq].categories[d2.seq].fields,5)))
    JOIN (d3
    WHERE (reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].fieldtype=code))
    JOIN (cve
    WHERE (cve.code_value=reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].fieldcode)
     AND cve.field_name=codeset
     AND cnvtreal(cve.field_value) > 0)
    JOIN (cv
    WHERE cv.code_set=cnvtreal(cve.field_value)
     AND cv.active_ind=1)
   HEAD ldseq
    cntoptions = 0
   DETAIL
    cntoptions += 1, stat = alterlist(reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].
     optionlist,cntoptions), reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].
    optionlist[cntoptions].optionvalue = cv.display,
    reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].optionlist[cntoptions].optioncd =
    cv.code_value
   WITH nocounter
  ;end select
  CALL logmessage("Main","Populate transaction aliases",log_debug)
  SET sptransaliasreq->trans_type = 0
  EXECUTE pft_rca_get_sp_trans_alias  WITH replace("REQUEST",sptransaliasreq), replace("REPLY",
   sptransaliasrep)
  IF ((sptransaliasrep->status_data.status != "S"))
   CALL exitservicefailure("Failed retrieving transaction aliases",go_to_exit_script)
  ELSE
   IF (size(sptransaliasrep->transaliaslist,5) > 0)
    SELECT INTO "nl:"
     ldseq = d3.seq
     FROM (dummyt d1  WITH seq = value(size(reply->serviceitems,5))),
      (dummyt d2  WITH seq = 1),
      (dummyt d3  WITH seq = 1),
      (dummyt d4  WITH seq = value(size(sptransaliasrep->transaliaslist,5)))
     PLAN (d1
      WHERE maxrec(d2,size(reply->serviceitems[d1.seq].categories,5)))
      JOIN (d2
      WHERE maxrec(d3,size(reply->serviceitems[d1.seq].categories[d2.seq].fields,5)))
      JOIN (d3
      WHERE uar_get_code_meaning(reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].
       fieldcode)="TRANSALIAS")
      JOIN (d4)
     HEAD ldseq
      cntoptions = 0
     DETAIL
      cntoptions += 1, stat = alterlist(reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq]
       .optionlist,cntoptions), reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].
      optionlist[cntoptions].optionvalue = concat(trim(sptransaliasrep->transaliaslist[d4.seq].
        trans_alias_disp),"-",nullterm(uar_get_code_display(sptransaliasrep->transaliaslist[d4.seq].
         trans_sub_type_cd)),"->",nullterm(uar_get_code_display(sptransaliasrep->transaliaslist[d4
         .seq].trans_reason_cd))),
      reply->serviceitems[d1.seq].categories[d2.seq].fields[d3.seq].optionlist[cntoptions].optioncd
       = sptransaliasrep->transaliaslist[d4.seq].trans_alias_id
     WITH nocounter
    ;end select
   ENDIF
  ENDIF
 ENDIF
 IF (isanesthesiaserviceitem=true)
  CALL populateanesthesiaflexfields(null)
  CALL appendasacategoryfields(null)
 ENDIF
 CALL logmessage("Main","Exiting script afc_rca_get_flds_for_svc_item.",log_debug)
 IF (size(reply->serviceitems,5) > 0)
  CALL exitservicesuccess("")
 ELSE
  CALL exitservicenodata("No fields for service item found",go_to_exit_script)
 ENDIF
 SUBROUTINE (setcategorycodevalue(pserviceitempos=i4,pcategoriespos=i4,pcategorycodevalue=f8) =null)
   IF (validate(reply->serviceitems[pserviceitempos].categories[pcategoriespos].categorycd))
    SET reply->serviceitems[pserviceitempos].categories[pcategoriespos].categorycd =
    pcategorycodevalue
    SET reply->serviceitems[pserviceitempos].categories[pcategoriespos].categorymeaning =
    uar_get_code_meaning(pcategorycodevalue)
   ENDIF
 END ;Subroutine
 SUBROUTINE (getnonanesthesiacategoriesandfields(dummyvar=i2) =i2)
   CALL logmessage("getNonAnesthesiaCategoriesAndFields","Entering...",log_debug)
   SELECT INTO "nl:"
    FROM bill_item_modifier bim,
     (dummyt d1  WITH seq = value(size(request->serviceitems,5)))
    PLAN (d1)
     ...
     WHERE (bim.bill_item_id=request->serviceitems[d1.seq].serviceitemid)
      ...
      AND bim.active_ind=1)
    ORDER BY bim.bill_item_id, bim.key2_id, bim.bill_item_type_cd,
     bim.bim1_int
    HEAD bim.bill_item_id
     cntbi += 1, cntcat = 0, stat = alterlist(reply->serviceitems,cntbi),
     reply->serviceitems[cntbi].serviceitemid = bim.bill_item_id
    HEAD bim.key2_id
     cntcat += 1, cntfields = 0, stat = alterlist(reply->serviceitems[cntbi].categories,cntcat)
     IF (bim.bill_item_type_cd=cs13019_flex_cd)
      reply->serviceitems[cntbi].categories[cntcat].categoryname = uar_get_code_display(bim.key2_id)
     ELSE
      reply->serviceitems[cntbi].categories[cntcat].categoryname = i18n_qty_calculation
     ENDIF
     CALL setcategorycodevalue(cntbi,cntcat,bim.key2_id)
    HEAD bim.bill_item_type_cd
     null
    HEAD bim.bim1_int
     cntfields += 1, stat = alterlist(reply->serviceitems[cntbi].categories[cntcat].fields,cntfields),
     reply->serviceitems[cntbi].categories[cntcat].fields[cntfields].fieldcode = bim.key1_id,
     reply->serviceitems[cntbi].categories[cntcat].fields[cntfields].fielddisplay =
     uar_get_code_display(bim.key1_id), reply->serviceitems[cntbi].categories[cntcat].fields[
     cntfields].fieldrequiredind = bim.bim_ind, reply->serviceitems[cntbi].categories[cntcat].fields[
     cntfields].fieldmeaning = uar_get_code_meaning(bim.key1_id)
    DETAIL
     null
    WITH nocounter
   ;end select
   CALL logmessage("getNonAnesthesiaCategoriesAndFields","Exiting...",log_debug)
   RETURN(true)
 END ;Subroutine
 SUBROUTINE appendasacategoryfields(null)
   DECLARE cntr = i4 WITH private, noconstant(0)
   DECLARE notexistcnt = i4 WITH private, noconstant(0)
   DECLARE existpos = i4 WITH private, noconstant(0)
   IF (size(reply->serviceitems,5)=0)
    SET stat = alterlist(reply->serviceitems,1)
   ENDIF
   SET index = locateval(idx,1,size(reply->serviceitems[1].categories,5),uar_get_code_display(
     cs4002353_anesthesia_cd),reply->serviceitems[1].categories[idx].categoryname)
   IF (index=0)
    SET cntcat += 1
    SET stat = alterlist(reply->serviceitems[1].categories,cntcat)
    SET reply->serviceitems[1].categories[cntcat].categorycd = asareply->categories[1].categorycd
    SET reply->serviceitems[1].categories[cntcat].categorymeaning = asareply->categories[1].
    categorymeaning
    SET reply->serviceitems[1].categories[cntcat].categoryname = asareply->categories[1].categoryname
    SET index = cntcat
   ENDIF
   SET asafieldcnt = size(reply->serviceitems[1].categories[index].fields,5)
   FOR (asareplycntr = 1 TO size(asareply->categories[1].fields,5))
    SET existpos = locateval(cntr,1,asafieldcnt,asareply->categories[1].fields[asareplycntr].
     fieldcode,reply->serviceitems[1].categories[index].fields[cntr].fieldcode)
    IF (existpos=0)
     SET asafieldcnt += 1
     SET stat = alterlist(reply->serviceitems[1].categories[index].fields,asafieldcnt)
     SET reply->serviceitems[1].categories[index].fields[asafieldcnt].fieldcode = asareply->
     categories[1].fields[asareplycntr].fieldcode
     SET reply->serviceitems[1].categories[index].fields[asafieldcnt].fielddisplay = asareply->
     categories[1].fields[asareplycntr].fielddisplay
     SET reply->serviceitems[1].categories[index].fields[asafieldcnt].fieldmeaning = asareply->
     categories[1].fields[asareplycntr].fieldmeaning
     SET reply->serviceitems[1].categories[index].fields[asafieldcnt].fieldrequiredind = asareply->
     categories[1].fields[asareplycntr].fieldrequiredind
     SET reply->serviceitems[1].categories[index].fields[asafieldcnt].fieldtype = asareply->
     categories[1].fields[asareplycntr].fieldtype
     IF (size(asareply->categories[1].fields[asareplycntr].optionlist,5) > 0)
      SET stat = moverec(asareply->categories[1].fields[asareplycntr].optionlist,reply->serviceitems[
       1].categories[index].fields[asafieldcnt].optionlist)
     ENDIF
    ENDIF
   ENDFOR
 END ;Subroutine
 SUBROUTINE getlineitemoverrideflexfields(null)
   SELECT INTO "nl:"
    FROM charge c,
     charge_mod cm
    PLAN (c
     WHERE (c.charge_item_id=request->serviceitems[1].chargeid)
      AND c.active_ind=true)
     JOIN (cm
     WHERE cm.charge_item_id=c.charge_item_id
      AND cm.charge_mod_type_cd=cs13019_flex_cd
      AND cm.field1_id=cs4002352_lineitemover_cd
      AND cm.active_ind=true)
    DETAIL
     IF (size(reply->serviceitems,5)=0)
      stat = alterlist(reply->serviceitems,1), reply->serviceitems[1].serviceitemid = request->
      serviceitems[1].serviceitemid
     ENDIF
     cntcat += 1, stat = alterlist(reply->serviceitems[1].categories,cntcat), reply->serviceitems[1].
     categories[cntcat].categoryname = lineitemoverride,
     stat = alterlist(reply->serviceitems[1].categories[cntcat].fields,1), reply->serviceitems[1].
     categories[cntcat].fields[1].fieldcode = cs4002352_lineitemover_cd, reply->serviceitems[1].
     categories[cntcat].fields[1].fielddisplay = uar_get_code_display(cs4002352_lineitemover_cd),
     reply->serviceitems[1].categories[cntcat].fields[1].fieldrequiredind = false, reply->
     serviceitems[1].categories[cntcat].fields[1].fieldmeaning = uar_get_code_meaning(
      cs4002352_lineitemover_cd)
    WITH maxrec = 1
   ;end select
 END ;Subroutine
#exit_script
