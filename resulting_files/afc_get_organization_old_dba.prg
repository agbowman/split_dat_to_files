CREATE PROGRAM afc_get_organization_old:dba
 RECORD reply(
   1 organization_qual = i2
   1 organization[10]
     2 org_name = vc
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   IF (mod(count1,10)=1
    AND count1 != 1)
    stat = alter(reply->organization,(count1+ 10))
   ENDIF
   reply->organization[count1].org_name = o.org_name, reply->organization[count1].organization_id = o
   .organization_id
  WITH nocounter
 ;end select
 SET stat = alter(reply->organization,count1)
