CREATE PROGRAM ags_claim_detail_purge:dba
 PROMPT
  "Backout Type (J, I, P) =" = "J",
  "AGS_JOB_ID (0.0) = " = 0.0
  WITH cbackout_type, djob_id
 CALL echo("***")
 CALL echo("***   BEG :: AGS_CLAIM_DETAIL_PURGE")
 CALL echo("***")
 ...
 IF ( NOT (validate(log,0)))
  ...
  DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_claim_detail_purge_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_CLAIM_DETAIL_PURGE"
 ...
 IF (the_type="P"
  ...
  SELECT INTO "nl:"
   ...
   PLAN (d
    ...
    WHERE (a.person_id=person_rec->qual[d.seq].person_id)
     AND a.hea_claim_visit_detail_id > 0)
   HEAD REPORT
   ...
   DETAIL
    ...
    ENDIF
    purge_rec->qual[sknt].mil_item_id = a.hea_claim_visit_detail_id, purge_rec->qual[sknt].
    delete_item_id = a.ags_claim_detail_data_id, purge_rec->qual[sknt].delete_person_id = a.person_id,
  ...
  IF (ierrcode > 0)
   ...
   SET log->qual[log->qual_knt].smsgtype = "ERROR"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("SELECT ERROR HEA_CLAIM_VISIT_DETAIL (delete items)>> ",
    trim(serrmsg))
   GO TO exit_script
  ENDIF
 ELSEIF (the_type="J"
  ...
  IF (ierrcode > 0)
   ...
   SET log->qual[log->qual_knt].smsgtype = "ERROR"
   SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
   SET log->qual[log->qual_knt].smsg = concat("SELECT ERROR AGS_CLAIM_DETAIL_DATA (delete items)>> ",
    trim(serrmsg))
   GO TO exit_script
  ENDIF
 ELSEIF (the_type="I")
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = "INVALID PURGE >> CLAIM_DETAIL by Item Not Allowed"
  GO TO exit_script
 ELSE
  SET failed = input_error
  SET ilog_status = 1
 ...
 SELECT INTO "nl:"
  ...
  HEAD d.seq
   purge_rec->qual[d.seq].bo_item_id = a.ags_claim_detail_data_id
  WITH nocounter
 ;end select
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("SELECT ERROR AGS_CLAIM_DETAIL_DATA (backout items)>> ",
   trim(serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 DELETE  FROM hea_claim_visit_detail h,
  ...
  PLAN (d
   ...
   JOIN (h
   WHERE (h.hea_claim_visit_detail_id=purge_rec->qual[d.seq].mil_item_id))
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("DELETE ERROR HEA_CLAIM_VISIT_DETAIL >> ",trim(serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 SET ierrcode = 0
 UPDATE  FROM ags_claim_detail_data a,
   (dummyt d  WITH seq = value(purge_rec->qual_knt))
  SET a.status = "BACK OUT", a.status_dt_tm = cnvtdatetime(curdate,curtime3), a
   .hea_claim_visit_detail_id = 0.0,
   a.hea_claim_visit_id = 0.0
  PLAN (d
   WHERE d.seq > 0
    AND (purge_rec->qual[d.seq].bo_item_id > 0))
   JOIN (a
   WHERE (a.ags_claim_detail_data_id=purge_rec->qual[d.seq].bo_item_id))
  WITH nocounter, maxcommit = 5000
 ;end update
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_CLAIM_DETAIL_DATA (backout items) >> ",
   trim(serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 ...
 UPDATE  FROM ags_claim_detail_data a,
  ...
  PLAN (d
   WHERE d.seq > 0
    ...
    AND (purge_rec->qual[d.seq].delete_item_id > 0))
   JOIN (a
   WHERE (a.ags_claim_detail_data_id=purge_rec->qual[d.seq].delete_item_id))
  WITH nocounter, maxcommit = 5000
 ;end update
 SET ierrcode = error(serrmsg,1)
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("UPDATE ERROR AGS_CLAIM_DETAIL_DATA (delete items) >> ",
   trim(serrmsg))
  GO TO exit_script
 ENDIF
 SET ierrcode = error(serrmsg,1)
 ...
 IF (ierrcode > 0)
  ...
  SET log->qual[log->qual_knt].smsgtype = "ERROR"
  SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
  SET log->qual[log->qual_knt].smsg = concat("DELETE ERROR AGS_CLAIM_DETAIL_DATA >> ",trim(serrmsg))
  GO TO exit_script
 ENDIF
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_CLAIM_DETAIL_PURGE"
 IF (define_logging_sub=true)
  CALL echo("***")
 ...
 ENDIF
 CALL echo("***")
 CALL echo("***   END :: AGS_CLAIM_DETAIL_PURGE")
 CALL echo("***")
