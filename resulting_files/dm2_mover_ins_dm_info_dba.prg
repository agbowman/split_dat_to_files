CREATE PROGRAM dm2_mover_ins_dm_info:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 SET readme_data->message = 'Insert "DM2_CYCLE_DATA_MOVERS" into table DM_INFO'
 ...
 IF (check_error("Query 'DM2_CYCLE_DATA_MOVERS' from dm_info table") != 0)
  ...
  SET readme_data->message = "FAILED: no row was selected"
  GO TO exit_program
 ENDIF
 IF (curqual=0)
  INSERT  FROM dm_info
   SET info_domain = "DM_STAT_GATHER", info_name = "DM2_CYCLE_DATA_MOVERS", info_number = 60,
  ...
  IF (check_error("Insert into table dm_info") != 0)
   ROLLBACK
   SET readme_data->status = "F"
   SET readme_data->message = "FAILED: no row was inserted"
   GO TO exit_program
  ENDIF
  COMMIT
 ENDIF
 SET readme_data->status = "S"
 SET readme_data->message = "The row was inserted successfully"
#exit_program
 EXECUTE dm_readme_status
