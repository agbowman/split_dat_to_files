 ...
 IF ((((orders->spoolout_ind=1)) OR ((request->print_prsnl_id > 0))) )
  ...
  SELECT INTO value(request->printer_name)
   ...
   HEAD PAGE
    ...
    "DOB:", xcol = 250,
    CALL print(calcpos(xcol,xrow)),
    orders->dob, xrow = (xrow+ 14), xcol = 50,
    row + 1, col 1,
    CALL print(calcpos(xcol,xrow)),
    "Sex:", xcol = 100,
    CALL print(calcpos(xcol,xrow)),
    orders->sex, xrow = (xrow+ 20), xcol = 50,
    row + 1, col 1,
    CALL print(calcpos(xcol,xrow)),
    "Primary Diagnosis:", xcol = 145,
    CALL print(calcpos(xcol,xrow)),
    orders->admit_diagnosis, xrow = (xrow+ 20), xcol = 44,
    row + 1,
