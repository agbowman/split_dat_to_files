CREATE PROGRAM aps_chk_move_pt_hold_comments:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_chk_move_pt_hold_comments"
...
#exit_script
 IF (itemcount > 0)
  ROLLBACK
  CALL logmsg("Processing_task, Long_text populate failed.",((e_rdmstatus+ e_dispmsg)+ e_logtofile))
  CALL logstatus("Execution failed - see log message for more details.","F")
 ELSE
  CALL logmsg("Processing_task, Long_text populate successful.",((e_rdmstatus+ e_dispmsg)+
   e_logtofile))
