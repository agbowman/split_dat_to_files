CREATE PROGRAM dm_eucb_ce_event_order_link:dba
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].script_name = "DM_EUCB_CE_EVENT_ORDER_LINK"
 ...
 IF ((rchildren->qual1[det_cnt].combine_action_cd=upt))
  IF ((rchildren->qual1[det_cnt].entity_id > 0))
   CALL cust_ucb_upt(null)
  ELSE
   SELECT INTO "nl:"
    FROM orders o
    WHERE (o.encntr_id=request->xxx_uncombine[ucb_cnt].to_xxx_id)
    HEAD REPORT
     orders_count = 0
    DETAIL
     orders_count += 1
     IF (orders_count > size(temp_rec->orders,5))
      stat = alterlist(temp_rec->orders,(orders_count+ 5))
     ENDIF
     temp_rec->orders[orders_count].order_id = o.order_id
    FOOT REPORT
     stat = alterlist(temp_rec->orders,orders_count)
    WITH nocounter
   ;end select
   UPDATE  FROM ce_event_order_link ceor
    SET ceor.encntr_id = request->xxx_uncombine[ucb_cnt].to_xxx_id, ceor.updt_dt_tm = ceor.updt_dt_tm,
     ceor.updt_applctx = reqinfo->updt_applctx,
     ceor.updt_id = reqinfo->updt_id, ceor.updt_cnt = (ceor.updt_cnt+ 1), ceor.updt_task = reqinfo->
     updt_task
    WHERE expand(num,1,size(temp_rec->orders,5),ceor.order_id,temp_rec->orders[num].order_id)
 ...
 SUBROUTINE cust_ucb_upt(null)
   ...
   SET cust_upt_buff = concat("update into ",trim(rchildren->qual1[det_cnt].entity_name),
    ...
    " and ce_event_order_link_id "," = rChildren->QUAL1[det_cnt]->ENTITY_ID ","with nocounter go ")
