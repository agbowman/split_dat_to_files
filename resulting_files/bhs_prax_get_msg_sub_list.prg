CREATE PROGRAM bhs_prax_get_msg_sub_list
 ...
 SELECT INTO  $1
  ...
  c.long_blob_id, template_cki = trim(replace(replace(replace(replace(replace(c.cki,"&","&amp;",0),
       "<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3), nt.note_type_id,
  note_type_desc = trim(replace(replace(replace(replace(replace(nt.note_type_description,"&","&amp;",
        0),"<","&lt;",0),">","&gt;",0),"'","&apos;",0),'"',"&quot;",0),3)
  ...
  PLAN (m
   ...
   JOIN (n
   WHERE n.template_id=c.template_id)
   JOIN (nt
  ...
  DETAIL
   ...
   v5, row + 1, v6 = build("<NoteTypeId>",cnvtint(nt.note_type_id),"</NoteTypeId>"),
   col + 1, v6, row + 1,
   v7 = build("<NoteType>",note_type_desc,"</NoteType>"), col + 1, v7,
   row + 1, col 1, "</SubjectList>",
   row + 1
  FOOT REPORT
