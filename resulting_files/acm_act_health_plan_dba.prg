CREATE PROGRAM acm_act_health_plan:dba
 ...
 FOR (index = 1 TO xref->act_cnt)
   SET reply->health_plan_qual[xref->act[index].idx].status = 0
 ENDFOR
 IF (t_val <= max_val)
  SET max_val = t_val
 ...
 UPDATE  FROM (dummyt d  WITH seq = value(xref->act_cnt)),
   health_plan h
  SET h.active_ind = xref->act[d.seq].active_ind, h.active_status_cd =
   ...
   h.active_status_dt_tm = cnvtdatetime(sysdate), h.updt_cnt = (h.updt_cnt+ 1), h.updt_dt_tm =
   cnvtdatetime(sysdate),
   h.updt_id = reqinfo->updt_id, h.updt_applctx = reqinfo->updt_applctx, h.updt_task = reqinfo->
   updt_task
  PLAN (d)
   JOIN (h
   WHERE (h.health_plan_id=acm_request->health_plan_qual[xref->act[d.seq].idx].health_plan_id))
  WITH nocounter, status(reply->health_plan_qual[xref->act[d.seq].idx].status)
 ;end update
 FOR (index = 1 TO xref->act_cnt)
   IF ((reply->health_plan_qual[xref->act[index].idx].status != 1))
 ...
 SUBROUTINE (getexistingrows(x=i4) =null)
   SELECT INTO "nl:"
    FROM health_plan h
    WHERE expand(t1,f_val,t_val,h.health_plan_id,acm_request->health_plan_qual[xref->act[t1].idx].
     health_plan_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,h.health_plan_id,acm_request->health_plan_qual[xref->act[t1].idx].
      health_plan_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->health_plan_qual[xref->act[index].idx].status=0))
