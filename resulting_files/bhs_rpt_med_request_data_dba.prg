CREATE PROGRAM bhs_rpt_med_request_data:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (rmr
   ...
   WHERE elh.encntr_id=e.encntr_id
    AND rmr.request_dt_tm BETWEEN elh.beg_effective_dt_tm AND elh.end_effective_dt_tm
  ...
  DETAIL
   ncnt += 1
   IF (curutc=1)
    requestdttmgmt = cnvtdatetimeutc(cnvtdatetime(rmr.request_dt_tm),1), processdttmgmt =
    cnvtdatetimeutc(cnvtdatetime(rmr.rx_action_dt_tm),1)
   ENDIF
   srequestdttm = utcdatetime(cnvtdatetime(rmr.request_dt_tm),rmr.request_tz,0,"@SHORTDATETIME"),
   sprocessdttm = utcdatetime(cnvtdatetime(rmr.rx_action_dt_tm),rmr.rx_action_tz,0,"@SHORTDATETIME"),
   srequesttz = utcshorttz(rmr.request_tz),
   sprocesstz = utcshorttz(rmr.rx_action_tz), requestdatetime = rmr.request_dt_tm, actiondatetime =
   rmr.rx_action_dt_tm,
   sstatus = uar_get_code_meaning(rmr.rx_status_cd), delapsedtime = get_queue_time(requestdatetime,
    actiondatetime,sstatus)
   IF (mod(ncnt,100)=1)
    stat = alterlist(medreqdata->qual,(ncnt+ 99))
   ENDIF
   medreqdata->qual[ncnt].patient_name = p.name_full_formatted, medreqdata->qual[ncnt].s_fin = ea
   .alias, medreqdata->qual[ncnt].facility_cd = e.loc_facility_cd,
   medreqdata->qual[ncnt].location_cd = elh.loc_nurse_unit_cd, medreqdata->qual[ncnt].order_desc = o
   .ordered_as_mnemonic, medreqdata->qual[ncnt].order_detail = o.clinical_display_line,
   medreqdata->qual[ncnt].requested_on = srequestdttm, medreqdata->qual[ncnt].request_tz = srequesttz,
   medreqdata->qual[ncnt].request_gmt = requestdttmgmt,
   medreqdata->qual[ncnt].requested_by = pr.name_full_formatted, medreqdata->qual[ncnt].priority_cd
    = rmr.request_priority_cd, medreqdata->qual[ncnt].request_reason_cd = rmr.request_reason_cd,
   medreqdata->qual[ncnt].request_comment = l.long_text, medreqdata->qual[ncnt].processed_on =
   sprocessdttm, medreqdata->qual[ncnt].process_tz = sprocesstz,
   medreqdata->qual[ncnt].process_gmt = processdttmgmt, medreqdata->qual[ncnt].processed_by = prs
   .name_full_formatted, medreqdata->qual[ncnt].action_cd = rmr.rx_action_cd,
   medreqdata->qual[ncnt].status_cd = rmr.rx_status_cd, medreqdata->qual[ncnt].process_reason_cd =
   rmr.rx_reason_cd, medreqdata->qual[ncnt].process_comment = lt.long_text,
 ...
 SUBROUTINE (get_queue_time(request_date_time=dq8,action_date_time=dq8,status=vc) =f8)
 ...
 SUBROUTINE export_data_to_csv(dummy_m)
   ...
   ELSE
    SELECT INTO  $OUTDEV
     patient_name = substring(1,100,medreqdata->qual[d.seq].patient_name), fin_# = substring(1,40,
      medreqdata->qual[d.seq].s_fin), facility = substring(1,40,uar_get_code_display(medreqdata->
       qual[d.seq].facility_cd)),
     location = substring(1,40,uar_get_code_display(medreqdata->qual[d.seq].location_cd)),
     order_description = substring(1,100,medreqdata->qual[d.seq].order_desc), order_detail =
     substring(1,255,medreqdata->qual[d.seq].order_detail),
     requested_on = substring(1,20,medreqdata->qual[d.seq].requested_on), requested_timezone =
     substring(1,40,medreqdata->qual[d.seq].request_tz), requested_on_gmt = substring(1,20,format(
       medreqdata->qual[d.seq].request_gmt,"@SHORTDATETIME")),
     requested_by = substring(1,100,medreqdata->qual[d.seq].requested_by), priority = substring(1,40,
      uar_get_code_display(medreqdata->qual[d.seq].priority_cd)), requested_reason = substring(1,40,
      uar_get_code_display(medreqdata->qual[d.seq].request_reason_cd)),
     requested_comment = check(substring(1,1000,medreqdata->qual[d.seq].request_comment),char(14)),
     processed_on = substring(1,20,medreqdata->qual[d.seq].processed_on), processed_timezone =
     substring(1,40,medreqdata->qual[d.seq].process_tz),
     processed_on_gmt = substring(1,20,format(medreqdata->qual[d.seq].process_gmt,"@SHORTDATETIME")),
     processed_by = substring(1,100,medreqdata->qual[d.seq].processed_by), action = substring(1,40,
      uar_get_code_display(medreqdata->qual[d.seq].action_cd)),
     status = substring(1,40,uar_get_code_display(medreqdata->qual[d.seq].status_cd)),
     processed_reason = substring(1,40,uar_get_code_display(medreqdata->qual[d.seq].process_reason_cd
 ...
 CALL echo("********** END RX_RPT_STD_MED_REQUEST **********")
END GO
