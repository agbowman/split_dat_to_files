CREATE PROGRAM dm2_replicate_expimp_menu:dba
 ...
 SUBROUTINE dm2_adb_check(dac_db_link,dac_adb_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE drem_eproc = vc WITH protect, noconstant("")
 DECLARE drem_emsg = vc WITH protect, noconstant("")
 DECLARE drem_adm_db_node = vc WITH protect, noconstant("")
 DECLARE drem_adm_db_instance = vc WITH protect, noconstant("")
 DECLARE drem_adm_dbase_name = vc WITH protect, noconstant("")
 DECLARE drem_adm_adb_ind = i2 WITH protect, noconstant(0)
 DECLARE drem_tgt_db_node = vc WITH protect, noconstant("")
 DECLARE drem_tgt_db_instance = vc WITH protect, noconstant("")
 DECLARE drem_tgt_dbase_name = vc WITH protect, noconstant("")
 DECLARE drem_tgt_language = vc WITH protect, noconstant("")
 DECLARE drem_tgt_territory = vc WITH protect, noconstant("")
 DECLARE drem_tgt_characterset = vc WITH protect, noconstant("")
 DECLARE drem_tgt_nls_lang = vc WITH protect, noconstant("")
 DECLARE drem_tgt_adb_ind = i2 WITH protect, noconstant(0)
 DECLARE drem_src_db_instance = vc WITH protect, noconstant("")
 DECLARE drem_src_dbase_name = vc WITH protect, noconstant("")
 DECLARE drem_src_language = vc WITH protect, noconstant("")
 DECLARE drem_src_territory = vc WITH protect, noconstant("")
 DECLARE drem_src_characterset = vc WITH protect, noconstant("")
 DECLARE drem_src_nls_lang = vc WITH protect, noconstant("")
 DECLARE drem_src_adb_ind = i2 WITH protect, noconstant(0)
 DECLARE drem_cur_accept = vc WITH protect, noconstant("")
 DECLARE drem_src_file = vc WITH protect, noconstant("")
 DECLARE drem_tgt_file = vc WITH protect, noconstant("")
 DECLARE drem_message = vc WITH protect, noconstant("")
 DECLARE drem_max_runners = i4 WITH protect, noconstant(0)
 DECLARE drem_gen_file_dir = vc WITH protect, noconstant("")
 DECLARE drem_tgt_db_version = vc WITH protect, noconstant("")
 DECLARE drem_display_main_menu(null) = null
 DECLARE drem_manage_files(dmf_src_file=vc,dmf_tgt_file=vc) = i2
 DECLARE drem_remove_reconnect_target(null) = i2
 DECLARE drem_setup(null) = i2
 DECLARE drem_cleanup(null) = i2
 IF (check_logfile("dm2_rep_expimp_menu",".log","dm2_replicate_expimp_menu")=0)
  GO TO exit_program
 ENDIF
 IF (validate(drrr_responsefile_in_use,0)=1)
  SET dm_err->eproc = "Response file detected and will be used to complete export/import menu work."
  CALL disp_msg("",dm_err->logfile,0)
 ...
 ELSE
  ...
  SET dm2_install_schema->cdba_connect_str = dm2_install_schema->connect_str
 ENDIF
 IF (dm2_get_dbase_name(drem_adm_dbase_name)=0)
  GO TO exit_program
 ENDIF
 IF (dm2_adb_check("",drem_adm_adb_ind)=0)
  GO TO exit_program
 ENDIF
 IF (drem_adm_adb_ind=0)
  SET dm_err->eproc = "Retrieve Admin Database/Instance and Node Name"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM v$instance v
   DETAIL
    drem_adm_db_instance = v.instance_name, drem_adm_db_node = v.host_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 ELSE
  ...
  SET dm2_install_schema->src_v500_connect_str = dm2_install_schema->connect_str
 ENDIF
 IF (dm2_get_dbase_name(drem_src_dbase_name)=0)
  GO TO exit_program
 ENDIF
 SET der_expimp_data->src_db_os = dm2_sys_misc->cur_db_os
 SET dm2_install_schema->src_dbase_name = drem_src_dbase_name
 IF (dm2_adb_check("",drem_src_adb_ind)=0)
  GO TO exit_program
 ENDIF
 IF (drem_src_adb_ind=0)
  SET dm_err->eproc = "Retrieve Primary Source Database/Instance and Node Name"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM v$instance v
   DETAIL
    drem_src_db_instance = v.instance_name, der_expimp_data->src_db_node = v.host_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
 ENDIF
 SET dm_err->eproc = "Retrieve Source NLS_LANG setting."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  ...
  DETAIL
   CASE (p.parameter)
    OF "NLS_LANGUAGE":
     drem_src_language = p.value
    OF "NLS_TERRITORY":
     drem_src_territory = p.value
    OF "NLS_CHARACTERSET":
     drem_src_characterset = p.value
   ENDCASE
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET drem_src_nls_lang = concat(drem_src_language,"_",drem_src_territory,".",drem_src_characterset)
 SET dm_err->eproc = "Get source environment_name."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 ...
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET drer_email_det->src_node = der_expimp_data->src_db_node
 SET dm_err->eproc = "TARGET CONNECTION."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 ...
 IF ((dm2_sys_misc->cur_db_os="AXP"))
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Check Target database OS."
  SET dm_err->emsg = "Target database can not be VMS."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF (dm2_get_dbase_name(drem_tgt_dbase_name)=0)
  GO TO exit_program
 ENDIF
 SET dm2_install_schema->target_dbase_name = drem_tgt_dbase_name
 SET der_expimp_data->tgt_dbname = dm2_install_schema->target_dbase_name
 SET der_expimp_data->tgt_db_os = dm2_sys_misc->cur_db_os
 IF (dm2_adb_check("",drem_tgt_adb_ind)=0)
  GO TO exit_program
 ENDIF
 IF (drem_tgt_adb_ind=0)
  SET dm_err->eproc = "Retrieve Primary Target Database/Instance and Node Name"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg("",dm_err->logfile,0)
  ENDIF
  SELECT INTO "nl:"
   FROM v$instance v
   DETAIL
    drem_tgt_db_instance = v.instance_name, drem_tgt_db_node = v.host_name
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc) != 0)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   GO TO exit_program
  ENDIF
  SET der_expimp_data->tgt_db_node = drem_tgt_db_node
 ENDIF
 SET dm_err->eproc = "Retrieve Target NLS_LANG setting."
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 SELECT INTO "nl:"
  ...
  DETAIL
   CASE (p.parameter)
    OF "NLS_LANGUAGE":
     drem_tgt_language = p.value
    OF "NLS_TERRITORY":
     drem_tgt_territory = p.value
    OF "NLS_CHARACTERSET":
     drem_tgt_characterset = p.value
   ENDCASE
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 SET drem_tgt_nls_lang = concat(drem_tgt_language,"_",drem_tgt_territory,".",drem_tgt_characterset)
 IF (dm2_get_rdbms_version(null)=0)
  GO TO exit_program
 ENDIF
 SET drem_tgt_db_version = trim(cnvtstring(dm2_rdbms_version->level1))
 SET drer_email_det->tgt_env = cnvtupper(logical("environment"))
 IF (drem_tgt_adb_ind=0)
  SET drer_email_det->tgt_node = drem_tgt_db_node
 ELSE
 ...
 IF ((dm_err->err_ind=1))
  GO TO exit_program
 ENDIF
 IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","S","ALL"," ")=0)
  GO TO exit_program
 ENDIF
 SET drem_max_runners_cnt = der_expimp_data->max_runners
 SET dm2_install_schema->dbase_name = dm2_install_schema->target_dbase_name
 SET dm2_install_schema->u_name = "V500"
 SET dm2_install_schema->p_word = dm2_install_schema->v500_p_word
 SET dm2_install_schema->connect_str = dm2_install_schema->v500_connect_str
 ...
 IF (validate(drrr_responsefile_in_use,0)=1)
  ...
  tgt_db_deploy_config IN ("OCI", "ADB")))) )
   SET der_expimp_data->dp_node_type = "APP"
  ELSE
   SET der_expimp_data->dp_node_type = "DB"
  ENDIF
  SET drem_max_runners = drrr_rf_data->tgt_expimp_runners
  SET der_expimp_data->tgt_ftp_user = "oracle"
  SET der_expimp_data->src_ftp_user = "oracle"
  SET der_expimp_data->tgt_sys_pass = drrr_rf_data->tgt_sys_pwd
  SET der_expimp_data->src_sys_pass = drrr_rf_data->src_sys_pwd
  SET der_expimp_data->src_temp_dir = drrr_rf_data->src_db_temp_dir
  IF ((der_expimp_data->dp_node_type="APP"))
   SET der_expimp_data->tgt_oracle_home = drrr_rf_data->tgt_app_oracle_home
   SET der_expimp_data->tgt_temp_dir = drrr_rf_data->tgt_db_temp_dir
   SET der_expimp_data->tgt_app_temp_dir = drrr_rf_data->tgt_app_dp_temp_dir
  ELSE
   SET der_expimp_data->tgt_oracle_home = drrr_rf_data->tgt_db_oracle_home
   SET der_expimp_data->tgt_temp_dir = drrr_rf_data->tgt_db_temp_dir
  ENDIF
  IF (findstring("/",trim(der_expimp_data->tgt_temp_dir),1,1)=size(trim(der_expimp_data->tgt_temp_dir
    )))
   SET der_expimp_data->tgt_temp_dir = trim(replace(der_expimp_data->tgt_temp_dir,"/","",2))
  ENDIF
  IF (findstring("/",trim(der_expimp_data->src_temp_dir),1,1)=size(trim(der_expimp_data->src_temp_dir
    )))
   SET der_expimp_data->src_temp_dir = trim(replace(der_expimp_data->src_temp_dir,"/","",2))
  ENDIF
  IF ((drrr_rf_data->tgt_db_deploy_config="ADB"))
   IF (findstring("/",trim(der_expimp_data->tgt_temp_dir),1,0)=1)
    SET der_expimp_data->tgt_temp_dir = trim(replace(der_expimp_data->tgt_temp_dir,"/","",1))
   ENDIF
  ENDIF
  IF ((drrr_rf_data->src_db_deploy_config="ADB"))
   IF (findstring("/",trim(der_expimp_data->src_temp_dir),1,0)=1)
    SET der_expimp_data->src_temp_dir = trim(replace(der_expimp_data->src_temp_dir,"/","",1))
   ENDIF
  ENDIF
  IF (findstring("/",trim(der_expimp_data->tgt_oracle_home),1,1)=size(trim(der_expimp_data->
    tgt_oracle_home)))
   SET der_expimp_data->tgt_oracle_home = trim(replace(der_expimp_data->tgt_oracle_home,"/","",2))
  ENDIF
  IF ((der_expimp_data->dp_node_type="APP"))
   IF (findstring("/",trim(der_expimp_data->tgt_app_temp_dir),1,1)=size(trim(der_expimp_data->
     tgt_app_temp_dir)))
    SET der_expimp_data->tgt_app_temp_dir = trim(replace(der_expimp_data->tgt_app_temp_dir,"/","",2))
   ENDIF
  ENDIF
  IF ((dm_err->debug_flag > 0))
   CALL echorecord(der_expimp_data)
  ENDIF
  IF ((drrr_misc_data->process_mode="EXPORT/IMPORT SETUP"))
   SET dm_err->eproc = "Beginning export/import setup..."
   CALL disp_msg("",dm_err->logfile,0)
   IF (drem_setup(null)=0)
    GO TO exit_program
   ENDIF
  ELSEIF ((drrr_misc_data->process_mode="EXPORT/IMPORT CLEANUP"))
   SET dm_err->eproc = "Beginning export/import cleanup..."
   CALL disp_msg("",dm_err->logfile,0)
   IF (drem_cleanup(null)=0)
    GO TO exit_program
   ENDIF
  ENDIF
  IF (drem_remove_reconnect_target(null)=0)
   GO TO exit_program
  ENDIF
  GO TO exit_program
 ENDIF
 ...
 CALL clear(1,1)
 CALL box(1,1,24,132)
 CALL text(1,2,"Database Replicate/Refresh Export/Import Summary")
 CALL text(3,2,"TARGET Database Data")
 CALL text(3,60,"SOURCE Database Data")
 CALL text(4,2,"--------------------")
 CALL text(4,60,"--------------------")
 CALL text(5,2,concat("Database Name : ",trim(dm2_install_schema->target_dbase_name)))
 CALL text(5,60,concat("Database Name : ",trim(dm2_install_schema->src_dbase_name)))
 CALL text(6,2,concat("V500 Password : ",trim(dm2_install_schema->v500_p_word)))
 CALL text(6,60,concat("V500 Password : ",trim(dm2_install_schema->src_v500_p_word)))
 CALL text(7,2,concat("Connect String : ",trim(dm2_install_schema->v500_connect_str)))
 CALL text(7,60,concat("Connect String : ",trim(dm2_install_schema->src_v500_connect_str)))
 IF (drem_tgt_adb_ind=0)
  CALL text(8,2,concat("Instance Name : ",trim(drem_tgt_db_instance)))
  CALL text(9,2,concat("Host Name : ",trim(drem_tgt_db_node)))
 ENDIF
 IF (drem_src_adb_ind=0)
  CALL text(8,60,concat("Instance Name : ",trim(drem_src_db_instance)))
  CALL text(9,60,concat("Host Name : ",trim(der_expimp_data->src_db_node)))
 ENDIF
 CALL text(10,2,concat("NLS LANG : ",trim(drem_tgt_nls_lang)))
 CALL text(10,60,concat("NLS LANG : ",trim(drem_src_nls_lang)))
 CALL text(13,2,"PLEASE REVIEW THE VALUES ABOVE BEFORE CONTINUING!")
 CALL text(15,2,"Enter 'C' to continue or 'Q' to quit (C or Q) :")
 CALL accept(15,50,"p;cu"," "
  WHERE curaccept IN ("C", "Q"))
 IF (curaccept="Q")
  SET message = nowindow
  SET dm_err->eproc = "Displaying Replicate/Refresh Export/Import Summary."
 ...
 WHILE (true)
   IF ((dm_err->debug_flag != 722))
    SET message = window
   ENDIF
   CALL drem_display_main_menu(null)
   CALL text(13,10,"Your Selection (0 to Exit)?")
   CALL text(18,2,drem_message)
   IF ((der_expimp_data->setup_complete_ind=0))
    CALL accept(13,38,"X;CU"," "
     WHERE curaccept IN ("0", "1"))
   ELSEIF ((der_expimp_data->max_runners_fnd=0))
    CALL accept(13,38,"X;CU"," "
     WHERE curaccept IN ("0", "2"))
   ELSE
    CALL accept(13,38,"X;CU"," "
     WHERE curaccept IN ("0", "2", "3"))
   ENDIF
   CASE (curaccept)
    OF "0":
     GO TO exit_program
    ...
    OF "2":
     ...
     IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","S","ALL","")=0)
      GO TO exit_program
     ENDIF
     SET drem_max_runners = der_expimp_data->max_runners
     CALL drem_display_main_menu(null)
     CALL text(18,2,"Max number of parallel runners to execute: ")
     CALL accept(18,45,"99",drem_max_runners)
     SET drem_max_runners = curaccept
     IF ((((drem_max_runners != der_expimp_data->max_runners)) OR ((der_expimp_data->max_runners_fnd=
     0))) )
      IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","U","MAX_RUNNERS",cnvtstring(
        drem_max_runners))=0)
       GO TO exit_program
      ENDIF
      SET der_expimp_data->max_runners = drem_max_runners
      SET der_expimp_data->max_runners_fnd = 1
      SET drem_message = "Max number of parallel runners updated successfully. "
     ENDIF
     IF (drem_remove_reconnect_target(null)=0)
      GO TO exit_program
     ENDIF
    OF "3":
     ...
     SET drer_email_det->process = "REPLICATE/REFRESH"
     SET drer_email_det->err_ind = dm_err->err_ind
     SET drer_email_det->eproc = dm_err->eproc
     SET drer_email_det->emsg = dm_err->emsg
     SET drer_email_det->user_action = dm_err->user_action
     CALL drer_fill_email_list(drer_email_det->src_env,drer_email_det->tgt_env)
     CALL drer_reset_pre_err(null)
     IF ((dm_err->debug_flag > 5))
      CALL echorecord(drer_email_list)
     ENDIF
     IF (der_process_expimp_screen(0,0)=0)
      GO TO exit_program
     ENDIF
    OF "4":
     IF (drem_cleanup(null)=0)
 ...
 ENDWHILE
 GO TO exit_program
 SUBROUTINE drem_display_main_menu(null)
   DECLARE ddmm_manage_status = vc WITH protect, noconstant("")
   DECLARE ddmm_progress_status = vc WITH protect, noconstant("")
   DECLARE ddmm_setup_status = vc WITH protect, noconstant("")
   DECLARE ddmm_cleanup_status = vc WITH protect, noconstant("")
   IF ((der_expimp_data->setup_complete_ind=0))
    SET ddmm_setup_status = "  "
    SET ddmm_manage_status = " (unavailable)"
   ELSE
    SET ddmm_manage_status = " "
    SET ddmm_setup_status = " (complete)"
   ENDIF
   IF ((((der_expimp_data->max_runners_fnd=0)) OR ((der_expimp_data->setup_complete_ind=0))) )
    SET ddmm_progress_status = " (unavailable)"
   ELSE
    SET ddmm_progress_status = " "
   ENDIF
   IF ((der_expimp_data->setup_complete_ind=1)
    AND (der_expimp_data->cleanup_complete_ind=0))
    SET ddmm_cleanup_status = " "
   ELSEIF ((der_expimp_data->setup_complete_ind=0))
    SET ddmm_cleanup_status = " (unavailable)"
   ELSEIF ((der_expimp_data->cleanup_complete_ind=1))
    SET ddmm_cleanup_status = " (complete)"
   ENDIF
   CALL clear(1,1)
   CALL box(1,1,24,131)
   CALL text(2,2,"Replicate/Refresh Export/Import Utility")
   CALL text(4,10,concat("1. Setup",ddmm_setup_status,"(Inactive)"))
   CALL text(6,10,concat("2. Manage Max Runners Value",ddmm_manage_status))
   CALL text(8,10,concat("3. View Progress",ddmm_progress_status))
   CALL text(10,10,concat("4. Cleanup",ddmm_cleanup_status,"(Inactive)"))
 END ;Subroutine
 SUBROUTINE drem_setup(null)
   DECLARE ds_dir_found_ind = i2 WITH protect, noconstant(0)
   DECLARE ds_chk_name = vc WITH protect, noconstant("")
   DECLARE ds_file_name = vc WITH protect, noconstant("")
   DECLARE ds_cmd = vc WITH protect, noconstant("")
   DECLARE ds_text = vc WITH protect, noconstant("")
   DECLARE ds_return = i2 WITH protect, noconstant(0)
   DECLARE ds_connect_str = vc WITH protect, noconstant("")
   DECLARE ds_file = vc WITH protect, noconstant("")
   DECLARE ds_obj_name = vc WITH protect, noconstant("")
   DECLARE ds_obj_type = vc WITH protect, noconstant("")
   SET drem_message = " "
   SET drem_cur_accept = " "
   IF (validate(drrr_responsefile_in_use,0)=0)
    CALL drem_display_main_menu(null)
    IF ((der_expimp_data->setup_complete_ind=1))
     CALL text(18,2," ")
     CALL text(18,2,"Setup work has already been completed.  Enter 'C' to continue or 'Q' to quit:")
     CALL accept(18,81,"A;CU"," "
      WHERE curaccept IN ("C", "Q"))
     SET drem_cur_accept = curaccept
    ENDIF
    IF (drem_cur_accept="Q")
     RETURN(1)
    ENDIF
    IF (der_dir_prompt(null)=0)
     RETURN(0)
    ENDIF
   ENDIF
   IF (validate(drrr_responsefile_in_use,0)=1)
    IF ((der_expimp_data->dp_node_type="DB"))
     IF (cnvtupper(curnode) != cnvtupper(der_expimp_data->tgt_db_node))
      IF (curuser != "ROOT")
       ...
       SET dm_err->emsg =
       "Must be connected to Target APP node as root in order to verify SSH on remote Target DB node."
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dm_err->err_ind = 1
       RETURN(0)
      ENDIF
      SET ds_cmd = concat("ssh oracle@",der_expimp_data->tgt_db_node,
       ' echo "Target_DB_Node:\`hostname\`"')
      SET dm_err->eproc = concat("Executing: ",ds_cmd)
      CALL disp_msg(" ",dm_err->logfile,0)
      SET dm_err->disp_dcl_err_ind = 0
      SET ds_return = dm2_push_dcl(ds_cmd)
      IF (ds_return=0
      ...
      ENDIF
      IF (findstring("TARGET_DB_NODE:",cnvtupper(dm_err->errtext),1,0) > 0)
       IF (findstring(cnvtupper(der_expimp_data->tgt_db_node),cnvtupper(dm_err->errtext),findstring(
         "TARGET_DB_NODE:",cnvtupper(dm_err->errtext),1,0),0)=0)
        SET dm_err->eproc = concat("Verify SSH setup between Target APP node (",trim(cnvtupper(
           curnode)),") and Target DB node (",cnvtupper(der_expimp_data->tgt_db_node),
         ") for the oracle user.")
        SET dm_err->emsg = concat(
         "SSH command did not return the Target DB node.  On the Target app node as ",
         "the oracle user, verify that the following o/s command returns a 'Target_DB_Node' value of ",
         der_expimp_data->tgt_db_node,": ",ds_cmd)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET dm_err->err_ind = 1
        RETURN(0)
       ENDIF
      ELSE
       SET dm_err->eproc = concat("Verify SSH setup between Target APP node (",trim(cnvtupper(curnode
          )),") and Target DB node (",cnvtupper(der_expimp_data->tgt_db_node),") for the root user.")
       SET dm_err->emsg = concat(
        "SSH command did not return the Target DB node.  On the Target app node as ",
        "the root user, verify that the following o/s command returns a 'Target_DB_Node' value of ",
        der_expimp_data->tgt_db_node,": ",ds_cmd)
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dm_err->err_ind = 1
       RETURN(0)
      ENDIF
      SET dm2ftpr->user_name = der_expimp_data->tgt_ftp_user
      SET dm2ftpr->user_pwd = der_expimp_data->tgt_ftp_pass
      SET dm2ftpr->remote_host = der_expimp_data->tgt_db_node
      SET dm2ftpr->options = "-b"
      IF (dfr_test_connect(null)=0)
       RETURN(0)
      ...
      IF ((dm2ftpr->exists_ind=0))
       ...
       SET dm_err->emsg = concat("Target database node temporary directory (",der_expimp_data->
        tgt_temp_dir,") not found on Target database node (",der_expimp_data->tgt_db_node,").")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dm_err->err_ind = 1
       RETURN(0)
      ENDIF
      ...
      IF ((dm2ftpr->exists_ind=0))
       ...
       ENDIF
      ENDIF
      SET dm2ftpr->dir_name = der_expimp_data->tgt_oracle_home
      IF (dfr_find_directory(null)=0)
       RETURN(0)
      ENDIF
      IF ((dm2ftpr->exists_ind=0))
       SET dm_err->eproc = "Verify Oracle Home on Target database node."
       SET dm_err->emsg = concat("Oracle Home directory (",der_expimp_data->tgt_oracle_home,
        ") not found on Target database node (",der_expimp_data->tgt_db_node,").")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dm_err->err_ind = 1
       RETURN(0)
      ENDIF
     ELSE
      SET ds_dir_found_ind = dm2_find_dir(der_expimp_data->tgt_temp_dir)
      IF ((dm_err->err_ind > 0))
       RETURN(0)
      ENDIF
      IF (ds_dir_found_ind=0)
       SET dm_err->eproc = "Verify Target database node temporary directory."
       SET dm_err->emsg = concat("Target database node temporary directory (",der_expimp_data->
        tgt_temp_dir,") not found on Target database node (",der_expimp_data->tgt_db_node,").")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       ...
       RETURN(0)
      ENDIF
      SET ds_dir_found_ind = dm2_find_dir(concat(der_expimp_data->tgt_temp_dir,"/",cnvtlower(
         der_expimp_data->tgt_dbname),"_exp"))
      IF ((dm_err->err_ind > 0))
       RETURN(0)
      ENDIF
      ...
      IF (dm2_push_dcl(concat("chmod 777 ",concat(der_expimp_data->tgt_temp_dir,"/",cnvtlower(
       ...
       RETURN(0)
      ENDIF
      SET ds_dir_found_ind = dm2_find_dir(der_expimp_data->tgt_oracle_home)
      IF ((dm_err->err_ind > 0))
       RETURN(0)
      ENDIF
      IF (ds_dir_found_ind=0)
      ...
      ENDIF
     ENDIF
     IF (cnvtint(drem_tgt_db_version) >= 11
      AND cnvtupper(der_expimp_data->src_db_node) != cnvtupper(der_expimp_data->tgt_db_node)
      AND cnvtupper(curnode) != cnvtupper(der_expimp_data->tgt_db_node)
      AND cnvtupper(curnode) != cnvtupper(der_expimp_data->src_db_node))
      IF (curuser != "ROOT")
       SET dm_err->eproc = "Verify current user is root."
       SET dm_err->emsg =
       "Must be connected to Target APP node as root in order to verify SSH between remote Target/Source DB nodes."
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       SET dm_err->err_ind = 1
       RETURN(0)
      ENDIF
      SET ds_file_name = build(logical("ccluserdir"),"/","dm2_check_db_nodes.ksh")
      SET dm_err->eproc = concat("Remove: ",ds_file_name)
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg("",dm_err->logfile,0)
      ENDIF
      IF (dm2_findfile(ds_file_name) > 0)
       SET ds_cmd = concat("rm ",ds_file_name)
       IF (dm2_push_dcl(ds_cmd)=0)
        RETURN(0)
       ENDIF
      ELSE
       IF ((dm_err->err_ind=1))
       ...
       ENDIF
      ENDIF
      SET dm_err->eproc = concat("Writing command file to check SSH setup between Target DB node (",
       cnvtupper(der_expimp_data->tgt_db_node),") and Source DB node (",cnvtupper(der_expimp_data->
        src_db_node),").")
      IF ((dm_err->debug_flag > 0))
       CALL disp_msg("",dm_err->logfile,0)
      ENDIF
      SET ds_text = concat("ssh oracle@",der_expimp_data->tgt_db_node,' "ssh oracle@',der_expimp_data
       ->src_db_node,^ 'echo "Source_DB_Node:\`hostname\`"'"^)
      SET dm_err->eproc = concat("Creating ksh file with following command: ",ds_text)
      CALL disp_msg(" ",dm_err->logfile,0)
      SELECT INTO value(ds_file_name)
       FROM (dummyt d  WITH seq = 1)
       DETAIL
        CALL print("#!/bin/ksh"), row + 1,
        CALL print(ds_text),
      ...
      IF (check_error(dm_err->eproc)=1)
       ...
       RETURN(0)
      ENDIF
      SET ds_cmd = concat("chmod 777 ",ds_file_name)
      IF (dm2_push_dcl(ds_cmd)=0)
       RETURN(0)
      ENDIF
      SET dm_err->eproc = concat("Executing: su - oracle -c ",ds_file_name)
      CALL disp_msg(" ",dm_err->logfile,0)
      SET ds_cmd = concat("su - oracle -c ",ds_file_name)
      IF (dm2_push_dcl(ds_cmd)=0)
       RETURN(0)
      ELSE
       IF (parse_errfile(dm_err->errfile)=0)
        RETURN(0)
       ENDIF
       CALL echo(concat("string = ",dm_err->errtext))
       IF (findstring("SOURCE_DB_NODE:",cnvtupper(dm_err->errtext),1,0) > 0)
        IF (findstring(cnvtupper(der_expimp_data->src_db_node),cnvtupper(dm_err->errtext),findstring(
          "SOURCE_DB_NODE:",cnvtupper(dm_err->errtext),1,0),0)=0)
         SET dm_err->eproc = concat("Verify SSH setup between Target DB node (",cnvtupper(
           der_expimp_data->tgt_db_node),") and Source DB node (",cnvtupper(der_expimp_data->
           src_db_node),") for the oracle user.")
         SET dm_err->emsg = concat(
          "SSH command did not return the Source DB node.  On the Target app node as ",
          "the root user, verify that the following o/s command returns a 'Source_DB_Node' value of ",
          der_expimp_data->src_db_node,": su - oracle -c ",ds_file_name)
         CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
         SET dm_err->err_ind = 1
         RETURN(0)
        ENDIF
     ...
     IF (cnvtint(drem_tgt_db_version) >= 11)
      IF (cnvtupper(curnode) != cnvtupper(der_expimp_data->src_db_node))
       SET dm2ftpr->user_name = der_expimp_data->src_ftp_user
       SET dm2ftpr->user_pwd = der_expimp_data->src_ftp_pass
       SET dm2ftpr->remote_host = der_expimp_data->src_db_node
       SET dm2ftpr->options = "-b"
       IF (dfr_test_connect(null)=0)
        RETURN(0)
       ...
       ENDIF
       IF ((dm2ftpr->exists_ind=0))
        SET dm_err->eproc = "Verify Source database node temporary directory."
        SET dm_err->emsg = concat("Source database node temporary directory (",der_expimp_data->
         src_temp_dir,") not found on Source database node (",der_expimp_data->src_db_node,").")
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        SET dm_err->err_ind = 1
        RETURN(0)
       ENDIF
       ...
       ENDIF
      ELSE
       SET ds_dir_found_ind = dm2_find_dir(der_expimp_data->src_temp_dir)
       IF ((dm_err->err_ind > 0))
        RETURN(0)
       ENDIF
       IF (ds_dir_found_ind=0)
        ...
        RETURN(0)
       ENDIF
       SET ds_dir_found_ind = dm2_find_dir(concat(der_expimp_data->src_temp_dir,"/",cnvtlower(
          der_expimp_data->tgt_dbname),"_exp"))
       IF ((dm_err->err_ind > 0))
        RETURN(0)
       ENDIF
    ...
    ENDIF
    IF ((der_expimp_data->dp_node_type="APP")
     AND (validate(dm2_bypass_dp_oos,- (1))=- (1)))
     SET dm_err->eproc = "Validate DBMS_CLOUD in Source"
     CALL disp_msg(" ",dm_err->logfile,0)
     IF (der_val_dbms_cloud(1)=0)
      RETURN(0)
     ENDIF
     SET dm_err->eproc = "Validate DBMS_CLOUD in Target"
     CALL disp_msg(" ",dm_err->logfile,0)
     IF (der_val_dbms_cloud(0)=0)
      RETURN(0)
     ENDIF
     SET dm_err->eproc = "Compile DM2_MANAGE_OOS in SOURCE."
     CALL disp_msg(" ",dm_err->logfile,0)
     IF (der_compile_oos_pkg(1)=0)
      RETURN(0)
     ENDIF
     SET dm_err->eproc = "Compile DM2_MANAGE_OOS in TARGET."
     CALL disp_msg(" ",dm_err->logfile,0)
     IF (der_compile_oos_pkg(0)=0)
      RETURN(0)
     ENDIF
     SET dm_err->eproc = "Validate Object Storage Config in Source"
     CALL disp_msg(" ",dm_err->logfile,0)
     SET dm2_install_schema->dbase_name = dm2_install_schema->src_dbase_name
     SET dm2_install_schema->u_name = "V500"
     SET dm2_install_schema->p_word = dm2_install_schema->src_v500_p_word
     ...
     IF ((dm_err->err_ind=1))
      GO TO exit_program
     ENDIF
     EXECUTE dm2_validate_oos drrr_rf_data->dp_cred_name, drrr_rf_data->dp_user, drrr_rf_data->
     dp_user_pwd,
     drrr_rf_data->dp_oos_dir_path
     IF ((dm_err->err_ind=1))
      RETURN(0)
     ENDIF
     SET dm_err->eproc = "Validate Object Storage Config in Target"
     CALL disp_msg(" ",dm_err->logfile,0)
     SET dm2_install_schema->dbase_name = dm2_install_schema->target_dbase_name
     SET dm2_install_schema->u_name = "V500"
     SET dm2_install_schema->p_word = dm2_install_schema->v500_p_word
   ...
   IF ((dm_err->err_ind=1))
    RETURN(0)
   ENDIF
   IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","D","SETUP_COMPLETE_IND","")=0)
    RETURN(0)
   ENDIF
   IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","D","CLEANUP_COMPLETE_IND","")=0)
    RETURN(0)
   ENDIF
   SET der_expimp_data->setup_complete_ind = 0
   SET der_expimp_data->cleanup_complete_ind = 0
   IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","U","ORACLE_HOME",der_expimp_data
    ->tgt_oracle_home)=0)
    RETURN(0)
   ENDIF
   IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","U","TEMP_DIRECTORY",
    der_expimp_data->tgt_temp_dir)=0)
    RETURN(0)
   ENDIF
   IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","U","SRC_TEMP_DIRECTORY",
    der_expimp_data->src_temp_dir)=0)
    RETURN(0)
   ENDIF
   IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","U","TGT_SYS_PASS",der_expimp_data
    ->tgt_sys_pass)=0)
    RETURN(0)
   ENDIF
   IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","U","DP_NODE_TYPE",der_expimp_data
    ->dp_node_type)=0)
    RETURN(0)
   ENDIF
   IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","U","TGT_APP_TEMP_DIRECTORY",
    der_expimp_data->tgt_app_temp_dir)=0)
    RETURN(0)
   ENDIF
   IF ((der_expimp_data->dp_node_type="DB"))
    IF (cnvtupper(curnode)=cnvtupper(drem_tgt_db_node)
     AND (dm2_sys_misc->cur_os != "AXP"))
     SET drem_gen_file_dir = concat(der_expimp_data->tgt_temp_dir,"/")
    ELSE
     SET drem_gen_file_dir = dm2_install_schema->ccluserdir
    ENDIF
   ELSEIF ((der_expimp_data->dp_node_type="APP"))
    IF (cnvtupper(curnode) != cnvtupper(drrr_rf_data->tgt_app_dp_node))
     SET dm_err->err_ind = 1
     SET dm_err->emsg = concat("Setup must run from the App node ",der_expimp_data->tgt_app_temp_dir,
      ".  The current node is ",curnode)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
    SET drem_gen_file_dir = concat(der_expimp_data->tgt_app_temp_dir,"/")
   ENDIF
   SET drem_src_file = build(dm2_install_schema->cer_install,"dm2_expimp_setup.txt")
   SET drem_tgt_file = build(drem_gen_file_dir,"dm2_",cnvtlower(dm2_install_schema->target_dbase_name
     ),"_expimp_setup.ksh")
   IF (drem_manage_files(drem_src_file,drem_tgt_file)=0)
    RETURN(0)
   ENDIF
   SET drem_src_file = build(dm2_install_schema->cer_install,"dm2_expimp_main.txt")
   SET drem_tgt_file = build(drem_gen_file_dir,"dm2_",cnvtlower(dm2_install_schema->target_dbase_name
     ),"_expimp_main.ksh")
   IF (drem_manage_files(drem_src_file,drem_tgt_file)=0)
    RETURN(0)
   ENDIF
   IF (cnvtint(drem_tgt_db_version) >= 11)
    SET drem_src_file = build(dm2_install_schema->cer_install,"dm2_datapump_runner.txt")
   ELSE
    SET drem_src_file = build(dm2_install_schema->cer_install,"dm2_expimp_runner.txt")
   ENDIF
   SET drem_tgt_file = build(drem_gen_file_dir,"dm2_",cnvtlower(dm2_install_schema->target_dbase_name
     ),"_expimp_runner.ksh")
   IF (drem_manage_files(drem_src_file,drem_tgt_file)=0)
    RETURN(0)
   ENDIF
   SET drem_src_file = build(dm2_install_schema->cer_install,"dm2_expimp_cleanup.txt")
   SET drem_tgt_file = build(drem_gen_file_dir,"dm2_",cnvtlower(dm2_install_schema->target_dbase_name
     ),"_expimp_cleanup.ksh")
   IF (drem_manage_files(drem_src_file,drem_tgt_file)=0)
    RETURN(0)
   ENDIF
   SET drem_src_file = build(dm2_install_schema->cer_install,"dm2_expimp_cron_remove.txt")
   SET drem_tgt_file = build(drem_gen_file_dir,"dm2_",cnvtlower(dm2_install_schema->target_dbase_name
     ),"_expimp_cron_remove.ksh")
   IF (drem_manage_files(drem_src_file,drem_tgt_file)=0)
    RETURN(0)
   ENDIF
   SET drem_src_file = build(dm2_install_schema->cer_install,"dm2_expimp_cron_add.txt")
   SET drem_tgt_file = build(drem_gen_file_dir,"dm2_",cnvtlower(dm2_install_schema->target_dbase_name
     ),"_expimp_cron_add.ksh")
   IF (drem_manage_files(drem_src_file,drem_tgt_file)=0)
    RETURN(0)
   ENDIF
   IF (validate(drrr_responsefile_in_use,0)=0)
   ...
   IF ((der_expimp_data->dp_node_type="DB"))
    IF (cnvtupper(curnode) != cnvtupper(drem_tgt_db_node)
     ...
     SET dm2ftpr->user_name = der_expimp_data->tgt_ftp_user
     SET dm2ftpr->user_pwd = der_expimp_data->tgt_ftp_pass
     SET dm2ftpr->remote_host = drem_tgt_db_node
     SET dm2ftpr->options = "-b"
     CALL dfr_add_putops_line(" "," "," "," "," ",
      1)
     CALL dfr_add_putops_line(" ",build(trim(logical("ccluserdir")),"/"),concat("dm2_",cnvtlower(
        dm2_install_schema->target_dbase_name),"_expimp_setup.ksh"),concat(der_expimp_data->
       tgt_temp_dir,"/"),concat("dm2_",cnvtlower(dm2_install_schema->target_dbase_name),
       "_expimp_setup.ksh"),
      0)
     CALL dfr_add_putops_line(" ",build(trim(logical("ccluserdir")),"/"),concat("dm2_",cnvtlower(
        dm2_install_schema->target_dbase_name),"_expimp_main.ksh"),concat(der_expimp_data->
       tgt_temp_dir,"/"),concat("dm2_",cnvtlower(dm2_install_schema->target_dbase_name),
       "_expimp_main.ksh"),
      0)
     CALL dfr_add_putops_line(" ",build(trim(logical("ccluserdir")),"/"),concat("dm2_",cnvtlower(
        dm2_install_schema->target_dbase_name),"_expimp_runner.ksh"),concat(der_expimp_data->
       tgt_temp_dir,"/"),concat("dm2_",cnvtlower(dm2_install_schema->target_dbase_name),
       "_expimp_runner.ksh"),
      0)
     CALL dfr_add_putops_line(" ",build(trim(logical("ccluserdir")),"/"),concat("dm2_",cnvtlower(
        dm2_install_schema->target_dbase_name),"_expimp_cleanup.ksh"),concat(der_expimp_data->
       tgt_temp_dir,"/"),concat("dm2_",cnvtlower(dm2_install_schema->target_dbase_name),
       "_expimp_cleanup.ksh"),
      0)
     CALL dfr_add_putops_line(" ",build(trim(logical("ccluserdir")),"/"),concat("dm2_",cnvtlower(
        dm2_install_schema->target_dbase_name),"_expimp_cron_remove.ksh"),concat(der_expimp_data->
       tgt_temp_dir,"/"),concat("dm2_",cnvtlower(dm2_install_schema->target_dbase_name),
       "_expimp_cron_remove.ksh"),
      0)
     CALL dfr_add_putops_line(" ",build(trim(logical("ccluserdir")),"/"),concat("dm2_",cnvtlower(
        dm2_install_schema->target_dbase_name),"_expimp_cron_add.ksh"),concat(der_expimp_data->
       tgt_temp_dir,"/"),concat("dm2_",cnvtlower(dm2_install_schema->target_dbase_name),
       "_expimp_cron_add.ksh"),
      0)
     IF (dfr_put_file(null)=0)
     ...
     CALL dfr_add_putops_line(" "," "," "," "," ",
      1)
     IF (validate(drrr_responsefile_in_use,0)=0)
      CALL text(2,2,"Replicate/Refresh Export/Import Utility: Setup")
      CALL text(4,2,concat("1) On Node ",drem_tgt_db_node,", at the o/s command line, "))
      CALL text(5,2,"   execute the following as the oracle user:")
      CALL text(7,5,concat(der_expimp_data->tgt_temp_dir,"/dm2_",cnvtlower(dm2_install_schema->
         target_dbase_name),"_expimp_setup.ksh"))
      CALL text(17,2,"Upon successful completion, please enter 'C' to Continue or 'Q' to Quit:")
      CALL accept(17,75,"A;CU"," "
       WHERE curaccept IN ("C", "Q"))
     ENDIF
     IF (validate(drrr_responsefile_in_use,0)=1)
      SET dm_err->eproc = concat("Executing Export/Import Utility Setup script (",concat(
        der_expimp_data->tgt_temp_dir,"/dm2_",cnvtlower(dm2_install_schema->target_dbase_name),
        "_expimp_setup.ksh"),") on remote node ",drem_tgt_db_node," as the oracle user...")
      CALL disp_msg(" ",dm_err->logfile,0)
      CALL dor_init_flex_cmds(null)
      CALL dor_add_flex_cmd(0,"oracle",drem_tgt_db_node," ",concat(der_expimp_data->tgt_temp_dir,
        "/dm2_",cnvtlower(dm2_install_schema->target_dbase_name),"_expimp_setup.ksh"),
       " ","EC")
    ...
    ELSE
     IF ((dm2_sys_misc->cur_os="AXP"))
      CALL text(2,2,"Replicate/Refresh Export/Import Utility: Setup")
      CALL text(4,2,"1) Copy the following files in CCLUSERDIR from this node to directory")
      CALL text(5,2,concat("   ",der_expimp_data->tgt_temp_dir," on node ",drem_tgt_db_node,":"))
      CALL text(7,5,
       "Note: Make sure to use ascii mode for FTP and that the file names are in lowercase.")
      CALL text(9,5,build("dm2_",cnvtlower(dm2_install_schema->target_dbase_name),"_expimp*.*"))
      CALL text(11,2,concat("2) On Node ",drem_tgt_db_node,", at the o/s command line, "))
      CALL text(12,2,"   execute the following as the root user:")
      CALL text(14,5,concat("chmod 777 ",der_expimp_data->tgt_temp_dir,"/dm2_",cnvtlower(
         dm2_install_schema->target_dbase_name),"_expimp*.*"))
      CALL text(15,5,concat(der_expimp_data->tgt_temp_dir,"/dm2_",cnvtlower(dm2_install_schema->
         target_dbase_name),"_expimp_setup.ksh"))
      CALL text(17,2,"Upon successful completion, please enter 'C' to Continue or 'Q' to Quit:")
      CALL accept(17,75,"A;CU"," "
       WHERE curaccept IN ("C", "Q"))
     ELSE
      IF (validate(drrr_responsefile_in_use,0)=0)
       CALL text(2,2,"Replicate/Refresh Export/Import Utility: Setup")
       CALL text(5,2,concat("Executing setup steps on node ",cnvtupper(drem_tgt_db_node),"...."))
      ENDIF
      SET dm_err->eproc = "Performing setup work."
      IF (dm2_push_dcl(concat("chmod 777 ",der_expimp_data->tgt_temp_dir,"/dm2_",cnvtlower(
         dm2_install_schema->target_dbase_name),"_expimp\*.\*"))=0)
       RETURN(0)
      ENDIF
      IF (dm2_push_dcl(concat(der_expimp_data->tgt_temp_dir,"/dm2_",cnvtlower(dm2_install_schema->
         target_dbase_name),"_expimp_setup.ksh"))=0)
       RETURN(0)
      ENDIF
     ENDIF
    ENDIF
   ELSEIF ((der_expimp_data->dp_node_type="APP"))
    SET dm_err->eproc = "Performing setup work."
    IF (dm2_push_dcl(concat("chmod 777 ",drem_gen_file_dir,"dm2_",cnvtlower(dm2_install_schema->
       target_dbase_name),"_expimp\*.\*"))=0)
     RETURN(0)
    ENDIF
    IF (dm2_push_dcl(concat(drem_gen_file_dir,"dm2_",cnvtlower(dm2_install_schema->target_dbase_name),
      "_expimp_setup.ksh"))=0)
     SET dm_err->emsg = concat("View errors in log ",drem_gen_file_dir,"dm2_",cnvtlower(
       dm2_install_schema->target_dbase_name),"_expimp_setup_ksh.log")
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     RETURN(0)
    ENDIF
   ENDIF
   ...
   IF (drem_cur_accept="Q")
    RETURN(1)
   ENDIF
   IF (((drem_cur_accept="C") OR (validate(drrr_responsefile_in_use,0)=1)) )
    IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","S","ALL","")=0)
     RETURN(0)
    ENDIF
    IF ((der_expimp_data->setup_complete_ind=1))
     SET drem_message = "Setup work completed. "
    ELSE
     SET dm_err->eproc = "Performing setup work."
     IF ((der_expimp_data->dp_node_type="DB"))
      IF (cnvtupper(curnode) != cnvtupper(drem_tgt_db_node))
       SET dm_err->emsg = concat("Error occurred during execution of dm2_",cnvtlower(
         dm2_install_schema->target_dbase_name),"_expimp_setup.ksh on remote Target database node ",
        cnvtupper(drem_tgt_db_node),".")
      ELSE
       SET dm_err->emsg = concat("Error occurred during execution of dm2_",cnvtlower(
         dm2_install_schema->target_dbase_name),"_expimp_setup.ksh on local Target database node ",
        cnvtupper(drem_tgt_db_node),".")
      ENDIF
     ELSEIF ((der_expimp_data->dp_node_type="APP"))
      SET dm_err->emsg = concat("Error occurred during execution of dm2_",cnvtlower(
        dm2_install_schema->target_dbase_name),"_expimp_setup.ksh on local Target App node ",
       cnvtupper(curnode),".")
     ENDIF
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
     SET dm_err->err_ind = 1
   ...
   ENDIF
   IF (validate(drrr_responsefile_in_use,0)=1)
    IF ((((drem_max_runners != der_expimp_data->max_runners)) OR ((der_expimp_data->max_runners_fnd=0
    ))) )
     IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","U","MAX_RUNNERS",cnvtstring(
       drem_max_runners))=0)
      RETURN(0)
     ...
     SET der_expimp_data->max_runners_fnd = 1
     SET drem_message =
     "Setup work completed and max number of parallel runners updated successfully. "
    ENDIF
   ENDIF
   IF (drem_remove_reconnect_target(null)=0)
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drem_cleanup(null)
   SET drem_message = " "
   SET drem_cur_accept = " "
   IF (validate(drrr_responsefile_in_use,0)=0)
    CALL drem_display_main_menu(null)
    IF ((der_expimp_data->cleanup_complete_ind=1))
     CALL text(18,2," ")
     CALL text(18,2,"Cleanup work has already been completed.  Enter 'C' to continue or 'Q' to quit:"
      )
     CALL accept(18,81,"A;CU"," "
      WHERE curaccept IN ("C", "Q"))
     SET drem_cur_accept = curaccept
   ...
   IF (cnvtupper(curnode) != cnvtupper(drem_tgt_db_node)
    ...
    IF (validate(drrr_responsefile_in_use,0)=0)
     CALL clear(1,1)
     CALL box(1,1,24,131)
     CALL text(2,2,"Replicate/Refresh Export/Import Utility: Cleanup")
     CALL text(4,2,concat("On the Node ",drem_tgt_db_node,", at the o/s command line,"))
     CALL text(5,2,"execute the following as the oracle user:")
     CALL text(7,5,concat(der_expimp_data->tgt_temp_dir,"/dm2_",cnvtlower(dm2_install_schema->
        target_dbase_name),"_expimp_cleanup.ksh"))
     CALL text(15,2,"Upon successful completion, please enter 'C' to Continue or 'Q' to Quit:")
     CALL accept(15,75,"A;CU"," "
      WHERE curaccept IN ("C", "Q"))
     SET drem_cur_accept = curaccept
    ENDIF
    IF (validate(drrr_responsefile_in_use,0)=1)
     IF ((der_expimp_data->dp_node_type="DB"))
      SET dm_err->eproc = concat("Executing Export/Import Utility Cleanup script (",concat(
        der_expimp_data->tgt_temp_dir,"/dm2_",cnvtlower(dm2_install_schema->target_dbase_name),
        "_expimp_cleanup.ksh"),") on remote node ",drem_tgt_db_node," as the oracle user...")
      CALL disp_msg(" ",dm_err->logfile,0)
      CALL dor_init_flex_cmds(null)
      CALL dor_add_flex_cmd(0,"oracle",drem_tgt_db_node," ",concat(der_expimp_data->tgt_temp_dir,
        "/dm2_",cnvtlower(dm2_install_schema->target_dbase_name),"_expimp_cleanup.ksh"),
       " ","EC")
      IF (dor_exec_flex_cmd(null)=0)
       RETURN(0)
      ENDIF
     ELSEIF ((der_expimp_data->dp_node_type="APP"))
      IF (dm2_push_dcl(concat(der_expimp_data->tgt_app_temp_dir,"/dm2_",cnvtlower(dm2_install_schema
         ->target_dbase_name),"_expimp_cleanup.ksh"))=0)
       SET dm_err->emsg = concat("View errors in log ",der_expimp_data->tgt_app_temp_dir,"/dm2_",
        cnvtlower(dm2_install_schema->target_dbase_name),"_expimp_cleanup_ksh.log")
       CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
       RETURN(0)
      ENDIF
     ENDIF
     IF (der_drop_dirs(null)=0)
      RETURN(0)
     ENDIF
     IF ((der_expimp_data->dp_node_type="APP")
      AND (validate(dm2_bypass_dp_oos,- (1))=- (1)))
      SET dm_err->eproc = concat("Drop Datapump Credential ",drrr_rf_data->dp_cred_name,
       " from the Target.")
      CALL disp_msg("",dm_err->logfile,0)
      EXECUTE dm2_manage_database_credential "DROP", drrr_rf_data->dp_cred_name, drrr_rf_data->
      dp_user,
      ""
      IF ((dm_err->err_ind=1))
       RETURN(0)
   ...
   ELSE
    IF ((dm2_sys_misc->cur_os="AXP"))
     ...
     CALL text(2,2,"Replicate/Refresh Export/Import Utility: Cleanup")
     CALL text(4,2,concat("On the Node ",drem_tgt_db_node,", at the o/s command line,"))
     CALL text(5,2,"execute the following as the root user:")
     CALL text(7,5,concat(der_expimp_data->tgt_temp_dir,"/dm2_",cnvtlower(dm2_install_schema->
        target_dbase_name),"_expimp_cleanup.ksh"))
     CALL text(15,2,"Upon successful completion, please enter 'C' to Continue or 'Q' to Quit:")
     CALL accept(15,75,"A;CU"," "
    ...
    ELSE
     IF (validate(drrr_responsefile_in_use,0)=0)
      ...
      CALL text(4,2,concat(
        "Are you sure you want to perform cleanup work (includes removing log files)?  ",
        "Enter 'C' to confirm or 'Q' to quit:"))
      CALL accept(4,116,"A;CU"," "
       WHERE curaccept IN ("C", "Q"))
      SET drem_cur_accept = curaccept
      IF (drem_cur_accept="Q")
       RETURN(1)
      ENDIF
      CALL text(6,2,concat("Executing cleanup steps on node ",cnvtupper(drem_tgt_db_node),"...."))
     ENDIF
    ENDIF
   ENDIF
   IF (drem_cur_accept="Q")
   ...
   ENDIF
   IF ((der_expimp_data->cleanup_complete_ind=1))
    SET drem_message = "Cleanup work completed."
    IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","D","ALL","")=0)
     RETURN(0)
    ENDIF
    IF (der_manage_admin_data(der_expimp_data->tgt_dbname,"DM_INFO","S","ALL","")=0)
     RETURN(0)
    ENDIF
   ELSE
    SET dm_err->eproc = "Performing cleanup work."
    IF ((der_expimp_data->dp_node_type="DB"))
     IF (cnvtupper(curnode) != cnvtupper(drem_tgt_db_node))
      SET dm_err->emsg = concat("Error occurred during execution of dm2_",cnvtlower(
        dm2_install_schema->target_dbase_name),"_expimp_cleanup.ksh on remote Target database node ",
       cnvtupper(drem_tgt_db_node),".")
     ELSE
      SET dm_err->emsg = concat("Error occurred during execution of dm2_",cnvtlower(
        dm2_install_schema->target_dbase_name),"_expimp_cleanup.ksh on local Target database node ",
       cnvtupper(drem_tgt_db_node),".")
     ENDIF
    ELSEIF ((der_expimp_data->dp_node_type="APP"))
     SET dm_err->emsg = concat("Error occurred during execution of dm2_",cnvtlower(dm2_install_schema
       ->target_dbase_name),"_expimp_cleanup.ksh on local Target app node ",cnvtupper(curnode),".")
    ENDIF
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET dm_err->err_ind = 1
    RETURN(0)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drem_remove_reconnect_target(null)
   ...
   RETURN(1)
 END ;Subroutine
 SUBROUTINE drem_manage_files(dmf_src_file,dmf_tgt_file)
   DECLARE dmf_temp_line = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Verify file needed exists."
   CALL disp_msg("",dm_err->logfile,0)
   IF (dm2_findfile(dmf_src_file)=0)
    SET dm_err->err_ind = 1
    SET dm_err->emsg = concat(dmf_src_file," is not found.")
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    RETURN(0)
   ENDIF
   FREE DEFINE rtl2
   ...
   SELECT INTO value(dmf_tgt_file)
    FROM rtl2t r
    DETAIL
     dmf_temp_line = r.line, dmf_temp_line = replace(dmf_temp_line,"<admin_dbname>",cnvtlower(
       drem_adm_dbase_name)), dmf_temp_line = replace(dmf_temp_line,"<admin_database_instance_name>",
      drem_adm_db_instance),
     dmf_temp_line = replace(dmf_temp_line,"<admin_database_host_name>",drem_adm_db_node),
     dmf_temp_line = replace(dmf_temp_line,"<admin_connect>",dm2_install_schema->cdba_connect_str),
     dmf_temp_line = replace(dmf_temp_line,"<admin_cdba_pwd>",dm2_install_schema->cdba_p_word),
     dmf_temp_line = replace(dmf_temp_line,"<src_dbname>",cnvtlower(dm2_install_schema->
       src_dbase_name)), dmf_temp_line = replace(dmf_temp_line,"<src_database_instance_name>",
      drem_src_db_instance), dmf_temp_line = replace(dmf_temp_line,"<src_database_host_name>",
      der_expimp_data->src_db_node),
     dmf_temp_line = replace(dmf_temp_line,"<src_connect>",dm2_install_schema->src_v500_connect_str),
     dmf_temp_line = replace(dmf_temp_line,"<src_v500_pwd>",dm2_install_schema->src_v500_p_word),
     dmf_temp_line = replace(dmf_temp_line,"<tgt_dbname>",cnvtlower(dm2_install_schema->
       target_dbase_name)),
     dmf_temp_line = replace(dmf_temp_line,"<tgt_database_instance_name>",drem_tgt_db_instance),
     dmf_temp_line = replace(dmf_temp_line,"<tgt_database_host_name>",drem_tgt_db_node),
     dmf_temp_line = replace(dmf_temp_line,"<tgt_connect>",dm2_install_schema->v500_connect_str),
     dmf_temp_line = replace(dmf_temp_line,"<tgt_v500_pwd>",dm2_install_schema->v500_p_word),
     dmf_temp_line = replace(dmf_temp_line,"<oracle_home>",der_expimp_data->tgt_oracle_home),
     dmf_temp_line = replace(dmf_temp_line,"<nls_lang>","AMERICAN_AMERICA.WE8MSWIN1252")
     IF (cnvtint(drem_tgt_db_version) > 10)
      IF ((der_expimp_data->dp_node_type="DB"))
       dmf_temp_line = replace(dmf_temp_line,"<expimp_oradir_path>",concat(der_expimp_data->
         tgt_temp_dir,"/",cnvtlower(der_expimp_data->tgt_dbname),"_exp"))
      ELSEIF ((der_expimp_data->dp_node_type="APP"))
       dmf_temp_line = replace(dmf_temp_line,"<expimp_oradir_path>",concat(der_expimp_data->
         tgt_app_temp_dir,"/",cnvtlower(der_expimp_data->tgt_dbname),"_exp"))
      ENDIF
      dmf_temp_line = replace(dmf_temp_line,"<src_expimp_oradir_path>",concat(der_expimp_data->
        src_temp_dir,"/",cnvtlower(der_expimp_data->tgt_dbname),"_exp"))
     ENDIF
     IF ((der_expimp_data->dp_node_type="DB"))
      dmf_temp_line = replace(dmf_temp_line,"<expimp_path>",der_expimp_data->tgt_temp_dir)
     ELSEIF ((der_expimp_data->dp_node_type="APP"))
      dmf_temp_line = replace(dmf_temp_line,"<expimp_path>",der_expimp_data->tgt_app_temp_dir)
     ENDIF
     dmf_temp_line = replace(dmf_temp_line,"<src_expimp_path>",der_expimp_data->src_temp_dir),
     dmf_temp_line = replace(dmf_temp_line,"<tgt_database_version>",drem_tgt_db_version),
     dmf_temp_line = replace(dmf_temp_line,"<src_ftp_user>",der_expimp_data->src_ftp_user),
     dmf_temp_line = replace(dmf_temp_line,"<dp_node_type>",der_expimp_data->dp_node_type),
     dmf_temp_line = replace(dmf_temp_line,"<admin_database_type>",drrr_rf_data->adm_db_deploy_config
      ), dmf_temp_line = replace(dmf_temp_line,"<tgt_database_type>",drrr_rf_data->
      tgt_db_deploy_config),
     dmf_temp_line = replace(dmf_temp_line,"<src_database_type>",drrr_rf_data->src_db_deploy_config),
     dmf_temp_line = replace(dmf_temp_line,"<oos_dir_path>",drrr_rf_data->dp_oos_dir_path),
     dmf_temp_line = replace(dmf_temp_line,"<dp_cred_name>",drrr_rf_data->dp_cred_name)
     IF ((drrr_rf_data->tgt_db_deploy_config="ADB"))
      dmf_temp_line = replace(dmf_temp_line,"<tgt_admin_pwd>",drrr_rf_data->tgt_sys_pwd)
     ENDIF
...
#exit_program
 ...
 ENDIF
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "dm2_replicate_expimp_menu has completed"
 ELSE
  SET drem_eproc = dm_err->eproc
  SET drem_emsg = dm_err->emsg
  CALL drem_remove_reconnect_target(null)
  SET dm_err->eproc = drem_eproc
  SET dm_err->emsg = drem_emsg
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 IF ((dm_err->debug_flag > 2))
  CALL echorecord(dm_err)
  ...
  CALL echorecord(der_expimp_data)
 ENDIF
 CALL final_disp_msg("dm2_rep_expimp_menu")
END GO
