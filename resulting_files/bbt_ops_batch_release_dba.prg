CREATE PROGRAM bbt_ops_batch_release:dba
 ...
 DECLARE sendproductevents(null) = null
 ...
 RECORD outbound_list(
   1 product_events[*]
     2 product_event_id = f8
 )
 DECLARE outbound_event_cnt = i4 WITH noconstant(0)
 ...
 inprogress_event_type_cd=0.0) OR (((autologous_event_type_cd=0.0) OR (((directed_event_type_cd=0.0)
  OR (((shipped_event_type_cd=0.0) OR (intransit_event_type_cd=0.0)) )) )) )) )) )) )) )) )) )) )
  SET reply->status_data.status = "F"
 ...
 SET product_count = 0
 SELECT INTO "nl:"
  xm.product_event_id, pe.product_event_id, pe.person_id,
  p.product_id, p.locked_ind, b.product_id
  FROM product p,
   blood_product b,
   product_event pe,
   crossmatch xm
  ...
  HEAD xm.product_id
   ...
   ENDIF
   IF (valid_prod_ind="T")
    count1 += 1, count2 = 0, product_count += 1,
    stat = alter(ops_request->productlist,count1), ops_request->productlist[count1].product_type =
    "B", ops_request->productlist[count1].supp_prefix = b.supplier_prefix,
  ...
  HEAD xm.product_event_id
   IF (valid_prod_ind="T")
    count2 += 1, stat = alterlist(ops_request->productlist[count1].productevent,count2), ops_request
    ->productlist[count1].productevent[count2].product_event_id = pe.product_event_id,
    ops_request->productlist[count1].productevent[count2].event_type_cd = pe.event_type_cd,
    ops_request->productlist[count1].productevent[count2].pe_updt_cnt = pe.updt_cnt, ops_request->
    productlist[count1].productevent[count2].order_id = pe.order_id,
    ops_request->productlist[count1].productevent[count2].person_id = pe.person_id, ops_request->
    productlist[count1].productevent[count2].release_reason_cd = xm_expired_reason_cd
   ENDIF
  WITH nocounter
 ;end select
 IF (ops_param_status=1)
  SELECT INTO "nl:"
   pe.product_event_id, pn.person_id, en.encntr_id,
   p.product_id, rel_reason = decode(pn.seq,"Patient Expired",en.seq,"Encounter Discharged","Y"),
   prod_type = decode(b.seq,"b",b1.seq,"b",d.seq,
    "d",de.seq,"d","x")
   FROM product_event pe,
    ...
    dummyt d4
   PLAN (pe
    WHERE pe.event_type_cd IN (xmtch_event_type_cd, assign_event_type_cd, inprogress_event_type_cd)
     AND pe.active_ind=1)
    JOIN (p
    WHERE pe.product_id=p.product_id
     AND p.active_ind=1
    ...
    WHERE pe.encntr_id=en.encntr_id
     ...
     AND en.disch_dt_tm <= cnvtdatetime(sysdate))
    JOIN (d4
    WHERE d4.seq=1)
    JOIN (((b1
   ...
   HEAD pe.product_id
    ...
    ENDIF
    IF (valid_prod_ind="T")
     pos = 0, pos = locateval(index,1,size(ops_request->productlist,5),p.product_id,ops_request->
     ...
     IF (pos=0)
      ...
      ops_request->productlist[count1].p_updt_cnt = p.updt_cnt, ops_request->productlist[count1].
      der_updt_cnt =
      IF (prod_type="d")
      ...
      ENDIF
      IF (prod_type="b")
       ops_request->productlist[count1].supp_prefix =
   ...
   HEAD pe.product_event_id
    IF (valid_prod_ind="T")
     IF (pos=0)
      count2 += 1, stat = alterlist(ops_request->productlist[count1].productevent,count2),
      ops_request->productlist[count1].productevent[count2].product_event_id = pe.product_event_id,
      ops_request->productlist[count1].productevent[count2].event_type_cd = pe.event_type_cd,
      ops_request->productlist[count1].productevent[count2].pe_updt_cnt = pe.updt_cnt, ops_request->
      productlist[count1].productevent[count2].order_id = pe.order_id,
      ops_request->productlist[count1].productevent[count2].person_id = pe.person_id, ops_request->
      productlist[count1].productevent[count2].release_reason_cd =
      ...
      ENDIF
     ELSE
      pos1 = 0, pos1 = locateval(index,1,size(ops_request->productlist[pos].productevent,5),pe
       .product_event_id,ops_request->productlist[pos].productevent[index].product_event_id)
      IF (pos1=0)
       count2 = (size(ops_request->productlist[pos].productevent,5)+ 1), stat = alterlist(ops_request
        ->productlist[pos].productevent,count2), ops_request->productlist[pos].productevent[count2].
       product_event_id = pe.product_event_id,
       ops_request->productlist[pos].productevent[count2].event_type_cd = pe.event_type_cd,
       ops_request->productlist[pos].productevent[count2].pe_updt_cnt = pe.updt_cnt, ops_request->
       productlist[pos].productevent[count2].order_id = pe.order_id,
       ops_request->productlist[pos].productevent[count2].person_id = pe.person_id, ops_request->
       productlist[pos].productevent[count2].release_reason_cd =
       IF (rel_reason="Patient Expired") pat_expired_reason_cd
       ELSE enc_discharged_reason_cd
       ENDIF
 ...
 FOR (i = 1 TO size(ops_request->productlist,5))
   SELECT INTO "nl:"
    pe.product_event_id, event = decode(a.seq,"ASSIGN",xm.seq,"XM","yy"), is_dispensed =
    ...
    FROM product_event pe,
     ...
     dummyt d1
    PLAN (pe
     WHERE expand(index,1,size(ops_request->productlist[i].productevent,5),pe.product_event_id,
    ...
    HEAD pe.product_event_id
     ...
     ELSEIF (event="XM")
      ops_request->productlist[i].productevent[pos].updt_cnt = xm.updt_cnt, ops_request->productlist[
      i].productevent[pos].xm_exp_dt_tm = cnvtdatetime(xm.crossmatch_exp_dt_tm)
     ENDIF
     IF ((ops_request->productlist[i].product_type="D"))
      IF (event="ASSIGN")
       ops_request->productlist[i].productevent[pos].release_qty = a.cur_assign_qty, ops_request->
       productlist[i].productevent[pos].release_iu = a.cur_assign_intl_units
      ENDIF
     ENDIF
     IF ((ops_request->productlist[i].product_type="B"))
      IF (is_dispensed="pd")
       ops_request->productlist[i].status = "F", ops_request->productlist[i].err_message =
       "Product issued"
      ELSE
 ...
 IF (cnvtupper(batch_field)="UPDATE")
  ...
  FOR (prod = 1 TO nbr_to_update)
     ...
     SELECT INTO "nl:"
      ...
      DETAIL
       ...
       ELSEIF (pe.event_type_cd=xmtch_event_type_cd)
        total_xm_events += 1, active_xm = "T", pos = locateval(index,1,size(ops_request->productlist[
          prod].productevent,5),pe.product_event_id,ops_request->productlist[prod].productevent[index
         ].product_event_id)
        IF (pos > 0)
         qual_xm_events += 1
        ENDIF
    ...
    FOR (count1 = 1 TO nbr_of_events)
      ...
      IF ((ops_request->productlist[prod].status="S"))
        ...
        ELSE
         UPDATE  FROM crossmatch xm
          SET xm.release_reason_cd = ops_request->productlist[prod].productevent[count1].
           release_reason_cd, xm.release_dt_tm = cnvtdatetime(sysdate), xm.release_prsnl_id = reqinfo
           ->updt_id,
           xm.release_qty = 0, xm.crossmatch_qty = 0, xm.updt_cnt = (xm.updt_cnt+ 1),
           xm.updt_dt_tm = cnvtdatetime(sysdate), xm.updt_task = reqinfo->updt_task, xm.updt_id =
           reqinfo->updt_id,
           xm.updt_applctx = reqinfo->updt_applctx, xm.active_ind = 0, xm.active_status_cd = reqdata
           ->inactive_status_cd,
           xm.active_status_dt_tm = cnvtdatetime(sysdate), xm.active_status_prsnl_id = reqinfo->
           updt_id
          PLAN (xm
           WHERE xm.product_event_id=temp_prod_event_id
            AND (xm.product_id=ops_request->productlist[prod].product_id)
         ...
         ELSE
          UPDATE  FROM product_event pe
           SET pe.active_ind = 0, pe.active_status_cd = reqdata->inactive_status_cd, pe
            .active_status_dt_tm = cnvtdatetime(sysdate),
            pe.active_status_prsnl_id = reqinfo->updt_id, pe.updt_cnt = (pe.updt_cnt+ 1), pe
            .updt_dt_tm = cnvtdatetime(sysdate),
            pe.updt_task = reqinfo->updt_task, pe.updt_id = reqinfo->updt_id, pe.updt_applctx =
          ...
          IF (curqual=0)
           ...
           SET failure_occured = "T"
          ELSEIF ((ops_request->productlist[prod].product_type="B"))
           SET outbound_event_cnt += 1
           SET stat = alterlist(outbound_list->product_events,outbound_event_cnt)
           SET outbound_list->product_events[outbound_event_cnt].product_event_id =
           temp_prod_event_id
          ENDIF
         ENDIF
        ENDIF
        ...
        ELSE
         UPDATE  FROM assign a
          SET a.cur_assign_qty =
           ...
           IF ((ops_request->productlist[prod].product_type="B")) 0
           ELSEIF ((quantity_iu <= ops_request->productlist[prod].productevent[count1].release_iu)) 0
           ELSE (quantity_iu - ops_request->productlist[prod].productevent[count1].release_iu)
           ENDIF
           , a.updt_cnt = (a.updt_cnt+ 1),
           a.updt_dt_tm = cnvtdatetime(sysdate), a.updt_task = reqinfo->updt_task, a.updt_id =
           reqinfo->updt_id,
           a.updt_applctx = reqinfo->updt_applctx, a.active_ind =
           IF ((ops_request->productlist[prod].product_type="B")) 0
           ELSEIF ((quantity_val=ops_request->productlist[prod].productevent[count1].release_qty)) 0
           ELSE 1
           ENDIF
           , a.active_status_cd =
         ...
         ELSE
          ...
          ELSE
           INSERT  FROM assign_release ar
            SET ar.assign_release_id = assign_release_id_val, ar.product_id = ops_request->
             productlist[prod].product_id, ar.product_event_id = temp_prod_event_id,
             ar.release_reason_cd = ops_request->productlist[prod].productevent[count1].
             release_reason_cd, ar.release_dt_tm = cnvtdatetime(sysdate), ar.release_prsnl_id =
             reqinfo->updt_id,
             ar.release_qty =
             IF ((ops_request->productlist[prod].product_type="B")) 0
             ...
             ENDIF
             , ar.updt_cnt = 0,
             ar.updt_dt_tm = cnvtdatetime(sysdate), ar.updt_task = reqinfo->updt_task, ar.updt_id =
             reqinfo->updt_id,
             ar.updt_applctx = reqinfo->updt_applctx, ar.active_ind = 1, ar.active_status_cd =
             reqdata->active_status_cd,
             ar.active_status_dt_tm = cnvtdatetime(sysdate), ar.active_status_prsnl_id = reqinfo->
             updt_id
            WITH nocounter
           ;end insert
           IF (curqual=0)
       ...
       inprogress_event_type_cd))
        CALL chg_product_event(temp_prod_event_id,cnvtdatetime(sysdate),reqinfo->updt_id,0,0,
         reqdata->inactive_status_cd,cnvtdatetime(sysdate),reqinfo->updt_id,temp_pe_updt_cnt,1,
         0)
        IF (curqual=0)
         SET error_process = "update event"
    ...
    product_type="D")))
     ...
     CALL add_product_event(ops_request->productlist[prod].product_id,0,0,0,0,
      avail_event_type_cd,cnvtdatetime(sysdate),reqinfo->updt_id,0,0,
    ...
    IF (failure_occured="F"
     ...
     FOR (count2 = 1 TO nbr_of_events)
       IF ((ops_request->productlist[prod].productevent[count2].event_type_cd=assign_event_type_cd))
        SET der_release_qty += ops_request->productlist[prod].productevent[count2].release_qty
        SET der_release_iu += ops_request->productlist[prod].productevent[count2].release_iu
       ENDIF
     ENDFOR
     UPDATE  FROM derivative der
      SET der.cur_avail_qty = (der.cur_avail_qty+ der_release_qty), der.cur_intl_units = (der
       .cur_intl_units+ der_release_iu), der.updt_cnt = (der.updt_cnt+ 1),
  ...
  ENDFOR
 ENDIF
 IF (size(outbound_list->product_events,5) > 0)
  CALL sendproductevents(null)
 ENDIF
 ...
 SET select_ok_ind = 0
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_batchrelease", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  per.person_id, per.name_full_formatted, pra.person_id,
  pra.alias, p.product_id, ord.accession,
  c_prod.display, sort1 =
  IF (sort_field > " ")
  ...
  PLAN (d_ar)
   ...
   WHERE p.product_id=pe.product_id
    AND expand(pe_index,1,size(ops_request->productlist[d_ar.seq].productevent,5),pe.product_event_id,
  ...
  ORDER BY p.cur_owner_area_cd, p.cur_inv_area_cd, sort1,
   sort2, per.person_id, p.product_nbr,
  ...
  HEAD REPORT
   ...
   cur_inv_area_disp = uar_get_code_display(p.cur_inv_area_cd), select_ok_ind = 0, formatted_acc =
   fillstring(25," ")
  HEAD PAGE
   row 0,
   CALL center(captions->rpt_batch_crossmatch,1,125), col 104,
   captions->as_of_time, col 119, request->ops_date"@TIMENOSECONDS;;M",
   row + 1, col 104, captions->as_of_date,
   col 119, request->ops_date"@DATECONDENSED;;d", inc_i18nhandle = 0,
   ...
   col 17, cur_inv_area_disp, row + 1,
   col 1, captions->prepared, col 11,
   curdate"@DATECONDENSED;;d", row + 3,
   CALL center(captions->unit_number,58,82),
   CALL center(captions->status,98,111), row + 1,
   ...
   CALL center(captions->xm_exp_date,113,125), row + 1,
   col 1, "-------------------", col 21,
   "------------------------------------", col 58, "-------------------------",
   col 84, "-------------", col 98,
   "--------------", col 113, "-------------",
   row + 2
   IF (((product_count <= 0) OR (count1 <= 1)) )
  ...
  HEAD p.product_id
   ...
   IF ((ops_request->productlist[d_ar.seq].product_type="B"))
    number = p.product_nbr, sub_number = p.product_sub_nbr, prod_num = concat(trim(ops_request->
      productlist[d_ar.seq].supp_prefix),trim(number,3)," ",trim(sub_number,3))
   ELSE
    prod_num = p.product_nbr
   ENDIF
   prod_disp = c_prod.display, col 1, med_num,
   col 21, pat_name
  HEAD pe.product_event_id
   IF (row >= 58)
    BREAK
   ENDIF
   reason = " ", pos = locateval(index,1,size(ops_request->productlist[d_ar.seq].productevent,5),pe
    .product_event_id,ops_request->productlist[d_ar.seq].productevent[index].product_event_id)
   IF (pos > 0)
    IF ((ops_request->productlist[d_ar.seq].status="S")
     AND cnvtupper(batch_field)="UPDATE")
     status = "Released", temp_string = " "
     ...
     ELSEIF (pe.event_type_cd=inprogress_event_type_cd)
      temp_string = "Inprog"
     ENDIF
     status = concat(temp_string," ",status), reason = uar_get_code_display(ops_request->productlist[
      d_ar.seq].productevent[pos].release_reason_cd), reason = substring(1,15,reason)
    ELSE
     status = "Not Released"
     ...
     ENDIF
    ENDIF
    col 58, prod_num, col 84,
    prod_disp, col 97, status,
    col 113, ops_request->productlist[d_ar.seq].productevent[pos].xm_exp_dt_tm"@DATETIMECONDENSED;;d",
    row + 1,
    formatted_acc = cnvtacc(ord.accession), col 58, formatted_acc,
    col 97, reason, row + 1
   ENDIF
  DETAIL
   row + 0
  FOOT PAGE
   ...
   col 60, captions->rpt_page, col 67,
   curpage"###", col 101, captions->printed,
   col 110, curdate"@DATECONDENSED;;d", col 119,
   curtime"@TIMENOSECONDS;;M"
  FOOT REPORT
   row 62, col 53, captions->end_of_report,
   select_ok_ind = 1
  WITH nocounter, outerjoin(d1), outerjoin(d2),
   outerjoin(d_pra), dontcare(pra), outerjoin(d_ord),
   maxrow = 63, compress, nolandscape,
   nullreport, expand = 1
 ;end select
 SET rpt_cnt += 1
 ...
 IF (select_ok_ind=1)
  SET reply->status_data.status = "S"
 ENDIF
 SUBROUTINE sendproductevents(null)
   DECLARE req225721 = i4 WITH protect, constant(225721)
   DECLARE hreq225721 = i4 WITH protect, noconstant(0)
   DECLARE hrep225721 = i4 WITH protect, noconstant(0)
   DECLARE hmsg225721 = i4 WITH protect, noconstant(0)
   DECLARE hstatus_data = i4 WITH protect, noconstant(0)
   DECLARE replystatus = vc WITH protect, noconstant("U")
   SET hmsg225721 = uar_srvselectmessage(req225721)
   IF (hmsg225721=0)
    CALL echo("Unable to create message handle")
    RETURN(- (1))
   ENDIF
   SET hreq225721 = uar_srvcreaterequest(hmsg225721)
   IF (hreq225721=0)
    CALL echo("Unable to create request handle")
    CALL uar_srvdestroyinstance(hmsg225721)
    RETURN(- (1))
   ENDIF
   SET hrep225721 = uar_srvcreatereply(hmsg225721)
   IF (hrep225721=0)
    CALL echo("Unable to create reply handle")
    CALL uar_srvdestroyinstance(hmsg225721)
    CALL uar_srvdestroyinstance(hreq225721)
    RETURN(- (1))
   ENDIF
   FOR (i = 1 TO size(outbound_list->product_events,5))
    SET hlist = uar_srvadditem(hreq225721,"product_events")
    SET stat = uar_srvsetdouble(hlist,"product_event_id",outbound_list->product_events[i].
     product_event_id)
   ENDFOR
   SET stat = uar_srvexecute(hmsg225721,hreq225721,hrep225721)
   CALL echo("Request and Reply in cer_log, file name: 225721_REQUEST.dat / 225721_REPLY.dat")
   CALL uar_crmlogmessage(hreq225721,"225721_REQUEST.dat")
   CALL uar_crmlogmessage(hrep225721,"225721_REPLY.dat")
   SET hstatus_data = uar_srvgetstruct(hrep225721,"status_data")
   CALL uar_srvgetstring(hstatus_data,"status",replystatus,uar_srvgetstringlen(hstatus_data,"status")
    )
   CALL echo(build("Status: ",replystatus))
   CALL uar_srvdestroyinstance(hmsg225721)
   CALL uar_srvdestroyinstance(hreq225721)
   CALL uar_srvdestroyinstance(hrep225721)
 END ;Subroutine
#exit_script
END GO
