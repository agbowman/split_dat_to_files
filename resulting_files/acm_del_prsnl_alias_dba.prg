CREATE PROGRAM acm_del_prsnl_alias:dba
 ...
 DELETE  FROM (dummyt d  WITH seq = value(xref->del_cnt)),
  ...
  PLAN (d)
   JOIN (p
   WHERE (p.prsnl_alias_id=acm_request->prsnl_alias_qual[xref->del[d.seq].idx].prsnl_alias_id))
  WITH nocounter, status(reply->prsnl_alias_qual[xref->del[d.seq].idx].status)
 ;end delete
 FOR (index = 1 TO xref->del_cnt)
   IF ((reply->prsnl_alias_qual[xref->del[index].idx].status != 1))
 ...
 SUBROUTINE getexistingrows(x)
   SELECT INTO "nl:"
    FROM prsnl_alias p
    WHERE expand(t1,f_val,t_val,p.prsnl_alias_id,acm_request->prsnl_alias_qual[xref->del[t1].idx].
     prsnl_alias_id,
     max_val)
    DETAIL
     t2 = locateval(t1,f_val,t_val,p.prsnl_alias_id,acm_request->prsnl_alias_qual[xref->del[t1].idx].
      prsnl_alias_id,
   ...
   FOR (index = f_val TO t_val)
     IF ((reply->prsnl_alias_qual[xref->del[index].idx].status=0))
