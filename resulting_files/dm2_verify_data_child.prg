CREATE PROGRAM dm2_verify_data_child
 ...
 DECLARE dvdc_mode = vc WITH protect, noconstant("")
 DECLARE dvdc_src_max_id = f8 WITH protect, noconstant(0.0)
 DECLARE dvdc_tgt_max_id = f8 WITH protect, noconstant(0.0)
 IF (dm2_prg_maint("BEGIN")=0)
  GO TO exit_program
 ENDIF
 SET dvdc_ndx =  $1
 ...
 CASE (dvdc_mode)
  OF "BEGIN":
   SET dm_err->eproc = concat("Get begin key for table:",dmvd->tbls[dvdc_ndx].table_name)
   CALL disp_msg("",dm_err->logfile,10)
   SELECT INTO "nl:"
    inline_dvdc_y = parser(concat("max(b.",dmvd->tbls[dvdc_ndx].keyid_col_name,")"))
    FROM (
     (
     (SELECT
      parser(concat("a.",dmvd->tbls[dvdc_ndx].keyid_col_name))
      FROM (value(concat("DM2VDATS",trim(cnvtstring(dmvd->tbls[dvdc_ndx].src_object_id)))) a)
      WHERE parser(concat("a.",dmvd->tbls[dvdc_ndx].keyid_col_name)) IS NOT null
       AND parser(concat("a.",dmvd->tbls[dvdc_ndx].keyid_col_name," > ",build(dmvd->tbls[dvdc_ndx].
         last_range_keyid_max)))
      ORDER BY parser(concat("a.",dmvd->tbls[dvdc_ndx].keyid_col_name))
      WITH sqltype("f8")))
     b)
    WHERE sqlpassthru("rownum<=1")
    DETAIL
     dmvd->tbls[dvdc_ndx].last_range_keyid_min = inline_dvdc_y
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
   IF (curqual=0)
    SET dmvd->tbls[dvdc_ndx].last_range_keyid_min = - (1.1)
   ENDIF
   SET dm_err->eproc = concat("Begin key:",build(dmvd->tbls[dvdc_ndx].last_range_keyid_min))
   CALL disp_msg("",dm_err->logfile,10)
  OF "END":
   SET dm_err->eproc = concat("Get end key for table:",dmvd->tbls[dvdc_ndx].table_name)
   CALL disp_msg("",dm_err->logfile,10)
   SELECT INTO "nl:"
    inline_dvdc_y = parser(concat("max(b.",dmvd->tbls[dvdc_ndx].keyid_col_name,")")), inline_dvdc_z
     = count(1)
    FROM (
     (
     (SELECT
      ...
      WHERE parser(concat("a.",dmvd->tbls[dvdc_ndx].keyid_col_name)) IS NOT null
       AND parser(concat("a.",dmvd->tbls[dvdc_ndx].keyid_col_name," >= ",build(dmvd->tbls[dvdc_ndx].
         last_range_keyid_min)))
      ORDER BY parser(concat("a.",dmvd->tbls[dvdc_ndx].keyid_col_name))
      WITH sqltype("f8")))
     b)
    WHERE sqlpassthru(build("rownum<=",dmvd->tbls[dvdc_ndx].rows_to_compare))
    DETAIL
     dmvd->tbls[dvdc_ndx].last_range_keyid_max = inline_dvdc_y, dmvd->tbls[dvdc_ndx].last_compare_cnt
      = inline_dvdc_z
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
   ...
   ENDIF
   IF (curqual=0)
    SET dmvd->tbls[dvdc_ndx].last_range_keyid_max = - (1.1)
   ENDIF
   SET dm_err->eproc = concat("End key:",build(dmvd->tbls[dvdc_ndx].last_range_keyid_max))
   CALL disp_msg("",dm_err->logfile,10)
  OF "MAX":
   SET dm_err->eproc = concat("Get SRC max key for table:",dmvd->tbls[dvdc_ndx].table_name)
   CALL disp_msg("",dm_err->logfile,10)
   SELECT INTO "nl:"
    inline_dvdc_x = t1.unique_id
    FROM (
     (
     (SELECT
      unique_id = parser(concat("max(t.",dmvd->tbls[dvdc_ndx].keyid_col_name,")"))
      FROM (value(concat("DM2VDATS",trim(cnvtstring(dmvd->tbls[dvdc_ndx].src_object_id)))) t)
      WITH sqltype("F8")))
     t1)
    DETAIL
     dvdc_src_max_id = inline_dvdc_x
    WITH nocounter
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_program
   ENDIF
   SET dmvd->tbls[dvdc_ndx].max_src_keyid = greatest(dmvd->tbls[dvdc_ndx].max_src_keyid,
    dvdc_src_max_id)
   IF ((dmvd->tgt_to_src=1))
    SET dm_err->eproc = concat("Get TGT max key for table:",dmvd->tbls[dvdc_ndx].table_name)
    CALL disp_msg("",dm_err->logfile,10)
    SELECT INTO "nl:"
     inline_dvdc_x = t1.unique_id
     FROM (
      ...
      (SELECT
       unique_id = parser(concat("max(t.",dmvd->tbls[dvdc_ndx].keyid_col_name,")"))
       FROM (value(concat("DM2VDATT",trim(cnvtstring(dmvd->tbls[dvdc_ndx].src_object_id)))) t)
       WITH sqltype("F8")))
      t1)
     DETAIL
      dvdc_tgt_max_id = inline_dvdc_x
     WITH nocounter
    ;end select
    IF (check_error(dm_err->eproc)=1)
     CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    ...
    SET dmvd->tbls[dvdc_ndx].max_src_keyid = greatest(dmvd->tbls[dvdc_ndx].max_src_keyid,
     dvdc_tgt_max_id)
   ENDIF
   SET dm_err->eproc = concat("Max key:",build(dmvd->tbls[dvdc_ndx].max_src_keyid))
   CALL disp_msg("",dm_err->logfile,10)
 ENDCASE
#exit_program
