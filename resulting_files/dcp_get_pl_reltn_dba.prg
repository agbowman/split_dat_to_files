CREATE PROGRAM dcp_get_pl_reltn:dba
 ...
 DECLARE parseencntrlochist(x=i4) = null
 ...
 IF ((criteria->reltn_prsnl_id=0.0))
  SET criteria->reltn_prsnl_id = reqinfo->updt_id
 ENDIF
 IF ((criteria->provider_grp_cnt > 0))
  CALL examineprovider(1)
  CALL examineuser(1)
 ENDIF
 IF ((request->rmv_pl_rows_flag=1))
  CALL delexpiredremovedrows(1)
 ENDIF
 IF ((criteria->vreltn_cnt != 0))
  CALL executeeprselect(1)
 ENDIF
 IF ((criteria->lreltn_cnt != 0))
  CALL executepprselect(1)
 ENDIF
 SET stat = alterlist(reply->patients,patient_cnt)
 ...
 ENDIF
 SUBROUTINE (executeeprselect(x=i4) =null)
   DECLARE continue = i2 WITH noconstant(0)
   DECLARE continuex = i4 WITH noconstant(0)
   DECLARE teamx = i4 WITH noconstant(0)
   DECLARE useelh = i2 WITH noconstant(0)
   IF ((criteria->time_frame_flag != notimeparameter)
    AND (((criteria->locations.unit_cnt > 0)) OR ((((criteria->service_cnt > 0)) OR ((criteria->
   type_cnt > 0))) )) )
    SET useelh = 1
   ENDIF
   CALL parser("select into 'nl:'")
   CALL parser("from ENCNTR_PRSNL_RELTN EPR")
   IF (useelh > 0)
    CALL parser(", ENCNTR_LOC_HIST ELH")
   ENDIF
   CALL parser(", ENCOUNTER E")
   CALL parser(", PERSON P")
   IF ((criteria->careteam_cnt > 0))
    CALL parser(", DCP_PL_CUSTOM_ENTRY CE")
   ENDIF
   CALL parser("plan EPR where")
   CALL parser("EPR.prsnl_person_id = criteria->reltn_prsnl_id")
   IF ((criteria->time_frame_flag=notimeparameter))
   ...
   ELSEIF ((criteria->time_frame_flag=notimeparameter))
    CALL parser("and EPR.expiration_ind+0 = 0")
    CALL parser("and EPR.end_effective_dt_tm >= cnvtdatetime(curdate, curtime3)")
   ELSEIF ((criteria->time_frame_flag=recenttimeparameter))
    CALL parser("and EPR.end_effective_dt_tm >= cnvtdatetime(criteria->end_effective_dt_tm)")
    CALL parser("and EPR.expiration_ind = 0")
    CALL parser("and EPR.beg_effective_dt_tm >= cnvtdatetime(criteria->beg_effective_dt_tm)")
   ELSE
   ...
   IF ((criteria->careteam_cnt > 0))
    CALL parser("join CE where CE.person_id = P.person_id")
    CALL parser(
     "and expand (teamX, 1, criteria->careteam_cnt, CE.prsnl_group_id+0, criteria->careteams[teamX].group_id)"
     )
   ENDIF
   ...
   ELSE
    ...
    IF ((criteria->vreltn_cnt > 0)
     AND (criteria->reltn_prsnl_id=reqinfo->updt_id))
     CALL parser("if (EPR.active_ind = 1 and EPR.expiration_ind = 1)")
     CALL parser("reply->patients[patient_cnt].filter_ind = 0")
     CALL parser("endif")
    ENDIF
    IF (confid_ind=1)
   ...
   CALL parser(
    "if (EPR.end_effective_dt_tm < cnvtdatetime(curdate,curtime3) or EPR.expiration_ind = 1)")
   CALL parser("reply->patients[patient_cnt].active_ind = 0")
   CALL parser("endif")
   IF (useelh > 0)
    CALL parser("if (ELH.end_effective_dt_tm < cnvtdatetime(curdate,curtime3))")
    CALL parser("reply->patients[patient_cnt].active_ind = 0")
    CALL parser("endif")
   ENDIF
   CALL parser("with nocounter go")
 END ;Subroutine
 SUBROUTINE (executepprselect(x=i4) =null)
   DECLARE continue = i2 WITH noconstant(0)
   DECLARE continuex = i4 WITH noconstant(0)
   DECLARE teamx = i4 WITH noconstant(0)
   DECLARE useelh = i2 WITH noconstant(0)
   ...
   ENDIF
   CALL parser("select into 'nl:'")
   CALL parser("from PERSON_PRSNL_RELTN PPR")
   CALL parser(", PERSON P")
   IF ((criteria->careteam_cnt > 0))
    CALL parser(", DCP_PL_CUSTOM_ENTRY CE")
    IF ((criteria->best_encntr_flag=0)
     AND encntr_org_sec_ind=1)
     SET usee = 1
    ENDIF
   ENDIF
   IF (usee > 0)
   ...
   IF (useelh > 0)
    CALL parser(", ENCNTR_LOC_HIST ELH")
   ENDIF
   CALL parser("plan PPR where")
   CALL parser("PPR.prsnl_person_id = criteria->reltn_prsnl_id")
   CALL parser("and PPR.active_ind = 1")
   IF ((criteria->lreltn_cnt > 0))
    CALL parser(
     "and expand (lifeX, 1, criteria->lreltn_cnt, PPR.person_prsnl_r_cd, criteria->lreltns[lifeX].lreltn_cd)"
   ...
   IF (patient_cnt > 0)
    CALL parser(
     "and not expand (continueX, 1, patient_cnt, PPR.person_id, reply->patients[continueX].person_id)"
     )
   ENDIF
   CALL parser("join P where P.person_id = PPR.person_id and P.active_ind = 1")
   IF ((criteria->careteam_cnt > 0))
    CALL parser("join CE where CE.person_id = P.person_id")
    CALL parser(
     "and expand (teamX, 1, criteria->careteam_cnt, CE.prsnl_group_id+0, criteria->careteams[teamX].group_id)"
    ...
    IF (usee > 0)
     IF ((criteria->best_encntr_flag > 0))
      CALL parseencounterbyencntr("E.person_id = CE.person_id",1)
     ELSE
   ...
   ELSEIF (usee > 0)
    ...
    ENDIF
   ENDIF
   CALL parser("order by PPR.person_id")
   IF (usee > 0)
    CALL parser(", E.encntr_id")
   ENDIF
   IF (useelh > 0)
    CALL parser(", ELH.end_effective_dt_tm desc")
   ENDIF
   IF (usee=1)
    CALL parser("head E.encntr_id")
   ELSE
    CALL parser("head PPR.person_id")
   ENDIF
   CALL parser("patient_cnt = patient_cnt + 1")
   CALL parser("if(mod(patient_cnt,50) = 1)")
   CALL parser("stat = alterlist (reply->patients, patient_cnt + 49)")
   ...
   ENDIF
   CALL parser("reply->patients[patient_cnt].active_ind = 1")
   CALL parser("if (PPR.end_effective_dt_tm < cnvtdatetime(curdate,curtime3))")
   CALL parser("reply->patients[patient_cnt].active_ind = 0")
   CALL parser("endif")
   IF (useelh > 0)
    CALL parser("if (ELH.end_effective_dt_tm < cnvtdatetime(curdate,curtime3))")
   ...
   CALL parser("with nocounter go")
 END ;Subroutine
 SUBROUTINE (examineprovider(x=i4) =null)
   DECLARE providerflag = i2 WITH noconstant(0)
   DECLARE ppr_cd_cnt = i4 WITH noconstant(0)
   DECLARE y = i4 WITH noconstant(0)
   SELECT INTO "nl:"
    FROM prsnl_group_reltn pgr
    PLAN (pgr
     WHERE expand(y,1,criteria->provider_grp_cnt,pgr.prsnl_group_id,criteria->provider_grps[y].
      group_id)
      AND (pgr.person_id=criteria->reltn_prsnl_id)
      AND pgr.active_ind=1)
    HEAD pgr.person_id
     IF ((pgr.person_id=criteria->reltn_prsnl_id))
      providerflag = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (providerflag=0)
    ...
    "Provider supplied does not belong to provider group"
    GO TO exit_script
   ENDIF
   DECLARE ppr_cd_cnt = i4 WITH noconstant(0)
   DECLARE y = i4 WITH noconstant(0)
   ...
   SELECT INTO "nl:"
    ...
    HEAD team.ppr_cd
     ppr_cd_cnt += 1
     IF (mod(ppr_cd_cnt,50)=1)
      stat = alterlist(reltn->reltns,(ppr_cd_cnt+ 49))
     ENDIF
     reltn->reltns[ppr_cd_cnt].reltn_cd = team.ppr_cd
    FOOT REPORT
     stat = alterlist(reltn->reltns,ppr_cd_cnt)
    WITH nocounter
   ;end select
   FOR (x = 1 TO ppr_cd_cnt)
     CALL addreltn(reltn->reltns[x].reltn_cd)
   ENDFOR
 END ;Subroutine
 SUBROUTINE (examineuser(x=i4) =null)
   DECLARE userflag = i2 WITH noconstant(0)
   DECLARE t = i4 WITH noconstant(0)
   SELECT INTO "nl:"
    FROM prsnl_group_reltn pgr
    PLAN (pgr
     WHERE expand(t,1,criteria->provider_grp_cnt,pgr.prsnl_group_id,criteria->provider_grps[t].
      group_id)
      AND pgr.active_ind=1)
    HEAD pgr.person_id
     IF ((pgr.person_id=reqinfo->updt_id))
      userflag = 1
     ENDIF
    WITH nocounter
   ;end select
   IF (userflag=0)
    ...
    SET reply->subeventstatus[1].operationstatus = "F"
    SET reply->subeventstatus[1].targetobjectvalue =
    "User supplied does not belong to provider group"
    GO TO exit_script
   ENDIF
 END ;Subroutine
 SUBROUTINE (delexpiredremovedrows(x=i4) =null)
   RECORD temp(
     1 row_cnt = i4
     1 row[*]
       2 priority_id = f8
   ...
   IF ((criteria->vreltn_cnt != 0)
    ...
    SELECT INTO "nl:"
     ...
     PLAN (d
      ...
      JOIN (epr
      WHERE (epr.prsnl_person_id=criteria->reltn_prsnl_id)
       AND (epr.encntr_id=temp->row[d.seq].encntr_id)
       AND expand(visitx,1,criteria->vreltn_cnt,epr.encntr_prsnl_r_cd,criteria->vreltns[visitx].
       vreltn_cd)
       AND epr.expiration_ind=0
       AND epr.active_ind=1
   ...
   IF ((criteria->lreltn_cnt != 0)
    ...
    SELECT INTO "nl:"
     ...
     PLAN (d
      ...
      JOIN (ppr
      WHERE (ppr.prsnl_person_id=criteria->reltn_prsnl_id)
       AND (ppr.person_id=temp->row[d.seq].person_id)
       AND expand(lifex,1,criteria->lreltn_cnt,ppr.person_prsnl_r_cd,criteria->lreltns[lifex].
       lreltn_cd)
       AND ppr.active_ind=1
