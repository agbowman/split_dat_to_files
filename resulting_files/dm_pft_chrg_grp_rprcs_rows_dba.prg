CREATE PROGRAM dm_pft_chrg_grp_rprcs_rows:dba
 ...
 SET reply->table_name = "PFT_CHRG_GRP_RPRCS"
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(pcgr.pft_chrg_grp_rprcs_id)
   FROM pft_chrg_grp_rprcs pcgr
   WHERE pcgr.pft_chrg_grp_rprcs_id > 0
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(pcgr.pft_chrg_grp_rprcs_id)
  FROM pft_chrg_grp_rprcs pcgr
  DETAIL
 ...
 WHILE (curminid <= maxid
  AND rowsleft > 0)
   SELECT INTO "nl:"
    pcgr.rowid
    FROM pft_chrg_grp_rprcs pcgr
    WHERE pcgr.pft_chrg_grp_rprcs_id BETWEEN curminid AND curmaxid
     AND parser(sbr_getrowidnotexists("pcgr.pft_chrg_grp_rprcs_id != 0","pcgr"))
     AND pcgr.updt_dt_tm <= cnvtdatetime((curdate - v_days_to_keep),curtime3)
    DETAIL
     rowcnt += 1
     IF (mod(rowcnt,100)=1)
      stat = alterlist(reply->rows,(rowcnt+ 99))
     ENDIF
     reply->rows[rowcnt].row_id = pcgr.rowid
    WITH nocounter, maxqual(pcgr,value(rowsleft))
   ;end select
