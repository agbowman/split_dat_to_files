CREATE PROGRAM bbt_rpt_quarantine:dba
 RECORD reply(
   1 rpt_list[*]
     2 rpt_filename = vc
   1 status_data
 ...
 SET captions->quarantine_reasons = uar_i18ngetmessage(i18nhandle,"quarantine_reasons",
  "QUARANTINE REASONS")
 ...
 SET rpt_cnt = 0
 EXECUTE cpm_create_file_name_logical "bbt_quarantine", "txt", "x"
 SELECT INTO cpm_cfn_info->file_name_logical
  ...
  PLAN (cv
   WHERE cv.code_set=1630)
   JOIN (cd
   WHERE cd.code_set=outerjoin(1630)
  ...
  HEAD PAGE
   ...
   col 50, captions->quarantine_reasons, row + 2,
