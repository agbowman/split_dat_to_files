CREATE PROGRAM dm_pcmb_wl_activity:dba
 ...
 DECLARE cur_encntr_id = f8
 SET v_cust_count1 = 0
 SET v_cust_count2 = 0
 SET index_occur_cnt = 0
 SET cur_to_id = request->xxx_combine[icombine].to_xxx_id
 SET cur_encntr_id = request->xxx_combine[icombine].encntr_id
 SUBROUTINE main(null)
   ...
   IF ((dm_cmb_cust_script->exc_maint_ind=1))
    ...
    SET dcem_request->qual[1].script_name = "DM_PCMB_WL_ACTIVITY"
   ...
   IF (cur_encntr_id > 0)
    SELECT INTO "nl:"
     ...
     WHERE (frm.person_id=request->xxx_combine[icombine].from_xxx_id)
      AND frm.encntr_id=cur_encntr_id
      AND ((frm.active_ind+ 0)=1)
     HEAD REPORT
      v_cust_count1 = 0
     DETAIL
      v_cust_count1 = (v_cust_count1+ 1), stat = alterlist(updatelist->qual,v_cust_count1),
      updatelist->qual[v_cust_count1].from_id = frm.person_id,
      updatelist->qual[v_cust_count1].person_id = frm.person_id, updatelist->qual[v_cust_count1].
      beg_effective_dt_tm = frm.beg_effective_dt_tm, updatelist->qual[v_cust_count1].encntr_id = frm
      .encntr_id,
      updatelist->qual[v_cust_count1].end_effective_dt_tm = frm.end_effective_dt_tm, updatelist->
      qual[v_cust_count1].sch_appt_id = frm.sch_appt_id, updatelist->qual[v_cust_count1].
      wl_criteria_id = frm.wl_criteria_id,
      updatelist->qual[v_cust_count1].worklist_cd = frm.worklist_cd, updatelist->qual[v_cust_count1].
      active_ind = frm.active_ind, updatelist->qual[v_cust_count1].active_status_cd = frm
      .active_status_cd,
      updatelist->qual[v_cust_count1].wl_activity_id = frm.wl_activity_id
     FOOT REPORT
      updatelist->cnt = v_cust_count1, stat = alterlist(updatelist->qual,v_cust_count1)
   ...
   ELSE
    SELECT INTO "nl:"
     frm.*
     FROM wl_activity frm
     WHERE frm.person_id IN (request->xxx_combine[icombine].from_xxx_id, cur_to_id)
      AND ((frm.active_ind+ 0)=1)
     HEAD REPORT
      v_cust_count1 = 0
     DETAIL
      ...
      beg_effective_dt_tm = frm.beg_effective_dt_tm, rreclist->from_rec[v_cust_count1].encntr_id =
      frm.encntr_id,
      rreclist->from_rec[v_cust_count1].end_effective_dt_tm = frm.end_effective_dt_tm, rreclist->
      from_rec[v_cust_count1].person_id = frm.person_id, rreclist->from_rec[v_cust_count1].
      sch_appt_id = frm.sch_appt_id,
      rreclist->from_rec[v_cust_count1].wl_criteria_id = frm.wl_criteria_id, rreclist->from_rec[
      v_cust_count1].worklist_cd = frm.worklist_cd, rreclist->from_rec[v_cust_count1].active_ind =
      frm.active_ind,
      ...
      v_cust_count1].wl_activity_id = frm.wl_activity_id, rreclist->from_rec[v_cust_count1].index_key
       = build(frm.wl_criteria_id,"_",frm.encntr_id,"_",frm.sch_appt_id)
     FOOT REPORT
    ...
    SELECT
     ...
     DETAIL
      ...
      beg_effective_dt_tm, temprows->qual[index_occur_cnt].encntr_id = rreclist->from_rec[d1.seq].
      encntr_id, temprows->qual[index_occur_cnt].end_effective_dt_tm = rreclist->from_rec[d1.seq].
      end_effective_dt_tm,
      temprows->qual[index_occur_cnt].person_id = rreclist->from_rec[d1.seq].person_id, temprows->
