CREATE PROGRAM cp_add_pathway_activity:dba
 PROMPT
  ...
  "Personnel Id" = 0.0,
  "Pathway Id" = 0.0,
  "Pathway Activity Status Meaning" = "",
  "Pathway Instance ID" = 0.0,
  "Automatic added indicator" = 0
  WITH outdev, dinput_person_id, dinput_encntr_id,
  dinput_prsnl_id, dinput_pathway_id, spathway_status_meaning,
  dinput_pathway_instance_id, dauto_add_ind
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE is_a_cd = f8 WITH constant(uar_get_code_by("MEANING",4003133,"IS-A")), protect
 DECLARE active_diagnosis_cd = f8 WITH constant(uar_get_code_by("MEANING",48,"ACTIVE")), protect
 DECLARE active_pathway_cd = f8 WITH constant(uar_get_code_by("MEANING",4003198,"ACTIVE")), protect
 SUBROUTINE (getisadiagnomenid(pathway_id=f8,person_id=f8,encntr_id=f8) =f8 WITH protect)
   DECLARE start_tm = dq8 WITH noconstant(curtime3), private
   DECLARE retval = f8 WITH noconstant(0.0), protect
   CALL log_message("Begin getIsADiagNomenId()",log_level_debug)
   SELECT INTO "nl:"
    rank_mean = uar_get_code_meaning(d.ranking_cd)
    FROM cp_pathway cp,
     cp_concept_group ccg,
     cp_concept_group_dtl ccgd,
     diagnosis d
    PLAN (cp
     WHERE cp.cp_pathway_id=cnvtreal(pathway_id)
      AND cp.pathway_status_cd=cnvtreal(active_pathway_cd))
     JOIN (ccg
     WHERE ccg.concept_cd=cp.cp_concept_cd
      AND ccg.concept_group_cd=cnvtreal(is_a_cd))
     JOIN (ccgd
     WHERE ccgd.concept_entity_name="NOMENCLATURE"
      AND ccgd.cp_concept_group_id=ccg.cp_concept_group_id)
     JOIN (d
     WHERE d.nomenclature_id=ccgd.concept_entity_id
      AND d.active_status_cd=cnvtreal(active_diagnosis_cd)
      AND d.person_id=cnvtreal(person_id)
      AND d.encntr_id=cnvtreal(encntr_id)
      AND d.diagnosis_group > 0)
    ORDER BY rank_mean
    HEAD cp.cp_pathway_id
     retval = d.nomenclature_id
    WITH nocounter
   ;end select
   CALL log_message(build("Exit getIsADiagNomenId(), Elapsed time in seconds:",((curtime3 - start_tm)
 ...
 RECORD mpagereply(
   1 cp_pathway_id = f8
   1 cp_pathway_activity_id = f8
   1 beg_effective_dt_tm = dq8
   1 pathway_status_meaning = vc
   1 pathway_instance_id = f8
   1 diagnosis_nomen_id = f8
   1 status_data
     2 status = c1
     2 subeventstatus[1]
       3 operationname = c25
 ...
 FREE RECORD currentpathwayactivities
 RECORD currentpathwayactivities(
   1 cnt = i4
   1 qual[*]
     2 cp_pathway_activity_id = f8
 )
 DECLARE verifyinputs(null) = null WITH protect
 DECLARE getpreviouspathwayactivity(null) = null WITH protect
 DECLARE addpathwayactivity(null) = null WITH protect
 DECLARE getnextmpageseq(null) = f8 WITH protect
 DECLARE dinput_pw_act_status_cd = f8 WITH constant(uar_get_code_by("MEANING",4003352,trim(
     $SPATHWAY_STATUS_MEANING,3))), protect
 DECLARE dinput_pathway_instance_id = f8 WITH constant( $DINPUT_PATHWAY_INSTANCE_ID)
 DECLARE pathway_id = f8 WITH constant(cnvtreal( $DINPUT_PATHWAY_ID)), protect
 DECLARE person_id = f8 WITH constant(cnvtreal( $DINPUT_PERSON_ID)), protect
 DECLARE encntr_id = f8 WITH constant(cnvtreal( $DINPUT_ENCNTR_ID)), protect
 DECLARE auto_add_ind = i2 WITH noconstant(cnvtint( $DAUTO_ADD_IND)), protect
 DECLARE active_act_status_cd = f8 WITH constant(uar_get_code_by("MEANING",4003352,"ACTIVE")),
 protect
 DECLARE proposed_act_status_cd = f8 WITH constant(uar_get_code_by("MEANING",4003352,"PROPOSED")),
 protect
 DECLARE complete_act_status_cd = f8 WITH constant(uar_get_code_by("MEANING",4003352,"COMPLETE")),
 protect
 DECLARE void_act_status_cd = f8 WITH constant(uar_get_code_by("MEANING",4003352,"VOID")), protect
 CALL log_message("Begin program inn_mp_ccl_unified_template",log_level_debug)
 DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
 SET mpagereply->status_data.status = "F"
 CALL verifyinputs(null)
 IF (dinput_pw_act_status_cd != proposed_act_status_cd)
  CALL getpreviouspathwayactivity(null)
 ENDIF
 CALL addpathwayactivity(null)
 SET mpagereply->status_data.status = "S"
 SUBROUTINE verifyinputs(null)
   CALL log_message("Begin VerifyInputs()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   IF (person_id <= 0)
    CALL reporterrorandexit("Validate","Person ID must be non-zero",cnvtstring(person_id))
   ENDIF
   IF (encntr_id <= 0)
    CALL reporterrorandexit("Validate","Encounter ID must be non-zero",cnvtstring(encntr_id))
   ENDIF
   IF (pathway_id <= 0)
    CALL reporterrorandexit("Validate","Pathway ID must be non-zero",cnvtstring(pathway_id))
   ENDIF
   IF (dinput_pw_act_status_cd <= 0)
    CALL reporterrorandexit("Validate","Status Cd must be valid",cnvtstring(dinput_pw_act_status_cd))
   ENDIF
   CALL log_message(build("Exit VerifyInputs(), Elapsed time in seconds:",((curtime3 - begin_curtime3
     )/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getpreviouspathwayactivity(null)
   CALL log_message("Begin GetPreviousPathwayActivity()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE dpreviouspwactid = f8 WITH noconstant(0), protect
   DECLARE iduplicateind = i2 WITH noconstant(0), protect
   SELECT INTO "NL:"
    FROM cp_pathway_activity cpa
    WHERE cpa.person_id=person_id
     AND cpa.cp_pathway_id=pathway_id
     AND cpa.end_effective_dt_tm > cnvtdatetime(sysdate)
    DETAIL
     IF (cpa.pathway_instance_id=dinput_pathway_instance_id
      AND ((cpa.pathway_activity_status_cd=dinput_pw_act_status_cd) OR (cpa
     .pathway_activity_status_cd IN (complete_act_status_cd, void_act_status_cd))) )
      iduplicateind = 1, dpreviouspwactid = cpa.cp_pathway_activity_id
     ELSE
      IF (cpa.pathway_activity_status_cd IN (active_act_status_cd, proposed_act_status_cd))
       currentpathwayactivities->cnt += 1, stat = alterlist(currentpathwayactivities->qual,
        currentpathwayactivities->cnt), currentpathwayactivities->qual[currentpathwayactivities->cnt]
       .cp_pathway_activity_id = cpa.cp_pathway_activity_id,
       auto_add_ind = cpa.auto_added_ind
      ENDIF
     ENDIF
    WITH nocounter
   ;end select
   CALL error_and_zero_check_rec(curqual,"CP_PATHWAY_ACTIVITY","SELECT",1,0,
    mpagereply)
   IF (iduplicateind=1)
    CALL reporterrorandexit("Validate","Active pathway activity with same status exists.",cnvtstring(
      dpreviouspwactid))
   ENDIF
   CALL log_message(build("Exit GetPreviousPathwayActivity(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE addpathwayactivity(null)
   CALL log_message("Begin AddPathwayActivity()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE dcppathwayactivityid = f8 WITH noconstant(0), protect
   DECLARE dcppathwayinstanceid = f8 WITH noconstant(dinput_pathway_instance_id), protect
   DECLARE beg_effective_dt_tm = dq8 WITH noconstant(0), protect
   DECLARE dprsnlid = f8 WITH noconstant( $DINPUT_PRSNL_ID), protect
   DECLARE iactcntr = i4 WITH noconstant(0), protect
   FOR (iactcntr = 1 TO currentpathwayactivities->cnt)
    UPDATE  FROM cp_pathway_activity cpa
     SET cpa.end_effective_dt_tm = cnvtdatetime(sysdate)
     WHERE cpa.cp_pathway_activity_id=value(currentpathwayactivities->qual[iactcntr].
      cp_pathway_activity_id)
     WITH nocounter
    ;end update
    CALL error_and_zero_check_rec(curqual,"CP_PATHWAY_ACTIVITY","UPDATE",1,1,
     mpagereply)
   ENDFOR
   SET dcppathwayactivityid = getnextmpageseq(null)
   IF (dcppathwayinstanceid=0)
    SET dcppathwayinstanceid = getnextmpageseq(null)
   ENDIF
   SET beg_effective_dt_tm = cnvtdatetime(sysdate)
   IF (dprsnlid=0.0)
    SET dprsnlid = reqinfo->updt_id
   ENDIF
   INSERT  FROM cp_pathway_activity cpa
    SET cpa.beg_effective_dt_tm = cnvtdatetime(sysdate), cpa.comment_txt = " ", cpa
     .cp_pathway_activity_id = dcppathwayactivityid,
     cpa.pathway_instance_id = dcppathwayinstanceid, cpa.cp_pathway_id = pathway_id, cpa.encntr_id =
     encntr_id,
     cpa.person_id = person_id, cpa.prsnl_id = dprsnlid, cpa.end_effective_dt_tm = cnvtdatetime(
      "31-JAN-2100 23:59:59.59"),
     cpa.pathway_activity_status_cd = dinput_pw_act_status_cd, cpa.auto_added_ind = auto_add_ind, cpa
     .updt_id = reqinfo->updt_id
    WITH nocounter
   ;end insert
   CALL error_and_zero_check_rec(curqual,"CP_PATHWAY_ACTIVITY","INSERT",1,1,
    mpagereply)
   SET mpagereply->cp_pathway_id = pathway_id
   SET mpagereply->cp_pathway_activity_id = dcppathwayactivityid
   SET mpagereply->beg_effective_dt_tm = beg_effective_dt_tm
   SET mpagereply->pathway_status_meaning = trim( $SPATHWAY_STATUS_MEANING,3)
   SET mpagereply->pathway_instance_id = dcppathwayinstanceid
   SET mpagereply->diagnosis_nomen_id = getisadiagnomenid(pathway_id,person_id,encntr_id)
   SET reqinfo->commit_ind = 1
   CALL log_message(build("Exit AddPathwayActivity(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE getnextmpageseq(null)
   CALL log_message("In GetNextMPageSeq()",log_level_debug)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE dmpagesseq = f8 WITH noconstant(0), protect
   SELECT INTO "nl:"
    nextseqnum = seq(mpages_seq,nextval)
    FROM dual
    DETAIL
     dmpagesseq = nextseqnum
    WITH nocounter
   ;end select
   CALL echo(build2("Next MPages Seq:",dmpagesseq))
   IF (dmpagesseq=0.0)
    CALL reporterror("PERFORM","INSERT"," Unable to generate MPAGES_SEQ.")
    ROLLBACK
    GO TO exit_script
   ENDIF
   CALL log_message(build("Exit GetNextMPageSeq(), Elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
   RETURN(dmpagesseq)
 END ;Subroutine
...
#exit_script
 ...
 ENDIF
 IF ((mpagereply->status_data.status != "S"))
 ...
 IF (( $OUTDEV != "NOFORMS"))
  ...
  FREE RECORD mpagereply
 ENDIF
 CALL log_message(build("Exit Script cp_add_pathway_activity, Elapsed time in seconds:",((curtime3 -
   begin_curtime3)/ 100.0)),log_level_debug)
END GO
