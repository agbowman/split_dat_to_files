CREATE PROGRAM dm2_apply_sizing_parser:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dasp_stmt = vc
 DECLARE dasp_tbl_name = vc
 DECLARE dasp_col_name = vc
 DECLARE dasp_flag = i2 WITH protect, noconstant(0)
 SET dasp_flag =  $1
 SET dasp_stmt =  $2
 SET dasp_tbl_name =  $3
 SET dasp_col_name =  $4
 SET dm_err->eproc = "Executing dm2_apply_sizing_parser..."
 IF (dasp_flag=1)
  SELECT INTO "nl:"
   s = sqlpassthru(dasp_stmt,0)
   FROM (value(dasp_tbl_name) d)
   WHERE parser(build("d.",dasp_col_name," != null"))
   HEAD REPORT
    das_avg_sum = 0.0
   FOOT REPORT
    das_avg_sum = avg(s)
 ...
 ELSEIF (dasp_flag=2)
  SELECT INTO "nl:"
   parser(build("t.",dasp_col_name)), ni = nullind(parser(build("t.",dasp_col_name)))
   FROM (value(dasp_tbl_name) t)
   HEAD REPORT
    das_a_sum = 0.0
   FOOT REPORT
    das_a_sum = sum(evaluate(ni,1,1,0))
