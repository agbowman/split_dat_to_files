CREATE PROGRAM dcpreqgen02:dba
 ...
 SELECT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   ENDIF
   IF (curutc > 0)
    orders->qual[ord_cnt].order_dt = trim(concat(trim(datetimezoneformat(oa.order_dt_tm,oa.order_tz,
        "@SHORTDATETIMENOSEC"))," ",trim(datetimezoneformat(oa.order_dt_tm,oa.order_tz,"ZZZ")))),
    orders->qual[ord_cnt].signed_dt = trim(concat(trim(datetimezoneformat(oa.order_dt_tm,oa.order_tz,
        "@SHORTDATETIMENOSEC"))," ",trim(datetimezoneformat(oa.order_dt_tm,oa.order_tz,"ZZZ"))))
   ELSE
    orders->qual[ord_cnt].order_dt = format(oa.order_dt_tm,"@SHORTDATETIMENOSEC"), orders->qual[
    ord_cnt].signed_dt = format(o.orig_order_dt_tm,"@SHORTDATETIMENOSEC")
   ENDIF
 ...
 SELECT INTO "nl:"
  ...
  HEAD od.oe_field_id
   ...
   IF (od.oe_field_meaning="REQSTARTDTTM")
    IF (curutc > 0)
     orders->qual[d1.seq].req_st_dt = trim(concat(trim(datetimezoneformat(od.oe_field_dt_tm_value,od
         .oe_field_tz,"@SHORTDATETIMENOSEC"))," ",trim(datetimezoneformat(od.oe_field_dt_tm_value,od
         .oe_field_tz,"ZZZ"))))
    ELSE
  ...
  DETAIL
   IF (odflag=1)
    ...
    IF (od.oe_field_dt_tm_value != null)
     IF (curutc > 0)
      orders->qual[d1.seq].d_qual[dc].value = trim(concat(trim(datetimezoneformat(od
          .oe_field_dt_tm_value,od.oe_field_tz,"@SHORTDATETIMENOSEC"))," ",trim(datetimezoneformat(od
          .oe_field_dt_tm_value,od.oe_field_tz,"ZZZ"))))
     ELSE
 ...
 IF ((orders->spoolout_ind=1))
  ...
  SELECT INTO value(tempfile1a)
   ...
   HEAD PAGE
    ...
    "ADMIT DX:", row + 1
    IF (textlen(orders->admit_diagnosis) > 35)
     "{CPI/12}{POS/125/130}",
    ...
    ENDIF
    "{CPI/12}{POS/410/130}", "AGE:  ", orders->age,
    row + 1, "{CPI/12}{POS/30/160}", "ADMIT DATE:  ",
    row + 1, "{CPI/12}{POS/125/160}", orders->admit_dt,
    row + 1, "{CPI/12}{POS/410/160}", "HGT / WT: ",
    CALL print(trim(orders->height)), "/",
    CALL print(trim(orders->weight)),
    row + 1, "{CPI/12}{POS/30/170}", "NURSING UNIT:",
    row + 1, "{CPI/10}{POS/125/170}{B}", orders->nurse_unit,
    "{ENDB}", row + 1, "{CPI/12}{POS/410/170}",
    "SEX:  ", orders->sex, row + 1,
    "{CPI/12}{POS/30/180}", "ROOM/BED:", row + 1,
    "{CPI/12}{POS/125/180}", orders->room, orders->bed,
    row + 1, "{CPI/10}{POS/30/210}", "ALLERGIES:    ",
    "{b}"
    ...
    ENDIF
    "{CPI/10}{POS/20/255}{BOX/75/2}", row + 1, "{CPI/8}{POS/24/260}{color/20/145}",
    row + 1, "{CPI/8}{POS/24/267}{color/20/145}", row + 1,
    "{CPI/8}{POS/24/274}{color/20/145}", row + 1, "{CPI/8}{POS/24/278}{color/20/145}",
    row + 1, "{CPI/12}{POS/30/295}{B}", "ORDER DATE/TIME:",
    "{CPI/12}{POS/30/307}", "ORDERING MD:", "{endb}",
    row + 1, "{CPI/12}{POS/30/319}", "ORDER ENTERED BY:",
    row + 1, "{CPI/12}{POS/30/331}", "ORDER NUMBER:",
    row + 1
    IF (saved_pos > 0)
     "{CPI/10}{POS/1/90}", " ", "{CPI/10}{b}",
     CALL center(orders->facility,1,190), row + 1, "{CPI/8}{POS/30/270}{b}",
     "ORDER:  ", orders->qual[saved_pos].mnemonic, row + 1,
     ...
     "{CPI/12}{POS/30/735}", "ORDER    ", "{CPI/9}{B}",
     orders->qual[saved_pos].mnemonic, saved_pos = 0
    ENDIF
   DETAIL
    FOR (vv = 1 TO value(ord_cnt))
      IF ((orders->qual[vv].display_ind=1))
       ...
       IF (go_ahead_and_print=1)
        ...
        "{CPI/10}{b}",
        CALL center(orders->facility,1,190), "{CPI/8}{POS/30/270}{b}",
        "ORDER:  ", orders->qual[vv].mnemonic, row + 1,
        ...
        "{CPI/12}{POS/30/735}", "ORDER    ", "{CPI/9}{B}",
        orders->qual[vv].mnemonic, xcol = 30, ycol = 363
        FOR (fsub = 1 TO 31)
