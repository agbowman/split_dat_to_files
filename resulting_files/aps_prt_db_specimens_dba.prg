CREATE PROGRAM aps_prt_db_specimens:dba
 ...
 SET captions->apsrpt = uar_i18ngetmessage(i18nhandle,"h1","REPORT: APS_PRT_DB_SPECIMENS.PRG")
 ...
 SET captions->dbspecimentool = uar_i18ngetmessage(i18nhandle,"h8","DB SPECIMENS TOOL")
 SET captions->ppage = uar_i18ngetmessage(i18nhandle,"h9","PAGE:")
 SET captions->status = uar_i18ngetmessage(i18nhandle,"h10","STATUS")
 ...
 SET captions->ttype = uar_i18ngetmessage(i18nhandle,"h13","TYPE")
 ...
 SELECT INTO "nl:"
  ...
  HEAD REPORT
   spec_cnt = 0
  DETAIL
   spec_cnt = (spec_cnt+ 1), stat = alterlist(temp->qual,spec_cnt), temp->qual[spec_cnt].
   spec_cdf_meaning = cv.cdf_meaning,
   temp->qual[spec_cnt].spec_display = cv.display, temp->qual[spec_cnt].spec_display_key = cv
   .display_key, temp->qual[spec_cnt].spec_description = cv.description,
   temp->qual[spec_cnt].spec_active_ind = cv.active_ind
  WITH nocounter
 ;end select
#report_maker
 EXECUTE cpm_create_file_name_logical "apsDbSpecimens", "dat", "x"
 ...
 SELECT INTO value(reply->print_status_data.print_filename)
  temp->qual[d.seq].spec_cdf_meaning, temp->qual[d.seq].spec_display, temp->qual[d.seq].
  spec_display_key,
  temp->qual[d.seq].spec_description, temp->qual[d.seq].spec_active_ind
  FROM (dummyt d  WITH seq = value(size(temp->qual,5)))
  PLAN (d)
  ORDER BY temp->qual[d.seq].spec_active_ind DESC, temp->qual[d.seq].spec_cdf_meaning, temp->qual[d
   .seq].spec_display_key
  HEAD REPORT
  ...
  HEAD PAGE
   ...
   row + 1,
   CALL center(captions->dbspecimentool,0,132), col 110,
   captions->ppage, col 117, curpage"###",
   row + 2, row + 1, col 0,
   captions->status, col 10, captions->code,
   col 27, captions->description, col 69,
   captions->ttype, row + 1, col 0,
   "--------", col 10, "---------------",
   col 27, "---------------------------------------", col 69,
   "----"
  DETAIL
   ...
   temp->qual[d.seq].spec_description, col 69, temp->qual[d.seq].spec_cdf_meaning
   IF (((row+ 7) > maxrow))
    BREAK
   ENDIF
  FOOT PAGE
