CREATE PROGRAM dm2_create_db_env:dba
 ...
 SUBROUTINE des_new_id_prompt(null)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dcde_idx = i4 WITH protect, noconstant(0)
 IF (check_logfile("dm2_cde",".log","dm2_create_db_env LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_create_db_env."
 CALL disp_msg(" ",dm_err->logfile,0)
 IF (validate(drrr_responsefile_in_use,0)=1)
  SET dm_err->eproc =
  "Response file detected and will be used to complete database environment setup."
  CALL disp_msg("",dm_err->logfile,0)
  ...
  FOR (dcde_idx = 1 TO drrr_misc_data->tgt_db_user_pwd_map_cnt)
    SET dir_db_users_pwds->cnt = (dir_db_users_pwds->cnt+ 1)
    SET stat = alterlist(dir_db_users_pwds->qual,dir_db_users_pwds->cnt)
  ...
  SET des_env_data->db_node_os = drrr_rf_data->tgt_db_node_os
  SET des_env_data->db_ora_version = drrr_rf_data->tgt_db_oracle_ver
  SET des_env_data->env_name = drrr_rf_data->tgt_db_env_name
  SET des_env_data->env_desc = drrr_rf_data->tgt_env_desc
  SET des_env_data->db_name = cnvtupper(drrr_rf_data->tgt_db_name)
  SET des_env_data->v500_connect_string = concat("V500/",drrr_rf_data->tgt_db_user_pwd,"@",
   drrr_rf_data->tgt_db_cnct_str)
 ELSE
  IF ((((dm2_install_schema->p_word="NONE")) OR ((dm2_install_schema->connect_str="NONE"))) )
   ...
   ENDIF
  ENDIF
  IF ((des_env_data->loaded_ind=0))
   IF (des_env_data_prompt(null)=0)
    GO TO exit_script
   ENDIF
  ENDIF
  SET dm_err->eproc = "Prompt for V500 password."
  IF (dir_load_users_pwds("V500")=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF (des_env_data_check(null)=0)
  GO TO exit_script
 ENDIF
 IF (des_get_env_data(0)=0)
  GO TO exit_script
 ENDIF
 IF ((des_env_data->env_exists=1))
  IF (des_env_switch(des_env_data->env_id)=0)
   GO TO exit_script
  ENDIF
  SET dir_env_maint_rs->tgt_env_id = des_env_data->env_id
  SET dir_env_maint_rs->tgt_hist_fnd = 0
  ...
  IF ((dir_env_maint_rs->tgt_hist_fnd=1)
   AND validate(dm2_skip_new_env_creation,- (1))=1)
   SET dm_err->eproc = concat("Skipping new environment creation due to history being found.  ",
    "User elected to leave existing environment intact.")
   CALL disp_msg(" ",dm_err->logfile,0)
  ELSEIF ((dir_env_maint_rs->tgt_hist_fnd=1))
   IF (validate(drrr_responsefile_in_use,0)=0)
    IF (des_new_id_prompt(null)=0)
     GO TO exit_script
    ENDIF
   ENDIF
   SET dm_err->eproc = concat("Update old environment name with 'OLD' context and ",
    "temporarily change description with 'OLD' context")
   CALL disp_msg(" ",dm_err->logfile,0)
   UPDATE  FROM dm_environment d
    SET d.environment_name = concat("OLD <",cnvtstring(des_env_data->env_id),">"), d.description =
     concat("OLD_NAME:",d.environment_name)
    WHERE (d.environment_id=des_env_data->env_id)
    WITH nocounter
   ;end update
   IF (check_error(dm_err->eproc)=1)
    ROLLBACK
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Ending dm2_create_db_env."
 ENDIF
 CALL final_disp_msg("dm2_cde")
END GO
