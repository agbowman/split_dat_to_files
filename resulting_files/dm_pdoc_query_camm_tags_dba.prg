CREATE PROGRAM dm_pdoc_query_camm_tags:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "User Id:" = 0.00,
  "Encounter Id :" = 0.00
  WITH outdev, userid, enctid
 ...
 DECLARE encounterid = f8 WITH public, noconstant(0.0)
 DECLARE storagecd = f8 WITH protect, noconstant(uar_get_code_by("MEANING",25,"MMF"))
 DECLARE tableexists = i2 WITH protect, noconstant(0)
 DECLARE userid = f8 WITH public, noconstant(0.0)
 DECLARE pdoc_tag = vc WITH public, constant("PDOC_TAG_HANDLE")
 SET reply->status_data.status = "F"
 SET userid =  $USERID
 SET encounterid =  $ENCTID
 SELECT INTO "nl:"
  FROM dprotect d
  WHERE d.object="T"
   AND d.object_name=pdoc_tag
  DETAIL
   tableexists = 1
 ;end select
 SET reply->status_data.status = "Z" WITH nocounter
 IF (tableexists=1)
  SELECT INTO "nl:"
   FROM pdoc_tag p,
    pdoc_tag_handle pth
   PLAN (p
    WHERE p.tag_user_id=userid
     AND p.encntr_id=encounterid
     AND p.tag_entity_name=pdoc_tag)
    JOIN (pth
    WHERE pth.pdoc_tag_handle_id=p.tag_entity_id
     AND pth.storage_cd=storagecd)
   DETAIL
    count += 1
    IF (mod(count,10)=1)
     stat = alterlist(reply->tags,(count+ 10))
    ENDIF
    reply->tags[count].pdoc_tag_id = p.pdoc_tag_id, reply->tags[count].media_identifier = pth
    .blob_handle, reply->tags[count].pdoc_tag_entity_id = p.tag_entity_id,
    reply->tags[count].emr_type_cd = p.emr_type_cd
   WITH nocounter
  ;end select
  IF (size(reply->tags,5) > 0)
   SET reply->status_data.status = "S"
   CALL alterlist(reply->tags,count)
  ELSE
