CREATE PROGRAM dm_rmc_mover_1295:dba
 ...
 SET dm_err->eproc = "Starting DM_RDM_CUTOVER_1295"
 SELECT INTO "nl:"
  FROM code_value cv
  WHERE cv.cdf_meaning="AVAILABLE"
   AND cv.code_set=291
   AND cv.active_ind=1
  DETAIL
   bed_status_default = cv.code_value
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (bed_status_default < 0)
  ...
  IF (bed_status_default < 0)
   ...
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   SET dm2_ref_data_reply->error_ind = 1
   SET dm2_ref_data_reply->error_msg = "NOMV34"
   GO TO exit_1295
  ENDIF
 ENDIF
 ...
 ENDIF
 IF (bed_idcd_check=1)
  CALL put_value(bed_tab_name,"BED_STATUS_CD",cnvtstring(bed_status_default))
