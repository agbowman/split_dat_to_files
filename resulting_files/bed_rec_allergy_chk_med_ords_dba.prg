CREATE PROGRAM bed_rec_allergy_chk_med_ords:dba
 ...
 SELECT INTO "nl:"
  FROM br_rec b,
   br_long_text bl
  PLAN (b
   WHERE b.rec_mean="ORDALLERGYCHKMEDORDERS2")
   JOIN (bl
 ...
 IF (detail_mode=1)
  IF (((powerchart_found=0) OR (powerchart_pref_row_exists=0)) )
   ...
   SET tempreply->rowlist[row_tot_cnt].celllist[2].string_value = "MULREAT"
  ...
  HEAD ap.application_number
   ...
   IF (priv_exists_ind=1
    ...
    IF (nvp.name_value_prefs_id > 0
     ...
     row_tot_cnt].celllist[2].string_value = "MULREAT", tempreply->rowlist[row_tot_cnt].celllist[3].
     string_value = "1",
