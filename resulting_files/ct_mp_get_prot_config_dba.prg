CREATE PROGRAM ct_mp_get_prot_config:dba
 PROMPT
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "ct_mp_get_prot_config"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   reply->item_list[cnt].item_mean = uar_get_code_meaning(pcv.item_cd), reply->item_list[cnt].
   value_mean = uar_get_code_meaning(pcv.config_value_cd)
  FOOT REPORT
