CREATE PROGRAM aps_chg_report_params:dba
 RECORD temp_org_struct(
   1 organizations[*]
 ...
 DECLARE lcounter2 = i4 WITH protect, noconstant(0)
 ...
 DECLARE lloop_counter = i4 WITH protect, noconstant(0)
 DECLARE lnew_pos = i4 WITH protect, noconstant(0)
 DECLARE lend_pos = i4 WITH protect, noconstant(0)
 DECLARE lend_pos2 = i4 WITH protect, noconstant(0)
 SET lend_pos = size(sac_org->organizations,5)
 SET lend_pos2 = size(request->org_del_qual,5)
 ...
 DECLARE lorgaddcnt = i4 WITH protect, noconstant(0)
 DECLARE lorgdelcnt = i4 WITH protect, noconstant(0)
 IF (size(request->org_del_qual,5) != 0)
  SELECT INTO "nl:"
   f.parent_entity_id, f.filter_entity1_id
   ...
   HEAD REPORT
    stat = alterlist(temp_org_struct->organizations,10)
   DETAIL
    lpos = locateval(lindex_var,1,lend_pos2,f.filter_entity1_id,request->org_del_qual[lindex_var].
     organization_id)
    IF (lpos=0)
     lloop_counter += 1
     IF (mod(lloop_counter,10)=1
      AND lloop_counter != 1)
      stat = alterlist(temp_org_struct->organizations,(lloop_counter+ 9))
     ENDIF
     temp_org_struct->organizations[lloop_counter].organization_id = f.filter_entity1_id
    ENDIF
   FOOT REPORT
    FOR (lcounter = 1 TO size(request->org_add_qual,5))
      lloop_counter += 1
      IF (mod(lloop_counter,10)=1
       AND lloop_counter != 1)
       stat = alterlist(temp_org_struct->organizations,(lloop_counter+ 9))
      ENDIF
      temp_org_struct->organizations[lloop_counter].organization_id = request->org_add_qual[lcounter]
      .organization_id
    ENDFOR
    stat = alterlist(temp_org_struct->organizations,lloop_counter)
   WITH nocounter
  ;end select
 ENDIF
 IF (size(temp_org_struct->organizations,5) != 0)
  WHILE (lindex=0
   AND lcounter2 < size(temp_org_struct->organizations,5))
    SET lcounter2 += 1
    SET dsearch_item = temp_org_struct->organizations[lcounter2].organization_id
 ...
 SET index = 0
 SET freetext_updt_err = 0
 SET synoptic_updt_err = 0
 SET del_longtext_cnt = 0
 SET lorgaddcnt = cnvtint(size(request->org_add_qual,5))
 SET lorgdelcnt = cnvtint(size(request->org_del_qual,5))
 SET chg_freetext_flag = 0
 SET chg_synoptic_flag = 0
 SET stat = alterlist(reply->qual,request->param_add_cnt)
 SET stat = alterlist(aptemp->add,request->param_add_cnt)
 SET stat = alterlist(aptemp->chg,request->param_chg_cnt)
 SELECT INTO "nl:"
  cv.code_value
  FROM code_value cv
  WHERE trim(request->short_name)=trim(cv.display)
   AND (cv.code_set=request->code_set)
   AND (cv.code_value != request->query_cd)
   AND cv.active_ind=1
  ...
  IF (curqual > 0)
   ...
   IF (curqual > 0)
    SELECT INTO "nl:"
     ...
     FROM filter_entity_reltn f
     PLAN (f
      WHERE expand(lindex_var,1,lend_pos,f.filter_entity1_id,sac_org->organizations[lindex_var].
       organization_id)
       AND (f.parent_entity_id=reply->query_cd)
       AND cnvtdatetime(sysdate) BETWEEN f.beg_effective_dt_tm AND f.end_effective_dt_tm)
     WITH nocounter
    ;end select
    IF (curqual=0)
 ...
 IF ((request->change_cv_ind=1))
  SET stat = alterlist(pcs_upd_req->qual,1)
  SET pcs_upd_req->qual[1].code_value = request->query_cd
  SET pcs_upd_req->qual[1].code_set = request->code_set
  SET pcs_upd_req->qual[1].display = trim(request->short_name)
  SET pcs_upd_req->qual[1].description = trim(request->description)
  SET pcs_upd_req->qual[1].active_ind = 1
  SET pcs_upd_req->qual[1].cv_key = "1"
  SET pcs_upd_req->qual[1].collation_seq = 0
  SET pcs_upd_req->qual[1].definition = ""
  SET pcs_upd_req->qual[1].cdf_meaning = "PATHNET-AP"
  EXECUTE pcs_upd_code_values  WITH replace("REQUEST",pcs_upd_req), replace("REPLY",pcs_upd_rep)
  IF ((pcs_upd_rep->status_data.status != "S"))
   SET stat = alter(reply->status_data.subeventstatus,1)
   SET reply->status_data.subeventstatus[1].operationstatus = "F"
   SET reply->status_data.subeventstatus[1].operationname = "Update"
   SET reply->status_data.subeventstatus[1].targetobjectname = "code_value"
   SET reply->status_data.subeventstatus[1].targetobjectvalue = build("query_cd: ",request->query_cd)
   GO TO exit_script
  ENDIF
  FREE SET pcs_upd_req
  FREE SET pcs_upd_rep
 ENDIF
 IF ((request->param_del_cnt > 0))
  SELECT INTO "nl:"
   adqp.query_cd
   FROM ap_diag_query_param adqp,
    (dummyt d  WITH seq = value(request->param_del_cnt))
   PLAN (d)
    JOIN (adqp
    WHERE (adqp.query_cd=request->query_cd)
     AND (adqp.query_param_id=request->param_del_qual[d.seq].query_param_id))
   HEAD REPORT
    count1 = 0, tmp_cnt = 0
   DETAIL
    count1 += 1
   ...
   WITH nocounter
  ;end select
  IF ((count1 != request->param_del_cnt))
   SET stat = alter(reply->status_data.subeventstatus,1)
   SET reply->status_data.subeventstatus[1].operationstatus = "F"
   SET reply->status_data.subeventstatus[1].operationname = "Select"
   SET reply->status_data.subeventstatus[1].targetobjectname = "AP_DIAG_QUERY_PARAM"
   SET reply->status_data.subeventstatus[1].targetobjectvalue = build("query_cd: ",request->query_cd)
   ROLLBACK
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((request->param_chg_cnt > 0))
  SELECT INTO "nl:"
   adqp.query_cd
   FROM ap_diag_query_param adqp,
    (dummyt d  WITH seq = value(request->param_chg_cnt))
   PLAN (d)
    JOIN (adqp
    WHERE (adqp.query_param_id=request->param_chg_qual[d.seq].query_param_id))
   HEAD REPORT
    count1 = 0
   DETAIL
    count1 += 1, chg_updt_cnts[count1] = adqp.updt_cnt
    IF ((request->param_chg_qual[d.seq].beg_value_id > 0.0))
     IF ((request->param_chg_qual[d.seq].param_name IN (scase_casetype, scase_specimen,
     spatient_ethnicgroup, spatient_gender, spatient_race,
     spatient_species, spatient_military, scase_taskassay, scase_catalogcd)))
      aptemp->chg[d.seq].parent_entity_name = scode_value
     ELSEIF ((request->param_chg_qual[d.seq].param_name IN (scase_reqphys, scase_resppath,
     scase_respresi, scase_verid)))
      aptemp->chg[d.seq].parent_entity_name = sprsnl
     ELSEIF ((request->param_chg_qual[d.seq].param_name=scase_accprefix))
      aptemp->chg[d.seq].parent_entity_name = sap_prefix
     ELSEIF ((request->param_chg_qual[d.seq].param_name=scase_client))
      aptemp->chg[d.seq].parent_entity_name = sorganization
     ELSEIF ((request->param_chg_qual[d.seq].param_name=scase_queryresult))
      aptemp->chg[d.seq].parent_entity_name = sap_case_query
     ELSEIF ((request->param_chg_qual[d.seq].param_name IN (scriteria_diagcode1, scriteria_diagcode2,
     scriteria_diagcode3, scriteria_diagcode4, scriteria_diagcode5,
     scriteria_internal)))
      aptemp->chg[d.seq].parent_entity_name = snomenclature
     ENDIF
    ENDIF
    IF ((request->param_chg_qual[d.seq].param_name="CRITERIA_FREETEXT"))
     chg_freetext_flag = 1
    ENDIF
    IF ((request->param_chg_qual[d.seq].param_name="CRITERIA_SYNOPTIC"))
     chg_synoptic_flag = 1
    ENDIF
   WITH nocounter, forupdate(adqp)
  ;end select
  IF ((count1 != request->param_chg_cnt))
   SET stat = alter(reply->status_data.subeventstatus,1)
   SET reply->status_data.subeventstatus[1].operationstatus = "F"
   ...
   GO TO exit_script
  ENDIF
  IF (chg_freetext_flag=1)
   SELECT INTO "nl:"
    lt.long_text_id
    FROM long_text lt,
     (dummyt d  WITH seq = value(request->param_chg_cnt))
    PLAN (d
     WHERE (request->param_chg_qual[d.seq].freetext_long_text_id != 0.0))
     JOIN (lt
     WHERE (lt.long_text_id=request->param_chg_qual[d.seq].freetext_long_text_id))
    DETAIL
     IF ((lt.updt_cnt != request->param_chg_qual[d.seq].freetext_updt_cnt))
      freetext_updt_err = 1
     ENDIF
   ...
   ENDIF
  ENDIF
  IF (chg_synoptic_flag=1)
   SELECT INTO "nl:"
    lt.long_text_id
    FROM long_text lt,
     (dummyt d  WITH seq = value(request->param_chg_cnt))
    PLAN (d
     WHERE (request->param_chg_qual[d.seq].synoptic_ccl_long_text_id != 0.0))
     JOIN (lt
     WHERE (lt.long_text_id=request->param_chg_qual[d.seq].synoptic_ccl_long_text_id))
    DETAIL
     IF ((lt.updt_cnt != request->param_chg_qual[d.seq].synoptic_updt_cnt))
      synoptic_updt_err = 1
     ENDIF
    WITH nocounter, forupdate(lt)
   ;end select
   ...
   SELECT INTO "nl:"
    ...
    FROM long_text lt,
     (dummyt d  WITH seq = value(request->param_chg_cnt))
    PLAN (d
     WHERE (request->param_chg_qual[d.seq].synoptic_xml_long_text_id != 0.0))
     JOIN (lt
     WHERE (lt.long_text_id=request->param_chg_qual[d.seq].synoptic_xml_long_text_id))
    DETAIL
     IF ((lt.updt_cnt != request->param_chg_qual[d.seq].synoptic_updt_cnt))
      synoptic_updt_err = 1
     ENDIF
  ...
  ENDIF
 ENDIF
 FOR (cnt = 1 TO request->param_chg_cnt)
   IF ((request->param_chg_qual[cnt].updt_cnt != chg_updt_cnts[cnt]))
    SET stat = alter(reply->status_data.subeventstatus,1)
    SET reply->status_data.subeventstatus[1].operationstatus = "F"
    SET reply->status_data.subeventstatus[1].operationname = "VerifyChg"
    SET reply->status_data.subeventstatus[1].targetobjectname = "AP_DIAG_QUERY_PARAM"
 ...
 IF ((request->param_chg_cnt > 0))
  IF (chg_freetext_flag=1)
   UPDATE  FROM long_text lt,
     (dummyt d  WITH seq = value(request->param_chg_cnt))
    SET lt.long_text = request->param_chg_qual[d.seq].freetext_query, lt.updt_dt_tm = cnvtdatetime(
  ...
  IF (chg_synoptic_flag=1)
   UPDATE  FROM long_text lt,
     (dummyt d  WITH seq = value(request->param_chg_cnt))
    SET lt.long_text = request->param_chg_qual[d.seq].synoptic_ccl_query, lt.updt_dt_tm =
     cnvtdatetime(sysdate), lt.updt_id = reqinfo->updt_id,
     lt.updt_task = reqinfo->updt_task, lt.updt_applctx = reqinfo->updt_applctx, lt.updt_cnt = (lt
     .updt_cnt+ 1)
    PLAN (d
     WHERE (request->param_chg_qual[d.seq].synoptic_ccl_long_text_id != 0.0))
   ...
   UPDATE  FROM long_text lt,
     (dummyt d  WITH seq = value(request->param_chg_cnt))
    SET lt.long_text = request->param_chg_qual[d.seq].synoptic_xml_query, lt.updt_dt_tm =
     cnvtdatetime(sysdate), lt.updt_id = reqinfo->updt_id,
     lt.updt_task = reqinfo->updt_task, lt.updt_applctx = reqinfo->updt_applctx, lt.updt_cnt = (lt
     .updt_cnt+ 1)
    PLAN (d
  ...
  UPDATE  FROM ap_diag_query_param adqp,
    (dummyt d  WITH seq = value(request->param_chg_cnt))
   SET adqp.query_cd = request->query_cd, adqp.param_name = request->param_chg_qual[d.seq].param_name,
    adqp.criteria_type_flag = request->param_chg_qual[d.seq].criteria_type_flag,
    adqp.date_type_flag = request->param_chg_qual[d.seq].date_type_flag, adqp.beg_value_id = request
    ->param_chg_qual[d.seq].beg_value_id, adqp.beg_value_dt_tm = cnvtdatetime(request->
     param_chg_qual[d.seq].beg_value_dt_tm),
    adqp.end_value_id = request->param_chg_qual[d.seq].end_value_id, adqp.end_value_dt_tm =
    cnvtdatetime(request->param_chg_qual[d.seq].end_value_dt_tm), adqp.negation_ind = request->
    param_chg_qual[d.seq].negation_ind,
    adqp.source_vocabulary_cd = request->param_chg_qual[d.seq].source_vocabulary_cd, adqp.sequence =
    request->param_chg_qual[d.seq].sequence, adqp.freetext_query_flag = request->param_chg_qual[d.seq
    ].freetext_query_flag,
    adqp.synoptic_query_flag = request->param_chg_qual[d.seq].synoptic_query_flag, adqp.updt_dt_tm =
    cnvtdatetime(sysdate), adqp.updt_id = reqinfo->updt_id,
    adqp.updt_task = reqinfo->updt_task, adqp.updt_applctx = reqinfo->updt_applctx, adqp.updt_cnt = (
    adqp.updt_cnt+ 1),
    adqp.parent_entity_name = aptemp->chg[d.seq].parent_entity_name
   PLAN (d)
    JOIN (adqp
    WHERE (adqp.query_param_id=request->param_chg_qual[d.seq].query_param_id))
   WITH nocounter, outerjoin = d, dontexist
  ;end update
  IF ((curqual != request->param_chg_cnt))
   SET stat = alter(reply->status_data.subeventstatus,1)
   SET reply->status_data.subeventstatus[1].operationstatus = "F"
   SET reply->status_data.subeventstatus[1].operationname = "Update"
   SET reply->status_data.subeventstatus[1].targetobjectname = "AP_DIAG_QUERY_PARAM"
 ...
 IF ((request->param_del_cnt > 0))
  DELETE  FROM ap_diag_query_param adqp,
    (dummyt d  WITH seq = value(request->param_del_cnt))
   SET adqp.query_param_id = request->param_del_qual[d.seq].query_param_id
   PLAN (d)
    JOIN (adqp
    WHERE (adqp.query_param_id=request->param_del_qual[d.seq].query_param_id))
   WITH nocounter
  ;end delete
  IF ((curqual != request->param_del_cnt))
   SET stat = alter(reply->status_data.subeventstatus,1)
   SET reply->status_data.subeventstatus[1].operationstatus = "F"
   ...
   GO TO exit_script
  ENDIF
  SET del_longtext_cnt = size(temp_long_text->qual,5)
  IF (del_longtext_cnt != 0)
   DELETE  FROM long_text lt,
     (dummyt d  WITH seq = value(del_longtext_cnt))
  ...
  ENDIF
 ENDIF
 IF ((request->param_add_cnt > 0))
  FOR (index = 1 TO request->param_add_cnt)
    SELECT INTO "nl:"
     seq_nbr = seq(reference_seq,nextval)
     FROM dual
     DETAIL
      reply->qual[index].query_param_id = seq_nbr, reply->qual[index].sequence = request->
      param_add_qual[index].sequence
     WITH format, counter
    ;end select
    IF (curqual=0)
     SET stat = alter(reply->status_data.subeventstatus,1)
     ...
     GO TO exit_script
    ENDIF
    IF ((request->param_add_qual[index].param_name="CRITERIA_FREETEXT"))
     SELECT INTO "nl:"
      seq_nbr = seq(long_data_seq,nextval)
      FROM dual
      DETAIL
       reply->qual[index].freetext_long_text_id = seq_nbr
      WITH format, counter
     ;end select
     IF (curqual=0)
      SET stat = alter(reply->status_data.subeventstatus,1)
     ...
     INSERT  FROM long_text lt
      SET lt.long_text_id = reply->qual[index].freetext_long_text_id, lt.long_text = request->
       param_add_qual[index].freetext_query, lt.parent_entity_name = "AP_DIAG_QUERY_PARAM",
       lt.parent_entity_id = reply->qual[index].query_param_id, lt.active_ind = 1, lt
     ...
     ENDIF
    ENDIF
    IF ((request->param_add_qual[index].param_name="CRITERIA_SYNOPTIC"))
     SELECT INTO "nl:"
      seq_nbr = seq(long_data_seq,nextval)
      FROM dual
      DETAIL
       reply->qual[index].synoptic_ccl_long_text_id = seq_nbr
      WITH format, counter
     ;end select
     IF (curqual=0)
      SET stat = alter(reply->status_data.subeventstatus,1)
     ...
     SELECT INTO "nl:"
      ...
      FROM dual
      DETAIL
       reply->qual[index].synoptic_xml_long_text_id = seq_nbr
      WITH format, counter
     ;end select
     IF (curqual=0)
      SET stat = alter(reply->status_data.subeventstatus,1)
     ...
     INSERT  FROM long_text lt
      SET lt.long_text_id = reply->qual[index].synoptic_ccl_long_text_id, lt.long_text = request->
       param_add_qual[index].synoptic_ccl_query, lt.parent_entity_name = "AP_DIAG_QUERY_PARAM",
       lt.parent_entity_id = reply->qual[index].query_param_id, lt.active_ind = 1, lt
       .active_status_cd = reqdata->active_status_cd,
       lt.active_status_dt_tm = cnvtdatetime(sysdate), lt.active_status_prsnl_id = reqinfo->updt_id,
       lt.updt_dt_tm = cnvtdatetime(sysdate),
     ...
     INSERT  FROM long_text lt
      SET lt.long_text_id = reply->qual[index].synoptic_xml_long_text_id, lt.long_text = request->
       param_add_qual[index].synoptic_xml_query, lt.parent_entity_name = "AP_DIAG_QUERY_PARAM",
       lt.parent_entity_id = reply->qual[index].query_param_id, lt.active_ind = 1, lt
       .active_status_cd = reqdata->active_status_cd,
       lt.active_status_dt_tm = cnvtdatetime(sysdate), lt.active_status_prsnl_id = reqinfo->updt_id,
       lt.updt_dt_tm = cnvtdatetime(sysdate),
     ...
     ENDIF
    ENDIF
    IF ((request->param_add_qual[index].beg_value_id > 0.0))
     IF ((request->param_add_qual[index].param_name IN (scase_casetype, scase_specimen,
     spatient_ethnicgroup, spatient_gender, spatient_race,
     spatient_species, spatient_military, scase_taskassay, scase_catalogcd)))
      SET aptemp->add[index].parent_entity_name = scode_value
     ELSEIF ((request->param_add_qual[index].param_name IN (scase_reqphys, scase_resppath,
     scase_respresi, scase_verid)))
      SET aptemp->add[index].parent_entity_name = sprsnl
     ELSEIF ((request->param_add_qual[index].param_name=scase_accprefix))
      SET aptemp->add[index].parent_entity_name = sap_prefix
     ELSEIF ((request->param_add_qual[index].param_name=scase_client))
      SET aptemp->add[index].parent_entity_name = sorganization
     ELSEIF ((request->param_add_qual[index].param_name=scase_queryresult))
      SET aptemp->add[index].parent_entity_name = sap_case_query
     ELSEIF ((request->param_add_qual[index].param_name IN (scriteria_diagcode1, scriteria_diagcode2,
     scriteria_diagcode3, scriteria_diagcode4, scriteria_diagcode5,
     scriteria_internal)))
      SET aptemp->add[index].parent_entity_name = snomenclature
     ENDIF
    ENDIF
  ENDFOR
  INSERT  FROM ap_diag_query_param adqp,
    (dummyt d  WITH seq = value(request->param_add_cnt))
   SET adqp.query_cd = request->query_cd, adqp.query_param_id = reply->qual[d.seq].query_param_id,
    adqp.param_name = request->param_add_qual[d.seq].param_name,
    adqp.criteria_type_flag = request->param_add_qual[d.seq].criteria_type_flag, adqp.date_type_flag
     = request->param_add_qual[d.seq].date_type_flag, adqp.beg_value_id = request->param_add_qual[d
    .seq].beg_value_id,
    adqp.beg_value_dt_tm = cnvtdatetime(request->param_add_qual[d.seq].beg_value_dt_tm), adqp
    .end_value_id = request->param_add_qual[d.seq].end_value_id, adqp.end_value_dt_tm = cnvtdatetime(
     request->param_add_qual[d.seq].end_value_dt_tm),
    adqp.negation_ind = request->param_add_qual[d.seq].negation_ind, adqp.source_vocabulary_cd =
    request->param_add_qual[d.seq].source_vocabulary_cd, adqp.sequence = reply->qual[d.seq].sequence,
    adqp.freetext_query_flag = request->param_add_qual[d.seq].freetext_query_flag, adqp
    .freetext_long_text_id = reply->qual[d.seq].freetext_long_text_id, adqp.synoptic_query_flag =
    request->param_add_qual[d.seq].synoptic_query_flag,
    adqp.synoptic_ccl_long_text_id = reply->qual[d.seq].synoptic_ccl_long_text_id, adqp
    .synoptic_xml_long_text_id = reply->qual[d.seq].synoptic_xml_long_text_id, adqp.updt_dt_tm =
    cnvtdatetime(sysdate),
    adqp.updt_id = reqinfo->updt_id, adqp.updt_task = reqinfo->updt_task, adqp.updt_applctx = reqinfo
    ->updt_applctx,
    adqp.updt_cnt = 0, adqp.parent_entity_name = aptemp->add[d.seq].parent_entity_name
   PLAN (d)
    JOIN (adqp
    WHERE (adqp.query_param_id=reply->qual[d.seq].query_param_id))
   WITH nocounter, outerjoin = d, dontexist
  ;end insert
  IF ((curqual != request->param_add_cnt))
   SET stat = alter(reply->status_data.subeventstatus,1)
   SET reply->status_data.subeventstatus[1].operationstatus = "F"
   SET reply->status_data.subeventstatus[1].operationname = "Insert"
   SET reply->status_data.subeventstatus[1].targetobjectname = "AP_DIAG_QUERY_PARAM"
 ...
 FREE SET aptemp
 IF (lorgaddcnt > 0)
  SET stat = alterlist(filter_entity_req->filter_entity,lorgaddcnt)
  FOR (lcnt = 1 TO lorgaddcnt)
    ...
    SET filter_entity_req->filter_entity[lcnt].filter_entity1_id = request->org_add_qual[lcnt].
    organization_id
  ...
  IF ((filter_entity_rep->status_data.status != "S"))
   GO TO orgs_failed
  ENDIF
  SET stat = alterlist(reply->org_qual,lorgaddcnt)
  FOR (lcnt = 1 TO lorgaddcnt)
   SET reply->org_qual[lcnt].organization_id = request->org_add_qual[lcnt].organization_id
   SET reply->org_qual[lcnt].filter_entity_id = filter_entity_rep->filter_entity[lcnt].values[1].
   filter_entity_reltn_id
  ENDFOR
 ENDIF
 IF (lorgdelcnt > 0)
  SET stat = alterlist(filter_entity_req->filter_entity,lorgdelcnt)
  FOR (lcnt = 1 TO lorgdelcnt)
    SET stat = alterlist(filter_entity_req->filter_entity[lcnt].values,1)
    SET filter_entity_req->filter_entity[lcnt].filter_type_cd = dtemplatefiltertype
    SET filter_entity_req->filter_entity[lcnt].filter_entity1_name = "ORGANIZATION"
    SET filter_entity_req->filter_entity[lcnt].filter_entity2_name = ""
    ...
    SET filter_entity_req->filter_entity[lcnt].filter_entity4_name = ""
    SET filter_entity_req->filter_entity[lcnt].filter_entity5_name = ""
    SET filter_entity_req->filter_entity[lcnt].filter_entity1_id = request->org_del_qual[lcnt].
    organization_id
    SET filter_entity_req->filter_entity[lcnt].action_flag = ppr_action_ina
    SET filter_entity_req->filter_entity[lcnt].values[1].parent_entity_id = request->query_cd
    SET filter_entity_req->filter_entity[lcnt].values[1].parent_entity_name = "CODE_VALUE"
    SET filter_entity_req->filter_entity[lcnt].values[1].exclusion_filter_ind = 0
    SET filter_entity_req->filter_entity[lcnt].values[1].action_flag_values = ppr_action_ina
    SET filter_entity_req->filter_entity[lcnt].values[1].filter_entity_reltn_id = request->
    org_del_qual[lcnt].filter_entity_id
  ENDFOR
  EXECUTE ppr_ens_filter_ref  WITH replace("REQUEST",filter_entity_req), replace("REPLY",
   filter_entity_rep)
  IF ((filter_entity_rep->status_data.status != "S"))
   CALL echo("error executing ppr_ens_filter_ref")
   GO TO orgs_failed
  ENDIF
