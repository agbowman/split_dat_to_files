CREATE PROGRAM acm_add_person_name:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->person_name_qual[xref->add[index].idx].person_name_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(person_seq,nextval)
     FROM dual
     DETAIL
      acm_request->person_name_qual[xref->add[index].idx].person_name_id = cnvtreal(nextseqnum),
      reply->person_name_qual[xref->add[index].idx].person_name_id = cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->person_name_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->person_name_qual[xref->add[index].idx].person_name_id = acm_request->person_name_qual[
    xref->add[index].idx].person_name_id
   ENDIF
   SET reply->person_name_qual[xref->add[index].idx].status = 0
   IF ((acm_request->person_name_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->person_name_qual[xref->add[index].idx].beg_effective_dt_tm = cnvtdatetime(
     curdate,curtime3)
   ENDIF
   IF ((acm_request->person_name_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->person_name_qual[xref->add[index].idx].end_effective_dt_tm = cnvtdatetime(
     "31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   person_name p
  SET p.person_name_id = acm_request->person_name_qual[xref->add[d.seq].idx].person_name_id, p
   .beg_effective_dt_tm = cnvtdatetime(acm_request->person_name_qual[xref->add[d.seq].idx].
    beg_effective_dt_tm), p.contributor_system_cd =
   IF ((acm_request->person_name_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0)) acm_request
    ->person_name_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   ,
   p.end_effective_dt_tm = cnvtdatetime(acm_request->person_name_qual[xref->add[d.seq].idx].
    end_effective_dt_tm), p.name_degree = acm_request->person_name_qual[xref->add[d.seq].idx].
   name_degree, p.name_first = acm_request->person_name_qual[xref->add[d.seq].idx].name_first,
   p.name_first_key = trim(cnvtupper(cnvtalphanum(acm_request->person_name_qual[xref->add[d.seq].idx]
      .name_first)),3), p.name_format_cd = acm_request->person_name_qual[xref->add[d.seq].idx].
   name_format_cd, p.name_full = acm_request->person_name_qual[xref->add[d.seq].idx].name_full,
   p.name_initials = acm_request->person_name_qual[xref->add[d.seq].idx].name_initials, p.name_last
    = acm_request->person_name_qual[xref->add[d.seq].idx].name_last, p.name_last_key = trim(cnvtupper
    (cnvtalphanum(acm_request->person_name_qual[xref->add[d.seq].idx].name_last)),3),
   p.name_middle = acm_request->person_name_qual[xref->add[d.seq].idx].name_middle, p.name_middle_key
    = trim(cnvtupper(cnvtalphanum(acm_request->person_name_qual[xref->add[d.seq].idx].name_middle)),3
    ), p.name_prefix = acm_request->person_name_qual[xref->add[d.seq].idx].name_prefix,
   p.name_suffix = acm_request->person_name_qual[xref->add[d.seq].idx].name_suffix, p.name_title =
   acm_request->person_name_qual[xref->add[d.seq].idx].name_title, p.name_type_cd = acm_request->
   person_name_qual[xref->add[d.seq].idx].name_type_cd,
   p.person_id =
   IF ((acm_request->person_name_qual[xref->add[d.seq].idx].person_id > 0)) acm_request->
    person_name_qual[xref->add[d.seq].idx].person_id
   ELSE reply->person_qual[acm_request->person_name_qual[xref->add[d.seq].idx].person_idx].person_id
   ENDIF
   , p.source_identifier = acm_request->person_name_qual[xref->add[d.seq].idx].source_identifier, p
   .name_type_seq = acm_request->person_name_qual[xref->add[d.seq].idx].name_type_seq,
   p.data_status_cd =
   IF ((acm_request->person_name_qual[xref->add[d.seq].idx].data_status_cd > 0.0)) acm_request->
    person_name_qual[xref->add[d.seq].idx].data_status_cd
   ELSE reqdata->data_status_cd
   ENDIF
   , p.data_status_dt_tm = cnvtdatetime(curdate,curtime3), p.data_status_prsnl_id = reqinfo->updt_id,
   p.active_ind =
   IF ((((acm_request->person_name_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((acm_request
   ->person_name_qual[xref->add[d.seq].idx].active_status_cd=reqdata->active_status_cd))) ) 1
   ELSE 0
   ENDIF
   , p.active_status_cd =
   IF ((acm_request->person_name_qual[xref->add[d.seq].idx].active_status_cd > 0.0)) acm_request->
    person_name_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (p)
  WITH nocounter, status(reply->person_name_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->person_name_qual[xref->add[index].idx].status=0))
 ...
 ENDFOR
 IF (acm_hist_ind=1)
  EXECUTE acm_add_person_name_hist
