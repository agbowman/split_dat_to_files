CREATE PROGRAM dm2_replicate_cleanup:dba
 ...
 END ;Subroutine
 DECLARE drc_env_name = vc WITH protect, noconstant("")
 DECLARE drc_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE drc_dbase_name = vc WITH protect, noconstant("")
 DECLARE drc_dbase_create_dt_tm = dq8 WITH protect, noconstant(0.0)
 DECLARE drc_drop_invalid_tables(ddit_env_name=vc,ddit_dbase_name=vc) = i2
 IF (check_logfile("dm2_replicate_clean",".log","dm2_replicate_cleanup LOGFILE")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Beginning dm2_replicate_cleanup"
 CALL disp_msg("",dm_err->logfile,0)
 IF (currdbuser != "V500")
  SET dm_err->err_ind = 1
  SET dm_err->eproc = "Determining current database user."
  SET dm_err->emsg = "This can only be executed under the V500 user."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if there are dm_tables_doc rows present"
 SELECT INTO "nl:"
  FROM dm_tables_doc dtd
  WITH maxqual(dtd,1)
 ;end select
 ...
 IF (curqual < 1)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "There are no rows present in dm_tables_doc.  Please check your ADMIN database."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if there are proper oracle user dm_info rows present."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE "DM2_ORACLE_USER"=di.info_domain
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Not all of the valid oracle user rows are present in dm_info."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Determining if there are proper cerner user dm_info rows present."
 SELECT INTO "nl:"
  FROM dm_info di
  WHERE "DM2_CERNER_USER"=di.info_domain
  WITH maxqual(di,1), nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ...
 IF (curqual=0)
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Not all of the valid cerner user rows are present in dm_info."
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   CASE (di.info_name)
    OF "DM_ENV_NAME":
     drc_env_name = di.info_char
    OF "DM_ENV_ID":
     drc_env_id = di.info_number
   ENDCASE
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_script
 ENDIF
 IF (drr_get_dbase_created_date(drc_dbase_create_dt_tm)=0)
  GO TO exit_script
 ENDIF
 SET drc_dbase_name = currdbname
 IF ((drr_clin_copy_data->process != "RESTORE")
  ...
  IF ((drer_email_list->email_cnt > 0))
   ...
   SET drer_email_det->status = "PAUSED"
   SET drer_email_det->status_dt_tm = cnvtdatetime(curdate,curtime3)
   SET drer_email_det->step = "Target domain confirmation"
   SET drer_email_det->email_level = 1
   SET drer_email_det->logfile = dm_err->logfile
   SET drer_email_det->err_ind = dm_err->err_ind
   SET drer_email_det->eproc = dm_err->eproc
   ...
   CALL drer_add_body_text(concat(
     "Prompt for user to confirm Target is NOT Production domain was displayed at ",format(
      drer_email_det->status_dt_tm,";;q")),1)
   CALL drer_add_body_text(concat("User Action : Return to dm2_domain_maint main session and ",
     "Enter 'Y' or 'N' to continue."),0)
  ...
  CALL video(n)
  CALL text(4,2,"Subsequent processing should NOT be performed against a production database.")
  CALL text(5,2,"Please confirm that the following database represents a NON-production domain.")
  CALL text(7,2,concat("Environment ID: ",build(drc_env_id)))
  CALL text(8,2,concat("Environment Name: ",drc_env_name))
  CALL text(9,2,concat("Database Name: ",drc_dbase_name))
  CALL text(10,2,concat("Database Create Date: ",format(drc_dbase_create_dt_tm,";;Q")))
  CALL text(13,2,"Is this a Production domain? (Y/N):")
  CALL accept(13,38,"A;CU","Y"
   WHERE curaccept IN ("Y", "N"))
  CALL clear(1,1)
  SET message = nowindow
  CASE (curaccept)
   OF "Y":
    SET dm_err->err_ind = 1
    SET dm_err->eproc = "Determining if current domain is a production domain."
    SET dm_err->emsg = "DM2_REPLICATE_CLEANUP should not be executed against a production domain."
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    GO TO exit_script
   OF "N":
    IF (drc_drop_invalid_tables(drc_env_name,drc_dbase_name)=0)
     GO TO exit_script
    ENDIF
  ENDCASE
 ELSE
  IF (validate(drrr_responsefile_in_use,0)=1)
   ...
   ENDIF
  ENDIF
  IF (drr_process_invalid_tables(null)=0)
   GO TO exit_script
  ENDIF
 ENDIF
 IF ((drr_cleanup_drop_list->cnt=0))
  SET dm_err->eproc = "There are no objects to clean up."
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 GO TO exit_script
 SUBROUTINE drc_drop_invalid_tables(ddit_env_name,ddit_dbase_name)
   DECLARE ddit_continue = i2 WITH protect, noconstant(0)
   IF (drr_get_invalid_tables_list(null)=0)
    RETURN(0)
   ENDIF
   IF ((drr_cleanup_drop_list->cnt > 0))
    IF ((dm2_install_schema->process_option="CLIN COPY")
     ...
     SET drer_email_det->status = "PAUSED"
     SET drer_email_det->status_dt_tm = cnvtdatetime(curdate,curtime3)
     SET drer_email_det->step = "Invalid tables report"
     SET drer_email_det->email_level = 1
     SET drer_email_det->logfile = dm_err->logfile
     SET drer_email_det->err_ind = dm_err->err_ind
     SET drer_email_det->eproc = dm_err->eproc
     SET drer_email_det->emsg = dm_err->emsg
     SET drer_email_det->user_action = dm_err->user_action
     CALL drer_add_body_text(concat("Invalid tables report was displayed at ",format(drer_email_det->
        status_dt_tm,";;q")),1)
     CALL drer_add_body_text(concat("User Action : Return to dm2_domain_maint main session and ",
       "review Invalid tables report displayed on the screen.  Press <enter> to continue."),0)
     CALL drer_add_body_text(concat("Log file name is ccluserdir: ",dm_err->logfile),0)
     IF (drer_compose_email(null)=1)
      CALL drer_send_email(drer_email_det->subject,drer_email_det->file_name,drer_email_det->
       email_level)
    ...
    ENDIF
    SET ddit_continue = 1
    WHILE (ddit_continue=1)
      SET message = window
      CALL clear(1,1)
      CALL box(1,1,24,131)
      CALL text(2,2,fillstring(126,"*"))
      CALL text(3,2,concat("Environment Name: ",ddit_env_name))
      CALL text(3,40,concat("Database Name: ",ddit_dbase_name))
      CALL text(4,2,fillstring(126,"*"))
      CALL text(5,2,"All exports should have been completed at this time.")
      CALL text(6,2,"Do you wish to proceed with dropping the tables?")
      CALL text(8,2,"Answering Y will cause the tables to be dropped.")
      CALL text(9,2,fillstring(126,"*"))
      CALL text(11,2,"(Y)es, (V)iew Drop Report: ")
      CALL accept(11,36,"A;CU","Y"
       WHERE curaccept IN ("Y", "V"))
      CALL clear(1,1)
      SET message = nowindow
      CASE (curaccept)
       OF "Y":
 ...
 END ;Subroutine
#exit_script
 SET dm_err->eproc = "Ending dm2_replicate_cleanup."
 CALL final_disp_msg("dm2_replicate_clean")
END GO
