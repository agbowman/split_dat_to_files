CREATE PROGRAM dm_ocd_chk_tspace:dba
 ...
 SELECT INTO "nl:"
  ...
  WHERE e.environment_name=env_name
   AND e.schema_version=f.schema_version
  DETAIL
 ...
 SELECT INTO "nl:"
  dt.table_name, dt.tablespace_name
  FROM dm_afd_tables dt,
   dm_alpha_features_env da
  WHERE dt.alpha_feature_nbr=da.alpha_feature_nbr
   AND da.alpha_feature_nbr=ocd_number
  ...
  DETAIL
   ...
   fslist->count].table_name = dt.table_name,
   fslist->qual[fslist->count].tablespace_name = dt.tablespace_name, fslist->qual[fslist->count].
   new_table_ind = 1, fslist->qual[fslist->count].new_tbspace_ind = 1
  WITH nocounter
 ;end select
 IF ((fslist->count=0))
  GO TO end_program
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   ...
   WHERE a.alpha_feature_nbr=ocd_number
    AND (a.table_name=fslist->qual[d.seq].table_name))
  HEAD a.table_name
   knt = 0
  DETAIL
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d)
   JOIN (a
   WHERE (a.table_name=fslist->qual[d.seq].table_name)
    AND a.alpha_feature_nbr=ocd_number)
  DETAIL
   tspace_exists = 0
   FOR (z = 1 TO tbcount)
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   WHERE (di.index_name=fslist->qual[d.seq].index_name[d2.seq].iname)
    AND di.alpha_feature_nbr=ocd_number)
   JOIN (dic
   WHERE di.index_name=dic.index_name
    AND di.alpha_feature_nbr=dic.alpha_feature_nbr)
   JOIN (dc
   WHERE dc.table_name=di.table_name
    AND dic.column_name=dc.column_name
    AND dc.alpha_feature_nbr=di.alpha_feature_nbr)
   JOIN (d3)
   JOIN (so
 ...
 SELECT INTO "nl:"
  ...
  PLAN (d
   ...
   WHERE (dt.table_name=fslist->qual[d.seq].table_name)
    AND dt.alpha_feature_nbr=ocd_number)
   JOIN (dc
   WHERE dc.table_name=dt.table_name
    AND dc.alpha_feature_nbr=dt.alpha_feature_nbr
    AND dc.nullable="N")
   JOIN (d3)
   JOIN (a
