CREATE PROGRAM dm2_alter_index_mon:dba
 ...
 SUBROUTINE ddf_context_lock_maint(dclm_mode,dclm_process,dclm_process_code,dclm_owner,
  dclm_table_name,dclm_object_id,dclm_lock_ind)
   ...
   RETURN(1)
 END ;Subroutine
 FREE RECORD daim_object_list
 RECORD daim_object_list(
   1 owner_cnt = i4
   1 owner[*]
     2 tbl_owner = vc
     2 tbl_cnt = i4
 ...
 SET daim_object_list->owner_cnt = 0
 DECLARE daim_owner = vc WITH protect, noconstant( $1)
 DECLARE daim_tname = vc WITH protect, noconstant( $2)
 DECLARE daim_idxname = vc WITH protect, noconstant( $3)
 DECLARE daim_status = vc WITH protect, noconstant("")
 DECLARE daim_owr_cnt = i2 WITH protect, noconstant(0)
 DECLARE daim_tbl_cnt = i4 WITH protect, noconstant(0)
 DECLARE daim_ind_cnt = i4 WITH protect, noconstant(0)
 DECLARE daim_ocnt = i2 WITH protect, noconstant(0)
 DECLARE daim_tcnt = i4 WITH protect, noconstant(0)
 DECLARE daim_icnt = i4 WITH protect, noconstant(0)
 DECLARE daim_blocking_str = vc WITH protect, noconstant("")
 DECLARE daim_in_parse_ind = i2 WITH protect, noconstant(0)
 DECLARE daim_loop_var = i2 WITH protect, noconstant(0)
 DECLARE daim_cmd_rscbsy_retry_cnt = i4 WITH protect, noconstant(0)
 DECLARE daim_cmd_rscbsy_error = i2 WITH protect, noconstant(0)
 DECLARE daim_str = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_atr_idx_monrp",".log","dm2_alter_index_mon LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_alter_index_mon"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dm_err->eproc = "Validating and storing input parameter"
 SET daim_status = cnvtupper( $4)
 IF ( NOT (daim_status IN ("MONITORING", "NOMONITORING")))
  SET dm_err->err_ind = 1
  SET dm_err->emsg = "Invalid 4th input parameter - should be 'MONITORING' or 'NOMONITORING'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "IndexMon: Get current Oracle version"
 IF ((dm_err->debug_flag > 0))
  CALL disp_msg("",dm_err->logfile,0)
 ENDIF
 IF (dm2_get_rdbms_version(null)=0)
 ...
 ENDIF
 IF ((dm2_rdbms_version->level1 < 11))
  SET dm_err->eproc = "IndexMon: Auto-successing due to oracle version < 11"
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_program
 ENDIF
 ...
 IF ((dm_err->err_ind > 0))
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "IndexMon: Loading data from dba_indexes into memory."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_indexes di
  WHERE di.table_owner=patstring(daim_owner)
   AND di.table_name=patstring(daim_tname)
   AND di.index_name=patstring(daim_idxname)
   AND di.index_type IN ("NORMAL", "FUNCTION-BASED NORMAL", "FUNCTION-BASED BITMAP", "BITMAP")
   AND  NOT ( EXISTS (
  (SELECT
   "x"
   FROM dm_info dmi
   WHERE dmi.info_name=di.owner
    AND dmi.info_domain="DM2_ORACLE_USER")))
  ORDER BY di.table_owner, di.table_name
  HEAD REPORT
   daim_ocnt = 0, daim_tcnt = 0
  HEAD di.table_owner
   daim_ocnt = (daim_ocnt+ 1), stat = alterlist(daim_object_list->owner,daim_ocnt), daim_object_list
   ->owner[daim_ocnt].tbl_owner = di.table_owner,
   daim_icnt = 0
  HEAD di.table_name
   daim_tcnt = (daim_tcnt+ 1), stat = alterlist(daim_object_list->owner[daim_ocnt].tbl,daim_tcnt),
   daim_object_list->owner[daim_ocnt].tbl[daim_tcnt].tbl_name = di.table_name
  DETAIL
   daim_icnt = (daim_icnt+ 1), stat = alterlist(daim_object_list->owner[daim_ocnt].tbl[daim_tcnt].ind,
    daim_icnt), daim_object_list->owner[daim_ocnt].tbl[daim_tcnt].ind[daim_icnt].ind_name = di
   .index_name,
   daim_object_list->owner[daim_ocnt].tbl[daim_tcnt].ind[daim_icnt].ind_owner = di.owner
  FOOT  di.table_name
   stat = alterlist(daim_object_list->owner[daim_ocnt].tbl[daim_tcnt].ind,daim_icnt),
   daim_object_list->owner[daim_ocnt].tbl[daim_tcnt].ind_cnt = daim_icnt, daim_icnt = 0
  FOOT  di.table_owner
   stat = alterlist(daim_object_list->owner[daim_ocnt].tbl,daim_tcnt), daim_object_list->owner[
   daim_ocnt].tbl_cnt = daim_tcnt, daim_tcnt = 0
  FOOT REPORT
   daim_object_list->owner_cnt = daim_ocnt, stat = alterlist(daim_object_list->owner,daim_object_list
    ->owner_cnt)
  WITH nocounter
 ;end select
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(daim_object_list)
 ENDIF
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 FOR (daim_owr_cnt = 1 TO daim_object_list->owner_cnt)
   FOR (daim_tbl_cnt = 1 TO daim_object_list->owner[daim_owr_cnt].tbl_cnt)
     SET daim_loop_var = 0
     SET daim_in_parse_ind = 1
     WHILE (daim_in_parse_ind=1
      AND daim_loop_var < 10)
       IF (ddf_check_in_parse(daim_object_list->owner[daim_owr_cnt].tbl_owner,daim_object_list->
        owner[daim_owr_cnt].tbl[daim_tbl_cnt].tbl_name,daim_in_parse_ind,daim_blocking_str)=0)
        GO TO exit_program
       ENDIF
       SET daim_loop_var = (daim_loop_var+ 1)
       IF (daim_in_parse_ind=1)
        SET dm_err->eproc = concat("Pausing 30 seconds due to hard parse event detected on ",
         daim_object_list->owner[daim_owr_cnt].tbl_owner,".",daim_object_list->owner[daim_owr_cnt].
         tbl[daim_tbl_cnt].tbl_name)
        CALL disp_msg(" ",dm_err->logfile,0)
        CALL pause(30)
       ENDIF
       IF (daim_loop_var=10
        AND daim_in_parse_ind=1)
        SET dm_err->eproc = "Validating whether the table is part of an long parsing query."
        SET dm_err->emsg = build(
         "Wait time exceeded (5 minutes) due to a long parsing query against table ",daim_object_list
         ->owner[daim_owr_cnt].tbl[daim_tbl_cnt].tbl_name," ",daim_blocking_str)
        SET dm_err->user_action = concat(
         "Please restart the process after waiting 15 minutes for the long ",
         "parsing query to finish.")
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        GO TO exit_program
       ENDIF
     ENDWHILE
     FOR (daim_ind_cnt = 1 TO daim_object_list->owner[daim_owr_cnt].tbl[daim_tbl_cnt].ind_cnt)
       SET dm_err->eproc = concat("Altering Index monitoring to ",daim_status," for ",
        daim_object_list->owner[daim_owr_cnt].tbl[daim_tbl_cnt].ind[daim_ind_cnt].ind_owner,".",
        daim_object_list->owner[daim_owr_cnt].tbl[daim_tbl_cnt].ind[daim_ind_cnt].ind_name)
       CALL disp_msg(" ",dm_err->logfile,0)
       SET daim_str = concat('rdb asis (^alter index "',daim_object_list->owner[daim_owr_cnt].tbl[
        daim_tbl_cnt].ind[daim_ind_cnt].ind_owner,'"."',daim_object_list->owner[daim_owr_cnt].tbl[
        daim_tbl_cnt].ind[daim_ind_cnt].ind_name,'" ',
        daim_status," USAGE ^) GO")
       SET daim_cmd_rscbsy_retry_cnt = 0
       SET daim_cmd_rscbsy_error = 1
       WHILE (daim_cmd_rscbsy_error=1
        AND daim_cmd_rscbsy_retry_cnt < cnvtint(dm2_db_options->resource_busy_maxretry))
         SET dm_err->err_ind = 0
         SET daim_cmd_rscbsy_error = 0
         EXECUTE dm2_exec_child_cmd "CMD", daim_str
         ...
         IF ((dm_err->err_ind > 0))
          ...
          ENDIF
         ENDIF
         IF (daim_cmd_rscbsy_error=1)
          CALL pause(3)
          SET daim_cmd_rscbsy_retry_cnt = (daim_cmd_rscbsy_retry_cnt+ 1)
         ENDIF
       ENDWHILE
       SET dm_err->eproc = "DM2_ALTER_INDEX_MON is checking for Resource Busy retry "
       SET dm_err->emsg =
       "DM2_ALTER_INDEX_MON has reached the maximum count for resource busy retry count."
       IF (daim_cmd_rscbsy_error=1)
        CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
        GO TO exit_program
       ENDIF
     ENDFOR
     SET dm_err->eproc = concat(
      "IndexMon: Pausing 30 seconds to allow invalidated Oracle SQL cursors for table ",
      daim_object_list->owner[daim_owr_cnt].tbl_owner,".",daim_object_list->owner[daim_owr_cnt].tbl[
      daim_tbl_cnt].tbl_name," to reparse.")
     CALL disp_msg(" ",dm_err->logfile,0)
     CALL pause(30)
   ENDFOR
 ENDFOR
#exit_program
 SET message = nowindow
 SET dm_err->eproc = "Ending dm2_alter_index_mon"
 CALL final_disp_msg("dm2_atr_idx_monrp")
END GO
