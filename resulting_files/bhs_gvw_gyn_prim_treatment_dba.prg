CREATE PROGRAM bhs_gvw_gyn_prim_treatment:dba
 FREE RECORD m_rec
 RECORD m_rec(
   1 l_cnt = i4
   1 qual[*]
     2 s_gyn_pri_treatment = vc
     2 f_gyn_pri_date = f8
     2 s_gyn_pri_date = vc
     2 f_gyn_pri_end_date = f8
     2 s_gyn_pri_end_date = vc
 ) WITH protect
 DECLARE mf_gynprimarytreatment_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "GYNPRIMARYTREATMENT"))
 DECLARE mf_gynprimarytreatmentdate_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "GYNPRIMARYTREATMENTDATE"))
 DECLARE mf_gynprimarytreatmentenddate_cd = f8 WITH protect, constant(uar_get_code_by("DISPLAYKEY",72,
   "GYNPRIMARYTREATMENTENDDATE"))
 ...
 SELECT INTO "nl:"
  FROM clinical_event ce,
   ce_date_result cdr
  PLAN (ce
   WHERE ce.encntr_id=mf_enc_id
    AND ce.event_cd IN (mf_gynprimarytreatment_cd, mf_gynprimarytreatmentdate_cd,
   mf_gynprimarytreatmentenddate_cd)
  ...
  HEAD REPORT
   ml_cnt = 0, ml_cnt += 1, m_rec->l_cnt = ml_cnt,
   stat = alterlist(m_rec->qual,ml_cnt)
  HEAD ce.event_cd
   CASE (ce.event_cd)
    OF mf_gynprimarytreatmentdate_cd:
     m_rec->qual[ml_cnt].s_gyn_pri_date = format(cdr.result_dt_tm,"mm/dd/yy;;D"),m_rec->qual[ml_cnt].
     f_gyn_pri_date = cdr.result_dt_tm
    OF mf_gynprimarytreatmentenddate_cd:
     m_rec->qual[ml_cnt].s_gyn_pri_end_date = format(cdr.result_dt_tm,"mm/dd/yy;;D")
    OF mf_gynprimarytreatment_cd:
     m_rec->qual[ml_cnt].s_gyn_pri_treatment = trim(ce.result_val)
   ENDCASE
  WITH nocounter
 ;end select
 IF (ml_cnt < 1)
 ...
 IF ((reqinfo->updt_task=3202004))
  SET ms_text_temp = concat("\trowd\trgaph30\cellx4000\cellx6000\cellx8000\intbl",ms_wb,
   "{Gyn Primary Treatment} ","\cell ",ms_wb,
   "{Gyn Primary Treatment Date} ","\cell ",ms_wb,"{Gyn Primary Treatment End Date} ","\cell ",
   "\row ")
  SET reply->text = concat(reply->text," ",ms_text_temp)
 ENDIF
 SELECT INTO "nl:"
  gyn_pri_date = m_rec->qual[d.seq].f_gyn_pri_date
  FROM (dummyt d  WITH seq = m_rec->l_cnt)
  ORDER BY gyn_pri_date DESC
  DETAIL
   IF ((reqinfo->updt_task=3202004))
    ms_text_temp = concat("\trowd\trgaph30\cellx4000\cellx6000\cellx8000\intbl",ms_wr,"{",trim(m_rec
      ->qual[d.seq].s_gyn_pri_treatment),"} ",
     "\cell ",ms_wr,"{",trim(m_rec->qual[d.seq].s_gyn_pri_date),"} ",
     "\cell ",ms_wr,"{",trim(m_rec->qual[d.seq].s_gyn_pri_end_date),"} ",
     "\cell ","\row "), reply->text = concat(reply->text," ",ms_text_temp)
   ELSE
    ms_text_temp = concat(ms_wb,ms_hi,"{Gyn Primary Treatment: } ",ms_wr,"{",
     trim(m_rec->qual[d.seq].s_gyn_pri_treatment),"} ",ms_reol,ms_wb,
     "{Gyn Primary Treatment Date: } ",
     ms_wr,"{",trim(m_rec->qual[d.seq].s_gyn_pri_date),"} ",ms_reol,
     ms_wb,"{Gyn Primary Treatment End Date: } ",ms_wr,"{",trim(m_rec->qual[d.seq].s_gyn_pri_end_date
      ),
