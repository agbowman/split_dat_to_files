CREATE PROGRAM dm_schema_estimate_op_log2:dba
...
#2000_process
 IF (dm_schema_log->op_id)
  IF (size(trim(dm_schema_log->file_name,3)))
   SELECT INTO value(dm_schema_log->file_name)
    ...
    DETAIL
     row + 1,
     CALL print(concat("dm_schema_actual_stop2 ",trim(cnvtstring(dm_schema_log->op_id),3)," go")),
     row + 2
 ...
 IF (eol_data->rows)
  ...
  SELECT INTO "nl:"
   ...
   WHERE i.info_domain=concat("SCHEMA BENCHMARK ",dm_schema_log->operation)
    ...
    AND i.info_number > 0.0
   DETAIL
    duration = (eol_data->rows * i.info_number)
   WITH nocounter
  ;end select
  IF ( NOT (curqual))
   SELECT INTO "nl:"
 ...
 INSERT  FROM dm_schema_op_log o
  SET o.op_id = dm_schema_log->op_id, o.run_id = dm_schema_log->run_id, o.begin_dt_tm = null,
   ...
   eol_data->rows,
   o.obj_name = dm_schema_log->object_name, o.filename = dm_schema_log->file_name, o.status = null
  WITH nocounter
 ;end insert
 IF ( NOT (curqual))
 ...
 IF (size(trim(dm_schema_log->file_name,3)))
  SELECT INTO value(dm_schema_log->file_name)
   ...
   DETAIL
    row + 1,
    CALL print(concat("dm_schema_actual_start2 ",trim(cnvtstring(dm_schema_log->op_id),3)," go")),
    row + 2
   WITH nocounter, maxcol = 150, maxrow = 2,
    format = variable, noformfeed, append
  ;end select
