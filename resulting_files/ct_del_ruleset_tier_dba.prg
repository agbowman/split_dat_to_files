CREATE PROGRAM ct_del_ruleset_tier:dba
 IF ("Z"=validate(ct_del_ruleset_tier_vrsn,"Z"))
  DECLARE ct_del_ruleset_tier_vrsn = vc WITH noconstant("45958.002")
 ENDIF
 SET ct_del_ruleset_tier_vrsn = "45958.002"
 ...
 CALL del_ct_ruleset_tier(action_begin,action_end)
 IF (failed != false)
  GO TO check_error
 ENDIF
#check_error
 ...
 ENDIF
 GO TO end_program
 SUBROUTINE del_ct_ruleset_tier(del_begin,del_end)
   FOR (x = del_begin TO del_end)
     ...
     IF ((request->ct_ruleset_tier[x].active_status_cd=0))
      SET stat = uar_get_meaning_by_codeset(48,nullterm(cdf_meaning),1,active_code)
     ENDIF
     UPDATE  FROM ct_ruleset_tier c
      SET c.active_ind = false, c.active_status_cd = nullcheck(active_code,request->ct_ruleset_tier[x
        ].active_status_cd,
        IF ((request->ct_ruleset_tier[x].active_status_cd=0)) 0
        ELSE 1
        ENDIF
