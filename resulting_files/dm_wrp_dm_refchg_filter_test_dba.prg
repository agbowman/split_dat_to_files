CREATE PROGRAM dm_wrp_dm_refchg_filter_test:dba
 ...
 SET readme_data->message = "Readme Failed: Starting script dm_wrp_dm_refchg_filter_test.prg..."
 DECLARE dwdrf_env_id = f8 WITH protect, noconstant(0.0)
 DECLARE dwdrf_oe_ind = i2 WITH protect, noconstant(0)
 IF (check_logfile("dm_wrp_dm_fltr",".log","dm_wrp_dm_refchg_filter_test LOG FILE...") != 1)
  SET dm_err->err_ind = 1
 ...
 ENDIF
 IF (dwdrf_oe_ind=0)
  EXECUTE dm_dbimport "cer_install:dm_refchg_filter_test.csv", "dm_imp_dm_refchg_filter_test", 100
 ELSE
