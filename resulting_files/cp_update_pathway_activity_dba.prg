CREATE PROGRAM cp_update_pathway_activity:dba
 PROMPT
  ...
  "Node ID" = 0.0,
  "Component ID" = 0.0,
  "Clinical Trial Indicator" = - (1),
  "Off Pathway Indicator" = - (1),
  "Reason Code" = 0.0,
  "Reason Freetext" = ""
  WITH outdev, pid, eid,
  prsnlid, pwid, pwinid,
  nodeid, componentid, clin_ind,
  off_ind, reason_cd, reason_freetext
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 DECLARE script_begin_curtime3 = dq8 WITH constant(curtime3), protect
 DECLARE curoffpathwayind = i4 WITH noconstant(0), protect
 DECLARE curclintrialind = i4 WITH noconstant(0), protect
 DECLARE pathwayactivityid = f8 WITH noconstant(0), protect
 DECLARE treatmentlinecd = f8 WITH noconstant(0), protect
 DECLARE offpathwayind = i4 WITH noconstant( $OFF_IND), protect
 DECLARE clintrialind = i4 WITH noconstant( $CLIN_IND), protect
 DECLARE main(null) = null WITH protect
 DECLARE getpathwayinfo(null) = f8 WITH protect
 DECLARE updatepathwayactivity(null) = null WITH protect
 DECLARE addpathwayaction(null) = null WITH protect
 DECLARE addactiondetails(null) = null WITH protect
 ...
 ENDIF
 SUBROUTINE getpathwayinfo(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   DECLARE retval = f8 WITH noconstant(0.0), protect
   CALL log_message("In getPathwayInfo()",log_level_debug)
   SELECT INTO "nl:"
    FROM cp_pathway_activity cpa
    PLAN (cpa
     WHERE (cpa.person_id= $PID)
      AND (cpa.cp_pathway_id= $PWID)
      AND (cpa.pathway_instance_id= $PWINID)
      AND cpa.end_effective_dt_tm > cnvtdatetime(sysdate))
    HEAD REPORT
     pathwayactivityid = cpa.cp_pathway_activity_id, curoffpathwayind = cpa.off_pathway_ind,
     curclintrialind = cpa.clinical_trial_ind
    WITH nocounter
   ;end select
   CALL error_and_zero_check_rec(curqual,"getPathwayInfo","Could not rerieve Activity ID",1,1,
    pathway_activity_reply)
   IF ((offpathwayind=- (1)))
    SET offpathwayind = curoffpathwayind
   ENDIF
   IF ((clintrialind=- (1)))
    SET clintrialind = curclintrialind
   ENDIF
   IF (( $NODEID > 0.0))
    SELECT INTO "NL:"
     FROM cp_node cn
     ...
     HEAD REPORT
      treatmentlinecd = cn.treatment_line_cd
     WITH nocounter
    ;end select
    CALL error_and_zero_check_rec(curqual,"getPathwayInfo","Could not rerieve Activity ID",1,0,
     pathway_activity_reply)
   ENDIF
   CALL log_message(build("Exiting getPathwayInfo(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE addpathwayaction(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   CALL log_message("In addPathwayAction()",log_level_debug)
   DECLARE lactionreqsize = i4 WITH noconstant(0), protect
   ...
   IF (offpathwayind != curoffpathwayind)
    SET lactionreqsize = size(actionreq->actions,5)
    SET lactionreqsize += 1
    SET stat = alterlist(actionreq->actions,lactionreqsize)
    SET actionreq->actions[lactionreqsize].encntr_id =  $EID
    SET actionreq->actions[lactionreqsize].pathway_instance_id =  $PWINID
    SET actionreq->actions[lactionreqsize].cp_node_id =  $NODEID
    SET actionreq->actions[lactionreqsize].cp_component_id =  $COMPONENTID
    SET actionreq->actions[lactionreqsize].prsnl_id =  $PRSNLID
    SET actionreq->actions[lactionreqsize].treatment_line_cd = treatmentlinecd
    SET actionreq->actions[lactionreqsize].action_dt_tm = cnvtdatetime(sysdate)
    IF (offpathwayind=1)
     SET actionreq->actions[lactionreqsize].action_type_mean = "OFFPATHWAY"
    ELSE
     SET actionreq->actions[lactionreqsize].action_type_mean = "ONPATHWAY"
    ENDIF
    CALL addactiondetails(null)
   ELSEIF (clintrialind != curclintrialind)
    SET lactionreqsize = size(actionreq->actions,5)
    SET lactionreqsize += 1
    SET stat = alterlist(actionreq->actions,lactionreqsize)
    SET actionreq->actions[lactionreqsize].encntr_id =  $EID
    ...
    SET actionreq->actions[lactionreqsize].action_dt_tm = cnvtdatetime(sysdate)
    IF (clintrialind=1)
     SET actionreq->actions[lactionreqsize].action_type_mean = "ONCLINTRIAL"
    ELSE
     SET actionreq->actions[lactionreqsize].action_type_mean = "OFFCLINTRIAL"
    ENDIF
    CALL addactiondetails(null)
   ENDIF
   IF (lactionreqsize > 0)
    CALL echorecord(actionreq)
    EXECUTE cp_add_pathway_action "NOFORMS"
   ENDIF
   CALL log_message(build("Exiting addPathwayAction(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE addactiondetails(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   CALL log_message("In addActionDetails()",log_level_debug)
   DECLARE lactiondetreqsize = i4 WITH noconstant(0), protect
   IF (( $REASON_CD > 0))
    SET lactiondetreqsize = size(actionreq->actions[1].details,5)
    SET lactiondetreqsize += 1
    SET stat = alterlist(actionreq->actions[1].details,lactiondetreqsize)
    SET actionreq->actions[1].details[lactiondetreqsize].action_detail_entity_id =  $REASON_CD
    SET actionreq->actions[1].details[lactiondetreqsize].action_detail_entity_name = "CODE_VALUE"
    SET actionreq->actions[1].details[lactiondetreqsize].cp_action_detail_type_mean = "PATHDEVREAS"
   ENDIF
   CALL echorecord(actionreq)
   IF (((( $REASON_FREETEXT > "")) OR (( $REASON_CD=0))) )
    SET lactiondetreqsize = size(actionreq->actions[1].details,5)
    SET lactiondetreqsize += 1
    SET stat = alterlist(actionreq->actions[1].details,lactiondetreqsize)
    SET actionreq->actions[1].details[lactiondetreqsize].action_detail_entity_text =
     $REASON_FREETEXT
    SET actionreq->actions[1].details[lactiondetreqsize].action_detail_entity_name = "LONG_TEXT"
    SET actionreq->actions[1].details[lactiondetreqsize].cp_action_detail_type_mean = "PATHDEVREAS"
   ENDIF
   CALL echorecord(actionreq)
   CALL log_message(build("Exiting addActionDetails(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE updatepathwayactivity(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   CALL log_message("In updatePathwayActivity()",log_level_debug)
   UPDATE  FROM cp_pathway_activity cpa
    SET cpa.clinical_trial_ind = clintrialind, cpa.off_pathway_ind = offpathwayind, cpa.updt_dt_tm =
     cnvtdatetime(sysdate),
     cpa.updt_id = reqinfo->updt_id
    PLAN (cpa
     WHERE cpa.cp_pathway_activity_id=pathwayactivityid)
    WITH nocounter
   ;end update
   CALL error_and_zero_check_rec(curqual,"updatePathwayActivity","Could not update Activity",1,1,
    pathway_activity_reply)
   SELECT INTO "nl:"
    FROM cp_pathway_activity cpa
    PLAN (cpa
     WHERE cpa.cp_pathway_activity_id=pathwayactivityid)
    HEAD REPORT
     pathway_activity_reply->activity_cnt = 1, stat = alterlist(pathway_activity_reply->activity_qual,
      1), pathway_activity_reply->activity_qual[1].off_pathway_ind = cpa.off_pathway_ind,
     pathway_activity_reply->activity_qual[1].clinical_trial_ind = cpa.clinical_trial_ind
    WITH nocounter
   ;end select
   CALL error_and_zero_check_rec(curqual,"getPathwayActivityId","Could not rerieve Activity ID",1,1,
    pathway_activity_reply)
   CALL log_message(build("Exiting getPathwayActivityId(), elapsed time in seconds:",((curtime3 -
     begin_curtime3)/ 100.0)),log_level_debug)
 END ;Subroutine
 SUBROUTINE main(null)
   DECLARE begin_curtime3 = dq8 WITH constant(curtime3), private
   CALL log_message("In main()",log_level_debug)
   CALL getpathwayinfo(null)
   IF (pathwayactivityid > 0.0)
    CALL updatepathwayactivity(null)
    CALL addpathwayaction(null)
   ENDIF
   CALL log_message(build("Exiting main(), elapsed time in seconds:",((curtime3 - begin_curtime3)/
     100)),log_level_debug)
 END ;Subroutine
 SET pathway_activity_reply->status_data.status = "F"
 CALL main(null)
 SET pathway_activity_reply->status_data.status = "S"
#exit_script
 IF ((pathway_activity_reply->status_data.status="S"))
  COMMIT
 ELSE
  ROLLBACK
 ENDIF
 IF (( $OUTDEV != "NOFORMS"))
  CALL putjsonrecordtofile(pathway_activity_reply)
 ENDIF
 CALL log_message(concat("Exiting script: ",log_program_name),log_level_debug)
 CALL log_message(build("Total time in seconds:",((curtime3 - script_begin_curtime3)/ 100.0)),
  log_level_debug)
END GO
