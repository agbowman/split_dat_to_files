CREATE PROGRAM cht_acc_pt_w_loc
 PROMPT
  "Output to File/Printer/MINE " = mine,
  "Enter Beginning Date of Report in MMDDYYYY Format " = curdate,
  "Enter Ending Date of Report in MMDDYYYY Format " = curdate
 SET mrn_ind = 0.0
 SET stat = uar_get_meaning_by_codeset(4,"MRN",1,mrn_ind)
 SET beg_date = cnvtdatetime(
  IF (( $2=curdate))  $2
  ELSE cnvtdate( $2)
  ENDIF
  ,0)
 SET end_date = cnvtdatetime(
  IF (( $3=curdate))  $3
  ELSE cnvtdate( $3)
  ENDIF
  ,235959)
 SET maxsecs = 0
 IF (validate(isodbc,0)=1)
  SET maxsecs = 15
 ENDIF
 SELECT INTO  $1
  p.person_id, p.prsnl_id, p.ppa_first_dt_tm"DD-MMM-YYYY  HH:MM:SS;;D",
  p.ppa_last_dt_tm"DD-MMM-YYYY  HH:MM:SS;;D", p.computer_name, pe.name_full_formatted,
  pe.birth_dt_tm, pe.person_id, pa.alias,
  pa.person_id, person_alias_type_disp = uar_get_code_display(pa.person_alias_type_cd), p
  .view_caption,
  pr.name_full_formatted, position_disp = uar_get_code_display(pr.position_cd), ppa_first_dt =
  cnvtdatetime(p.ppa_first_dt_tm),
  date1 = format(p.ppa_first_dt_tm,"MM/DD/YYYY;;D"), pr.person_id, e.person_id,
  e.encntr_id, encntr_type_disp = uar_get_code_display(e.encntr_type_cd), el.encntr_id,
  med_service_disp = uar_get_code_display(el.med_service_cd), el.beg_effective_dt_tm, el.arrive_dt_tm,
  el.depart_dt_tm, el.end_effective_dt_tm, location_disp = uar_get_code_display(el.location_cd),
  loc_building_disp = uar_get_code_display(el.loc_building_cd), loc_nurse_unit_disp =
  uar_get_code_display(el.loc_nurse_unit_cd), loc_room_disp = uar_get_code_display(el.loc_room_cd)
  FROM person_prsnl_activity p,
   person pe,
   person_alias pa,
   prsnl pr,
   encounter e,
   encntr_loc_hist el
  PLAN (p
   WHERE p.ppa_first_dt_tm BETWEEN cnvtdatetime(
    IF (( $2=curdate))  $2
    ELSE cnvtdate( $2)
    ENDIF
    ,0) AND cnvtdatetime(
    IF (( $3=curdate))  $3
    ELSE cnvtdate( $3)
    ENDIF
    ,235959)
    AND p.view_caption > " ")
   JOIN (pe
   WHERE p.person_id=pe.person_id)
   JOIN (pa
   WHERE p.person_id=pa.person_id
    AND pa.person_alias_type_cd=mrn_ind)
   JOIN (pr
   WHERE p.prsnl_id=pr.person_id)
   JOIN (e
   WHERE p.person_id=e.person_id)
   JOIN (el
   WHERE e.encntr_id=el.encntr_id)
  ORDER BY pe.name_full_formatted, date1, 0
  HEAD REPORT
   m_numlines = 0,
   SUBROUTINE cclrtf_print(par_flag,par_startcol,par_numcol,par_blob,par_bloblen,par_check)
     m_output_buffer_len = 0, blob_out = fillstring(32768," "), blob_buf = fillstring(200," "),
     blob_len = 0, m_linefeed = concat(char(10)), textindex = 0,
     numcol = par_numcol, whiteflag = 0,
     CALL uar_rtf(par_blob,par_bloblen,blob_out,size(blob_out),m_output_buffer_len,par_flag),
     m_output_buffer_len = minval(m_output_buffer_len,size(trim(blob_out)))
     IF (m_output_buffer_len > 0)
      m_cc = 1
      WHILE (m_cc > 0)
       m_cc2 = findstring(m_linefeed,blob_out,m_cc),
       IF (m_cc2)
        blob_len = (m_cc2 - m_cc)
        IF (blob_len <= par_numcol)
         m_blob_buf = substring(m_cc,blob_len,blob_out), col par_startcol
         IF (par_check)
          CALL print(trim(check(m_blob_buf)))
         ELSE
          CALL print(trim(m_blob_buf))
         ENDIF
         row + 1
        ELSE
         m_blobbuf = substring(m_cc,blob_len,blob_out),
         CALL cclrtf_printline(par_startcol,par_numcol,m_blobbuf,blob_len,par_check), row + 1
        ENDIF
        IF (m_cc2 >= m_output_buffer_len)
         m_cc = 0
        ELSE
         m_cc = (m_cc2+ 1)
        ENDIF
       ELSE
        blob_len = ((m_output_buffer_len - m_cc)+ 1), m_blobbuf = substring(m_cc,blob_len,blob_out),
        CALL cclrtf_printline(par_startcol,par_numcol,m_blobbuf,blob_len,par_check),
        m_cc = 0
       ENDIF
      ENDWHILE
     ENDIF
   END ;Subroutine report
   ,
   SUBROUTINE cclrtf_printline(par_startcol,par_numcol,blob_out,blob_len,par_check)
     textindex = 0, numcol = par_numcol, whiteflag = 0,
     lastline = 0, m_linefeed = concat(char(10)), m_maxchar = concat(char(128)),
     m_find = 0
     WHILE (blob_len > 0)
       IF (blob_len <= par_numcol)
        numcol = blob_len, lastline = 1
       ENDIF
       textindex = (m_cc+ par_numcol)
       IF (lastline=0)
        whiteflag = 0
        WHILE (whiteflag=0)
         IF (((substring(textindex,1,blob_out)=" ") OR (substring(textindex,1,blob_out)=m_linefeed))
         )
          whiteflag = 1
         ELSE
          textindex = (textindex - 1)
         ENDIF
         ,
         IF (((textindex=m_cc) OR (textindex=0)) )
          textindex = (m_cc+ par_numcol), whiteflag = 1
         ENDIF
        ENDWHILE
        numcol = ((textindex - m_cc)+ 1)
       ENDIF
       m_blob_buf = substring(m_cc,numcol,blob_out)
       IF (m_blob_buf > " ")
        col par_startcol
        IF (par_check)
         CALL print(trim(check(m_blob_buf)))
        ELSE
         CALL print(trim(m_blob_buf))
        ENDIF
        row + 1
       ELSE
        blob_len = 0
       ENDIF
       m_cc = (m_cc+ numcol)
       IF (blob_len > numcol)
        blob_len = (blob_len - numcol)
       ELSE
        blob_len = 0
       ENDIF
     ENDWHILE
   END ;Subroutine report
   ,
   row 1, col 38, "CHART ACCESS BY PATIENT INCLUDING ENCOUNTER INFORMATION",
   row + 2
  HEAD PAGE
   col 5, "Report Generated By:", col 32,
   curuser, col 48, "Report Generated On:",
   col 69, curdate, col 81,
   "Page:", col 87, curpage,
   row + 2, col 14, "From:",
   col 29, beg_date"mmm/dd/yyyy;;d", col 66,
   "To:", col 75, end_date"mmm/dd/yyyy;;d",
   row + 2
  HEAD pe.name_full_formatted
   col 5,
   "********************************************************************************************************",
   row + 1,
   name_full_formatted1 = substring(1,40,pe.name_full_formatted), alias1 = substring(1,10,pa.alias),
   col 2,
   "Patient Name:", col 17, name_full_formatted1,
   col 63, "Medical Record #:", col 78,
   alias1, row + 1, col 2,
   "Date of Birth:", col 17, pe.birth_dt_tm,
   col 38, p.person_id, row + 1,
   encntr_type_disp1 = substring(1,20,encntr_type_disp), med_service_disp1 = substring(1,20,
    med_service_disp), col 2,
   "Encounter ID:", col 17, e.encntr_id,
   col 48, "Encounter Type:", col 63,
   encntr_type_disp1, col 87, "Service:",
   col 97, med_service_disp1, row + 1,
   location_disp1 = substring(1,20,location_disp), loc_nurse_unit_disp1 = substring(1,10,
    loc_nurse_unit_disp), col 2,
   "Begin Date:", col 23, el.beg_effective_dt_tm,
   col 35, "End Date:", col 51,
   el.end_effective_dt_tm, col 63, "Location:",
   col 72, location_disp1, col 94,
   "Nurse Unit:", col 106, loc_nurse_unit_disp1,
   row + 1, loc_building_disp1 = substring(1,20,loc_building_disp), loc_room_disp1 = substring(1,10,
    loc_room_disp),
   col 63, "Building:", col 72,
   loc_building_disp1, col 94, "Room:",
   col 106, loc_room_disp1, row + 2
  DETAIL
   IF (((row+ 3) >= maxrow))
    BREAK
   ENDIF
   name_full_formatted2 = substring(1,40,pr.name_full_formatted), col 2, "Time Access Began:",
   col 20, p.ppa_first_dt_tm, col 63,
   "User Name:", col 75, name_full_formatted2,
   row + 1, computer_name1 = substring(1,20,p.computer_name), col 2,
   "Time Access Ended:", col 20, p.ppa_last_dt_tm,
   col 63, "Computer Name:", col 75,
   computer_name1, row + 1, view_caption1 = substring(1,40,p.view_caption),
   position_disp1 = substring(1,30,position_disp), col 2, "Tab Accessed:",
   col 20, view_caption1, col 63,
   "Position:", col 75, position_disp1,
   row + 1
  FOOT  pe.name_full_formatted
   row + 2
  WITH maxcol = 500, noheading, format = variable,
   time = value(maxsecs)
 ;end select
END GO
