CREATE PROGRAM dm_med_history_audit_rows:dba
 ...
 SET reply->table_name = "MED_HISTORY_AUDIT"
 SET reply->rows_between_commit = minval(100,request->max_rows)
 SET i18nhandle = 0
 SET h = uar_i18nlocalizationinit(i18nhandle,curprog,"",curcclrev)
 DECLARE months_to_keep = f8 WITH protect, noconstant(3.0)
 ...
 IF (batch_ndx=1)
  SELECT INTO "nl:"
   minval = min(m.med_history_audit_id)
   FROM med_history_audit m
   DETAIL
 ...
 ENDIF
 SELECT INTO "nl:"
  maxval = max(m.med_history_audit_id)
  FROM med_history_audit m
  DETAIL
 ...
 WHILE (v_cur_min_id <= v_max_id
  AND v_rows_left > 0)
   SELECT INTO "nl:"
    m.rowid
    FROM med_history_audit m
    WHERE m.med_history_audit_id BETWEEN v_cur_min_id AND v_cur_max_id
     AND parser(sbr_getrowidnotexists(
      "m.audit_dt_tm < cnvtlookbehind(strUnit,cnvtdatetime(curdate,curtime3))","m"))
    DETAIL
     v_rows = (v_rows+ 1)
     IF (mod(v_rows,100)=1)
      stat = alterlist(reply->rows,(v_rows+ 99))
     ENDIF
     reply->rows[v_rows].row_id = m.rowid
    WITH nocounter, maxqual(m,value(v_rows_left))
   ;end select
