CREATE PROGRAM cn_ident_unauth_child_docs:dba
 PROMPT
  "Enter begin date in DD-MMM-YYYY format (defaults to 10-NOV-2010) : " = "10-NOV-2010",
  "Enter the number of days to process per batch (defaults to 1)  : " = "1"
  WITH begindate, batchsize
 IF (datetimecmp(cnvtdatetime( $BEGINDATE),cnvtdatetime("10-NOV-2010")) < 0)
  GO TO exit_script
 ENDIF
 ...
 DECLARE logfilename = vc WITH constant(build("cer_temp:ident_unauth_child_docs_log",format(curdate,
 ...
 DECLARE identifyunauthchildstatusdocs(null) = null WITH protect
 CALL echo(build("Output file name is: ",logfilename))
 CALL identifyunauthchildstatusdocs(null)
 SUBROUTINE identifyunauthchildstatusdocs(null)
   ...
   SELECT INTO value(logfilename)
    DETAIL
     ...
     ',"Document Title","Perform Date and Time","Service Date and Time"'
   ...
   FOR (iteration = 1 TO numberofiteration)
     ...
     SELECT DISTINCT INTO value(logfilename)
      ce.event_id, ce3.event_end_dt_tm, ce3.event_title_text,
      ce3.performed_prsnl_id, ce3.result_status_cd, ce3.event_cd,
      ce3.person_id, ce3.performed_dt_tm
      ...
      PLAN (ce
       ...
       WHERE ce2.parent_event_id=ce.event_id
        ...
        AND ce2.result_status_cd=unauth_cd
      ...
      DETAIL
       ...
       CALL subr_out(build(ce3.performed_prsnl_id)),
       quote_str = '"',
       CALL subr_out(build(pat.name_full_formatted)),
       CALL subr_out(build(ce3.person_id)),
...
#exit_script
 IF (datetimecmp(cnvtdatetime( $BEGINDATE),cnvtdatetime("10-NOV-2010")) < 0)
  CALL echo("   You entered a date before 10-NOV-2010. Please pick a date on or after 10-NOV-2010")
 ENDIF
 IF (cnvtreal( $BATCHSIZE) < 1)
