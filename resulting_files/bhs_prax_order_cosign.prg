CREATE PROGRAM bhs_prax_order_cosign
 ...
 DECLARE mpersonid = f8 WITH protect, constant(request->prsnl[1].prsnl_id)
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  HEAD o.order_id
   ...
   orders_n->prsnl[cnt].orderdescription = uar_get_code_display(oo.catalog_cd), orders_n->prsnl[cnt].
   orderdetail = oo.clinical_display_line, orders_n->prsnl[cnt].originatorid = oa.order_provider_id,
 ...
 SELECT DISTINCT INTO "nl:"
  ...
  PLAN (pg
   ...
   WHERE o.to_prsnl_group_id=pg.prsnl_group_id
    AND o.notification_status_flag=1
    AND o.notification_type_flag IN (1, 2)
    AND o.notification_display_dt_tm BETWEEN cnvtdatetime(begindate) AND cnvtdatetime(enddate))
   JOIN (oo
   WHERE oo.order_id=o.order_id)
   JOIN (p
   WHERE p.person_id=oo.person_id)
