CREATE PROGRAM aps_rdm_move_pc_comments:dba
 ...
 END ;Subroutine
#start_script
 SET sreadme_name = "aps_rdm_move_pc_comments"
 ...
 CALL logmsg("Getting ready to begin reading for pathology_case comment.",((e_dispmsg+ e_rdmstatus)+
  e_logtofile))
 SELECT INTO "nl:"
 ...
 WHILE (pc_cntr >= 1)
   ...
   IF (curqual != value(pc_commit_recs_whole))
    ROLLBACK
    CALL logmsg("Failed to update pathology_case table.",((e_dispmsg+ e_logtofile)+ e_rdmstatus))
    SET sprogramfailed = "T"
    GO TO exit_script
   ENDIF
   SET sprogramfailed = "F"
