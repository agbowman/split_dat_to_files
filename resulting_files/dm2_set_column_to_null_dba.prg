CREATE PROGRAM dm2_set_column_to_null:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE dsct_logfile_prefix = vc WITH protect, constant("dm2_set_null")
 DECLARE dsct_table_name = vc WITH protect, noconstant(" ")
 DECLARE dsct_column_name = vc WITH protect, noconstant(" ")
 DECLARE dsct_sc_str = vc WITH protect, noconstant(" ")
 DECLARE dsct_cons_found = i2 WITH protect, noconstant(0)
 DECLARE dsct_cons_name = vc WITH protect, noconstant(" ")
 DECLARE dsct_push_cmd = vc WITH protect, noconstant(" ")
 FREE RECORD dsct_cons
 RECORD dsct_cons(
   1 cnt = i2
   1 qual[*]
     2 name = vc
 )
 IF (check_logfile(dsct_logfile_prefix,".log","DM2_SET_COLUMN_TO_NULL LOGFILE")=0)
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Beginning dm2_set_column_to_null process..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dsct_table_name = cnvtupper(trim( $1,3))
 IF (dsct_table_name=" ")
  SET dm_err->err_ind = 1
  ...
  GO TO exit_program
 ENDIF
 SET dsct_column_name = cnvtupper(trim( $2,3))
 IF (dsct_column_name=" ")
  SET dm_err->err_ind = 1
  ...
  GO TO exit_program
 ENDIF
 SET dm_err->eproc = "Attempt to set column to NULL"
 CALL disp_msg(" ",dm_err->logfile,0)
 SET dsct_push_cmd = concat("RDB ASIS (^ ALTER TABLE ",trim(dsct_table_name)," MODIFY ","(",
  dsct_column_name,
  " NULL ) ^) GO ")
 IF (dm2_push_cmd(dsct_push_cmd,1)=0)
  ROLLBACK
  GO TO exit_program
 ELSE
  COMMIT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (dc
   WHERE dc.table_name=dsct_table_name
    AND dc.owner=currdbuser
    AND dc.constraint_type="C")
   JOIN (dcc
   WHERE dcc.constraint_name=dc.constraint_name
    AND dcc.owner=dc.owner
    AND dcc.table_name=dsct_table_name
    AND dcc.column_name=dsct_column_name)
  DETAIL
   dsct_sc_str = cnvtupper(trim(trim(dc.search_condition,3),4))
   IF ((dm_err->debug_flag > 1))
    CALL echo(build("search_consition =",dsct_sc_str))
   ENDIF
   IF (findstring("ISNOTNULL",dsct_sc_str) > 0)
    dsct_cons_found = 1, dsct_cons->cnt = (dsct_cons->cnt+ 1), stat = alterlist(dsct_cons->qual,
     dsct_cons->cnt),
    dsct_cons->qual[dsct_cons->cnt].name = dc.constraint_name
   ENDIF
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc)=1)
  ...
  GO TO exit_program
 ENDIF
 IF (dsct_cons_found=1)
  FOR (dsct_i = 1 TO dsct_cons->cnt)
    SET dm_err->eproc = concat("Drop check constraint ",trim(dsct_cons->qual[dsct_i].name,3),
     " for table ",trim(dsct_table_name,3)," column ",
     trim(dsct_column_name,3))
    CALL disp_msg(" ",dm_err->logfile,0)
    SET dsct_push_cmd = concat("RDB ASIS (^ ALTER TABLE ",trim(dsct_table_name,3)," DROP CONSTRAINT ",
     trim(dsct_cons->qual[dsct_i].name,3),"^) GO")
    IF (dm2_push_cmd(dsct_push_cmd,1)=0)
     ROLLBACK
     GO TO exit_program
    ELSE
 ...
 GO TO exit_program
#exit_program
 SET dm_err->eproc = "DM2_SET_COLUMN_TO_NULL COMPLETED"
 CALL final_disp_msg(dsct_logfile_prefix)
END GO
