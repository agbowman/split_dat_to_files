CREATE PROGRAM dm2_ref_data_mover_1545:dba
 ...
 SUBROUTINE dm2_rdds_get_tgt_id(s_gmti_tgt_rs)
   ...
   RETURN(drgmti_return_val)
 END ;Subroutine
 DECLARE cust_entity_mean = vc
 DECLARE cust_tab_name = vc
 DECLARE cust_entity1_val = vc
 DECLARE cust_entity2_val = vc
 DECLARE cust_no_query = i2
 DECLARE cust_val = f8
 DECLARE cust_idcd_check = i2
 DECLARE cust_from_id = vc
 DECLARE cust_new_id = vc
 DECLARE cust_ins_upd = i2
 DECLARE cust_entity1_name = vc
 DECLARE cust_entity2_name = vc
 SET cust_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET cust_entity_mean = get_value(cust_tab_name,"ENTITY_RELTN_MEAN","FROM")
 SET cust_entity1_val = get_value(cust_tab_name,"ENTITY1_ID","FROM")
 SET cust_entity2_val = get_value(cust_tab_name,"ENTITY2_ID","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_1545
 ENDIF
 CASE (cust_entity_mean)
  OF "DRUG/DRUG":
  OF "DRUG/ALLERGY":
   CALL put_value(cust_tab_name,"ENTITY1_ID",cust_entity1_val)
   CALL put_value(cust_tab_name,"ENTITY2_ID",cust_entity2_val)
  OF "DRUG/FOOD":
   CALL put_value(cust_tab_name,"ENTITY1_ID",cust_entity1_val)
   CALL put_value(cust_tab_name,"ENTITY2_ID",cust_entity2_val)
  OF "DRUG/TEXT":
  ...
  OF "OEF/PERFLOC":
   ...
   SET cust_entity1_name = "ORDER_ENTRY_FORMAT_PARENT"
   SET rdds_exception->qual[1].tab_col_name = "ENTITY1_ID"
   SET rdds_exception->qual[1].tru_tab_name = "ORDER_ENTRY_FORMAT_PARENT"
   SET rdds_exception->qual[1].tru_col_name = "OE_FORMAT_ID"
   SET cust_entity2_name = "CODE_VALUE"
   SET rdds_exception->qual[2].tab_col_name = "ENTITY2_ID"
   SET rdds_exception->qual[2].tru_tab_name = "CODE_VALUE"
   SET rdds_exception->qual[2].tru_col_name = "CODE_VALUE"
  OF "ALTSEL/NOMCT":
   SET stat = alterlist(rdds_exception->qual,2)
   SET cust_entity1_name = "ALT_SEL_CAT"
   SET rdds_exception->qual[1].tab_col_name = "ENTITY1_ID"
   SET rdds_exception->qual[1].tru_tab_name = "ALT_SEL_CAT"
   SET rdds_exception->qual[1].tru_col_name = "ALT_SEL_CATEGORY_ID"
   SET cust_entity2_name = "NOMEN_CATEGORY"
   SET rdds_exception->qual[2].tab_col_name = "ENTITY2_ID"
   SET rdds_exception->qual[2].tru_tab_name = "NOMEN_CATEGORY"
   SET rdds_exception->qual[2].tru_col_name = "NOMEN_CATEGORY_ID"
  OF "ALTSEL/NOMEN":
   SET stat = alterlist(rdds_exception->qual,2)
   SET cust_entity1_name = "ALT_SEL_CAT"
   SET rdds_exception->qual[1].tab_col_name = "ENTITY1_ID"
  ...
  OF "SNT ANNOUNCE":
   ...
   SET cust_entity2_name = "LONG_TEXT"
   SET rdds_exception->qual[1].tab_col_name = "ENTITY2_ID"
   SET rdds_exception->qual[1].tru_tab_name = "LONG_TEXT"
   SET rdds_exception->qual[1].tru_col_name = "LONG_TEXT_ID"
  OF "TASK/LOC":
   SET stat = alterlist(rdds_exception->qual,2)
   SET cust_entity1_name = "CODE_VALUE"
   SET rdds_exception->qual[1].tab_col_name = "ENTITY1_ID"
 ...
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_1545
 ENDIF
 IF (is_translated(cust_tab_name,"ENTITY1_ID")=0
  AND cust_entity1_name > "")
  SET cust_new_id = select_merge_translate(cust_entity1_val,cust_entity1_name)
  IF (cust_new_id != "No Trans")
   CALL put_value(cust_tab_name,"ENTITY1_ID",cust_new_id)
  ENDIF
 ENDIF
 IF (is_translated(cust_tab_name,"ENTITY2_ID")=0
  AND cust_entity2_name > "")
  SET cust_new_id = select_merge_translate(cust_entity2_val,cust_entity2_name)
  IF (cust_new_id != "No Trans")
   CALL put_value(cust_tab_name,"ENTITY2_ID",cust_new_id)
  ENDIF
 ENDIF
 SET temp_col_cnt = get_col_pos(cust_tab_name,"DCP_ENTITY_RELTN_ID")
 IF ((dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].col_qual[temp_col_cnt].translated=0))
  ...
  IF (cust_no_query=1)
   ...
   IF (cust_val < 0)
    IF ((cust_val=- (3)))
     SET cust_val = get_seq(cust_tab_name,"DCP_ENTITY_RELTN_ID")
     IF (cust_val > 0)
      CALL put_value(cust_tab_name,"DCP_ENTITY_RELTN_ID",cnvtstring(cust_val))
      SET current_merges = (current_merges+ 1)
      SET child_merge_audit->num[current_merges].action = "NEWSEQ"
      SET child_merge_audit->num[current_merges].text = concat(cust_tab_name,"  DCP_ENTITY_RELTN_ID")
     ELSE
      GO TO exit_1545
     ENDIF
    ELSE
  ...
  SET cust_from_id = get_value(cust_tab_name,"DCP_ENTITY_RELTN_ID","FROM")
  IF ((dm2_ref_data_reply->error_ind=1))
   GO TO exit_1545
  ELSE
   SET dm2_ref_data_reply->qual[1].from_value = cnvtreal(cust_from_id)
  ENDIF
  SET cust_new_id = get_value(cust_tab_name,"DCP_ENTITY_RELTN_ID","TO")
  IF ((dm2_ref_data_reply->error_ind=1))
   GO TO exit_1545
  ELSE
