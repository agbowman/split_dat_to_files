CREATE PROGRAM dm2_mig_verify_data_setup
 ...
 SUBROUTINE dvdr_get_verify2(null)
  ...
  RETURN(1)
 END ;Subroutine
 DECLARE dmvds_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmvds_mngd_tables = vc WITH protect, noconstant("")
 IF (check_logfile("dm2_mig_vdata_setup",".log","dm2_mig_vdata_setup LOGFILE")=0)
  GO TO exit_script
 ENDIF
 ...
 IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Load date column override list"
 CALL compile("cer_install:dm2_mig_vdata_date_col_ovr.ccl")
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 ENDIF
 IF (dvdr_get_data_verification_info(1)=0)
  GO TO exit_script
 ENDIF
 SET message = nowindow
 SET dmvds_mngd_tables = ""
 IF (dmr_load_managed_tables(dmvds_mngd_tables)=0)
  GO TO exit_script
 ENDIF
 FOR (dmvds_cnt = 1 TO dmr_user_list->cnt)
   SET dmvd->owner = dmr_user_list->qual[dmvds_cnt].user
   SET dmvd->table_name = "*"
   SET dm2_install_schema->curprog = "dm2_mig_vdata_setup"
   SET dm_err->eproc = concat("Check if ",dmr_user_list->qual[dmvds_cnt].user," has table objects.")
   SELECT INTO "nl:"
    FROM dba_tables d
    WHERE (d.owner=dmr_user_list->qual[dmvds_cnt].user)
     AND parser(concat(" d.table_name not in (",dmvds_mngd_tables,")"))
    WITH nocounter, maxqual(d,1)
   ;end select
   IF (check_error(dm_err->eproc)=1)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
   ...
   ENDIF
 ENDFOR
 EXECUTE dm2_mig_verify_data_excl
 IF ((dm_err->err_ind=1))
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Load full table compare list"
 CALL compile("cer_install:dm2_mig_vdata_cdtables_full.ccl")
 IF (check_error(dm_err->eproc)=1)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Exclude FSIESO_QUE_DETAILS"
 UPDATE  FROM dm_vdata_master a
  SET a.compare_status = "EXCLUDE"
  WHERE a.table_name="FSIESO_QUE_DETAILS"
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Denote to not pull row times for CODE_VALUE_EXTNSN_MODS"
 UPDATE  FROM dm_vdata_master a
  SET a.row_dt_tm_col_name = "DM2NOTSET"
  WHERE a.table_name="CODE_VALUE_EXTNSN_MODS"
   AND a.owner_name="V500"
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "COMPARE ORDER DETAIL"
 UPDATE  FROM dm_vdata_master a
  SET a.rows_to_compare = 5000
  WHERE a.table_name="ORDER_DETAIL"
   AND a.owner_name="V500"
   AND a.rows_to_compare < 5000
  WITH nocounter
 ;end update
 IF (check_error(dm_err->eproc)=1)
  ROLLBACK
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "EXCLUDE PROCESS DETAILS"
 UPDATE  FROM dm_vdata_master a
  SET a.compare_status = "EXCLUDE"
  WHERE a.table_name IN ("DM_PROCESS_QUEUE", "DM_PROCESS_EVENT", "DM_PROCESS_EVENT_DTL", "DM_PROCESS"
  )
   AND a.owner_name="V500"
  WITH nocounter
 ;end update
 ...
 ENDIF
 COMMIT
 SET dm_err->eproc = "Creating v500.migdelivlagoffset trigger"
 CALL dm2_push_cmd(concat("rdb asis(^CREATE OR REPLACE TRIGGER v500.migdelivlagoffset ",char(10),"^)"
   ),0)
 CALL dm2_push_cmd(concat("asis(^ BEFORE UPDATE or INSERT ON DM_VDATA_MASTER",char(10),"^)"),0)
 CALL dm2_push_cmd(concat("asis(^ FOR EACH row WHEN (new.lag_dt_tm IS NOT NULL) ",char(10),"^)"),0)
 CALL dm2_push_cmd(concat("asis(^ BEGIN ",char(10),"^)"),0)
 CALL dm2_push_cmd(concat(
   "asis(^ IF(:new.lag_dt_tm != to_date('1900-01-01 12:00:00','yyyy-mm-dd hh24:mi:ss')) ",char(10),
   "^)"),0)
 CALL dm2_push_cmd(concat("asis(^ THEN",char(10),"^)"),0)
 CALL dm2_push_cmd(concat("asis(^   --add 240 min to the lag time",char(10),"^)"),0)
 CALL dm2_push_cmd(concat("asis(^     :new.lag_dt_tm := :new.lag_dt_tm -(240/60/24);",char(10),"^)"),
  0)
 CALL dm2_push_cmd(concat("asis(^   END IF; ",char(10),"^)"),0)
 CALL dm2_push_cmd(concat("asis(^ END migdelivlagoffset; ",char(10),"^)"),0)
 ...
 IF (curqual=0)
  INSERT  FROM dm_info d
   SET d.info_domain = "DM2_VDATA_INFO", d.info_name = "COMPARE GG LAG OFFSET", d.info_number = 0
   WITH nocounter
  ;end insert
 ENDIF
 IF (check_error(dm_err->eproc)=1)
...
#exit_script
 IF ((dm_err->err_ind=0))
  SET dm_err->eproc = "Dm2_mig_verify_data_setup completed successfully."
 ELSE
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
 ENDIF
 CALL final_disp_msg("dm2_mig_verify_data_setup")
END GO
