CREATE PROGRAM dash_upd_anes_breaks_position:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Position:" = "",
  "New Status:" = 1
  WITH outdev, position, newstatus
 ...
 SET log_program_name = "dash_upd_anes_breaks_position"
 ...
 INSERT  FROM dash_break_hist
  ...
  exclude_ind, updt_cnt)(SELECT
   ...
   FROM dash_break db
   WHERE (db.position_ident= $POSITION))
 ;end insert
 ...
 UPDATE  FROM dash_break db
  SET db.status_flag =  $NEWSTATUS, db.updt_cnt = (db.updt_cnt+ 1), db.status_dt_tm = cnvtdatetime(
    curdate,curtime)
  WHERE (db.position_ident= $POSITION)
 ;end update
 IF (error(err_msg,0) > 0)
...
#exit_script
 IF (failed=1)
  ...
  CALL log_message(concat("Error updating provider statuses: ",err_msg),log_level_debug)
 ELSE
