CREATE PROGRAM dm_transaction_addr_verif_rows:dba
 ...
 SET reply->table_name = "TRANSACTION_ADDRESS_VERIFY"
 ...
 DECLARE v_err_code = i4 WITH protect, noconstant(0)
 DECLARE t_idx = i4 WITH protect, noconstant(0)
 DECLARE stat = i4 WITH protect, noconstant(0)
 DECLARE rows = i4 WITH protect, noconstant(0)
 DECLARE v_days_back = dq8 WITH protect, noconstant(0.0)
 DECLARE v_batch_size = f8 WITH protect, noconstant(10000.0)
 ...
 FOR (t_idx = 1 TO size(request->tokens,5))
   IF ((request->tokens[t_idx].token_str="DAYSTOKEEP"))
    SET v_days_to_keep = ceil(cnvtreal(request->tokens[t_idx].value))
   ENDIF
 ENDFOR
 ...
 ELSE
  IF (batch_ndx=1)
   SELECT INTO "nl:"
    minval = min(tv.transaction_address_verify_id)
    FROM transaction_address_verify tv
    WHERE tv.transaction_address_verify_id > 0
    DETAIL
  ...
  ENDIF
  SELECT INTO "nl:"
   maxval = max(tv.transaction_address_verify_id)
   FROM transaction_address_verify tv
   DETAIL
  ...
  WHILE (v_cur_min_id <= v_max_id
   AND v_rows_left > 0)
    SELECT INTO "nl:"
     tv.rowid
     FROM transaction_address_verify tv
     WHERE parser(sbr_getrowidnotexists("tv.sent_dt_tm < cnvtdatetime(v_days_back)","tv"))
      AND tv.transaction_address_verify_id BETWEEN v_cur_min_id AND v_cur_max_id
     DETAIL
      rows += 1
      IF (mod(rows,100)=1)
       stat = alterlist(reply->rows,(rows+ 99))
      ENDIF
      reply->rows[rows].row_id = tv.rowid
     WITH nocounter, maxqual(tv,value(v_rows_left))
    ;end select
    SET v_errcode = error(v_errmsg,1)
    IF (v_errcode != 0)
     SET reply->err_code = v_errcode
     SET reply->status_data.status = "F"
    ...
    SET v_cur_max_id = (v_cur_min_id+ (v_batch_size - 1))
    SET v_rows_left = (request->max_rows - rows)
  ENDWHILE
