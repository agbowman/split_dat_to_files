CREATE PROGRAM dm2_mig_updt_triggers
 ...
 SUBROUTINE dctx_restore_prev_contexts(null)
   ...
   RETURN(1)
 END ;Subroutine
 IF (check_logfile("dm2_mig_updt_trig",".log","dm2_mig_updt_triggers")=0)
  GO TO exit_program
 ENDIF
 DECLARE dmut_prompted = i2 WITH protect, noconstant(0)
 DECLARE dmut_owner_in = vc WITH protect, noconstant(cnvtupper( $1))
 DECLARE dmut_status_in = vc WITH protect, noconstant(cnvtupper( $2))
 DECLARE dmut_op_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmut_remove_reconnect_target(null) = i2
 FREE RECORD dmut_disabled_trigs
 RECORD dmut_disabled_trigs(
   1 cnt = i4
   1 qual[*]
     2 trig_name = vc
     2 owner = vc
 )
 SET dmut_disabled_trigs->cnt = 0
 FREE RECORD dmut_trigs
 RECORD dmut_trigs(
   1 cnt = i4
   1 qual[*]
     2 trig_name = vc
 ...
 SET dmut_trigs->cnt = 0
 SET dm_err->eproc = "Starting dm2_mig_updt_triggers"
 CALL disp_msg(" ",dm_err->logfile,0)
 IF ( NOT (dmut_status_in IN ("ENABLE", "DISABLE")))
  SET dm_err->eproc = "Validate mode of execution."
  SET dm_err->emsg = concat("Invalid mode of execution,",dmut_status_in,
   ". Valid modes are ENABLE and DISABLE")
  SET dm_err->err_ind = 1
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_program
 ENDIF
 IF ((((dm2_install_schema->src_v500_p_word="NONE")
  AND (dm2_install_schema->src_v500_connect_str="NONE")) OR ((dm2_install_schema->v500_p_word="NONE")
 ...
 IF (dmut_prompted=1)
  ...
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  IF (dmr_issue_summary_screen(1,1,concat("Continuing this process will ",dmut_status_in,
    " triggers on the TARGET database."))=0)
   GO TO exit_program
  ENDIF
 ENDIF
 IF (dmut_status_in="ENABLE")
  IF ((((dmr_mig_data->cur_db_type != "SOURCE")) OR (currdbuser != "V500")) )
   IF (dmr_prompt_connect_data("SOURCE","V500","CO")=0)
    GO TO exit_program
   ENDIF
  ENDIF
  SET dm_err->eproc = "While connected to source, get status of triggers"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SELECT
   IF (dmut_owner_in="ALL")
    WHERE d.owner IN (
    (SELECT
     di.info_name
     FROM dm_info di
     WHERE di.info_domain IN ("DM2_MIG_USER", "DM2_MIG_DT_USER")))
     AND d.status="DISABLED"
   ELSE
   ENDIF
   INTO "nl:"
   FROM dba_triggers d
   WHERE d.owner=dmut_owner_in
    AND d.status="DISABLED"
   ORDER BY d.trigger_name
   DETAIL
    IF (d.owner="V500"
     AND d.table_name="DM_INFO"
     AND d.trigger_name IN ("TRG*LUTS", "TRG*TXNSCN"))
     IF ((dm_err->debug_flag > 0))
      CALL echo(
      "Bypassing crawler triggers on V500.DM_INFO as they should always be enabled in Target")
     ENDIF
    ELSE
     dmut_disabled_trigs->cnt = (dmut_disabled_trigs->cnt+ 1)
     IF (mod(dmut_disabled_trigs->cnt,100)=1)
      stat = alterlist(dmut_disabled_trigs->qual,(dmut_disabled_trigs->cnt+ 99))
     ENDIF
     dmut_disabled_trigs->qual[dmut_disabled_trigs->cnt].trig_name = d.trigger_name,
     dmut_disabled_trigs->qual[dmut_disabled_trigs->cnt].owner = d.owner
    ENDIF
   FOOT REPORT
    stat = alterlist(dmut_disabled_trigs->qual,dmut_disabled_trigs->cnt)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF ((dm_err->debug_flag > 2))
   CALL echorecord(dmut_disabled_trigs)
  ...
  IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
   ...
   ENDIF
  ENDIF
  SET dm_err->eproc = "Get all triggers that are disabled"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SELECT
   IF (dmut_owner_in="ALL")
    ...
    (SELECT
     1
     FROM recyclebin r
     WHERE r.object_name=d.table_name
      AND r.type="TABLE")))
   ELSE
   ENDIF
   INTO "nl:"
   ...
   HEAD REPORT
    loc = 0
   DETAIL
    IF (locateval(loc,1,dmut_disabled_trigs->cnt,d.trigger_name,dmut_disabled_trigs->qual[loc].
     trig_name,
     d.owner,dmut_disabled_trigs->qual[loc].owner)=0)
     dmut_trigs->cnt = (dmut_trigs->cnt+ 1)
     IF (mod(dmut_trigs->cnt,100)=1)
      stat = alterlist(dmut_trigs->qual,(dmut_trigs->cnt+ 99))
     ENDIF
     dmut_trigs->qual[dmut_trigs->cnt].trig_name = d.trigger_name, dmut_trigs->qual[dmut_trigs->cnt].
     owner = d.owner
    ELSE
     IF ((dm_err->debug_flag > 0))
      CALL echo(concat("Bypassing enable of trigger:",d.trigger_name))
     ENDIF
    ENDIF
   FOOT REPORT
    stat = alterlist(dmut_trigs->qual,dmut_trigs->cnt)
   WITH nocounter
  ;end select
  IF (check_error(dm_err->eproc)=1)
   CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  ...
  ENDIF
  IF ((dm_err->debug_flag > 2))
   CALL echorecord(dmut_trigs)
  ENDIF
 ENDIF
 IF (dmut_status_in="DISABLE")
  IF ((((dmr_mig_data->cur_db_type != "TARGET")) OR (currdbuser != "V500")) )
   IF (dmr_prompt_connect_data("TARGET","V500","CO")=0)
    GO TO exit_program
   ENDIF
  ENDIF
  SET dm_err->eproc = "Get all triggers that are enabled"
  IF ((dm_err->debug_flag > 0))
   CALL disp_msg(" ",dm_err->logfile,0)
  ENDIF
  SELECT
   IF (dmut_owner_in="ALL")
    ...
    (SELECT
     ...
     AND  NOT (d.table_name IN (
    (SELECT
     r.object_name
     FROM recyclebin r
     WHERE r.type="TABLE")))
     AND d.trigger_name != "UTC_CONVERT*"
     AND d.trigger_name != "MIGDELIVLAGOFFSET"
   ELSE
   ENDIF
   INTO "nl:"
   FROM dba_triggers d
 ...
 ENDIF
 IF ((dmut_trigs->cnt > 0))
  FOR (dmut_op_cnt = 1 TO value(dmut_trigs->cnt))
    SET dm_err->eproc = concat(dmut_status_in," Trigger:",dmut_trigs->qual[dmut_op_cnt].trig_name)
    IF ((dm_err->debug_flag > 0))
     CALL disp_msg(" ",dm_err->logfile,0)
    ENDIF
    IF (dm2_push_cmd(concat("rdb alter trigger ",dmut_trigs->qual[dmut_op_cnt].owner,".",dmut_trigs->
      qual[dmut_op_cnt].trig_name," ",
      dmut_status_in," go"),1)=0)
     GO TO exit_program
    ENDIF
  ENDFOR
 ENDIF
 SUBROUTINE dmut_remove_reconnect_target(null)
   DECLARE drrt_connect_str = vc WITH protect, noconstant("")
   SET dm_err->eproc = "Disconnect and Reconnect to Target Database."
   IF ((dm_err->debug_flag > 0))
    CALL disp_msg("",dm_err->logfile,0)
   ENDIF
   SET dm2_install_schema->dbase_name = dmr_mig_data->tgt_db_name
   ...
   IF ((dm2_install_schema->p_word != "NONE"))
    IF ((dm2_install_schema->connect_str > " "))
     SET drrt_connect_str = cnvtlower(build(dm2_install_schema->u_name,"/",dm2_install_schema->p_word,
 ...
 END ;Subroutine
#exit_program
 CALL dmut_remove_reconnect_target(null)
 SET dm_err->eproc = "dm2_mig_updt_triggers completed successfully"
 CALL final_disp_msg("dm2_mig_updt_trig")
END GO
