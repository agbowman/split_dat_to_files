CREATE PROGRAM dm_oucb_patient_event_deta_drr:dba
 ...
 DECLARE cust_ucb_del_event(dummy) = i4
 DECLARE cust_ucb_del_detail(dummy) = i4
 DECLARE initialize_hist_id(null) = i4
 DECLARE encntr_event_ind = i2 WITH protect, noconstant(0)
 DECLARE parent_entity_id = i4 WITH protect, noconstant(0)
 ...
 IF ((dm_cmb_cust_script->exc_maint_ind=1))
  ...
  SET dcem_request->qual[1].child_entity = "PATIENT_EVENT_DETA3085DRR"
  SET dcem_request->qual[1].op_type = "UNCOMBINE"
  SET dcem_request->qual[1].script_name = "dm_oucb_patient_event_deta_drr"
 ...
 IF ((rchildren->qual1[det_cnt].combine_action_cd=del))
  IF ("PATIENT_EVENT4050DRR"=trim(rchildren->qual1[det_cnt].attribute_name))
   IF (cust_ucb_del_event(null)=0)
    GO TO exit_sub
   ENDIF
  ELSE
   IF (cust_ucb_del_detail(null)=0)
    GO TO exit_sub
   ENDIF
  ENDIF
 ...
 ELSE
  ...
  GO TO exit_sub
 ENDIF
 SUBROUTINE cust_ucb_del_event(dummy)
   SET encntr_event_ind = false
   SET parent_entity_id = 0
   SELECT INTO "nl:"
    pe.encntr_id, pe.person_id
    FROM patient_event4050drr pe
    WHERE (pe.patient_event_id=rchildren->qual1[det_cnt].entity_pk[1].data_number)
    DETAIL
     IF (pe.encntr_id > 0)
   ...
   IF (parent_entity_id > 0)
    UPDATE  FROM patient_event4050drr pe
     SET pe.active_ind = rchildren->qual1[det_cnt].prev_active_ind, pe.active_status_cd = rchildren->
      qual1[det_cnt].prev_active_status_cd, pe.active_status_dt_tm = cnvtdatetime(trans_dt_tm),
      pe.active_status_prsnl_id = reqinfo->updt_id, pe.updt_cnt = (pe.updt_cnt+ 1), pe.updt_id =
      reqinfo->updt_id,
      pe.updt_applctx = reqinfo->updt_applctx, pe.updt_task = reqinfo->updt_task, pe.updt_dt_tm =
    ...
    IF (add_event_hist(rchildren->qual1[det_cnt].entity_pk[1].data_number)=0)
     SET ucb_failed = insert_error
     SET error_table = rchildren->qual1[det_cnt].entity_name
     SET request->error_message = substring(1,132,build(
       "Could not insert history for patient_event4050drr with pk val=",rchildren->qual1[det_cnt].
   ...
   ENDIF
 END ;Subroutine
 SUBROUTINE cust_ucb_del_detail(dummy)
   SET encntr_event_ind = false
   SET parent_entity_id = 0
   SELECT INTO "nl:"
    pe.encntr_id, pe.person_id
    FROM patient_event_deta3085drr ped,
     patient_event4050drr pe
    PLAN (ped
     WHERE (ped.patient_event_detail_id=rchildren->qual1[det_cnt].entity_pk[1].data_number)
      AND ped.pe_value_id=evaluate2(
      IF ("PE_VALUE_ID"=trim(rchildren->qual1[det_cnt].attribute_name)) request->xxx_uncombine[
       ucb_cnt].from_xxx_id
      ELSE ped.pe_value_id
      ENDIF
      ))
     JOIN (pe
     WHERE pe.patient_event_id=ped.patient_event_id)
    DETAIL
     IF (pe.encntr_id > 0)
      encntr_event_ind = true, parent_entity_id = pe.encntr_id
     ELSE
   ...
   ;end select
   IF (parent_entity_id > 0)
    UPDATE  FROM patient_event_deta3085drr ped
     SET ped.active_ind = rchildren->qual1[det_cnt].prev_active_ind, ped.active_status_cd = rchildren
      ->qual1[det_cnt].prev_active_status_cd, ped.active_status_dt_tm = cnvtdatetime(trans_dt_tm),
      ped.active_status_prsnl_id = reqinfo->updt_id, ped.updt_cnt = (ped.updt_cnt+ 1), ped.updt_id =
      reqinfo->updt_id,
      ped.updt_applctx = reqinfo->updt_applctx, ped.updt_task = reqinfo->updt_task, ped.updt_dt_tm =
      cnvtdatetime(trans_dt_tm),
      ped.pe_value_id = evaluate2(
       IF ("PE_VALUE_ID"=trim(rchildren->qual1[det_cnt].attribute_name)) request->xxx_uncombine[
        ucb_cnt].to_xxx_id
       ELSE ped.pe_value_id
       ENDIF
    ...
    ENDIF
    SET activity_updt_cnt += 1
    IF (add_detail_hist(rchildren->qual1[det_cnt].entity_pk[1].data_number)=0)
     SET ucb_failed = insert_error
     SET error_table = rchildren->qual1[det_cnt].entity_name
     SET request->error_message = substring(1,132,build(
       "Could not insert history for patient_event_deta3085drr with pk val=",rchildren->qual1[det_cnt
       ].entity_pk[1].data_number))
     RETURN(0)
    ENDIF
    RETURN(1)
 ...
 SUBROUTINE cust_ucb_upt(dummy)
   ...
   SELECT INTO "nl:"
    ...
    PLAN (ped
     WHERE (ped.patient_event_detail_id=rchildren->qual1[det_cnt].entity_pk[1].data_number)
      AND (ped.pe_value_id=request->xxx_uncombine[ucb_cnt].from_xxx_id))
     JOIN (pe
     WHERE pe.patient_event_id=ped.patient_event_id)
    DETAIL
     IF (pe.encntr_id > 0)
   ...
   IF (parent_entity_id > 0)
    UPDATE  FROM patient_event_deta3085drr ped
     SET ped.updt_cnt = (ped.updt_cnt+ 1), ped.updt_id = reqinfo->updt_id, ped.updt_applctx = reqinfo
      ->updt_applctx,
      ped.updt_task = reqinfo->updt_task, ped.updt_dt_tm = cnvtdatetime(trans_dt_tm), ped.pe_value_id
    ...
    IF (curqual=0)
     ...
     SET error_table = rchildren->qual1[det_cnt].entity_name
     SET request->error_message = substring(1,132,build("Uncombine failed to update pk val=",
       rchildren->qual1[det_cnt].entity_pk[1].data_number))
     RETURN(0)
    ENDIF
    SET activity_updt_cnt += 1
 ...
 SUBROUTINE initialize_hist_id(null)
   IF (v_hist_init_ind=0)
    ...
    SET dcipht_request->transaction_reason_txt = "DM_OUCB_PATIENT_EVENT_DETA_DRR"
 ...
 SUBROUTINE (add_detail_hist(s_at_to_parent_pk_id=f8) =i4)
   ...
   SET dm_cmb_cust_cols2->tbl_name = "PATIENT_EVENT_DETA4647DRR"
   SET dm_cmb_cust_cols2->sub_select_from_tbl = "PATIENT_EVENT_DETA3085DRR"
 ...
 SUBROUTINE (add_event_hist(s_at_to_parent_pk_id=f8) =i4)
   ...
   SET dm_cmb_cust_cols2->where_col_val[1].col_value = build(s_at_to_parent_pk_id)
   SET dm_cmb_cust_cols2->tbl_name = "PATIENT_EVENT_HIST4646DRR"
   SET dm_cmb_cust_cols2->sub_select_from_tbl = "PATIENT_EVENT4050DRR"
   EXECUTE dm_cmb_get_cust_cols  WITH replace("DM_CMB_CUST_COLS","DM_CMB_CUST_COLS2")
   IF ((dm_err->err_ind=1))
    RETURN(0)
   ENDIF
