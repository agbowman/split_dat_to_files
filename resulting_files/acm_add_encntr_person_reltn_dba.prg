CREATE PROGRAM acm_add_encntr_person_reltn:dba
 ...
 FOR (index = 1 TO xref->add_cnt)
   IF ((acm_request->encntr_person_reltn_qual[xref->add[index].idx].encntr_person_reltn_id <= 0.0))
    SELECT INTO "nl:"
     nextseqnum = seq(encounter_seq,nextval)
     FROM dual
     DETAIL
      acm_request->encntr_person_reltn_qual[xref->add[index].idx].encntr_person_reltn_id = cnvtreal(
       nextseqnum), reply->encntr_person_reltn_qual[xref->add[index].idx].encntr_person_reltn_id =
      cnvtreal(nextseqnum)
     WITH nocounter, format
    ;end select
    IF (curqual=0)
     SET reply->encntr_person_reltn_qual[xref->add[index].idx].status = gen_nbr_error
     SET failed = gen_nbr_error
     GO TO exit_script
    ENDIF
   ELSE
    SET reply->encntr_person_reltn_qual[xref->add[index].idx].encntr_person_reltn_id = acm_request->
    encntr_person_reltn_qual[xref->add[index].idx].encntr_person_reltn_id
   ENDIF
   SET reply->encntr_person_reltn_qual[xref->add[index].idx].status = 0
   IF ((acm_request->encntr_person_reltn_qual[xref->add[index].idx].beg_effective_dt_tm <= 0))
    SET acm_request->encntr_person_reltn_qual[xref->add[index].idx].beg_effective_dt_tm =
    cnvtdatetime(curdate,curtime3)
   ENDIF
   IF ((acm_request->encntr_person_reltn_qual[xref->add[index].idx].end_effective_dt_tm <= 0))
    SET acm_request->encntr_person_reltn_qual[xref->add[index].idx].end_effective_dt_tm =
    cnvtdatetime("31-DEC-2100 00:00:00.00")
   ENDIF
 ENDFOR
 INSERT  FROM (dummyt d  WITH seq = value(xref->add_cnt)),
   encntr_person_reltn e
  SET e.encntr_person_reltn_id = acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].
   encntr_person_reltn_id, e.beg_effective_dt_tm = cnvtdatetime(acm_request->
    encntr_person_reltn_qual[xref->add[d.seq].idx].beg_effective_dt_tm), e.contact_role_cd =
   acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].contact_role_cd,
   e.contributor_system_cd =
   IF ((acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].contributor_system_cd > 0.0))
    acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].contributor_system_cd
   ELSE pmhc_contributory_system_cd
   ENDIF
   , e.encntr_id =
   IF ((acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].encntr_id > 0)) acm_request->
    encntr_person_reltn_qual[xref->add[d.seq].idx].encntr_id
   ELSE reply->encounter_qual[acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].encntr_idx]
    .encntr_id
   ENDIF
   , e.end_effective_dt_tm = cnvtdatetime(acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx]
    .end_effective_dt_tm),
   e.free_text_cd = acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].free_text_cd, e
   .ft_rel_person_name = acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].
   ft_rel_person_name, e.genetic_relationship_ind = acm_request->encntr_person_reltn_qual[xref->add[d
   .seq].idx].genetic_relationship_ind,
   e.internal_seq = acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].internal_seq, e
   .living_with_ind = acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].living_with_ind, e
   .person_reltn_cd = acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].person_reltn_cd,
   e.person_reltn_type_cd = acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].
   person_reltn_type_cd, e.priority_seq = acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx]
   .priority_seq, e.related_person_id = acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].
   related_person_id,
   e.related_person_reltn_cd = acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].
   related_person_reltn_cd, e.visitation_allowed_cd = acm_request->encntr_person_reltn_qual[xref->
   add[d.seq].idx].visitation_allowed_cd, e.data_status_cd =
   IF ((acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].data_status_cd > 0.0))
    acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].data_status_cd
   ELSE reqdata->data_status_cd
   ENDIF
   ,
   e.data_status_dt_tm = cnvtdatetime(curdate,curtime3), e.data_status_prsnl_id = reqinfo->updt_id, e
   .active_ind =
   IF ((((acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].active_status_cd=0.0)) OR ((
   acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].active_status_cd=reqdata->
   active_status_cd))) ) 1
   ELSE 0
   ENDIF
   ,
   e.active_status_cd =
   IF ((acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].active_status_cd > 0.0))
    acm_request->encntr_person_reltn_qual[xref->add[d.seq].idx].active_status_cd
  ...
  PLAN (d)
   JOIN (e)
  WITH nocounter, status(reply->encntr_person_reltn_qual[xref->add[d.seq].idx].status)
 ;end insert
 FOR (index = 1 TO xref->add_cnt)
   IF ((reply->encntr_person_reltn_qual[xref->add[index].idx].status=0))
