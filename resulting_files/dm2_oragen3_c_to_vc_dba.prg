CREATE PROGRAM dm2_oragen3_c_to_vc:dba
 ...
 ENDIF
 IF (validate(doctv_list->cnt,1)=1
  AND validate(doctv_list->cnt,2)=2)
  FREE RECORD doctv_list
  RECORD doctv_list(
    1 cnt = i4
    1 needs_oragen3_cnt = i4
    1 processed_cnt = i4
    1 qual[*]
      2 owner_name = vc
      2 table_name = vc
      2 column_name_list = vc
      2 needs_oragen3_ind = i2
  )
 ENDIF
 DECLARE doctv_owner = vc WITH protect, noconstant(" ")
 DECLARE doctv_cnt = i4 WITH protect, noconstant(0)
 DECLARE doctv_findcolname = vc WITH protect, noconstant(" ")
 IF (check_logfile("dm2_o3_ctovc",".log","dm2_o3_ctovc log file")=0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Verifying and storing input parameters."
 SET doctv_owner = trim(cnvtupper( $1),3)
 IF (check_error(dm_err->eproc)=1)
  SET dm_err->emsg = "Parameter usage: dm2_oragen3_c_to_vc  '<owner>'"
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  GO TO exit_script
 ENDIF
 ...
 IF ((dm2_rdbms_version->level1 < 11))
  SET dm_err->eproc = concat("Autosuccessing for database < 11g")
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = concat("Gathering candidate tables for c-to-vc ORAGEN3 call for owner: ",
  doctv_owner)
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "nl:"
  FROM dba_tab_columns dt
  WHERE dt.owner=patstring(doctv_owner)
   AND dt.data_type="VARCHAR2"
   AND dt.data_length < 50
  ORDER BY dt.table_name
  HEAD REPORT
   doctv_list->cnt = 0
  HEAD dt.table_name
   doctv_list->cnt = (doctv_list->cnt+ 1)
   IF (mod(doctv_list->cnt,100)=1)
    stat = alterlist(doctv_list->qual,(doctv_list->cnt+ 99))
   ENDIF
   doctv_list->qual[doctv_list->cnt].owner_name = dt.owner, doctv_list->qual[doctv_list->cnt].
   table_name = dt.table_name
  DETAIL
   doctv_list->qual[doctv_list->cnt].column_name_list = build(doctv_list->qual[doctv_list->cnt].
    column_name_list,dt.column_name,":")
  FOOT REPORT
   stat = alterlist(doctv_list->qual,doctv_list->cnt)
  WITH nocounter
 ;end select
 IF (check_error(dm_err->eproc) != 0)
  ...
  GO TO exit_script
 ELSEIF (curqual=0)
  SET dm_err->eproc = concat("No candidate tables found for owner: ",doctv_owner)
  CALL disp_msg("",dm_err->logfile,0)
  GO TO exit_script
 ENDIF
 SET dm_err->eproc = "Cross checking candidate tables against CCL Dictionary..."
 CALL disp_msg(" ",dm_err->logfile,0)
 SELECT INTO "NL:"
  FROM dtable t,
   dtableattr ta,
  ...
  HEAD REPORT
   locndx = 0
  HEAD t.table_name
   locndx = 0, locndx = locateval(locndx,1,doctv_list->cnt,t.table_name,doctv_list->qual[locndx].
    table_name)
  DETAIL
   IF (locndx > 0)
    doctv_findcolname = concat(trim(tl.attr_name),":")
    IF (findstring(doctv_findcolname,doctv_list->qual[locndx].column_name_list,0,0) > 0)
     doctv_list->qual[locndx].needs_oragen3_ind = 1
    ENDIF
   ENDIF
  FOOT  t.table_name
   IF (locndx > 0)
    IF ((doctv_list->qual[locndx].needs_oragen3_ind=1))
     doctv_list->needs_oragen3_cnt = (doctv_list->needs_oragen3_cnt+ 1)
    ENDIF
   ENDIF
  WITH nocounter
 ;end select
 ...
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echorecord(doctv_list)
 ENDIF
 IF ((doctv_list->needs_oragen3_cnt > 0))
  SET dm_err->eproc = build("There are (",doctv_list->needs_oragen3_cnt,
   ") tables needing oragen3 conversion from c to vc.")
  CALL disp_msg(" ",dm_err->logfile,0)
  FOR (x = 1 TO doctv_list->cnt)
    IF ((doctv_list->qual[x].needs_oragen3_ind=1))
     EXECUTE oragen3 value(concat(doctv_list->qual[x].owner_name,".",doctv_list->qual[x].table_name))
     IF ((dm_err->err_ind=1))
      GO TO exit_script
     ENDIF
     SET doctv_list->processed_cnt = (doctv_list->processed_cnt+ 1)
     IF (mod(doctv_list->processed_cnt,100)=1)
      SET dm_err->eproc = build("Table (",doctv_list->processed_cnt,") of (",doctv_list->
       needs_oragen3_cnt,") successfully processed.")
      CALL disp_msg(" ",dm_err->logfile,0)
     ENDIF
    ENDIF
  ENDFOR
 ELSE
  SET dm_err->eproc = "There were no tables found needing oragen3 conversion from c to vc."
  CALL disp_msg(" ",dm_err->logfile,0)
 ENDIF
 FREE RECORD doctv_list
#exit_script
 SET dm_err->eproc = "dm2_oragen_c_to_vc COMPLETED."
 CALL final_disp_msg("dm2_o3_ctovc")
END GO
