CREATE PROGRAM ce_event_modifier_upd:dba
 RECORD reply(
   1 array_size = i4
   1 num_inserted = i4
   1 error_code = i4
 ...
 SET error_code = 0
 UPDATE  FROM ce_event_modifier t,
  ...
  SET t.event_id = evaluate2(
    ...
    ENDIF
    ), t.modifier_cd = evaluate2(
    IF ((request->lst[d.seq].modifier_cd=- (1))) 0
    ELSE request->lst[d.seq].modifier_cd
    ENDIF
    ), t.modifier_value_cd = evaluate2(
    IF ((request->lst[d.seq].modifier_value_cd=- (1))) 0
    ELSE request->lst[d.seq].modifier_value_cd
    ENDIF
    ),
   t.modifier_val_ft = request->lst[d.seq].modifier_val_ft, t.modifier_value_person_id = evaluate2(
    IF ((request->lst[d.seq].modifier_value_person_id=- (1))) 0
    ELSE request->lst[d.seq].modifier_value_person_id
    ENDIF
   ...
   t.group_sequence = request->lst[d.seq].group_sequence, t.item_sequence = request->lst[d.seq].
   item_sequence
  PLAN (d)
   JOIN (t
   WHERE (t.event_id=request->lst[d.seq].event_id)
    AND (t.modifier_cd=request->lst[d.seq].modifier_cd)
