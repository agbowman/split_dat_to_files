CREATE PROGRAM ct_mp_validate_prot:dba
 PROMPT
  "Output to File/Printer/MINE" = "MINE",
  "Protocol ID:" = 0
  WITH outdev, protocolid
 ...
 SUBROUTINE (istaskgranted(task_number=i4(val)) =i2 WITH protect)
   ...
   RETURN(task_granted)
 END ;Subroutine
 SET log_program_name = "ct_mp_validate_prot"
 ...
 SET reply->status_data.status = "F"
 IF (( $PROTOCOLID=0))
  CALL populate_subeventstatus_rec("REQUEST","F","ct_mp_validate_prot","Invalid/Partial Input Data",
   reply)
  GO TO exit_script
 ENDIF
 SELECT INTO "nl:"
  pr_status_meaning = uar_get_code_meaning(pr_m.prot_status_cd), pr_m.beg_effective_dt_tm
  FROM prot_master pr_m
  WHERE (pr_m.prot_master_id= $PROTOCOLID)
  DETAIL
   IF (pr_m.end_effective_dt_tm > cnvtdatetime(sysdate)
    AND pr_status_meaning="ACTIVATED")
    reply->valid_prot = 1
   ELSE
    reply->valid_prot = 0
   ENDIF
