CREATE PROGRAM bhs_rpt_columbia_suic_score_lo
 PROMPT
 ...
 SUBROUTINE query1(dummy)
   SELECT
    pats_s_pat = substring(1,30,colsuic->pats[d1.seq].s_pat), det_s_mrn = substring(1,30,colsuic->
     pats[d1.seq].det[d2.seq].s_mrn), det_d_charted = colsuic->pats[d1.seq].det[d2.seq].d_charted,
    pats_m_pt_total = colsuic->pats[d1.seq].m_pt_total, pats_f_per_high = colsuic->pats[d1.seq].
    f_per_high, pats_f_per_med = colsuic->pats[d1.seq].f_per_med,
    pats_f_per_low = colsuic->pats[d1.seq].f_per_low
    FROM (dummyt d1  WITH seq = size(colsuic->pats,5)),
     (dummyt d2  WITH seq = 1)
    PLAN (d1
     WHERE maxrec(d2,size(colsuic->pats[d1.seq].det,5)))
     JOIN (d2)
    HEAD REPORT
 ...
 SUBROUTINE (headreportsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __unit = vc WITH noconstant(build2(concat("Nurse Unit: ",colsuic->unit),char(0))), protect
   DECLARE __facilty = vc WITH noconstant(build2(concat("Facility: ",colsuic->facility),char(0))),
   protect
   DECLARE __date_range = vc WITH noconstant(build2(concat("Date Range :",format(cnvtdatetime(
     ...
     char(0))), protect
   DECLARE __run_by_per = vc WITH noconstant(build2(concat("Run By : ",colsuic->runby),char(0))),
   protect
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2(
      "Columbia Repeat Since Last Asked Patient Score Report ",char(0)))
    SET rptsd->m_flags = 0
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
 ...
 SUBROUTINE (headpagesectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   IF (ncalc=rpt_render)
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Columbia Score",char(0)))
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.251)
    ...
    SET rptsd->m_width = 0.563
    SET rptsd->m_height = 0.500
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("High Scores > 13",char(0)))
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.251)
    ...
    SET rptsd->m_width = 0.563
    SET rptsd->m_height = 0.500
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Medium Scores >2-12",char(0)))
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.251)
    ...
    SET rptsd->m_width = 0.563
    SET rptsd->m_height = 0.500
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Low Scores 1-2",char(0)))
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    SET rptsd->m_paddingwidth = 0.000
    SET rptsd->m_y = (offsety+ 0.251)
    ...
    SET rptsd->m_width = 0.563
    SET rptsd->m_height = 0.500
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("Average Columbia Score",char(0)))
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 ...
 SUBROUTINE (detailsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __patient_name = vc WITH noconstant(build2(colsuic->pats[d1.seq].s_pat,char(0))), protect
   DECLARE __date_charted = vc WITH noconstant(build2(format(colsuic->pats[d1.seq].det[d2.seq].
      d_charted,"MM/DD/yyyy HH:mm;3;D"),char(0))), protect
   DECLARE __score = vc WITH noconstant(build2(format(colsuic->pats[d1.seq].det[d2.seq].f_score,"###"
      ),char(0))), protect
   IF (d2.seq=1)
    DECLARE __high_scores = vc WITH noconstant(build2(colsuic->pats[d1.seq].m_cnthigh,char(0))),
    protect
   ENDIF
   IF (d2.seq=1)
    DECLARE __medium = vc WITH noconstant(build2(colsuic->pats[d1.seq].m_cntmedium,char(0))), protect
   ENDIF
   IF (d2.seq=1)
    DECLARE __low = vc WITH noconstant(build2(colsuic->pats[d1.seq].m_cntlow,char(0))), protect
   ENDIF
   DECLARE __mrn = vc WITH noconstant(build2(colsuic->pats[d1.seq].det[d2.seq].s_mrn,char(0))),
   protect
   IF (d2.seq=1)
    DECLARE __average = vc WITH noconstant(build2(format(colsuic->pats[d1.seq].f_avg,"###.#"),char(0)
      )), protect
   ENDIF
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 0
 ...
 SUBROUTINE (footreportsectionabs(ncalc=i2,offsetx=f8,offsety=f8) =f8 WITH protect)
   ...
   DECLARE __percent_high = vc WITH noconstant(build2(format(colsuic->f_per_hightot,"###%"),char(0))
    ), protect
   DECLARE __number_results = vc WITH noconstant(build2(format(cnvtreal(colsuic->m_cnttot_results),
      "###"),char(0))), protect
   DECLARE __percent_med = vc WITH noconstant(build2(format(colsuic->f_per_medtot,"###%"),char(0))),
   protect
   DECLARE __percent_low = vc WITH noconstant(build2(format(colsuic->f_per_lowtot,"###%"),char(0))),
   protect
   DECLARE __total_patients_with_score = vc WITH noconstant(build2(build("Total Patients: ",colsuic->
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_width = 0.438
    SET rptsd->m_height = 0.198
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__percent_low)
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "BHS_RPT_COLUMBIA_SUIC_SCORE_LO"
