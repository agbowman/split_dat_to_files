CREATE PROGRAM bsc_get_locs_for_discern_rpts:dba
 ...
 DECLARE logical_dm_ind = i2 WITH protect, noconstant(0)
 DECLARE vnurseunit = vc WITH protect, noconstant("")
 SET v_cv_count = 0
 DECLARE vfacilitycd = c10
 SET vfacility = cnvtreal(request->val2)
    ...
    SELECT DISTINCT INTO "nl:"
     ...
     PLAN (o
      ...
      WHERE cv.code_value=l.location_cd
       ...
       AND ((cv.display_key > cnvtupper(context->start_value)
       AND cv.display_key=value(concat(cnvtupper(request->start_value),"*"))) OR (cv.display_key=
      cnvtupper(context->start_value)
       AND (cv.code_value > context->num1)))
       AND cv.active_ind=1)
      JOIN (nu
     ...
     DETAIL
      v_cv_count += 1, stat = alterlist(reply->datacoll,v_cv_count), vnurseunit = concat(trim(cv
        .display,1),"(",trim(uar_get_code_display(nu.loc_facility_cd,1)),")"),
      reply->datacoll[v_cv_count].description = vnurseunit, reply->datacoll[v_cv_count].currcv = trim
      (format(cv.code_value,"################.##;I;F"),3)
     WITH nocounter
    ;end select
   ELSEIF (vfacility=1)
    SET request->start_value = cnvtalphanum(request->start_value)
    SELECT DISTINCT INTO "nl:"
     ...
     ORDER BY cv.display
     DETAIL
      v_cv_count += 1, stat = alterlist(reply->datacoll,v_cv_count), reply->datacoll[v_cv_count].
      description = cv.display,
      reply->datacoll[v_cv_count].currcv = trim(format(cv.code_value,"################.##;I;F"),3)
     WITH nocounter
    ;end select
   ELSEIF (vfacility=3)
    SET request->start_value = cnvtalphanum(request->start_value)
    SELECT DISTINCT INTO "nl:"
     org_name = o.org_name, o.organization_id
     FROM organization o
     PLAN (o
      WHERE o.active_ind=1
       AND o.beg_effective_dt_tm <= cnvtdatetime(curdatetime)
       AND o.end_effective_dt_tm > cnvtdatetime(curdatetime)
       AND ((o.org_name_key > cnvtupper(context->start_value)
       AND o.org_name_key=value(concat(cnvtupper(trim(request->start_value,4)),"*"))) OR (o
      .org_name_key=cnvtupper(context->start_value)
       AND (o.organization_id > context->num1))) )
     ORDER BY org_name
     DETAIL
      v_cv_count += 1, stat = alterlist(reply->datacoll,v_cv_count), reply->datacoll[v_cv_count].
      description = o.org_name,
      reply->datacoll[v_cv_count].currcv = trim(format(o.organization_id,"################.##;I;F"),3
       )
