CREATE PROGRAM ccps_pha_lbt_medsyr
 PROMPT
 ...
 DECLARE demographics_sec(ncalc=i2) = f8 WITH protect
 DECLARE demographics_secabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE label_desc_sec(ncalc=i2) = f8 WITH protect
 DECLARE label_desc_secabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE dose_brand_name_sec(ncalc=i2) = f8 WITH protect
 DECLARE dose_brand_name_secabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE foot_sec(ncalc=i2) = f8 WITH protect
 DECLARE foot_secabs(ncalc=i2,offsetx=f8,offsety=f8) = f8 WITH protect
 DECLARE loc_room_bed_dummy(ncalc=i2,maxheight=f8,bcontinue=i2(ref)) = f8 WITH protect
 DECLARE loc_room_bed_dummyabs(ncalc=i2,offsetx=f8,offsety=f8,maxheight=f8,bcontinue=i2(ref)) = f8
 WITH protect
 ...
 DECLARE _remperson_loc_room_bed = i4 WITH noconstant(1), protect
 DECLARE _bholdcontinue = i2 WITH noconstant(0), protect
 DECLARE _bcontloc_room_bed_dummy = i2 WITH noconstant(0), protect
 DECLARE _default60 = i4 WITH noconstant(0), protect
 DECLARE _default100 = i4 WITH noconstant(0), protect
 DECLARE _default816777215 = i4 WITH noconstant(0), protect
 DECLARE _default80 = i4 WITH noconstant(0), protect
 DECLARE _default1016777215 = i4 WITH noconstant(0), protect
 DECLARE _default616777215 = i4 WITH noconstant(0), protect
 ...
 END ;Subroutine
 SUBROUTINE demographics_sec(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = demographics_secabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE demographics_secabs(ncalc,offsetx,offsety)
   ...
   DECLARE __syringe_literal = vc WITH noconstant(build2(build2(trim(label_data->med.route),
      " Syringe"),char(0))), protect
   ...
   IF (ncalc=rpt_render)
    ...
    IF (loc_cond=8)
     SET _fntcond = _default816777215
    ELSEIF (loc_cond=6)
     SET _fntcond = _default616777215
    ELSE
     SET _fntcond = _default1016777215
    ENDIF
    SET rptsd->m_y = (offsety+ 0.000)
    ...
    SET oldbackcolor = uar_rptsetbackcolor(_hreport,rpt_black)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__person_loc_room_bed)
    SET oldbackcolor = uar_rptresetbackcolor(_hreport)
    ...
    SET _dummyfont = uar_rptsetfont(_hreport,_default816777215)
    SET oldbackcolor = uar_rptsetbackcolor(_hreport,rpt_black)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__syringe_literal)
    SET oldbackcolor = uar_rptresetbackcolor(_hreport)
    SET rptsd->m_flags = 0
    SET rptsd->m_y = (offsety+ 0.167)
    SET rptsd->m_x = (offsetx+ 0.002)
 ...
 END ;Subroutine
 SUBROUTINE label_desc_sec(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = label_desc_secabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE label_desc_secabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.170000), private
   DECLARE __label_description = vc WITH noconstant(build2(trim(label_data->ingredients[ingred_idx].
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE dose_brand_name_sec(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = dose_brand_name_secabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE dose_brand_name_secabs(ncalc,offsetx,offsety)
   DECLARE sectionheight = f8 WITH noconstant(0.170000), private
   IF ((label_data->ingredients[ingred_idx].identifiers.brand_name > " "))
   ...
   ENDIF
   DECLARE __ingredients_freetext_dose = vc WITH noconstant(build2(concat("Dose: ",replace(trim(
        label_data->ingredients[ingred_idx].freetext_dose)," / "," = ")),char(0))), protect
   IF (ncalc=rpt_render)
    SET rptsd->m_flags = 8
    SET rptsd->m_borders = rpt_sdnoborders
    SET rptsd->m_padding = rpt_sdnoborders
    ...
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,__ingredients_freetext_dose)
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ENDIF
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE foot_sec(ncalc)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = foot_secabs(ncalc,_xoffset,_yoffset)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE foot_secabs(ncalc,offsetx,offsety)
   ...
   IF (ncalc=rpt_render)
    ...
    SET rptsd->m_rotationangle = 0
    SET rptsd->m_y = (offsety+ 0.000)
    SET rptsd->m_x = (offsetx+ 1.616)
    SET rptsd->m_width = 0.458
    SET rptsd->m_height = 0.198
    SET _oldfont = uar_rptsetfont(_hreport,_default80)
    SET _oldpen = uar_rptsetpen(_hreport,_pen14s0c0)
    SET _fdrawheight = uar_rptstringdraw(_hreport,rptsd,build2("EXPIRES:",char(0)))
    SET rptsd->m_flags = 32
    SET rptsd->m_y = (offsety+ 0.135)
    SET rptsd->m_x = (offsetx+ 1.731)
    ...
    SET _rptdummy = uar_rptbarcodeinit(rptbce,rpt_aztec,(offsetx+ 2.564),(offsety+ 0.000))
    SET rptbce->m_recsize = 88
    SET rptbce->m_width = 0.20
    SET rptbce->m_height = 0.20
    ...
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 0.273),(offsety+ 0.109),(offsetx+ 0.711),(offsety+
     0.109))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 1.148),(offsety+ 0.109),(offsetx+ 1.586),(offsety+
     0.109))
    SET _rptstat = uar_rptline(_hreport,(offsetx+ 2.085),(offsety+ 0.109),(offsetx+ 2.523),(offsety+
     0.109))
    SET _dummyfont = uar_rptsetfont(_hreport,_oldfont)
    SET _dummypen = uar_rptsetpen(_hreport,_oldpen)
    SET _yoffset = (offsety+ sectionheight)
   ...
   RETURN(sectionheight)
 END ;Subroutine
 SUBROUTINE loc_room_bed_dummy(ncalc,maxheight,bcontinue)
   DECLARE a1 = f8 WITH noconstant(0.0), private
   SET a1 = loc_room_bed_dummyabs(ncalc,_xoffset,_yoffset,maxheight,bcontinue)
   RETURN(a1)
 END ;Subroutine
 SUBROUTINE loc_room_bed_dummyabs(ncalc,offsetx,offsety,maxheight,bcontinue)
   DECLARE sectionheight = f8 WITH noconstant(0.170000), private
 ...
 SUBROUTINE initializereport(dummy)
   ...
   SET rptreport->m_reportname = "CCPS_PHA_LBT_MEDSYR"
 ...
 SUBROUTINE _createfonts(dummy)
   ...
   SET _default816777215 = uar_rptcreatefont(_hreport,rptfont)
   SET rptfont->m_pointsize = 6
   SET _default616777215 = uar_rptcreatefont(_hreport,rptfont)
   SET rptfont->m_pointsize = 8
   SET rptfont->m_rgbcolor = rpt_black
 ...
 SET loc_cond = 10
 SET _yoffset = 0.17
 SET loc_room_bed_dummy_ht = 0.0
 SET loc_room_bed_dummy_ht = loc_room_bed_dummy(rpt_calcheight,(_fenddetail - _yoffset),
  _bholdcontinue)
 CALL echo(build2("loc_room_bed_dummy_ht  = ",loc_room_bed_dummy_ht))
 CALL echo(build2("loc_cond  = ",loc_cond))
 IF (loc_room_bed_dummy_ht > 0.17)
  SET loc_cond = 8
  SET loc_room_bed_dummy_ht = loc_room_bed_dummy(rpt_calcheight,(_fenddetail - _yoffset),
   _bholdcontinue)
  CALL echo(build2("loc_room_bed_dummy_ht  = ",loc_room_bed_dummy_ht))
 ...
 ENDIF
 CALL pagebreak(0)
 SET _fdrawheight = demographics_sec(rpt_calcheight)
 IF (((_yoffset+ _fdrawheight) > _fenddetail))
  CALL pagebreak(0)
 ENDIF
 SET dummy_val = demographics_sec(rpt_render)
 FOR (ingred_idx = 1 TO size(label_data->ingredients,5))
   SET _fdrawheight = label_desc_sec(rpt_calcheight)
   IF (((_yoffset+ _fdrawheight) > _fenddetail))
    CALL pagebreak(0)
   ENDIF
   SET dummy_val = label_desc_sec(rpt_render)
   SET _fdrawheight = dose_brand_name_sec(rpt_calcheight)
   IF (((_yoffset+ _fdrawheight) > _fenddetail))
    CALL pagebreak(0)
   ENDIF
   SET dummy_val = dose_brand_name_sec(rpt_render)
 ENDFOR
 SET _fdrawheight = foot_sec(rpt_calcheight)
 IF (((_yoffset+ _fdrawheight) > _fenddetail))
  CALL pagebreak(0)
 ENDIF
 SET dummy_val = foot_sec(rpt_render)
