CREATE PROGRAM dm2_ref_data_mover_2350:dba
 ...
 SUBROUTINE dm2_rdds_get_tgt_id(s_gmti_tgt_rs)
   ...
   RETURN(drgmti_return_val)
 END ;Subroutine
 DECLARE dx_tab_name = vc
 DECLARE dx_check_null = i2
 DECLARE dx_from_pen = vc
 DECLARE dx_pe_table = vc
 DECLARE dx_pe_column = vc
 DECLARE dx_from_pe_id = vc
 DECLARE dx_to_pe_id = vc
 DECLARE dx_dmt_cnt = i4
 DECLARE dx_idcd_check = i2
 DECLARE dx_ins_upd = i2
 SET dx_dmt_cnt = 0
 SET dx_tab_name = dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].table_name
 SET dx_check_null = get_nullind(dx_tab_name,"PARENT_ENTITY_NAME")
 SET dx_from_pen = get_value(dx_tab_name,"PARENT_ENTITY_NAME","FROM")
 SET dx_from_pe_id = get_value(dx_tab_name,"PARENT_ENTITY_ID","FROM")
 IF ((dm2_ref_data_reply->error_ind=1))
  GO TO exit_2350
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("dx_from_pen: ",dx_from_pen))
 ENDIF
 SET stat = alterlist(rdds_exception->qual,1)
 CASE (dx_from_pen)
  OF "DEVICE":
   SET dx_pe_table = "DEVICE"
   SET dx_pe_column = "DEVICE_CD"
   SET rdds_exception->qual[1].tab_col_name = "PARENT_ENTITY_ID"
   SET rdds_exception->qual[1].tru_tab_name = "DEVICE"
   SET rdds_exception->qual[1].tru_col_name = "DEVICE_CD"
  OF "LOCATION":
   SET dx_pe_table = "CODE_VALUE"
   SET dx_pe_column = "CODE_VALUE"
   SET rdds_exception->qual[1].tab_col_name = "PARENT_ENTITY_ID"
   SET rdds_exception->qual[1].tru_tab_name = "CODE_VALUE"
   SET rdds_exception->qual[1].tru_col_name = "CODE_VALUE"
  OF "ORGANIZATION":
   SET dx_pe_table = "ORGANIZATION"
   SET dx_pe_column = "ORGANIZATION_ID"
   SET rdds_exception->qual[1].tab_col_name = "PARENT_ENTITY_ID"
   SET rdds_exception->qual[1].tru_tab_name = "ORGANIZATION"
   SET rdds_exception->qual[1].tru_col_name = "ORGANIZATION_ID"
  OF "PRINTER / QUEUE":
   SET dx_pe_table = "DEVICE"
   SET dx_pe_column = "DEVICE_CD"
   SET rdds_exception->qual[1].tab_col_name = "PARENT_ENTITY_ID"
   ...
   SET rdds_exception->qual[1].tru_col_name = "DEVICE_CD"
  OF "PRSNL":
   SET dx_pe_table = "PRSNL"
   SET dx_pe_column = "PERSON_ID"
   SET rdds_exception->qual[1].tab_col_name = "PARENT_ENTITY_ID"
   SET rdds_exception->qual[1].tru_tab_name = "PRSNL"
   SET rdds_exception->qual[1].tru_col_name = "PERSON_ID"
  OF "QUEUE / NODE":
   SET dx_pe_table = "DEVICE"
   SET dx_pe_column = "DEVICE_CD"
   SET rdds_exception->qual[1].tab_col_name = "PARENT_ENTITY_ID"
  ...
  ELSE
   ...
   SET dm2_ref_data_reply->error_msg = concat(dm2_ref_data_doc->tbl_qual[temp_tbl_cnt].custom_script,
    ": The PARENT_ENTITY_NAME <",dx_from_pen,"> was not recognized.")
   GO TO exit_2350
 ENDCASE
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("dx_pe_table: ",dx_pe_table))
  CALL echo(concat("dx_pe_column: ",dx_pe_column))
 ENDIF
 SET dx_to_pe_id = select_merge_translate(dx_from_pe_id,dx_pe_table)
 IF (dx_to_pe_id != "No Trans")
  CALL put_value(dx_tab_name,"PARENT_ENTITY_ID",dx_to_pe_id)
  IF ((dm2_ref_data_reply->error_ind=1))
   GO TO exit_2350
  ENDIF
 ENDIF
 IF ((dm_err->debug_flag > 0))
  CALL echo(concat("dx_to_pe_id: ",dx_to_pe_id))
 ENDIF
 SET dx_idcd_check = 0
 CALL echo("")
 CALL echo("")
 ...
 CALL echo("")
 CALL echo("")
 SET dx_idcd_check = is_translated(dx_tab_name,"ALL")
 IF (drdm_error_out_ind=1)
  GO TO exit_2350
 ENDIF
 IF (dx_idcd_check=1)
  SET dx_ins_upd = insert_update_row(temp_tbl_cnt,perm_col_cnt)
 ...
 ENDIF
#exit_2350
 FREE SET dx_tab_name
 FREE SET dx_check_null
 FREE SET dx_from_pen
 FREE SET dx_pe_table
 FREE SET dx_pe_column
 FREE SET dx_from_pe_id
 FREE SET dx_dmt_cnt
 FREE SET dx_to_pe_id
 FREE SET dx_idcd_check
 FREE SET dx_ins_upd
END GO
