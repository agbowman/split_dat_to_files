CREATE PROGRAM bhs_rw_cam_icu_genview:dba
 ...
 ELSE
  ...
  GO TO exit_script
 ENDIF
 SET work->form_desc = "Confusion Assessment Method for ICU"
 SELECT INTO "NL:"
  e.person_id
  FROM encounter e
  PLAN (e
 ...
 IF ((work->s_cnt=0))
  ...
  GO TO print_output
 ENDIF
 SET reply->text = build2(beg_doc,"\fs20\b\ul Confusion Assessment Method (CAM-ICU)\b0\ul0\par",
  newline)
