CREATE PROGRAM bed_aud_glb_dta_ec_es:dba
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   .task_assay_cd,
   temp->dlist[dcnt].dtaname = dta.mnemonic, temp->dlist[dcnt].dtadesc = dta.description, temp->
   dlist[dcnt].event_cd = vec.event_cd,
   temp->dlist[dcnt].event_disp = vec.event_cd_disp, temp->dlist[dcnt].event_desc = vec
   .event_cd_descr, temp->dlist[dcnt].event_set_cd = vesc.event_set_cd,
   temp->dlist[dcnt].event_set_disp = vesc.event_set_cd_disp, temp->dlist[dcnt].event_set_desc = vesc
   .event_set_cd_descr
   IF (((dta.mnemonic != vec.event_cd_disp) OR (dta.mnemonic != vesc.event_set_cd_disp)) )
 ...
 SET reply->collist[8].header_text = "task_assay_cd"
 SET reply->collist[8].data_type = 2
 SET reply->collist[8].hide_ind = 1
 SET reply->collist[9].header_text = "event_cd"
 SET reply->collist[9].data_type = 2
 SET reply->collist[9].hide_ind = 1
 SET reply->collist[10].header_text = "event_set_cd"
 ...
 SELECT INTO "nl:"
  ...
  DETAIL
   ...
   ].string_value = temp->dlist[d.seq].event_disp,
   reply->rowlist[row_nbr].celllist[4].string_value = temp->dlist[d.seq].event_desc, reply->rowlist[
   row_nbr].celllist[5].string_value = temp->dlist[d.seq].event_set_disp, reply->rowlist[row_nbr].
   celllist[6].string_value = temp->dlist[d.seq].event_set_desc
   IF ((temp->dlist[d.seq].diff_ind=1))
    reply->rowlist[row_nbr].celllist[7].string_value = "X"
   ENDIF
   reply->rowlist[row_nbr].celllist[8].double_value = temp->dlist[d.seq].task_assay_cd, reply->
   rowlist[row_nbr].celllist[9].double_value = temp->dlist[d.seq].event_cd, reply->rowlist[row_nbr].
   celllist[10].double_value = temp->dlist[d.seq].event_set_cd
  WITH nocounter
 ;end select
#exit_script
