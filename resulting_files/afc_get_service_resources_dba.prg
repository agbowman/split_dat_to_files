CREATE PROGRAM afc_get_service_resources:dba
 DECLARE versionnbr = vc
 SET versionnbr = "000"
 CALL echo(build("AFC_GET_SERVICE_RESOURCES Version: ",versionnbr))
 RECORD reply(
   1 service_resource[*]
 ...
 IF ( NOT (getlogicaldomain(ld_concept_organization,curlogicaldomainid)))
  GO TO end_program
 ENDIF
 IF (trim(request->serviceresourcetype)="")
  GO TO end_program
 ENDIF
 SET occurances = 1
 SET stat = uar_get_meaning_by_codeset(code_set,nullterm(request->serviceresourcetype),occurances,
  codevalue)
 SET stat = memrealloc(code_list,occurances,"f8")
 CALL uar_get_code_list_by_meaning(code_set,nullterm(request->serviceresourcetype),start_index,
  occurances,total_remaining,
  code_list)
 IF (size(code_list,5) > 0)
  SELECT INTO "nl:"
   ...
   DETAIL
    count2 += 1
    IF (mod(count2,10)=1)
     stat = alterlist(reply->service_resource,(count2+ 9))
    ENDIF
    reply->service_resource[count2].service_resource_cd = sr.service_resource_cd
   WITH nocounter, expand = 1
  ;end select
  SET stat = alterlist(reply->service_resource,count2)
  IF (curqual > 0)
