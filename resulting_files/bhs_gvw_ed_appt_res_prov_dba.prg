CREATE PROGRAM bhs_gvw_ed_appt_res_prov:dba
 ...
 SELECT INTO "nl:"
  FROM sch_appt a,
   sch_event_disp ed
  PLAN (a
   WHERE (a.encntr_id=request->visit[1].encntr_id)
    AND a.active_ind=1
    AND a.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
   JOIN (ed
   WHERE ed.sch_event_id=a.sch_event_id
    AND ed.disp_field_meaning="PRIMARYRES"
    AND ed.version_dt_tm=cnvtdatetime("31-DEC-2100 00:00:00.00"))
  ORDER BY a.beg_dt_tm DESC
  HEAD REPORT
   reply->text = concat(ms_rhead,"{",trim(ed.disp_display,3),"}",ms_reol,
    ms_rtfeof)
  WITH nocounter
 ;end select
#exit_script
