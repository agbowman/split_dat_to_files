CREATE PROGRAM djh_cid_detail_rpt_v0:dba
 PROMPT
 ...
 SELECT INTO "nl:"
  ...
  PLAN (pr
   WHERE pr.physician_ind >= 0
    AND pr.active_ind >= 0
    AND (pr.person_id= $CID))
   JOIN (pa
   WHERE pa.person_id=pr.person_id
    AND pa.active_ind >= 0)
  ORDER BY pr.person_id, pr.person_id, pa.active_ind DESC,
   pa.alias_pool_cd
  HEAD REPORT
 ...
 SELECT INTO  $1
  ...
  HEAD PAGE
   col 5, line, row + 1,
   col 50, "CID Detail Report", row + 1,
   col 5, line, row + 1
  DETAIL
