CREATE PROGRAM dm2_make_merge_trans_child:dba
 ...
 SUBROUTINE dir_get_admin_db_link(dgadl_report_fail_ind,dgadl_admin_db_link,dgadl_fail_ind)
   ...
   RETURN(1)
 END ;Subroutine
 DECLARE mrg_cnt = i4 WITH protect, noconstant(0)
 DECLARE dmmt_finished = i4 WITH protect, noconstant(0)
 DECLARE dmmt_insert_cnt = i4 WITH protect, noconstant(0)
 DECLARE query_size = i4 WITH protect, constant(5000)
 FREE RECORD make_mrg
 RECORD make_mrg(
   1 value_cnt = i4
   1 list[*]
     2 from_value = f8
  ...
  PLAN (s
   ...
   (SELECT
    ...
    WHERE d.from_value=t.from_value
     AND d.table_name=t.table_name
     AND d.env_source_id=mrgsrc_env_id
     AND d.env_target_id=cur_env_id))))
  DETAIL
   mrg_cnt = (mrg_cnt+ 1)
   IF (mod(mrg_cnt,5000)=1)
    stat = alterlist(make_mrg->list,(mrg_cnt+ 4999))
   ENDIF
   make_mrg->list[mrg_cnt].from_value = s.from_value, make_mrg->list[mrg_cnt].table_name = s
   .table_name
  FOOT REPORT
   make_mrg->value_cnt = mrg_cnt, stat = alterlist(make_mrg->list,mrg_cnt)
  WITH nocounter, maxqual(t,5000)
 ;end select
 IF (check_error("ERROR: selecting data from current database and remote database ") != 0)
  CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
  SET mrgins_flag = 1
  GO TO exit_program
 ENDIF
 ...
 ENDIF
 SET dmmt_finished = 0
 SET dmmt_insert_cnt = 0
 WHILE ((dmmt_finished < make_mrg->value_cnt))
   SET dmmt_finished = (dmmt_insert_cnt+ 1000)
   INSERT  FROM dm_merge_translate dmt,
     (dummyt d  WITH seq = value(dmmt_finished))
    SET dmt.from_value = make_mrg->list[d.seq].from_value, dmt.table_name = make_mrg->list[d.seq].
     table_name, dmt.env_source_id = mrgsrc_env_id,
     dmt.env_target_id = cur_env_id, dmt.to_value = make_mrg->list[d.seq].from_value, dmt.status_flg
      = 0,
     dmt.error_nbr = 0, dmt.error_msg = null, dmt.merge_id = 0
    PLAN (d
     WHERE d.seq > dmmt_insert_cnt
      AND (d.seq <= make_mrg->value_cnt))
     JOIN (dmt)
    WITH nocounter
   ;end insert
   IF (check_error("ERROR occurred when inserting rows into dm_merge_translate") != 0)
    CALL disp_msg(dm_err->emsg,dm_err->logfile,1)
    SET mrgins_flag = 1
    SET dmmt_finished = (make_mrg->value_cnt+ 1)
    GO TO exit_program
   ENDIF
   COMMIT
   SET dmmt_insert_cnt = (dmmt_insert_cnt+ 1000)
 ENDWHILE
 SET stat = alterlist(make_mrg->list,0)
 SET make_mrg->value_cnt = 0
#exit_program
 FREE RECORD make_mrg
 FREE SET mrg_cnt
 FREE SET dmmt_finished
 FREE SET dmmt_insert_cnt
 FREE SET query_size
END GO
