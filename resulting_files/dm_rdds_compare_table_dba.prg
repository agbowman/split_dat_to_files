CREATE PROGRAM dm_rdds_compare_table:dba
 ...
 DECLARE iparm_cnt = i4 WITH protect, noconstant(0)
 DECLARE iwhile_cnt = i4 WITH protect, noconstant(0)
 DECLARE sfuncstr = vc WITH protect, noconstant("")
 ...
 FOR (iidx = 1 TO table_info->tables[itableidx].column_cnt)
   IF ((table_info->tables[itableidx].columns[iidx].custom_object_name > ""))
    SET iwhile_cnt = 1
    SET iparm_cnt = size(table_info->tables[itableidx].columns[iidx].parm_list,5)
    SET sfuncstr = concat(trim(replace(trim(table_info->tables[itableidx].columns[iidx].
        custom_object_name,3),"::SOURCE TARGET MAPPING::",ssrctgtmapid),3),"(2,s.")
    WHILE (iparm_cnt >= iwhile_cnt)
     SET sfuncstr = concat(sfuncstr,table_info->tables[itableidx].columns[iidx].parm_list[iwhile_cnt]
      .column_name,",s.")
     SET iwhile_cnt = (iwhile_cnt+ 1)
    ENDWHILE
    SET sfuncstr = concat(substring(1,(size(sfuncstr) - 3),sfuncstr),")")
    IF ((dm_err->debug_flag >= 2))
     CALL echo("*********************************************************************************")
     CALL echo(sfuncstr)
     CALL echo("*********************************************************************************")
    ENDIF
    CALL parser(concat("declare ",sfuncstr," = ",table_info->tables[itableidx].columns[iidx].
      ccl_data_type," go"))
   ENDIF
   SET icolumncnt = (icolumncnt+ 1)
   SET smergedelete = trim(cnvtstring(table_info->tables[itableidx].columns[iidx].no_backfill_ind),3)
   SET sdefattind = trim(cnvtstring(table_info->tables[itableidx].columns[iidx].
     defining_attribute_ind),3)
   ...
   SET parser_buf_1_2->qual[sdetailsectionidx_1_2].line = concat(
    "audit_info->audit_types[1].audit_values[iCnt].data_values[",trim(cnvtstring(icolumncnt),3),
   ...
   IF ((table_info->tables[itableidx].columns[iidx].no_compare_ind != 1))
    ...
    IF (nsize > 0)
     ...
     IF ((table_info->tables[itableidx].columns[iidx].custom_object_name > ""))
      SET sselectlist_3 = concat(trim(sselectlist_3,3),",",table_info->tables[itableidx].columns[iidx
       ].column_name,"_src = ",sfuncstr)
      IF ((dm_err->debug_flag >= 2))
       CALL echo("*********************************************************************************")
       CALL echo(sselectlist_3)
       CALL echo("*********************************************************************************")
      ENDIF
      SET scondition_3 = concat(trim(scondition_3,3)," OR ",sfuncstr," !=t.",table_info->tables[
       itableidx].columns[iidx].column_name,
       " ")
      IF ((dm_err->debug_flag >= 2))
       CALL echo("*********************************************************************************")
       CALL echo(scondition_3)
       CALL echo("*********************************************************************************")
      ENDIF
     ELSE
     ...
     IF ((table_info->tables[itableidx].columns[iidx].custom_object_name > ""))
      SET sselectlist = concat(trim(sselectlist,3),",",table_info->tables[itableidx].columns[iidx].
       ...
       trim(cnvtstring(imaxerrors),3),"),-1,",sfuncstr,",NULL)^,30)")
     ELSE
      SET sselectlist = concat(trim(sselectlist,3),",",table_info->tables[itableidx].columns[iidx].
       column_name,"_src = ","sqlpassthru(^decode(sign(rownum - ",
       trim(cnvtstring(imaxerrors),3),"),-1,::TRANSLATE(",replace(sdecode,"DECODE(TRIM",
        "DECODE(RTRIM(LTRIM"),substring(2,size(sdecodeparent,1),sdecodeparent),"),",
    ...
    ELSEIF ((table_info->tables[itableidx].columns[iidx].translation_ind=1))
     IF ((table_info->tables[itableidx].columns[iidx].pk_ind != 1))
      ...
      ELSE
       SET scondition_3 = concat(trim(scondition_3,3)," or rdds_diff_translate_src(","'",table_info->
        ...
        trim(cnvtstring(table_info->tables[itableidx].columns[iidx].sequence_match),3),",s.",
        table_info->tables[itableidx].columns[iidx].column_name,",",smergedelete,
        ",",sdefattind,",",trim(cnvtstring(table_info->tables[itableidx].columns[iidx].exception_flg),
         3),") != t.",
        table_info->tables[itableidx].columns[iidx].column_name)
      ENDIF
     ENDIF
     ...
     ELSE
      SET sselectlist_3 = concat(trim(sselectlist_3,3),",",table_info->tables[itableidx].columns[iidx
       ...
       "'",table_info->tables[itableidx].columns[iidx].root_entity_name,"',",trim(cnvtstring(
         table_info->tables[itableidx].columns[iidx].sequence_match),3),",s.",
       table_info->tables[itableidx].columns[iidx].column_name,",",smergedelete,",",sdefattind,
       ",",trim(cnvtstring(table_info->tables[itableidx].columns[iidx].exception_flg),3),")")
     ENDIF
     IF ((table_info->tables[itableidx].columns[iidx].custom_object_name > ""))
      SET sselectlist = concat(trim(sselectlist,3),",",table_info->tables[itableidx].columns[iidx].
       column_name,"_src =","sqlpassthru(^decode(sign(rownum - ",
     ...
     ELSE
      SET sselectlist = concat(trim(sselectlist,3),",",table_info->tables[itableidx].columns[iidx].
       column_name,"_src =","sqlpassthru(^decode(sign(rownum - ",
       trim(cnvtstring(imaxerrors),3),"),-1,::TRANSLATE(","'",table_info->tables[itableidx].columns[
       iidx].root_entity_name,"',",
       trim(cnvtstring(table_info->tables[itableidx].columns[iidx].sequence_match),3),",s.",
       table_info->tables[itableidx].columns[iidx].column_name,",",smergedelete,
       ",",sdefattind,",",trim(cnvtstring(table_info->tables[itableidx].columns[iidx].exception_flg),
        3),"),NULL)^,30)")
    ...
    ELSE
     ...
     SET sselectlist = concat(trim(sselectlist,3),",",table_info->tables[itableidx].columns[iidx].
      column_name,"_src =","sqlpassthru(^decode(sign(rownum - ",
      trim(cnvtstring(imaxerrors),3),"),-1,s.",table_info->tables[itableidx].columns[iidx].
      column_name,",NULL)^,30)")
     SET sselectlist_3 = concat(trim(sselectlist_3,3),",",table_info->tables[itableidx].columns[iidx]
      .column_name,"_src =","s.",
      table_info->tables[itableidx].columns[iidx].column_name)
   ...
   IF ((table_info->tables[itableidx].columns[iidx].pk_ind=1))
    ...
    IF (nsize > 0)
     IF ((table_info->tables[itableidx].columns[iidx].custom_object_name > ""))
      SET sviewselecttrans = concat(",",sfuncstr," as ",table_info->tables[itableidx].columns[iidx].
       column_name,sviewselecttrans)
      SET sjoin_3 = concat(" AND ",sfuncstr," = t.",table_info->tables[itableidx].columns[iidx].
       column_name,sjoin_3)
      SET iviewjoinind = 0
     ELSE
      SET sviewselecttrans = concat(",rdds_diff_translate_src(",sdecode,substring(2,size(
         sdecodeparent,1),sdecodeparent),"),",sdecode,
     ...
     IF ((table_info->tables[itableidx].columns[iidx].translation_ind=1))
      ...
      ELSE
       SET sviewselecttrans = concat(",rdds_diff_translate_src(","'",table_info->tables[itableidx].
        columns[iidx].root_entity_name,"',",trim(cnvtstring(table_info->tables[itableidx].columns[
          iidx].sequence_match),3),
        ",",table_info->tables[itableidx].columns[iidx].column_name,",",smergedelete,",",
        sdefattind,",",trim(cnvtstring(table_info->tables[itableidx].columns[iidx].exception_flg),3),
        ") as ",table_info->tables[itableidx].columns[iidx].column_name,
        sviewselecttrans)
       SET stablenamestr = concat(stablenamestr,"','",table_info->tables[itableidx].columns[iidx].
        root_entity_name)
       SET sjoin_3 = concat(" AND rdds_diff_translate_src(","'",table_info->tables[itableidx].
        columns[iidx].root_entity_name,"',",trim(cnvtstring(table_info->tables[itableidx].columns[
          iidx].sequence_match),3),
        ",s.",table_info->tables[itableidx].columns[iidx].column_name,",",smergedelete,",",
        sdefattind,",",trim(cnvtstring(table_info->tables[itableidx].columns[iidx].exception_flg),3),
        ") = t.",table_info->tables[itableidx].columns[iidx].column_name,
        sjoin_3)
      ENDIF
     ELSE
 ...
 IF (size(table_info->tables[itableidx].src_filter_function,1) > 0)
 ...
 ENDIF
 IF (ilimitchglog=1)
  SET soradecodetgt = concat("decode(sign(rownum-",trim(cnvtstring(imaxerrors),3),
  ...
  SET soradecodesrc = concat("decode(sign(rownum-",trim(cnvtstring(imaxerrors),3),
 ...
 ELSE
  SET ssql = concat("rdb asis(^create or replace view v_rdds_translate as ",char(10),
   ...
   char(10),"where dmt.env_source_id = ",trim(cnvtstring(isrcenvid),3),char(10),
   "      and dmt.env_target_id = ",
   trim(cnvtstring(itgtenvidmock),3),char(10),"      and dmt.table_name in (",stablenamestr,") ",
   ...
   " sdmt ",
   char(10),"where sdmt.env_source_id = ",trim(cnvtstring(itgtenvidmock),3),char(10),
   "      and sdmt.env_target_id = ",
   trim(cnvtstring(isrcenvid),3),char(10),"      and sdmt.table_name in (",stablenamestr,") ",
  ...
  SET ssql = concat("rdb asis(^create or replace view v_rdds_translate_2 as ",
   ...
   "where dmt.env_source_id = ",trim(cnvtstring(isrcenvid),3),
   " and dmt.env_target_id = ",trim(cnvtstring(itgtenvidmock),3)," and dmt.table_name in (",
   stablenamestr,") ",
   "group by table_name, to_value ","union ",char(10),"select table_name, from_value, max(to_value) ",
   "from dm_merge_translate",
   sdblink," sdmt ","where sdmt.env_source_id = ",trim(cnvtstring(itgtenvidmock),3),
   "      and sdmt.env_target_id = ",
   trim(cnvtstring(isrcenvid),3),"      and sdmt.table_name in (",stablenamestr,") ",
 ...
 ELSE
  SET ssql = concat("rdb asis(^create or replace view v_rdds_tgt_compare as ",char(10),"   select ",
   ...
   spkcolumnname,",-1)),v.from_value) ",char(10),"   from ",table_info->tables[itableidx].table_name,
   " tt,",char(10),"        v_rdds_translate_2 v",char(10),"   where tt.",
   spkcolumnname," = v.to_value (+) ",char(10),"^) go")
 ENDIF
