CREATE PROGRAM ags_exe_pvd_org_load:dba
 CALL echo("***")
 CALL echo("***   BEG AGS_EXE_PVD_ORG_LOAD")
 CALL echo("***")
 ...
 DECLARE sstatus_file_name = vc WITH public, noconstant(concat("ags_exe_pvd_org_load_",format(
 ...
 SET log->qual[log->qual_knt].smsg = "BEG >> AGS_EXE_PVD_ORG_LOAD"
 CALL echo("***")
...
#skip_parse
 ...
 SELECT INTO "nl:"
  FROM dm_info di
  PLAN (di
   WHERE di.info_name="ORG_MIGRATION")
  DETAIL
   found_info = true
  WITH nocounter
 ;end select
 ...
 FOR (exe_idx = 1 TO work->qual_knt)
   ...
   SET ierrcode = error(serrmsg,1)
   SET ierrcode = 0
   EXECUTE ags_pvd_org_load value(current_task_id)
 ENDFOR
#skip_org_load
 SET log->qual_knt = (log->qual_knt+ 1)
 SET stat = alterlist(log->qual,log->qual_knt)
...
#exit_script
 ...
 SET log->qual[log->qual_knt].smsgtype = "INFO"
 SET log->qual[log->qual_knt].dmsg_dt_tm = cnvtdatetime(curdate,curtime3)
 SET log->qual[log->qual_knt].smsg = "END >> AGS_EXE_PVD_ORG_LOAD"
 CALL echo("***")
 CALL echo("***   END LOGGING")
 CALL echo("***")
 CALL handle_logging(sstatus_file_name,sstatus_email,ilog_status)
 CALL echo("***")
 CALL echo("***   END AGS_EXE_PVD_ORG_LOAD")
 CALL echo("***")
 SET script_ver = "001 08/02/06"
END GO
